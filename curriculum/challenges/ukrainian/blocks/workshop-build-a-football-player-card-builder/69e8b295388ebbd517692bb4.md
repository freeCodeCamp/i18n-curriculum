...
Всередині `card` div додайте `div` з `className` `card-header`. Цей розділ міститиме рейтинг гравця, позицію, значок рівня та назву клубу.

# --hints--

Ви повинні додати `div` з `className` `card-header` всередині `div` з `className` `.card`.
