...
Um die Füße des Pinguins noch pinguinartiger wirken zu lassen, drehen Sie den linken Fuß um `80deg` und den rechten um `-80deg`.

# --hints--

Sie sollten `.foot.left` eine `transform` von `rotate(80deg)` geben.
...
Sie sollten `.foot.right` eine `transform` von `rotate(-80deg)` geben.
