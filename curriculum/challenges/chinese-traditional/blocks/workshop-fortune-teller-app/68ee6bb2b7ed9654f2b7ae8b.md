...
為全域 `document` 物件添加另一個事件監聽器，該監聽器會呼叫 `showFortune` 函式。在回呼中，包含一個名為 `e`、型別為 `Event` 的參數，並將 `e` 作為引數傳遞給 `showFortune` 方法。

`showFortune` 將控制卡片點擊事件，例如從 DOM 讀取被點擊的卡片 ID 並顯示相符的卡片運勢詳情。你將在接下來的步驟中創建它。

# --hints--

你應該為全域 `click` 指定一個 `document` 事件監聽器。

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

你的事件監聽器應該有一個名為 `e` 的回呼參數。
