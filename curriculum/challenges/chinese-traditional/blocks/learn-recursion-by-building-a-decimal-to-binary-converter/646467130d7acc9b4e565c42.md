...
將所有 `parseInt(numberInput.value)` 實體取代為 `inputInt`。

# --hints--

你應該將 `isNaN(parseInt(numberInput.value))` 取代為 `isNaN(inputInt)`，並將 `parseInt(numberInput.value) < 0` 取代為 `inputInt < 0`。
...
你應該將 `parseInt(numberInput.value) === 5` 取代為 `inputInt === 5`。
...
你應該將 `decimalToBinary(parseInt(numberInput.value))` 取代為 `decimalToBinary(inputInt)`。
...
你的 `checkUserInput()` 函式（程式）應該只包含一個 `parseInt(numberInput.value)` 實體。
