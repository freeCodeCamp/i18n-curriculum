...
마지막으로, 두 번째 `label` 다음 새 줄에 또 다른 `input` 요소를 만드세요. 이 요소에는 `type` 속성을 `number`로, `min` 속성을 `0`로 설정하세요(음수 칼로리가 추가되지 않도록 하기 위해), `placeholder` 속성을 `Calories`로, 그리고 `id` 속성은 두 번째 `for` 요소의 `label` 속성과 일치하도록 하세요.
...
`input` 요소는 `placeholder` 속성을 `Calories`로 설정해야 합니다.
...
`input` 요소는 `id` 속성을 `${entryDropdown.value}-${entryNumber}-calories`로 설정해야 합니다.
