...
باستخدام محدِّد صنف، أعطِ عنصر `.cat-right-inner-eye` عرضًا مقداره `10px` وارتفاعًا مقداره `20px`. وأيضًا، أعطِه لون خلفية `#fff`.

# --hints--

يجب أن يكون لديك محدِّد `.cat-right-inner-eye`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye'))
```

يجب أن يحتوي محدِّد `.cat-right-inner-eye` على خاصية `width` بقيمة `10px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.width, '10px')
```

يجب أن يحتوي محدِّد `.cat-right-inner-eye` على خاصية `height` بقيمة `20px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.height, '20px')
```

يجب أن يحتوي محدِّد `.cat-right-inner-eye` على خاصية `background-color` بقيمة `#fff`.
