...
Crie uma constante chamada `linkText` e atribua a ela o valor de `selectedCategory.category`.Isto exibirá o nome da categoria no elemento anchor.

# --hints--

Você deve ter uma constante chamada `linkText`.

```js
assert(code.match(/const\s+linkText\s*=/));
```

Você deve atribuir `selectedCategory.category` à sua variável `linkText`.
