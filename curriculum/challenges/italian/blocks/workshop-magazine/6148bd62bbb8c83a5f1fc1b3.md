...
Crea un selettore `.hero-title` e dagli una proprietà `text-align` impostata su `center`, una proprietà `color` impostata su `orangered` e una proprietà `font-size` impostata su `8rem`.

# --hints--

Dovresti avere un selettore `.hero-title`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

Il tuo selettore `.hero-title` dovrebbe avere una proprietà `text-align` impostata su `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign, 'center');
```

Il tuo selettore `.hero-title` dovrebbe avere una proprietà `color` impostata su `orangered`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color, 'orangered');
```

Il tuo selettore `.hero-title` dovrebbe avere una proprietà `font-size` impostata su `8rem`.
