...
如果你稍微玩一下你的播放器，你会注意到一些错误。首先，当你播放一首歌，暂停它，然后按上一首按钮或下一首按钮时，歌曲不会从头开始播放。

为你的 `playSong` 函数添加名为 `start` 的第二个参数。将其默认值设为 `true`。

# --hints--

你的 `playSong` 函数应有第二个名为 `start` 的参数，默认值为 `true`。
