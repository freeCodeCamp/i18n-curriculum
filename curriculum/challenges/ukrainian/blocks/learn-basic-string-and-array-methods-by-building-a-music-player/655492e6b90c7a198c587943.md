...
Тепер потрібно додати атрибут назад до пісні, яка зараз відтворюється.

Створіть інструкцію `if` з умовою `songToHighlight`. Для інструкції використайте `setAttribute` на `songToHighlight`, щоб передати `"aria-current"` і `"true"` як перший і другий аргументи.

# --hints--

Ви повинні створити інструкцію `if` з умовою `songToHighlight`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

Ви повинні використати метод `setAttribute()` на `songToHighlight` всередині вашої інструкції `if`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

Ви повинні передати `"aria-current"` і `"true"` як перший і другий аргументи вашого методу `setAttribute`.
