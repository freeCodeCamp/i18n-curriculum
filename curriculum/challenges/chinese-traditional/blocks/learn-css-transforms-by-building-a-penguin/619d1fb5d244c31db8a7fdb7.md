...
選擇 `.foot` 元素，並將其 `width` 設置爲 `15%`、`height` 設置爲 `30%`、`background-color` 設置爲 `orange`。
...
你應該將 `.foot` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.width, '15%');
```

你應該將 `.foot` 的 `height` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.height, '30%');
```

你應該將 `.foot` 的 `background-color` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
