...
Adicione um elemento anchor com o texto `Read our terms and conditions` antes do `label` recém-adicionado. Defina o `href` para:

```md
https://www.freecodecamp.org/news/terms-of-service/
```

Então adicione o texto `I accept the terms and conditions` imediatamente após o elemento de entrada no label.

# --hints--

Você deve ter um elemento `a` logo antes do `label` com o texto `Read our terms and conditions`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.innerText.trim().replaceAll(/\s+/g, ' '), /Read our terms and conditions\.?/i);
```

O elemento `a` deve ter um `href` de `https://www.freecodecamp.org/news/terms-of-service/`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.href, /https:\/\/www\.freecodecamp\.org\/news\/terms-of-service\/?/);
```

Você deve ter um elemento `input` dentro do `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) + a + label > input'));
```

Dentro do `label`, imediatamente após o elemento `input` você deve ter o texto `I accept the terms and conditions`.
