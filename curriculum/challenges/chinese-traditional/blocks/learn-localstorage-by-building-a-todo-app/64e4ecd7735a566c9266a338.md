...
現在你已經完成開啟和關閉 modal，接著是從輸入欄位取得值，將它們存入 `taskData` 陣列，並顯示在頁面上。

首先，為你的 `taskForm` 元素添加一個 `submit` 事件監聽器，並將 `e` 作為箭頭函式的參數傳入。在大括號內，使用 `preventDefault()` 方法來阻止瀏覽器在提交表單後重新整理頁面。

# --hints--

你應該在你的 `taskForm` 變數上呼叫 `addEventListener()` 方法。
...
你應該使用箭頭語法將你的事件監聽器設為帶有 `e` 參數的空花括號對組。

```js
assert.match(code, /taskForm\.addEventListener\(\s*('|"|`)submit\1\s*,\s*(\(\s*e\s*\)|e)\s*=>\s*\{/)
```

你應該使用 `e.preventDefault()` 方法來阻止瀏覽器重新載入頁面。
