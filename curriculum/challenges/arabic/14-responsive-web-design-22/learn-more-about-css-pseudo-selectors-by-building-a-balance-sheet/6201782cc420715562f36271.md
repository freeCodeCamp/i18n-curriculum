...
أعطِي المنتقي الجديد خطًا `bold`، وعرض `4.5rem`، والنص متحاذي (aligned) لليمين.

# --hints--

يجب أن يكون لديك منتقي `#years span[class]`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]'));
```

يجب يحتوي منتقي `#years span[class]` الخاص بك على خاصية `font-weight` بقيمة `bold`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]')?.getPropertyValue('font-weight') === 'bold');
```

يجب يحتوي منتقي `#years span[class]` الخاص بك على خاصية `width` بقيمة `4.5rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]')?.getPropertyValue('width') === '4.5rem');
```

يجب يحتوي منتقي `#years span[class]` الخاص بك على خاصية `text-align` بقيمة `right`.
