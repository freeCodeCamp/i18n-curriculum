...
Mstatili uliotolewa umeundwa kwa mraba *m* × *n*. Ikiwa *m* na *n* si zote mbili ni nambari zisizo za mnyoofu, basi inawezekana kukata njia kupitia mstatili kando ya mipaka ya mraba ili mstatili ugawanyike vipande viwili vinavyounganishwa vyenye umbo sawa (baada ya kugeuza kipande kimoja kwa digrii 180°). Njia zote kama hizi kwa mistari ya 2 × 2 na 4 × 3 zinaonyeshwa hapa chini.
...
<div style="width: 100%; text-align: center;">
  <svg xmlns="https://www.w3.org/2000/svg" xmlns:xlink="https://www.w3.org/1999/xlink" width="520" height="170" aria-hidden="true" alt="Diagram showing the possible paths for 2 by 2 and 4 by 3 rectangles">
    <style>
.diagram-g { haijajazwa; mstari: #ccc }
      .diagram-s, .diagram-s2 { jaza: #bff; mstari: nyeusi; uwazi-wa-jaza: .4 }
      .diagram-s2 { jaza: #fbf }
      .diagram-d { mstari:nyeusi; haijajazwa}
    </style>
    <defs>
      <g id="diagram-m">
        <g id="diagram-h4">
...
Andika kitendakazi kinachohesabu idadi ya njia tofauti za kukata mstatili wa *m* × *n*.

# --hints--

`cutRectangle` inapaswa kuwa kitendakazi.

```js
assert(typeof cutRectangle == 'function');
```

`cutRectangle(2, 2)` inapaswa kurudisha nambari.

```js
assert(typeof cutRectangle(2, 2) == 'number');
```

`cutRectangle(2, 2)` inapaswa kurudisha `2`.

```js
assert.equal(cutRectangle(2, 2), 2);
```

`cutRectangle(4, 3)` inapaswa kurudisha `9`.

```js
assert.equal(cutRectangle(4, 3), 9);
```

`cutRectangle(4, 4)` inapaswa kurudisha `22`.

```js
assert.equal(cutRectangle(4, 4), 22);
```

`cutRectangle(8, 3)` inapaswa kurudisha `53`.

```js
assert.equal(cutRectangle(8, 3), 53);
```

`cutRectangle(7, 4)` inapaswa kurudisha `151`.
