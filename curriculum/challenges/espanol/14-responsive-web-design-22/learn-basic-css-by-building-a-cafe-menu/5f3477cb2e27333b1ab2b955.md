...
Ahora tienes que vincular el archivo `styles.css` para que los estilos se apliquen de nuevo. Dentro del elemento `head`, añade un elemento `link`. Dale un atributo `rel` con el valor `"stylesheet"` y un atributo `href` con un valor de `"styles.css"`.
