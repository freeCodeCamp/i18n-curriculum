...
现在，当玩家跳起时，有可能超越画布的高度。

为了解决这个问题，你需要添加一个条件来阻止玩家跌落到画布的高度以外。

创建一个空的 `if` 语句，检查玩家的 `y` 位置、高度和 `y` 速度的总和是否小于或等于画布的高度。

# --hints--

你的 `update()` 方法应该有一个 `if` 语句。
...
`if` 语句的条件应该检查玩家的 `y` 位置、高度和 `y` 速度的总和是否小于或等于画布的高度。
