...
Нарешті! Ви дісталися до останньої будівлі! Додайте до неї повторюваний градієнт із напрямком `90deg`. Використайте колір будівлі від `0%` до `10%` і `transparent` від `10%` до `30%`.

# --hints--

Ви повинні додати `repeating-linear-gradient` до `.fb6` у властивість `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

Ви повинні задати `repeating-linear-gradient` напрямок `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Ви повинні задати `repeating-linear-gradient` перший колір `--building-color3` від `0%` до `10%`.
...
Ви повинні задати `repeating-linear-gradient` другий колір `transparent` від `10%` до `30%`.
