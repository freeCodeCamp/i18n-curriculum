...
المثال أعلاه يختار عناصر `input` التي تحتوي على خاصية `name` بقيمة `password`.

الآن، استخدم محدِّد الخاصية لتنسيق زر الإرسال بحيث تكون قيمة `display` هي `block`، و`width` هي `60%`.
...
يجب أن تعطي زر الإرسال قيمة `width` تساوي `60%`.
