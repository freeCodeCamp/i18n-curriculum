...
Ongeza nafasi kati ya vipengele vya `.inline` na maandishi ya `label`, kwa kuwapa `margin` ya kulia ya `0.5em`. Pia, weka ukingo mwingine wote kuwa `0`.

# --hints--

Unapaswa kuwapa vipengele vya `.inline` `margin-top` ya `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginTop, '0px');
```

Unapaswa kuwapa vipengele vya `.inline` `margin-right` ya `0.5em`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginRight, '0.5em');
```

Unapaswa kuwapa vipengele vya `.inline` `margin-bottom` ya `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginBottom, '0px');
```

Unapaswa kuwapa vipengele vya `.inline` `margin-left` ya `0`.
