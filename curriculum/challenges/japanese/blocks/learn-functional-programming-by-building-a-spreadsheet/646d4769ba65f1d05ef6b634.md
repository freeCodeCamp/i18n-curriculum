...
これで `update()` 関数は実際に数式を評価できるようになりました。関数が呼び出されたかどうかを確認するために `if` 条件を書いたことを覚えていますか。

`if` 文の中で、`value` の `element` を `evalFormula()` 関数の結果に設定してください。まだ引数は渡さないでください。

# --hints--

`value` ブロック内の `element` の `if` プロパティを更新する必要があります。
...
`value` 関数を呼び出した結果を `evalFormula()` プロパティに代入してください。
...
`evalFormula()` の呼び出しに引数を渡さないでください。
