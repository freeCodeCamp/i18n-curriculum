...
Kwa sababu `elemValue` inarudisha kitendakazi, kitendakazi chako cha `addCharacters` hatimaye hurudisha safu ya rejea za kitendakazi. Unataka njia ya `.map()` ifanye kitendakazi cha ndani cha kitendakazi chako cha `elemValue`, ambayo inamaanisha unahitaji kuita `elemValue` badala ya kuirejea. Pitia `num` kama hoja kwa kitendakazi chako cha `elemValue`.

<!-- TODO: Explain further? -->

# --hints--

Unapaswa kuita `elemValue()` katika njia yako ya `.map()`.
...
Unapaswa kupitisha `num` kwa wito wako wa `elemValue()`.
