...
إرسال النموذج مع اختيار خيار لن يرسل قيمة مفيدة إلى الخادم. لذلك، يحتاج كل عنصر `option` إلى خاصية `value`. بدون هذه الخاصية، يُرسل محتوى النص الخاص بـ `option` إلى الخادم.

امنح أول `option` خاصية `value` بقيمة `""`، وامنح عناصر `option` التالية خاصيات `value` من `1` إلى `4`.

# --hints--

يجب أن تعطي أول `option` خاصية `value` بقيمة `""`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

يجب أن تعطي ثاني `option` خاصية `value` بقيمة `1`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

يجب أن تعطي ثالث `option` خاصية `value` بقيمة `2`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

يجب أن تعطي رابع `option` خاصية `value` بقيمة `3`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

يجب أن تعطي خامس `option` خاصية `value` بقيمة `4`.
