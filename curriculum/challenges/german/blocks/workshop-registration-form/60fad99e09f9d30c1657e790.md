...
Zentrieren Sie das `form`-Element, indem Sie ihm eine `margin` von `0 auto` geben. Fixieren Sie dann seine Größe auf eine maximale Breite von `500px` und eine minimale Breite von `300px`. Innerhalb dieses Bereichs soll es eine `width` von `60vw` haben dürfen.

# --hints--

Sie sollten einen `form`-Selektor verwenden, um das `form`-Element zu stylen.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('form'));
```

Sie sollten dem `form` eine `margin` von `0 auto` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.margin, '0px auto');
```

Sie sollten dem `form` eine `max-width` von `500px` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.maxWidth, '500px');
```

Sie sollten dem `form` eine `min-width` von `300px` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.minWidth, '300px');
```

Sie sollten dem `form` eine `width` von `60vw` geben.
