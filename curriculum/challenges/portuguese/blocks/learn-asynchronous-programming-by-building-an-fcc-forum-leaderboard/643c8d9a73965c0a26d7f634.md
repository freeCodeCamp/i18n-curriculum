...
O objeto `topic_list` contém um array `topics` que contém os tópicos mais recentes postados no fórum.

Desestruture o array `topics` do objeto `topic_list`.

# --hints--

Você deve usar `const` para desestruturar a variável `topics` do objeto `topic_list`.
