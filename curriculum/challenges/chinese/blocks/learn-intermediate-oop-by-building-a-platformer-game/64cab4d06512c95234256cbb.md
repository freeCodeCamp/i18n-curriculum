...
接下来，添加一个 `if` 语句，检查 `collisionDetectionRules` 数组中的每个规则是否为真。 确保为此使用 `every` 方法。

在 `if` 语句的主体内，将数字 0 分配给玩家的 `y` 速度，然后跟上 `return` 语句。

# --hints--

你应该有一个 if 语句，使用 `every` 方法来检查 `collisionDetectionRules` 数组中的每个规则是否为真。

```js
assert.match(code, /if\s*\(\s*collisionDetectionRules\.every\(\s*(?:\(\s*(.+)\s*\)\s*=>\s*\1|([^\s()]+)\s*=>\s*\2)\s*\)\s*\)\s*\{\s*/);
```

你应该将数字 `0` 分配给玩家的 `y` 速度，然后在 `if` 语句主体内添加 `return` 语句。
