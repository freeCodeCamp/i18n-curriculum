...
Katika kipengele chako cha tatu cha `tr`, ongeza kipengele cha `th` chenye maandishi ya `Savings Funds set aside for emergencies.`. Zungusha maandishi hayo, isipokuwa `Savings `, katika kipengele cha `span` chenye sifa ya kipengele ya `class` iliyowekwa kuwa `description`.

Baada ya hapo, ongeza vipengele vitatu vya `td` vyenye maandishi yafuatayo (kwa mpangilio): `$500`, `$650`, `$728`. Toa kipengele cha tatu cha `td` sifa ya kipengele ya `class` iliyowekwa kuwa `current`.
...
Kipengele chako cha kwanza cha `td` kinapaswa kuwa na maandishi ya `$500`.
