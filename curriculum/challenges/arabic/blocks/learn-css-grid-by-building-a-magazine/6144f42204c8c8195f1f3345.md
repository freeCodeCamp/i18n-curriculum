...
لإضافة مساحة بين الصفوف في تخطيط الشبكة، يمكنك استخدام خاصية `row-gap`. أعطِ المحدِّد `main` خاصية `row-gap` بقيمة `3rem`.

# --hints--

يجب أن يحتوي المحدِّد `main` على خاصية `row-gap` بقيمة `3rem`.
