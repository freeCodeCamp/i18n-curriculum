...
带有类 `div` 的 `card-wrapper` 元素创建了卡片的外部边框和裁剪形状。

现在直接在 `div` 内添加带有 `card-wrapper` 为 `className` 的内部 `card`。这会为卡片提供深色背景。

# --hints--

你应该在 `div` 元素内添加一个带有 `className` 为 `card` 的 `.card-wrapper`。
