...
Setze die `animation`-Eigenschaft der `.cabin`-Regel auf `cabins 10s linear infinite`. Dadurch werden die `animation-name`, `animation-duration`, `animation-timing-function` und `animation-iteration-count`-Eigenschaft in dieser Reihenfolge gesetzt.

# --hints--

Dein `.cabin`-Selektor sollte eine `animation`-Eigenschaft auf `cabins 10s linear infinite` gesetzt haben.
