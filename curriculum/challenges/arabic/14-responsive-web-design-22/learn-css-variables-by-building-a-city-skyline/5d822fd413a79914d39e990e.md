...
يجب عليك إعطاء `.bb4-window` خاصية `width` بقيمة `18%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

يجب عليك إعطاء `.bb4-window` خاصية `height` بقيمة `90%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

يجب عليك إعطاء `.bb4-window` خاصية `background-color` بقيمة `--window-color4`.
