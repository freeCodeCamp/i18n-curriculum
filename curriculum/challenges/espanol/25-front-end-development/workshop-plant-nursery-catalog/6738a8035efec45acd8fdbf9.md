...
Como puedes ver, ahora el conjunto está completamente vacío. Retira tanto las llamadas a `clear` y `has` de tu código.

# --hints--

No deberías tener `plantsSet.clear()` en tu código.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

No deberías tener `plantsSet.has(ballerina)` en tu código.
