...
العودة إلى تصميم الصفحة. حدد عناصر القائمة (list) داخل شريط التنقل (navigation bar)، وأعطيه الأساليب (styles) التالية:
...
يجب عليك استخدام منتقي `nav li` او `nav > ul > li`.
...
يجب عليك إعطاء عناصر `li` خاصية `color` بقيمة `#dfdfe2`.
...
يجب عليك إعطاء عناصر `li` خاصية `margin` بقيمة `0 0.2rem`.
...
يجب عليك إعطاء عناصر `li` خاصية `padding` بقيمة `0.2rem`.
...
يجب عليك إعطاء عناصر `li` خاصية `display` بقيمة `block`.
