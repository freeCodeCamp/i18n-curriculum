...
Você deve ter uma declaração `if` no final da sua função `validate`.
...
Sua declaração `if` deve ser executada quando `is_invalid` for `True`.
...
Você deve retornar `False` dentro do seu comando `if`.
