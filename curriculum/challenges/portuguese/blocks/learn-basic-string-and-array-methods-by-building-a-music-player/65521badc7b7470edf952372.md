...
Defina uma função `pauseSong` usando a palavra-chave `const` e a sintaxe de função arrow. A função não deve receber parâmetros.

# --hints--

Você deve usar `const` para criar uma função vazia chamada `pauseSong`.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` deve ser uma função.

```js
assert.isFunction(pauseSong)
```

Sua função `pauseSong` deve usar uma sintaxe de arrow.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Sua função `pauseSong` deve estar vazia.
