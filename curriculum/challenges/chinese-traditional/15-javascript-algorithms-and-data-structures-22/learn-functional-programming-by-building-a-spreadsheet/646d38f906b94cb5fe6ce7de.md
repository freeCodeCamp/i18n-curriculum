...
你的 `addCharacters(char1)` 也返回一個函數，該函數返回另一個函數。 你需要進行另一個函數調用來訪問 `.map()` 回調的最內層函數引用。 JavaScript 允許你立即調用返回的函數：

```js
myFunc(1)("hi");
```

立即調用從 `addCharacters(char1)` 調用返回的函數，並將 `char2` 作爲參數傳遞。

# --hints--

你應該將函數調用鏈接到 `addCharacters(char1)` 調用。
