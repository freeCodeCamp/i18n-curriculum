...
Hai imparato in una lezione precedente che la parola chiave `this` si riferisce all'istanza corrente che viene costruita, come hai visto nel costruttore `Dessert`. Qui applicherai la stessa idea alla classe `ShoppingCart`.
...
Nel tuo costruttore, usa la parola chiave `this` per impostare la proprietà `items` su un array vuoto. Inoltre, imposta la proprietà `total` su `0` e la proprietà `taxRate` su `8.25`.
