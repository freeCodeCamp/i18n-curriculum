...
Deine `calculate`-Funktion sollte eine `standardDeviation`-Variable haben, die auf das Ergebnis von `getStandardDeviation(numbers)` gesetzt ist.

```js
assert.match(calculate.toString(), /standardDeviation\s*=\s*getStandardDeviation\(\s*numbers\s*\)/);
```

Du solltest den `textContent` des `#standardDeviation`-Elements so aktualisieren, dass er die `standardDeviation`-Variable ergibt.
