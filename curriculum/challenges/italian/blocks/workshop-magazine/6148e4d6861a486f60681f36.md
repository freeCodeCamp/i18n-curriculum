...
Dai al selettore `.image-wrapper` la proprietà `grid-template-columns` impostata su `2fr 1fr` e la proprietà `grid-template-rows` impostata su `repeat(3, min-content)`. In questo modo, la griglia avrà delle righe che si regolano in altezza in base al contenuto, ma le colonne avranno una larghezza fissa basata sul contenitore.

# --hints--

Il selettore `.image-wrapper` dovrebbe avere una proprietà `grid-template-columns` impostata su `2fr 1fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

Il selettore `.image-wrapper` dovrebbe avere una proprietà `grid-template-rows` impostata su `repeat(3, min-content)`.
