...
Wechseln Sie zum Listenelement und fügen Sie die Klassen `mt-6`, `space-y-2`, `text-sm` und `text-fuchsia-100` zum `ul` hinzu, um die Listenelemente auseinanderzurücken und optisch ansprechender zu gestalten.

# --hints--

Ihr `ul` Element sollte die Klasse `mt-6` haben.
...
Ihr `ul` Element sollte die Klasse `space-y-2` haben.
...
Ihr `ul` Element sollte die Klasse `text-sm` haben.
...
Ihr `ul`-Element sollte die Klasse `text-fuchsia-100` haben.
