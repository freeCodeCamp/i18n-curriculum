...
Тепер створіть тег зображення та надайте йому `class` `"user-img"`. Use string interpolation to set the `src` attribute to the `image` you destructured earlier. Встановіть атрибут `alt` на `author`, а слідом текст `"avatar"`. Переконайтеся, що між змінною `author` та словом `"avatar"` є пробіл, наприклад, `"Quincy Larson avatar"`.
