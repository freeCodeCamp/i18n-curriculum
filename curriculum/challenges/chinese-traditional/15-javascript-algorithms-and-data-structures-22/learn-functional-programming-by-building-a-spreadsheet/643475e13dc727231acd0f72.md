...
現在在嵌套的 `.forEach()` 調用中，聲明一個 `input` 變量。 使用 `document` 對象的 `.createElement()` 方法創建 `input` 元素。 將 `type` 屬性設置爲 `"text"`，將 `id` 屬性設置爲 `letter + number`。
