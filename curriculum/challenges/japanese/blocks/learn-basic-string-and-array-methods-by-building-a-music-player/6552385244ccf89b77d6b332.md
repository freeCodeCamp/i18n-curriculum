...
次の曲と前の曲を再生する処理に取り組む必要があります。そのために、`playNextSong` と `playPreviousSong` 関数が必要です。

`const` とアロー構文を使って、空の `playNextSong` 関数を作成してください。

# --hints--

空の関数 `const` を作成するには `playNextSong` を使うべきです。

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` は関数である必要があります。

```js
assert.isFunction(playNextSong)
```

`playNextSong` 関数はアロー構文を使うべきです。

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

`playNextSong` 関数は空であるべきです。
