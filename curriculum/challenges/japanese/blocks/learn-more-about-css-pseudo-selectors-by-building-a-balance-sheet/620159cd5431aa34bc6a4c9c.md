...
フレックスコンテナを`h1 .flex`セレクターで指定してください。フレックスボックスレイアウトを有効にするために、`display`プロパティを`flex`に設定します。次に、ネストされた要素を下から上に表示するために、`flex-direction`プロパティを`column-reverse`に設定してください。最後に、要素間にスペースを作るために、`gap`プロパティを`1rem`に設定します。
