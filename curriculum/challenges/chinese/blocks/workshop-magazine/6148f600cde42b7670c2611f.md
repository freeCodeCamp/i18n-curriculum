...
为 `only screen` 创建另一个 `@media` 查询，其中 `max-width` 为 `600px`。 在其中，创建一个 `.text-with-images` 规则，并为其设置一个值为 `1fr` 的 `grid-template-columns` 属性。

这会将你的底部文本区域在较小屏幕上折叠成单列。

# --hints--

你应该为 `only screen and (max-width: 600px)` 创建一个新的 `@media` 查询。 这应该在你之前的 `@media` 查询下面。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText, 'only screen and (max-width: 600px)');
```

你的新 `@media` 查询应该有一个 `.text-with-images` 选择器。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText, '.text-with-images');
```

你的新 `.text-with-images` 选择器应该有一个 `grid-template-columns` 属性，其值为 `1fr`。
