...
我們不希望玩家的唯一武器被損壞。 邏輯“與”運算符檢查兩個語句是否爲真。

使用 <dfn>邏輯 AND</dfn> 運算符 `&&` 向 `if` 語句添加第二個條件。 只有當 `inventory.length` 不等於 (`!==`) 一時，玩家的武器纔會損壞。
...
你應該使用邏輯 AND 運算符來檢查 `inventory.length` 是否不等於 `1`。
