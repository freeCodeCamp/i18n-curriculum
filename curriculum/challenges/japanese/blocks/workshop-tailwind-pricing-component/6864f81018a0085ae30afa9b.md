...
リスト項目の先頭にある各`span`のチェックマークは装飾用です。これらの`aria-hidden`要素に`span`を追加して、スクリーンリーダーから非表示にしてください。

また、各`text-green-700`要素に`mr-2`と`span`のクラスを代入してください。

# --hints--

あなたの2つの`span`要素には`aria-hidden`属性を付ける必要があります。
...
あなたの2つの`span`要素には`text-green-700`クラスを付ける必要があります。
...
あなたの2つの`span`要素には`mr-2`クラスを付ける必要があります。
