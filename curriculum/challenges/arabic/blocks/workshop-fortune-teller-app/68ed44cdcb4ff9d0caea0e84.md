...
بما أنك تستدعي `fetchCardsData` و `initializeEventListeners` داخل دالة البناء، يجب عليك الآن تعريفهما داخل الصنف. أنشئهما كطريقتين `private`. تأكد من أن `fetchCardsData` غير متزامنة لأنها ستتعامل مع استدعاءات API.

# --hints--

يجب أن يكون لديك طريقة `private` باسم `fetchCardsData`.
...
يجب أن تكون طريقتك `fetchCardsData` `async`.
...
يجب أن يكون لديك طريقة `private` باسم `initializeEventListeners`.
