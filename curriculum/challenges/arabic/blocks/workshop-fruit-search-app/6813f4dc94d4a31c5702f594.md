...
الآن بعد أن حصلت على البيانات من واجهة برمجة التطبيقات (API)، حدّث حالة `results` باستدعاء `setResults` وتمرير مصفوفة تحتوي فقط على خاصية `name` من كل ثمرة في مصفوفة `data`. استخدم الطريقة `map()` لهذا الغرض.

# --hints--

يجب تحديث `setResults` باستخدام `data.map(fruit => fruit.name)`.
