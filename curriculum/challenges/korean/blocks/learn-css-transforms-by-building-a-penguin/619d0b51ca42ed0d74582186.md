...
`.eye` 요소를 선택하고 `width`의 `15%`, `height`의 `17%`, `background-color`의 `black`를 지정하세요.
...
`.eye`에 `width`인 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾았습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.width, '15%');
```

`.eye`에 `height`인 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾았습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.height, '17%');
```

`.eye`에 `background-color`인 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾았습니다.
