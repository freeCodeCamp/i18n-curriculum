...
最後の `fieldset` 要素のボーダーが少し違和感があります。

前のレッスンでは、`last-of-type` CSS 疑似クラスを使ってこのように操作する方法を学びました。
...
最後の `fieldset` 要素をターゲットにする新しいセレクターを作成し、その `border-bottom` を `none` に設定してください。
