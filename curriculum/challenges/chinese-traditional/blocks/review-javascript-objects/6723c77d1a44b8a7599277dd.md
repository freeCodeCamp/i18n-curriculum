...
- **`Object.hasOwn()` 方法**：這是檢查物件是否擁有某個屬性（非繼承）的現代且推薦方式。語法為 `Object.hasOwn(object, propertyName)`。如果該屬性存在於物件上，則傳回 `true`，若不存在則傳回 `false` — 不論該屬性的值為何。這使得它比 `hasOwnProperty()` 更安全，且在值可能為 `0`、`false`、`null` 或 `undefined` 時，比 `if (obj.prop)` 更可靠。
