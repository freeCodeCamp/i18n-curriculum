...
不用擔心底部的空間，當你為建築頂部的元素添加一些高度時，所有東西都會往下移動。

為你的 `.fb1c` 添加一個 `repeating-linear-gradient`，角度為 `90deg`，`--building-color4` 從 `0%` 到 `10%`，`transparent` 從 `10%` 到 `15%`。

# --hints--

你應該給 `.fb1c` 一個帶有 `repeating-linear-gradient` 的 `background`。
...
你應該使用第一個顏色 `--building-color4` 從 `0%` 到 `10%`。
...
你應該使用第二種 `transparent` 顏色，從 `10%` 到 `15%`。
