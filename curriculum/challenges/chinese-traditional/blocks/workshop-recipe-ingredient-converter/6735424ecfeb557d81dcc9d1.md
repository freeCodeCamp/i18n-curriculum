...
在此步驟中，為你的 `recipeForm` 添加一個 `submit` 事件監聽器，當點擊 `Convert` 按鈕時，會呼叫 `updateResultsList()` 函式（程式）。

為你的 `recipeForm` 附加一個 `submit` 事件監聽器。當表單被提交時，呼叫 `event.preventDefault()` 以停止預設行為，然後呼叫 `updateResultsList()`。

這將允許你透過填寫表單並按下 `Convert`，從 UI 觸發函式，對於在下一步新增轉換邏輯時進行偵錯非常有用。

# --hints--

讓點擊 `Convert` 按鈕透過為 `recipeForm` 添加一個 `"submit"` 事件監聽器來執行 `updateResultsList()`，該監聽器會呼叫 `event.preventDefault()` 以阻止表單的預設行為。
