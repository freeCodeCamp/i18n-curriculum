...
Щоб зробити застосунок більш доступним, важливо, щоб кнопка відтворення описувала поточну пісню або першу пісню у плейлисті.

Почніть зі створення пустої стрілкової функції з назвою `setPlayButtonAccessibleText`.

# --hints--

Ви повинні використати `const` для створення пустої функції з назвою `setPlayButtonAccessibleText`.
...
Ваша функція `setPlayButtonAccessibleText` має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*/)
```

Ваша функція `setPlayButtonAccessibleText` має бути пустою.
