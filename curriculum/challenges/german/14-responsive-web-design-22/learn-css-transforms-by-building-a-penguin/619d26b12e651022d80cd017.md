...
Schau dir die `.arm` Elemente an, und gib ihnen eine `width` von `30%`, eine `height` von `60%`, und eine `background-color` des linear gradient bei `90deg` im Uhrzeigersinn, beginnend bei `gray`, und endend bei `rgb(209, 210, 199)`.

# --hints--

Du solltest den `.arm`-Selektor verwenden.

```js
assert.match(code, /\.arm\s*\{/);
```

Du solltest `.arm` eine `width` mit dem Wert `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

Du solltest `.arm` eine `height` mit dem Wert `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.
