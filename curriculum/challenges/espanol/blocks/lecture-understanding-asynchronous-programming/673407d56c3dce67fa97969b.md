...
`async`/`await`, construido sobre promesas, facilita escribir y leer código asincrónico. Cuando pones la palabra clave `async` antes de una función, significa que esa función siempre devolverá una `Promise`. La palabra clave `await`, que te permite esperar a que se resuelva una `Promise` antes de pasar a la siguiente línea de código, solo puede usarse dentro de funciones asincrónicas o en los cuerpos de nivel superior de los módulos. Aquí tienes un ejemplo para ilustrar cómo funciona `async`/`await`:
...
Cuando llamamos a esta función, verás que `First Printed Message!` aparece antes del saludo. Esto se debe a que la función es asincrónica, no bloquea el resto del código para que se ejecute.
