...
`% Daily Value *` 和 `Saturated Fat 1g 5%` 元素下的底部邊框沒有擴展至標籤的整個寬度。 將 `no-divider` 添加到這兩個元素的 `class` 中。

# --hints--

你的文本爲 `% Daily Value *` 的 `p` 元素應該將 `no-divider` 添加到 `class` 屬性。 不要刪除現有的類。
...
你的文本爲 `Saturated Fat 1g 5%` 的 `p` 元素應該將 `no-divider` 添加到 `class` 屬性。 不要刪除現有的類。
