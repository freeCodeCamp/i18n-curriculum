...
在开始模拟之前，你需要一个对象来跟踪每个门在所有时钟周期中处理的参与者数量。调用 `initializeThroughput()` 并传入 `gates`，将结果赋值给名为 `throughputSummary` 的变量。

# --hints--

你应该调用 `initializeThroughput()` 并传入 `gates`，将结果赋值给名为 `throughputSummary` 的变量。
...
你的 `simulateFestival` 函数应该调用 `initializeThroughput()` 函数并传入 `gates`。
