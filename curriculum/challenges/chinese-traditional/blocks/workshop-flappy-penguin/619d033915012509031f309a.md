...
在 `.face.right` 元素下方，添加一個具有 `class` 為 `chin` 的 `div` 元素。

# --hints--

你應該在 `.penguin-head` 中添加一個 `div` 元素。預期有 --fcc-expected-- `.penguin-head > div` 元素，但實際找到 --fcc-actual--。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 3);
```

你應該給 `div` 一個 `class` 為 `chin`。

```js
assert.exists(document.querySelector('.penguin-head > div.chin'));
```

你應該將 `div` 元素放在 `.face.right` 元素下方。
