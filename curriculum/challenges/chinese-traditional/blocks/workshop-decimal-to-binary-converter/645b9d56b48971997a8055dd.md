...
在 `if` 述語的主體中，將 `result` 的 `innerText` 屬性設定為字串 `"0"`。然後，使用早期的 `return` 述語以提前跳出函式。

# --hints--

你應該將 `result` 的 `innerText` 屬性設為字串 `"0"`。

```js
assert.match(String(decimalToBinary), /if\s*\(\s*input\s*===?\s*0\s*\)\s*\{\s*result\.innerText\s*=\s*('|"|`)\s*0\s*\1/);
```

在設定 `result` 元素的 `innerText` 之後，你應該使用早期的 `return` 述語來提前跳出函式。
