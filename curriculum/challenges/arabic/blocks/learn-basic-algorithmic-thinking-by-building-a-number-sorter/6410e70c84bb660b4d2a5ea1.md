...
لتحقيق نتيجة "الصعود الفقاعي"، يجب التحقق مما إذا كان العنصر الحالي أكبر من العنصر التالي. يمكنك فعل ذلك من خلال الوصول إلى `array` عند `j` و `j+1`.

أنشئ شرط `if` يتحقق مما إذا كان العنصر الحالي أكبر من العنصر التالي.

# --hints--

لا يجب إزالة جملة `console.log` الخاصة بك.
...
يجب استخدام جملة `if` داخل الحلقة المسكنة الخاصة بك.
...
يجب أن تتحقق جملة `if` الخاصة بك مما إذا كانت `array[j]` أكبر من `array[j+1]`.
...
يجب أن تكون جملة `if` الخاصة بك فارغة.
