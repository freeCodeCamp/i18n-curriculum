...
Es hora de posicionar las cabinas alrededor de la rueda. Selecciona el primer elemento de `.cabin`. Set the `right` property to `-8.5%` and the `top` property to `50%`.

# --hints--

Debes tener un selector `.cabin:nth-of-type(1)`.
...
Tu selector `.cabin:nth-of-type(1)` debe tener una propiedad `right` establecida en `-8.5%`.
...
Tu selector `.cabin:nth-of-type(1)` debe tener una propiedad `top` establecida en `50%`.
