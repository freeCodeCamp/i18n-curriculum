...
`hasOwnProperty` 方法定義在 `Object.prototype` 中，`Bird.prototype` 可以存取它，而 `duck` 又可以存取它。這是 `prototype` 串鏈的範例。在這個 `prototype` 串鏈中，`Bird` 是 `duck` 的 `supertype`，而 `duck` 是 `subtype`。`Object` 是 `Bird` 和 `duck` 兩者的 `supertype`。`Object` 是 JavaScript 中所有物件的 `supertype`。因此，任何物件都可以使用 `hasOwnProperty` 方法。
