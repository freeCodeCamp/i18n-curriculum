...
`@media` のために、`only screen` の `max-width` を使った別の `600px` クエリを作成してください。その中に `.text-with-images` ルールを作成し、`grid-template-columns` の値を持つ `1fr` プロパティを与えてください。

これにより、小さい画面で下部のテキストエリアが1列に折りたたまれます。

# --hints--

`@media` のために新しい `only screen and (max-width: 600px)` クエリを作成してください。これは前の `@media` クエリの下に置く必要があります。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

新しい `@media` クエリには `.text-with-images` セレクターを使うべきです。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

新しい `.text-with-images` セレクターには、値が `grid-template-columns` の `1fr` プロパティを持たせてください。
