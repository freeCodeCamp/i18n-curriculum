...
Indente seu `for` loop existente em um nível para que ele execute quando o catálogo tiver filmes. Então, substitua `self.items` por `movies` para iterar apenas sobre os filmes.

# --hints--

Você deve indentar o `for` existente em um nível e substituir `self.items` por `movies` para iterar apenas sobre os filmes.
