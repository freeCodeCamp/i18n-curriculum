...
La sección de información del jugador ya está completa. Después del cierre de `div` de la sección de información del jugador y aún dentro de `form-panel`, agrega un nuevo `div`. Dentro de este, añade un elemento `p` con el `className` de `form-section-title` y el texto `Player Stats`. Luego agrega un `div` con el `className` de `stats-grid`. Deja la cuadrícula de estadísticas vacía por ahora.

# --hints--

Tu `div.form-panel` debe contener un segundo `div` para la sección de estadísticas del jugador.
...
La sección de estadísticas del jugador debe contener un `p` con el `className` de `form-section-title` y el texto `Player Stats`.
...
La sección de estadísticas del jugador debe contener un `div` con el `className` de `stats-grid`.
