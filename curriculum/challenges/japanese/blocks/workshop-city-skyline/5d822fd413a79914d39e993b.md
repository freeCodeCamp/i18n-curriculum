...
`sky` クラス全体とそのすべてのプロパティと値をメディアクエリにコピー＆ペーストしてください。スカイラインの昼から夜へのカラースキームをもう一つ作成します。

注意：クラスをコピーするために編集可能な領域をスクロールして通り過ぎる必要があります。

# --hints--

既存の `.sky` 宣言は削除しないでください。
