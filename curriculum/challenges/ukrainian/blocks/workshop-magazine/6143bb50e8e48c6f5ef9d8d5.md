...
У межах вашого елемента `aside` створіть два елементи `img`, один елемент `blockquote` і третій елемент `img`. Дайте елементу `blockquote` множину `class`, встановлену на `image-quote`.

# --hints--

Ви повинні створити три елементи `img` у межах вашого елемента `aside`.

```js
assert.lengthOf(document.querySelectorAll('aside img'), 3);
```

Ви повинні створити елемент `blockquote` у межах вашого елемента `aside`.

```js
assert.exists(document.querySelector('aside blockquote'));
```

Ваш елемент `blockquote` має мати множину `class`, встановлену на `image-quote`.

```js
assert.isTrue(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

Ваші нові елементи мають бути в правильному порядку.
