...
既然 `level` 設值函式已經處理了無效層級的驗證，請從 `if level not in Employee._base_salaries` 中移除 `__init__` 的檢查。這部分將在下一步由設值函式負責。

# --hints--

你的 `__init__` 方法不應該包含 `level not in Employee._base_salaries` 的檢查。
