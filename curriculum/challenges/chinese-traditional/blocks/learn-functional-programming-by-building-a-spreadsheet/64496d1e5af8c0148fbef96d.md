...
現在宣告一個 `length` 變數並將其設值為你排序後陣列的長度，以及一個 `middle` 變數，其值為長度除以 `2` 後再減去 `1`。

# --hints--

你應該在你的 `sorted` 變數之後宣告一個 `length` 變數。
...
你應該將 `sorted` 陣列的長度指定給你的 `length` 變數。
...
你應該在你的 `length` 變數之後宣告一個 `middle` 變數。
...
你應該將 `middle` 指定為將你的 `length` 變數除以 `2` 的值。
