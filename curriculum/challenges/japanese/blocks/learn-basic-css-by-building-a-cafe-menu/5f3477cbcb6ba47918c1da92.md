...
モバイルでもデスクトップやノートパソコンと同じような見た目になるようにページのスタイルを調整するには、特別な`content`属性を持つ`meta`要素を追加する必要があります。
...
`meta`要素には、値が`viewport`の`name`属性が必要です。
...
`meta`要素には、値が`width=device-width, initial-scale=1.0`の`content`属性が必要です。
