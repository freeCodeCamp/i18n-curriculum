...
في هذه المهمة، ستتدرب على تحديد الأقسام من خلال الأفعال التي تؤديها.

# --instructions--

استمع وأكمل الجملة بالقسم الصحيح.
...
هذا القسم يدرب الفريق.

# --explanation--

`El departamento de aprendizaje y desarrollo capacita al equipo` تعني "قسم التعلم والتطوير يدرب الفريق".

كلا من `apoyar` ("يدعم") و`capacitar` ("يدرب") هما أفعال مفردة في الغائب تنتهي بـ `-ar`.

تتبع نفس نمط `desarrolla` و`verifica` و`crea` من المهام السابقة.
