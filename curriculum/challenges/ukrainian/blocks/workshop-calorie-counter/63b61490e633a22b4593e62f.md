...
Відповідно до того самого шаблону призначте свій `#add-entry` елемент `addEntryButton`, свій `#clear` елемент `clearButton`, а свій `#output` елемент `output`.

# --hints--

Ви повинні оголосити змінну з назвою `addEntryButton`.

```js
assert.isDefined(addEntryButton);
```

Ви повинні використати `document.getElementById()` щоб отримати `#add-entry` елемент.

```js
assert.match(code, /document\.getElementById\(\s*('|")add-entry\1\s*\)/g);
```

Ви повинні призначити `#add-entry` елемент `addEntryButton`.

```js
assert.deepEqual(addEntryButton, document.getElementById('add-entry'));
```

Ви повинні оголосити змінну з назвою `clearButton`.

```js
assert.isDefined(clearButton);
```

Ви повинні використати `document.getElementById()` щоб отримати `#clear` елемент.

```js
assert.match(code, /document\.getElementById\(\s*('|")clear\1\s*\)/g);
```

Ви повинні призначити `#clear` елемент `clearButton`.

```js
assert.deepEqual(clearButton, document.getElementById('clear'));
```

Ви повинні оголосити змінну з назвою `output`.
...
Ви повинні використати `document.getElementById()` щоб отримати `#output` елемент.

```js
assert.match(code, /document\.getElementById\(\s*('|")output\1\s*\)/g);
```

Ви повинні призначити `#output` елемент `output`.
