...
`.note` セレクターに左右のパディングを `8px` に設定し、上下のパディングはなくしてください。また、`text-indent` プロパティを `-8px` に設定してください。

これらの最後の変更で、栄養ラベルが完成します！

# --hints--

`.note` セレクターには `padding` プロパティを `0 8px` に設定してください。
...
`.note` セレクターには `text-indent` プロパティを `-8px` に設定してください。
