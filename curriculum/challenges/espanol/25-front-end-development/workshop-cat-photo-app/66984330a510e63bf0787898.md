...
Your `ul` element should have an opening tag. Las etiquetas de apertura tienen esta sintaxis: `<elementName>`.

```js
assert.exists(document.querySelector('ul'));
```

Your `ul` element should have a closing tag. Las etiquetas de cierre tiene una `/` después del carácter `<`.
