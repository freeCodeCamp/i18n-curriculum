...
Rimuovi l'array `myTaskArr` e tutto il codice per `localStorage` perché non ti servono più.

# --hints--

Dovresti rimuovere `myTaskArr` e tutto il codice relativo a `localStorage` che hai appena imparato.
...
Dovresti rimuovere eventuali riferimenti rimanenti a `myTaskArr` in tutto il codice.

```js
assert.notMatch(code, /myTaskArr/);
```

Dovresti rimuovere qualsiasi riferimento a `localStorage.getItem` per l'elemento `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Dovresti rimuovere eventuali riferimenti rimanenti a `getTaskArrObj` in tutto il codice.
