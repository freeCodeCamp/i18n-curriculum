...
在之前的課程中，你學習了狀態以及如何使用 `useState` 掛鉤。為這次工作坊，你將創建一個應用程式，當使用者點擊按鈕時，會在螢幕上隱藏／顯示一段文字。

所有樣板程式碼，包括 CSS，都已為你提供。

首先，在 `ToggleApp` 組件內傳回一個具有 `id` 為 `toggle-container` 的 `div` 元素。
...
你的 `div` 元素應該有一個名為 `toggle-container` 的 `id`。
