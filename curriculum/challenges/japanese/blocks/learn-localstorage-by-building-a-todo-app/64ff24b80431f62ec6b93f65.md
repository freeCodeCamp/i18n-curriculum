...
`myTaskArr` 配列と `localStorage` に関するすべてのコードを削除してください。もう必要ありません。

# --hints--

`myTaskArr` と、学んだばかりの `localStorage` に関連するすべてのコードを削除してください。
...
コード内のどこにあっても `myTaskArr` への残りの参照はすべて削除してください。

```js
assert.notMatch(code, /myTaskArr/);
```

`localStorage.getItem` の項目に関する `"data"` への参照はすべて削除してください。

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

コード内のどこにあっても `getTaskArrObj` への残りの参照はすべて削除してください。
