...
Ви повинні надати `.penguin-body::before` властивість `top` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

Ви повинні надати `.penguin-body::before` властивість `left` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.
