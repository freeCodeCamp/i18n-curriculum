...
在 `figure` 元素中的嵌套的图像之后，添加一个 `figcaption` 元素，文本为：
...
嵌套在 `figure` 元素中的 `figcaption` 元素应位于 `img` 元素下方。 你把它们的顺序写错了。
...
你的 `figcaption` 元素中的文本应该是 `Cats love lasagna.`。你要么遗漏了文本，要么拼写有误。
