...
参加者がシステム内をどのように移動しているかを追跡するために、次のメッセージをコンソールに記録してください：`overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`。文字列の連結かテンプレートリテラルのどちらかを使えます。

# --hints--

あなたの`rerouteOverflow`関数は、次のメッセージをコンソールに記録する必要があります：`overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`。
