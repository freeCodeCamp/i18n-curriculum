...
이제 현재 몬스터의 이름과 체력 텍스트를 업데이트해야 합니다.

먼저 `monsters[fighting].name`를 `innerText`의 `monsterName` 속성에 할당하세요. 그런 다음 `monsterHealth`를 `innerText`의 `monsterHealthText` 속성에 할당하세요.

# --hints--

점 표기법을 사용해 `innerText`의 `monsterName` 속성에 접근하세요.

```js
assert.match(goFight.toString(), /monsterName\.innerText/);
```

`monsters[fighting].name`를 `monsterName.innerText`에 할당하세요.

```js
assert.match(goFight.toString(), /monsterName\.innerText\s*=\s*monsters\s*\[\s*fighting\s*\]\s*\.name/);
```

점 표기법을 사용해 `innerText`의 `monsterHealthText` 속성에 접근하세요.

```js
assert.match(goFight.toString(), /monsterHealthText\.innerText/);
```

`monsterHealth`를 `monsterHealthText.innerText`에 할당하세요.
