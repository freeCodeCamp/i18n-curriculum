...
今度はアロー関数を使って `POSITIONS` 配列をマップし、各ポジションの文字列を `pos` として受け取り、`option` 要素を返してください。option は `key` の `pos` を持ち、value も `pos` であるべきです。また、生成された各 `pos` のテキストとして `option` を表示してください。

map のパラメーターは何でも構いません。必ずしも `pos` である必要はありません。

# --hints--

あなたの position `select` は `option` 配列からマップされた `POSITIONS` 要素を含むべきです。
...
各 option の `value` は表示されるテキストと一致し、有効なポジションである必要があります。
