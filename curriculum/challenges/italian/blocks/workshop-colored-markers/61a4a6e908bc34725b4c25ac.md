...
Imposta quindi il punto di colore per il rosso su `0%`, il punto di colore per il verde su `50%` e il punto di colore per il blu su `100%`.

# --hints--

La tua regola CSS `.red` dovrebbe avere una proprietà `background` impostata su `linear-gradient(180deg, rgb(255, 0, 0) 0%, rgb(0, 255, 0) 50%, rgb(0, 0, 255) 100%)`.
