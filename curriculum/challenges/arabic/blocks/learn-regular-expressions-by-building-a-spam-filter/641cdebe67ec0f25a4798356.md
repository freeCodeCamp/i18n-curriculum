...
يجب أن يتحقق بيان `if` مما إذا كانت قيمة `value` الخاصة بـ `messageInput` هي سلسلة نصية فارغة.
...
يجب أن تعرض جملة `if` تنبيهًا للمستخدم بالرسالة `"Please enter a message."`.
...
يجب أن تخرج جملة `if` من تنفيذ الدالة.
