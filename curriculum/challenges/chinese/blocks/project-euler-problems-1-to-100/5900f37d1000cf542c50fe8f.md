...
2 <sup> 15 </> = 32768，其位数之和为3 + 2 + 7 + 6 + 8 = 26。

2<sup><code>exponent</code></sup> 这个数字的各位数字之和是多少？

# --hints--

`powerDigitSum(15)` 应该返回一个数字。

```js
assert.isNumber(powerDigitSum(15));
```

`powerDigitSum(15)` 应该返回 26。

```js
assert.strictEqual(powerDigitSum(15), 26);
```

`powerDigitSum(128)` 应该返回 166。

```js
assert.strictEqual(powerDigitSum(128), 166);
```

`powerDigitSum(1000)` 应该返回 1366。
