...
Il nuovo selettore `.left-container p` dovrebbe avere una proprietà `margin` con il valore `-5px -2px`.
...
Il nuovo selettore `.left-container p` dovrebbe avere una proprietà `font-size` con il valore `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

Il nuovo selettore `.left-container p` dovrebbe avere una proprietà `font-weight` con il valore `700`.
