...
`.blush.right` 요소 아래에 `div`인 `class`를 각각 가진 `beak` 요소 두 개를 추가하세요. 또한, 첫 번째 `.beak` 요소에는 `class`인 `top`를, 두 번째 `.beak` 요소에는 `class`인 `bottom`를 부여하세요.
