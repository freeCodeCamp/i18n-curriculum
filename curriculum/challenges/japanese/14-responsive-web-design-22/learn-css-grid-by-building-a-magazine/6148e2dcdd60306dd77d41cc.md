...
画像は `image-wrapper` クラスが設定された `aside` 要素で囲まれているので、`.image-wrapper` セレクターを作成してください。 `display` プロパティを `grid` に設定してください。

# --hints--

`.image-wrapper` セレクターが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

`.image-wrapper` セレクターの `display` プロパティを `grid` に設定する必要があります。
