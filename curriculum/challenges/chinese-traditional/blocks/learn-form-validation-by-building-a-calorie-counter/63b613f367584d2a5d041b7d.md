...
取得你的 `#budget` 元素並將它指定給 `budgetNumberInput`，取得你的 `#entry-dropdown` 元素並將它指定給 `entryDropdown`。

# --hints--

你應該宣告一個名為 `budgetNumberInput` 的變數。

```js
assert.isDefined(budgetNumberInput);
```

你應該使用 `document.getElementById()` 來取得 `#budget` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")budget\1\s*\)/g);
```

你應該將 `#budget` 元素儲存在名為 `budgetNumberInput` 的變數中。

```js
assert.equal(budgetNumberInput, document.getElementById('budget'));
```

你應該宣告一個名為 `entryDropdown` 的變數。

```js
assert.isDefined(entryDropdown);
```

你應該使用 `document.getElementById()` 來取得 `#entry-dropdown` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")entry-dropdown\1\s*\)/g);
```

你應該將 `#entry-dropdown` 元素儲存在名為 `entryDropdown` 的變數中。
