...
次に、DOMから要素を非表示にする関数を作成する必要があります。`hideElements` という名前の関数を宣言し、`...elements` を `HTMLElement[]` 型のパラメータとして受け取ってください。

この関数は、CSSの`hidden`クラスを使い、アプリ全体でUI要素の可視性を切り替えるために使用されます。

# --hints--

`hideElements` という名前の関数を宣言しているはずです。
...
`hideElements` 関数は、`...elements` 型の`HTMLElement[]` という名前のパラメータを持っているはずです。
