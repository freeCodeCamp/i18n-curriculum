...
在你的 `elemValue` 函式中，宣告一個名為 `inner` 的函式，該函式帶有一個 `character` 參數。
...
你的 `inner` 函式（程式）應該只有 `character` 作為唯一參數。
