...
قم بتعيين لون الخلفية (background color) ألى `footer` بقيمة `#2a2a40`, واستخدم _Flexbox_ لتركيز النص أفقيا.

# --hints--

يجب عليك استخدام منتقي العنصر `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

يجب عليك إعطاء `footer` خاصية `background-color` بقيمة `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

يجب عليك إعطاء `footer` خاصية `display` بقيمة `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

يجب عليك إعطاء `footer` خاصية `justify-content` بقيمة `center`.
