...
アバターのサイズをカスタマイズするには、`30` の値に設定できます。

まず、`avatar` という定数を作成してください。次に、`replace` に対して `user.avatar_template` メソッドを使った結果を代入します。

`replace` メソッドの最初の引数には `/{size}/` を、2番目の引数には数値の `30` を使ってください。
...
`user.avatar_template.replace()` 定数に `avatar` を代入してください。

```js
assert(code.match(/const\s+avatar\s*=\s*user\.avatar_template\.replace\(\s*.*\s*\)/));
```

`/{size}/` メソッドの最初の引数には `30` を、2番目の引数には数値の `replace` を使うはずです。
