...
Labda umeona kuwa msimbo wako unakuwa wa kurudia sana! Kuna njia bora ya kushughulikia vipengele vingi vinavyofanana.

Badala ya kuchagua kila kitufe kimoja kimoja, tumia `querySelectorAll(".emoji-btn")` kuchagua vitufe vyote vyenye darasa la vitu `emoji-btn` mara moja. Hifadhi matokeo katika kigezo kinachoitwa `btns`.
...
Unapaswa kutumia `querySelectorAll` kuchagua vitufe vyote vya emoji.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

Unapaswa kuhifadhi matokeo katika kigezo kinachoitwa `btns`.
...
Kigezo `btns` kinapaswa kuwa na vitufe vyote 4 vya emoji.
