...
Спершу зробіть так, щоб компонент відтворював елемент `input`, елемент `button` та елемент `ul` у методі `render()`. Коли елемент `input` змінюється, він повинен запускати метод `handleChange()`. До того ж елемент `input` має відтворити значення `input` зі стану компонента. Якщо натиснути елемент `button`, має запуститись метод `submitMessage()`.

Після цього напишіть ці два методи. Метод `handleChange()` має оновлювати `input` тим, що вводить користувач. Метод `submitMessage()` має приєднати поточне повідомлення (збережене в `input`) до масиву `messages` в локальному стані та очистити значення `input`.

Вкінці використайте `ul`, щоб мапувати масив `messages` та відтворити його на дисплеї як список елементів `li`.

# --hints--

Компонент `DisplayMessages` має ініціалізуватися станом `{ input: "", messages: [] }`.
...
Компонент `DisplayMessages` має відтворити `div`, який містить елемент `h2`, елемент `button`, елемент `ul` та елементи `li` як дочірні компоненти.
...
Використайте `.map` до масиву `messages`.

```js
assert(code.match(/this\.state\.messages\.map/g));
```

Елемент `input` має відтворити значення `input` в локальному стані.
...
Виклик методу `handleChange` має оновити значення стану `input` до поточного введення.
...
Натискання кнопки `Add message` має викликати метод `submitMessage`, який повинен додати поточний `input` до масиву `messages`.
