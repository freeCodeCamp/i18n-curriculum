...
Створіть ще один елемент `p` після останнього `.divider` і додайте йому текст `Total Sugars 12g`. Призначте цьому елементу `p` необхідні значення `class`, щоб зробити відступ і прибрати нижній кордон. Потім створіть ще один елемент `.divider` під вашим новим елементом `p`.

# --hints--

Ви маєте створити новий елемент `p` і `div` в кінці вашого елемента `.daily-value`.
...
Ваш новий елемент `p` повинен містити текст `Total Sugars 12g`.
...
Ваш новий елемент `div` повинен мати атрибут `class`, призначений значенню `divider`.
