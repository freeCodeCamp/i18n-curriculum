...
ワークショップの最後の部分として、下部の連絡先セクションにいくつか変更が必要です。

最初の変更は、連絡先セクションを `footer` 要素で囲むことです。これにより、スクリーンリーダーがこのセクションをページのフッターとして認識しやすくなります。

# --hints--

ページには `footer` 要素が必要です。
...
`footer` 要素には、テキスト `h3` を含む `Contact Me` 要素が含まれている必要があります。
...
`footer` 要素には、テキスト `p` を含む `Email me at camperbot@blog.io` 要素が含まれている必要があります。
