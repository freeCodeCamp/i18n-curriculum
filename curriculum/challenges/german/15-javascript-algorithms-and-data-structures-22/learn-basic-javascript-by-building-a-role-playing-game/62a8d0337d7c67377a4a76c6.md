...
Du solltest mit `text.innerText` eine neue Zeile hinzufügen.
...
Du solltest den `+=`-Operator verwenden, um etwas zu `text.innerText` hinzuzufügen.

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*\+=\s*/)
```

Du solltest den String `" In your inventory you have: "` am Ende von `text.innerText` hinzufügen. Achte auf die Leerzeichen!
