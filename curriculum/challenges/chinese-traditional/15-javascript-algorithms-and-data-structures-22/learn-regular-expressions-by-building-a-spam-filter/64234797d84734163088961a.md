...
最後，允許你的正則表達式匹配整個單詞（就像你用 `freeRegex` 做的那樣）。

# --hints--

你的 `stockRegex` 應該使用非捕獲組。

```js
assert.match(stockRegex.source, /\(\?:/);
```

你的 `stockRegex` 應該使用非捕獲組來匹配 `\s` 或 `^`。

```js
assert.match(stockRegex.source, /\(\?:(\^\|\\s|\\s\|\^)\)/);
```

你的 `stockRegex` 應該使用第二個非捕獲組。
...
你的 `stockRegex` 應該匹配 `it's stock alert time`。

```js
assert.match("it's stock alert time", stockRegex);
```

你的 `stockRegex` 應該匹配 `stock alert time`。

```js
assert.match('stock alert time', stockRegex);
```

你的 `stockRegex` 應該匹配 `it's stock alert`。

```js
assert.match("it's stock alert", stockRegex);
```

你的 `stockRegex` 應該匹配 `stock alert`。
