...
title: btn-dangerで危険な操作をユーザーに警告する
challengeType: 0
forumTopicId: 18375
...
Bootstrapにはボタン用のいくつかの事前定義された色があります。`btn-danger`クラスは、猫の写真を削除するなどの破壊的な操作を行うボタンでユーザーに通知するためのボタン色です。

テキストが`Delete`のボタンを作成し、`btn-danger`クラスを付けてください。

これらのボタンには`btn`と`btn-block`のクラスも必要なことに注意してください。

# --hints--

テキストが`button`の新しい`Delete`要素を作成してください。
...
すべてのBootstrapボタンには`btn`と`btn-block`のクラスを付ける必要があります。

```js
assert.lengthOf(document.querySelectorAll('button.btn-block.btn'),3);
```

新しいボタンには`btn-danger`クラスを付けてください。
