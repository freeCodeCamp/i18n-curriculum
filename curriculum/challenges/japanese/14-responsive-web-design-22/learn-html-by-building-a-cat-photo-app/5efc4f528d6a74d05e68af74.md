...
チェックボックスの要素の後に、新しく `label`要素を追加してください。 Make sure it has both an opening and closing tag.
...
`label` 要素には値が `loving` の `for` 属性が必要です。  Expected `--fcc-expected--`, but found `--fcc-actual--`.
