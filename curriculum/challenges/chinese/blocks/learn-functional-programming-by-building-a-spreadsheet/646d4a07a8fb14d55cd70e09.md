...
向你的 `spreadsheetFunctions` 添加 `someeven` 属性 - 使用 `.some()` 方法检查数组中的任何元素是否为偶数。

# --hints--

你的 `spreadsheetFunctions` 对象应该具有 `someeven` 属性。

```js
assert.property(spreadsheetFunctions, "someeven");
```

你的 `someeven` 属性应该是一个函数。

```js
assert.isFunction(spreadsheetFunctions.someeven);
```

如果数组中的某些元素是偶数，则 `someeven` 函数应该返回 `true`。

```js
assert.isTrue(spreadsheetFunctions.someeven([1, 2, 3]));
```

如果数组中没有偶数元素，则 `someeven` 函数应该返回 `false`。
