...
Ndani ya njia ya `fetchCardsData`, tangaza kigezo kinachoitwa `response` na kimupe thamani kwa kufanya wito wa kitendakazi cha `fetch` kwa URL ifuatayo: `` `${CDN_URL}/card_data.json` ``.

Ongeza neno kuu la `await` kabla ya wito la `fetch`, na funika mantiki yote ndani ya blokki ya `try...catch`.

# --hints--

Unapaswa kutangaza kigezo cha `response` ndani ya njia ya `fetchCardsData`.
...
Kigezo chako cha `response` kinapaswa kupewa thamani kwa kufanya wito wa `await fetch` na URL `` `${CDN_URL}/card_data.json` ``.
...
Wito lako la `fetch` linapaswa kufunikwa ndani ya blokki ya `try...catch`, ukiwa na `error` kama kigezo cha `catch`.
