...
`0%`와 `100%` 선택자 사이에 `50%` 선택자를 추가하세요. 이 선택자는 애니메이션 주기 중간에 적용됩니다. `background-color`를 `purple`로 설정하세요.

# --hints--

`50%` 규칙 안에 새로운 `@keyframes cabins` 선택자를 만드세요.
