...
Agrega un elemento `title` con el texto `Magazine`, un elemento `link` para la hoja de estilos de la fuente `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap`, un `link` para la hoja de estilos FontAwesome `https://use.fontawesome.com/releases/v5.8.2/css/all.css` y un `link` para tu hoja de estilos `./styles.css`.
...
Uno de tus elementos de enlace debe tener el `href` configurado en `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap`.
...
El proyecto debe tener el título `Magazine`. Recuerda que las mayúsculas, minúsculas y la ortografía son importantes para el título.
