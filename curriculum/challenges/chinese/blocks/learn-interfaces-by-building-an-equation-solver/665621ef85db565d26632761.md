...
在你的新函数中，创建一个 `if` 语句，检查 `equation` 是否不是 `Equation` 类的实例，并使用字符串 `'Argument must be an Equation object'` 引发 `TypeError` 以提供自定义消息。

# --hints--

你应该创建一个 `if` 语句来检查 `equation` 是否不是 `solver` 函数中的 `Equation` 类的实例。
...
你应该在新的 `if` 语句中使用提供的字符串引发 `TypeError`。
