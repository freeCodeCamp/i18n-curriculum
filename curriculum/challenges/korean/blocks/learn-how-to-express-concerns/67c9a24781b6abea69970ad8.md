...
이 질문에 대한 정답은 `Key Points` 섹션과 `Suggested Actions` 섹션에서 찾을 수 있으며, 특히 `morale`를 언급하는 부분에서 찾을 수 있습니다. `morale`는 특정 시점에 개인이나 그룹이 가지는 열정과 자신감으로 정의할 수 있습니다. 텍스트에서 morale라는 단어를 찾아보면 답을 알 수 있습니다.
