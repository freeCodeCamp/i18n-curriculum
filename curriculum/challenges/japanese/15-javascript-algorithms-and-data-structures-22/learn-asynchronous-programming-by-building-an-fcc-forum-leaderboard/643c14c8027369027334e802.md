...
レスポンスボディを扱うため、JSON オブジェクトとして取得したいです。 ここでの変数 `res` の `.json()` メソッドはプロミスを返します。つまり、これを `await` で待たなければなりません。

`data` と言う名前の const 変数を作成し、`await res.json()` の値を代入してください。

# --hints--

`data` という `const` 変数が必要です。

```js
assert.match(code, /const\s+data/);
```

変数 `data` に `await res.json()` を割り当ててください。
