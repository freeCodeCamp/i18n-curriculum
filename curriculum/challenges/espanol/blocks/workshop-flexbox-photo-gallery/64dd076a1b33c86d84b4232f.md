...
Asigne a su elemento `.gallery` un `width` de `50%` y un `border` de `5px solid red`.

Luego, a sus elementos `img` asígneles un `width` de `100%`, un `padding` de `5px` y un `border` de `5px solid blue`.
...
Tu selector `.gallery` debe tener la propiedad `border` establecida en `5px solid red`.
...
Tu selector `img` debe tener la propiedad `border` establecida en `5px solid blue`.
