...
Ili kuunda mandhari ya nyuma, utaongeza milima miwili.

Juu ya kipengele cha `.penguin`, ongeza `div` yenye `class` ya `left-mountain`.

# --hints--

Unapaswa kuongeza `div` mpya ndani ya `body`. Inatarajiwa kuona vipengele vya `--fcc-expected--` `div`, lakini vilipatikana `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('body > div')?.length, 3);
```

Unapaswa kumpa `div` `class` ya `left-mountain`.

```js
assert.include(document.querySelector('body > div:not(.ground, .penguin)')?.className, 'left-mountain');
```

Unapaswa kuweka `.left-mountain` kabla ya `.penguin`.
