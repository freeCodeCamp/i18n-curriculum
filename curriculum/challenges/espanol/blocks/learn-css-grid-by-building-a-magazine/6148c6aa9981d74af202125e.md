...
Da al selector `.text` una propiedad `font-size` establecida en `1.8rem` y una propiedad `letter-spacing` establecida en `0.6px`.

# --hints--

Tu selector `.text` debe tener una propiedad `font-size` establecida en `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize === '1.8rem');
```

Tu selector `.text` debe tener una propiedad `letter-spacing` establecida en `0.6px`.
