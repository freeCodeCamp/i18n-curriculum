...
如果 `numberInput` 的 `value` 等于 `5`，则调用 `showAnimation()` 函数，然后提前 `return`。

# --hints--

你应该在第二个 `if` 语句的主体内调用 `showAnimation()` 函数。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

调用 `showAnimation()` 函数后，应使用 `return` 关键字提早退出 `checkUserInput()` 函数。
