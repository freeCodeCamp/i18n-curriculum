...
此實作具有指數時間複雜度，因為大量重複計算。當計算 `climbStairsRecursive(5)` 時，會發生以下情況：

- `climbStairsRecursive(5)` 會呼叫 `climbStairsRecursive(4)` 和 `climbStairsRecursive(3)`
    
- `climbStairsRecursive(4)` 會呼叫 `climbStairsRecursive(3)` 和 `climbStairsRecursive(2)`
    
- 現在 `climbStairsRecursive(3)` 被計算了 **兩次**
    
- `climbStairsRecursive(3)` 呼叫 `climbStairsRecursive(2)` 和 `climbStairsRecursive(1)`
    
- `climbStairsRecursive(2)` 總共計算了 **3 次**
    

僅對 `n=5`，我們進行了 9 次函式呼叫，而實際只需要 5 次獨特計算。隨著 `n` 增長，這種冗餘會呈指數爆炸——`climbStairsRecursive(30)` 將需要超過數百萬次函式呼叫！時間複雜度變為 `O(2^n)`，使其對較大 `n` 值來說既低效又不實用。
...
記憶化非常高效，因為從 `1` 到 `n` 的每個唯一值只計算一次。當我們再次需要 `climbStairsMemo(3)` 時，不是重新計算（這會觸發更多遞迴呼叫），而是直接在我們的備忘錄字典中以 `O(1)` 時間查找。

讓我們用自頂向下的方法追蹤 `climbStairsMemo(5)` 的執行，看看備忘錄如何消除重複工作：
...
- **實際影響**：`climbStairsMemo(30)` 從數百萬次函式呼叫降到約 30 次獨特計算！
...
讓我們看看自底向上的方法如何運作，了解我們如何系統性地建造解法。以下是 `climbStairsTabulation(5)` 的反覆構件過程：
