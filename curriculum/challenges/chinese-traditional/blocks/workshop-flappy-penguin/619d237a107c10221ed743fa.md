...
在 `.penguin-body` 中，於 `.foot` 元素之前，添加兩個 `div` 元素，每個元素的 `class` 為 `arm`。將第一個 `.arm` 的 `class` 設為 `left`，第二個 `.arm` 的 `class` 設為 `right`。
