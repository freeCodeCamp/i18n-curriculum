...
在這個實驗中，你將創建一個應用程式，用來顯示和儲存抽認卡資料，並且可以在之後取回。

**目標：** 完成以下使用者故事並通過所有測試以完成此實驗。

**使用者故事：**

1) 你應該有一個 id 為 `flashcard` 的 HTML 元素。
2) 你應該有一個名為 `FlashCard` 的介面。
3) `FlashCard` 介面應包含以下屬性 `questionText` 和 `questionAnswer`，兩者型別皆為 `string`。
4) 你應該有一個名為 `FlashCard` 的 `currentCards` 元素群集。
5) 當 `#flashcard` 元素被點擊時，該卡片應該具有 `flipped` 類別。
6) 你應該有一個 id 為 `delete-btn` 的元素。
7) 當 `#delete-btn` 元素被點擊時，應該移除目前的卡片並顯示上一張卡片的資料。
8) 你應該創建一個 id 為 `entry-form` 的輸入表單，以便在提交時能將更多抽認卡新增到 `currentCards` 群集中。
9) 表單內的兩個元素應分別有 id 為 `front-text` 和 `back-text`。
10) 當輸入表單中的問題文字或答案為空時，你應該創建並呼叫一個 `InvalidUserInputError`。

# --hints--

你應該有一個 id 為 `flashcard` 的 HTML 元素。
...
`FlashCard` 介面應有一個 `questionText` 屬性，型別為 `string`。
...
`FlashCard` 介面應有一個 `questionAnswer` 屬性，型別為 `string`。
...
你應該有一個名為 `FlashCard` 的 `currentCards` 元素群集，型別為 `FlashCard[]`。
...
當 `#flashcard` 元素第一次被點擊時，該元素應該獲得 `flipped` 類別。
...
當 `delete-btn` 被點擊時，應該從 `currentCards` 群集中移除一個抽認卡元素。
...
當 `delete-btn` 被點擊時，應該顯示上一張抽認卡的資料。
...
你應該在表單內有兩個 `textarea` 元素，id 分別為 `front-text` 和 `back-text`。
...
當輸入表單中的問題文字或答案為空時，應該發出一個 `InvalidUserInputError`。
