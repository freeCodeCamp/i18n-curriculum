...
У вашому JavaScript-файлі виберіть елемент `.story-container` і збережіть його у змінну з назвою `storyContainer`.

# --hints--

У вас має бути змінна `storyContainer`.

```js
assert.isDefined(storyContainer);
```

Ви маєте вибрати елемент `.story-container` за допомогою методу `querySelector` і зберегти його у змінну `storyContainer`.
