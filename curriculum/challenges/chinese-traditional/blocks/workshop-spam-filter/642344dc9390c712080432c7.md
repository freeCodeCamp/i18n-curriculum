...
像你的 `freeRegex` 一樣，更新你的 `stockRegex`，將 `e` 和 `o` 字元取代為字元類別，以符合該字母及相應的數字。

# --hints--

你的 `stockRegex` 應該使用字元類別來匹配字母 `e` 和數字 `3`。

```js
assert.match(stockRegex.source, /\[(e3|3e)\]/);
```

你的 `stockRegex` 應該使用字元類別來匹配字母 `o` 和數字 `0`。
...
你的 `stockRegex` 應該匹配 `st0ck al3rt`。
