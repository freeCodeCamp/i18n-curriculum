...
你為你的腳本添加了 `console.log()` 呼叫來驗證你的 `EVAChunks` 陣列，現在該呼叫已不再需要。為了讓終端機在接下來的步驟中保持整潔，請刪除用來從 `EVAChunks` 記錄資料的 `for loop` 區塊。

# --hints--

你應該移除記錄來自 `EVAChunks` 的資料的 `for loop` 區塊。刪除包含兩個迴圈的整個區塊。
