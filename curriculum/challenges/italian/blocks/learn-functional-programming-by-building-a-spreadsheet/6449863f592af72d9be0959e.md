...
Dopo che il tuo `rangeRegex` trova il `:`, deve cercare lo stesso pattern di lettere e numeri di prima.

Copia i tuoi due gruppi di cattura esistenti e incollali dopo i due punti.

# --hints--

Dovresti aggiungere un terzo gruppo di cattura al tuo `rangeRegex`, dopo i due punti.
...
Il tuo terzo gruppo di cattura dovrebbe usare una classe di caratteri.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[.*\]\)/);
```

Il tuo terzo gruppo di cattura dovrebbe corrispondere ai caratteri `A` fino a `J`.
...
Dovresti aggiungere un quarto gruppo di cattura al tuo `rangeRegex`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(.*\)/);
```

Il tuo quarto gruppo di cattura dovrebbe corrispondere a una o due cifre.
