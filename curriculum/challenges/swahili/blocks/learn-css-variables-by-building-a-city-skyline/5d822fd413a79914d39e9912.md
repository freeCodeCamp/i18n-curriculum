...
Toa `.fb1b` `width` ya `60%` na `height` ya `10%`, na `.fb1c` `width` ya `100%` na `height` ya `80%`.

# --hints--

Unapaswa kumpa `.fb1b` `width` ya `60%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.width, "60%");
```

Unapaswa kumpa `.fb1b` `height` ya `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.height, "10%");
```

Unapaswa kumpa `.fb1c` `width` ya `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1c")?.width, "100%");
```

Unapaswa kumpa `.fb1c` `height` ya `80%`.
