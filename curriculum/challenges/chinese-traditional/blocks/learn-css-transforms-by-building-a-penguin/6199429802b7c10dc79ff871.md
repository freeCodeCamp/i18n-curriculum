...
將 `.penguin-head` 元素放置在其父元素頂部 `10%` 和左側 `25%` 的位置。
...
你應該將 `.penguin-head` 的 `top` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
...
你應該將 `.penguin-head` 的 `left` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
