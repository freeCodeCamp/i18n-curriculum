...
이제 `salary` 세터가 만들어졌으니, `__init__`을 직접 `self._salary`에 할당하는 대신 이 세터를 호출하도록 업데이트하세요. `self._salary = Employee._base_salaries[level]`을 `self.salary = Employee._base_salaries[level]`로 변경하세요.

# --hints--

`__init__` 메서드는 `self.salary = Employee._base_salaries[level]` 대신 `self._salary = Employee._base_salaries[level]`을 사용해야 합니다.
