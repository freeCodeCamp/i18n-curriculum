...
现在，你需要更新当前怪物的名称和健康状况的文本。

首先将 `monsters[fighting].name` 分配给 `monsterName` 的 `innerText` 属性。 然后，将 `monsterHealth` 分配给 `monsterHealthText` 的 `innerText` 属性。

# --hints--

你应该使用点符号来访问 `monsterName` 的 `innerText` 属性。

```js
assert.match(goFight.toString(), /monsterName\.innerText/);
```

你应该将 `monsters[fighting].name` 分配给 `monsterName.innerText`。

```js
assert.match(goFight.toString(), /monsterName\.innerText\s*=\s*monsters\s*\[\s*fighting\s*\]\s*\.name/);
```

你应该使用点符号来访问 `monsterHealthText` 的 `innerText` 属性。

```js
assert.match(goFight.toString(), /monsterHealthText\.innerText/);
```

你应该将 `monsterHealth` 分配给 `monsterHealthText.innerText`。
