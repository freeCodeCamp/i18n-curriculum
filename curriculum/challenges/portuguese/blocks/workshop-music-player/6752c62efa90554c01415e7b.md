...
Crie uma função chamada `setPlayerDisplay`. Dentro da função, defina o `textContent` de `playingSong` para o título da música atual, se definido. Caso contrário, defina-o como uma string vazia.

Abaixo disso, defina o `textContent` de `songArtist` para o artista da música atual, se definido. Caso contrário, defina como string vazia. 


# --hints--

Você deve ter uma função chamada `setPlayerDisplay`.

```js
assert.isFunction(setPlayerDisplay);
```

Sua função `setPlayerDisplay` deve definir o `textContent` de `playingSong` para o título da música atual, se definido, ou para uma string vazia.
...
Sua função `setPlayerDisplay` deve definir o `textContent` de `songArtist` para o artista da música atual, se definido, ou para uma string vazia.
