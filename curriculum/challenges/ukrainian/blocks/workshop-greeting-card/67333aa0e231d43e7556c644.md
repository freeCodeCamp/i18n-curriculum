...
Додайте новий селектор, який націлюється на `.card-links a` елементи, коли вони активні. Встановіть `background-color` у `midnightblue`.

# --hints--

У вас має бути селектор `.card-links a:active`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:active"));
```

Селектор `.card-links a:active` має мати `background-color` зі значенням `midnightblue`.
