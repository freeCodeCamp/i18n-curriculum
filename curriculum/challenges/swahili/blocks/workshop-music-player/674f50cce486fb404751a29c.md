...
Sasa unajua jinsi ya kupata wimbo unaofuata kuchezwa, tengeneza kitendakazi kinachoitwa `playNextSong`. Ndani ya kitendakazi cha `playNextSong`, tengeneza tamko la `if` ili kuangalia kama `currentSong` ya `userData` ni sawa kabisa na `null`. Hii itahakikisha kama hakuna wimbo unaochezwa kwa sasa katika kitu cha `userData`.

Kama hali hiyo ni kweli, fanya wito wa kitendakazi cha `playSong` ukiwa na `id` ya wimbo wa kwanza katika safu ya data ya `userData.songs` kama hoja.

Kisha ongeza usemi wa `return` mwishoni mwa sehemu ya `if`.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `playNextSong`.

```js
assert.isFunction(playNextSong);
```

Wakati `currentSong` ya `userData` ni sawa kabisa na `null`, kitendakazi chako cha `playNextSong` kinapaswa kufanya wito wa kitendakazi cha `playSong` ukiwa na `id` ya wimbo wa kwanza katika safu ya data ya `userData.songs` kama hoja.

```js
assert.match(
  playNextSong.toString(),
  /if\s*\(\s*userData\.currentSong\s*===\s*null\s*\)\s*\{[\s\S]*playSong\(\s*userData\.songs\s*\[\s*0\s*\]\.id\s*\)/
);
...
playNextSong();
assert.isUndefined(userData.currentSong);
...
Unapaswa kuwa na usemi wa `return` mwishoni mwa sehemu ya `if`.
