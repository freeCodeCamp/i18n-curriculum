...
콜백 함수 내에서 `removeAttribute()` 메서드를 사용해 `"aria-current"` 속성을 제거하세요. 이렇게 하면 각 노래에 대해 속성이 제거됩니다.

# --hints--

기존 `forEach()`와 그 내용을 수정하지 마세요.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

`removeAttribute()`에서 `songEl` 메서드를 사용하고 `"aria-current"`를 전달해야 합니다.
