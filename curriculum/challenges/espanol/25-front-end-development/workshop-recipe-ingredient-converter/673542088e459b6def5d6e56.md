...
Necesitarás una función que realmente actualice el DOM. Declara una función `updateResultsList`.

Para empezar, la función debería limpiar el HTML del elemento `resultList`.

# --hints--

Deberías definir una función `updateResultsList`.

```js
assert.isFunction(updateResultsList);
```

Tu función `updateResultsList` debería establecer `resultList.innerHTML` en una cadena vacía.
