...
還有另一種方法來寫 `forEach`。 你可以使用表達式主體 `() =>`，而不是使用塊主體 `() => {}` 進行回調。

你必須將 `if...else` 語句轉換爲表達式。 將表達式寫爲三元表達式，並對三元表達式使用單個賦值。

```js
assignment = condition ? exprIfTrue : exprIfFalse
```

將 `forEach` 回調轉換爲使用表達式體並用三元組替換語句。

# --hints--

你的函數仍應正確增加 `counts` 變量。
