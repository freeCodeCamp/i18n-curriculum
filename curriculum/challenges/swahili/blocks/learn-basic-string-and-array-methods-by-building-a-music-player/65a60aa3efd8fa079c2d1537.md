...
Hali ya pili katika mfano huu inakagua kama `a.name > b.name`. Ikiwa ni hivyo, kitendakazi hurejesha `1`, ambayo huandaa tunda la kwanza baada ya tunda la pili.
...
Ndani ya kitendakazi chako cha mwitiko, ongeza tamko lingine la `if` ili kukagua kama `a.title` ni kubwa kuliko `b.title`. Ikiwa ni hivyo, rejesha nambari `1`.

# --hints--

Unapaswa kuwa na tamko la pili la `if` ndani ya kitendakazi chako cha mwitiko.
...
Tamko lako la `if` linapaswa kukagua kama `a.title` ni kubwa kuliko `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
```

Tamko lako la `if` linapaswa kurejesha nambari `1` ikiwa `a.title` ni kubwa kuliko `b.title`.
