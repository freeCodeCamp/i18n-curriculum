...
この時点で、カードをクリックするとそれぞれのカードが示す運勢が見えるはずです。残っているのは、新しい読み取りボタンを動作させることだけです。

`newReading` メソッドの中で、`showElements()` と `hideElements()` を呼び出してください。

# --hints--

`showElements` メソッドの中で `newReading` 関数を呼び出す必要があります。

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

`hideElements` メソッドの中で `newReading` 関数を呼び出す必要があります。
