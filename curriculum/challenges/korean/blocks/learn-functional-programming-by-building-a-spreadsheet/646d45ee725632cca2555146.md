...
이제 `applyFunction`가 결과를 반환해야 합니다. `.replace()`에서 `str2` 메서드를 호출한 결과를 반환하세요. `functionCall` 정규식과 빈 콜백을 전달하세요.

# --hints--

`applyFunction` 함수는 `.replace()`에서 `str2` 메서드를 호출한 결과를 반환해야 합니다.
...
`functionCall` 호출의 첫 번째 인수로 `.replace()`을 전달해야 합니다.
...
`.replace()` 호출의 두 번째 인수로 빈 화살표 함수를 전달해야 합니다.
