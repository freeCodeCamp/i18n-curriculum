...
1. Si el producto ya está presente en el inventario, la función `addProduct` debe actualizar el valor de `quantity` sumando la cantidad pasada a la función a la `quantity` actual y registrar en la consola el nombre del producto seguido de un espacio y `quantity updated`.
...
1. Si la cantidad después de la resta es cero, `removeProduct` debe eliminar el objeto producto del inventario. Si la cantidad en el inventario no es suficiente para realizar la resta, la función `removeProduct` debe registrar en la consola la cadena `Not enough <product-name> available, remaining pieces: <product-quantity>`.
...
Si la `quantity` después de la resta es cero, `removeProduct` debe eliminar el objeto de producto del inventario.
