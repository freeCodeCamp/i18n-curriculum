...
بما أن المستخدمين سيتمكنون من خلط وحذف الأغاني من قائمة التشغيل، يجب عليك إنشاء نسخة من مصفوفة `allSongs` دون تعديل الأصل. هنا يأتي دور معامل النشر `spread`.

معامل النشر (`...`) يسمح لك بنسخ كل العناصر من مصفوفة إلى أخرى. يمكن أيضًا استخدامه لدمج عدة مصفوفات في واحدة. في المثال أدناه، تم نشر كل من `arr1` و `arr2` داخل `combinedArr`:
...
داخل كائن `userData` أنشئ خاصية `songs`. للقيمة، انشر `allSongs` داخل مصفوفة.

# --hints--

يجب أن يحتوي كائن `userData` على مفتاح يسمى `songs`.

```js
assert.property(userData, "songs");
```

يجب أن تكون قيمة خاصية `songs` هي `[...allSongs]`.
