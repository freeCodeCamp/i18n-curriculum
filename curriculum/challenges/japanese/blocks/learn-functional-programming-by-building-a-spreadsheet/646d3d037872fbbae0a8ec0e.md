...
オブジェクトの値は、文字列や数値のようなプリミティブ型である必要はありません。関数にすることもできます。

`infixToFunction` オブジェクトに `+` プロパティを追加してください。そのプロパティは、`x` と `y` というパラメータを受け取り、その2つのパラメータの合計を暗黙的に返す関数である必要があります。

`+` は英数字ではないため、プロパティ名をクォートで囲む必要があります。

# --hints--

`infixToFunction` オブジェクトには `+` プロパティが必要です。
