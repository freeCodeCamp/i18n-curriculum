...
Ili kupata wimbo uliopita, toa `1` kutoka `currentSongIndex` ya `userData?.songs` na weka thamani yake katika thabiti `previousSong`. Baada ya hapo, fanya wito wa kitendakazi `playSong` na pitisha `previousSong.id` kama hoja.
...
Unapaswa kuweka thamani ya `userData?.songs[currentSongIndex - 1]` katika thabiti ya `previousSong`.
...
Unapaswa kufanya wito wa kitendakazi `playSong` ukiwa na `previousSong.id`.
