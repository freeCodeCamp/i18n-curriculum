...
在 `.penguin-body` 元素中，添加兩個 `div` 元素，每個元素都有一個 `class` 為 `foot`。將第一個 `.foot` 設定為 `class` 為 `left`，將第二個 `.foot` 設定為 `class` 為 `right`。

# --hints--

你應該在 `.penguin-body` 中添加兩個 `div` 元素。預期有 --fcc-expected-- 個 `div` 元素，但實際找到 --fcc-actual-- 個。

```js
assert.lengthOf(document.querySelectorAll('.penguin-body > div'), 2);
```

你應該給第一個新的 `div` 一個 `class` 為 `foot`。

```js
assert.exists(document.querySelector('.penguin-body > div.foot'));
```

你應該給第二個新的 `div` 一個 `class` 為 `foot`。

```js
assert.lengthOf(document.querySelectorAll('.penguin-body > div.foot'), 2);
```

你應該給一個 `div` 一個 `class` 為 `left`。

```js
assert.exists(document.querySelector('.penguin-body > div.foot.left'));
```

你應該給另一個 `div` 一個 `class` 為 `right`。

```js
assert.exists(document.querySelector('.penguin-body > div.foot.right'));
```

你應該將 `.foot.right` 放在 `.foot.left` 之後。
