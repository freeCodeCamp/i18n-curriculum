...
Sposta la proprietà e il valore `background-color` da `.fb1` a `.fb1b`.

# --hints--

Dovresti rimuovere `background-color` da `.fb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb1")?.backgroundColor);
```

Dovresti aggiungere una `background-color` di `--building-color4` a `.fb1b`.
