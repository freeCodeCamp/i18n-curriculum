...
`div` 要素をもう 1 つ作成してください。 その中に、フォームを `submit` (送信) するための `button` をネストしてください。 このボタンには `Calculate Remaining Calories` というテキストを入れてください。

次に、フォームをクリアするために `id` を `clear` に設定した `button` を追加してください (フォームが送信されないようにする `type` 属性を忘れずに付けてください)。 このボタンには `Clear` というテキストを入れてください。
