...
這個建築物的頂部將會有另一個三角形。為頂部區塊設定 `border-top` 為 `5vh solid transparent`，並設定 `border-left` 為 `8vw`、`solid`，且使用你的建築物顏色變數作為顏色。

# --hints--

你應該為 `.fb4a` 設定一個 `border-top`，其值為 `5vh solid transparent`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4a")?.borderTop.trim(), "5vh solid transparent")
```

你應該給 `.fb4a` 一個 `border-left`，值為 `8vw solid var(--building-color1)`。
