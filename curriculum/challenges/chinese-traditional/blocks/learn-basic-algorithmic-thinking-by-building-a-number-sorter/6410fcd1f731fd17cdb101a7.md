...
在你的巢狀 `for` 迴圈中，為每次迭代添加一個 `console.log()` 呼叫，以檢查 `array`、`array[j]` 和 `array[minIndex]` 的值。你可以點擊 `Sort` 按鈕來查看你的演算法如何遍歷陣列。

然後撰寫一個 `if` 述語，檢查 `array[j]` 的值是否小於 `array[minIndex]` 的值。如果是，將 `minIndex` 設為 `j`。

# --hints--

你應該在你的巢狀 `for` 迴圈中有一個 `console.log()` 呼叫。
...
你應該將 `array` 作為傳遞給 `console.log()` 的第一個引數。
...
你應該將 `array[j]` 作為第二個引數傳遞給 `console.log()`。
...
你應該將 `array[minIndex]` 作為第三個引數傳遞給 `console.log()`。
...
你應該在你的 `console.log()` 呼叫之後有一個 `if` 述語。
...
你應該在你的 `if` 述語中有一個條件，用來檢查 `array[j]` 是否小於 `array[minIndex]`。
