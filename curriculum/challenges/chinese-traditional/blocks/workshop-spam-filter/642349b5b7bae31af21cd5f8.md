...
為了將你所學的全部整合起來，更新你的 `dearRegex`，將母音對應到相應的數字（注意 `i` 應該匹配 `1`，並且也要匹配符號 `|`），並匹配整個字、單字。

這樣，你的垃圾郵件過濾器專案就完成了。

# --hints--

你的 `dearRegex` 應該使用字元類別來匹配 `e` 或 `3`。

```js
assert.match(dearRegex.source, /\[(e3|3e)\]/);
```

你的 `dearRegex` 應該使用字元類別來匹配 `a`、`@` 或 `4`。

```js
assert.match(dearRegex.source, /\[(a@4|a4@|4a@|4@a|@a4|@4a)\]/);
```

你的 `dearRegex` 應該使用字元類別來匹配 `i`、`1` 或 `|`。

```js
assert.match(dearRegex.source, /\[(i1\||i\|1|1i\||1\|i|\|1i|\|i1)\]/);
```

你的 `dearRegex` 應該使用非捕獲群組。

```js
assert.match(dearRegex.source, /\(\?:/);
```

你的 `dearRegex` 應該使用非捕獲群組來匹配 `\s` 或 `^`。

```js
assert.match(dearRegex.source, /\(\?:(\^\|\\s|\\s\|\^)\)/);
```

你的 `dearRegex` 應該使用第二個非捕獲群組。
