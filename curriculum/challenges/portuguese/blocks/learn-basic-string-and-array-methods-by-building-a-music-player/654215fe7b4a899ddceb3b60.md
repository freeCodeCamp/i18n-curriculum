...
No momento, o `songsHTML` é um array. Se você tentasse exibir isso como está, veria as músicas separadas por vírgulas. Este não é o resultado desejado porque você quer exibir as músicas como uma lista. Para corrigir isso, você precisará unir o array em uma única string usando o método <dfn>join()</dfn>.

O método `join()` é usado para concatenar todos os elementos de um array em uma única string. Ele recebe um parâmetro opcional chamado `separator` que é usado para separar cada elemento do array. Por exemplo:
...
Encadeie o método `join()` ao seu método `map()` e passe uma string vazia como separador.

Para encadear múltiplos métodos, você pode chamar o método `join()` no resultado do método `map()`. Por exemplo:
...
Você deve adicionar `join("")` ao código existente.
