...
En este taller, continuarás practicando con TypeScript construyendo una aplicación de adivinación.

Comienza declarando una interfaz llamada `Card`. Dentro de la interfaz `Card`, agrega las siguientes propiedades: `name`, `name_short` y `value`. Las propiedades `name` y `name_short` deben tener los tipos establecidos en `string` y `value` debe tener un tipo unión establecido en `string | number`.

# --hints--

Debes declarar una interfaz llamada `Card`.
...
Tu interfaz `Card` debe tener una propiedad `name` de tipo `string`.
...
Tu interfaz `Card` debe tener una propiedad `name_short` de tipo `string`.
...
Tu interfaz `Card` debe tener una propiedad `value` con un tipo unión de `string` y `number`.
