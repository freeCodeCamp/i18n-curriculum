...
title: Construir un gráfico de barras
challengeType: 25
...
En este proyecto usarás los datos de `https://cdn.freecodecamp.org/curriculum/labs/data/bar-chart/GDP-data.json` para construir un gráfico de barras.

Las pruebas requieren que los ejes se generen usando la propiedad axis de D3, que crea automáticamente marcas (ticks) a lo largo del eje. Estas marcas son necesarias para pasar las pruebas de D3 porque sus posiciones se usan para determinar la alineación de los elementos graficados.

**Objetivo:** Cumple las historias de usuario que aparecen a continuación y pasa todas las pruebas para completar el laboratorio.

**Historias de usuario:**

1. Tu gráfico debe tener un título con un `id="title"` correspondiente.
1. Tu gráfico debe tener un eje x con un elemento `g` y un `id="x-axis"` correspondiente.
1. Tu gráfico debe tener un eje y con un elemento `g` y un `id="y-axis"` correspondiente.
1. Ambos ejes deben contener múltiples etiquetas de marcas, cada una con un `class="tick"` correspondiente.
1. Tu gráfico debe tener un elemento `rect` para cada punto de datos con un `class="bar"` correspondiente que muestre los datos.
1. Cada `.bar` debe tener los atributos `data-date` y `data-gdp` que contengan valores `date` y `GDP`.
1. Los atributos `.bar` de los elementos `data-date` deben coincidir con el orden de los datos proporcionados.
1. Los atributos `.bar` de los elementos `data-gdp` deben coincidir con el orden de los datos proporcionados.
1. La altura de cada elemento `.bar` debe representar con precisión el `GDP` correspondiente de los datos.
1. El atributo `data-date` y su elemento `.bar` correspondiente deben alinearse con el valor correspondiente en el eje x.
1. El atributo `data-gdp` y su elemento `.bar` correspondiente deben alinearse con el valor correspondiente en el eje y.
1. Debes poder pasar el cursor sobre un área y ver una información emergente (tooltip) con un `id="tooltip"` correspondiente que muestre más información sobre el área.
1. Tu tooltip debe tener un atributo `data-date` que corresponda al `data-date` del área activa.
...
El gráfico debe tener un eje x con un elemento `g` y un id `x-axis` correspondiente.

```js
assert.isNotEmpty(document.querySelector('g#x-axis'))
```

El gráfico debe tener un eje y con un elemento `g` y un id `y-axis` correspondiente.

```js
assert.isNotEmpty(document.querySelector('g#y-axis'))
```

El eje x debe contener múltiples etiquetas de marcas, cada una con el `class="tick"` correspondiente.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

El eje y debe contener múltiples etiquetas de marcas, cada una con el `class="tick"` correspondiente.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

Tu gráfico debe tener un elemento `rect` para cada punto de datos con un `class="bar"` correspondiente que muestre los datos.

```js
assert.lengthOf(document.querySelectorAll('rect.bar'), GDPDataJson.data.length);
```

Cada barra debe tener los atributos `data-date` y `data-gdp` que contengan valores de fecha y PIB.
...
Los atributos `data-date` de los elementos barra deben coincidir con el orden de los datos proporcionados.
...
Los atributos `data-gdp` de los elementos barra deben coincidir con el orden de los datos proporcionados.
...
La altura de cada elemento barra debe representar con precisión el PIB correspondiente de los datos.
...
El atributo `data-date` y su elemento barra correspondiente deben alinearse con el valor correspondiente en el eje x. Puedes usar `d3.scaleTime()` para crear una escala temporal para el eje x.
...
El atributo `data-gdp` y su elemento barra correspondiente deben alinearse con el valor correspondiente en el eje y. Puedes usar `d3.scaleLinear()` para crear una escala lineal para el eje y.
...
Al pasar el cursor sobre un área, tu gráfico de barras debe mostrar un tooltip con un `id="tooltip"` correspondiente que muestre más información sobre el área.
...
El tooltip debe tener un atributo `data-date` que corresponda al `data-date` del área activa.
