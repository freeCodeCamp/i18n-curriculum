...
Chini ya kipengele cha `.face.right`, ongeza kipengele cha `div` chenye `class` ya `chin`.

# --hints--

Unapaswa kuongeza kipengele kimoja cha `div` ndani ya `.penguin-head`. Inatarajiwa kuwa na vipengele vya `.penguin-head > div`, lakini vilipatikana --fcc-actual--.

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 3);
```

Unapaswa kumpa `div` `class` ya `chin`.
