...
Для вашого елемента `textarea` додайте атрибут `id` і атрибут `name` зі значенням `"comments"`.

Для атрибута `cols` встановіть значення `30`, а для атрибута `rows` встановіть значення `10`.

# --hints--

Ваш елемент `textarea` має мати атрибут `id`, встановлений у `"comments"`.

```js
assert.exists(document.querySelector('textarea#comments'));
```

Ваш елемент `textarea` має мати атрибут `name`, встановлений у `"comments"`.

```js
assert.exists(document.querySelector('textarea[name="comments"]'));
```

Ваш елемент `textarea` має мати атрибут `cols`, встановлений у `"30"`.

```js
assert.exists(document.querySelector('textarea[cols="30"]'));
```

Ваш елемент `textarea` має мати атрибут `rows`, встановлений у `"10"`.
