...
`button` 요소 안에 `span` 요소 두 개를 더 만드세요.

첫 번째 `span` 요소는 `class`의 `playlist-song-artist`를 가져야 합니다. `span` 태그 사이에 `${song.artist}`를 추가하세요.

두 번째 `span` 요소는 `class`의 `playlist-song-duration`를 가져야 합니다. `span` 태그 사이에 `${song.duration}`를 추가하세요.

# --hints--

기존 `button` 요소와 그 내용을 수정하지 마세요.
...
`span` 클래스를 가진 `playlist-song-artist` 요소를 만들어야 합니다.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

`span` 클래스를 가진 `playlist-song-artist` 요소는 `song.artist`를 내용으로 포함해야 합니다.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

`span` 클래스를 가진 `playlist-song-duration` 요소를 만들어야 합니다.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

`span` 클래스를 가진 `playlist-song-duration` 요소는 `song.duration`를 내용으로 포함해야 합니다.
