...
`inner` 함수에서 `idToText`를 인수로 하여 `character + num`을 호출한 결과를 반환하세요.

# --hints--

`inner` 함수는 명시적인 return을 사용해야 합니다.
...
`inner` 함수는 `idToText` 함수를 호출한 결과를 반환해야 합니다.
...
`character + num` 함수에 `idToText`를 인수로 전달해야 합니다.
