...
Primero necesitas manejar los operadores de mayor precedencia. Declara una variable `noHigh` y asígnale el resultado de la llamada a `highPrecedence()` con `str` como argumento.

# --hints--

Debes declarar una variable `noHigh`.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*(?:var|let|const)\s+noHigh\s*=/);
```

Debes usar `const` para declarar tu variable `noHigh`.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=/);
```

Debes asignar a `noHigh` el resultado de la llamada a `highPrecedence()`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(/);
```

Debes pasar `str` como argumento a tu llamada a `highPrecedence()`.
