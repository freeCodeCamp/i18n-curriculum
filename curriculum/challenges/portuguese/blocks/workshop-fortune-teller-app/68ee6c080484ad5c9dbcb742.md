...
Agora você vai começar a criar os métodos.

Crie um método chamado `singleCardSelected`. Dentro dele, faça a chamada da função `hideElements` e passe as seguintes props: `singleCardBtn`, `multipleCardsBtn`, `multipleCard`, `text` e `headerTitle`.

Lembre-se de que você deve acessar todas elas usando `this.elements`.

# --hints--

Você deve definir um método chamado `singleCardSelected`.
...
Dentro de `singleCardSelected`, você deve fazer a chamada da função `hideElements`.
...
Você deve passar `singleCardBtn` para a função `hideElements`.
...
Você deve passar `multipleCardsBtn` para a função `hideElements`.
...
Você deve passar `multipleCard` para a função `hideElements`.
...
Você deve passar `text` para a função `hideElements`.
...
Você deve passar `headerTitle` para a função `hideElements`.
