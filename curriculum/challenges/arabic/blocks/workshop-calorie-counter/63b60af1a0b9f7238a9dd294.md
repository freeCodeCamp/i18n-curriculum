...
تحتاج قائمة الاختيار إلى خيارات لكل من عناصر `fieldset` الخاصة بالطعام والتمرين التي أنشأتها في الخطوات السابقة. استخدم عنصر `option` لإنشاء خيار جديد لكل `fieldset`. يجب أن تكون الخاصية `value` لكل خيار هي `id` الخاص بـ `fieldset`، ونص كل خيار هو نص `legend`.
