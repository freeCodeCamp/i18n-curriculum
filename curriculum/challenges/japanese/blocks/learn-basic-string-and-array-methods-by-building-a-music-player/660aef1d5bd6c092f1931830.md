...
通常の関数と同様に、アロー関数も値を返すことができます。

こちらは、アロー関数が 2 つの数字を掛け算した結果を返す例です。
...
では新しい変数 `addTwoNumbers` を作成し、アロー関数を割り当ててください。 このアロー関数は `num1` と `num2` という 2 つの仮引数を取るようにしてください。 関数の本体内で、`num1` と `num2` を足し算する式を返してください。

そして `addTwoNumbers` 関数の後にコンソール文を追加してください。 そのコンソール文の括弧内で `addTwoNumbers` 関数を呼び出し、実引数として数字 `3` と `4` を渡してください。
...
`addTwoNumbers` 関数にはアロー関数構文を使用してください。

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

`addTwoNumbers` 関数は `num1` と `num2` という 2 つの仮引数を持つようにしてください。

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

`addTwoNumbers` 関数は `num1` と `num2` を足し算する式を返すようにしてください。

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

`console.log(addTwoNumbers(3, 4));` という文が必要です。
