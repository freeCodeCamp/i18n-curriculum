...
Dovressti avere una nuova `@media` query per `only screen` con una `max-width` di `550px`. Dovrebbe trovarsi dopo le due precedenti.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

La nuova regola `@media` dovrebbe avere un selettore `.hero-title`, un selettore `.hero-subtitle, .author, .quote, .list-title`, un selettore `.social-icons` e un selettore `.text`. I selettori dovrebbero essere in questo ordine.
...
Il selettore `.hero-title` dovrebbe avere una proprietà `font-size` impostata su `6rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[0]?.style?.fontSize === '6rem');
```

Il selettore `.hero-subtitle, .author, .quote, .list-title` dovrebbe avere una proprietà `font-size` con il valore `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[1]?.style?.fontSize === '1.8rem');
```

Il selettore `.social-icons` dovrebbe avere una proprietà `font-size` impostata a `2rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize === '2rem');
```

Il selettore `.text` dovrebbe avere una proprietà `font-size` impostata a `1.6rem`.
