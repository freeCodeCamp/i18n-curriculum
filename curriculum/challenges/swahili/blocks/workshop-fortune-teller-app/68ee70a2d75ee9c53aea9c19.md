...
Ndani ya `map`, rudisha kitendakazi cha `renderCard` kwa kupitisha `type`, `isReversed`, `name_short`, na `img` kama hoja, na ongeza njia ya `join` kwenye `map` kwa mfuatano wa herufi tupu.

# --hints--

Unapaswa kurudisha kitendakazi cha `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

Unapaswa kupitisha `type` kama hoja ya kwanza katika kitendakazi cha `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

Unapaswa kupitisha `isReversed` kama hoja ya pili katika kitendakazi cha `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?isReversed\s*/);
```

Unapaswa kupitisha `card.name_short` kama hoja katika kitendakazi cha `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*/);
```

Unapaswa kupitisha `card.img` kama hoja katika kitendakazi cha `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

Unapaswa kuambatanisha `join("")` kwenye njia ya `map`.
