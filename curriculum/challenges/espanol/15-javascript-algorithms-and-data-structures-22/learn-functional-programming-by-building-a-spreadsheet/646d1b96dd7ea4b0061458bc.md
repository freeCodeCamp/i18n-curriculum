...
Puedes pasar una función <dfn>referencia</dfn> como parámetro de devolución de llamada. Una referencia de función es un nombre de función sin los paréntesis. Por ejemplo:
...
El método `.map()` aquí llamará a la función `myFunc`, pasando los mismos argumentos que toma un callback de `.map()`. El primer argumento es el valor del arreglo en la iteración actual, así que `newArray` sería `[value: 1, value: 2, value: 3]`.

Pasa una referencia a tu función `elemValue` como el callback en tu método `.map()`.

# --hints--

No deberías llamar a tu función `elemValue`.
...
Deberías pasar una referencia a `elemValue` como el callback en tu método `.map()`.
