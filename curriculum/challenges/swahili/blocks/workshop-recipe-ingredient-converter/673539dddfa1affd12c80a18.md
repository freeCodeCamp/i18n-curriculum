...
Utahitaji kujaribu kitendakazi chako kipya cha curried. Tangaza mabadiliko ya `servingsResult`, na weka thamani yake matokeo ya kurekebisha kutoka kwa kiasi cha msingi cha 4 hadi kwa sehemu ya 6.

Andika thamani hiyo kwenye konsoli.

# --hints--

Unapaswa kutangaza mabadiliko ya `servingsResult`.

```js
assert.isDefined(servingsResult);
```

Mabadiliko yako ya `servingsResult` yanapaswa kuwa na matokeo ya wito wa `adjustQuantity(4)(6)`.

```js
assert.strictEqual(servingsResult, 24);
```

Unapaswa kuandika thamani yako ya `servingsResult`.
