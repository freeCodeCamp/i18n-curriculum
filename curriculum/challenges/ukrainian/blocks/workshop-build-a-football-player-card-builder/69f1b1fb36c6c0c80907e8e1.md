...
Всередині елемента `preview-panel` додайте елемент `p` типу `className` з `preview-label` і текстом `Live Preview`, ще один елемент `p` типу `className` з `preview-hint` і текстом `Updates as you type`, а також `div` з пропом `className`, встановленим у шаблонний рядок `preview-box tier-${getPlayerTier(player.overallRating)}`.

Всередині останнього `div` перемістіть компонент `PlayerCard` і його проп.

# --hints--

Ваш елемент `div.preview-panel` має містити `p` з пропом `className` типу `preview-label` і текстом `Live Preview`.
...
Ваш `div.preview-panel` має містити `p` з пропом `className` типу `preview-hint` і текстом `Updates as you type`.
...
Ваш `div.preview-panel` має містити `div` з пропом `className` типу `preview-box` і динамічним рівнем `className`, префіксованим `tier-`.
...
Ваш `PlayerCard` має бути відтворений всередині елемента `div` з пропом `className` типу `preview-box`.
