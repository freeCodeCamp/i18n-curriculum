...
Als Nächstes müssen Sie den nächsten Song in der Wiedergabeliste abrufen. Dazu müssen Sie den Index des aktuellen Songs ermitteln und dann `1` dazu addieren.

Erstellen Sie eine Konstante namens `nextSong` und weisen Sie ihr `userData?.songs[currentSongIndex + 1]` zu.

Rufen Sie zuletzt die `playSong`-Funktion auf und übergeben Sie `nextSong.id` als Argument.

# --hints--

Sie sollten die bestehende `if`-Anweisung, ihren `else`-Block und deren Inhalt nicht verändern.
...
Sie sollten `userData?.songs[currentSongIndex + 1]` einer `nextSong`-Konstante zuweisen.
...
Sie sollten die `playSong`-Funktion mit `nextSong.id` aufrufen.
