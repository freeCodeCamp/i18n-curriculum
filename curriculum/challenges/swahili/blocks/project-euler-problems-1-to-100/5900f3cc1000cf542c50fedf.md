...
Su Doku (kwa Kijapani maana yake ni *mahali pa nambari*) ni jina linalotolewa kwa dhana maarufu ya fumbo. Asili yake haijulikani, lakini sifa inapaswa kumilikiwa na Leonhard Euler aliyeanzisha fumbo linalofanana, na gumu zaidi, linaloitwa Latin Squares. Lengo la fumbo za Su Doku, hata hivyo, ni kubadilisha sehemu tupu (au sifuri) katika gridi ya 9 kwa 9 kwa njia ambayo kila safu, nguzo, na kisanduku cha 3 kwa 3 kina kila nambari kutoka 1 hadi 9. Hapo chini kuna mfano wa gridi ya fumbo ya kuanzia na gridi yake ya suluhisho.
...
Fumbo la Su Doku lililojengwa vizuri lina suluhisho la kipekee na linaweza kutatuliwa kwa mantiki, ingawa inaweza kuwa muhimu kutumia njia za "kisia na jaribu" ili kuondoa chaguzi (kuna maoni mengi yanayopingana kuhusu hili). Ugumu wa utafutaji huamua ugumu wa fumbo; mfano ulio hapo juu unachukuliwa kuwa rahisi kwa sababu unaweza kutatuliwa kwa utambuzi wa moja kwa moja.

Safu ya data `puzzlesArr` ina mfuatano tofauti wa fumbo za Su Doku zenye viwango tofauti vya ugumu, lakini zote zina suluhisho la kipekee.

Kwa kutatua fumbo zote katika `puzzlesArr`, pata jumla ya nambari za tarakimu 3 zilizopo kona ya juu kushoto ya kila gridi ya suluhisho; kwa mfano, 483 ni nambari ya tarakimu 3 iliyopo kona ya juu kushoto ya gridi ya suluhisho iliyo hapo juu.
...
`suDoku(testPuzzles1)` inapaswa kurudisha nambari.

```js
assert(typeof suDoku(_testPuzzles1) === 'number');
```

`suDoku(testPuzzles1)` inapaswa kurudisha `1190`.

```js
assert.strictEqual(suDoku(_testPuzzles1), 1190);
```

`suDoku(testPuzzles2)` inapaswa kurudisha `24702`.
