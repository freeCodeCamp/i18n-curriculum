...
在遊戲的最後部分，您需要創建一個算法來檢查順子是否存在。 一個小順子是指四個骰子按照任意順序具有連續的值（例如， 在擲 `41423` 時，我們得到 `1234`），結果是 `30` 分。 一個大順子是指五個骰子按照任意順序具有連續的值（例如， 在擲 `35124` 時，我們得到 `12345`），結果是 `40` 分。

聲明一個 `checkForStraights` 函數，接受一個數組作爲參數。 如果用戶得到大順子，請用 `40` 的分數更新第五個單選按鈕。 如果用戶得到小順子，請用 `30` 的分數更新第四個單選按鈕。 無論如何，它應該始終更新最後一個單選按鈕以顯示分數 0，並具​​有正確的屬性。

當點擊 `rollDiceBtn` 時，調用你的 `checkForStraights` 函數，以完成你的骰子游戲！

# --hints--

你的 `checkForStraights` 變量應該是一個函數。

```js
assert.isFunction(checkForStraights);
```

如果擲出了小順子，你的 `checkForStraights` 函數應該啓用第四個單選按鈕，將值設爲 `30`，並更新顯示文本爲 `, score = 30`，最後禁用第五個單選按鈕。
...
如果擲出了大順子，你的 `checkForStraights` 函數應該啓用第四個單選按鈕，將值設爲 `30`，並更新顯示文本爲 `, score = 30`。 此外，該函數應啓用第五個單選按鈕，將值設置爲 `40`並將顯示的文本更新爲 `, score = 40`。
...
如果沒有擲出順子，你的 `checkForStraights` 函數不應啓用第四或第五個單選按鈕。
...
如果沒有擲出順子，你的 `checkForStraights` 函數應該啓用最後一個單選按鈕，將值設置爲 `0` ，並更新顯示文本爲 `, score = 0`。
...
當點擊 `rollDiceBtn` 時，你應該調用 `checkForStraights` 函數。
