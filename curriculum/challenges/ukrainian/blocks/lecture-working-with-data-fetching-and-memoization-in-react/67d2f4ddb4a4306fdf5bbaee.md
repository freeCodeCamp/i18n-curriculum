...
Це гарантує, що функція буде мемоізована шляхом кешування результату. Навіть якщо компонент `ExpensiveSquare` все ще повторно відтворюється щоразу, коли оновлюється стан `timer` у батьківському компоненті, обчислення `calculateSquare` виконується лише під час ініціального відтворення та коли змінюється `num`.
