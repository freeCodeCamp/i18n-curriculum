...
上部のスペースは良くなりました。メニューの下部にある住所の下のスペースは、メニューの上部や`h1`要素のスペースより少し大きくなっています。

住所`p`要素の下のデフォルトのマージンスペースを減らすために、`address`という名前のクラスセレクターを作成し、`5px`プロパティに`margin-bottom`の値を設定してください。

# --hints--

`.address`セレクターを追加してください。
...
`.address`セレクターには`margin-bottom`プロパティを`5px`に設定してください。
