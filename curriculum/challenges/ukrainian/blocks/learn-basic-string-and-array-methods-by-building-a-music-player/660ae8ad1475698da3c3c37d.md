...
Щоб викликати іменований вираз стрілкової функції, ви можете звернутися до функції за її іменем.

```js
exampleArrowFunction();
```

Під визначенням вашої `printGreeting` функції викличте цю функцію. Відкрийте консоль, щоб побачити вихідні дані.

# --hints--

Ви повинні викликати вашу `printGreeting` функцію.

```js
assert.match(code, /printGreeting\(\);?/);
```

Виклик вашої `printGreeting` функції має бути розташований під визначенням `printGreeting` функції.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

Виклик вашої `printGreeting` функції має вивести `Hello there!` у консоль.
