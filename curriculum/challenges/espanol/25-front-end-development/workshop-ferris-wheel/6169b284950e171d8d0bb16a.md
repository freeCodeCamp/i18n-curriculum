...
Finalmente, crea un nuevo selector `75%` entre tus selectores `50%` y `100%`. Dale a este nuevo selector una propiedad `background-color` establecida en `yellow`.

Con eso, tu animación es mucho más suave y tu rueda de la fortuna está completa.

# --hints--

Debes crear un nuevo selector `75%` en tu regla `@keyframes cabins`.
...
Tu selector `75%` debe estar entre tus selectores `50%` y `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText, '75%');
```

Tu selector `75%` debe tener una propiedad `background-color` configurada en `yellow`.
