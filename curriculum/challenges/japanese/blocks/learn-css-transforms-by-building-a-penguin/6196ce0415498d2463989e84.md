...
山脈の効果を出すために、`div`のすぐ後に新しい`.left-mountain`を作成してもう一つ山を追加し、新しい`div`に`class`の`back-mountain`を与えてください。
