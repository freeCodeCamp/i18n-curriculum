...
由於你已將每個動畫影格的時間存儲在 `addElDelay`，你可以使用該值搭配 `setTimeout()` 來設定將元素添加到 DOM 的延遲時間。

在 `.forEach()` 方法的回呼函式主體內，為你的 `setTimeout()` 函式添加。將一個空的回呼函式作為第一個引數，並將 `obj.addElDelay` 作為第二個引數傳入。

# --hints--

你應該在 `.forEach()` 方法的回呼函式主體內添加 `setTimeout()` 函式（程式）。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\([\s\S]+\{\s*setTimeout\(/);
```

你應該將一個空的回呼函式作為第一個引數傳入 `setTimeout()` 函式。
...
你的 `setTimeout()` 函式（程式）回呼函式（程式）的主體應該是空的。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\([\s\S]+\{\s*setTimeout\([\s\S]+\{\s*\}/);
```

你應該將 `obj.addElDelay` 作為第二個引數傳入 `setTimeout()` 函式（程式）。
