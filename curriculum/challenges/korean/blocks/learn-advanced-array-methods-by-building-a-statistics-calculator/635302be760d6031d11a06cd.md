...
마지막으로 `calculate` 함수를 다른 함수들처럼 표준 편차 로직을 포함하도록 업데이트하세요.

축하합니다! 프로젝트가 완료되었습니다.

# --hints--

`calculate` 함수에는 `standardDeviation`의 결과로 설정된 `getStandardDeviation(numbers)` 변수가 있어야 합니다.

```js
assert.match(calculate.toString(), /standardDeviation\s*=\s*getStandardDeviation\(\s*numbers\s*\)/);
```

`textContent` 요소의 `#standardDeviation`를 `standardDeviation` 변수로 업데이트해야 합니다.
