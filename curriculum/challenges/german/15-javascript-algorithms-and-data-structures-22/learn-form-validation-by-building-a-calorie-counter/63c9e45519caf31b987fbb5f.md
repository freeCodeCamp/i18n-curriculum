...
Your `getCaloriesFromInputs` function will set the global error flag `isError` to `true` if an invalid input is detected. Füge eine `if`-Anweisung zu deiner `calculateCalories`-Funktion hinzu, die den Wahrheitsgehalt deines globalen Fehler-Flags prüft, und wenn er wahrheitsgemäß ist, verwende `return`, um die Funktionsausführung zu beenden.
