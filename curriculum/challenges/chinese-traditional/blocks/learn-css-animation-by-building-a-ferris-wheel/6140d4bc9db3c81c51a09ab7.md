...
延續樣式，選取以下 `.cabin` 元素並對它們套用特定規則：

- 第二個 `.cabin` 應該將 `right` 屬性設為 `17%`，並將 `top` 屬性設為 `93.5%`。
- 第三個 `.cabin` 應該將 `right` 屬性設為 `67%`，並將 `top` 屬性設為 `93.5%`。
- 第四個 `.cabin` 應該將 `left` 屬性設為 `-8.5%`，並將 `top` 屬性設為 `50%`。
- 第五個 `.cabin` 應該將 `left` 屬性設為 `17%`，並將 `top` 屬性設為 `7%`。
- 第六個 `.cabin` 應該將 `right` 屬性設為 `17%`，並將 `top` 屬性設為 `7%`。
...
你的 `.cabin:nth-of-type(2)` 選擇器應該有一個 `right` 屬性設定為 `17%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cabin:nth-of-type(2)')?.right === '17%');
```

你的 `.cabin:nth-of-type(2)` 選擇器應該有一個 `top` 屬性設定為 `93.5%`。
...
你的 `.cabin:nth-of-type(3)` 選擇器應該有一個 `right` 屬性設定為 `67%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cabin:nth-of-type(3)')?.right === '67%');
```

你的 `.cabin:nth-of-type(3)` 選擇器應該有一個 `top` 屬性設定為 `93.5%`。
...
你的 `.cabin:nth-of-type(4)` 選擇器應該有一個 `left` 屬性設定為 `-8.5%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cabin:nth-of-type(4)')?.left === '-8.5%');
```

你的 `.cabin:nth-of-type(4)` 選擇器應該有一個 `top` 屬性設定為 `50%`。
...
你的 `.cabin:nth-of-type(5)` 選擇器應該有一個 `left` 屬性設定為 `17%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cabin:nth-of-type(5)')?.left === '17%');
```

你的 `.cabin:nth-of-type(5)` 選擇器應該有一個 `top` 屬性設定為 `7%`。
...
你的 `.cabin:nth-of-type(6)` 選擇器應該有一個 `right` 屬性設定為 `17%`。
...
你的 `.cabin:nth-of-type(6)` 選擇器應該有一個 `top` 屬性設定為 `7%`。
