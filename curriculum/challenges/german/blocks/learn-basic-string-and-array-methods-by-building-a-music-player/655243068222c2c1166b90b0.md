...
Innerhalb der `playNextSong`-Funktion erstellen Sie eine `if`-Anweisung, um zu prüfen, ob das `currentSong` von `userData` strikt gleich `null` ist. Damit wird überprüft, ob im `userData`-Objekt gerade kein Song abgespielt wird.
...
Sie sollten die bestehende `playNextSong`-Funktion und deren Inhalt nicht verändern.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Sie sollten eine `if`-Anweisung mit der Bedingung `userData?.currentSong === null` erstellen.
...
Sie sollten die `playSong`-Funktion mit `userData?.songs[0].id` innerhalb der `if`-Anweisung aufrufen.
