...
`.cabin` ルールの `animation` プロパティを `cabins 10s linear infinite` に設定してください。 これは、`animation-name`、`animation-duration`、`animation-timing-function`、`animation-iteration-count` プロパティをこの順番で設定します。

# --hints--

`.cabin` セレクターの `animation` プロパティを `cabins 10s linear infinite` に設定する必要があります。
