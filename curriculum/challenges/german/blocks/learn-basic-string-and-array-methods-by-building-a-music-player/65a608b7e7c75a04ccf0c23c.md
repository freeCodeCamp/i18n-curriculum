...
Jetzt, wo die Liste der Songs auf dem Bildschirm angezeigt wird, wäre es schön, sie alphabetisch nach Titel zu sortieren. Sie könnten das `allSongs`-Array manuell aktualisieren, aber JavaScript hat eine Array-Methode, die Sie verwenden können, die <dfn>sort()</dfn> heißt.

Die `sort()`-Methode wandelt Elemente eines Arrays in Strings um und sortiert sie an Ort und Stelle basierend auf ihren Werten in der `UTF-16`-Kodierung.
...
Fügen Sie innerhalb Ihrer `sortSongs`-Funktion die `sort()`-Methode zu `userData?.songs` hinzu.

# --hints--

Sie sollten die `sort()`-Methode zu `userData?.songs` hinzufügen.
