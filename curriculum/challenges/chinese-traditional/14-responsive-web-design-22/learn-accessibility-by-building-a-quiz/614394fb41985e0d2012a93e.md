...
問題編號不夠詳細。 對於視力受損的用戶來說尤其如此。 解決這個問題的方法之一是在元素中添加只有屏幕閱讀器才能讀取的文本，而無需在元素中添加可見文本。

爲每個 `h3` 元素添加一個 `class` 爲 `sr-only` 的 `span` 元素。

# --hints--

你應該在第一個 `h3` 元素中添加一個 `span` 元素。

```js
assert.exists(document.querySelector('h3 > span'));
```

你應該給第一個 `span` 元素設置 `class` 爲 `sr-only`。
...
你應該給第二個 `span` 元素設置 `class` 爲 `sr-only`。
