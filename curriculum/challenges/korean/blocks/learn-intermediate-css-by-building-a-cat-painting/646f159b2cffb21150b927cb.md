...
클래스 선택자를 사용해 `.cat-whisker-left-middle` 요소를 선택하세요. 그런 다음 `position` 속성을 `absolute`로, `top`을 `127px`로, `left`를 `52px`로 설정해 위치를 조정하세요.

# --hints--

`.cat-whisker-left-middle` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-middle'))
```

`.cat-whisker-left-middle` 선택자에 `position` 속성을 `absolute`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-middle')?.position === 'absolute')
```

`.cat-whisker-left-middle` 선택자에 `top` 속성을 `127px`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-middle')?.top === '127px')
```

`.cat-whisker-left-middle` 선택자에 `left` 속성을 `52px`로 설정해야 합니다.
