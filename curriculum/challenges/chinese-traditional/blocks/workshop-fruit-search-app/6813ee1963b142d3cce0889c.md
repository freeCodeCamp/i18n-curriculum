...
在 `try` 區塊內，使用 `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` URL 呼叫 `fetch` 函式，並將結果存放在名為 `response` 的變數中。由於你在 `async` 函式內，應該在 `fetch` 前使用 `await`。

# --hints--

你應該使用 `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` 呼叫 `fetch`，並將結果儲存在 `response` 變數中。
