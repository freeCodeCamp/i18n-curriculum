...
**Objetivo:**  
Neste exercício, você vai praticar adicionar CSS a um arquivo HTML usando os três métodos: CSS externo, CSS interno e CSS inline. Você deve usar apenas seletores de tipo para este exercício ao adicionar estilos através dos métodos externo e interno. Você também deve usar palavras-chave para cores (por exemplo, "blue") em vez de usar valores `RGB` ou `HEX`.
