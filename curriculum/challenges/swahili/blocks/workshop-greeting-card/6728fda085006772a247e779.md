...
Toa `.card` `max-width` ya `400px`, `padding` ya `40px` upande wote, na `margin` ya `0` kwa juu na chini na `auto` kwa kushoto na kulia (tumia sifa ya ufupisho).

# --hints--

`.card` inapaswa kuwa na `max-width` ya `400px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("max-width"), "400px");
```

`.card` inapaswa kuwa na `padding` ya `40px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("padding"), "40px");
```

`.card` inapaswa kuwa na `margin` ya `0 auto`.
