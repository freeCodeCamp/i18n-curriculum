...
Attualmente, se digiti negli elementi `input` o `textarea`, non riuscirai a vedere il testo. Inoltre, la loro altezza è troppo piccola per essere facilmente utilizzabile.

Risolvi questo impostando `color` su `#ffffff` e impostando il loro `min-height` su `2em`.
