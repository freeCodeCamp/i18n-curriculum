...
占いアプリは複数のカードをレンダリングすることに対応しているので、複数のカードをレンダリングできるメソッドが必要です。`multipleCardSelected` という名前のメソッドを宣言してください。

メソッドの中で、`hideElements` 関数を呼び出し、`singleCard`、`singleCardBtn`、`multipleCardsBtn`、`headerTitle` のプロパティを渡してください。これらは `this.elements` からアクセスする必要があることを忘れないでください。

# --hints--

`multipleCardSelected` メソッドを宣言する必要があります。
...
`hideElements` メソッドの中で `multipleCardSelected` 関数を呼び出す必要があります。
...
`singleCard` 関数の最初の引数として `hideElements` を渡す必要があります。
...
`singleCardBtn` 関数に `hideElements` を引数として渡す必要があります。
...
`multipleCardsBtn` 関数に `hideElements` を引数として渡す必要があります。
...
`headerTitle` 関数に `hideElements` を引数として渡す必要があります。
