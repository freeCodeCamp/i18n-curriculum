...
Atualmente sua função `range` retorna um array com o comprimento correto, mas todos os valores são o valor de `start`. Para corrigir isso, encadeie o método `.map()` ao seu método `.fill()`.
