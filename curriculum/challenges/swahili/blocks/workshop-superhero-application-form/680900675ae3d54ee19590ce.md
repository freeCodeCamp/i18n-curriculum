...
Kutakuwa na visanduku sita vya kuchagua, kwa hivyo ni bora pia kuunda safu ya data kwa ajili yao na kupita ndani yake kuonyesha kama visanduku vya kuchagua. Kwa ajili hiyo, safu ya `powersOptions` imeandaliwa kwa ajili yako.

Tumia kitendakazi cha mshale kupita ndani ya safu ya `powersOptions` ukitumia kigezo cha `power`. Ndani ya map, unda kipengele cha `label` chenye `key` ya `power`, na maandishi ya `Hello`.

Baada ya hapo, utakuwa unaona maandishi sita ya `Hello`. Utajaza vipengele vya safu hiyo katika hatua inayofuata.

# --hints--

Unapaswa kupita ndani ya `powersOptions`.

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\(/)
```

Unapaswa kutumia `power` kama kigezo cha map yako.

```js
assert.match(code, /{\s*powersOptions\s*\.\s*map\s*\(\s*(power|\(\s*power\s*\))\s*=>/)
```

Unapaswa kuunda kipengele cha `label` chenye `key` ya `power`.

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\([\s\S]*<\s*label\s+key\s*=\s*\{\s*power\s*\}\s*>[\s\S]*<\s*\/\s*label\s*>/)
```

Kipengele chako cha `label` kinapaswa kuwa na maandishi ya `Hello`.
