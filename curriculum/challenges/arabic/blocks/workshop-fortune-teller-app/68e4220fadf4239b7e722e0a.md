...
بما أن تطبيق fortune يحتوي فقط على ثلاثة أزمنة هي Past و Present و Future، أعلن عن تعداد (enum) باسم `DrawingType` بثلاث خصائص: `Past` و `Present` و `Future`. يجب أن تحتوي كل خاصية على قيمة سلسلة نصية تطابق اسمها.

# --hints--

يجب أن يكون لديك `enum` باسم `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

يجب أن يحتوي التعداد على خاصية `Past` بقيمة سلسلة نصية `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

يجب أن يحتوي التعداد على خاصية `Present` بقيمة سلسلة نصية `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

يجب أن يحتوي التعداد على خاصية `Future` بقيمة سلسلة نصية `future`.
