...
`h1 .flex` セレクターで、フレックスコンテナーを選択します。 フレックスボックスレイアウトを有効にするため、`display` プロパティを `flex` に設定してください。 次に `flex-direction` プロパティを `column-reverse` に設定してください。これで、ネストされている要素が下から上に表示されます。 最後に、要素の間に余白を作るため、`gap` プロパティを `1rem` に設定してください。
