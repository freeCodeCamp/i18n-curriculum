...
现在，如果在 `input` 或者 `textarea` 元素内输入，会发现无法看见文字。 另外，它们的高度也太小了，难以使用。

通过将 `color` 设置为 `#ffffff` 并将其 `min-height` 设置为 `2em` 来修复此问题。

# --hints--

你应该将 `color` 设置为 `#ffffff`。
