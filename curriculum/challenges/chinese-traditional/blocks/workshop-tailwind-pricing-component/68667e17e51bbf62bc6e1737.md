...
現在你應該為 CTA 添加樣式，使其看起來更像按鈕。

為背景和前景顏色指定類別 `bg-gradient-to-r`、`from-fuchsia-500`、`to-indigo-600`、`text-white`、`hover:from-fuchsia-600` 和 `hover:to-indigo-700`。也為它指定類別 `rounded-md` 和 `block`。

# --hints--

你的錨點元素應該有 `bg-gradient-to-r` 類別。
...
你的錨點元素應該有類別 `hover:from-fuchsia-600`。
...
你的錨點元素應該有類別 `hover:to-indigo-700`。
