...
Nella callback `forEach`, racchiudi l'assegnazione a destra `totalCountPerProduct[dessert.id] || 0` tra parentesi `()` per assicurarti che venga valutata correttamente, poi incrementa il valore di uno.
...
Dovresti racchiudere `totalCountPerProduct[dessert.id] || 0` tra parentesi `()` prima di incrementarlo di uno.
