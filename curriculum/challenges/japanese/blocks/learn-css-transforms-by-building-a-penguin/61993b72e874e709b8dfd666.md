...
`.penguin-head` 要素をターゲットにして、その親要素の半分の `width` と `height` の `45%` を与えてください。それから、`background` を `45deg` で始まり `gray` から `rgb(239, 240, 228)` で終わる線形グラデーションに設定してください。

# --hints--

`.penguin-head` セレクターを使うべきです。

```js
assert.match(code, /\.penguin-head\s*\{/);
```

`.penguin-head` に `width` の `50%` を与えるべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

`.penguin-head` に `height` の `45%` を与えるべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

`.penguin-head` に `background` の `linear-gradient(45deg, gray, rgb(239, 240, 228))` を与えるべきです。
