...
色标已经设置好，你将对 `linear-gradient` 函数中的每个颜色参数应用不同的红色色调。 马克笔的顶部和底部边缘的色调会更深，而中间的色调会更浅，就像上面有光一样。

对于当前为纯红色的第一个颜色参数，更新 `rgb` 函数，使红色值为 `122`，绿色值为 `74`，蓝色值为`14`。

# --hints--

你的 `.red` CSS 规则应该有一个 `background` 属性，设置为 `linear-gradient(180deg, rgb(122, 74, 14) 0%, rgb(0, 255, 0) 50%, rgb(0, 0, 255) 100%)`。
