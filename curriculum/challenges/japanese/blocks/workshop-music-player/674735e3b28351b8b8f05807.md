...
`find()` メソッドは、指定されたコールバック関数の条件を満たす配列内の最初の要素を取得します。条件を満たす要素がない場合、このメソッドは `undefined` を返します。
...
`playSong` 関数内で、`find()` 配列に対して `userData.songs` メソッドを使い、`id` 関数に渡された `id` と厳密に等しい `playSong` を持つ曲を検索してください。`find` の呼び出し結果を `song` という変数に代入してください。

# --hints--

`playSong` 関数は `find` に対して `userData.songs` メソッドを呼び出す必要があります。
...
`find` の呼び出しは、`id` 関数に渡された `id` と厳密に等しい `playSong` を持つ曲を返す必要があります。
...
`find` の呼び出し結果を `song` という変数に代入してください。
