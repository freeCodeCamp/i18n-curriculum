...
HTMLの文字列が与えられたら、すべてのタグを取り除いてプレーンテキストの内容を返してください。

# --hints--

`extract_content('<p>hello world</p>')` は `"hello world"` を返すべきです。
...
`extract_content('<p>hello <span>world</span></p>')` は `"hello world"` を返すべきです。
...
`extract_content('<a href="example.com">Click me</a>')` は `"Click me"` を返すべきです。
...
`extract_content('<p><button onClick="learnToCode()">Learn</button> to <code>code<code> <br/>for <strong>free</strong> <br/>on <a href="https://freecodecamp.org/" target="_blank"><span class="highlight">freecodecamp</span>.org</a>')` は `"Learn to code for free on freecodecamp.org"` を返すべきです。
