...
انتقل إلى عنصر الـ `footer`، واجعل كل النصوص بحجم خط `14px`.

# --hints--

يجب أن يكون لديك محدِّد `footer`.
...
يجب أن يكون محدِّد الـ `footer` أسفل تعليقك.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

يجب تعيين خاصية `font-size` إلى `14px`.
...
يجب أن يكون لعنصر الـ `footer` خاصية `font-size` بقيمة `14px`.
