...
Debajo de los valores desestructurados `value` y `checked`, actualiza la lista de poderes seleccionados.

Llama a la función `setPowers`, que recibe un argumento de tipo arreglo. Usa un operador ternario para verificar si `checked` es true. Si lo es, extiende el arreglo existente `powers` en uno nuevo y agrega el `value`. Si no es true, filtra el `value` de `powers` con `powers.filter(p => p !== value)`.

# --hints--

Debes usar un operador ternario para verificar si `checked` es true. Si lo es, extiende el arreglo existente `powers` y el `value` en un arreglo. Si no, filtra el `value` de `powers`.
