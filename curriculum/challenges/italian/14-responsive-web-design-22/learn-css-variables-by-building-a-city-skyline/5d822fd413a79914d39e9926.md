...
Dovresti dare a `.fb3-window` una proprietà `width` del `25%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

Dovresti dare a `.fb3-window` una proprietà `height` dell'`80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

Dovresti dare a `.fb3-window` una proprietà `background-color` con il valore `--window-color1`.
