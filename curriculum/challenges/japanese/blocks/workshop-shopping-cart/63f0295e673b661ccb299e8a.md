...
カート内のアイテムの合計数にアクセスする方法が必要です。これを行う最良の方法は、`ShoppingCart` クラスに別のメソッドを追加することであり、`items` 配列に直接アクセスするのではありません。

`getCounts` クラスに `ShoppingCart` メソッドを追加してください。このメソッドは `items` 配列内のアイテム数を返す必要があります。

# --hints--

`ShoppingCart` クラスには `getCounts` メソッドが必要です。
...
`getCounts` メソッドは `items` 配列内のアイテム数を返す必要があります。
