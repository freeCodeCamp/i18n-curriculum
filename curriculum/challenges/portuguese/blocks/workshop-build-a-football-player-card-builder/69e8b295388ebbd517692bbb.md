...
Após o elemento `card-image-wrap`, adicione um `div` com o `className` de `card-name-strip`. Dentro dele, adicione um `span` com o `className` de `card-name` e o texto `PELE`.

# --hints--

Você deve adicionar um `div` com o `className` de `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

Você deve adicionar um `span` com o `className` de `card-name` dentro do `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

O span `.card-name` deve conter o texto `PELE`.
