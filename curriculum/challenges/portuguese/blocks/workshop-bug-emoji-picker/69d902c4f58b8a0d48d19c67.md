...
Agora preencha `bugMap` com duas entradas. Adicione `"bee"` como uma chave mapeada para uma nova instância `Bee`, e `"spider"` como uma chave mapeada para uma nova instância `Spider`. Passe `bugEmojiElement` como argumento para ambas.

# --hints--

Seu `bugMap` deve ter uma entrada `"bee"` definida como `new Bee(bugEmojiElement)`.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

Seu `bugMap` deve ter uma entrada `"spider"` definida como `new Spider(bugEmojiElement)`.
