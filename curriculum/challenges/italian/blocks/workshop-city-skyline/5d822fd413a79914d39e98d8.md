...
Usa la stessa variabile come `background-color` delle classi `.bb1b`, `.bb1c` e `.bb1d` per completare il resto dell’edificio.

# --hints--

La `background-color` dell’elemento `.bb1b` dovrebbe essere impostata.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1b')?.backgroundColor)
```

Dovresti usare `var(--building-color1)` per impostare la `background-color` dell’elemento `.bb1b`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1b')?.getPropVal('background-color', true), 'var(--building-color1)');
```

La `background-color` dell’elemento `.bb1c` dovrebbe essere impostata.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1c')?.backgroundColor)
```

Dovresti usare `var(--building-color1)` per impostare la `background-color` dell’elemento `.bb1c`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1c')?.getPropVal('background-color', true), 'var(--building-color1)');
```

La `background-color` dell’elemento `.bb1d` dovrebbe essere impostata.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1d')?.backgroundColor)
```

Dovresti usare `var(--building-color1)` per impostare la `background-color` dell’elemento `.bb1d`.
