...
`byDecade` のログは役目を終えました。コードから `console.log(byDecade)` を削除してください。

次に、カタログのエントリーを見やすくするために、`renderEntry` をパラメータに取る `entry` という名前の関数を作成してください。関数の本体は今は空のままで構いません。関数の下に `renderEntry(catalog[0])` をログ出力してください。

# --hints--

`renderEntry` は関数であるべきです。

```js
assert.isFunction(renderEntry);
```

`renderEntry` は `entry` というパラメータを持つ関数であるべきです。
