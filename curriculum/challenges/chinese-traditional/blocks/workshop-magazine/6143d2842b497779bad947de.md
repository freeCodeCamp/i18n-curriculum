...
開始寫 CSS，通過使用 `*`，包括 `::before` 和 `::after` 僞選擇器，定位所有元素來規範 CSS 規則，。

將 `padding` 和 `margin` 屬性均設置爲 `0`，並將 `box-sizing` 屬性設置爲 `border-box`。
