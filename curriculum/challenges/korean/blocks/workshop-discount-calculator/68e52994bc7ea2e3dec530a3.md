...
`calculate_best_price` 메서드 안에 `prices`를 포함하는 `product.price`라는 리스트를 만드세요. 이렇게 하면 할인이 적용되지 않을 경우 고객이 정가를 지불하게 됩니다.

# --hints--

`calculate_best_price` 안에 `prices`를 포함하는 `product.price`라는 리스트를 만들어야 합니다.
