...
創建一個名為 `outputValueNode` 的變數，並將其值設定為呼叫 `document.getElementById()` 方法的結果。使用模板字串語法將 ``output-value-${i}`` 字串傳入 `.getElementById()`。

# --hints--

在你的 `.forEach()` 回呼中使用 `const` 宣告一個 `outputValueNode` 變數。

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=/)
```

`outputValueNode` 應該被指定為呼叫 `document.getElementById()` 的值。

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=\s*document\.getElementById\s*\(/)
```

使用模板字面量語法將 `output-value-${i}` 字串傳入 `.getElementById()`。
