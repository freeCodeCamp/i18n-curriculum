...
Vincula los elementos de formulario aplicables con sus elementos `label`.

Usa `profile-picture`, `age`, `referrer`, y `bio` como valores para los atributos `id` respectivos.

# --hints--

El primer elemento `input` debe tener un `id` de `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[0]?.matches('#profile-picture'))
```

El segundo elemento `input` debe tener un `id` de `age`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[1]?.matches('#age'))
```

El elemento `select` debe tener un `id` de `referrer`.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) select')?.matches('#referrer'))
```

El elemento `textarea` debe tener un `id` de `bio`.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) textarea')?.matches('#bio'))
```

El primer elemento `label` debe tener un atributo `for` con un valor de `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[0]?.matches('label[for="profile-picture"]'))
```

El segundo elemento `label` debe tener un atributo `for` con un valor de `age`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[1]?.matches('label[for="age"]'))
```

El tercer elemento `label` debe tener un atributo `for` con un valor de `referrer`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[2]?.matches('label[for="referrer"]'))
```

El cuarto elemento `label` debe tener un atributo `for` con un valor de `bio`.
