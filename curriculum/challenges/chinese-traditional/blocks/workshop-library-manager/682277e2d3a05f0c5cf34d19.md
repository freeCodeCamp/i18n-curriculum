...
在你的 `getBooksByAuthor()` 函式（程式）下方添加一個 `console.log()`。

在 `console.log()` 內，使用 `library` 和 `"Arvid Kahl"` 作為引數，呼叫 `getBooksByAuthor()` 函式。

現在，你應該會在主控台看到該特定作者的所有書籍。

# --hints--

你應該有一個 `console.log(getBooksByAuthor(library, "Arvid Kahl"))`。
