...
Knight's Tour Problem: You have an empty `width` \* `height` chessboard, but for a single knight on some square. 骑士必须执行一系列合法移动，使得骑士只访问棋盘上的每个方格一次。 请注意，*不* 要求“闭环”旅程；也就是说，骑士不需要在其起始位置的一次移动范围内结束旅程。

# --instructions--

编写一个函数，接受 `width` 和 `height` 作为参数，并返回可以实现上述任务的初始位置数量。
