...
في هذه الخطوة، ستكمل دالة `updateResultsList`.

الآن قم بالتكرار خلال مصفوفة `units`. لكل قيمة، تحقق مما إذا كانت لا تساوي الوحدة الحالية (`unitToConvert.value`).

إذا تحقق الشرط، حوّل الوحدة الموجودة إلى الوحدة الحالية وأضف عنصر قائمة جديد إلى `resultList` بالتنسيق التالي:

`ingredient: quantity unit`

على سبيل المثال، إذا كان المُدخَل هو `flour`، و`5`، و`gram`، يجب أن تكون النتيجة:
...
عند النقر على الزر، يجب أن تقوم دالة `updateResultsList` بتحديث DOM بشكل صحيح.
