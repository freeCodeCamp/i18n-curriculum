...
你的第一个 `img` 元素应该有一个 `src` 属性值为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`。
