...
下一棟建築將包含三個部分。 在 `.bb4` 中嵌套三個 `div` 元素。 按順序給它們添加 `bb4a`、`bb4b` 和 `bb4c` 類。
...
你應該給第一個新的 `div` 一個 `bb4a` 類。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你應該給第二個新的 `div` 一個 `bb4b` 類。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你應該給第三個新的 `div` 一個 `bb4c` 類。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你應該按 `.bb4a + .bb4b + .bb4c` 的順序排列新的 `div` 元素。
