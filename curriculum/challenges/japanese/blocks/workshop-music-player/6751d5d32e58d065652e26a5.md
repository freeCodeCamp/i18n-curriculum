...
現在再生中の曲に `aria-current` 属性を追加する必要があります。

`songToHighlight` が存在するか確認してください。存在する場合は、`setAttribute` に対して `songToHighlight` メソッドを使い、`"aria-current"` と `"true"` をそれぞれ1番目と2番目の引数として渡してください。

# --hints--

`highlightCurrentSong` は現在再生中の曲に `aria-current` 属性を追加し、前に再生されていた曲からその属性を削除する必要があります。
