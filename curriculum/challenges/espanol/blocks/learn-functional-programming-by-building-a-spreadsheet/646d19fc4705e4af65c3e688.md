...
Tu función `addCharacters` en última instancia devuelve un rango de caracteres. Deseas que devuelva un arreglo de IDs de celda. Encadena el método `.map()` a tu llamada `charRange()`. No pases una función de devolución de llamada todavía.

# --hints--

Deberías encadenar `.map()` a tu llamada `charRange()`.
...
No deberías pasar nada a tu llamada `.map()`.
