...
`for` ループを使って `odd_digits` リストの各数字を繰り返してください。前のステップからの `print` 呼び出しを `for` ループ内に移動し、各数字を出力するように変更してください。

# --hints--

`for` に対して `odd_digits` ループを使うべきです。
...
`--fcc-expected--` ループ内に `for` があるはずです。
