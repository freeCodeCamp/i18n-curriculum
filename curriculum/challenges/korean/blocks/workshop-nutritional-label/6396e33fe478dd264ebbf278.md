...
새로운 `.calories-info` 선택자를 만들고 `display` 속성을 `flex`로 설정하세요. 또한 `justify-content` 속성을 `space-between`로, `align-items` 속성을 `flex-end`로 설정하세요.

# --hints--

`.calories-info` 선택자를 만들어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info'));
```

`.calories-info` 선택자에 `display` 속성을 `flex`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info')?.display === 'flex');
```

`.calories-info` 선택자에 `justify-content` 속성을 `space-between`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info')?.justifyContent === 'space-between');
```

`.calories-info` 선택자에 `align-items` 속성을 `flex-end`로 설정하세요.
