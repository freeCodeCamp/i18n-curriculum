...
Ndani ya kitendakazi cha `getElement`, tumia `querySelector` na generics na hifadhi matokeo katika kigezo. Ongeza taarifa ya `if` ili ukague kama kipengele hakipo, na ikiwa ni hivyo, tupa kosa lenye ujumbe wa `Element not found: ${error message here}`. Ikiwa kipengele kiko, rirudisha.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `el` ndani ya kitendakazi cha `getElement`.
...
Kigezo chako cha `el` kinapaswa kupewa thamani ya `document.querySelector<T>(selector)`.
...
Unapaswa kuwa na taarifa ya if inayokagua kama `el` haipatikani.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

Unapaswa kutupa kosa jipya la Error lenye ujumbe wa `Element not found: ${selector}` ndani ya taarifa yako ya `if`.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

Unapaswa kurudisha kigezo cha `el` ndani ya kitendakazi cha getElement.
