...
الآن بعد أن أنشأت `resetButton`، تحتاج إلى تعيين خاصيتي `id` و `aria-label` له. يوفر JavaScript الخاصيتين `id` و `ariaLabel` اللتين تحتاج لاستخدامهما لهذا الغرض.

على سبيل المثال، `element.id` يعين خاصية `id`، و`element.ariaLabel` يعين خاصية `aria-label`. كلاهما يقبل قيمتهما كسلسلة نصية.

عيّن خاصية `id` لعنصر `resetButton` إلى `"reset"` وخاصية `aria-label` إلى `"Reset playlist"`.

# --hints--

لا يجب تعديل جملة `if` الموجودة ومحتواها.
...
استخدم `resetButton.id` لإنشاء خاصية `id` باسم `"reset"` لـ `resetButton`.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

استخدم `resetButton.ariaLabel` لإنشاء خاصية `"aria-label"` باسم `"Reset playlist"` لـ `resetButton`.
