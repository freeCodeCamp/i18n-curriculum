...
Em seguida, crie um elemento `fieldset` com o `id` definido como `lunch`.

Dentro desse elemento, crie um elemento `legend` com o texto `Lunch` e um `div` vazio com a `class` definida como `input-container`.
