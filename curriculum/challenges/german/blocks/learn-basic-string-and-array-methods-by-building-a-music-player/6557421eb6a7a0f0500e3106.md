...
Fügen Sie innerhalb des button-Elements in der `renderSongs`-Funktion ein `onclick`-Attribut hinzu. Rufen Sie für den Wert die `deleteSong`-Funktion auf und interpolieren Sie `song.id`.

# --hints--

Sie sollten dem Löschen-Button ein `onclick`-Attribut hinzufügen und `"deleteSong(${song.id})"` übergeben.
