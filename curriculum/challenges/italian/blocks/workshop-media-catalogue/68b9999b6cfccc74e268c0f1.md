...
Prima del tuo loop `for`, crea un'istruzione `if` che aggiunga la stringa `=== MOVIES ===` (terminando con un carattere di nuova riga) al valore corrente di `result` quando la lista `movies` non è vuota.

# --hints--

Dovresti avere un'istruzione `if` nel tuo metodo `__str__` che verifichi se la lista `movies` non è vuota.
...
Dovresti avere `result += '=== MOVIES ===\n'` all'interno della tua istruzione `if`.
