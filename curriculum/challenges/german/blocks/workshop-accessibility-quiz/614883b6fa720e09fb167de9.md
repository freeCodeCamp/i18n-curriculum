...
Fügen Sie den `.info`-Elementen Innenabstand oben und links hinzu und setzen Sie die anderen Werte auf `0`.

# --hints--

Sie sollten den `.info`-Selektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.info'));
```

Sie sollten `.info` einen `padding-top` von mindestens `1px` geben.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('.info')?.paddingTop?.replace(/\D+/, '')), 1);
```

Sie sollten `.info` einen `padding-right` von `0` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingRight, '0px');
```

Sie sollten `.info` einen `padding-bottom` von `0` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingBottom, '0px');
```

Sie sollten `.info` einen `padding-left` von mindestens `1px` geben.
