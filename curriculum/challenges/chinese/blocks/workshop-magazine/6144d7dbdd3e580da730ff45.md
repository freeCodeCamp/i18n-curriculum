...
创建一个 `body` 选择器。 将 `font-family` 属性设置为 `Baskervville`，回退值为 `serif`。 将 `color` 属性设置为 `linen`，将 `background-color` 属性设置为 `rgb(20, 30, 40)`。
...
你的 `body` 选择器应该有一个 `font-family` 属性，值为 `Baskervville`，回退值为 `serif`。
...
你的 `body` 选择器应该有一个 `color` 属性，值为 `linen`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.color, 'linen');
```

你的 `body` 选择器应该有一个 `background-color` 属性，值为 `rgb(20, 30, 40)`。
