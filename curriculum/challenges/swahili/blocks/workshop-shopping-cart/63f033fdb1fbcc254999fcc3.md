...
Gari lako la manunuzi kwa sasa halionekani kwenye ukurasa wa mtandao. Ili kuufanya uonekane, anza kwa kuongeza msikilizaji wa tukio kwa kigezo cha `cartBtn`, ukisikiliza tukio la bofya. Kitendakazi cha mwitiko hakihitaji vigezo vyovyote.

# --hints--

Unapaswa kuongeza msikilizaji wa tukio kwa kigezo cha `cartBtn`.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(/);
```

Unapaswa kusikiliza tukio la `click` kwenye kigezo cha `cartBtn`.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

Unapaswa kuongeza kitendakazi cha mwitiko tupu (ukitumia sintaksia ya mshale) kwa msikilizaji wa tukio. Kumbuka kwamba hakihitaji vigezo vyovyote.
