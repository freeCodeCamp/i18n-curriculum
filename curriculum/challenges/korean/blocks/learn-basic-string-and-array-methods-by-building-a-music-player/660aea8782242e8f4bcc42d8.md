...
일반 함수처럼 화살표 함수도 여러 매개변수를 받을 수 있습니다.

다음은 매개변수가 하나인 이름 있는 화살표 함수 예시입니다:
...
함수에 매개변수가 하나뿐이라면, 매개변수 목록을 감싸는 소괄호를 이렇게 생략할 수 있습니다:
...
`printMessage`라는 이름의 새 화살표 함수를 만드세요. 이 함수는 `org`라는 매개변수를 하나 가집니다. 함수 본문 안에 콘솔 문장을 추가하세요. 그 콘솔 문장 안에 템플릿 리터럴 `${org} is awesome!`를 넣으세요.

`printMessage` 함수 아래에 함수를 호출하고 문자열 `"freeCodeCamp"`를 인수로 전달하세요.

결과를 확인하려면 콘솔을 열어보세요.

# --hints--

`printMessage` 함수를 만들어야 합니다.
...
`printMessage` 함수는 `org`라는 매개변수를 가져야 합니다.
...
`printMessage` 함수 안에 콘솔 문장이 있어야 합니다.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

콘솔 문장에는 템플릿 리터럴 `${org} is awesome!`가 포함되어야 합니다.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

`printMessage` 함수를 호출하고 문자열 `"freeCodeCamp"`를 전달해야 합니다.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

함수 호출은 `printMessage` 함수 정의 아래에 있어야 합니다.
...
`printMessage` 함수 호출은 콘솔에 `freeCodeCamp is awesome!`를 출력해야 합니다.
