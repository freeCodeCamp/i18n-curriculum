...
`click` に `scaryStoryBtn` イベントのイベントリスナーを追加し、`displayStory` 関数を渡してください。それから、ボタンをクリックしてコンソールに出力を確認しましょう。

# --hints--

`click` に `scaryStoryBtn` イベントのイベントリスナーを追加し、`displayStory` 関数を渡す必要があります。
