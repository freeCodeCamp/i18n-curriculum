...
Luhn テストは、クレジットカード会社によって有効なクレジットカード番号と数字のランダムな選択を区別するために使用されることがあります。

Luhn テストで検証可能なクレジットカード番号を使用している企業が発行するカード番号は、以下のテストをパスします。
...
Luhn テストで数値を検証する関数を記述してください。 有効な数値の場合は true を返します。 それ以外は、false を返します。
