...
让我们测试你的新建柯里的函数。声明一个 `servingsResult` 变量，并赋值为从基础数量 4 调整到 6 份的结果。

将该变量记录到控制台日志。

# --hints--

你应该声明一个 `servingsResult` 变量。

```js
assert.isDefined(servingsResult);
```

你的 `servingsResult` 变量应包含调用 `adjustQuantity(4)(6)` 的结果。

```js
assert.strictEqual(servingsResult, 24);
```

你应该记录你的 `servingsResult` 值。
