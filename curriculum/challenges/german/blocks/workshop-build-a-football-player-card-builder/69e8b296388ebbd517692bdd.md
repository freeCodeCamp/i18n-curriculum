...
Fügen Sie dem PAS `value` ein `player.pas`-Attribut mit dem Wert `input` hinzu. Fügen Sie dann ein `onChange`-Attribut hinzu, das auf eine Pfeilfunktion gesetzt ist, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `pas` auf `Number(e.target.value)` gesetzt ist.

# --hints--

Ihr PAS `input` sollte ein `value` haben, das dem `pas`-Wert des Spielers entspricht.
...
Ihr PAS `input` sollte einen `onChange`-Handler haben, der `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `pas` auf `Number(e.target.value)` gesetzt ist.
