...
Agrega un atributo `value` de `player.dri` al input DRI. Luego agrega un atributo `onChange` configurado como una función flecha que toma `e` como parámetro y llama a `setPlayer` con un spread de `player` y `dri` establecido en `Number(e.target.value)`.

# --hints--

Tu `input` DRI debe tener un `value` igual a la estadística `dri` del jugador.
...
Tu `input` DRI debe tener un manejador `onChange` que llame a `setPlayer` con un spread de `player` y `dri` establecido en `Number(e.target.value)`.
