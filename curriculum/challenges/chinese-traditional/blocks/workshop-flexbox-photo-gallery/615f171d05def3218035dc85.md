...
移除你的 body 元素的邊距，將 `font-family` 設為 `sans-serif`，並將 `background-color` 設為值 `#f5f6f7`。
...
你的 `body` 選擇器應該有一個 `margin` 屬性，並將值設為 0。
...
你的 `body` 選擇器應該有一個 `font-family` 屬性，並將值設為 `sans-serif`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily, 'sans-serif');
```

你的 `body` 選擇器應該有一個 `background-color` 屬性，並將值設為 `#f5f6f7`。
