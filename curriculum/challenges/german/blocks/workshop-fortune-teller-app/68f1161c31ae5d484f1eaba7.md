...
Fügen Sie nun eine weitere Methode namens `showFortune` hinzu. Übergeben Sie als Parameter `e` und geben Sie ihm den Typ `Event`. Erstellen Sie außerdem eine `newReading`-Methode und lassen Sie sie vorerst leer.

# --hints--

Sie sollten eine Methode namens `showFortune` deklarieren.
...
Ihre `showFortune`-Methode sollte einen `e`-Parameter mit dem Typ `Event` haben.
...
Sie sollten eine Methode namens `newReading` deklarieren.
