...
`div` 요소 아래에 새로운 `.medium` 요소를 만드세요. `class` 속성을 `daily-value small-text` 값으로 설정하세요. 이 새로운 `div` 안에 `p` 요소를 추가하고 텍스트는 `% Daily Value *`로 설정하며, `class` 속성을 `bold right` 값으로 지정하세요.

# --hints--

`div` 요소 다음에 새로운 `.medium` 요소를 만들어야 합니다.
...
새로운 `div` 요소에는 `class` 속성을 `daily-value small-text` 값으로 설정하세요.
