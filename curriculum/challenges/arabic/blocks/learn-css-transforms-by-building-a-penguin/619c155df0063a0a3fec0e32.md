...
امنح عناصر `.face` خاصية `width` بقيمة `60%`، و`height` بقيمة `70%`، و`background-color` بقيمة `white`.

# --hints--

يجب أن تستخدم محدِّد `.face`.

```js
assert.match(code, /\.face\s*\{/);
```

يجب أن تعطي `.face` خاصية `width`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.width);
```

يجب أن تعطي `.face` خاصية `width` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.width, '60%');
```

يجب أن تعطي `.face` خاصية `height`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.height);
```

يجب أن تعطي `.face` خاصية `height` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.height, '70%');
```

يجب أن تعطي `.face` خاصية `background-color`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.backgroundColor);
```

يجب أن تعطي `.face` خاصية `background-color` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
