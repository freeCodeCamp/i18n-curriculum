...
你應該使用 `console.log()` 來列印出 `inputValues` 的結果。請在 `sortInputArray` 函式（程式）內撰寫這段程式碼。

要查看已記錄的 `inputValues` 陣列，請點擊排序按鈕並打開主控台。你應該會看到像這樣的字串陣列：

```js
[ "8", "2", "4", "1", "3" ]
```

在繼續之前，請確保你觀察主控台中列印結果的資料型別。
...
你應該將 `inputValues` 傳遞給你的 `console.log()` 呼叫。
