...
将 `.shirt` 元素放置在距离其父元素顶部 `165px`、左侧 `127.5px` 的位置。 然后，提高其堆叠顺序，使其出现在 `.penguin-body` 元素上方。

# --hints--

你应该将 `.shirt` 的 `top` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.top, '165px');
```

你应该将 `.shirt` 的 `left` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.left, '127.5px');
```

你应该将 `.shirt` 元素的 `z-index` 设置为 `1`。
