...
Debes usar el selector existente `li > a` para establecer el `text-decoration` en `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

Debes usar el selector `nav > ul > li:hover` o `nav li:hover` para estilizar los elementos al pasar el cursor.
