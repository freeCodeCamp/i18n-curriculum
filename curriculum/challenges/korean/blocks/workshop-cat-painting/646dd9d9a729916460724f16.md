...
오른쪽 안쪽 귀를 `position`로 설정된 `absolute` 속성, `top`인 `22px`, 그리고 `left`인 `-20px`로 위치시키세요.

# --hints--

`.cat-right-inner-ear` 선택자에 `position`로 설정된 `absolute` 속성이 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.position, 'absolute')
```

`.cat-right-inner-ear` 선택자에 `top`로 설정된 `22px` 속성이 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.top, '22px')
```

`.cat-right-inner-ear` 선택자에 `left`로 설정된 `-20px` 속성이 있어야 합니다.
