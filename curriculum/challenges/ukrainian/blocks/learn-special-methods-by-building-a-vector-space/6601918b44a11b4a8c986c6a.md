...
Після методу `__lt__`, так само, як ви зробили для `__lt__`, реалізуйте метод `__gt__`. Зверніть увагу на використання відповідного оператора.

# --hints--

Ви повинні визначити метод `__gt__` у класі `R2Vector`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__gt__")`)) })
```

Ваш метод `__lt__` має мати два параметри, `self` та `other`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__gt__").has_args("self, other")`)) })
```

Ви повинні створити інструкцію `if`, яка перевіряє, чи `self` та `other` не належать до одного класу.
...
Ви повинні повернути `NotImplemented` з тіла вашого методу `if`.
...
Метод `__gt__` має повертати результат порівняння норми поточного екземпляра з нормою `other` за допомогою оператора «більше ніж».
