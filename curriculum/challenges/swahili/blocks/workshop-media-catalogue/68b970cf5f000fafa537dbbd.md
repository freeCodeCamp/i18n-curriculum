...
Kwa kuwa unahifadhi kitu kinachosababisha hitilafu ndani ya kitu cha hitilafu, unaweza kukitumia kwa namna fulani.

Baada ya wito wako wa `print` uliopo, ongeza `print(f'Unable to add {e.obj}: {type(e.obj)}')`.

Aina hii ya ujumbe inaweza kuwa na msaada mkubwa unapojaribu kutafuta makosa kwenye msimbo wako.

# --hints--

Unapaswa kuwa na `print(f'Unable to add {e.obj}: {type(e.obj)}')` baada ya wito wako wa `print(f'Media Error: {e}')` uliopo.
