...
要构造一个正向先行断言，你需要先从你想匹配的模式开始。然后，使用括号将你想用作条件的模式包裹起来。在开括号后，使用 `?=` 来定义该模式为正向先行断言。
...
记住，`RegExp.prototype.test` 只确认一个字串是否匹配正则表达式。让我们用负向后行断言配合 `String.prototype.match` 来看看断言如何影响匹配结果：
