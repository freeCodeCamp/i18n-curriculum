...
你应该只为单选按钮添加一个 `input` 元素。 删除任何多余的。

```js
assert(document.querySelectorAll('form > input'), 2);
```

你的新 `input` 元素应该在现有的 `input` 上方，`type` 属性应该设置为 `text` 。 你把它们的顺序写错了。
...
你的新 `input` 元素应该只有一个 `type` 属性。 删除任何多余的。
...
你的新 `input` 元素应该具有一个 `type` 属性，其值为 `radio`。 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
...
`radio` 按钮的 `Indoor` 文本应位于其后，而不是之前。
...
文本 ` Indoor` 应位于你的 `radio` 按钮的正右侧。你要么遗漏了该文本，要么有拼写错误。
