...
由于你在 `addElDelay` 中存储了每个动画帧的时间，因此你可以将该值与 `setTimeout()` 一起使用来设置将元素添加到 DOM 的延迟。

在 `.forEach()` 方法的回调函数主体内，添加一个 `setTimeout()` 函数。 传入一个空的回调函数作为第一个参数，并传入 `obj.addElDelay` 作为第二个参数。

# --hints--

你应该在 `.forEach()` 方法的回调函数主体中添加 `setTimeout()` 函数。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\([\s\S]+\{\s*setTimeout\(/);
```

你应该将一个空的回调函数作为第一个参数传递给 `setTimeout()` 函数。
