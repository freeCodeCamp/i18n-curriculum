...
回到 `MediaCatalogue` 類別，為它添加一個說明字串，內容為 `A catalogue that can store different types of media items.`

# --hints--

你應該為你的 `MediaCatalogue` 類別添加 docstring `A catalogue that can store different types of media items.`。
