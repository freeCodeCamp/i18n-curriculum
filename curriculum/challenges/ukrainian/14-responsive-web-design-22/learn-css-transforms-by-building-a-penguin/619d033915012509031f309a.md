...
Ви повинні додати один елемент `div` в межах `.penguin-head`. Очікувались елементи `div` `--fcc-expected--`, знайдено `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 3);
```

Для `div` потрібно надати `class` зі значенням `chin`.
