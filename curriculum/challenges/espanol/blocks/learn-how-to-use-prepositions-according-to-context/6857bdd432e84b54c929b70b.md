...
`Storage room` is a space used to keep items not in immediate use. Por ejemplo:

`The extra cables are in the storage room.` – This means the cables are stored there.

`Go back` means to return or move in the opposite direction. Por ejemplo:

`Go back to the entrance if you're lost.` – This means return to the starting point.

`Towards` means in the direction of something. Por ejemplo:
