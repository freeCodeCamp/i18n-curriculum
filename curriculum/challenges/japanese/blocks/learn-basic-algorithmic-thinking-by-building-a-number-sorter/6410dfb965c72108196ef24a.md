...
要素を比較する必要があるため、ネストされた`for`ループを使う必要があります。このループは配列の最後の要素を除くすべての要素を繰り返し処理する必要があります。内側のループの反復変数には`j`を使ってください。
...
内側の`for`ループは`j`を`0`に初期化する必要があります。
...
内側の`for`ループは配列の最後の要素を除くすべての要素を繰り返し処理する必要があります。
...
内側の`for`ループは毎回`j`を`1`ずつ増やす必要があります。

```js
assert.match(code, /const\s+bubbleSort\s*=\s*(\(\s*array\s*\)|array)\s*=>\s*{\s*for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*array\.length\s*;\s*i\s*\+\+\s*\)\s*{\s*for\s*\(\s*let\s+j\s*=\s*0\s*;\s*j\s*<\s*array\.length\s*-\s*1\s*;\s*j\s*\+\+\s*\)/);
```

内側の`for`ループは空である必要があります。
