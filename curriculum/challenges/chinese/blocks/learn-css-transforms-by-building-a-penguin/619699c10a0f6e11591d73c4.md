...
使用 `margin` 属性将 `.penguin` 元素水平居中，并将 `margin-top` 设置为 `75px`。

# --hints--

你应该将 `.penguin` 的 `margin` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
...
你应该将 `.penguin` 的 `margin-top` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
