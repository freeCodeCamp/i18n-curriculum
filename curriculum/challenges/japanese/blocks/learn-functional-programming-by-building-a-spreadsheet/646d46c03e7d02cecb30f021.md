...
これで、`evalFormula` ロジックに関数パーサーを適用し始めることができます。`functionExpanded` 変数を宣言し、`applyFunction` 文字列を使って `cellExpanded` を呼び出した結果を代入してください。

# --hints--

`functionExpanded` 変数を宣言してください。
...
`cellExpanded` を `applyFunction` の呼び出しに渡すべきです。
