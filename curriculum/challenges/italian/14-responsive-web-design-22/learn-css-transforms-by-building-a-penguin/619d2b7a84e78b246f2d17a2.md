...
Dovresti dare a `.arm.left` un valore `top` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.top, '35%');
```

Dovresti dare a `.arm.left` un valore `left` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.left, '5%');
```

Dovresi usare il selettore `.arm.right`.

```js
assert.match(code, /\.arm\.right\s*\{/);
```

Dovresti dare a `.arm.right` un valore `top` dello `0%`.

```js
assert.include(['0%', '0', '0px'], new __helpers.CSSHelp(document).getStyle('.arm.right')?.top);
```

Dovresti dare a `.arm.right` un valore `right` di `--fcc-expected--` invece di `--fcc-actual--`.
