...
Du solltest `const` verwenden, um eine leere Funktion mit dem Namen `playNextSong` zu erstellen.
...
Deine `playNextSong`-Funktion sollte eine Arrow-Syntax verwenden.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Deine `playNextSong` Funktion sollte leer sein.
