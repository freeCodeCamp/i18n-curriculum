...
Fügen Sie am Ende Ihres `p`-Elements ein drittes `.text`-Element hinzu und geben Sie ihm den folgenden Text:
...
Sie sollten ein drittes `p`-Element in Ihrem `.text`-Element erstellen.

```js
assert(document.querySelectorAll('.text p')?.length === 3);
```

Ihr drittes `p`-Element sollte den vorgegebenen Text enthalten.
