...
仍在 `elif` 語句中，將 `expenses_from_category` 迭代器傳遞給 `print_expenses` 呼叫。

# --hints--

你應該在你的 `elif` 語句中呼叫 `print_expenses()`，並傳入 `expenses_from_category` 作為引數。
...
你的 `print_expenses()` 呼叫應該在 `expenses_from_category` 變數之後。
