...
美元价值可能不止一位数字。 为了匹配这一点，可以使用 `+` 量词 - 它匹配一个或多个连续的出现。 例如，正则表达式 `/a+/` 与一个或多个连续的 `a` 字符匹配。

更新正则表达式以匹配一个或多个连续的数字。

# --hints--

你的 `dollarRegex` 应该使用 `+` 量词。

```js
assert.match(dollarRegex.source, /\+/);
```

你的 `dollarRegex` 应该在 `[0-9]` 字符类上使用 `+` 量词。
...
你的 `dollarRegex` 应该仍然与 `"3 dollars"` 匹配。
