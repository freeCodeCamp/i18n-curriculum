...
首先為你的程式碼添加一個 `return`，接著加入一組模板字串。在模板字串內，添加一個 `img` 元素。 

在 `img` 頁籤內，為 `src` 屬性添加值 `${userAvatarUrl}`。對於 `alt` 屬性，添加值 `${user.name}`。
...
你應該將 `src` 屬性的值設定為名為 `userAvatarUrl` 的常數。
...
你應該將 `alt` 屬性的值設定為名為 `user.name` 的常數。
