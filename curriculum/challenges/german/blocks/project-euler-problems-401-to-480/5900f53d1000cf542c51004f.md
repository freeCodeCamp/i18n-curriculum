...
title: 'Problem 464: Möbiusfunktion und Intervalle'
challengeType: 1
forumTopicId: 302139
...
Die Möbiusfunktion, bezeichnet mit $μ(n)$, ist definiert als:

- $μ(n) = (-1)^{ω(n)}$ falls $n$ quadratfrei ist (wobei $ω(n)$ die Anzahl der eindeutigen Primfaktoren von $n$ ist).
- $μ(n) = 0$ falls $n$ nicht quadratfrei ist.

Sei $P(a, b)$ die Anzahl der Integer $n$ im Intervall $[a, b]$, so dass $μ(n) = 1$.

Sei $N(a, b)$ die Anzahl der Integer $n$ im Intervall $[a, b]$, so dass $μ(n) = -1$.

Zum Beispiel $P(2, 10) = 2$ und $N(2, 10) = 4$.

Sei $C(n)$ die Anzahl an Integer-Paaren $(a, b)$, so dass:
