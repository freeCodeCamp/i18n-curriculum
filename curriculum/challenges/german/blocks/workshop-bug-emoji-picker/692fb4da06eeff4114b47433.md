...
Erstellen Sie innerhalb der `Bee`-Klasse einen Konstruktor, der einen einzelnen Parameter `emojiElement` vom Typ `HTMLParagraphElement` entgegennimmt.

Rufen Sie innerhalb des Konstruktors die `super`-Funktion mit dem `emojiElement`-Parameter auf, um den Konstruktor der übergeordneten `Bug`-Klasse auszuführen. Setzen Sie anschließend die `emoji`-Eigenschaft auf den String-Wert `"🐝"`.

# --hints--

Ihre `Bee`-Klasse sollte einen Konstruktor haben.
...
Ihr `Bee`-Konstruktor sollte einen Parameter namens `emojiElement` vom Typ `HTMLParagraphElement` haben.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Bee[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Ihr `Bee`-Konstruktor sollte `super` mit dem Parameter `emojiElement` aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /super\s*\(\s*emojiElement\s*\)/);
```

Ihr `Bee`-Konstruktor sollte `this.emoji` auf `"🐝"` setzen.
