...
Crie outra cláusula `elif` que verifique se `choice == '4'`. Dentro do novo `elif`, crie uma variável `category` e atribua a ela `input('Enter category to filter: ')` para filtrar a categoria de despesa.

# --hints--

Você deve criar uma cláusula `elif` que verifique se `choice == '4'`.
...
Você deve declarar uma variável chamada `category` na sua nova cláusula `elif`.
...
Você deve atribuir `input('Enter category to filter: ')` à sua variável `category`.
