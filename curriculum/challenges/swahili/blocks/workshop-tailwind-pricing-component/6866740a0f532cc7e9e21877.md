...
Sasa tuchukue hatua kuelekea baji ya `Most popular`. Unahitaji kuiweka kona ya juu kulia ya kadi, kwa hivyo unaweza kutumia baadhi ya madarasa ya msaada ya nafasi ya Tailwind kufanya hivyo.

Hivi ndivyo unavyotumia nafasi ya CSS katika Tailwind:
...
Weka darasa la `relative` kwa kifungashio cha `div` na `absolute` kwa `div` yenye maandishi ya `Most popular`.

# --hints--

Kifungashio chako cha `div` kwa mpango wa premium kinapaswa kuwa na darasa la `relative`.
...
Kipengele chako cha `div` chenye maandishi ya `Most popular` kinapaswa kuwa na darasa la `absolute`.
