...
Die Dropdown-Optionen sind derzeit leer. Um ihnen Inhalt zu geben, fügen Sie den folgenden Text zu jedem nachfolgenden `option`-Element hinzu:
...
Dem vierten `option`-Element sollten Sie den Text `freeCodeCamp Forum` geben.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.textContent, 'freeCodeCamp Forum');
```

Dem fünften `option`-Element sollten Sie den Text `Other` geben.
