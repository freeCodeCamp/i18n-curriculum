...
حان الوقت لرؤية دالتك المتسلسلة أثناء التنفيذ. إليك كيف يمكنك استدعاء دالتنا المثال:

```js
const dinner = tikkaMasala("curry powder")("tofu")("onion");
```

أعلن عن متغير `gramsResult`، وقم بتعيين نتيجة استخدام دالتك المتسلسلة لتحويل `2` وحدة من المقياس `cup` إلى `gram`.

سجل تلك القيمة في وحدة التحكم.

# --hints--

يجب أن يكون لديك متغير `gramsResult`.

```js
assert.isDefined(gramsResult);
```

يجب أن يحتوي متغير `gramsResult` على نتيجة استدعاء `convertQuantity("cup")("gram")(2)`.

```js
assert.strictEqual(gramsResult, convertQuantity("cup")("gram")(2));
```

يجب أن تسجل `gramsResult` في وحدة التحكم.
