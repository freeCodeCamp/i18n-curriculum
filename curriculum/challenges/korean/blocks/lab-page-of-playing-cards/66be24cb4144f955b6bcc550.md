...
1. 최소 세 개의 플레이잉 카드를 표시하는 웹페이지를 만드세요.
1. `playing-cards` ID를 가진 `main` 요소가 있어야 합니다.
1. `#playing-cards` 요소 안에는 클래스가 `card`인 `div` 요소가 최소 세 개 있어야 합니다.
1. 각 `.card` 요소 안에는 세 개의 `div` 요소가 있어야 하며, 첫 번째는 클래스가 `left`, 두 번째는 `middle`, 세 번째는 `right`여야 합니다.
1. `#playing-cards` 요소는 플렉스박스를 사용해 자식 요소들을 가로로 가운데 정렬하고, 줄 바꿈을 허용하며, 요소들 사이에 `20px` 간격을 두어야 합니다.
1. 각 `.card` 요소는 플렉스박스를 사용해 자식 요소들을 `space-between`으로 정렬해야 합니다.
1. 각 `.left` 요소는 플렉스박스 아이템 속성을 사용해 부모 컨테이너의 시작 부분에 정렬해야 합니다.
1. 각 `.middle` 요소는 플렉스박스 아이템 속성을 사용해 부모 컨테이너의 중앙에 정렬해야 합니다.
1. 각 `.right` 요소는 플렉스박스 아이템 속성을 사용해 부모 컨테이너의 끝 부분에 정렬해야 합니다.
1. 각 `.middle` 요소는 플렉스박스를 사용해 자식 요소들을 세로 방향(컬럼)으로 표시해야 합니다.
