...
Debe crear dos nuevos elementos `p` al final de su elemento `.daily-value`.
...
Su primer nuevo elemento `p` debe tener el texto `Calcium 260mg 20%`.
...
Su primer nuevo elemento `p` debe tener un elemento `span`.
...
Su primer elemento `span` no debe estar en negrita.
