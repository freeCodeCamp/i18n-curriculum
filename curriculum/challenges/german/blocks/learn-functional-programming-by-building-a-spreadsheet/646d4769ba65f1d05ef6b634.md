...
Jetzt kann Ihre `update()`-Funktion tatsächlich Formeln auswerten. Denken Sie daran, dass Sie die `if`-Bedingung geschrieben haben, um zu überprüfen, ob eine Funktion aufgerufen wurde.

Setzen Sie innerhalb Ihrer `if`-Anweisung die `value` der `element` auf das Ergebnis Ihrer `evalFormula()`-Funktion. Übergeben Sie noch keine Argumente.

# --hints--

Sie sollten die `value`-Eigenschaft von `element` in Ihrem `if`-Block aktualisieren.
...
Sie sollten der `value`-Eigenschaft das Ergebnis des Aufrufs Ihrer `evalFormula()`-Funktion zuweisen.
...
Sie sollten Ihrem `evalFormula()`-Aufruf keine Argumente übergeben.
