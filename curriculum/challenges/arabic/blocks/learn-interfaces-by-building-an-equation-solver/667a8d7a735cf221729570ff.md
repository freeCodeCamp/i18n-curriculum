...
أضف `case` أخرى عندما تكون المعادلة تربيعية. استخدم قاموسًا بنفس التنسيق الذي تُرجعه طريقة `analyze` في `QuadraticEquation`.

بعد ذلك، عيّن لـ `details_list` قائمة تحتوي على سلسلتين نصيتين بالصيغة `concavity = <concavity>` و `<min_max> = (<x>, <y>)` على التوالي. نسق `<x>` و `<y>` لعرض `3` أرقام عشرية.

أخيرًا، بعد كتلة `match`/`case`، قم بالتكرار خلال `details_list` وأضف كل عنصر إلى القيمة الحالية لـ `output_string`. تأكد من أن كل عنصر سلسلة ينتهي بحرف سطر جديد. لا تستخدم أي خيار تنسيق إضافي هنا.

# --hints--

لا تعدل قيمة الموضوع في عبارة `match` الخاصة بك.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[1].find_match_subject().is_equivalent("details")`)) })
```

لا تعدل كتلة `case` الموجودة لديك.
