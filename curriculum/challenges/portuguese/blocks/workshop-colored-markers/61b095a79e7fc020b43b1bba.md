...
Você ainda não verá gradiente porque a função `linear-gradient` precisa de pelo menos dois argumentos de cores para funcionar.

Na mesma função`linear-gradient`, use a função `rgb` para definir o segundo argumento de cores como verde puro.

# --hints--

A regra do CSS `.red` deve ter uma propriedade `background` definida para `linear-gradient(90deg, rgb(255, 0, 0), rgb(0, 255, 0))`.
