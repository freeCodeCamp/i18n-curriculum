...
Для блоку `catch` додайте параметр `error`, а потім використайте `console.error` для логування будь-якої помилки, що виникає під час отримання або обробки даних.

# --hints--

Ваш блок `catch` має мати параметр `error`.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

Ви маєте логувати будь-яку помилку у блоці `catch` за допомогою `console.error`.
