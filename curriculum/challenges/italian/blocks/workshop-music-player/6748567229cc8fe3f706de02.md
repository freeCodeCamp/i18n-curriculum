...
Ora devi lavorare sulla pausa della canzone in riproduzione.

Definisci una funzione `pauseSong` che non prende parametri. All’interno della tua nuova funzione, per archiviare il tempo corrente della canzone quando viene messa in pausa, imposta la proprietà `songCurrentTime` dell’oggetto `userData` al valore `currentTime` della variabile `audio`.


# --hints--

Dovresti avere una funzione `pauseSong`.

```js
assert.isFunction(pauseSong);
```

La tua funzione `pauseSong` dovrebbe impostare la proprietà `songCurrentTime` dell’oggetto `userData` al valore `currentTime` della variabile `audio`.
