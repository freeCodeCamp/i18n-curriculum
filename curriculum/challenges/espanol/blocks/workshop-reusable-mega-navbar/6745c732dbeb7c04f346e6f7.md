...
Cree un elemento `ul` dentro del elemento `nav`. Dentro de ese elemento `ul`, cree tres elementos de lista.

# --hints--

Debes crear un elemento `ul` dentro de tu elemento `nav`.

```js
assert.exists(document.querySelector('nav ul'));
```

Debes crear tres elementos `li` dentro de tu elemento `ul`.
