...
Agora, é hora de definir o que acontece quando o `radio button` está selecionado.

Você está estilizando o círculo interno somente quando o botão de rádio está selecionado, então seu seletor deve ser `.contact-method-radio-btn:checked::before`.

Na regra, defina `transform` como `translate(3px, 3px) scale(1)` e `background-color` como `lightgreen`.

O `transform` move o círculo interno ligeiramente e o escala para o tamanho total com `scale(1)`, e um fundo de lightgreen preenche o círculo com cor para mostrar que a opção está selecionada, tornando-o visível.

# --hints--

Você deve direcionar o pseudo-elemento `.contact-method-radio-btn:checked::before`.
