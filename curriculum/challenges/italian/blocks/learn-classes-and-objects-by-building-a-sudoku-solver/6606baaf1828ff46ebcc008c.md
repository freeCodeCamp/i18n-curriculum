...
`find_empty_cell` restituisce `(0, 0)`, che è la posizione della prima cella vuota nella griglia del sudoku.

Trasforma il primo `0` all'interno di `puzzle` in un `1`. Vedrai nell'output che verrà trovata la prossima cella vuota.

# --hints--

Dovresti modificare il primo elemento della prima lista in `puzzle` in un `1`.
