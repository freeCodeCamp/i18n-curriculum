...
Usa la tua funzione `chunkCrew()` per creare blocchi di dimensione `3` dal tuo array `EVAReadySquad` e archiviarli in una nuova variabile chiamata `EVAChunks`.

# --hints--

Dovresti chiamare `chunkCrew()` con `EVAReadySquad` e `3` e archiviare il risultato in una nuova variabile chiamata `EVAChunks`.
