...
在这个项目中，你将使用来自 `https://cdn.freecodecamp.org/curriculum/labs/data/scatterplot/cyclist-data.json` 的数据来构建散点图。

该任务需要使用 D3 的坐标轴属性生成坐标轴，这个属性会自动生成沿轴的刻度。 通过 D3 测试需要这些刻度，因为它们的位置被用来确定绘制元素的对齐方式。
...
y 轴应包含多个刻度标签，每个标签都带有相应的 `class="tick"`。
...
每个点应具有属性 `data-xvalue` 和 `data-yvalue`，其中包含它们对应的 x 和 y 值。
...
`data-xvalue` 及其对应的点应与 x 轴上的相应点/值对齐。
...
`data-yvalue` 及其对应的点应与 y 轴上的相应点/值对齐。
...
你可以看到一个包含描述性文本且具有 `id="legend"` 的图例。
...
当鼠标悬停在某个区域时，你的散点图应有一个对应的 `id="tooltip"` 的工具提示，显示该区域的更多信息。
...
工具提示应具有一个 `data-year` 属性，该属性对应于活动区域的 `data-xvalue`。
