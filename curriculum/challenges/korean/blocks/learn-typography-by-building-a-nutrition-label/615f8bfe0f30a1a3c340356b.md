...
또 다른 `p` 요소를 만들고, 텍스트를 `Calcium 260mg 20%`로 지정하세요. `20%`를 오른쪽 정렬하세요. 그 아래에 `p` 요소를 만들고 텍스트를 `Iron 8mg 45%`로 지정한 후 `45%`를 오른쪽 정렬하세요.

# --hints--

`p` 요소 끝에 두 개의 새로운 `.daily-value` 요소를 만들어야 합니다.
...
첫 번째 새 `p` 요소에는 텍스트 `Calcium 260mg 20%`가 있어야 합니다.
...
첫 번째 새 `p` 요소에는 `span` 요소가 있어야 합니다.
...
첫 번째 `span` 요소는 텍스트 `20%`를 감싸야 합니다.
