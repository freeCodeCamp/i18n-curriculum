...
Per il secondo `fieldset`, vuoi che il testo `input` e `label` appaia sulla stessa riga.

Inizia assegnando agli elementi `input` nel secondo `fieldset` una classe `inline`.

# --hints--

Dovresti assegnare al primo `input` una classe `inline`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

Dovresti assegnare al secondo `input` una classe `inline`.
