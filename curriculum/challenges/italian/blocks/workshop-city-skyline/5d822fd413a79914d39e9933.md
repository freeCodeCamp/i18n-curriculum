...
Aggiungi un altro `repeating-linear-gradient` sotto quello che hai appena aggiunto. Dagli una direzione `90deg`, usa il colore di build da `0%` a `12%` e il colore della finestra da `12%` a `44%`. Questo creerà una serie di finestre rettangolari.

# --hints--

Dovresti dare a `.fb5` una seconda `repeating-linear-gradient` nella proprietà `background`.
...
Dovresti dare al secondo `repeating-linear-gradient` una direzione `90deg`.
...
Dovresti dare al secondo `repeating-linear-gradient` un primo colore `--building-color2` da `0%` a `12%`.
...
Dovresti dare al secondo `repeating-linear-gradient` un secondo colore `--window-color2` da `12%` a `44%`.
