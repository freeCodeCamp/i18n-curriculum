...
透過使用 `row`、`col` 和 `num` 作為引數呼叫 `valid_in_square()` 方法，檢查該數字是否可在指定的 3x3 方格中插入，並將結果指定給變數 `valid_in_square`。

# --hints--

你應該宣告一個變數 `valid_in_square` 並指定、設值為 `self.valid_in_square(row, col, num)`。
