...
داخل استدعاء راجع `useEffect`، أضف شرط `if` للتحقق مما إذا كانت `query.trim() === ''`. هذا يتحقق مما إذا كانت `query` سلسلة نصية فارغة أو تحتوي فقط على مسافات. إذا كان الأمر كذلك، استدعِ `setResults([])` وارجع مبكرًا.

# --hints--

يجب التحقق مما إذا كانت `query.trim() === ''`.
...
إذا كانت `query.trim() === ''` صحيحة، استدعِ `setResults()` مع مصفوفة فارغة، ثم استخدم الكلمة المفتاحية `return` بعدها.
