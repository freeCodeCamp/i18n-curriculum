...
下一步是创建一个函数，用于检测用户何时从 `playersDropdownList` 中做出选择。 

在 `playersDropdownList` 上使用 `.addEventListener()` 方法。 在事件监听器内部，传入一个 `"change"` 事件类型和一个空的回调函数。

# --hints--

你应该在 `playersDropdownList` 变量上调用 `.addEventListener()` 方法。
