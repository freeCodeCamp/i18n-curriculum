...
Es ist hilfreich, eine Funktion zu haben, die während einer Simulation eine Zusammenfassung des Durchsatzes anzeigt. Erstellen Sie eine leere Funktion namens `printSummary` mit einem Parameter `summary`.

# --hints--

Sie sollten eine Funktion namens `printSummary` erstellen.

```js
assert.isFunction(printSummary);
```

Ihre `printSummary`-Funktion sollte einen Parameter `summary` haben.
