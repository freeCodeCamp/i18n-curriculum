...
就像 Flexbox 一樣，使用 CSS Grid 你可以用 `align-items` 和 `justify-items` 對齊格線項目的內容。`align-items` 將沿著行軸對齊子元素，`justify-items` 將沿著列軸對齊子元素。

給 `.social-icons` 選擇器添加一個 `align-items` 屬性，並設為 `center`。

# --hints--

你的 `.social-icons` 選擇器應該有一個設定為 `center` 的 `align-items` 屬性。
