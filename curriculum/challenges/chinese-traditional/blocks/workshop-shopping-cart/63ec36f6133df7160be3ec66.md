...
記得你可以使用解構賦值，在一個述語中從陣列或物件中擷取多個值。

在你的回呼函式（callback function）的第一個參數中，從傳入的 `name` 實體解構賦值 `id`、`price`、`category` 和 `Dessert` 屬性。

# --hints--

你應該使用解構賦值來宣告 `name`、`id`、`price` 和 `category` 參數。這個測試中，順序很重要。
