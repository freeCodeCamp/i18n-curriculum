...
あなたの方程式は現在 `11x**2 -1x +1 = 0` として表されていますが、係数が1のときに \\( x \\) にかかる係数を表示しないほうが見やすいです。そこで、その方程式は `11x**2 -x +1 = 0` として表されます。

`re` モジュールをインポートしてください。次のステップで、この場合の係数を置き換えるために正規表現を使います。
