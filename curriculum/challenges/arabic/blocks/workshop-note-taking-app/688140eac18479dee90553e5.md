...
بما أنك تستخدم عنصر `div` آخر، ستحتاج إلى جعله أكثر وصولًا لقارئي الشاشة.

في درس سابق، تعلّمت عن الخاصية `aria-live` التي تُستخدم لإنشاء منطقة حية على الصفحة. هذا يسمح لمستخدمي قارئ الشاشة بأن يتم إعلامهم تلقائيًا عند تغير محتوى المنطقة الحية، دون الحاجة إلى تركيز يدوي أو التفاعل معها.

في عنصر `#status` الخاص بك، أضف خاصية `aria-live` واضبط قيمتها على `"polite"`.

القيمة `polite` تخبر قارئي الشاشة بالانتظار حتى انتهاء أي إعلان جاري أو حتى يتوقف المستخدم عن الكتابة قبل إعلان التحديث.

# --hints--

يجب أن يحتوي عنصر `#status` على الخاصية `aria-live`.

```js
assert.isTrue(document.getElementById("status").hasAttribute("aria-live"));
```

يجب أن يحتوي عنصر `#status` على الخاصية `aria-live` مضبوطة على القيمة `"polite"`.
