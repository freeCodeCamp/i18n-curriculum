...
Às vezes, você precisará testar mais de uma coisa de cada vez. O operador <dfn>e lógico</dfn> (`&&`) retorna `true` se, e somente se, os <dfn>operandos</dfn> à esquerda e à direita dele forem verdadeiros.
...
Substitua as duas instruções if por uma única instrução, usando o operador `&&`, que retornará a string `Yes` se `val` for menor ou igual a `50` e maior ou igual a `25`. Caso contrário, retornará a string `No`.
