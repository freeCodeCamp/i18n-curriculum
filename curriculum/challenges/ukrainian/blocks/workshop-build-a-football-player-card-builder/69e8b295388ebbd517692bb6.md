...
Під `card-rating` div додайте `div` з `className` `card-position` і текстом `ST`, що позначає striker.

# --hints--

Ви повинні додати `div` з `className` `card-position` під `.card-rating` div.

```js
assert.exists(document.querySelector('.card-rating + .card-position'));
```

Елемент `.card-position` має містити текст `ST`.
