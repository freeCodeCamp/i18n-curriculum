...
`--building-color1` 変数を `background-color` の `.fb4b` プロパティの値として追加してください。それから、`background-color` から `.fb4` を削除してください。

# --hints--

`background-color` から `.fb4` を削除する必要があります。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb4")?.backgroundColor);
```

`.fb4b` に `background-color` の `--building-color1` を与える必要があります。
