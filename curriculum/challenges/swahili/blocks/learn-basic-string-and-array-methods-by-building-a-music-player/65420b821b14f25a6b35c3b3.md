...
Wakati nyimbo zinaonyeshwa kwenye ukurasa, inapaswa kuonyesha kichwa, msanii, muda wa kila wimbo na kitufe cha kufuta.

Katika hatua chache zinazofuata, utafundishwa jinsi ya kuongeza HTML mpya kwa kila wimbo kwa kutumia njia ya <dfn>map()</dfn>. Njia hii itafafanuliwa kwa undani zaidi katika hatua inayofuata.

Anza kwa kutumia `const` kutangaza kigezo kinachoitwa `songsHTML` na kuweka thamani `array.map()`.

# --hints--

Unapaswa kuwa na kigezo cha `const` kinachoitwa `songsHTML`.

```js
assert.match(code, /const\s+songsHTML/);
```

Unapaswa kuweka thamani ya `array.map()` kwa `songsHTML` yako.
