...
是時候實作播放清單的刪除功能了。這將管理從播放清單中移除歌曲、控制刪除歌曲時的其他相關動作，並創建一個 `Reset Playlist` 按鈕。

使用 `const` 和箭頭語法創建一個空的 `deleteSong` 函式（程式），並傳入 `id` 作為參數。

# --hints--

你應該使用 `const` 來創建一個名為 `deleteSong` 的空函式（程式）。

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` 應該是一個函式（程式）。

```js
assert.isFunction(deleteSong)
```

你的 `deleteSong` 函式應該使用箭頭語法。

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

你的 `deleteSong` 函式（程式）應該接受一個 `id` 參數。

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

你的 `deleteSong` 函式（程式）應該是空的。
