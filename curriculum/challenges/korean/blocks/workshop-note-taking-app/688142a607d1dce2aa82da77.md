...
편집한 변경 사항을 콘솔에서 확인할 수 있으면 좋겠습니다.

`console.log(currentContent);`를 추가하세요.

변경 사항을 테스트하려면, 노트를 편집한 다음 노트 밖을 클릭해 보세요. 이제 콘솔에서 편집한 노트를 볼 수 있어야 합니다.

# --hints--

이벤트 리스너 안에 `console.log(currentContent);`를 추가하세요. `newContent`를 `currentContent`에 할당한 후에 추가하면 됩니다.
