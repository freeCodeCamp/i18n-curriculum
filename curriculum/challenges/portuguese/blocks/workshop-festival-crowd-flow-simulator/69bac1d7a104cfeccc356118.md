...
Para completar sua função `simulateFestival`, exiba um resumo dos resultados da simulação. Faça uma chamada a `printSummary()` com seu objeto local `throughputSummary`.

# --hints--

Você deve fazer uma chamada a `printSummary()` com `throughputSummary`.
