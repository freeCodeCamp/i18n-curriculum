...
Dentro da função de callback, use o método `removeAttribute()` para remover o atributo `"aria-current"`. Isso removerá o atributo para cada uma das músicas.

# --hints--

Você não deve modificar o `forEach()` existente e seu conteúdo.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

Você deve usar o método `removeAttribute()` em `songEl` e passar `"aria-current"`.
