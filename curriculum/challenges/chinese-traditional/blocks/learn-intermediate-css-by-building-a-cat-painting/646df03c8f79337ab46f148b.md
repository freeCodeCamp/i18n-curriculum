...
將鼻子移動到合適的位置，設置 `position` 爲 `absolute`、`top` 爲 `108px`，`left` 爲 `85px`。

# --hints--

你的 `.cat-nose` 選擇器應將 `position` 屬性設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.position === 'absolute')
```

你的 `.cat-nose` 選擇器應將 `top` 屬性設置爲 `108px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.top === '108px')
```

你的 `.cat-nose` 選擇器應將 `left` 屬性設置爲 `85px`。
