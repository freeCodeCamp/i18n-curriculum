...
`mean` の値を表示するために、アプリにはすでに `#mean` 要素が用意されています。

`.querySelector` を使ってその要素を見つけ、`.textContent` を `mean` の値に設定してください。

# --hints--

`calculate` 関数は `document.querySelector()` を使うべきです。

```js
assert.lengthOf(calculate.toString().match(/document\.querySelector\(/g), 2);
```

`.querySelector()` は `#mean` 要素をターゲットにしてください。

```js
assert.match(calculate.toString(), /document\.querySelector\(\s*('|")#mean\1\s*\)/);
```

`calculate` 関数は `#mean` 要素の `.textContent` を `mean` の値に設定するべきです。
