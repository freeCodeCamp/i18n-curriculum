...
第一个 `option` 元素的 `value` 为 `""`，文本为 `Select an option`。 第二个 `option` 的 `value` 为 `yes`，文本为 `Yes`。 第三个 `option` 的 `value` 为 `no`，文本为 `No`。
...
你应将第二个 `option` 元素的 `value` 设置为 `yes`。
...
你应将第三个 `option` 元素的 `value` 设置为 `no`。
