...
Le impostazioni predefinite per CSS Grid creeranno righe aggiuntive secondo necessità, a differenza di Flexbox. Imposta la proprietà `.social-icons` del selettore `grid-template-columns` su `repeat(5, 1fr)` per disporre le icone in un'unica riga.

# --hints--

Il tuo selettore `.social-icons` dovrebbe avere una proprietà `grid-template-columns` impostata su `repeat(5, 1fr)`.
