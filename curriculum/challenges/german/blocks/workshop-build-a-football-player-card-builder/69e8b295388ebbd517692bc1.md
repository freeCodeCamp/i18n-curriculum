...
Da das `PlayerData`-Interface jetzt definiert ist, aktualisieren Sie die `PlayerCard`-Komponente, damit sie eine `player`-Prop akzeptiert, die als `PlayerData` typisiert ist, um dem `PlayerData`-Interface zu entsprechen.

Denken Sie daran, dass Sie Props so destrukturieren können:
...
Sie sollten `PlayerCard` aktualisieren, damit es eine `player`-Prop akzeptiert, die als `PlayerData` typisiert ist.
