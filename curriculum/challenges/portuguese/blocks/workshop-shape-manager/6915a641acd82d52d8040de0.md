...
De volta à função `initializeApp`, é aqui que você vai colocar em uso os tipos inline e a função `getElement()` que criou anteriormente, obtendo os elementos do DOM necessários para o app.

Dentro da função `initializeApp`, reatribua `shapeTypeSelect` chamando `getElement()` com `"shape-type"`, depois use a palavra-chave `as` para converter o resultado para `HTMLSelectElement`.

# --hints--

Você deve reatribuir `shapeTypeSelect` com o valor apropriado.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

Você deve chamar a função `getElement` com `shape-type` e atribuí-la a `shapeTypeSelect`.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

Você deve chamar `getElement` com `HTMLSelectElement` como o tipo de `shapeTypeSelect` usando a palavra-chave `as`.
