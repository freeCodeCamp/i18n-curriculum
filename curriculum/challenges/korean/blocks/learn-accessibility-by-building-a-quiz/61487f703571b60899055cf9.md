...
`section` 요소의 너비를 부모 컨테이너 너비의 `80%`로 설정하세요. 그런 다음, `section` 요소를 가운데 정렬하기 위해 여백을 사용하고, 아래쪽 여백에는 `10px`를 추가하세요.

또한, `section` 요소가 너비가 `600px`보다 커질 수 없도록 하세요.
