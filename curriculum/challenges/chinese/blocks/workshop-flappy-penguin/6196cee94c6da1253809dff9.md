...
定位 `.back-mountain` 元素，并将其 `width` 和 `height` 设置为 `300px`。 然后，将其 `background` 设置为从 `rgb(203, 241, 228)` 到 `rgb(47, 170, 255)` 的线性渐变。

# --hints--

你应该使用 `.back-mountain` 选择器。

```js
assert.match(code, /\.back-mountain\s*\{/);
```

你应该将 `.back-mountain` 的 `width` 设置为`300px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

你应该将 `.back-mountain` 的 `height` 设置为`300px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

你应该将 `.back-mountain` 的 `background` 设置为 `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`。
