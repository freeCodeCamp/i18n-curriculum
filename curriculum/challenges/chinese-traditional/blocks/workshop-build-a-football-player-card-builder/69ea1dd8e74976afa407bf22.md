...
接著，創建一個型別 `Position`，其值為 `typeof POSITIONS[number]`。這會將 `POSITIONS` 陣列拆解成其各個字串字面值的聯集（數學），實際上讓它看起來像 `"GK" | "CB" | "LB" | ...` 等等。

# --hints--

你應該創建一個型別別名，名為 `Position`。
...
你的 `Position` 型別應該設定為 `typeof POSITIONS[number]`。
