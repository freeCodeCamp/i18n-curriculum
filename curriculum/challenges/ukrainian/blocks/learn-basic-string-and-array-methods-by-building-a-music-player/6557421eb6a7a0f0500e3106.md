...
У межах елемента button у функції `renderSongs` додайте атрибут `onclick`. Для значення викличте функцію `deleteSong` і вставте `song.id`.

# --hints--

Ви повинні додати атрибут `onclick` до кнопки видалення і передати `"deleteSong(${song.id})"`.
