...
配列の先頭に数値 `10` が配置されているのに気づきましたか。これは `.sort()` のデフォルトの動作が数値の値を文字列に変換し、アルファベット順に並べ替えるためです。そして `"10"` は `"2"` よりもアルファベット順で先に来ます。

これを修正するには、`.sort()` メソッドにコールバック関数を渡すことができます。コールバック関数には2つのパラメータがあり、ここでは `a` と `b` を使います。`a` と `b` のパラメータは、並べ替えられる配列内の数値の値を表します。

関数は今のところ空のままにしてください。

# --hints--

`.sort()` メソッドにコールバック関数を渡す必要があります。アロー構文を使うことを忘れないでください。

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

コールバック関数には2つのパラメータ、`a` と `b` が必要です。
