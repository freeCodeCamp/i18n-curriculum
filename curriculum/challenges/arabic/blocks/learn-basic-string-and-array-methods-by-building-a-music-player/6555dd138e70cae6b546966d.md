...
تحتاج إلى الحصول على الأغنية التي تُشغل حاليًا أو أول أغنية في قائمة التشغيل. للقيام بذلك، أنشئ ثابتًا باسم `song` واستخدم عامل OR (`||`) لتعيينه إلى الأغنية الحالية في `userData`، **أو** أول أغنية في مصفوفة `userData?.songs`.

لا تنسَ استخدام التسلسل الاختياري.

# --hints--

لا يجب تعديل الدالة `setPlayButtonAccessibleText` الحالية أو محتواها.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

يجب أن تصل إلى `userData?.currentSong` أو `userData?.songs[0]`.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

يجب تعيين `userData?.currentSong || userData?.songs[0]` إلى ثابت `song`.
