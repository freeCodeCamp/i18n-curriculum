...
Usa un loop `for...in` per iterare su ogni gate nell'oggetto `summary` di input e registra il totale dei partecipanti processati.

Per ogni gate, registra un messaggio in questo formato: `gate + ": " + summary[gate] + " attendees processed"`.

Come esempio, per un oggetto `summary` di input definito come `{ North: 5, East: 3, South: 4, West: 2 }`, dovresti registrare:
...
Dovresti iterare su ogni elemento nell'oggetto `summary` e registrare il seguente messaggio una volta per elemento: `element + ": " + summary[element] + " attendees processed`.
