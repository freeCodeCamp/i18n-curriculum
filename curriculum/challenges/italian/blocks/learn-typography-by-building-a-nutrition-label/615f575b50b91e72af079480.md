...
Crea un nuovo selettore `.left-container p` impostando il margine superiore e inferiore su `-5px` e il margine sinistro e destro su `-2px`. Imposta anche la proprietà `font-size` su `2em` e `font-weight` su `700`.

# --hints--

Dovresti avere un nuovo selettore `.left-container p`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

Il tuo nuovo selettore `.left-container p` dovrebbe avere una proprietà `margin` impostata su `-5px -2px`.
...
Il tuo nuovo selettore `.left-container p` dovrebbe avere una proprietà `font-size` impostata su `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

Il tuo nuovo selettore `.left-container p` dovrebbe avere una proprietà `font-weight` impostata su `700`.
