...
Erstelle unter dein Element mit dem `Total Fat`-Text ein neues `p`-Element mit dem Text `Saturated Fat 1g 5%`. Verpacke die `5%` in ein `span` mit dem `class` Attribut von `bold`. In diesem Fall reicht dies aus, um den Prozentsatz auf `5%` auszurichten.
