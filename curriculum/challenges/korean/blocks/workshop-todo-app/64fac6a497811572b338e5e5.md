...
또한 `hidden` 이벤트 리스너 안에서 `taskForm`에 `discardBtn` 클래스를 토글하는 기존 코드를 제거하고 대신 `reset` 함수를 호출하세요. `Discard` 버튼을 클릭하면 입력 필드의 모든 내용이 사라져야 하기 때문입니다.

# --hints--

`hidden`에 `taskForm` 클래스를 토글하는 코드를 제거해야 합니다.
