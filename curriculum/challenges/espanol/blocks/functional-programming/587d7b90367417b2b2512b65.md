...
Utilice el método `slice` en la función `sliceArray` para devolver parte del arreglo `anim` dados los índices `beginSlice` y `endSlice` proporcionados. La función debe retornar un arreglo.
...
La variable `inputAnim` no debería cambiar.
