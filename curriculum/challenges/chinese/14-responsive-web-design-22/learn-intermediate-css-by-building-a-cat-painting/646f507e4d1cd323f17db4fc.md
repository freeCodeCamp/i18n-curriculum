...
使用类选择器来定位 `.cat-whisker-right-bottom` 元素，然后通过设置 `position` 为`absolute`、`top` 为 `134px`、`left` 为 `109px` 来将其移动到适当的位置。


# --hints--

你应该有一个 `.cat-whisker-right-bottom` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom'))
```

你的 `.cat-whisker-right-bottom` 选择器应设置一个 `position` 属性为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.position === 'absolute')
```

你的 `.cat-whisker-right-bottom` 选择器应将 `top` 属性设置为 `134px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.top === '134px')
```

你的 `.cat-whisker-right-bottom` 选择器应将 `left` 属性设置为 `109px`。
