...
أنشئ محدِّدًا جديدًا باسم `.indent` وأعطه خاصية `margin-left` بقيمة `1em`.

# --hints--

يجب أن يكون لديك محدِّد جديد باسم `.indent`.

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('.indent'));
```

يجب أن يحتوي محدِّد `.indent` على خاصية `margin-left` بقيمة `1em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.indent')?.marginLeft, '1em');
```

يجب أن يحدد محدِّد `.indent` خاصية `margin-left` فقط، وليس خصائص الهامش الأخرى.
