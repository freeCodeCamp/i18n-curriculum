...
Ändere die `height` Eigenschaft der `hr` auf `2px`, so dass die Gesamthöhe `4px` ergibt.

# --hints--

Du solltest die `height` Eigenschaft auf `2px` setzen.
...
Dein `hr` Element sollte eine `height` von `2px` haben.
