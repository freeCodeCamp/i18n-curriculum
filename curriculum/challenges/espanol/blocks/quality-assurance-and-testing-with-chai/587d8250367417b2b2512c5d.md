...
Cuando la solicitud se completa con éxito, el código cliente añade un `<div>` que contiene la información de la respuesta al DOM.
...
Primero, el método `fill` del objeto `browser` llena el campo `surname` del formulario con el valor `'Polo'`. `fill` devuelve una promesa, por lo que se encadena `then` a partir de ella.
...
4.  Hay `1` elemento `<span id='dates'></span>`.
