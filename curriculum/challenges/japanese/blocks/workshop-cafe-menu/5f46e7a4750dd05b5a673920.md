...
では、`123 Free Code Camp Drive` という住所を含む `p` 要素に、`address` クラスを適用してください。

# --hints--

`class="address"` という属性を加える必要があります。

```js
assert.match(code, /class=('|")address\1/i);
```

`.address` 要素は `p` 要素である必要があります。

```js
assert.equal(document.querySelector('.address')?.tagName, 'P');
```

`.address` 要素には `123 Free Code Camp Drive` というテキストが必要です。
