...
接下来，使用 `includes()` 方法查看 `item` 的小写版本是否包含 `query` 的小写版本作为子字串。

例如，下面演示如何使用 `toLowerCase()` 和 `includes()` 方法来查看字串 `freeCodeCamp` 是否包含 `code`：
...
将 `includes()` 方法链式调用到小写的 `item`，并查看它是否包含小写的 `query`。记得也要将 `query` 转为小写。

# --hints--

你应该将 `includes()` 方法链式调用到小写的 `item`。
...
你应该查看小写的 `item` 字串是否包含小写的 `query` 作为子串。
