...
Tengeneza njia nyingine iitwayo `get_tv_series` na uifanye irudishe orodha yenye matukio yote ya `TVSeries` unayoweza kuyapata katika `self.items`.

# --hints--

Darasa lako la `MediaCatalogue` linapaswa kuwa na njia iitwayo `get_tv_series`.
...
Njia yako ya `get_tv_series` inapaswa kurudisha orodha inayojumuisha vitu vyote katika `self.items` ambavyo ni matukio ya darasa la `TVSeries`.
