...
في هذه المرحلة، يجب أن تكون قادرًا على رؤية الحظ الذي تظهره كل بطاقة بمجرد نقر بطاقة. الشيء الوحيد المتبقي هو جعل زر القراءة الجديدة يعمل.

داخل طريقة `newReading`، نفذ استدعاء `showElements()` و `hideElements()`.

# --hints--

يجب أن تستدعي دالة `showElements` داخل طريقة `newReading`.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

يجب أن تستدعي دالة `hideElements` داخل طريقة `newReading`.
