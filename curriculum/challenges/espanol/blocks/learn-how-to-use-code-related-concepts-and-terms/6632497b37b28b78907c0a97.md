...
Los pronombres demostrativos como `this`, `that`, `these` y `those` se usan para señalar cosas o personas específicas. `This` y `these` se refieren a objetos que están cerca del hablante. `This` se usa para elementos singulares, y `these` para plurales. `That` y `those` se refieren a objetos más lejanos, usando `that` para singular y `those` para plural. 
