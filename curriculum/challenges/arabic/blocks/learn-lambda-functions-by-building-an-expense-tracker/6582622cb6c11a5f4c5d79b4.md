...
لا تزال داخل بند `elif`، مرر المكرر `expenses_from_category` إلى استدعاء `print_expenses`.

# --hints--

يجب أن تستدعي `print_expenses()` مع تمرير `expenses_from_category` كمعلمة في بند `elif` الخاص بك.
...
يجب أن يأتي استدعاء `print_expenses()` بعد متغير `expenses_from_category`.
