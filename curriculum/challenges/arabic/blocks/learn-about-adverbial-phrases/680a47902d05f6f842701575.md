...
`current load`، `definitely`، `feature`، `increase`، `responds`، `start small`، و`write up`.
...
هذا يشير إلى جزء أو دالة محددة في منتج برمجي.
...
هذا يضيف تأكيدًا على اقتراح أو قرار.
...
هذا يعني كيف يتفاعل أو يرد شيء ما على طلب أو إجراء. استخدم صيغة الفعل المفرد الغائب لهذا الفعل.
...
هذا يتكون من صفة واسم. الصفة تشير إلى الوقت الحالي، والاسم يشير إلى كمية العمل التي يتعامل معها النظام.
...
هذا يتكون من فعل وصفة. يعني البدء بنطاق محدود.
...
هذا يعني جعل شيء ما أكبر في الكمية أو العدد.
...
هذا الفعل المركب يعني إعداد وثيقة أو تقرير مفصل.
