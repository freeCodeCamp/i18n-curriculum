...
配列のサイズは範囲の大きさにする必要があります。これは `end` と `start` の差を求め、その結果に `1` を加えることで計算できます。

この計算結果を `Array()` コンストラクタの引数として渡してください。

# --hints--

`start` から `end` を引く必要があります。

```js
assert.match(code, /end\s*-\s*start/);
```

`1` の計算に `end - start` を加える必要があります。

```js
assert.match(code, /end\s*-\s*start\s*\+\s*1/);
```

計算結果を `Array()` コンストラクタに渡してください。
