...
بداخل عنصر `.sun` يجب أن تكون خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.top, '-75px');
```

بداخل منتقي `.sun` يجب أن تكون خاصية `right` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
