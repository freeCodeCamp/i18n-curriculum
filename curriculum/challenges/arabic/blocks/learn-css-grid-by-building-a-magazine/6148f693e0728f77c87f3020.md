...
أنشئ استعلام `@media` ثالث لـ `only screen` مع `max-width` بقيمة `550px`. بداخله، أنشئ محدِّد `.hero-title` مع تعيين `font-size` إلى `6rem`، ومحدِّد `.hero-subtitle, .author, .quote, .list-title` مع تعيين `font-size` إلى `1.8rem`، ومحدِّد `.social-icons` مع تعيين `font-size` إلى `2rem`، ومحدِّد `.text` مع تعيين `font-size` إلى `1.6rem`.

# --hints--

يجب أن يكون لديك استعلام `@media` جديد لـ `only screen` مع `max-width` بقيمة `550px`. يجب أن يأتي هذا بعد الاستعلامين السابقين.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

يجب أن يحتوي قاعدة `@media` الجديدة على محدِّد `.hero-title`، ومحدِّد `.hero-subtitle, .author, .quote, .list-title`، ومحدِّد `.social-icons`، ومحدِّد `.text`. يجب أن تكون هذه المحدِّدات بهذا الترتيب.
