...
你将使用我们的 Ona 公式化代码<a href="https://app.ona.com/?autostart=true#https://github.com/freeCodeCamp/boilerplate-rock-paper-scissors/" target="_blank" rel="noopener noreferrer nofollow">来完成这个项目</a>。
...
*提示：要击败所有四个对手，你的程序可能需要根据对手的出招变化采用多种策略。*
...
这个项目的单元测试在 `test_module.py` 中。 为了你的方便，我们将测试从 `test_module.py` 导入到 `main.py`。 你如果移除 `main.py` 中最后一行的注释，那么每次在控制台执行 `python main.py` 时，测试都会自动运行。
