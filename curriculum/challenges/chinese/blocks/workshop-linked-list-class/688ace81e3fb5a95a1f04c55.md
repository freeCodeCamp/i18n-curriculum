...
接下来，创建一个 `else` 块，将 `current_node.next` 赋值给 `self.head`。

这处理了要删除的元素位于头节点的分支。通过将 `self.head` 更新为指向 `current_node.next`，头节点从链表中被移除。

# --hints--

你应该创建一个 `else` 块，将 `current_node.next` 赋值给 `self.head`。
