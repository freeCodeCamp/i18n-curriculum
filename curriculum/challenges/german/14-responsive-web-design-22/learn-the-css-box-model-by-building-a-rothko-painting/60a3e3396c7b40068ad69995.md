...
Verwende die `transform`-Eigenschaft beim `.one`-Selektor, um es um 0,6 Grad gegen den Uhrzeigersinn mittels `rotate` zu drehen.

# --hints--

Du solltest die `transform` Eigenschaft auf `rotate(-0.6deg)` setzen.
...
Dein `.one`-Element sollte einen `transform`-Wert von `rotate(-0.6deg)` enthalten.
