...
Crie um elemento `button` com a classe `playlist-song-info`.Dentro do `button`, adicione um elemento `span` com a classe `playlist-song-title`, depois interpole `song.title` como o texto.

# --hints--

Você deve criar um `button` com a classe `playlist-song-info`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

Você deve criar um elemento `span` dentro do seu elemento `button`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

Seu elemento `span` deve ter a classe `playlist-song-title`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

Você deve interpolar `song.title` como o conteúdo do seu elemento `span`.
