...
開発中に`fieldset`要素の周りにあるデフォルトのボーダーを見るのは便利です。しかし、それが望むスタイルとは限りません。

`.question`要素のボーダーと下のパディングを削除してください。

# --hints--

`.question`セレクターを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

`.question`に`border`の`none`を与えるべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

`.question`に`padding-bottom`の`0`を与えるべきです。
