...
Ora combina tutte le righe in un'unica stringa CSV che puoi esportare o condividere.

Dopo il primo loop, dichiara `let csv = header`. Aggiungi un altro loop `for` per aggiungere ogni riga a `csv` con una nuova riga: `csv = csv + "\n" + rows[i]`. Ritorna `csv`. Poi, sotto la funzione, stampa `exportToCSV(catalog)` per vedere l'output formattato.

# --hints--

`exportToCSV` dovrebbe restituire una stringa.

```js
assert.isString(exportToCSV(catalog));
```

La prima riga della stringa restituita dovrebbe essere `"Title,Author,Year,Location"`.

```js
assert.strictEqual(exportToCSV(catalog).split("\n")[0], "Title,Author,Year,Location");
```

L'output dovrebbe contenere `"King, Stephen"` (tra virgolette per gestire la virgola).

```js
assert.include(exportToCSV(catalog), '"King, Stephen"');
```

Dovresti stampare `exportToCSV(catalog)` nella console.
