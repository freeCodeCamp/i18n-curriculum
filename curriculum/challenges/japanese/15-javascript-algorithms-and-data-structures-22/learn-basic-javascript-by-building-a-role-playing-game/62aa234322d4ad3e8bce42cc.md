...
プレイヤーの唯一の武器は壊したくありません。 論理 AND 演算子は、2 つのステートメントがどちらも真であるかどうかを確認します。

<dfn>論理積 (AND)</dfn> 演算子 `&&` を使って、`if` 文に第二の条件を追加しましょう。 プレイヤーの武器は、`inventory.length` が 1 に等しくない (`!==`) 場合にのみ壊れるようにしてください。

こちらが、二つの条件を持つ `if` 文の例です:
...
論理 AND 演算子 `&&` を使用してください。

```js
assert.match(attack.toString(), /if\s*\(\s*Math\.random\(\s*\)\s*\<=\s*\.1\s*&&/);
```

論理 AND 演算子を使用して、`inventory.length` が `1` に等しくないことをチェックしてください。
