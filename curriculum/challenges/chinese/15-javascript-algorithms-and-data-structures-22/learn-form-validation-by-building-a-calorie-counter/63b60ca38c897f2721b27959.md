...
你的表单需要某个地方来显示结果。 添加一个空的 `div` 元素并为其指定 `id` `output`，以及 `class` 值 `output` 和 `hide`。

# --hints--

你应该在 `form` 后添加一个 `div` 元素。

```js
assert.exists(document.querySelector('.container > div'));
```

你的新 `div` 元素应将 `id` 设置为 `output`。

```js
assert.equal(document.querySelector('.container > div')?.id, 'output');
```

你的新 `div` 元素应具有 `output` 和 `hide` 的 `class` 值。
