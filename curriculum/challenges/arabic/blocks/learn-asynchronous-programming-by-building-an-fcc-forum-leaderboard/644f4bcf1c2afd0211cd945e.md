...
كل فئة سيكون لها عنوان URL يشير إلى الفئة على [منتدى freeCodeCamp](https://forum.freecodecamp.org/).

أنشئ ثابتًا اسمه `url` وقم بتعيين قالب حرفي له. داخل هذا القالب الحرفي، ضع القيمة `${forumCategoryUrl}${selectedCategory.className}/${id}`.

# --hints--

يجب أن يكون لديك ثابت اسمه `url`.

```js
assert.match(code, /const\s+url\s*=/);
```

يجب تعيين قوالب حرفية إلى المتغير `url` الخاص بك.

```js
assert.match(code, /const\s+url\s*=\s*`[\s\S]*`/);
```

يجب أن تحتوي القوالب الحرفية الخاصة بك على القيمة `${forumCategoryUrl}${selectedCategory.className}/${id}`.
