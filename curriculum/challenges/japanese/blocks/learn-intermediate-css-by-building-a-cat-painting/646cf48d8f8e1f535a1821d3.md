...
右耳を、`position` プロパティを `absolute` に、`top` を `-26px` に、`left` を `163px` に設定して位置調整してください。

# --hints--

`.cat-right-ear` セレクターには、`position` プロパティを `absolute` に設定してください。セミコロンを忘れないでください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.position === 'absolute')
```

`.cat-right-ear` セレクターには、`top` プロパティを `-26px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.top === '-26px')
```

`.cat-right-ear` セレクターには、`left` プロパティを `163px` に設定してください。
