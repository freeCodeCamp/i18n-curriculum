...
أضف مفتاحًا آخر `gender` إلى قاموس `constraints`. باتباع صيغة التعبير الذي كتبته في الخطوات السابقة، تحقق من أن `gender` هو سلسلة نصية. ثم استخدم عامل `and` للتحقق من أن `gender` بالأحرف الصغيرة موجود في `('male', 'female')`.

# --hints--

يجب أن يحتوي قاموس `constraints` على مفتاح `gender`.
...
يجب أن يكون لمفتاح `gender` في قاموس `constraints` القيمة `isinstance(gender, str) and gender.lower() in ('male', 'female')`.
