...
إذا لم يكن الفرق ضمن `tolerance` المحددة، أنشئ جملة `elif` للتحقق مما إذا كان `square_mid` أقل من `square_target`.

عيّن قيمة `mid` إلى `low` لأن الجذر التربيعي سيكون الآن بين `low` و `mid`.

# --hints--

يجب أن تنشئ جملة `elif` تتحقق مما إذا كان `square_mid < square_target`.
