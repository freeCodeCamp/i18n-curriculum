...
지금까지 `.face`와 `.chin` 요소는 같은 `background-color`를 가지고 있습니다.

`--penguin-face`라는 사용자 정의 CSS 속성을 만들고, 그것을 `white`로 설정하세요.
...
`:root`에 `--penguin-face` 속성을 지정하세요.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--penguin-face'));
```

`:root`에 `--penguin-face`의 `--fcc-expected--`를 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.
