...
你的最终正则表达式将会查找类似 `dear friend` 的字符串。 声明一个 `dearRegex` 并赋值一个匹配字符串 `dear friend` 的正则表达式。 记得忽略大小写，并将其添加到你的 `denyList` 数组中。
...
你的 `dearRegex` 应该是不区分大小写的。

```js
assert.include(dearRegex.flags, 'i');
```

你的 `denyList` 数组应包含 `dearRegex`。
