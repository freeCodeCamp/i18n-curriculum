...
最初の `td` 要素内に、埋め込み式 `${}` を追加してください。 その埋め込み式の中で、`forumCategory` 関数に実引数 `category_id` を渡して呼び出してください。

これで各トピックの下にカテゴリーが表示されるようになるはずです。

# --hints--

You should call the `forumCategory` function with the argument of `category_id`
