...
اربط `.catch()` بآخر `.then()`. مرر دالة استدعاء راجع مع `err` كمعلمة. داخل الدالة، استخدم `console.error()` لتسجيل الأخطاء المحتملة في وحدة التحكم مع النص `There was an error: ${err}`. بما أنك تستخدم `err` في النص، لا تنسَ استخدام سلسلة نصية قالبية مع علامات دقيقة (`\`\`) بدلاً من علامات اقتباس مفردة أو مزدوجة.
...
يجب ربط طريقة `.catch()` بطريقة `.then()` الثانية الموجودة لديك.
