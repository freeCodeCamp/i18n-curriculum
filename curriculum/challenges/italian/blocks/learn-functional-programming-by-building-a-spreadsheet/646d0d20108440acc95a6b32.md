...
Nella tua funzione `elemValue` hai dichiarato esplicitamente una funzione chiamata `inner` e l'hai restituita. Tuttavia, poiché stai usando la sintassi a funzione freccia, puoi restituire implicitamente una funzione. Per esempio:

```js
const curry = soup => veggies => {};
```

`curry` è una funzione che prende un parametro `soup` e restituisce una funzione che prende un parametro `veggies`. Usando questa sintassi, aggiorna la tua funzione `addCharacters` per restituire una funzione vuota che prende un parametro `character2`.

# --hints--

La tua funzione `addCharacters` dovrebbe usare un ritorno implicito.
...
La tua funzione `addCharacters` dovrebbe restituire una funzione freccia che ha un parametro `character2`.
...
La tua funzione freccia interna dovrebbe essere vuota.
