...
У попередніх кроках ви створили функціональність для відтворення пісні. Тепер потрібно додати функціональність до кнопки відтворення, щоб вона відтворювала поточну пісню, коли її натиснуто.

Використайте метод `addEventListener()` та передайте подію `"click"` для першого аргументу та порожню функцію зворотного виклику зі стрілковим синтаксисом для другого аргументу, наприклад, `() => {}`.

# --hints--

Викличте метод `addEventListener()` на змінній `playButton`.

```js
assert.match(code, /playButton\.addEventListener\(/)
```

Слухач подій повинен слухати по подію `"click"`.

```js
assert.match(code, /playButton\.addEventListener\(\s*('|"|`)click\1/)
```

Використайте стрілковий синтаксис, щоб передати порожній зворотний виклик до слухача подій. Не забудьте, що вам також потрібна порожня пара фігурних дужок.
