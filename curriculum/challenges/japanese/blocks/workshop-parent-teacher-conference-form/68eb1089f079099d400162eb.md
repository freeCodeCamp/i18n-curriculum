...
HTMLのいくつかの要素に`center`クラスを追加しました。次に、そのクラスを定義して要素を中央に配置してください。

`center`クラスをターゲットにして、そのクラスを持つ要素を`text-align`プロパティを`center`に設定して中央に配置してください。

# --hints--

`center`クラスを持つ要素をターゲットにしてください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.center'));
```

`text-align`プロパティを`center`に設定してください。
