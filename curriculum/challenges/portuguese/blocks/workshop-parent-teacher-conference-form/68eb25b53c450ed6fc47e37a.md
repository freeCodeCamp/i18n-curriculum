...
Por padrão, os elementos `label` são inline. Para fazê-los empilhar verticalmente (exceto para os seus labels de botão de rádio), use a pseudo-classe `:not()`.

A pseudo-classe `:not()` nega uma seleção. Aqui, ela seleciona todos os elementos `label` que não possuem a classe `contact-method`.

Selecione `label:not(.contact-method)`, depois defina `display` como `block` e `margin` como `10px 0`.

# --hints--

Você deve ter um seletor `label:not(.contact-method)`.
...
Defina a propriedade `margin` para `10px 0`.
