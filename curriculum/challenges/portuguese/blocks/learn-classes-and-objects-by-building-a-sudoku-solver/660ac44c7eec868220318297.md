...
Modifique a compreensão `row_str` para fornecer uma string somente quando o item não for zero e um caractere asterisco caso contrário.

# --hints--

A list comprehension atribuída à variável `row_str` deve chamar `str()` em cada item `i` em `row` se `i` for truthy e deve avaliar para `'*'` caso contrário.
