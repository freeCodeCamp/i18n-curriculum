...
接下来，你将构建 `userAvatarUrl`。

首先创建一个名为 `userAvatarUrl` 的常量。 然后使用三元运算符检查 `avatar` 是否以 `"/user_avatar/"` 开头。 

如果是这样，使用 `concat` 方法将 `avatar` 连接到 `avatarUrl`。 否则返回 `avatar`。

这将确保头像 URL 无论是相对 URL 还是绝对 URL 都能正确形成。

# --hints--

你应该有一个名为 `userAvatarUrl` 的常量。 

```js
assert(code.match(/const\s+userAvatarUrl\s*=/));
```

你的 `ternary` 应该检查 `avatar` 是否以 `"/user_avatar/"` 开头，并返回 `avatarUrl.concat(avatar)` 或者 `avatar`。
