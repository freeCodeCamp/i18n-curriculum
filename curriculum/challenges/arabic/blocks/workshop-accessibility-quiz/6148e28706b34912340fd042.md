...
وسّط كل النص أفقيًا داخل عنصر `address`، وأضف بعض الحشو.

# --hints--

يجب استخدام محدِّد عنصر `address`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

يجب إعطاء `address` خاصية `text-align` بقيمة `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

يجب إعطاء `address` خاصية `padding-top` بقيمة لا تقل عن `1px`.
...
يجب إعطاء `address` خاصية `padding-right` بقيمة لا تقل عن `1px`.
...
يجب إعطاء `address` خاصية `padding-bottom` بقيمة لا تقل عن `1px`.
...
يجب إعطاء `address` خاصية `padding-left` بقيمة لا تقل عن `1px`.
