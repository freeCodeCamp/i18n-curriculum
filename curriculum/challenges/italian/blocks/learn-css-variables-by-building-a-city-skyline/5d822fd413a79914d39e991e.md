...
Dovresti dare agli elementi `.fb2-window` una proprietà `width` del `22%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

Dovresti dare agli elementi `.fb2-window` una proprietà `height` del `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

Dovresti dare agli elementi `.fb2-window` una proprietà `background-color` con il valore `--window-color3`.
