...
`regex`에 전달할 `infixEval` 함수는 두 숫자 사이에 연산자가 있는 패턴과 일치합니다. 첫 번째 숫자는 콜백에서 `arg1`에 할당되고, 두 번째 숫자는 `arg2`에, 연산자는 `operator`에 할당됩니다.

콜백 함수가 `operator` 객체의 `infixToFunction` 속성을 암묵적으로 반환하게 하세요. `operator`은 실제 속성명이 아니라 속성명 문자열을 담고 있는 변수임을 기억하세요.

# --hints--

콜백은 암묵적 반환을 사용해야 합니다.
...
콜백 함수는 `infixToFunction` 객체에 접근해야 합니다.
...
콜백 함수는 `infixToFunction` 객체의 `operator` 매개변수 값과 일치하는 속성에 대괄호 표기법으로 접근해야 합니다.
