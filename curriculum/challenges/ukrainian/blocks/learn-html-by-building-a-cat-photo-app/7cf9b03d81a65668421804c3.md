...
Елемент `input` повинен мати атрибут `name` зі значенням `catphotourl`.  Ви або не написали значення, або маєте друкарську помилку. Пам’ятайте, що значення атрибутів мають бути в лапках.
