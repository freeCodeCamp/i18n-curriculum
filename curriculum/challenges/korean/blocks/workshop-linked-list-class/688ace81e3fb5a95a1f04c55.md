...
다음으로, `else` 블록을 만들어 `current_node.next`에 `self.head`를 할당하세요.

이것은 제거할 요소가 헤드 노드에 있을 때를 처리합니다. `self.head`를 `current_node.next`를 가리키도록 업데이트하면 헤드 노드가 연결 리스트에서 제거됩니다.

# --hints--

`else` 블록을 만들어 `current_node.next`에 `self.head`를 할당해야 합니다.
