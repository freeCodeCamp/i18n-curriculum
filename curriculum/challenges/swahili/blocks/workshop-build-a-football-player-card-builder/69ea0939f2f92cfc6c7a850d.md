...
Ifuatayo, ongeza wito wa `useEffect` unaotekelezwa kila wakati hali ya `player` inapotofautiana. Ndani yake, tumia kitendakazi cha mshale na ongeza `try..catch` yenye `error` kama kigezo cha kipande cha `catch`.

# --hints--

Unapaswa kuita `useEffect` ndani ya `FootballPlayerCard`.
...
`useEffect` yako inapaswa kuwa na safu ya utegemezi `[player]`.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

Wito wa kurudisha wa `useEffect` unapaswa kuwa na kipande cha `try...catch` chenye `error` kama kigezo cha kipande cha `catch`.
