...
استبدل الهامش العلوي (top margin) للعناصر `h2` بقيمة `60px` من top padding.

# --hints--

يجب عليك إعطاء `h2` خاصية `margin-top` بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h2')?.marginTop, '0px');
```

يجب عليك إعطاء `h2` خاصية `padding-top` بقيمة `60px`.
