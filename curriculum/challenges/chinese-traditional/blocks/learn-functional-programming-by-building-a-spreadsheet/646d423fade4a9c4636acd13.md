...
首先你需要控制代碼較高優先的運算子。宣告一個 `noHigh` 變數，並將呼叫 `highPrecedence()` 並以 `str` 作為引數的結果指定給它。
