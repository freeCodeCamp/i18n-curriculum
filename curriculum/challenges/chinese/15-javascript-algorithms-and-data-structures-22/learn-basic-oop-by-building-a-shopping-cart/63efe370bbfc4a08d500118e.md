...
在 `forEach` 回调中， 将右侧赋值语句 `totalCountPerProduct[dessert.id] || 0` 放在括号 `()` 中来确保正确计算，然后将值加 1。
...
你应该使用方括号来访问与 `dessert.id` 相对应的 `totalCountPerProduct` 属性。
...
你应该使用赋值运算符来更新与 `dessert.id` 相对应的 `totalCountPerProduct` 属性的值。
...
你应该在加 1 之前将 `totalCountPerProduct[dessert.id] || 0` 放到括号 `()` 中。
