...
Il selettore `.hero-subtitle` dovrebbe avere una proprietà `font-size` impostata a `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

Il selettore `.hero-subtitle` dovrebbe avere una proprietà `color` impostata su `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

Il selettore `.hero-subtitle` dovrebbe avere una proprietà `text-align` impostata su `center`.
