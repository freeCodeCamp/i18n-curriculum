...
使用箭頭語法創建 `deleteTask` 函式（程式）。將 `buttonEl` 作為參數傳遞，並為函式（程式）主體定義一組空的大括號。

# --hints--

你應該使用 `const` 和箭頭語法來創建 `deleteTask` 函式（程式）。

```js
assert.match(code, /const\s+deleteTask\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/);
```

你的 `deleteTask` 函式應該接受一個 `buttonEl` 參數。

```js
assert.match(deleteTask.toString(), /\(\s*buttonEl\s*\)/);
```

你的 `deleteTask` 函式（程式）應該是空的。
