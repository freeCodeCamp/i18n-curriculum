...
سيؤدي هذا إلى دمج مساحة النص السفلية في عمود واحد على الشاشات الصغيرة.
...
يجب أن يحتوي استعلام `@media` الجديد على محدد `.text-with-images`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText, '.text-with-images');
```

يجب أن يحتوي محدد `.text-with-images` الجديد على خاصية `grid-template-columns` بقيمة `1fr`.
