...
Dovresti usare un selettore di classe per fare riferimento alla classe `sleeve`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.sleeve'));
```

La regola CSS `.sleeve` dovrebbe avere una proprietà `width` con il valore `110px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.sleeve')?.width === '110px');
```

La regola CSS `.sleeve` dovrebbe avere una proprietà `height` con il valore `25px`.
