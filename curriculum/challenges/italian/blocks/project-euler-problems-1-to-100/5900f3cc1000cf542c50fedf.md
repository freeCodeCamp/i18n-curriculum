...
Su Doku (significato giapponese *number place*) è il nome dato a un popolare concetto di puzzle. La sua origine non è chiara, ma il merito va attribuito a Leonhard Euler che inventò un puzzle simile, e molto più difficile, chiamato quadrati latini. L’obiettivo dei puzzle Su Doku, però, è sostituire gli spazi vuoti (o zeri) in una griglia 9 per 9 in modo che ogni riga, colonna e riquadro 3 per 3 contenga ciascuna delle cifre da 1 a 9. Qui sotto c’è un esempio di una griglia di partenza tipica e della sua griglia soluzione.
...
Un puzzle Su Doku ben costruito ha una soluzione unica e può essere risolto con la logica, anche se potrebbe essere necessario impiegare metodi di "tentativo ed errore" per eliminare opzioni (su questo c’è molta opinione contrastante). La complessità della ricerca determina la difficoltà del puzzle; l’esempio sopra è considerato facile perché può essere risolto con una deduzione diretta semplice.

L’array `puzzlesArr` contiene diverse stringhe di puzzle Su Doku di difficoltà variabile, ma tutte con soluzioni uniche.

Risolvendo tutti i puzzle in `puzzlesArr`, trova la somma dei numeri a 3 cifre che si trovano nell’angolo in alto a sinistra di ogni griglia soluzione; per esempio, 483 è il numero a 3 cifre che si trova nell’angolo in alto a sinistra della griglia soluzione sopra.
