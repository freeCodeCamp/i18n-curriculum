...
到目前为止，你已经有了三个函数：`getElement`、`hideElements` 和 `showElements`。
现在你需要一个函数来帮你获取一个随机的签文。

声明一个名为 `getRandomItem` 的函数，它接受一个 `items` 数组作为参数。这个函数要使用泛型。

在函数内部，使用 `Math.random()` 和 `Math.floor()` 从 `items` 数组中选择一个随机的签文。
别忘了返回选中的签文。

# --hints--

你应该声明一个名为 `getRandomItem` 的函数。
...
你的 `getRandomItem` 函数应该使用泛型类型参数 `T`。
...
你的 `getRandomItem` 函数应该有一个名为 `items` 的参数，类型为 `T[]`。
...
你的 `getRandomItem` 函数应该有一个返回类型 `T`。
...
你应该在 `index` 函数内部声明一个名为 `getRandomItem` 的变量。
...
`index` 变量应该使用 `Math.floor()` 和 `Math.random()` 以及 `items.length` 来生成一个随机索引。
...
你的 `getRandomItem` 函数应该返回 `items[index]`。
