...
다음으로 `JavaScript` 작업을 시작할 것입니다. 이를 위해 `script.js` 파일을 HTML에 연결하세요.

`script` 요소를 만들고 그 `src` 속성이 `script.js` 파일을 가리키도록 하세요. 예를 들어:
...
`script` 요소는 `src` 속성이 `script.js`로 설정되어야 합니다.
