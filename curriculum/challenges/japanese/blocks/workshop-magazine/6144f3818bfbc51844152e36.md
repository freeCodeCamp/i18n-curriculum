...
`minmax` 関数を使って、どのデバイスでもカラムをレスポンシブにしてください。`minmax` 関数は2つの引数を取り、最初が最小値、次が最大値です。これらの値は長さ、パーセンテージ、`fr`、あるいは `max-content` のようなキーワードでもかまいません。

すでに定義している `grid-template-columns` プロパティの各値を `minmax` 関数でラップし、それぞれの値を第2引数として使ってください。第1引数はそれぞれ `2rem`、`min-content`、`2rem` にしてください。

# --hints--

`main` セレクターには `grid-template-columns` プロパティを `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)` に設定してください。
