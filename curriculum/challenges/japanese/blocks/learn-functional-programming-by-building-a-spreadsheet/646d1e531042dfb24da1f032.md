...
`.replace()` メソッドの2番目の引数は文字列である必要はありません。代わりに、マッチした文字列に対してより複雑な処理を行うコールバック関数を渡すことができます。

コールバック関数はいくつかのパラメータを受け取ります。最初のパラメータはマッチした文字列です。空のコールバック関数を `.replace()` の呼び出しに渡し、`match` パラメータを与えてください。

# --hints--

`.replace()` メソッドの2番目の引数にはアロー関数を渡すべきです。
