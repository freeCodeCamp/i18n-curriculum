...
子孫セレクターを使って、`div` 要素の中にある3つの `.cat-whiskers-left` 要素を指定してください。`width` の `40px`、`height` の `1px`、`background-color` の `#000` をそれぞれ設定してください。

# --hints--

`.cat-whiskers-left div` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div'))
```

`.cat-whiskers-left div` セレクターには `width` に設定された `40px` があるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.width === '40px')
```

`.cat-whiskers-left div` セレクターには `height` に設定された `1px` があるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.height === '1px')
```

`.cat-whiskers-left div` セレクターには `background-color` に設定された `#000` があるはずです。
