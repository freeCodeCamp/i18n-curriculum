...
Zurück zum Stylen der Seite. Wählen Sie die Listenelemente innerhalb der Navigationsleiste aus und geben Sie ihnen die folgenden Stile:
...
Sie sollten entweder den `nav li` oder `nav > ul > li` Selektor verwenden.
...
Sie sollten den `li` Elementen eine `color` von `#dfdfe2` geben.
...
Sie sollten den `li` Elementen eine `margin` von `0 0.2rem` geben.
...
Sie sollten den `li` Elementen eine `padding` von `0.2rem` geben.
...
Sie sollten den `li` Elementen eine `display` von `block` geben.
