...
Crea un loop `while` vuoto che continua finché ci sono partecipanti in attesa (`currentTickQueue > 0`) e il cancello ha ancora capacità disponibile (`processed < gate.capacity`).

# --hints--

La condizione del tuo loop `while` dovrebbe essere `currentTickQueue > 0` e `processed < gate.capacity`.
