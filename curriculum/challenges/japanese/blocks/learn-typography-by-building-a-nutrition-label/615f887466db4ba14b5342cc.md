...
最後の区切りの後に、テキスト `p` を持つ別の `Protein 3g` 要素を作成してください。下部のボーダーを削除するために必要なクラスを使い、`span` を太字にするために `Protein` を使います。次に、新しい `Protein 3g` 要素を含むテキスト `span` を新しい `span` 要素で囲んでください。

この要素の後に、大きな区切りを作成してください。
...
新しい `div` には `class` 属性を `divider large` に設定してください。
