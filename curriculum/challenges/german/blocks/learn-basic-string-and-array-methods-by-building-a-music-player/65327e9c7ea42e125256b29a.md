...
Wir haben die restlichen Songs für Sie dem `allSongs` Array hinzugefügt.

Als Nächstes lernen Sie die <dfn>Web Audio API</dfn> kennen und wie Sie sie verwenden, um Songs abzuspielen. Alle modernen Browser unterstützen die Web Audio API, mit der Sie Audio in Webanwendungen erzeugen und verarbeiten können.

Verwenden Sie `const`, um eine Variable namens `audio` zu erstellen und ihr den Wert `new Audio()` zuzuweisen. Dadurch wird ein neues HTML5 `audio` Element erzeugt.

# --hints--

Sie sollten das Schlüsselwort `new` verwenden, um eine Instanz des `Audio` Objekts zu erstellen.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

Sie sollten das `Audio` Objekt einer Konstanten namens `audio` zuweisen.
