...
如果滿足此條件，則通過將其 `disabled` 屬性設置爲 `true` 來禁用該按鈕。 另外，將按鈕的 `textContent` 設置爲 `"No more data to load"`。
...
你應該將 `loadMoreBtn` 的 `disabled` 屬性設置爲 `true`。

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

你應該將 `loadMoreBtn` 的 `textContent` 設置爲 `"No more data to load"`。
