...
Il selettore `.quote` dovrebbe avere una proprietà `color` impostata su `#00beef`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.color === 'rgb(0, 190, 239)');
```

Il selettore `.quote` dovrebbe avere una proprietà `font-size` impostata a `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize === '2.4rem');
```

Il selettore `.quote` dovrebbe avere una proprietà `text-align` impostata su `center`.
