...
All'interno della funzione `getElement`, usa `querySelector` con i generici e archivia il risultato in una variabile. Aggiungi un'istruzione `if` per verificare se l'elemento non esiste e, in tal caso, lancia un errore con il messaggio `Element not found: ${error message here}`. Se l'elemento esiste, restituiscilo.

# --hints--

Dovresti dichiarare una variabile chiamata `el` all'interno della funzione `getElement`.
...
La tua variabile `el` dovrebbe essere assegnata al valore `document.querySelector<T>(selector)`.
...
Dovresti avere un'istruzione if che verifica se `el` non è stato trovato.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

Dovresti lanciare un nuovo Error con il messaggio `Element not found: ${selector}` all'interno della tua istruzione `if`.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

Dovresti restituire la variabile `el` all'interno della funzione getElement.
