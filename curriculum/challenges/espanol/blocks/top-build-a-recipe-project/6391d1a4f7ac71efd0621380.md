...
Debes tener un elemento `title` dentro del elemento `head` que contenga el texto `The Odin Recipes`.

```js
assert(document.querySelectorAll('HEAD > TITLE')[0]?.innerText == 'The Odin Recipes');
```

Debe tener un elemento `h1<code> dentro de su elemento <code>body` que contenga el siguiente texto `Creamy Chocolate Fudge`.
...
Debe tener un elemento `ul` anidado con elementos `li` que contengan los ingredientes.
...
Debe tener un elemento `ol` que contenga elementos `li` con los pasos de la receta.
...
Debe tener un elemento `ul` anidado con elementos `li` que contengan elementos `a` que enlacen a otras recetas.
...
Los elementos de anclaje que enlacen a las recetas deben de tener un atributo `href` con el valor `#`.
