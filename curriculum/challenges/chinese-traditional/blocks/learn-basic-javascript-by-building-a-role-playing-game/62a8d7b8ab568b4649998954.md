...
使用 `else` 述語來執行當 `inventory` 長度不超過 1。將 `text.innerText` 設定為 `"Don't sell your only weapon!"`。

# --hints--

你的 `sellWeapon` 函式應該有一個 `else` 述語。

```js
assert.match(sellWeapon.toString(), /else/);
```

你應該將 `text.innerText` 設定為 `"Don't sell your only weapon!"`。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")Don't sell your only weapon!\1/);
```

你的新 `text.innerText` 行應該在你的 `else` 述語中。
