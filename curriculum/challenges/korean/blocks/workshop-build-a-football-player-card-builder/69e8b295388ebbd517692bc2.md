...
JSX와 TSX에서는 중괄호를 사용해 JavaScript 표현식을 속성 값으로 전달합니다. 템플릿 리터럴은 백틱과 `${}`를 사용해 보간합니다. 예를 들어:

```tsx
<div className={`prefix-${someVariable}`}>
```

카드 래퍼 `className`는 현재 `card-wrapper tier-gold`로 하드코딩되어 있습니다. `gold`을 템플릿 리터럴을 사용해 `getPlayerTier`를 `player.overallRating`와 함께 호출하는 동적 값으로 바꾸세요. 이렇게 하면 플레이어의 전체 등급에 따라 CSS에서 올바른 티어 클래스를 참조합니다.

# --hints--

`getPlayerTier(player.overallRating)`의 `className`에 `.card-wrapper`를 사용한 템플릿 리터럴을 사용해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

플레이어 등급이 98일 때 `.card-wrapper` 요소는 `tier-elite` 클래스를 가져야 합니다.
