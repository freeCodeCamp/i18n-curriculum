...
Die Skyline nimmt Gestalt an. Füllen Sie die `background-color` Eigenschaft der Vordergrundgebäude aus. Verwenden Sie Ihre `--building-color1` Variable, um `.fb3` und `.fb4`, `--building-color2` für `.fb5`, `--building-color3` für `.fb2` und `.fb6` sowie `--building-color4` für `.fb1` auszufüllen.

# --hints--

Sie sollten `.fb1` eine `background-color` mit `--building-color4` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb1')?.getPropVal('background-color', true), 'var(--building-color4)');
```

Sie sollten `.fb2` eine `background-color` mit `--building-color3` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb2')?.getPropVal('background-color', true), 'var(--building-color3)');
```

Sie sollten `.fb3` eine `background-color` mit `--building-color1` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb3')?.getPropVal('background-color', true), 'var(--building-color1)');
```

Sie sollten `.fb4` eine `background-color` mit `--building-color1` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.getPropVal('background-color', true), 'var(--building-color1)');
```

Sie sollten `.fb5` eine `background-color` mit `--building-color2` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb5')?.getPropVal('background-color', true), 'var(--building-color2)');
```

Sie sollten `.fb6` eine `background-color` mit `--building-color3` geben.
