...
Obtén el elemento `select` con ID `species` del DOM y guárdalo en una constante llamada `selectElement`. Usa el operador de afirmación no nulo `!` para asegurar que el elemento no es `null`

# --hints--

Deberías tener una constante llamada `selectElement`.
...
Tu `selectElement` debe consultar el elemento con el ID `species` usando `document.querySelector` con un argumento de tipo `HTMLSelectElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

Tu `selectElement` debe usar el operador de afirmación no nulo (`!`).
