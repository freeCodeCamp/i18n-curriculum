...
Em seguida, você precisa verificar se a playlist está vazia.Se for, você deve redefinir o objeto `userData` para seu estado original.

Use uma declaração `if` para verificar se `userData?.songs` tem um comprimento de `0`.

# --hints--

Você deve criar uma declaração `if` vazia com a condição `userData?.songs.length === 0`.
