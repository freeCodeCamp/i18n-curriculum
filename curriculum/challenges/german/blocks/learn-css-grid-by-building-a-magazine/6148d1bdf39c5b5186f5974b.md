...
Erstelle einen `hr`-Selektor und gib ihm eine `margin`-Eigenschaft mit dem Wert `1.5rem 0`.

# --hints--

Du solltest einen `hr`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('hr'));
```

Dein `hr`-Selektor sollte eine `margin`-Eigenschaft mit dem Wert `1.5rem 0` haben.
