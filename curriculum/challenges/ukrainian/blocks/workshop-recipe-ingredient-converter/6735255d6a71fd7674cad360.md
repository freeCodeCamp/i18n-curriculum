...
Нарешті, надайте своєму `conversionTable` об’єкт `teaspoon` з властивістю `cup`, встановленою в `1/48`, властивістю `gram`, встановленою в `5`, і властивістю `ounce`, встановленою в `0.167`.

# --hints--

Ваш `conversionTable` має мати властивість `teaspoon`.

```js
assert.property(conversionTable, "teaspoon");
```

Ваша властивість `teaspoon` має бути об’єктом.

```js
assert.isObject(conversionTable.teaspoon);
```

Ваш об’єкт `teaspoon` має мати властивість `cup`.

```js
assert.property(conversionTable.teaspoon, "cup");
```

Ваша властивість `cup` має бути встановлена в `1/48`.

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

Ваш об’єкт `teaspoon` має мати властивість `gram`.

```js
assert.property(conversionTable.teaspoon, "gram");
```

Ваша властивість `gram` має бути встановлена в `5`.

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

Ваш об’єкт `teaspoon` має мати властивість `ounce`.

```js
assert.property(conversionTable.teaspoon, "ounce");
```

Ваша властивість `ounce` має бути встановлена в `0.167`.
