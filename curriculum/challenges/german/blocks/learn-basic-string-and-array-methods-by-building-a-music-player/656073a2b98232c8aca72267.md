...
Um die Informationen des aktuellen Songs zu verwalten und seine Wiedergabezeit zu verfolgen, erstellen Sie eine `currentSong`- und `songCurrentTime`-Eigenschaft. Setzen Sie die Werte jeweils auf `null` und `0`.

# --hints--

Sie sollten das bestehende `userData`-Objekt und dessen Inhalt nicht verändern.

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

Ihr `userData`-Objekt sollte einen `currentSong`-Schlüssel mit dem Wert `null` haben.

```js
assert.propertyVal(userData, "currentSong", null)
```

Ihr `userData`-Objekt sollte einen `songCurrentTime`-Schlüssel mit dem Wert `0` haben.
