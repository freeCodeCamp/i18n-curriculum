...
ID가 `select`인 `species` 요소를 DOM에서 가져와 `selectElement`라는 상수에 저장하세요. 요소가 `!`가 아님을 어서션하기 위해 `null` 비널 어서션 연산자를 사용하세요.

# --hints--

`selectElement`라는 상수가 있어야 합니다.
...
`selectElement`는 `species` 타입 인수를 사용하여 `document.querySelector`로 ID가 `HTMLSelectElement`인 요소를 쿼리해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

`selectElement`는 비널 어서션 연산자(`!`)를 사용해야 합니다.
