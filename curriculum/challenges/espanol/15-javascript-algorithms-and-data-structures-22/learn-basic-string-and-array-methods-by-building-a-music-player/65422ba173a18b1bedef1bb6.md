...
Dentro de la función flecha del escuchador de eventos, agrega un `if` para verificar si `userData?.currentSong` es `null`.

Dentro del bloque `if`, llama a la función `playSong()` con el `id` de la primera canción en el array `userData?.songs`. Esto asegurará que la primera canción de la lista de reproducción se reproduzca primero.

# --hints--

Deberías crear una sentencia `if` con la condición `userData?.currentSong === null`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{/);
```

Deberías llamar a la función `playSong` con `userData?.songs[0].id` dentro de tu bloque `if`.
