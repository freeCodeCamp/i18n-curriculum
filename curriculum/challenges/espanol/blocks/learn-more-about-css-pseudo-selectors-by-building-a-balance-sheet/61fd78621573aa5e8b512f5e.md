...
En tu tercer elemento `tr`, agrega un elemento `th` con el texto `Savings Funds set aside for emergencies.`. Envuelve ese texto, excepto `Savings `, en un elemento `span` con el atributo `class` establecido en `description`.

A continuación, agregue tres elementos `td` con el siguiente texto (en orden): `$500`, `$650`, `$728`. Asigne al tercer elemento `td` el atributo `class` establecido en `current`.
