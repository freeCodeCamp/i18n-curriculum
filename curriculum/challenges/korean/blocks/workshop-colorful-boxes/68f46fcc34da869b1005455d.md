---
id: 68f46fcc34da869b1005455d
title: 37단계
challengeType: 0
dashedName: step-37
---

# --description--

이제 박스 스타일링을 계속하려면 새로운 `.box3` 선택자를 만들고 `background` 속성을 값 `#ffd166`로 추가하세요.

그다음, 새 선택자에 `order` 속성을 값 `2`로 추가하세요. 그러면 이 박스가 `.box1` 요소 뒤로 이동하는 것을 볼 수 있습니다.

또한 `flex-shrink` 속성을 값 `3`로 추가하세요. 이 속성은 이 박스가 줄에 공간이 부족할 때 다른 박스보다 세 배 더 많이 줄어든다는 뜻입니다.

# --hints--

`.box3` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.box3'));
```

`.box3` 선택자에 `background` 속성을 값 `#ffd166`로 추가해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.box3')?.getPropVal('background'), 'rgb(255, 209, 102)');
```

`.box3` 선택자에 `order` 속성을 값 `2`로 추가해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.box3')?.getPropVal('order'), '2');
```

`.box3` 선택자에 `flex-shrink` 속성을 값 `3`로 추가해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.box3')?.getPropVal('flex-shrink'), '3');
```


# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8" />
	<meta name="viewport" content="width=device-width, initial-scale=1.0" />
	<title>Colored Boxes</title>
	<link rel="stylesheet" href="styles.css" />
</head>
<body>
	<header>
		<h1>Colored Boxes Layout</h1>
	</header>
	<div class="flex-container">
		<div class="box box1">
			<h2>Box 1</h2>
			<p>Red</p>
		</div>
		<div class="box box2">
			<h2>Box 2</h2>
			<p>Orange</p>
		</div>
		<div class="box box3">
			<h2>Box 3</h2>
			<p>Yellow</p>
		</div>
		<div class="box">
			<h2>Box 4</h2>
			<p>Green</p>
		</div>
		<div class="box">
			<h2>Box 5</h2>
			<p>Blue</p>
		</div>
		<div class="box">
			<h2>Box 6</h2>
			<p>Indigo</p>
		</div>
	</div>
</body>
</html>
```

```css
h1 {
  text-align: center;
  margin-bottom: 10px;
}

.box {
  max-height: 120px;
  color: #000;
  border: 1px solid #000;
  display: flex;
  flex: 0 0 150px;
  flex-direction: column;
  align-items: center;
  margin: 10px;
  font-weight: bold;
  font-size: 1.125rem;
  border-radius: 5px;
  order: 0; 
}

.flex-container {
  display: flex;
  flex-wrap: wrap;
  width: 70%;
  height: 600px;
  padding: 10px;
  margin: 20px auto;
  align-content: space-evenly;
}

.box1 {
  background: #f16e79;
  order: 1; 
  flex-grow: 1;
}

.box2 {
  background: #f4a261;
  order: 0; 
}

--fcc-editable-region--

--fcc-editable-region--
```
