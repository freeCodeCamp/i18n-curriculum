...
لقد أضفت سابقًا استدعاء `console.log()` إلى دالتك `addCrewMember` للتحقق من سلوكها، والآن لم يعد هذا الاستدعاء ضروريًا. لمنع تكدس وحدة التحكم في الخطوات التالية، قم بحذف استدعاء `console.log()` الحالي.

# --hints--

احذف استدعاء `console.log()` من دالتك `addCrewMember`. يجب حذف السطر بأكمله.
