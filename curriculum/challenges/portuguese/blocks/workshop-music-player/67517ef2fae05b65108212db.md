...
Você precisa obter o `id` da música que está tocando no momento. Para isso, você pode usar `userData.currentSong?.id`.

Use `getElementById()` para selecionar a música que está tocando no momento: passe um template literal para prefixar o `id` com `song-`. Atribua a uma variável chamada `songToHighlight`.

# --hints--

Você não deve modificar a função `highlightCurrentSong` existente e seu conteúdo.
...
Você deve usar `document.getElementById()` e passar ``song-${userData.currentSong?.id}``.

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Você deve atribuir seu `getElementById()` a uma variável `songToHighlight`.
