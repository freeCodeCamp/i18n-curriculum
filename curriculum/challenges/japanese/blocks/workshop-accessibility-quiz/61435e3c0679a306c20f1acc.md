...
ページ内を移動できるように、各アンカー要素に`href`を、`id`要素の`h2`に対応するものを付けてください。

# --hints--

最初の`a`要素には、`href`の`#student-info`を付ける必要があります。

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

2番目の`a`要素には、`href`の`#html-questions`を付ける必要があります。

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

3番目の`a`要素には、`href`の`#css-questions`を付ける必要があります。
