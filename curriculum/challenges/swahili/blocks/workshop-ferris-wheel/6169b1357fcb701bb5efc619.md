...
Tengeneza kichaguzi kipya cha `25%` kati ya vichaguzi vyako vya `0%` na `50%`. Toa sifa ya `background-color` kwa kichaguzi hiki kipya na uiweke kuwa `yellow`.

# --hints--

Unapaswa kutengeneza kichaguzi kipya cha `25%` katika sheria yako ya `@keyframes cabins`.
...
Kichaguzi chako cha `25%` kinapaswa kuwa kati ya vichaguzi vyako vya `0%` na `50%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText , '25%');
```

Kichaguzi chako cha `25%` kinapaswa kuwa na sifa ya `background-color` iliyowekwa kuwa `yellow`.
