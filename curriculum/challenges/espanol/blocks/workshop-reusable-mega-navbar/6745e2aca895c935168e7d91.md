...
Para terminar, asigna el texto `Calendar` a la primera etiqueta de ancla, `Chat` a la segunda y `Email` a la tercera.

¡Con eso, el componente mega barra de navegación está completo!

# --hints--

Debe establecer un texto de `Calendar` para el primer elemento ancla.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Debe establecer un texto de `Chat` para el segundo elemento ancla.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Debe establecer un texto de `Email` para el tercer elemento ancla.
