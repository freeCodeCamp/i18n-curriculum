...
Fügen Sie innerhalb der `playPreviousSong`-Funktion eine `if`-Anweisung mit der Bedingung `userData?.currentSong === null` hinzu. Damit wird überprüft, ob gerade kein Lied abgespielt wird. Falls keines abgespielt wird, verlassen Sie die Funktion mit einem `return`.

Erstellen Sie innerhalb des `else`-Blocks eine Konstante namens `currentSongIndex` und weisen Sie ihr `getCurrentSongIndex()` zu.

# --hints--

Sie sollten die bestehende `playPreviousSong`-Funktion und deren Inhalt nicht verändern.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Sie sollten eine `if`-Anweisung mit der Bedingung `userData?.currentSong === null` innerhalb Ihrer `playPreviousSong`-Funktion haben.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

Sie sollten `return` innerhalb des Blocks Ihrer `if`-Anweisung haben.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

Ihre `if`-Anweisung sollte einen `else`-Block haben.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

Sie sollten `getCurrentSongIndex` aufrufen und das Ergebnis `currentSongIndex` innerhalb des `else`-Blocks Ihrer `if`-Anweisung zuweisen.
