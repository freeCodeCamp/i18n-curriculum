...
如果你想添加更多社媒圖標，但將它們保持在同一行，則需要更新 `grid-template-columns` 以創建其他列。 作爲替代方案，你可以使用 `grid-auto-flow` 屬性。

此屬性將 `row` 或 `column` 作爲第一個值，第二個值 `dense` 爲可選值 。 `grid-auto-flow` 使用自動放置算法來調整網格佈局。 將其設置爲 `column`，將告訴算法根據需要爲內容創建新列。 `dense` 值允許算法回溯並用較小的項目填充網格中的空白，這可能導致項目出現亂序。

對於你的 `.social-icons` 選擇器，將 `grid-auto-flow` 屬性設置爲 `column`。

# --hints--

你的 `.social-icons` 選擇器應該將 `grid-auto-flow` 屬性設置爲 `column`。
