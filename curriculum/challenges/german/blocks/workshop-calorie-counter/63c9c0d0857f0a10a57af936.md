...
Nach dem gleichen Muster kannst du deine `number`-Eingaben in den `#dinner`-, `#snacks`- und `#exercise`-Elementen abfragen. Weise sie Variablen zu, die dem Namensschema der beiden vorherigen entsprechen.

# --hints--

Du solltest eine `dinnerNumberInputs`-Variable deklarieren.
...
Du solltest das Ergebnis deines `document.querySelectorAll()`-Aufrufs `dinnerNumberInputs` zuweisen.
...
Du solltest eine `snacksNumberInputs`-Variable deklarieren.
...
Du solltest das Ergebnis deines `document.querySelectorAll()`-Aufrufs `snacksNumberInputs` zuweisen.
...
Du solltest eine `exerciseNumberInputs`-Variable deklarieren.
...
Du solltest das Ergebnis deines `document.querySelectorAll()`-Aufrufs `exerciseNumberInputs` zuweisen.
