...
title: Minimum- und Maximalwert in einem binären Suchbaum finden
challengeType: 1
forumTopicId: 301642
...
In dieser Aufgabe definieren Sie zwei Methoden, `findMin` und `findMax`. Diese Methoden sollen den Minimum- bzw. Maximalwert zurückgeben, der im binären Suchbaum gespeichert ist (machen Sie sich vorerst keine Sorgen darum, Werte zum Baum hinzuzufügen, wir haben bereits einige im Hintergrund eingefügt). Falls Sie nicht weiterkommen, denken Sie an die Invariante, die für binäre Suchbäume gelten muss: Jeder linke Teilbaum ist kleiner oder gleich seinem Elternknoten, und jeder rechte Teilbaum ist größer oder gleich seinem Elternknoten. Außerdem nehmen wir an, dass unser Baum nur Ganzzahlen speichern kann. Wenn der Baum leer ist, soll jede der beiden Methoden `null` zurückgeben.
...
Die `findMin` Methode sollte den Minimumwert im binären Suchbaum zurückgeben.
...
Die `findMax` Methode sollte den Maximalwert im binären Suchbaum zurückgeben.
...
Die `findMin` und `findMax` Methoden sollten für einen leeren Baum `null` zurückgeben.
