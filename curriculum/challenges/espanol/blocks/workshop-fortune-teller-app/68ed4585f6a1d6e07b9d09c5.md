...
Dentro del método `fetchCardsData`, declara una variable llamada `response` y asígnale el resultado de llamar a la función `fetch` con la siguiente URL: `` `${CDN_URL}/card_data.json` ``.

Agrega la palabra clave `await` antes de la llamada a `fetch` y envuelve toda la lógica dentro de un bloque `try...catch`.

# --hints--

Debes declarar una variable `response` dentro del método `fetchCardsData`.
...
Tu variable `response` debe asignarse llamando a `await fetch` con la URL `` `${CDN_URL}/card_data.json` ``.
...
Tu llamada a `fetch` debe estar envuelta dentro de un bloque `try...catch`, con `error` como parámetro `catch`.
