...
الآن يمكن لدالتك `update()` تقييم الصيغ فعليًا. تذكر أنك كتبت شرط `if` للتحقق من استدعاء دالة.

داخل جملة `if`، عيّن `value` للـ `element` ليكون ناتج دالتك `evalFormula()`. لا تمرر أي معلمات بعد.

# --hints--

يجب تحديث خاصية `value` للـ `element` داخل كتلة `if` الخاصة بك.
...
يجب تعيين خاصية `value` إلى ناتج استدعاء دالتك `evalFormula()`.
...
يجب ألا تمرر أي معلمات إلى استدعاء `evalFormula()`.
