...
الآن، أنشئ دالة باسم `getNextSong`. اجعلها تُرجع الأغنية الموجودة عند `getCurrentSongIndex()` زائد واحد في `userData.songs`، أو `undefined` إذا كانت الأغنية الحالية هي الأخيرة في قائمة التشغيل.

# --hints--

يجب أن يكون لديك دالة باسم `getNextSong`.

```js
assert.isFunction(getNextSong);
```

يجب أن تستدعي دالتك `getNextSong` الدالة `getCurrentSongIndex`.

```js
assert.match(getNextSong.toString(), /getCurrentSongIndex\s*\(\s*\)/);
```

يجب أن تُرجع دالتك `getNextSong` الأغنية التي تلي الأغنية الحالية.
...
يجب أن تُرجع دالتك `getNextSong` القيمة `undefined` عندما تكون الأغنية الحالية هي الأخيرة في قائمة التشغيل.
