...
Ihre `calculateTaxes`-Methode sollte den Wert von `taxRate` (geteilt durch 100, um ihn in Prozent umzuwandeln) multipliziert mit dem `amount`-Parameter zurückgeben.

Zur Verdeutlichung umschließen Sie die `taxRate / 100`-Berechnung mit runden Klammern.

# --hints--

Sie sollten den `taxRate` in Ihrer `calculateTaxes`-Methode durch 100 teilen. Denken Sie daran, das Schlüsselwort `this` zu verwenden.
...
Sie sollten die `this.taxRate / 100`-Berechnung in runde Klammern setzen.
...
Sie sollten den `amount`-Parameter in Ihrer `taxRate`-Methode mit dem durch 100 geteilten `calculateTaxes` multiplizieren.
