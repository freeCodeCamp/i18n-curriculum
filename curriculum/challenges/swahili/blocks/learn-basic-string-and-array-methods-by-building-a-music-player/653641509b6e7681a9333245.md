...
Unahitaji kusahisha wimbo unaochezwa sasa pamoja na muonekano wa kipengele cha `playButton`.

Weka thamani ya `song` kwenye sifa ya `currentSong` katika kitu cha `userData`.

*Kumbuka*: Haufai kutumia kielezi cha uunganishaji wa mfululizo `?.` katika hatua hii kwa sababu `userData.currentSong` haitakuwa `null` au `undefined` kwa sasa.

# --hints--

Unapaswa kuweka thamani ya `song` kwa `userData.currentSong`.
