...
Il selettore `.text-with-images` dovrebbe avere una proprietà `grid-template-columns` impostata su `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

Il selettore `.text-with-images` dovrebbe avere una proprietà `column-gap` impostata su `3rem`.
