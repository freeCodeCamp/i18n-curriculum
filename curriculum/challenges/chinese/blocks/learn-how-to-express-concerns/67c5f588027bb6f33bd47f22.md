...
When you say something `works smoothly`, it means that something happens without difficulties or interruptions. `Smoothly` comes from `smooth`, which describes something without roughness or obstacles, but they are used in different contexts. 例如：
