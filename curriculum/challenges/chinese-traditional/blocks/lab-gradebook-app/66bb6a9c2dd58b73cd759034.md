...
1. 你應該有一個名為 `getGrade` 的函式（程式），它接受學生分數作為參數，並傳回一個字串，表述根據分數的等第。以下是分數與其對應的等第：
...
1. 你應該有一個名為 `studentMsg` 的函式（程式），它接受一個分數陣列和一個學生分數作為參數。該函式（程式）應該傳回具有以下格式的字串：

    - `"Class average: average-goes-here. Your grade: grade-goes-here. You passed the course."`，如果學生通過了課程。
    - `"Class average: average-goes-here. Your grade: grade-goes-here. You failed the course."`，如果學生未通過課程。

    將 `average-goes-here` 替換為總分數的平均值，並將 `grade-goes-here` 替換為學生的等第。
    使用 `getAverage` 取得平均分數，並使用 `getGrade` 取得學生的等第。
...
`studentMsg([92, 88, 12, 77, 57, 100, 67, 38, 97, 89], 37)` 應該傳回以下訊息：`"Class average: 71.7. Your grade: F. You failed the course."`。
...
`studentMsg([12, 22, 32, 42, 52, 62, 72, 92], 85)` 應該傳回以下訊息：`"Class average: 48.25. Your grade: B. You passed the course."`。
