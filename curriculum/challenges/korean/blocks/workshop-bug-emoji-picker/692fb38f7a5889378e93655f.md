...
다음으로, 제네릭 매개변수가 `Bee`로 설정된 `Bug` 클래스를 확장하는 `string`라는 이름의 서브클래스를 만드세요.

# --hints--

`Bee`라는 이름의 클래스가 있어야 합니다.
...
`Bee` 클래스는 `Bug` 타입 인수를 가진 `string`를 확장해야 합니다.
