...
創建一個帶有類 `playlist-song-info` 的 `button` 元素。 在 `button` 內部，添加一個帶有 `playlist-song-title` 類的 `span` 元素，然後插入 `song.title` 作爲文本。

# --hints--

你應該創建一個帶有類 `playlist-song-info` 的 `button`。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

你應該在 `button` 元素內創建一個 `span` 元素。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

你的 `span` 元素應具有 `playlist-song-title` 類。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

你應該將 `song.title` 插入爲 `span` 元素的內容。
