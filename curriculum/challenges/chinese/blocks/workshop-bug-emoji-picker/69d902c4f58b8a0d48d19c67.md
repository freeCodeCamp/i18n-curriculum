...
现在为 `bugMap` 添加两个条目。添加 `"bee"` 作为键，映射到一个新的 `Bee` 实例；添加 `"spider"` 作为键，映射到一个新的 `Spider` 实例。将 `bugEmojiElement` 作为参数传入这两个实例。

# --hints--

你的 `bugMap` 应该有一个 `"bee"` 条目，其值设置为 `new Bee(bugEmojiElement)`。

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

你的 `bugMap` 应该有一个 `"spider"` 条目，其值设置为 `new Spider(bugEmojiElement)`。
