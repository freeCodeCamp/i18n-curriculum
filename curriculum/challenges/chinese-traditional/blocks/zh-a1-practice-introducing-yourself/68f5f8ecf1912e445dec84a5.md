...
`我是 (wǒ shì)`, `數據分析師 (shù jù fēn xī shī)`, `加拿大人 (jiā ná dà rén)`, and `你們好 (nǐ men hǎo)`.
...
Chen Na first greets a group of people with `你們好 (nǐ men hǎo)`, then uses `我是 (wǒ shì)` to introduce her name. She also introduces her nationality as `加拿大人 (jiā ná dà rén)` and her profession as `數據分析師 (shù jù fēn xī shī)`.
