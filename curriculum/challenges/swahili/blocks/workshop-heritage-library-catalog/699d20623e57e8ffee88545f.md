...
Rekodi ya `byDecade` imetimiza kusudi lake. Ondoa `console.log(byDecade)` kutoka kwenye msimbo wako.

Kisha, ili kufanya maingizo ya katalogi yawe rahisi kusomwa, tengeneza kitendakazi kinachoitwa `renderEntry` kinachopokea `entry` kama kigezo. Acha mwili wa kitendakazi huo ukiwa tupu kwa sasa. Chini ya kitendakazi, andika rekodi ya `renderEntry(catalog[0])`.

# --hints--

`renderEntry` inapaswa kuwa kitendakazi.

```js
assert.isFunction(renderEntry);
```

`renderEntry` inapaswa kuwa kitendakazi chenye kigezo `entry`.
...
Wito wa `console.log(byDecade)` unapaswa kuondolewa.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*byDecade\s*\)/);
```

Unapaswa kuandika rekodi ya `renderEntry(catalog[0])`.
