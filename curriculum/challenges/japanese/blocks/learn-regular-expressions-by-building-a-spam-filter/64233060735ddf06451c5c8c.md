...
新しい正規表現を`denyList`配列に追加して、テストできるようにしてください。

# --hints--

`denyList`配列には`freeRegex`変数を含める必要があります。

```js
assert.deepInclude(denyList, freeRegex);
```

`denyList`配列には`dollarRegex`変数を含める必要があります。

```js
assert.deepInclude(denyList, dollarRegex);
```

`denyList`配列には`helpRegex`変数を含める必要があります。
