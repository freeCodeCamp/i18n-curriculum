...
A busca binária é um algoritmo de eficiência **O(log(n))** para pesquisar um array ordenado para encontrar um elemento. Ele opera usando as seguintes etapas:

1. Encontre o `value` do meio de um array ordenado. Se `value == target` retorne `true` (O valor foi encontrado e a busca está completa).
1. Se o valor do meio `value < target`, pesquise na metade direita do array na próxima comparação.
1. Se o valor do meio `value > target`, pesquise a metade esquerda do array na próxima comparação.
...
**Nota:** O seguinte array será usado nos testes:
