...
الآن مُرّ على مصفوفة `POSITIONS` باستخدام دالة سهمية تتلقّى كل سلسلة موضع كـ `pos` وتُعيد عنصر `option`. وينبغي أن يحمل العنصر option قيمة `key` تساوي `pos` والقيمة أيضًا `pos`. كما اعرض `pos` كنص لكل عنصر `option` مُولَّد.

يمكنك استخدام أي اسم كمعامل في دالة map، لا يجب أن يكون `pos` بالضرورة.

# --hints--

يجب أن يحتوي عنصر `select` الخاص بالموقع على عناصر `option` تم إنشاؤها من مصفوفة `POSITIONS`.
...
يجب أن تتطابق قيمة `value` لكل خيار مع النص المعروض وأن تكون موقعًا صالحًا.
