...
Erstellen Sie eine neue CSS-Regel mit dem `.two`-Selektor und setzen Sie dessen `width` auf 475 Pixel.

# --hints--

Sie sollten einen `.two`-Selektor haben.
...
Sie sollten die `width`-Eigenschaft auf `475px` setzen.
...
Ihr `.two`-Element sollte einen `width`-Wert von `475px` haben.
