...
تهانينا! لقد اكتملت مجلتك الآن.

# --hints--

يجب أن يكون لديك `@media` جديد لـ `only screen and (max-width: 420px)`. يجب أن يكون هذا آخر query في قائمة `@media` للـ query.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

يجب أن يحتوي query الـ `@media` الجديد على منتقى `.hero-title`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText === '.hero-title');
```

يجب أن يكون لمنتقي `.hero-title` الخاص بك خاصية `font-size` بقيمة `4.5rem`.
