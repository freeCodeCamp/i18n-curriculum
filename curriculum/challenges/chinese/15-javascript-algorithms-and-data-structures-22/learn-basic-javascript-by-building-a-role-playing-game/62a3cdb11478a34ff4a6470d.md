...
现在创建一个 `fightDragon` 函数，将 `"Fighting dragon."` 打印到控制台。

# --hints--

你应该使用 `function` 关键字来声明 `fightDragon`。
...
你的代码中应该有一个 `console.log("Fighting dragon.");` 行。

```js
assert.match(code, /console\.log\(\s*('|")Fighting dragon\.\1\s*\)/);
```

你的 `console.log("Fighting dragon.");` 行应该位于你的 `fightDragon` 函数内。
