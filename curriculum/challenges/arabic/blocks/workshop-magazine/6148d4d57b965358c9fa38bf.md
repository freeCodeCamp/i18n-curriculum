...
حان الوقت الآن لتنسيق الـ `section` الثالث. لاحظ أنه يحتوي على قيم `text` و `text-with-images` لخاصية الـ `class`، مما يعني أنه يرث الأنماط من قاعدة `.text` الخاصة بك.

أنشئ محدِّد `.text-with-images` واضبط خاصية `display` على `grid`.

# --hints--

يجب أن يكون لديك محدِّد `.text-with-images`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

يجب أن يحتوي محدِّد `.text-with-images` على خاصية `display` مضبوطة على `grid`.
