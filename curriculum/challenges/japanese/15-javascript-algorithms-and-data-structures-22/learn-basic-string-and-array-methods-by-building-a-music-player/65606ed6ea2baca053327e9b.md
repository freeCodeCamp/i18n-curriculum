...
次のプロパティと値を使って、3 つ目のオブジェクトを追加してください。
...
配列 `allSongs` の 3 番目の要素はオブジェクトであるべきです。

```js
assert.isObject(allSongs[2]);
```

配列 `allSongs` 内の 3 番目のオブジェクトは、`id` プロパティが数値の `2` に設定されているようにしてください。

```js
assert.equal(allSongs[2].id, 2);
```

配列 `allSongs` 内の 3 番目のオブジェクトは、`title` プロパティが文字列の `"Still Learning"` に設定されているようにしてください。

```js
assert.equal(allSongs[2].title, "Still Learning");
```

配列 `allSongs` 内の 3 番目のオブジェクトは、`artist` プロパティが文字列の `"Quincy Larson"` に設定されているようにしてください。

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

配列 `allSongs` 内の 3 番目のオブジェクトは、`duration` プロパティが文字列の `"3:51"` に設定されているようにしてください。

```js
assert.equal(allSongs[2].duration, "3:51");
```

配列 `allSongs` 内の 3 番目のオブジェクトは、`src` プロパティが文字列の `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"` に設定されているようにしてください。
