...
Em seguida, você quer verificar se `age` é um inteiro. Então adicione outra chave `age` ao dicionário `constraints`. Para seu valor, chame `isinstance` passando `age` e `int` como seus argumentos.

# --hints--

Seu dicionário `constraints` deve ter uma chave `age`.
...
A chave `age` do seu dicionário `constraints` deve ter o valor de `isinstance(age, int)`.
