...
最後に、`75%` と `50%` のセレクターの間に新しい `100%` セレクターを作成してください。この新しいセレクターに `background-color` プロパティを設定し、その値を `yellow` にしてください。

これでアニメーションがより滑らかになり、観覧車が完成します。

# --hints--

`75%` ルール内に新しい `@keyframes cabins` セレクターを作成してください。
...
`75%` セレクターは `50%` と `100%` のセレクターの間にあるべきです。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText === '75%');
```

`75%` セレクターには `background-color` プロパティを `yellow` に設定してください。
