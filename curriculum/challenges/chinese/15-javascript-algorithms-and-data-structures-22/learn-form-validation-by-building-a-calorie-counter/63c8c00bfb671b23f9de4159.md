...
你还需要确认输入是否有效。 声明一个 `invalidInputMatch` 变量，并将其分配给使用 `currVal` 作为参数调用 `isInvalidInput` 函数的结果。

# --hints--

你应该声明一个 `invalidInputMatch` 变量。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=/);
```

你应该将调用 `isInvalidInput` 的结果分配给你的 `invalidInputMatch` 变量。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(/);
```

你应该将 `currVal` 作为参数传递给 `isInvalidInput`。
