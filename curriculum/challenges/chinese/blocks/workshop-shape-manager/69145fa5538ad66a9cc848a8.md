...
`updateResult` 函数将从下拉菜单中读取所选的形状类型，收集相应的输入值，创建一个 `shape` 对象，将其传递给 `calculateArea()` 函数以计算面积，并在 `resultText` 元素中显示计算结果。

在函数内部，查看所选的形状并构建正确的对象以传递给 `calculateArea` 函数。

如果形状是 `"circle"`，则调用 `calculateArea` 函数，传入一个包含类型和半径输入数值的对象。

# --hints--

你的 `updateResult` 函数应在形状为圆形时正确更新结果文本。
