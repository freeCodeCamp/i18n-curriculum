...
創建一個帶有條件 `songToHighlight` 的 `if` 語句。 對於語句，在 `songToHighlight` 上使用 `setAttribute` 來傳入 `"aria-current"` 和 `"true"` 作爲第一個和第二個參數。

# --hints--

你應該創建一個帶有條件 `songToHighlight` 的 `if` 語句。

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

你應該在 `if` 語句中的 `songToHighlight` 上使用 `setAttribute()` 方法。

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

你應該將 `"aria-current"` 和 `"true"` 作爲 `setAttribute` 方法的第一個和第二個參數傳入。
