...
Дано масив `arr`. Ітеруйте по ньому та видаляйте кожен елемент, починаючи з першого елемента (індекс 0), доки функція `func` не поверне `true` при передачі їй ітерованого елемента.

Потім поверніть залишок масиву, коли умова буде виконана, інакше `arr` має бути повернений як порожній масив.
