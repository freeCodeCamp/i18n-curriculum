...
Створіть правило `.image-1, .image-3` і надайте йому властивість `grid-column` зі значенням `1 / -1`. Це дозволить першому та третьому зображенням зайняти всю ширину сітки.

# --hints--

У вас має бути селектор `.image-1, .image-3`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

Ваш селектор `.image-1, .image-3` повинен мати властивість `grid-column`, встановлену в `1 / -1`.
