...
`invalid_records` 変数のすぐ後に、`for` ループを作成してそれを繰り返してください。無効なレコードごとに `Unexpected format '<key>: <val>' at position <index>.` を出力してください。`<key>`、`<val>`、および `<index>` は現在のキー、値、インデックスに置き換えてください。

`invalid_records` は現在の `dictionary` にある無効なレコードを指すキーのリストであることを覚えておいてください。`invalid_records` からキーを取得し、`dictionary` で値を参照する必要があります。

Position または `index` は、関数内の外側の `medical_records` ループで定義された `for` の現在の辞書を指します。

すでに作成したループや変数を思い出すために、コードをレビューしてください。

その後、`is_invalid` を `True` に設定してください。

無効なデータで `validate` 関数をテストして、バリデーションメッセージを確認してもかまいません。

これで医療バリデーターのワークショップは完了です。

# --hints--

`for` を繰り返す `invalid_records` ループがあるはずです。
...
`Unexpected format '<key>: <val>' at position <index>.` ループ内で `<key>` を出力してください（`<val>`、`<index>`、および `for` は現在のキー、値、インデックスに置き換えられるべきです）。
...
新しい `is_invalid` ループ内で `True` を `for` に設定してください。
