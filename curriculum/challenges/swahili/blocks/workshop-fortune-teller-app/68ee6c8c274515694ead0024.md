...
Bahati lazima iwe ya bahati nasibu, kwa hivyo kitendakazi msaidizi kinahitajika kuchagua bahati nasibu. Tangaza kigezo kinachoitwa `chosenCard` ndani ya njia ya `singleCardSelected`. Tumia njia ya `getRandomItem` kupata kadi ya bahati nasibu na pita `cards` kama hoja.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `chosenCard` ndani ya `singleCardSelected`.
...
Kigezo chako cha `chosenCard` kinapaswa kupewa thamani kwa kuita `getRandomItem(this.cards)`.
...
Unapaswa kupitisha `this.cards` kama hoja kwa `getRandomItem`.
