...
Додайте атрибут `value` зі значенням `player.club` до Club `input`. Після цього додайте множину атрибутів `onChange`, встановлену на стрілкову функцію, яка приймає `e` як параметр і викликає `setPlayer` з розгортанням `player` та `club`, встановленим у `e.target.value`.

# --hints--

Ваш Club `input` має мати `value`, рівний клубу гравця.
...
Ваш Club `input` має мати обробник `onChange`, який викликає `setPlayer` з розгортанням `player` та `club`, встановленим у `e.target.value`.
