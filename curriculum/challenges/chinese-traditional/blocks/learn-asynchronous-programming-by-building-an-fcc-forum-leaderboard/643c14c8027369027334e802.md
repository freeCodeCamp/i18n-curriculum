...
你想要取得回應主體作為 JSON 物件。你的 `res` 變數的 `.json()` 方法會傳回一個 promise，這表示你需要對它使用 `await`。

創建一個名為 `data` 的常數，並將其指定為 `await res.json()` 的值。

# --hints--

你應該有一個名為 `data` 的 `const` 變數。

```js
assert.match(code, /const\s+data/);
```

你應該將 `await res.json()` 指定給 `data` 變數。
