...
Você deve ter um seletor `.cat-whisker-right-top`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top'));
```

O seletor `.cat-whisker-right-top` deve ter a propriedade `position` definida como `absolute`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.position, 'absolute');
```

O seletor `.cat-whisker-right-top` deve ter a propriedade `top` definida como `120px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.top, '120px');
```

O seletor `.cat-whisker-right-top` deve ter a propriedade `left` definida como `109px`.
