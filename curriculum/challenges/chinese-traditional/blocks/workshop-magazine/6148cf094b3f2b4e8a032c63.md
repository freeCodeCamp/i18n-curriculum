...
`::first-letter` 偽選擇器允許你定位元素文字內容中的第一個字母。

創建 `.first-paragraph::first-letter` 選擇器並將 `font-size` 屬性設定為 `6rem`。同時為它設定 `color` 屬性為 `orangered`，以使其突出。

# --hints--

你應該有一個 `.first-paragraph::first-letter` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

你的 `.first-paragraph::first-letter` 選擇器應該有一個 `font-size` 屬性設定為 `6rem`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize, '6rem');
```

你的 `.first-paragraph::first-letter` 選擇器應該有一個設定為 `orangered` 的 `color` 屬性。
