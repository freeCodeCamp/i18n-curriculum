...
Crea un nuevo `div` debajo de tu elemento `.large` y asígnale un atributo `class` con el valor `calories-info`.
...
Su nuevo `div` debe tener un atributo `class` con valor `calories-info`.

```js
assert(document.querySelector('.label')?.lastElementChild?.classList?.contains('calories-info'));
```

Su nuevo `div` debe ir después del elemento `.large`.
