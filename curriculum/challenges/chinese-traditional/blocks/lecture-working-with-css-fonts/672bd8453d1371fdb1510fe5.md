...
你可以使用位於可輸入預覽文字欄位旁邊的藍色滾軸，自訂預覽文字的字型大小。
...
當你準備好將字型添加到你的專案時，請點擊頂部的藍色 `Get font` 按鈕。系統會帶你到一個摘要頁面，在那裡你可以看到目前選取的字型。你可以同時選取多個字型。
...
如果你選擇 `@import` 選項，則需要將該規則添加到你的 CSS 檔案中。
...
另一個很棒的選項是 Font Squirrel，在那裡你可以找到並下載你為設計所想像的訂製字型。
...
如果你搜尋字型並點擊結果，你會看到更多關於該字型的詳細資訊，包括樣本、試用、字形和 Webfont 套件。
...
你也可以看到該字型不同風格和變化的範例，例如 thin、light、medium、bold 和 black。
...
點擊「Download @font-face Kit」按鈕將會下載一個壓縮（zip）檔案。如果你解壓縮它，你會找到一個「web fonts」資料夾、以程式本文檔形式的授權文件，以及一個包含如何使用網頁字型說明的 HTML 檔案。
...
web fonts 資料夾中會有你專案所需的所有字型檔案。你會發現它們被分組到不同字型樣式的特定資料夾中。
