...
给定的矩形由 *m* × *n* 个正方形组成。 如果 *m* 和 *n* 不是奇数，那么可以沿着矩形切割一条路径方形边缘，使得矩形分成两个连接的具有相同形状的部分（将其中一个部分旋转 180° 后）。 2 × 2 和 4 × 3 矩形的所有此类路径如下所示。
