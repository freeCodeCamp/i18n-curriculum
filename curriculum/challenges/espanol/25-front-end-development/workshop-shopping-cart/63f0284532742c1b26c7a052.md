...
Añade tu función de callback al método `forEach`. Debe tomar un parámetro de `btn`. Luego, en el callback, añade un evento oyente al `btn`. El evento oyente debe escuchar un evento de `click`, y debe tomar otro callback con un parámetro de `event`. El segundo callback debe estar vacío.

# --hints--

Debes usar la sintaxis de flecha para añadir una función de callback al método `forEach` que tome un parámetro `btn`.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(\s*(\(\s*btn\s*\)|btn)\s*=>\s*(?:\{|\s*)/);
```

Debes añadir un evento oyente a la variable `btn`.
...
Debes escuchar un evento de `click` en la variable `btn`.
...
Debes añadir una función de callback vacía al evento oyente. Recuerda darle un parámetro de `event`.
