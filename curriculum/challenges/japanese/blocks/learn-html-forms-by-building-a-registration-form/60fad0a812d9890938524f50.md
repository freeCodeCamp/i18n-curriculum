...
Campersが自己紹介文に何を書くかの参考になるように、`placeholder` 属性が使われます。`placeholder` はテキストの値を受け取り、ユーザーが入力を始めるまで表示されます。

`textarea` に `placeholder` を `I like coding on the beach...` として与えてください。

# --hints--

`textarea` に `placeholder` 属性を与える必要があります。

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

`placeholder` に `I like coding on the beach...` の値を与える必要があります。
