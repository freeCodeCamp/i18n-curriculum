...
بما أنك أصبحت تفهم أفضل كيفية العمل مع طرق `split` و `reverse` و `join`، يمكنك حذف المتغير `exampleSentence` وعبارة وحدة التحكم.

# --hints--

يجب ألا يكون لديك متغير `exampleSentence` في شفرتك.

```js
assert.notMatch(code, /const\s+exampleSentence\s*=\s*['"]selur pmaCedoCeerf['"]\s*\.split\(['"]['"]\)\s*\.reverse\(\)\s*\.join\(['"]['"]\)/);
```

يجب ألا يكون لديك عبارة `console.log` في شفرتك.
