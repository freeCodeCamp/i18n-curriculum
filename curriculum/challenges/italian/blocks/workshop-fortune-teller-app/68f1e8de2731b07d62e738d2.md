...
Dichiara un'altra variabile chiamata `foundCard` e assegnale il risultato della chiamata al metodo `find()` su `cards`. Passa `card` come parametro callback e confronta `card.name_short` con `cardId` per trovare la carta corrispondente usando l'uguaglianza stretta.

# --hints--

Dovresti dichiarare una variabile chiamata `foundCard` all'interno di `showFortune`.
...
La tua `foundCard` dovrebbe essere assegnata chiamando `find` su `this.cards` con il callback corretto.
...
Il callback `find` dovrebbe avere un parametro chiamato `card`.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

All'interno del callback, `card.name_short` dovrebbe essere confrontato con `cardId`.
