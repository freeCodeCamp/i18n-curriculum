...
All'interno della classe `Bee`, implementa il metodo `render`, preceduto dalla parola chiave `override`, per aggiornare la proprietà `innerText` del `emojiElement` con il valore della proprietà `emoji`.

# --hints--

La tua classe `Bee` dovrebbe avere un metodo `render` che utilizza la parola chiave `override`.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

Dovresti impostare `this.emojiElement.innerText` su `this.emoji` all'interno del metodo `render` di `Bee`.
