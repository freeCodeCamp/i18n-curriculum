...
لتمييز الحاوية، يجب إضافة `box-shadow` بقيمة `0 5px 15px black`.

يضيف هذا تأثير ظل حول الحاوية. تتحكم القيم في الإزاحة الأفقية، والإزاحة الرأسية، ونصف قطر التمويه، واللون على التوالي.

# --hints--

يجب استهداف صنف `.container`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.container'));
```

يجب تعيين خاصية `box-shadow` إلى `0 5px 15px black`.
