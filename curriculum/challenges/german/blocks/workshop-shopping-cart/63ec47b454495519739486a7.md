...
Sie müssen die verfügbaren Produkte in Ihrem HTML anzeigen. Beginnen Sie damit, den Additionszuweisungsoperator zu verwenden, um einen leeren Template-Literal-String zur `innerHTML`-Eigenschaft der `dessertCards`-Variablen hinzuzufügen.

# --hints--

Sie sollten auf die `innerHTML`-Eigenschaft der `dessertCards`-Variablen zugreifen.

```js
assert.match(code, /dessertCards\.innerHTML/);
```

Sie sollten den Additionszuweisungsoperator auf die `innerHTML`-Eigenschaft anwenden.

```js
assert.match(code, /dessertCards\.innerHTML\s*\+=\s*/);
```

Sie sollten einen leeren Template-Literal-String zur `innerHTML`-Eigenschaft hinzufügen.
