...
A continuación, necesitas mostrar el título de la canción actual y el artista en la pantalla del reproductor.

Como primer paso, accede a los elementos `#player-song-title` y `#player-song-artist`, responsables de mostrar el título de la canción y el artista, y almacénalos en las variables `playingSong` y `songArtist`, respectivamente.

# --hints--

Debes tener una variable llamada `playingSong`.

```js
assert.isDefined(playingSong);
```

Tu variable `playingSong` debería tener el valor del elemento `#player-song-title`.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

Debes tener una variable llamada `songArtist`.
...
Tu variable `songArtist` debería tener el valor del elemento `#player-song-artist`.
