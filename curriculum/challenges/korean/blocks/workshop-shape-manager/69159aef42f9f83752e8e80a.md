...
이제 올바른 입력 필드를 표시하려면 `chooseShape()` 함수를 `val.value`과 함께 호출하고, 그다음 `updateResult()`를 호출해 영역을 다시 계산하고 표시하세요.

# --hints--

`chooseShape` 함수를 `val.value`와 함께 호출해야 합니다.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

`updateResult()` 함수를 호출해야 합니다.
