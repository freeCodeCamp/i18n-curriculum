...
للبناء التالي، قم بالتسكين لأربعة عناصر `div` داخل `.fb3` مع أصناف `fb3a`، `fb3b`، ثم `fb3a` مرة أخرى، و`fb3b` مرة أخرى، بهذا الترتيب. سيكون لهذا البناء أربعة أقسام، والقسمان العلويان سيكونان متشابهان تقريبًا مع القسمين السفليين.

# --hints--

يجب إضافة أربعة عناصر `div` داخل `.fb3`.

```js
assert.lengthOf(document.querySelectorAll("div.fb3 > div"), 4);
```

يجب إعطاء أول عنصر `div` جديد الصنف `fb3a`.

```js
assert.equal(document.querySelector("div.fb3").firstElementChild, document.querySelector("div.fb3a"));
```

يجب إعطاء ثاني عنصر `div` جديد الصنف `fb3b`.

```js
assert.equal(document.querySelector("div.fb3 :nth-child(2)"), document.querySelector("div.fb3b"));
```

يجب إعطاء ثالث عنصر `div` جديد الصنف `fb3a`.

```js
assert.equal(document.querySelector("div.fb3 :nth-child(3)"), document.querySelector("div.fb3b + div.fb3a"));
```

يجب إعطاء رابع عنصر `div` جديد الصنف `fb3b`.
