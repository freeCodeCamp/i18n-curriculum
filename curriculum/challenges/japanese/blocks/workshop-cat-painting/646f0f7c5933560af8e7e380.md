...
`.cat-whiskers-left` 要素の中に、クラスが `div`、`cat-whisker-left-top`、`cat-whisker-left-middle` の `cat-whisker-left-bottom` 要素を3つ作成してください。

# --hints--

既存のクラス `div` を持つ `cat-whiskers-left` 要素は変更しないでください。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers-left'), 1);
```

`div` 要素の中に `.cat-whiskers-left` 要素を3つ作成する必要があります。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers-left div'), 3);
```

`div` 要素の中の最初の `.cat-whiskers-left` 要素にはクラス `cat-whisker-left-top` を付けてください。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-left div')[0].classList.contains('cat-whisker-left-top'));
```

`div` 要素の中の2番目の `.cat-whiskers-left` 要素にはクラス `cat-whisker-left-middle` を付けてください。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-left div')[1].classList.contains('cat-whisker-left-middle'));
```

`div` 要素の中の3番目の `.cat-whiskers-left` 要素にはクラス `cat-whisker-left-bottom` を付けてください。
