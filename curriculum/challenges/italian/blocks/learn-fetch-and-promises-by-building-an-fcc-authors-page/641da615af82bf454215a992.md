...
Passa una funzione di callback vuota al metodo `.forEach()`. Per il primo parametro della callback, destruttura gli elementi `author`, `image`, `url` e `bio`. 

Per il secondo parametro, passa `index`. Questo rappresenterà la posizione di ogni autore e sarà utile per la paginazione in seguito.
...
Dovresti destrutturare `author`, `image`, `url` e `bio` come primo parametro della tua funzione di callback. Non dimenticare di metterli dentro parentesi graffe.
...
Dovresti destrutturare `index` come secondo parametro della tua funzione di callback. Deve essere esterno alle parentesi graffe in cui hai messo gli altri.

```js
assert.match(code, /authors\.forEach\(\s*\(\s*\{.*\}\s*,\s*index\s*\)/)
```

La tua funzione di callback dovrebbe usare la sintassi a freccia.
