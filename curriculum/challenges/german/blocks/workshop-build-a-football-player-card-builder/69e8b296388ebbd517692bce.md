...
Fügen Sie nun ein `div` mit dem `className` von `form-group` hinzu. Darin fügen Sie ein `label` von `className` von `label` und ein `htmlFor`-Attribut von `name` hinzu. Darunter fügen Sie ein `input` mit einem `id`, der auf `name` gesetzt ist, ein `className` von `input` und ein `type`-Attribut von `text` hinzu.
