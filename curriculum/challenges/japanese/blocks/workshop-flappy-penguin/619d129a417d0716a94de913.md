...
`.beak` 要素を `class` の `bottom` で指定し、`width` より `4%` 小さく、親要素の上からは `.beak.top` 離れ、左からは `5%` 離れた `2%` を与えてください。ただし、左からの距離は `.beak.top` よりも離してください。

# --hints--

`.beak.bottom` セレクターを用意してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak']));
```

`.beak.bottom` に `width` の `--fcc-expected--` を与えるべきですが、`--fcc-actual--` が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak'])?.width, '16%');
```

`.beak.bottom` に `top` の `--fcc-expected--` を与えるべきですが、`--fcc-actual--` が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak'])?.top, '65%');
```

`.beak.bottom` に `left` の `--fcc-expected--` を与えるべきですが、`--fcc-actual--` が見つかりました。
