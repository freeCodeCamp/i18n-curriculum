...
在事件監聽器的箭頭函式中，為你的 `if` 添加檢查 `userData?.currentSong` 是否為 `null`。

在 `if` 區塊內，使用 `userData?.songs` 陣列中第一首歌曲的 `id` 呼叫 `playSong()` 函式。這將確保播放清單中的第一首歌曲先被播放。
...
你應該在你的 `if` 區塊內使用 `userData?.songs[0].id` 來呼叫 `playSong` 函式（程式）。
