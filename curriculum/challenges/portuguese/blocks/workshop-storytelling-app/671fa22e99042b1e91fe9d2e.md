...
Dentro do `.btn-container` , crie três botões para cada tipo de story.Dê a cada botão uma classe `btn` e um `id` de `scary-btn`, `funny-btn` e `adventure-btn`.O texto para cada botão deve ser `Scary Story`, `Funny Story` e `Adventure Story`, respectivamente.
