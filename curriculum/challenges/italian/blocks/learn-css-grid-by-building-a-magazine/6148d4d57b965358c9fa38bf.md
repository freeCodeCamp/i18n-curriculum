...
Ora è il momento di definire lo stile del tuo terzo `section`. Nota che ha i valori `text` e `text-with-images` per l'attributo `class`, il che significa che sta già ereditando gli stili dalla tua regola `.text`.

Crea un selettore `.text-with-images` e imposta la proprietà `display` su `grid`.

# --hints--

Dovresti avere un selettore `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

Il tuo selettore `.text-with-images` dovrebbe avere una proprietà `display` impostata su `grid`.
