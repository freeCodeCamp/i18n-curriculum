...
残りの楽器はすでに`instrumentsArr`配列に追加されています。

次に、2つの変数`selectContainer`と`productsContainer`を宣言し、それぞれにHTMLのドロップダウンとクラス`products-container`を持つ要素を代入してください。

# --hints--

`selectContainer`変数を用意してください。

```js
assert.exists(selectContainer);
```

`selectContainer`変数には`select`要素の値を代入してください。

```js
assert.strictEqual(selectContainer, document.querySelector("select"));
```

`productsContainer`変数を用意してください。

```js
assert.exists(productsContainer);
```

`productsContainer`変数には`.products-container`要素の値を代入してください。
