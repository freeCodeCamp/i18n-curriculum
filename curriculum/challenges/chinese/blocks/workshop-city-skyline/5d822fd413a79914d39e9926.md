...
为 `.fb3-window` 元素赋予 `25%` 的 `width`、`80%` 的 `height`，并使用 `--window-color1` 变量作为 `background-color` 的值。

# --hints--

你应该给 `.fb3-window` 一个值为 `25%` 的 `width`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

你应该给 `.fb3-window` 一个值为 `80%` 的 `height`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

你应该给 `.fb3-window` 一个值为 `--window-color1` 的 `background-color`。
