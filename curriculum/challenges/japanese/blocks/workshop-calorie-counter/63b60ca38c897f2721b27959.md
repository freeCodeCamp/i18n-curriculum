...
フォームには結果を表示する場所が必要です。空の`div`要素を追加し、`id`に`output`を、`class`に`output`と`hide`の値を設定してください。

# --hints--

`div`の後に`form`要素を追加してください。

```js
assert.exists(document.querySelector('.container > div'));
```

新しい`div`要素には`id`を`output`に設定してください。

```js
assert.equal(document.querySelector('.container > div')?.id, 'output');
```

新しい`div`要素には`class`の値として`output`と`hide`を設定してください。
