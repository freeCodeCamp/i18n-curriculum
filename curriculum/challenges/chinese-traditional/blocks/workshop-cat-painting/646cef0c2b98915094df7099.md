...
那些邊緣對耳朵來說太銳利了。所以，將 `border-top-left-radius` 設為 `90px`，並將 `border-top-right-radius` 設為 `10px`。

# --hints--

你的 `.cat-left-ear` 選擇器應該有一個 `border-top-left-radius` 屬性設為 `90px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-ear')?.borderTopLeftRadius, '90px')
```

你的 `.cat-left-ear` 選擇器應該有一個 `border-top-right-radius` 屬性設定為 `10px`。
