...
`sky` 클래스를 그 안의 모든 프로퍼티와 값과 함께 미디어 쿼리 안에 복사해서 붙여넣으세요. 스카이라인의 색상 체계를 낮에서 밤으로 바꾸는 또 다른 색상 체계를 만들 것입니다.

참고: 클래스를 복사하려면 편집 가능한 영역을 스크롤해서 지나가야 합니다.
