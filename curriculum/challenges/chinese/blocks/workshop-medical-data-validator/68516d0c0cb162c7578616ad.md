...
为你的 `constraints` 字典添加最后一个密钥 `last_visit_id`。对于它的值，使用 `isinstance` 来验证 `last_visit_id` 是一个字串。

# --hints--

你的 `constraints` 字典应包含一个 `last_visit_id` 密钥。
...
你的 `constraints` 字典中的 `last_visit_id` 密钥应具有 `isinstance(last_visit_id, str)` 的值。
