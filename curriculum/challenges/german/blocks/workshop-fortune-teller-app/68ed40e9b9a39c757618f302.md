...
Jetzt benötigen Sie eine Hauptklasse, die dabei hilft, die Benutzeroberfläche dynamisch im DOM zu rendern.

Erstellen Sie eine Klasse namens `Game`. Fügen Sie darin eine Eigenschaft namens `cards` vom Typ `Card[]` hinzu, und initialisieren Sie sie vorerst mit einem leeren Array.

# --hints--

Sie sollten eine Klasse namens `Game` haben.
...
Sie sollten eine `cards`-Eigenschaft innerhalb der `Game`-Klasse haben.
...
Ihre `cards`-Eigenschaft sollte den Typ `Card[]` haben.
...
Ihre cards-Eigenschaft sollte auf ein leeres `array` gesetzt sein.
