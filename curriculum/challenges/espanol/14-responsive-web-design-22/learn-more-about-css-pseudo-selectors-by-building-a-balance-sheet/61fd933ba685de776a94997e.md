...
Dentro del primer `tr`, agrega un elemento `th` con el texto `Loans The outstanding balance on our startup loan.`. Anida ese texto, excepto `Loans`, dentro de un elemento `span` con la `class` establecida a `description`.
