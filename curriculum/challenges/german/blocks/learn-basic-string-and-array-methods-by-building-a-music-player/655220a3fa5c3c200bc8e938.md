...
Verwenden Sie die `classList`- und `remove()`-Methode, um die `playing`-Klasse vom `playButton` zu entfernen, da das Lied an dieser Stelle pausiert wird.

Um das Lied schließlich zu pausieren, verwenden Sie die `pause()`-Methode auf der `audio`-Variable. `pause()` ist eine Methode der Web Audio API zum Pausieren von Musikdateien.

# --hints--

Sie sollten die vorhandene `pauseSong`-Funktion und deren Inhalt nicht verändern.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

Sie sollten die `classList`-Eigenschaft und die `remove()`-Methode verwenden, um die Klasse `"playing"` vom `playButton` zu entfernen.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

Sie sollten die `pause()`-Methode auf Ihrer `audio`-Variable verwenden.
