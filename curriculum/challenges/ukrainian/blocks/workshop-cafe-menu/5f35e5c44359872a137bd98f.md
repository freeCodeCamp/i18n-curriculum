...
Вилучіть прокоментовану властивість `background-color`.

```js
assert.notMatch(code, /\/\*\s*background-color:\s*burlywood;?\s*\*\//i);
```

Селектор `body` не повинен містити жодних коментарів.

```js
assert.notMatch(code, /body\s*{\s*\/\*/i);
```

Встановіть властивість `background-image` на `url(https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg)`.
...
Елемент `body` повинен містити фон з кавовими бобами.
