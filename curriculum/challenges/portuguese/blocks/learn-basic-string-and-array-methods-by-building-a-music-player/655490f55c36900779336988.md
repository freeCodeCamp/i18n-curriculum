...
Percorra os `playlistSongElements` com o método `forEach`. 

O método `forEach` é usado para percorrer um array e executar uma função em cada elemento do array.Por exemplo, suponha que você tenha um array de números e queira registrar cada número no console.
...
Use o método `forEach` em `playlistSongElements`.Passe `songEl` como parâmetro e use a sintaxe de arrow para adicionar um callback vazio.

# --hints--

Você deve encadear `forEach()` a `playlistSongElements`.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

Seu método `forEach()` deve ter um parâmetro `songEl` e usar a sintaxe de arrow.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

Seu `forEach` deve ter um par vazio de chaves.
