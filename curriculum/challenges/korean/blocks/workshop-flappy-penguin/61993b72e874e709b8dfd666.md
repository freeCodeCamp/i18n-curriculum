...
`.penguin-head` 요소를 선택하고, 부모 너비의 `width` 절반과 `height`의 `45%`를 지정하세요. 그런 다음, `background`를 `45deg` 위치에서 시작해 `gray`에서 시작하고 `rgb(239, 240, 228)`에서 끝나는 선형 그라디언트로 설정하세요.

# --hints--

`.penguin-head` 선택자를 사용하세요.

```js
assert.match(code, /\.penguin-head\s*\{/);
```

`.penguin-head`에 `width`의 `50%`을 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

`.penguin-head`에 `height`의 `45%`을 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

`.penguin-head`에 `background`의 `linear-gradient(45deg, gray, rgb(239, 240, 228))`을 지정하세요.
