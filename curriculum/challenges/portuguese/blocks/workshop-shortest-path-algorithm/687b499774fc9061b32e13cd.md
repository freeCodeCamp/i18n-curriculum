...
No final da sua função, retorne os dados que você calculou para que possam ser usados fora da função.

Retorne tanto a lista das menores `distances` do `start_node` para todos os outros nós quanto a lista de `paths` que levam a cada nó

# --hints--

Você deve ter uma declaração `return` no final da sua função.
...
Você deve retornar tanto `distances` e `paths`.
