...
用另一个作者测试你的 `getBooksByAuthor` 函数会很好。

首先将消息 `"\nList of books by James Clear:\n"` 记录到控制台日志中。 

在那个 `console.log()` 下面，添加另一个 `console.log()`。在那个 `console.log()` 内，使用 `library` 和 `"James Clear"` 作为参数调用 `getBooksByAuthor()` 函数。
...
你应该有一个 `console.log(getBooksByAuthor(library, "James Clear"))`。
