...
以前のプロジェクトで、次のように連結演算子を使って変数を文字列に挿入する方法を学びました。
...
文字列 `"You now have a new weapon."` を `"You now have a "` に変更し、その後に新しい武器の名前を連結してください。文をピリオドで終わらせることも忘れないでください。


# --hints--

`text.innerText` への代入を、文字列 `"You now have a "` で始まるように変更してください。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1/);
```

`text.innerText` の文字列の末尾に、連結演算子を使用して `newWeapon` を連結してください。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1\s*\+\s*newWeapon/);
```

連結演算子を使用して、`text.innerText` の文字列を `.` で終わらせてください。
