...
Chini ya kipengele cha `.blush.right`, ongeza vipengele viwili vya `div` kila kikiwa na `class` ya `beak`. Pia, mpe kipengele cha kwanza cha `.beak` `class` ya `top`, na kipengele cha pili cha `.beak` `class` ya `bottom`.

# --hints--

Unapaswa kuongeza vipengele viwili vya `div` ndani ya `.penguin-head`. Vipengele vinavyotarajiwa vya `--fcc-expected--` `div`, vilivyoripotiwa `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 9);
```

Unapaswa kumpa `div` mpya la kwanza `class` ya `beak`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

Unapaswa kumpa `div` mpya la pili `class` ya `beak`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div.beak')?.length, 2);
```

Unapaswa kumpa `div` mpya la kwanza `class` ya `top`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

Unapaswa kumpa `div` mpya la pili `class` ya `bottom`.
