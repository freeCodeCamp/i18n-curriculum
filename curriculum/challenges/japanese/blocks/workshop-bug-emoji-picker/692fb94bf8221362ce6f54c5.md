...
`Spider` クラスの中で、`render` キーワードを接頭辞として付けた `override` メソッドを実装し、`innerText` の `emojiElement` プロパティを `emoji` プロパティの値で更新してください。

# --hints--

あなたの `Spider` クラスには `render` メソッドが必要です。
...
あなたの `Spider` `render` メソッドは `override` キーワードを使うべきです。

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?override\s+render/);
```

あなたの `Spider` `render` メソッドは `this.emojiElement.innerText` を `this.emoji` に設定するべきです。
