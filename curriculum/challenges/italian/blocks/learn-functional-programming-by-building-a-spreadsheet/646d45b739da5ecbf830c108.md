...
La tua funzione `apply` restituisce la funzione spreadsheet, ma non la applica effettivamente. Aggiorna `apply` per chiamare la funzione. Passa il risultato della chiamata a `toNumberList` con `args` come argomento.

# --hints--

La tua funzione `apply` dovrebbe chiamare la funzione `spreadsheetFunctions[fn.toLowerCase()]`.
...
Dovresti passare una chiamata `toNumberList()` alla tua chiamata `spreadsheetFunctions[fn.toLowerCase()]`.
...
Dovresti passare `args` alla tua chiamata `toNumberList()`.
