...
Y como tercer argumento de color, pasa la función `hsl` con los valores `240` para el tono (hue), `56%` para la saturación y `42%` para la luminosidad.

# --hints--

La regla CSS `.blue` debe tener una propiedad `background` establecida en `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))`.
