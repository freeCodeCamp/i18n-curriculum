...
Para la condición de la sentencia `if`, verifica si `target` no es una instancia de `HTMLElement`. Luego, dentro de ella, agrega un `return`.

# --hints--

Debes verificar que en tu sentencia `if` esté la condición `!(target instanceof  HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

Deberías tener un `return` dentro de tu sentencia `if`.
