...
Agrega un atributo `onChange` al input para capturar lo que el usuario teclea. Asígnalo a una función flecha que toma `e` como argumento. Dentro de la función, actualiza el estado `query` pasando `e.target.value` a la función asignadora, que contiene el valor actual del input. Aquí hay un ejemplo:
