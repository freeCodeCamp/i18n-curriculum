...
现在，你需要为当前播放的歌曲添加 `aria-current` 属性。

查看 `songToHighlight` 是否存在。如果存在，使用 `setAttribute` 方法对 `songToHighlight` 调用，并将 `"aria-current"` 和 `"true"` 作为第一个和第二个参数传入。 

# --hints--

`highlightCurrentSong` 应为当前播放的歌曲添加 `aria-current` 属性，并从之前播放的歌曲中移除该属性。
