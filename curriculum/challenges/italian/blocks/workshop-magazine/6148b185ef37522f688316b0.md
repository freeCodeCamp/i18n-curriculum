...
Ora puoi definire lo stile del contenuto dell'elemento `.heading` con CSS Grid.

La funzione CSS `repeat()` serve a ripetere un valore, invece di scriverlo manualmente, ed è utile per i layout a griglia. Per esempio, impostando la proprietà `grid-template-columns` su `repeat(20, 200px)` si creerebbero 20 colonne larghe ciascuna `200px`.

Dai al tuo elemento `.heading` una proprietà `grid-template-columns` impostata su `repeat(2, 1fr)` per creare due colonne di larghezza uguale.

# --hints--

Il tuo selettore `.heading` dovrebbe avere una proprietà `grid-template-columns` impostata su `repeat(2, 1fr)`.
