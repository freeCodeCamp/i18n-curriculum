...
Тепер заповніть `renderEntry`. Оголосіть запасну константу для кожного поля: `const title = entry.title || "Unknown"`, і застосуйте той самий шаблон для `author`, `year` та `location`.

Поверніть багаторядковий шаблонний рядок, який форматуватиме запис як бібліотечну картку. Використайте `"-".repeat(25)` для верхнього та нижнього кордону, а також включіть мітки для `Title`, `Author`, `Year` і `Location`. Наприклад, мітка для назви виглядатиме як `Title: ${title}`.

# --hints--

`renderEntry` має повертати рядок.

```js
assert.isString(renderEntry(catalog[0]));
```

При виклику з `catalog[0]` повернутий рядок має містити `"From a Buick 8"`.

```js
assert.include(renderEntry(catalog[0]), "From a Buick 8");
```

При виклику з `catalog[0]` повернутий рядок має містити `"King, Stephen"`.

```js
assert.include(renderEntry(catalog[0]), "King, Stephen");
```

При виклику з `catalog[0]` повернутий рядок має містити рік `2002`.

```js
assert.include(renderEntry(catalog[0]), "2002");
```

При виклику з `catalog[0]` повернутий рядок має містити `"Shelf K7"`.

```js
assert.include(renderEntry(catalog[0]), "Shelf K7");
```

Повернутий рядок має містити верхній та нижній кордон, використовуючи `"-".repeat(25)`.
