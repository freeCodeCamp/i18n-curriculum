...
Quando as músicas são exibidas na página, deve mostrar o título, artista, duração de cada música e um botão de exclusão. 

Nos próximos passos, você aprenderá como adicionar novo HTML para cada música usando o método <dfn>map()</dfn>.Este método será explicado com mais detalhes na próxima etapa.

Comece usando `const` para declarar uma variável chamada `songsHTML` e atribuir a ela `array.map()`.

# --hints--

Você deve ter uma variável `const` chamada `songsHTML`.

```js
assert.match(code, /const\s+songsHTML/);
```

Você deve atribuir `array.map()` ao seu `songsHTML`.
