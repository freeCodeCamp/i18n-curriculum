...
`.blush` 요소를 `class`인 `left`으로 선택하고, 부모의 왼쪽에 `15%` 위치시키세요. 그런 다음 `.blush` 요소를 `class`인 `right`으로 선택하고, 부모의 오른쪽에 `15%` 위치시키세요.

# --hints--

`.blush.left` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.blush.left', '.left.blush']));
```

`.blush.left`에 `left`인 `--fcc-expected--`을 지정해야 하지만 `--fcc-actual--`이(가) 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.blush.left', '.left.blush'])?.left, '15%');
```

`.blush.right` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.blush.right', '.right.blush']));
```

`.blush.right`에 `right`인 `--fcc-expected--`을 지정해야 하지만 `--fcc-actual--`이(가) 발견되었습니다.
