...
Використовуючи список вибору (`selector1, selector2`), надайте обом елементам `#controls` та `#stats` `border` в `1px solid #0a0a23`, колір тексту `#0a0a23` та `5px` `padding`.

# --hints--

Ви повинні мати список селекторів `#controls, #stats`.
...
Селектор `#controls, #stats` повинен мати `border` зі значенням `1px solid #0a0a23`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#controls, #stats')?.border, '1px solid rgb(10, 10, 35)');
```

Селектор `#controls, #stats` повинен мати `color` зі значенням `#0a0a23`.
...
Селектор `#controls, #stats` повинен мати відступ зі значенням `5px`.
