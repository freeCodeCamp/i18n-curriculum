...
Establece `font-size` en `1.1rem`, `display` en `block` y `margin` en `auto` para centrar el botón.

# --hints--

Debes continuar apuntando a la clase `.submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Establece la propiedad `font-size` en `1.1rem`.
...
Establece la propiedad `margin` en `auto`.
