...
In questo passaggio, completerai la funzione `updateResultsList`.

Ora itera su `units` array. Per ogni valore, verifica se non è uguale all’unità corrente (`unitToConvert.value`).

Se la condizione è vera, converti l’unità esistente nell’unità corrente e aggiungi un nuovo elemento di lista a `resultList` nel formato:

`ingredient: quantity unit`

Per esempio, se l’input è `flour`, `5` e `gram`, il risultato dovrebbe essere:
...
Quando clicchi il pulsante, la tua funzione `updateResultsList` dovrebbe aggiornare correttamente il DOM.
