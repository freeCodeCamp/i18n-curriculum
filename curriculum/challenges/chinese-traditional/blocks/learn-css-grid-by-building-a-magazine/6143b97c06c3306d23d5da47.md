...
在你的 `article` 元素中，創建一個 `h3` 元素，將 `class` 設置爲 `list-title`，文本爲 `A Brief History`。 在此之下，創建一個 `p` 元素，文本爲 `Of the Curriculum`。 然後創建一個類爲 `lists` 的 `ul` 元素。

# --hints--

你應該在 `article` 元素中創建一個 `h3` 元素。

```js
assert.exists(document.querySelector('article h3'));
```

你應該在 `article` 元素中創建一個 `p` 元素。

```js
assert.exists(document.querySelector('article p'));
```

你應該在 `article` 元素中創建一個 `ul` 元素。

```js
assert.exists(document.querySelector('article ul'));
```

`article` 元素中的元素應按正確的順序排列。
