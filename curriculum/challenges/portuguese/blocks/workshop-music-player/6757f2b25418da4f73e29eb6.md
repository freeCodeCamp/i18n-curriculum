...
Em seguida, você precisa exibir o título da música atual e o artista no display do player.

Como primeiro passo, acesse os elementos `#player-song-title` e `#player-song-artist`, responsáveis por exibir o título da música e o artista, e armazene-os nas variáveis `playingSong` e `songArtist`, respectivamente.

# --hints--

Você deve ter uma variável chamada `playingSong`.

```js
assert.isDefined(playingSong);
```

Sua variável `playingSong` deve ter o valor do elemento `#player-song-title`.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

Você deve ter uma variável chamada `songArtist`.
...
Sua variável `songArtist` deve ter o valor do elemento `#player-song-artist`.
