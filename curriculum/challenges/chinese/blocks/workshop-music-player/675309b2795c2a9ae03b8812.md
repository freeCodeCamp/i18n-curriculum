...
请注意，超文本标记语言中的专辑封面和 `userData.songs` 数组中的歌曲已更改。我们用更短的歌曲替换了原始歌曲，供你测试你的应用程序。

所有核心功能现已到位。 现在唯一的问题是，当前播放的歌曲结束时不会自动播放下一首歌曲。 

为了解决这个问题，你可以设置一个事件监听器来检测当前播放的歌曲何时结束。 `"ended"` 事件监听器适合这种情况。 当媒体播放结束时会触发此事件。

为你的 `audio` 元素添加一个监听 `"ended"` 事件的事件监听器，并传入对 `playNextSong` 函数的引用。这样音乐播放器项目就完成了！

# --hints--

你应该将 `addEventListener()` 方法链接到你的 `audio` 变量。

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

你在 `audio` 变量上使用的事件监听器应该监听 `"ended"` 事件。

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

你应该将对 `playNextSong` 的引用传递给你的 `"ended"` 事件监听器。
