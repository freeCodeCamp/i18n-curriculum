...
Usando la sintassi arrow completa la funzione di callback `forEach`. All’interno del corpo della funzione di callback usa un’assegnazione con addizione per impostare il `innerHTML` di `tasksContainer` su backtick vuoti.

# --hints--

Non dovresti modificare il `taskData.forEach()` esistente e i valori che hai destrutturato.
...
Dovresti usare la sintassi arrow e allegare `innerHTML` a `tasksContainer`.
