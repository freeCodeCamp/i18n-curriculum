...
プログラミングでは、`null` は値が存在しないことを表すために使われます。この場合、ユーザーが無効な入力をしたら警告を表示し、その後関数が失敗したことを示すために `null` を返したいです。

引き続き `if` ブロック内で、`isError` に `true` を設定し、`null` を返してください。

# --hints--

`alert` の後に、`isError` を `true` に設定してください。
...
`isError` を変更した後、`return` で値 `null` を行う必要があります。
