...
El código de prueba `kingBooks` confirmó que `findByAuthor` funciona correctamente. Antes de continuar con `groupByDecade`, limpia la salida de la consola eliminándola.

Elimina la línea `const kingBooks = ...`, `console.log(kingBooks.length)` y el bucle `for` que muestra los títulos y años de los libros.

# --hints--

La variable `kingBooks`, la instrucción `console.log(kingBooks.length)` y el bucle `for` deben eliminarse todos.
