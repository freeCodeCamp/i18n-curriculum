...
O formulário agora está completo. Para fazer o cartão de pré-visualização atualizar enquanto você digita, é preciso conectar as entradas ao estado `player`.

Começando pelo `input` Nome, adicione a ele um atributo `value` com valor `player.name`. Depois, adicione um atributo `onChange` definido como uma arrow function que recebe `e` como parâmetro e chama `setPlayer` com um spread de `player` e `name` definido como `e.target.value`. Não esqueça de colocar tanto o spread quanto a definição do valor `name` dentro de um objeto.

# --hints--

Sua entrada de nome `value` deve estar vinculada ao campo `name` do estado player.
...
Sua entrada de nome deve ter um manipulador `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

O manipulador `onChange` deve chamar `setPlayer` com `setPlayer` contendo um objeto que espalha `player` e define `name` como `e.target.value`.
