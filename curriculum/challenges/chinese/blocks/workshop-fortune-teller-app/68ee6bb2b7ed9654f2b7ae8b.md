...
为全局 `document` 对象添加另一个事件监听器，该监听器调用 `showFortune` 函数。在回调函数内部，包含一个名为 `e` 的参数，类型为 `Event`，并将 `e` 作为参数传递给 `showFortune` 方法。

`showFortune` 将处理卡片点击事件，比如从 DOM 中读取被点击卡片的 ID 并显示匹配的卡片运势详情。你将在接下来的步骤中创建它。

# --hints--

你应该为全局 `click` 赋值一个 `document` 事件监听器。
