...
Agora você tem algo que começa a se parecer com um edifício. Você está pronto para criar sua primeira variável.  In previous lessons you learned that variable declarations begin with two dashes (`-`) and are given a name and a value like this: `--variable-name: value;`. Na regra para a classe `bb1`, crie uma variável chamada `--building-color1` e dê a ela um valor de `#999`.
