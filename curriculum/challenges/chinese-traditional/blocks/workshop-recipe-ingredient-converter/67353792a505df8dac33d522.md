...
你最內層的柯里化函式（curried function）將會包含轉換該值的邏輯。

首先，使用 `fromUnit` 引數來存取你的 `conversionTable` 物件，然後使用 `toUnit` 引數來存取巢狀物件。

傳回將該轉換值與 `quantity` 值相乘的結果。

# --hints--

你的函式（程式）應該傳回 `convertQuantity("gram")("cup")(1)` 的正確轉換結果。

```js
assert.strictEqual(convertQuantity("gram")("cup")(1), 1/240)
```

你的函式（程式）應該傳回 `convertQuantity("teaspoon")("ounce")(6)` 的正確轉換結果。
