...
O restante da sua equipe foi criado em um array chamado `remainingCrew` com os seguintes dados: 
...
Percorra o `remainingCrew` array e adicione cada astronauta ao `squad` usando a `addCrewMember()` função.

# --hints--

Você deve usar um `for` loop para chamar `addCrewMember()` com cada astronauta em `remainingCrew`.
