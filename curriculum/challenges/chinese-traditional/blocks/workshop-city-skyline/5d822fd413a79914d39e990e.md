...
將 `bb4-window` 類的 `width` 設置爲 `18%`，`height` 設置爲 `90%`，並將`background-color` 的值設爲變量`--window-color4` 。

# --hints--

你應該給 `.bb4-window` 添加一個值爲 `18%` 的 `width` 。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

你應該給 `.bb4-window` 添加一個值爲 `90%` 的 `height` 。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

你應該給 `.bb4-window` 添加一個值爲 `--window-color4` 的 `background-color` 。
