...
Das Problem ist derzeit, dass sobald die Vorschau aktualisiert wird oder Sie die Seite neu laden, alles, was Sie für die Spielerkarte hinzufügen, verschwindet.

Das Speichern aller Eingaben in `localStorage` verhindert dies. Um damit zu beginnen, erstellen Sie eine `STORAGE_KEY`-Konstante mit dem String `football_player_card` als Wert.

# --hints--

Sie sollten eine `STORAGE_KEY`-Variable deklarieren.
...
Ihre `STORAGE_KEY` sollte den String `football_player_card` als Wert haben.
