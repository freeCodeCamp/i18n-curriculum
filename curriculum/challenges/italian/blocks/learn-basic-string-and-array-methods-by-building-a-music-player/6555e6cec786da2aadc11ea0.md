...
Dovresti anche renderizzare di nuovo le canzoni, mettere in pausa la canzone attualmente in riproduzione, impostare il display del lettore e impostare di nuovo il testo accessibile del pulsante di riproduzione.

Chiama la funzione `renderSongs` passando `userData?.songs` come argomento. Inoltre, chiama le funzioni `pauseSong`, `setPlayerDisplay` e `setPlayButtonAccessibleText`.

# --hints--

Non dovresti modificare la funzione `shuffle` esistente né il suo contenuto.
