...
继续为页面添加样式。 选择导航栏中的列表元素，并给它们添加下面的样式：
...
你应将 `li` 元素的 `margin` 设置为 `0 0.2rem`。
