...
Usando la regola sopra e partendo da 13, generiamo la seguente sequenza:

<div style='text-align: center;'>13 → 40 → 20 → 10 → 5 → 16 → 8 → 4 → 2 → 1</div>

Si può vedere che questa sequenza (che inizia da 13 e termina a 1) contiene 10 termini. Anche se non è ancora stato dimostrato (problema di Collatz), si pensa che tutti i numeri di partenza finiscano a 1.

Quale numero di partenza, sotto il dato `limit`, produce la catena più lunga?

**Nota:** Una volta iniziata la catena, i termini possono superare `limit`.
