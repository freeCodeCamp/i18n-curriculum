...
HTMLを見ると、2番目と3番目のパネルが非表示になっているのがわかります。これは`hidden`属性を使って行われています。最初のパネルはデフォルトでは表示されています。ユーザーがタブをクリックすると、すべてのパネルが非表示になる必要があります。

`forEach`メソッドを使って`panels`をループし、各パネルの`hidden`属性を`true`に設定してください。

# --hints--

`panels`をループして、各パネルの`hidden`属性を`true`に設定してください。
