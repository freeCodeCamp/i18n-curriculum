...
Ora dovresti implementare il loop principale della simulazione. Questo loop assicurerà che ogni gate venga elaborato per ogni segno di graduazione nella simulazione fino a quando tutti i segni di graduazione non saranno completati.

Crea un loop `while` vuoto che continui finché `tickIndex` è minore di `maxTicks`.

# --hints--

Dovresti creare un loop `while` vuoto con la condizione `(tickIndex < maxTicks)`.
