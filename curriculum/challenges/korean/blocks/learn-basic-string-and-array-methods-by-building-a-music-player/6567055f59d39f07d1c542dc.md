...
다음으로, HTML 문서에서 재생 목록을 업데이트하여 노래를 표시하세요.

`songsHTML` 요소의 `innerHTML` 속성에 `playlistSongs`를 할당하세요. 이렇게 하면 방금 만든 `li` 요소가 이미 제공된 `ul` 파일의 `HTML` 요소 안에 삽입됩니다.

# --hints--

`innerHTML`에 `songsHTML`를 추가하려면 `playlistSongs` 속성을 사용하세요.
