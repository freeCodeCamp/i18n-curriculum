...
استخدم `classList` وطريقة `remove()` لإزالة الصنف `playing` من `playButton`، إذ ستتوقف الأغنية في هذه النقطة.

لإيقاف الأغنية نهائيًا، استخدم طريقة `pause()` على المتغير `audio`. `pause()` هي طريقة من واجهة برمجة تطبيقات الصوت في الويب لإيقاف ملفات الموسيقى مؤقتًا.

# --hints--

لا تعدل دالة `pauseSong` الموجودة ومحتواها.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

يجب استخدام الخاصية `classList` والطريقة `remove()` لإزالة الصنف `"playing"` من `playButton`.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

استخدم الطريقة `pause()` على متغير `audio` الخاص بك.
