...
Створіть ще один `p` елемент під вашим `.first-paragraph` елементом і додайте йому такий текст:
...
Ви маєте створити другий `p` елемент всередині вашого `.text` елемента.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 2);
```

Ваш другий `p` елемент повинен містити наданий текст.
