...
Ще всередині блоку `else` створіть цикл `while`, який перевіряє, що `current_node.next` не є `None`.

Всередині циклу `while` призначте `current_node.next` значенню `current_node`.

# --hints--

Ви повинні створити цикл `while`, який перевіряє, чи `current_node.next` не є `None`.
