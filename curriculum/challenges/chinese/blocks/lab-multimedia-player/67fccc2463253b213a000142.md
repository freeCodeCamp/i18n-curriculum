...
In the prior lessons, you were introduced to working with `audio` and `video` elements. 在本实验中，你将构建一个多媒体播放器，它会展示一个 `audio` 曲目和一个带有字幕的 `video`。
...
8. 在 `video` 元素内部，应该有一个 `source` 元素，带有指向视频文件的 `src` 属性和 `type` 属性。 You are free to use this video URL if you like: `https://cdn.freecodecamp.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4`
