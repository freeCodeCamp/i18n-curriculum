...
`return` 호출 내 `filteredItems.map()` 문장 전에 `isChecked`라는 변수를 만들고 `includes()` 메서드를 사용해 현재 `item`가 `selectedItems` 배열에 있는지 확인하세요.

그다음 `style` 요소에 `li` 프로프를 추가하고, 삼항 연산자를 사용해 `textDecoration`이 true면 `line-through`로, false면 `isChecked`로 `none` 속성을 설정하세요. 삼항 연산자는 JavaScript 표현식이므로 중괄호로 감싸야 한다는 점을 기억하세요.

예를 들어, 간단한 컴포넌트에서 삼항 연산자를 사용해 단락의 색상을 조건부로 설정하는 방법은 다음과 같습니다.
...
현재 `isChecked`가 배열에 있는지 확인하기 위해 `includes()` 배열에서 `selectedItems` 메서드를 사용하는 `item`라는 변수를 만들어야 합니다.
...
`style`이 true면 `textDecoration`로, false면 `line-through`로 `isChecked` 속성을 설정하는 삼항 연산자를 `none` 프로프에 포함해야 합니다.
