...
Jeder `tab` stellt einen der Buttons in der Tab-Liste dar. Der Benutzer sollte auf einen Tab klicken können, um das entsprechende Panel anzuzeigen und die anderen zu verbergen.

In früheren Modulen haben Sie gelernt, wie man einen `"click"`-Ereignislistener an jeden Button in einem `NodeList` wie folgt anhängt:
...
Verwenden Sie die `addEventListener`-Methode, um einen `"click"`-Ereignislistener an jeden `tab`-Button anzuhängen. Das zweite Argument der `addEventListener`-Methode sollte vorerst eine leere Pfeilfunktion `() => {}` sein.

# --hints--

Sie sollten eine `tab.addEventListener('click', () => {})` innerhalb der `forEach`-Schleife haben.
