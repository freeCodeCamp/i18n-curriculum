...
Per segnare le caselle di spunta, aggiungi un attributo `checked` alla casella di spunta `input`. Usa il metodo `includes` per verificare se l'attuale `power` è nell'array `powers`.

La `onChange` sarà una funzione separata, impostala su `handlePowersChange` per ora. Questo causerà un errore che correggerai nel passaggio successivo.

# --hints--

Dovresti impostare lo stato `checked` della casella di spunta per riflettere se l'attuale valore `power` esiste nell'array powers.

```js
assert.match(code, /checked\s*=\s*\{\s*powers\s*\.\s*includes\s*\(\s*power\s*\)\s*\}/)
```

Dovresti impostare l'attributo `onChange` della casella di spunta su `{handlePowersChange}`.
