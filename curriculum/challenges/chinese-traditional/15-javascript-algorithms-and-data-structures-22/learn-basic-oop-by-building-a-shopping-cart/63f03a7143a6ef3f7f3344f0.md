...
僅當用戶確認提示時，你才需要清空購物車。 創建一個 `if` 語句來檢查用戶是否確認提示。

在 `if` 語句中，將 `items` 屬性設置回空數組，然後將 `total` 屬性設置爲 `0`。

# --hints--

你應該創建一個 `if` 語句來檢查用戶是否確認了提示。 請記住，你可以直接檢查 `isCartCleared` 的真實性。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

你的 `if` 語句應將 `items` 屬性設置回空數組。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

你的 `if` 語句應將 `total` 屬性設置爲 `0`。
