...
Agora que você salvou a tarefa no array `taskData`, você deve exibir a tarefa na página percorrendo-o.

Use `forEach()` em `taskData`, depois desestruture `id`, `title`, `date`, `description` como os parâmetros.Não retorne nada ainda.

# --hints--

Você deve usar `forEach()` em `taskData`.

```js
assert.match(code, /taskData\.forEach\(\s*/)
```

Você deve usar `{id, title, date, description}` como os parâmetros desestruturados da função callback `forEach()`.
