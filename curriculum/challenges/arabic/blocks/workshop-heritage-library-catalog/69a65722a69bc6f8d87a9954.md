...
لننهي بملخص. سجّل `catalog.length` و `Object.keys(byDecade).length` لرؤية إجمالي الكتب ومجموعات العقود.

أعلن `let oldestYear = Infinity` و `let newestYear = 0`. استخدم حلقة `for` على `catalog`: لكل إدخال حيث السنة ليست `"Unknown"`، حدّث `oldestYear` إذا كانت السنة أصغر و`newestYear` إذا كانت أكبر. سجّل القيمتين.

لقد أكملت ورشة عمل فهرس مكتبة التراث!

# --hints--

يجب أن تسجل `catalog.length` في وحدة التحكم.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*catalog\.length\s*\)/);
```

يجب أن تسجل `Object.keys(byDecade).length` في وحدة التحكم.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*Object\.keys\s*\(\s*byDecade\s*\)\.length\s*\)/);
```

يجب أن تستخدم حلقة `for` على `catalog` لإيجاد أقدم وأحدث السنوات.

```js
assert.match(__helpers.removeJSComments(code), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

أقدم سنة في الفهرس يجب أن تكون `850`.

```js
assert.strictEqual(oldestYear, 850);
```

أحدث سنة في الفهرس يجب أن تكون `2011`.

```js
assert.strictEqual(newestYear, 2011);
```

يجب أن تسجل `oldestYear` في وحدة التحكم.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*oldestYear\s*\)/);
```

يجب أن تسجل `newestYear` في وحدة التحكم.
