...
좋아요! datetime 형식을 연습했으니, 코드 맨 아래에 있는 `current_time` 변수와 출력문을 제거하세요. 다음 단계에서 `Email` 클래스에 타임스탬프를 통합할 예정입니다.

# --hints--

코드에 `current_time = datetime.datetime.now()")` 문장이 없어야 합니다.
...
코드에 `strftime()`를 사용한 연습용 출력문이 없어야 합니다.
