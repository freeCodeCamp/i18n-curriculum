...
Створіть селектор `a` і надайте йому множину властивостей `text-decoration` зі значенням `none` та множину властивостей `color` зі значенням `linen`.

# --hints--

У вас має бути селектор `a`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('a'));
```

Ваш селектор `a` має мати множину властивостей `text-decoration` зі значенням `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration, 'none');
```

Ваш селектор `a` має мати множину властивостей `color` зі значенням `linen`.
