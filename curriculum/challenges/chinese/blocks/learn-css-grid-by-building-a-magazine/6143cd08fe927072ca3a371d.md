...
在你的 `.image-wrapper` 元素中，给第二个 `img` 元素添加一个值为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png` 的 `src`、一个值为 `image of a calculator project` 的 `alt`、一个值为 `lazy` 的 `loading` 属性、一个值为 `image-2` 的 `class`、一个值为 `400` 的 `width` 属性和一个值为 `400` 的 `height` 属性。

# --hints--

你的第一个 `img` 元素应该有一个 `src` 属性，值为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`。
