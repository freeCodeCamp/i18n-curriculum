...
Tengeneza hoja ya utafutaji ya mwisho ya `@media` kwa `only screen` yenye `max-width` ya `420px`. Ndani yake, tengeneza kichaguzi cha `.hero-title` chenye sifa ya `font-size` iliyowekwa kuwa `4.5rem`.

Hongera! Jarida lako sasa limekamilika.

# --hints--

Unapaswa kuwa na hoja mpya ya utafutaji ya `@media` kwa `only screen and (max-width: 420px)`. Hii inapaswa kuwa hoja ya mwisho katika orodha ya hoja za utafutaji za `@media`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

Hoja yako mpya ya utafutaji ya `@media` inapaswa kuwa na kichaguzi cha `.hero-title`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText === '.hero-title');
```

Kichaguzi chako cha `.hero-title` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `4.5rem`.
