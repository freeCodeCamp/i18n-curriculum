...
在 `form` 元素内，添加一个 `label` 元素。你的 `label` 元素应包含文本 `Search for fruits:` 和一个值为 `"search-input"` 的 `htmlFor` 属性。

# --hints--

你应该在 `form` 元素内有一个 `label` 元素。
...
`label` 元素应具有一个 `htmlFor` 属性，其值为 `"search-input"`。
...
你的 `label` 元素应包含文本 `Search for fruits:`。
