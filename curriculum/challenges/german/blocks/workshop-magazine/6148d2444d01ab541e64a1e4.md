...
Erstellen Sie einen `.quote`-Selektor. Geben Sie ihm eine `color`-Eigenschaft mit dem Wert `#00beef`, eine `font-size`-Eigenschaft mit dem Wert `2.4rem` und eine `text-align`-Eigenschaft mit dem Wert `center`.

# --hints--

Sie sollten einen `.quote`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote'));
```

Ihr `.quote`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `#00beef` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.color, 'rgb(0, 190, 239)');
```

Ihr `.quote`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `2.4rem` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize, '2.4rem');
```

Ihr `.quote`-Selektor sollte eine `text-align`-Eigenschaft mit dem Wert `center` haben.
