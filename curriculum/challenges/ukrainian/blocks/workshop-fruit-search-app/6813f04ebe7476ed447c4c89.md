...
Далі викличте `.json()` на відповіді та збережіть результат у змінній з назвою `data`. Оскільки ви все ще всередині функції `async`, слід використовувати `await` перед `response.json()`.

# --hints--

Ви повинні викликати `json()` на `response` і зберегти це у змінній `data`.
