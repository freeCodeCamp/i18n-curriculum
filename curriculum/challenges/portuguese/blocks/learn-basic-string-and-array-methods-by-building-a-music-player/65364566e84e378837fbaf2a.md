...
Nos passos anteriores você desenvolveu a funcionalidade para tocar uma música. Agora você precisa adicionar a funcionalidade ao botão de play para que ele toque a música atual quando for clicado.

Use o método `addEventListener()` e passe um evento `"click"` como primeiro argumento e uma função de callback vazia com sintaxe de arrow como segundo argumento, por exemplo, `() => {}`.

# --hints--

Você deve chamar o método `addEventListener()` na sua variável `playButton`.

```js
assert.match(code, /playButton\.addEventListener\(/)
```

Seu listener de evento deve escutar um evento `"click"`.

```js
assert.match(code, /playButton\.addEventListener\(\s*('|"|`)click\1/)
```

Você deve usar a sintaxe de arrow para passar um callback vazio no seu event listener. Não esqueça que você também precisa de um par vazio de chaves.
