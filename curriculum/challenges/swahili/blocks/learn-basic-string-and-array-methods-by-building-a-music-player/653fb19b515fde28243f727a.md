...
Ili kuonyesha nyimbo katika UI (Muonekano wa Mtumiaji), utahitaji kuunda kitendakazi.

Tumia sintaksia ya kitendakazi cha mshale kuunda kitendakazi cha `renderSongs` kinachopokea `array` kama kigezo chake.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi tupu kinachoitwa `renderSongs`.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs` inapaswa kuwa kitendakazi.

```js
assert.isFunction(renderSongs)
```

Kitendakazi chako cha `renderSongs` kinapaswa kutumia sintaksia ya kitendakazi cha mshale.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

Kitendakazi chako cha `renderSongs` kinapaswa kupokea `array` kama kigezo.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

Kitendakazi chako cha `renderSongs` kinapaswa kuwa tupu.
