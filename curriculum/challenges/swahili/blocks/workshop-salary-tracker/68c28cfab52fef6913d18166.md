...
Sasa ondoa wito la `print` la mwisho wawili.

# --hints--

Haupaswi kuwa na `print(charlie_brown.name)` katika msimbo wako.

```js
({ test: () => assert(runPython(`not _Node(_code).has_stmt("print(charlie_brown.name)")`)) })
```

Haupaswi kuwa na `print(charlie_brown.level)` katika msimbo wako.
