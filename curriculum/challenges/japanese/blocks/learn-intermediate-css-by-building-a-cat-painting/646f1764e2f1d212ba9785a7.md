...
クラスセレクターを使って、`.cat-whisker-left-bottom`を`position`の`absolute`、`top`の`134px`、`left`の`52px`で位置調整してください。

# --hints--

`.cat-whisker-left-bottom`セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom'))
```

`.cat-whisker-left-bottom`セレクターには`position`に設定された`absolute`プロパティが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.position === 'absolute')
```

`.cat-whisker-left-bottom`セレクターには`top`に設定された`134px`プロパティが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.top === '134px')
```

`.cat-whisker-left-bottom`セレクターには`left`に設定された`52px`プロパティが必要です。
