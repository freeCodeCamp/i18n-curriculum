...
عادةً ما يستخدم برنامج الجداول الحسابية `=` في بداية الخلية للدلالة على وجوب إجراء عملية حسابية، ويجب تقييم دوال الجداول الحسابية.

استخدم العامل `&&` لإضافة شرط ثاني إلى جملة `if` الخاصة بك يتحقق أيضًا مما إذا كان الحرف الأول من `value` هو `"="`.

# --hints--

يجب استخدام العامل `&&` لإضافة شرط ثاني إلى جملة `if` الخاصة بك يتحقق أيضًا مما إذا كان الحرف الأول من `value` هو `"="`. يمكنك استخدام `[0]` أو `.startsWith()` أو `.charAt(0)`.
