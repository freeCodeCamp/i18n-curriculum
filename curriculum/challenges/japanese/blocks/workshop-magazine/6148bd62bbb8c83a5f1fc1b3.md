...
`.hero-title` セレクターを作成し、`text-align` プロパティを `center` に、`color` プロパティを `orangered` に、`font-size` プロパティを `8rem` に設定してください。

# --hints--

`.hero-title` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

`.hero-title` セレクターには `text-align` プロパティが `center` に設定されているはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign, 'center');
```

`.hero-title` セレクターには `color` プロパティが `orangered` に設定されているはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color, 'orangered');
```

`.hero-title` セレクターには `font-size` プロパティが `8rem` に設定されているはずです。
