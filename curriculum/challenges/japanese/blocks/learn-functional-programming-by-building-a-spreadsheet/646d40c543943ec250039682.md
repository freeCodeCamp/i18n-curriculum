...
掛け算または割り算にマッチする正規表現ができたので、その式を評価できます。

`str2` 変数を宣言し、`infixEval` を `str` と `regex` を引数として呼び出した結果を代入してください。
...
`regex` を `infixEval` 呼び出しの2番目の引数として渡してください。
