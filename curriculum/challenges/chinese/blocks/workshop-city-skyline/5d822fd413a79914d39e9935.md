...
为这座建筑添加另一个重复渐变；使其与你刚刚添加的相同，但不要添加 `90deg` 方向，而且是使用其窗口颜色而不是两个 `transparent` 颜色。

# --hints--

你应该在 `background` 属性中为 `.fb6` 添加第二个 `repeating-linear-gradient`。
...
你应该将从 `0%` 到 `10%` 的 `--building-color3` 作为第二个 `repeating-linear-gradient` 的第一种颜色。
...
你应该将从 `10%` 到 `30%` 的 `--window-color3` 作为第二个 `repeating-linear-gradient` 的第二种颜色。
