...
Um caractere de palavra é definido como qualquer letra, de `a` a `z` ou `A` a `Z`, ou um número de `0` a `9`, ou o caractere underscore (`_`).

A inclusão do underscore pode parecer estranha, mas considere as convenções de nomenclatura para variáveis: nomes de variáveis frequentemente incluem underscores, então `\w` foi criado para combinar com isso também.
...
Você também pode misturar letras e números na sua classe de caracteres. Por exemplo, se quisesse o comportamento da classe `\w` sem o underscore, poderia criar a sua própria:
...
A lição menciona uma classe "curinga" representada por um símbolo específico.
