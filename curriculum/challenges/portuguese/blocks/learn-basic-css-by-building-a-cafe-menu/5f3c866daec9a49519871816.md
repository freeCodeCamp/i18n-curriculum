...
Elementos `article` comumente contêm vários elementos que possuem informações relacionadas. Neste caso, ele vai conter um sabor de café e um preço por esse sabor.

Coloque dois elementos `p` dentro do elemento `article`. O primeiro texto deve ser `French Vanilla` e o segundo texto deve ser `3.00`.
