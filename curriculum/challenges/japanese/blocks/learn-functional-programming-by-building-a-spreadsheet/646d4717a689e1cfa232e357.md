...
`highPrecedence()` 関数で行ったように、`evalFormula()` 関数もすべて評価されて置き換えられていることを確認する必要があります。

三項演算子を使って、`functionExpanded` が元の文字列 `x` と等しいかどうかをチェックしてください。等しければ `functionExpanded` を返し、そうでなければ `evalFormula()` を `functionExpanded` と `cells` を引数として再度呼び出した結果を返してください。

# --hints--

`evalFormula` 関数は `return` キーワードを使うべきです。
