...
Функція `getAverageRating` має повернути число.

```js
assert.isNumber(getAverageRating(recipe1.ratings))
```

`getAverageRating(recipe1.ratings)` має повернути `4.5`.

```js
assert.strictEqual(getAverageRating(recipe1.ratings), 4.5)
```

`getAverageRating(recipe2.ratings)` має повернути `4.75`.

```js
assert.strictEqual(getAverageRating(recipe2.ratings), 4.75)
```

`getAverageRating(recipe3.ratings)` має повернути `4`.
