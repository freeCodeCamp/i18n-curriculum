...
É hora de começar a implementar a funcionalidade para reproduzir as músicas exibidas.

Defina uma função `playSong` usando `const`. A função deve receber um parâmetro `id` que representará o identificador único da música que você deseja reproduzir. 

# --hints--

Você deve usar `const` para criar uma função vazia chamada `playSong`.

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

`playSong` deve ser uma função.

```js
assert.isFunction(playSong)
```

Sua função `playSong` deve usar uma sintaxe de arrow.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Sua função `playSong` deve receber um parâmetro `id`.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

Sua função `playSong` deve estar vazia.
