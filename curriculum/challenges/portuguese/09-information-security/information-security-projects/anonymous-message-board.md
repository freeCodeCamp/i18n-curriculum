...
You can send a DELETE request to `/api/replies/{board}` and pass along the `thread_id`, `reply_id`, & `delete_password`. O conteúdo retornado será a string `incorrect password` ou `success`. On success, the text of the `reply_id` will be changed to `[deleted]`.
...
You can send a PUT request to `/api/replies/{board}` and pass along the `thread_id` & `reply_id`. O conteúdo retornado será a string `reported`. The `reported` value of the `reply_id` will be changed to `true`.
