...
`parseCard` è stato testato e funziona correttamente. La variabile `cardResult` non è più necessaria.

Inizia eliminando la riga `const cardResult` e la riga `console.log(cardResult)`. Poi crea una funzione chiamata `parseCatalog` che prende `rawCards` come parametro. Per ora, dichiara un array vuoto chiamato `catalog` all'interno della funzione e restituiscilo.

Sotto la funzione, chiama `parseCatalog(rawCatalogCards)`, assegna il risultato a una variabile chiamata `catalog` e stampa `catalog.length`.

# --hints--

`parseCatalog` dovrebbe essere una funzione che restituisce un array.

```js
assert.isArray(parseCatalog(rawCatalogCards));
```

`catalog` dovrebbe essere definito e impostato su `parseCatalog(rawCatalogCards)`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+catalog\s*=\s*parseCatalog\s*\(/);
```

Dovresti eliminare la variabile `cardResult` e la relativa chiamata `console.log()`.

```js
assert.notMatch(__helpers.removeJSComments(code), /\bcardResult\b/);
```

Dovresti stampare `catalog.length` sulla console.
