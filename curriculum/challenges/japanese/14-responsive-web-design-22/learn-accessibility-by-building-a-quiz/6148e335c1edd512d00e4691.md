...
ナビゲーションのリンクをクリックすると、関連するセクションへ表示領域がジャンプするはずです。 ただし、このジャンプが一部のユーザーを混乱させることがあります。

すべての要素を選択し、`scroll-behavior` の値を `smooth` に設定してください。
...
`*` の `scroll-behavior` を `smooth` に設定する必要があります。
