...
Дайте новим `div` елементам ці `width` і `height` значення: `3%` і `10%` до `.bb4a`, `80%` і `5%` до `.bb4b`, а також `100%` і `85%` до `.bb4c`.

# --hints--

Ви повинні надати `.bb4a` значення `width` рівне `3%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.width, "3%");
```

Ви повинні надати `.bb4a` значення `height` рівне `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.height, "10%");
```

Ви повинні надати `.bb4b` значення `width` рівне `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.width, "80%");
```

Ви повинні надати `.bb4b` значення `height` рівне `5%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.height, "5%");
```

Ви повинні надати `.bb4c` значення `width` рівне `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4c")?.width, "100%");
```

Ви повинні надати `.bb4c` значення `height` рівне `85%`.
