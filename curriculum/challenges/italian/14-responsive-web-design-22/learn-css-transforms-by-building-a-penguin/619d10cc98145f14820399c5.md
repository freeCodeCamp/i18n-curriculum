...
Dovresti dare al primo nuovo `div` un valore `class` di `beak`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

Dovresti dare al secondo nuovo `div` un valore `class` di `beak`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div.beak')?.length, 2);
```

Dovresti dare al primo nuovo `div` un valore `class` di `top`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

Dovresti dare al secondo nuovo `div` un valore `class` di `bottom`.
