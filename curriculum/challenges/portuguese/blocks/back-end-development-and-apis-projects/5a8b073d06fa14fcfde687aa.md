...
Você pode adicionar os parâmetros `from`, `to` e `limit` a uma requisição `GET /api/users/:_id/logs` para recuperar parte do log de qualquer usuário. `from` e `to` são datas no formato `yyyy-mm-dd`. `limit` é um inteiro que indica quantos logs devem ser retornados.
