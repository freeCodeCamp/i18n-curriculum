...
`allSongs` 配列の中に、次のプロパティと値を持つオブジェクトを作成してください。
...
あなたの `allSongs` 配列には1つの値があるはずです。

```js
assert.equal(allSongs.length, 1);
```

あなたの `allSongs` 配列の最初の値はオブジェクトであるべきです。

```js
assert.isObject(allSongs[0]);
```

あなたの `allSongs` 配列には、`id` プロパティが数値 `0` に設定されたオブジェクトがあるはずです。

```js
assert.strictEqual(allSongs[0].id, 0);
```

あなたの `allSongs` 配列には、`title` プロパティが文字列 `"Scratching The Surface"` に設定されたオブジェクトがあるはずです。

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

あなたの `allSongs` 配列には、`artist` プロパティが文字列 `"Quincy Larson"` に設定されたオブジェクトがあるはずです。

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

あなたの `allSongs` 配列には、`duration` プロパティが文字列 `"4:25"` に設定されたオブジェクトがあるはずです。

```js
assert.equal(allSongs[0].duration, "4:25");
```

あなたの `allSongs` 配列には、`src` プロパティが文字列 `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"` に設定されたオブジェクトがあるはずです。
