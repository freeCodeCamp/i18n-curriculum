...
Dai alla classe `bb4-window` una `width` di `18%`, una `height` di `90%` e aggiungi la tua variabile `--window-color4` come `background-color`.

# --hints--

Dovresti dare a `.bb4-window` una `width` di `18%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

Dovresti dare a `.bb4-window` una `height` di `90%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

Dovresti dare a `.bb4-window` una `background-color` di `--window-color4`.
