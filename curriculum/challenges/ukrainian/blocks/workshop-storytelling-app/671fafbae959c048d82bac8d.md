...
Змініть слухача події `scaryStoryBtn`, щоб він викликав функцію `displayStory` з аргументом `"scary"`. Також додайте слухача події `click` до кнопок `funnyStoryBtn` і `adventureStoryBtn`, які викликатимуть функцію `displayStory` з аргументами `"funny"` і `"adventure"` відповідно.

З цим ви успішно завершили практичне заняття.

# --hints--

Коли ви натиснете на `scaryStoryBtn`, абзац `#result` має відобразити страшну історію `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` з кольором кордону `#ee4b2b`.
...
Коли ви натиснете на `funnyStoryBtn`, абзац `#result` має відобразити кумедну історію з кольором кордону `#f1be32`.
...
Коли ви натиснете на `adventureStoryBtn`, абзац `#result` має відобразити пригодницьку історію з кольором кордону `#acd157`.
