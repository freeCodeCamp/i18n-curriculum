...
Mara tu unapofuta `multipleCard`, ndani ya njia ya `singleCardSelected`, fanya wito wa kitendakazi cha `showElements` na pita `singleCard` kama hoja ya kwanza na `fortuneContainer` kama hoja ya pili.

# --hints--

Unapaswa kufanya wito wa kitendakazi cha `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

Unapaswa kupita `elements.singleCard` kama hoja ya kwanza.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

Unapaswa kupita `elements.fortuneContainer` kama hoja ya pili.
