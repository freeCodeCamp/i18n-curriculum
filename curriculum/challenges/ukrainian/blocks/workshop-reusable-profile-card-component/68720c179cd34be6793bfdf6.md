...
Хоч ваш застосунок працює як очікувалося, зазвичай ви не додавали б кілька `Card` компонентів вручну таким чином.

Видаліть три `<Card />` компоненти всередині `<div className="flex-container">` елемента. У наступних кількох кроках ви створите масив даних і використаєте `map()` функцію, щоб відтворити `Card` компоненти — це більш поширений підхід у React.

# --hints--

Вам слід видалити три `<Card />` компоненти з вашого коду.
