...
A complexidade de tempo do merge sort é `O(n log n)` porque a lista é continuamente dividida ao meio (`log n`) e depois mesclada (`O(n)`). Diferente de outros algoritmos de ordenação como o bubble sort, o merge sort não é um algoritmo de ordenação in-place e tem uma complexidade de espaço de `O(n)`.
...
Qual é a complexidade de tempo do algoritmo merge sort?
