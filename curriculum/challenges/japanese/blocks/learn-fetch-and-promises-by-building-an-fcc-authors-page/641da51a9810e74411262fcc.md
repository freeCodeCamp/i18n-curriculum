...
2番目の `.then()` メソッドに `data` パラメータを追加してください。
...
`.catch()` メソッドに `err` パラメータを追加してください。
...
`.catch()` メソッドは `console.error()` を使い、テキスト `` `There was an error: ${err}` `` とともに `err` をコンソールに記録してください。バッククォートを使うのを忘れないでください。
