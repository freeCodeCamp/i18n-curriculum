...
title: ¿Cómo funciona el constructor `Audio`? ¿Cuáles son algunos métodos comunes?
challengeType: 19
...
Cuando llamas al constructor, puedes opcionalmente pasar una URL como el (único) argumento. Esta URL debe apuntar a la fuente del archivo de audio que deseas reproducir. O, si necesitas cambiar la fuente dinámicamente, puedes asignar la URL a la propiedad `src` del elemento de audio devuelto.
...
Finalmente, el método `canPlayType()` puede usarse para determinar si un navegador probablemente podrá reproducir tu formato de audio específico. Aprenderás sobre formatos de audio y video en la siguiente lección.
...
La lección menciona las dos cosas que necesitas hacer para pausar y reiniciar el audio desde el principio.
