...
もし `value` の `numberInput` が `5` と等しい場合、`showAnimation()` 関数を呼び出してから、`return` で早期に終了してください。

# --hints--

2つ目の `showAnimation()` 文の本体内で `if` 関数を呼び出す必要があります。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

`showAnimation()` 関数を呼び出した後、`return` 関数を早期に終了するために `checkUserInput()` キーワードを使うべきです。
