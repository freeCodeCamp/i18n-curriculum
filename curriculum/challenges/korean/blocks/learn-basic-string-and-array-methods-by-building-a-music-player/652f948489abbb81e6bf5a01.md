...
이 프로젝트에서는 음악 플레이어 앱을 만들면서 기본 문자열과 배열 메서드를 배우게 됩니다. 노래를 재생, 일시정지, 건너뛰기, 셔플할 수 있습니다.

이 프로젝트의 HTML과 CSS는 제공되었으니 JavaScript에 포커스를 맞추세요.

`#playlist-songs` 메서드를 사용해 `#play`, `#pause`, `getElementById()` 요소에 접근하는 것부터 시작하세요. 각각을 변수 `playlistSongs`, `playButton`, `pauseButton`에 할당하세요.


# --hints--

`playlistSongs`라는 변수가 있어야 합니다. 

```js
assert.isDefined(playlistSongs);
```

`document.getElementById()`를 사용해 `#playlist-songs` 요소를 가져와야 합니다.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

`#playlist-songs` 요소를 변수 `playlistSongs`에 할당해야 합니다. 

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

`playButton`라는 변수가 있어야 합니다. 

```js
assert.isDefined(playButton);
```

`document.getElementById()`를 사용해 `#play` 요소를 가져와야 합니다.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

`#play` 요소를 변수 `playButton`에 할당해야 합니다. 

```js
assert.equal(playButton, document.getElementById('play'));
```

`pauseButton`라는 변수가 있어야 합니다. 

```js
assert.isDefined(pauseButton);
```

`document.getElementById()`를 사용해 `#pause` 요소를 가져와야 합니다.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

`#pause` 요소를 변수 `pauseButton`에 할당해야 합니다. 
