...
`However` is used to introduce a contrast or different idea. 例如：
...
`Bonuses` are extra payments given to employees as rewards for good work. 例如：

`The company gives yearly bonuses based on performance.` - This means employees receive extra money for doing well.

`Stock` refers to shares in a company that employees can own. 例如：
