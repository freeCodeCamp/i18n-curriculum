...
Finalmente, dê ao seu `conversionTable` um objeto `teaspoon` com uma propriedade `cup` definida como `1/48`, uma propriedade `gram` definida como `5` e uma propriedade `ounce` definida como `0.167`.

# --hints--

Sua `conversionTable` deve ter uma propriedade `teaspoon`.

```js
assert.property(conversionTable, "teaspoon");
```

Sua propriedade `teaspoon` deve ser um objeto.

```js
assert.isObject(conversionTable.teaspoon);
```

Seu objeto `teaspoon` deve ter uma propriedade `cup`.

```js
assert.property(conversionTable.teaspoon, "cup");
```

A sua propriedade `cup` deve ser definida como `1/48`.

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

Seu objeto `teaspoon` deve ter uma propriedade `gram`.

```js
assert.property(conversionTable.teaspoon, "gram");
```

Sua propriedade `gram` deve ser configurada para `5`.

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

Seu objeto `teaspoon` deve ter uma propriedade `ounce`.

```js
assert.property(conversionTable.teaspoon, "ounce");
```

A sua propriedade `ounce` deve ser definida como `0.167`.
