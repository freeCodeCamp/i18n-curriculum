...
버튼을 만들었으니 이제 텍스트를 할당해야 합니다. 이를 위해 DOM의 `createTextNode()` 메서드를 사용하세요.

`createTextNode()` 메서드는 텍스트 노드를 만들 때 사용합니다. 호출할 때 문자열로 텍스트를 전달하세요:
...
`createTextNode()` 메서드를 사용해 `"Reset Playlist"` 텍스트를 만들고, `resetText` 상수에 할당하세요.
...
`document.createTextNode()`를 사용해 `"Reset Playlist"` 텍스트를 만들어야 합니다.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

만든 텍스트 노드를 `resetText` 상수에 할당하세요.
