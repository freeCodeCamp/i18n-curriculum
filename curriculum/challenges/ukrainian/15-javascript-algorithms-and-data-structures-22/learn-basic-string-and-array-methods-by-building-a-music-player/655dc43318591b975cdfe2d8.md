...
Масив `allSongs` повинен мати об’єкт з властивістю `id` зі значенням числа `0`.

```js
assert.equal(allSongs[0].id, 0);
```

Масив `allSongs` повинен мати об’єкт з властивістю `title` зі значенням рядка `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Масив `allSongs` повинен мати об’єкт з властивістю `artist` зі значенням рядка `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Масив `allSongs` повинен мати об’єкт з властивістю `duration` зі значенням рядка `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Масив `allSongs` повинен мати об’єкт з властивістю `src` зі значенням рядка `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
