...
Katika hatua hii, unganisha msikilizaji wa tukio la `submit` kwa `recipeForm` ili wakati kitufe cha `Convert` kinapobofyiwa, kitendakazi cha `updateResultsList()` kiitwe.

Unganisha msikilizaji wa tukio la `submit` kwa `recipeForm`. Wakati fomu inawasilishwa, ita wito `event.preventDefault()` kuzuia tabia ya kawaida, kisha ita wito `updateResultsList()`.

Hii itakuwezesha kuanzisha kitendakazi kupitia kiolesura kwa kujaza fomu na kubonyeza `Convert`, muhimu kwa kurekebisha makosa wakati wa kuongeza mantiki ya uongofu katika hatua inayofuata.

# --hints--

Fanya kitufe cha `Convert` kinapobofyiwa kiendeshe `updateResultsList()` kwa kuongeza msikilizaji wa tukio la `"submit"` kwa `recipeForm` unaoitwa `event.preventDefault()` kuzuia tabia ya kawaida ya fomu.
