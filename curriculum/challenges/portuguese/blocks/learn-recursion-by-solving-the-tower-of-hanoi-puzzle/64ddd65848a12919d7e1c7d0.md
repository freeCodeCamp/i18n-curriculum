...
É hora de mover algum código da função `move()` para a função `make_allowed_move()`. Mova o código aninhado dentro da primeira declaração `if` (exceto a primeira chamada `print()`) para sua nova função. Preste muita atenção à indentação. Não esqueça de remover a palavra-chave `pass`.

# --hints--

Você deve mover o código aninhado dentro da primeira declaração `if` (exceto a primeira chamada `print()`) para sua nova função.
