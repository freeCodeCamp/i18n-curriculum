...
Тепер створіть змінні стану та сеттери для `powerSource` і `powers`. `powerSource` має мати ініціальне значення порожнього рядка, а `powers` — ініціальне значення порожнього масиву.

# --hints--

Ви повинні використати деструктуризацію масиву, щоб задати змінну стану `powerSource` та сеттер `setPowerSource`.

```js
assert.match(code, /(const|let|var)\s+\[\s*powerSource\s*,\s*setPowerSource\s*\]/);
```

Ваші `powerSource` і `setPowerSource` мають використовувати хук `useState`.
...
Ваш хук `useState` для `powerSource` має мати ініціальне значення порожнього рядка.
...
Ви повинні використати деструктуризацію масиву, щоб задати змінну стану `powers` та сеттер `setPowers`.

```js
assert.match(code, /(const|let|var)\s+\[\s*powers\s*,\s*setPowers\s*\]/);
```

Ваші `powers` і `setPowers` мають використовувати хук `useState`.
...
Ваш хук `useState` для `powers` має мати ініціальне значення порожнього масиву.
