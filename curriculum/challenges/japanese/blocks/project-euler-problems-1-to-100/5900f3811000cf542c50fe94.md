...
d(`n`) を `n` の真の約数の和（`n` 未満で `n` を割り切る数）と定義します。

もし d(`a`) = `b` かつ d(`b`) = `a` であり、`a` ≠ `b` ならば、`a` と `b` は友愛数の組であり、`a` と `b` はそれぞれ友愛数と呼ばれます。

例えば、220 の真の約数は 1、2、4、5、10、11、20、22、44、55、110 なので、d(220) = 284 です。284 の真の約数は 1、2、4、71、142 なので、d(284) = 220 となります。

`n` 未満のすべての友愛数の和を求めてください。
