...
Gib den neuen `div` Elementen folgende `width` und `height` Werte: `3%` und `10%` für `.bb4a`, `80%` und `5%` für `.bb4b`, und `100%` und `85%` für `.bb4c`.

# --hints--

Du solltest `.bb4a` eine `width` von `3%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.width, "3%");
```

Du solltest `.bb4a` eine `height` von `10%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.height, "10%");
```

Du solltest `.bb4b` eine `width` von `80%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.width, "80%");
```

Du solltest `.bb4b` eine `height` von `5%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.height, "5%");
```

Du solltest `.bb4c` eine `width` von `100%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4c")?.width, "100%");
```

Du solltest `.bb4c` eine `height` von `85%` geben.
