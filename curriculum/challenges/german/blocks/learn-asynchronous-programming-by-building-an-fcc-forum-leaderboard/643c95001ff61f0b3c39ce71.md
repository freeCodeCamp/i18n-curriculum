...
Innerhalb der `map`-Methode destrukturieren Sie die folgenden Eigenschaften aus dem `item`-Objekt:
...
Sie sollten die Eigenschaften `id`, `title`, `views`, `posts_count`, `slug`, `posters`, `category_id`, `bumped_at` aus dem `item`-Objekt destrukturieren.
