...
بعد آخر `.divider` لديك، أنشئ عنصر `p` جديدًا بالنص `Cholesterol 0mg 0%`. ثم غلف النص `Cholesterol` في عنصر `span`، و`0%` في عنصر آخر، وأعط كلًا منهما صنفًا `bold`.
...
يجب أن يحتوي عنصر `span` الخاص بـ `Cholesterol` على الخاصية `class` مضبوطة على `bold`.
...
يجب أن يحتوي عنصر `span` الخاص بـ `0%` على الخاصية `class` مضبوطة على `bold`.
...
يجب أن يُغلف عنصر `span` الخاص بـ `Cholesterol` مع نص `0mg` داخل عنصر `span`.
