...
Unter Ihrem letzten `p`-Element erstellen Sie ein weiteres `p`-Element mit dem Text `Sodium 160mg 7%`. Setzen Sie `Sodium` und `7%` jeweils in ein eigenes `span` mit einer Klasse von `bold`, so wie Sie es bei den anderen gemacht haben.

Fügen Sie dann erneut ein zusätzliches `span`-Element um `Sodium 160mg` für die Ausrichtung hinzu.
...
Ihr neues `p`-Element sollte den Text `Sodium 160mg 7%` enthalten.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText?.match(/Sodium[\s|\n]160mg[\s|\n]+7%/));
```

Ihr neues `p`-Element sollte drei `span`-Elemente enthalten.
...
Das `span` um `Sodium` sollte das `class`-Attribut mit dem Wert `bold` haben.
...
Das `span`-Element um `7%` sollte das `class`-Attribut mit dem Wert `bold` haben.
