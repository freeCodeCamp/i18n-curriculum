...
У циклі перевірте, чи містить автор поточного запису (перетворений на нижній регістр) `searchTerm`. Метод `.includes()` повертає `true`, якщо рядок містить інший рядок як підрядок. Якщо умова істинна, додайте запис у `results`. Після циклу поверніть `results`.

# --hints--

`findByAuthor(catalog, "king").length` має бути `6`.

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

`findByAuthor(catalog, "asimov").length` має бути `2`.

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

`findByAuthor(catalog, "xyz").length` має бути `0`.
