...
在 JavaScript 中，所有算術運算符都有一種複合賦值形式。 Compound assignment operators provide a concise shorthand for an operation on a variable followed by storing the result in that same variable. They combine the operation and assignment into a shorter form like `x += y`, which is equivalent to writing `x = x + y` but without repeating the variable name. 相較於這樣寫：
