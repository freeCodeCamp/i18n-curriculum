...
Dovresti dare a `.shirt div` un valore `top` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt div')?.top, '22.5px');
```

Dovresti dare a `.shirt div` un valore `left` di `--fcc-expected--` invece di `--fcc-actual--`.
