...
點 ${P}(x_1, y_1)$ 和 ${Q}(x_2, y_2)$ 繪製於整數座標，並與原點 ${O}(0, 0)$ 相連，形成 ${\Delta}OPQ$。
...
當每個座標介於 0 和 2（含）之間時，恰好可以形成十四個包含直角的三角形；也就是說，$0 ≤ x_1, y_1, x_2, y_2 ≤ 2$。
...
假設 $0 ≤ x_1, y_1, x_2, y_2 ≤ limit$，可以形成多少個直角三角形？

# --hints--

`rightTrianglesIntCoords(2)` 應該傳回一個數字。

```js
assert(typeof rightTrianglesIntCoords(2) === 'number');
```

`rightTrianglesIntCoords(2)` 應該傳回 `14`。

```js
assert.strictEqual(rightTrianglesIntCoords(2), 14);
```

`rightTrianglesIntCoords(10)` 應該傳回 `448`。

```js
assert.strictEqual(rightTrianglesIntCoords(10), 448);
```

`rightTrianglesIntCoords(25)` 應該傳回 `3207`。

```js
assert.strictEqual(rightTrianglesIntCoords(25), 3207);
```

`rightTrianglesIntCoords(50)` 應該傳回 `14234`。
