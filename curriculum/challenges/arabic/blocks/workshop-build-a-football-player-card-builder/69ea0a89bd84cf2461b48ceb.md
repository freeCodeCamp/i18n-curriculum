...
داخل كتلة `try`، احفظ بيانات اللاعب الحالية في `localStorage` باستدعاء `localStorage.setItem` مع مفتاح التخزين والكائن player بعد تحويله إلى سلسلة نصية.

وفي كتلة `catch`، سجّل السلسلة النصية `Failed to save player data:` متبوعة بالمعلمة `error` في وحدة التحكم.

# --hints--

يجب أن تستدعي كتلة try في `useEffect` الدالة `localStorage.setItem` مع `STORAGE_KEY` والكائن `player` بعد تحويله إلى سلسلة نصية.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

يجب أن تسجل كتلة catch في `useEffect` السلسلة `'Failed to save player data:'` والمعلمة `error` في وحدة التحكم.
