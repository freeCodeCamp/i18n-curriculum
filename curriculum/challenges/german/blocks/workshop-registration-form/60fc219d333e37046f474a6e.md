...
Wähle nur die `.inline`-Elemente aus und gib ihnen jeweils eine `width`-Eigenschaft mit dem Wert `unset`. Dies wird die vorherige Regel entfernen, die alle `input`-Elemente auf `width: 100%` gesetzt hat.

# --hints--

Du solltest den `.inline`-Selektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.inline'));
```

Du solltest den `.inline`-Elementen jeweils eine `width`-Eigenschaft mit dem Wert `unset` zuweisen.
