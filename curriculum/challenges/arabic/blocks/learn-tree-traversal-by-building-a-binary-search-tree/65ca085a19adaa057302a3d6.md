...
داخل جسم حلقة `while`، استبدل `pass` بـ `node` وقم بتعيينها إلى الابن الأيسر للعقدة.

# --hints--

يجب إزالة الكلمة المفتاحية `pass` من حلقة `while`.
...
يجب تعيين `node.left` إلى `node` داخل حلقة `while`.
