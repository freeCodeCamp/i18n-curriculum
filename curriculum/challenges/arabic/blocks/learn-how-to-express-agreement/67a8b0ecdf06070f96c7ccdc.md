---
id: 67a8b0ecdf06070f96c7ccdc
title: المهمة 33
challengeType: 22
dashedName: task-33
lang: en-US
---

<!-- (Audio) Linda: I'll also keep you updated on the design process, and we can schedule regular meetings to stay on track. -->

# --instructions--

استمع إلى الصوت وأكمل الجملة أدناه.

# --fillInTheBlank--

## --sentence--

`I'll also BLANK BLANK BLANK on the design process, and we can schedule regular meetings to stay on track.`

## --blanks--

`keep`

### --feedback--

تعني هذه الكلمة الاستمرار أو المحافظة على شيء ما.

---

`you`

### --feedback--

هذا ضمير يشير إلى الشخص المخاطب.

---

`updated`

### --feedback--

تعني هذه الكلمة تقديم أحدث المعلومات أو الأخبار.

# --explanation--

تعني عبارة `Keep you updated` الاستمرار في تقديم أحدث المعلومات أو الحالة حول شيء ما. على سبيل المثال:

تعني الجملة `I'll keep you updated on the project status.` أنك ستُعلمهم بانتظام عن تقدم المشروع أو التغييرات التي تطرأ عليه.

# --scene--

```json
{
  "setup": {
    "background": "company2-boardroom.png",
    "characters": [
      {
        "character": "Linda",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_10-1.mp3",
      "startTime": 1,
      "startTimestamp": 37.44,
      "finishTimestamp": 42.14
    }
  },
  "commands": [
    {
      "character": "Linda",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Linda",
      "startTime": 1,
      "finishTime": 5.7,
      "dialogue": {
        "text": "I'll also keep you updated on the design process and we can schedule regular meetings to stay on track.",
        "align": "center"
      }
    },
    {
      "character": "Linda",
      "opacity": 0,
      "startTime": 6.2
    }
  ]
}
```
