...
1. 你的函数必须始终返回整个 `records` 对象。
2. 如果 `value` 是空字符串，从专辑里删除指定的 `prop`。
3. 如果 `prop` 不是 `tracks`，并且 `value` 不是一个空字符串，将 `value` 赋给那个专辑的 `prop`。
4. 如果 `prop` 是 `tracks` 并且 `value` 不是空字符串，但是专辑没有 `tracks` 属性，为该属性创建一个空数组并添加 `value` 作为其元素。
5. 如果 prop 是 `tracks` 并且 `value` 不是一个空字符串，将 `value` 添加到专辑现有 `tracks` 数组的末尾。

**注意：** 测试中使用了 `recordCollection` 对象的拷贝。你的函数不应直接参考 `recordCollection` 对象，只能使用函数参数。
