...
现在你的 `applyFunction` 需要返回一个结果。 返回对 `str2` 调用 `.replace()` 方法的结果。 传递你的 `functionCall` 正则表达式和一个空的回调。

# --hints--

你的 `applyFunction` 函数应该返回在 `str2` 上调用 `.replace()` 方法的结果。
...
你应该将 `functionCall` 作为第一个参数传递给 `.replace()` 调用。
...
你应该将一个空的箭头函数作为第二个参数传递给 `.replace()` 调用。
