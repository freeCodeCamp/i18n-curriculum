...
你的美元金額目前未對齊。創建一個選擇器以標的你的 `tr.data` 元素內的 `td` 元素。將文字垂直對齊至頂部，水平對齊至右側，並將內距設為 `0.3rem 0.25rem 0`。

# --hints--

你應該有一個 `tr.data td` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td'));
```

你的 `tr.data td` 選擇器應該有一個設定為 `top` 的 `vertical-align` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td')?.getPropertyValue('vertical-align') === 'top');
```

你的 `tr.data td` 選擇器應該有一個 `text-align` 屬性設定為 `right`。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td')?.getPropertyValue('text-align') === 'right');
```

你的 `tr.data td` 選擇器應該設定 `padding` 屬性為 `0.3rem 0.25rem 0`。
