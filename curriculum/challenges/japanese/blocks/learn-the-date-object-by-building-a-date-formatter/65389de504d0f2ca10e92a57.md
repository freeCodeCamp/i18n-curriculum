...
ユーザーがドロップダウンから `Month, Day, Year, Hours, Minutes` のオプションを選んだ場合は、`mm-dd-yyyy h Hours m Minutes` の形式で日付を表示する必要があります。

`mm-dd-yyyy-h-mm` の `case` では、文字列補間を使って上記の形式に整形した日付を、`currentDateParagraph` の `textContent` プロパティに代入してください。 回答には変数 `month`、`day`、`year`、`hours`、`minutes` を使用してください。
