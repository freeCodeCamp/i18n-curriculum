...
すでに学んだように、`delete` メソッドはマップからエントリーを削除するために使われます。セットには、要素を削除するために使える `delete` メソッドがあります。

時には、マップやセットからすべての要素を削除する必要がある場合があります。`clear` メソッドはまさにその役割を果たします。

`clear` に対して `plantsSet` メソッドを呼び出して、すべての要素を削除してください。

# --hints--

`clear` に対して `plantsSet` メソッドを呼び出す必要があります。
