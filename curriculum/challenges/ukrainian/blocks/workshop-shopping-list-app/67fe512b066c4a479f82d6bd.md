...
У межах елемента `li`, над текстом `item`, додайте елемент `input` з атрибутом `type`, встановленим у `checkbox`. Також встановіть його атрибут `onChange` як анонімну функцію, яка викликає `toggleItem()` з `item` як аргументом.

Нарешті, обгорніть текст `input` і `item` у елемент `label` для кращої доступності.

# --hints--

Ви повинні додати ще один елемент `input` з атрибутом `type`, встановленим у `checkbox`.
...
Ваш новий `input` має мати атрибут `onChange`, встановлений як анонімна функція, що викликає `toggleItem()` з `item` як аргументом.
...
Ваш новий `input` і текст `item` мають бути обгорнуті в елемент `label`.
