...
爲 `"midfielder"` 添加一個新的 `case`，按照與上一步相同的模式檢查 `player.position` 是否等於 `"midfielder"`。

# --hints--

你應該在 `switch` 語句中爲 `"midfielder"` 添加一個新的 `case`。

```js
assert.match(code, /\s*case\s*('|"|`)\s*midfielder\s*\1\s*:\s*/)
```

在你的 `"midfielder"` `case` 中，你應該調用 `setPlayerCards` 函數。
...
你的 `filter()` 回調應該隱式返回檢查 `player.position` 是否爲 `"midfielder"` 的結果。
