...
Sería bueno probar tu función `getBooksByAuthor` con otro autor.
...
Debes registrar el mensaje `"\nList of books by James Clear:\n"` en la consola.

```js
assert.match(code, /console\.log\(("|')\\nList\s+of\s+books\s+by\s+James\s+Clear:\\n\1\);?/);
```

Deberías tener un `console.log(getBooksByAuthor(library, "James Clear"))`.
