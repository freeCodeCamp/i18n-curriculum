...
在第一个 `tr` 中，添加一个带有文本 `Loans The outstanding balance on our startup loan.` 的 `th` 元素。将该文本中除 `Loans ` 之外的部分包裹在一个 `class` 设置为 `description` 的 `span` 元素内。

在其下方添加三个 `td` 元素，并按顺序为它们提供以下文本：`$500`、`$250` 和 `$0`。 给第三个 `td` 添加一个 `class`，设置为 `current`。
...
你的 `th` 元素应包含文本 `Loans The outstanding balance on our startup loan.`。
...
你应该将文本 `The outstanding balance on our startup loan.` 包裹在 `span` 元素中。
