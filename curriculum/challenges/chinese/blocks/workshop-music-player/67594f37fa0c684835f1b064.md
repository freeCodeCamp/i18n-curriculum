...
当你点击播放列表中的歌曲时，你想播放该歌曲。为此，你需要为每个作为 `song` 元素子元素的 `button` 元素添加一个针对 `click` 事件的事件监听器。

完成你的 `forEach` 的回调以实现该功能。在 `addEventListener` 回调内，调用 `playSong(n)`，其中 `n` 是你可以从 `song` 的 `id` 属性获取的歌曲 id。

请注意，每个 `song` 都有一个 `id` 属性，格式为 `song-n`：你需要获取 `n` 部分并将其转换为数字，然后再传递给 `playSong()`。


# --hints--

你应该为每个作为 `.playlist-song` 元素子元素的 `button` 元素添加 `click` 事件监听器，并使用对应歌曲的 `id` 调用 `playSong`。
