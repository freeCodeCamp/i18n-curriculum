...
أضف فاصلًا متوسطًا بعد عنصر `.daily-value` الخاص بك. أسفل ذلك الفاصل الجديد، أنشئ عنصر `p` مع تعيين خاصية `class` إلى `note`.

أعطِ عنصر `p` النص التالي:
...
يجب أن تنشئ عنصر `div` جديدًا بعد عنصر `.daily-value` الخاص بك.

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

يجب أن يكون للعنصر `div` الجديد الخاص بك خاصية `class` مضبوطة على `divider medium`.
...
يجب أن تنشئ عنصر `p` بعد عنصر `div` الجديد الخاص بك.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'p');
```

يجب أن يكون للعنصر `p` الجديد خاصية `class` مضبوطة على `note`.

```js
assert(document.querySelector('.label')?.lastElementChild?.classList?.contains('note'));
```

يجب أن يحتوي العنصر `p` الجديد على النص المقدم.
