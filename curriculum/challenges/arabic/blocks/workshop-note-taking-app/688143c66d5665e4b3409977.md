...
يبدو أن الأمور تعمل باستثناء مشكلة صغيرة واحدة. إذا حررت ملاحظة ثم غادرت تلك الملاحظة، يجب أن ترى رسالة `"Note saved successfully!"`. لكن إذا حاولت تحرير الملاحظة مرة أخرى، تظل الرسالة معروضة.

السلوك المطلوب هو أن تختفي الرسالة عندما يركز المستخدم على العنصر. يمكنك استخدام حدث `focus` لهذا الغرض.
...
أرفق `addEventListener` إلى المتغير `noteEl`. يجب أن يكون الوسيط الأول هو حدث `"focus"` والوسيط الثاني دالة استدعاء راجع. داخل تلك الدالة، عيّن `statusEl.textContent` إلى سلسلة فارغة. هذا سيعيد تعيين المنطقة الحية عند تركيز الملاحظة.

وبهذا التغيير الأخير، تكون ورشة العمل قد اكتملت!

# --hints--

عندما يركز المستخدم على الملاحظة، يجب أن تعيّن `statusEl.textContent` إلى سلسلة فارغة.
