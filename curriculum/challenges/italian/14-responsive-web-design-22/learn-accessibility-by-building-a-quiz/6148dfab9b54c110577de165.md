...
Dovresti dare a `button` una proprietà `display` con il valore `block`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('button')?.display, 'block');
```

Dovresti dare a `button` una proprietà `margin` con il valore `40px auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('button')?.margin, '40px auto');
```

Dovresti dare a `button` una proprietà `width` del `40%`.
...
Dovresti dare a `button` una proprietà `font-size` di `23px`.
...
Dovresti dare a `button` una proprietà `border` con il valore `3px solid #3b3b4f`.
