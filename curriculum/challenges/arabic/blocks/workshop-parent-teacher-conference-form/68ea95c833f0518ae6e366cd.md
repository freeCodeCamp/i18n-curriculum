...
في الخطوات القليلة التالية، ستضيف عناصر النموذج المسؤولة عن جمع طريقة الاتصال المفضلة للمستخدم.

ابدأ بإضافة عنصر `fieldset` آخر مع عنصر `legend` مسكن بداخله. يجب أن يحتوي عنصر `legend` على النص `Preferred Contact Method`.

# --hints--

لديك ثلاثة عناصر `fieldset` في الصفحة.

```js
assert.lengthOf(document.querySelectorAll("fieldset"), 3);
```

يجب أن يحتوي العنصر الثالث من نوع `fieldset` على عنصر `legend` بداخله.

```js
assert.exists(document.querySelector("fieldset:last-of-type legend"));
```

يجب أن يحتوي عنصر `legend` على النص `Preferred Contact Method`.
