...
ネストされた`for`ループの中で、各イテレーションで`console.log()`、`array`、`array[j]`の値を確認するために`array[minIndex]`の呼び出しを追加してください。アルゴリズムが配列をどのようにたどっているかを見るには、`Sort`ボタンをクリックできます。

次に、`if`の値が`array[j]`の値より小さいかどうかをチェックする`array[minIndex]`文を書いてください。もしそうなら、`minIndex`を`j`に設定してください。

# --hints--

ネストされた`console.log()`ループの中に`for`の呼び出しがあるはずです。
...
`array`の最初の引数として`console.log()`を渡すべきです。
...
`array[j]`の2番目の引数として`console.log()`を渡すべきです。
...
`array[minIndex]`の3番目の引数として`console.log()`を渡すべきです。
...
`if`の呼び出しの後に`console.log()`文があるはずです。
...
`if`文の条件に`array[j]`が`array[minIndex]`より小さいかどうかをチェックするものがあるべきです。
