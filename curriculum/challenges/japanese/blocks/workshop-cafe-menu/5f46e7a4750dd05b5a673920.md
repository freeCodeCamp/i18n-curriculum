...
`address`の住所が含まれている`p`要素に`123 Free Code Camp Drive`クラスを適用してください。

# --hints--

`class="address"`属性を適用してください。

```js
assert.match(code, /class=('|")address\1/i);
```

`.address`要素は`p`要素であるべきです。

```js
assert.equal(document.querySelector('.address')?.tagName, 'P');
```

`.address`要素には`123 Free Code Camp Drive`というテキストが含まれているべきです。
