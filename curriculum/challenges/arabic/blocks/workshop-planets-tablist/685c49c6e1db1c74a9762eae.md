...
إذا نظرت إلى HTML الخاص بك، فإن `tab` الأول محدد بالفعل افتراضيًا ويحمل خاصية `aria-selected` مضبوطة على `"true"`. وعندما ينقر المستخدم على تبويب مختلف، ينبغي أولًا ضبط جميع التبويبات على `aria-selected="false"`.

لتحقيق ذلك، يمكنك استخدام طريقة `setAttribute` لتغيير خاصية `aria-selected` لكل تبويب.

إليك مثالًا على كيفية استخدام طريقة `setAttribute` على قائمة من عناصر `div`:
...
استخدم `forEach` على مجموعة `tabs` للتكرار عبر كل تبويب وضبط خاصية `aria-selected` له على `"false"`. بالنسبة لدالة الاستدعاء الراجع، يمكنك استخدام أي اسم معلمة تريده لكن من الأفضل عدم استخدام `tab` لأنه مستخدم بالفعل في حلقة `forEach` الخارجية.

# --hints--

عندما ينقر المستخدم على تبويب، يجب ضبط خاصية `aria-selected` لجميع التبويبات على `"false"`.
