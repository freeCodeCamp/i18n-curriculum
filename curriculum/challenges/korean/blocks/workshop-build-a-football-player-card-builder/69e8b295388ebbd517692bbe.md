...
`PlayerCard` 컴포넌트는 작동하지만 하드코딩된 값을 사용합니다. 유연하게 만들려면 TypeScript 인터페이스를 사용해 플레이어 데이터의 형태를 설명해야 합니다.

`PlayerCard` 함수 앞에 `PlayerData`라는 인터페이스를 만들고, `name`는 `string` 타입, `overallRating`는 `number` 타입, `position`는 `string` 타입, `club`는 `string` 타입으로 지정하세요.

# --hints--

`PlayerData`라는 인터페이스를 만들어야 합니다.
...
`PlayerData` 인터페이스는 `name`가 `string` 타입이어야 합니다.
...
`PlayerData` 인터페이스는 `overallRating`가 `number` 타입이어야 합니다.
...
`PlayerData` 인터페이스는 `position`가 `string` 타입이어야 합니다.
...
`playerData` 인터페이스는 `club`가 `string` 타입이어야 합니다.
