...
Crea un elemento `button` con la classe `playlist-song-info`. All'interno del `button`, aggiungi un elemento `span` con la classe `playlist-song-title`, quindi inserisci `song.title` come testo.

# --hints--

Dovresti creare un `button` con la classe `playlist-song-info`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

Dovresti creare un elemento `span` all'interno del tuo elemento `button`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

Il tuo elemento `span` dovrebbe avere la classe `playlist-song-title`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

Dovresti inserire `song.title` come contenuto del tuo elemento `span`.
