...
Si se encuentra `0`, el método debe devolver inmediatamente una tupla que contenga el índice de la fila y el índice de la columna de la celda vacía.

Dentro del bloque `try`, después de la asignación de `col`, devuelve `row, col`.

# --hints--

Debes retornar `row, col` desde el bloque `try`.
