...
Será útil tener una función para mostrar un resumen del rendimiento durante una simulación. Crea una función vacía llamada `printSummary` con un parámetro `summary`.

# --hints--

Debes crear una función llamada `printSummary`.

```js
assert.isFunction(printSummary);
```

Tu función `printSummary` debe tener un parámetro `summary`.
