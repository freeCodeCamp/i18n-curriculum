...
Ми зіткнулися з невеликою проблемою. Ви намагаєтеся запитати на своїй сторінці елемент кнопки, але тег `script` знаходиться в `head` вашого HTML. Це означає, що код запускається до того, як браузер завершить читання HTML, та `document.querySelector()` не побачить кнопку, оскільки браузер ще не обробив її.
