...
في بداية الـ setter الخاص بك، أنشئ جملة `if` ترفع `TypeError` مع الرسالة `'salary' must be a number.` عندما لا يكون `new_salary` عددًا صحيحًا أو عددًا عشريًا.

# --hints--

يجب أن تحتوي الـ setter الخاص بـ `salary` على جملة `if`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[0]`)) })
```

يجب أن ترفع `TypeError` مع الرسالة `'salary' must be a number.` عندما لا يكون `new_salary` عددًا صحيحًا أو عددًا عشريًا.
...
لا يجب أن ترفع أي استثناء عندما يكون `new_salary` عددًا صحيحًا أو عددًا عشريًا.
