...
創建一個 `.quote` 選擇器。為它添加一個 `color` 屬性，設定為 `#00beef`，一個 `font-size` 屬性，設定為 `2.4rem`，以及一個 `text-align` 屬性，設定為 `center`。
