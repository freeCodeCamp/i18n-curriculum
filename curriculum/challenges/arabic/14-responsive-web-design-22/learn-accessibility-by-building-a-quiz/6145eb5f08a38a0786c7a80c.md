...
إذا نقرت على مدخلات radio، فقد تلاحظ أنه يمكن تحديد كلا المدخلين true/false في نفس fieldset في نفس الوقت.

قم بتجميع المدخلات ذات الصلة معًا بحيث يمكن تحديد مدخل واحد فقط من الزوج في المرة الواحدة.

# --hints--

يجب أن تعطي أول `input` سمة `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[0]?.name);
```

يجب أن تعطي ثاني `input` سمة `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[1]?.name);
```

يجب أن تعطي ثالث `input` سمة `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[2]?.name);
```

يجب أن تعطي رابع `input` سمة `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[3]?.name);
```

يجب أن تعطي عنصر `input` الثاني، السمة `name` التي تطابق سمة `name` لعنصر `input` الأول.
...
يجب أن تعطي عنصر `input` الرابع، السمة `name` التي تطابق سمة `name` لعنصر `input` الثالث.
...
يجب عليك إعطاء سمات `name` مختلفة إلى `input` الأول و `input` الثالث.
