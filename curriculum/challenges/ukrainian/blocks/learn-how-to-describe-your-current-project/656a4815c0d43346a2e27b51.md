---
id: 656a4815c0d43346a2e27b51
title: Завдання 44
challengeType: 22
dashedName: task-44
lang: en-US
---

<!-- (Audio) Amy: Yes, I am. I'm experimenting with video interviews and interactive infographics to make the content more engaging for our readers. It's a bit challenging, but I'm learning a lot in the process. -->

# --description--

`Engaging` — це прикметник, який означає цікавий і здатний утримувати вашу увагу. Наприклад:

`The teacher gave an engaging lesson.` — тут `engaging` означає, що урок був веселим і тримав студентів зацікавленими.

`Challenging` — це прикметник, який означає складний, але такий, що може допомогти вам розвиватися або навчатися. Наприклад:

`The project was challenging, but we learned a lot from it.` — тут `challenging` означає, що проєкт був нелегким, але це був хороший досвід.

# --fillInTheBlank--

## --sentence--

`Yes, I am. I'm experimenting with video interviews and interactive infographics to make the content more BLANK for our readers. It's a bit BLANK, but I'm BLANK a lot in the process.`

## --blanks--

`engaging`

### --feedback--

Цей прикметник описує щось як цікаве і таке, що утримує увагу людей.

---

`challenging`

### --feedback--

Цей прикметник стосується чогось складного, але корисного.

---

`learning`

### --feedback--

Цей дієслово означає здобуття нових знань або навичок. Закінчується на `-ing`.

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Amy",
        "position": {"x":50,"y":0,"z":1.4},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "2.3-4.mp3",
      "startTime": 1,
      "startTimestamp": 31.98,
      "finishTimestamp": 42.08
    }
  },
  "commands": [
    {
      "character": "Amy",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Amy",
      "startTime": 1,
      "finishTime": 7.98,
      "dialogue": {
        "text": "Yes, I am. I'm experimenting with video interviews and interactive infographics to make the content more engaging for our readers.",
        "align": "center"
      }
    },
    {
      "character": "Amy",
      "startTime": 8.54,
      "finishTime": 11.10,
      "dialogue": {
        "text": "It's a bit challenging, but I'm learning a lot in the process.",
        "align": "center"
      }
    },
    {
      "character": "Amy",
      "opacity": 0,
      "startTime": 11.60
    }
  ]
}
```
