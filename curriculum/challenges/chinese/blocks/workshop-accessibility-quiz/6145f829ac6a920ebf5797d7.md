...
该项目的最后的两个语义化 HTML 元素是 `footer` 和 `address` 元素。 `footer` 元素是与页面相关的内容集合的容器，`address` 元素是页面作者的联系信息的容器。

在 `main` 元素之后，添加一个 `footer` 元素，并在其中嵌套一个 `address` 元素。

# --hints--

你应该在 `main` 元素之后添加一个 `footer` 元素。

```js
assert.exists(document.querySelector('main + footer'));
```

你应该在 `footer` 元素中嵌套一个 `address` 元素。
