...
CSV (valores separados por vírgula) é um formato usado para planilhas e importação de dados. Cada linha representa uma entrada com campos separados por vírgulas.

Comece removendo a chamada `console.log(exportToJSON(...))`, pois ela foi usada apenas para testes. Depois, crie uma função chamada `exportToCSV` que recebe `catalog` como parâmetro. Dentro dela, declare `const header = "Title,Author,Year,Location"` e `const rows = []`.

# --hints--

Você deve remover a chamada `exportToJSON` console.log.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

Você deve ter uma função chamada `exportToCSV` com um parâmetro `catalog`.
...
Você deve declarar uma variável chamada `header` com valor `"Title,Author,Year,Location"`.

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

Você deve declarar uma variável chamada `rows` definida como um array vazio.
