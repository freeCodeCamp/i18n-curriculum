...
Il resto del tuo equipaggio è stato creato in un array chiamato `remainingCrew` con i seguenti dati: 
...
Itera sull'array `remainingCrew` e aggiungi ogni astronauta a `squad` usando la funzione `addCrewMember()`.

# --hints--

Dovresti usare un loop `for` per chiamare `addCrewMember()` con ogni astronauta in `remainingCrew`.
