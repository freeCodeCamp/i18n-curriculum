...
Створіть ще один елемент `label` (на новій лінії) в кінці вашого `HTMLString`. Цей `label` повинен містити текст `Entry # Calories`, використовуючи синтаксис шаблонних рядків, щоб замінити `#` на значення `entryNumber`, а атрибут `for` має бути встановлений у `X-#-calories`, де `X` — це `value` `entryDropdown`, а `#` — значення `entryNumber`.

# --hints--

У вашому `label` має бути два елементи `HTMLString`.
...
Новий елемент `label` має бути на новій лінії.
...
Новий елемент `label` має йти після вашого елемента `input`.
...
Новий елемент `label` має мати атрибут `for`, встановлений у `${entryDropdown.value}-${entryNumber}-calories`.
...
Новий елемент `label` має містити текст `Entry ${entryNumber} Calories`.
...
Ви не повинні змінювати існуючі елементи.
