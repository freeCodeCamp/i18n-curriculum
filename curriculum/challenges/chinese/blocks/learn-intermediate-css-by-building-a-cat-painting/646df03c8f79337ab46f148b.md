...
将鼻子移动到合适的位置，设置 `position` 为 `absolute`、`top` 为 `108px`，`left` 为 `85px`。

# --hints--

你的 `.cat-nose` 选择器应将 `position` 属性设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.position === 'absolute')
```

你的 `.cat-nose` 选择器应将 `top` 属性设置为 `108px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.top === '108px')
```

你的 `.cat-nose` 选择器应将 `left` 属性设置为 `85px`。
