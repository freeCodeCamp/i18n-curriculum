...
`parseCatalog` наразі повертає порожній масив. Щоб його заповнити, додайте `for` цикл всередині функції, який ітерує над `rawCards`. Усередині циклу викличте `parseCard` для кожного елемента і додайте результат у `catalog`.

# --hints--

`parseCatalog(rawCatalogCards).length` має бути `19`.

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

`parseCatalog` має використовувати `for` цикл для ітерації над `rawCards`.

```js
assert.match(parseCatalog.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*rawCards\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Ви маєте викликати `parseCard` всередині циклу.
