...
Per impostazione predefinita, gli elementi `label` sono in linea. Per farli impilare verticalmente (eccetto le etichette dei tuoi pulsanti di opzione), usa la pseudo-classe `:not()`.

La pseudo-classe `:not()` nega una selezione. Qui, seleziona tutti gli elementi `label` che non hanno la classe `contact-method`.

Seleziona `label:not(.contact-method)`, poi imposta la proprietà `display` su `block` e la proprietà `margin` su `10px 0`.

# --hints--

Dovresti avere un selettore `label:not(.contact-method)`.
...
Imposta la proprietà `margin` su `10px 0`.
