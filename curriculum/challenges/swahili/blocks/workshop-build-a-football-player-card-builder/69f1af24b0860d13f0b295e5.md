...
Sasa badilisha nambari ngumu zilizobaki na zifuatazo:
...
Unapaswa kubadilisha thamani ngumu ya `99` kwa `DRI` na `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

Unapaswa kubadilisha thamani ngumu ya `41` kwa `DEF` na `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

Unapaswa kubadilisha thamani ngumu ya `99` kwa `PHY` na `player.phy`.
