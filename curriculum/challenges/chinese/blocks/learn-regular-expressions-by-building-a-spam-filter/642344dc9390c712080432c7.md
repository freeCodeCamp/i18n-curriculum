...
与 `freeRegex` 类似，更新 `stockRegex` 以将 `e` 和 `o` 字符替换为字符类，以匹配字母和相应的数字。

# --hints--

你的 `stockRegex` 应该使用字符类来匹配字母 `e` 和数字 `3`。

```js
assert.match(stockRegex.source, /\[(e3|3e)\]/);
```

你的 `stockRegex` 应该使用字符类来匹配字母 `o` 和数字 `0`。
...
你的 `stockRegex` 应该与 `st0ck al3rt` 匹配。
