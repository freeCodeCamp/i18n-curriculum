...
为了将你所学的一切结合起来，更新你的 `dearRegex` 将元音映射到相应的数字（注意 `i` 应该匹配 `1`，并且还匹配管道符号 `|`），并匹配整个单词。
...
你的 `dearRegex` 应该匹配 `dear friend`。

```js
assert.match('dear friend', dearRegex);
```

你的 `dearRegex` 应该匹配 `d34r fr13nd`。

```js
assert.match('d34r fr13nd', dearRegex);
```

你的 `dearRegex` 应该匹配 `d3@r fr|3nd`。

```js
assert.match('d3@r fr|3nd', dearRegex);
```

你的 `dearRegex` 应该匹配 `my dear friend Naomi`。

```js
assert.match('my dear friend Naomi', dearRegex);
```

你的 `dearRegex` 应该匹配 `dear friend Naomi`。

```js
assert.match('dear friend Naomi', dearRegex);
```

你的 `dearRegex` 应该匹配 `my dear friend`。
...
你的 `dearRegex` 不应该匹配 `dear friend-o`。
