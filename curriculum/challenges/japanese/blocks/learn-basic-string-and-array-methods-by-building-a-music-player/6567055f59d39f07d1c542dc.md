...
次に、HTMLドキュメント内のプレイリストを更新して曲を表示する必要があります。

`songsHTML` 要素の `innerHTML` プロパティに `playlistSongs` を代入してください。これにより、先ほど作成した `li` 要素が、すでに提供されている `ul` ファイル内の `HTML` 要素に挿入されます。

# --hints--

`innerHTML` に `songsHTML` を追加するには、`playlistSongs` プロパティを使うべきです。
