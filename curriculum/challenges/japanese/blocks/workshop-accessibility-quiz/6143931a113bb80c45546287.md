...
2番目の`section`要素の中に、クラスが`question-block`の`div`要素を2つ追加してください。

次に、それぞれの`div.question-block`要素の中に、`h3`から始まる連番のテキストを持つ`1`要素を1つと、クラスが`question`の`fieldset`要素を1つ追加してください。
