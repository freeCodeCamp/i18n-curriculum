...
Ein Teil der Gesamtkosten wird die Steuer enthalten, daher müssen Sie diese ebenfalls berechnen. Erstellen Sie eine `calculateTaxes`-Methode in der `ShoppingCart`-Klasse. Diese Methode soll einen `amount`-Parameter entgegennehmen.

# --hints--

Sie sollten eine `calculateTaxes`-Methode in der `ShoppingCart`-Klasse erstellen.

```js
assert.isFunction(cart.calculateTaxes);
```

Ihre `calculateTaxes`-Methode soll einen `amount`-Parameter entgegennehmen.
