...
Kituo cha udhibiti kinahitaji njia ya kunakili wanaanga wanaostahili EVA kutoka kwa kikosi. Unda kitendakazi tupu kinachoitwa `getEVAReadyCrew` kinachokubali kigezo cha `crew`.

# --hints--

Unapaswa kuunda kitendakazi kinachoitwa `getEVAReadyCrew`.

```js
assert.isFunction(getEVAReadyCrew);
```

Kitendakazi chako cha `getEVAReadyCrew` kinapaswa kuwa na kigezo cha `crew`.
