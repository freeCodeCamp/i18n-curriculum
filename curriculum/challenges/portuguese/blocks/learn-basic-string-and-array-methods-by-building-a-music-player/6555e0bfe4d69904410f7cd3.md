...
Usando `const` e a sintaxe de arrow para criar uma função vazia chamada `shuffle`.

Esta função é responsável por embaralhar as músicas na playlist e realizar as atualizações necessárias de gerenciamento de estado após o embaralhamento.

# --hints--

Você deve usar `const` para criar uma função vazia chamada `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

`shuffle` deve ser uma função.

```js
assert.isFunction(shuffle)
```

Sua função `shuffle` deve usar uma sintaxe de arrow e não deve receber um parâmetro.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

Sua função `shuffle` deve estar vazia.
