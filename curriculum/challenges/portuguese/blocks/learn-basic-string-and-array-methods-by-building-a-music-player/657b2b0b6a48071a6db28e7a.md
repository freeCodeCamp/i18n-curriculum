...
O método `map()` é usado para iterar por um array e retornar um novo array. É útil quando você quer criar um novo array baseado nos valores de um array existente. Por exemplo:
...
Observe que o método `map()` recebe uma função como argumento. Isto é chamado de <dfn>callback function</dfn>, que é uma função passada para outra função como argumento. No exemplo acima, a função de callback é `(number) => number * 2` e ela é executada em cada elemento do array `numbers`. O método `map()` então retorna um novo array com os resultados.

Passe uma função de callback para o método `map()`. A função de callback deve receber `song` como parâmetro, usar a sintaxe de função arrow e ter um corpo vazio.

# --hints--

A função de callback do seu método `map()` deve usar a sintaxe de função arrow.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

Seu método `map()` deve ter um parâmetro `song` na sua função de callback.
