...
给 `.bb3` 添加 `repeating-linear-gradient`。 将方向设置为 `90deg`，`building-color3` 作为前两种颜色，在 `15%` 处采用 `window-color3` 作为第三种颜色。 当你不指定颜色的距离时，它将使用有意义的值。 在这个例子中，前两种颜色将默认为在 `0%` 处和 `7.5%` 处，因为它从 `0%` 开始，而 `7.5%` 是 `15%` 的一半。

# --hints--

你应该使用 `repeating-linear-gradient` 给 `.bb3` 一个 `background`。
