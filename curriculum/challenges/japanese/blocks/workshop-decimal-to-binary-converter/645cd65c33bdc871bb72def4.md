...
これは簡単な例ですが、呼び出しスタックがどのようにコードを順に処理し、複数の関数を呼び出すかを示しています。

ここからは再帰に入り、呼び出しスタックがどのように関係しているかを見ていきましょう。

`callStack` 配列、`a()`、`b()`、`c()` 関数、および `console.log()` 文を削除してください。

# --hints--

コードから `callStack` 配列を削除してください。

```js
assert.notMatch(code, /(var|let|const)\s+callStack\s*=\s*\[\s*\]/);
```

コードから `a()`、`b()`、`c()` 関数を削除してください。
