...
Ad ogni iterazione del tuo `while` loop, sta trovando un elemento più grande del valore attuale. Devi spostare quell'elemento a destra per fare spazio al valore attuale.

Fallo assegnando il valore all'indice `j` all'indice successivo.

# --hints--

Prima di decrementare `j`, assegna il valore a `j` all'indice `j + 1`.
