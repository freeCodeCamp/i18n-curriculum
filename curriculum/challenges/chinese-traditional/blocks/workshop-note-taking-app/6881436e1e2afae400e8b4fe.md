...
目前，任何時候使用者編輯筆記並點擊筆記外部時，`currentContent` 變數都會被更新。但如果使用者聚焦在筆記上卻決定離開而沒有進行任何編輯，該怎麼辦？

那麼，在這種情況下，你可以添加一個條件來控制代碼該使用案例。 

在你的 `currentContent = newContent;` 行上方，添加一個 `if` 述語來檢查 `currentContent` 是否相等於 `newContent`。如果是，則傳回。

# --hints--

你應該為你的程式碼添加一個 `if` 述語來檢查 `currentContent` 是否與 `newContent` 相等。如果是，則傳回。
