...
اختر فقط العناصر ذات الصنف `.inline`، وأعطها `width` بقيمة `unset`. هذا سيزيل القاعدة السابقة التي جعلت كل عناصر الـ `input` تأخذ `width: 100%`.

# --hints--

يجب أن تستخدم المحدِّد `.inline`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.inline'));
```

يجب أن تعطي العناصر ذات الصنف `.inline` `width` بقيمة `unset`.
