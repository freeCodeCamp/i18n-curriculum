...
前のステップで作成したループの中に、`for` ループをネストして、`col_start` から始まる3つの要素のシーケンスを繰り返してください。このシーケンスは `range()` 関数で生成し、ループ変数には `col_no` を使います。

# --hints--

既存のループ本体の中で、`for` を繰り返す `range(col_start, col_start + 3)` ループを作成してください。
...
`for` ループはループ変数として `col_no` を使う必要があります。
