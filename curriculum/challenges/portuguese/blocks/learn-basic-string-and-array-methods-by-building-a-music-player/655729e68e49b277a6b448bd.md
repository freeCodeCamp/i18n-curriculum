...
Agora que você criou o `resetButton`, você precisa atribuir a ele os atributos `id` e `aria-label`. JavaScript fornece as propriedades `id` e `ariaLabel` que você precisa usar para isso.

Por exemplo, `element.id` definiria um atributo `id` e `element.ariaLabel` definiria um atributo `aria-label`. Ambos aceitam seus valores como uma string.

Defina o atributo `id` do elemento `resetButton` para `"reset"` e seu atributo `aria-label` para `"Reset playlist"`.

# --hints--

Você não deve modificar a declaração `if` existente e seu conteúdo.
...
Você deve usar `resetButton.id` para criar um atributo `id` chamado `"reset"` para o `resetButton`.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

Você deve usar `resetButton.ariaLabel` para criar um atributo `"aria-label"` chamado `"Reset playlist"` para o `resetButton`.
