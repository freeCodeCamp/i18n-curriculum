...
當傳入的可迭代物件中所有元素皆為真值時，`all` 函式（程式）傳回 `True`，否則傳回 `False`：
...
將列表 `[isinstance(i, str) for i in medications]` 傳遞給 `all` 函式，以確保其中的每個元素都是字串。

# --hints--

你應該透過將 `[isinstance(i, str) for i in medications]` 傳遞給 `all` 函式（程式），來修改你的 `medications` 鍵的值。
