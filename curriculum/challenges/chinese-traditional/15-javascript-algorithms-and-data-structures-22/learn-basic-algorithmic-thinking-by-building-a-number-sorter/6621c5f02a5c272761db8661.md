...
你應該使用 `console.log()` 來打印 `inputValues` 的結果。 在 `sortInputArray` 函數中編寫此代碼。

要查看記錄的 `inputValues` 數組，請點擊排序按鈕並打開控制檯。 你應該會看到一個字符串數組如下：

```js
[ "8", "2", "4", "1", "3" ]
```

在繼續之前，請確保你注意到在控制檯中打印結果的數據類型。
...
你應該將 `inputValues` 傳遞給你的 `console.log()` 調用。
