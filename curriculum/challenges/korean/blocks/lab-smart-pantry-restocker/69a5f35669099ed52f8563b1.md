...
`rawData` 배열은 `sku|name|qty|expires|zone` 형식의 파이프 구분 문자열을 포함하며, `zone`은 선택 사항입니다.

**사용자 스토리:**

1. 문자열 배열을 받아 `parseShipment(rawData)` 속성을 가진 객체 배열을 반환하는 `{ sku, name, qty, expires, zone }` 함수를 구현해야 합니다.

   - 도착 물품에 중복된 `sku` 값은 무시해야 합니다.
   - `zone` 구간이 없으면 기본값으로 `"general"`을 할당해야 합니다.
   - `qty` 값은 숫자로 변환해야 합니다.

2. 현재 식료품 저장실과 도착한 물품을 비교해 `planRestock(pantry, shipment)` 형태의 작업 배열을 반환하는 `{ type, item }` 함수를 구현해야 합니다. 여기서 `type`은 `"restock"`, `"discard"`, `"donate"` 중 하나이며, `item`은 파싱된 도착 물품 객체입니다.

   `pantry` 매개변수는 파싱된 도착 물품과 같은 형태의 객체 배열입니다 (`{ sku, name, qty, expires, zone }`).

- 도착 물품의 `qty`가 `0` 이하이면, 식료품 저장실에 있든 없든 작업 `type`은 `"discard"`여야 합니다.
   - 그렇지 않고 도착 물품의 `sku`가 식료품 저장실에 이미 있으면 작업 `type`은 `"restock"`이어야 합니다.
   - 그렇지 않으면(도착 물품의 `sku`가 식료품 저장실에 없으면) 작업 `type`은 `"donate"`여야 합니다.

3. 각 아이템의 `zone` 속성에 따라 작업을 저장 구역별로 그룹화하는 `groupByZone(actions)` 함수를 구현해야 합니다. 이 함수는 각 키가 구역 이름이고 값이 해당 구역에 속한 작업 배열인 객체를 반환해야 합니다. 예를 들어, 작업에 `"fridge"`와 `"pantry"` 구역이 포함되어 있으면 결과는 `{ fridge: [...], pantry: [...] }` 형태여야 합니다.
...
`groupByZone` 함수는 작업을 올바른 내용과 개수로 정확히 그룹화해야 합니다.
