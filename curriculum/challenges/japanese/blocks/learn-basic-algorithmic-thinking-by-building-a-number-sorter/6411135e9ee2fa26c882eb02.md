...
`while` ループの各イテレーションで、現在の値より大きい要素を見つけています。その要素を右に移動させて、現在の値のためのスペースを作る必要があります。

`j` のインデックスにある値を次のインデックスに代入することでこれを行ってください。

# --hints--

`j` をデクリメントする前に、`j` にある値を `j + 1` のインデックスに代入してください。
