...
Fügen Sie ein weiteres `hr`-Element zwischen dem `main`-Element und dem `footer`-Element hinzu.

# --hints--

Sie sollten ein zweites `hr`-Element hinzufügen.

```js
assert.lengthOf(document.querySelectorAll('hr'), 2);
```

Ihr neues `hr`-Element sollte zwischen dem `main`-Element und dem `footer`-Element liegen.
