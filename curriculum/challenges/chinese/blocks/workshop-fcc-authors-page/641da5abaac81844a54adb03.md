...
现在，是时候创建负责在页面上显示作者的函数了。 

首先创建一个名为 `displayAuthors` 的函数，该函数接受一个 `authors` 参数。然后你的函数应循环遍历 `authors` 数组，并为每个作者向 `authorContainer` 元素添加一个卡片。 
...
- 你可以使用任何类型的循环来迭代 `authors` 数组。例如 `for`、`forEach`、`map` 等。
- 你应该使用 `innerHTML` 属性在循环内将卡片添加到 `authorContainer` 中。你还需要使用模板字面语法来添加标记。
...
当调用 `displayAuthors` 函数并传入作者数组时，它应为每个作者向 `#authorContainer` 元素添加一个卡片。
