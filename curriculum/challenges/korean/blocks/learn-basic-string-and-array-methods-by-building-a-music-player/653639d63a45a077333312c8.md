...
노래를 재생하기 전에 처음부터 시작하는지 확인해야 합니다. 이는 `currentTime` 객체의 `audio` 속성을 사용해 달성할 수 있습니다.

`if` 문을 추가하여 `userData?.currentSong`이 falsy인지 *또는* `userData?.currentSong.id`가 `song.id`와 엄격하게 같지 않은지 확인하세요. 이 조건은 현재 노래가 재생 중이지 않거나 재생하려는 노래와 다를 경우를 검사합니다.

`if` 블록 안에서 `currentTime` 객체의 `audio` 속성을 `0`로 설정하세요.

# --hints--

조건 `if`을 가진 `userData?.currentSong === null || userData?.currentSong.id !== song.id` 문을 만드세요. 선택적 체이닝을 포함하는 것을 잊지 마세요.
...
`audio.currentTime` 블록 안에서 `0`를 `if`으로 설정하세요.
