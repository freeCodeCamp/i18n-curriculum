...
子组合选择器 `>` 用于选择器之间，以仅匹配第二个选择器且是第一个选择器直接子元素的元素。

当你有嵌套层级较深的元素并希望控制样式的范围时，这将会很有帮助。

使用 `>` 选择器定位 `nav` 元素内的无序列表元素，并使用 *Flexbox* 均匀分布子元素。
...
你应该为 `nav > ul` 元素设置 `display` 为 `flex`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('nav > ul')?.display, 'flex');
```

你应该为 `nav > ul` 元素设置 `justify-content` 为 `space-evenly`。
