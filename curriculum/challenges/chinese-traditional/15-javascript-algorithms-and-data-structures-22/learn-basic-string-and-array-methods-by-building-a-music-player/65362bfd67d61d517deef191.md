...
<dfn>find()</dfn> 方法檢索數組中滿足提供的回調函數中指定的條件的第一個元素。 如果沒有元素滿足條件，該方法返回 `undefined`。

在下面的例子中，`find()` 方法用於查找第一個大於 25 的數字：
...
使用 `const` 創建一個名爲 `song` 的變量，並將其分配給 `userData?.songs` 數組上的 `find()` 方法調用的結果。 使用 `song` 作爲 `find()` 回調的參數，並檢查 `song.id` 是否嚴格等於 `id`。

這將遍歷 `userData?.songs` 數組，搜索與傳遞到 `playSong` 函數的 `id` 相對應的歌曲。
