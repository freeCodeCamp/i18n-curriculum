...
Перед тим як додати останній елемент до цілі, додайте коментар із текстом `move the nth disk from source to target`.

# --hints--

Ви повинні додати коментар із наданим текстом перед `rods[target].append(rods[source].pop())`.
