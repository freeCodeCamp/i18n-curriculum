...
最後に、許可された手を出力する`else`節を追加し、`make_allowed_move`を呼び出してください。正しい引数を考えてみましょう。
...
`print(f'Move {i + 1} allowed between {source} and {target}')`ブロック内に`else`があるはずです。
...
`make_allowed_move(source, target)`ブロック内に`else`があるはずです。
