...
Dentro de tu elemento `.image-wrapper`, da al segundo elemento `img` un `src` de `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`, un `alt` de `imagen de un proyecto de calculadora`, un atributo `loading` establecido a `lazy`, una `class` establecida a `image-2`, un atributo `width` establecido a `400`, y un atributo `height` establecido a `400`.

# --hints--

Tu segundo elemento `img` debe tener un `src` establecido a `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`.
...
Tu segundo elemento `img` debe tener un `alt` establecido a `imagen de un proyecto de calculadora`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('alt') === 'image of a calculator project');
```

Tu segundo elemento `img` debe tener un atributo `loading` establecido a `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('loading') === 'lazy');
```

Tu segundo elemento `img` debe tener una `class` establecida a `image-2`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.classList?.contains('image-2'));
```

Tu segundo elemento `img` debe tener un `width` establecido a `400`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('width') === '400');
```

Tu segundo elemento `img` debe tener un `height` establecido a `400`.
