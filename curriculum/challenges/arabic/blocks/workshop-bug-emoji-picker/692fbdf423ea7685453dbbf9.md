...
ابدأ الآن بالإشارة إلى عناصر DOM اللازمة.

احصل على عنصر الفقرة الذي يحمل المعرف `bug-emoji` وخزنه في ثابت باسم `bugEmojiElement`. استخدم عامل تحقق عدم الفراغ `!` لتأكيد أن العنصر ليس `null`.

# --hints--

يجب أن يكون لديك ثابت باسم `bugEmojiElement`.
...
يجب أن يستعلم `bugEmojiElement` عن العنصر الذي يحمل المعرف `bug-emoji` باستخدام `document.querySelector` مع تمرير نوع `HTMLParagraphElement` كمعلمة نوع.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

يجب أن يستخدم `bugEmojiElement` عامل تحقق عدم الفراغ (`!`).
