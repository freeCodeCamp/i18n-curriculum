...
Ihr Auswahlmenü benötigt Optionen für jedes der Lebensmittel- und Bewegungs-`fieldset`-Elemente, die Sie in den vorherigen Schritten erstellt haben. Verwenden Sie das `option`-Element, um für jedes `fieldset` eine neue Option zu erstellen. Das `value`-Attribut jeder Option sollte der `id` des `fieldset` sein, und der Text jeder Option sollte der Text des `legend` sein.

Setzen Sie die `Breakfast`-Option als die `selected`-Option.

# --hints--

Sie sollten fünf `option`-Elemente innerhalb Ihres `select`-Elements erstellen.

```js
assert.equal(document.querySelectorAll('.controls select option')?.length, 5);
```

Ihr erstes `option` sollte den Text `Breakfast` haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[0]?.textContent?.trim(), 'Breakfast');
```

Ihr erstes `option` sollte das `value`-Attribut auf `breakfast` gesetzt haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[0]?.value, 'breakfast');
```

Ihr zweites `option` sollte den Text `Lunch` haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[1]?.textContent?.trim(), 'Lunch');
```

Ihr zweites `option` sollte das `value`-Attribut auf `lunch` gesetzt haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[1]?.value, 'lunch');
```

Ihr drittes `option` sollte den Text `Dinner` haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[2]?.textContent?.trim(), 'Dinner');
```

Ihr drittes `option` sollte das `value`-Attribut auf `dinner` gesetzt haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[2]?.value, 'dinner');
```

Ihr viertes `option` sollte den Text `Snacks` haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[3]?.textContent?.trim(), 'Snacks');
```

Ihr viertes `option` sollte das `value`-Attribut auf `snacks` gesetzt haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[3]?.value, 'snacks');
```

Ihr fünftes `option` sollte den Text `Exercise` haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[4]?.textContent?.trim(), 'Exercise');
```

Ihr fünftes `option` sollte das `value`-Attribut auf `exercise` gesetzt haben.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[4]?.value, 'exercise');
```

Ihr erstes `option` sollte standardmäßig ausgewählt sein.
