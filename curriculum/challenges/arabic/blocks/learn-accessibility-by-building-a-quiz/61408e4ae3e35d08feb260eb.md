...
غيّر لون خط `h1` إلى `#f1be32`، واضبط حجم الخط على `min(5vw, 1.2em)`.

# --hints--

يجب أن تستخدم محدِّد عنصر `h1`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1'));
```

يجب أن تعطي `h1` خاصية `color` بالقيمة `#f1be32`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.color, 'rgb(241, 190, 50)');
```

يجب أن تعطي `h1` خاصية `font-size` بالقيمة `min(5vw, 1.2em)`.
