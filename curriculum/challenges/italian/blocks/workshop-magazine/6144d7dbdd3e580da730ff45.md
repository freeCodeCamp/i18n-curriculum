...
Crea un selettore `body`. Imposta la proprietà `font-family` su `Baskervville`, con un fallback di `serif`. Imposta la proprietà `color` su `linen` e la proprietà `background-color` su `rgb(20, 30, 40)`.
...
Il tuo selettore `body` dovrebbe avere una proprietà `font-family` impostata su `Baskervville`, con un fallback di `serif`.
...
Il tuo selettore `body` dovrebbe avere una proprietà `color` impostata su `linen`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.color, 'linen');
```

Il tuo selettore `body` dovrebbe avere una proprietà `background-color` impostata su `rgb(20, 30, 40)`.
