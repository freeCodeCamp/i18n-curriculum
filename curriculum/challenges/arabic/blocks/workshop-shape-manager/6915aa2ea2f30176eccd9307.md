...
استخدم حدث `oninput` على `shapeTypeSelect` واضبطه على `handleShapeSelect`. هذا سيجعل الدالة تُنفذ في كل مرة يتغير فيها اختيار الشكل.

بعد القيام بذلك، يجب أن ترى حقول الإدخال لكل شكل. لكن العمليات الحسابية لن تعمل بعد، وهذا ما ستتعامل معه في الخطوة التالية.

# --hints--

يجب أن تضبط `shapeTypeSelect.oninput` على `handleShapeSelect`.
