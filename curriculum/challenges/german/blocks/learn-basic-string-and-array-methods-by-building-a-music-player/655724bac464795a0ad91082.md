...
Als Nächstes müssen Sie prüfen, ob die Wiedergabeliste leer ist. Falls ja, sollten Sie das `userData`-Objekt auf seinen ursprünglichen Zustand zurücksetzen.

Verwenden Sie eine `if`-Anweisung, um zu überprüfen, ob die `userData?.songs` eine Länge von `0` hat.

# --hints--

Sie sollten eine leere `if`-Anweisung mit der Bedingung `userData?.songs.length === 0` erstellen.
