...
Sasa, ongeza `div` yenye darasa la vitu `img-loader`. Baada ya hapo, tengeneza kipengele cha `img` na kipa URL ya picha inayotegemea masharti kwa kutumia `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}`.

# --hints--

Unapaswa kuwa na kipengele cha `div` ndani ya kipengele chako cha `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

`div` yako inapaswa kuwa na darasa la vitu `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

Unapaswa kuwa na kipengele cha `img` ndani ya kipengele chako cha `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

Kipengele cha `img` kinapaswa kutumia `CDN_URL` na `img` kwa `src` yake wakati `img` inatolewa, na kurudi kwa `LOCAL_DEFAULT_IMG` wakati haipatikani.
...
Kipengele chako cha `img` kinapaswa kuwa na madarasa ya vitu `card-img` na `hidden`.
