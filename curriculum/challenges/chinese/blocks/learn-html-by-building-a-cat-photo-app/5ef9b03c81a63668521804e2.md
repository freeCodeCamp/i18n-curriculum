...
作为复选框的 `input` 元素应该有一个开始标签，而不是一个结束标签。

```js
assert.isTrue(document.querySelector('fieldset > input') && !code.match(/<\/input\>/g));
```

You should only have added one input element for your checkbox. 删除任何多余的。

```js
assert.lengthOf(document.querySelectorAll('fieldset > input'), 1);
```

Your new `input` element should be below the `legend` element with the text `What's your cat's personality?`. 你把它们的顺序写错了。
...
Your new `input` element should have a `type` attribute with the value `checkbox`. 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
...
The text `Loving` should be located directly to the right of your checkbox. Make sure there is a space between the element and the text. 你要么遗漏了文本，要么拼写有误。
