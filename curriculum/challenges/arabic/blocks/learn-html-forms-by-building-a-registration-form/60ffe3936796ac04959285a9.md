...
حاليًا، إذا كتبت في عناصر `input` أو `textarea`، فلن تتمكن من رؤية النص. كما أن ارتفاعها صغير جدًا ليكون سهل الاستخدام.

صلح هذا بتعيين `color` إلى `#ffffff`، وتعيين `min-height` إلى `2em`.

# --hints--

يجب تعيين `color` إلى `#ffffff`.
