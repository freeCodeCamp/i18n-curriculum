...
In früheren Lektionen haben Sie gelernt, wie Sie mit der `filter()`-Methode arbeiten, die verwendet wird, um ein neues Array mit gefilterten Ergebnissen zurückzugeben.
...
Erstellen Sie eine `getBooksByAuthor`-Funktion mit zwei Parametern – einem Array mit Buchobjekten und einem String mit dem Autor.

Die Funktion muss ein Array zurückgeben, das die Bücher eines bestimmten Autors enthält.

# --hints--

Sie sollten eine `getBooksByAuthor`-Funktion erstellen.

```js
assert.isFunction(getBooksByAuthor);
```

Ihre `getBooksByAuthor`-Funktion sollte zwei Parameter haben.

```js
assert.lengthOf(getBooksByAuthor, 2);
```

Ihre `getBooksByAuthor`-Funktion sollte ein Array zurückgeben.

```js
assert.isArray(getBooksByAuthor(library, ''));
```

Ihre `getBooksByAuthor`-Funktion sollte für jeden der Autoren die korrekte Anzahl an Büchern zurückgeben.
