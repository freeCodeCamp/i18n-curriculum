...
現在你已經準備好開始組合網格配置。在之前的課程中你已經學到，CSS Grid 提供二維的網格基礎配置，讓你能夠水平和垂直置中項目，同時仍保有控制元件來執行像是重疊元素等操作。

首先建立一個 `main` 選擇器，並為它設定 `display` 屬性為 `grid`。
...
你的 `main` 選擇器應該有一個設定為 `grid` 的 `display` 屬性。
