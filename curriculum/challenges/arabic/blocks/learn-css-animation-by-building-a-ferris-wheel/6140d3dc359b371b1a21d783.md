...
حان الوقت لوضع الكبائن حول العجلة. اختر أول عنصر `.cabin`. عرّف الخاصية `right` لتكون `-8.5%` والخاصية `top` لتكون `50%`.
...
يجب أن يحتوي محدِّد `.cabin:nth-of-type(1)` على خاصية `right` بقيمة `-8.5%`.
...
يجب أن يحتوي محدِّد `.cabin:nth-of-type(1)` على خاصية `top` بقيمة `50%`.
