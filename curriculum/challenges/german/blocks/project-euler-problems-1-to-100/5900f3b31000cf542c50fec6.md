...
title: 'Aufgabe 71: Geordnete Brüche'
challengeType: 1
forumTopicId: 302184
...
Betrachten Sie den Bruch $\frac{n}{d}$, wobei `n` und `d` positive ganze Zahlen sind. Wenn `n` &lt; `d` gilt und der größte gemeinsame Teiler, ${{HCF}(n, d)} = 1$, dann nennt man ihn einen gekürzten echten Bruch.
...
Man sieht, dass $\frac{2}{5}$ der Bruch ist, der unmittelbar links von $\frac{3}{7}$ steht.

Indem Sie die Menge der gekürzten echten Brüche für `d` ≤ `limit` in aufsteigender Reihenfolge auflisten, finden Sie den Zähler des Bruchs, der unmittelbar links von $\frac{3}{7}$ steht.
