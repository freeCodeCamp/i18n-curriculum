...
Ahora, asigna al `span` dentro del elemento `p` las clases `text-base`, `font-medium` y `text-gray-500`.

# --hints--

El `span` dentro de tu elemento `p` debe tener la clase `text-base`.
...
El `span` dentro de tu elemento `p` debe tener la clase `font-medium`.
...
El `span` dentro de tu elemento `p` debe tener la clase `text-gray-500`.
