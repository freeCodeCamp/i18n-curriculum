...
داخل دالة `playPreviousSong`، أضف جملة `if` بشرط `userData?.currentSong === null`. هذا سيتحقق مما إذا لم يكن هناك أي أغنية تُشغل حاليًا. إذا لم تكن هناك، اخرج من الدالة باستخدام `return`.

داخل كتلة `else`، أنشئ ثابتًا باسم `currentSongIndex` وقم بتعيين القيمة الناتجة من `getCurrentSongIndex()` له.

# --hints--

لا يجب تعديل دالة `playPreviousSong` الحالية أو محتواها.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

يجب أن تحتوي دالة `playPreviousSong` على جملة `if` بالشرط `userData?.currentSong === null`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

يجب أن تحتوي كتلة جملة `if` على `return`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

يجب أن تحتوي جملة `if` على كتلة `else`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

يجب استدعاء `getCurrentSongIndex` وتعيينه إلى `currentSongIndex` داخل كتلة `else` لجملة `if`.
