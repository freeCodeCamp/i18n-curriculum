...
Deklarieren Sie innerhalb der `showFortune`-Methode eine Variable namens `target` und weisen Sie ihr `e.target` zu. Erstellen Sie außerdem eine weitere leere if-Anweisung.

# --hints--

Sie sollten eine Variable namens `target` innerhalb von `showFortune` deklarieren.
...
Ihr `target`-Variable sollte `e.target` zugewiesen sein.
...
Sie sollten direkt nach Ihrer `if`-Variable eine leere `target`-Anweisung haben.
