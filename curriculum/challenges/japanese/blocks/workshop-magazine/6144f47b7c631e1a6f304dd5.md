...
あなたの雑誌には3つの主要なセクションがあります。全体のレイアウトはすでに`main`ルールで設定されていますが、子ルールで配置を調整できます。

1つの方法は`grid-column`プロパティを使うことで、これは`grid-column-start`と`grid-column-end`の省略形です。`grid-column`プロパティはグリッドアイテムがどのグリッドラインから始まりどのグリッドラインで終わるかを指定します。

`.heading`ルールを作成し、`grid-column`プロパティを`2 / 3`に設定してください。これにより`.heading`要素がグリッドライン2から始まりグリッドライン3で終わるようになります。

# --hints--

`.heading`セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.heading'));
```

`.heading`セレクターには`grid-column`プロパティが`2 / 3`に設定されているはずです。
