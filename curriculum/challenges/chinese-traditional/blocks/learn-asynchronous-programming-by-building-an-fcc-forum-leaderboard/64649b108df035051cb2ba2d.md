...
對於開啟的 `a` 頁籤，將 `target` 屬性設為 `"_blank"`。然後，將 `href` 屬性設為 `${forumTopicUrl}${slug}/${id}`。 

有了這些更改，你的 freeCodeCamp 論壇排行榜專案現在已完成！

# --hints--

你的開啟錨點標籤應該有一個 `target` 屬性設定為 `"_blank"`。
...
你的開啟錨點標記應該有一個 `href` 屬性，設定為 `${forumTopicUrl}${slug}/${id}`。
