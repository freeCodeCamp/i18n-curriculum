...
title: Aggiungi attributi agli elementi circle
challengeType: 6
forumTopicId: 301471
...
L'ultima sfida ha creato gli elementi `circle` per ogni punto nell'array `dataset`, e li ha aggiunti in coda all'SVG. Ma D3 ha bisogno di più informazioni sulla posizione e la dimensione di ogni `circle` per mostrarli correttamente.

Un `circle` in SVG ha tre attributi principali. Gli attributi `cx` e `cy` sono le coordinate. Indicano a D3 dove posizionare il _centro_ della forma sull'SVG. Il raggio (attributo `r`) dà la dimensione del `circle`.

Proprio come la coordinata `rect` `y`, l'attributo `cy` per un `circle` si misura dall'alto dell'SVG, non dal basso.

Tutti e tre gli attributi possono usare una funzione di callback per impostare dinamicamente i loro valori. Ricorda che tutti i metodi concatenati dopo `data(dataset)` vengono eseguiti una volta per ogni elemento nell'array `dataset`. Il parametro `d` nella funzione di callback si riferisce all'elemento corrente nell'array `dataset`, che è un array per ogni punto. Usi la notazione a parentesi, come `d[0]`, per consultare i valori in quell'array.

# --instructions--

Aggiungi gli attributi `cx`, `cy` e `r` agli elementi `circle`. Il valore `cx` dovrebbe essere il primo numero nell'array per ogni elemento in `dataset`. Il valore `cy` dovrebbe basarsi sul secondo numero nell'array, ma assicurati di mostrare il grafico dritto e non invertito. Il valore `r` dovrebbe essere `5` per tutti i cerchi.
