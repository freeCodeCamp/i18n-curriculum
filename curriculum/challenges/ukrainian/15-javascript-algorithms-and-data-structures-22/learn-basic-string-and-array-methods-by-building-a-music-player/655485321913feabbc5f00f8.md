...
Додайте слухач події `"click"` до елемента `previousButton`, а потім передайте `playPreviousSong` як другий аргумент.

# --hints--

Викличте метод `addEventListener()` на змінній `previousButton`.
...
Передайте `playPreviousSong` як другий аргумент методу `addEventListener`.
