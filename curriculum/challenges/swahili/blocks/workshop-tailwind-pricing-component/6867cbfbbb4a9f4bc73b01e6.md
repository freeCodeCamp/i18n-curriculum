...
Alama za vitu vya orodha zinahitaji kuwekwa nafasi kidogo. Kwa hivyo, toa darasa la `span` na `text-green-700` kwa kila `mr-2` katika vitu vya orodha.

Pia, ongeza sifa ya kipengele ya `aria-hidden` kwa kila kipengele cha `span`.

# --hints--

Vipengele vyako vya `span` vinapaswa kuwa na darasa la `text-green-700`.
