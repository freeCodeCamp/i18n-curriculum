...
다음 메시지를 콘솔에 기록하세요: `gate.queue[tickIndex] + " attendees arriving."`.

# --hints--

`handleGateAtTick` 함수는 다음 메시지를 콘솔에 기록해야 합니다: `gate.queue[tickIndex] + " attendees arriving."`.
...
`handleGateAtTick` 함수는 주어진 tickIndex에서 게이트 대기열에 따라 올바른 참석자 수를 기록해야 합니다.
