...
حان الوقت الآن لاستخدام دالة `handleToggleVisibility` الخاصة بك.

في الدروس السابقة، تعلمت عن نظام الأحداث التركيبية في React وكيفية العمل مع الخصائص مثل `onClick` و`onSubmit`.

تذكر أنه يمكنك تمرير مرجع دالة إلى خاصية `onClick` بهذه الطريقة:
...
حدّث عنصر `button` الحالي لديك لإضافة خاصية `onClick` ومرر مرجع دالة `handleToggleVisibility`.

الآن، عند النقر على الزر، سترى مستوى الظهور لنص الفقرة يتبدل بين مخفي ومرئي على الصفحة.

# --hints--

عند النقر على `toggle-button`، يجب أن تظهر الرسالة `"I love freeCodeCamp!"` على الشاشة.
