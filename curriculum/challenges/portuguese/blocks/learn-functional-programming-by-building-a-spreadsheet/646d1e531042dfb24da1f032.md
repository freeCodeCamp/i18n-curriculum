...
O segundo argumento para o método `.replace()` não precisa ser uma string.Em vez disso, você pode passar uma função de callback para executar uma lógica mais complexa na string correspondente.

A função de callback receber alguns parâmetros.O primeiro é a string correspondente.Passe uma função de callback vazia para a chamada de `.replace()` e dê a ela um parâmetro `match`.

# --hints--

Você deve passar uma arrow function como segundo argumento para o método `.replace()`.
...
A arrow function deve ter um parâmetro `match`.
