...
通过在 `.back-mountain` 元素后创建一个新的 `div` 元素，并赋予其 `sun` 类，为背景添加一个太阳以完成背景。

# --hints--

你应该在 `body` 中添加一个新的 `div` 元素。 应该是 --fcc-expected-- `div` 元素，而不是 --fcc-actual--。

```js
assert.lengthOf(document.querySelectorAll('body > div'), 5);
```

你应该将新的 `div` 元素的 `class` 设置为 `sun`。

```js
assert.include(document.querySelector('div:not(.back-mountain, .left-mountain, .penguin, .ground)')?.className, 'sun');
```

你应该将新的 `div` 元素放在 `.back-mountain` 元素之后。
