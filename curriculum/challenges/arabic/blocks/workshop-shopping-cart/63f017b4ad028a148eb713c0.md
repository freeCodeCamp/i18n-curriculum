...
للتعبير الكاذب الخاص بك، ستحتاج إلى إضافة HTML جديد إلى `productsContainer`. ابدأ بإزالة `undefined`، ثم استخدم معامل التعيين بالإضافة وصيغة القالب النصي لإضافة `div` مع تعيين `class` إلى `product` و`id` إلى `dessert${id}` في خاصية `innerHTML` الخاصة بـ `productsContainer`.

# --hints--

يجب إزالة `undefined` من التعبير الكاذب الخاص بك.
...
يجب استخدام معامل التعيين بالإضافة لإضافة HTML إلى `productsContainer`. تذكر أن HTML يُضاف في خاصية `innerHTML`.
...
يجب استخدام صيغة القالب النصي لإضافة HTML إلى `productsContainer`.

```js
assert.match(code, /productsContainer\.innerHTML\s*\+=\s*`/);
```

يجب إضافة `div` إلى `productsContainer`.
...
يجب أن يحتوي `div` الخاص بك على `class` مضبوط على `product`.
...
يجب أن يحتوي `div` الخاص بك على `id` مضبوط على `dessert${id}`.
