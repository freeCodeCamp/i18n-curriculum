...
Puede que hayas notado que `EVAChunks` es esencialmente un arreglo con arreglos dentro, o un arreglo bidimensional (2D). Se puede usar un `for` anidado para registrar datos de un arreglo 2D:
...
En el ejemplo anterior, `rootArray` es el arreglo principal, `rootArray[i]` es un subarreglo dentro del arreglo principal, y `rootArray[i][j]` es un objeto dentro de ese subarreglo y `.property` accede a un valor en ese objeto.

Usa un `for` anidado para iterar a través de `EVAChunks` y registrar el `name` de cada astronauta en cada chunk. Asegúrate de:

- usa `EVAChunks` como tu arreglo raíz
- registra en el log `Chunk ${i+1}:` entre fragmentos

# --hints--

Debes usar un `for` anidado para iterar a través de `EVAChunks` y registrar el `name` de cada astronauta en cada chunk. Registra `Chunk ${i+1}:` entre chunks.
