...
为你的文档添加一个带有文本 `Magazine` 的 `title` 元素，一个用于 `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap` 字体样式表的 `link` 元素，一个用于 `https://use.fontawesome.com/releases/v5.8.2/css/all.css` FontAwesome 样式表的 `link`，以及一个用于你的 `./styles.css` 样式表的 `link`。
...
你的项目应该有一个标题为 `Magazine`。 请记住，标题的大小写和拼写很重要。
