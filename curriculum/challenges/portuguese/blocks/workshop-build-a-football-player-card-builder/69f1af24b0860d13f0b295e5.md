...
Agora substitua os números fixos restantes pelos seguintes:
...
Você deve substituir o valor fixo `99` por `DRI` com `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

Você deve substituir o valor fixo `41` por `DEF` com `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

Você deve substituir o valor fixo `99` por `PHY` com `player.phy`.
