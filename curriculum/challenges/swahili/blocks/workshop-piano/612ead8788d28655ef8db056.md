...
`.key.black--key::after` yako inapaswa kuwa na sifa ya `width` iliyowekwa kuwa `32px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.key.black--key::after')?.width, '32px');
```

`.key.black--key::after` yako inapaswa kuwa na sifa ya `height` iliyowekwa kuwa `100px`.
