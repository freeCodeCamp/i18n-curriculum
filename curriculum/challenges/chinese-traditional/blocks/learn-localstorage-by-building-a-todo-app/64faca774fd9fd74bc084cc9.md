...
只有在輸入欄位中有文字時，你才應該向使用者顯示 `Cancel` 和 `Discard` 按鈕。

首先移除 `confirmCloseDialog.showModal();` 這一行。

然後在 `closeTaskFormBtn` 事件監聽器中，撰寫一些邏輯來檢查 `titleInput`、`dateInput` **或** `descriptionInput` 欄位中是否有值。

如果任何那些輸入欄位有值，則對 `confirmCloseDialog` 使用 `showModal()` 方法。否則，如果沒有變更，呼叫 `reset()` 函式以清除輸入欄位並隱藏表單對話窗。

# --hints--

如果任何輸入欄位有值，則應顯示確認對話窗。否則，應呼叫 `reset` 函式（程式）。
