...
クリックされたことのないリンクのデフォルト色は、通常は青です。 すでに訪問されたことがあるリンクのデフォルト色は、通常は紫になります。

`footer` のリンクの色を、訪問されたかどうかに関わらず同じにするために、アンカー (`a`) 要素の要素型セレクターを使用して `color` プロパティに `black` の値を設定してください。
