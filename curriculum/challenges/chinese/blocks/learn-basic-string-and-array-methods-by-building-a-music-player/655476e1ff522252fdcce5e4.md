...
向 `if` 语句添加 `else` 块。 在 `else` 块内，调用 `getCurrentSongIndex()` 函数并将其分配给名为 `currentSongIndex` 的常量。
...
你应该在 `else` 块内将 `currentSongIndex` 常量设置为 `getCurrentSongIndex()`。
