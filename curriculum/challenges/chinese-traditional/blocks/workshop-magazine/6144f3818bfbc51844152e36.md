...
使用 `minmax` 函數使你的列在任何設備上都能響應。 `minmax` 函數有兩個參數，第一個是最小值，第二個是最大值。 這些值可以是長度、百分比、`fr`，甚至是像 `max-content` 這樣的關鍵字。

將 `grid-template-columns` 屬性的每個已定義的值放在 `minmax` 函數中作爲第二個參數。 第一個參數應分別爲 `2rem`、`min-content` 和 `2rem`。

# --hints--

你的 `main` 選擇器應該有一個 `grid-template-columns` 屬性。值爲 `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`。
