...
`src` の値が `https://cdn.freecodecamp.org/curriculum/cat-photo-app/relaxing-cat.jpg` に設定された `img` 要素が必要です。 You may have accidentally deleted it.
...
Your anchor (`a`) element should have an opening tag. 開始タグは `<elementName>` のような形式の構文です。
...
Your anchor (`a`) element should have a closing tag. 終了タグは `<` の直後に `/` があります。
