...
الآن عليك الاستعداد لتحديث العرض بالمنتج الجديد الذي أضافه المستخدم. أعلن عن متغير `currentProductCount`، وقم بتعيين قيمته إلى الخاصية في كائن `totalCountPerProduct` التي تطابق `id` الخاص بـ `product`.

# --hints--

يجب إعلان متغير `currentProductCount` في دالة `addItem` الخاصة بك.
...
يجب استخدام `const` لإعلان `currentProductCount`.
...
يجب تعيين قيمة `totalCountPerProduct[product.id]` إلى `currentProductCount`.
