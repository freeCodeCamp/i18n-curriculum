...
Викличте метод `.map()` на вашому виклику `rangeFromString()`, передаючи референс на `addCharacters` як функцію зворотного виклику.

# --hints--

Ви повинні викликати метод `.map()` на вашому виклику `rangeFromString()`.
...
Ви повинні передати референс на `addCharacters` як функцію зворотного виклику до вашого методу `.map()`.
