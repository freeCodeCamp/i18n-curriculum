...
Dovresti dare a `.beak.top` un valore `width` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.width, '20%');
```

Dovresti dare a `.beak.top` un valore `top` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.top, '60%');
```

Dovresti dare a `.beak.top` un valore `left` di `--fcc-expected--` invece di `--fcc-actual--`.
