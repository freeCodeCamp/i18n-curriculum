...
你需要一个实际更新 DOM 的函数。声明一个 `updateResultsList` 函数。

首先，函数应清除 `resultList` 元素中的超文本标记语言。

# --hints--

你应该定义一个 `updateResultsList` 函数。

```js
assert.isFunction(updateResultsList);
```

你的 `updateResultsList` 函数应将 `resultList.innerHTML` 设置为空字串。
