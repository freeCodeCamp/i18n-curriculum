...
將偽元素 `10%` 從其父元素的頂部定位，並將 `25%` 從左側定位。

# --hints--

你應該給 `.penguin-body::before` 一個 `top` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

你應該給 `.penguin-body::before` 一個 `left` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。
