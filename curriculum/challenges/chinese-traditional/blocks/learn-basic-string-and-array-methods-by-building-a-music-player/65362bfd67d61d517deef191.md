...
<dfn>find()</dfn> 方法會取得陣列中第一個符合所提供回呼函式中指定條件的元素。如果沒有元素符合條件，該方法會傳回 `undefined`。

在以下範例中，使用 `find()` 方法來尋找第一個大於 25 的數字：
...
使用 `const` 來創建一個名為 `song` 的變數，並將 `userData?.songs` 陣列上 `find()` 方法呼叫的結果指定給它。使用 `song` 作為 `find()` 回呼的參數，並檢查 `song.id` 是否嚴格相等於 `id`。

這將會迭代 `userData?.songs` 陣列，搜尋對應傳入 `playSong` 函式（程式）的 `id` 的歌曲。

# --hints--

你應該在 `userData?.songs` 上使用 `find` 方法。

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

你的 `find` 方法的箭頭函式回呼參數應該是 `song`。

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

你的 `find` 方法應該使用嚴格相等性來檢查 `song.id` 是否與 `id` 相等。

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

你的 `find` 方法應該指定給一個 `song` 常數。
