...
Під елементом `h2` у другій картці додайте елемент `p` з таким текстом:
...
Усередині другого елемента з класом `p` має бути вкладений елемент `card`.
...
Текст елемента `p` у другій картці має бути `This is the story of Dave as he learns to cook everything from pancakes to pasta, one recipe at a time.`
...
Елемент `p` у другій картці має бути розташований нижче за елемент `h2`.
