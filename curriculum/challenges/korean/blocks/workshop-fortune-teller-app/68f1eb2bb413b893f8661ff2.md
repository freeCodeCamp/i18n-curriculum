...
이제 카드를 클릭하면 각 카드가 보여주는 운세를 볼 수 있어야 합니다. 남은 것은 새 운세 보기 버튼이 작동하도록 만드는 것입니다.

`newReading` 메서드 안에서 `showElements()`과 `hideElements()`를 호출하세요.

# --hints--

`showElements` 메서드 안에서 `newReading` 함수를 호출해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

`hideElements` 메서드 안에서 `newReading` 함수를 호출해야 합니다.
