...
У класі `Bee` створіть конструктор, який приймає один параметр `emojiElement` типу `HTMLParagraphElement`.

У конструкторі викличте функцію `super` з параметром `emojiElement`, щоб викликати конструктор батьківського класу `Bug`. Потім встановіть властивість `emoji` у рядкове значення `"🐝"`.

# --hints--

Ваш клас `Bee` має мати конструктор.
...
Ваш конструктор `Bee` має мати параметр з іменем `emojiElement` типу `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Bee[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Ваш конструктор `Bee` має викликати `super` з параметром `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /super\s*\(\s*emojiElement\s*\)/);
```

Ваш конструктор `Bee` має встановити `this.emoji` у `"🐝"`.
