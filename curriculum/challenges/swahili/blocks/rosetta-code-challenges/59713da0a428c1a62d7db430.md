...
<a href="https://rosettacode.org/wiki/Cramer%27s_rule" target="_blank" rel="noopener noreferrer nofollow">Kanuni ya Cramer</a> ni fomula ya kutatua mfumo wa mlinganyo wa mstari kwa kutumia determinanti za matriki zilizoundwa kutoka kwa seti ndogo za viashiria na thamani za upande wa kulia.

Determinanti ya matriki yenye safu 2 na nguzo 2 hutolewa na:

$\begin{aligned}|A|={\begin{vmatrix}a&b\\\c&d\end{vmatrix}}=ad-bc.\end{aligned}$

Ikiwa na mfumo wa mlinganyo wa mstari:
...
ambao kwa muundo wa matriki ni
...
`cramersRule` inapaswa kuwa kitendakazi.

```js
assert(typeof cramersRule === 'function');
```

`cramersRule([[2, -1, 5, 1], [3, 2, 2, -6], [1, 3, 3, -1], [5, -2, -3, 3]], [-3, -32, -47, 49])` inapaswa kurudisha `[2, -12, -4, 1]`.

```js
assert.deepEqual(cramersRule(matrices[0], freeTerms[0]), answers[0]);
```

`cramersRule([[3, 1, 1], [2, 2, 5], [1, -3, -4]], [3, -1, 2])` inapaswa kurudisha `[1, 1, -1]`.
