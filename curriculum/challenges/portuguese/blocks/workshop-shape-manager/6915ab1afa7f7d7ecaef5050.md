...
Para finalmente mostrar os cálculos, itere sobre o objeto `propertyInputs` usando um loop `for...of` com `Object.entries()`. Você pode ignorar a chave usando uma vírgula antes de `input` na declaração do loop.

# --hints--

Você deve criar um loop `for...of` que desconstrua cada entrada de `Object.entries(propertyInputs)` pulando a chave e mantendo apenas `input`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Seu loop `for...of` deve iterar sobre `Object.entries(propertyInputs)`.
