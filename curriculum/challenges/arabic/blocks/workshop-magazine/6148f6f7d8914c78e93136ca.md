...
أنشئ استعلام `@media` نهائي واحد لـ `only screen` مع `max-width` بقيمة `420px`. بداخله، أنشئ محدِّد `.hero-title` مع خاصية `font-size` مضبوطة على `4.5rem`.

تهانينا! المجلة الخاصة بك أصبحت مكتملة الآن.

# --hints--

يجب أن يكون لديك استعلام `@media` جديد لـ `only screen and (max-width: 420px)`. يجب أن يكون هذا آخر استعلام في قائمة استعلامات `@media`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

يجب أن يحتوي استعلام `@media` الجديد على محدِّد `.hero-title`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

يجب أن يحتوي محدِّد `.hero-title` على خاصية `font-size` مضبوطة على `4.5rem`.
