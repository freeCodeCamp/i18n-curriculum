...
All'interno di ogni elemento `label` corrispondente, e subito dopo l'elemento `input`, aggiungi uno spazio e il seguente testo:
...
Dovresti dare al primo `label` testo uno spazio davanti.

```js
assert.equal(document.querySelector('fieldset:nth-child(2) > label')?.innerText?.[0], ' ');
```

Dovresti dare al secondo `label` testo uno spazio davanti.
