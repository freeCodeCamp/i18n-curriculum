...
プレイリストの削除機能を実装する時が来ました。これはプレイリストから曲を削除し、曲が削除されたときに関連する他の処理を管理し、`Reset Playlist` ボタンを作成します。

`const` とアロー構文を使って空の `deleteSong` 関数を作成し、`id` をパラメータとして渡してください。

# --hints--

`const` を使って `deleteSong` という名前の空の関数を作成してください。

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` は関数である必要があります。

```js
assert.isFunction(deleteSong)
```

`deleteSong` 関数はアロー構文を使うべきです。

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

`deleteSong` 関数は `id` パラメータを受け取るべきです。

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

`deleteSong` 関数は空であるべきです。
