...
如果你不熟悉 `let`，请查看<a href="/learn/javascript-algorithms-and-data-structures/basic-javascript/explore-differences-between-the-var-and-let-keywords" target="_blank" rel="noopener noreferrer nofollow">关于 <code>let</code> 和 <code>var</code> 之间区别的这个挑战</a>。
...
`i` 未定义，因为它没有在全局作用域中声明。它只在 `for` 循环语句内声明。`printNumTwo()` 返回了正确的值，因为在循环语句内通过 `let` 关键字创建了三个具有唯一值（0、1 和 2）的不同 `i` 变量。
