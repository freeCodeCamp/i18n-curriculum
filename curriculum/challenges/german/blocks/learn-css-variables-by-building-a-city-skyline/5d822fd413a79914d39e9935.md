...
Fügen Sie diesem Gebäude einen weiteren sich wiederholenden Farbverlauf hinzu; er soll genauso sein wie der, den Sie gerade hinzugefügt haben, aber ohne die `90deg`-Richtung und verwenden Sie Ihre Fensterfarbe anstelle der beiden `transparent`-Farben.

# --hints--

Sie sollten `.fb6` eine zweite `repeating-linear-gradient` in der `background`-Eigenschaft geben.
...
Sie sollten dem zweiten `repeating-linear-gradient` eine erste Farbe von `--building-color3` von `0%` bis `10%` geben.
...
Sie sollten dem zweiten `repeating-linear-gradient` eine zweite Farbe von `--window-color3` von `10%` bis `30%` geben.
