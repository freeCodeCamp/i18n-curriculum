...
Ahora agrega un `div` con el `className` de `form-group`. Dentro, añade un `label` de `className` de `label` y un atributo `htmlFor` de `name`. Debajo, agrega un `input` con un `id` establecido en `name`, un `className` de `input` y un atributo `type` de `text`.
