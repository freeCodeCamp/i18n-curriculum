...
사용자가 입력 필드에 입력하면, 쿼리와 일치하는 과일 목록이 나타나야 합니다.

이를 처리하려면, 초기값이 빈 배열(`results`)인 `[]`라는 상태 변수를 만드세요. 또한 `setResults` 훅을 사용해 이 변수의 설정 함수인 `useState`도 정의하세요.

# --hints--

`results`와 `setResults` 변수에는 배열 구조 분해 구문을 사용해야 합니다.
