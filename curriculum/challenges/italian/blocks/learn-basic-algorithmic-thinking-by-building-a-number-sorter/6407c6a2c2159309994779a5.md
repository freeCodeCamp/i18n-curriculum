...
Imposta la proprietà `innerText` di `outputValueNode` su `num`.

# --hints--

Dovresti usare `innerText` per impostare il testo di `outputValueNode`.
...
Dovresti usare `innerText` per impostare il testo di `outputValueNode` su `num`.
