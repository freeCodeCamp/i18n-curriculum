...
`To be okay` means to be acceptable or permitted. Por ejemplo:

`Is it okay if I borrow your book?` – This means asking if it is acceptable to borrow the book.

`Briefly` means that an action (normally, a verb) happens for a short period of time or is talked about using in a few words. It differs from `brief`, which is an adjective to describe something that is short in duration or length. Por ejemplo:
