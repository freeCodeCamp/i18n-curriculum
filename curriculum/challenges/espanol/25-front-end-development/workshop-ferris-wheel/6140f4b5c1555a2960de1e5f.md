...
Crea otra regla `@keyframes` con el nombre `cabins`. Usa las mismas propiedades que tu `@keyframes wheel`, copiando tanto las reglas `0%` como `100%`, pero ajusta la propiedad `transform` del selector `100%` a `rotate(-360deg)`.
...
Tu nueva regla `@keyframes` debe aparecer después de tu regla `@keyframes wheel`.
...
Tu selector `0%` debe tener una propiedad `transform` configurada en `rotate(0deg)`.
...
Tu selector `100%` debe aparecer después de tu selector `0%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '100%')
```

Tu selector `100%` debe tener una propiedad `transform` configurada en `rotate(-360deg)`.
