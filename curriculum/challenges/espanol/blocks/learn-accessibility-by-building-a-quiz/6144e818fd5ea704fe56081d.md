...
Debes darle al primer `fieldset` un atributo `name` adecuado. *Consejo: yo usaría `html-question-one`*

```js
assert.notEmpty(document.querySelectorAll('fieldset')?.[0]?.name);
```

Debes darle al segundo `fieldset` un atributo `name` adecuado. *Consejo: usaría `html-question-two`*
