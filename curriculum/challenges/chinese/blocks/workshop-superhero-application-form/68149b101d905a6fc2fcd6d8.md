...
Call the `setPowers` function, which takes an array argument. Use a ternary operator to check if `checked` is true. If it is, spread the existing `powers` array into a new array and add the `value` to it. 如果不是，则使用 `powers.filter(p => p !== value)` 从 `powers` 中过滤掉 `value`。

# --hints--

你应该使用三元运算符来检查 `checked` 是否为真。 如果是，则将现有的 `powers` 和 `value` 分散到数组中。 如果不是，则从 `powers` 中过滤掉 `value`。
