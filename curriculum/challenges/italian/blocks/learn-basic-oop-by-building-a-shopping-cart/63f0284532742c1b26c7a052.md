...
Aggiungi la tua funzione di callback al metodo `forEach`. Deve prendere un parametro `btn`. Poi, nella callback, aggiungi un event listener a `btn`. L'event listener deve ascoltare un evento `click` e deve prendere un'altra callback con un parametro `event`. La seconda callback deve essere vuota.

# --hints--

Dovresti usare la sintassi arrow per aggiungere una funzione di callback al metodo `forEach` che prende un parametro `btn`.
...
Dovresti ascoltare un evento `click` sulla variabile `btn`.
...
Dovresti aggiungere una funzione di callback vuota all'event listener. Ricorda di darle un parametro `event`.
