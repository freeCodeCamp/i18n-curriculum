...
Usa `personal-account` e `business-account` come valori per i rispettivi attributi `id`.

# --hints--

Il primo elemento `input` dovrebbe avere un `id` di `personal-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[0]?.matches('#personal-account'))
```

Il secondo elemento `input` dovrebbe avere un `id` di `business-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[1]?.matches('#business-account'))
```

Il primo elemento `label` dovrebbe avere un attributo `for` con valore `personal-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) label')?.[0]?.matches('label[for="personal-account"]'))
```

Il secondo elemento `label` dovrebbe avere un attributo `for` con valore `business-account`.
