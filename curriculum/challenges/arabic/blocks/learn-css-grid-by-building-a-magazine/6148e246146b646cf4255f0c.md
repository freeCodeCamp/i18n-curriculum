...
أنشئ محدِّد `.list-title, .list-subtitle` واضبط خاصية `color` على `#00beef`.

# --hints--

يجب أن يكون لديك محدِّد `.list-title, .list-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'))
```

يجب أن يحتوي محدِّد `.list-title, .list-subtitle` على خاصية `color` مضبوطة على `#00beef`.
