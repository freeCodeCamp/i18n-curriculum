...
**SHA-1** o **SHA1** è una funzione di hash unidirezionale; calcola un digest del messaggio di 160 bit.

SHA-1 appare spesso nei protocolli di sicurezza; per esempio, molti siti HTTPS usano RSA con SHA-1 per proteggere le loro connessioni.

BitTorrent usa SHA-1 per verificare i download.

Git e Mercurial usano digest SHA-1 per identificare i commit.

Uno standard del governo degli Stati Uniti, <a href="https://rosettacode.org/wiki/SHA-1/FIPS-180-1" target="_blank" rel="noopener noreferrer nofollow">FIPS 180-1</a>, definisce SHA-1.

# --instructions--

Scrivi una funzione che restituisca il digest SHA-1 per una data stringa.
