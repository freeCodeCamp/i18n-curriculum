...
Adesso la funzione `update` deve utilizzare gli argomenti che le vengono passati.

All'interno della funzione `update`, cambia il valore dell'assegnazione di `button1.innerText` in `location["button text"]`. That way, you use bracket notation to get the `"button text"` property of the `location` object passed into the function.
