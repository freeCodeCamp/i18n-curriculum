...
ユーザーが10進数の`5`を2進数に変換しようとしたときにアニメーションを表示するので、そのチェックを`checkUserInput()`関数内に追加する必要があります。

`if`文を使って、`numberInput`の`value`属性が数値の`5`と等しいかどうかを確認してください。`5`と比較する前に、文字列を数値に変換するために`parseInt()`関数を使うことを忘れないでください。`if`文は今のところ空のままにしてください。

# --hints--

既存の`if`文の後に、`checkUserInput()`関数に2つ目の`if`文を追加してください。

```js
assert.match(String(checkUserInput), /if\s*\(\s*!numberInput\s*\.\s*value\s*\|\|\s*isNaN\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)[\s\S]+if\s*\(/);
```

新しい`if`文の条件内で`parseInt()`関数を使うべきです。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(/);
```

`numberInput`の`value`属性が`5`と等しいかどうかを確認してください。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)/);
```

新しい`if`文の本体は空にしてください。
