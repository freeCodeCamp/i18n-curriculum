...
ابدأ، بإعطاء العناصر `input` في `fieldset` الثاني class بقيمة `inline`.

# --hints--

يجب عليك إعطاء أول `input` الـ class بقيمة `inline`.

```js
assert(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

يجب عليك إعطاء ثاني `input` الـ class بقيمة `inline`.
