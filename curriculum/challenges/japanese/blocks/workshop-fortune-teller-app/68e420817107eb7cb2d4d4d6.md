...
画像要素に`onerror`ハンドラを追加して、読み込み失敗を処理してください。最初の失敗時には`dataset.failed`を使ってマークし、ソースをローカルのデフォルト画像に置き換えます。画像が再度失敗した場合は、hiddenクラスを削除し、`previousElementSibling`を使って読み込みプレースホルダーを非表示にしてください。

# --hints--

`img`要素には`onerror`ハンドラが必要です。
...
`onerror`ハンドラには、`if`が偽の場合をチェックする`dataset.failed`文を含めてください。
...
`dataset.failed`は`'1'`に設定し、`if`ブロック内で行います。ここで`this`キーワードを忘れないでください。
...
`if`ブロックでは、`this.src`をローカルのデフォルト画像に更新してください。ここでも`this`キーワードを忘れないでください。
...
`hidden` クラスを削除する `else` ブロックを用意してください。ここで `this` キーワードを忘れないでください。
...
`else`ブロックでは、`previousElementSibling.style.display`を`'none'`に設定してください。`this`キーワードを忘れないでください。
