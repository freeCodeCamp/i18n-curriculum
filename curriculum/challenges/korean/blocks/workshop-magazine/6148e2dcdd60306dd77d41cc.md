...
잡지의 마지막 섹션인 이미지에 스타일을 적용할 시간입니다.

이미지는 `aside` 요소로 감싸져 있으며 `image-wrapper` 클래스를 사용하므로 `.image-wrapper` 선택자를 만드세요. `display` 속성을 `grid`로 설정하세요.

# --hints--

`.image-wrapper` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

`.image-wrapper` 선택자에 `display` 속성을 `grid`로 설정해야 합니다.
