...
现在是时候测试 `playNextSong` 函数了。 

向 `nextButton` 元素添加 `"click"` 事件监听器，然后传入 `playNextSong` 作为事件监听器的第二个参数。 这是事件监听器将运行的函数。

通过首先单击“播放”按钮然后单击“下一步”按钮来测试你的应用。 你应该看到一切都按预期进行。

# --hints--

你应该在 `nextButton` 变量上调用 `addEventListener()` 方法。
...
你应该将 `playNextSong` 作为 `addEventListener` 方法的第二个参数传入。
