...
ご覧のとおり、`series1` は親の `Movie` メソッドを継承しているため、`__str__` オブジェクトと同じ形式で表示されています。次のステップでは、`__str__` メソッドをオーバーライドしてカスタムの文字列表現を提供します。

今は、コードから `print(series1)` の行を削除してください。

# --hints--

コードに `print(series1)` が含まれていないことを確認してください
