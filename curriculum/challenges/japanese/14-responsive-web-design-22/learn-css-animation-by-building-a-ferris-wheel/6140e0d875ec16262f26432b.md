...
`animation-timing-function`プロパティは、時間の経過と共にアニメーションがどう進行するかを設定します。 このプロパティに設定できる値はいくつかありますが、あなたは観覧車のアニメーションを最初から最後まで同じ速度で動かそうとしています。 `.wheel` セレクター内の `animation-timing-function` を `linear` に設定してください。

# --hints--

`.wheel` セレクターの `animation-iteration-count` プロパティを `infinite` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.wheel')?.animationIterationCount === 'infinite');
```

`.wheel` セレクターの `animation-timing-function` プロパティを `linear` に設定する必要があります。
