...
Sasa, unapaswa kuunda mduara wa ndani ambao utaonekana wakati kitufe cha radio kikiwa kimechaguliwa. Utatumia kipengele cha bandia cha `::before` kwa hili.

Kipengele cha bandia kama `::before` hukuruhusu kuingiza maudhui ya ziada kabla ya kipengele halisi. Hii mara nyingi hutumika kwa madhumuni ya mapambo.

Ukilenga kipengele cha bandia cha `.contact-method-radio-btn::before`, weka sifa ya `display` kuwa `block`, sifa ya `content` kuwa `" "`, sifa ya `width` kuwa `10px`, sifa ya `height` kuwa `10px`, na sifa ya `border-radius` kuwa `50%`.

# --hints--

Unapaswa kuwa na kichaguzi cha `.contact-method-radio-btn::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Weka sifa ya `display` kuwa `block`.
...
Weka sifa ya `border-radius` kuwa `50%`.
