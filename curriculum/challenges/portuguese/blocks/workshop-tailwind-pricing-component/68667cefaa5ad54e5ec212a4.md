...
Indo para o elemento da lista, adicione as classes `mt-6`, `space-y-2`, `text-sm` e `text-fuchsia-100` ao `ul` para espaçar os itens da lista e deixá-los mais atraentes.

# --hints--

Seu elemento `ul` deve ter a classe `mt-6`.
...
Seu elemento `ul` deve ter a classe `space-y-2`.
...
Seu elemento `ul` deve ter a classe `text-sm`.
...
Seu elemento `ul` deve ter a classe `text-fuchsia-100`.
