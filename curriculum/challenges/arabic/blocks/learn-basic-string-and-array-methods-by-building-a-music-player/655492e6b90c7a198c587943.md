...
الآن عليك إضافة الخاصية مرة أخرى إلى الأغنية التي تُشغل حاليًا.

أنشئ جملة `if` بشرط `songToHighlight`. استخدم في الجملة `setAttribute` على `songToHighlight` لتمرير `"aria-current"` و`"true"` كالمعلمة الأولى والثانية.

# --hints--

يجب أن تنشئ جملة `if` بشرط `songToHighlight`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

يجب أن تستخدم الطريقة `setAttribute()` على `songToHighlight` داخل جملة `if` الخاصة بك.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

يجب أن تمرر `"aria-current"` و`"true"` كالمعلمة الأولى والثانية لطريقة `setAttribute`.
