...
Per iniziare a lavorare con i dati, dovrai usare un altro metodo `.then()`.

Concatena un altro `.then()` al metodo `.then()` esistente. Questa volta, passa `data` come parametro per la funzione di callback. Per la callback, usa le parentesi graffe perché avrai più di un'espressione. All'interno della tua funzione di callback, registra `data` nella console per vedere com'è fatto.
...
La tua prima `.then()` non dovrebbe avere un punto e virgola.
...
Il tuo secondo `.then()` non dovrebbe avere un punto e virgola dopo.
