...
도큐스트링은 클래스나 함수의 첫 번째 문장으로 위치하는 문자열로, 해당 클래스나 함수에 대한 문서화를 제공하는 데 사용됩니다. 도큐스트링은 보통 세 개의 큰따옴표로 감싸집니다:
...
다음 도큐스트링을 추가하여 `Movie` 클래스를 완성하세요: `Parent class representing a movie.`

# --hints--

도큐스트링 `Parent class representing a movie.`를 `Movie` 클래스에 추가해야 합니다.
