...
创建一个名为 `outputValueNode` 的变量，并将其值设置为调用 `document.getElementById()` 方法的结果。 使用模板字面量语法将 `` `output-value-${i}` `` 字符串传递给 `.getElementById()`。

# --hints--

使用 `const` 在 `.forEach()` 回调中声明一个 `outputValueNode` 变量。

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=/)
```

`outputValueNode` 应该被赋值为调用 `document.getElementById()` 的结果。

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=\s*document\.getElementById\s*\(/)
```

使用模板字面量语法将 `output-value-${i}` 字符串传递给 `.getElementById()`。
