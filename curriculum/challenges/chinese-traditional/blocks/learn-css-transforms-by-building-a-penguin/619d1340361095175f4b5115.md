...
企鵝的身體看起來有點單調。 在 `.penguin-body` 前面添加一個 `class` 爲 `shirt` 的 `div` 元素，來裝扮一下它。

# --hints--

你應該在 `.penguin` 中添加一個 `div` 元素。 應該是 `--fcc-expected--` `div` 元素，而不是 `--fcc-actual--`。

```js
assert.equal(document.querySelectorAll('.penguin > div')?.length, 3);
```

你應該給新的 `div` 一個 `shirt` `class`。

```js
assert.exists(document.querySelector('.penguin > div.shirt'));
```

你應該將新的 `div` 放在 `.penguin-body` 前面。
