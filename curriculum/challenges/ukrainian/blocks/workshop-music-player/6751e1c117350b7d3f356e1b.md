...
В межах функції `playSong` викличте функцію `highlightCurrentSong`.

Після цього пограйте з кнопками керування, щоб побачити, як працює функція `highlightCurrentSong`.
