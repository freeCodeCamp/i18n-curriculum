...
Erstellen Sie einen `body`-Selektor. Setzen Sie die `font-family`-Eigenschaft auf `Baskervville` mit einem Fallback von `serif`. Setzen Sie die `color`-Eigenschaft auf `linen` und die `background-color`-Eigenschaft auf `rgb(20, 30, 40)`.
...
Ihr `body`-Selektor sollte eine `font-family`-Eigenschaft haben, die auf `Baskervville` gesetzt ist, mit einem Fallback von `serif`.
...
Ihr `body`-Selektor sollte eine `color`-Eigenschaft haben, die auf `linen` gesetzt ist.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.color === 'linen');
```

Ihr `body`-Selektor sollte eine `background-color`-Eigenschaft haben, die auf `rgb(20, 30, 40)` gesetzt ist.
