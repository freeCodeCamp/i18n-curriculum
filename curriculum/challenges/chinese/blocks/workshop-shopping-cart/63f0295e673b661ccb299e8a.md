...
你需要一种方法来访问购物车中的商品总数。 最好的方法是向 `ShoppingCart` 类添加另一种方法，而不是直接访问 `items` 数组。

向 `ShoppingCart` 类添加 `getCounts` 方法。 它应该返回 `items` 数组中的项目数。

# --hints--

你的 `ShoppingCart` 类应该有一个 `getCounts` 方法。
...
你的 `getCounts` 方法应该返回 `items` 数组中的项目数。
