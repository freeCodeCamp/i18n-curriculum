...
新しい `@media` クエリを作成し、`only screen` かつ `max-width` が `550px` の場合を対象にしてください。 このクエリは前の 2 つの後に置く必要があります。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

新しい `@media` ルールには `.hero-title` セレクター、`.hero-subtitle, .author, .quote, .list-title` セレクター、`.social-icons` セレクター、および `.text` セレクターが必要です。 以上のセレクターはこの順序でなければなりません。
