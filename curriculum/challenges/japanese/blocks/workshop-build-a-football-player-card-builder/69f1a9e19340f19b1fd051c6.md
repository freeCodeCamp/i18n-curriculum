...
各 `span` に、それぞれ `className` と `stat-value` の `stat-label` を持つ `stat-row` 要素を2つ追加してください。

# --hints--

2番目の `.stat-row` の各 `.stat-col` は、クラス `span` の `stat-value` とクラス `span` の `stat-label` を含む必要があります。
