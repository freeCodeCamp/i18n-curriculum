...
`.quote` セレクターを作成してください。 その `color` プロパティを `#00beef` に、`font-size` プロパティを `2.4rem` に、`text-align` プロパティを `center` に設定してください。
...
`.quote` セレクターの `color` プロパティを `#00beef` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.color === 'rgb(0, 190, 239)');
```

`.quote` セレクターの `font-size` プロパティを `2.4rem` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize === '2.4rem');
```

`.quote` セレクターの `text-align` プロパティを `center` に設定する必要があります。
