...
title: Opzioni identiche multiple nelle istruzioni switch
challengeType: 1
forumTopicId: 18242
...
Se l'istruzione `break` viene omessa da un `switch` nella sua `case`, le istruzioni `case` successive vengono eseguite fino a quando non si incontra un `break`. Se hai più input con lo stesso output, puoi rappresentarli in un'istruzione `switch` in questo modo:
...
Scrivi un'istruzione switch per impostare `answer` per i seguenti intervalli:  
`1-3` - `Low`  
`4-6` - `Mid`  
`7-9` - `High`

**Nota:** Ti servirà un'istruzione `case` per ogni numero nell'intervallo.
...
Dovresti avere nove istruzioni `case`
