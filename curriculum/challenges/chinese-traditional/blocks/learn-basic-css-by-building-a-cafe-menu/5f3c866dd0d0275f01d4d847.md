...
好吧，那樣做行不通。 給 `p` 元素添加 `inline-block` 樣式，並將它們放置在代碼中的單獨行，此時會在第一個 `p` 元素的右側創建一個空間，進而導致第二個元素轉移到下一行。

解決此問題的一種方法：可以將每個 `p` 元素的寬度設置爲略小於 `50%`。 將每個 類 的 `width` 值更改爲 `49%`，看看會發生什麼。
