...
프로젝트의 로직을 준비하려면 `const`와 화살표 구문을 사용해 `sortInputArray` 함수를 선언하세요. 이 함수는 하나의 `event` 매개변수를 받아야 합니다.

# --hints--

`sortInputArray` 함수를 `const`로 선언해야 합니다.

```js
assert.match(code, /const\s+sortInputArray\s*=/);
```

`sortInputArray`는 함수여야 합니다.

```js
assert.isFunction(sortInputArray);
```

`sortInputArray`는 하나의 `event` 매개변수를 받아야 합니다.

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)/);
```

`sortInputArray`는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)\s*=>/);
```

`sortInputArray` 함수는 비어 있어야 합니다.
