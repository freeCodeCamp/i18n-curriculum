...
Sasa umeunda kitufe, unahitaji kuweka thamani ya maandishi kwake. Ili kufanya hivyo, unahitaji kutumia njia ya `createTextNode()` ya DOM.

Njia ya `createTextNode()` hutumika kuunda nodi ya maandishi. Ili kuitumia, unaitisha na kuipatia maandishi kama mfuatano wa herufi:

```js
document.createTextNode("your text")
```

Unaweza pia kuiweka kwenye kigezo:

```js
const myText = document.createTextNode("your text")
```

Tumia njia ya `createTextNode()` kuunda maandishi ya `"Reset Playlist"`, kisha weka thamani yake kwenye kigezo cha `resetText`.
...
Unapaswa kutumia `document.createTextNode()` kuunda maandishi ya `"Reset Playlist"`.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

Unapaswa kuweka nodi ya maandishi uliyounda kwenye kigezo cha `resetText`.
