...
Hasta ahora, no cambia nada cuando seleccionas un error diferente.

Agrega un listener de eventos `change` a `selectElement`. En la función de callback, usa `isSelect` para verificar si `e.target` es un `HTMLSelectElement`. Aún no necesitas hacer nada dentro del bloque `if`.

# --hints--

Tu `selectElement` debe tener un listener de eventos `change` con una función de callback que reciba un parámetro de evento.
...
La función de callback de tu listener de eventos debe tener una sentencia `if` con la condición `isSelect(e.target)`.
