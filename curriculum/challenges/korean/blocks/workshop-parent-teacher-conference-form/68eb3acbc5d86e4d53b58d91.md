...
버튼을 가운데 정렬하려면 `font-size`를 `1.1rem`로, `display`를 `block`로, `margin`를 `auto`로 설정하세요.

# --hints--

계속해서 `.submit-btn` 클래스를 대상으로 지정하세요.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

`font-size` 속성을 `1.1rem`로 설정하세요.
