...
現在、`songsHTML` は配列です。 このまま表示しようとすると、曲がコンマで区切られて表示されます。 これは望ましい結果ではありません。なぜなら、曲を HTML のリストとして表示したいからです。 これを修正するには、<dfn>join()</dfn> メソッドを使用して、配列を 1 つの文字列に結合する必要があります。

`join()` メソッドは、配列のすべての要素を連結して 1 つの文字列にするために使用します。 またこのメソッドは `separator` (区切り文字) という省略可能な引数をとり、配列の各要素を区切るために使用します。 例えば、次のように記述できます。
...
では、`map()` メソッドに `join()` メソッドをチェーンして、区切り文字として空の文字列を渡してください。

複数のメソッドを「チェーンする」とは、`map()` メソッドの結果に対して `join()` メソッドを呼び出すことです。 例えば、次のようになります。
