...
Poi, usa la proprietà `classList` e il metodo `add()` per aggiungere la classe `playing` all'elemento `playButton`. Questo cercherà la classe `playing` nel file CSS e la aggiungerà all'elemento `playButton`.

Quindi, imposta `userData.currentSong` su `song` e per far partire la canzone, usa il metodo `play()` sulla variabile `audio`. `play()` è un metodo dell'API audio web per riprodurre un file mp3.

# --hints--

Dovresti aggiungere la classe `playing` all'elemento `playButton` all'interno della tua funzione `playSong`.
...
Dovresti impostare `userData.currentSong` su `song` all'interno della tua funzione `playSong`.
...
Dovresti chiamare il metodo `play()` sulla variabile `audio` all'interno della tua funzione `playSong`.
