...
لكي تتمكن من التنقل داخل الصفحة، أعطِ كل عنصر وسم الرابط `href` يتوافق مع `id` لعناصر `h2`.

# --hints--

يجب أن تعطي أول عنصر `a` قيمة `href` تساوي `#student-info`.

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

يجب أن تعطي ثاني عنصر `a` قيمة `href` تساوي `#html-questions`.

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

يجب أن تعطي ثالث عنصر `a` قيمة `href` تساوي `#css-questions`.
