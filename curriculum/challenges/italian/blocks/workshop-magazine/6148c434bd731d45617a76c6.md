...
Questa proprietà accetta come primo valore `row` o `column`, con un secondo valore opzionale `dense`. `grid-auto-flow` usa un algoritmo di posizionamento automatico per adattare il layout della griglia. Impostandolo su `column` l'algoritmo creerà nuove colonne per il contenuto secondo necessità. Il valore `dense` permette all'algoritmo di tornare indietro e riempire gli spazi vuoti nella griglia con elementi più piccoli, il che può far apparire gli elementi fuori ordine.
