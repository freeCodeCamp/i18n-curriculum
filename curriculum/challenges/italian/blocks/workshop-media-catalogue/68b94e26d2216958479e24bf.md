...
Sarebbe utile validare l'elemento passato al metodo `add` prima di aggiungerlo al catalogo.

Crea un'istruzione `if` che usi `isinstance` per verificare se `media_item` è un'istanza di `Movie` e sollevi un `TypeError` con il messaggio `Only Movie or TVSeries instances can be added`.

Nota che `isinstance(obj, ClassName)` restituisce `True` se `obj` è un'istanza di `ClassName` o *di una qualsiasi delle sue sottoclassi*, perché considera l'intera catena di ereditarietà.

# --hints--

Dovresti sollevare un `TypeError` con il messaggio `Only Movie or TVSeries instances can be added` quando `media_item` non è un'istanza di `Movie` o delle sue sottoclassi.
...
Non dovresti sollevare un `TypeError` quando `media_item` è un elemento valido.
