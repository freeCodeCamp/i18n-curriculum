...
프로젝트가 이제 HTML DOM 요소와 작동하므로, 포춘을 DOM에 렌더링하는 방법이 필요합니다.

네 개의 인수 `renderCard`, `drawingType`, `isReversed`, `shortName`를 받는 `img`라는 함수를 선언하세요.

매개변수 `drawingType`, `shortName`, `img`는 `string` 타입이어야 하며, `isReversed`는 `boolean` 타입이어야 합니다. 함수의 반환 타입은 `string`이어야 합니다.

# --hints--

`renderCard`라는 함수가 있어야 합니다.
...
`renderCard` 함수는 네 개의 매개변수를 가져야 합니다.
...
첫 번째 매개변수는 `drawingType`이며 `string` 타입이어야 합니다.
...
두 번째 매개변수는 `isReversed`이며 `boolean` 타입이어야 합니다.
...
세 번째 매개변수는 `shortName`이며 `string` 타입이어야 합니다.
...
네 번째 매개변수는 `img`이며 `string` 타입이어야 합니다.
...
`renderCard` 함수의 반환 타입은 `string`이어야 합니다.
...
`renderCard` 함수는 템플릿 리터럴을 반환해야 합니다.
