...
Itakuwa na msaada kuwa na kitendakazi cha kuonyesha muhtasari wa throughput wakati wa uigaji. Unda kitendakazi tupu kinachoitwa `printSummary` chenye kigezo `summary`.

# --hints--

Unapaswa kuunda kitendakazi kinachoitwa `printSummary`.

```js
assert.isFunction(printSummary);
```

Kitendakazi chako cha `printSummary` kinapaswa kuwa na kigezo `summary`.
