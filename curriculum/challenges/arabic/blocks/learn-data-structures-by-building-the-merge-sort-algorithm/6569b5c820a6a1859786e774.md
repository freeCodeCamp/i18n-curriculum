...
داخل حلقة `while`، عيّن `right_part[right_array_index]` إلى `array[sorted_index]`.

# --hints--

يجب أن يكون لديك `array[sorted_index] = right_part[right_array_index]` في جسم الحلقة. لا تنسَ إزالة الكلمة المفتاحية `pass` من جسم الحلقة.
...
يجب ألا توجد كلمة `pass` في حلقة `while`
