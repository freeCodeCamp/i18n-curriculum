...
Зверніть увагу, що обкладинка альбому в HTML та пісні в масиві `userData.songs` змінилися. We've swapped out the original songs for shorter ones that you can use to test your app.
...
To fix that, you can set up an event listener which will detect when the currently playing song ends. The `"ended"` event listener is appropriate for this. Він запускається, коли відтворення медіафайлу досягає кінця.
...
Під'єднайте метод `addEventListener()` до своїй змінній `audio`.
