...
Como antes, você deve validar sua entrada. Especificamente, se `size` for menor que `1`, você deve registrar `"Chunk size must be >= 1"` no console e então fazer a chamada `return` para sair da função.

# --hints--

Se o `size` da entrada for inválido, você deve registrar a seguinte mensagem de erro no console, `"Chunk size must be >= 1"`.
...
Após registrar a mensagem de erro, você deve imediatamente fazer a chamada `return` para sair da função antecipadamente.
...
Sua `chunkCrew` função deve passar pela validação sem erros quando `size` for maior ou igual a 1.
