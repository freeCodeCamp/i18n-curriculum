...
`+=` 演算子を使用して、`text.innerText` の末尾に `" Your <weapon> breaks."` を追加してください。`Your` の前にはスペースを入れましょう。 `<weapon>` の部分は `inventory` 配列の最後のアイテムに置き換えてください。`inventory.pop()` を使用すると、配列の最後のアイテムを取り除いて返すので、それを文字列で使用できます。

# --hints--

`text.innerText` を変更するために複合代入演算子を使用してください。
...
`inventory` 配列に対して `pop` メソッドを使用する必要があります。
