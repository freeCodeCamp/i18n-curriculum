...
删除您的 `print` 调用，并在您的`while`循环语句后创建一个名为`targets_to_print`的变量。
 当 `target` 为真时，使用三元语法赋值为 `[target]`，否则为 `graph`。
...
当target为真时，你应该使用三元语法将`[target]`赋值给`target`，否则`graph`赋值给你的`targets_to_print`变量。
