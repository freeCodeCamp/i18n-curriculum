...
`draw` 메서드 안에서 `"#acd157"`을 `ctx.fillStyle`에 할당하세요.

그 아래에 `ctx.fillRect` 메서드를 호출하고 `x`와 `y` 좌표, 그리고 `width`와 `height` 속성을 전달하세요. 각 속성 앞에 `this`을 포함하는 것을 잊지 마세요.

# --hints--

`"#acd157"`을 `ctx.fillStyle`에 할당해야 합니다.

```js
assert.match(code, /ctx\.fillStyle\s*=\s*('|")#acd157\1\s*;?/);
```

올바른 인수를 전달하여 `ctx.fillRect` 메서드를 호출해야 합니다.
