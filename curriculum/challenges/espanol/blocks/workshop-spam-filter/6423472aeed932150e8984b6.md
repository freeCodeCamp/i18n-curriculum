...
Utilizando la misma sintaxis, actualiza `c` para coincidir con `c`, `{`, `[`, y `(`.

# --hints--

Tu `stockRegex` debería usar una clase de caracteres para coincidir con `c`, `{`, `[`, y `(`.
...
Tu `stockRegex` debería coincidir con `570(k 4l3r7`.

```js
assert.match('570(k 4l3r7', stockRegex);
```

Tu `stockRegex` debería coincidir con `sto{k alert`.
