...
Dovresti aggiungere un elemento `div` all'interno di `.penguin`. Dovresti avere `--fcc-expected--` elementi `div` invece di `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin > div')?.length, 3);
```

Dovresti assegnare al nuovo `div` un valore `class` di `shirt`.

```js
assert.exists(document.querySelector('.penguin > div.shirt'));
```

Dovresti inserire il nuovo `div` prima di `.penguin-body`.
