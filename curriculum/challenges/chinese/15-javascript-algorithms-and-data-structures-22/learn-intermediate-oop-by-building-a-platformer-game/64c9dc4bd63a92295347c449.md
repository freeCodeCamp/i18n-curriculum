...
接下来，添加一个 `else if` 语句来检查左键是否被按下以及 `isCheckpointCollisionDetectionActive` 是否为真。

在该条件内，添加一个 `forEach` 循环来遍历 `platforms` 数组。

在循环内部，使用加法赋值运算符将 5 添加到平台的 `x` 位置。

# --hints--

你应该有一个条件来检查左键是否被按下以及 `isCheckpointCollisionDetectionActive` 是否为真。
