...
Sasa jaza `bugMap` na maingizo mawili. Ongeza `"bee"` kama ufunguo unaoelekezwa kwa mfano mpya wa `Bee`, na `"spider"` kama ufunguo unaoelekezwa kwa mfano mpya wa `Spider`. Pitia `bugEmojiElement` kama hoja ya zote mbili.

# --hints--

`bugMap` yako inapaswa kuwa na maingizo ya `"bee"` yaliyowekwa kuwa `new Bee(bugEmojiElement)`.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

`bugMap` yako inapaswa kuwa na maingizo ya `"spider"` yaliyowekwa kuwa `new Spider(bugEmojiElement)`.
