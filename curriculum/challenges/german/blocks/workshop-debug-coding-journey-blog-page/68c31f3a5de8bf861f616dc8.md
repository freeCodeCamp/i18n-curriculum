...
Im Moment hat der `footer` Abschnitt eine `h3` Überschrift. Aber es sollte eine `h2` Überschrift sein, um eine korrekte Überschriftenhierarchie zu gewährleisten.

Ändern Sie das `h3` in ein `h2`.

# --hints--

Ihr `footer` Element sollte ein `h2` Element enthalten, anstatt eines `h3` Elements.
