...
Dale a tu tercer elemento `th` el atributo `class` configurado como `current`.
...
Cada uno de tus nuevos elementos `span` debe tener el atributo `class` configurado como `sr-only year`.
...
Tu tercer elemento `th` debe tener la `class` configurada como `current`.
