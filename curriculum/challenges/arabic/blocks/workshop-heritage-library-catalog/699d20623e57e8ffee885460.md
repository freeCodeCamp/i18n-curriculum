...
قبل الوثوق بالبيانات المحللة، ابدأ بالتحقق من الحقول المطلوبة.

أنشئ دالة باسم `validateEntry` تأخذ `entry` كمعلمة. أعلن `let isValid = true`.

أضف تحققًا للـ `title`: اجعل `isValid = false` إذا كان `title` مفقودًا أو قيمته خاطئة أو يساوي `"Unknown"`.

أرجع `isValid`. ستضيف تحققًا للحقول الأخرى في الخطوات التالية.

# --hints--

يجب أن تُرجع `validateEntry` قيمة منطقية.

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })` يجب أن تُرجع `true`.

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})` يجب أن تُرجع `false`.

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })` يجب أن تُرجع `false`.

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })` يجب أن تُرجع `false`.
