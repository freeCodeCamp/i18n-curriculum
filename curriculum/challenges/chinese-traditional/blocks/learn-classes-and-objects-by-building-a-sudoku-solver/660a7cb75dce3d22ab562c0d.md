...
在迴圈主體內，將 `pass` 取代為一個 `if` 述語，用來檢查該數字是否為目前儲存格的有效選擇。

使用 `is_valid` 呼叫，並將 `next_empty` 和 `guess` 作為引數，來建置 `if` 條件。

# --hints--

你應該刪除 `pass` 並創建一個 `if` 述語來檢查 `self.is_valid(next_empty, guess)` 是否為 `True`。
