...
<dfn>find()</dfn> メソッドは、配列内で指定されたコールバック関数の条件を満たす最初の要素を取得します。条件を満たす要素がない場合、このメソッドは `undefined` を返します。

以下の例では、`find()` メソッドを使って 25 より大きい最初の数値を見つけています。
...
`const` を使って `song` という変数を作成し、`find()` 配列に対して `userData?.songs` メソッドを呼び出した結果を代入してください。`song` コールバックのパラメータとして `find()` を使い、`song.id` が `id` と厳密に等しいかどうかをチェックしてください。

これにより、`userData?.songs` 配列を繰り返し処理し、`id` 関数に渡された `playSong` に対応する曲を検索します。

# --hints--

`find` に対して `userData?.songs` メソッドを使う必要があります。

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

`find` メソッドのアロー関数コールバックのパラメータは `song` にしてください。

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

`find` メソッドでは、`song.id` が `id` と厳密に等しいかどうかをチェックしてください。

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

`find` メソッドは `song` 定数に代入してください。
