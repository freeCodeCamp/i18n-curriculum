...
創建一個名爲 `movePlayer` 的新箭頭函數，它具有三個參數，分別稱爲 `key`，`xVelocity`，`isPressed`。

# --hints--

你應該聲明一個 `movePlayer` 函數。

```js
assert.isFunction(movePlayer);
```

你的 `movePlayer` 函數應該採用 `key`、`xVelocity` 和 `isPressed` 的參數。

```js
assert.match(movePlayer.toString(), /\(\s*key\s*,\s*xVelocity\s*,\s*isPressed\s*\)/);
```

你的 `movePlayer` 函數應該是空的。
