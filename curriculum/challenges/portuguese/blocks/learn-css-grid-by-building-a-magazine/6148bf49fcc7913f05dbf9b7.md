...
Crie um seletor `.author-name a:hover` e dê a ele a propriedade `background-color` definida como `#306203`.
...
Você deve ter um seletor `.author-name a:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

O seletor `.author-name a:hover` deve ter a propriedade `background-color` definida como `#306203`.
