...
Crea una función `getBookSummaries` con un único parámetro que acepte un arreglo con objetos de libro.

La función `getBookSummaries` debería devolver un arreglo de cadenas que representan el resumen de cada libro.

# --hints--

Debes crear una función `getBookSummaries`.

```js
assert.isFunction(getBookSummaries);
```

Tu función `getBookSummaries` debe tener un parámetro.

```js
assert.lengthOf(getBookSummaries, 1);
```

Tu función `getBookSummaries` debe devolver un arreglo.

```js
assert.isArray(getBookSummaries(library));
```

Tu función `getBookSummaries` debe devolver un arreglo con la misma longitud que el arreglo pasado a la función.
...
Tu función `getBookSummaries` debe devolver un arreglo con la propiedad `about` de cada libro del arreglo pasado a la función.
