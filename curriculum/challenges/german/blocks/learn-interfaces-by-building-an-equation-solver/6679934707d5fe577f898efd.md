...
Fügen Sie einen weiteren Fall hinzu, wenn die Länge von `results` `2` ist. Weisen Sie diesmal `result_list` eine Liste zu, die zwei Strings im Format `x1 = <root1>` und `x2 = <root2>` enthält. Lassen Sie die Lösung erneut sowohl positive als auch negative Vorzeichen anzeigen.
...
Sie sollten Ihre vorhandenen `case`-Blöcke nicht ändern.
...
Sie sollten nach dem vorhandenen `case`-Block einen neuen `2` mit dem Muster `case` erstellen.
...
Sie sollten innerhalb des neuen `x1 = <root1>`-Blocks `x2 = <root2>` eine Liste zuweisen, die zwei Strings im Format `result_list` und `case` enthält. Zeigen Sie für die Ergebnisse sowohl positive als auch negative Vorzeichen an.
