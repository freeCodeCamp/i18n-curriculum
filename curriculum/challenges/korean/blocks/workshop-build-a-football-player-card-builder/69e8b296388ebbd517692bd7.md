...
이제 위치 입력을 select로 변경하는 과정을 시작하세요.

입력을 id가 `position`이고 `className`가 `input`이며 `value`가 `player.position`인 `select` 요소로 변경하세요.

# --hints--

위치 `input`를 `id`가 `position`이고 `className`가 `input`인 `select` 요소로 변경해야 합니다.
...
위치 `select`에는 `value`가 `player.position`여야 합니다.
