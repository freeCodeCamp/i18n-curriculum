...
마지막으로 노래를 재생하려면 `play()` 변수에 `audio` 메서드를 사용하세요. `play()`은 mp3 파일을 재생하기 위한 웹 오디오 API의 메서드입니다.

# --hints--

`classList` 속성과 `add` 메서드를 사용해 `"playing"`에 `playButton` 클래스를 추가해야 합니다.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

`play()` 변수에 `audio` 메서드를 사용해야 합니다.
