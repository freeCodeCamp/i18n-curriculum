...
`p` 要素内の3つの `.text` 要素の後に、`blockquote` 要素を作成してください。その中に、`hr` 要素、`p` が `class` に設定された `quote` 要素、そして2つ目の `hr` 要素を追加してください。

`.quote` 要素にテキスト `The entire curriculum should be a series of projects` を設定してください。

# --hints--

`blockquote` 要素内に新しい `.text` 要素を作成してください。

```js
assert.exists(document.querySelector('.text blockquote'));
```

3つの `blockquote` 要素の後に `p` 要素が来るようにしてください。

```js
assert.equal(document.querySelector('.text')?.children?.[3]?.localName, 'blockquote');
```

`blockquote` 要素は2つの `hr` 要素を持つ必要があります。

```js
assert.lengthOf(document.querySelectorAll('.text blockquote hr'), 2);
```

`blockquote` 要素は `p` 要素を持つ必要があります。

```js
assert.exists(document.querySelector('.text blockquote p'));
```

`blockquote` の子要素は正しい順序である必要があります。
...
新しい `p` 要素は `class` が `quote` に設定されている必要があります。

```js
assert.equal(document.querySelector('.text blockquote p')?.className, 'quote');
```

新しい `p` 要素にはテキスト `The entire curriculum should be a series of projects` が設定されている必要があります。
