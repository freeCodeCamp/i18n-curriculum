...
Inside the `fetchMoreAuthors` function, set the `startingIndex` y `endingIndex` variables a `+= 8` cada uno.
...
Debe establecer la variable `startingIndex` a `+=8`.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Debe establecer la variable `endingIndex` a `+=8`.
