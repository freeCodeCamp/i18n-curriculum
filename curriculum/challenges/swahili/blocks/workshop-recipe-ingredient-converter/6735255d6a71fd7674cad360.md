...
Mwishowe, mpe `conversionTable` yako kitu cha `teaspoon` chenye sifa ya `cup` iliyowekwa kuwa `1/48`, sifa ya `gram` iliyowekwa kuwa `5`, na sifa ya `ounce` iliyowekwa kuwa `0.167`.

# --hints--

`conversionTable` yako inapaswa kuwa na sifa ya `teaspoon`.

```js
assert.property(conversionTable, "teaspoon");
```

Sifa yako ya `teaspoon` inapaswa kuwa kitu.

```js
assert.isObject(conversionTable.teaspoon);
```

Kitu chako cha `teaspoon` kinapaswa kuwa na sifa ya `cup`.

```js
assert.property(conversionTable.teaspoon, "cup");
```

Sifa yako ya `cup` inapaswa kuwekwa kuwa `1/48`.

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

Kitu chako cha `teaspoon` kinapaswa kuwa na sifa ya `gram`.

```js
assert.property(conversionTable.teaspoon, "gram");
```

Sifa yako ya `gram` inapaswa kuwekwa kuwa `5`.

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

Kitu chako cha `teaspoon` kinapaswa kuwa na sifa ya `ounce`.

```js
assert.property(conversionTable.teaspoon, "ounce");
```

Sifa yako ya `ounce` inapaswa kuwekwa kuwa `0.167`.
