...
Створіть новий селектор `25%` між селекторами `0%` та `50%`. Надайте цьому новому селектору властивість `background-color` зі значенням `yellow`.

# --hints--

Ви повинні створити новий селектор `25%` в своєму правилі `@keyframes cabins`.
...
Ваш селектор `25%` повинен бути між селекторами `0%` та `50%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '25%');
```

Ваш селектор `25%` повинен мати властивість `background-color` зі значенням `yellow`.
