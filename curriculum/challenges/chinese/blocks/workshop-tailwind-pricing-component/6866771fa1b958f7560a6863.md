...
那会移动徽章文本，但不完全是你想要的位置。关键在于 `right-[num]`、`left-[num]`、`top-[num]` 和 `bottom-[num]` 实用类。
...
为你的 `Most popular` 徽章 `div` 添加类 `-top-3` 和 `right-3`。

# --hints--

你的 `div` 元素中带有文本 `Most popular` 应该具有类 `-top-3`。
...
你的 `div` 元素中带有文本 `Most popular` 应该具有类 `right-3`。
