...
Використайте методи `classList` і `remove()`, щоб видалити клас `playing` з `playButton`, оскільки пісня буде призупинена на цьому етапі.

Щоб остаточно призупинити пісню, застосуйте метод `pause()` до змінної `audio`. `pause()` — це метод Web Audio API для призупинення музичних файлів.

# --hints--

Не слід змінювати існуючу функцію `pauseSong` та її вміст.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

Використайте властивість `classList` і метод `remove()`, щоб видалити клас `"playing"` з `playButton`.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

Використайте метод `pause()` для вашої змінної `audio`.
