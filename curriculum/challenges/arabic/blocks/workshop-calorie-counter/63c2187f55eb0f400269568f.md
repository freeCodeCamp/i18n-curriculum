...
أخيرًا، في سطر جديد بعد `label` الثاني، أنشئ عنصر `input` آخر. اجعل له خاصية `type` بقيمة `number`، وخاصية `min` بقيمة `0` (لضمان عدم إضافة سعرات حرارية سالبة)، وخاصية `placeholder` بقيمة `Calories`، وخاصية `id` تطابق خاصية `for` في عنصر `label` الثاني.

# --hints--

يجب أن يكون لديك عنصران `input` في `HTMLString` الخاص بك.
...
يجب أن يكون عنصر `input` الجديد في سطر جديد.
...
يجب أن يأتي عنصر `input` الجديد بعد عنصر `label` الثاني.
...
يجب أن يحتوي عنصر `input` الجديد على خاصية `type` بقيمة `number`.
...
يجب أن يحتوي عنصر `input` على خاصية `placeholder` بقيمة `Calories`.
...
يجب أن يحتوي عنصر `input` على خاصية `id` بقيمة `${entryDropdown.value}-${entryNumber}-calories`.
