...
Для завершення стилізації додайте застосовані назви класів `flavor` та `price` решті елементів `p`.

# --hints--

Ви повинні мати п'ять елементів `.flavor`.

```js
assert.lengthOf(document.querySelectorAll('.flavor'), 5);
```

Ви повинні мати п'ять елементів `.price`.

```js
assert.lengthOf(document.querySelectorAll('.price'), 5);
```

Ваші елементи `.flavor` повинні бути елементами `p` з текстом `French Vanilla`, `Caramel Macchiato`, `Pumpkin Spice`, `Hazelnut` та `Mocha`.
...
Ваші елементи `.price` повинні бути елементами `p` з текстом `3.00`, `3.75`, `3.50`, `4.00` та `4.50`.
