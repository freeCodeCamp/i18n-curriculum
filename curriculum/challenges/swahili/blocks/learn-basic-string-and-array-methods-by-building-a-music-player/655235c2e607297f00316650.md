...
Ifuatayo, tumia sifa ya `classList` na njia ya `add()` kuongeza darasa la vitu la `"playing"` kwenye kipengele cha `playButton`. Hii itatafuta darasa la vitu la `"playing"` katika faili la CSS na kuliongeza kwenye kipengele cha `playButton`.

Ili kucheza wimbo hatimaye, tumia njia ya `play()` kwenye mabadiliko ya `audio`. `play()` ni njia kutoka kwa API ya sauti ya mtandao kwa ajili ya kucheza faili la mp3.

# --hints--

Unapaswa kutumia sifa ya `classList` na njia ya `add` kuongeza darasa la vitu la `"playing"` kwenye `playButton`.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

Unapaswa kutumia njia ya `play()` kwenye mabadiliko ya `audio`.
