...
title: "المشكلة 91: مثلثات قائمة الزاوية بإحداثيات صحيحة"
challengeType: 1
forumTopicId: 302208
...
النقاط ${P}(x_1, y_1)$ و${Q}(x_2, y_2)$ مرسومة عند إحداثيات صحيحة ومرتبطة بالأصل، ${O}(0, 0)$، لتشكيل ${\Delta}OPQ$.
...
يوجد بالضبط أربعة عشر مثلثًا يحتوي على زاوية قائمة يمكن تشكيلها عندما تكون كل إحداثية بين 0 و2 شاملة؛ أي $0 ≤ x_1, y_1, x_2, y_2 ≤ 2$.
...
بما أن $0 ≤ x_1, y_1, x_2, y_2 ≤ limit$، كم عدد المثلثات القائمة التي يمكن تشكيلها؟

# --hints--

`rightTrianglesIntCoords(2)` يجب أن تُرجع رقمًا.

```js
assert(typeof rightTrianglesIntCoords(2) === 'number');
```

`rightTrianglesIntCoords(2)` يجب أن تُرجع `14`.

```js
assert.strictEqual(rightTrianglesIntCoords(2), 14);
```

`rightTrianglesIntCoords(10)` يجب أن تُرجع `448`.

```js
assert.strictEqual(rightTrianglesIntCoords(10), 448);
```

`rightTrianglesIntCoords(25)` يجب أن تُرجع `3207`.

```js
assert.strictEqual(rightTrianglesIntCoords(25), 3207);
```

`rightTrianglesIntCoords(50)` يجب أن تُرجع `14234`.
