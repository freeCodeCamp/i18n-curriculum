...
Інший текст змістився з місця. Відновіть його положення, задавши селектору `.first-paragraph::first-letter` множину властивостей `float` зі значенням `left` та `margin-right` зі значенням `1rem`.
