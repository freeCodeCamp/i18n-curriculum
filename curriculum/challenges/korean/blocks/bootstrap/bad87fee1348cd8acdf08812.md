...
부트스트랩은 `button` 요소에 자체 스타일을 제공하며, 이는 기본 HTML 스타일보다 훨씬 보기 좋습니다.

큰 새끼 고양이 사진 아래에 새로운 `button` 요소를 만드세요. `btn`와 `btn-default` 클래스를 부여하고, 텍스트는 `Like`로 설정하세요.

# --hints--

텍스트가 `button`인 새로운 `Like` 요소를 만들어야 합니다.
...
새 버튼에는 `btn`과 `btn-default` 두 개의 클래스가 있어야 합니다.
