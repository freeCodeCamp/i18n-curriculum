...
배열이나 객체에서 여러 값을 한 문장으로 구조 분해할 수 있다는 점을 기억하세요.

콜백 함수의 첫 번째 매개변수로 전달된 `name` 인스턴스에서 `id`, `price`, `category`, `Dessert` 프로퍼티를 구조 분해하세요.

# --hints--

`name`, `id`, `price`, `category` 매개변수를 구조 분해로 선언해야 합니다. 이 테스트에서는 순서가 중요합니다.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

구조 분해는 콜백 함수의 첫 번째 매개변수여야 합니다.
