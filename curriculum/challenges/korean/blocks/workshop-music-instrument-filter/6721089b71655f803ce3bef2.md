...
이제 드롭다운에 네 개의 옵션 `All`, `Woodwinds`, `Brass`, `Percussion`를 이 순서대로 추가하세요. 각 옵션에 대해 소문자 option을 사용해 `id` 속성과 `value` 속성을 지정하세요.

# --hints--

`option` 요소 안에 `select` 요소 네 개가 있어야 합니다.

```js
assert.lengthOf(document.querySelectorAll("select>option"), 4);
```

첫 번째 `option` 요소에는 텍스트 `All`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[0].innerText,"All");
```

`All` 옵션에는 `id` 속성과 값이 `value`인 `all` 속성이 있어야 합니다.
...
두 번째 `option` 요소에는 텍스트 `Woodwinds`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[1].innerText,"Woodwinds");
```

`Woodwinds` 옵션에는 `id` 속성과 값이 `value`인 `woodwinds` 속성이 있어야 합니다.
...
세 번째 `option` 요소에는 텍스트 `Brass`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[2].innerText,"Brass");
```

`Brass` 옵션에는 `id` 속성과 값이 `value`인 `brass` 속성이 있어야 합니다.
...
네 번째 `option` 요소에는 텍스트 `Percussion`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[3].innerText,"Percussion");
```

`Percussion` 옵션에는 `id` 속성과 값이 `value`인 `percussion` 속성이 있어야 합니다.
