...
`topic_list` オブジェクトには、フォーラムに投稿された最新のトピックを含む `topics` 配列が含まれています。

分割代入を使って、`topic_list` オブジェクトから `topics` 配列を取り出してください。

# --hints--

`const` を使用して、`topic_list` オブジェクトから変数 `topics` を分割代入してください。
