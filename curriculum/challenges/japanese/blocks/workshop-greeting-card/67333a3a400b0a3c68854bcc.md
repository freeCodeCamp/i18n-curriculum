...
`transition` セレクターに `.card-links a` プロパティを追加し、その値を `background-color 0.3s ease` にしてください。

# --hints--

`.card-links a` セレクターには `transition` プロパティが `background-color 0.3s ease` に設定されている必要があります。
