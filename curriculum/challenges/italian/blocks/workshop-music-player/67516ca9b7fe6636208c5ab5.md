...
Infine, passa `false` come secondo argomento alla chiamata della funzione `playSong(userData.currentSong.id)`. Poi, verifica che tutto funzioni come previsto riproducendo una canzone, mettendola in pausa e quindi premendo il pulsante precedente o il pulsante successivo.

# --hints--

Dovresti passare `false` come secondo argomento alla chiamata della funzione `playSong(userData.currentSong.id)`.
