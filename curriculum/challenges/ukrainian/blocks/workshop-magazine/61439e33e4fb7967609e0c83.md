...
Після трьох елементів `p` всередині вашого елемента `.text` створіть елемент `blockquote`. Усередині нього додайте елемент `hr`, елемент `p` з множиною `class`, встановленою в `quote`, і другий елемент `hr`.

Дайте елементу `.quote` текст `The entire curriculum should be a series of projects`.

# --hints--

Ви повинні створити новий елемент `blockquote` всередині вашого елемента `.text`.

```js
assert.exists(document.querySelector('.text blockquote'));
```

Ваш елемент `blockquote` має йти після трьох елементів `p`.

```js
assert.equal(document.querySelector('.text')?.children?.[3]?.localName, 'blockquote');
```

Ваш елемент `blockquote` має містити два елементи `hr`.

```js
assert.lengthOf(document.querySelectorAll('.text blockquote hr'), 2);
```

Ваш елемент `blockquote` має містити елемент `p`.

```js
assert.exists(document.querySelector('.text blockquote p'));
```

Дочірні елементи `blockquote` мають бути в правильному порядку.
...
Ваш новий елемент `p` має містити текст `The entire curriculum should be a series of projects`.
