...
大多数浏览器会为不同的元素注入自己的默认 CSS 属性和值。 如果你仔细观察，你可能会注意到文件 `input` 小于其他文本 `input` 元素。 默认情况下，你可以输入的 `input` 元素的 `padding` 是 `1px 2px`。

使用另一个属性选择器，为 `type` 为 `file` 的 `input` 设置样式，使它的内边距与另一个 `input` 元素的相同。

# --hints--

你应该使用属性选择器来设置 `input` 元素的样式。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="file"]'));
```

你应该给 `input[type="file"]` 一个 `1px 2px` 的 `padding`。
