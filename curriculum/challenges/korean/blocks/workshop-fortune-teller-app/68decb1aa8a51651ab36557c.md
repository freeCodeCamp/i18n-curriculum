...
`getElement` 함수 안에서 `querySelector`를 제네릭과 함께 사용하고 결과를 변수에 저장하세요. 요소가 존재하지 않는지 확인하는 `if` 문을 추가하고, 그렇다면 `Element not found: ${error message here}` 메시지와 함께 오류를 발생시키세요. 요소가 존재하면 반환하세요.

# --hints--

`el` 함수 안에 `getElement`라는 변수를 선언하세요.
...
`el` 변수에 `document.querySelector<T>(selector)` 값을 할당해야 합니다.
