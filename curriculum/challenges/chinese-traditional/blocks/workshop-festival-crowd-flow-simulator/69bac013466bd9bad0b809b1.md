...
首先，你會在 `gates` 陣列中找到特定閘門的位置。

使用 `indexOf()` 方法在 `gates` 上定位 `currentGate` 元素，並將結果指定給名為 `currentIndex` 的變數。
...
你應該在 `indexOf()` 陣列上使用 `gates` 來存取 `currentGate` 的索引，並將它指定給名為 `currentIndex` 的變數。這應該在一行中完成。
