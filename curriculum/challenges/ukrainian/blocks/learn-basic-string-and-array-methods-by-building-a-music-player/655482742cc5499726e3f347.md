...
У функції `playPreviousSong` додайте інструкцію `if` з умовою `userData?.currentSong === null`. Це перевірить, чи наразі не відтворюється жодна пісня. Якщо ні, вийдіть із функції за допомогою `return`.

Всередині блоку `else` створіть константу з ім’ям `currentSongIndex` і призначте їй `getCurrentSongIndex()`.

# --hints--

Вам не слід змінювати існуючу функцію `playPreviousSong` та її вміст.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

У функції `if` має бути інструкція `userData?.currentSong === null` з умовою `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

У блоці інструкції `return` має бути `if`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

Інструкція `if` має мати блок `else`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

Ви маєте викликати `getCurrentSongIndex` і призначити його `currentSongIndex` всередині блоку `else` інструкції `if`.
