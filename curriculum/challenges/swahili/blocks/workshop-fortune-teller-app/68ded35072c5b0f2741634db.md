...
Unahitaji kitendakazi kingine kinachoonyesha vipengele katika DOM. Kama kitendakazi kilichopita, tengeneza kitendakazi kinachoitwa `showElements`. Muda huu tumia `classList.remove("hidden")`.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `showElements`.
...
Kitendakazi chako cha `showElements` kinapaswa kuwa na kigezo kinachoitwa `...elements` chenye aina ya `HTMLElement[]`.
...
Katika kitendakazi chako cha `showElements` unapaswa kuwa na njia ya `forEach` inayounganishwa na kigezo cha `elements`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Njia yako ya `forEach` inapaswa kuchukua kigezo kinachoitwa `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Unapaswa kuondoa darasa la `hidden` kwa kutumia njia ya `classList.remove` ndani ya njia ya `forEach`.
