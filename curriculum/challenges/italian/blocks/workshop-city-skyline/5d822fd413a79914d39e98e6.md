...
Annida sei elementi `div` all'interno di `.foreground-buildings` e assegna loro le classi da `fb1` a `fb6` in questo ordine. "fb" sta per "foreground building". Questi saranno sei edifici in più per il primo piano.

# --hints--

Dovresti creare un nuovo `div` con una classe `fb1`.

```js
assert.exists(document.querySelector('div.fb1'));
```

Dovresti creare un nuovo `div` con una classe `fb2`.

```js
assert.exists(document.querySelector('div.fb2'));
```

Dovresti creare un nuovo `div` con una classe `fb3`.

```js
assert.exists(document.querySelector('div.fb3'));
```

Dovresti creare un nuovo `div` con una classe `fb4`.

```js
assert.exists(document.querySelector('div.fb4'));
```

Dovresti creare un nuovo `div` con una classe `fb5`.

```js
assert.exists(document.querySelector('div.fb5'));
```

Dovresti creare un nuovo `div` con una classe `fb6`.

```js
assert.exists(document.querySelector('div.fb6'));
```

Dovresti inserire questi nuovi elementi `div` all'interno dell'elemento `.foreground-buildings`.
