...
在先前的專案中，你使用 `fetch()` 搭配 `.then()` 方法來在 promise 被決議後執行邏輯。現在你將使用 `await` 關鍵字來控制 `fetch()` 方法的非同步特性。

`await` 關鍵字會等待 promise 決議並傳回結果。
...
在 `try` 區塊內，創建一個名為 `res` 的常數並指定其為 `await fetch()`。對於 `fetch` 呼叫，傳入 `forumLatest` 變數。

# --hints--

你應該在你的 `try` 區塊內有一個名為 `res` 的常數。

```js
assert.match(code, /\s*const\s+res/)
```

你應該將 `await fetch()` 指定到你的 `res` 變數。

```js
assert.match(code, /\s*const\s+res\s*=\s*await\s+fetch\(\s*(.*)\s*\)/);
```

你應該將 `forumLatest` 常數傳入 `fetch` 呼叫。
