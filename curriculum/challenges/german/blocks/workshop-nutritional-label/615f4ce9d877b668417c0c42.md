...
Fassen Sie alles innerhalb des `.label`-Elements in einem neuen `header`-Element zusammen.

# --hints--

Sie sollten ein neues `header`-Element erstellen.

```js
assert(document.querySelector('header'));
```

Ihr `header`-Element sollte sich innerhalb Ihres `.label`-Elements befinden.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

Ihre `h1`-, `div`- und `p`-Elemente sollten sich innerhalb Ihres neuen `header`-Elements befinden.
