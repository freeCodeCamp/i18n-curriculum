...
الآن، تحتاج إلى الحصول على الرقم الأول في النص `0/10`، بطريقة تعمل أيضًا مع `5/10` أو `10/10`.

حلل محتوى النص لعنصر `.count` لاستخراج الرقم الحالي.

حوّل العدد من سلسلة نصية إلى رقم.

استبدل السطر السابق `console.log(countEl.textContent)` بسجل جديد خاص بك.

سجل العدد الحالي بهذا الشكل: `console.log("Current count:", currCount)`.

تلميح: تذكر أنه يمكنك استخدام طريقة `split` لتقسيم السلسلة النصية إلى مصفوفة من السلاسل الفرعية.

# --hints--

يجب أن تسجل العدد الحالي كرقم.
...
يجب أن تسجل العدد الحالي كرقم بطريقة تعمل مع القيم المختلفة الممكنة.
