...
Crea una funzione chiamata `isSelect` che prende un singolo parametro `element` di tipo `EventTarget | null`. Questa funzione verificherà se l'elemento selezionato è un elemento HTML `<select>`.

# --hints--

Dovresti avere una funzione chiamata `isSelect`.
...
La tua funzione `isSelect` dovrebbe avere un parametro chiamato `element` di tipo `EventTarget | null`.
...
La tua funzione `isSelect` dovrebbe avere un tipo di ritorno `element is HTMLSelectElement`.
