...
ユーザーが同じ数字を3つ、別の数字を2つ出した場合、これをフルハウスと呼びます。単一の引数を受け取る`detectFullHouse`関数を宣言してください。関数は呼び出されたときに`diceValuesArr`配列が渡されます。

`detectFullHouse`関数は、ユーザーが同じ数字を3つ、別の数字を2つ出したかどうかをチェックしてください。そうであれば、3番目のラジオボタンをスコア`25`で更新し、正しい属性を設定してください。いずれの場合も、最後のラジオボタンはスコア0で更新し、正しい属性を設定してください。

サイコロが振られたときに新しい関数を呼び出すのを忘れないでください。

# --hints--

`detectFullHouse`関数を用意してください。

```js
assert.isFunction(detectFullHouse);
```

`detectFullHouse`関数にはパラメータが必要です。

```js
assert.match(code, /(?:var|let|const)\s+detectFullHouse\s*=\s*(\(\s*\w+\s*\)|\s*\w+\s*)\s*=>\s*|function\s+detectFullHouse\s*\(\s*\w+\s*\)/);
```

フルハウスが出た場合、`detectFullHouse`関数は3番目のラジオボタンを有効にし、その値を`25`に設定し、3番目の`span`にテキスト`, score = 25`を表示させる必要があります。
...
フルハウスが出なかった場合、`detectFullHouse`関数は最後のラジオボタンを有効にし、その値を`0`に設定し、最後の`span`にテキスト`, score = 0`を表示させる必要があります。
...
`detectFullHouse`関数は`rollDiceBtn`がクリックされたときに呼び出してください。
