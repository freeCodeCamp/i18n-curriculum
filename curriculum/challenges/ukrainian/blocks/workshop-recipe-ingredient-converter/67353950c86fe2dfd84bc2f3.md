...
Вам потрібно врахувати кількість порцій, які обчислюються. Це чудова можливість використати ще одну карировану функцію.

Оголосіть функцію `adjustForServings`, яка приймає аргумент `baseQuantity`. Вона має повертати другу функцію, яка приймає аргумент `newServings`.

Внутрішня функція має повертати результат множення `baseQuantity` на `newServings`.

# --hints--

Ви повинні оголосити функцію `adjustForServings`.

```js
assert.isFunction(adjustForServings);
```

Ваша функція `adjustForServings` має приймати аргумент `baseQuantity`.

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

Ваша функція `adjustForServings` має повертати функцію.

```js
assert.isFunction(adjustForServings?.());
```

Ваша внутрішня функція має мати параметр `newServings`.

```js
assert.match(adjustForServings?.()?.toString(), /\(newServings\)/);
```

Ваша внутрішня функція має повертати результат множення `baseQuantity` і `newServings`.
