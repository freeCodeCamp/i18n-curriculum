...
Sempre all'interno della clausola `elif`, passa l'iteratore `expenses_from_category` a una chiamata `print_expenses`.

# --hints--

Dovresti chiamare `print_expenses()` passando `expenses_from_category` come argomento nella tua clausola `elif`.
...
La tua chiamata `print_expenses()` dovrebbe venire dopo la variabile `expenses_from_category`.
