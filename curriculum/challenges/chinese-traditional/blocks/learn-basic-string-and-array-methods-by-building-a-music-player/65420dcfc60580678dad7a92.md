...
在 `map()` 內，為你負責顯示歌曲詳細資料的所有元素，添加一個帶有反引號的 `return` 述語。

在反引號內，創建一個帶有 `id` 屬性為 `song-${song.id}` 和 `class` 屬性為 `playlist-song` 的 `li` 元素。

# --hints--

你應該添加一個帶有空的反引號配對的 `return` 述語。
...
你的開啟 `li` 頁籤應該設定一個 `id` 屬性為 `song-${song.id}`。

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

你的開啟 `li` 頁籤應該有一個設定為 `playlist-song` 的 `class` 屬性。
