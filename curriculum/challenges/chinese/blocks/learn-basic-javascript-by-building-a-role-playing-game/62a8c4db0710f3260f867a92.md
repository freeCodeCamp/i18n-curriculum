...
如果玩家没有足够的金币来购买健康怎么办？ 你应该使用 `if` 语句来检查玩家是否有足够的金币来购买健康。 

在上一个项目中，你学习了如何使用 `if` 语句，如下所示：
...
首先将 `buyHealth` 函数中的所有代码放在 `if` 语句内。 对于 `if` 语句条件，检查 `gold` 是否大于或等于 `10`。

# --hints--

你的 `buyHealth` 函数应该有一个 `if` 语句。

```js
assert.match(buyHealth.toString(), /if/);
```

你的 `if` 语句应检查 `gold` 是否大于或等于 `10`。

```js
assert.match(buyHealth.toString(), /if\s*\(\s*gold\s*>=\s*10\s*\)/);
```

所有 `buyHealth` 代码都应位于 `if` 语句内。
