---
id: 656918c77e73780c34392e17
title: タスク 33
challengeType: 22
dashedName: task-33
lang: en-US
---

<!-- (Audio) Brian: Hey, Amy. I'm not working on anything big at the moment. I'm taking a short break to learn more about front-end. It's good to stop a bit and refresh my skills. How about you? -->

# --description--

`learn` とは、何かについて知識や技術を身につけることを意味します。例えば：

`She is learning to play the guitar.` - ここでの `learning` は、彼女がギターを練習して技術を身につけていることを意味します。

`refresh` とは、何かを更新したり改善したりすることを意味します。例えば：

`I refresh my memory by reviewing notes.` - ここでの `refresh` は、ノートをもう一度見直して情報を新しくはっきりとしたものにすることを意味します。

`Skills` は、物事をうまく行う能力のことです。例えば：

`Good writing skills are important.` - ここでの `skills` は、勉強や仕事に役立つ、文章を書く能力を意味します。

# --fillInTheBlank--

## --sentence--

`Hey, Amy. I'm not working on BLANK big at the moment. I'm taking a BLANK break to learn more about BLANK. It's good to stop a bit and BLANK my skills. How about you?`

## --blanks--

`anything`

### --feedback--

この単語は、ブライアン が今大きな仕事をしていないことを意味します。

---

`short`

### --feedback--

この単語は、ブライアン の休憩時間があまり長くないことを表します。

---

`front-end`

### --feedback--

これらの2語は、ウェブ開発の中でユーザーが見る部分に関係しています。

---

`refresh`

### --feedback--

ブライアン はこの動詞を使って、自分の能力を向上させたり更新したりしていることを表しています。

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Brian",
        "position": {"x":50,"y":15,"z":1.2},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "2.3-4.mp3",
      "startTime": 1,
      "startTimestamp": 2.98,
      "finishTimestamp": 12.72
    }
  },
  "commands": [
    {
      "character": "Brian",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Brian",
      "startTime": 1,
      "finishTime": 3.64,
      "dialogue": {
        "text": "Hey, Amy. I'm not working on anything big at the moment.",
        "align": "center"
      }
    },
    {
      "character": "Brian",
      "startTime": 3.82,
      "finishTime": 6.5,
      "dialogue": {
        "text": "I'm taking a short break to learn more about front-end.",
        "align": "center"
      }
    },
    {
      "character": "Brian",
      "startTime": 7.04,
      "finishTime": 10.74,
      "dialogue": {
        "text": "It's good to stop a bit and refresh my skills. How about you?",
        "align": "center"
      }
    },
    {
      "character": "Brian",
      "opacity": 0,
      "startTime": 11.24
    }
  ]
}
```
