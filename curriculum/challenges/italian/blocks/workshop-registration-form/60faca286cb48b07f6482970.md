...
Inviare il modulo con un'opzione selezionata non invierebbe un valore utile al server. Per questo motivo, ogni elemento `option` ha bisogno di un attributo `value`. Senza questo attributo, il contenuto testuale del `option` verrà inviato al server.

Dai al primo `option` un attributo `value` con valore `""`, e agli elementi `option` successivi attributi `value` con valori da `1` a `4`.

# --hints--

Dovresti dare al primo `option` un attributo `value` con valore `""`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

Dovresti dare al secondo `option` un attributo `value` con valore `1`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

Dovresti dare al terzo `option` un attributo `value` con valore `2`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

Dovresti dare al quarto `option` un attributo `value` con valore `3`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

Dovresti dare al quinto `option` un attributo `value` con valore `4`.
