...
Dentro do método `fetchCardsData`, declare uma variável chamada `response` e atribua-a fazendo uma chamada à função `fetch` com a seguinte URL: `` `${CDN_URL}/card_data.json` ``.

Adicione a palavra-chave `await` antes da chamada `fetch` e envolva toda a lógica dentro de um bloco `try...catch`.

# --hints--

Você deve declarar uma variável `response` dentro do método `fetchCardsData`.
...
Sua variável `response` deve ser atribuída fazendo uma chamada a `await fetch` com a URL `` `${CDN_URL}/card_data.json` ``.
...
Sua chamada `fetch` deve estar envolvida dentro de um bloco `try...catch`, com `error` como parâmetro `catch`.
