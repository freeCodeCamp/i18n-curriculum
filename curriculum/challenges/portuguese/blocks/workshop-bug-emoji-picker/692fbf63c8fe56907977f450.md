...
Em seguida, defina um objeto chamado `bugMap` com o tipo `Record<string, Bug<string>>` e atribua a ele um objeto vazio por enquanto.

# --hints--

Você deve ter uma constante chamada `bugMap`.
...
Seu `bugMap` deve ter uma anotação de tipo `Record<string, Bug<string>>`.
...
Seu `bugMap` deve ser inicializado com um objeto vazio.
