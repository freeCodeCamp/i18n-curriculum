...
شارة المستوى لا تزال تعرض النص الثابت `GOLD`. عليك استبداله بتعبير ديناميكي ليُعرض المستوى الصحيح.

استدعِ `getPlayerTier()` مع `player.overallRating` للحصول على سلسلة نصية للمستوى، ثم استدعِ `.toUpperCase()` على النتيجة لعرضها بحروف كبيرة. لا تنسَ تغليف التعبير كله بأقواس متعرجة.

# --hints--

يجب أن تستخدم `getPlayerTier(player.overallRating).toUpperCase()` داخل `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

يجب أن يعرض عنصر `.card-tier-badge` النص `ELITE` للاعب بتقييم 98.
