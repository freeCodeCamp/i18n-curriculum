...
Dentro de la clase `Bee`, implementa el método `render`, precedido por la palabra clave `override`, para actualizar la propiedad `innerText` del `emojiElement` con el valor de la propiedad `emoji`.

# --hints--

Tu clase `Bee` debe tener un método `render` usando la palabra clave `override`.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

Debes asignar `this.emojiElement.innerText` a `this.emoji` dentro del método `render` de `Bee`.
