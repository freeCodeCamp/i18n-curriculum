...
산과 `.ground` 요소가 더 잘 겹치도록, 산에 `margin-top`를 `100px`로 주고 `.ground` 요소에 `margin-top`를 `-58px`로 주세요.

# --hints--

`.left-mountain`에 `margin-top`를 `100px`로 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.marginTop, '100px');
```

`.ground`에 `margin-top`를 `-58px`로 주어야 합니다.
