...
Tangaza kigezo kinachoitwa `plantsSet` na weka thamani yake kuwa wito wa kitendakazi `displayPlantsSet`.

Kisha andika `plantsSet` kwenye konsoli. Utahitaji kufungua konsoli ya kivinjari ili kuona seti yako.


# --hints--

Unapaswa kuwa na kigezo kinachoitwa `plantsSet`.

```js
assert.exists(plantsSet);
```

Unapaswa kuita `displayPlantsSet` na kuwekea thamani `plantsSet`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

Unapaswa kuandika `plantsSet` kwenye konsoli.
