...
现在，你需要在 `selectedCategory` 对象中添加 `className` 和 `category` 属性。

首先将 `className` 变量赋值给 `selectedCategory.className`。 然后将 `category` 变量赋值给 `selectedCategory.category`。

# --hints--

你应使用点符号来选中 `selectedCategory` 对象的 `className` 属性。

```js
assert.match(code, /selectedCategory\.className/);
```

你应该将 `allCategories` 对象的 `className` 属性赋值给 `selectedCategory` 对象的 `className` 属性。

```js
assert.match(code, /selectedCategory\.className\s*=\s*className/);
```

你应该使用点符号来选中 `selectedCategory` 对象的 `category` 属性。

```js
assert.match(code, /selectedCategory\.category/);
```

你应该将 `allCategories` 对象的 `category` 属性赋值给 `selectedCategory` 对象的 `category` 属性。
