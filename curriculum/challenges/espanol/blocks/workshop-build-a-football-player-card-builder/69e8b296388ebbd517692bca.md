...
`FootballPlayerCard` actualmente renderiza la tarjeta con un `defaultPlayer` fijo. Para hacer que la aplicación sea interactiva, necesitas manejar los datos del jugador en el estado de React para que las actualizaciones puedan activar nuevos renderizados.

Dentro del componente `FootballPlayerCard`, crea una variable de estado llamada `player` con un setter `setPlayer` que contendrá los datos actuales del jugador. Inicializa este estado con `defaultPlayer` y especifica que el estado usa el tipo genérico `PlayerData` para que TypeScript sepa qué estructura debe tener el objeto jugador.

Después de eso, actualiza el return para pasar la variable de estado `player` como prop a `PlayerCard`.

# --hints--

Debes usar la desestructuración de arreglos para establecer una variable de estado `player` y un setter `setPlayer`.

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

Tu `player` y `setPlayer` deben usar el hook `useState`.
...
Tu `useState` debe inicializarse con `defaultPlayer` y tener `PlayerData` como su tipo genérico.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

Debes tener `player` como valor de la prop `player` pasada a `PlayerCard`.
