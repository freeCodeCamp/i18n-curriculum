...
現在是時候開始準備計算了。 首先聲明一個 `consumedCalories` 變量，併爲其分配 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的總和（請注意，順序對測試很重要）。 請確保在 `if` 語句之後執行此操作。

# --hints--

你的函數應該有一個 `consumedCalories` 變量。

```js
assert.match(calculateCalories.toString(), /consumedCalories\s*=/);
```

你的 `consumedCalories` 變量應位於 `if` 語句之後。

```js
assert.isAbove(calculateCalories.toString().indexOf('consumedCalories'), calculateCalories.toString().indexOf('if'));
```

你應該按順序計算 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的總和。

```js
assert.match(calculateCalories.toString(), /breakfastCalories\s*\+\s*lunchCalories\s*\+\s*dinnerCalories\s*\+\s*snacksCalories\s*;?\s*$/m);
```

你應該將 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的總和分配給 `consumedCalories`。
