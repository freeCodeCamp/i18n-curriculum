...
Crea un elemento àncora all'interno del primo elemento `li`. Dai all'elemento `a` un attributo `href` con valore `#` e il testo `Dashboard`.

# --hints--

Dovresti creare un elemento `a` all'interno del primo elemento della lista.

```js
assert.exists(document.querySelector('li a'));
```

Il tuo elemento `a` dovrebbe avere l'attributo `href` impostato su `#`.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), '#');
```

Il tuo elemento `a` dovrebbe contenere il testo `Dashboard`.
