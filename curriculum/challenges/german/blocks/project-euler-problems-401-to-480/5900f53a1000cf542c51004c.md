...
Sei `f(k, n)` = $e^\frac{k}{n} - 1$ für alle nicht-negativen ganzen Zahlen `k`.

Bemerkenswerterweise ist `f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3,1415926… ≈ π.

Tatsächlich ist es die beste Approximation von π der Form `f(a, 200) + f(b, 200) + f(c, 200) + f(d, 200)`.

Sei `almostPi(n)` = a<sup>2</sup> + b<sup>2</sup> + c<sup>2</sup> + d<sup>2</sup> für a, b, c, d, die den Fehler minimieren: $\lvert f(a,n) + f(b,n) + f(c,n) + f(d,n) - \Pi\rvert$

Es ist gegeben `almostPi(200)` = 6<sup>2</sup> + 75<sup>2</sup> + 89<sup>2</sup> + 226<sup>2</sup> = 64658.
