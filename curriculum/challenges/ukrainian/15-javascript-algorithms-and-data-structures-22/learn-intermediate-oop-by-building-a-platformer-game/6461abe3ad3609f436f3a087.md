...
Наступний крок — націлювання на елемент абзацу в межах елемента `.checkpoint-screen`.

Використайте `document.querySelector` та дочірній комбінатор `>`, щоб націлити елемент абзацу.

Призначте це значення `const` змінній під назвою `checkpointMessage`.

# --hints--

Використайте дочірній комбінатор `>` для доступу до елемента абзацу в межах елемента `.checkpoint-screen`.
