...
下一栋建筑将包含三个部分。 在 `.bb4` 中嵌套三个 `div` 元素。 按顺序给它们添加 `bb4a`、`bb4b` 和 `bb4c` 类。

# --hints--

你应该在 `.bb4` 中添加三个 `div` 元素。

```js
assert.equal(document.querySelector("div.bb4")?.children?.length, 3);
```

你应该给第一个新的 `div` 一个 `bb4a` 类。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你应该给第二个新的 `div` 一个 `bb4b` 类。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你应该给第三个新的 `div` 一个 `bb4c` 类。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你应该按 `.bb4a + .bb4b + .bb4c` 的顺序排列新的 `div` 元素。
