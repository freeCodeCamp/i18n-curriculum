...
創建 `.daily-value p` 選擇器以標的 `daily-value` 區段中所有的 `p` 元素。為這個新選擇器設定 `border-bottom` 為 `1px solid #888989`。

# --hints--

你應該有一個新的 `.daily-value p` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

你的 `.daily-value p` 選擇器應該有一個設定為 `1px solid #888989` 的 `border-bottom` 屬性。
