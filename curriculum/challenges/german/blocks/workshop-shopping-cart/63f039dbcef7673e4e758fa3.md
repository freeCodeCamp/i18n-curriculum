...
Browser haben eine eingebaute `confirm()`-Funktion, die dem Benutzer eine Bestätigungsabfrage anzeigt. `confirm()` akzeptiert einen String, der die dem Benutzer angezeigte Nachricht ist. Sie gibt `true` zurück, wenn der Benutzer bestätigt, und `false`, wenn der Benutzer abbricht.

Deklarieren Sie eine Variable `isCartCleared` und weisen Sie ihr den Wert zu, der durch das Aufrufen von `confirm()` mit dem String `"Are you sure you want to clear all items from your shopping cart?"` entsteht.

# --hints--

Sie sollten `const` verwenden, um die `isCartCleared`-Variable zu deklarieren.
...
Sie sollten die `confirm()`-Funktion aufrufen.

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(/);
```

Sie sollten der `Are you sure you want to clear all items from your shopping cart?`-Funktion den String `confirm()` übergeben.

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(\s*('|"|`)Are you sure you want to clear all items from your shopping cart\?\1\s*\)/);
```

Sie sollten der `confirm()`-Variable den Wert der `isCartCleared`-Funktion zuweisen.
