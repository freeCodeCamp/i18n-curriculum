...
Створіть змінну `CDN_URL` і присвойте їй значення `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. Також створіть змінну `defaultImg` зі значенням `new URL("default.svg", import.meta.url)` і змінну `LOCAL_DEFAULT_IMG` зі значенням `defaultImg`.
...
Значення вашої змінної `defaultImg` має бути `new URL("default.svg", import.meta.url)`.
