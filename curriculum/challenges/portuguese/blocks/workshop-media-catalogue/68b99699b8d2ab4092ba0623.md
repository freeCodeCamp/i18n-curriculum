...
Após a declaração `if` no seu método `__str__`, crie duas variáveis chamadas `movies` e `series` e atribua a elas uma chamada ao método `get_movies` e uma chamada ao método `get_tv_series`, respectivamente.

# --hints--

Você deve ter uma variável chamada `movies` no seu método `__str__`.
...
Você atribui `self.get_movies()` à sua variável `movies`.
...
Você deve ter uma variável chamada `series` no seu método `__str__`.
...
Você atribui `self.get_tv_series()` à sua variável `series`.
