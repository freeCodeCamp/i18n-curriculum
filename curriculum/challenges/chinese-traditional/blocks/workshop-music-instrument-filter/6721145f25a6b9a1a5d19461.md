...
如你在先前課程中所學，當使用者修改某些輸入元素的值時，會觸發 `change` 事件。你希望在每次從下拉選單選取新值時都能更新你的頁面。為此，為 `selectContainer` 添加 `change` 事件監聽器。

目前，在回呼內將字串 `this is a test` 記錄到主控台。然後，從你的下拉選單中選擇不同的選項，並檢查主控台以測試事件監聽器是否正常運作。

# --hints--

你應該為 `selectContainer` 添加 `change` 事件監聽器，並在傳遞給該方法的回呼中記錄字串 `this is a test`。
