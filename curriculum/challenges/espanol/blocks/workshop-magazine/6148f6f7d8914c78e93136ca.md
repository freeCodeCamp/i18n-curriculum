...
Crea una última consulta `@media` para `only screen` con un `max-width` de `420px`. Dentro, crea un selector `.hero-title` con una propiedad `font-size` establecida en `4.5rem`.

¡Felicidades! Tu revista ahora está completa.

# --hints--

Deberías tener una nueva consulta `@media` para `only screen and (max-width: 420px)`. Esto debería ser la última consulta en la lista de consultas `@media`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

Tu nueva consulta `@media` debería tener un selector `.hero-title`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

Tu selector `.hero-title` debería tener una propiedad `font-size` establecida en `4.5rem`.
