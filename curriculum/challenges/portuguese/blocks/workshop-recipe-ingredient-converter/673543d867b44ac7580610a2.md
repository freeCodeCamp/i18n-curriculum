...
Nesta etapa, você completará a função `updateResultsList`. 

Agora itere pelo array `units`. Para cada valor, verifique se ele não é igual à unidade atual (`unitToConvert.value`).

Se a condição for verdadeira, converta a unidade existente para a unidade atual e adicione um novo item à lista em `resultList` no formato:

`ingredient: quantity unit`

Por exemplo, se a entrada for `flour`, `5` e `gram`, o resultado deve ser:
...
Ao clicar no botão sua função `updateResultsList` deve atualizar corretamente o DOM.
