...
Dovresti dare alla proprietà `background` un `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Dovresti dare al `repeating-linear-gradient` un primo colore di `--building-color2` dallo `0%` al `5%`.
...
Dovresti dare al `repeating-linear-gradient` un secondo colore di `transparent` dal `5%` al `10%`.
