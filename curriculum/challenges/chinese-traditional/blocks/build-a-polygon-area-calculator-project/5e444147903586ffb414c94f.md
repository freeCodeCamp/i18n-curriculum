...
在這個專案中，你將使用物件導向的程式設計來創建一個 `Rectangle` 類別和一個 `Square` 類別。`Square` 類別應該是 `Rectangle` 的子類別，並繼承其方法和屬性。

## Rectangle 類別

當創建 `Rectangle` 物件時，應該用 `width` 和 `height` 屬性進行初始化。該類別還應包含以下方法：

- `set_width`
- `set_height`
- `get_area`：傳回面積（`width * height`）
- `get_perimeter`：傳回周長（`2 * width + 2 * height`）
- `get_diagonal`：傳回對角線長度（`(width ** 2 + height ** 2) ** .5`）
- `get_picture`：傳回一個字串，該字串使用 `'*'` 線條表述形狀。線條數量應該相等於高度，每條線中的 `'*'` 數量應該相等於寬度。每條線的結尾應該有一個換行符號（`\n`）。如果寬度或高度大於 50，則應傳回字串：`'Too big for picture.'`。
- `get_amount_inside`：接受另一個形狀（正方形或長方形）作為引數。傳回傳入形狀在該形狀內可容納的次數（不旋轉）。例如，寬度為 4、高度為 8 的長方形可以容納兩個邊長為 4 的正方形。

此外，如果一個 `Rectangle` 的實體被表述為字串，它應該看起來像：`'Rectangle(width=5, height=10)'`。

## Square 類別

`Square` 類別應該是 `Rectangle` 的子類別。當創建 `Square` 物件時，會傳入單一邊長。`__init__` 方法應該將邊長同時儲存在 `Rectangle` 類別的 `width` 和 `height` 屬性中。

`Square` 類別應該能夠存取 `Rectangle` 類別的方法，但也應該包含一個 `set_side` 方法。如果 `Square` 的實體以字串表述，應該看起來像：`'Square(side=9)'`。

此外，`Square` 類別上的 `set_width` 和 `set_height` 方法應該同時設定寬度和高度。
...
`Square(5)` 的字串表述應該是 `'Square(side=5)'`。
...
當 `width` 或 `height` 屬性大於 `50` 時，`.get_picture()` 方法應該傳回字串 `'Too big for picture.'`。
