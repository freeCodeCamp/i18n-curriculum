...
`make_allowed_move()` nimmt `rod1` und `rod2` als Parameter entgegen. Hier ist eine kleine Umstrukturierung nötig. Ändern Sie jede Vorkommnis von `source` in `rod1`.

# --hints--

Sie sollten jede Vorkommnis von `source` innerhalb der `make_allowed_move()` Funktion in `rod1` ändern.
