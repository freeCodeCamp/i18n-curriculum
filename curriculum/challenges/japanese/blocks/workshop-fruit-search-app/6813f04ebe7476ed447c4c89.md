...
次に、レスポンスに対して `.json()` を呼び出し、その結果を `data` という変数にストアしてください。まだ `async` 関数の中にいるので、`await` の前に `response.json()` を使う必要があります。

# --hints--

レスポンスの `json()` に対して `response` を呼び出し、その結果を `data` という変数にストアしてください。
