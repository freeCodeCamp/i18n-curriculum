...
線條可以幫助分隔和分組重要內容，尤其是在空間有限的情況下。

在 `h1` 元素下方創建一個 `div` 元素，並將其 `class` 屬性設置爲 `divider`。
