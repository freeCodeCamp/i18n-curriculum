...
Ahora, crea una función llamada `getPreviousSong`. Haz que devuelva la canción que se puede encontrar en `getCurrentSongIndex()` menos uno en `userData.songs`, o `undefined` si la canción actual es la primera de la lista de reproducción.

# --hints--

Debes tener una función llamada `getPreviousSong`.

```js
assert.isFunction(getPreviousSong);
```

Tu función `getPreviousSong` debe llamar a `getCurrentSongIndex`.
...
Tu función `getPreviousSong` debe devolver la canción anterior a la canción actual.
...
Tu función `getPreviousSong` debe devolver `undefined` cuando la canción actual es la primera de la lista de reproducción.
