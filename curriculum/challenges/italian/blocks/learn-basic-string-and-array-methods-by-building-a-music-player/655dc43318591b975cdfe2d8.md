...
All'interno dell'array `allSongs`, crea un oggetto con le seguenti proprietà e valori:
...
Il tuo array `allSongs` dovrebbe contenere un solo valore.

```js
assert.equal(allSongs.length, 1);
```

Il tuo array `allSongs` dovrebbe avere un oggetto come primo valore.

```js
assert.isObject(allSongs[0]);
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `id` impostata al numero `0`.

```js
assert.equal(allSongs[0].id, 0);
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `title` impostata alla stringa `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `artist` impostata alla stringa `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `duration` impostata alla stringa `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `src` impostata alla stringa `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
