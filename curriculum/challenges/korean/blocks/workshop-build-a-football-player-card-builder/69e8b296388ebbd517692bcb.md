...
`FootballPlayerCard`는 현재 기본 카드만 렌더링합니다. 전체 페이지를 만들려면 `page`의 `className`를 가진 `div` 요소를 추가하세요.

그 `div` 안에 `header`의 `className`인 `header` 요소를 추가하세요. `header` 안에는 `header-inner`의 `className`인 `div`를 추가하세요. `header-inner` 요소 안에는 각각 `Football Card Builder`과 `Customize your player card`인 두 개의 문단을 추가하세요. 문단에는 각각 `header-title`와 `header-subtitle`의 `className`도 있어야 합니다.

닫는 `header` 요소 뒤이지만 `page`의 `className`를 가진 `div` 안에는 메인 레이아웃이 추가될 때까지 임시 자리 표시자로서 프로프를 가진 `PlayerCard` 컴포넌트를 넣으세요.

# --hints--

`FootballPlayerCard`는 `page`의 `className`를 가진 `div`를 렌더링해야 합니다.
...
`div`는 `page`의 `className`를 가진 `header` 요소를 포함해야 하며, 그 요소는 `className`가 `className`인 `header`를 가져야 합니다.
...
`header`는 `header-inner`의 `className`를 가진 `div`를 포함해야 합니다.
...
`header-inner` 요소는 `header-title`의 `className`를 가진 `p`와 `Football Card Builder` 텍스트를 포함해야 합니다.
...
`header-inner` 요소는 `header-subtitle`의 `className`를 가진 `p`와 `Customize your player card` 텍스트를 포함해야 합니다.
...
`PlayerCard` 컴포넌트는 `div.page` 안에 렌더링되어야 합니다.
