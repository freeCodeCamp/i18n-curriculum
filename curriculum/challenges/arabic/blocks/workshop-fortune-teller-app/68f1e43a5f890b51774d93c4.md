...
داخل طريقة `showFortune` أعلن متغيرًا اسمه `target` وقم بتعيينه إلى `e.target` وأنشئ جملة `if` فارغة أخرى.

# --hints--

يجب إعلان متغير باسم `target` داخل `showFortune`.
...
يجب تعيين متغير `target` إلى `e.target`.
...
يجب أن يكون لديك جملة `if` فارغة مباشرة بعد متغير `target`.
