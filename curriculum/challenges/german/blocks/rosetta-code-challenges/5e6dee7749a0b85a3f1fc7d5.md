...
Lucas-Lehmer-Test: Für $p$ eine ungerade Primzahl ist die Mersenne-Zahl $2^p-1$ genau dann eine Primzahl, wenn $2^p-1$ $S(p-1)$ teilt, wobei $S(n+1)=(S(n))^2-2$ gilt und $S(1)=4$ ist.

# --instructions--

Schreiben Sie eine Funktion, die zurückgibt, ob die gegebene Mersenne-Zahl eine Primzahl ist oder nicht.
