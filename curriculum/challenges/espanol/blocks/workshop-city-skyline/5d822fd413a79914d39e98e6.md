...
Anida seis elementos `div` dentro de `.foreground-buildings` y asígnales las clases de `fb1` a `fb6` en ese orden. "fb" significa "foreground building". Estos serán seis edificios más para el primer plano.

# --hints--

Debe crear un nuevo `div` con la clase `fb1`.

```js
assert.exists(document.querySelector('div.fb1'));
```

Debe crear un nuevo `div` con la clase `fb2`.

```js
assert.exists(document.querySelector('div.fb2'));
```

Debe crear un nuevo `div` con la clase `fb3`.

```js
assert.exists(document.querySelector('div.fb3'));
```

Debe crear un nuevo `div` con la clase `fb4`.

```js
assert.exists(document.querySelector('div.fb4'));
```

Debe crear un nuevo `div` con la clase `fb5`.

```js
assert.exists(document.querySelector('div.fb5'));
```

Debe crear un nuevo `div` con la clase `fb6`.
