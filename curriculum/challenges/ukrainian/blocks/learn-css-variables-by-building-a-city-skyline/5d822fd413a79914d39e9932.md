...
Переходимо до наступної будівлі! Це зелена будівля на передньому плані. Додайте їй `repeating-linear-gradient` з вашим кольором будівлі від `0%` до `5%`, а `transparent` від `5%` до `10%`.
...
Ви повинні надати `background` `repeating-linear-gradient`.
