...
Додайте ще один `case` для випадку, коли довжина `results` дорівнює `1`. У цьому випадку призначте `result_list` список, що містить рядок у форматі `x = <root>`, де `<root>` — розв’язок рівняння. Відформатуйте рядок так, щоб для розв’язку відображалися як додатний, так і від’ємний знак.
...
Ви повинні створити новий `case` із шаблоном `1` після існуючого блоку `case`.
...
Ви повинні призначити списку, що містить `f'x = {results[0]:+}'`, `result_list` всередині тіла вашого нового `case`.
