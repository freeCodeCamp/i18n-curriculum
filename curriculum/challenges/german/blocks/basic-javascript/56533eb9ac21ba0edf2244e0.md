...
Wenn Sie viele Optionen zur Auswahl haben, kann eine `switch`-Anweisung leichter zu schreiben sein als viele verkettete `if`/`else if`-Anweisungen. Folgendes:
...
Ändern Sie die verketteten `if`/`else if`-Anweisungen in eine `switch`-Anweisung.

# --hints--

Sie sollten im Editor keine `else`-Anweisungen verwenden

```js
assert(!/else/g.test(__helpers.removeJSComments(code)));
```

Sie sollten im Editor keine `if`-Anweisungen verwenden

```js
assert(!/if/g.test(__helpers.removeJSComments(code)));
```

Sie sollten mindestens vier `break`-Anweisungen haben
