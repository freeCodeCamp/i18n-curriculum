...
Після того, як ваш `rangeRegex` знайде `:`, йому потрібно шукати той самий шаблон із літер і цифр, що й раніше.

Скопіюйте свої дві існуючі групи захоплення і вставте їх після двокрапки.

# --hints--

Ви повинні додати третю групу захоплення до свого `rangeRegex`, після двокрапки.
...
Ваша третя група захоплення має використовувати клас символів.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[.*\]\)/);
```

Ваша третя група захоплення має відповідати символам `A` до `J`.
...
Ви повинні додати четверту групу захоплення до свого `rangeRegex`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(.*\)/);
```

Ваша четверта група захоплення має відповідати одній або двом цифрам.
