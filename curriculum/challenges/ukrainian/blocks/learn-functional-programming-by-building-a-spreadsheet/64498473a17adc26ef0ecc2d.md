...
Ваша функція `idToText` наразі повертає елемент `input`. Оновіть її, щоб вона повертала властивість `value` цього елемента `input`.

# --hints--

Ви маєте повернути властивість `value` поверненого значення методу `.find()`.
