...
Para almacenar el tiempo actual de la canción cuando está en pausa, establece la `songCurrentTime` del objeto `userData` al `currentTime` de la variable `audio`.

*Note*: No debes usar el encadenamiento opcional para este paso porque `userData.songCurrentTime` no será `null` o `undefined` en este punto.
...
Debes asignar `audio.currentTime` a `userData.songCurrentTime`.
