...
音楽プレーヤーは曲、現在再生中の曲、そして現在の曲の再生時間を管理する必要があります。これを実現するために、これらの情報をストアするオブジェクトを作成してください。

まず、`let` キーワードを使って `userData` という新しい変数を宣言し、空のオブジェクトを代入してください。

# --hints--

`let` キーワードを使って `userData` 変数を作成してください。

```js
assert.match(code, /let\s+userData\s*=/)
```

`userData` 変数に空のオブジェクトを代入してください。
