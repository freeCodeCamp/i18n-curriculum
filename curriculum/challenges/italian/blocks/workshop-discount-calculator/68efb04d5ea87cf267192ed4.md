...
Crea un'istanza di `DiscountEngine` e archiviala nella variabile `engine`. Dopodiché, calcola il prezzo migliore usando il metodo `calculate_best_price` e archivialo in una variabile chiamata `best_price`.

# --hints--

Dovresti creare un'istanza di `DiscountEngine` con la lista `strategies`.
...
Dovresti chiamare `engine.calculate_best_price(product, user_tier)` e archiviare il risultato in `best_price`.
