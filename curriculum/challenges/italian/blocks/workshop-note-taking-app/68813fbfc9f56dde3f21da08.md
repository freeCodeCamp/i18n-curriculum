...
Quando un utente modifica una nota e preme tab o clicca all'esterno della scheda, deve apparire un messaggio di stato che informa che la nota è stata salvata con successo.

Per cominciare, crea un elemento `div` con un attributo `id` impostato su `"status"`.

# --hints--

Dovresti avere un secondo elemento `div`.

```js
assert.lengthOf(document.querySelectorAll("div"), 2);
```

Il tuo secondo elemento `div` dovrebbe avere un attributo `id`.

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

Il tuo secondo elemento `div` dovrebbe avere un attributo `id` impostato su `"status"`.
