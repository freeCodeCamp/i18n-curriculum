...
앱이 지원하는 단위 목록을 유지해야 합니다.

`units` 배열을 선언하고, 문자열 값 `"cup"`, `"gram"`, `"ounce"`, 그리고 `"teaspoon"`를 할당하세요.
...
`units` 배열은 네 개의 요소를 가져야 합니다.

```js
assert.lengthOf(units, 4);
```

`units` 배열에는 문자열 `"cup"`가 포함되어야 합니다.

```js
assert.include(units, "cup");
```

`units` 배열에는 문자열 `"gram"`가 포함되어야 합니다.

```js
assert.include(units, "gram");
```

`units` 배열에는 문자열 `"ounce"`가 포함되어야 합니다.

```js
assert.include(units, "ounce");
```

`units` 배열에는 문자열 `"teaspoon"`가 포함되어야 합니다.
