...
أخيرًا، استخدم الطريقة `.appendChild()` لإضافة عنصر `label` إلى عنصر `container`.

# --hints--

يجب أن تصل إلى الطريقة `.appendChild()` الخاصة بعنصر `container`.

```js
assert.match(code, /container\.appendChild\(/);
```

يجب أن تمرر عنصر `label` إلى الطريقة `.appendChild()`.

```js
assert.match(code, /container\.appendChild\(\s*label\s*\)/);
```

يجب أن تضيف `label` بعد تعيين الخصائص.
