...
配列 `allSongs` の中に、次のプロパティと値を持つオブジェクトを作成してください。
...
`allSongs` 配列内に要素が 1 つ必要です。

```js
assert.equal(allSongs.length, 1);
```

配列 `allSongs` の最初の要素はオブジェクトであるべきです。

```js
assert.isObject(allSongs[0]);
```

配列 `allSongs` は、`id` プロパティが数値の `0` に設定されたオブジェクトを持つようにしてください。

```js
assert.equal(allSongs[0].id, 0);
```

配列 `allSongs` は、`title` プロパティが文字列の `"Scratching The Surface"` に設定されたオブジェクトを持つようにしてください。

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

配列 `allSongs` は、`artist` プロパティが文字列の `"Quincy Larson"` に設定されたオブジェクトを持つようにしてください。

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

配列 `allSongs` は、`duration` プロパティが文字列の `"4:25"` に設定されたオブジェクトを持つようにしてください。

```js
assert.equal(allSongs[0].duration, "4:25");
```

配列 `allSongs` は、`src` プロパティが文字列の `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"` に設定されたオブジェクトを持つようにしてください。
