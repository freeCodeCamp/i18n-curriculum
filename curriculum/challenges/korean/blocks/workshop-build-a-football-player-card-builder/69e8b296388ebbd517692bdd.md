...
PAS `value`에 `player.pas` 속성을 `input`로 추가하세요. 그런 다음 `onChange`를 매개변수로 받고 `e`를 펼친 것과 `setPlayer`를 `player`로 설정하여 `pas`를 호출하는 화살표 함수로 설정된 `Number(e.target.value)` 속성을 추가하세요.

# --hints--

PAS `input`는 플레이어의 `value` 스탯과 같은 `pas`을 가져야 합니다.
...
PAS `input`는 `onChange`로 설정된 `setPlayer`와 `player`를 펼쳐서 `pas`를 호출하는 `Number(e.target.value)` 핸들러를 가져야 합니다.
