...
將 `repeating-linear-gradient` 添加到 `.fb1c` 已有的漸變下方，從 `0%` 到 `10%` 處爲 `--building-color4`，從 `10%` 到 `90%` 處爲 `--window-color4`。 這將在你之前添加的漸變後面。

# --hints--

你不應更改第一個 `repeating-linear-gradient`。
...
你應該添加一個 `repeating-linear-gradient`，從 `0%` 到 `10%` 處爲第一個顏色 `--building-color4`。
...
你應該在從 `10%` 到 `90%` 處添加第二種顏色 `--window-color4`。
