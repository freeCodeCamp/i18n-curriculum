...
接下來，你需要創建一個函式來隱藏 DOM 中的元素。宣告一個名為 `hideElements` 的函式，該函式接受一個名為 `...elements`、型別為 `HTMLElement[]` 的參數。

這個函式會使用一個 CSS `hidden` 類別，並在整個應用程式中用來切換 UI 元素的可見性。

# --hints--

你應該已經宣告了一個名為 `hideElements` 的函式。
...
你的 `hideElements` 函式應該有一個名為 `...elements`、型別為 `HTMLElement[]` 的參數。
