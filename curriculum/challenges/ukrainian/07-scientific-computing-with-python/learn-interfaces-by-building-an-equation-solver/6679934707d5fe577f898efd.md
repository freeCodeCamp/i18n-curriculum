...
Додайте інший випадок, для випадку коли довжина `results` є `2`. Цього разу призначте `result_list` список, що містить два рядки у форматі `x1 = <root1>` та `x2 = <root2>`. Знову зробіть так, щоб розв’язок мав як позитивні, так і негативні знаки.
...
Створіть новий `case` із шаблоном `2` після наявного блоку `case`.
...
Призначте список, що містить два рядки у форматі `x1 = <root1>` та `x2 = <root2>` до `result_list` всередині тіла нового `case`. Відображайте як позитивні, так і негативні знаки для результатів.
