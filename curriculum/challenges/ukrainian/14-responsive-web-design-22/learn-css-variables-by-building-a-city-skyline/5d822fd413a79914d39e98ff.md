...
Потім видаліть `width` та `height` з `.bb2a` і змініть `border-left` та `border-right`, щоб використовувати `5vw` замість `1vw`. Тепер елемент матиме нульовий розмір, а кордони зійдуться посередині.
