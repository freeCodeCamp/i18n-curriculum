...
`.bb1` 要素をフレックスボックスの親にして、建物のパーツを中央に配置してください。子要素を中央に配置するには、`flex-direction` と `align-items` のプロパティを使います。

# --hints--

`width` 要素の `height` や `.bb1` のプロパティは変更しないでください。
...
`.bb1` 要素に `display` を `flex` に設定してください。
...
`.bb1` 要素に `flex-direction` を `column` に設定してください。
...
`.bb1` 要素に `align-items` を `center` に設定してください。
