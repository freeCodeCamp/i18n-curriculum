...
你需要一個函式（程式）來更新顯示排序後的數字。首先使用箭頭語法宣告一個名為 `updateUI` 的函式（程式），該函式（程式）接受一個單一的 `array` 參數。

因為你將會撰寫不會立即有返回值的演算法，所以為 `array` 設定一個備用值為空陣列。以下是一個具有備用值的函式範例：
...
`updateUI` 應該接受一個單一的 `array` 參數。

```js
assert.match(code, /const\s+updateUI\s*=\s*\(?\s*array/);
```

`updateUI` 應該有一個空陣列作為 `array` 的備用值。
