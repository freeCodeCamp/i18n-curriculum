...
Використайте `const` та стрілковий синтаксис, щоб створити порожню функцію з назвою `shuffle`.

Ця функція відповідає за перемішування пісень у плейлисті та виконання необхідних оновлень стану після перемішування.

# --hints--

Ви повинні використати `const`, щоб створити порожню функцію з назвою `shuffle`.
...
Ваша функція `shuffle` має бути стрілковою і не повинна приймати параметрів.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

Ваша функція `shuffle` має бути порожньою.
