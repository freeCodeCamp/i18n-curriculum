...
Оголосіть змінну `rangeExpanded` та присвоїте до неї результат виклику методу `.replace()` на параметрі `x`. Передайте змінну `rangeRegex` як аргумент.

# --hints--

Оголосіть змінну `rangeExpanded`.
...
Використайте `const`, щоб оголосити змінну `rangeExpanded`.
...
Присвоїте результат виклику `.replace()` на `x` змінній `rangeExpanded`.
