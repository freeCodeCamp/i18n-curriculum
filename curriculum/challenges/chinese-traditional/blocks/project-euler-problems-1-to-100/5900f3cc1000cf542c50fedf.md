...
Su Doku（日文意為*數字位置*）是指一種流行的謎題概念。其起源不明，但必須歸功於 Leonhard Euler，他發明了一種類似且更為困難的謎題概念，稱為 Latin Squares。Su Doku 謎題的目標是將 9×9 格子中的空白（或零）取代，使每一列、每一行及每個 3×3 方格都包含 1 到 9 的數字。以下是典型起始謎題格子及其解答格子的範例。
...
一個良好構成的 Su Doku 謎題具有唯一的解法，且可以透過邏輯來解決，儘管可能需要使用「猜測與測試」的方法來排除選項（此點存在許多爭議）。搜尋的複雜度決定謎題的難度；上述範例被認為是簡單的，因為它可以透過直接推導來解決。

`puzzlesArr` 陣列包含不同難度的數獨謎題字串，但所有謎題皆有唯一解。

透過解決 `puzzlesArr` 中的所有謎題，找出每個解決方案網格左上角的三位數字總和；例如，483 是上述解決方案網格左上角找到的三位數字。

# --hints--

`suDoku(testPuzzles1)` 應該傳回一個數字。

```js
assert(typeof suDoku(_testPuzzles1) === 'number');
```

`suDoku(testPuzzles1)` 應該傳回 `1190`。

```js
assert.strictEqual(suDoku(_testPuzzles1), 1190);
```

`suDoku(testPuzzles2)` 應該傳回 `24702`。
