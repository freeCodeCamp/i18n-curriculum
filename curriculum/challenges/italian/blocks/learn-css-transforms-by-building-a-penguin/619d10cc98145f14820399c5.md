...
Sotto l'elemento `.blush.right`, aggiungi due elementi `div` ciascuno con un `class` di `beak`. Inoltre, assegna al primo elemento `.beak` un `class` di `top`, e al secondo elemento `.beak` un `class` di `bottom`.

# --hints--

Dovresti aggiungere due elementi `div` all'interno di `.penguin-head`. Previsti `--fcc-expected--` elementi `div`, trovati `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 9);
```

Dovresti assegnare al primo nuovo `div` un `class` di `beak`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

Dovresti assegnare al secondo nuovo `div` un `class` di `beak`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div.beak')?.length, 2);
```

Dovresti assegnare al primo nuovo `div` un `class` di `top`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

Dovresti assegnare al secondo nuovo `div` un `class` di `bottom`.
