...
배열이 여러 데이터 타입을 넣을 수 있는 것처럼, 배열의 길이는 고정되어 있지 않습니다. 배열은 요소의 개수와 관계없이 정의할 수 있고, 나중에 요소를 추가하거나 삭제할 수도 있습니다. 다시 말해, 배열은 <dfn>mutable</dfn> 합니다. 즉, 선언 후 배열을 변경할 수 있습니다. 이번 챌린지에서는 배열을 프로그래밍적으로 수정할 수 있는 두 가지 메서드, `Array.push()`와 `Array.unshift()`를 살펴보겠습니다.
