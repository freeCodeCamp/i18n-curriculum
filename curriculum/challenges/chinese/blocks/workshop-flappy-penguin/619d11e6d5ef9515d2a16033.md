...
选择 `class` 为 `top` 的 `.beak` 元素，将其 `width` 设置为 `20%`，并放置在父元素顶部 `60%` 左侧 `40%` 的位置。
...
你应该将 `.beak.top` 的 `width` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.width, '20%');
```

你应该将 `.beak.top` 的 `top` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.top, '60%');
```

你应该将 `.beak.top` 的 `left` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
