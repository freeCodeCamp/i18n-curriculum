...
`.blush.right` 要素の下に、`div` の `class` を持つ `beak` 要素を2つ追加してください。また、最初の `.beak` 要素には `class` の `top` を、2番目の `.beak` 要素には `class` の `bottom` を付けてください。

# --hints--

`div` 内に `.penguin-head` 要素を2つ追加する必要があります。期待される `div` 要素は --fcc-expected-- ですが、実際には --fcc-actual-- でした。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 9);
```

最初の新しい `div` に `class` の `beak` を付けてください。

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

2番目の新しい `div` に `class` の `beak` を付けてください。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div.beak'), 2);
```

最初の新しい `div` に `class` の `top` を付けてください。

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

2番目の新しい `div` に `class` の `bottom` を付けてください。

```js
assert.exists(document.querySelector('.penguin-head > div.beak.bottom'));
```

`div.beak.top` の後に `div.blush.right` を配置してください。

```js
assert.exists(document.querySelector('.blush.right + .beak.top'));
```

`div.beak.bottom` の後に `div.beak.top` を配置してください。
