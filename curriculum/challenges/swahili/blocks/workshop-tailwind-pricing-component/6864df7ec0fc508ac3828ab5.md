...
Kutoka kwa CSS ya kawaida, ulijifunza kuwa kuweka thamani ya display ya `grid` kwa kipengele hakufanyi chochote hadi uweke safu na nguzo za gridi. Hivyo ndivyo inavyofanya kazi katika Tailwind pia.

Katika Tailwind, kuweka safu ya gridi na nguzo ya gridi, utatumia madarasa ya huduma yao kama ifuatavyo:
...
Weka madarasa ya huduma `div` na `grid-cols-1` kwa kifungashio cha `md:grid-cols-3`.

Darasa la `grid-cols-1` linaweka vitu vitatu katika safu moja, vikikusanyika wima. Kwenye skrini za wastani na kubwa zaidi, `md:grid-cols-3` hubadilisha mpangilio kuwa nguzo tatu, ikionyesha vitu kando kando.

# --hints--

Kipengele chako cha `div` kinapaswa kuwa na darasa la `grid-cols-1`.
...
Kipengele chako cha `div` kinapaswa kuwa na darasa la `md:grid-cols-3`.
