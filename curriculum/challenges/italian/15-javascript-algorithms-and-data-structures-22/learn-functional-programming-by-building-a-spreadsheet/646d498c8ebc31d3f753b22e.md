...
L'oggetto `spreadsheetFunctions` dovrebbe avere una proprietà `firsttwo`.

```js
assert.property(spreadsheetFunctions, "firsttwo");
```

La proprietà `firsttwo` dovrebbe essere una funzione.

```js
assert.isFunction(spreadsheetFunctions.firsttwo);
```

La funzione `firsttwo` dovrebbe restituire i primi due numeri dell'array.

```js
assert.deepEqual(spreadsheetFunctions.firsttwo([1, 2, 3, 4, 5, 6]), [1, 2]);
```

L'oggetto `spreadsheetFunctions` dovrebbe avere una proprietà `lasttwo`.

```js
assert.property(spreadsheetFunctions, "lasttwo");
```

La proprietà `lasttwo` dovrebbe essere una funzione.

```js
assert.isFunction(spreadsheetFunctions.lasttwo);
```

La funzione `lasttwo` dovrebbe restituire gli ultimi due numeri dell'array.
