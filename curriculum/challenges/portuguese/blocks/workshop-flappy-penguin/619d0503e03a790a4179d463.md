...
Posicione o elemento `.chin` a `25%` da parte superior e a `5%` da esquerda do elemento pai.Em seguida, dê aos cantos superiores um raio de `70%` e aos cantos inferiores um raio de `100%`.

# --hints--

Você deve dar a `.chin` uma propriedade `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Você deve dar ao `.chin` um `top` de `--fcc-expected--`, mas foi encontrado `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Você deve dar a `.chin` uma propriedade `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Você deve dar ao `.chin` um `left` de `--fcc-expected--`, mas foi encontrado `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Você deve dar a `.chin` um `border-radius` de `70% 70% 100% 100%`.
