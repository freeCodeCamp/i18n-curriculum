...
通過從兩個 `linear-gradient` 函數中刪除 `gradientDirection` 參數來進一步清理你的代碼。

# --hints--

你應該從 `.red` 和 `.green` CSS 規則中的 `linear-gradient` 函數中刪除 `gradientDirection` 參數。

```js
assert(!code.match(/linear-gradient\s*\(\s*180deg/gi));
```

你的 `.red` CSS 規則應該有一個 `background` 屬性設置爲 `linear-gradient(rgb(122, 74, 14), rgb(245, 62, 113), rgb(162, 27, 27))`。
...
你的 `.green` CSS 規則應將 `background` 屬性設置爲 `linear-gradient(#55680D, #71F53E, #116C31)`。
