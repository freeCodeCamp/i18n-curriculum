...
`cartTaxes` 요소의 `textContent` 속성에 접근하세요.

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

`tax` 변수에서 `.toFixed()` 메서드를 호출하고 `2`를 인수로 전달하세요.
...
`cartTotal` 요소의 `textContent` 속성에 접근하세요.

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

`total` 변수에서 `.toFixed()` 메서드를 호출하고 `2`를 인수로 전달하세요. `this` 키워드를 사용하는 것을 잊지 마세요.
