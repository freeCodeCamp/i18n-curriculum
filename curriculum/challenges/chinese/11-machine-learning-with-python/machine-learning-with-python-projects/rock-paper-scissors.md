...
你将 <a href="https://gitpod.io/?autostart=true#https://github.com/freeCodeCamp/boilerplate-rock-paper-scissors/" target="_blank" rel="noopener noreferrer nofollow">使用我们的 Gitpod 启动代码来处理这个项目</a>。
...
此项目的单元测试在 `test_module.py` 中。 为了你的方便，我们将测试从 `test_module.py` 导入到 `main.py`。 你如果移除 `main.py` 中最后一行的注释，那么每次在控制台执行 `python main.py` 时，测试都会自动运行。
