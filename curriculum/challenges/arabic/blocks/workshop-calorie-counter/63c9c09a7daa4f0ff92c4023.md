...
باستخدام نفس الصيغة النحوية، استعلم عن مُدخَلات `number` داخل عنصر `#lunch` وقم بتعيينها إلى `lunchNumberInputs`.

# --hints--

يجب إعلان متغير `lunchNumberInputs`.

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=/);
```

يجب استدعاء `document.querySelectorAll()` مع المحدد `#lunch input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

يجب تعيين نتيجة استدعاء `document.querySelectorAll()` إلى `lunchNumberInputs`.
