...
これで、`.author-name` 内の `a` 要素に対してのみ有効なホバーエフェクトが作成され、背景に freeCodeCamp の初期のテーマカラーであった緑色が表示されます。

# --hints--

`.author-name a:hover` セレクターが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

`.author-name a:hover` セレクターの `background-color` プロパティを `#306203` に設定する必要があります。
