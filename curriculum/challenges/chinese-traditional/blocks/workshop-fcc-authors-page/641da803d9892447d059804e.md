...
在 `displayAuthors` 函式中，已為你解構每位作者的資料。

現在創建一個影像標記並為它添加 `class` 屬性為 `"user-img"`。使用字串插值將 `src` 屬性設定為 `image` 屬性。將 `alt` 屬性設定為 `author`，後面接文字 `"avatar"`。確保在 `author` 變數和字 `"avatar"` 之間有一個空格，例如 `"Quincy Larson avatar"`。
...
你應該將你的 `img` 元素的 `alt` 屬性設為 `${author} avatar`。
