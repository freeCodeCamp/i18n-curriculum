...
Após o `form-row`, adicione um `div` com o `className` de `form-group`. Dentro dele, adicione um elemento `label` com o `className` de `label`, um `htmlFor` de `club` e o texto `Club`. Depois adicione um elemento input com um `id` de `club`, um `className` de `input` e um `type` de `text`.
...
Seu `div.form-group` deve ter um `label` com um atributo `htmlFor` de `club`, um `className` de `label` e o texto `Club`.
...
Seu Club `form-group` deve conter um `input` com um `id` de `club`, um `className` de `input` e um `type` de `text`.
