...
Wakati kuna sufuria za kutosha kuuza unahitaji kusahihisha katalogi kwa kutoa sufuria za ukubwa ulioainisha. Katika hali hiyo, rudisha `Catalog successfully updated.`

Badilisha kitendakazi chako kufanikisha hilo. Kisha, sasisha wito wa kitendakazi chako kwa kupitisha `10` kama hoja ya tatu badala ya `25`.

# --hints--

Unapaswa kuandika `sellPlants(ballerina, "small", 10)` kwenye konsoli.

```js
assert.match(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

Unapaswa kuwa na wito mmoja tu wa `sellPlants` katika msimbo wako.

```js
assert.lengthOf(__helpers.removeJSComments(code).match(/(?<!function\s+)sellPlants\s*\(/g), 1)
```

`sellPlants(ballerina, "small", 25)` inapaswa kurudisha `Not enough small size pots for Lavandula stoechas 'Ballerina'. Only 10 left.`
...
`sellPlants(ballerina, "small", 10)` inapaswa kurudisha `Catalog successfully updated.` na kutoa `10` kutoka kwa sifa ya `small` ya kitu kinachohusiana na ufunguo wa `ballerina` katika katalogi.
