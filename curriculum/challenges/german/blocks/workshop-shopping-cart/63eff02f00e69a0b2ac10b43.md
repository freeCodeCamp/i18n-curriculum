...
Jetzt müssen Sie sich darauf vorbereiten, die Anzeige mit dem neuen Produkt zu aktualisieren, das der Benutzer hinzugefügt hat. Deklarieren Sie eine `currentProductCount`-Variable und weisen Sie ihr den Wert der `totalCountPerProduct`-Objekteigenschaft zu, die dem `id` von `product` entspricht.

# --hints--

Sie sollten eine `currentProductCount`-Variable in Ihrer `addItem`-Funktion deklarieren.
...
Sie sollten `const` verwenden, um `currentProductCount` zu deklarieren.
...
Sie sollten den Wert von `totalCountPerProduct[product.id]` `currentProductCount` zuweisen.
