...
Kwa sasa, unapaswa kuona bahati ambayo kila kadi inaonyesha mara unapo bofya kadi. Kitu pekee kilichobaki ni kufanya kitufe cha kusoma upya kifanye kazi.

Ndani ya njia ya `newReading`, fanya wito kwa `showElements()` na `hideElements()`.

# --hints--

Unapaswa kufanya wito kwa kitendakazi cha `showElements` ndani ya njia ya `newReading`.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

Unapaswa kufanya wito kwa kitendakazi cha `hideElements` ndani ya njia ya `newReading`.
