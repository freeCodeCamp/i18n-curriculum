...
Dentro da função `playSong`, chame a função `highlightCurrentSong`.

Depois disso, experimente os botões de controle para ver como a função `highlightCurrentSong` funciona.

# --hints--

Você deve chamar `highlightCurrentSong` dentro da função `playSong`.
