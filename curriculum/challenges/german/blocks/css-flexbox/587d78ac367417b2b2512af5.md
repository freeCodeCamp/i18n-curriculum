...
title: "Die Eigenschaft flex-direction anwenden, um eine Spalte im Tweet-Embed zu erstellen"
challengeType: 0
...
Das Tweet-Embed `header` und `footer` verwendeten zuvor die Eigenschaft `flex-direction` mit dem Wert row. Ebenso würden die Elemente innerhalb des `.profile-name`-Elements gut als Spalte gestapelt funktionieren.

# --instructions--

Fügen Sie dem `flex-direction`-Element der Kopfzeile die CSS-Eigenschaft `.profile-name` hinzu und setzen Sie den Wert auf `column`.
...
Das `.profile-name`-Element sollte eine `flex-direction`-Eigenschaft mit dem Wert `column` haben.
