...
`.push()` を使って、`quotient` 変数を `quotients` 配列に追加してください。また、`remainder` 変数を `remainders` 配列に追加してください。

# --hints--

`.push()` ループ内で `quotients` 配列に対して `while` メソッドを使うべきです。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*quotients\.push\(/);
```

`.push()` 配列に `quotient` を追加するには、`quotients` メソッドを使うべきです。

```js
assert.match(String(decimalToBinary), /quotients\.push\(\s*quotient\s*\)\s*;?/);
```

`.push()` ループ内で `remainders` 配列に対して `while` メソッドを使うべきです。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*remainders\.push\(/);
```

`.push()` 配列に `remainder` を追加するには、`remainders` メソッドを使うべきです。
