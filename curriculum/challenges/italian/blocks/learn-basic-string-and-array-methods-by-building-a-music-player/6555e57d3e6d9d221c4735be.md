...
Quando si preme il pulsante di mescolamento, vuoi impostare `currentSong` a nulla e `songCurrentTime` a `0`.

Imposta `userData.currentSong` su `null` e `userData.songCurrentTime` su `0`.

*Nota*: Non dovresti usare il concatenamento opzionale per questo passaggio perché stai esplicitamente impostando le proprietà `currentSong` e `songCurrentTime` per essere rispettivamente `null` e `0`.

# --hints--

Dovresti impostare `userData.currentSong` su `null`.

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

Dovresti impostare `userData.songCurrentTime` su `0`.
