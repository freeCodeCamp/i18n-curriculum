...
داخل الدالة `playNextSong`، استدعِ الدوال `setPlayerDisplay` و`highlightCurrentSong` و`setPlayButtonAccessibleText` لتحديث المشغل بشكل صحيح عند انتهاء قائمة التشغيل.

# --hints--

يجب استدعاء `setPlayerDisplay` داخل جملة `else` في دالتك `playNextSong`.
...
يجب استدعاء `highlightCurrentSong` داخل جملة `else` في دالتك `playNextSong`.
...
يجب استدعاء `setPlayButtonAccessibleText` داخل جملة `else` في دالتك `playNextSong`.
