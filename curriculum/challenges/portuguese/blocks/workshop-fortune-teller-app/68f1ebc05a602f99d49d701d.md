...
Agora, dentro do `showElements`, passe estes como argumentos: `singleCardBtn`, `multipleCardsBtn` e `headerTitle`. Lembre-se de que você os obtém do `elements` e acessa-os com a palavra-chave `this`.

# --hints--

Você deve passar `singleCardBtn` como o primeiro argumento para a função `showElements`.
...
Você deve passar `multipleCardsBtn` como o segundo argumento para a função `showElements`.
...
Você deve passar `headerTitle` como argumento para a função `showElements`.
