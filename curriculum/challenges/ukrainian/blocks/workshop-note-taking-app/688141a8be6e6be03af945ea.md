...
Коли користувач редагує нотатку, ви хочете відслідковувати відредаговану версію.

Використайте `let`, щоб створити змінну з назвою `currentContent` і призначити їй порожній рядок.

*ПРИМІТКА*: У реальному застосунку ви зазвичай зберігали б нотатки у базі даних. Або навіть могли б зберігати їх у локальному сховищі. Проте робота з базами даних і локальним сховищем виходить за межі цього практичного заняття, і ці поняття будуть викладені пізніше.

# --hints--

Ви повинні використати `let`, щоб створити змінну `currentContent`.

```js
assert.match(code, /let\s+currentContent/);
```

Ваша змінна `currentContent` має бути рядком.

```js
assert.isString(currentContent);
```

Ваша змінна `currentContent` має бути ініціалізована порожнім рядком.
