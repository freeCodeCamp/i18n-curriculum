...
Тепер додайте `div` з `className` `form-group`. Всередині нього додайте `label` `className` `label` та атрибут `htmlFor` зі значенням `name`. Під цим додайте `input` з множиною `id`, встановленою на `name`, `className` `input` та атрибут `type` зі значенням `text`.
