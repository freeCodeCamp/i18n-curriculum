...
I moduli usano comunemente delle caselle di spunta (checkbox) per le domande che possono avere più di una risposta. The `input` element with a `type` attribute set to `checkbox` creates a checkbox.
