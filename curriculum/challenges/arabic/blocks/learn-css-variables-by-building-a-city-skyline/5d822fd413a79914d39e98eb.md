...
الخط الأفقي يتشكل تدريجيًا. املأ خاصية `background-color` للمباني الأمامية. استخدم المتغير `--building-color1` لملء `.fb3` و `.fb4`، و`--building-color2` لـ `.fb5`، و`--building-color3` لـ `.fb2` و `.fb6`، و`--building-color4` لـ `.fb1`.

# --hints--

يجب أن تعطي `.fb1` خاصية `background-color` باستخدام `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb1')?.getPropVal('background-color', true), 'var(--building-color4)');
```

يجب أن تعطي `.fb2` خاصية `background-color` باستخدام `--building-color3`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb2')?.getPropVal('background-color', true), 'var(--building-color3)');
```

يجب أن تعطي `.fb3` خاصية `background-color` باستخدام `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb3')?.getPropVal('background-color', true), 'var(--building-color1)');
```

يجب أن تعطي `.fb4` خاصية `background-color` باستخدام `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.getPropVal('background-color', true), 'var(--building-color1)');
```

يجب أن تعطي `.fb5` خاصية `background-color` باستخدام `--building-color2`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb5')?.getPropVal('background-color', true), 'var(--building-color2)');
```

يجب أن تعطي `.fb6` خاصية `background-color` باستخدام `--building-color3`.
