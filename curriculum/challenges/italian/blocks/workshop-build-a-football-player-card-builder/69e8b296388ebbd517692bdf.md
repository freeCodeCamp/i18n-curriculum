...
Aggiungi gli attributi `value` e `onChange` a entrambi gli input DEF e PHY.

Per DEF, imposta `value` su `player.def` e `onChange` su una funzione freccia che chiama `setPlayer` con uno spread di `player` e `def` impostato su `Number(e.target.value)`.

Per PHY, imposta `value` su `player.phy` e `onChange` su una funzione freccia che chiama `setPlayer` con uno spread di `player` e `phy` impostato su `Number(e.target.value)`.

Con questo passaggio, tutti gli input sono ora collegati allo stato e l'anteprima live si aggiornerà per ogni campo!

# --hints--

Il tuo `input` DEF dovrebbe avere un `value` uguale allo stato `def` del giocatore.
...
Il tuo `input` DEF dovrebbe avere un gestore `onChange` che chiama `setPlayer` con lo spread di `player` e `def` impostato su `Number(e.target.value)`.
...
Il tuo `input` PHY dovrebbe avere un `value` uguale allo stato `phy` del giocatore.
...
Il tuo `input` PHY dovrebbe avere un gestore `onChange` che chiama `setPlayer` con lo spread di `player` e `phy` impostato su `Number(e.target.value)`.
