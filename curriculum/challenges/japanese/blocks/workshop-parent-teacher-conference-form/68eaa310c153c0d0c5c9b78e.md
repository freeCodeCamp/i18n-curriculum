...
ユーザーが追加のメモや懸念事項を入力できるようにするのは良い習慣です。

3つ目の `fieldset` 要素の下に、新しい `fieldset` 要素を追加してください。その中に、テキスト `legend` を持つ `Additional Notes` 要素を追加してください。


# --hints--

このページには合計で4つの `fieldset` 要素があるはずです。
...
3つ目の `fieldset` 要素の下に `fieldset` 要素を追加してください。
...
新しい `fieldset` 要素の中には `legend` 要素が含まれている必要があります。
...
`legend` 要素にはテキスト `Additional Notes` があるべきです。
