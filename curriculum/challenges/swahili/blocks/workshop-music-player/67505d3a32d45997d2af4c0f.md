...
Tengeneza kitendakazi kinachoitwa `playPreviousSong`. Ndani ya kitendakazi chako kipya, tumia thamani inayorejeshwa na kitendakazi cha `getPreviousSong` kutafuta wimbo wa kuchezwa. Wakati `getPreviousSong()` ni `undefined`, cheza wimbo wa kwanza katika orodha yako ya nyimbo. Vinginevyo, cheza wimbo uliotangulia wimbo unaochezwa sasa.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `playPreviousSong`.

```js
assert.isFunction(playPreviousSong);
```

Kitendakazi cha `playPreviousSong` kinapaswa kucheza wimbo uliotangulia wimbo unaochezwa sasa katika orodha ya nyimbo.
...
Wakati `getPreviousSong()` ni `undefined`, kitendakazi cha `playPreviousSong` kinapaswa kucheza wimbo wa kwanza katika orodha ya nyimbo.
