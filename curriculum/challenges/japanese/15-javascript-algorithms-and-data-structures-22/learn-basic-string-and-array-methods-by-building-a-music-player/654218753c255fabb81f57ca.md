...
では、ついに UI に曲を表示するために、`userData?.songs` を渡して `renderSongs` 関数を呼び出します。

オプショナルチェーン (`?.`) は、null または undefined の可能性があるネストされたプロパティにアクセスする際のエラーを防ぐのに役立ちます。 例えば、次のように使います。
...
では、`userData` の `songs` プロパティを渡して `renderSongs` 関数を呼び出してください。 これで、プレイリストに曲が表示されます。

# --hints--

`renderSongs` 関数に `userData?.songs` を渡して呼び出してください。
