...
Für diesen Algorithmus sollten Sie eine `while`-Schleife verwenden. Diese Schleife benötigt zwei Bedingungen:

- Erstens sollte sie nicht über den Anfang des Arrays hinauslaufen (aufgerufen mit `j`).
- Zweitens sollte die Schleife nicht weiterlaufen, nachdem sie einen Wert gefunden hat, der kleiner als der aktuelle Wert ist.

Um eine Endlosschleife zu verhindern, verringern Sie `j` innerhalb Ihrer Schleife.
...
Ihre `while`-Schleife sollte als erste Bedingung prüfen, ob der Wert von `j` größer oder gleich `0` ist.
...
Ihre `while`-Schleife sollte den UND-Operator verwenden.
...
Ihre `while`-Schleife sollte eine Bedingung haben, die prüft, ob der Wert von `array[j]` größer als `currValue` ist.
...
Ihre `while`-Schleife sollte `j` innerhalb der Schleife verringern.
