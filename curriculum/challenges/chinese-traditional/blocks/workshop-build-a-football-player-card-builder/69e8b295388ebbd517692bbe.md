...
`PlayerCard` 組件可以運作，但使用了硬編碼的值。為了讓它更具彈性，你需要使用 TypeScript 介面來描述玩家資料的結構。

在 `PlayerCard` 函式之前，創建一個名為 `PlayerData` 的介面，該介面包含一個型別為 `name` 的 `string`，一個型別為 `overallRating` 的 `number`，一個型別為 `position` 的 `string`，以及一個型別為 `club` 的 `string`。

# --hints--

你應該創建一個名為 `PlayerData` 的介面。
...
你的 `PlayerData` 介面應該有一個型別為 `name` 的 `string`。
...
你的 `PlayerData` 介面應該有一個型別為 `overallRating` 的 `number`。
...
你的 `PlayerData` 介面應該有一個型別為 `position` 的 `string`。
...
你的 `playerData` 介面應該有一個型別為 `club` 的 `string`。
