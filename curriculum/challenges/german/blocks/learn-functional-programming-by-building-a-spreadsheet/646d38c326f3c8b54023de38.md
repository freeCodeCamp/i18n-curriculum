...
`addCharacters` gibt eine Funktion zurück, daher sollten Sie sie aufrufen. Übergeben Sie `char1` als Argument.

# --hints--

Sie sollten Ihre `addCharacters()`-Funktion in Ihrer `.map()`-Methode aufrufen.
...
Sie sollten `char1` als Argument an Ihren `addCharacters()`-Aufruf übergeben.
