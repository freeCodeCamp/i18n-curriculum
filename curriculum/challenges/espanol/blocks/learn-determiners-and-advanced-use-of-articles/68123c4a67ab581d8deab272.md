...
This determiner is used to talk about every single person individually. La primer letra se escribe en mayúscula.
...
This word means pieces of work that need to be completed. Usa la forma plural.

# --explanation--

`Each` is used to talk about every single member of a group separately. Por ejemplo:

`Each team member has a different role.` - Every person in the team does something different.

`Their` is a possessive determiner that matches plural meaning without showing gender (his/her). Por ejemplo:

`They updated their software yesterday.` - The software that belongs to them was updated.

`Tasks` are specific jobs or pieces of work assigned to someone. Por ejemplo:
