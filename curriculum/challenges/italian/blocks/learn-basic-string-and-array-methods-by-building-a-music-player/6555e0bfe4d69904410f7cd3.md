...
Usare `const` e la sintassi a freccia per creare una funzione vuota chiamata `shuffle`.

Questa funzione si occupa di mescolare le canzoni nella playlist e di eseguire gli aggiornamenti necessari allo stato dopo il mescolamento.

# --hints--

Dovresti usare `const` per creare una funzione vuota chiamata `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

`shuffle` dovrebbe essere una funzione.

```js
assert.isFunction(shuffle)
```

La tua funzione `shuffle` dovrebbe usare la sintassi a freccia e non dovrebbe prendere parametri.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

La tua funzione `shuffle` dovrebbe essere vuota.
