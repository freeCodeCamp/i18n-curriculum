...
CSV (значення, розділені комами) — це формат, який використовують для електронних таблиць і імпорту даних. Кожен рядок представляє один запис із полями, розділеними комами.

Почніть із видалення виклику `console.log(exportToJSON(...))`, оскільки він був потрібен лише для тестування. Потім створіть функцію з назвою `exportToCSV`, яка прийматиме `catalog` як параметр. Всередині оголосіть `const header = "Title,Author,Year,Location"` і `const rows = []`.

# --hints--

Ви повинні видалити виклик `exportToJSON` console.log.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

У вас має бути функція з назвою `exportToCSV` з параметром `catalog`.
...
Ви повинні оголосити змінну з назвою `header` зі значенням `"Title,Author,Year,Location"`.

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

Ви повинні оголосити змінну з назвою `rows`, яка дорівнює порожньому масиву.
