...
가장 안쪽에 있는 커리된 함수가 값을 변환하는 논리를 담고 있습니다.

먼저 `fromUnit` 인수를 사용해 `conversionTable` 객체에 접근한 다음, `toUnit` 인수를 사용해 중첩된 객체에 접근하세요.

그 변환 값을 `quantity` 값과 곱한 결과를 반환하세요.

# --hints--

함수가 `convertQuantity("gram")("cup")(1)`에 대해 올바른 변환 값을 반환해야 합니다.

```js
assert.strictEqual(convertQuantity("gram")("cup")(1), 1/240)
```

함수가 `convertQuantity("teaspoon")("ounce")(6)`에 대해 올바른 변환 값을 반환해야 합니다.
