...
القيم النهائية الثابتة هي إحصائيات اللاعب الستة. استبدل أول ثلاث مجموعات من الأرقام الثابتة بخاصية `player` المقابلة:
...
يجب استبدال القيمة `97` لـ `PAC` بـ `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

يجب استبدال القيمة `98` لـ `SHO` بـ `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

يجب استبدال القيمة `83` لـ `PAS` بـ `player.pas`.
