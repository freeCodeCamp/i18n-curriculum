...
title: بناء تطبيق اختبار بطاقات الفلاش
challengeType: 25
...
في هذا المختبر، ستنشئ تطبيقًا يعرض ويخزن بيانات البطاقات التعليمية التي يمكن
استرجاعها لاحقًا.
...
1) يجب أن يكون لديك عنصر HTML بمعرف `flashcard`.
2) يجب أن يكون لديك واجهة تسمى `FlashCard`.
3) يجب أن تحتوي واجهة `FlashCard` على الخاصيتين `questionText` و `questionAnswer` وكلاهما من نوع `string`.
4) يجب أن يكون لديك مجموعة من عناصر `FlashCard` تسمى `currentCards`.
5) عند النقر على العنصر `#flashcard`، يجب أن تحصل البطاقة على الصنف `flipped`.
6) يجب أن يكون لديك عنصر بمعرف `delete-btn`.
7) عند النقر على العنصر `#delete-btn`، يجب أن يُحذف البطاقة الحالية ويُعرض بيانات البطاقة السابقة.
8) يجب إنشاء نموذج إدخال بمعرف `entry-form` لتتمكن من إضافة المزيد من بطاقات الفلاش إلى مجموعة `currentCards` عند الإرسال.
9) يجب أن يكون للعنصرين داخل النموذج معرفا `front-text` و `back-text` على التوالي.
10) يجب إنشاء واستدعاء `InvalidUserInputError` عندما يكون نص السؤال أو جواب السؤال فارغًا في نموذج الإدخال.

# --hints--

يجب أن يكون لديك عنصر HTML بمعرف `flashcard`.
...
يجب أن يكون لديك واجهة `FlashCard`.
...
يجب أن تحتوي واجهة `FlashCard` على خاصية `questionText` من نوع `string`.
...
يجب أن تحتوي واجهة `FlashCard` على خاصية `questionAnswer` من نوع `string`.
...
يجب أن يكون لديك مجموعة من عناصر `FlashCard` تسمى `currentCards` من النوع `FlashCard[]`.
...
عند النقر لأول مرة على العنصر `#flashcard`، يجب أن يحصل العنصر على الصنف `flipped`.
...
يجب أن يكون لديك عنصر بمعرف `delete-btn`.
...
عند النقر على `delete-btn`، يجب حذف عنصر بطاقة فلاش من مجموعة `currentCards`.
...
عند النقر على `delete-btn`، يجب عرض بيانات بطاقة الفلاش السابقة.
...
يجب إنشاء نموذج إدخال بمعرف `entry-form`.
...
يجب أن يحتوي النموذج على عنصرين `textarea` بمعرفي `front-text` و `back-text` على التوالي.
...
يجب رمي `InvalidUserInputError` عندما يكون نص السؤال أو جواب السؤال فارغًا في نموذج الإدخال.
