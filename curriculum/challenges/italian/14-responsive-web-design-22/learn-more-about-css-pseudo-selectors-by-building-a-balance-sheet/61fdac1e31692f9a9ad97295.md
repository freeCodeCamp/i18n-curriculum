...
Il selettore `span[class~="sr-only"]` dovrebbe avere una proprietà `clip` con il valore `rect(1px, 1px, 1px, 1px)`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('clip') === 'rect(1px, 1px, 1px, 1px)');
```

Il selettore `span[class~="sr-only"]` dovrebbe avere una proprietà `clip-path` con il valore `inset(50%)`.
