...
新しい`getVariance`関数を`calculate`関数に追加し、対応するHTML要素を更新してください。

# --hints--

`calculate`関数には、`variance`の値に設定された`getVariance(numbers)`変数が必要です。

```js
assert.match(calculate.toString(), /variance\s*=\s*getVariance\(\s*numbers\s*\)/);
```

`textContent`要素の`#variance`を`variance`変数の値に設定してください。
