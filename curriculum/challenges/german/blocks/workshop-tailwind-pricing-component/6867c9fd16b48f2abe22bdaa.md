...
Das nächste Element, das gestaltet werden soll, ist der Preistext, einschließlich des `span`. Geben Sie dem `p`-Element des Preises die Klassen `text-4xl`, `font-bold` und `text-gray-900`.

# --hints--

Ihr `p`-Element sollte die Klasse `text-4xl` haben.
...
Ihr `p`-Element sollte die Klasse `font-bold` haben.
...
Ihr `p`-Element sollte die Klasse `text-gray-900` haben.
