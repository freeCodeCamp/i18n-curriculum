...
Die Wahrsager-App unterstützt das Rendern mehrerer Karten, daher benötigen Sie eine Methode, die beim Rendern mehrerer Karten hilft. Deklarieren Sie eine Methode namens `multipleCardSelected`.

Rufen Sie innerhalb der Methode die Funktion `hideElements` auf und übergeben Sie die Props `singleCard`, `singleCardBtn`, `multipleCardsBtn` und `headerTitle`. Vergessen Sie nicht, dass Sie auf diese über `this.elements` zugreifen müssen.

# --hints--

Sie sollten eine `multipleCardSelected`-Methode deklarieren.
...
Sie sollten die Funktion `hideElements` innerhalb der Methode `multipleCardSelected` aufrufen.
...
Sie sollten `singleCard` als erstes Argument an die Funktion `hideElements` übergeben.
...
Sie sollten `singleCardBtn` als Argument an die Funktion `hideElements` übergeben.
...
Sie sollten `multipleCardsBtn` als Argument an die Funktion `hideElements` übergeben.
...
Sie sollten `headerTitle` als Argument an die Funktion `hideElements` übergeben.
