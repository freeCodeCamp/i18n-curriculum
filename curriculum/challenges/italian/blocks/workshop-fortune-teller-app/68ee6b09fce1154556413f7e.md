...
All'interno del metodo `initializeEventListeners`, allega un listener per l'evento `click` a `singleCardBtn`. Nel gestore dell'evento, chiama il metodo `singleCardSelected`. Ricorda di prendere `singleCardBtn` da `elements`.

Creerai questo metodo nei passaggi successivi.

# --hints--

Dovresti allegare un evento `click` all'elemento `singleCardBtn`. Non dimenticare che qui ti serve la parola chiave `this`.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

All'interno della callback dell'evento, dovresti chiamare il metodo `singleCardSelected`.
