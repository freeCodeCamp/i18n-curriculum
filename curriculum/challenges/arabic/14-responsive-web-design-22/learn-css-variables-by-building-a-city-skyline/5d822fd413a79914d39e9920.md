...
يجب عليك إعطاء `.fb2a` خاصية `border-bottom` من `10vh solid var(--building-color3)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderBottom.trim(), "10vh solid var(--building-color3)");
```

يجب عليك إعطاء `.fb2a` خاصية `border-left` بقيمة `1vw solid transparent`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderLeft, "1vw solid transparent");
```

يجب عليك إعطاء `.fb2a` خاصية `border-right` بقيمة `1vw solid transparent`.
