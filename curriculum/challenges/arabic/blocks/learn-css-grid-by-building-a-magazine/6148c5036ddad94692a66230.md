...
الآن ستبدأ خوارزمية التمركز التلقائي بالعمل عند إضافة عنصر أيقونة جديد. مع ذلك، تُعيّن الخوارزمية عرض العمود الجديد افتراضيًا إلى `auto`، وهذا لن يتطابق مع أعمدتك الحالية.

يمكنك تجاوز ذلك باستخدام خاصية `grid-auto-columns`. أضف إلى المحدِّد `.social-icons` خاصية `grid-auto-columns` بقيمة `1fr`.

# --hints--

يجب أن يحتوي المحدِّد `.social-icons` على خاصية `grid-auto-columns` بقيمة `1fr`.
