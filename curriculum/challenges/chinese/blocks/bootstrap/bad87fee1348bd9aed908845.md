...
你可以通过在你的提交 `button` 元素内添加 `<i class="fa fa-paper-plane"></i>` 来添加 `fa-paper-plane` Font Awesome 图标。
...
所有带有类 `.form-control` 的文本 `<input>`、`<textarea>` 和 `<select>` 元素宽度为 100%。
...
你应该在你的提交 `button` 元素内添加一个 `<i class="fa fa-paper-plane"></i>`。
