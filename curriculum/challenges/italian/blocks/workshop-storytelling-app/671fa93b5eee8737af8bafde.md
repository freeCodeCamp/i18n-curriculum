...
Crea una funzione chiamata `displayStory`. All'interno della funzione, stampa `"You clicked the button"` nella console.

# --hints--

Dovresti avere una funzione chiamata `displayStory`.

```js
assert.isFunction(displayStory);
```

Dovresti stampare `"You clicked the button"` nella console all'interno della funzione `displayStory`.
