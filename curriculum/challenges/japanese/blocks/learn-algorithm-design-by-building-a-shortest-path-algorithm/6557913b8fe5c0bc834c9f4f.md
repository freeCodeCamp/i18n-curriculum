...
`.extend()` メソッドは、イテラブルから要素をリストの末尾に追加できます。
...
`else` 節を作成し、`.extend()` メソッドを使って現在のノードパスを隣接ノードパスに追加してください。

# --hints--

ネストされた `else` 文の後に `if` 節を作成してください。
...
`paths[node].extend(paths[current])` 節に `else` があるはずです。
