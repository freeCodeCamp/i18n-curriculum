...
Agregaste una llamada a `console.log()` en tu script para validar tu arreglo `EVAReadySquad`, y ahora la llamada ya no es necesaria. Para evitar que el terminal se llene en pasos futuros, adelante y elimina el `for` actual que registra en la consola a los astronautas de `EVAReadySquad`.

# --hints--

Elimina el `for` loop que registra en la consola a los astronautas de `EVAReadySquad`. Debes borrar todo el bloque del loop.
