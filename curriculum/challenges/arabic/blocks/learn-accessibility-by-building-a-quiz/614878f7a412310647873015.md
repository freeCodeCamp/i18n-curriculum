...
نظّم الـ `header` باستخدام _Flexbox_ لوضع مسافة بين العناصر الفرعية ومحاذاتها عموديًا في الوسط.

# --hints--

يجب أن تعطي الـ `header` خاصية `justify-content` بقيمة `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

يجب أن تعطي الـ `header` خاصية `align-items` بقيمة `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

يجب أن تعطي الـ `header` خاصية `top` بقيمة `0`.
