...
このワークショップでは、TypeScriptを使った練習を続けて、占いアプリを作成します。

まず、`Card`という名前のインターフェースを宣言してください。`Card`インターフェースの中に、次のプロパティを追加します：`name`、`name_short`、`value`。`name`と`name_short`のプロパティは`string`型に設定し、`value`は`string | number`のユニオン型に設定してください。

# --hints--

`Card`という名前のインターフェースを宣言してください。
...
`Card`インターフェースには、`name`プロパティがあり、その型は`string`である必要があります。
...
`Card`インターフェースには、`name_short`プロパティがあり、その型は`string`である必要があります。
...
`Card`インターフェースには、`value`プロパティがあり、その型は`string`と`number`のユニオン型である必要があります。
