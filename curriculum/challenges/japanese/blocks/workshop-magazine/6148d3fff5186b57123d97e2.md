...
引用は適切な引用符がなければ本当の引用とは言えません。CSSの疑似セレクターでこれらを追加できます。

`.quote::before` セレクターを作成し、`content` プロパティを `"` に設定して、その後にスペースを入れてください。

また、`.quote::after` セレクターを作成し、`content` プロパティを `"` に設定して、その前にスペースを入れてください。

# --hints--

`.quote::before` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

`.quote::before` セレクターには `content` プロパティが `'" '` に設定されているはずです。

```js
assert.match(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content, /\"\\"\s\"/);
```

`.quote::after` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

`.quote::after` セレクターには `content` プロパティが `' "'` に設定されているはずです。
