...
為你的專案邏輯做準備，使用 `const` 和箭頭語法宣告一個 `sortInputArray` 函式（程式）。它應該接受一個單一的 `event` 參數。

# --hints--

你應該使用 `const` 宣告一個 `sortInputArray` 函式（程式）。

```js
assert.match(code, /const\s+sortInputArray\s*=/);
```

你的 `sortInputArray` 應該是一個函式（程式）。

```js
assert.isFunction(sortInputArray);
```

你的 `sortInputArray` 應該接受一個單一的 `event` 參數。

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)/);
```

你的 `sortInputArray` 應該使用箭頭語法。

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)\s*=>/);
```

你的 `sortInputArray` 函式應該是空的。
