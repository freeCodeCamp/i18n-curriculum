...
Utahitaji kuwa na safu moja ya maandishi na safu moja ya picha. Toa sifa ya `.text-with-images` kwa kichaguzi cha `grid-template-columns` iwe `1fr 2fr`. Pia weka sifa ya `column-gap` kuwa `3rem` ili kutoa nafasi zaidi kati ya safu hizo.

# --hints--

Kichaguzi chako cha `.text-with-images` kinapaswa kuwa na sifa ya `grid-template-columns` iliyowekwa kuwa `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

Kichaguzi chako cha `.text-with-images` kinapaswa kuwa na sifa ya `column-gap` iliyowekwa kuwa `3rem`.
