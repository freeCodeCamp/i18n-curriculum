...
`infixToFunction[operator]` 傳回一個函式。直接呼叫該函式，並將 `arg1` 和 `arg2` 作為引數傳入。

# --hints--

你的回呼函式（程式）應該傳回呼叫 `infixToFunction[operator]` 的結果。
...
你應該將 `arg1` 作為第一個引數傳遞給你的 `infixToFunction[operator]` 呼叫。
...
你應該將 `arg2` 作為第二個引數傳遞給你的 `infixToFunction[operator]` 呼叫。
