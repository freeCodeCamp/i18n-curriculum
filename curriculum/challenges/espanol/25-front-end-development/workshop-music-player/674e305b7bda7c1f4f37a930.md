...
Antes de comenzar a trabajar en reproducir la canción siguiente y anterior, debe obtener el índice de cada canción en la propiedad `songs` de `userData`. Para hacer eso, puede usar el método de array `indexOf()`, que devuelve el primer índice en el que un elemento dado puede ser encontrado en el array, o `-1` si el elemento no está presente.
