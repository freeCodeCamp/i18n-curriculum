...
在 `.penguin` 元素上面，添加一个 `div`，其 `class` 为 `left-mountain`。

# --hints--

你应该在 `body` 中添加一个新的 `div`。 应该是 `--fcc-expected--` `div` 元素，而不是 `--fcc-actual--`。

```js
assert.equal(document.querySelectorAll('body > div')?.length, 3);
```

你应该将 `div` 的 `class` 设置为 `left-mountain`。

```js
assert.include(document.querySelector('body > div:not(.ground, .penguin)')?.className, 'left-mountain');
```

你应该将 `.left-mountain` 放在 `.penguin` 前面。
