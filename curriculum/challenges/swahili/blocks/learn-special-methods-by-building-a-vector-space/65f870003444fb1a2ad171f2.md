...
Badilisha wito mbili za `print` ili uchapishe norm ya vekta zako na uhakikishe kuwa njia ya `norm()` inafanya kazi vizuri.

# --hints--

Unapaswa kuchapisha `v1.norm()`.

```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(v1.norm())")`)) })
```

Unapaswa kuchapisha `v2.norm()`.
