...
세 번째와 네 번째 웨이포인트에 대해 `transform` 패턴을 한 번 더 반복하세요.

# --hints--

`30%` 웨이포인트에 `transform`의 `rotate(110deg) scaleX(-1)`를 지정하세요.
...
`40%` 웨이포인트에 `transform`의 `rotate(130deg) scaleX(-1)`를 지정하세요.
