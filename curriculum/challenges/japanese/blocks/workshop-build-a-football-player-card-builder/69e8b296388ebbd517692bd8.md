...
Overall `value` に `player.overallRating` の `input` 属性を追加してください。その後、`onChange` をパラメータに取り、`e` のスプレッドと `setPlayer` を `player` に設定して `overallRating` を呼び出すアロー関数を設定した `Number(e.target.value)` 属性を追加してください。

`overallRating` は数値なので、入力からの文字列の値を変換するために `Number()` を使ってください。

# --hints--

Overall `input` は現在のプレイヤーの総合評価と等しい `value` を持つ必要があります。
...
Overall `input` は `onChange` 属性を持つ必要があります。

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Overall `onChange` の `input` は `setPlayer` に設定された `player` と `overallRating` のスプレッドを使って `Number(e.target.value)` を呼び出す必要があります。
