...
Da Sie jetzt einen regulären Ausdruck haben, der Multiplikation oder Division erkennt, können Sie diesen Ausdruck auswerten.

Deklarieren Sie eine `str2`-Variable und weisen Sie ihr das Ergebnis zu, das durch den Aufruf von `infixEval` mit `str` und `regex` als Argumente entsteht.

# --hints--

Sie sollten eine `str2`-Variable deklarieren.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*(?:const|let|var)\s+str2/);
```

Sie sollten `const` verwenden, um Ihre `str2`-Variable zu deklarieren.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2/);
```

Sie sollten `str2` das Ergebnis zuweisen, das durch den Aufruf Ihrer `infixEval`-Funktion entsteht.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(/);
```

Sie sollten `str` als erstes Argument an Ihren `infixEval`-Aufruf übergeben.
...
Sie sollten `regex` als zweites Argument an Ihren `infixEval`-Aufruf übergeben.
