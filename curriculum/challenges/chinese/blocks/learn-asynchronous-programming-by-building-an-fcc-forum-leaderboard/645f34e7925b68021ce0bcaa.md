...
下一步是在 `users` 数组中找到正确的用户。

首先创建一个名为 `user` 的常量，并将其赋值给 `users.find()`。 `find` 方法应该有一个参数为 `user` 的回调函数。 

在 `find` 方法的回调函数中，隐式返回检查 `user.id` 是否严格等于 `poster.user_id` 的结果。
