...
Torna alla tua `showLatestPosts()` funzione e usa la destrutturazione per ottenere le proprietà `topic_list` e `users` dall’oggetto `data`.

# --hints--

Dovresti usare `const` per destrutturare `topic_list` e `users` dall’oggetto `data`.
