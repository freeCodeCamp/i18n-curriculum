...
最後に、ユーザーアバターの画像を返す必要があります。

まず、`return` を追加し、その後にテンプレートリテラルのセットを追加してください。テンプレートリテラルの中に `img` 要素を追加します。

`img` タグの中に、値が `src` の `${userAvatarUrl}` 属性を追加してください。`alt` 属性には、値として `${user.name}` を追加します。
...
`src` 属性の値を `userAvatarUrl` という定数に設定してください。
...
`alt` 属性の値を `user.name` という定数に設定してください。
