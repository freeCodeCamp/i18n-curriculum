...
بعد ذلك، ستُنشئ `userAvatarUrl`.

ابدأ بإنشاء ثابت يسمى `userAvatarUrl`. ثم استخدم معامل ثلاثي للتحقق مما إذا كان `avatar` يبدأ بـ `"/user_avatar/"`.

إذا كان الأمر كذلك، استخدم الطريقة `concat` لربط `avatar` بـ `avatarUrl`. وإلا فأعد `avatar`.

هذا يضمن تكوين عنوان URL للصورة الرمزية بشكل صحيح سواء كان نسبيًا أو مطلقًا.

# --hints--

يجب أن يكون لديك ثابت باسم `userAvatarUrl`.

```js
assert(code.match(/const\s+userAvatarUrl\s*=/));
```

يجب أن يتحقق الـ `ternary` الخاص بك مما إذا كان `avatar` يبدأ بـ `"/user_avatar/"` ويُرجع `avatarUrl.concat(avatar)` أو `avatar`.
