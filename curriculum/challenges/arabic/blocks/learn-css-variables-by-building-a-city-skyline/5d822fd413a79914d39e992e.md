...
املأ النوافذ باللون الثانوي لهذا المبنى. أضف أيضًا `margin` بقيمة `10%` لإعطاء النوافذ بعض الفراغ.

# --hints--

يجب أن تعطي `.fb4-window` خاصية `background-color` بقيمة `--window-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.backgroundColor.trim(), "var(--window-color1)");
```

يجب أن تعطي `.fb4-window` هامشًا (`margin`) بقيمة `10%`.
