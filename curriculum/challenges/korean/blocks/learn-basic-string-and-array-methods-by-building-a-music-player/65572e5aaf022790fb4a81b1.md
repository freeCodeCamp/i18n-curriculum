...
이제 `resetButton`에 리셋 기능을 추가할 차례입니다. 이 기능은 클릭하면 재생 목록의 노래들을 다시 불러옵니다.

`resetButton` 변수에 클릭 이벤트 리스너를 추가하세요. 화살표 구문을 사용해 콜백을 전달하고, 일단 비워 두세요.

# --hints--

`addEventListener()`에서 `resetButton` 메서드를 사용해야 합니다.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

`resetButton` 이벤트 리스너는 `"click"` 이벤트를 들어야 합니다.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

이벤트 리스너의 콜백 함수는 화살표 구문을 사용하고 중괄호를 빈 상태로 두세요.
