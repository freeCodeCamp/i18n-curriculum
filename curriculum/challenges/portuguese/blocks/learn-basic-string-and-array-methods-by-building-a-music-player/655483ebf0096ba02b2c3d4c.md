...
Para obter a música anterior, subtraia `1` do `currentSongIndex` de `userData?.songs` e atribua-o à constante `previousSong`.Depois disso, chame a função `playSong` e passe `previousSong.id` como argumento.
...
Você deve atribuir `userData?.songs[currentSongIndex - 1]` a uma constante `previousSong`.
...
Você deve chamar a função `playSong` com `previousSong.id`.
