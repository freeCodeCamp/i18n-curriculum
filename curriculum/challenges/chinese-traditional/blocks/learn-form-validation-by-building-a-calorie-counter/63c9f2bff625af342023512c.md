...
為完成這個專案，為 `clearButton` 按鈕添加一個事件監聽器。當按鈕被點擊時，應該呼叫 `clearForm` 函式（程式）。

恭喜！你的專案已完成。

# --hints--

你應該為你的 `clearButton` 按鈕添加一個事件監聽器。

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(/);
```

你應該將 `"click"` 作為第一個引數傳遞給 `.addEventListener()`。

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

你應該將 `clearForm` 作為第二個引數傳遞給 `.addEventListener()`。
