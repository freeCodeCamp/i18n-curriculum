...
title: Побудувати конвертер валют
challengeType: 25
...
1. Ваш компонент `CurrencyConverter` має відтворювати елемент `input` для введення суми, яку потрібно конвертувати.
2. Ваш елемент `input` має приймати числа.
3. Ваш компонент `CurrencyConverter` має відтворювати два елементи `select` для вибору валюти, з якої конвертують, і в яку конвертують.
4. Ваш елемент `select` має містити опції принаймні для `USD`, `EUR`, `GBP` та `JPY`. Ви можете використовувати будь-який обмінний курс, за умови, що між валютами немає однозначного співвідношення. Ось кілька прикладів хороших і поганих співвідношень:
...
5. Ваш компонент `CurrencyConverter` має мемоізувати обчислення конвертованих сум для валюти **з**, так щоб зміна опції `select` валюти **в** не призводила до повторного обчислення конвертованих сум.
6. Ваш компонент `CurrencyConverter` має відтворювати елемент, який показує конвертовану суму у форматі `XX.XX CCC`, де `XX.XX` — це конвертована сума, а `CCC` — код валюти.
7. Конвертована сума має бути округлена до двох десяткових знаків.

# --hints--

Ви маєте експортувати компонент `CurrencyConverter`.
...
Ви маєте мати один елемент `input[type="number"]` для введення суми, яку потрібно конвертувати.
...
Ви маєте мати два елементи `select`.
...
Елементи `select` мають містити опції принаймні для `USD`, `EUR`, `GBP` та `JPY`.
...
Зміна значення першого елемента `select` має спричинити повторне обчислення конвертованих сум.
...
Зміна значення другого елемента `select` не має спричиняти повторне обчислення конвертованих сум.
...
Зміна значення першого елемента `select` має відобразити нову конвертовану суму.
...
Зміна значення другого елемента `select` має відобразити нову конвертовану суму та валюту.
...
Конвертована сума має відображатися у форматі `XX.XX CCC`, де `XX.XX` — це конвертована сума, округлена до двох десяткових знаків, а `CCC` — код валюти.
...
Конвертована сума має відрізнятися від вхідної суми.
