...
Jetzt sind Sie bereit, das Grid-Layout zusammenzustellen. In vorherigen Lektionen haben Sie gelernt, dass CSS Grid ein zweidimensionales, rasterbasiertes Layout bietet, mit dem Sie Elemente horizontal und vertikal zentrieren können, während Sie gleichzeitig die Kontrolle behalten, um beispielsweise Elemente übereinander zu legen.

Beginnen Sie damit, einen `main`-Selektor zu erstellen und ihm eine `display`-Eigenschaft mit dem Wert `grid` zuzuweisen.

# --hints--

Sie sollten einen `main`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Ihr `main`-Selektor sollte eine `display`-Eigenschaft mit dem Wert `grid` haben.
