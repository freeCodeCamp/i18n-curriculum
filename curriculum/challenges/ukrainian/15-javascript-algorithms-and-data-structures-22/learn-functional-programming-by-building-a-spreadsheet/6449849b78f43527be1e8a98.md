...
Ви повинні вміти зіставляти діапазони клітинок у формулі. Діапазони клітинок можуть мати такий вигляд: `A1:B12` або `A3:A25`. Ви можете використати регулярний вираз, щоб узгодити ці шаблони.

Почніть з оголошення змінної `rangeRegex` та призначте до неї регулярний вираз, який відповідає `A` до `J` (діапазон стовпців у електронній таблиці). Щоб це досягти використайте групу захоплення з символьним класом.

# --hints--

Оголосіть змінну `rangeRegex` після функції `idToText`.
...
Використайте `const`, щоб оголосити змінну `rangeRegex`.
...
Змінна `rangeRegex` має бути регулярним виразом.
...
Ваш `rangeRegex` повинен використовувати групу захоплення.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(.*\)\/\s*;?/);
```

`rangeRegex` має використовувати символьний клас у групі захоплення.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[.*\]\)\/\s*;?/);
```

`rangeRegex` має використовувати символьний клас для відповідності від `A` до `J`.
