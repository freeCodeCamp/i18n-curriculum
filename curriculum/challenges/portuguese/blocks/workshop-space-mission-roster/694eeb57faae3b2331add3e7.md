...
De volta dentro da função `getEVAReadyCrew`, faça a chamada de `sortByPriorityDescending()` com o array local `eligible` para que ele seja ordenado antes de ser retornado.

# --hints--

Você deve fazer uma chamada para `sortByPriorityDescending()` com o `array` local `eligible`.
