...
`forEach` 콜백에서 오른쪽 대입 `totalCountPerProduct[dessert.id] || 0`을 올바르게 평가하도록 소괄호 `()`로 감싼 다음, 값을 1만큼 증가시키세요.
...
`totalCountPerProduct`의 `dessert.id`에 해당하는 속성 값을 업데이트하려면 대입 연산자를 사용하세요.
...
값을 1만큼 증가시키기 전에 `totalCountPerProduct[dessert.id] || 0`을 소괄호 `()`로 감싸세요.
