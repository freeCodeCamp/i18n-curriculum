...
我們不希望玩家唯一的武器壞掉。邏輯 AND 運算子會檢查兩個述語是否為真。

使用 <dfn>邏輯 AND</dfn> 運算子 `&&` 為你的 `if` 述語添加第二個條件。只有當 `inventory.length` 不相等（`!==`）於一時，玩家的武器才會壞掉。
...
你應該使用邏輯 AND 運算子來檢查 `inventory.length` 是否不相等於 `1`。
