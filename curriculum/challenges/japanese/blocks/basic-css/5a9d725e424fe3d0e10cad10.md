...
title: 複数の要素を一度に変更するために CSS 変数を使う
challengeType: 0
...
<dfn>CSS 変数</dfn>は、1つの値を変えるだけで多くの CSS スタイルプロパティを一度に変更できる強力な方法です。

以下の指示に従って、3つの値を変えるだけで多くの要素のスタイルが変わる様子を確認してください。

# --instructions--

`penguin` クラスで、`black` の値を `gray` に、`gray` の値を `white` に、`yellow` の値を `orange` に変更してください。

# --hints--

`penguin` クラスは `--penguin-skin` 変数を宣言し、`gray` を代入する必要があります。

```js
assert.match(__helpers.removeHtmlComments(code), /.penguin\s*?{[\s\S]*--penguin-skin\s*?:\s*?gray\s*?;[\s\S]*}/gi);
```

`penguin` クラスは `--penguin-belly` 変数を宣言し、`white` を代入する必要があります。

```js
assert.match(code, /.penguin\s*?{[\s\S]*--penguin-belly\s*?:\s*?white\s*?;[\s\S]*}/gi);
```

`penguin` クラスは `--penguin-beak` 変数を宣言し、`orange` を代入する必要があります。
