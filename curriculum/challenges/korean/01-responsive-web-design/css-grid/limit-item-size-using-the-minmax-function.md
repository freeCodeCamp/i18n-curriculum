...
`grid-template-columns`와 `grid-template-rows`와 함께 사용할 수 있는 `minmax`라는 내장 함수가 있습니다. 그리드 컨테이너 크기가 변경될 때 아이템의 크기를 제한하는데 사용됩니다. 이를 위해서는 아이템에 대한 허용 가능한 크기 범위를 지정해야 합니다. 여기 예시가 있습니다.
