...
لجعل القسم الأول يبدو أكثر مضمَّنًا، استهدف فقط عناصر `input` داخل عناصر `.info`، واضبط `width` الخاص بها إلى `50%`، ووجّه النص إلى اليسار.

# --hints--

يجب أن تستخدم محدِّد `.info input` أو `.info > input`.
...
يجب أن تعطي عناصر `input` قيمة `width` تساوي `50%`.
...
يجب أن تعطي عناصر `input` خاصية `text-align` بقيمة `left`.
