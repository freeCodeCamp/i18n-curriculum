...
これでグリッドレイアウトを組み立て始める準備ができました。前のレッスンでは、CSS Gridが二次元のグリッドベースのレイアウトを提供し、アイテムを水平および垂直に中央揃えにしながら、要素の重なりなども制御できることを学びました。

まず`main`セレクターを作成し、`display`プロパティに`grid`を設定してください。
...
`main`セレクターには`display`プロパティを`grid`に設定してください。
