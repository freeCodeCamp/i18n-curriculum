...
재생 목록의 노래를 클릭하면 그 노래를 재생하고 싶을 것입니다. 이를 위해서는 `click` 요소의 자식인 각 `button` 요소에 `song` 이벤트에 대한 이벤트 리스너를 추가해야 합니다.

`forEach`의 콜백을 완성하여 그렇게 하세요. `addEventListener` 콜백 안에서 `playSong(n)`를 호출하는데, 여기서 `n`는 `id`의 `song` 속성에서 얻을 수 있는 노래 ID입니다.

각 `song`에는 `id` 값인 `song-n` 속성이 있다는 점을 주의하세요. `n`에 전달하기 전에 `playSong()` 부분을 추출하여 숫자로 변환해야 합니다.


# --hints--

`click` 요소의 자식인 각 `button` 요소에 `.playlist-song` 이벤트에 대한 이벤트 리스너를 추가하고, 해당 노래의 `playSong`를 사용해 `id`를 호출해야 합니다.
