...
注意線條的粗細看起來變大了嗎？名為 `border-width` 的屬性的預設值是 `1px`，適用於所有 `hr` 元素的邊緣。透過將邊框改為與背景相同的顏色，線條的總高度為 `5px`（`3px` 加上上下邊框各 `1px`）。

將 `hr` 的 `height` 屬性更改為 `2px`，使其總高度變為 `4px`。

# --hints--

你應該將 `height` 屬性設為 `2px`。
