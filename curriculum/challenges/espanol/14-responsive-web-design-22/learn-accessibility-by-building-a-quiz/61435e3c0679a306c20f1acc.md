...
Debes dar al primer elemento `a` un `href` de `#student-info`.

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

Debes dar al segundo elemento `a` un `href` de `#html-questions`.

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

Debes dar al tercer elemento `a` un `href` de `#css-questions`.
