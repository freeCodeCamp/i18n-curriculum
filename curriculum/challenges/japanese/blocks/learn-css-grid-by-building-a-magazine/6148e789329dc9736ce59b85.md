...
`.image-1, .image-3` ルールを作成し、`grid-column` プロパティを `1 / -1` に設定してください。これにより、1番目と3番目の画像がグリッドの全幅に広がります。

# --hints--

`.image-1, .image-3` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

`.image-1, .image-3` セレクターには、`grid-column` プロパティを `1 / -1` に設定してください。
