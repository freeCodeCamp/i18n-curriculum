...
CSS 變數可以簡化你使用媒體查詢的方式。

例如，當你的螢幕比你的媒體查詢斷點小或大時，你可以更改變數的值，並且它會在任何使用該變數的地方套用其樣式。

# --instructions--

在媒體查詢的 `:root` 選擇器中，將 `--penguin-size` 重新定義並賦值為 `200px`。同時，重新定義 `--penguin-skin` 並賦值為 `black`。然後調整預覽大小以查看此變更的效果。

# --hints--

`:root` 應該重新指定 `--penguin-size` 變數為 `200px`。
...
`:root` 應該重新指定 `--penguin-skin` 變數為 `black`。
