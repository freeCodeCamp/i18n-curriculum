...
選択ソートは、配列内の最小値のインデックスを追跡することに依存しています。変数`minIndex`を宣言し、`i`に設定してください。これにより、現在の値が最小の場合は自分自身と交換され、移動しないことが保証されます。配列を繰り返し処理する際に、`minIndex`の値を再代入できるようにする必要があります。

次に、`for`ループをもう一つ書き、`j`をイテレーターとして使います。このループは`i`の次のインデックスから始まり、配列の残りを繰り返し処理する必要があります。

# --hints--

`let`を使って`minIndex`を宣言してください。
...
`for`ループは`j`を`i + 1`に初期化する必要があります。
