...
你的雜誌將有三個主要部分。你已經在 `main` 規則中設定整體版面配置，但你可以在子規則中調整位置。

一個選項是 `grid-column` 屬性，它是 `grid-column-start` 和 `grid-column-end` 的簡寫。`grid-column` 屬性告訴網格項目從哪一條網格行開始和結束。

創建 `.heading` 規則並將 `grid-column` 屬性設定為 `2 / 3`。這會告訴 `.heading` 元素從網格第 2 行開始，到第 3 行結束。
...
你的 `.heading` 選擇器應該有一個設定為 `2 / 3` 的 `grid-column` 屬性。
