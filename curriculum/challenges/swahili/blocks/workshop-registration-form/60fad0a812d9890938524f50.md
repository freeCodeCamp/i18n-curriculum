...
Ili kuwapa Wanafunzi wazo la kile cha kuweka katika wasifu wao, sifa ya kipengele `placeholder` inatumika. `placeholder` inakubali thamani ya maandishi, ambayo inaonyeshwa hadi mtumizi aanze kuandika.

Mpe `textarea` sifa ya kipengele `placeholder` yenye thamani ya `I like coding on the beach...`.

# --hints--

Unapaswa kumpa `textarea` sifa ya kipengele `placeholder`.

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

Unapaswa kumpa `placeholder` thamani ya `I like coding on the beach...`.
