...
미디어 쿼리의 `sky` 클래스에서 두 개의 `#ffcf33` 색상 값을 `#ccc`로, `#ffff66`를 `#445`로, `#bbeeff`를 `#223`로 변경하세요. 그런 다음 창 크기를 조절하여 배경색이 바뀌는 것을 확인할 수 있습니다.

# --hints--

첫 번째 색상 값은 `#ffcf33`에서 `#ccc`로 변경해야 합니다.
...
두 번째 색상 값은 `#ffff66`에서 `#445`로 변경해야 합니다.
...
세 번째 색상 값은 `#bbeeff`에서 `#223`로 변경해야 합니다.
