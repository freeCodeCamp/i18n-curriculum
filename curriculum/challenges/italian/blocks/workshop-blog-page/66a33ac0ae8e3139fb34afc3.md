...
Ora che hai finito di aggiungere tutti i post del blog, prova a cliccare sul collegamento `Posts` e dovresti vedere che la pagina scende fino alla sezione `Posts`.

L'ultimo componente da aggiungere alla tua pagina del blog sarà la sezione contatti.

Sotto l'elemento `main`, aggiungi un elemento `footer`.

# --hints--

Il tuo elemento `footer` dovrebbe avere un tag di apertura.

```js
assert.match(code, /<footer\>/);
```

Il tuo elemento `footer` dovrebbe avere un tag di chiusura.

```js
assert.match(code, /<\/footer\>/);
```

Il tuo elemento `footer` dovrebbe essere all'interno del tuo elemento `body`.
