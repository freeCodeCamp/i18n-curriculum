...
Erstellen Sie eine `if`-Anweisung. Verwenden Sie für deren Bedingung den `not`-Operator, um `is_sequence` zu negieren. Innerhalb der `if`-Anweisung geben Sie `Invalid format: expected a list or tuple.` aus und geben `False` zurück.

# --hints--

Sie sollten eine `if`-Anweisung innerhalb Ihrer `validate`-Funktion erstellen.
...
Ihre `if`-Anweisung sollte `not is_sequence` als Bedingung haben.
...
Sie sollten `Invalid format: expected a list or tuple.` innerhalb Ihrer `if`-Anweisung ausgeben.
...
Sie sollten am Ende Ihrer `False`-Anweisung `if` zurückgeben.
