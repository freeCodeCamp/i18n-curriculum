...
创建一个 `CDN_URL` 变量并将其设置为 `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`。同时创建一个 `defaultImg` 变量，值为 `new URL("default.svg", import.meta.url)`，以及一个 `LOCAL_DEFAULT_IMG` 变量，值为 `defaultImg`。
...
你的 `defaultImg` 变量应该设置为 `new URL("default.svg", import.meta.url)`。
