...
title: Duka la pikipiki la maabara
challengeType: 25
...
Kwa maabara hii, umepewa HTML na CSS zote. Utatumia TypeScript kukamilisha kazi za ukurasa wa duka la pikipiki.
...
Unapaswa kuwa na `type` inayoitwa `Category`.
...
Aina yako ya `Category` inapaswa kuwekwa kuwa muungano wa thamani sahihi.
...
Unapaswa kuwa na `interface` inayoitwa `Motorcycle`.
...
Unapaswa kuwa na sifa zote zilizoainishwa kwenye kiolesura cha `Motorcycle`.
...
Unapaswa kuwa na kitendakazi kinachoitwa `fetchMotorcycles`.

```js
assert.isFunction(fetchMotorcycles);
```

Unapaswa kutumia kiishio cha `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
...
Unapaswa kurejesha ahadi ya vitu 25 vya `Motorcycle` kutoka `fetchMotorcycles`.
...
Unapaswa kuwa na kitendakazi kinachoitwa `renderMotorcycleCard`.

```js
assert.isFunction(renderMotorcycleCard);
```

Kitendakazi cha `renderMotorcycle` kinapaswa kupokea kigezo kilichoandikwa kwa aina ya `Motorcycle`.
...
Kitendakazi cha `renderMotorcycleCard` kinapaswa kurejesha aina ya `string` waziwazi.
...
Kitendakazi cha `renderMotorcycleCard` kinapaswa kurejesha `string`.
...
Unapaswa kuwa na darasa linaloitwa `MotorcycleGalleryApp`.
...
`MotorcycleGalleryApp` inapaswa kuwa na safu inayoitwa `allMotorcycles`.
...
Sifa ya `allMotorcycles` inapaswa kuwa `private`.
...
Sifa ya `allMotorcycles` inapaswa kuandikwa kwa aina ya `Motorcycle[]`.
...
`MotorcycleGalleryApp` inapaswa kuwa na kitendakazi kinachoitwa `renderMotorcycles`.
...
Kitendakazi cha `renderMotorcycles` kinapaswa kuonyesha vipengele 25 vyenye darasa la `motorcycle-card-image-container`.
...
Kitendakazi cha `renderMotorcycles` kinapaswa kuonyesha vipengele 25 vyenye darasa la `motorcycle-card-year-badge`.
...
Kitendakazi cha `renderMotorcycles` kinapaswa kuonyesha vipengele 25 vyenye darasa la `motorcycle-card-title`.
...
Kitendakazi cha `renderMotorcycles` kinapaswa kuonyesha vipengele 25 vyenye darasa la `motorcycle-card-manufacturer`.
...
Kitendakazi cha `renderMotorcycles` kinapaswa kuonyesha vipengele 25 vyenye darasa la `motorcycle-card-category`.
...
Kitendakazi cha `renderMotorcycles` kinapaswa kuonyesha vipengele 25 vyenye darasa la `motorcycle-card-description`.
...
Kitendakazi cha `renderMotorcycles` kinapaswa kuonyesha vipengele 25 vyenye darasa la `motorcycle-card-price`.
...
Kitendakazi cha `renderMotorcycles` kinapaswa kuonyesha vipengele 25 vyenye darasa la `motorcycle-card-engine`.
