...
Remember that the increment operator `++` can be used to increase a variable's value by `1`. There is also a <dfn>decrement operator</dfn> `--` that can be used to decrease a variable's value by `1`. Por exemplo :
