...
L'attributo `alt` dell'immagine dovrebbe descrivere il contenuto dell'immagine. I lettori di schermo annunciano il testo alternativo al posto delle immagini. Se l'immagine non può essere caricata, questo testo viene mostrato al posto dell'immagine.

Per completare il progetto, aggiungi un attributo `alt` a tutte e nove le immagini dei gatti per descriverle. Usa un valore di almeno cinque caratteri per ciascuna.


# --hints--

Tutti e nove i tuoi elementi `img` dovrebbero avere un attributo `alt` con un testo che descriva ogni immagine, con ogni descrizione che utilizzi almeno `5` lettere.
