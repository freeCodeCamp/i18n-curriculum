...
Щоб позначити прапорці, додайте атрибут `checked` до прапорця `input`. Використовуйте метод `includes` для перевірки, чи знаходиться поточне значення `power` у масиві `powers`.

`onChange` буде окремою функцією, наразі встановіть її в `handlePowersChange`. Це призведе до помилки, яку ви виправите на наступному кроці.

# --hints--

Ви повинні встановити стан прапорця `checked`, щоб відобразити, чи існує поточне значення `power` у масиві powers.

```js
assert.match(code, /checked\s*=\s*\{\s*powers\s*\.\s*includes\s*\(\s*power\s*\)\s*\}/)
```

Ви повинні встановити атрибут прапорця `onChange` у `{handlePowersChange}`.
