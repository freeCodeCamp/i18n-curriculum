...
title: 'Завдання 78: Розбиття монет'
challengeType: 1
forumTopicId: 302191
...
Нехай ${p}(n)$ представляє кількість різних способів, у які `n` монет можна розділити на купки. Наприклад, п’ять монет можна розділити на купки рівно сімома різними способами, тому ${p}(5) = 7$.
...
Знайдіть найменше значення `n`, для якого ${p}(n)$ ділиться на `divisor`.

# --hints--

`coinPartitions(7)` має повертати число.

```js
assert(typeof coinPartitions(7) === 'number');
```

`coinPartitions(7)` має повертати `5`.

```js
assert.strictEqual(coinPartitions(7), 5);
```

`coinPartitions(10000)` має повертати `599`.

```js
assert.strictEqual(coinPartitions(10000), 599);
```

`coinPartitions(100000)` має повертати `11224`.

```js
assert.strictEqual(coinPartitions(100000), 11224);
```

`coinPartitions(1000000)` має повертати `55374`.
