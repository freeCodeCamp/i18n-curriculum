...
我們稱一個 `n` 位數字為全數字 pandigital，如果它恰好使用了從 1 到 `n` 的所有數字各一次。 例如，2143 是一個 4 位全數字 pandigital，且同時是質數。

存在的最大 `n` 位數字全排列質數是什麼？

# --hints--

`pandigitalPrime(4)` 應該傳回一個數字。

```js
assert(typeof pandigitalPrime(4) === 'number');
```

`pandigitalPrime(4)` 應該傳回 4231。

```js
assert(pandigitalPrime(4) == 4231);
```

`pandigitalPrime(7)` 應該傳回 7652413。
