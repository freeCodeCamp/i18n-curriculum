...
أعلن عن متغير باسم `cardId`، وعيّن له قيمة الخاصية `data-id` من `cardElement` باستخدام الطريقة `getAttribute()`.

# --hints--

يجب إعلان متغير باسم `cardId` داخل `showFortune`.
...
يجب تعيين متغير `cardId` باستدعاء `cardElement.getAttribute("data-id")`.
...
يجب استخدام الطريقة `getAttribute` على `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

يجب تمرير `'data-id'` إلى `getAttribute`.
