...
Agora, adicione um `div` com a classe `img-loader`. Depois disso, crie um elemento `img` e dê a ele uma URL condicional para a imagem usando `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}`.

# --hints--

Você deve ter um elemento `div` dentro do seu elemento `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

Seu `div` deve ter a classe `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

Você deve ter um elemento `img` dentro do seu elemento `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

O elemento `img` deve usar `CDN_URL` e `img` para seu `src` quando `img` for fornecido, e usar `LOCAL_DEFAULT_IMG` como alternativa quando não for.
...
Seu elemento `img` deve ter as classes `card-img` e `hidden`.
