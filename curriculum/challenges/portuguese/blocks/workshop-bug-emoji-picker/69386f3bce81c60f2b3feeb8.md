...
Obtenha o elemento `select` com o id `species` do DOM e armazene-o em uma constante chamada `selectElement`. Use o operador de assertiva não nula `!` para afirmar que o elemento não é `null`

# --hints--

Você deve ter uma constante chamada `selectElement`.
...
Sua `selectElement` deve consultar o elemento com o id `species` usando `document.querySelector` com um argumento de tipo `HTMLSelectElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

Sua `selectElement` deve usar o operador de assertiva não nula (`!`).
