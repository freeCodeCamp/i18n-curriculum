...
Dai agli elementi `.fb3-window` un `width` di `25%`, un `height` di `80%` e usa la tua variabile `--window-color1` come valore `background-color`.

# --hints--

Dovresti dare a `.fb3-window` un `width` di `25%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

Dovresti dare a `.fb3-window` un `height` di `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

Dovresti dare a `.fb3-window` un `background-color` di `--window-color1`.
