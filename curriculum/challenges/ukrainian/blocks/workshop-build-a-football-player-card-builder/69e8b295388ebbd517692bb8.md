...
Під елементом `div` з `className` `card-tier-badge` додайте `div` з `className` `card-club` і текстом `Santos FC`.

# --hints--

Ви повинні додати `div` з `className` `card-club` під `.card-tier-badge`.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

Елемент `.card-club` має містити текст `Santos FC`.
