...
حتى الآن، كل التدرجات التي أنشأتها كانت من الأعلى إلى الأسفل، وهذا هو الاتجاه الافتراضي. يمكنك تحديد اتجاه آخر بإضافته قبل ألوانك هكذا:
...
املأ `.bb3` بـ `repeating-linear-gradient`. استخدم `90deg` للاتجاه، و`building-color3` لأول لونين، و`window-color3` عند `15%` للون الثالث.

عندما لا تحدد مسافة للون، سيستخدم القيم المنطقية. في هذه الحالة، اللونان الأولان سيأخذان القيم الافتراضية `0%` و`7.5%` لأن التدرج يبدأ عند `0%`، و`7.5%` هو نصف `15%`، لذلك لا تحتاج لتحديدهما.

# --hints--

يجب أن تُعطي `.bb3` خاصية `background` باستخدام `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb3")?.background, "repeating-linear-gradient");
```

يجب أن تستخدم `90deg` للاتجاه في المعلمة الأولى من `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb3")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

لا يجب أن تحدد النسبة المئوية للون الأول.

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle(".bb3")?.getPropVal('background', true), "var(--building-color3)0%");
```

لا يجب أن تحدد النسبة المئوية للون الثاني.

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle(".bb3")?.getPropVal('background', true), "var(--building-color3)7.5%");
```

يجب أن تستخدم `--building-color3` لأول لونين.
...
يجب أن تستخدم `--window-color3` عند `15%` للون الثالث.
