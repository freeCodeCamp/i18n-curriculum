...
최종 정규 표현식은 `dear friend`와 같은 문자열을 찾을 것입니다. `dearRegex`를 선언하고 `dear friend` 문자열과 일치하는 정규 표현식을 할당하세요. 대소문자를 구분하지 않도록 설정하고 `denyList` 배열에 추가하세요.

# --hints--

`dearRegex` 변수를 선언해야 합니다.
...
`dearRegex`는 대소문자를 구분하지 않아야 합니다.
