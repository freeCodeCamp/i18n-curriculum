...
次に、`users` 配列から該当するユーザーを探します。

まず `user` という名前の const 変数 を作成し、`users.find()` を代入してください。 `find` メソッドには、仮引数 `user` を持つコールバック関数を渡してください。
...
`user` 定数に `users.find()` を代入してください。

```js
assert(code.match(/\s*const\s+user\s*=\s*users\.find\(/g));
```

コールバック関数に `poster` という仮引数を渡してください。
