...
Quando la larghezza dello schermo è piccola, l'elemento `h1` non si dispone su più righe come dovrebbe. Allinea al centro il testo dell'elemento `h1`.

Poi, dai all'elemento `main` un padding che renda completamente visibile l'intestazione di sezione `Student Info`.

# --hints--

Dovresti dare ad `h1` una proprietà `text-align` con il valore `center`.
...
Dovresti dare all'elemento `main` un `padding-top` di almeno `25px`.
