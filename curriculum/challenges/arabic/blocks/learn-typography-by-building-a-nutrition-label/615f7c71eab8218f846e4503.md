...
بعد آخر `.divider` لديك، أنشئ عنصر `p` آخر يحتوي على النص `Trans Fat 0g`. اجعل كلمة `Trans` مائلة بتغليفها داخل عنصر `i`. أعطِ عنصر `p` الجديد الخاصية `class` بقيمة `indent no-divider`. غلف النص `Trans Fat 0g` داخل عنصر `span` لمحاذاته.
...
يجب أن يحتوي عنصر `p` الجديد على النص `Trans Fat 0g`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText === 'Trans Fat 0g');
```

يجب أن يكون لعنصر `p` الجديد الخاصية `class` بقيمة `indent no-divider`.
...
يجب أن يغلف عنصر `span` النص `Trans Fat 0g`.
