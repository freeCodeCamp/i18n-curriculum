...
Centra gli elementi `h1` e `p` definendo loro un `margin` di `1em auto`. Poi, allinea anche il loro testo al centro con `center`.

# --hints--

Dovresti usare un selettore di elementi separati da virgola per definire lo stile degli elementi `h1` e `p`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

Dovresti usare un `margin` di `1em auto` per centrare gli elementi `h1` e `p`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

Dovresti usare un `text-align` di `center` per centrare il testo `h1` e `p`.
