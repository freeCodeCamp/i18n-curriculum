...
ネストされた`for`ループの後、最小の値を見つけました。これを現在の値と入れ替える必要があります。

バブルソートで行ったように、`temp`変数を使って`array[i]`の値を取り出し、`array[i]`と`array[minIndex]`の値を入れ替えてください。

# --hints--

ネストされた`for`ループの後、`temp`変数を宣言してください。
...
`array[minIndex]`を`array[i]`に代入してください。
...
`temp`を`array[minIndex]`に代入してください。
