...
`address` 要素内のすべてのテキストを水平方向に中央揃えにし、パディングを追加してください。

# --hints--

`address` 要素のセレクターを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

`address` に `text-align` を `center` で指定するべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

`address` に少なくとも `padding-top` の `1px` を指定するべきです。
...
`address` に少なくとも `padding-right` の `1px` を指定するべきです。
...
`address` に少なくとも `padding-bottom` の `1px` を指定するべきです。
...
`address` に少なくとも `padding-left` の `1px` を指定するべきです。
