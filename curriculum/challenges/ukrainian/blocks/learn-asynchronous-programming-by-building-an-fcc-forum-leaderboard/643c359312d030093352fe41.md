...
Тепер настав час відобразити дані на сторінці.

Почніть зі створення стрілкової функції з назвою `showLatestPosts`, яка приймає один параметр `data`.

# --hints--

У вас має бути стрілкова функція з назвою `showLatestPosts`.

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

Ваша функція `showLatestPosts` має мати параметр `data`.

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

`showLatestPosts` має бути визначено.

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts` має бути функцією.
