...
Colora `.bb3` con un `repeating-linear-gradient`. Usa `90deg` per la direzione, `building-color3` per i primi due colori e `window-color3` al `15%` per il terzo. Quando non specifichi una distanza per un colore, verranno usati dei valori che hanno senso. In questo caso, i primi due colori saranno di default allo `0%` e al `7.5%` perché l'inizio è allo `0%` e `7.5%` è la metà del `15%`.

# --hints--

Dovresti dare a `.bb3` una proprietà `background` usando `repeating-linear-gradient`.
...
Dovresti usare `--window-color3` al `15%` per il terzo colore.
