...
Quando o botão de embaralhar for pressionado, você deve definir o `currentSong` como nada e o `songCurrentTime` como `0`.

Defina `userData.currentSong` como `null` e `userData.songCurrentTime` como `0`.

*Nota*: Você não deve usar encadeamento opcional para esta etapa porque está definindo explicitamente as propriedades `currentSong` e `songCurrentTime` para serem `null` e `0` respectivamente.
