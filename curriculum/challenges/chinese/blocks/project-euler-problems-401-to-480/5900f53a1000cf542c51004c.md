...
令 `f(k, n)` = $e^\frac{k}{n} - 1$，对于所有非负整数 `k`。

值得注意的是，`f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3.1415926… ≈ π。

事实上，它是形式 `f(a, 200) + f(b, 200) + f(c, 200) + f(d, 200)` 的 π 的最佳近似。

令 `almostPi(n)` = a<sup>2</sup> + b<sup>2</sup> + c<sup>2</sup> + d<sup>2</sup>，其中 a、b、c、d 使得误差最小：$\lvert f(a,n) + f(b,n) + f(c,n) + f(d,n) - \Pi\rvert$
