...
Die Bilder sollten in einem Layout mit zwei Spalten und drei Reihen angeordnet sein.

Geben Sie dem `.image-wrapper`-Selektor eine `grid-template-columns`-Eigenschaft mit dem Wert `2fr 1fr` und eine `grid-template-rows`-Eigenschaft mit dem Wert `repeat(3, min-content)`. Dadurch erhalten unsere Rasterreihen eine Höhe, die sich nach dem Inhalt richtet, während die Spalten eine feste Breite basierend auf dem Container behalten.

# --hints--

Ihr `.image-wrapper`-Selektor sollte eine `grid-template-columns`-Eigenschaft mit dem Wert `2fr 1fr` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

Ihr `.image-wrapper`-Selektor sollte eine `grid-template-rows`-Eigenschaft mit dem Wert `repeat(3, min-content)` haben.
