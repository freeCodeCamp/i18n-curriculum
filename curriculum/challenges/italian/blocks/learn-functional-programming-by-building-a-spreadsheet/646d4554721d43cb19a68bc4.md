...
Il parametro `fn` riceverà il nome di una funzione, come `"SUM"`. Aggiorna `apply` per restituire implicitamente la funzione dal tuo oggetto `spreadsheetFunctions` usando la variabile `fn` come chiave per consultare l'oggetto.

Ricorda che `fn` potrebbe non contenere una stringa in minuscolo, quindi dovrai convertirla in una stringa minuscola.

# --hints--

La tua funzione `apply` dovrebbe usare un ritorno implicito.
...
La tua funzione `apply` dovrebbe consultare l'oggetto `spreadsheetFunctions`.
...
La tua funzione `apply` dovrebbe consultare la proprietà dell'oggetto `spreadsheetFunctions` che corrisponde al valore `fn`.
...
La tua funzione `apply` dovrebbe chiamare il metodo `.toLowerCase()` su `fn` nell'accesso alla proprietà.
