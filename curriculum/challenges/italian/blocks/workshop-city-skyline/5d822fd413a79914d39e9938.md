...
Dai alla classe `sky` una `radial-gradient`. Usa `#ffcf33` da `0%` a `20%`, `#ffff66` a `21%` e `#bbeeff` a `100%`. Questo aggiungerà un gradiente circolare allo sfondo che sarà il tuo sole.

# --hints--

Dovresti dare a `.sky` una `radial-gradient` nella proprietà `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".sky")?.background, "radial-gradient");
```

Dovresti dare a `radial-gradient` un primo colore `#ffcf33`.

```js
assert.match(new __helpers.CSSHelp(document).getStyle(".sky")?.background, /radial-gradient\(rgb\(255, 207, 51\)( 0%)?, rgb\(255, 207, 51\) 20%/);
```

Dovresti dare a `radial-gradient` un secondo colore `#ffff66` a `21%`.
...
Dovresti dare a `radial-gradient` un terzo colore `#bbeeff` a `100%`.
