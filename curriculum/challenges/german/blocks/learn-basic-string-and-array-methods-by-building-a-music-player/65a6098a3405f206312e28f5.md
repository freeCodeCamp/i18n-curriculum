...
Um die Songs alphabetisch nach Titel zu sortieren, müssen Sie eine Callback-Funktion zum Vergleichen an Ihre `sort()`-Methode übergeben.

Hier ist ein Beispiel, wie man eine Liste von Früchten nach Namen sortiert.
...
In den nächsten Schritten lernen Sie, was jede dieser `if`-Anweisungen innerhalb dieser Callback-Funktion bewirkt. Fügen Sie vorerst eine leere Callback-Funktion zu Ihrer `sort()`-Methode hinzu und verwenden Sie `a` und `b` als Parameternamen.

# --hints--

Sie sollten eine leere Callback-Funktion `()=>{}` innerhalb Ihrer `sort()`-Methode haben, mit `a` und `b` als Parameternamen.
