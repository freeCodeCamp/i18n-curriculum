...
`Bee` クラスの中で、`render` キーワードを接頭辞として付けた `override` メソッドを実装し、`innerText` の `emojiElement` プロパティを `emoji` プロパティの値で更新してください。

# --hints--

`Bee` クラスには、`render` キーワードを使った `override` メソッドが必要です。

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

`this.emojiElement.innerText` の `this.emoji` メソッド内で `render` を `Bee` に設定してください。
