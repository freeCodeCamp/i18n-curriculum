...
Kipengele cha `textarea` hufanya kazi kama kipengele cha `input` cha aina ya `text`, lakini kina faida ya ziada ya kupokea maandishi yenye mistari mingi, pamoja na idadi ya awali ya mistari na safu za maandishi.

Watumiaji wataweza kujisajili na wasifu. Ongeza `label` yenye maandishi ya `Provide a bio:` mwishoni mwa `fieldset`. Ongeza kipengele cha `textarea` ndani ya kipengele cha `label`. Kumbuka kuwa `textarea` inahitaji lebo ya kufunga.

# --hints--

Unapaswa kuongeza kipengele cha `label` mwishoni mwa `fieldset` ya tatu, baada ya vipengele vya `label` vilivyopo.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4)'));
```

Unapaswa kumpa `label` maandishi ya `Provide a bio:`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label:nth-child(4)')?.innerText, /Provide a bio/);
```

Unapaswa kupanga ndani kipengele cha `textarea` ndani ya `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea'));
```

Unapaswa kumpa kipengele cha `textarea` lebo za kufungua na kufunga.
