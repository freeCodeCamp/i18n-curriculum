...
Você precisa obter a música que está tocando no momento ou a primeira música da playlist. Para fazer isso, crie uma constante `song` e use o operador OR (`||`) para defini-la como a música atual de `userData` **ou** a primeira música no array `userData?.songs`.

Não esqueça de usar optional chaining.

# --hints--

Você não deve modificar o `setPlayButtonAccessibleText` existente e seu conteúdo.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Você deve acessar `userData?.currentSong` ou `userData?.songs[0]`.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

Você deve atribuir `userData?.currentSong || userData?.songs[0]` a uma constante `song`.
