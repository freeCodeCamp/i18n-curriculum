...
你的最终正则表达式将查找类似 `dear friend` 的字符串。 声明一个 `dearRegex` 并为其分配一个与字符串 `dear friend` 匹配的正则表达式。 记住不区分大小写，并将其添加到你的 `denyList` 数组中。

# --hints--

你应该使用 `const` 来声明你的 `dearRegex` 变量。

```js
assert.match(code, /const\s+dearRegex\s*=/);
```

你的 `dearRegex` 变量应该被分配一个正则表达式。
