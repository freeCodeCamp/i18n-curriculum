...
Halte das zweite `td` Element leer, weil Du ihm später Inhalte hinzufügen wirst.

Füge im dritten `td` Element den folgenden eingebetteten Ausdruck hinzu: `${posts_count - 1}`.

Dies wird die Anzahl der Antworten auf das Thema anzeigen.

# --hints--

Du solltest `${posts_count - 1}` innerhalb des dritten `td` Elements haben.
