...
表示したいノードを定義したので、それらを繰り返す必要があります。

`for` ループを追加して、`node_no` リスト内の各 `targets` を繰り返してください。ループ本体の中に `pass` をプレースホルダーとして追加してください。

# --hints--

`for` を使って `node_no` を繰り返す `targets` ループを作成してください。
