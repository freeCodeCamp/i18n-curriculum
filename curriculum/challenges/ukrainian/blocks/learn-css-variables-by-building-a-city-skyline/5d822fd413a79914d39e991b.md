...
Дайте `.fb2a` `width` з `100%`, а `.fb2b` — `width` з `100%` і `height` з `75%`.

# --hints--

Ви повинні дати `.fb2a` `width` з `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.width, "100%");
```

Ви повинні дати `.fb2b` `width` з `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2b")?.width, "100%");
```

Ви повинні дати `.fb2b` `height` з `75%`.
