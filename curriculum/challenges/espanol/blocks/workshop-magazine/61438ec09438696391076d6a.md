...
Debajo de su elemento `.heading`, cree un nuevo elemento `section` con `class` establecido en `text`. Dentro de eso, crea un elemento `p` con la `class` establecida en `first-paragraph` y el siguiente texto:
...
Tu nuevo elemento `section` debe ir después de tu elemento `.heading`.

```js
assert.equal(document.querySelectorAll('section')?.[1]?.previousElementSibling?.className, 'heading');
```

Tu elemento `section` debe tener su `class` establecido en `text`.

```js
assert.equal(document.querySelectorAll('section')?.[1]?.className, 'text');
```

Debe crear un nuevo elemento `p` dentro de su elemento `.text`.

```js
assert.lengthOf(document.querySelector('.text')?.querySelectorAll('p'), 1);
```

Tu nuevo elemento `p` debe tener `class` establecida en `first-paragraph`.

```js
assert.equal(document.querySelector('.text p')?.className, 'first-paragraph');
```

Tu nuevo elemento `p` debe tener el texto proporcionado.
