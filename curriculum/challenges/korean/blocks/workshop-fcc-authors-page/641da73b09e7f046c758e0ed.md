...
페이지에 저자 이름을 보려면 두 번째 `displayAuthors` 메서드 안에서 `.then()` 함수를 호출해야 합니다. 하지만 그 전에 빈 `authorDataArr` 배열에 저자 데이터를 할당해야 합니다.

먼저 `console.log()` 문장을 제거하세요. 그런 다음 `data`를 `authorDataArr` 변수에 할당하세요.
