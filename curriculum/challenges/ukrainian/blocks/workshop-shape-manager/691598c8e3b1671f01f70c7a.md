...
Усередині функції `handleShapeSelect` викличте метод `.preventDefault()` на `e`, а також викличте функцію `clearInputFields()`, щоб очистити поля введення.

# --hints--

Ви повинні викликати `.preventDefault()` на `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

Ви повинні викликати функцію `clearInputFields`.
