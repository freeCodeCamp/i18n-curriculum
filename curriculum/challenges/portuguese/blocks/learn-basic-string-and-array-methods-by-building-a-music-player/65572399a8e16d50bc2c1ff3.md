...
Se houver uma correspondência então defina `userData.currentSong` como `null` e `userData.songCurrentTime` como `0`. 

Depois disso, chame a função `pauseSong()` para parar a reprodução e a função `setPlayerDisplay()` para atualizar a exibição do player.
