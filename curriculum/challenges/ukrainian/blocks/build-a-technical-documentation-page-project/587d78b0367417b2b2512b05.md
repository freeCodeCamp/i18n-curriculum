...
1. В межах елемента `#main-doc` можна побачити декілька елементів `section`, кожен з яких має клас `main-section`. There should be a minimum of five
1. Першим елементом в `.main-section` має бути елемент `header`, який містить заголовок розділу.
1. Кожен елемент `section` з класом `main-section` також повинен мати `id`, що відповідає тексту елемента `header`. Any spaces should be replaced with underscores (e.g. The section that contains the header "JavaScript and Java" should have a corresponding `id="JavaScript_and_Java"`)
...
1. Крім того, навігаційна панель має містити елементи-посилання (`a`) з класом `nav-link`. There should be one for every element with the class `main-section`
