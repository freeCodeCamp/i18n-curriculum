...
Entre os seletores `0%` e `100%`, adicione um seletor `50%`.Isso será aplicado no meio do ciclo de animação.Defina a`background-color` para `purple`.

# --hints--

Para fazer isso, crie uma nova regra `50%` dentro da sua regra `@keyframes cabins`.
...
O elemento `50%` deve estar entre os seletores `0%` e `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '50%');
```

O seletor `50%` deve ter a propriedade `background-color` definida como `purple`.
