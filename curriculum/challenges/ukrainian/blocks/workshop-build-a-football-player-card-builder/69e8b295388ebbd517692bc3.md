...
Пам’ятайте з JSX, що фігурні дужки дозволяють вставити будь-який JavaScript-вираз безпосередньо всередину JSX. Саме так ви відображаєте динамічні дані з пропсів. У TSX це робиться так само.

Заголовок картки досі показує жорстко закодовані значення. Замініть текст `98` всередині `card-rating` на `player.overallRating`, а `ST` всередині `card-position` на `player.position`.

# --hints--

Ви маєте замінити жорстко закодований `98` на `player.overallRating` у `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

Ви маєте замінити жорстко закодований `ST` на `player.position` у `.card-position`.
