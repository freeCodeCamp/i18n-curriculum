...
从 DOM 中获取 ID 为 `select` 的 `species` 元素，并将其保存到名为 `selectElement` 的常量中。使用 `!` 非空断言操作符来断言该元素不是 `null`

# --hints--

你应该有一个名为 `selectElement` 的常量。
...
你的 `selectElement` 应该使用带有 `species` 类型参数的 `document.querySelector` 来查询 ID 为 `HTMLSelectElement` 的元素。

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

你的 `selectElement` 应该使用非空断言操作符（`!`）。
