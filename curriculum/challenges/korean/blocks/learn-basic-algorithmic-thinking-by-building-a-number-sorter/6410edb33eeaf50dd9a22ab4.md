...
`if` 조건이 참일 때, 두 요소를 교환하여 더 큰 요소가 배열 끝 쪽으로 "버블링"되도록 해야 합니다.

이를 위해 `temp` 변수를 선언하고 `array[j]` 값을 할당하세요. 그런 다음 `array[j]`에 `array[j + 1]` 값을 할당하세요. 마지막으로 `array[j + 1]`에 `temp` 값을 할당하세요.

# --hints--

`if` 문 안에서 `temp` 변수를 선언해야 합니다.
...
`array[j]`에 `array[j + 1]` 값을 할당해야 합니다.
