...
Тепер настав час працювати над відображенням правильного панеля, коли натискають на вкладку. Кожен із панелей має `id`, який відповідає атрибуту `aria-controls` відповідної вкладки. Наприклад, перша панель має `id` зі значенням `panel-earth`, що співпадає зі значенням атрибута `aria-controls` першої вкладки, яке дорівнює `panel-earth`.

Створіть змінну з назвою `associatedPanel` і призначте їй значення `tab.getAttribute("aria-controls")`. Це дасть вам `id` панелі, що відповідає натиснутій вкладці.

# --hints--

У вас має бути змінна з назвою `associatedPanel`, яка зберігає `id` панелі, пов’язаної з натиснутою вкладкою.
