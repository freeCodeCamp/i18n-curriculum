...
عندما تريد أن يختار المستخدمون من قائمة منسدلة، يمكنك استخدام العنصرين `select` و `option`.

إليك مثالًا على استخدام العنصرين `select` و `option` لإنشاء قائمة منسدلة لمدن مختلفة:
...
ابدأ بإضافة عنصر `select` مع تعيين الخاصيتين `name` و `id` إلى القيمة `"service"`.

# --hints--

يجب أن تحتوي مجموعة الحقول `fieldset` على عنصر `select`.

```js
assert.exists(document.querySelector('fieldset:nth-of-type(4) select'));
```

يجب أن يحتوي عنصر `select` على خاصية `name` بقيمة `"service"`.

```js
assert.exists(document.querySelector('select[name="service"]'));
```

يجب أن يحتوي عنصر `select` على خاصية `id` بقيمة `"service"`.
