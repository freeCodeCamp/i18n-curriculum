...
Folgen Sie dem gleichen Muster wie bei Ihrem `cartSubTotal` und aktualisieren Sie `cartTaxes`, damit der Wert von `tax` angezeigt wird, und `cartTotal`, damit die Eigenschaft `total` angezeigt wird.

# --hints--

Sie sollten auf die Eigenschaft `textContent` des Elements `cartTaxes` zugreifen.

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

Sie sollten die Methode `.toFixed()` auf der Variablen `tax` aufrufen und `2` als Argument übergeben.

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

Sie sollten die Template-Literal-Syntax verwenden, um das Dollarzeichen vor Ihrem Aufruf von `.toFixed()` hinzuzufügen.
...
Sie sollten die Eigenschaft `textContent` des Elements `cartTaxes` auf den Template-String `tax.toFixed` setzen.
...
Sie sollten auf die Eigenschaft `textContent` des Elements `cartTotal` zugreifen.

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

Sie sollten die Methode `.toFixed()` auf der Variablen `total` aufrufen und `2` als Argument übergeben. Denken Sie daran, das Schlüsselwort `this` zu verwenden.
...
Sie sollten die Eigenschaft `textContent` des Elements `cartTotal` auf Ihren Template-String `total.toFixed` setzen.
