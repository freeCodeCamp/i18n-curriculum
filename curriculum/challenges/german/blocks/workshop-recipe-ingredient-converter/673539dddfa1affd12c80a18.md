...
Sie müssen Ihre neue curried Funktion testen. Deklarieren Sie eine `servingsResult`-Variable und weisen Sie ihr das Ergebnis zu, das sich aus der Anpassung von einer Basismenge von 4 auf eine Portion von 6 ergibt.

Geben Sie diese Variable in der Konsole aus.

# --hints--

Sie sollten eine `servingsResult`-Variable deklarieren.

```js
assert.isDefined(servingsResult);
```

Ihre `servingsResult`-Variable sollte das Ergebnis des Aufrufs von `adjustQuantity(4)(6)` enthalten.

```js
assert.strictEqual(servingsResult, 24);
```

Sie sollten Ihren `servingsResult`-Wert in der Konsole ausgeben.

```js
assert.match(code, /console\.log\(\s*servingsResult\s*\)/);
```
