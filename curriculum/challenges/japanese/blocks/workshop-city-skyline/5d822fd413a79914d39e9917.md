...
要素に複数のグラデーションを追加するには、カンマ（`,`）で区切って次のようにします。
...
下にある`repeating-linear-gradient`の下に`.fb1c`を追加してください。`--building-color4`から`0%`の`10%`と、`--window-color4`と`10%`の`90%`を使います。これにより、最後に追加したグラデーションの背後が埋まります。

# --hints--

最初の`repeating-linear-gradient`は変更しないでください。
...
最初の`,`の後にカンマ（`repeating-linear-gradient`）があるはずです。
...
`repeating-linear-gradient`から`--building-color4`の`0%`を最初の色として`10%`に追加してください。
...
`--window-color4`から`10%`の`90%`を2番目の色として使うべきです。
