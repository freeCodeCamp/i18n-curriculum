...
All'interno della funzione `displayStory`, verifica se l'oggetto `storyObj` ha una proprietà che corrisponde al parametro `genre` e imposta il contenuto testuale del paragrafo `#result` con la storia del genere selezionato.

Imposta inoltre il colore del bordo di `storyContainer` sulla proprietà `borderColor` del genere selezionato.

Testa questo chiamando la funzione `displayStory` con il genere `scary`. Il colore del bordo dovrebbe essere `#ee4b2b` e la storia dovrebbe essere `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.

# --hints--

Dovresti chiamare la funzione `displayStory` con il genere `"scary"`.

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

Quando chiami `displayStory("scary")`, il colore del bordo dovrebbe essere `#ee4b2b`.
...
Quando chiami `displayStory("scary")`, la storia mostrata dovrebbe essere `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.
