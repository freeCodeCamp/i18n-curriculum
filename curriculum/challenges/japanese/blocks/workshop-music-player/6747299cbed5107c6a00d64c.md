...
残りの曲は`allSongs` 配列に追加されています。

前のレッスンでは、<dfn>Web Audio API</dfn>と、それを使って曲を再生する方法を学びました。すべての最新ブラウザはWeb Audio APIをサポートしており、ウェブアプリケーションで音声を生成および処理できます。

`audio` という名前の変数を作成し、`new Audio()` と等しく設定してください。これにより、新しいHTML5の`audio` 要素が作成されます。

# --hints--

`audio` という名前の変数を用意してください。

```js
assert.isDefined(audio);
```

`Audio` という名前の変数に、`audio` オブジェクトの新しいインスタンスを代入してください。
