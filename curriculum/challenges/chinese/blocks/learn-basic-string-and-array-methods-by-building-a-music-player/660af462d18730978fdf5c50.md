...
现在是时候将你所学到的关于箭头函数的知识应用到你的音乐播放器项目中了。 

首先删除 `printGreeting`、`printMessage` 和 `addTwoNumbers` 函数。 另外，删除所有控制台语句和函数调用。 

# --hints--

你不应该有 `printGreeting` 函数。

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

你不应该有 `printMessage` 函数。

```js
assert.notMatch(code, /const printMessage = org => {/);
```

你不应该有 `addTwoNumbers` 函数。

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

你不应该有 `printGreeting();` 函数调用。

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

你不应该有 `printMessage('freeCodeCamp');` 函数调用。

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

你不应该有 `console.log(addTwoNumbers(3,4));` 控制台语句。
