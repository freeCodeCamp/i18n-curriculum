...
Su Doku (японське значення *number place*) — це назва популярної головоломки. Її походження не зовсім зрозуміле, але заслуга належить Леонгарду Ейлеру, який винайшов подібну, але значно складнішу головоломку під назвою латинські квадрати. Мета головоломок Su Doku — замінити пропуски (або нулі) у сітці 9 на 9 так, щоб кожен рядок, стовпець і блок 3 на 3 містили всі цифри від 1 до 9. Нижче наведено приклад типової початкової сітки головоломки та її розв’язку.
...
Добре складена головоломка Su Doku має унікальне розв’язання і може бути розв’язана логікою, хоча іноді може знадобитися застосування методів «вгадування і перевірки» для виключення варіантів (це питання викликає багато суперечок). Складність пошуку визначає складність головоломки; наведений вище приклад вважається легким, бо його можна розв’язати прямим логічним висновком.

Масив `puzzlesArr` містить різні рядки головоломок Su Doku різної складності, але всі з унікальними розв’язками.

Розв’язавши всі головоломки в `puzzlesArr`, знайдіть суму тризначних чисел, розташованих у верхньому лівому куті кожної сітки розв’язку; наприклад, 483 — це тризначне число, розташоване у верхньому лівому куті сітки розв’язку вище.
...
`suDoku(testPuzzles1)` має повертати число.

```js
assert(typeof suDoku(_testPuzzles1) === 'number');
```

`suDoku(testPuzzles1)` має повертати `1190`.

```js
assert.strictEqual(suDoku(_testPuzzles1), 1190);
```

`suDoku(testPuzzles2)` має повертати `24702`.
