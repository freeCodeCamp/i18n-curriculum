...
title: Tumia sifa ya flex-direction kuunda mistari katika ingizo la tweet
challengeType: 0
...
`header` na `footer` katika mfano wa ingizo la tweet zina vitu vya watoto ambavyo vinaweza kupangwa kama mistari kwa kutumia sifa ya `flex-direction`. Hii inaeleza CSS kupanga watoto wakiwa wima kwa usawa.

# --instructions--

Ongeza sifa ya CSS `flex-direction` kwa `header` na `footer` zote na weka thamani kuwa `row`.
...
`header` inapaswa kuwa na sifa ya `flex-direction` iliyowekwa kuwa `row`.

```js
assert.match(code, /header\s*?{[^}]*?flex-direction:\s*?row;/g);
```

`footer` inapaswa kuwa na sifa ya `flex-direction` iliyowekwa kuwa `row`.
