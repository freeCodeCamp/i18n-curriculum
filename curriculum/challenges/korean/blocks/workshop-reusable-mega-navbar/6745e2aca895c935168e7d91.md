...
마지막으로, 첫 번째 앵커 태그에는 `Calendar` 텍스트를, 두 번째에는 `Chat` 텍스트를, 세 번째에는 `Email` 텍스트를 설정하세요.

이렇게 하면 mega 내비게이션 바 컴포넌트가 완성됩니다!

# --hints--

첫 번째 앵커 요소에는 `Calendar` 텍스트를 설정해야 합니다.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

두 번째 앵커 요소에는 `Chat` 텍스트를 설정해야 합니다.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

세 번째 앵커 요소에는 `Email` 텍스트를 설정해야 합니다.
