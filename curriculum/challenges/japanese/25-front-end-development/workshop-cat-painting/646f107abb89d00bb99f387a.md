...
子孫セレクターを使用して、`.cat-whiskers-left` の要素内にある 3 つの `div` 要素を選択してください。 Give it a `width` of `40px`, a `height` of `1px`, and a `background-color` of `#000`.

# --hints--

`.cat-whiskers-left div` セレクターが必要です。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div'));
```

`.cat-whiskers-left div` セレクターの `width` を `40px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.width, '40px');
```

`.cat-whiskers-left div` セレクターの `height` を `1px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.height, '1px');
```

`.cat-whiskers-left div` セレクターの `background-color` を `#000` に設定してください。
