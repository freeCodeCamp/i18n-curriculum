...
Nell’esempio, se `a.name` è uguale a `b.name`, allora la funzione restituisce `0`. Questo significa che nulla cambia e l’ordine di `a` e `b` rimane lo stesso.
...
Sotto le tue istruzioni `if`, restituisci il numero `0` per lasciare invariato l’ordine dei due elementi.

# --hints--

Dovresti restituire il numero `0` sotto le tue istruzioni `if`.
