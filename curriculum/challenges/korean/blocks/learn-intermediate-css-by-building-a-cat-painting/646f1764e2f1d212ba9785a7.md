...
클래스 선택자를 사용해 `.cat-whisker-left-bottom`를 `position`의 `absolute`와 `top`의 `134px`, 그리고 `left`의 `52px` 위치로 이동하세요.

# --hints--

`.cat-whisker-left-bottom` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom'))
```

`.cat-whisker-left-bottom` 선택자에 `position`로 설정된 `absolute` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.position === 'absolute')
```

`.cat-whisker-left-bottom` 선택자에 `top`로 설정된 `134px` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.top === '134px')
```

`.cat-whisker-left-bottom` 선택자에 `left`로 설정된 `52px` 속성이 있어야 합니다.
