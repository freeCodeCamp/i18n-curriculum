...
다음으로 `userAvatarUrl`를 구성하세요.

먼저 `userAvatarUrl`라는 상수를 만드세요. 그런 다음 삼항 연산자를 사용해 `avatar`가 `"/user_avatar/"`로 시작하는지 확인하세요.

만약 그렇다면 `concat` 메서드를 사용해 `avatar`를 `avatarUrl`에 연결하세요. 그렇지 않으면 `avatar`를 반환하세요.

이렇게 하면 아바타 URL이 상대 경로든 절대 경로든 올바르게 형성됩니다.

# --hints--

`userAvatarUrl`라는 상수가 있어야 합니다.

```js
assert(code.match(/const\s+userAvatarUrl\s*=/));
```

`ternary`는 `avatar`가 `"/user_avatar/"`로 시작하는지 확인하고 `avatarUrl.concat(avatar)` 또는 `avatar`를 반환해야 합니다.
