...
Prima di eliminare una canzone, devi verificare se la canzone è attualmente in riproduzione. Se lo è, devi mettere in pausa la canzone e riprodurre la canzone successiva nella playlist.

Usa un'istruzione `if` per controllare se il `userData?.currentSong?.id` è uguale al `id` della canzone che vuoi eliminare.

# --hints--

Dovresti creare un'istruzione `if` vuota con la condizione `userData?.currentSong?.id === id`.
