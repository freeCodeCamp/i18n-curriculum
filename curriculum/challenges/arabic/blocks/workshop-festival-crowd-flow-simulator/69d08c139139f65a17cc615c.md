...
داخل حلقة `while` الخاصة بك:

- سجّل الرسالة، `"\nTick " + (tickIndex + 1)`.
- استعرض كل بوابة في مصفوفة `gates` واستدعِ الدالة `handleGateAtTick()` مع هذه المعلمات:
  - `gates`: مصفوفة الإدخال الكاملة للبوابات.
  - `gate`: كائن البوابة الحالي.
  - `tickIndex`: التوقيت الحالي.
  - `throughputSummary`: الكائن المحلي الذي يتتبع إجمالي الحضور المعالج لكل بوابة.
- زد قيمة `tickIndex` بواحد للانتقال إلى التوقيت التالي.

# --hints--

يجب أن تسجل الرسالة التالية في وحدة التحكم، `"\nTick " + (tickIndex + 1)`.
...
بعد التسجيل في وحدة التحكم، يجب أن تستعرض كل بوابة في مصفوفة `gates` وتستدعي الدالة `handleGateAtTick()` مع `gates`، `gate`، `tickIndex`، و`throughputSummary`.
...
بعد استعراضك لمصفوفة `gates`، يجب أن تزيد قيمة `tickIndex` بواحد.
