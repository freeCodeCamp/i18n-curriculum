...
Para completar o workshop, adicione uma declaração final de print para exibir o melhor preço com duas casas decimais usando uma f-string e este formato: `Best price for [product.name] for [user_tier] user: $[best_price]` onde `[product.name]`, `[user_tier]` e `[best_price]` devem ser substituídos pelos valores reais das variáveis.

Um exemplo de saída poderia ser: `Best price for Wireless Buds for Premium user: $93.00`. Note que o preço tem duas casas decimais.

Com isso, a oficina do calculador de desconto está completa!

# --hints--

Seu resultado deve corresponder ao formato: `Best price for {product.name} for {user_tier} user: ${best_price:.2f}`.
