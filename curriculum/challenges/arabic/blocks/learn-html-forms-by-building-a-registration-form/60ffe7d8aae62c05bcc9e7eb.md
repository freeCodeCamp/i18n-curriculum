...
مع `display` بقيمة `block`، يكون زر الإرسال ملتصقًا بحافة اليسار للعنصر الأصل.

استخدم نفس التقنية التي استخدمتها لمركزة الـ `form` لمركزة زر الإرسال.

# --hints--

يجب إعطاء زر الإرسال قيمة `margin` تساوي `0 auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.margin, '0px auto');
```

لا يجب أن تعطي زر الإرسال `min-width` أو `max-width`.
