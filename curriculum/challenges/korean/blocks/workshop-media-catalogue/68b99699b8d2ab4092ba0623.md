...
`if` 메서드의 `__str__` 문장 뒤에 `movies`와 `series`라는 두 변수를 만들고 각각 `get_movies` 메서드 호출과 `get_tv_series` 메서드 호출을 할당하세요.

# --hints--

`movies` 메서드에 `__str__`라는 변수가 있어야 합니다.
...
`self.get_movies()` 변수에 `movies`를 할당하세요.
...
`series` 메서드에 `__str__`라는 변수가 있어야 합니다.
...
`self.get_tv_series()` 변수에 `series`를 할당하세요.
