...
داخل جملة `if`، أنشئ متغيرًا باسم `category` ليخزن فئة المصروف. عيّن له استدعاءً للدالة `input()` واستخدم `'Enter category: '` كمعلمة.

# --hints--

يجب إعلان متغير باسم `category` داخل جملة `if`.
...
يجب تعيين `input('Enter category: ')` إلى متغير `category`.
