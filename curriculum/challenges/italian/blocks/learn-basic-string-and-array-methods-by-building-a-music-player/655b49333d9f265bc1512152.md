...
Nota che la copertina dell'album nell'HTML e le canzoni nell'array `userData.songs` sono cambiate. Abbiamo sostituito le canzoni originali con versioni più brevi che puoi usare per testare la tua app nei passaggi successivi.

Adesso devi verificare se esiste una canzone successiva da riprodurre. Recupera l'indice della canzone corrente chiamando la funzione `getCurrentSongIndex()` e salvalo in una costante `currentSongIndex`.

Dopodiché, crea una costante `nextSongExists` che contenga il valore booleano `true` o `false` a seconda che la canzone successiva esista o meno.

# --hints--

Non dovresti modificare l'event listener `ended` esistente e il suo contenuto.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Dovresti creare una costante `currentSongIndex` e impostarla alla chiamata della funzione `getCurrentSongIndex`.
...
Dovresti verificare se esiste una canzone successiva confrontando `userData.songs.length` e `currentSongIndex` e assegnare il risultato a una costante `nextSongExists`. Se l'ultimo indice dell'array delle canzoni (`userData.songs.length - 1`) è maggiore di currentSongIndex significa che esiste una canzone successiva.
