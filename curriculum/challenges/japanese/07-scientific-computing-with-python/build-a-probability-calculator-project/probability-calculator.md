...
- `expected_balls`: 実験で帽子から取り出そうとするボールの正確なグループを示すオブジェクト。 For example, to determine the probability of drawing 2 blue balls and 1 red ball from the hat, set `expected_balls` to `{'blue':2, 'red':1}`.
