...
Використайте оператор `+=`, щоб додати `" Your <weapon> breaks."`, з пробілом перед `Your`, до кінця `text.innerText`. Замініть `<weapon>` на останній елемент у масиві `inventory`, використовуючи `inventory.pop()`, який видалить останній предмет у масиві ТА поверніть його так, щоб він з’явився у рядку.

# --hints--

Ви повинні використати складений оператор присвоювання, щоб змінити `text.innerText`.
...
Додайте значення, яке повертається `inventory.pop()` до рядка `" Your "`.
