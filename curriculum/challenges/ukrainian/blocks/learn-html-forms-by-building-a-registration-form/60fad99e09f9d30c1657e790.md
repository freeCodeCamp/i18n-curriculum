...
Використайте селектор `form`, щоб стилізувати елемент `form`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('form'));
```

Надайте елементу `form` властивість `margin` зі значенням `0 auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.margin, '0px auto');
```

Надайте елементу `form` властивість `max-width` зі значенням `500px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.maxWidth, '500px');
```

Надайте елементу `form` властивість `min-width` зі значенням `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.minWidth, '300px');
```

Надайте елементу `form` властивість `width` зі значенням `60vw`.
