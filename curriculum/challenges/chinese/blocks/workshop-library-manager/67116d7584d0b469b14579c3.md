...
在之前的课程中，你学习了如何使用 `filter()` 方法，该方法用于返回一个过滤结果的新数组。
...
创建一个带有两个参数的 `getBooksByAuthor` 函数——一个包含书籍对象的数组和一个包含作者的字串。
...
你的 `getBooksByAuthor` 函数应有两个参数。

```js
assert.lengthOf(getBooksByAuthor, 2);
```

你的 `getBooksByAuthor` 函数应返回一个数组。

```js
assert.isArray(getBooksByAuthor(library, ''));
```

你的 `getBooksByAuthor` 函数应返回任何作者的正确书籍数量。
