...
현재 재생 중인 노래를 일시 정지하는 작업을 하세요.

매개변수를 받지 않는 `pauseSong` 함수를 정의하세요. 새 함수 안에서 노래가 일시 정지될 때 현재 시간을 저장하려면 `songCurrentTime` 변수의 `userData` 값을 `currentTime` 객체의 `audio`에 설정하세요.


# --hints--

`pauseSong` 함수가 있어야 합니다.

```js
assert.isFunction(pauseSong);
```

`pauseSong` 함수는 `songCurrentTime` 변수의 `userData` 값을 `currentTime` 객체의 `audio`에 설정해야 합니다.
