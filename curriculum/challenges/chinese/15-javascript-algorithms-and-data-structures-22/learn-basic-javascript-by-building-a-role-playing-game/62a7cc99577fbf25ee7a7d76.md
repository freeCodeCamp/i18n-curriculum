...
`goTown` 和 `goStore` 函数中有重复。 代码中的重复表明你需要另一个功能。

在上一个项目中，你学习了如何使用函数参数，如下所示：
...
创建一个空的 `update` 函数，该函数接受一个名为 `location` 的参数。

# --hints--

你应该使用 `function` 关键字来声明 `update`。

```js
assert.match(code, /function\s+update/);
```

你的 `update` 函数应该接受一个名为 `location` 的参数。
