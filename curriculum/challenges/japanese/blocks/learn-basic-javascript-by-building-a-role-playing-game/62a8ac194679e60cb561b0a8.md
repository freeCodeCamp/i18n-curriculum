...
空の `button text` 配列に3つの文字列要素を入れてください。`innerText` 関数内でボタンの `goTown` プロパティに代入されている3つの文字列を使います。配列の値はカンマで区切ることを忘れないでください。

# --hints--

`button text` 配列には3つの要素が必要です。

```js
assert.lengthOf(locations[0]["button text"], 3);
```

`button text` 配列には3つの文字列が必要です。
...
`button text` 配列の最初の値は `"Go to store"` であるべきです。

```js
assert.equal(locations[0]["button text"][0], "Go to store");
```

`button text` 配列の2番目の値は `"Go to cave"` であるべきです。

```js
assert.equal(locations[0]["button text"][1], "Go to cave");
```

`button text` 配列の3番目の値は `"Fight dragon"` であるべきです。
