...
더 이상 필요 없으니 `myTaskArr` 배열과 `localStorage` 관련 모든 코드를 제거하세요.

# --hints--

방금 배운 `myTaskArr`와 `localStorage` 관련 모든 코드를 제거하세요.
...
코드 내 어디에서든 남아 있는 `myTaskArr` 참조를 모두 제거하세요.

```js
assert.notMatch(code, /myTaskArr/);
```

`localStorage.getItem` 항목에 대한 `"data"` 참조를 모두 제거하세요.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

코드 내 어디에서든 남아 있는 `getTaskArrObj` 참조를 모두 제거하세요.
