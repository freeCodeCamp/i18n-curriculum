...
Ora è il momento di applicare ciò che hai imparato sulle funzioni freccia al tuo progetto di lettore musicale.

Inizia rimuovendo le funzioni `printGreeting`, `printMessage` e `addTwoNumbers`. Rimuovi anche tutte le istruzioni console e le chiamate alle funzioni.

# --hints--

Non dovresti avere una funzione `printGreeting`.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

Non dovresti avere una funzione `printMessage`.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

Non dovresti avere una funzione `addTwoNumbers`.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

Non dovresti avere una chiamata alla funzione `printGreeting();`.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

Non dovresti avere una chiamata alla funzione `printMessage('freeCodeCamp');`.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

Non dovresti avere un’istruzione console `console.log(addTwoNumbers(3,4));`.
