...
在先前的步驟中，你已經建立了播放歌曲的功能。現在你需要為播放按鈕添加該功能。

在 `playButton` 上使用 `addEventListener()` 方法，並將 `"click"` 事件作為第一個引數傳入。暫時使用一個回呼作為第二個引數，該回呼會呼叫 `playSong(0)`。
...
你應該為 `playButton` 添加 `click` 事件監聽器，並將一個呼叫 `playSong(0)` 的回呼函式作為第二個引數傳遞給 `addEventListener` 方法。
