...
要準備你的項目邏輯，使用 `const` 和箭頭語法聲明一個 `sortInputArray` 函數。 它應該採用單個 `event` 參數。

# --hints--

你應該用 `const` 聲明一個 `sortInputArray` 函數。

```js
assert.match(code, /const\s+sortInputArray\s*=/);
```

你的 `sortInputArray` 應該是一個函數。

```js
assert.isFunction(sortInputArray);
```

你的 `sortInputArray` 應該採用單個 `event` 參數。

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)/);
```

你的 `sortInputArray` 函數應該使用箭頭語法。

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)\s*=>/);
```

你的 `sortInputArray` 函數應該是空的。
