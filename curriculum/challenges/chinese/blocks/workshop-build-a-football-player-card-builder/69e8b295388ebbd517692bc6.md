...
球员照片是从一个硬编码的 URL 装载的。将 `src` 属性值 `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` 替换为 `player.imageUrl`。

同时，将硬编码的 `alt` 文本替换为使用 `player.name` 和文本 `headshot` 的模板字面量动态值。

注意，当你在 TSX 中将 JavaScript 表达式用作属性值时，使用括弧状的花括弧 `{}` 而不是引号。例如 `src={player.imageUrl}`，而不是 `src="{player.imageUrl}"`。

# --hints--

你应该将硬编码的图像 URL 替换为 `player.imageUrl` 作为 `src` 属性。

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

你应该将硬编码的 `alt` 文本替换为使用 `player.name` 和文本 `headshot` 的模板字面量。
