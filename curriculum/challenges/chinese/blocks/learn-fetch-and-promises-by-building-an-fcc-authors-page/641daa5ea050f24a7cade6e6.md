...
如果满足此条件，则通过将其 `disabled` 属性设置为 `true` 来禁用该按钮。 另外，将按钮的 `textContent` 设置为 `"No more data to load"`。
...
你应该将 `loadMoreBtn` 的 `disabled` 属性设置为 `true`。

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

你应该将 `loadMoreBtn` 的 `textContent` 设置为 `"No more data to load"`。
