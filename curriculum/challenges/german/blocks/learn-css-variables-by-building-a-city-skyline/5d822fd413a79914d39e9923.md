...
Entfernen Sie die `background-color` Eigenschaft und den Wert aus `.fb3` und fügen Sie sie zu `.fb3a` und `.fb3b` hinzu.

# --hints--

Sie sollten die `background-color` aus `.fb3` entfernen.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

Sie sollten `.fb3a` eine `background-color` von `--building-color1` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

Sie sollten `.fb3b` eine `background-color` von `--building-color1` geben.
