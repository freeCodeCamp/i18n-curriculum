...
Створіть функцію `showResults` з параметром під назвою `userOption`.

Inside your `showResults` function, the `roundResultsMsg` should be updated with the result of the round.

Потім `playerScoreSpanElement` та `computerScoreSpanElement` також слід оновити, щоб відображати оновлені результати гравця та комп’ютера.
...
Ви повинні мати функцію під назвою `showResults`.

```js
assert.isFunction(showResults);
```

Функція `showResults` повинна мати параметр під назвою `userOption`.
