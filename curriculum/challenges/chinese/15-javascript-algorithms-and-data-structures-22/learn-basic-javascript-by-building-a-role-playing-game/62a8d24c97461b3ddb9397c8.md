...
一旦玩家拥有最强大的武器，你就可以赋予他们出售旧武器的能力。

在外部 `else` 语句中，将 `button2.innerText` 设置为 `"Sell weapon for 15 gold"`。 还将 `button2.onclick` 设置为函数名称 `sellWeapon`。

# --hints--

你应该设置 `button2.innerText` 的值。

```js
assert.match(buyWeapon.toString(), /button2\.innerText/);
```

你应该将 `button2.innerText` 的值设置为 `"Sell weapon for 15 gold"`。

```js
assert.match(buyWeapon.toString(), /button2\.innerText\s*=\s*('|")Sell weapon for 15 gold\1\s*;?/);
```

你应该设置 `button2.onclick` 的值。

```js
assert.match(buyWeapon.toString(), /button2\.onclick/);
```

你应该将 `button2.onclick` 的值设置为 `sellWeapon`。
