...
Necesitarás ajustar para el número de porciones que se están calculando. Esta es una gran oportunidad para usar otra función curried.

Declara una función `adjustForServings`, que acepte un argumento `baseQuantity`. Debe devolver una segunda función que acepte un argumento `newServings`.

La función interna debe devolver el resultado de multiplicar `baseQuantity` y `newServings`.

# --hints--

Debe declarar una función `adjustForServings`.

```js
assert.isFunction(adjustForServings);
```

Tu función `adjustForServings` debe aceptar un argumento `baseQuantity`.

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

Tu función `adjustForServings` debe devolver una función.

```js
assert.isFunction(adjustForServings?.());
```

Tu función interna debe tener un parámetro `newServings`.

```js
assert.match(adjustForServings?.()?.toString(), /\(newServings\)/);
```

Tu función interna debe devolver el resultado de multiplicar `baseQuantity` y `newServings`.
