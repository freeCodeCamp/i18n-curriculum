...
Agora, é hora de adicionar a funcionalidade de reset ao `resetButton`.Isso trará de volta as músicas na playlist quando clicado.

Adicione um listener de evento de clique à variável `resetButton`.Passe uma callback usando a sintaxe de arrow e deixe-a vazia por enquanto.

# --hints--

Você deve usar o método `addEventListener()` em `resetButton`.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

O seu listener de evento `resetButton` deve escutar um evento `"click"`.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

A função de callback do seu event listener deve usar a sintaxe arrow e ter um par vazio de chaves.
