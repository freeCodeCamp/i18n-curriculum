...
Unapaswa kuongeza kipengele cha nne cha `p` katika kipengele chako cha `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 5);
```

Kipengele chako kipya cha `p` kinapaswa kuja baada ya kipengele chako cha `blockquote`.
