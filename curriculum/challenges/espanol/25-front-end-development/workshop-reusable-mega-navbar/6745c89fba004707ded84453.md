...
Crea un elemento de anclaje dentro del primer elemento `li`. Asigna al elemento `a` un atributo `href` con el valor `#` y el texto `Dashboard`.

# --hints--

Debes crear un elemento `a` dentro del primer elemento de la lista.

```js
assert.exists(document.querySelector('li a'));
```

Tu elemento `a` debería tener su atributo `href` establecido como `#`.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), '#');
```

Tu elemento `a` debería tener el texto `Dashboard`.
