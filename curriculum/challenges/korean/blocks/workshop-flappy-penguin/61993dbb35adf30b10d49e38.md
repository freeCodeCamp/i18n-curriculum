...
`.penguin-body` 요소를 선택하고 `width` 값을 가진 `53%`과 `height` 값을 가진 `45%`을 지정하세요. 그런 다음 `background`를 `45deg`에서 선형 그라디언트로 설정하고, `rgb(134, 133, 133)`은 `0%`에서, `rgb(234, 231, 231)`은 `25%`에서, `white`은 `67%`에서 시작하도록 하세요.

# --hints--

`.penguin-body` 선택자를 사용하세요.

```js
assert.match(code, /\.penguin-body\s*\{/);
```

`.penguin-body`에 `width` 값을 가진 `--fcc-expected--`을 지정해야 하지만 `--fcc-actual--`이 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

`.penguin-body`에 `height` 값을 가진 `--fcc-expected--`을 지정해야 하지만 `--fcc-actual--`이 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

`.penguin-body`에 `background` 값을 가진 `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)`을 지정해야 합니다.
