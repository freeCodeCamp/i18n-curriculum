...
Deklarieren Sie eine Variable namens `plantsSet` und weisen Sie ihr einen Aufruf der `displayPlantsSet`-Funktion zu.

Geben Sie anschließend `plantsSet` in der Konsole aus. Sie müssen die Browser-Konsole öffnen, um Ihre Ausgabe zu sehen.


# --hints--

Sie sollten eine Variable namens `plantsSet` haben.

```js
assert.exists(plantsSet);
```

Sie sollten `displayPlantsSet` aufrufen und es `plantsSet` zuweisen.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

Sie sollten `plantsSet` in der Konsole ausgeben.
