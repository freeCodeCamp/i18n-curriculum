...
Il selettore `.first-paragraph::first-letter` dovrebbe avere una proprietà `float` impostata su `left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float === 'left');
```

Il selettore `.first-paragraph::first-letter` dovrebbe avere una proprietà `margin-right` impostata su `1rem`.
