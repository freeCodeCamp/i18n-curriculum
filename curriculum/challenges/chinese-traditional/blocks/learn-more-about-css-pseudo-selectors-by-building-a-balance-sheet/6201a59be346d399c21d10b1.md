...
爲 `tr.data` 元素中的 `td` 元素創建另一個選擇器，但要特別選擇最後一個。 將它的右側內邊距設置爲 `0.5rem`。

至此，你的資產負債表就完成了！

# --hints--

你應該有一個 `tr.data td:last-of-type` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td:last-of-type'));
```

你的 `tr.data td:last-of-type` 選擇器應該有一個 `padding-right` 屬性，設置爲 `0.5rem`。
