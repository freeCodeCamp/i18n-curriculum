...
Ifuatayo, unahitaji kukagua kama orodha ya nyimbo ni tupu. Ikiwa ni tupu, unapaswa kuweka tena kitu cha `userData` katika hali yake ya awali.

Tumia usemi wa `if` kukagua kama `userData?.songs` ina urefu wa `0`.

# --hints--

Unapaswa kuunda usemi wa `if` tupu wenye sharti la `userData?.songs.length === 0`.
