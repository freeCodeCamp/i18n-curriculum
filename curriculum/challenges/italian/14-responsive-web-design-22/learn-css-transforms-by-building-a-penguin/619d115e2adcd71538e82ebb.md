...
Dovresti dare a `.beak` una proprietà `height` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.height, '10%');
```

Dovresti dare a `.beak` una proprietà `background-color` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.backgroundColor, 'orange');
```

Dovresti dare a `.beak` una proprietà `border-radius` con il valore `--fcc-expected--` invece di `--fcc-actual--`.
