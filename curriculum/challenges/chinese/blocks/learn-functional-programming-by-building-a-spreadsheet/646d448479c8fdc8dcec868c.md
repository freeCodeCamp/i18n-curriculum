...
声明一个 `functionCall` 变量，并为其分配以下正则表达式：`/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

该表达式将查找类似 `sum(1, 4)` 的函数调用。

# --hints--

你应该声明一个 `functionCall` 变量。
...
你应该使用 `const` 来声明你的 `functionCall` 变量。
