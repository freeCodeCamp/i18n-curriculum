...
Crea otro método llamado `get_tv_series` y haz que devuelva una lista con todas las instancias de `TVSeries` que puedas encontrar en `self.items`.

# --hints--

Tu clase `MediaCatalogue` debería tener un método llamado `get_tv_series`.
...
Tu método `get_tv_series` debería devolver una lista que contenga todos los elementos en `self.items` que sean instancias de la clase `TVSeries`.
