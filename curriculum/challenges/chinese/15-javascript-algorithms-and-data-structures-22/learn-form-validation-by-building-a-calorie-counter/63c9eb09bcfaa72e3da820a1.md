...
你需要获取所有输入容器。 声明一个 `inputContainers` 变量，并将其赋值为查询文档中所有属于 `input-container` 类的元素的值。

# --hints--

你应该在 `clearForm` 函数中声明一个 `inputContainers` 变量。

```js
assert.match(clearForm.toString(), /inputContainers\s*=/);
```

你应该使用 `querySelectorAll` 方法来获取所有属于 `input-container` 类的元素。

```js
assert.match(clearForm.toString(), /document\.querySelectorAll\(\s*('|")\.input-container\1\s*\)/);
```

你应该将 `querySelectorAll` 方法的值分配给 `inputContainers` 变量。
