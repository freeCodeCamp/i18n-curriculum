...
All'interno della funzione, ottieni i riferimenti agli elementi HTML responsabili di mostrare il titolo della canzone e l'artista.

Consulta gli elementi `#player-song-title` e `#player-song-artist` con il metodo `getElementById()`. Assegnali rispettivamente alle variabili `playingSong` e `songArtist`.

# --hints--

Non dovresti modificare la funzione `setPlayerDisplay` esistente e il suo contenuto.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#player-song-title`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Dovresti assegnare l'elemento `#player-song-title` alla variabile `playingSong`. Non dimenticare di usare `const` per dichiarare la variabile.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#player-song-artist`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Dovresti assegnare l'elemento `#player-song-artist` alla variabile `songArtist`. Non dimenticare di usare `const` per dichiarare la variabile.
