...
删除 `myTaskArr` 数组和 `localStorage` 的所有代码，因为你不再需要它们。

# --hints--

你应该删除 `myTaskArr` 以及你刚刚学到的与 `localStorage` 相关的所有代码。
...
你应该移除代码中任何地方对 `myTaskArr` 的引用。

```js
assert.notMatch(code, /myTaskArr/);
```

你应该移除对 `"data"` 元素 `localStorage.getItem` 的任何引用。

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

你应该从代码中移除对 `localStorage.clear()` 的任何引用。

```js
assert.notMatch(code, /localStorage\.clear\(\s*\)\s*;?/);
```

你应该移除代码中任何地方对 `getTaskArrObj` 的引用。
