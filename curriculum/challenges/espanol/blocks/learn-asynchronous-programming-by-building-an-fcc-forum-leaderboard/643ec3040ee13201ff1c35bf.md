...
Mantén el segundo elemento `td` vacío porque más adelante añadirás contenido. 

En el tercer elemento `td`, suma la siguiente expresión incrustada: `${posts_count - 1}`.

Esto mostrará el número de respuestas al tema.

# --hints--

Debes tener `${posts_count - 1}` dentro del tercer elemento `td`.
