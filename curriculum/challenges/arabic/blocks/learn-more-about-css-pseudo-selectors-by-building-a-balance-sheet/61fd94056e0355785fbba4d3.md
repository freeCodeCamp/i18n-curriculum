...
داخل العنصر الثاني `tr`، أضف عنصر `th` بالنص `Expenses Annual anticipated expenses, such as payroll.`. لف ذلك النص، ما عدا `Expenses `، داخل عنصر `span` مع تعيين `class` إلى `description`.
