...
`parseCard` imethibitishwa na inafanya kazi ipasavyo. Kigezo cha `cardResult` hakihitajiki tena.

Anza kwa kufuta mstari wa `const cardResult` na mstari wa `console.log(cardResult)`. Kisha tengeneza kitendakazi kinachoitwa `parseCatalog` kinachopokea `rawCards` kama kigezo. Kwa sasa, tangaza safu ya data tupu iitwayo `catalog` ndani ya kitendakazi na irudishe. 

Chini ya kitendakazi, fanya wito wa `parseCatalog(rawCatalogCards)`, weka matokeo kwenye kigezo kinachoitwa `catalog`, na andika `catalog.length` kwenye konsoli.

# --hints--

`parseCatalog` inapaswa kuwa kitendakazi kinachorudisha safu ya data.

```js
assert.isArray(parseCatalog(rawCatalogCards));
```

`catalog` inapaswa kutangazwa na kuwekwa sawa na `parseCatalog(rawCatalogCards)`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+catalog\s*=\s*parseCatalog\s*\(/);
```

Unapaswa kufuta kigezo cha `cardResult` na wito wa `console.log()` unaohusiana nacho.
