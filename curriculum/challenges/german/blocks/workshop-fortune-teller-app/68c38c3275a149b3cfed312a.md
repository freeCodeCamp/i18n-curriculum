...
In diesem Workshop üben Sie weiterhin den Umgang mit TypeScript, indem Sie eine Wahrsage-App erstellen.

Beginnen Sie damit, ein Interface mit dem Namen `Card` zu deklarieren. Fügen Sie innerhalb des `Card`-Interfaces die folgenden Eigenschaften hinzu: `name`, `name_short` und `value`. Die Eigenschaften `name` und `name_short` sollten den Typ `string` haben, und `value` sollte einen Union-Typ mit `string | number` erhalten.

# --hints--

Sie sollten ein Interface mit dem Namen `Card` deklarieren.
...
Ihr `Card`-Interface sollte eine `name`-Eigenschaft vom Typ `string` haben.
...
Ihr `Card`-Interface sollte eine `name_short`-Eigenschaft vom Typ `string` haben.
...
Ihr `Card`-Interface sollte eine `value`-Eigenschaft mit einem Union-Typ aus `string` und `number` haben.
