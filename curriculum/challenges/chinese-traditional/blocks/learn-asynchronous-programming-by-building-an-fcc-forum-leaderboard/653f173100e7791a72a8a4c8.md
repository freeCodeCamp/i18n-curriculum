...
在上一個專案中，你使用 `.catch()` 方法來控制代碼錯誤。在這裡，你將改用 `try...catch` 述語。

`try` 區塊用來處理潛在錯誤，若發生錯誤，`catch` 區塊內的程式碼將會被執行。
...
在你的 `fetchData` 函式（程式）中，為你的 `try...catch` 述語添加一個 `catch` 區塊。`catch` 區塊應該有一個名為 `err` 的錯誤參數。

# --hints--

你應該在你的 `fetchData` 函式（程式）內有一個 `try` 區塊。

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}/);
```

你應該在你的 `try` 區塊後面加上 `catch` 區塊。

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}\s*catch\s*\(.*\)\s*{[^}]*\s*}/);
```

你的 `catch` 區塊應該有一個 `err` 參數。
