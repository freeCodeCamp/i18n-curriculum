...
Щоб почати свій CSS, нормалізуйте CSS-правила, націлюючись на всі елементи з `*`, включно з псевдо-селекторами `::before` і `::after`.

Встановіть властивості `padding` і `margin` обом у значення `0`, а властивість `box-sizing` — у значення `border-box`.

# --hints--

У вас має бути селектор `*, ::before, ::after`.

```js
assert(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after'));
```

Ваш селектор `*, ::before, ::after` має мати властивість `padding`, встановлену в `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.padding === '0px');
```

Ваш селектор `*, ::before, ::after` має мати властивість `margin`, встановлену в `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.margin === '0px');
```

Ваш селектор `*, ::before, ::after` має мати властивість `box-sizing`, встановлену в `border-box`.
