...
チェックボックスに印を付けるには、チェックボックスの`checked`に`input`属性を追加してください。現在の`includes`が`power`配列にあるかどうかを確認するには、`powers`メソッドを使います。

`onChange`は別の関数になりますが、今は`handlePowersChange`に設定してください。これは次のステップで修正するエラーにつながります。

# --hints--

現在の`checked`の値がpowers配列に存在するかどうかを反映するように、チェックボックスの`power`状態を設定してください。

```js
assert.match(code, /checked\s*=\s*\{\s*powers\s*\.\s*includes\s*\(\s*power\s*\)\s*\}/)
```

チェックボックスの`onChange`属性を`{handlePowersChange}`に設定してください。
