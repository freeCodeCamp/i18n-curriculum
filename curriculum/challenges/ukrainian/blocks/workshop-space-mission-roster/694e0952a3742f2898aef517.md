...
Центр управління місією потребує методу для копіювання астронавтів, які мають право на EVA, із екіпажу. Створіть порожню функцію з назвою `getEVAReadyCrew`, яка приймає параметр `crew`.

# --hints--

Ви повинні створити функцію з назвою `getEVAReadyCrew`.

```js
assert.isFunction(getEVAReadyCrew);
```

Ваша функція `getEVAReadyCrew` має мати параметр `crew`.
