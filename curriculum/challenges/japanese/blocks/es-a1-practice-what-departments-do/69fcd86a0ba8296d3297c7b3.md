...
このタスクでは、各部署が行う行動から部署を特定する練習をします。

# --instructions--

音声を聞いて、正しい部署名で各文を完成させてください。
...
この部署はチームのトレーニングを担当しています。

# --explanation--

`El departamento de aprendizaje y desarrollo capacita al equipo.` — Learning and Development 部署はチームのトレーニングを担当しています。

`apoyar`（支援する）と `capacitar`（トレーニングする）はどちらも三人称単数の `-ar` 動詞です。

以前のタスクの `desarrolla`、`verifica`、`crea` と同じパターンに従っています。
