...
少しビルらしく見えてきました。 ではここで初めての変数を作成しましょう。 In the previous lectures you learned that variable declarations begin with two dashes (`-`) and are given a name and a value like this: `--variable-name: value;`. `bb1` クラスのルール内に `--building-color1` という名前の変数を作成し、`#999` という値を代入してください。
