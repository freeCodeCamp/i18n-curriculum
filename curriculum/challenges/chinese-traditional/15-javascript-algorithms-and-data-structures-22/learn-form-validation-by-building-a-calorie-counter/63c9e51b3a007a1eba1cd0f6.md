...
現在聲明一個 `remainingCalories` 變量，並賦予它從 `budgetCalories` 中減去 `consumedCalories` 並加上 `exerciseCalories` 的值。

# --hints--

你的 `calculateCalories` 函數應該有一個 `remainingCalories` 變量。
...
你應該將 `budgetCalories - consumedCalories + exerciseCalories` 的值分配給 `remainingCalories`。
