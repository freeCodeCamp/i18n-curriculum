...
2.  `Bird` => Está `eat()` definido aqui? => Sim. Execute-o e pare de procurar.
3.  `Animal` => `eat()` também está definido, mas o JavaScript parou de procurar antes de alcançar esse nível.
4.  Objeto => O JavaScript parou de buscar antes de alcançar este nível.
