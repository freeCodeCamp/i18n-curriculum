...
Zentrieren Sie die `h1` und `p` Elemente, indem Sie ihnen eine `margin` von `1em auto` geben. Richten Sie dann auch deren Text im `center` aus.

# --hints--

Sie sollten einen durch Kommas getrennten Elementselektor verwenden, um die `h1` und `p` Elemente zu stylen.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

Sie sollten eine `margin` von `1em auto` verwenden, um die `h1` und `p` Elemente zu zentrieren.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

Sie sollten eine `text-align` von `center` verwenden, um den Text der `h1` und `p` zu zentrieren.
