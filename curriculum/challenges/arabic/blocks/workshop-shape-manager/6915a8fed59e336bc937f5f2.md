...
مجموعة العناصر الأخيرة التي يجب اختيارها هي عناصر النتيجة. استخدم دالة `getElement` الخاصة بك للحصول على العنصرين ذوي المعرفين `result-text` و `result-card`. يجب أن يستخدم كلاهما `HTMLElement` كنوع.

تذكر أنك قد أعلنت بالفعل عن المتغيرين `resultText` و `resultCard` لهما، لذا تحتاج فقط إلى تعيينهما.

# --hints--

يجب تعيين قيمة للمتغير `resultText`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

يجب أن يختار `resultText` العنصر `result-text`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

يجب استخدام `HTMLElement` كنوع للدالة `getElement` التي تستخدمها لاختيار العنصر `result-text`.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

يجب تعيين قيمة للمتغير `resultCard`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

يجب أن يختار `resultCard` العنصر `result-card`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

يجب استخدام `HTMLElement` كنوع للدالة `getElement` التي تستخدمها لاختيار العنصر `result-card`.
