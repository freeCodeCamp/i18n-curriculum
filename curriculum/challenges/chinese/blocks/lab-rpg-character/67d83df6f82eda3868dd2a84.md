...
当 `create_character` 被调用且第一个参数 **不是** 字串时，应返回 `The character name should be a string`。
...
当 `create_character` 被调用且第一个参数是字串时，**不应** 返回 `The character name should be a string`。
...
当 `create_character` 被调用且第一个参数是空字串时，它应返回 `The character should have a name`。
...
当 `create_character` 被调用且第一个参数 **不是** 空字串时，**不应** 返回 `The character should have a name`。
...
当 `create_character` 被调用且第一个参数长度超过 10 个字符时，应返回 `The character name is too long`。
...
当 `create_character` 函数的字符长度不超过 10 个字符时，**不应** 提示字符过长。
...
当 `create_character` 被调用且第一个参数*不包含*空格时，**不应** 返回 `The character name should not contain spaces`。
...
当 `create_character` 被调用且第二、第三或第四个参数 **不是** 整数时，应返回 `All stats should be integers`。
...
当 `create_character` 被调用且第二、第三和第四个参数均为整数时，**不应** 返回 `All stats should be integers`。
...
当 `create_character` 被调用且第二、第三和第四个参数都不小于 `1` 时，**不应** 返回 `All stats should be no less than 1`。
...
当 `create_character` 被调用且第二、第三或第四个参数大于 `4` 时，应返回 `All stats should be no more than 4`。
...
当 `create_character` 被调用且第二、第三和第四个参数都不超过 `4` 时，**不应** 返回 `All stats should be no more than 4`。
...
当 `create_character` 被调用且第二、第三或第四个参数**不**相加为 `7` 时，应返回 `The character should start with 7 points`。
...
当 `create_character` 被调用且第二、第三和第四个参数相加为 `7` 时，**不应** 返回 `The character should start with 7 points`。
