...
Logo depois de limpar `multipleCard`, dentro do método `singleCardSelected`, faça a chamada da função `showElements` e passe `singleCard` como primeiro argumento e `fortuneContainer` como segundo argumento.

# --hints--

Você deve fazer a chamada da função `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

Você deve passar `elements.singleCard` como primeiro argumento.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

Você deve passar `elements.fortuneContainer` como segundo argumento.
