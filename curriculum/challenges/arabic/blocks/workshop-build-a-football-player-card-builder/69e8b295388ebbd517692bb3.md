...
عنصر `div` الذي يحمل الصنف `card-wrapper` يُنشئ الحدود الخارجية والشكل المقصوص للبطاقة.

أضف الآن عنصر `div` الداخلي مباشرة داخل `card-wrapper` مع `className` بقيمة `card`. هذا يمنح البطاقة خلفيتها الداكنة.

# --hints--

يجب إضافة عنصر `div` مع `className` بقيمة `card` داخل عنصر `.card-wrapper`.
