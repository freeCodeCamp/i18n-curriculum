...
Du solltest einen `tbody td`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('tbody td'));
```

Dein `tbody td`-Selektor sollte eine `width`-Eigenschaft von `100vw` haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('tbody td')?.getPropertyValue('width') === '100vw');
```

Dein `tbody td`-Selektor sollte eine `min-width`-Eigenschaft von `4rem` haben.
