...
Como feito para o `multipleCard` anterior, obtenha novamente o elemento `multipleCard` e atribua seu `innerHTML` iterando pelo `DrawingType`. Use `Object.values()` e chame o método `map` nele, passando `type` como argumento.

# --hints--

Você deve atribuir `multipleCard.innerHTML` chamando `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

Você deve passar `DrawingType` para o método `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

Você deve chamar o método `map` em `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Seu método `map` deve ter um parâmetro chamado `type`.
