...
为了将你所学到的一切整合在一起，请更新你的 `dearRegex` 以将元音映射到相应的数字（请注意，`i` 应该匹配 `1`，并且还要匹配管道符号 `|`），并匹配整个单词。

这样，你的垃圾邮件过滤项目就完成了。

# --hints--

你的 `dearRegex` 应该使用字符类来匹配 `e` 或 `3`。

```js
assert.match(dearRegex.source, /\[(e3|3e)\]/);
```

你的 `dearRegex` 应该使用字符类来匹配 `a`、`@` 或 `4`。

```js
assert.match(dearRegex.source, /\[(a@4|a4@|4a@|4@a|@a4|@4a)\]/);
```

你的 `dearRegex` 应该使用字符类来匹配 `i`、`1` 或 `|`。

```js
assert.match(dearRegex.source, /\[(i1\||i\|1|1i\||1\|i|\|1i|\|i1)\]/);
```

你的 `dearRegex` 应该使用非捕获组。

```js
assert.match(dearRegex.source, /\(\?:/);
```

你的 `dearRegex` 应该使用非捕获组来匹配 `\s` 或 `^`。

```js
assert.match(dearRegex.source, /\(\?:(\^\|\\s|\\s\|\^)\)/);
```

你的 `dearRegex` 应该使用第二个非捕获组。
...
你的 `dearRegex` 应该与 `dear friend` 匹配。

```js
assert.match('dear friend', dearRegex);
```

你的 `dearRegex` 应该与 `d34r fr13nd` 匹配。

```js
assert.match('d34r fr13nd', dearRegex);
```

你的 `dearRegex` 应该与 `d3@r fr|3nd` 匹配。

```js
assert.match('d3@r fr|3nd', dearRegex);
```

你的 `dearRegex` 应该与 `my dear friend Naomi` 匹配。

```js
assert.match('my dear friend Naomi', dearRegex);
```

你的 `dearRegex` 应该与 `dear friend Naomi` 匹配。

```js
assert.match('dear friend Naomi', dearRegex);
```

你的 `dearRegex` 应该与 `my dear friend` 匹配。

```js
assert.match('my dear friend', dearRegex);
```

你的 `dearRegex` 不应匹配 `non-dear friend`。

```js
assert.notMatch('non-dear friend', dearRegex);
```

你的 `dearRegex` 不应匹配 `dear friend-o`。
