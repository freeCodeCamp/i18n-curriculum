...
Imposta il `.cabin` in modo che abbia una proprietà `transform-origin` pari a `50% 0%`. Questo imposterà il punto di origine con un offset di `50%` da sinistra e `0%` dall'alto, posizionandolo al centro del bordo superiore dell'elemento.
