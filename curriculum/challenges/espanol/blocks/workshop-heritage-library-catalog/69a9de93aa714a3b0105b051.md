...
Ahora combina todas las filas en una cadena CSV que puedas exportar o compartir.

Después del primer ciclo, declara `let csv = header`. Añade otro ciclo `for` para agregar cada fila a `csv` con un salto de línea: `csv = csv + "\n" + rows[i]`. Devuelve `csv`. Luego, debajo de la función, registra `exportToCSV(catalog)` para previsualizar la salida formateada.

# --hints--

`exportToCSV` debe devolver una cadena.

```js
assert.isString(exportToCSV(catalog));
```

La primera línea de la cadena devuelta debe ser `"Title,Author,Year,Location"`.

```js
assert.strictEqual(exportToCSV(catalog).split("\n")[0], "Title,Author,Year,Location");
```

La salida debe contener `"King, Stephen"` (entre comillas para manejar la coma).

```js
assert.include(exportToCSV(catalog), '"King, Stephen"');
```

Debes registrar `exportToCSV(catalog)` en la consola.
