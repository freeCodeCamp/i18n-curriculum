...
给 `.foreground-buildings` 添加与 `.background-buildings` 相同的 `display`、`align-items` 和 `justify-content` 属性和值。 同样，这将使用 Flexbox 将建筑物均匀地分布在其容器底部。

# --hints--

你应该给 `.foreground-buildings` 添加一个值为 `flex` 的 `display` 属性。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.display, 'flex');
```

你应该给 `.foreground-buildings` 添加一个值为 `flex-end` 的 `align-items` 属性。
