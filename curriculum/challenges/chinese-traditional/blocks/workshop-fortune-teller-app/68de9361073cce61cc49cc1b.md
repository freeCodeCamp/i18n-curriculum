...
在之前的課程中，你已經學過型別推論是 TypeScript 根據使用方式和背景關係，自動判斷並指定變數、函式回傳值和運算式型別的能力。

在 TypeScript 中，泛型讓函式、類別和型別更具彈性。泛型允許建立型別變數，省去每次都必須明確定義型別的麻煩。這使得同一個函式能安全地處理不同型別。
...
函式可以使用兩個或更多泛型。慣例上使用 `T`，並且可以根據需要添加像 `S` 這樣的額外型別變數。
...
創建一個名為 `getElement` 的函式，該函式接受一個型別為 `string` 的 `selector` 參數。它還應該接受一個繼承自 `HTMLElement` 的泛型型別 `T`，並且返回型別為 `T`。

# --hints--

你應該有一個名為 `getElement` 的箭頭函式。
...
你的 `getElement` 函式應該接受一個名為 `selector`、型別為 `string` 的參數。
