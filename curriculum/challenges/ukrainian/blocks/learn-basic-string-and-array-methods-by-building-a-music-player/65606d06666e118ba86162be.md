...
Додайте другий об’єкт із такими ключами та значеннями:
...
Ваш масив `allSongs` має містити два значення.

```js
assert.equal(allSongs.length, 2);
```

Другим значенням у вашому масиві `allSongs` має бути об’єкт.

```js
assert.isObject(allSongs[1]);
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `id`, встановлену в число `1`.

```js
assert.equal(allSongs[1].id, 1);
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `title`, встановлену в рядок `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `artist`, встановлену в рядок `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `duration`, встановлену в рядок `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `src`, встановлену в рядок `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
