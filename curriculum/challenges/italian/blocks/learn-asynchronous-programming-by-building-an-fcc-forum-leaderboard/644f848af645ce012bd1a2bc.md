...
Crea una costante chiamata `linkText` e assegna a essa il valore di `selectedCategory.category`. Questo mostrerà il nome della categoria nell'elemento àncora.

# --hints--

Dovresti avere una costante chiamata `linkText`.

```js
assert(code.match(/const\s+linkText\s*=/));
```

Dovresti assegnare `selectedCategory.category` alla tua variabile `linkText`.
