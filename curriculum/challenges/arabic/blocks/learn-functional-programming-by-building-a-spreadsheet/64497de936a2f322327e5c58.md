...
الآن عليك التحقق مما إذا كان `value` لا يتضمن `id` الخاص بالعنصر. أنشئ شرط `if` لذلك.
...
يجب أن يتحقق شرط `if` من أن `value` لا يتضمن `id` الخاص بـ`element`.
