...
Sie müssen die Buttons in Ihrer `addToCartBtns`-Variablen durchlaufen. Allerdings gibt `.getElementsByClassName()` eine Collection zurück, die keine `forEach`-Methode besitzt.

Verwenden Sie den Spread-Operator auf der `addToCartBtns`-Variablen, um sie in ein Array umzuwandeln. Verwenden Sie dann die `forEach`-Methode, um das Array zu durchlaufen. Übergeben Sie noch keine Callback-Funktion.

# --hints--

Sie sollten den Spread-Operator auf der `addToCartBtns`-Variablen verwenden.

```js
assert.match(code, /\.\.\.addToCartBtns/);
```

Sie sollten die `addToCartBtns`-Variable in ein Array spreaden.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]/);
```

Sie sollten die `forEach`-Methode auf das erstellte Array anwenden.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(/);
```

Sie sollten der `forEach`-Methode keine Callback-Funktion übergeben.
