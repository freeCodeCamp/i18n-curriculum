...
入力された数値が負でないことを検証する関数が必要です。

そのために、型が`handleInput`の`e`パラメータを持つ`Event`関数を作成してください。その中で、targetの値を数値に変換し、`value`キーワードを使って`HTMLInputElement`型にキャストした`as`変数を作成してください。

# --hints--

`handleInput`関数を作成してください。

```js
assert.isFunction(handleInput)
```

`handleInput`関数には、型が`e`の`Event`パラメータが必要です。
