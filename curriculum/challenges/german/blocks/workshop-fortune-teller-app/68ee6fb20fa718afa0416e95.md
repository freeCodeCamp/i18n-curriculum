...
Wie beim vorherigen `multipleCard` holen Sie erneut das `multipleCard`-Element und setzen dessen `innerHTML`, indem Sie durch `DrawingType` iterieren. Verwenden Sie `Object.values()` und rufen Sie die `map`-Methode darauf auf, wobei Sie `type` als Argument übergeben.

# --hints--

Sie sollten `multipleCard.innerHTML` zuweisen, indem Sie `Object.values` aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

Sie sollten `DrawingType` an die `Object.values`-Methode übergeben.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

Sie sollten die `map`-Methode auf `Object.values` aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Ihre `map`-Methode sollte einen Parameter namens `type` haben.
