...
給 `.bb2b` 添加一個 `linear-gradient`，從 `0%` 到 `6%` 處使用 `--building-color2`，從 `6%` 到 `9%` 處使用 `--window-color2`。
...
你應該在從 `0%` 到 `6%` 處使用 `--building-color2`。
...
`.bb2b` 應該有一個 `linear-gradient`，從 `0%` 到 `6%` 處爲 `--building-color2`，過渡到從 `6%` 到 `9%` 處爲 `--window-color2`。
