...
- **`Object.hasOwn()` 方法**：这是现代且推荐的查看对象是否拥有某个自身（非继承）属性的方式。语法是 `Object.hasOwn(object, propertyName)`。如果属性存在于对象上，则返回 `true`，如果不存在，则返回 `false`——无论属性值为何。这使得它比 `hasOwnProperty()` 更安全，也比在值可能为 `0`、`false`、`null` 或 `undefined` 时使用 `if (obj.prop)` 更可靠。
