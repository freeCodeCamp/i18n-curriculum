...
Después de la sentencia `if` en tu método `__str__`, crea dos variables llamadas `movies` y `series` y asígnales una llamada al método `get_movies` y una llamada al método `get_tv_series`, respectivamente.

# --hints--

Debes tener una variable llamada `movies` en tu método `__str__`.
...
Asignas `self.get_movies()` a tu variable `movies`.
...
Debes tener una variable llamada `series` en tu método `__str__`.
...
Asignas `self.get_tv_series()` a tu variable `series`.
