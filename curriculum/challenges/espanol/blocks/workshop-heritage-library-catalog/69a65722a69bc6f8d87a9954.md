...
Terminemos con un resumen. Registra `catalog.length` y `Object.keys(byDecade).length` para ver el total de libros y los grupos por década.

Declara `let oldestYear = Infinity` y `let newestYear = 0`. Usa un ciclo `for` sobre `catalog`: para cada entrada donde el año no sea `"Unknown"`, actualiza `oldestYear` si el año es menor y `newestYear` si es mayor. Registra ambos valores.

¡Has completado el taller del catálogo de la biblioteca Heritage!

# --hints--

Debes registrar `catalog.length` en la consola.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*catalog\.length\s*\)/);
```

Debes registrar `Object.keys(byDecade).length` en la consola.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*Object\.keys\s*\(\s*byDecade\s*\)\.length\s*\)/);
```

Debes usar un ciclo `for` sobre `catalog` para encontrar los años más antiguo y más reciente.

```js
assert.match(__helpers.removeJSComments(code), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

El año más antiguo en el catálogo debe ser `850`.

```js
assert.strictEqual(oldestYear, 850);
```

El año más reciente en el catálogo debe ser `2011`.

```js
assert.strictEqual(newestYear, 2011);
```

Debes registrar `oldestYear` en la consola.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*oldestYear\s*\)/);
```

Debes registrar `newestYear` en la consola.
