...
最後に、送信ボタンにホバーエフェクトを追加して、ユーザーのフィードバックを向上させましょう。

`.submit-btn:hover` を選択し、その `background-color` を `midnightblue` に設定してください。

ワークショップの修了おめでとうございます！

# --hints--

`.submit-btn:hover` 疑似クラスをターゲットにしてください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.submit-btn:hover'));
```

`background-color` プロパティを `midnightblue` に設定してください。
