...
카드의 기본 이미지와 API 요청을 위한 URL이 제공되었으니, 이를 저장할 변수를 몇 개만 만들면 됩니다.

`CDN_URL` 변수를 만들고 값을 `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`로 설정하세요. 또한 `defaultImg` 변수를 만들고 값은 `new URL("default.svg", CDN_URL + "/")`로, `LOCAL_DEFAULT_IMG` 변수는 `defaultImg` 값으로 설정하세요.

# --hints--

`CDN_URL`라는 변수를 만들어야 합니다.
...
`CDN_URL` 변수를 `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"`로 설정해야 합니다.
...
`defaultImg` 변수는 `new URL("default.svg", CDN_URL + "/")`로 설정해야 합니다.
...
`LOCAL_DEFAULT_IMG`라는 변수를 만들어야 합니다.
...
`LOCAL_DEFAULT_IMG` 변수를 `defaultImg`로 설정해야 합니다.
