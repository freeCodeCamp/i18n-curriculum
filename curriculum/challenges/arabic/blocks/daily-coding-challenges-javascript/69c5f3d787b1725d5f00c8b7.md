...
أعطِ سلسلة نصية تمثل اختصارًا، أعد الاسم الكامل للمنظمة التي ينتمي إليها من القائمة أدناه:
...
يجب أن تطابق كل حرف في الاختصار المعطى الحرف الأول من كل كلمة في المنظمة التي ينتمي إليها، بنفس الترتيب.

# --hints--

`findOrg("NASA")` يجب أن تُعيد `"National Avocado Storage Authority"`.

```js
assert.equal(findOrg("NASA"), "National Avocado Storage Authority");
```

`findOrg("CIA")` يجب أن تُعيد `"Cats Infiltration Agency"`.

```js
assert.equal(findOrg("CIA"), "Cats Infiltration Agency");
```

`findOrg("FBI")` يجب أن تُعيد `"Fluffy Beanbag Inspectors"`.

```js
assert.equal(findOrg("FBI"), "Fluffy Beanbag Inspectors");
```

`findOrg("DOJ")` يجب أن تُعيد `"Department Of Jelly"`.

```js
assert.equal(findOrg("DOJ"), "Department Of Jelly");
```

`findOrg("WHO")` يجب أن تُعيد `"Wild Honey Organization"`.

```js
assert.equal(findOrg("WHO"), "Wild Honey Organization");
```

`findOrg("EPA")` يجب أن تُعيد `"Eating Pancakes Administration"`.
