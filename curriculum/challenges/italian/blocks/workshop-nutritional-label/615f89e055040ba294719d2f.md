...
Crea un altro elemento `p` sotto il tuo grande divisore. Dai all'elemento `p` il testo `Vitamin D 2mcg 10%`.

L'elemento `p` contiene solo testo, puoi racchiudere la percentuale in un elemento `span` in modo che sia considerata un'entità separata dal resto del testo e venga spostata a destra.
...
Il tuo elemento `span` non dovrebbe essere in grassetto.
