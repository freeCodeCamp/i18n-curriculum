...
Il selettore `body` dovrebbe avere una proprietà `font-family` impostata su `Baskervville` con un fallback di `serif`.
...
Il selettore `body` dovrebbe avere una proprietà `color` impostata su `linen`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.color === 'linen');
```

Il selettore `body` dovrebbe avere una proprietà `background-color` impostata a `rgb(20, 30, 40)`.
