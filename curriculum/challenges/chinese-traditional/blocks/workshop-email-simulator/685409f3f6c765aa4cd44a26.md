...
在寄送電子郵件之前，為你的程式碼添加標準的 Python 慣用語 `if __name__ == '__main__':`，接著呼叫 `main()`。這可確保主函式只在腳本被直接執行時執行，而非被匯入為模組時。

# --hints--

你應該在 `main` 函式外面添加 `if __name__ == '__main__':` 檢查。
...
你應該在 `if __name__ == '__main__':` 區塊內呼叫 `main()`。
