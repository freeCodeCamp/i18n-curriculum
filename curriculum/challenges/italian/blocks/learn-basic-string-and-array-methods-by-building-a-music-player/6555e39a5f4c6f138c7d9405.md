...
Nei passaggi precedenti, hai imparato a lavorare con il metodo `sort()` per ordinare le canzoni in ordine alfabetico. Un altro caso d’uso della funzione di callback è randomizzare un array.

Un modo per randomizzare un array di elementi sarebbe sottrarre `0.5` da `Math.random()`, il che produce valori casuali che possono essere positivi o negativi. Questo rende il risultato del confronto una combinazione di valori positivi e negativi, portando a un ordinamento casuale degli elementi.
...
Usa il metodo `sort()` sull’array `userData?.songs`. Passa una funzione di callback al metodo e restituisci il risultato di `Math.random() - 0.5`.


# --hints--

Non dovresti modificare la funzione `shuffle` esistente.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

Dovresti usare il metodo `sort()` su `userData?.songs`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

Il tuo metodo `sort()` dovrebbe avere una funzione di callback che usa la sintassi arrow.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

La callback del tuo metodo `sort()` dovrebbe restituire `Math.random() - 0.5`.
