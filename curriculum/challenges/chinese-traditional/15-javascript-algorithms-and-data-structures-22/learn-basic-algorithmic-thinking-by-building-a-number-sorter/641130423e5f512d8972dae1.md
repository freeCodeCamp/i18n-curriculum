...
注意數字 `10` 如何放置在數組的開始位置。 這是因爲 `.sort()` 的默認行爲是將數字值轉換爲字符串，並按字母順序排序。 而字母順序上，`"10"` 在 `"2"` 前面。

爲了解決這個問題，你可以將回調函數傳遞到 `.sort()` 方法。 回調函數有兩個參數 - 對於你的函數，使用 `a` 和 `b`。 `a` 和 `b` 的參數表示數組中將要排序的數字值。

函數體暫時保留爲空。

# --hints--

你應該將回調函數傳遞給 `.sort()` 方法。 記得使用箭頭語法。
