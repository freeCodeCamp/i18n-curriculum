...
Para garantir que a exibição do jogador seja atualizada sempre que uma nova música começar a tocar, chame a função `setPlayerDisplay()` dentro da função `playSong()`.

Agora você deve ver o título da música e o artista aparecerem no display.

# --hints--

Você deve chamar a função `setPlayerDisplay` dentro da sua função `playSong`.
