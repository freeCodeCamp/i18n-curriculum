...
`playNextSong` 함수 안에서 플레이리스트가 끝날 때 플레이어를 올바르게 업데이트하도록 `setPlayerDisplay`, `highlightCurrentSong`, `setPlayButtonAccessibleText` 함수를 호출하세요.

# --hints--

`setPlayerDisplay` 함수의 `else` 문 안에서 `playNextSong`를 호출해야 합니다.
...
`highlightCurrentSong` 함수의 `else` 문 안에서 `playNextSong`를 호출해야 합니다.
...
`setPlayButtonAccessibleText` 함수의 `else` 문 안에서 `playNextSong`를 호출해야 합니다.
