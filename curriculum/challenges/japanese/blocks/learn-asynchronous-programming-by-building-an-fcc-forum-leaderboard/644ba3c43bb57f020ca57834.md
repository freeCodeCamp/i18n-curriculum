...
`className` オブジェクトに `category` と `selectedCategory` のプロパティを追加する必要があります。

まず `className` 変数に `selectedCategory.className` を代入してください。次に `category` 変数に `selectedCategory.category` を代入してください。

# --hints--

ドット記法を使って `className` オブジェクトの `selectedCategory` プロパティを指定してください。

```js
assert.match(code, /selectedCategory\.className/);
```

`className` オブジェクトの `allCategories` プロパティを `className` オブジェクトの `selectedCategory` プロパティに代入してください。

```js
assert.match(code, /selectedCategory\.className\s*=\s*className/);
```

ドット記法を使って `category` オブジェクトの `selectedCategory` プロパティを指定してください。

```js
assert.match(code, /selectedCategory\.category/);
```

`category` オブジェクトの `allCategories` プロパティを `category` オブジェクトの `selectedCategory` プロパティに代入してください。
