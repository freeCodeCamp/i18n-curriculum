...
Jetzt sollten Sie das Ergebnis für jeden erreichbaren Knoten ausgeben.

Fügen Sie nach der `path`-Variable einen `console.log()`-Aufruf mit einem Template-Literal hinzu, das Folgendes ausgibt:

```md
\n{startNode}-{nodeNo} distance: {distances[nodeNo]}\nPath: {path}
```

Zum Beispiel, wenn `startNode` `0` ist und `nodeNo` `3` ist, sollte die Ausgabe so aussehen:
...
Sie sollten eine `console.log()` innerhalb der `for...of`-Schleife haben.
...
Die `console.log()` sollte ein Template-Literal verwenden, das `${startNode}-${nodeNo} distance: ${distances[nodeNo]}` enthält.
...
Das Template-Literal sollte auch `${path}` enthalten.
