...
Fornisci una funzione per trovare i due punti più vicini tra un insieme di punti dati in due dimensioni.

La soluzione semplice è un algoritmo $O(n^2)$ (che possiamo chiamare *algoritmo a forza bruta*); il pseudo-codice (usando gli indici) potrebbe essere semplicemente:
...
Un algoritmo migliore si basa sull'approccio ricorsivo divide et impera, che è $O(n\log n)$; un pseudo-codice potrebbe essere:

<pre><strong>closestPair</strong> di (xP, yP)
  dove xP è P(1) .. P(N) ordinato per coordinata x, e
  yP è P(1) .. P(N) ordinato per coordinata y (ordine crescente)
<strong>if</strong> N ≤ 3 <strong>then</strong>
  <strong>return</strong> punti più vicini di xP usando l'algoritmo a forza bruta
<strong>else</strong>
  xL ← punti di xP da 1 a ⌈N/2⌉
  xR ← punti di xP da ⌈N/2⌉+1 a N
  xm ← xP(⌈N/2⌉)<sub>x</sub>
  yL ← { p ∈ yP : p<sub>x</sub> ≤ xm }
  yR ← { p ∈ yP : p<sub>x</sub> > xm }
  (dL, pairL) ← closestPair di (xL, yL)
  (dR, pairR) ← closestPair di (xR, yR)
  (dmin, pairMin) ← (dR, pairR)
  <strong>if</strong> dL &#x3C; dR <strong>then</strong>
    (dmin, pairMin) ← (dL, pairL)
  <strong>endif</strong>
  yS ← { p ∈ yP : |xm - p<sub>x</sub>| &#x3C; dmin }
  nS ← numero di punti in yS
  (closest, closestPair) ← (dmin, pairMin)
  <strong>for</strong> i <strong>da</strong> 1 <strong>a</strong> nS - 1
    k ← i + 1
    <strong>while</strong> k ≤ nS <strong>e</strong> yS(k)<sub>y</sub> - yS(i)<sub>y</sub> &#x3C; dmin
...
Per l'input, aspetta che l'argomento sia un array di oggetti `Point` con membri `x` e `y` impostati a numeri. Restituisci un oggetto contenente le coppie chiave:valore per `distance` e `pair` (la coppia dei due punti più vicini).

Per esempio `getClosestPair` con array di input `points`:
...
**Nota:** Ordina l'array `pair` in base ai loro valori `x` in ordine crescente.
