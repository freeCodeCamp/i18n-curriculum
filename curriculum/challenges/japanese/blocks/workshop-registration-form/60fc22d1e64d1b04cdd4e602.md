...
`.inline` 要素と `label` テキストの間にスペースを追加してください。`margin` の右 `0.5em` を設定します。また、他のすべてのマージンは `0` に設定してください。

# --hints--

`.inline` 要素に `margin-top` の `0` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginTop, '0px');
```

`.inline` 要素に `margin-right` の `0.5em` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginRight, '0.5em');
```

`.inline` 要素に `margin-bottom` の `0` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginBottom, '0px');
```

`.inline` 要素に `margin-left` の `0` を設定してください。
