...
Unda kipengele cha `button` chenye `className` ya `submit-btn` na `type` ya `submit`. Toa kitufe hicho maandishi ya `Join the League`.

Baada ya kufanya hivyo, jaribu wasilisha fomu na uone kinachotokea.
...
Kipengele chako cha kitufe kinapaswa kuwa na sifa ya kipengele ya `className` iliyowekwa kuwa `submit-btn`.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

Kipengele chako cha `button` kinapaswa kuwa na `Join the League` kama maandishi yake.
