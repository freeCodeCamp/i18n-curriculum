...
حتى الآن، لديك ثلاث دوال: `getElement`، و`hideElements`، و`showElements`.
الآن تحتاج إلى دالة تساعدك في الحصول على حظ عشوائي.

أعلن دالة باسم `getRandomItem` تأخذ مصفوفة من `items` كمعلمة. استخدم الأنواع العامة لهذه الدالة.

داخل الدالة، استخدم `Math.random()` و `Math.floor()` لاختيار حظ عشوائي من مصفوفة `items`.
لا تنسَ إرجاع الحظ المختار.

# --hints--

يجب أن تعلن دالة باسم `getRandomItem`.
...
يجب أن تستخدم دالة `getRandomItem` معلمة النوع العامة `T`.
...
يجب أن تحتوي دالة `getRandomItem` على معلمة باسم `items` من النوع `T[]`.
...
يجب أن يكون نوع الإرجاع في دالة `getRandomItem` هو `T`.
...
يجب أن تعلن متغيرًا باسم `index` داخل دالة `getRandomItem`.
...
يجب أن يستخدم المتغير `index` دالتي `Math.floor()` و`Math.random()` مع `items.length` لإنتاج فهرس عشوائي.
...
يجب أن تُرجع دالة `getRandomItem` القيمة `items[index]`.
