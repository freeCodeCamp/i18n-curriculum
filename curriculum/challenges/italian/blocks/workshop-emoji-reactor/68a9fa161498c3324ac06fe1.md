...
È ora di iniziare a lavorare sugli altri pulsanti. Gli elementi `button` rimanenti sono stati aggiunti per te nel file HTML.

Per ogni nuovo pulsante (`#confused-btn`, `#sad-btn` e `#loving-btn`), selezionalo usando `querySelector` con il suo ID, poi aggiungi un listener per l'evento click che chiama `updateCount` con quel pulsante.

Usa i nomi delle variabili `confusedBtn`, `sadBtn`, `lovingBtn`.

# --hints--

Dovresti creare una variabile chiamata `confusedBtn`.

```js
assert.exists(confusedBtn);
```

Dovresti memorizzare l'elemento `#confused-btn` nella variabile `confusedBtn`.

```js
assert.equal(confusedBtn, document.querySelector('#confused-btn'));
```

Dovresti creare una variabile chiamata `sadBtn`.

```js
assert.exists(sadBtn);
```

Dovresti memorizzare l'elemento `#sad-btn` nella variabile `sadBtn`.

```js
assert.equal(sadBtn, document.querySelector('#sad-btn'));
```

Dovresti creare una variabile chiamata `lovingBtn`.

```js
assert.exists(lovingBtn);
```

Dovresti memorizzare l'elemento `#loving-btn` nella variabile `lovingBtn`.

```js
assert.equal(lovingBtn, document.querySelector('#loving-btn'));
```

L'elemento `#confused-btn` dovrebbe avere un listener per l'evento click che chiama `updateCount`.
...
L'elemento `#sad-btn` dovrebbe avere un listener per l'evento click che chiama `updateCount`.
...
L'elemento `#loving-btn` dovrebbe avere un listener per l'evento click che chiama `updateCount`.
