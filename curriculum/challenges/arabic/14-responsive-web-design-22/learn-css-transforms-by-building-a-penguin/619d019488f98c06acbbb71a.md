...
لإصلاح هذا، أضف `class` من `left` إلى أول عنصر `.face`، و`class` من `right` إلى الثاني عنصر `.face`.

# --hints--

يجب عليك إعطاء `class` من `left` لعنصر `.face` الأول.

```js
assert.include(document.querySelector('.face:nth-of-type(1)').className, 'left');
```

يجب عليك إعطاء `class` من `right` لعنصر `.face` الثاني.
