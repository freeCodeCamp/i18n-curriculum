...
Ваша електронна таблиця тепер функціональна. Однак ви не підтримуєте дуже багато формул.

Додайте властивість `even` до своій `spreadsheetFunctions`. Вона має приймати параметр `nums` та повернути результат фільтрації масиву `nums`, щоб включати лише парні числа. Скористайтеся посиланням на функцію `isEven` для допомоги.
...
Передайте посилання на функцію `isEven()` як зворотний виклик для методу `.filter()`.

```js
assert.match(code, /even\s*:\s*(\(\s*nums\s*\)|nums)\s*=>\s*nums\s*\.\s*filter\s*\(\s*isEven\s*\)/)
```

Функція `even` має повернути масив парних чисел.
