...
Declare uma variável chamada `cardId` e atribua a ela o valor do atributo `data-id` de `cardElement` usando o método `getAttribute()`.

# --hints--

Você deve declarar uma variável chamada `cardId` dentro de `showFortune`.
...
Sua variável `cardId` deve ser atribuída chamando `cardElement.getAttribute("data-id")`.
...
Você deve usar o método `getAttribute` em `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

Você deve passar `'data-id'` para `getAttribute`.
