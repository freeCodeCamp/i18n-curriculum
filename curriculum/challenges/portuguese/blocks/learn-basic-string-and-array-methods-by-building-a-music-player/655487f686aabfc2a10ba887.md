...
Antes de começar a trabalhar em tocar a próxima e a música anterior, você precisa obter o índice de cada música na propriedade `songs` de `userData`. 

Comece criando uma função arrow chamada `getCurrentSongIndex`.

# --hints--

Você deve ter uma função `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Sua função `getCurrentSongIndex` deve usar a sintaxe de arrow.
