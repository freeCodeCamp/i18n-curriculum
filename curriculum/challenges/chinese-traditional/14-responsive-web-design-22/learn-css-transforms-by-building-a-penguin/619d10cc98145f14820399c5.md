...
在 `.blush.right` 元素下，添加兩個 `div` 元素，每個元素都包含 `beak` `class`。 Also, give the first `.beak` element a `class` of `top`, and the second `.beak` element a `class` of `bottom`.

# --hints--

應該在 `.penguin-head` 中添加兩個 `div` 元素。 應該是 `--fcc-expected--` `div` 元素，而不是 `--fcc-actual--`。

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 9);
```

第一個新的 `div` 應該設置 `class` `beak`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

第二個新的 `div` 應該設置 `class` `beak`。

```js
assert.equal(document.querySelectorAll('.penguin-head > div.beak')?.length, 2);
```

第一個新的 `div` 應該設置 `class` `top`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

第二個新的 `div` 應該設置 `class` `bottom`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak.bottom'));
```

應該將 `div.beak.top` 放在 `div.blush.right` 之後。

```js
assert.exists(document.querySelector('.blush.right + .beak.top'));
```

應該將 `div.beak.bottom` 放在 `div.beak.top` 之後。
