...
ミッションコントロールでは、乗組員の概要を記録するための関数がもう1つ必要です。`printCrewSummary` という名前の空の関数を作成し、`crew` パラメータを受け取るようにしてください。

# --hints--

`printCrewSummary` という名前の関数を作成してください。

```js
assert.isFunction(printCrewSummary);
```

`printCrewSummary` 関数は `crew` パラメータを持つ必要があります。
