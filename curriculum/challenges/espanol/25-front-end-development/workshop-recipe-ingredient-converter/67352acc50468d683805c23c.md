...
Ahora, siguiendo la sintaxis de arriba, actualice su función `convertQuantity` para devolver una segunda función. Esta función interna debe aceptar un argumento `toUnit` y debe estar vacía.

# --hints--

Su función `convertQuantity` debe devolver una función.

```js
assert.isFunction(convertQuantity());
```

La función devuelta debe tener un parámetro `toUnit`.

```js
assert.match(convertQuantity()?.toString(), /\(toUnit\)/);
```

La función devuelta debe estar vacía.
