...
Successivamente, aggiungi una chiave `medications` al dizionario `constraints`. Per il suo valore usa `isinstance` per verificare che `medications` sia una lista.

# --hints--

Il tuo dizionario `constraints` dovrebbe avere una chiave `medications`.
...
La chiave `medications` del tuo dizionario `constraints` dovrebbe avere il valore `isinstance(medications, list)`.
