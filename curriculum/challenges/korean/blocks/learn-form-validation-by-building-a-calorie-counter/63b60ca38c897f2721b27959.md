...
결과를 표시할 공간이 필요합니다. 빈 `div` 요소를 추가하고 `id`을 `output`로, `class` 값을 `output`와 `hide`로 지정하세요.

# --hints--

`div` 다음에 `form` 요소를 추가하세요.

```js
assert.exists(document.querySelector('.container > div'));
```

새로운 `div` 요소에 `id`를 `output`으로 설정해야 합니다.

```js
assert.equal(document.querySelector('.container > div')?.id, 'output');
```

새로운 `div` 요소에 `class` 값을 `output`과 `hide`로 지정해야 합니다.
