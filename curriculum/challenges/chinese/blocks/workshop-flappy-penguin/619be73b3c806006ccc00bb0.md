...
将伪元素的 `width` 设置为父元素的一半，`height` 设置为 `45%`，`background-color` 设置为 `gray`。

# --hints--

你应该将 `.penguin-body::before` 的 `width` 设置为 `50%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.width, '50%');
```

你应该将 `.penguin-body::before` 的 `height` 设置为 `45%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.height, '45%');
```

你应该将 `.penguin-body::before` 的 `background-color` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
