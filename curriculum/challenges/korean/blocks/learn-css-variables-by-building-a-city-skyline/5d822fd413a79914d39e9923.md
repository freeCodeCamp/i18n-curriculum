...
`background-color`에서 `.fb3` 속성과 값을 제거하고 `.fb3a`와 `.fb3b`에 추가하세요.

# --hints--

`background-color`에서 `.fb3`를 제거하세요.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

`.fb3a`에 `background-color` 속성으로 `--building-color1`를 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

`.fb3b`에 `background-color` 속성으로 `--building-color1`를 지정하세요.
