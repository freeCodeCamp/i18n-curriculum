...
الآن ستبدأ ببناء النموذج. داخل `div` الذي يحمل `className` بقيمة `form-panel`، أضف `div` لتغليف قسم "Player Info". داخل هذا الـ `div` الجديد، أضف `p` مع `className` بقيمة `form-section-title` والنص `Player Info`.

ستستمر في إضافة حقول إلى هذا القسم في الخطوات التالية.

# --hints--

يجب أن يحتوي `div.form-panel` على `div` لتغليف قسم Player Info.
...
يجب أن يحتوي `div` داخل `div.form-panel` على `p` مع `className` بقيمة `form-section-title` والنص `Player Info`.
