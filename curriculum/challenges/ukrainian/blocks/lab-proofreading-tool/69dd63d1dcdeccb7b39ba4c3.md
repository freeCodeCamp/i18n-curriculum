...
3. Ви повинні визначити функцію з ім’ям `findRepeatedPhrases`, яка приймає масив `words` і число `phraseLength` як аргументи. Вона має повертати масив усіх початкових індексів, де послідовність із `phraseLength` підряд ідущих слів зустрічається в масиві більше одного разу — включно з індексом першої появи. Якщо `phraseLength` більше або дорівнює довжині `words`, має повертатися порожній масив. Перекривні послідовності також мають враховуватися.
...
`findRepeatedPhrases` має повертати всі початкові індекси для перекривних повторюваних фраз.
