...
`resetButton`를 생성했으니 이제 `id`와 `aria-label` 속성을 할당해야 합니다. JavaScript는 이를 위해 사용해야 하는 `id`와 `ariaLabel` 프로퍼티를 제공합니다.

예를 들어, `element.id`는 `id` 속성을 설정하고, `element.ariaLabel`는 `aria-label` 속성을 설정합니다. 둘 다 값을 문자열로 받습니다.

`id` 요소의 `resetButton` 속성을 `"reset"`로, `aria-label` 속성을 `"Reset playlist"`로 설정하세요.

# --hints--

기존 `if` 문장과 그 내용을 수정하지 마세요.
...
`resetButton.id`를 사용해 `id`에 `"reset"` 속성인 `resetButton`를 생성하세요.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

`resetButton.ariaLabel`를 사용해 `"aria-label"`에 `"Reset playlist"` 속성인 `resetButton`를 생성하세요.
