...
在這個項目中，你將通過構建音樂播放器應用程序學習基本的字符串和數組方法。 你將能夠播放、暫停、跳過和隨機播放歌曲。

該項目的 HTML 和 CSS 已爲你提供，因此你可以專注於 JavaScript。

首先使用 `getElementById()` 方法訪問 `#playlist-songs`、`#play` 和 `#pause` 元素。 將它們分別分配給變量 `playlistSongs`、`playButton` 和 `pauseButton`。


# --hints--

你應該有一個名爲 `playlistSongs` 的變量。

```js
assert.isDefined(playlistSongs);
```

你應該使用 `document.getElementById()` 來獲取 `#playlist-songs` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

你應該將 `#playlist-songs` 元素分配給變量 `playlistSongs`。

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

你應該有一個名爲 `playButton` 的變量。

```js
assert.isDefined(playButton);
```

你應該使用 `document.getElementById()` 來獲取 `#play` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

你應該將 `#play` 元素分配給變量 `playButton`。

```js
assert.equal(playButton, document.getElementById('play'));
```

你應該有一個名爲 `pauseButton` 的變量。

```js
assert.isDefined(pauseButton);
```

你應該使用 `document.getElementById()` 來獲取 `#pause` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

你應該將 `#pause` 元素分配給變量 `pauseButton`。
