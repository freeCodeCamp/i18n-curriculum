...
まず、各`button`要素に`aria-controls`属性を追加してください。最初のボタンの`aria-controls`属性の値は`"panel-earth"`に設定します。2番目のボタンの`aria-controls`属性は`"panel-saturn"`に設定します。3番目のボタンの`aria-controls`属性は`"panel-mars"`に設定してください。
