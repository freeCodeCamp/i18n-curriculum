...
Após a verificação `"Unknown"`, adicione a lógica de agrupamento por década para livros com anos numéricos. Declare `const decade = Math.floor(book.year / 10) * 10`. Por exemplo, `Math.floor(1977 / 10)` resulta em `197`, vezes `10` resulta em `1970`.

Depois declare `const decadeKey =` `` `${decade}s` `` to produce a string like `"1970s"`.

Use notação por colchetes para agrupar o livro: verifique se `grouped[decadeKey]` ainda não existe e inicialize como um array vazio, depois insira `book` nele. A notação por colchetes é necessária aqui porque a chave está armazenada em uma variável.

# --hints--

Você deve declarar uma variável chamada `decade` definida como `Math.floor(book.year / 10) * 10`.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+decade\s*=\s*Math\.floor\s*\(\s*book\.year\s*\/\s*10\s*\)\s*\*\s*10/);
```

Você deve declarar uma variável chamada `decadeKey` que inclua `decade` e a letra `"s"`.
...
Você deve verificar se `grouped[decadeKey]` ainda não existe e inicializá-lo como um array vazio.
...
Você deve inserir `book` em `grouped[decadeKey]`.
