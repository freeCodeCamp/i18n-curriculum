...
キャビンのアニメーションをより自然な揺れ動く動きに見せるために、`ease-in-out` タイミング関数を使うことができます。この設定は、アニメーションの開始と終了をゆっくりにし、サイクルの中間ではより速く動くように指示します。

`linear` セレクター内の `ease-in-out` を `.cabin` に置き換えてください。

# --hints--

`.cabin` セレクターには `animation` プロパティが `cabins 10s ease-in-out infinite` に設定されている必要があります。
