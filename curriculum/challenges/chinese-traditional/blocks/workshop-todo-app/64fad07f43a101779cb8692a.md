...
你可以透過將 `submit` 事件監聽器重構為兩個獨立的函式來提升程式碼的可讀性和維護性。第一個函式可以用來將輸入值添加到 `taskData`，而第二個函式則負責將任務添加到 DOM。

使用箭頭語法來創建 `addOrUpdateTask` 函式（程式）。然後將 `dataArrIndex` 變數、`taskObj` 物件和 `if` 述語移動到 `addOrUpdateTask` 函式（程式）中。

# --hints--

你應該使用 `const` 和箭頭語法來創建 `addOrUpdateTask` 函式（程式）。

```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

你應該將 `dataArrIndex` 變數移動到 `addOrUpdateTask` 函式中。
...
你應該將 `taskObj` 物件移動到 `addOrUpdateTask` 函式中。
...
你應該將條件為 `dataArrIndex === -1` 的 `if` 述語移動到你的 `addOrUpdateTask` 函式中。
