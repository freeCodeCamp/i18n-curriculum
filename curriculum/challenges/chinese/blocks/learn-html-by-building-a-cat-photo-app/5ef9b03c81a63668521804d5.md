...
在第三个 `section` 元素内，添加一个文本如下的 `h2` 元素：

`Cat Form`

# --hints--

找不到第三个 `section` 元素。 你可能不小心删除了它或开始标签或结束标签。
...
你的 `h2` 元素应该有一个开始标签和一个结束标签。 你可能缺少一个或两个必需的标签。
...
你应该只添加一个 `h2` 元素。 请删除任何多余的。

```js
assert(document.querySelectorAll('h2').length === 3);
```

新的 `h2` 元素应该位于第三个 `section` 元素的结束标签的上方。
