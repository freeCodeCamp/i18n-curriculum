...
Definir una función `pauseSong` usando la palabra clave `const` y la sintaxis de función de flecha. La función no debe tomar parámetros.

# --hints--

Debes usar `const` para crear una función vacía llamada `pauseSong`.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` debe ser una función.

```js
assert.isFunction(pauseSong)
```

Tu función `pauseSong` debe usar una sintaxis de flecha.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Tu función `pauseSong` debe estar vacía.
