...
Dé a `.fb1b` un `ancho` de `60%` y un `alto` de `10%`, y a `.fb1c` un `ancho` de `100%` y un `alto` de `80%`.

# --hints--

Debe dar a `.fb1b` un `ancho` de `60%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.width, "60%");
```

Debe dar a `.fb1b` un `alto` de `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.height, "10%");
```

Debe dar a `.fb1c` un `ancho` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1c")?.width, "100%");
```

Debe dar a `.fb1c` un `alto` de `80%`.
