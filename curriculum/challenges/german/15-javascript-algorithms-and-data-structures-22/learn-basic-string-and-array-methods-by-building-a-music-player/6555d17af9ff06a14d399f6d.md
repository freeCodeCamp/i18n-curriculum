...
Du solltest die `document.getElementById()` verwenden, um das `#player-song-title`-Element zu erhalten.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Du solltest das `#player-song-title`-Element der Variable `playingSong` zuweisen. Denke daran, `const` zu verwenden, um die Variable zu deklarieren.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Du solltest die `document.getElementById()` verwenden, um das `#player-song-artist`-Element zu erhalten.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Du solltest das `#player-song-artist`-Element der `songArtist`-Variable zuweisen. Denke daran, `const` zu verwenden, um die Variable zu deklarieren.
