...
Ora, sostituisci il loop `for` e l’istruzione `if` che hai aggiunto nel passaggio precedente con un’istruzione `if` che usa la funzione integrata `any()`.

# --hints--

La condizione della tua nuova istruzione `if` dovrebbe essere una chiamata a `any()`.
...
Dovresti passare un’espressione generatrice come argomento alla tua chiamata `any()`.
...
L’espressione generatrice passata a `any()` dovrebbe iterare su `args`.
...
La tua istruzione `if` dovrebbe verificare se uno qualsiasi degli argomenti in `args` non è un’istanza né di `int` né di `float`.
...
Dovresti usare la stringa fornita per sollevare un `TypeError` all’interno della tua nuova istruzione `if`.
