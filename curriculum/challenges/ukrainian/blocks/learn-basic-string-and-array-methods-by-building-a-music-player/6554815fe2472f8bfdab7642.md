...
Використайте `const` та стрілковий синтаксис, щоб створити порожню функцію `playPreviousSong`.

# --hints--

Ви повинні використати `const` для створення порожньої функції з іменем `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` має бути функцією.

```js
assert.isFunction(playPreviousSong)
```

Ваша функція `playPreviousSong` має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Ваша функція `playPreviousSong` має бути порожньою.
