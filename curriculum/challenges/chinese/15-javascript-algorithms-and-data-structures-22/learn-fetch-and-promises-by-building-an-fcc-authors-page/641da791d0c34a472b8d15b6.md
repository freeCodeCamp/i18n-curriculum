...
现在 `authorDataArr` 与你刚才输出到控制台的 `data` 相同。 将 `authorDataArr` 输出到控制台以确认这一点。

在 `console.log()` 语句中，添加文本 `"Author Data Array:"` 作为第一个参数，添加 `authorDataArr` 作为第二个参数。 使用逗号将文本与 `authorDataArr` 分隔。
...
你应该有一个带有文本 `"Author Data Array:"` 的控制台日志。
...
`authorDataArr` 应该是控制台日志语句的第二个参数。
