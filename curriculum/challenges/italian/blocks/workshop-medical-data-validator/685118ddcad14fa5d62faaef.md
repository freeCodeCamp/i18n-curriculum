...
Il tuo dizionario `constraints` dovrebbe avere una chiave `diagnosis`.
...
La chiave `diagnosis` del tuo dizionario `constraints` dovrebbe avere il valore di `isinstance(diagnosis, str) or diagnosis is None`.
