...
Ili kuchezwa wimbo wakati wowote mtumizi anapobofya juu yake, ongeza sifa ya kipengele ya `onclick` kwenye kipengele cha kitufe cha kwanza. Ndani ya `onclick`, fanya wito wa kitendakazi cha `playSong` kwa `song.id`.

Usisahau unahitaji kuingiza thamani kwa alama ya dola hapa.

# --hints--

Unapaswa kuongeza sifa ya kipengele ya `onclick` yenye thamani `playSong(${song.id})` kwenye kipengele cha `button`.
