...
Ändere die Hintergrundfarbe des `hr`-Elements in `brown`, damit sie zur Farbe der Kaffeebohnen passt.

# --hints--

Du solltest den Wert der `background-color`-Eigenschaft auf `brown` setzen.
...
Dein `hr`-Element sollte eine `background-color` von `brown` haben.
