...
関数 `setPlayerCards` に実引数として `players.filter()` を渡して呼び出してください。

`filter` メソッドの括弧内には、コールバック関数と `player` という仮引数を追加してください。そして、`player.nickname` が `null` でないことをチェックした結果を (return を明記せずに) 暗黙的に返してください。
...
`filter()` のコールバック関数は、`player.nickname` が `null` でないことをチェックした結果を、暗黙的に返す必要があります。
