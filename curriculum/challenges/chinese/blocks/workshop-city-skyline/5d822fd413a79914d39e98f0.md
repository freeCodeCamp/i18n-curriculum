...
在前面的模块中，你了解了 CSS 中的渐变是跨元素距离在颜色之间过渡的一种方式。 它们应用于 `background` 属性，语法如下所示：
...
在示例中，`color1` 在顶部是纯色，`color2` 在底部是纯色，而在它们之间均匀地从一个颜色过渡到下一个颜色。 在 `.bb1a` 中，在 `background-color` 属性下面添加 `background` 属性。 将其设置为 `linear-gradient` 类型的渐变，使用 `--building-color1` 作为其第一种颜色，使用 `--window-color1` 作为第二种颜色。
...
你应该给 `background` 一个 `linear-gradient`。
