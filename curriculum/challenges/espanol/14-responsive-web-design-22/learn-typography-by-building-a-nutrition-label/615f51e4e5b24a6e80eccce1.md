...
Dentro de su elemento `.calories-info`, cree un elemento `div`. Give that `div` element a `class` attribute set to `left-container`. Dentro del elemento `div` recién creado, cree un elemento `h2` con el texto `Amount per serving`. Asigne al elemento `h2` un atributo `class` establecido en `bold small-text`.
