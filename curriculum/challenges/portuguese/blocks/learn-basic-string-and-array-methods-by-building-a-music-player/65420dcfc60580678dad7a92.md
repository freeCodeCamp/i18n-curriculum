...
Dentro do `map()`, adicione uma declaração `return` com backticks onde você irá interpolar todos os elementos responsáveis por exibir os detalhes da música.

Dentro dos backticks, crie um elemento `li` com um atributo `id` de `song-${song.id}` e um atributo `class` de `playlist-song`.

# --hints--

Você deve adicionar uma declaração `return` com um par vazio de backticks.

```js
assert.match(code, /return\s*`/)
```

Você deve criar uma tag `li` dentro dos backticks.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

Sua tag de abertura `li` deve ter um atributo `id` definido como `song-${song.id}`.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

Sua tag de abertura `li` deve ter um atributo `class` definido como `playlist-song`.
