...
Este `useEffect` obtiene los datos con la Fetch API y establece todos los estados. 
...
Como aprendiste en una lección anterior sobre hooks personalizados, la obtención de datos es una lógica que puedes extraer en un hook personalizado. Entonces, si estás obteniendo datos en múltiples componentes y páginas, es mejor crear un hook `useFetch`.
...
Porque `useEffect</ code> se ejecuta sincrónicamente con el ciclo de renderizado.
...
Es una API más antigua utilizada para realizar solicitudes HTTP antes de Fetch.
