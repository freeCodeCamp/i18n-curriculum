...
Fügen Sie in Ihrem ersten `p`-Element ein `span`-Element hinzu. Geben Sie dem `span` eine Klasse von `product-count` und ein `id` von `product-count-for-id${id}`. Danach, außerhalb des span, geben Sie Ihrem `p`-Element den Text der `name`-Variablen.

# --hints--

Ihr erstes `p`-Element sollte ein `span`-Element enthalten.
...
Ihr `span`-Element sollte eine `class` von `product-count` haben.
...
Ihr `span`-Element sollte eine `id` von `product-count-for-id${id}` haben.
...
Ihr erstes `p`-Element sollte den Text der `name`-Variablen enthalten. Dies sollte außerhalb des span sein.
