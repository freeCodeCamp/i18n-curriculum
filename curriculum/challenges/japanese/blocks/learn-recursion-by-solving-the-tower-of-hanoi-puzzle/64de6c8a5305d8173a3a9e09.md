...
`make_allowed_move()` は `rod1` と `rod2` をパラメータとして受け取ります。ここで少しリファクタリングが必要です。`source` のすべての出現箇所を `rod1` に変更してください。

# --hints--

`source` 関数内の `make_allowed_move()` のすべての出現箇所を `rod1` に変更してください。
