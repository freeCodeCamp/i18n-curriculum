...
要获取当前歌曲的索引，可以使用 <dfn>indexOf()</dfn> 方法。 `indexOf()` 数组方法返回在数组中找到给定元素的第一个索引，如果元素不存在，则返回 `-1`。
