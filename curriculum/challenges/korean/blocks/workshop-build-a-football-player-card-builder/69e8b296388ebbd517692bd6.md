...
폼이 이제 완성되었습니다. 입력할 때 미리보기 카드가 업데이트되도록 하려면 입력을 `player` 상태에 연결해야 합니다.

Name `input`부터 시작해서 `value`의 `player.name` 속성을 추가하세요. 그런 다음 `onChange`를 매개변수로 받고 `e`를 펼친 객체와 `setPlayer`로 설정된 `player`를 사용해 `name`를 호출하는 화살표 함수로 설정된 `e.target.value` 속성을 추가하세요. 펼친 객체와 `name` 값 설정을 객체 안에 넣는 것을 잊지 마세요.

# --hints--

이름 입력 `value`은 player 상태 `name` 필드에 바인딩되어야 합니다.
...
이름 입력에는 `onChange` 핸들러가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

`onChange` 핸들러는 `setPlayer`를 호출해야 하며, `setPlayer`에는 `player`를 펼친 객체와 `name`를 `e.target.value`로 설정한 객체가 있어야 합니다.
