...
`console.log` の呼び出しを修正して、ドロップダウンから選択された値をログに出力してください。

`select` 要素の現在の値には、その `.value` プロパティを使ってアクセスできます。例えば：
...
`change` で `selectContainer` イベントが発生したときに、ドロップダウンから選択された値をログに出力してください。
