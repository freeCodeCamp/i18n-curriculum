...
Você deve testar seu progresso até agora. Crie uma instância da classe `Game` e atribua-a a uma variável `game`.

Neste ponto, você verá que pode visualizar as cartas ao clicar nos botões Single Card Reading e
Three Card Spread.

# --hints--

Você deve declarar uma variável chamada `game`.
...
Sua variável `game` deve ser inicializada com `new Game()`.
