...
Use sua `chunkCrew()` função para criar pedaços de tamanho `3` do seu array `EVAReadySquad` e armazená-los em uma nova variável chamada `EVAChunks`.

# --hints--

Você deve fazer a chamada de `chunkCrew()` com `EVAReadySquad` e `3` e armazenar o resultado em uma nova variável chamada `EVAChunks`.
