...
既然已經創建了 `salary` 設值函式，請更新 `__init__`，改為呼叫它，而不是直接指定給 `self._salary`。將 `self._salary = Employee._base_salaries[level]` 改成 `self.salary = Employee._base_salaries[level]`。

# --hints--

你的 `__init__` 方法應該使用 `self.salary = Employee._base_salaries[level]`，而非 `self._salary = Employee._base_salaries[level]`。
