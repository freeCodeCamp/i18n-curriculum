...
`The programmer who wrote this code is brilliant.` (`who` se refere ao programador)

`The artist whom we met at the gallery is internationally recognized.` (`whom` se refere ao artista)

`The tool that you lent me was useful.` (`that` se refere à ferramenta)

`The book, which I bought yesterday, is already a bestseller.` (`which` se refere ao livro)
