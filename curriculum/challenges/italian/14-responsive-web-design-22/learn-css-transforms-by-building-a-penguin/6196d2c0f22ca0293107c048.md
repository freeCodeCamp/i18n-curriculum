...
Dovresti usare il selettore `.sun`.

```js
assert.match(code, /\.sun\s*\{/);
```

Dovresti dare all'elemento `.sun` una proprietà `width` con il valore `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.width, '200px');
```

Dovresti dare all'elemento `.sun` una proprietà `height` con il valore `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.height, '200px');
```

Dovresti dare all'elemento `.sun` una proprietà `background-color` con il valore `yellow`.
