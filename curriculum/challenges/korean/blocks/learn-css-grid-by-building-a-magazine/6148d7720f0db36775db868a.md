...
텍스트용 열과 이미지용 열이 필요합니다. `.text-with-images` 선택자에 `grid-template-columns` 속성을 `1fr 2fr`로 설정하세요. 또한 열 사이 간격을 더 주기 위해 `column-gap` 속성을 `3rem`로 설정하세요.

# --hints--

`.text-with-images` 선택자에 `grid-template-columns` 속성을 `1fr 2fr`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

`.text-with-images` 선택자에 `column-gap` 속성을 `3rem`로 설정해야 합니다.
