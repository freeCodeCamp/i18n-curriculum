...
Weka kipengele cha `.chin` kwa njia kwamba kiko `25%` kutoka juu, na `5%` kutoka kushoto kwa mzazi wake. Kisha, toa pembe za juu mduara wa `70%`, na pembe za chini mduara wa `100%`.

# --hints--

Unapaswa kumpa `.chin` sifa ya `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Unapaswa kumpa `.chin` sifa ya `top` yenye thamani ya `--fcc-expected--`, lakini umeipata `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Unapaswa kumpa `.chin` sifa ya `left`.
...
Unapaswa kumpa `.chin` sifa ya `border-radius` yenye thamani ya `70% 70% 100% 100%`.
