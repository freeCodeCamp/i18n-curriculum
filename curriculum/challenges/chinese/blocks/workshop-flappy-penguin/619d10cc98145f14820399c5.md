...
在 `.blush.right` 元素下，添加两个 `div` 元素，每个元素都包含 `beak` `class`。 此外，为第一个 `.beak` 元素添加一个 `top` `class`，为第二个 `.beak` 元素添加一个 `bottom` `class`。
