...
テキストが`label`で、タイプが`Real Name`の`input`を内包する別の`password`を作成してください。`input`には`value`の`realName`を設定します。

次に、`onChange`イベントを使って、`realName`を更新したのと同じ方法で`heroName`を更新してください。

# --hints--

`label`要素を作成する必要があります。
...
`label`要素のテキストは`Real Name`であるべきです。
...
`input`要素の中にタイプが`password`の`label`を作成してください。
...
`input`要素の値は`realName`の状態変数に設定する必要があります。

```js
assert.match(code, /value\s*=\s*\{\s*realName\s*\}/)
```

`onChange`フィールドに入力できるように、`realName`イベントを使って`input`状態変数の値を設定してください。
