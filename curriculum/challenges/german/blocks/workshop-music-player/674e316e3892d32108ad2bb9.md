...
Jedes Mal, wenn Sie auf die Wiedergabetaste klicken, wird immer das erste Lied Ihrer Wiedergabeliste abgespielt. Um das zu beheben, ersetzen Sie innerhalb der Pfeilfunktion des Ereignis-Listeners `playSong(0)` durch eine `if`-Anweisung, die überprüft, ob `userData.currentSong` `null` ist.

Rufen Sie innerhalb des `if`-Blocks die `playSong`-Funktion mit dem `id` des ersten Lieds im `userData.songs`-Array auf.

# --hints--

Sie sollten den Callback des Ereignis-Listeners so ändern, dass die `playSong`-Funktion mit dem `id` des ersten Lieds im `userData.songs`-Array aufgerufen wird, wenn `userData.currentSong` `null` ist.
