...
Ora è il momento di iniziare ad aggiungere la funzionalità alla tablist. Il primo passaggio è consultare tutti gli elementi tab e panel nella tablist.

Crea una variabile chiamata `tabs` e assegnale il risultato della query per tutti gli elementi con l'attributo `role` impostato su `tab`.

Poi, crea una variabile chiamata `panels` e assegnale il risultato della query per tutti gli elementi con l'attributo `role` impostato su `tabpanel`.

Ricorda che puoi usare il metodo `querySelectorAll` per selezionare più elementi contemporaneamente.

# --hints--

Dovresti avere una variabile chiamata `tabs`.

```js
assert.isDefined(tabs);
```

Dovresti assegnare `document.querySelectorAll('[role="tab"]')` alla variabile `tabs`.
...
Dovresti avere una variabile chiamata `panels`.

```js
assert.isDefined(panels);
```

Dovresti assegnare `document.querySelectorAll('[role="tabpanel"]')` alla variabile `panels`.
