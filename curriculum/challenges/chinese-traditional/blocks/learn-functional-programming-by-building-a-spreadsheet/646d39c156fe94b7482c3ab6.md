...
宣告一個變數 `cellRegex` 來匹配儲存格參考。它應該匹配從 `A` 到 `J` 的字母，接著是一個從 `1` 到 `9` 的數字，以及一個從 `0` 到 `9` 的選用數字。使這個正規表達式不區分大小寫且為全域的。

# --hints--

你應該宣告一個 `cellRegex` 變數。
