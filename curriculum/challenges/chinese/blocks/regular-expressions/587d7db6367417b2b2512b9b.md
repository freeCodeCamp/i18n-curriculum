...
但是，你可以使用 `?` 字符将其更改为惰性匹配。`"titanic"` 与调整后的正则表达式 `/t[a-z]*?i/` 匹配时返回 `["ti"]`。

**注意：**应该避免使用正则表达式解析 HTML，但是可以用正则表达式匹配 HTML 字符串。

# --instructions--

修正正则表达式 `/<.*>/` 以返回超文本标记语言标签 `<h1>`，而不是文本 `"<h1>Winter is coming</h1>"`。请记住，正则表达式中的通配符 `.` 匹配任何字符。

# --hints--

`result` 变量应是包含 `<h1>` 的数组
