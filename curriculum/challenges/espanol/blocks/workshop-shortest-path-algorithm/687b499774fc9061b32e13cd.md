...
Al final de tu función, devuelve los datos que has calculado para que puedan usarse fuera de la función.

Devuelve tanto la lista de las `distances` más cortas desde el `start_node` a todos los demás nodos como la lista de los `paths` que conducen a cada nodo

# --hints--

Debes tener una sentencia `return` al final de tu función.
...
Debes devolver tanto `distances` como `paths`.
