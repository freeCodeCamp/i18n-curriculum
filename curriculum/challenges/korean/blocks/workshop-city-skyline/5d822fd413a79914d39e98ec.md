...
`div` 요소의 위쪽과 아래쪽에 각각 빈 `.foreground-buildings` 요소 두 개를 추가하고, `.fb2`와 `.fb3` 사이에 하나를 더 넣어 건물들을 다시 모으세요.

# --hints--

`div`의 첫 번째 자식으로 `.foreground-buildings` 요소 두 개를 추가하세요.
...
`div`과 `.fb2` 사이에 `.fb3` 요소 하나를 추가하세요.
...
`div`의 마지막 자식으로 `.foreground-buildings` 요소 두 개를 추가하세요.
