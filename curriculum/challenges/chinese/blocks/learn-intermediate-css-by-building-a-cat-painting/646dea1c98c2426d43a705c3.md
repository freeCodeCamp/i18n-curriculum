...
将左内眼移动到合适的位置，设置 `position` 为 `absolute`、`top` 为 `8px`，`left` 为 `2px`。 另外，给它设置一个 `60%` 的边框半径，并且旋转 `10` 度。

# --hints--

你的 `.cat-left-inner-eye` 选择器应该将 `position` 属性设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.position === 'absolute')
```

你的 `.cat-left-inner-eye` 选择器应该将 `top` 属性设置为 `8px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.top === '8px')
```

你的 `.cat-left-inner-eye` 选择器应该将 `left` 属性设置为 `2px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.left === '2px')
```

你的 `.cat-left-inner-eye` 选择器应该将 `border-radius` 属性设置为 `60%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.borderRadius === '60%')
```

你的 `.cat-left-inner-eye` 选择器应该将 `transform` 属性设置为 `10deg`。
