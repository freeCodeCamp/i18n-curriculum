...
接下來，添加另一個 `label` 元素，文本爲 `Location`， `for` 屬性設置爲 `"location"`。

對於該複選框 `input`，`id`、`name` 和 `value` 屬性均設置爲 `"location"`。

在該 `input` 元素下方，添加另一個 `label`元素，文本爲 `Reputation`，`for` 屬性設置爲 `"reputation"`。

對於該複選框 `input`，`id`、`name` 和 `value` 屬性均設置爲 `"reputation"`。

# --hints--

你應該有一個 `label` 元素，其文本爲 `Location`。
...
在地點複選框下方，你應該有一個 `label` 元素，其文本爲 `Reputation`。
...
你的第四個複選框 `input` 應將 `id` 設置爲 `"reputation"`。
...
你的第四個複選框應將 `name` 設置爲 `"reputation"`。
...
你的第四個複選框應將 `value` 設置爲 `"reputation"`。
