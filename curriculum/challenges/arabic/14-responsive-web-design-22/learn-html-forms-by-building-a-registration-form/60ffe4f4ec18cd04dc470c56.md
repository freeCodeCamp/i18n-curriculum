...
قم بنقل الخاصية والقيمة `min-height` بحيث يكون لكل أنواع العناصر الثلاثة نفس قيمة `min-height`، ولا يزال العنصر `select` يحتوي على خلفية بيضاء.

# --hints--

يجب عليك نقل خاصية `min-height` والقيمة إلى منتقى `input, textarea, select`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight);
```

يجب عليك إعطاء منتقي `input, textarea, select` الـ `min-height` بقيمة `2em`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight, '2em');
```

يجب عليك إزالة تعريف `min-height` من منتقي `input, textarea`.
