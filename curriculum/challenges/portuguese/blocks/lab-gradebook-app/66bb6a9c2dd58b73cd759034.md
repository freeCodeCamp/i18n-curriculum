...
1. Você deve ter uma função chamada `getGrade` que recebe a pontuação de um aluno como parâmetro e retorna uma string representando uma nota em letra com base na pontuação. Aqui estão as pontuações e suas respectivas notas em letra:
...
1. A função `hasPassingGrade` deve usar a função `getGrade` para obter a nota em letra e usá-la para determinar se a nota é aprovada. Uma nota aprovada é qualquer coisa diferente de `"F"`.
1. Você deve ter uma função chamada `studentMsg` que receba um array de pontuações e a pontuação de um aluno como os parâmetros. A função deve retornar uma string com este formato:

    - `"Class average: average-goes-here. Your grade: grade-goes-here. You passed the course."`, se o aluno passou no curso.
    - `"Class average: average-goes-here. Your grade: grade-goes-here. You failed the course."`, se o aluno não passou no curso.

    Substitua `average-goes-here` pela média total das pontuações e `grade-goes-here` pela nota do aluno.
    Use `getAverage` para obter a pontuação média e `getGrade` para obter a nota do aluno.
...
`studentMsg([92, 88, 12, 77, 57, 100, 67, 38, 97, 89], 37)` deve retornar a seguinte mensagem: `"Class average: 71.7. Your grade: F. You failed the course."`.

```js
assert.strictEqual(studentMsg([92, 88, 12, 77, 57, 100, 67, 38, 97, 89], 37), "Class average: 71.7. Your grade: F. You failed the course.");
```

`studentMsg([56, 23, 89, 42, 75, 11, 68, 34, 91, 19], 100)` deve retornar a seguinte mensagem: `"Class average: 50.8. Your grade: A+. You passed the course."`.

```js
assert.strictEqual(studentMsg([56, 23, 89, 42, 75, 11, 68, 34, 91, 19], 100), "Class average: 50.8. Your grade: A+. You passed the course.");
```

`studentMsg([12, 22, 32, 42, 52, 62, 72, 92], 85)` deve retornar a seguinte mensagem: `"Class average: 48.25. Your grade: B. You passed the course."`.

```js
assert.strictEqual(studentMsg([12, 22, 32, 42, 52, 62, 72, 92], 85), "Class average: 48.25. Your grade: B. You passed the course.");
```

`studentMsg([15, 25, 35, 45, 55, 60, 70, 60], 75)` deve retornar a seguinte mensagem: `"Class average: 45.625. Your grade: C. You passed the course."`.
