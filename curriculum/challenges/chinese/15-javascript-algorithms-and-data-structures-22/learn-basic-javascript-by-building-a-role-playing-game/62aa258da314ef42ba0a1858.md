...
创建两个新函数，分别命名为 `pickTwo` 和 `pickEight`。

在这两个函数内部，调用 `pick()` 函数，并根据函数名称传递 `2` 或 `8` 作为参数。

# --hints--

你应该使用 `function` 关键字声明 `pickTwo`。

```js
assert.match(code, /function\s+pickTwo\s*/);
```

你应该使用 `function` 关键字声明 `pickEight`。

```js
assert.match(code, /function\s+pickEight\s*/);
```

`pickTwo` 应该调用 `pick` 函数。

```js
assert.match(pickTwo.toString(), /pick\(/);
```

`pickTwo` 应该调用 `pick` 函数，并传递 `2` 作为参数。

```js
assert.match(pickTwo.toString(), /pick\(\s*2\s*\)/);
```

`pickEight` 应该调用 `pick` 函数。

```js
assert.match(pickEight.toString(), /pick\(/);
```

`pickEight` 应该调用 `pick` 函数，并传递 `8` 作为参数。
