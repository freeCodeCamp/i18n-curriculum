...
استخدم `const` وصيغة السهم لإنشاء دالة `playPreviousSong` فارغة.

# --hints--

يجب استخدام `const` لإنشاء دالة فارغة باسم `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

يجب أن تكون `playPreviousSong` دالة.

```js
assert.isFunction(playPreviousSong)
```

يجب أن تستخدم دالة `playPreviousSong` صيغة السهم.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

يجب أن تكون دالة `playPreviousSong` فارغة.
