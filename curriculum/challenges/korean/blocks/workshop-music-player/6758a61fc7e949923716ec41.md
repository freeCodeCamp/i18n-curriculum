...
이제 `querySelectorAll` 메서드를 사용해 `.playlist-song` 요소들을 선택하고 `songs`라는 변수에 저장하세요.

그다음 `forEach`에서 `songs` 메서드를 호출하세요. 메서드에 매개변수로 `song`를 사용하는 콜백 함수를 전달하세요. 중괄호는 일단 비워 두세요.

# --hints--

`songs`라는 변수가 있어야 합니다.

```js
assert.isDefined(songs);
```

모든 `.playlist-song` 요소들의 집합을 `songs` 변수에 할당해야 합니다.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

`forEach`에서 `songs` 메서드를 호출하고, 매개변수로 `song`를 사용하는 콜백을 전달해야 합니다.
