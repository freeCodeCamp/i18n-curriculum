...
Dê à sua `conversionTable` uma propriedade `gram` e atribua a ela um objeto com uma propriedade `cup` definida como `1/240`, uma propriedade `ounce` definida como `0.0353` e uma propriedade `teaspoon` definida como `0.2`.

# --hints--

Sua `conversionTable` deve ter uma propriedade `gram`.

```js
assert.property(conversionTable, "gram");
```

Sua propriedade `gram` deve ser um objeto.

```js
assert.isObject(conversionTable.gram);
```

Seu objeto `gram` deve ter uma propriedade `cup`.

```js
assert.property(conversionTable.gram, "cup");
```

A sua propriedade `cup` deve ser definida como `1/240`.

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

Seu objeto `gram` deve ter uma propriedade `ounce`.

```js
assert.property(conversionTable.gram, "ounce");
```

A sua propriedade `ounce` deve ser configurada para `0.0353`.

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

Seu objeto `gram` deve ter uma propriedade `teaspoon`.

```js
assert.property(conversionTable.gram, "teaspoon");
```

A sua propriedade `teaspoon` deve ser definida como `0.2`.
