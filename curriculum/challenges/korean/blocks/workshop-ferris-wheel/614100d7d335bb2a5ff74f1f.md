...
`.wheel` 선택자로 애니메이션을 제어하는 네 가지 다른 속성을 만들었습니다. `.cabin` 선택자에는 `animation` 속성을 사용해 이 모든 속성을 한 번에 설정할 수 있습니다.

`animation` 규칙의 `.cabin` 속성을 `cabins 10s linear infinite`로 설정하세요. 그러면 `animation-name`, `animation-duration`, `animation-timing-function`, `animation-iteration-count` 속성이 그 순서대로 설정됩니다.

# --hints--

`.cabin` 선택자에는 `animation` 속성이 `cabins 10s linear infinite`로 설정되어야 합니다.
