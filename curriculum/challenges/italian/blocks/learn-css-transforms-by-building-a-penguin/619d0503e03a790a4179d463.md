...
Posiziona l'elemento `.chin` in modo che sia `25%` dall'alto e `5%` da sinistra del suo genitore. Poi, dai agli angoli superiori un raggio di `70%` e agli angoli inferiori un raggio di `100%`.

# --hints--

Dovresti dare a `.chin` una proprietà `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Dovresti dare a `.chin` una `top` di `--fcc-expected--`, ma è stato trovato `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Dovresti dare a `.chin` una proprietà `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Dovresti dare a `.chin` una `left` di `--fcc-expected--`, ma è stato trovato `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Dovresti dare a `.chin` una `border-radius` di `70% 70% 100% 100%`.
