...
Posicione el elemento `.chin` de tal manera que esté a `25%` desde la parte superior y a `5%` del lado izquierdo de su elemento padre. Luego, dé a las esquinas superiores un radio de `70%`, y a las esquinas inferiores un radio de `100%`.

# --hints--

Debe dar al elemento `.chin` una propiedad de `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Debe dar al `.chin` un `top` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Debe dar al elemento `.chin` una propiedad de `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Debe dar al `.chin` un `left` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Debe dar al `.chin` un `border-radius` de `70% 70% 100% 100%`.
