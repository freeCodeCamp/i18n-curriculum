...
Sasa utaanza kurejelea vipengele vinavyohitajika vya DOM.

Pata kipengele cha aya chenye kitambulisho cha `bug-emoji` na uhifadhi katika thabiti iitwayo `bugEmojiElement`. Tumia kiendeshaji cha uthibitisho wa non-null cha `!` kuthibitisha kuwa kipengele si `null`.

# --hints--

Unapaswa kuwa na thabiti iitwayo `bugEmojiElement`.
...
`bugEmojiElement` yako inapaswa kuhoji kipengele chenye kitambulisho `bug-emoji` kwa kutumia `document.querySelector` na hoja ya aina ya `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

`bugEmojiElement` yako inapaswa kutumia kiendeshaji cha uthibitisho wa non-null (`!`).
