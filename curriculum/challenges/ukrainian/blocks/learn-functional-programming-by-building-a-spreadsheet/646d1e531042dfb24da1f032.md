...
Другий аргумент методу `.replace()` не обов’язково має бути рядком. Замість цього ви можете передати функцію зворотного виклику, щоб виконати складнішу логіку над знайденим рядком.

Функція зворотного виклику приймає кілька параметрів. Перший — це знайдений рядок. Передайте порожню функцію зворотного виклику у ваш виклик `.replace()` і дайте їй параметр `match`.

# --hints--

Ви маєте передати стрілкову функцію як другий аргумент методу `.replace()`.
...
Ваша стрілкова функція має приймати параметр `match`.
