...
Оскільки `elemValue` повертає функцію, ваша функція `addCharacters` в кінцевому підсумку повертає масив референсів функцій. Ви хочете, щоб метод `.map()` запускав внутрішню функцію вашої функції `elemValue`, а це означає, що потрібно викликати `elemValue`, а не посилатися на неї. Передайте `num` як аргумент у вашу функцію `elemValue`.

<!-- TODO: Explain further? -->

# --hints--

Ви повинні викликати `elemValue()` у вашому методі `.map()`.
...
Ви повинні передати `num` у виклик `elemValue()`.
