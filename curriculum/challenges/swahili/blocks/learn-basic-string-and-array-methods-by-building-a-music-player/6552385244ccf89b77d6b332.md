...
Unahitaji kufanya kazi ya kuchezwa wimbo unaofuata na wimbo uliotangulia. Kwa hili, utahitaji kitendakazi cha `playNextSong` na `playPreviousSong`.

Tumia `const` na sintaksia ya mshale kuunda kitendakazi cha `playNextSong` kisicho na maudhui.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi kisicho na maudhui chenye jina `playNextSong`.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` inapaswa kuwa kitendakazi.

```js
assert.isFunction(playNextSong)
```

Kitendakazi chako cha `playNextSong` kinapaswa kutumia sintaksia ya mshale.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Kitendakazi chako cha `playNextSong` kinapaswa kuwa kisicho na maudhui.
