...
الآن، باتباع الصيغة النحوية أعلاه، حدّث دالة `convertQuantity` لتُرجع دالة ثانية. يجب أن تقبل هذه الدالة الداخلية معلمة `toUnit`، ويجب أن تكون فارغة.

# --hints--

يجب أن تُرجع دالة `convertQuantity` دالة.

```js
assert.isFunction(convertQuantity());
```

يجب أن تحتوي الدالة التي تُرجعها على معلمة `toUnit`.

```js
assert.match(convertQuantity()?.toString(), /\(?toUnit\)?/);
```

يجب أن تكون الدالة التي تُرجعها فارغة.
