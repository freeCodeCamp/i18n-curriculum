...
title: 'Aufgabe 73: Zählen von Brüchen in einem Bereich'
challengeType: 1
forumTopicId: 302186
...
Betrachten Sie den Bruch, $\frac{n}{d}$, wobei `n` und `d` positive ganze Zahlen sind. Wenn `n` &lt; `d` und der größte gemeinsame Teiler, ${HCF}(n, d) = 1$, ist, nennt man ihn einen gekürzten echten Bruch.

Wenn wir die Menge der gekürzten echten Brüche für `d` ≤ 8 in aufsteigender Reihenfolge auflisten, erhalten wir:
...
Man sieht, dass es `3` Brüche zwischen $\frac{1}{3}$ und $\frac{1}{2}$ gibt.

Wie viele Brüche liegen zwischen $\frac{1}{3}$ und $\frac{1}{2}$ in der sortierten Menge der gekürzten echten Brüche für `d` ≤ `limit`?
