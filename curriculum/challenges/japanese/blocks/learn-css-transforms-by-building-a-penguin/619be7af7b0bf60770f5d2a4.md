...
疑似要素 `10%` を親要素の上から、`25%` を左から配置してください。

# --hints--

`.penguin-body::before` に `top` の `--fcc-expected--` を指定する必要がありますが、`--fcc-actual--` が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

`.penguin-body::before` に `left` の `--fcc-expected--` を指定する必要がありますが、`--fcc-actual--` が見つかりました。
