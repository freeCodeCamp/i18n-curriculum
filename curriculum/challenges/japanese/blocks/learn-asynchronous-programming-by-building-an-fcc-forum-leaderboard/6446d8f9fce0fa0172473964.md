...
`allCategories` オブジェクトの残りの部分はすでに完成しています。

次のいくつかのステップでは、`allCategories` オブジェクトからカテゴリ名を取得する関数を作成します。

まず、`forumCategory` をパラメータ名として持つ `id` という名前のアロー関数を作成してください。

# --hints--

空の関数 `forumCategory` を作成する必要があります。

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

`id` 関数には `forumCategory` という名前のパラメータが必要です。
