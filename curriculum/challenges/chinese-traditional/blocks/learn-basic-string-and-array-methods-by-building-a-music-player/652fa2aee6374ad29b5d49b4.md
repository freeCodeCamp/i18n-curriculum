...
從 HTML 檔案中存取 `#next`、`#previous` 和 `#shuffle` 元素。

分別將它們指定、設值給名為 `nextButton`、`previousButton` 和 `shuffleButton` 的變數。

# --hints--

你應該有一個名為 `nextButton` 的變數。

```js
assert.isDefined(nextButton);
```

你應該使用 `document.getElementById()` 來取得 `#next` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

你應該將 `#next` 元素指定到變數 `nextButton`。 

```js
assert.equal(nextButton, document.getElementById('next'));
```

你應該有一個名為 `previousButton` 的變數。

```js
assert.isDefined(previousButton);
```

你應該使用 `document.getElementById()` 來取得 `#previous` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

你應該將 `#previous` 元素指定給變數 `previousButton`。 

```js
assert.equal(previousButton, document.getElementById('previous'));
```

你應該有一個名為 `shuffleButton` 的變數。

```js
assert.isDefined(shuffleButton);
```

你應該使用 `document.getElementById()` 來取得 `#shuffle` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

你應該將 `#shuffle` 元素指定到變數 `shuffleButton`。 
