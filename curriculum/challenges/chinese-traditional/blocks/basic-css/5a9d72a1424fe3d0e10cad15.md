...
當你在 `:root` 中創建你的變數時，將會為整個頁面設定該變數的值。

你可以在特定選擇器中再次設定這些變數，從而覆寫它們。

# --instructions--

將 `penguin` 類別中的 `--penguin-belly` 值更改為 `white`。

# --hints--

`penguin` 類別應該將 `--penguin-belly` 變數重新指定為 `white`。

```js
assert.match(__helpers.removeCssComments(code), /\.penguin\s*?{[\s\S]*--penguin-belly\s*?:\s*?white\s*?;[\s\S]*}/gi);
```

`penguin` 類別不應包含 `background-color` 屬性。
