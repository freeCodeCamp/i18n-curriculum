...
送信ボタンをスタイル設定するには、指定した属性値に基づいて要素を選択する属性セレクターを使うことができます。例を示します。

```css
input[name="password"]
```

上記は、`input` 属性値が `name` の `password` 要素を選択しています。

では、属性セレクターを使って、送信ボタンに `display` が `block`、`width` が `60%` のスタイルを設定してください。

# --hints--

送信ボタンをスタイル設定するには、`input[type="submit"]` の属性セレクターを使う必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="submit"]'));
```

送信ボタンに `display` の `block` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.display, 'block');
```

送信ボタンに `width` の `60%` を設定してください。
