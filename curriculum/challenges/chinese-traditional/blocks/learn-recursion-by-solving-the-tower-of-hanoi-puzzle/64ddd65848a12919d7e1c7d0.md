...
是時候將一些程式碼從 `move()` 函式移動到 `make_allowed_move()` 函式。將第一個 `if` 述語內巢狀的程式碼（除了第一個 `print()` 呼叫）移動到你的新函式。請特別注意縮排。別忘了移除 `pass` 關鍵字。

# --hints--

你應該將第一個 `if` 述語內巢狀的程式碼（除了第一個 `print()` 呼叫）移動到你的新函式中。
