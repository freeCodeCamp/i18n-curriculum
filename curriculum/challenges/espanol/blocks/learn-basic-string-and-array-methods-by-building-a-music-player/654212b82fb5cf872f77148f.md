...
Dentro del elemento `button`, cree dos elementos `span` más.

El primer elemento `span` debería tener una `class` de `playlist-song-artist`. Entre las etiquetas `span`, agregue `${song.artist}`.

El segundo elemento `span` debería tener una `class` de `playlist-song-duration`. Entre las etiquetas `span`, agregue `${song.duration}`.

# --hints--

No debe modificar el elemento `button` existente y su contenido.
...
Debe crear un elemento `span` con la clase `playlist-song-artist`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

Su elemento `span` con la clase `playlist-song-artist` debería interpolar `song.artist` como su contenido.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

Debe crear un elemento `span` con la clase `playlist-song-duration`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

Su elemento `span` con la clase `playlist-song-duration` debería interpolar `song.duration` como su contenido.
