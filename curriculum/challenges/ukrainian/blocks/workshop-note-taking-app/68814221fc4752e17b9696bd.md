...
Коли користувач натискає на нотатку, вносить зміни, а потім натискає поза межами нотатки, вам знадобиться спосіб отримати цю нову відредаговану версію. Це можна зробити, використовуючи подію `blur`. Ця подія спрацьовує, коли елемент втрачає фокус. Ось приклад:
...
Приєднайте метод `addEventListener` до змінної `noteEl`. Першим аргументом має бути подія `"blur"`, а другим — стрілкова функція. Усередині тіла цієї стрілкової функції використайте `const`, щоб створити нову змінну з назвою `newContent` і призначити їй значення `noteEl.innerHTML`. 

`innerHTML` тут використовується, щоб допомогти зберегти відступи та форматування нотатки. 

# --hints--

Ви повинні приєднати метод `addEventListener` до змінної `noteEl`.

```js
assert.match(code, /noteEl\.addEventListener/);
```

Ваш метод `addEventListener` має мати першим аргументом подію `"blur"`.

```js
assert.match(code, /noteEl\.addEventListener\(['"]blur['"],/);
```

У тілі стрілкової функції слід використати `const`, щоб створити нову змінну з назвою `newContent` і призначити їй значення `noteEl.innerHTML`.
