...
仅对于 `n=5`，我们进行了 9 次函数调用，而实际上只需要 5 次唯一计算。随着 `n` 的增长，这种冗余呈指数级爆炸——`climb_stairs(30)` 需要几百万次函数调用。时间复杂度变为 `O(2^n)`，这使得对于较大值的 `n` 来说效率低下且不切实际。
...
- **实际影响**：`climb_stairs(30)` 从数百万次函数调用减少到大约 30 次唯一子问题计算
