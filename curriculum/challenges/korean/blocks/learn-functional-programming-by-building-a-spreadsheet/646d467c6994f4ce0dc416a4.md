...
삼항 연산자를 사용해 `.hasOwnProperty()` 호출을 조건으로 만드세요. 객체에 해당 속성이 있으면 `apply`와 `fn`을 인수로 하여 `args`을 호출한 결과를 반환하세요. 그렇지 않으면 `match`를 반환하세요.

# --hints--

콜백 함수는 삼항 구문을 사용해야 합니다.
...
삼항 조건이 참이면 콜백 함수는 `apply()`를 호출한 결과를 반환해야 합니다.
