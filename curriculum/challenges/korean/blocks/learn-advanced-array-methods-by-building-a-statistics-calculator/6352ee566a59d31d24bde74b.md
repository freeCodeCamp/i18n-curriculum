...
`mode` 은 배열이므로 `.join()` 메서드를 사용해 문자열로 반환하세요. 요소들은 쉼표와 공백으로 구분하세요.

# --hints--

`getMode` 함수는 `mode` 변수를 반환해야 합니다.

```js
assert.match(getMode.toString(), /return\s+mode\s*/)
```

`mode` 변수는 `join` 메서드를 사용해 문자열로 반환해야 합니다.

```js
assert.match(getMode.toString(), /return\s+mode\.join\(/)
```

`mode` 배열의 요소들은 쉼표와 공백으로 구분해야 합니다.
