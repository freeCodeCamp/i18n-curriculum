...
مع مراعاة أفضل الممارسات لمُدخَلات النماذج، عرّف لكل `input` خاصية `type` و`name` مناسبة. ثم، أضف خاصية `placeholder` لأول `input`.

# --hints--

يجب أن تعطي أول `input` خاصية `type` بقيمة `text`.

```js
assert.equal(document.querySelectorAll('input')?.[0]?.type, 'text');
```

يجب أن تعطي ثاني `input` خاصية `type` بقيمة `email`.

```js
assert.equal(document.querySelectorAll('input')?.[1]?.type, 'email');
```

يجب أن تعطي ثالث `input` خاصية `type` بقيمة `date`.

```js
assert.equal(document.querySelectorAll('input')?.[2]?.type, 'date');
```

يجب أن تعطي أول `input` خاصية `name` مناسبة.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[0]?.name?.length, 1);
```

يجب أن تعطي ثاني `input` خاصية `name` مناسبة.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[1]?.name?.length, 1);
```

يجب أن تعطي ثالث `input` خاصية `name` مناسبة.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[2]?.name?.length, 1);
```

يجب أن تعطي أول `input` خاصية `placeholder`.
