...
La función `apply` está devolviendo la función de hoja de cálculo, pero no realmente aplicándola. Actualiza `apply` para llamar a la función. Pasa el resultado de llamar a `toNumberList` con `args` como argumento.

# --hints--

La función `apply` debe llamar a la función `spreadsheetFunctions[fn.toLowerCase()]`.
...
Debes pasar una llamada a `toNumberList()` a tu llamada `spreadsheetFunctions[fn.toLowerCase()]`.
...
Debes pasar `args` a tu llamada `toNumberList()`.
