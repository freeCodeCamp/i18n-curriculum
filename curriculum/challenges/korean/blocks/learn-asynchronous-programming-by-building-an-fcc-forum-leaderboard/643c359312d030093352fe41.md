...
이제 페이지에 데이터를 표시할 차례입니다.

하나의 `showLatestPosts` 매개변수를 받는 `data`라는 화살표 함수를 만들어 시작하세요.

# --hints--

`showLatestPosts`라는 화살표 함수가 있어야 합니다.

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

`showLatestPosts` 함수는 `data` 매개변수를 가져야 합니다.

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

`showLatestPosts`가 정의되어 있어야 합니다.

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts`는 함수여야 합니다.
