...
كالمعلمة اللونية الثانية، مرر دالة `hsl` بالقيم `223` للون، و`90%` للتشبع، و`60%` للفاتح اللون.

# --hints--

يجب أن تحتوي قاعدة CSS الخاصة بـ `.blue` على خاصية `background` مضبوطة على `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%))`.
