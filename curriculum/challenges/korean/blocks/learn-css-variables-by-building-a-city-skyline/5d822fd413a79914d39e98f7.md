...
`orange` 그라디언트에서 `.bb1d`을 제거하고 `80%`를 `50%`로 변경하세요. 이렇게 하면 상단 절반은 `--building-color1`가 단색이 되고 하단 절반은 `--window-color1`로 전환됩니다.

# --hints--

`orange`에서 `linear-gradient`을 제거해야 합니다.

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'orange');
```

이제 첫 번째 `linear-gradient` 색상을 `50%`에서 전환하도록 변경하세요.
