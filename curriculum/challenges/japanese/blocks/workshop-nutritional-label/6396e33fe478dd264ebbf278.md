...
新しい `.calories-info` セレクターを作成し、`display` プロパティを `flex` に設定してください。また、`justify-content` プロパティを `space-between` に、`align-items` プロパティを `flex-end` に設定してください。
...
`.calories-info` セレクターには `display` プロパティを `flex` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info')?.display === 'flex');
```

`.calories-info` セレクターには `justify-content` プロパティを `space-between` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info')?.justifyContent === 'space-between');
```

`.calories-info` セレクターには `align-items` プロパティを `flex-end` に設定してください。
