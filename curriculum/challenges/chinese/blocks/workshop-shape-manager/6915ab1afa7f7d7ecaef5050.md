...
要最终显示计算结果，使用带有 `propertyInputs` 的 `for...of` 循环遍历 `Object.entries()` 对象。你可以通过在循环声明中 `input` 前加逗号来忽略密钥。

# --hints--

你应该创建一个 `for...of` 循环，通过跳过密钥并只保留 `Object.entries(propertyInputs)` 来解构 `input` 中的每个条目。

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

你的 `for...of` 循环应该迭代 `Object.entries(propertyInputs)`。
