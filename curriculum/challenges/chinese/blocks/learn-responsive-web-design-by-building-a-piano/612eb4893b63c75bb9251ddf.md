...
通过创建 `.logo` 选择器开始设置 logo 样式。 将 `width` 设置为 `200px`、 `position` 设置为 `absolute`、`top` 设置为 `23px`。
