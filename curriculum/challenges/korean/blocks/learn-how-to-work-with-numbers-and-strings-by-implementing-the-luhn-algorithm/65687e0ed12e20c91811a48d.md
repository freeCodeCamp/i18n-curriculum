...
`for` 함수의 두 번째 `verify_card_number` 반복문 아래에 `total`라는 변수를 만들고, 홀수와 짝수 숫자의 합을 할당하세요. `total`을 콘솔에 출력하세요.

# --hints--

`total = sum_of_odd_digits + sum_of_even_digits` 함수 안에 `verify_card_number`가 있어야 합니다.
...
`print(total)` 함수 안에 `verify_card_number`가 있어야 합니다.
