...
Перемістіть властивість `background-color` та її значення з `.fb1` до `.fb1b`.

# --hints--

Ви повинні видалити `background-color` з `.fb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb1")?.backgroundColor);
```

Ви повинні додати проп `background-color` зі значенням `--building-color4` до `.fb1b`.
