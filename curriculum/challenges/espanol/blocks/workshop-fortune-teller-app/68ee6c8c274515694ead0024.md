...
La fortuna debe ser aleatoria, por lo que se requiere una función auxiliar para seleccionar una fortuna al azar. Declara una variable llamada `chosenCard` dentro del método `singleCardSelected`. Usa el método `getRandomItem` para obtener una carta aleatoria y pasa `cards` como argumento.

# --hints--

Debes declarar una variable llamada `chosenCard` dentro de `singleCardSelected`.
...
Tu variable `chosenCard` debe asignarse llamando a `getRandomItem(this.cards)`.
...
Debes pasar `this.cards` como argumento a `getRandomItem`.
