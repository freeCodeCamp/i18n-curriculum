...
在你的 `styles.css` 文件中，创建一个 `.box2` 选择器，并为它添加值为 `#f4a261` 的 `background` 属性。

另外，为你的元素添加 `order` 属性，值为 `0`。
