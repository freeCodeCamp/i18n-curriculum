...
创建一个名为 `hasPlayerWonTheRound` 的函数，带有两个参数，分别称为 `playerChoice` 和 `computerChoice`。这些参数将接受字串值 `"Rock"`、`"Paper"` 或 `"Scissors"`，表现玩家和计算机所做的选择。

如果玩家赢得了这一局，该函数应该返回 `true`。如果玩家输了或打平了这一局，该函数则应返回 `false`。
...
你应该有一个名为 `hasPlayerWonTheRound` 的函数。

```js
assert.isFunction(hasPlayerWonTheRound);
```

你的函数应有两个名为 `playerChoice` 和 `computerChoice` 的参数。
