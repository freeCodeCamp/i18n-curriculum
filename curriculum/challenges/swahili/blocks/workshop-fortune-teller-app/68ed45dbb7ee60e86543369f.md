...
Baada ya kupata data, hifadhi jibu katika kigezo na ulitafsiri kuwa JSON. Toa aina `Deck` kwa data iliyotafsiriwa, kisha weka thamani yake kwenye sifa ya `cards`. Ndani ya blokki ya `catch`, hakikisha unaandika kosa kwa ujumbe huu: `"Error fetching cards:", error`.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `data` ndani ya `fetchCardsData`.
...
Kigezo chako cha `data` kinapaswa kuwa na aina `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

Kigezo chako cha `data` kinapaswa kupewa thamani kwa kutafsiri jibu kama JSON.
...
Unapaswa kuhifadhi `data.cards` katika `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Blokki yako ya `catch` inapaswa kutumia `console.error()` kuandika kosa kwa ujumbe `"Error fetching cards:", error`.
