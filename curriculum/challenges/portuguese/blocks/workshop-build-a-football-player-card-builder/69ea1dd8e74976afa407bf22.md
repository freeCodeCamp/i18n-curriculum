...
Em seguida, crie um tipo `Position` com o valor `typeof POSITIONS[number]`. Isso desempacota o array `POSITIONS` em uma união de suas literais string individuais, fazendo com que pareça com `"GK" | "CB" | "LB" | ...` e assim por diante.

# --hints--

Você deve criar um alias de tipo chamado `Position`.
...
Seu tipo `Position` deve ser definido como `typeof POSITIONS[number]`.
