...
第一個，新增 `label`，其 `className` 為 `label`、`htmlFor` 為 `pac`，文字為 `PAC`，接著新增一個輸入欄位，具有 `id` 為 `pac`、`className` 為 `input`，以及 `type` 為 `number`。
...
你的第一個 `className` 為 `form-group` 的 `div` 元素應該有一個 `label`，其具有 `htmlFor` 屬性為 `pac`、`className` 為 `label`，以及文字 `PAC`。
...
你的第一個 `className` 為 `form-group` 的 `div` 元素應該有一個 `input`，其具有 `className` 為 `input`、`id` 為 `pac`，以及 `type` 為 `number`。
...
你的第二個 `className` 為 `form-group` 的 `div` 元素應該有一個 `label`，其具有 `htmlFor` 屬性為 `sho`、`className` 為 `label`，以及文字 `SHO`。
...
你的第二個 `className` 為 `form-group` 的 `div` 元素應該有一個 `input`，其具有 `id` 為 `sho`、`className` 為 `input`，以及 `type` 為 `number`。
