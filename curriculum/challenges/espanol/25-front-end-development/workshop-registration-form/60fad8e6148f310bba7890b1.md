...
Centrar los elementos `h1` y `p` dándoles un `margin` de `1em auto`. Luego, alinea su texto en el `center` también.

# --hints--

Debe usar un selector de elementos separados por comas para dar estilo a los elementos `h1` y `p`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

Debe usar un `margin` de `1em auto` para centrar los elementos `h1` y `p`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

Debe usar un `text-align` de `center` para centrar el texto de `h1` y `p`.
