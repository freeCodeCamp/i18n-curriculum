...
Por ejemplo, considera el número `453914881`. Los pasos para validarlo usando el algoritmo de Luhn son:
...
`verify_card_number('453914889')` debería devolver `VALID!`.
...
`verify_card_number('4111-1111-1111-1111')` debería devolver `VALID!`.
...
`verify_card_number('453914881')` debería devolver `INVALID!`.
...
`verify_card_number('1234 5678 9012 3456')` debería devolver `INVALID!`.
...
`verify_card_number` debería devolver `VALID!` cuando se llame con un número de tarjeta de crédito válido.
...
`verify_card_number` debería devolver `INVALID!` cuando se llame con un número de tarjeta de crédito inválido.
