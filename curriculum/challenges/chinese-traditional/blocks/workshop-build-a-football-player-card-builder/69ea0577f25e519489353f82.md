...
在 `catch` 區塊中，將字串 `Failed to load player data, using defaults:` 和 `error` 參數依序記錄到主控台。

接著在 `catch` 區塊外，但仍在 `loadPlayer` 函式內，傳回 `defaultPlayer`。

這些動作會載入玩家資料，將它們填入使用者介面，並在處理程序失敗時載入 `defaultPlayer` 物件。

# --hints--

你的 `catch` 區塊應該將 `'Failed to load player data, using defaults:', error` 記錄到主控台。

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

你的 `loadPlayer` 函式應該傳回 `defaultPlayer` 作為備用值。
