...
現在是時候爲播放列表實現刪除功能了。 這將管理從播放列表中刪除歌曲，處理刪除歌曲時的其他相關操作，並創建一個 `Reset Playlist` 按鈕。

使用 `const` 和箭頭語法創建一個空的 `deleteSong` 函數並傳入 `id` 作爲參數。

# --hints--

你應該使用 `const` 創建一個名爲 `deleteSong` 的空函數。
