...
现在，回到 `_delete` 方法，你必须使用 `_min_value` 辅助函数来选择后继者。

在 `elif` 块之后添加 `_min_value` 调用，并将 `node.right` 作为参数传递。 将其分配给当前节点键。

# --hints--

你应该在 `elif` 块之后将 `self._min_value(node.right)` 分配给 `node.key`。
