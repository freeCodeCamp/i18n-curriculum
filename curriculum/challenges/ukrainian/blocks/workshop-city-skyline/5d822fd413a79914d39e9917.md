...
Ви можете додати кілька градієнтів до елемента, розділяючи їх комою (`,`) ось так:
...
Додайте `repeating-linear-gradient` до `.fb1c` нижче того, що вже є; використайте свій `--building-color4` від `0%` до `10%` і `--window-color4` від `10%` до `90%`. Це заповнить фон за останнім доданим градієнтом.

# --hints--

Не слід змінювати перший `repeating-linear-gradient`.
...
Після першого `,` має бути кома (`repeating-linear-gradient`).
...
Ви маєте додати `repeating-linear-gradient` з першим кольором `--building-color4` від `0%` до `10%`.
...
Використайте другий колір `--window-color4` від `10%` до `90%`.
