...
次の建物に進みましょう！`div` の中に `.fb2` 要素を2つネストし、それぞれに順番に `fb2a` と `fb2b` のクラスを付けてください。

# --hints--

`div` の中に `.fb2` 要素を2つ追加してください。

```js
assert.equal(document.querySelectorAll("div.fb2 > div")?.length, 2);
```

最初の新しい `div` に `fb2a` のクラスを付けてください。
...
2つ目の新しい `div` に `fb2b` のクラスを付けてください。
