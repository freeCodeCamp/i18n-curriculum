...
向你的 `spreadsheetFunctions` 添加 `someeven` 屬性 - 使用 `.some()` 方法檢查數組中的任何元素是否爲偶數。

# --hints--

你的 `spreadsheetFunctions` 對象應該具有 `someeven` 屬性。

```js
assert.property(spreadsheetFunctions, "someeven");
```

你的 `someeven` 屬性應該是一個函數。

```js
assert.isFunction(spreadsheetFunctions.someeven);
```

如果數組中的某些元素是偶數，則 `someeven` 函數應該返回 `true`。

```js
assert.isTrue(spreadsheetFunctions.someeven([1, 2, 3]));
```

如果數組中沒有偶數元素，則 `someeven` 函數應該返回 `false`。
