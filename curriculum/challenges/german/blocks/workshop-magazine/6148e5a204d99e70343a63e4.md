...
Die Eigenschaft `gap` ist eine Kurzschreibweise, um die Werte von `column-gap` und `row-gap` gleichzeitig festzulegen. Wenn ein Wert angegeben wird, setzt sie sowohl `column-gap` als auch `row-gap` auf diesen Wert. Wenn zwei Werte angegeben werden, setzt sie `row-gap` auf den ersten Wert und `column-gap` auf den zweiten.

Geben Sie dem Selektor `.image-wrapper` eine Eigenschaft `gap`, die auf `2rem` gesetzt ist.

# --hints--

Ihr Element `.image-wrapper` sollte eine Eigenschaft `gap` haben, die auf `2rem` gesetzt ist.
