...
모든 핵심 기능이 이제 완성되었습니다. 현재 문제는 재생 중인 노래가 끝나면 다음 노래가 자동으로 재생되지 않는다는 점입니다.

이 문제를 해결하려면, 재생 중인 노래가 끝났을 때를 감지하는 이벤트 리스너를 설정할 수 있습니다. `"ended"` 이벤트 리스너가 적합합니다. 이 이벤트는 미디어 재생이 끝에 도달했을 때 발생합니다.

`audio` 요소에 `"ended"` 이벤트를 감지하는 이벤트 리스너를 추가하세요. 화살표 구문을 사용해 중괄호가 빈 콜백을 전달하세요.

# --hints--

`addEventListener()` 변수에 `audio` 메서드를 체인으로 연결해야 합니다.
...
화살표 구문을 사용해 `"ended"` 이벤트 리스너에 빈 콜백을 전달해야 합니다.
