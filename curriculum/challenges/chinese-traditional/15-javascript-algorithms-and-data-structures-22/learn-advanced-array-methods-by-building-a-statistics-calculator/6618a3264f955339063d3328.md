...
要找出偶數表中數字的中位數，需要找出中間的兩個數字，然後計算這兩個數字的平均數。

下面介紹如何找出偶數項列表中間的兩個數字：
...
要找到中位數，你可以使用 `getMean` 函數，該函數將中間的數字相加，然後將和除以 `2`。
...
創建一個 `evenListMedian` 變量，並將找到 `testArr2` 中位數的結果賦值給它。

然後，將 `evenListMedian` 變量記錄到控制檯。

# --hints--

你應該有一個名爲 `evenListMedian` 的變量。

```js
assert.isDefined(evenListMedian);
```

你應該將找到 `testArr2` 中位數的結果賦值給 `evenListMedian` 變量。 如需更多幫助，請參考示例。

```js
assert.strictEqual(evenListMedian, 3.5);
```

你應該將 `evenListMedian` 變量記錄到控制檯。
