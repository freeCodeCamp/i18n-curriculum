...
Usando la sintassi a freccia completa la funzione di callback `forEach`. All’interno del corpo della funzione di callback usa un’assegnazione con addizione per impostare il `innerHTML` di `tasksContainer` su backtick vuoti.

# --hints--

Non dovresti modificare il `taskData.forEach()` esistente né i valori che hai destrutturato.
...
Dovresti usare la sintassi a freccia e allegare `innerHTML` a `tasksContainer`.

```js
assert.match(code, /taskData\.forEach\(\s*\(\s*\{.*\}\s*\)\s*=>\s*(\s*\{?\(\s*tasksContainer\.innerHTML\s*\)\s*\}?)?/)
```

Dovresti usare un’assegnazione con addizione per impostare il `innerHTML` di `tasksContainer` su una coppia vuota di backtick.
