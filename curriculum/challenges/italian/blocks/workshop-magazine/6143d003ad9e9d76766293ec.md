...
All'interno del tuo elemento `.image-quote`, annida un elemento `hr`, un elemento `p` e un secondo elemento `hr`. Dai all'elemento `p` un insieme `class` impostato su `quote` e il testo `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.

# --hints--

Dovresti aggiungere due elementi `hr` al tuo elemento `.image-quote`.

```js
assert.lengthOf(document.querySelectorAll('.image-quote hr'), 2);
```

Dovresti aggiungere un elemento `p` al tuo elemento `.image-quote`.

```js
assert.lengthOf(document.querySelectorAll('.image-quote p'), 1);
```

I tuoi figli `.image-quote` dovrebbero essere nel corretto ordine.
...
Il tuo nuovo elemento `p` dovrebbe avere un insieme `class` impostato su `quote`.

```js
assert.isTrue(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Il tuo nuovo elemento `p` dovrebbe avere il testo `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
