...
لجعل عناصر `input` و `textarea` تندمج مع خلفية السمة، عيّن `background-color` إلى `#0a0a23`. ثم أضف لها حدودًا `1px`، `solid` بلون `#0a0a23`.

# --hints--

يجب استخدام محدِّد عناصر مفصول بفاصلة لاختيار عناصر `input` و `textarea`.
...
يجب إعطاء عناصر `input` و `textarea` `background-color` بقيمة `#0a0a23`.
