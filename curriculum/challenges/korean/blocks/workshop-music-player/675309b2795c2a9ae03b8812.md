...
HTML의 앨범 아트와 `userData.songs` 배열의 노래가 바뀐 것을 확인하세요. 원래 노래 대신 앱을 테스트할 수 있도록 더 짧은 노래로 교체했습니다.

핵심 기능은 모두 갖춰졌습니다. 이제 남은 문제는 현재 재생 중인 노래가 끝나면 다음 노래가 자동으로 재생되지 않는다는 점입니다.

이를 고치려면 현재 재생 중인 노래가 끝났을 때 감지하는 이벤트 리스너를 설정할 수 있습니다. `"ended"` 이벤트 리스너가 적합합니다. 이 이벤트는 미디어 재생이 끝에 도달했을 때 발생합니다.

`audio` 요소에 `"ended"` 이벤트를 감지하는 이벤트 리스너를 추가하고 `playNextSong` 함수에 대한 참조를 전달하세요. 이렇게 하면 음악 플레이어 프로젝트가 완성됩니다!

# --hints--

`addEventListener()` 변수에 `audio` 메서드를 체인으로 연결하세요.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

`audio` 변수에 사용한 이벤트 리스너는 `"ended"` 이벤트를 감지해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

`playNextSong` 이벤트 리스너에 `"ended"`에 대한 참조를 전달하세요.
