...
上部のスペースはこれでよさそうですね。 メニューの一番下にある住所の下のスペースは、メニューの上端から `h1` 要素までのスペースよりも少し大きいようです。

住所が入っている `p` 要素の下のデフォルトのマージンを減らすために、`address` という名前のクラスセレクターを作成して、`margin-bottom` プロパティの値を `5px` に設定してください。

# --hints--

`.address` セレクターを追加する必要があります。
...
`.address` セレクターには値が `5px` に設定されている `margin-bottom` プロパティが必要です。
