...
次のステップは、フォーラムのデータを表示するテーブルを作成することです。

分割代入の下に、`return` キーワードとテンプレートリテラルのセットを追加してください。そのテンプレートリテラルの中に、テーブル行の `tr` 要素を追加します。

# --hints--

テンプレートリテラルのセットを返す必要があります。

```js
assert.match(code, /return\s*`[\s\S]*`/);
```

テンプレートリテラルには空の `tr` 要素を含めてください。
