...
Il costruttore `Set` accetta anche un iterabile come argomento e crea un insieme da esso:
...
Per completare questo workshop, modifica il tuo `displayPlantsSet` in modo che restituisca un insieme dei nomi comuni unici delle piante nel tuo catalogo.

Per farlo, crea un array contenente gli `commonName` di ogni oggetto pianta archiviato come chiave della mappa `catalog`. Passa l'array al costruttore `Set` e restituisci l'insieme dalla funzione.

# --hints--

La tua funzione `displayPlantsSet` dovrebbe restituire un insieme contenente gli `commonName` di ogni pianta nel tuo catalogo.
