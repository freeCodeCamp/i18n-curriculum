...
同じ重要度の要素間の水平スペースは、テキストの読みやすさを高めます。

テキスト `2/3 cup (55g)` を `span` 要素で囲んでください。

# --hints--

新しい `span` 要素を作成してください。

```js
assert(document.querySelector('span'));
```

`span` 要素にはテキスト `2/3 cup (55g)` が含まれている必要があります。

```js
assert(document.querySelector('span')?.textContent?.trim() === '2/3 cup (55g)');
```

`p` 要素には引き続きテキスト `Serving size 2/3 cup (55g)` が含まれている必要があります。
