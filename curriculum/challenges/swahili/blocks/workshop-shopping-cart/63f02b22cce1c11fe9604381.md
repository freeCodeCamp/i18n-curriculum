...
Sehemu ya jumla ya gharama itajumuisha kodi, kwa hivyo unahitaji kuhesabu hiyo pia. Unda njia ya `calculateTaxes` katika darasa la vitu la `ShoppingCart`. Njia hii inapaswa kuchukua kigezo cha `amount`.

# --hints--

Unapaswa kuunda njia ya `calculateTaxes` katika darasa la vitu la `ShoppingCart`.

```js
assert.isFunction(cart.calculateTaxes);
```

Njia yako ya `calculateTaxes` inapaswa kuchukua kigezo cha `amount`.
