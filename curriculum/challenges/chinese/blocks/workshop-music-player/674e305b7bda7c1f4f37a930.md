...
在开始实现播放下一首和上一首歌曲之前，你需要获取 `userData` 的 `songs` 属性中每首歌曲的索引。为此，你可以使用 `indexOf()` 数组方法，该方法返回给定元素在数组中首次出现的索引，如果元素不存在则返回 `-1`。
...
创建一个名为 `getCurrentSongIndex` 的函数，该函数返回 `userData.currentSong` 在 `userData.songs` 中的位置索引，当当前歌曲为 `null` 时返回 `-1`。

# --hints--

你应该有一个名为 `getCurrentSongIndex` 的函数。

```js
assert.isFunction(getCurrentSongIndex);
```

你的 `getCurrentSongIndex` 函数应返回 `userData.currentSong` 在 `userData.songs` 中的位置索引。
...
当 `userData.currentSong` 为 `null` 时，你的 `getCurrentSongIndex` 函数应返回 `-1`。
