...
ネストされた `label` を持つ `input` 要素には必須ではありませんが、`label` と対応する `input` 要素を明示的に関連付けることはベストプラクティスです。

では、4つの `for` のそれぞれに `label` 属性を追加して、`label` を対応するラジオ `input` にリンクしてください。

# --hints--

最初の `label` に `for` 属性を付けてください。

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

最初の `label` に、その `for` 要素の `id` と一致する `input` 属性を付けてください。
...
2番目の `label` に `for` 属性を付けてください。

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

2番目の `label` に、その `for` 要素の `id` と一致する `input` 属性を付けてください。
...
3番目の `label` に `for` 属性を付けてください。

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

3番目の `label` に、その `for` 要素の `id` と一致する `input` 属性を付けてください。
...
4番目の `label` に `for` 属性を付けてください。

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

4番目の `label` に、その `for` 要素の `id` と一致する `input` 属性を付けてください。
