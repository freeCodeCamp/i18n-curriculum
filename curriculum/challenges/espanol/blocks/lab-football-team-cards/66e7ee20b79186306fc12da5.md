...
1. La propiedad `position` debería tener uno de los siguientes valores: `"forward"`, `"midfielder"`, `"defender"` o `"goalkeeper"`.
...
1. Debes mostrar los datos de los jugadores en la página dentro del elemento `#player-cards`, cada jugador debe mostrarse en un `div` con la clase `player-card`, y anidado en él, un `h2` que contenga el nombre del jugador, y `(Captain)` en caso de que el jugador sea capitán, y un `p` que contenga `Position:` y la posición del jugador. 

    Aquí tienes un ejemplo de cómo debería verse el HTML:
...
La propiedad `position` debería tener uno de los valores `"forward"`, `"midfielder"`, `"defender"` o `"goalkeeper"`.
