...
Para criar um grupo de não captura em uma expressão regular, você pode adicionar `?:` após o parêntese de abertura de um grupo. Por exemplo, `(?:a|b)` corresponderá a `a` ou a `b`, mas não capturará o resultado.
...
`(hundred|thousand|million|billion)` ainda deve ser opcional.
