...
`left` 클래스를 가진 `.eye` 요소를 선택하고, 부모의 왼쪽에서 `25%`만큼 위치시키세요. 그런 다음 `right` 클래스를 가진 `.eye` 요소를 선택하고, 부모의 오른쪽에서 `25%`만큼 위치시키세요.

# --hints--

`.eye.left` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.eye.left', '.left.eye']));
```

`.eye.left`에 `left`의 `--fcc-expected--`을 주어야 하지만 `--fcc-actual--`이 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.eye.left', '.left.eye'])?.left, '25%');
```

`.eye.right` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.eye.right', '.right.eye']));
```

`.eye.right`에 `right`의 `--fcc-expected--`을 주어야 하지만 `--fcc-actual--`이 발견되었습니다.
