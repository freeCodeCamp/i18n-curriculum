...
`showFortune` メソッド内で `target` という変数を宣言し、`e.target` を代入してください。そして、もう一つ空の `if` 文を作成してください。

# --hints--

`target` 内で `showFortune` という名前の変数を宣言する必要があります。
...
`target` 変数には `e.target` を代入してください。
