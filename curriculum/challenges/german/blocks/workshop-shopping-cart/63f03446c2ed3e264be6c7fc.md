...
Beginnen Sie damit, den Wert von `isCartShowing` zu invertieren. Denken Sie daran, dass Sie den logischen Nicht-Operator `!` verwenden können, um den Wert eines Booleans zu invertieren. Weisen Sie den invertierten Wert `isCartShowing` zu.

# --hints--

Sie sollten den Wert von `isCartShowing` invertieren.

```js
assert.match(code, /!isCartShowing/);
```

Sie sollten den invertierten Wert von `isCartShowing` `isCartShowing` zuweisen.
