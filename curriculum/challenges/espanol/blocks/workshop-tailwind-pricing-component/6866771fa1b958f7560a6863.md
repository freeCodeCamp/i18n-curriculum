...
Eso mueve el texto del badge, pero no exactamente donde quieres. La magia está en las clases de utilidad `right-[num]`, `left-[num]`, `top-[num]` y `bottom-[num]`.
...
También puedes indicar valores negativos anteponiéndoles un guion:
...
Agrega las clases `-top-3` y `right-3` al `div` de la insignia `Most popular`.

# --hints--

Tu elemento `div` con el texto `Most popular` debe tener la clase `-top-3`.
...
Tu elemento `div` con el texto `Most popular` debe tener la clase `right-3`.
