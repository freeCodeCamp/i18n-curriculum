...
現在、プレイヤーがジャンプすると、キャンバスの高さを超えて移動してしまう場合があります。

これを修正するには、プレイヤーがキャンバスの高さを超えて落下しないようにする条件を追加する必要があります。

プレイヤーの`if`位置、高さ、`y`速度の合計がキャンバスの高さ以下であるかどうかをチェックする空の`y`文を作成してください。

# --hints--

あなたの`update()`メソッドには`if`文が必要です。
...
`if`文の条件は、プレイヤーの`y`位置、高さ、`y`速度の合計がキャンバスの高さ以下であるかどうかをチェックしてください。
