...
Dopo il tuo elemento `h2`, crea due elementi `p`. Dai al primo un `class` di `dessert-price` e imposta il testo su un segno di dollaro `"$"` seguito dal valore della variabile `price`. Dai al secondo un `class` di `product-category` e il testo `"Category: "` seguito dal valore della variabile `category`.
...
I tuoi elementi `p` devono venire dopo il tuo elemento `h2`.
...
Il tuo primo elemento `p` dovrebbe avere un `class` di `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Il tuo primo elemento `p` dovrebbe avere il segno di dollaro `"$"` seguito dal valore della variabile `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Il tuo secondo elemento `p` dovrebbe avere un `class` di `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Il tuo secondo elemento `p` dovrebbe avere il testo `"Category: "` seguito dal valore della variabile `category`.
