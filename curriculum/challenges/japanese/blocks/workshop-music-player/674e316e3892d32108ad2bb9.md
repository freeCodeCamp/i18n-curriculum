...
再生ボタンをクリックするたびに、プレイリストの最初の曲が常に再生されます。これを修正するには、イベントリスナーのアロー関数内で、`playSong(0)` を `if` 文に置き換え、`userData.currentSong` が `null` かどうかをチェックしてください。

`if` ブロック内で、`playSong` 関数を `id` 配列の最初の曲の `userData.songs` で呼び出してください。

# --hints--

イベントリスナーのコールバックを修正して、`playSong` が `id` のときに、`userData.songs` 配列の最初の曲の `userData.currentSong` を使って `null` 関数を呼び出すようにしてください。
