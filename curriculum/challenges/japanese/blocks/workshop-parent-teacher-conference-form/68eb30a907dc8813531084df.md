...
デフォルトでは、ラジオボタンのようなフォーム要素は、特にラベルのテキストが高さがあったりフォント設定が異なる場合、ラベルと完全に整列しないことがあります。

ラジオボタンをラベルと縦方向に整列させるには、`vertical-align` プロパティを使います。このプロパティは、インラインまたはインラインブロック要素が周囲のテキストと縦方向にどのように並ぶかを制御します。

では、ラジオボタンに対して `vertical-align` を `bottom` に設定してください。`middle` や `top` など他の値を試して整列の違いを確認することもできますが、ラジオボタンとラベルの隣接には通常 `bottom` がよく合います。

# --hints--

引き続き `.contact-method-radio-btn` クラスを対象にしてください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

`vertical-align` プロパティを `bottom` に設定してください。
