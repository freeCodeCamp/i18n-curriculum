...
Unapaswa pia kuonyesha tena nyimbo, kusitisha wimbo unaocheza kwa sasa, kuweka onyesho la mchezaji, na kuweka tena maandishi ya kitufe cha kucheza yanayopatikana.

Fanya wito wa kitendakazi cha `renderSongs` na upitie `userData?.songs` kama hoja. Pia, fanya wito wa vitendakazi vya `pauseSong`, `setPlayerDisplay`, na `setPlayButtonAccessibleText`.

# --hints--

Haupaswi kubadilisha kitendakazi cha `shuffle` kilichopo na yaliyomo ndani yake.
...
Unapaswa kufanya wito wa kitendakazi cha `renderSongs` ukiwa na `userData?.songs`.
