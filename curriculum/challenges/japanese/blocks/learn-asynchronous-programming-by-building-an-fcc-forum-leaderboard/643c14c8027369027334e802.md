...
レスポンスの本文を JSON オブジェクトとして取得したいです。`.json()` 変数の `res` メソッドは Promise を返すため、`await` する必要があります。

`data` の値を代入した `await res.json()` という定数を作成してください。

# --hints--

`const` という `data` 変数を用意してください。

```js
assert.match(code, /const\s+data/);
```

`await res.json()` 変数に `data` を代入してください。
