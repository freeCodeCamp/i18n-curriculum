...
在你的 `while` 迴圈中，透過列印以下三個字串來提供其他選單選項：`'3. Show total expenses'`、`'4. Filter expenses by category'` 和 `'5. Exit'`。持續依序添加 `print()` 呼叫。

# --hints--

你應該在你的 `while` 迴圈中有 `print('3. Show total expenses')`。
...
你應該在你的 `while` 迴圈中有 `print('4. Filter expenses by category')`。
...
你應該在你的 `while` 迴圈中有 `print('5. Exit')`。
