...
Inside your `getMode` function, on the empty line above your `return` statement, call `forEach` on `array`. Deine `.forEach()` Methode solle eine leere callback Funktion haben die ein `el` Parameter verwendet.
...
Deine `.forEach()`-Methode sollte eine Callback-Funktion haben, die einen `el`-Parameter verwendet.
