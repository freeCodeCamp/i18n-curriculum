...
请在 `.env` 文件中添加 `SESSION_SECRET`，并给它赋一个随机值。 便于加密 cookie、计算哈希。

在你做了所有这些后，告诉你的 Express 应用程序**使用** `passport.initialize()` 和 `passport.session()`。

当你认为页面已经正确时提交。如果遇到误差，你可以<a href="https://forum.freecodecamp.org/t/advanced-node-and-express/567135#set-up-passport-3" target="_blank" rel="noopener noreferrer nofollow">查看到目前为止完成的项目</a>。

# --hints--

应添加 Passport 和 Express-session 作为依赖。
