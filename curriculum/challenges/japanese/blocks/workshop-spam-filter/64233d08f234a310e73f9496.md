...
次の正規表現は `stock alert` のような文字列にマッチします。`stockRegex` 変数を宣言し、`stock alert` の文字列にマッチする正規表現を代入してください。大文字小文字を区別しないようにしてください。

また、それを `denyList` 配列にも追加してください。

# --hints--

`stockRegex` 変数を宣言してください。

```js
assert.exists(stockRegex);
```

`stockRegex` 変数には正規表現を代入してください。
