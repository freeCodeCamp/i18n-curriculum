...
这个程序应该是原始 subleq “机器代码”——空格分隔的十进制数，没有符号名称或其他程序集级扩展，加载到从地址 0 开始的内存中。  Show the output of your solution when fed this "Hello, world!" program. (Note that the example assumes ASCII or a superset of it, such as any of the Latin-N character sets or Unicode. 如果你的实现是在非 ASCII 兼容的环境中，你可以将其转换为另一个字符集。）
