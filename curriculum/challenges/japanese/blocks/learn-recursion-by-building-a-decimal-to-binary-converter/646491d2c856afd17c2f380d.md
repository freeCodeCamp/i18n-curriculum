...
各アニメーションフレームのタイミングが `addElDelay` に保存されているので、その値を `setTimeout()` と組み合わせて、DOMに要素を追加する遅延を設定できます。

`.forEach()` メソッドのコールバック関数の本体内に、`setTimeout()` 関数を追加してください。最初の引数に空のコールバック関数を渡し、2番目の引数に `obj.addElDelay` を渡してください。

# --hints--

`setTimeout()` メソッドのコールバック関数の本体内に `.forEach()` 関数を追加する必要があります。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\([\s\S]+\{\s*setTimeout\(/);
```

`setTimeout()` 関数の最初の引数に空のコールバック関数を渡してください。
