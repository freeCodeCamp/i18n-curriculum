...
Ви можете покращити читабельність і підтримуваність коду, розділивши обробник події `submit` на дві окремі функції. Перша функція може додавати вхідні значення до `taskData`, а друга відповідатиме за додавання завдань у DOM.

Використайте стрілковий синтаксис для створення функції `addOrUpdateTask`. Потім перемістіть змінну `dataArrIndex`, об’єкт `taskObj` і інструкцію `if` у функцію `addOrUpdateTask`.

# --hints--

Ви повинні використати `const` та стрілковий синтаксис для створення функції `addOrUpdateTask`.

```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Ви повинні перемістити змінну `dataArrIndex` у функцію `addOrUpdateTask`.
...
Ви повинні перемістити об’єкт `taskObj` у функцію `addOrUpdateTask`.
...
Ви повинні перемістити інструкцію `if` з умовою `dataArrIndex === -1` у вашу функцію `addOrUpdateTask`.
