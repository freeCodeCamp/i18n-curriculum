...
Ihre `addCharacters`-Funktion gibt letztlich einen Bereich von Zeichen zurück. Sie möchten, dass sie ein Array von Zell-IDs zurückgibt. Verketten Sie die `.map()`-Methode an Ihren `charRange()`-Aufruf. Übergeben Sie noch keine Callback-Funktion.

# --hints--

Sie sollten `.map()` an Ihren `charRange()`-Aufruf verketten.
...
Sie sollten Ihrem `.map()`-Aufruf nichts übergeben.
