...
Recuerda que puedes usar destructuración para extraer múltiples valores de un arreglo u objeto en una sola instrucción.

Para el primer parámetro de tu función callback, desestructura las propiedades `name`, `id`, `price` y `category` de la instancia `Dessert` que se pasa.

# --hints--

Debes usar destructuración para declarar los parámetros `name`, `id`, `price` y `category`. Para esta prueba, el orden importa.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

Tu destructuración debe ser el primer parámetro de la función callback.
