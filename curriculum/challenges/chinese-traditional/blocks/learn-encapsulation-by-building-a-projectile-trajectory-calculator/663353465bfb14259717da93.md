...
創建一個方法 `create_coordinates_table` ，然後在代碼末尾打印 `graph.create_coordinates_table())`。

該方法應使用 `__coordinates` 屬性並返回包含所有座標的字符串，其中 x 和 y 始終具有相同的位置，x 的值爲整數，y 的值四捨五入到小數點後兩位：
