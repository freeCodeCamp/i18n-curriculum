...
Se essa condição for atendida, desabilite o botão definindo a propriedade `disabled` como `true`. Além disso, defina o `textContent` do botão para `"No more data to load"`.
...
Você deve definir a propriedade `disabled` de `loadMoreBtn` como `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Você deve definir o `textContent` de `loadMoreBtn` para `"No more data to load"`.
