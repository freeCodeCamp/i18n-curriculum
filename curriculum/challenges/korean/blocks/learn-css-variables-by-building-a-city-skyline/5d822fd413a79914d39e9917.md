...
쉼표(`,`)로 구분하여 요소에 여러 개의 그라디언트를 추가할 수 있습니다. 예를 들어 다음과 같습니다:
...
아래에 있는 `repeating-linear-gradient` 바로 아래에 `.fb1c`을 추가하세요. `--building-color4`에서 `0%`까지 `10%`을, `--window-color4`과 `10%`에서 `90%`을 사용하세요. 이렇게 하면 마지막에 추가한 그라디언트 뒤를 채울 수 있습니다.

# --hints--

첫 번째 `repeating-linear-gradient`은 변경하지 마세요.
...
`repeating-linear-gradient`에서 `--building-color4`까지 `0%`을 첫 번째 색상으로 하는 `10%`을 추가해야 합니다.
...
`--window-color4`에서 `10%`까지 `90%`를 두 번째 색상으로 사용해야 합니다.
