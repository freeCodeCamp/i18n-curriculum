...
Sotto l'elemento con il testo `Total Fat`, crea un nuovo elemento `p` con il testo `Saturated Fat 1g 5%`. Includi `5%` in un altro elemento `span` con l'attributo `class` impostato su `bold`. In questo caso è sufficiente per allineare la percentuale al `5%`.
