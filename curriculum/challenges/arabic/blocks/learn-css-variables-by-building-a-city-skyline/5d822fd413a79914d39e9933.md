...
أضف `repeating-linear-gradient` آخر أسفل الذي أضفته للتو. اجعله باتجاه `90deg`، واستخدم لون المبنى من `0%` إلى `12%` ولون النافذة من `12%` إلى `44%`. هذا سيُنتج مجموعة من النوافذ المستطيلة.

# --hints--

يجب أن تعطي `.fb5` `repeating-linear-gradient` ثاني في خاصية `background`.
...
يجب أن تعطي `repeating-linear-gradient` الثاني اتجاهًا بقيمة `90deg`.
...
يجب أن تعطي `repeating-linear-gradient` الثاني لونًا أولًا من `--building-color2` من `0%` إلى `12%`.
...
يجب أن تعطي `repeating-linear-gradient` الثاني لونًا ثانيًا من `--window-color2` من `12%` إلى `44%`.
