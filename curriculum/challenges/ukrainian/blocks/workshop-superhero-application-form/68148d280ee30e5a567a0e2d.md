...
Видаліть текст `Hello` всередині `label`, потім створіть `input` типу `checkbox` і `value` типу `power`.

Нарешті, відобразіть значення з масиву поруч із кожним прапорцем, створивши елемент `span` безпосередньо перед закриваючим `label` з текстовим вмістом `{power}`.

# --hints--

Ви повинні видалити текст `Hello` з елемента `label`.

```js
assert.notMatch(code, /hello/i)
```

Ви повинні створити `input` з `type`, встановленим у `checkbox`, і `value`, встановленим у `power`, як перший дочірній елемент мітки.

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

Ви повинні створити елемент `span` з `{power}` як текстовим вмістом, розміщений останнім дочірнім елементом мітки.
