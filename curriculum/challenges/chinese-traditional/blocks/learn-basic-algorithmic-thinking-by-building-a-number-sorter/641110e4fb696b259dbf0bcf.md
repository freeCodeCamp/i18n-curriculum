...
對於這個演算法，你會想使用 `while` 迴圈。這個迴圈需要兩個條件：

- 首先，它不應該超出陣列的起始位置（以 `j` 存取）。
- 其次，當迴圈找到比目前值更小的值時，不應該繼續執行。
...
你的 `while` 迴圈應該有第一個條件，檢查 `j` 的值是否大於或相等於 `0`。
...
你的 `while` 迴圈應該有一個條件，用來檢查 `array[j]` 的值是否大於 `currValue`。
