...
Fügen Sie innerhalb des `preview-panel`-Elements ein `p`-Element von `className` von `preview-label` und den Text `Live Preview` hinzu, ein weiteres `p`-Element von `className` von `preview-hint` und den Text `Updates as you type` sowie ein `div` mit dem `className`, das auf das Template-Literal `preview-box tier-${getPlayerTier(player.overallRating)}` gesetzt ist.

Verschieben Sie innerhalb des letzten `div` die `PlayerCard`-Komponente und deren Prop.

# --hints--

Ihr `div.preview-panel`-Element sollte ein `p` mit dem `className` von `preview-label` und dem Text `Live Preview` enthalten.
...
Ihr `div.preview-panel` sollte ein `p` mit dem `className` von `preview-hint` und dem Text `Updates as you type` enthalten.
...
Ihr `div.preview-panel` sollte ein `div` mit dem `className` von `preview-box` und einem dynamischen Tier-`className` enthalten, das mit `tier-` als Präfix versehen ist.
...
Ihr `PlayerCard` sollte innerhalb des `div`-Elements mit `className` von `preview-box` gerendert werden.
