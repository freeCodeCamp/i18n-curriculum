...
داخل `thead`، أنشئ عنصر `tr`. أضف فيه عنصر `td` وثلاثة عناصر `th`. داخل كل عنصر من عناصر `th`، أضف عنصر `span` مع تعيين `class` إلى `sr-only` والنص التالي بالترتيب: `2019`، `2020`، و`2021`.
...
يجب أن تحتوي كل عناصر `span` الجديدة على الخاصية `class` مع القيمة `sr-only`.
