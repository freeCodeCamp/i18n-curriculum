...
داخل عنصر `form`، بعد المسمى، أضف مُدخَلًا له `id` بقيمة `search-input` و`type` بقيمة `search`.

# --hints--

يجب أن يكون لديك `input` داخل عنصر `form` له `id` بقيمة `search-input`.
...
يجب أن يكون لـ `input` الخاص بك قيمة `type` تساوي `search`.
