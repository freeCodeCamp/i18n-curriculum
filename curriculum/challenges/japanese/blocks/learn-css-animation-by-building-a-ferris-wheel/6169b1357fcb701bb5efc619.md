...
`25%` と `0%` のセレクターの間に新しい `50%` セレクターを作成してください。この新しいセレクターに `background-color` プロパティを `yellow` に設定してください。

# --hints--

`25%` ルール内に新しい `@keyframes cabins` セレクターを作成する必要があります。
...
`25%` セレクターは `0%` と `50%` のセレクターの間にあるべきです。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '25%');
```

`25%` セレクターには `background-color` プロパティを `yellow` に設定してください。
