...
Du solltest `titleInput.value` auf einen leeren String setzen.

```js
assert.match(reset.toString(), /titleInput\.value\s*=\s*('|")\1\s*;?/)
```

Du solltest `dateInput.value` auf einen leeren String setzen.

```js
assert.match(reset.toString(), /dateInput\.value\s*=\s*('|")\1\s*;?/)
```

Du solltest `descriptionInput.value` auf einen leeren String setzen.

```js
assert.match(reset.toString(), /descriptionInput\.value\s*=\s*('|")\1\s*;?/)
```

Du solltest `classList` verwenden, um die Klasse `hidden` im `taskForm` umzuschalten

```js
assert.match(reset.toString(), /taskForm\.classList\.toggle\(\s*('|")hidden\1\s*\)/)
```

Du solltest `currentTask` auf ein leeres Objekt setzen.
