...
如果箭頭函數返回一個簡單的表達式，則可以省略 `return` 關鍵字和花括號 `{}`。 這被稱爲隱式返回。

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

如果你的箭頭函數在函數體中有多行代碼，那麼你需要使用 `return` 關鍵字和花括號 `{}`。
...
重構或更新你的 `addTwoNumbers` 函數以刪除 `return` 關鍵字和花括號 `{}`。 你的 `addTwoNumbers` 函數應該使用隱式返回。

打開控制檯以確保你仍然獲得正確的輸出。

# --hints--

你重構的 `addTwoNumbers` 函數應該使用隱式返回。
