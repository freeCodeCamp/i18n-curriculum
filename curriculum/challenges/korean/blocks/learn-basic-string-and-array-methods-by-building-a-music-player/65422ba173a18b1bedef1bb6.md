...
이벤트 리스너의 화살표 함수 안에 `if`를 추가하여 `userData?.currentSong`이 `null`인지 확인하세요.

`if` 블록 안에서 `playSong()` 함수를 `id` 배열의 첫 번째 노래의 `userData?.songs`와 함께 호출하세요. 이렇게 하면 재생 목록에서 첫 번째 노래가 먼저 재생됩니다.

# --hints--

조건 `if`을 가진 `userData?.currentSong === null` 문을 만드세요.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{/);
```

`playSong` 블록 안에서 `userData?.songs[0].id`과 함께 `if` 함수를 호출하세요.
