...
Il selettore `.hero-title` dovrebbe avere una proprietà `text-align` impostata su `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign === 'center');
```

Il selettore `.hero-title` dovrebbe avere una proprietà `color` impostata su `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color === 'orangered');
```

Il selettore `.hero-title` dovrebbe avere una proprietà `font-size` impostata su `8rem`.
