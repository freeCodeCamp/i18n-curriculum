...
이제 `showElements` 안에서 `singleCardBtn`, `multipleCardsBtn`, `headerTitle`를 인수로 전달하세요. 이들은 `elements`에서 가져오며 `this` 키워드로 접근할 수 있습니다.

# --hints--

`singleCardBtn` 함수에 첫 번째 인수로 `showElements`를 전달해야 합니다.
...
`multipleCardsBtn` 함수에 두 번째 인수로 `showElements`를 전달해야 합니다.
...
`headerTitle` 함수에 인수로 `showElements`를 전달해야 합니다.
