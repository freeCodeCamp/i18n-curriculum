...
前のモジュールでは、次のように`NodeList`を繰り返し処理する方法を学びました。
...
`forEach`メソッドを使って`tabs`を繰り返し処理してください。コールバック関数には、単一のパラメータ`tab`を受け取るアロー関数を使いましょう。

# --hints--

コードには`tabs.forEach(tab => {})`が含まれているはずです。
