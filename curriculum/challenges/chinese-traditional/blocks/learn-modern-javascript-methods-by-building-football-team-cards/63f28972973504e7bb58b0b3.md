...
下一步是創建一個函式（程式），用來偵測使用者何時從 `playersDropdownList` 中做出選擇。 

在 `playersDropdownList` 上使用 `.addEventListener()` 方法。在事件監聽器內，傳入 `"change"` 事件型別和一個空的回呼函式（函數）。

# --hints--

你應該對 `playersDropdownList` 變數呼叫 `.addEventListener()` 方法。
