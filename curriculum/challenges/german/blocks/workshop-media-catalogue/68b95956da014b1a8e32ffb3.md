...
In einem vorherigen Modul haben Sie gelernt, dass Sie eigene benutzerdefinierte Ausnahmen erstellen können, indem Sie von der eingebauten `Exception` Klasse erben:
...
Erstellen Sie eine Klasse namens `MediaError`. Lassen Sie sie von der `Exception` Klasse erben und fügen Sie ihr einen Docstring hinzu, der `Custom exception for media-related errors.` sagt.

# --hints--

Sie sollten eine Klasse namens `MediaError` erstellen.
...
Ihre `MediaError` Klasse sollte von `Exception` erben.
...
Sie sollten den Docstring `Custom exception for media-related errors.` zu Ihrer `MediaError` Klasse hinzufügen.
