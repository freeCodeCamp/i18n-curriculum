...
`.getElementById()` メソッドを使って、現在のオブジェクトの `id` プロパティの値を持つ `inputVal` 属性を持つ要素をターゲットにしてください。その後、その要素に対して `.remove()` メソッドを使い、遅延後に DOM から削除してください。

# --hints--

`.getElementById()` メソッドを使って、現在のオブジェクトの `id` と一致する `inputVal` 属性を持つ要素をターゲットにする必要があります。
...
`.remove()` メソッドを `.getElementById()` メソッドにチェーンしてください。
