...
<a href="https://rosettacode.org/wiki/Cramer%27s_rule" target="_blank" rel="noopener noreferrer nofollow">크래머의 공식</a>은 계수와 우변 값의 부분집합으로 구성된 행렬의 행렬식을 사용해 연립일차방정식을 푸는 공식입니다.

2행 2열 행렬의 행렬식은 다음과 같습니다:
...
$\\begin{bmatrix} a_1 & b_1 & c_1 \\\\ a_2 & b_2 & c_2 \\\\ b_3 & b_3 & c_3 \\end{bmatrix}\\begin{bmatrix} x \\\\ y \\\\ z \\end{bmatrix}=\\begin{bmatrix} {\\color{red}d_1} \\\\ {\\color{red}d_2} \\\\ {\\color{red}d_3} \\end{bmatrix}.$

그렇다면 $x, y, z$의 값은 다음과 같이 구할 수 있습니다:
...
`cramersRule`는 함수여야 합니다.

```js
assert(typeof cramersRule === 'function');
```

`cramersRule([[2, -1, 5, 1], [3, 2, 2, -6], [1, 3, 3, -1], [5, -2, -3, 3]], [-3, -32, -47, 49])`는 `[2, -12, -4, 1]`를 반환해야 합니다.

```js
assert.deepEqual(cramersRule(matrices[0], freeTerms[0]), answers[0]);
```

`cramersRule([[3, 1, 1], [2, 2, 5], [1, -3, -4]], [3, -1, 2])`는 `[1, 1, -1]`를 반환해야 합니다.
