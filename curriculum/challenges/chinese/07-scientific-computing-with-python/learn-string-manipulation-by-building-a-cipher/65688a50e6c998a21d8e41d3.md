...
字符串是不可变的，这意味着一旦创建，它们就不能被更改。 For example, you might think that the following code changes the value of `my_string` into the string `'train'`, but this is not valid:
