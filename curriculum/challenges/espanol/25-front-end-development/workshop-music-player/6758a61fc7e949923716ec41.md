...
Ahora, usa el método `querySelectorAll` para seleccionar los elementos `.playlist-song` y almacenarlos en una variable llamada `songs`.

Luego, llama al método `forEach` sobre `songs`. Pasa una función de retorno que utiliza `song` como parámetro del método. Deja las llaves vacías por ahora.

# --hints--

Deberías tener una variable llamada `songs`.

```js
assert.isDefined(songs);
```

Deberías asignar la colección de todos los elementos `.playlist-song` a tu variable `songs`.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

Deberías llamar al método `forEach` sobre `songs` y pasar un retorno que usa `song` como parámetro al método.
