...
ينطبق نفس النمط على الحقول الثلاثة الأخرى. أضف فحوصات مكافئة لـ `author` و `year` و `location`.

لاحظ أن `isValid` يتراكم عبر جميع الفحوصات الأربع: إذا فشل أي حقل، يصبح `false` ويبقى `false` حتى لو نجحت الفحوصات المتبقية. هذا يعني أن الدالة تفحص كل حقل دائمًا بدلاً من التوقف عند المشكلة الأولى.

# --hints--

يجب أن تُرجع الدالة `validateEntry(catalog[0])` القيمة `true`.

```js
assert.isTrue(validateEntry(catalog[0]));
```

يجب أن تُرجع الدالة `validateEntry(catalog[16])` القيمة `false`.

```js
assert.isFalse(validateEntry(catalog[16]));
```

يجب أن تُرجع الدالة `validateEntry(catalog[17])` القيمة `false`.

```js
assert.isFalse(validateEntry(catalog[17]));
```

يجب أن تُرجع الدالة `validateEntry(catalog[18])` القيمة `false` لأن الموقع هو `"Unknown"`.
