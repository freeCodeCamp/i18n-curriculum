...
`healthText.innerText` と `monsterHealthText.innerText` を、`health` と `monsterHealth` に等しくなるように更新してください。

# --hints--

`healthText.innerText` を更新してください。

```js
assert.match(attack.toString(), /healthText\.innerText/);
```

`healthText.innerText` を `health` に設定してください。

```js
assert.match(attack.toString(), /healthText\.innerText\s*=\s*health/);
```

`monsterHealthText.innerText` を更新してください。

```js
assert.match(attack.toString(), /monsterHealthText\.innerText/);
```

`monsterHealthText.innerText` を `monsterHealth` に設定してください。
