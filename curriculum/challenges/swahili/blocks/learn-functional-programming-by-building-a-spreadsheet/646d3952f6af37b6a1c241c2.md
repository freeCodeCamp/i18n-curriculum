...
Sasa kitendakazi chako cha `.map()` kinapokea rejea ya kitendakazi kilichorejeshwa cha `num => charRange(...).map(...)` kutoka kwa wito zilizopangwa za `addCharacters`, kitapita vizuri juu ya vipengele na kupitisha kila kipengele kama `n` kwa kitendakazi hicho.

Utaona kwamba hutatumi kigezo chako cha `match`. Katika JavaScript, ni mkataba wa kawaida kuweka kiambishi awali cha mstari chini `_` kwa kigezo kisichotumika. Pia unaweza kuacha kigezo hicho wazi kama hivi: `(, char1)` lakini mara nyingi ni wazi zaidi kumnukuu kigezo kwa ajili ya usomeaji wa baadaye.

Weka kiambishi awali cha mstari chini kwa kigezo chako cha `match`.

# --hints--

Unapaswa kuweka kiambishi awali cha mstari chini kwa kigezo chako cha `match`.
