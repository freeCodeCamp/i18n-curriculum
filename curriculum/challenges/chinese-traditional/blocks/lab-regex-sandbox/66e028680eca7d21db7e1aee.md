...
在這個實驗中，你會從已經寫好的 CSS 和 HTML 開始。你將使用 JavaScript 來啟用正規表達式沙盒，測試正規表達式對字串的匹配並凸顯標示結果。
 
**目標：** 完成以下使用者故事並通過所有測試以完成實驗。

**使用者故事：**

1. 你應該存取 `#pattern`、`#test-string`、`#test-btn` 和 `#result` 元素，並分別將它們存入變數 `regexPattern`、`stringToTest`、`testButton` 和 `testResult`。
1. 你應該存取 `#i` 和 `#g` 旗標核取方塊，並分別將它們存入變數 `caseInsensitiveFlag` 和 `globalFlag`。
1. 你應該有一個名為 `getFlags` 的函式（程式），它會傳回一個字串，包含已勾選的旗標核取方塊的旗標，若沒有勾選任何旗標則傳回空字串。
1. 當你點擊 `testButton` 按鈕時，如果正規表達式樣式與測試字串匹配，匹配的文字應該被凸顯標示。為了凸顯匹配結果，更新 `stringToTest` 的 `innerHTML`，將每個匹配的文字替換成被一個帶有 `highlight` 類別的 `span` 元素包圍的相同文字。注意相關的 CSS 已經為你加入。
1. 當有匹配時，匹配的文字應該顯示在 `#result` 裡。如果有多個匹配，每個匹配的文字之間應以逗號和空格分隔。
1. 當沒有匹配時，`#result` 裡應顯示文字 `no match`。
