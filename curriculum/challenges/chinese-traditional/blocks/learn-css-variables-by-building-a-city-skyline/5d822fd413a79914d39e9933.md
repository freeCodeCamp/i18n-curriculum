...
在剛剛添加的漸變下面添加另一個 `repeating-linear-gradient`。 給它一個 `90deg` 的方向，從 `0%` 到 `12%` 使用其建築顏色，從 `12%` 到`44%` 使用窗戶顏色。 這將形成一些矩形窗戶。

# --hints--

你應該在 `background` 屬性中爲 `.fb5` 添加第二個 `repeating-linear-gradient`。
...
你應該給第二個 `repeating-linear-gradient` 一個 `90deg` 的方向。
...
你應該將從 `0%` 到 `12%` 的 `--building-color2` 作爲第二個 `repeating-linear-gradient` 的第一個顏色。
...
你應該將從 `12%` 到 `44%` 的 `--window-color2` 作爲第二個 `repeating-linear-gradient` 的第二個顏色。
