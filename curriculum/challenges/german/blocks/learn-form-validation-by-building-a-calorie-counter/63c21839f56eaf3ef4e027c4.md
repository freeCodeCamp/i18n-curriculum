...
Erstelle am Ende deines `HTMLString` ein weiteres `label`-Element (in einer neuen Zeile). Dieses `label` sollte den Text `Entry # Calories` enthalten, wobei du die Template-Literal verwendest, um `#` durch den Wert von `entryNumber` zu ersetzen und das `for`-Attribut auf `X-#-calories` zu setzen, wobei `X` die `value` von `entryDropdown` und `#` der Wert von `entryNumber` ist.

# --hints--

Du solltest zwei `label`-Elemente in deinem `HTMLString` haben.
...
Dein neues `label`-Element sollte in einer neuen Zeile stehen.
...
Dein neues `label`-Element sollte nach deinem `input`-Element stehen.
...
Dein neues `label`-Element sollte ein `for`-Attribut auf `${entryDropdown.value}-${entryNumber}-calories` gesetzt haben.
...
Dein neues `label`-Element sollte den Text `Entry ${entryNumber} Calories` haben.
...
Du solltest deine vorhandenen Elemente nicht verändern.
