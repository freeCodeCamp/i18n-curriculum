...
Тепер, коли ви знаєте, до яких воріт перенаправити, можна перемістити учасників переповнення.

Візьміть `overflowAmount` і додайте його до черги наступних воріт на тому ж `tickIndex`.

Зокрема, потрібно:

- Отримати доступ до наступних воріт за допомогою `nextGateIndex`.
- Отримати доступ до їхнього `queue`.
- Додати `overflowAmount` до значення на позиції `tickIndex`.

# --hints--

Ваша функція `rerouteOverflow` має додати `overflowAmount` до значення на `tickIndex` у `queue` наступних воріт.
