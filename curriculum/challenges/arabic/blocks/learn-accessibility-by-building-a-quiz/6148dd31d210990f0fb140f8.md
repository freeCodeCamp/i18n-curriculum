...
من المفيد رؤية الحدود الافتراضية حول عناصر `fieldset` أثناء التطوير. مع ذلك، قد لا تكون هذه هي الطريقة التي تريدها.

احذف الحدود والحشو السفلي على عناصر `.question`.

# --hints--

يجب أن تستخدم المحدِّد `.question`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

يجب أن تعطي عنصر `.question` قيمة `border` تساوي `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

يجب أن تعطي `.question` قيمة `padding-bottom` تساوي `0`.
