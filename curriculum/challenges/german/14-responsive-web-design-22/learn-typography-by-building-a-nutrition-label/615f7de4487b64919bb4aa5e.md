...
Erstelle nach deinem letzten `.divider` ein neues `p`-Element mit dem Text `Cholesterol 0mg 0%`. Dann schließe den Text `Cholesterol` in ein `span` Element ein und `0%` in ein weiteres, und gib jedem der beiden eine Klasse von `bold`.

Verschachtele schließlich das `span` Element, das den Text `Cholesterol` mit dem Text `0mg`, in einem zusätzlichen `span` Element für die Ausrichtung.
...
Dein `Cholseterol` `span` sollte eine `class` mit dem Attribut von `bold` haben.
...
Dein `0%` `span` sollte eine `class` mit dem Attribut von `bold`.
...
Dein `Cholesterol` `span` und dein `0mg` Text sollte in ein `span` verschachtelt sein.
