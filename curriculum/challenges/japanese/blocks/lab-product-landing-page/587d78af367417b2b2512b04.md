...
1. プロダクトランディングページには、対応する`header`を持つ`id="header"`要素が必要です。
2. `header`要素内に、対応する`id="header-img"`を持つ画像が必要です（ここではロゴが良い画像になります）。
3. `#header`要素内に、対応する`nav`を持つ`id="nav-bar"`要素が必要です。
4. `nav`要素の中に、クラス`nav-link`を持つクリック可能な要素を少なくとも3つ含めてください。
5. `.nav-link`要素内の`nav`ボタンをクリックすると、ランディングページの対応するセクションに移動する必要があります。
6. `id="video"`を持つ埋め込みのプロダクト動画が必要です。
7. ランディングページには、対応する`form`を持つ`id="form"`要素が必要です。
8. フォーム内に、メールアドレスを入力できる`input`を持つ`id="email"`フィールドが必要です。
9. `#email`入力フィールドには、ユーザーにフィールドの目的を知らせるプレースホルダーテキストが必要です。
10. `#email`入力フィールドは、入力されたテキストがメールアドレスであることを確認するHTML5バリデーションを使用してください。
11. フォーム内に、対応する`input`を持つ送信`id="submit"`が必要です。
12. `#submit`要素をクリックすると、メールが静的ページに提出される必要があります（このモックURLを使用してください：`https://www.freecodecamp.org/email-submit`）。
13. ナビゲーションバーは常にビューポートの上部に表示される必要があります。
14. プロダクトランディングページには少なくとも1つのメディアクエリが必要です。
15. プロダクトランディングページは少なくとも1回CSSのflexboxを利用してください。
...
`#header-img`の`src`値は有効なURL（`http`で始まる）である必要があります。
...
`#email`は`type`を`email`に設定してHTML5バリデーションを使用する必要があります。
...
`#form`は`action`属性が`https://www.freecodecamp.org/email-submit`である必要があります。
...
プロダクトランディングページは少なくとも1つのメディアクエリを使用する必要があります。
...
プロダクトランディングページは少なくとも1回CSSのFlexboxを使用する必要があります。
