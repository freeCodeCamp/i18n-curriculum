...
تريد الحصول على جسم الاستجابة ككائن JSON. طريقة `.json()` للمتغير `res` تُرجع وعدًا، مما يعني أنك ستحتاج إلى `await` له.

أنشئ ثابتًا اسمه `data` وقم بتعيين قيمة `await res.json()` له.

# --hints--

يجب أن يكون لديك متغير `const` اسمه `data`.

```js
assert.match(code, /const\s+data/);
```

يجب تعيين `await res.json()` إلى المتغير `data`.
