...
`aside` 要素の中に、2つの `img` 要素、1つの `blockquote` 要素、そして3つ目の `img` 要素を作成してください。`blockquote` 要素には、`class` を `image-quote` に設定してください。

# --hints--

`img` 要素の中に3つの `aside` 要素を作成する必要があります。

```js
assert(document.querySelectorAll('aside img')?.length === 3);
```

`blockquote` 要素の中に `aside` 要素を作成してください。

```js
assert.exists(document.querySelector('aside blockquote'));
```

`blockquote` 要素には `class` を `image-quote` に設定してください。

```js
assert(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

新しい要素は正しい順序で配置してください。
