...
`.social-icons` 선택자를 만드세요. `display` 속성을 `grid`로 설정하고, `font-size` 속성을 `3rem`로 설정하세요.

# --hints--

`.social-icons` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

`.social-icons` 선택자는 `display` 속성을 `grid`로 설정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

`.social-icons` 선택자는 `font-size` 속성을 `3rem`로 설정해야 합니다.
