...
Ваш перший будинок тепер виглядає досить добре. Вкладіть три нові елементи `div` у контейнер `.background-buildings` і надайте їм класи `bb2`, `bb3` та `bb4` у такому порядку. Це будуть ще три будинки для фону.
...
Ви повинні створити 3 нові елементи `div`.

```js
assert.equal(document.querySelectorAll('div')?.length, 9);
```

Ви повинні розмістити ці елементи `div` всередині елемента `.background-buildings`.

```js
assert.equal(document.querySelector('div.background-buildings')?.children?.length, 4);
```

Ви повинні розмістити елементи у правильному порядку.
