...
배열의 요소를 정렬하려면 `.sort()`라는 내장 메서드를 사용할 수 있습니다. 따라서 `sortedValues` 배열에서 `.sort()`를 호출한 결과를 할당하여 `inputValues` 변수를 업데이트할 수 있습니다.

# --hints--

`sortedValues` 배열에서 `.sort()` 메서드를 호출한 결과로 `inputValues` 변수를 업데이트하세요.

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(/);
```

`.sort()` 메서드에 아무것도 전달하지 마세요.

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*\)/);
```

코드에서 `selectionSort`를 호출하지 마세요.
