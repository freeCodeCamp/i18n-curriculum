...
Створіть правило `.image-1, .image-3` та надайте йому властивість `grid-column` зі значенням `1 / -1`. This will allow the first and third images to span the full width of the grid.

# --hints--

Ви повинні мати селектор `.image-1, .image-3`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

Ваш селектор `.image-1, .image-3` повинен мати властивість `grid-column` зі значенням `1 / -1`.
