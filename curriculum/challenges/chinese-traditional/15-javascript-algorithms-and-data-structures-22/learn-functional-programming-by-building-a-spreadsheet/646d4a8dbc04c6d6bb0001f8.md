...
添加一個 `range` 屬性，生成從 `nums` 中的第一個數字到 `nums` 中的第二個數字的範圍。 請記住，你有一個可以在此處重複使用的 `range` 函數。

# --hints--

你的 `spreadsheetFunctions` 對象應該具有 `range` 屬性。
...
你的 `range` 函數應該返回從參數中的第一個數字到參數中的第二個數字的連續數字數組。
