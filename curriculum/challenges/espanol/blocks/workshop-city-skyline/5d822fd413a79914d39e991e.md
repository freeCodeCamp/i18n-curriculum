...
Debes darle a los elementos `.fb2-window` un `width` de `22%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

Debes darle a los elementos `.fb2-window` un `height` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

Debes darle a los elementos `.fb2-window` un `background-color` de `--window-color3`.
