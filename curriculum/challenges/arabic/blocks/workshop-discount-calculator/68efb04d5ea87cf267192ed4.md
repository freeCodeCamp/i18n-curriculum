...
أنشئ مثيلًا من `DiscountEngine` واحفظه في المتغير `engine`. بعد ذلك، احسب أفضل سعر باستخدام طريقة `calculate_best_price` واحفظه في متغير باسم `best_price`.

# --hints--

يجب أن تنشئ مثيلًا من `DiscountEngine` باستخدام قائمة `strategies`.
...
يجب أن تستدعي `engine.calculate_best_price(product, user_tier)` وتحفظ النتيجة في `best_price`.
