...
2番目の色の引数として、`hsl` 関数に `223` を色相、`90%` を彩度、`60%` を明度の値として渡してください。

# --hints--

あなたの `.blue` CSS ルールには、`background` プロパティが `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%))` に設定されている必要があります。
