...
現在，使用 `onload` 事件在影像載入時移除 `hidden` 類別，並使用其 `previousElementSibling` 隱藏載入中的佔位符。

# --hints--

你的 `img` 元素應該有一個 `onload` 處理常式。
...
你的 `onload` 處理常式應該使用 `classList.remove` 來移除 `hidden` 類別。別忘了這裡需要 `this` 關鍵字。
...
你的 `onload` 處理常式應該將 `previousElementSibling.style.display` 設為 `'none'`。別忘了這裡需要 `this` 關鍵字。
