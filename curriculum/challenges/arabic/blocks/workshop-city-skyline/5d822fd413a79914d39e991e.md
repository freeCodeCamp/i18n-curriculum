...
امنح عناصر `.fb2-window` `width` بقيمة `22%`، و`height` بقيمة `100%`، و`background-color` باستخدام متغير `--window-color3` الخاص بك.

# --hints--

يجب أن تمنح عناصر `.fb2-window` `width` بقيمة `22%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

يجب أن تمنح عناصر `.fb2-window` `height` بقيمة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

يجب أن تمنح عناصر `.fb2-window` `background-color` بقيمة `--window-color3`.
