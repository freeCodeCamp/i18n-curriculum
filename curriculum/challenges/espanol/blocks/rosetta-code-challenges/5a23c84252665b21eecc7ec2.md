...
Dos caracteres de \(s_1\) y \(s_2\) respectivamente, se consideran *coincidentes* solo si son iguales y no están a más de \(\left\lfloor\frac{\max(|s_1|,|s_2|)}{2}\right\rfloor-1\) de distancia.

Cada carácter de \(s_1\) se compara con todos sus caracteres coincidentes en \(s_2\). El número de caracteres coincidentes (pero en diferente orden de secuencia) dividido por 2 define el número de *transposiciones*.

**Ejemplo**

Dadas las cadenas \(s_1\) *DWAYNE* y \(s_2\) *DUANE* encontramos:
