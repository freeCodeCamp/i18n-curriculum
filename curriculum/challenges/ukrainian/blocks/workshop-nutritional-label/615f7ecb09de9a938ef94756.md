...
Під останнім елементом `p` додайте ще один елемент `p` з текстом `Total Carbohydrate 37g 13%`. Як і раніше, використайте елементи `span`, щоб зробити шрифт текстів `Total Carbohydrate` та `13%` жирним. Потім обгорніть поживну речовину та її кількість в `span` з метою центрування.

# --hints--

Створіть новий елемент `p` в кінці елемента `.daily-value`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'p');
```

Новий елемент `p` повинен мати текст `Total Carbohydrate 37g 13%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText?.match(/Total Carbohydrate[\s|\n]+37g[\s|\n]+13%/));
```

Новий елемент `p` повинен мати три елементи `span`.
...
Другий елемент `span` повинен обгортати текст `13%`.
