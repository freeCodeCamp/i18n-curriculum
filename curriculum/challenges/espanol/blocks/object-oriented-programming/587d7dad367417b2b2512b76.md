...
Aunque esta es una forma válida de acceder a la propiedad del objeto, aquí existe un error. Si el nombre de la variable cambia, cualquier código que haga referencia al nombre original también tendría que ser actualizado. En una definición de objeto corta, no es un problema, pero si un objeto tiene muchas referencias a sus propiedades hay una mayor probabilidad de error.

Una forma de evitar estos problemas es con esta palabra clave `this`:
...
Modifique el método `dog.sayLegs` para eliminar cualquier referencia a `dog`. Utilice el ejemplo de `duck` como guiá.

# --hints--

`dog.sayLegs()` debe devolver un string.
