...
`playSong` 함수 안에서 `audio.src` 속성을 `song.src`와 같게 설정하세요. 이렇게 하면 오디오 요소가 선택한 노래의 오디오 데이터를 어디서 찾을지 알게 됩니다.

또한 `audio.title` 속성을 `song.title`와 같게 설정하세요. 이렇게 하면 오디오 요소가 노래 제목으로 무엇을 표시할지 알게 됩니다.

# --hints--

기존 `playSong` 함수와 그 내용을 수정하지 마세요.
...
`audio.src`를 `song.src`로 설정해야 합니다.
...
`audio.title`를 `song.title`로 설정해야 합니다.
