...
`:not()` 僞選擇器用於定位與選擇器不匹配的所有元素——在這種情況下，我們希望定位沒有 `sr-only` 類的任何 `span` 元素。 這可以確保之前的 `span[class~="sr-only"]` 類規則不會被覆蓋。

# --hints--

應該有一個 `span:not(.sr-only)` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)'));
```

`span:not(.sr-only)` 選擇器應該有一個 `font-weight` 屬性設置爲 `normal`。
