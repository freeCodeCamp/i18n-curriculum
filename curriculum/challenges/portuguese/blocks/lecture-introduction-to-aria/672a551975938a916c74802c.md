...
Habilite o editor interativo e digite alguns caracteres no campo de entrada da senha. Você verá que a senha fica oculta na janela de visualização. Também deve perceber que o texto `password-help` permanece vermelho até você digitar 8 ou mais caracteres no campo.
...
Outro bom caso de uso para o atributo `aria-describedby` é quando você tem um `button` de excluir. Aqui está um exemplo de um `button` de excluir seguido por uma mensagem que descreve o que acontecerá quando o botão for clicado:
