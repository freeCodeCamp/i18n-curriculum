...
Tengeneza kichaguzi cha `img` na kipa sifa ya `width` iwe `100%`, na sifa ya `object-fit` iwe `cover`.

Sifa ya `object-fit` inaeleza kivinjari jinsi ya kuweka kipengele ndani ya kontena lake. Katika kesi hii, `cover` itafanya picha ijaze kontena, ikikatwa inapohitajika ili kuepuka kubadilisha uwiano wa sura.
...
Kichaguzi chako cha `img` kinapaswa kuwa na sifa ya `object-fit` iwe `cover`.
