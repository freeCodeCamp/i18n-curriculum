...
`th` 요소 안에 있는 `tr.data` 요소를 선택하세요. 텍스트를 왼쪽 정렬하고, 위쪽 패딩은 `0.3rem`, 왼쪽 패딩은 `0.5rem`로 설정하세요.

# --hints--

`tr.data th` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data th'));
```

`tr.data th` 선택자에 `text-align` 속성을 `left`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data th')?.getPropertyValue('text-align') === 'left');
```

`tr.data th` 선택자에 `padding-top` 속성을 `0.3rem`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data th')?.getPropertyValue('padding-top') === '0.3rem');
```

`tr.data th` 선택자에 `padding-left` 속성을 `0.5rem`로 설정해야 합니다.
