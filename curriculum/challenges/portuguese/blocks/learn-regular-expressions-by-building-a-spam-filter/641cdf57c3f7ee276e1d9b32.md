...
Após a declaração `if`, use um operador ternário para verificar a veracidade da chamada `isSpam()` com `messageInput.value` como argumento.Se verdadeiro, defina a propriedade `textContent` no elemento `result` para `"Oh no! This looks like a spam message."`.Caso contrário, defina-o como `"This message does not seem to contain any spam."`

Então defina a propriedade `value` do elemento `messageInput` como uma string vazia.

# --hints--

Você deve usar o operador da atribuição para definir a propriedade `textContent` do elemento `result`.
...
Você deve chamar a função `isSpam()` após o operador de atribuição `=` e antes do operador ternário `?`.
...
Você deve usar a sintaxe de operador ternário para verificar se `isSpam(messageInput.value)` é verdadeiro.
...
A expressão truthy do seu ternário deve definir a propriedade `textContent` do elemento `result` para `"Oh no! This looks like a spam message."`.
...
A expressão falsa do seu ternário deve definir a propriedade `textContent` do elemento `result` para `"This message does not seem to contain any spam."`.
...
Após seu ternário, defina a propriedade `value` no elemento `messageInput` como uma string vazia.
