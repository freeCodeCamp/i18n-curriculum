...
就像一般函式（程式）一樣，箭頭函式（程式）可以接受多個參數。 

這是一個帶有一個參數的具名箭頭函式（function）的範例： 
...
如果函式（程式）只有一個參數，你可以省略參數列周圍的小括弧，如下所示：
...
創建一個名為 `printMessage` 的新箭頭函式（程式），該函式有一個名為 `org` 的參數。在該函式（程式）主體內，添加一個主控台述語。在該主控台述語內，添加模板字串 `${org} is awesome!`。 

在你的 `printMessage` 函式（程式）下方，呼叫該函式並傳入字串 `"freeCodeCamp"` 作為引數。

打開主控台以查看結果。 

# --hints--

你應該有一個 `printMessage` 函式（程式）。
...
你的 `printMessage` 函式應該有一個名為 `org` 的參數。
...
你的 `printMessage` 函式呼叫應該要在主控台輸出 `freeCodeCamp is awesome!`。 
