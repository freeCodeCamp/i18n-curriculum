...
The `function` keyword tells JavaScript that the `name` variable is going to be a function. `parameter` is a variable that represents a value that is passed into the function when it is used. Una función puede tener tantos <dfn>parámetros</dfn> como queramos. Como un bucle `for`, el espacio entre llaves es el <dfn>cuerpo de la función</dfn>.
