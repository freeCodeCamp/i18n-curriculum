...
title: aggiungere angoli arrotondati con il raggio del bordo
challengeType: 0
...
La tua foto del gatto ha attualmente angoli netti. Possiamo arrotondare quegli angoli con una proprietà CSS chiamata `border-radius`.

# --instructions--

Puoi specificare un `border-radius` in pixel. Dai alla tua foto del gatto un `border-radius` di `10px`.

**Nota:** Questa sfida permette più soluzioni possibili. Per esempio, puoi aggiungere `border-radius` sia alla classe `.thick-green-border` sia alla classe `.smaller-image`.

# --hints--

Il tuo elemento immagine dovrebbe avere la classe `thick-green-border`.

```js
assert.isTrue(document.querySelector('img').classList.contains('thick-green-border'));
```

La tua immagine dovrebbe avere un raggio del bordo di `10px`.
