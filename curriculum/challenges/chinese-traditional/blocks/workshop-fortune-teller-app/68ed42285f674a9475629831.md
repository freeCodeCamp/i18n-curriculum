...
在類別內，新增一個私有 `elements` 屬性，型態為物件。在其中，新增屬性 `singleCardBtn`、`singleCard` 和 `multipleCardsBtn`，每個屬性的型別皆為 `HTMLElement`。

# --hints--

你應該在 `private` 類別內宣告一個名為 `elements` 的 `Game` 屬性。
...
你的 `elements` 私有屬性應該有一個 `singleCardBtn` 屬性，型別為 `HTMLElement`。
...
你的 `elements` 私有屬性應該有一個 `multipleCardsBtn` 屬性，型別為 `HTMLElement`。
...
你的 `elements` 私有屬性應該有一個 `singleCard` 屬性，型別為 `HTMLElement`。
