...
1. Você deve ter um elemento `html` com o atributo `lang` definido como `en`.
1. Você deve ter um elemento `head` contendo um elemento `title` com o nome da sua receita e um elemento `meta` com o atributo `charset` definido como `UTF-8`.
1. Você deve ter um elemento `body`.
1. Você deve ter apenas um elemento `h1` com o nome da sua receita.
1. Você deve ter um elemento `p` que introduza a receita abaixo do `h1`.
1. Você deve ter um elemento `h2` com o texto `Ingredients` para a seção de ingredientes.
1. Você deve ter uma lista não ordenada (elemento `ul`) com pelo menos quatro itens de lista (elementos `li`) que listem seus ingredientes abaixo do primeiro elemento `h2`.
1. Você deve ter um segundo elemento `h2` com o texto `Instructions` para a seção de instruções.
1. Você deve ter uma lista ordenada (elemento `ol`) com pelo menos quatro itens de lista que listem os passos da receita em ordem, abaixo do segundo `h2`.
1. Você deve ter um elemento `img` com um atributo `src` definido para uma imagem válida e um atributo `alt` descrevendo a imagem. Você pode usar `https://cdn.freecodecamp.org/curriculum/labs/recipe.jpg` se quiser.
...
Seu elemento `html` deve ter uma tag de fechamento. Tags de fechamento têm uma `/` logo após o caractere `<`.
...
Seu elemento `body` deve ter uma tag de fechamento. Tags de fechamento têm uma `/` logo após o caractere `<`.

```js
assert.match(code, /<\/\s*body\s*>/i);
```

Você deve ter apenas um elemento `h1` com o nome da sua receita.
...
Você deve ter um elemento `p` abaixo do seu elemento `h1`.

```js
assert.strictEqual(document.querySelector('h1')?.nextElementSibling, document.querySelector('p'));
```

Seu primeiro elemento `p` deve descrever sua receita.
