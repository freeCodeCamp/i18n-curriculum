...
The function should accept two parameters.O primeiro receberá `n` ou o intervalo como um array.O segundo receberá um booleano, que especifica se a função retorna os emirps como um array ou um único número (o número de primos no intervalo ou o <code>n<sup>th</sup></code> primo).De acordo com os parâmetros, a função deve retornar um array ou um número.
