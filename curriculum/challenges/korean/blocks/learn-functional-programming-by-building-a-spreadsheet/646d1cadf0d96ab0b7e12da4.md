...
`elemValue`이 함수를 반환하기 때문에, `addCharacters` 함수는 결국 함수 참조의 배열을 반환합니다. `.map()` 메서드가 `elemValue` 함수의 내부 함수를 실행하도록 하려면, 참조하는 대신 `elemValue`를 호출해야 합니다. `num`를 `elemValue` 함수의 인수로 전달하세요.

<!-- TODO: Explain further? -->

# --hints--

`elemValue()`를 `.map()` 메서드에서 호출해야 합니다.
