...
$n$ の値が小さい場合、各数列に個別に名前を付けるためにギリシャ数字のプレフィックスが使用されることがあります。
...
類似の数列は、初期値が変更された場合に生成されます。ルーカス (リュカ) 数列は、$n=2$ のフィボナッチ数列と同様に先行する 2 つの値を合計しますが、$\[2, 1]$ を初期値として使用します。
