...
Ascolta e completa ogni frase con il reparto corretto.
...
`El departamento de aprendizaje y desarrollo capacita al equipo.` — Il reparto Learning and Development forma il team.

Sia `apoyar` (to support) sia `capacitar` (to train) sono verbi `-ar` alla terza persona singolare.

Seguendo lo stesso pattern di `desarrolla`, `verifica` e `crea` delle attività precedenti.
