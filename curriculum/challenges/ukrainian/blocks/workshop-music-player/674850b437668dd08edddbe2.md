...
У попередніх кроках ви реалізували функціонал відтворення пісні. Тепер потрібно додати функціонал для кнопки відтворення.

Використайте метод `addEventListener()` на `playButton` і передайте подію `"click"` як перший аргумент. Наразі використайте функцію зворотного виклику, яка викликає `playSong(0)` як другий аргумент.

Потім протестуйте, натиснувши на кнопку відтворення.


# --hints--

Ви маєте додати слухача події для події `click` на `playButton` і передати функцію зворотного виклику, яка викликає `playSong(0)` як другий аргумент методу `addEventListener`.
