...
Agrega un atributo `value` de `player.overallRating` al `input` general. Después, añade un atributo `onChange` que sea una función flecha que tome `e` como parámetro y llame a `setPlayer` con una expansión de `player` y `overallRating` establecido en `Number(e.target.value)`.

Como `overallRating` es un número, usa `Number()` para convertir el valor de cadena del input.

# --hints--

Tu `input` general debe tener un `value` igual a la calificación general actual del jugador.
...
Tu `input` general debe tener un atributo `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

El `onChange` de tu `input` general debe llamar a `setPlayer` con una expansión de `player` y `overallRating` establecido en `Number(e.target.value)`.
