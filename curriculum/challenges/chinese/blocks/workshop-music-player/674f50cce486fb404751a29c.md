...
Now that you know how to find the next song to play, create a function named `playNextSong`. 在 `playNextSong` 函数内部，创建一个 `if` 语句来检查 `userData` 的 `currentSong` 是否严格等于 `null`。 这将检查 `userData` 对象中是否没有正在播放的歌曲。

如果条件为真，则使用 `userData.songs` 数组中第一首歌曲的 `id` 作为参数调用 `playSong` 函数。

然后在 `if` 块的末尾添加一个 `return` 语句。

# --hints--

你应该有一个名为 `playNextSong` 的函数。

```js
assert.isFunction(playNextSong);
```

当 `userData` 的 `currentSong` 严格相等于 `null` 时，你的 `playNextSong` 函数应调用 `playSong` 函数，并将 `userData.songs` 数组中第一首歌曲的 `id` 作为参数传入。
