...
JavaScriptファイルで、`.story-container` 要素を選択し、`storyContainer` という変数にストアしてください。

# --hints--

`storyContainer` という変数が必要です。

```js
assert.isDefined(storyContainer);
```

`.story-container` メソッドを使って `querySelector` 要素を選択し、`storyContainer` 変数にストアしてください。
