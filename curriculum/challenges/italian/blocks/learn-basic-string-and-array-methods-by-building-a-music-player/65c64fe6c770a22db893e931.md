...
Ora che hai la lista delle canzoni visualizzata sullo schermo, sarebbe bello ordinarle in ordine alfabetico per titolo.

Inizia creando una funzione freccia chiamata `sortSongs`. 

# --hints--

Dovresti avere una funzione chiamata `sortSongs`.

```js
assert.isFunction(sortSongs);
```

La tua funzione `sortSongs` dovrebbe usare la sintassi della funzione freccia.
