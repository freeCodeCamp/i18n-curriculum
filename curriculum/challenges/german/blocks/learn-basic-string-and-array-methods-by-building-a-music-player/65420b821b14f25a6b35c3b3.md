...
Wenn die Songs auf der Seite angezeigt werden, sollten der Titel, der Künstler, die Dauer jedes Songs und eine Löschen-Schaltfläche angezeigt werden.

In den nächsten Schritten lernen Sie, wie Sie für jeden Song neues HTML mit der <dfn>map()</dfn>-Methode hinzufügen. Diese Methode wird im nächsten Schritt ausführlicher erklärt.

Beginnen Sie damit, `const` zu verwenden, um eine Variable namens `songsHTML` zu deklarieren und ihr `array.map()` zuzuweisen.

# --hints--

Sie sollten eine `const`-Variable namens `songsHTML` haben.

```js
assert.match(code, /const\s+songsHTML/);
```

Sie sollten `array.map()` Ihrer `songsHTML` zuweisen.
