...
<a href="https://rosettacode.org/wiki/Cramer%27s_rule" target="_blank" rel="noopener noreferrer nofollow">Правило Крамера</a> — це формула для розв’язання системи лінійних рівнянь за допомогою визначників матриць, утворених із підмножин коефіцієнтів та значень правої частини.

Визначник матриці розміром 2 на 2 задається так:
...
розв’яжіть її для $w$, $x$, $y$ та $z$, використовуючи правило Крамера.
...
`cramersRule([[2, -1, 5, 1], [3, 2, 2, -6], [1, 3, 3, -1], [5, -2, -3, 3]], [-3, -32, -47, 49])` має повертати `[2, -12, -4, 1]`.

```js
assert.deepEqual(cramersRule(matrices[0], freeTerms[0]), answers[0]);
```

`cramersRule([[3, 1, 1], [2, 2, 5], [1, -3, -4]], [3, -1, 2])` має повертати `[1, 1, -1]`.
