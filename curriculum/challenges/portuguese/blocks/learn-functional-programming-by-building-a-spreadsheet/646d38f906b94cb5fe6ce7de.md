...
`addCharacters(char1)` também está retornando uma função, que retorna outra função. Você precisa fazer outra chamada de função para acessar essa referência da função mais interna para a função de callback de `.map()`. JavaScript permite que você invoque imediatamente funções retornadas:

```js
myFunc(1)("hi");
```

Invoque imediatamente a função retornada da sua chamada `addCharacters(char1)` e passe `char2` como argumento.

# --hints--

Você deve encadear uma chamada de função à chamada de `addCharacters(char1)`.
...
Você deve passar `char2` como o primeiro argumento para a chamada de função encadeada.
