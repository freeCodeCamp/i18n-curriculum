...
Agora você deve implementar o loop principal da simulação. Esse loop vai garantir que cada porta seja processada a cada tick da simulação até que todos os ticks tenham sido concluídos.

Crie um loop `while` vazio que continue enquanto `tickIndex` for menor que `maxTicks`.

# --hints--

Você deve criar um loop `while` vazio com a condição `(tickIndex < maxTicks)`.
