...
Concentrandoti sugli elementi del menu e sui prezzi, c’è uno spazio piuttosto ampio tra ogni riga.

Usa il selettore esistente che prende di mira tutti gli elementi `p` annidati in elementi con la classe chiamata `item` e imposta il loro margine superiore e inferiore su `5px`.

# --hints--

Dovresti impostare la proprietà `margin-top` su `5px`.
...
Dovresti impostare la proprietà `margin-bottom` su `5px`.
...
Dovresti usare il selettore `.item p` esistente.
...
I tuoi elementi `p` annidati nei tuoi elementi `.item` dovrebbero avere un margine `margin-top` di `5px`.
...
I tuoi elementi `p` annidati nei tuoi elementi `.item` dovrebbero avere un margine `margin-bottom` di `5px`.
...
I tuoi elementi `p` annidati nei tuoi elementi `.item` non dovrebbero avere un margine `margin-left` impostato.
...
I tuoi elementi `p` annidati nei tuoi elementi `.item` non dovrebbero avere un margine `margin-right` impostato.
