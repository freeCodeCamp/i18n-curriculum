...
Sistema il `header` usando _Flexbox_ per mettere spazio tra i figli e centrarli verticalmente.

# --hints--

Dovresti dare al `header` un `justify-content` di `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

Dovresti dare al `header` un `align-items` di `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

Dovresti dare al `header` un `top` di `0`.
