...
Il bordo dell'ultimo elemento `fieldset` sembra un po' fuori posto.

Nelle lezioni precedenti, hai imparato a lavorare con la pseudo-classe CSS `last-of-type` in questo modo:

```css
p:last-of-type { }
```

Questo selezionerà l'ultimo elemento `p`.

Crea un nuovo selettore che abbia come obiettivo l'ultimo elemento `fieldset` e imposta il suo `border-bottom` su `none`.
