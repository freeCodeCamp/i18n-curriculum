...
Verwenden Sie Ihre neue `flavor`-Klasse als Selektor und setzen Sie den Wert der `text-align`-Eigenschaft auf `left`.

# --hints--

Sie sollten einen `flavor`-Klassen-Selektor haben.
...
Sie sollten die `text-align`-Eigenschaft auf `left` setzen.
...
Ihr `flavor`-Klassen-Selektor sollte die `text-align`-Eigenschaft auf `left` setzen.
