...
في عنصر `tr` الثاني، أضف عنصر `th` بالنص `Checking Our primary transactional account.`. غلف هذا النص، ما عدا `Checking `، في عنصر `span` مع تعيين الخاصية `class` إلى `description`.

بعد ذلك، أضف ثلاثة عناصر `td` بالنصوص التالية (بالترتيب): `$54`، `$56`، `$53`. اجعل الخاصية `class` في عنصر `td` الثالث تساوي `current`.
