...
Rufen Sie innerhalb des `div`-Elements die `avatars`-Funktion auf und übergeben Sie die Argumente `posters` und `users`.

Jetzt sollten die Avatare auf der Seite angezeigt werden.

# --hints--

Sie sollten die Funktion mit dem Namen `avatars` aufrufen.
...
Ihr Funktionsaufruf sollte die Argumente `posters` und `users` enthalten.
