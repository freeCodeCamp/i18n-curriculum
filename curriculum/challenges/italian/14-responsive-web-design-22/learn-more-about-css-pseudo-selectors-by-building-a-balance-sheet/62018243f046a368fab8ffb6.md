...
Il selettore `table` dovrebbe avere una proprietà `border-collapse` impostata su `collapse`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('border-collapse') === 'collapse');
```

Il selettore `table` dovrebbe avere una proprietà `border` impostata a `0`.
