...
Задайте ціль `a` елементам всередині `.card-links` і надайте їм:

- властивість `text-decoration`, встановлену в `none`.
- властивість `font-size`, встановлену в `1em`
- властивість `padding`, встановлену в `10px 20px`
- властивість `border-radius`, встановлену в `5px`
- властивість `color`, встановлену в `white`
- властивість `background-color`, встановлену в `midnightblue`

# --hints--

Ви повинні мати селектор `.card-links a`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a"));
```

Селектор `.card-links a` має мати властивість `text-decoration`, встановлену в `none`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card-links a")?.getPropertyValue("text-decoration"), "none");
```

Селектор `.card-links a` має мати властивість `font-size`, встановлену в `1em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card-links a")?.getPropertyValue("font-size"), "1em");
```

Селектор `.card-links a` має мати властивість `padding`, встановлену в `10px 20px`.
...
Селектор `.card-links a` має мати властивість `border-radius`, встановлену в `5px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card-links a")?.getPropertyValue("border-radius"), "5px");
```

Селектор `.card-links a` має мати властивість `color`, встановлену в `white`.
...
Селектор `.card-links a` має мати властивість `background-color`, встановлену в `midnightblue`.
