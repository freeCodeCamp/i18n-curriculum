...
`Have to` é uma expressão usada para expressar necessidade ou obrigação, especialmente em um contexto de trabalho.Por exemplo:

`You have to wear a helmet on the construction site.` - Isso significa que usar um capacete é uma regra de segurança necessária.
...
Essas duas palavras são usadas para indicar uma ação necessária.
