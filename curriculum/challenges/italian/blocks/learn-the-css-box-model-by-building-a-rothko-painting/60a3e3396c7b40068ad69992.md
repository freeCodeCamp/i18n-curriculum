...
Gli angoli di ogni rettangolo sono ancora troppo spigolosi.

Arrotonda ogni angolo dell'elemento `.one` di 9 pixel, usando la proprietà `border-radius`.

# --hints--

Dovresti impostare la proprietà `border-radius` su `9px`.
...
Il tuo elemento `.one` dovrebbe avere un valore `border-radius` di `9px`.
