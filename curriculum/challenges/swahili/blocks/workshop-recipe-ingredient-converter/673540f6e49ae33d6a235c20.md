...
Utahitaji kuweka orodha ya vitengo ambavyo programu yako inaunga mkono.

Tangaza safu ya data `units`, na ipe thamani za mfuatano wa herufi `"cup"`, `"gram"`, `"ounce"`, na `"teaspoon"`.

# --hints--

Unapaswa kutangaza variable `units`.

```js
assert.isDefined(units);
```

Variable yako ya `units` inapaswa kuwa safu ya data.

```js
assert.isArray(units);
```

Safu yako ya data `units` inapaswa kuwa na vipengele vinne.

```js
assert.lengthOf(units, 4);
```

Safu yako ya data `units` inapaswa kujumuisha mfuatano wa herufi `"cup"`.

```js
assert.include(units, "cup");
```

Safu yako ya data `units` inapaswa kujumuisha mfuatano wa herufi `"gram"`.

```js
assert.include(units, "gram");
```

Safu yako ya data `units` inapaswa kujumuisha mfuatano wa herufi `"ounce"`.

```js
assert.include(units, "ounce");
```

Safu yako ya data `units` inapaswa kujumuisha mfuatano wa herufi `"teaspoon"`.
