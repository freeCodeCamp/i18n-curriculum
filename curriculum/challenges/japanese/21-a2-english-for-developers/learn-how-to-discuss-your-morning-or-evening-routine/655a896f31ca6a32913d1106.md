...
Prepositions like `by` affect the verb that comes after, When you use `by`, it often needs the verb to end with `-ing`. 以下が例です:
...
Adjectives can also change the meaning of the following word by adding details to them. The word `light` can serve as an adjective to describe something not heavy or not intense. 例えば、次のようになります。
...
Even words like `family` and `outdoor` can be used as adjectives, they describe the noun that follows them. 例:
