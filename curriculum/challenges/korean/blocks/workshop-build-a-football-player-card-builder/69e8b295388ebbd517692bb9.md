...
카드 헤더가 완성되었습니다. 이제 플레이어 사진 섹션을 추가해야 합니다.

`div`의 `card-header` 닫는 태그 뒤이면서 `div`의 `className`가 있는 `card` 안에 `div`의 `className`인 `card-image-wrap`를 추가하세요. 지금은 비워 두세요. 이미지는 다음 단계에서 추가할 것입니다.

# --hints--

`div`의 `className`인 `card-image-wrap`를 추가해야 합니다.
