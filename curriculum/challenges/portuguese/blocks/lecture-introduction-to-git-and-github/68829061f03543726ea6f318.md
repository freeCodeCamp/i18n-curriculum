...
Falamos um pouco sobre Git e GitHub, então agora podemos começar a juntar tudo. Para fazer isso, você precisa de um repositório. Mas o que é um repositório? Você pode pensar em um repositório como um container para um projeto - se você está trabalhando em um app, você manteria os arquivos desse app juntos em um repositório.
...
Sua próxima opção é selecionar um proprietário. Você pode escolher sua conta de usuário ou qualquer organização que você possua ou tenha acesso. Depois, pode nomear o repositório — em geral, você vai querer usar um nome que descreva de forma sucinta o projeto que está construindo. Como melhor prática geral, evite espaços no nome do seu repositório. O GitHub vai substituí-los automaticamente por hífens.
...
Para clonar um repositório público, você na verdade não precisará se autenticar. Mas para clonar um repositório privado, ou para enviar alterações para repositórios públicos ou privados, será necessário estar autenticado. Uma URL remota HTTPS era usada para autenticar você via seu nome de usuário e senha do GitHub. No entanto, por questões de segurança, isso não é mais suportado, e você precisa usar um nome de usuário e um token de acesso. Mesmo assim, esse é um método muito menos seguro que o SSH, que usa suas chaves privadas e públicas. Isso traz a vantagem adicional de autenticar você automaticamente, sem precisar fornecer um token toda vez. Haverá uma lição separada que vai aprofundar o SSH e como ele funciona, mas você também pode ler a documentação do GitHub sobre como configurar o SSH.
