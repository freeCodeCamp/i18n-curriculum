...
Ora, usa il metodo `querySelectorAll` per selezionare gli elementi `.playlist-song` e archiviarli in una variabile chiamata `songs`.

Poi, chiama il metodo `forEach` su `songs`. Passa una funzione di callback che usa `song` come parametro del metodo. Lascia vuote le parentesi graffe per ora.

# --hints--

Dovresti avere una variabile chiamata `songs`.

```js
assert.isDefined(songs);
```

Dovresti assegnare la raccolta di tutti gli elementi `.playlist-song` alla tua variabile `songs`.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

Dovresti chiamare il metodo `forEach` su `songs` e passare una callback che usa `song` come parametro del metodo.
