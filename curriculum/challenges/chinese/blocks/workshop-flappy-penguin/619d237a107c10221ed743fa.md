...
在 `.penguin-body` 中，在 `.foot` 元素前面添加两个 `div` 元素，每个的 `class` 都是 `arm`。 为第一个 `.arm` 添加一个 `left` `class`，为第二个 `.arm` 添加一个 `right` `class`。
