...
Crie um elemento `button` com o `className` `submit-btn` e um `type` `submit`. Dê ao botão o texto `Join the League`.

Depois de fazer isso, tente enviar o formulário e veja o que acontece.
...
Seu elemento button deve ter um atributo `className` definido como `submit-btn`.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

Seu elemento `button` deve ter `Join the League` como seu texto.
