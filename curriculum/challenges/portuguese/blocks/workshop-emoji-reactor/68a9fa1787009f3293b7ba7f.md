...
Você provavelmente percebeu que seu código está ficando bastante repetitivo! Há uma maneira melhor de lidar com múltiplos elementos semelhantes.

Em vez de selecionar cada botão individualmente, use `querySelectorAll(".emoji-btn")` para selecionar todos os botões com a classe `emoji-btn` de uma vez. Armazene o resultado em uma variável chamada `btns`.
...
Você deve usar `querySelectorAll` para selecionar todos os botões de emoji.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

Você deve armazenar o resultado em uma variável chamada `btns`.
...
A variável `btns` deve conter todos os 4 botões de emoji.
