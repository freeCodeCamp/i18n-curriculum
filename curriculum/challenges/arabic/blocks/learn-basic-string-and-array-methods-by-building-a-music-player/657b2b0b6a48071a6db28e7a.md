...
تُستخدم طريقة `map()` للتكرار خلال مصفوفة وإرجاع مصفوفة جديدة. تكون مفيدة عندما تريد إنشاء مصفوفة جديدة بناءً على قيم مصفوفة موجودة. على سبيل المثال:
...
لاحظ أن طريقة `map()` تأخذ دالة كمعلمة. تُسمى هذه <dfn>دالة استدعاء راجع</dfn>، وهي دالة تُمرر إلى دالة أخرى كمعلمة. في المثال أعلاه، دالة الاستدعاء الراجع هي `(number) => number * 2`، وتُنفذ على كل عنصر في مصفوفة `numbers`. ثم تُرجع طريقة `map()` مصفوفة جديدة بالنتائج.

مرر دالة استدعاء راجع إلى طريقة `map()`. يجب أن تأخذ دالة الاستدعاء الراجع المعلمة `song`، وتستخدم صيغة الدالة السهمية، وأن يكون جسمها فارغًا.

# --hints--

يجب أن تستخدم دالة الاستدعاء الراجع في طريقة `map()` صيغة الدالة السهمية.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

يجب أن تحتوي طريقة `map()` على معلمة `song` في دالة الاستدعاء الراجع الخاصة بها.
