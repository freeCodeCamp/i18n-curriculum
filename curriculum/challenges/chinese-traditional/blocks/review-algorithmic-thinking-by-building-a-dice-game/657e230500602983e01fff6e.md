...
在遊戲的最後部分，你需要創建一個演算法來檢查是否存在順子。小順子是指四顆骰子有連續的數值，順序不限（例如，在一次擲出 `41423` 中，我們有 `1234`），得分為 `30` 分。大順子是指五顆骰子都有連續的數值，順序不限（例如，在一次擲出 `35124` 中，我們有 `12345`），得分為 `40` 分。

宣告一個接受數字陣列的 `checkForStraights` 函式。若使用者獲得大順，將第五個圓鈕的分數更新為 `40`。若使用者獲得小順，將第四個圓鈕的分數更新為 `30`。無論如何，都應該更新最後一個圓鈕以顯示分數 0，並設定正確的屬性。

當 `rollDiceBtn` 被點擊時，呼叫你的 `checkForStraights` 函式以完成你的骰子遊戲！

# --hints--

你的 `checkForStraights` 變數應該是一個函式（程式）。

```js
assert.isFunction(checkForStraights);
```

如果擲出小順子，你的 `checkForStraights` 函式應該啟用第四個圓鈕，將值設為 `30`，更新顯示的文字為 `, score = 30`，並保持第五個圓鈕停用。
...
如果擲出一個大型順子，你的 `checkForStraights` 函式應該啟用第四個按鈕，將值設為 `30`，並將顯示的文字更新為 `, score = 30`。此外，該函式應該啟用第五個圓鈕，將值設為 `40`，並將顯示的文字更新為 `, score = 40`。 
...
如果沒有擲出順子，你的 `checkForStraights` 函式不應該啟用第四或第五個圓鈕。
...
如果沒有擲出順子，你的 `checkForStraights` 函式應該啟用最後一組圓鈕，將值設為 `0`，並將顯示的文字更新為 `, score = 0`。
...
當 `rollDiceBtn` 被點擊時，你應該呼叫 `checkForStraights` 函式（程式）。
