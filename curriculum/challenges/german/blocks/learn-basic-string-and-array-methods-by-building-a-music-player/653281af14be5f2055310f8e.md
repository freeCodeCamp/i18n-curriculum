...
Ihr Musikplayer sollte die Songs, den aktuell abgespielten Song und die Zeit des aktuellen Songs verfolgen. Dafür müssen Sie ein Objekt erstellen, um diese Informationen zu speichern.

Beginnen Sie damit, das Schlüsselwort `let` zu verwenden, um eine neue Variable namens `userData` zu deklarieren und ihr ein leeres Objekt zuzuweisen.

# --hints--

Sie sollten das Schlüsselwort `let` verwenden, um die Variable `userData` zu erstellen.

```js
assert.match(code, /let\s+userData\s*=/)
```

Sie sollten Ihrer Variable `userData` ein leeres Objekt zuweisen.
