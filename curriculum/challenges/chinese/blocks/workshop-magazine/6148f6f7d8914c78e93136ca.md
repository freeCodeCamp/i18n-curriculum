...
为 `only screen` 创建一个最终 `@media` 查询，其中 `max-width` 为 `420px`。 在其中，创建一个 `.hero-title` 选择器，并将 `font-size` 属性设置为 `4.5rem`。

恭喜！ 你的杂志现在已经完成了。

# --hints--

你应该有一个新的 `@media` 查询，对应 `only screen and (max-width: 420px)`。 这应该是 `@media` 查询列表中的最后一个查询。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

你的新 `@media` 查询应该有一个 `.hero-title` 选择器。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

你的 `.hero-title` 选择器应该有一个 `font-size` 属性，设置为 `4.5rem`。
