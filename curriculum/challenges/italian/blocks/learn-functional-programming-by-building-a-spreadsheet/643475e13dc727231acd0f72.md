...
Ora nella chiamata `.forEach()` annidata, dichiara una variabile `input`. Usa il metodo `.createElement()` dell'oggetto `document` per creare un elemento `input`. Set the `type` attribute to `"text"` and the `id` attribute to `letter + number`.
