...
向名为 `text` 的对象添加一个最终属性。 赋予此属性与在 `goTown` 函数中分配给 `text.innerText` 的字符串值相同的字符串值。

# --hints--

你的第一个 `locations` 值应该具有 `text` 属性。
...
你的 `text` 属性应该具有值 `"You are in the town square. You see a sign that says \"Store\"."`
