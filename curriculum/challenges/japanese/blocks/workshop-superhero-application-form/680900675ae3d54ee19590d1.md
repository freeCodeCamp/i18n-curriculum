...
次に、フォームの送信方法を指定しましょう。フォーム要素に`method`属性を値`post`で、`action`属性を値`https://superhero-application-form.freecodecamp.org`で追加してください。

これでフォームを送信すると、そのURLに送信されます。次のステップでそれを行います。

# --hints--

`form`要素には`method`属性を`post`に設定してください。
...
`form`要素には`action`属性を`https://superhero-application-form.freecodecamp.org`に設定してください。
