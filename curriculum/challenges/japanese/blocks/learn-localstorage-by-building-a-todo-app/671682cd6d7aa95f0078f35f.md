...
最後の問題に取り組む時間です。名前や説明の中に引用符のような特殊文字が含まれていると、アプリケーションが壊れてしまいます。見た目は問題なさそうでも、正しい項目が欠落しているように表示されます。

これを修正するために、文字列を入力として受け取り、すべての特殊文字を削除する`removeSpecialChars`という関数を作成する必要があります。

# --hints--

`removeSpecialChars`関数を定義してください。

```js
assert.isFunction(removeSpecialChars);
```

`removeSpecialChars`はスペースを削除しないようにしてください。
