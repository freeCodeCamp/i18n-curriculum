...
Sotto il `value` e `checked` destrutturati, aggiorna la lista dei poteri selezionati.

Chiama la funzione `setPowers`, che prende un array come argomento. Usa un operatore ternario per verificare se `checked` è true. Se lo è, espandi l'array `powers` esistente in un nuovo array e aggiungi `value`. Se non è true, filtra `value` da `powers` usando `powers.filter(p => p !== value)`.

# --hints--

Dovresti usare un operatore ternario per verificare se `checked` è true. Se lo è, espandi l'esistente `powers` e `value` in un array. Se non lo è, filtra `value` da `powers`.
