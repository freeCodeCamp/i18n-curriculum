...
Aggiungi un attributo `onChange` impostato su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `position` impostato su `e.target.value`, convertito in `Position`.

# --hints--

La tua posizione `select` dovrebbe avere un gestore `onChange` che chiama `setPlayer` con uno spread di `player` e aggiorna `position`.
