...
クラスが`main`の`story-container`要素を作成してください。`.story-container`要素の中に、クラスが`div`の`btn-container`を作成してください。

# --hints--

クラスが`main`の`story-container`要素があるはずです。

```js
assert.exists(document.querySelector('main.story-container'));
```

`.story-container`要素の中に、クラスが`div`の`btn-container`があるはずです。
