...
Використовуючи той самий синтаксис інтерполяції, додайте другий `p` елемент з текстом `consumedCalories Calories Consumed` і третій з текстом `exerciseCalories Calories Burned`. Не забудьте замінити свої `consumedCalories` і `exerciseCalories` змінні на відповідні значення.
