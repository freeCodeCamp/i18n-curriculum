...
Racchiudi tutto il contenuto all'interno dell'elemento `.label` in un nuovo elemento `header`.
...
Il tuo elemento `header` dovrebbe essere all'interno del tuo elemento `.label`.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

I tuoi elementi `h1`, `div` e `p` dovrebbero essere all'interno del tuo nuovo elemento `header`.
