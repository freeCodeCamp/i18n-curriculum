...
Definisci una funzione `playSong` che prenda un singolo parametro che rappresenterà l'identificatore unico della canzone che vuoi riprodurre.

# --hints--

Dovresti avere una funzione vuota chiamata `playSong`.

```js
assert.isFunction(playSong);
```

La tua funzione `playSong` dovrebbe prendere un singolo parametro.
