...
Під деструктурованими `value` та `checked` оновіть список вибраних ступенів.

Викличте функцію `setPowers`, яка приймає аргумент-масив. Використайте тернарний оператор, щоб перевірити, чи `checked` є істинним. Якщо так, розгорніть існуючий масив `powers` у новий масив і додайте до нього `value`. Якщо ні, відфільтруйте `value` з `powers` за допомогою `powers.filter(p => p !== value)`.

# --hints--

Ви повинні використати тернарний оператор, щоб перевірити, чи `checked` є істинним. Якщо так, розгорніть існуючі `powers` та `value` у масив. Якщо ні, відфільтруйте `value` з `powers`.
