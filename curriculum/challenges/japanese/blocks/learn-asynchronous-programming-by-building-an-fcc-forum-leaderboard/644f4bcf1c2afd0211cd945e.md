...
すべてのカテゴリには、[freeCodeCamp forum](https://forum.freecodecamp.org/) 上のそのカテゴリを指すURLがあります。

`url` という定数を作成し、テンプレートリテラルを代入してください。そのテンプレートリテラルの中に、`${forumCategoryUrl}${selectedCategory.className}/${id}` の値を入れてください。
...
`url` 変数にはテンプレートリテラルを代入してください。

```js
assert.match(code, /const\s+url\s*=\s*`[\s\S]*`/);
```

テンプレートリテラルの中に `${forumCategoryUrl}${selectedCategory.className}/${id}` の値があるはずです。
