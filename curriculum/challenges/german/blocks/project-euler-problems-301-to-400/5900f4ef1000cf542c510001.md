...
Sei $n$ eine ganze Zahl und $S(n)$ die Menge der Teiler von $n$.

Eine Teilmenge $A$ von $S(n)$ heißt Antikette von $S(n)$, wenn $A$ nur ein Element enthält oder wenn keines der Elemente von $A$ eines der anderen Elemente von $A$ teilt.
...
Sei $N(n)$ die maximale Länge einer Antikette von $S(n)$.

Finden Sie $\sum N(n)$ für $1 ≤ n ≤ {10}^8$
