...
Dichiara una funzione `apply` che prenda un parametro `fn` e un parametro `args`.

# --hints--

Dovresti dichiarare una variabile `apply`.
...
Dovresti usare `const` per dichiarare la tua variabile `apply`.
...
La tua variabile `apply` dovrebbe essere assegnata a una funzione freccia.
...
La tua funzione `apply` dovrebbe avere `fn` come primo parametro.
...
La tua funzione `apply` dovrebbe avere `args` come secondo parametro.
...
La tua funzione `apply` dovrebbe essere vuota.
