...
正如你在前一步中所做的那樣，使用後代選擇器來選中你的 `.cat-whiskers-right` 元素中的三個 `div` 元素。 爲它添加 `40px` 的 `width`、`1px` 的 `height` 以及 `#000` 的 `background-color`。
...
你的 `.cat-whiskers-right div` 選擇器應該將 `background-color` 設置爲 `#000`。
