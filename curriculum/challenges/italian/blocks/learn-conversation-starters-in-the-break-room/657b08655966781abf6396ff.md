---
id: 657b08655966781abf6396ff
title: Attività 3
challengeType: 19
dashedName: task-3
lang: en-US
---

<!-- (audio) Sarah: Hi, Tom. Are you happy with the workplace so far? -->

# --description--

Ascolta l’audio e rispondi alla domanda qui sotto.

# --questions--

## --text--

Quale frase è grammaticalmente corretta?

## --answers--

`You are happy with the workplace so far?`

### --feedback--

Questa è la sequenza delle parole per un’affermazione, non per una domanda.

---

`Are you happy with the workplace so far?`

---

`Is you happy with the workplace so far?`

### --feedback--

`Is` non è la forma corretta di `to be` per il soggetto `you`.

---

`Happy you are with the workplace so far?`

### --feedback--

Questa opzione ha le parole in un ordine misto che non forma una domanda corretta.

## --video-solution--

2

# --scene--

```json
{
  "setup": {
    "background": "company1-reception.png",
    "characters": [
      {
        "character": "Sarah",
        "position": {"x":50,"y":0,"z":1.4},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "1.3-1.mp3",
      "startTime": 1,
      "startTimestamp": 0.00,
      "finishTimestamp": 2.78
    }
  },
  "commands": [
    {
      "character": "Sarah",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sarah",
      "startTime": 1,
      "finishTime": 3.78,
      "dialogue": {
        "text": "Hi, Tom. Are you happy with the workplace so far?",
        "align": "center"
      }
    },
    {
      "character": "Sarah",
      "opacity": 0,
      "startTime": 4.28
    }
  ]
}
```
