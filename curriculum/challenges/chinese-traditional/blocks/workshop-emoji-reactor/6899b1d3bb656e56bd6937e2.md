...
在 `p` 元素下方，創建一個帶有類別 `btn-container` 的 `div` 元素。

在 `div` 元素內創建一個 `button` 元素。

這個 `button` 應該有一個 `id` 為 `happy-btn`、一個類別為 `emoji-btn`，以及一個 `aria-label` 為 `Happy face emoji`。

# --hints--

在 `p` 元素下方創建一個新的 `div` 元素。

```js
assert.exists(document.querySelector("p + div"));
```

新的 `div` 元素應該有一個 `btn-container` 的類別。

```js
assert.equal(document.querySelector("p + div")?.className, "btn-container");
```

`div` 元素內應該有一個 `button` 元素。

```js
assert.exists(document.querySelector("div > button"));
```

`button` 元素應該有一個 `id` 為 `happy-btn`。

```js
assert.equal(document.querySelector("div > button")?.id, "happy-btn");
```

`button` 元素應該有一個 `class` 為 `emoji-btn`。

```js
assert.equal(document.querySelector("div > button")?.className, "emoji-btn");
```

`button` 元素應該有一個 `aria-label` 為 `Happy face emoji`。
