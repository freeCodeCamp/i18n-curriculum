...
La famiglia `SHA-2` è un'alternativa più robusta a `SHA-1`. La differenza principale tra loro è la lunghezza dell'hash.
Questo significa che `SHA-1` fornisce un codice più corto con meno possibilità di combinazioni uniche. `SHA-2` o `SHA-256` crea un hash più lungo e quindi più complesso con più possibilità.

# --instructions--

Indaga i dettagli di implementazione e scrivi una funzione che prenda una stringa come parametro e restituisca un hash usando `SHA-256`
