...
2. 你應該在 `header` 元素中放置一個具有對應 `id="header-img"` 的影像（這裡放一個標誌會是一個不錯的影像）。
3. 在 `#header` 元素內，你應該有一個帶有對應 `id="nav-bar"` 的 `nav` 元素。
4. 你應該在 `nav` 元素內至少有三個可點擊的元素，每個元素都帶有 `nav-link` 類別。
5. 當你點擊 `nav` 元素中的 `.nav-link` 按鈕時，應該會被帶到對應的登陸頁面區段。
6. 你應該有一個內嵌的產品 `id="video"`。
7. 你的登陸頁面應該有一個帶有對應 `id="form"` 的 `form` 元素。
8. 在表單中，應該有一個 `input` 欄位，其 `id="email"`，你可以在其中輸入電子郵件位址。
9. `#email` 輸入欄位應該有佔位文字，讓使用者知道該欄位的用途。
10. `#email` 輸入欄位應使用 HTML5 驗證來確認輸入的文字是電子郵件位址。
11. 在表單中，應該有一個帶有對應 `id="submit"` 的提交 `input`。
12. 當你點擊 `#submit` 元素時，電子郵件應該提交到靜態頁面（使用此模擬 URL：`https://www.freecodecamp.org/email-submit`）。
13. 導覽列應該始終位於視窗頂部。
14. 你的產品登陸頁面應該至少有一個媒體查詢。
15. 你的產品登陸頁面應該至少使用一次 CSS flexbox。
...
你的 `#form` 應該有一個 `action` 屬性，其值為 `https://www.freecodecamp.org/email-submit`。
