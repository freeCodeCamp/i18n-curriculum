...
Всередині елемента `form`, після мітки, додайте вхідні дані з `id` рівним `search-input` та `type` рівним `search`.

# --hints--

Усередині елемента `input` має бути `form` з `id`, рівним `search-input`.
...
Ваш `input` має мати значення `type`, рівне `search`.
