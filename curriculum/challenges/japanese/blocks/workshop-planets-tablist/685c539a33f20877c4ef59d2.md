...
タブがクリックされたときに正しいパネルを表示する作業を始めましょう。各パネルには、対応するタブの`id`属性と一致する`aria-controls`があります。たとえば、最初のパネルには`id`の`panel-earth`があり、これは最初のタブの`aria-controls`属性値である`panel-earth`と一致します。

`associatedPanel`という変数を作成し、`tab.getAttribute("aria-controls")`の値を代入してください。これにより、クリックされたタブに対応するパネルの`id`が得られます。

# --hints--

クリックされたタブに関連付けられたパネルの`associatedPanel`を保持する`id`という変数があるはずです。
