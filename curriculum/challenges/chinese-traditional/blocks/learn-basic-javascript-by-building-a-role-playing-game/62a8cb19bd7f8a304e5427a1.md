...
現在更新 `goldText` 元素以顯示 `gold` 的新數值，並更新 `text` 元素以顯示 `"You now have a new weapon."`。

# --hints--

你應該將 `goldText` 元素的 `innerText` 屬性更新為 `gold`。

```js
assert.match(buyWeapon.toString(), /goldText\.innerText\s*=\s*gold/);
```

你應該將 `text` 元素的 `innerText` 屬性更新為 `"You now have a new weapon."`。
