...
Nesta etapa, anexe um listener de evento `submit` ao `recipeForm` para que, quando o botão `Convert` for clicado, a função `updateResultsList()` seja chamada.

Anexe um listener de evento `submit` ao `recipeForm`. Quando o formulário for enviado, chame `event.preventDefault()` para impedir o comportamento padrão e depois chame `updateResultsList()`.

Isso permitirá que você acione a função através da interface do usuário preenchendo o formulário e pressionando `Convert`, útil para depuração enquanto adiciona a lógica de conversão na próxima etapa.

# --hints--

Faça com que clicar no botão `Convert` execute `updateResultsList()` adicionando um listener de evento `"submit"` ao `recipeForm` que chama `event.preventDefault()` para impedir o comportamento padrão do formulário.
