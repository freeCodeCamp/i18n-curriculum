...
你 `% Daily Value *` 和 `Saturated Fat 1g 5%` 元素下方的底部邊框未延伸至標籤的全寬。為這兩個元素的 `class` 添加 `no-divider`。

# --hints--

你的 `p` 元素中帶有文字 `% Daily Value *`，應該為 `class` 屬性添加 `no-divider`。不要移除現有的類別。
...
你的 `p` 元素中帶有文字 `Saturated Fat 1g 5%`，應該為 `class` 屬性添加 `no-divider`。不要移除現有的類別。
