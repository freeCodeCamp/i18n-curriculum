...
لعرض الوقت منذ آخر منشور، استدعِ الدالة `timeAgo` ومرر المتغير `bumped_at` كمعلمة. ضع استدعاء هذه الدالة داخل العنصر `td` الأخير.

بعد إجراء هذه التغييرات، مرر عبر الجدول لترى القيم الجديدة المعروضة في عمود `Activity`.

# --hints--

يجب أن تستدعي الدالة `timeAgo` مع المعلمة `bumped_at` داخل العنصر `td` الأخير.
