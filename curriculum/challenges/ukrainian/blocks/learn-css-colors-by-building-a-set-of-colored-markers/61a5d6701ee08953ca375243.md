...
Як другий колірний аргумент передайте функцію `hsl` зі значеннями `223` для відтінку, `90%` для насиченості та `60%` для світлості.

# --hints--

Ваше CSS-правило `.blue` має містити властивість `background`, встановлену в `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%))`.
