...
继续添加一个 `div`，它是 `className` 的 `form-row`。在其中，添加两个 `div` 元素，每个都有 `className` 的 `form-group`。

第一个 `div` 元素应有一个 `label`，它是 `className` 的 `label`，一个 `htmlFor` 属性为 `position`，一个文本 `Position`，以及一个 `input`，其中包含 `id` 的 `position`，`className` 的 `input` 和 `type` 的 `text`。

# --hints--

你的 Player Info 部分应包含一个 `div`，它是 `className` 的 `form-row`。
...
你的 `div.form-row` 应包含两个 `div` 元素，每个都有 `className` 的 `form-group`。
...
你行中的第一个 `form-group` 应有一个 `label`，它是 `className` 的 `label`，以及一个 `htmlFor`，它是 `position`。
...
你行中的第一个 `form-group` 应有一个 `input`，它是 `className` 的 `input`，一个 `id`，它是 `position`，以及 `type` 的 `text`。
