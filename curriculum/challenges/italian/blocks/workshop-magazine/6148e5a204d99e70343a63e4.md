...
La proprietà `gap` è un modo abbreviato per impostare il valore di `column-gap` e `row-gap` contemporaneamente. Se viene fornito un solo valore, imposta sia `column-gap` che `row-gap` su quel valore. Se vengono forniti due valori, imposta `row-gap` sul primo valore e `column-gap` sul secondo.

Imposta la proprietà `.image-wrapper` su `gap` per il selettore `2rem`.
