...
DRI `form-group` 뒤에 `div` 안에 `className`의 `form-group`를 각각 가진 `stats-grid` 요소 두 개를 더 추가하세요. 하나는 DEF용이고 다른 하나는 PHY용입니다.

DEF용에는 `label`의 `htmlFor`를 가진 `def`와 텍스트 `DEF`를 추가한 다음, `id`의 `def`와 `type`의 `number`를 가진 input을 추가하세요.

PHY용에는 `htmlFor`의 `phy`를 가진 레이블과 텍스트 `PHY`를 추가한 다음, `id`의 `phy`와 `type`의 `number`를 가진 input을 추가하세요.

# --hints--

`div`의 `className`의 `form-group` 안에 `div`의 `className`를 각각 가진 `stats-grid` 요소 두 세트를 더 추가해야 합니다.
...
`div`의 `className`의 다섯 번째 `form-group` 요소에는 `label`의 `htmlFor` 속성을 가진 `def`와 `className`의 `label`, 그리고 텍스트 `DEF`가 있어야 합니다.
...
`div`의 `className`의 다섯 번째 `form-group` 요소에는 `input`의 `className`를 가진 `input`와 `id`의 `def`, 그리고 `type`의 `number`가 있어야 합니다.
...
`div`의 `className`의 여섯 번째 `form-group` 요소에는 `label`의 `htmlFor` 속성을 가진 `phy`와 `className`의 `label`, 그리고 텍스트 `PHY`가 있어야 합니다.
...
`div`의 `className`의 네 번째 `form-group` 요소에는 `input`의 `className`를 가진 `input`와 `id`의 `phy`, 그리고 `type`의 `number`가 있어야 합니다.
