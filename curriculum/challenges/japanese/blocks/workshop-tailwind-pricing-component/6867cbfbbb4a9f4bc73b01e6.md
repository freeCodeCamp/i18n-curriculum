...
リスト項目のマーカーに少し間隔を空ける必要があります。そこで、リスト項目内の各 `span` にクラス `text-green-700` と `mr-2` を付けてください。

また、各 `aria-hidden` 要素に `span` 属性を追加してください。

# --hints--

`span` 要素にはクラス `text-green-700` を付けてください。
