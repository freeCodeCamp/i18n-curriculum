...
聲明一個 `checkForStraights` 函數，接受一個數組作爲參數。 如果用戶得到大順子，請用 `40` 的分數更新第五個單選按鈕。 如果用戶得到小順子，請用 `30` 的分數更新第四個單選按鈕。 Regardless, it should always update the last radio button to display a score of 0, with the correct attributes.
