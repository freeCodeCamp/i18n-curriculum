...
次に、JavaScriptファイルで`happyBtn`変数を作成してください。

`querySelector`を使って、`#happy-btn`変数内の`happyBtn`要素への参照を保存してください。

# --hints--

`happyBtn`という名前の変数があるはずです。

```js
assert.exists(happyBtn);
```

`happyBtn`変数は`#happy-btn`要素への参照を保持している必要があります。
