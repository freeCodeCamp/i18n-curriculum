...
Ondoa maandishi ya `Hello` ndani ya `label`, kisha tengeneza `input` ya aina ya `checkbox` na `value` ya `power`.

Mwishowe, onyesha thamani kutoka kwenye safu ya data kando ya kila kisanduku cha kuchagua kwa kuunda kipengele cha `span` kabla ya kufunga `label` na kuwa na maudhui ya maandishi ya `{power}`.

# --hints--

Unapaswa kuondoa maandishi ya `Hello` kutoka kwenye kipengele cha `label`.

```js
assert.notMatch(code, /hello/i)
```

Unapaswa kuunda `input` yenye `type` kuwekwa kuwa `checkbox` na `value` kuwekwa kuwa `power` kama mtoto wa kwanza wa lebo.

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

Unapaswa kuunda kipengele cha `span` chenye `{power}` kama maudhui ya maandishi kama mtoto wa mwisho wa lebo.
