...
Una matriz de adyacencia es una lista bidimensional en la que las filas y columnas representan los vértices del grafo.

Los valores en la matriz representan las aristas o conexiones entre los nodos.

Por ejemplo, si tienes una matriz almacenada en una variable llamada `matrix`, el valor almacenado en `matrix[i][j]`, donde `i` es la fila y `j` es la columna, representa el borde o la conexión entre el nodo `i` y el nodo `j`.
...
Una de las grandes ventajas de usar una matriz de adyacencia es que verificar si hay una arista entre dos nodos toma un tiempo constante `O(1)`. Esto se debe a que el programa solo necesita encontrar ese valor particular en la lista 2D.
...
Y esta es la misma matriz de adyacencia, pero implementada en código Python:
...
Una lista de adyacencia es un arreglo o diccionario que almacena todos los vecinos de cada nodo.

Hay dos maneras de implementar listas de adyacencia:

* Como un arreglo, donde cada índice representa un nodo y la lista almacenada en ese índice contiene sus vecinos.
    
* Como un diccionario, donde cada clave representa un nodo y el valor asociado a esa clave (una lista) contiene sus vecinos.
...
El proceso de búsqueda puede tomar `O(V)` tiempo en el peor de los casos, ya que puede tener que buscar a través de una lista muy larga de vecinos si el nodo está conectado a todos los demás nodos en el grafo.
...
Esta lista de adyacencia está implementada como un diccionario. Cada clave en el diccionario representa un nodo, y el valor asociado a esa clave es una lista con todos los vecinos del nodo correspondiente:
...
Alternativamente, podríamos implementarlo como una lista 2D, donde cada índice representa un nodo. Por ejemplo, el índice 0 representa el nodo A, el índice 1 representa el nodo B, y así sucesivamente:
...
Nota que aunque esta lista 2D pueda parecer similar a la matriz de adyacencia, son bastante diferentes.
