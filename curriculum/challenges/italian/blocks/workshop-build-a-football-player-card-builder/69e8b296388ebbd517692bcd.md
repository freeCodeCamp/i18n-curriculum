...
Ora inizierai a buildare il modulo. All'interno del `div` con il `className` di `form-panel`, aggiungi un `div` per racchiudere la sezione "Player Info". All'interno di quel nuovo `div`, aggiungi `p` con il `className` di `form-section-title` e il testo `Player Info`.

Continuerai ad aggiungere campi a questa sezione nei passaggi successivi.

# --hints--

Il tuo `div.form-panel` dovrebbe contenere un `div` per racchiudere la sezione Player Info.
...
Il `div` dentro `div.form-panel` dovrebbe contenere un `p` con il `className` di `form-section-title` e il testo `Player Info`.
