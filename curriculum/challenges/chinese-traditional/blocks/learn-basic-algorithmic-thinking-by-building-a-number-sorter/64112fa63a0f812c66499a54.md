...
當點擊 `Sort` 按鈕時，它可能看起來運作正常，但這只是因為陣列中的所有值都是單一數字，對於更複雜的值，排序可能不會如預期般運作。

將 `selected` 為 `1` 的 `option` 元素的 `value` 和文字改為 `10`，然後再次點擊 `Sort` 按鈕。

# --hints--

你應該將被 `selected` 的 `option` 元素的 `value` 更新為 10。
...
你應該將被 `selected` 的 `option` 元素的文字更新為 `10`。
