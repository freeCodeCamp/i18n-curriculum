...
select 요소에 안전하게 접근하고 `null` 값을 방지할 방법이 필요합니다. 이를 위해 `val` 키워드를 사용해 `e.currentTarget` 값을 가진 `HTMLSelectElement` 상수를 `as` 타입으로 만드세요.

상수 아래에 `if` 문을 사용해 `val`이 falsy인지 부정 연산자로 확인하세요. `if` 블록 안에서는 `target value not found`을 반환하세요.
...
`val` 상수는 `e.currentTarget` 키워드를 사용해 `HTMLSelectElement` 타입으로 `as` 값을 가져야 합니다.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

`if` 문이 `val`가 null인지 확인해야 합니다.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

`if` 문은 `target value not found`을 반환해야 합니다.
