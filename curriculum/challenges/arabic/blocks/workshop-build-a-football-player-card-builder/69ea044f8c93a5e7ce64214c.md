...
في كتلة جملة `if`، أعد كائنًا ينشر كلًا من `defaultPlayer` و `JSON.parse(saved)` بشكل منفصل. تأكد من فصلهما بفاصلة، إذ هما كائنان منفصلان يتم دمجهما في واحد.

# --hints--

يجب أن تُعيد كتلة `if (saved)` كائنًا ينشر كلًا من `defaultPlayer` والبيانات المحفوظة التي تم تحليلها.
