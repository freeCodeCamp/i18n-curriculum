...
노래가 일시정지될 때 현재 시간을 저장하려면 `songCurrentTime` 변수의 `userData`를 `currentTime` 객체의 `audio`에 할당하세요.

*참고*: 이 단계에서는 `userData.songCurrentTime`가 아직 `null`나 `undefined`가 아니기 때문에 선택적 체이닝을 사용하면 안 됩니다.

# --hints--

기존 `pauseSong` 함수와 그 내용을 수정하면 안 됩니다.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

`audio.currentTime`에 `userData.songCurrentTime`를 할당해야 합니다.
