...
Ora che sai come trovare la prossima canzone da riprodurre, crea una funzione chiamata `playNextSong`. All'interno della funzione `playNextSong`, crea un'istruzione `if` per verificare se il `currentSong` di `userData` è strettamente uguale a `null`. Questo controllerà se non c'è nessuna canzone attualmente in riproduzione nell'oggetto `userData`.

Se la condizione è vera, chiama la funzione `playSong` con il `id` della prima canzone nell'array `userData.songs` come argomento.

Poi aggiungi un'istruzione `return` alla fine del blocco `if`.

# --hints--

Dovresti avere una funzione chiamata `playNextSong`.

```js
assert.isFunction(playNextSong);
```

Quando il `currentSong` di `userData` è strettamente uguale a `null`, la tua funzione `playNextSong` dovrebbe chiamare la funzione `playSong` con il `id` della prima canzone nell'array `userData.songs` come argomento.
...
Dovresti avere un'istruzione `return` alla fine del blocco `if`.
