...
Давайте завершимо підсумком. Виведіть у консоль `catalog.length` та `Object.keys(byDecade).length`, щоб побачити загальну кількість книг і групи за десятиліттями.

Оголосіть `let oldestYear = Infinity` та `let newestYear = 0`. Використайте цикл `for` по `catalog`: для кожного запису, де рік не дорівнює `"Unknown"`, оновіть `oldestYear`, якщо рік менший, і `newestYear`, якщо більший. Виведіть обидва значення.

Ви завершили практичне заняття Heritage Library Catalog!
