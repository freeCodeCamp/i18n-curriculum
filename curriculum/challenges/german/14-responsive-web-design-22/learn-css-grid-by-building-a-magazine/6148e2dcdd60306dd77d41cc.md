...
Zeit für die Gestaltung des letzten Teil deines Magazins - der Bilder.

Die Bilder sind mit einem `aside`-Element umhüllt, das die `image-wrapper`-Klasse verwendet, also erstelle einen `.image-wrapper`-Selektor. Setze die `display`-Eigenschaft auf `grid`.

# --hints--

Du solltest einen `.image-wrapper`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

Dein `.image-wrapper`-Selektor sollte eine `display`-Eigenschaft auf `grid` gesetzt haben.
