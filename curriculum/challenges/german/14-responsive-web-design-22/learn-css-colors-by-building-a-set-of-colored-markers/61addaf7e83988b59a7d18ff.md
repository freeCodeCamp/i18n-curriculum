...
Ändere in der `.sleeve`-CSS-Regel die `border-left-width`-, `border-left-style`- und `border-left-color`-Eigenschaften mit der zusammenfassenden `border-left`-Eigenschaft. Die Werte für die Breite, Stil und Farbe des linken Randes sollten gleich sein.
...
Deine `.sleeve`-CSS-Regel sollte eine zusammenfassende `border-left`-Eigenschaft und den Wert `10px solid black` haben.
