...
在 `playSong` 函式（程式）中，將 `audio.src` 屬性設定為與 `song.src` 相等。這告訴音訊元素在哪裡找到所選歌曲的音訊資料。

此外，將 `audio.title` 屬性設定為與 `song.title` 相等。這會告訴音訊元素要顯示什麼作為歌曲的標題。

# --hints--

你不應該修改現有的 `playSong` 函式（程式）及其內容。
...
你應該將 `audio.title` 設定為 `song.title`。
