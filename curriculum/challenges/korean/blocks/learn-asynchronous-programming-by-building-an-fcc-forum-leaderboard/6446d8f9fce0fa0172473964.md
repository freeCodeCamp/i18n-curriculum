...
나머지 `allCategories` 객체는 이미 완성되어 있습니다.

다음 몇 단계에서는 `allCategories` 객체에서 카테고리 이름을 가져오는 함수를 만들 것입니다.

먼저 `forumCategory`을 매개변수 이름으로 하는 `id`라는 화살표 함수를 만드세요.

# --hints--

빈 함수를 `forumCategory`라는 이름으로 만들어야 합니다.

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

`id` 함수에 `forumCategory`라는 매개변수가 있어야 합니다.
