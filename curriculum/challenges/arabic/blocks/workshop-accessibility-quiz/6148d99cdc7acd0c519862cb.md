...
استهدف كل عناصر `label` داخل عناصر `.info`، واضبط `width` لتكون `10%`، واجعلها لا تقل عن `55px` في العرض.

# --hints--

يجب أن تستخدم إما المحدِّد `.info label` أو `.info > label`.
...
يجب أن تعطي عناصر `label` قيمة `width` تساوي `10%`.
...
يجب أن تعطي عناصر `label` قيمة `min-width` تساوي `55px`.
