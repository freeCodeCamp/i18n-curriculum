...
Agrega un ciclo `for` sobre `catalog`. Dentro del ciclo, declara `const entry = catalog[i]`. Construye una cadena para la fila con los cuatro campos separados por comas y agrégala a `rows`.

Los campos de texto necesitan caracteres literales de comillas dobles en la salida (por ejemplo, `"King, Stephen"`), para que las comas dentro de los valores no rompan el formato CSV. Como `year` es un número, no los necesita.
...
Cada fila debe tener los campos de la entrada separados por comas.

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

Los campos de texto deben estar entre comillas dobles.
