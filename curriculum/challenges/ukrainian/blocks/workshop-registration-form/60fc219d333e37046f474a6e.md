...
Виберіть лише елементи `.inline` та надайте їм `width` зі значенням `unset`. Це видалить попереднє правило, яке встановлювало для всіх елементів `input` властивість `width: 100%`.

# --hints--

Використайте селектор `.inline`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.inline'));
```

Надайте елементам `.inline` властивість `width` зі значенням `unset`.
