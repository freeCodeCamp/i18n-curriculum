...
يجب عليك إضافة عنصر `div` إلى `.penguin`. متوقع عدد `--fcc-expected--` من عناصر `div`، لكن وجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin > div')?.length, 3);
```

يجب عليك إعطاء أول عنصر `div` الـ `class` بقيمة `shirt`.

```js
assert.exists(document.querySelector('.penguin > div.shirt'));
```

يجب عليك وضع `div` قبل `.penguin-body`.
