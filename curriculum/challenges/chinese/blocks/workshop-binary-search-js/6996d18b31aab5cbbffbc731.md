...
你可以添加的最后一个小改进是显示找到目标 `value` 的索引。

所以，不仅仅是在 `if` 语句中返回 `pathToTarget`，还要添加一个包含消息 `Value found at index ${mid}` 的模板字面量。

这样，你的二分查找算法练习就完成了！

# --hints--

你应该在 ` inside the `if` 语句中返回 `pathToTarget, \`Value found at index ${mid}\`。
