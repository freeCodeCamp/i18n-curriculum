...
Fügen Sie dem Club-`input` ein `value`-Attribut mit dem Wert `player.club` hinzu. Danach fügen Sie ein `onChange`-Attribut hinzu, das auf eine Pfeilfunktion gesetzt ist, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `club` auf `e.target.value` gesetzt ist.

# --hints--

Ihr Club-`input` sollte ein `value` haben, das dem Club des Spielers entspricht.
...
Ihr Club-`input` sollte einen `onChange`-Handler haben, der `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `club` auf `e.target.value` gesetzt ist.
