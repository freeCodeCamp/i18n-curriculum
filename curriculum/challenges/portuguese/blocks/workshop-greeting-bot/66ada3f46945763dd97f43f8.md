...
Nas lições anteriores, você aprendeu como trabalhar com concatenação de strings usando o operador `+` para concatenar strings assim:
...
Você deve usar concatenação de strings com o operador `+` para juntar a string `"My name is "` com a variável `bot` seguida por um ponto (`.`). Preste atenção aos espaços. Atribua essa string concatenada à variável `botIntroduction`.

```js
assert.equal(botIntroduction, "My name is teacherBot.");
```

Você deve ter uma quarta declaração `console.log()` no seu código.
