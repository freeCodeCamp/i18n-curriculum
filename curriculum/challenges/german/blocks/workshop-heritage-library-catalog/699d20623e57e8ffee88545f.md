...
Das `byDecade`-Log hat seinen Zweck erfüllt. Entfernen Sie `console.log(byDecade)` aus Ihrem Code.

Erstellen Sie dann, um die Katalogeinträge leichter durchsuchbar zu machen, eine Funktion namens `renderEntry`, die `entry` als Parameter erhält. Lassen Sie den Funktionskörper vorerst leer. Protokollieren Sie unterhalb der Funktion `renderEntry(catalog[0])`.

# --hints--

`renderEntry` sollte eine Funktion sein.

```js
assert.isFunction(renderEntry);
```

`renderEntry` sollte eine Funktion mit einem `entry`-Parameter sein.
...
Der `console.log(byDecade)`-Aufruf sollte entfernt werden.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*byDecade\s*\)/);
```

Sie sollten `renderEntry(catalog[0])` protokollieren.
