...
最后，在第二个 `label` 后的新行上，创建另一个 `input` 元素。 将此 `type` 属性设置为 `number`，将 `min` 属性设置为 `0`（以确保不能添加负卡路里），将 `placeholder` 属性设置为 `Calories`，并将 `id` 属性设置为与第二个 `label` 元素的 `for` 属性相匹配。

# --hints--

你的 `HTMLString` 中应该有两个 `input` 元素。
...
你的新 `input` 元素应将 `type` 属性设置为 `number`。
...
你的 `input` 元素应将 `placeholder` 属性设置为 `Calories`。
...
你的 `input` 元素应将 `id` 属性设置为 `${entryDropdown.value}-${entryNumber}-calories`。
