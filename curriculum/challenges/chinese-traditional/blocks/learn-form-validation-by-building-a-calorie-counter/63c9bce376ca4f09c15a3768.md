...
請記住 `return` 會結束函式的執行。在你的 `if` 區塊之後，你需要控制代碼以處理輸入有效時的邏輯。因為你的 `if` 述語會傳回一個值，所以你不需要 `else` 述語。

使用加法指定運算子將 `currVal` 加到你的 `calories` 總和。你需要使用 `Number` 建構式將 `currVal` 轉換為數字。

`Number` 建構式是一個將值轉換為數字的函式。如果該值無法轉換，則會傳回代表「非數字」的 `NaN`。 
