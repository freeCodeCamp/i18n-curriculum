...
`p` 요소 안에 텍스트 `No Copyright - freeCodeCamp.org`를 가진 `footer` 요소를 중첩하세요.

# --hints--

`footer` 요소를 삭제했거나 여는 태그 또는 닫는 태그가 없습니다.
...
`footer` 요소 안에 `p` 요소가 있어야 합니다. `p` 요소의 여는 태그와 닫는 태그를 반드시 추가하세요.
...
`p` 요소의 텍스트는 `No Copyright - freeCodeCamp.org`이어야 합니다. 텍스트를 빼먹었거나 오타가 있거나 `p` 요소의 여는 태그와 닫는 태그 사이에 있지 않습니다.
