...
이번 단계에서는 `updateResultsList` 함수를 완성하세요.

이제 `units` 배열을 순차적으로 반복하세요. 각 값에 대해 현재 단위(`unitToConvert.value`)와 같지 않은지 확인하세요.

조건이 참이면 기존 단위를 현재 단위로 변환하고 `resultList`에 다음 형식으로 새 목록 항목을 추가하세요:

`ingredient: quantity unit`

예를 들어, 입력이 `flour`, `5`, `gram`이라면 결과는 다음과 같아야 합니다:
...
버튼을 클릭할 때 `updateResultsList` 함수가 DOM을 제대로 업데이트하는지 확인하세요.
