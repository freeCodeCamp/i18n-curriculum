...
到目前為止，你已經有三個函式（程式）：`getElement`、`hideElements` 和 `showElements`。
現在你需要一個函式來幫助你取得隨機的運勢。

宣告一個名為 `getRandomItem` 的函式，它接受一個 `items` 陣列作為引數。這個函式要使用泛型。

在函式（程式）內，使用 `Math.random()` 和 `Math.floor()` 從 `items` 陣列中選取一個隨機的幸運籤。
別忘了傳回所選的幸運籤。

# --hints--

你應該宣告一個名為 `getRandomItem` 的函式。
...
你的 `getRandomItem` 函式應該使用泛型型別參數 `T`。
...
你的 `getRandomItem` 函式應該有一個名為 `items` 的參數，型別為 `T[]`。
...
你的 `getRandomItem` 函式應該有一個返回型別 `T`。
...
你應該在 `index` 函式內宣告一個名為 `getRandomItem` 的變數。
...
`index` 變數應該使用 `Math.floor()` 和 `Math.random()` 搭配 `items.length` 來產生一個隨機索引。
...
你的 `getRandomItem` 函式應該傳回 `items[index]`。
