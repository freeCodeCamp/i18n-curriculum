...
要取得目前歌曲的索引，你可以使用 <dfn>indexOf()</dfn> 方法。`indexOf()` 陣列方法會傳回陣列中第一個可以找到指定元素的索引，若元素不存在則傳回 `-1`。
...
在你的 `getCurrentSongIndex` 函式（程式）中，傳回 `userData?.songs.indexOf()`。對於 `indexOf()` 引數，將它設定為 `userData?.currentSong`。

# --hints--

你應該在你的 `getCurrentSongIndex` 函式中傳回 `userData?.songs.indexOf()`。

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

你應該將 `userData?.currentSong` 傳入 `indexOf()` 方法。
