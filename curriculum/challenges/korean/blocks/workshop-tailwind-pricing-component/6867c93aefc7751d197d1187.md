...
이제 plan의 `h2`를 대상으로 하여 `text-lg`, `font-semibold`, `text-indigo-600` 클래스를 할당하세요.

# --hints--

`h2` 요소는 `text-lg` 클래스를 가져야 합니다.
...
`h2` 요소는 `font-semibold` 클래스를 가져야 합니다.
...
`h2` 요소는 `text-indigo-600` 클래스를 가져야 합니다.
