...
إذا لم توجد أغنية تالية في قائمة التشغيل، استخدم كتلة `else` لإعادة تعيين مفتاح `currentSong` في `userData` إلى `null`، وخصيصة `songCurrentTime` إلى `0`.
...
يجب استخدام ترميز النقطة لتعيين الخاصية `currentSong` في `userData` إلى `null` داخل كتلة `else` في جملة `if` الخاصة بك.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

يجب استخدام ترميز النقطة لتعيين خاصية `songCurrentTime` في `userData` إلى القيمة `0` داخل كتلة `else` في جملة `if` الخاصة بك.
