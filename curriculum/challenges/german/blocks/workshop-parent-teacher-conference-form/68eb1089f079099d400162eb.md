...
Sie haben einigen Elementen im HTML eine `center`-Klasse hinzugefügt. Jetzt sollten Sie diese Klasse definieren, um diese Elemente zu zentrieren.

Zielen Sie auf die `center`-Klasse und zentrieren Sie die Elemente, die sie haben, mit einer `text-align`-Eigenschaft, die auf `center` gesetzt ist.

# --hints--

Sie sollten die Elemente mit der Klasse `center` ansprechen.

```js
assert(new __helpers.CSSHelp(document).getStyle('.center'));
```

Sie sollten die `text-align`-Eigenschaft auf `center` setzen.
