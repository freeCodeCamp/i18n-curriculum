...
¡Hacia el siguiente edificio! Es el verde que está en primer plano. Déle un `repeating-linear-gradient` con el color de su edificio de `0%` a `5%`, y `transparent` de `5%` a `10%`.

# --hints--

Debe darle a `.fb5` una propiedad de `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

Debe darle al `background` un `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Debe darle al `repeating-linear-gradient` un primer color de `--building-color2` de `0%` a `5%`.
...
Debe darle al `repeating-linear-gradient` un segundo color de `transparent` de `5%` a `10%`.
