...
ページからすべてのセルを素早く取得するには、`#container` 要素をその `id` で取得し、結果の `children` プロパティにアクセスします。それを `evalFormula()` 呼び出しの2番目のパラメータとして渡してください。

# --hints--

`evalFormula()` 呼び出しの2番目のパラメータには、`.getElementById()` オブジェクトの `document` メソッドを呼び出すべきです。
...
`container` 呼び出しには `.getElementById()` を引数として渡してください。
...
`children` 呼び出しの結果の `.getElementById()` プロパティにアクセスしてください。
