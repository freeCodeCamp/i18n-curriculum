...
Створіть селектор `.quote`. Надайте йому властивість `color` зі значенням `#00beef`, властивість `font-size` зі значенням `2.4rem` та властивість `text-align` зі значенням `center`.

# --hints--

Ви повинні мати селектор `.quote`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote'));
```

Ваш селектор `.quote` повинен мати властивість `color` зі значенням `#00beef`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.color === 'rgb(0, 190, 239)');
```

Ваш селектор `.quote` повинен мати властивість `font-size` зі значенням `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize === '2.4rem');
```

Ваш селектор `.quote` повинен мати властивість `text-align` зі значенням `center`.
