...
如果没有适当的引号，引用就不能算是真正的引用。 你可以使用 CSS 伪选择器添加它们。

创建一个 `.quote::before` 选择器，将 `content` 属性设置为 `"`，并在其后添加一个空格。

另外，创建一个 `.quote::after` 选择器，将 `content` 属性设置为 `"`，并在其前面添加一个空格。

# --hints--

你应该有一个 `.quote::before` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

你的 `.quote::before` 选择器应该有一个 `content` 属性，设置为 `'" '`。

```js
assert.match(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content, /\"\\"\s\"/);
```

你应该有一个 `.quote::after` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

你的 `.quote::after` 选择器应该有一个 `content` 属性，设置为 `' "'`。
