...
Aggiungi un attributo `value` di `player.dri` all'input DRI. Poi aggiungi un attributo `onChange` impostato su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `dri` impostato su `Number(e.target.value)`.

# --hints--

Il tuo `input` DRI dovrebbe avere un `value` uguale allo stat `dri` del giocatore.
...
Il tuo `input` DRI dovrebbe avere un gestore `onChange` che chiama `setPlayer` con uno spread di `player` e `dri` impostato su `Number(e.target.value)`.
