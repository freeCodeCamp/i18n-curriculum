...
Crea una instancia de `DiscountEngine` y guárdala en la variable `engine`. Después, calcula el mejor precio usando el método `calculate_best_price` y guárdalo en una variable llamada `best_price`.

# --hints--

Debes crear una instancia de `DiscountEngine` con la lista `strategies`.
...
Debes llamar a `engine.calculate_best_price(product, user_tier)` y almacenar el resultado en `best_price`.
