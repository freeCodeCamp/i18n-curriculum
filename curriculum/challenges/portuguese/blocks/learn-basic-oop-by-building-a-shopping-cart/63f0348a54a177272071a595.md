...
Agora, atribua ao `textContent` da variável `showHideCartSpan` o resultado de uma expressão ternária que verifica se `isCartShowing` é verdadeiro.Se for, defina o `textContent` como `"Hide"`, caso contrário defina como `"Show"`.
...
Você deve definir o `textContent` da variável `showHideCartSpan` para `"Hide"` se `isCartShowing` for verdadeiro.

```js
assert.match(code, /showHideCartSpan\s*\.\s*textContent\s*=\s*isCartShowing\s*\?\s*('|"|`)Hide\1\s*:\s*/)
```

Você deve definir o `textContent` da variável `showHideCartSpan` para `"Show"` se `isCartShowing` for falso.
