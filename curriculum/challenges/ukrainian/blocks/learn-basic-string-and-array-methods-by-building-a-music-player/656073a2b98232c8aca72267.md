...
Щоб обробляти інформацію про поточну пісню та відстежувати час її відтворення, створіть властивості `currentSong` і `songCurrentTime`. Встановіть їхні значення відповідно в `null` і `0`.

# --hints--

Вам не слід змінювати існуючий об’єкт `userData` та його вміст.

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

Ваш об’єкт `userData` має містити ключ `currentSong` зі значенням `null`.

```js
assert.propertyVal(userData, "currentSong", null)
```

Ваш об’єкт `userData` має містити ключ `songCurrentTime` зі значенням `0`.
