...
Оновіть функцію зворотного виклику, щоб приймати `match`, `fn` та `args` як параметри. Вона має неявно повернути результат перевірки, чи має `spreadsheetFunctions` власну властивість `fn`.

Не забудьте зробити `fn` малим регістром.

Щоб перевірити, чи існує властивість даного об’єкта, можна скористатися методом `.hasOwnProperty()`.

Метод `hasOwnProperty()` повертає `true` або `false` залежно від того, знайдено властивість в об’єкті чи ні.

Ось приклад як використати метод `hasOwnProperty()`:
...
Першим параметром функції зворотного виклику має бути `match`.
...
Другим параметром функції зворотного виклику має бути `fn`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(?:\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(?:\(\s*args\s*\)|args)\s*=>\s*args\.split\(\s*('|"|`),\1\s*\)\.map\(\s*parseFloat\s*\)\s*;?\s*const\s+apply\s*=\s*\(\s*fn\s*,\s*args\s*\)\s*=>\s*spreadsheetFunctions\s*\[\s*fn\.toLowerCase\(\s*\)\s*\]\s*\(\s*toNumberList\(\s*args\s*\)\s*\)\s*;?\s*return\s+str2\.replace\(\s*functionCall\s*,\s*\(\s*match\s*,\s*fn/);
```

Третім параметром функції зворотного виклику має бути `args`.
...
Викличте метод `.toLowerCase()` на `fn`.
