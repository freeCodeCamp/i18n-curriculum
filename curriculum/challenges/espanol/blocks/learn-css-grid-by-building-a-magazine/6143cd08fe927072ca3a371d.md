...
Dentro de tu elemento `.image-wrapper`, da al segundo elemento `img` un `src` de `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`, un `alt` de `image of a calculator project`, un atributo `loading` establecido a `lazy`, una `class` establecida a `image-2`, un atributo `width` establecido a `400`, y un atributo `height` establecido a `400`.
...
Tu segundo elemento `img` debe tener un `alt` establecido a `image of a calculator project`.
