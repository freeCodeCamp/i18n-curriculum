...
Kwa kuwa pande zote za `4` za menyu zina nafasi sawa ndani, ondoa sifa nne na tumia sifa moja ya `padding` yenye thamani ya `20px`.

# --hints--

Unapaswa kuondoa sifa ya `padding-left`.

```js
assert.notMatch(code, /padding-left/i);
```

Unapaswa kuondoa sifa ya `padding-right`.

```js
assert.notMatch(code, /padding-right/i);
```

Unapaswa kuondoa sifa ya `padding-top`.

```js
assert.notMatch(code, /padding-top/i);
```

Unapaswa kuondoa sifa ya `padding-bottom`.

```js
assert.notMatch(code, /padding-bottom/i);
```

Unapaswa kuweka sifa ya `padding` kuwa `20px`.
