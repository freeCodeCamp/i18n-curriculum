...
Docstrings können über das `__doc__`-Attribut aufgerufen werden, das standardmäßig auf `None` gesetzt ist.

Überprüfen Sie dies, indem Sie `movie1.__doc__` und `series1.__doc__` ausgeben. Beachten Sie, dass das `__doc__`-Attribut von keiner Kindklasse geerbt wird.

# --hints--

Sie sollten `movie1.__doc__` ausgeben.
...
Sie sollten `series1.__doc__` ausgeben.
