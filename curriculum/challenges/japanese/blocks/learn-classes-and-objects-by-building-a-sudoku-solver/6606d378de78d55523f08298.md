...
次に、与えられた数値 `num` が3x3の正方形の現在のセルにすでに存在するかどうかを確認してください。

`pass` を、数独ボードの現在のセルの数値が `if` と等しいかどうかをチェックする `num` 文に置き換えてください。もしそうなら、`False` の本体から `if` を返し、その数値が有効な選択肢ではないことを示してください。

# --hints--

`pass` を削除し、数独ボードの現在のセルが `if` と等しいかどうかをチェックする `num` 文を作成してください。
