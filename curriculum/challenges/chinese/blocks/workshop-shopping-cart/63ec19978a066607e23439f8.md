...
现在你需要获取两个按钮。 继续该模式，获取 `#cart-btn` 和 `#clear-cart-btn` 元素。 将它们分别存储在名为 `cartBtn` 和 `clearCartBtn`的变量中。

# --hints--

你应该使用 `document.getElementById()` 来获取 `#cart-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)cart-btn\1\s*\)/);
```

你应该将 `#cart-btn` 元素分配给一个名为 `cartBtn` 的变量。 记住使用 `const` 来声明变量。

```js
assert.match(code, /const\s+cartBtn\s*=\s*document\.getElementById\(\s*('|"|`)cart-btn\1\s*\)/);
```

你应该使用 `document.getElementById()` 来获取 `#clear-cart-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)clear-cart-btn\1\s*\)/);
```

你应该将 `#clear-cart-btn` 元素分配给一个名为 `clearCartBtn` 的变量。 记住使用 `const` 来声明变量。
