...
Infelizmente, a propriedade `children` está retornando uma coleção de elementos que são semelhantes a arrays, mas não são arrays.Envolva o segundo argumento em `Array.from()` para convertê-lo em um array.

# --hints--

Você deve envolver `document.getElementById('container').children` in `Array.from()`.
