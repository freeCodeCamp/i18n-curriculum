...
Tumia njia ya `filter()` kuondoa kipengee cha wimbo kinacholingana na kigezo cha `id` kutoka kwenye safu ya data ya `userData?.songs`.

Njia ya `filter` huhifadhi tu vipengele vya safu ya data vinavyokidhi kitendakazi cha mwitiko kilichopitishwa kwake:
...
Tumia njia ya `filter()` kwenye `userData?.songs`. Pitisha `song` kama kigezo cha kitendakazi cha mshale cha mwitiko na tumia kurudisha kwa njia isiyo wazi ili ukague kama `song.id` si sawa kabisa na `id`. Weka yote hayo kwenye `userData.songs`.

*Kumbuka*: Hupaswi kutumia uunganishaji wa hiari wakati unaweka matokeo ya `userData?.songs.filter` kwenye `userData.songs` kwa sababu safu ya data ya `allSongs` haitakuwa `undefined` wala `null` wakati huo.

# --hints--

Hupaswi kubadilisha kitendakazi cha `deleteSong` kilichopo.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Unapaswa kutumia njia ya `filter()` kwenye `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

Kitendakazi cha mwitiko cha njia yako ya `filter()` kinapaswa kutumia sintaksia ya mshale na kuwa na kigezo cha `song`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

Njia yako ya filter inapaswa kurudisha `song.id !== id` kwa njia isiyo wazi.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

Unapaswa kuweka njia ya `filter()` uliyotumia kwenye `userData?.songs` kwenye `userData.songs`.
