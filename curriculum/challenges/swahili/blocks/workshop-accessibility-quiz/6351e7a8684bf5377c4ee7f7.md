...
Ingawa si sharti kwa vipengele vya `label` vilivyo na `input` zilizopangwa ndani, bado ni mbinu bora kuunganisha wazi `label` na kipengele chake kinacholingana cha `input`.

Sasa, ongeza sifa ya kipengele ya `for` kwa kila moja ya `label` zako nne inayounganisha `label` na kipengele chake kinacholingana cha redio `input`.

# --hints--

Unapaswa kumpa `label` ya kwanza sifa ya kipengele ya `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

Unapaswa kumpa `label` ya kwanza sifa ya kipengele ya `for` inayolingana na `id` ya kipengele chake cha `input`.
...
Unapaswa kumpa `label` ya pili sifa ya kipengele ya `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

Unapaswa kumpa `label` ya pili sifa ya kipengele ya `for` inayolingana na `id` ya kipengele chake cha `input`.
...
Unapaswa kumpa `label` ya tatu sifa ya kipengele ya `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

Unapaswa kumpa `label` ya tatu sifa ya kipengele ya `for` inayolingana na `id` ya kipengele chake cha `input`.
...
Unapaswa kumpa `label` ya nne sifa ya kipengele ya `for` inayolingana na `id` ya kipengele chake cha `input`.
