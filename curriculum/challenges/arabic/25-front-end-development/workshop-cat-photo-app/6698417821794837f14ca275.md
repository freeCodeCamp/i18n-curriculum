...
Your anchor (`a`) element does not have a `target` attribute. تحقق من وجود مسافة بعد اسم opening tag و/أو أن هناك مسافات قبل جميع أسماء السمات.

```js
assert.isTrue(document.querySelector('a').hasAttribute('target'));
```

The value of the `target` attribute should be `_blank`. إما أنك حذفت القيمة أو لديك خطأ إملائي. تذكر أن تحيط قيمة السمة بعلامات اقتباس.
