...
Um die localStorage-Funktion endlich zum Laufen zu bringen, ändern Sie die Initialisierung des Zustands von `defaultPlayer` zu `loadPlayer`. So stellen Sie sicher, dass der Zustand mit allen verfügbaren gespeicherten Daten startet.

Testen Sie nun die localStorage-Funktion. Alles, was Sie eingeben, sollte nach dem Aktualisieren der Vorschau oder dem Neuladen der Seite erhalten bleiben.

Sie können auch den Bild-`src`-Link zu `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` ändern, um die Bildänderung in Echtzeit zu sehen. Achten Sie nur darauf, den Spielernamen in Maradona zu ändern.

Damit ist Ihr Fußballspieler-Karten-Builder-Projekt abgeschlossen!

# --hints--

Ihr `useState` sollte mit `loadPlayer` statt mit `defaultPlayer` initialisiert werden.
