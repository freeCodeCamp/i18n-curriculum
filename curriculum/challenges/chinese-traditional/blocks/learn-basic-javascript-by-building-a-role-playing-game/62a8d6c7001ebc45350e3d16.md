...
現在使用 `+=` 運算子將字串 `" In your inventory you have: "` 和 `inventory` 的內容添加到 `text.innerText`。請確保包含 `" In your inventory you have: "` 字串開頭和結尾的空格。

# --hints--

你應該再添加一行 `text.innerText`。
...
你應該將字串 `" In your inventory you have: "` 添加到第二行的 `text.innerText`。空格很重要。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*\+=\s*('|") In your inventory you have: \1/);
```

你應該使用串接運算子將 `inventory` 的值添加到你的第二行 `text.innerText` 的結尾。
