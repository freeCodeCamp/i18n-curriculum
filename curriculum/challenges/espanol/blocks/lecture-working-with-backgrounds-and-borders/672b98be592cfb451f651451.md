...
En este ejemplo, estamos usando la propiedad CSS `background` con un valor de `linear gradient`.
...
Este CSS crea un gradiente lineal que transiciona de `red` en la `left` a `yellow` en la `right`. El gradiente se aplica a un elemento con una altura de `40%` de la altura del viewport.  You'll learn more about `vh` units in a future lesson.
...
El `size` determina el tamaño de la forma final del degradado, que puede ser `closest-side`, `closest-corner`, `farthest-side` o `farthest-corner`.
...
Este CSS crea un gradiente radial circular centrado en el elemento. Comienza con `red` en el centro, transiciona a `yellow` en `50%` del radio y termina con `green`.
...
Hace que el gradiente cubra toda la `width` del elemento.
...
Hace que el gradiente solo aparezca dentro del `padding` del elemento.
