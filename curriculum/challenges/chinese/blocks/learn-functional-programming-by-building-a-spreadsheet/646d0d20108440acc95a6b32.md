...
在你的 `elemValue` 函数中，你明确声明了一个名为 `inner` 的函数并返回它。 但是，由于你使用箭头语法，因此可以隐式返回一个函数。 举个例子：

```js
const curry = soup => veggies => {};
```

`curry` 是一个接受 `soup` 参数的函数，并返回一个接受 `veggies` 参数的函数。 使用此语法，更新你的 `addCharacters` 函数以返回一个采用 `character2` 参数的空函数。

# --hints--

你的 `addCharacters` 函数应该使用隐式返回。
...
你的 `addCharacters` 函数应该返回一个具有 `character2` 参数的箭头函数。
