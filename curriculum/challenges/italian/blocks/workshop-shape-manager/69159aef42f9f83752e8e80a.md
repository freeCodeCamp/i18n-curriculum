...
Ora, chiama la funzione `chooseShape()` con `val.value` per mostrare i campi di input corretti, poi chiama `updateResult()` per ricalcolare e visualizzare l'area.

# --hints--

Dovresti chiamare la funzione `chooseShape` con `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

Dovresti chiamare la funzione `updateResult()`.
