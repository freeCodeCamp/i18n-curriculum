...
Aggiungi un altro listener per l'evento all'oggetto globale `document` che chiama la funzione `showFortune`. All'interno della callback, includi un parametro `e` di tipo `Event` e passa `e` come argomento al metodo `showFortune`.

`showFortune` gestirà gli eventi di clic sulle carte, come leggere l'id della carta cliccata dal DOM e mostrare i dettagli della fortuna corrispondente. Lo creerai nei passaggi successivi.

# --hints--

Dovresti assegnare un listener per l'evento `click` all'oggetto globale `document`.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Il tuo listener per l'evento dovrebbe avere un parametro callback chiamato `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

Il parametro della callback dovrebbe avere il tipo `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

All'interno della callback, dovresti chiamare la funzione `showFortune`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

All'interno della funzione `showFortune`, dovresti passare l'argomento `e`.
