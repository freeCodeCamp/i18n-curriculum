...
O problema agora é que, assim que a prévia atualiza ou você recarrega a página, tudo que você adiciona ao cartão do jogador desaparece.

Salvar todas as entradas no localstorage vai impedir isso. Para começar, crie uma constante `STORAGE_KEY` com a string `football_player_card` como seu valor.

# --hints--

Você deve declarar uma variável `STORAGE_KEY`.
...
Sua `STORAGE_KEY` deve ter a string `football_player_card` como valor.
