...
Anche se non è obbligatorio per gli elementi `label` con un `input` annidato, è comunque una buona pratica collegare esplicitamente un `label` con il suo corrispondente elemento `input`.

Ora, aggiungi un attributo `for` a ciascuno dei tuoi quattro `label` che colleghi il `label` al suo corrispondente `input` radio.

# --hints--

Dovresti dare al primo `label` un attributo `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

Dovresti dare al primo `label` un attributo `for` che corrisponda al `id` del suo elemento `input`.
...
Dovresti dare al secondo `label` un attributo `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

Dovresti dare al secondo `label` un attributo `for` che corrisponda al `id` del suo elemento `input`.
...
Dovresti dare al terzo `label` un attributo `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

Dovresti dare al terzo `label` un attributo `for` che corrisponda al `id` del suo elemento `input`.
...
Dovresti dare al quarto `label` un attributo `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

Dovresti dare al quarto `label` un attributo `for` che corrisponda al `id` del suo elemento `input`.
