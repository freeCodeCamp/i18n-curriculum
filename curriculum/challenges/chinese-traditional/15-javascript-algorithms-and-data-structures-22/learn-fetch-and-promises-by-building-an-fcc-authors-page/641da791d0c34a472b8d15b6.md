...
現在 `authorDataArr` 與你剛纔輸出到控制檯的 `data` 相同。 將 `authorDataArr` 輸出到控制檯以確認這一點。

在 `console.log()` 語句中，添加文本 `"Author Data Array:"` 作爲第一個參數，添加 `authorDataArr` 作爲第二個參數。 使用逗號將文本與 `authorDataArr` 分隔。
