...
각 목록 항목 안의 `span` 요소에 `text-green-500`과 `mr-2` 클래스를 할당하여 마커를 녹색으로 만들고 텍스트에서 약간 떨어지게 하세요.

또한, 첫 번째 가격 계획의 `aria-hidden` 요소에 했던 것처럼 `span` 속성도 할당하세요.

# --hints--

`span` 요소에는 `text-green-500` 클래스를 할당해야 합니다.
...
`span` 요소에는 `aria-hidden` 속성을 할당해야 합니다.
