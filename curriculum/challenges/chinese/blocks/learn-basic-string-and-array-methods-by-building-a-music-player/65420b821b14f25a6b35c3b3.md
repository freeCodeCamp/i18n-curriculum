...
当歌曲显示在页面上时，它应该显示每首歌曲的标题、艺术家、时长以及删除按钮。 

在接下来的几个步骤中，你将学习如何使用 <dfn>map()</dfn> 方法为每首歌曲添加新的 HTML。 下一步将更详细地解释该方法。

首先使用 `const` 声明一个名为 `songsHTML` 的变量，并将其分配给 `array.map()`。

# --hints--

你应该有一个名为 `songsHTML` 的 `const` 变量。

```js
assert.match(code, /const\s+songsHTML/);
```

你应该将 `array.map()` 分配给你的 `songsHTML`。
