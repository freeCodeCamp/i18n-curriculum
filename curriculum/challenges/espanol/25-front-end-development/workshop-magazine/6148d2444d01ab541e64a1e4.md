...
Crea un selector `.quote`. Asigna una propiedad `color` establecida en `#00beef`, una propiedad `font-size` establecida en `2.4rem` y una propiedad `text-align` establecida en `center`.

# --hints--

Debes tener un selector `.quote`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote'));
```

Tu selector `.quote` debe tener una propiedad `color` establecida en `#00beef`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.color, 'rgb(0, 190, 239)');
```

Tu selector `.quote` debe tener una propiedad `font-size` establecida en `2.4rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize, '2.4rem');
```

Tu selector `.quote` debe tener una propiedad `text-align` establecida en `center`.
