...
`btn-primary` 클래스는 앱에서 주로 사용할 기본 색상입니다. 사용자가 취하기를 원하는 동작을 강조할 때 유용합니다.

버튼에서 부트스트랩의 `btn-default` 클래스를 `btn-primary`로 바꾸세요.

이 버튼에는 여전히 `btn`와 `btn-block` 클래스가 필요하다는 점을 참고하세요.

# --hints--

버튼에 `btn-primary` 클래스가 있어야 합니다.

```js
assert.isTrue(document.querySelector('button')?.classList?.contains('btn-primary'));
```

버튼에는 여전히 `btn`와 `btn-block` 클래스가 있어야 합니다.
