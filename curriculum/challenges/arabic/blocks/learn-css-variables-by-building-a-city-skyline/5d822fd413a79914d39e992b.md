...
أضف المتغير `--building-color1` كقيمة لخاصية `background-color` في `.fb4b`. ثم، احذف خاصية `background-color` من `.fb4`.

# --hints--

يجب حذف خاصية `background-color` من `.fb4`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb4")?.backgroundColor);
```

يجب إعطاء `.fb4b` خاصية `background-color` بقيمة `--building-color1`.
