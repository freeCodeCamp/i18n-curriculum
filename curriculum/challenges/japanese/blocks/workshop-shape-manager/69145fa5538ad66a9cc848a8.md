...
`updateResult` 関数はドロップダウンから選択された形状タイプを読み取り、対応する入力値を集め、`shape` オブジェクトを作成し、それを `calculateArea()` 関数に渡してエリアを計算し、計算結果を `resultText` 要素に表示します。

関数内で選択された形状を確認し、`calculateArea` 関数に渡す正しいオブジェクトを作成してください。

形状が `"circle"` の場合は、タイプと半径入力の数値を含むオブジェクトを使って `calculateArea` 関数を呼び出してください。

# --hints--

`updateResult` 関数は形状が circle のときに結果のテキストを正しく更新する必要があります。
