...
All'interno della stringa letterale, crea un elemento `div` con il `id` impostato sul `index` ottenuto dal metodo array `.forEach()`. Ricorda di usare l'interpolazione di stringa per farlo.

Inoltre, aggiungi un `class` di `"user-card"` al `div`.
...
Dovresti creare un elemento `div` con la classe `"user-card"`.
...
Il tuo elemento `div` dovrebbe avere il `id` di `index`. Ricorda di usare l'interpolazione di stringa per impostare il `id`.
...
Il tuo elemento `div` dovrebbe essere vuoto.
