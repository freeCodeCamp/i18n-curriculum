...
Ndani ya safu ya data `allSongs`, tengeneza kitu chenye sifa na thamani zifuatazo:
...
Safu ya data `allSongs` inapaswa kuwa na thamani moja.

```js
assert.equal(allSongs.length, 1);
```

Safu ya data `allSongs` inapaswa kuwa na kitu kama thamani yake ya kwanza.

```js
assert.isObject(allSongs[0]);
```

Safu ya data `allSongs` inapaswa kuwa na kitu chenye sifa ya `id` iliyowekwa kwa nambari `0`.

```js
assert.equal(allSongs[0].id, 0);
```

Safu ya data `allSongs` inapaswa kuwa na kitu chenye sifa ya `title` iliyowekwa kwa mfuatano wa herufi `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Safu ya data `allSongs` inapaswa kuwa na kitu chenye sifa ya `artist` iliyowekwa kwa mfuatano wa herufi `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Safu ya data `allSongs` inapaswa kuwa na kitu chenye sifa ya `duration` iliyowekwa kwa mfuatano wa herufi `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Safu ya data `allSongs` inapaswa kuwa na kitu chenye sifa ya `src` iliyowekwa kwa mfuatano wa herufi `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
