...
在 `update` 函數的第一行，使用 `monsterStats.style.display` 將 `display` 的值更改爲 `none`。

# --hints--

你應該在 `update` 函數中使用 `monsterStats.style.display`。

```js
assert.match(update.toString(), /monsterStats\.style\.display/);
```

你應該將 `monsterStats.style.display` 設置爲 `none`。

```js
assert.match(update.toString(), /monsterStats\.style\.display\s*=\s*('|")none\1/);
```

你應該在 `update` 函數的第一行添加你的代碼。
