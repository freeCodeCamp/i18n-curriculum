...
Necesitas mostrar los productos disponibles en tu HTML. Comienza usando el operador de asignación por suma para añadir una cadena vacía de plantilla literal a la propiedad `innerHTML` de la variable `dessertCards`.

# --hints--

Debes acceder a la propiedad `innerHTML` de la variable `dessertCards`.

```js
assert.match(code, /dessertCards\.innerHTML/);
```

Debes usar el operador de asignación por suma en la propiedad `innerHTML`.

```js
assert.match(code, /dessertCards\.innerHTML\s*\+=\s*/);
```

Debes añadir una cadena vacía de plantilla literal a la propiedad `innerHTML`.
