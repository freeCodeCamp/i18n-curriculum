...
Повернувшись у слухач подій, потрібно оновити текст елемента `result`. Ви можете використати тернарний оператор для досягнення цього завдання.

Ось приклад призначення результату тернарного оператора текстовому вмісту елемента:

```js
el.textContent = condition ? "Use this text if the condition is true" : "Use this text if the condition is false";
```

After the `if` statement, use a ternary operator to check the truthiness of calling `isSpam()` with `messageInput.value` as the argument. If true, set the `textContent` property on the `result` element to `"Oh no! This looks like a spam message."`. В іншому випадку встановіть її на `"This message does not seem to contain any spam."`
...
Використайте тернарний синтаксис, щоб перевірити правдивість `isSpam(messageInput.value)`.
...
Помилковий вираз вашого потрійного повинен встановити властивість `textContent` елемента `result` на `"This message does not seem to contain any spam."`.
...
Після трійкового, встановіть властивість `value` на елементі `messageInput` на порожній рядок.
