...
داخل عنصر `.image-wrapper`، أعطِ العنصر الثاني `img` خاصية `src` بقيمة `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`، وخاصية `alt` بقيمة `image of a calculator project`، وخاصية `loading` بقيمة `lazy`، وصنف `class` بقيمة `image-2`، وخاصية `width` بقيمة `400`، وخاصية `height` بقيمة `400`.

# --hints--

يجب أن يحتوي العنصر الثاني `img` على خاصية `src` بقيمة `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`.
