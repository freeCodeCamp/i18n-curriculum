...
通過在 `.foreground-buildings` 元素的頂部和底部添加兩個空 `div` 元素，以及在 `.fb2` 和 `.fb3` 之間再添加一個，來再次使建築物緊湊。

# --hints--

你應該添加兩個 `div` 元素作爲 `.foreground-buildings` 的最初的子元素。
...
你應該添加兩個 `div` 元素作爲 `.foreground-buildings` 的最後的子元素。
