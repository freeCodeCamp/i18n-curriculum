...
為 `.fb2-window` 元素設定 `width` 為 `22%`、`height` 為 `100%`，以及 `background-color` 為你的 `--window-color3` 變數。

# --hints--

你應該給 `.fb2-window` 元素一個 `width` 為 `22%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

你應該給 `.fb2-window` 元素一個 `height` 為 `100%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

你應該為 `.fb2-window` 元素設定 `background-color` 為 `--window-color3`。
