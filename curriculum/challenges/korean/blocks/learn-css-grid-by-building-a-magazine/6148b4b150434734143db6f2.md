...
`grid-column` 속성은 요소가 시작하고 끝나는 열을 결정한다는 점을 기억하세요. 그리드에 몇 개의 열이 있을지 확실하지 않을 때가 있지만, 요소가 마지막 열에서 멈추게 하고 싶을 수 있습니다. 이럴 때는 끝 열에 `-1`을 사용할 수 있습니다.

`.hero` 선택자를 만들고 `grid-column` 속성을 `1 / -1`로 설정하세요. 이렇게 하면 요소가 그리드 전체 너비에 걸치게 됩니다.
...
`.hero` 선택자에 `grid-column` 속성을 `1 / -1`로 설정해야 합니다.
