...
Uma última pequena melhoria que você pode adicionar é também mostrar o índice em que o alvo `value` é encontrado.

Então, em vez de retornar apenas `pathToTarget` na declaração `if`, adicione um template literal com a mensagem `Value found at index ${mid}`.

Com isso, sua oficina de algoritmo de busca binária estará completa!

# --hints--

Você deve retornar `pathToTarget, \`Valor encontrado no índice ${mid}\`` inside the `na declaração `if`.
