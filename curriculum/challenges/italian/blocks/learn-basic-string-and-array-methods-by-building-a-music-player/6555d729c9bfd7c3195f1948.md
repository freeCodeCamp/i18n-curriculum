...
`textContent` imposta il testo di un nodo e ti permette di impostare o recuperare il contenuto testuale di un elemento HTML.
...
Puoi usare un operatore ternario per impostare condizionatamente il valore del contenuto testuale. Ecco un esempio di assegnazione del risultato di un operatore ternario a una variabile:

```js
const example = condition ? "I'm true" : "I'm false";
```

Usa un operatore ternario per verificare se `currentTitle` valuta a un valore truthy. Se sì, imposta `playingSong.textContent` su `currentTitle`. Altrimenti, impostalo su una stringa vuota.

Poi, subito sotto, usa un operatore `ternary` per verificare se `currentArtist` è truthy. Se sì, imposta `songArtist.textContent` su `currentArtist`. Altrimenti, impostalo su una stringa vuota.

# --hints--

Dovresti concatenare la proprietà `textContent` a `playingSong`.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

Dovresti usare l'operatore `ternary` per impostare la proprietà `textContent` di `playingSong` su `currentTitle` o `""`.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

Dovresti concatenare la proprietà `textContent` a `songArtist`.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

Dovresti usare l'operatore `ternary` per impostare la proprietà `textContent` di `songArtist` su `currentArtist` o `""`.
