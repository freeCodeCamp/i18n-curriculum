...
Ahora que el método `__init__` está completo, crea una variable llamada `series1` dentro del bloque `try`. Asígnale una instancia de `TVSeries` pasando argumentos de tu elección. Luego, imprime `series1` en la consola.

# --hints--

Debes asignar una instancia de `TVSeries` a una variable llamada `series1` dentro del bloque `try`.
...
Debes imprimir `series1` dentro del bloque `try`.
