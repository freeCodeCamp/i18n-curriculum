...
Elimina la propiedad `background-color` y su valor de `.bb4`, y agrégala a las tres nuevas secciones `.bb4a`, `.bb4b`, y `.bb4c`, para que solo las secciones estén rellenadas.

# --hints--

Debes eliminar el `background-color` de `.bb4`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb4")?.backgroundColor);
```

Debes dar a `.bb4a` un `background-color` de `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.backgroundColor.trim(), "var(--building-color4)");
```

Debes dar a `.bb4b` un `background-color` de `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.backgroundColor.trim(), "var(--building-color4)");
```

Debes dar a `.bb4c` un `background-color` de `--building-color4`.
