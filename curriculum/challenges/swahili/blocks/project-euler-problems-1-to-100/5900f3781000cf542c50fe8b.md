...
title: 'Zoezi la 12: Nambari ya pembetatu yenye kugawanyika kwa urahisi sana'
challengeType: 1
forumTopicId: 301746
...
Mfululizo wa nambari za pembetatu hutengenezwa kwa kuongeza nambari asilia. Hivyo nambari ya pembetatu ya nambari 7 itakuwa 1 + 2 + 3 + 4 + 5 + 6 + 7 = 28. Vipengele kumi vya kwanza vitakuwa:

<div style='text-align: center;'>1, 3, 6, 10, 15, 21, 28, 36, 45, 55, ...</div>

Tuchapishe vizingiti vya nambari za pembetatu saba za kwanza:
...
Tunaweza kuona kuwa 28 ni nambari ya pembetatu ya kwanza kuwa na vizingiti zaidi ya tano.

Ni thamani gani ya nambari ya pembetatu ya kwanza kuwa na vizingiti zaidi ya `n`?

# --hints--

`divisibleTriangleNumber(5)` inapaswa kurudisha nambari.

```js
assert.isNumber(divisibleTriangleNumber(5));
```

`divisibleTriangleNumber(5)` inapaswa kurudisha 28.

```js
assert.strictEqual(divisibleTriangleNumber(5), 28);
```

`divisibleTriangleNumber(23)` inapaswa kurudisha 630.

```js
assert.strictEqual(divisibleTriangleNumber(23), 630);
```

`divisibleTriangleNumber(167)` inapaswa kurudisha 1385280.

```js
assert.strictEqual(divisibleTriangleNumber(167), 1385280);
```

`divisibleTriangleNumber(374)` inapaswa kurudisha 17907120.

```js
assert.strictEqual(divisibleTriangleNumber(374), 17907120);
```

`divisibleTriangleNumber(500)` inapaswa kurudisha 76576500.
