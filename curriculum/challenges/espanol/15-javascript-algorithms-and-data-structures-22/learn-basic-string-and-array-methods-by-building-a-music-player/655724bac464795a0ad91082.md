...
A continuación, necesitas comprobar si la lista de reproducción está vacía. Si lo está, debes restablecer el objeto `userData` a su estado original.

Usa una declaración `if` para verificar si `userData?.songs` tiene una longitud de `0`.

# --hints--

Debes crear una declaración vacía `if` con la condición `userData?.songs.length === 0`.
