...
如果碟的數量是偶數且餘數相等於 `1`，則允許在原始碼桿和輔助桿之間移動。為你的 `else` 語句添加列印允許移動的內容，並以正確的引數呼叫 `make_allowed_move()`。

如果你查看輸出，你會看到執行在第三次移動時因為 `IndexError` 而停止。這是因為程式碼仍未完成，且需要你即將撰寫的 `else` 語句。為了讓它能運作，將你的 `make_allowed_move()` 呼叫改成註解。
...
你應該在 `else` 語句中有 `print(f'Move {i + 1} allowed between {source} and {auxiliary}')`。
...
你應該在 `else` 語句中包含 `# make_allowed_move(source, auxiliary)`。
