...
L'oggetto `topic_list` contiene un array `topics` che include gli ultimi argomenti pubblicati nel forum.

Destruttura l'array `topics` dall'oggetto `topic_list`.

# --hints--

Dovresti usare `const` per destrutturare la variabile `topics` dall'oggetto `topic_list`.
