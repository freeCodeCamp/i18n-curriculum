...
现在让我们深入了解从 90 到 99 的数字。

在西班牙语中，数字 90 被称为 `noventa`。

这些数字是通过使用表示十位数的单词 `noventa`（90），后跟连词 `y`（"and"），然后是个位数字（1-9）形成的。
...
数字 `noventa y uno` (91) 在这个范围内是独特的，因为它在阳性名词前变成 `noventa y un`。例如：
