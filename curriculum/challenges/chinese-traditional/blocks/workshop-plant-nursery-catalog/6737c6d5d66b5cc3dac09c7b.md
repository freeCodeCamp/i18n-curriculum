...
Maps 和 Sets 共享一些具有相同名稱和用途的方法。例如，你可以在 set 上使用 `has` 方法來檢查它是否包含特定元素。

使用 `has` 方法檢查 `ballerina` 物件是否包含在你的 `plantsSet` 中，並將結果記錄到主控台。

# --hints--

你應該將 `plantsSet.has(ballerina)` 記錄到主控台。
