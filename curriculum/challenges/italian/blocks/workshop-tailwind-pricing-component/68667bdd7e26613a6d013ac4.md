...
Aggiungi le classi `text-4xl`, `font-bold` e `text-white` all'elemento `p` che contiene il testo del prezzo per renderlo più visibile.
...
Il tuo elemento `p` dovrebbe avere la classe `text-white`.
