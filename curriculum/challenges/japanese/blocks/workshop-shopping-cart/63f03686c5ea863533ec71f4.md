...
変数`tax`を宣言し、新しい`.calculateTaxes()`メソッドを呼び出して、その引数に`subTotal`を渡した値を代入してください。

# --hints--

`const`を使って、`tax`という名前の変数を宣言してください。
...
新しい`.calculateTaxes()`メソッドを呼び出し、`subTotal`を引数として渡した値を`tax`変数に代入してください。`this`キーワードを使うのを忘れないでください。
