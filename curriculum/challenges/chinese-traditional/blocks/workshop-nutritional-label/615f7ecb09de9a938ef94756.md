...
在最後一個 `p` 元素下方，添加另一個帶有文本 `Total Carbohydrate 37g 13%` 的 `p` 元素。 像之前一樣，使用 `span` 元素使文本 `Total Carbohydrate` 和 `13%` 加粗。 然後，將營養成分和數量放在一個 `span` 中，以便對齊。
...
你的新的 `p` 元素應包含文本 `Total Carbohydrate 37g 13%`。
...
`Total Carbohydrate` 周圍的 `span` 元素應該將 `class` 屬性設置爲 `bold`。
...
`13%` 周圍的 `span` 元素應該將 `class` 屬性設置爲 `bold`。
