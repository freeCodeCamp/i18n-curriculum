...
이제 `.no-divider` 요소 아래에 별도의 구분선을 추가해야 합니다.

첫 번째 `.no-divider` 요소 뒤에는 `.divider`가 있습니다. 두 번째 `.divider` 요소 뒤에도 또 다른 `.no-divider`를 만드세요.

# --hints--

`div` 요소 끝에 새 `.daily-value`를 만들어야 합니다.
