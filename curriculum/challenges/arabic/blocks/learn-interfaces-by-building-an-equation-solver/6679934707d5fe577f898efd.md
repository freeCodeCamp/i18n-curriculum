...
أضف حالة أخرى عندما يكون طول `results` هو `2`. في هذه المرة، عيّن لـ `result_list` قائمة تحتوي على سلسلتين نصيتين بالصيغة `x1 = <root1>` و `x2 = <root2>`. اجعل الحل يعرض العلامتين الموجبة والسالبة معًا.

# --hints--

لا تعدل قيمة الموضوع في تعبير `match` الخاص بك.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("len(results)")`)) })
```

لا تعدل كتل `case` الموجودة لديك.
...
يجب إنشاء `case` جديدة بالنمط `2` بعد كتلة الـ `case` الموجودة.
...
يجب تعيين قائمة تحتوي على سلسلتين نصيتين بالصيغة `x1 = <root1>` و `x2 = <root2>` إلى `result_list` داخل جسم الـ `case` الجديد. اعرض العلامتين الموجبة والسالبة للنتائج.
