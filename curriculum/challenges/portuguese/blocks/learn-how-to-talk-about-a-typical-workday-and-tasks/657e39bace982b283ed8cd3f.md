...
A expressão `not that much` é usada para indicar uma pequena quantidade ou grau, muitas vezes em resposta a uma pergunta sobre quantidade ou frequência. For example:
...
Por outro lado, `at all times` significa sempre ou continuamente, indicando algo que acontece sem interrupção. For example:
