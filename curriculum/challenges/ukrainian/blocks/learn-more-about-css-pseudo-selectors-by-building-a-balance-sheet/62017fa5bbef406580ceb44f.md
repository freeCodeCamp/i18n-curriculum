...
Тепер, коли ви додали ключове слово `!important`, ви можете видалити `:not(.sr-only)` із свого селектора `span`.

# --hints--

У вас не має бути селектора `span:not(.sr-only)`.

```js
assert(!(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)')));
```

У вас має бути селектор `span`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span'));
```

Ви не повинні змінювати `font-weight`.
