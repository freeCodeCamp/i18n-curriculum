...
title: Rahmen um Ihre Elemente hinzufügen
challengeType: 0
...
Zum Beispiel, wenn wir einen roten Rahmen von 5 Pixeln um ein HTML-Element erstellen wollten, könnten wir diese Klasse verwenden:
...
Erstellen Sie eine Klasse namens `thick-green-border`. Diese Klasse soll einen 10px breiten, durchgezogenen, grünen Rahmen um ein HTML-Element hinzufügen. Wenden Sie die Klasse auf Ihr Katzenfoto an.

Denken Sie daran, dass Sie mehrere Klassen auf ein Element anwenden können, indem Sie das `class`-Attribut verwenden und die Klassennamen mit einem Leerzeichen trennen. Zum Beispiel:
...
Ihr `img`-Element sollte die Klasse `thick-green-border` haben.

```js
assert.isTrue(document.querySelector('img').classList.contains('thick-green-border'));
```

Ihr Bild sollte eine Rahmenbreite von `10px` haben.
...
Ihr Bild sollte einen Rahmenstil von `solid` haben.
...
Der Rahmen um Ihr `img`-Element sollte grün sein.
