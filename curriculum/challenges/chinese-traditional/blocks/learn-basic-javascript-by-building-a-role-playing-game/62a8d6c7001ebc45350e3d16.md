...
現在使用 `+=` 運算符將字符串 `" In your inventory you have: "` 和 `inventory` 的內容添加到 `text.innerText` 中。 確保 `" In your inventory you have: "` 字符串的開頭和結尾包含空格。

# --hints--

你應該添加另一行 `text.innerText`。
...
你應該將字符串 `" In your inventory you have: "` 添加到第二行 `text.innerText`。 間距很重要。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*\+=\s*('|") In your inventory you have: \1/);
```

你應該使用連接運算符將 `inventory` 的值添加到第二個 `text.innerText` 行的末尾。
