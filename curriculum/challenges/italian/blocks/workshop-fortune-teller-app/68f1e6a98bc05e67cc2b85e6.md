...
Dichiara una variabile chiamata `cardElement` e usa la classe `card_container` per ottenere l'elemento. Usa il selettore `closest` per prenderlo da `target`.

# --hints--

Dovresti dichiarare una variabile chiamata `cardElement` all'interno di `showFortune`.
...
La tua `cardElement` dovrebbe usare `target?.closest` per trovare l'elemento `.card_container`.
...
Dovresti passare `.card_container` come argomento a `closest`.
