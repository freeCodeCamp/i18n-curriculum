...
Ahora, es momento de definir qué sucede cuando el radio button está seleccionado.

Estás estilizando solo el círculo interior cuando el botón de opción está seleccionado, así que tu selector debe ser `.contact-method-radio-btn:checked::before`.

En la regla, establece `transform` a `translate(3px, 3px) scale(1)` y `background-color` a `lightgreen`.

El `transform` mueve ligeramente el círculo interior y lo escala al tamaño completo con `scale(1)`, y un fondo de lightgreen llena el círculo con color para mostrar que la opción está seleccionada, haciéndolo visible.

# --hints--

Debes apuntar al pseudo-elemento `.contact-method-radio-btn:checked::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

Establece la propiedad `transform` a `translate(3px, 3px) scale(1)`.
...
Establece la propiedad `background-color` a `lightgreen`.
