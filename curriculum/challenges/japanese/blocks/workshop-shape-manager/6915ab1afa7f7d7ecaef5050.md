...
計算結果を最終的に表示するために、`propertyInputs` ループを使って `for...of` オブジェクトを `Object.entries()` で繰り返してください。ループの宣言で `input` の前にカンマを置くことでキーを無視できます。

# --hints--

`for...of` の各エントリーをキーをスキップして `Object.entries(propertyInputs)` のみを分解代入する `input` ループを作成してください。

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

`for...of` ループは `Object.entries(propertyInputs)` を繰り返す必要があります。
