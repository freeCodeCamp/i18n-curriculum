...
見た目が良くなりましたね。次は、メニューの上下にも同じ`20px`のパディングを追加してみましょう。

# --hints--

`padding-left`や`padding-right`のプロパティは削除しないでください。
...
`padding-top`のプロパティを`20px`に設定してください。
...
`padding-bottom`のプロパティを`20px`に設定してください。
...
`.menu`要素には`padding-top`が`20px`である必要があります。
...
`.menu`要素には`padding-bottom`が`20px`である必要があります。
