...
Отримайте доступ до елементів `#next` і `#previous` з HTML-файлу.

Призначте їх змінним з іменами `nextButton` і `previousButton` відповідно.

# --hints--

У вас має бути змінна з ім’ям `nextButton`.

```js
assert.isDefined(nextButton);
```

Ви маєте призначити елемент `#next` змінній `nextButton`.

```js
assert.equal(nextButton, document.getElementById('next'));
```

У вас має бути змінна з ім’ям `previousButton`.

```js
assert.isDefined(previousButton);
```

Ви маєте призначити елемент `#previous` змінній `previousButton`.
