...
Додайте елемент `img` перед вашим елементом `.keys`. Дайте `img` атрибут `class` зі значенням `logo`, а множину `src` встановіть у `https://cdn.freecodecamp.org/platform/universal/fcc_primary.svg`. Додайте йому текст `alt` зі значенням `freeCodeCamp Logo`.
