...
`Est. 2020` をイタリック体にするには、`established` クラスセレクターを作成し、`font-style` プロパティに `italic` を指定してください。
...
`font-style` プロパティを `italic` に設定してください。
...
`.established` セレクターは `font-style` プロパティを `italic` に設定する必要があります。
