...
Attualmente, sia i film sia le serie tv vengono mostrati tutti insieme quando stampi il catalogo. Nei passaggi successivi, ti occuperai di filtrare gli elementi in due categorie da visualizzare separatamente.

All'interno della classe `MediaCatalogue`, crea un metodo chiamato `get_movies` e fallo restituire una lista contenente solo le istanze della classe genitore `Movie` che si trovano in `self.items`.

Usa l'approccio che preferisci per farlo, ma ricorda di usare la funzione `type` dato che `isinstance` non sarà in grado di distinguere tra classi genitore e figlio.

# --hints--

La tua classe `MediaCatalogue` dovrebbe avere un metodo chiamato `get_movies`.
...
Il tuo metodo `get_movies` dovrebbe restituire una lista contenente tutti gli elementi in `self.items` che sono istanze della classe `Movie`.
