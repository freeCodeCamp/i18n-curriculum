...
次に、`label` と `className` の `section` を持つ `column` を作成し、テキスト `How did you get your powers?` を追加してください。`label` の中に `select` 要素を作成してください。
...
`label` 要素にはテキスト `How did you get your powers?` を含めてください。
...
`select` 要素の中に `label` 要素を作成してください。
