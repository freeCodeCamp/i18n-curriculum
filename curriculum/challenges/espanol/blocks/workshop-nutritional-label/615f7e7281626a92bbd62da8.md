...
Debajo de su último elemento `p`, cree otro elemento `p` con el texto `Sodium 160mg 7%`. Ponga `Sodium` y `7%` cada uno en su propio `span` con una clase de `bold`, como hizo con los otros.

Luego, agregue un elemento `span` adicional alrededor de `Sodium 160mg` para la alineación nuevamente.
...
El `span` alrededor de `Sodium` debe tener el atributo `class` establecido en `bold`.
...
Un elemento `span` debería envolver el texto `7%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.lastElementChild?.innerText === '7%');
```

El elemento `span` alrededor de `7%` debe tener el atributo `class` establecido en `bold`.
