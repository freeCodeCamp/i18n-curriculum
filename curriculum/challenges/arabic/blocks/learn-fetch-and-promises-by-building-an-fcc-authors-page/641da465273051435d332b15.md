...
البيانات التي تحصل عليها من طلب `GET` غير قابلة للاستخدام في البداية. لجعل البيانات قابلة للاستخدام، يمكنك استخدام الطريقة `.json()` على كائن `Response` لتحليلها إلى JSON. إذا وسّعت `Prototype` لكائن `Response` في وحدة التحكم بالمتصفح، سترى الطريقة `.json()` هناك.

احذف `console.log(res)` وأعد ضمنيًا `res.json()` بدلاً منه.
...
يجب إعادة `res.json()` ضمنيًا في `.then()` الخاص بك.
