...
Después de los tres elementos `p` dentro de tu elemento `.text`, crea un elemento `blockquote`. Dentro de eso, agrega un elemento `hr`, un elemento `p` con la `clase` establecida como `quote`, y un segundo elemento `hr`.

Dale al elemento `.quote` el texto `El programa entero debe ser una serie de proyectos`.

# --hints--

Debes crear un nuevo elemento `blockquote` dentro de tu elemento `.text`.

```js
assert.exists(document.querySelector('.text blockquote'));
```

Tu elemento `blockquote` debe venir después de tus tres elementos `p`.

```js
assert.equal(document.querySelector('.text')?.children?.[3]?.localName, 'blockquote');
```

Tu elemento `blockquote` debe tener dos elementos `hr`.

```js
assert.lengthOf(document.querySelectorAll('.text blockquote hr'), 2);
```

Tu elemento `blockquote` debe tener un elemento `p`.

```js
assert.exists(document.querySelector('.text blockquote p'));
```

Los hijos de tu `blockquote` deben estar en el orden correcto.
...
Tu nuevo elemento `p` debe tener la `clase` establecida como `quote`.

```js
assert.equal(document.querySelector('.text blockquote p')?.className, 'quote');
```

Tu nuevo elemento `p` debe tener el texto `El programa entero debe ser una serie de proyectos`.
