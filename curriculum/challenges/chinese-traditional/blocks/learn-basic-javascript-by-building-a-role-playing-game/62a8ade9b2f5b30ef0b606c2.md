...
為你的物件添加一個名為 `text` 的最後屬性。將此屬性的字串值指定為在 `goTown` 函式（程式）中指定給 `text.innerText` 的字串值。

# --hints--

你的第一個 `locations` 值應該有一個 `text` 屬性。
...
你的 `text` 屬性應該有值 `"You are in the town square. You see a sign that says \"Store\"."`
