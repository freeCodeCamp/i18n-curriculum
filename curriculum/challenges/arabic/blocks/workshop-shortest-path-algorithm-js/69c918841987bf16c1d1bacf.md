...
الآن، يجب طباعة النتيجة لكل عقدة يمكن الوصول إليها.

بعد متغير `path`، أضف استدعاء `console.log()` مع نص قالب يطبع:

```md
\n{startNode}-{nodeNo} distance: {distances[nodeNo]}\nPath: {path}
```

على سبيل المثال، إذا كانت `startNode` تساوي `0` و`nodeNo` تساوي `3`، يجب أن يكون الإخراج بهذا الشكل:
...
يجب أن يكون لديك استدعاء `console.log()` داخل حلقة `for...of`.
...
يجب أن يستخدم استدعاء `console.log()` نص قالب يحتوي على `${startNode}-${nodeNo} distance: ${distances[nodeNo]}`.
...
يجب أن يتضمن نص القالب أيضًا `${path}`.
