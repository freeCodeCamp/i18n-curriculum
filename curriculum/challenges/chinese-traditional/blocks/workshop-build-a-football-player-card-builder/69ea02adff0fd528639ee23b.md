...
接著，創建一個 `loadPlayer` 函式（程式），其 `PlayerData` 為返回型別。在裡面，加入一個 `try...catch` 語法，並以 `error` 作為 `catch` 區塊的參數。其他部分暫時保持空白。

# --hints--

你應該創建一個名為 `loadPlayer` 的函式（程式）。
...
你的 `loadPlayer` 函式（程式）應該有 `PlayerData` 的返回型別。
...
你的 `loadPlayer` 函式（程式）應該包含一個 `try...catch` 區塊。
