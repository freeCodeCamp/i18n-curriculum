...
Dopo il tuo loop annidato `for`, hai trovato il valore più piccolo. Devi scambiarlo con il valore corrente.

Come hai fatto nel tuo ordinamento a bolle, usa una variabile `temp` per estrarre il valore a `array[i]`, poi scambia i valori a `array[i]` e `array[minIndex]`.

# --hints--

Dopo il tuo loop annidato `for`, dovresti dichiarare una variabile `temp`.
...
Dovresti assegnare `array[i]` a `temp`.
...
Dovresti assegnare `array[minIndex]` a `array[i]`.
...
Dovresti assegnare `temp` a `array[minIndex]`.
