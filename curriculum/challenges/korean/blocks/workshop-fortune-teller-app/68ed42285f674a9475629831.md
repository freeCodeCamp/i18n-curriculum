...
클래스 내부에 객체로서 private `elements` 속성을 추가하세요. 그 안에 `singleCardBtn`, `singleCard`, `multipleCardsBtn` 속성을 각각 `HTMLElement` 타입으로 추가하세요.

# --hints--

`private` 클래스 내부에 `elements`라는 이름의 `Game` 속성을 선언해야 합니다.
...
`elements` private 속성은 `singleCardBtn` 속성을 가지고 있어야 하며, 이 속성은 `HTMLElement` 타입이어야 합니다.
...
`elements` private 속성은 `multipleCardsBtn` 속성을 가지고 있어야 하며, 이 속성은 `HTMLElement` 타입이어야 합니다.
...
`elements` private 속성은 `singleCard` 속성을 가지고 있어야 하며, 이 속성은 `HTMLElement` 타입이어야 합니다.
