...
在 `.image-wrapper` 元素中，给第一个 `img` 元素添加一个值为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png` 的 `src`、一个值为 `image of the quote machine project` 的 `alt`、一个值为 `image-1` 的 `class`、一个值为 `lazy` 的 `loading` 属性、一个值为 `600` 的 `width` 属性和一个值为 `400` 的 `height` 属性。

# --hints--

你的第一个 `img` 元素应该有一个 `src` 属性，值为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`。
