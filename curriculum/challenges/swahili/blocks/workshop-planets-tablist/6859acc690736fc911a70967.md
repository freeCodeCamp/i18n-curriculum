...
Kwa sasa, ukweli wote tatu unaonekana kwa wakati mmoja. Lakini ukweli mmoja tu ndilo unapaswa kuonekana kwa wakati mmoja.

Ili kufanikisha hili, unahitaji kuweka sifa ya kipengele `hidden` kwenye vipengele vya `tabpanel` vya pili na vya tatu.

# --hints--

Kipengele chako cha `tabpanel` cha pili kinapaswa kuwa na sifa ya kipengele `hidden`.

```js
assert.isTrue(document.querySelectorAll('div[role="tabpanel"]')[1].hasAttribute("hidden"));
```

Kipengele chako cha `tabpanel` cha tatu kinapaswa kuwa na sifa ya kipengele `hidden`.
