...
Debe haber un elemento `div` dentro del elemento `.cat-left-eye`.

```js
assert(document.querySelectorAll('.cat-left-eye div').length === 1);
```

Debe haber un elemento `div` dentro del elemento `.cat-right-eye`.

```js
assert(document.querySelectorAll('.cat-right-eye div').length === 1);
```

El nuevo elemento `div` dentro de `.cat-left-eye` debe tener la clase `cat-left-inner-eye`.

```js
assert(document.querySelectorAll('.cat-left-eye div')[0]?.classList.contains('cat-left-inner-eye'));
```

El nuevo elemento `div` dentro de `.cat-right-eye` debe tener la clase `cat-right-inner-eye`.
