...
在先前的步驟中，你學會了如何使用 `sort()` 方法將歌曲依字母順序排序。回呼函式的另一個使用案例是將陣列隨機化。 

將 `Math.random()` 減去 `0.5` 是隨機化一個陣列項目的一種方法，這會產生正值或負值的隨機數。這使比較結果呈現正負值混合，導致元素的隨機排序。
