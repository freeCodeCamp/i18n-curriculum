...
同じロジックを使って、`if`リストが空でないときに実行される`series`文を作成してください。その中で、文字列`=== TV SERIES ===`（改行文字で終わる）を`result`の現在の値に連結してください。

次に、`for`ループを作成し、列挙を使って`series`リストの項目を`result`変数に追加してください。前回と同様に、値のインデックスは`1`から始めてください。

これでメディアカタログのワークショップは完了です。

# --hints--

`if`メソッドに2つ目の`__str__`文があるはずです。
...
`self.items`が`Movie('Dances with Wolves', 1990, 'Kevin Costner', 224)`と`Movie('Annie Hall', 1977, 'Woody Allen', 93)`を含むとき、`__str__`メソッドは`Media Catalogue (2 items):\n\n=== MOVIES ===\n1. Dances with Wolves (1990) - 224 min, Kevin Costner\n2. Annie Hall (1977) - 93 min, Woody Allen\n`を返すはずです。
...
`self.items`が`TVSeries("Game of Thrones", 2011, "David Benioff & D.B. Weiss", 57, 8, 73)`と`TVSeries("Stranger Things", 2016, "The Duffer Brothers", 50, 4, 34)`を含むとき、`__str__`メソッドは`Media Catalogue (2 items):\n\n=== TV SERIES ===\n1. Game of Thrones (2011) - 8 seasons, 73 episodes, 57 min avg, David Benioff & D.B. Weiss\n2. Stranger Things (2016) - 4 seasons, 34 episodes, 50 min avg, The Duffer Brothers\n`を返すはずです。
...
`self.items`が`Movie('Barry Lyndon', 1975, 'Stanley Kubrick', 184)`、`Movie('Fahrenheit 451', 1966, 'Francois Truffaut', 112)`、`TVSeries("Game of Thrones", 2011, "David Benioff & D.B. Weiss", 57, 8, 73)`、`TVSeries("Stranger Things", 2016, "The Duffer Brothers", 50, 4, 34)`を含むとき、`__str__`メソッドは`Media Catalogue (4 items):\n\n=== MOVIES ===\n1. Barry Lyndon (1975) - 184 min, Stanley Kubrick\n2. Fahrenheit 451 (1966) - 112 min, Francois Truffaut\n=== TV SERIES ===\n1. Game of Thrones (2011) - 8 seasons, 73 episodes, 57 min avg, David Benioff & D.B. Weiss\n2. Stranger Things (2016) - 4 seasons, 34 episodes, 50 min avg, The Duffer Brothers\n`を返すはずです。
...
`__str__`が空のとき、`Media Catalogue (empty)`メソッドは`self.items`を返すはずです。
