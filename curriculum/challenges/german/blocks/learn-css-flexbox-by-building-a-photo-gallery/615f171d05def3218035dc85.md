...
Entfernen Sie den Außenabstand von Ihrem body-Element, setzen Sie das `font-family` auf `sans-serif` und geben Sie ihm eine `background-color` mit dem Wert `#f5f6f7`.
...
Ihr `body`-Selektor sollte eine `margin`-Eigenschaft mit dem Wert 0 haben.
...
Ihr `body`-Selektor sollte eine `font-family`-Eigenschaft mit dem Wert `sans-serif` haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily === 'sans-serif');
```

Ihr `body`-Selektor sollte eine `background-color`-Eigenschaft mit dem Wert `#f5f6f7` haben.
