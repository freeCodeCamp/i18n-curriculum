...
你還需要確認輸入是有效的。宣告一個 `invalidInputMatch` 變數，並將呼叫你的 `isInvalidInput` 函式，並以 `currVal` 作為引數的結果指定給它。

# --hints--

你應該宣告一個 `invalidInputMatch` 變數。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=/);
```

你應該將呼叫 `isInvalidInput` 的結果指定給你的 `invalidInputMatch` 變數。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(/);
```

你應該將 `currVal` 作為引數傳遞給 `isInvalidInput`。
