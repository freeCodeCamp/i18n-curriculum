...
Para a condição da declaração `if`, verifique se o `target` não é uma instância de `HTMLElement`. Então, dentro dela, adicione um `return`.

# --hints--

Você deve verificar na sua declaração `if` se a condição `!(target instanceof  HTMLElement)` está presente.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

Você deve ter um `return` dentro da sua declaração `if`.
