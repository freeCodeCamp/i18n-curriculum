...
La funzione `all` restituisce `True` se tutti gli elementi dell'iterabile passato sono truthy, altrimenti restituisce `False`:
...
Passa la lista `[isinstance(i, str) for i in medications]` alla funzione `all` per assicurarti che ogni elemento sia una stringa.

# --hints--

Dovresti modificare il valore della tua chiave `medications` passando `[isinstance(i, str) for i in medications]` alla funzione `all`.
