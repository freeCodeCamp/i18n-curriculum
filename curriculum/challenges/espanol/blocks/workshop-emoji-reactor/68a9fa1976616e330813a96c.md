...
Ahora haz un poco de espacio, elimina todas las variables de botón y el `eventListener` que has creado para cada uno.

# --hints--

La variable `happyBtn` no debería existir.
...
La variable `confusedBtn` no debería existir.
...
La variable `sadBtn` no debería existir.
...
La variable `lovingBtn` no debería existir.
