...
Innerhalb des Kontakt-Abschnitts möchten Sie die Kontaktinformationen des Blogautors anzeigen. Dafür verwenden Sie ein <dfn>address</dfn>-Element.

Das `address`-Element wird verwendet, um Kontaktinformationen für eine Person oder Organisation darzustellen.

Hier ist ein Beispiel, das das `address`-Element für eine physische Adresse verwendet. Das `br`-Element wird hier genutzt, um einen Zeilenumbruch zwischen dem Text zu erzeugen.
...
Unter Ihrem `h2`-Element fügen Sie ein `address`-Element hinzu.

# --hints--

Ihr Code sollte ein öffnendes `address`-Tag enthalten.

```js
assert.match(code, /<address>/i);
```

Ihr Code sollte ein schließendes `address`-Tag enthalten.

```js
assert.match(code, /<\/address>/i);
```

Ihr `address`-Element sollte sich innerhalb Ihres `section`-Elements befinden.
