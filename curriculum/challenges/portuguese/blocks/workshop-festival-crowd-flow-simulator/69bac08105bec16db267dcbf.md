...
Em seguida, registre a seguinte mensagem no console: `gate.queue[tickIndex] + " attendees arriving."`.

# --hints--

Sua função `handleGateAtTick` deve registrar a seguinte mensagem no console: `gate.queue[tickIndex] + " attendees arriving."`.
...
Sua função `handleGateAtTick` deve registrar o número correto de participantes com base na fila do portão no tickIndex fornecido.
