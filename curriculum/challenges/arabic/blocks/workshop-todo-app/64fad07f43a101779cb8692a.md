...
استخدم صيغة السهم لإنشاء دالة `addOrUpdateTask`. ثم انقل متغير `dataArrIndex`، وكائن `taskObj`، وعبارة `if` إلى داخل دالة `addOrUpdateTask`.
...
يجب أن تنقل متغير `dataArrIndex` إلى داخل دالة `addOrUpdateTask`.
...
يجب أن تنقل كائن `taskObj` إلى داخل دالة `addOrUpdateTask`.
