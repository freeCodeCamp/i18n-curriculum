...
Weka rangi ya nyuma ya `footer` kuwa `#2a2a40`, na tumia _Flexbox_ kuweka maandishi katikati kwa usawa.

# --hints--

Unapaswa kutumia kichaguzi cha kipengele cha `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

Unapaswa kumpa `footer` `background-color` ya `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

Unapaswa kumpa `footer` `display` ya `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Unapaswa kumpa `footer` `justify-content` ya `center`.
