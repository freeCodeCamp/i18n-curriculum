...
title: "Usa le funzioni di ordine superiore map, filter o reduce per risolvere un problema complesso"
challengeType: 1
forumTopicId: 301311
...
Ora che hai affrontato alcune sfide usando funzioni di ordine superiore come `map()`, `filter()` e `reduce()`, puoi applicarle per risolvere una sfida più complessa.

# --instructions--

Completa il codice della funzione `squareList` usando qualsiasi combinazione di `map()`, `filter()` e `reduce()`. La funzione deve restituire un nuovo array contenente i quadrati *solo* dei numeri interi positivi (i numeri decimali non sono numeri interi) quando le viene passato un array di numeri reali. Un esempio di array di numeri reali è `[-3, 4.8, 5, 3, -3.2]`.

**Nota:** La tua funzione non deve usare alcun tipo di loop `for` o `while` né la funzione `forEach()`.
...
Dovresti usare `map`, `filter` o `reduce`.
