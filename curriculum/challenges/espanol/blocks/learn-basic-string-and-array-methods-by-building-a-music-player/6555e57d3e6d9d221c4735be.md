...
Cuando se presiona el botón de mezcla, debes establecer `currentSong` en nada y `songCurrentTime` en `0`.

Establece `userData.currentSong` en `null` y `userData.songCurrentTime` en `0`.

*Nota*: No debes usar el encadenamiento opcional para este paso porque estás estableciendo explícitamente las propiedades `currentSong` y `songCurrentTime` en `null` y `0` respectivamente.

# --hints--

Debes establecer `null` en `userData.currentSong`.

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

Debes establecer `0` en `userData.songCurrentTime`.
