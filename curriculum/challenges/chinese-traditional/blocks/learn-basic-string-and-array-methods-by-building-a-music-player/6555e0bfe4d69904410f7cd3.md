...
使用 `const` 和箭頭語法來創建一個名為 `shuffle` 的空函式（程式）。

此函式（程式）負責將播放列表中的歌曲隨機排序，並在隨機排序後執行必要的狀態管理更新。

# --hints--

你應該使用 `const` 來創建一個名為 `shuffle` 的空函式（程式）。
...
你的 `shuffle` 函式應該使用箭頭語法，且不應接受任何參數。

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `shuffle` 函式（程式）應該是空的。
