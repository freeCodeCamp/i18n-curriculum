...
`html` セレクターを作成し、`font-size` プロパティに `62.5%` を設定してください。これにより、ウェブページのデフォルトのフォントサイズが 10px に設定されます（ブラウザーのデフォルトは 16px です）。

これにより、後で `rem` 単位を扱いやすくなります。なぜなら `2rem` は 20px になるからです。
...
`html` セレクターには `font-size` プロパティを `62.5%` に設定してください。
