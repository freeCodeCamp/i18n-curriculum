...
最後一個 `fieldset` 元素的邊框看起來有點不協調。你可以使用 `last-of-type` CSS 偽類別來選取特定類型的最後一個元素，如下所示： 

```css
p:last-of-type { }
```

那將會選取最後一個 `p` 元素。創建一個新的選擇器，標的為最後一個 `fieldset` 元素，並將其 `border-bottom` 設為 `none`。

# --hints--

你可以使用 `:last-of-type` 偽類別來選取某種類型的最後一個元素。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type'));
```

你的 `fieldset:last-of-type` 應該將 `border-bottom` 設為 `none`。
