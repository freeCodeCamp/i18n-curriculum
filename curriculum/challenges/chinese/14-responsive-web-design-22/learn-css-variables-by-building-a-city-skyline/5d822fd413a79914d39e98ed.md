...
Move the position of `.fb4` relative to where it is now by adding a `position` of `relative` and `left` of `10%` to it. Do the same for `.fb5` but use `right` instead of `left`. 这将覆盖建筑物之间剩余的空白区域。

# --hints--

你应该将 `.fb4` 的 `position` 设置为 `relative`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.position, 'relative');
```

你应该将 `.fb4` 的 `left` 设置为 `10%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.left, '10%');
```

你应该将 `.fb5` 的 `position` 设置为 `relative`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb5')?.position, 'relative');
```

你应该将 `.fb5` 的 `right` 设置为 `10%`。
