...
أخيرًا، يجب أن ترتب شفرتك قليلاً عبر إعادة الهيكلة! قد تلاحظ أن شفرتك فعلت زر الاختيار الأخير ("None of the Above") عدة مرات داخل دوال `getHighestDuplicates` و `detectFullHouse` و `checkForStraights`. أعد هيكلة شفرتك بحيث لا يتم تفعيل زر الاختيار الأخير في نهاية كل دالة، بل يتم ذلك مرة واحدة فقط داخل `rollDiceBtn`.

# --hints--

يجب ألا تفعل دالة `getHighestDuplicates` زر الاختيار النهائي، أو تضبط نص العنصر `span` النهائي، سواء تم رمي أزواج مكررة أم لا.
...
يجب ألا تفعل دالة `detectFullHouse` زر الاختيار النهائي، أو تضبط نص العنصر `span` النهائي، سواء تم رمي Full House أم لا.
...
يجب ألا تفعل دالة `checkForStraights` زر الاختيار النهائي، أو تضبط نص العنصر `span` النهائي، سواء تم رمي تسلسل مستقيم أم لا.
...
يجب تفعيل زر الاختيار النهائي، وضبط القيمة إلى `0`، وتحديث النص المعروض إلى `, score = 0` عند نقر `rollDiceBtn`.
