...
Wenn Sie genau hinschauen, sehen Sie, dass der aktuell abgespielte Song in der Wiedergabeliste nicht hervorgehoben ist, sodass Sie nicht wirklich wissen, welcher Song gerade spielt. Sie können das beheben, indem Sie eine Funktion erstellen, die jeden Song hervorhebt, der gerade abgespielt wird.

Erstellen Sie eine Funktion mit dem Namen `highlightCurrentSong`. Überprüfen Sie innerhalb der Funktion, ob ein `.playlist-song`-Element mit `aria-current="true"` existiert. Falls ja, verwenden Sie die `removeAttribute`-Methode, um das `aria-current`-Attribut von diesem zu entfernen.

# --hints--

Sie sollten eine Funktion mit dem Namen `highlightCurrentSong` haben.

```js
assert.isFunction(highlightCurrentSong)
```

Ihre `highlightCurrentSong`-Funktion sollte die `removeAttribute`-Methode verwenden, um das `aria-current`-Attribut von dem `.playlist-song`-Element mit `aria-current="true"` zu entfernen, falls dieses Element existiert.
