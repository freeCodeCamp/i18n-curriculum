...
你需要实现播放下一首歌和上一首歌。 为此，你将需要一个 `playNextSong` 和 `playPreviousSong` 函数。

使用 `const` 和箭头语法创建一个空的 `playNextSong` 函数。

# --hints--

你应该使用 `const` 创建一个名为 `playNextSong` 的空函数。

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` 应该是一个函数。

```js
assert.isFunction(playNextSong)
```

你的 `playNextSong` 函数应该使用箭头语法。

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `playNextSong` 函数应该是空的。
