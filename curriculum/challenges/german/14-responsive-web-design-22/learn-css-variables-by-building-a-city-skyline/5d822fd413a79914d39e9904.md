...
Fülle `.bb3` mit einem `repeating-linear-gradient` aus. Verwende `90deg` für die Richtung, deine `building-color3` für die ersten beiden Farben und `window-color3` bei `15%` für die dritte. Wenn du für eine Farbe keinen Abstand angibst, werden die sinnvollen Werte verwendet. In diesem Fall sind die ersten beiden Farben standardmäßig `0%` und `7.5%`, da sie bei `0%` beginnen und `7.5%` die Hälfte von `15%` ist.

# --hints--

Du solltest `.bb3` mit `repeating-linear-gradient` einen `background` geben.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb3")?.background, "repeating-linear-gradient");
```

Du solltest `90deg` für die Richtung im ersten Argument von `repeating-linear-gradient` verwenden.
