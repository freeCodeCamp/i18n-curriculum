...
與 `^` 錨點一樣，你可以使用 `$` 錨點來匹配字符串的末尾。

更新正則表達式以匹配字符串的末尾或空格，就像對字符串的開頭所做的那樣。
...
你的 `freeRegex` 應該與 `it's free money time` 匹配。

```js
assert.match("it's free money time", freeRegex);
```

你的 `freeRegex` 應該與 `free money time` 匹配。

```js
assert.match('free money time', freeRegex);
```

你的 `freeRegex` 應該與 `it's free money` 匹配。
...
你的 `freeRegex` 不應匹配 `hands-free money time`。

```js
assert.notMatch('hands-free money', freeRegex);
```

你的 `freeRegex` 不應匹配 `free money-management`。
