...
スコアとラウンド結果のメッセージを更新する時間です。

`showResults` という名前の関数を、`userOption` というパラメータ付きで作成してください。

`showResults` 関数の中で、ラウンドの結果で `roundResultsMsg` を更新します。

次に、プレイヤーとコンピューターの更新されたスコアを表示するために、`playerScoreSpanElement` と `computerScoreSpanElement` も更新してください。

ここで順序が重要です。まず `roundResultsMsg` を更新し、次に `playerScoreSpanElement`、最後に `computerScoreSpanElement` を更新する必要があります。なぜなら `roundResultsMsg` がスコアを決定するために使われるからです。

# --hints--

`showResults` という関数があるはずです。

```js
assert.isFunction(showResults);
```

`showResults` 関数は `userOption` というパラメータを持つべきです。

```js
assert.match(code, /showResults\s*(?:=\s*)?(?:\(\s*)?userOption(?:\s*\))?\s*/);
```

`showResults` 関数はラウンドの結果で `roundResultsMsg` を更新するべきです。
...
`showResults` 関数はコンピューターの更新されたスコアを表示するために `computerScoreSpanElement` を更新するべきです。
...
`showResults` 関数はプレイヤーの更新されたスコアを表示するために `playerScoreSpanElement` を更新するべきです。
