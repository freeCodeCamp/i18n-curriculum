...
对于这个算法，你会想要使用 `while` 循环。 这个循环需要两个条件：

- 首先，程序不应运行超出数组的开头（通过 `j` 访问）。
- 其次，当找到比当前值小的值时，循环不应该继续运行。

为了防止无限循环，请在你的循环中递减 `j`。
...
你的 `while` 循环的第一个条件应该检查 `j` 的值是否大于或等于 `0`。
...
你的 `while` 循环应该有一个条件来检查 `array[j]` 的值是否大于 `currValue`。
