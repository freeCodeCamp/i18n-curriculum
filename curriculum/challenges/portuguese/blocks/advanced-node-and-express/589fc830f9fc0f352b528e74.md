...
`socket.io@~2.3.0` já foi adicionado como uma dependência, então importe/instancie-o no seu servidor da seguinte forma com `http` (vem embutido com Node.js):
...
Agora que o servidor *http* está montado no *express app*, você precisa escutar a partir do servidor *http*. Altere a linha com `app.listen` para `http.listen`.
...
**Nota:**`io()` funciona apenas ao conectar-se a um socket hospedado na mesma url/servidor. Para se conectar a um socket externo hospedado em outro lugar, você usaria `io.connect('URL');`.

Envie sua página quando você achar que ela está certa. Se você estiver enfrentando erros, pode <a href="https://forum.freecodecamp.org/t/advanced-node-and-express/567135#set-up-the-environment-6" target="_blank" rel="noopener noreferrer nofollow">verificar o projeto concluído até este ponto</a>.
