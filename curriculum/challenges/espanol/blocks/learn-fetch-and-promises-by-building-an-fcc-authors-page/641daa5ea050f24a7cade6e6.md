...
Si se cumple esta condición, deshabilite el botón estableciendo su propiedad `disabled` a `true`. Además, establezca el `textContent` del botón a `"No more data to load"`.
...
Debería establecer la propiedad `disabled` de `loadMoreBtn` a `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Debería establecer el `textContent` de `loadMoreBtn` a `"No more data to load"`.
