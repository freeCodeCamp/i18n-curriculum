...
为确保表格填充其容器，将 `width` 设置为 `100%` 。然后将其相对定位并为其提供 `3rem` 的上边距。

# --hints--

你的 `table` 选择器应该有一个 `width` 属性，设置为 `100%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('width') === '100%');
```

你的 `table` 选择器应该有一个 `position` 属性，设置为 `relative`。

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('position') === 'relative');
```

你的 `table` 选择器应该有一个 `margin-top` 属性，设置为 `3rem`。
