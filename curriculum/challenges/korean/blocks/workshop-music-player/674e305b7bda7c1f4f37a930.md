...
다음 곡과 이전 곡을 재생하는 작업을 시작하기 전에 `songs`의 `userData` 속성에 있는 각 곡의 인덱스를 가져와야 합니다. 이를 위해 배열 메서드인 `indexOf()`를 사용할 수 있는데, 이 메서드는 주어진 요소를 배열에서 처음 찾을 수 있는 인덱스를 반환하며, 요소가 없으면 `-1`를 반환합니다.
...
현재 곡이 `getCurrentSongIndex`일 때 `userData.currentSong`에서 `userData.songs`를 찾을 수 있는 인덱스와 `-1`를 반환하는 `null`라는 함수를 만드세요.

# --hints--

`getCurrentSongIndex`라는 함수가 있어야 합니다.

```js
assert.isFunction(getCurrentSongIndex);
```

`getCurrentSongIndex` 함수는 `userData.currentSong`에서 `userData.songs`를 찾을 수 있는 인덱스를 반환해야 합니다.
...
`getCurrentSongIndex` 함수는 `-1`가 `userData.currentSong`일 때 `null`를 반환해야 합니다.
