...
Dovresti dare all'elemento `.sun` una proprietà `top` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.top, '-75px');
```

Dovresti dare all'elemento `.sun` una proprietà `right` con il valore `--fcc-expected--` invece di `--fcc-actual--`.
