...
Debajo de `valor` y `checked` destrucurados, actualice la lista de **powers** marcados.

Llame a la función `setPowers`, que toma un argumento de arreglo. Use un operador ternario para comprobar si `checked` es verdadero. Si lo es, esparza el arreglo de `powers` existente en uno nuevo y agregue el `valor` a este.  If it is not true, filter out the `value` from `powers` with `powers.filter(p => p !== value)`.

# --hints--

Debería usar un operador ternario para comprobar si `checked` es verdadero. Si lo es, esparza los `powers` existentes y `valor` en un arreglo. Si no lo es, filtre el `valor` de `powers`.
