...
Per l’ultima parte del gioco, dovrai creare un algoritmo che verifichi la presenza di una scala. Una piccola scala si ha quando quattro dadi hanno valori consecutivi in qualsiasi ordine (es. in un lancio di `41423`, abbiamo `1234`) con un punteggio di `30` punti. Una grande scala si ha quando tutti e cinque i dadi hanno valori consecutivi in qualsiasi ordine (es. in un lancio di `35124`, abbiamo `12345`) con un punteggio di `40` punti.

Dichiara una funzione `checkForStraights` che accetta un array di numeri. Se l’utente ottiene una grande scala, aggiorna il quinto pulsante di opzione con un punteggio di `40`. Se l’utente ottiene una piccola scala, aggiorna il quarto pulsante di opzione con un punteggio di `30`. In ogni caso, dovrebbe sempre aggiornare l’ultimo pulsante di opzione per mostrare un punteggio di 0, con gli attributi corretti.

Chiama la tua funzione `checkForStraights` quando si clicca su `rollDiceBtn` per completare il tuo gioco dei dadi!

# --hints--

La tua variabile `checkForStraights` dovrebbe essere una funzione.

```js
assert.isFunction(checkForStraights);
```

Se esce una piccola scala, la tua funzione `checkForStraights` dovrebbe abilitare il quarto pulsante di opzione, impostare il valore a `30`, aggiornare il testo mostrato a `, score = 30` e lasciare il quinto pulsante di opzione disabilitato.
...
Se esce una grande scala, la tua funzione `checkForStraights` dovrebbe abilitare il quarto pulsante, impostare il valore a `30`, aggiornare il testo mostrato a `, score = 30`. Inoltre, la funzione dovrebbe abilitare il quinto pulsante di opzione, impostare il valore a `40` e aggiornare il testo mostrato a `, score = 40`.
...
Se non esce nessuna scala, la tua funzione `checkForStraights` non dovrebbe abilitare né il quarto né il quinto pulsante di opzione.
...
Se non esce nessuna scala, la tua funzione `checkForStraights` dovrebbe abilitare l’ultimo pulsante di opzione, impostare il valore a `0` e aggiornare il testo mostrato a `, score = 0`.
...
Dovresti chiamare la funzione `checkForStraights` quando si clicca su `rollDiceBtn`.
