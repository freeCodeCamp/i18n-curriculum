...
Per il secondo post del blog, dovrai aggiungere un altro elemento `article`.

All'interno dell'elemento `article`, aggiungi un elemento `h3` con il testo `Mr. Whiskers' First Bath`.

Sotto il tuo elemento `h3`, aggiungi due paragrafi di testo lorem ipsum.

# --hints--

Dovresti avere un secondo elemento `article` all'interno del tuo elemento `section` con la proprietà `id` impostata su `posts`.
...
Dovresti avere un elemento `h3` dentro l'elemento `article` con il testo `Mr. Whiskers' First Bath`.
