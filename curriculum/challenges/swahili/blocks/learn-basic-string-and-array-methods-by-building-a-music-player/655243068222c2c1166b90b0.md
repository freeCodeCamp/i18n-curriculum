...
Ndani ya kitendakazi cha `playNextSong`, tengeneza usemi wa `if` kuangalia kama `currentSong` ya `userData` ni sawa kabisa na `null`. Hii itakagua kama hakuna wimbo unaochezwa kwa sasa katika kitu cha `userData`. 

Kama hali ni kweli, fanya wito wa kitendakazi cha `playSong` ukiwa na `id` wa wimbo wa kwanza katika safu ya data ya `userData?.songs` kama hoja.

# --hints--

Haupaswi kubadilisha kitendakazi cha `playNextSong` kilichopo na yaliyomo ndani yake.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Unapaswa kutengeneza usemi wa `if` wenye hali ya `userData?.currentSong === null`.
...
Unapaswa kufanya wito wa kitendakazi cha `playSong` ukiwa na `userData?.songs[0].id` ndani ya usemi wa `if`.
