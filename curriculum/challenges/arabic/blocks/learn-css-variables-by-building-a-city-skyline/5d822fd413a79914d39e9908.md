...
أزل خاصية وقيمة `background-color` من `.bb4`، وأضفها إلى الأقسام الثلاثة الجديدة `.bb4a` و`.bb4b` و`.bb4c`، بحيث تملأ الأقسام فقط.

# --hints--

يجب إزالة `background-color` من `.bb4`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb4")?.backgroundColor);
```

يجب إعطاء `.bb4a` خاصية `background-color` بقيمة `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.backgroundColor.trim(), "var(--building-color4)");
```

يجب إعطاء `.bb4b` خاصية `background-color` بقيمة `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.backgroundColor.trim(), "var(--building-color4)");
```

يجب إعطاء `.bb4c` خاصية `background-color` بقيمة `--building-color4`.
