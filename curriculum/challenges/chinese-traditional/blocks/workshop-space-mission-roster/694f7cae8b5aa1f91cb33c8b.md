...
如同之前，你應該驗證你的輸入。具體來說，如果 `size` 小於 `1`，你應該將 `"Chunk size must be >= 1"` 記錄到主控台，然後呼叫 `return` 以離開函式。

# --hints--

如果輸入的 `size` 無效，你應該將以下錯誤訊息記錄到主控台，`"Chunk size must be >= 1"`。
...
在記錄錯誤訊息後，你應該立即呼叫 `return` 以提前離開函式。
...
當 `size` 大於或相等於 1 時，你的 `chunkCrew` 函式應該通過驗證且不會有錯誤。
