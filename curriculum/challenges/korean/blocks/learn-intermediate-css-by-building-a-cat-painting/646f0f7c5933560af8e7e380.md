...
`.cat-whiskers-left` 요소 안에 `div`, `cat-whisker-left-top`, `cat-whisker-left-middle` 클래스를 가진 `cat-whisker-left-bottom` 요소 세 개를 만드세요.

# --hints--

기존 `div` 요소와 `cat-whiskers-left` 클래스는 변경하지 마세요.

```js
assert(document.querySelectorAll('div.cat-whiskers-left').length === 1)
```

`div` 요소 안에 `.cat-whiskers-left` 요소 세 개를 만들어야 합니다.

```js
assert(document.querySelectorAll('.cat-whiskers-left div').length === 3)
```

`div` 요소 안의 첫 번째 `.cat-whiskers-left` 요소는 `cat-whisker-left-top` 클래스를 가져야 합니다.

```js
assert(document.querySelectorAll('.cat-whiskers-left div')[0].classList.contains('cat-whisker-left-top'))
```

`div` 요소 안의 두 번째 `.cat-whiskers-left` 요소는 `cat-whisker-left-middle` 클래스를 가져야 합니다.

```js
assert(document.querySelectorAll('.cat-whiskers-left div')[1].classList.contains('cat-whisker-left-middle'))
```

`div` 요소 안의 세 번째 `.cat-whiskers-left` 요소는 `cat-whisker-left-bottom` 클래스를 가져야 합니다.
