...
`href` 속성 뒤에 `class`라는 상수로 `linkClass` 속성을 설정하세요.

`class` 속성 뒤에 `target` 속성을 `"_blank"`로 설정하세요.

마지막으로, 앵커 태그 사이에 `linkText` 상수를 넣어 링크에 텍스트가 표시되게 하세요.

# --hints--

`class` 속성 뒤에 `href` 속성이 있어야 합니다.
...
`class` 속성의 값을 `linkClass`라는 상수로 설정해야 합니다.
...
`target` 속성 뒤에 `class` 속성이 있어야 합니다.
...
`target` 속성의 값을 `"_blank"`로 설정해야 합니다.
...
앵커 태그 사이에 `linkText` 상수를 넣어야 합니다.
