...
你可以使用字型大小為標題、副標題、段落和其他元素創建視覺階層體系。
...
例如，網頁上的主標題應該使用較大的字型，接著是使用較小字型大小的副標題。這將為階層體系中的每個元素賦予特定的字型大小，幫助使用者透過視覺瀏覽結構。
...
一旦你選擇這兩種或三種字型，就在不同頁面和元素中持續使用它們。這些字型應該與你的網頁應用程式、產品或服務的整體設計保持一致。
...
你也應該提供選項，可調整字型大小並盡可能避免使用複雜字型。一般規則是，應盡可能使用簡單字型。如果你的內容易於閱讀，使用者更有可能與之互動。
...
關於排版的最後一個建議是，確保你的字型能在不同螢幕大小和裝置上正確顯示。
