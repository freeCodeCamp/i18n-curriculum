...
Всередині методу `showFortune` оголосіть змінну з назвою `target` і призначте їй значення `e.target`, а також створіть порожню інструкцію `if`.

# --hints--

Ви повинні оголосити змінну з ім’ям `target` всередині `showFortune`.
...
Вашій змінній `target` має бути призначене значення `e.target`.
...
Після змінної `if` має бути порожня інструкція `target`.
