...
Conditional sentences often use `if` to describe a situation (cause) that will lead to a particular result (consequence). They typically consist of two parts: the `if` clause (condition) and the main clause (result). Por ejemplo:
