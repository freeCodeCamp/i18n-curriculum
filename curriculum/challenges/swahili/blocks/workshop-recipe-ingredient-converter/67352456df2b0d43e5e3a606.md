...
Toa `conversionTable` yako sifa ya `gram`, na weka thamani yake kuwa kitu chenye sifa ya `cup` iliyowekwa kuwa `1/240`, sifa ya `ounce` iliyowekwa kuwa `0.0353`, na sifa ya `teaspoon` iliyowekwa kuwa `0.2`.

# --hints--

`conversionTable` yako inapaswa kuwa na sifa ya `gram`.

```js
assert.property(conversionTable, "gram");
```

Sifa yako ya `gram` inapaswa kuwa kitu.

```js
assert.isObject(conversionTable.gram);
```

Kitu chako cha `gram` kinapaswa kuwa na sifa ya `cup`.

```js
assert.property(conversionTable.gram, "cup");
```

Sifa yako ya `cup` inapaswa kuwekwa kuwa `1/240`.

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

Kitu chako cha `gram` kinapaswa kuwa na sifa ya `ounce`.

```js
assert.property(conversionTable.gram, "ounce");
```

Sifa yako ya `ounce` inapaswa kuwekwa kuwa `0.0353`.

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

Kitu chako cha `gram` kinapaswa kuwa na sifa ya `teaspoon`.

```js
assert.property(conversionTable.gram, "teaspoon");
```

Sifa yako ya `teaspoon` inapaswa kuwekwa kuwa `0.2`.
