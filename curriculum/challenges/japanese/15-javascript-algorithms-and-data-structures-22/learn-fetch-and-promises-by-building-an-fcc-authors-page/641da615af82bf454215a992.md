...
`.forEach()` メソッドに、空のコールバック関数を渡してください。 コールバック関数の 1 番目の仮引数については、分割代入で `author`、`image`、`url`、`bio` の項目を取り出すようにしてください。

2 番目の仮引数については、`index` を渡してください。 このインデックスは、それぞれの著者が何番目かの位置を表し、後ほどページネーションに役立ちます。

# --hints--

`author`、`image`、`url`、`bio` をコールバック関数の最初のパラメーターとして分割してください。 これらを波括弧の中に入れるのを忘れないでください。
...
`index` をコールバック関数の 2 番目のパラメーターとして分割してください。 これは波括弧の外に配置する必要があります。

```js
assert.match(code, /authors\.forEach\(\s*\(\s*\{.*\}\s*,\s*index\s*\)/)
```

コールバック関数はアロー構文を使用する必要があります。
