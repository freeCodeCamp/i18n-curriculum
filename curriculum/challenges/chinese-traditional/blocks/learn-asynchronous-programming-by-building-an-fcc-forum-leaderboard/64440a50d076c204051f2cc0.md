...
你需要一個函式（程式）來將觀看次數轉換成更易讀的格式。例如，如果觀看次數是 `1000`，它應該顯示為 `1k`，如果觀看次數是 `100,000`，它應該顯示為 `100k`。

創建一個帶有 `views` 參數的 `viewCount` 函式（程式）。如果 `views` 大於或相等於 `1000`，傳回一個字串，該字串包含 `views` 除以 `1000` 的值，並在其後附加字母 `k`。請確保將 `views / 1000` 向下取整到最接近的整數。

否則，傳回 `views` 值。

例如，如果 `views` 是 `1000`，你的返回值應該是字串 `"1k"`。

# --hints--

你應該有一個名為 `viewCount` 的函式（程式）。

```js
assert.isFunction(viewCount);
```

你的 `viewCount` 函式應該有一個名為 `views` 的參數。

```js
assert.match(viewCount.toString(), /\(?views\)?/);
```

當 `views` 是 `1000` 時，你的 `viewCount` 函式（程式）應該傳回字串 `"1k"`。

```js
assert.strictEqual(viewCount(1000), "1k");
```

你的 `viewCount` 函式應該在 `views` 為 `100000` 時傳回字串 `"100k"`。

```js
assert.strictEqual(viewCount(100000), "100k");
```

當 `views` 大於或相等於 `1000` 時，你的 `viewCount` 函式應該傳回正確的字串。
...
你的 `viewCount` 函式應該在 `views` 為 `200` 時傳回 `views`。

```js
assert.strictEqual(viewCount(200), 200);
```

你的 `viewCount` 函式應該在 `views` 小於 `1000` 時傳回 `views`。
