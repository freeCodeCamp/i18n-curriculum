...
أنشئ عنصر `div` جديدًا أسفل عنصر `.medium` الخاص بك. امنحه خاصية `class` بقيمة `daily-value small-text`. داخل هذا العنصر الجديد `div`، أضف عنصر `p` يحتوي على النص `% Daily Value *`، واضبط خاصية `class` على `bold right`.
