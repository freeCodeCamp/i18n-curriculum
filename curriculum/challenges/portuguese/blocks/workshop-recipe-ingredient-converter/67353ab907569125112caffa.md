...
Agora é hora de usar suas funções. Declare uma nova função `processIngredient` que aceita quatro argumentos: `baseQuantity`, `baseUnit`, `newUnit` e `newServings`.

Sua função deve ajustar as porções para a quantidade base para as novas porções, converter essa quantidade ajustada da unidade base para a nova unidade e retornar a quantidade convertida como uma string fixa com duas casas decimais.

Se você ficar preso, consulte suas chamadas de função de exemplo que você registrou. Você pode alterar os valores em ambos esses exemplos para explorar.

# --hints--

Você deve declarar uma função `processIngredient`.

```js
assert.isFunction(processIngredient);
```

Sua função `processIngredient` deve ter `baseQuantity` como o primeiro parâmetro.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity/);
```

Sua função `processIngredient` deve ter `baseUnit` como o segundo parâmetro.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit/);
```

Sua função `processIngredient` deve ter `newUnit` como o terceiro parâmetro.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit/);
```

Sua função `processIngredient` deve ter `newServings` como o quarto parâmetro.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit,\s*newServings\)/);
```

Sua função `processIngredient` deve retornar uma string.

```js
assert.isString(processIngredient(10, "gram", "cup", 5));
```

Sua função `processIngredient` deve retornar o valor correto.
