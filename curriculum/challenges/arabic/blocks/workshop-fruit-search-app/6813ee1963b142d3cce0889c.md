...
داخل كتلة `try`، نفذ استدعاء الدالة `fetch` مع عنوان URL `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}`، واحفظ النتيجة في متغير اسمه `response`. بما أنك داخل دالة `async`، يجب استخدام `await` قبل `fetch`.

# --hints--

يجب استدعاء `fetch` مع `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` وحفظ النتيجة في متغير `response`.
