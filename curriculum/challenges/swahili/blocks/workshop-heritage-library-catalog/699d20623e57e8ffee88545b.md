...
Ni wakati wa kuthibitisha kuwa utafutaji unafanya kazi. Stephen King ana vitabu sita katika orodha.

Fanya wito `findByAuthor(catalog, "king")` na weka thamani yake katika kigezo kinachoitwa `kingBooks`. Andika `kingBooks.length` ili kuthibitisha idadi.

Kisha tumia mzunguko wa `for` kuandika kila matokeo. Tumia template literal kuonyesha kichwa na mwaka, kama `` `${kingBooks[i].title} (${kingBooks[i].year})` ``.

# --hints--

`kingBooks` inapaswa kuwa na urefu wa `6`.

```js
assert.strictEqual(kingBooks.length, 6);
```

Unapaswa kuandika `kingBooks.length` kwenye konsoli.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*kingBooks\.length\s*\)/);
```

Unapaswa kutumia mzunguko wa `for` kuandika kichwa na mwaka wa kila kitabu.
