...
Adicione um atributo `value` com valor `player.club` ao Club `input`. Depois disso, adicione um atributo `onChange` definido como uma arrow function que recebe `e` como parâmetro e chama `setPlayer` com um spread de `player` e `club` definido como `e.target.value`.

# --hints--

Seu Club `input` deve ter um `value` igual ao clube do jogador.
...
Seu Club `input` deve ter um manipulador `onChange` que chama `setPlayer` com um spread de `player` e `club` definido como `e.target.value`.
