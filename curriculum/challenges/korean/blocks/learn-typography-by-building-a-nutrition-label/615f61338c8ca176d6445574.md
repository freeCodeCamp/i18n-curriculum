...
`.calories-info` 요소 아래에 `div`을 `class` 속성을 `divider medium`로 설정하여 추가하세요.

# --hints--

`div` 요소 안에 새로운 `.label`를 만드세요.

```js
assert(document.querySelectorAll('.label > div')?.length === 3)
```

새로운 `div`는 `class` 속성을 `divider medium`로 설정해야 합니다. 이 div는 `.label` 요소 안에서 마지막 요소여야 합니다.
