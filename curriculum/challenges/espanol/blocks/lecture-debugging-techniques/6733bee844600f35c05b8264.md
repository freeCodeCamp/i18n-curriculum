...
En la lección anterior, aprendiste cómo lanzar excepciones en tus programas. En esta lección, veremos cómo manejar estos errores de forma elegante en un bloque `try...catch...finally`.
...
El bloque `finally` se ejecuta después de los bloques `try` y `catch`, sin importar si ocurrió un error o no. Se usa comúnmente para tareas de limpieza, como cerrar archivos o liberar recursos.

Aquí tienes un ejemplo de cómo usar un bloque `try...catch`:
...
En este ejemplo, tenemos una función llamada `processInput` que primero verifica si la `input` no es del tipo `string`. Si ese es el caso, entonces lanzamos un error. De lo contrario, devolvemos el resultado de usar el método `toUpperCase` en la `input`.

Llamamos a la función dentro de un bloque `try`. Como la llamada a la función lanza un error, este será capturado dentro del bloque `catch` y el mensaje de error se mostrará en la consola.
...
La sentencia `finally` se ejecuta sin importar si se lanzó o no una excepción.
...
Es para ejecutar código automáticamente otra vez después de que ocurre un error.

### --feedback--

Piensa en el orden de los bloques en las sentencias `try...catch...finally` y en los tipos de tareas que maneja cada bloque.
...
Es para la configuración antes de que se lance un error.
...
Es un envoltorio seguro para código que podría lanzar un error.
