...
`row_str` の内包表記を修正して、アイテムがゼロでない場合は文字列を返し、そうでない場合はアスタリスク文字を返すようにしてください。

# --hints--

`row_str` 変数に代入されたリスト内包表記は、`str()` が真の場合に `i` の各アイテム `row` に対して `i` を呼び出し、そうでない場合は `'*'` を評価する必要があります。
