...
В межах `evalFormula` оголосіть стрілкову функцію `idToText`, яка приймає параметр `id`.

Функція `idToText` має повернути результат виклику `.find()` на масиві `cells` з функціеї зворотного виклику, яка приймає параметр `cell` та повертає `cell.id === id`.

Обидві ваші функції мають використати неявні повернення.
...
Використайте `const`, щоб оголосити змінну `idToText`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText/);
```

Змінна `idToText` має бути стрілковою функцією.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

Функція `idToText` повинна мати параметр `id`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>/);
```

Функція `idToText` має повернути результат виклику методу `.find()` на масиві `cells`. Функція зворотного виклику має використати неявне повернення.
...
Функція зворотного виклику має повернути, чи `cell.id` суворо дорівнює `id`.
