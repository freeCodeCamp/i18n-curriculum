...
Kadi zingine zote za katalogi zimeongezwa. Angalia maingizo matatu karibu na mwisho yenye sehemu zilizokosekana; utashughulikia hizo katika hatua inayofuata.

`parseCard` itachukua mfuatano wa herufi wa katalogi ghafi na kurudisha kitu kilicho na muundo chenye vigezo vinne: `title`, `author`, `year`, na `location`. Kwa sasa, tengeneza kitendakazi `parseCard` chenye kigezo kinachoitwa `rawString`.

# --hints--

Unapaswa kuwa na kitendakazi chenye jina `parseCard`.

```js
assert.isFunction(parseCard);
```

`parseCard` inapaswa kuwa kitendakazi chenye kigezo kinachoitwa `rawString`.
