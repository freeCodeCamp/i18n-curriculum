...
Um das vorherige Lied zu erhalten, subtrahieren Sie `1` von der `currentSongIndex` von `userData?.songs` und weisen Sie das Ergebnis der Konstanten `previousSong` zu. Rufen Sie danach die `playSong`-Funktion auf und übergeben Sie `previousSong.id` als Argument.
...
Sie sollten `userData?.songs[currentSongIndex - 1]` einer `previousSong`-Konstanten zuweisen.
...
Sie sollten die `playSong`-Funktion mit `previousSong.id` aufrufen.
