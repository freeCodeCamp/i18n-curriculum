...
残念ながら、その `children` プロパティは要素のコレクションを返しており、配列のようですが配列ではありません。2番目の引数を `Array.from()` でラップして配列に変換してください。

# --hints--

`document.getElementById('container').children` を `Array.from()` でラップする必要があります。
