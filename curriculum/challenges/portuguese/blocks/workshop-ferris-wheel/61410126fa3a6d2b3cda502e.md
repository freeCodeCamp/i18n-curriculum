...
Para fazer com que a animação de cabine pareça mais com um movimento natural de balanço, você pode usar a função de tempo de `ease-in-out`.Essa configuração diz para a animação iniciar e terminar em um ritmo mais lento, mas que se mova mais rapidamente no meio do ciclo.

No seletor `.cabin` substitua `linear` por `ease-in-out`.

# --hints--

O seletor `.cabin` deve ter a propriedade `animation` definida para `cabins 10s ease-in-out infinite`.
