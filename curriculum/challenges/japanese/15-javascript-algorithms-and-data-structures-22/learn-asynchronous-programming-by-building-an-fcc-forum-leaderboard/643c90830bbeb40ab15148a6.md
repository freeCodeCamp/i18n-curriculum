...
では、`postsContainer` の中にデータを表示していきましょう。

まず `topics` 配列に対して `map()` メソッドを呼び出します。 コールバック関数には、仮引数に `item` を取る空のアロー関数を使用してください。

次に、`map()` メソッドの結果を `postsContainer.innerHTML` に割り当ててください。

# --hints--

変数 `topics` の `map()` メソッドを使用してください。

```js
assert.match(code, /topics\.map\(.*/);
```

`map()` メソッドに、`item` をパラメーター名としてコールバック関数を追加してください。
...
`map()` メソッドの結果を `postsContainer.innerHTML` に割り当ててください。
