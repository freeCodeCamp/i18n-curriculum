...
Wiederholen Sie für den dritten und vierten Wegpunkt das `transform`-Muster noch einmal.

# --hints--

Sie sollten dem `30%`-Wegpunkt eine `transform` von `rotate(110deg) scaleX(-1)` geben.
...
Sie sollten dem `40%`-Wegpunkt eine `transform` von `rotate(130deg) scaleX(-1)` geben.
