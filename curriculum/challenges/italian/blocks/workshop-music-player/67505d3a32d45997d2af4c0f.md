...
Crea una funzione chiamata `playPreviousSong`. All'interno della tua nuova funzione, usa il valore restituito dalla funzione `getPreviousSong` per trovare la canzone da riprodurre. Quando `getPreviousSong()` è `undefined`, riproduci la prima canzone della tua playlist. Altrimenti, riproduci la canzone precedente a quella attualmente in riproduzione.

# --hints--

Dovresti avere una funzione chiamata `playPreviousSong`.

```js
assert.isFunction(playPreviousSong);
```

La funzione `playPreviousSong` dovrebbe riprodurre la canzone precedente a quella attualmente in riproduzione nella playlist.
...
Quando `getPreviousSong()` è `undefined`, la funzione `playPreviousSong` dovrebbe riprodurre la prima canzone nella playlist.
