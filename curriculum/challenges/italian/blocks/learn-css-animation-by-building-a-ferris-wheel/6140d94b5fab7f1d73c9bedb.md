...
Le regole `@keyframes` richiedono che venga assegnato loro un nome, da utilizzare come riferimento in altre regole. Ad esempio, la regola `@keyframes freeCodeCamp { }` ha il nome `freeCodeCamp`.
...
La nuova regola `@keyframes` dovrebbe avere il nome `wheel`.
