...
I tuoi tre elementi `link` dovrebbero avere un attributo `rel` con il valore `stylesheet`.
...
Uno dei tuoi elementi link dovrebbe avere il `href` impostato su `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap`.
...
Uno dei tuoi elementi link dovrebbe avere il `href` impostato su `https://use.fontawesome.com/releases/v5.8.2/css/all.css`.
...
Uno dei tuoi elementi `link` dovrebbe avere il `href` impostato su `styles.css`.
...
Il tuo elemento `title` dovrebbe essere all'interno del tuo elemento `head`.
...
Il tuo progetto dovrebbe avere un titolo `Magazine`. Ricorda che la maiuscola e la grafia sono importanti per il titolo.
