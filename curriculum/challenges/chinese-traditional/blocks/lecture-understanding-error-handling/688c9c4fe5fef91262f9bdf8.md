...
這裡關鍵字 `raise`（無引數）會重新引發正在處理的當前例外。
...
這裡你可以看到 `raise ... from None` 抑制了原始例外的上下脈絡：
...
而 `raise ... from e` 將新的例外串鏈到原始例外，保留錯誤軌跡：
