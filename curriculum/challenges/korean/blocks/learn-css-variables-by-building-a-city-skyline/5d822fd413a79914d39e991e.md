...
`.fb2-window` 요소에 `width`의 `22%`, `height`의 `100%`, 그리고 `background-color` 변수의 `--window-color3`를 주세요.

# --hints--

`.fb2-window` 요소에 `width`의 `22%`를 줘야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

`.fb2-window` 요소에 `height`의 `100%`를 줘야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

`.fb2-window` 요소에 `background-color`의 `--window-color3`를 줘야 합니다.
