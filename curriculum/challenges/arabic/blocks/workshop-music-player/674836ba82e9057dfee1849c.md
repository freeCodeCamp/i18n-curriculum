...
بعد ذلك، استخدم الخاصية `classList` والطريقة `add()` لإضافة الصنف `playing` إلى عنصر `playButton`. سيبحث هذا عن الصنف `playing` في ملف CSS ويضيفه إلى عنصر `playButton`.

ثم، عيّن `userData.currentSong` إلى `song`، ولتشغيل الأغنية أخيرًا، استخدم الطريقة `play()` على المتغير `audio`. `play()` هي طريقة من واجهة برمجة تطبيقات صوت الويب لتشغيل ملف mp3.

# --hints--

يجب أن تضيف الصنف `playing` إلى عنصر `playButton` داخل دالتك `playSong`.
...
يجب أن تعيّن `userData.currentSong` إلى `song` داخل دالتك `playSong`.
...
يجب أن تستدعي الطريقة `play()` على المتغير `audio` داخل دالتك `playSong`.
