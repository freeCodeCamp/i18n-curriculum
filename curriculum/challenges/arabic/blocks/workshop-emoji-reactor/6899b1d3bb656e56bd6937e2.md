...
أسفل عنصر `p`، أنشئ عنصر `div` مع الصنف `btn-container`.

داخل عنصر `div`، أنشئ عنصر `button`.

يجب أن يحتوي هذا الـ`button` على `id` بقيمة `happy-btn`، وصنف `emoji-btn`، و`aria-label` بقيمة `Happy face emoji`.

# --hints--

أنشئ عنصر `div` جديد أسفل عنصر `p`.

```js
assert.exists(document.querySelector("p + div"));
```

يجب أن يحتوي عنصر `div` الجديد على صنف `btn-container`.

```js
assert.equal(document.querySelector("p + div")?.className, "btn-container");
```

يجب أن يكون هناك عنصر `button` داخل عنصر `div`.

```js
assert.exists(document.querySelector("div > button"));
```

يجب أن يحتوي عنصر `button` على معرف `id` باسم `happy-btn`.

```js
assert.equal(document.querySelector("div > button")?.id, "happy-btn");
```

يجب أن يحتوي عنصر `button` على صنف `class` باسم `emoji-btn`.

```js
assert.equal(document.querySelector("div > button")?.className, "emoji-btn");
```

يجب أن يحتوي عنصر `button` على `aria-label` باسم `Happy face emoji`.
