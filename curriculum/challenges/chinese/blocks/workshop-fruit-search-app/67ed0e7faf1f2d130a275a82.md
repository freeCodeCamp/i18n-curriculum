...
当用户在 `input` 字段中输入时，应显示一个与其 `query` 匹配的水果 `list`。

为此，创建一个名为 `results` 的状态变量，初始值为一个空数组（`[]`）。还要使用 `useState` Hook 定义它的设置函数 `setResults`。

# --hints--

你应该为 `results` 和 `setResults` 变量使用数组解构语法。
