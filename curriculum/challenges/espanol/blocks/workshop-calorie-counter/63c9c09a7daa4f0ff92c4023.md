...
Usando esa misma sintaxis, consulta tus `number` inputs en el elemento `#lunch` y asígnalos a `lunchNumberInputs`.
...
Debes llamar a `document.querySelectorAll()` con `#lunch input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Debes asignar el resultado de tu llamada `document.querySelectorAll()` a `lunchNumberInputs`.
