...
Ahora agrega un selector `.logo` a la consulta `@media`, y establece la propiedad `width` en `150px`.

# --hints--

Tu regla `@media` debería tener un selector `.logo`.
...
Tu nuevo selector `.logo` debería tener un `width` de `150px`.
