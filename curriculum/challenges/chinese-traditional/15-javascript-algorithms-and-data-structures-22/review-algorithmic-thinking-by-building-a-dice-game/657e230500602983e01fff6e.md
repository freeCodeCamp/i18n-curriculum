...
聲明一個 `checkForStraights` 函數，接受一個數組作爲參數。 如果用戶得到大順子，請用 `40` 的分數更新第五個單選按鈕。 如果用戶得到小順子，請用 `30` 的分數更新第四個單選按鈕。 如果用戶沒有得到順子，請將最後一個單選按鈕更新爲顯示 `0`。
...
如果擲出了小順子，你的 `checkForStraights` 函數應該啓用第四個單選按鈕，將值設爲 `30`，並更新顯示文本爲 `, score = 30`。
...
如果擲出了大順子，你的 `checkForStraights` 函數應該啓用第五個單選按鈕，將值設爲 `40`，並更新顯示文本爲 `, score = 40`。
...
如果擲出了大順子，你的 `checkForStraights` 函數也應該啓用第四個單選按鈕，將值設爲 `30`，並更新顯示文本爲 `, score = 30`。
...
如果沒有擲出順子，你的 `checkForStraights` 函數應該啓用最後一個單選按鈕，將值設爲 `0`，並更新顯示文本爲 `, score = 0`。
