...
أعطِ عنصر الـ span النص `remainingCalories Calorie surplusOrDeficit`، مستخدمًا الاستيفاء لاستبدال `remainingCalories` و `surplusOrDeficit` بالمتغيرات المناسبة.

# --hints--

يجب أن يحتوي `span` على النص `${remainingCalories} Calorie ${surplusOrDeficit}`.
