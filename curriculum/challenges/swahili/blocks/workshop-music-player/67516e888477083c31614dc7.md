...
Unda kitendakazi kinachoitwa `highlightCurrentSong`. Ndani ya kitendakazi hicho, hakikisha kuwa kipengele cha `.playlist-song` chenye `aria-current="true"` kipo. Ikiwa kipo, tumia njia ya `removeAttribute` kuondoa sifa ya kipengele ya `aria-current` kutoka kwake.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `highlightCurrentSong`.

```js
assert.isFunction(highlightCurrentSong)
```

Kitendakazi chako cha `highlightCurrentSong` kinapaswa kutumia njia ya `removeAttribute` kuondoa sifa ya kipengele ya `aria-current` kutoka kwa kipengele cha `.playlist-song` chenye `aria-current="true"` ikiwa kipengele hicho kipo.
