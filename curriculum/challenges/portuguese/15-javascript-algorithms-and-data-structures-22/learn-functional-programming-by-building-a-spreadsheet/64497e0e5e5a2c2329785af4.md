...
Use a operador `&&` para adicionar uma segunda condição à instrução `if`, que também verifique se o primeiro caractere de `value` é `"="`.

# --hints--

Você deve usar o operador `&&` para adicionar uma segunda condição à instrução `if`, que também verifique se o primeiro caractere de `value` é `"="`. Você pode usar `[0]`, `.startsWith()` ou `.charAt(0)`.
