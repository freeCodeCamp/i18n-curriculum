...
考虑数字 142857。我们可以通过将最后一位数字（7）移动到最前面来对该数字进行右旋转，得到 714285。

它可以验证714285美元=5 × 142857美元。

这证明了142857的一个不寻常的属性：它是右旋的除数

对于数字$a$和$b$的整数，都有这个属性
找到所有整数$n$的最后5位数，$10^a &lt; n &lt; 10^b$, 都有这个属性

# --hints--

`numberRotations(2, 10)`应该返回`98311`.

```js
assert.strictEqual(numberRotations(2, 10), 98311);
```

`numberRotations(2, 100)` 应该返回 `59206`。
