...
Ви повинні додати четвертому елементу `option` текст `freeCodeCamp Forum`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.textContent, 'freeCodeCamp Forum');
```

Ви повинні додати п’ятому елементу `option` текст `Other`.
