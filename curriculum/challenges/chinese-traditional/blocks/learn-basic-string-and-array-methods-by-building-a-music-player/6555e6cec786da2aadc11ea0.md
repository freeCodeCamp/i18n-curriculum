...
你也應該重新算繪歌曲、暫停目前播放的歌曲、設定播放器顯示，並再次設定播放按鈕的存取文字。

呼叫 `renderSongs` 函式（程式），並傳入 `userData?.songs` 作為引數。也呼叫 `pauseSong`、`setPlayerDisplay` 和 `setPlayButtonAccessibleText` 函式（程式）。

# --hints--

你不應該修改現有的 `shuffle` 函式（程式）及其內容。
