...
버튼 텍스트에 했던 것과 같은 패턴을 따라, 세 버튼의 `onclick` 할당을 `"button functions"` 배열의 첫 번째, 두 번째, 세 번째 값으로 업데이트하세요.

# --hints--

`"button functions"` 매개변수의 `location` 속성 첫 번째 요소에 접근해야 합니다.

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

`button1.onclick` 매개변수의 `"button functions"` 속성 첫 번째 요소가 되도록 `location` 속성을 설정해야 합니다.

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

`"button functions"` 매개변수의 `location` 속성 두 번째 요소에 접근해야 합니다.

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

`button2.onclick` 매개변수의 `"button functions"` 속성 두 번째 요소가 되도록 `location` 속성을 설정해야 합니다.

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

`"button functions"` 매개변수의 `location` 속성 세 번째 요소에 접근해야 합니다.

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*2\s*\]/);
```

`button3.onclick` 매개변수의 `"button functions"` 속성 세 번째 요소가 되도록 `location` 속성을 설정해야 합니다.
