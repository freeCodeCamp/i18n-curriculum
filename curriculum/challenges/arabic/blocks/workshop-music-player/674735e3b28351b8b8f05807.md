...
تسترجع الطريقة `find()` أول عنصر داخل مصفوفة يحقق الشروط المحددة في دالة الاستدعاء الراجع المقدمة. إذا لم يحقق أي عنصر الشرط، تُرجع الطريقة القيمة `undefined`.

في المثال أدناه، تُستخدم الطريقة `find()` لإيجاد أول رقم أكبر من 25:
...
داخل دالتك `playSong`، استخدم الطريقة `find()` على مصفوفة `userData.songs` للبحث عن أغنية لها `id` يساوي تمامًا `id` الممرر إلى دالة `playSong`. عيّن نداء `find` إلى متغير اسمه `song`.

# --hints--

يجب أن تستدعي دالتك `playSong` الطريقة `find` على `userData.songs`.
...
يجب أن تُرجع نداء `find` أغنية يكون `id` الخاص بها مساويًا تمامًا لـ `id` الممرر إلى دالة `playSong`.
...
يجب أن تعيّن نداء `find` إلى متغير اسمه `song`.
