...
لمساعدتك في تتبع حركة الحضور عبر النظام، سجّل الرسالة التالية في وحدة التحكم: `overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`. يمكنك استخدام إما ربط السلاسل النصية أو القوالب الحرفية.

# --hints--

يجب أن تسجل دالة `rerouteOverflow` الخاصة بك الرسالة التالية في وحدة التحكم `overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`.
