...
你將需要有一個文本列和一個圖像列。 將 `.text-with-images` 選擇器的 `grid-template-columns` 屬性設置爲 `1fr 2fr`。 並將 `column-gap` 屬性設置爲 `3rem`，以提供更大的列間距。

# --hints--

你的 `.text-with-images` 選擇器應該有一個 `grid-template-columns` 屬性，設置爲 `1fr 2fr`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

你的 `.text-with-images` 選擇器應該有一個 `column-gap` 屬性，設置爲 `3rem`。
