...
Se la differenza è entro il `tolerance` specificato, imposta il valore di `root` su `mid` e esci dal loop.

# --hints--

Dovresti rimuovere l'istruzione `pass` esistente.
...
Dovresti assegnare il valore di `mid` a `root` ed uscire dal loop.
