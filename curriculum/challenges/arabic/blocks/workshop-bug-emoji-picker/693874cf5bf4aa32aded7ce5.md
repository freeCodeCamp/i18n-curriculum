...
حتى الآن، لا يتغير شيء عند اختيار خطأ برمجي مختلف.

أضف مستمع حدث `change` إلى `selectElement`. في الاستدعاء الراجع، استخدم `isSelect` للتحقق مما إذا كان `e.target` هو `HTMLSelectElement`. لا تحتاج إلى فعل أي شيء داخل كتلة `if` بعد.

# --hints--

يجب أن يحتوي `selectElement` على مستمع حدث `change` مع استدعاء راجع يأخذ معلمة حدث.
...
يجب أن يحتوي الاستدعاء الراجع لمستمع الحدث على جملة `if` بشرط `isSelect(e.target)`.
