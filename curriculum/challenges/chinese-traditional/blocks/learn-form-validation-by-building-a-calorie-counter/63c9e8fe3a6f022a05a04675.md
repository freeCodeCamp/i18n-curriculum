...
使用相同的插值語法，為你的第二個 `p` 元素添加文字 `consumedCalories Calories Consumed`，並為第三個添加文字 `exerciseCalories Calories Burned`。記得將你的 `consumedCalories` 和 `exerciseCalories` 變數取代為適當的值。

# --hints--

你應該為你的模板字面值添加第二個 `p` 元素。
...
你的第二個 `p` 元素應該有文字 `${consumedCalories} Calories Consumed`。
...
你的第三個 `p` 元素應該有文字 `${exerciseCalories} Calories Burned`。
