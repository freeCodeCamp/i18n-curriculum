...
Crea un selector `.author` y dale una propiedad `font-size` establecida en `2rem` y una propiedad `font-family` establecida en `Raleway` con una alternativa de `sans-serif`.

# --hints--

Debe haber un selector `.author`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.author'));
```

Tu selector `.author` debe tener una propiedad `font-size` establecida en `2rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize, '2rem');
```

Tu selector `.author` debe tener una propiedad `font-family` establecida en `Raleway` con una alternativa de `sans-serif`.
