...
在你的 `.image-wrapper` 元素中，给第三个 `img` 元素添加一个值为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg` 的 `src`、一个值为 `four people working on code` 的 `alt`、一个值为 `lazy` 的 `loading` 属性、一个值为 `image-3` 的 `class`、一个值为 `600` 的 `width` 属性和一个值为 `400` 的 `height` 属性。

# --hints--

你的第三个 `img` 元素应该有一个 `src` 属性，值为 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`。
...
你的第三个 `img` 元素应该有一个 `alt`，值为 `four people working on code`。
...
你的第三个 `img` 元素应该有一个 `loading` 属性，值为 `lazy`。
...
你的第三个 `img` 元素应该有一个 `class`，值为 `image-3`。
...
你的第三个 `img` 元素应该有一个 `width` 属性，值为 `600`。
...
你的第三个 `img` 元素应该有一个 `height` 属性，值为 `400`。
