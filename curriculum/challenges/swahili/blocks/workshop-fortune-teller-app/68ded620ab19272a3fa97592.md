...
Kwa kuwa mradi sasa unafanya kazi na vipengele vya HTML DOM, unahitaji njia ya kuonyesha bahati kwenye DOM.

Tangaza kitendakazi kinachoitwa `renderCard` kinachopokea hoja nne: `drawingType`, `isReversed`, `shortName`, na `img`.

Vigezo `drawingType`, `shortName`, na `img` vinapaswa kuwa vya aina ya `string`, wakati `isReversed` inapaswa kuwa ya aina ya `boolean`. Kitendakazi kinapaswa kuwa na aina ya kurudisha `string`.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `renderCard`.
...
Kitendakazi chako cha `renderCard` kinapaswa kuwa na vigezo vinne.
...
Kigezo chako cha kwanza kinapaswa kuwa `drawingType` cha aina ya `string`.
...
Kigezo chako cha pili kinapaswa kuwa `isReversed` cha aina ya `boolean`.
...
Kigezo chako cha tatu kinapaswa kuwa `shortName` cha aina ya `string`.
...
Kigezo chako cha nne kinapaswa kuwa `img` cha aina ya `string`.
...
Kitendakazi chako cha `renderCard` kinapaswa kuwa na aina ya kurudisha `string`.
...
Kitendakazi chako cha `renderCard` kinapaswa kurudisha template literal.

```js
assert.match(
  __helpers.removeJSComments(code),
  /renderCard\s*=\s*\([^)]*\)\s*:\s*string\s*=>\s*\{?\s*`/
);
```
