...
Unahitaji kuonyesha bidhaa zinazopatikana katika HTML yako. Anza kwa kutumia opereta ya kuongeza kuongeza mfuatano wa herufi wa template tupu kwenye sifa ya `innerHTML` ya variable `dessertCards`.

# --hints--

Unapaswa kupata upatikanaji wa sifa ya `innerHTML` ya variable `dessertCards`.

```js
assert.match(code, /dessertCards\.innerHTML/);
```

Unapaswa kutumia opereta ya kuongeza kwenye sifa ya `innerHTML`.

```js
assert.match(code, /dessertCards\.innerHTML\s*\+=\s*/);
```

Unapaswa kuongeza mfuatano wa herufi wa template tupu kwenye sifa ya `innerHTML`.
