...
Tienes un pequeño error. `arg1` y `arg2` son cadenas, no números. `infixToFunction['+']("1", "2")` devolvería `12`, lo cual no es matemáticamente correcto.

Envuelve cada uno de tus argumentos de `infixToFunction[operator]` en una llamada de `parseFloat()`.

# --hints--

Debes envolver `arg1` en una llamada de `parseFloat()`.
...
Debes envolver `arg2` en una llamada de `parseFloat()`.
