...
`if` 文の中で、`cardTitle` 要素を取得し、その `textContent` を `foundCard.name` に設定してください。同様に、`description` 要素の `textContent` を `foundCard.desc` に設定してください。

# --hints--

`cardTitle` の `textContent` を `foundCard.name` に設定する必要があります。`this` キーワードを使うのを忘れないでください。

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

`description` の `textContent` を `foundCard.desc` に設定する必要があります。`this` キーワードを使うのを忘れないでください。
