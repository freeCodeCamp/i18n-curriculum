...
Оскільки `elemValue` повертає функцію, функція `addCharacters` зрештою повертає масив посилань на функції. Ви хочете, щоб метод `.map()` запускав внутрішню функцію функції `elemValue`, тобто потрібно викликати `elemValue` замість посилання на неї. Передайте `num` як аргумент функції `elemValue`.

<!-- TODO: Explain further? -->

# --hints--

Викличте `elemValue()` у методі `.map()`.
...
Передайте `num` до виклику `elemValue()`.
