...
У цьому практичному занятті ви дізнаєтеся про абстрактні класи та дженерики в TypeScript, створюючи селектор видів помилок, який відображає різні емодзі помилок.

HTML і CSS для проєкту вже надані, щоб ви могли зосередитися на TypeScript. Ви можете переглянути файли, щоб ознайомитися з елементами, які будете вибирати пізніше.

Для початку вам потрібен абстрактний базовий клас для всіх помилок. Абстрактні класи не можна створювати напряму, вони призначені для розширення іншими класами.

Створіть абстрактний клас з іменем `Bug` з дженериком `T`.

# --hints--

У вас має бути абстрактний клас з іменем `Bug`.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+class\s+Bug/);
```

Ваш клас `Bug` має мати дженерик `T`.
