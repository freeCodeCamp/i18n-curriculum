...
`noteEl` 변수에 `addEventListener` 메서드를 연결하세요. 첫 번째 인수는 `"blur"` 이벤트여야 하며, 두 번째 인수는 화살표 함수여야 합니다. 그 화살표 함수 본문 안에서 `const`를 사용해 `newContent`라는 새 변수를 만들고 `noteEl.innerHTML` 값을 할당하세요.

`innerHTML`는 노트의 간격과 형식을 유지하는 데 사용됩니다.

# --hints--

`noteEl` 변수에 `addEventListener` 메서드를 연결해야 합니다.
