...
シャッフルボタンが押されたとき、`currentSong` を空にし、`songCurrentTime` を `0` に設定してください。

`userData.currentSong` を `null` に、`userData.songCurrentTime` を `0` に設定してください。

*注意*: このステップでは、`currentSong` と `songCurrentTime` のプロパティをそれぞれ `null` と `0` に明示的に設定しているため、オプショナルチェーンは使用しないでください。

# --hints--

`userData.currentSong` を `null` に設定してください。

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

`userData.songCurrentTime` を `0` に設定してください。
