...
استخدم `addEventListener` لإضافة مستمع حدث `"click"` إلى `loadMoreBtn`. أيضًا، مرر مرجعًا إلى الدالة `fetchMoreAuthors` ليتم تنفيذها كلما نُقر الزر.

بعد ذلك، عند نقر الزر يجب أن ترى 8 مؤلفين إضافيين.
...
يجب أن تستدعي الطريقة `addEventListener()` على المتغير `loadMoreBtn` الخاص بك.
...
يجب أن يأخذ مستمع الحدث الخاص بك الدالة `fetchMoreAuthors` كدالة للتنفيذ.
