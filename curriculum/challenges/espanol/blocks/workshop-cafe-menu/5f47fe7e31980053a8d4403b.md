...
Hay una forma más fácil, simplemente añade un margen superior negativo a los elementos `img` para moverlos de sus posiciones actuales. Los valores negativos se crean usando un `-` delante del valor. Para completar este taller, adelante y usa un margen superior negativo de `25px` en el selector de tipo `img`.

¡Felicidades! Has completado el taller del Menú de Café. 

# --hints--

La propiedad `margin-top` debe tener el valor `-25px`.
...
Tu elemento `img` debe tener un `margin-top` (margen superior) dé `-25px`.
