...
Cada rol de `region` requiere una etiqueta, y ayuda a los usuarios de lectores de pantalla a entender el propósito de la región. Un método para agregar una etiqueta es añadir un elemento de encabezado dentro de la región y luego referenciarlo con el atributo `aria-labelledby`.
...
Luego, dentro de cada elemento `section`, anida un elemento `h2` con un `id` que coincida con el atributo `aria-labelledby` correspondiente. Proporcione un contenido de texto adecuado para cada `h2`.
...
Debes dar al primer elemento `h2` un contenido de texto adecuado. _Pista: Yo habría elegido `Student Info`_

```js
assert.isAtLeast(document.querySelectorAll('h2')?.[0]?.textContent?.length, 1);
```

Debes dar al segundo elemento `h2` un contenido de texto adecuado. _Pista: Yo habría elegido `HTML`_

```js
assert.isAtLeast(document.querySelectorAll('h2')?.[1]?.textContent?.length, 1);
```

Debes dar al tercer elemento `h2` un contenido de texto adecuado. _Pista: Yo habría elegido `CSS`_
