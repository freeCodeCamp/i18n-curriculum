...
title: Sostituire catene di if else con switch
challengeType: 1
forumTopicId: 18266
...
Trasforma le istruzioni concatenate `if`/`else if` in un'istruzione `switch`.

# --hints--

Non dovresti usare alcuna istruzione `else` in tutto l'editor

```js
assert(!/else/g.test(__helpers.removeJSComments(code)));
```

Non dovresti usare alcuna istruzione `if` in tutto l'editor
