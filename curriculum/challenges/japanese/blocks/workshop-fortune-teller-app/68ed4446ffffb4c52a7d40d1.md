...
コンストラクタの中で、`fetchCardsData()` と `initializeEventListeners()` を `this` キーワードを使って呼び出し、クラスが初期化されたときに実行されるようにしてください。

`fetchCardsData` メソッドはAPIからタロットカードのデータを読み込む処理を担当し、`initializeEventListeners` はボタンのクリックハンドラを設定します。これらのメソッドは今後のステップで作成します。

# --hints--

`fetchCardsData()` メソッドは `constructor` の中で呼び出す必要があります。
...
コンストラクタの中で `initializeEventListeners()` メソッドを呼び出す必要があります。
