...
你的下一個正規表達式將尋找像 `free money` 這樣的字串。宣告一個 `freeRegex` 變數並指定一個能匹配字串 `free money` 的正規表達式。記得要設為不區分大小寫。

然後，將你的新正規表達式添加到你的 `denyList` 陣列中，以便你可以測試它。

# --hints--

你應該宣告一個 `freeRegex` 變數。

```js
assert.exists(freeRegex);
```

你的 `freeRegex` 變數應該是一個正規表達式。
...
你的 `freeRegex` 應該是不區分大小寫的。

```js
assert.include(freeRegex.flags, 'i');
```

你的 `denyList` 陣列應該包含你的 `freeRegex` 變數。

```js
assert.deepInclude(denyList, freeRegex);
```

你的 `denyList` 陣列應該包含你的 `dollarRegex` 變數。

```js
assert.deepInclude(denyList, dollarRegex);
```

你的 `denyList` 陣列應該包含你的 `helpRegex` 變數。
