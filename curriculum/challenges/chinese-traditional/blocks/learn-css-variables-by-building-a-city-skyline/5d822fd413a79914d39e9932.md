...
構建下一棟建築！ 這是前景中的綠色建築。 將其背景設置爲 `repeating-linear-gradient`，從 `0%` 到 `5%` 使用其建築顏色，從 `5%` 到 `10%` 使用 `transparent`。

# --hints--

你應該給 `.fb5` 一個 `background` 屬性。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

你應該給 `background` 一個 `repeating-linear-gradient`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

你應該將從 `0%` 到 `5%` 的 `--building-color2` 設置爲 `repeating-linear-gradient` 的第一種顏色。
...
你應該將從 `5%` 到 `10%` 的 `transparent` 設置爲 `repeating-linear-gradient` 的第二種顏色。
