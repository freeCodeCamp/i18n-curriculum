...
الآن ستستخدم الطريقة `join()` لدمج تمثيلات السلاسل النصية لأرقام العقد في مسار واحد قابل للقراءة.

تأخذ الطريقة `join()` قابلًا للتكرار (مثل تعبير المولِّد الخاص بك) وتدمج كل العناصر في سلسلة نصية واحدة، مع وضع الفاصل بين كل عنصر. على سبيل المثال:
...
أنشئ متغيرًا اسمه `path` وعيّن له نتيجة دمج `string_path` باستخدام `' -> '` كفاصل.

# --hints--

يجب أن تنشئ متغيرًا اسمه `path`.
...
تستخدم `join` لدمج العناصر في `string_path` مع `' -> '` كفاصل وتعيّن النتيجة إلى `path`.
