...
Sería bueno ver los cambios editados en la consola.

Agrega un `console.log(currentContent);`. 

Para probar tus cambios, intenta editar la nota y luego haz clic fuera de la nota. Ahora deberías ver la nota editada en la consola. 

# --hints--

Debes agregar un `console.log(currentContent);` dentro del event listener, después de asignar `newContent` a `currentContent`.
