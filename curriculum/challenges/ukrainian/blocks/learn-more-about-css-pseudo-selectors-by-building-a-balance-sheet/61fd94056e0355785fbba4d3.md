...
До другого `tr` додайте елемент `th` із текстом `Expenses Annual anticipated expenses, such as payroll.`. Обгорніть цей текст, окрім `Expenses`, елементом `span` з `class` зі значенням `description`.
