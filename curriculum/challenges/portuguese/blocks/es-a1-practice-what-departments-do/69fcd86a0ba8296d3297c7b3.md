...
Ouça e complete cada frase com o departamento correto.
...
`El departamento de aprendizaje y desarrollo capacita al equipo.` — O departamento de aprendizagem e desenvolvimento treina a equipe.

Tanto `apoyar` (to support) quanto `capacitar` (to train) são verbos `-ar` na terceira pessoa do singular.

Seguindo o mesmo padrão de `desarrolla`, `verifica` e `crea` das tarefas anteriores.
