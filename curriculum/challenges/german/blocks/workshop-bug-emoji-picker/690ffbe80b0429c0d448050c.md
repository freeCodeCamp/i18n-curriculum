...
Jetzt müssen Sie die Eigenschaften definieren, die alle Bugs haben werden.

Deklarieren Sie innerhalb der `Bug`-Klasse eine Eigenschaft namens `emoji` mit dem Typ `T` unter Verwendung des Operators für die Zusicherung der definitiven Zuweisung (`!`). Das teilt TypeScript mit, dass die Eigenschaft später zugewiesen wird.

# --hints--

Ihre `Bug`-Klasse sollte eine Eigenschaft namens `emoji` haben.
...
Ihre `emoji`-Eigenschaft sollte den Typ `T` haben.
...
Ihre `emoji`-Eigenschaft sollte den Operator für die Zusicherung der definitiven Zuweisung (`!`) verwenden.
