...
title: aggiungere etichette ai cerchi del grafico a dispersione
challengeType: 6
forumTopicId: 301477
...
Puoi aggiungere testo per creare etichette per i punti in un grafico a dispersione.

L’obiettivo è mostrare i valori separati da virgola per il primo (`x`) e il secondo (`y`) campo di ogni elemento in `dataset`.

I nodi `text` hanno bisogno degli attributi `x` e `y` per posizionarli nell’SVG. In questa sfida, il valore `y` (che determina l’altezza) può usare lo stesso valore che `circle` usa per il suo attributo `cy`. Il valore `x` può essere leggermente più grande del valore `cx` del `circle`, così l’etichetta è visibile. Questo spingerà l’etichetta a destra del punto tracciato.

# --instructions--

Etichetta ogni punto nel grafico a dispersione usando gli elementi `text`. Il testo dell’etichetta deve essere i due valori separati da una virgola e uno spazio. Per esempio, l’etichetta per il primo punto è `34, 78`. Imposta l’attributo `x` in modo che sia `5` unità più del valore che hai usato per l’attributo `cx` sul `circle`. Imposta l’attributo `y` nello stesso modo in cui è usato per il valore `cy` sul `circle`.
...
La prima etichetta dovrebbe avere testo `34, 78`, un valore `x` di `39` e un valore `y` di `422`.
...
La seconda etichetta dovrebbe avere testo `109, 280`, un valore `x` di `114` e un valore `y` di `220`.
...
La terza etichetta dovrebbe avere testo `310, 120`, un valore `x` di `315` e un valore `y` di `380`.
...
La quarta etichetta dovrebbe avere testo `79, 411`, un valore `x` di `84` e un valore `y` di `89`.
...
La quinta etichetta dovrebbe avere testo `420, 220`, un valore `x` di `425` e un valore `y` di `280`.
...
La sesta etichetta dovrebbe avere testo `233, 145`, un valore `x` di `238` e un valore `y` di `355`.
...
La settima etichetta dovrebbe avere testo `333, 96`, un valore `x` di `338` e un valore `y` di `404`.
...
L’ottava etichetta dovrebbe avere testo `222, 333`, un valore `x` di `227` e un valore `y` di `167`.
...
La nona etichetta dovrebbe avere testo `78, 320`, un valore `x` di `83` e un valore `y` di `180`.
...
La decima etichetta dovrebbe avere testo `21, 123`, un valore `x` di `26` e un valore `y` di `377`.
