...
У своєму першому елементі `p` додайте елемент `span`. Присвойте `span` клас `product-count` та `id` як `product-count-for-id${id}`. Потім після span додайте до свого елемента `p` текст змінної `name`.
