...
现在，让我们深入了解一下按位运算符。 这些按运算符对数字的二进制表示进行运算。  JavaScript provides several bitwise operators, including AND (`&`), OR (`|`), XOR (`^`), NOT (`~`), left shift (`<<`), and right shift (`>>`).

按位与（`&`）操作符在两个操作对象的对应位都为 `1` 时，在该位位置返回 `1`。下面是一个示例：
