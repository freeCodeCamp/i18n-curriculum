...
Agora você precisará criar sua primeira função curried. Lembre-se de que uma função curried é uma função que retorna outra função, assim:
...
Comece declarando uma função `convertQuantity`. Deve ter um único parâmetro `fromUnit` e deve estar vazio.

# --hints--

Você deve declarar uma função `convertQuantity`.

```js
assert.isFunction(convertQuantity)
```

Sua função `convertQuantity` deve ter um parâmetro `fromUnit`.

```js
assert.match(convertQuantity?.toString(), /\(fromUnit\)/);
```

Sua função `convertQuantity` deve estar vazia.
