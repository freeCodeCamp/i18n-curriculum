...
接下來，使用 `.forEach()` 方法循環遍歷 `animationData` 數組。 對於 `.forEach()` 方法的回調函數，傳入 `obj` 作爲參數，但回調函數體暫時留空。

# --hints--

你應該使用 `.forEach()` 方法循環遍歷 `animationData` 數組。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\(/);
```

你應該將 `obj` 作爲參數傳遞給 `.forEach()` 方法的回調函數。
