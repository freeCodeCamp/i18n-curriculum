...
Настав час розпочати реалізувати функціональності для відтворення відображених пісень.

Визначте функцію `playSong` використавши `const`. Функція має приймати параметр `id`, який представлятиме унікальний ідентифікатор пісні, яку ви хочете відтворити.

# --hints--

Використайте `const`, щоб створити порожню функцію під назвою `playSong`.
...
Функція `playSong` повинна використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Функція `playSong` має приймати параметр `id`.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

Функція `playSong` має бути порожньою.
