...
왼쪽 팔에 `wave` 애니메이션을 사용하세요. 애니메이션은 `3s` 동안 지속되고, 무한히 반복하며, 선형 타이밍 함수를 가지게 하세요.

# --hints--

`.arm.left`에 `animation-name`의 `--fcc-expected--`를 주어야 하지만 `--fcc-actual--`를 발견했습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationName, 'wave');
```

`.arm.left`에 `animation-duration`의 `--fcc-expected--`를 주어야 하지만 `--fcc-actual--`를 발견했습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationDuration, '3s');
```

`.arm.left`에 `animation-iteration-count`의 `--fcc-expected--`를 주어야 하지만 `--fcc-actual--`를 발견했습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationIterationCount, 'infinite');
```

`.arm.left`에 `animation-timing-function`의 `--fcc-expected--`를 주어야 하지만 `--fcc-actual--`를 발견했습니다.
