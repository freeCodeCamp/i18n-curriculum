...
1. 你應該將每個 `input`、`select` 和 `textarea` 元素與 `label` 元素關聯。
1. 你應該有一個 `button` 元素，其型別為 `submit`，用於提交表單。
1. 為你的 `input` 和 `textarea` 元素添加 `:focus` 偽類，以在聚焦時更改它們的邊框顏色並移除預設的外框。
...
你應該將每個 `input`、`select` 和 `textarea` 元素與 `label` 元素關聯。 
...
你應該使用 `input` 和 `textarea` 的 `:focus` 偽類選擇器來移除聚焦時的預設外框。
...
`input`、`select` 和 `textarea` 元素應該有一個 `:valid` 偽類，當輸入有效時會將邊框顏色變更為綠色。請依照給定順序使用列表選擇器。
