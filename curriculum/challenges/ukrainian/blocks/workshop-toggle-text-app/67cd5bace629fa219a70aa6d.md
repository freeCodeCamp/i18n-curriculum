...
Тепер настав час використати вашу функцію `handleToggleVisibility`.

У попередніх уроках ви дізналися про систему синтетичних подій React і як працювати з атрибутами, такими як `onClick` і `onSubmit`.

Пам’ятайте, що ви можете передати референс функції в атрибут `onClick` ось так:
...
Оновіть свій існуючий елемент `button`, щоб додати атрибут `onClick` і передати референс функції `handleToggleVisibility`.

Тепер, коли ви натиснете на кнопку, ви побачите, як видимість тексту абзацу перемикається між прихованим і показаним на сторінці.

# --hints--

Коли натискають на `toggle-button`, на екрані має з’явитися повідомлення `"I love freeCodeCamp!"`.
