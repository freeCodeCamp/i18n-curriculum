...
Другий об’єкт у вашому масиві `allSongs` має мати властивість `id` зі значенням числа `1`.

```js
assert.strictEqual(allSongs[1].id, 1);
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `title` зі значенням рядка `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `artist` зі значенням рядка `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `duration` зі значенням рядка `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

Другий об’єкт у вашому масиві `allSongs` має мати властивість `src` зі значенням рядка `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
