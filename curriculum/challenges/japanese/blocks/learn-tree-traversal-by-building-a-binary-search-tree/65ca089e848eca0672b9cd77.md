...
さて、`_delete` メソッドに戻り、`_min_value` ヘルパー関数を使って後続を選ぶ必要があります。

`_min_value` ブロックの後に `elif` の呼び出しを追加し、`node.right` を引数として渡してください。現在のノードキーに代入します。

# --hints--

`self._min_value(node.right)` ブロックの後に `node.key` を `elif` に代入する必要があります。
