...
Finalmente, você pode executar suas simulações.

Comece com o turno da manhã. Faça uma chamada a `simulateFestival()` com `morningGates` e `"Morning"`.

# --hints--

Você deve fazer uma chamada a `simulateFestival()` com `morningGates` e `"Morning"`.
