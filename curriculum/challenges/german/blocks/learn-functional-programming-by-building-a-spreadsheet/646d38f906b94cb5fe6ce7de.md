...
Ihre `addCharacters(char1)` gibt ebenfalls eine Funktion zurück, die eine weitere Funktion zurückgibt. Sie müssen einen weiteren Funktionsaufruf tätigen, um auf diese innerste Funktionsreferenz für den `.map()` Callback zuzugreifen. JavaScript erlaubt es Ihnen, zurückgegebene Funktionen sofort aufzurufen:

```js
myFunc(1)("hi");
```

Rufen Sie die Funktion, die von Ihrem `addCharacters(char1)` Aufruf zurückgegeben wird, sofort auf, und übergeben Sie `char2` als Argument.

# --hints--

Sie sollten einen Funktionsaufruf an Ihren `addCharacters(char1)` Aufruf anhängen.
...
Sie sollten `char2` als Argument an Ihren verketteten Funktionsaufruf übergeben.
