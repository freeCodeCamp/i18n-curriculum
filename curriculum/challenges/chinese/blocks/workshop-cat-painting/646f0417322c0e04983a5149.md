...
使用类选择器，将 `.cat-mouth-line-left` 元素的 `position` 设置为 `absolute`，`top` 设置为 `88px`，`left` 设置为 `74px`。 这会将其移动到正确的位置。

# --hints--

你应该有一个 `.cat-mouth-line-left` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left'));
```

你的 `.cat-mouth-line-left` 选择器应将 `position` 属性设置为 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.position, 'absolute');
```

你的 `.cat-mouth-line-left` 选择器应将 `top` 属性设置为 `88px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.top, '88px');
```

你的 `.cat-mouth-line-left` 选择器应将 `left` 属性设置为 `74px`。
