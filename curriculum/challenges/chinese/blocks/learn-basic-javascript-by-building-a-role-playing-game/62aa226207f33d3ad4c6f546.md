...
使用 `+=` 操作符将带有前置空格的 `" Your <weapon> breaks."` 添加到 `text.innerText` 的末尾。使用 `inventory.pop()` 替换 `<weapon>` 为 `inventory` 数组中的最后一项，`inventory.pop()` 会移除数组中的最后一项并返回它，使其出现在你的字串中。

# --hints--

你应该使用复合赋值运算符来修改 `text.innerText`。
