...
Ricorda che puoi usare la destrutturazione per estrarre multipli valori da un array o oggetto in una singola istruzione.

Per il primo parametro della tua funzione di callback, destruttura le proprietà `name`, `id`, `price` e `category` dall'istanza `Dessert` passata.

# --hints--

Dovresti usare la destrutturazione per dichiarare i parametri `name`, `id`, `price` e `category`. Per questo test, l'ordine è importante.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

La tua destrutturazione dovrebbe essere il primo parametro della funzione di callback.
