...
Quando você clica em uma música da sua playlist, você quer tocar essa música.Para isso você precisará adicionar um event listener para o evento `click` em cada elemento `button` que é filho de um elemento `song`.

Complete o callback do seu `forEach` para fazer isso.Dentro do callback do `addEventListener`, chame `playSong(n)`, onde `n` é o id da música que você pode obter do atributo `id` de `song`.

Observe que cada `song` tem um atributo `id` com o valor `song-n`: você precisará obter a parte `n` e convertê-la em um número antes de passá-la para `playSong()`.


# --hints--

Você deve adicionar um event listener para o evento `click` em cada elemento `button` que é filho de um elemento `.playlist-song` e chamar `playSong` com o `id` da música correspondente.
