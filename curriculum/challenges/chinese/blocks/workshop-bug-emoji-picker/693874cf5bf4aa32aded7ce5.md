...
到目前为止，选择不同的错误时没有任何变化。

为 `change` 添加一个 `selectElement` 事件监听器。在回调中，使用 `isSelect` 检查 `e.target` 是否为 `HTMLSelectElement`。你暂时不需要在 `if` 块内做任何事情。

# --hints--

你的 `selectElement` 应该有一个 `change` 事件监听器，回调函数带有一个事件参数。
...
你的事件监听器回调函数应该有一个 `if` 语句，条件是 `isSelect(e.target)`。
