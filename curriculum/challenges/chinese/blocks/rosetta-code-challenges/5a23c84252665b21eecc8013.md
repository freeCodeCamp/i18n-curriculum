...
**Strand sort** 创建被合并以创建最终结果的排序子集。 

考虑一个 `unsortedArray = [3, 1, 4, 2]`。 选择第一个元素 `3` 并将其复制到一个单独的数组中。 搜索此元素之后任何更大的元素。 当你找到一个更大的元素时（在此例中为 `4`），将它复制到单独的数组 `[3, 4]` 中，并将后续元素与这个新值 `4` 进行比较。

到达数组末尾之后，删除所复制的项目，`[3, 4]` ，然后重新开始，使用 `unsortedArray` 中剩余的第一个项目，在本例中为 `1` 。

按照这个进程结果排列两个排序数组， `[3, 4]` and `[1, 2]`。 合并这两个数组以创建 `strandSortedArray`。
