...
Você criou um calculador de trajetória de projétil.

Agora para concluir, modifique o código que você escreveu fora das classes e incorpore-o em uma pequena função utilitária chamada `projectile_helper` que recebe os valores desejados para velocidade, altura e ângulo e imprime no terminal em sequência, os detalhes do projétil, a tabela de coordenadas e o gráfico da trajetória.

Chame `projectile_helper` uma vez com os valores de sua escolha. 

# --hints--

Você deve criar `projectile_helper` no escopo global.
...
Você deve imprimir as strings solicitadas na ordem.Não imprima outros valores.
...
Você deve chamar a função `projectile_helper` pelo menos uma vez.
...
Você não deve ter variáveis ou chamadas de print no escopo global.
