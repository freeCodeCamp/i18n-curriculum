...
為了幫助追蹤參加者如何在系統中移動，請將以下訊息記錄到主控台：`overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`。你可以使用字串串接或模板字串。

# --hints--

你的 `rerouteOverflow` 函式應該將以下訊息記錄到主控台 `overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`。
