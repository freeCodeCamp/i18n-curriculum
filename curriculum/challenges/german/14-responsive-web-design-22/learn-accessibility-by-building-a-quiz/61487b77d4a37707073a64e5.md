...
Wenn die Bildschirmbreite gering ist, wird der Textinhalt von `h1` nicht wie gewünscht umbrochen. Richte den Text für das `h1`-Element in der Mitte aus.

Gib anschließend das `main`-Padding so an, dass die `Student Info`-Header-Sektion vollständig angezeigt werden kann.

# --hints--

Du solltest `h1` ein `text-align` von `center` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

Du solltest einen `main`-Selektor hinzufügen, um das `main`-Element auszuwählen.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Du solltest `main` ein `padding-top` von mindestens `25px` geben.
