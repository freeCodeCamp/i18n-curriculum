...
HTML 파일에서 `#next`, `#previous`, `#shuffle` 요소에 접근하세요.

각각을 `nextButton`, `previousButton`, `shuffleButton`라는 변수에 할당하세요.

# --hints--

`nextButton`라는 변수가 있어야 합니다.

```js
assert.isDefined(nextButton);
```

`document.getElementById()`를 사용해 `#next` 요소를 가져와야 합니다.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

`#next` 요소를 `nextButton` 변수에 할당해야 합니다.

```js
assert.equal(nextButton, document.getElementById('next'));
```

`previousButton`라는 변수가 있어야 합니다.

```js
assert.isDefined(previousButton);
```

`document.getElementById()`를 사용해 `#previous` 요소를 가져와야 합니다.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

`#previous` 요소를 `previousButton` 변수에 할당해야 합니다.

```js
assert.equal(previousButton, document.getElementById('previous'));
```

`shuffleButton`라는 변수가 있어야 합니다.

```js
assert.isDefined(shuffleButton);
```

`document.getElementById()`를 사용해 `#shuffle` 요소를 가져와야 합니다.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

`#shuffle` 요소를 `shuffleButton` 변수에 할당해야 합니다.
