...
Функція Ейлера, ${\phi}(n)$ (іноді її називають функцією фі), використовується для визначення кількості додатних чисел, менших або рівних `n`, які є взаємно простими з `n`. Наприклад, числа 1, 2, 4, 5, 7 і 8 всі менші за дев’ять і взаємно прості з дев’ятьма, тому ${\phi}(9) = 6$. Число 1 вважається взаємно простим з будь-яким додатним числом, тому ${\phi}(1) = 1$.

Цікаво, що ${\phi}(87109) = 79180$, і можна побачити, що 87109 є перестановкою 79180.

Знайдіть значення `n`, для якого 1 &lt; `n` &lt; `limit`, при якому ${\phi}(n)$ є перестановкою `n`, а відношення $\displaystyle\frac{n}{{\phi}(n)}$ є мінімальним.

# --hints--

`totientPermutation(10000)` має повертати число.

```js
assert(typeof totientPermutation(10000) === 'number');
```

`totientPermutation(10000)` має повертати `4435`.

```js
assert.strictEqual(totientPermutation(10000), 4435);
```

`totientPermutation(100000)` має повертати `75841`.

```js
assert.strictEqual(totientPermutation(100000), 75841);
```

`totientPermutation(500000)` має повертати `474883`.

```js
assert.strictEqual(totientPermutation(500000), 474883);
```

`totientPermutation(10000000)` має повертати `8319823`.
