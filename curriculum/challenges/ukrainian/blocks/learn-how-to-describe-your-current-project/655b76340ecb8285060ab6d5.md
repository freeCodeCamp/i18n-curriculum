...
Яке речення правильно описує, що Софі та Джеймс зараз роблять у своїх проєктах?
...
Цей варіант неправильно використовує час `present simple` для Софі та час `past simple` для Джеймса. Вони не відображають безперервний, поточний характер їхніх дій.
...
Цей варіант неправильно використовує час `past simple` для дії Софі та `present simple` для дії Джеймса, що не відповідає їхнім поточним і безперервним діям.
...
Цей варіант неправильно припускає майбутню дію для Софі та минулу безперервну дію для Джеймса, що не відповідає розмові.
