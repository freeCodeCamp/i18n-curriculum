...
De volta à sua função `showLatestPosts()`, use destructuring para obter as propriedades `topic_list` e `users` do objeto `data`.

# --hints--

Você deve usar `const` para desestruturar `topic_list` e `users` do objeto `data`.
