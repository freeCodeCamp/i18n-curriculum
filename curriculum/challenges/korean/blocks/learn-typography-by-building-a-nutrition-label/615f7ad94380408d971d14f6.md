...
`:not` 의사선택자는 주어진 CSS 규칙과 일치하지 않는 모든 요소를 선택하는 데 사용할 수 있습니다.
...
위 코드는 `div`인 `id`를 갖지 않는 모든 `example` 요소를 선택합니다.

`.daily-value p` 선택자를 수정하여 `.no-divider` 요소를 제외하세요.

# --hints--

`.daily-value p:not(.no-divider)` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p:not(.no-divider)'));
```

`.daily-value p` 선택자는 없어야 합니다.

```js
assert(!new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

`.daily-value p:not(.no-divider)` 선택자의 속성은 변경하지 마세요.
