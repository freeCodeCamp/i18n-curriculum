...
fortuneアプリには過去、現在、未来の3つの瞬間しかないので、`DrawingType`という名前のenumを宣言し、3つのプロパティ`Past`、`Present`、`Future`を持たせてください。それぞれのプロパティは、その名前に対応する文字列の値を持つ必要があります。

# --hints--

`enum`という名前の`DrawingType`を持っているはずです。

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

enumには文字列の値が`Past`である`past`プロパティが必要です。

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

enumには文字列の値が`Present`である`present`プロパティが必要です。

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

enumには文字列の値が`Future`である`future`プロパティが必要です。
