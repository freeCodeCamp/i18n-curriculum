...
To get the index for the current song, you can use the <dfn>indexOf()</dfn> method. Die `indexOf()` Array-Mehode gibt den ersten Index zurück, bei dem ein gegebenes Element im Array gefunden werden kann, oder sie gibt `-1` zurück, wenn das Element nicht vorhanden ist.
...
You should return `userData?.songs.indexOf()` inside your `getCurrentSongIndex` function.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

You should pass in `userData?.currentSong` into the `indexOf()` method.
