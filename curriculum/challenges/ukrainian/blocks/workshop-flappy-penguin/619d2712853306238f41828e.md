...
Створіть кастомну CSS-змінну з іменем `--penguin-skin` і встановіть їй значення `gray`. Потім замініть усі відповідні значення властивостей на неї.

# --hints--

Ви повинні надати `:root` властивість `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

Ви повинні надати `--penguin-skin` значення `gray`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

Ви повинні надати `.penguin-head` властивість `background` зі значенням `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
Ви повинні надати `.penguin-body::before` властивість `background-color` зі значенням `var(--penguin-skin)`, але знайдено `--fcc-actual--`.
...
Ви повинні надати `.arm` властивість `background` зі значенням `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
