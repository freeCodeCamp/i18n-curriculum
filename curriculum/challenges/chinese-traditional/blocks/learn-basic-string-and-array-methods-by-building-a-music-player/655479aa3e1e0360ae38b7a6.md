...
接下來，你需要取得播放列表中的下一首歌曲。為此，你需要取得目前歌曲的索引，然後將 `1` 加到該索引上。

創建一個名為 `nextSong` 的常數，並將 `userData?.songs[currentSongIndex + 1]` 指定給它。

最後，呼叫 `playSong` 函式（程式），並將 `nextSong.id` 作為引數傳入。

# --hints--

你不應該修改現有的 `if` 述語、其 `else` 區塊和內容。
...
你應該將 `userData?.songs[currentSongIndex + 1]` 指定給一個 `nextSong` 常數。
...
你應該使用 `nextSong.id` 來呼叫 `playSong` 函式（程式）。
