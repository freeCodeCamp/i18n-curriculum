...
为图像元素添加一个 `onerror` 处理器来处理加载失败。在第一次失败时，使用 `dataset.failed` 标记它，并将源替换为局部的默认图像。如果图像再次失败，移除 hidden 类并使用 `previousElementSibling` 隐藏加载占位符。

# --hints--

你的 `img` 元素应该有一个 `onerror` 处理器。
...
你的 `onerror` 处理器应该包含一个 `if` 语句，用来检查 `dataset.failed` 是否为假。
...
`dataset.failed` 应该在你的 `'1'` 块内被设置为 `if`。这里别忘了 `this` 关键字。
...
你的 `if` 块应该更新 `this.src` 为局部的默认图像。这里别忘了 `this` 关键字。
...
你应该有一个 `else` 块，用来移除 `hidden` 类。这里别忘了使用 `this` 关键字。
...
你的 `else` 块应该将 `previousElementSibling.style.display` 设置为 `'none'`。这里别忘了 `this` 关键字。
