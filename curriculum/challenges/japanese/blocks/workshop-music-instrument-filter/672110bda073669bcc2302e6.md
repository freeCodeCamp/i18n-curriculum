...
`instrumentsArr` は楽器カードそれぞれのオブジェクトを保持します。各オブジェクトは `category`、`instrument`、`price` のキーを持ちます。

配列にフルートを表す最初のオブジェクトを追加してください。値にはそれぞれ `woodwinds`、`Flute`、`500` を使います。

# --hints--

`instrumentsArr` 配列には1つの要素があるはずです。

```js
assert.lengthOf(instrumentsArr, 1);
```

`instrumentsArr` の中に1つのオブジェクトがあるはずです。

```js
assert.isObject(instrumentsArr[0]);
```

`instrumentsArr` の中のオブジェクトは `category` キーを持ち、その値は `woodwinds` であるべきです。

```js
assert.equal(instrumentsArr[0].category, "woodwinds");
```

`instrumentsArr` の中のオブジェクトは `instrument` キーを持ち、その値は `Flute` であるべきです。

```js
assert.equal(instrumentsArr[0].instrument, "Flute");
```

`instrumentsArr` の中のオブジェクトは `price` キーを持ち、その値は `500` であるべきです。
