...
Para alinear las casillas de entrada entre sí, crea un nuevo conjunto de reglas que apunte a todos los elementos `input` y `label` dentro de un elemento `.info` y establece la propiedad `display` en `inline-block`.

Además, alinea el texto del elemento `label` a la derecha.

# --hints--

Deberías usar un selector como `.info > label, .info > input` o `.info label, .info input`.
...
Debes dar a los elementos `input` y `label` un `display` de `inline-block`.
...
Debes dar al elemento `label` un `text-align` de `right`.
