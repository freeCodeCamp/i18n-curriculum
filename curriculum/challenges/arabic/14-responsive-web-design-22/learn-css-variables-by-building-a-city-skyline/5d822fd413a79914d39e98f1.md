...
يجب عليك إنشاء منتقي لفئة جديد يسمى `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

يجب عليك نقل خاصية `height` والقيمة من `.bb1a` إلى `.bb1-window`.
...
يجب عليك نقل خاصية `background` والقيمة من `.bb1a` إلى `.bb1-window`.
...
لا يجب عليك نقل خاصية `background-color` من `.bb1a`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

يجب ألا تحرك خاصية `width` من `.bb1a`.
