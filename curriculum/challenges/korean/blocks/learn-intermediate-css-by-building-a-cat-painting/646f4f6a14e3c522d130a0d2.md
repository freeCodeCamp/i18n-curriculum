...
클래스 선택자를 사용해 `.cat-whisker-right-middle` 요소를 지정한 다음, `position`의 `absolute`과 `top`의 `127px`, `left`의 `109px`를 사용해 오른쪽 중간 수염을 위치시키세요.

# --hints--

`.cat-whisker-right-middle` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle'))
```

`.cat-whisker-right-middle` 선택자에 `position` 속성이 `absolute`으로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.position === 'absolute')
```

`.cat-whisker-right-middle` 선택자에 `top` 속성이 `127px`으로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.top === '127px')
```

`.cat-whisker-right-middle` 선택자에 `left` 속성이 `109px`으로 설정되어야 합니다.
