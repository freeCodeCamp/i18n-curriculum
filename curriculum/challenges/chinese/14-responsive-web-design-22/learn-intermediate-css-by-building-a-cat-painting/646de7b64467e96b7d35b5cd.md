...
将右眼移动到位，其 `position` 属性为 `absolute`，`top` 为 `54px`，`left` 为 `134px`。

# --hints--

你的 `.cat-right-eye` 选择器应该设置一个 `position` 属性为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.position === 'absolute')
```

你的 `.cat-right-eye` 选择器应该设置一个 `top` 属性为 `54px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.top === '54px')
```

你的 `.cat-right-eye` 选择器应该设置一个 `left` 属性为 `134px`。
