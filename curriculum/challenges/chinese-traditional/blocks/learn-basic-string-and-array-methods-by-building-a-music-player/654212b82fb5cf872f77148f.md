...
在 `button` 元素內，創建兩個 `span` 元素。 

第一個 `span` 元素應該有一個 `class` 為 `playlist-song-artist`。在 `span` 頁籤之間，添加 `${song.artist}`。 

第二個 `span` 元素應該有一個 `class` 為 `playlist-song-duration`。在 `span` 頁籤之間，添加 `${song.duration}`。 

# --hints--

你不應該修改現有的 `button` 元素及其內容。
...
你應該創建一個帶有類別 `playlist-song-artist` 的 `span` 元素。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

你的 `span` 元素，其類別為 `playlist-song-artist`，應該將 `song.artist` 插值為其內容。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

你應該創建一個帶有類別 `playlist-song-duration` 的 `span` 元素。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

你的 `span` 元素，且帶有類別 `playlist-song-duration`，應該將 `song.duration` 插值為其內容。
