...
Aggiungi un attributo `value` con valore `player.club` al Club `input`. Dopodiché, aggiungi un attributo `onChange` impostato su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `club` impostato su `e.target.value`.

# --hints--

Il tuo Club `input` dovrebbe avere un `value` uguale al club del giocatore.
...
Il tuo Club `input` dovrebbe avere un gestore `onChange` che chiama `setPlayer` con uno spread di `player` e `club` impostato su `e.target.value`.
