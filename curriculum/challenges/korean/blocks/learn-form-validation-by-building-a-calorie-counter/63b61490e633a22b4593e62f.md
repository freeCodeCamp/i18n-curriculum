...
같은 패턴을 따라 `#add-entry` 요소를 `addEntryButton`에, `#clear` 요소를 `clearButton`에, `#output` 요소를 `output`에 할당하세요.

# --hints--

`addEntryButton`라는 변수를 선언하세요.

```js
assert.isDefined(addEntryButton);
```

`document.getElementById()`를 사용해 `#add-entry` 요소를 가져오세요.

```js
assert.match(code, /document\.getElementById\(\s*('|")add-entry\1\s*\)/g);
```

`#add-entry` 요소를 `addEntryButton`에 할당하세요.

```js
assert.deepEqual(addEntryButton, document.getElementById('add-entry'));
```

`clearButton`라는 변수를 선언하세요.

```js
assert.isDefined(clearButton);
```

`document.getElementById()`를 사용해 `#clear` 요소를 가져오세요.

```js
assert.match(code, /document\.getElementById\(\s*('|")clear\1\s*\)/g);
```

`#clear` 요소를 `clearButton`에 할당하세요.
...
`document.getElementById()`를 사용해 `#output` 요소를 가져오세요.

```js
assert.match(code, /document\.getElementById\(\s*('|")output\1\s*\)/g);
```

`#output` 요소를 `output`에 할당하세요.
