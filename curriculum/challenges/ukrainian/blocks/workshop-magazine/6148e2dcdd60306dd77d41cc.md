...
Час стилізувати останній розділ журналу — зображення.

Зображення обгорнуті елементом `aside` з класом `image-wrapper`, тож створіть селектор `.image-wrapper`. Встановіть властивість `display` у значення `grid`.

# --hints--

У вас має бути селектор `.image-wrapper`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

Ваш селектор `.image-wrapper` повинен мати властивість `display`, встановлену в `grid`.
