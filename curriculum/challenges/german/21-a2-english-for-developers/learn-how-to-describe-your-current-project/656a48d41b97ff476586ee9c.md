...
Brian uses specific expressions in his conversation. `To be out`, in this context, means to be available or published. Hier ein Beispiel:

`The new book is out next week.` means the book will be available then.

`Looking forward to` + a verb that ends in `-ing` is a common way to express excitement about a future event. Zum Beispiel:
