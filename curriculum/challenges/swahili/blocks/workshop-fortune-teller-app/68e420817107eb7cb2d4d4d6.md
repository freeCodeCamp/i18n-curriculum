...
Ongeza mshughulikiaji wa `onerror` kwa kipengele cha picha kushughulikia kushindwa kwa upakiaji. Kwa kushindwa kwa kwanza, kiweke alama kwa kutumia `dataset.failed` na badilisha chanzo na picha ya chaguo-msingi ya ndani. Ikiwa picha itashindwa tena, ondoa darasa la kuficha na ficha kiwakilishi cha upakiaji kwa kutumia `previousElementSibling`.

# --hints--

Kipengele chako cha `img` kinapaswa kuwa na mshughulikiaji wa `onerror`.
...
Mshughulikiaji wako wa `onerror` unapaswa kuwa na usemi wa `if` unaokagua kama `dataset.failed` ni falsy.
...
`dataset.failed` inapaswa kuwekwa kuwa `'1'` ndani ya blokki yako ya `if`. Usisahau neno kuu la `this` hapa.
...
Blokki yako ya `if` inapaswa kusahihisha `this.src` kuwa picha ya chaguo-msingi ya ndani. Usisahau neno kuu la `this` hapa.
...
Unapaswa kuwa na sehemu ya `else` inayotoa darasa la vitu la `hidden`. Usisahau kutumia neno kuu `this` hapa.
...
Blokki yako ya `else` inapaswa kuweka `previousElementSibling.style.display` kuwa `'none'`. Usisahau neno kuu la `this` hapa.
