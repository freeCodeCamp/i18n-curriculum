...
Crea un selector `.author-name a:hover` y dale una propiedad `background-color` establecida en `#306203`.

Esto creará un efecto de hover solo para el elemento `a` dentro del `.author-name`, mostrando el verde original de freeCodeCamp en el fondo.

# --hints--

Debes tener un selector `.author-name a:hover`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

Tu selector `.author-name a:hover` debe tener una propiedad `background-color` establecida en `#306203`.
