...
قبل تشغيل الأغنية، يجب التأكد من أنها تبدأ من البداية. يمكن تحقيق ذلك باستخدام خاصية `currentTime` على كائن `audio`.

أضف عبارة `if` للتحقق من عدم تشغيل أي أغنية حالية عبر التأكد من أن `userData.currentSong` قيمة خاطئة.
داخل كتلة `if`، عيّن خاصية `currentTime` لكائن `audio` إلى `0`.

# --hints--

يجب أن تضبط دالة `playSong` خاصية `currentTime` لكائن `audio` على `0` عندما لا تكون هناك أغنية حالية قيد التشغيل.
