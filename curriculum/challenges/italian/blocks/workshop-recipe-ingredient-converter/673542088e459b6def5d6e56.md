...
Ti serve una funzione che aggiorni effettivamente il DOM. Dichiara una funzione `updateResultsList`.

Per cominciare, la funzione dovrebbe cancellare l'HTML dall'elemento `resultList`.

# --hints--

Dovresti definire una funzione `updateResultsList`.

```js
assert.isFunction(updateResultsList);
```

La tua funzione `updateResultsList` dovrebbe impostare `resultList.innerHTML` su una stringa vuota.
