...
你应该使用 `nav > ul > li:hover` 或 `nav li:hover` 选择器来设置元素的悬停样式。
...
你应将 `li` 元素的悬停 `background-color` 设置为 `#dfdfe2`。
...
你应将 `li` 元素的悬停 `cursor` 设置为 `pointer`。
