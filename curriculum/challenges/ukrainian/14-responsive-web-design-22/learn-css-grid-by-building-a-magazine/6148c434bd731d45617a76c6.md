...
Ця властивість приймає `row` або `column` як перше значення, з додатковим другим значенням `dense`. `grid-auto-flow` використовує алгоритм автоматичного розміщення для налаштування макета сітки. Якщо встановити значення на `column`, алгоритм створюватиме нові стовпчики для вмісту за потреби. Значення `dense` дозволяє алгоритму відстежувати та заповнювати діри в сітці меншими елементами, що може призвести до того, що елементи зображатимуться не за порядком.

Встановіть властивість `grid-auto-flow` свого селектора `.social-icons` на `column`.

# --hints--

Ваш селектор `.social-icons` повинен мати властивість `grid-auto-flow` зі значенням `column`.
