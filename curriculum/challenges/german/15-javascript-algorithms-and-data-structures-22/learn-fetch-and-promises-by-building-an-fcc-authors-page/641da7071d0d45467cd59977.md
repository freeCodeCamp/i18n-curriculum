...
Nun musst du einige Informationen über den Autor angeben. Zeige zunächst den Namen des Autors an.

Create an `h2` tag with the `class` `"author-name"`. Interpoliere dann den `author` innerhalb des `h2`-Tags. Dies ist der Name des Autors.
...
Du solltest ein `h2`-Element innerhalb deines `div`-Elements erstellen.
...
Du solltest den `author` in deinem `h2`-Tag interpolieren, um die Namen der Autoren anzuzeigen
