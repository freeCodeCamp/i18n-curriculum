...
Successivamente, usa la proprietà `classList` e il metodo `add()` per aggiungere la classe `"playing"` all'elemento `playButton`. Questo cercherà la classe `"playing"` nel file CSS e la aggiungerà all'elemento `playButton`.

Per far partire finalmente la canzone, usa il metodo `play()` sulla variabile `audio`. `play()` è un metodo dell'API audio web per riprodurre un file mp3.

# --hints--

Dovresti usare la proprietà `classList` e il metodo `add` per aggiungere la classe `"playing"` a `playButton`.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

Dovresti usare il metodo `play()` sulla variabile `audio`.
