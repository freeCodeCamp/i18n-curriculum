...
Зараз `currentContent` ініціалізовано значенням порожнього рядка. Але бажаним результатом буде, щоб `currentContent` містив значення поточного тексту нотатки.

У попередньому уроці ви навчились працювати з подією `DOMContentLoaded` так:
...
Коли весь HTML-документ буде завантажено та проаналізовано, ви зможете безпечно отримати доступ до `noteEl`.

Почніть із приєднання методу `addEventListener` до об’єкта `window`. Першим аргументом для методу `addEventListener` має бути подія `"DOMContentLoaded"`. Другим аргументом має бути стрілкова функція. Усередині тіла стрілкової функції призначте значення `noteEl.textContent` змінній `currentContent`.

# --hints--

Ви повинні приєднати метод `addEventListener` до об’єкта `window`.

```js
assert.match(code, /window\.addEventListener/);
```

Ваш слухач подій має слухати подію `"DOMContentLoaded"`.

```js
assert.match(code, /window\.addEventListener\(['"]DOMContentLoaded['"]\,/);
```

У тілі стрілкової функції призначте значення `noteEl.textContent` змінній `currentContent`.
