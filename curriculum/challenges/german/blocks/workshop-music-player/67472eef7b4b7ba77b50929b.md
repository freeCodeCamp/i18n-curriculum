...
Erstellen Sie in Ihrem `userData`-Objekt eine `songs`-Eigenschaft und setzen Sie deren Wert auf das `allSongs`-Array.

Um dann die Informationen zum aktuellen Lied zu verwalten und dessen Wiedergabezeit zu verfolgen, erstellen Sie `currentSong`- und `songCurrentTime`-Eigenschaften. Setzen Sie die Werte jeweils auf `null` und `0`.

# --hints--

Ihr `userData`-Objekt sollte einen `songs`-Schlüssel haben, der auf das `allSongs`-Array gesetzt ist.

```js
assert.propertyVal(userData, "songs", allSongs);
```

Ihr `userData`-Objekt sollte einen `currentSong`-Schlüssel haben, der auf `null` gesetzt ist.

```js
assert.propertyVal(userData, "currentSong", null);
```

Ihr `userData`-Objekt sollte einen `songCurrentTime`-Schlüssel haben, der auf `0` gesetzt ist.
