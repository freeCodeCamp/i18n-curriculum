...
Adicione um atributo `value` de `player.overallRating` ao `input` geral. Depois disso, adicione um atributo `onChange` definido como uma arrow function que recebe `e` como parâmetro e faz uma chamada `setPlayer` com um spread de `player` e `overallRating` definido para `Number(e.target.value)`.

Como `overallRating` é um número, use `Number()` para converter o valor string da entrada.

# --hints--

Seu `input` geral deve ter um `value` igual à avaliação geral atual do jogador.
...
Seu `input` geral deve ter um atributo `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

O `onChange` do seu `input` geral deve fazer uma chamada `setPlayer` com um spread de `player` e `overallRating` definido para `Number(e.target.value)`.
