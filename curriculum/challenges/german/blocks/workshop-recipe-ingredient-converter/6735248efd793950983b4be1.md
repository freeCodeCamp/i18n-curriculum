...
Geben Sie Ihrem `conversionTable` eine `ounce`-Eigenschaft und weisen Sie ihr ein Objekt zu, das eine `cup`-Eigenschaft mit dem Wert `0.125`, eine `gram`-Eigenschaft mit dem Wert `28.35` und eine `teaspoon`-Eigenschaft mit dem Wert `6` enthält.

# --hints--

Ihr `conversionTable`-Objekt sollte eine `ounce`-Eigenschaft haben.

```js
assert.property(conversionTable, "ounce");
```

Ihre `ounce`-Eigenschaft sollte ein Objekt sein.

```js
assert.isObject(conversionTable.ounce);
```

Ihr `ounce`-Objekt sollte eine `cup`-Eigenschaft haben.

```js
assert.property(conversionTable.ounce, "cup");
```

Ihre `cup`-Eigenschaft sollte auf `0.125` gesetzt sein.

```js
assert.strictEqual(conversionTable.ounce?.cup, 0.125);
```

Ihr `ounce`-Objekt sollte eine `gram`-Eigenschaft haben.

```js
assert.property(conversionTable.ounce, "gram");
```

Ihre `gram`-Eigenschaft sollte auf `28.35` gesetzt sein.

```js
assert.strictEqual(conversionTable.ounce?.gram, 28.35);
```

Ihr `ounce`-Objekt sollte eine `teaspoon`-Eigenschaft haben.

```js
assert.property(conversionTable.ounce, "teaspoon");
```

Ihre `teaspoon`-Eigenschaft sollte auf `6` gesetzt sein.
