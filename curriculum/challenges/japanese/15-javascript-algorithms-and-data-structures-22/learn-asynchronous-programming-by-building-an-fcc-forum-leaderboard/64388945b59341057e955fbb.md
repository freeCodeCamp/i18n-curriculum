...
フォーラムリーダーボードに表示するデータを、API にリクエストする必要があります。 このような処理は<dfn>非同期</dfn>処理と呼ばれ、タスクがプログラムのメインのフローから独立して実行されます。

`async` というキーワードを使うと、プロミスを返す非同期関数を作成できます。
...
では、`async` キーワードを使って、`fetchData` という名前の非同期アロー関数を作成してください。

# --hints--

`async` キーワードを使って、`fetchData` という名前の非同期アロー関数を作成してください。

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*[^}]*\s*}/);
```

`fetchData` を定義する必要があります。

```js
assert.isDefined(fetchData);
```

`fetchData` は関数でなければなりません。
