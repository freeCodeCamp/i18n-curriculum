...
Il primo elemento `img` dovrebbe avere un attributo `src` con il valore `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`.
...
Il primo elemento `img` dovrebbe avere un attributo `alt` con il valore `image of the quote machine project`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('alt') === 'image of the quote machine project');
```

Il primo elemento `img` dovrebbe avere l'attributo `class` impostato su `image-1`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.classList?.contains('image-1'));
```

Il primo elemento `img` dovrebbe avere un attributo `loading` impostato su `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('loading') === 'lazy');
```

Il primo elemento `img` dovrebbe avere un attributo `width` impostato a `600`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('width') === '600');
```

Il primo elemento `img` dovrebbe avere un attributo `height` impostato a `400`.
