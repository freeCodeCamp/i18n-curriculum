...
Unapaswa kuunda nakala ya kinafupi ya safu ya data `crew` kwa kutumia njia ya `slice()`. Ita safu mpya `updatedCrew`.

Kumbuka kwamba kuita `slice()` bila hoja hurudisha nakala ya safu yote:
...
Unapaswa kutumia njia ya `slice()` kunakili safu ya data ya ingizo `crew` na kuhifadhi matokeo katika kigezo kipya kinachoitwa `updatedCrew`.
