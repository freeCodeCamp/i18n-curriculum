...
Tu revista tendrá tres secciones principales. Ya estableciste el diseño general en la regla `main`, pero puedes ajustar la colocación en las reglas hijas.

Una opción es la propiedad `grid-column`, que es una abreviatura de `grid-column-start` y `grid-column-end`. La propiedad `grid-column` indica al elemento de la cuadrícula en qué línea de la cuadrícula comenzar y terminar.

Crea una regla `.heading` y establece la propiedad `grid-column` en `2 / 3`. Esto indicará al elemento `.heading` que comience en la línea de cuadrícula 2 y termine en la línea de cuadrícula 3.
...
Tu selector `.heading` debe tener una propiedad `grid-column` establecida en `2 / 3`.
