...
Beginnen Sie damit, eine neue Variable namens `userData` zu deklarieren und weisen Sie ihr ein leeres Objekt zu.

# --hints--

Sie sollten eine Variable mit dem Namen `userData` haben.

```js
assert.isDefined(userData);
```

Sie sollten Ihrer `userData`-Variable ein leeres Objekt zuweisen.
