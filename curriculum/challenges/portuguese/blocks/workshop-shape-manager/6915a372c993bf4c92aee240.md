...
Agora, crie uma função `initializeApp` e deixe-a vazia por enquanto.

Ela vai inicializar todo o app assim que o DOM for carregado. Então, é a função que você logo vai passar para um evento `DOMContentLoaded` que fará o app funcionar.

# --hints--

Você deve criar uma função `initializeApp`.
