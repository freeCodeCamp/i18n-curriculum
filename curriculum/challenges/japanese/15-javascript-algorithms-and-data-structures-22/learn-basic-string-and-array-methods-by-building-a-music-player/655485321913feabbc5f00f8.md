...
`"click"` イベントリスナーを `previousButton` の要素に追加して、第二引数として `playPreviousSong` を渡してください。

# --hints--

変数 `previousButton` に対して `addEventListener()` メソッドを呼び出してください。
...
`addEventListener` メソッドに第二引数として `playPreviousSong` を渡してください。
