...
在 `href` 属性后，将 `class` 属性设置为名为 `linkClass` 的常量。 

在 `class` 属性后，将 `target` 属性设置为 `"_blank"`。 

最后，将 `linkText` 常量放在锚标签之间，以显示链接中的文本。

# --hints--

你应该在 `href` 属性后有一个 `class` 属性。
...
你应该将 `class` 属性的值设置为名为 `linkClass` 的常量。
