...
Agora, é hora de personalizar os botões de opção. Por padrão, os botões de opção têm uma aparência nativa do navegador. Você quer remover isso para poder criar um visual personalizado.

Selecione o elemento com a classe `contact-method-radio-btn`. Defina `appearance` como `none`.

# --hints--

Você deve direcionar o elemento com a classe `contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Defina a propriedade `appearance` como `none`.
