...
이제 `0/10` 텍스트에서 첫 번째 숫자를 가져와야 합니다. 이 방법은 `5/10` 또는 `10/10`에도 작동해야 합니다.

`.count` 요소의 텍스트 내용을 파싱하여 현재 숫자를 추출하세요.

문자열에서 숫자로 변환하세요.

이전의 `console.log(countEl.textContent)`를 새 로그로 바꾸세요.

현재 숫자를 다음 형식으로 기록하세요: `console.log("Current count:", currCount)`.

힌트: 문자열을 하위 문자열 배열로 나누려면 `split` 메서드를 사용할 수 있다는 점을 기억하세요.

# --hints--

현재 숫자를 숫자형으로 기록해야 합니다.
...
다양한 가능한 값에 대해 작동하는 방식으로 현재 숫자를 숫자형으로 기록해야 합니다.
