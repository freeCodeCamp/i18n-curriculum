...
Du solltest einen `.calories-info span`-Selektor erstellen.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span'));
```

Dein `.calories-info span`-Selektor sollte eine `font-size`-Eigenschaft auf `2.4em` gesetzt haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span')?.fontSize === '2.4em');
```

Dein `.calories-info span`-Selektor sollte eine `font-weight`-Eigenschaft auf `700` gesetzt haben.
