...
プロジェクトがHTML DOM要素で動作するようになったので、フォーチュンをDOMにレンダリングする方法が必要です。

`renderCard`という名前の関数を宣言してください。この関数は4つの引数、`drawingType`、`isReversed`、`shortName`、`img`を取ります。

パラメータ`drawingType`、`shortName`、`img`は`string`型でなければならず、`isReversed`は`boolean`型でなければなりません。関数の戻り値の型は`string`にしてください。

# --hints--

`renderCard`という名前の関数があるはずです。
...
最初のパラメータは`drawingType`で、型は`string`であるべきです。
...
2番目のパラメータは`isReversed`で、型は`boolean`であるべきです。
...
3番目のパラメータは`shortName`で、型は`string`であるべきです。
...
4番目のパラメータは`img`で、型は`string`であるべきです。
...
`renderCard`関数の戻り値の型は`string`であるべきです。
...
`renderCard`関数はテンプレートリテラルを返すべきです。
