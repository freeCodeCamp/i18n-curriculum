...
Щоб кнопка відправки виглядала більш помітною, надайте їй `background-color` зі значенням `#3b3b4f`, а також `border-color` зі значенням `white`.

# --hints--

Ви повинні надати кнопці відправки `background-color` зі значенням `#3b3b4f`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.backgroundColor, 'rgb(59, 59, 79)');
```

Ви повинні надати кнопці відправки `border-color` зі значенням `white`.
