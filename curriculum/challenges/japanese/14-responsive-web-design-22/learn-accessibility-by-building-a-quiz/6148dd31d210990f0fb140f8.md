...
`fieldset` 要素の周りに表示されているデフォルトのボーダーは、開発中は役に立ちます。 ですが、それが望んでいるスタイルであるとは限りません。

`.question` 要素のボーダーと下部のパディングを除去してください。

# --hints--

`.question` セレクターを使用する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

`.question` の `border` を `none` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

`.question` の `padding-bottom` を `0` に設定する必要があります。
