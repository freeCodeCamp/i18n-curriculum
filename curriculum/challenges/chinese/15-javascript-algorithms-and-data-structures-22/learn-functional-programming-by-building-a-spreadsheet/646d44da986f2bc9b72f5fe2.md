...
声明一个 `toNumberList` 函数，该函数接受 `args` 参数并隐式返回用逗号拆分 `args` 的结果。 然后将 `map` 方法链接到 `split` 方法，并将 `parseFloat` 作为参数传递给 `map` 方法。

# --hints--

你应该声明一个 `toNumberList` 变量。
...
你应该使用 `const` 来声明你的 `toNumberList` 变量。
...
你的 `toNumberList` 函数应该有一个 `args` 参数。

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

你的 `toNumberList` 函数应该使用隐式返回。
...
你的 `toNumberList` 函数应该返回调用 `args` 的 `.split()` 方法的结果。
...
你应该将对 `parseFloat` 的引用作为对 `.map()` 的回调传递。
