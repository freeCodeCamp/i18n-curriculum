...
Sasa, ni wakati wa kufafanua kinachotokea wakati kitufe cha radio kimechaguliwa.

Unapopanga mtindo wa mduara wa ndani tu wakati kitufe cha radio kimechaguliwa, kwa hiyo kichaguzi chako kinapaswa kuwa `.contact-method-radio-btn:checked::before`.

Katika sheria hiyo, weka `transform` kuwa `translate(3px, 3px) scale(1)` na `background-color` kuwa `lightgreen`.

`transform` husogeza kidogo mduara wa ndani na kuupanua hadi ukubwa kamili kwa `scale(1)`, na rangi ya nyuma ya lightgreen hujazaa mduara kwa rangi kuonyesha kuwa chaguo limechaguliwa, na kuufanya uonekane.

# --hints--

Unapaswa kulenga kipengele cha bandia cha `.contact-method-radio-btn:checked::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

Weka sifa ya `transform` kuwa `translate(3px, 3px) scale(1)`.
...
Weka sifa ya `background-color` kuwa `lightgreen`.
