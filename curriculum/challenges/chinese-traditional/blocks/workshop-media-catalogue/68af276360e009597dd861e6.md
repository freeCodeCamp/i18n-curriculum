...
創建另一個 `if` 述語來檢查 `director` 是否為空或僅包含空白，並引發帶有訊息 `Director cannot be empty` 的 `ValueError`。

# --hints--

當 `director` 為空或僅包含空白時，你應該引發一個帶有訊息 `Director cannot be empty` 的 `ValueError`。
...
當 `director` 是有效的字串時，你不應該引發 `ValueError`。
