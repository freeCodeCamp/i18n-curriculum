...
У межах `header` надайте контекст про сторінку, вклавши один `img`, `h1` та `nav` елемент.

`img` має вказувати на `https://cdn.freecodecamp.org/platform/universal/fcc_primary.svg`, мати атрибут `id` зі значенням `logo` і містити текст `alt` зі значенням `freeCodeCamp`.
...
Ви повинні розмістити елементи `img`, `h1` та `nav` у такому порядку.
...
Ви повинні надати елементу `img` атрибут `alt` зі значенням `freeCodeCamp`.
