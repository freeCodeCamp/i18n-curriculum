...
Usijali kuhusu nafasi chini, kila kitu kitawekwa chini baadaye utakapoongeza urefu kwa kipengele kilicho juu ya jengo.

Ongeza `repeating-linear-gradient` kwa `.fb1c` kwa pembe ya `90deg`, `--building-color4` kutoka `0%` hadi `10%` na `transparent` kutoka `10%` hadi `15%`.

# --hints--

Unapaswa kumpa `.fb1c` `background` yenye `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.background, "repeating-linear-gradient");
```

Unapaswa kutumia mwelekeo wa `90deg`.
