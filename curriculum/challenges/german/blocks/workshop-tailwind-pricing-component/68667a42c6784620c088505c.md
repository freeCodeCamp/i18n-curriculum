...
Abgesehen vom `Go Premium` CTA befindet sich der Rest des Inhalts in einem `div`. Das macht CSS Grid zu einer guten Option, um alles anzuordnen.

Fügen Sie die Klassen `grid`, `grid-rows-[auto_auto_auto_1fr]` und `gap-y-2` dem `div` hinzu, der die Elemente `h2`, `p` und `ul` umschließt.

# --hints--

Ihr `div`-Element, das die Elemente `h2`, `p` und `ul` umschließt, sollte die Klasse `grid` haben.
...
Ihr `div`-Element, das die Elemente `h2`, `p` und `ul` umschließt, sollte die Klasse `grid-rows-[auto_auto_auto_1fr]` haben.
...
Ihr `div`-Element, das die Elemente `h2`, `p` und `ul` umschließt, sollte die Klasse `gap-y-2` haben.
