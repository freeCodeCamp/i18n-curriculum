...
أعطِ عناصر `.formrow` هامشًا علويًا، وحشوًا أيسر وأيمن. يجب أن تكون قيم الحشو الأخرى `0`.

ثم، زد حجم الخط لجميع عناصر `input`.

# --hints--

يجب أن تستخدم محدِّد `.formrow` لاستهداف عناصر `.formrow`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.formrow'));
```

يجب أن تعطي `.formrow` هامشًا علويًا (`margin-top`) لا يقل عن `1px`.
...
يجب أن تعطي `.formrow` حشوًا علويًا (`padding-top`) بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingTop, '0px');
```

يجب أن تعطي `.formrow` حشوًا أيمنًا (`padding-right`) لا يقل عن `1px`.
...
يجب أن تعطي `.formrow` حشوًا سفليًا (`padding-bottom`) بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingBottom, '0px');
```

يجب أن تعطي `.formrow` حشوًا أيسرًا (`padding-left`) لا يقل عن `1px`.
...
يجب أن تستخدم محدِّد `input` لاستهداف عناصر `input`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input'));
```

يجب أن تعطي `input` حجم خط (`font-size`) أكبر من `13px`.
