...
你還看不到漸層，因為 `linear-gradient` 函式需要至少兩個顏色引數才能運作。

在相同的 `linear-gradient` 函式中，使用 `rgb` 函式將第二個顏色引數設為純綠色。

# --hints--

你的 `.red` CSS 規則應該有一個 `background` 屬性，設定為 `linear-gradient(90deg, rgb(255, 0, 0), rgb(0, 255, 0))`。
