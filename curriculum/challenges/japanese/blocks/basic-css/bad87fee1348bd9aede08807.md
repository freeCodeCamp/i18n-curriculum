...
`font-family` プロパティを使って、要素が使用するフォントを設定できます。

例えば、`h2` 要素のフォントを `sans-serif` に設定したい場合は、次のようなCSSを使います。
...
すべての `p` 要素に `monospace` フォントを使うようにしてください。

# --hints--

`p` 要素には `monospace` フォントを使う必要があります。
