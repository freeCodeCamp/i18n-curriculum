...
Створіть селектор `.publish-date` і надайте йому властивість `color` зі значенням `rgba(255, 255, 255, 0.5)`.

# --hints--

У вас має бути селектор `.publish-date`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

Ваш селектор `.publish-date` повинен мати властивість `color`, встановлену в `rgba(255, 255, 255, 0.5)`.
