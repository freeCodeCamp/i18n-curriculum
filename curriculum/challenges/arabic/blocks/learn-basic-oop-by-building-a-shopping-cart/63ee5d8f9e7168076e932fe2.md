...
يجب أن يحتوي أول عنصر `p` على `class` باسم `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

يجب أن يحتوي أول عنصر `p` على علامة الدولار `"$"` متبوعة بقيمة المتغير `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

يجب أن يحتوي ثاني عنصر `p` على `class` باسم `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

يجب أن يحتوي ثاني عنصر `p` على النص `"Category: "` متبوعًا بقيمة المتغير `category`.
