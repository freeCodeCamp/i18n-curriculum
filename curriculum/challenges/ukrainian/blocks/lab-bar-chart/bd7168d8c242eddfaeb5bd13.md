...
У цьому проєкті ви використаєте дані з `https://cdn.freecodecamp.org/curriculum/labs/data/bar-chart/GDP-data.json`, щоб побудувати гістограму.

Тести вимагають, щоб осі були згенеровані за допомогою властивості осі D3, яка автоматично створює поділки вздовж осі. Ці поділки потрібні для проходження тестів D3, оскільки їхні позиції використовуються для визначення вирівнювання елементів графіка.
...
1. Ваша діаграма повинна мати `g` елемент осі x з відповідним `id="x-axis"`.
1. Ваша діаграма повинна мати `g` елемент осі y з відповідним `id="y-axis"`.
1. Обидві осі повинні містити кілька міток поділок, кожна з відповідним `class="tick"`.
1. Ваша діаграма повинна мати `rect` елемент для кожної точки даних із відповідним `class="bar"`, що відображає дані.
1. Кожен `.bar` повинен мати атрибути `data-date` і `data-gdp`, що містять значення `date` і `GDP`.
1. Атрибути `.bar` елементів `data-date` повинні відповідати порядку наданих даних.
1. Атрибути `.bar` елементів `data-gdp` повинні відповідати порядку наданих даних.
1. Висота кожного елемента `.bar` повинна точно представляти відповідне значення `GDP` у даних.
1. Атрибут `data-date` і відповідний елемент `.bar` повинні бути вирівняні з відповідним значенням на осі x.
1. Атрибут `data-gdp` і відповідний елемент `.bar` повинні бути вирівняні з відповідним значенням на осі y.
1. Ви повинні мати змогу навести курсор миші на область і побачити підказку з відповідним `id="tooltip"`, яка відображає більше інформації про цю область.
1. Ваша підказка повинна мати атрибут `data-date`, який відповідає `data-date` активної області.
...
Діаграма повинна мати елемент із `id` зі значенням `title` для заголовка.

```js
assert.exists(document.getElementById('title'));
```

Діаграма повинна мати `g` елемент осі x з відповідним ідентифікатором `x-axis`.

```js
assert.isNotEmpty(document.querySelector('g#x-axis'))
```

Діаграма повинна мати `g` елемент осі y з відповідним ідентифікатором `y-axis`.

```js
assert.isNotEmpty(document.querySelector('g#y-axis'))
```

Вісь x повинна містити кілька міток поділок, кожна з відповідним `class="tick"`.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

Вісь y повинна містити кілька міток поділок, кожна з відповідним `class="tick"`.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

Ваша діаграма повинна мати `rect` елемент для кожної точки даних із відповідним `class="bar"`, що відображає дані.

```js
assert.lengthOf(document.querySelectorAll('rect.bar'), GDPDataJson.data.length);
```

Кожна гістограма повинна мати атрибути `data-date` і `data-gdp`, що містять значення дати та ВВП.
...
Атрибути `data-date` елементів гістограми повинні відповідати порядку наданих даних.
...
Атрибути `data-gdp` елементів гістограми повинні відповідати порядку наданих даних.
...
Висота кожного елемента гістограми повинна точно представляти відповідне значення ВВП у даних.
...
Атрибут `data-date` і відповідний елемент гістограми повинні бути вирівняні з відповідним значенням на осі x. Ви можете використати `d3.scaleTime()` для створення часової шкали для осі x.
...
Атрибут `data-gdp` і відповідний елемент гістограми повинні бути вирівняні з відповідним значенням на осі y. Ви можете використати `d3.scaleLinear()` для створення лінійної шкали для осі y.
...
При наведенні курсору на область ваша гістограма повинна показувати підказку з відповідним `id="tooltip"`, яка відображає більше інформації про цю область.
...
Підказка повинна мати атрибут `data-date`, який відповідає `data-date` активної області.
