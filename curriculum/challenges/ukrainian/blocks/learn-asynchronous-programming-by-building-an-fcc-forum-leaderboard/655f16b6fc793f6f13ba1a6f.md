...
У вікні попереднього перегляду ви маєте бачити стовпець ком. Щоб це виправити, слід додати метод `join` до вашого методу `map`. Для роздільника передайте порожній рядок.

# --hints--

Слід додати метод `join` до методу `map`.

```js
assert(/topics\s*\.\s*map\s*\(\s*(\(\s*item\s*\)|item)\s*=>\s*{[\s\S]*?}\s*\)\s*\.\s*join\s*\(\s*.*?\s*\)/.test(code));
```

Для роздільника потрібно передати порожній рядок.
