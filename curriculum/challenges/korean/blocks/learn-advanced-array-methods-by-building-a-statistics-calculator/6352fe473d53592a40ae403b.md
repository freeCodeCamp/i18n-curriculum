...
빈 `.reduce()` 콜백 안에서 `difference` 변수를 선언하고 `el`에서 `mean`을 뺀 값으로 설정하세요. 그런 다음 `squared` 변수를 선언하고 `difference`의 `2` 제곱 값으로 설정하세요. 마지막으로 `acc`과 `squared`의 합을 반환하세요.

# --hints--

`reduce` 콜백에는 `difference` 변수가 있어야 합니다.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

`difference` 변수는 `el`에서 `mean`을 뺀 값으로 설정해야 합니다.
...
`reduce` 콜백에는 `squared` 변수가 있어야 합니다.
...
`squared` 변수는 `difference`의 2제곱 값으로 설정해야 합니다.
...
`reduce` 콜백은 `acc`와 `squared`의 합을 반환해야 합니다.
...
`0`의 초기값은 `.reduce()` 메서드에서 제거하지 마세요.
