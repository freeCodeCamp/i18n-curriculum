...
`margin` プロパティを使って `.penguin` 要素を水平方向に中央揃えにし、`margin-top` を `75px` に設定してください。

# --hints--

`.penguin` に `margin` の `--fcc-expected--` を設定する必要がありますが、`--fcc-actual--` が見つかりました。
...
`.penguin` に `margin-top` の `--fcc-expected--` を設定する必要がありますが、`--fcc-actual--` が見つかりました。
