...
接下來，創建一個 `const` 變量，名爲 `forumCategoryUrl`，並給其賦值字符串 `"https://forum.freecodecamp.org/c/"`。

在其下方，創建另一個 `const` 變量，名爲 `avatarUrl`，並給其賦值字符串 `"https://sea1.discourse-cdn.com/freecodecamp"`。

# --hints--

你應該用 `const` 聲明一個 `forumCategoryUrl` 變量。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=/);
```

你應該將 `"https://forum.freecodecamp.org/c/"` 賦值給你的 `forumCategoryUrl` 變量。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=\s*('|")https:\/\/forum\.freecodecamp\.org\/c\/\1\s*/);
```

你應該使用 `const` 聲明一個 `avatarUrl` 變量。

```js
assert.match(code, /const\s+avatarUrl\s*=/);
```

你應該將 `"https://sea1.discourse-cdn.com/freecodecamp"` 賦值給 `avatarUrl` 變量。
