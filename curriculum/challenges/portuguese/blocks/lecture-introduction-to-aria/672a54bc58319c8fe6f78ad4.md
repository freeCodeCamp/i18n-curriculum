...
É importante lembrar que o `aria-hidden` apenas oculta o conteúdo da tecnologia assistiva, como leitores de tela. Se o conteúdo deve ser ocultado de todos, então você não deve usar `aria-hidden` para escondê-lo. Por exemplo, um menu hambúrguer que está atualmente recolhido deve ser ocultado de todos os usuários de teclado, não apenas dos usuários de leitor de tela. Nesse caso, você pode definir a propriedade CSS `display` como `none` no menu para removê-lo da página renderizada e da árvore de acessibilidade quando estiver recolhido.
...
Nesses três casos, o elemento já está oculto da árvore de acessibilidade, então o atributo `aria-hidden` não é necessário.
...
Em geral, todo o conteúdo e funcionalidade disponíveis na página também devem estar disponíveis para pessoas que usam tecnologia assistiva. O uso do `aria-hidden` é muito restrito e deve ser limitado principalmente a tornar a experiência mais limpa para usuários de leitor de tela, removendo informações puramente decorativas ou duplicadas. Não use `aria-hidden` para ocultar conteúdo que você acha que os usuários de leitor de tela teriam interesse. Usuários de leitor de tela merecem ter acesso a todas as informações da página.
