...
`initializeEventListeners` メソッドの中で、`click` に `singleCardBtn` イベントリスナーを取り付けてください。イベントハンドラ内で、`singleCardSelected` メソッドを呼び出してください。`singleCardBtn` から `elements` を取得することを忘れないでください。

このメソッドは今後のステップで作成します。

# --hints--

`click` 要素に `singleCardBtn` イベントを取り付ける必要があります。ここで `this` キーワードが必要なことを忘れないでください。

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

イベントのコールバック内で、`singleCardSelected` メソッドを呼び出すべきです。
