...
`map()` 方法用於迭代陣列並傳回一個新的陣列。當你想要根據現有陣列的值創建一個新的陣列時，它非常有用。例如：
...
請注意，`map()` 方法會將一個函式作為引數。這稱為 <dfn>回呼函式</dfn>，是指作為引數傳遞給另一個函式的函式。在上述範例中，回呼函式是 `(number) => number * 2`，並且會在 `numbers` 陣列中的每個元素上執行。接著，`map()` 方法會傳回一個包含結果的新陣列。

將回呼函式傳入 `map()` 方法。回呼函式應該以 `song` 作為參數，使用箭頭函式語法，且函式主體為空。

# --hints--

你的 `map()` 方法的回呼函式應該使用箭頭函式語法。

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

你的 `map()` 方法的回呼函式中應該有一個 `song` 參數。
