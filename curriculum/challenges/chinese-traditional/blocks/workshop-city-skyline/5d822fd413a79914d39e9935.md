...
爲這座建築添加另一個重複漸變；使其與你剛剛添加的相同，但不要添加 `90deg` 方向，而且是使用其窗口顏色而不是兩個 `transparent` 顏色。

# --hints--

你應該在 `background` 屬性中爲 `.fb6` 添加第二個 `repeating-linear-gradient`。
...
你應該將從 `0%` 到 `10%` 的 `--building-color3` 作爲第二個 `repeating-linear-gradient` 的第一種顏色。
...
你應該將從 `10%` 到 `30%` 的 `--window-color3` 作爲第二個 `repeating-linear-gradient` 的第二種顏色。
