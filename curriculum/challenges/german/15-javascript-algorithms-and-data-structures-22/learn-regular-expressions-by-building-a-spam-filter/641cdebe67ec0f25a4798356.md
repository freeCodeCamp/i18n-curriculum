...
Beende danach die Ausführung der Funktion.

# --hints--

Deine Callback-Funktion sollte eine `if`-Anweisung haben.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

Deine `if`-Anweisung sollte prüfen, ob die `value` von `messageInput` ein leerer String ist.
...
Deine `if`-Anweisung sollte die Ausführung der Funktion beenden.
