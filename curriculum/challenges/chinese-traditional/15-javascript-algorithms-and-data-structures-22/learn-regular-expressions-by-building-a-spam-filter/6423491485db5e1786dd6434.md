...
你的最終正則表達式將會查找類似 `dear friend` 的字符串。 聲明一個 `dearRegex` 並賦值一個匹配字符串 `dear friend` 的正則表達式。 記得忽略大小寫，並將其添加到你的 `denyList` 數組中。

# --hints--

你應該使用 `const` 來聲明你的 `dearRegex` 變量。

```js
assert.match(code, /const\s+dearRegex\s*=/);
```

你的 `dearRegex` 變量應該被賦值爲一個正則表達式。
