...
Ni wakati wa kuona validator ikamata tatizo halisi. `catalog[0]` ni "From a Buick 8" na Stephen King, ambayo ina sehemu zote nne zimejazwa. `catalog[16]` ni "Unknown Title", ambayo sehemu ya mwandishi ni tupu.

Andika `validateEntry(catalog[0])` na `validateEntry(catalog[16])`. Ya kwanza inapaswa kurudisha `true`; ya pili inapaswa kurudisha `false`.

# --hints--

Unapaswa kuandika `validateEntry(catalog[0])` kwenye konsoli.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

Unapaswa kuandika `validateEntry(catalog[16])` kwenye konsoli.
