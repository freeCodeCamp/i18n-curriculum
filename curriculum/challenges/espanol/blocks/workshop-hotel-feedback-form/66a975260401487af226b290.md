...
Para hacer un `option` seleccionado por defecto, puedes añadir el atributo `selected` al elemento `option` que deseas que esté seleccionado.

Aquí hay un ejemplo usando el atributo `selected`:

```html
<option selected value="amazing">Amazing</option>
```

Dentro de tu elemento `select`, añade el atributo `selected` al elemento `option` con el valor de `"excelente"`.

# --hints--

Tu elemento `option` con el valor de `"excelente"` debería tener el atributo `selected`.
