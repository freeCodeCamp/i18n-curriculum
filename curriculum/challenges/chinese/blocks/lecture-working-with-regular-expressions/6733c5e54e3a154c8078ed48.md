...
这确实有效：它会匹配四个数字字符。但你不必多次写出相同的类，可以给它添加一个量词。
...
注意，你不能仅用这种语法设置最大值：你必须总是设置最小值。但如果你将最小值设为 `1`，实际上可以达到相同的效果。
...
但实际上有一个针对单个可选字符的特殊简写量词，即问号（`?`）。让我们用问号替换我们的量词：
...
但我们的模式又变长了。幸运的是，还有另一个简写，表示“匹配前一个字符零次或多次”，即星号（`*`）符号。让我们用它替换模式中的量词，并进行测试：
...
现在我们成功匹配了任意以零个或多个字母开头，后跟四到六个数字的标识符。但事实证明这导致系统崩溃：我们实际上必须至少要求一个字母。

同样，我们可以使用最小值为一且没有定义最大值的量词，或者使用另一个特殊语法，加号（`+`）符号：
