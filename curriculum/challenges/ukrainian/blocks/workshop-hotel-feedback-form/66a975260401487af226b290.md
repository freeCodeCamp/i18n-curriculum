...
Щоб зробити `option` вибраним за замовчуванням, ви можете додати атрибут `selected` до елемента `option`, який хочете вибрати.

Ось приклад із використанням атрибута `selected`:

```html
<option selected value="amazing">Amazing</option>
```

Всередині вашого елемента `select` додайте атрибут `selected` до елемента `option` зі значенням `"excellent"`.

# --hints--

Ваш елемент `option` зі значенням `"excellent"` має мати атрибут `selected`.
