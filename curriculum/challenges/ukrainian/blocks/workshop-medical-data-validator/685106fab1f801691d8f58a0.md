...
Додайте ще один ключ `gender` до словника `constraints`. Відповідно до формату виразу, який ви написали на попередніх кроках, перевірте, що `gender` є рядком. Потім використайте оператор `and`, щоб перевірити, що рядок у нижньому регістрі `gender` міститься в `('male', 'female')`.

# --hints--

Ваш словник `constraints` має містити ключ `gender`.
...
Ключ `gender` вашого словника `constraints` має мати значення `isinstance(gender, str) and gender.lower() in ('male', 'female')`.
