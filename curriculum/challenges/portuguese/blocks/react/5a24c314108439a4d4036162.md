...
Existem duas maneiras de criar um componente React. A primeira maneira é usar uma função JavaScript. Definir um componente dessa forma cria um *stateless functional component*. O conceito de estado numa aplicação será abordado em desafios posteriores. Por enquanto, pense em um componente stateless como aquele que pode receber dados e renderizá-los, mas não gerencia nem rastreia mudanças nesses dados. (Cobriremos a segunda forma de criar um componente React no próximo desafio.)
...
Depois de ser transpiled, o `<div>` terá uma classe CSS `customClass`.
...
**Nota:** O texto é considerado um filho do elemento `div`, então você não poderá usar uma tag self-closing.
