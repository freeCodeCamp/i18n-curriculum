...
Dovresti creare un nuovo elemento `div` alla fine del tuo elemento `.daily-value`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'div');
```

Il tuo nuovo elemento `div` dovrebbe avere l'attributo `class` impostato su `divider`.
