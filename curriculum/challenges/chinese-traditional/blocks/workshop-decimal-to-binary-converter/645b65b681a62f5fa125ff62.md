...
使用 `.push()` 將 `quotient` 變量附加到 `quotients` 數組。 另外，將 `remainder` 變量附加到 `remainders` 數組。

# --hints--

你應該在 `while` 循環中的 `quotients` 數組上使用 `.push()` 方法。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*quotients\.push\(/);
```

你應該使用 `.push()` 方法將 `quotient` 附加到 `quotients` 數組。

```js
assert.match(String(decimalToBinary), /quotients\.push\(\s*quotient\s*\)\s*;?/);
```

你應該在 `while` 循環中的 `remainders` 數組上使用 `.push()` 方法。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*remainders\.push\(/);
```

你應該使用 `.push()` 方法將 `remainder` 附加到 `remainders` 數組。
