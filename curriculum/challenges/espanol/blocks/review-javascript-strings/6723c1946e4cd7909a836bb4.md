...
- **ASCII**: ASCII (Código Estándar Americano para el Intercambio de Información) es un estándar de codificación de caracteres usado para representar caracteres básicos en inglés mediante valores numéricos. Lecciones anteriores introducen `charCodeAt()` y `fromCharCode()` usando ejemplos de ASCII.
- **Unicode**: Las cadenas de JavaScript usan Unicode internamente, específicamente la codificación UTF-16. Para los primeros 128 caracteres (letras latinas básicas, dígitos y símbolos comunes), los valores Unicode coinciden con los códigos ASCII. Por eso los ejemplos basados en ASCII siguen funcionando en JavaScript.
- **El método `charCodeAt()`**: Este método devuelve la unidad de código UTF-16 del carácter en un índice especificado. Para caracteres latinos básicos, este valor coincide con el código ASCII.
