...
`.foot` 要素をターゲットにして、`width` の `15%`、`height` の `30%`、`background-color` の `orange` を与えてください。
...
`.foot` に `width` の `--fcc-expected--` を与えるべきです。`--fcc-actual--` にあります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.width, '15%');
```

`.foot` に `height` の `--fcc-expected--` を与えるべきです。`--fcc-actual--` にあります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.height, '30%');
```

`.foot` に `background-color` の `--fcc-expected--` を与えるべきです。`--fcc-actual--` にあります。
