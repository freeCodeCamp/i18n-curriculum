...
마지막으로, 현재 노드의 오른쪽 자식에 `_inorder_traversal` 메서드를 재귀적으로 호출하세요.

이 재귀 호출은 중위 순서 방식으로 오른쪽 부분 트리 전체를 탐색합니다.

# --hints--

`_inorder_traversal`을 호출할 때 `node.right`와 `result`을 인수로 전달해야 합니다.
