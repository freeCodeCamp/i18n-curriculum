...
右目を `position` プロパティに `absolute`、`top` に `54px`、`left` に `134px` を設定して位置を調整してください。

# --hints--

`.cat-right-eye` セレクターには `position` プロパティを `absolute` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.position, 'absolute')
```

`.cat-right-eye` セレクターには `top` プロパティを `54px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.top, '54px')
```

`.cat-right-eye` セレクターには `left` プロパティを `134px` に設定してください。
