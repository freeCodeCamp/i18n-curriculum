...
أضف خاصية `value` بقيمة `player.imageUrl` إلى `input` الخاص برابط الصورة (Image URL). ثم اضبط الخاصية `onChange` على دالة سهمية تأخذ `e` كمعلمة لها وتستدعي `setPlayer` مع نشر `player` وضبط `imageUrl` على `e.target.value`.

# --hints--

يجب أن يحتوي `input` الخاص بعنوان صورة اللاعب على `value` يساوي عنوان صورة اللاعب.
...
يجب أن يحتوي `input` الخاص برابط الصورة (Image URL) على معالِج `onChange` يستدعي `setPlayer` مع نشر `player` وضبط `imageUrl` على `e.target.value`.
