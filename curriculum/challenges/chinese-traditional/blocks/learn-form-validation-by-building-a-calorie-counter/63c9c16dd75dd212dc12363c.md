...
現在宣告一個 `lunchCalories` 變數，並將呼叫 `getCaloriesFromInputs` 並帶入你的 `lunchNumberInputs` 的結果指定給它。

# --hints--

你的 `calculateCalories` 函式應該有一個 `lunchCalories` 變數。

```js
assert.match(calculateCalories.toString(), /lunchCalories\s*=/);
```

你應該以 `lunchNumberInputs` 作為引數呼叫 `getCaloriesFromInputs`。

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*lunchNumberInputs\s*\)/);
```

你應該將 `getCaloriesFromInputs` 的結果指定給 `lunchCalories`。
