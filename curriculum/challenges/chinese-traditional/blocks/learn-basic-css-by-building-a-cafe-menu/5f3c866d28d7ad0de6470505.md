...
口味和價格目前堆疊在一起，並且與各自的 `p` 元素置中。如果口味在左邊，價格在右邊會比較好。

將類別名稱 `flavor` 添加到 `French Vanilla` 的 `p` 元素。

# --hints--

你應該為你的 `p` 元素添加 `flavor` 類別。
...
你的 `flavor` 類別應該放在帶有 `French Vanilla` 文字的 `p` 元素上。
