...
在 `.btn-container` 內，為每種故事類型創建三個按鈕。為每個按鈕指定一個 `btn` 類別和 `scary-btn`、`funny-btn` 及 `adventure-btn` 的 `id`。每個按鈕的文字分別應為 `Scary Story`、`Funny Story` 和 `Adventure Story`。

# --hints--

在 `.btn-container` 元素內，你應該有三個按鈕。
...
你應該有一個 `id` 為 `scary-btn` 且文字為 `Scary Story` 的按鈕。
...
你應該有一個 `id` 為 `funny-btn` 且文字為 `Funny Story` 的按鈕。
...
你應該有一個 `id` 為 `adventure-btn` 且文字為 `Adventure Story` 的按鈕。
