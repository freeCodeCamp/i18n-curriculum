...
請記住，`document.querySelectorAll` 返回 `NodeList`，它類似於數組，但不是數組。 但是，`Array` 對象有一個 `.from()` 方法，它接受類似數組的對象並返回一個數組。 當你想要訪問更強大的數組方法時這很有用，你將在未來的項目中瞭解這些方法。

以下示例採用 `li` 元素的 `NodeList` 並將其轉換爲 `li` 元素數組：
...
將你的 `inputContainers` 查詢選擇器包裝在 `Array.from()` 中。 在與聲明相同的行上執行此操作。

# --hints--

你應該將 `querySelectorAll` 包裝在 `Array.from()` 中。

```js
assert.match(clearForm, /Array\.from\(\s*document\.querySelectorAll\(\s*('|")\.input-container*\1\s*\)\s*\)/);
```

你應該將 `Array.from()` 的結果分配給 `inputContainers` 變量。
