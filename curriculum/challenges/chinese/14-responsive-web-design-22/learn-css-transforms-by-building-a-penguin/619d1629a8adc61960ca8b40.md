...
应该将 `.shirt` 的 `font-size` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontSize, '25px');
```

应该将 `.shirt` 的 `font-family` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontFamily, 'Helvetica, sans-serif');
```

应该将 `.shirt` 的 `font-weight` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
