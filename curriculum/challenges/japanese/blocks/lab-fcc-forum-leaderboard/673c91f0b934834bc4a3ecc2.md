...
1. 各`tr`要素には5つの`td`要素が含まれます：
   ...
   - `td`には`div`クラスの`avatar-container`要素が含まれ、`avatars`関数を`posters`と`users`の引数で呼び出して返された画像が含まれます。
...
`img` 関数が返す文字列内の各`avatars`要素は、投稿者の`alt`プロパティの値を持つ`name`テキストを持つべきです。
...
`td` が返す文字列の各テーブル行の2番目の`showLatestPosts`要素は、`avatars`関数を`posters`と`users`の引数で呼び出して返された画像を、`div`クラスの`avatar-container`要素内にネストして含みます。
