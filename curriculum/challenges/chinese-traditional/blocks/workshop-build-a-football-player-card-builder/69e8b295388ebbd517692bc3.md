...
還記得在 JSX 中，大括號讓你可以直接在 JSX 裡嵌入任何 JavaScript 表達式。這就是你如何從 props 顯示動態資料的方式。在 TSX 中也是一樣。

卡片標題仍然顯示硬編碼的值。將 `98` 裡的文字 `card-rating` 替換成 `player.overallRating`，並將 `ST` 裡的 `card-position` 替換成 `player.position`。

# --hints--

你應該將 `98` 中硬編碼的 `player.overallRating` 替換成 `.card-rating`。

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

你應該將 `ST` 中硬編碼的 `player.position` 替換成 `.card-position`。
