...
Aquí tienes un ejemplo de un elemento button. En el CSS, usaremos posicionamiento absoluto y el pseudo-elemento `::before` para agregar una estrella antes del texto `Learn More` del botón. Aprenderás más sobre posicionamiento absoluto en lecciones futuras.
...
En el pseudo-elemento `::after`, la propiedad `transition` se usa para animar cambios durante 0.3 segundos con un efecto de easing, creando una transformación suave y gradual en lugar de una repentina. Aprenderás más sobre la propiedad `transition` en lecciones futuras. 
...
Eso es lo que hace la propiedad `transform`: te permite rotar, sesgar, escalar o trasladar un elemento en una dirección particular. Aprenderás más sobre eso en lecciones futuras.

En el siguiente ejemplo, veremos el pseudo-elemento `::first-letter`. El pseudo-elemento `::first-letter` apunta a la primera letra del contenido de un elemento, permitiéndote estilizarla.  Here's an example of some paragraph text. Si queremos estilizar la primera letra, podemos utilizar el pseudo-elemento `::first-letter` de esta forma:
...
En el último ejemplo, veremos el pseudo-elemento `::marker`, que te permite seleccionar el marcador, viñeta o numeración de los elementos de lista para estilizar. El pseudo-elemento `::marker` ofrece una forma de mejorar la identidad de marca de tu sitio web personalizando los marcadores de lista para que coincidan con tu esquema de colores.

Aquí tienes un ejemplo de una lista desordenada y una lista ordenada. Para cambiar el color y tamaño del marcador del elemento de la lista, puedes usar el pseudo-elemento `::marker` así:
...
En esta lección, hemos cubierto solo algunos pseudo-elementos. Pero hay muchos más como `::placeholder`, `::spelling-error` y `::selection` que te animo a explorar por tu cuenta.
