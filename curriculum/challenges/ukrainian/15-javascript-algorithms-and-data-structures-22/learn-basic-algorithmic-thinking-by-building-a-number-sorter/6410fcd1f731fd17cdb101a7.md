...
Всередині вкладеного циклу `for` додайте виклик `console.log()`, щоб перевірити значення `array`, `array[j]` та `array[minIndex]` на кожній ітерації. Ви можете натиснути кнопку `Sort`, щоб побачити як ваш алгоритм обходить масив.

Потім напишіть інструкцію `if`, яка перевіряє, чи значення `array[j]` є меншим за значення `array[minIndex]`. Якщо так встановіть `minIndex` на `j`.

# --hints--

Вкладений цикл `for` повинен містити виклик `console.log()`.
...
Передайте `array` як перший аргумент до `console.log()`.
...
Передайте `array[j]` як другий аргумент до `console.log()`.
...
Передайте `array[minIndex]` як третій аргумент до `console.log()`.
...
Ви повинні мати інструкцію `if` після виклику `console.log()`.
...
Ви повинні мати умову в межах інструкції `if`, яка перевіряє, чи `array[j]` менше `array[minIndex]`.
...
Інструкція `if` повинна встановлювати `minIndex` на `j`.
