...
Verwenden Sie das `oninput`-Ereignis auf `shapeTypeSelect` und setzen Sie es auf `handleShapeSelect`. Dadurch wird die Funktion jedes Mal ausgeführt, wenn sich die Formauswahl ändert.

Nachdem Sie das gemacht haben, sollten Sie die Eingabefelder jeder Form sehen können. Die Berechnungen funktionieren jedoch noch nicht, und genau darum kümmern Sie sich als Nächstes.

# --hints--

Sie sollten `shapeTypeSelect.oninput` auf `handleShapeSelect` setzen.
