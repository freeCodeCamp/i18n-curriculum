...
Ili kuzuia hilo, ongeza sifa ya `disabled` ili kuzima kitufe cha kuwasilisha ikiwa `heroName`, `realName`, au `powerSource` ni za `falsy`, au ikiwa urefu wa `powers` ni `0`.
...
Unapaswa kuzima `button` ikiwa yoyote kati ya `heroName`, `realName`, na `powerSource` ni `false`, au ikiwa urefu wa `powers` ni `0`.
