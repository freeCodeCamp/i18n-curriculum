...
Aggiungi un `repeating-linear-gradient` a `.fb1c` usando `--building-color4` dallo `0%` al `10%` e `--window-color4` dal `10%` al `90%`. Si unirà al gradiente che hai già aggiunto.
...
Dovresti aggiungere un `repeating-linear-gradient` con `--building-color4` dallo `0%` al `10%` come primo colore.
...
Dovresti usare `--window-color4` dal `10%` al `90%` come secondo colore.
