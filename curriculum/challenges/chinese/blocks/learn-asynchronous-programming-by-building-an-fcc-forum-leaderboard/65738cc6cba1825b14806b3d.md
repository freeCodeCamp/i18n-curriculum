...
接下来，创建一个 `const` 变量，名为 `forumCategoryUrl`，并给其赋值字符串 `"https://forum.freecodecamp.org/c/"`。 

在其下方，创建另一个 `const` 变量，名为 `avatarUrl`，并给其赋值字符串 `"https://sea1.discourse-cdn.com/freecodecamp"`。

# --hints--

你应该用 `const` 声明一个 `forumCategoryUrl` 变量。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=/);
```

你应该将 `"https://forum.freecodecamp.org/c/"` 赋值给你的 `forumCategoryUrl` 变量。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=\s*('|")https:\/\/forum\.freecodecamp\.org\/c\/\1\s*/);
```

你应该使用 `const` 声明一个 `avatarUrl` 变量。

```js
assert.match(code, /const\s+avatarUrl\s*=/);
```

你应该将 `"https://sea1.discourse-cdn.com/freecodecamp"` 赋值给 `avatarUrl` 变量。
