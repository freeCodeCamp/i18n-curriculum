...
下一步是為負責將玩家移動穿越螢幕的功能添加機能。 

創建一個名為 `movePlayer` 的新箭頭函式（程式），該函式有三個參數，分別為 `key`、`xVelocity` 和 `isPressed`。 

# --hints--

你應該宣告一個 `movePlayer` 函式（程式）。

```js
assert.isFunction(movePlayer);
```

你的 `movePlayer` 函式（程式）應該接受 `key`、`xVelocity` 和 `isPressed` 的參數。

```js
assert.match(movePlayer.toString(), /\(\s*key\s*,\s*xVelocity\s*,\s*isPressed\s*\)/);
```

你的 `movePlayer` 函式（程式）應該是空的。
