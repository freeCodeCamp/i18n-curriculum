...
`form-row`の後に、`className`が`form-group`の`div`を追加してください。その中に、`className`が`label`、`htmlFor`が`club`、テキストが`Club`の`label`要素を追加してください。次に、`id`が`club`、`className`が`input`、`type`が`text`の`input`要素を追加してください。
...
`div.form-group`には、`htmlFor`属性が`club`、`className`が`label`、テキストが`Club`の`label`があるはずです。
...
Club用の`form-group`には、`id`が`club`、`className`が`input`、`type`が`text`の`input`が含まれているはずです。
