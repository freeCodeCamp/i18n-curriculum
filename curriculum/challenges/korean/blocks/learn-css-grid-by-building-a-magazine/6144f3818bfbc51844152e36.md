...
`minmax` 함수를 사용해 어떤 기기에서도 컬럼이 반응형이 되도록 만드세요. `minmax` 함수는 두 개의 인수를 받는데, 첫 번째는 최솟값이고 두 번째는 최댓값입니다. 이 값들은 길이, 백분율, `fr`, 또는 `max-content` 같은 키워드일 수 있습니다.

이미 정의한 `grid-template-columns` 속성의 각 값을 `minmax` 함수로 감싸고, 각 값을 두 번째 인수로 사용하세요. 첫 번째 인수는 각각 `2rem`, `min-content`, `2rem` 이어야 합니다.

# --hints--

`main` 선택자에 `grid-template-columns` 속성을 `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`로 설정해야 합니다.
