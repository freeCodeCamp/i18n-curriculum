...
または、キーと値のペアを含むマップイテレーターを返す`entries`メソッドを使って、マップオブジェクトを繰り返すこともできます。

`displayCatalog`関数を修正して、`catalog`マップのエントリーを繰り返し、それぞれのエントリーをコンソールにログしてください。

# --hints--

`displayCatalog`関数は`entries`で`catalog`メソッドを呼び出す必要があります。
