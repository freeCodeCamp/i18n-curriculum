...
`showLatestPosts()` 함수를 작성하면서 변경 사항을 확인하려면 호출해야 합니다.

`showLatestPosts()` 블록 끝에서 `try` 함수를 호출하고 인수로 `data`을 전달하세요.

# --hints--

`showLatestPosts()` 블록 끝에서 `try`를 호출해야 합니다.

```js
assert.match(code, /\s*showLatestPosts\(.*\)\s*;?\s*/);
```

`data`에 인수로 `showLatestsPosts()`을 전달해야 합니다.
