...
Ndani ya kila kipengele kinacholingana cha `label`, na mara moja baada ya kipengele cha `input`, ongeza nafasi na ongeza maandishi yafuatayo:
...
Unapaswa kumpa `label` ya kwanza maandishi ya `Personal`.

```js
assert.include(document.querySelector('fieldset:nth-child(2) > label')?.innerText, 'Personal');
```

Unapaswa kumpa `label` ya pili maandishi ya `Business`.
...
Unapaswa kumpa `label` ya pili maandishi nafasi moja mbele.
