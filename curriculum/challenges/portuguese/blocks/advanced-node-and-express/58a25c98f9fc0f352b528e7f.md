...
Voltando à seção de segurança da informação, você deve se lembrar que armazenar senhas em texto simples *nunca* é aceitável.Agora é a hora de implementar o BCrypt para resolver este problema.
...
Na estratégia de autenticação, verificamos o seguinte no código antes de concluir o processo: `if (password !== user.password) return done(null, false);`.Depois de fazer as alterações acima, `user.password` passa a ser um hash.Antes de fazer uma alteração no código existente, observe como a instrução está verificando se a senha **não** é igual e então retorna não autenticado.Com isso em mente, altere o código para que se pareça assim para verificar corretamente a senha inserida com relação ao hash:
...
Envie sua página quando você achar que ela está certa.Se você estiver encontrando erros, pode <a href="https://forum.freecodecamp.org/t/advanced-node-and-express/567135#hashing-your-passwords-1" target="_blank" rel="noopener noreferrer nofollow">verificar o projeto concluído até este ponto</a>.
