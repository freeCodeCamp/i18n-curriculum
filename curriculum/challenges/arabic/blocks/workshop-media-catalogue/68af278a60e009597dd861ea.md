...
يحتاج صنف `MediaCatalogue` إلى طريقة لإضافة عناصر الوسائط إلى الكتالوج. أنشئ طريقة `add` تستقبل عنصرًا `media_item` وتضيفه إلى قائمة `items`.

# --hints--

يجب أن يحتوي صنف `MediaCatalogue` على طريقة `add`.
...
يجب أن تحتوي طريقة `add` على معلمتين: `self` و `media_item`.
...
يجب أن تضيف طريقة `add` العنصر الممرر إليها إلى الخاصية `items`.
