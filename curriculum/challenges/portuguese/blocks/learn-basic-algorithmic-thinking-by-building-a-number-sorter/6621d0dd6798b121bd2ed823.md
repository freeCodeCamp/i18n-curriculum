...
Atualize seu callback `.map()` para chamar a função `Number()`.Passe `dropdown.value` para essa chamada de função.

Abra a aba Console para ver que seu `inputValues` é um array de números agora.

# --hints--

Você deve chamar a função `Number()` no seu callback `.map()`. 
...
Você deve passar `dropdown.value` para sua chamada `Number()`.
