...
Baadhi ya kadi za katalogi zina sehemu zilizokosekana. Kwa mfano, kipengele cha 16 kina mwandishi ambaye hana jina, hivyo `parseCard` kwa sasa inarudisha mfuatano wa herufi tupu kwa sehemu hiyo.

Kipengele cha `||` kinatoa njia mbadala: `a || b` huchambua kuwa `a` wakati `a` ni kweli, na `b` vinginevyo. Kwa kuwa mfuatano wa herufi tupu ni uongo, `"" || "Unknown"` huchambua kuwa `"Unknown"`.

Sahihisha `title`, `author`, na `location` katika taarifa ya return ili kutumia `|| "Unknown"` kama njia mbadala. Acha `year` kwa hatua inayofuata.

# --hints--

Wakati unapoitwa na mfuatano wa herufi wenye sehemu ya mwandishi tupu, sifa ya `author` ya kitu kinachorudishwa inapaswa kuwa `"Unknown"`.

```js
assert.strictEqual(parseCard("X |  | 2000 | Y").author, "Unknown");
```

Wakati unapoitwa na mfuatano wa herufi wenye sehemu ya kichwa tupu, sifa ya `title` ya kitu kinachorudishwa inapaswa kuwa `"Unknown"`.

```js
assert.strictEqual(parseCard(" | A | 2000 | Y").title, "Unknown");
```

Wakati unapoitwa na mfuatano wa herufi wenye sehemu ya eneo tupu, sifa ya `location` ya kitu kinachorudishwa inapaswa kuwa `"Unknown"`.

```js
assert.strictEqual(parseCard("X | A | 2000 | ").location, "Unknown");
```

Haujapaswa kuongeza njia mbadala kwa `year` bado.

```js
assert.strictEqual(parseCard("X | A |  | Y").year, "");
```

Wakati sehemu zote zipo, `parseCard` inapaswa kurudisha thamani zao halisi na si `"Unknown"`.
