...
ثم، أنشئ محدِّدًا جديدًا يستهدف عناصر قائمة التنقل بحيث عندما يحوم المؤشر فوقها، يتبدل لون الخلفية ولون النص، ويصبح المؤشر على شكل مؤشر يد.

# --hints--

يجب استخدام محدِّد `li > a` الموجود لضبط `text-decoration` إلى `none`.
...
يجب أن تُعطى عناصر `li` التي تحوم فوقها خاصية `cursor` بقيمة `pointer`.
