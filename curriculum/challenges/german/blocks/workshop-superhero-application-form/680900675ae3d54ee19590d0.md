...
Um den Fehler zu beheben, erstellen Sie eine `handlePowersChange` Pfeilfunktion mit einem `e` Parameter.

Innerhalb der Funktion destrukturieren Sie `value` und `checked` von `e.target`, um den Wert des Kontrollkästchens und dessen Aktivierungsstatus zu erhalten.

# --hints--

Sie sollten eine `handlePowersChange` Funktion mit einem `e` Parameter erstellen.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

Sie sollten `value` und `checked` von `e.target` destrukturieren.
