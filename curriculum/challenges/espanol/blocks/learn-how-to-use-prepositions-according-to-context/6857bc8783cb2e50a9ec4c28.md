...
`Keyboards` are hardware devices used to input text and commands into a computer. Por ejemplo:

`The office ordered new keyboards for every desk.` – This means each desk is getting one hardware device.

`Shipment` refers to a group of goods sent from one place to another. Por ejemplo:
