...
في الجزء الأخير من اللعبة، ستحتاج إلى إنشاء خوارزمية تتحقق من وجود تسلسل متتالي. التسلسل الصغير هو عندما يكون لدى أربعة من النرد قيم متتالية بأي ترتيب (مثال: في رمية `41423`، لدينا `1234`) مما يمنحك نتيجة قدرها `30` نقطة. التسلسل الكبير هو عندما يكون لدى جميع النرد الخمسة قيم متتالية بأي ترتيب (مثال: في رمية `35124`، لدينا `12345`) مما يمنحك نتيجة قدرها `40` نقطة.

أعلن دالة `checkForStraights` التي تقبل مصفوفة من الأرقام. إذا حصل المستخدم على تسلسل كبير، حدّث زر الاختيار الخامس ليعرض نتيجة `40`. إذا حصل المستخدم على تسلسل صغير، حدّث زر الاختيار الرابع ليعرض نتيجة `30`. وبغض النظر عن ذلك، يجب دائمًا تحديث زر الاختيار الأخير ليعرض نتيجة 0، مع الخصائص الصحيحة.

استدعِ دالة `checkForStraights` عند نقر زر `rollDiceBtn` لإكمال لعبتك بالنرد!

# --hints--

يجب أن تكون متغير `checkForStraights` دالة.

```js
assert.isFunction(checkForStraights);
```

إذا تم رمي small straight، يجب أن تُفعّل دالة `checkForStraights` زر الاختيار الرابع، وتضبط القيمة على `30`، وتحدّث النص المعروض إلى `, score = 30` مع ترك زر الاختيار الخامس معطلاً.
...
إذا تم الحصول على large straight، يجب أن تُفعّل دالة `checkForStraights` زر الاختيار الرابع، وتضبط القيمة على `30`، وتُحدّث النص المعروض إلى `, score = 30`. بالإضافة إلى ذلك، يجب أن تُفعّل الدالة زر الاختيار الخامس، وتضبط القيمة على `40`، وتُحدّث النص المعروض إلى `, score = 40`.
...
إذا لم يتم رمي أي تسلسل، يجب ألا تفعّل دالة `checkForStraights` زر الاختيار الرابع أو الخامس.
...
إذا لم يتم رمي أي تسلسل، يجب أن تُفعّل دالة `checkForStraights` زر الاختيار الأخير، وتعيّن القيمة إلى `0`، وتُحدّث النص المعروض إلى `, score = 0`.
...
يجب أن تستدعي دالة `checkForStraights` عند نقر زر `rollDiceBtn`.
