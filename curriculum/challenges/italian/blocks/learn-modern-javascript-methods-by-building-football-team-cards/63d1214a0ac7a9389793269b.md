...
All'interno di quell'array `players`, crea un nuovo oggetto con le seguenti proprietà:
...
_NOTA_: I numeri per la squadra sono organizzati in ordine alfabetico per cognome. Questo differisce dalla numerazione convenzionale, dove i numeri corrispondono a quelli sulle maglie dei giocatori.

# --hints--

Il tuo array `myFavoriteFootballTeam.players` dovrebbe avere come valore un oggetto.
...
La proprietà `name` dovrebbe essere impostata su `"Sergio Almirón"`.
...
La proprietà `position` dovrebbe essere impostata su `"forward"`.
...
La proprietà `number` dovrebbe essere impostata su `1`.
