...
現在、`currentContent` は空の文字列の値で初期化されています。しかし、望ましい結果は `currentContent` に現在のノートの文字列の値を保持させることです。

以前のレッスンで、`DOMContentLoaded` イベントを次のように扱う方法を学びました：
...
HTMLドキュメントのすべてが読み込まれ解析された後で、`noteEl` に安全にアクセスできるようになります。

まず、`addEventListener` オブジェクトに `window` メソッドを取り付けてください。`addEventListener` メソッドの最初の引数は `"DOMContentLoaded"` イベントである必要があります。2番目の引数はアロー関数にしてください。アロー関数の本体内で、`noteEl.textContent` の値を `currentContent` に代入してください。

# --hints--

`addEventListener` オブジェクトに `window` メソッドを取り付ける必要があります。

```js
assert.match(code, /window\.addEventListener/);
```

イベントリスナーは `"DOMContentLoaded"` イベントを監視する必要があります。

```js
assert.match(code, /window\.addEventListener\(['"]DOMContentLoaded['"]\,/);
```

アロー関数の本体内で、`noteEl.textContent` の値を `currentContent` に代入してください。
