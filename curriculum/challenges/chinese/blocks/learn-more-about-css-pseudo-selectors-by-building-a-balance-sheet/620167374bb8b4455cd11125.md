...
表格标题的最后一部分是年份。 创建一个 `#years` 选择器，并启用 flexbox。 将内容对齐到 flex 方向的末尾，并使元素具有粘性。 使用 `top: 0` 将其固定到其容器的顶部。
