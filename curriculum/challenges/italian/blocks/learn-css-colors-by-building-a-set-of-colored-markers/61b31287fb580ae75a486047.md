...
Nella regola CSS `.red`, aggiorna i valori delle proprietà `box-shadow` in modo che `offsetX` sia `0`,`offsetY` sia `0`, `blurRadius` sia `20px`, `spreadRadius` sia `0` e `color` sia `red`. Ricorda che non è necessario aggiungere unità a un valore zero.
