...
개발 중에 `fieldset` 요소 주위의 기본 테두리를 보는 것이 유용합니다. 하지만 원하는 스타일이 아닐 수도 있습니다.

`.question` 요소의 테두리와 아래쪽 패딩을 제거하세요.
