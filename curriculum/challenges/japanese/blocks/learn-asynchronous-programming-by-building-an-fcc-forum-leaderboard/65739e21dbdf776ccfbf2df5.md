...
`fetch` の呼び出しでエラーが発生した場合、`catch` ブロックがそれを処理します。

`catch` ブロックの中に、`console.log` パラメータをログに記録するための `err` を追加してください。

また、`console.log(data);` の呼び出しから返される内容がわかったので、`try` ブロックから `fetch` を削除してください。

# --hints--

`console.log` 変数をログに記録するには `err` を使うべきです。

```js
assert.match(code, /\s*console\.log\(\s*err\s*\)/);
```

`console.log` は `catch` ブロックの中にあるべきです。

```js
assert.match(code, /\s*try\s*{[^}]*\s*}\s*catch\s*\(\s*err\s*\)\s*{[^}]*\s*console\.log\(\s*err\s*\)[^}]*}/);
```

`console.log(data);` ブロックから `try` を削除してください。
