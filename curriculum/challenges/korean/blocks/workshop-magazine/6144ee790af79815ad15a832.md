...
이제 그리드 레이아웃을 구성할 준비가 되었습니다. 이전 수업에서 CSS Grid가 2차원 그리드 기반 레이아웃을 제공하여 항목을 수평과 수직으로 가운데 정렬하면서도 요소 겹치기 같은 작업을 제어할 수 있다는 것을 배웠습니다.

`main` 선택자를 만들고 `display` 속성을 `grid`로 설정하는 것부터 시작하세요.
...
`main` 선택자에 `display` 속성을 `grid`로 설정해야 합니다.
