...
يحتاج النموذج إلى مكان لعرض النتائج. أضف عنصر `div` فارغ وأعطه معرف `id` بقيمة `output` وقيم `class` هي `output` و `hide`.

# --hints--

يجب أن تضيف عنصر `div` بعد `form` الخاص بك.

```js
assert.exists(document.querySelector('.container > div'));
```

يجب أن يحتوي عنصر `div` الجديد على معرف `id` مضبوط على `output`.

```js
assert.equal(document.querySelector('.container > div')?.id, 'output');
```

يجب أن يحتوي عنصر `div` الجديد على قيم `class` هي `output` و `hide`.
...
يجب أن يكون عنصر `div` الجديد فارغًا.
