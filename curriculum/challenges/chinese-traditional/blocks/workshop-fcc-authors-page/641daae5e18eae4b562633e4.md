...
最後，如果發生錯誤且作者資料載入失敗怎麼辦？那麼你需要在使用者介面中顯示錯誤。這正是 `.catch()` 方法的用途－處理錯誤。

在 `.catch()` 內，移除 `console.error()` 並將 `innerHTML` 的 `authorContainer` 設為一個具有 `p` 屬性為 `class` 且文字為 `"error-msg"` 的 `"There was an error loading the authors"` 元素。
...
你應該存取 `innerHTML` 的 `authorContainer` 並將它設為一個 `p` 元素。別忘了用模板字面值包住 `p` 元素。
