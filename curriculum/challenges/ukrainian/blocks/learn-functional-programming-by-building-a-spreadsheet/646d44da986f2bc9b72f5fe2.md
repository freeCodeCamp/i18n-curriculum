...
Оголосіть функцію `toNumberList`, яка приймає параметр `args` та неявно повертає результат розділення `args` комами. Потім з’єднайте метод `map` до методу `split` та передайте `parseFloat` як аргумент методу `map`.
...
Приєднайте метод `.map()` до методу `.split()`.
...
Передайте посилання на `parseFloat` як зворотний виклик до `.map()`.
