...
将 `.eye-lid` 元素放置在其父元素顶部 `25%` 和左侧 `-23%` 的位置。 然后，将所有角的半径设置为 `50%`。

# --hints--

你应该将 `.eye-lid` 的 `top` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.top, '25%');
```

你应该将 `.eye-lid` 的 `left` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.left, '-23%');
```

你应该将 `.eye-lid` 的 `border-radius` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
