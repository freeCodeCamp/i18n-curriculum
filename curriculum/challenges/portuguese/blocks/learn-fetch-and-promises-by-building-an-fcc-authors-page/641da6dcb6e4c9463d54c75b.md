...
Dentro da string de template literals, crie um elemento `div` com o `id` definido como o `index` do método de array `.forEach()`. Lembre-se de usar a interpolação de strings para isso.

Além disso, adicione uma `class` de `"user-card"` ao `div`.
...
Você deve criar um elemento `div` com a classe `"user-card"`.
...
O elemento `div` deve ter o `id` `index`. Lembre-se de usar a interpolação de strings para definir o `id`.
...
O elemento `div` deve estar vazio.
