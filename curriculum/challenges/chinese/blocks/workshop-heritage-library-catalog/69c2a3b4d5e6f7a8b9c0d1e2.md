...
`kingBooks` 测试代码确认 `findByAuthor` 正常工作。在继续 `groupByDecade` 之前，清理控制台输出，将其移除。

删除 `const kingBooks = ...` 行、`console.log(kingBooks.length)` 和显示书名及年份的 `for` 循环。

# --hints--

应删除 `kingBooks` 变量、`console.log(kingBooks.length)` 语句和 `for` 循环。
