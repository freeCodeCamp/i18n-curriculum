...
<a href="https://rosettacode.org/wiki/Cramer%27s_rule" target="_blank" rel="noopener noreferrer nofollow">قاعدة كرامر</a> هي صيغة لحل نظام من المعادلات الخطية باستخدام المحددات لمصفوفات مكوّنة من مجموعات جزئية من المعاملات وقيم الطرف الأيمن.

يُعطى محدد المصفوفة ذات الصفين والعمودين بالصيغة التالية:

$\begin{aligned}|A|={\begin{vmatrix}a&b\\\c&d\end{vmatrix}}=ad-bc.\end{aligned}$

معطى نظام من المعادلات الخطية:
...
والذي يكون في صيغة المصفوفة كما يلي
...
يجب أن تكون `cramersRule` دالة.

```js
assert(typeof cramersRule === 'function');
```

يجب أن تُرجع `cramersRule([[2, -1, 5, 1], [3, 2, 2, -6], [1, 3, 3, -1], [5, -2, -3, 3]], [-3, -32, -47, 49])` القيمة `[2, -12, -4, 1]`.

```js
assert.deepEqual(cramersRule(matrices[0], freeTerms[0]), answers[0]);
```

يجب أن تُرجع `cramersRule([[3, 1, 1], [2, 2, 5], [1, -3, -4]], [3, -1, 2])` القيمة `[1, 1, -1]`.
