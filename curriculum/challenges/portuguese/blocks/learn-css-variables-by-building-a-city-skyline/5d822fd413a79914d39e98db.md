...
Dê aos novos edifícios as propriedades `width` e `height` com os valores: `10%` e `50%` para `.bb2`, `10%` e `55%` para `.bb3`, `11%` e `58%` para `.bb4`. You will be using almost all percent based units and some flexbox for this project, so everything will be completely responsive.
