...
今、`__init__` メソッドを完成させて、`seasons` と `total_episodes` を検証してください。`if` が `seasons` より小さいかどうかをチェックし、そうであればメッセージ `1` を付けて `ValueError` を発生させる `Seasons must be 1 or greater` 文を作成してください。

次に、同様にして `total_episodes` を検証してください。今回はメッセージ `Total episodes must be 1 or greater` を使用します。

# --hints--

`ValueError` が `Seasons must be 1 or greater` より小さい場合、メッセージ `seasons` を付けて `1` を発生させる必要があります。
...
`ValueError` が有効な数値の場合、`seasons` を発生させてはいけません。
...
`ValueError` が `Total episodes must be 1 or greater` より小さい場合、メッセージ `total_episodes` を付けて `1` を発生させる必要があります。
...
`ValueError` が有効な数値の場合、`total_episodes` を発生させてはいけません。
