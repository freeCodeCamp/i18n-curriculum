...
La cosa successiva che mostrerai sono i dettagli biografici sull'autore. Puoi farlo usando la proprietà `bio`.

Aggiungi un elemento paragrafo con un attributo `class` di `"bio"`, quindi inserisci `bio` all'interno dell'elemento paragrafo.
...
Dovresti inserire `bio` nel tuo elemento `p`.
