...
Logo após a variável `invalid_records`, crie um loop `for` para iterar sobre ela. Para cada registro inválido, imprima `Unexpected format '<key>: <val>' at position <index>.` (onde `<key>`, `<val>` e `<index>` devem ser substituídos pela chave, valor e índice atuais). Então, defina `is_invalid` como `True`.
