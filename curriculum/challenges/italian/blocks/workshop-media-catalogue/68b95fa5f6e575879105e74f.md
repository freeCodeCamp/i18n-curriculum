...
Ora che la classe `MediaError` è completa, modifica l’istruzione `if` nel metodo `add`. Sostituisci il `TypeError` con un `MediaError` usando lo stesso messaggio.

Inoltre, passa `media_item` come secondo argomento dopo il messaggio. Questo ti permetterà di archiviare l’elemento che causa l’errore nell’oggetto eccezione.

# --hints--

Dovresti sollevare un `MediaError` con il messaggio `Only Movie or TVSeries instances can be added` quando `media_item` non è un’istanza di `Movie` o delle sue sottoclassi.
...
Dovresti passare `media_item` come secondo argomento a `MediaError`.
...
Non dovresti sollevare un `MediaError` quando `media_item` è un elemento valido.
...
Il tuo metodo `add` non dovrebbe sollevare un `TypeError`.
