...
Дайте елементам `label` текст так, щоб `input` стояв перед текстом. Потім дайте елементам `input` `value`, який відповідає тексту.

Текст має бути або `True`, або `False`.
...
Ви повинні розмістити перший текстовий вміст `label` після елемента `input`.

```js
assert.match(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.innerHTML, />[\s\S]+[a-zA-Z]/);
```

Ви повинні розмістити другий текстовий вміст `label` після елемента `input`.

```js
assert.match(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.innerHTML, />[\s\S]+[a-zA-Z]/);
```

Ви повинні розмістити третій текстовий вміст `label` після елемента `input`.

```js
assert.match(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.innerHTML, />[\s\S]+[a-zA-Z]/);
```

Ви повинні розмістити четвертий текстовий вміст `label` після елемента `input`.
...
Ви повинні надати першому `input` `value`, який відповідає текстовому вмісту `label`.
...
Ви повинні надати другому `input` `value`, який відповідає текстовому вмісту `label`.
...
Ви повинні надати третьому `input` `value`, який відповідає текстовому вмісту `label`.
...
Ви повинні надати четвертому `input` `value`, який відповідає текстовому вмісту `label`.
