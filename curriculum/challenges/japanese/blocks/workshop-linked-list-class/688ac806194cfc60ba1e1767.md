...
`while` ループの後に、`if` が `current_node` かどうかを確認する `None` 文を作成してください。

これが起きると、連結リストを要素を見つけずに最後までたどったことになり、関数は実行を停止する必要があります。したがって、`return` の中に `if` キーワードを追加してください。

# --hints--

`if` が `current_node` かどうかを確認する `None` 文を作成し、その中に `return` キーワードを追加してください。
