...
변수 `tax`를 선언하고, `.calculateTaxes()`를 인수로 전달하여 새 `subTotal` 메서드를 호출한 값을 할당하세요.

# --hints--

`const`를 사용해 `tax`라는 변수를 선언하세요.
...
`.calculateTaxes()`를 인수로 전달하여 새 `subTotal` 메서드를 호출한 값을 `tax` 변수에 할당하세요. `this` 키워드를 사용하는 것을 잊지 마세요.
