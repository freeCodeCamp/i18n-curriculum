...
حان الوقت الآن لبدء ملء البيانات داخل `postsContainer`.

ابدأ باستدعاء الطريقة `map()` على مصفوفة `topics`. استخدم دالة سهمية فارغة كدالة استدعاء راجع تأخذ `item` كمعلمة.

ثم قم بتعيين نتيجة الطريقة `map()` إلى `postsContainer.innerHTML`.

# --hints--

يجب أن تستخدم الطريقة `map()` على مصفوفة `topics`.

```js
assert.match(code, /topics\.map\(.*/);
```

يجب أن تضيف دالة استدعاء راجع إلى الطريقة `map()` تحمل `item` كاسم للمعلمة.

```js
assert.match(code, /topics\.map\(\s*(\(\s*item\s*\)|item)\s*=>/g);
```

يجب أن يكون جسم دالة الاستدعاء الراجع فارغًا.

```js
assert.match(code, /topics\.map\(\s*(\(\s*item\s*\)|item)\s*=>\s*{\s*}/g);
```

يجب أن تعين نتيجة الطريقة `map()` إلى `postsContainer.innerHTML`.
