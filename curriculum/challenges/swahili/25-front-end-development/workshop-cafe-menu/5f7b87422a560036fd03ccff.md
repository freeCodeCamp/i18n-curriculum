...
Unapaswa kuwa na kipengele kimoja cha `p` chenye class ya `dessert`.

```js
assert.lengthOf(document.querySelectorAll('.dessert'), 1);
```

Kipengele chako cha `p` chenye maandishi `Donut` kinapaswa kuwa na darasa la `dessert`.

```js
assert.match(document.querySelector('.dessert')?.innerText, /donut/i);
```

Kipengele chako cha `p` chenye maandishi `1.50` kinapaswa kuwa na class ya `price`.
