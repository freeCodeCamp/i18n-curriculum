...
要取得上一首歌曲，請從 `userData?.songs` 的 `currentSongIndex` 中減去 `1`，並將結果指定給常數 `previousSong`。之後，呼叫 `playSong` 函式，並將 `previousSong.id` 作為引數傳入。
...
你應該將 `userData?.songs[currentSongIndex - 1]` 指定給一個 `previousSong` 常數。
...
你應該使用 `previousSong.id` 呼叫 `playSong` 函式（程式）。
