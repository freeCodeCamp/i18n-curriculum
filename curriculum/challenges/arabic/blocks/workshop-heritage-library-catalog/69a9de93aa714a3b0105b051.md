...
الآن اجمع كل الصفوف في سلسلة CSV واحدة يمكنك تصديرها أو مشاركتها.

بعد الحلقة الأولى، أعلن `let csv = header`. أضف حلقة `for` أخرى لإضافة كل صف إلى `csv` مع سطر جديد: `csv = csv + "\n" + rows[i]`. أرجع `csv`. ثم، أسفل الدالة، سجل `exportToCSV(catalog)` لمعاينة الناتج المنسق.

# --hints--

يجب أن تُرجع `exportToCSV` سلسلة نصية.

```js
assert.isString(exportToCSV(catalog));
```

يجب أن يكون السطر الأول من السلسلة المرجعة هو `"Title,Author,Year,Location"`.

```js
assert.strictEqual(exportToCSV(catalog).split("\n")[0], "Title,Author,Year,Location");
```

يجب أن يحتوي الناتج على `"King, Stephen"` (مقتبس للتعامل مع الفاصلة).

```js
assert.include(exportToCSV(catalog), '"King, Stephen"');
```

يجب أن تسجل `exportToCSV(catalog)` في وحدة التحكم.
