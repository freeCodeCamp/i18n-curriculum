...
`forEach` メソッドにコールバック関数を追加してください。その関数は `btn` パラメータを受け取る必要があります。次に、コールバック内で `btn` にイベントリスナーを追加してください。イベントリスナーは `click` イベントを監視し、`event` パラメータを持つ別のコールバックを受け取る必要があります。2つ目のコールバックは空の関数にしてください。

# --hints--

`forEach` メソッドに `btn` パラメータを受け取るコールバック関数をアロー構文で追加してください。

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(\s*(\(\s*btn\s*\)|btn)\s*=>\s*(?:\{|\s*)/);
```

`btn` 変数にイベントリスナーを追加してください。
...
イベントリスナーに空のコールバック関数を追加してください。`event` パラメータを与えることを忘れないでください。
