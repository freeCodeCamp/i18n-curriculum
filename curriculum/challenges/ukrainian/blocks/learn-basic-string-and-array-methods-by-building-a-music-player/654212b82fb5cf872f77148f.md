...
Всередині елемента `button` створіть ще два елементи `span`.

Перший елемент `span` повинен мати `class` зі значенням `playlist-song-artist`. Між тегами `span` додайте `${song.artist}`.

Другий елемент `span` повинен мати `class` зі значенням `playlist-song-duration`. Між тегами `span` додайте `${song.duration}`.

# --hints--

Ви не повинні змінювати існуючий елемент `button` та його вміст.
...
Ви повинні створити елемент `span` з класом `playlist-song-artist`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

Ваш елемент `span` з класом `playlist-song-artist` повинен інтерполювати `song.artist` як свій вміст.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

Ви повинні створити елемент `span` з класом `playlist-song-duration`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

Ваш елемент `span` з класом `playlist-song-duration` повинен інтерполювати `song.duration` як свій вміст.
