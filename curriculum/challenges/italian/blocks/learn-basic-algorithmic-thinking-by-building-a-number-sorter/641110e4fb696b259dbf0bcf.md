...
- Prima, non dovrebbe andare oltre l'inizio dell'array (consultato con `j`).
- Secondo, il loop non dovrebbe continuare dopo aver trovato un valore più piccolo del valore corrente.

Per evitare un loop infinito, decrementa `j` all'interno del tuo loop.
...
Il tuo loop `while` dovrebbe avere come prima condizione il controllo che il valore di `j` sia maggiore o uguale a `0`.
...
Il tuo loop `while` dovrebbe usare l'operatore AND.
...
Il tuo loop `while` dovrebbe avere una condizione che verifica che il valore di `array[j]` sia maggiore di `currValue`.
...
Il tuo loop `while` dovrebbe decrementare `j` all'interno del loop.
