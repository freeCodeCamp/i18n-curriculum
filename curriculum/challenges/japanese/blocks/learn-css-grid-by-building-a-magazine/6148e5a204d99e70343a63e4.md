...
`gap` プロパティは、`column-gap` と `row-gap` の値を同時に設定する省略記法です。1つの値が与えられた場合、`column-gap` と `row-gap` の両方にその値を設定します。2つの値が与えられた場合、`row-gap` に最初の値を、`column-gap` に2番目の値を設定します。

`.image-wrapper` セレクターに `gap` プロパティを `2rem` に設定してください。

# --hints--

`.image-wrapper` 要素に `gap` プロパティを `2rem` に設定してください。
