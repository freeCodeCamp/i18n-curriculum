...
title: "applica la proprietà flex-direction per creare righe nell'incorporamento del tweet"
challengeType: 0
...
I `header` e `footer` nell'esempio di incorporamento del tweet hanno elementi figli che potrebbero essere disposti come righe usando la proprietà `flex-direction`. Questa proprietà dice a CSS di allineare i figli orizzontalmente.

# --instructions--

Aggiungi la proprietà CSS `flex-direction` sia a `header` che a `footer` e imposta il valore su `row`.
...
Il `header` dovrebbe avere una proprietà `flex-direction` impostata su `row`.
