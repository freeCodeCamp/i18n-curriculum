...
為你的 `buyWeapon` 函式添加一個 `else` 述語。在該述語中，將 `text.innerText` 設定為相等於 `"You do not have enough gold to buy a weapon."`。

# --hints--

你應該為你的 `buyWeapon` 函式添加一個 `else` 述語。

```js
assert.match(buyWeapon.toString(), /else/);
```

你的 `else` 述語應該放在你的 `if` 述語之後。
...
你應該將 `text.innerText` 設定為 `"You do not have enough gold to buy a weapon."`。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You do not have enough gold to buy a weapon.\1/);
```

你的 `else` 述語應該將 `text.innerText` 設定為 `"You do not have enough gold to buy a weapon."`。
