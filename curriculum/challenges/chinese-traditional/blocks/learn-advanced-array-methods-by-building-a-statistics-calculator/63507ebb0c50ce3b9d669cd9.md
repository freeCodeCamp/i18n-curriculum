...
陣列方法通常可以串鏈在一起，以同時執行多個操作行為。舉例來說：

```js
array.map().filter();
```

在陣列上呼叫 `.map()` 方法，然後在 `.map()` 方法的結果上呼叫 `.filter()` 方法。這稱為方法串鏈。

依照該範例，移除你的 `filtered` 變數，並將你的 `.filter()` 呼叫串鏈到上方的 `.map()` 呼叫。不要移除任一個回呼函式。
