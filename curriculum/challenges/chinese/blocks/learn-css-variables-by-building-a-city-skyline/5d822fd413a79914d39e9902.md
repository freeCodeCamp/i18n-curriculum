...
最后，在 `.bb2a` 的 `border-bottom` 属性上，将 `1vw` 更改为 `5vh`，并将 `#000` 颜色更改为 `--building-color2` 变量。 好了，现在看起来不错！ 在整个项目中的任何时候，你都可以注释掉或删除你在开始时添加到所有内容的 `border` 属性，以查看删除它之后建筑物的外观。

# --hints--

你应该将 `border-bottom` 的值改为使用 `5vh`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderBottom, "5vh");
```

你应该将 `border-bottom` 的值改为使用 `--building-color2`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderBottom.trim(), "var(--building-color2)");
```

`border-bottom` 的值应该是 `5vh solid var(--building-color2)`。
