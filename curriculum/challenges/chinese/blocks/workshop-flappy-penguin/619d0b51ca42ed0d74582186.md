...
选择 `.eye` 元素，并将其 `width` 设置为 `15%`、`height` 设置为 `17%`，`background-color` 设置为 `black`。
...
你应该将 `.eye` 的 `width` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.width, '15%');
```

你应该将 `.eye` 的 `height` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.height, '17%');
```

你应该将 `.eye` 的 `background-color` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
