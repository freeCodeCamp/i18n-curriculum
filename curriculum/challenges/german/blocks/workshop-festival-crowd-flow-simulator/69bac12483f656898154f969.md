...
Verwenden Sie eine `for...in`-Schleife, um jedes Tor im Eingabe-`summary`-Objekt zu durchlaufen und die insgesamt verarbeiteten Teilnehmer zu protokollieren.

Pro Tor protokollieren Sie eine Nachricht in folgendem Format: `gate + ": " + summary[gate] + " attendees processed"`.

Als Beispiel sollten Sie für ein Eingabe-`summary`-Objekt, das als `{ North: 5, East: 3, South: 4, West: 2 }` definiert ist, Folgendes protokollieren:
...
Sie sollten jedes Element im `summary`-Objekt durchlaufen und für jedes Element einmal die folgende Nachricht protokollieren: `element + ": " + summary[element] + " attendees processed`.
