...
`matrix_list` はリストのリストで、各要素は `(x, y)` 座標を持っています。`rounded_coords` にある座標のリストを使って、`matrix_list` のリスト内の該当座標の要素を `PROJECTILE` 変数の記号に変更してください。座標グラフは左下隅に `(0, 0)` があることを覚えておいてください。
...
メソッドは異なるインスタンスに対しても正しい値を返す必要があります。
