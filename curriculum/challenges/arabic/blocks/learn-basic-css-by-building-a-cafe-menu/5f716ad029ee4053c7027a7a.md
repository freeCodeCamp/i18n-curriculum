...
قم بتسكين عنصرين `p` داخل عنصر `article` الخاص بك. يجب أن يكون نص العنصر الأول `Donut`، ونص العنصر الثاني `1.50`. ضع كلاهما في نفس السطر مع التأكد من عدم وجود مسافة بينهما.

# --hints--

لا تغير عنصر `article` الموجود لديك.

```js
assert.lengthOf(document.querySelectorAll('article'), 6);
```

يجب أن يحتوي عنصر `article` الجديد على عنصرين `p`.
...
يجب أن يحتوي عنصر `p` الأول على النص `Donut`.
...
يجب أن يحتوي عنصر `p` الثاني على النص `1.50`.
