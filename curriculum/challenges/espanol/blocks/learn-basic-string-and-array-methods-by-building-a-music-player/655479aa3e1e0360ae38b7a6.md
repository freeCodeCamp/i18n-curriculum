...
A continuación, necesitarás recuperar la siguiente canción en la lista de reproducción. Para ello, necesitarás obtener el índice de la canción actual y luego añadir `1` a él.

Crea una constante llamada `nextSong` y asígnale `userData?.songs[currentSongIndex + 1]`.

Por último, llama a la función `playSong` y pasa `nextSong.id` como argumento.

# --hints--

No deberías modificar la declaración `if` existente, su bloque `else` y contenido.
...
Deberías asignar `userData?.songs[currentSongIndex + 1]` a una constante `nextSong`.
...
Deberías llamar a la función `playSong` con `nextSong.id`.
