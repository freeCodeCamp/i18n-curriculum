...
سيؤدي هذا إلى طي منطقة النص السفلية إلى عمود واحد على الشاشات الأصغر.

# --hints--

يجب عليك إنشاء query للـ `@media` جديد لـ `only screen and (max-width: 600px)`. يجب أن يكون هذا تحت query الـ `@media` السابق.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

يجب أن يحتوي query الـ `@media` الجديد على منتقى `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

يجب أن يحتوي منتقي `.text-with-images` على خاصية `grid-template-columns` بقيمة `1fr`.
