...
Dentro do método `map`, desestruture as seguintes propriedades do objeto `item`:
...
Você deve desestruturar as propriedades `id`, `title`, `views`, `posts_count`, `slug`, `posters`, `category_id` e `bumped_at` do objeto `item`.
