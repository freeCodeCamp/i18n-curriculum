...
В межах умови`else` призначте рядок `"general"` до `selectedCategory.className`.

Нижче цього призначте рядок `"General"` до `selectedCategory.category`.

Наприкінці, призначте число `1` до `selectedCategory.id`.

# --hints--

Призначте рядок `"general"` до `selectedCategory.className`.

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

Призначте рядок `"General"` до `selectedCategory.category`.

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

Призначте число `1` до `selectedCategory.id`.
