...
عنصر `.card` يحتاج إلى المزيد من التنسيق: أضف `border-radius` بقيمة `10px`، و`box-shadow` بقيمة `0 4px 8px gray`.

# --hints--

يجب أن يحتوي محدِّد `.card` على خاصية `border-radius` بقيمة `10px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("border-radius"), "10px");
```

يجب أن يحتوي محدِّد `.card` على خاصية `box-shadow` بقيمة `0 4px 8px gray`.
