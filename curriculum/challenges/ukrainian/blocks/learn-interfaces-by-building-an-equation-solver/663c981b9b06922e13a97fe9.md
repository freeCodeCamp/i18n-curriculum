...
Створіть умову `elif` для випадку `n == 1`. Усередині умови `elif` створіть f-рядок, що містить коефіцієнт, безпосередньо за яким іде рядок у нижньому регістрі `x`, і додайте його до списку `terms`.

# --hints--

Ви маєте створити умову `elif`, щоб перевірити, чи дорівнює `n` `1`.
...
Ви маєте додати `f'{coefficient}x'` до списку `terms` усередині нової умови `elif`.
