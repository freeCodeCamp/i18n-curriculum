...
次に、freeCodeCamp News の著者ページへのリンクを追加してください。

`class` 属性に `"author-link"` を持つアンカー要素を追加し、`url` 属性の値として `href` を挿入し、`target` を `"_blank"` に設定してください。アンカー要素のテキストには、`author` に続けてテキスト `"'s author page"` を挿入してください。例えば、`"Quincy Larson's author page"` のようになります。
