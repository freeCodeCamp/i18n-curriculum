...
각 `span`에 `className`와 `stat-value`의 `stat-label`를 각각 가진 두 개의 `stat-row` 요소를 추가하세요.

# --hints--

두 번째 `.stat-row`의 각 `.stat-col`에는 `span` 클래스를 가진 `stat-value`와 `span` 클래스를 가진 `stat-label`가 포함되어야 합니다.
