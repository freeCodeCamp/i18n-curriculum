...
在循环内部，查看当前条目的作者（转换为小写）是否包含 `searchTerm`。`.includes()` 方法如果字串包含另一个字串作为子串，则返回 `true`。如果条件为真，将该条目推入 `results`。循环结束后，返回 `results`。

# --hints--

`findByAuthor(catalog, "king").length` 应该是 `6`。

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

`findByAuthor(catalog, "asimov").length` 应该是 `2`。

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

`findByAuthor(catalog, "xyz").length` 应该是 `0`。
