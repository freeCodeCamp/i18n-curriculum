...
在 `displayAuthors` 函数内部，每个作者的 数据 已为你解构。

Now create an image tag and give it a `class` attribute of `"user-img"`.  Use string interpolation to set the `src` attribute to the `image` property. 将 `alt` 属性设置为 `author`，后跟文本 `"avatar"`。 确保 `author` 变量和单词 `"avatar"` 之间有一个空格，例如 `"Quincy Larson avatar"`。
