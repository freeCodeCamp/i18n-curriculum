...
Tu diccionario `constraints` debe tener una clave `gender`.
...
La clave `gender` de tu diccionario `constraints` debe tener el valor de `isinstance(gender, str) and gender.lower() in ('male', 'female')`.
