...
在上一个项目中，你学习了如何使用连接运算符将变量插入到字符串中，如下所示：
...
将字符串 `"You now have a new weapon."` 更新为 `"You now have a "`，后跟新武器的名称，并记得以句号结束句子。


# --hints--

你应该更新 `text.innerText` 分配以以字符串 `"You now have a "` 开头。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1/);
```

你应该使用连接运算符将 `newWeapon` 添加到 `text.innerText` 字符串的末尾。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1\s*\+\s*newWeapon/);
```

你应该使用连接运算符以 `.` 结束 `text.innerText` 字符串。
