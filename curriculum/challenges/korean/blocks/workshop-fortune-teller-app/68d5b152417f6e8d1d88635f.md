...
`Card` 인터페이스에 추가할 속성이 더 있습니다. `value_int`을 `number` 타입으로 설정하는 것부터 시작하세요. 나머지 모든 속성은 `string` 타입이어야 합니다: `suit`, `type`, `img`, `meaning_up`, `meaning_rev`, 그리고 `desc`.

# --hints--

`value_int` 속성이 `number` 타입이어야 합니다.
...
`meaning_up` 속성이 `string` 타입이어야 합니다.
...
`meaning_rev` 속성이 `string` 타입이어야 합니다.
