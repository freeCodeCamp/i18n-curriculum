...
عرّف دالة `pauseSong` باستخدام الكلمة المفتاحية `const` وصيغة الدالة السهمية. يجب ألا تأخذ الدالة أي معلمات.

# --hints--

استخدم `const` لإنشاء دالة فارغة باسم `pauseSong`.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

يجب أن تكون `pauseSong` دالة.

```js
assert.isFunction(pauseSong)
```

يجب أن تستخدم دالة `pauseSong` صيغة الدالة السهمية.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

يجب أن تكون دالة `pauseSong` فارغة.
