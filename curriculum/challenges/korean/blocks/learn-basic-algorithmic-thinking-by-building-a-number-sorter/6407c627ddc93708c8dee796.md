...
`outputValueNode`라는 변수를 만들고 `document.getElementById()` 메서드를 호출한 결과로 값을 설정하세요. 템플릿 리터럴 구문을 사용해 `` `output-value-${i}` `` string to `.getElementById()`를 전달하세요.

# --hints--

`const`를 사용해 `outputValueNode` 콜백 안에 `.forEach()` 변수를 선언하세요.

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=/)
```

`outputValueNode`에는 `document.getElementById()`를 호출한 값을 할당해야 합니다.

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=\s*document\.getElementById\s*\(/)
```

템플릿 리터럴 구문을 사용해 `output-value-${i}`에 `.getElementById()` 문자열을 전달하세요.
