...
У межах вашого `.image-wrapper` елемента надайте другому `img` елементу `src` зі значенням `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`, `alt` зі значенням `image of a calculator project`, атрибут `loading` зі значенням `lazy`, `class` зі значенням `image-2`, атрибут `width` зі значенням `400` та атрибут `height` зі значенням `400`.

# --hints--

Ваш другий `img` елемент повинен мати `src` зі значенням `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`.
...
Ваш другий `img` елемент повинен мати атрибут `alt` зі значенням `image of a calculator project`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('alt') === 'image of a calculator project');
```

Ваш другий `img` елемент повинен мати атрибут `loading` зі значенням `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('loading') === 'lazy');
```

Ваш другий `img` елемент повинен мати `class` зі значенням `image-2`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.classList?.contains('image-2'));
```

Ваш другий `img` елемент повинен мати `width` зі значенням `400`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('width') === '400');
```

Ваш другий `img` елемент повинен мати `height` зі значенням `400`.
