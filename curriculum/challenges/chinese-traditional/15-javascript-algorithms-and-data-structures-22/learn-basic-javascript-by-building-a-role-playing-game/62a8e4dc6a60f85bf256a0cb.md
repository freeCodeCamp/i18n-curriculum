...
JavaScript 中的 `Math` 對象包含數學常量和函數的靜態屬性和方法。 其中之一是 `Math.random()`，它生成一個從 `0`（含）到 `1`（不含）的隨機數。 另一個是 `Math.floor()`，它將給定的數字向下舍入爲最接近的整數。

使用這些，你可以生成一個範圍內的隨機數。 例如，這會生成一個介於 `1` 和 `5` 之間的隨機數：`Math.floor(Math.random() * 5) + 1;`。

按照這種模式，使用加法運算符（`+`）將 `1` 和 `xp` 的值之間的隨機數添加到 `monsterHealth -= weapons[currentWeaponIndex].power`。

# --hints--

你應該使用已經寫過的相同 `monsterHealth` 行。

```js
assert.lengthOf(attack.toString().match(/monsterHealth/g), 1);
```

你應該添加 `weapons[currentWeaponIndex].power` 值。
...
你應該使用 `Math.floor()` 將 `Math.random() * xp` 的結果向下舍入。
