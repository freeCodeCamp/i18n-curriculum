...
次に、`bugMap` という名前のオブジェクトを `Record<string, Bug<string>>` 型で定義し、今は空のオブジェクトを代入してください。

# --hints--

`bugMap` という定数を用意してください。
...
`bugMap` には `Record<string, Bug<string>>` の型注釈を付けてください。
