...
# --description--
`highPrecedence` 関数内で、`const` を使って変数を宣言し、`str` パラメータに渡された文字列が「数字の後に `*` または `/` の演算子が続き、さらに別の数字が続く」というパターンにマッチするかをチェックする正規表現を代入してください。

関数はブール値を返す必要があります。`test()` メソッドを使えることを覚えておいてください。

# --hints--

`highPrecedence` 関数内で正規表現用の変数を宣言してください。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*(?:const|let|var)\s+\w+/);
```

正規表現変数の宣言には `const` を使うべきです。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+\w+/);
```

正規表現変数には正規表現を代入してください。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+\w+\s*=\s*\//);
```

`highPrecedence` 関数はブール値を返す必要があります。

```js
assert.isBoolean(highPrecedence("12*2"));
```

`highPrecedence` 関数は、文字列が「数字の後に `*` または `/` の演算子が続き、さらに別の数字が続く」というパターンに正しくマッチするかをチェックしてください。
