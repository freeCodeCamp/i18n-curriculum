...
أضف خاصية `value` بقيمة `player.pac` إلى `input` الخاص بـ PAC. ثم أضف خاصية `onChange` مضبوطة على دالة سهمية تأخذ `e` كمعلمة وتستدعي `setPlayer` مع نسخ من `player` و`pac` مضبوطة على `Number(e.target.value)`.

# --hints--

يجب أن يحتوي `input` الخاص بـ PAC على خاصية `value` تساوي قيمة `pac` للاعب.
...
يجب أن يحتوي `input` الخاص بـ PAC على معالِج `onChange` يستدعي `setPlayer` مع نسخ من `player` و`pac` مضبوطة على `Number(e.target.value)`.
