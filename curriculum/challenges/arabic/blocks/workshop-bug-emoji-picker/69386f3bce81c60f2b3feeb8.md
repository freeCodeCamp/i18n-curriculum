...
احصل على عنصر `select` الذي يحمل المعرف `species` من DOM وخزنه في ثابت باسم `selectElement`. استخدم عامل التحقق من عدم وجود القيمة `!` لتأكيد أن العنصر ليس `null`

# --hints--

يجب أن يكون لديك ثابت باسم `selectElement`.
...
يجب أن يستعلم `selectElement` عن العنصر الذي يحمل المعرف `species` باستخدام `document.querySelector` مع تمرير نوع `HTMLSelectElement` كمعلمة نوع.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

يجب أن يستخدم `selectElement` عامل التحقق من عدم وجود القيمة (`!`).
