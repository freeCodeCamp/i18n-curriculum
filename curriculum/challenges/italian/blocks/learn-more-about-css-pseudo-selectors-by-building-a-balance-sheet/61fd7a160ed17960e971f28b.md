...
Nel tuo quarto elemento `tr`, aggiungi un elemento `th` con il testo `Total Assets`. Avvolgi il testo `Assets` in un elemento `span` con l'attributo `class` impostato su `sr-only`.

Dopodiché, aggiungi tre elementi `td` con il seguente testo (in ordine): `$579`, `$736`, `$809`. Dai al terzo elemento `td` un attributo `class` impostato su `current`.
...
Il tuo elemento `th` dovrebbe avere il testo `Total Assets`.

```js
assert(document.querySelector('tbody')?.querySelectorAll('tr')?.[3]?.querySelector('th')?.innerText === 'Total Assets');
```

Dovresti avvolgere il testo `Assets` in un elemento `span`.
