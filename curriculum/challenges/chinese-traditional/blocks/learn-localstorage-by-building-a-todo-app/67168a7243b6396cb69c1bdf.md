...
在你的 `taskObj` 中，對 `title` 和 `description` 屬性呼叫 `removeSpecialChars`。對於 `id` 屬性，僅對屬性值的 `titleInput.value` 零件呼叫它。
