...
Entferne das Standardstyling für die `.answers-list`-Elemente, indem du den Stil auf `none` setzt, und entferne das Padding von der unsortierten Liste.
...
Du solltest `.answers-list` ein `list-style` von `none` zuweisen.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.answers-list')?.listStyle, 'none');
```

Du solltest `.answers-list` ein `padding` von `0` zuweisen.
