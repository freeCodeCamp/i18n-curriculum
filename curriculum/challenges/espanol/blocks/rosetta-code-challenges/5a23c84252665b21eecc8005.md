...
title: Algoritmos de ordenamiento/Comb sort
challengeType: 1
forumTopicId: 302313
...
El **Comb Sort** es una variante del Bubble Sort.

Al igual que el Shell sort, el Comb Sort aumenta el espacio utilizado en las comparaciones e intercambios.

Dividir el espacio por $(1-e^{-\varphi})^{-1} \approx 1.247330950103979$ funciona mejor, pero 1.3 puede ser más práctico.

Algunas implementaciones usan el ordenamiento por inserción una vez que el gap es menor a una cierta cantidad.
...
Escribe una función que ordene un arreglo dado usando Comb sort.

# --hints--

`combSort` debería ser una función.

```js
assert(typeof combSort == 'function');
```

`combSort([25, 32, 12, 7, 20])` debería devolver un arreglo.

```js
assert(Array.isArray(combSort([25, 32, 12, 7, 20])));
```

`combSort([25, 32, 12, 7, 20])` debería devolver `[7, 12, 20, 25, 32]`.

```js
assert.deepEqual(combSort([25, 32, 12, 7, 20]), [7, 12, 20, 25, 32]);
```

`combSort([38, 45, 35, 8, 13])` debería devolver `[8, 13, 35, 38, 45]`.

```js
assert.deepEqual(combSort([38, 45, 35, 8, 13]), [8, 13, 35, 38, 45]);
```

`combSort([43, 36, 20, 34, 24])` debería devolver `[20, 24, 34, 36, 43]`.

```js
assert.deepEqual(combSort([43, 36, 20, 34, 24]), [20, 24, 34, 36, 43]);
```

`combSort([12, 33, 26, 18, 1, 16, 38])` debería devolver `[1, 12, 16, 18, 26, 33, 38]`.
...
`combSort([3, 39, 48, 16, 1, 4, 29])` debería devolver `[1, 3, 4, 16, 29, 39, 48]`.
