...
接下来，你需要创建一个函数来隐藏 DOM 中的元素。声明一个名为 `hideElements` 的函数，该函数接受一个名为 `...elements` 的参数，参数类型为 `HTMLElement[]`。

这个函数将使用一个 CSS `hidden` 类，在整个应用中用于切换 UI 元素的可见性。

# --hints--

你应该声明一个名为 `hideElements` 的函数。
...
你的 `hideElements` 函数应该有一个名为 `...elements` 的参数，参数类型为 `HTMLElement[]`。
