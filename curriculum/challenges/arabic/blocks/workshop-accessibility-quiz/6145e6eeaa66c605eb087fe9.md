...
أضف `id` إلى جميع عناصر `input` الخاصة بزر الاختيار حتى تربط المسميات بها. أعطِ الأول `id` بقيمة `q1-a1`. وأعطِ البقية `id`s بقيم `q1-a2`، و`q2-a1`، و`q2-a2` على التوالي.

# --hints--

يجب أن تعطي عنصر `input` الأول `id` بقيمة `q1-a1`.

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[0]?.id, "q1-a1");
```

يجب أن تعطي عنصر `input` الثاني `id` بقيمة `q1-a2`.

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[1]?.id, "q1-a2");
```

يجب أن تعطي عنصر `input` الثالث `id` بقيمة `q2-a1`.

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[2]?.id, "q2-a1");
```

يجب أن تعطي عنصر `input` الرابع `id` بقيمة `q2-a2`.
