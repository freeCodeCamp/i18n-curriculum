...
`stats-grid` 안에 `div` 요소 두 개를 각각 `form-group`의 `className`로 추가하세요.

첫 번째에는 `label`의 `className`와 `pac`의 `htmlFor`, 그리고 텍스트 `PAC`를 가진 `label`를 추가한 다음, `pac`의 `id`, `input`의 `className`, `number`의 `type`를 가진 `input`를 추가하세요.

두 번째에는 `sho`의 `htmlFor`와 텍스트 `SHO`를 가진 `label`를 추가한 다음, `sho`의 `id`와 `number`의 `type`를 가진 `input`를 추가하세요.

# --hints--

`div`의 `className` 안에 있는 `stats-grid` 안에 `form-group`의 `className`를 가진 `div` 요소 두 개가 있어야 합니다.
...
첫 번째 `form-group`의 `className` 안에 있는 `div` 요소는 `pac`의 `htmlFor` 속성을 가진 `label`와 `label`의 `className`, 그리고 텍스트 `PAC`를 가져야 합니다.
...
첫 번째 `form-group`의 `className` 안에 있는 `div` 요소는 `input`의 `className`를 가진 `input`, `pac`의 `id`, 그리고 `number`의 `type`를 가져야 합니다.
...
두 번째 `form-group`의 `className` 안에 있는 `div` 요소는 `sho`의 `htmlFor` 속성을 가진 `label`, `label`의 `className`, 그리고 텍스트 `SHO`를 가져야 합니다.
...
두 번째 `form-group`의 `className` 안에 있는 `div` 요소는 `input`를 가진 `input`와 `sho`의 `id`, `input`의 `className`, 그리고 `number`의 `type`를 가져야 합니다.
