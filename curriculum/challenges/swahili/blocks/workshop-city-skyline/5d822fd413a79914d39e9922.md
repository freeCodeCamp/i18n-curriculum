...
Toa kipengele cha `.fb3a` `width` ya `80%` na `height` ya `15%`. Kisha toa kipengele cha `.fb3b` `width` ya `100%` na `height` ya `35%`.

# --hints--

Unapaswa kumpa `.fb3a` `width` ya `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

Unapaswa kumpa `.fb3a` `height` ya `15%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

Unapaswa kumpa `.fb3b` `width` ya `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3b")?.width, "100%");
```

Unapaswa kumpa `.fb3b` `height` ya `35%`.
