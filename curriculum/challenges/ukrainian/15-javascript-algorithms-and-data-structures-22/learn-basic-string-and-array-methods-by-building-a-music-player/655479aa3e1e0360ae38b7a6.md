...
Далі вам потрібно буде отримати наступну пісню у списку відтворення. Для цього потрібно буде отримати індекс поточної пісні, а потім додати до нього `1`.

Створіть константу під назвою `nextSong` та призначте до неї `userData?.songs[currentSongIndex + 1]`.

Наприкінці, викличте функцію `playSong` та передайте `nextSong.id` як аргумент.

# --hints--

Не змінюйте наявну інструкцію `if`, її блок `else` та вміст.
...
Призначте `userData?.songs[currentSongIndex + 1]` до константи `nextSong`.
...
Викличте функцію `playSong` з `nextSong.id`.
