...
È ora di definire lo stile del pulsante di invio.

Seleziona l'elemento con la classe `submit-btn`. Imposta `cursor` su `pointer` (per indicare che è cliccabile), `background-color` su `royalblue`, `color` su `whitesmoke`, `border` su `none`, `border-radius` su `6px` e `padding` su `12px 20px`.

# --hints--

Dovresti mirare all'elemento con la classe `submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Imposta la proprietà `cursor` su `pointer`.
...
Imposta la proprietà `background-color` su `royalblue`.
...
Imposta la proprietà `border` su `none`.
...
Imposta la proprietà `border-radius` su `6px`.
...
Imposta la proprietà `padding` su `12px 20px`.
