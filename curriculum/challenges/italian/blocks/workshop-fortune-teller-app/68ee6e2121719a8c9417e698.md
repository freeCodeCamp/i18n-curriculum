...
Subito dopo aver cancellato `multipleCard`, all'interno del metodo `singleCardSelected`, chiama la funzione `showElements` e passa `singleCard` come primo argomento e `fortuneContainer` come secondo argomento.

# --hints--

Dovresti chiamare la funzione `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

Dovresti passare `elements.singleCard` come primo argomento.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

Dovresti passare `elements.fortuneContainer` come secondo argomento.
