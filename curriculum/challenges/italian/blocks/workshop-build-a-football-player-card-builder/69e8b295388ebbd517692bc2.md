...
In JSX e TSX, passi espressioni JavaScript come valori degli attributi usando le parentesi graffe. I template literal usano i segni di graduazione e `${}` per l'interpolazione. Per esempio:

```tsx
<div className={`prefix-${someVariable}`}>
```

Il contenitore della card `className` è attualmente codificato come `card-wrapper tier-gold`. Sostituisci `gold` con un valore dinamico usando un template literal che chiama `getPlayerTier` con `player.overallRating`. Questo farà riferimento alla classe tier corretta dal CSS, basata sul punteggio complessivo del giocatore.

# --hints--

Dovresti usare un template literal con `getPlayerTier(player.overallRating)` per il `className` di `.card-wrapper`.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

L'elemento `.card-wrapper` dovrebbe avere la classe `tier-elite` quando il punteggio del giocatore è 98.
