...
所有核心功能現已到位。 現在唯一的問題是，當前播放的歌曲結束時不會自動播放下一首歌曲。

爲了解決這個問題，你可以設置一個事件監聽器來檢測當前播放的歌曲何時結束。 `"ended"` 事件監聽器適合這種情況。 當媒體播放結束時會觸發此事件。

向 `audio` 元素添加一個事件監聽器，用於監聽 `"ended"` 事件。 使用帶有空花括號的箭頭語法傳入回調。

# --hints--

你應該將 `addEventListener()` 方法鏈接到你的 `audio` 變量。
...
你應該使用箭頭語法將空回調傳遞給你的 `"ended"` 事件監聽器。
