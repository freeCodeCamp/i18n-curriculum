...
마지막으로 선택할 요소들은 결과 요소입니다. `getElement` 함수를 사용해 `result-text`과 `result-card` 아이디를 가진 요소를 가져오세요. 두 요소 모두 `HTMLElement`를 타입으로 사용해야 합니다.

이미 `resultText`와 `resultCard` 변수를 선언했으니, 이제 할당만 하면 됩니다.

# --hints--

`resultText` 변수에 값을 할당해야 합니다.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText`는 `result-text` 요소를 선택해야 합니다.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

`HTMLElement`를 `getElement`가 `result-text` 요소를 선택할 때 타입으로 사용해야 합니다.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

`resultCard` 변수에 값을 할당해야 합니다.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard`는 `result-card` 요소를 선택해야 합니다.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

`HTMLElement`를 `getElement`가 `result-card` 요소를 선택할 때 타입으로 사용해야 합니다.
