...
在你的 `for` 迴圈之後，且仍在 `validate` 函式（程式）內，創建一個 `if` 述語。如果 `is_invalid` 是 `True`，傳回 `False`。

# --hints--

你應該在你的 `validate` 函式底部放置一個 `if` 述語。
...
當 `is_invalid` 為 `True` 時，你的 `if` 述語應該執行。
