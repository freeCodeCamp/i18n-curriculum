...
いよいよ `postsContainer` の中にデータを入れていきます。

まず `map()` 配列の `topics` メソッドを呼び出してください。コールバック関数には、`item` をパラメータに取る空のアロー関数を使います。

その後、`map()` メソッドの結果を `postsContainer.innerHTML` に代入してください。

# --hints--

`map()` 配列の `topics` メソッドを使う必要があります。

```js
assert.match(code, /topics\.map\(.*/);
```

`map()` メソッドに、パラメータ名が `item` のコールバック関数を追加してください。

```js
assert.match(code, /topics\.map\(\s*(\(\s*item\s*\)|item)\s*=>/g);
```

コールバック関数の本体は空にしてください。

```js
assert.match(code, /topics\.map\(\s*(\(\s*item\s*\)|item)\s*=>\s*{\s*}/g);
```

`map()` メソッドの結果を `postsContainer.innerHTML` に代入してください。
