...
垃圾訊息經常使用數字取代字母以繞過過濾器。你的正規表達式應該能捕捉到這些。

將你正規表達式中的 `e` 字元取代為符合 `e` 和 `3` 的字元類別。

# --hints--

你的 `freeRegex` 應該使用字元類別。

```js
assert.match(freeRegex.source, /\[.*\]/);
```

你的 `freeRegex` 應該使用字元類別來匹配 `e` 和 `3`。

```js
assert.match(freeRegex.source, /\[(e3|3e)\]/);
```

你的 `freeRegex` 應該使用三個字元類別來匹配 `e` 和 `3`。
