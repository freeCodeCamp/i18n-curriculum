...
title: Dimensiona le tue immagini
challengeType: 0
forumTopicId: 18282
...
Il CSS ha una proprietà chiamata `width` che controlla la larghezza di un elemento. Proprio come con i caratteri tipografici, useremo `px` (pixel) per specificare la larghezza dell'immagine.

Per esempio, se volessimo creare una classe CSS chiamata `larger-image` che desse agli elementi HTML una larghezza di 500 pixel, useremmo:
...
Crea una classe chiamata `smaller-image` e usala per ridimensionare l'immagine in modo che sia larga solo 100 pixel.
