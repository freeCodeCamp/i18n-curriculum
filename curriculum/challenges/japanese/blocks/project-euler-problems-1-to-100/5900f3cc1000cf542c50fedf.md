...
スドク（日本語で「数字の場所」の意味）は人気のあるパズルの名前です。その起源は不明ですが、類似した、より難しいパズルのアイデアであるラテン方陣を発明したレオンハルト・オイラーに功績が帰されます。スドクパズルの目的は、9×9のグリッドの空白（またはゼロ）を置き換え、各行、列、3×3のボックスに1から9までの数字がそれぞれ一度ずつ含まれるようにすることです。以下は典型的な開始パズルグリッドとその解答グリッドの例です。
...
よく作られたスドクパズルは一意の解があり、論理的に解くことができますが、選択肢を絞るために「推測と検証」方式を使う必要がある場合があります（この点については多くの議論があります）。探索の複雑さがパズルの難易度を決めます。上記の例は、単純な直接的な推論で解けるため、簡単とされています。

`puzzlesArr` 配列には難易度が異なる複数のスドクパズルの文字列が含まれていますが、すべて一意の解を持っています。

`puzzlesArr` のすべてのパズルを解くことで、各解答グリッドの左上隅にある3桁の数字の合計を求めてください。例えば、上記の解答グリッドの左上隅にある3桁の数字は 483 です。
