...
أضف حلقة `for` على `catalog`. داخل الحلقة، أعلن `const entry = catalog[i]`. أنشئ سلسلة نصية للصف تحتوي على الحقول الأربعة مفصولة بفواصل وادفعها إلى `rows`.

تحتاج الحقول النصية إلى علامات اقتباس مزدوجة حرفية في الناتج (مثل `"King, Stephen"`) حتى لا تكسر الفواصل داخل القيم تنسيق CSV. بما أن `year` رقم، فلا يحتاج إليها.

# --hints--

يجب استخدام حلقة `for` للتكرار على `catalog`.

```js
assert.match(exportToCSV.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

يجب أن يحتوي كل صف على حقول الإدخال مفصولة بفواصل.

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

يجب تغليف الحقول النصية بعلامات اقتباس مزدوجة.
