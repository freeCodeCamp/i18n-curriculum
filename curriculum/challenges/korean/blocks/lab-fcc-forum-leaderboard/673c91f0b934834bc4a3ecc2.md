...
API 엔드포인트는 다음과 같은 형태의 JSON을 반환합니다:
...
`avatar_template` 값은 상대 경로임을 주의하세요: `/`로 시작하며 픽셀 크기로 교체해야 하는 `{size}` 자리 표시자를 포함합니다. `avatarUrl` 상수는 경로가 상대적일 때 앞에 붙일 기본 URL입니다.
...
1. `timeAgo` 함수는 인수로 전달된 시간과 현재 시간 사이의 차이를 계산하여 다음을 반환해야 합니다:
   - `xm ago` (`x`는 분을 나타냄), 경과한 분이 `60` 미만일 때
   - `xh ago` (`x`는 시간을 나타냄), 경과한 시간이 `24` 미만일 때
   - 그렇지 않으면 `xd ago` (`x`는 일을 나타냄)
1. 게시물 조회 수를 인수로 받는 `viewCount`라는 함수를 만들어야 합니다.
1. 인수로 전달된 조회 수가 `1000` 이상이면 `viewCount` 함수는 조회 수를 `1000`로 나눈 후 내림하여 정수로 만들고 `k` 문자를 붙인 문자열을 반환해야 합니다. 그렇지 않으면 조회 수 값을 그대로 반환해야 합니다.
1. 선택된 카테고리의 id를 인수로 받는 `forumCategory`라는 함수를 만들어야 합니다.
1. `forumCategory` 함수는 선택된 카테고리 id가 `allCategories` 객체의 속성인지 확인하고 다음을 포함하는 앵커 요소 문자열을 반환해야 합니다:
   ...
   - `href`는 `<className>/<id>`을 `forumCategoryUrl`에 붙여 만든 문자열 (참고: `forumCategoryUrl`은 이미 `/`로 끝나므로 추가 `/` 구분자를 넣지 마세요), 여기서 `<className>`은 선택된 카테고리의 `className` 속성이고 `id`은 `forumCategory`에 전달된 인수입니다.
1. `allCategories` 객체에 선택된 카테고리 id가 속성으로 없으면 `category`은 `General`로 표시되고 `className`은 `general`로 표시되어야 합니다.
1. 게시자 배열과 사용자 배열 두 개를 인수로 받는 `avatars`라는 함수를 만들어야 합니다.
1. `avatars` 함수는 `img` 배열의 각 `user_id`에 대해 `posters` 요소를 연결한 문자열을 반환해야 합니다. `img` URL은 `user_id` 배열에서 `posters` 속성을 찾아서 `id` 배열에서 일치하는 `users` 속성을 찾음으로써 구합니다.
1. `avatars` 함수는 일치하는 사용자 객체(`avatar_template` 배열에서 `users`와 `id`를 `user_id`에서 비교하여 찾음)의 `posters` 속성에 접근하여 각 아바타 크기를 설정하고 URL 문자열의 `{size}` 자리 표시자를 `30`로 교체해야 합니다.
1. 각 이미지 요소는 게시자의 `name` 속성 값을 alt 텍스트로 가져야 합니다.
1. 각 `img` 요소는 일치하는 사용자의 `src` 값을 가진 `avatar_template` 속성을 가져야 합니다. `avatar_template`가 `/`로 시작하면 `avatarUrl`를 바로 앞에 붙이세요.
1. 단일 매개변수를 받는 `showLatestPosts`라는 함수를 만들어야 합니다.
1. `showLatestPosts`는 인수로 전달된 객체에서 `users`과 `topic_list` 속성을 추출해야 합니다. 또한 `topics`에 포함된 `topic_list` 배열의 객체에서 다음 속성을 처리해야 합니다:
   - `id`: 게시물 id
   - `title`: 게시물 제목
   - `views`: 게시물 조회 수
...
1. `showLatestPosts`는 `#posts-container`의 내부 HTML을 `tr`의 각 항목에 대해 `topics` 요소를 연결한 문자열로 설정해야 합니다.
1. 각 `tr` 요소는 다섯 개의 `td` 요소를 포함해야 합니다:
   - `td`는 두 개의 앵커 요소를 포함하며, 하나는 `post-title` 클래스를 가지고 `href`의 `<forumTopicUrl><slug>/<id>`를 가지며 `<title>` 앵커 텍스트를 갖고, 다른 하나는 `forumCategory`을 `category_id`로 호출하여 얻은 것입니다.
   - `td`는 `avatar-container` 클래스를 가진 `div` 요소를 포함하며, `avatars` 함수가 `posters`과 `users`을 인수로 호출하여 반환한 이미지를 포함합니다.
   - `td`는 게시물에 대한 답글 수를 포함합니다. _힌트:_ `posts_count - 1`를 사용하세요.
   - `td`는 게시물 조회 수를 포함합니다.
   - `td`는 마지막 활동 이후 경과 시간을 포함합니다.
1. `fetchData`라는 비동기 함수를 만들어야 합니다.
1. `fetchData` 함수는 `forumLatest`에서 데이터를 요청하고 응답을 JSON으로 파싱하여 `showLatestPosts`에 전달해야 합니다.
1. 데이터를 가져오는 중 오류가 발생하면 `fetchData` 함수가 콘솔에 오류를 기록해야 합니다. 이때 `console.log`를 사용하세요.

# --hints--

단일 인수를 받는 `timeAgo`라는 함수를 만들어야 합니다.
...
인수로 전달된 시간과 현재 시간 사이의 차이가 `50` 분일 때 `timeAgo`는 `50m ago`을 반환해야 합니다.
...
인수로 전달된 시간과 현재 시간 사이의 차이가 `60` 분일 때 `timeAgo`는 `1h ago`을 반환해야 합니다.
...
인수로 전달된 시간과 현재 시간 사이의 차이가 `115` 분일 때 `timeAgo`는 `1h ago`을 반환해야 합니다.
...
인수로 전달된 시간과 현재 시간 사이의 차이가 `15` 시간일 때 `timeAgo`는 `15h ago`을 반환해야 합니다.
...
인수로 전달된 시간과 현재 시간 사이의 차이가 `24` 시간일 때 `timeAgo`는 `1d ago`을 반환해야 합니다.
...
인수로 전달된 시간과 현재 시간 사이의 차이가 `46` 시간일 때 `timeAgo`는 `1d ago`을 반환해야 합니다.
...
인수로 전달된 시간과 현재 시간 사이의 차이가 `3` 일일 때 `timeAgo`는 `3d ago`을 반환해야 합니다.
...
단일 인수를 받는 `viewCount`라는 함수를 만들어야 합니다.
...
단일 인수를 받는 `forumCategory`라는 함수를 만들어야 합니다.
...
`forumCategory(299)`는 `Career Advice` 텍스트를 가진 앵커 요소를 포함하는 문자열을 반환해야 합니다.
...
`forumCategory(299)`는 `href="https://forum.freecodecamp.org/c/career/299"`를 가진 앵커 요소를 포함하는 문자열을 반환해야 합니다.
...
`forumCategory(299)`는 `class="category career"`를 가진 앵커 요소를 포함하는 문자열을 반환해야 합니다.
...
`forumCategory(200)`는 `General` 텍스트를 가진 앵커 요소를 포함하는 문자열을 반환해야 합니다.
...
`forumCategory(200)`는 `href="https://forum.freecodecamp.org/c/general/200"`를 가진 앵커 요소를 포함하는 문자열을 반환해야 합니다.
...
`forumCategory(200)`는 `class="category general"`를 가진 앵커 요소를 포함하는 문자열을 반환해야 합니다.
...
두 개의 인수를 받는 `avatars`라는 함수를 만들어야 합니다.
...
`avatars` 함수가 반환한 문자열의 각 `img` 요소는 게시자의 `alt` 속성 값을 가진 `name` 텍스트를 가져야 합니다.
...
`avatars` 함수는 `avatar_template` 속성에 접근하여 `{size}`를 `30`로 교체하여 각 아바타 크기를 설정해야 합니다.
...
각 `img` 요소는 일치하는 사용자의 `src` 값을 가진 `avatar_template` 속성을 가져야 합니다. `avatar_template`가 `/`로 시작하면 `avatarUrl`를 바로 앞에 붙이세요.
...
단일 매개변수를 받는 `showLatestPosts`라는 함수를 만들어야 합니다.
...
오류가 발생하면 `fetchData` 함수가 콘솔에 오류를 기록해야 하며 `console.log`를 사용하세요.
...
`showLatestPosts`는 `#posts-container`의 내부 HTML을 `tr`의 각 항목에 대해 `topics` 요소를 연결한 문자열로 설정해야 합니다.
...
`tr`가 반환한 문자열의 각 `showLatestPosts` 요소는 다섯 개의 `td` 요소를 포함해야 합니다.
...
`td`가 반환한 문자열의 각 표 행의 첫 번째 `showLatestPosts` 요소는 두 개의 앵커 요소를 포함해야 하며, 첫 번째는 `post-title` 클래스를 가지고 `href`의 `<forumTopicUrl><slug>/<id>`를 가지며 `<title>` 앵커 텍스트를 갖고, 두 번째는 `forumCategory`을 `category_id`로 호출하여 얻은 것입니다.
...
`td`가 반환한 문자열의 각 표 행의 두 번째 `showLatestPosts` 요소는 `avatars` 함수가 `posters`과 `users`를 인수로 호출하여 반환한 이미지를 `avatar-container` 클래스를 가진 `div` 요소 안에 중첩하여 포함해야 합니다.
...
`td`가 반환한 문자열의 각 표 행의 세 번째 `showLatestPosts` 요소는 게시물에 대한 답글 수를 포함해야 합니다. _힌트:_ `posts_count - 1`를 사용하세요.
...
`td`가 반환한 문자열의 각 표 행의 네 번째 `showLatestPosts` 요소는 게시물 조회 수를 포함해야 합니다.
...
`td`가 반환한 문자열의 각 표 행의 다섯 번째 `showLatestPosts` 요소는 `timeAgo` 함수를 사용해 생성한 마지막 활동 이후 경과 시간을 포함해야 합니다.
