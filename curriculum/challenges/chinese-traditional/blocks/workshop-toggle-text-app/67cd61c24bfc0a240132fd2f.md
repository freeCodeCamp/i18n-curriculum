...
根據 `isVisible` 狀態動態顯示字、單字 `Show Message` 或 `Hide Message` 會很不錯。 

之前，你已經複習過如何處理行內條件式呈現。你將會將類似的邏輯應用到按鈕文字上。

以下是使用三元運算子有條件地顯示字 `Hide` 和 `Show` 的範例：

```jsx
<button>{isMenuVisible ? "Hide" : "Show"} Menu</button>
```

在工作坊的最後一步，使用三元運算子有條件地顯示 `Hide` 或 `Show` 字於訊息按鈕。將條件放在 `Message` 文字之前，使其顯示為 `Show Message` 或 `Hide Message`。

隨著最後一次更改，你的工作坊已完成！測試你的按鈕以查看其運作效果。

# --hints--

當 `isVisible` 是 `false` 時，按鈕文字應該顯示為 `Show Message`。
...
當 `isVisible` 為 `true` 時，按鈕文字應顯示為 `Hide Message`。
