...
`generateHex("yellow")` deve retornar `"Invalid color"`.

```js
assert.equal(generateHex("yellow"), "Invalid color");
```

`generateHex("red")` deve retornar uma string de seis caracteres.

```js
assert.lengthOf(generateHex("red"), 6);
```

`generateHex("red")` deve retornar um código de cor hexadecimal válido de seis caracteres.
