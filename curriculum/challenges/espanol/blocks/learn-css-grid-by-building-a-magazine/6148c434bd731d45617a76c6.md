...
Si deseas añadir más iconos sociales, pero mantenerlos en la misma fila, necesitarías actualizar `grid-template-columns` para crear columnas adicionales. Como alternativa, puedes usar la propiedad `grid-auto-flow`.

Esta propiedad acepta ya sea `row` o `column` como el primer valor, con un segundo valor opcional de `dense`. `grid-auto-flow` utiliza un algoritmo de auto-colocación para ajustar la disposición de la cuadrícula. Establecerlo en `column` indicará al algoritmo que cree nuevas columnas para el contenido según sea necesario. El valor `dense` permite al algoritmo retroceder y llenar huecos en la cuadrícula con elementos más pequeños, lo que puede resultar en elementos que aparecen fuera de orden.

Para tu selector `.social-icons`, establece la propiedad `grid-auto-flow` en `column`.

# --hints--

Tu selector `.social-icons` debe tener una propiedad `grid-auto-flow` establecida en `column`.
