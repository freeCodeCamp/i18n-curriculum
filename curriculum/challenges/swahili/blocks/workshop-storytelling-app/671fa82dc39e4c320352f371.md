...
Ili kuhifadhi aina tofauti za hadithi, tengeneza kitu kinachoitwa `storyObj`. Kitu hicho kinapaswa kuwa na sifa tatu: `scary`, `funny`, na `adventure`. Kila thamani ya sifa inapaswa kuwa kitu chenye sifa za `story` na `borderColor`.

Weka thamani zifuatazo kwa vitu binafsi:
...
Unapaswa kuwa na kitu `storyObj`.

```js
assert.isObject(storyObj);
```

Kitu chako cha `storyObj` kinapaswa kuwa na sifa tatu: `scary`, `funny`, na `adventure`.

```js
assert.containsAllKeys(storyObj, ['scary', 'funny', 'adventure']);
```

Kila sifa inapaswa kuwa na sifa za `story` na `borderColor`.
...
Sifa ya `story` kwa `scary` inapaswa kuwa na thamani ya mfuatano wa herufi ya `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`
...
Sifa ya `borderColor` kwa `scary` inapaswa kuwa na thamani ya mfuatano wa herufi ya `"#ee4b2b"`.

```js
assert.equal(storyObj.scary.borderColor, "#ee4b2b");
```

Sifa ya `story` kwa `funny` inapaswa kuwa na thamani ya mfuatano wa herufi ya `"During a camping trip, Mark decided to show off his culinary skills by cooking dinner over an open fire. However, his attempt caused him to burn the dinner as well as his eyebrows off."`
...
Sifa ya `borderColor` kwa `funny` inapaswa kuwa na thamani ya mfuatano wa herufi ya `"#f1be32"`.

```js
assert.equal(storyObj.funny.borderColor, "#f1be32");
```

Sifa ya `story` kwa `adventure` inapaswa kuwa na thamani ya mfuatano wa herufi ya `"Lost in the heart of the Amazon rain forest, Sarah and Jake stumbled upon an ancient temple. They braved deadly traps and encountered strange wildlife, all while deciphering cryptic clues left behind by a mysterious civilization."`
...
Sifa ya `borderColor` kwa `adventure` inapaswa kuwa na thamani ya mfuatano wa herufi ya `"#acd157"`.
