...
CSS変数を使うと、メディアクエリの利用が簡単になります。

例えば、画面サイズがメディアクエリのブレークポイントより小さいまたは大きい場合に、変数の値を変更すれば、その変数が使われているすべてのスタイルに変更が適用されます。

# --instructions--

メディアクエリの`:root`セレクター内で、`--penguin-size`を再定義し、`200px`の値を代入してください。また、`--penguin-skin`も再定義し、`black`の値を代入してください。その後、プレビューのサイズを変更して、この変更が反映されるのを確認しましょう。

# --hints--

`:root`は`--penguin-size`変数に`200px`を代入する必要があります。
...
`:root`は`--penguin-skin`変数に`black`を代入する必要があります。
