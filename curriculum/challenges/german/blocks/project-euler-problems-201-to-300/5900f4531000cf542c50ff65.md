...
Für zwei beliebige Strings aus Ziffern, $A$ und $B$, definieren wir $F_{A,B}$ als die Folge ($A, B, AB, BAB, ABBAB, \ldots$), in der jedes Element die Verkettung der beiden vorherigen ist.

Außerdem definieren wir $D_{A,B}(n)$ als die $n^{\text{te}}$ Ziffer im ersten Element von $F_{A,B}$, das mindestens $n$ Ziffern enthält.

Beispiel:

Sei $A = 1\\,415\\,926\\,535$, $B = 8\\,979\\,323\\,846$. Wir wollen zum Beispiel $D_{A,B}(35)$ finden.

Die ersten Elemente von $F_{A,B}$ sind:
...
Dann ist $D_{A,B}(35)$ die ${35}^{\text{te}}$ Ziffer im fünften Element, also 9.

Nun verwenden wir für $A$ die ersten 100 Ziffern von $π$ hinter dem Komma:
...
Finden Sie $\sum_{n = 0, 1, \ldots, 17} {10}^n × D_{A,B}((127 + 19n) × 7^n)$.
