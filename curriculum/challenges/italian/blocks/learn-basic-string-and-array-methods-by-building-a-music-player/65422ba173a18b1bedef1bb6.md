...
All'interno della funzione freccia del listener dell'evento, aggiungi un'istruzione `if` per verificare se `userData?.currentSong` è `null`.

Dentro il blocco `if`, chiama la funzione `playSong()` con l'id `id` della prima canzone nell'array `userData?.songs`. Questo garantirà che la prima canzone della playlist venga riprodotta per prima.
...
Dovresti chiamare la funzione `playSong` con `userData?.songs[0].id` all'interno del tuo blocco `if`.
