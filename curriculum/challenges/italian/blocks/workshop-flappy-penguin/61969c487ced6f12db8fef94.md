...
Seleziona l'elemento `.left-mountain` e imposta il suo `width` e `height` su `300px`. Poi, imposta il `background` su un gradiente lineare che inizia da `rgb(203, 241, 228)` e termina a `rgb(80, 183, 255)`.

# --hints--

Dovresti usare il selettore `.left-mountain`.

```js
assert.match(code, /\.left-mountain\s*\{/);
```

Dovresti dare a `.left-mountain` un `width` di `300px`. `--fcc-actual--` dovrebbe essere `--fcc-expected--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.width, '300px');
```

Dovresti dare a `.left-mountain` un `height` di `300px`. `--fcc-actual--` dovrebbe essere `--fcc-expected--`.
