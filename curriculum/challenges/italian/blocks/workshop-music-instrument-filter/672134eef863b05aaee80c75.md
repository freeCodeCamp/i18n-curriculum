...
Per implementare la funzionalità di filtro, ti serve una funzione. Dichiara una funzione vuota chiamata `instrumentCards` che accetta un solo parametro.

# --hints--

Dovresti avere una funzione chiamata `instrumentCards`.

```js
assert.isFunction(instrumentCards);
```

La tua funzione `instrumentCards` dovrebbe accettare un solo parametro.
