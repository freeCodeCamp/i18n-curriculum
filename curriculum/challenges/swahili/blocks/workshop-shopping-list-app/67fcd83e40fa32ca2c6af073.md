...
Sasa baada ya kuwa na muundo wa msingi wa programu, ni wakati wa kuongeza vitu na kufanya kazi ya utafutaji ifanye kazi.

Tengeneza safu ya data na weka thamani yake katika kigezo kinachoitwa `items`. Ndani ya safu ya data, ongeza mfuatano wa herufi `Apples`, `Bananas`, `Strawberries`, `Blueberries`, `Mangoes`, `Pineapple`, `Lettuce`, `Broccoli`, `Paper Towels`, na `Dish Soap`. Hii itakuwa orodha ya vitu vitakavyoonyeshwa katika programu.

# --hints--

`items` ni safu ya data.

```js
assert.match(code, /(const|let|var)\s+items\s*=\s*\[/i);
```

Safu yako ya data ya `items` inapaswa kuwa na mfuatano wa herufi `Apples`, `Bananas`, `Strawberries`, `Blueberries`, `Mangoes`, `Pineapple`, `Lettuce`, `Broccoli`, `Paper Towels`, na `Dish Soap`.
