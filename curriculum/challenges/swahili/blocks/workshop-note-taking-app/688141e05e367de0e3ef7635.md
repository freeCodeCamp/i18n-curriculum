...
Kwa sasa, `currentContent` imeanzishwa na thamani ya mfuatano wa herufi tupu. Lakini matokeo yanayotarajiwa ni kuwa `currentContent` iwe na thamani ya maandishi ya noti ya sasa.

Katika funzo la awali, ulijifunza jinsi ya kufanya kazi na tukio la `DOMContentLoaded` kama ifuatavyo:
...
Wakati kila kitu katika hati ya HTML kimepakuliwa na kuchambuliwa, utaweza kupata upatikanaji wa `noteEl` kwa usalama.

Anza kwa kuambatisha njia ya `addEventListener` kwenye kitu cha `window`. Hoja ya kwanza kwa njia ya `addEventListener` inapaswa kuwa tukio la `"DOMContentLoaded"`. Hoja ya pili inapaswa kuwa kitendakazi cha mshale. Ndani ya mwili wa kitendakazi cha mshale, weka thamani ya `noteEl.textContent` kwa `currentContent`.

# --hints--

Unapaswa kuambatisha njia ya `addEventListener` kwenye kitu cha `window`.

```js
assert.match(code, /window\.addEventListener/);
```

Msikilizaji wako wa tukio unapaswa kusikiliza tukio la `"DOMContentLoaded"`.

```js
assert.match(code, /window\.addEventListener\(['"]DOMContentLoaded['"]\,/);
```

Ndani ya mwili wa kitendakazi cha mshale, weka thamani ya `noteEl.textContent` kwa `currentContent`.
