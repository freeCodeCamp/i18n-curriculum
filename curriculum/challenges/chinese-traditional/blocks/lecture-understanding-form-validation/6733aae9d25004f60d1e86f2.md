...
它會立即回報無效狀態，而不是等我們提交表單。但它仍然使用預設訊息。這是因為 `reportValidity()` 方法只告訴瀏覽器輸入是無效的，瀏覽器仍然決定如何顯示為何無效。這時候就會用到 `setCustomValidity()` 方法。
...
有幾個有用的屬性，它們的值都是布林值 `true` 或 `false`。

這些有用的屬性中，你可以自己多多探索的有 `valueMissing` 屬性，當必填輸入欄位留空時會是 `true`，還有 `patternMismatch` 屬性，如果值不符合指定的正則表達式樣式時會是 `true`。
