...
邏輯運算子可用來構建更複雜的媒體查詢。`and` 邏輯運算子用於查詢兩個媒體條件。

例如，針對顯示寬度介於 500px 和 1000px 之間的媒體查詢為：
...
為你的 `@media` 規則添加另一個條件，以便在瀏覽器視窗寬度大於 `769px` 但小於 `1199px` 時套用。
...
你的 `@media` 查詢應該有 `min-width` 為 `769px` 和 `max-width` 為 `1199px`。
