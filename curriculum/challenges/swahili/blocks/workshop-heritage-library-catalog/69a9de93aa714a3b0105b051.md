...
Sasa ungana mistari yote kuwa mfuatano wa herufi wa CSV unaweza kuuza au kushiriki.

Baada ya mzunguko wa kwanza, tangaza `let csv = header`. Ongeza mzunguko mwingine wa `for` ili kuongeza kila mstari kwenye `csv` ukiwa na mstari mpya: `csv = csv + "\n" + rows[i]`. Rudisha `csv`. Kisha, chini ya kitendakazi, andika `exportToCSV(catalog)` kwenye konsoli ili kuonyesha muonekano uliopangwa.

# --hints--

`exportToCSV` inapaswa kurudisha mfuatano wa herufi.

```js
assert.isString(exportToCSV(catalog));
```

Mstari wa kwanza wa mfuatano uliorudishwa unapaswa kuwa `"Title,Author,Year,Location"`.

```js
assert.strictEqual(exportToCSV(catalog).split("\n")[0], "Title,Author,Year,Location");
```

Matokeo yanapaswa kuwa na `"King, Stephen"` (imewekwa nukuu kushughulikia koma).

```js
assert.include(exportToCSV(catalog), '"King, Stephen"');
```

Unapaswa kuandika `exportToCSV(catalog)` kwenye konsoli.
