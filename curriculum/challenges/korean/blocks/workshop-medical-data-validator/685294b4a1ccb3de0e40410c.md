...
`validate` 함수로 돌아가서, 두 개의 `if` 문장 다음이자 `for` 반복문 안에, `invalid_records`라는 변수를 만드세요.

그다음, `find_invalid_records` 연산자를 사용해 `**`를 풀어서 `dictionary`를 호출한 값을 할당하세요.

# --hints--

`invalid_records` 함수 안 `for` 반복문 맨 아래에 `validate`라는 변수가 있어야 합니다.
...
`invalid_records` 변수는 `find_invalid_records(**dictionary)` 값을 가져야 합니다.
