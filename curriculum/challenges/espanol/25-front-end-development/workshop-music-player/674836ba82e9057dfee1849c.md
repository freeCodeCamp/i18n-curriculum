...
A continuación, utilice la propiedad `classList` y el método `add()` para añadir la clase `playing` al elemento `playButton`. Esto buscará la clase `playing` en el archivo CSS y la añadirá al elemento `playButton`.

Luego, establezca `userData.currentSong` a `song` y, para finalmente reproducir la canción, utilice el método `play()` en la variable `audio`. `play()` es un método de la API de audio web para reproducir un archivo mp3.

# --hints--

Deberías añadir la clase `playing` al elemento `playButton` dentro de tu función `playSong`.
...
Deberías establecer `userData.currentSong` a `song` dentro de tu función `playSong`.
...
Deberías llamar al método `play()` en la variable `audio` dentro de tu función `playSong`.
