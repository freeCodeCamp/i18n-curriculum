...
為你的核取方塊 `input` 元素添加值為 `personality` 的 `name` 屬性。

雖然你在瀏覽器中不會注意到這點，但這樣做會讓伺服端更容易處理你的網頁表單，尤其是在有多個核取方塊時。
...
核取方塊 `input` 元素沒有 `name` 屬性。請檢查開啟標記名稱後是否有空格。

```js
assert.isTrue(document.querySelector('input[type="checkbox"]')?.hasAttribute('name'));
```

核取方塊 `input` 元素應該有一個 `name` 屬性，其值為 `personality`。你要麼遺漏了該值，要麼有拼寫錯誤。請記得屬性值應該用引號括起來。
