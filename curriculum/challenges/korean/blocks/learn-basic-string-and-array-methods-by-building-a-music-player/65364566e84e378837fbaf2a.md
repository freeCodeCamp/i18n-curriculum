...
이전 단계에서 노래를 재생하는 기능을 구현했습니다. 이제 재생 버튼에 현재 노래를 클릭했을 때 재생할 수 있는 기능을 추가해야 합니다.

`addEventListener()` 메서드를 사용하고 첫 번째 인수로 `"click"` 이벤트를 전달하며 두 번째 인수로는 화살표 구문을 사용한 빈 콜백 함수를 전달하세요. 예를 들어 `() => {}`와 같습니다.

# --hints--

`addEventListener()` 변수에서 `playButton` 메서드를 호출해야 합니다.
...
화살표 구문을 사용해 이벤트 리스너에 빈 콜백을 전달하세요. 중괄호 쌍도 빈 상태로 꼭 포함해야 합니다.
