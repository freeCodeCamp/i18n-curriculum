...
在你的 `aside` 元素中，创建两个 `img` 元素、一个 `blockquote` 元素和第三个 `img` 元素。 将 `blockquote` 元素的 `class` 设置为 `image-quote`。

# --hints--

你应该在 `aside` 元素中创建三个 `img` 元素。

```js
assert(document.querySelectorAll('aside img')?.length === 3);
```

你应该在 `aside` 元素中创建一个 `blockquote` 元素。

```js
assert.exists(document.querySelector('aside blockquote'));
```

你的 `blockquote` 元素应该有一个设置为 `image-quote` 的 `class`。
