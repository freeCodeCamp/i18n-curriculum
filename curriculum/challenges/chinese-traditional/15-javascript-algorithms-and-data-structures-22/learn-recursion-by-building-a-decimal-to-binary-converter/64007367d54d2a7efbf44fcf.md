...
接下來，你需要檢查 `parseInt()` 函數返回的值是否是數字。

爲此，你可以使用 `isNaN()` 函數。 此函數以字符串或數字作爲參數，如果計算結果爲 `NaN`，則返回 `true`。 例如：
...
更新 `if` 語句中的第二個條件，以使用 `isNaN()` 函數檢查 `parseInt()` 返回的值是否爲 `NaN`。

另外，正如我們在步驟 1 中提到的，我們只考慮正數，我們應該在 `if` 語句中添加第三個條件來檢查數字是否小於 0（即負數）。
...
你應該將 `parseInt()` 返回的值包裝在 `isNaN()` 函數中。 並添加第三個條件，檢查 `parseInt()` 返回的值是否小於 0。
