...
Poiché questo è un quiz, ti serve un modulo per permettere agli utenti di inviare le risposte. Puoi separare semanticamente il contenuto all’interno del modulo usando elementi `section`.

All’interno dell’elemento `main`, crea un modulo che contenga tre elementi `section`. Poi, fai in modo che il modulo invii i dati a `https://freecodecamp.org/practice-project/accessibility-quiz` usando il metodo corretto.
