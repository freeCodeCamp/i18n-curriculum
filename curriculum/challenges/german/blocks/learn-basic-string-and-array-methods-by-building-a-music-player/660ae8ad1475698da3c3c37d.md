...
Um eine benannte Pfeilfunktion aufzurufen, können Sie die Funktion mit ihrem Namen referenzieren.

```js
exampleArrowFunction();
```

Rufen Sie unterhalb Ihrer `printGreeting`-Funktionsdefinition die Funktion auf. Öffnen Sie die Konsole, um die Ausgabe zu sehen.

# --hints--

Sie sollten Ihre `printGreeting`-Funktion aufrufen.

```js
assert.match(code, /printGreeting\(\);?/);
```

Ihr `printGreeting`-Funktionsaufruf sollte unterhalb der `printGreeting`-Funktionsdefinition stehen.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

Ihr `printGreeting`-Funktionsaufruf sollte `Hello there!` in der Konsole ausgeben.
