...
現在更新 `button2.innerText` 和 `button3.innerText`，分別分配 `"button text"` 數組的第二個和第三個值。

# --hints--

你應該訪問 `location` 參數的 `"button text"` 屬性的第二個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*1\s*\]/);
```

你應該將 `button2.innerText` 屬性設置爲 `location` 參數的 `"button text"` 屬性的第二個元素。

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*location\s*\[\s*('|")button text\1\s*\]\s*\[\s*1\s*\]/);
```

你應該訪問 `location` 參數的 `"button text"` 屬性的第三個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*2\s*\]/);
```

你應該將 `button3.innerText` 屬性設置爲 `location` 參數的 `"button text"` 屬性的第三個元素。
