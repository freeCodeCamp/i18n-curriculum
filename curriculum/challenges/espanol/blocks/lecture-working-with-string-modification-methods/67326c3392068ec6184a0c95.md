...
El método `repeat()` es una función incorporada en JavaScript que te permite repetir una cadena un número especificado de veces. Esta es la sintáxis básica:

```js
string.repeat(count);
```

`string` es la cadena que deseas repetir, y `count` es el número de veces que deseas que la cadena se repita. Aquí tienes un ejemplo:
...
En este caso, la cadena `Hello!` se repite tres veces, resultando en `Hello!Hello!Hello!`.

Si bien el método `repeat()` es útil, hay algunas excepciones y limitaciones a tener en cuenta.

El parámetro `count` debe ser un número no negativo. Si pasas un número negativo, JavaScript lanzará un `RangeError`.
...
El `count` debe ser un número finito. Si intentas repetir una cadena un número infinito de veces o usas `Infinity` como conteo, también obtendrás un `RangeError`.
