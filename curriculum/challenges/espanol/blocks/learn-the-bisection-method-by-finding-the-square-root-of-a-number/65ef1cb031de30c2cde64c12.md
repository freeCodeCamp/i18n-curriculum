...
Si la diferencia está dentro de la `tolerance` especificada, asigna el valor de `root` a `mid` y sal del ciclo.

# --hints--

Debes eliminar la declaración `pass` existente. 
...
Debes asignar el valor de `mid` a `root` y salir del ciclo.
