...
In questo workshop creerai un'app per la musica capace di riprodurre, mettere in pausa e saltare canzoni da una playlist.

L'HTML e il CSS di questo progetto sono stati forniti, così puoi concentrarti sul JavaScript.

Inizia consultando gli elementi `#playlist-songs`, `#play` e `#pause` e assegnali alle variabili `playlistSongs`, `playButton` e `pauseButton`, rispettivamente.
