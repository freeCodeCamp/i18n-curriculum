...
يجب عليك إنشاء `div` جديد مع class يدعى `fb1`.

```js
assert.exists(document.querySelector('div.fb1'));
```

يجب عليك إنشاء `div` جديد مع class يدعى `fb2`.

```js
assert.exists(document.querySelector('div.fb2'));
```

يجب عليك إنشاء `div` جديد مع class يدعى `fb3`.

```js
assert.exists(document.querySelector('div.fb3'));
```

يجب عليك إنشاء `div` جديد مع class يدعى `fb4`.

```js
assert.exists(document.querySelector('div.fb4'));
```

يجب عليك إنشاء `div` جديد مع class يدعى `fb5`.

```js
assert.exists(document.querySelector('div.fb5'));
```

يجب عليك إنشاء `div` جديد مع class يدعى `fb6`.

```js
assert.exists(document.querySelector('div.fb6'));
```

يجب عليك وضع عناصر `div` الجديدة يحمل عنصر `.foreground-buildings`.
