...
يجب عليك إعطاء `.fb5` خاصية `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

يجب عليك إعطاء `background` وظيفة `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

يجب عليك إضافة `repeating-linear-gradient` يحمل لون أولي `--building-color2` من `0%` إلى `5%`.
...
يجب أن تعطي `repeating-linear-gradient` لون ثان `transparent` من `5%` إلى `10%`.
