...
1. Debe ver un elemento `figure` o un elemento `div` con un `id` de `img-div`.
...
Su elemento `img` debe tener un `display` de `block`.
...
Su `#image` debe tener un `max-width` de `100%`.
...
Su `#image` debe tener un `height` de `auto`.
