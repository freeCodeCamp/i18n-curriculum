...
依照你為按鈕文字所做的相同樣式，將三個按鈕的 `onclick` 指派更新為 `"button functions"` 陣列的第一、第二和第三個值。

# --hints--

你應該存取 `location` 參數中 `"button functions"` 屬性的第一個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

你應該將 `button1.onclick` 屬性設定為 `location` 參數中 `"button functions"` 屬性的第一個元素。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

你應該存取 `location` 參數中 `"button functions"` 屬性的第二個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

你應該將 `button2.onclick` 屬性設為 `location` 參數中 `"button functions"` 屬性的第二個元素。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

你應該存取 `location` 參數中 `"button functions"` 屬性的第三個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*2\s*\]/);
```

你應該將 `button3.onclick` 屬性設為 `location` 參數中 `"button functions"` 屬性的第三個元素。
