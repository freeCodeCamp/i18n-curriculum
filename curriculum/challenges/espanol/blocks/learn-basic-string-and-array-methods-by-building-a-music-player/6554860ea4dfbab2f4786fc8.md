...
Si observas de cerca, verás que la canción que se está reproduciendo actualmente no está resaltada en la lista de reproducción, por lo que realmente no sabes qué canción está sonando. Puedes solucionar esto creando una función para resaltar cualquier canción que se esté reproduciendo.

Usando la sintaxis de función de flecha, crea una función `highlightCurrentSong`. Dentro de la función, usa `querySelectorAll` para obtener el elemento `.playlist-song` y asignarlo a una constante `playlistSongElements`.

# --hints--

Debes usar `const` para crear una función `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` debe ser una función.

```js
assert.isFunction(highlightCurrentSong)
```

Tu función `highlightCurrentSong` debe utilizar la sintaxis de función de flecha.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

Debes crear una constante `playlistSongElements` dentro de tu función `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

Debes usar el método `querySelectorAll()` para seleccionar el elemento `.playlist-song` y asignarlo a la constante `playlistSongElements`.
