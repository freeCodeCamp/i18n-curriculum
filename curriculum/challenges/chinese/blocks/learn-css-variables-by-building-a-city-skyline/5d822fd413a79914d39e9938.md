...
给 `sky` 类添加一个 `radial-gradient`。 从 `0%` 到 `20%` 使用 `#ffcf33`，从 `21%` 使用 `#ffff66`，从 `100%` 使用 `#bbeeff`。 这将为背景添加圆形渐变，这将是你的太阳。

# --hints--

你应该在 `background` 属性中为 `.sky` 添加一个 `radial-gradient`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".sky")?.background, "radial-gradient");
```

你应该给 `radial-gradient` 添加第一种颜色 `#ffcf33`。

```js
assert.match(new __helpers.CSSHelp(document).getStyle(".sky")?.background, /radial-gradient\(rgb\(255, 207, 51\)( 0%)?, rgb\(255, 207, 51\) 20%/);
```

你应该在 `21%` 处为 `radial-gradient` 添加第二种颜色 `#ffff66`。
...
你应该在 `100%` 处为 `radial-gradient` 添加第三种颜色 `#bbeeff`。
