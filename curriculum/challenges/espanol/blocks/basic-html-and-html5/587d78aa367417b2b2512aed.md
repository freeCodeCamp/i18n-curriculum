...
Le indicas al navegador esta información agregando la etiqueta `<!DOCTYPE ...>` en la primera línea, donde la parte `...` es la versión de HTML. Para HTML5, usas `<!DOCTYPE html>`.
...
A continuación, el resto de tu código HTML debe estar envuelto en etiquetas `html`. La apertura `<html>` va directamente debajo de la línea `<!DOCTYPE html>`, y el cierre `</html>` va al final de la página.
