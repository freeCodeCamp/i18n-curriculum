...
All'interno della funzione `playNextSong`, chiama le funzioni `setPlayerDisplay`, `highlightCurrentSong` e `setPlayButtonAccessibleText` per aggiornare correttamente il giocatore quando la playlist termina.

# --hints--

Dovresti chiamare `setPlayerDisplay` all'interno dell'istruzione `else` della tua funzione `playNextSong`.
...
Dovresti chiamare `highlightCurrentSong` all'interno dell'istruzione `else` della tua funzione `playNextSong`.
...
Dovresti chiamare `setPlayButtonAccessibleText` all'interno dell'istruzione `else` della tua funzione `playNextSong`.
