...
You should add a new `label` element after the checkbox element. Hakikisha kuwa ina tagi ya kufungua na kufunga.
...
Your `label` element should have a `for` attribute with the value `loving`.  `--fcc-expected--` inatarajiwa lakini inayopatikana ni `--fcc-actual--`.
