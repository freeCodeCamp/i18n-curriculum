...
Il centro di controllo ha richiesto una nuova funzione per suddividere un equipaggio in blocchi di dimensioni variabili. Crea una funzione vuota chiamata `chunkCrew` che accetta due parametri, `crew` e `size`.

# --hints--

Dovresti creare una funzione chiamata `chunkCrew`.

```js
assert.isFunction(chunkCrew);
```

La tua funzione `chunkCrew` dovrebbe avere i parametri `crew` e `size`.
