...
목록 항목의 마커 사이에 약간 간격을 두어야 합니다. 따라서 목록 항목의 각 `span`에 `text-green-700`과 `mr-2` 클래스를 추가하세요.

또한 각 `aria-hidden` 요소에 `span` 속성을 추가하세요.

# --hints--

`span` 요소에는 `text-green-700` 클래스를 지정해야 합니다.
