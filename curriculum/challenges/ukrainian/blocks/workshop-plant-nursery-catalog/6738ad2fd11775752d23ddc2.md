...
Конструктор `Set` також приймає ітерований об’єкт як аргумент і створює з нього множину:
...
Щоб завершити це практичне заняття, змініть свій `displayPlantsSet`, щоб він повертав множину унікальних загальних назв рослин у вашому каталозі.

Для цього створіть масив, що містить `commonName` кожного об’єкта рослини, збереженого як ключ у `catalog` мапі. Передайте масив конструктору `Set` і поверніть множину з функції.

# --hints--

Ваша функція `displayPlantsSet` має повертати множину, що містить `commonName` кожної рослини у вашому каталозі.
