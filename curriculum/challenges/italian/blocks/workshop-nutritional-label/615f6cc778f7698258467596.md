...
Sotto il tuo elemento con il testo `Total Fat`, crea un nuovo elemento `p` con il testo `Saturated Fat 1g 5%`. Avvolgi il `5%` in un `span` con l'attributo `class` impostato su `bold`. In questo caso basta per allineare la percentuale a `5%`.
...
Il tuo nuovo elemento `p` dovrebbe avere il testo `Saturated Fat 1g 5%`.
