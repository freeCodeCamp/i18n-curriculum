...
Os navegadores aplicam estilos padrão a muitos elementos. No caso dos elementos de entrada, sua capacidade de estilizar com CSS pode parecer bastante limitada. Por isso, você pode querer usar `appearance: none` para ocultar aspectos do elemento padrão e criar o seu próprio em vez disso. Por exemplo, isso ocultaria as caixas de seleção padrão de uma entrada do tipo checkbox, permitindo usar indicadores personalizados como um sinal verde de confirmação e um X vermelho para mostrar o estado. Para uma entrada de busca, navegadores baseados em WebKit exibem um ícone de busca padrão e um botão de cancelar. Ocultá-los permite criar seus próprios indicadores que apareceriam em todos os navegadores.
