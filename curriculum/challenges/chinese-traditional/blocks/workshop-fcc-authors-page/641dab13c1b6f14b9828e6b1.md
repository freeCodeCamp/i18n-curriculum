...
還有一件事。如果你持續點擊 `Load More Authors` 按鈕直到沒有更多資料可載入，且文字變成 `"No more data to load"`，游標的值仍然是 `pointer`。為什麼不把游標的值改成 `not-allowed` 呢？

存取 `style` 按鈕的 `Load More Authors` 屬性，並將 `cursor` 設為 `"not-allowed"`。
...
你應該使用點記法存取 `style` 的 `loadMoreBtn` 屬性。

```js
assert.match(code, /loadMoreBtn\.style/)
```

你應該使用點記法從 `cursor` 屬性存取 `style`。

```js
assert.match(code, /loadMoreBtn\.style\.cursor/)
```

你應該將 `cursor` 屬性的值設為 `"not-allowed"`。
