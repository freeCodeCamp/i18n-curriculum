...
同樣地，使用者應該能夠點擊電子郵件 `address` 並從他們的預設電子郵件 `client` 發送電子郵件。
...
在這最後一步，將文字 `fake@email.com` 包裹在錨點元素中，並使用 `mailto:` 使其成為可點擊的電子郵件位址。

有了這些更改後，你的部落格頁面現在已完成。

# --hints--

你應該將文字 `fake@email.com` 包裹在一個 `a` 元素中。
...
你的新 `a` 元素的 `href` 屬性應該設為 `mailto:fake@email.com`。
