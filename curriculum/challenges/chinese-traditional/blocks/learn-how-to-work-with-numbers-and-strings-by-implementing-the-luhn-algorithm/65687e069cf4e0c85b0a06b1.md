...
在 `if` 述語外，將 `number` 添加到 `sum_of_even_digits`。同時，移除 `print` 呼叫。

# --hints--

你應該在 `for` 迴圈中有 `sum_of_even_digits += number`。
...
你不應該在 `if` 述語中有 `sum_of_even_digits += number`。
