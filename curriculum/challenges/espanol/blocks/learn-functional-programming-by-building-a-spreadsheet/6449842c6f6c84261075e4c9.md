...
En tu `evalFormula`, declara una función de flecha `idToText` que toma un parámetro `id`.

Su función `idToText` debe devolver el resultado de llamar a `.find()` en la matriz `cells` con una función de devolución de llamada que toma un parámetro `cell` y devuelve `cell.id === id`.

Ambas funciones deben usar devoluciones implícitas.

# --hints--

Debes declarar una variable `idToText` en tu función `evalFormula`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*(?:const|let|var)\s+idToText/);
```

Debes usar `const` para declarar la constante `idToText`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText/);
```

Tu variable `idToText` debería ser una función de flecha.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

Tu función `idToText` debería tener un parámetro `id`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>/);
```

Tu función `idToText` debería devolver el resultado de llamar al método `.find()` en tu matriz `cells`. Tu función del callback debe utilizar un retorno implícito.
...
Tu función de callback debería devolver si `cell.id` es estrictamente igual a `id`.
