...
Now it's time to use the pseudo-selectors you set up earlier. 黒鍵を作成するには、新しい `.key.black--key::after` セレクターを追加します。 This targets elements with both `key` and `black--key` classes and selects the pseudo-element created after these elements in the HTML.
