...
انقل خاصية وقيمة `background-color` من `.fb2` إلى `.fb2b` لتلوين القسم فقط وليس الحاوية.

# --hints--

يجب إزالة خاصية `background-color` من `.fb2`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb2")?.backgroundColor);
```

يجب إعطاء `.fb2b` خاصية `background-color` بقيمة `--building-color3`.
