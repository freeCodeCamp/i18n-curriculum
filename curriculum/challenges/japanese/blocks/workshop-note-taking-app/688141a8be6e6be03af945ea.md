...
ユーザーがノートを編集したときに、編集されたバージョンを追跡したいです。

`let` を使って `currentContent` という変数を作成し、空の文字列を代入してください。

*注意*: 実際のアプリケーションでは、通常ノートをデータベースに保存します。またはローカルストレージに保存する場合もあります。ただし、データベースやローカルストレージの扱いはこのワークショップの範囲外であり、それらの概念は後で学習します。

# --hints--

`let` を使って `currentContent` 変数を作成してください。

```js
assert.match(code, /let\s+currentContent/);
```

`currentContent` 変数は文字列であるべきです。

```js
assert.isString(currentContent);
```

`currentContent` 変数は空の文字列で初期化してください。
