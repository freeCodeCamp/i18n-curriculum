...
画面の幅が小さい時、`h1` のテキストコンテンツの折り返し方が適切ではありませんね。 `h1` 要素のテキストを中央に揃えてください。

次に、`main` にパディングを加えて、`Student Info` セクションの見出しが完全に見えるようにしてください。

# --hints--

`h1` 要素の `text-align` の値を `center` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

`main` セレクターを追加して `main` 要素を選択する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

`main` 要素の `padding-top` の値を最低でも `25px` に設定する必要があります。

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

`padding-top` の値だけを変更してください。
