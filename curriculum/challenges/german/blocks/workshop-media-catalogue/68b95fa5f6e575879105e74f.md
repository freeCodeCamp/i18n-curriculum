...
Da die `MediaError`-Klasse nun vollständig ist, ändern Sie die `if`-Anweisung in der `add`-Methode. Ersetzen Sie die `TypeError` durch eine `MediaError` mit derselben Nachricht.

Geben Sie außerdem `media_item` als zweiten Argument nach der Nachricht mit an. So können Sie das Element, das den Fehler verursacht, im Ausnahmeobjekt speichern.

# --hints--

Sie sollten eine `MediaError` mit der Nachricht `Only Movie or TVSeries instances can be added` auslösen, wenn `media_item` keine Instanz von `Movie` oder dessen Unterklassen ist.
...
Sie sollten `media_item` als zweiten Argument an `MediaError` übergeben.
...
Sie sollten keine `MediaError` auslösen, wenn `media_item` ein gültiges Element ist.
...
Ihre `add`-Methode sollte keine `TypeError` auslösen.
