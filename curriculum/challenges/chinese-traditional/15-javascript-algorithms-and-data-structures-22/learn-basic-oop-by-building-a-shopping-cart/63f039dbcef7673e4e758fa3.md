...
瀏覽器具有內置的 `confirm()` 函數，可向用戶顯示確認提示。 `confirm()` 接受一個字符串，這是向用戶顯示的消息。 如果用戶確認則返回 `true`，如果用戶取消則返回 `false`。

聲明一個變量 `isCartCleared`，併爲其分配使用字符串 `"Are you sure you want to clear all items from your shopping cart?"` 調用 `confirm()` 的值。

# --hints--

你應該使用 `const` 來聲明 `isCartCleared` 變量。
...
你應該將字符串 `Are you sure you want to clear all items from your shopping cart?` 傳遞給 `confirm()` 函數。

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(\s*('|"|`)Are you sure you want to clear all items from your shopping cart\?\1\s*\)/);
```

你應該將 `confirm()` 函數的值分配給 `isCartCleared` 變量。
