...
다음 단계는 `users` 배열에서 올바른 사용자를 찾는 것입니다.

`user`이라는 상수를 만들고 `users.find()`를 할당하는 것부터 시작하세요. `find` 메서드는 `user`라는 매개변수를 가진 콜백 함수를 가져야 합니다.

`find` 메서드의 콜백 함수 안에서 `user.id`가 `poster.user_id`와 엄격하게 같은지 확인한 결과를 암묵적으로 반환하세요.

# --hints--

`user`라는 상수를 만들어야 합니다.

```js
assert(code.match(/\s*const\s+user\s*=/));
```

`users.find()`를 `user` 상수에 할당해야 합니다.

```js
assert(code.match(/\s*const\s+user\s*=\s*users\.find\(/g));
```

콜백 함수는 `user`라는 매개변수를 가져야 합니다.

```js
assert(code.match(/\s*const\s+user\s*=\s*users\.find\(\s*(\(\s*user\s*\)|user)\s*=>\s*/g));
```

`user.id`가 `poster.user_id`와 엄격하게 같은지를 암묵적으로 반환해야 합니다.
