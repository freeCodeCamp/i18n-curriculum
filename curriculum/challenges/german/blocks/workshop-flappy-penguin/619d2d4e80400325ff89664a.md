...
Um den linearen Farbverlauf auf der richtigen Seite des linken Arms des Pinguins zu halten, drehen Sie ihn zuerst um `130deg` und invertieren dann die x-Achse.

# --hints--

Sie sollten `.arm.left` eine `transform` von `rotate(130deg) scaleX(-1)` geben.
