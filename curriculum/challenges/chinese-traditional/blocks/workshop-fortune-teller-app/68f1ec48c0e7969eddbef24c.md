...
最後，在 `hideElements` 裡，傳入引數 `singleCard`、`multipleCard`、`fortuneContainer` 和 `fortuneDescription`。記得從元素中取得它們。

這樣，你的算命應用程式就完成了！

# --hints--

你應該將 `singleCard` 作為第一個引數傳給 `hideElements`。
...
你應該將 `multipleCard` 作為引數傳給 `hideElements`。
...
你應該將 `fortuneContainer` 作為引數傳給 `hideElements`。
...
你應該將 `fortuneDescription` 作為引數傳給 `hideElements`。
