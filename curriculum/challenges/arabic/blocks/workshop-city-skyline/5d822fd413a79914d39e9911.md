...
يبدو جيدًا! لننتقل إلى المباني الأمامية! حوّل المبنى `.fb1` إلى ثلاثة أقسام عن طريق تسكين ثلاثة عناصر `div` جديدة بداخله. امنحهم الأصناف `fb1a` و `fb1b` و `fb1c` بهذا الترتيب.

# --hints--

أضف ثلاثة عناصر `div` داخل `.fb1`.

```js
assert.lengthOf(document.querySelector("div.fb1")?.children, 3);
```

امنح أول عنصر `div` جديد الصنف `fb1a`.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1a"));
```

امنح ثاني عنصر `div` جديد الصنف `fb1b`.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1b"));
```

امنح ثالث عنصر `div` جديد الصنف `fb1c`.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1c"));
```

ضع عناصر `div` الجديدة بهذا الترتيب `.fb1a + .fb1b + .fb1c`.
