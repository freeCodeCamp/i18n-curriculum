...
`.penguin` 要素の上に、`div` を `class` が `left-mountain` の状態で追加してください。

# --hints--

`div` の中に新しい `body` を追加する必要があります。--fcc-expected-- 個の `div` 要素があることが期待されますが、実際には --fcc-actual-- 個見つかりました。

```js
assert.lengthOf(document.querySelectorAll('body > div'), 3);
```

`div` に `class` の `left-mountain` を付ける必要があります。
