...
Declare uma variável `currValue` e atribua a ela o valor em `array[i]`.Em seguida, declare uma variável `j` e atribua a ela `i - 1`.A variável `j` deve ser atribuível novamente.
...
Você deve atribuir a `currValue` o valor em `array[i]`.
