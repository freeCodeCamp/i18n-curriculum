...
`@media` の `only screen` を持つ `max-width` に対して、3つ目の `550px` クエリを作成してください。その中に、`.hero-title` に設定された `font-size` を持つ `6rem` セレクター、`.hero-subtitle, .author, .quote, .list-title` に設定された `font-size` を持つ `1.8rem` セレクター、`.social-icons` に設定された `font-size` を持つ `2rem` セレクター、そして `.text` に設定された `font-size` を持つ `1.6rem` セレクターを作成してください。

# --hints--

`@media` の `only screen` を持つ `max-width` に対して、新しい `550px` クエリがあるはずです。これは前の2つの後に来るべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText, 'only screen and (max-width: 550px)');
```

新しい `@media` ルールには、`.hero-title` セレクター、`.hero-subtitle, .author, .quote, .list-title` セレクター、`.social-icons` セレクター、`.text` セレクターがあるはずです。これらのセレクターはこの順番で並べてください。
