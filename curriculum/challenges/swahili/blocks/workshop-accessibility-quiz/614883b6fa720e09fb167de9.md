...
Ongeza nafasi ya ndani (padding) juu na kushoto kwa vipengele vya `.info`, na weka thamani zingine kuwa `0`.

# --hints--

Unapaswa kutumia kichaguzi cha `.info`.
...
Unapaswa kumpa `.info` nafasi ya ndani ya `padding-right` ya `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingRight, '0px');
```

Unapaswa kumpa `.info` nafasi ya ndani ya `padding-bottom` ya `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingBottom, '0px');
```

Unapaswa kumpa `.info` nafasi ya ndani ya `padding-left` angalau `1px`.
