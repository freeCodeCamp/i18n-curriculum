...
`.fb1c` に、今ある物の下にもう 1 つ `repeating-linear-gradient` を追加してください。`0%` から `10%` までは `--building-color4`、`10%` から `90%` までは `--window-color4` を設定してください。 これで、一つ前のステップで追加したグラデーションの後ろが塗りつぶされます。
...
`repeating-linear-gradient` を追加し、最初の色 `--building-color4` を `0%` から `10%` まで使用する必要があります。
...
2 番目の色 `--window-color4` を `10%` から `90%` まで使用する必要があります。
