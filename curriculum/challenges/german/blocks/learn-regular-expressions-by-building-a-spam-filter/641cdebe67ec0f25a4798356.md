...
Wenn der `messageInput` leer ist, zeigen Sie dem Benutzer eine Warnmeldung mit der Nachricht `"Please enter a message."` an.

Beenden Sie dann die Ausführung der Funktion.

# --hints--

Ihre Callback-Funktion sollte eine `if`-Anweisung enthalten.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

Ihre `if`-Anweisung sollte prüfen, ob der `value` von `messageInput` ein leerer String ist.
...
Ihre `if`-Anweisung sollte dem Benutzer eine Warnmeldung mit der Nachricht `"Please enter a message."` anzeigen.
...
Ihre `if`-Anweisung sollte die Ausführung der Funktion beenden.
