...
就像 `^` 锚一样，你可以使用 `$` 锚来匹配字符串的结尾。

更新你的正则表达式，以匹配字符串的结尾或空格，就像你对字符串的开头所做的一样。
...
你的 `freeRegex` 应该匹配 `it's free money time`。

```js
assert.match("it's free money time", freeRegex);
```

你的 `freeRegex` 应该匹配 `free money time`。

```js
assert.match('free money time', freeRegex);
```

你的 `freeRegex` 应该匹配 `it's free money`。

```js
assert.match("it's free money", freeRegex);
```

你的 `freeRegex` 应该匹配 `free money`。

```js
assert.match('free money', freeRegex);
```

你的 `freeRegex` 不应该匹配 `hands-free money time`。

```js
assert.notMatch('hands-free money', freeRegex);
```

你的 `freeRegex` 不应该匹配 `free money-management`。
