...
Accede a los elementos `#next`, `#previous` y `#shuffle` desde el archivo HTML.

Asígnalos a las variables denominadas `nextButton`, `previousButton`, y `shuffleButton`, respectivamente.

# --hints--

Debes tener una variable llamada `nextButton`.

```js
assert.isDefined(nextButton);
```

Debes usar `document.getElementById()` para obtener el elemento `#next`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

Debes asignar el elemento `#next` a la variable `nextButton`.

```js
assert.equal(nextButton, document.getElementById('next'));
```

Debes tener una variable llamada `previousButton`.

```js
assert.isDefined(previousButton);
```

Debes usar `document.getElementById()` para obtener el elemento `#previous`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

Debes asignar el elemento `#previous` a la variable `previousButton`.

```js
assert.equal(previousButton, document.getElementById('previous'));
```

Debes tener una variable llamada `shuffleButton`.

```js
assert.isDefined(shuffleButton);
```

Debes usar `document.getElementById()` para obtener el elemento `#shuffle`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

Debes asignar el elemento `#shuffle` a la variable `shuffleButton`.
