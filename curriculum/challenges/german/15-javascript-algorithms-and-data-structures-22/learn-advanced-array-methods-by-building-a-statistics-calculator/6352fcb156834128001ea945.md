...
Als Nächstes musst du die Summen der quadrierten Differenzen ermitteln.

Deklariere eine `sumSquaredDifferences`-Variable und weise ihr den Wert von `squaredDifferences.reduce()` zu. Gib für den Callback die Summe von `acc` und `el` zurück. Denke daran, den Ausgangswert auf `0` zu setzen.

# --hints--

Du solltest eine `sumSquaredDifferences`-Variable haben.

```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=/);
```

Deine `sumSquaredDifferences`-Variable sollte die `squaredDifferences.reduce()`-Methode verwenden.

```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(/);
```

Deine `sumSquaredDifferences`-Variable sollte die `acc`- und `el`-Parameter in der Callback-Funktion verwenden.
...
Dir fehlt der Startwert der `reduce`.
