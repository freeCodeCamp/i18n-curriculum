...
La sequenza di Farey <code>F<sub>n</sub></code> di ordine `n` è la sequenza di frazioni completamente ridotte tra `0` e `1` che, ridotte ai termini più bassi, hanno denominatori minori o uguali a `n`, ordinate in ordine crescente.

La *sequenza di Farey* è talvolta chiamata erroneamente *serie di Farey*.
...
Le sequenze di Farey di ordine `1` a `5` sono:
...
Scrivi una funzione che restituisca la sequenza di Farey di ordine `n`. La funzione deve avere un parametro che è `n`. Deve restituire la sequenza come un array.
