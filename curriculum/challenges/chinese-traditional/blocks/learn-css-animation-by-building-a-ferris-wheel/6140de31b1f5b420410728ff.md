...
現在給 `@keyframes wheel` 規則添加一個 `100%` 的選擇器。 在這裏面，將 `transform` 設置爲 `rotate(360deg)`。 通過這樣做，你的動畫將完成完整的旋轉。
