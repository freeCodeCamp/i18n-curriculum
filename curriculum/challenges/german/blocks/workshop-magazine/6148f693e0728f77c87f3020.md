...
Erstellen Sie eine dritte `@media` Abfrage für `only screen` mit einem `max-width` von `550px`. Erstellen Sie darin einen `.hero-title` Selektor mit einem `font-size`, der auf `6rem` gesetzt ist, einen `.hero-subtitle, .author, .quote, .list-title` Selektor mit einem `font-size`, der auf `1.8rem` gesetzt ist, einen `.social-icons` Selektor mit einem `font-size`, der auf `2rem` gesetzt ist, und einen `.text` Selektor mit einem `font-size`, der auf `1.6rem` gesetzt ist.

# --hints--

Sie sollten eine neue `@media` Abfrage für `only screen` mit einem `max-width` von `550px` haben. Diese sollte nach Ihren vorherigen beiden kommen.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText, 'only screen and (max-width: 550px)');
```

Ihre neue `@media` Regel sollte einen `.hero-title` Selektor, einen `.hero-subtitle, .author, .quote, .list-title` Selektor, einen `.social-icons` Selektor und einen `.text` Selektor enthalten. Diese Selektoren sollten in dieser Reihenfolge sein.
...
Ihr `.hero-title` Selektor sollte einen `font-size` haben, der auf `6rem` gesetzt ist.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[0]?.style?.fontSize, '6rem');
```

Ihr `.hero-subtitle, .author, .quote, .list-title` Selektor sollte einen `font-size` haben, der auf `1.8rem` gesetzt ist.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[1]?.style?.fontSize, '1.8rem');
```

Ihr `.social-icons` Selektor sollte einen `font-size` haben, der auf `2rem` gesetzt ist.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize, '2rem');
```

Ihr `.text` Selektor sollte einen `font-size` haben, der auf `1.6rem` gesetzt ist.
