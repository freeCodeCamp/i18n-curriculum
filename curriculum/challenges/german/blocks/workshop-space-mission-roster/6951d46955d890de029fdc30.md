...
Sie hatten zuvor einen `console.log()`-Aufruf zu Ihrer `swapCrewMembers`-Funktion hinzugefügt, um deren Verhalten zu validieren, und dieser Aufruf wird jetzt nicht mehr benötigt. Um zu verhindern, dass das Terminal in den nächsten Schritten überfüllt wird, entfernen Sie bitte die aktuelle `for`-Schleife, die in die Konsole schreibt.

# --hints--

Entfernen Sie die `for`-Schleife aus der `swapCrewMembers`-Funktion. Sie sollten den gesamten Schleifenblock löschen.
