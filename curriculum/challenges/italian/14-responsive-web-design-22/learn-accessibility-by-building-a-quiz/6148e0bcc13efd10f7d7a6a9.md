...
Dovresti dare al `footer` un `background-color` di `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

Dovresti dare al `footer` una proprietà `display` con il valore `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Dovresti dare al `footer` una proprietà `justify-content` con il valore `center`.
