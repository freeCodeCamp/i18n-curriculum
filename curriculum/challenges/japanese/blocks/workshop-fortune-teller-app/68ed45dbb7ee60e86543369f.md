...
データを取得した後、レスポンスを変数にストアし、それをJSONにパースしてください。パースしたデータには`Deck`型を付け、その後`cards`プロパティに代入します。`catch`ブロック内では、エラーをこのメッセージ`"Error fetching cards:", error`でログに記録してください。

# --hints--

`data`内に`fetchCardsData`という名前の変数を宣言してください。
...
`data.cards`を`this.cards`にストアしてください。

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

`catch`ブロックでは`console.error()`を使い、メッセージ`"Error fetching cards:", error`でエラーをログに記録してください。
