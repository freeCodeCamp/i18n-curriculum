...
在 `title` 上調用 `removeSpecialChars`，並在 `taskObj` 中調用 `description` 屬性。 對於 `id` 屬性，僅在屬性值的 `titleInput.value` 部分調用它。
