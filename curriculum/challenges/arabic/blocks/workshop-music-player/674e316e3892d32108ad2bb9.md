...
في كل مرة تنقر فيها على زر التشغيل، ستُشغل الأغنية الأولى في قائمة التشغيل دائمًا. لإصلاح ذلك، داخل الدالة السهمية لمستمع الحدث، استبدل `playSong(0)` بعبارة `if` تتحقق مما إذا كانت `userData.currentSong` تساوي `null`.

داخل كتلة `if`، نفذ استدعاء الدالة `playSong` باستخدام `id` الأغنية الأولى في مصفوفة `userData.songs`.

# --hints--

يجب تعديل استدعاء راجع مستمع الحدث لاستدعاء الدالة `playSong` باستخدام `id` الأغنية الأولى في مصفوفة `userData.songs` عندما تكون `userData.currentSong` تساوي `null`.
