...
بعد ذلك، أضف مفتاح `medications` إلى قاموس `constraints`. استخدم `isinstance` لقيمة هذا المفتاح للتحقق من أن `medications` قائمة.

# --hints--

يجب أن يحتوي قاموس `constraints` على مفتاح `medications`.
...
يجب أن تكون قيمة مفتاح `medications` في قاموس `constraints` هي `isinstance(medications, list)`.
