...
Crea un ultimo elemento `p` alla fine del tuo elemento `.text` e assegnagli il seguente testo:
...
Dovresti aggiungere un sesto elemento `p` all'elemento `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 7);
```

Il tuo sesto elemento `p` dovrebbe contenere il testo fornito.
