...
在你的嵌套 `for` 循环中，添加 `console.log()` 调用来检查每次迭代中的 `array`，`array[j]` 和 `array[minIndex]` 的值。 你可以点击 `Sort` 按钮查看你的算法如何遍历数组。

然后写一个 `if` 语句来检查 `array[j]` 的值是否小于 `array[minIndex]` 的值。 如果是，请将 `minIndex` 设置为 `j`。

# --hints--

你的嵌套 `for` 循环中应该有一个 `console.log()` 调用。
...
你应该将 `array` 作为第一个参数传递给 `console.log()`。
...
你应该将 `array[j]` 作为第二个参数传递给 `console.log()`。
...
你应该将 `array[minIndex]` 作为第三个参数传递给 `console.log()`。
...
在你的 `console.log()` 调用之后应该有一个 `if` 语句。
...
你应该在你的 `if` 语句中添加一个条件，以检查 `array[j]` 是否小于 `array[minIndex]`。
