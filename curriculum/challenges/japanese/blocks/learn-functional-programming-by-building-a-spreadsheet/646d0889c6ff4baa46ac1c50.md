...
あなたの `elemValue` 関数内で、`inner` パラメータを受け取る `character` という関数を宣言してください。

その後、`inner` 関数を返してください。

# --hints--

`inner` 変数を宣言する必要があります。
...
`inner` 関数は `character` を唯一のパラメータとして持つべきです。
