...
Як ви пам’ятаєте з попередніх уроків, ви можете використовувати метод `map` для ітерування по різних об’єктах і динамічного відтворення компонентів ось так:
...
Використайте метод `map()` для відтворення одного `Card` з масиву. Передайте `name`, `title` і `bio` як пропси до компонента `Card`.

# --hints--

Ви маєте використовувати метод `map()` для ітерування по масиву `profiles`.

```js
assert.match(code, /profiles\.map\s*\(/);
```

Ви маєте передати пропси `name`, `title` і `bio` з об’єкта `profile`.
