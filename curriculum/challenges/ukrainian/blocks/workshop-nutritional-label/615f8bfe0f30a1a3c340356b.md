...
Створіть ще один `p` елемент, додайте йому текст `Calcium 260mg 20%`. Вирівняйте `20%` по правому краю. Нижче створіть `p` елемент з текстом `Iron 8mg 45%`, вирівнявши `45%` по правому краю.

# --hints--

Ви маєте створити два нові `p` елементи в кінці вашого `.daily-value` елемента.
...
Перший новий `p` елемент повинен містити текст `Calcium 260mg 20%`.
...
Перший новий `p` елемент повинен містити `span` елемент.
...
Перший `span` елемент не повинен бути жирним.
...
Перший `span` елемент повинен обгорнути текст `20%`.
