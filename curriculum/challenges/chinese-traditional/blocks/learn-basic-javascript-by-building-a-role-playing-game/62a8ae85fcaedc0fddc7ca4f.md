...
為你的 `locations` 陣列添加第二個物件（記得用逗號分隔）。依照你在第一個物件中使用的樣式，創建相同的屬性，但使用來自 `goStore` 函式（程式）的值。將 `name` 屬性設為 `store`。

# --hints--

你的 `locations` 陣列應該有兩個值。

```js
assert.lengthOf(locations, 2);
```

兩個 `locations` 值都應該是物件。
...
你的第二個 `locations` 物件應該有一個 `name` 屬性，其值為 `store`。

```js
assert.equal(locations[1].name, "store");
```

你的第二個 `locations` 物件應該有一個 `button text` 屬性，該屬性為陣列。

```js
assert.isArray(locations[1]["button text"]);
```

你的 `button text` 屬性應該有字串值 `"Buy 10 health (10 gold)"`、`"Buy weapon (30 gold)"` 和 `"Go to town square"`。
...
你的第二個 `locations` 物件應該有一個 `button functions` 屬性，該屬性是一個陣列。

```js
assert.isArray(locations[1]["button functions"]);
```

你的 `button functions` 屬性應該有函式值 `buyHealth`、`buyWeapon` 和 `goTown`。
...
你的第二個 `locations` 物件應該有一個 `text` 屬性，該屬性為字串。

```js
assert.isString(locations[1].text);
```

你的第二個 `locations` 物件應該有一個 `text` 屬性，值為 `"You enter the store."`。

```js
assert.equal(locations[1].text, "You enter the store.");
```

你不應該修改第一個 `locations` 物件。
