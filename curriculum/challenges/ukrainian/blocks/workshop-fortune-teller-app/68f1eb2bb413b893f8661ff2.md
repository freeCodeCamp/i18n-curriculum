...
На цьому етапі ви повинні бачити передбачення, яке показує кожна карта після того, як ви натиснете на неї. Залишилось лише зробити так, щоб кнопка нового читання працювала.

Всередині методу `newReading` викличте `showElements()` та `hideElements()`.

# --hints--

Ви повинні викликати функцію `showElements` всередині методу `newReading`.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

Ви повинні викликати функцію `hideElements` всередині методу `newReading`.
