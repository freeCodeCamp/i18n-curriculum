...
Adicione um `console.log()` abaixo da sua função `getBooksByAuthor()`.

Dentro do `console.log()`, chame a função `getBooksByAuthor()` com `library` e `"Arvid Kahl"` como argumentos.

Agora, você deve ver todos os livros desse autor específico no console.

# --hints--

Você deve ter um `console.log(getBooksByAuthor(library, "Arvid Kahl"))`.
