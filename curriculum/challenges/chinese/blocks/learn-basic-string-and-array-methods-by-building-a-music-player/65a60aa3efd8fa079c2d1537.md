...
在这个示例中，第二个条件检查 `a.name > b.name`。 如果是，函数返回 `1`，将第一个水果排在第二个水果之后。
...
在回调函数内部，添加另一个 `if` 语句来检查 `a.title` 是否大于 `b.title`。 如果是，则返回数字 `1`。

# --hints--

你的回调函数内部应该有第二个 `if` 语句。
...
你的 `if` 语句应该检查 `a.title` 是否大于 `b.title`。

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
```

如果 `a.title` 大于 `b.title`，则 `if` 语句应返回数字 `1`。
