...
Ітеруйте над `playlistSongElements` за допомогою методу `forEach`.

Метод `forEach` використовується для циклічного перегляду масиву та виконання функції на кожному елементі масиву. Наприклад, припустімо, що у вас є масив чисел і ви хочете записати кожне число на консолі.
...
Використайте метод `forEach` на `playlistSongElements`. Передайте `songEl` як параметр та використайте стрілковий синтаксис, щоб додати порожній зворотний виклик.

# --hints--

Приєднайте `forEach()` до `playlistSongElements`.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

Метод `forEach()` повинен мати параметр `songEl` та використати стрілковий синтаксис.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

`forEach` має містити пусту пару фігурних дужок.
