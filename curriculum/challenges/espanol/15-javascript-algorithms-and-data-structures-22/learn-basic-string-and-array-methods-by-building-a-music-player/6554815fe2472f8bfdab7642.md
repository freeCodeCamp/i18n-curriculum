...
Use `const` y la sintaxis de flecha para crear una función `playPreviousSong` vacía.

# --hints--

Debes usar `const` para crear una función vacía llamada `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` debe ser una función.

```js
assert.isFunction(playPreviousSong)
```

Tu función `playPreviousSong` debe utilizar la sintaxis de flecha.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Tu función `playPreviousSong` debe estar vacía.
