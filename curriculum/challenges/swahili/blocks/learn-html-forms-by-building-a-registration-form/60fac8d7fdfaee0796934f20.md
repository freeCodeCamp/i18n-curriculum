...
Chaguzi za dropdown kwa sasa hazina maudhui. Ili kuzitoa maudhui, ongeza maandishi yafuatayo kwa kila kipengele cha `option` kinachofuata:
...
Unapaswa kumpa kipengele cha kwanza cha `option` maandishi ya `(select one)`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.textContent, '(select one)');
```

Unapaswa kumpa kipengele cha pili cha `option` maandishi ya `freeCodeCamp News`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.textContent, 'freeCodeCamp News');
```

Unapaswa kumpa kipengele cha tatu cha `option` maandishi ya `freeCodeCamp YouTube Channel`.
...
Unapaswa kumpa kipengele cha nne cha `option` maandishi ya `freeCodeCamp Forum`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.textContent, 'freeCodeCamp Forum');
```

Unapaswa kumpa kipengele cha tano cha `option` maandishi ya `Other`.
