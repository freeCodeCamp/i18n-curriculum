...
Kichaguzi bandia cha `::first-letter` kinakuwezesha kulenga herufi ya kwanza katika maudhui ya maandishi ya kipengee.

Tengeneza kichaguzi cha `.first-paragraph::first-letter` na weka sifa ya `font-size` kuwa `6rem`. Pia mpe sifa ya `color` iwe `orangered` ili iweze kujitokeza.
...
Kichaguzi chako cha `.first-paragraph::first-letter` kinapaswa kuwa na sifa ya `font-size` ikiwa `6rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize, '6rem');
```

Kichaguzi chako cha `.first-paragraph::first-letter` kinapaswa kuwa na sifa ya `color` ikiwa `orangered`.
