...
В межах елементу `article` створіть елемент `h3` з `class` зі значенням `list-title` та текстом `A Brief History`. Під ним створіть елемент `p` із текстом `Of the Curriculum`. Потім створіть елемент `ul` з класом `lists`.
