...
接下来，你想验证 `age` 是一个整数。所以为 `constraints` 字典添加另一个密钥 `age`。对于它的值，调用 `isinstance`，传入 `age` 和 `int` 作为参数。

# --hints--

你的 `constraints` 字典应包含一个密钥 `age`。
...
你的 `constraints` 字典中的 `age` 密钥应具有 `isinstance(age, int)` 的值。
