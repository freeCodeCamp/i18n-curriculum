...
如果用戶點擊 `Cancel` 按鈕，你需要取消該過程並關閉窗口，以便用戶可以繼續編輯。 HTML `dialog` 元素具有 `close()` 方法，可用於關閉網頁上的模態對話框。

```js
dialogElement.close();
```

爲 `cancelBtn` 元素添加事件監聽器，併爲第一個參數傳入 `click` 事件，爲第二個參數傳入回調函數。

對於回調函數，調用 `confirmCloseDialog` 元素上的 `close()` 方法。

# --hints--

你應該在 `cancelBtn` 變量上調用 `addEventListener()` 方法。
...
你的事件監聽器的回調函數應該調用 `confirmCloseDialog` 元素上的 `close()` 方法。
