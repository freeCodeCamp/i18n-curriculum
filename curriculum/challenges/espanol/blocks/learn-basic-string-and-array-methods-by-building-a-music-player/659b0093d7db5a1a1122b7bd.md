...
Para obtener el índice de la canción actual, puedes utilizar el método <dfn>indexOf()</dfn>. El método de arrays `indexOf()` devuelve el primer índice en el que se puede encontrar un elemento en el array, o `-1` si el elemento no está presente.
...
Dentro de tu función `getCurrentSongIndex`, regresa `userData?.songs.indexOf()`. Para el argumento de `indexOf()`, establécelo como `userData?.currentSong`.

# --hints--

Debes regresar `userData?.songs.indexOf()` dentro de tu función `getCurrentSongIndex`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

Debes pasar `userData?.currentSong` al método `indexOf()`.
