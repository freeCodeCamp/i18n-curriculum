...
Erstellen Sie ein neues `div`-Element unterhalb Ihres `.medium`-Elements. Geben Sie ihm ein `class`-Attribut mit dem Wert `daily-value small-text`. Fügen Sie innerhalb dieses neuen `div`-Elements ein `p`-Element mit dem Text `% Daily Value *` hinzu, und setzen Sie das `class`-Attribut auf `bold right`.

# --hints--

Sie sollten ein neues `div`-Element nach Ihrem `.medium`-Element erstellen.
...
Ihr neues `div` sollte ein `class`-Attribut mit dem Wert `daily-value small-text` haben.
...
Ihr neues `div`-Element sollte ein `p`-Element enthalten.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.localName === 'p');
```

Ihr neues `p`-Element sollte den Text `% Daily Value *` haben.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.textContent === '% Daily Value *');
```

Ihr neues `p`-Element sollte ein `class`-Attribut mit dem Wert `bold right` haben.
