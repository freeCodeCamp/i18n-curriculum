...
Define una función `playSong` que tome un solo parámetro, el cual representará el identificador único de la canción que deseas reproducir.

# --hints--

Deberías tener una función vacía llamada `playSong`.

```js
assert.isFunction(playSong);
```

Tu función `playSong` debería tomar un solo parámetro.
