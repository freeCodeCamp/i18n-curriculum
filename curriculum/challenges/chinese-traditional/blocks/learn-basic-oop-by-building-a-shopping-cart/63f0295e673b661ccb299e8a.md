...
你需要一種方法來存取購物車中項目的總數。最好的方法是為你的 `ShoppingCart` 類別添加另一個方法，而不是直接存取 `items` 陣列。

為你的 `ShoppingCart` 類別添加一個 `getCounts` 方法。它應該傳回 `items` 陣列中的項目數量。

# --hints--

你的 `ShoppingCart` 類別應該有一個 `getCounts` 方法。
...
你的 `getCounts` 方法應該傳回 `items` 陣列中的項目數量。
