...
Continúa agregando un `div` de `className` de `form-row`. Dentro, añade dos elementos `div` con el `className` de `form-group` cada uno.

El primer elemento `div` debe tener un `label` de `className` de `label`, un atributo `htmlFor` de `position`, un texto `Position` y un `input` con el `id` de `position`, un `className` de `input` y un `type` de `text`.

# --hints--

Tu sección Player Info debe contener un `div` con el `className` de `form-row`.
...
Tu `div.form-row` debe contener dos elementos `div`, cada uno con el `className` de `form-group`.
...
Tu primer `form-group` en la fila debe tener un `label` con el `className` de `label` y un `htmlFor` de `position`.
...
Tu `label` de posición debe tener el texto `Position`.
...
Tu primer `form-group` en la fila debe tener un `input` con el `className` de `input`, un `id` de `position` y `type` de `text`.
