...
headCoach 객체 안에 키가 `coachName`이고 문자열 값이 `"Carlos Bilardo"`인 속성을 추가하세요. 그 아래에 `matches`라는 또 다른 키를 숫자 값 `7`으로 추가하세요.

# --hints--

`headCoach` 객체에 `coachName` 속성이 있어야 합니다.

```js
assert.property(myFavoriteFootballTeam.headCoach, 'coachName');
```

`coachName` 속성은 `"Carlos Bilardo"`로 설정해야 합니다.

```js
assert.equal(myFavoriteFootballTeam.headCoach.coachName, 'Carlos Bilardo');
```

`headCoach` 객체에 `matches` 속성이 있어야 합니다.

```js
assert.property(myFavoriteFootballTeam.headCoach, 'matches');
```

`matches` 속성은 숫자여야 합니다.

```js
assert.isNumber(myFavoriteFootballTeam.headCoach.matches);
```

`matches` 속성은 `7`로 설정해야 합니다.
