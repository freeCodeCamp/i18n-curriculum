...
この例の2つ目の条件は`a.name > b.name`かどうかをチェックします。もしそうなら、関数は`1`を返し、2つ目の果物の後に最初の果物を並べ替えます。
...
コールバック関数の中に、もう1つ`if`文を追加して、`a.title`が`b.title`より大きいかどうかをチェックしてください。もしそうなら、数値`1`を返してください。

# --hints--

コールバック関数の中に2つ目の`if`文があるはずです。
...
`if`文は`a.title`が`b.title`より大きいかどうかをチェックしてください。

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
```

`if`文は、`1`が`a.title`より大きい場合に数値`b.title`を返すはずです。
