...
أنشئ عنصر `button` مع الصنف `playlist-song-info`. داخل عنصر الـ `button`، أضف عنصر `span` مع الصنف `playlist-song-title`، ثم استبدل النص بـ `song.title`.

# --hints--

يجب أن تنشئ عنصر `button` مع الصنف `playlist-song-info`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

يجب أن تنشئ عنصر `span` داخل عنصر الـ `button` الخاص بك.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

يجب أن يحتوي عنصر الـ `span` على الصنف `playlist-song-title`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

يجب أن تستبدل محتوى عنصر الـ `span` بـ `song.title`.
