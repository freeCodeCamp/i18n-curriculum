...
Crea un selector `a`, y dale una propiedad `text-decoration` establecida en `none` y una propiedad `color` establecida en `linen`.

# --hints--

Debes tener un selector `a`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('a'));
```

Tu selector `a` debe tener una propiedad `text-decoration` establecida en `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration, 'none');
```

Tu selector `a` debe tener una propiedad `color` establecida en `linen`.
