...
`--penguin-skin`라는 사용자 정의 CSS 변수를 만들고 `gray`로 설정하세요. 그런 다음 관련된 모든 속성 값을 이 변수로 바꾸세요.

# --hints--

`:root`에 `--penguin-skin` 속성을 지정해야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

`--penguin-skin`에 `gray` 값을 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

`.penguin-head`에 `background` 속성으로 `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`를 지정해야 합니다.
...
`.penguin-body::before`에 `background-color` 속성으로 `var(--penguin-skin)`를 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.
...
`.arm`에 `background` 속성으로 `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`를 지정해야 합니다.
