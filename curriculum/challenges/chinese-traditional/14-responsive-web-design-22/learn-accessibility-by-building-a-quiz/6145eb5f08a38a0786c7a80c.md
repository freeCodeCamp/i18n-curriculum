...
如果單擊單選輸入，你可能會注意到可以同時選擇同一真/假字段集中的兩個輸入。

將相關的輸入分組，以便一次只能選擇一對中的一個輸入。

# --hints--

你應該給第一個 `input` 一個 `name` 屬性。
...
你應該爲第四個 `input` 添加一個與第一個 `input` 的 `name` 屬性匹配的 `name` 屬性。
...
您應該爲第一個 `input` 和第三個 `input` 添加不同的 `name` 屬性。
