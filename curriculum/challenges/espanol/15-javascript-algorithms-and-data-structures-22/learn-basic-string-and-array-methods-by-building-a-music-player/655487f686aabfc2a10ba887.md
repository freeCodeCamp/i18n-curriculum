...
Antes de empezar a trabajar en reproducir la siguiente y la anterior canción, necesitas obtener el índice de cada canción en la propiedad `songs` de `userData`.

Comienza creando una función flecha llamada `getCurrentSongIndex`.

# --hints--

Debes tener una función `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Tu función `getCurrentSongIndex` debe usar sintaxis de flecha.
