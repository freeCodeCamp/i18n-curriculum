...
Всередині функції отримайте референси на HTML-елементи, які відповідають за відображення назви пісні та виконавця.

Отримайте елементи `#player-song-title` і `#player-song-artist` за допомогою методу `getElementById()`. Призначте їх відповідно змінним `playingSong` і `songArtist`.

# --hints--

Ви не повинні змінювати існуючу функцію `setPlayerDisplay` та її вміст.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Використовуйте `document.getElementById()` для отримання елемента `#player-song-title`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Призначте елемент `#player-song-title` змінній `playingSong`. Не забудьте оголосити змінну за допомогою `const`.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Використовуйте `document.getElementById()` для отримання елемента `#player-song-artist`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Призначте елемент `#player-song-artist` змінній `songArtist`. Не забудьте оголосити змінну за допомогою `const`.
