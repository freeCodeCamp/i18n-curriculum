...
`map` メソッドの中で、分割代入を使って以下のプロパティを `item` オブジェクトから取り出してください:
...
You should destructure the `id`, `title`, `views`, `posts_count`, `slug`, `posters`, `category_id`, `bumped_at` properties from the `item` object.
