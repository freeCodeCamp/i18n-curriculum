...
Ora, all'interno di `showElements`, passa questi come argomenti: `singleCardBtn`, `multipleCardsBtn` e `headerTitle`. Ricorda che li ottieni da `elements` e li consulti con la parola chiave `this`.

# --hints--

Dovresti passare `singleCardBtn` come primo argomento alla funzione `showElements`.
...
Dovresti passare `multipleCardsBtn` come secondo argomento alla funzione `showElements`.
...
Dovresti passare `headerTitle` come argomento alla funzione `showElements`.
