...
Crea un'istruzione `if` per verificare se l'oggetto `allCategories` ha una proprietà `id`. Ricorda, puoi usare il metodo `hasOwnProperty()` per questo.

# --hints--

Dovresti avere un'istruzione `if` che verifica se l'oggetto `allCategories` ha una proprietà `id`
