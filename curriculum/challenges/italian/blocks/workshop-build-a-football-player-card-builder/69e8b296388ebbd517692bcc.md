...
Ora avvolgi il componente `PlayerCard` in un elemento `main` di `className` di `main`.

All'interno dell'elemento `main`, aggiungi `div` di `className` di `layout`. All'interno di `layout`, aggiungi due elementi `div` separati di `className` di `form-panel` e `preview-panel` rispettivamente.

# --hints--

Il tuo `FootballPlayerCard` dovrebbe renderizzare un elemento `main` con il `className` di `main` all'interno di `div.page`.
...
Il tuo `main.main` dovrebbe contenere un `div` con il `className` di `layout`.
...
Il tuo `layout` dovrebbe contenere un `div` con il `className` di `form-panel`.
...
Il tuo `layout` dovrebbe contenere un `div` con il `className` di `preview-panel`.
