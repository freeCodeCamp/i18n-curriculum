...
`background-color`から`.fb3`のプロパティと値を削除し、`.fb3a`と`.fb3b`に追加してください。

# --hints--

`background-color`から`.fb3`を削除してください。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

`.fb3a`に`background-color`の`--building-color1`を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

`.fb3b`に`background-color`の`--building-color1`を設定してください。
