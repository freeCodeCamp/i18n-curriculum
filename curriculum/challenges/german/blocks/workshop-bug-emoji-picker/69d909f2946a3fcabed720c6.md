...
Jetzt, wo die `if`-Prüfung implementiert ist, verwenden Sie `e.target.value`, um den entsprechenden Bug in `bugMap` nachzuschlagen und dessen `render`-Methode aufzurufen. Danach sollten Sie die Bugs auswählen können und das Ergebnis sofort sehen.

Damit ist Ihr Bug Emoji Picker Workshop abgeschlossen!

# --hints--

Sie sollten die `render`-Methode beim entsprechenden Bug aus `bugMap` aufrufen.
