...
لجعل ميزة localStorage تعمل أخيرًا، غيّر تهيئة الحالة من `defaultPlayer` إلى `loadPlayer`. هذا يضمن أن تبدأ الحالة بأي بيانات محفوظة متاحة.

اختبر الآن ميزة localStorage. كل ما تدخله يجب أن يبقى بعد أن يتم تحديث المعاينة أو تعيد تحميل الصفحة.

يمكنك أيضًا تغيير رابط `src` للصورة إلى `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` لترى تغير الصورة في الوقت الحقيقي. فقط تأكد من تغيير اسم اللاعب إلى Maradona.

بهذا، يكتمل مشروع بناء بطاقة لاعب كرة القدم الخاص بك!

# --hints--

يجب تهيئة `useState` باستخدام `loadPlayer` بدلًا من `defaultPlayer`.
