...
Asigna a tu `conversionTable` una propiedad de `gram`, y asígnale un objeto con una propiedad de `cup` establecida en `1/240`, una propiedad de `ounce` establecida en `0.0353`, y una propiedad de `teaspoon` establecida en `0.2`.

# --hints--

Tu `conversionTable` debería tener una propiedad de `gram`.

```js
assert.property(conversionTable, "gram");
```

Tu propiedad de `gram` debería ser un objeto.

```js
assert.isObject(conversionTable.gram);
```

Tu objeto de `gram` debería tener una propiedad de `cup`.

```js
assert.property(conversionTable.gram, "cup");
```

Tu propiedad de `cup` debería establecerse en `1/240`.

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

Tu objeto de `gram` debería tener una propiedad de `ounce`.

```js
assert.property(conversionTable.gram, "ounce");
```

Tu propiedad de `ounce` debería establecerse en `0.0353`.

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

Tu objeto de `gram` debería tener una propiedad de `teaspoon`.

```js
assert.property(conversionTable.gram, "teaspoon");
```

Tu propiedad de `teaspoon` debería establecerse en `0.2`.
