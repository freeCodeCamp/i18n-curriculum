...
Agora, itere apenas sobre as linhas dentro do quadrado 3x3 criando um loop `for`. Use a função `range()` para gerar uma sequência começando em `row_start` e use `row_no` como a variável do loop.

# --hints--

Você deve criar um loop `for` que itere sobre `range(row_start, row_start + 3)`.
...
Seu loop `for` deve usar `row_no` como a variável do loop.
