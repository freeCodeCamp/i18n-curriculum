...
カスタムラジオボタンにサイズと形を指定しましょう。

引き続き `.contact-method-radio-btn` をターゲットにして、`width` を `20px` に、`height` を `20px` に、`border-radius` を `50%` に、`border` を `2px solid gray` に設定してください。

# --hints--

引き続き `.contact-method-radio-btn` クラスをターゲットにしてください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

`width` プロパティを `20px` に設定してください。
...
`border` プロパティを `2px solid gray` に設定してください。
