...
Für dieses Lab wurde Ihnen der gesamte HTML- und CSS-Code bereitgestellt. Sie werden TypeScript verwenden, um die Funktionalitäten einer Motorradladen-Seite zu vervollständigen.

**Ziel:** Erfüllen Sie die untenstehenden User Stories und bestehen Sie alle Tests, um das Lab abzuschließen.

**User Stories:**

1. Sie sollten einen `type` namens `Category` erstellen, der aus den folgenden möglichen Werten besteht: `'Sport'`, `'Cruiser'`, `'Touring'`, `'Dirt'`, `'Adventure'`, `'Naked'`, `'Electric'`.
2. Sie sollten ein `interface` namens `Motorcycle` erstellen, das die folgenden Eigenschaften hat: eine `id` vom Typ `string`, eine `name` vom Typ `string`, eine `manufacturer` vom Typ `string`, eine `category` vom Typ `Category`, eine `price` vom Typ `number`, eine `image_url` vom Typ `string`, eine `created_at`-Eigenschaft vom Typ `Date`, eine `description` vom Typ `string` und ein Jahr vom Typ `number`.
3. Sie sollten eine Funktion namens `fetchMotorcycles` erstellen, die ein `Promise` zurückgibt, das ein Array von `Motorcycle`-Objekten auflöst.
4. Sie sollten `Motorcycle`-Daten vom `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`-Endpunkt laden.
5. Sie sollten eine Funktion namens `renderMotorcycleCard` erstellen, die ein einzelnes `Motorcycle`-Objekt als Parameter entgegennimmt und einen String zurückgibt, der den HTML-Code für die Motorradkarte formatiert enthält.
   1. Die `renderMotorCycleCard`-Funktion sollte einen Parameter namens `motorcycle` vom Typ `Motorcycle` annehmen.
   2. Die `renderMotorCycleCard`-Funktion sollte einen Rückgabetyp von `string` haben.
   3. Jede Motorradkarten-Komponente sollte ein `img`-Element haben, das ein gültiges Bild und die Klasse `motorcycle-card-image-container` besitzt.
   4. Jede Motorradkarten-Komponente sollte ein Element mit der Klasse `motorcycle-card-year-badge` haben, das das Jahr als Klartext anzeigt.
   5. Jede Motorradkarten-Komponente sollte ein Element mit der Klasse `motorcycle-card-title` haben, das den Namen des Motorrads als Klartext anzeigt.
   6. Jede Motorradkarten-Komponente sollte ein Element mit der Klasse `motorcycle-card-manufacturer` haben, das den Namen des Motorradherstellers als Klartext anzeigt.
   7. Jede Motorradkarten-Komponente sollte ein Element mit der Klasse `motorcycle-card-category` haben, das die Kategorie des Motorrads als Klartext anzeigt.
   8. Jede Motorradkarten-Komponente sollte ein Element mit der Klasse `motorcycle-card-description` haben, das die Beschreibung des Motorrads als Klartext anzeigt.
   9. Jede Motorradkarten-Komponente sollte ein Element mit der Klasse `motorcycle-card-price` haben, das den Preis des Motorrads als Klartext anzeigt.
   10. Jede Motorradkarten-Komponente sollte ein Element mit der Klasse `motorcycle-card-engine` haben, das die PS-Zahl jedes Motors anzeigt.
6. Sie sollten eine Klasse namens `MotorcycleGalleryApp` haben.
   1. Die Klasse sollte ein `private`-Array namens `allMotorcycles` haben, das statisch auf einen Typ eines Arrays von `Motorcycle`-Objekten typisiert ist.
   2. Diese Klasse sollte eine Funktion namens `renderMotorcycles` haben, die die entsprechenden Motorradkarten-Komponenten der `allMotorcycles`-Variable innerhalb des Elements mit der ID `motorcycle-grid` rendert.
7. Die Anzahl der angezeigten `Motorcycle`-Elemente sollte in einem Element mit der ID `results-number` aufgelistet werden.
8. Der Filter sollte bei einem `input`-Ereignis aktualisiert werden.

# --hints--

Sie sollten ein `type` namens `Category` haben.
...
Ihr `Category`-Typ sollte auf eine Vereinigung der korrekten Werte gesetzt sein.
...
Sie sollten ein `interface` namens `Motorcycle` haben.
...
Sie sollten alle angegebenen Eigenschaften im `Motorcycle`-Interface haben.
...
Sie sollten eine Funktion namens `fetchMotorcycles` haben.

```js
assert.isFunction(fetchMotorcycles);
```

Sie sollten den `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`-Endpunkt verwenden.
...
Sie sollten ein Promise von 25 `Motorcycle`-Objekten von `fetchMotorcycles` zurückgeben.
...
Sie sollten eine Funktion namens `renderMotorcycleCard` haben.

```js
assert.isFunction(renderMotorcycleCard);
```

Die `renderMotorcycle`-Funktion sollte einen statisch typisierten Parameter vom Typ `Motorcycle` akzeptieren.
...
Die `renderMotorcycleCard`-Funktion sollte explizit einen Typ von `string` zurückgeben.
...
Die `renderMotorcycleCard`-Funktion sollte ein `string` zurückgeben.
...
Sie sollten eine Klasse namens `MotorcycleGalleryApp` haben.
...
Die `MotorcycleGalleryApp` sollte ein Array namens `allMotorcycles` haben.
...
Die `allMotorcycles`-Eigenschaft sollte `private` sein.
...
Die `allMotorcycles`-Eigenschaft sollte statisch auf `Motorcycle[]` typisiert sein.
...
Die `MotorcycleGalleryApp` sollte eine Funktion namens `renderMotorcycles` haben.
...
Die `renderMotorcycles`-Funktion sollte 25 Elemente mit der Klasse `motorcycle-card-image-container` rendern.
...
Die `renderMotorcycles`-Funktion sollte 25 Elemente mit der Klasse `motorcycle-card-year-badge` rendern.
...
Die `renderMotorcycles`-Funktion sollte 25 Elemente mit der Klasse `motorcycle-card-title` rendern.
...
Die `renderMotorcycles`-Funktion sollte 25 Elemente mit der Klasse `motorcycle-card-manufacturer` rendern.
...
Die `renderMotorcycles`-Funktion sollte 25 Elemente mit der Klasse `motorcycle-card-category` rendern.
...
Die `renderMotorcycles`-Funktion sollte 25 Elemente mit der Klasse `motorcycle-card-description` rendern.
...
Die `renderMotorcycles`-Funktion sollte 25 Elemente mit der Klasse `motorcycle-card-price` rendern.
...
Die `renderMotorcycles`-Funktion sollte 25 Elemente mit der Klasse `motorcycle-card-engine` rendern.
