...
Aquí tienes una aplicación sencilla para mostrarte cómo puedes inspeccionar componentes y cualquier prop y estado que tengan. Esto es similar al código que usamos en una lección anterior sobre prop drilling:
...
Si miras en la pestaña de Componentes en las React DevTools, puedes ver la vista en árbol de los componentes. El componente `App` está en la parte superior, seguido por los componentes `Parent`, `Child` y `Grandchild`.

Si seleccionas cualquiera de estos componentes, puedes ver las propiedades y el estado en ellos. Si seleccionas el componente `Parent`, puedes ver las propiedades `greeting` y `response`, que son `Hello, Prop Drilling!` y `I'm not here to play!`, respectivamente.

Puedes ver las propiedades y actualizar el estado en tiempo real, y cambiarlos si es necesario. Por ejemplo, puedes seleccionar el componente `Grandchild` y cambiar la propiedad `greeting` de `Hello, Prop Drilling!` a `Hello, Welcome to Prop Drilling!`, y verla reflejada en la página de inmediato.

Para registrar datos de un componente en la consola, inspecciona los elementos del DOM coincidientes y ve el código fuente del componente. Los iconos en la esquina superior derecha te permiten hacer eso. Si seleccionas el componente `Grandchild` y haz clic en el botón `Log the component data to the console`, registrará las propiedades, estado, ganchos, nodos y otros datos en la consola.

Un error común que podrías encontrar en React se llama desajuste de props.

Por ejemplo, supón que para el componente `Child`, pasas por error `reply` como la prop en vez de `response`:
...
Recuerda que `Grandchild` espera una propiedad de `response`. Debido a que el componente recibe una propiedad diferente, no puede mostrar ese texto en la página, y solo agrega un elemento `h2` vacío al DOM. En su lugar, solo verás el elemento `h1` con el texto `Hello, Prop Drilling!`, junto con los otros botones y texto ya presentes en la página. El elemento `h2` vacío todavía está allí, pero como está vacío, no puedes verlo sin inspeccionar el DOM.

Para solucionar esto, puedes inspeccionar la progresión de la propiedad desde el componente `Parent` hacia el `Child` y editar directamente el nombre de la propiedad. Si vas a la pestaña de Componentes, selecciona el componente `Child` y cambia la propiedad `reply` a `response`, verás el elemento `h2` en la página con el texto `I'm not here to play!`.
