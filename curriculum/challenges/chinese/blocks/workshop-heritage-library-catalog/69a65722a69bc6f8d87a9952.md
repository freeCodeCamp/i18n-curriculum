...
JSON（JavaScript 对象表示法）是一种在系统之间共享数据的标准格式。`JSON.stringify` 会将 JavaScript 值转换为 JSON 格式的字串。

首先移除两个 `validateEntry` 的 console.log 调用，因为它们只是用于测试。然后创建一个名为 `exportToJSON` 的函数，它接受 `catalog` 作为参数。返回 `JSON.stringify(catalog, null, 2)`。第一个参数是要转换的数据。第二个参数（`null`）表示不进行自定义过滤。第三个参数（`2`）添加两个空格的缩排，使输出更易读。

在函数下面，记录 `exportToJSON(catalog.slice(0, 2))` 以预览前两个条目的 JSON 输出。

# --hints--

你应该移除 `validateEntry` 的 console.log 调用。

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry/);
```

`exportToJSON` 应该返回一个字串。

```js
assert.isString(exportToJSON(catalog));
```

`JSON.parse(exportToJSON(catalog))` 应该产生一个有效的数组。

```js
assert.isArray(JSON.parse(exportToJSON(catalog)));
```

你应该使用 `JSON.stringify`，并将 `null` 和 `2` 作为第二和第三个参数。

```js
assert.match(exportToJSON.toString(), /JSON\.stringify\s*\(\s*catalog\s*,\s*null\s*,\s*2\s*\)/);
```

你应该将 `exportToJSON(catalog.slice(0, 2))` 记录到控制台。
