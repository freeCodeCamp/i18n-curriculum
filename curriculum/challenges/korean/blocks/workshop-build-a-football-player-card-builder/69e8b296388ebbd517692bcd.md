...
이제 폼을 만들기 시작하세요. `div`의 `className` 안에 있는 `form-panel` 내부에 "Player Info" 섹션을 감싸는 `div`를 추가하세요. 그 새 `div` 안에 `p`의 `className`와 텍스트 `form-section-title`가 포함된 `Player Info`를 추가하세요.

다음 단계에서 이 섹션에 필드를 계속 추가할 것입니다.

# --hints--

`div.form-panel` 안에 Player Info 섹션을 감싸는 `div`가 있어야 합니다.
...
`div` 안의 `div.form-panel`에는 `p`의 `className`와 텍스트 `form-section-title`가 포함된 `Player Info`가 있어야 합니다.
