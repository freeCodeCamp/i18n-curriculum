...
تم إنشاء بقية الطاقم في مصفوفة باسم `remainingCrew` تحتوي على البيانات التالية:

| id | name | دور | isEVAEligible | أولوية |
| -- | ---- | ---- | ------------- | -------- |
...
استعرض مصفوفة `remainingCrew` وأضف كل رائد فضاء إلى `squad` باستخدام الدالة `addCrewMember()`.

# --hints--

استخدم حلقة `for` لاستدعاء `addCrewMember()` مع كل رائد فضاء في `remainingCrew`.
