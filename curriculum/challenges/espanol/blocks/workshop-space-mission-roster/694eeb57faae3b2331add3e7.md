...
Dentro de la función `getEVAReadyCrew`, llama a `sortByPriorityDescending()` con el arreglo local `eligible` para que esté ordenado antes de ser retornado.

# --hints--

Debes llamar a `sortByPriorityDescending()` con el arreglo local `eligible`.
