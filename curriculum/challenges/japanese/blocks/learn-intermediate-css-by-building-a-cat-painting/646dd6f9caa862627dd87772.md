...
内耳を、`position` プロパティを `absolute` に、`top` を `22px` に、`left` を `-20px` に設定して位置調整してください。

# --hints--

`.cat-left-inner-ear` セレクターには、`position` プロパティを `absolute` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.position === 'absolute')
```

`.cat-left-inner-ear` セレクターには、`top` プロパティを `22px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.top === '22px')
```

`.cat-left-inner-ear` セレクターには、`left` プロパティを `-20px` に設定してください。
