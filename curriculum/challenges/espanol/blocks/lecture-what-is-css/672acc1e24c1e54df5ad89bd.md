...
Los elementos en línea comunes son los elementos `span`, `a` y `img`.
...
En este ejemplo, tenemos un elemento `span` anidado dentro de un párrafo. El elemento `span` tiene un color de texto `red` con un borde `green` alrededor para que puedas ver mejor la palabra resaltada.
