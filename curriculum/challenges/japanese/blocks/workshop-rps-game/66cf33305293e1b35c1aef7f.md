...
ラウンドの結果を取得する時が来ました。`getRoundResults` 関数を完成させてください。

プレイヤーがラウンドに勝った場合は、`playerScore` を `1` で更新し、メッセージ `"Player wins! [player's choice] beats [computer's choice]"` を返してください。

コンピューターとプレイヤーが同じ選択をした場合は、メッセージ `"It's a tie! Both chose [player's choice]"` を返してください。

コンピューターがラウンドに勝った場合は、`computerScore` を `1` で更新し、メッセージ `"Computer wins! [computer's choice] beats [player's choice]"` を返してください。

`[computer's choice]` は `computerResult` に置き換えられ、`[player's choice]` は `userOption` に置き換えられる必要があります。

# --hints--

あなたの `getRoundResults` は文字列を返す必要があります。

```js
assert.isString(getRoundResults("Rock"));
```

あなたの `getRoundResults` 関数は、ラウンドの勝者に基づいて正しいメッセージを返す必要があります。誰も勝たなかった場合は、引き分けのメッセージを返してください。
