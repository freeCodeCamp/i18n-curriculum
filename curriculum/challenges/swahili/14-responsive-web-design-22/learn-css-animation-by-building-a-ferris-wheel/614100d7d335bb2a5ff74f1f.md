...
Weka sifa ya `animation` kwa sheria ya `.cabin` kuwa `cabins 10s linear infinite`. Hii itaweka sifa za `animation-name`, `animation-duration`, `animation-timing-function`, na `animation-iteration-count` kwa mpangilio huo.

# --hints--

Kichaguzi chako cha `.cabin` kinafaa kuwa na sifa ya `animation` iliyowekwa kuwa `cabins 10s linear infinite`.
