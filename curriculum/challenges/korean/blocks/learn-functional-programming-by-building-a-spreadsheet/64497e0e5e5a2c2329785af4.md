...
스프레드시트 소프트웨어는 일반적으로 셀의 시작 부분에 `=`를 사용하여 계산을 수행해야 하며, 스프레드시트 함수가 평가되어야 함을 나타냅니다.

`&&` 연산자를 사용해 `if` 문에 두 번째 조건을 추가하여 `value`의 첫 번째 문자가 `"="`인지도 확인하세요.

# --hints--

`&&` 연산자를 사용해 `if` 문에 두 번째 조건을 추가하여 `value`의 첫 번째 문자가 `"="`인지도 확인하세요. `[0]`, `.startsWith()`, 또는 `.charAt(0)`을 사용할 수 있습니다.
