...
이 알고리즘에서는 `while` 반복문을 사용하세요. 이 반복문에는 두 가지 조건이 필요합니다:

- 첫째, 배열의 시작 부분(`j`으로 접근)보다 앞서 실행되지 않아야 합니다.
- 둘째, 현재 값보다 작은 값을 찾으면 반복문이 더 이상 실행되지 않아야 합니다.

무한 반복을 방지하려면 반복문 안에서 `j`를 감소시키세요.
...
`while` 반복문의 첫 번째 조건은 `j`의 값이 `0`보다 크거나 같은지 확인해야 합니다.
...
`while` 반복문에는 `array[j]`의 값이 `currValue`보다 큰지 확인하는 조건이 있어야 합니다.
...
`while` 반복문은 반복문 안에서 `j`를 감소시켜야 합니다.
