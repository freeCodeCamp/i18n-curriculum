...
Jetzt ist es Zeit, die Optionsschaltflächen anzupassen. Standardmäßig haben Optionsschaltflächen ein natives Browser-Design. Sie möchten dieses entfernen, um ein eigenes Aussehen zu erstellen.

Wählen Sie das Element mit der Klasse `contact-method-radio-btn` aus. Setzen Sie `appearance` auf `none`.

# --hints--

Sie sollten das Element mit der Klasse `contact-method-radio-btn` ansprechen.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Setzen Sie die Eigenschaft `appearance` auf `none`.
