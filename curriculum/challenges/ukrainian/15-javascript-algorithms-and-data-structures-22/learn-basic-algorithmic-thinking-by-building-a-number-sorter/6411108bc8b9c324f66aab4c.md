...
Оголосіть змінну `currValue` та присвоїте до неї значення `array[i]`. Потім оголосіть змінну `j` та присвойте до неї `i - 1`. Змінна `j` має бути пере присвоюваною.

# --hints--

Оголосіть змінну `currValue` за допомогою `const`.
...
Присвоїте `currValue` значення за місцем `array[i]`.
...
Оголосіть змінну `j` за допомогою `let`.
