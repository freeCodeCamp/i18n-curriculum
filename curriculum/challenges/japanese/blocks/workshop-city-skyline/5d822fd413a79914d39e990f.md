...
ウィンドウはセクションの左側で紫色の建物の後ろに重なっています。`.building-wrap` の下に新しいクラス `window-wrap` を追加してください。`.window-wrap` をフレックスボックスのコンテナにし、`align-items` と `justify-content` のプロパティを使って子要素を縦方向に中央揃えし、親要素内で均等に配置してください。

# --hints--

`.window-wrap` セレクターを作成してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".window-wrap"));
```

`.window-wrap` に `display` を `flex` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".window-wrap")?.display, "flex");
```

`.window-wrap` に `align-items` を `center` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".window-wrap")?.alignItems, "center");
```

`.window-wrap` に `justify-content` を `space-evenly` に設定してください。
