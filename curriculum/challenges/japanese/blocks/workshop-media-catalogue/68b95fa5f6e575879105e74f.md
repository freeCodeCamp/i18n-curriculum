...
`MediaError` クラスが完成したので、`if` メソッド内の `add` 文を修正してください。`TypeError` を同じメッセージを使った `MediaError` に置き換えます。

また、メッセージの後に `media_item` を2番目の引数として渡してください。これにより、例外オブジェクトにエラーの原因となったアイテムを格納できます。

# --hints--

`MediaError` が `Only Movie or TVSeries instances can be added` またはそのサブクラスのインスタンスでない場合、メッセージ `media_item` を使って `Movie` を発生させる必要があります。
...
`media_item` の2番目の引数として `MediaError` を渡す必要があります。
...
`MediaError` が有効なアイテムの場合は `media_item` を発生させてはいけません。
...
`add` メソッドは `TypeError` を発生させてはいけません。
