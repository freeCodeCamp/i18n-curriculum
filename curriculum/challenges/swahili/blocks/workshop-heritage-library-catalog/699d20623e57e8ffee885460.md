...
Kabla ya kuamini data iliyochambuliwa, anza kwa kuthibitisha sehemu zinazohitajika.

Tengeneza kitendakazi kinachoitwa `validateEntry` kinachopokea `entry` kama kigezo. Tangaza `let isValid = true`.

Ongeza ukaguzi wa uthibitisho kwa `title`: weka `isValid = false` ikiwa `title` haipo, ni thamani ya siyo kweli, au ni sawa na `"Unknown"`.

Rudisha `isValid`. Utaongeza ukaguzi wa sehemu zingine katika hatua zinazofuata.

# --hints--

`validateEntry` inapaswa kurudisha thamani ya kweli au siyo kweli.

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })` inapaswa kurudisha `true`.

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})` inapaswa kurudisha `false`.

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })` inapaswa kurudisha `false`.

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })` inapaswa kurudisha `false`.
