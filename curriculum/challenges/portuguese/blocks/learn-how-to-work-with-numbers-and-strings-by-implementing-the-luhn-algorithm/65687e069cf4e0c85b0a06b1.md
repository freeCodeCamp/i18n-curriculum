...
Fora da declaração `if`, adicione `number` a `sum_of_even_digits`.Além disso, remova a chamada `print`.

# --hints--

Você deve ter `sum_of_even_digits += number` dentro do loop `for`.
...
Você não deve ter `print(number)` dentro do loop `for`.
...
Você não deve ter `sum_of_even_digits += number` dentro da declaração `if`.
