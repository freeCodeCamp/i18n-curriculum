...
Друга умова в цьому прикладі перевіряє, чи `a.name > b.name`. Якщо так, функція повертає `1`, яка сортує перший фрукт після другого.
...
У функцію зворотного виклику додайте ще одну інструкцію `if`, щоб перевірити, чи `a.title` є більшим за `b.title`. Якщо так, поверніть число `1`.

# --hints--

Функція зворотного виклику має містити другу інструкцію `if`.
...
Інструкція `if` повинна перевіряти, чи `a.title` більше `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
```

Інструкція `if` має повернути число `1`, якщо `a.title` більше `b.title`.
