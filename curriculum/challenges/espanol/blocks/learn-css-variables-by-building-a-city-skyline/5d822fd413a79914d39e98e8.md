...
Agrega las mismas propiedades y valores de `display`, `align-items` y `justify-content` a `.foreground-buildings` que usaste en `.background-buildings`. Nuevamente, esto usará Flexbox para espaciar uniformemente los edificios a lo largo del fondo de su contenedor.

# --hints--

Debe dar a `.foreground-buildings` una propiedad `display` de `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.display, 'flex');
```

Debe dar a `.foreground-buildings` una propiedad `align-items` de `flex-end`.
