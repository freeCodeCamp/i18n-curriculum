...
Fügen Sie unter Ihrem letzten `p`-Element ein weiteres `p`-Element mit dem Text `Dietary Fiber 4g` hinzu. Geben Sie dem `p`-Element die `class`, die notwendig ist, um es einzurücken, und entfernen Sie den Trennlinien-Rahmen. Erstellen Sie dann unter diesem `p`-Element eine Trennlinie.
...
Ihr neues `p`-Element sollte den Text `Dietary Fiber 4g` enthalten.
