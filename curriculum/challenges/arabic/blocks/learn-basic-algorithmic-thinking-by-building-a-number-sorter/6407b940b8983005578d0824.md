...
تحتاج إلى الحصول على القيم من عناصر `select` الخاصة بك. هذه القيم ستكون حاليًا سلاسل نصية وستحولها إلى أرقام.

استخدم الدالة `map` للتكرار على المصفوفة. مرر دالة استدعاء راجع إلى `map` تأخذ معلمة `dropdown` وتُرجع `dropdown.value`.

# --hints--

يجب ربط طريقة `map` مباشرة على مصفوفة البيانات الخاصة بك.

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.getElementsByClassName\(\s*('|"|`)values-dropdown\1\s*\)\s*\]\s*\.map\(/);
```

يجب أن تستخدم دالة الاستدعاء الراجعة لـ `map` صيغة الدالة السهمية.

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.getElementsByClassName\(\s*('|"|`)values-dropdown\1\s*\)\s*\]\s*\.map\s*\(\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

يجب أن تأخذ دالة الاستدعاء الراجعة لـ `map` معلمة `dropdown`.
...
يجب أن تُرجع دالة الاستدعاء الراجعة لـ `map` ضمنيًا القيمة `dropdown.value`.
