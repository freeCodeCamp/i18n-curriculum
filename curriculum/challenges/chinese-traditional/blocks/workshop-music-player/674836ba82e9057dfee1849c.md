...
接著，使用 `classList` 屬性和 `add()` 方法，為 `playButton` 元素添加 `playing` 類別。這會在 CSS 檔案中尋找 `playing` 類別並將其添加到 `playButton` 元素。

然後，將 `userData.currentSong` 設定為 `song`，最後要播放歌曲，請對 `audio` 變數使用 `play()` 方法。`play()` 是來自網頁音訊 API 用於播放 mp3 檔案的方法。

# --hints--

你應該在你的 `playSong` 函式中，為 `playButton` 元素添加 `playing` 類別。
...
你應該在你的 `playSong` 函式中將 `userData.currentSong` 設定為 `song`。
...
你應該在你的 `playSong` 函式中對 `audio` 變數呼叫 `play()` 方法。
