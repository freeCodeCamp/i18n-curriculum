...
في هذا المشروع ستتعلم الطرق الأساسية للسلاسل النصية والمصفوفات من خلال بناء تطبيق مشغل موسيقى. ستتمكن من تشغيل الأغاني، إيقافها مؤقتًا، تخطيها، وتشغيلها بشكل عشوائي.

تم توفير HTML وCSS لهذا المشروع لك، حتى تركز على JavaScript.

ابدأ بالوصول إلى العناصر `#playlist-songs`، `#play`، و`#pause` باستخدام الطريقة `getElementById()`. عيّنها إلى المتغيرات `playlistSongs`، `playButton`، و`pauseButton` على التوالي.


# --hints--

يجب أن يكون لديك متغير يسمى `playlistSongs`.

```js
assert.isDefined(playlistSongs);
```

يجب أن تستخدم `document.getElementById()` للحصول على العنصر `#playlist-songs`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

يجب أن تعيّن العنصر `#playlist-songs` إلى المتغير `playlistSongs`.

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

يجب أن يكون لديك متغير يسمى `playButton`.

```js
assert.isDefined(playButton);
```

يجب أن تستخدم `document.getElementById()` للحصول على العنصر `#play`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

يجب أن تعيّن العنصر `#play` إلى المتغير `playButton`.

```js
assert.equal(playButton, document.getElementById('play'));
```

يجب أن يكون لديك متغير يسمى `pauseButton`.

```js
assert.isDefined(pauseButton);
```

يجب أن تستخدم `document.getElementById()` للحصول على العنصر `#pause`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

يجب أن تعيّن العنصر `#pause` إلى المتغير `pauseButton`.
