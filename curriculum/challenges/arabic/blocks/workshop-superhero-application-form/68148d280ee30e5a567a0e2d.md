...
احذف نص `Hello` داخل الـ `label`، ثم أنشئ `input` من نوع `checkbox` بقيمة `value` تساوي `power`.

أخيرًا، اعرض القيم من المصفوفة بجانب كل مربع اختيار بإنشاء عنصر `span` قبل إغلاق الـ `label` مباشرة مع محتوى نصي `{power}`.

# --hints--

يجب حذف نص `Hello` من عنصر الـ `label`.

```js
assert.notMatch(code, /hello/i)
```

يجب إنشاء `input` يكون `type` الخاص به مضبوطًا على `checkbox` و`value` مضبوطًا على `power` كأول عنصر فرعي داخل المسمى.

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

يجب إنشاء عنصر `span` بمحتوى نصي `{power}` كآخر عنصر فرعي داخل الـ label.
