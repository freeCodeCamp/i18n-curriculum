...
الإعدادات الافتراضية لـ CSS Grid ستُنشئ صفوفًا إضافية حسب الحاجة، على عكس Flexbox. أضف إلى المحدِّد `.social-icons` خاصية `grid-template-columns` بقيمة `repeat(5, 1fr)` لترتيب الأيقونات في صف واحد.

# --hints--

يجب أن يحتوي المحدِّد `.social-icons` على خاصية `grid-template-columns` بقيمة `repeat(5, 1fr)`.
