...
Der Rest der Songs wurde für Sie dem `allSongs`-Array hinzugefügt.

In vorherigen Lektionen haben Sie die <dfn>Web Audio API</dfn> kennengelernt und wie Sie sie zum Abspielen von Songs verwenden. Alle modernen Browser unterstützen die Web Audio API, mit der Sie Audio in Webanwendungen erzeugen und verarbeiten können.

Erstellen Sie eine Variable mit dem Namen `audio` und weisen Sie ihr den Wert `new Audio()` zu. Dadurch wird ein neues HTML5-`audio`-Element erstellt.

# --hints--

Sie sollten eine Variable mit dem Namen `audio` haben.

```js
assert.isDefined(audio);
```

Sie sollten eine neue Instanz des `Audio`-Objekts der Variablen mit dem Namen `audio` zuweisen.
