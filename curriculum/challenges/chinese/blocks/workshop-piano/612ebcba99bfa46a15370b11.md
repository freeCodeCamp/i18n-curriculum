...
`@media` @ 规则，也被称为媒体查询，用于根据指定条件应用 CSS。 媒体查询通常基于视口宽度的 `max-width` 和 `min-width` 属性来应用指定 CSS 。

在下面的示例中，当视口为 `960px` 宽及以下时，内边距将应用于 `.card` 类。
...
添加一个媒体查询，该查询将在视口宽度为 `768px` 及以下时应用。
...
你的 `@media` 查询的 `max-width` 应该为 `768px`。
