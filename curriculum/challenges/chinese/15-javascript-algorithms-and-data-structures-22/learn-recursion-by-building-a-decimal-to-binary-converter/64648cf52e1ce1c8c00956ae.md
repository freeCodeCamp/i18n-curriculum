...
首先，将 `result` 的 `innerText` 属性设置为 `"Call Stack Animation"`。

# --hints--

你应该将 `result` 的 `innerText` 属性设置为字符串 `"Call Stack Animation"`。

```js
assert.match(String(showAnimation), /result\.innerText\s*=\s*('|"|`)Call Stack Animation\1/i);
```

调用 `showAnimation` 函数后，`result` 的 `innerText` 属性应为 `"Call Stack Animation"`。
