...
到目前為止，`.face` 和 `.chin` 元素具有相同的 `background-color`。

創建一個名為 `--penguin-face` 的訂製 CSS 屬性，並將其設定為 `white`。
