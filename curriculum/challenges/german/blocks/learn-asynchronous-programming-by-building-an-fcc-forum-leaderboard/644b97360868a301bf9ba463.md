...
Zerlegen Sie im `if`-Ausdruck `className` und `category` aus dem `allCategories[id]`-Objekt.

# --hints--

Sie sollten `className` und `category` aus dem `allCategories[id]`-Objekt zerlegen. Die Eigenschaften können in beliebiger Reihenfolge stehen.
