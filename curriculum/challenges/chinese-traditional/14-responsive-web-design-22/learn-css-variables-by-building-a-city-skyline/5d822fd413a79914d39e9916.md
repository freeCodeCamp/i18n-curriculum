...
將 `repeating-linear-gradient` 添加到 `.fb1c` 中，角度爲 `90deg`，從 `0%` 到 `10%` 處爲 `--building-color4`，從 `10%` 到 `15%` 處爲 `transparent`。

# --hints--

你應該給 `.fb1c` 一個 `background` 和一個 `repeating-linear-gradient`。
...
你應該在從 `0%` 到 `10%` 處使用第一種顏色 `--building-color4`。
...
你應該在從 `10%` 到 `15%` 處使用第二種顏色 `transparent`。
