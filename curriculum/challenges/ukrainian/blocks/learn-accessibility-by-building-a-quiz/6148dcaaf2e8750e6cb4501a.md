...
Зробіть елементи `h3` пріоритетнішими, додавши такі CSS-властивості:
...
Задайте елементу `h3` проп `margin-top` зі значенням `5px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Задайте елементу `h3` проп `padding-left` зі значенням `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Задайте елементу `h3` проп `font-size` зі значенням `1.375ren`.
