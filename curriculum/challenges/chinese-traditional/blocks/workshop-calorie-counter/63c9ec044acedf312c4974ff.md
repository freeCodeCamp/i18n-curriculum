...
在你的迴圈完成後，你需要清除 `budgetNumberInput`。將 `budgetNumberInput` 的 `value` 屬性設為空字串。

# --hints--

你的 `clearForm` 函式（程式）應該存取 `budgetNumberInput` 元素的 `value` 屬性。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value/);
```

你的 `clearForm` 函式（程式）應該將 `budgetNumberInput` 元素的 `value` 屬性設為空字串。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value\s*=\s*('|"|`)\1/);
```

你應該在你的 `for` 迴圈之後修改 `budgetNumberInput`。
