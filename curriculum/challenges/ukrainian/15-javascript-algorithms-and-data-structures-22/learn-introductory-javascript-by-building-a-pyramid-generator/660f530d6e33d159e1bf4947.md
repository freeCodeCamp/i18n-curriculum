...
Якщо піраміда не перевернута, тоді вам захочеться мати блок `else`, який будує піраміду в звичайному порядку.

У попередніх кроках ви навчилися як працювати з інструкцією `else` ось так:
...
Блок `else` повинен бути порожнім.
