...
注意線條邊緣的灰色。那些邊緣稱為<dfn>邊框</dfn>。元素的每一側可以有不同的顏色，或者全部相同。

使用 `border-color` 屬性將 `hr` 元素的所有邊緣設為與其背景相同的顏色。

# --hints--

你應該將 `border-color` 屬性設為 `brown`。
...
你的 `hr` 元素應該有 `brown` 的 `border-color`。
