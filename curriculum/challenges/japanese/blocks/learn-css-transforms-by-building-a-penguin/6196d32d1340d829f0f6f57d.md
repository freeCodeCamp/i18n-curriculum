...
ページレイアウトでスペースを取らないように太陽の`position`プロパティを設定し、太陽の形が円になるように`border-radius`を設定してください。

# --hints--

`.sun`に`position`として`absolute`を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.position, 'absolute');
```

`.sun`に`border-radius`として`50%`を設定してください。
