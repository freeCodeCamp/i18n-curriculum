...
title: 'Aufgabe 66: Diophantische Gleichung'
challengeType: 1
forumTopicId: 302178
...
Betrachten Sie quadratische diophantische Gleichungen der Form:

<div style='text-align: center;'>x<sup>2</sup> – Dy<sup>2</sup> = 1</div>

Zum Beispiel, wenn D=13 ist, ist die minimale Lösung in x 649<sup>2</sup> – 13×180<sup>2</sup> = 1.

Es kann angenommen werden, dass es keine Lösungen in positiven ganzen Zahlen gibt, wenn D eine Quadratzahl ist.

Indem man minimale Lösungen in x für D = {2, 3, 5, 6, 7} findet, erhält man Folgendes:
...
Daher erhält man, wenn man minimale Lösungen in `x` für D ≤ 7 betrachtet, den größten `x`, wenn D=5 ist.

Finden Sie den Wert von D ≤ `n` in minimalen Lösungen von `x`, für den der größte Wert von `x` erreicht wird.
