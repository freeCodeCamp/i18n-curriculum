...
يمكن للتدرجات اللونية استخدام عدد غير محدود من الألوان كما يلي:
...
أضف `linear-gradient` إلى `.bb1d` مع `orange` كلون أول، و`--building-color1` كلون ثاني، و`--window-color1` كلون ثالث. تذكر استخدام التدرج على خاصية `background`.

# --hints--

يجب استخدام خاصية `background` على `.bb1d`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background);
```

يجب أن تعطي خاصية `background` قيمة `linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'linear-gradient');
```

يجب استخدام `orange` كلون أول في `linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.getPropVal('background', true), 'linear-gradient(orange');
```

يجب استخدام `--building-color1` كلون ثاني في `linear-gradient`.
...
يجب استخدام `--window-color1` كلون ثالث في `linear-gradient`.
