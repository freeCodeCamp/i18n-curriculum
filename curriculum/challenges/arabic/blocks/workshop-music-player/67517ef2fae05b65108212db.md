...
استخدم `getElementById()` لاختيار الأغنية التي تُشغل حاليًا: مرر قالب نصي ليضيف بادئة `song-` إلى `id`. عيّن النتيجة إلى متغير باسم `songToHighlight`.

# --hints--

لا تعدل دالة `highlightCurrentSong` الموجودة ومحتواها.
...
يجب أن تستخدم `document.getElementById()` وتمرر `` `song-${userData.currentSong?.id}` ``.

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

عيّن نتيجة `getElementById()` إلى متغير باسم `songToHighlight`.
