...
Dovresti aggiungere un elemento `div` all'interno di `.penguin-head`. Dovresti avere `--fcc-expected--` elementi `div` invece di `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 3);
```

Dovresti dare al `div` un valore `class` di `chin`.
