...
أنشئ محدِّد `body`. عرّف خاصية `font-family` إلى `Baskervville` مع النظام البديل `serif`. عرّف خاصية `color` إلى `linen` وخاصية `background-color` إلى `rgb(20, 30, 40)`.
...
يجب أن يحتوي محدِّد `body` على خاصية `font-family` معرّفة إلى `Baskervville` مع النظام البديل `serif`.
...
يجب أن يحتوي محدِّد `body` على خاصية `color` معرّفة إلى `linen`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.color === 'linen');
```

يجب أن يحتوي محدِّد `body` على خاصية `background-color` معرّفة إلى `rgb(20, 30, 40)`.
