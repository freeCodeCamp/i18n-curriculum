...
All'interno della funzione `fetchMoreAuthors`, imposta le variabili `startingIndex` e `endingIndex` su `+= 8` ciascuna.
...
Dovresti impostare la variabile `startingIndex` su `+=8`.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Dovresti impostare la variabile `endingIndex` su `+=8`.
