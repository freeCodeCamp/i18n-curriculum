...
Фото гравця завантажується з жорстко закодованої URL-адреси. Замініть значення атрибута `src` з `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` на `player.imageUrl`.

Також замініть жорстко закодований текст `alt` на динамічне значення, використовуючи шаблонний літерал з `player.name` та текстом `headshot`.

Зверніть увагу, що коли ви використовуєте JavaScript-вираз як значення атрибута в TSX, ви використовуєте фігурні дужки `{}` замість лапок. Наприклад, `src={player.imageUrl}`, а не `src="{player.imageUrl}"`.

# --hints--

Ви повинні замінити жорстко закодовану URL-адресу зображення на `player.imageUrl` як значення атрибута `src`.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

Ви повинні замінити жорстко закодований текст `alt` на шаблонний літерал, використовуючи `player.name` та текст `headshot`.
