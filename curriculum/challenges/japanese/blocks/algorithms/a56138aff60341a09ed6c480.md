...
2次元配列に保存されている在庫を、新しい納品の2次元配列と比較して更新してください。現在の既存在庫アイテムの数量（`arr1`内）を更新します。アイテムが見つからない場合は、新しいアイテムと数量を在庫配列に追加してください。返される在庫配列は、アイテム名のアルファベット順で並べ替えられている必要があります。
...
`updateInventory([[21, "Bowling Ball"], [2, "Dirty Sock"], [1, "Hair Pin"], [5, "Microphone"]], [[2, "Hair Pin"], [3, "Half-Eaten Apple"], [67, "Bowling Ball"], [7, "Toothpaste"]])`は長さ6の配列を返す必要があります。
