...
قبل بدء المحاكاة، تحتاج إلى كائن لتتبع عدد الحضور الذين يعالجهم كل بوابة عبر جميع العلامات الدقيقة. استدعِ `initializeThroughput()` مع `gates` وقم بتعيين النتيجة إلى متغير باسم `throughputSummary`.

# --hints--

يجب استدعاء `initializeThroughput()` مع `gates` وتعيين النتيجة إلى متغير باسم `throughputSummary`.
...
يجب أن تستدعي دالة `simulateFestival` دالة `initializeThroughput()` مع `gates`.
