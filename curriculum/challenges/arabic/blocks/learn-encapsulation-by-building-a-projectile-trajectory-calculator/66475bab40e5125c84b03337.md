...
الآن بعد أن حصلت على الإحداثيات المقربة، ابحث عن القيمة العظمى بين جميع إحداثيات x والقيمة العظمى بين جميع إحداثيات y.

ستكون هذه القيم العظمى عدد الصفوف والأعمدة في الرسم البياني. احفظ الأولى في متغير جديد باسم `x_max` والثانية في متغير باسم `y_max`.

أرجع `x_max, y_max`.

# --hints--

يجب أن تُرجع الطريقة `x_max, y_max`.
...
يجب أن يحتوي المتغير `x_max` على أعلى قيمة لـ x.
...
يجب أن يحتوي المتغير `y_max` على أعلى قيمة لـ y.
