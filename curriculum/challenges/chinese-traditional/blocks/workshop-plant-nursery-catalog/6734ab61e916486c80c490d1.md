...
當與鍵相關的值是物件時，`get` 方法會傳回該物件的引用。你可以使用該引用來修改映射中的物件。

要測試它，將 `getBallerina` 的 `small` 屬性設為不等於 `20` 的整數。然後，查看瀏覽器主控台以確認你的目錄中的值已更改。

# --hints--

你應該將 `getBallerina` 的 `small` 屬性設為不等於 `20` 的整數。
