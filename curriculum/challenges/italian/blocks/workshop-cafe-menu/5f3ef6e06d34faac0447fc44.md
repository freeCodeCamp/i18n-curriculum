...
Metti in corsivo il `Est. 2020` creando un selettore di classe `established` e impostando su di esso una proprietà `font-style` con valore `italic`.
...
Dovresti impostare la proprietà `font-style` su `italic`.
...
Il tuo selettore `.established` dovrebbe impostare la proprietà `font-style` su `italic`.
