...
在工作坊的最后一部分，你将复习如何获取一组数字的最小值和最大值。

首先添加一个 `console.log()`，将信息 `"The Math.max() and Math.min() methods are used to get the maximum and minimum number from a range."` 记录到控制台。

# --hints--

你应该将信息 `"The Math.max() and Math.min() methods are used to get the maximum and minimum number from a range."` 记录到控制台。
