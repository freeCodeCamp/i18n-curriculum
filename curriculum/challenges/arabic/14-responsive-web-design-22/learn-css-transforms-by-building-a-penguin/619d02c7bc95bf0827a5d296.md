...
يجب عليك استخدام منتقي `.face.right`.

```js
assert.match(code, /\.face\.right\s*\{/);
```

يجب عليك إعطاء عنصر `.face.right` خاصية الـ `right`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face.right')?.right);
```

بداخل عنصر `.face.right` يجب أن تكون خاصية `right` بقيمة `--fcc-expected--`، ولكن وجدت بقيمة `--fcc-actual--`.
