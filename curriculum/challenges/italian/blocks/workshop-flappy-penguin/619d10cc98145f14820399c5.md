...
Sotto l'elemento `.blush.right`, aggiungi due elementi `div` ciascuno con un `class` di `beak`. Inoltre, dai al primo elemento `.beak` un `class` di `top`, e al secondo elemento `.beak` un `class` di `bottom`.

# --hints--

Dovresti aggiungere due elementi `div` all'interno di `.penguin-head`. Previsti --fcc-expected-- elementi `div`, ma trovati --fcc-actual--.

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 9);
```

Dovresti dare al primo nuovo `div` un `class` di `beak`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

Dovresti dare al secondo nuovo `div` un `class` di `beak`.

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div.beak'), 2);
```

Dovresti dare al primo nuovo `div` un `class` di `top`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

Dovresti dare al secondo nuovo `div` un `class` di `bottom`.
