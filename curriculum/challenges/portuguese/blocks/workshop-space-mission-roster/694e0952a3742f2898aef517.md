...
O controle da missão requer um método para copiar astronautas elegíveis para EVA de uma tripulação. Crie uma função vazia chamada `getEVAReadyCrew` que aceite um parâmetro `crew`.

# --hints--

Você deve criar uma função chamada `getEVAReadyCrew`.

```js
assert.isFunction(getEVAReadyCrew);
```

Sua `getEVAReadyCrew` função deve ter um parâmetro `crew`.
