...
`cleanInputString` 関数のテストが完了したら、コンソール文を両方とも削除してください。

# --hints--

コード内に `console.log(cleanInputString("+-99"));` という行があってはいけません。

```js
assert.notMatch(code, /console\.log\(\s*cleanInputString\(\s*('|")\s*\+\-9\s*9\s*\1\s*\)\s*\);?/)
```

コード内に `console.log("original string: ", str);` という行があってはいけません。
