...
Mwishowe, ongeza kifungu cha `else` kinachochapisha hatua inayoruhusiwa na wito `make_allowed_move`. Jaribu kubaini hoja sahihi.
...
Unapaswa kuwa na `print(f'Move {i + 1} allowed between {source} and {target}')` ndani ya blokki ya `else`.
...
Unapaswa kuwa na `make_allowed_move(source, target)` ndani ya blokki ya `else`.
