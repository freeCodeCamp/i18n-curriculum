...
Luego, asigna `userData.currentSong` a `song`, y para finalmente reproducir la canción, usa el método `play()` en la variable `audio`. `play()` es un método de la API de audio web para reproducir un archivo mp3.
