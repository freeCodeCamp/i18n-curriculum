...
Sarebbe bello mostrare dinamicamente le parole `Show Message` o `Hide Message` in base allo stato `isVisible`.

In precedenza, hai visto come lavorare con il render condizionale in linea. Ora applicherai una logica simile al testo del pulsante.

Ecco un esempio di utilizzo dell'operatore ternario per mostrare condizionatamente le parole `Hide` e `Show`:

```jsx
<button>{isMenuVisible ? "Hide" : "Show"} Menu</button>
```

Per l'ultimo passaggio del workshop, usa l'operatore ternario per mostrare condizionatamente le parole `Hide` o `Show` per il pulsante del messaggio. Metti la condizione prima del testo `Message` in modo che mostri `Show Message` o `Hide Message`.

E con quest'ultima modifica, il tuo workshop è completo! Prova il pulsante per vederlo in azione.

# --hints--

Quando `isVisible` è `false`, il testo del pulsante dovrebbe dire `Show Message`.
...
Quando `isVisible` è `true`, il testo del pulsante dovrebbe dire `Hide Message`.
