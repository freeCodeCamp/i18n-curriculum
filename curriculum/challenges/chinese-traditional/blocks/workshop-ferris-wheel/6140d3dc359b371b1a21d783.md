...
將艙室定位於輪子周圍的時間。選擇第一個 `.cabin` 元素。將 `right` 屬性設為 `-8.5%`，並將 `top` 屬性設為 `50%`。
...
你的 `.cabin:nth-of-type(1)` 選擇器應該有一個 `right` 屬性設定為 `-8.5%`。
...
你的 `.cabin:nth-of-type(1)` 選擇器應該有一個 `top` 屬性設定為 `50%`。
