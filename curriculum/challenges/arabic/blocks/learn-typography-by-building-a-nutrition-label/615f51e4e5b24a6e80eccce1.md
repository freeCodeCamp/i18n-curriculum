...
داخل عنصر `.calories-info`، أنشئ عنصر `div`. أعطِ هذا العنصر `div` خاصية `class` بقيمة `left-container`. داخل عنصر `div` الجديد، أنشئ عنصر `h2` يحتوي على النص `Amount per serving`. أعطِ عنصر `h2` خاصية `class` بقيمة `bold small-text`.

# --hints--

يجب أن تنشئ عنصر `div` جديد داخل عنصر `.calories-info`.

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.localName === 'div');
```

يجب أن يحتوي عنصر `div` الجديد على خاصية `class` بقيمة `left-container`.

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.classList.contains('left-container'));
```

يجب أن تنشئ عنصر `h2` جديد داخل عنصر `.left-container`.

```js
assert(document.querySelector('.left-container')?.children?.[0]?.localName === 'h2');
```

يجب أن يحتوي عنصر `h2` الجديد على خاصية `class` بقيمة `bold small-text`.
