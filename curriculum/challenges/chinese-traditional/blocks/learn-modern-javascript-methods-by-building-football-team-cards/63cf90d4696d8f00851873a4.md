...
在 `team` 屬性下方，為你的屬性添加四個屬性鍵值配對。

1. 一個名為 `sport` 的鍵和一個值為 `"Football"` 的字串。
2. 一個名為 `year` 的鍵和一個數值為 `1986`。
3. 一個名為 `isWorldCupWinner` 的鍵和一個設定為 `true` 的布林值。
4. 一個名為 `headCoach` 的新鍵，其值為一個空物件。

# --hints--

你的 `myFavoriteFootballTeam` 物件應該有一個 `sport` 屬性。

```js
assert.property(myFavoriteFootballTeam, 'sport');
```

你的 `sport` 屬性應該設定為 `Football`。

```js
assert.equal(myFavoriteFootballTeam.sport, 'Football');
```

你的 `myFavoriteFootballTeam` 物件應該有一個 `year` 屬性。
