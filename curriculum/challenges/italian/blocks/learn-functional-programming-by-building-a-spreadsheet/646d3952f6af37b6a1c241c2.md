...
Ora che la tua funzione `.map()` sta ricevendo il riferimento alla funzione `num => charRange(...).map(...)` restituita dalle chiamate curryficate `addCharacters`, itererà correttamente sugli elementi e passerà ogni elemento come `n` a quella funzione.

Noterai che non stai usando il parametro `match`. In JavaScript, è una convenzione comune prefissare un parametro non usato con un underscore `_`. Potresti anche lasciare il parametro vuoto così: `(, char1)` ma spesso è più chiaro nominare il parametro per una migliore leggibilità futura.

Prefissa il parametro `match` con un underscore.

# --hints--

Dovresti prefissare il parametro `match` con un underscore.
