...
Додайте ще один повторюваний градієнт до цієї будівлі; зробіть його таким самим, як той, що ви щойно додали, але не додавайте `90deg` напрямок і використовуйте колір свого вікна замість двох `transparent` кольорів.

# --hints--

Ви повинні додати `.fb6` другий `repeating-linear-gradient` у властивість `background`.
...
Ви повинні задати другому `repeating-linear-gradient` перший колір `--building-color3` від `0%` до `10%`.
...
Ви повинні задати другому `repeating-linear-gradient` другий колір `--window-color3` від `10%` до `30%`.
