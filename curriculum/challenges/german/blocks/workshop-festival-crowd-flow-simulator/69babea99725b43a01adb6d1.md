...
Zuerst müssen Sie die Anzahl der Teilnehmer ermitteln, die während des aktuellen Ticks ankommen. Greifen Sie auf den Wert im `queue`-Array des Eingangsports am Index `tickIndex` zu und weisen Sie ihn einer Variablen mit dem Namen `currentTickQueue` zu.

# --hints--

Sie sollten das Element an `gate.queue[tickIndex]` einer Variablen mit dem Namen `currentTickQueue` zuweisen.
