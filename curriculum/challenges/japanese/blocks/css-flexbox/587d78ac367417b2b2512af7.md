...
title: ツイート埋め込みでjustify-contentプロパティを使う
challengeType: 0
...
前回のチャレンジでは`justify-content`プロパティの例を示しました。ツイートの埋め込みでは、このプロパティを`.profile-name`要素内のアイテムの配置に適用できます。

# --instructions--

ヘッダーの`justify-content`要素にCSSプロパティ`.profile-name`を追加し、前回のチャレンジで使ったいずれかの値を設定してください。

# --hints--

`.follow-btn`はページ上にレンダリングされているはずです。広告ブロッカーなどの拡張機能はオフにしてください。
...
`.profile-name`要素には、`justify-content`プロパティを次のいずれかの値に設定してください：`center`、`flex-start`、`flex-end`、`space-between`、`space-around`、または`space-evenly`。
