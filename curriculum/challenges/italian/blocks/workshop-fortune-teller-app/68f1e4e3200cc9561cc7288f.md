...
Per la condizione dell'istruzione `if`, verifica se `target` non è un'istanza di `HTMLElement`. Poi al suo interno, aggiungi un `return`.

# --hints--

Dovresti controllare che nella tua istruzione `if` ci sia la condizione `!(target instanceof  HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

Dovresti avere un `return` all'interno della tua istruzione `if`.
