...
Du solltest `.penguin-body::before` ein `top` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

Du solltest `.penguin-body::before` ein `left` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.
