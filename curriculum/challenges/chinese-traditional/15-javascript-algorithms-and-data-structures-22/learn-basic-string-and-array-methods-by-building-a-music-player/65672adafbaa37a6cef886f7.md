...
現在你已經創建了按鈕，你需要爲其分配一個文本。 爲此，你需要使用 DOM 的 `createTextNode()` 方法。

`createTextNode()` 方法用於創建文本節點。 要使用它，你可以調用它並將文本作爲字符串傳入：
...
使用 `createTextNode()` 方法創建 `"Reset Playlist"` 文本，然後將其分配給 `resetText` 常量。
...
你應該使用 `document.createTextNode()` 來創建 `"Reset Playlist"` 文本。

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

你應該將創建的文本節點分配給 `resetText` 常量。
