...
Ihre `apply`-Funktion gibt die Tabellenkalkulationsfunktion zurück, wendet sie aber nicht tatsächlich an. Aktualisieren Sie `apply`, damit die Funktion aufgerufen wird. Übergeben Sie das Ergebnis des Aufrufs von `toNumberList` mit `args` als Argument.

# --hints--

Ihre `apply`-Funktion sollte die `spreadsheetFunctions[fn.toLowerCase()]`-Funktion aufrufen.
...
Sie sollten einen `toNumberList()`-Aufruf an Ihren `spreadsheetFunctions[fn.toLowerCase()]`-Aufruf übergeben.
...
Sie sollten `args` an Ihren `toNumberList()`-Aufruf übergeben.
