...
Ahora agrega un `div` con el `className` de `card-stats`. Dentro, agrega un `div` con el `className` de `stat-col`. Dentro de ese, agrega tres elementos `div` cada uno con el `className` de `stat-row`.

# --hints--

Debes agregar un `div` con el `className` de `card-stats` después de `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

Debes agregar un `div` con el `className` de `stat-col` dentro de `.card-stats`.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

Debes agregar tres elementos `div` cada uno con el `className` de `stat-row` dentro de `.stat-col`.
