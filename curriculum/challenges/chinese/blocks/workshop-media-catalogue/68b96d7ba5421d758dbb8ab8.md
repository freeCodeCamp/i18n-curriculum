...
创建另一个捕获 `MediaError as e` 的 `except` 子句。在 `except` 块内，使用 f-string 打印 `Media Error:` 后跟一个空格和错误信息。

# --hints--

你应该有一个 `except` 子句来捕获 `MediaError as e`。
...
你应该使用 f-string 在你新建的 `except` 块中打印 `Media Error:`，后跟一个空格和错误信息。
