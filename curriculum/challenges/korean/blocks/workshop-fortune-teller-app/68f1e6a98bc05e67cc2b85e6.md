...
`cardElement`라는 변수를 선언하고 `card_container` 클래스를 사용해 요소를 가져오세요. `closest` 선택자를 사용해 `target`에서 가져오세요.

# --hints--

`cardElement`라는 변수를 `showFortune` 안에 선언하세요.
...
`cardElement`는 `target?.closest`를 사용해 `.card_container`를 찾아야 합니다.
...
`.card_container`를 `closest`의 인수로 전달하세요.
