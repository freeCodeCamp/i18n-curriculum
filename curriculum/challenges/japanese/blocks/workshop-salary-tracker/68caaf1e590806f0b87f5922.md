...
setter の最初に、`if` が整数でも浮動小数点数でもない場合に、メッセージ `TypeError` を持つ `'salary' must be a number.` を発生させる `new_salary` 文を作成してください。

# --hints--

`if` の setter に `salary` 文があるはずです。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[0]`)) })
```

`TypeError` が整数でも浮動小数点数でもない場合に、メッセージ `'salary' must be a number.` を持つ `new_salary` を発生させるべきです。
...
`new_salary` が整数または浮動小数点数の場合は、例外を発生させてはいけません。
