...
داخل دالة البناء، أضف استدعاءات إلى `fetchCardsData()` و `initializeEventListeners()` باستخدام الكلمة المفتاحية `this`، لكي تُنفذ عند تهيئة الصنف.

طريقة `fetchCardsData` ستتعامل مع تحميل بيانات بطاقات التاروت من واجهة برمجة التطبيقات، و`initializeEventListeners` ستُعد معالجات نقر الأزرار. ستنشئ الطريقتين في الخطوات القادمة.

# --hints--

يجب استدعاء طريقة `fetchCardsData()` داخل `constructor`.
...
يجب استدعاء طريقة `initializeEventListeners()` داخل دالة البناء.
