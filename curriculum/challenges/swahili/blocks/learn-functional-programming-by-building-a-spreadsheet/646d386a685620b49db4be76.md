...
Fanya wito wa njia ya `.map()` kwenye wito wako wa `rangeFromString()`, ukitumia rejea ya `addCharacters` kama kitendakazi cha mwitiko.

# --hints--

Unapaswa kufanya wito wa njia ya `.map()` kwenye wito wako wa `rangeFromString()`.
...
Unapaswa kupitisha rejea ya `addCharacters` kama mwitiko kwa njia yako ya `.map()`.
