...
У функції `attack` знайдіть рядок коду, який оновлює змінну `monsterHealth`, та розмістіть її в блоці `if` з умовою, яка викликає функцію `isMonsterHit`.
...
Інструкція `if` має викликати `isMonsterHit` в умові.

```js
assert.match(attack.toString(), /if\s*\(\s*isMonsterHit\s*\(\s*\)/)
```

Розмістіть `monsterHealth -= weapons[currentWeaponIndex].power + Math.floor(Math.random() * xp) + 1` у блок `if`.
