...
`regex` に渡す `infixEval` 関数は、2つの数字の間に演算子があるパターンにマッチします。最初の数字はコールバック内で `arg1` に代入され、2番目の数字は `arg2` に、演算子は `operator` に代入されます。

コールバック関数は `operator` オブジェクトの `infixToFunction` プロパティを暗黙的に返すようにしてください。`operator` は実際のプロパティ名ではなく、プロパティ名を保持する変数であることを忘れないでください。

# --hints--

コールバックは暗黙の return を使うべきです。
...
コールバック関数は `infixToFunction` オブジェクトにアクセスする必要があります。
...
コールバック関数はブラケット記法を使って、`infixToFunction` パラメータの値に一致する `operator` オブジェクトのプロパティにアクセスする必要があります。
