...
`showResults` 함수를 완성하세요. `playerScoreSpanElement`과 `computerScoreSpanElement`는 플레이어와 컴퓨터의 업데이트된 점수를 보여주도록 수정해야 합니다.

`roundResultsMsg`도 라운드 결과로 업데이트해야 합니다.

**팁**

- 요소의 텍스트 내용을 업데이트하기 위해 `innerText` 속성을 다루는 방법을 배운 것을 기억하세요.
- 라운드 결과를 얻으려면 `getRoundResults` 함수를 사용할 수 있습니다.

# --hints--

`showResults` 함수는 라운드 결과로 `roundResultsMsg`을 업데이트해야 합니다.
...
`showResults` 함수는 컴퓨터의 업데이트된 점수를 보여주도록 `computerScoreSpanElement`을 업데이트해야 합니다.
...
`showResults` 함수는 플레이어의 업데이트된 점수를 보여주도록 `playerScoreSpanElement`을 업데이트해야 합니다.
