...
Agora, crie uma função chamada `getNextSong`.Faça com que retorne a música que pode ser encontrada em `getCurrentSongIndex()` mais um em `userData.songs`, ou `undefined` se a música atual for a última na playlist.

# --hints--

Você deve ter uma função chamada `getNextSong`.

```js
assert.isFunction(getNextSong);
```

Sua função `getNextSong` deve chamar `getCurrentSongIndex`.
...
Sua função `getNextSong` deve retornar a música seguinte à música atual.
...
Sua função `getNextSong` deve retornar `undefined` quando a música atual for a última da playlist.
