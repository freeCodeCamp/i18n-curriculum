...
Der Hauptunterschied zwischen `tr[class="total"]` und `tr.total` ist, dass das erste `tr`-Elemente auswählen wird, bei denen die *einzige* Klasse `total` ist. Das zweite wird `tr` Elemente auswählen, bei denen die Klasse `total` *enthält*.
...
Du solltest einen `tr.total td`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td'));
```

Dein `tr.total td`-Selektor sollte eine `text-align`-Eigenschaft auf `right` gesetzt haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td')?.getPropertyValue('text-align') === 'right');
```

Dein `tr.total td`-Selektor sollte eine `padding`-Eigenschaft auf `0 0.25rem` gesetzt haben.
