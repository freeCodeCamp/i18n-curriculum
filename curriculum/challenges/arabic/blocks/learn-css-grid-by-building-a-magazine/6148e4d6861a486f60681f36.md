...
يجب أن تكون الصور ضمن تخطيط عمودين وثلاثة صفوف.

أعط محدِّد `.image-wrapper` خاصية `grid-template-columns` بقيمة `2fr 1fr` وخاصية `grid-template-rows` بقيمة `repeat(3, min-content)`. هذا سيجعل صفوف الشبكة تتكيف في الارتفاع بناءً على المحتوى، بينما تبقى الأعمدة بعرض ثابت بناءً على الحاوية.

# --hints--

يجب أن يحتوي محدِّد `.image-wrapper` على خاصية `grid-template-columns` بقيمة `2fr 1fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns === '2fr 1fr');
```

يجب أن يحتوي محدِّد `.image-wrapper` على خاصية `grid-template-rows` بقيمة `repeat(3, min-content)`.
