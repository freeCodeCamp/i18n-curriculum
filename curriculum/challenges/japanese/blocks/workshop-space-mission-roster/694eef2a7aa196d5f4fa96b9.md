...
`getEVAReadyCrew()` を `updatedSquad` のロスターで呼び出し、その結果を `EVAReadySquad` という新しい変数にストアしてください。次に、`for` ループを使って、`name` 配列内のすべての宇宙飛行士の `EVAReadySquad` をログに出力してください。

# --hints--

`getEVAReadyCrew()` を `updatedSquad` で呼び出し、その結果を `EVAReadySquad` という新しい変数にストアしてください。
...
`for` ループを使って `EVAReadySquad` 配列を繰り返し処理し、各宇宙飛行士の `name` をログに出力してください。
