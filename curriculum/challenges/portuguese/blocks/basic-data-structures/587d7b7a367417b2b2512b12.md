...
O próximo método que abordaremos é `slice()`. Em vez de modificar um array, `slice()` copia ou *extrai* um número dado de elementos para um novo array, deixando o array no qual é chamado intacto. `slice()` aceita apenas 2 parâmetros — o primeiro é o índice no qual a extração deve começar, e o segundo é o índice no qual a extração deve parar (a extração ocorrerá até, mas não incluindo o elemento neste índice). Considere isto:
