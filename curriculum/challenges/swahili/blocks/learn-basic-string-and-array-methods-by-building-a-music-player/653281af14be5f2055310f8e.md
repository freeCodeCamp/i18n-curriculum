...
Kicheza muziki chako kinapaswa kufuatilia nyimbo, wimbo unaochezwa sasa, na muda wa wimbo huo. Ili kufanya hivyo, utahitaji kuunda kitu cha kuhifadhi taarifa hizi.

Anza kwa kutumia neno kuu la `let` kutangaza kigezo kipya kinachoitwa `userData` na ukaweke thamani ya kitu tupu.

# --hints--

Unapaswa kutumia neno kuu la `let` kuunda kigezo cha `userData`.

```js
assert.match(code, /let\s+userData\s*=/)
```

Unapaswa kuweka thamani ya kitu tupu kwa kigezo chako cha `userData`.
