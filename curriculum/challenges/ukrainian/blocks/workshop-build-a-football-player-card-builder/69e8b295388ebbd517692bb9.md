...
Заголовок картки завершено. Тепер потрібно додати розділ із фотографією гравця.

Після закриваючого `div` `card-header` і все ще всередині `div` з `className` `card`, додайте `div` з `className` `card-image-wrap`. Поки що залиште його порожнім. Зображення додасте на наступному кроці.

# --hints--

Ви повинні додати `div` з `className` `card-image-wrap`.
