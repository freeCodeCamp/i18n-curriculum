...
Adicione um conjunto de atributos `onChange` para uma arrow function que recebe `e` como seu parâmetro e faz uma chamada `setPlayer` com um spread de `player` e `position` definido para `e.target.value`, convertido para `Position`.

# --hints--

Sua posição `select` deve ter um manipulador `onChange` que faz uma chamada `setPlayer` com um spread de `player` e atualiza `position`.
