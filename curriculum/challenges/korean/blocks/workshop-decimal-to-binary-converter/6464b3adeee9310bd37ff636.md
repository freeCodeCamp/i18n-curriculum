...
`.getElementById()` 메서드를 사용해 현재 객체의 `id` 속성 값을 가진 `inputVal` 속성이 있는 요소를 선택하세요. 그런 다음, `.remove()` 메서드를 사용해 지연 후에 해당 요소를 DOM에서 제거하세요.
...
`.remove()` 메서드에 `.getElementById()` 메서드를 체인으로 연결하세요.
