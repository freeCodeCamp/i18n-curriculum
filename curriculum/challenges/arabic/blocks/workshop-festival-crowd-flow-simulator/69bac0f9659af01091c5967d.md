...
سيكون من المفيد وجود دالة لعرض ملخص الإنتاجية أثناء المحاكاة. أنشئ دالة فارغة باسم `printSummary` تحتوي على معلمة `summary`.

# --hints--

يجب أن تنشئ دالة باسم `printSummary`.

```js
assert.isFunction(printSummary);
```

يجب أن تحتوي دالتك `printSummary` على معلمة `summary`.
