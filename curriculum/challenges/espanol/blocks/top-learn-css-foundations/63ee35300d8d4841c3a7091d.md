...
En general, no puedes encadenar más de un selector de tipo porque un elemento no puede ser dos tipos diferentes al mismo tiempo. Por ejemplo, encadenar dos selectores de tipo como `div` y `p` nos daría el selector `divp`, que no funcionaría porque el selector intentaría encontrar un elemento literal `<divp>`, que no existe.
