...
Después del atributo `href`, establece el atributo `class` a la constante llamada `linkClass`.

Después del atributo `class`, establece el atributo `target` a `"_blank"`.

Por último, coloca la constante `linkText` entre las etiquetas de anclaje para mostrar el texto en el enlace.

# --hints--

Debes tener un atributo `class` después del atributo `href`.
...
Debes establecer el valor del atributo `class` a la constante llamada `linkClass`.
...
Debes tener un atributo `target` después del atributo `class`.
...
Debes establecer el valor del atributo `target` a `"_blank"`.
...
Debes colocar la constante `linkText` entre las etiquetas de anclaje.
