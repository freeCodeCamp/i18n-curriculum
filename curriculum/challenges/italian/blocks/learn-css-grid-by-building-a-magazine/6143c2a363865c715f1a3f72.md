...
All'interno dell'elemento `.image-wrapper`, assegna al tuo primo elemento `img` un attributo `src` con valore `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, un attributo `alt` con valore `image of the quote machine project`, un attributo `class` con valore `image-1`, un attributo `loading` impostato su `lazy`, un attributo `width` con valore `600` e un attributo `height` con valore `400`.

# --hints--

Il tuo primo elemento `img` dovrebbe avere un attributo `src` impostato su `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`.
...
Il tuo primo elemento `img` dovrebbe avere un attributo `alt` impostato su `image of the quote machine project`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('alt') === 'image of the quote machine project');
```

Il tuo primo elemento `img` dovrebbe avere un attributo `class` impostato su `image-1`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.classList?.contains('image-1'));
```

Il tuo primo elemento `img` dovrebbe avere un attributo `loading` impostato su `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('loading') === 'lazy');
```

Il tuo primo elemento `img` dovrebbe avere un attributo `width` impostato su `600`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('width') === '600');
```

Il tuo primo elemento `img` dovrebbe avere un attributo `height` impostato su `400`.
