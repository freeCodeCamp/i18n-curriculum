...
`.face` 요소를 `right` 클래스로 선택하고, 부모 요소의 오른쪽 `5%`에 위치시키세요.

# --hints--

`.face.right` 선택자를 사용하세요.

```js
assert.match(code, /\.face\.right\s*\{/);
```

`.face.right`에 `right` 속성을 지정하세요.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face.right')?.right);
```

`.face.right`에 `right`을 `--fcc-expected--`로 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.
