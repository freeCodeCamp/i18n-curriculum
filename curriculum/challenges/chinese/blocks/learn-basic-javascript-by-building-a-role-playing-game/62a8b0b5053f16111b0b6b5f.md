...
现在你可以合并一些代码。 首先从 `goTown` 函数内部复制代码并将其粘贴到 `update` 函数中。 然后，删除 `goTown` 和 `goStore` 函数内部的所有代码。

# --hints--

你的 `update` 函数应该将 `button1.innerText` 设置为 `"Go to store"`。

```js
assert.match(update.toString(), /button1\.innerText\s*=\s*('|")Go to store\1/);
```

你的 `update` 函数应该将 `button2.innerText` 设置为 `"Go to cave"`。

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*('|")Go to cave\1/);
```

你的 `update` 函数应该将 `button3.innerText` 设置为 `"Fight dragon"`。

```js
assert.match(update.toString(), /button3\.innerText\s*=\s*('|")Fight dragon\1/);
```

你的 `update` 函数应该将 `button1.onclick` 设置为 `goStore`。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*goStore/);
```

你的 `update` 函数应该将 `button2.onclick` 设置为 `goCave`。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*goCave/);
```

你的 `update` 函数应该将 `button3.onclick` 设置为 `fightDragon`。

```js
assert.match(update.toString(), /button3\.onclick\s*=\s*fightDragon/);
```

你的 `update` 函数应该将 `text.innerText` 设置为 `"You are in the town square. You see a sign that says \"Store\"."`。
