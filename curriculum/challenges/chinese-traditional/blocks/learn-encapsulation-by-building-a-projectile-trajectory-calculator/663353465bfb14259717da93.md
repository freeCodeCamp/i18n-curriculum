...
創建一個方法 `create_coordinates_table`，然後在程式碼結尾列印 `graph.create_coordinates_table())`。

該方法應該使用 `__coordinates` 屬性並傳回一個包含所有座標的字串，其中 x 和 y 始終保持相同位置，x 的值始終為整數，y 的值始終四捨五入到小數點後兩位：
...
你應該有一個 `create_coordinates_table()` 方法。
...
`Projectile(10, 3, 45)` 的 `create_coordiantes_table` 方法應該傳回 `'\n  x      y\n  0   3.00\n  1   3.90\n  2   4.61\n  3   5.12\n  4   5.43\n  5   5.55\n  6   5.47\n  7   5.19\n  8   4.72\n  9   4.05\n 10   3.19\n 11   2.13\n 12   0.87\n'`。
...
你應該列印 `graph.create_coordiantes_table()`。
