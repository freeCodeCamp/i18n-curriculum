...
`.social-icons` セレクターを作成してください。`display` プロパティに `grid` を設定し、`font-size` プロパティに `3rem` を設定してください。

# --hints--

`.social-icons` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

`.social-icons` セレクターには `display` プロパティが `grid` に設定されているはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

`.social-icons` セレクターには `font-size` プロパティが `3rem` に設定されているはずです。
