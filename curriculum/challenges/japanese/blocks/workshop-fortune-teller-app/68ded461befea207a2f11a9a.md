...
これまでに、`getElement`、`hideElements`、`showElements` の3つの関数があります。
次に、ランダムなおみくじを取得するのに役立つ関数が必要です。

`getRandomItem` という名前の関数を宣言してください。この関数は `items` の配列を引数として受け取ります。ジェネリクスを使ってください。

関数の中で、`Math.random()` と `Math.floor()` を使って `items` 配列からランダムなおみくじを選びます。
選んだおみくじを返すのを忘れないでください。

# --hints--

`getRandomItem` という名前の関数を宣言してください。
...
`getRandomItem` 関数はジェネリック型パラメータ `T` を使うべきです。
...
`getRandomItem` 関数は `items` という名前のパラメータを持ち、その型は `T[]` であるべきです。
...
`getRandomItem` 関数の戻り値の型は `T` であるべきです。
...
`index` という名前の変数を `getRandomItem` 関数の中で宣言してください。
...
`index` 変数は `Math.floor()` と `Math.random()` を `items.length` と組み合わせてランダムなインデックスを生成するのに使うべきです。
...
`getRandomItem` 関数は `items[index]` を返すべきです。
