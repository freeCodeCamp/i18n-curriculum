...
和之前一样，你应该验证你的 `input`。具体来说，如果 `size` 小于 `1`，你应该向控制台记录 `"Chunk size must be >= 1"`，然后调用 `return` 以退出函数。

# --hints--

如果 `size` 的 `input` 无效，你应该将以下错误信息记录到控制台，`"Chunk size must be >= 1"`。
...
当 `size` 大于或相等于 1 时，你的 `chunkCrew` 函数应通过验证且无误差。
