...
他の変数の下に `--building-color3` という名前の新しい変数を作成し、値を `#cc6699` にしてください。それから `background-color` クラスの `.bb3` として使い、フォールバック値を `pink` に設定してください。

# --hints--

`--building-color3` という名前の新しいプロパティ変数を定義してください。

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color3'));
```

`--building-color3` に `#cc6699` の値を与えてください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1')?.getPropertyValue('--building-color3')?.trim(), '#cc6699');
```

`background-color` の `.bb3` を設定してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb3')?.backgroundColor);
```

`background-color` 変数を使って `--building-color3` を設定し、フォールバックは `pink` にしてください。
