...
`calc()` 函數是一個 CSS 函數，它允許你根據其他值計算一個值。 例如，你可以使用它來計算視口的寬度減去元素的邊距：
...
給 `#years` 一個值爲 `0 -2px` 的 `margin`，和一個值爲 `0.5rem calc(1.25rem + 2px) 0.5rem 0` 的 `padding`。

# --hints--

你的 `#years` 選擇器應該有一個 `margin` 屬性，設置爲 `0 -2px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('margin') === '0px -2px');
```

你的 `#years` 選擇器應該有一個 `padding` 屬性，設置爲 `0.5rem calc(1.25rem + 2px) 0.5rem 0`。
