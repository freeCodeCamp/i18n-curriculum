---
id: 615f42a021625f656101ef93
title: Step 22
challengeType: 0
dashedName: step-22
---

# --description--

동등하게 중요한 요소들 사이의 가로 간격은 텍스트 가독성을 높일 수 있습니다.

텍스트 `2/3 cup (55g)`를 `span` 요소로 감싸세요.

# --hints--

새로운 `span` 요소를 만들어야 합니다.

```js
assert(document.querySelector('span'));
```

`span` 요소에는 텍스트 `2/3 cup (55g)`가 있어야 합니다.

```js
assert(document.querySelector('span')?.textContent?.trim() === '2/3 cup (55g)');
```

`p` 요소에는 여전히 텍스트 `Serving size 2/3 cup (55g)`가 있어야 합니다.

```js
assert(document.querySelectorAll('p')?.[1]?.innerText === 'Serving size 2/3 cup (55g)');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <title>Nutrition Label</title>
  <link href="https://fonts.googleapis.com/css?family=Open+Sans:400,700,800" rel="stylesheet">
  <link href="./styles.css" rel="stylesheet">
</head>

<body>
--fcc-editable-region--
  <div class="label">
    <h1 class="bold">Nutrition Facts</h1>
    <div class="divider"></div>
    <p>8 servings per container</p>
    <p class="bold">Serving size 2/3 cup (55g)</p>
  </div>
--fcc-editable-region--
</body>
</html>
```

```css
* {
  box-sizing: border-box;
}

html {
  font-size: 16px;
}

body {
  font-family: 'Open Sans', sans-serif;
}

.label {
  border: 2px solid black;
  width: 270px;
  margin: 20px auto;
  padding: 0 7px;
}

h1 {
  text-align: center;
  margin: -4px 0;
  letter-spacing: 0.15px
}

p {
  margin: 0;
}

.divider {
  border-bottom: 1px solid #888989;
  margin: 2px 0;
}

.bold {
  font-weight: 800;
}
```
