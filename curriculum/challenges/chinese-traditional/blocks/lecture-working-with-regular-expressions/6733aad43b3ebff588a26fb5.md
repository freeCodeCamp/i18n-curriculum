...
這就帶我們來到第一個方法，`test()` 方法。`test()` 方法存在於 `RegExp` 物件上，這些物件是用來表述正規表達式的物件（就像我們剛定義的那個）。
...
`test()` 方法會傳回一個 `boolean`，用來表示字串是否符合該正規表達式。
...
你可以看到 `replace()` 傳回已更新的字串，並且匹配的樣式已被 `replaced`。

正規表達式以及所有相關的方法看起來可能很複雜且令人不知所措，但你將有機會在接下來的課程中更深入探索它們。
