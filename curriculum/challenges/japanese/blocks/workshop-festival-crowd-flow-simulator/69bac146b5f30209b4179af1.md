...
これでフェスティバルをシミュレートする関数を作成できます。`simulateFestival` という名前の空の関数を、パラメータ `gates` と `timeBlock` で作成してください。

# --hints--

`simulateFestival` という名前の関数を作成してください。

```js
assert.isFunction(simulateFestival);
```

`simulateFestival` 関数にはパラメータ `gates` と `timeBlock` が必要です。
