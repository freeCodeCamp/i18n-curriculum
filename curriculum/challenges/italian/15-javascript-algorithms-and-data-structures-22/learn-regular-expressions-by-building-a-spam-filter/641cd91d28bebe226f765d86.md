...
Collega un event listener a `checkMessageButton`, in ascolto per l'evento `click`. Dagli una funzione callback vuota.

# --hints--

Dovresti chiamare `.addEventListener()` sull'elemento `checkMessageButton`.
...
Il metodo `.addEventListener()` dovrebbe avere una funzione callback vuota.
