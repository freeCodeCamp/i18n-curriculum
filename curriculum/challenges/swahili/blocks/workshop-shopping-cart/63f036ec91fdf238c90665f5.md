...
Sahihisha thamani ya `total` iwe jumla ya thamani za `subTotal` na `tax`.

# --hints--

Unapaswa kusahihisha thamani ya `total`. Kumbuka kutumia neno kuu la `this`.

```js
assert.match(cart.calculateTotal.toString(), /this\.total/);
```

Unapaswa kuweka thamani ya `total` iwe jumla ya thamani za `subTotal` na `tax`.
