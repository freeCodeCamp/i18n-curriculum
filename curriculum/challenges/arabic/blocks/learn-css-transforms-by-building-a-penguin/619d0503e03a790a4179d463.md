...
ضع عنصر `.chin` بحيث يكون على بعد `25%` من الأعلى، و`5%` من اليسار بالنسبة لوالده. ثم، امنح الزوايا العلوية انحناءً بنسبة `70%`، والزوايا السفلية انحناءً بنسبة `100%`.

# --hints--

يجب أن تعرّف خاصية `top` لعنصر `.chin`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

يجب أن تكون قيمة `top` لعنصر `.chin` هي `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

يجب أن تعرّف خاصية `left` لعنصر `.chin`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

يجب أن تكون قيمة `left` لعنصر `.chin` هي `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

يجب أن تعرّف خاصية `border-radius` لعنصر `.chin` بقيمة `70% 70% 100% 100%`.
