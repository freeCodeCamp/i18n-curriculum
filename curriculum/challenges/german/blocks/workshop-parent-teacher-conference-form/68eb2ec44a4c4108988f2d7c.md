...
Sie sollten Ihren benutzerdefinierten Optionsschaltflächen eine Größe und Form geben.

Zielen Sie weiterhin auf `.contact-method-radio-btn`, setzen Sie `width` auf `20px`, `height` auf `20px`, `border-radius` auf `50%` und `border` auf `2px solid gray`.

# --hints--

Sie sollten weiterhin die `.contact-method-radio-btn`-Klasse ansprechen.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Setzen Sie die `width`-Eigenschaft auf `20px`.
...
Setzen Sie die `height`-Eigenschaft auf `20px`.
...
Setzen Sie die `border`-Eigenschaft auf `2px solid gray`.
