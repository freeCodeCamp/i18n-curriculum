...
Você precisa encontrar o índice da tarefa que deseja excluir primeiro.

Crie uma variável `dataArrIndex` e defina seu valor usando o método `findIndex()` no array `taskData`.Passe `item` como parâmetro para a função callback de seta e dentro do callback, verifique se o `id` de `item` é igual ao `id` do `parentElement` de `buttonEl`.

# --hints--

Você não deve alterar a função `deleteTask`.
...
Você deve passar `item` como parâmetro do callback da função arrow `findIndex()`.

```js
assert.match(code, /const\s+deleteTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)/)
```

Sua função callback arrow deve verificar se `item.id === buttonEl.parentElement.id`. Não use chaves.
