...
Fügen Sie dem `catch`-Block einen `error`-Parameter hinzu und verwenden Sie dann `console.error`, um alle Fehler zu protokollieren, die beim Abrufen oder Verarbeiten der Daten auftreten.

# --hints--

Ihr `catch`-Block sollte einen `error`-Parameter haben.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

Sie sollten alle Fehler im `catch`-Block mit `console.error` protokollieren.
