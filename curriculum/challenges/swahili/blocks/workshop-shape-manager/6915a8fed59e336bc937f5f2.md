...
The last set of elements to select are the result elements. Use your `getElement` function to get the element with the ids `result-text` and `result-card`. Both should use `HTMLElement` as the type.

Remember you have already declared the `resultText` and `resultCard` variables for them, so you only need to assign them.
