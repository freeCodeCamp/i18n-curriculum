...
O caractere curinga `.` captura qualquer caractere. O curinga também pode ser chamado de `dot` e `period`. Você pode usar o curinga como qualquer outro caractere na regex. Por exemplo, se você quiser encontrar `hug`, `huh`, `hut` ou `hum`, você pode usar a regex `/hu./` para capturar todas as quatro palavras.
