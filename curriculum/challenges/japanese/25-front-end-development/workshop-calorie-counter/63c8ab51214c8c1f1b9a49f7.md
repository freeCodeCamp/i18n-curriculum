...
作成した関数内で、`calories` という変数を宣言し、値 `0` を割り当ててください。 この変数は後で再代入する予定なので、宣言には `let` を使ってください。

# --hints--

変数 `calories` を宣言してください。

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

変数 `calories` の値は `0` にしてください。

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

変数 `calories` は `let` で宣言してください。
