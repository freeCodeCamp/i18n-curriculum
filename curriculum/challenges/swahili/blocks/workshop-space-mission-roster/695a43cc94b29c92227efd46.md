...
Umeongeza wito wa `console.log()` kwenye script yako kuthibitisha safu ya data ya `EVAChunks`, na sasa wito huo hauhitajiki tena. Ili kuweka terminal safi kwa hatua zinazofuata, futa kipengele cha `for loop` kinachorekodi data kutoka `EVAChunks`.

# --hints--

Unapaswa kufuta kipengele cha `for loop` kinachorekodi data kutoka `EVAChunks`. Futa kipengele kizima chenye mizunguko yote miwili.
