...
JSX から覚えているように、波括弧を使うと任意の JavaScript 式を JSX 内に直接埋め込めます。これがプロップスから動的なデータを表示する方法です。TSX でも同じことをします。

カードのヘッダーはまだハードコードされた値を表示しています。`98` 内のテキスト `card-rating` を `player.overallRating` に置き換え、`ST` 内の `card-position` を `player.position` に置き換えてください。

# --hints--

`98` 内のハードコードされた `player.overallRating` を `.card-rating` に置き換えてください。

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

`ST` 内のハードコードされた `player.position` を `.card-position` に置き換えてください。
