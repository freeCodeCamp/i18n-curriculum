...
最後に、ボタンのスタイルを追加しましょう。 まず、`cursor` プロパティを `pointer` に設定してください。 次に、文字色 `color` を `#0a0a23` に、背景色 `background-color` を `#feac32` に設定してください。

その後、`background-image` プロパティを `linear-gradient(#fecc4c, #ffac33)` に設定してください。 最後に、`border` を `3px solid #feac32` に設定してください。
...
`button` セレクターの `cursor` プロパティを `pointer` に設定してください。
...
`button` セレクターの `color` プロパティを `#0a0a23` に設定してください。
...
`button` セレクターの `background-color` プロパティを `#feac32` に設定してください。
...
`button` セレクターの `background-image` プロパティを `linear-gradient(#fecc4c, #ffac33)` に設定してください。
...
`button` セレクターの `border` プロパティを `3px solid #feac32` に設定してください。
