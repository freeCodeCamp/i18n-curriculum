...
Agora adicione um `div` com o `className` de `form-group`. Dentro dele, adicione um `label` de `className` de `label` e um atributo `htmlFor` de `name`. Abaixo disso, adicione um `input` com um conjunto `id` definido para `name`, um `className` de `input` e um atributo `type` de `text`.
