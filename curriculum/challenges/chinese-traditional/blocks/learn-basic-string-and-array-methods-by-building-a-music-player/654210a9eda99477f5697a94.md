...
創建一個帶有類別 `playlist-song-info` 的 `button` 元素。在 `button` 內部，添加一個帶有類別 `playlist-song-title` 的 `span` 元素，然後將 `song.title` 插入為文字。

# --hints--

你應該創建一個帶有類別 `playlist-song-info` 的 `button`。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

你應該在你的 `button` 元素內創建一個 `span` 元素。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

你的 `span` 元素應該有 `playlist-song-title` 類別。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

你應該將 `song.title` 插值為你的 `span` 元素的內容。
