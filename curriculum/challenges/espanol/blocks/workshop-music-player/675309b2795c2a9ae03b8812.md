...
Observa que la carátula del álbum en el HTML y las canciones en el arreglo `userData.songs` han cambiado. Hemos intercambiado las canciones originales por otras más cortas que puede usar para probar su aplicación.

Todas las funciones principales ahora están en lugar. El único problema ahora es que la siguiente canción no se reproduce automáticamente cuando la canción que se está reproduciendo termina.

Para solucionar eso, puede establecer un escucha de eventos que detecte cuando la canción que se está reproduciendo termina. El escucha de evento `"ended"` es apropiado para esto. Se activa cuando la reproducción de un medio alcanza el final.

Agregue un escucha de eventos al elemento `audio` que escuche el evento `"ended"` y pase una referencia a la función `playNextSong`. ¡Con eso, el proyecto del reproductor de música está completo!

# --hints--

Debe encadenar el método `addEventListener()` a su variable `audio`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

El escucha de eventos que usó en su variable `audio` debería escuchar un evento `"ended"`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

Debe pasar una referencia a `playNextSong` a su escucha de eventos `"ended"`.
