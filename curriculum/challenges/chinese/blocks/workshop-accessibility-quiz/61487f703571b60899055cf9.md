...
将 `section` 元素的宽度设置为其父容器的 `80%`。 然后使用外边距来居中 `section` 元素，将底部外边距设置为 `10px`。

另外，确保 `section` 元素的宽度不能大于 `600px`。
