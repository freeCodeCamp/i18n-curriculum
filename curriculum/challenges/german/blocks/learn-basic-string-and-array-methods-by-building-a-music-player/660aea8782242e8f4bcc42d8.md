...
Genau wie reguläre Funktionen können Pfeilfunktionen mehrere Parameter akzeptieren.

Hier ist ein Beispiel für eine benannte Pfeilfunktion mit einem Parameter:
...
Wenn die Funktion nur einen Parameter hat, können Sie die runden Klammern um die Parameterliste weglassen, so:
...
Erstellen Sie eine neue benannte Pfeilfunktion namens `printMessage`, die einen Parameter namens `org` hat. Fügen Sie im Körper dieser Funktion eine Konsolenausgabe hinzu. Innerhalb dieser Konsolenausgabe fügen Sie den Template-String `${org} is awesome!` ein.

Rufen Sie unterhalb Ihrer `printMessage`-Funktion die Funktion auf und übergeben Sie den String `"freeCodeCamp"` als Argument.

Öffnen Sie die Konsole, um das Ergebnis zu sehen.

# --hints--

Sie sollten eine `printMessage`-Funktion haben.

```js
assert.isFunction(printMessage);
```

Sie sollten die Syntax der Pfeilfunktion verwenden.

```js
assert.match(code, /(let|const)\s+printMessage\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Ihre `printMessage`-Funktion sollte einen Parameter namens `org` haben.
...
Ihre `printMessage`-Funktion sollte eine Konsolenausgabe enthalten.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

Ihre Konsolenausgabe sollte den Template-String `${org} is awesome!` enthalten.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

Sie sollten die `printMessage`-Funktion aufrufen und den String `"freeCodeCamp"` übergeben.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

Ihr Funktionsaufruf sollte unterhalb der Definition Ihrer `printMessage`-Funktion stehen.
...
Ihr `printMessage`-Funktionsaufruf sollte `freeCodeCamp is awesome!` in der Konsole ausgeben.
