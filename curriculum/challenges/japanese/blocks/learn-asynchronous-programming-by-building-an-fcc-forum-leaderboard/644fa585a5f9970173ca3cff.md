...
`linkClass` という定数を作成し、テンプレートリテラルを代入してください。そのテンプレートリテラルの中に `category ${selectedCategory.className}` の値を追加してください。

これらのクラス名はアンカー要素のスタイルを適用するために使われます。

# --hints--

`linkClass` という名前の定数があるはずです。

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

`linkClass` 変数にテンプレートリテラルを代入してください。

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

テンプレートリテラルの中に `category ${selectedCategory.className}` の値があるはずです。
