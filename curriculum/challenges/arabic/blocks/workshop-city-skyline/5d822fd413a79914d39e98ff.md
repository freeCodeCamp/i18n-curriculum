...
بعد ذلك، احذف `width` و `height` من `.bb2a`، وغيّر `border-left` و `border-right` لاستخدام `5vw` بدلاً من `1vw`. سيصبح حجم العنصر صفرًا وستلتقي الحدود في المنتصف.

# --hints--

يجب حذف `width` من `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.width);
```

يجب حذف `height` من `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.height);
```

يجب تغيير `border-left` لاستخدام `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid rgb(153, 153, 153)");
```

يجب تغيير `border-right` لاستخدام `5vw`.
