...
أعلن دالة سهمية `rangeFromString` تأخذ معلمتين، `num1` و `num2`. يجب أن تُرجع الدالة ضمنيًا نتيجة استدعاء `range` مع `num1` و `num2` كمعلمات.

لتكون آمنًا، حوّل `num1` و `num2` إلى أعداد صحيحة أثناء تمريرهما إلى `range`.

# --hints--

يجب أن تعلن متغير `rangeFromString` بعد `rangeRegex` الخاص بك.
...
يجب أن تستخدم `const` لإعلان متغير `rangeFromString` الخاص بك.
...
يجب أن يكون متغير `rangeFromString` دالة سهمية.
...
يجب أن تحتوي دالة `rangeFromString` على `num1` كأول معلمة.
...
يجب أن تحتوي دالة `rangeFromString` على `num2` كمعلمة ثانية.
...
يجب أن تستخدم دالة `rangeFromString` إرجاعًا ضمنيًا.
...
يجب أن تُرجع دالة `rangeFromString` نتيجة استدعاء دالتك `range`.
...
يجب أن تستدعي `parseInt` مع `num1` كمعلمة وتمرر النتيجة إلى استدعاء `range`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

يجب أن تستدعي `parseInt` مع `num2` كمعلمة وتمرر النتيجة إلى استدعاء `range`.
