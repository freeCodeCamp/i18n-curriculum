...
Seti ya mwisho ya vipengele vya kuchagua ni vipengele vya matokeo. Tumia kitendakazi chako cha `getElement` kupata kipengele chenye vitambulisho `result-text` na `result-card`. Vyote vinapaswa kutumia `HTMLElement` kama aina.

Kumbuka tayari umetangaza vigezo vya `resultText` na `resultCard` kwa ajili yao, kwa hivyo unahitaji tu kuwaweka thamani.

# --hints--

Unapaswa kuweka thamani kwa kigezo cha `resultText`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` inapaswa kuchagua kipengele cha `result-text`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

Unapaswa kutumia `HTMLElement` kama aina ya `getElement` unayotumia kuchagua kipengele cha `result-text`.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

Unapaswa kuweka thamani kwa kigezo cha `resultCard`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` inapaswa kuchagua kipengele cha `result-card`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

Unapaswa kutumia `HTMLElement` kama aina ya `getElement` unayotumia kuchagua kipengele cha `result-card`.
