...
Você deve ter um seletor `.daily-value p:not(.no-divider)`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p:not(.no-divider)'));
```

Você não deve ter um seletor `.daily-value p`.

```js
assert(!new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

Você não deve alterar as propriedades no seletor `.daily-value p:not(.no-divider)`.
