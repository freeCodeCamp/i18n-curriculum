...
现在，创建一个 `if` 语句，用来查看 `value` 是否小于 `0`。如果为真，说明输入的值是负数。于是，使用 alert 向用户显示消息 `Negative values are not allowed.`，然后通过调用 `clearInputFields()` 函数清空输入字段。

在 `if` 语句下，调用 `updateResult` 函数重新计算面积。

# --hints--

你应该在 `if` 函数内创建一个条件为 `value < 0` 的 `handleInput` 语句。

```js
assert.match(handleInput.toString(), /if\s*\(\s*value\s*<\s*0\s*\)\s*\{\s*/)
```

你应该在 `Negative values are not allowed.` 语句内使用 alert 显示 `if`。

```js
assert.match(handleInput.toString(), /if\s*\(\s*value\s*<\s*0\s*\)\s*\{\s*(window.)?alert\(["']Negative\s+values\s+are\s+not\s+allowed\.["']\);?\s*/)
```

你应该在 `clearInputFields` 语句内调用 `if` 函数。
