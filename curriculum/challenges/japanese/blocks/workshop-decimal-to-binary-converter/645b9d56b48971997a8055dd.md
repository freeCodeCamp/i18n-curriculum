...
`if` 文の本体内で、`innerText` の `result` プロパティを文字列 `"0"` に設定してください。その後、早期に `return` 文を使って関数から抜け出してください。

# --hints--

`innerText` の `result` プロパティを文字列 `"0"` に設定する必要があります。

```js
assert.match(String(decimalToBinary), /if\s*\(\s*input\s*===?\s*0\s*\)\s*\{\s*result\.innerText\s*=\s*('|"|`)\s*0\s*\1/);
```

`innerText` 要素の `result` を設定した後、早期に `return` 文を使って関数から抜け出してください。
