...
이전에 `__eq__` 메서드에 대해 했던 것과 같은 방식으로, `if`와 `self`가 같은 클래스에 속하지 않는지 확인하고 `other`를 반환하는 `NotImplemented` 문을 만드세요.

# --hints--

`if`와 `self`가 같은 클래스에 속하지 않는지 확인하는 `other` 문을 만드세요.
...
`NotImplemented` 본문에서 `if`를 반환해야 합니다.
