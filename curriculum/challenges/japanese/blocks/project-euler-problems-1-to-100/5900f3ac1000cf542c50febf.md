...
すべての平方根は連分数として表すと周期的であり、次の形で書けます：

$\\displaystyle \\quad \\quad \\sqrt{N}=a_0+\\frac 1 {a_1+\\frac 1 {a_2+ \\frac 1 {a3+ \\dots}}}$

例えば、$\\sqrt{23}$を考えてみましょう：
...
このシーケンスが繰り返されていることがわかります。簡潔に表すために、$\\sqrt{23}=\[4;(1,3,1,8)]$という表記を使い、(1,3,1,8)のブロックが無限に繰り返されることを示します。

最初の10個の（無理数の）平方根の連分数表現は次の通りです：
...
$N \\le 13$の範囲で、奇数周期を持つ連分数はちょうど4つあります。

$N \\le n$の範囲で奇数周期を持つ連分数はいくつありますか？
