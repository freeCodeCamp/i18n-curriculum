...
`players` 配列の中に、以下のプロパティを持つオブジェクトを新しく作成してください:
...
_注_: チームメンバーの番号は、名字のアルファベット順になっています。 これは、背番号とは異なります。

# --hints--

`myFavoriteFootballTeam.players` 配列にオブジェクトを追加してください。
...
`name` プロパティの値を `"Sergio Almirón"` にしてください。
...
`position` プロパティの値を `"forward"` にしてください。
...
`isCaptain` プロパティの値をブール値にしてください。

```js
assert.isBoolean(myFavoriteFootballTeam.players[0]?.isCaptain);
```

`isCaptain` プロパティの値を `false` にしてください。
...
`nickname` プロパティの値を `null` にしてください。
