...
La propiedad `place-items` puede usarse para establecer los valores `align-items` y `justify-items` al mismo tiempo. La propiedad `place-items` acepta uno o dos valores. Si se proporciona un valor, se usa tanto para las propiedades `align-items` como `justify-items`. Si se proporcionan dos valores, el primer valor se usará para la propiedad `align-items` y el segundo valor se usará para la propiedad `justify-items`.

Dale al selector `.image-wrapper` una propiedad `place-items` establecida en `center`.

# --hints--

Tu selector `.image-wrapper` debe tener una propiedad `place-items` establecida en `center`.
