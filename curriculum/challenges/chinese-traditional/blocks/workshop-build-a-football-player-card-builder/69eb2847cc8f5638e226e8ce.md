...
在 `PlayerCard` 裡被引用的 `FootballPlayerCard` 中，接受 `player` prop，並暫時傳入一個具有以下屬性的物件：
...
你應該在 `player` 裡向 `PlayerCard` 傳入 `FootballPlayerCard` prop。
...
你的 `player` prop 應該有一個 `name` 屬性，值為 `'PELE'`。
...
你的 `player` prop 應該有一個 `overallRating` 屬性，值為 `98`。
...
你的 `player` prop 應該有一個 `position` 屬性，值為 `'ST'`。
...
你的 `player` prop 應該有一個 `club` 屬性，值為 `'Santos FC'`。
...
你的 `player` prop 應該有一個 `imageUrl` 屬性，值為 Pele 影像 URL。
...
你的 `player` prop 應該有一個 `pac` 屬性，值為 `97`。
...
你的 `player` prop 應該有一個 `sho` 屬性，值為 `98`。
...
你的 `player` prop 應該有一個 `pas` 屬性，值為 `83`。
...
你的 `player` prop 應該有一個 `dri` 屬性，值為 `99`。
...
你的 `player` prop 應該有一個 `def` 屬性，值為 `41`。
...
你的 `player` prop 應該有一個 `phy` 屬性，值為 `75`。
