...
在之前的课程中，你已经学习了如何使用 `audio` 和 `video` 元素。在本实验中，你将构建一个多媒体播放器，该播放器将显示一个 `audio` 音轨和带有文字记录的 `video`。
...
8. 在 `video` 元素内，你应该有一个带有指向视频文件的 `src` 属性和一个 `type` 属性的 `source` 元素。如果你愿意，可以使用此视频 URL：`https://cdn.freecodecamp.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4`
9. 在 `source` 元素下，你应该有一个带有指向字幕文件的 `src` 属性、一个 `kind` 属性、一个 `srclang` 属性和一个 `label` 属性的 `track` 元素。如果你愿意，可以使用此字幕 URL：`https://cdn.freecodecamp.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.vtt`
10. 在第三个 `section` 元素内，你应该有一个用于该部分标题的 `h2` 元素，例如 `"Transcript"`。
