...
前のレッスンでは、`map()` メソッドを使ってオプションのリストをレンダリングする方法を学びました。
...
`key` は常にユニークである必要があることを覚えておいてください。これは React がどのアイテムが変更され、追加または削除されたかを識別するのに役立ちます。

最初の `option` の後、アロー関数を使って `powerSourceOptions` 配列を `source` をパラメータとして map してください。

次に `map()` メソッドの中で、`option` 要素を `key` が `source` で `value` が `source` のものとして作成します。最後に `{source}` を使ってドロップダウン内の各電源の名前を表示してください。

# --hints--

`powerSourceOptions` を map する必要があります。

```js
assert.match(code, /\{\s*powerSourceOptions\s*\.\s*map\s*\(/)
```

map のパラメータとして `source` を渡してください。

```js
assert.match(code, /\{\s*powerSourceOptions\s*\.\s*map\s*\(\s*(source|\(\s*source\s*\))\s*=>/)
```

`option` 要素を `key` が `source` で `value` が `source` のものとして作成してください。
...
最後の 6 つの `option` 要素のテキスト内容は、`powerSourceOptions` 配列の電源オプションのいずれかに対応している必要があります。
