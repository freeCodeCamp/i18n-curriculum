...
لعرض نتائج البيانات، سجّل المتغير `data` في وحدة التحكم داخل كتلة `try` الخاصة بك.

أسفل تعريف `fetchData`، استدعِ الدالة وافتح وحدة التحكم لرؤية النتائج.

# --hints--

يجب أن تستخدم `console.log` لتسجيل المتغير `data` في وحدة التحكم.

```js
assert.match(code, /\s*console\.log\(\s*data\s*\)/);
```

يجب أن يكون `console.log` داخل كتلة `try`.

```js
assert.match(code, /\s*try\s*{[\s\S]*\s*console\.log\(\s*data\s*\)[\s\S]*}\s*catch\s*\(\s*err\s*\)\s*{\s*}/);
```

يجب استدعاء الدالة `fetchData()` بعد تعريف الدالة `fetchData()`.
