...
Наступний крок — пройтися циклом по масиву `posters`, щоб отримати всі їхні аватари.

Почніть із додавання ключового слова `return`, за яким слідує `posters.map()`. Для функції зворотного виклику додайте параметр з ім’ям `poster`.

# --hints--

Ви повинні повернути `posters.map()`.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

У вашої функції зворотного виклику має бути параметр із назвою `poster`.
