...
将 `parseInt(numberInput.value)` 的所有实例替换为 `inputInt`。

# --hints--

你应该将 `isNaN(parseInt(numberInput.value))` 替换为 `isNaN(inputInt)`，并将 `parseInt(numberInput.value) < 0` 替换为 `inputInt < 0`。
...
你应该将 `parseInt(numberInput.value) === 5` 替换为 `inputInt === 5`。
...
你应该将 `decimalToBinary(parseInt(numberInput.value))` 替换为 `decimalToBinary(inputInt)`。
...
你的 `checkUserInput()` 函数应该只包含一个 `parseInt(numberInput.value)` 实例。
