...
Als Nächstes müssen Sie den aktuellen Songtitel und den Künstler im Player-Display anzeigen. Verwenden Sie `const` und die Pfeilsyntax, um eine leere `setPlayerDisplay`-Funktion zu erstellen.

# --hints--

Sie sollten `const` verwenden, um eine leere Funktion mit dem Namen `setPlayerDisplay` zu erstellen.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` sollte eine Funktion sein.

```js
assert.isFunction(setPlayerDisplay)
```

Ihre `setPlayerDisplay`-Funktion sollte die Pfeilsyntax verwenden.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

Ihre `setPlayerDisplay`-Funktion sollte leer sein.
