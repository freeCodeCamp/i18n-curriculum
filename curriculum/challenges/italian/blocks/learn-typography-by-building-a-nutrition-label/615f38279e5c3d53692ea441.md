...
Raggruppa i tuoi elementi `h1` e `p` in un elemento `div`. Dai a quell'elemento `div` un attributo `class` impostato su `label`.
...
Il tuo nuovo elemento `div` dovrebbe avere l'attributo `class` impostato su `label`.

```js
assert(document.querySelector('div')?.classList?.contains('label'));
```

I tuoi elementi `h1` e `p` dovrebbero essere all'interno del nuovo elemento `.label`.
