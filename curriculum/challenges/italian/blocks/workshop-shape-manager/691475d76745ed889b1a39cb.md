...
Dovresti anche lavorare su una funzione che cancelli i campi di input.

Crea una `clearInputFields` funzione. Al suo interno, usa `querySelectorAll` per ottenere tutti i campi di input e iterali usando `forEach()`. All'interno del `forEach()`, imposta il valore di ogni `input` su una stringa vuota.

# --hints--

Dovresti creare una `clearInputFields` funzione.

```js
assert.isFunction(clearInputFields)
```

La tua `clearInputFields` funzione dovrebbe impostare ogni campo di input su una stringa vuota.
