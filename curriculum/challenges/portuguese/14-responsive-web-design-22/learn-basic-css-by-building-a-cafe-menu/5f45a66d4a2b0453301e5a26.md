...
Descendo para o elemento `footer`, faça com que todo o texto tenha um valor de `14px` para o tamanho da fonte.

# --hints--

Você deve ter um seletor `footer`.
...
O seletor `footer` deve estar abaixo do comentário.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Você deve definir a propriedade `font-size` como `14px`.
...
O elemento `footer` deve ter a `font-size` de `14px`.
