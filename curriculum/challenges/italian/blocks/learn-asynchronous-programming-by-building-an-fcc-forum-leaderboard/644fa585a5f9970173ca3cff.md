...
Crea una costante chiamata `linkClass` e assegnale un letterale template. All’interno di quel letterale template, aggiungi il valore di `category ${selectedCategory.className}`.

Questi nomi di classe saranno usati per definire lo stile dell’elemento àncora.

# --hints--

Dovresti avere una costante chiamata `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

Dovresti assegnare un letterale template alla tua variabile `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

Dovresti avere il valore di `category ${selectedCategory.className}` all’interno del letterale template.
