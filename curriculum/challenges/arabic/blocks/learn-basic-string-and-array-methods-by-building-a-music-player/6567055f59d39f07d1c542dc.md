...
بعد ذلك، تحتاج إلى تحديث قائمة التشغيل في مستند HTML لعرض الأغاني.

عيّن `songsHTML` إلى الخاصية `innerHTML` لعنصر `playlistSongs`. سيُدرج هذا عنصر `li` الذي أنشأته للتو داخل عنصر `ul` في ملف الـ `HTML` المقدم بالفعل.

# --hints--

يجب استخدام الخاصية `innerHTML` لإضافة `songsHTML` إلى `playlistSongs`.
