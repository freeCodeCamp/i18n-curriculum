...
O Mission Control requer mais uma função para registrar um resumo da sua tripulação. Crie uma função vazia chamada `printCrewSummary` que aceite um parâmetro `crew`.

# --hints--

Você deve criar uma função chamada `printCrewSummary`.

```js
assert.isFunction(printCrewSummary);
```

Sua `printCrewSummary` função deve ter um parâmetro `crew`.
