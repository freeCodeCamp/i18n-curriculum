...
2つの`make_allowed_move()`節の中で再び`elif`関数を呼び出し、正しい引数を渡してください。

# --hints--

最初の`make_allowed_move(source, auxiliary)`節の中で`elif`を呼び出す必要があります。
...
2番目の`make_allowed_move(auxiliary, target)`節の中で`elif`を呼び出す必要があります。
