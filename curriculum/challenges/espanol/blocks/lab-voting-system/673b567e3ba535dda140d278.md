...
`addOption("Egypt")` debería devolver `Option "Egypt" added to the poll.`.
...
Añadir una opción vacía debería devolver `"Option cannot be empty."`.

```js
assert.equal(addOption(''), 'Option cannot be empty.');
```

Cuando `Turkey` ya está añadida, `addOption("Turkey")` debería devolver `Option "Turkey" already exists.`.
...
Cuando `Malaysia` existe en las opciones de votación, `vote("Malaysia", "traveler1")` debería devolver `Voter traveler1 voted for "Malaysia".`.
...
Cuando `traveler1` intenta votar por `Algeria` de nuevo, `vote("Algeria", "traveler1")` debería devolver `Voter traveler1 has already voted for "Algeria".`.
...
Cuando `Nigeria` no está en las opciones de votación, `vote("Nigeria", "traveler2")` debería devolver `Option "Nigeria" does not exist.`.
