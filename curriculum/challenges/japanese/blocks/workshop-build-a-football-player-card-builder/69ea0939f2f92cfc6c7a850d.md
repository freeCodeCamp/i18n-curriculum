...
次に、`useEffect` の状態が変わるたびに実行される `player` フックを追加してください。その中でアロー関数を使い、`try..catch` ブロックのパラメータとして `error` を使った `catch` を追加してください。

# --hints--

`useEffect` の中で `FootballPlayerCard` を呼び出す必要があります。
...
`useEffect` には `[player]` を依存関係の配列として設定してください。

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

`useEffect` のコールバックには、`try...catch` ブロックのパラメータとして `error` を使った `catch` ブロックを含めてください。
