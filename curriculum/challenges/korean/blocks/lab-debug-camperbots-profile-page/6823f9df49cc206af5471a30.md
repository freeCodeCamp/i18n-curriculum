...
Camperbot의 모든 오류를 고쳐서 프로필 페이지를 계속 만들 수 있게 하세요. 아래 사용자 스토리의 항목을 완료하고 "Check Your Code"를 눌러 모든 오류가 고쳐졌는지 확인하세요.

**사용자 스토리:**

1. Camperbot이 `heading2` 요소를 사용하려고 하지만, 그 요소는 존재하지 않습니다. 올바른 두 번째 수준 제목 요소를 사용하도록 태그를 고치세요.
2. Camperbot이 `pp`로 두 개의 단락을 추가하려고 하지만, 그것들도 존재하지 않습니다. 올바른 단락 태그를 사용하도록 고치세요.
3. Camperbot이 `Background and Interests` 하위 제목에 `h3` 요소를 사용하고 있는데, 구문 오류가 있습니다. 문제를 찾아 해결하세요.
