...
API에서 데이터를 가져왔으니, `results` 배열에 있는 각 과일의 `setResults` 속성만 포함하는 배열을 전달하여 `name`을 호출하고 `data` 상태를 업데이트하세요. 이때 `map()` 메서드를 사용하세요.

# --hints--

`setResults`를 `data.map(fruit => fruit.name)`로 업데이트해야 합니다.
