...
Baada ya kipengele cha `card-image-wrap`, ongeza `div` yenye `className` ya `card-name-strip`. Ndani yake, ongeza `span` yenye `className` ya `card-name` na maandishi ya `PELE`.

# --hints--

Unapaswa kuongeza `div` yenye `className` ya `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

Unapaswa kuongeza `span` yenye `className` ya `card-name` ndani ya `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

Kipengele cha `.card-name` kinapaswa kuwa na maandishi ya `PELE`.
