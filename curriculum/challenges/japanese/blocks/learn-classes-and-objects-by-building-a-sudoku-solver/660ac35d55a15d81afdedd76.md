...
`for` ループの中で、変数 `row_str` を宣言し、`row` を反復して `i` の各アイテム `row` を文字列に変換するリスト内包表記を代入してください。そのために `str()` 関数を使います。

# --hints--

変数 `row_str` を宣言してください。
...
`row` を反復するリスト内包表記を `row_str` 変数に代入してください。
...
`row_str` 変数に代入されたリスト内包表記は、`i` を反復変数として使うべきです。
...
`row_str` 変数に代入されたリスト内包表記は、`str()` の各アイテム `i` に対して `row` を呼び出すべきです。
