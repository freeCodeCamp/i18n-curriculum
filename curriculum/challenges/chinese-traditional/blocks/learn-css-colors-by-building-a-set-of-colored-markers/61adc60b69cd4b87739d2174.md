...
創建一個新的 CSS 規則，標的為類別 `sleeve`。將 `width` 屬性設定為 `110px`，並將 `height` 屬性設定為 `25px`。

# --hints--

你應該使用類別選擇器來標的類別 `sleeve`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.sleeve'));
```

你的 `.sleeve` CSS 規則應該有一個 `width` 屬性設定為 `110px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.sleeve')?.width === '110px');
```

你的 `.sleeve` CSS 規則應該有一個 `height` 屬性設定為 `25px`。
