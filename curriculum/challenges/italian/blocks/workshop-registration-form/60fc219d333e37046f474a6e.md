...
Seleziona solo gli elementi `.inline` e assegna loro `width` di `unset`. Questo rimuoverà la regola precedente che impostava tutti gli elementi `input` su `width: 100%`.
...
Dovresti dare agli elementi `.inline` un `width` di `unset`.
