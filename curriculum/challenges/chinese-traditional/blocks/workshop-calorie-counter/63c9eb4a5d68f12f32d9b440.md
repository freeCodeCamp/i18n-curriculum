...
請記住 `document.querySelectorAll` 傳回的是一個 `NodeList`，它類似陣列但不是陣列。然而，`Array` 物件有一個 `.from()` 方法，可以接受類陣列並傳回陣列。當你想存取更強固的陣列方法時，這會很有幫助，你將在未來的專案中學習這些方法。

以下範例將 `li` 元素的 `NodeList` 轉換為 `li` 元素的陣列：
...
將你的 `inputContainers` 查詢選擇器包裹在 `Array.from()` 中。請在你的宣告同一行完成此操作。

# --hints--

你應該將你的 `querySelectorAll` 包裝在 `Array.from()` 中。

```js
assert.match(clearForm, /Array\.from\(\s*document\.querySelectorAll\(\s*('|")\.input-container*\1\s*\)\s*\)/);
```

你應該將 `Array.from()` 的結果指定到你的 `inputContainers` 變數。
