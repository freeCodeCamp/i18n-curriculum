...
為了使輸入框彼此對齊，請創建一個新的規則集，標的為 `.info` 元素內所有的 `input` 和 `label` 元素，並將 `display` 屬性設為 `inline-block`。

此外，將 `label` 元素的文字對齊到右側。

# --hints--

你應該使用 `.info > label, .info > input` 或 `.info label, .info input` 選擇器。
...
你應該給 `input` 和 `label` 元素設定 `display` 為 `inline-block`。
...
你應該為 `label` 元素設定 `text-align` 為 `right`。
