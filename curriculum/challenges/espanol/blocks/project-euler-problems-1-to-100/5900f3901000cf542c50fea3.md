...
title: 'Problema 36: Palíndromos de doble base'
challengeType: 1
forumTopicId: 302020
...
El número decimal, 585 = 1001001001<sub>2</sub> (binario), es palindrómico en ambas bases.

Encuentra la suma de todos los números, menores que `n`, donde 1000 ≤ `n` ≤ 1000000, que son palindrómicos en base 10 y base 2.

(Ten en cuenta que el número palíndromo, en cualquiera de las bases, no puede incluir ceros a la izquierda.)

# --hints--

`doubleBasePalindromes(1000)` debería devolver un número.

```js
assert(typeof doubleBasePalindromes(1000) === 'number');
```

`doubleBasePalindromes(1000)` debería devolver 1772.
