...
Erstellen Sie eine Funktion namens `playPreviousSong`. Verwenden Sie innerhalb Ihrer neuen Funktion den von der `getPreviousSong`-Funktion zurückgegebenen Wert, um den abzuspielenden Song zu finden. Wenn `getPreviousSong()` `undefined` ist, spielen Sie den ersten Song in Ihrer Wiedergabeliste ab. Andernfalls spielen Sie den Song vor dem aktuell abgespielten Song.

# --hints--

Sie sollten eine Funktion namens `playPreviousSong` haben.

```js
assert.isFunction(playPreviousSong);
```

Die `playPreviousSong`-Funktion sollte den Song vor dem aktuell abgespielten Song in der Wiedergabeliste abspielen.
...
Wenn `getPreviousSong()` `undefined` ist, sollte die `playPreviousSong`-Funktion den ersten Song in der Wiedergabeliste abspielen.
