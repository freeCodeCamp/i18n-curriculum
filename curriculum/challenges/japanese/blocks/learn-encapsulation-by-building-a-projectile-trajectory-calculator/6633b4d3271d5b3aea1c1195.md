...
次はグラフの番です。`create_trajectory` というメソッドを作成し、コードの最後にある `print` の呼び出しを `print(graph.create_trajectory())` に置き換えてください。

この新しいメソッドの最初のステップとして、座標のローカルコピーを作成しますが、すべての値を整数に丸めてください。この新しい座標のバージョンを `rounded_coords` という変数に保存し、この変数を返してください。

# --hints--

`create_trajectory` という名前のメソッドを定義してください。
...
`print(graph.create_coordinates_table())` を `print(graph.create_trajectory())` に置き換えてください。
...
`rounded_coords` 変数には丸められた座標が含まれている必要があります。
