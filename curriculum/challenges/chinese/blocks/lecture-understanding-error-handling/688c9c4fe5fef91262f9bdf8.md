...
这里关键字 `raise`（无参数）会重新引发当前正在处理的异常。
...
这里你可以看到 `raise ... from None` 会抑制原始异常的上下文：
...
而 `raise ... from e` 会将新异常与原始异常链起来，保留错误轨迹：
