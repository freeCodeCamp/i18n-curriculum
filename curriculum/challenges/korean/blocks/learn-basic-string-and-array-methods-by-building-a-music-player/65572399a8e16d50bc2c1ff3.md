...
일치하는 항목이 있으면 `userData.currentSong`를 `null`로, `userData.songCurrentTime`를 `0`로 설정하세요.

그다음, 재생을 멈추기 위해 `pauseSong()` 함수를 호출하고 플레이어 표시를 업데이트하기 위해 `setPlayerDisplay()` 함수를 호출하세요.
