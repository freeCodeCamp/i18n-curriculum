...
Sie müssen den `id` des aktuell gespielten Songs erhalten. Dafür können Sie `userData.currentSong?.id` verwenden.

Verwenden Sie `getElementById()`, um den aktuell gespielten Song auszuwählen: Übergeben Sie einen Template-Literal, um das `id` mit `song-` zu präfixen. Weisen Sie es einer Variablen namens `songToHighlight` zu.

# --hints--

Sie sollten die bestehende `highlightCurrentSong`-Funktion und deren Inhalt nicht verändern.
...
Sie sollten Ihre `getElementById()` einer `songToHighlight`-Variablen zuweisen.
