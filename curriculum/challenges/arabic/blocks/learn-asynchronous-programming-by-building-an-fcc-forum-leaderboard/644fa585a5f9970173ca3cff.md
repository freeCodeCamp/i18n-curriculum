...
أنشئ ثابتًا اسمه `linkClass` وقم بتعيين قالب نصي له. داخل هذا القالب النصي، أضف القيمة `category ${selectedCategory.className}`.

ستُستخدم أسماء الأصناف هذه لتطبيق الأنماط على عنصر وسم الرابط.

# --hints--

يجب أن يكون لديك ثابت باسم `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

يجب أن تعيّن قالبًا نصيًا لمتغير `linkClass` الخاص بك.

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

يجب أن تحتوي القالب النصي على القيمة `category ${selectedCategory.className}`.
