...
Agora você precisa de uma classe principal que ajudará a renderizar a interface dinamicamente no DOM.

Crie uma classe chamada `Game`. Dentro dela, adicione uma propriedade chamada `cards` do tipo `Card[]`, e por enquanto, inicialize-a com um array vazio.

# --hints--

Você deve ter uma classe chamada `Game`.
...
Você deve ter uma propriedade `cards` dentro da classe `Game`.
...
Sua propriedade `cards` deve ter o tipo `Card[]`.
...
Sua propriedade cards deve ser definida como um `array` vazio.
