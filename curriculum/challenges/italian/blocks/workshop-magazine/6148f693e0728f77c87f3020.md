...
Crea una terza query `@media` per `only screen` con un `max-width` di `550px`. All’interno, crea un selettore `.hero-title` con un insieme `font-size` impostato su `6rem`, un selettore `.hero-subtitle, .author, .quote, .list-title` con un insieme `font-size` impostato su `1.8rem`, un selettore `.social-icons` con un insieme `font-size` impostato su `2rem` e un selettore `.text` con un insieme `font-size` impostato su `1.6rem`.
...
Il tuo selettore `.hero-title` dovrebbe avere un insieme `font-size` impostato su `6rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[0]?.style?.fontSize, '6rem');
```

Il tuo selettore `.hero-subtitle, .author, .quote, .list-title` dovrebbe avere un insieme `font-size` impostato su `1.8rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[1]?.style?.fontSize, '1.8rem');
```

Il tuo selettore `.social-icons` dovrebbe avere un insieme `font-size` impostato su `2rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize, '2rem');
```

Il tuo selettore `.text` dovrebbe avere un insieme `font-size` impostato su `1.6rem`.
