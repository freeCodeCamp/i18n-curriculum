...
Your `ol` element should have an opening tag. Початкові теги мають такий синтаксис: `<elementName>`.

```js
assert.exists(document.querySelector('ol'));
```

Your `ol` element should have a closing tag. Кінцеві теги мають `/` одразу після символу `<`.
