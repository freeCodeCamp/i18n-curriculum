...
Ordena el `header`, usando _Flexbox_ para colocar espacio entre los hijos y centrarlos verticalmente.

Luego, fija el `header` en la parte superior de la ventana gráfica.

# --hints--

Debes dar al `header` un `justify-content` de `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

Debes dar al `header` un `align-items` de `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

Debes dar al `header` un `position` de `fixed`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.position, 'fixed');
```

Debes dar al `header` un `top` de `0`.
