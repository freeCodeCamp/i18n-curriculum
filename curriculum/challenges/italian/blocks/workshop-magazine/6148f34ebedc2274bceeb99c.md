...
Ora che il layout della rivista è finito, devi renderlo responsivo.

Inizia con una query `@media` per `only screen` con una `max-width` di `720px`. All’interno, crea un selettore `.image-wrapper` e imposta su di esso una proprietà `grid-template-columns` di `1fr`.
