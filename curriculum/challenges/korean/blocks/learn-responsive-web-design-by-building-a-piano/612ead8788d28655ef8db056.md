...
`.key.black--key::after`에 대해 `width`을 `32px`로, `height`을 `100px`로 설정하세요.

# --hints--

`.key.black--key::after`에 `width` 속성을 `32px`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.key.black--key::after')?.width === '32px');
```

`.key.black--key::after`에 `height` 속성을 `100px`로 설정해야 합니다.
