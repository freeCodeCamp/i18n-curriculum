...
Ora inizierai a fare riferimento agli elementi DOM necessari.

Prendi l'elemento paragrafo con l'id `bug-emoji` e salvalo in una costante chiamata `bugEmojiElement`. Usa l'operatore di asserzione non-null `!` per affermare che l'elemento non è `null`.

# --hints--

Dovresti avere una costante chiamata `bugEmojiElement`.
...
Il tuo `bugEmojiElement` dovrebbe interrogare l'elemento con l'id `bug-emoji` usando `document.querySelector` con un argomento di tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

Il tuo `bugEmojiElement` dovrebbe usare l'operatore di asserzione non-null (`!`).
