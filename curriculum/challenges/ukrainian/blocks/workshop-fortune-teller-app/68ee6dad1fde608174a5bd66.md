...
У методі `singleCardSelected` отримайте елемент `singleCard` та оновіть його `innerHTML`, викликавши функцію `renderCard`. Передайте `"Click the card and reveal the fortune"` як перший аргумент, а `isReversed` — як другий.

# --hints--

Ви повинні оновити `this.elements.singleCard.innerHTML`, викликавши функцію `renderCard`.
...
Ваш виклик `renderCard` має мати `"Click the card and reveal the fortune"` як перший аргумент.
...
Ви повинні передати `isReversed` як другий аргумент у `renderCard`.
