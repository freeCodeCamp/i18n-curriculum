...
Vipe vipengele vya `.formrow` ukingo wa juu, na pedi za kushoto na kulia. Thamani zingine za kuweka pedi zinapaswa kuwa `0`.

Kisha, ongeza ukubwa wa fonti kwa vipengele vyote vya `input`.

# --hints--

Unapaswa kutumia kichaguzi cha `.formrow` ili kulenga vipengele vya `.formrow`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.formrow'));
```

Unapaswa kuipa `.formrow` `margin-top` ya angalau `1px`.
...
Unapaswa kuipa `.formrow` `padding-top` ya `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingTop, '0px');
```

Unapaswa kuipa `.formrow` `padding-right` ya angalau `1px`.
...
Unapaswa kuipa `.formrow` `padding-bottom` ya `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingBottom, '0px');
```

Unapaswa kuipa `.formrow` `padding-left` ya angalau `1px`.
...
Unapaswa kutumia kichaguzi cha `input` ili kulenga vipengele vya `input`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input'));
```

Unapaswa kuipa `input` `font-size` zaidi ya `13px`.
