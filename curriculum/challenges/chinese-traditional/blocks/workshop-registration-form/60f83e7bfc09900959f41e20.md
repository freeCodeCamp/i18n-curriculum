...
某些 `type` 屬性值內建表單驗證。例如，`type="email"` 要求該值為有效的電子郵件位址。

為你的密碼 `input` 元素添加訂製驗證，方法是添加一個值為 `8` 的 `minlength` 屬性。這樣可以防止提交少於 8 個字元的輸入。

# --hints--

你應該為密碼 `input` 元素添加 `minlength` 屬性。

```js
assert.notEqual(document.querySelector('input[type="password"]')?.minLength, -1);
```

你應該為 `minlength` 屬性指定一個值為 `8`。
