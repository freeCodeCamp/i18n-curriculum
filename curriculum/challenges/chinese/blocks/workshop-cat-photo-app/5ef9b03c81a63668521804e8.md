...
在 `footer` 元素中嵌套一个 `p` 元素，其文本为 `No Copyright - freeCodeCamp.org`。
...
你的 `footer` 元素应该包含一个 `p` 元素。 确保为 `p` 元素添加一个开始标签和结束标签。
...
你的 `p` 元素的文本应该是 `No Copyright - freeCodeCamp.org`。 你要么遗漏了文本，要么拼写有误，要么没有把文本放在 `p` 元素的开始和结束标签之间。
