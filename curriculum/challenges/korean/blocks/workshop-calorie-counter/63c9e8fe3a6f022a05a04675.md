...
같은 보간 구문을 사용하여 두 번째 `p` 요소에 `consumedCalories Calories Consumed` 텍스트를, 세 번째 요소에는 `exerciseCalories Calories Burned` 텍스트를 추가하세요. `consumedCalories`와 `exerciseCalories` 변수를 적절한 값으로 바꾸는 것도 잊지 마세요.

# --hints--

템플릿 리터럴에 두 번째 `p` 요소를 추가하세요.
...
두 번째 `p` 요소에는 `${consumedCalories} Calories Consumed` 텍스트가 있어야 합니다.
...
세 번째 `p` 요소에는 `${exerciseCalories} Calories Burned` 텍스트가 있어야 합니다.
