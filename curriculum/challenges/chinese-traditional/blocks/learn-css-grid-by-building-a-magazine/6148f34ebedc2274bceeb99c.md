...
現在雜誌版面設計完成後，你需要讓它具備響應式設計。

從 `@media` 查詢開始，條件為 `only screen` 且 `max-width` 為 `720px`。在裡面，創建一個 `.image-wrapper` 選擇器並給它一個 `grid-template-columns` 屬性，值為 `1fr`。

這將在較小的螢幕上將三個影像摺疊成一個行。

# --hints--

你應該有一個新的 `@media` 規則，針對 `only screen and (max-width: 720px)`。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

你的新 `@media` 規則應該有一個 `.image-wrapper` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

你的新 `.image-wrapper` 選擇器應該有一個 `grid-template-columns` 屬性，值為 `1fr`。
