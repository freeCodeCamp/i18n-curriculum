...
Rudi kwenye kupamba ukurasa. Chagua vipengele vya orodha ndani ya bar ya urambazaji, na wape mitindo ifuatayo:
...
Unapaswa kutumia kichaguzi `nav li` au `nav > ul > li`.
...
Unapaswa kuwapa vipengele vya `li` `color` ya `#dfdfe2`.
...
Unapaswa kuwapa vipengele vya `li` `margin` ya `0 0.2rem`.
...
Unapaswa kuwapa vipengele vya `li` `padding` ya `0.2rem`.
...
Unapaswa kuwapa vipengele vya `li` `display` ya `block`.
