...
找出第一組連續四個整數，每個整數都有四個不同的質數因數。這些數字中第一個是什麼？

# --hints--

`distinctPrimeFactors(2, 2)` 應該傳回一個數字。

```js
assert(typeof distinctPrimeFactors(2, 2) === 'number');
```

`distinctPrimeFactors(2, 2)` 應該傳回 14。

```js
assert.strictEqual(distinctPrimeFactors(2, 2), 14);
```

`distinctPrimeFactors(3, 3)` 應該傳回 644。

```js
assert.strictEqual(distinctPrimeFactors(3, 3), 644);
```

`distinctPrimeFactors(4, 4)` 應該傳回 134043。
