...
次に、`if` 文を書いて、`prevToggleItem` が `toggleItem` と厳密に等しくないかどうかを確認してください。等しくない場合は、文字列 `New toggleItem function` をコンソールに出力します。その後、`prevToggleItem` に `toggleItem` を代入してください。

# --hints--

`if` が `prevToggleItem` と厳密に等しくないかどうかを確認する `toggleItem` 文を書く必要があります。

```js
assert.match(code, /if\s*\(\s*prevToggleItem\s*!==\s*toggleItem\s*\)/);
```

条件が満たされた場合、`New toggleItem function` 文の中で文字列 `if` をコンソールに出力してください。
...
条件が満たされた場合、`prevToggleItem` 文の中で `toggleItem` に `if` を代入してください。
