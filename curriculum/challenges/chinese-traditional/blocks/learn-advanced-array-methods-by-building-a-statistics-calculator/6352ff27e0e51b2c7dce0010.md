...
將你的新 `getVariance` 函式添加到 `calculate` 函式，並更新相應的 HTML 元素。

# --hints--

你的 `calculate` 函式（程式）應該有一個 `variance` 變數，其值設定為 `getVariance(numbers)`。

```js
assert.match(calculate.toString(), /variance\s*=\s*getVariance\(\s*numbers\s*\)/);
```

你應該將 `#variance` 元素的 `textContent` 設定為 `variance` 變數的值。
