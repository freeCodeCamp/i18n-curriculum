...
現在是時候測試 `playNextSong` 函數了。

向 `nextButton` 元素添加 `"click"` 事件監聽器，然後傳入 `playNextSong` 作爲事件監聽器的第二個參數。 這是事件監聽器將運行的函數。

通過首先單擊“播放”按鈕然後單擊“下一步”按鈕來測試你的應用。 你應該看到一切都按預期進行。

# --hints--

你應該在 `nextButton` 變量上調用 `addEventListener()` 方法。
...
你應該將 `playNextSong` 作爲 `addEventListener` 方法的第二個參數傳入。
