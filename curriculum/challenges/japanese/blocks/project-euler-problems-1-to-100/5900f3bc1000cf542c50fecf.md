...
自然数の平方根が整数でない場合、それは無理数であることはよく知られています。そのような平方根の小数展開は、繰り返しのパターンが全くなく無限に続きます。

2の平方根は `1.41421356237309504880...` であり、最初の100桁の10進数の桁の和は `475` です。

最初の `n` 個の自然数について、無理数の平方根すべての最初の100桁の10進数の桁の和の合計を求めてください。
