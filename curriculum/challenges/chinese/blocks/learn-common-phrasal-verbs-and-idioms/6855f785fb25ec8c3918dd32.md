...
`To prompt` 意味着促使或鼓励某人做某事或给出响应。例如：

`The app prompts users to update their settings.` – 这意味着应用提醒或要求用户采取行动。

某些 `unique` 是独一无二的，或与其他任何东西不同。例如：

`Each user gets a unique code for account activation.` – 这意味着每个人的代码都是不同的。
