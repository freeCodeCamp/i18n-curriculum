...
`.text` 선택자에 `font-size` 속성을 `1.8rem`로, `letter-spacing` 속성을 `0.6px`로 설정하세요.

# --hints--

`.text` 선택자에 `font-size` 속성을 `1.8rem`로 설정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize, '1.8rem');
```

`.text` 선택자에 `letter-spacing` 속성을 `0.6px`로 설정해야 합니다.
