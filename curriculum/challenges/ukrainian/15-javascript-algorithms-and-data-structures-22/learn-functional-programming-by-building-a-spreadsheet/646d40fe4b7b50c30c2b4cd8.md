...
Функція `infixEval` обчислює тільки першу операцію множення або ділення, останній `regex` не є глобальним. Це означає, що ви захочете використовувати рекурсивний підхід для оцінки всього рядка.

Якщо `infixEval` не знайде збігів, він поверне значення `str` як є. Використовуючи потрійний вираз, перевірте, чи `str2` дорівнює `str`. Якщо так, поверніть `str`, інакше поверніть результат виклику `highPrecedence()` на `str2`.

# --hints--

Функція `highPrecedence` має використовувати ключове слово `return`.
...
Використайте ключове слово `return` з умовою, щоб перевірити, чи `str` дорівнює `str2`.
...
Ви повинні використати тернарний синтаксис з інструкцією `return`.
...
Якщо умова потрійного вірна, поверніть `str`.
...
Якщо умова потрійного хибна, потрібно повернути результат виклику `highPrecedence()`.
...
Передайте `str2` до виклику `highPrecedence()`.
