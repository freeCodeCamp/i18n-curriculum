...
يجب أن يكون لديك منتقي `table`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table'));
```

يجب أن يكون لمنتقي `table` الخاص بك الخاصية `border-collapse` بقيمة `collapse`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('border-collapse') === 'collapse');
```

يجب يحتوي لمنتقي `table` الخاص بك على الخاصية `border` بقيمة `0`.
