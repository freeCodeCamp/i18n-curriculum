...
Ahora necesitas iterar a través de tu array `units`. Con cada valor en esa matriz, verifica si el valor actual **no** es igual a la unidad actual (encontrada en `unitToConvert.value`).

Si es así, necesitarás convertir la unidad existente a tu unidad actual y agregar un elemento de lista a `resultList` con el siguiente formato:

`ingrediente: cantidad unidad`.

Por ejemplo, una entrada de `harina`, `5` y `gramo` debería resultar en lo siguiente:
...
Tu función `updateResultsList` debería actualizar adecuadamente el DOM.
