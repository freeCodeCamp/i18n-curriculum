...
为 `onChange` 添加一个属性，属性值是一个箭头函数，该函数以 `e` 作为参数，并调用 `setPlayer`，传入 `player` 的展开和 `position` 设置为 `e.target.value`，并将其强制转换为 `Position`。

# --hints--

你的位置 `select` 应该有一个 `onChange` 处理程序，该处理程序调用 `setPlayer`，传入 `player` 的展开，并更新 `position`。
