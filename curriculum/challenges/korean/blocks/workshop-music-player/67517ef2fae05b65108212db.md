...
현재 재생 중인 노래의 `id`을 가져와야 합니다. 이를 위해 `userData.currentSong?.id`을 사용할 수 있습니다.

`getElementById()`를 사용해 현재 재생 중인 노래를 선택하세요: 템플릿 리터럴을 전달해 `id`에 `song-` 접두사를 붙이세요. 이를 `songToHighlight`라는 변수에 할당하세요.
...
`document.getElementById()`을 사용하고 `` `song-${userData.currentSong?.id}` ``를 전달해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

`getElementById()`을 `songToHighlight` 변수에 할당해야 합니다.
