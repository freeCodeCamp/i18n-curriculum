...
Chaves GPG, ou GNU Privacy Guard, são normalmente usadas para assinar arquivos ou commits. Alguém pode então usar sua chave pública GPG para verificar que a assinatura do arquivo é da sua chave e que o conteúdo do arquivo não foi modificado ou adulterado. Chaves SSH, ou Secure SHell, são geralmente usadas para autenticar uma conexão remota a um servidor — via o utilitário `ssh`. No entanto, como você vai aprender nesta lição, também pode usar uma chave SSH para assinar commits.
