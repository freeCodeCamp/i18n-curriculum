...
`div` 要素に `card-wrapper` クラスを付けると、カードの外側のボーダーとクリップされた形状が作られます。

次に、`div` の直下に `card-wrapper` を追加し、`className` の `card` を設定してください。これでカードに暗い背景が付きます。

# --hints--

`div` 要素の中に、`className` の `card` を持つ `.card-wrapper` を追加してください。
