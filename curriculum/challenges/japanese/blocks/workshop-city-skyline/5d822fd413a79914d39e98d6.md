...
これで建物のようなものができました。最初の変数を作成する準備ができています。前のレッスンで、変数の宣言は2つのダッシュ（`-`）で始まり、名前と値が次のように与えられることを学びました：`--variable-name: value;`。`bb1` クラスのルール内で、`--building-color1` という名前の変数を作成し、`#999` の値を与えてください。
