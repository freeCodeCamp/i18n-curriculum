...
`Flickers` describes an effect where the image or light on the screen changes quickly and repeatedly, often in a way that can be distracting. Por ejemplo:

`The light flickers when the power is unstable.` – This means the light goes on and off quickly.

`Switch between tabs` means to change from one tab to another within an application or webpage. Por ejemplo:
