...
Додайте атрибут `value` зі значенням `player.dri` до вхідних даних DRI. Потім додайте атрибут `onChange`, значенням якого буде стрілкова функція, що приймає `e` як параметр і викликає `setPlayer` з розгортанням `player` та `dri`, встановленим у `Number(e.target.value)`.

# --hints--

Ваш `input` DRI повинен мати `value`, рівний значенню статистики `dri` гравця.
...
Ваш `input` DRI повинен мати обробник `onChange`, який викликає `setPlayer` з розгортанням `player` та `dri`, встановленим у `Number(e.target.value)`.
