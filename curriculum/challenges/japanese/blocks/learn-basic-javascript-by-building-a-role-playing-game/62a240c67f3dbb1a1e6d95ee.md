...
`#game` に最大幅 `500px`、最大高さ `400px` を設定してください。`background-color` を `#ffffff` に、`color` を `#ffffff` に設定します。

マージンを使って中央に配置するために、上のマージンを `30px`、下のマージンを `0px`、左右のマージンを `auto` に設定してください。

最後に、4辺すべてに `10px` のパディングを与えます。
...
`#game` セレクターには `max-width` を `500px` に設定してください。
...
`#game` セレクターには `max-height` を `400px` に設定してください。
...
`#game` セレクターには `background-color` を `#ffffff` に設定してください。
...
`#game` セレクターには `color` を `#ffffff` に設定してください。
...
`#game` セレクターには `margin-top` を `30px` に設定してください。
...
`#game` セレクターには `margin-left` を `auto` に設定してください。
...
`#game` セレクターには `margin-right` を `auto` に設定してください。
...
`#game` セレクターには `margin-bottom` を `0px` に設定してください。
...
`#game` セレクターには 4辺すべてにパディング `10px` を設定してください。
