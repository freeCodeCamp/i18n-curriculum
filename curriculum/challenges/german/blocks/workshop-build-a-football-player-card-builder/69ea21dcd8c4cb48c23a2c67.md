...
Fügen Sie ein `onChange`-Attribut hinzu, das auf eine Pfeilfunktion gesetzt ist, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `position` auf `e.target.value` gesetzt und als `Position` gecastet wird.

# --hints--

Ihre Position `select` sollte einen `onChange`-Handler haben, der `setPlayer` mit einer Ausbreitung von `player` aufruft und `position` aktualisiert.
