...
CSS 中的 `opacity` 屬性用於調整項目的不透明度，或相反地，透明度。
...
所給的值將套用到整個元素，不論該元素是具有透明度的影像，或是文字區塊的前景色與背景色。

# --instructions--

使用 `links` 類別選取錨點標記，並將其 `opacity` 設為 0.7。

# --hints--

你的程式碼應該透過選取 `links` 的類別，將錨點標記的 `opacity` 屬性設為 0.7。
