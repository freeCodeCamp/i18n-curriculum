...
最後，允許你的 regex 匹配整個字（就像你對 `freeRegex` 所做的那樣）。

# --hints--

你的 `stockRegex` 應該使用非捕獲群組。

```js
assert.match(stockRegex.source, /\(\?:/);
```

你的 `stockRegex` 應該使用非捕獲群組來匹配 `\s` 或 `^`。

```js
assert.match(stockRegex.source, /\(\?:(\^\|\\s|\\s\|\^)\)/);
```

你的 `stockRegex` 應該使用第二個非捕獲群組。
...
你的 `stockRegex` 應該符合 `it's stock alert time`。

```js
assert.match("it's stock alert time", stockRegex);
```

你的 `stockRegex` 應該符合 `stock alert time`。

```js
assert.match('stock alert time', stockRegex);
```

你的 `stockRegex` 應該符合 `it's stock alert`。

```js
assert.match("it's stock alert", stockRegex);
```

你的 `stockRegex` 應該符合 `stock alert`。

```js
assert.match('stock alert', stockRegex);
```

你的 `stockRegex` 不應該匹配 `hands-stock alert time`。

```js
assert.notMatch('hands-stock alert', stockRegex);
```

你的 `stockRegex` 不應該匹配 `stock alert-management`。
