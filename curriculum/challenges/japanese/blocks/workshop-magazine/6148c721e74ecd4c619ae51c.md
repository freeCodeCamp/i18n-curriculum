...
あなたの`.text`要素はCSSグリッドではありませんが、`column-width`プロパティを使えばグリッドを使わずに要素内にカラムを作成できます。

`.text`セレクターに`column-width`プロパティを`25rem`に設定してください。

# --hints--

`.text`セレクターには`column-width`プロパティを`25rem`に設定してください。
