...
أنشئ محدِّدًا لعناصر الـ `section`، وأعطها:

- خاصية `margin` بقيمة `20px auto`،
- خاصية `max-width` بقيمة `600px`.
- خاصية `background-color` بقيمة `whitesmoke`
- خاصية `padding` بقيمة `20px`
- خاصية `border-radius` بقيمة `10px`

# --hints--

يجب أن يكون لديك محدِّد `section`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle("section"));
```

يجب أن يحتوي محدِّد `section` على خاصية `margin` بقيمة `20px auto`.
...
يجب أن يحتوي محدِّد `section` على خاصية `max-width` بقيمة `600px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("max-width"), "600px");
```

يجب أن يحتوي محدِّد `section` على خاصية `background-color` بقيمة `whitesmoke`.
...
يجب أن يحتوي محدِّد `section` على خاصية `padding` بقيمة `20px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("padding"), "20px");
```

يجب أن يحتوي محدِّد `section` على خاصية `border-radius` بقيمة `10px`.
