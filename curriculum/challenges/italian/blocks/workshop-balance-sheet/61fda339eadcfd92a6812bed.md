...
The `span[class~="sr-only"]` selector will select any `span` element whose `class` _includes_ `sr-only`. Crea questo selettore e dagli una proprietà `border` con il valore `0`.
