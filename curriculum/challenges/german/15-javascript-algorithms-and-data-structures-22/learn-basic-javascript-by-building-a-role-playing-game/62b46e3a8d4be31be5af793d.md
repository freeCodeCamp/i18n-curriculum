---
id: 62b46e3a8d4be31be5af793d
title: Schritt 16
challengeType: 0
dashedName: step-16
---

# --description--

Wir sind auf ein kleines Problem gestoßen. Du versuchst, deine Seite nach einem Button-Element zu durchsuchen, aber dein `script`-Tag befindet sich im `head` deines HTMLs. Dies bedeutet, dass dein Code ausgeführt wird, bevor der Browser das HTML zu Ende gelesen hat, und dass dein `document.querySelector()` die Schaltfläche nicht sehen wird - da der Browser sie noch nicht verarbeitet hat.

Um dies zu beheben, verschiebe dein `script` -Element aus dem `head`-Element heraus und platziere es am Ende deines `body`-Elements (kurz vor dem abschließenden `</body>`-Tag).

# --hints--

Dein `script`-Element sollte sich nicht in deinem `head`-Element befinden.

```js
const script = document.querySelector('script[data-src$="script.js"]');
assert.notEqual(script.parentElement.tagName, "HEAD");
```

Dein `script`-Element sollte am Ende deines `body`-Elements stehen.

```js
const script = document.querySelector('script[data-src$="script.js"]');
assert.equal(script.previousElementSibling.tagName, "DIV");
// When building the test frame, the runner script is always inserted after user
// code. This means the learner's script should be the penultimate element in 
// the body.
assert.equal(script.nextElementSibling.id, "fcc-test-runner");
assert.equal(script.parentElement.tagName, "BODY");
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
--fcc-editable-region--
  <head>
    <meta charset="utf-8">
    <link rel="stylesheet" href="./styles.css">
    <title>RPG - Dragon Repeller</title>
    <script src="./script.js"></script>
  </head>
  <body>
    <div id="game">
      <div id="stats">
        <span class="stat">XP: <strong><span id="xpText">0</span></strong></span>
        <span class="stat">Health: <strong><span id="healthText">100</span></strong></span>
        <span class="stat">Gold: <strong><span id="goldText">50</span></strong></span>
      </div>
      <div id="controls">
        <button id="button1">Go to store</button>
        <button id="button2">Go to cave</button>
        <button id="button3">Fight dragon</button>
      </div>
      <div id="monsterStats"></div>
      <div id="text"></div>
    </div>

  </body>
--fcc-editable-region--
</html>
```

```js
let xp = 0;
let health = 100;
let gold = 50;
let currentWeaponIndex = 0;
let fighting;
let monsterHealth;
let inventory = ["stick"];

let button1 = document.querySelector("#button1");
```
