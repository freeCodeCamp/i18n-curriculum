...
Als Nächstes sagen Sie dem Formular, wie es abgesendet werden soll. Fügen Sie dem Formularelement ein `method`-Attribut mit dem Wert `post` und ein `action`-Attribut mit dem Wert `https://superhero-application-form.freecodecamp.org` hinzu.

Jetzt wird das Absenden des Formulars es an diese URL senden. Das machen Sie im nächsten Schritt.
...
Ihr `form`-Element sollte ein `action`-Attribut mit dem Wert `https://superhero-application-form.freecodecamp.org` haben.
