...
Otro ejemplo común de cuándo usar el método `e.preventDefault()` tiene que ver con el envío de formularios. Por defecto, enviar un formulario envía datos al servidor y recarga la página. Usar `e.preventDefault()` evita que esto suceda.
...
Prevenir el comportamiento predeterminado es excelente cuando necesitas más control sobre cómo un usuario interactúa con la página, pero es importante tener en cuenta aspectos como la accesibilidad: tu comportamiento personalizado debe ofrecer las mismas funcionalidades que el predeterminado.
