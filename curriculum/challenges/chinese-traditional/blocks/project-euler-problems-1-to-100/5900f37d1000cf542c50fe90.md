...
如果數字 1 到 5 用字、單字寫出來：one、two、three、four、five，則總共使用了 3 + 3 + 5 + 4 + 4 = 19 個字母。

如果將從 1 到指定的 `limit`（包含）所有數字寫成字，將會使用多少字？

**注意：** 不要計算空格或連字號。 例如，342（三百四十二）包含 23 個字母，115（一百一十五）包含 20 個字母。書寫數字時使用 "and" 是符合英式用法。

# --hints--

`numberLetterCounts(5)` 應該傳回一個數字。

```js
assert.isNumber(numberLetterCounts(5));
```

`numberLetterCounts(5)` 應該傳回 19。

```js
assert.strictEqual(numberLetterCounts(5), 19);
```

`numberLetterCounts(150)` 應該傳回 1903。

```js
assert.strictEqual(numberLetterCounts(150), 1903);
```

`numberLetterCounts(1000)` 應該傳回 21124。
