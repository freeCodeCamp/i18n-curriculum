...
عيّن عرض عناصر الـ `section` إلى `80%` من الحاوية الأصلية لها. ثم استخدم الهوامش لمركزة عناصر الـ `section`، مع إضافة `10px` إلى الهامش السفلي.

تأكد أيضًا من أن عناصر الـ `section` لا يمكن أن تكون أوسع من `600px`.

# --hints--

يجب أن تستخدم محدِّد `section`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('section'));
```

يجب أن تعطي الـ `section` عرضًا (`width`) بقيمة `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.width, '80%');
```

يجب أن تعطي الـ `section` هامشًا علويًا (`margin-top`) بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginTop, '0px');
```

يجب أن تعطي الـ `section` هامشًا أيمنًا (`margin-right`) بقيمة `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginRight, 'auto');
```

يجب أن تعطي الـ `section` هامشًا سفليًا (`margin-bottom`) بقيمة `10px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginBottom, '10px');
```

يجب أن تعطي الـ `section` هامشًا أيسرًا (`margin-left`) بقيمة `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginLeft, 'auto');
```

يجب أن تعطي الـ `section` أقصى عرض (`max-width`) بقيمة `600px`.
