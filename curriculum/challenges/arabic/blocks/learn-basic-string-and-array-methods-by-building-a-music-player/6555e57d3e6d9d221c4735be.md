...
عند الضغط على زر الخلط، يجب أن تجعل `currentSong` لا شيء و`songCurrentTime` تساوي `0`.

اجعل `userData.currentSong` تساوي `null` و`userData.songCurrentTime` تساوي `0`.

*ملاحظة*: لا تستخدم التسلسل الاختياري في هذه الخطوة لأنك تحدد صراحة خاصيتي `currentSong` و`songCurrentTime` لتكونا `null` و`0` على التوالي.

# --hints--

يجب أن تجعل `userData.currentSong` تساوي `null`.

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

يجب أن تجعل `userData.songCurrentTime` تساوي `0`.
