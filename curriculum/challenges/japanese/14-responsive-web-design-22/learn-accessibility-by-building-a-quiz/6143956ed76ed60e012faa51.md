...
1 つ目の `span` 要素内に、`Question` というテキストを追加してください。

2 つ目の `span` 要素内にも同様に、`Question` というテキストを追加してください。

# --hints--

1 つ目の `span` 要素のテキストを `Question` にしてください。

```js
assert.equal(document.querySelector('h3 > span')?.textContent, 'Question');
```

2 つ目の `span` 要素のテキストを `Question` にしてください。
