...
استبدل الهامش العلوي لعناصر `h2` بـ `60px` من الحشو العلوي.

# --hints--

يجب أن تعطي `h2` هامشًا علويًا (`margin-top`) بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h2')?.marginTop, '0px');
```

يجب أن تعطي `h2` حشوًا علويًا (`padding-top`) بقيمة `60px`.
