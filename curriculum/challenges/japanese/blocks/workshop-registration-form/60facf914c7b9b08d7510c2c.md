...
`textarea` が小さすぎます。初期サイズを設定するには、`rows` と `cols` の属性を追加できます。

初期サイズとして `3` 行と `30` 列を追加してください。

# --hints--

`textarea` に値が `rows` の `3` 属性を付ける必要があります。

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

`textarea` に値が `cols` の `30` 属性を付ける必要があります。
