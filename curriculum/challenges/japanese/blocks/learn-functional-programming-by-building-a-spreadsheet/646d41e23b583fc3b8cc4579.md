...
これで、文字列に対して関数の解析ロジックを適用し始めることができます。`applyFunction` という名前の関数を宣言してください。この関数は `str` というパラメータを受け取ります。

# --hints--

`applyFunction` 変数を宣言してください。

```js
assert.match(code, /(?:var|let|const)\s+applyFunction\s*=/);
```

`const` を使って `applyFunction` 変数を宣言してください。

```js
assert.match(code, /const\s+applyFunction\s*=/);
```

`applyFunction` 変数は関数である必要があります。

```js
assert.isFunction(applyFunction);
```

`applyFunction` 関数はアロー構文を使ってください。

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`applyFunction` 関数は `str` パラメータを持つべきです。

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>/);
```

`applyFunction` は空であるべきです。
