...
Baada ya kijenzi, tengeneza njia ya `render` isiyo na utekelezaji ambayo ina aina ya kurudisha `void`. Utaitumia hii katika madarasa madogo kusasisha emoji inayoonyeshwa.

# --hints--

Darasa lako la `Bug` linapaswa kuwa na njia ya `render` isiyo na utekelezaji.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

Njia yako ya `render` inapaswa kuwa na aina ya kurudisha `void`.
