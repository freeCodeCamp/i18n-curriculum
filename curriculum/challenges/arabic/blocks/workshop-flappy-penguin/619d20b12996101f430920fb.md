...
منقار وطرفا قدم البطريق يشتركان في نفس الـ`color`.

أنشئ متغير CSS مخصص جديد باسم `--penguin-picorna`، واستبدل كل قيم الخاصية ذات الصلة به.

# --hints--

يجب أن تعطي `:root` خاصية `--penguin-picorna`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-picorna'));
```

يجب أن تعطي `--penguin-picorna` قيمة `orange`، لكن وُجد `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-picorna', true), 'orange');
```

يجب أن تعطي `.beak` خاصية `background-color` بقيمة `var(--penguin-picorna)`، لكن وُجد `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.getPropVal('background-color', true), 'var(--penguin-picorna)');
```

يجب أن تعطي `.foot` خاصية `background-color` بقيمة `var(--penguin-picorna)`، لكن وُجد `--fcc-actual--`.
