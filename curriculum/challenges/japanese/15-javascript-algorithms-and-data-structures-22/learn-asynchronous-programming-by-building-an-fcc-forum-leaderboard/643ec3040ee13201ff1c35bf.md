...
2 番目の `td` 要素には後ほどコンテンツを追加するため、今は空のままにしておきます。

3 番目の `td` 要素内に、埋め込み式 `${posts_count - 1}` を追加してください。

これにより、トピックへの返信の件数が表示されます。

# --hints--

You should have `${posts_count - 1}` inside the third `td` element.
