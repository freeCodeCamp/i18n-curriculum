...
La elección entre estos depende del número y género de la persona sobre la que preguntas. Usa `does` con pronombres singulares (como `he` o `she`) cuando te refieras a una persona y `do` con `they` cuando te refieras a alguien cuyo género podría no ser conocido o si te refieres a personas en un sentido no binario o plural.
