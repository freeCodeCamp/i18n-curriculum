...
関数の仮引数 (parameter) は、デフォルト値で初期化することができます。 関数が実引数 (argument) なしで呼び出された場合、デフォルト値が使われます:
...
`setPlayerCards` 関数に `arr` という仮引数を追加して、デフォルト値として `players` を割り当ててください。 `175` 行目で、`myFavoriteFootballTeam` オブジェクトから `players` 変数を分割代入で取り出したことを思い出してください。
