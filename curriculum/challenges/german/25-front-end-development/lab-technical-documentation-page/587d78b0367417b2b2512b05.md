...
1. Innerhalb des `#main-doc`-Elements kannst du mehrere `section`-Elemente sehen, jedes mit einer `main-section`-Klasse. There should be a minimum of five.
...
1. Jedes `section`-Element der Klasse `main-section` sollte auch eine `id` haben, die mit dem Text von jedem `header` übereinstimmt. Any spaces should be replaced with underscores (e.g. The section that contains the header "JavaScript and Java" should have a corresponding `id="JavaScript_and_Java"`).
...
1. Außerdem sollte die Navigationsleiste auch (`a`) -Link-Elemente der Klasse `nav-link` enthalten. There should be one for every element with the class `main-section`.
