...
Тепер ваш `applyFunction` має повертати результат. Поверніть результат виклику методу `.replace()` на `str2`. Передайте свій регулярний вираз `functionCall` та порожню функцію зворотного виклику.

# --hints--

Ваша функція `applyFunction` має повертати результат виклику методу `.replace()` на `str2`.
...
Ви маєте передати `functionCall` як перший аргумент у виклик `.replace()`.
...
Ви маєте передати порожню стрілкову функцію як другий аргумент у виклик `.replace()`.
