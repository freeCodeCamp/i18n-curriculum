...
بعد ذلك، تحتاج إلى عرض عنوان الأغنية الحالية والفنان في شاشة المشغل.

كخطوة أولى، احصل على عناصر `#player-song-title` و `#player-song-artist`، المسؤولة عن عرض عنوان الأغنية والفنان، وخزنها في المتغيرين `playingSong` و `songArtist` على التوالي.

# --hints--

يجب أن يكون لديك متغير باسم `playingSong`.

```js
assert.isDefined(playingSong);
```

يجب أن يحتوي متغير `playingSong` على قيمة عنصر `#player-song-title`.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

يجب أن يكون لديك متغير باسم `songArtist`.
...
يجب أن يحتوي متغير `songArtist` على قيمة عنصر `#player-song-artist`.
