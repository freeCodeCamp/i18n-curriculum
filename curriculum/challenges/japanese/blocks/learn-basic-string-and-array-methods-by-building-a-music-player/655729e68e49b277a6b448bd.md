...
`resetButton` を作成したので、`id` と `aria-label` の属性を代入する必要があります。JavaScript にはこれに使う `id` と `ariaLabel` のプロパティがあります。

たとえば、`element.id` は `id` 属性を設定し、`element.ariaLabel` は `aria-label` 属性を設定します。どちらも値は文字列として受け取ります。

`id` 要素の `resetButton` 属性を `"reset"` に、`aria-label` 属性を `"Reset playlist"` に設定してください。

# --hints--

既存の `if` 文とその内容は変更しないでください。
...
`resetButton.id` を使って `id` に `"reset"` という名前の `resetButton` 属性を作成してください。

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

`resetButton.ariaLabel` を使って `"aria-label"` に `"Reset playlist"` という名前の `resetButton` 属性を作成してください。
