...
Agora você pode estilizar o conteúdo do elemento `.heading` com CCS Grid.

A função CSS `repeat()` é usada para repetir um valor, em vez de escrevê-la manualmente, e é útil para layouts de grade. Por exemplo, definindo a propriedade `grid-template-columns` para `repeat(20, 200px)` criaria 20 colunas a cada `200px` de largura.
...
O seletor `.heading` deve ter a propriedade `grid-template-columns` definida como `repeat(2, 1fr)`.
