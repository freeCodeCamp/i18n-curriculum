...
첫 번째 `td` 요소 안에 임베드된 표현식 `${}`을 추가하세요. 그 표현식 안에서 `forumCategory` 인수를 사용해 `category_id` 함수를 호출하세요.

이제 각 게시물 주제 아래에 카테고리가 표시되는 것을 볼 수 있을 것입니다.

# --hints--

`forumCategory` 인수를 사용해 `category_id` 함수를 호출해야 합니다.
