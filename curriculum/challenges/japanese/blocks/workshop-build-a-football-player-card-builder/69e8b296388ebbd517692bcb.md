...
`FootballPlayerCard` は現在、カード本体のみをレンダリングしています。完全なページを作成するには、`div` の `className` を持つ `page` 要素を追加してください。

その `div` の中に、`header` の `className` を持つ `header` 要素を追加します。`header` の中には、`div` の `className` を持つ `header-inner` を追加してください。`header-inner` 要素の中には、それぞれ `Football Card Builder` と `Customize your player card` のテキストを持つ2つの段落を追加します。段落にはそれぞれ `className` と `header-title` の `header-subtitle` も付けてください。

閉じる `header` 要素の後、しかし `div` の `className` を持つ `page` の中に、メインレイアウトが追加されるまでの一時的なプレースホルダーとして、プロップ付きの `PlayerCard` コンポーネントを移動させてください。

# --hints--

あなたの `FootballPlayerCard` は、`div` の `className` を持つ `page` をレンダリングする必要があります。
...
あなたの `div` は、`className` の `page` を持ち、`header` の `className` を持つ `header` 要素を含む必要があります。
...
あなたの `header` は、`div` の `className` を持つ `header-inner` を含む必要があります。
...
あなたの `header-inner` 要素は、`p` の `className` を持ち、テキスト `header-title` を含む `Football Card Builder` を含む必要があります。
...
あなたの `header-inner` 要素は、`p` の `className` を持ち、テキスト `header-subtitle` を含む `Customize your player card` を含む必要があります。
...
あなたの `PlayerCard` コンポーネントは、`div.page` の中にレンダリングされる必要があります。
