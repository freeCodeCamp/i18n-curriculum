...
Вкладіть два нові елементи `div` всередину `.bb2`, надайте їм класи `bb2a` та `bb2b` відповідно. Це будуть два розділи для цієї будівлі.
...
Першому `div` слід додати клас `bb2a`.

```js
assert.exists(document.querySelector('div.bb2a'));
```

Другому `div` слід додати клас `bb2b`.
