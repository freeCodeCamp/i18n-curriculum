...
Crea otro elemento `p` debajo de tu elemento `.first-paragraph`, y asígalo con el siguiente texto:
...
Debe crear un segundo elemento `p` dentro de su elemento `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 2);
```

Su segundo elemento `p` debe tener el texto proporcionado.
