...
Para comenzar a trabajar con los datos, necesitarás usar otro método `.then()`.

Encadena otro `.then()` con el método `.then()` existente. Esta vez, pasa `data` como el parámetro para la función de devolución de llamada. Para la devolución de llamada, usa llaves porque tendrás más de una expresión. Dentro de tu función de devolución de llamada, registra `data` en la consola para ver cómo se ve.
...
Tu primer `.then()` no debe tener un punto y coma.

```js
assert.notMatch(code, /\.then\(\s*(\(\s*res\s*\)|res)\s*=>\s*res\.json\(\s*\)\s*\)\s*;/)
```

Deberías encadenar otro `.then()` al `.then()` existente.
...
Tu segundo método `.then()` debería usar la sintaxis de función de flecha.
...
Tu segundo `.then()` no debe tener punto y coma después de él.
