...
Aggiorna la funzione callback per accettare `match`, `fn` e `args` come parametri. Dovrebbe verificare se `spreadsheetFunctions` ha una proprietà `fn` e restituire implicitamente il risultato del controllo.

Ricordati di rendere `fn` in minuscolo.
