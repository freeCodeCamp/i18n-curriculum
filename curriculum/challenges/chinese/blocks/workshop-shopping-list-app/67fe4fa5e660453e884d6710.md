...
创建一个名为 `toggleItem` 的函数，该函数以 `item` 作为参数。

在函数内部，目前只需调用 `setSelectedItems()` 函数。 我们稍后会回来实现切换选定项目的逻辑。

# --hints--

你应该有一个名为 `toggleItem` 的函数，该函数以 `item` 作为参数。
...
你应该在 `toggleItem()` 函数内调用 `setSelectedItems()` 函数。
