...
Una de las herramientas más poderosas es la consola de desarrollador. Dependiendo de tu navegador, esta puede abrirse pulsando `F12` o `Ctrl+Shift+I`. En Mac, puedes presionar `Option + ⌘ + C` y seleccionar "Console". También puede hacer clic en el botón "Consola" sobre la ventana de vista previa para ver nuestra consola incorporada.
...
La línea `console.log("Hello World");` debe estar entre las etiquetas `script`.
