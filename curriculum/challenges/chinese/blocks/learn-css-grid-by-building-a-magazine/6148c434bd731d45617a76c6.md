...
如果你想添加更多社媒图标，但将它们保持在同一行，则需要更新 `grid-template-columns` 以创建其他列。 作为替代方案，你可以使用 `grid-auto-flow` 属性。

This property takes either `row` or `column` as the first value, with an optional second value of `dense`. `grid-auto-flow` uses an auto-placement algorithm to adjust the grid layout. 将其设置为 `column`，将告诉算法根据需要为内容创建新列。 `dense` 值允许算法回溯并用较小的项目填充网格中的空白，这可能导致项目出现乱序。

对于你的 `.social-icons` 选择器，将 `grid-auto-flow` 属性设置为 `column`。

# --hints--

你的 `.social-icons` 选择器应该将 `grid-auto-flow` 属性设置为 `column`。
