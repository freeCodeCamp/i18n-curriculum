...
عدّل تعبير الفهم الخاص بـ `row_str` ليُعطي سلسلة نصية فقط عندما لا تكون القيمة صفرًا، ويُعطي رمز النجمة (*) خلاف ذلك.

# --hints--

يجب أن تستدعي قائمة الفهم المُعيَّنة للمتغير `row_str` الدالة `str()` على كل عنصر `i` في `row` إذا كان `i` صحيحًا، وأن تُقيَّم إلى `'*'` خلاف ذلك.
