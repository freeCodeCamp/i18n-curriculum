...
Consulta gli elementi `#next`, `#previous` e `#shuffle` dal file HTML.

Assegna ciascuno a variabili chiamate rispettivamente `nextButton`, `previousButton` e `shuffleButton`.

# --hints--

Dovresti avere una variabile chiamata `nextButton`.

```js
assert.isDefined(nextButton);
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#next`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

Dovresti assegnare l'elemento `#next` alla variabile `nextButton`. 

```js
assert.equal(nextButton, document.getElementById('next'));
```

Dovresti avere una variabile chiamata `previousButton`.

```js
assert.isDefined(previousButton);
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#previous`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

Dovresti assegnare l'elemento `#previous` alla variabile `previousButton`. 

```js
assert.equal(previousButton, document.getElementById('previous'));
```

Dovresti avere una variabile chiamata `shuffleButton`.

```js
assert.isDefined(shuffleButton);
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#shuffle`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

Dovresti assegnare l'elemento `#shuffle` alla variabile `shuffleButton`. 
