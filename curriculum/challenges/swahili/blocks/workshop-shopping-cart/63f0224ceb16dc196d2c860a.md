...
Unahitaji kupata vifungo vyote vya `Add to cart` ulivyoviweka kwenye DOM awali. Tangaza kigezo cha `addToCartBtns`, na weka thamani yake kwa wito wa njia ya `getElementsByClassName()` kwenye kitu cha `document`, ukipitisha mfuatano wa herufi `"add-to-cart-btn"`.

# --hints--

Unapaswa kutumia `const` kutangaza kigezo chako cha `addToCartBtns`.

```js
assert.match(code, /const\s+addToCartBtns\s*=/);
```

Unapaswa kuita njia ya `getElementsByClassName()` kwenye kitu cha `document`.

```js
assert.match(code, /document\s*\.\s*getElementsByClassName\s*\(/);
```

Unapaswa kupitisha mfuatano wa herufi `"add-to-cart-btn"` kwa njia ya `getElementsByClassName()`.

```js
assert.match(code, /document\s*\.\s*getElementsByClassName\s*\(\s*('|"|`)add-to-cart-btn\1\s*\)/);
```

Unapaswa kuweka thamani inayorejeshwa na njia ya `getElementsByClassName()` kwenye kigezo cha `addToCartBtns`.
