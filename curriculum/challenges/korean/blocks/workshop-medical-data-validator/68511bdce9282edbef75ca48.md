...
이전 수업에서 배운 것처럼, 리스트 내포를 사용해 기존 반복 가능한 객체에서 리스트를 만들 수 있습니다:
...
`medications` 리스트의 각 항목은 문자열이어야 합니다. 이 단계와 다음 단계에서 이를 확인하는 표현식을 작성할 것입니다. `and` 연산자를 사용해 `medications` 키의 값에 또 다른 표현식을 추가하세요.

`and` 연산자의 오른쪽에, 리스트 내포 구문을 사용해 `isinstance(i, str)`의 각 `i`에 대해 `medications`를 평가하여 만든 리스트를 만드세요.

# --hints--

`medications` 딕셔너리의 `constraints` 키는 `isinstance(medications, list) and [isinstance(i, str) for i in medications]` 값을 가져야 합니다.
