...
Передайте <dfn>посилання</dfn> на функцію як параметр зворотного виклику. Посилання на функцію — це назва функції без дужок. Наприклад:
...
Метод `.map()` тут викличе функцію `myFunc`, передавши ті самі аргументи, яки приймає зворотній виклик `.map()`. Перший аргумент — це значення масиву на поточній ітерації, тому `newArray` буде `[value: 1, value: 2, value: 3]`.

Передайте посилання на функцію `elemValue` як зворотний виклик до методу `.map()`.

# --hints--

Не викликайте функцію `elemValue`.
...
Передайте посилання на `elemValue` як зворотний виклик до методу `.map()`.
