...
當你的 `if` 條件為真時，你需要交換這兩個元素，將較大的元素「冒泡」到陣列的末端。

為此，宣告一個 `temp` 變數並將 `array[j]` 的值指定給它。然後將 `array[j]` 的值指定為 `array[j + 1]`。最後，將 `temp` 的值指定給 `array[j + 1]`。

# --hints--

在你的 `if` 述語中，你應該宣告一個 `temp` 變數。
