...
`fightSlime` と `fightBeast` という名前の空の関数をさらに2つ作成してください。これらの関数は、今後の `cave` オブジェクトで使用されます。

# --hints--

`fightSlime` は関数である必要があります。

```js
assert.isFunction(fightSlime);
```

`fightBeast` は関数である必要があります。

```js
assert.isFunction(fightBeast);
```

`fightSlime` は空である必要があります。

```js
assert.match(fightSlime.toString(), /function fightSlime\(\s*\) \{\}/);
```

`fightBeast` は空である必要があります。
