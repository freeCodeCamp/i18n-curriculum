...
建物が現在、重なって積み上げられています。`.background-buildings` 要素をフレックスボックスの親にして建物を整列させてください。`align-items` と `justify-content` のプロパティを使って、要素の下部に建物を均等に配置してください。

# --hints--

`display` クラスに `flex` の `background-buildings` を追加してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.background-buildings')?.display, 'flex');
```

`align-items` クラスに `flex-end` の `background-buildings` を追加してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.background-buildings')?.alignItems, 'flex-end');
```

`justify-content` クラスに `space-evenly` の `background-buildings` を追加してください。
