...
Sasa pitia safu ya data ya `POSITIONS` ukitumia kitendakazi cha mshale kinachopokea kila mfuatano wa herufi wa nafasi kama `pos` na kurudisha kipengele cha `option`. Chaguo linapaswa kuwa na `key` ya `pos` na thamani pia ya `pos`. Pia, onyesha `pos` kama maandishi ya kila `option` iliyotengenezwa.

Tafadhali tumia chochote kama parameta wa map, haipaswi kuwa `pos`.

# --hints--

`select` yako ya nafasi inapaswa kuwa na vipengele vya `option` vilivyopitiwa kutoka kwa safu ya data ya `POSITIONS`.
...
`value` ya kila chaguo inapaswa kuendana na maandishi yanayoonyeshwa na iwe nafasi halali.
