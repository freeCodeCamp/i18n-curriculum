...
تُغيّر خصائص الرابط عندما يزوره المستخدم باستخدام <dfn>محدد زائف</dfn> يشبه `a:visited { propertyName: propertyValue; }`.

غيّر لون رابط تذييل `Visit our website` إلى اللون `grey` عندما يزور المستخدم الرابط.

# --hints--

يجب أن تستخدم المحدد الزائف `a:visited`.
...
يجب أن يكون لـ `a:visited` الخاص بك خاصية `color` بقيمة `grey`.
