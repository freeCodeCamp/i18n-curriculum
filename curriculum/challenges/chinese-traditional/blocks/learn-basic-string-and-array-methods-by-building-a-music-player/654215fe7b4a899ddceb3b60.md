...
目前 `songsHTML` 是一個陣列。如果你嘗試直接顯示它，你會看到歌曲以逗號分隔。這不是你想要的結果，因為你想將歌曲顯示為列表。為了解決這個問題，你需要使用 <dfn>join()</dfn> 方法將陣列合併成一個字串。

`join()` 方法用於將陣列中所有元素串接成一個字串。它接受一個名為 `separator` 的選用參數，用來分隔陣列中的每個元素。例如：
...
將 `join()` 方法串鏈到你的 `map()` 方法，並傳入空字串作為分隔符。

要將多個方法串鏈在一起，你可以在 `map()` 方法的結果上呼叫 `join()` 方法。例如：
