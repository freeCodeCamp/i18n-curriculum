...
你的 `allSongs` 數組應該有一個對象作爲其第三個值。

```js
assert.isObject(allSongs[2]);
```

`allSongs` 數組中的第三個對象應將 `id` 屬性設置爲數字 `2`。

```js
assert.equal(allSongs[2].id, 2);
```

`allSongs` 數組中的第三個對象應將 `title` 屬性設置爲字符串 `"Still Learning"`。

```js
assert.equal(allSongs[2].title, "Still Learning");
```

`allSongs` 數組中的第三個對象應將 `artist` 屬性設置爲字符串 `"Quincy Larson"`。

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

`allSongs` 數組中的第三個對象應將 `duration` 屬性設置爲字符串 `"3:51"`。

```js
assert.equal(allSongs[2].duration, "3:51");
```

`allSongs` 數組中的第三個對象應該有 `src` 屬性並將其設置爲 `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`。
