...
Ora ti serve una classe principale che aiuti a renderizzare l'interfaccia utente dinamicamente nel DOM.

Crea una classe chiamata `Game`. Al suo interno, aggiungi una proprietà chiamata `cards` di tipo `Card[]`, e per ora inizializzala con un array vuoto.

# --hints--

Dovresti avere una classe chiamata `Game`.
...
Dovresti avere una proprietà `cards` all'interno della classe `Game`.
...
La tua proprietà `cards` dovrebbe avere un tipo `Card[]`.
...
La tua proprietà cards dovrebbe essere impostata su un `array` vuoto.
