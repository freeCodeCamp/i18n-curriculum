...
Після останнього елемента `.divider` створіть елемент `p` та надайте йому текст `Total Fat 8g 10%`. Потім обгорніть текст `Total Fat` одним елементом `span`, текст `10%` — іншим елементом, та надайте їм клас `bold`.
...
Елемент `p` повинен мати два елементи `span`.
