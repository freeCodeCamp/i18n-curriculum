...
`Unusual activity` refers to suspicious or unexpected actions in a system. Por ejemplo:

`The system sent a warning about unusual activity.` - It gave an alert because something didn't look normal.

`Failed login attempts` means someone tried to log in but was unsuccessful. Por ejemplo:
