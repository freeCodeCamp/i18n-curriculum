...
Cambia il colore di sfondo dell'elemento `hr` in `brown` in modo che corrisponda al colore dei chicchi di caffè.

# --hints--

Dovresti assegnare alla proprietà `background-color` il valore `brown`.
...
L'elemento `hr` dovrebbe avere una proprietà `background-color` con il valore `brown`.
