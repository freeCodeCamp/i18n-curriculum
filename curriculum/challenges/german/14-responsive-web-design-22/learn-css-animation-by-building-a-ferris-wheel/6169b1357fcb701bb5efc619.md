...
Du solltest einen neuen `25%`-Selektor in deiner `@keyframes cabins`-Regel erstellen.
...
Dein `25%`-Selektor sollte sich zwischen deinen `0%`- und `50%`-Selektoren befinden.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '25%');
```

Dein `25%`-Selektor sollte eine `background-color`-Eigenschaft mit dem Wert `yellow` haben.
