...
根据对话，哪句话显示了基于当前证据正确使用 `going to`？
...
这个句子使用了 `will` 来表示一个可能的将来动作，但缺乏基于当前证据所做决定的含义。
...
这里，`will` 用于基于当前情况的未来动作，但由于决策似乎基于团队当前的专业知识，使用 `going to` 会更合适。
...
这个句子使用了 `will` 来表示一个自发的决定（`calling the designer`），这是合适的，但它没有展示基于当前证据使用 `going to`。
