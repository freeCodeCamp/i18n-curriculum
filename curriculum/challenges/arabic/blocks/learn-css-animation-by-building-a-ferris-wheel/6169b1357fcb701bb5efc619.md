...
أنشئ محدِّدًا جديدًا بنسبة `25%` بين محدِّدات `0%` و `50%`. امنح هذا المحدِّد الجديد خاصية `background-color` بقيمة `yellow`.

# --hints--

يجب أن تنشئ محدِّدًا جديدًا بنسبة `25%` في قاعدة `@keyframes cabins` الخاصة بك.
...
يجب أن يحتوي محدِّد `25%` على خاصية `background-color` بقيمة `yellow`.
