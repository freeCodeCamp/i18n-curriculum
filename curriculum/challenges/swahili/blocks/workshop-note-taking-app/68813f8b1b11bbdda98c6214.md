...
Kwa kuwa kipengele cha `div` kinatumika kwa ajili ya noti, visomaji vya skrini havitaelewa kusudi lake. Unaweza kurekebisha hilo kwa kuongeza sifa ya kipengele ya `aria-label`. Hii itasaidia katika Ufikikaji na kutoa maana kwa kipengele hiki cha `div`. 

Katika lebo yako ya ufunguzi ya `div`, ongeza sifa ya kipengele ya `aria-label` na weka thamani yake kuwa `"Note editor"`.

# --hints--

Lebo yako ya ufunguzi ya `div` inapaswa kuwa na sifa ya kipengele ya `aria-label`.

```js
assert.isTrue(document.getElementById("note").hasAttribute("aria-label"));
```

Lebo yako ya ufunguzi ya `div` inapaswa kuwa na sifa ya kipengele ya `aria-label` ikiwa imewekwa kuwa `"Note editor"`.
