...
これから、著者データでUIを埋める関数を作成します。この関数は2つ目の`.then()`メソッド内で呼び出します。

`displayAuthors`をパラメータに取る、空のアロー関数`authors`を作成してください。
...
`const`を使って、空の関数`displayAuthors`を作成してください。
