...
Ikiwa `userData.currentSong` si `null` unahitaji kutafuta wimbo unaofuata kuchezwa. Kumbuka kwamba unaweza kutumia `getNextSong` kwa ajili ya hilo.

Kamilisha kitendakazi chako cha `playNextSong` ili kicheze wimbo unaofuata kwenye orodha ya nyimbo. Ikiwa wimbo wa sasa ni wa mwisho, weka `userData.currentSong` kuwa `null`, `userData.songCurrentTime` kuwa sifuri na fanya wito wa kitendakazi cha `pauseSong`.

# --hints--

Wakati `currentSong` ya `userData` ni kweli na si wimbo wa mwisho wa orodha ya nyimbo, kitendakazi cha `playNextSong` kinapaswa kucheza wimbo unaofuata wa wimbo wa sasa.
...
Wakati `currentSong` ya `userData` ni wimbo wa mwisho wa orodha ya nyimbo, kitendakazi cha `playNextSong` kinapaswa kuweka `userData.currentSong` kuwa `null`.
...
Wakati `currentSong` ya `userData` ni wimbo wa mwisho wa orodha ya nyimbo, kitendakazi cha `playNextSong` kinapaswa kuweka `userData.songCurrentTime` kuwa `0`.
...
Wakati `currentSong` ya `userData` ni wimbo wa mwisho wa orodha ya nyimbo, kitendakazi cha `playNextSong` kinapaswa kuita kitendakazi cha `pauseSong`.
...
Wakati `currentSong` ya `userData` ni sawa na `null`, kitendakazi chako cha `playNextSong` kinapaswa bado kuita kitendakazi cha `playSong` kwa `id` ya wimbo wa kwanza katika safu ya data ya `userData.songs` kama hoja.
