...
O método `map()` cria um array aplicando a função `(arrayValue) => { return arrayValue * 2; }` a cada elemento do array original. Isso é particularmente útil quando você quer transformar os elementos de um array sem alterar o array original.
