---
id: 66c051d13a6a20255a963695
title: 스타일이 적용된 할 일 목록 만들기
challengeType: 25
dashedName: build-a-stylized-to-do-list
demoType: onClick
---

# --description--

이 실습에서는 링크에 마우스를 올리거나, 포커스가 가거나, 클릭되거나, 방문되었을 때 적용할 수 있는 다양한 스타일을 연습합니다.

**목표:** 아래 사용자 스토리를 모두 충족하고 모든 테스트를 통과하여 실습을 완료하세요.

**사용자 스토리:**

1. 클래스가 `todo-list`인 순서 없는 목록이 하나 있어야 합니다.
2. 순서 없는 목록 안에는 네 개의 목록 항목이 있어야 합니다.
3. 각 목록 항목 안에는 다음이 있어야 합니다:

   - 타입이 `input`이고 `checkbox`가 고유한 값으로 설정된 `id` 요소.
   - 해당 `label` 요소의 `for`와 일치하는 값을 가진 `input` 속성이 설정된 `id` 요소.
   - 클래스가 `sub-item`인 순서 없는 목록.
   - 앵커 요소가 포함된 목록 항목. 앵커는 클래스 `sub-item-link`, 유효한 `href` 값, 그리고 새 탭에서 링크가 열리도록 하는 `target` 값을 가져야 합니다.

4. `a` 요소에는 텍스트 장식이 없어야 합니다.
5. 방문하지 않은 링크의 텍스트 색상은 원하는 색상으로 설정하세요.
6. 링크가 방문되었을 때 색상이 다른 원하는 색상으로 바뀌어야 합니다.
7. 링크에 마우스를 올렸을 때 색상이 다른 원하는 색상으로 바뀌어야 합니다.
8. 링크에 포커스가 있을 때 링크 주위에 색상이 있는 윤곽선이 있어야 합니다.
9. 링크를 클릭할 때 색상이 다른 원하는 색상으로 바뀌어야 합니다.

**참고:** HTML에 스타일시트를 연결하고 CSS를 적용하는 것을 잊지 마세요.

# --hints--

클래스가 `todo-list`인 순서 없는 목록이 하나 있어야 합니다.

```js
assert.exists(document.querySelector('ul.todo-list'));
assert.lengthOf(document.querySelectorAll('ul.todo-list'), 1);
```

순서 없는 목록 안에 네 개의 목록 항목이 있어야 합니다.

```js
const li = document.querySelectorAll('ul.todo-list > li');
assert.lengthOf(li, 4);
```

클래스 `li`인 `ul` 안의 `todo-list`에는 `input`인 `type`를 가진 `checkbox` 요소가 있어야 합니다.

```js
const checkboxes = document.querySelectorAll(
  'ul.todo-list li input[type="checkbox"]'
);
assert.lengthOf(checkboxes, 4);
```

클래스 `li`인 `ul` 안의 `todo-list`에는 `label` 요소가 있어야 합니다.

```js
const labels = document.querySelectorAll(
  'ul.todo-list li label'
);
assert.lengthOf(labels, 4);
```

모든 `input` 요소에는 `id`가 있어야 합니다.

```js
const inputs = document.querySelectorAll('input');
assert.isAbove(inputs.length, 0);
for (let input of inputs) {
  assert.isAbove(input?.id.length, 0);
}
```

모든 `label` 요소에는 `for` 속성이 있어야 합니다.

```js
const labels = document.querySelectorAll('label');
assert.isAbove(labels.length, 0);
for (let label of labels) {
  assert.isAbove(label.htmlFor.length, 0);
}
```

모든 `label` 요소에는 텍스트가 있어야 합니다.

```js
const labels = document.querySelectorAll('label');
assert.isAbove(labels.length, 0);
for (let label of labels) {
  assert.isAbove(label.innerText.length, 0);
}
```

`id`와 `for` 요소 쌍의 `input`와 `label` 속성은 서로 대응하는 값을 가져야 합니다.

```js
const labels = document.querySelectorAll("label");
assert.isNotEmpty(labels);

document.querySelectorAll("input").forEach((input, index) => {
  assert.equal(input.id, labels[index].htmlFor);
});
```

`label` 요소 뒤에는 클래스 `sub-item`인 순서 없는 목록이 있어야 합니다.

```js
const labelSbItems = document.querySelectorAll('ul.todo-list li label + ul.sub-item');
assert.lengthOf(labelSbItems, 4);
```

클래스 `li`인 `ul` 안의 `sub-item`에는 클래스 `sub-item-link`인 앵커 요소가 있어야 합니다.

```js
const allItems = document.querySelectorAll(
  'ul.todo-list li label + ul.sub-item li a.sub-item-link'
);
assert.lengthOf(allItems, 4);
```

모든 앵커 요소에는 유효한 `href` 속성이 있어야 합니다.

```js
const links = document.querySelectorAll('a');
assert.isAbove(links.length, 0);
for (let link of links) {
  assert.isAbove(link.href.length, 0);
}
```

모든 앵커 요소에는 텍스트가 있어야 합니다.

```js
const links = document.querySelectorAll('a');
assert.isAbove(links.length, 0);
for (let link of links) {
  assert.isAbove(link.innerText.length, 0);
}
```

각 `a` 요소에는 `target` 값을 가진 `_blank` 속성이 있어야 합니다.

```js
const anchors = document.querySelectorAll('a');
assert.isAbove(anchors.length, 0);
for (let anchor of anchors) {
  assert.equal(anchor.target, '_blank');
}
```

`a` 요소에는 텍스트 장식이 없어야 합니다.

```js
assert.isTrue(
  new __helpers.CSSHelp(document).getStyle('a.sub-item-link')
    ?.textDecoration === 'none' ||
    new __helpers.CSSHelp(document).getStyle('a')?.textDecoration === 'none' ||
    new __helpers.CSSHelp(document).getStyle('.sub-item-link')
      ?.textDecoration === 'none'
);
```

방문하지 않은 링크는 원하는 색상으로 텍스트 색상이 설정되어야 합니다.

```js
assert.exists(
  new __helpers.CSSHelp(document).getStyle('.sub-item-link:link')?.color ||
    new __helpers.CSSHelp(document).getStyle('a:link')?.color ||
    new __helpers.CSSHelp(document).getStyle('a.sub-item-link:link')?.color
);
```

링크에 마우스를 올렸을 때 색상이 바뀌어야 합니다.

```js
assert.exists(
  new __helpers.CSSHelp(document).getStyle('.sub-item-link:hover')?.color ||
    new __helpers.CSSHelp(document).getStyle('a:hover')?.color ||
    new __helpers.CSSHelp(document).getStyle('a.sub-item-link:hover')?.color
);
```

링크를 클릭할 때 색상이 바뀌어야 합니다.

```js
assert.exists(
  new __helpers.CSSHelp(document).getStyle('.sub-item-link:active')?.color ||
    new __helpers.CSSHelp(document).getStyle('a:active')?.color ||
    new __helpers.CSSHelp(document).getStyle('a.sub-item-link:active')?.color
);
```

링크에 포커스가 있을 때 윤곽선이 있어야 합니다.

```js
const css = new __helpers.CSSHelp(document);
assert.exists(
  css.getStyle('.sub-item-link:focus')?.outline ||
    css.getStyle('a:focus')?.outline ||
    css.getStyle('a.sub-item-link:focus')?.outline ||
    css.getStyle('.sub-item-link:focus')?.border ||
    css.getStyle('a:focus')?.border ||
    css.getStyle('a.sub-item-link:focus')?.border
);
```

링크가 방문되었을 때 색상이 바뀌어야 합니다.

```js
assert.exists(
  new __helpers.CSSHelp(document).getStyle('.sub-item-link:visited')?.color ||
    new __helpers.CSSHelp(document).getStyle('a:visited')?.color ||
    new __helpers.CSSHelp(document).getStyle('a.sub-item-link:visited')?.color
);
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <title>Styled To-Do List</title>
</head>

<body>

</body>

</html>
```

```css

```

# --solutions--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Styled To-Do List</title>
    <link rel="stylesheet" href="styles.css" />
  </head>

  <body>
    <div class="container">
      <h1>My To-Do List</h1>
      <ul class="todo-list">
        <li>
          <input type="checkbox" id="task1" />
          <label for="task1">Explore gaming keyboards</label>
          <ul class="sub-item">
            <li>
              <a href="#" target="_blank" class="sub-item-link">Store link</a>
            </li>
          </ul>
        </li>

        <li>
          <input type="checkbox" id="task2" />
          <label for="task2">Finish the report</label>
          <ul class="sub-item">
            <li>
              <a href="#" target="_blank" class="sub-item-link"
                >Request Access</a
              >
            </li>
          </ul>
        </li>

        <li>
          <input type="checkbox" id="task3" />
          <label for="task3">View Phone's Warranty</label>
          <ul class="sub-item">
            <li>
              <a href="#" target="_blank" class="sub-item-link"
                >View Receipts</a
              >
            </li>
          </ul>
        </li>

        <li>
          <input type="checkbox" id="task4" />
          <label for="task4">Check Processor Specs</label>
          <ul class="sub-item">
            <li>
              <a href="#" target="_blank" class="sub-item-link"
                >View Model Number</a
              >
            </li>
          </ul>
        </li>
      </ul>
    </div>
</body>
</html>
```

```css
body {
  font-family: Arial, sans-serif;
  background-color: #f0f0f0;
  margin: 0;
  padding: 20px;
}

.container {
  max-width: 400px;
  margin: 0 auto;
  background-color: #fff;
  padding: 20px;
  border-radius: 10px;
  box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);
}

h1 {
  text-align: center;
  color: #333;
}

.todo-list {
  list-style-type: none;
  padding: 0;
}

.todo-list li {
  margin-bottom: 15px;
  line-height: 1.6;
  padding: 10px;
  background-color: #e5f1e5;
  border-radius: 10px;
  position: relative;
}

a.sub-item-link {
  margin-left: 10px;
  text-decoration: none;
}

.sub-item-link:link {
  color: blue;
}

.sub-item-link:visited {
  color: purple;
}

.sub-item-link:hover {
  color: red;
}

.sub-item-link:focus {
  outline: 2px solid blue;
  outline-offset: 2px;
}

.sub-item-link:active {
  color: green;
}

.sub-item {
  list-style-type: square;
}
```
