...
Sasa unahitaji kujiandaa kusahihisha onyesho na bidhaa mpya ambayo mtumizi ameongeza. Tangaza mabadiliko ya `currentProductCount`, na weka thamani yake kuwa sifa ya kitu cha `totalCountPerProduct` inayolingana na `id` ya `product`.

# --hints--

Unapaswa kutangaza mabadiliko ya `currentProductCount` katika kitendakazi chako cha `addItem`.
...
Unapaswa kutumia `const` kutangaza `currentProductCount`.
...
Unapaswa kuweka thamani ya `totalCountPerProduct[product.id]` kwa `currentProductCount`.
