...
Unganisha vipengele vinavyofaa vya fomu na vipengele vya `label` pamoja.

Tumia `profile-picture`, `age`, `referrer`, na `bio` kama thamani za sifa za `id` zinazohusika.

# --hints--

Kipengele cha kwanza cha `input` kinapaswa kuwa na `id` ya `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[0]?.matches('#profile-picture'))
```

Kipengele cha pili cha `input` kinapaswa kuwa na `id` ya `age`.
...
Kipengele cha kwanza cha `label` kinapaswa kuwa na sifa ya `for` yenye thamani ya `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[0]?.matches('label[for="profile-picture"]'))
```

Kipengele cha pili cha `label` kinapaswa kuwa na sifa ya `for` yenye thamani ya `age`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[1]?.matches('label[for="age"]'))
```

Kipengele cha tatu cha `label` kinapaswa kuwa na sifa ya `for` yenye thamani ya `referrer`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[2]?.matches('label[for="referrer"]'))
```

Kipengele cha nne cha `label` kinapaswa kuwa na sifa ya `for` yenye thamani ya `bio`.
