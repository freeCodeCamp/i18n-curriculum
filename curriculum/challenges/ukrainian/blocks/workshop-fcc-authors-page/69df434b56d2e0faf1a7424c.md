...
Тепер, коли ваша сторінка авторів fCC повністю функціональна, давайте рефакторимо її, щоб покращити читабельність, використовуючи `async` та `await` замість методу ланцюжка промісів `then`.

Нагадаємо, що для використання оператора `await` для очікування функції, яка повертає `Promise`, потрібно обгорнути виклик функції у `async` функцію (якщо вона не знаходиться в основному тілі модуля).

Оскільки ваш виклик `fetch` не визначений ні в асинхронній функції, ні в основному тілі модуля, ви не можете використовувати `await` до виправлення цього.

Обгорніть інструкцію `fetch` та всю послідовність ланцюжка методів then і catch у нову асинхронну стрілкову функцію з назвою `initialFetch`, яка не приймає аргументів.

Приклад оголошення `async` функції:
...
`initialFetch` має бути функцією.

```js
assert.isFunction(initialFetch)
```

Ви повинні використовувати `const` для створення `initialFetch` функції.

```js
assert.match(code, /const\s+initialFetch\s*=\s*/)
```

`initialFetch` має бути асинхронною функцією.
...
Ваша функція `initialFetch` не повинна приймати жодних параметрів.

```js
assert.match(code, /const\s+initialFetch\s*=\s*.*\s*\(\s*\)\s*/)
```

Ваша функція `initialFetch` має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+initialFetch\s*=\s*.*\s*\(.*\)\s*=>\s*/)
```

Ваша функція `initialFetch` не повинна бути порожньою.
...
Ваша функція `initialFetch` має містити інструкцію `fetch` та ланцюжок методів.
