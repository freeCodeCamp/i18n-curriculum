...
由於 fortune 應用程式只有三個時刻：Past、Present 和 Future，請宣告一個名為 `DrawingType` 的列舉，並包含三個屬性：`Past`、`Present` 和 `Future`。每個屬性都應該有一個與其名稱相對應的字串值。

# --hints--

你應該有一個名為 `enum` 的 `DrawingType`。

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

你的列舉應該有一個 `Past` 屬性，字串值為 `past`。

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

你的列舉應該有一個 `Present` 屬性，字串值為 `present`。

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

你的列舉應該有一個 `Future` 屬性，字串值為 `future`。
