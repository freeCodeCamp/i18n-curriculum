...
次に、`json` メソッドを呼び出しているコードの行を、`await` 演算子を使うように変更してください。
...
そして、結果を `authorDataArr` にストアし、`displayAuthors` を呼び出しているコードの行を、新しい `await` 文の直後に移動させてください。
最後に、もう不要になった `then` 文とそのコードブロックを削除してください。

この時点でコンソールに `SyntaxError` が表示されるのがわかるでしょう。次のステップでは、このエラーを解決するために `catch` メソッドのリファクタリングを行います。
