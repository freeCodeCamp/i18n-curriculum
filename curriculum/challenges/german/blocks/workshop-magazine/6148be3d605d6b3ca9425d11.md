...
Der Untertitel muss ebenfalls gestylt werden. Erstellen Sie einen `.hero-subtitle`-Selektor und geben Sie ihm eine `font-size`-Eigenschaft mit dem Wert `2.4rem`, eine `color`-Eigenschaft mit dem Wert `orangered` und eine `text-align`-Eigenschaft mit dem Wert `center`.

# --hints--

Sie sollten einen `.hero-subtitle`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

Ihr `.hero-subtitle`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `2.4rem` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize, '2.4rem');
```

Ihr `.hero-subtitle`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `orangered` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color, 'orangered');
```

Ihr `.hero-subtitle`-Selektor sollte eine `text-align`-Eigenschaft mit dem Wert `center` haben.
