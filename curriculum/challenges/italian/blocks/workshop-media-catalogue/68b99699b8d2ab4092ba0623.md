...
Dopo l'istruzione `if` nel tuo metodo `__str__`, crea due variabili chiamate `movies` e `series` e assegna loro rispettivamente una chiamata al metodo `get_movies` e una chiamata al metodo `get_tv_series`.

# --hints--

Dovresti avere una variabile chiamata `movies` nel tuo metodo `__str__`.
...
Assegni `self.get_movies()` alla tua variabile `movies`.
...
Dovresti avere una variabile chiamata `series` nel tuo metodo `__str__`.
...
Assegni `self.get_tv_series()` alla tua variabile `series`.
