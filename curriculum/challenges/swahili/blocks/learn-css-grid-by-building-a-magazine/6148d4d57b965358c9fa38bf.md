...
Sasa ni wakati wa kupamba `section` yako ya tatu. Kumbuka ina thamani za `text` na `text-with-images` kwa sifa ya kipengele ya `class`, ambayo ina maana tayari inarithi mitindo kutoka kwa sheria yako ya `.text`.

Tengeneza kichaguzi cha `.text-with-images` na weka sifa ya `display` kuwa `grid`.
...
Kichaguzi chako cha `.text-with-images` kinapaswa kuwa na sifa ya `display` iliyowekwa kuwa `grid`.
