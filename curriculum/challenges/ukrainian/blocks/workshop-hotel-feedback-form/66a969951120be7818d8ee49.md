...
Наступний розділ форми дасть користувачам змогу залишити оцінку готелю.

Почніть із додавання нового `fieldset` елемента з вкладеним `legend` елементом. `legend` має містити текст `Ratings`.

Під `legend` елементом додайте `label` елемент із текстом `How was the service?`. Атрибут `for` має бути встановлений на `"service"`.

# --hints--

У вас має бути `fieldset` елемент.

```js
assert.lengthOf(document.querySelectorAll('fieldset'), 4);
```

Ваш `fieldset` має містити `legend` елемент.

```js
assert.lengthOf(document.querySelectorAll('fieldset:nth-of-type(4) legend'), 1);
```

У вашого legend має бути текст `Ratings`.

```js
assert.strictEqual(document.querySelectorAll('fieldset:nth-of-type(4) legend')[0]?.textContent.trim(), 'Ratings');
```

Ваш `fieldset` має містити `label` елемент під `legend`.

```js
assert.lengthOf(document.querySelectorAll('fieldset:nth-of-type(4) legend + label'), 1);
```

Ваш `label` має містити текст `How was the service?`.

```js
assert.strictEqual(document.querySelectorAll('fieldset:nth-of-type(4) legend + label')[0]?.textContent.trim(), 'How was the service?');
```

Ваш `label` має мати атрибут `for`, встановлений на `"service"`.
