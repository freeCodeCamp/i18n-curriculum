...
你的第三个 `div` 元素的 `className` 的 `form-group` 应该有一个 `label`，它有 `htmlFor` 属性为 `pas`，`className` 为 `label`，以及文本 `PAS`。
...
你的第三个 `div` 元素的 `className` 的 `form-group` 应该有一个 `input`，它有 `className` 为 `input`，`id` 为 `pas`，以及 `type` 为 `number`。
...
你的第四个 `div` 元素的 `className` 的 `form-group` 应该有一个 `label`，它有 `htmlFor` 属性为 `dri`，`className` 为 `label`，以及文本 `DRI`。
...
你的第四个 `div` 元素的 `className` 的 `form-group` 应该有一个 `input`，它有 `className` 为 `input`，`id` 为 `dri`，以及 `type` 为 `number`。
