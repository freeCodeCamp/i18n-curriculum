...
Lembre-se de que você pode usar desestruturação para extrair múltiplos valores de um array ou objeto em uma única declaração.

Para o primeiro parâmetro da sua função de callback, desestruture as propriedades `name`, `id`, `price` e `category` da instância `Dessert` passada.

# --hints--

Você deve usar desestruturação para declarar os parâmetros `name`, `id`, `price` e `category`. Para este teste, a ordem importa.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

Sua desestruturação deve ser o primeiro parâmetro da função de callback.
