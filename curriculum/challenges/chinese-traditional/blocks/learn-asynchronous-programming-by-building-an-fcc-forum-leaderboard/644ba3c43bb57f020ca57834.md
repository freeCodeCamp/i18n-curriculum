...
你現在需要為你的 `selectedCategory` 物件添加 `className` 和 `category` 屬性。

首先將 `className` 變數指定為 `selectedCategory.className`。然後將 `category` 變數指定為 `selectedCategory.category`。

# --hints--

你應該使用點記法來標的 `selectedCategory` 物件的 `className` 屬性。

```js
assert.match(code, /selectedCategory\.className/);
```

你應該將 `allCategories` 物件的 `className` 屬性指定給 `selectedCategory` 物件的 `className` 屬性。

```js
assert.match(code, /selectedCategory\.className\s*=\s*className/);
```

你應該使用點記法來標的 `selectedCategory` 物件的 `category` 屬性。

```js
assert.match(code, /selectedCategory\.category/);
```

你應該將 `allCategories` 物件的 `category` 屬性指定到 `selectedCategory` 物件的 `category` 屬性。
