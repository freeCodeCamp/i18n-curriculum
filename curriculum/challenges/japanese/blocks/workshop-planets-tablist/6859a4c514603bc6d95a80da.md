...
タブパネルをそれぞれのタブに関連付ける時間です。

最初の `tabpanel` 要素に、値が `id` の `"panel-earth"` 属性と、値が `aria-labelledby` の `"tab-earth"` 属性を追加してください。

# --hints--

最初の `tabpanel` 要素には `id` 属性が必要です。

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("id"));
```

`id` 属性の値は `"panel-earth"` にしてください。

```js
assert.equal(document.querySelector('[role="tabpanel"]').getAttribute("id"), "panel-earth");
```

最初の `tabpanel` 要素には `aria-labelledby` 属性が必要です。

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("aria-labelledby"));
```

`aria-labelledby` 属性の値は `"tab-earth"` にしてください。
