...
Finalmente, anide el elemento `span` que contiene el texto `Cholesterol` junto con el texto `0mg`, en un elemento `span` adicional para alineación.
...
Su `span` de `Cholesterol` debe tener el atributo `class` establecido a `bold`.
...
Su `span` de `0%` debe tener el atributo `class` establecido a `bold`.
...
Su `span` de `Cholesterol` y su texto `0mg` deben estar envueltos en un `span`.
