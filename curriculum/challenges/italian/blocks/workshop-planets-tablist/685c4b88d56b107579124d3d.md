...
Se guardi l'HTML, vedrai che il secondo e il terzo pannello sono nascosti. Questo viene fatto usando l'attributo `hidden`. Il primo pannello è visibile per impostazione predefinita. Quando un utente clicca su una scheda, tutti i pannelli dovrebbero essere nascosti.

Usa il metodo `forEach` per iterare attraverso `panels` e impostare l'attributo `hidden` su `true` per ogni pannello.

# --hints--

Dovresti iterare attraverso `panels` e impostare l'attributo `hidden` di ogni pannello su `true`.
