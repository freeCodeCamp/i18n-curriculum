...
`% Daily Value *` 和 `Saturated Fat 1g 5%` 元素下的底部边框没有扩展至标签的整个宽度。 将 `no-divider` 添加到这两个元素的 `class` 中。

# --hints--

你的文本为 `% Daily Value *` 的 `p` 元素应该将 `no-divider` 添加到 `class` 属性。 不要删除现有的类。
...
你的文本为 `Saturated Fat 1g 5%` 的 `p` 元素应该将 `no-divider` 添加到 `class` 属性。 不要删除现有的类。
