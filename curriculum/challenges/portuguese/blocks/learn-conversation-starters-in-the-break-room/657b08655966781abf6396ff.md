---
id: 657b08655966781abf6396ff
title: Tarefa 3
challengeType: 19
dashedName: task-3
---

<!-- (audio) Sarah: Hi, Tom. Are you happy with the workplace so far? -->

# --description--

Ouça o áudio e responda a pergunta abaixo.

# --questions--

## --text--

Qual frase está gramaticalmente correta?

## --answers--

`You are happy with the workplace so far?`

### --feedback--

Esta é a ordem das palavras para uma declaração, não uma pergunta.

---

`Are you happy with the workplace so far?`

---

`Is you happy with the workplace so far?`

### --feedback--

`Is` não é a forma correta de `to be` para o sujeito `you`.

---

`Happy you are with the workplace so far?`

### --feedback--

Esta opção tem palavras em uma ordem misturada que não forma uma pergunta correta.

## --video-solution--

2

# --scene--

```json
{
  "setup": {
    "background": "company1-reception.png",
    "characters": [
      {
        "character": "Sarah",
        "position": {"x":50,"y":0,"z":1.4},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "1.3-1.mp3",
      "startTime": 1,
      "startTimestamp": 0.00,
      "finishTimestamp": 2.78
    }
  },
  "commands": [
    {
      "character": "Sarah",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sarah",
      "startTime": 1,
      "finishTime": 3.78,
      "dialogue": {
        "text": "Hi, Tom. Are you happy with the workplace so far?",
        "align": "center"
      }
    },
    {
      "character": "Sarah",
      "opacity": 0,
      "startTime": 4.28
    }
  ]
}
```
