...
`PlayerCard` 내부에서 참조되는 `FootballPlayerCard`에 `player` 프로프를 받아 다음 속성을 가진 객체를 전달하세요:
...
`player` 내부의 `PlayerCard`에 `FootballPlayerCard` 프로프를 전달해야 합니다.
...
`player` 프로프는 값이 `name`인 `'PELE'` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `overallRating`인 `98` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `position`인 `'ST'` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `club`인 `'Santos FC'` 속성을 가져야 합니다.
...
`player` 프로프는 Pele 이미지 URL이 값인 `imageUrl` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `pac`인 `97` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `sho`인 `98` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `pas`인 `83` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `dri`인 `99` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `def`인 `41` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `phy`인 `75` 속성을 가져야 합니다.
