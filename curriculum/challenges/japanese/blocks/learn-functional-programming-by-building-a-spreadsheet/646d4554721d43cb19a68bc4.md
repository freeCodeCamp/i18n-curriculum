...
`fn` パラメータには、`"SUM"` のような関数名が渡されます。`apply` オブジェクトから `spreadsheetFunctions` 変数をキーとしてアクセスし、その関数を暗黙的に返すように `fn` を更新してください。

`fn` は小文字の文字列でない場合があるので、小文字の文字列に変換する必要があることを覚えておいてください。

# --hints--

`apply` 関数は暗黙の return を使うべきです。
...
`apply` 関数は `spreadsheetFunctions` オブジェクトにアクセスするべきです。
...
`apply` 関数は `spreadsheetFunctions` オブジェクトの `fn` の値に一致するプロパティにアクセスするべきです。
...
`apply` 関数はプロパティアクセスで `.toLowerCase()` の `fn` メソッドを呼び出すべきです。
