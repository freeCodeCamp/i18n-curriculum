...
もし `messageInput` が空の場合、メッセージ `"Please enter a message."` を表示するアラートをユーザーに出してください。

その後、関数の実行を終了してください。

# --hints--

コールバック関数には `if` 文が必要です。

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

`if` 文では、`value` の `messageInput` が空の文字列かどうかを確認してください。
...
`if` 文では、メッセージ `"Please enter a message."` を表示するアラートをユーザーに出してください。
