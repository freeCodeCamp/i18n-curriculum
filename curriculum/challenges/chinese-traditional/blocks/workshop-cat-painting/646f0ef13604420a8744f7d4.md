...
在 `.cat-whiskers` 元素內，創建兩個具有類別 `cat-whiskers-left` 和 `cat-whiskers-right` 的 `div` 元素。

# --hints--

你不應該更改具有 `cat-whiskers` 類別的現有 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers'), 1);
```

你應該在 `.cat-whiskers` 元素內有兩個 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers div'), 2);
```

`.cat-whiskers` 元素內的第一個 `div` 元素應該有 `cat-whiskers-left` 類別。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers div')[0].classList.contains('cat-whiskers-left'));
```

`.cat-whiskers` 元素內的第二個 `div` 元素應該有 `cat-whiskers-right` 類別。
