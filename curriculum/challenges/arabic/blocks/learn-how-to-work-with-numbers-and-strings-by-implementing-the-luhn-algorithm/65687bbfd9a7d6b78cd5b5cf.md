...
كرر على الأرقام الزوجية واطبع كل رقم في وحدة التحكم.

# --hints--

يجب أن يكون لديك حلقة `for` على `even_digits` داخل دالة `verify_card_number`.
...
يجب أن يحتوي حلقة `for` على `--fcc-expected--`.
