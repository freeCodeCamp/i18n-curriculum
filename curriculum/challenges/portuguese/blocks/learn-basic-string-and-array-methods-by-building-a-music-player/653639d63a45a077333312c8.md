...
Antes de tocar a música, você precisa garantir que ela comece do início. Isso pode ser alcançado pelo uso da propriedade `currentTime` no objeto `audio`. 

Adicione uma declaração `if` para verificar se `userData?.currentSong` é falso *OU* se `userData?.currentSong.id` é estritamente diferente de `song.id`. Esta condição verificará se nenhuma música está tocando no momento ou se a música atual é diferente daquela que está prestes a ser reproduzida.

Dentro do bloco `if`, defina a propriedade `currentTime` do objeto `audio` para `0`.

# --hints--

Você deve criar uma declaração `if` com a condição `userData?.currentSong === null || userData?.currentSong.id !== song.id`. Não esqueça de incluir optional chaining.
...
Você deve definir `audio.currentTime` para `0` dentro do seu bloco `if`.
