...
為 DEF 和 PHY 兩個輸入框添加 `value` 和 `onChange` 屬性。

對於 DEF，將 `value` 設為 `player.def`，並將 `onChange` 設為一個箭頭函式，該函式呼叫 `setPlayer`，並使用展開運算子展開 `player`，同時將 `def` 設為 `Number(e.target.value)`。

對於 PHY，將 `value` 設為 `player.phy`，並將 `onChange` 設為一個箭頭函式，該函式呼叫 `setPlayer`，並使用展開運算子展開 `player`，同時將 `phy` 設為 `Number(e.target.value)`。

完成此步驟後，所有輸入框都已連接到狀態，實時預覽將會隨每個欄位更新！

# --hints--

你的 DEF `input` 應該有一個 `value`，其值等於玩家的 `def` 屬性。
...
你的 DEF `input` 應該有一個 `onChange` 處理常式，該處理常式呼叫 `setPlayer`，並使用展開運算子展開 `player`，同時將 `def` 設為 `Number(e.target.value)`。
...
你的 PHY `input` 應該有一個 `value`，其值等於玩家的 `phy` 屬性。
...
你的 PHY `input` 應該有一個 `onChange` 處理常式，該處理常式呼叫 `setPlayer`，並使用展開運算子展開 `player`，同時將 `phy` 設為 `Number(e.target.value)`。
