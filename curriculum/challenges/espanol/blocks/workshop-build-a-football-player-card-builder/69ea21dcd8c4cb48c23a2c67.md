...
Agrega un atributo `onChange` asignado a una función flecha que toma `e` como parámetro y llama a `setPlayer` con un spread de `player` y `position` establecido en `e.target.value`, convertido a `Position`. 

# --hints--

Tu posición `select` debe tener un manejador `onChange` que llame a `setPlayer` con un spread de `player` y actualice `position`.
