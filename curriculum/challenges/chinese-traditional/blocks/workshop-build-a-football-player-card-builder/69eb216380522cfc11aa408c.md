...
三列中的 `span` 元素應該有以下這些值：

- `97` 和 `PAC`
- `98` 和 `SHO`
- `83` 和 `PAS`

`PAC` 代表速度，`SHO` 表述射門，`PAS` 表述傳球。

數字放在 `stat-value` 範圍內，屬性放在 `stat-label` 範圍內。

# --hints--

第一個 `.stat-row` 應該有 `.stat-value` 為 `97`，以及 `.stat-label` 為 `PAC`。
...
第二個 `.stat-row` 應該有 `.stat-value` 為 `98`，以及 `.stat-label` 為 `SHO`。
...
第三個 `.stat-row` 應該有 `.stat-value` 為 `83`，以及 `.stat-label` 為 `PAS`。
