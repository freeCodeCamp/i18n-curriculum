...
那會移動徽章的文字，但不完全是在你想要的位置。關鍵在於 `right-[num]`、`left-[num]`、`top-[num]` 和 `bottom-[num]` 工具類別。
...
為你的 `Most popular` 徽章 `div` 添加類別 `-top-3` 和 `right-3`。

# --hints--

你的 `div` 元素中帶有 `Most popular` 文字，應該有 `-top-3` 類別。
...
你的 `div` 元素中帶有 `Most popular` 文字應該有 `right-3` 類別。
