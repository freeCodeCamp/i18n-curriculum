...
Dentro do terceiro item da lista, crie um elemento `button` com um atributo `aria-expanded` definido como `false`. Além disso, defina o texto do `button` para `Apps`.

O atributo `aria-expanded` comunicará aos usuários de leitores de tela que o botão aciona um submenu expansível.

# --hints--

Você deve criar um elemento `button` dentro do terceiro item da lista.
...
Seu elemento `button` deve ter o texto `Apps`.
