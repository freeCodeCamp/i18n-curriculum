...
选择 `.blush` 元素，并将其 `width` 设置为 `15%`、`height` 设置为 `10%`，`background-color` 设置为 `pink`。
...
你应该将 `.blush` 的 `width` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.width, '15%');
```

你应该将 `.blush` 的 `height` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.height, '10%');
```

你应该将 `.blush` 的 `background-color` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
