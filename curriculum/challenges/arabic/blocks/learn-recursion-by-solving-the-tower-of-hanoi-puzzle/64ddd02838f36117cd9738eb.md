...
خارج كتلة `else`، أضف تعليقًا يقول `display our progress` واطبع محتوى القوائم للتحقق من أن كل شيء يعمل.

# --hints--

يجب استخدام النص المقدم لإضافة تعليق خارج كتلة `else`.

```js
({ test: () => assert.match(code, /rods\s*\[\s*source\s*\]\s*\.append\s*\(\s*rods\s*\[\s*target\s*\]\s*\.pop\s*\(\s*\)\s*\)\s+#\s*display\sour\sprogress/) })
```

يجب طباعة كائن `rods` خارج كتلة `else`، بعد التعليق.
