...
No tempo `Present Perfect`, `for` e `since` são usados para discutir duração de tempo e pontos de início. 

`For` indica há quanto tempo algo está acontecendo. Por exemplo, `for five years` ou `for two months`.

`Since` indica quando algo começou. Por exemplo, `since 2020` ou `since last week`.
...
Esta preposição expressa a duração de uma ação ou situação.
