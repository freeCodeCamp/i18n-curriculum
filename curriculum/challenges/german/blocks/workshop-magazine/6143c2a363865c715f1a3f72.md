...
Innerhalb des `.image-wrapper`-Elements geben Sie Ihrem ersten `img`-Element ein `src` von `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, ein `alt` von `image of the quote machine project`, ein `class` von `image-1`, ein `loading`-Attribut mit dem Wert `lazy`, ein `width`-Attribut mit dem Wert `600` und ein `height`-Attribut mit dem Wert `400`.

# --hints--

Ihr erstes `img`-Element sollte ein `src`-Attribut mit dem Wert `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png` haben.
...
Ihr erstes `img`-Element sollte ein `alt`-Attribut mit dem Wert `image of the quote machine project` haben.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('alt'), 'image of the quote machine project');
```

Ihr erstes `img`-Element sollte ein `class`-Attribut mit dem Wert `image-1` haben.

```js
assert.isTrue(document.querySelectorAll('.image-wrapper img')?.[0]?.classList?.contains('image-1'));
```

Ihr erstes `img`-Element sollte ein `loading`-Attribut mit dem Wert `lazy` haben.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('loading'), 'lazy');
```

Ihr erstes `img`-Element sollte ein `width`-Attribut mit dem Wert `600` haben.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('width'), '600');
```

Ihr erstes `img`-Element sollte ein `height`-Attribut mit dem Wert `400` haben.
