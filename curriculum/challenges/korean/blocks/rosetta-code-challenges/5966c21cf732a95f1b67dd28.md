---
id: 5966c21cf732a95f1b67dd28
title: 날짜 조작
challengeType: 1
forumTopicId: 302244
dashedName: date-manipulation
---

# --description--

EST 기준 날짜 문자열이 주어지면, 시간에 12시간을 더한 날짜를 문자열로 출력하세요. 시간대는 유지해야 합니다.

예시 입력: `"March 6 2009 7:30pm EST"`

예시 출력: `"March 7 2009 7:30am EST"`

# --hints--

`add12Hours`는 함수여야 합니다.

```js
assert(typeof add12Hours === 'function');
```

`add12Hours(dateString)`는 문자열을 반환해야 합니다.

```js
assert(typeof add12Hours('January 17 2017 11:43am EST') === 'string');
```

`add12Hours("January 17 2017 11:43am EST")`는 `"January 17 2017 11:43pm EST"`를 반환해야 합니다.

```js
assert(
  add12Hours('January 17 2017 11:43am EST') === 'January 17 2017 11:43pm EST'
);
```

날짜 변경을 처리해야 합니다. `add12Hours("March 6 2009 7:30pm EST")`는 `"March 7 2009 7:30am EST"`를 반환해야 합니다.

```js
assert(add12Hours('March 6 2009 7:30pm EST') === 'March 7 2009 7:30am EST');
```

윤년의 월 변경을 처리해야 합니다. `add12Hours("February 29 2004 9:15pm EST")`는 `"March 1 2004 9:15am EST"`를 반환해야 합니다.

```js
assert(add12Hours('February 29 2004 9:15pm EST') === 'March 1 2004 9:15am EST');
```

평년의 월 변경을 처리해야 합니다. `add12Hours("February 28 1999 3:15pm EST")`는 `"March 1 1999 3:15am EST"`를 반환해야 합니다.

```js
assert(add12Hours('February 28 1999 3:15pm EST') === 'March 1 1999 3:15am EST');
```

연도 변경을 처리해야 합니다. `add12Hours("December 31 2020 1:45pm EST")`는 `"January 1 2021 1:45am EST"`를 반환해야 합니다.

```js
assert(
  add12Hours('December 31 2020 1:45pm EST') === 'January 1 2021 1:45am EST'
);
```

# --seed--

## --seed-contents--

```js
function add12Hours(dateString) {

  return true;
}
```

# --solutions--

```js
function add12Hours(dateString) {
  const months = ['January', 'February', 'March', 'April', 'May', 'June',
    'July', 'August', 'September', 'October', 'November', 'December'];
  // Get the parts of the string
  const parts = dateString.split(' ');
  const month = months.indexOf(parts[0]);
  const day = parseInt(parts[1], 10);
  const year = parseInt(parts[2], 10);
  const time = parts[3].split(':');
  let hours = parseInt(time[0], 10);
  if (time[1].slice(-2) === 'pm') {
    hours += 12;
  }
  const minutes = parseInt(time[1].slice(0, -2), 10);

  // Create a date with given parts, and updated hours
  const date = new Date();
  date.setFullYear(year, month, day);
  date.setHours(hours + 12);  // Add 12 hours
  date.setMinutes(minutes);

  let hoursOutput = date.getHours();
  let abbreviation = 'am';
  if (hoursOutput > 12) {
    hoursOutput -= 12;
    abbreviation = 'pm';
  }

  return `${months[date.getMonth()]} ${date.getDate()} ${date.getFullYear()} ${hoursOutput}:${date.getMinutes()}${abbreviation} EST`;
}
```
