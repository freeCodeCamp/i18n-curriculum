...
Um den Zitattext hervorzuheben, geben Sie dem `.quote`-Selektor eine `font-family`-Eigenschaft mit dem Wert `Raleway` und einem Fallback von `sans-serif`.

# --hints--

Ihr `.quote`-Selektor sollte eine `font-family`-Eigenschaft mit dem Wert `Raleway` und einem Fallback von `sans-serif` haben.
