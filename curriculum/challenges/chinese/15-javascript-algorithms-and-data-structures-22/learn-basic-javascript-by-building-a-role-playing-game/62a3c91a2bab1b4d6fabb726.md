...
现在创建一个 `goCave` 函数，将 `"Going to cave."` 打印到控制台。

# --hints--

你应该使用 `function` 关键字来声明 `goCave`。
...
你的代码中应该有一个 `console.log("Going to cave.");` 行。

```js
assert.match(code, /console\.log\(\s*('|")Going to cave\.\1\s*\)/);
```

你的 `console.log("Going to cave.");` 行应该位于你的 `goCave` 函数内。
