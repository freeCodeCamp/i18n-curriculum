...
وسّط نص `.header`. اجعل النص بأحرف كبيرة باستخدام الخاصية `text-transform` مع القيمة `uppercase`.

أضف حشو `32px` من جميع الجهات. اضبط الخلفية على `#0a0a23` والنص على اللون `#fff`.
