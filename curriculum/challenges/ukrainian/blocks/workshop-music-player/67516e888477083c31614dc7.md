...
Якщо придивитися, то ви побачите, що пісня, яка зараз відтворюється, не виділена у плейлисті, тож ви насправді не знаєте, яка пісня грає. Ви можете виправити це, створивши функцію, яка виділятиме будь-яку пісню, що відтворюється.

Створіть функцію з іменем `highlightCurrentSong`. Усередині функції перевірте, чи існує елемент `.playlist-song` з `aria-current="true"`. Якщо так, використайте метод `removeAttribute`, щоб видалити атрибут `aria-current` з цього елемента.

# --hints--

У вас має бути функція з іменем `highlightCurrentSong`.

```js
assert.isFunction(highlightCurrentSong)
```

Ваша функція `highlightCurrentSong` має використовувати метод `removeAttribute`, щоб видалити атрибут `aria-current` з елемента `.playlist-song` з `aria-current="true"`, якщо цей елемент існує.
