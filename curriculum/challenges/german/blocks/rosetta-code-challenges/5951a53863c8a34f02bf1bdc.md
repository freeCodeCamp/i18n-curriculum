...
title: Problem der nächsten Punktpaare
challengeType: 1
forumTopicId: 302232
...
Stellen Sie eine Funktion bereit, die die zwei nächstgelegenen Punkte aus einer Menge gegebener Punkte in zwei Dimensionen findet.

Die einfache Lösung ist ein $O(n^2)$-Algorithmus (den wir *Brute-Force-Algorithmus* nennen können); der Pseudocode (unter Verwendung von Indizes) könnte einfach so aussehen:

<pre><strong>bruteForceClosestPair</strong> von P(1), P(2), ... P(N)
<strong>wenn</strong> N &#x3C; 2 <strong>dann</strong>
  <strong>gib zurück</strong> ∞
<strong>sonst</strong>
  minDistance ← |P(1) - P(2)|
  minPoints ← { P(1), P(2) }
  <strong>für jedes</strong> i ∈ [1, N-1]
    <strong>für jedes</strong> j ∈ [i+1, N]
      <strong>wenn</strong> |P(i) - P(j)| &#x3C; minDistance <strong>dann</strong>
        minDistance ← |P(i) - P(j)|
        minPoints ← { P(i), P(j) }
      <strong>endif</strong>
    <strong>endfor</strong>
  <strong>endfor</strong>
  <strong>gib zurück</strong> minDistance, minPoints
<strong>endif</strong>
</pre>

Ein besserer Algorithmus basiert auf dem rekursiven Divide-and-Conquer-Ansatz, der $O(n\log n)$ ist; ein Pseudocode könnte so aussehen:

<pre><strong>closestPair</strong> von (xP, yP)
  wobei xP P(1) .. P(N) sortiert nach x-Koordinate ist, und
  yP P(1) .. P(N) sortiert nach y-Koordinate (aufsteigend) ist
<strong>wenn</strong> N ≤ 3 <strong>dann</strong>
  <strong>gib zurück</strong> die nächsten Punkte von xP mit dem Brute-Force-Algorithmus
<strong>sonst</strong>
  xL ← Punkte von xP von 1 bis ⌈N/2⌉
  xR ← Punkte von xP von ⌈N/2⌉+1 bis N
  xm ← xP(⌈N/2⌉)<sub>x</sub>
  yL ← { p ∈ yP : p<sub>x</sub> ≤ xm }
  yR ← { p ∈ yP : p<sub>x</sub> > xm }
  (dL, pairL) ← closestPair von (xL, yL)
  (dR, pairR) ← closestPair von (xR, yR)
  (dmin, pairMin) ← (dR, pairR)
  <strong>wenn</strong> dL &#x3C; dR <strong>dann</strong>
    (dmin, pairMin) ← (dL, pairL)
  <strong>endif</strong>
  yS ← { p ∈ yP : |xm - p<sub>x</sub>| &#x3C; dmin }
  nS ← Anzahl der Punkte in yS
  (closest, closestPair) ← (dmin, pairMin)
  <strong>für</strong> i <strong>von</strong> 1 <strong>bis</strong> nS - 1
    k ← i + 1
    <strong>solange</strong> k ≤ nS <strong>und</strong> yS(k)<sub>y</sub> - yS(i)<sub>y</sub> &#x3C; dmin
      <strong>wenn</strong> |yS(k) - yS(i)| &#x3C; closest <strong>dann</strong>
        (closest, closestPair) ← (|yS(k) - yS(i)|, {yS(k), yS(i)})
      <strong>endif</strong>
      k ← k + 1
    <strong>endsolange</strong>
  <strong>endfor</strong>
  <strong>gib zurück</strong> closest, closestPair
<strong>endif</strong>
</pre>

Für die Eingabe erwarten Sie, dass das Argument ein Array von `Point`-Objekten ist, bei denen die `x`- und `y`-Mitglieder auf Zahlen gesetzt sind. Geben Sie ein Objekt zurück, das die Schlüssel-Wert-Paare für `distance` und `pair` enthält (das Paar der zwei nächstgelegenen Punkte).

Zum Beispiel `getClosestPair` mit dem Eingabearray `points`:
...
**Hinweis:** Sortieren Sie das `pair`-Array nach ihren `x`-Werten in aufsteigender Reihenfolge.
