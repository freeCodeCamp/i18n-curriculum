...
이제 CSS Grid로 `.heading` 요소의 내용을 스타일링할 수 있습니다.

CSS `repeat()` 함수는 값을 수동으로 작성하는 대신 반복하는 데 사용되며, 그리드 레이아웃에 유용합니다. 예를 들어, `grid-template-columns` 속성을 `repeat(20, 200px)`로 설정하면 각각 `200px` 너비인 20개의 열이 만들어집니다.

`.heading` 요소에 `grid-template-columns` 속성을 `repeat(2, 1fr)`로 설정하여 너비가 같은 두 개의 열을 만드세요.

# --hints--

`.heading` 선택자에 `grid-template-columns` 속성을 `repeat(2, 1fr)`로 설정하세요.
