...
你還需要在用戶添加商品時更新購物車的總價。 在 `ShoppingCart` 類中創建一個 `calculateTotal` 方法。

在該方法中，聲明一個 `subTotal` 變量，並在 `items` 數組上使用 `reduce` 方法計算數組中每個項目的 `price` 屬性的總和。 使用 `total` 和 `item` 作爲回調的參數。

請記住在 `reduce` 方法中設置初始值。

# --hints--

你應該在 `ShoppingCart` 類中創建一個 `calculateTotal` 方法。

```js
assert.isFunction(cart.calculateTotal);
```

你的 `calculateTotal` 方法應該有一個用 `const` 聲明的 `subTotal` 變量。
...
你的 `calculateTotal` 方法應該在 `items` 數組上使用 `reduce` 方法。
...
你的 `reduce` 回調應使用 `total` 和 `item` 作爲第一個和第二個參數。 記得使用箭頭函數語法。
...
你的 `reduce` 回調應返回 `total` 和 `item.price` 的總和。 使用隱式返回。
