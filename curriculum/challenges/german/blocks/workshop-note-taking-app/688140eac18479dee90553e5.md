...
Da Sie ein weiteres `div`-Element verwenden, müssen Sie dieses für Screenreader besser zugänglich machen.

In einer vorherigen Lektion haben Sie das `aria-live`-Attribut kennengelernt, mit dem eine Live-Region auf einer Seite erstellt wird. Dadurch werden Screenreader-Nutzer automatisch benachrichtigt, wenn sich der Inhalt der Live-Region ändert, ohne dass sie manuell den Fokus darauf setzen oder damit interagieren müssen.

Fügen Sie in Ihrem `#status`-Element ein `aria-live`-Attribut hinzu und setzen Sie dessen Wert auf `"polite"`.

Der `polite`-Wert weist Screenreader an, mit der Ansage der Aktualisierung zu warten, bis eine laufende Ansage beendet ist oder der Nutzer mit der Eingabe aufhört.

# --hints--

Ihr `#status`-Element sollte das `aria-live`-Attribut haben.

```js
assert.isTrue(document.getElementById("status").hasAttribute("aria-live"));
```

Ihr `#status`-Element sollte das `aria-live`-Attribut mit dem Wert `"polite"` haben.
