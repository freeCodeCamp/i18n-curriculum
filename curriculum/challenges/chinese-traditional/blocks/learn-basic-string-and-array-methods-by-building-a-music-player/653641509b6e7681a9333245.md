...
你需要更新目前正在播放的歌曲以及 `playButton` 元素的外觀。

將 `song` 指定到 `userData` 物件上的 `currentSong` 屬性。

*注意*：你不應該在此步驟中使用選用串鏈運算子 `?.`，因為此時 `userData.currentSong` 不會是 `null` 或 `undefined`。

# --hints--

你應該將 `song` 指定到 `userData.currentSong`。
