...
Щоб скинути список відтворення до початкового стану, розгорніть `allSongs` у масив та призначте його `userData.songs`.

*Примітка*: ви не повинні використовувати необов’язковий ланцюжок для `userData.songs`, оскільки пісня не буде `null` або `undefined` на цю мить.
...
Призначте `[...allSongs]` до `userData.songs`.
