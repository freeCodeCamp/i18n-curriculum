...
В межах порожньої функції зворотного виклику `.reduce()` оголосіть змінну `difference` та налаштуйте її значення на `el` мінус `mean`. Потім оголосіть змінну `squared` та налаштуйте її значення на `difference` у `2` степені. Вкінці поверніть значення `acc` плюс `squared`.

# --hints--

Функція зворотного виклику `reduce` повинна мати змінну `difference`.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

Змінна `difference` повинна мати значення `el` мінус `mean`.
...
Функція зворотного виклику `reduce` повинна мати змінну `squared`.
...
Змінна `squared` повинна мати значення `difference` у другому степені.
...
Функція зворотного виклику `reduce` повинна повернути значення `acc` плюс `squared`.
...
Не вилучайте початкове значення `0` з методу `.reduce()`.
