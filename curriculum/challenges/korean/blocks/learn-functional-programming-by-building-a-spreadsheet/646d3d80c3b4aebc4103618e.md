...
같은 패턴을 따라 곱하기 `*`와 나누기 `/`에 적절한 함수를 가진 속성을 추가하세요.

# --hints--

`infixToFunction` 객체는 `*` 속성을 가져야 합니다.
