...
Utiliza el selector existente que apunta a todos los elementos `p` anidados dentro de los elementos con el nombre de clase llamado `item` y establecer su margen superior e inferior a `5px`.
...
Tus elementos `p` anidados en tus elementos `.item` no deben tener un `margin-left`.
...
Tus elementos `p` anidados en tus elementos `.item` no deben tener un `margin-right`.
