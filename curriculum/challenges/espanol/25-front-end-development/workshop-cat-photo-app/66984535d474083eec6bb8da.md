...
Your `figcaption` element should have a closing tag. Las etiquetas de cierre tienen una `/` inmediatamente después del carácter `<`.
...
The `figcaption` element nested in the `figure` element should be below the `img` element. Los tienes en el orden incorrecto.
