...
你需要做的最後一件事是添加 `else if` 語句。

你的條件應該檢查玩家的 `x` 位置是否大於或等於檢查點的 `x` 位置並且小於或等於檢查點的 `x` 位置加上 `40`。

在 `else if` 語句的主體內，你需要調用 `showCheckpointScreen` 函數並傳入字符串 `"You reached a checkpoint!"` 作爲參數。

恭喜！ 你已完成平臺遊戲項目！

# --hints--

你應該添加一個 `else if` 子句來檢查玩家的 `x` 位置是否大於或等於檢查點的 `x` 位置，並且小於或等於檢查點的 `x` 位置加 `40`。
...
你應該調用 `showCheckpointScreen` 函數並傳入 “You reached a checkpoint!” 作爲參數。
