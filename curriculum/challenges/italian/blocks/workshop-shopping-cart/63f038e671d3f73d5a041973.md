...
La prima cosa da fare è verificare se l'array `items` è vuoto. Se lo è, mostra un `alert` all'utente con il testo `Your shopping cart is already empty`, quindi esci dalla funzione.

Ricorda che `0` è un valore falsy, quindi puoi usare l'operatore `!` per controllare se l'array è vuoto.

Dopo aver mostrato l'alert, esci dalla funzione per interrompere l'esecuzione.

# --hints--

Dovresti creare un'istruzione `if` che verifica se l'array `items` è vuoto, usando l'operatore di negazione.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

La tua istruzione `if` dovrebbe mostrare un `alert` all'utente con il testo `Your shopping cart is already empty`.
...
La tua istruzione `if` dovrebbe restituire il controllo dalla funzione.
