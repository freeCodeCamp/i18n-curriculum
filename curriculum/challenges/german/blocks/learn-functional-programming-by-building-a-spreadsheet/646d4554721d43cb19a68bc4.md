...
Der `fn`-Parameter erhält den Namen einer Funktion, wie zum Beispiel `"SUM"`. Aktualisieren Sie `apply`, sodass die Funktion aus Ihrem `spreadsheetFunctions`-Objekt implizit zurückgegeben wird, wobei die `fn`-Variable als Schlüssel für den Objektzugriff verwendet wird.

Denken Sie daran, dass `fn` möglicherweise keinen String in Kleinbuchstaben enthält, daher müssen Sie ihn in einen String mit Kleinbuchstaben umwandeln.

# --hints--

Ihre `apply`-Funktion sollte eine implizite Rückgabe verwenden.
...
Ihre `apply`-Funktion sollte auf das `spreadsheetFunctions`-Objekt zugreifen.
...
Ihre `apply`-Funktion sollte auf die Eigenschaft des `spreadsheetFunctions`-Objekts zugreifen, die dem Wert von `fn` entspricht.
...
Ihre `apply`-Funktion sollte die `.toLowerCase()`-Methode auf `fn` im Eigenschaftszugriff aufrufen.
