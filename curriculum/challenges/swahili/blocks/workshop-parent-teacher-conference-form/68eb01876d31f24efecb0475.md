...
Sasa baada ya muundo wa HTML kukamilika, unapaswa kuendelea na urembo wa CSS.

Fungua faili lako la `styles.css`. Kwanza, weka rangi ya nyuma yenye giza kwa `body` na rangi nyepesi kwa maandishi.

Chagua kipengele cha `body` na weka sifa yake ya `background-color` kuwa `midnightblue` na `color` kuwa `whitesmoke`.

# --hints--

Unapaswa kulenga kipengele cha `body`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body'));
```

Weka sifa ya `background-color` kuwa `midnightblue`.
