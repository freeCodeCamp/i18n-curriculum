...
استهدف العنصر `.penguin`، واضبط `width` و`height` إلى `300px`.

# --hints--

يجب أن تستخدم المحدِّد `.penguin`.

```js
assert.match(code, /\.penguin\s*\{/);
```

يجب أن تعطي `.penguin` `width` بقيمة `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.width, '300px');
```

يجب أن تعطي `.penguin` `height` بقيمة `300px`.
