...
أنشئ عنصر `div` جديدًا أسفل عنصر `header`، وأعطه خاصية `class` بقيمة `divider large`.
...
يجب أن يأتي عنصر `div` الجديد بعد عنصر `header`.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'div');
```

يجب أن يحتوي عنصر `div` الجديد على خاصية `class` بقيمة `divider large`.
