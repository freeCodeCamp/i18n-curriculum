---
id: 5900f4341000cf542c50ff46
title: 'Problema 199: incastro iterativo di cerchi'
challengeType: 1
forumTopicId: 301837
dashedName: problem-199-iterative-circle-packing
---

# --description--

Tre cerchi di raggio uguale sono posizionati all'interno di un cerchio più grande in modo che ogni coppia di cerchi sia tangente tra loro e i cerchi interni non si sovrappongano. Ci sono quattro "spazi" scoperti che devono essere riempiti iterativamente con altri cerchi tangenti.

<img alt="a diagram of non-overlapping circles" src="https://cdn-media-1.freecodecamp.org/project-euler/199-circles-in-circles.gif" style="background-color: white; padding: 10px; display: block; margin-right: auto; margin-left: auto; margin-bottom: 1.2rem;">

Ad ogni iterazione, un cerchio di dimensione massima viene posizionato in ogni spazio, creando nuovi spazi per l'iterazione successiva. Dopo 3 iterazioni (come mostrato), ci sono 108 spazi e la frazione dell'area non coperta dai cerchi è 0.06790342, arrotondata a otto decimali.

Quale frazione dell'area non è coperta dai cerchi dopo `n` iterazioni? Fornisci la tua risposta arrotondata a otto decimali usando il formato x.xxxxxxxx .

# --hints--

`iterativeCirclePacking(10)` dovrebbe restituire un numero.

```js
assert.isNumber(iterativeCirclePacking(10));
```

`iterativeCirclePacking(10)` dovrebbe restituire `0.00396087`.

```js
assert.strictEqual(iterativeCirclePacking(10), 0.00396087);
```

`iterativeCirclePacking(3)` dovrebbe restituire `0.06790342`.

```js
assert.strictEqual(iterativeCirclePacking(3), 0.06790342);
```

# --seed--

## --seed-contents--

```js
function iterativeCirclePacking(n) {

  return true;
}

iterativeCirclePacking(10);
```

# --solutions--

```js
function iterativeCirclePacking(n) {
  let k1 = 1;
  let k0 = k1 * (3 - 2 * Math.sqrt(3));
  let a0 = 1 / (k0 * k0);
  let a1 = 3 / (k1 * k1);
  a1 += 3 * getArea(k0, k1, k1, n);
  a1 += getArea(k1, k1, k1, n);
  let final = ((a0 - a1) / a0).toFixed(8);

  return parseFloat(final);
  function getArea(k1, k2, k3, depth) {
      if (depth == 0) return 0.0;
      let k4 = k1 + k2 + k3 + 2 * Math.sqrt(k1 * k2 + k2 * k3 + k3 * k1);
      let a = 1 / (k4 * k4);
      a += getArea(k1, k2, k4, depth - 1);
      a += getArea(k2, k3, k4, depth - 1);
      a += getArea(k3, k1, k4, depth - 1);
      return a;
  }
}
```
