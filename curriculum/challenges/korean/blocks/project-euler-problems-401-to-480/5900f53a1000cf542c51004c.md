...
모든 비음의 정수 `f(k, n)`에 대해 `k` = $e^\frac{k}{n} - 1$라고 하겠습니다.

놀랍게도, `f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3.1415926… ≈ π입니다.

사실, 이것은 `f(a, 200) + f(b, 200) + f(c, 200) + f(d, 200)` 형태의 π에 대한 최상의 근사값입니다.

오차 $\lvert f(a,n) + f(b,n) + f(c,n) + f(d,n) - \Pi\rvert$를 최소화하는 a, b, c, d에 대해 `almostPi(n)` = a<sup>2</sup> + b<sup>2</sup> + c<sup>2</sup> + d<sup>2</sup>라고 하겠습니다.

`almostPi(200)` = 6<sup>2</sup> + 75<sup>2</sup> + 89<sup>2</sup> + 226<sup>2</sup> = 64658가 주어집니다.
