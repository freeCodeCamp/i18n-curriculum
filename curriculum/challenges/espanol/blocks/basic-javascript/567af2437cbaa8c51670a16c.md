...
Para verificar si una propiedad en un objeto dado existe o no, puedes usar el método `.hasOwnProperty()`. `someObject.hasOwnProperty(someProperty)` devuelve `true` o `false` dependiendo de si la propiedad se encuentra en el objeto o no.
