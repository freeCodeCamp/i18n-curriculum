...
Fügen Sie unter dem gerade hinzugefügten einen weiteren `repeating-linear-gradient` hinzu. Geben Sie ihm eine `90deg`-Richtung, verwenden Sie Ihre Gebäude-Farbe von `0%` bis `12%` und die Fensterfarbe von `12%` bis `44%`. Dadurch entstehen viele rechteckige Fenster.
