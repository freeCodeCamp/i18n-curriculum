...
ラジオボタンがチェックされたときに何が起こるかを定義する時間です。

ラジオボタンが選択されたときだけ内側の円をスタイリングするので、セレクターは `.contact-method-radio-btn:checked::before` にしてください。

ルール内で、`transform` を `translate(3px, 3px) scale(1)` に、`background-color` を `lightgreen` に設定してください。

`transform` は内側の円を少し動かし、`scale(1)` で完全なサイズに拡大し、背景にライトグリーンを設定して円を色で満たし、選択されたオプションであることを示して見えるようにします。

# --hints--

`.contact-method-radio-btn:checked::before` 疑似要素をターゲットにしてください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

`transform` プロパティを `translate(3px, 3px) scale(1)` に設定してください。
...
`background-color` プロパティを `lightgreen` に設定してください。
