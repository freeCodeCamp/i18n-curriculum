...
각 카테고리는 [freeCodeCamp forum](https://forum.freecodecamp.org/)의 해당 카테고리를 가리키는 URL을 갖습니다.

`url`라는 상수를 만들고 템플릿 리터럴을 할당하세요. 그 템플릿 리터럴 안에 `${forumCategoryUrl}${selectedCategory.className}/${id}`의 값을 넣으세요.
...
`url` 변수에 템플릿 리터럴을 할당해야 합니다.

```js
assert.match(code, /const\s+url\s*=\s*`[\s\S]*`/);
```

템플릿 리터럴 안에 `${forumCategoryUrl}${selectedCategory.className}/${id}`의 값이 있어야 합니다.
