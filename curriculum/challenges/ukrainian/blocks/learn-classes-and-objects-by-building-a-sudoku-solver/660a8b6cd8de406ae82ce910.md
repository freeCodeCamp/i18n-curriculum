...
Вкладіть інструкцію `if` всередину поточної інструкції `if`. Для умови `if` використайте рекурсивний виклик `solver()` і поверніть `True` з нового тіла `if`.

# --hints--

Ви повинні вкласти інструкцію `if` всередину поточної інструкції `if` і використати `self.solver()` як умову.
...
Ви повинні повернути `True` з вашої нової інструкції `if`.
