...
احذف خاصية وقيمة `background-color` من `.fb3`، وأضفها إلى `.fb3a` و `.fb3b`.

# --hints--

يجب حذف `background-color` من `.fb3`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

يجب إعطاء `.fb3a` خاصية `background-color` بقيمة `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

يجب إعطاء `.fb3b` خاصية `background-color` بقيمة `--building-color1`.
