...
下記の例の 2 番目の条件は、`a.name > b.name` かどうかをチェックしています。 当てはまる場合、関数は `1` を返します。これは最初の果物を 2 番目の果物の後に並べることを意味します。
...
ではコールバック関数内に `if` 文をもう 1 つ追加して、`a.title` が `b.title` より大きいかどうかを確認してください。 当てはまる場合、数値 `1` を返してください。

# --hints--

コード内に 2 つ目の `if` 文を追加してください。
...
`if` 文では `a.title` が `b.title` より大きいことをチェックしてください。

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
```

`if` 文で、`a.title` が `b.title` より大きい場合は数値の `1` を返してください。
