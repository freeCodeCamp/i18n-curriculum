...
ほとんどのブラウザーは、さまざまな要素に対して独自のデフォルトのCSSプロパティと値を挿入します。よく見ると、`input`のファイルが他のテキスト`input`要素よりも小さいことに気づくかもしれません。デフォルトでは、`padding`要素（入力可能な要素）に`1px 2px`の`input`が与えられています。

別の属性セレクターを使って、`input`に`type`の`file`を設定し、他の`input`要素と同じパディングにしてください。

# --hints--

属性セレクターを使って`input`要素をスタイルしてください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="file"]'));
```

`input[type="file"]`に`padding`の`1px 2px`を与える必要があります。
