...
Crea un loop `for` per iterare su `targets_to_print` e stampare la seguente f-string: `f'\n{start}-{node} distance: {distances[node]}\nPath: {" -> ".join(paths[node])}'`.

# --hints--

Dovresti creare un loop `for` per iterare su `targets_to_print`. Usa `node` come variabile del loop.
...
Dovresti stampare la stringa fornita all’interno del tuo nuovo loop `for`.
