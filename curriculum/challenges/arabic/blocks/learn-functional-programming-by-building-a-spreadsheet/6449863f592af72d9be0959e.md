...
بعد أن يجد `rangeRegex` العلامة `:`, يحتاج إلى البحث عن نفس نمط الحروف والأرقام كما فعل سابقًا.

انسخ مجموعتي الالتقاط الموجودتين لديك والصقهما بعد النقطتين.

# --hints--

يجب أن تضيف مجموعة التقاط ثالثة إلى `rangeRegex` بعد النقطتين.
...
يجب أن تستخدم مجموعة الالتقاط الثالثة فئة أحرف.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[.*\]\)/);
```

يجب أن تطابق مجموعة الالتقاط الثالثة الأحرف من `A` إلى `J`.
...
يجب أن تضيف مجموعة التقاط رابعة إلى `rangeRegex`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(.*\)/);
```

يجب أن تطابق مجموعة الالتقاط الرابعة رقمًا واحدًا أو رقمين.
