...
Unahitaji kuweka mduara wa ndani mahali na kuuficha mwanzoni.

Bado ukilenga `.contact-method-radio-btn::before`, weka `transform` kuwa `translate(3px, 3px) scale(0)`. Kitendakazi cha `translate` kinahamisha mduara kidogo, na `scale(0)` kinakifanya kidogo hadi ukubwa wa sifuri ili kufichwa kwa chaguo-msingi.

Pia, ongeza `transition` ya `all 0.3s ease-in` kuunda uhuishaji laini unapoonekana. Hii inamaanisha mabadiliko yoyote ya mtindo yatahuishwa kwa sekunde 0.3, kuanzia polepole na kisha kuharakisha.

# --hints--

Unapaswa kuendelea kulenga kipengele cha bandia cha `.contact-method-radio-btn::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Weka sifa ya `transform` kuwa `translate(3px, 3px) scale(0)`.
...
Weka sifa ya `transition` kuwa `all 0.3s ease-in`.
