...
次の建物に進みましょう！`--window-color3` に `:root` という新しい変数を作成し、`#d98cb3` の値を設定してください。これはピンクの建物のセカンダリーカラーになります。

# --hints--

新しいプロパティ変数 `--window-color3` を定義してください。

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed("--window-color3"));
```

`--window-color3` に `#d98cb3` の値を設定してください。
