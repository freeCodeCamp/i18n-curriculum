...
لتمكين التنقل داخل الصفحة، أعطِ كل عنصر وسم الرابط `href` يتوافق مع `id` لعناصر `h2`.

# --hints--

يجب أن تعطي أول عنصر `a` خاصية `href` بقيمة `#student-info`.

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

يجب أن تعطي ثاني عنصر `a` خاصية `href` بقيمة `#html-questions`.

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

يجب أن تعطي ثالث عنصر `a` خاصية `href` بقيمة `#css-questions`.
