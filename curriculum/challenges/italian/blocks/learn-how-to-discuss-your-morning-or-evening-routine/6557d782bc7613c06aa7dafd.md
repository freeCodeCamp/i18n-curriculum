...
The word `never` is an adverb used to mean "not at any time" or "not ever". It's often used to talk about things that someone doesn't do or hasn't done. Per esempio:
...
The word `thought` is the past tense of `think`. `Thought of that` means to have considered or come up with an idea in the past. Ad esempio:
