...
Em JSX e TSX, você passa expressões JavaScript como valores de atributo usando chaves. Template literals usam ticks e `${}` para interpolação. Por exemplo:

```tsx
<div className={`prefix-${someVariable}`}>
```

O wrapper do card `className` está atualmente codificado como `card-wrapper tier-gold`. Substitua `gold` por um valor dinâmico usando um template literal que chama `getPlayerTier` com `player.overallRating`. Isso vai referenciar a classe tier correta do CSS, com base na avaliação geral do jogador.

# --hints--

Você deve usar um template literal com `getPlayerTier(player.overallRating)` para o `className` de `.card-wrapper`.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

O elemento `.card-wrapper` deve ter a classe `tier-elite` quando a avaliação do jogador for 98.
