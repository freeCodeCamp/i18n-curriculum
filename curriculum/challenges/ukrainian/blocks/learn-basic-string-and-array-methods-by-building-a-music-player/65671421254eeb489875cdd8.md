...
У функції зворотного виклику використайте метод `removeAttribute()` для видалення атрибута `"aria-current"`. Це видалить атрибут у кожної пісні.

# --hints--

Не слід змінювати існуючий `forEach()` та його вміст.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

Використайте метод `removeAttribute()` на `songEl` і передайте `"aria-current"`.
