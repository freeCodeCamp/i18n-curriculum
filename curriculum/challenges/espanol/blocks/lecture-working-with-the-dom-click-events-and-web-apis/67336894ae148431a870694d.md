...
`querySelector()` es más amplio que `getElementById()`. Con `querySelector()`, puedes obtener el primer elemento en el documento HTML que coincida con el selector CSS pasado como argumento. 

En este ejemplo, obtendrás el elemento que coincide con el selector `section`:
...
Si quieres seleccionar un elemento por su nombre de clase usando `querySelector()`, necesitas anteponer un punto (`.`) al nombre de la clase. Aquí tienes un ejemplo:
...
También tienes otros métodos para coincidir con múltiples elementos, como `getElementsByClassName()` y `querySelectorAll()`. Aprenderás más sobre ellos en lecciones futuras.
