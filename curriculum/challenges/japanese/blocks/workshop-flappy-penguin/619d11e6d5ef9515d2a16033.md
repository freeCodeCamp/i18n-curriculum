...
`.beak` 要素を `class` が `top` のものでターゲットにし、`width` を `20%` に設定し、親要素の上から `60%`、左から `40%` の位置に配置してください。

# --hints--

`.beak.top` セレクターを用意してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.beak.top', '.top.beak']));
```

`.beak.top` に `width` を `--fcc-expected--` に設定するべきですが、現在は `--fcc-actual--` になっています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.top', '.top.beak'])?.width, '20%');
```

`.beak.top` に `top` を `--fcc-expected--` に設定するべきですが、現在は `--fcc-actual--` になっています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.top', '.top.beak'])?.top, '60%');
```

`.beak.top` に `left` を `--fcc-expected--` に設定するべきですが、現在は `--fcc-actual--` になっています。
