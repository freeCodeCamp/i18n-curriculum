...
為 `only screen` 創建第三個 `@media` 查詢，並設定 `max-width` 為 `550px`。在其中，為 `.hero-title` 選擇器創建一個 `font-size` 設為 `6rem`，為 `.hero-subtitle, .author, .quote, .list-title` 選擇器創建一個 `font-size` 設為 `1.8rem`，為 `.social-icons` 選擇器創建一個 `font-size` 設為 `2rem`，以及為 `.text` 選擇器創建一個 `font-size` 設為 `1.6rem`。

# --hints--

你應該有一個新的 `@media` 查詢，針對 `only screen` 且 `max-width` 為 `550px`。這應該放在你之前的兩個之後。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText, 'only screen and (max-width: 550px)');
```

你的新 `@media` 規則應該包含 `.hero-title` 選擇器、`.hero-subtitle, .author, .quote, .list-title` 選擇器、`.social-icons` 選擇器，以及 `.text` 選擇器。這些選擇器應該按照此順序排列。
