...
將你的 `--building-color1` 變數添加為 `.fb4b` 的 `background-color` 屬性值。然後，從 `.fb4` 中移除 `background-color`。

# --hints--

你應該從 `.fb4` 中移除 `background-color`。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb4")?.backgroundColor);
```

你應該給 `.fb4b` 一個 `background-color` 為 `--building-color1`。
