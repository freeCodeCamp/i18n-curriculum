...
Adicione uma tag `<meta>` com o `charset` apropriado e uma tag `<meta>` para responsividade móvel dentro do elemento `head`.
...
Você deve ter uma tag de abertura `<html>` e ela deve ter `lang` com valor `en`.
