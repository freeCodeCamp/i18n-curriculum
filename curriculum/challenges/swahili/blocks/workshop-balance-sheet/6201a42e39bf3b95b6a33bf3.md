...
Tengeneza kichaguzi cha `tr.data th .description` ili kulenga vipengele vyenye `class` iliyowekwa kuwa `description` ambavyo viko ndani ya vipengele vyako vya `th` katika mistari ya jedwali la `.data`. Wape onyesho la block, fanya maandishi yawe ya italic na uzito wa kawaida, na waweke kwa `padding` iliyowekwa kuwa `1rem 0 0.75rem` pamoja na ukingo wa kulia wa `-13.5rem`.
