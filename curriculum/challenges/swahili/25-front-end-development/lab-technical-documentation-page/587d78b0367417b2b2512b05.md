...
1. Ndani ya kipengele cha `#main-doc`, unaweza kuona vipengele kadhaa vya `section`, kila kimoja kikiwa na class ya`main-section`. There should be a minimum of five.
...
1. Kila kipengele cha `section` chenye class ya `main-section` lazima pia kiwe na `id` ambayo inalingana na maandishi ya kila `header` zilizomo ndani yake. Any spaces should be replaced with underscores (e.g. The section that contains the header "JavaScript and Java" should have a corresponding `id="JavaScript_and_Java"`).
...
1. Zaidi ya hayo, navbar inapaswa kuwa na viungo (`a`) vipengele vilivyo na class ya `nav-link`. There should be one for every element with the class `main-section`.
