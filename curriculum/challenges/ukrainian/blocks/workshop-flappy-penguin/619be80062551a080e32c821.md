...
Зробіть округлення гребеня, задавши псевдоелементу радіус округлення нижніх кутів `100%`, залишивши верхні кути з радіусом `0%`.

# --hints--

Ви повинні використати властивість `border-radius` для округлення гребеня.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

Ви повинні задати `.penguin-body::before` властивість `border-radius` зі значенням `0% 0% 100% 100%`.
