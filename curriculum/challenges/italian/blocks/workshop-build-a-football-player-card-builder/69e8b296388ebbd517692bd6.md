...
Il modulo è ora completo. Per fare in modo che la scheda di anteprima si aggiorni mentre digiti, devi collegare gli input allo stato `player`.

Partendo dal `input` Name, aggiungi un attributo `value` con valore `player.name`. Poi aggiungi un attributo `onChange` impostato su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `name` impostato su `e.target.value`. Non dimenticare di mettere sia lo spread sia l'impostazione del valore `name` dentro un oggetto.

# --hints--

Il tuo input name `value` dovrebbe essere collegato al campo `name` dello stato player.
...
Il tuo input name dovrebbe avere un gestore `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Il gestore `onChange` dovrebbe chiamare `setPlayer` con `setPlayer` che ha un oggetto che espande `player` e imposta `name` su `e.target.value`.
