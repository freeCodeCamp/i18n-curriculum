...
- 経過時間が `60` 分未満の場合、文字列 `"xm ago"` を返す。 `x` は分数を表す。
- 経過時間が `24` 時間未満の場合、文字列 `"xh ago"` を返す。 `x` は時間数を表す。
- それ以外の場合は、文字列 `"xd ago"` を返す。 `x` は日数を表す。
...
経過時間が `30` 分の場合、文字列 `"30m ago"` を返してください。
...
経過時間が `1` 時間の場合、文字列 `"1h ago"` を返してください。
...
経過日数が `1` 日の場合、文字列 `"1d ago"` を返してください。
