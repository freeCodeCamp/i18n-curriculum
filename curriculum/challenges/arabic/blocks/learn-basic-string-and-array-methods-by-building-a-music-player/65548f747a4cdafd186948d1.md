...
تحتاج إلى الحصول على `id` للأغنية التي تُشغل حاليًا. لهذا، يمكنك استخدام `userData?.currentSong?.id`.

استخدم `getElementById()` للحصول على `id` للأغنية التي تُشغل حاليًا، ثم استخدم قوالب حرفية لإضافة بادئة `song-`. عيّنها إلى الثابت `songToHighlight`.

# --hints--

لا يجب تعديل الدالة `highlightCurrentSong` الحالية أو محتواها.
...
يجب استخدام `document.getElementById()` وتمرير `` `song-${userData?.currentSong?.id}` ``.

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

يجب تعيين نتيجة `getElementById()` إلى ثابت باسم `songToHighlight`.
