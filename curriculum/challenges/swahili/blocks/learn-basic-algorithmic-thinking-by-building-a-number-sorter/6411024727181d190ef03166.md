...
Baada ya mzunguko wako uliopangwa ndani wa `for`, umebaini thamani ndogo kabisa. Unahitaji kubadilisha nafasi yake na thamani yako ya sasa.

Kama ulivyofanya katika bubble sort yako, tumia kigezo cha `temp` kunasa thamani iliyoko `array[i]`, kisha badilisha nafasi za thamani zilizoko `array[i]` na `array[minIndex]`.

# --hints--

Baada ya mzunguko wako uliopangwa ndani wa `for`, unapaswa kutangaza kigezo cha `temp`.
...
Unapaswa kuweka thamani ya `array[i]` kwa `temp`.
...
Unapaswa kuweka thamani ya `array[minIndex]` kwa `array[i]`.
...
Unapaswa kuweka thamani ya `temp` kwa `array[minIndex]`.
