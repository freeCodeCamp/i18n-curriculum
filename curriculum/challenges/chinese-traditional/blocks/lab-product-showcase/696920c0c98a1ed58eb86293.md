...
1. 你應該創建一個名為 `Book` 的介面，該介面繼承自 `Item`。其 `type` 屬性應設為字面值 `"book"`。此外，還應有型別為 `title` 和 `author` 的 `string` 屬性。

1. 你應該創建一個名為 `Electronics` 的介面，該介面繼承自 `Item`。其 `type` 屬性應設為字面值 `"electronics"`。此外，還應有型別為 `item` 和 `model` 的 `string` 屬性，以及一個選用的型別為 `warranty` 的 `number` 屬性。

1. 你應該創建一個名為 `Clothing` 的介面，該介面繼承自 `Item`。其 `type` 屬性應設為字面值 `"clothing"`。此外，還應有型別為 `item` 和 `brand` 的 `string` 屬性，以及一個選用的 `size` 屬性，其值只能是 `"S"`、`"M"` 或 `"L"`。
