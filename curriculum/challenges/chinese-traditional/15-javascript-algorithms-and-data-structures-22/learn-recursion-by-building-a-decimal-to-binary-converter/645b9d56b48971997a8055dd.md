...
在 `if` 語句的主體內，將 `result` 的 `innerText` 屬性設置爲等於字符串 `"0"`。 然後，使用早期的 `return` 語句儘早退出該函數。

# --hints--

你應該將 `result` 的 `innerText` 屬性設置爲等於字符串 `"0"`。

```js
assert.match(String(decimalToBinary), /if\s*\(\s*input\s*===?\s*0\s*\)\s*\{\s*result\.innerText\s*=\s*('|"|`)\s*0\s*\1/);
```

在設置 `result` 元素的 `innerText` 之後，你應該使用早期的 `return` 語句來儘早退出該函數。
