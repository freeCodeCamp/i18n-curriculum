...
이제 오른쪽 수염을 제자리에 옮기는 작업을 하세요. 클래스 선택자를 사용해 `.cat-whisker-right-top` 요소를 선택하고 `position` 값을 가진 `absolute` 속성, `top` 값을 가진 `120px` 속성, `left` 값을 가진 `109px` 속성을 부여하세요.

# --hints--

`.cat-whisker-right-top` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top'));
```

`.cat-whisker-right-top` 선택자에는 `position` 값으로 설정된 `absolute` 속성이 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.position, 'absolute');
```

`.cat-whisker-right-top` 선택자에는 `top` 값으로 설정된 `120px` 속성이 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.top, '120px');
```

`.cat-whisker-right-top` 선택자에는 `left` 값으로 설정된 `109px` 속성이 있어야 합니다.
