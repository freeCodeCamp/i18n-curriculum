...
Njia ya `.remove()` huondoa kutoka kwenye orodha kipengele cha kwanza kinacholingana kinachotumwa kama hoja:
...
Maliza mzunguko wa `while` kwa kuondoa nodi ya sasa kutoka kwenye orodha ya `unvisited`. Zingatia uelekezaji wa mstari.

# --hints--

Unapaswa kutumia njia ya `.remove()` kuondoa nodi ya sasa kutoka `unvisited` baada ya mzunguko wako wa `for`.
