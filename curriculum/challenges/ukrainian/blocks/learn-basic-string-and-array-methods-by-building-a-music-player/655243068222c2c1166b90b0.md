...
Усередині функції `playNextSong` створіть інструкцію `if`, щоб перевірити, чи `currentSong` об’єкта `userData` строго дорівнює `null`. Це перевірить, чи наразі в об’єкті `userData` не відтворюється жодна пісня.

Якщо умова істинна, викличте функцію `playSong` з аргументом `id` першої пісні в масиві `userData?.songs`.

# --hints--

Ви не повинні змінювати існуючу функцію `playNextSong` та її вміст.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Ви маєте створити інструкцію `if` з умовою `userData?.currentSong === null`.
...
Ви маєте викликати функцію `playSong` з `userData?.songs[0].id` всередині інструкції `if`.
