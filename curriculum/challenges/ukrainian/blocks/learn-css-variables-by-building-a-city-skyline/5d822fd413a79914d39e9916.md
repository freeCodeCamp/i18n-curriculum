...
Не хвилюйтеся через пробіл унизу, усе буде зсунуте вниз пізніше, коли ви додасте висоту елементу вгорі будівлі.

Додайте `repeating-linear-gradient` до `.fb1c` під кутом `90deg`, ваш `--building-color4` від `0%` до `10%` і `transparent` від `10%` до `15%`.

# --hints--

Ви повинні надати `.fb1c` `background` з `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.background, "repeating-linear-gradient");
```

Використовуйте напрям `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Використовуйте перший колір `--building-color4` від `0%` до `10%`.
...
Використовуйте другий колір `transparent` від `10%` до `15%`.
