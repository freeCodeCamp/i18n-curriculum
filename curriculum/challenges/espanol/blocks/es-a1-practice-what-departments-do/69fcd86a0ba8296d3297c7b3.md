...
Escucha y completa cada oración con el departamento correcto.
...
`El departamento de aprendizaje y desarrollo capacita al equipo.` — El departamento de aprendizaje y desarrollo capacita al equipo.

Tanto `apoyar` (apoyar) como `capacitar` (capacitar) son verbos `-ar` en tercera persona del singular.

Siguiendo el mismo patrón que `desarrolla`, `verifica` y `crea` de tareas anteriores.
