...
Fügen Sie jedem `span` zwei `className`-Elemente mit dem `stat-value` von `stat-label` beziehungsweise `div.stat-row` hinzu.

# --hints--

Jedes `.stat-row` sollte ein `span` mit `className` von `stat-value` und ein `span` mit `className` von `stat-label` enthalten.
