...
Коли користувач редагує нотатку і натискає Tab або клікає поза межами картки, має з’явитися повідомлення про статус, яке сповіщає, що нотатку успішно збережено.

Для початку створіть елемент `div` з атрибутом `id`, встановленим у `"status"`.

# --hints--

У вас має бути другий елемент `div`.

```js
assert.lengthOf(document.querySelectorAll("div"), 2);
```

Ваш другий елемент `div` має мати атрибут `id`.

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

Ваш другий елемент `div` має мати атрибут `id`, встановлений у `"status"`.
