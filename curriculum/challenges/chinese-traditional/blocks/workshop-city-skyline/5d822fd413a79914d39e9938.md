...
為 `sky` 類別添加 `radial-gradient`。使用 `#ffcf33` 從 `0%` 到 `20%`，`#ffff66` 在 `21%`，以及 `#bbeeff` 在 `100%`。這將為你的背景添加圓形漸層，這將是你的太陽。

# --hints--

你應該在 `.sky` 的 `background` 屬性中給予 `radial-gradient`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".sky")?.background, "radial-gradient");
```

你應該給 `radial-gradient` 一個第一個顏色為 `#ffcf33`。

```js
assert.match(new __helpers.CSSHelp(document).getStyle(".sky")?.background, /radial-gradient\(rgb\(255, 207, 51\)( 0%)?, rgb\(255, 207, 51\) 20%/);
```

你應該在 `21%` 處給 `radial-gradient` 第二個顏色 `#ffff66`。
...
你應該在 `radial-gradient` 的 `100%` 處給予第三個顏色 `#bbeeff`。
