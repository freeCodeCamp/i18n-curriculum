...
1. You should have a function outside the `Category` class named `create_spend_chart(categories)` that returns a bar-chart string. To build the chart:
   ...
   - Calcula porcentajes solo a partir de los retiros y no de los depósitos. El porcentaje debe ser el porcentaje del monto gastado para cada categoría respecto al total gastado en todas las categorías (redondeado al múltiplo de 10 más cercano).
