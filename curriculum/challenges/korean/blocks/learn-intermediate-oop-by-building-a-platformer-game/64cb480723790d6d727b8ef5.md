...
`else if` 문장 안에 `forEach` 반복문을 추가해 `checkpoints` 배열을 순차적으로 반복하세요. 콜백 함수의 매개변수 이름으로 `checkpoint`를 사용하세요.

`forEach` 콜백 안에서 덧셈 대입 연산자를 사용해 checkpoints의 `x` 위치에 5를 더하세요.

# --hints--

`forEach` 배열을 순차적으로 반복하는 `checkpoints` 반복문이 있어야 합니다.
...
`checkpoint` 콜백 함수 안에서 `forEach` 매개변수를 사용해야 합니다.
...
덧셈 대입 연산자를 사용해 `checkpoint`의 `x` 위치에 5를 더해야 합니다.
