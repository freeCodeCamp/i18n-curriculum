...
스도쿠(Su Doku, 일본어로 *숫자 놓기*라는 뜻)는 인기 있는 퍼즐 개념의 이름입니다. 그 기원은 명확하지 않지만, 레온하르트 오일러가 라틴 스퀘어라는 훨씬 더 어려운 퍼즐 아이디어를 발명한 공로를 인정해야 합니다. 스도쿠 퍼즐의 목표는 9x9 격자에서 빈칸(또는 0)을 각 행, 열, 그리고 3x3 박스마다 1부터 9까지의 숫자가 각각 한 번씩 들어가도록 채우는 것입니다. 아래는 일반적인 시작 퍼즐 격자와 그 해답 격자의 예입니다.
...
잘 만들어진 스도쿠 퍼즐은 유일한 해답을 가지며 논리적으로 풀 수 있지만, 선택지를 제거하기 위해 "추측과 검증" 방법을 써야 할 수도 있습니다(이에 대해 많은 의견이 분분합니다). 탐색의 복잡도가 퍼즐 난이도를 결정하며, 위 예시는 직접적인 추론으로 풀 수 있어 쉬운 편으로 간주됩니다.

`puzzlesArr` 배열에는 난이도가 다양한 스도쿠 퍼즐 문자열이 들어 있으며, 모두 유일한 해답을 가집니다.

`puzzlesArr`에 있는 모든 퍼즐을 풀어 각 해답 격자의 왼쪽 상단에 있는 3자리 숫자의 합을 구하세요. 예를 들어 위 해답 격자의 왼쪽 상단에 있는 3자리 숫자는 483입니다.
...
`suDoku(testPuzzles1)`는 숫자를 반환해야 합니다.

```js
assert(typeof suDoku(_testPuzzles1) === 'number');
```

`suDoku(testPuzzles1)`는 `1190`를 반환해야 합니다.

```js
assert.strictEqual(suDoku(_testPuzzles1), 1190);
```

`suDoku(testPuzzles2)`는 `24702`를 반환해야 합니다.
