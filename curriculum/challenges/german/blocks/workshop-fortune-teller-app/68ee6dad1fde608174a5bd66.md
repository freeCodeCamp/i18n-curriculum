...
Rufen Sie innerhalb der `singleCardSelected`-Methode das `singleCard`-Element ab und aktualisieren Sie dessen `innerHTML`, indem Sie die `renderCard`-Funktion aufrufen. Übergeben Sie `"Click the card and reveal the fortune"` als erstes Argument und `isReversed` als zweites Argument.

# --hints--

Sie sollten `this.elements.singleCard.innerHTML` aktualisieren, indem Sie die `renderCard`-Funktion aufrufen.
...
Ihr `renderCard`-Aufruf sollte `"Click the card and reveal the fortune"` als erstes Argument haben.
...
Sie sollten `isReversed` als zweites Argument an `renderCard` übergeben.
