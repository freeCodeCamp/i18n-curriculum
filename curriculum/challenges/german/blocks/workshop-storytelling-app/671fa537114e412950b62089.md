...
Wählen Sie in Ihrer JavaScript-Datei das `.story-container`-Element aus und speichern Sie es in einer Variablen namens `storyContainer`.

# --hints--

Sie sollten eine `storyContainer`-Variable haben.

```js
assert.isDefined(storyContainer);
```

Sie sollten das `.story-container`-Element mit der `querySelector`-Methode auswählen und es in der `storyContainer`-Variablen speichern.
