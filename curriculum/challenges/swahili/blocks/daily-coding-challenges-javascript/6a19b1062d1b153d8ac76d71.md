...
title: "Zoezi la 323: Kipata Hisia za Wimbo"
challengeType: 28
...
Ukichukua mfuatano wa herufi wa aina ya muziki na nambari ya BPM kwa wimbo, amua hisia kwa kutumia jedwali lifuatalo:

| Hisia | Aina ya muziki | Mzunguko wa BPM |
|------|-------|-----------|
...
`getMood("rock", 111)` inapaswa kurudisha `"happy"`.

```js
assert.equal(getMood("rock", 111), "happy");
```

`getMood("electronic", 74)` inapaswa kurudisha `"focus"`.

```js
assert.equal(getMood("electronic", 74), "focus");
```

`getMood("classical", 180)` inapaswa kurudisha `"happy"`.

```js
assert.equal(getMood("classical", 180), "happy");
```

`getMood("rock", 155)` inapaswa kurudisha `"hype"`.

```js
assert.equal(getMood("rock", 155), "hype");
```

`getMood("electronic", 90)` inapaswa kurudisha `"happy"`.

```js
assert.equal(getMood("electronic", 90), "happy");
```

`getMood("classical", 67)` inapaswa kurudisha `"focus"`.

```js
assert.equal(getMood("classical", 67), "focus");
```

`getMood("pop", 100)` inapaswa kurudisha `"happy"`.

```js
assert.equal(getMood("pop", 100), "happy");
```

`getMood("electronic", 135)` inapaswa kurudisha `"hype"`.
