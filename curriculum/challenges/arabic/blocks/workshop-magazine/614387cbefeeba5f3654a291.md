...
يجب أن يحتوي عنصر `div` على `class` معين إلى `author`.
...
يجب أن يكون العنصران الجديدان `p` داخل عنصر `div` الخاص بك.

```js
assert.exists(document.querySelector('div')?.querySelectorAll('p')?.length === 2);
```

يجب أن يحتوي أول عنصر `p` جديد على `class` معين إلى `author-name`.
...
يجب أن يحتوي ثاني عنصر `p` جديد على `class` معين إلى `publish-date`.
