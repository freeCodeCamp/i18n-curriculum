...
title: trovare tutti i caratteri non numerici
challengeType: 1
forumTopicId: 301347
...
L'ultima sfida ha mostrato come cercare le cifre usando la scorciatoia `\d` con un `d` minuscolo. Puoi anche cercare i caratteri non numerici usando una scorciatoia simile che invece usa un `D` maiuscolo.

La scorciatoia per cercare i caratteri non numerici è `\D`. Questo equivale alla classe di caratteri `[^0-9]`, che cerca un singolo carattere che non sia un numero da zero a nove.

# --instructions--

Usa la classe di caratteri abbreviata per i non numerici `\D` per contare quanti caratteri non numerici ci sono nei titoli dei film.

# --hints--

La tua espressione regolare dovrebbe usare la scorciatoia per i caratteri non numerici
