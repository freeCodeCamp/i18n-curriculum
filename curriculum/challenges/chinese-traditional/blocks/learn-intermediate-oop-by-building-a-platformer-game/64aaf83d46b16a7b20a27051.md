...
在你的 `startGame` 函式中，你需要顯示 `canvas` 元素並隱藏 `startScreen` 容器。

使用 `canvas.style.display` 將顯示值更改為 `"block"`。 

在那下面，使用 `startScreen.style.display` 將顯示值改為 `"none"`。

# --hints--

你應該使用點記法來存取 `canvas` 的 `style` 屬性。
...
你應該將 `canvas` 的 `display` 屬性設為 `"block"`。

```js
assert.match(startGame.toString(), /canvas\.style\.display\s*=\s*('|")block\1/);
```

你應該使用點記法來存取 `startScreen` 的 `style` 屬性。
...
你應該將 `startScreen` 的 `display` 屬性設為 `"none"`。
