...
目前，如果你在 `input` 或 `textarea` 元素中輸入文字，你將無法看到文字。此外，它們的高度太小，不易使用。

透過將 `color` 設定為 `#ffffff`，並將它們的 `min-height` 設定為 `2em` 來修正此問題。

# --hints--

你應該將 `color` 設定為 `#ffffff`。
