...
創建 `.calories-info span` 選擇器，將其 `font-size` 設定為 `2.4em`，並將 `font-weight` 設定為 `700`。

# --hints--

你應該創建 `.calories-info span` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span'));
```

你的 `.calories-info span` 選擇器應該有一個 `font-size` 屬性設定為 `2.4em`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span')?.fontSize === '2.4em');
```

你的 `.calories-info span` 選擇器應該有一個 `font-weight` 屬性設定為 `700`。
