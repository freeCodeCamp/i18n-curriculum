...
Якщо `value` з `numberInput` дорівнює `5`, викличте функцію `showAnimation()`, а потім `return` рано.

# --hints--

Викличте функцію `showAnimation()` в тілі другої інструкції `if`.

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

Після виклику функції `showAnimation()` використайте ключове слово `return`, щоб завчасно вийти з функції `checkUserInput()`.
