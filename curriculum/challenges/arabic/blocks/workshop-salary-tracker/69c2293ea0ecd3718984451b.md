...
الآن بعد إنشاء المُعيّن `salary`، حدّث `__init__` لاستدعائه بدلًا من التعيين المباشر إلى `self._salary`. غيّر `self._salary = Employee._base_salaries[level]` إلى `self.salary = Employee._base_salaries[level]`.

# --hints--

يجب أن تستخدم طريقة `__init__` الخاصة بك `self.salary = Employee._base_salaries[level]` بدلًا من `self._salary = Employee._base_salaries[level]`.
