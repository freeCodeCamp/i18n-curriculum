...
在第二個 `div.answer` 元素中嵌套一個 `textarea` 元素，並設置它的行數和列數。

然後，給 `textarea` 添加描述示例答案的佔位符文本。

# --hints--

你應該在第二個 `div.answer` 元素中嵌套一個 `textarea` 元素。

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

你應該給 `textarea` 一個帶有數字的 `rows` 屬性。
...
您應該給 `textarea` 添加一個 `placeholder` 屬性，其中包含描述示例答案的文本。
