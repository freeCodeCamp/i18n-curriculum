...
次に、`medications` 辞書に `constraints` キーを追加してください。その値には `isinstance` を使って、`medications` がリストであることを確認してください。

# --hints--

あなたの `constraints` 辞書には `medications` キーがあるはずです。
...
あなたの `medications` 辞書の `constraints` キーは `isinstance(medications, list)` の値を持つべきです。
