...
Ora devi recuperare la canzone successiva nella playlist. Per farlo, devi ottenere l'indice della canzone corrente e poi aggiungere `1` a esso.

Crea una costante chiamata `nextSong` e assegnale `userData?.songs[currentSongIndex + 1]`.

Infine, chiama la funzione `playSong` e passa `nextSong.id` come argomento.

# --hints--

Non dovresti modificare l'istruzione `if` esistente, il suo blocco `else` e il contenuto.
...
Dovresti assegnare `userData?.songs[currentSongIndex + 1]` a una costante `nextSong`.
...
Dovresti chiamare la funzione `playSong` con `nextSong.id`.
