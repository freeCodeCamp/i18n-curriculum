...
Для цього потрібно встановити наступні властивості CSS:
...
Надайте `.sr-only` атрибут `overflow` зі значенням `hidden`.
...
Надайте `.sr-only` атрибут `white-space` зі значенням `nowrap`.
