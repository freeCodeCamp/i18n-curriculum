...
Unda kichaguzi cha `.author` na kipa sifa ya `font-size` iliyowekwa kuwa `2rem` na sifa ya `font-family` iliyowekwa kuwa `Raleway` ikiwa na njia mbadala ya `sans-serif`.
...
Kichaguzi chako cha `.author` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `2rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize, '2rem');
```

Kichaguzi chako cha `.author` kinapaswa kuwa na sifa ya `font-family` iliyowekwa kuwa `Raleway` ikiwa na njia mbadala ya `sans-serif`.
