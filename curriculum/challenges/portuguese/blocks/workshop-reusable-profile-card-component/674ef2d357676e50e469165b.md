...
Dentro da sua declaração de retorno, substitua a string vazia por um elemento `div` com uma propriedade `className` de `flex-container`.

# --hints--

Seu componente `App` não deve renderizar uma string vazia.
...
Seu componente `App` deve retornar um único elemento `div`.
...
Seu `div` deve ter uma propriedade `className` com o valor `flex-container`.
