...
이 조건이 충족되면, 버튼의 `disabled` 속성을 `true`로 설정하여 비활성화하세요. 또한, 버튼의 `textContent`를 `"No more data to load"`로 설정하세요.
...
`disabled`의 `loadMoreBtn` 속성을 `true`로 설정해야 합니다.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

`textContent`의 `loadMoreBtn`를 `"No more data to load"`로 설정해야 합니다.
