...
在你的 `.image-wrapper` 元素中，為第二個 `img` 元素設定 `src` 為 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`、`alt` 為 `image of a calculator project`、`loading` 屬性設為 `lazy`、`class` 設為 `image-2`、`width` 屬性設為 400，以及 `height` 屬性設為 400。

# --hints--

你的第二個 `img` 元素應該將 `src` 設定為 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`。
