...
Finalmente, a acessibilidade de navegação pode ser melhorada fornecendo atalhos de teclado.

O atributo `accesskey` aceita uma lista de teclas de acesso separadas por espaços. Por exemplo:

```html
<button type="submit" accesskey="s">Submit</button>
```

Dê a cada um dos links de navegação uma tecla de acesso de uma única letra.

_Observação: nem sempre é aconselhável usar teclas de acesso, mas elas podem ser úteis_

Bom trabalho. Você concluiu o projeto prático _Teste de acessibilidade_.

# --hints--

Você deve dar ao primeiro elemento `a` uma `accesskey` de uma única letra.

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('accesskey')?.length, 1);
```

Você deve dar ao segundo elemento `a` uma `accesskey` de uma única letra.

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('accesskey')?.length, 1);
```

Você deve dar ao terceiro elemento `a` uma `accesskey` de uma única letra.
