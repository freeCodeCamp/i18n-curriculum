...
创建一个名为 `highlightCurrentSong` 的函数。在函数内部，查看是否存在带有 `aria-current="true"` 的 `.playlist-song` 元素。如果存在，使用 `removeAttribute` 方法从该元素中移除 `aria-current` 属性。

# --hints--

你应该有一个名为 `highlightCurrentSong` 的函数。

```js
assert.isFunction(highlightCurrentSong)
```

你的 `highlightCurrentSong` 函数应使用 `removeAttribute` 方法从具有 `aria-current="true"` 的 `.playlist-song` 元素中移除 `aria-current` 属性（如果该元素存在）。
