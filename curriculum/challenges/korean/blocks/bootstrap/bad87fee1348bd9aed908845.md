...
제출 `fa-paper-plane` 요소 안에 `<i class="fa fa-paper-plane"></i>`를 추가하여 `button` Font Awesome 아이콘을 넣을 수 있습니다.

폼의 텍스트 입력 필드에 `form-control` 클래스를 지정하세요. 폼의 제출 버튼에는 `btn btn-primary` 클래스를 지정하세요. 또한 이 버튼에 `fa-paper-plane` Font Awesome 아이콘을 넣으세요.

클래스가 `<input>`인 모든 텍스트 `<textarea>`, `<select>`, `.form-control` 요소는 너비가 100%입니다.

# --hints--

폼의 제출 버튼에는 `btn btn-primary` 클래스를 지정해야 합니다.
...
제출 `<i class="fa fa-paper-plane"></i>` 요소 안에 `button`를 추가해야 합니다.
