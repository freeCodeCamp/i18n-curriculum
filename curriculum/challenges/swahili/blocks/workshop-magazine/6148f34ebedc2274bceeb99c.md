...
Sasa baada ya mpangilio wa jarida kukamilika, unahitaji kuufanya ujibadilishe kulingana na kifaa.

Anza na hoja ya utafutaji ya `@media` kwa `only screen` yenye `max-width` ya `720px`. Ndani yake, tengeneza kichaguzi cha `.image-wrapper` na kipa sifa ya `grid-template-columns` yenye thamani ya `1fr`.

Hii itapunguza picha tatu kuwa safu moja kwenye skrini ndogo.

# --hints--

Unapaswa kuwa na sheria mpya ya `@media` kwa `only screen and (max-width: 720px)`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText, 'only screen and (max-width: 720px)');
```

Sheria yako mpya ya `@media` inapaswa kuwa na kichaguzi cha `.image-wrapper`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText, '.image-wrapper');
```

Kichaguzi chako kipya cha `.image-wrapper` kinapaswa kuwa na sifa ya `grid-template-columns` yenye thamani ya `1fr`.
