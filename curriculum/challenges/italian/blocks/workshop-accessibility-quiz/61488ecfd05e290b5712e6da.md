...
Per far sembrare la prima sezione più in linea, prendi di mira solo gli elementi `input` all'interno degli elementi `.info`, e imposta il loro `width` su `50%`, allineando il testo a sinistra.

# --hints--

Dovresti usare il selettore `.info input` oppure `.info > input`.
...
Dovresti dare agli elementi `input` un `width` di `50%`.
...
Dovresti dare agli elementi `input` un `text-align` di `left`.
