...
`bb4-window` 클래스에 `width`의 `18%`을, `height`의 `90%`을 주고, `--window-color4` 변수를 `background-color`로 추가하세요.

# --hints--

`.bb4-window`에 `width`의 `18%`을 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

`.bb4-window`에 `height`의 `90%`을 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

`.bb4-window`에 `background-color`의 `--window-color4`을 주어야 합니다.
