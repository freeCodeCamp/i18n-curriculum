...
次に、変数 `lunchCalories` を宣言し、`lunchNumberInputs` を引数として `getCaloriesFromInputs` を呼び出した結果を代入してください。

# --hints--

`calculateCalories` 関数内に変数 `lunchCalories` が必要です。

```js
assert.match(calculateCalories.toString(), /lunchCalories\s*=/);
```

`lunchNumberInputs` を引数として `getCaloriesFromInputs` を呼び出してください。

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*lunchNumberInputs\s*\)/);
```

`getCaloriesFromInputs` の結果を `lunchCalories` に割り当ててください。
