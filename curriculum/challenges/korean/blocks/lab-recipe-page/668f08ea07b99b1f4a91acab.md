...
1. 레시피 이름이 들어간 `title` 요소와 `charset` 속성이 `UTF-8`로 설정된 `meta` 요소를 포함하는 `head` 요소가 있어야 합니다.
1. `body` 요소가 있어야 합니다.
1. 레시피 이름이 들어간 `h1` 요소가 하나만 있어야 합니다.
1. `h1` 아래에 레시피를 소개하는 `p` 요소가 있어야 합니다.
1. 재료 섹션을 위한 `Ingredients` 텍스트가 들어간 `h2` 요소가 하나 있어야 합니다.
1. 첫 번째 `h2` 요소 아래에 재료를 나열하는 최소 네 개의 항목(`li` 요소)을 포함한 순서 없는 목록(`ul` 요소)이 있어야 합니다.
1. 지침 섹션을 위한 `Instructions` 텍스트가 들어간 두 번째 `h2` 요소가 있어야 합니다.
1. 두 번째 `h2` 아래에 레시피 단계를 순서대로 나열하는 최소 네 개의 항목을 포함한 순서 있는 목록(`ol` 요소)이 있어야 합니다.
1. 유효한 이미지가 설정된 `src` 속성과 이미지를 설명하는 `alt` 속성을 가진 `img` 요소가 하나 있어야 합니다. 원한다면 `https://cdn.freecodecamp.org/curriculum/labs/recipe.jpg`를 사용할 수 있습니다.
...
`html` 요소에는 닫는 태그가 있어야 합니다. 닫는 태그는 `<` 문자 바로 뒤에 `/`가 옵니다.
...
`body` 요소에는 닫는 태그가 있어야 합니다. 닫는 태그는 `<` 문자 바로 뒤에 `/`가 옵니다.

```js
assert.match(code, /<\/\s*body\s*>/i);
```

레시피 이름이 들어간 `h1` 요소가 하나만 있어야 합니다.
