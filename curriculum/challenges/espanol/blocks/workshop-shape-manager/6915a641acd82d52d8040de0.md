...
Volviendo a la función `initializeApp`, aquí es donde usarás los tipos en línea y la función `getElement()` que creaste antes para obtener los elementos del DOM necesarios para la aplicación.

Dentro de la función `initializeApp`, reasigna `shapeTypeSelect` llamando a `getElement()` con `"shape-type"`, luego usa la palabra clave `as` para convertir el resultado a `HTMLSelectElement`.

# --hints--

Debes reasignar `shapeTypeSelect` con el valor apropiado.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

Debes llamar a la función `getElement` con `shape-type` y asignarla a `shapeTypeSelect`.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

Debes llamar a `getElement` con `HTMLSelectElement` como el tipo de `shapeTypeSelect` usando la palabra clave `as`.
