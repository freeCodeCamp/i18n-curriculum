...
`getNextSong` という名前の関数を作成してください。現在の曲がプレイリストの最後の曲でない場合は、`getCurrentSongIndex()` の `userData.songs` に 1 を足した位置にある曲を返し、最後の曲の場合は `undefined` を返すようにしてください。

# --hints--

`getNextSong` という名前の関数があるはずです。

```js
assert.isFunction(getNextSong);
```

`getNextSong` 関数は `getCurrentSongIndex` を呼び出すべきです。
...
`getNextSong` 関数は現在の曲の次の曲を返すべきです。
...
`getNextSong` 関数は現在の曲がプレイリストの最後の曲の場合、`undefined` を返すべきです。
