...
Щоб переглянути результати даних, зареєструйте змінну `data` на консолі в межах блоку `try`.

Під визначенням `fetchData` викличте функцію та відкрийте консоль, щоб побачити результати.

# --hints--

Ви повинні використати `console.log` для реєстрації змінної `data` на консолі.

```js
assert.match(code, /\s*console\.log\(\s*data\s*\)/);
```

`console.log` має бути всередині блоку `try`.

```js
assert.match(code, /\s*try\s*{[\s\S]*\s*console\.log\(\s*data\s*\)[\s\S]*}\s*catch\s*\(\s*err\s*\)\s*{\s*}/);
```

Викличте функцію `fetchData()` після визначення функції `fetchData()`.
