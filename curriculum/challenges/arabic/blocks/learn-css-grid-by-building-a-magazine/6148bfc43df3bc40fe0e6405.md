...
أنشئ محدِّد `.publish-date` وأعطه خاصية `color` بقيمة `rgba(255, 255, 255, 0.5)`.

# --hints--

يجب أن يكون لديك محدِّد `.publish-date`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

يجب أن يحتوي محدِّد `.publish-date` على خاصية `color` مضبوطة على `rgba(255, 255, 255, 0.5)`.
