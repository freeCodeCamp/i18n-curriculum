...
选择 `.penguin-head` 元素，使其 `width` 为父元素的一半，`height` 为 `45%`。 然后，将其 `background` 设置为沿 `45deg` 从 `gray` 到 `rgb(239, 240, 228)` 的线性渐变。

# --hints--

你应该使用 `.penguin-head` 选择器。

```js
assert.match(code, /\.penguin-head\s*\{/);
```

你应该将 `.penguin-head` 的 `width` 设置为 `50%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

你应该将 `.penguin-head` 的 `height` 设置为 `45%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

你应该将 `.penguin-head` 的 `background` 设置为 `linear-gradient(45deg, gray, rgb(239, 240, 228))`。
