...
localStorage 機能を最終的に動作させるために、状態の初期化を `defaultPlayer` から `loadPlayer` に変更してください。これにより、利用可能な保存データで状態が開始されます。

次に、localStorage 機能をテストしてください。入力した内容は、プレビューが更新されたりページをリロードした後も保持されるはずです。

また、画像の `src` のリンクを `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` に変更すると、画像がリアルタイムで変わるのを確認できます。ただし、プレイヤー名は Maradona に変更してください。

これで、サッカー選手カードビルダーのプロジェクトは完成です！

# --hints--

`useState` は `loadPlayer` ではなく `defaultPlayer` で初期化する必要があります。
