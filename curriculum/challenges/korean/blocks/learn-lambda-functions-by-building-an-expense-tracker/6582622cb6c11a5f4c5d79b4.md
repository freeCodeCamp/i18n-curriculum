...
여전히 `elif` 절 안에서 `expenses_from_category` 반복자를 `print_expenses` 호출에 전달하세요.

# --hints--

`print_expenses()` 절에서 `expenses_from_category`를 인수로 전달하여 `elif`를 호출해야 합니다.
...
`print_expenses()` 호출은 `expenses_from_category` 변수 뒤에 와야 합니다.
