...
Create a new variable called `--window-color4` in `:root` and give it a value of `#8cb3d9`. 这将是最后一个背景建筑的间色。

# --hints--

你应该定义一个新的属性变量 `--window-color4`。

```js
assert(new __helpers.CSSHelp(document).isPropertyUsed("--window-color4"));
```

你应该将 `--window-color4` 的值设置为 `#8cb3d9`。
