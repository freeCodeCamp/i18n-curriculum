...
Dentro do array `allSongs`, crie um objeto com as seguintes propriedades e valores:
...
Seu array `allSongs` deve ter um valor.

```js
assert.equal(allSongs.length, 1);
```

Seu array `allSongs` deve ter um objeto como seu primeiro valor.

```js
assert.isObject(allSongs[0]);
```

Seu array `allSongs` deve ter um objeto com uma propriedade `id` definida para o número `0`.

```js
assert.equal(allSongs[0].id, 0);
```

Seu array `allSongs` deve ter um objeto com uma propriedade `title` definida para a string `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Seu array `allSongs` deve ter um objeto com uma propriedade `artist` definida para a string `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Seu array `allSongs` deve ter um objeto com uma propriedade `duration` definida para a string `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Seu array `allSongs` deve ter um objeto com uma propriedade `src` definida para a string `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
