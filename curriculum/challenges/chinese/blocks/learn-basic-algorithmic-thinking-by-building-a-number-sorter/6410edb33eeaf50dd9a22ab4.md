...
当你的 `if` 条件为真时，你需要交换这两个元素，将较大的元素 "冒泡" 到数组的末尾。

为此，声明一个 `temp` 变量并为其赋值 `array[j]` 的值。 然后将 `array[j]` 的值赋给 `array[j + 1]`。 最后，将 `array[j + 1]` 的值赋给 `temp`。

# --hints--

在你的 `if` 语句中，你应该声明一个 `temp` 变量。
