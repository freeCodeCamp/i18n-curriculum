...
將具有 `class` 為 `left` 的 `.arm` 元素標的，並將其從父元素的頂部定位在 `35%`，從左側定位在 `5%`。然後，將具有 `class` 為 `right` 的 `.arm` 元素標的，並將其從父元素的頂部定位在 `0%`，從右側定位在 `-5%`。
