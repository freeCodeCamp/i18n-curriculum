...
`timeAgo` 関数では、現在の時刻とトピックの最後のアクティビティの時刻との差を計算したいです。これにより、トピックで最後にアクティビティがあってからどれくらい時間が経過したかを表示できます。

以下の要件を満たす `timeAgo` 関数を完成させてください。

- 経過した分数が `60` 未満の場合、文字列 `"xm ago"` を返してください。`x` は分数を表します。
- 経過した時間が `24` 未満の場合、文字列 `"xh ago"` を返してください。`x` は時間を表します。
- それ以外の場合は、文字列 `"xd ago"` を返してください。`x` は日数を表します。

時間差を計算するのに役立ついくつかの式は以下の通りです。
...
経過した分数が `"30m ago"` 分の場合、文字列 `30` を返す必要があります。
...
経過した時間が `"1h ago"` 時間の場合、文字列 `1` を返す必要があります。
...
経過した日数が `"1d ago"` 日の場合、文字列 `1` を返す必要があります。
