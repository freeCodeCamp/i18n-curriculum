...
Nach deiner verschachtelten `for`-Schleife hast du den kleinsten Wert gefunden. Du musst ihn mit deinem aktuellen Wert austauschen.

Verwende wie bei deiner Bubble-Sortierung eine `temp`-Variable, um den Wert bei `array[i]`, zu extrahieren, und vertausche dann die Werte bei `array[i]` und `array[minIndex]`.

# --hints--

Nach deiner verschachtelten `for`Schleife solltest du eine `temp`-Variable deklarieren.
...
Du solltest dem `temp` `array[i]` zuweisen.
...
Du solltest dem `array[i]` `array[minIndex]` zuweisen.
...
Du solltest dem `array[minIndex]` `temp` zuweisen.
