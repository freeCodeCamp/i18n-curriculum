...
Debes tener un selector `table`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table'));
```

Tu selector `table` debe tener una propiedad `border-collapse` establecida en `collapse`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('border-collapse') === 'collapse');
```

Tu selector `table` debe tener una propiedad `border` establecida en `0`.
