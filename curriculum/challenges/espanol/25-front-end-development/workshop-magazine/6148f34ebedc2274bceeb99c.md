...
Ahora que el diseño de la revista está terminado, necesitas hacerlo adaptable.

Empieza con una consulta `@media` para `only screen` con un `max-width` de `720px`. Dentro, crea un selector `.image-wrapper` y asígnale una propiedad `grid-template-columns` de `1fr`.

Esto colapsará las tres imágenes en una sola columna en pantallas más pequeñas.

# --hints--

Deberías tener una nueva regla `@media` para `only screen and (max-width: 720px)`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText, 'only screen and (max-width: 720px)');
```

Tu nueva regla `@media` debería tener un selector `.image-wrapper`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText, '.image-wrapper');
```

Tu nuevo selector `.image-wrapper` debería tener una propiedad `grid-template-columns` de `1fr`.
