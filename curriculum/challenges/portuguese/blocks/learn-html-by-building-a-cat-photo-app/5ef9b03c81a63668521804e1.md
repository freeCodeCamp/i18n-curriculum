...
O elemento `fieldset` é usado para agrupar entradas e rótulos relacionados juntos em um formulário web. Os elementos `fieldset` são <dfn>elementos de nível de bloco</dfn>, o que significa que eles aparecem em uma nova linha.
...
O elemento `fieldset` deve ter uma tag de abertura. As tags de abertura possuem a seguinte sintaxe: `<elementName>`.

```js
assert(document.querySelector('fieldset'));
```

Seu elemento `fieldset` deve ter uma tag de fechamento. As tags de fechamento têm uma `/` logo após o caractere `<`.
