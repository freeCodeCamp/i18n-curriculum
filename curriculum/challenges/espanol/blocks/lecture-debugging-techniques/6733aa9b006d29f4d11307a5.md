...
Cada elemento del arreglo debe estar separado por una coma, de lo contrario resultará en un mensaje de error.
...
En este ejemplo, intentamos mostrar la variable `price` en la consola pero no ha sido definida. Esto resultará en un `ReferenceError`.
...
El ejemplo anterior resultará en un error `Cannot access 'b' before initialization`.
...
Este ejemplo resultará en un error `developerObj.map is not a function` porque el método `map` se usa para arreglos y no para objetos.

El último error común que veremos es el `RangeError`.

Un `RangeError` ocurre cuando tu código intenta usar un valor que está fuera del rango que JavaScript puede manejar. Aquí hay un ejemplo de asignar un valor inválido a la longitud de un arreglo:
