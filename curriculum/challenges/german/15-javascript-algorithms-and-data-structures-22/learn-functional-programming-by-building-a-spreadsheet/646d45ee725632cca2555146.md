...
Jetzt muss deine `applyFunction` ein Ergebnis zurückgeben. Gib das Ergebnis der `.replace()`-Methode für `str2` zurück. Übergib deinen `functionCall`-Regex und einen leeren Callback.

# --hints--

Deine `applyFunction`-Funktion sollte das Ergebnis des Aufrufs der `.replace()`-Methode für `str2` zurückgeben.
...
Du solltest deinem `.replace()`-Aufruf `functionCall` als erstes Argument übergeben.
...
Du solltest deinem `.replace()`-Aufruf eine leere Arrow-Funktion als zweites Argument übergeben.
