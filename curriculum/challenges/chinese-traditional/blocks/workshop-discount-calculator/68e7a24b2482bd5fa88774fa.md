...
你的程式碼中不應該有 `discount = PercentageDiscount(10)` 這一行。
...
你不應該在你的程式碼中有 `print(discount.apply_discount(product))` 這一行。
...
你的程式碼中不應該有 `fixed_discount = FixedAmountDiscount(5)` 這一行。
...
你的程式碼中不應該有 `print(fixed_discount.apply_discount(product))` 這一行。
