...
تسمح لك الطريقة `.extend()` بإضافة عناصر من قابل للتكرار إلى نهاية قائمة:
...
أنشئ بند `else` واستخدم الطريقة `.extend()` لإضافة مسار العقدة الحالية إلى مسار العقدة المجاورة.

# --hints--

يجب أن تنشئ بند `else` بعد جملة `if` المسكنة.
...
يجب أن تحتوي بند `else` على `paths[node].extend(paths[current])`.
