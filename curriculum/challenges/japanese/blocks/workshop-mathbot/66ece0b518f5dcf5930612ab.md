...
ワークショップの最後の部分では、数値の集合の最小値と最大値を取得する方法をレビューします。

まず、コンソールにメッセージ `console.log()` を記録する `"The Math.max() and Math.min() methods are used to get the maximum and minimum number from a range."` を追加してください。

# --hints--

コンソールにメッセージ `"The Math.max() and Math.min() methods are used to get the maximum and minimum number from a range."` を記録する必要があります。
