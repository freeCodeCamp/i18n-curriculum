...
Vamos chamar um inteiro positivo $k$ de square-pivot, se existir um par de inteiros $m > 0$ e $n ≥ k$, tal que a soma dos ($m + 1$) quadrados consecutivos até $k$ seja igual à soma dos $m$ quadrados consecutivos a partir de ($n + 1$):
