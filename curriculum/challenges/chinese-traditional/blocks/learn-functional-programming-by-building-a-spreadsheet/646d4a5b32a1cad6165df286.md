...
創建一個 `random` 屬性。此屬性應該使用陣列中的前兩個數字來產生一個隨機整數。此數字的範圍從這兩個數字中的第一個（包含）開始，到這兩個數字的總和之前結束。使用 `Math.floor()` 和 `Math.random()` 方法進行計算。

# --hints--

你的 `spreadsheetFunctions` 物件應該有一個 `random` 屬性。

```js
assert.property(spreadsheetFunctions, "random");
```

你的 `random` 屬性應該是一個函式（程式）。

```js
assert.isFunction(spreadsheetFunctions.random);
```

你的 `random` 函式（程式）應該傳回陣列中前兩個數字之間的隨機數。
