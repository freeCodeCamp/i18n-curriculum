...
Gegeben sind zwei Mengen *A* und *B*. Berechnen Sie $(A \\setminus B) \\cup (B \\setminus A).$ Das heißt, listen Sie die Elemente auf, die in *A* oder *B*, aber nicht in beiden enthalten sind. Diese Menge wird als symmetrische Differenz von *A* und *B* bezeichnet. Anders ausgedrückt: $(A \\cup B) \\setminus (A \\cap B)$ (die Menge der Elemente, die in mindestens einer der Mengen *A* oder *B* enthalten sind, abzüglich der Elemente, die in beiden Mengen *A* und *B* enthalten sind).
...
Schreiben Sie eine Funktion, die zwei Arrays als Parameter entgegennimmt und die symmetrische Differenz zurückgibt. Sortieren Sie das Ergebnis-Array vor der Rückgabe.
