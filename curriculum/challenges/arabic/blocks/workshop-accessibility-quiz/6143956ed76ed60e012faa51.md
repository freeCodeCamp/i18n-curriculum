...
داخل العنصر الأول `span`، أضف النص `Question`.

في العنصر الثاني `span`، أضف النص `Question`.

# --hints--

يجب أن تعطي العنصر الأول `span` النص `Question`.

```js
assert.equal(document.querySelector('h3 > span')?.textContent.trim(), 'Question');
```

يجب أن تعطي العنصر الثاني `span` النص `Question`.
