...
新しい関数の中で、選択されたカテゴリに応じて楽器をフィルターする必要があります。

`instrumentsArr` からアイテムをフィルターし、`instrumentCategory` と同じカテゴリの楽器オブジェクトを含む配列を返すように関数を作成してください。もし `instrumentCategory` が `all` と等しい場合は、`instrumentsArr` 配列全体を返してください。

次に、イベントリスナーのコールバックから `console.log` を削除し、ドロップダウンメニューから選択されたオプションを引数にして `instrumentCards` を呼び出した結果をログに出力して、異なるカテゴリオプションを選択して関数をテストできるようにしてください。

# --hints--

`instrumentCards("all")` は `instrumentsArr` を返すべきです。

```js
assert.sameDeepMembers(instrumentCards("all"), instrumentsArr);
```

`instrumentCards("woodwinds")` は `[{ category: "woodwinds", instrument: "Flute", price: 500 }, { category: "woodwinds", instrument: "Clarinet", price: 200 }, { category: "woodwinds", instrument: "Oboe", price: 4000 }]` を返すべきです。
...
`instrumentCards("brass")` は `[{ category: "brass", instrument: "Trumpet", price: 200 }, { category: "brass", instrument: "Trombone", price: 300 }, { category: "brass", instrument: "French Horn", price: 4300 }]` を返すべきです。
...
`instrumentCards("percussion")` は `[{ category: "percussion", instrument: "Drum Set", price: 500 }, { category: "percussion", instrument: "Xylophone", price: 3000 }, { category: "percussion", instrument: "Cymbals", price: 200 }, { category: "percussion", instrument: "Marimba", price: 3000 }]` を返すべきです。
...
ドロップダウンメニューで選択されたカテゴリを変更すると、`instrumentCards(selectContainer.value)` の結果がコンソールにログ出力されるはずです。
