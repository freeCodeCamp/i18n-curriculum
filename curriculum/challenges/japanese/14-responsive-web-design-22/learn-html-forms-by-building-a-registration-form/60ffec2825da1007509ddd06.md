...
属性セレクターをもう 1 つ使用して、`type` が `file` の `input` 要素のパディングを、他の `input` 要素と同じにしてください。

# --hints--

属性セレクターを使用して `input` 要素のスタイルを指定する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="file"]'));
```

`input[type="file"]` の `padding` を `1px 2px` に設定する必要があります。
