...
你将会<a href="https://colab.research.google.com/github/freeCodeCamp/boilerplate-book-recommendation-engine/blob/master/fcc_book_recommendation_knn.ipynb" target="_blank" rel="noopener noreferrer nofollow">使用 Google Colaboratory 进行此项目的工作</a>。
...
在这个项目中，你将使用 Book-Crossings 数据集，该数据集包含 90,000 名用户对 270,000 本书的 110 万条评分（评分范围为 1-10）。该数据集已在笔记本中导入，因此无需额外下载。

使用来自 `sklearn.neighbors` 的 `NearestNeighbors` 开发一个模型，显示与给定书籍相似的书籍。最近邻算法通过测量距离来确定实例的“接近度”。
