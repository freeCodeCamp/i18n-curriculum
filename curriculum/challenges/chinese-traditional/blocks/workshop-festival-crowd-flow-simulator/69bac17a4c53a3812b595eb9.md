...
在開始模擬之前，你需要一個物件來追蹤每個閘門在所有時脈中處理的參加者數量。呼叫 `initializeThroughput()` 並傳入 `gates`，將結果指定給名為 `throughputSummary` 的變數。

# --hints--

你應該呼叫 `initializeThroughput()` 並傳入 `gates`，將結果指定給名為 `throughputSummary` 的變數。
...
你的 `simulateFestival` 函式應該呼叫 `initializeThroughput()` 函式並傳入 `gates`。
