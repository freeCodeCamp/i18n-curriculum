...
Verwende die Arrow-Syntax, um eine `addOrUpdateTask`-Funktion zu erstellen. Then move the `dataArrIndex` variable, the `taskObj` object, and the `if` statement into the `addOrUpdateTask` function.

# --hints--

Du solltest `const` und Arrow-Syntax vewenden, um eine `addOrUpdateTask`-Funktion zu erstellen.

```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Du solltest die `dataArrIndex`-Variable in die `addOrUpdateTask`-Funktion verschieben.
...
Du solltest das `taskObj`-Objekt in die `addOrUpdateTask`-Funktion verschieben.
