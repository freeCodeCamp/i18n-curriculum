...
Ndani ya kipengele chako cha `aside`, tengeneza vipengele viwili vya `img`, kipengele kimoja cha `blockquote`, na kipengele cha tatu cha `img`. Toa kipengele cha `blockquote` sifa ya `class` iliyowekwa kuwa `image-quote`.

# --hints--

Unapaswa kutengeneza vipengele vitatu vya `img` ndani ya kipengele chako cha `aside`.

```js
assert.lengthOf(document.querySelectorAll('aside img'), 3);
```

Unapaswa kutengeneza kipengele cha `blockquote` ndani ya kipengele chako cha `aside`.

```js
assert.exists(document.querySelector('aside blockquote'));
```

Kipengele chako cha `blockquote` kinapaswa kuwa na sifa ya `class` iliyowekwa kuwa `image-quote`.
