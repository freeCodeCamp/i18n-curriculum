...
سيطابق الـ `regex` الذي سترسله إلى دالتك `infixEval` رقمين مع عامل بينهما. سيتم تعيين الرقم الأول إلى `arg1` في الاستدعاء الراجع، والثاني إلى `arg2`، والعامل إلى `operator`.

اجعل دالة الاستدعاء الراجع تُرجع ضمنيًا الخاصية `operator` من كائن `infixToFunction`. تذكر أن `operator` هو متغير يحمل اسم الخاصية، وليس اسم الخاصية الفعلي.

# --hints--

يجب أن يستخدم الاستدعاء الراجع إرجاعًا ضمنيًا.
...
يجب أن تصل دالة الاستدعاء الراجع إلى كائن `infixToFunction`.
...
يجب أن تستخدم دالة الاستدعاء الراجع ترميز الأقواس للوصول إلى الخاصية في كائن `infixToFunction` التي تطابق قيمة معلمة `operator`.
