...
1. Você deve ter uma função `getBookmarks` que retorna o array `bookmarks` armazenado no armazenamento local. Se a chave `bookmarks` ainda não foi configurada, a função `getBookmarks` deve retornar um array vazio.
1. A chave `bookmarks` armazenada no armazenamento local deve ser um array de objetos favoritos. Cada objeto favorito deve ter três chaves: `name`, `category` e `url`.
...
1. Ao clicar em `#add-bookmark-button-form`, você deve atualizar a chave `bookmarks` armazenada no armazenamento local adicionando um objeto favorito ao final do array. O objeto deve ter `name` definido como o valor da entrada `#name`, `category` definido como o valor da opção selecionada no menu suspenso de categorias e `url` definido como o valor da entrada `#url`.
...
1. Se um ou mais favoritos no armazenamento local tiverem a categoria selecionada, deve adicionar um botão de rádio com atributos `id` e `valor` definidos para o nome do favorito no HTML interno de `#category-list` para cada um desses favoritos.
...
1. Ao clicar em `#close-list-button`, você deve executar a função para ocultar a `#bookmark-list-section` e exibir a seção principal.
1. Ao clicar em `#delete-bookmark-button`, você deve excluir o favorito correspondente ao botão de opção selecionado do armazenamento local e atualizar a lista de favoritos exibida.
...
Ao clicar em `#view-category-button`, você deve modificar o HTML interno de `#category-list`, adicionando um botão de rádio com os atributos `id` e `value` definidos como o nome do favorito para cada favorito na categoria selecionada.
...
Ao clicar em `#delete-bookmark-button`, você deve excluir o favorito correspondente ao botão de opção selecionado do armazenamento local e atualizar a lista de favoritos exibida.
