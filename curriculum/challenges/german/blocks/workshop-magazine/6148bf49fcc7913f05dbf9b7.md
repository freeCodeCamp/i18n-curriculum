...
Erstellen Sie einen `.author-name a:hover`-Selektor und geben Sie ihm eine `background-color`-Eigenschaft mit dem Wert `#306203`.

Dadurch wird ein Hover-Effekt nur für das `a`-Element innerhalb des `.author-name` erzeugt, der den ursprünglichen freeCodeCamp-Grünton im Hintergrund anzeigt.

# --hints--

Sie sollten einen `.author-name a:hover`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

Ihr `.author-name a:hover`-Selektor sollte eine `background-color`-Eigenschaft mit dem Wert `#306203` haben.
