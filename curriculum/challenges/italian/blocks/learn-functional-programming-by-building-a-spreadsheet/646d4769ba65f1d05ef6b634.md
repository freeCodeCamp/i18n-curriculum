...
Ora la tua funzione `update()` può effettivamente valutare formule. Ricorda che hai scritto la condizione `if` per verificare che una funzione sia stata chiamata.

All'interno della tua istruzione `if`, imposta la proprietà `value` di `element` come risultato della tua funzione `evalFormula()`. Non passare ancora argomenti.

# --hints--

Dovresti aggiornare la proprietà `value` di `element` nel tuo blocco `if`.
...
Dovresti assegnare alla proprietà `value` il risultato della chiamata della tua funzione `evalFormula()`.
...
Non dovresti passare argomenti alla tua chiamata `evalFormula()`.
