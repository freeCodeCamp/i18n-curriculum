...
يجب عليك إعطاء عنصر `:root` خاصية `--penguin-picorna`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-picorna'));
```

يجب أن تكون خاصية `--penguin-picorna` بقيمة `orange`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-picorna', true), 'orange');
```

يجب أن يحتوي منتقي `.beak` على خاصية `background-color` بقيمة `var(--penguin-picorna)`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.getPropVal('background-color', true), 'var(--penguin-picorna)');
```

يجب أن يحتوي منتقي `.foot` على خاصية `background-color` بقيمة `var(--penguin-picorna)`, ولكن وجدت بقيمة `--fcc-actual--`.
