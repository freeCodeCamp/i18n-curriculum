...
Antes de tu ciclo `for`, crea una estructura `if` que agregue la cadena `=== MOVIES ===` (terminando con un carácter de nueva línea) al valor actual de `result` cuando la lista `movies` no esté vacía.

# --hints--

Debes tener una estructura `if` en tu método `__str__` que verifique si la lista `movies` no está vacía.
