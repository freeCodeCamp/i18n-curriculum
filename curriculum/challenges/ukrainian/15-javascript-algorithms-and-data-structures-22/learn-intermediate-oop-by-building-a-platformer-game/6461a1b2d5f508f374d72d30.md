...
Зберігайте їх у `const` змінних під назвою `startScreen` та `checkpointScreen` відповідно.
...
Отримайте елемент із `class` типу `start-screen`. Не забудьте включити `.` всередину методу `document.querySelector`.
...
Використайте метод `document.querySelector()`, щоб отримати елемент `.checkpoint-screen`.
