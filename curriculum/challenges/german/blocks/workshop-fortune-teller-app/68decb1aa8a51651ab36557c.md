...
Innerhalb der `getElement`-Funktion verwenden Sie `querySelector` mit den Generics und speichern das Ergebnis in einer Variablen. Fügen Sie eine `if`-Anweisung hinzu, um zu prüfen, ob das Element nicht existiert, und werfen Sie in diesem Fall einen Fehler mit der Nachricht `Element not found: ${error message here}`. Wenn das Element existiert, geben Sie es zurück.

# --hints--

Sie sollten eine Variable namens `el` innerhalb der `getElement`-Funktion deklarieren.
...
Ihrer `el`-Variablen sollte der Wert `document.querySelector<T>(selector)` zugewiesen werden.
...
Sie sollten eine if-Anweisung haben, die prüft, ob `el` nicht gefunden wurde.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

Sie sollten innerhalb Ihrer `Element not found: ${selector}`-Anweisung einen neuen Error mit der Nachricht `if` werfen.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

Sie sollten die `el`-Variable innerhalb der getElement-Funktion zurückgeben.
