...
`.author` セレクターを作成し、`font-size` プロパティを `2rem` に、`font-family` プロパティを `Raleway` に設定し、フォールバックフォントとして `sans-serif` を指定してください。
...
`.author` セレクターの `font-size` プロパティを `2rem` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

`.author` セレクターの `font-family` プロパティには `Raleway` と、フォールバックとして `sans-serif` を設定する必要があります。
