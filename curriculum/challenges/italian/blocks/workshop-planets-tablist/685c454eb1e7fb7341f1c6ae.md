...
Ogni `tab` rappresenta uno dei pulsanti nella lista delle schede. L’utente dovrebbe poter cliccare su una scheda per mostrare il pannello corrispondente e nascondere gli altri.

Nei moduli precedenti, hai imparato come allegare un listener per l’evento `"click"` a ogni pulsante in un `NodeList` così:
...
Usa il metodo `addEventListener` per allegare un listener per l’evento `"click"` a ogni pulsante `tab`. Il secondo argomento del metodo `addEventListener` dovrebbe essere per ora una funzione freccia vuota `() => {}`.

# --hints--

Dovresti avere un `tab.addEventListener('click', () => {})` dentro il loop `forEach`.
