...
Los pasos restantes para el taller implicarán agregar el código para mostrar dinámicamente el mensaje de notas guardadas y registrar las notas actuales en la consola.

Comienza creando una variable llamada `noteEl` y asignándole el resultado de consultar el documento para el elemento con el `id` de `note`.

Luego crea otra variable llamada `statusEl` y asígnale el resultado de consultar el documento para el elemento con el `id` de `status`.

# --hints--

Debes tener una variable `noteEl` y asignarle el resultado de consultar el documento por el elemento con el `id` de `note`.

```js
assert.equal(noteEl, document.getElementById("note"));
``` 

Debes tener una variable `statusEl` y asignarle el resultado de consultar el documento para el elemento con el `id` de `status`.
