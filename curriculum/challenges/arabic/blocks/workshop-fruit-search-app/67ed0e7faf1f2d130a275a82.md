...
عندما يكتب المستخدم في حقل الإدخال، يجب أن تظهر قائمة بالفواكه التي تطابق استعلامه.

للتعامل مع هذا، أنشئ متغير حالة اسمه `results` بقيمة ابتدائية مصفوفة فارغة (`[]`). عرّف أيضًا دالة التعيين الخاصة به، `setResults`، باستخدام الـ `useState` Hook.

# --hints--

يجب أن تستخدم صيغة التفكيك للمصفوفة للمتغيرين `results` و `setResults`.

```js
assert.match(code, /(const|let|var)\s+\[\s*(results)\s*,\s*(setResults)\s*\]\s*/);
```

يجب أن تستخدم الـ `useState` Hook.
...
يجب أن يكون للـ `useState` Hook قيمة ابتدائية مصفوفة فارغة (`[]`).
