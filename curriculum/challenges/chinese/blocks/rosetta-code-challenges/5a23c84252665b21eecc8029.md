...
使用**跨棋盘**方法实现加密和解密消息的函数。

每个函数将接受两个参数：一个 `message` 字符串和一个 `alphabet` 数组。 `alphabet` 数组将包含 3 个字符串，代表跨行棋盘（straddling checkerboard）的 3 行。

`straddle()` 函数的输出应是一串十进制数字。数字应通过在每个数字前插入转义字符，然后包含该数字<em>未加密</em>来加密。
