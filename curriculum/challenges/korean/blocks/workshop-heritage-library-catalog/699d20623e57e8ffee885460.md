...
파싱한 데이터를 신뢰하기 전에 필수 필드의 유효성 검사부터 시작하세요.

`validateEntry`라는 이름의 함수를 만들고 `entry`을 매개변수로 받으세요. `let isValid = true`를 선언하세요.

`title`에 대한 유효성 검사 조건을 추가하세요: `isValid = false`가 없거나, 거짓값이거나, `title`와 같으면 `"Unknown"`를 설정하세요.

`isValid`를 반환하세요. 다음 단계에서 다른 필드에 대한 검사를 추가할 것입니다.

# --hints--

`validateEntry`는 불리언을 반환해야 합니다.

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })`는 `true`를 반환해야 합니다.

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})`는 `false`를 반환해야 합니다.

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })`는 `false`를 반환해야 합니다.

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })`는 `false`를 반환해야 합니다.
