...
화살표 함수가 단순한 표현식을 반환한다면, `return` 키워드와 중괄호 `{}`를 생략할 수 있습니다. 이를 암묵적 반환이라고 합니다.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

화살표 함수의 함수 본문에 여러 줄의 코드가 있다면, `return` 키워드와 중괄호 `{}`를 사용해야 합니다.
...
`addTwoNumbers` 함수를 리팩터링하거나 업데이트하여 `return` 키워드와 중괄호 `{}`를 제거하세요. 대신 `addTwoNumbers` 함수는 암묵적 반환을 사용해야 합니다.

콘솔을 열어 여전히 올바른 출력이 나오는지 확인하세요.

# --hints--

리팩터링한 `addTwoNumbers` 함수는 암묵적 반환을 사용해야 합니다.
