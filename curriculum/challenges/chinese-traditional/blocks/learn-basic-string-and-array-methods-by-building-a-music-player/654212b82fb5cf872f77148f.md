...
在 `button` 元素內，再創建兩個 `span` 元素。

第一個 `span` 元素應該具有 `playlist-song-artist` 的 `class`。 在 `span` 標籤之間添加 `${song.artist}`。

第二個 `span` 元素應該具有 `playlist-song-duration` 的 `class`。 在 `span` 標籤之間添加 `${song.duration}`。

# --hints--

你不應修改現有的 `button` 元素及其內容。
...
你應該創建一個帶有類 `playlist-song-artist` 的 `span` 元素。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

你的具有 `playlist-song-artist` 類的 `span` 元素應該插入 `song.artist` 作爲其內容。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

你應該創建一個帶有類 `playlist-song-duration` 的 `span` 元素。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

你的具有 `playlist-song-duration` 類的 `span` 元素應該插入 `song.duration` 作爲其內容。
