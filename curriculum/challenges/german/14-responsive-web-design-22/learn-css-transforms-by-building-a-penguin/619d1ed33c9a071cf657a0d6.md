...
Fun Fact: Pinguine können nicht stehen, wenn sie keine zwei Füße haben.

Füge innerhalb des `.penguin-body`-Elements zwei `div`-Elemente mit jeweils einer `class` von `foot` hinzu. Gib dem ersten `.foot` eine `class` von `left` und den zweiten `.foot` eine `class` von `right`.
...
Du solltest dem ersten neuen `div` eine `class` von `foot` geben.

```js
assert.exists(document.querySelector('.penguin-body > div.foot'));
```

Du solltest dem zweiten neuen `div` eine `class` von `foot` geben.
...
Du solltest `.foot.right` nach `.foot.left` positionieren.
