...
Використайте `const` та стрілковий синтаксис, щоб створити порожню функцію під назвою `shuffle`.

Ця функція відповідає за перемішування пісень у списку відтворення та виконує необхідні оновлення керування станом після перемішування.

# --hints--

Використайте `const`, щоб створити порожню функцію під назвою `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

`shuffle` має бути функцією.

```js
assert.isFunction(shuffle)
```

Функція `shuffle` має використовувати стрілковий синтаксис та не приймати параметр.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

Функція `shuffle` має бути порожньою.
