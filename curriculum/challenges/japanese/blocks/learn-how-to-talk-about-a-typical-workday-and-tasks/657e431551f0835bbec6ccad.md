---
id: 657e431551f0835bbec6ccad
title: タスク60
challengeType: 19
dashedName: task-60
lang: en-US
---

<!-- (audio) Jake: I can't forget to inspect the alarm systems. They have to be in top condition in case of emergencies. -->

# --description--

音声を聞いて、以下の質問に答えてください。

# --questions--

## --text--

なぜジェイクはアラームシステムの点検を忘れてはいけないと言っているのでしょうか？

## --answers--

なぜなら、それらはめったに故障しないからです。

### --feedback--

フォーカスは故障の頻度ではなく、緊急時の準備にあります。

---

緊急時に備えて最高の状態にしておくためです。

---

なぜなら、点検を楽しんでいるからです。

### --feedback--

楽しみのために点検しているわけではありません。

---

時間を早く過ごすためです。

### --feedback--

理由は安全に関することであり、時間を過ごすことではありません。

## --video-solution--

2

# --scene--

```json
{
  "setup": {
    "background": "company1-reception.png",
    "characters": [
      {
        "character": "Jake",
        "position": {"x":50,"y":0,"z":1.4},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "2.1-3.mp3",
      "startTime": 1,
      "startTimestamp": 17.24,
      "finishTimestamp": 23.18
    }
  },
  "commands": [
    {
      "character": "Jake",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Jake",
      "startTime": 1,
      "finishTime": 6.94,
      "dialogue": {
        "text": "I can't forget to inspect the alarm systems. They have to be in top condition in case of emergencies.",
        "align": "center"
      }
    },
    {
      "character": "Jake",
      "opacity": 0,
      "startTime": 7.44
    }
  ]
}
```
