...
Deklarieren Sie eine `currValue`-Variable und weisen Sie ihr den Wert bei `array[i]` zu. Deklarieren Sie dann eine `j`-Variable und weisen Sie ihr `i - 1` zu. Ihre `j`-Variable sollte neu zuweisbar sein.

# --hints--

Sie sollten eine `currValue`-Variable mit `const` deklarieren.
...
Sie sollten `currValue` den Wert bei `array[i]` zuweisen.
...
Sie sollten eine `j`-Variable mit `let` deklarieren.
...
Sie sollten `j` den Wert von `i - 1` zuweisen.
