...
更新 `healthText.innerText` 和 `monsterHealthText.innerText` 使其等于 `health` 和 `monsterHealth`。

# --hints--

你应该更新 `healthText.innerText`。

```js
assert.match(attack.toString(), /healthText\.innerText/);
```

你应该将 `healthText.innerText` 设置为 `health`。

```js
assert.match(attack.toString(), /healthText\.innerText\s*=\s*health/);
```

你应该更新 `monsterHealthText.innerText`。

```js
assert.match(attack.toString(), /monsterHealthText\.innerText/);
```

你应该将 `monsterHealthText.innerText` 设置为 `monsterHealth`。
