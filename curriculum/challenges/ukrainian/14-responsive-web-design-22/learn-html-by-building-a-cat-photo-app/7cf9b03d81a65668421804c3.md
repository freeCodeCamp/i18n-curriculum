...
Your `input` element does not have a `name` attribute. Перевірте, чи є пробіл після назви початкового тега та/або є пробіли перед усіма назвами атрибутів.

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('name'));
```

Your `input` element should have a `name` attribute with the value `catphotourl`.  Ви або не написали значення, або маєте друкарську помилку. Пам'ятайте, що значення атрибутів повинні бути в лапках.
