...
Nel blocco `try` crea una variabile `saved`. Per il valore, usa il metodo `getItem()` per recuperare `STORAGE_KEY`. Subito sotto quella variabile, crea un'istruzione `if` con la condizione `saved`. Lascia per ora vuoto il blocco `if`.

# --hints--

La tua funzione `loadPlayer` dovrebbe chiamare `localStorage.getItem` con `STORAGE_KEY` all'interno del blocco `try`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Il risultato di `localStorage.getItem` dovrebbe essere memorizzato in una variabile chiamata `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Il blocco `try` dovrebbe avere un'istruzione `if` con la condizione `saved`.
