...
あなたの`allSongs`配列には2つの値があるはずです。

```js
assert.equal(allSongs.length, 2);
```

あなたの`allSongs`配列の2番目の値はオブジェクトであるべきです。

```js
assert.isObject(allSongs[1]);
```

あなたの`allSongs`配列の2番目のオブジェクトは、`id`プロパティが数値の`1`に設定されているべきです。

```js
assert.equal(allSongs[1].id, 1);
```

あなたの`allSongs`配列の2番目のオブジェクトは、`title`プロパティが文字列の`"Can't Stay Down"`に設定されているべきです。

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

あなたの`allSongs`配列の2番目のオブジェクトは、`artist`プロパティが文字列の`"Quincy Larson"`に設定されているべきです。

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

あなたの`allSongs`配列の2番目のオブジェクトは、`duration`プロパティが文字列の`"4:15"`に設定されているべきです。

```js
assert.equal(allSongs[1].duration, "4:15");
```

あなたの`allSongs`配列の2番目のオブジェクトは、`src`プロパティが文字列の`"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`に設定されているべきです。
