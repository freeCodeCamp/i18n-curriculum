...
새로운 `.large` 선택자를 만들고 `height` 속성을 `10px`로 설정하세요. 또한 `.large, .medium` 선택자를 만들고 `background-color` 속성을 `black`로 설정하세요.

# --hints--

새로운 `.large` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.large'));
```

`.large` 선택자의 `height` 속성은 `10px`로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.large')?.height === '10px');
```

새로운 `.large, .medium` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.large, .medium'));
```

`.large, .medium` 선택자의 `background-color` 속성은 `black`로 설정되어야 합니다.
