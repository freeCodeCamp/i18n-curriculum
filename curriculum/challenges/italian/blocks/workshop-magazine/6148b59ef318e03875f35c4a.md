...
Crea un selettore `img` e dagli una proprietà `width` impostata su `100%` e una proprietà `object-fit` impostata su `cover`.

La proprietà `object-fit` indica al browser come posizionare l'elemento all'interno del suo contenitore. In questo caso, `cover` farà sì che l'immagine riempia il contenitore, ritagliando se necessario per evitare di modificare il rapporto d'aspetto.
...
Il tuo selettore `img` dovrebbe avere una proprietà `object-fit` impostata su `cover`.
