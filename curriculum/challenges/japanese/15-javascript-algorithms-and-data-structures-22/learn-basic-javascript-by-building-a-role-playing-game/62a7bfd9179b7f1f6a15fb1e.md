...
空の関数を新しく 3 つ作成してください。それぞれ `buyHealth`、`buyWeapon`、`goTown` という名前にしてください。

# --hints--

`function` キーワードを使用して `buyHealth` を宣言してください。

```js
assert.match(code, /function\s+buyHealth/);
```

`function` キーワードを使用して `buyWeapon` を宣言してください。

```js
assert.match(code, /function\s+buyWeapon/);
```

`function` キーワードを使用して `goTown` を宣言してください。

```js
assert.match(code, /function\s+goTown/);
```

`buyHealth` は空の関数にしてください。

```js
assert.equal(buyHealth.toString(), 'function buyHealth() {}');
```

`buyWeapon` は空の関数にしてください。
