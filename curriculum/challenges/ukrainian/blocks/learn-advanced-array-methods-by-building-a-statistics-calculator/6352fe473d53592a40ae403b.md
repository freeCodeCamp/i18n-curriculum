...
У вашій порожній функції зворотного виклику `.reduce()` оголосіть змінну `difference` і встановіть її значення рівним `el` мінус `mean`. Потім оголосіть змінну `squared` і встановіть її значення рівним `difference` у степені `2`. Нарешті, поверніть значення `acc` плюс `squared`.

# --hints--

У вашій функції зворотного виклику `reduce` має бути змінна `difference`.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

Значення вашої змінної `difference` має бути рівним `el` мінус `mean`.
...
У вашій функції зворотного виклику `reduce` має бути змінна `squared`.
...
Значення вашої змінної `squared` має бути рівним `difference` у степені 2.
...
Ваша функція зворотного виклику `reduce` має повертати значення `acc` плюс `squared`.
...
Ви не повинні видаляти ініціальне значення `0` у вашому методі `.reduce()`.
