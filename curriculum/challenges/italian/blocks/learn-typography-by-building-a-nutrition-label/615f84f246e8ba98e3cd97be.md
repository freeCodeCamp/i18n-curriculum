...
Sotto il tuo elemento `.double-indent`, aggiungi un nuovo elemento `p` con il testo `Includes 10g Added Sugars 20%`. Il tuo nuovo elemento `p` dovrebbe essere anche indentato doppiamente e non avere bordo inferiore. Usa un `span` per rendere il `20%` in grassetto e allineato a destra.
