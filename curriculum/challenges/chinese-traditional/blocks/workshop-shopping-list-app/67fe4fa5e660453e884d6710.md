...
創建一個名為 `toggleItem` 的函式（程式），該函式以 `item` 作為引數。

在函式中，暫時只需呼叫 `setSelectedItems()` 函式。稍後我們會回來實作切換已選取項目的邏輯。

# --hints--

你應該有一個名為 `toggleItem` 的函式（程式），它以 `item` 作為引數。
...
你應該在 `toggleItem()` 函式（程式）內呼叫 `setSelectedItems()` 函式（程式）。
