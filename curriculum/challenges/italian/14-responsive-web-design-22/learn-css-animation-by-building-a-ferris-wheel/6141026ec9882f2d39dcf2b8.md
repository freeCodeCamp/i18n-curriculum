...
Dovresti creare un nuovo selettore `50%` nella regola `@keyframes cabins`.
...
Il selettore `50%` dovrebbe essere tra i selettori `0%` e `100%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '50%');
```

Il selettore `50%` dovrebbe avere una proprietà `background-color` impostata su `purple`.
