...
記住，`grid-column` 屬性決定了元素開始和結束的列。 有時你不確定網格將有多少列，但你希望元素停在最後一列。 爲此，你可以使用 `-1` 作爲結束列。

創建一個 `.hero` 選擇器，並將其 `grid-column` 屬性設置爲 `1 / -1`。 這將告訴元素跨越網格的整個寬度。
...
你的 `.hero` 選擇器應該有一個 `grid-column` 屬性，設置爲 `1 / -1`。
