...
一旦玩家擁有了最好的武器，他們就不能購買另一個。 將 `buyWeapon` 函數中的所有代碼包裝在另一個 `if` 語句中。 條件應該檢查 `currentWeaponIndex` 是否小於 `3`（最後一個武器的索引）。

# --hints--

你應該有一個新的 `if` 語句來檢查 `currentWeaponIndex` 是否小於 `3`。

```js
assert.match(buyWeapon.toString(), /currentWeaponIndex\s*\<\s*3/)
```

你現有的 `if` 語句應位於新的 `if` 語句內。
