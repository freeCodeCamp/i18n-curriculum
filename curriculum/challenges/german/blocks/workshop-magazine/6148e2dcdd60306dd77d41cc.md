...
Es ist Zeit, den letzten Abschnitt des Magazins zu gestalten – die Bilder.

Die Bilder sind mit einem `aside`-Element umschlossen, das die `image-wrapper`-Klasse verwendet. Erstellen Sie also einen `.image-wrapper`-Selektor. Setzen Sie die `display`-Eigenschaft auf `grid`.

# --hints--

Sie sollten einen `.image-wrapper`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

Ihr `.image-wrapper`-Selektor sollte eine `display`-Eigenschaft mit dem Wert `grid` haben.
