...
メソッドチェーンで、既存の `.then()` メソッドに `.then()` をもう 1 つ繋げてください。 今回は、コールバック関数の仮引数として `data` を渡してください。 また、コールバックに複数の式を入れるため、波括弧を使用してください。 そしてコールバック関数内で `data` をコンソールに出力して、どのようになっているか確認しましょう。
