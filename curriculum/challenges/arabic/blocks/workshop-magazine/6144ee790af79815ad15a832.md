...
أنت الآن جاهز لبدء تجميع تخطيط الشبكة. في الدروس السابقة تعلمت أن CSS Grid يوفر تخطيطًا ثنائي الأبعاد قائمًا على الشبكة، مما يسمح لك بمحاذاة العناصر أفقيًا وعموديًا مع الاحتفاظ بالتحكم للقيام بأشياء مثل تراكب العناصر.

ابدأ بإنشاء محدِّد `main` ومنحه خاصية `display` بقيمة `grid`.

# --hints--

يجب أن يكون لديك محدِّد `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

يجب أن يحتوي محدِّد `main` على خاصية `display` بقيمة `grid`.
