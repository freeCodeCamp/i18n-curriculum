...
Kwa hali ya tamko la `if`, angalia kama `target` si mfano wa `HTMLElement`. Kisha ndani yake, ongeza `return`.

# --hints--

Unapaswa kuangalia katika tamko lako la `if` linapaswa kuwa na hali ya `!(target instanceof  HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

Unapaswa kuwa na `return` ndani ya tamko lako la `if`.
