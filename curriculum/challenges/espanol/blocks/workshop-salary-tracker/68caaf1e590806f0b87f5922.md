...
Al comienzo de tu setter, crea una estructura `if` que lance un `TypeError` con el mensaje `'salary' must be a number.` cuando `new_salary` no sea ni un entero ni un float.

# --hints--

Debes tener una estructura `if` en el setter de `salary`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[0]`)) })
```

Debes lanzar un `TypeError` con el mensaje `'salary' must be a number.` cuando `new_salary` no sea ni un entero ni un float.
...
No debes generar ninguna excepción cuando `new_salary` sea un entero o un float.
