...
أضف كائنين آخرين إلى `instrumentsArr` لتمثيل آلات النفخ الخشبية المتبقية، الكلارينيت والأوبوا.

# --hints--

يجب أن يحتوي `instrumentsArr` على `{ category: "woodwinds", instrument: "Clarinet", price: 200 }`.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

يجب أن يحتوي `instrumentsArr` على `{ category: "woodwinds", instrument: "Oboe", price: 4000 }`.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

يجب أن يحتوي `instrumentsArr` على `{ category: "woodwinds", instrument: "Flute", price: 500 }` أيضًا.
