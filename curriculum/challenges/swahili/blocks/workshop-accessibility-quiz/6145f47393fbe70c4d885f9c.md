...
Panga ndani kipengele kimoja cha `textarea` ndani ya kipengele cha pili cha `div.answer`, na weka idadi ya safu na nguzo kilizonazo.

# --hints--

Unapaswa kupanga ndani kipengele kimoja cha `textarea` ndani ya kipengele cha pili cha `div.answer`.

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

Unapaswa kumpa `textarea` sifa ya kipengele ya `rows` yenye nambari.
...
Unapaswa kumpa `textarea` sifa ya kipengele ya `cols` yenye nambari.
