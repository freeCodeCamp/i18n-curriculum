...
現在你需要對你的 `elif` 述語做同樣的處理：將 `print()` 和 `make_allowed_move()` 呼叫放入 `if` 述語中，以便在 `n` 為奇數時執行。

此外，將 `# make_allowed_move(source, auxiliary)` 註解轉換成程式碼。

# --hints--

你應該將你的 `print()` 和 `make_allowed_move()` 呼叫巢狀在 `if` 述語中，以便在 `n` 為奇數時執行。
