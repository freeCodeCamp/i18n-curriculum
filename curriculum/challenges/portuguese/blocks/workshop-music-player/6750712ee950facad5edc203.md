...
Se você brincar um pouco com seu player, vai notar alguns bugs. Primeiro, quando você tocar uma música, pause-a e então pressione o botão anterior ou o botão próximo, a música não começará do início.

Para começar a corrigir isso, adicione um segundo parâmetro chamado `start` à sua função `playSong`. Dê o valor padrão `true`.

# --hints--

Sua função `playSong` deve ter um segundo parâmetro chamado `start` com o valor padrão `true`.
