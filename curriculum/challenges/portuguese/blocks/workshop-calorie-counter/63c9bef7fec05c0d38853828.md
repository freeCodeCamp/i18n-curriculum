...
Adicione uma linha à função `calculateCalories` que chame o método `preventDefault()` no parâmetro `e`. Então, redefina sua flag global de erro `isError` para `false`.
...
Sua função `calculateCalories` deve redefinir a flag global de erro `isError` para `false`.
