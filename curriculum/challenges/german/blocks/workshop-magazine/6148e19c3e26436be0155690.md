...
Erstellen Sie eine `.lists li`-Regel, um die Listenelemente innerhalb Ihres `.lists`-Elements anzusprechen. Geben Sie ihr eine `margin-bottom`-Eigenschaft mit dem Wert `1.5rem`.

# --hints--

Sie sollten einen `.lists li`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

Ihr `.lists li`-Selektor sollte eine `margin-bottom`-Eigenschaft mit dem Wert `1.5rem` haben.
