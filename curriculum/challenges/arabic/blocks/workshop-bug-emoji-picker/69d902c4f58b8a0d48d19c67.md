...
املأ الآن `bugMap` بمدخلين. أضف المفتاح `"bee"` مرتبطًا بنسخة جديدة من `Bee`، والمفتاح `"spider"` مرتبطًا بنسخة جديدة من `Spider`. مرر `bugEmojiElement` كمعلمة لكل منهما.

# --hints--

يجب أن يحتوي `bugMap` على مدخل `"bee"` مضبوط على `new Bee(bugEmojiElement)`.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

يجب أن يحتوي `bugMap` على مدخل `"spider"` مضبوط على `new Spider(bugEmojiElement)`.
