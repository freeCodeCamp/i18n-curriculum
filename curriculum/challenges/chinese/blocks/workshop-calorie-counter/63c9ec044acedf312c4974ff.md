...
循环完成后，你需要清除 `budgetNumberInput`。 将 `budgetNumberInput` 的 `value` 属性设置为空字符串。

# --hints--

你的 `clearForm` 函数应该访问 `budgetNumberInput` 元素的 `value` 属性。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value/);
```

你的 `clearForm` 函数应该将 `budgetNumberInput` 元素的 `value` 属性设置为空字符串。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value\s*=\s*('|"|`)\1/);
```

你应该在 `for` 循环之后修改 `budgetNumberInput`。
