...
創建另一個捕捉 `MediaError as e` 的 `except` 語句。在 `except` 區塊中，使用 f-string 列印 `Media Error:` 後接一個空格和錯誤訊息。

# --hints--

你應該有一個 `except` 語句來捕捉 `MediaError as e`。
...
你應該使用 f-string 在你的新 `except` 區塊中列印 `Media Error:`，後面接一個空格和錯誤訊息。
