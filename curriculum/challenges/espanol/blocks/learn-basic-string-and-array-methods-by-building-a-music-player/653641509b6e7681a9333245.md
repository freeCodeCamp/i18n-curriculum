...
Necesitas actualizar la canción actual que se está reproduciendo, así como la apariencia del elemento `playButton`.

Asigna `song` a la propiedad `currentSong` en el objeto `userData`.

*Nota*: No debes usar el operador de encadenamiento opcional `?.` en este paso porque `userData.currentSong` no será `null` o `undefined` en este punto.

# --hints--

Debes asignar `song` a `userData.currentSong`.
