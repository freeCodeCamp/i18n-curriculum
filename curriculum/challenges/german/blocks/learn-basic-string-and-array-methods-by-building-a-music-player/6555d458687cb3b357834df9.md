...
Greifen Sie auf die `userData?.currentSong?.title` und `userData?.currentSong?.artist` Eigenschaften zu und weisen Sie sie jeweils den Variablen `currentTitle` und `currentArtist` zu.

# --hints--

Sie sollten auf die `title` von `currentSong` im `userData` Objekt zugreifen. Vergessen Sie nicht, optionale Verkettung zu verwenden.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

Sie sollten die `title` von `currentSong` einer `currentTitle` Konstanten zuweisen.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

Sie sollten auf die `artist` von `currentSong` im `userData` Objekt zugreifen. Vergessen Sie nicht, optionale Verkettung zu verwenden.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

Sie sollten die `artist` von `currentSong` einer `currentArtist` Konstanten zuweisen.
