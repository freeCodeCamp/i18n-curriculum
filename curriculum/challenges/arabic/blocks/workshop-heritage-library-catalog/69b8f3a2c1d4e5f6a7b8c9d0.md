...
تعيد `parseCatalog` حاليًا مصفوفة فارغة. لملئها، أضف حلقة `for` داخل الدالة تقوم بالتكرار على `rawCards`. داخل الحلقة، استدعِ `parseCard` على كل عنصر وادفع النتيجة إلى `catalog`.

# --hints--

يجب أن تكون قيمة `parseCatalog(rawCatalogCards).length` هي `19`.

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

يجب أن تستخدم `parseCatalog` حلقة `for` للتكرار على `rawCards`.

```js
assert.match(parseCatalog.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*rawCards\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

يجب استدعاء `parseCard` داخل الحلقة.
