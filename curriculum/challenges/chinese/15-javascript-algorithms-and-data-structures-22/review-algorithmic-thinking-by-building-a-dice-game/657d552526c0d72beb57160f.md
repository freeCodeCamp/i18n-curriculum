...
在游戏的这个阶段，你可以掷骰子，做出选择并进行几轮游戏。 然而，你应该注意，游戏没有结束，并且有无限的轮次。 根据规则，总共应该有六轮游戏，然后以最终得分结束。

当用户选择分数后，运行你的逻辑，你应该检查是否已经进行了6轮。 如果是这样，在500毫秒后显示用户的最终得分警报。

# --hints--

如果已经进行了六轮，你应该向用户显示一个警报。
