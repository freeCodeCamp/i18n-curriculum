...
Вам потрібно написати `return`, а потім пробіл та значення, яке має повернути функція. Once the `return` statement is found, that value is returned and the execution of the function stops, proceeding to the next line of code after the function call. Функція `foo` повертає рядок `'spam'` в прикладі вище.
