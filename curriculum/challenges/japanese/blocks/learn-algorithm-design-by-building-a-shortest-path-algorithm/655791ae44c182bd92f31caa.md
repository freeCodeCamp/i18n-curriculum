...
`.remove()` メソッドは、リストから引数として渡された最初の一致する要素を削除します。
...
現在のノードを `while` リストから削除して `unvisited` ループを終了してください。インデントに注意してください。

# --hints--

`.remove()` ループの後に、現在のノードを `unvisited` から削除するには `for` メソッドを使うべきです。
