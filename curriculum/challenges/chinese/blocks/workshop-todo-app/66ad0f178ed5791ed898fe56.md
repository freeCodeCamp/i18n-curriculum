...
通过在函数开头添加检查来修复此问题，检查是否为 `!titleInput.value.trim()`。 如果没有标题，则显示带有文本 `Please provide a title` 和 `return` 的警报。

# --hints--

你应该在 `addOrUpdateTask` 函数内添加一个带有条件 `!titleInput.value.trim()` 的 `if` 语句。
