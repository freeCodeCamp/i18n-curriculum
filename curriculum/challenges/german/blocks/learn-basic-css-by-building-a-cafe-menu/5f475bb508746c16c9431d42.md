...
Das Bild, das Sie hinzugefügt haben, ist nicht horizontal zentriert wie die `Coffee` Überschrift darüber. `img` Elemente sind „wie“ Inline-Elemente.

Um das Bild wie Block-Elemente wie Überschriften zu verhalten, erstellen Sie einen `img` Typselektor. Setzen Sie die `display` Eigenschaft auf `block` und verwenden Sie die `margin-left` und `margin-right` Eigenschaften, um es horizontal zu zentrieren.

# --hints--

Sie sollten einen `img` Selektor verwenden.
...
Sie sollten die `margin-left` Eigenschaft auf `auto` setzen.
...
Sie sollten die `margin-right` Eigenschaft auf `auto` setzen.
...
Ihr `img` Element sollte eine `display` von `block` haben.
...
Ihr `img` Element sollte eine `margin-left` und `margin-right` von `auto` haben.
