...
`.publish-date` セレクターを作成し、`color` プロパティに `rgba(255, 255, 255, 0.5)` を設定してください。

# --hints--

`.publish-date` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

`.publish-date` セレクターには `color` プロパティが `rgba(255, 255, 255, 0.5)` に設定されているはずです。
