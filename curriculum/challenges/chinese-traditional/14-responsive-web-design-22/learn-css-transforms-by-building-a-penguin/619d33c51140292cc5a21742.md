...
應該將 `.arm.left` 的 `animation-name` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationName, 'wave');
```

應該將 `.arm.left` 的 `animation-duration` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationDuration, '3s');
```

應該將 `.arm.left` 的 `animation-iteration-count` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationIterationCount, 'infinite');
```

應該將 `.arm.left` 的 `animation-timing-function` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
