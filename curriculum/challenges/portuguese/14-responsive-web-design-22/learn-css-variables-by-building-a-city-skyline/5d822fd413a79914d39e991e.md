...
Você deve dar aos elementos `.fb2-window` uma `width` de `22%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

Você deve dar aos elementos `.fb2-window` uma `height` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

Você deve dar a `.fb2-window` uma `background-color` de `--window-color3`.
