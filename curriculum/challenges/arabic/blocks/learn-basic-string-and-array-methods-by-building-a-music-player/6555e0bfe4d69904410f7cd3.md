...
استخدم `const` وصيغة السهم لإنشاء دالة فارغة تُسمى `shuffle`.

تتولى هذه الدالة خلط الأغاني في قائمة التشغيل وتنفيذ التحديثات اللازمة لإدارة الحالة بعد الخلط.

# --hints--

يجب استخدام `const` لإنشاء دالة فارغة باسم `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

يجب أن تكون `shuffle` دالة.

```js
assert.isFunction(shuffle)
```

يجب أن تستخدم دالة `shuffle` صيغة السهم وألا تأخذ معلمة.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

يجب أن تكون دالة `shuffle` فارغة.
