...
Devi renderizzare di nuovo le canzoni, evidenziarla e impostare il testo accessibile del pulsante di riproduzione dato che la lista delle canzoni cambierà.

Chiama la funzione `renderSongs` e passa l'array `userData?.songs` come argomento, questo mostra la playlist modificata.

Dopodiché, chiama la funzione `highlightCurrentSong` per evidenziare la canzone corrente se ce n'è una e la funzione `setPlayButtonAccessibleText` per aggiornare il testo accessibile del pulsante di riproduzione.

# --hints--

Non dovresti modificare la funzione `deleteSong` esistente e il suo contenuto.
...
Dovresti chiamare la funzione `renderSongs` con `userData?.songs`.
