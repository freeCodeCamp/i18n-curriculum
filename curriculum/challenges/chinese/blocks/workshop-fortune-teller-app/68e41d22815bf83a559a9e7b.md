...
现在，使用 `onload` 事件在图像装载时移除 `hidden` 类，并使用其 `previousElementSibling` 隐藏加载占位符。

# --hints--

你的 `img` 元素应该有一个 `onload` 处理器。
...
你的 `onload` 处理器应该使用 `classList.remove` 来移除 `hidden` 类。别忘了这里需要 `this` 关键字。
...
你的 `onload` 处理器应该将 `previousElementSibling.style.display` 设置为 `'none'`。别忘了这里需要 `this` 关键字。
