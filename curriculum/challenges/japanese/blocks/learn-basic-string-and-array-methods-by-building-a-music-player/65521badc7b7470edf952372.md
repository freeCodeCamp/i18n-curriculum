...
次は、現在再生中の曲を一時停止する機能が必要です。

`const` キーワードとアロー関数構文を使用して、`pauseSong` 関数を定義してください。 この関数は引数を取る必要はありません。

# --hints--

`const` を使用して `pauseSong` という空の関数を作成してください。

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` は関数にしてください。

```js
assert.isFunction(pauseSong)
```

`pauseSong` 関数はアロー構文を使用する必要があります。

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

`pauseSong` 関数は空にしてください。
