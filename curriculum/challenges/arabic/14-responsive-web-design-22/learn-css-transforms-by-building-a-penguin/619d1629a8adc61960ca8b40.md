...
يجب عليك استخدام منتقي `.shirt`.

```js
assert.match(code, /\.shirt\s*\{/);
```

بداخل عنصر `.shirt` يجب أن تكون خاصية `font-size` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontSize, '25px');
```

بداخل عنصر `.shirt` يجب أن تكون خاصية `font-family` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontFamily, 'Helvetica, sans-serif');
```

بداخل عنصر `.shirt` يجب أن تكون خاصية `font-weight` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
