...
在 JavaScript 中，`change` 事件用於偵測 HTML 元素的值何時已改變：
...
將 `addEventListener` 方法附加到 `dateOptionsSelectElement`。事件監聽器的第一個引數應該是字串 `"change"`，第二個引數應該是一個空的箭頭函式。

# --hints--

你應該將 `addEventListener` 方法附加到 `dateOptionsSelectElement`。

```js
assert(code.match(/dateOptionsSelectElement\.addEventListener\s*\(/g));
```

你的事件監聽器應該監聽 `"change"` 事件。

```js
assert(code.match(/dateOptionsSelectElement\.addEventListener\s*\(\s*('|")change\1/g));
```

你應該為 `addEventListener` 方法添加一個空的箭頭函式作為第二個引數。
