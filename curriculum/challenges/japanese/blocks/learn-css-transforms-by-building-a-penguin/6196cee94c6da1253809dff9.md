...
`.back-mountain` 要素をターゲットにして、その `width` と `height` を `300px` に設定してください。それから、`background` を `rgb(203, 241, 228)` から始まり `rgb(47, 170, 255)` で終わる線形グラデーションに設定してください。

# --hints--

`.back-mountain` セレクターを使うべきです。

```js
assert.match(code, /\.back-mountain\s*\{/);
```

`.back-mountain` に `width` を `300px` に設定するべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

`.back-mountain` に `height` を `300px` に設定するべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

`.back-mountain` に `background` を `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))` に設定するべきです。
