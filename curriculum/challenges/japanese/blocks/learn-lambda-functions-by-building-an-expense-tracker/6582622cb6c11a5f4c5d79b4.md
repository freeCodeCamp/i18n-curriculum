...
まだ`elif`節の中で、`expenses_from_category`イテレーターを`print_expenses`呼び出しに渡してください。

# --hints--

`print_expenses()`節で`expenses_from_category`を引数として渡して`elif`を呼び出す必要があります。
...
`print_expenses()`変数の後に`expenses_from_category`呼び出しを配置してください。
