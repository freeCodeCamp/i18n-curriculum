...
`body-parser` já foi instalado e está no arquivo `package.json` do seu projeto. `require` ele no topo do arquivo `myApp.js` e armazene-o em uma variável chamada `bodyParser`. O middleware para manipular dados com codificação do URL é retornado por `bodyParser.urlencoded({extended: false})`. Passe a função retornada pela chamada do método anterior para `app.use()`. Como sempre, o middleware deve ser montado antes de todas as rotas que dependem dele.

**Nota:** `extended` é uma opção de configuração que informa ao `body-parser` qual análise deve ser usada. Quando `extended=false` ele usa a biblioteca clássica de codificação, `querystring`. Quando `extended=true` ele usa a biblioteca `qs` para a análise. 
