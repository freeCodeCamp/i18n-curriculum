...
階級徽章仍然顯示硬編碼的文字 `GOLD`。你需要用動態表達式取代它，這樣才能顯示正確的階級。

呼叫 `getPlayerTier()` 並傳入 `player.overallRating` 以取得階級字串，然後對結果呼叫 `.toUpperCase()` 以大寫顯示。別忘了將整個表達式包在大括號中。

# --hints--

你應該在 `getPlayerTier(player.overallRating).toUpperCase()` 裡使用 `.card-tier-badge`。

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

`.card-tier-badge` 元素應該顯示 `ELITE`，當玩家評分為 98 時。
