...
在你的 `HTMLString` 結尾處（新行）創建另一個 `label` 元素。這個 `label` 應該有文字 `Entry # Calories`，使用你的模板字串語法將 `#` 替換為 `entryNumber` 的值，並將 `for` 屬性設為 `X-#-calories`，其中 `X` 是 `entryDropdown` 的 `value`，`#` 是 `entryNumber` 的值。

# --hints--

你應該在你的 `HTMLString` 中有兩個 `label` 元素。
...
你的新 `label` 元素應該有一個 `for` 屬性，設定為 `${entryDropdown.value}-${entryNumber}-calories`。
...
你的新 `label` 元素應該有文字 `Entry ${entryNumber} Calories`。
