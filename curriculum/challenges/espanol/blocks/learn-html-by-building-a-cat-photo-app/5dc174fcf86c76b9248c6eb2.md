...
Tu elemento `h1` debe tener una etiqueta de cierre. Las etiquetas de cierre tienen un `/` justo después del carácter `<`.
...
El texto de tu elemento `h1` debe de ser `CatPhotoApp`. O has omitido el texto, tienes un error tipográfico, o no está entre las etiquetas de apertura y cierre del elemento `h1`.
