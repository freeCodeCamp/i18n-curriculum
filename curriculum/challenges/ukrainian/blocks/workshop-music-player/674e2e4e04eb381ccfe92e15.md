...
Тепер настав час перевірити кнопку паузи.

Додайте слухача події `"click"` до елемента `pauseButton`, а потім передайте `pauseSong` як другий аргумент слухача події. Це функція, яку слухач події виконуватиме.
...
Ви повинні викликати метод `addEventListener()` для вашої змінної `pauseButton`.
...
Ви повинні передати `pauseSong` як другий аргумент методу `addEventListener`.
