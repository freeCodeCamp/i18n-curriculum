...
1. Ви повинні мати функцію з іменем `validateForm`, яка повертає об’єкт із такими ключами: `full-name`, `email`, `order-no`, `product-code`, `quantity`, `complaints-group`, `complaint-description`, `solutions-group` і `solution-description`. Значення кожного ключа має бути `true`, якщо відповідне поле форми заповнене правильно, і `false` — інакше. Умовні поля опису мають бути `true`, коли вони не потрібні, інакше їх слід перевіряти на наявність щонайменше двадцяти символів.
...
Коли `#other-complaint` не встановлений, `validateForm()["complaint-description"]` має бути `true`.
...
Коли `#other-solution` не встановлений, `validateForm()["solution-description"]` має бути `true`.
