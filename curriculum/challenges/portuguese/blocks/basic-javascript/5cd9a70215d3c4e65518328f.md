...
A princípio, isso parece contraintuitivo, já que o valor de `n` *diminui*, mas os valores no array final estão *aumentando*. Isso acontece porque a adição no array (push) acontece por último, após a chamada recursiva ter retornado. No ponto onde `n` é adicionado ao array, `countup(n - 1)` já foi avaliado e retornou `[1, 2, ..., n -1]`.
