...
Use um loop `for` para iterar sobre cada dígito na lista `odd_digits`.Mova sua chamada `print` do passo anterior para dentro do loop `for` e altere-a para imprimir cada dígito.

# --hints--

Você deve usar um loop `for` sobre `odd_digits`.
...
Você deve ter `--fcc-expected--` dentro do loop `for`.
