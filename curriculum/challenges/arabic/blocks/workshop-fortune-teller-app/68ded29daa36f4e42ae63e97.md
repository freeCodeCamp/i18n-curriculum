...
اختم الدالة باستخدام طريقة المصفوفة `forEach` للتكرار على المعلمة elements. داخل دالة الاستدعاء الراجع، أضف الصنف hidden إلى كل عنصر باستخدام طريقة `.add()` من `classList`.

# --hints--

يجب أن تحتوي المعلمة `elements` على طريقة `forEach`.
...
يجب أن تأخذ طريقة forEach معلمة استدعاء راجع.
...
داخل طريقة forEach، يجب إضافة الصنف hidden باستخدام طريقة `classList.add`.
