...
Дотримуючись того самого шаблону, зробіть запит для своїх `number` в `#dinner`, `#snacks` та `#exercise` елементах. Призначте їх змінним, дотримуючись схеми іменування з попередніх двох.

# --hints--

Ви повинні оголосити змінну `dinnerNumberInputs`.

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

Ви повинні зробити виклик `document.querySelectorAll()` з `#dinner input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Ви повинні призначити результат виклику `document.querySelectorAll()` змінній `dinnerNumberInputs`.
...
Ви повинні оголосити змінну `snacksNumberInputs`.

```js
assert.match(calculateCalories.toString(), /snacksNumberInputs\s*=/);
```

Ви повинні зробити виклик `document.querySelectorAll()` з `#snacks input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#snacks input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Ви повинні призначити результат виклику `document.querySelectorAll()` змінній `snacksNumberInputs`.
...
Ви повинні оголосити змінну `exerciseNumberInputs`.

```js
assert.match(calculateCalories.toString(), /exerciseNumberInputs\s*=/);
```

Ви повинні зробити виклик `document.querySelectorAll()` з `#exercise input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#exercise input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Ви повинні призначити результат виклику `document.querySelectorAll()` змінній `exerciseNumberInputs`.
