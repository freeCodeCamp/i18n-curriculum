...
当 React 遇到引用另一个组件的自定义超文本标记语言标签（如本例中用 `< />` 包裹的组件名称）时，它会在该标签的位置渲染该组件的标记。这应当说明 `App` 组件与 `Navbar`、`Dashboard` 和 `Footer` 之间的父子关系。
...
**注意：** `ChildComponent` 是使用 ES6 的箭头函数定义的，这是使用 React 时非常常见的做法。
