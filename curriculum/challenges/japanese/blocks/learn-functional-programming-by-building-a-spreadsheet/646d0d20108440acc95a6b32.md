...
あなたの`elemValue`関数では、`inner`という関数を明示的に宣言して返しました。しかし、アロー構文を使う場合は、関数を暗黙的に返すことができます。例えば：

```js
const curry = soup => veggies => {};
```

`curry`は`soup`パラメータを受け取り、`veggies`パラメータを受け取る関数を返す関数です。この構文を使って、`addCharacters`関数を更新し、`character2`パラメータを受け取る空の関数を返すようにしてください。

# --hints--

あなたの`addCharacters`関数は暗黙の返却を使うべきです。
...
あなたの`addCharacters`関数は`character2`パラメータを持つアロー関数を返すべきです。
