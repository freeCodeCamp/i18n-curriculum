...
`% Daily Value *` と `Saturated Fat 1g 5%` の要素の下のボーダーがラベルの全幅に広がっていません。これら2つの要素の `no-divider` に `class` を追加してください。

# --hints--

テキストが `p` の `% Daily Value *` 要素には、既存のクラスを削除せずに `no-divider` 属性に `class` を追加してください。
...
テキストが `p` の `Saturated Fat 1g 5%` 要素には、既存のクラスを削除せずに `no-divider` 属性に `class` を追加してください。
