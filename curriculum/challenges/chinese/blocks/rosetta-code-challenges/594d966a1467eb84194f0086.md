...
计算整数集合 $1$ 到 $10$（含） 的所有三种 <a href="https://en.wikipedia.org/wiki/Pythagorean_means" target="_blank" rel="noopener noreferrer nofollow">毕达哥拉斯平均数</a>。
...
`pythagoreanMeans` 应该是一个函数。

```js
assert(typeof pythagoreanMeans === 'function');
```

`pythagoreanMeans([1, 2, ..., 10])` 应该与上面相同的输出相等。
