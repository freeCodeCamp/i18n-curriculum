...
In diesem Workshop erstellen Sie eine Musikplayer-App, die Songs aus einer Wiedergabeliste abspielen, pausieren und überspringen kann.

Das HTML und CSS dieses Projekts wurden für Sie bereitgestellt, sodass Sie sich auf das JavaScript konzentrieren können.

Beginnen Sie damit, auf die Elemente `#playlist-songs`, `#play` und `#pause` zuzugreifen und sie den Variablen `playlistSongs`, `playButton` und `pauseButton` zuzuweisen.
...
Sie sollten das Element `#playlist-songs` der Variable `playlistSongs` zuweisen. 
...
Sie sollten das Element `#play` der Variable `playButton` zuweisen. 
...
Sie sollten das Element `#pause` der Variable `pauseButton` zuweisen. 
