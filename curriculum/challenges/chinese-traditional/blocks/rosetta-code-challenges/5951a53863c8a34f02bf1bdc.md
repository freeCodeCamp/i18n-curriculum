...
提供一個函式（程式），用於尋找一組給定二維點中距離最近的兩個點。

直接的解決方案是一個 $O(n^2)$ 演算法（我們可以稱之為 *brute-force algorithm*）；使用索引的偽程式碼可以簡單表示為：
...
更好的演算法是基於遞迴分治法，時間複雜度為 $O(n\log n)$，一個偽程式碼可能是：
...
對於輸入，預期引數為一個包含 `Point` 物件的陣列，且 `x` 和 `y` 成員設為數字。傳回一個物件，該物件包含 `distance` 和 `pair`（兩個最近點的配對）這些鍵值對組。

例如 `getClosestPair`，輸入陣列為 `points`：
...
**注意：** 將 `pair` 陣列依其 `x` 值以遞增順序排序。


# --hints--

`getClosestPair` 應該是一個函式（程式）。
