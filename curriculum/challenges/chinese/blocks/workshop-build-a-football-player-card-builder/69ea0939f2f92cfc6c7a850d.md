...
接下来，添加一个 `useEffect` 钩子，它会在 `player` 状态变化时运行。在其中，使用箭头函数，并添加一个 `try..catch`，其 `error` 块的参数是 `catch`。

# --hints--

你应该在 `useEffect` 内调用 `FootballPlayerCard`。
...
你的 `useEffect` 应该有 `[player]` 作为它的依赖数组。

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

你的 `useEffect` 回调应该包含一个 `try...catch` 块，其 `error` 块的参数是 `catch`。
