...
Якщо ви виберете радіокнопку `Indoor` та надішлете форму, дані форми для кнопки будуть засновані на її атрибутах `name` та `value`. Оскільки радіокнопки не мають атрибута `value`, дані форми включатимуть `indoor-outdoor=on`, що не принесе користі, якщо ви маєте декілька кнопок.
