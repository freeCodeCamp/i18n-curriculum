...
在你的 `h2` 元素之后，创建两个 `p` 元素。 为第一个元素指定一个 `dessert-price` 的 `class`，并将文本设置为美元符号 `"$"` 后跟 `price` 变量的值。 为第二个元素指定一个 `product-category` 的 `class`，以及文本 `"Category: "`，后跟 `category` 变量的值。
