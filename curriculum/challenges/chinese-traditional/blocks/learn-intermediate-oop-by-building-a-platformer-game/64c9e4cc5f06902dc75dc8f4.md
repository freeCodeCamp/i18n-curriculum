...
當你開始遊戲時，你會注意到平台的位置會隨著玩家一起動畫。但如果你嘗試從其中一個平台下方跳躍，你就會直接穿過它。

為了解決此問題，你需要為遊戲添加碰撞檢測邏輯。

首先在 `platforms` 陣列上呼叫 `forEach` 方法，並將 `platform` 作為回呼函式的參數傳入。
