...
그다음 `userData.currentSong`을 `song`로 설정하고, 마지막으로 노래를 재생하려면 `audio` 변수에서 `play()` 메서드를 호출하세요. `play()`은 mp3 파일을 재생하기 위한 웹 오디오 API의 메서드입니다.

# --hints--

`playSong` 함수 내에서 `playButton` 요소에 `playing` 클래스를 추가해야 합니다.
...
`playSong` 함수 내에서 `audio` 변수에 `play()` 메서드를 호출해야 합니다.
