...
انتقل إلى العنصر الداخلي `div` في **خطة المستمع**، وأضف له الأصناف `grid`، و`grid-rows-[auto_auto_auto_1fr]`، و`gap-y-2`.

هذه الأصناف تجعل العنصر الداخلي `div` حاوية شبكة، وتحدد أربعة صفوف تتكدس فوق بعضها، مع ضبط الثلاثة الأولى على `auto`، والأخير يشغل المساحات المتبقية، وأخيرًا تضيف تباعدًا بمقدار `0.5rem` بين الصفوف.

# --hints--

يجب أن يحتوي العنصر الداخلي `div` على الصنف `grid`.
...
يجب أن يحتوي العنصر الداخلي `div` على الصنف `grid-rows-[auto_auto_auto_1fr]`.
...
يجب أن يحتوي العنصر الداخلي `div` على الصنف `gap-y-2`.
