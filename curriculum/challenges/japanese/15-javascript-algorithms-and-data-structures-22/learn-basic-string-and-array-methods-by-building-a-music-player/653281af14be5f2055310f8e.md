...
音楽プレーヤーは、すべての曲、再生中の曲、および再生中の曲の長さを把握する必要があります。 この情報を格納するためのオブジェクトを作成しましょう。

まず `let` キーワードを使用して `userData` という新しい変数を宣言し、空のオブジェクトを割り当ててください。

# --hints--

`let` キーワードを使用して変数 `userData` を作成してください。

```js
assert.match(code, /let\s+userData\s*=/)
```

変数 `userData` には空のオブジェクトを割り当ててください。
