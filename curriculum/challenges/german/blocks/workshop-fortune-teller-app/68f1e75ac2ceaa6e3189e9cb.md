...
Prüfen Sie, ob `cardElement` keine Instanz von `HTMLElement` ist, und fügen Sie in diesem Fall eine `return` hinzu.

# --hints--

Sie sollten eine weitere `if`-Anweisung mit der Bedingung `!(cardElement instanceof HTMLElement)` hinzufügen.
