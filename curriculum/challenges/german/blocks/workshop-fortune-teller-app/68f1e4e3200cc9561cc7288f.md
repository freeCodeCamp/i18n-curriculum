...
Für die Bedingung der `if`-Anweisung prüfen Sie, ob die `target` keine Instanz von `HTMLElement` ist. Fügen Sie dann darin eine `return` hinzu.

# --hints--

Sie sollten in Ihrer `if`-Anweisung die Bedingung `!(target instanceof  HTMLElement)` prüfen.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

Sie sollten eine `return` innerhalb Ihrer `if`-Anweisung haben.
