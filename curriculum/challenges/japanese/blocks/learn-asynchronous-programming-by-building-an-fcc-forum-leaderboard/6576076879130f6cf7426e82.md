...
フォーラムの各トピックには、Python や JavaScript のようなカテゴリが含まれています。次のいくつかのステップで、すべてのフォーラムカテゴリと `classNames` のスタイリングを保持するカテゴリオブジェクトを作成します。

まず、`allCategories` という新しい定数を作成し、空のオブジェクトを代入してください。

# --hints--

`allCategories` という定数があるはずです。

```js
assert.match(code, /const\s+allCategories\s*=/)
```

`allCategories` 変数に空のオブジェクトを代入してください。
