...
У методі `initializeEventListeners` приєднайте обробник події `click` до `singleCardBtn`. Усередині обробника події викличте метод `singleCardSelected`. Не забудьте отримати `singleCardBtn` з `elements`.
