...
次のプロパティと値を持つ3つ目のオブジェクトを追加してください：
...
あなたの`allSongs`配列の3番目の値はオブジェクトである必要があります。

```js
assert.isObject(allSongs[2]);
```

あなたの`allSongs`配列の3番目のオブジェクトは、数値`id`に設定された`2`プロパティを持つ必要があります。

```js
assert.equal(allSongs[2].id, 2);
```

あなたの`allSongs`配列の3番目のオブジェクトは、文字列`title`に設定された`"Still Learning"`プロパティを持つ必要があります。

```js
assert.equal(allSongs[2].title, "Still Learning");
```

あなたの`allSongs`配列の3番目のオブジェクトは、文字列`artist`に設定された`"Quincy Larson"`プロパティを持つ必要があります。

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

あなたの`allSongs`配列の3番目のオブジェクトは、文字列`duration`に設定された`"3:51"`プロパティを持つ必要があります。

```js
assert.equal(allSongs[2].duration, "3:51");
```

あなたの`allSongs`配列の3番目のオブジェクトは、文字列`src`に設定された`"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`プロパティを持つ必要があります。
