...
これでタブリストに機能を追加する準備が整いました。最初のステップは、タブリスト内のすべてのタブとパネルの要素にアクセスすることです。

`tabs` という変数を作成し、`role` 属性が `tab` に設定されているすべての要素をクエリして取得した結果を代入してください。

次に、`panels` という変数を作成し、`role` 属性が `tabpanel` に設定されているすべての要素をクエリして取得した結果を代入してください。

複数の要素を一度に選択するには、`querySelectorAll` メソッドを使えることを覚えておいてください。
...
`document.querySelectorAll('[role="tab"]')` を `tabs` 変数に代入してください。
...
`panels` という変数を用意してください。

```js
assert.isDefined(panels);
```

`document.querySelectorAll('[role="tabpanel"]')` を `panels` 変数に代入してください。
