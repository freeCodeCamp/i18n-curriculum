...
現在你可以為你的網格佈局設計樣式。CSS Grid 類似於 Flexbox，因為它對父元素和子元素都有特殊的屬性。 

在這種情況下，你的父元素是 `main` 元素。透過添加一個值為 `1fr 94rem 1fr` 的 `grid-template-columns` 屬性，將內容設定為三行佈局。這將創建三行，其中中間的行寬為 `94rem`，而第一行和最後一行則各佔網格容器剩餘空間的 1 份。

# --hints--

你的 `main` 區段應該有一個 `grid-template-columns` 屬性設定為 `1fr 94rem 1fr`。
