...
بعد ذلك، تحتاج إلى عرض عنوان الأغنية الحالية والفنان في شاشة المشغل. استخدم `const` وصيغة السهم لإنشاء دالة `setPlayerDisplay` فارغة.

# --hints--

يجب أن تستخدم `const` لإنشاء دالة فارغة باسم `setPlayerDisplay`.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

يجب أن تكون `setPlayerDisplay` دالة.

```js
assert.isFunction(setPlayerDisplay)
```

يجب أن تستخدم دالة `setPlayerDisplay` صيغة السهم.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

يجب أن تكون دالة `setPlayerDisplay` فارغة.
