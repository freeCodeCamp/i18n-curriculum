...
在你的 `checkUserInput` 函式中，移除 `console.log()` 述語。然後，呼叫 `decimalToBinary` 函式，並將 `numberInput` 的 `value` 作為引數傳入。還要確保使用 `parseInt()` 函式將輸入轉換為數字。

# --hints--

你應該從你的 `checkUserInput` 函式中移除 `console.log()` 述語。

```js
assert.notMatch(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*console\.log\((\s|.)*\)/);
```

你應該在你的 `checkUserInput` 函式（程式）主體內的 `if` 述語之後呼叫 `decimalToBinary` 函式（程式）。

```js
assert.match(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*decimalToBinary\(/);
```

你應該使用 `parseInt()` 將 `numberInput` 的 `value` 轉換成數字，並將其作為引數傳遞給 `decimalToBinary` 函式。
