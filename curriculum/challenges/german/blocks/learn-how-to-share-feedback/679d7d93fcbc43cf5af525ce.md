...
The `Present Perfect` (`I've investigated`) is used after `once` to show that the action must be fully completed before the next step happens. This emphasizes completion of the task rather than the moment in time when it happens. Zum Beispiel:
