---
id: 657b8487605c881dc8ebde9e
title: 任務 38
challengeType: 22
dashedName: task-38
lang: en-US
---

<!-- (audio) Sophie: That's cool. I like photography, but I don't think of it as a hobby. I play the guitar in my free time. -->

# --description--

當談論你的日常活動時，你也可能會說出你不做的事情。例如：

- `I don't watch TV in the morning.`－這表示你從不在中午前看電視。

- `She doesn't drink coffee at night.`－這表示她避免在晚上喝咖啡。

# --instructions--

聆聽音訊並完成以下句子。

# --fillInTheBlank--

## --sentence--

`That's cool. I like photography, but I BLANK think of it as a hobby. I play the guitar in my free time.`

## --blanks--

`don't`

### --feedback--

Sophie 正在談論一項她不認為是嗜好的活動，所以句子應該使用否定形式。這是 `do not` 的縮寫。

# --scene--

```json
{
  "setup": {
    "background": "company2-breakroom.png",
    "characters": [
      {
        "character": "Sophie",
        "position": {"x":50,"y":0,"z":1.4},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "1.3-2.mp3",
      "startTime": 1,
      "startTimestamp": 10.50,
      "finishTimestamp": 15.40
    }
  },
  "commands": [
    {
      "character": "Sophie",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sophie",
      "startTime": 1,
      "finishTime": 3.94,
      "dialogue": {
        "text": "That's cool. I like photography, but I don't think of it as a hobby.",
        "align": "center"
      }
    },
    {
      "character": "Sophie",
      "startTime": 4.22,
      "finishTime": 5.90,
      "dialogue": {
        "text": "I play the guitar in my free time.",
        "align": "center"
      }
    },
    {
      "character": "Sophie",
      "opacity": 0,
      "startTime": 6.40
    }
  ]
}
```
