...
再び、`padRow` と `i` の変数を使って `count` を呼び出した結果を `rows` 配列に追加してください。

コンソールを開いて、逆さピラミッドを確認しましょう。

# --hints--

`for` ループ内で `rows.push()` を呼び出す必要があります。

```js
assert.match(__helpers.removeJSComments(code), /for\s*\(\s*let\s+i\s*=\s*count\s*;\s*i\s*>\s*0\s*;\s*i\s*=\s*i\s*-\s*1\s*\)\s*\{\s*rows\.push\(/);
```

`padRow()` の呼び出し内で `.push()` を呼び出す必要があります。
...
`i` の呼び出しに最初の引数として `padRow()` を渡す必要があります。
...
`count` の呼び出しに2番目の引数として `padRow()` を渡す必要があります。
