...
Ändern Sie die Hintergrundfarbe des `hr`-Elements zu `brown`, damit sie zur Farbe der Kaffeebohnen passt.

# --hints--

Sie sollten den Wert der `background-color`-Eigenschaft auf `brown` setzen.

```js
const hasBackgroundColor = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style['background-color'] === 'brown');
```

Ihr `hr`-Element sollte eine `background-color` von `brown` haben.
