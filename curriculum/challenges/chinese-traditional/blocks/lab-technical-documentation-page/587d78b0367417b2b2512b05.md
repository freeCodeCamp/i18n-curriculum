...
1. 在 `#main-doc` 元素中，你可以看到數個 `section` 元素，每個都有一個 `main-section` 類別。應該至少有五個。
...
1. `.main-section` 元素總共應包含至少 10 個 `p` 元素（不是每個）。
1. `.main-section` 元素總共應包含至少五個 `code` 元素（不是每個）。
1. `.main-section` 元素總共應包含至少五個 `li` 項目（不是每個）。
1. 你可以看到一個具有對應 `id="navbar"` 的 `nav` 元素。
1. 導覽列元素應包含一個 `header` 元素，該元素包含描述技術文件主題的文字。
1. 此外，navbar 應該包含具有 `nav-link` 類別的聯結（`a`）元素。每個具有 `main-section` 類別的元素都應該有一個。
1. `#navbar` 中的 `header` 元素必須位於導覽列中任何連結（`a`）元素之前。
1. 每個具有 `nav-link` 類別的元素都應包含與每個 `section` 中的 `header` 文字相對應的文字（例如，如果你有一個「Hello world」的區段/標題，你的導覽列應該有一個包含「Hello world」文字的元素）。
