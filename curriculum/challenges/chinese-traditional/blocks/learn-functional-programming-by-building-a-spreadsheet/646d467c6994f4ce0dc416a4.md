...
使用三元運算符將 `.hasOwnProperty()` 調用轉換爲條件。 如果對象具有該屬性，則返回以 `fn` 和 `args` 作爲參數調用 `apply` 的結果。 否則，返回 `match`。
...
如果三元條件爲真，則回調函數應該返回調用 `apply()` 的結果。
