...
如果你現在使用該應用程式，你會看到每次勾選或取消勾選一個項目，或在搜尋欄輸入時，`toggleItem()` 函式（程式）都會被重新生成。

為了提升效率，你可以使用 `useCallback()` 掛鉤來快取 `toggleItem()` 函式（程式）。然後 React 只會在其依賴項改變時重新建立該函式（程式）。

首先，在你的檔案頂部從 React 解構 `useCallback()` 掛鉤。

# --hints--

你應該在檔案頂端從 `React` 解構 `useCallback`。確保你沒有移除任何你已經在使用的其他掛鉤。
