...
`ul`/`li` 요소는 목록 항목에 불릿을 제공하는 데 훌륭하지만, 라디오 버튼에는 필요하지 않습니다. `list-style` 속성으로 불릿 모양을 제어할 수 있습니다. 예를 들어 다음과 같이 불릿을 원형으로 바꿀 수 있습니다:
...
`.answers-list` 항목의 기본 스타일을 `none`로 설정하여 제거하고, 순서 없는 목록의 패딩도 제거하세요.

# --hints--

`.answers-list` 선택자를 사용하세요.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.answers-list'));
```

`.answers-list`에 `list-style`를 `none`로 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.answers-list')?.listStyle, 'none');
```

`.answers-list`에 `padding`를 `0`로 지정하세요.
