...
現在新增另一個名為 `showFortune` 的方法。作為參數，傳入 `e` 並指定其型別為 `Event`。同時也創建一個 `newReading` 方法，暫時保持空白。

# --hints--

你應該宣告一個名為 `showFortune` 的方法。
...
你的 `showFortune` 方法應該有一個型別為 `e` 的 `Event` 參數。
