...
创建一个名为 `filteredItems` 的变量，并将使用 `filter()` 方法过滤 `items` 数组的结果赋给它。 将一个匿名函数传递给以 `item` 作为参数的 `filter()` 方法。 在匿名函数内部，目前只需返回 `item`。

# --hints--

你应该创建一个名为 `filteredItems` 的变量。

```js
assert.match(code, /(const|let|var)\s+filteredItems\s*=/);
```

你应该在 `items` 数组上使用 `filter()` 方法。

```js
assert.match(code, /items\.filter\s*\(/);
```

你的 `filter()` 方法应该采用一个匿名函数，该函数以 `item` 作为参数并返回 `item`。
