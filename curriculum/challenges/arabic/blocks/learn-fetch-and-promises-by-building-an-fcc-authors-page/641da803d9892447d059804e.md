...
الآن أنشئ وسم صورة وأضف له الـ `class` `"user-img"`. استخدم الاستيفاء النصي لضبط خاصية `src` إلى المتغير `image` الذي فككتَه سابقًا. اضبط خاصية `alt` إلى المتغير `author` متبوعًا بالنص `"avatar"`. تأكد من وجود مسافة بين متغير `author` وكلمة `"avatar"`، مثل: `"Quincy Larson avatar"`.
