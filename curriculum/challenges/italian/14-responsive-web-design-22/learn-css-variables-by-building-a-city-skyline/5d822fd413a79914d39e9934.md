...
Dovresti dare al `repeating-linear-gradient` una direzione di `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Dovresti dare al `repeating-linear-gradient` un primo colore di `--building-color3` dallo `0%` al `10%`.
...
Dovresti dare al `repeating-linear-gradient` un secondo colore di `transparent` dal `10%` al `30%`.
