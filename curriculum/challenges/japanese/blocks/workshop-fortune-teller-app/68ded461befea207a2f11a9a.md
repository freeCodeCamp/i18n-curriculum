...
So far, you have three functions: `getElement`, `hideElements`, and `showElements`. Now you need a function that helps you get a random fortune.

Declare a function named `getRandomItem` that takes an array of `items` as an argument. Use generics for this function.

Inside the function, use `Math.random()` and `Math.floor()` to select a random fortune from the `items` array. Don’t forget to return the selected fortune.
