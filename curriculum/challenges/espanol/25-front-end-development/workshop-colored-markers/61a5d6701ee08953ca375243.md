...
Como segundo argumento de color, pasa la función `hsl` con los valores `223` para el tono, `90%` para la saturación, y `60%` para el brillo.

# --hints--

Tu regla CSS `.blue` debe tener una propiedad `background` configurada en `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%))`.
