...
- Itera su ogni gate nell'array di input `gates`. Per ogni gate, aggiungi una proprietà a `summary` dove:
...
Chiamare `initializeThroughput()` con `[]` dovrebbe restituire `{}` senza modificare l'input sorgente.
...
Chiamare `initializeThroughput()` con `[{ id: "North", capacity: 5, queue: [1, 2, 3] }]` dovrebbe restituire `{ North: 0 }` senza modificare l'input sorgente.
...
Chiamare `initializeThroughput()` con `[{ id: "North", capacity: 5, queue: [] }, { id: "East", capacity: 3, queue: [] }, { id: "South", capacity: 4, queue: [] }]` dovrebbe restituire `{ North: 0, East: 0, South: 0}` senza modificare l'input sorgente.
...
Dopo il loop, dovresti restituire `summary`. Questa dovrebbe essere l'ultima riga della funzione.
