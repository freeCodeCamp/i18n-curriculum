...
يجب إن يكون المحدد `.cabin` الخاص بك يحمل خاصية `position`و تكون قيمة الخاصية `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cabin')?.position === 'absolute');
```

يجب إن يكون المحدد `.cabin` الخاص بك يحمل خاصية `border`و تكون قيمة الخاصية `2px solid`.
