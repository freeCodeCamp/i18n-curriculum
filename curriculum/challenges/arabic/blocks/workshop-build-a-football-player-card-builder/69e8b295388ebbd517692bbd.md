...
داخل عنصر `card-stats`، بعد أول `stat-col`، تحتاج إلى إضافة طفلين آخرين.

أولاً، عنصر `div` مغلق ذاتيًا مع `className` باسم `stat-divider` لعرض خط عمودي رفيع بين عمودين الإحصائيات، وعنصر `div` ثاني مع `className` باسم `stat-col` يحتوي على ثلاثة عناصر `div` أخرى، كل منها يحمل `className` باسم `stat-row`.

# --hints--

يجب إضافة عنصر `div` مغلق ذاتيًا مع `className` باسم `stat-divider` بعد أول `.stat-col`.

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

يجب إضافة عنصر `div` ثاني مع `className` باسم `stat-col` بعد `.stat-divider`.

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

يجب أن يحتوي `.stat-col` الثاني على ثلاثة عناصر `div`، كل منها يحمل `className` باسم `stat-row`.
