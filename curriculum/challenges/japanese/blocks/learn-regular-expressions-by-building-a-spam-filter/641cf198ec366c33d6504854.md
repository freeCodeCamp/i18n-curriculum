...
<dfn>代替シーケンス</dfn> `|` は、`|` の左側のテキストか右側のテキストのどちらかにマッチさせるために使えます。例えば、正規表現 `/yes|no/` は `yes` または `no` のどちらかにマッチします。

`helpRegex` を更新して、`please help` または `assist me` のどちらかにマッチさせてください。

# --hints--

`helpRegex` は `|` の代替シーケンスを使うべきです。
