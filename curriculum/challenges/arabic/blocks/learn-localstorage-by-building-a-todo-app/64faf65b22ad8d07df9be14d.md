...
أنشئ متغيرًا باسم `dataArrIndex` وضع قيمته باستخدام طريقة `findIndex()` على مصفوفة `taskData`. مرر `item` كمعلمة لدالة الاستدعاء الراجعة السهمية، وداخل الاستدعاء تحقق مما إذا كان `id` الخاص بـ `item` يساوي `id` الخاص بـ `parentElement` لـ `buttonEl`.
