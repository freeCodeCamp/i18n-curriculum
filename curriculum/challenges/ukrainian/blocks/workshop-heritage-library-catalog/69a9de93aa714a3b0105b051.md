...
Тепер об’єднайте всі рядки в один CSV-рядок, який можна експортувати або поділитися.

Після першого циклу оголосіть `let csv = header`. Додайте ще один цикл `for`, щоб додати кожен рядок до `csv` з переходом на новий рядок: `csv = csv + "\n" + rows[i]`. Поверніть `csv`. Потім, під функцією, виведіть у консоль `exportToCSV(catalog)`, щоб переглянути відформатований вміст.

# --hints--

`exportToCSV` має повертати рядок.

```js
assert.isString(exportToCSV(catalog));
```

Перший рядок повернутого рядка має бути `"Title,Author,Year,Location"`.

```js
assert.strictEqual(exportToCSV(catalog).split("\n")[0], "Title,Author,Year,Location");
```

Вихідні дані мають містити `"King, Stephen"` (у лапках, щоб обробити кому).

```js
assert.include(exportToCSV(catalog), '"King, Stephen"');
```

Ви маєте вивести `exportToCSV(catalog)` у консоль.
