...
`#text` 要素に `background-color` の `#0a0a23`、`color` の `#ffffff`、そして全方向にパディングを `10px` してください。
...
`#text` 要素は `background-color` の `#0a0a23` を持つべきです。
...
`#text` 要素は `color` の `#ffffff` を持つべきです。
...
`#text` 要素は全方向にパディングを `10px` してください。
