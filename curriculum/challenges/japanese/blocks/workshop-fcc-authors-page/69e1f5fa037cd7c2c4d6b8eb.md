...
最後に、`catch` メソッドを処理する必要があります。まず、`fetch` 文、`json` 文、および displayAuthors の呼び出しを `try` 内の `initialFetch` ブロックにネストしてください。それから、連鎖している `catch` メソッドを、以下の例に示すように catch ブロックに変更してください。
...
これを正しく行うと、`try` ブロックには3行のコードが含まれ、`catch` ブロックには1行のコードだけが含まれます。また、`SyntaxError` は解決され、アプリは正常に動作するはずです。

よく頑張りました！`async` と `await` 演算子を使って非同期コードをより読みやすくする方法を学びました！これで著者ページは完成です。
...
`try/catch` ブロックを作成してください

```js
assert.match(code, /try\s*\{[\s\S]*?\}\s*catch\s*\([^)]*\)\s*\{/);
```

`catch` はもはや連鎖したメソッドではないはずです

```js
assert.notMatch(code, /\.catch/);
```

`try` ブロックにはデータを取得し、`json` を作成し、それを表示する3つの文が含まれているはずです
...
fCC Authors ページが正しくレンダリングされているはずです
