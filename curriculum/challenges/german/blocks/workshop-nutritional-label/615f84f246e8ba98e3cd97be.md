...
Fügen Sie unter Ihrem `.double-indent`-Element ein neues `p`-Element mit dem Text `Includes 10g Added Sugars 20%` hinzu. Ihr neues `p`-Element sollte ebenfalls doppelt eingerückt sein und keinen unteren Rahmen haben. Verwenden Sie ein `span`, um den `20%` fett und rechtsbündig zu machen.

Erstellen Sie dann eine weitere Trennlinie nach diesem `p`-Element.

# --hints--

Sie sollten ein neues `p`- und `div`-Element am Ende Ihres `.daily-value`-Elements erstellen.
...
Ihr neues `p`-Element sollte den Text `Includes 10g Added Sugars 20%` enthalten.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Includes 10g Added Sugars[\s|\n]+20%/));
```

Ihr neues `p`-Element sollte das `class`-Attribut auf `double-indent no-divider` gesetzt haben.
