...
今のところ、曲の表示順は変わっていません。 ここまで記述してきた `sort` メソッドを使う処理は、`sortSongs` 関数が呼び出されるまで実行されないからです。

では `renderSongs` 関数の引数を変更して、`sortSongs` 関数を呼び出すようにしてください。

そうすると曲がアルファベット順で表示されるはずです。

# --hints--

`renderSongs(sortSongs())` を使用してください。
