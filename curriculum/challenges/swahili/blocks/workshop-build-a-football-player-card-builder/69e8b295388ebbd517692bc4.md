...
Bango la ngazi bado linaonyesha maandishi yaliyowekwa moja kwa moja `GOLD`. Unahitaji kubadilisha na usemi wa kihesabu unaobadilika ili ngazi sahihi ionyeshwe.

Fanya wito `getPlayerTier()` kwa `player.overallRating` kupata mfuatano wa herufi wa ngazi, kisha fanya wito `.toUpperCase()` kwenye matokeo ili kuionyesha kwa herufi kubwa. Usisahau kufunika usemi mzima wa kihesabu na mabano ya mviringo.

# --hints--

Unapaswa kutumia `getPlayerTier(player.overallRating).toUpperCase()` ndani ya `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

Kipengele cha `.card-tier-badge` kinapaswa kuonyesha `ELITE` kwa mchezaji mwenye kiwango cha 98.
