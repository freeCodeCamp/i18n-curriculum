...
Між селекторами `0%` та `100%` додайте селектор `50%`. Це буде застосовано в середині циклу анімації. Встановіть `background-color` на `purple`.

# --hints--

Ви повинні створити новий селектор `50%` в своєму правилі `@keyframes cabins`.
...
Ваш селектор `50%` повинен бути між селекторами `0%` та `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '50%');
```

Ваш селектор `50%` повинен мати властивість `background-color` зі значенням `purple`.
