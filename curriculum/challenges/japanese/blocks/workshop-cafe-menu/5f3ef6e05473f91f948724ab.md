...
テキストの`font-family`を変更して、ブラウザのデフォルトフォントとは違う見た目にできます。各ブラウザには共通して使えるフォントがいくつか用意されています。

`body`内のすべてのテキストを、値が`font-family`の`sans-serif`プロパティを追加して変更してください。これはかなり一般的で読みやすいフォントです。

# --hints--

`font-family`プロパティを`sans-serif`に設定してください。

```js
const hasFontFamily = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style['font-family'] === 'sans-serif');
```

`body`には`font-family`の`sans-serif`があるはずです。
