...
在该方法内，在之前，使用条件式表达式将字串 `Read` 赋值给变量 `status`（如果邮件已读取），如果未读取则赋值为 `Unread`。 

# --hints--

你应该在 `__str__` 方法中有一个 `status` 变量。
...
你的条件式应评估为字串 `Read`（如果邮件已读取）或 `Unread`（如果未读取）。
