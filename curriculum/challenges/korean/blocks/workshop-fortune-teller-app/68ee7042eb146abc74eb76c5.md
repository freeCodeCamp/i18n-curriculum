...
다시 카드가 무작위이고 뒤집히도록 만드세요. `isReversed`와 `card`라는 변수를 선언하세요. 첫 번째 변수에는 `Math.random` 메서드를 호출하고 `< 0.5`와 비교하세요. 두 번째 변수에는 `getRandomItem`를 호출하고 `this.cards`를 전달하세요.

# --hints--

`isReversed`라는 변수를 선언해야 합니다.
...
`isReversed` 변수는 카드가 뒤집혔는지 무작위로 결정하기 위해 `Math.random()`를 사용해야 합니다.
...
`card` 변수는 `getRandomItem`를 인수로 하여 `this.cards` 메서드를 호출한 결과로 할당해야 합니다.
