...
你應該將 `.arm.left` 的 `top` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.top, '35%');
```

你應該將 `.arm.left` 的 `left` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
...
你應該將 `.arm.right` 的 `top` 設置爲 `0%`。

```js
assert.include(['0%', '0', '0px'], new __helpers.CSSHelp(document).getStyle('.arm.right')?.top);
```

你應該將 `.arm.right` 的 `right` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
