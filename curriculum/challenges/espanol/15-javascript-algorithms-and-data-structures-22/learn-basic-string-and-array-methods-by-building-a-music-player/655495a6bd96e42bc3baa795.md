...
A continuación, necesitas mostrar el título de la canción actual y el artista en la pantalla del reproductor. Usa `const` y la sintaxis de flecha para crear una función vacía `setPlayerDisplay`.

# --hints--

Debes usar `const` para crear una función vacía llamada `setPlayerDisplay`.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` debe ser una función.

```js
assert.isFunction(setPlayerDisplay)
```

Tu función `setPlayerDisplay` debe usar una sintaxis de flecha.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

Tu función `setPlayerDisplay` debe estar vacía.
