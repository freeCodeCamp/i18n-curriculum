...
選擇排序依賴於跟蹤數組中最小值的索引。 聲明一個變量 `minIndex` 並將其設置爲 `i`——這確保瞭如果當前值是最小的，它將與自身交換而不會移動。 在迭代數組時，你需要能夠重新分配 `minIndex` 的值。

然後，寫另一個使用 `j` 作爲迭代器的 `for` 循環。 該循環需要從 `i` 之後的索引開始，並迭代數組的剩餘部分。

# --hints--

你應該使用 `let` 來聲明 `minIndex`。
