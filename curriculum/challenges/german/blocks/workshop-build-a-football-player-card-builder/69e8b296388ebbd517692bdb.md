...
Fügen Sie dem PAC `value` ein `player.pac`-Attribut mit dem Wert `input` hinzu. Fügen Sie dann ein `onChange`-Attribut hinzu, das auf eine Pfeilfunktion gesetzt ist, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `pac` auf `Number(e.target.value)` gesetzt ist.

# --hints--

Ihr PAC `input` sollte ein `value` haben, das dem `pac`-Wert des Spielers entspricht.
...
Ihr PAC `input` sollte einen `onChange`-Handler haben, der `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `pac` auf `Number(e.target.value)` gesetzt ist.
