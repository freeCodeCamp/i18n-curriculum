...
前のステップでは曲を再生する機能を作りました。次は再生ボタンに機能を追加する必要があります。

`addEventListener()`の`playButton`メソッドを使い、最初の引数に`"click"`イベントを渡してください。今は、2番目の引数に`playSong(0)`を呼び出すコールバックを使いましょう。

その後、再生ボタンをクリックして動作を確認してください。


# --hints--

`click`に`playButton`イベントのイベントリスナーを追加し、`playSong(0)`メソッドの2番目の引数に`addEventListener`を呼び出すコールバック関数を渡す必要があります。
