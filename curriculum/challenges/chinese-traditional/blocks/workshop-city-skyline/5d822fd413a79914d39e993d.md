...
在你的媒體查詢頂部為你的 `:root` 選擇器添加一個。然後在那裡重新定義所有四個 `--building-color` 變數以使用值 `#000`。

# --hints--

你應該為你的媒體查詢添加一個 `:root` 選擇器。
...
你應該為你的 `--building-color1` 添加值為 `#000`。
...
你應該為你的 `--building-color2` 添加值為 `#000`。
...
你應該為你的 `--building-color3` 添加值為 `#000`。
...
你應該為你的 `--building-color4` 添加值為 `#000`。
