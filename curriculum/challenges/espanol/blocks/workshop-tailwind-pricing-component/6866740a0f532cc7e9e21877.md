...
Ahora pasemos a la insignia `Most popular`. Necesitas colocarla en la esquina superior derecha de la tarjeta, así que puedes usar algunas de las clases de utilidad de posicionamiento de Tailwind para hacerlo.

Aquí tienes cómo usar la posición CSS en Tailwind:
...
Asigna la clase `relative` al `div` contenedor y `absolute` al `div` con el texto `Most popular`.

# --hints--

Tu elemento `div` contenedor para el plan premium debe tener la clase `relative`.
...
Tu elemento `div` con el texto `Most popular` debe tener la clase `absolute`.
