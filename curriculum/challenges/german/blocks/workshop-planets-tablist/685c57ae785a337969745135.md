...
Für den letzten Schritt müssen Sie das Panel sichtbar machen, wenn auf den zugehörigen Tab geklickt wird. Das erreichen Sie, indem Sie die `hidden`-Eigenschaft des `panel` auf `false` setzen.

Klicken Sie auf einen der Tabs, und Sie sollten den entsprechenden Panel-Inhalt angezeigt bekommen.

Und mit dieser letzten Änderung ist Ihr Workshop abgeschlossen!

# --hints--

Wenn auf einen Tab geklickt wird, sollten Sie die `hidden`-Eigenschaft des Panels auf `false` setzen.
