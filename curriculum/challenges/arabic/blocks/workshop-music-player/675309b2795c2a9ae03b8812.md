...
لاحظ أن صورة الألبوم في HTML والأغاني في مصفوفة `userData.songs` قد تغيرت. استبدلنا الأغاني الأصلية بأغاني أقصر يمكنك استخدامها لاختبار تطبيقك.

جميع الوظائف الأساسية موجودة الآن. المشكلة الوحيدة هي أن الأغنية التالية لا تُشغل تلقائيًا عند انتهاء تشغيل الأغنية الحالية.

لإصلاح ذلك، يمكنك إعداد مستمع حدث يكتشف متى تنتهي الأغنية الحالية. مستمع الحدث `"ended"` مناسب لهذا الغرض. يُطلق هذا الحدث عند انتهاء تشغيل الوسائط.

أضف مستمع حدث إلى عنصر `audio` يستمع إلى حدث `"ended"` ومرر مرجعًا إلى الدالة `playNextSong`. بذلك يكتمل مشروع مشغل الموسيقى!

# --hints--

يجب ربط طريقة `addEventListener()` بمتغير `audio` الخاص بك.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

يجب أن يستمع مستمع الحدث الذي استخدمته على متغير `audio` إلى حدث `"ended"`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

يجب أن تمرر مرجعًا إلى `playNextSong` إلى مستمع الحدث `"ended"`.
