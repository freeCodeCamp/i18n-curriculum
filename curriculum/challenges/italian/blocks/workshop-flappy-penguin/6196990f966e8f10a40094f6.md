...
Seleziona l'elemento `.penguin` e imposta il suo `width` e `height` su `300px`.
...
Dovresti dare a `.penguin` un `width` di `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.width, '300px');
```

Dovresti dare a `.penguin` un `height` di `300px`.
