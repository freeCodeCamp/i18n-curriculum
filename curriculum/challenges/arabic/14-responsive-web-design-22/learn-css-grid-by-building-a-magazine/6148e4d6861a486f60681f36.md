...
اعطي منتقي `.image-wrapper` الخاصية `grid-template-columns` بقيمة `2fr 1fr` و خاصية `grid-template-rows` بقيمة `repeat(3, min-content)`. هذا سيعطي الـ grid صفوف تعدل من ارتفاعها بناء علي المحتوى، ولكن أعمدة يظل عرضهاً ثابت بناء علي الحاوية (container).

# --hints--

يجب أن يحتوي منتقي `.image-wrapper` الجديد على خاصية `grid-template-columns` بقيمة `2fr 1fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns === '2fr 1fr');
```

يجب أن يكون لمنتقي `.image-wrapper` الخاص بك خاصية `grid-template-rows` بقيمة `repeat(3, min-content)`.
