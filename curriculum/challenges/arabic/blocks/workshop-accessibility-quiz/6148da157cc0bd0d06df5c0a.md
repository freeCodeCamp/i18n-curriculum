...
لمحاذاة صناديق الإدخال مع بعضها، أنشئ مجموعة قواعد جديدة تستهدف جميع عناصر `input` و`label` داخل عنصر `.info` واضبط خاصية `display`
على `inline-block`.

أيضًا، حاذِ نص عنصر الـ `label` إلى اليمين.

# --hints--

يجب أن تستخدم محددًا إما `.info > label, .info > input` أو `.info label, .info input`.
...
يجب أن تعطي عناصر الـ `input` و`label` خاصية `display` بقيمة `inline-block`.
...
يجب أن تعطي عنصر الـ `label` خاصية `text-align` بقيمة `right`.
