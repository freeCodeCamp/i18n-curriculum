...
では計算を始めましょう。 まず、摂取カロリーを表す `consumedCalories` という変数を宣言し、`breakfastCalories`、`lunchCalories`、`dinnerCalories`、`snacksCalories` の合計を代入してください (テストではこの順序もチェックします)。 これは必ず `if` 文の後に記述してください。

# --hints--

関数内に `consumedCalories` という変数が必要です。

```js
assert.match(calculateCalories.toString(), /consumedCalories\s*=/);
```

変数 `consumedCalories` は `if` 文の後に来る必要があります。

```js
assert.isAbove(calculateCalories.toString().indexOf('consumedCalories'), calculateCalories.toString().indexOf('if'));
```

`breakfastCalories`、`lunchCalories`、`dinnerCalories`、`snacksCalories` の合計をこの順番で計算してください。

```js
assert.match(calculateCalories.toString(), /breakfastCalories\s*\+\s*lunchCalories\s*\+\s*dinnerCalories\s*\+\s*snacksCalories\s*;?\s*$/m);
```

`breakfastCalories`、`lunchCalories`、`dinnerCalories`、`snacksCalories` の合計を `consumedCalories` に代入してください。
