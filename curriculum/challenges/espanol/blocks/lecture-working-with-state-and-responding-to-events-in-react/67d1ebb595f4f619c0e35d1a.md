...
React trata el estado como inmutable, lo que significa que no debe modificarlo directamente.
...
Esto es porque el nuevo objeto que pasaste a la función set solo contenía una clave/valor para `age`.

Para evitar que esto ocurra, puedes copiar primero el objeto existente y luego actualizar solo la propiedad que deseas actualizar, en este caso, `age`.
...
Para actualizar las propiedades restantes `name` y `city`, puedes escribirlas como funciones de configuración separadas y conectarlas a sus respectivas entradas:
...
Para que esto funcione, cada campo de entrada debe tener un atributo `name`.
...
Usa `Object.assign()` sin llamar al setter.
...
La función set actualiza la variable `state`.
