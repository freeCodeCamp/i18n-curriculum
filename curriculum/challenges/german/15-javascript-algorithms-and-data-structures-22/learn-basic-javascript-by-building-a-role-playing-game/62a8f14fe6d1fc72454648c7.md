...
Du solltest `monsterStats.style.display` in der `update`-Funktion verwenden.

```js
assert.match(update.toString(), /monsterStats\.style\.display/);
```

Du solltest `monsterStats.style.display` auf `none` setzen.

```js
assert.match(update.toString(), /monsterStats\.style\.display\s*=\s*('|")none\1/);
```

Du solltest deinen Code in der ersten Zeile der `update`-Funktion einfügen.
