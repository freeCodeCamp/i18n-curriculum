...
現在の曲のインデックスを取得するには、<dfn>indexOf()</dfn> メソッドを使用します。 配列メソッド `indexOf()` は、指定された要素が配列内で最初に見つかったインデックスを返し、該当する要素が存在しない場合は `-1` を返します。
...
では `getCurrentSongIndex` 関数内で、`userData?.songs.indexOf()` を返してください。 `indexOf()` の引数には `userData?.currentSong` を使用してください。

# --hints--

`getCurrentSongIndex` 関数内で、`userData?.songs.indexOf()` を返してください。

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

`indexOf()` メソッドに `userData?.currentSong` を渡してください。
