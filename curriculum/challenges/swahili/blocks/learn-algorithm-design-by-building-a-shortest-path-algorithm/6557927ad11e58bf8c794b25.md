...
Tengeneza mzunguko wa `for` rudia juu ya `targets_to_print` na chapisha f-string ifuatayo: `f'\n{start}-{node} distance: {distances[node]}\nPath: {" -> ".join(paths[node])}'`.

# --hints--

Unapaswa kutengeneza mzunguko wa `for` rudia juu ya `targets_to_print`. Tumia `node` kama kigezo cha mzunguko.
...
Unapaswa kuchapisha mfuatano wa herufi uliotolewa ndani ya mzunguko wako mpya wa `for`.
