...
`messageInput` が空の場合、ユーザーに `"Please enter a message."` というメッセージのアラートを表示してください。

そしてアラートの表示後、関数の実行を終了してください。

# --hints--

コールバック関数に `if` 文を持たせてください。

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

`if` 文で `messageInput` の `value` が空の文字列かどうかをチェックしてください。
...
`if` 文でユーザーに `"Please enter a message."` というメッセージのアラートを表示してください。
