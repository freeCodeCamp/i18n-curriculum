...
노래를 재생하기 전에 처음부터 시작하는지 확인해야 합니다. 이는 `currentTime` 객체의 `audio` 속성을 사용하여 달성할 수 있습니다.

현재 노래가 재생되고 있지 않은지 `if`가 falsy인지 확인하는 `userData.currentSong` 문을 추가하세요.
`if` 블록 안에서 `currentTime` 객체의 `audio` 속성을 `0`로 설정하세요.

# --hints--

`playSong` 함수는 현재 노래가 재생되고 있지 않을 때 `currentTime` 객체의 `audio` 속성을 `0`로 설정해야 합니다.
