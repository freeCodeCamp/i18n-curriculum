...
`textContent` 設置節點的文本並允許你設置或檢索 HTML 元素的文本內容。
...
你可以使用三元運算符有條件地設置文本內容值。 以下是將三元運算符的結果分配給變量的示例：

```js
const example = condition ? "I'm true" : "I'm false";
```

使用三元運算符檢查 `currentTitle` 是否計算爲真值。 如果是，則將 `playingSong.textContent` 設置爲 `currentTitle`。 否則，將其設置爲空字符串。

然後在下面使用 `ternary` 運算符來檢查 `currentArtist` 是否爲真。 如果是，則將 `songArtist.textContent` 設置爲 `currentArtist`。 否則，將其設置爲空字符串。

# --hints--

你應該將 `textContent` 屬性鏈接到 `playingSong`。

```js
assert.match(code, /playingSong\.textContent\s*/)
```

你應該使用 `ternary` 運算符將 `playingSong` 的 `textContent` 屬性設置爲 `currentTitle` 或 `""`。

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

你應該將 `textContent` 屬性鏈接到 `songArtist`。

```js
assert.match(code, /songArtist\.textContent\s*/)
```

你應該使用 `ternary` 運算符將 `songArtist` 的 `textContent` 屬性設置爲 `currentArtist` 或 `""`。
