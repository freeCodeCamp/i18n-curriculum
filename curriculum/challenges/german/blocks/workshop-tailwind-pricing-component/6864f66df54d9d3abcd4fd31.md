...
Geben Sie dem `span` innerhalb des `p`-Elements, das den Text `/month` enthält, die Klassen `text-base`, `font-medium` und `text-gray-500`. Dadurch wird es vom Preis selbst unterschieden, indem der Text heller dargestellt wird.

# --hints--

Ihr `span`-Element sollte die Klasse `text-base` haben.
...
Ihr `span`-Element sollte die Klasse `font-medium` haben.
...
Ihr `span`-Element sollte die Klasse `text-gray-500` haben.
