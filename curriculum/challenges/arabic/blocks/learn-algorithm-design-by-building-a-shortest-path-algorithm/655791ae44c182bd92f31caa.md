...
تُزيل الطريقة `.remove()` من القائمة أول عنصر مطابق يُمرر كمعلمة:
...
أنهِ حلقة `while` بإزالة العقدة الحالية من قائمة `unvisited`. انتبه إلى المسافة البادئة.

# --hints--

يجب استخدام الطريقة `.remove()` لإزالة العقدة الحالية من `unvisited` بعد حلقة `for`.
