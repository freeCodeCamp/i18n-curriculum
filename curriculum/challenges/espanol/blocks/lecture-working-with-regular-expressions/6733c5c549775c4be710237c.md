...
¡Oh no! `match()` solo devolvió la primera coincidencia, y `replace()` solo reemplazó la primera coincidencia. Esto se debe a que, por defecto, `match()` y `replace()` solo operan sobre la primera ocurrencia del patrón.
...
¡Buenas noticias! Nuestro `replaceAll()` funcionó exactamente como queríamos: reemplazó todas las ocurrencias de `freecodecamp` en minúsculas con la versión en camelcase correcta.

¿Pero qué es ese objeto vacío? Bueno, `matchAll()` devuelve un tipo especial de objeto llamado `Iterator`, que la consola de freeCodeCamp no está preparada para manejar.

Si echamos un vistazo en la consola de nuestro navegador, el `Iterator` tiene un método `next()`, que podemos llamar para obtener el siguiente valor:
...
¡Ahí está nuestro array de coincidencias! `next()` nos da un objeto con dos valores: `done`, que es `false` cuando hay más elementos disponibles en el iterador, y `value`, que es el valor sobre el que acabamos de iterar. Entonces, si lo llamamos una vez más:
...
Actualicemos nuestro código para usar eso: vamos a limpiar nuestras llamadas a `replaceAll` ya que sabemos que funcionan:
...
La lección menciona un modificador específico para la coincidencia global.
...
La lección describe un tipo especial de objeto que devuelve `matchAll()`.
...
La lección demuestra un método específico para convertir el `Iterator` en un arreglo.
