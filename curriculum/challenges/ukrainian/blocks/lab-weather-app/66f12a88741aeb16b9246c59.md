...
title: Створіть застосунок погоди
challengeType: 25
...
У цій лабораторній роботі ви створите застосунок, який надає інформацію про погоду в різних містах.

Ви будете використовувати API погоди. Вихідні дані мають такий формат:
...
1. Ви повинні мати елемент `button` з `id` рівним `get-weather-btn`.

1. Ви повинні мати елемент `select` з сімома вкладеними елементами `option`. Перший варіант повинен мати порожній рядок як текст і атрибут `value`. Решта повинні мати такі тексти та значення (значення у нижньому регістрі):
...
1. Якщо місто не вибране, натискання кнопки не повинно нічого робити.

1. Якщо місто вибране, повинні з’явитися елементи для відображення погоди:

- Ви повинні мати елемент `img` з ідентифікатором `weather-icon` для відображення іконки погоди.

- Ви повинні мати елемент з ідентифікатором `main-temperature` для відображення основної температури.

- Ви повинні мати елемент з ідентифікатором `feels-like` для відображення відчуття температури.

- Ви повинні мати елемент з ідентифікатором `humidity` для відображення рівня вологості повітря.

- Ви повинні мати елемент з ідентифікатором `wind` для відображення швидкості вітру.

- Ви повинні мати елемент з ідентифікатором `wind-gust` для відображення поривів вітру.

- Ви повинні мати елемент з ідентифікатором `weather-main` для відображення основного типу погоди.

- Ви повинні мати елемент з ідентифікатором `location` для відображення поточного місця розташування.

1. Ви повинні мати асинхронну функцію з назвою `getWeather`, яка отримує інформацію про погоду з API `https://weather-proxy.freecodecamp.rocks/api/city/<CITY>` і повертає її. Зверніть увагу, що цей API повертає дані у метричній системі, тобто м/с для швидкості вітру та градуси Цельсія для температури.

1. Асинхронна функція `getWeather` повинна приймати місто як аргумент.

1. Ви повинні обробляти будь-які помилки, що виникають у функції `getWeather`, і виводити їх у консоль.

1. Ви повинні мати асинхронну функцію `showWeather`, яка приймає місто як параметр.

1. Функція `showWeather` повинна викликати функцію `getWeather` для отримання даних про погоду для вибраного міста зі списку.

1. Якщо у функції `getWeather` сталася помилка, застосунок повинен показати лише сповіщення з текстом `Something went wrong, please try again later`.

1. Якщо дані з `getWeather` придатні для використання, функція `showWeather` повинна відобразити дані про погоду у відповідних елементах. Якщо певне значення з API є `undefined`, ви повинні записати `N/A` у відповідний елемент.

1. Функції `getWeather` та `showWeather` не слід оголошувати за допомогою `const`, оскільки тести потребують можливості їх переназначення.

**Примітка:** Функції `getWeather` та `showWeather` не слід оголошувати за допомогою `const`, оскільки тести потребують можливості їх переназначення. Також тести займатимуть деякий час, тому поки ви бачите в консолі `// running tests`, вони виконуються.
...
Ви повинні мати елемент `button` з `id` рівним `get-weather-btn`.

```js
assert.exists(document.querySelector('button#get-weather-btn'));
```

У вас має бути елемент `select`.

```js
assert.exists(document.querySelector('select'));
```

Всередині елемента `select` першим дочірнім елементом повинен бути елемент `option` з порожнім атрибутом `value`.
...
Всередині елемента `select` має бути 6 елементів `option`, по одному для кожного з наступних міст: Paris, London, Tokyo, Los Angeles, Chicago, New York.
...
Ви повинні мати функцію `showWeather`.

```js
assert.isFunction(showWeather);
```

Ви повинні мати функцію `getWeather`.

```js
assert.isFunction(getWeather);
```

Функція `getWeather` повинна приймати місто як єдиний аргумент і повертати JSON з API погоди.
...
Функції `getWeather` та `showWeather` не слід оголошувати за допомогою `const`.
...
Функція `showWeather` повинна викликати функцію `getWeather` для отримання даних про погоду.
...
Функція `showWeather` повинна обробляти випадок, коли `getWeather` повертає `undefined`.
...
Коли вибрано New York, функція `showWeather` повинна відобразити дані з API у відповідних HTML-елементах. Якщо значення з API є `undefined`, ви повинні показати `N/A`.
...
Коли вибрано Chicago, функція `showWeather` повинна відобразити дані з API у відповідних HTML-елементах. Якщо значення з API є `undefined`, ви повинні показати `N/A`.
...
Коли вибрано London, функція `showWeather` повинна відобразити дані з API у відповідних HTML-елементах. Якщо значення з API є `undefined`, ви повинні показати `N/A`.
...
Коли вибрано Tokyo, функція `showWeather` повинна відобразити дані з API у відповідних HTML-елементах. Якщо значення з API є `undefined`, ви повинні показати `N/A`.
...
Коли вибрано Los Angeles, функція `showWeather` повинна відобразити дані з API у відповідних HTML-елементах. Якщо значення з API є `undefined`, ви повинні показати `N/A`.
...
Якщо сталася помилка, ваша функція `getWeather` повинна вивести помилку у консоль.
...
Коли вибрано Paris, застосунок повинен показати сповіщення з текстом `Something went wrong, please try again later`.
