...
`.image-wrapper` 요소 안에서 세 번째 `img` 요소에 `src` 값을 가진 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`를, `alt` 값을 가진 `four people working on code`를, `loading` 값을 가진 `lazy` 속성을, `class`로 설정된 `image-3`를, `width`로 설정된 `600` 속성을, 그리고 `height`로 설정된 `400` 속성을 각각 지정하세요.

# --hints--

세 번째 `img` 요소는 `src`로 설정된 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`를 가져야 합니다.
