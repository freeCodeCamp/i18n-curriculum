...
`To ensure` means to make certain that something happens or is achieved. Por ejemplo:

`We must ensure that all safety regulations are followed.` - This means to make sure that the regulations are adhered to.

`To be on track` means to be progressing as planned or expected. Por ejemplo:
