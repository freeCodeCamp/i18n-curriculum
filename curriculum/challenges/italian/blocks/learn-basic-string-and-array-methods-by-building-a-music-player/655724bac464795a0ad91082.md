...
Ora devi verificare se la playlist è vuota. Se lo è, dovresti resettare l'oggetto `userData` al suo stato originale.

Usa un'istruzione `if` per controllare se `userData?.songs` ha una lunghezza pari a `0`.

# --hints--

Dovresti creare un'istruzione `if` vuota con la condizione `userData?.songs.length === 0`.
