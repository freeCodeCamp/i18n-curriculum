...
Kwa sasa, filamu na mfululizo wa televisheni zinaonyeshwa pamoja wakati unachapisha orodha ya vitu. Katika hatua zinazofuata, utahakikisha vitu vinachujwa katika makundi mawili ili kuonyeshwa tofauti.

Ndani ya darasa la `MediaCatalogue`, tengeneza njia iitwayo `get_movies` na ifanye irudishe orodha yenye tu matukio ya darasa la mzazi `Movie` yanayopatikana katika `self.items`.

Tumia mbinu unayopendelea kufanya hivyo, lakini kumbuka kutumia kitendakazi cha `type` kwa sababu `isinstance` haitakuwa na uwezo wa kutofautisha kati ya madarasa ya mzazi na watoto.

# --hints--

Darasa lako la `MediaCatalogue` linapaswa kuwa na njia iitwayo `get_movies`.
...
Njia yako ya `get_movies` inapaswa kurudisha orodha yenye vitu vyote vilivyoko katika `self.items` ambavyo ni matukio ya darasa la `Movie`.
