...
Dovresti dare a `.eye-lid` un valore `top` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.top, '25%');
```

Dovresti dare ad `.eye-lid` un valore `left` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.left, '-23%');
```

Dovresti dare a `.eye-lid` un valore `border-radius` di `--fcc-expected--` invece di `--fcc-actual--`.
