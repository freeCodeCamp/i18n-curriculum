...
Jetzt müssen Sie das `aria-current`-Attribut zum aktuell abgespielten Lied hinzufügen.

Prüfen Sie, ob `songToHighlight` existiert. Falls ja, verwenden Sie die `setAttribute`-Methode auf `songToHighlight` und übergeben Sie `"aria-current"` und `"true"` als erstes und zweites Argument.

# --hints--

`highlightCurrentSong` sollte das `aria-current`-Attribut zum aktuell abgespielten Lied hinzufügen und das Attribut vom zuvor abgespielten Lied entfernen.
