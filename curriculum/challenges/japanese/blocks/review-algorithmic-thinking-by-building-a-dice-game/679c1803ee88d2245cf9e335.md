...
最後に、コードを少しリファクタリングして整理しましょう！`getHighestDuplicates`、`detectFullHouse`、`checkForStraights` の各関数内で、最後のラジオボタン（「None of the Above」）を何度も有効にしていることに気づくかもしれません。コードをリファクタリングして、最後のラジオボタンを各関数の最後で有効にするのではなく、`rollDiceBtn` の中で一度だけ有効にするようにしてください。

# --hints--

`getHighestDuplicates` 関数は、重複が出た場合でも最後のラジオボタンを有効にしたり、最後の `span` テキストを設定したりしてはいけません。
...
`detectFullHouse` 関数は、フルハウスが出た場合でも最後のラジオボタンを有効にしたり、最後の `span` テキストを設定したりしてはいけません。
...
`checkForStraights` 関数は、ストレートが出た場合でも最後のラジオボタンを有効にしたり、最後の `span` テキストを設定したりしてはいけません。
...
`0` がクリックされたときに、最後のラジオボタンを有効にし、値を `, score = 0` に設定し、表示されるテキストを `rollDiceBtn` に更新してください。
