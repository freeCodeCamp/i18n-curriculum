...
Nel blocco dell'istruzione `if`, restituisci un oggetto che unisca separatamente sia `defaultPlayer` sia `JSON.parse(saved)`. Assicurati che siano separati da una virgola, dato che sono due oggetti distinti uniti in uno solo.

# --hints--

Il tuo blocco `if (saved)` dovrebbe restituire un oggetto che unisca sia `defaultPlayer` sia i dati salvati analizzati.
