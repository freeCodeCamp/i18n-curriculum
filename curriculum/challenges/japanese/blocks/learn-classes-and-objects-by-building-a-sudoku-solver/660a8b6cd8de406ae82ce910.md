...
現在の`if`の中に`if`文をネストしてください。`if`の条件には、`solver()`を再帰的に呼び出し、新しい`True`の本体から`if`を返してください。

# --hints--

現在の`if`文の中に`if`文をネストし、条件として`self.solver()`を使う必要があります。
