...
今のところは、`#monsterStats` の要素を `display` プロパティを使って非表示にしてください。 他のスタイルは変更しないでください。

# --hints--

`#monsterStats` セレクターが必要です。
...
`#monsterStats` セレクターの `display` プロパティを `none` に設定してください。
...
`#monsterStats` セレクターの `border` を `1px solid #0a0a23` に設定してください。
...
`#monsterStats` セレクターには `5px` のパディングが必要です。
...
`#monsterStats` セレクターの `color` を `#ffffff` に設定してください。
...
`#monsterStats` セレクターの `background-color` を `#c70d0d` に設定してください。
