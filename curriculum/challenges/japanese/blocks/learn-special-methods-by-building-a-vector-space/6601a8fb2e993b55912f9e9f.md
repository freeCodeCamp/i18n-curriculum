...
2つの3次元ベクトル \\( \mathbf{a} \\) と \\( \mathbf{b} \\) のクロス積は、次のように計算できます。

\\[ \mathbf{a} \times \mathbf{b} = \begin{pmatrix} a_yb_z - a_zb_y \\\ a_zb_x - a_xb_z \\\ a_xb_y - a_yb_x \end{pmatrix}
 \\]
 
結果のベクトルは列ベクトルとして表されます。
 
上記の式を実装して、2つの3次元ベクトルのクロス積を計算し、`cross()` メソッドから結果のベクトルを返してください。

# --hints--

`cross()` メソッドは、クロス積の計算結果として新しい `R3Vector` インスタンスを返す必要があります。
