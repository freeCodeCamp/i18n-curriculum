...
イベントリスナー内で、`result` 要素のテキストを更新する必要があります。この作業には三項演算子を使うことができます。

以下は、三項演算子の結果を要素のテキストコンテンツに代入する例です。

```js
el.textContent = condition ? "Use this text if the condition is true" : "Use this text if the condition is false";
```

`if` 文の後に、`isSpam()` を引数として `messageInput.value` を呼び出した結果の真偽値を三項演算子でチェックしてください。真の場合は、`textContent` 要素の `result` プロパティに `"Oh no! This looks like a spam message."` を代入します。そうでなければ `"This message does not seem to contain any spam."` を代入してください。

その後、`messageInput` 要素の `value` プロパティを空文字に設定してください。

# --hints--

`textContent` 要素の `result` プロパティを設定するには代入演算子を使う必要があります。
...
代入演算子 `isSpam()` の後、三項演算子 `=` の前に `?` 関数を呼び出す必要があります。
...
`isSpam(messageInput.value)` の真偽値をチェックするには三項構文を使う必要があります。
...
三項演算子の真の場合の式は、`textContent` 要素の `result` プロパティに `"Oh no! This looks like a spam message."` を代入してください。
...
三項演算子の偽の場合の式は、`textContent` 要素の `result` プロパティに `"This message does not seem to contain any spam."` を代入してください。
...
三項演算子の後に、`value` 要素の `messageInput` プロパティを空文字に設定してください。
