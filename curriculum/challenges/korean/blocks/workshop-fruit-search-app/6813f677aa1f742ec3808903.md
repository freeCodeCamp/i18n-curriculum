...
`catch` 블록에 `error` 매개변수를 추가한 다음, 데이터를 가져오거나 처리하는 동안 발생하는 모든 오류를 `console.error`로 기록하세요.

# --hints--

`catch` 블록에는 `error` 매개변수가 있어야 합니다.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

`catch` 블록에서 발생하는 모든 오류를 `console.error`로 기록해야 합니다.
