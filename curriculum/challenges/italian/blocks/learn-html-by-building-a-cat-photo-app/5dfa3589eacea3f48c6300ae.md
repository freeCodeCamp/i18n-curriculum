...
L'elemento `h2` dovrebbe avere un tag di apertura. Opening tags have this syntax: `<elementName>`.

```js
assert(document.querySelectorAll('h2').length === 2);
```

L'elemento `h2` dovrebbe avere un tag di chiusura. Closing tags have a `/` just after the `<` character.
