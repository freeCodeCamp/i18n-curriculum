...
Além do CTA `Go Premium`, o restante do conteúdo está em um `div`. Isso faz do CSS grid uma boa opção para organizar tudo.

Adicione as classes `grid`, `grid-rows-[auto_auto_auto_1fr]` e `gap-y-2` ao `div` que envolve os elementos `h2`, `p` e `ul`.

# --hints--

Seu elemento `div` que envolve os elementos `h2`, `p` e `ul` deve ter a classe `grid`.
...
Seu elemento `div` que envolve os elementos `h2`, `p` e `ul` deve ter a classe `grid-rows-[auto_auto_auto_1fr]`.
...
Seu elemento `div` que envolve os elementos `h2`, `p` e `ul` deve ter a classe `gap-y-2`.
