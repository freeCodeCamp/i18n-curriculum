...
Para corregir el error, cree una función de flecha `handlePowersChange` con un parámetro `e`.

Dentro de la función, desestructure `value` y `checked` de `e.target` para obtener el valor de la casilla de verificación y si está marcada o no.

# --hints--

Debe crear una función `handlePowersChange` con un parámetro `e`.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

Debe desestructurar `value` y `checked` de `e.target`.
