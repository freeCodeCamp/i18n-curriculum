...
視覚的アクセシビリティの話題では、要素間のコントラストが重要な要素です。例えば、見出しのテキストと背景のコントラスト比は少なくとも4.5:1であるべきです。

リスト要素内のすべてのアンカー要素のフォントカラーを、コントラスト比が少なくとも7:1になるように変更してください。

# --hints--

セレクターはリスト項目内のリンクをターゲットにするべきです。
...
リスト項目内の`a`要素に`color`プロパティを付与してください。
...
`color`プロパティには背景とのコントラスト比が少なくとも7:1になるようにしてください。_ヒント: `#dfdfe2`を使うと良いでしょう_
