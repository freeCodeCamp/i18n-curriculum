...
现在添加一个 `div`，其 `className` 为 `form-group`。在其中，添加一个 `label`，其 `className` 为 `label`，并且有一个 `htmlFor` 属性为 `name`。在其下方，添加一个 `input`，其 `id` 设为 `name`，`className` 为 `input`，以及一个 `type` 属性为 `text`。
