...
Crea un nuovo selettore `.left-container p` impostando il margine superiore e inferiore su `-5px` e il margine sinistro e destro su `-2px`. Imposta anche la `font-size` su `2em` e la `font-weight` su `700`.
