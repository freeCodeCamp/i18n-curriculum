...
你使用 `table-wrap` 类将表格包裹在了容器中。 为该类创建一个选择器，并将其 `padding` 设置为 `0 0.75rem 1.5rem 0.75rem`。

# --hints--

你应该有一个 `.table-wrap` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.table-wrap'));
```

你的 `.table-wrap` 选择器应该有一个 `padding` 属性，设置为 `0 0.75rem 1.5rem 0.75rem`。
