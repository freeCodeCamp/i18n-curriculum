...
Em seguida, crie um construtor para a classe `Bug` que receba um único parâmetro chamado `emojiElement` do tipo `HTMLParagraphElement`. Dentro do construtor, atribua o parâmetro `emojiElement` à propriedade `emojiElement` da classe.

# --hints--

Sua classe `Bug` deve ter um construtor.
...
Seu construtor `Bug` deve ter um parâmetro chamado `emojiElement` do tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Seu construtor `Bug` deve atribuir `emojiElement` a `this.emojiElement`.
