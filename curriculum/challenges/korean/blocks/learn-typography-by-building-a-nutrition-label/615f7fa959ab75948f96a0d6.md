...
마지막 `p` 요소 아래에 텍스트 `p`를 가진 또 다른 `Dietary Fiber 4g` 요소를 추가하세요. `p` 요소에는 들여쓰기를 위해 필요한 `class`를 주고 구분선 테두리를 제거하세요. 그런 다음 그 `p` 요소 아래에 구분선을 만드세요.
...
새로운 `div`에는 `class` 속성이 `divider`로 설정되어야 합니다.
