...
ループは完了しました。関数から `grouped` を返してください。

次に、関数の下で `groupByDecade(catalog)` を呼び出し、その結果を `byDecade` という名前の変数に代入してください。グループ化された結果を調べるために `byDecade` をログに出力すると、`"1950s"`、`"1960s"`、`"1970s"` のようなキーを持つオブジェクトが表示され、それぞれのキーがその年代の本の配列を指しているはずです。

# --hints--

`groupByDecade` はオブジェクトを返す必要があります。

```js
assert.isObject(groupByDecade(catalog));
```

`byDecade["1980s"].length` は `5` である必要があります。

```js
assert.strictEqual(groupByDecade(catalog)["1980s"].length, 5);
```

`byDecade["Unknown"].length` は `1` である必要があります。

```js
assert.strictEqual(groupByDecade(catalog)["Unknown"].length, 1);
```

`groupByDecade(catalog)` の結果を `byDecade` という名前の変数に代入してください。

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+byDecade\s*=\s*groupByDecade\s*\(\s*catalog\s*\)/);
```

`byDecade` をコンソールにログ出力してください。
