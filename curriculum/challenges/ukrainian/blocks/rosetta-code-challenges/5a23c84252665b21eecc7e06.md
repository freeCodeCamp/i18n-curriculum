...
Дано прямокутник, утворений із *m* × *n* квадратів. Якщо *m* і *n* не є обома непарними, тоді можливо прорізати шлях через прямокутник уздовж країв квадратів так, щоб прямокутник розділився на дві зв’язані частини з однаковою формою (після повороту однієї з частин на 180°). Усі такі шляхи для прямокутників 2 × 2 і 4 × 3 показані нижче.
...
Напишіть функцію, яка обчислює кількість різних способів розрізати прямокутник розміром *m* × *n*.
...
`cutRectangle(2, 2)` має повертати число.

```js
assert(typeof cutRectangle(2, 2) == 'number');
```

`cutRectangle(2, 2)` має повертати `2`.

```js
assert.equal(cutRectangle(2, 2), 2);
```

`cutRectangle(4, 3)` має повертати `9`.

```js
assert.equal(cutRectangle(4, 3), 9);
```

`cutRectangle(4, 4)` має повертати `22`.

```js
assert.equal(cutRectangle(4, 4), 22);
```

`cutRectangle(8, 3)` має повертати `53`.

```js
assert.equal(cutRectangle(8, 3), 53);
```

`cutRectangle(7, 4)` має повертати `151`.
