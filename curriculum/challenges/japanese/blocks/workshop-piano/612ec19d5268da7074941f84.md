...
論理演算子は、より複雑なメディアクエリを作成するために使えます。`and` 論理演算子は、2つのメディア条件をクエリするために使われます。

例えば、表示幅が500pxから1000pxの間を対象とするメディアクエリは次のようになります。
...
ブラウザのウィンドウ幅が`@media`より広く、`769px`より小さい場合に適用されるように、もう1つ`1199px`ルールを追加してください。
...
あなたの`@media`クエリは、`min-width`の`769px`と`max-width`の`1199px`を持つべきです。
