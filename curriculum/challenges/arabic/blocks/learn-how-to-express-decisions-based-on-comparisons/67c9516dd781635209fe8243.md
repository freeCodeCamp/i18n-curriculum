---
id: 67c9516dd781635209fe8243
title: المهمة 21
challengeType: 22
dashedName: task-21
lang: en-US
---

<!-- (audio) Sophie: SecureSoft does have a nicer interface, but if SafeGuard is more secure, it's probably the better choice. -->

# --instructions--

استمع إلى الصوت وأكمل الجملة أدناه.

# --fillInTheBlank--

## --sentence--

`SecureSoft does have a BLANK BLANK, but if SafeGuard is more secure, it's probably the better choice.`

## --blanks--

`nicer`

### --feedback--

هذا هو الشكل المقارن لـ `nice`، ويعني أكثر جاذبية بصريًا أو أكثر إرضاءً.

---

`interface`

### --feedback--

يشير هذا إلى تخطيط وتصميم برنامج برمجي يتفاعل معه المستخدمون.

# --explanation--

مصطلح `Nicer interface` يعني تصميم برنامج يبدو أفضل وأسهل في الاستخدام مقارنةً بآخر. على سبيل المثال:

`The new app has a nicer interface than the old version, with better colors and simpler navigation.` - هذا يعني أن التصميم الجديد للتطبيق أكثر جاذبية بصريًا وأسهل في الاستخدام.

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Sophie",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_13-1.mp3",
      "startTime": 1,
      "startTimestamp": 28.78,
      "finishTimestamp": 33.98
    }
  },
  "commands": [
    {
      "character": "Sophie",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sophie",
      "startTime": 1,
      "finishTime": 6.2,
      "dialogue": {
        "text": "SecureSoft does have a nicer interface, but if Safeguard is more secure, it's probably the better choice.",
        "align": "center"
      }
    },
    {
      "character": "Sophie",
      "opacity": 0,
      "startTime": 6.7
    }
  ]
}
```
