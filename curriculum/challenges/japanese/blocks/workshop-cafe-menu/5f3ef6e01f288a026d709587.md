...
異なる内容のセクション間に区切りを表示するには、`hr` 要素を使うことができます。
...
まず、クラス `hr` を持つ `p` 要素と最初の `established` 要素の間に `section` 要素を追加してください。

`hr` 要素は空要素であることに注意してください。
...
`hr` 要素は空要素なので、終了タグ `</hr>` を付けてはいけません。

```js
assert.notMatch(code, /<\/hr>/i);
```

クラス `p` を持つ既存の `established` 要素は変更しないでください。
...
`hr` 要素は `p` 要素と `section` 要素の間に配置してください。
