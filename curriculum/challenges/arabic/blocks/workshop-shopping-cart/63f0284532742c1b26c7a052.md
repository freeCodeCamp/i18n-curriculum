...
أضف دالة الاستدعاء الراجع إلى طريقة `forEach`. يجب أن تأخذ معلمة `btn`. ثم، في دالة الاستدعاء الراجع، أضف مستمع حدث إلى `btn`. يجب أن يستمع مستمع الحدث إلى حدث `click`، ويأخذ دالة استدعاء راجع أخرى بمعلمة `event`. يجب أن تكون دالة الاستدعاء الراجع الثانية فارغة.

# --hints--

يجب أن تستخدم صيغة السهم لإضافة دالة استدعاء راجع إلى طريقة `forEach` التي تأخذ معلمة `btn`.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(\s*(\(\s*btn\s*\)|btn)\s*=>\s*(?:\{|\s*)/);
```

يجب أن تضيف مستمع حدث إلى المتغير `btn`.
...
يجب أن تستمع إلى حدث `click` على المتغير `btn`.
...
يجب أن تضيف دالة استدعاء راجع فارغة إلى مستمع الحدث. تذكر أن تعطيها معلمة `event`.
