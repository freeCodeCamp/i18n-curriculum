...
1. 你應該有一個帶有 `form-container` 類別的 `div` 來容納你的表單。

2. 你應該在 `.form-container` 元素內有一個 `form` 元素。

3. 在 `form` 元素內，你應該有以下元素和輸入欄位：

   - 一個帶有一些文字的 `h2` 元素。
   - 一個名稱的文字輸入欄，`type` 設為 `text`，並包含 `id`、`name`、`required` 屬性。
   - 一個電子郵件輸入欄，其 `type` 設為 `email`，並包含 `id`、`name`、`required` 屬性。
   - 一個帶有 `id`、`name` 和 `required` 屬性的訊息文字區域。
   - 一個 `button` 元素，文字為 `"Submit"`，且具有 `type` 屬性為 `submit`。

4. 每個 `input` 和 `textarea` 元素都應該有對應的 `label` 元素，且其 `for` 屬性設定為該元素的 `id`。

5. `.form-container` 元素應該有背景色，並且為 `border-radius`、`padding` 和 `width` 設定值。

6. `label` 元素應該有邊距和字型顏色。

7. `input` 和 `textarea` 元素應該有以下的值：
...
9.  按鈕應該有一個暫留效果，會改變背景顏色。

**注意：** 請務必在你的 HTML 中聯結你的樣式表並套用你的 CSS。

# --hints--

你應該有一個帶有 `form-container` 類別的 `div`。
...
你應該在 `.form-container` 元素內有一個 `form` 元素。
...
在 `form` 中，你應該有一個文字輸入欄，且型別設定為 `text`。
...
你的 `type=text` 輸入欄位應該有 `id`、`name` 和 `required` 屬性。
...
在 `form` 中，你應該有一個電子郵件輸入欄，且 `type` 設定為 `email`。
...
你的 `type=email` 輸入欄位應該有 `id`、`name` 和 `required` 屬性。
...
在 `form` 中，你應該有一個帶有 `id`、`name` 和 `required` 屬性的 `textarea`。
...
在 `form` 中，你應該有一個 `button` 元素，其 `type` 屬性為 `submit`。
...
你應該為 `#name` 元素設置一個 `label` 元素，其 `for` 屬性與對應 `input` 上的 `id` 值相符。
...
你應該為 `#email` 元素設置一個 `label` 元素，其 `for` 屬性與對應 `input` 上的 `id` 值相符。
...
你應該為 `#message` 元素準備一個 `label` 元素，其 `for` 屬性與 `textarea` 上的 `id` 值相符。
...
你的 `button` 元素應該有一個暫留效果，可以改變背景顏色。
