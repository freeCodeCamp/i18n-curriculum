...
Fügen Sie das `astronaut`-Objekt in das `crew`-Array ein und geben Sie `Added ${astronaut.name} as ${astronaut.role}` in der Konsole aus. Sie können entweder Template-Literale oder String-Verkettung für die Ausgabemeldung verwenden.

# --hints--

Sie sollten das `astronaut` in das `crew`-Array einfügen.
...
Sie sollten den `name` des Astronauten im folgenden Format `Added {astronaut.name} as {astronaut.role}` in der Konsole ausgeben.
