...
創建一個 `dataArrIndex` 變量並使用 `taskData` 數組上的 `findIndex()` 方法設置其值。 將 `item` 作爲箭頭回調函數的參數傳遞，並在回調中檢查 `item` 的 `id` 是否等於 `buttonEl` 的 `parentElement` 的 `id`。
