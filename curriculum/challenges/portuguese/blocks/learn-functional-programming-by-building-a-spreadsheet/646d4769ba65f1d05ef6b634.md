...
Agora, a função `update()` pode realmente avaliar fórmulas.Lembre-se de que você escreveu a condição `if` para verificar se uma função foi chamada.

Dentro da instrução `if`, defina `value` em `element` como o resultado da função `evalFormula()`.Não passe nenhum argumento ainda.

# --hints--

Você deve atualizar a propriedade `value` de `element` no bloco `if`.
...
Você deve atribuir a `value` o resultado da chamada da função `evalFormula()`.
...
Você não deve passar argumentos argumento para a chamada de `evalFormula()`.
