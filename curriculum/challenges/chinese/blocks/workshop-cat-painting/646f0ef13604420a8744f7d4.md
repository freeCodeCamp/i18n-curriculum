...
在 `.cat-whiskers` 元素内，创建两个具有类 `cat-whiskers-left` 和 `cat-whiskers-right` 的 `div` 元素。

# --hints--

你不应该更改现有的类为 `cat-whiskers` 的 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers'), 1);
```

你应该在 `.cat-whiskers` 元素中放置两个 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers div'), 2);
```

第一个 `div` 元素在 `.cat-whiskers` 元素内，并且具有类 `cat-whiskers-left`。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers div')[0].classList.contains('cat-whiskers-left'));
```

第二个 `div` 元素应位于 `.cat-whiskers` 元素内，并且具有类 `cat-whiskers-right`。
