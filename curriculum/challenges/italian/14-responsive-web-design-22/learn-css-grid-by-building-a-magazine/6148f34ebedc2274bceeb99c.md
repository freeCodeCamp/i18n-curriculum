...
Inizia con una `@media` query per `only screen` e una `max-width` di `720px`. All'interno, crea un selettore `.image-wrapper` e assegnagli una proprietà `grid-template-columns` di `1fr`.

Le tre immagini verranno visualizzate in una colonna su schermi più piccoli del valore specificato.
...
La nuova regola `@media` dovrebbe avere un selettore `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

Il nuovo selettore `.image-wrapper` dovrebbe avere una proprietà `grid-template-columns` di `1fr`.
