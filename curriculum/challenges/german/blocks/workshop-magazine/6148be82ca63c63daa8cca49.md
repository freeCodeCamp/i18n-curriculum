...
Erstellen Sie einen `.author`-Selektor und geben Sie ihm eine `font-size`-Eigenschaft mit dem Wert `2rem` sowie eine `font-family`-Eigenschaft mit dem Wert `Raleway` und einem Fallback von `sans-serif`.

# --hints--

Sie sollten einen `.author`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.author'));
```

Ihr `.author`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `2rem` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize, '2rem');
```

Ihr `.author`-Selektor sollte eine `font-family`-Eigenschaft mit dem Wert `Raleway` und einem Fallback von `sans-serif` haben.
