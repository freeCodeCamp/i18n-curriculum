...
创建一个 `.author` 选择器，并给它一个 `font-size` 属性设置为 `2rem`，一个 `font-family` 属性设置为 `Raleway`，并以 `sans-serif` 作为备用字体。
...
你的 `.author` 选择器应该有一个 `font-size` 属性，设置为 `2rem`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

你的 `.author` 选择器应该有一个 `font-family` 属性，设置为 `Raleway` 并以 `sans-serif` 作为备用字体。
