...
現在你可以構建 `attack` 函數。 首先，將 `text` 消息更新爲 `"The <monster name> attacks."`，將 `<monster name>` 替換爲怪物的名稱。 請記住，你可以爲此使用連接運算符。

# --hints--

你應該使用點符號來訪問 `text` 的 `innerText` 屬性。

```js
assert.match(attack.toString(), /text\.innerText/);
```

你應該將字符串 `The` 分配給 `text` 的 `innerText` 屬性。

```js
assert.match(attack.toString(), /text\.innerText\s*=\s*('|")The \1/);
```

你應該使用連接運算符將 `monsters[fighting].name` 的值添加到 `The` 字符串。

```js
assert.match(attack.toString(), /text\.innerText\s*=\s*('|")The \1\s*\+\s*monsters\s*\[\s*fighting\s*\]\s*\.name/);
```

你應該使用連接運算符將字符串 `attacks.` 添加到 `monsters[fighting].name` 字符串。
