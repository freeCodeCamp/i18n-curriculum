...
我們已將其餘歌曲添加到 `allSongs` 數組中。

接下來，你將瞭解 <dfn>Web Audio API</dfn> 以及如何使用它來播放歌曲。 所有現代瀏覽器都支持 Web Audio API，它允許你在 Web 應用程序中生成和處理音頻。

使用 `const` 創建一個名爲 `audio` 的變量，並將其設置爲等於 `new Audio()`。 這將創建一個新的 HTML5 `audio` 元素。

# --hints--

你應該使用 `new` 關鍵字來創建 `Audio` 對象的實例。

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

你應該將 `Audio` 對象分配給名爲 `audio` 的常量。
