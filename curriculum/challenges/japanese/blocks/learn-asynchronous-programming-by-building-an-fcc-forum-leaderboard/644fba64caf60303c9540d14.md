...
`href` 属性の後に、`class` という名前の定数を `linkClass` 属性に設定してください。

`class` 属性の後に、`target` 属性を `"_blank"` に設定してください。

最後に、リンク内のテキストを表示するために、アンカー タグの間に `linkText` 定数を配置してください。

# --hints--

`class` 属性の後に `href` 属性があるはずです。
...
`class` 属性の値を `linkClass` という名前の定数に設定してください。
...
アンカー タグの間に `linkText` 定数を配置してください。
