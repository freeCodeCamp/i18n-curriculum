...
يجب عليك استخدام منتقي `.blush.left`.

```js
assert.match(code, /\.blush\.left\s*\{/);
```

يجب أن يحتوي منتقي `.blush.left` على خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush.left')?.left, '15%');
```

يجب عليك استخدام منتقي `.blush.right`.

```js
assert.match(code, /\.blush\.right\s*\{/);
```

يجب أن يحتوي منتقي `.blush.right` على خاصية `right` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
