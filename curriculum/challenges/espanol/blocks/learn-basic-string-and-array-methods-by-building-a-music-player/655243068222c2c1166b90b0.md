...
Dentro de la función `playNextSong`, crea una declaración `if` para verificar si el `currentSong` de `userData` es estrictamente igual a `null`. Esto verificará si no hay una canción actual reproduciéndose en el objeto `userData`. 

Si la condición es verdadera, llama a la función `playSong` con el `id` de la primera canción en el arreglo `userData?.songs` como argumento.

# --hints--

No debes modificar la función existente `playNextSong` y su contenido.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Debes crear una declaración `if` con la condición `userData?.currentSong === null`.
...
Debes llamar a la función `playSong` con `userData?.songs[0].id` dentro de la declaración `if`.
