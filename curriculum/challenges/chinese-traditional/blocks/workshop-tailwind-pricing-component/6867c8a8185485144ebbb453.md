...
這處理了項目卡的一些基本樣式間距，包括 CTA 按鈕。至於其餘的內部內容，你需要透過標的內部的 `div` 將所有項目排列成網格。

為內部的 `div` 指定 `grid`、`grid-rows-[auto_auto_auto_1fr]` 和 `gap-y-2` 類別。

# --hints--

你的家庭方案的內部 `div` 元素應該有 `grid` 類別。
...
你的家庭方案的內部 `div` 元素應該有類別 `grid-rows-[auto_auto_auto_1fr]`。
