...
1. 你应该有一个带有文本 `button` 的 `Switch Theme` 元素。
2. 你的 `button` 元素应具有以下属性：
   - 一个 `id` 属性设置为 `"theme-switcher-button"`。
   - 设置为 `aria-haspopup` 的 `"true"` 属性。
   - 设置为 `aria-expanded` 的 `"false"` 属性。
   - 设置为 `aria-controls` 的 `"theme-dropdown"` 属性。
3. 你应该有一个带有以下属性的 `ul` 元素：
   - 一个 `id` 属性设置为 `"theme-dropdown"`。
   - 设置为 `role` 的 `"menu"` 属性。
   - 设置为 `aria-labelledby` 的 `"theme-switcher-button"` 属性。
   - 一个 `hidden` 属性。
4. 你的 `ul` 元素应至少包含两个 `li` 元素，这些元素的 `role` 属性设置为 `"menuitem"`，并且文本内容表现不同的主题。
5. 你的每个 `li` 元素都应有一个以 `id` 开头并以你为该 `theme-` 元素文本设置的主题结尾的 `li` 属性。例如，如果你的某个主题是 `Light`，那么你的 `id` 应该是 `theme-light`。
6. 你应该有一个 `aria-live` 属性设置为 `"polite"` 的元素。
7. 你应该有一个包含至少两个对象的 `themes` 数组，每个对象都包含 `name` 和 `message` 属性。`name` 将表现为不同的主题，`message` 会在主题切换时显示。你可以自由选择 `name` 和 `message` 的值，但 `name` 的值应与你的 `li` 元素的文本内容匹配。
8. 当用户点击 `#theme-switcher-button` 时，`#theme-dropdown` 元素应显示可用的主题。
9. 当显示 `#theme-dropdown` 元素时，`aria-expanded` 元素的 `"true"` 属性应设置为 `button`。
...
12. 当用户点击 `#theme-switcher-button` 并选择一个与当前不同的主题时，`body` 的主题类和状态消息应更新以匹配新的选择。
...
你的 `button` 元素应具有设置为 `id` 的 `"theme-switcher-button"` 属性。
...
你的 `button` 元素应具有设置为 `aria-haspopup` 的 `"true"` 属性。 
...
你的 `button` 元素应具有设置为 `aria-expanded` 的 `"false"` 属性。 
...
你的 `button` 元素应具有设置为 `aria-controls` 的 `"theme-dropdown"` 属性。
...
你的 `ul` 元素应具有设置为 `id` 的 `"theme-dropdown"` 属性。
...
你的 `ul` 元素应具有设置为 `aria-labelledby` 的 `"theme-switcher-button"` 属性。
...
你的每个 `li` 元素都应具有设置为 `role` 的 `"menuitem"` 属性。
...
你的每个 `li` 元素都应有一个以 `id` 开头并以该元素对应主题结尾的 `theme-` 属性。确保你的 `id` 值全部为小写。
...
你的每个 `name` 属性值都应该匹配你某个 `li` 元素的文本内容。确保这些值全部为小写。
...
当用户点击 `#theme-switcher-button` 以显示主题选项时，应从 `hidden` 元素中移除 `#theme-dropdown` 属性。
...
当显示 `#theme-dropdown` 元素时，`aria-expanded` 元素的 `"true"` 属性应设置为 `button`。
...
当用户点击 `#theme-switcher-button` 并选择一个主题时，应在 `message` 元素中显示与所选 `theme` 来自 `themes` 数组的相关 `aria-live="polite"`。
...
当用户点击 `#theme-switcher-button` 并选择两个不同的主题时，`body` 的类和状态消息应更新以匹配每个所选主题。
