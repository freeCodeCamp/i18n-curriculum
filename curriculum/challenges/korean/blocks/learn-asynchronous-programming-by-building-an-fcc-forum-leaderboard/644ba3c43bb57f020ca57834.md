...
이제 `className` 객체에 `category`와 `selectedCategory` 속성을 추가해야 합니다.

먼저 `className` 변수를 `selectedCategory.className`에 할당하세요. 그런 다음 `category` 변수를 `selectedCategory.category`에 할당하세요.

# --hints--

점 표기법을 사용해 `className` 객체의 `selectedCategory` 속성을 지정하세요.

```js
assert.match(code, /selectedCategory\.className/);
```

`className` 객체의 `allCategories` 속성을 `className` 객체의 `selectedCategory` 속성에 할당하세요.

```js
assert.match(code, /selectedCategory\.className\s*=\s*className/);
```

점 표기법을 사용해 `category` 객체의 `selectedCategory` 속성을 지정하세요.

```js
assert.match(code, /selectedCategory\.category/);
```

`category` 객체의 `allCategories` 속성을 `category` 객체의 `selectedCategory` 속성에 할당하세요.
