...
まだ実際には何もフィルターしていませんが、`filteredItems` が `items` 配列に一致しているので、ページにアイテムのリストが表示されるといいですね。

まず、`ul` の `p` 要素の下に `form` 要素を追加してください。番号なしリストの中で、`map()` メソッドを使って `filteredItems` 配列を繰り返し、無名関数を渡してください。その無名関数は `item` を引数に取り、各アイテムに対して `li` 要素を返すようにします。各 `li` は `item` のテキストを表示する必要があります。

# --hints--

`form` 要素には `ul` 要素が含まれている必要があります。
...
`map()` 要素内の `filteredItems` 配列に対して `ul` メソッドを使うべきです。
...
`map()` メソッドには `item` を引数に取る無名関数を渡してください。
...
`map()` の無名関数は、`li` 配列の各要素に対して `item` のテキストを持つ `filteredItems` 要素を返す必要があります。
