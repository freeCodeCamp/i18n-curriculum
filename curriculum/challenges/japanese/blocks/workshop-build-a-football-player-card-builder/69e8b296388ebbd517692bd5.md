...
DRI `form-group`の後に、`div`の中に`className`の`form-group`を持つ`stats-grid`要素を2つ追加してください。1つはDEF用、もう1つはPHY用です。

DEF用には、`label`の`htmlFor`を持ち、テキストが`def`の`DEF`を追加し、その後に`id`の`def`と`type`の`number`を持つ入力を追加してください。

PHY用には、`htmlFor`の`phy`を持ち、テキストが`PHY`のラベルを追加し、その後に`id`の`phy`と`type`の`number`を持つ入力を追加してください。

# --hints--

`div`の`className`の`form-group`の中に、`div`の`className`を持つ`stats-grid`要素を2セット追加してください。
...
5番目の`div`の`className`の`form-group`要素には、`label`があり、その`htmlFor`属性は`def`、`className`の`label`、そしてテキスト`DEF`を持つ必要があります。
...
5番目の`div`の`className`の`form-group`要素には、`input`があり、その`className`は`input`、`id`の`def`、そして`type`の`number`を持つ必要があります。
...
6番目の`div`の`className`の`form-group`要素には、`label`があり、その`htmlFor`属性は`phy`、`className`の`label`、そしてテキスト`PHY`を持つ必要があります。
...
4番目の`div`の`className`の`form-group`要素には、`input`があり、その`className`は`input`、`id`の`phy`、そして`type`の`number`を持つ必要があります。
