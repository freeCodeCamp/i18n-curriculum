...
Ahora manejarás la selección de la posición del jugador. Tienes que refactorizar el input a un select, luego recorrer un arreglo de posiciones de fútbol válidas como valor de las opciones del select.

Para comenzar, crea un arreglo `POSITIONS` con los valores de cadena `GK`, `CB`, `LB`, `RB`, `CDM`, `CM`, `CAM`, `LW`, `RW`, `ST` y `CF`. Añade `as const` justo después del corchete de cierre para que TypeScript reduzca el tipo de un `string[]` general a una tupla de solo lectura con esos valores literales exactos.

Esto significa que TypeScript sabe exactamente qué cadenas son válidas, en lugar de solo saber que es algún arreglo de cadenas.

# --hints--

Debes declarar una constante `POSITIONS`.
...
Tu arreglo `POSITIONS` debe contener todas las cadenas requeridas de posiciones de fútbol; usa `as const` para reducir su tipo a literales de cadena de solo lectura.
