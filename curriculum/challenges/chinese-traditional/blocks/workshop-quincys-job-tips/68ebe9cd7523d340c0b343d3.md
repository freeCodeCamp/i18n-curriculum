...
在第三個也是最後一個 `section` 元素中，你將創建與前一個區段相同的 HTML 結構。

首先，在最後一節內，添加一個帶有文字 `Importance of Building a Reputation` 的 `h2` 元素。

然後，在 `h2` 元素下方，添加一個區塊引用元素，並將 `cite` 屬性設為 `https://www.freecodecamp.org/news/learn-to-code-book/`。

最後，在區塊引用元素內依序巢狀三個段落元素，內容為以下文字：
...
你應該在第三個區段內巢狀一個 `h2` 元素。

```js
assert.exists(document.querySelector('main > section:nth-of-type(3) > h2'));
```

你的 `h2` 元素應該有文字 `Importance of Building a Reputation`。
...
你應該在第三個區段內巢狀放置一個 `blockquote` 元素。
...
你應該在第三個區段的 `blockquote` 元素內巢狀三個 `p` 元素。
...
你的第一個 `p` 元素應該有文字 `Share short video demos of your projects.`
...
你的第二個 `p` 元素應該有文字 `Keep applying to speak at bigger and bigger conferences.`
...
你的第三個 `p` 元素應該有文字 `Hang out at hackerspaces and help people who are even newer to coding than you.`
