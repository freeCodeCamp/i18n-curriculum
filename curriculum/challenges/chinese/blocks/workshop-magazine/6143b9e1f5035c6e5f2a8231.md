...
在你的 `ul` 元素中，创建六个 `li` 元素。 在每个 `li` 元素中添加一个 `class` 设置为 `list-subtitle` 的 `h4` 元素和一个 `p` 元素。
...
- `V4 - 2016`：我们将课程扩展到 3 个认证，包括 Frontend、Backend 和 Data Visualization。它们各自有 10 个必做项目，但只有 Frontend 部分有自己的挑战。对于其他认证，我们仍然使用像 Node School 这样的外部资源。
...
你的第四个 `p` 应该包含文本 `We expanded the curriculum to 3 certifications, including Frontend, Backend, and Data Visualization. They each had 10 required projects, but only the Frontend section had its own challenges. For the other certs, we were still using external resources like Node School.`
...
你的第五个 `p` 应该包含文本 `We added the backend and data visualization challenges.`
