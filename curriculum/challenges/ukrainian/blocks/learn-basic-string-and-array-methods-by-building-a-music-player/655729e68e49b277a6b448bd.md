...
Тепер, коли ви створили `resetButton`, потрібно призначити йому атрибути `id` і `aria-label`. JavaScript надає властивості `id` і `ariaLabel`, які потрібно використати для цього.

Наприклад, `element.id` призначить атрибут `id`, а `element.ariaLabel` призначить атрибут `aria-label`. Обидва приймають свої значення у вигляді рядка.

Призначте атрибут `id` елементу `resetButton` зі значенням `"reset"` та атрибут `aria-label` зі значенням `"Reset playlist"`.

# --hints--

Ви не повинні змінювати існуючу інструкцію `if` та її вміст.
...
Використайте `resetButton.id`, щоб створити атрибут `id` з іменем `"reset"` для `resetButton`.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

Використайте `resetButton.ariaLabel`, щоб створити атрибут `"aria-label"` з іменем `"Reset playlist"` для `resetButton`.
