...
Debes utilizar el selector `.sr-only`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.sr-only'));
```

Debes dar al `.sr-only` una `position` `absolute`.
...
Debes dar al `.sr-only` una `height` de `1px`.
...
Debes darle al `.sr-only` un `overflow` de `hidden`.
