...
Ваш елемент `.text` не є CSS-сіткою, але ви можете створювати колонки всередині елемента без використання сітки, застосовуючи властивість `column-width`.

Додайте селектору `.text` властивість `column-width` зі значенням `25rem`.

# --hints--

Ваш селектор `.text` має мати властивість `column-width` зі значенням `25rem`.
