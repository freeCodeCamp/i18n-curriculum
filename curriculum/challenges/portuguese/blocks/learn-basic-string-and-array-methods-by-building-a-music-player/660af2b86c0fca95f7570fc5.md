...
Se a arrow function estiver retornando uma expressão simples, você pode omitir a palavra-chave `return` e as chaves `{}`. Isso é chamado de retorno implícito.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

Se a sua função arrow tiver múltiplas linhas de código no corpo da função, então você precisa usar a palavra-chave `return` e as chaves `{}`.
...
Abra o console para garantir que você ainda está obtendo a saída correta.

# --hints--

Sua função `addTwoNumbers` refatorada deve usar um retorno implícito.
