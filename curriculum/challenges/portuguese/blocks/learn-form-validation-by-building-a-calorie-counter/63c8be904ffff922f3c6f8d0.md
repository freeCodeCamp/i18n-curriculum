...
Atualize sua declaração de `currVal` para ser o resultado da chamada de `cleanInputString` com `item.value`.

# --hints--

Você deve chamar a função `cleanInputString` dentro da função `getCaloriesFromInputs`.

```js
assert.match(getCaloriesFromInputs.toString(), /cleanInputString\(/);
```

Você deve passar `item.value` como argumento para a função `cleanInputString`.
