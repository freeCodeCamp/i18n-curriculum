...
الآن بعد أن عرضت قائمة الأغاني على الشاشة، سيكون من الجيد ترتيبها أبجديًا حسب العنوان.

ابدأ بإنشاء دالة سهمية تسمى `sortSongs`.

# --hints--

يجب أن يكون لديك دالة تسمى `sortSongs`.

```js
assert.isFunction(sortSongs);
```

يجب أن تستخدم دالتك `sortSongs` صيغة نحوية للدالة السهمية.
