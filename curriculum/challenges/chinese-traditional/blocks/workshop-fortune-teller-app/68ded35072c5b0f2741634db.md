...
你需要另一個函式來顯示 DOM 中的元素。類似於上一個函式，創建一個名為 `showElements` 的函式。這次使用 `classList.remove("hidden")`。

# --hints--

你應該有一個名為 `showElements` 的函式。
...
你的 `showElements` 函式應該有一個名為 `...elements`、型別為 `HTMLElement[]` 的參數。
