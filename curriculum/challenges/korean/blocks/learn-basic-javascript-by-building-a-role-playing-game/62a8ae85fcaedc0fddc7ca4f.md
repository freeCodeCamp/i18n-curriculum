...
`locations` 배열에 두 번째 객체를 추가하세요(쉼표로 구분하는 것을 잊지 마세요). 첫 번째 객체에서 사용한 패턴을 따라 같은 속성을 만들되, 값은 `goStore` 함수에서 가져오세요. `name` 속성은 `store`로 설정하세요.

# --hints--

`locations` 배열에는 두 개의 값이 있어야 합니다.

```js
assert.lengthOf(locations, 2);
```

두 `locations` 값 모두 객체여야 합니다.
...
두 번째 `locations` 객체에는 `name` 속성이 있고, 그 값은 `store`이어야 합니다.

```js
assert.equal(locations[1].name, "store");
```

두 번째 `locations` 객체에는 배열인 `button text` 속성이 있어야 합니다.

```js
assert.isArray(locations[1]["button text"]);
```

`button text` 속성에는 문자열 값 `"Buy 10 health (10 gold)"`, `"Buy weapon (30 gold)"`, `"Go to town square"`가 있어야 합니다.
...
두 번째 `locations` 객체에는 배열인 `button functions` 속성이 있어야 합니다.

```js
assert.isArray(locations[1]["button functions"]);
```

`button functions` 속성에는 함수 값 `buyHealth`, `buyWeapon`, `goTown`가 있어야 합니다.
...
두 번째 `locations` 객체에는 문자열인 `text` 속성이 있어야 합니다.
