...
이제 `product`를 장바구니의 `items` 배열에 추가해야 합니다. `this` 키워드를 사용하는 것을 잊지 마세요.

# --hints--

`push` 배열에서 `items` 메서드를 호출해야 합니다.
...
`this` 배열에 접근하려면 `items` 키워드를 사용해야 한다는 점을 기억하세요.
...
`push` 배열에 `product` 변수를 `items` 해야 합니다.
