...
Fügen Sie Ihrem `textarea`-Element ein `id`- und ein `name`-Attribut mit dem Wert `"comments"` hinzu.

Für das `cols`-Attribut setzen Sie den Wert auf `30` und für das `rows`-Attribut setzen Sie den Wert auf `10`.

# --hints--

Ihr `textarea`-Element sollte ein `id`-Attribut mit dem Wert `"comments"` haben.

```js
assert.exists(document.querySelector('textarea#comments'));
```

Ihr `textarea`-Element sollte ein `name`-Attribut mit dem Wert `"comments"` haben.

```js
assert.exists(document.querySelector('textarea[name="comments"]'));
```

Ihr `textarea`-Element sollte ein `cols`-Attribut mit dem Wert `"30"` haben.

```js
assert.exists(document.querySelector('textarea[cols="30"]'));
```

Ihr `textarea`-Element sollte ein `rows`-Attribut mit dem Wert `"10"` haben.
