...
Crea un altro elemento `p`, dagli il testo `Calcium 260mg 20%`. Allinea `20%` a destra. Sotto, crea un elemento `p` con il testo `Iron 8mg 45%`, allineando `45%` a destra.
...
Il tuo primo elemento `span` non dovrebbe essere in grassetto.
...
Il tuo primo elemento `span` dovrebbe racchiudere il testo `20%`.
...
Il tuo secondo elemento `span` non dovrebbe essere in grassetto.
