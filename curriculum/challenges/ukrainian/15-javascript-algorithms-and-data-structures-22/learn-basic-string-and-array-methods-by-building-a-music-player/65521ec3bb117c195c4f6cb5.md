...
Щоб зберегти поточний час пісні, коли її призупинено, встановіть `songCurrentTime` об’єкта `userData` на `currentTime` змінної `audio`.

*Примітка*: ви не повинні використати необов’язковий ланцюжок на цьому кроці, оскільки `userData.songCurrentTime` не буде `null` або `undefined` на цьому етапі.
...
Призначте `audio.currentTime` до `userData.songCurrentTime`.
