...
Erstellen Sie einen `hr`-Selektor und geben Sie ihm eine `margin`-Eigenschaft mit dem Wert `1.5rem 0`.

# --hints--

Sie sollten einen `hr`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('hr'));
```

Ihr `hr`-Selektor sollte eine `margin`-Eigenschaft mit dem Wert `1.5rem 0` haben.
