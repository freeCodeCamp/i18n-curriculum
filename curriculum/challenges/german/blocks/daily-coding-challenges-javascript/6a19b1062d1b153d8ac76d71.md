...
title: "Aufgabe 323: Song-Stimmungsfinder"
challengeType: 28
...
Gegeben ist ein Genre-String und eine BPM-Zahl für einen Song. Bestimmen Sie die Stimmung anhand der folgenden Tabelle:

| Stimmung | Genre | BPM-Bereich |
|------|-------|-----------|
...
`getMood("rock", 111)` sollte `"happy"` zurückgeben.

```js
assert.equal(getMood("rock", 111), "happy");
```

`getMood("electronic", 74)` sollte `"focus"` zurückgeben.

```js
assert.equal(getMood("electronic", 74), "focus");
```

`getMood("classical", 180)` sollte `"happy"` zurückgeben.

```js
assert.equal(getMood("classical", 180), "happy");
```

`getMood("rock", 155)` sollte `"hype"` zurückgeben.

```js
assert.equal(getMood("rock", 155), "hype");
```

`getMood("electronic", 90)` sollte `"happy"` zurückgeben.

```js
assert.equal(getMood("electronic", 90), "happy");
```

`getMood("classical", 67)` sollte `"focus"` zurückgeben.

```js
assert.equal(getMood("classical", 67), "focus");
```

`getMood("pop", 100)` sollte `"happy"` zurückgeben.

```js
assert.equal(getMood("pop", 100), "happy");
```

`getMood("electronic", 135)` sollte `"hype"` zurückgeben.
