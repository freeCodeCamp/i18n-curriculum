...
Зображення повинні розташовуватися в два стовпчики та три рядки.

Надайте селектору `.image-wrapper` властивість `grid-template-columns` зі значенням `2fr 1fr` та властивість `grid-template-rows` зі значенням `repeat(3, min-content)`. Це надасть нашій сітці рядки, які регулюються в висоті залежно від вмісту, але стовпчики матимуть фіксовану ширину на основі контейнера.

# --hints--

Ваш селектор `.image-wrapper` повинен мати властивість `grid-template-columns` зі значенням `2fr 1fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

Ваш селектор `.image-wrapper` повинен мати властивість `grid-template-rows` зі значенням `repeat(3, min-content)`.
