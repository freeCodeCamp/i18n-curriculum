...
استدعِ الدالة `setPlayerCards` مع معلمة `players.filter()`.

داخل طريقة `filter`، أضف دالة استدعاء راجع بمعلمة تسمى `player` وأرجع ضمنيًا أن `player.nickname` ليست `null`.

# --hints--

في حالة `nickname`، يجب أن تستدعي الدالة `setPlayerCards`.
...
يجب أن تُرجع دالة الاستدعاء الراجع لـ `filter()` ضمنيًا نتيجة التحقق من أن `player.nickname` ليست `null`.
