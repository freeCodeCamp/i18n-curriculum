...
Il nuovo elemento `section` dovrebbe trovarsi dopo l'elemento `.text`.

```js
assert(document.querySelectorAll('section')?.[2]?.previousElementSibling?.className === 'text')
```

Il nuovo elemento `section` dovrebbe avere l'attributo `class` impostato su `text text-with-images`.

```js
assert(document.querySelectorAll('section')?.[2]?.className === 'text text-with-images')
```

Il nuovo elemento `section` dovrebbe avere un elemento `article`.

```js
assert.exists(document.querySelector('.text-with-images article'));
```

Il nuovo elemento `section` dovrebbe avere un elemento `aside`.

```js
assert.exists(document.querySelector('.text-with-images aside'));
```

L'elemento `article` dovrebbe precedere l'elemento `aside`.

```js
assert(document.querySelector('.text-with-images article')?.nextElementSibling?.localName === 'aside');
```

L'elemento `article` dovrebbe avere l'attributo `class` impostato su `brief-history`.

```js
assert(document.querySelector('.text-with-images article')?.className === 'brief-history');
```

L'elemento `aside` dovrebbe avere l'attributo `class` impostato su `image-wrapper`.
