...
Weisen Sie innerhalb Ihrer `else`-Klausel den String `"general"` `selectedCategory.className` zu.

Weisen Sie darunter den String `"General"` `selectedCategory.category` zu.

Weisen Sie zuletzt die Zahl `1` `selectedCategory.id` zu.

# --hints--

Sie sollten den String `"general"` `selectedCategory.className` zuweisen.

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

Sie sollten den String `"General"` `selectedCategory.category` zuweisen.

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

Sie sollten die Zahl `1` `selectedCategory.id` zuweisen.
