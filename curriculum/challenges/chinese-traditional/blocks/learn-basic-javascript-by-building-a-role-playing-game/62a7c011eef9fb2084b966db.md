...
將你的 `goTown` 函式（程式）移動到你的 `goStore` 函式（程式）上方。然後將 `goStore` 函式（程式）的內容複製並貼上到 `goTown` 函式（程式）中。

# --hints--

你的 `goTown` 函式（程式）應該在你的 `goStore` 函式（程式）之前。
...
你的 `goTown` 函式（程式）應該與你的 `goStore` 函式（程式）具有相同的內容。
