...
為了修正錯誤，請創建一個帶有 `e` 參數的 `handlePowersChange` 箭頭函式（程式）。

在函式（程式）內，從 `e.target` 解構 `value` 和 `checked`，以取得核取方塊的值及其是否被選取。

# --hints--

你應該創建一個帶有 `e` 參數的 `handlePowersChange` 函式（程式）。

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

你應該從 `e.target` 解構 `value` 和 `checked`。
