...
배경을 완성하려면 `div` 요소 바로 뒤에 새로운 `.back-mountain` 요소를 만들고, `sun` 클래스를 부여해 태양을 추가하세요.

# --hints--

새로운 `div` 요소를 `body`에 추가해야 합니다. 예상된 --fcc-expected-- `div` 요소가 있지만, 실제로는 --fcc-actual-- 요소가 발견되었습니다.

```js
assert.lengthOf(document.querySelectorAll('body > div'), 5);
```

새로운 `div` 요소에 `class`으로 `sun`를 지정해야 합니다.

```js
assert.include(document.querySelector('div:not(.back-mountain, .left-mountain, .penguin, .ground)')?.className, 'sun');
```

새로운 `div` 요소를 `.back-mountain` 요소 바로 뒤에 배치해야 합니다.
