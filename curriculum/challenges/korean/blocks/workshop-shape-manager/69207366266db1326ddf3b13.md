...
루트 문서에 `DOMContentLoaded` 이벤트를 추가하고 실행할 콜백 함수로 `initializeApp`을 전달하세요. 이렇게 하면 DOM이 완전히 로드된 후에만 모든 초기화가 이루어집니다.

# --hints--

루트 `DOMContentLoaded`에 `document` 이벤트를 추가해야 합니다.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

이벤트 리스너에서 실행할 함수로 `initializeApp`를 참조해야 합니다.
