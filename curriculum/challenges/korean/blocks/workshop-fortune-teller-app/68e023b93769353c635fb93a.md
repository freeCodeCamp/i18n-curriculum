...
이제 `renderCard` 함수의 템플릿 리터럴 안에 `div` 요소를 만드세요. `div` 안에는 `h2`를 출력하는 `drawingType` 요소와 `figure` 클래스를 가진 `card_container ${isReversed ? "reversed-card" : ""}` 속성이 있는 `data-id="${shortName}"` 요소를 추가하세요.

# --hints--

`div` 함수의 템플릿 리터럴 안에 `renderCard` 요소가 있어야 합니다.
...
`h2` 요소 안에 `drawingType`을 보간하는 `div` 요소가 있어야 합니다.
...
`figure` 요소는 `card_container` 클래스를 가져야 하며, 삼항 연산자를 사용해 조건부로 `reversed-card`을 추가해야 합니다.
...
`figure` 요소는 `data-id`으로 설정된 `${shortName}` 속성을 가져야 합니다.
