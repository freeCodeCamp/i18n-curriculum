...
لننتقل إلى البناء التالي! قم بتسكين عنصرين `div` داخل `.fb2` وأعطهما أصنافًا `fb2a` و `fb2b` بهذا الترتيب.

# --hints--

يجب إضافة عنصرين `div` داخل `.fb2`.

```js
assert.equal(document.querySelectorAll("div.fb2 > div")?.length, 2);
```

يجب إعطاء أول عنصر `div` جديد الصنف `fb2a`.
...
يجب إعطاء ثاني عنصر `div` جديد الصنف `fb2b`.
