...
在這個專案中，你將使用 `https://cdn.freecodecamp.org/curriculum/labs/data/bar-chart/GDP-data.json` 的資料來建置一個長條圖。

測試要求使用 D3 的 axis 屬性來產生軸，該屬性會自動在軸上產生刻度。這些刻度是通過 D3 測試的必要條件，因為它們的位置用來判斷圖表元素的對齊。

**目標：** 完成以下使用者故事並通過所有測試以完成此實驗。

**使用者故事：**

1. 你的圖表應該有一個標題，並有對應的 `id="title"`。
1. 你的圖表應該有一個 `g` 元素的 x 軸，並有對應的 `id="x-axis"`。
1. 你的圖表應該有一個 `g` 元素的 y 軸，並有對應的 `id="y-axis"`。
1. 兩個軸都應包含多個刻度標籤，每個標籤都有對應的 `class="tick"`。
1. 你的圖表應該為每個資料點有一個 `rect` 元素，並有對應的 `class="bar"` 顯示資料。
1. 每個 `.bar` 都應有屬性 `data-date` 和 `data-gdp`，分別包含 `date` 和 `GDP` 的值。
1. `.bar` 元素的 `data-date` 屬性應與提供的資料順序相符。
1. `.bar` 元素的 `data-gdp` 屬性應與提供的資料順序相符。
1. 每個 `.bar` 元素的高度應準確表述資料中對應的 `GDP`。
1. `data-date` 屬性及其對應的 `.bar` 元素應與 x 軸上的對應值對齊。
1. `data-gdp` 屬性及其對應的 `.bar` 元素應與 y 軸上的對應值對齊。
1. 你應該能將滑鼠暫留在某區域，並看到一個帶有對應 `id="tooltip"` 的提示框，顯示該區域的更多資訊。
1. 你的提示框應有一個 `data-date` 屬性，對應作用中區域的 `data-date`。
...
`data-date` 屬性及其對應的長條元素應與 x 軸上的對應值對齊。你可以使用 `d3.scaleTime()` 來為 x 軸建立時間尺度。
...
`data-gdp` 屬性及其對應的長條元素應與 y 軸上的對應值對齊。你可以使用 `d3.scaleLinear()` 來為 y 軸建立線性尺度。
...
當滑鼠暫留在某區域時，你的長條圖應有一個帶有對應 `id="tooltip"` 的提示框，顯示該區域的更多資訊。
...
提示框應有一個 `data-date` 屬性，對應作用中區域的 `data-date`。
