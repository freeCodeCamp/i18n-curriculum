...
The value of an input element is always a string, even if the input type is `number`. Du musst dieses Array von Strings in ein Array von Zahlen umwandeln. Hierzu kannst du die `.map()`-Methode verwenden.
