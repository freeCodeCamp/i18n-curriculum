...
Toa vipengele vya `.fb3-window` `width` ya `25%`, `height` ya `80%`, na tumia tofauti yako ya `--window-color1` kama thamani ya `background-color`.

# --hints--

Unapaswa kutoa `.fb3-window` `width` ya `25%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

Unapaswa kutoa `.fb3-window` `height` ya `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

Unapaswa kutoa `.fb3-window` `background-color` ya `--window-color1`.
