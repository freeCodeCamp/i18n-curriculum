...
在工作坊的最後零件，底部的聯絡部分需要做一些更改。

第一個變更是將聯絡區塊包裹在 `footer` 元素內。這將有助於螢幕閱讀器識別此區塊為頁面的頁腳。
...
你的 `footer` 元素應該包含一個帶有文字 `Contact Me` 的 `h3` 元素。
...
你的 `footer` 元素應該包含一個帶有文字 `Email me at camperbot@blog.io` 的 `p` 元素。
