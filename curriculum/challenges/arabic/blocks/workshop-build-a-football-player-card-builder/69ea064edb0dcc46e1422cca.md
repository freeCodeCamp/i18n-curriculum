...
الآن ستضيف المنطق الخاص بحفظ البيانات في `localStorage` باستخدام `useEffect`.

يُعتبر الوصول إلى البيانات أو حفظها في `localStorage` تأثيرًا جانبيًا في React، لذا يجب استخدام `useEffect` لهذا المنطق.

ابدأ بتفكيك `useEffect` من React، تمامًا كما فعلت مع `useState`.

# --hints--

يجب عليك تفكيك `useEffect` من `React`.
