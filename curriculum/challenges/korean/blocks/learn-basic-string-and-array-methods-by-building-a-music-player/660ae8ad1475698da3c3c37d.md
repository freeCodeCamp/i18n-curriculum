...
이름이 있는 화살표 함수 표현식을 호출하려면 함수 이름으로 참조할 수 있습니다.

```js
exampleArrowFunction();
```

`printGreeting` 함수 정의 아래에 함수를 호출하세요. 출력 결과를 보려면 콘솔을 열어보세요.

# --hints--

`printGreeting` 함수를 호출해야 합니다.

```js
assert.match(code, /printGreeting\(\);?/);
```

`printGreeting` 함수 호출은 `printGreeting` 함수 정의 아래에 있어야 합니다.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

`printGreeting` 함수 호출은 콘솔에 `Hello there!`를 출력해야 합니다.
