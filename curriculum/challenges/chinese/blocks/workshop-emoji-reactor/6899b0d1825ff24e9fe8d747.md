...
在 `h1` 元素下方，添加一个带有类 `description` 的 `p` 元素。

在 `p` 元素内写入文本 `Click on the buttons below to rate your emotions.`

# --hints--

你应该在 `h1` 元素下方创建一个新建的 `p` 元素。

```js
assert.exists(document.querySelector("h1 + p"));
```

新建的 `p` 元素应具有 `description` 的类。

```js
assert.equal(document.querySelector("h1 + p")?.className, "description");
```

新建的 `p` 元素应包含文本 `Click on the buttons below to rate your emotions.`。
