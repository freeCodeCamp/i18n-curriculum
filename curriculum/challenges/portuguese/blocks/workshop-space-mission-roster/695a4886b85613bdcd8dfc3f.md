...
Primeiro, crie uma cópia superficial do `array` de entrada `crew` usando `slice()` e atribua-a a uma variável chamada `sorted`.

# --hints--

Você deve usar o método `slice()` para copiar o array de entrada `crew` para uma nova variável chamada `sorted`.
