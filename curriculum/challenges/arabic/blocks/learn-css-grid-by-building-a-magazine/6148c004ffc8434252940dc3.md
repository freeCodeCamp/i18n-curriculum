...
أنشئ محدِّد `.social-icons`. امنحه خاصية `display` بقيمة `grid`، وخاصية `font-size` بقيمة `3rem`.

# --hints--

يجب أن يكون لديك محدِّد `.social-icons`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

يجب أن يحتوي محدِّد `.social-icons` على خاصية `display` بقيمة `grid`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display === 'grid');
```

يجب أن يحتوي محدِّد `.social-icons` على خاصية `font-size` بقيمة `3rem`.
