...
Erstelle einen `html`-Selektor und gib ihm eine `font-size`-Eigenschaft von `62.5%`. Dadurch wird die Standardschriftgröße deiner Webseite auf 10px gesetzt (Die Browser-Standardeinstellung ist 16px).

Dies erleichtert dir später mit `rem`-Einheiten zu arbeiten, da `2rem` 20 px wäre.
