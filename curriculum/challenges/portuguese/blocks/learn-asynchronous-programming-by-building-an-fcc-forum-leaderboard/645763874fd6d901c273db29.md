...
Dentro do primeiro elemento `td`, adicione uma expressão embutida `${}`. Dentro dessa expressão, chame a função `forumCategory` com o argumento `category_id`. 

Agora, você deve ver uma categoria exibida abaixo de cada tópico de postagem. 

# --hints--

Você deve chamar a função `forumCategory` com o argumento `category_id`
