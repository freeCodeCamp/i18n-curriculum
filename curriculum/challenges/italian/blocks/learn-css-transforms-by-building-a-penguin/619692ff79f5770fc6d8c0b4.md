...
Sopra l'elemento `.ground`, aggiungi un `div` con un `class` di `penguin`. Questo `div` conterrà Flappy Penguin.

# --hints--

Dovresti aggiungere un nuovo `div` all'interno del `body`.

```js
assert.equal(document.querySelectorAll('body > div')?.length, 2);
```

Dovresti dare al `div` un `class` di `penguin`.

```js
assert.include(document.querySelector('body > div:not(.ground)')?.className, 'penguin');
```

Dovresti posizionare `div.penguin` prima di `div.ground`.
