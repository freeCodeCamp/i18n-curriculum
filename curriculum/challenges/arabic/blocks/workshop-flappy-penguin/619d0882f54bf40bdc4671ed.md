...
عند الاقتضاء، استبدل قيم الخاصية بمتغير `--penguin-face` الخاص بك.

# --hints--

امنح `.face` خاصية `background-color` بقيمة `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

امنح `.chin` خاصية `background-color` بقيمة `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

لا تستخدم `var(--penguin-face)` في محدِّد `.penguin-body`.
