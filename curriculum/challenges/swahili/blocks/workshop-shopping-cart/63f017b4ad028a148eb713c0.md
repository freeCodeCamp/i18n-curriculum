...
Kwa usemi wako wa kihesabu wa falsy, utahitaji kuongeza HTML mpya kwenye `productsContainer` yako. Anza kwa kuondoa `undefined`, kisha tumia opereta ya kuongeza na sintaksia ya template literal kuongeza `div` yenye `class` iliyowekwa kuwa `product` na `id` iliyowekwa kuwa `dessert${id}` kwenye sifa ya `innerHTML` ya `productsContainer`.

# --hints--

Unapaswa kuondoa `undefined` kutoka kwenye usemi wako wa kihesabu wa falsy.
...
Unapaswa kutumia opereta ya kuongeza kuongeza HTML kwenye `productsContainer`. Kumbuka kuwa HTML inawekwa kwenye sifa ya `innerHTML`.
...
Unapaswa kutumia sintaksia ya template literal kuongeza HTML kwenye `productsContainer`.

```js
assert.match(code, /productsContainer\.innerHTML\s*\+=\s*`/);
```

Unapaswa kuongeza `div` kwenye `productsContainer`.
...
`div` yako inapaswa kuwa na `class` imewekwa kuwa `product`.
...
`div` yako inapaswa kuwa na `id` imewekwa kuwa `dessert${id}`.
