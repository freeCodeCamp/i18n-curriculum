...
Jetzt, da Sie die Grundstruktur der App haben, ist es Zeit, einige Elemente hinzuzufügen und die Suchfunktion zum Laufen zu bringen.

Erstellen Sie ein Array und weisen Sie es einer Variablen namens `items` zu. Fügen Sie innerhalb des Arrays die Strings `Apples`, `Bananas`, `Strawberries`, `Blueberries`, `Mangoes`, `Pineapple`, `Lettuce`, `Broccoli`, `Paper Towels` und `Dish Soap` hinzu. Dies wird die Liste der Elemente sein, die in der App angezeigt werden.
...
Ihr `items`-Array sollte die Strings `Apples`, `Bananas`, `Strawberries`, `Blueberries`, `Mangoes`, `Pineapple`, `Lettuce`, `Broccoli`, `Paper Towels` und `Dish Soap` enthalten.
