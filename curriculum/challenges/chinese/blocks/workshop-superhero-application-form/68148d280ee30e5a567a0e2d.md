...
删除 `label` 内的 `Hello` 文本，然后创建 `checkbox` 类型的 `input` 和 `power` 类型的 `value`。

最后，通过在结束 `label` 之前创建一个 `span` 元素，并在其文本内容为 `{power}` 的情况下，显示每个复选框旁边的数组中的值。

# --hints--

你应该从 `label` 元素中删除 `Hello` 文本。

```js
assert.notMatch(code, /hello/i)
```

你应该创建一个 `input`，并将其 `type` 设置为 `checkbox`，并将 `value` 设置为 `power`，作为该标签的第一个子项。

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

你应该创建一个 `span` 元素，并以 `{power}` 作为其文本内容，作为标签的最后一个子元素。
