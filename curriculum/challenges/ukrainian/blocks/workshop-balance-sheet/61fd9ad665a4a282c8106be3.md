...
У вашому `thead` створіть елемент `tr`. Усередині нього додайте `td` та три елементи `th`. У кожному з елементів `th` додайте елемент `span` з атрибутом `class`, встановленим у `sr-only`, і наступним текстом у порядку: `2019`, `2020` та `2021`.
...
Ваш елемент `tr` має мати елемент `td` як першого дочірнього елемента.
...
Кожен із нових елементів `span` має мати атрибут `class`, встановлений у `sr-only`.
