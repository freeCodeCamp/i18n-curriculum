...
Se você observar de perto, verá que a música que está tocando no momento não está destacada na playlist, então você realmente não sabe qual música está tocando.Você pode corrigir isso criando uma função para destacar qualquer música que esteja sendo reproduzindo.

Usando a sintaxe de seta, crie uma função `highlightCurrentSong`.Dentro da função, use `querySelectorAll` para obter o elemento `.playlist-song` e atribua a uma constante `playlistSongElements`.

# --hints--

Você deve usar `const` para criar uma função `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` deve ser uma função.

```js
assert.isFunction(highlightCurrentSong)
```

Sua função `highlightCurrentSong` deve usar uma sintaxe de arrow.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

Você deve criar uma constante `playlistSongElements` dentro da sua função `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

Você deve usar o método `querySelectorAll()` para selecionar o elemento `.playlist-song` e atribuí-lo à constante `playlistSongElements`.
