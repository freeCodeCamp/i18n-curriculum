...
Il selettore `.social-icons` dovrebbe avere una propietà `display` impostata su `grid`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display === 'grid');
```

Il selettore `.social-icons` dovrebbe avere una proprietà `font-size` impostata a `3rem`.
