...
Масив `allSongs` повинен мати два значення.

```js
assert.equal(allSongs.length, 2);
```

Масив `allSongs` повинен мати об’єкт як друге значення.

```js
assert.isObject(allSongs[1]);
```

Другий об’єкт у масиві `allSongs` повинен мати властивість `id` встановлену на число `1`.

```js
assert.equal(allSongs[1].id, 1);
```

Другий об’єкт у масиві `allSongs` повинен мати властивість `title` встановлену на рядок `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

Другий об’єкт у масиві `allSongs` повинен мати властивість `artist` встановлену на рядок `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

Другий об’єкт у масиві `allSongs` повинен мати властивість `duration` встановлену на рядок `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

Другий об’єкт у масиві `allSongs` повинен мати властивість `src`, встановлену на рядок `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
