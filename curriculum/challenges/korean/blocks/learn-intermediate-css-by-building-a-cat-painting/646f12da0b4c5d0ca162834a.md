...
클래스 선택자를 사용하여 `.cat-whisker-left-top` 요소를 `position`의 `absolute`, `top`의 `120px`, `left`의 `52px` 위치로 이동하세요.

# --hints--

`.cat-whisker-left-top` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top'))
```

`.cat-whisker-left-top` 선택자에 `position` 속성이 `absolute`으로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.position === 'absolute')
```

`.cat-whisker-left-top` 선택자에 `top` 속성이 `120px`으로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.top === '120px')
```

`.cat-whisker-left-top` 선택자에 `left` 속성이 `52px`으로 설정되어야 합니다.
