...
Ifuatayo, utahitaji kupata wimbo unaofuata katika orodha ya nyimbo. Kwa hiyo, utahitaji kupata kielezo cha wimbo wa sasa kisha kuongeza `1` kwenye hicho.

Tengeneza thabiti iitwayo `nextSong` na weka `userData?.songs[currentSongIndex + 1]` ndani yake.

Mwishowe, fanya wito wa kitendakazi cha `playSong` na upitie `nextSong.id` kama hoja.

# --hints--

Haupaswi kubadilisha tamko la `if` lililopo, sehemu ya `else`, na yaliyomo.
...
Unapaswa kuweka `userData?.songs[currentSongIndex + 1]` kwenye thabiti ya `nextSong`.
...
Unapaswa kufanya wito wa kitendakazi cha `playSong` kwa `nextSong.id`.
