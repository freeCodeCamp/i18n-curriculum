...
`#budget` 요소를 가져와 `budgetNumberInput`에 할당하고, `#entry-dropdown` 요소를 가져와 `entryDropdown`에 할당하세요.

# --hints--

`budgetNumberInput`라는 변수를 선언하세요.

```js
assert.isDefined(budgetNumberInput);
```

`document.getElementById()`를 사용해 `#budget` 요소를 가져오세요.

```js
assert.match(code, /document\.getElementById\(\s*('|")budget\1\s*\)/g);
```

`#budget` 요소를 `budgetNumberInput`라는 변수에 저장하세요.

```js
assert.equal(budgetNumberInput, document.getElementById('budget'));
```

`entryDropdown`라는 변수를 선언하세요.

```js
assert.isDefined(entryDropdown);
```

`document.getElementById()`를 사용해 `#entry-dropdown` 요소를 가져오세요.

```js
assert.match(code, /document\.getElementById\(\s*('|")entry-dropdown\1\s*\)/g);
```

`#entry-dropdown` 요소를 `entryDropdown`라는 변수에 저장하세요.
