...
O bloco `finally` é executado após os blocos `try` e `catch`, independentemente de um erro ter ocorrido. Ele é comumente usado para tarefas de limpeza, como fechar arquivos ou liberar recursos.
...
Chamamos a função dentro de um bloco `try`. Como a chamada da função lança um erro, ele será capturado dentro do bloco `catch` e a mensagem de erro será exibida no console.
...
É para executar o código novamente automaticamente após um erro ocorrer.

### --feedback--

Pense na ordem dos blocos nas declarações `try...catch...finally` e nos tipos de tarefas que cada bloco executa.
...
É para preparar antes que um erro seja lançado.
...
É um invólucro seguro para código que pode lançar um erro.
