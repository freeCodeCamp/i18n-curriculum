...
Dentro da função `handleShapeSelect`, faça a chamada do método `.preventDefault()` em `e` e chame a função `clearInputFields()` para limpar os campos de entrada.

# --hints--

Você deve fazer a chamada `.preventDefault()` em `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

Você deve chamar a função `clearInputFields`.
