...
L'ultima modifica riguarderà il testo dell'email all'interno del piè di pagina. Nelle lezioni e nei workshop precedenti, hai imparato a lavorare con il collegamento `mailto` in questo modo:

```html
<p>Email me at <a href="mailto:janedoe@email.com">janedoe@email.com</a></p>
```

Inizia racchiudendo l'indirizzo email `camperbot@blog.io` all'interno di un elemento àncora. Poi, aggiungi l'attributo `href` all'elemento àncora e impostalo uguale a `mailto:camperbot@blog.io`.

Con quest'ultima modifica, hai risolto con successo tutti i problemi nella pagina del blog!

# --hints--

Il tuo indirizzo email `camperbot@blog.io` dovrebbe essere racchiuso all'interno di un elemento àncora.
...
Il tuo elemento àncora dovrebbe avere un attributo `href` con valore `mailto:camperbot@blog.io`.
