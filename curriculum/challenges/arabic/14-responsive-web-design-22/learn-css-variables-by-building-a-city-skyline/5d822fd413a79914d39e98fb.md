...
أضف `linear-gradient` إلى `.bb2b` الذي يستخدم `--building-color2` من `0%` إلى `6%` و `--window-color2` من `6%` إلى `9%`.

# --hints--

يجب عليك إعطاء `.bb2b` خاصية `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

يجب عليك استخدام `linear-gradient` على `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

يجب عليك استخدام `--building-color2` من `0%` إلى `6%`.
...
يجب عليك استخدام `--window-color2` من `6%` إلى `9%`.
...
`.bb2b` يجب أن يكون نقل `linear-gradient` من `--building-color2` يبدء في `0%` ينتهي في `6%`، و`--window-color2` يبدء في `6%` ينتهي في `9%`.
