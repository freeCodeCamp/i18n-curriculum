...
CSS 변수는 미디어 쿼리를 사용하는 방식을 간소화할 수 있습니다.

예를 들어, 화면 크기가 미디어 쿼리 중단점보다 작거나 클 때 변수를 변경하면, 해당 변수가 사용된 모든 곳에 스타일이 적용됩니다.

# --instructions--

미디어 쿼리의 `:root` 선택자에서 `--penguin-size`이 다시 정의되고 `200px` 값을 갖도록 변경하세요. 또한 `--penguin-skin`도 다시 정의하고 `black` 값을 할당하세요. 그런 다음 미리보기 크기를 조절하여 이 변경 사항이 적용되는 것을 확인하세요.

# --hints--

`:root`는 `--penguin-size` 변수를 `200px`로 다시 할당해야 합니다.
...
`:root`는 `--penguin-skin` 변수를 `black`로 다시 할당해야 합니다.
