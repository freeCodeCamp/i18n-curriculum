...
`cartTaxes` 要素の `textContent` プロパティにアクセスしてください。

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

`tax` 変数の `.toFixed()` メソッドを呼び出し、`2` を引数として渡してください。
...
`cartTotal` 要素の `textContent` プロパティにアクセスしてください。

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

`total` 変数の `.toFixed()` メソッドを呼び出し、`2` を引数として渡してください。`this` キーワードを使うことを忘れないでください。
