...
由于你从玩家身上减去了生命值，你需要检查玩家的 `health` 是否小于或等于 `0`。 如果是的话，调用 `lose` 函数。

# --hints--

你的 `pick` 函数应该在 `else` 代码块中嵌套一个 `if` 语句。
...
你的 `if` 语句应该检查玩家的 `health` 是否小于或等于 `0`。

```js
assert.match(pick.toString(), /if\s*\(\s*health\s*<=\s*0\s*\)/);
```

你在 `else` 代码块中的 `if` 语句应该调用 `lose` 函数。
