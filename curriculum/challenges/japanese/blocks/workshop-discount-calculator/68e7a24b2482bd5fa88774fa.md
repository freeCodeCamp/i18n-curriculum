...
コードに `print(product)` の行があってはいけません。
...
コードに `discount = PercentageDiscount(10)` の行があってはいけません。
...
コードに `print(discount.apply_discount(product))` の行があってはいけません。
...
コードに `fixed_discount = FixedAmountDiscount(5)` の行があってはいけません。
...
コードに `print(fixed_discount.apply_discount(product))` の行があってはいけません。
