...
Оскільки ви викликаєте `fetchCardsData` і `initializeEventListeners` всередині конструктора, тепер потрібно визначити їх у класі. Створіть обидва як `private` методи. Переконайтеся, що `fetchCardsData` є асинхронним, оскільки він буде обробляти виклики API.

# --hints--

У вас має бути `private` метод з назвою `fetchCardsData`.
...
Ваш `fetchCardsData` метод має бути `async`.
...
У вас має бути `private` метод з назвою `initializeEventListeners`.
