...
Ora hai qualcosa che somiglia a un edificio. Sei pronto per creare la tua prima variabile. Nelle lezioni precedenti hai imparato che le dichiarazioni di variabili iniziano con due trattini (`-`) e ricevono un nome e un valore così: `--variable-name: value;`. Nella regola per la classe `bb1`, crea una variabile chiamata `--building-color1` e assegnale un valore di `#999`.
...
Dovresti assegnare a `--building-color1` un valore di `#999`.
