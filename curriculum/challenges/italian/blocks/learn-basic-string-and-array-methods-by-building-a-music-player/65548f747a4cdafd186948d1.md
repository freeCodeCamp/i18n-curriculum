...
Usa `getElementById()` per ottenere il `id` della canzone attualmente in riproduzione, poi usa i template literal per aggiungergli come prefisso `song-`. Assegna il risultato alla costante `songToHighlight`.
...
Dovresti assegnare il tuo `getElementById()` a una costante `songToHighlight`.
