...
Hai bisogno di un modo per consultare in sicurezza l'elemento select e proteggerti dai valori `null`. Per questo, crea una costante `val` con il valore `e.currentTarget` e tipala `HTMLSelectElement` usando la parola chiave `as`.

Sotto la costante, usa un'istruzione `if` per verificare se `val` è falsy negandolo. Nel blocco `if`, restituisci `target value not found`.

# --hints--

Dovresti avere una costante chiamata `val`.

```js
assert.match(code, /const\s+val/)
```

La tua costante `val` dovrebbe avere un valore `e.currentTarget` tipato `HTMLSelectElement` usando la parola chiave `as`.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

Dovresti avere un'istruzione `if` che verifica se `val` è null.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

La tua istruzione `if` dovrebbe restituire `target value not found`.
