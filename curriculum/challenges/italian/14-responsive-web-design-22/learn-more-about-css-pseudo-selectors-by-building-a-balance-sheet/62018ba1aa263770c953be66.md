...
Il selettore `table caption` dovrebbe avere la proprietà `position` impostata su `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('position') === 'absolute');
```

Il selettore `table caption` dovrebbe avere la proprietà `top` impostata a `-2.25rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('top') === '-2.25rem');
```

Il selettore `table caption` dovrebbe avere la proprietà `left` impostata a `0.5rem`.
