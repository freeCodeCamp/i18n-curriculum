...
`"click"` 요소에 `previousButton` 이벤트 리스너를 추가한 다음, 두 번째 인수로 `playPreviousSong`를 전달하세요.

# --hints--

`addEventListener()` 변수에서 `previousButton` 메서드를 호출해야 합니다.
...
`playPreviousSong` 메서드의 두 번째 인수로 `addEventListener`를 전달해야 합니다.
