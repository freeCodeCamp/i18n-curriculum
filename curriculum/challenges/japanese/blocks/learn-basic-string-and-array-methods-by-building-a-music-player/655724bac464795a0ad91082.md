...
次に、プレイリストが空かどうかを確認する必要があります。空の場合は、`userData` オブジェクトを元の状態にリセットしてください。

`if` 文を使って、`userData?.songs` の長さが `0` であるかどうかを確認してください。

# --hints--

条件 `if` を使った空の `userData?.songs.length === 0` 文を作成してください。
