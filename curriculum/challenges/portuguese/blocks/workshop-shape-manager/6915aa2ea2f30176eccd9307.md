...
Use o evento `oninput` em `shapeTypeSelect` e defina-o como `handleShapeSelect`. Isso fará com que a função seja executada toda vez que a seleção da forma mudar.

Depois de fazer isso, você deverá conseguir ver os campos de entrada de cada forma. Mas os cálculos ainda não funcionarão, e é isso que você vai resolver a seguir.

# --hints--

Você deve definir `shapeTypeSelect.oninput` para `handleShapeSelect`.
