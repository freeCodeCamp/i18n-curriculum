...
関数呼び出しのすべての引数がキーワード引数である限り、引数の順序は問題になりません。

これを確認するために、関数呼び出しの中で`length=8`と`nums=1`の順序を変えてみてください。

# --hints--

`length=8`呼び出しの中で`nums=1`と`generate_password()`の順序を変えてみてください。
