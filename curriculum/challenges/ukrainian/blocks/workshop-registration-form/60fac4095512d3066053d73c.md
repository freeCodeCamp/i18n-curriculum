...
Додати випадаючий список у форму легко за допомогою елемента `select`. Елемент `select` є контейнером для групи елементів `option`, а елемент `option` виконує роль мітки для кожного варіанту випадаючого списку. Обидва елементи потребують кінцевих тегів.

Почніть із додавання елемента `select` під останнім елементом `label`. Потім вкладіть 5 елементів `option` всередину елемента `select`.
...
Ви повинні вкласти 5 елементів `option` всередину елемента `select`.
