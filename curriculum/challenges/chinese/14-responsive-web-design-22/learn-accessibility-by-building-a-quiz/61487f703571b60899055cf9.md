...
将 `section` 元素的宽度设置为其父容器的 `80%`。 然后使用 margin 来居中 `section` 元素，将底部 margin 设置为 `10px`。

另外，确认 `section` 元素的宽度不能大于 `600px`。
