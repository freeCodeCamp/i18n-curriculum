...
Erstellen Sie vor Ihrer `for`-Schleife eine `if`-Anweisung, die den String `=== MOVIES ===` (endet mit einem Zeilenumbruch) zum aktuellen Wert von `result` hinzufügt, wenn die `movies`-Liste nicht leer ist.

# --hints--

Sie sollten eine `if`-Anweisung in Ihrer `__str__`-Methode haben, die überprüft, ob die `movies`-Liste nicht leer ist.
...
Sie sollten `result += '=== MOVIES ===\n'` innerhalb Ihrer `if`-Anweisung haben.
