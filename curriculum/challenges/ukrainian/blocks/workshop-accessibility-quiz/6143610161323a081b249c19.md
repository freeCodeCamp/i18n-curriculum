...
Потім у кожному `div` вкладіть по одному `label` елементу та одному `input` елементу.

# --hints--

Ви повинні вкласти три `div` елементи під `h2#student-info` елемент.
...
У першому `label` слід вкласти один `div` елемент.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.querySelectorAll('label')?.length, 1);
```

У першому `input`, після `div`, слід вкласти один `label` елемент.
...
У другому `label` слід вкласти один `div` елемент.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.querySelectorAll('label')?.length, 1);
```

У другому `input`, після `div`, слід вкласти один `label` елемент.
...
У третьому `label` слід вкласти один `div` елемент.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.querySelectorAll('label')?.length, 1);
```

У третьому `input`, після `div`, слід вкласти один `label` елемент.
