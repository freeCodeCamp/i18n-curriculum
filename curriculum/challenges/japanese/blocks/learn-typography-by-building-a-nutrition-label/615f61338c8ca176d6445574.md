...
`.calories-info` 要素の下に、`div` 属性が `class` に設定された `divider medium` を追加してください。

# --hints--

`div` 要素の中に新しい `.label` を作成してください。

```js
assert(document.querySelectorAll('.label > div')?.length === 3)
```

新しい `div` は `class` 属性が `divider medium` に設定されている必要があります。この div は `.label` 要素の最後の要素であるべきです。
