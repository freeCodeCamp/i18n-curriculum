...
`sticky`-Positionierung ist ein Hybrid aus der `relative`- und `fixed`-Positionierung. Sie ermöglicht es einem Element, an einer bestimmten Position innerhalb seines Elements oder Viewports zu **haften**, basierend auf der Scrollposition.
...
**Note**: To see how `sticky` works, you have to place a couple of texts before and after your `.cat-head` element. Wenn du anschließend nach unten scrollst, siehst du, dass der `.cat-head` oben feststeckt und dort bleibt.
