...
خارج جملة `if`، أضف `number` إلى `sum_of_even_digits`. أيضًا، احذف استدعاء `print`.

# --hints--

يجب أن يكون لديك `sum_of_even_digits += number` داخل حلقة `for`.
...
يجب ألا يكون لديك `print(number)` داخل حلقة `for`.
...
يجب ألا يكون لديك `sum_of_even_digits += number` داخل جملة `if`.
