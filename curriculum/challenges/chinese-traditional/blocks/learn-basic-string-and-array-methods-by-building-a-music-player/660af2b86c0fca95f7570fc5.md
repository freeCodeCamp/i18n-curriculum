...
如果箭頭函式傳回的是簡單的表達式，則你可以省略 `return` 關鍵字和大括號 `{}`。這稱為隱式傳回。

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

如果你的箭頭函式在函式主體中有多行程式碼，則你需要使用 `return` 關鍵字和大括號 `{}`。
...
重構或更新你的 `addTwoNumbers` 函式，移除 `return` 關鍵字和大括號 `{}`。你的 `addTwoNumbers` 函式應該改用隱式傳回。

打開主控台以確保你仍然獲得正確的輸出。

# --hints--

你重構後的 `addTwoNumbers` 函式應該使用隱式傳回。
