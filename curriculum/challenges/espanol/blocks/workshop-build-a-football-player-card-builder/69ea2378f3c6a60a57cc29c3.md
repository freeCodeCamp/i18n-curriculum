...
Ahora recorre el arreglo `POSITIONS` usando una función flecha que reciba cada cadena de posición como `pos` y devuelva un elemento `option`. La opción debe tener un `key` de `pos` y el valor también `pos`. Además, muestra `pos` como el texto de cada `option` generado.

Puedes usar cualquier nombre como parámetro de tu map, no tiene que ser `pos`.

# --hints--

Tu `select` de posición debe contener elementos `option` mapeados desde el arreglo `POSITIONS`.
...
El `value` de cada opción debe coincidir con su texto mostrado y ser una posición válida.
