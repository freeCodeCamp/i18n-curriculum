...
في هذه الورشة، ستطبق مفاهيم البرمجة الوظيفية التي تعلمتها في الدروس السابقة لبناء تطبيق محول المكونات.

تم توفير HTML و CSS لك بالفعل.

ابدأ بإعلان كائن `conversionTable` فارغ.

# --hints--

يجب أن تعلن عن متغير `conversionTable`.

```js
assert.isDefined(conversionTable);
```

يجب أن يكون متغير `conversionTable` كائنًا.

```js
assert.isObject(conversionTable);
```

يجب أن يكون كائن `conversionTable` فارغًا.
