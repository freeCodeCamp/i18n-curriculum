...
`#results` 要素の中で、三項演算子を使って `results` 配列の長さがゼロより大きいかどうかを確認してください。もしそうなら、アイテムをマップして、それぞれをクラスが `p` の `result-item` 要素に表示してください。各段落には `key` 属性も必ず含めてください。

`results` が空の場合は、`No results found` というメッセージを含む `p` 要素をレンダリングしてください。

# --hints--

`#results` コンテナには、クエリに基づいた一致する果物のリストが含まれているはずです。各結果はクラスが `p` の `result-item` 要素で、果物の名前を表示してください。
...
`#results` 要素には、`p` 配列が空の場合にテキスト `No results found` を含む `results` 要素が含まれているはずです。
