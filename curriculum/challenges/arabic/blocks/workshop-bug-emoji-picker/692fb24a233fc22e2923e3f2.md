...
بعد دالة البناء، أنشئ طريقة `render` مجردة لها نوع إرجاع `void`. ستستخدمها في الأصناف الفرعية لتحديث الرموز التعبيرية المعروضة.

# --hints--

يجب أن يحتوي صنف `Bug` على طريقة `render` مجردة.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

يجب أن تكون طريقة `render` لديك ذات نوع إرجاع `void`.
