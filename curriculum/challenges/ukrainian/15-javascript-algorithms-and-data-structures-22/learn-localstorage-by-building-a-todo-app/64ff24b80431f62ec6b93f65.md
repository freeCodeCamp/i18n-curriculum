...
Видаліть масив `myTaskArr` та весь код для `localStorage`, оскільки вони вам більше не потрібні.

# --hints--

Видаліть `myTaskArr` та весь код, пов’язаний з `localStorage`, який ви щойно вивчили.
...
Видаліть будь-які посилання на `myTaskArr`, що залишилися, будь-де в коді.

```js
assert.notMatch(code, /myTaskArr/);
```

Видаліть будь-які посилання на `localStorage.getItem` для елемента `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Видаліть будь-які посилання на `localStorage.clear()` з коду.

```js
assert.notMatch(code, /localStorage\.clear\(\s*\)\s*;?/);
```

Видаліть будь-які посилання на `getTaskArrObj`, що залишилися, будь-де в коді.
