...
장바구니에 담긴 총 아이템 수에 접근할 방법이 필요합니다. `ShoppingCart` 배열에 직접 접근하기보다는 `items` 클래스에 또 다른 메서드를 추가하는 것이 가장 좋습니다.

`getCounts` 클래스에 `ShoppingCart` 메서드를 추가하세요. 이 메서드는 `items` 배열에 담긴 아이템 수를 반환해야 합니다.

# --hints--

`ShoppingCart` 클래스에 `getCounts` 메서드가 있어야 합니다.
...
`getCounts` 메서드는 `items` 배열에 담긴 아이템 수를 반환해야 합니다.
