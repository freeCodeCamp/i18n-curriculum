...
次に、`width`から`height`と`.bb2a`を削除し、`border-left`と`border-right`を`5vw`ではなく`1vw`を使うように変更してください。これで要素のサイズがゼロになり、ボーダーが中央で重なります。

# --hints--

`width`から`.bb2a`を削除してください。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.width);
```

`height`から`.bb2a`を削除してください。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.height);
```

`border-left`を`5vw`を使うように変更してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid rgb(153, 153, 153)");
```

`border-right`を`5vw`を使うように変更してください。
