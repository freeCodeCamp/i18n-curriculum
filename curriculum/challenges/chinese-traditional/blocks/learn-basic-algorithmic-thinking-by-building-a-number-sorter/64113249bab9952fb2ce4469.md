...
如果你再次按下 `Sort` 按鈕，你應該會看到 `10` 現在位於 `Output` 的正確位置。 

要完成這個專案，將你的 `option` 改回 `value` 和 `1` 的文字。

# --hints--

你應該將被 `selected` 的 `option` 元素的 `value` 更新為 1。
...
你應該將 `selected` 的 `option` 元素的文字更新為 `1`。
