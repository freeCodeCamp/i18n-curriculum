...
Debes crear un selector `75%` en tu regla `@keyframes cabins`.
...
Tu selector `75%` debe encontrarse entre los selectores `50%` y `100%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText === '75%');
```

Tu selector `75%` debe tener la propiedad `background-color` establecida en `yellow`.
