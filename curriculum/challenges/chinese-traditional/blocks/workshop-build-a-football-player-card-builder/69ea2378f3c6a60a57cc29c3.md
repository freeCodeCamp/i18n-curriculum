...
現在使用箭頭函式對 `POSITIONS` 陣列進行映射，該函式接收每個位置字串作為 `pos`，並傳回一個 `option` 元素。該選項應該有一個 `key` 為 `pos`，且 value 也為 `pos`。同時，將 `pos` 顯示為每個產生的 `option` 的文字。

你可以自由使用任何參數名稱作為 map 的參數，不一定要是 `pos`。

# --hints--

你的位置 `select` 應該包含從 `option` 陣列映射而來的 `POSITIONS` 元素。
...
每個選項的 `value` 應該與其顯示的文字相符，且是有效的位置。
