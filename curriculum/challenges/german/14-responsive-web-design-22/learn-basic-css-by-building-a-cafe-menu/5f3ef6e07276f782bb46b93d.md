...
Du solltest ein öffnendes `<footer>`-Tag haben.

```js
assert.match(code,(/<footer>/i));
```

Du solltest ein schließendes `</footer>`-Tag haben.

```js
assert.match(code,(/<\/footer>/i));
```

Du solltest nicht das vorhandene `main`-Element ändern.

```js
assert.lengthOf(document.querySelectorAll('main'), 1);
```

Dein `footer`-Element sollte unterhalb deines `main`-Elements stehen.
