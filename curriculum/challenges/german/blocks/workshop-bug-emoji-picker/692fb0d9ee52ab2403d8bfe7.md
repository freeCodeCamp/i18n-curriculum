...
Erstellen Sie als Nächstes einen Konstruktor für die `Bug`-Klasse, der einen einzelnen Parameter namens `emojiElement` vom Typ `HTMLParagraphElement` entgegennimmt. Weisen Sie im Konstruktor den `emojiElement`-Parameter der `emojiElement`-Eigenschaft der Klasse zu.

# --hints--

Ihre `Bug`-Klasse sollte einen Konstruktor haben.
...
Ihr `Bug`-Konstruktor sollte einen Parameter namens `emojiElement` vom Typ `HTMLParagraphElement` haben.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Ihr `Bug`-Konstruktor sollte `emojiElement` `this.emojiElement` zuweisen.
