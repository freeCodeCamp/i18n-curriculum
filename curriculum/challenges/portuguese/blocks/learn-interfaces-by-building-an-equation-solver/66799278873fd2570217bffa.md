...
Adicione outro `case` para quando o tamanho de `results` for `1`. Neste caso, atribua a `result_list` uma lista contendo uma string com o formato `x = <root>`, onde `<root>` é a solução da equação. Formate a string para que tanto o sinal positivo quanto o negativo sejam exibidos para a solução.
...
Você deve criar um novo `case` com o padrão `1` após o bloco `case` existente.
...
Você deve atribuir uma lista contendo `f'x = {results[0]:+}'` a `result_list` dentro do seu novo corpo `case`.
