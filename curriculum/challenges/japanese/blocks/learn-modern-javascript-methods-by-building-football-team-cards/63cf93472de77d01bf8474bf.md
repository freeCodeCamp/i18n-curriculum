...
headCoach オブジェクトの中に、`coachName` というキーと文字列 `"Carlos Bilardo"` の値を持つプロパティを追加してください。 さらにそのプロパティの下に、`matches` というキーと数値 `7` を追加してください。

# --hints--

`headCoach` オブジェクトに `coachName` プロパティを追加してください。

```js
assert.property(myFavoriteFootballTeam.headCoach, 'coachName');
```

`coachName` プロパティの値は `"Carlos Bilardo"` にしてください。

```js
assert.equal(myFavoriteFootballTeam.headCoach.coachName, 'Carlos Bilardo');
```

`headCoach` オブジェクトに `matches` プロパティを追加してください。

```js
assert.property(myFavoriteFootballTeam.headCoach, 'matches');
```

`matches` プロパティの値は数値にしてください。

```js
assert.isNumber(myFavoriteFootballTeam.headCoach.matches);
```

`matches` プロパティの値は `7` にしてください。
