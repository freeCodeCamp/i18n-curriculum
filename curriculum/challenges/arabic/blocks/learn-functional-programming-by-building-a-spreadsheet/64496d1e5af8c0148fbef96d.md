...
أعلن الآن عن متغير `length` وعيّنه بطول المصفوفة المرتبة، وعن متغير `middle` بقيمة الطول مقسومًا على `2` ناقصًا `1`.

# --hints--

يجب إعلان متغير `length` بعد متغير `sorted`.
...
يجب استخدام `const` لإعلان متغير `length`.
...
يجب تعيين طول المصفوفة `sorted` إلى متغير `length`.
...
يجب إعلان متغير `middle` بعد متغير `length`.
...
يجب استخدام `const` لإعلان متغير `middle`.
...
يجب تعيين قيمة قسمة متغير `length` على `2` إلى `middle`.
