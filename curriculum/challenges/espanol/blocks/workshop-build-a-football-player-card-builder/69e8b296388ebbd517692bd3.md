...
Tu primer elemento `div` de `className` de `form-group` debería tener un `label` con un atributo `htmlFor` de `pac`, un `className` de `label` y el texto `PAC`.
...
Tu segundo elemento `div` de `className` de `form-group` debería tener un `label` con un atributo `htmlFor` de `sho`, un `className` de `label` y el texto `SHO`.
