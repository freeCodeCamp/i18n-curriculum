...
Überprüfen Sie innerhalb der Schleife, ob der Autor des aktuellen Eintrags (in Kleinbuchstaben umgewandelt) `searchTerm` enthält. Die `.includes()`-Methode gibt `true` zurück, wenn ein String einen anderen String als Teilstring enthält. Wenn die Bedingung wahr ist, fügen Sie den Eintrag in `results` ein. Geben Sie nach der Schleife `results` zurück.

# --hints--

`findByAuthor(catalog, "king").length` sollte `6` sein.

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

`findByAuthor(catalog, "asimov").length` sollte `2` sein.

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

`findByAuthor(catalog, "xyz").length` sollte `0` sein.
