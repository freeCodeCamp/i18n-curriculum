...
回到頁面樣式設計。在導覽列中選取列表元素，並為它們設定以下樣式：
...
你應該使用 `nav li` 或 `nav > ul > li` 選擇器其中之一。
...
你應該給 `li` 元素設定 `0 0.2rem` 的 `margin`。
