...
更新你的 `isSpam()` 函数以在 `helpRegex` 上使用 `.test()` 方法来测试 `msg` 是否匹配。

然后，尝试在你的页面上输入一些消息并查看结果。

# --hints--

你的 `isSpam()` 函数应该使用 `.test()` 方法。
...
你的 `isSpam()` 函数应该在 `helpRegex` 上调用 `.test()` 方法，并以 `msg` 作为参数。
...
你的 `isSpam()` 函数应该返回 `helpRegex.test(msg)` 的结果。
