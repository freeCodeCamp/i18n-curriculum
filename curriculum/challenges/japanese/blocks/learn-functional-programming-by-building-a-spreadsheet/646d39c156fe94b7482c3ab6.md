...
セル参照にマッチする変数`cellRegex`を宣言してください。`A`から`J`までの文字、続いて`1`から`9`までの数字、そして`0`から`9`までの数字がオプションで続くものにマッチする必要があります。正規表現は大文字・小文字を区別せず、グローバルにしてください。

# --hints--

`cellRegex`変数を宣言してください。
