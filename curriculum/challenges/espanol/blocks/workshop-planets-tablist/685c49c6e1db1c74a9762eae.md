...
Si miras tu HTML, la primera `tab` ya está seleccionada por defecto y tiene un atributo `aria-selected` establecido en `"true"`. Cuando un usuario hace clic en una pestaña diferente, todas las pestañas deben establecerse primero con `aria-selected="false"`.
