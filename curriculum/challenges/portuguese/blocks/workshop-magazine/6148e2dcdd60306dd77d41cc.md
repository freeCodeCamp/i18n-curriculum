...
Hora de estilizar a última seção da revista - as imagens.

As imagens são envolvidas por um elemento `aside` usando a classe `image-wrapper`, então crie um seletor `.image-wrapper`. Defina a propriedade `display` como `grid`.

# --hints--

Você deve ter um seletor `.image-wrapper`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

O seletor `.image-wrapper` deve ter a propriedade `display` definida como `grid`.
