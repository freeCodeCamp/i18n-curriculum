...
Il selettore `tr.data td` dovrebbe avere una proprietà `vertical-align` impostata su `top`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td')?.getPropertyValue('vertical-align') === 'top');
```

Il selettore `tr.data td` dovrebbe avere una proprietà `text-align` impostata su `right`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td')?.getPropertyValue('text-align') === 'right');
```

Il selettore `tr.data td` dovrebbe avere una proprietà `padding` impostata su `0.3rem 0.25rem 0`.
