...
Кожен `fieldset` міститиме істинні або хибні висловлювання.

Усередині кожного `fieldset` вкладіть один `legend` елемент і один `ul` елемент із двома варіантами.

# --hints--

Ви повинні вкласти один `legend` елемент у перший `fieldset` елемент.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(1) > fieldset > legend')?.length, 1);
```

Ви повинні вкласти один `ul` елемент у перший `fieldset` елемент.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(1) > fieldset > ul')?.length, 1);
```

Ви повинні вкласти два `li` елементи у перший `ul` елемент.

```js
assert.equal(document.querySelectorAll('fieldset > ul')?.[0]?.querySelectorAll('li')?.length, 2);
```

Ви повинні вкласти один `legend` елемент у другий `fieldset` елемент.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(2) > fieldset > legend')?.length, 1);
```

Ви повинні вкласти один `ul` елемент у другий `fieldset` елемент.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(2) > fieldset > ul')?.length, 1);
```

Ви повинні вкласти два `li` елементи у другий `ul` елемент.
