...
你想為接下來的兩個區段添加相同的漸層。與其那樣做，不如創建一個名為 `bb1-window` 的新類別選擇器，並將 `.bb1a` 中的 `height` 和 `background` 屬性及其值移動到新的類別選擇器。

# --hints--

你應該創建一個名為 `bb1-window` 的新類別選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

你應該將 `height` 屬性和值從 `.bb1a` 移動到 `.bb1-window`。
...
你應該將 `background` 屬性和值從 `.bb1a` 移動到 `.bb1-window`。
