...
Überprüfen Sie innerhalb der `displayStory`-Funktion, ob das `storyObj`-Objekt eine Eigenschaft hat, die mit dem `genre`-Parameter übereinstimmt, und setzen Sie den Textinhalt des `#result`-Absatzes auf die Geschichte des ausgewählten Genres.

Setzen Sie außerdem die Rahmenfarbe von `storyContainer` auf die `borderColor`-Eigenschaft des ausgewählten Genres.

Testen Sie dies, indem Sie die `displayStory`-Funktion mit dem `scary`-Genre aufrufen. Die Rahmenfarbe sollte `#ee4b2b` sein und die Geschichte `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.

# --hints--

Sie sollten die `displayStory`-Funktion mit dem `"scary"`-Genre aufrufen.

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

Wenn Sie `displayStory("scary")` aufrufen, sollte die Rahmenfarbe `#ee4b2b` sein.
...
Wenn Sie `displayStory("scary")` aufrufen, sollte die angezeigte Geschichte `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` sein.
