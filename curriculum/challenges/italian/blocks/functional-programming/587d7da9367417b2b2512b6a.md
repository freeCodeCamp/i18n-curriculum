...
title: "Restituire un array ordinato senza modificare l'array originale"
challengeType: 1
forumTopicId: 301237
...
Un effetto collaterale del metodo `sort` è che modifica l'ordine degli elementi nell'array originale. In altre parole, muta l'array sul posto. Un modo per evitare questo è prima concatenare un array vuoto a quello da ordinare (ricorda che `slice` e `concat` restituiscono un nuovo array), quindi eseguire il metodo `sort`.
...
`nonMutatingSort(globalArray)` non dovrebbe essere codificato in modo fisso.

```js
assert(!nonMutatingSort.toString().match(/\[.*?[23569].*?\]/gs));
```

La funzione dovrebbe restituire un nuovo array, non l'array passato.
