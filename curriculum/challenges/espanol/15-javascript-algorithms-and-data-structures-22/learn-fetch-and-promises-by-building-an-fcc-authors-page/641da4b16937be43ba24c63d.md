...
Para empezar a trabajar con los datos, necesitarás utilizar otro método `.then()`.

Encadena otro `.then()` al método `.then()` existente. Esta vez, pasa `data` como parámetro para la función de callback. Para el callback, usa llaves porque tendrás más de una expresión. Dentro de tu función de callback, registra `data` en la consola para ver cómo se ve.
...
Deberías utilizar el método `fetch()` para hacer una solicitud `GET` a `"https://cdn.freecodecamp.org/curriculum/news-author-page/authors.json"`.

```js
assert.match(code, /fetch\(\s*('|"|`)https:\/\/cdn\.freecodecamp\.org\/curriculum\/news\-author\-page\/authors\.json\1\s*\)/)
```

Tu primer `.then()` no debería tener punto y coma.
...
Tu segundo `.then()` no debería tener punto y coma después de él.
