...
`.penguin` 要素のスタックレベルを変更して、`.ground` 要素の前に表示されるようにし、`position` を `relative` に設定してください。

# --hints--

スタックレベルを変更するには `z-index` プロパティを使う必要があります。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex);
```

`.penguin` 要素に `z-index` を `4` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex, '4');
```

`.penguin` に `position` プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin')?.position);
```

`.penguin` に `position` を `relative` に設定してください。
