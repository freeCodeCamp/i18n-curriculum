...
In manchen Fällen müssen die Karten umgedreht werden, da sie nicht immer in der richtigen Position sind.

Deklarieren Sie eine Variable namens `isReversed` innerhalb der `singleCardSelected`-Methode. Verwenden Sie die `Math.random()`-Methode, um einen zufälligen Boolean-Wert zu erzeugen. Um sicherzustellen, dass der Wert im zulässigen Bereich bleibt, vergleichen Sie ihn mit `0.5`.
