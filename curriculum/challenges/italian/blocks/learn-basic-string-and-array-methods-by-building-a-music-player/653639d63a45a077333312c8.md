...
Prima di far partire la canzone, devi assicurarti che inizi dall'inizio. Questo si può ottenere usando la proprietà `currentTime` sull'oggetto `audio`.

Aggiungi un'istruzione `if` per verificare se `userData?.currentSong` è falsy *OPPURE* se `userData?.currentSong.id` non è strettamente uguale a `song.id`. Questa condizione controllerà se non c'è nessuna canzone in riproduzione o se la canzone attuale è diversa da quella che sta per essere riprodotta.

All'interno del blocco `if`, imposta la proprietà `currentTime` dell'oggetto `audio` su `0`.

# --hints--

Dovresti creare un'istruzione `if` con la condizione `userData?.currentSong === null || userData?.currentSong.id !== song.id`. Non dimenticare di includere il concatenamento opzionale.
...
Dovresti impostare `audio.currentTime` su `0` all'interno del tuo blocco `if`.
