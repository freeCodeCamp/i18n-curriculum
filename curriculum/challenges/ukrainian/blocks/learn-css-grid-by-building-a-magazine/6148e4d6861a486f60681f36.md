...
Зображення мають бути розташовані у макеті з двома стовпцями та трьома рядками.

Надайте селектору `.image-wrapper` множину властивостей `grid-template-columns`, встановлену в `2fr 1fr`, та множину властивостей `grid-template-rows`, встановлену в `repeat(3, min-content)`. Це дозволить нашим рядкам сітки змінювати висоту залежно від вмісту, а стовпцям залишатися фіксованої ширини на основі контейнера.

# --hints--

Ваш селектор `.image-wrapper` має мати властивість `grid-template-columns`, встановлену в `2fr 1fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns === '2fr 1fr');
```

Ваш селектор `.image-wrapper` має мати властивість `grid-template-rows`, встановлену в `repeat(3, min-content)`.
