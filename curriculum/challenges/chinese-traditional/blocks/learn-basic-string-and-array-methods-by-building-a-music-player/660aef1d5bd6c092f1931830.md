...
創建一個名爲 `addTwoNumbers` 的新變量併爲其分配一個箭頭函數。 這個箭頭函數應該採用兩個參數，分別稱爲 `num1` 和 `num2`。 在函數主體內部，返回 `num1` 與 `num2` 相加的表達式。

在 `addTwoNumbers` 函數下方，添加一個控制檯語句。 在該控制檯語句中，調用 `addTwoNumbers` 函數並傳入數字 `3` 和 `4` 作爲參數。
