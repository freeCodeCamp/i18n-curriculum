...
재생 중인 노래가 없을 때 이전 버튼을 누르면 첫 번째 노래가 재생됩니다. 이를 고치려면 `if` 함수 시작 부분에 `playPreviousSong` 문을 추가하세요. 현재 노래가 `null`인지 확인하고 `return` 문으로 함수 실행을 정지하세요.

# --hints--

`if` 함수 안에 `userData.currentSong === null`를 확인하는 `return` 문을 추가하고 `playPreviousSong` 문으로 함수 실행을 정지해야 합니다.
