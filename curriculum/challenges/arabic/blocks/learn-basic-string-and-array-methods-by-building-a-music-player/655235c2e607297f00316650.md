...
بعد ذلك، استخدم الخاصية `classList` والطريقة `add()` لإضافة الصنف `"playing"` إلى عنصر `playButton`. هذا سيبحث عن الصنف `"playing"` في ملف CSS ويضيفه إلى عنصر `playButton`.

لتشغيل الأغنية أخيرًا، استخدم الطريقة `play()` على المتغير `audio`. `play()` هي طريقة من واجهة برمجة تطبيقات الصوت في الويب لتشغيل ملف mp3.

# --hints--

يجب أن تستخدم الخاصية `classList` والطريقة `add` لإضافة الصنف `"playing"` إلى `playButton`.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

يجب أن تستخدم الطريقة `play()` على المتغير `audio`.
