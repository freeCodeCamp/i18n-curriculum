...
Badilisha rangi ya fonti ya `h1` kuwa `#f1be32`, na weka ukubwa wa fonti kuwa `min(5vw, 1.2em)`.

# --hints--

Unapaswa kutumia kichaguzi cha kipengele cha `h1`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1'));
```

Unapaswa kumpa `h1` `color` ya `#f1be32`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.color, 'rgb(241, 190, 50)');
```

Unapaswa kumpa `h1` `font-size` ya `min(5vw, 1.2em)`.
