...
`a` セレクターを作成し、`text-decoration` プロパティに `none` を、`color` プロパティに `linen` を設定してください。
...
`a` セレクターには `text-decoration` プロパティが `none` に設定されているはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration, 'none');
```

`a` セレクターには `color` プロパティが `linen` に設定されているはずです。
