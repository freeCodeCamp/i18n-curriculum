...
你還需要確認輸入是否有效。 聲明一個 `invalidInputMatch` 變量，並將其分配給使用 `currVal` 作爲參數調用 `isInvalidInput` 函數的結果。

# --hints--

你應該聲明一個 `invalidInputMatch` 變量。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=/);
```

你應該將調用 `isInvalidInput` 的結果分配給你的 `invalidInputMatch` 變量。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(/);
```

你應該將 `currVal` 作爲參數傳遞給 `isInvalidInput`。
