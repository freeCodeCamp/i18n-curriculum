...
Ora devi mostrare il titolo e l'artista della canzone corrente nel display del lettore.

Come primo passaggio, consulta gli elementi `#player-song-title` e `#player-song-artist`, responsabili di mostrare il titolo della canzone e l'artista, e archiviali nelle variabili `playingSong` e `songArtist`, rispettivamente.

# --hints--

Dovresti avere una variabile chiamata `playingSong`.

```js
assert.isDefined(playingSong);
```

La tua variabile `playingSong` dovrebbe contenere il valore dell'elemento `#player-song-title`.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

Dovresti avere una variabile chiamata `songArtist`.
...
La tua variabile `songArtist` dovrebbe contenere il valore dell'elemento `#player-song-artist`.
