...
Es wäre schön, unsere `getBooksByAuthor`-Funktion mit einem anderen Autor zu testen.

Beginnen Sie damit, die Nachricht `"\nList of books by James Clear:\n"` in der Konsole auszugeben.

Fügen Sie darunter `console.log()` einen weiteren `console.log()` hinzu. Rufen Sie innerhalb dieses `console.log()` die `getBooksByAuthor()`-Funktion mit `library` und `"James Clear"` als Argumente auf.

Jetzt sollten Sie alle Bücher dieses bestimmten Autors in der Konsole sehen.
...
Sie sollten einen `console.log(getBooksByAuthor(library, "James Clear"))` haben.
