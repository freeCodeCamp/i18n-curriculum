...
Agrega un atributo `value` de `player.club` al Club `input`. Después, añade un atributo `onChange` configurado como una función flecha que toma `e` como parámetro y llama a `setPlayer` con un spread de `player` y `club` establecido en `e.target.value`.

# --hints--

Tu Club `input` debe tener un `value` igual al club del jugador.
...
Tu Club `input` debe tener un manejador `onChange` que llame a `setPlayer` con un spread de `player` y `club` establecido en `e.target.value`.
