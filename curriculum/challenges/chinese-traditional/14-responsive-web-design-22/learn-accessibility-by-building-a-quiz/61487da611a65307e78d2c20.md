...
在小屏幕上，導航欄中的無序列表會溢出屏幕的右側。

通過使用 _Flexbox_ 來包含 `ul` 內容來解決此問題。 然後，設置以下 CSS 屬性以正確對齊文本：
