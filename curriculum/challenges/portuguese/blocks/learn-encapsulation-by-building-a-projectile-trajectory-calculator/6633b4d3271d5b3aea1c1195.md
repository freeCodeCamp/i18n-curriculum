...
Agora é hora do gráfico. Crie um método `create_trajectory` e substitua a última chamada `print` no final do seu código por `print(graph.create_trajectory())`.

Como o primeiro passo deste novo método, faça uma cópia local das coordenadas mas onde todos os valores estejam arredondados para inteiros. Salve esta nova versão das coordenadas em uma variável chamada `rounded_coords` e retorne esta variável.

# --hints--

Você deve definir um método chamado `create_trajectory`.
...
Você deve substituir `print(graph.create_coordinates_table())` por `print(graph.create_trajectory())`.
...
A função deve retornar `rounded_coords`.
...
A variável `rounded_coords` deve conter as coordenadas arredondadas.
