...
為了讓 Campers 知道該在個人簡介中填寫什麼，使用了 `placeholder` 屬性。`placeholder` 接受一個文字值，該文字會顯示直到使用者開始輸入為止。

為你的 `textarea` 添加 `placeholder` 為 `I like coding on the beach...`。

# --hints--

你應該為 `textarea` 添加 `placeholder` 屬性。

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

你應該給 `placeholder` 一個值為 `I like coding on the beach...`。
