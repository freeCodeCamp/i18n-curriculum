...
`Every month` means something happens once per month, regularly. 例如：
...
`Every two weeks` means something happens once every two weeks, or twice a month. 例如：
...
`Vigilant` means being watchful and careful to avoid risks or problems. 例如：
