...
クラスセレクターを使って、`.cat-right-inner-eye` 要素に幅 `10px` と高さ `20px` を指定してください。また、背景色を `#fff` に設定してください。

# --hints--

`.cat-right-inner-eye` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye'))
```

`.cat-right-inner-eye` セレクターには `width` を `10px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.width === '10px')
```

`.cat-right-inner-eye` セレクターには `height` を `20px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.height === '20px')
```

`.cat-right-inner-eye` セレクターには `background-color` を `#fff` に設定してください。
