...
`span[class~="sr-only"]` 選擇器將選擇任何 `span` 元素，其 `class` *包括* `sr-only`。 創建該選擇器，並將其 `border` 屬性設置爲 `0`。

# --hints--

應該有一個 `span[class~="sr-only"]` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]'));
```

`span[class~="sr-only"]` 選擇器應該有一個 `border` 屬性設置爲 `0`。
