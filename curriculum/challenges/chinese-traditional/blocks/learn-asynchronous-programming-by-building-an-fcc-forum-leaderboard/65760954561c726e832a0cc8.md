...
在你的 `allCategories` 物件中，為數字 299 添加一個新鍵，其值為空物件。

在該物件內，為你的物件添加一個鍵為 `category` 且值為字串 `"Career Advice"` 的屬性。在該屬性下方，添加另一個鍵為 `className` 且值為字串 `"career"`。

# --hints--

你的 `allCategories` 物件應該有一個 `299` 屬性。
