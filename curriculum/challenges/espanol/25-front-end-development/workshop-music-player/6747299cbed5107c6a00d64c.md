...
Las demás canciones se han añadido a la matriz `allSongs` para ti.

En el video de la conferencia anterior, aprendiste sobre la <dfn>API de Audio Web</dfn> y cómo usarla para reproducir canciones. Todos los navegadores modernos soportan la API de Audio Web, que permite generar y procesar audio en aplicaciones web.

Crea una variable llamada `audio` y asígnale `new Audio()`. Esto creará un nuevo elemento HTML5 `audio`.

# --hints--

Deberías tener una variable llamada `audio`.

```js
assert.isDefined(audio);
```

Debes asignar una nueva instancia del objeto `Audio` a la variable llamada `audio`.
