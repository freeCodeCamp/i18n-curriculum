...
Sia d(`n`) definito come la somma dei divisori propri di `n` (numeri minori di `n` che dividono esattamente `n`).

Se d(`a`) = `b` e d(`b`) = `a`, dove `a` ≠ `b`, allora `a` e `b` formano una coppia amicabile e ciascuno tra `a` e `b` è chiamato numero amicabile.

Per esempio, i divisori propri di 220 sono 1, 2, 4, 5, 10, 11, 20, 22, 44, 55 e 110; quindi d(220) = 284. I divisori propri di 284 sono 1, 2, 4, 71 e 142; quindi d(284) = 220.

Calcola la somma di tutti i numeri amicabili inferiori a `n`.
