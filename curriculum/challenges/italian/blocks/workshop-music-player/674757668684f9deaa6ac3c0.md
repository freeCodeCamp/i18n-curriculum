...
Sempre all'interno della funzione `playSong`, imposta la proprietà `audio.src` uguale a `song.src`. Questo indica all'elemento audio dove trovare i dati audio per la canzone selezionata.

Imposta anche la proprietà `audio.title` uguale a `song.title`. Questo indica all'elemento audio cosa mostrare come titolo della canzone.

# --hints--

Dovresti impostare la proprietà `audio.src` uguale a `song.src` all'interno della tua funzione `playSong`.
...
Dovresti impostare la proprietà `audio.title` uguale a `song.title` all'interno della tua funzione `playSong`.
