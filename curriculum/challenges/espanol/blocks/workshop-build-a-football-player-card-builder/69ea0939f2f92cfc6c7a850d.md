...
A continuación, agrega un hook `useEffect` que se ejecute cada vez que cambie el estado `player`. Dentro, usa una función flecha y añade un `try..catch` con `error` como parámetro del bloque `catch`.

# --hints--

Debes llamar a `useEffect` dentro de `FootballPlayerCard`.
...
Tu `useEffect` debe tener a `[player]` como su arreglo de dependencias.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

El callback `useEffect` debe contener un bloque `try...catch` con `error` como parámetro del bloque `catch`.
