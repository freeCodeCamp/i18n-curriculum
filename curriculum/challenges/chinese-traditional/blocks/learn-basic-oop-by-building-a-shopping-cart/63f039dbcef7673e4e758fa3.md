...
瀏覽器內建有一個 `confirm()` 函式（程式），會向使用者顯示確認提示。`confirm()` 接受一個字串，該字串是顯示給使用者的訊息。如果使用者確認，則傳回 `true`，如果使用者取消，則傳回 `false`。

宣告一個變數 `isCartCleared` 並指定其值為呼叫 `confirm()`，並傳入字串 `"Are you sure you want to clear all items from your shopping cart?"`。

# --hints--

你應該使用 `const` 來宣告 `isCartCleared` 變數。
...
你應該將字串 `Are you sure you want to clear all items from your shopping cart?` 傳遞給 `confirm()` 函式（程式）。

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(\s*('|"|`)Are you sure you want to clear all items from your shopping cart\?\1\s*\)/);
```

你應該將 `confirm()` 函式（程式）的值指定給 `isCartCleared` 變數。
