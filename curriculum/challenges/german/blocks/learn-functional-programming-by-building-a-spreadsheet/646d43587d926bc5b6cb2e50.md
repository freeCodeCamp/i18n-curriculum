...
Deklarieren Sie eine `str2`-Variable und weisen Sie ihr das Ergebnis des Aufrufs von `infixEval()` mit `noHigh` und `infix` als Argumente zu.
...
Sie sollten `str2` das Ergebnis des Aufrufs von `infixEval()` zuweisen.
...
Sie sollten `noHigh` als erstes Argument an `infixEval()` übergeben.
...
Sie sollten `infix` als zweites Argument an `infixEval()` übergeben.
