...
请注意，HTML 中的专辑封面和 `userData.songs` 数组中的歌曲已经发生了变化。 我们将原始歌曲替换为较短的歌曲，你可以在接下来的步骤中使用这些歌曲来测试你的应用程序。

接下来，你需要检查是否有下一首歌曲要播放。 通过调用 `getCurrentSongIndex()` 函数检索当前歌曲索引，并将其保存在 `currentSongIndex` 常量中。

之后，创建一个 `nextSongExists` 常量，其中包含布尔值 `true` 或 `false`，具体取决于下一首歌曲是否存在。

# --hints--

你不应修改现有的 `ended` 事件监听器及其内容。

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

你应该创建一个 `currentSongIndex` 常量并将其设置为 `getCurrentSongIndex` 函数的调用。
...
你应该比较 `userData.songs.length` 和 `currentSongIndex` 来检查是否存在下一首歌曲，并将其设置为 `nextSongExists` 常量。 如果歌曲数组的最后一个索引（`userData.songs.length - 1`）大于 currentSongIndex，则表示有下一首歌曲。
