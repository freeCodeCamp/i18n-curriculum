...
다음으로, 왼쪽 키가 눌렸는지 그리고 `else if`이 true인지 확인하는 `isCheckpointCollisionDetectionActive` 문을 추가하세요.

그 조건문 안에 `forEach` 배열을 순차적으로 반복하는 `platforms` 반복문을 추가하세요.

반복문 안에서 덧셈 대입 연산자를 사용해 플랫폼의 `x` 위치에 5를 더하세요.

# --hints--

왼쪽 키가 눌렸는지 그리고 `isCheckpointCollisionDetectionActive`이 true인지 확인하는 조건문이 있어야 합니다.
