...
Ahora actualiza tu función más interna en la cadena `addCharacters` para devolver implícitamente el resultado de llamar a `charRange()` con `character1` y `character2` como argumentos.

# --hints--

Tu función más interna debe usar un retorno implícito.
...
Tu función más interna debe devolver el resultado de llamar a `charRange()`.
...
Debes pasar `character1` como el primer argumento a tu llamada de `charRange()`.
...
Debes pasar `character2` como el segundo argumento a tu llamada de `charRange()`.
