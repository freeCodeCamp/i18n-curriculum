...
正如数字可以在位置符号中表示为十（十进制）或二（二进制）的幂的倍数之和；所有正整数都可以表示为斐波那契数列不同成员的 1 倍或 0 倍之和。 回想一下，前六个不同的斐波那契数是：`1, 2, 3, 5, 8, 13`。

十进制数十一可以写成 `0*13 + 1*8 + 0*5 + 1*3 + 0*2 + 0*1` 或 `010100` 的位置符号其中列表示与序列中特定成员的乘法。  Leading zeroes are dropped so that 11 decimal becomes `10100`. 10100 is not the only way to make 11 from the Fibonacci numbers however `0*13 + 1*8 + 0*5 + 0*3 + 1*2 + 1*1` or 010011 would also represent decimal 11. 对于真正的 Zeckendorf 数，还有一个附加限制，即*不能使用两个连续的斐波那契数*，这导致了上述的唯一解。
