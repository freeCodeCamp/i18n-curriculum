...
Zentrieren Sie das `.three`-Element auf dem Canvas, indem Sie seine `margin` auf `auto` setzen.

# --hints--

Sie sollten die `margin`-Eigenschaft auf `auto` setzen.
...
Ihr `.three`-Element sollte einen `margin`-Wert von `auto` haben.
