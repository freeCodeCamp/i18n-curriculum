...
القسم التالي من النموذج سيوفر للمستخدمين إمكانية ترك تقييم للفندق.

ابدأ بإضافة عنصر `fieldset` جديد بداخله عنصر `legend` مسكن. يجب أن يحتوي `legend` على النص `Ratings`.

أسفل عنصر `legend`، أضف عنصر `label` يحتوي على النص `How was the service?`. يجب تعيين الخاصية `for` إلى `"service"`.
