...
والآن، لا يمكننا قراءة النص. استهدف `footer` وعنصر الرابط بالداخل لتعيين لون الخط (font color) إلى لون بنسبة تباين (contrast ratio) مناسبة.

# --hints--

يجب عليك استخدام منتقي `footer, footer a`.
...
يجب عليك تعيين `color` إلى قيمة مع نسبة تباين لا تقل عن `7:1`. _تلميح: أقترح `#dfdfe2`_
