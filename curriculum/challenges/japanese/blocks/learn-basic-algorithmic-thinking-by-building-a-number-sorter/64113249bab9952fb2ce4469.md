...
`Sort` ボタンをもう一度押すと、`10` が `Output` の正しい位置にあるのが確認できます。

このプロジェクトを完成させるには、`option` を `value` に戻し、`1` のテキストを変更してください。

# --hints--

`value` で `option` である `selected` 要素の `1` を更新する必要があります。
...
`option` である `selected` 要素のテキストを `1` に更新してください。
