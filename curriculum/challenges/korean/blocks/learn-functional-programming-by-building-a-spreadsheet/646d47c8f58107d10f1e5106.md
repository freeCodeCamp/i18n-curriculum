...
`evalFormula` 호출의 첫 번째 인수는 셀의 내용이어야 합니다(`value`에 저장한 내용). 하지만, 내용이 `=` 문자로 시작하여 함수를 실행하므로 `value`의 `1` 인덱스부터 시작하는 부분 문자열을 전달해야 합니다.

# --hints--

`value` 호출의 첫 번째 인수로 `evalFormula()`를 전달하세요.
