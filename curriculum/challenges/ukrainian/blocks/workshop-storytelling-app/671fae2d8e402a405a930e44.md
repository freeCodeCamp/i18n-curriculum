...
Додайте слухача події для події `click` до `scaryStoryBtn` і передайте йому функцію `displayStory`. Потім натисніть на кнопку і подивіться вихідні дані в консолі.

# --hints--

Ви повинні додати слухача події для події `click` до `scaryStoryBtn`, передаючи йому функцію `displayStory`.
