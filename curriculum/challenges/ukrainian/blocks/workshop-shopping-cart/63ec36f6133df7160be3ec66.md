...
Пам’ятайте, що ви можете використовувати деструктуризацію, щоб витягти кілька значень із масиву або об’єкта в одній інструкції.

Для першого параметра вашої функції зворотного виклику деструктуруйте пропи `name`, `id`, `price` і `category` з переданого екземпляра `Dessert`.

# --hints--

Ви повинні використати деструктуризацію, щоб оголосити параметри `name`, `id`, `price` і `category`. Для цього тесту порядок має значення.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

Ваша деструктуризація має бути першим параметром функції зворотного виклику.
