...
في هذا المشروع ستستخدم البيانات من `https://cdn.freecodecamp.org/curriculum/labs/data/scatterplot/cyclist-data.json` لبناء مخطط انتشار.

تتطلب الاختبارات إنشاء المحاور باستخدام خاصية المحور في D3، التي تولد تلقائيًا علامات دقيقة على طول المحور. هذه العلامات ضرورية لاجتياز اختبارات D3 لأن مواقعها تُستخدم لتحديد محاذاة العناصر المرسومة.

**الهدف:** أتمم قصص المستخدم أدناه واجتز جميع الاختبارات لإكمال المختبر.

**قصص المستخدم:**

1. يجب أن يحتوي مخططك على عنوان مع `id="title"` مطابق.
1. يجب أن يحتوي مخططك على عنصر `g` لمحور x مع `id="x-axis"` مطابق.
1. يجب أن يحتوي مخططك على عنصر `g` لمحور y مع `id="y-axis"` مطابق.
1. يجب أن يحتوي كلا المحورين على عدة مسميات علامات دقيقة، كل منها مع `class="tick"` مطابق.
1. يجب أن يحتوي مخططك على عنصر `circle` لكل نقطة بيانات مع `class="dot"` يعرض البيانات.
1. يجب أن يحتوي كل `.dot` على الخاصيتين `data-xvalue` و `data-yvalue` اللتين تحتويان على قيم `year` و `time`. بالنسبة لـ `data-xvalue`، الأعداد الصحيحة (السنوات الكاملة) أو عناصر التاريخ مقبولة لتقييم الاختبار. بالنسبة لـ `data-yvalue` (الدقائق)، استخدم عناصر التاريخ.
1. يجب أن تتطابق خاصيات `data-xvalue` لعناصر `.dot` مع ترتيب البيانات المقدمة.
1. يجب أن تتطابق خاصيات `data-yvalue` لعناصر `.dot` مع ترتيب البيانات المقدمة.
1. يجب أن يتماشى كل عنصر `.dot` مع القيمة المقابلة على محور x.
1. يجب أن يتماشى كل عنصر `.dot` مع القيمة المقابلة على محور y.
1. يجب أن يكون هناك عدة مسميات علامات دقيقة على محور y بتنسيق وقت `%M:%S`.
1. يجب أن يكون هناك عدة مسميات علامات دقيقة على محور x تعرض السنة.
1. يجب أن يكون نطاق مسميات محور x ضمن نطاق بيانات محور x الفعلية.
1. يجب أن يكون نطاق مسميات محور y ضمن نطاق بيانات محور y الفعلية.
1. يجب أن يحتوي المخطط على وسيلة إيضاح تحتوي على نص وصفي مع `id="legend"`.
1. يجب أن تتمكن من تحريك مؤشر الفأرة فوق منطقة ورؤية تلميح مع `id="tooltip"` يعرض مزيدًا من المعلومات عن المنطقة.
1. يجب أن يحتوي التلميح على خاصية `data-year` التي تتوافق مع `data-xvalue` للمنطقة النشطة.
...
يجب أن يحتوي المخطط على عنصر بـ `id` يساوي `title` ليحتوي العنوان.

```js
assert.exists(document.getElementById('title'));
```

يجب أن يحتوي المخطط على `x-axis` مع `id="x-axis"` المقابل.
...
يجب أن يحتوي المخطط على `y-axis` مع `id="y-axis"` المقابل.
...
يجب أن يحتوي محور x على عدة مسميات علامات دقيقة، كل منها مع `class="tick"` مطابق.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

يجب أن يحتوي محور y على عدة مسميات علامات دقيقة، كل منها مع `class="tick"` مطابق.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

يجب أن يحتوي مخططك على نقاط، كل منها يحمل صنف `dot`، تمثل البيانات المرسومة.

```js
assert.isNotEmpty(document.querySelectorAll('circle.dot'));
```

يجب أن يحتوي كل نقطة على الخاصيتين `data-xvalue` و `data-yvalue` اللتين تحتويان على قيم x و y المقابلة.
...
يجب أن تكون خاصيتا `data-xvalue` و `data-yvalue` لكل نقطة ضمن نطاق البيانات الفعلية وبالصيغة الصحيحة. بالنسبة لـ `data-xvalue`، الأعداد الصحيحة (السنوات الكاملة) أو عناصر التاريخ مقبولة لتقييم الاختبار. بالنسبة لـ `data-yvalue` (الدقائق)، استخدم عناصر التاريخ.
...
يجب أن تتماشى خاصية `data-xvalue` والنقطة المقابلة معها مع النقطة/القيمة المقابلة على محور x.
...
يجب أن تتماشى خاصية `data-yvalue` والنقطة المقابلة معها مع النقطة/القيمة المقابلة على محور y.
...
يمكنك رؤية عدة مسميات علامات دقيقة على محور y بتنسيق وقت `%M:%S`.
...
يمكنك رؤية عدة مسميات علامات دقيقة على محور x تعرض السنة.
...
نطاق مسميات محور x ضمن نطاق بيانات محور x الفعلية.
...
نطاق مسميات محور y ضمن نطاق بيانات محور y الفعلية.
...
يمكنك رؤية وسيلة إيضاح تحتوي على نص وصفي مع `id="legend"`.
...
عند تحريك مؤشر الفأرة فوق منطقة، يجب أن يحتوي مخطط الانتشار على تلميح مع `id="tooltip"` يعرض مزيدًا من المعلومات عن المنطقة.
...
يجب أن يحتوي التلميح على خاصية `data-year` التي تتوافق مع `data-xvalue` للمنطقة النشطة.
