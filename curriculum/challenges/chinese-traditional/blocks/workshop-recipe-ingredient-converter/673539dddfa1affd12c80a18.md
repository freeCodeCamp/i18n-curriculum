...
讓我們測試你的新柯里化函式。宣告一個 `servingsResult` 變數，並將從基數量 4 調整到 6 份的結果指定給它。

將該變數記錄到主控台。

# --hints--

你應該宣告一個 `servingsResult` 變數。

```js
assert.isDefined(servingsResult);
```

你的 `servingsResult` 變數應該包含呼叫 `adjustQuantity(4)(6)` 的結果。

```js
assert.strictEqual(servingsResult, 24);
```

你應該記錄你的 `servingsResult` 值。
