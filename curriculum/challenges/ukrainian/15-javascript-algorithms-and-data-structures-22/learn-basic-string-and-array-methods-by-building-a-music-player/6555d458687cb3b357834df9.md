...
Використайте властивості `userData?.currentSong?.title` та `userData?.currentSong?.artist` і призначте їх змінним `currentTitle` та `currentArtist` відповідно.

# --hints--

Ви повинні використати `title` з `currentSong` з об’єкта `userData`. Не забудьте використати необов’язковий ланцюжок.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

Призначте `title` з `currentSong` до константи `currentTitle`.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

Ви повинні використати `artist` з `currentSong` з об’єкта `userData`. Не забудьте використати опціональну послідовність.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

Призначте `artist` з `currentSong` до константи `currentArtist`.
