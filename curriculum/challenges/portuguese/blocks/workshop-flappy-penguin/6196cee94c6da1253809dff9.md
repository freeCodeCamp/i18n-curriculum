...
Coloque em foco o elemento `.back-mountain` e defina `width` e `height` como `300px`.Depois, defina o `background` como um gradiente linear começando em `rgb(203, 241, 228)` e terminando em `rgb(47, 170, 255)`.

# --hints--

Você deve usar o seletor `.back-mountain`.

```js
assert.match(code, /\.back-mountain\s*\{/);
```

Você deve dar a `.back-mountain` uma `width` de `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

Você deve dar a `.back-mountain` uma `height` de `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

Você deve dar a `.back-mountain` um `background` de `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`.
