...
다음으로, 플레이어 화면에 현재 노래 제목과 아티스트를 표시해야 합니다.

첫 번째 단계로, 노래 제목과 아티스트를 표시하는 `#player-song-title`와 `#player-song-artist` 요소에 접근하여 각각 `playingSong`와 `songArtist` 변수에 저장하세요.

# --hints--

`playingSong`라는 변수가 있어야 합니다.

```js
assert.isDefined(playingSong);
```

`playingSong` 변수는 `#player-song-title` 요소의 값을 가져야 합니다.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

`songArtist`라는 변수가 있어야 합니다.

```js
assert.isDefined(songArtist);
```

`songArtist` 변수는 `#player-song-artist` 요소의 값을 가져야 합니다.
