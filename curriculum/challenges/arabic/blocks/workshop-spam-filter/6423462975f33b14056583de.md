...
يمكن لأصناف الأحرف أن تحتوي على أكثر من حرفين. استبدل الحرف `a` بصنف أحرف يطابق `a` و`@` و`4`.

# --hints--

يجب أن يستخدم `stockRegex` صنف أحرف لمطابقة `a` و`@` و`4`.
...
يجب أن يطابق `stockRegex` النص `stock @lert`.

```js
assert.match('stock @lert', stockRegex);
```

يجب أن يطابق `stockRegex` النص `stock 4lert`.

```js
assert.match('stock 4lert', stockRegex);
```

يجب أن يطابق `stockRegex` النص `570ck 4l3r7`.
