...
이 프로젝트를 마무리하는 마지막으로, `display` 선택자에 `none` 속성을 `section` 값으로 추가하세요.

그다음, `section:target` 선택자를 만들고, 링크를 클릭했을 때만 `display` 요소가 보이도록 `block` 속성을 `section` 값으로 추가하세요.

# --hints--

`section` 선택자에는 `display` 속성을 `none` 값으로 설정해야 합니다.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section")?.getPropertyValue("display"), "none");
```

`section:target` 선택자를 만들어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle("section:target"));
```

`section:target` 선택자에는 `display` 속성을 `block` 값으로 설정해야 합니다.
