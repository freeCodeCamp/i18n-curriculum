...
现在将 `PlayerCard` 组件包裹在 `main` 元素中，该元素属于 `className` 的 `main`。

在 `main` 元素内，添加 `div`，它属于 `className` 的 `layout`。在 `layout` 内，分别添加两个独立的 `div` 元素，它们分别属于 `className` 的 `form-panel` 和 `preview-panel`。

# --hints--

你的 `FootballPlayerCard` 应该渲染一个 `main` 元素，里面包含 `className` 的 `main`，该元素位于 `div.page` 内。
...
你的 `main.main` 应该包含一个 `div`，里面有 `className` 的 `layout`。
...
你的 `layout` 应该包含一个 `div`，里面有 `className` 的 `form-panel`。
...
你的 `layout` 应该包含一个 `div`，里面有 `className` 的 `preview-panel`。
