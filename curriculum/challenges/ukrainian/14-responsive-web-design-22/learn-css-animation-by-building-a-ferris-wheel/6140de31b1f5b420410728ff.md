...
Тепер надайте правилу `@keyframes wheel` селектор `100%`. Всередині встановіть `transform` на `rotate(360deg)`. Завдяки цьому анімація завершить повний оберт.

# --hints--

Ваше правило `@keyframes wheel` повинне мати селектор `100%`.
