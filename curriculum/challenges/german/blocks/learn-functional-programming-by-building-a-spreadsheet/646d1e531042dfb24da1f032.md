...
Das zweite Argument der `.replace()`-Methode muss kein String sein. Stattdessen können Sie eine Callback-Funktion übergeben, um komplexere Logik auf den gefundenen String anzuwenden.

Die Callback-Funktion erhält einige Parameter. Der erste ist der gefundene String. Übergeben Sie eine leere Callback-Funktion an Ihren `.replace()`-Aufruf und geben Sie ihr einen `match`-Parameter.

# --hints--

Sie sollten eine Pfeilfunktion als zweites Argument an Ihre `.replace()`-Methode übergeben.
...
Ihre Pfeilfunktion sollte einen `match`-Parameter annehmen.
