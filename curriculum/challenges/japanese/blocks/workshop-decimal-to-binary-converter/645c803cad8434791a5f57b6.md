...
変換結果を表示したいので、関数の最後で `binary` の `innerText` プロパティに `result` 変数を代入してください。

# --hints--

`binary` の `innerText` プロパティに `result` を代入してください。

```js
assert.match(String(decimalToBinary), /result\.innerText\s*=\s*binary\s*/);
```

関数の最後で `binary` の `innerText` プロパティに `result` を代入してください。
