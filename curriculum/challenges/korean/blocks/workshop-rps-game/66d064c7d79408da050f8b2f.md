...
다음 단계는 플레이어와 컴퓨터 점수를 추적하는 변수를 만드는 것입니다.

`playerScore`라는 변수를 만들고 값 `0`로 초기화하세요.

그다음 `computerScore`라는 변수를 만들고 값 `0`로 초기화하세요.

이 변수들은 게임 내내 값이 바뀌므로 `let`를 사용해 선언해야 합니다.

# --hints--

`playerScore`라는 변수가 있어야 합니다.

```js
assert.isNotNull(playerScore);
```

`let` 변수를 선언할 때 `playerScore`를 사용해야 합니다.

```js
assert.match(code, /let\s+playerScore/);
```

`playerScore`는 값 `0`로 초기화해야 합니다.

```js
assert.strictEqual(playerScore, 0);
```

`computerScore`라는 변수가 있어야 합니다.

```js
assert.isNotNull(computerScore);
```

`let` 변수를 선언할 때 `computerScore`를 사용해야 합니다.

```js
assert.match(code, /let\s+computerScore/);
```

`computerScore`는 값 `0`로 초기화해야 합니다.
