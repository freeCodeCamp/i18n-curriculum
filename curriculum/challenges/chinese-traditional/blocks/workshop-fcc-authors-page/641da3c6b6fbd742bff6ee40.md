...
<dfn>Fetch API</dfn> 是一個內建的 JavaScript 介面，用來向伺服器發出網路請求。它有一個 `fetch()` 方法，你可以用來發出 `GET`、`POST`、`PUT` 或 `PATCH` 請求。在這個專案中，你將對一個 URL 發出 `GET` 請求，以取得 freeCodeCamp News 上關於作者的 JSON 檔案資訊。

以下是你如何使用 `fetch()` 方法發出 `GET` 請求的方法：

```js
fetch("url-goes-here")
```

對此 URL 發出 `GET` 請求：`"https://cdn.freecodecamp.org/curriculum/news-author-page/authors.json"`。暫時不要用分號結束你的程式碼。
