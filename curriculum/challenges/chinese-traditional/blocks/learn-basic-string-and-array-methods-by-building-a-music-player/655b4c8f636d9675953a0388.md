...
如果播放列表中沒有下一首歌曲，請使用 `else` 區塊將 `userData` 的 `currentSong` 鍵重設為 `null`，並將其 `songCurrentTime` 屬性重設為 0。
...
你應該在你的 `if` 述語的 `else` 區塊中，使用點記法將 `userData` 的 `currentSong` 屬性設為 `null`。

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

你應該在你的 `if` 述語的 `else` 區塊中，使用點記法將 `userData` 的 `songCurrentTime` 屬性設為 `0`。
