...
沒有適當的引號，引用就不是真正的引用。你可以使用 CSS 偽選擇器為你的內容添加這些引號。

創建一個 `.quote::before` 選擇器，並將 `content` 屬性設為 `"`，後面加一個空格。

另外，創建 `.quote::after` 選擇器並將 `content` 屬性設為 `"`，前面加一個空格。
...
你的 `.quote::before` 選擇器應該有一個設定為 `'" '` 的 `content` 屬性。
...
你的 `.quote::after` 選擇器應該有一個 `content` 屬性設定為 `' "'`。
