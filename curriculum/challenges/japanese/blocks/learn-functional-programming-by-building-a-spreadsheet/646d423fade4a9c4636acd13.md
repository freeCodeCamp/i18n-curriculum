...
まずは優先順位の高い演算子を処理する必要があります。`noHigh` 変数を宣言し、`highPrecedence()` を引数として `str` を呼び出した結果を代入してください。
