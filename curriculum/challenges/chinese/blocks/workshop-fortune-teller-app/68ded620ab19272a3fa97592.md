...
由于项目现在处理超文本标记语言 DOM 元素，你需要一种方法将运势渲染到 DOM。

声明一个名为 `renderCard` 的函数，它接受四个参数：`drawingType`、`isReversed`、`shortName` 和 `img`。

参数 `drawingType`、`shortName` 和 `img` 必须是 `string` 类型，而 `isReversed` 必须是 `boolean` 类型。该函数应具有 `string` 类型的返回值。

# --hints--

你应该有一个名为 `renderCard` 的函数。
...
你的第一个参数应该是 `drawingType`，类型为 `string`。
...
你的第二个参数应该是 `isReversed`，类型为 `boolean`。
...
你的第三个参数应该是 `shortName`，类型为 `string`。
...
你的 `renderCard` 函数应该有 `string` 类型的返回值。
