...
يمكنك الآن تحديث العدد الإجمالي للعناصر على صفحة الويب. عيّن قيمة دالة `.getCounts()` الخاصة بكائن `cart` إلى `textContent` للمتغير `totalNumberOfItems`.

# --hints--

يجب أن تصل إلى خاصية `textContent` للمتغير `totalNumberOfItems`.

```js
assert.match(code, /totalNumberOfItems\s*\.\s*textContent/)
```

يجب أن تعيّن قيمة دالة `.getCounts()` الخاصة بكائن `cart` إلى خاصية `textContent` للمتغير `totalNumberOfItems`.
