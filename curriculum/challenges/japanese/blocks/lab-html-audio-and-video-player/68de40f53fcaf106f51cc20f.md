...
4. `h2`要素の下に、`controls`および`width`属性を持つ`video`要素が必要です。`width`属性は`640`に設定してください。
5. `video`要素の中に、ビデオファイルを指す`src`属性と`type`属性を持つ`source`要素が必要です。
...
7. `h2`要素の下に、`controls`および`loop`属性を持ち、オーディオファイルを指す`src`属性を持つ`audio`要素が必要です。
