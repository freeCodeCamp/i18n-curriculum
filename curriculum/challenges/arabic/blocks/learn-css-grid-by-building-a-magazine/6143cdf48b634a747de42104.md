...
داخل عنصر `.image-wrapper`، عرّف للعنصر الثالث `img` الخاص بك خاصية `src` بقيمة `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`، وخاصية `alt` بقيمة `four people working on code`، وخاصية `loading` بقيمة `lazy`، وصنف `class` بقيمة `image-3`، وخاصية `width` بقيمة `600`، وخاصية `height` بقيمة `400`.

# --hints--

يجب أن يحتوي العنصر الثالث `img` على خاصية `src` بقيمة `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`.
