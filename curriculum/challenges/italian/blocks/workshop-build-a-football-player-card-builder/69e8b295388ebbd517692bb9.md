...
L'intestazione della card è completa. Ora devi aggiungere la sezione della foto del giocatore.

Dopo la chiusura di `div` di `card-header` e ancora all'interno di `div` con `className` di `card`, aggiungi un `div` con il `className` di `card-image-wrap`. Lascialo vuoto per ora. Aggiungerai l'immagine nel passaggio successivo.

# --hints--

Dovresti aggiungere un `div` con il `className` di `card-image-wrap`.
