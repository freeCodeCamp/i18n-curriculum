...
Cada vez que haces clic en el botón de reproducir, la primera canción de la lista de reproducción siempre empezará a sonar. Para solucionar esto, dentro de la función flecha del escuchador de eventos, reemplaza `playSong(0)` por una declaración `if` que verifique si `userData.currentSong` es `null`.

Dentro del bloque `if`, llama a la función `playSong` con el `id` de la primera canción en el arreglo `userData.songs`.

# --hints--

Debes modificar el callback del escuchador de eventos para llamar a la función `playSong` con el `id` de la primera canción en el arreglo `userData.songs` cuando `userData.currentSong` es `null`.
