...
Crea un selettore `.author` e dagli una proprietà `font-size` impostata su `2rem` e una proprietà `font-family` impostata su `Raleway` con un fallback di `sans-serif`.

# --hints--

Dovresti avere un selettore `.author`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author'));
```

Il tuo selettore `.author` dovrebbe avere una proprietà `font-size` impostata su `2rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

Il tuo selettore `.author` dovrebbe avere una proprietà `font-family` impostata su `Raleway` con un fallback di `sans-serif`.
