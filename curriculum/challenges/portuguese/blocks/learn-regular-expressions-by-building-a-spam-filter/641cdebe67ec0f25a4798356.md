...
A função de callback deve ter uma instrução `if`.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

A instrução `if` deve verificar se `value` de `messageInput` é uma string vazia.
...
Sua declaração `if` deve exibir um alerta para o usuário com a mensagem `"Please enter a message."`.
...
A instrução `if` deve sair da execução da função.
