...
`address` 要素は物理的な地理的場所を含む必要はありません。主題へのリンクを提供するために使うことができます。

テキスト `freeCodeCamp` の周りにリンクを設定し、その場所を `https://freecodecamp.org` にしてください。
...
`a` 要素に `href` 属性として `https://freecodecamp.org` を設定してください。

```js
assert.equal(document.querySelector('address > a')?.getAttribute('href'), 'https://freecodecamp.org');
```

既存のテキスト `a` の周りに `freeCodeCamp` 要素をラップしてください。
