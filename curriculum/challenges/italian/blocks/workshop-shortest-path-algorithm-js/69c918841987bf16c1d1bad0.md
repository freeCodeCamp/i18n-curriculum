...
La funzione dovrebbe restituire i dati calcolati in modo che chi la chiama possa usarli programmaticamente.

Dopo il loop `for...of`, aggiungi un'istruzione `return` che restituisca un array contenente `distances` e `paths`, in questo ordine.

# --hints--

Dovresti restituire `[distances, paths]` dopo il loop `for...of`.
