...
Ora la tua `applyFunction` deve restituire un risultato. Restituisci il risultato della chiamata al metodo `.replace()` su `str2`. Passa la tua espressione regolare `functionCall` e una callback vuota.

# --hints--

La tua funzione `applyFunction` dovrebbe restituire il risultato della chiamata al metodo `.replace()` su `str2`.
...
Dovresti passare `functionCall` come primo argomento alla tua chiamata `.replace()`.
...
Dovresti passare una funzione freccia vuota come secondo argomento alla tua chiamata `.replace()`.
