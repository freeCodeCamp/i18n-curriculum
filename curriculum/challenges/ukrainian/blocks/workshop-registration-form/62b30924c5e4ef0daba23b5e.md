...
Кордон останнього елемента `fieldset` виглядає трохи недоречно.

У попередніх уроках ви навчились працювати з псевдокласом `last-of-type` у CSS так:

```css
p:last-of-type { }
```

Це вибирає останній елемент `p`.

Створіть новий селектор, який націлюється на останній елемент `fieldset`, і встановіть для нього `border-bottom` значення `none`.
