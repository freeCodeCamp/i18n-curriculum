...
存取 `userData?.currentSong?.title` 和 `userData?.currentSong?.artist` 屬性，並分別將它們指定給 `currentTitle` 和 `currentArtist` 變數。

# --hints--

你應該從 `userData` 物件中存取 `currentSong` 的 `title`。別忘了使用選用串鏈。

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

你應該將 `currentSong` 的 `title` 指定給一個 `currentTitle` 常數。

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

你應該從 `userData` 物件中存取 `currentSong` 的 `artist`。別忘了使用選用串鏈。

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

你應該將 `currentSong` 的 `artist` 指定給一個 `currentArtist` 常數。
