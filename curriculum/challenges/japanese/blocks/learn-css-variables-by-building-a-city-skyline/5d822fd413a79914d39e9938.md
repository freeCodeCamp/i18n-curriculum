...
`sky` クラスに `radial-gradient` を付けてください。`#ffcf33` から `0%` まで `20%`、`#ffff66` に `21%`、`#bbeeff` に `100%` を使います。これで背景に円形のグラデーションが追加され、太陽になります。

# --hints--

`.sky` に `radial-gradient` プロパティで `background` を付けてください。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".sky")?.background, "radial-gradient");
```

`radial-gradient` に最初の色として `#ffcf33` を付けてください。

```js
assert.match(new __helpers.CSSHelp(document).getStyle(".sky")?.background, /radial-gradient\(rgb\(255, 207, 51\)( 0%)?, rgb\(255, 207, 51\) 20%/);
```

`radial-gradient` に 2 番目の色として `#ffff66` を `21%` に付けてください。
...
`radial-gradient` に 3 番目の色として `#bbeeff` を `100%` に付けてください。
