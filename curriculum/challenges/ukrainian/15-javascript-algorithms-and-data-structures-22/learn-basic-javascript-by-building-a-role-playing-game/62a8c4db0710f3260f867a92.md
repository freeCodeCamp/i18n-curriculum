...
Що робити, якщо у гравця недостатньо золота, щоб купити здоров'я? Використовуйте інструкцію `if`, щоб перевірити, чи достатньо у гравця золота, щоб купити здоров’я.

У попередньому проєкті ви навчилися як працювати з інструкціями `if` ось так:
...
Почніть с розміщення всього коду у функції `buyHealth` в межах інструкції `if`. For the `if` statement condition, check if `gold` is greater than or equal to `10`.
