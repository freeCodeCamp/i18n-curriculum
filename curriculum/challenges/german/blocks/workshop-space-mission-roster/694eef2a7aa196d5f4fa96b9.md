...
Rufen Sie `getEVAReadyCrew()` mit Ihrem `updatedSquad` Roster auf und speichern Sie das Ergebnis in einer neuen Variablen namens `EVAReadySquad`. Verwenden Sie dann eine `for`-Schleife, um den `name` jedes Astronauten im `EVAReadySquad`-Array zu protokollieren.

# --hints--

Sie sollten `getEVAReadyCrew()` mit `updatedSquad` aufrufen und das Ergebnis in einer neuen Variablen namens `EVAReadySquad` speichern.
...
Sie sollten eine `for`-Schleife verwenden, um das `EVAReadySquad`-Array zu durchlaufen und den `name` jedes Astronauten zu protokollieren.
