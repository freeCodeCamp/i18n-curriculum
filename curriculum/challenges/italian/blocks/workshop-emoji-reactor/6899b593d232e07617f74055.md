...
All'interno dell'elemento `button`, crea un elemento `span` che contenga `😊`.

Dai all'elemento `span` un attributo `role` con valore `img` e un attributo `aria-hidden` con valore `true`.

Poi, crea un secondo elemento `span` con un `class` di `count` contenente il testo `0/10`.

# --hints--

All'interno dell'elemento `button` dovrebbero esserci due elementi `span`.

```js
assert.lengthOf(document.querySelectorAll("button > span"), 2);
```

Il primo elemento `span` dovrebbe avere un contenuto testuale di `😊`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.textContent.trim(), "😊");
```

Il primo elemento `span` dovrebbe avere un attributo `role` con valore `img`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("role"), "img");
```

Il primo elemento `span` dovrebbe avere un attributo `aria-hidden` con valore `true`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("aria-hidden"), "true");
```

Il secondo elemento `span` dovrebbe avere un attributo `class` con valore `count`.

```js
assert.equal(document.querySelectorAll("button > span")[1]?.className, "count");
```

Il secondo elemento `span` dovrebbe avere un contenuto testuale di `0/10`.
