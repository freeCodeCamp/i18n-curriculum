...
就像你对 `highPrecedence()` 函数所做的那样，你的 `evalFormula()` 函数需要确保它已评估并替换了所有内容。

使用三元组检查 `functionExpanded` 是否等于原始字符串 `x`。 如果是，则返回 `functionExpanded`，否则返回使用 `functionExpanded` 和 `cells` 作为参数再次调用 `evalFormula()` 的结果。

# --hints--

你的 `evalFormula` 函数应该使用 `return` 关键字。
