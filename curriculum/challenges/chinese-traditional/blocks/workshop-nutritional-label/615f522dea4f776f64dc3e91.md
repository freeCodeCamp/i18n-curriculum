...
`rem` 單位代表 `root em`，並且是相對於 `html` 元素的字型大小。

創建一個 `.small-text` 選擇器並將 `font-size` 設定為 `0.85rem`，這將計算為大約 `13.6px`（請記得你將你的 `html` 設定為 `font-size` 為 `16px`）。
...
你的 `.small-text` 選擇器應該有一個 `font-size` 屬性設定為 `0.85rem`。
