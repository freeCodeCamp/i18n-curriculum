...
- **字符串**：字符串是用引号括起来的一串字符或文本。 `"I like coding"` 和 `'JavaScript is fun'` 都是字符串的例子。
...
- One of the biggest advantages of `async/await` is error handling via `try/catch` blocks. 这是一个示例：
