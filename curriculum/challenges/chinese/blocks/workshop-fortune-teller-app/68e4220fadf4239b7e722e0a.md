...
由于 fortune 应用只有三个时刻：过去、现在和未来，声明一个名为 `DrawingType` 的枚举，包含三个属性：`Past`、`Present` 和 `Future`。每个属性都应有一个与其名称对应的字串值。

# --hints--

你应该有一个名为 `enum` 的 `DrawingType`。

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

你的枚举应该有一个 `Past` 属性，其字串值为 `past`。

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

你的枚举应该有一个 `Present` 属性，其字串值为 `present`。

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

你的枚举应该有一个 `Future` 属性，其字串值为 `future`。
