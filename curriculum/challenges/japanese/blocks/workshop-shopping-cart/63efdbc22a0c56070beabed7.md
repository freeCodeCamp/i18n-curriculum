...
`id`の`dessert`プロパティにアクセスするにはドット記法を使うべきです。
...
`totalCountPerProduct`の`dessert.id`に対応するプロパティにアクセスするにはブラケット記法を使うべきです。
...
`totalCountPerProduct[dessert.id]`を`0`のデフォルト値で、式の最後に`||`演算子を使って初期化するべきです。
