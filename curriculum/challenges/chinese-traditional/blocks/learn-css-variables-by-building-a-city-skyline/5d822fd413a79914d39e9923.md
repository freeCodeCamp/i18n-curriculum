...
從 `.fb3` 中移除 `background-color` 屬性和值，並將它們添加到 `.fb3a` 和 `.fb3b`。

# --hints--

你應該從 `.fb3` 中刪除 `background-color`。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

你應該給 `.fb3a` 一個值爲 `--building-color1` 的 `background-color`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

你應該給 `.fb3b` 一個值爲 `--building-color1` 的 `background-color`。
