...
Wenn `userData.currentSong` nicht `null` ist, müssen Sie den nächsten Song zum Abspielen finden. Denken Sie daran, dass Sie dafür `getNextSong` verwenden können.

Vervollständigen Sie Ihre `playNextSong`-Funktion so, dass sie den nächsten Song in der Wiedergabeliste abspielt. Wenn der aktuelle Song der letzte ist, setzen Sie `userData.currentSong` auf `null`, `userData.songCurrentTime` auf null und rufen Sie die `pauseSong`-Funktion auf.

# --hints--

Wenn das `currentSong` von `userData` wahr ist und nicht der letzte Song der Wiedergabeliste, sollte die `playNextSong`-Funktion den Song abspielen, der auf den aktuellen folgt.
...
Wenn das `currentSong` von `userData` der letzte Song der Wiedergabeliste ist, sollte die `playNextSong`-Funktion `userData.currentSong` auf `null` setzen.
...
Wenn das `currentSong` von `userData` der letzte Song der Wiedergabeliste ist, sollte die `playNextSong`-Funktion `userData.songCurrentTime` auf `0` setzen.
...
Wenn das `currentSong` von `userData` der letzte Song der Wiedergabeliste ist, sollte die `playNextSong`-Funktion die `pauseSong`-Funktion aufrufen.
...
Wenn das `currentSong` von `userData` gleich `null` ist, sollte Ihre `playNextSong`-Funktion dennoch die `playSong`-Funktion mit dem `id` des ersten Songs im `userData.songs`-Array als Argument aufrufen.
