...
使用 `const` 和箭頭語法創建一個空的 `playPreviousSong` 函數。

# --hints--

你應該使用 `const` 創建一個名爲 `playPreviousSong` 的空函數。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` 應該是一個函數。

```js
assert.isFunction(playPreviousSong)
```

你的 `playPreviousSong` 函數應該使用箭頭語法。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `playPreviousSong` 函數應該是空的。
