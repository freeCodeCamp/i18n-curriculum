...
為 `onChange` 添加一個屬性，該屬性設為一個箭頭函式，該函式以 `e` 作為參數，並呼叫 `setPlayer`，傳入展開的 `player` 和 `position`，其中 position 設為 `e.target.value`，並轉型為 `Position`。

# --hints--

你的位置 `select` 應該有一個 `onChange` 處理常式，該處理常式呼叫 `setPlayer`，傳入展開的 `player`，並更新 `position`。
