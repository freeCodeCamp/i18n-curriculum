...
現在你可以設置網格佈局的樣式。 CSS Grid 類似於 Flexbox，因爲它對父元素和子元素都有一個特殊的屬性。

在這裏，你的父元素是 `main` 元素。 通過添加值爲 `1fr 94rem 1fr` 的 `grid-template-columns` 屬性，將內容設置爲具有三列的佈局。 這將創建三列，中間列的寬度爲 `94rem`，第一列和最後一列都是網格容器中剩餘空間的一半。

# --hints--

你的 `main` 部分應該有一個 `grid-template-columns` 屬性，值爲 `1fr 94rem 1fr`。
