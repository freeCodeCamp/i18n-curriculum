...
Dentro da classe `Bee`, implemente o método `render`, prefixado pela palavra-chave `override`, para atualizar a propriedade `innerText` do `emojiElement` com o valor da propriedade `emoji`.

# --hints--

Sua classe `Bee` deve ter um método `render` usando a palavra-chave `override`.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

Você deve definir `this.emojiElement.innerText` como `this.emoji` dentro do método `render` de `Bee`.
