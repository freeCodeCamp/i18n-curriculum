...
Ongeza sifa ya kipengele ya `value` ya `player.club` kwa Club `input`. Baada ya hapo, ongeza sifa ya kipengele ya `onChange` iliyowekwa kuwa kitendakazi cha mshale kinachochukua `e` kama kigezo chake na hufanya wito wa `setPlayer` kwa kueneza `player` na `club` iliyowekwa kuwa `e.target.value`.

# --hints--

Club yako ya `input` inapaswa kuwa na `value` sawa na club ya mchezaji.
...
Club yako ya `input` inapaswa kuwa na mshughulikiaji wa `onChange` anayefanya wito wa `setPlayer` kwa kueneza `player` na `club` iliyowekwa kuwa `e.target.value`.
