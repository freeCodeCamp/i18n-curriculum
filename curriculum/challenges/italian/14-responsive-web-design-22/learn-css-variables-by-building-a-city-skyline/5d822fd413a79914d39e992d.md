...
Dovresti dare a `.fb4-window` una proprietà `width` del `30%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

Dovresti dare a `.fb4-window` una proprietà `height` del `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

Dovresti dare a `.fb4-window` una proprietà `border-radius` del `50%`.
