...
이제 화살표 함수를 사용해 `POSITIONS` 배열을 순회하며 각 위치 문자열을 `pos`로 받고 `option` 요소를 반환하세요. 옵션은 `key`의 `pos`를 가져야 하며 값도 `pos`여야 합니다. 또한 생성된 각 `pos`의 텍스트로 `option`를 표시하세요.

map의 매개변수는 `pos`가 아니어도 아무거나 사용해도 됩니다.

# --hints--

위치 `select`에는 `option` 배열에서 매핑된 `POSITIONS` 요소가 포함되어야 합니다.
...
각 옵션의 `value`는 표시된 텍스트와 일치하고 유효한 위치여야 합니다.
