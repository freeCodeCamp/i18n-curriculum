...
不要な繰り返しを避けるため、`h3` 要素の `before` 疑似要素を指定して、その `content` プロパティの値を `"Question #"` に設定してください。

# --hints--

`h3::before` セレクターを使用する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3::before'));
```

疑似要素 `h3::before` の `content` プロパティを `"Question #"` に設定してください。
