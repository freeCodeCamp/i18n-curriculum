...
الآن ركز على كتابة setter لـ `salary`. بعد getter الخاص بـ `salary`، أنشئ setter بسيط لخاصية `salary` يعيّن `self._salary` إلى القيمة التي تُمرر إلى الطريقة كمعلمتها. بعد ذلك، اطبع `Salary updated to $` متبوعًا بالراتب الجديد ونقطة.

ستتولى التحقق من صحة الراتب الجديد في الخطوات التالية.

# --hints--

يجب أن يحتوي صنف `Employee` على طريقة `salary` مزينة بـ `@salary.setter`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].has_decorators("salary.setter")`)) })
```

يجب أن يعين setter الخاص بـ `salary` قيمة `self._salary` إلى القيمة التي تُمرر إليه كمعلمة.
...
يجب أن يطبع setter الخاص بـ `salary` العبارة `Salary updated to $` متبوعة بالراتب الجديد ونقطة.
