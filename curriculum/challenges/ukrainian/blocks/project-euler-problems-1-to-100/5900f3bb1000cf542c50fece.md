...
title: 'Завдання 79: Виведення коду доступу'
challengeType: 1
forumTopicId: 302192
...
Загальним методом безпеки, який використовують для онлайн-банкінгу, є запит користувача про три випадкові символи з коду доступу. Наприклад, якщо код доступу був `531278`, можуть попросити 2-й, 3-й і 5-й символи; очікувана відповідь буде: `317`.

Масиви `keylog1`, `keylog2` і `keylog3` містять п’ятдесят успішних спроб входу.

Враховуючи, що три символи завжди запитують у порядку, проаналізуйте масив, щоб визначити найкоротший можливий секретний код доступу невідомої довжини.

# --hints--

`passcodeDerivation(keylog1)` має повертати число.
...
`passcodeDerivation(keylog1)` має повертати `531278`.
...
`passcodeDerivation(keylog2)` має повертати `1230567`.
...
`passcodeDerivation(keylog3)` має повертати `73162890`.
