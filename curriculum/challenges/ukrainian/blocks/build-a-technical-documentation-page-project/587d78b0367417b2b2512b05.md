...
**Мета:** Створіть застосунок, який функціонально схожий на <a href="https://technical-documentation-page.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://technical-documentation-page.freecodecamp.rocks</a>. **Не копіюйте цей демонстраційний проєкт**.

**Історії користувача:**

1. Ви повинні бачити елемент `main` з відповідним `id="main-doc"`, який містить основний вміст сторінки (технічну документацію)
1. Всередині елемента `#main-doc` має бути кілька елементів `section`, кожен з класом `main-section`. Їх має бути не менше п’яти
1. Першим елементом у кожному `.main-section` має бути елемент `header`, який містить текст, що описує тему цього розділу.
1. Кожен елемент `section` з класом `main-section` повинен також мати `id`, який відповідає тексту кожного `header`, що міститься в ньому. Усі пробіли мають бути замінені на підкреслення (наприклад, розділ із заголовком "JavaScript and Java" повинен мати відповідний `id="JavaScript_and_Java"`)
1. Елементи `.main-section` повинні містити загалом не менше десяти `p` елементів (не в кожному, а всього)
1. Елементи `.main-section` повинні містити загалом не менше п’яти `code` елементів (не в кожному, а всього)
1. Елементи `.main-section` повинні містити загалом не менше п’яти `li` елементів (не в кожному, а всього)
1. Ви повинні бачити елемент `nav` з відповідним `id="navbar"`
1. Елемент панелі навігації повинен містити один елемент `header`, який містить текст, що описує тему технічної документації
1. Крім того, панель навігації повинна містити посилання (елементи `a`) з класом `nav-link`. Їх має бути по одному для кожного елемента з класом `main-section`
1. Елемент `header` у `#navbar` повинен розташовуватися перед будь-якими посиланнями (елементами `a`) у панелі навігації
1. Кожен елемент з класом `nav-link` повинен містити текст, який відповідає тексту `header` у кожному `section` (наприклад, якщо у вас є розділ/заголовок "Hello world", ваша панель навігації повинна містити елемент із текстом "Hello world")
1. Коли ви натискаєте на елемент панелі навігації, сторінка має перейти до відповідного розділу елемента `#main-doc` (наприклад, якщо ви натиснули на елемент `.nav-link` із текстом "Hello world", сторінка переходить до елемента `section` з таким ідентифікатором і відповідним заголовком)
1. На пристроях із звичайним розміром екрану (ноутбуки, десктопи) елемент із `id="navbar"` має відображатися з лівого боку екрану і завжди бути видимим для користувача
1. Ваша технічна документація має використовувати принаймні один медіазапит

Виконайте всі історії користувача та пройдіть усі тести нижче, щоб завершити цей проєкт. Додайте свій особистий стиль. Успішного кодування!

**Примітка:** Обов’язково додайте `<link rel="stylesheet" href="styles.css">` у ваш HTML, щоб підключити таблицю стилів і застосувати CSS

# --hints--

Ви повинні мати елемент `main` з `id` рівним `main-doc`.
...
Ви повинні мати принаймні п’ять елементів `section` з класом `main-section`.
...
Усі ваші елементи `.main-section` мають бути елементами `section`.
...
Ви повинні мати принаймні п’ять елементів `.main-section`, які є нащадками `#main-doc`.
...
Першим дочірнім елементом кожного `.main-section` має бути елемент `header`.
...
Жоден із ваших елементів `header` не повинен бути порожнім.
...
Усі ваші елементи `.main-section` мають мати `id`.
...
Кожен `.main-section` має мати `id`, який відповідає тексту його першого дочірнього елемента, при цьому всі пробіли в тексті замінені на підкреслення (`_`) для ідентифікаторів.
...
Ви повинні мати загалом не менше 10 елементів `p` всередині ваших елементів `.main-section`.
...
Ви повинні мати принаймні п’ять елементів `code`, які є нащадками елементів `.main-section`.
...
Ви повинні мати принаймні п’ять елементів `li`, які є нащадками елементів `.main-section`.
...
Ви повинні мати елемент `nav` з `id` рівним `navbar`.
...
Ваш `#navbar` має містити рівно один елемент `header`.
...
Ви повинні мати принаймні один елемент `a` з класом `nav-link`.
...
Усі ваші елементи `.nav-link` мають бути елементами посилань (`a`).
...
Усі ваші елементи `.nav-link` мають бути в `#navbar`.
...
Кількість елементів `.nav-link` має дорівнювати кількості елементів `.main-section`.
...
Елемент `header` у `#navbar` має розташовуватися перед будь-якими посиланнями (елементами `a`) також у `#navbar`.
...
Кожен `.nav-link` має містити текст, який відповідає тексту `header` пов’язаного з ним `section` (наприклад, якщо у вас є розділ/заголовок "Hello world", ваш `#navbar` має мати `.nav-link` із текстом "Hello world").
...
Кожен `.nav-link` має мати атрибут `href`, який посилається на відповідний `.main-section` (наприклад, якщо ви натискаєте на елемент `.nav-link` із текстом "Hello world", сторінка переходить до елемента `section` з таким ідентифікатором).
...
Ваш `#navbar` має завжди бути на лівому краю вікна.
...
Ваш проєкт технічної документації має використовувати принаймні один медіазапит.
