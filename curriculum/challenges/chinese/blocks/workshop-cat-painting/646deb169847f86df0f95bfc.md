...
使用类选择器，给你的 `.cat-right-inner-eye` 元素一个宽度为 `10px` 和高度为 `20px` 的样式。 另外，将其背景颜色设置为 `#fff`。

# --hints--

你应该有一个 `.cat-right-inner-eye` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye'))
```

你的 `.cat-right-inner-eye` 选择器应该设置 `width` 为 `10px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.width, '10px')
```

你的 `.cat-right-inner-eye` 选择器应该设置 `height` 为 `20px` 。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.height, '20px')
```

你的 `.cat-right-inner-eye` 选择器应该设置 `background-color` 为 `#fff`。
