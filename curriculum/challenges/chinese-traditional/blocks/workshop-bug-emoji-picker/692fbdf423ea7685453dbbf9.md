...
取得 ID 為 `bug-emoji` 的段落元素，並將它存放在名為 `bugEmojiElement` 的常數中。使用 `!` 非空斷言運算子來斷言該元素不是 `null`。

# --hints--

你應該有一個名為 `bugEmojiElement` 的常數。
...
你的 `bugEmojiElement` 應該使用帶有 `bug-emoji` 型別引數的 `document.querySelector` 查詢 ID 為 `HTMLParagraphElement` 的元素。

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

你的 `bugEmojiElement` 應該使用非空斷言運算子（`!`）。
