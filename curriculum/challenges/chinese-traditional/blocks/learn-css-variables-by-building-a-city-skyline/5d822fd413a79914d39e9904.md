...
到目前為止，你創建的所有漸層都是從上到下，這是預設方向。你可以在顏色前面添加方向來指定其他方向，如下所示：
...
在 `.bb3` 中填入 `repeating-linear-gradient`。使用 `90deg` 作為方向，為前兩個顏色使用你的 `building-color3`，並在 `15%` 處使用 `window-color3` 作為第三個顏色。

當你沒有為顏色指定距離時，它會使用合理的值。在這種情況下，前兩個顏色會預設為 `0%` 和 `7.5%`，因為它從 `0%` 開始，而 `7.5%` 是 `15%` 的一半，所以你不需要設定它們。

# --hints--

你應該使用 `repeating-linear-gradient` 為 `.bb3` 設定 `background`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb3")?.background, "repeating-linear-gradient");
```

你應該在 `repeating-linear-gradient` 的第一個引數中使用 `90deg` 作為方向。
...
你應該在第三個顏色使用 `--window-color3`，數值為 `15%`。
