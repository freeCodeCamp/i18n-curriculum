...
为你的 `User` 类添加两个方法以读取和删除邮件：`read_email` 和 `delete_email`。两者都应接受一个 `index` 参数并调用 `self.inbox` 上的相应方法。
...
`read_email` 方法应调用 `self.inbox.read_email(index)`。
...
`delete_email` 方法应调用 `self.inbox.delete_email(index)`。
