...
클래스 선택자를 사용해 `.cat-mouth-line-left` 요소에 `position`인 `absolute`과 `top`인 `88px`, 그리고 `left`인 `74px`를 지정하세요. 이렇게 하면 요소가 오른쪽 위치로 이동합니다.

# --hints--

`.cat-mouth-line-left` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left'))
```

`.cat-mouth-line-left` 선택자에 `position`으로 설정된 `absolute` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.position === 'absolute')
```

`.cat-mouth-line-left` 선택자에 `top`으로 설정된 `88px` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.top === '88px')
```

`.cat-mouth-line-left` 선택자에 `left`으로 설정된 `74px` 속성이 있어야 합니다.
