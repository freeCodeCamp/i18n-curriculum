...
現在你可以開始將函數解析邏輯應用於字符串。 聲明一個名爲 `applyFunction` 的函數，它接受一個 `str` 參數。

# --hints--

你應該聲明一個 `applyFunction` 變量。

```js
assert.match(code, /(?:var|let|const)\s+applyFunction\s*=/);
```

你應該使用 `const` 來聲明你的 `applyFunction` 變量。

```js
assert.match(code, /const\s+applyFunction\s*=/);
```

你的 `applyFunction` 變量應該是一個函數。

```js
assert.isFunction(applyFunction);
```

你的 `applyFunction` 函數應該使用箭頭語法。

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

你的 `applyFunction` 函數應該有一個 `str` 參數。
