...
Lenga vipengele vya `.foot`, na uwape `width` ya `15%`, `height` ya `30%`, na `background-color` ya `orange`.
...
Unapaswa kumpa `.foot` `width` ya `--fcc-expected--`, iliyopatikana `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.width, '15%');
```

Unapaswa kumpa `.foot` `height` ya `--fcc-expected--`, iliyopatikana `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.height, '30%');
```

Unapaswa kumpa `.foot` `background-color` ya `--fcc-expected--`, iliyopatikana `--fcc-actual--`.
