...
Всередині методу `fetchCardsData` оголосіть змінну з іменем `response` і призначте їй значення, виконавши виклик функції `fetch` з таким URL: `` `${CDN_URL}/card_data.json` ``.

Додайте ключове слово `await` перед викликом `fetch` і обгорніть усю логіку в блок `try...catch`.

# --hints--

Ви повинні оголосити змінну `response` всередині методу `fetchCardsData`.
...
Змінна `response` має бути призначена викликом `await fetch` з URL `` `${CDN_URL}/card_data.json` ``.
...
Виклик `fetch` має бути обгорнутий у блок `try...catch`, з `error` як параметром `catch`.
