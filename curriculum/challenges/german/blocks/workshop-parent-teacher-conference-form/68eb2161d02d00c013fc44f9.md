...
Um den `legend`-Text hervorzuheben, müssen Sie ihn gestalten.

Wählen Sie das `fieldset legend`-Element aus. Setzen Sie seine `font-size` auf `1.3rem` und seine `font-weight` auf `600`.

`font-weight` steuert die Fettheit des Texts.

Gängige Werte für `font-weight` sind:

- `normal` → Normale Schriftstärke (Standard)
- `bold` → Fettdruck
- `lighter` → Leichter als das Elternelement
- `bolder` → Fettiger als das Elternelement
...
Sie sollten einen `fieldset legend`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('fieldset legend'));
```

Setzen Sie die `font-size`-Eigenschaft auf `1.3rem`.
...
Setzen Sie die `font-weight`-Eigenschaft auf `600`.
