...
创建一个新的 `.left-container p` 选择器，将上下外边距设置为 `-5px`，将左右外边距设置为 `-2px`。 再将 `font-size` 设置为 `2em`，将 `font-weight` 设置为 `700`。
...
你的新的 `.left-container p` 选择器应将 `margin` 属性设置为 `-5px -2px`。
...
你的新的 `.left-container p` 选择器应将 `font-size` 属性设置为 `2em`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

你的新的 `.left-container p` 选择器应将 `font-weight` 属性设置为 `700`。
