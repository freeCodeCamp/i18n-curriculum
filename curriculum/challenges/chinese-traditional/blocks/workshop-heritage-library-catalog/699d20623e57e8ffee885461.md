...
相同的樣式也適用於其他三個欄位。為 `author`、`year` 和 `location` 添加等價的檢查。

請注意 `isValid` 會在所有四個檢查中累積：如果任何欄位失敗，它會變成 `false`，即使後續檢查通過也會保持 `false`。這表示函式會檢查每個欄位，而不會在遇到第一個問題時就停止。

# --hints--

`validateEntry(catalog[0])` 應該傳回 `true`。

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` 應該傳回 `false`。

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` 應該傳回 `false`。

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` 應該傳回 `false`，因為位置是 `"Unknown"`。
