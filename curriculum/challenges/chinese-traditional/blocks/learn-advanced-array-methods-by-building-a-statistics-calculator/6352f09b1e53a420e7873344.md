...
將你的 `getMode()` 函式添加到你的 `calculate` 邏輯中，並更新相應的 HTML 元素。

# --hints--

你的 `calculate` 函式（程式）應該有一個 `mode` 變數，其值為 `getMode(numbers)`。

```js
assert.match(calculate.toString(), /mode\s*=\s*getMode\(\s*numbers\s*\)\s*;?/);
```

你的 `calculate` 函式（程式）應該查詢 DOM 中的 `#mode` 元素，並將 `textContent` 設定為 `mode`。
