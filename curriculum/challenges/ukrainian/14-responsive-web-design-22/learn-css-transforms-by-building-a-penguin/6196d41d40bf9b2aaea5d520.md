...
Ви повинні надати `.sun` властивість `top` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.top, '-75px');
```

Ви повинні надати `.sun` властивість `right` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.
