...
يمكنك إضافة تدرجات متعددة إلى عنصر بفصلها بفاصلة (`,`) مثل هذا:
...
أضف `repeating-linear-gradient` إلى `.fb1c` أسفل التدرج الموجود؛ استخدم `--building-color4` من `0%` إلى `10%` و`--window-color4` من `10%` إلى `90%`. هذا سيملأ خلف التدرج الذي أضفته سابقًا.

# --hints--

لا يجب تعديل أول `repeating-linear-gradient`.
...
يجب أن يكون هناك فاصلة (`,`) بعد أول `repeating-linear-gradient`.
...
يجب إضافة `repeating-linear-gradient` بلون أول `--building-color4` من `0%` إلى `10%`.
...
يجب استخدام لون ثاني `--window-color4` من `10%` إلى `90%`.
