...
Collega insieme gli elementi modulo applicabili e i loro rispettivi elementi `label`.

Usa `profile-picture`, `age`, `referrer` e `bio` come valori per i rispettivi attributi `id`.

# --hints--

Il primo elemento `input` dovrebbe avere un `id` di `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[0]?.matches('#profile-picture'))
```

Il secondo elemento `input` dovrebbe avere un `id` di `age`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[1]?.matches('#age'))
```

L'elemento `select` dovrebbe avere un `id` di `referrer`.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) select')?.matches('#referrer'))
```

L'elemento `textarea` dovrebbe avere un `id` di `bio`.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) textarea')?.matches('#bio'))
```

Il primo elemento `label` dovrebbe avere un attributo `for` con valore `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[0]?.matches('label[for="profile-picture"]'))
```

Il secondo elemento `label` dovrebbe avere un attributo `for` con valore `age`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[1]?.matches('label[for="age"]'))
```

Il terzo elemento `label` dovrebbe avere un attributo `for` con valore `referrer`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[2]?.matches('label[for="referrer"]'))
```

Il quarto elemento `label` dovrebbe avere un attributo `for` con valore `bio`.
