...
CSS-властивість `clip` використовують, щоб визначити видимі частини елемента. Встановіть селектору `span[class~="sr-only"]` властивість `clip` зі значенням `rect(1px, 1px, 1px, 1px)`.
...
Ваш селектор `span[class~="sr-only"]` повинен мати властивість `clip` зі значенням `rect(1px, 1px, 1px, 1px)`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('clip') === 'rect(1px, 1px, 1px, 1px)');
```

Ваш селектор `span[class~="sr-only"]` повинен мати властивість `clip-path` зі значенням `inset(50%)`.
