...
现在，是时候向 `resetButton` 添加重置功能了。 单击后将返回播放列表中的歌曲。

向 `resetButton` 变量添加点击事件监听器。 使用箭头语法传入回调并暂时将其留空。

# --hints--

你应该在 `resetButton` 上使用 `addEventListener()` 方法。

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

你的 `resetButton` 事件监听器应该监听 `"click"` 事件。

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

事件监听器的回调函数应该使用箭头语法并且有一对空的花括号。
