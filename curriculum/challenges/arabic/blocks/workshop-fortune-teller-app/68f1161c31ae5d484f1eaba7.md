...
أضف الآن طريقة أخرى تسمى `showFortune`. مرر `e` كمعلمة وأعطها النوع `Event`. أنشئ أيضًا طريقة `newReading` واتركها فارغة في الوقت الحالي.

# --hints--

يجب إعلان طريقة باسم `showFortune`.
...
يجب أن تحتوي طريقة `showFortune` على معلمة `e` من النوع `Event`.
...
يجب إعلان طريقة باسم `newReading`.
