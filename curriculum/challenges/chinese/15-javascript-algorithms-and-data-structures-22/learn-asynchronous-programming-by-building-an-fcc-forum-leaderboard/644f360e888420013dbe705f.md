...
在你的 `else` 子句中，将字符串 `"general"` 赋值给`selectedCategory.className`。

在其下面，将字符串 `"General"` 赋值给 `selectedCategory.category`。

最后，将数字 `1` 赋值给 `selectedCategory.id`。

# --hints--

你应该将字符串 `"general"` 赋值给 `selectedCategory.className`。

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

你应该将字符串 `"General"` 赋值给 `selectedCategory.category`。

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

你应该将数字 `1` 赋值给 `selectedCategory.id`。
