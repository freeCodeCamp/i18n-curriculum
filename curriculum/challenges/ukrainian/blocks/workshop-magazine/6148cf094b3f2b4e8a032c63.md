...
Створіть селектор `.first-paragraph::first-letter` і встановіть властивість `font-size` у значення `6rem`. Також додайте йому властивість `color` зі значенням `orangered`, щоб він виділявся.
...
Ваш селектор `.first-paragraph::first-letter` має мати властивість `font-size`, встановлену в `6rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize, '6rem');
```

Ваш селектор `.first-paragraph::first-letter` має мати властивість `color`, встановлену в `orangered`.
