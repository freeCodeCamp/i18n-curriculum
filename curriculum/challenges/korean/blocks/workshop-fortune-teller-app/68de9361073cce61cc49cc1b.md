...
이전 강의에서 타입 추론이 TypeScript가 변수, 함수 반환값, 표현식에 대해 사용과 문맥을 기반으로 자동으로 타입을 결정하고 할당하는 능력임을 배웠습니다.

TypeScript에서 제네릭은 함수, 클래스, 타입을 더 유연하게 만듭니다. 제네릭은 타입 변수를 생성할 수 있게 하여 매번 명시적으로 타입을 정의할 필요를 없애줍니다. 이를 통해 같은 함수가 서로 다른 타입과도 안전하게 작동할 수 있습니다.
...
함수는 두 개 이상의 제네릭을 사용할 수 있습니다. `T`는 관례적으로 사용되며, 필요에 따라 `S`와 같은 추가 타입 변수를 추가할 수 있습니다.
...
`selector`를 `string` 타입 매개변수로 받고, `HTMLElement`를 확장하는 제네릭 타입 `T`를 사용하며, 반환 타입이 `T`인 `getElement` 함수를 만드세요.

# --hints--

`getElement`라는 이름의 화살표 함수가 있어야 합니다.
...
`getElement` 함수는 `selector`라는 이름의 `string` 타입 매개변수를 받아야 합니다.
...
제네릭 타입 매개변수 `T`는 `HTMLElement` 타입을 확장해야 합니다.
...
`getElement` 함수의 반환 타입은 `T`여야 합니다.
