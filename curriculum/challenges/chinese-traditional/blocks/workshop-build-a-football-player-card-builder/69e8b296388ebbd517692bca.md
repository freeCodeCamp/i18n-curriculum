...
`FootballPlayerCard` 目前以固定的 `defaultPlayer` 呈現卡片。為了讓應用程式具有互動性，你需要在 React 狀態中管理玩家資料，這樣更新才能觸發重新算繪。

在 `FootballPlayerCard` 組件內，創建一個名為 `player` 的狀態變數，並搭配設值函式 `setPlayer`，用來保存目前的玩家資料。使用 `defaultPlayer` 初始化此狀態，並指定該狀態使用 `PlayerData` 型別作為泛型，讓 TypeScript 知道玩家物件應該具有的結構。

接著，更新傳回內容，將 `player` 狀態變數作為屬性傳給 `PlayerCard`。

# --hints--

你應該使用陣列解構來設定 `player` 狀態變數和 `setPlayer` 設值函式。

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

你的 `player` 和 `setPlayer` 應該使用 `useState` 掛鉤。
...
你的 `useState` 應以 `defaultPlayer` 初始化，並以 `PlayerData` 作為泛型型別。

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

你應該將 `player` 作為 `player` 屬性的值，並傳給 `PlayerCard`。
