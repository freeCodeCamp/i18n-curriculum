...
APIからデータを取得したので、`results`の状態を更新してください。`setResults`を呼び出し、`name`配列内の各果物から`data`プロパティだけを含む配列を渡してください。これには`map()`メソッドを使います。

# --hints--

`setResults`を`data.map(fruit => fruit.name)`で更新してください。
