...
你有一個小錯誤。`arg1` 和 `arg2` 是字串，不是數字。`infixToFunction['+']("1", "2")` 會傳回 `12`，這在數學上是不正確的。

將你的每個 `infixToFunction[operator]` 引數包裹在 `parseFloat()` 呼叫中。

# --hints--

你應該將 `arg1` 包裝在 `parseFloat()` 呼叫中。
