...
Crea una subclase `Spider` que extienda la clase `Bug` con el parámetro de tipo genérico establecido en `string`.

# --hints--

Deberías tener una clase llamada `Spider`.
...
Tu clase `Spider` debería extender `Bug`.
...
Tu clase `Spider` debería extender `Bug` con un argumento de tipo `string`.
