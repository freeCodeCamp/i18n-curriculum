...
Tu selector `.indent` debe tener una propiedad `margin-left` establecida en `1em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.indent')?.marginLeft, '1em');
```

Tu selector `.indent` sólo debe establecer la propiedad `margin-left`, no otras propiedades de margen.
