...
Ndani ya wito wako wa kurudisha wa `.reduce()` usio na kitu, tangaza kigezo `difference` na uweke thamani yake kuwa `el` pungufu `mean`. Kisha tangaza kigezo `squared`, na uweke thamani yake kuwa `difference` kwa nguvu ya `2`. Mwishowe, rudisha thamani ya `acc` jumlishwe na `squared`.

# --hints--

Wito wako wa kurudisha wa `reduce` unapaswa kuwa na kigezo `difference`.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

Kigezo chako cha `difference` kinapaswa kuwekwa thamani ya `el` pungufu `mean`.
...
Wito wako wa kurudisha wa `reduce` unapaswa kuwa na kigezo `squared`.
...
Kigezo chako cha `squared` kinapaswa kuwekwa thamani ya `difference` kwa nguvu ya 2.
...
Wito wako wa kurudisha wa `reduce` unapaswa kurudisha thamani ya `acc` jumlishwe na `squared`.
...
Hupaswi kuondoa thamani ya awali ya `0` kutoka kwa njia yako ya `.reduce()`.
