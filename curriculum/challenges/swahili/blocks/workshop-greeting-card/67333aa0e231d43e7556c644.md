...
Ongeza kichaguzi kipya kinacholenga vipengele vya `.card-links a` wakati viko hai. Weka `background-color` kuwa `midnightblue`.

# --hints--

Unapaswa kuwa na kichaguzi cha `.card-links a:active`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:active"));
```

Kichaguzi cha `.card-links a:active` kinapaswa kuwa na `background-color` ya `midnightblue`.
