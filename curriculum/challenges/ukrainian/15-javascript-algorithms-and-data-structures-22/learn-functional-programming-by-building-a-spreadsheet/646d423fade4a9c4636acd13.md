...
Спочатку вам потрібно обробити оператори з вищим пріоритетом. Оголосіть змінну `noHigh` та призначте до неї результат виклику `highPrecedence()` з `str` як аргументом.

# --hints--

Оголосіть змінну `noHigh`.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*(?:var|let|const)\s+noHigh\s*=/);
```

Використайте `const`, щоб оголосити змінну `noHigh`.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=/);
```

Призначте `noHigh` результат виклику `highPrecedence()`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(/);
```

Передайте `str` як аргумент до виклику `highPrecedence()`.
