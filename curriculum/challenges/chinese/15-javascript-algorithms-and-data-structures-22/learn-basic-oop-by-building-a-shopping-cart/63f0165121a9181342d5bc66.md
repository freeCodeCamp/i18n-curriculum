...
对于你的真值表达式，删除 `undefined`，你需要将 `currentProductCountSpan` 的 `textContent` 更新为 `currentProductCount`，后跟 `x`。 使用模板字面量来执行此操作。

# --hints--

你应该从真值表达式中删除 `undefined`。
...
你应该访问 `currentProductCountSpan` 的 `textContent` 属性。
...
你应该使用模板字面量语法将 `textContent` 更新为 `${currentProductCount}x`。
