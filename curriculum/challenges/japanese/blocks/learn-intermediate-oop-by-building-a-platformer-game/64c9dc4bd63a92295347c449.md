...
次に、左キーが押されたかどうかと `else if` が true かどうかを確認する `isCheckpointCollisionDetectionActive` 文を追加してください。

その条件の中に、`forEach` 配列を繰り返し処理する `platforms` ループを追加してください。

ループの中で、加算代入演算子を使ってプラットフォームの `x` 位置に 5 を加えてください。

# --hints--

左キーが押されたかどうかと `isCheckpointCollisionDetectionActive` が true かどうかを確認する条件が必要です。
