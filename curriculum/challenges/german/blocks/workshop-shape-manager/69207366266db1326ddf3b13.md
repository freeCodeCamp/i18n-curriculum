...
Fügen Sie das `DOMContentLoaded`-Ereignis zum Root-Dokument hinzu und übergeben Sie `initializeApp` als Callback-Funktion, die ausgeführt werden soll. So stellen Sie sicher, dass die gesamte Initialisierung erst nach dem vollständigen Laden des DOM erfolgt.

# --hints--

Sie sollten das `DOMContentLoaded`-Ereignis zum Root-`document` hinzufügen.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

Sie sollten `initializeApp` als Funktion angeben, die in Ihrem Event-Listener ausgeführt wird.
