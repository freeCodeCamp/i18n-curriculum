...
Für diesen Algorithmus solltest du eine `while`-Schleife verwenden. Diese Schleife benötigt zwei Bedingungen:

- Erstens sollte sie nicht über den Anhang des Arrays hinausgehen (Zugriff mit `j`).
...
Verringere `j` innerhalb deiner Schleife, um eine Endlosschleife zu verhindern.
...
Deine `while`-Schleife sollte ihre erste Bedingung enthalten, die sicherstellt, dass der Wert `j` größer oder gleich `0` ist.
...
Deine `while`-Schleife sollte den AND-Operator verwenden.
...
Deine `while`-Schleife sollte eine Bedingung enthalten, die sicherstellt, dass der Wert von `array[j]` größer ist als `currValue`.
...
Deine `while`-Schleife sollte `j` innerhalb der Schleife verringern.
