...
现在添加另一个名为 `showFortune` 的方法。作为参数，传入 `e` 并赋予它 `Event` 类型。同时创建一个 `newReading` 方法，暂时保持为空。

# --hints--

你应该声明一个名为 `showFortune` 的方法。
...
你的 `showFortune` 方法应该有一个类型为 `e` 的 `Event` 参数。
