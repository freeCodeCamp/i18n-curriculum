...
ついに！最後の建物にたどり着きました！`90deg` の方向で繰り返しグラデーションを追加してください。`0%` から `10%` まで建物の色を使い、`transparent` から `10%` まで `30%` を使ってください。

# --hints--

`repeating-linear-gradient` プロパティに `.fb6` を `background` として追加してください。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

`repeating-linear-gradient` に `90deg` の方向を指定してください。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

`repeating-linear-gradient` に `--building-color3` から `0%` まで `10%` の最初の色を指定してください。
...
`repeating-linear-gradient` に `transparent` から `10%` まで `30%` の2番目の色を指定してください。
