...
أنشئ عنصر `p` آخر بعد آخر `.divider` لديك، وأضف إليه النص `Total Sugars 12g`. عيّن لهذا العنصر `p` قيم `class` اللازمة لإزاحته وإزالة الحدود السفلية. ثم أنشئ `.divider` آخر أسفل عنصر `p` الجديد.

# --hints--

يجب أن تنشئ عنصرين جديدين `p` و `div` في نهاية عنصر `.daily-value` الخاص بك.
...
يجب أن يحتوي عنصر `p` الجديد على النص `Total Sugars 12g`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Total Sugars[\s|\n]+12g/));
```

يجب أن يكون للعنصر `p` الجديد الخاصية `class` مضبوطة على `indent no-divider`.
...
يجب أن يكون للعنصر `div` الجديد الخاصية `class` مضبوطة على `divider`.
