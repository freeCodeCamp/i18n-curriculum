...
`.logo` セレクターを作成してロゴのスタイリングを始めましょう。`width` を `200px` に、`position` を `absolute` に、`top` を `23px` に設定してください。
...
`.logo` セレクターには `width` プロパティが `200px` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.logo')?.width === '200px');
```

`.logo` セレクターには `position` プロパティが `absolute` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.logo')?.position === 'absolute');
```

`.logo` セレクターには `top` プロパティが `23px` に設定されているはずです。
