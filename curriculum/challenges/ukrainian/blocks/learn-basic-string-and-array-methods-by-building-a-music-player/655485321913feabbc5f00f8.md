...
Додайте слухача події `"click"` до елемента `previousButton`, потім передайте `playPreviousSong` як другий аргумент.

# --hints--

Ви повинні викликати метод `addEventListener()` на вашій змінній `previousButton`.
...
Ви повинні передати `playPreviousSong` як другий аргумент методу `addEventListener`.
