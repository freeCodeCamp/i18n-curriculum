...
`Later` is the comparative form of `late`, used to indicate a time or event that is after the expected, usual, or planned time. Los comparativos se usan para comparar dos cosas. Por ejemplo:

- `We will leave later than planned.` - Esto significa salir después de la hora planeada.

- `She arrived later than me.` - Esto indica que ella llegó después que el hablante.
...
`Later` no implica inmediatamente.
