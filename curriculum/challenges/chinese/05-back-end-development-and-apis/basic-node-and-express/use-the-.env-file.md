...
You will need to use the `dotenv` package. 它将环境变量从你的 `.env` 文件加载到 `process.env` 中。 `dotenv` 包已经安装，并且在你项目的 `package.json` 文件中。 在你的 `myApp.js` 文件的顶端，添加 `require('dotenv').config()` 以加载环境变量。
