...
プレイヤーが体力を買うのに十分なゴールドを持っていない場合はどうしたらいいでしょうか？ プレイヤーが体力を買うのに十分なゴールドを持っているかどうかを確認するために、`if` 文を使いましょう。

以前のプロジェクトで、次のような `if` 文の使い方を学びました:
...
ではまず、`buyHealth` 関数内のすべてのコードを `if` 文の中に入れてください。 `if` 文の条件として、`gold` が `10` 以上かどうかをチェックしてください。

# --hints--

`buyHealth` 関数には `if` 文を持たせてください。

```js
assert.match(buyHealth.toString(), /if/);
```

`if` 文で `gold` が `10` 以上かどうかをチェックしてください。

```js
assert.match(buyHealth.toString(), /if\s*\(\s*gold\s*>=\s*10\s*\)/);
```

`buyHealth` 内のコードはすべて `if` 文の中に入れてください。
