...
أزل الهامش من عنصر body، واضبط `font-family` على `sans-serif`، وأعطه `background-color` بقيمة `#f5f6f7`.
...
يجب أن يحتوي محدِّد `body` على خاصية `margin` بقيمة 0.
...
يجب أن يحتوي محدِّد `body` على خاصية `font-family` بقيمة `sans-serif`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily === 'sans-serif');
```

يجب أن يحتوي محدِّد `body` على خاصية `background-color` بقيمة `#f5f6f7`.
