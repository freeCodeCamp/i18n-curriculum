...
`fieldset` елементи мають бути безпосередніми дочірніми елементами `form`.

```js
assert.equal(document.querySelectorAll('form > fieldset')?.length, 3);
```

Чотири `label` елементи мають бути безпосередніми дочірніми елементами першого `fieldset`. Переконайтеся, що ви закриваєте `label` елементи.
