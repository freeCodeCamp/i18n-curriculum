...
いくつかの楽器カードが表示されたページが用意されています。この静的なページに機能を追加して、楽器のファミリーに応じてフィルターできるようにします。

まず、クラス名が`select-container`のドロップダウンメニューを作成してください。
...
`select`要素にはクラス`select-container`を付けてください。
