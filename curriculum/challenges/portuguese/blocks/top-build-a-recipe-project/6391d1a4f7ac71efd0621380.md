...
Você deve ter um elemento `title` dentro do elemento `head` que contenha o texto `The Odin Recipes`.

```js
assert(document.querySelectorAll('HEAD > TITLE')[0]?.innerText == 'The Odin Recipes');
```

Você deve ter um elemento `h1` dentro do seu elemento `body` que contenha o texto `Creamy Chocolate Fudge`.
...
Você deve ter um elemento `ul` aninhado com elementos `li` que contenham os ingredientes.
...
Você deve ter um elemento `ol` que contenha elementos `li` com os passos da receita.
...
Você deve ter um elemento `ul` aninhado com elementos `li` que contenham elementos `a` que linkem para outras receitas.
...
Seus elementos de âncora que linkam para as receitas devem ter um atributo `href` com o valor definido como `#`.
