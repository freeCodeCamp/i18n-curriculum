...
Dai a `.card` una `max-width` di `400px`, una `padding` di `40px` su tutti i lati e una `margin` di `0` sopra e sotto e `auto` a sinistra e a destra (usa la proprietà abbreviata).

# --hints--

`.card` dovrebbe avere una `max-width` di `400px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("max-width"), "400px");
```

`.card` dovrebbe avere una `padding` di `40px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("padding"), "40px");
```

`.card` dovrebbe avere una `margin` di `0 auto`.
