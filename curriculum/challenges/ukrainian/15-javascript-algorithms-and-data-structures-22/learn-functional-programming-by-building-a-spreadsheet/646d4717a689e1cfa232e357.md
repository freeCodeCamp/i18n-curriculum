...
Як ви зробили з вашої функцією `highPrecedence()`, функція `evalFormula()` має переконатися, що вона оцінила та замінила все.

Використайте потрійний, щоб перевірити, чи дорівнює `functionExpanded` оригінальному рядку `x`. Якщо так, поверніть `functionExpanded`, інакше поверніть результат виклику `evalFormula()` знову з `functionExpanded` та `cells` як аргументами.

# --hints--

Функція `evalFormula` має використовувати ключове слово `return`.
...
Ваша інструкція `return` має перевірити, чи `functionExpanded` дорівнює `x`.
