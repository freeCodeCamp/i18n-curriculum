...
添加 `else` 塊來處理歌曲的當前播放時間。 這使你可以從暫停點繼續播放當前歌曲。

在 `else` 塊內，將 `audio` 對象的 `currentTime` 屬性設置爲存儲在 `userData?.songCurrentTime` 中的值。
...
你應該在 `if` 語句的 `else` 塊內將 `audio.currentTime` 設置爲 `userData?.songCurrentTime`。
