...
もし別の種類のメディアアイテム、例えばテレビシリーズを表したい場合はどうしますか？別のクラスを作成し、それを`Movie`から継承させて、すべてのメソッドと属性にアクセスできるようにすることができます。

空のクラス`TVSeries`を作成し、`Movie`から継承させてください。`Child`クラスが`Parent`クラスを継承する方法は以下の通りです。
...
`TVSeries`クラスは`Movie`から継承しているはずです。
