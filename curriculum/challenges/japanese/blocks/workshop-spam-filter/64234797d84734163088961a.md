...
最後に、正規表現が単語全体にマッチするようにしてください（`freeRegex`と同じように）。

# --hints--

`stockRegex` はキャプチャしないグループを使う必要があります。

```js
assert.match(stockRegex.source, /\(\?:/);
```

`stockRegex` は `\s` または `^` にマッチするキャプチャしないグループを使う必要があります。

```js
assert.match(stockRegex.source, /\(\?:(\^\|\\s|\\s\|\^)\)/);
```

`stockRegex` は2つ目のキャプチャしないグループを使う必要があります。
...
`stockRegex` は `it's stock alert time` にマッチする必要があります。

```js
assert.match("it's stock alert time", stockRegex);
```

`stockRegex` は `stock alert time` にマッチする必要があります。

```js
assert.match('stock alert time', stockRegex);
```

`stockRegex` は `it's stock alert` にマッチする必要があります。

```js
assert.match("it's stock alert", stockRegex);
```

`stockRegex` は `stock alert` にマッチする必要があります。

```js
assert.match('stock alert', stockRegex);
```

`stockRegex` は `hands-stock alert time` にマッチしてはいけません。

```js
assert.notMatch('hands-stock alert', stockRegex);
```

`stockRegex` は `stock alert-management` にマッチしてはいけません。
