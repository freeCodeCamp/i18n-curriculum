...
`initializeEventListeners` 메서드 안에서 `singleCardBtn`에 `click` 이벤트 핸들러를 붙이세요. 이벤트 핸들러 내부에서 `singleCardSelected` 메서드를 호출하세요. `singleCardBtn`은 `elements`에서 가져오는 것을 잊지 마세요.
