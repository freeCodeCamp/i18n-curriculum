...
Першим аргументом для вашого виклику `evalFormula` має бути вміст клітинки (який ви зберегли в `value`). Однак вміст починається з символу `=`, який запускає функцію, тому потрібно передати підрядок `value`, починаючи з індексу `1`.

# --hints--

Ви повинні передати `value` як перший аргумент для вашого виклику `evalFormula()`.
...
Ви повинні викликати метод `.slice()` на аргументі `value`.
...
Ви повинні передати число `1` як аргумент для вашого виклику `.slice()`.
