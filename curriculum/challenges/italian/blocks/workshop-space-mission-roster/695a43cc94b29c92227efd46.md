...
Hai aggiunto una chiamata `console.log()` al tuo script per validare il tuo array `EVAChunks`, e ora la chiamata non è più necessaria. Per mantenere il terminale pulito per i passaggi successivi, elimina il blocco `for loop` che registra i dati da `EVAChunks`.

# --hints--

Dovresti rimuovere il blocco `for loop` che registra i dati da `EVAChunks`. Elimina l'intero blocco che contiene entrambi i loop.
