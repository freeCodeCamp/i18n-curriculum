...
由於你的 `update` 事件作爲 `change` 事件監聽器運行，因此 `event` 參數將是 change 事件。

Change 事件的 `target` 屬性代表發生變化的元素。 將 `target` 屬性分配給名爲 `element` 的新變量。

# --hints--

你應該在 `update` 函數中聲明一個 `element` 變量。

```js
assert.match(code, /const\s+update\s*=\s*(\(\s*event\s*\)|event)\s*=>\s*\{\s*(?:var|let|const)\s+element/);
```

你應該使用 `const` 來聲明你的 `element` 變量。

```js
assert.match(code, /const\s+update\s*=\s*(\(?\s*event\s*\)|event)\s*=>\s*\{\s*const\s+element/);
```

你應該將 `event` 參數的 `target` 屬性分配給你的 `element` 變量。
