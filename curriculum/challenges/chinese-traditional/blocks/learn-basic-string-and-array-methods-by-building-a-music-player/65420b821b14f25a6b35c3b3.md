...
當歌曲顯示在頁面上時，應該顯示每首歌曲的標題、藝術家、時長和刪除按鈕。 

在接下來的幾個步驟中，你將學習如何使用 <dfn>map()</dfn> 方法為每首歌曲添加新的 HTML。這個方法會在下一步中更詳細說明。

首先使用 `const` 宣告一個名為 `songsHTML` 的變數，並將 `array.map()` 指定給它。

# --hints--

你應該有一個名為 `songsHTML` 的 `const` 變數。

```js
assert.match(code, /const\s+songsHTML/);
```

你應該將 `array.map()` 指定到你的 `songsHTML`。
