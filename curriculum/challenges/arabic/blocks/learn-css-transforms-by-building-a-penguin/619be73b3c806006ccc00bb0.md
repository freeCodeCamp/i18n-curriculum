...
أعطِ العنصر المزيف `width` نصف عرض العنصر الأصل، و`height` بقيمة `45%`، و`background-color` بلون `gray`.

# --hints--

يجب أن تعطي `.penguin-body::before` `width` بقيمة `50%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.width, '50%');
```

يجب أن تعطي `.penguin-body::before` `height` بقيمة `45%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.height, '45%');
```

يجب أن تعطي `.penguin-body::before` `background-color` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
