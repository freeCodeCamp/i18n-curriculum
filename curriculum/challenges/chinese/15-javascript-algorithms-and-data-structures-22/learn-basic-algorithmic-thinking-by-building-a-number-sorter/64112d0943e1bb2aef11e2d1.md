...
要对数组中的元素进行排序，可以使用名为 `.sort()` 的内置方法。 因此，你可以将在 `inputValues` 数组上调用 `.sort()` 的结果赋值给 `sortedValues` 变量，从而更新这个变量。

# --hints--

你应该更新 `sortedValues` 变量，将在 `inputValues` 数组上调用 `.sort()` 方法的结果赋值给它。
