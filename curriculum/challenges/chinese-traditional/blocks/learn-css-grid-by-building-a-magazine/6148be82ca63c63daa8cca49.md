...
創建一個 `.author` 選擇器，並為它設定 `font-size` 屬性為 `2rem`，以及設定 `font-family` 屬性為 `Raleway`，備用字型為 `sans-serif`。
...
你的 `.author` 選擇器應該有一個 `font-size` 屬性設定為 `2rem`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

你的 `.author` 選擇器應該有一個 `font-family` 屬性設定為 `Raleway`，並以 `sans-serif` 作為備用字型。
