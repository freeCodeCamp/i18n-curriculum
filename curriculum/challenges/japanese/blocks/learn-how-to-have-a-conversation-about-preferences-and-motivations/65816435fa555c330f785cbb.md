---
id: 65816435fa555c330f785cbb
title: タスク 44
challengeType: 22
dashedName: task-44
lang: en-US
---
<!-- (Audio) Tom: It's exhausting, and it feels like I'm constantly running without knowing where I'm going. -->

# --description--

何かが `exhausting` のとき、それは長い仕事や運動の後のようにとても疲れた気持ちにさせます。たとえば、一日中サッカーをしてその後とても疲れたとき、`Playing soccer all day is exhausting` と言うでしょう。

`Constantly` は、絶えずまたは非常に頻繁に起こることを意味します。もし毎日欠かさず宿題をしているなら、`I'm constantly doing these tasks` と言えます。

# --fillInTheBlank--

## --sentence--

`It's BLANK, and it feels like I'm BLANK running BLANK knowing where I'm going.`

## --blanks--

`exhausting`

### --feedback--

この単語は、とても疲れた気持ちを表すときに使います。

---

`constantly`

### --feedback--

この単語は、何かが絶えずまたは非常に頻繁に起こっていることを意味します。

---

`without`

### --feedback--

この単語は、何かが存在しないことを示すときに使います。

# --scene--

```json
{
  "setup": {
    "background": "company2-roof.png",
    "characters": [
      {
        "character": "Tom",
        "position": {"x":50,"y":15,"z":1.2},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "4.1-2.mp3",
      "startTime": 1,
      "startTimestamp": 12.82,
      "finishTimestamp": 17.12
    }
  },
  "commands": [
    {
      "character": "Tom",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Tom",
      "startTime": 1,
      "finishTime": 5.30,
      "dialogue": {
        "text": "It's exhausting and it feels like I'm constantly running without knowing where I'm going.",
        "align": "center"
      }
    },
    {
      "character": "Tom",
      "opacity": 0,
      "startTime": 5.80
    }
  ]
}
```
