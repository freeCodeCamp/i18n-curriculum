...
نظف المخرجات قليلاً. احذف `print(char, index)`، وحول آخر استدعاء لـ `print()` إلى `print('char:', char, 'new char:', new_char)`.

# --hints--

يجب ألا يحتوي شفرتك على `print(char, index)`.
...
غيّر `print(new_char)` إلى `print('char:', char, 'new char:', new_char)`.
