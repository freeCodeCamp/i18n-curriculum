...
`freeRegex` のように、`stockRegex` を更新して、`e` と `o` の文字を文字クラスに置き換え、文字と対応する数字にマッチさせてください。

# --hints--

`stockRegex` は文字クラスを使って、文字 `e` と数字 `3` にマッチさせる必要があります。

```js
assert.match(stockRegex.source, /\[(e3|3e)\]/);
```

`stockRegex` は文字クラスを使って、文字 `o` と数字 `0` にマッチさせる必要があります。
...
`stockRegex` は `st0ck al3rt` にマッチする必要があります。
