...
接下來，你需要在播放器顯示中顯示目前的歌曲標題和歌手。使用 `const` 和箭頭語法來創建一個空的 `setPlayerDisplay` 函式（程式）。

# --hints--

你應該使用 `const` 來創建一個名為 `setPlayerDisplay` 的空函式（程式）。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` 應該是一個函式（程式）。

```js
assert.isFunction(setPlayerDisplay)
```

你的 `setPlayerDisplay` 函式（程式）應該使用箭頭語法。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `setPlayerDisplay` 函式（程式）應該是空的。
