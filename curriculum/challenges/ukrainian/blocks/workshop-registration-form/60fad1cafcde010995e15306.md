...
При відправленні форм корисно і є доброю практикою надавати кожному елементу, який можна відправити, атрибут `name`. Цей атрибут використовується для ідентифікації елемента у відправленій формі.

Окрім двох `radio` вхідних даних (які ви вже назвали), надайте кожному елементу, який можна відправити, унікальний атрибут `name` на свій вибір.

# --hints--

Ви повинні надати `input`, який очікує ім’я, атрибут `name`. _PS Я б обрав `first-name`_

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(1) > label:nth-child(1) > input')?.name);
```

Ви повинні надати `input`, який очікує прізвище, атрибут `name`. _PS Я б обрав `last-name`_

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(1) > label:nth-child(2) > input')?.name);
```

Ви повинні надати `email` атрибут `name`. _PS Я б обрав `email`_

```js
assert.isNotEmpty(document.querySelector('input[type="email"]')?.name);
```

Ви повинні надати `password` атрибут `name`. _PS Я б обрав `password`_

```js
assert.isNotEmpty(document.querySelector('input[type="password"]')?.name);
```

Ви повинні надати `checkbox` атрибут `name`. _PS Я б обрав `terms`_

```js
assert.isNotEmpty(document.querySelector('input[type="checkbox"]')?.name);
```

Ви повинні надати `file` атрибут `name`. _PS Я б обрав `file`_

```js
assert.isNotEmpty(document.querySelector('input[type="file"]')?.name);
```

Ви повинні надати `number` атрибут `name`. _PS Я б обрав `age`_

```js
assert.isNotEmpty(document.querySelector('input[type="number"]')?.name);
```

Ви повинні надати `select` елементу атрибут `name`. _PS Я б обрав `referrer`_

```js
assert.isNotEmpty(document.querySelector('select')?.name);
```

Ви повинні надати `textarea` елементу атрибут `name`. _PS Я б обрав `bio`_

```js
assert.isNotEmpty(document.querySelector('textarea')?.name);
```

Ви не повинні надавати жодному з `option` елементів атрибут `name`.

```js
[...document.querySelectorAll('option')]?.forEach(option => assert.isUndefined(option?.name));
```

Ви не повинні надавати жодному з `label` елементів атрибут `name`.

```js
[...document.querySelectorAll('label')]?.forEach(label => assert.isUndefined(label?.name));
```

Ви не повинні надавати жодному з `fieldset` елементів атрибут `name`.
