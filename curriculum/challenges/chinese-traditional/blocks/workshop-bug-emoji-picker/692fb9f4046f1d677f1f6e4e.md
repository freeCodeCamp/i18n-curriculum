...
創建一個名為 `isSelect` 的函式（程式），它接受一個型別為 `element` 的單一參數 `EventTarget | null`。此函式將檢查所選元素是否為 `<select>` HTML 元素。

# --hints--

你應該有一個名為 `isSelect` 的函式（程式）。
...
你的 `isSelect` 函式（程式）應該有一個名為 `element`、型別為 `EventTarget | null` 的參數。
...
你的 `isSelect` 函式（程式）應該有 `element is HTMLSelectElement` 的返回型別。
