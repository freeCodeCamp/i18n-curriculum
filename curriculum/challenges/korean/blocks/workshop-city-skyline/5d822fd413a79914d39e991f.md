...
`background-color` 속성과 값을 `.fb2`에서 `.fb2b`로 옮겨 컨테이너가 아니라 섹션에만 색이 적용되게 하세요.

# --hints--

`background-color`에서 `.fb2` 속성을 제거해야 합니다.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb2")?.backgroundColor);
```

`.fb2b`에 `background-color` 값을 가진 `--building-color3`를 지정해야 합니다.
