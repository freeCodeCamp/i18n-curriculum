...
プレイリストが空の場合、`resetButton` 要素とそのテキストを作成する必要があります。このボタンはプレイリストが空の場合にのみ表示されます。

`createElement()` は、JavaScriptを使って動的に要素を作成できるDOMメソッドです。`createElement()` を使うには、それを呼び出し、タグ名を文字列で渡します。
...
`if` 文の中で、`resetButton` 定数を宣言し、`createElement()` を使って`"button"` を作成してください。
...
`createElement` メソッドを使って`"button"` を作成してください。

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

新しく作成した`button` 要素を`resetButton` 定数に代入してください。
