...
يجب أن تكون بداخل عنصر `.eye` خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.top, '45%');
```

يجب أن تكون بداخل عنصر `.eye` خاصية `border-radius` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
