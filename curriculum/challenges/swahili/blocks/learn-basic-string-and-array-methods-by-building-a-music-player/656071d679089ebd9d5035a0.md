...
Kwa kuwa watumizi wataweza kuchanganya na kufuta nyimbo kutoka kwenye orodha ya nyimbo, utahitaji kuunda nakala ya safu ya data ya `allSongs` bila kubadilisha asili yake. Hapa ndipo opereta ya spread (`spread`) inavyosaidia.

Opereta ya spread (`...`) inakuwezesha kunakili vipengele vyote kutoka kwenye safu ya data moja kwenda nyingine. Pia inaweza kutumika kuunganisha safu za data nyingi kuwa moja. Katika mfano uliopo hapa chini, `arr1` na `arr2` zote zimeenea ndani ya `combinedArr`:
...
Ndani ya kitu cha `userData` tengeneza sifa ya `songs`. Kwa thamani, enea `allSongs` ndani ya safu ya data.

# --hints--

Kitu chako cha `userData` kinapaswa kuwa na ufunguo uitwao `songs`.

```js
assert.property(userData, "songs");
```

Sifa yako ya `songs` inapaswa kuwa na thamani ya `[...allSongs]`.
