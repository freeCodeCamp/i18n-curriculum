...
Sasa, ndani ya template literal ya kitendakazi cha `renderCard`, tengeneza kipengele cha `div`. Ndani ya `div`, ongeza kipengele cha `h2` kinachotoa `drawingType`, na kipengele cha `figure` chenye darasa la vitu la `card_container ${isReversed ? "reversed-card" : ""}` na sifa ya kipengele `data-id="${shortName}"`.

# --hints--

Unapaswa kuwa na kipengele cha `div` ndani ya template literal ya kitendakazi cha `renderCard`.

```js
const tmpl = __helpers.removeJSComments(code).match(/renderCard[\s\S]*?=>\s*\{?\s*`([\s\S]*?)`/)?.[1] ?? '';
assert.match(tmpl, /<div>/);
```

Unapaswa kuwa na kipengele cha `h2` kinachojumuisha `drawingType` ndani ya kipengele cha `div`.
...
Unapaswa kuwa na kipengele cha `figure` ndani ya kipengele cha `div`.
...
Kipengele chako cha `figure` kinapaswa kuwa na darasa la vitu la `card_container` na kutumia kikokotoo cha masharti cha pande tatu kuongeza `reversed-card` kwa masharti.
...
Kipengele chako cha `figure` kinapaswa kuwa na sifa ya kipengele ya `data-id` iliyowekwa kuwa `${shortName}`.
