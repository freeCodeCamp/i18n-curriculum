...
Вражає, що `f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3.1415926… ≈ π.

Насправді, це найкраще наближення π у формі `f(a, 200) + f(b, 200) + f(c, 200) + f(d, 200)`.

Нехай `almostPi(n)` = a<sup>2</sup> + b<sup>2</sup> + c<sup>2</sup> + d<sup>2</sup> для a, b, c, d, які мінімізують помилку: $\lvert f(a,n) + f(b,n) + f(c,n) + f(d,n) - \Pi\rvert$

Вам задано `almostPi(200)` = 6<sup>2</sup> + 75<sup>2</sup> + 89<sup>2</sup> + 226<sup>2</sup> = 64658.
...
`almostPi` має повертати число.

```js
assert.strictEqual(typeof almostPi(10), 'number');
```

`almostPi(29)` має повертати `1208`.

```js
assert.strictEqual(almostPi(29), 1208);
```

`almostPi(50)` має повертати `4152`.

```js
assert.strictEqual(almostPi(50), 4152);
```

`almostPi(200)` має повертати `64658`.
