...
声明一个变量 `cellRegex` 来匹配单元格引用。 它应该匹配从 `A` 到 `J` 的一个字母，后跟从 `1` 到 `9` 的一个数字，以及从 `0` 到 `9` 的一个可选数字。 使正则表达式不区分大小写并且具有全局性。

# --hints--

你应该声明一个 `cellRegex` 变量。
