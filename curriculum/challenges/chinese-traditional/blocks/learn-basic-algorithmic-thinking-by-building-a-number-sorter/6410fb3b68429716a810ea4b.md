...
選擇排序依賴於追蹤陣列中最小值的索引。宣告一個變數 `minIndex` 並將它設值為 `i`－這確保如果你目前的值是最小的，它將與自己交換而不會被移動。當你迭代陣列時，你需要能夠重新指定 `minIndex` 的值。

然後，寫另一個使用 `j` 作為迭代器的 `for` 迴圈。這個迴圈需要從 `i` 之後的索引開始，並迭代整個陣列的其餘部分。

# --hints--

你應該使用 `let` 來宣告 `minIndex`。
