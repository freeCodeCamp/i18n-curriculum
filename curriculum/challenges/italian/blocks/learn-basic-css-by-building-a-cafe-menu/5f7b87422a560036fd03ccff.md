...
Per i due elementi `p` che hai appena aggiunto, aggiungi `dessert` come valore dell'attributo `p` del primo elemento `class` e il valore `price` come attributo `p` del secondo elemento `class`.
...
Il tuo elemento `p` con il testo `Donut` dovrebbe avere la classe `dessert`.

```js
assert.match(document.querySelector('.dessert')?.innerText, /donut/i);
```

Il tuo elemento `p` con il testo `1.50` dovrebbe avere la classe `price`.
