...
Ora sei pronto per iniziare a mettere insieme il layout a griglia. Nelle lezioni precedenti hai imparato che CSS Grid offre un layout bidimensionale basato su griglia, che ti permette di centrare gli elementi orizzontalmente e verticalmente mantenendo comunque il controllo per fare cose come sovrapporre elementi.

Inizia creando un selettore `main` e impostando su di esso una proprietà `display` con valore `grid`.

# --hints--

Dovresti avere un selettore `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Il tuo selettore `main` dovrebbe avere una proprietà `display` impostata su `grid`.
