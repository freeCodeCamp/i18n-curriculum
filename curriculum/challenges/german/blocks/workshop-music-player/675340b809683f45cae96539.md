...
Wenn Sie die Zurück-Taste drücken, während kein Lied abgespielt wird, wird das erste Lied abgespielt. Um das zu beheben, fügen Sie am Anfang der `if`-Funktion eine `playPreviousSong`-Anweisung hinzu. Prüfen Sie, ob das aktuelle Lied `null` ist, und stoppen Sie die Ausführung der Funktion mit einer `return`-Anweisung.

# --hints--

Sie sollten eine `if`-Anweisung hinzufügen, die überprüft, ob `userData.currentSong === null` ist, und die Ausführung der Funktion mit einer `return`-Anweisung innerhalb Ihrer `playPreviousSong`-Funktion stoppt.

```js
assert.match(
  playPreviousSong.toString(),
  /if\s*\(\s*userData\.currentSong\s*===\s*null\s*\)\s*return/
);
```
