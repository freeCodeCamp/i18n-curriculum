...
Вам потрібно підтвердити, що користувач прочитав умови.

Додайте елемент `label`. Всередині новоствореного елемента `label` додайте елемент `input` і встановіть атрибут `type` зі значенням `checkbox`. Зробіть цей елемент `input` `required`, щоб користувачі не могли зареєструватися без згоди з умовами.

Додайте атрибут `id` і `for` зі значенням `terms-and-conditions` до елементів для доступності.

# --hints--

Ви повинні додати `label` після третього елемента `fieldset`.
