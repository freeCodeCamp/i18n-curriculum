...
O cartão de preço premium deve ser o que mais se destaca na página.Além disso, é o único cartão que terá um selo `Most popular`.Para destacar este card e seu badge, você pode usar as classes utilitárias de posicionamento relativo e escala do Tailwind.

Aqui está como você aumentaria o tamanho de um elemento com scaling no Tailwind CSS:
...
Você cuidará do posicionamento nas próximas etapas.Por enquanto, atribua a classe `scale-105` ao `div` wrapper para torná-lo um pouco maior que os outros cartões de preços.

# --hints--

Seu elemento wrapper `div` para o plano premium deve ter a classe `scale-105`.
