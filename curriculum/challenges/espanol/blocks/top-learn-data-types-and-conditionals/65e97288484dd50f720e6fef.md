...
Declaraciones condicionales más complejas pueden incluir múltiples condiciones. Es por eso que se usan los operadores lógicos para combinar múltiples condiciones.  The logical operators are `&&`, `||` and `!` which are used to represent `and`, `or` and `not` respectively.
...
The logical operator `&&` is used to combine two boolean conditions. Devuelve `true` solo si ambas condiciones son `true`. De lo contrario, devuelve `false`:
