...
新しく追加した `Read our terms and conditions` の前に、テキスト `label` を持つアンカー要素を追加してください。`href` を次のように設定します：

```md
https://www.freecodecamp.org/news/terms-of-service/
```

その後、ラベル内の入力要素の直後にテキスト `I accept the terms and conditions` を追加してください。

# --hints--

`a` の直前に、テキスト `label` を持つ `Read our terms and conditions` 要素があるはずです。

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.innerText.trim().replaceAll(/\s+/g, ' '), /Read our terms and conditions\.?/i);
```

`a` 要素は `href` の `https://www.freecodecamp.org/news/terms-of-service/` を持つべきです。

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.href, /https:\/\/www\.freecodecamp\.org\/news\/terms-of-service\/?/);
```

`input` の中に `label` 要素があるはずです。

```js
assert.exists(document.querySelector('fieldset:nth-child(3) + a + label > input'));
```

`label` の中で、`input` 要素の直後にテキスト `I accept the terms and conditions` があるべきです。
