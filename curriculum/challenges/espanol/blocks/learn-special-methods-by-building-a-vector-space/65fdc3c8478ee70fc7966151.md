...
De la misma manera que hiciste antes para el método `__eq__`, crea una estructura `if` que verifique si `self` y `other` no pertenecen a la misma clase y devuelve `NotImplemented`.
