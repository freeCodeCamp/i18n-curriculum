...
코드를 작성하면서 테스트하려면 `sortButton` 요소에 이벤트 리스너를 장착하세요. `"click"` 이벤트를 듣고 `sortInputArray`를 콜백으로 사용해야 합니다.

# --hints--

`.addEventListener()` 요소에서 `sortButton` 메서드를 호출하세요.
...
`.addEventListener()` 메서드는 `sortInputArray`를 콜백으로 받아야 합니다. 함수 호출이 아니라 함수 참조를 전달하는 것을 기억하세요.
