...
Перш ніж почати працювати з відтворенням наступної та попередньої пісні, потрібно отримати індекс кожної пісні у властивості `songs` об’єкта `userData`.

Почніть зі створення стрілкової функції з назвою `getCurrentSongIndex`.

# --hints--

У вас має бути функція `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Ваша функція `getCurrentSongIndex` має використовувати стрілковий синтаксис.
