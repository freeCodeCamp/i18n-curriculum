...
Bleiben Sie innerhalb der `while`-Schleife und erhöhen Sie den Wert von `left_array_index` um 1.

# --hints--

Sie sollten den aktuellen Wert von `left_array_index` innerhalb der `while`-Schleife um 1 erhöhen. Beachten Sie, dass die Erhöhung nach der Zuweisung des linken Teils zum sortierten Array erfolgen sollte.
