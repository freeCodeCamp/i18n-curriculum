...
この最後のステップでは、前のステップで計算した値を使って映画のチケットの最終価格を計算します。

最終的なチケット価格は、基本価格に追加料金とサービス料金を加え、そこから割引を差し引いて計算します。

最後の`if`文の本文内で、`print('Service charges:', service_charges)`行の下に、チケットの最終価格を計算し、`final_price`という名前の変数にストアしてください。

最後に、`Final price of ticket:`に続けて`final_price`の値を出力するメッセージを出力してください。

# --hints--

`final_price`という名前の変数を宣言してください。

```js
({ test: () => (runPython(`assert _Node(_code).find_ifs()[4].find_bodies()[0].has_variable("final_price")`)) })
```

`base_price + extra_charges + service_charges - discount`に`final_price`の値を代入してください。
...
`print('Final price of ticket:', final_price)`変数の下に`final_price`があることを確認してください。
