...
Ndani ya kipengele chako cha `.image-quote`, panga ndani kipengele cha `hr`, kipengele cha `p` na kipengele cha pili cha `hr`. Toa kipengele cha `p` sifa ya `class` ikiwa sawa na `quote` na maandishi ya `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.

# --hints--

Unapaswa kuongeza vipengele viwili vya `hr` ndani ya kipengele chako cha `.image-quote`.

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

Unapaswa kuongeza kipengele cha `p` ndani ya kipengele chako cha `.image-quote`.

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

Watoto wako wa `.image-quote` wanapaswa kuwa katika mpangilio sahihi.
...
Kipengele chako kipya cha `p` kinapaswa kuwa na sifa ya `class` ikiwa sawa na `quote`.

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Kipengele chako kipya cha `p` kinapaswa kuwa na maandishi ya `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
