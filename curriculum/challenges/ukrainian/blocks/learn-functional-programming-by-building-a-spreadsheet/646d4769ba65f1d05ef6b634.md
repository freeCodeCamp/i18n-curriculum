...
Тепер ваша функція `update()` справді може обчислювати формули. Пам’ятайте, що ви написали умову `if`, щоб перевірити, що функцію викликали.

Всередині інструкції `if` призначте властивість `value` об’єкта `element` результат вашої функції `evalFormula()`. Поки що не передавайте жодних аргументів.

# --hints--

Ви повинні оновити властивість `value` об’єкта `element` у вашому блоці `if`.
...
Ви повинні призначити властивість `value` результат виклику вашої функції `evalFormula()`.
...
Ви не повинні передавати жодних аргументів у виклик `evalFormula()`.
