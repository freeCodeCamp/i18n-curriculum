...
Teniendo en cuenta las mejores prácticas para los campos de formulario, dale a cada `input` un atributo `type` y `name` apropiado. Luego, dale al primer `input` un atributo `placeholder`.
...
Debes dar al primer `input` un atributo de `name` apropiado.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[0]?.name?.length, 1);
```

Debes dar al segundo `input` un atributo de `name` apropiado.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[1]?.name?.length, 1);
```

Debes dar al tercer `input` un atributo de `name` apropiado.
