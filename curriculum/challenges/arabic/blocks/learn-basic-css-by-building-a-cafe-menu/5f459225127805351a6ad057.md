...
غيّر لون الخلفية لعنصر `hr` إلى `brown` ليطابق لون حبوب القهوة.

# --hints--

يجب أن تضبط قيمة خاصية `background-color` على `brown`.

```js
const hasBackgroundColor = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style['background-color'] === 'brown');
```

يجب أن يكون لعنصر `hr` الخاص بك `background-color` بقيمة `brown`.
