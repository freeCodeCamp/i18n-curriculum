...
`MYVAR` não é o mesmo que `MyVar` nem `myvar`.É possível ter diversas variáveis distintas com o mesmo nome mas com capitalização diferente.É fortemente recomendado que, para maior clareza, você *não* utilize este recurso da linguagem.
