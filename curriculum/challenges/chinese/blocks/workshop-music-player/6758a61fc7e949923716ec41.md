...
现在，使用 `querySelectorAll` 方法定位 `.playlist-song` 元素，并将它们保存到名为 `songs` 的变量中。

然后，对 `songs` 调用 `forEach` 方法。传入一个回调函数，该函数使用 `song` 作为方法的参数。暂时将花括弧留空。
...
你应该将所有 `.playlist-song` 元素的集合赋值给你的 `songs` 变量。

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

你应该在 `songs` 上调用 `forEach` 方法，并传入一个使用 `song` 作为该方法参数的回调。
