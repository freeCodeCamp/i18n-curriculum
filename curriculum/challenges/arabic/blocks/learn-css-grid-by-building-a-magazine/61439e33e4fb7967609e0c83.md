...
بعد ثلاثة عناصر `p` داخل عنصر `.text`، أنشئ عنصر `blockquote`. بداخله، أضف عنصر `hr`، وعنصر `p` مع تعيين `class` إلى `quote`، وعنصر `hr` ثاني.

امنح عنصر `.quote` النص `The entire curriculum should be a series of projects`.

# --hints--

يجب أن تنشئ عنصر `blockquote` جديد داخل عنصر `.text`.

```js
assert.exists(document.querySelector('.text blockquote'));
```

يجب أن يأتي عنصر `blockquote` بعد ثلاثة عناصر `p` الخاصة بك.

```js
assert(document.querySelector('.text')?.children?.[3]?.localName === 'blockquote');
```

يجب أن يحتوي عنصر `blockquote` على عنصرين `hr`.

```js
assert(document.querySelectorAll('.text blockquote hr')?.length === 2);
```

يجب أن يحتوي عنصر `blockquote` على عنصر `p`.

```js
assert.exists(document.querySelector('.text blockquote p'));
```

يجب أن تكون عناصر الأطفال داخل `blockquote` بالترتيب الصحيح.
...
يجب أن يحتوي عنصر `p` الجديد على `class` معين إلى `quote`.

```js
assert(document.querySelector('.text blockquote p')?.className === 'quote');
```

يجب أن يحتوي عنصر `p` الجديد على النص `The entire curriculum should be a series of projects`.
