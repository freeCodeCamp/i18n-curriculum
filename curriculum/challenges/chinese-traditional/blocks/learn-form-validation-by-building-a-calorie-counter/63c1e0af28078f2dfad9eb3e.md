...
現在你需要標的具有你的 `targetId` 的元素內的 `.input-container` 元素。宣告一個新的 `targetInputContainer` 變數，並將 `document.querySelector()` 的值指定給它。使用串接將 `targetId` 和 `'.input-container'` 以空格分隔，並將該字串傳遞給 `querySelector()`。

# --hints--

你的 `addEntry` 函式應該有一個 `targetInputContainer` 變數。

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=/);
```

你的 `targetInputContainer` 變數應該設定為 `document.querySelector()`。

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=\s*document\.querySelector\(/);
```

你應該將 `targetId` 傳遞給你的 `querySelector()` 方法。

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=\s*document\.querySelector\(\s*targetId\s*/);
```

你應該將 `' .input-container'` 串接到 `targetId`。請記得在 `.input-container` 開頭包含空格。
