...
Para este algoritmo, querrás usar un bucle `while`. Este bucle necesita dos condiciones:

- Primero, no debe ejecutarse más allá del comienzo de la matriz (accedido con `j`).
- Segundo, el bucle no debe ejecutarse después de encontrar un valor menor que el valor actual.

Para evitar un bucle infinito, decrementa `j` dentro de tu bucle.
...
Tu bucle `while` debe tener su primera condición que verifique que el valor de `j` es mayor o igual a `0`.
...
Tu bucle `while` debe utilizar el operador AND.
...
Tu bucle `while` debe tener una condición que verifique que el valor de `array[j]` es mayor que `currValue`.
...
Tu bucle `while` debe decrementar `j` dentro del bucle.
