...
Перед тим як рухатися далі, видаліть виклики `console.log` зі свого коду.

# --hints--

У вашому коді не повинно бути `console.log(removePlant(ballerina))`.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*removePlant\s*\(\s*ballerina\s*\)\s*\)/);
```

У вашому коді не повинно бути `console.log(sellPlants(ballerina, "small", 10));`.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

У вашому коді не повинно бути `console.log(catalog)`.
