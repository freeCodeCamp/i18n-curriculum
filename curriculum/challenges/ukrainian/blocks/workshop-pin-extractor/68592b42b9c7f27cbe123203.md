...
У третій лінії вірша бракує третього слова, тому шпилька коротша, ніж очікувалося.

Додайте умову `else`, яка конкатенує `'0'` з `secret_code`, коли слів недостатньо, щоб усі рядки вірша використовувалися для пошуку цифр.
...
Умова `else` має містити `secret_code += '0'`.
...
`pin_extractor(poem)` має повертати `5202`.
