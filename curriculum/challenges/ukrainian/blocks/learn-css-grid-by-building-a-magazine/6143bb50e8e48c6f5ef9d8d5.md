...
У межах вашого елемента `aside` створіть два елементи `img`, один елемент `blockquote` і третій елемент `img`. Дайте елементу `blockquote` множину `class`, встановлену в `image-quote`.

# --hints--

Ви маєте створити три елементи `img` всередині вашого елемента `aside`.

```js
assert(document.querySelectorAll('aside img')?.length === 3);
```

Ви маєте створити елемент `blockquote` всередині вашого елемента `aside`.

```js
assert.exists(document.querySelector('aside blockquote'));
```

Ваш елемент `blockquote` має мати множину `class`, встановлену в `image-quote`.
