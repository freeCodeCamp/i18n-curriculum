...
텍스트가 `label`인 또 다른 `Real Name`를 만들고 그 안에 `input` 유형의 `password`를 만드세요. `input`에 `value`인 `realName`를 지정하세요.

그런 다음 `onChange` 이벤트를 사용해 `realName`를 업데이트한 것과 같은 방식으로 `heroName`를 업데이트하세요.
...
`label` 요소는 텍스트로 `Real Name`를 가져야 합니다.
...
`input` 요소 안에 `password` 유형의 `label`를 만들어야 합니다.
...
`input` 요소는 값이 `realName` 상태 변수로 설정되어야 합니다.

```js
assert.match(code, /value\s*=\s*\{\s*realName\s*\}/)
```

`onChange` 이벤트를 사용해 `realName` 필드에 입력할 수 있도록 `input` 상태 변수 값을 설정해야 합니다.
