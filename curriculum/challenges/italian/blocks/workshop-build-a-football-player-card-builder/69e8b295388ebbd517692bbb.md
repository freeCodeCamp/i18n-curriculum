...
Dopo l'elemento `card-image-wrap`, aggiungi un `div` con il `className` di `card-name-strip`. Al suo interno, aggiungi un `span` con il `className` di `card-name` e il testo `PELE`.

# --hints--

Dovresti aggiungere un `div` con il `className` di `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

Dovresti aggiungere un `span` con il `className` di `card-name` all'interno di `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

Lo span `.card-name` dovrebbe contenere il testo `PELE`.
