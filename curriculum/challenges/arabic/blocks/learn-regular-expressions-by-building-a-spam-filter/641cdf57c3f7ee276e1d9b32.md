...
إليك مثال على تعيين نتيجة معامل ثلاثي إلى محتوى نص عنصر:

```js
el.textContent = condition ? "Use this text if the condition is true" : "Use this text if the condition is false";
```

بعد جملة `if`، استخدم معامل ثلاثي للتحقق من صحة استدعاء `isSpam()` مع `messageInput.value` كمعلمة. إذا كانت النتيجة صحيحة، عيّن خاصية `textContent` في عنصر `result` إلى `"Oh no! This looks like a spam message."`. إذا لم تكن كذلك، عيّنها إلى `"This message does not seem to contain any spam."`

ثم عيّن خاصية `value` في عنصر `messageInput` إلى سلسلة فارغة.

# --hints--

يجب أن تستخدم معامل التعيين لتعيين خاصية `textContent` في عنصر `result`.
...
يجب أن تستدعي الدالة `isSpam()` بعد معامل التعيين `=` وقبل معامل الثلاثي `?`.
...
يجب أن تستخدم صيغة المعامل الثلاثي للتحقق من صحة `isSpam(messageInput.value)`.
...
يجب أن تُعيّن التعبير الصحيح في الثلاثي الخاص بك خاصية `textContent` لعنصر `result` إلى `"Oh no! This looks like a spam message."`.
...
يجب أن تُعيّن التعبير الزائف في الثلاثي الخاص بك الخاصية `textContent` لعنصر `result` إلى `"This message does not seem to contain any spam."`.
...
بعد المعامل الثلاثي، عيّن خاصية `value` في عنصر `messageInput` إلى سلسلة فارغة.
