...
لتمييز مربعات الاختيار، أضف خاصية `checked` إلى مربع اختيار الـ `input`. استخدم طريقة `includes` للتحقق مما إذا كانت الـ `power` الحالية موجودة في مصفوفة `powers`.

سيكون `onChange` دالة منفصلة، عيّنها إلى `handlePowersChange` في الوقت الحالي. هذا سيؤدي إلى ظهور خطأ ستصلحه في الخطوة التالية.

# --hints--

يجب أن تضبط حالة `checked` لمربع الاختيار لتعكس ما إذا كانت قيمة القوة الحالية `power` موجودة في مصفوفة القوى.

```js
assert.match(code, /checked\s*=\s*\{\s*powers\s*\.\s*includes\s*\(\s*power\s*\)\s*\}/)
```

يجب أن تضبط خاصية `onChange` لمربع الاختيار إلى `{handlePowersChange}`.
