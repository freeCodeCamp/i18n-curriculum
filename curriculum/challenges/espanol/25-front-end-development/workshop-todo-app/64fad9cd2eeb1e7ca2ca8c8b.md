...
Usa la sintaxis de flecha para crear una función `updateTaskContainer`. Luego mueve `taskData.forEach()` y su contenido del evento `submit` del `taskForm` a la nueva función creada.
...
Debes mover `taskData.forEach()` y su contenido a la función `updateTaskContainer()`.
