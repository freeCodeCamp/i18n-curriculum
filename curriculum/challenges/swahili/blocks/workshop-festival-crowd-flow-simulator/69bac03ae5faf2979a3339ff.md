...
Sasa unajua ni lango gani la njia ya data ya kuhamisha, unaweza kusogeza washiriki waliokuzidi.

Chukua `overflowAmount` na uiongeze kwenye foleni ya lango lijalo kwa `tickIndex` ile ile.

Hasa, lazima:

- Upate lango lijalo kwa kutumia `nextGateIndex`.
- Upate `queue` yake.
- Ongeza `overflowAmount` kwenye thamani iliyoko kwenye nafasi ya `tickIndex`.

# --hints--

Kitendakazi chako cha `rerouteOverflow` kinapaswa kuongeza `overflowAmount` kwenye thamani ya `tickIndex` katika `queue` ya lango lijalo.
