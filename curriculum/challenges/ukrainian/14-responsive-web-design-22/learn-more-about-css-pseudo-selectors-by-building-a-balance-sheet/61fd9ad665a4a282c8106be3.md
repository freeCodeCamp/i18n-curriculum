...
Створіть елемент `tr` в `thead`. Всередину додайте `td` та три елементи `th`. Within each of the `th` elements, add a `span` element with the `class` set to `sr-only` and the following text, in order: `2019`, `2020`, and `2021`.
...
Кожен з ваших нових елементів `span` повинен мати атрибут `class` зі значенням `sr-only`.
