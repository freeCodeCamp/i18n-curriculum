...
سيحدد منتقي `span[class~="sr-only"]` أي عنصر `span` يحتوي على سمة `class` التي *تتضمن* اسم `sr-only`. أنشئ هذا منتقي، واعطيه خاصية `border` بقيمة `0`.

# --hints--

يجب أن يكون لديك منتقي `span[class~="sr-only"]`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]'));
```

يجب أن يحتوي منتقيك `span[class~="sr-only"]` على خاصية `border` بقيمة `0`.
