...
All'interno del secondo elemento `section`, aggiungi due elementi `div` con una classe `question-block`.

Poi, all'interno di ogni elemento `div.question-block`, aggiungi un elemento `h3` con un testo di numeri incrementali, a partire da `1`, e un elemento `fieldset` con una classe `question`.

# --hints--

Dovresti annidare due elementi `div` all'interno del secondo elemento `section`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.length, 2);
```

Dovresti dare al primo nuovo elemento `div` una classe `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[0]?.className, 'question-block');
```

Dovresti dare al secondo nuovo elemento `div` una classe `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[1]?.className, 'question-block');
```

Dovresti annidare un elemento `h3` all'interno di ogni elemento `div.question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')?.length, 2);
```

Dovresti dare al primo elemento `h3` un testo `1`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')[0]?.textContent.trim(), '1');
```

Dovresti dare al secondo elemento `h3` un testo `2`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')[1]?.textContent.trim(), '2');
```

Dovresti annidare un elemento `fieldset` all'interno di ogni elemento `div.question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > fieldset')?.length, 2);
```

Dovresti posizionare il primo elemento `fieldset` dopo il primo elemento `h3`.

```js
assert.exists(document.querySelector('section:nth-of-type(2) > div.question-block > h3 + fieldset'));
```

Dovresti posizionare il secondo elemento `fieldset` dopo il secondo elemento `h3`.

```js
assert.exists(document.querySelector('section:nth-of-type(2) > div.question-block:nth-of-type(2) > h3 + fieldset'));
```

Dovresti dare al primo elemento `fieldset` una classe `question`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > fieldset')?.[0]?.className, 'question');
```

Dovresti dare al secondo elemento `fieldset` una classe `question`.
