...
Друга умова в цьому прикладі перевіряє, чи `a.name > b.name`. Якщо так, функція повертає `1`, що сортує перший фрукт після другого.
...
Усередині вашої функції зворотного виклику додайте ще одну інструкцію `if`, щоб перевірити, чи `a.title` більше за `b.title`. Якщо так, поверніть число `1`. 

# --hints--

У вашій функції зворотного виклику має бути друга інструкція `if`.
...
Ваша інструкція `if` має перевіряти, чи `a.title` більше за `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
``` 

Ваша інструкція `if` має повертати число `1`, якщо `a.title` більше за `b.title`.
