...
잡지에서는 인쇄된 내용의 레이아웃을 구성하고 내용 흐름을 조절하기 위해 종종 양쪽 정렬 텍스트를 사용합니다. 인쇄물에서는 효과적이지만, 웹사이트에서 양쪽 정렬 텍스트는 접근성 문제를 일으킬 수 있으며, 예를 들어 난독증이 있는 사용자에게 어려움을 줄 수 있습니다.

프로젝트가 인쇄된 잡지처럼 보이도록 `.text` 선택자에 `text-align` 속성을 `justify`로 설정하세요.

# --hints--

`.text` 선택자에 `text-align` 속성을 `justify`로 설정해야 합니다.
