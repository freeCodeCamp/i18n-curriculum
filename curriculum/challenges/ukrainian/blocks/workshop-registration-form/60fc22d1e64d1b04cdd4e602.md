...
Додайте трохи пробілу між елементами `.inline` і текстом `label`, задавши праве поле `margin` зі значенням `0.5em`. Також встановіть усі інші поля в значення `0`.

# --hints--

Ви повинні задати елементам `.inline` поле `margin-top` зі значенням `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginTop, '0px');
```

Ви повинні задати елементам `.inline` поле `margin-right` зі значенням `0.5em`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginRight, '0.5em');
```

Ви повинні задати елементам `.inline` поле `margin-bottom` зі значенням `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginBottom, '0px');
```

Ви повинні задати елементам `.inline` поле `margin-left` зі значенням `0`.
