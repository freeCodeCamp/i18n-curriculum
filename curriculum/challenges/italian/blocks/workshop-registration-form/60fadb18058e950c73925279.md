...
Durante lo sviluppo, è utile vedere i bordi predefiniti di `fieldset`. Tuttavia, rendono il contenuto troppo separato.

Rimuovi il `border` e aggiungi `2rem` di spaziatura interna solo nella parte superiore e inferiore di ogni `fieldset`. Assicurati di rimuovere il `padding` da sinistra e destra.

# --hints--

Puoi usare un valore di `none` o `0` per rimuovere il `border`.

```js
assert.match(new __helpers.CSSHelp(document).getStyle('fieldset')?.border, /(none)|(0px)|(medium)/);
```

Dovresti aggiungere `padding` di `2rem` nella parte superiore e inferiore di ogni `fieldset`.
...
Dovresti rimuovere il `padding` da sinistra e destra di ogni `fieldset`.
