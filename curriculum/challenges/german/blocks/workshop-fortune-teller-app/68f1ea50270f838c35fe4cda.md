...
Weisen Sie nun `subTitle` `meaning_up` zu und `title` `foundCard.name`. Rufen Sie danach die `showElements`-Methode auf und übergeben Sie `fortuneDescription` aus den Elementen.

# --hints--

Sie sollten `this.elements.subTitle.textContent` `foundCard.meaning_up` zuweisen.
...
Sie sollten `this.elements.title.textContent` `foundCard.name` zuweisen.
...
Sie sollten die `showElements`-Funktion innerhalb von `showFortune` aufrufen.
...
Sie sollten `this.elements.fortuneDescription` als Argument an `showElements` übergeben.
