...
title: "تحدٍّ 306: مستخرج محتوى HTML"
challengeType: 29
...
بالنظر إلى سلسلة نصية من HTML، أعد المحتوى النصي العادي مع إزالة كل الوسوم.

# --hints--

`extract_content('<p>hello world</p>')` يجب أن تُعيد `"hello world"`.
...
`extract_content('<p>hello <span>world</span></p>')` يجب أن تُعيد `"hello world"`.
...
`extract_content('<a href="example.com">Click me</a>')` يجب أن تُعيد `"Click me"`.
...
`extract_content('<p><button onClick="learnToCode()">Learn</button> to <code>code<code> <br/>for <strong>free</strong> <br/>on <a href="https://freecodecamp.org/" target="_blank"><span class="highlight">freecodecamp</span>.org</a>')` يجب أن تُعيد `"Learn to code for free on freecodecamp.org"`.
