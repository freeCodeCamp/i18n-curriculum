...
لإنهاء الخلفية، أضف شمسًا بإنشاء عنصر `div` جديد مباشرة بعد عنصر `.back-mountain`، وأعطه الصنف `sun`.

# --hints--

يجب إضافة عنصر `div` جديد إلى `body`. متوقع وجود `--fcc-expected--` عناصر `div`، لكن وُجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('body > div')?.length, 5);
```

يجب أن تُعطِي عنصر الـ `div` الجديد صنفًا (`class`) اسمه `sun`.

```js
assert.include(document.querySelector('div:not(.back-mountain, .left-mountain, .penguin, .ground)')?.className, 'sun');
```

يجب وضع عنصر `div` الجديد مباشرة بعد عنصر `.back-mountain`.
