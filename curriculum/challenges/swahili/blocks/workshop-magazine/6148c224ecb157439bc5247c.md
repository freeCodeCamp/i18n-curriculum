...
Mipangilio ya chaguo-msingi kwa CSS Grid itaunda safu za ziada inapohitajika, tofauti na Flexbox. Toa sifa ya `.social-icons` kwa kichaguzi cha `grid-template-columns` ikiwa na thamani ya `repeat(5, 1fr)` ili kupanga alama katika safu moja.

# --hints--

Kichaguzi chako cha `.social-icons` kinapaswa kuwa na sifa ya `grid-template-columns` ikiwa na thamani ya `repeat(5, 1fr)`.
