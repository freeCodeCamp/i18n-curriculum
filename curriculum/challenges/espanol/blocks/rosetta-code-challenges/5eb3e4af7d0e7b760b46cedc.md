...
Dados los dos conjuntos `{A,B}` y `{B,D}`, entonces hay un elemento común `B` entre los conjuntos y el resultado es el conjunto único `{B,D,A}`. (Ten en cuenta que el orden de los elementos en un conjunto no importa: `{A,B,D}` es lo mismo que `{B,D,A}` y `{D,A,B}`, etc.).
