...
Sie müssen auch den Gesamtpreis des Warenkorbs aktualisieren, wenn der Benutzer einen Artikel hinzufügt. Erstellen Sie eine `calculateTotal`-Methode in der `ShoppingCart`-Klasse.

Deklarieren Sie in dieser Methode eine `subTotal`-Variable und verwenden Sie die `reduce`-Methode auf dem `items`-Array, um die Summe der `price`-Eigenschaft jedes Elements im Array zu berechnen. Verwenden Sie `total` und `item` als Parameter für Ihr Callback.

Denken Sie daran, Ihren Anfangswert in der `reduce`-Methode festzulegen.

# --hints--

Sie sollten eine `calculateTotal`-Methode in der `ShoppingCart`-Klasse erstellen.

```js
assert.isFunction(cart.calculateTotal);
```

Ihre `calculateTotal`-Methode sollte eine `subTotal`-Variable mit `const` deklarieren.
...
Ihre `calculateTotal`-Methode sollte die `reduce`-Methode auf dem `items`-Array verwenden.
...
Ihr `reduce`-Callback sollte `total` und `item` als ersten und zweiten Parameter verwenden. Denken Sie daran, die Syntax der Pfeilfunktion zu verwenden.
...
Ihr `reduce`-Callback sollte die Summe von `total` und `item.price` zurückgeben. Verwenden Sie eine implizite Rückgabe.
...
Ihr `reduce`-Aufruf sollte einen Anfangswert von `0` haben.
