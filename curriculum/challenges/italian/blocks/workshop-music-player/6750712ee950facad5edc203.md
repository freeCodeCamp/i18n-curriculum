...
Se provi a usare un po’ il tuo lettore noterai qualche bug. Per prima cosa, quando riproduci una canzone, la metti in pausa e poi premi il pulsante precedente o successivo, la canzone non riparte dall’inizio.

Per cominciare a risolvere questo problema, aggiungi un secondo parametro chiamato `start` alla tua funzione `playSong`. Dagli il valore predefinito `true`.

# --hints--

La tua funzione `playSong` dovrebbe avere un secondo parametro chiamato `start` con valore predefinito `true`.
