...
للحصول على الأغنية السابقة، اطرح `1` من `currentSongIndex` في `userData?.songs` وقم بتعيينها إلى الثابت `previousSong`. بعد ذلك، استدعِ الدالة `playSong` ومرر `previousSong.id` كمعلمة.
...
يجب تعيين `userData?.songs[currentSongIndex - 1]` إلى الثابت `previousSong`.
...
يجب استدعاء الدالة `playSong` مع `previousSong.id`.
