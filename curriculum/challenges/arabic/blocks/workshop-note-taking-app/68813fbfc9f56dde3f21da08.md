...
عندما يُحرر المستخدم ملاحظة ويضغط على مفتاح التبويب أو ينقر خارج البطاقة، يجب عرض رسالة حالة تُعلم المستخدمين أن الملاحظة حُفظت بنجاح.

ابدأ بإنشاء عنصر `div` مع خاصية `id` مضبوطة على `"status"`.

# --hints--

يجب أن يكون لديك عنصر `div` ثاني.

```js
assert.lengthOf(document.querySelectorAll("div"), 2);
```

يجب أن يحتوي عنصر `div` الثاني على خاصية `id`.

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

يجب أن تحتوي خاصية `id` في عنصر `div` الثاني على القيمة `"status"`.
