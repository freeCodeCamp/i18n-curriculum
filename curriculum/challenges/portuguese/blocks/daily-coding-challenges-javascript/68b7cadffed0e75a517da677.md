...
`checkStrength("123456")` deve retornar `"weak"`.

```js
assert.equal(checkStrength("123456"), "weak");
```

`checkStrength("pass!!!")` deve retornar `"weak"`.

```js
assert.equal(checkStrength("pass!!!"), "weak");
```

`checkStrength("Qwerty")` deve retornar `"weak"`.

```js
assert.equal(checkStrength("Qwerty"), "weak");
```

`checkStrength("PASSWORD")` deve retornar `"weak"`.
...
`checkStrength("qwerty12345")` deve retornar `"medium"`.
