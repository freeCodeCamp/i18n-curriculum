...
Fügen Sie einen Event-Listener für das `click`-Ereignis zu `scaryStoryBtn` hinzu und übergeben Sie ihm die `displayStory`-Funktion. Klicken Sie dann auf Ihren Button und sehen Sie sich die Ausgabe in der Konsole an.

# --hints--

Sie sollten einen Event-Listener für das `click`-Ereignis zu `scaryStoryBtn` hinzufügen und ihm die `displayStory`-Funktion übergeben.
