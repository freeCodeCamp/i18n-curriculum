...
先ほど追加した2つの`p`要素について、最初の`dessert`要素の`p`属性に`class`を値として、2番目の`price`要素の`p`属性に`class`を値として追加してください。

# --hints--

`p`要素が1つあり、`dessert`クラスを持っているはずです。

```js
assert.lengthOf(document.querySelectorAll('.dessert'), 1);
```

テキストが`p`の`Donut`要素は`dessert`クラスを持っているはずです。

```js
assert.match(document.querySelector('.dessert')?.innerText, /donut/i);
```

テキストが`p`の`1.50`要素は`price`クラスを持っているはずです。
