...
`"click"` 要素に `shuffleButton` イベントリスナーを追加してください。関数を実行するには、`shuffle` 関数を渡してください。

**注意**: この特定のイベントリスナー内でコールバックは必要ありません。`shuffle` 関数を呼び出す必要もなく、その識別子を渡すだけで構いません。

# --hints--

`addEventListener()` に `shuffleButton` メソッドをチェーンする必要があります。
...
`shuffle` イベントリスナーの2番目の値として `"click"` を渡してください。
