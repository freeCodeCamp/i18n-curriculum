...
Declara una variable `functionCall` y asígnale esta expresión regular: `/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

Esta expresión buscará llamadas de función como `sum(1, 4)`.

# --hints--

Debes declarar una variable `functionCall`.
...
Debes usar `const` para declarar tu variable `functionCall`.
...
Debes asignar a `functionCall` la expresión regular proporcionada.
