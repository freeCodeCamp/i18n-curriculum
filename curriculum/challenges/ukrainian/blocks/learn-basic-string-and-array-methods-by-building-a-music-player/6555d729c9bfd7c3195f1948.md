...
`textContent` встановлює текст вузла і дозволяє вам встановлювати або отримувати текстовий вміст HTML-елемента.
...
Ви можете використати тернарний оператор, щоб умовно встановити значення текстового вмісту. Ось приклад призначення результату тернарного оператора змінній:

```js
const example = condition ? "I'm true" : "I'm false";
```

Використайте тернарний оператор, щоб перевірити, чи `currentTitle` оцінюється як правдиве значення. Якщо так, призначте `playingSong.textContent` значення `currentTitle`. Інакше призначте порожній рядок.

Потім нижче використайте оператор `ternary`, щоб перевірити, чи `currentArtist` є правдивим значенням. Якщо так, призначте `songArtist.textContent` значення `currentArtist`. Інакше призначте порожній рядок.

# --hints--

Ви повинні ланцюжком викликати властивість `textContent` у `playingSong`.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

Ви повинні використати `ternary` оператор, щоб встановити властивість `textContent` у `playingSong` значенням `currentTitle` або `""`.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

Ви повинні ланцюжком викликати властивість `textContent` у `songArtist`.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

Ви повинні використати `ternary` оператор, щоб встановити властивість `textContent` у `songArtist` значенням `currentArtist` або `""`.
