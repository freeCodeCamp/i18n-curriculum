...
`conversionTable`에 `cup` 속성을 추가하고, 그 속성에 `gram` 속성이 `240`으로 설정된 객체, `ounce` 속성이 `8.0`로 설정된 객체, 그리고 `teaspoon` 속성이 `48`로 설정된 객체를 할당하세요.

# --hints--

`conversionTable`에는 `cup` 속성이 있어야 합니다.
