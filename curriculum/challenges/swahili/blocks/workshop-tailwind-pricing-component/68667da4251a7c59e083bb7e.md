...
Kwa vipengele vya `span` ndani ya kila kipengee cha orodha, weka darasa la vitu `text-green-500` na `mr-2` ili kufanya alama ziwe za kijani na kuzisukuma kidogo mbali na maandishi.

Pia, weka sifa ya kipengele `aria-hidden` kama ulivyofanya kwa vipengele vya `span` vya mpango wa kwanza wa bei.

# --hints--

Vipengele vyako vya `span` vinapaswa kuwa na darasa la vitu `text-green-500`.
...
Vipengele vyako vya `span` vinapaswa kuwa na darasa la vitu `mr-2`.
...
Vipengele vyako vya `span` vinapaswa kuwa na sifa ya kipengele `aria-hidden`.
