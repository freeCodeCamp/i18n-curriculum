...
Ahora que has creado el `resetButton`, necesitas asignarle atributos `id` y `aria-label`. JavaScript proporciona las propiedades `id` y `ariaLabel` que necesitas usar para esto.

Por ejemplo, `element.id` establecería un atributo de `id`, y `element.ariaLabel` establecería un atributo de `aria-label`. Ambos aceptan sus valores como una cadena de texto.

Establece el atributo `id` del elemento `resetButton` a `"reset"` y su atributo `aria-label` a `"Reset playlist"`.

# --hints--

No debes modificar la declaración existente `if` y su contenido.
...
Deberías usar `resetButton.id` para crear un atributo `id` llamado `"reset"` para el `resetButton`.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

Deberías usar `resetButton.ariaLabel` para crear un atributo `"aria-label"` llamado `"Reset playlist"` para el `resetButton`.
