...
Dentro de la devolución de llamada `useEffect`, agrega una condición `if` para verificar si `query.trim() === ''`. Esto verifica si `query` es una cadena vacía o contiene solo espacios. Si es así, llama a `setResults([])` y devuelve temprano.

# --hints--

Debes verificar si `query.trim() === ''`.
...
Si `query.trim() === ''` es verdadero, llama a `setResults()` con un arreglo vacío, luego usa la palabra clave `return` después de eso.
