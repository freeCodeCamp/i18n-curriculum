...
يجب عليك إعطاء عنصر `.chin` خاصية الـ `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

بداخل عنصر `.chin` يجب أن تكون خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

يجب عليك إعطاء عنصر `.chin` خاصية الـ `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

بداخل عنصر `.chin` يجب أن تكون خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

يجب أن تعطي `.chin` الـ `border-radius` بقيمة `70% 70% 100% 100%`.
