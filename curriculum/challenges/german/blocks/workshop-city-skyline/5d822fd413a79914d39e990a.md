...
Verschieben Sie die Eigenschaften und Werte `display`, `flex-direction` und `align-items` von `.bb1` in die neue Klasse `building-wrap`.

# --hints--

Sie sollten `display` aus `.bb1` entfernen.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.display);
```

Sie sollten `display` mit dem Wert `flex` nach `.building-wrap` verschieben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.display, "flex");
```

Sie sollten `flex-direction` aus `.bb1` entfernen.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.flexDirection);
```

Sie sollten `flex-direction` mit dem Wert `column` nach `.building-wrap` verschieben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.flexDirection, "column");
```

Sie sollten `align-items` aus `.bb1` entfernen.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.alignItems);
```

Sie sollten `align-items` mit dem Wert `center` nach `.building-wrap` verschieben.
