...
Видаліть поле з вашого елемента body, встановіть `font-family` у `sans-serif`, а також задайте йому властивість `background-color` зі значенням `#f5f6f7`.
...
Ваш селектор `body` має мати властивість `margin` зі значенням 0.
...
Ваш селектор `body` має мати властивість `font-family` зі значенням `sans-serif`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily, 'sans-serif');
```

Ваш селектор `body` має мати властивість `background-color` зі значенням `#f5f6f7`.
