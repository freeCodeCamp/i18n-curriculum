...
En algunos casos, las cartas deben estar al revés, ya que no siempre están en la posición correcta.

Declara una variable llamada `isReversed` dentro del método `singleCardSelected`. Usa el método `Math.random()` para generar un valor booleano aleatorio. Para asegurarte de que el valor se mantenga dentro de los límites, compáralo con `0.5`.
