...
Пройдіться циклом по `playlistSongElements` за допомогою методу `forEach`.

Метод `forEach` використовується, щоб пройтися циклом по масиву і виконати функцію для кожного елемента масиву. Наприклад, припустімо, у вас є масив чисел, і ви хочете вивести кожне число в консоль.
...
Використайте метод `forEach` на `playlistSongElements`. Передайте `songEl` як параметр і використайте стрілковий синтаксис, щоб додати порожню функцію зворотного виклику.

# --hints--

Ви повинні приєднати `forEach()` до `playlistSongElements`.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

Ваш метод `forEach()` має мати параметр `songEl` і використовувати стрілковий синтаксис.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

Ваш `forEach` має містити порожню пару фігурних дужок.
