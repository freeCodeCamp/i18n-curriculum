...
ペンギンの足をもっとペンギンらしく見せるために、左足を`80deg`回転させ、右足を`-80deg`回転させてください。

# --hints--

`.foot.left`に`transform`の`rotate(80deg)`を設定してください。
...
`.foot.right`に`transform`の`rotate(-80deg)`を設定してください。
