...
기억하세요, `class` 속성은 여러 값을 가질 수 있습니다. 흰색과 검은색 건반을 구분하려면 두 번째, 세 번째, 다섯 번째, 여섯 번째, 일곱 번째 `class` 요소에 `black--key`의 두 번째 `.key` 값을 추가하세요.

# --hints--

두 번째 `.key` 요소에도 `class`의 `black--key`가 있어야 합니다.
...
세 번째 `.key`에는 `black--key`의 `class`가 있어야 합니다.
...
다섯 번째 `.key`에는 `black--key`의 `class`가 있어야 합니다.
...
여섯 번째 `.key`에는 `black--key`의 `class`가 있어야 합니다.
...
일곱 번째 `.key`에는 `black--key`의 `class`가 있어야 합니다.
