...
编写一个 `elif` 语句来检查是否 `key > node.key`。

在 `elif` 块内，使用当前节点的右子节点和 `key` 作为参数调用 `_delete` 方法，并将结果分配给右节点。

# --hints--

你应该编写一个 `elif` 语句来检查是否 `key > node.key`。
...
你应该使用 `node.right` 和 `key` 作为参数来调用 `_delete` 方法。

```js
assert.match(code, /self\._delete\(\s*node\.right\s*,\s*key\s*\)/);
```

你应该将 `_delete()` 调用的结果分配给 `node.right`。
