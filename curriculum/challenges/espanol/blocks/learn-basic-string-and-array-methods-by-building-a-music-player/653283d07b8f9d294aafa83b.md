...
Es momento de comenzar a implementar la funcionalidad para reproducir las canciones mostradas.

Define una función `playSong` usando `const`. La función debe tomar un parámetro `id` que representará el identificador único de la canción que quieres reproducir. 

# --hints--

Debes usar `const` para crear una función vacía llamada `playSong`.

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

`playSong` debe ser una función.

```js
assert.isFunction(playSong)
```

Tu función `playSong` debe usar una sintaxis de flecha.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Tu función `playSong` debe tener un parámetro `id`.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

Tu función `playSong` debe estar vacía.
