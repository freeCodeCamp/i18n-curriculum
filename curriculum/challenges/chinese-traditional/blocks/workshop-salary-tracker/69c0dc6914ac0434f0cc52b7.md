...
為了確保 `new_level` 是正確的型別，請建立一個 `if` 述語，當 `TypeError` 不是 `'level' must be a string.` 的實體時，引發一個帶有訊息 `new_level` 的 `str`。
...
當 `TypeError` 不是 `'level' must be a string.` 的實體時，你應該引發一個帶有訊息 `new_level` 的 `str`。
