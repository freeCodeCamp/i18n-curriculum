...
Prendi l'elemento `select` con id `species` dal DOM e salvalo in una costante chiamata `selectElement`. Usa l'operatore di asserzione non-null `!` per affermare che l'elemento non è `null`

# --hints--

Dovresti avere una costante chiamata `selectElement`.
...
Il tuo `selectElement` dovrebbe cercare l'elemento con l'id `species` usando `document.querySelector` con un argomento di tipo `HTMLSelectElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

Il tuo `selectElement` dovrebbe usare l'operatore di asserzione non-null (`!`).
