...
使用 `.getElementById()` 方法取得 `#date-options` 元素，並使用 `const` 將它指定給名為 `dateOptionsSelectElement` 的變數。

# --hints--

你應該使用 `const` 來宣告一個 `dateOptionsSelectElement` 變數。

```js
assert.match(code, /const\s+dateOptionsSelectElement\s*=/);
```

你應該使用 `document.getElementById()` 來取得 `#date-options` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)date-options\1\s*\)/);
```

你應該將 `#date-options` 元素指定給你的 `dateOptionsSelectElement` 變數。
