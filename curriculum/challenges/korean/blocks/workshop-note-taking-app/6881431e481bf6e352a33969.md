...
앞서 언급했듯이, 사용자가 노트를 편집한 후 노트 밖을 클릭하면 노트가 저장되었다는 메시지가 표시되어야 합니다.

이를 위해 `statusEl.textContent`를 문자열 `"Note saved successfully!"`로 설정하세요.

이제 테스트하면 화면에 해당 메시지가 표시되는 것을 볼 수 있습니다.

# --hints--

사용자가 노트를 편집하면 상태 메시지가 노트가 저장되었음을 나타내도록 업데이트되어야 합니다.
