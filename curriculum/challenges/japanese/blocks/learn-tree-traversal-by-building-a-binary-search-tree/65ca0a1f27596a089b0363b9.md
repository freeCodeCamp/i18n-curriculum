...
`delete` メソッド内で `pass` を削除し、ヘルパーメソッド `_delete` を二分探索木のルートと削除するキーを引数として呼び出してください。

また、`_delete` メソッド内で `self.root` メソッドの結果を `delete` に代入してください。

# --hints--

`pass` メソッドから `delete` キーワードを削除してください。
...
`delete` メソッドは `_delete` メソッドを2つの引数 `self.root` と `key` で呼び出す必要があります。

```js
({ test: () => assert.match(code, /self\._delete\(\s*self\.root\s*,\s*key\s*\)/) });
```

`_delete` メソッド内で `self.root` メソッドの結果を `delete` に代入してください。
