...
Додайте блок `else`, щоб обробляти поточний час відтворення пісні. Це дозволить відновити поточну пісню з того місця, де її було призупинено.

У межах блоку `else` встановіть властивість `currentTime` об’єкта `audio` у значення, збережене в `userData?.songCurrentTime`.
...
Ви повинні додати блок `else` до існуючого блоку `if`.
...
Ви повинні встановити `audio.currentTime` у `userData?.songCurrentTime` всередині блоку `else` вашої інструкції `if`.
