...
Fügen Sie im Konstruktor Aufrufe von `fetchCardsData()` und `initializeEventListeners()` mit dem Schlüsselwort `this` hinzu, damit sie ausgeführt werden, wenn die Klasse initialisiert wird.

Die `fetchCardsData`-Methode wird das Laden der Tarotkarten-Daten von einer API übernehmen, und `initializeEventListeners` richtet die Button-Klick-Handler ein. Sie erstellen beide Methoden in den kommenden Schritten.

# --hints--

Sie sollten die `fetchCardsData()`-Methode innerhalb des `constructor` aufrufen.
...
Sie sollten die `initializeEventListeners()`-Methode im Konstruktor aufrufen.
