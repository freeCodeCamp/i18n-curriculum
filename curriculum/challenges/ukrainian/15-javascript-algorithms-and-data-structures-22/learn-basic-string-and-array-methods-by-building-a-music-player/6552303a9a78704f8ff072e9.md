...
Додайте слухач подій `"click"` до елемента `pauseButton`, а потім передайте `pauseSong` як другий аргумент слухача подій. Це функція, яку запускатиме слухач подій.

Перевірте свій застосунок, натиснувши спочатку кнопку відтворення, а потім кнопку паузи. Ви повинні побачити, що все працює, як очікувалося.

# --hints--

Викличте метод `addEventListener()` на змінній `pauseButton`.
...
Передайте `pauseSong` як другий аргумент методу `addEventListener`.
