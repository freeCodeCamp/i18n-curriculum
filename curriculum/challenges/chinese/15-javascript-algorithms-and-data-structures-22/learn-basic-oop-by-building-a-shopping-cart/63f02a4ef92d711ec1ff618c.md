...
你还需要在用户添加商品时更新购物车的总价。 在 `ShoppingCart` 类中创建一个 `calculateTotal` 方法。

在该方法中，声明一个 `subTotal` 变量，并在 `items` 数组上使用 `reduce` 方法计算数组中每个项目的 `price` 属性的总和。 使用 `total` 和 `item` 作为回调的参数。

请记住在 `reduce` 方法中设置初始值。

# --hints--

你应该在 `ShoppingCart` 类中创建一个 `calculateTotal` 方法。

```js
assert.isFunction(cart.calculateTotal);
```

你的 `calculateTotal` 方法应该有一个用 `const` 声明的 `subTotal` 变量。
...
你的 `calculateTotal` 方法应该在 `items` 数组上使用 `reduce` 方法。
...
你的 `reduce` 回调应使用 `total` 和 `item` 作为第一个和第二个参数。 记得使用箭头函数语法。
...
你的 `reduce` 回调应返回 `total` 和 `item.price` 的总和。 使用隐式返回。
