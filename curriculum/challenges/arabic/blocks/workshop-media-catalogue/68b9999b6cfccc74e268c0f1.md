...
قبل حلقة `for`، أنشئ جملة `if` تضيف السلسلة النصية `=== MOVIES ===` (تنتهي بحرف سطر جديد) إلى القيمة الحالية لـ `result` عندما لا تكون قائمة `movies` فارغة.

# --hints--

يجب أن تحتوي طريقة `__str__` على جملة `if` تتحقق مما إذا كانت قائمة `movies` غير فارغة.
...
يجب أن تحتوي جملة `if` على `result += '=== MOVIES ===\n'`.
