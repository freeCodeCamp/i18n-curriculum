...
입력 필드를 지우는 함수를 작성하세요.

`clearInputFields` 함수를 만드세요. 그 안에서 `querySelectorAll`을 사용해 모든 입력 필드를 가져오고 `forEach()`로 반복하세요. `forEach()` 안에서 각 `input`의 값을 빈 문자열로 설정하세요.

# --hints--

`clearInputFields` 함수를 만드세요.

```js
assert.isFunction(clearInputFields)
```

`clearInputFields` 함수는 각 입력 필드의 값을 빈 문자열로 설정해야 합니다.
