...
Зараз порядок пісень не змінився. Це тому, що оновлення, які ви зробили використавши метод `sort`, не відбудуться, доки не буде викликана функція `sortSongs`.

Змініть функцію `renderSongs`, щоб викликати функцію `sortSongs`.

Тепер ви повинні побачити пісні в алфавітному порядку.

# --hints--

Ви повинні мати `renderSongs(sortSongs())`.
