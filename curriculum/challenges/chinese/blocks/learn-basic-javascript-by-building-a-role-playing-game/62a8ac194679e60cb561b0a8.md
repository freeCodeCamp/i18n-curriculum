...
为空的 `button text` 数组提供三个字符串元素。 在 `goTown` 函数中使用分配给按钮 `innerText` 属性的三个字符串。 请记住，数组值以逗号分隔。

# --hints--

你的 `button text` 数组应该包含三个元素。

```js
assert.lengthOf(locations[0]["button text"], 3);
```

你的 `button text` 数组应该有三个字符串。
...
`button text` 数组中的第一个值应该是 `"Go to store"`。

```js
assert.equal(locations[0]["button text"][0], "Go to store");
```

`button text` 数组中的第二个值应该是 `"Go to cave"`。

```js
assert.equal(locations[0]["button text"][1], "Go to cave");
```

`button text` 数组中的第三个值应该是 `"Fight dragon"`。
