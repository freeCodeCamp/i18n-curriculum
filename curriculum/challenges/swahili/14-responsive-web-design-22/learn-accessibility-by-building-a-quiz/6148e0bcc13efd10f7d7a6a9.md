...
Weka rangi ya mandharinyuma ya `footer` kuwa `#2a2a40`, na utumie _Flexbox_ ili kuweka maandishi katikati kwa mlalo (horizontally center).

# --hints--

Unapaswa kutumia kipengele kichaguzi cha `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

Unapaswa kuipa `footer` `background-color` ya `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

Unapaswa kuipa`footer` `display` ya `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Unapaswa kuipa`footer` `justify-content` ya `center`.
