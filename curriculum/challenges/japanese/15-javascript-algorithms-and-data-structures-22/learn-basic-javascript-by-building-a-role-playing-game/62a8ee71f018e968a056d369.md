...
コードの一番下に、関数 `defeatMonster` と `lose` を作成してください。 どちらも今は空のままにしておいてください。

# --hints--

`function` キーワードを使用して `defeatMonster` を宣言してください。

```js
assert.match(code, /function\s+defeatMonster/);
```

`defeatMonster` は空の関数にしてください。

```js
assert.match(defeatMonster.toString(), /defeatMonster\s*\(\s*\)\s*\{\s*\}/);
```

`function` キーワードを使用して `lose` を宣言してください。
