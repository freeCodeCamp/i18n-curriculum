...
定位 `.penguin-body` 元素，并将其 `width` 设置为 `53%`，`height` 设置为 `45%`。 然后，将 `background` 设置为渐变轴为 `45deg` 的线性渐变，起始点 `0%` 为 `rgb(134, 133, 133)`，过渡到中间 `25%` 位置的 `rgb(234, 231, 231)`，再过渡到 `67%` 位置的 `white`。

# --hints--

你应该使用 `.penguin-body` 选择器。

```js
assert.match(code, /\.penguin-body\s*\{/);
```

你应该将 `.penguin-body` 的 `width` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

你应该将 `.penguin-body` 的 `height` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

你应该将 `.penguin-body` 的 `background` 设置为 `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)`。
