...
現在你已經有了 `x_max` 和 `y_max`，你可以將它們用作列數和行數來開始建置圖形結構：創建一個列表的列表，其中外部列表包含 `y_max` +1 個列表，每個列表內含有 `x_max` +1 個元素，每個元素都是包含單一空格的字串。

將這個列表的列表存檔到名為 `matrix_list` 的變數中並傳回此值。
...
`create_trajectory` 方法應該為任何實體傳回正確的值。
