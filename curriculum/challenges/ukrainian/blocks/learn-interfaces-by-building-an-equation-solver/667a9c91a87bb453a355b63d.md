...
Не соромтеся змінювати коефіцієнти свого `lin_eq`, щоб побачити, як змінюються вихідні дані.

Потім видаліть свій виклик `print(solver(lin_eq))` і виведіть результат виклику `solver()` з `quadr_eq` як аргументом.

# --hints--

У вашому коді не повинно бути `print(solver(lin_eq))`.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

Ви повинні вивести `solver(quadr_eq)`.
