...
前のレッスンで学んだように、リスト内包表記は既存のイテラブルからリストを作成するために使えます。
...
`medications` リストの各アイテムは文字列である必要があります。このステップと次のステップでは、それを確認する式を書きます。`and` 演算子を使って、`medications` キーの値に別の式を追加してください。

`and` 演算子の右側には、リスト内包表記の構文を使って、`isinstance(i, str)` の各 `i` に対して `medications` を評価した結果からなるリストを作成してください。

# --hints--

`medications` 辞書の `constraints` キーには `isinstance(medications, list) and [isinstance(i, str) for i in medications]` の値を設定してください。
