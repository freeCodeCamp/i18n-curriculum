...
부트스트랩의 `col-xs-*` 클래스는 모든 `form` 요소에 적용되므로, 체크박스에도 사용할 수 있습니다! 이렇게 하면 화면 해상도가 얼마나 넓든 체크박스가 페이지 전체에 고르게 배치됩니다.

# --instructions--

세 개의 체크박스를 모두 `<div class="row">` 요소 안에 중첩하세요. 그런 다음 각각을 `<div class="col-xs-4">` 요소 안에 중첩하세요.

# --hints--

모든 체크박스는 `div` 클래스를 가진 하나의 `row` 안에 중첩되어야 합니다.
...
각 체크박스는 `div` 클래스를 가진 자신의 `col-xs-4` 안에 중첩되어야 합니다.
