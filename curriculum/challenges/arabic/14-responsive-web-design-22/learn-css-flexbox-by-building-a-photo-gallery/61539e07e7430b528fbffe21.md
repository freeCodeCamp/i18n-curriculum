...
ثم أعطيه `max-width` بقيمة `1400px` وأضف `margin` بقيمة `0 auto` لوضعها بالمنتصف.

# --hints--

يجب أن يحتوي منتقي `.gallery` على خاصية `padding` بقيمة `20px 10px`.
...
يجب أن يحتوي منتقي `.gallery` على خاصية `max-width` بقيمة `1400px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.gallery')?.maxWidth === '1400px');
```

يجب وضع عنصرك `.gallery` بالمنتصف بواسطة إضافة `margin` بقيمة `0 auto`.
