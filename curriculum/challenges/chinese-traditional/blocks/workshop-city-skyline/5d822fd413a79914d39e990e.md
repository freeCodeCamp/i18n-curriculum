...
為 `bb4-window` 類別設定 `width` 為 `18%`、`height` 為 `90%`，並將你的 `--window-color4` 變數添加為 `background-color`。

# --hints--

你應該給 `.bb4-window` 一個 `width` 為 `18%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

你應該給 `.bb4-window` 一個 `height` 為 `90%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

你應該為 `.bb4-window` 設定 `background-color` 為 `--window-color4`。
