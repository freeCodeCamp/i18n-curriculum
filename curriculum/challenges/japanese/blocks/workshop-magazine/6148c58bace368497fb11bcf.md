...
Flexboxと同様に、CSS Gridでも`align-items`と`justify-items`を使ってグリッドアイテムのコンテンツを揃えられます。`align-items`は子要素を列軸に沿って揃え、`justify-items`は子要素を行軸に沿って揃えます。

`.social-icons`セレクターに`align-items`プロパティを設定し、値を`center`にしてください。
