...
ابحث عن حلول للغز *المجموع إلى مئة*.

أضف (أدرج) العمليات الحسابية **+** أو **─** (زائد أو ناقص) قبل أي من الأرقام في السلسلة الرقمية العشرية **123456789** بحيث يكون التعبير الرياضي الناتج يساوي مجموعًا معينًا (في هذه الحالة الشهيرة، **100**).
...
اكتب دالة تأخذ رقمًا كمعلمة. يجب أن تُرجع الدالة مصفوفة تحتوي على كل الحلول للرقم المعطى. يجب أن تكون الحلول سلاسل نصية تمثل التعبيرات. على سبيل المثال: "1+23-456+78-9". رتب المصفوفة قبل إرجاعها.

# --hints--

`sumTo100` يجب أن تكون دالة.

```js
assert(typeof sumTo100 == 'function');
```

`sumTo100(199)` يجب أن تُرجع مصفوفة.

```js
assert(Array.isArray(sumTo100(199)));
```

`sumTo100(199)` يجب أن تُرجع `["-1+2-3+45+67+89", "123-4+5+6+78-9", "123-4+56+7+8+9"]`.
...
`sumTo100(209)` يجب أن تُرجع `["1+234+56+7-89"]`.

```js
assert.deepEqual(sumTo100(209), ['1+234+56+7-89']);
```

`sumTo100(243)` يجب أن تُرجع `["-1-234+567-89", "-12+345+6-7-89", "123+45+6+78-9"]`.
...
`sumTo100(197)` يجب أن تُرجع `["1-2-3+45+67+89", "12+34-5+67+89", "123+4-5+6+78-9"]`.
