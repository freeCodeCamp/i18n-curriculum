...
Chiama il metodo `.map()` sulla tua chiamata `rangeFromString()`, passando un riferimento a `addCharacters` come funzione di callback.

# --hints--

Dovresti chiamare il metodo `.map()` sulla tua chiamata `rangeFromString()`.
...
Dovresti passare un riferimento a `addCharacters` come callback al tuo metodo `.map()`.
