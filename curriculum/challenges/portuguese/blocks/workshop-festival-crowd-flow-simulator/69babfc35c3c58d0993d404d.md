...
Para completar sua função `processGateFlow`, retorne um objeto contendo duas propriedades:

- `processed`: o número de participantes que o portão conseguiu processar durante este tick.
- `overflow`: o número de participantes restantes na fila após o processamento (que é `currentTickQueue` depois do loop).

Você deve retornar diretamente o objeto assim:
...
Você deve retornar um objeto com as propriedades `processed` e `overflow`. Retorne diretamente o objeto sem atribuí-lo a uma variável.
...
A propriedade `processed` do objeto retornado deve ter o valor `processed`.
...
A propriedade `overflow` do objeto retornado deve ter o valor `currentTickQueue`.
