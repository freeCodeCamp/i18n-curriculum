...
أضف `repeating-linear-gradient` إلى `.fb1c` أسفل ذلك الموجود هناك؛ استخدم `--building-color4` الخاص بك من `0%` إلى `10%` و`--window-color4` من `10%` و`90%`. هذا سيملأ خلف معامل التدرج الذي أضفته حديثًا.

# --hints--

لا يجب عليك تغيير أول `repeating-linear-gradient`.
...
يجب عليك إضافة `repeating-linear-gradient` يحمل لون أولي `--building-color4` من `0%` إلى `10%`.
...
يجب عليك استخدام لون ثاني من `--window-color4` يبدأ من `10%` إلى `90%`.
