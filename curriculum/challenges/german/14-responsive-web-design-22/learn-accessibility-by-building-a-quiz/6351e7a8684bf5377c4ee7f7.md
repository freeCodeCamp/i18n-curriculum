...
Obwohl es nicht für `label`-Elemente mit einem eingebauten `input`-Element erforderlich ist, ist es nach wie vor die beste Vorgehensweise, ein `label` mit dem entsprechenden `input`-Element explizit zu verbinden.

Füge nun ein `for`-Attribut zu jedem deiner vier `label` hinzu, die das `label` mit ihrem entsprechenden Radio-`input` verbindet.

# --hints--

Du solltest dem ersten `label` ein `for`-Attribut geben.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

Du solltest dem ersten `label` ein `for`-Attribut geben, das der `id` seines `input`-Elements entspricht.
...
Du solltest dem zweiten `label` ein `for`-Attribut geben.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

Du solltest dem zweiten `label` ein `for`-Attribut geben, das der `id` seines `input`-Elements entspricht.
...
Du solltest dem dritten `label` ein `for`-Attribut geben, das der `id` seines `input`-Elements entspricht.
...
Du solltest dem vierten `label` ein `for`-Attribut geben.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

Du solltest dem vierten `label` ein `for`-Attribut geben, das der `id` seines `input`-Elements entspricht.
