...
使用 `.getElementById()` 方法获取 `#date-options` 元素，并使用 `const` 将其赋值给名为 `dateOptionsSelectElement` 的变量。

# --hints--

你应该使用 `const` 来声明一个 `dateOptionsSelectElement` 变量。

```js
assert.match(code, /const\s+dateOptionsSelectElement\s*=/);
```

你应该使用 `document.getElementById()` 来获取 `#date-options` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)date-options\1\s*\)/);
```

你应将 `#date-options` 元素分配给你的 `dateOptionsSelectElement` 变量。
