...
`+=` 演算子を使って、`" Your <weapon> breaks."` の前にスペースを入れた `Your` を `text.innerText` の末尾に追加してください。`<weapon>` は `inventory` を使って `inventory.pop()` 配列の最後の要素に置き換えます。これにより配列の最後の要素が削除され、戻り値として取得されて文字列に表示されます。

# --hints--

`text.innerText` を変更するには複合代入演算子を使うべきです。
