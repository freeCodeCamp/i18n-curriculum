...
在 `getElement` 函式（程式）內，使用帶有泛型的 `querySelector` 並將結果存入一個變數。新增一個 `if` 述語來檢查該元素是否不存在，如果不存在，則丟出一個帶有訊息 `Element not found: ${error message here}` 的錯誤。如果元素存在，則傳回它。

# --hints--

你應該在 `el` 函式（程式）內宣告一個名為 `getElement` 的變數。
...
你的 `el` 變數應該被指定（設值）為 `document.querySelector<T>(selector)`。
