...
La aplicación de la adivina soporta renderizar múltiples cartas, así que necesitas tener un método que pueda ayudar a renderizar varias cartas. Declara un método llamado `multipleCardSelected`.

Dentro del método, llama a la función `hideElements` y pasa las propiedades `singleCard`, `singleCardBtn`, `multipleCardsBtn` y `headerTitle`. No olvides que tienes que acceder a ellas desde `this.elements`.

# --hints--

Debes declarar un método `multipleCardSelected`.
...
Debes llamar a la función `hideElements` dentro del método `multipleCardSelected`.
...
Debes pasar `singleCard` como primer argumento a la función `hideElements`.
...
Debes pasar `singleCardBtn` como argumento a la función `hideElements`.
...
Debes pasar `multipleCardsBtn` como argumento a la función `hideElements`.
...
Debes pasar `headerTitle` como argumento a la función `hideElements`.
