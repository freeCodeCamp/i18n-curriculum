...
在这个项目中，你将创建一个数字排序器。 HTML 和 CSS 已为你提供。 请随意探索它们。

当你准备好时，声明一个 `sortButton` 变量并分配给它带有参数 `"sort"` 的 `.getElementById()` 的值。

# --hints--

你应该用 `const` 声明一个 `sortButton` 变量。

```js
assert.match(code, /const\s+sortButton\s*=/);
```

你应该调用 `document.getElementById()` 并使用 `"sort"` 作为参数。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)sort\1\s*\)/);
```

你应该将 `document.getElementById()` 的值分配给 `sortButton`。
