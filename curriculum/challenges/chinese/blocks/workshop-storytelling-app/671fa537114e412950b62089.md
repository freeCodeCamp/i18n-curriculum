...
在你的 JavaScript 文件中，选择 `.story-container` 元素并将其存储在名为 `storyContainer` 的变量中。

# --hints--

你应该有一个 `storyContainer` 变量。

```js
assert.isDefined(storyContainer);
```

你应该使用 `querySelector` 方法选择 `.story-container` 元素并将其存储在 `storyContainer` 变量中。
