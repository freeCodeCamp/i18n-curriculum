...
يُحدد تنسيق الخط لعناصر العنوان (مثل `h1`، `h2`) بقيم افتراضية من متصفحات المستخدمين.

أضف محددي نوع جديدين (`h1` و `h2`). استخدم خاصية `font-size` لكليهما، لكن استخدم القيمة `40px` لـ `h1` و `30px` لـ `h2`.
...
يجب أن يكون لعنصر `h1` خاصية `font-size` بقيمة `40px`.
...
يجب أن يكون لعنصر `h2` خاصية `font-size` بقيمة `30px`.
