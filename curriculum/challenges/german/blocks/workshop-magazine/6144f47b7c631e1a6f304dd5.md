...
Ihr Magazin wird drei Hauptabschnitte haben. Sie haben bereits das Gesamtlayout in der `main`-Regel festgelegt, aber Sie können die Platzierung in den untergeordneten Regeln anpassen.

Eine Möglichkeit ist die `grid-column`-Eigenschaft, die eine Kurzschreibweise für `grid-column-start` und `grid-column-end` ist. Die `grid-column`-Eigenschaft gibt dem Grid-Element an, an welcher Gitterlinie es beginnen und enden soll.

Erstellen Sie eine `.heading`-Regel und setzen Sie die `grid-column`-Eigenschaft auf `2 / 3`. Dadurch wird dem `.heading`-Element gesagt, dass es an Gitterlinie 2 beginnen und an Gitterlinie 3 enden soll.

# --hints--

Sie sollten einen `.heading`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.heading'));
```

Ihr `.heading`-Selektor sollte eine `grid-column`-Eigenschaft mit dem Wert `2 / 3` haben.
