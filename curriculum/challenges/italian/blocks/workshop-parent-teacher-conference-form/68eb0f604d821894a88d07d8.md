...
Per far risaltare il contenitore, dovresti aggiungere un insieme `box-shadow` impostato su `0 5px 15px black`.

Questo aggiunge un effetto ombra intorno al contenitore. I valori controllano rispettivamente l'offset orizzontale, l'offset verticale, il raggio di sfocatura e il colore.

# --hints--

Dovresti mirare alla classe `.container`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.container'));
```

Dovresti impostare la proprietà `box-shadow` su `0 5px 15px black`.
