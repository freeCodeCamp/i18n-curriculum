...
Um die Wiedergabeliste auf ihren ursprünglichen Zustand zurückzusetzen, verbreiten Sie `allSongs` in ein Array und weisen Sie es `userData.songs` zu.

*Hinweis*: Sie sollten keine optionale Verkettung für `userData.songs` verwenden, da der Song zu diesem Zeitpunkt weder `null` noch `undefined` sein wird.

# --hints--

Sie sollten den vorhandenen Ereignis-Listener und dessen Inhalt nicht ändern.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*\)\s*;?/)
```

Sie sollten `[...allSongs]` `userData.songs` zuweisen.
