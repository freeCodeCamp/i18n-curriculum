...
Füge innerhalb deiner verschachtelten `for`-Schleife einen `console.log()`-Aufruf hinzu, um die Werte `array`, `array[j]` und `array[minIndex]` bei jeder Iteration zu überprüfen. Du kannst auf den `Sort`-Button klicken, um zu sehen, wie dein Algorithmus das Array durchläuft.

Schreibe anschließend eine `if`-Anweisung, die prüft, ob der Wert bei `array[j]` kleiner ist als der Wert bei `array[minIndex]`. Ist dies der Fall, dann setze `minIndex` auf `j`.

# --hints--

Du solltest einen `console.log()`-Aufruf innerhalb deiner verschachtelten `for`-Schleife haben.
...
Du solltest `array` als erstes Argument an `console.log()` übergeben.
...
Du solltest `array[j]` als zweites Argument an `console.log()` übergeben.
...
Du solltest `array[minIndex]` als drittes Argument an `console.log()` übergeben.
...
Nach deiner `if`-Anweisung sollte ein `console.log()`-Aufruf stehen.
...
Du solltest eine Bedingung in deiner `if`-Anweisung haben, die prüft, ob `array[j]` kleiner ist als `array[minIndex]`.
...
Deine `if`-Anweisung sollte `minIndex` auf `j` setzen.
