...
`.image-wrapper` 要素内の2番目の `img` 要素に、`src` の `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`、`alt` の `image of a calculator project`、`loading` に設定された `lazy` 属性、`class` に設定された `image-2`、`width` に設定された `400` 属性、そして `height` に設定された `400` 属性を付与してください。

# --hints--

2番目の `img` 要素には `src` に設定された `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png` が必要です。
...
2番目の `img` 要素には `alt` に設定された `image of a calculator project` が必要です。
...
2番目の `img` 要素には `loading` に設定された `lazy` 属性が必要です。
...
2番目の `img` 要素には `class` に設定された `image-2` が必要です。
...
2番目の `img` 要素には `width` に設定された `400` が必要です。
...
2番目の `img` 要素には `height` に設定された `400` が必要です。
