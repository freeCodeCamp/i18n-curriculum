...
هل لاحظت أنه يمكنك إرسال النموذج دون ملء المُدخَلات؟

لمنع ذلك، أضف الخاصية `disabled` لتعطيل زر الإرسال إذا كانت أي من `heroName` أو `realName` أو `powerSource` قيمتها خاطئة، أو إذا كان طول `powers` يساوي `0`.

الآن، سيُعطّل الزر إذا لم يُملأ شيء ما. املأ النموذج بمعلومات البطل الخارق الخاص بك وأرسله للانضمام إلى الرابطة.

بهذا، يصبح نموذج طلب البطل الخارق الخاص بك مكتملًا.

# --hints--

لا يجب تعديل المحتوى الحالي للزر.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2/)
```

يجب أن يحتوي عنصر `button` على خاصية `disabled`.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

يجب تعطيل `button` إذا كانت أي من `heroName` أو `realName` أو `powerSource` خاطئة، أو إذا كان طول `powers` يساوي `0`.
