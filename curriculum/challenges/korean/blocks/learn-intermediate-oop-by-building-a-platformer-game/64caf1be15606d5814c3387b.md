...
플레이어의 `y` 위치에 플레이어의 높이를 더한 값이 플랫폼의 `y` 위치보다 크거나 같은지 확인하는 또 다른 불리언 표현식을 추가하세요.

그 아래에, 플레이어의 `y` 위치가 플랫폼의 `y` 위치에 플랫폼 높이를 더한 값보다 작거나 같은지 확인하는 또 다른 불리언 표현식을 추가하세요.

# --hints--

기존 `platformDetectionRules` 배열과 그 내용을 변경하지 마세요.
...
플레이어의 `y` 위치에 플레이어의 높이를 더한 값이 플랫폼의 `y` 위치보다 크거나 같은지 확인하는 불리언 표현식이 있어야 합니다.
...
플레이어의 `y` 위치가 플랫폼의 `y` 위치에 플랫폼 높이를 더한 값보다 작거나 같은지 확인하는 불리언 표현식이 있어야 합니다.
