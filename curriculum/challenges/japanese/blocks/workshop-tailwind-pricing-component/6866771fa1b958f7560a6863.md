...
これでバッジのテキストは動きますが、まだ完全に思い通りの場所ではありません。魔法は `right-[num]`、`left-[num]`、`top-[num]`、そして `bottom-[num]` のユーティリティクラスにあります。
...
また、マイナスの値はハイフンを接頭辞として付けることで示せます：
...
`-top-3` バッジ `right-3` に `Most popular` と `div` のクラスを追加してください。

# --hints--

テキスト `div` を持つ `Most popular` 要素には `-top-3` クラスを付けてください。
...
テキスト `div` を持つ `Most popular` 要素には `right-3` クラスを付けてください。
