...
新しいクラスにはまだスタイルがありません。`.bold` セレクターを作成し、テキストを太字にするために `font-weight` プロパティを `800` に設定してください。

また、`font-weight` セレクターから `h1` プロパティを削除してください。
...
`.bold` セレクターには `font-weight` プロパティを `800` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.bold')?.fontWeight === '800');
```

`h1` セレクターには `font-weight` プロパティがあってはいけません。
