...
Crea una sottoclasse `Spider` che estende la classe `Bug` con il parametro di tipo generico impostato su `string`.

# --hints--

Dovresti avere una classe chiamata `Spider`.
...
La tua classe `Spider` dovrebbe estendere `Bug`.
...
La tua classe `Spider` dovrebbe estendere `Bug` con un argomento di tipo `string`.
