...
Nota: O método `fetch()` usa `GET` como o método `HTTP` padrão. Isso significa que você não precisa especificá-lo explicitamente para a recuperação básica de dados.
