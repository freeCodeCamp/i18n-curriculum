...
另一個常見使用 `e.preventDefault()` 方法的例子是與表單提交有關。預設情況下，提交表單會將資料送到伺服器並重新載入頁面。使用 `e.preventDefault()` 可以防止這種情況發生。
...
當你需要更細緻地控制使用者如何與頁面互動時，防止預設行為非常有用，但同時也要注意無障礙性：你的訂製行為應該提供與預設相同的功能。
