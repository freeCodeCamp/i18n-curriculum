...
一致があれば `userData.currentSong` に `null` を、`userData.songCurrentTime` に `0` を設定してください。

その後、再生を停止するために `pauseSong()` 関数を呼び出し、プレーヤーの表示を更新するために `setPlayerDisplay()` 関数を呼び出してください。
