...
CSS Grid 的默认设置将根据需要创建额外的行，这与 Flexbox 不同。 给 `.social-icons` 选择器一个 `grid-template-columns` 属性，设置为 `repeat(5, 1fr)`，将图标排列成一行。

# --hints--

你的 `.social-icons` 选择器应该有一个 `grid-template-columns` 属性，设置为 `repeat(5, 1fr)`。
