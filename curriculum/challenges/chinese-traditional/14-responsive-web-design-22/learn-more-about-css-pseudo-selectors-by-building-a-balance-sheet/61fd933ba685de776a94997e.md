...
在其下方添加三個 `td` 元素，並按順序爲它們提供以下文本：`$500`、`$250` 和 `$0`。 給第三個 `td` 元素一個 `class` 設置爲 `current`。
...
`th` 元素應包含文本 `Loans The outstanding balance on our startup loan.`。
...
應該將文本 `The outstanding balance on our startup loan.` 包含在 `span` 元素中。
