...
如果 `numberInput` 的 `value` 等於 `5`，則調用 `showAnimation()` 函數，然後提前 `return`。

# --hints--

你應該在第二個 `if` 語句的主體內調用 `showAnimation()` 函數。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

調用 `showAnimation()` 函數後，應使用 `return` 關鍵字提早退出 `checkUserInput()` 函數。
