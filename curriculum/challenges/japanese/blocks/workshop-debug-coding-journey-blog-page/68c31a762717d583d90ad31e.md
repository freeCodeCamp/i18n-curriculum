...
ブログページには、合計で3つのブログ投稿が一般的な`div`要素で囲まれています。しかし、それぞれの投稿を囲むのに意味のあるセマンティック要素を使うほうが良いです。

各ブログ投稿を囲んでいる`div`要素を`article`要素に変更してください。

# --hints--

最初の投稿のための最初の`div`要素は`article`要素に変更してください。
...
2番目の投稿のための2番目の`div`要素は`article`要素に変更してください。
...
3番目の投稿のための3番目の`div`要素は`article`要素に変更してください。
