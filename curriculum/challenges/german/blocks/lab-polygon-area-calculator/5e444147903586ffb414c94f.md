...
title: Einen Flächenrechner für Polygone erstellen
challengeType: 27
...
1. Sie sollten eine `Rectangle`-Klasse erstellen.

2. Wenn ein `Rectangle`-Objekt erstellt wird, sollte es mit den Attributen `width` und `height` initialisiert werden. Die Klasse sollte außerdem die folgenden Methoden enthalten:

    - `set_width`: Legt die Breite des Rechtecks fest.
    - `set_height`: Legt die Höhe des Rechtecks fest.
    - `get_area`: Gibt die Fläche zurück (\\( \text{width} \times \text{height} \\)).
    - `get_perimeter`: Gibt den Umfang zurück \\( 2(\text{width} + \text{height}) \\).
    - `get_diagonal`: Gibt die Diagonale zurück \\( \sqrt{\text{width}^2 + \text{height}^2} \\).
    - `get_picture`: Gibt einen String zurück, der die Form mit Linien aus `*` darstellt. Die Anzahl der Linien sollte der Höhe entsprechen, und die Anzahl der `*` in jeder Linie sollte der Breite entsprechen. Am Ende jeder Linie sollte ein Zeilenumbruch (`\n`) stehen. Wenn die Breite oder Höhe größer als `50` ist, sollte dieser String zurückgegeben werden: `Too big for picture.`.
    - `get_amount_inside`: Nimmt eine andere Form (Quadrat oder Rechteck) als Argument. Gibt zurück, wie oft die übergebene Form ohne Drehungen in die Form passt. Zum Beispiel könnte ein Rechteck mit einer Breite von `4` und einer Höhe von `8` zwei Quadrate mit Seitenlänge `4` aufnehmen.

3. Wenn eine Instanz einer `Rectangle` als String dargestellt wird, sollte sie so aussehen: `Rectangle(width=5, height=10)`.

4. Sie sollten eine `Square`-Klasse erstellen, die eine Unterklasse von `Rectangle` ist.
5. Wenn ein `Square`-Objekt erstellt wird, sollte es mit einer einzelnen Seitenlänge initialisiert werden. Die `__init__`-Methode sollte die Seitenlänge sowohl im `width`- als auch im `height`-Attribut der `Rectangle`-Klasse speichern.
6. Die `Square`-Klasse sollte die folgenden Methoden enthalten:

    - `set_width`: Überschreibt die `set_width`-Methode aus der `Rectangle`-Klasse. Sie sollte die Breite und Höhe auf die Seitenlänge setzen.
    - `set_height`: Überschreibt die `set_height`-Methode aus der `Rectangle`-Klasse. Sie sollte die Breite und Höhe auf die Seitenlänge setzen.
    - `set_side`: Legt die Höhe und Breite des Quadrats auf die Seitenlänge fest.

7. Die `Square`-Klasse sollte auf die Methoden der `Rectangle`-Klasse zugreifen können.
8. Wenn eine Instanz einer `Square` als String dargestellt wird, sollte sie so aussehen: `Square(side=9)`.
...
Sie sollten eine `Rectangle`-Klasse haben.

```js
({ test: () => assert(runPython(`_Node(_code).has_class("Rectangle")`)) })
```

Sie sollten eine `Square`-Klasse haben.
...
Die `Square`-Klasse sollte eine eigenständige Klasse sein, die sich von der `Rectangle`-Klasse unterscheidet.
