...
Kama kuna mlingano basi weka `userData.currentSong` kuwa `null` na `userData.songCurrentTime` kuwa `0`. 

Baada ya hapo, fanya wito wa kitendakazi cha `pauseSong()` kuacha uchezaji na kitendakazi cha `setPlayerDisplay()` kusahisha onyesho la mchezaji.
