...
Kwanza, utapata nafasi ya lango maalum ndani ya safu ya data `gates`.

Tumia njia ya `indexOf()` kwenye `gates` kupata kipengele cha `currentGate` na weka thamani hiyo kwenye kigezo kinachoitwa `currentIndex`.

Hapa kuna ukumbusho wa jinsi hii inaweza kufanywa:
...
Unapaswa kutumia `indexOf()` kwenye safu ya data `gates` kupata kielezo cha `currentGate` na kuweka thamani yake kwenye kigezo kinachoitwa `currentIndex`. Hii inapaswa kufanywa kwa mstari mmoja.
