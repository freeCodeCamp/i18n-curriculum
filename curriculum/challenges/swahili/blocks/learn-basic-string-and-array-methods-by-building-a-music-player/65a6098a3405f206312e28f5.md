...
Ili kupanga nyimbo kwa mpangilio wa alfabeti kulingana na kichwa, utahitaji kupitisha kitendakazi cha mwitiko cha kulinganisha katika njia yako ya `sort()`.

Hapa kuna mfano wa kupanga orodha ya matunda kwa jina.
...
Katika hatua chache zijazo, utajifunza kile kila kauli ya `if` inachofanya ndani ya kitendakazi hicho cha mwitiko. Lakini kwa sasa, ongeza kitendakazi cha mwitiko tupu katika njia yako ya `sort()` na tumia `a` na `b` kama majina ya vigezo.

# --hints--

Unapaswa kuwa na kitendakazi cha mwitiko tupu `()=>{}` ndani ya njia yako ya `sort()` ukiwa na `a` na `b` kama majina ya vigezo.
