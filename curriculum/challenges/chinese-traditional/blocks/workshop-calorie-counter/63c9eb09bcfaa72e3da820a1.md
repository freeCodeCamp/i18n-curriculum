...
你需要取得所有的輸入容器。宣告一個 `inputContainers` 變數，並將它指定為查詢文件中所有具有類別 `input-container` 的元素的值。

# --hints--

你應該在你的 `clearForm` 函式（程式）中宣告一個 `inputContainers` 變數。

```js
assert.match(clearForm.toString(), /inputContainers\s*=/);
```

你應該使用 `querySelectorAll` 方法來取得所有具有 `input-container` 類別的元素。

```js
assert.match(clearForm.toString(), /document\.querySelectorAll\(\s*('|")\.input-container\1\s*\)/);
```

你應該將 `querySelectorAll` 方法的值指定給 `inputContainers` 變數。
