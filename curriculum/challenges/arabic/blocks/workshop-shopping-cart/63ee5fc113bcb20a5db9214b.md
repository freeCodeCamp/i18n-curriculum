...
تحتوي الأصناف على طريقة خاصة تسمى `constructor`، تُستدعى عند إنشاء نسخة جديدة من الصنف. تُعد طريقة `constructor` مكانًا ممتازًا لتهيئة خصائص الصنف. إليك مثالًا على صنف يحتوي على طريقة `constructor`:
...
أضف طريقة `constructor` فارغة إلى صنف `ShoppingCart`.

# --hints--

يجب أن تضيف طريقة `constructor` إلى صنف `ShoppingCart`.

```js
assert.match(code,  /class\s+ShoppingCart\s*\{\s*constructor\s*\(\s*\)\s*/)
```

يجب أن تكون طريقة `constructor` فارغة.
