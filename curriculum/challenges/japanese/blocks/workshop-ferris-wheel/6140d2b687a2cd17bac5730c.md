...
`.cabin` に `position` の `absolute` と、`border` の `2px solid` を与えてください。

# --hints--

`.cabin` セレクターには `position` に設定された `absolute` プロパティが必要です。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cabin')?.position, 'absolute');
```

`.cabin` セレクターには `border` に設定された `2px solid` プロパティが必要です。
