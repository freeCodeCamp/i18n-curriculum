...
Ви повинні використати властивість `border-radius`, щоб округлити гребінь.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

Ви повинні надати `.penguin-body::before` властивість `border-radius` зі значенням `0% 0% 100% 100%`.
