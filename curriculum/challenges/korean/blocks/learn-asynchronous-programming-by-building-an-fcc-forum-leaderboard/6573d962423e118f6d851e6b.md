...
다음으로, `#posts-container` 메서드를 사용해 `getElementById()` 요소에 접근하세요. 그리고 이를 `postsContainer`라는 새 상수에 할당하세요.

# --hints--

`const` 변수를 선언하려면 `postsContainer`를 사용하세요.

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

`document.getElementById()` 요소를 얻으려면 `#posts-container`를 사용하세요.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

`#posts-container` 요소를 `postsContainer` 변수에 할당하세요.
