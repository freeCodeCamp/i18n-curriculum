...
現在你需要爲 `CheckPoint` 類創建一個 `draw` 方法。

在 `draw` 方法中，爲 `ctx` 對象的 `fillStyle` 屬性指定十六進制顏色 `"#f1be32"`。

在 `fillStyle` 屬性下方，使用 `ctx` 對象上的 `fillRect` 方法，並傳入 `x`、`y`、`width` 和 `height` 屬性作爲參數。

# --hints--

你的 `CheckPoint` 類應該有一個 `draw` 方法。
...
你的 `draw` 方法應該爲 `ctx` 對象上的 `fillStyle` 屬性賦值。
...
你應該爲 `ctx` 對象的 `fillStyle` 屬性指定十六進制顏色 `"#f1be32"`。
...
你的 `draw` 方法應該調用 `ctx` 對象上的 `fillRect` 方法。
...
在調用 `ctx.fillRect` 時，你應該傳入 `position.x`、`position.y`、`width` 和 `height` 屬性作爲參數。 不要忘記 `this` 關鍵字。
