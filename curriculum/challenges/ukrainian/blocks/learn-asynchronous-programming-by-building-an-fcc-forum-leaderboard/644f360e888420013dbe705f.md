...
Усередині вашої умови `else` призначте рядок `"general"` змінній `selectedCategory.className`.

Нижче призначте рядок `"General"` змінній `selectedCategory.category`.

Нарешті, призначте число `1` змінній `selectedCategory.id`.

# --hints--

Ви повинні призначити рядок `"general"` змінній `selectedCategory.className`.

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

Ви повинні призначити рядок `"General"` змінній `selectedCategory.category`.

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

Ви повинні призначити число `1` змінній `selectedCategory.id`.
