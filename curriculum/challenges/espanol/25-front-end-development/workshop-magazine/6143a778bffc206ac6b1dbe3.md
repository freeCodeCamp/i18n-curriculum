...
Crea un último elemento `p` al final del elemento `.text` y dale el siguiente texto:
...
Debes añadir un sexto elemento `p` al elemento `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 7);
```

Tu sexto elemento `p` debe tener el texto proporcionado.
