...
لدمج عناصر `input` و `textarea` مع موضوع الخلفية، عيّن `background-color` الخاص بهما إلى `#0a0a23`. ثم، أضف لهما حدودًا `1px`، `solid` بلون `#0a0a23`.

# --hints--

يجب استخدام محدِّد عناصر مفصول بفواصل لاختيار عناصر `input` و `textarea`.
...
يجب إعطاء عناصر `input` و `textarea` لون خلفية `background-color` بقيمة `#0a0a23`.
...
يجب إعطاء عناصر `input` و `textarea` حدودًا `1px`، `solid` بلون `#0a0a23`.
