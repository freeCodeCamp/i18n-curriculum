...
Sie sollten nun die Hauptschleife der Simulation implementieren. Diese Schleife stellt sicher, dass jedes Gate für jeden Tick in der Simulation verarbeitet wird, bis alle Ticks abgeschlossen sind.

Erstellen Sie eine leere `while`-Schleife, die so lange läuft, wie `tickIndex` kleiner ist als `maxTicks`.

# --hints--

Sie sollten eine leere `while`-Schleife mit der Bedingung `(tickIndex < maxTicks)` erstellen.
