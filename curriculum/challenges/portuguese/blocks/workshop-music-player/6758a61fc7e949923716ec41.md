...
Agora, use o método `querySelectorAll` para selecionar os elementos `.playlist-song` e armazená-los em uma variável chamada `songs`.

Então, chame o método `forEach` em `songs`.Passe uma função de callback que use `song` como parâmetro para o método.Deixe as chaves vazias por enquanto.

# --hints--

Você deve ter uma variável chamada `songs`.

```js
assert.isDefined(songs);
```

Você deve atribuir a coleção de todos os elementos `.playlist-song` à sua variável `songs`.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

Você deve chamar o método `forEach` em `songs` e passar um callback que usa `song` como parâmetro para o método.
