...
No te preocupes por el espacio en la parte inferior, todo se moverá hacia abajo más adelante cuando agregues algo de altura al elemento en la parte superior del edificio.

Agrega un `repeating-linear-gradient` a `.fb1c` con un ángulo de `90deg`, tu `--building-color4` desde `0%` hasta `10%` y `transparent` desde `10%` hasta `15%`.

# --hints--

Debes darle a `.fb1c` un `background` con un `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.background, "repeating-linear-gradient");
```

Deberías usar una dirección de `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Deberías usar un primer color de `--building-color4` desde `0%` hasta `10%`.
...
Deberías usar un segundo color de `transparent` desde `10%` hasta `15%`.
