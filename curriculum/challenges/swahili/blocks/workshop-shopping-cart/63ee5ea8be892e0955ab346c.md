...
Sasa umeunda darasa la vitu la `Dessert`, unda darasa la vitu la `ShoppingCart`.
...
Unapaswa kutumia neno kuu la `class` kutangaza darasa la vitu la `ShoppingCart`.

```js
assert.match(code, /class\s+ShoppingCart\s*/);
```

Darasa lako la vitu la `ShoppingCart` linapaswa kuwa tupu.
