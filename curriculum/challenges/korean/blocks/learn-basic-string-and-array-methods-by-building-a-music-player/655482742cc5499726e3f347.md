...
`playPreviousSong` 함수 안에 `if` 문을 `userData?.currentSong === null` 조건과 함께 추가하세요. 이 조건은 현재 재생 중인 노래가 없는지 확인합니다. 노래가 없다면 `return`를 사용해 함수를 종료하세요.

`else` 블록 안에 `currentSongIndex`라는 상수를 만들고 `getCurrentSongIndex()`를 할당하세요.

# --hints--

기존 `playPreviousSong` 함수와 그 내용을 변경하지 마세요.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

`if` 함수 안에 `userData?.currentSong === null` 조건을 가진 `playPreviousSong` 문이 있어야 합니다.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

`return` 문 블록 안에 `if`이 있어야 합니다.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

`if` 문은 `else` 블록을 가져야 합니다.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

`getCurrentSongIndex` 문의 `currentSongIndex` 블록 안에서 `else`를 호출하고 `if`에 할당해야 합니다.
