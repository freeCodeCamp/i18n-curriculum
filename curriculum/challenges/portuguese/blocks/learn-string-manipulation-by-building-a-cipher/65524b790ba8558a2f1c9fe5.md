...
Limpe um pouco a saída. Delete `print(char, index)` e transforme a última chamada `print()` em `print('char:', char, 'new char:', new_char)`.

# --hints--

Você não deve ter `print(char, index)` no seu código.
...
Você deve alterar `print(new_char)` para `print('char:', char, 'new char:', new_char)`.
