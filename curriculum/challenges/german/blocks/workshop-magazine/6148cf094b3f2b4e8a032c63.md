...
Der `::first-letter`-Pseudo-Selektor ermöglicht es Ihnen, den ersten Buchstaben im Textinhalt eines Elements anzusprechen.

Erstellen Sie einen `.first-paragraph::first-letter`-Selektor und setzen Sie die `font-size`-Eigenschaft auf `6rem`. Geben Sie ihm außerdem eine `color`-Eigenschaft mit dem Wert `orangered`, damit er hervorsticht.

# --hints--

Sie sollten einen `.first-paragraph::first-letter`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

Ihr `.first-paragraph::first-letter`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `6rem` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize, '6rem');
```

Ihr `.first-paragraph::first-letter`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `orangered` haben.
