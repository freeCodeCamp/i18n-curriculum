...
使用 `const` 和箭头语法创建一个空的 `playPreviousSong` 函数。

# --hints--

你应该使用 `const` 创建一个名为 `playPreviousSong` 的空函数。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` 应该是一个函数。

```js
assert.isFunction(playPreviousSong)
```

你的 `playPreviousSong` 函数应该使用箭头语法。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `playPreviousSong` 函数应该是空的。
