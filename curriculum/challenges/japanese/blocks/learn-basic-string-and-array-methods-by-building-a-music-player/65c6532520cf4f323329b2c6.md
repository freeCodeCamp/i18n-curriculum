...
今のところ曲の順番は変わっていません。これは、`sort` メソッドを使って行った更新が、`sortSongs` 関数が呼び出されるまで実行されないためです。

`renderSongs` 関数を変更して、`sortSongs` 関数を呼び出すようにしてください。

これで曲がアルファベット順に表示されるはずです。

# --hints--

`renderSongs(sortSongs())` があるはずです。
