...
Sposta le proprietà e i valori `display`, `flex-direction` e `align-items` da `.bb1` alla nuova classe `building-wrap`.

# --hints--

Dovresti rimuovere `display` da `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.display);
```

Dovresti spostare `display` con un valore di `flex` in `.building-wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.display, "flex");
```

Dovresti rimuovere `flex-direction` da `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.flexDirection);
```

Dovresti spostare `flex-direction` con un valore di `column` in `.building-wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.flexDirection, "column");
```

Dovresti rimuovere `align-items` da `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.alignItems);
```

Dovresti spostare `align-items` con un valore di `center` in `.building-wrap`.
