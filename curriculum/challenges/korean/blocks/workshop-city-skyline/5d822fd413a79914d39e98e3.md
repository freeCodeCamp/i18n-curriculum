...
`--building-color4`라는 새 변수를 만들고 `#538cc6` 값을 할당하세요. 이번에는 `:root` 선택자 안에 있어야 합니다. 그런 다음 마지막 건물을 채우는 데 사용하세요.

# --hints--

`--building-color4`라는 새 속성 변수를 정의하세요.

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color4'));
```

`--building-color4` 선택자 안에서 `#538cc6`에 `:root` 값을 할당하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color4').trim(), '#538cc6');
```

`background-color: var(--building-color4)` 선택자 안에 `.bb4`을 추가하세요.
