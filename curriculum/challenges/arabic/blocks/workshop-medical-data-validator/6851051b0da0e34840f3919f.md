...
يجب ألا يكون `age` عددًا صحيحًا فقط، بل يجب أن يكون عددًا صحيحًا موجبًا أكبر من أو يساوي `18`.

باستخدام العامل `and`، أضف تعبيرًا ثانيًا إلى قيمة المفتاح `age` للتحقق من ذلك.

# --hints--

يجب أن يحتوي المفتاح `age` في قاموس `constraints` على القيمة `isinstance(age, int) and age >= 18`.
