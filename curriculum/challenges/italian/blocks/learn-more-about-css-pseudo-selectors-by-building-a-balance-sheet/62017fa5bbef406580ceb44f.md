...
Ora che hai aggiunto la parola chiave `!important`, puoi rimuovere `:not(.sr-only)` dal tuo selettore `span`.

# --hints--

Non dovresti avere un selettore `span:not(.sr-only)`.

```js
assert(!(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)')));
```

Dovresti avere un selettore `span`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span'));
```

Non dovresti modificare `font-weight`.
