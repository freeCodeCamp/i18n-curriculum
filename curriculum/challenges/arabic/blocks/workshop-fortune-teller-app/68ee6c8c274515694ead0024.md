...
يجب أن تكون النبوءة عشوائية، لذا تحتاج إلى دالة مساعدة لاختيار نبوءة عشوائية. أعلن عن متغير باسم `chosenCard` داخل الطريقة `singleCardSelected`. استخدم الطريقة `getRandomItem` للحصول على بطاقة عشوائية ومرر `cards` كمعلمة.

# --hints--

يجب إعلان متغير باسم `chosenCard` داخل `singleCardSelected`.
...
يجب تعيين متغير `chosenCard` الخاص بك عن طريق استدعاء `getRandomItem(this.cards)`.
...
يجب تمرير `this.cards` كمعلمة إلى `getRandomItem`.
