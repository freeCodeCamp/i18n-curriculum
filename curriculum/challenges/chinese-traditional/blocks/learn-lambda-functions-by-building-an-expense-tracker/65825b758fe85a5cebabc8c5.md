...
要顯示總支出，請創建一個 `elif` 語句來檢查是否爲 `choice == '3'`。

如果爲真，則表示用戶想要查看總支出。 因此調用 `print()` 並將字符串 `'\nTotal Expenses: '` 作爲第一個參數傳遞，將 `total_expenses(expenses)` 作爲第二個參數傳遞。

# --hints--

你應該創建一個 `elif` 子句來檢查是否 `choice == '3'`。
