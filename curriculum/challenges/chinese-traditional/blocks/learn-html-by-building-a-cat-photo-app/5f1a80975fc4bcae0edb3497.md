...
Both radio buttons should have a `value` attribute. 檢查開始標籤的名稱後面是否有空格和/或所有屬性名稱之前是否有空格。
...
Both radio buttons should have a `type` attribute. 檢查開始標籤的名稱後面是否有空格和/或所有屬性名稱之前是否有空格。
...
The `Indoor` radio button's `value` attribute should be set to `indoor`. 你要麼遺漏了屬性值，要麼拼寫有誤。 記住，屬性值應該放在引號中。
...
The `Outdoor` radio button's `value` attribute should be set to `outdoor`. 你要麼遺漏了屬性值，要麼拼寫有誤。 記住，屬性值應該放在引號中。
