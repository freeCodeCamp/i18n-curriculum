...
创建一个名为 `setPlayerDisplay` 的函数。在函数内部，如果定义了当前歌曲的标题，则将 `playingSong` 的 `textContent` 设置为该标题。否则，将其设置为空字串。

在此之下，将 `songArtist` 的 `textContent` 设置为当前歌曲的艺术家（如果已定义）。否则，将其设置为空字串。 


# --hints--

你应该有一个名为 `setPlayerDisplay` 的函数。

```js
assert.isFunction(setPlayerDisplay);
```

你的 `setPlayerDisplay` 函数应将 `playingSong` 的 `textContent` 设置为当前歌曲的标题（如果已定义），否则设置为空字串。
...
你的 `setPlayerDisplay` 函数应将 `songArtist` 的 `textContent` 设置为当前歌曲的艺术家（如果已定义），否则设置为空字串。
