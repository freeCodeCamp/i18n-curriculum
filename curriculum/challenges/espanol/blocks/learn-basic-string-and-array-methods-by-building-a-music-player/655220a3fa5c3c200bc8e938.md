...
Usa `classList` y el método `remove()` para eliminar la clase `playing` del `playButton`, ya que la canción estará pausada en este punto.

Para pausar finalmente la canción, usa el método `pause()` en la variable `audio`. `pause()` es un método de la Web Audio API para pausar archivos de música.

# --hints--

No debes modificar la función existente `pauseSong` ni su contenido.
