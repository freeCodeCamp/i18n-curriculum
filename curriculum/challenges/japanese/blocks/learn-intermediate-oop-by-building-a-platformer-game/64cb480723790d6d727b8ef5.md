...
`else if` 文の中に、`forEach` 配列を繰り返し処理するための `checkpoints` ループを追加してください。コールバック関数のパラメータ名には `checkpoint` を使ってください。

`forEach` コールバックの中で、加算代入演算子を使って checkpoints の `x` 位置に 5 を加えてください。

# --hints--

`forEach` 配列を繰り返し処理する `checkpoints` ループがあるはずです。
...
`checkpoint` コールバック関数の中で `forEach` パラメータを使うべきです。
...
加算代入演算子を使って `checkpoint` の `x` 位置に 5 を加えるべきです。
