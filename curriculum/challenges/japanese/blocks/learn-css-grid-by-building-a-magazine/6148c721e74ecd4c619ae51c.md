...
`.text` 要素は CSS グリッドではありませんが、`column-width` プロパティを使用することで、グリッドを使用せずに要素内に列を作成できます。

`.text` セレクターの `column-width` プロパティを `25rem` に設定してください。

# --hints--

`.text` セレクターの `column-width` プロパティを `25rem` に設定する必要があります。
