...
`25%` 선택자와 `0%` 선택자 사이에 새로운 `50%` 선택자를 만드세요. 이 새로운 선택자에 `background-color` 속성을 `yellow`로 설정하세요.

# --hints--

`25%` 규칙 안에 새로운 `@keyframes cabins` 선택자를 만들어야 합니다.
