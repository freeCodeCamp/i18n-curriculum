...
Geben Sie schließlich den Minimumwert aus der `prices`-Liste mit der `min()`-Funktion zurück. So erhalten Sie den besten (niedrigsten) Preis, nachdem alle anwendbaren Rabatte berücksichtigt wurden.

# --hints--

Ihre `calculate_best_price`-Methode sollte den Minimumwert aus der `prices`-Liste mit der `min`-Funktion zurückgeben.
