...
왼쪽 눈을 `position`의 `absolute` 속성, `top`의 `54px` 속성, 그리고 `left`의 `39px` 속성으로 위치시키세요.

# --hints--

`.cat-left-eye` 선택자에 `position` 속성을 `absolute`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.position === 'absolute')
```

`.cat-left-eye` 선택자에 `top` 속성을 `54px`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.top === '54px')
```

`.cat-left-eye` 선택자에 `left` 속성을 `39px`로 설정하세요.
