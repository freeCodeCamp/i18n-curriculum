...
將標的 `.penguin-body` 元素，並給它 `width` 為 `53%`，以及 `height` 為 `45%`。然後，將 `background` 設定為在 `45deg` 的線性漸層，從 `0%` 的 `rgb(134, 133, 133)`、`25%` 的 `rgb(234, 231, 231)`，以及 `67%` 的 `white`。
...
你應該給 `.penguin-body` 一個 `width` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

你應該給 `.penguin-body` 一個 `height` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

你應該給 `.penguin-body` 一個 `background`，其值為 `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)`。
