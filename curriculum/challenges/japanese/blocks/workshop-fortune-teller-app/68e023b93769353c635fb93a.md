...
今、`renderCard` 関数のテンプレートリテラル内に `div` 要素を作成してください。`div` の中に、`h2` を出力する `drawingType` 要素と、クラス `figure` と属性 `card_container ${isReversed ? "reversed-card" : ""}` を持つ `data-id="${shortName}"` 要素を追加してください。

# --hints--

`div` 関数のテンプレートリテラル内に `renderCard` 要素があるはずです。
...
`h2` 要素の中に `drawingType` を埋め込んだ `div` 要素があるはずです。
...
`figure` 要素はクラス `card_container` を持ち、三項演算子を使って条件付きで `reversed-card` を追加してください。
...
`figure` 要素は `data-id` 属性を `${shortName}` に設定してください。
