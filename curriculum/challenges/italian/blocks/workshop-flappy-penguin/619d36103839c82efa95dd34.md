...
Dovresti dare a `.penguin` una proprietà `transition-duration` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionDuration, '1s');
```

Dovresti dare a `.penguin` una proprietà `transition-timing-function` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionTimingFunction, 'ease-in-out');
```

Dovresti dare a `.penguin` un proprietà `transition-delay` con il valore `--fcc-expected--` invece di `--fcc-actual--`.
