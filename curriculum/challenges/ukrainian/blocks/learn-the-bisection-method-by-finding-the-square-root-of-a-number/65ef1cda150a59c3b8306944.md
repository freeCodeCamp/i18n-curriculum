...
Якщо різниця не знаходиться в межах заданого `tolerance`, створіть інструкцію `elif`, щоб перевірити, чи `square_mid` менше за `square_target`.

Призначте значення `mid` змінній `low`, оскільки квадратний корінь тепер лежатиме між `low` та `mid`.

# --hints--

Ви повинні створити інструкцію `elif`, яка перевіряє, чи `square_mid < square_target`.
...
Ви повинні призначити значення `mid` змінній `low`.
