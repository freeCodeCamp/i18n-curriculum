...
Gehen Sie zum `footer`-Element und stellen Sie für den gesamten Text eine Schriftgröße von `14px` ein.

# --hints--

Sie sollten einen `footer`-Selektor haben.
...
Ihr `footer`-Selektor sollte unter Ihrem Kommentar stehen.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Sie sollten die `font-size`-Eigenschaft auf `14px` setzen.
...
Ihr `footer`-Element sollte eine `font-size` von `14px` haben.
