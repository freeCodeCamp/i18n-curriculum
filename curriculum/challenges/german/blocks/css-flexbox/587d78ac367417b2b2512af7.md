...
title: Die Eigenschaft justify-content im Tweet-Embed verwenden
challengeType: 0
...
Die letzte Aufgabe zeigte ein Beispiel für die `justify-content`-Eigenschaft. Für das Tweet-Embed kann diese Eigenschaft angewendet werden, um die Elemente im `.profile-name`-Element auszurichten.

# --instructions--

Fügen Sie die CSS-Eigenschaft `justify-content` zum `.profile-name`-Element der Kopfzeile hinzu und setzen Sie den Wert auf eine der Optionen aus der letzten Aufgabe.
...
Das `.profile-name`-Element sollte die `justify-content`-Eigenschaft mit einem der folgenden Werte haben: `center`, `flex-start`, `flex-end`, `space-between`, `space-around` oder `space-evenly`.
