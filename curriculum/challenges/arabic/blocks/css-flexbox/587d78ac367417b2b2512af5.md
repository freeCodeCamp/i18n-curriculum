...
title: تطبيق خاصية flex-direction لإنشاء عمود في تضمين التغريدة
challengeType: 0
...
استخدم القسم العلوي `header` والتذييل `footer` في تضمين التغريدة خاصية `flex-direction` سابقًا بقيمة row. وبالمثل، ستعمل العناصر داخل عنصر `.profile-name` بشكل جيد إذا تم ترتيبها كعمود.

# --instructions--

أضف خاصية CSS `flex-direction` إلى عنصر `.profile-name` في القسم العلوي واضبط القيمة على `column`.

# --hints--

يجب عرض زر `.follow-btn` على الصفحة. تأكد من تعطيل أي إضافات مثل مانعات الإعلانات.
...
يجب أن يحتوي عنصر `.profile-name` على خاصية `flex-direction` مضبوطة على `column`.
