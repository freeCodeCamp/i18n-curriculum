...
將事件監聽器附加到你的 `checkMessageButton`，監聽 `click` 事件。

如果 `messageInput` 爲空，則向用戶顯示帶有消息 `"Please enter a message."` 的警告。
...
當你單擊 `checkMessageButton` 並且 `messageInput` 爲空時，你應該使用消息 `"Please enter a message."` 提醒用戶。
...
當你單擊 `checkMessageButton` 時，僅當 `messageInput` 爲空時才應提醒用戶。
