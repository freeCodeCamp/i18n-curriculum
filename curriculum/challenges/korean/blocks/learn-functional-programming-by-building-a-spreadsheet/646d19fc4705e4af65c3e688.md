...
당신의 `addCharacters` 함수는 결국 문자 범위를 반환합니다. 셀 ID 배열을 반환하도록 만들고 싶습니다. `.map()` 호출에 `charRange()` 메서드를 연결하세요. 아직 콜백 함수는 전달하지 마세요.

# --hints--

`.map()` 호출에 `charRange()`을 연결해야 합니다.
