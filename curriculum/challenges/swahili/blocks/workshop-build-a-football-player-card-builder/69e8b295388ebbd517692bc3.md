...
Kumbuka kutoka JSX kwamba mabano ya mviringo hukuruhusu kuingiza usemi wowote wa JavaScript moja kwa moja ndani ya JSX. Hivi ndivyo unavyoonyesha data inayobadilika kutoka kwa vigezo vinavyopitishwa. Unafanya vivyo hivyo katika TSX.

Kichwa cha kadi bado kinaonyesha maadili yaliyowekwa moja kwa moja. Badilisha maandishi `98` ndani ya `card-rating` na `player.overallRating`, na badilisha `ST` ndani ya `card-position` na `player.position`.

# --hints--

Unapaswa kubadilisha `98` iliyowekwa moja kwa moja na `player.overallRating` ndani ya `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

Unapaswa kubadilisha `ST` iliyowekwa moja kwa moja na `player.position` ndani ya `.card-position`.
