...
我们不希望玩家的唯一武器被损坏。 逻辑“与”运算符检查两个语句是否为真。

使用 <dfn>逻辑 AND</dfn> 运算符 `&&` 向 `if` 语句添加第二个条件。 只有当 `inventory.length` 不等于 (`!==`) 一时，玩家的武器才会损坏。
...
你应该使用逻辑 AND 运算符来检查 `inventory.length` 是否不等于 `1`。
