...
Para crear un paisaje en el fondo, agregarás dos montañas.

Sobre el elemento `.penguin`, añade un `div` con una `class` de `left-mountain`.

# --hints--

Debes agregar un nuevo `div` dentro de `body`. Se esperaba ver elementos `--fcc-expected--` `div`, pero se encontraron `--fcc-actual--`.
...
Debes colocar `.left-mountain` antes de `.penguin`.
