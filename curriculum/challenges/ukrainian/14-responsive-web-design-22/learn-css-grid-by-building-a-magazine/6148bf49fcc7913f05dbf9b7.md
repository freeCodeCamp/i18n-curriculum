...
Це створить ефект наведення лише для елемента `a` в межах `.author-name`, показуючи вихідний freeCodeCamp зеленим в фоновому режимі.

# --hints--

Ви повинні мати селектор `.author-name a:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

Ваш селектор `.author-name a:hover` повинен мати властивість `background-color` зі значенням `#306203`.
