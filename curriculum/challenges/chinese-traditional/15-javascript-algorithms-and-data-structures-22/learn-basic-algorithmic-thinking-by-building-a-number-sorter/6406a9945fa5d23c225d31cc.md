...
要測試你寫的代碼，將事件監聽器掛載到你的 `sortButton` 元素上。 它應該監聽 `"click"` 事件，並將 `sortInputArray` 作爲回調。

# --hints--

你應該在你的 `sortButton` 元素上調用 `.addEventListener()` 。
...
你的 `.addEventListener()` 方法應該把 `sortInputArray` 作爲回調。 記住要傳遞函數引用，而不是函數調用。
