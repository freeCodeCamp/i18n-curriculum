...
最后，你应该通过一些重构来整理你的代码！ 你可能会注意到，你的代码在 `getHighestDuplicates`、`detectFullHouse` 和 `checkForStraights` 函数中多次启用了最后一个单选按钮（“以上都不是”）。 重构你的代码，只在 `rollDiceBtn` 内部执行一次，而不是在每个函数末尾启用最后一个单选按钮。

# --hints--

无论是否掷出重复点数，你的 `getHighestDuplicates` 函数都不应该启用最后一个单选按钮，或设置最后一个 `span` 文本。
...
无论是否掷出葫芦（或满堂红，英文为 full house），你的 `detectFullHouse` 函数都不应该启用最后一个单选按钮，或设置最后一个 `span` 文本。
...
无论是否掷出顺子，你的 `checkForStraights` 函数都不应该启用最后一个单选按钮，或设置最后一个 `span` 文本。
...
当你的 `rollDiceBtn` 被点击时，你应该启用最后一个单选按钮，并将值设置为 `0`，同时更新显示文本为 `, score = 0`。
