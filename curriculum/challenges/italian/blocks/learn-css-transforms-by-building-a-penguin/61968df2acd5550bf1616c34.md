...
Normalizza le dimensioni della tua pagina, rimuovendo il `body` elemento `margin` e `padding`.

# --hints--

Dovresti dare a `body` un `margin` di `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.margin, '0px');
```

Dovresti dare a `body` un `padding` di `0`.
