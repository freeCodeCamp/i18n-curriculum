...
`map()` メソッドは配列を繰り返し処理して新しい配列を返すために使います。既存の配列の値に基づいて新しい配列を作成したいときに便利です。例えば：
...
`map()` メソッドが関数を引数として受け取ることに注目してください。これは <dfn>コールバック関数</dfn> と呼ばれ、別の関数に引数として渡される関数です。上の例では、コールバック関数は `(number) => number * 2` で、`numbers` 配列の各要素に対して実行されます。`map()` メソッドはその結果を使って新しい配列を返します。

`map()` メソッドにコールバック関数を渡してください。コールバック関数は `song` をパラメータとして受け取り、アロー関数構文を使い、空の本体にしてください。

# --hints--

`map()` メソッドのコールバック関数はアロー関数構文を使うべきです。

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

`map()` メソッドのコールバック関数には `song` パラメータが必要です。
