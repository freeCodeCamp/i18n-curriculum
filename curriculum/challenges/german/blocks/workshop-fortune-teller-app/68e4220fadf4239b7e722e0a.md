...
Da die Fortune-App nur drei Zeitpunkte hat: Vergangenheit, Gegenwart und Zukunft, deklarieren Sie ein Enum namens `DrawingType` mit drei Eigenschaften: `Past`, `Present` und `Future`. Jede Eigenschaft soll einen String-Wert haben, der ihrem Namen entspricht.

# --hints--

Sie sollten `enum` mit dem Namen `DrawingType` haben.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Ihr Enum sollte eine `Past`-Eigenschaft mit dem String-Wert `past` haben.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Ihr Enum sollte eine `Present`-Eigenschaft mit dem String-Wert `present` haben.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Ihr Enum sollte eine `Future`-Eigenschaft mit dem String-Wert `future` haben.
