...
在 `card-image-wrap` 元素後，添加一個 `div`，其 `className` 為 `card-name-strip`。在其中，添加一個 `span`，其 `className` 為 `card-name`，並包含文字 `PELE`。

# --hints--

你應該添加一個 `div`，其 `className` 為 `card-name-strip`。

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

你應該在 `span` 裡添加一個 `className`，其 `card-name` 為 `.card-name-strip`。

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

`.card-name` 範圍應該包含文字 `PELE`。
