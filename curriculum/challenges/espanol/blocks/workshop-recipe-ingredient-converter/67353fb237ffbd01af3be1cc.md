...
Finalmente puedes comenzar a integrar tu lógica en la interfaz de usuario. Necesitarás consultar el DOM para obtener algunos elementos por ID. Obtén los elementos con los IDs `ingredient`, `quantity`, `unit`, `servings`, `recipe-form`, y `result-list`, y asígnalos a las variables `ingredientName`, `ingredientQuantity`, `unitToConvert`, `numberOfServings`, `recipeForm`, y `resultList`, respectivamente.

# --hints--

Deberías definir una variable `ingredientName`.

```js
assert.isDefined(ingredientName);
```

Tu variable `ingredientName` debería ser el elemento devuelto por `.getElementById()` con el ID `ingredient`.

```js
assert.strictEqual(ingredientName, document.getElementById('ingredient'));
```

Deberías definir una variable `ingredientQuantity`.

```js
assert.isDefined(ingredientQuantity);
```

Tu variable `ingredientQuantity` debería ser el elemento devuelto por `.getElementById()` con el ID `quantity`.

```js
assert.strictEqual(ingredientQuantity, document.getElementById('quantity'));
```

Deberías definir una variable `unitToConvert`.

```js
assert.isDefined(unitToConvert);
```

Tu variable `unitToConvert` debería ser el elemento devuelto por `.getElementById()` con el ID `unit`.

```js
assert.strictEqual(unitToConvert, document.getElementById('unit'));
```

Deberías definir una variable `numberOfServings`.

```js
assert.isDefined(numberOfServings);
```

Tu variable `numberOfServings` debería ser el elemento devuelto por `.getElementById()` con el ID `servings`.

```js
assert.strictEqual(numberOfServings, document.getElementById('servings'));
```

Deberías definir una variable `recipeForm`.

```js
assert.isDefined(recipeForm);
```

Tu variable `recipeForm` debería ser el elemento devuelto por `.getElementById()` con el ID `recipe-form`.

```js
assert.strictEqual(recipeForm, document.getElementById("recipe-form"));
```

Deberías definir una variable `resultList`.

```js
assert.isDefined(resultList);
```

Tu variable `resultList` debería ser el elemento devuelto por `.getElementById()` con el ID `result-list`.
