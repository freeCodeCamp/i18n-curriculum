...
Dato che stai chiamando `fetchCardsData` e `initializeEventListeners` all'interno del costruttore, ora devi definirli all'interno della classe. Crea entrambi come metodi `private`. Assicurati che `fetchCardsData` sia asincrono perché gestirà chiamate API.

# --hints--

Dovresti avere un metodo `private` chiamato `fetchCardsData`.
...
Il tuo metodo `fetchCardsData` dovrebbe essere `async`.
...
Dovresti avere un metodo `private` chiamato `initializeEventListeners`.
