...
Далі потрібно перевірити, чи список відтворення порожній. Якщо це так, потрібно скинути об’єкт `userData` до початкового стану.

Використайте інструкцію `if`, щоб перевірити, чи `userData?.songs` має довжину `0`.

# --hints--

Створіть порожню інструкцію `if` з умовою `userData?.songs.length === 0`.
