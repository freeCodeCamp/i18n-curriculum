...
`instrumentsArr` 將會保存每張樂器卡的物件。每個物件都會有 `category`、`instrument` 和 `price` 這些鍵。

為你的陣列添加第一個表述長笛的物件。對於值，分別使用 `woodwinds`、`Flute` 和 `500`。

# --hints--

你的 `instrumentsArr` 陣列應該有一個元素。

```js
assert.lengthOf(instrumentsArr, 1);
```

你應該在 `instrumentsArr` 裡面有一個物件。

```js
assert.isObject(instrumentsArr[0]);
```

`instrumentsArr` 裡的物件應該有一個 `category` 鍵，其值為 `woodwinds`。

```js
assert.equal(instrumentsArr[0].category, "woodwinds");
```

`instrumentsArr` 內的物件應該有一個 `instrument` 鍵，其值為 `Flute`。

```js
assert.equal(instrumentsArr[0].instrument, "Flute");
```

`instrumentsArr` 中的物件應該有一個 `price` 鍵，其值為 `500`。
