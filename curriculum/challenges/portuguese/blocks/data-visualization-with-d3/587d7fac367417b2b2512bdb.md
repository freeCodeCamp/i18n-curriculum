...
Os métodos `domain()` e `range()` definem esses valores para a escala. Os dois métodos recebem um array de pelo menos dois elementos como argumento. Aqui está um exemplo:
...
**Nota:** Você pode encadear os métodos `domain()` e `range()` na variável `scale`.
