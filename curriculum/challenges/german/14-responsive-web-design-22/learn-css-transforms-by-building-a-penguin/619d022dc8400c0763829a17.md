...
Du solltest den `.face.left`-Selektor verwenden.

```js
assert.match(code, /\.face\.left\s*\{/);
```

Du solltest `.face.left` eine `left`-Eigenschaft geben.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face.left')?.left);
```

Du solltest `.face.left` eine `left` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.
