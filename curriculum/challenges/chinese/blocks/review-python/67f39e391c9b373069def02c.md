...
- **`upper()`**：此字串方法返回一个所有字符均转换为大写的新建字串：
...
- **`lower()`**：此字串方法返回一个所有字符均转换为小写的新建字串：
...
- **`strip()`**：此字串方法返回删除指定前导和尾随字符（如果未传递参数给该方法，则删除前导和尾随空白）的字串拷贝。
...
- **`startswith(prefix)`**：此字串方法返回一个布尔值，指示字串是否以指定的前缀开头：
...
- **`endswith(suffix)`**：此字串方法返回一个布尔值，指示字串是否以指定的后缀结尾：
...
- **`find()`**：此字串方法返回子串首次出现的索引。如果未找到，则返回 `-1`：
...
- **`count(substring)`**：此字串方法计算子字串在字串中出现的次数：
...
- **`capitalize()`**：此字串方法返回一个新字串，其首个字符大写，其他字符小写：
...
- **`isupper()`**：如果字串中的所有字母都是大写，则此字串方法返回 `True`，否则返回 `False`：
...
- **`islower()`**：如果字串中的所有字母都是小写，则此字串方法返回 `True`，否则返回 `False`：
...
- **`title()`**：此字串方法返回一个新建字串，其中每个单词的首字母均大写：
...
- **`maketrans()`**：这个字串方法用于创建一个 1 对 1 字符映射的表格以进行翻译。它通常与 `translate()` 方法一起使用，后者将该表格应用于字串并返回翻译结果。
...
- **切片列表**：切片是通过使用切片操作符 `:` 来访问列表部分内容的概念。要切片一个从索引 `1` 开始且在索引 `3` 之前结束的列表，你可以使用以下语法：
...
- **sorted()**：一个内置函数，返回一个新建的排序列表，而不是修改原始列表：
...
- 如果 `count()` 方法中指定的项在元组中根本不存在，则返回值将是 `0`：
...
- 如果没有传递任何参数给 `count()` 方法，那么 Python 会返回一个 `TypeError`。
...
- **差集操作符 (`-`)**：差集操作符 `-` 返回一个新建集合，其中包含第一个集合中不在另一个集合内的元素。
...
- **IndexError**：如果你访问一个在列表或其他序列如元组和字串中不存在的索引，就会得到一个 `IndexError`。例如，在一个 `Hello world` 字串中，最后一个字符的索引是 `10`。如果你继续这样访问字符，`greet = "hello world"; print(greet[11])`，你将得到一条消息为 `IndexError: string index out of range` 的误差。
...
- **`try...except`**：用于执行可能引起异常的代码块。`try` 块是你预期可能发生错误的地方，而 `except` 块会捕获指定的异常并在该错误引发时运行。示例如下：
...
以下是如何使用 `login` 函数和 `InvalidCredentialsError` 异常的方法：
...
- **在对象上调用方法**：你可以调用对象上的方法来执行操作或检索信息。
...
- **算术操作，如加法、减法、乘法、除法和其他**。`__add__()` 用于加法，`__sub__()` 用于减法，`__mul__()` 用于乘法，`__truediv__()` 用于除法。
...
- **`super()` 函数**：一个函数，允许你在类对该方法有不同实现或扩展该方法时，**调用**父类的方法，而无需重复代码。
