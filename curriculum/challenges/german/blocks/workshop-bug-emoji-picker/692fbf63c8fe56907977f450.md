...
Definieren Sie als Nächstes ein Objekt mit dem Namen `bugMap` vom Typ `Record<string, Bug<string>>` und weisen Sie ihm vorerst ein leeres Objekt zu.

# --hints--

Sie sollten eine Konstante mit dem Namen `bugMap` haben.
...
Ihre `bugMap` sollte eine Typannotation von `Record<string, Bug<string>>` haben.
...
Ihre `bugMap` sollte mit einem leeren Objekt initialisiert werden.
