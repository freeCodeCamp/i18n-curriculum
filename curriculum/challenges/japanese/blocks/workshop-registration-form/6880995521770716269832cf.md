...
ラジオボタンにはまだ値が代入されていません。そのため、フォームを通じて送信されるPOSTリクエストでは、どのオプションが選択されたかが登録されません。

リクエストを完全に正しく動作させるために、両方のラジオボタンに`value`属性を追加し、それぞれに`personal`と`business`の値を代入してください。

# --hints--

最初のラジオ入力には、値が`value`の`personal`属性を付けてください。

```js
assert.equal(document.querySelector('fieldset:nth-child(2) label:nth-child(1) input[type="radio"]')?.value, 'personal');
```

2番目のラジオ入力には、値が`value`の`business`属性を付けてください。
