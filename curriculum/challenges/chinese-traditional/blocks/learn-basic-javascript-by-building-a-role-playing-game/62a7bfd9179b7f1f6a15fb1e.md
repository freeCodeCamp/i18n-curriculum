...
創建三個新的空函式（程式），分別稱為 `buyHealth`、`buyWeapon` 和 `goTown`。

# --hints--

你應該使用 `function` 關鍵字宣告 `buyHealth`。

```js
assert.match(code, /function\s+buyHealth/);
```

你應該使用 `function` 關鍵字宣告 `buyWeapon`。

```js
assert.match(code, /function\s+buyWeapon/);
```

你應該使用 `function` 關鍵字宣告 `goTown`。

```js
assert.match(code, /function\s+goTown/);
```

`buyHealth` 應該是一個空的函式（程式）。

```js
assert.equal(buyHealth.toString(), 'function buyHealth() {}');
```

`buyWeapon` 應該是一個空的函式（程式）。
