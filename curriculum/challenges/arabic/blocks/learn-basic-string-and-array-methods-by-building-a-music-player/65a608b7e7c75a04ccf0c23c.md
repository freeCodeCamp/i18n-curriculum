...
الآن بعد أن عرضت قائمة الأغاني على الشاشة، سيكون من الجيد ترتيبها أبجديًا حسب العنوان. يمكنك تحديث مصفوفة `allSongs` يدويًا، لكن جافاسكريبت توفر طريقة للمصفوفات يمكنك استخدامها تسمى <dfn>sort()</dfn>.

تُحوّل طريقة `sort()` عناصر المصفوفة إلى سلاسل نصية وتُرتبها في مكانها بناءً على قيمها في ترميز `UTF-16`.
...
داخل دالة `sortSongs`، أضف طريقة `sort()` إلى `userData?.songs`.

# --hints--

يجب أن تضيف طريقة `sort()` إلى `userData?.songs`.
