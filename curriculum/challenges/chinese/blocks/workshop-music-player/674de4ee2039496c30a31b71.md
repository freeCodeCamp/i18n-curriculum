...
在 `else` 块中，将 `audio` 对象的 `currentTime` 属性设置为存储在 `userData.songCurrentTime` 中的值。

# --hints--

当有当前歌曲播放时，你的 `playSong` 函数应将 `audio` 对象的 `currentTime` 属性设置为存储在 `userData.songCurrentTime` 中的值。
