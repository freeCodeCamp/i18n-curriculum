...
Зробіть так, щоб елементи `h3` мали вищий пріоритет за допомогою таких CSS-властивостей:
...
Ви повинні використати селектор елемента `h3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3'));
```

Ви повинні надати елементу `h3` проп `margin-top` зі значенням `5px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Ви повинні надати елементу `h3` проп `padding-left` зі значенням `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Ви повинні надати елементу `h3` проп `font-size` зі значенням `1.375ren`.
