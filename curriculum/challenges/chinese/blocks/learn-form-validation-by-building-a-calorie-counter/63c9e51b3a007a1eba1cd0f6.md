...
现在声明一个 `remainingCalories` 变量，并赋予它从 `budgetCalories` 中减去 `consumedCalories` 并加上 `exerciseCalories` 的值。

# --hints--

你的 `calculateCalories` 函数应该有一个 `remainingCalories` 变量。

```js
assert.match(calculateCalories.toString(), /remainingCalories\s*=/);
```

你应该找到 `budgetCalories - consumedCalories + exerciseCalories` 的值。

```js
assert.match(calculateCalories.toString(), /budgetCalories\s*-\s*consumedCalories\s*\+\s*exerciseCalories/);
```

你应该将 `budgetCalories - consumedCalories + exerciseCalories` 的值分配给 `remainingCalories`。
