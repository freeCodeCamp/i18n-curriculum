...
Füge innerhalb deiner `else if`-Anweisung eine `forEach`-Schleife hinzu, um das `checkpoints`-Array zu durchlaufen. Verwende `checkpoint` als Parameternamen für die Callback-Funktion.
...
Du solltest einen `checkpoint`-Parameter innerhalb deiner `forEach`-Callback-Funktion verwenden.
