...
Crie uma declaração CSS para o seu id `orange-text` no seu elemento `style`. Aqui está um exemplo de como isso fica:
...
**Nota:** Não importa se você declara este CSS acima ou abaixo da classe `pink-text`, pois o atributo `id` sempre terá precedência.
