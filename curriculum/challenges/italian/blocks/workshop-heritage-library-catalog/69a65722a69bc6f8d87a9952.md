...
JSON (JavaScript Object Notation) è un formato standard per condividere dati tra sistemi. `JSON.stringify` converte un valore JavaScript in una stringa formattata JSON.

Inizia rimuovendo le due chiamate `validateEntry` console.log, perché servivano solo per i test. Poi crea una funzione chiamata `exportToJSON` che prende `catalog` come parametro. Ritorna `JSON.stringify(catalog, null, 2)`. Il primo argomento è il dato da convertire. Il secondo argomento (`null`) indica nessun filtro personalizzato. Il terzo (`2`) aggiunge un'indentazione di due spazi, rendendo l'output leggibile.

Sotto la funzione, fai il log di `exportToJSON(catalog.slice(0, 2))` per vedere in anteprima l'output JSON dei primi due elementi.

# --hints--

Dovresti rimuovere le chiamate `validateEntry` console.log.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry/);
```

`exportToJSON` dovrebbe restituire una stringa.

```js
assert.isString(exportToJSON(catalog));
```

`JSON.parse(exportToJSON(catalog))` dovrebbe produrre un array valido.

```js
assert.isArray(JSON.parse(exportToJSON(catalog)));
```

Dovresti usare `JSON.stringify` con `null` e `2` come secondo e terzo argomento.

```js
assert.match(exportToJSON.toString(), /JSON\.stringify\s*\(\s*catalog\s*,\s*null\s*,\s*2\s*\)/);
```

Dovresti fare il log di `exportToJSON(catalog.slice(0, 2))` nella console.
