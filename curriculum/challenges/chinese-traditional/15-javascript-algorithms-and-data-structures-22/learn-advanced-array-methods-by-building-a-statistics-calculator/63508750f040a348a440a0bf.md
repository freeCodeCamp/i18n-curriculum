...
爲了顯示 `mean` 的值，你的應用已經準備好了 `#mean` 元素。

使用 `.querySelector` 查找該元素，然後將其 `.textContent` 設置爲 `mean` 的值。

# --hints--

你的 `calculate` 函數應該使用 `document.querySelector()`。

```js
assert.lengthOf(calculate.toString().match(/document\.querySelector\(/g), 2);
```

你的 `.querySelector()` 應該選中 `#mean` 元素。

```js
assert.match(calculate.toString(), /document\.querySelector\(\s*('|")#mean\1\s*\)/);
```

你的 `calculate` 函數應該將 `#mean` 元素的 `.textContent` 設置爲 `mean` 的值。
