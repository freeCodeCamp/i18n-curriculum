...
在編程中，`null` 表示沒有值。 在這種情況下，如果用戶輸入了無效的輸入，你需要提醒他們，然後返回 `null` 以指示該功能已失敗。

仍在 `if` 塊內，將 `isError` 設置爲 `true` 並返回 `null`。

# --hints--

在 `alert` 之後，你應該將 `isError` 設置爲 `true`。
...
修改 `isError` 後，你應該 `return` 值 `null`。
