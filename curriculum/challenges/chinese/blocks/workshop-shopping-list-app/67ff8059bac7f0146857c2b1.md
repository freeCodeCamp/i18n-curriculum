...
如果你现在使用该应用，你会看到每次你查看或取消查看一项，或在搜索栏中输入时，`toggleItem()` 函数都会被重新创建。

为了提高性能，你可以使用 `useCallback()` 钩子来记忆或缓存 `toggleItem()` 函数。然后 React 只会在其依赖项发生变化时重新创建该函数。

首先，在你的 `file` 顶部从 React 中解构 `useCallback()` 钩子。

# --hints--

你应该在文件顶部从 `React` 中解构 `useCallback`。确保你没有移除任何你已经使用的其他 hooks。
