...
Se questa condizione è soddisfatta, disabilita il pulsante impostando la sua proprietà `disabled` su `true`. Inoltre, imposta la `textContent` del pulsante su `"No more data to load"`.
...
Dovresti impostare la proprietà `disabled` di `loadMoreBtn` su `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Dovresti impostare la `textContent` di `loadMoreBtn` su `"No more data to load"`.
