...
然后，在复选框 `input` 元素中，添加一个 `checked` 属性并将其设置为 `isChecked`。虽然不严格要求设置 `checked` 属性，但这样做是一个好习惯，以确保每个复选框都反映 `selectedItems` 数组的当前状态。

# --hints--

你应该为复选框 `input` 元素添加一个 `checked` 属性。
