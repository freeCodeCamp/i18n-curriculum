...
استخدم خاصية `transform` على منتقى `.one` من اجل (تدويره) `rotate` عكس اتجاه عقارب الساعة بمقدار 0.6 درجة.

# --hints--

يجب عليك تعيين الخاصية `transform` إلى `rotate(-0.6deg)`.
...
يجب أن يكون لعنصر `.one` الخاص بك `transform` بقيمة `rotate(-0.6deg)`.
