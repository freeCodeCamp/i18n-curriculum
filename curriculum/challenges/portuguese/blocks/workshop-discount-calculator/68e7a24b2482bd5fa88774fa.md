...
Agora, remova as seguintes linhas do final do seu código:
...
Você não deve ter a linha `print(product)` no seu código.
...
Você não deve ter a linha `discount = PercentageDiscount(10)` no seu código.
...
Você não deve ter a linha `print(discount.apply_discount(product))` no seu código.
...
Você não deve ter a linha `fixed_discount = FixedAmountDiscount(5)` no seu código.
...
Você não deve ter a linha `print(fixed_discount.apply_discount(product))` no seu código.
