...
看起來比較好了，但綠色標記的底邊需要更深，以增加一點層次感。

在相同的 `linear-gradient` 函式中，將紅色為 `11`、綠色為 `6C`、藍色為 `31` 的十六進位色碼添加為第三個顏色引數。

# --hints--

你的 `.green` CSS 規則應該有一個 `background` 屬性，設定為 `linear-gradient(180deg, #55680D, #71F53E, #116C31)`。
