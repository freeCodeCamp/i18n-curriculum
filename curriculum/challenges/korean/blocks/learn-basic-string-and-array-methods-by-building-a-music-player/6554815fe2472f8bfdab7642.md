...
`const`와 화살표 구문을 사용해 빈 `playPreviousSong` 함수를 만드세요.

# --hints--

빈 `const` 함수를 만들려면 `playPreviousSong`를 사용하세요.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong`는 함수여야 합니다.

```js
assert.isFunction(playPreviousSong)
```

`playPreviousSong` 함수는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

`playPreviousSong` 함수는 비어 있어야 합니다.
