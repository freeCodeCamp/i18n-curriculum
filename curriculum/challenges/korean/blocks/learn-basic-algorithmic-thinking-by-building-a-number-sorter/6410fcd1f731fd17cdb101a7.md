...
중첩된 `for` 반복문 안에 `console.log()` 호출을 추가하여 각 반복에서 `array`, `array[j]`, `array[minIndex]`의 값을 확인하세요. `Sort` 버튼을 클릭하여 알고리즘이 배열을 어떻게 탐색하는지 볼 수 있습니다.

그다음 `if` 문을 작성하여 `array[j]` 위치의 값이 `array[minIndex]` 위치의 값보다 작은지 확인하세요. 만약 그렇다면 `minIndex`를 `j`으로 설정하세요.

# --hints--

중첩된 `console.log()` 반복문 안에 `for` 호출이 있어야 합니다.
...
`array`에 첫 번째 인수로 `console.log()`을 전달해야 합니다.
...
`array[j]`에 두 번째 인수로 `console.log()`를 전달해야 합니다.
...
`array[minIndex]`에 세 번째 인수로 `console.log()`를 전달해야 합니다.
...
`if` 호출 뒤에 `console.log()` 문이 있어야 합니다.
...
`if` 문 안에 `array[j]`가 `array[minIndex]`보다 작은지 확인하는 조건이 있어야 합니다.
...
`if` 문은 `minIndex`를 `j`으로 설정해야 합니다.
