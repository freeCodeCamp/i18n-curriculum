...
當 `create_character` 被呼叫且第一個引數是 **非** 字串時，應該傳回 `The character name should be a string`。
...
當 `create_character` 被呼叫且第一個引數是字串時，不應該傳回 `The character name should be a string`。
...
當 `create_character` 被呼叫且第一個引數是 **非** 空字串時，不應該傳回 `The character should have a name`。
...
`create_character` 函式（程式）**不應該**在字元長度不超過 10 個字元時顯示字元過長的訊息。
...
當 `create_character` 被呼叫且第一個引數*不*包含空格時，不應該傳回 `The character name should not contain spaces`。
...
當 `create_character` 被呼叫且第二、第三或第四個引數是 **非** 整數時，應該傳回 `All stats should be integers`。
...
當 `create_character` 被呼叫且第二、第三和第四個引數皆為整數時，不應該傳回 `All stats should be integers`。
...
當 `create_character` 被呼叫且第二、第三和第四個引數皆不小於 `1` 時，不應該傳回 `All stats should be no less than 1`。
...
當 `create_character` 被呼叫且第二、第三和第四個引數皆不超過 `4` 時，不應該傳回 `All stats should be no more than 4`。
...
當 `create_character` 被呼叫且第二、第三或第四個引數的總和**不等於** `7` 時，應該傳回 `The character should start with 7 points`。
...
當 `create_character` 被呼叫且第二、第三和第四個引數加總為 `7` 時，不應該傳回 `The character should start with 7 points`。
