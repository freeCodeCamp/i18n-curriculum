...
마지막으로, 내부 함수를 수정하여 세 번째 함수를 반환하게 하세요. 이 세 번째 함수도 `quantity`라는 단일 인수를 받아야 하며, 함수 본문은 비워 두세요.

# --hints--

두 번째 함수는 또 다른 함수를 반환해야 합니다.

```js
assert.isFunction(convertQuantity()());
```

가장 안쪽 함수는 `quantity` 매개변수를 가져야 합니다.

```js
assert.match(convertQuantity()()?.toString(), /\(quantity\)/);
```

가장 안쪽 함수는 비어 있어야 합니다.
