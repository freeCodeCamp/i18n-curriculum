...
Тепер настав час протестувати кнопку паузи.

Додайте слухача події `"click"` до елемента `pauseButton`, а потім передайте `pauseSong` як другий аргумент слухача події. Це та функція, яку слухач події виконуватиме.

Перевірте свій застосунок, спочатку натиснувши на кнопку відтворення, а потім на кнопку паузи. Ви повинні побачити, що все працює як очікувалося.

# --hints--

Ви повинні викликати метод `addEventListener()` на вашій змінній `pauseButton`.
...
Ви повинні передати `pauseSong` як другий аргумент вашого методу `addEventListener`.
