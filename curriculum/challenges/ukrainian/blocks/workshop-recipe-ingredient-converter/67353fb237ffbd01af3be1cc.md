...
Нарешті ви можете почати підключати свою логіку до користувацького інтерфейсу. Вам потрібно буде зробити запит до DOM, щоб отримати кілька елементів за ідентифікатором. Отримайте елементи з ідентифікаторами `ingredient`, `quantity`, `unit`, `servings`, `recipe-form` та `result-list` і призначте їх відповідно змінним `ingredientName`, `ingredientQuantity`, `unitToConvert`, `numberOfServings`, `recipeForm` та `resultList`.

# --hints--

Ви повинні визначити змінну `ingredientName`.

```js
assert.isDefined(ingredientName);
```

Ваша змінна `ingredientName` має бути елементом, який повертається з `.getElementById()` за ідентифікатором `ingredient`.

```js
assert.strictEqual(ingredientName, document.getElementById('ingredient'));
```

Ви повинні визначити змінну `ingredientQuantity`.

```js
assert.isDefined(ingredientQuantity);
```

Ваша змінна `ingredientQuantity` має бути елементом, який повертається з `.getElementById()` за ідентифікатором `quantity`.

```js
assert.strictEqual(ingredientQuantity, document.getElementById('quantity'));
```

Ви повинні визначити змінну `unitToConvert`.

```js
assert.isDefined(unitToConvert);
```

Ваша змінна `unitToConvert` має бути елементом, який повертається з `.getElementById()` за ідентифікатором `unit`.

```js
assert.strictEqual(unitToConvert, document.getElementById('unit'));
```

Ви повинні визначити змінну `numberOfServings`.

```js
assert.isDefined(numberOfServings);
```

Ваша змінна `numberOfServings` має бути елементом, який повертається з `.getElementById()` за ідентифікатором `servings`.

```js
assert.strictEqual(numberOfServings, document.getElementById('servings'));
```

Ви повинні визначити змінну `recipeForm`.

```js
assert.isDefined(recipeForm);
```

Ваша змінна `recipeForm` має бути елементом, який повертається з `.getElementById()` за ідентифікатором `recipe-form`.

```js
assert.strictEqual(recipeForm, document.getElementById("recipe-form"));
```

Ви повинні визначити змінну `resultList`.

```js
assert.isDefined(resultList);
```

Ваша змінна `resultList` має бути елементом, який повертається з `.getElementById()` за ідентифікатором `result-list`.
