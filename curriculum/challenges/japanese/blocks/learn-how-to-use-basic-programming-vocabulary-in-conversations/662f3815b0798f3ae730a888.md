...
`classes'` は所有を示し、メソッドがクラスに属していることを表します。

動詞 `perform` は、一連の操作を実行する行為を指し、プログラミングで関数やメソッドが行う動作を説明する際によく使われます。
...
この所有格はメソッドがクラスに属していることを示します。特に語尾が「s」で終わる単語の後に所有や所属を表すために使われます。
...
この動詞は、何かを実行したり、遂行したり、行うことを意味します。
