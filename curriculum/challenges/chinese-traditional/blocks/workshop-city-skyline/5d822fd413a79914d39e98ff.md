...
接著，從 `.bb2a` 中移除 `width` 和 `height`，並將 `border-left` 和 `border-right` 改為使用 `5vw` 而非 `1vw`。該元素現在將沒有大小，邊框會在中間合併。
