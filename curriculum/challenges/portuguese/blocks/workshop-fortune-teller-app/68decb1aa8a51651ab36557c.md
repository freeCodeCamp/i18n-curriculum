...
Dentro da função `getElement`, use `querySelector` com os genéricos e armazene o resultado em uma variável. Adicione uma declaração `if` para verificar se o elemento não existe e, se for o caso, lance um erro com a mensagem `Element not found: ${error message here}`. Se o elemento existir, retorne-o.

# --hints--

Você deve declarar uma variável chamada `el` dentro da função `getElement`.
...
Sua variável `el` deve ser atribuída com o valor `document.querySelector<T>(selector)`.
...
Você deve ter uma instrução if que verifica se `el` não foi encontrado.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

Você deve lançar um novo Error com a mensagem `Element not found: ${selector}` dentro da sua declaração `if`.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

Você deve retornar a variável `el` dentro da função getElement.
