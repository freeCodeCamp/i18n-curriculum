...
이제 `renderSongs` 함수를 호출하고 `userData?.songs`을 전달해서 UI에 노래를 최종적으로 표시하세요.

선택적 체이닝(`?.`)은 null이거나 undefined일 수 있는 중첩된 속성에 접근할 때 오류를 방지하는 데 도움이 됩니다. 예를 들어:
...
`renderSongs`의 `songs` 속성을 사용해 `userData` 함수를 호출하세요. 이렇게 하면 재생 목록에 노래가 렌더링됩니다.
