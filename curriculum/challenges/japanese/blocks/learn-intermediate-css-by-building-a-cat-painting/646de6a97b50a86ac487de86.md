...
クラスセレクターを使って、`.cat-right-eye` 要素に幅 `30px` と高さ `40px` を指定してください。また、背景色を `#000` に設定してください。

# --hints--

`.cat-right-eye` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye'))
```

`.cat-right-eye` セレクターには `width` を `30px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.width === '30px')
```

`.cat-right-eye` セレクターには `height` を `40px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.height === '40px')
```

`.cat-right-eye` セレクターには `background-color` を `#000` に設定してください。
