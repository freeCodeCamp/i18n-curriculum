...
Movendo o texto do preço dentro do `p` e `span`, dê ao elemento `p` as classes `text-4xl`, `font-bold` e `text-gray-900` para que o preço fique mais visível.

# --hints--

Seu elemento `p` deve ter a classe `text-4xl`.
...
Seu elemento `p` deve ter a classe `font-bold`.
...
Seu elemento `p` deve ter a classe `text-gray-900`.
