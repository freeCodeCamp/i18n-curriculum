...
Ndani ya kipengele cha `.image-wrapper`, mpe kipengele chako cha kwanza cha `img` sifa ya kipengele ya `src` yenye thamani `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, sifa ya kipengele ya `alt` yenye thamani `image of the quote machine project`, sifa ya kipengele ya `class` yenye thamani `image-1`, sifa ya kipengele ya `loading` iliyowekwa kuwa `lazy`, sifa ya kipengele ya `width` yenye thamani `600`, na sifa ya kipengele ya `height` yenye thamani `400`.

# --hints--

Kipengele chako cha kwanza cha `img` kinapaswa kuwa na sifa ya kipengele `src` iliyowekwa kuwa `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`.
...
Kipengele chako cha kwanza cha `img` kinapaswa kuwa na sifa ya kipengele `alt` iliyowekwa kuwa `image of the quote machine project`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('alt') === 'image of the quote machine project');
```

Kipengele chako cha kwanza cha `img` kinapaswa kuwa na sifa ya kipengele `class` iliyowekwa kuwa `image-1`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.classList?.contains('image-1'));
```

Kipengele chako cha kwanza cha `img` kinapaswa kuwa na sifa ya kipengele `loading` iliyowekwa kuwa `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('loading') === 'lazy');
```

Kipengele chako cha kwanza cha `img` kinapaswa kuwa na sifa ya kipengele `width` iliyowekwa kuwa `600`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('width') === '600');
```

Kipengele chako cha kwanza cha `img` kinapaswa kuwa na sifa ya kipengele `height` iliyowekwa kuwa `400`.
