...
다른 텍스트가 제자리를 벗어났습니다. `.first-paragraph::first-letter` 선택자에 `float` 속성을 `left`로, `margin-right` 속성을 `1rem`로 설정하여 위치를 맞추세요.

# --hints--

`.first-paragraph::first-letter` 선택자에 `float` 속성을 `left`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

`.first-paragraph::first-letter` 선택자에 `margin-right` 속성을 `1rem`로 설정하세요.
