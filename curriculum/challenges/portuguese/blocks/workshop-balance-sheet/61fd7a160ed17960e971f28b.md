...
In your fourth `tr` element, add a `th` element with the text `Total Assets`. Envolva o texto `Assets` em um elemento `span` com o atributo `class` definido como `sr-only`.
