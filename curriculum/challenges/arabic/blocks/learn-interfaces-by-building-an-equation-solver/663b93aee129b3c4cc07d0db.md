...
بعد قائمة `terms` مباشرة، أنشئ حلقة `for` واستخدم الطريقة `.items()` للتكرار على المفاتيح والقيم المخزنة في الخاصية `coefficients`. استخدم `n` و `coefficient` كمتغيرات للحلقة.

داخل الحلقة، أنشئ جملة `if` تتحقق مما إذا كانت قيمة المعامل في التكرار الحالي قيمة خاطئة وتخطي التكرار في هذه الحالة. ذلك لأنك لا تريد تمثيل المعاملات التي قيمتها صفر.

# --hints--

يجب أن تنشئ حلقة `for` تتكرر على `coefficients.items()`.
...
يجب أن تستخدم حلقة `for` المتغيرين `n` و `coefficient` للتكرار على `coefficients.items()`.
...
يجب أن تنشئ جملة `if` للتحقق مما إذا كانت `coefficient` قيمة خاطئة داخل حلقة `for` الخاصة بك.
...
يجب أن تستخدم الكلمة المفتاحية `continue` داخل جملة `if` الجديدة.
...
يجب أن توضع حلقة `for` الخاصة بك مباشرة بعد الإعلان عن `terms`.
