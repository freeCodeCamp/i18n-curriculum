...
이제 마무리로, 클래스 외부에 작성한 코드를 수정하여 속도, 높이, 각도에 원하는 값을 입력받아 발사체의 세부 정보, 좌표 표, 궤적 그래프를 순서대로 터미널에 출력하는 `projectile_helper`라는 작은 유틸리티 함수에 통합하세요.

`projectile_helper`을 한 번 호출하여 원하는 값을 전달하세요.

# --hints--

글로벌 범위에 `projectile_helper`를 만드세요.
...
요청된 문자열을 순서대로 출력하세요. 다른 값은 출력하지 마세요.
...
`projectile_helper` 함수를 최소 한 번 호출하세요.
