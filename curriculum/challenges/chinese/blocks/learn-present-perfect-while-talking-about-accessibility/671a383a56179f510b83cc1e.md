...
Tom 的响应，`We haven't been receiving as many negative comments`，使用了否定形式的 `Present Perfect Continuous`。这种时态由 `haven't/hasn't + been + verb ending in -ing` 组成，用于描述在一段时间内未发生的动作。在这种情况下，它表明随着时间推移收到的负面评论减少，可能表示有所改善。
