...
JSON (JavaScript Object Notation) es un formato estándar para compartir datos entre sistemas. `JSON.stringify` convierte un valor de JavaScript en una cadena con formato JSON.

Comienza eliminando las dos llamadas `validateEntry` console.log, ya que solo se usaron para pruebas. Luego crea una función llamada `exportToJSON` que reciba `catalog` como parámetro. Devuelve `JSON.stringify(catalog, null, 2)`. El primer argumento es el dato a convertir. El segundo argumento (`null`) significa que no hay filtrado personalizado. El tercero (`2`) añade una sangría de dos espacios, haciendo que la salida sea legible.

Debajo de la función, registra `exportToJSON(catalog.slice(0, 2))` para previsualizar la salida JSON de las dos primeras entradas.

# --hints--

Debes eliminar las llamadas `validateEntry` console.log.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry/);
```

`exportToJSON` debe devolver una cadena.

```js
assert.isString(exportToJSON(catalog));
```

`JSON.parse(exportToJSON(catalog))` debe producir un arreglo válido.

```js
assert.isArray(JSON.parse(exportToJSON(catalog)));
```

Debes usar `JSON.stringify` con `null` y `2` como segundo y tercer argumento.

```js
assert.match(exportToJSON.toString(), /JSON\.stringify\s*\(\s*catalog\s*,\s*null\s*,\s*2\s*\)/);
```

Debes registrar `exportToJSON(catalog.slice(0, 2))` en la consola.
