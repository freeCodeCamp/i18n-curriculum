...
`else` ブロックを追加してください。`else` ブロックの中で、現在再生中の曲の `playSong` を引数として `id` 関数を呼び出してください。

これにより、再生ボタンがクリックされたときに現在再生中の曲が再生され続けます。

# --hints--

`playButton` がクリックされ、かつ `userData.currentSong` が `null` でない場合、現在再生中の曲の `playSong` を引数として `id` 関数を呼び出す必要があります。
...
`playSong` がクリックされ、かつ `id` が `playButton` の場合でも、プレイリストの最初の曲の `userData.currentSong` を引数として `null` 関数を呼び出す必要があります。
