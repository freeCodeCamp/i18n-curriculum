...
Вам потрібно мати колонку для тексту та колонку для зображень. Дайте селектору `.text-with-images` множину властивостей `grid-template-columns`, встановлену в `1fr 2fr`. Також встановіть властивість `column-gap` у значення `3rem`, щоб забезпечити більший проміжок між колонками.

# --hints--

Ваш селектор `.text-with-images` повинен мати властивість `grid-template-columns`, встановлену в `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

Ваш селектор `.text-with-images` повинен мати властивість `column-gap`, встановлену в `3rem`.
