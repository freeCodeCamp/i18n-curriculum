...
Dovresti dare a `.bb4-window` una proprietà `width` del `18%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

Dovresti dare a `.bb4-window` una proprietà `height` del `90%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

Dovresti dare a `.bb4-window` una proprietà `background-color` con il valore `--window-color4`.
