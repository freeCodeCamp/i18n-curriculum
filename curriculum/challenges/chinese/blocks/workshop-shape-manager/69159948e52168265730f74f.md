...
你需要一种安全访问 select 元素并防止 `null` 值的方法。为此，使用 `val` 关键字创建一个值为 `e.currentTarget` 且类型为 `HTMLSelectElement` 的 `as` 常量。

在该常量下，使用一个 `if` 语句通过取反来检查 `val` 是否为假值。在 `if` 块中，返回 `target value not found`。
