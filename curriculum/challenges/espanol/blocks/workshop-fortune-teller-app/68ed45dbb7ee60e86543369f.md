...
Después de obtener los datos, guarda la respuesta en una variable y conviértela a JSON. Asigna el tipo `Deck` a los datos convertidos y luego asígnalos a la propiedad `cards`. Dentro del bloque `catch`, asegúrate de registrar el error con este mensaje: `"Error fetching cards:", error`.

# --hints--

Debes declarar una variable llamada `data` dentro de `fetchCardsData`.
...
Tu variable `data` debe tener el tipo `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

Tu variable `data` debe asignarse convirtiendo la respuesta a JSON.
...
Debes almacenar `data.cards` en `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Tu bloque `catch` debe usar `console.error()` para registrar el error con el mensaje `"Error fetching cards:", error`.
