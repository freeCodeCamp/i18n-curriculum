...
使用三元运算符将 `.hasOwnProperty()` 调用转换为条件。 如果对象具有该属性，则返回以 `fn` 和 `args` 作为参数调用 `apply` 的结果。 否则，返回 `match`。
...
如果三元条件为真，则回调函数应该返回调用 `apply()` 的结果。
