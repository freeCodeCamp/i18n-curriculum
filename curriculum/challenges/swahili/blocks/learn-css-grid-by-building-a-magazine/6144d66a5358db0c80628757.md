...
Tengeneza kichaguzi cha `html` na umpe sifa ya `font-size` iliyowekwa kuwa `62.5%`. Hii itaweka ukubwa wa fonti chaguo-msingi kwa ukurasa wako wa mtandao kuwa 10px (chaguo-msingi cha kivinjari ni 16px).

Hii itakuwezesha kufanya kazi kwa urahisi na vipimo vya `rem` baadaye, kwani `2rem` itakuwa 20px.

# --hints--

Unapaswa kutengeneza kichaguzi cha `html`.

```js
assert(new __helpers.CSSHelp(document).getStyle('html'));
```

Kichaguzi chako cha `html` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `62.5%`.
