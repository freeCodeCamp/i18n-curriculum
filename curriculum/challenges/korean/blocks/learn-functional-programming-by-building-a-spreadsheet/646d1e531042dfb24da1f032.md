...
`.replace()` 메서드의 두 번째 인수는 문자열일 필요가 없습니다. 대신 일치한 문자열에 대해 더 복잡한 로직을 실행하는 콜백 함수를 전달할 수 있습니다.

콜백 함수는 몇 가지 매개변수를 받습니다. 첫 번째는 일치한 문자열입니다. `.replace()` 호출에 빈 콜백 함수를 전달하고 `match` 매개변수를 주세요.

# --hints--

`.replace()` 메서드의 두 번째 인수로 화살표 함수를 전달해야 합니다.
