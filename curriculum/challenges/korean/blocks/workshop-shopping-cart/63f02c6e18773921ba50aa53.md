...
컴퓨터가 숫자를 저장하고 처리하는 방식 때문에, 소수 계산에서 이상한 동작이 발생할 수 있습니다. 예를 들어, `0.1 + 0.2`는 `0.3`와 같지 않습니다. 이는 컴퓨터가 소수를 이진 분수로 저장하는데, 일부 이진 분수는 정확히 소수 분수로 나타낼 수 없기 때문입니다.

계산 결과 숫자를 정리하고 싶습니다. 계산식을 소괄호로 감싸세요(`return` 문장은 포함하지 마세요!) 그리고 그 위에 `.toFixed()` 메서드를 호출하세요. `.toFixed()` 메서드에 숫자 `2`를 인수로 전달하세요. 이렇게 하면 숫자가 소수점 둘째 자리까지 반올림되고 문자열로 반환됩니다.
...
`.toFixed()` 메서드에 숫자 `2`를 인수로 전달해야 합니다.
