...
`const` とアロー構文を使って、`shuffle` という空の関数を作成します。

この関数はプレイリストの曲をシャッフルし、シャッフル後に必要な状態管理の更新を行う役割があります。

# --hints--

`const` を使って、`shuffle` という名前の空の関数を作成してください。

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

`shuffle` は関数である必要があります。

```js
assert.isFunction(shuffle)
```

`shuffle` 関数はアロー構文を使い、パラメータを取らないようにしてください。

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

`shuffle` 関数は空である必要があります。
