...
4 つ目の `input` 要素に `name` 属性を加え、その値を 3 つ目の `input` の `name` 属性の値と一致させる必要があります。
...
1 つ目の `input`と 3 つ目の `input` は、`name` 属性に異なる値を設定する必要があります。
