...
`cartSubTotal` と同じパターンに従って、`cartTaxes` を更新し `tax` の値を表示し、`cartTotal` を更新して `total` プロパティを表示してください。

# --hints--

`textContent` 要素の `cartTaxes` プロパティにアクセスしてください。

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

`.toFixed()` 変数の `tax` メソッドを呼び出し、`2` を引数として渡してください。

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

テンプレートリテラル構文を使って、`.toFixed()` の呼び出しの前にドル記号を追加してください。
...
`textContent` 要素の `cartTaxes` を `tax.toFixed` テンプレート文字列に設定してください。
...
`textContent` 要素の `cartTotal` プロパティにアクセスしてください。

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

`.toFixed()` 変数の `total` メソッドを呼び出し、`2` を引数として渡してください。`this` キーワードを使うことを忘れないでください。
...
`textContent` 要素の `cartTotal` を `total.toFixed` テンプレート文字列に設定してください。
