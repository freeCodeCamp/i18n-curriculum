...
title: 定義 HTML `document` 的 Head 和 Body
challengeType: 0
...
你可以在你的 HTML `document` 中的 `html` 標記內，使用 `head` 和 `body` 元素添加另一個組織層級。任何關於你的頁面資訊的標記都會放入 `head` 頁籤。然後，任何包含頁面內容（顯示給使用者看的內容）的標記都會放入 `body` 頁籤。

詮釋資料元素，例如 `link`、`meta`、`title` 和 `style`，通常放在 `head` 元素內。
...
編輯標記，使其包含 `head` 和 `body`。`head` 元素應該只包含 `title`，而 `body` 元素應該只包含 `h1` 和 `p`。
