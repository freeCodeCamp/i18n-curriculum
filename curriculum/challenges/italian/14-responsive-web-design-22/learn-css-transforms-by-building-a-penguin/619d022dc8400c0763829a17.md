...
Dovresi usare il selettore `.face.left`.

```js
assert.match(code, /\.face\.left\s*\{/);
```

Dovresti dare a `.face.left` una proprietà `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face.left')?.left);
```

Dovresti dare a `.face.left` una proprietà `left` con il valore `--fcc-expected--` invece di `--fcc-actual--`.
