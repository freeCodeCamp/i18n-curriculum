...
Adicione um bloco `else` para lidar com o tempo atual de reprodução da música. Isso permite que você retome a música atual no ponto em que foi pausada.

Dentro do bloco `else`, defina a propriedade `currentTime` do objeto `audio` com o valor armazenado em `userData.songCurrentTime`.

# --hints--

Sua função `playSong` deve definir a propriedade `currentTime` do objeto `audio` para o valor armazenado em `userData.songCurrentTime` quando houver uma música atual tocando.
