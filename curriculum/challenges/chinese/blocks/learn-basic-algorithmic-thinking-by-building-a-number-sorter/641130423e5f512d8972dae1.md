...
注意数字 `10` 如何放置在数组的开始位置。 这是因为 `.sort()` 的默认行为是将数字值转换为字符串，并按字母顺序排序。 而字母顺序上，`"10"` 在 `"2"` 前面。

为了解决这个问题，你可以将回调函数传递到 `.sort()` 方法。 回调函数有两个参数 - 对于你的函数，使用 `a` 和 `b`。 `a` 和 `b` 的参数表示数组中将要排序的数字值。

函数体暂时保留为空。

# --hints--

你应该将回调函数传递给 `.sort()` 方法。 记得使用箭头语法。
