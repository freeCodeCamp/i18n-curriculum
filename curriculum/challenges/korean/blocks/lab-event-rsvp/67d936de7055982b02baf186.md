...
1. 이름, 이메일, 참석자 수, 식단 선호도, 추가 손님 동반 여부를 나타내는 옵션이 포함된 폼을 만들어야 합니다.

2. 이름을 입력하는 텍스트 입력 필드가 있어야 합니다 (필수).

3. 이메일 주소를 입력하는 이메일 입력 필드가 있어야 합니다 (필수). 폼은 올바른 이메일 형식인지 검증해야 합니다.

4. 참석자 수를 입력하는 숫자 입력 필드가 있어야 합니다 (필수), 숫자는 1보다 작을 수 없습니다.

5. 식단 선호도를 입력하는 텍스트 입력 필드가 있어야 하며, 이 정보는 선택 사항입니다.

6. 추가 손님을 데려오는지 여부를 나타내는 체크박스를 선택하거나 해제할 수 있어야 합니다.

7. 폼을 제출하는 버튼이 있어야 하며, 제출 시 페이지가 새로고침되지 않아야 합니다.

8. 제출 후 폼 아래에 확인 메시지가 표시되어야 하며, 입력한 세부 정보(이름, 이메일, 참석자 수, 식단 선호도, 선택적 추가 손님)가 보여야 합니다.

폼 제출 후 예시 메시지는 다음과 같습니다:
...
`input` 타입이 `text`인 요소가 두 개 있어야 합니다.
...
`input` 타입이 `email`인 요소가 하나 있어야 합니다.
...
`input` 타입이 `number`인 요소가 하나 있어야 합니다.
...
`input` 타입이 `checkbox`인 요소가 하나 있어야 합니다.
...
`input[type="number"]` 요소는 필수여야 합니다.
...
`input[type="number"]` 요소는 1보다 작은 값을 허용하지 않아야 합니다.
...
`input[type="text"]` 요소의 값이 변경되면 컴포넌트 상태가 업데이트되어야 합니다.
...
`input[type="email"]` 요소의 값이 변경되면 컴포넌트 상태가 업데이트되어야 합니다.
...
`input[type="number"]` 요소의 값이 변경되면 컴포넌트 상태가 업데이트되어야 합니다.
...
`input[type="checkbox"]` 요소의 값이 변경되면 컴포넌트 상태가 업데이트되어야 합니다.
...
제출 후 페이지에 이름 `input` 상태를 나타내는 요소가 있어야 합니다.
...
제출 후 페이지에 이메일 `input` 상태를 나타내는 요소가 있어야 합니다.
...
제출 후 페이지에 참석자 수 `input` 상태를 나타내는 요소가 있어야 합니다.
...
제출 후 페이지에 식단 선호도 `input` 상태를 나타내는 요소가 있어야 합니다.
...
제출 후 페이지에 추가 손님 `input` 상태를 나타내는 요소가 있어야 합니다.
