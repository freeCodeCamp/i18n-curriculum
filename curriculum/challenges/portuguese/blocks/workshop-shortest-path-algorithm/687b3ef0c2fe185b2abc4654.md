...
Após o término do loop externo `for _ in range(n):`, crie uma variável chamada `targets`. Use uma expressão condicional para atribuir a ela `[target_node]` se `target_node` não for `None`, caso contrário atribua `list(range(n))`.
