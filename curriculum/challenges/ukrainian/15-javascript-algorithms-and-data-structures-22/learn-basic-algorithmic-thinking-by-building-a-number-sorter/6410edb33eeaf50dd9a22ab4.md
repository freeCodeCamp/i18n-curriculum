...
Якщо умова `if` вірна, потрібно поміняти місцями два елементи, «спливати» більший елемент до кінця масиву.

Щоб зробити це оголосіть змінну `temp` та присвойте до неї значення `array[j]`. Потім присвоїте `array[j]` значення `array[j + 1]`. Наприкінці присвоїте `array[j + 1]` значення `temp`.

# --hints--

Оголосіть змінну `temp` в межах інструкції `if`.
...
Призначте значення `array[j]` до `temp`.
...
Призначте значення `array[j + 1]` до `array[j]`.
...
Призначте значення `temp` до `array[j + 1]`.
