...
在你的巢狀 `for` 迴圈之後，你已經找到最小的值。你需要將它與你目前的值交換。

就像你在泡沫排序中所做的，使用一個 `temp` 變數來取出 `array[i]` 的值，然後交換 `array[i]` 和 `array[minIndex]` 的值。

# --hints--

在你的巢狀 `for` 迴圈之後，你應該宣告一個 `temp` 變數。
...
你應該將 `array[minIndex]` 指定到 `array[i]`。
