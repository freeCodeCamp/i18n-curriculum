...
`textContent`는 노드의 텍스트를 설정하며 HTML 요소의 텍스트 내용을 설정하거나 가져올 수 있게 합니다.
...
삼항 연산자를 사용해 조건부로 텍스트 내용 값을 설정할 수 있습니다. 다음은 삼항 연산자의 결과를 변수에 할당하는 예시입니다:

```js
const example = condition ? "I'm true" : "I'm false";
```

삼항 연산자를 사용해 `currentTitle`이 참 같은 값인지 확인하세요. 참이면 `playingSong.textContent`를 `currentTitle`으로 설정하세요. 그렇지 않으면 빈 문자열로 설정하세요.

그 아래에서는 `ternary` 연산자를 사용해 `currentArtist`가 참 같은 값인지 확인하세요. 참이면 `songArtist.textContent`를 `currentArtist`으로 설정하세요. 그렇지 않으면 빈 문자열로 설정하세요.

# --hints--

`textContent` 속성을 `playingSong`에 연결해야 합니다.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

`ternary` 연산자를 사용해 `textContent`의 `playingSong` 속성을 `currentTitle` 또는 `""`로 설정해야 합니다.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

`textContent` 속성을 `songArtist`에 연결해야 합니다.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

`ternary` 연산자를 사용해 `textContent`의 `songArtist` 속성을 `currentArtist` 또는 `""`로 설정해야 합니다.
