...
Aktualisieren Sie schließlich die `display`-Eigenschaft des `style`-Objekts der `cartContainer`-Variable zu einem weiteren ternären Ausdruck, der prüft, ob `isCartShowing` wahr ist. Wenn ja, setzen Sie die `display`-Eigenschaft auf `"block"`, andernfalls setzen Sie sie auf `"none"`.

Jetzt sollten Sie Ihren Warenkorb sehen und Artikel hinzufügen können.

# --hints--

Sie sollten auf die `display`-Eigenschaft der `style`-Eigenschaft der `cartContainer`-Variable zugreifen.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display/)
```

Sie sollten den Zuweisungsoperator auf die `display`-Eigenschaft der `style`-Eigenschaft der `cartContainer`-Variable anwenden.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*/)
```

Sie sollten die ternäre Syntax verwenden, um zu prüfen, ob `isCartShowing` wahr ist.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*isCartShowing\s*\?\s*/)
```

Sie sollten die `display`-Eigenschaft der `style`-Eigenschaft der `cartContainer`-Variable auf `"block"` setzen, wenn `isCartShowing` wahr ist.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*isCartShowing\s*\?\s*('|"|`)block\1\s*:\s*/)
```

Sie sollten die `display`-Eigenschaft der `style`-Eigenschaft der `cartContainer`-Variable auf `"none"` setzen, wenn `isCartShowing` falsch ist.
