...
Declara una variable llamada `cardId` y asígnale el valor del atributo `data-id` de `cardElement` usando el método `getAttribute()`.

# --hints--

Debes declarar una variable llamada `cardId` dentro de `showFortune`.
...
Tu variable `cardId` debe asignarse llamando a `cardElement.getAttribute("data-id")`.
...
Debes usar el método `getAttribute` en `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

Debes pasar `'data-id'` a `getAttribute`.
