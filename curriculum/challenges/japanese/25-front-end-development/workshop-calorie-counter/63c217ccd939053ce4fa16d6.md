...
`label` 要素の後、テンプレート文字列内の新しい行に、`input` 要素を作成してください。 Give it a `type` attribute set to `text`, a `placeholder` attribute set to `Name`, and an `id` attribute that matches the `for` attribute of your `label` element.
