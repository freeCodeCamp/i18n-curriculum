...
左の内側の目を、`position` プロパティに `absolute`、`top` に `8px`、`left` に `2px` を設定して位置を調整してください。また、ボーダー半径を `60%` にし、`10` 度回転させてください。

# --hints--

`.cat-left-inner-eye` セレクターには、`position` プロパティを `absolute` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.position === 'absolute')
```

`.cat-left-inner-eye` セレクターには、`top` プロパティを `8px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.top === '8px')
```

`.cat-left-inner-eye` セレクターには、`left` プロパティを `2px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.left === '2px')
```

`.cat-left-inner-eye` セレクターには、`border-radius` プロパティを `60%` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.borderRadius === '60%')
```

`.cat-left-inner-eye` セレクターには、`transform` プロパティを `10deg` に設定してください。
