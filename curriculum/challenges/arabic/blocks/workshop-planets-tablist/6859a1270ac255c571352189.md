...
حان الوقت الآن لإنشاء الترميز لكل لوحة ستعرض الحقائق عن كل كوكب.

ابدأ بإضافة ثلاثة عناصر `div` لكل منها خاصية `role` مضبوطة على `"tabpanel"`. داخل كل عنصر `div` يجب أن يكون هناك عنصر `p`.

# --hints--

يجب أن يكون لديك ثلاثة عناصر `div` بخاصية `role` مضبوطة على `"tabpanel"`.

```js
assert.lengthOf(document.querySelectorAll('div[role="tabpanel"]'), 3);
```

يجب أن يحتوي كل عنصر `tabpanel` على عنصر `p` بداخله.
