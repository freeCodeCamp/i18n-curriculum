...
創建一個 `img` 選擇器，並為它設定 `width` 屬性為 `100%`，以及 `object-fit` 屬性為 `cover`。

`object-fit` 屬性告訴瀏覽器如何在其容器內定位元素。在這種情況下，`cover` 將設定影像填滿容器，並在不改變長寬比的情況下進行裁切。
...
你的 `img` 選擇器應該有一個設定為 `cover` 的 `object-fit` 屬性。
