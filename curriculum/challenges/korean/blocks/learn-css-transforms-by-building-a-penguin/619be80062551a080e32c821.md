...
가상 요소의 아래쪽 모서리에 `100%` 반경을 주어 꼭대기를 둥글게 만들고, 위쪽 모서리는 `0%`로 유지하세요.

# --hints--

꼭대기를 둥글게 만들려면 `border-radius` 속성을 사용하세요.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

`.penguin-body::before`에 `border-radius`의 `0% 0% 100% 100%`를 주어야 합니다.
