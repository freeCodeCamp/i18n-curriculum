...
An dieser Stelle möchten Sie nur Ergebnisse für Knoten anzeigen, die nicht der Startknoten sind und von diesem aus erreichbar sind.

Fügen Sie eine Bedingung hinzu, die überprüft, ob `node_no` gleich `start_node` **oder** ob `distances[node_no]` gleich `INF` ist.

Wenn eine der beiden Bedingungen zutrifft, verwenden Sie `continue`, um zur nächsten Iteration der Schleife zu springen.

# --hints--

Sie sollten eine Bedingung innerhalb der Schleife hinzufügen.
...
Ihre `if`-Anweisung sollte prüfen, ob `node_no` gleich `start_node` *oder* ob `distances[node_no]` gleich `INF` ist.
...
Sie sollten `continue` verwenden, um zur nächsten Iteration der Schleife zu springen.
