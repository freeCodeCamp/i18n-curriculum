...
Now create an image tag and give it the `class` `"user-img"`. Use a interpolação de strings para definir o atributo `src` como sendo a `image` que você desestruturou anteriormente. Set the `alt` attribute to `author` followed by the text `"avatar"`. Make sure there is a space between the `author` variable and the word `"avatar"`, for example, `"Quincy Larson avatar"`.
