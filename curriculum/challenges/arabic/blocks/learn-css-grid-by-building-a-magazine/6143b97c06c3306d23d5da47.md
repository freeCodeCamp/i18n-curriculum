...
داخل عنصر `article` الخاص بك، أنشئ عنصر `h3` مع تعيين `class` إلى `list-title` والنص `A Brief History`. أسفله، أنشئ عنصر `p` بالنص `Of the Curriculum`. ثم أنشئ عنصر `ul` مع الصنف `lists`.

# --hints--

يجب أن تنشئ عنصر `h3` داخل عنصر `article` الخاص بك.

```js
assert.exists(document.querySelector('article h3'));
```

يجب أن تنشئ عنصر `p` داخل عنصر `article` الخاص بك.

```js
assert.exists(document.querySelector('article p'));
```

يجب أن تنشئ عنصر `ul` داخل عنصر `article` الخاص بك.

```js
assert.exists(document.querySelector('article ul'));
```

يجب أن تكون العناصر داخل عنصر `article` بالترتيب الصحيح.
...
يجب أن يحتوي عنصر `h3` الجديد على `class` معين إلى `list-title`.

```js
assert(document.querySelector('article h3')?.className === 'list-title');
```

يجب أن يحتوي عنصر `h3` الجديد على النص `A Brief History`.

```js
assert(document.querySelector('article h3')?.innerText === 'A Brief History');
```

يجب أن يحتوي عنصر `p` الجديد على النص `Of the Curriculum`.

```js
assert(document.querySelector('article p')?.innerText === 'Of the Curriculum');
```

يجب أن يحتوي عنصر `ul` الجديد على `class` معين إلى `lists`.
