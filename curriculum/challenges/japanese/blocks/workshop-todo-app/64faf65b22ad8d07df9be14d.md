...
まず削除したいタスクのインデックスを見つける必要があります。

`dataArrIndex` 変数を作成し、`findIndex()` 配列の `taskData` メソッドを使って値を設定してください。アロー関数のコールバックには `item` をパラメータとして渡し、コールバック内で `id` の `item` が `id` の `parentElement` の `buttonEl` と等しいかどうかを確認してください。

# --hints--

`deleteTask` 関数は変更しないでください。
...
`item` のアロー関数コールバックのパラメータとして `findIndex()` を渡してください。

```js
assert.match(code, /const\s+deleteTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)/)
```

アロー関数のコールバックでは `item.id === buttonEl.parentElement.id` をチェックしてください。波括弧は使わないでください。
