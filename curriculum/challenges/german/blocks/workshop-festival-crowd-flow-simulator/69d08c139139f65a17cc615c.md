...
Innerhalb Ihrer `while`-Schleife:

- Protokollieren Sie die Nachricht `"\nTick " + (tickIndex + 1)`.
- Durchlaufen Sie jedes Tor im `gates`-Array und rufen Sie `handleGateAtTick()` mit diesen Argumenten auf:
  - `gates`: das vollständige Eingabe-Array der Tore.
  - `gate`: das aktuelle Tor-Objekt.
  - `tickIndex`: den aktuellen Tick.
  - `throughputSummary`: das lokale Objekt, das die insgesamt verarbeiteten Teilnehmer pro Tor verfolgt.
- Erhöhen Sie `tickIndex` um 1, um zum nächsten Tick zu wechseln.

# --hints--

Sie sollten die folgende Nachricht in der Konsole protokollieren: `"\nTick " + (tickIndex + 1)`.
...
Nach dem Protokollieren in der Konsole sollten Sie jedes Tor im `gates`-Array durchlaufen und `handleGateAtTick()` mit `gates`, `gate`, `tickIndex` und `throughputSummary` aufrufen.
...
Nach Ihrer Schleife durch das `gates`-Array sollten Sie `tickIndex` um 1 erhöhen.
