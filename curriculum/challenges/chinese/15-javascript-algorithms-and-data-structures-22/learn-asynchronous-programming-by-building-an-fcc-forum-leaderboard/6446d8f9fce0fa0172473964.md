...
`allCategories` 对象的其余部分已经为你写好了。

在接下来的几个步骤中，你将创建一个函数，用于从 `allCategories` 对象中检索类别名称。

首先创建一个名为 `forumCategory` 的箭头函数，并将 `id` 作为参数名。

# --hints--

你需要创建一个名为 `forumCategory` 的空函数。

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

你应该在 `forumCategory` 函数中有一个名为 `id` 的参数。
