...
Jetzt müssen Sie einige Informationen über den Autor anzeigen. Zeigen Sie zuerst den Namen des Autors an.

Erstellen Sie ein `h2`-Tag mit dem `class` `"author-name"`. Dann interpolieren Sie `author` innerhalb des `h2`-Tags. Dies ist der Name des Autors.
...
Ihr `h2`-Element sollte eine Klasse mit dem Wert `"author-name"` haben.
...
Sie sollten `author` in Ihrem `h2`-Tag interpolieren, um die Namen der Autoren anzuzeigen.
