...
Poiché si tratta di un quiz, ti serve un modulo per permettere agli utenti di inviare le risposte. Puoi separare semanticamente il contenuto all’interno del modulo usando elementi `section`.

All’interno dell’elemento `main`, crea un modulo con tre elementi `section` annidati. Poi, fai in modo che il modulo venga inviato a `https://freecodecamp.org/practice-project/accessibility-quiz`, usando il metodo corretto.
...
Dovresti dare all’attributo `action` un valore `https://freecodecamp.org/practice-project/accessibility-quiz`.

```js
assert.equal(document.querySelector('main > form')?.action, 'https://freecodecamp.org/practice-project/accessibility-quiz');
```

Dovresti dare all’elemento `form` un attributo `method`.

```js
assert.notEmpty(document.querySelector('main > form')?.method);
```

Dovresti dare all’elemento `form` un attributo `method` con valore `post`.
