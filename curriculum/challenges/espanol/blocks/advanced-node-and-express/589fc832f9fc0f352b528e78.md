...
¡Ahora tu cliente tendrá toda la información necesaria para mostrar correctamente el conteo actual de usuarios y anunciar cuando un usuario se conecta o desconecta! Para manejar este evento en el lado del cliente, debemos escuchar `'user'`, luego actualizar el conteo actual de usuarios usando jQuery para cambiar el texto de `#num-users` a `'{NUMBER} users online'`, así como agregar un `<li>` a la lista desordenada con id `messages` con `'{NAME} has {joined/left} the chat.'`.
...
Envía tu página cuando creas que está correcta. Si tienes errores, puedes consultar <a href="https://forum.freecodecamp.org/t/advanced-node-and-express/567135/3#announce-new-users-10" target="_blank" rel="noopener noreferrer nofollow">el proyecto completado hasta este punto</a>.
