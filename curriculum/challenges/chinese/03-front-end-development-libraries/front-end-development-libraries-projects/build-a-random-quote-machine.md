...
**目标：** 构建一个功能类似于此的应用程序：<a href="https://random-quote-machine.freecodecamp.rocks/" target="_blank" rel="noopener noreferrer nofollow">https://random-quote-machine.freecodecamp.rocks/</a>。
...
当你完成了本项目并且该项目所有测试运行通过，请提交项目的 URL。

**注意：** Twitter 不允许在 iframe 中加载链接。 如果你的推文无法加载，请尝试在 `#tweet-quote` 元素上使用 `target="_blank"` 或 `target="_top"` 属性。 `target="_top"` 将替换当前选项卡，因此请确保你的工作已保存。
