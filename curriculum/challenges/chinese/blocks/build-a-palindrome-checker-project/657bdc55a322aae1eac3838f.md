...
**目标：** 构建一个功能上类似于 <a href="https://palindrome-checker.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://palindrome-checker.freecodecamp.rocks</a> 的应用程序。
...
1. 当 `#text-input` 元素包含文本 `1 eye for of 1 eye.`，并且 `#check-btn` 元素被点击，`#result` 元素应该包含文本 `1 eye for of 1 eye. is not a palindrome`。
1. 当 `#text-input` 元素包含文本 `0_0 (: /-\ :) 0-0`，并且 `#check-btn` 元素被点击时，`#result` 元素应该包含文本 `0_0 (: /-\ :) 0-0 is a palindrome`。
1. 当 `#text-input` 元素包含文本 `five|\_/|four`，并且 `#check-btn` 元素被点击，`#result` 元素应该包含文本 `five|\_/|four is not a palindrome`。
