...
在 `map` 内，通过传入 `renderCard`、`type`、`isReversed` 和 `name_short` 作为参数，返回 `img` 函数，并在 `join` 上添加一个带空字串的 `map` 方法。
