---
id: 67acb8285b01d71220db1208
title: المهمة 25
challengeType: 22
dashedName: task-25
lang: en-US
---

<!-- (Audio) Bob: True, but good office furniture can help prevent health issues that result from bad posture and make remote work more enjoyable. -->

# --instructions--

استمع إلى الصوت وأكمل الجملة أدناه.

# --fillInTheBlank--

## --sentence--

`True, but good office furniture can help prevent health issues that result from BLANK BLANK and make remote work more enjoyable.`

## --blanks--

`bad`

### --feedback--

تعني هذه الكلمة سيء أو ضار. هي عكس `good`.

---

`posture`

### --feedback--

تشير هذه الكلمة إلى الطريقة التي يجلس أو يقف بها الشخص.

# --explanation--

`Bad posture` تشير إلى الجلوس أو الوقوف بطريقة غير صحية للجسم. على سبيل المثال:

- `Bad posture can cause neck and back pain over time.` - الجلوس بشكل خاطئ لفترات طويلة قد يؤدي إلى الألم والانزعاج.

- `Adjusting your desk height can prevent bad posture.` - تعديل ارتفاع المكتب يمكن أن يساعد في الحفاظ على وضعية جلوس صحية.

يمكن للأثاث الجيد أن يساعد في تحسين الوضعية ويمنع هذه المشاكل.

# --scene--

```json
{
  "setup": {
    "background": "company1-boardroom.png",
    "characters": [
      {
        "character": "Bob",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_11-1.mp3",
      "startTime": 1,
      "startTimestamp": 40.3,
      "finishTimestamp": 47.34
    }
  },
  "commands": [
    {
      "character": "Bob",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Bob",
      "startTime": 1,
      "finishTime": 4.22,
      "dialogue": {
        "text": "True, but good office furniture can prevent health issues",
        "align": "center"
      }
    },
    {
      "character": "Bob",
      "startTime": 4.22,
      "finishTime": 8.04,
      "dialogue": {
        "text": "that result from bad posture and make remote work more enjoyable.",
        "align": "center"
      }
    },
    {
      "character": "Bob",
      "opacity": 0,
      "startTime": 8.54
    }
  ]
}
```
