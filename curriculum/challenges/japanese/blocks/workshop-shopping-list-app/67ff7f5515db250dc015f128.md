...
`toggleItem()` 関数定義が最初に設定されたり変更されたりしたときにわかるようになったので、レンダリング間で関数が同じである場合もログに記録しましょう。

先ほど書いた `else` 文に `if` 文を追加してください。`else` ブロックの中で、文字列 `Current toggleItem function` をコンソールにログ出力してください。

# --hints--

前のステップで書いた `else` 文に `if` 文を追加してください。

```js
assert.match(code, /if\s*\(\s*prevToggleItem\s*!==\s*toggleItem\s*\)\s*{.*}\s*else\s*{/si);
```

`Current toggleItem function` 文の中で文字列 `else` をコンソールにログ出力してください。
