...
Створіть третій `@media` запит для `only screen` з `max-width` рівним `550px`. Всередині створіть `.hero-title` селектор з `font-size`, встановленим у `6rem`, `.hero-subtitle, .author, .quote, .list-title` селектор з `font-size`, встановленим у `1.8rem`, `.social-icons` селектор з `font-size`, встановленим у `2rem`, і `.text` селектор з `font-size`, встановленим у `1.6rem`.

# --hints--

У вас має бути новий `@media` запит для `only screen` з `max-width`, рівним `550px`. Він має йти після ваших двох попередніх.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText, 'only screen and (max-width: 550px)');
```

Ваше нове правило `@media` має містити `.hero-title` селектор, `.hero-subtitle, .author, .quote, .list-title` селектор, `.social-icons` селектор і `.text` селектор. Ці селектори мають бути в такому порядку.
