...
为了使第一部分看起来更像内联，只针对 `.info`元素中的 `input` 元素，将其 `width` 设置为 `50%`，并将其文本左对齐。

# --hints--

你应该使用 `.info input` 或 `.info > input` 选择器。
...
你应该给 `input` 元素一个 `text-align`，值为 `left`。
