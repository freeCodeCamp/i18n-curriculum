---
id: 68592730aaf34b61ae279168
title: Paso 10
challengeType: 20
dashedName: step-10
---

# --description--

La longitud devuelta por `len` es un entero, necesitas convertirla a una cadena usando la función `str`.

Actualiza la llamada a `print` para imprimir la longitud como una cadena.

# --hints--

Deberías tener `print(str(len(words[line_index])))` al final del ciclo.

```js
({
    test: () => runPython(`
    assert _Node(_code).find_function('pin_extractor').find_for_loops()[0].find_bodies()[0].has_call('print(str(len(words[line_index])))')
    `)
})
```

# --seed--

## --seed-contents--

```py
--fcc-editable-region--
def pin_extractor(poem):
    secret_code = ''
    lines = poem.split('\n')
    for line_index, line in enumerate(lines):
        print(line_index, line)
        words = line.split()
        print(len(words[line_index]))


poem = """Stars and the moon
shine in the sky
white and bright
until the end of the night"""

pin_extractor(poem)
--fcc-editable-region--

```
