...
Ändere die Stapelebene des `.back-mountain`-Elements, so dass es direkt hinter dem `.left-mountain`-Element liegt.
...
Du solltest die `z-index`-Eigenschaft auf `1` setzen.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.zIndex, '1');
```

Du solltest die `z-index`-Eigenschaft des `.left-mountain`-Elements nicht ändern.
