...
اكتمل القسم العلوي للبطاقة. الآن عليك إضافة قسم صورة اللاعب.

بعد إغلاق `div` الخاص بـ `card-header` ولا تزال داخل `div` الذي يحمل `className` بقيمة `card`، أضف `div` يحمل `className` بقيمة `card-image-wrap`. اتركه فارغًا الآن. ستضيف الصورة في الخطوة التالية.

# --hints--

يجب أن تضيف `div` يحمل `className` بقيمة `card-image-wrap`.
