...
سيكون من الجيد التحقق من صحة العنصر الممرر إلى الطريقة `add` قبل إضافته إلى الكتالوج.

أنشئ جملة `if` تستخدم `isinstance` للتحقق مما إذا كان `media_item` مثيلاً لـ `Movie` وارفع استثناء `TypeError` مع الرسالة `Only Movie or TVSeries instances can be added`.

لاحظ أن `isinstance(obj, ClassName)` تُرجع `True` إذا كان `obj` مثيلاً لـ `ClassName` أو *أي من أصنافها الفرعية*، لأنها تأخذ في الاعتبار سلسلة الوراثة الكاملة.

# --hints--

يجب رفع استثناء `TypeError` مع الرسالة `Only Movie or TVSeries instances can be added` عندما لا يكون `media_item` مثيلاً لـ `Movie` أو أصنافها الفرعية.
...
لا يجب رفع استثناء `TypeError` عندما يكون `media_item` عنصرًا صالحًا.
