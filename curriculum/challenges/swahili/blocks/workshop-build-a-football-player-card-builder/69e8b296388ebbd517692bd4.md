...
Kipengele chako cha tatu cha `div` chenye `className` ya `form-group` kinapaswa kuwa na `input` yenye `className` ya `input`, `id` ya `pas` na `type` ya `number`.
...
Kipengele chako cha nne cha `div` chenye `className` ya `form-group` kinapaswa kuwa na `label` yenye sifa ya kipengele `htmlFor` ya `dri`, `className` ya `label`, na maandishi ya `DRI`.
