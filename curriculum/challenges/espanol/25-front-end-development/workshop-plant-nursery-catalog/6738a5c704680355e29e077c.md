...
Como ya aprendiste, el método `delete` se usa para eliminar una entrada de un mapa. Los conjuntos tienen un método `delete` que también puedes usar para eliminar un elemento de ellos.

A veces puede que necesites eliminar todos los elementos de un mapa o un conjunto. El método `clear` hace exactamente ese trabajo.

Llama al método `clear` en `plantsSet` para eliminar todos los elementos.

# --hints--

Deberías llamar al método `clear` en `plantsSet`.
