...
Bei jeder Iteration Ihrer `while`-Schleife wird ein Element gefunden, das größer ist als Ihr aktueller Wert. Sie müssen dieses Element nach rechts verschieben, um Platz für Ihren aktuellen Wert zu schaffen.

Tun Sie dies, indem Sie den Wert am `j`-Index dem nächsten Index zuweisen.

# --hints--

Bevor Sie `j` dekrementieren, weisen Sie den Wert bei `j` dem Index `j + 1` zu.
