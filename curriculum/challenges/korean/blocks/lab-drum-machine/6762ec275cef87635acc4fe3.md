...
5. 각 `.drum-pad`는 클래스가 `clip`인 `audio` 요소를 포함해야 하며, 오디오 클립을 가리키는 `src` 속성과 부모 `.drum-pad` 요소의 내부 텍스트에 해당하는 `id`를 가져야 합니다 (예: `id="Q"`, `id="W"`, `id="E"` 등).
...
각 `.drum-pad`는 클래스가 `clip`인 `audio` 요소를 포함해야 하며, 오디오 클립을 가리키는 `src` 속성과 부모 `.drum-pad` 요소의 내부 텍스트에 해당하는 `id`를 가져야 합니다 (예: `id="Q"`, `id="W"`, `id="E"` 등).
