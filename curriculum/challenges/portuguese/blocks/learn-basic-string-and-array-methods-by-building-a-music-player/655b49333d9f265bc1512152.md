...
Observe que a arte do álbum no HTML e as músicas no array `userData.songs` foram alteradas.Trocamos as músicas originais por versões mais curtas que você pode usar para testar seu app nas próximas etapas.

Em seguida, você precisa verificar se há uma próxima música para tocar.Recupere o índice da música atual chamando a função `getCurrentSongIndex()`, e salve-o em uma constante `currentSongIndex`.

Depois disso, crie uma constante `nextSongExists` que contenha o valor booleano `true` ou `false` dependendo se a próxima música existe.

# --hints--

Você não deve modificar o listener do evento `ended` existente e seu conteúdo.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Você deve criar uma constante `currentSongIndex` e defini-la como a chamada da função `getCurrentSongIndex`.
...
Você deve verificar se existe uma próxima música comparando `userData.songs.length` e `currentSongIndex` e definir isso em uma constante `nextSongExists`.Se o último índice do array de músicas (`userData.songs.length - 1`) for maior que o `currentSongIndex` isso significa que há uma próxima música.
