...
Agora, você precisa adicionar o atributo `aria-current` à música que está tocando no momento.

Verifique se `songToHighlight` existe.Se for o caso, use o método `setAttribute` em `songToHighlight` e passe `"aria-current"` e `"true"` como o primeiro e o segundo argumentos. 

# --hints--

`highlightCurrentSong` deve adicionar o atributo `aria-current` à música que está tocando no momento e remover o atributo da música que foi tocada anteriormente.
