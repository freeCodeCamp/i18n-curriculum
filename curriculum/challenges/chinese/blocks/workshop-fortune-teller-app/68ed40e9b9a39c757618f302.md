...
现在你需要一个主类来帮助在 DOM 中动态渲染 UI。

创建一个名为 `Game` 的类。在其中，添加一个名为 `cards` 的属性，类型为 `Card[]`，暂时将其初始化为空数组。
