...
أضف مستمع حدث `"click"` إلى عنصر `shuffleButton`. لتمكين تشغيل الدالة، مرر الدالة `shuffle`.

**ملاحظة**: لا تحتاج إلى استدعاء راجع داخل هذا المستمع للحدث بالذات. كما أنك لا تحتاج إلى استدعاء الدالة `shuffle`، فقط مرر معرفها.

# --hints--

يجب ربط طريقة `addEventListener()` بـ `shuffleButton`.
...
يجب أن تمرر `shuffle` كالقيمة الثانية لمستمع حدث `"click"`.
