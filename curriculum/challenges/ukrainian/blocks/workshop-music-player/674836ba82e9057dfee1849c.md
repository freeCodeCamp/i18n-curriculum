...
Далі використайте властивість `classList` і метод `add()`, щоб додати клас `playing` до елемента `playButton`. Це шукатиме клас `playing` у CSS-файлі і додасть його до елемента `playButton`.

Потім встановіть `userData.currentSong` у `song`, а щоб нарешті відтворити пісню, виконайте метод `play()` на змінній `audio`. `play()` — це метод з веб-аудіо API для відтворення mp3-файлу.

# --hints--

Ви повинні додати клас `playing` до елемента `playButton` у вашій функції `playSong`.
...
Ви повинні встановити `userData.currentSong` у `song` у вашій функції `playSong`.
...
Ви повинні викликати метод `play()` на змінній `audio` у вашій функції `playSong`.
