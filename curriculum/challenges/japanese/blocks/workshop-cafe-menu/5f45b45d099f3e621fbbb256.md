...
リンクがクリックされているときにリンクのプロパティを変更するには、`a:active { propertyName: propertyValue; }`のような<dfn>疑似セレクター</dfn>を使います。

クリックされたときにフッターの`Visit our website`リンクの色を`white`に変更してください。

# --hints--

`a:active`疑似セレクターを使うべきです。
...
あなたの`a:active`は`color`の`white`を持つべきです。
