...
完成 `showResults` 函式（程式）。應該更新 `playerScoreSpanElement` 和 `computerScoreSpanElement` 以顯示玩家和電腦的更新分數。

`roundResultsMsg` 也應該根據該回合的結果進行更新。

**提示**

- 請記住你已經學會如何使用 `innerText` 屬性來更新元素的文字內容。
- 你可以使用 `getRoundResults` 函式（程式）來取得該回合的結果。
