...
Después del elemento `card-image-wrap`, agrega un `div` con el `className` de `card-name-strip`. Dentro, añade un `span` con el `className` de `card-name` y el texto `PELE`.

# --hints--

Debes agregar un `div` con el `className` de `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

Debes agregar un `span` con el `className` de `card-name` dentro de `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

El span `.card-name` debe tener el texto `PELE`.
