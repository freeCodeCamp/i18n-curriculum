...
`evalFormula` 調用的第一個參數需要是單元格的內容（存儲在 `value` 中）。 但是，內容以 `=` 字符開頭來觸發該函數，因此你需要傳遞從索引 `1` 開始的 `value` 子字符串。

# --hints--

你應該將 `value` 作爲第一個參數傳遞給 `evalFormula()` 調用。
