...
`try` ブロック内で `saved` 変数を作成してください。値には `getItem()` メソッドを使って `STORAGE_KEY` を取得します。その変数のすぐ下に、条件が `if` の `saved` 文を作成してください。`if` ブロックは今は空のままにしてください。

# --hints--

`loadPlayer` 関数は `localStorage.getItem` ブロック内で `STORAGE_KEY` を使って `try` を呼び出す必要があります。

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

`localStorage.getItem` の結果は `saved` という変数に保存してください。

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

`try` ブロックには条件が `if` の `saved` 文が必要です。
