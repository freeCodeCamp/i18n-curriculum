...
そのオブジェクトの下に、次のプロパティを持つ新しいオブジェクトを作成してください。
...
あなたの`myFavoriteFootballTeam.players`配列は2つの値を持つ必要があります。
