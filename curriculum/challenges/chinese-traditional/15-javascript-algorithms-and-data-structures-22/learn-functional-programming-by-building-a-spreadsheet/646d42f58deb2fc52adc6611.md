...
現在你已經解析並評估了乘法和除法運算符，你需要對加法和減法運算符執行相同的操作。

聲明一個 `infix` 變量，併爲其分配一個正則表達式，該正則表達式與一個數字（包括十進制數）匹配，後跟 `+` 或 `-` 運算符，後跟另一個數字。
...
你的第一個捕獲組應該匹配一個或多個數字或小數點。 使用 `\d` 字符類。
...
你的 `infix` 正則表達式應該使用第三個捕獲組。

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(/);
```

你的第三個捕獲組應該與你的第一個捕獲組相同。
