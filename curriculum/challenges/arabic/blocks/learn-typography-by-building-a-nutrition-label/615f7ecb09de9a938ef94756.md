...
أسفل آخر عنصر `p` لديك، أضف عنصر `p` آخر يحتوي على النص `Total Carbohydrate 37g 13%`. كما في السابق، استخدم عناصر `span` لجعل النص `Total Carbohydrate` و `13%` عريضًا. ثم لف المغذي والكمية في عنصر `span` للمحاذاة مرة أخرى.

# --hints--

يجب أن تنشئ عنصر `p` جديد في نهاية عنصر `.daily-value` الخاص بك.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'p');
```

يجب أن يحتوي عنصر `p` الجديد على النص `Total Carbohydrate 37g 13%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText?.match(/Total Carbohydrate[\s|\n]+37g[\s|\n]+13%/));
```

يجب أن يحتوي عنصر `p` الجديد على ثلاثة عناصر `span`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelectorAll('span')?.length === 3);
```

يجب أن يلف عنصر `span` واحد النص `Total Carbohydrate 37g`.
...
يجب أن يلف عنصر `span` واحد النص `Total Carbohydrate`.
...
يجب أن يحتوي عنصر `span` حول `Total Carbohydrate` على الخاصية `class` مضبوطة على `bold`.
