...
O último desafio mostrou como usar o método `concat` para criar um novo array a partir da mescla de outros sem modificar os originais. Compare `concat` com o método `push`. `push` adiciona itens ao final do mesmo array em que é chamado, o que muta esse array. Aqui está um exemplo:
