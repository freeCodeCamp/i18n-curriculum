...
HTMLのアルバムアートと`userData.songs`配列の曲が変わっていることに気づきましたか。元の曲を短い曲に入れ替えたので、アプリのテストに使えます。

すべての基本機能は揃いました。現在の問題は、再生中の曲が終わったときに次の曲が自動で再生されないことです。

これを直すには、再生中の曲が終わったことを検知するイベントリスナーを設定できます。`"ended"`イベントリスナーが適しています。これはメディアの再生が終わったときに発火します。

`audio`要素に`"ended"`イベントを監視するイベントリスナーを追加し、`playNextSong`関数の参照を渡してください。これでミュージックプレイヤープロジェクトは完成です！

# --hints--

`addEventListener()`変数に`audio`メソッドをチェーンしてください。

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

`audio`変数に使ったイベントリスナーは`"ended"`イベントを監視する必要があります。

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

`playNextSong`イベントリスナーに`"ended"`の参照を渡してください。
