...
Necesitarás llevar un registro de las unidades que tu aplicación soporta.

Declara un array de `units` y asígnale los valores de cadena `"cup"`, `"gram"`, `"ounce"` y `"teaspoon"`.

# --hints--

Deberías declarar una variable de `units`.

```js
assert.isDefined(units);
```

Tu variable de `units` debería ser un array.

```js
assert.isArray(units);
```

Tu array de `units` debería tener cuatro elementos.

```js
assert.lengthOf(units, 4);
```

Tu array de `units` debería incluir la cadena `"cup"`.

```js
assert.include(units, "cup");
```

Tu array de `units` debería incluir la cadena `"gram"`.

```js
assert.include(units, "gram");
```

Tu array de `units` debería incluir la cadena `"ounce"`.

```js
assert.include(units, "ounce");
```

Tu array de `units` debería incluir la cadena `"teaspoon"`.
