...
Ora esegui il mapping dell'array `POSITIONS` usando una funzione freccia che riceve ogni stringa di posizione come `pos` e restituisce un elemento `option`. L'opzione dovrebbe avere un `key` pari a `pos` e il valore anch'esso `pos`. Inoltre, mostra `pos` come testo di ogni `option` generato.

Puoi usare qualsiasi nome come parametro della tua map, non deve essere per forza `pos`.

# --hints--

Il tuo `select` di posizione dovrebbe contenere elementi `option` mappati dall'array `POSITIONS`.
...
Il `value` di ogni opzione dovrebbe corrispondere al testo mostrato ed essere una posizione valida.
