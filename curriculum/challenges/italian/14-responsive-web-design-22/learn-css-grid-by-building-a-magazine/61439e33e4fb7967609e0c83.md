...
Dovresti creare un nuovo elemento `blockquote` all'interno dell'elemento `.text`.

```js
assert.exists(document.querySelector('.text blockquote'));
```

L'elemento `blockquote` dovrebbe trovarsi dopo i tre elementi `p`.

```js
assert(document.querySelector('.text')?.children?.[3]?.localName === 'blockquote');
```

L'elemento `blockquote` dovrebbe avere due elementi `hr`.

```js
assert(document.querySelectorAll('.text blockquote hr')?.length === 2);
```

L'elemento `blockquote` dovrebbe contenere un elemento `p`.

```js
assert.exists(document.querySelector('.text blockquote p'));
```

Gli elementi figli di `blockquote` dovrebbero essere nell'ordine corretto.
...
Il nuovo elemento `p` dovrebbe avere l'attributo `class` impostato su `quote`.

```js
assert(document.querySelector('.text blockquote p')?.className === 'quote');
```

Il nuovo elemento `p` dovrebbe contenere il testo `The entire curriculum should be a series of projects`.
