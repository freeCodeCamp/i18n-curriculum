...
Below your last `p` element, add another `p` element with the text `Total Carbohydrate 37g 13%`. Kama hapo awali, tumia vipengee vya `span` ili kufanya maandishi `Total Carbohydrate` na `13%` kuwa nzito. Then, wrap the nutrient and amount in a `span` for alignment again.
