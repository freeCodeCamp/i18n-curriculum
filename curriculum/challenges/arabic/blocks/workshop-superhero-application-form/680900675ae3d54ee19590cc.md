...
تظهر قيم `powerSourceOptions` الآن كخيارات لعنصر `select`.

الآن، استخدم `powerSource` كقيمة لعنصر `select`. ثم، عرّف معالِج `onChange` باستخدام دالة سهمية تأخذ `e` كمعلمة وتُرجع ضمنيًا `setPowerSource(e.target.value)`.

# --hints--

يجب استخدام `{powerSource}` كقيمة لعنصر `select`.

```js
assert.match(code, /<\s*select\s+[\s\S]*value\s*=\s*\{\s*powerSource\s*\}/)
```

يجب تعيين قيمة عنصر `select` باستخدام `onChange` مع دالة سهمية تُرجع ضمنيًا لتحديث حالة `powerSource` بواسطة `setPowerSource`.
