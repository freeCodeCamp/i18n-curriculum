...
你的 `ShoppingCart` 類別需要能夠添加條款。創建一個空的 `addItem` 方法，該方法接受兩個參數：`id` 和 `products`。創建方法可能看起來像這樣：
...
第一個參數 `id` 是使用者已加入購物車的產品的 `id`。第二個參數 `products` 是一個 `Dessert` 實體的陣列。透過使用參數而非直接取用你現有的 `products` 陣列，這個方法在未來若想添加額外的產品列表時會更具彈性。

# --hints--

你的 `ShoppingCart` 類別應該有一個 `addItem` 方法。
...
你的 `addItem` 方法應該接受兩個參數：`id` 和 `products`。
