...
実際のインデックスを取得した後、if文を作成して`actual_index`が0未満か、`self.emails`リストの長さ以上かどうかを確認してください。もしそうなら、メッセージ`Invalid email number.\n`を出力し、`return`を使ってメソッドを終了してください。
...
インデックスが無効な場合は、メッセージ`Invalid email number.\n`を出力し、`return`を使ってメソッドを終了してください。
