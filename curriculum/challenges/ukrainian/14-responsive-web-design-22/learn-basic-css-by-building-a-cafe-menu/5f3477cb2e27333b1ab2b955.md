...
Now you need to link the `styles.css` file, so the styles will be applied again. Додайте елемент `link` до елемента `head`. Надайте йому атрибут `rel` зі значенням `"stylesheet"` та атрибут `href` зі значенням `"styles.css"`.
