...
もし `userData.currentSong` が `null` でなければ、次に再生する曲を見つける必要があります。`getNextSong` を使ってそれができることを覚えておいてください。

`playNextSong` 関数を完成させて、プレイリストの次の曲を再生するようにしてください。現在の曲が最後の曲の場合は、`userData.currentSong` を `null` に、`userData.songCurrentTime` をゼロに設定し、`pauseSong` 関数を呼び出してください。

# --hints--

`currentSong` の `userData` が真で、プレイリストの最後の曲でない場合、`playNextSong` 関数は現在の曲の次の曲を再生するはずです。
...
`currentSong` の `userData` がプレイリストの最後の曲の場合、`playNextSong` 関数は `userData.currentSong` を `null` に設定するはずです。
...
`currentSong` の `userData` がプレイリストの最後の曲の場合、`playNextSong` 関数は `userData.songCurrentTime` を `0` に設定するはずです。
...
`currentSong` の `userData` がプレイリストの最後の曲の場合、`playNextSong` 関数は `pauseSong` 関数を呼び出すはずです。
...
`currentSong` の `userData` が `null` と等しい場合でも、あなたの `playNextSong` 関数は `playSong` 配列の最初の曲の `id` を引数として `userData.songs` 関数を呼び出すはずです。
