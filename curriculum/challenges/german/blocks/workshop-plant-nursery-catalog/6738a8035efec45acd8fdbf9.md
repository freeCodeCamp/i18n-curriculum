...
Wie Sie sehen, ist Ihr Set jetzt vollständig leer. Entfernen Sie sowohl die `clear`- als auch die `has`-Aufrufe aus Ihrem Code.

# --hints--

Sie sollten `plantsSet.clear()` nicht in Ihrem Code haben.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

Sie sollten `plantsSet.has(ballerina)` nicht in Ihrem Code haben.
