...
次に、`.forEach()` メソッドを使って `animationData` 配列をループしてください。`.forEach()` メソッドのコールバック関数には `obj` をパラメータとして渡しますが、コールバック関数の本体は今は空のままにしてください。

# --hints--

`.forEach()` メソッドを使って `animationData` 配列をループする必要があります。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\(/);
```

`obj` メソッドのコールバック関数に `.forEach()` をパラメータとして渡してください。
...
`.forEach()` メソッドのコールバック関数の本体は空にしてください。
