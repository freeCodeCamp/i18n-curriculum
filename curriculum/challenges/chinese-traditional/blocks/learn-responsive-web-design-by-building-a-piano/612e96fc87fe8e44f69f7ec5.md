---
id: 612e96fc87fe8e44f69f7ec5
title: 步驟 13
challengeType: 0
dashedName: step-13
---

# --description--

開始為按鍵設計樣式。在 `#piano` 規則下方，使用 `class` 選擇器標的 `.keys` 元素。為新的規則設定 `background-color` 屬性為 `#040404`、`width` 屬性為 949px 以及 `height` 屬性為 180px。

# --hints--

你應該有一個 `.keys` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.keys'));
```

你的 `.keys` 選擇器應該設定 `background-color` 屬性為 `#040404`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.keys')?.backgroundColor === 'rgb(4, 4, 4)');
```

你的 `.keys` 選擇器應該設定 `width` 屬性為 `949px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.keys')?.width === '949px');
```

你的 `.keys` 選擇器應該設定 `height` 屬性為 `180px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.keys')?.height === '180px');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <title>Piano</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="./styles.css">
  </head>
  <body>
    <div id="piano">
      <div class="keys">
        <div class="key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>
        <div class="key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>

        <div class="key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>
        <div class="key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>

        <div class="key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>
        <div class="key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>
        <div class="key black--key"></div>
      </div>
    </div>
  </body>
</html>
```

```css
html {
  box-sizing: border-box;
}

*, *::before, *::after {
  box-sizing: inherit;
}

#piano {
  background-color: #00471b;
  width: 992px;
  height: 290px;
  margin: 80px auto;
}

--fcc-editable-region--

--fcc-editable-region--
```
