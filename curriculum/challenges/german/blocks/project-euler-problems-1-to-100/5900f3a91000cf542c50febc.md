...
title: 'Problem 61: Zyklische figurale Zahlen'
challengeType: 1
forumTopicId: 302173
...
Dreieckszahlen, Quadratzahlen, Fünfeckzahlen, Sechseckzahlen, Siebeneckzahlen und Achteckzahlen sind alle figurale (polygonale) Zahlen und werden durch die folgenden Formeln erzeugt:

| Zahlentyp     | Formel                      | Folge                 |
| -------------- | ---------------------------- | --------------------- |
| Dreieck       | $P_3(n) = \frac{n(n+1)}{2}$  | 1, 3, 6, 10, 15, ...  |
| Quadrat       | $P_4(n) = n^2$               | 1, 4, 9, 16, 25, ...  |
| Fünfeck       | $P_5(n) = \frac{n(3n−1)}2$   | 1, 5, 12, 22, 35, ... |
| Sechseck      | $P_6(n) = n(2n−1)$           | 1, 6, 15, 28, 45, ... |
| Siebeneck     | $P_7(n) = \frac{n(5n−3)}{2}$ | 1, 7, 18, 34, 55, ... |
| Achteck       | $P_8(n) = n(3n−2)$           | 1, 8, 21, 40, 65, ... |

Die geordnete Menge von drei vierstelligen Zahlen: 8128, 2882, 8281, hat drei interessante Eigenschaften.

1. Die Menge ist zyklisch, da die letzten beiden Ziffern jeder Zahl die ersten beiden Ziffern der nächsten Zahl sind (einschließlich der letzten Zahl mit der ersten).
2. Jeder polygonale Typ: Dreieck ($P_3(127) = 8128$), Quadrat ($P_4(91) = 8281$) und Fünfeck ($P_5(44) = 2882$), wird durch eine andere Zahl in der Menge dargestellt.
3. Dies ist die einzige Menge von vierstelligen Zahlen mit dieser Eigenschaft.

Finden Sie die Summe aller Zahlen in geordneten Mengen von `n` zyklischen vierstelligen Zahlen, für die jeder der polygonalen Typen von $P_3$ bis $P_{n + 2}$ durch eine andere Zahl in der Menge dargestellt wird.
