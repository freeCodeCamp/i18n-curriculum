...
Nel tuo elemento `span`, crea un elemento `label` per un `entry-dropdown` e dagli il testo `Add food or exercise:`. Poi crea un elemento `select` con l'attributo `id` impostato su `entry-dropdown` e un `name` impostato su `options`. Sotto, aggiungi un elemento `button` con l'attributo `id` impostato su `add-entry` e il testo `Add Entry`.
...
Il tuo nuovo elemento `label` dovrebbe avere il testo `Add food or exercise:`.
