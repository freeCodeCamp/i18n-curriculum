...
Für das zweite `fieldset` möchtest du, dass der `input` und der `label`-Text in der gleichen Zeile erscheinen.

Beginne damit, indem du den `input`-Elementen im zweiten `fieldset` die Klasse `inline` zuweist.

# --hints--

Du solltest dem ersten `input` die Klasse `inline` zuweisen.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

Du solltest dem zweiten `input` die Klasse `inline` zuweisen.
