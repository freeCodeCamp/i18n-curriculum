...
最後のハードコードされた値は6つのプレイヤーステータスです。最初の3つのハードコードされた数字のセットを対応する`player`プロパティに置き換えてください。
