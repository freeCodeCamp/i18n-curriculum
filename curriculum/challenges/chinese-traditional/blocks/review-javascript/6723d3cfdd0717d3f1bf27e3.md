...
- **`Object.hasOwn()` 方法**：這是現代且推薦用來檢查物件是否擁有某個屬性（非繼承）的方式。語法為 `Object.hasOwn(object, propertyName)`。如果該屬性存在於物件上，則傳回 `true`，若不存在則傳回 `false` — 不論該屬性持有何種值。當值可能是 `0`、`false`、`null` 或 `undefined` 時，這比檢查 `if (obj.prop)` 更安全。
...
在 `if` 述語中使用 `Object.hasOwn()` 來存取值之前，可防止錯誤：
...
- **定義**：JavaScript 中的類別用於定義用來創建物件和封裝資料的藍圖。類別包含一個建構式，這是一個特殊的方法，當從類別創建新物件時會自動被呼叫。它用來初始化物件的屬性。這裡使用 `this` 關鍵字來取用類別的當前實體。在建構式下面，你可以有稱為方法的方法。方法是在類別內定義的函式，用來對類別的資料或狀態執行動作或操作行為。它們用來定義類別實體可以執行的行為。
...
- **靜態方法**：這些方法通常用於不需要存取物件特定狀態的工具函式。它們定義在類別中以封裝相關功能。
