...
Всередині функції `getElement` використайте `querySelector` з дженеріками та збережіть результат у змінну. Додайте інструкцію `if`, щоб перевірити, чи елемент не існує, і якщо так, викиньте помилку з повідомленням `Element not found: ${error message here}`. Якщо елемент існує, поверніть його.

# --hints--

Ви повинні оголосити змінну з ім’ям `el` всередині функції `getElement`.
...
Вашій змінній `el` має бути призначене значення `document.querySelector<T>(selector)`.
...
Ви повинні мати оператор if, який перевіряє, чи `el` не знайдено.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

Ви повинні викинути нову помилку Error з повідомленням `Element not found: ${selector}` всередині вашої інструкції `if`.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

Ви повинні повернути змінну `el` всередині функції getElement.
