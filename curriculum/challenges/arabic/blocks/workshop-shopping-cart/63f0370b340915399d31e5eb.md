...
ستقوم بتحديث HTML في هذه الطريقة أيضًا. عيّن `textContent` لعنصر `cartSubTotal` ليكون قيمة `subTotal` مع تثبيت رقمين عشريين. استخدم صيغة القالب النصي لإضافة علامة الدولار في بداية القيمة.
...
يجب أن تعيّن `textContent` لعنصر `cartSubTotal` إلى سلسلة القالب النصي الخاصة بك.
