...
Chini ya kipengele chako cha `p`, tengeneza kipengele kingine cha `p` chenye maandishi ya `Sodium 160mg 7%`. Weka `Sodium` na `7%` kila moja katika `span` yenye darasa la `bold` kama ulivyofanya na zingine.

Kisha, ongeza kipengele kingine cha `span` kuzunguka `Sodium 160mg` kwa ajili ya ulinganifu tena.
...
Kipengele chako kipya cha `p` kinapaswa kuwa na maandishi ya `Sodium 160mg 7%`.
...
Kipengele cha `span` kinapaswa kuzunguka maandishi ya `Sodium 160mg`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.firstElementChild?.innerText?.match(/Sodium[\s|\n]160mg/));
```

Kipengele cha `span` kinapaswa kuzunguka maandishi ya `Sodium`.
...
`span` inayozunguka `Sodium` inapaswa kuwa na sifa ya `class` iliyowekwa kuwa `bold`.
...
Kipengele cha `span` kinapaswa kuzunguka maandishi ya `7%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.lastElementChild?.innerText === '7%');
```

Kipengele cha `span` kinachozunguka `7%` kinapaswa kuwa na sifa ya `class` iliyowekwa kuwa `bold`.
