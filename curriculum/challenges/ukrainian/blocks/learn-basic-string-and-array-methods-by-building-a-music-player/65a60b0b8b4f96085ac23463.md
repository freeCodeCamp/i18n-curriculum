...
У прикладі, якщо `a.name` дорівнює `b.name`, тоді функція повертає `0`. Це означає, що нічого не змінюється, і порядок `a` та `b` залишається таким самим.
...
Під вашими інструкціями `if` поверніть число `0`, щоб залишити порядок двох елементів незмінним.

# --hints--

Ви повинні повернути число `0` під вашими інструкціями `if`.
