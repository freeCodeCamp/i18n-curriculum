...
創建另一個 `button` 元素，其類名爲 `playlist-song-delete`，並將 `aria-label` 屬性設置爲 `Delete`，並用 `song.title` 進行插值。 對於刪除圖標的內容，粘貼以下 SVG：
