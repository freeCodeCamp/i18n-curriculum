...
你從 `GET` 請求中獲得的數據最初是不可用的。 爲了使數據可用，你可以使用 `Response` 對象上的 `.json()` 方法將其解析爲 JSON。 如果你在瀏覽器控制檯中展開 `Response` 對象的 `Prototype`，你將在那裏看到 `.json()` 方法。

刪除 `console.log(res)` 並隱式返回 `res.json()`。
