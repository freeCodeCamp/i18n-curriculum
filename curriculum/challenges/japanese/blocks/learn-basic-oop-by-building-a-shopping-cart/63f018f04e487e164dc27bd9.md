...
最初の `p` 要素に `span` 要素を追加してください。`span` に `product-count` クラスと `id` の `product-count-for-id${id}` を付けてください。その後、span の後に `p` 要素に `name` 変数のテキストを入れてください。

# --hints--

最初の `p` 要素には `span` 要素が必要です。
...
`span` 要素には `class` の `product-count` を付けてください。
...
`span` 要素には `id` の `product-count-for-id${id}` を付けてください。
...
最初の `p` 要素には `name` 変数のテキストを入れてください。これは span の外に置きます。
