...
Ora che l'interfaccia `PlayerData` è definita, aggiorna il componente `PlayerCard` per accettare una prop `player` tipizzata come `PlayerData` in modo da corrispondere all'interfaccia `PlayerData`.

Ricorda che puoi destrutturare le props in questo modo:
...
Dovresti aggiornare `PlayerCard` per accettare una prop `player` tipizzata come `PlayerData`.
