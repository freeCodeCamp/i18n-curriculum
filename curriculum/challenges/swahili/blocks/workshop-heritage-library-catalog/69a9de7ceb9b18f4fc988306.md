...
Ndani ya mzunguko, angalia kama mwandishi wa kipengee cha sasa (aliyebadilishwa kuwa herufi ndogo) anajumuisha `searchTerm`. Njia ya `.includes()` inarudisha `true` ikiwa mfuatano wa herufi una mfuatano mwingine wa herufi kama sehemu ndogo. Ikiwa hali ni kweli, sukuma kipengee hicho ndani ya `results`. Baada ya mzunguko, rudisha `results`.

# --hints--

`findByAuthor(catalog, "king").length` inapaswa kuwa `6`.

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

`findByAuthor(catalog, "asimov").length` inapaswa kuwa `2`.

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

`findByAuthor(catalog, "xyz").length` inapaswa kuwa `0`.
