...
- **超文本标记语言 元素**：用于表现页面上的内容。大多数由一个起始标签和一个结束标签组成（例如，`<h1></h1>`、`<p></p>`）。
- **HTML 结构**：HTML 由 `head` 和 `body` 组成，分别用于组织元数据、样式和内容。
- **常见超文本标记语言元素**：标题（`<h1>` - `<h6>`）、段落（`<p>`）和 div 容器（`<div>`）。
...
- **内部链接**：通过在 `a` 元素上使用 `href="#id"` 并为目标元素赋予相同的 `id`，链接到页面的另一个部分。

## 语义化 HTML 的重要性

- **标题元素的结构层次**：使用正确的标题元素来保持内容的结构层次非常重要。 `h1` 元素是最高级别的标题，`h6` 元素是最低级别的标题。
- **表现型超文本标记语言元素**：定义内容外观的元素，例如废弃的 `center`、`big` 和 `font` 元素。
...
- **`button` 元素**：用于创建可点击的按钮。按钮还可以有一个 `type` 属性，用于控件按钮激活时的行为，例如 `submit`、`reset`、`button`。
...
- **良好的链接文本的重要性**：你应使用描述性链接文本，以帮助用户理解链接的用途。
...
- **`aria-label` 和 `aria-labelledby` 属性**：这些属性用于为元素定义一个程序化（或可访问的）名称，帮助辅助技术（例如屏幕阅读器）理解该元素的用途。当元素的视觉标签是图像或符号而非文本时，它们经常被使用。`aria-label` 允许你直接在属性中定义名称，而 `aria-labelledby` 允许你引用页面上已有的文本。
