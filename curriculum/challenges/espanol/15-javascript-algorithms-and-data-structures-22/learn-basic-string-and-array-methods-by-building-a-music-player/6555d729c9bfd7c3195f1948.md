...
`textContent` establece el texto de un nodo y te permite establecer o recuperar el contenido de texto de un elemento HTML.
...
Puedes usar un operador ternario para condicionalmente establecer el valor de contenido de texto. Aquí tienes un ejemplo de asignar el resultado de un operador ternario a una variable:

```js
const example = condition ? "I'm true" : "I'm false";
```

Usa un operador ternario para verificar si `currentTitle` evalúa a un valor verdadero. Si es así, establece `playingSong.textContent` en `currentTitle`. De lo contrario, establezca en una cadena vacía.

Luego, debajo de eso, utiliza un operador `ternary` para verificar si `currentArtist` es verdadero. Si es así, establece `songArtist.textContent` en `currentArtist`. De lo contrario, establezca en cadena vacía.

# --hints--

Debes encadenar la propiedad `textContent` a `playingSong`.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

Debes usar el operador `ternary` para establecer la propiedad `textContent` de `playingSong` a `currentTitle` o `""`.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

Debes encadenar la propiedad `textContent` a `songArtist`.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

Debes usar el operador `ternary` para establecer la propiedad `textContent` de `songArtist` a `currentArtist` o `""`.
