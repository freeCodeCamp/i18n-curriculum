...
Agora é hora de testar a função `playNextSong`. 

Adicione um listener de evento `"click"` ao elemento `nextButton`, depois passe `playNextSong` como o segundo argumento do seu listener de evento. Esta é a função que o listener de evento irá executar.

Teste seu app clicando primeiro no botão de play seguido pelo botão next. Você deve ver que tudo está funcionando conforme o esperado.

# --hints--

Você deve chamar o método `addEventListener()` na sua variável `nextButton`.
...
Você deve passar `playNextSong` como o segundo argumento do seu método `addEventListener`.
