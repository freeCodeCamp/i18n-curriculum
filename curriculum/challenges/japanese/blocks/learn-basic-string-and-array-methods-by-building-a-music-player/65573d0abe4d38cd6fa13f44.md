...
すべての基本機能は揃いました。現在の問題は、再生中の曲が終わったときに次の曲が自動的に再生されないことです。

これを解決するには、再生中の曲が終了したことを検知するイベントリスナーを設定できます。`"ended"` イベントリスナーが適しています。これはメディアの再生が終了したときに発火します。

`audio` 要素に `"ended"` イベントを監視するイベントリスナーを追加してください。空の波括弧を使ったアロー構文でコールバックを渡してください。

# --hints--

`addEventListener()` 変数に `audio` メソッドをチェーンする必要があります。

```js
assert.match(code, /audio\.addEventListener\(/)
```

`audio` 変数に設定したイベントリスナーは `"ended"` イベントを監視する必要があります。

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1/)
```

`"ended"` イベントリスナーに空のコールバックをアロー構文で渡してください。
