...
Ora devi mostrare alcune informazioni sull'autore. Per prima cosa, mostra il nome dell'autore.

Crea un tag `h2` con il `class` `"author-name"`. Poi, inserisci `author` all'interno del tag `h2`. Questo è il nome dell'autore.
...
Il tuo elemento `h2` dovrebbe avere una classe impostata su `"author-name"`
...
Dovresti inserire `author` nel tuo tag `h2` per mostrare i nomi degli autori
