...
Ahora elimina las dos últimas llamadas a `print`.

# --hints--

No deberías tener `print(charlie_brown.name)` en tu código.

```js
({ test: () => assert(runPython(`not _Node(_code).has_stmt("print(charlie_brown.name)")`)) })
```

No deberías tener `print(charlie_brown.level)` en tu código.
