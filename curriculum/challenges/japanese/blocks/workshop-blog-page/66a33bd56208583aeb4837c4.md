...
`footer` 要素の中に、`section` に設定された `id` を持つ `contact` 要素を追加してください。

`section` 要素の中に、`h2` のテキストを持つ `Contact` 要素を追加してください。

# --hints--

`section` 要素の中に `footer` 要素があるはずです。
...
`section` 要素は `id` に設定された `"contact"` を持つべきです。
