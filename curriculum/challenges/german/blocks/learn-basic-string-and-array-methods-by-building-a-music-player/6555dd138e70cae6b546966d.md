...
Sie müssen den aktuell abgespielten Song oder den ersten Song in der Wiedergabeliste erhalten. Erstellen Sie dazu eine `song`-Konstante und verwenden Sie den ODER-Operator (`||`), um sie auf den aktuellen Song von `userData` **oder** den ersten Song im `userData?.songs`-Array zu setzen.

Vergessen Sie nicht, optionale Verkettung zu verwenden.

# --hints--

Sie sollten `setPlayButtonAccessibleText` und dessen Inhalt nicht verändern.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Sie sollten auf `userData?.currentSong` oder `userData?.songs[0]` zugreifen.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

Sie sollten `userData?.currentSong || userData?.songs[0]` einer `song`-Konstante zuweisen.
