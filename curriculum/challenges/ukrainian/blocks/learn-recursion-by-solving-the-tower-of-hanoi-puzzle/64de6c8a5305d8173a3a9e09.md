...
`make_allowed_move()` приймає `rod1` і `rod2` як параметри. Тут потрібне невелике рефакторинг. Замініть кожне входження `source` на `rod1`.

# --hints--

Ви повинні змінити кожне входження `source` всередині функції `make_allowed_move()` на `rod1`.
