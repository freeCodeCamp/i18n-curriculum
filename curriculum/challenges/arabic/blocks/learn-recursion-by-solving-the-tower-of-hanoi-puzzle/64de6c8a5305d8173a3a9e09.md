...
تأخذ الدالة `make_allowed_move()` المعلمتين `rod1` و `rod2`. تحتاج إلى بعض إعادة الهيكلة هنا. غيّر كل ظهور لـ `source` إلى `rod1`.

# --hints--

يجب أن تغيّر كل ظهور لـ `source` داخل الدالة `make_allowed_move()` إلى `rod1`.
