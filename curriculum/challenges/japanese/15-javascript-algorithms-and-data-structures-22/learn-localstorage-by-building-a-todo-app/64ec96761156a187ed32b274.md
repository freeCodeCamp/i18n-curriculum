...
タスク管理を可能にするために、各タスクに対して削除ボタンと編集ボタンを追加しましょう。

`button` 要素を 2 つ作成し、`type` 属性を `button` に、`class` 属性を `btn` に設定してください。 最初のボタンのテキストを `Edit` に、2 番目のボタンのテキストを `Delete` に設定してください。

# --hints--

`button` 要素を作成して、type は `button`、class は `btn`、テキストは `"Edit"`に、この順で設定してください。
