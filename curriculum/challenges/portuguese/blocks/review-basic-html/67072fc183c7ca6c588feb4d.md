...
- **Papel do HTML**: HTML representa o conteúdo e a estrutura da página web.
...
Um atributo booleano é um atributo que pode estar presente ou ausente em uma tag HTML. Se estiver presente, o valor é true, caso contrário é falso. Exemplos de atributos booleanos incluem `disabled`, `readonly` e `required`.
...
- **Elemento `button`**: Este elemento é usado para criar um botão clicável.
...
Nomes de ID não podem ter espaços. Se seu nome de ID contiver várias palavras, você pode usar traços ou underscores entre as palavras assim:
...
- **Elemento `html`**: Representa o elemento de nível superior ou a raiz de um documento HTML. Para especificar o idioma do documento, você deve usar o atributo `lang`.
...
- **Elementos `meta`**: Esses elementos representam os metadados do seu site. Eles contêm detalhes como codificação de caracteres e como sites como o Twitter devem pré-visualizar o link da sua página, entre outros.
...
- **Elemento Meta (`description`)**: É usado para fornecer uma descrição curta para a página web e melhorar o SEO.
...
- **Tags Open Graph**: O protocolo open graph permite controlar como o conteúdo do seu site aparece em várias plataformas de mídia social, como Facebook, LinkedIn e muitas outras. Ao definir essas propriedades open graph, você pode atrair usuários para clicarem e interagirem com seu conteúdo. Você pode definir essas propriedades por meio de uma coleção de elementos `meta` dentro da seção `head` do seu HTML.
- **Propriedade `og:title`**: Usada para definir o título que será exibido nas postagens em redes sociais.
...
Existem outros elementos substituídos, como `video` e `embed`. E alguns elementos se comportam como elementos substituídos em circunstâncias específicas. Aqui está um exemplo de um elemento `input` com o atributo `type` definido como `image`:
...
- **Formatos de imagem**: Dois dos formatos de arquivo mais comuns são PNG e JPG, mas eles não são mais os formatos ideais para servir imagens. A menos que você precise de suporte para navegadores antigos, deve considerar usar um formato mais otimizado, como WEBP ou AVIF.
- **Licenças de imagem**: Uma imagem em domínio público não tem direitos autorais e pode ser usada livremente sem restrições. Imagens licenciadas especificamente sob a licença Creative Commons 0 são consideradas domínio público. Algumas imagens podem ser liberadas sob uma licença permissiva, como uma licença Creative Commons ou a licença BSD que o freeCodeCamp usa.
- **SVGs**: Scalable Vector Graphics rastreiam dados baseados em caminhos e equações para traçar pontos, linhas e curvas. Isso significa que um gráfico vetorial, como um SVG, pode ser redimensionado para qualquer tamanho sem perder qualidade.
...
- **Atributo `autoplay`**: O atributo `autoplay` é um atributo booleano que faz o áudio ou vídeo começar a tocar automaticamente assim que puder sem interrupção.
...
- **Atributo `target`**: Este atributo informa ao navegador onde abrir a URL para o elemento âncora. Existem quatro valores importantes possíveis para este atributo: `_self`, `_blank`, `_parent` e `_top`. Há um quinto valor, chamado `_unfencedTop`, que atualmente é usado para a API experimental `FencedFrame`. Provavelmente você ainda não terá motivo para usar este.
- **Valor `_self`**: Este é o valor padrão para o atributo `target`. Ele abre o link no contexto de navegação atual. Na maioria dos casos, será a aba ou janela atual.
...
- **Valor `_parent`**: Abre o link no contexto pai do contexto atual. Por exemplo, se seu site tem um iframe, um valor `_parent` nesse iframe abriria no separador/janela do seu site, não no frame incorporado.

```html
<a href="https://freecodecamp.org" target="_parent">Visit freeCodeCamp</a>
```

- **Valor `_top`**: Abre o link no contexto de navegação mais alto, pense como "o pai do pai". Isso é semelhante ao `_parent`, mas o link sempre abrirá na aba/janela completa do navegador, mesmo para frames incorporados aninhados.
...
- **Caminho absoluto**: Um caminho absoluto é uma rota completa para um recurso dentro de um sistema de arquivos. Ele começa a partir do diretório raiz, inclui todos os outros diretórios e finalmente o nome do arquivo e extensão. O "diretório raiz" se refere ao diretório ou pasta de nível mais alto em uma hierarquia.
- **URL absoluta**: Uma URL absoluta é baseada no caminho absoluto para recursos na web. Além do caminho, inclui o protocolo, que pode ser `http`, `https` ou `file`, e o nome do domínio. Aqui está um exemplo de uma URL absoluta que linka para o logo do freeCodeCamp:
...
- **`:link`**: Este é o estado padrão. Representa um link que o usuário ainda não visitou, clicou ou interagiu. Você pode pensar nesse estado como o estilo base para todos os links na sua página. Os outros estados se baseiam nele.
- **`:visited`**: Aplica-se quando o usuário já visitou a página vinculada. Por padrão, isso deixa o link roxo, mas você pode usar CSS para fornecer uma indicação visual diferente ao usuário.
- **`:hover`**: Aplica-se quando o usuário está passando o cursor sobre um link. Esse estado ajuda a dar mais atenção ao link, para garantir que o usuário realmente pretende clicar.
- **`:focus`**: Aplica-se quando focamos em um link.
- **`:active`**: Aplica-se a links que estão sendo ativados pelo usuário. Normalmente significa clicar no link com o botão principal do mouse, geralmente o clique esquerdo.
