...
Includi gli argomenti rimanenti `chosenCard.name_short` e `chosenCard.img`. Poi, esternamente alla funzione `renderCard`, azzera la proprietà `multipleCard` dell'elemento `innerHTML` impostandola su una stringa vuota.

# --hints--

Dovresti passare `chosenCard.name_short` come argomento a `renderCard`.
...
Dovresti passare `chosenCard.img` come ultimo argomento a `renderCard`.
...
Dovresti azzerare `this.elements.multipleCard.innerHTML` impostandolo su una stringa vuota.
