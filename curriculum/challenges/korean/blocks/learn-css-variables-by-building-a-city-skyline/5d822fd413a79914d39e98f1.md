...
다음 두 섹션에 같은 그라디언트를 추가하려고 합니다. 대신, `bb1-window`라는 새 클래스 선택자를 만들고 `height`에서 `background`과 `.bb1a` 속성과 값을 새 클래스 선택자로 옮기세요.

# --hints--

`bb1-window`라는 새 클래스 선택자를 만들어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

`height`에서 `.bb1a` 속성과 값을 `.bb1-window`로 옮겨야 합니다.
...
`background`에서 `.bb1a` 속성과 값을 `.bb1-window`로 옮겨야 합니다.
