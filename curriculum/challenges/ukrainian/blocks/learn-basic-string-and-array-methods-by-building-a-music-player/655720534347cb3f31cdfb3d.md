...
Перед тим як видалити пісню, потрібно перевірити, чи вона зараз відтворюється. Якщо так, потрібно призупинити пісню і відтворити наступну пісню у плейлисті.

Використайте інструкцію `if`, щоб перевірити, чи `userData?.currentSong?.id` дорівнює `id` пісні, яку ви хочете видалити.

# --hints--

Ви маєте створити порожню інструкцію `if` з умовою `userData?.currentSong?.id === id`.
