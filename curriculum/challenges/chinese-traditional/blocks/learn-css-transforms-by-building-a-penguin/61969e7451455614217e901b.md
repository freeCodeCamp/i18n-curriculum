...
要使山看起來更像一座山，可以使用 `skew` 變換函數，它需要兩個參數。 第一個是沿 x 軸傾斜的角度，第二個是沿 y 軸傾斜的角度。

使用 `transform` 屬性來使山體沿 x 軸傾斜 `0deg`，沿 y 軸傾斜 `44deg`。
