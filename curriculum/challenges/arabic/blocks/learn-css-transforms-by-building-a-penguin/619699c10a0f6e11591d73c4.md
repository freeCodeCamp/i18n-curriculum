...
استخدم خاصية `margin` لمركزة العنصر `.penguin` أفقيًا، واضبط `margin-top` على `75px`.

# --hints--

يجب أن تعطي `.penguin` خاصية `margin` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
...
يجب أن تعطي `.penguin` خاصية `margin-top` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
