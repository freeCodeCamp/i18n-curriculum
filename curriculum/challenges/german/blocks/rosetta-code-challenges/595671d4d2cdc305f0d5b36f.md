---
id: 595671d4d2cdc305f0d5b36f
title: Hash aus zwei Arrays
challengeType: 1
forumTopicId: 302283
dashedName: hash-from-two-arrays
---

# --description--

Gegeben sind zwei Arrays. Erstellen Sie ein Hash-Objekt, bei dem die Elemente des einen Arrays (die Schlüssel) mit den Elementen des anderen Arrays (die Werte) verknüpft sind.

Beispiel:

```js
const firstArr = [1, 2, 3];
const secondArr = ["a", "b", "c"];
const hashObj = arrToObj(firstArr, secondArr);
```

Das aus den obigen Arrays erstellte Hash-Objekt wird `{1: "a", 2: "b", 3: "c"}` sein.

# --before-each--

```js
const testCases = [
  [[1, 2, 3, 4, 5], ['a', 'b', 'c', 'd', 'e']],
  [[1, 2, 3, 4, 5], ['a', 'b', 'c', 'd']],
  [[1, 2, 3], ['a', 'b', 'c', 'd', 'e']],
  [['a', 'b', 'c', 'd', 'e'], [1, 2, 3, 4, 5]],
  [['a', 'b', 'c', 'd', 'e'], [1, 2, 3, 4]],
  [['a', 'b', 'c'], [1, 2, 3, 4, 5]]
];

const res = [
  { 1: 'a', 2: 'b', 3: 'c', 4: 'd', 5: 'e' },
  { 1: 'a', 2: 'b', 3: 'c', 4: 'd', 5: undefined },
  { 1: 'a', 2: 'b', 3: 'c' },
  { a: 1, b: 2, c: 3, d: 4, e: 5 },
  { a: 1, b: 2, c: 3, d: 4, e: undefined },
  { a: 1, b: 2, c: 3 }
];
```

# --hints--

`arrToObj` sollte eine Funktion sein.

```js
assert(typeof arrToObj === 'function');
```

`arrToObj([1, 2, 3, 4, 5], ["a", "b", "c", "d", "e"])` sollte `{ 1: "a", 2: "b", 3: "c", 4: "d", 5: "e" }` zurückgeben.

```js
assert.deepEqual(arrToObj(...testCases[0]), res[0]);
```

`arrToObj([1, 2, 3, 4, 5], ["a", "b", "c", "d"])` sollte `{ 1: "a", 2: "b", 3: "c", 4: "d", 5: undefined }` zurückgeben.

```js
assert.deepEqual(arrToObj(...testCases[1]), res[1]);
```

`arrToObj([1, 2, 3], ["a", "b", "c", "d", "e"])` sollte `{ 1: "a", 2: "b", 3: "c" }` zurückgeben.

```js
assert.deepEqual(arrToObj(...testCases[2]), res[2]);
```

`arrToObj(["a", "b", "c", "d", "e"], [1, 2, 3, 4, 5])` sollte `{ "a": 1, "b": 2, "c": 3 , "d": 4, "e": 5 }` zurückgeben.

```js
assert.deepEqual(arrToObj(...testCases[3]), res[3]);
```

`arrToObj(["a", "b", "c", "d", "e"], [1, 2, 3, 4])` sollte `{ "a": 1, "b": 2, "c": 3 , "d": 4, "e": undefined }` zurückgeben.

```js
assert.deepEqual(arrToObj(...testCases[4]), res[4]);
```

`arrToObj(["a", "b", "c"], [1, 2, 3, 4, 5])` sollte `{ "a": 1, "b": 2, "c": 3 }` zurückgeben.

```js
assert.deepEqual(arrToObj(...testCases[5]), res[5]);
```

# --seed--

## --seed-contents--

```js
function arrToObj (keys, vals) {

  return true;
}
```

# --solutions--

```js
function arrToObj (keys, vals) {
  return keys.reduce((map, key, index) => {
    map[key] = vals[index];
    return map;
  }, {});
}
```
