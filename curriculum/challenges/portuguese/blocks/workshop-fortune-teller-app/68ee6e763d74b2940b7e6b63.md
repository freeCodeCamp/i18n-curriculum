...
O app de cartomante suporta renderizar múltiplas cartas, então você precisa ter um método que possa ajudar a renderizar várias cartas. Declare um método chamado `multipleCardSelected`.

Dentro do método, faça a chamada da função `hideElements` e passe as propriedades `singleCard`, `singleCardBtn`, `multipleCardsBtn` e `headerTitle`. Não esqueça que você precisa acessá-las a partir de `this.elements`.

# --hints--

Você deve declarar um método `multipleCardSelected`.
...
Você deve chamar a função `hideElements` dentro do método `multipleCardSelected`.
...
Você deve passar `singleCard` como primeiro argumento para a função `hideElements`.
...
Você deve passar `singleCardBtn` como argumento para a função `hideElements`.
...
Você deve passar `multipleCardsBtn` como argumento para a função `hideElements`.
...
Você deve passar `headerTitle` como argumento para a função `hideElements`.
