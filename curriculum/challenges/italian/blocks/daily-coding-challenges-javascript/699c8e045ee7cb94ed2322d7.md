---
id: 699c8e045ee7cb94ed2322d7
title: "Sfida 216: il giorno di pi greco"
challengeType: 28
dashedName: challenge-216
---

# --description--

Buon giorno di pi greco (π)!

Dato un numero intero (`n`), dove `n` è compreso tra 1 e 1000 (inclusi), restituisci la `n`esima cifra decimale di π.

- Assicurati di restituire un numero e non una stringa.

π con le sue prime cinque cifre decimali è 3.14159. Quindi, dato `5` per esempio, restituisci `9`, la quinta cifra decimale.

Potresti dover trovare da qualche parte le prime 1000 cifre decimali di π.

# --hints--

`getPiDecimal(5)` dovrebbe restituire `9`.

```js
assert.strictEqual(getPiDecimal(5), 9);
```

`getPiDecimal(10)` dovrebbe restituire `5`.

```js
assert.strictEqual(getPiDecimal(10), 5);
```

`getPiDecimal(22)` dovrebbe restituire `6`.

```js
assert.strictEqual(getPiDecimal(22), 6);
```

`getPiDecimal(39)` dovrebbe restituire `7`.

```js
assert.strictEqual(getPiDecimal(39), 7);
```

`getPiDecimal(76)` dovrebbe restituire `2`.

```js
assert.strictEqual(getPiDecimal(76), 2);
```

`getPiDecimal(384)` dovrebbe restituire `4`.

```js
assert.strictEqual(getPiDecimal(384), 4);
```

`getPiDecimal(601)` dovrebbe restituire `0`.

```js
assert.strictEqual(getPiDecimal(601), 0);
```

`getPiDecimal(1000)` dovrebbe restituire `9`.

```js
assert.strictEqual(getPiDecimal(1000), 9);
```

# --seed--

## --seed-contents--

```js
function getPiDecimal(n) {

  return n;
}
```

# --solutions--

```js
function getPiDecimal(n) {
  const pi = ""

  return Number(pi[n]);
}
```
