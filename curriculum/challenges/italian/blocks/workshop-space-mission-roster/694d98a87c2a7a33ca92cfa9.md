...
Usa un loop `for` per registrare il `name` di ogni astronauta nell'array `updatedCrew`. Dopo il loop, restituisci l'array `updatedCrew` per completare la tua funzione `swapCrewMembers`.

# --hints--

Dovresti usare un loop `for` per iterare su l'array `updatedCrew` e registrare il `name` di ogni astronauta.
...
Dovresti restituire l'array `updatedCrew` dopo aver registrato i nomi degli astronauti.
