...
在第二個 `tr` 中，添加一個帶有文字 `Expenses Annual anticipated expenses, such as payroll.` 的 `th` 元素。將該文字中除 `Expenses ` 之外的部分包裹在一個 `class` 設為 `description` 的 `span` 元素中。

在其下方添加三個 `td` 元素，並依序為它們設定以下文字：`$200`、`$300` 和 `$400`。將第三個 `td` 元素的 `class` 設定為 `current`。
...
你的 `th` 元素應該有文字 `Expenses Annual anticipated expenses, such as payroll.`
...
你應該將文字 `Annual anticipated expenses, such as payroll.` 包裹在 `span` 元素中。
