...
Ви не можете використовувати `didn't` з деякими дієсловами, щоб утворити заперечення в минулому часі. Один із прикладів — дієслово `to be`.

Якщо ви хочете створити заперечне речення з дієсловом `to be` у минулому, просто додайте `not` після `was` або `were`. Наприклад:
...
`Wasn't` — це скорочення від `was not`, а `weren't` — скорочення від `were not`.
...
Це слово використовується для утворення заперечної інструкції в минулому часі.
