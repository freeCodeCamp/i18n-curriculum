...
クラスセレクターを使って `.cat-whisker-right-middle` 要素を指定し、右の中間ウィスカーを `position` の `absolute`、`top` の `127px`、および `left` の `109px` で位置調整してください。

# --hints--

`.cat-whisker-right-middle` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle'))
```

`.cat-whisker-right-middle` セレクターには `position` に設定された `absolute` プロパティが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.position === 'absolute')
```

`.cat-whisker-right-middle` セレクターには `top` に設定された `127px` プロパティが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.top === '127px')
```

`.cat-whisker-right-middle` セレクターには `left` に設定された `109px` プロパティが必要です。
