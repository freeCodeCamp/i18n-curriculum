...
Kufuata muundo uleule wa `cartSubTotal`, sahihisha `cartTaxes` ili ionyeshe thamani ya `tax`, na `cartTotal` ionyeshe sifa ya `total`.

# --hints--

Unapaswa kupata upatikanaji wa sifa ya `textContent` ya kipengele cha `cartTaxes`.

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

Unapaswa kuita njia ya `.toFixed()` kwenye kigezo cha `tax`, ukipitisha `2` kama hoja.

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

Unapaswa kutumia sintaksia ya mfuatano wa herufi wa template kuongeza alama ya dola kabla ya wito wako wa `.toFixed()`.
...
Unapaswa kuweka `textContent` ya kipengele cha `cartTaxes` kuwa mfuatano wa template wa `tax.toFixed`.
...
Unapaswa kupata upatikanaji wa sifa ya `textContent` ya kipengele cha `cartTotal`.

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

Unapaswa kuita njia ya `.toFixed()` kwenye kigezo cha `total`, ukipitisha `2` kama hoja. Kumbuka kutumia neno kuu la `this`.
