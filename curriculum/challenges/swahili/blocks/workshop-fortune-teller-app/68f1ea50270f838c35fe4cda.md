...
Sasa fanya vivyo hivyo kwa `subTitle` kwa kuweka thamani yake kwa `meaning_up`, na kwa `title` weka thamani yake kwa `foundCard.name`. Baada ya hapo, wito njia ya `showElements` na pitisha `fortuneDescription` kutoka kwa elements.

# --hints--

Unapaswa kuweka `this.elements.subTitle.textContent` kuwa `foundCard.meaning_up`.
...
Unapaswa kuweka `this.elements.title.textContent` kuwa `foundCard.name`.
...
Unapaswa kuita kitendakazi cha `showElements` ndani ya `showFortune`.
...
Unapaswa kupitisha `this.elements.fortuneDescription` kama hoja kwa `showElements`.
