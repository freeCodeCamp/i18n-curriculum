...
Ora, crea una funzione chiamata `getNextSong`. Deve restituire la canzone che si trova a `getCurrentSongIndex()` più uno in `userData.songs`, oppure `undefined` se la canzone corrente è l’ultima nella playlist.

# --hints--

Dovresti avere una funzione chiamata `getNextSong`.

```js
assert.isFunction(getNextSong);
```

La tua funzione `getNextSong` dovrebbe chiamare `getCurrentSongIndex`.
...
La tua funzione `getNextSong` dovrebbe restituire la canzone successiva a quella corrente.
...
La tua funzione `getNextSong` dovrebbe restituire `undefined` quando la canzone corrente è l’ultima nella playlist.
