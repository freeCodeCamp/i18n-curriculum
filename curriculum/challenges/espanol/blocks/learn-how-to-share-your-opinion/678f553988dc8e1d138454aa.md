...
`Channel` is used to refer to a communication tool where team members can exchange messages or information. Por ejemplo:

`We discuss project updates on the Slack channel.` - This means that you use the Slack platform to talk about updates.

`Poll` refers to a process of gathering opinions by asking a group of people to choose from various options. Por ejemplo:
...
`Votes` are the selections made by participants in a poll, representing their opinion. Por ejemplo:
