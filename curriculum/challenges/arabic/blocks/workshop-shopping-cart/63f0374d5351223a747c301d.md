...
باتباع نفس النمط المستخدم في `cartSubTotal`، حدّث `cartTaxes` لعرض قيمة `tax`، و`cartTotal` لعرض الخاصية `total`.

# --hints--

يجب الوصول إلى الخاصية `textContent` لعنصر `cartTaxes`.

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

يجب استدعاء الطريقة `.toFixed()` على المتغير `tax`، مع تمرير `2` كمعلمة.

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

يجب استخدام صيغة القالب النصي لإضافة علامة الدولار قبل استدعاء `.toFixed()`.
...
يجب تعيين الخاصية `textContent` لعنصر `cartTaxes` إلى سلسلة القالب `tax.toFixed`.
...
يجب الوصول إلى الخاصية `textContent` لعنصر `cartTotal`.

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

يجب استدعاء الطريقة `.toFixed()` على المتغير `total`، مع تمرير `2` كمعلمة. تذكر استخدام الكلمة المفتاحية `this`.
...
يجب تعيين الخاصية `textContent` لعنصر `cartTotal` إلى سلسلة القالب `total.toFixed`.
