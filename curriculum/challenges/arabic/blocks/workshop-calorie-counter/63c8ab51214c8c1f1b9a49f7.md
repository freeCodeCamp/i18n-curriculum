...
في دالتك الجديدة، أعلن عن متغير `calories` وقم بتعيينه القيمة `0`. استخدم `let` لإعلانه، لأنك ستُعيد تعيينه لاحقًا.

# --hints--

يجب أن تعلن عن متغير `calories`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

يجب أن يُعيَّن لمتغير `calories` القيمة `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

يجب أن تعلن عن متغير `calories` باستخدام `let`.
