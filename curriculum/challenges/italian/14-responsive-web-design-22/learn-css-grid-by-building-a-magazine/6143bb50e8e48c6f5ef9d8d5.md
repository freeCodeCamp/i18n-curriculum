...
Dovresti creare tre elementi `img` all'interno dell'elemento `aside`.
...
L'elemento `blockquote` dovrebbe avere l'attributo `class` impostato su `image-quote`.

```js
assert(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

I nuovi elementi dovrebbero essere nell'ordine corretto.
