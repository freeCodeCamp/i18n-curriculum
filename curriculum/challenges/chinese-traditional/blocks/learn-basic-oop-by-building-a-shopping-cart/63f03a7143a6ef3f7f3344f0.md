...
你只有在使用者確認提示時才想清除購物車。創建一個 `if` 述語來檢查使用者是否確認了提示。

在 `if` 述語中，將 `items` 屬性設回空陣列，然後將 `total` 屬性設為 `0`。

# --hints--

你應該創建一個 `if` 述語來檢查使用者是否確認了提示。請記住你可以直接檢查 `isCartCleared` 的真值。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

你的 `if` 述語應該將 `items` 屬性設回空陣列。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

你的 `if` 述語應該將 `total` 屬性設為 `0`。
