...
Sarebbe meglio far apparire il testo `label` sopra gli elementi del modulo.

Seleziona tutti gli elementi `input`, `textarea` e `select` e fai in modo che occupino tutta la larghezza dei loro elementi genitori.

Inoltre, aggiungi un `10px` di `margin` nella parte superiore degli elementi selezionati. Imposta gli altri margini su `0`.
...
Dovresti impostare la proprietà `width` su `100%`.
...
Dovresti impostare la proprietà `margin-top` su `10px`.
...
Dovresti impostare la proprietà `margin-bottom` su `0`.
...
Dovresti impostare la proprietà `margin-left` su `0`.
...
Dovresti impostare la proprietà `margin-right` su `0`.
