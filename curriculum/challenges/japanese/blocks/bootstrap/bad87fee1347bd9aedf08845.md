...
次に、無効なリンクを含む`p`要素を削除してください。そして、`h2`要素から`red-text`クラスを削除し、代わりにブートストラップの`text-primary`クラスを追加してください。

最後に、最初の`img`要素から`smaller-image`クラスを削除し、`img-responsive`クラスに置き換えてください。
