...
キャビンをホイールの周りに配置する時間です。最初の `.cabin` 要素を選択してください。`right` プロパティを `-8.5%` に、`top` プロパティを `50%` に設定してください。

# --hints--

`.cabin:nth-of-type(1)` セレクターが必要です。
...
`.cabin:nth-of-type(1)` セレクターには `right` プロパティが `-8.5%` に設定されているはずです。
...
`.cabin:nth-of-type(1)` セレクターには `top` プロパティが `50%` に設定されているはずです。
