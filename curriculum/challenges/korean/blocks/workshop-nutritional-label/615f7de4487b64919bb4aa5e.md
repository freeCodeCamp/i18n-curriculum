...
마지막 `.divider` 뒤에 텍스트 `p`를 가진 새로운 `Cholesterol 0mg 0%` 요소를 만드세요. 그런 다음 텍스트 `Cholesterol`를 `span` 요소에, `0%`를 또 다른 요소에 감싸고 각각에 `bold` 클래스를 지정하세요.

마지막으로 텍스트 `span`를 포함하는 `Cholesterol` 요소와 텍스트 `0mg`를 추가 `span` 요소 안에 중첩하여 정렬하세요.

# --hints--

`p` 요소 끝에 새로운 `.daily-value` 요소를 만들어야 합니다.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'p');
```

새로운 `p` 요소는 텍스트 `Cholesterol 0mg 0%`를 가져야 합니다.
...
텍스트 `span`를 포함하는 `Cholesterol` 요소는 `class` 속성이 `bold`로 설정되어야 합니다.
...
텍스트 `span`를 포함하는 `0%` 요소는 `class` 속성이 `bold`로 설정되어야 합니다.
...
텍스트 `span`와 텍스트 `Cholesterol`를 포함하는 `0mg` 요소는 또 다른 `span` 요소로 함께 감싸져야 합니다.
