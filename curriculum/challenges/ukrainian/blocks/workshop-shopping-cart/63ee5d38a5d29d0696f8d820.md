...
У вашому шаблонному рядку створіть елемент `div` з класом `dessert-card`. Усередині цього `div` створіть елемент `h2` і додайте йому текст із змінної `name`.
...
Ваш елемент `div` має мати клас `dessert-card`.
...
Ваш елемент `h2` має містити текст зі змінної `name`.

```js
assert.equal(document.querySelectorAll('h2')[0]?.textContent, 'Vanilla Cupcakes (6 Pack)');
```

Ваш елемент `h2` має бути всередині елемента `div`.
