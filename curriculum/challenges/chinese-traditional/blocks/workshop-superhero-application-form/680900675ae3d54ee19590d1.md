...
接著，告訴表單如何提交。為你的表單元素添加一個 `method` 屬性，值為 `post`，以及一個 `action` 屬性，值為 `https://superhero-application-form.freecodecamp.org`。

現在，提交表單將會將它傳送到該 URL。你會在下一個步驟中執行該操作。
...
你的 `form` 元素應該有一個 `action` 屬性，設定為 `https://superhero-application-form.freecodecamp.org`。
