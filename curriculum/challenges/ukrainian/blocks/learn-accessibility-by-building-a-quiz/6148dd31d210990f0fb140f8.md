...
Під час розробки корисно бачити кордон за замовчуванням навколо елементів `fieldset`. Однак це може бути не той стиль, який ви хочете.

Видаліть кордон і нижній відступ у елементів `.question`.
...
Ви повинні задати `.question` відступ `border` розміром `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

Ви повинні задати `.question` відступ `padding-bottom` розміром `0`.
