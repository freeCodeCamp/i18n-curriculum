...
Ora è il momento di usare la tua funzione `handleToggleVisibility`.

Nelle lezioni precedenti, hai imparato il sistema di eventi sintetici di React e come lavorare con attributi come `onClick` e `onSubmit`.

Ricorda che puoi passare un riferimento a una funzione a un attributo `onClick` in questo modo:
...
Aggiorna il tuo elemento `button` esistente per aggiungere un attributo `onClick` e passare un riferimento a una funzione `handleToggleVisibility`.

Ora, quando clicchi sul pulsante, vedrai la visibilità del testo del paragrafo alternarsi tra nascosta e mostrata sulla pagina.

# --hints--

Quando si clicca sul `toggle-button`, il messaggio `"I love freeCodeCamp!"` dovrebbe apparire sullo schermo.
