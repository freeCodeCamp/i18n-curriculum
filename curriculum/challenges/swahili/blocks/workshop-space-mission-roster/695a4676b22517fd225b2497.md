...
Mission Control inahitaji kitendakazi kingine kwa ajili ya kurekodi muhtasari wa kikosi chako. Unda kitendakazi tupu kinachoitwa `printCrewSummary` kinachokubali kigezo `crew`.

# --hints--

Unapaswa kuunda kitendakazi kinachoitwa `printCrewSummary`.

```js
assert.isFunction(printCrewSummary);
```

Kitendakazi chako cha `printCrewSummary` kinapaswa kuwa na kigezo `crew`.
