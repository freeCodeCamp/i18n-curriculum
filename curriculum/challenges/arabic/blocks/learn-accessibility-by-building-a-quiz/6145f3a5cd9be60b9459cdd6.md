...
اربط أول عنصر `label` بعنصر `select`، وأضف خاصية `name` لعنصر `select`.

# --hints--

يجب أن تضيف خاصية `for` لعنصر `label`.

```js
assert.notEmpty(document.querySelector('.question-block > label')?.htmlFor);
```

يجب أن تضيف خاصية `id` لعنصر `select`.

```js
assert.notEmpty(document.querySelector('.answer > select')?.id);
```

يجب أن تكون خاصية `id` لعنصر `select` مطابقة لخاصية `for` في عنصر `label`.

```js
assert.equal(document.querySelector('.answer > select')?.id, document.querySelector('.question-block > label')?.htmlFor);
```

يجب أن تضيف خاصية `name` لعنصر `select`.
