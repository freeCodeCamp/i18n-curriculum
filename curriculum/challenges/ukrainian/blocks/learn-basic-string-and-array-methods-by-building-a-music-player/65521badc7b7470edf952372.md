...
Тепер вам потрібно попрацювати над призупиненням поточної пісні.

Визначте функцію `pauseSong`, використовуючи ключове слово `const` та синтаксис стрілкової функції. Функція не повинна приймати параметрів.

# --hints--

Використайте `const`, щоб створити порожню функцію під назвою `pauseSong`.
...
Функція `pauseSong` повинна використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Функція `pauseSong` має бути порожньою.
