...
Cree un nuevo selector `25%` entre sus selectores `0%` y `50%`. Dé a este nuevo selector la propiedad `background-color` establecida en `yellow`.

# --hints--

Debe crear un nuevo selector `25%` en su regla `@keyframes cabins`.
...
Su selector `25%` debería estar entre sus selectores `0%` y `50%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText , '25%');
```

El selector `25%` debería tener una propiedad `background-color` establecida en `yellow`.
