...
Wenn `target` leer ist, sollte die Scheibe unbedingt von `source` nach `target` verschoben werden.

Fügen Sie nach der Deklaration von `forward` eine `if`-Anweisung hinzu, um zu prüfen, ob `rods[target]` leer ist. Falls ja, ändern Sie `forward` zu `True`.

# --hints--

Sie sollten eine `if`-Anweisung haben, um zu prüfen, ob `rods[target]` leer ist, und `True` `forward` zuweisen.
