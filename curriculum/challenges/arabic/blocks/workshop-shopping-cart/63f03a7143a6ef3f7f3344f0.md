...
تريد مسح السلة فقط إذا أكد المستخدم الرسالة. أنشئ جملة `if` تتحقق مما إذا أكد المستخدم الرسالة.

في جملة الـ `if`، أعد تعيين خاصية `items` إلى مصفوفة فارغة، ثم عيّن خاصية `total` إلى `0`.

# --hints--

يجب أن تنشئ جملة `if` تتحقق مما إذا أكد المستخدم الرسالة. تذكر أنه يمكنك التحقق مباشرة من صحة `isCartCleared`.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

يجب أن تعيد جملة الـ `if` تعيين خاصية `items` إلى مصفوفة فارغة.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

يجب أن تعيد جملة الـ `if` تعيين خاصية `total` إلى `0`.
