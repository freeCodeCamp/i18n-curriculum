...
Під вашим елементом `.double-indent` додайте новий елемент `p` з текстом `Includes 10g Added Sugars 20%`. Ваш новий елемент `p` також має бути подвійно відступлений і не мати нижнього кордону. Використайте `span`, щоб зробити `20%` жирним і вирівняти його по правому краю.
...
Ваш новий елемент `p` має містити текст `Includes 10g Added Sugars 20%`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Includes 10g Added Sugars[\s|\n]+20%/));
```

Ваш новий елемент `p` має мати атрибут `class`, встановлений у `double-indent no-divider`.
