...
**Soundex 算法** 涉及 *意图* 词语。 它为相似的发音单词创建一个表示。 

它用于查找<em>名称</em>和<em>地址</em>。这意味着填写<em>名称</em>的人可以专注于它的发音，而不是纠正<em>名称</em>的拼写。

例如： 

如果你第一次听到名字 `Quenci` 并且写错了它， 您将得到 **Soundex** 代码 `Q520` 

当你下次正确拼写名称 `Quincy` 时，你仍然会得到相同的代码 `Q520`，这意味着你可以将多个名称发音链接到同一个<em>person</em>，而无需为每个拼写添加。 
