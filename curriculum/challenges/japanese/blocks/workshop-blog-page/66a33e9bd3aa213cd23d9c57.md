...
このステップでは、ブログの著者の電話番号とメールアドレスを追加する必要があります。

`address` 要素の中に、`Phone: 555-555-5555` のテキストを持つ段落要素を追加してください。

その段落要素の下に、`Email: fake@email.com` のテキストを持つ別の段落要素を追加してください。

# --hints--

`p` 要素の中に `address` 要素が2つあるはずです。
...
最初の `p` 要素には `Phone: 555-555-5555` のテキストがあるはずです。
...
2つ目の `p` 要素には `Email: fake@email.com` のテキストがあるはずです。
