...
Gnome sortは<a href="https://rosettacode.org/wiki/Insertion_sort" target="_blank" rel="noopener noreferrer nofollow">Insertion sort</a>に似たソートアルゴリズムですが、要素を適切な位置に移動させる際に<a href="https://rosettacode.org/wiki/Bubble" target="_blank" rel="noopener noreferrer nofollow">Bubble Sort</a>のように一連の交換操作で行う点が異なります。

このアルゴリズムの疑似コードは以下の通りです：
...
上記の疑似コードを実装する関数を書いてください。関数はソート済みの配列を返す必要があります。

# --hints--

`gnomeSort` は関数である必要があります。
