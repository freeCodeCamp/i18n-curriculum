...
Das Formular ist jetzt vollständig. Damit die Vorschau-Karte sich beim Tippen aktualisiert, müssen Sie die Eingaben mit dem `player`-Zustand verbinden.

Beginnen Sie mit dem Name-`input`. Fügen Sie ihm ein `value`-Attribut mit dem Wert `player.name` hinzu. Dann fügen Sie ein `onChange`-Attribut hinzu, das auf eine Pfeilfunktion gesetzt ist, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `name` auf `e.target.value` gesetzt wird. Vergessen Sie nicht, sowohl die Ausbreitung als auch die `name`-Wertzuweisung in ein Objekt zu packen.

# --hints--

Ihr Name-Eingabefeld `value` sollte an das `name`-Feld des player-Zustands gebunden sein.
...
Ihr Name-Eingabefeld sollte einen `onChange`-Handler haben.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Der `onChange`-Handler sollte `setPlayer` mit `setPlayer` aufrufen, das ein Objekt enthält, das `player` ausbreitet und `name` auf `e.target.value` setzt.
