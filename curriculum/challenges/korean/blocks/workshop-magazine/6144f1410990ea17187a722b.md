...
이제 그리드 레이아웃을 스타일링할 수 있습니다. CSS Grid는 부모 요소와 자식 요소 모두에 특별한 속성이 있다는 점에서 Flexbox와 비슷합니다.

이 경우, 부모 요소는 `main` 요소입니다. `grid-template-columns` 속성을 `1fr 94rem 1fr` 값으로 추가하여 콘텐츠를 3열 레이아웃으로 설정하세요. 이렇게 하면 가운데 열은 `94rem` 너비이고, 첫 번째와 마지막 열은 그리드 컨테이너의 나머지 공간 중 각각 1분수만큼 차지하는 세 개의 열이 만들어집니다.

# --hints--

`main` 섹션에 `grid-template-columns` 속성을 `1fr 94rem 1fr`으로 설정하세요.
