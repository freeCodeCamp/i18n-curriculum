...
「バブルアップ」結果を得るには、現在の要素が次の要素より大きいかどうかをチェックする必要があります。これは、`array` を `j` と `j+1` でアクセスすることで行えます。

現在の要素が次の要素より大きいかどうかをチェックする `if` 条件を作成してください。

# --hints--

`console.log` 文は削除しないでください。
...
`if` 文は `array[j]` が `array[j+1]` より大きいかどうかをチェックする必要があります。
