...
Наступний `input` — це прапорець, щоб позначити всі супергеройські здібності, які підходять користувачу.

Створіть `label` елемент з `className` рівним `section` і `column`, а також текстом `List your powers (select all that apply):`.

# --hints--

Ви повинні створити `label` елемент з `className` рівним `section` і `column`.
...
Ваш `label` елемент має містити текст `List your powers (select all that apply):`.
