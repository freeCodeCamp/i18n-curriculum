...
Tu función de callback debe tener una sentencia `if`.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

Tu sentencia `if` debe verificar si el `value` de `messageInput` es una cadena vacía.
...
Tu sentencia `if` debe mostrar una alerta al usuario con el mensaje `"Please enter a message."`.
...
Tu sentencia `if` debe salir de la ejecución de la función.
