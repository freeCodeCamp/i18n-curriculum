...
ホバーされたときに `.card-links a` 要素を対象にする疑似クラスセレクターを作成してください。

`background-color` は `orangered` に変わるはずです。

# --hints--

`.card-links a:hover` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:hover"));
```

`.card-links a:hover` セレクターには `background-color` プロパティが `orangered` に設定されているはずです。
