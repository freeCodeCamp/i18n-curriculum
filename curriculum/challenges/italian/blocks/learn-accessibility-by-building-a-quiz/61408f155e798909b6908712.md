...
Per abilitare la navigazione nella pagina, aggiungi una lista non ordinata con i seguenti tre elementi della lista:

- `INFO`
- `HTML`
- `CSS`

Il testo degli elementi della lista deve essere racchiuso in tag àncora.

# --hints--

Dovresti annidare un elemento `ul` all'interno di `nav`.

```js
assert.equal(document.querySelectorAll('nav > ul')?.length, 1);
```

Dovresti annidare tre elementi `li` all'interno dell'elemento `ul`.

```js
assert.equal(document.querySelectorAll('nav > ul > li')?.length, 3);
```

Dovresti annidare un elemento `a` all'interno di ogni elemento `li`.
