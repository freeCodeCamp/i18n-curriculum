...
Kabla ya kuanza uigaji, unahitaji kitu kufuatilia ni wangapi washiriki kila lango linashughulikia katika alama zote za kupigia. Fanya wito wa `initializeThroughput()` na `gates` na weka thamani ya matokeo katika kigezo kinachoitwa `throughputSummary`.

# --hints--

Unapaswa kufanya wito wa `initializeThroughput()` na `gates` na weka thamani ya matokeo katika kigezo kinachoitwa `throughputSummary`.
...
Kitendakazi chako cha `simulateFestival` kinapaswa kufanya wito wa kitendakazi cha `initializeThroughput()` na `gates`.
