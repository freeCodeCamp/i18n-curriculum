...
أخيرًا، بعد عناصر `p`، أنشئ عنصر `button`. عرّف له `id` بقيمة المتغير `id`، و`class` بقيمة `btn add-to-cart-btn`، واستخدم النص `"Add to cart"`.
...
يجب أن يأتي عنصر `button` بعد عناصر `p`.
