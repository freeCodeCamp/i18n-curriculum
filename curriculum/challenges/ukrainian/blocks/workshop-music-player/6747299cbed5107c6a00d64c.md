...
Залишок пісень уже додано до масиву `allSongs` для вас.

У попередніх уроках ви дізналися про <dfn>Web Audio API</dfn> і як його використовувати для відтворення пісень. Усі сучасні браузери підтримують Web Audio API, який дозволяє генерувати та обробляти аудіо у веб-застосунках.

Створіть змінну з ім’ям `audio` і призначте їй значення `new Audio()`. Це створить новий HTML5 елемент `audio`.

# --hints--

У вас має бути змінна з ім’ям `audio`.

```js
assert.isDefined(audio);
```

Ви маєте призначити новий екземпляр об’єкта `Audio` змінній з ім’ям `audio`.
