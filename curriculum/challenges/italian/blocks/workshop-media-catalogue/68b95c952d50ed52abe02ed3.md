...
Dai alla tua classe `MediaError` un metodo `__init__` con i seguenti parametri: `self`, `message` e `obj`.

Al suo interno, chiama `super().__init__(message)` per assicurarti che i meccanismi per memorizzare e mostrare il messaggio di errore, già presenti nella classe `Exception`, funzionino correttamente.

Dopodiché, assegna `obj` a `self.obj`.

# --hints--

La tua classe `MediaError` dovrebbe avere un metodo `__init__` con i parametri `self`, `message` e `obj`.
...
Dovresti chiamare `super().__init__(message)` all’interno del tuo metodo `__init__`.
...
Dovresti assegnare `obj` a `self.obj` all’interno del tuo metodo `__init__`.
