...
Crea un nuevo selector `.left-container p` estableciendo el margen superior e inferior en `-5px`, y el margen izquierdo y derecho en `-2px`. También establece el `font-size` en `2em` y el `font-weight` en `700`.

# --hints--

Debe tener un nuevo selector `.left-container p`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

Su nuevo selector `.left-container p` debe tener una propiedad `margin` configurada en `-5px -2px`.
...
Su nuevo selector `.left-container p` debe tener una propiedad `font-size` configurada en `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

Su nuevo selector `.left-container p` debe tener una propiedad `font-weight` configurada en `700`.
