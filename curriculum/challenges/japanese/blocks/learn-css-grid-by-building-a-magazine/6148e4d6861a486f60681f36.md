...
画像は2列3行のレイアウト内に配置してください。

`.image-wrapper` セレクターに `grid-template-columns` プロパティを `2fr 1fr` に、`grid-template-rows` プロパティを `repeat(3, min-content)` に設定してください。これにより、コンテンツに応じて高さが調整されるグリッドの行と、コンテナの幅に基づいて固定幅の列が作成されます。

# --hints--

`.image-wrapper` セレクターには `grid-template-columns` プロパティを `2fr 1fr` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns === '2fr 1fr');
```

`.image-wrapper` セレクターには `grid-template-rows` プロパティを `repeat(3, min-content)` に設定してください。
