...
Agora, chame a função `chooseShape()` com `val.value` para mostrar os campos de entrada corretos, depois chame `updateResult()` para recalcular e exibir a área.

# --hints--

Você deve chamar a função `chooseShape` com `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

Você deve chamar a função `updateResult()`.
