...
Dovresti dare a `.penguin-body::before` una proprietà `top` di `--fcc-expected--`, invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

Dovresti dare a `.penguin-body::before` una proprietà `left` di `--fcc-expected--`, invece di `--fcc-actual--`.
