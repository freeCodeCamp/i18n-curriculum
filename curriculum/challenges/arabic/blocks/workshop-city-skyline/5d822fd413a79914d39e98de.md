...
أنشئ متغيرًا جديدًا تحت متغير `--building-color1`. سمِّ المتغير الجديد `--building-color2` وأعطه القيمة `#66cc99`. ثم اجعله `background-color` لعنصر `.bb2`.

# --hints--

يجب تعريف متغير خاصية جديد باسم `--building-color2`.

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color2'));
```

يجب إعطاء `--building-color2` القيمة `#66cc99` داخل الصنف `.bb1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1')?.getPropertyValue('--building-color2').trim(), '#66cc99');
```

يجب تعيين `background-color` لعنصر `.bb2`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb2')?.backgroundColor);
```

يجب تعيين `background-color` باستخدام متغير `--building-color2`.
