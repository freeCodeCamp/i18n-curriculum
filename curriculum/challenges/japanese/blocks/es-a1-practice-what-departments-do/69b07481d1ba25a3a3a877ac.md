...
`Verifican` は `ellos/ellas` の形ですが、`el departamento` は単数です。
...
`Verificar` は不定詞の形なので、ここでは文を正しく完成しません。
...
`Verificas` は `tú` の形であり、`el departamento` の形ではありません。
...
`El departamento de control de calidad verifica el software.` - 品質管理部門はソフトウェアをチェックします。
