...
现在在你的 `DiscountEngine` 类中创建一个名为 `calculate_best_price` 的方法。该方法应接受 `self`、`product` 和 `user_tier` 作为参数，并返回一个 `float`。

目前，只需在方法体中使用 `pass`。你将在接下来的步骤中实现逻辑。

# --hints--

你的 `DiscountEngine` 类应有一个名为 `calculate_best_price` 的方法。
...
你的 `calculate_best_price` 方法应包含参数 `self`、`product: Product` 和 `user_tier: str`。
...
你的 `calculate_best_price` 方法应有一个返回类型提示 `-> float`。
