...
當處理屬性名稱和值相同的對象時，可以使用簡寫屬性名稱語法。 如果屬性值與屬性名稱相同，則此語法允許你省略屬性值。
...
在構造函數中，添加 `this.position` 併爲其分配一個具有 `x` 和 `y` 座標的對象。 確保使用簡寫屬性語法。

# --hints--

你應該有一個 `this.position` 屬性。

```js
assert.match(Platform.toString(), /this\.position\s*;?/);
```

`this.position` 屬性應該是一個具有 `x` 和 `y` 座標的對象。 確保使用簡寫屬性語法。
