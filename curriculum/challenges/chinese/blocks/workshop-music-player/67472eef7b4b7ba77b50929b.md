...
在你的 `userData` 对象中，创建一个 `songs` 属性并将其值设置为 `allSongs` 数组。

然后，为了处理当前歌曲的信息并跟踪其播放时间，创建 `currentSong` 和 `songCurrentTime` 属性。将它们的值分别设置为 `null` 和 `0`。

# --hints--

你的 `userData` 对象应有一个 `songs` 密钥，设置为 `allSongs` 数组。

```js
assert.propertyVal(userData, "songs", allSongs);
```

你的 `userData` 对象应该有一个设置为 `null` 的 `currentSong` 键。

```js
assert.propertyVal(userData, "currentSong", null);
```

你的 `userData` 对象应该有一个设置为 `0` 的 `songCurrentTime` 键。
