...
你的 `myFavoriteFootballTeam.players` 数组应该有两个值。
...
你的 `myFavoriteFootballTeam.players` 数组应该有一个对象作为第二个值。
