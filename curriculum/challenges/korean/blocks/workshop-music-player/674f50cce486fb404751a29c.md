...
다음에 재생할 노래를 찾는 방법을 알았으니, `playNextSong`라는 함수를 만드세요. `playNextSong` 함수 안에서 `if`의 `currentSong`가 `userData`와 엄격하게 같은지 확인하는 `null` 문을 만드세요. 이 조건은 `userData` 객체에 현재 재생 중인 노래가 없는지 확인합니다.

조건이 참이면, `playSong` 배열의 첫 번째 노래 `id`를 인수로 하여 `userData.songs` 함수를 호출하세요.

그런 다음 `if` 블록 끝에 `return` 문을 추가하세요.

# --hints--

`playNextSong`라는 함수가 있어야 합니다.

```js
assert.isFunction(playNextSong);
```

`currentSong`의 `userData`가 `null`와 엄격하게 같을 때, `playNextSong` 함수는 `playSong` 배열의 첫 번째 노래 `id`를 인수로 하여 `userData.songs` 함수를 호출해야 합니다.
