...
يمكنك إضافة تحسين صغير أخير وهو عرض الفهرس الذي وُجد عنده `value` الهدف.

لذا، بدلًا من إرجاع `pathToTarget` فقط في جملة `if`، أضف نص قالب مع الرسالة `Value found at index ${mid}`.

بهذا، تكتمل ورشة عمل خوارزمية البحث الثنائي الخاصة بك!

# --hints--

يجب أن تُعيد `pathToTarget, \`Value found at index ${mid}\`` داخل عبارة `if`.
