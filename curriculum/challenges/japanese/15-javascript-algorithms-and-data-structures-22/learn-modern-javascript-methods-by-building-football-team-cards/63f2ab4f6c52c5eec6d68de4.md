...
最後の手順は、他の `case` 節がいずれもユーザーの選択と一致しなかった場合の `default` 節を追加することです。

`default` 節では、関数 `setPlayerCards` を、実引数を渡さずに呼び出してください。

ではドロップダウンメニューをテストしてみましょう。ポジションまたはニックネームの有無で、選手カードがフィルターできるようになっているはずです。

サッカーチームカードのプロジェクトが完成しました。おめでとうございます！
