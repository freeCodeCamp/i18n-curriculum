...
Un numero naturale, `N`, che può essere scritto come somma e prodotto di un insieme dato di almeno due numeri naturali, $\\{a_1, a_2, \ldots , a_k\\}$ si chiama numero prodotto-somma: $N = a_1 + a_2 + \cdots + a_k = a_1 × a_2 × \cdots × a_k$.

Per esempio, 6 = 1 + 2 + 3 = 1 × 2 × 3.

Per un insieme di dimensione `k`, chiameremo il più piccolo N con questa proprietà un numero prodotto-somma minimo. I numeri prodotto-somma minimi per insiemi di dimensione `k` = 2, 3, 4, 5 e 6 sono i seguenti.
...
Quindi per 2 ≤ `k` ≤ 6, la somma di tutti i numeri prodotto-somma minimi è 4 + 6 + 8 + 12 = 30; nota che `8` viene conteggiato solo una volta nella somma.

Infatti, dato che l'insieme completo dei numeri prodotto-somma minimi per 2 ≤ `k` ≤ 12 è $\\{4, 6, 8, 12, 15, 16\\}$, la somma è `61`.

Qual è la somma di tutti i numeri prodotto-somma minimi per 2 ≤ `k` ≤ `limit`?
...
`productSumNumbers(300)` dovrebbe restituire `12686`.
