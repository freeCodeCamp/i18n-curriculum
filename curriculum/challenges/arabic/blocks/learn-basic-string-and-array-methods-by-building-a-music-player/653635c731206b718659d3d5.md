...
داخل دالة `playSong`، عرّف الخاصية `audio.src` لتساوي `song.src`. هذا يخبر عنصر الصوت بمكان العثور على بيانات الصوت للأغنية المختارة.

أيضًا، عرّف الخاصية `audio.title` لتساوي `song.title`. هذا يخبر عنصر الصوت بما يعرضه كعنوان للأغنية.

# --hints--

لا تعدل دالة `playSong` الحالية أو محتواها.
...
يجب أن تعرّف `audio.src` لتساوي `song.src`.
...
يجب أن تعرّف `audio.title` لتساوي `song.title`.
