...
Тепер настав час додати кольори. Дайте `body` `background-color` зі значенням `brown`, а також дайте `.card` `background-color` зі значенням `white`.

# --hints--

Елемент `body` має мати `background-color` зі значенням `brown`.
...
Ви маєте мати селектор `.card`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card"));
```

Елемент `.card` має мати `background-color` зі значенням `white`.
