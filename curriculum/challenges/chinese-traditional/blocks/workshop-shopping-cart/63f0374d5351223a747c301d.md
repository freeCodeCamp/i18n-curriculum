...
遵循與你的 `cartSubTotal` 相同的樣式，更新 `cartTaxes` 以顯示 `tax` 值，並更新你的 `cartTotal` 以顯示 `total` 屬性。

# --hints--

你應該存取 `cartTaxes` 元素的 `textContent` 屬性。

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

你應該在 `tax` 變數上呼叫 `.toFixed()` 方法，並傳入 `2` 作為引數。

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

你應該使用模板字串語法，為你的 `.toFixed()` 呼叫添加美元符號。
...
你應該將 `cartTaxes` 元素的 `textContent` 設定為 `tax.toFixed` 模板字串。
...
你應該存取 `cartTotal` 元素的 `textContent` 屬性。

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

你應該在 `total` 變數上呼叫 `.toFixed()` 方法，並傳入 `2` 作為引數。記得使用 `this` 關鍵字。
...
你應該將 `cartTotal` 元素的 `textContent` 設定為你的 `total.toFixed` 模板字串。
