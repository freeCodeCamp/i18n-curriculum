...
In the previous lecture video, you learned about the <dfn>Web Audio API</dfn> and how to use it to play songs. 主要な最新のブラウザーはすべて Web Audio API をサポートしており、ウェブアプリケーション内で音声を生成および処理できます。

Create a variable named `audio` and set it equal to `new Audio()`. これにより、新しい HTML5 `audio` 要素が作成されます。
