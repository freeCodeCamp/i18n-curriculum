...
Tu selector `span[class~="sr-only"]` debe tener una propiedad `position` establecida en `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('position') === 'absolute');
```

Tu selector `span[class~="sr-only"]` debe tener una propiedad `padding` establecida en `0`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('padding') === '0px');
```

Tu selector `span[class~="sr-only"]` debe tener una propiedad `margin` establecida en `-1px`.
