...
为每个 `span` 添加两个 `className` 元素，分别具有 `stat-value` 为 `stat-label` 和 `div.stat-row`。

# --hints--

每个 `.stat-row` 应该包含一个 `span`，其 `className` 为 `stat-value`，以及一个 `span`，其 `className` 为 `stat-label`。
