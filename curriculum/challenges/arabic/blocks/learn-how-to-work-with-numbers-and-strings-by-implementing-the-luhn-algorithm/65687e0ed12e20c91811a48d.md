...
أسفل الحلقة الثانية `for` في دالة `verify_card_number`، أنشئ متغيرًا باسم `total`، وعيّن له قيمة مجموع الأرقام الفردية والزوجية. اطبع `total` في وحدة التحكم.

# --hints--

يجب أن يكون لديك `total = sum_of_odd_digits + sum_of_even_digits` داخل دالة `verify_card_number`.
...
يجب أن يكون لديك `print(total)` داخل دالة `verify_card_number`.
