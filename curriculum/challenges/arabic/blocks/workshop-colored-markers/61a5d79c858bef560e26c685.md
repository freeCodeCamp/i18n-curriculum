...
وكمعلمة اللون الثالثة، مرر دالة `hsl` بالقيم `240` للون، و`56%` للتشبع، و`42%` لفاتح اللون.

# --hints--

يجب أن تحتوي قاعدة CSS الخاصة بـ `.blue` على خاصية `background` مضبوطة على `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))`.
