...
Unataka kuongeza mwelekeo huo huo wa rangi katika sehemu mbili zinazofuata. Badala ya kufanya hivyo, tengeneza kichaguzi kipya cha darasa la vitu kinachoitwa `bb1-window`, na hamisha sifa na thamani za `height` na `background` kutoka `.bb1a` kwenda kichaguzi kipya cha darasa la vitu.

# --hints--

Unapaswa kutengeneza kichaguzi kipya cha darasa la vitu kinachoitwa `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

Unapaswa kuhamisha sifa na thamani ya `height` kutoka `.bb1a` kwenda `.bb1-window`.
...
Unapaswa kuhamisha sifa na thamani ya `background` kutoka `.bb1a` kwenda `.bb1-window`.
