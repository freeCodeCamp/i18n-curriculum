...
Tenendo presente le migliori pratiche per gli input dei moduli, assegna a ogni `input` un appropriato attributo `type` e `name`. Poi, assegna al primo `input` un attributo `placeholder`.

# --hints--

Dovresti assegnare al primo `input` un `type` di `text`.

```js
assert.equal(document.querySelectorAll('input')?.[0]?.type, 'text');
```

Dovresti assegnare al secondo `input` un `type` di `email`.

```js
assert.equal(document.querySelectorAll('input')?.[1]?.type, 'email');
```

Dovresti assegnare al terzo `input` un `type` di `date`.

```js
assert.equal(document.querySelectorAll('input')?.[2]?.type, 'date');
```

Dovresti assegnare al primo `input` un appropriato attributo `name`.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[0]?.name?.length, 1);
```

Dovresti assegnare al secondo `input` un appropriato attributo `name`.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[1]?.name?.length, 1);
```

Dovresti assegnare al terzo `input` un appropriato attributo `name`.
