...
次のステップは、`users` 配列の中から正しいユーザーを見つけることです。

まず、`user` という定数を作成し、`users.find()` を代入してください。`find` メソッドは、`user` というパラメータを持つコールバック関数を持つべきです。

`find` メソッドのコールバック関数の中で、`user.id` が `poster.user_id` と厳密に等しいかどうかをチェックした結果を暗黙的に返してください。

# --hints--

`user` という名前の定数を作成してください。

```js
assert(code.match(/\s*const\s+user\s*=/));
```

`users.find()` を `user` 定数に代入してください。

```js
assert(code.match(/\s*const\s+user\s*=\s*users\.find\(/g));
```

コールバック関数は `user` というパラメータを持つべきです。

```js
assert(code.match(/\s*const\s+user\s*=\s*users\.find\(\s*(\(\s*user\s*\)|user)\s*=>\s*/g));
```

`user.id` が `poster.user_id` と厳密に等しいかどうかを暗黙的に返してください。
