...
JSX と TSX では、JavaScript の式を属性値として波括弧で渡します。テンプレートリテラルはバックティックと `${}` を使って補間します。例えば：

```tsx
<div className={`prefix-${someVariable}`}>
```

カードラッパー `className` は現在 `card-wrapper tier-gold` としてハードコードされています。`gold` をテンプレートリテラルを使って動的な値に置き換え、`getPlayerTier` を `player.overallRating` で呼び出してください。これにより、プレイヤーの総合評価に基づいて CSS の正しいティアクラスを参照できます。

# --hints--

`getPlayerTier(player.overallRating)` の `className` のために `.card-wrapper` を使ったテンプレートリテラルを使用してください。

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

プレイヤーの評価が 98 のとき、`.card-wrapper` 要素は `tier-elite` クラスを持つべきです。
