...
サブタイトルにもスタイルを適用する必要があります。`.hero-subtitle` セレクターを作成し、`font-size` プロパティに `2.4rem` を、`color` プロパティに `orangered` を、`text-align` プロパティに `center` を設定してください。

# --hints--

`.hero-subtitle` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

`.hero-subtitle` セレクターには `font-size` プロパティが `2.4rem` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

`.hero-subtitle` セレクターには `color` プロパティが `orangered` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

`.hero-subtitle` セレクターには `text-align` プロパティが `center` に設定されているはずです。
