...
`userData.currentSong`가 `null`가 아니면 다음에 재생할 노래를 찾아야 합니다. 이를 위해 `getNextSong`를 사용할 수 있다는 점을 기억하세요.

현재 재생 목록에서 다음 노래를 재생하도록 `playNextSong` 함수를 완성하세요. 현재 노래가 마지막 노래라면 `userData.currentSong`를 `null`로, `userData.songCurrentTime`를 0으로 설정하고 `pauseSong` 함수를 호출하세요.

# --hints--

`currentSong`의 `userData`가 참이고 재생 목록의 마지막 노래가 아닐 때, `playNextSong` 함수는 현재 노래 다음 노래를 재생해야 합니다.
...
`currentSong`의 `userData`가 재생 목록의 마지막 노래일 때, `playNextSong` 함수는 `userData.currentSong`를 `null`로 설정해야 합니다.
...
`currentSong`의 `userData`가 재생 목록의 마지막 노래일 때, `playNextSong` 함수는 `userData.songCurrentTime`를 `0`로 설정해야 합니다.
...
`currentSong`의 `userData`가 재생 목록의 마지막 노래일 때, `playNextSong` 함수는 `pauseSong` 함수를 호출해야 합니다.
...
`currentSong`의 `userData`가 `null`와 같을 때, `playNextSong` 함수는 `playSong` 배열 첫 번째 노래의 `id`를 인수로 하여 `userData.songs` 함수를 호출해야 합니다.
