...
当用户尝试将十进制数 `5` 转换为二进制时，你将显示动画，因此你需要在 `checkUserInput()` 函数中添加检查。

使用 `if` 语句检查 `numberInput` 的 `value` 属性是否等于数字 `5`。 请记住在将字符串与 `5` 进行比较之前使用 `parseInt()` 函数将字符串转换为数字。 暂时将 `if` 语句留空。

# --hints--

你应该在 `checkUserInput()` 函数中现有的 `if` 语句之后添加第二个 `if` 语句。

```js
assert.match(String(checkUserInput), /if\s*\(\s*!numberInput\s*\.\s*value\s*\|\|\s*isNaN\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)[\s\S]+if\s*\(/);
```

你应该在新的 `if` 语句的条件中使用 `parseInt()` 函数。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(/);
```

你应该检查 `numberInput` 的 `value` 属性是否等于 `5`。
