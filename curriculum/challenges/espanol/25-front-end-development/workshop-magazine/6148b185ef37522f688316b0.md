...
Ahora puedes estilizar el contenido del elemento `.heading` con CSS Grid.

La función CSS `repeat()` se utiliza para repetir un valor, en lugar de escribirlo manualmente, y es útil para diseños de rejilla. Por ejemplo, establecer la propiedad `grid-template-columns` a `repeat(20, 200px)` crearía 20 columnas cada una de `200px` de ancho.
...
Tu selector `.heading` debe tener una propiedad `grid-template-columns` establecida en `repeat(2, 1fr)`.
