...
Dein `.first-paragraph::first-letter`- Selektor sollte eine `float`-Eigenschaft mit dem Wert `left` haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float === 'left');
```

Dein `.first-paragraph::first-letter`-Selektor sollte eine `margin-right`- Eigenschaft mit dem Wert `1rem` haben.
