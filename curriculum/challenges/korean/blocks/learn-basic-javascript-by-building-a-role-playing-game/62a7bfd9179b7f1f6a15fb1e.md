...
`buyHealth`, `buyWeapon`, `goTown`라는 이름의 새로운 빈 함수를 세 개 만드세요.

# --hints--

`buyHealth`는 `function` 키워드로 선언해야 합니다.

```js
assert.match(code, /function\s+buyHealth/);
```

`buyWeapon`는 `function` 키워드로 선언해야 합니다.

```js
assert.match(code, /function\s+buyWeapon/);
```

`goTown`는 `function` 키워드로 선언해야 합니다.

```js
assert.match(code, /function\s+goTown/);
```

`buyHealth`는 빈 함수여야 합니다.

```js
assert.equal(buyHealth.toString(), 'function buyHealth() {}');
```

`buyWeapon`는 빈 함수여야 합니다.
