...
你的 `apply` 函式正在傳回試算表函式，但實際上並未套用它。更新 `apply` 以呼叫該函式。將使用 `args` 作為引數呼叫 `toNumberList` 的結果傳入。

# --hints--

你的 `apply` 函式應該呼叫 `spreadsheetFunctions[fn.toLowerCase()]` 函式。
...
你應該將 `toNumberList()` 呼叫傳遞給你的 `spreadsheetFunctions[fn.toLowerCase()]` 呼叫。
...
你應該將 `args` 傳遞給你的 `toNumberList()` 呼叫。
