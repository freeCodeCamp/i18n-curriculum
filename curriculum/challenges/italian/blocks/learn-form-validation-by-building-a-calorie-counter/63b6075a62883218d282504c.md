...
Il tuo elemento `input` ha bisogno di alcuni attributi aggiuntivi. Dagli un insieme `type` impostato su `number` per consentire solo input numerici, un attributo `min` impostato su `0` per consentire solo numeri positivi e un insieme `placeholder` impostato su `Daily calorie budget`.
...
Il tuo elemento `input` dovrebbe avere un attributo `placeholder` impostato su `Daily calorie budget`.
