...
Você precisa obter os valores dos elementos `select`.Esses valores atualmente serão strings e você os converterá em números.

Use a função `map` para iterar sobre o array.Passe uma função callback para `map` que recebe um parâmetro `dropdown` e retorna `dropdown.value`.
...
Sua função de callback `map` deve retornar implicitamente `dropdown.value`.
