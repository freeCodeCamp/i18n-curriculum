...
Para ver os nomes dos autores na página, você precisa chamar a função `displayAuthors` dentro do segundo método `.then()`. Antes disso, porém, você precisa atribuir os dados do autor ao array `authorDataArr` vazio.

Primeiro, remova a instrução do `console.log()`. Em seguida, atribua `data` à variável `authorDataArr`.
...
Você deve remover o registro do console que mostra `data`.
