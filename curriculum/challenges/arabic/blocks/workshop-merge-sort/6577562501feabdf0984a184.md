...
بعد تعيين العنصر في `left_part` إلى المصفوفة المرتبة، زد قيمة `left_array_index` بمقدار `1`.

# --hints--

يجب أن تضيف `1` إلى القيمة الحالية لـ `left_array_index` بعد إدخال العنصر في المصفوفة المرتبة.
