...
Geben Sie nach der `if`-Anweisung den String `Valid format.` aus. Geben Sie dann `True` zurück.

# --hints--

Sie sollten `Valid format.` nach Ihrer `if`-Anweisung ausgeben.
...
Sie sollten `True` nach Ihrem `print()`-Aufruf zurückgeben.
