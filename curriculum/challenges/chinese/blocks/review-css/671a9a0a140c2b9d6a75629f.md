...
- **什么是 CSS？**：层叠样式表（CSS）是一种标记语言，用于在 HTML 元素上应用样式。 CSS is used for colors, background images, layouts, and more.
...
- **White Space (Negative Space)**: This is the empty space in a design. 这是元素周围的区域。
...
- **UX (User Experience)**: UX is about how users feel when using a product or service. 一个具有精心设计的用户体验的应用程序是直观的、易于使用的、高效的、可访问的和令人愉悦的。
