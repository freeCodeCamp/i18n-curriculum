...
Додайте новий селектор, який змінює `background-color` елемента `.card` на `khaki`, коли на нього наводять курсор.

# --hints--

У вас має бути селектор `.card:hover`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card:hover"));
```

Селектор `.card:hover` повинен мати множину `background-color`, встановлену в `khaki`.
