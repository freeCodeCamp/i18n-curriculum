...
`if` 문장 안에 `category`이라는 변수를 만들어 비용 카테고리를 저장하세요. `input()`를 호출한 결과를 할당하고 `'Enter category: '`를 인수로 사용하세요.

# --hints--

`category` 문장 안에 `if`라는 변수를 선언해야 합니다.
...
`input('Enter category: ')` 변수에 `category`를 할당해야 합니다.
