...
Ndani ya kipengele chako cha `ul`, tengeneza vipengele sita vya `li`. Ongeza kipengele cha `h4` chenye `class` iliyowekwa kuwa `list-subtitle` na kipengele cha `p` kwa kila kipengele chako cha `li`.

Kisha toa maandishi yafuatayo kwa vipengele vya `h4` na `p`, kwa mpangilio, ambapo kila `h4` itatumia kile kilicho upande wa kushoto wa nukta ya wima, na kila `p` itatumia kile kilicho upande wa kulia:
...
Kipengele chako cha `ul` kinapaswa kuwa na vipengele sita vya `li`.

```js
assert.lengthOf(document.querySelectorAll('.lists li'), 6);
```

Kila kipengele kipya cha `li` kinapaswa kuwa na kipengele cha `h4` na `p`.
...
`p` yako ya pili inapaswa kuwa na maandishi ya `We added interactive algorithm challenges.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[1]?.innerText, 'We added interactive algorithm challenges.');
```

`h4` yako ya tatu inapaswa kuwa na maandishi ya `V3 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[2]?.innerText, 'V3 - 2015');
```

`p` yako ya tatu inapaswa kuwa na maandishi ya `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`
...
`h4` yako ya tano inapaswa kuwa na maandishi ya `V5 - 2017`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[4]?.innerText, 'V5 - 2017');
```

`p` yako ya tano inapaswa kuwa na maandishi ya `We added the back-end and data visualization challenges.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[4]?.innerText, 'We added the back-end and data visualization challenges.');
```

`h4` yako ya sita inapaswa kuwa na maandishi ya `V6 - 2018`.
...
Vipengele vyako sita vya `h4` vinapaswa kila kutozwa darasa la vitu la `list-subtitle`.
