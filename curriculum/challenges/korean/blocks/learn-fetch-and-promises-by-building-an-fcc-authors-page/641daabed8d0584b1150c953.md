...
일부 작성자 소개는 다른 것보다 훨씬 깁니다. 카드의 모양을 균일하게 만들려면 각 소개의 처음 50자를 추출하고 나머지는 줄임표 `("...")`로 대체할 수 있습니다. 그렇지 않으면 전체 소개를 보여줄 수 있습니다.
...
`bio` 텍스트가 `50`자보다 크면 `slice()`으로 처음 50자를 추출하고 나머지는 `"..."`로 대체하세요. 인덱스가 0부터 시작한다는 점을 잊지 마세요.
