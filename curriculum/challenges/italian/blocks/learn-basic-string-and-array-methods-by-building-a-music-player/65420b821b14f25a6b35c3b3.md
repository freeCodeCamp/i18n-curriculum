...
Quando le canzoni vengono visualizzate sulla pagina, devono mostrare il titolo, l'artista, la durata di ogni canzone e un pulsante per eliminarla.

Nei prossimi passaggi, imparerai come aggiungere nuovo HTML per ogni canzone usando il metodo <dfn>map()</dfn>. Questo metodo sarà spiegato più nel dettaglio nel passaggio successivo.

Inizia usando `const` per dichiarare una variabile chiamata `songsHTML` e assegnarle `array.map()`.

# --hints--

Dovresti avere una variabile `const` chiamata `songsHTML`.

```js
assert.match(code, /const\s+songsHTML/);
```

Dovresti assegnare `array.map()` alla tua `songsHTML`.
