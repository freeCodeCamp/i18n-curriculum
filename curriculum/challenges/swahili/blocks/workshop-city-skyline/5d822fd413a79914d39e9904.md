...
Hadi sasa, mwelekeo wote wa mabadiliko ya rangi uliyoanzisha umekuwa kutoka juu kwenda chini, huo ndio mwelekeo wa kawaida. Unaweza kubainisha mwelekeo mwingine kwa kuuweka kabla ya rangi zako kama ifuatavyo:
...
Jaza `.bb3` na `repeating-linear-gradient`. Tumia `90deg` kwa mwelekeo, `building-color3` kwa rangi mbili za kwanza, na `window-color3` kwa `15%` kwa rangi ya tatu. Unapokosa kubainisha umbali kwa rangi, itatumia thamani zinazofaa. Katika kesi hii, rangi mbili za kwanza zitakuwa `0%` na `7.5%` kwa sababu inaanza kwa `0%`, na `7.5%` ni nusu ya `15%`.
