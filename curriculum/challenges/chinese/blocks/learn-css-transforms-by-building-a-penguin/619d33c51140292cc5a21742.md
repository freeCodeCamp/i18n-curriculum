...
在左臂上应用 `wave` 动画。 让动画持续 `3s`，无限循环，并具有线性缓动函数。

# --hints--

你应该将 `.arm.left` 的 `animation-name` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationName, 'wave');
```

你应该将 `.arm.left` 的 `animation-duration` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationDuration, '3s');
```

你应该将 `.arm.left` 的 `animation-iteration-count` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationIterationCount, 'infinite');
```

你应该将 `.arm.left` 的 `animation-timing-function` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
