...
Você pode ver que chamamos o método `test()` em nossa nova regex e passamos a string `e` como argumento. Também registramos o resultado:
...
O que você acha que cada linha vai imprimir? Bem, aqui está o resultado:
...
Isso te surpreendeu? Note como as três primeiras strings retornaram `true`. Essas strings todas contêm o texto `freeCodeCamp`, exatamente, em algum lugar da string. 
...
E vamos dar uma olhada no resultado:
