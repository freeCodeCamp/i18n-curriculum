...
Dovresti dare a `.window-wrap` una proprietà `display` con il valore `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".window-wrap")?.display, "flex");
```

Dovresti dare a `.window-wrap` una proprietà `align-items` con il valore `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".window-wrap")?.alignItems, "center");
```

Dovresti dare a `.window-wrap` una proprietà `justify-content` con il valore `space-evenly`.
