...
在你的 `forumCategory` 函數中，創建一個名爲 `selectedCategory` 的新 `let` 變量，併爲其賦值一個空對象。 這將用於存儲每個類別的類別名稱和類名。

# --hints--

你應該使用 `let` 聲明一個名爲 `selectedCategory` 的變量。

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

你應該爲 `selectedCategory` 變量分配一個空對象。
