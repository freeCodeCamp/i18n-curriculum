...
在左手臂上使用 `wave` 動畫。讓動畫持續 `3s`，無限迭代，並使用線性時間函式。

# --hints--

你應該給 `.arm.left` 一個 `animation-name` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationName, 'wave');
```

你應該給 `.arm.left` 一個 `animation-duration` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationDuration, '3s');
```

你應該給 `.arm.left` 一個 `animation-iteration-count` 為 `--fcc-expected--`，但發現為 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationIterationCount, 'infinite');
```

你應該給 `.arm.left` 一個 `animation-timing-function` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。
