...
Endlich! Sie haben das letzte Gebäude erreicht! Fügen Sie ihm einen sich wiederholenden Verlauf mit einer `90deg` Richtung hinzu. Verwenden Sie die Gebäude-Farbe von `0%` bis `10%` und `transparent` von `10%` bis `30%`.

# --hints--

Sie sollten eine `repeating-linear-gradient` zu `.fb6` in der `background` Eigenschaft hinzufügen.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

Sie sollten der `repeating-linear-gradient` eine Richtung von `90deg` geben.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Sie sollten der `repeating-linear-gradient` eine erste Farbe von `--building-color3` von `0%` bis `10%` geben.
...
Sie sollten der `repeating-linear-gradient` eine zweite Farbe von `transparent` von `10%` bis `30%` geben.
