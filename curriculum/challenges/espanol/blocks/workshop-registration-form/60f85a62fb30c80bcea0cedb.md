...
Los usuarios podrán elegir si su cuenta es para propósitos de `Personal` o `Business`.

Para hacer esto, dentro de cada uno de los primeros dos elementos `label`, añade un elemento `input` con `type="radio"`.

# --hints--

Debes añadir dos elementos `input`.

```js
assert.lengthOf(document.querySelectorAll('fieldset:nth-child(2) input'), 2);
```

Debes añadir un `input` a cada uno de los primeros dos elementos `label`.
...
Debes dar a ambos elementos `input` un `type` de `radio`.
