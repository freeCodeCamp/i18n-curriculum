...
بعد ذلك، في نهاية `remove` الطريقة، قلل قيمة `self.length` بمقدار 1. هذا يُقلل طول القائمة المرتبطة ليعكس إزالة عقدة.

# --hints--

يجب أن تقلل `self.length` بمقدار 1 في نهاية `remove` الطريقة.
