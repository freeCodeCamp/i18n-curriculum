...
移動を出力した後、元の棒から最後の要素を取り除き、それを目的の棒に追加する必要があります。そのために`.pop()`メソッドと`.append()`メソッドを使ってください。

# --hints--

`rods[source]`から最後の要素を取り除く必要があります。

```js
({ test: () => assert.match(code, /rods\s*\[\s*source\s*\]\s*\.pop\s*\(\s*\)/) })
```

コード内に`rods[target].append(rods[source].pop())`があるはずです。
