...
Terminate the `while` loop by removing the current node from the `unvisited` list. 請注意縮進。

# --hints--

你應該在你的 `for` 循環後面調用 `.remove()` 方法刪除 `unvisited` 的當前節點。
