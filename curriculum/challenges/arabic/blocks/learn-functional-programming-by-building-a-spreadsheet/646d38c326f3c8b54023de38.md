...
تعيد `addCharacters` دالة، لذا عليك استدعاؤها. مرر `char1` كمعلمة.

# --hints--

يجب استدعاء دالة `addCharacters()` داخل طريقة `.map()` الخاصة بك.
...
يجب تمرير `char1` كمعلمة عند استدعاء `addCharacters()`.
