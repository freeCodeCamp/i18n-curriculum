...
`form` 要素を中央に配置するために、`margin` を `0 auto` に設定してください。それから、最大幅を `500px`、最小幅を `300px` に固定してください。その範囲内では、`width` を `60vw` に設定できるようにしてください。

# --hints--

`form` 要素をスタイル設定するには `form` セレクターを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('form'));
```

`form` に `margin` を `0 auto` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.margin, '0px auto');
```

`form` に `max-width` を `500px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.maxWidth, '500px');
```

`form` に `min-width` を `300px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.minWidth, '300px');
```

`form` に `width` を `60vw` に設定してください。
