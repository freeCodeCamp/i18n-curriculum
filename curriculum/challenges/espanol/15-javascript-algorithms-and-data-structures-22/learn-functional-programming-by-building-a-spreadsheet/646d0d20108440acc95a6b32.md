...
En tu función `elemValue`, declaraste explícitamente una función llamada `inner` y la devolviste. Sin embargo, debido a que estás utilizando la sintaxis de flecha, puedes devolver una función de manera implícita. Por ejemplo:

```js
const curry = soup => veggies => {};
```

`curry` es una función que toma un parámetro `soup` y devuelve una función que toma un parámetro `veggies`. Usando esta sintaxis, actualiza tu función `addCharacters` para devolver una función vacía que toma un parámetro `character2`.

# --hints--

Tu función `addCharacters` debe utilizar un retorno implícito.
...
Tu función `addCharacters` debe devolver una función de flecha que tenga un parámetro `character2`.
...
Tu función de flecha interna debe estar vacía.
