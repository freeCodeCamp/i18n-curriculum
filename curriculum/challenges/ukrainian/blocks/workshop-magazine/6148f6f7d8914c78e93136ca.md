...
Створіть один фінальний `@media` запит для `only screen` з `max-width` типу `420px`. Всередині створіть `.hero-title` селектор з `font-size` властивістю, встановленою в `4.5rem`.

Вітаємо! Ваш журнал тепер завершено.

# --hints--

У вас має бути новий `@media` запит для `only screen and (max-width: 420px)`. Це має бути останній запит у списку `@media` запитів.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

Ваш новий `@media` запит повинен мати `.hero-title` селектор.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

Ваш `.hero-title` селектор повинен мати `font-size` властивість, встановлену в `4.5rem`.
