---
id: 6331d23eb51aeedd1a2bd646
title: "Resolvendo para X"
challengeType: 15
videoId: _U9PEFKjyb8
dashedName: solving-for-x
---

# --description--

Este primeiro vídeo mostrará a essência da álgebra e, em seguida, como o código em Python faz a mesma tarefa de uma maneira diferente.

Aqui está o <a href="https://colab.research.google.com/drive/11Zi77gs1FKoEqfPqYa2HtTENiWZyQAO2?usp=sharing" target="_blank" rel="noopener noreferrer nofollow">notebook Colab para acompanhar este vídeo.</a> Adicione o código do vídeo ao seu notebook Colab de álgebra para ver como resolver para X usando Python. Em seguida, mude o código se você quiser, teste-o e compare-o à resolução com lápis e papel. Lembre-se de que a entrada da equação precisa estar na sintaxe do Python.

# --assignment--

Adicione o código para resolver para x do vídeo ao seu notebook do Colaboratory de álgebra.

# --questions--

## --text--

Em Python, qual é a biblioteca que você importa para resolver problemas de álgebra com variáveis?

## --answers--

symba

---

matemática

---

sympsons

---

sympy

## --video-solution--

4
