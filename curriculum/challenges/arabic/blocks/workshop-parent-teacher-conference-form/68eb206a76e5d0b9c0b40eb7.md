...
حان الوقت لتنسيق عناصر `fieldset` لإعطائها حدودًا مميزة.

اختر عنصر `fieldset`. عيّن `border` إلى `1px solid gray`، و`border-radius` إلى `5px`، و`margin` إلى `20px 0`، و`padding` إلى `20px`.

# --hints--

يجب أن تستهدف عنصر `fieldset`.
