...
Kutumia orodha ya wachaguzi (`selector1, selector2`) wape vipengele vyako vya `#controls` na `#stats` `border` ya `1px solid #0a0a23`, rangi ya maandishi ya `#0a0a23`, na `5px` ya `padding`.

# --hints--

Unapaswa kuwa na orodha ya wachaguzi wa `#controls, #stats`.
...
Kichaguzi chako cha `#controls, #stats` kinapaswa kuwa na `border` ya `1px solid #0a0a23`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#controls, #stats')?.border, '1px solid rgb(10, 10, 35)');
```

Kichaguzi chako cha `#controls, #stats` kinapaswa kuwa na `color` ya `#0a0a23`.
...
Kichaguzi chako cha `#controls, #stats` kinapaswa kuwa na `5px` ya nafasi ya ndani.
