...
Para armazenar o tempo atual da música quando ela estiver pausada, defina o `songCurrentTime` do objeto `userData` para o `currentTime` da variável `audio`.

*Nota*: Você não deve usar encadeamento opcional para esta etapa porque `userData.songCurrentTime` não será `null` ou `undefined` neste ponto.
...
Você deve atribuir `audio.currentTime` a `userData.songCurrentTime`.
