...
`try` 블록 안에 `saved` 변수를 만드세요. 값으로는 `getItem()` 메서드를 사용해 `STORAGE_KEY`를 가져오세요. 그 변수 바로 아래에 `if` 조건을 가진 `saved` 문을 만드세요. `if` 블록은 일단 비워 두세요.

# --hints--

`loadPlayer` 함수는 `localStorage.getItem` 블록 안에서 `STORAGE_KEY`를 인자로 하여 `try`를 호출해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

`localStorage.getItem`의 결과는 `saved`라는 변수에 저장해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

`try` 블록에는 `if` 조건을 가진 `saved` 문이 있어야 합니다.
