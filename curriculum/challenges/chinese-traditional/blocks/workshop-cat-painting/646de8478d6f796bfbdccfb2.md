...
為了讓右眼看起來像眼睛，為它設定 `60%` 的邊框半徑。此外，使用 `transform` 屬性，將它旋轉 `-25` 度。

# --hints--

你的 `.cat-right-eye` 選擇器應該設定 `border-radius` 屬性為 `60%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.borderRadius, '60%');
```

你的 `.cat-right-eye` 選擇器應該有一個設定為 `rotate(-25deg)` 的 `transform` 屬性。
