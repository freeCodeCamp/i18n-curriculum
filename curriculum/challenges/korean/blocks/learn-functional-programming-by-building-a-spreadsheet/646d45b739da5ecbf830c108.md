...
`apply` 함수가 스프레드시트 함수를 반환하지만 실제로 적용하지는 않습니다. `apply`을 업데이트하여 함수를 호출하세요. `toNumberList`을 인수로 하여 `args`를 호출한 결과를 전달하세요.

# --hints--

`apply` 함수는 `spreadsheetFunctions[fn.toLowerCase()]` 함수를 호출해야 합니다.
...
`toNumberList()` 호출에 `spreadsheetFunctions[fn.toLowerCase()]` 호출을 전달해야 합니다.
...
`args` 호출에 `toNumberList()`을 전달해야 합니다.
