...
既然你已經熟悉了二進位數字，現在是時候完成建置用來為你進行轉換的函式（程式）了。你將先從一個較簡單的解法開始，然後將其重構為遞迴解法。

首先，你需要創建一些陣列來儲存你在接下來步驟中進行除法的輸入和結果。這些將使你更容易理解十進位轉二進位的運作方式。

從你的 `decimalToBinary` 函式中移除 `return` 述語。然後，宣告名為 `inputs`、`quotients` 和 `remainders` 的變數，並為它們各自指定一個空的陣列。

# --hints--

你應該從你的 `decimalToBinary` 函式中移除 `return` 述語。

```js
assert.notMatch(String(decimalToBinary), /return\s*('|"|`)(\s|\d)*\1\s*;?\s*\}$/);
```

你應該宣告一個名為 `inputs` 的變數。
