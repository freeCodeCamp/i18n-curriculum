...
title: Construye un gráfico de dispersión
challengeType: 25
...
En este proyecto usarás los datos de `https://cdn.freecodecamp.org/curriculum/labs/data/scatterplot/cyclist-data.json` para construir un gráfico de dispersión.

Las pruebas requieren que los ejes se generen utilizando la propiedad de eje D3, que genera automáticamente marcas a lo largo del eje. Estas marcas son necesarias para pasar las pruebas de D3, ya que sus posiciones se utilizan para determinar el alineamiento de los elementos gráficos.

**Objetivo:** Cumplir con las historias de usuario a continuación y pasar todas las pruebas para completar el laboratorio.

**Historias de usuario:**

1. Tu gráfico debe tener un título con un correspondiente `id="title"`.
1. Tu gráfico debe tener un elemento `g` para el eje x con un `id="x-axis"` correspondiente.
1. Tu gráfico debe tener un elemento `g` para el eje y con un `id="y-axis"` correspondiente.
1. Ambos ejes deben contener múltiples etiquetas de marcas, cada una con un correspondiente `class="tick"`.
1. Tu gráfico debe tener un elemento `circle` para cada punto de datos con un `class="dot"` correspondiente que muestre los datos.
1. Cada `.dot` debe tener los atributos `data-xvalue` y `data-yvalue` que contengan los valores de `year` y `time`. Para `data-xvalue`, se aceptan enteros (años completos) u objetos Date para la evaluación de la prueba. Para `data-yvalue` (minutos), usa objetos Date.
1. Los atributos `data-xvalue` de los elementos `.dot` deben coincidir con el orden de los datos proporcionados.
1. Los atributos `data-yvalue` de los elementos `.dot` deben coincidir con el orden de los datos proporcionados.
1. Cada elemento `.dot` debe alinearse con el valor correspondiente en el eje x.
1. Cada elemento `.dot` debe alinearse con el valor correspondiente en el eje y.
1. Debe haber múltiples etiquetas de marcas en el eje y con formato de tiempo `%M:%S`.
1. Debe haber múltiples etiquetas de ticks en el eje x que muestren el año.
1. El rango de las etiquetas del eje x debe estar dentro del rango de los datos reales del eje x.
1. El rango de las etiquetas del eje y debe estar dentro del rango de los datos reales del eje y.
1. Debe haber una leyenda que contenga texto descriptivo que tenga `id="legend"`.
1. Deberías poder pasar el cursor sobre un área y ver un tooltip con un correspondiente `id="tooltip"` que muestra más información sobre el área.
1. Tu tooltip debe tener un atributo `data-year` que corresponda al `data-xvalue` del área activa.
...
El gráfico debe tener un elemento con el `id` de `title` para contener el título.

```js
assert.exists(document.getElementById('title'));
```

El gráfico debe tener un `x-axis` con un correspondiente `id="x-axis"`.
...
El gráfico debe tener un `y-axis` con un correspondiente `id="y-axis"`.
...
El eje x debe contener múltiples etiquetas de marcas, cada una con la correspondiente `class="tick"`.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

El eje y debe contener múltiples etiquetas de marcas, cada una con el correspondiente `class="tick"`.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

Tu gráfico debe tener puntos, que cada uno tenga una clase `dot`, que representen los datos que se están graficando.

```js
assert.isNotEmpty(document.querySelectorAll('circle.dot'));
```

Cada punto debe tener las propiedades `data-xvalue` y `data-yvalue` que contengan sus valores correspondientes de x e y.
...
El `data-xvalue` y el `data-yvalue` de cada punto deben estar dentro del rango de los datos reales y en el formato de datos correcto. Para `data-xvalue`, se aceptan enteros (años completos) u objetos Date para la evaluación de la prueba. Para `data-yvalue` (minutos), usa objetos Date.
...
El `data-xvalue` y su punto correspondiente deben alinearse con el punto/valor correspondiente en el eje x.
...
El `data-yvalue` y su punto correspondiente deben alinearse con el punto/valor correspondiente en el eje y.
...
Puedes ver múltiples etiquetas de marcas en el eje y con el formato de tiempo `%M:%S`.
...
Puedes ver múltiples etiquetas de ticks en el eje x que muestran el año.
...
El rango de las etiquetas del eje x está dentro del rango de los datos reales del eje x.
...
El rango de las etiquetas del eje y está dentro del rango de los datos reales del eje y.
...
Puedes ver una leyenda que contiene texto descriptivo que tiene `id="legend"`.
...
Al pasar el cursor sobre un área, tu gráfico de dispersión debe tener un tooltip correspondiente con `id="tooltip"` que muestre más información sobre el área.
...
El tooltip debe tener un atributo `data-year` que corresponda al `data-xvalue` del área activa.
