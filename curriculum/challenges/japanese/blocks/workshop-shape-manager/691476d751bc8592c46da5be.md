...
ユーザーがドロップダウンで形状の選択を変更したときに対応する方法が必要です。これには、`handleShapeSelect` 関数を `e` パラメータ（型は `Event`）付きで作成してください。

この関数は、すべての入力をクリアし、デフォルトの動作を防止し、選択を検証し、形状が選択されているかどうかに応じて結果カードを表示・非表示にし、選択された形状に応じた入力フィールドを表示し、エリアを再計算する処理を行います。

関数は今のところ空のままにしてください。

# --hints--

`handleShapeSelect` 関数を作成してください。

```js
assert.isFunction(handleShapeSelect)
```

`handleShapeSelect` 関数は `e` パラメータを持ち、その型は `Event` である必要があります。
