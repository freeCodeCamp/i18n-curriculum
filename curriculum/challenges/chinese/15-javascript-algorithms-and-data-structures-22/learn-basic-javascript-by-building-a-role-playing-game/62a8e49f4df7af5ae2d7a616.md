...
将 `monsterHealth` 设置为 `monsterHealth` 减去玩家当前武器的威力。

请记住，你可以使用 `weapons[currentWeaponIndex].power` 访问玩家当前武器的威力。

# --hints--

你应该使用复合赋值来修改 `monsterHealth` 变量。

```js
assert.match(attack.toString(), /monsterHealth\s*-=/);
```

你应该使用带有 `currentWeaponIndex` 的括号表示法来访问 `weapons`。

```js
assert.match(attack.toString(), /weapons\s*\[\s*currentWeaponIndex\s*\]/);
```

你应该使用点符号来访问 `weapons[currentWeaponIndex]` 的 `power` 属性。

```js
assert.match(attack.toString(), /weapons\s*\[\s*currentWeaponIndex\s*\]\s*\.power/);
```

你应该从 `monsterHealth` 中减去当前武器的 `power`。
