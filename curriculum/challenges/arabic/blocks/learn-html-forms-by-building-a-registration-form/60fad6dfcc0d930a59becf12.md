...
تم الانتهاء من شفرة HTML لنموذج التسجيل. الآن، يمكنك تحسين مظهره قليلاً.

ابدأ بإضافة `Tahoma` كخط مع حجم خط `16px` في محدِّد `body`.

# --hints--

يجب استخدام خاصية `font-family` لتغيير الخط.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily);
```

يجب تعيين خاصية `font-family` إلى `Tahoma`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily, 'Tahoma');
```

يجب تعيين خاصية `font-size` إلى `16px`.
