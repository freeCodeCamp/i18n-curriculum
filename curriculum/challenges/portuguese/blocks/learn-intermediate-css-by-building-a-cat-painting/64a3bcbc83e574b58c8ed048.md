...
Como você fez na etapa anterior, use um seletor descendente que aponte para os três elementos `div` dentro do elemento `.cat-whiskers-right`. Dê ao seletor uma `width` de `40px`, uma `height` de `1px` e uma `background-color` de `#000`.

# --hints--

Você deve ter um seletor `.cat-whiskers-right div`.
