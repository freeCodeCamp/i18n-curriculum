---
id: 66a93bbe65a26169dbf3bc39
title: Крок 13
challengeType: 0
dashedName: step-13
---

# --description--

Your hotel feedback form should also collect an email address from the user.

Start by adding a new `label` element with the text `Email address*:` to the form. Елемент `label` повинен мати атрибут `for` зі значенням `"email"`.

# --hints--

Ви повинні мати елемент `label`.

```js
assert.strictEqual(document.querySelectorAll('fieldset label').length, 2);
```

Елемент `label` повинен містити текст `Email*:`.

```js
assert.strictEqual(document.querySelector('fieldset label:nth-of-type(2)').innerText, 'Email address*:');
```

Елемент `label` повинен мати атрибут `for` зі значенням `"email"`.

```js
assert.strictEqual(document.querySelector('fieldset label[for="email"]').getAttribute('for'), 'email');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <title>Hotel Feedback Form</title>
  </head>
  <body>
    <header>
      <h1>Hotel Feedback Form</h1>
      <p>
        Thank you for staying with us. Please provide feedback on your recent
        stay.
      </p>
    </header>
    <main>
      <form method="POST" action="https://hotel-feedback.freecodecamp.org">
        <fieldset>
          <legend>Personal Information</legend>
          <label for="full-name">Name*:</label>
          <input type="text" id="full-name" name="name" placeholder="Ex. John Doe" required>
        --fcc-editable-region--

        --fcc-editable-region--
        </fieldset>
      </form>
    </main>
  </body>
</html>
```
