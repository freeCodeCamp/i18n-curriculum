...
**目标：** 构建一个功能上类似于 <a href="https://technical-documentation-page.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://technical-documentation-page.freecodecamp.rocks</a> 的应用。**不要拷贝此演示项目**。
...
1. 每个具有 `main-section` 类的 `section` 元素也应具有一个与其中包含的每个 `header` 的文本对应的 `id`。任何空格都应替换为下划线（例如，包含头部 `"JavaScript and Java"` 的部分应具有对应的 `id="JavaScript_and_Java"`）。
