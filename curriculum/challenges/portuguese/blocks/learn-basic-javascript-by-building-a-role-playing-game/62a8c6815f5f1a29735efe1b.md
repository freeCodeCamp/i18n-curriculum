...
Use `const` para criar uma variável `weapons` acima do array `locations`. Atribua a ela um array vazio.

# --hints--

Você deve usar `const` para declarar a variável `weapons`.

```js
assert.match(code, /const\s+weapons/i);
```

A variável `weapons` deve ser um array.

```js
assert.isArray(weapons);
```

A variável `weapons` deve estar vazia.
