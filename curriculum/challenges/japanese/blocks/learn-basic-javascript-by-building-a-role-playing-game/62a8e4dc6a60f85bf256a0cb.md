...
JavaScript の `Math` オブジェクトには、数学定数や関数のための静的プロパティとメソッドが含まれています。そのうちの一つが `Math.random()` で、`0`（含む）から `1`（含まない）までの乱数を生成します。もう一つは `Math.floor()` で、与えられた数値を最も近い整数に切り捨てます。

これらを使うと、範囲内の乱数を生成できます。例えば、これは `1` から `5` までの乱数を生成します：`Math.floor(Math.random() * 5) + 1;`。

このパターンに従い、加算演算子（`+`）を使って、`1` から `xp` の値までの乱数を `monsterHealth -= weapons[currentWeaponIndex].power` に加えてください。

# --hints--

すでに書いた `monsterHealth` の行を使うべきです。

```js
assert.lengthOf(attack.toString().match(/monsterHealth/g), 1);
```

`weapons[currentWeaponIndex].power` の値に加算するべきです。
