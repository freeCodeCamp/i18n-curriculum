...
你可以看到，輸出中缺少 `+` 符號。 僅當爲負數時才默認顯示數字符號。 To change this behavior, you can write a colon after the expression to be evaluated within the curly braces of your f-string, and specify the option `+`. 這將允許你顯示正數和負數的符號。

通過在 `coefficient` 後的花括號內添加 `:+` 來修改兩個條件子句中的字符串。

# --hints--

你應該修改字符串以將其追加到 `if` 語句中的 `terms` 列表中，並將其放入 `f'{coefficient:+}'` 中。
...
你應該修改字符串以將其插入到 `elif` 子句中的 `terms` 列表中，並將其放入 `f'{coefficient:+}x'` 中。
