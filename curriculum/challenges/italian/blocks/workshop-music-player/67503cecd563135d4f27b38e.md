...
Ora è il momento di testare la funzione `playNextSong`.

Aggiungi un listener per l'evento `"click"` all'elemento `nextButton`, poi passa `playNextSong` come secondo argomento del listener. Questa è la funzione che il listener eseguirà.

Prova la tua app cliccando prima sul pulsante play e poi sul pulsante next. Dovresti vedere che tutto funziona come previsto.

# --hints--

Dovresti chiamare il metodo `addEventListener()` sulla tua variabile `nextButton`.
...
Dovresti passare `playNextSong` come secondo argomento del metodo `addEventListener`.
