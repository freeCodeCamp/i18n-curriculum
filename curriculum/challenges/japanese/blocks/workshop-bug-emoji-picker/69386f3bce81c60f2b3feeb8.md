...
DOMからIDが`select`の`species`要素を取得し、`selectElement`という名前の定数に格納してください。要素が`!`でないことを示すために、`null`の非nullアサーション演算子を使ってください。

# --hints--

`selectElement`という名前の定数があるはずです。
...
`selectElement`は、型引数に`species`を指定して、`document.querySelector`を使いIDが`HTMLSelectElement`の要素をクエリすべきです。

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

`selectElement`は非nullアサーション演算子（`!`）を使うべきです。
