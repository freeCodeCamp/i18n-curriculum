...
Du solltest eine neue `case`-Klausel für `"ArrowUp"` innerhalb deiner `switch`-Anweisung hinzufügen.

```js
assert.match(code, /\s*case\s*('|"|`)\s*ArrowUp\s*\1\s*:\s*/)
```

Du solltest eine neue `case`-Klausel für `" "` innerhalb deiner `switch`-Anweisung hinzufügen.

```js
assert.match(code, /\s*case\s*('|"|`)\s\1\s*:\s*/);
```

Du solltest eine neue `case`-Klausel für `"Spacebar"` innerhalb deiner `switch`-Anweisung hinzufügen.
