...
無障礙樹是一種結構，供輔助技術（例如螢幕閱讀器）用來解讀和互動網頁上的內容。
...
- **`display: none;`**：使用 `display: none;` 表示螢幕閱讀器和其他輔助技術無法存取這些內容，因為它們不包含在無障礙樹中。因此，只有當你想要完全從視覺呈現和無障礙兩方面移除內容時，才應該使用這個方法。
- **`visibility: hidden;`**：這個屬性和值會將內容視覺上隱藏，但仍保留在文件流程中，也就是說它仍佔據頁面空間。這些元素也不會被螢幕閱讀器讀取，因為它們已從無障礙樹中移除。
- **`.sr-only` CSS 類別**：這是一種常見技巧，用來視覺上隱藏內容，同時保持它對螢幕閱讀器可存取。
...
在上述範例中，如果使用者尚未在其裝置上設定減少動態效果的偏好，則會啟用平滑滾動。
...
使用 placeholder 文字對無障礙性不太理想。使用者常常會將 placeholder 文字誤認為是實際的輸入值，並以為輸入框中已經有值。
