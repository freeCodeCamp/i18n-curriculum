...
حان الوقت لتنسيق القسم الأخير من المجلة - الصور.

الصور ملفوفة بعنصر `aside` يستخدم صنف `image-wrapper`، فأنشئ محدِّد `.image-wrapper`. عيّن الخاصية `display` إلى `grid`.

# --hints--

يجب أن يكون لديك محدِّد `.image-wrapper`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

يجب أن يحتوي محدِّد `.image-wrapper` على خاصية `display` مضبوطة على `grid`.
