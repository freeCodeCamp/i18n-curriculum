...
يجب أن يحتوي عنصر `.blush` على خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.top, '65%');
```

يجب أن يحتوي عنصر `.blush` على خاصية `border-radius` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
