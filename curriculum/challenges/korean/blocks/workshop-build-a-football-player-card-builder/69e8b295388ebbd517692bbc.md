...
이제 `div`의 `className`인 `card-stats`를 추가하세요. 그 안에 `div`의 `className`인 `stat-col`를 추가하세요. 그 안에 각각 `div`의 `className`인 `stat-row` 요소 세 개를 추가하세요.

# --hints--

`div` 다음에 `className`의 `card-stats`인 `.card-name-strip`를 추가하세요.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

`div` 안에 `className`의 `stat-col`인 `.card-stats`를 추가하세요.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

`div` 안에 각각 `className`의 `stat-row`인 `.stat-col` 요소 세 개를 추가하세요.
