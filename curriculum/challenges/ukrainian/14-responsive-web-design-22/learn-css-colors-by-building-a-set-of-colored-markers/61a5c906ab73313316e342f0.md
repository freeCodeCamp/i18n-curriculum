...
Ще трохи очистьте свій код, видаливши аргумент `gradientDirection` з обох функцій `linear-gradient`.

# --hints--

Ви повинні видалити аргументи `gradientDirection` із функцій `linear-gradient` в CSS-правилах `.red` та `.green`.

```js
assert(!code.match(/linear-gradient\s*\(\s*180deg/gi));
```

Ваше CSS-правило `.red` повинне мати властивість `background` зі значенням `linear-gradient(rgb(122, 74, 14), rgb(245, 62, 113), rgb(162, 27, 27))`.
...
Ваше CSS-правило `.green` повинне мати властивість `background` зі значенням `linear-gradient(#55680D, #71F53E, #116C31)`.
