...
背景を完成させるために、`div` 要素の直後に新しい `.back-mountain` 要素を作成し、`sun` のクラスを付けて太陽を追加してください。

# --hints--

`div` に新しい `body` 要素を追加する必要があります。期待される `div` 要素の数は --fcc-expected-- ですが、実際には --fcc-actual-- でした。

```js
assert.lengthOf(document.querySelectorAll('body > div'), 5);
```

新しい `div` 要素に `class` の `sun` を付ける必要があります。

```js
assert.include(document.querySelector('div:not(.back-mountain, .left-mountain, .penguin, .ground)')?.className, 'sun');
```

新しい `div` 要素は `.back-mountain` 要素の直後に配置してください。
