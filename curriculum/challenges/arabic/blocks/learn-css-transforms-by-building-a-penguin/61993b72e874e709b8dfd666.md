...
استهدف العنصر `.penguin-head`، وأعطه `width` نصف عرض العنصر الأصل، و`height` بنسبة `45%`. ثم، اضبط `background` ليكون تدرجًا خطيًا بزاوية `45deg` يبدأ من `gray` وينتهي عند `rgb(239, 240, 228)`.

# --hints--

يجب أن تستخدم المحدِّد `.penguin-head`.

```js
assert.match(code, /\.penguin-head\s*\{/);
```

يجب أن تعطي `.penguin-head` `width` بقيمة `50%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

يجب أن تعطي `.penguin-head` `height` بقيمة `45%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

يجب أن تعطي `.penguin-head` `background` بقيمة `linear-gradient(45deg, gray, rgb(239, 240, 228))`.
