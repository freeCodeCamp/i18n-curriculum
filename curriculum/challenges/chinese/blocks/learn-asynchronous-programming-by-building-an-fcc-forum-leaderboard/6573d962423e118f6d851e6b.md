...
接下来，使用 `getElementById()` 方法访问 `#posts-container` 元素。 将其赋值给一个名为 `postsContainer` 的新常量。

# --hints--

你应该使用 `const` 来声明 `postsContainer` 变量。

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

你应该使用 `document.getElementById()` 来获取 `#posts-container` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

你应该将 `#posts-container` 元素赋值给 `postsContainer` 变量。
