...
`textContent` setzt den Text eines Knotens und ermöglicht es Ihnen, den Textinhalt eines HTML-Elements zu setzen oder abzurufen.
...
Sie können einen ternären Operator verwenden, um den Wert des Textinhalts bedingt zu setzen. Hier ist ein Beispiel, wie das Ergebnis eines ternären Operators einer Variablen zugewiesen wird:

```js
const example = condition ? "I'm true" : "I'm false";
```

Verwenden Sie einen ternären Operator, um zu prüfen, ob `currentTitle` einen truthy-Wert ergibt. Wenn ja, setzen Sie `playingSong.textContent` auf `currentTitle`. Andernfalls setzen Sie es auf einen leeren String.

Verwenden Sie darunter einen `ternary`-Operator, um zu prüfen, ob `currentArtist` truthy ist. Falls ja, setzen Sie `songArtist.textContent` auf `currentArtist`. Andernfalls setzen Sie es auf einen leeren String.

# --hints--

Sie sollten die Eigenschaft `textContent` an `playingSong` anhängen.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

Sie sollten den `ternary`-Operator verwenden, um die Eigenschaft `textContent` von `playingSong` auf `currentTitle` oder `""` zu setzen.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

Sie sollten die Eigenschaft `textContent` an `songArtist` anhängen.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

Sie sollten den `ternary`-Operator verwenden, um die Eigenschaft `textContent` von `songArtist` auf `currentArtist` oder `""` zu setzen.
