...
Deine `apply`-Funktion gibt die Tabellenfunktion zurück, wendet sie aber nicht wirklich an. Aktualisiere `apply`, um die Funktion aufzurufen. Übergib ihr das Ergebnis des Aufrufs von `toNumberList` mit `args` als Argument.

# --hints--

Deine `apply`-Funktion sollte die `spreadsheetFunctions[fn.toLowerCase()]`-Funktion aufrufen.
...
Du solltest einen `toNumberList()`-Aufruf an deinen `spreadsheetFunctions[fn.toLowerCase()]`-Aufruf übergeben.
...
Du solltest deinem `toNumberList()`-Aufruf `args` übergeben.
