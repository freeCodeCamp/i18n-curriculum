...
Fanya ukubwa wa ukurasa wako kuwa wa kawaida, kwa kuondoa `body` kipengele cha `margin` na `padding`.

# --hints--

Unapaswa kumpa `body` `margin` ya `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.margin, '0px');
```

Unapaswa kumpa `body` `padding` ya `0`.
