...
Agrega las clases `text-4xl`, `font-bold` y `text-white` al elemento `p` que tiene el texto del precio para que se vea más visible.
...
Tu elemento `p` debe tener la clase `text-white`.
