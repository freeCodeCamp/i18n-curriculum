...
Fügen Sie nach dem `card-image-wrap`-Element ein `div` mit dem `className` von `card-name-strip` hinzu. Darin fügen Sie ein `span` mit dem `className` von `card-name` und dem Text `PELE` ein.

# --hints--

Sie sollten ein `div` mit dem `className` von `card-name-strip` hinzufügen.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

Sie sollten ein `span` mit dem `className` von `card-name` innerhalb von `.card-name-strip` hinzufügen.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

Der `.card-name`-Bereich sollte den Text `PELE` enthalten.
