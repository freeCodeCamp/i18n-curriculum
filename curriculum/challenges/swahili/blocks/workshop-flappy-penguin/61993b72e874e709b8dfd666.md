...
Lenga kipengele cha `.penguin-head`, na kimpatie `width` nusu ya ile ya mzazi wake, na `height` ya `45%`. Kisha, weka `background` kuwa mwelekeo wa mstari wa mviringo kuanzia `gray` hadi `rgb(239, 240, 228)` katika `45deg`.
...
Unapaswa kumpa `.penguin-head` `width` ya `50%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

Unapaswa kumpa `.penguin-head` `height` ya `45%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

Unapaswa kumpa `.penguin-head` `background` ya `linear-gradient(45deg, gray, rgb(239, 240, 228))`.
