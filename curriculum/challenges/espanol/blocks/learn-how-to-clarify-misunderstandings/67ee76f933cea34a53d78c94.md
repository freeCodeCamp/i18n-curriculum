...
`Consistent` means something that happens regularly without changes. Por ejemplo:
...
`Happens enough` means that something occurs often enough to be noticeable, but not all the time. Por ejemplo:
...
`Annoying` refers to something that bothers or frustrates you. Por ejemplo:
