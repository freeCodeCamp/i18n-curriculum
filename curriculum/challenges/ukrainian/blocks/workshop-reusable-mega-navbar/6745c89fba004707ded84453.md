...
Створіть елемент посилання всередині першого елемента `li`. Додайте елементу `a` атрибут `href` зі значенням `#` та текст `Dashboard`.

# --hints--

Ви повинні створити елемент `a` всередині першого елемента списку.

```js
assert.exists(document.querySelector('li a'));
```

Ваш елемент `a` має мати атрибут `href`, встановлений у `#`.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), '#');
```

Ваш елемент `a` має містити текст `Dashboard`.
