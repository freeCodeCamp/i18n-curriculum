...
仍然在 `handleShapeSelect` 函数内部，创建一个 `hasSelection` 变量，用来检查 `val.value` 是否存在。确保它被 `Boolean()` 包裹。

在此之下，使用你的 `toggleResultCard()` 变量调用 `hasSelection` 函数。

这样只有在实际选择了形状时才会显示结果卡片。

# --hints--

你应该创建一个 `hasSelection` 变量。
