...
الآن عليك تعريف الخصائص التي ستتضمنها كل الأخطاء البرمجية.

داخل صنف `Bug`، أعلن عن خاصية باسم `emoji` من النوع `T` باستخدام عامل تحقق التعيين المؤكد (`!`). هذا يخبر TypeScript أن الخاصية ستُعيَّن لاحقًا.

# --hints--

يجب أن يحتوي صنف `Bug` على خاصية باسم `emoji`.
...
يجب أن تكون خاصية `emoji` من النوع `T`.
...
يجب أن تستخدم خاصية `emoji` عامل تحقق التعيين المؤكد (`!`).
