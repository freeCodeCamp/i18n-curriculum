...
Necesitas otra función que muestre elementos en el DOM. Similar a la función anterior, crea una función llamada `showElements`. Esta vez usa `classList.remove("hidden")`.

# --hints--

Deberías tener una función llamada `showElements`.
...
Tu función `showElements` debe tener un parámetro llamado `...elements` con el tipo `HTMLElement[]`.
...
En tu función `showElements` debes tener un método `forEach` adjunto al parámetro `elements`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Tu método `forEach` debe recibir un parámetro llamado `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Debes eliminar la clase `hidden` usando el método `classList.remove` dentro del método `forEach`.
