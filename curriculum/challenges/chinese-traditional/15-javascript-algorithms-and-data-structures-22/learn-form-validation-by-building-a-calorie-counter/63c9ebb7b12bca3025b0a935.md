...
是時候進行另一次循環了。 創建一個 `for...of` 循環，其中有一個名爲 `container` 的變量，以遍歷 `inputContainers` 數組。

在循環內部，將 `container` 的 `innerHTML` 屬性設置爲空字符串。 這將清除該輸入容器的所有內容。

# --hints--

你的 `for...of` 循環應該遍歷 `inputContainers` 數組。 循環的變量名應該是 `container`。
...
你的 `for...of` 循環應將 `container` 的 `innerHTML` 屬性設置爲空字符串。
