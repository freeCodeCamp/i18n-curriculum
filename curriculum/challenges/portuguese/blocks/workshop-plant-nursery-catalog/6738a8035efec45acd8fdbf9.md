...
Como você pode ver, agora seu conjunto está completamente vazio.Remova as chamadas `clear` e `has` do seu código.

# --hints--

Você não deve ter `plantsSet.clear()` no seu código.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

Você não deve ter `plantsSet.has(ballerina)` no seu código.
