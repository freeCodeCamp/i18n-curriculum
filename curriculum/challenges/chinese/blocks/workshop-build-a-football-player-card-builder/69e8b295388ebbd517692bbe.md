...
`PlayerCard` 组件可以正常工作，但使用了硬编码的值。为了使其更灵活，你需要使用 TypeScript 接口描述玩家数据的结构。

在 `PlayerCard` 函数之前，创建一个名为 `PlayerData` 的接口，该接口包含一个类型为 `name` 的 `string`，一个类型为 `overallRating` 的 `number`，一个类型为 `position` 的 `string`，以及一个类型为 `club` 的 `string`。

# --hints--

你应该创建一个名为 `PlayerData` 的接口。
...
你的 `PlayerData` 接口应该有一个类型为 `name` 的 `string`。
...
你的 `PlayerData` 接口应该有一个类型为 `overallRating` 的 `number`。
...
你的 `PlayerData` 接口应该有一个类型为 `position` 的 `string`。
...
你的 `playerData` 接口应该有一个类型为 `club` 的 `string`。
