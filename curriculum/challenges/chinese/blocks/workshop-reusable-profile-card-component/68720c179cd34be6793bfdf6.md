...
虽然你的应用按预期工作，但你通常不会像这样手动添加多个 `Card` 组件。

删除 `<div className="flex-container">` 元素内的三个 `<Card />` 组件。在接下来的几个步骤中，你将创建一个数据数组并使用 `map()` 函数来渲染 `Card` 组件，这是 React 中更常见的方法。

# --hints--

你应该从你的代码中移除三个 `<Card />` 组件。
