...
Die Missionskontrolle hat eine neue Funktion angefordert, um eine Crew in Abschnitte variabler Größe aufzuteilen. Erstellen Sie eine leere Funktion namens `chunkCrew`, die zwei Parameter akzeptiert, `crew` und `size`.

# --hints--

Sie sollten eine Funktion namens `chunkCrew` erstellen.

```js
assert.isFunction(chunkCrew);
```

Ihre `chunkCrew`-Funktion sollte die Parameter `crew` und `size` haben.
