...
`.map()` コールバックを更新して、`Number()` 関数を呼び出してください。その関数呼び出しに `dropdown.value` を渡してください。

コンソールタブを開いて、`inputValues` が数値の配列になっていることを確認してください。

# --hints--

`Number()` コールバック内で `.map()` 関数を呼び出す必要があります。
...
`dropdown.value` 呼び出しに `Number()` を渡す必要があります。
