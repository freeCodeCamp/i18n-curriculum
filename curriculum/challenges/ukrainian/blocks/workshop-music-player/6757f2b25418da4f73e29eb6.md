...
Далі потрібно відобразити поточну назву пісні та виконавця на дисплеї плеєра.

На першому кроці отримайте доступ до елементів `#player-song-title` та `#player-song-artist`, які відповідають за відображення назви пісні та виконавця, і збережіть їх у змінні `playingSong` та `songArtist` відповідно.

# --hints--

У вас має бути змінна з назвою `playingSong`.

```js
assert.isDefined(playingSong);
```

Значення змінної `playingSong` має бути значенням елемента `#player-song-title`.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

У вас має бути змінна з назвою `songArtist`.

```js
assert.isDefined(songArtist);
```

Значення змінної `songArtist` має бути значенням елемента `#player-song-artist`.
