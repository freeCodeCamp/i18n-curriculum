...
전체 `value`에 `player.overallRating` 속성을 `input`으로 추가하세요. 그다음, `onChange`를 매개변수로 받고 `e`를 펼친 것과 `setPlayer`을 `player`로 설정하여 `overallRating`를 호출하는 화살표 함수로 설정된 `Number(e.target.value)` 속성을 추가하세요.

`overallRating`가 숫자이므로 입력에서 문자열 값을 변환하려면 `Number()`을 사용하세요.

# --hints--

전체 `input`에는 현재 선수 전체 등급과 같은 `value`가 있어야 합니다.
...
전체 `input`에는 `onChange` 속성이 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

전체 `onChange`의 `input`는 `setPlayer`를 펼친 것과 `player`를 `overallRating`로 설정하여 `Number(e.target.value)`를 호출해야 합니다.
