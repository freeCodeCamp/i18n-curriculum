...
Recuerda que en JSX las llaves te permiten insertar cualquier expresión de JavaScript directamente dentro del JSX. Así es como muestras datos dinámicos desde las props. En TSX haces lo mismo.

El encabezado de la tarjeta aún muestra valores codificados. Reemplaza el texto `98` dentro de `card-rating` con `player.overallRating`, y reemplaza `ST` dentro de `card-position` con `player.position`.

# --hints--

Debes reemplazar el `98` codificado con `player.overallRating` en `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

Debes reemplazar el `ST` codificado con `player.position` en `.card-position`.
