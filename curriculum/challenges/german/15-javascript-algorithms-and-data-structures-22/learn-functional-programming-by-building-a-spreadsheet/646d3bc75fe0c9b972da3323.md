...
Aktualisiere deine Callback-Funktion, um das Ergebnis des Aufrufs von `idToText()` mit `match` als Argument zurückzugeben. Denke daran, dass dein regulärer Ausdruck die Groß- und Kleinschreibung nicht berücksichtigt, also musst du `toUpperCase()` für `match` aufrufen, bevor du ihn an `idToText()` übergibst.
...
Deine Callback-Funktion sollte `idToText()` aufrufen.
...
Du solltest deinem `idToText()`-Aufruf `match` übergeben.
