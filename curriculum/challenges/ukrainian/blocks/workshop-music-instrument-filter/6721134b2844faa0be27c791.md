...
Залишилися інструменти вже додані для вас у масив `instrumentsArr`.

Тепер оголосіть дві змінні `selectContainer` і `productsContainer` та призначте їм відповідно ваш HTML-випадаючий список і елемент із класом `products-container`.

# --hints--

У вас має бути змінна `selectContainer`.

```js
assert.exists(selectContainer);
```

Значення вашої змінної `selectContainer` має бути елементом `select`.

```js
assert.strictEqual(selectContainer, document.querySelector("select"));
```

У вас має бути змінна `productsContainer`.

```js
assert.exists(productsContainer);
```

Значення вашої змінної `productsContainer` має бути елементом `.products-container`.
