...
Vincula el primer elemento `label` al elemento `select`, y dale al elemento `select` un atributo `name`.

# --hints--

Debes darle al elemento `label` un atributo `for`.

```js
assert.notEmpty(document.querySelector('.question-block > label')?.htmlFor);
```

Debes darle al elemento `select` un atributo `id`.

```js
assert.notEmpty(document.querySelector('.answer > select')?.id);
```

Debes darle al elemento `select` un `id` que coincida con el atributo `for` del elemento `label`.

```js
assert.equal(document.querySelector('.answer > select')?.id, document.querySelector('.question-block > label')?.htmlFor);
```

Debes darle al elemento `select` un atributo `name`.
