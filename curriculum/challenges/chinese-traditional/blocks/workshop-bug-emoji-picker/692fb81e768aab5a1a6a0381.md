...
在 `Spider` 類別中，創建一個建構式，該建構式接受一個名為 `emojiElement`、型別為 `HTMLParagraphElement` 的參數。

在建構式內，使用 `super` 參數呼叫 `emojiElement` 函式，以喚起父類別 `Bug` 的建構式。接著，將 `emoji` 屬性設定為字串值 `"🕷️"`。

# --hints--

你的 `Spider` 類別應該有一個建構式。
