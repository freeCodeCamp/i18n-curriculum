...
Erstellen Sie eine weitere `@media`-Abfrage für `only screen` mit einem `max-width` von `600px`. Legen Sie darin eine `.text-with-images`-Regel an und geben Sie ihr eine `grid-template-columns`-Eigenschaft mit dem Wert `1fr`.

Dadurch wird Ihr unterer Textbereich auf kleineren Bildschirmen zu einer einzigen Spalte zusammengeklappt.

# --hints--

Sie sollten eine neue `@media`-Abfrage für `only screen and (max-width: 600px)` erstellen. Diese sollte unter Ihrer vorherigen `@media`-Abfrage liegen.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

Ihre neue `@media`-Abfrage sollte einen `.text-with-images`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

Ihr neuer `.text-with-images`-Selektor sollte eine `grid-template-columns`-Eigenschaft mit dem Wert `1fr` besitzen.
