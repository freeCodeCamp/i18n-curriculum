...
Agrega otro `input` de casilla de verificación con los atributos `id` y `value` establecidos en `"recommendation"`, y un atributo `name` establecido en `"choice"`.

Debajo del `input` de casilla de verificación, agrega otro `label` con el texto `Personal Recommendation`. El atributo `for` debe establecerse en `"recommendation"`.

# --hints--

Debes tener un `input` de casilla de verificación debajo de tu elemento `label`.

```js
assert.isNotNull(document.querySelector('fieldset:nth-of-type(3) label + input[type="checkbox"]'));
```

Tu `input` de casilla de verificación debe tener un atributo `id` establecido en `"recommendation"`.
...
Tu `input` de casilla de verificación debe tener un atributo `value` establecido en `"recommendation"`.
...
Tu elemento `label` debe tener el texto `Personal Recommendation`.
...
Tu elemento `etiqueta` debe tener un atributo `para` configurado en `"recommendation"`.
