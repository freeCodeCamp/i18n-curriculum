...
Wakati hali yako ya `if` ni kweli, unahitaji kubadilisha vipengele viwili, ukifanya "bubble" kipengele kikubwa kuelekea mwisho wa safu ya data.

Ili kufanya hivyo, tangaza kigezo cha `temp` na weka thamani yake kuwa `array[j]`. Kisha weka `array[j]` thamani ya `array[j + 1]`. Mwishowe, weka `array[j + 1]` thamani ya `temp`.

# --hints--

Ndani ya tamko lako la `if`, unapaswa kutangaza kigezo cha `temp`.
...
Unapaswa kuweka `temp` thamani ya `array[j]`.
...
Unapaswa kuweka `array[j]` thamani ya `array[j + 1]`.
...
Unapaswa kuweka `array[j + 1]` thamani ya `temp`.
