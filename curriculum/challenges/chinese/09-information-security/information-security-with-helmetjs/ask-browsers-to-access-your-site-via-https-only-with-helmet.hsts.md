...
配置 `helmet.hsts()` 以在未来 90 天内使用 HTTPS。 传递配置对象 `{maxAge: timeInSeconds, force: true}`。 你可以创建一个变量 `ninetyDaysInSeconds = 90*24*60*60;` 来用于 `timeInSeconds`。 Gitpod already has hsts enabled. 要覆盖它的设置，你需要在配置对象中把 “force” 字段设置为 true。 We will intercept and restore the Gitpod header, after inspecting it for testing.
