...
`GET` リクエストから得られたデータは、そのままでは使えません。 データを使える状態にするには、`Response` オブジェクトに `.json()` メソッドを使って、JSON データとして解析します。 If you expand the `Prototype` of the `Response` object in the browser console, you will see the `.json()` method there.
