...
Chini ya kipengele cha `div` chenye `className` ya `card-tier-badge`, ongeza `div` yenye `className` ya `card-club` na maandishi ya `Santos FC`.

# --hints--

Unapaswa kuongeza `div` yenye `className` ya `card-club` chini ya `.card-tier-badge`.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

Kipengele cha `.card-club` kinapaswa kuwa na maandishi ya `Santos FC`.
