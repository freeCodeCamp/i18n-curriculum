...
Щоб вони були на одній лінії, вам потрібно застосувати певну стилізацію для елементів `p`, щоб вони поводились як <dfn>вбудовані</dfn> елементи. Для цього почніть із додавання атрибуту `class` зі значенням `item` до першого елемента `article` під заголовком `Coffee`.
...
Перший елемент `article` повинен мати клас `item`.
