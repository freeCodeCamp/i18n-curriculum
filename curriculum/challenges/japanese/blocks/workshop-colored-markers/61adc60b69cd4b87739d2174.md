...
クラス `sleeve` を選択する新しい CSS ルールを作成してください。 `width` プロパティを `110px` に、`height` プロパティを `25px` に設定してください。　
...
`.sleeve` CSS ルールの `width` プロパティを `110px` に設定する必要があります。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.sleeve')?.width, '110px');
```

`.sleeve` CSS ルールの `height` プロパティを `25px` に設定する必要があります。
