...
Vuoi aggiungere lo stesso gradiente alle prossime due sezioni. Invece di farlo, crea un nuovo selettore di classe chiamato `bb1-window` e sposta le proprietà e i valori `height` e `background` da `.bb1a` al nuovo selettore di classe.

# --hints--

Dovresti creare un nuovo selettore di classe chiamato `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

Dovresti spostare la proprietà e il valore `height` da `.bb1a` a `.bb1-window`.
...
Dovresti spostare la proprietà e il valore `background` da `.bb1a` a `.bb1-window`.
