...
이전 수업에서 필터링된 결과의 새 배열을 반환하는 데 사용되는 `filter()` 메서드를 다루는 방법을 배웠습니다.
...
책 객체가 들어 있는 배열과 저자 이름이 들어 있는 문자열, 두 개의 매개변수를 가진 `getBooksByAuthor` 함수를 만드세요.

이 함수는 특정 저자의 책들로 이루어진 배열을 반환해야 합니다.

# --hints--

`getBooksByAuthor` 함수를 만드세요.

```js
assert.isFunction(getBooksByAuthor);
```

`getBooksByAuthor` 함수는 두 개의 매개변수를 가져야 합니다.

```js
assert.lengthOf(getBooksByAuthor, 2);
```

`getBooksByAuthor` 함수는 배열을 반환해야 합니다.

```js
assert.isArray(getBooksByAuthor(library, ''));
```

`getBooksByAuthor` 함수는 어떤 저자에 대해서도 올바른 책 수를 반환해야 합니다.
