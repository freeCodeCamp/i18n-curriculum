...
오른쪽 눈이 눈처럼 보이도록, `60%`의 테두리 반경을 주세요. 또한 `transform` 속성을 사용해 `-25`도 회전시키세요.

# --hints--

`.cat-right-eye` 선택자에 `border-radius` 속성을 `60%`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.borderRadius, '60%');
```

`.cat-right-eye` 선택자에 `transform` 속성을 `rotate(-25deg)`로 설정하세요.
