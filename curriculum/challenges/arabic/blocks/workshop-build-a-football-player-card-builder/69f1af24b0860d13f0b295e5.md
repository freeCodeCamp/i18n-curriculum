...
استبدل الآن الأرقام الثابتة المتبقية بالتالي:
...
يجب استبدال الرقم الثابت `99` لقيمة `DRI` بـ `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

يجب استبدال الرقم الثابت `41` لقيمة `DEF` بـ `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

يجب استبدال الرقم الثابت `99` لقيمة `PHY` بـ `player.phy`.
