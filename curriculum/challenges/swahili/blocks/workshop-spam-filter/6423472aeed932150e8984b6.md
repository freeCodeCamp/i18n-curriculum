...
Ukizingatia sintaksia ile ile, sahihisha `c` ili ifanana na `c`, `{`, `[`, na `(`.

# --hints--

`stockRegex` yako inapaswa kutumia darasa la herufi ili ifanane na `c`, `{`, `[`, na `(`.
...
`stockRegex` yako inapaswa kufanana na `570(k 4l3r7`.

```js
assert.match('570(k 4l3r7', stockRegex);
```

`stockRegex` yako inapaswa kufanana na `sto{k alert`.
