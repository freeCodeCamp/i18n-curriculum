...
Quando ci sono abbastanza vasi da vendere devi aggiornare il catalogo sottraendo i vasi della dimensione specificata. In quel caso, restituisci `Catalog successfully updated.`

Modifica la tua funzione per ottenere questo risultato. Poi, aggiorna la chiamata della funzione passando `10` come terzo argomento invece di `25`.

# --hints--

Dovresti stampare `sellPlants(ballerina, "small", 10)` nella console.

```js
assert.match(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

Dovresti avere solo una chiamata `sellPlants` nel tuo codice.

```js
assert.lengthOf(__helpers.removeJSComments(code).match(/(?<!function\s+)sellPlants\s*\(/g), 1)
```

`sellPlants(ballerina, "small", 25)` dovrebbe restituire `Not enough small size pots for Lavandula stoechas 'Ballerina'. Only 10 left.`
...
`sellPlants(ballerina, "small", 10)` dovrebbe restituire `Catalog successfully updated.` e sottrarre `10` dalla proprietà `small` dell'oggetto associato alla chiave `ballerina` nel catalogo.
