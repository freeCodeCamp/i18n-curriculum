...
Remova o texto `Hello` dentro do `label`, depois crie um `input` do tipo `checkbox` e um `value` de `power`.

Finalmente, exiba os valores do array ao lado de cada caixa de seleção criando um elemento `span` logo antes do fechamento do `label` com um conteúdo de texto `{power}`.

# --hints--

Você deve remover o texto `Hello` do elemento `label`.

```js
assert.notMatch(code, /hello/i)
```

Você deve criar um `input` com seu `type` definido como `checkbox` e um `value` definido como `power` como o primeiro filho do label.

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

Você deve criar um elemento `span` com `{power}` como seu conteúdo de texto como o último filho do label.
