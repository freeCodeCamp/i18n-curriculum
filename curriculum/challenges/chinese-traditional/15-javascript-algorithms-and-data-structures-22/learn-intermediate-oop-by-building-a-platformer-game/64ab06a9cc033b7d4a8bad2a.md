...
向 `startBtn` 添加 `addEventListener`，並傳入 `click` 事件和對 `startGame` 函數的引用。

單擊開始遊戲按鈕，你將會在屏幕上看到一個淺藍色的方塊，它代表主玩家。

# --hints--

你應該調用 `startBtn` 的 `.addEventListener()` 方法。
...
你應該將對 `startGame` 的引用作爲第二個參數傳遞給 `.addEventListener()` 方法。
