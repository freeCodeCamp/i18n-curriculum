...
이제 `taskData` 배열에 작업을 저장했으니, 이를 반복하여 페이지에 작업을 표시하세요.

`forEach()`에서 `taskData`을 사용하고, 매개변수로 `id`, `title`, `date`, `description`를 구조 분해하세요. 아직 아무것도 반환하지 마세요.

# --hints--

`forEach()`에서 `taskData`을 사용하세요.

```js
assert.match(code, /taskData\.forEach\(\s*/)
```

`{id, title, date, description}` 콜백 함수의 구조 분해된 매개변수로 `forEach()`을 사용하세요.
