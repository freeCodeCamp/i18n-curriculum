...
첫 번째 매개변수 `id`는 사용자가 장바구니에 추가한 상품의 `id`입니다. 두 번째 매개변수 `products`는 `Dessert` 인스턴스들의 배열입니다. 기존 `products` 배열을 직접 참조하는 대신 매개변수를 사용하면, 나중에 추가 상품 목록을 더 넣고 싶을 때 이 메서드가 더 유연해집니다.
...
`addItem` 메서드는 두 개의 매개변수 `id`와 `products`를 받아야 합니다.
