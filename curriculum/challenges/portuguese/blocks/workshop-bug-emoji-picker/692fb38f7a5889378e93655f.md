...
Em seguida, crie uma subclasse chamada `Bee` que estenda a classe `Bug` com o parâmetro de tipo genérico definido como `string`.

# --hints--

Você deve ter uma classe chamada `Bee`.
...
Sua classe `Bee` deve estender `Bug`.
...
Sua classe `Bee` deve estender `Bug` com um argumento de tipo `string`.
