...
`processGateFlow()`を`gate`と`tickIndex`で呼び出し、その結果を`result`という名前の変数に代入して、現在のティックの入力ゲートを処理してください。

# --hints--

あなたの`handleGateAtTick`関数は`processGateFlow()`メソッドを呼び出す必要があります。
...
`processGateFlow()`を`gate`と`tickIndex`で呼び出し、その結果を`result`という名前の変数に代入してください。
