...
La complejidad temporal de merge sort es `O(n log n)` porque la lista se divide continuamente a la mitad (`log n`) y luego se fusiona (`O(n)`). A diferencia de otros algoritmos de ordenar como bubble sort, merge sort no es un algoritmo de ordenamiento in situ y tiene una complejidad de espacio de `O(n)`.
