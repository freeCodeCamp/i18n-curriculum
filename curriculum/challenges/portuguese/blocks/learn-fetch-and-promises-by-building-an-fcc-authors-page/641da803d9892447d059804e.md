...
Agora crie uma tag de imagem e dê a ela a `class` `"user-img"`. Use interpolação de strings para definir o atributo `src` para a `image` que você desestruturou anteriormente. Defina o atributo `alt` como `author` seguido do texto `"avatar"`. Certifique-se de que haja um espaço entre a variável `author` e a palavra `"avatar"`, por exemplo, `"Quincy Larson avatar"`.
