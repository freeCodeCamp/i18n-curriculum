...
現在你的 `decimalToBinary` 函數已完成。 請隨意嘗試一下。

但也有一些方法可以改進它。 例如，沒有必要跟蹤輸入和商。 我們可以清理一些東西以使功能更加高效。

首先，刪除 `decimalToBinary` 函數主體中的所有內容。 然後，使用 `let` 創建一個名爲 `binary` 的變量併爲其分配一個空字符串。

# --hints--

你應該使用 `let` 來創建 `binary` 變量。

```js
assert.match(code, /let\s+binary\s*/);
```

你應該在 `decimalToBinary` 函數內創建一個名爲 `binary` 的變量。

```js
assert.match(code, /const\s+decimalToBinary\s*=\s*(\(\s*input\s*\)|input)\s*=>\s*\{\s*let\s+binary\s*/);
```

你應該爲 `binary` 分配一個空字符串。

```js
assert.match(code, /let\s+binary\s*=\s*('|"|`)\1\s*;?/);
```

你應該從 `decimalToBinary` 函數主體中刪除所有其他內容。
