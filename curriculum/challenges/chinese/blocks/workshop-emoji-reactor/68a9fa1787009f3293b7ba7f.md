...
你可能已经注意到你的代码变得相当重复！有一种更好的方法来处理多个相似的元素。

不要单独选择每个按钮，使用 `querySelectorAll(".emoji-btn")` 一次性选择所有具有 `emoji-btn` 类的按钮。将结果保存到名为 `btns` 的变量中。
