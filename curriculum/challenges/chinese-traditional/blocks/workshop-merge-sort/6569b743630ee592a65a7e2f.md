...
在測試 `merge_sort()` 函數之前，你需要創建一個基本案例，當 `array` 的長度小於或等於 `1` 時停止函數執行。

此基本案例將停止遞歸調用。 Without it, the merge sort operation would continue to run even when the list has been sorted or has no elements in it.

在函數聲明之後，立即創建一個具有以下條件的 `if` 語句：`len(array) <= 1`。 Within the body of the `if` statement add a `return` statement to stop the execution of the function.
