...
最后，你应该再次呈现歌曲，更新播放按钮的可访问文本，并从播放列表中删除重置按钮。 你还需要从 DOM 中删除 `resetButton`。

调用 `renderSongs()` 函数并以 `sortSongs()` 作为参数，再次按字母顺序呈现歌曲。

调用 `setPlayButtonAccessibleText()` 函数来更新播放按钮的可访问文本。

通过调用 `resetButton` 变量上的 `remove()` 方法从播放列表中删除重置按钮。

**注意**：现在你可以尝试删除所有歌曲，看看会发生什么。
...
你应该使用 `sortSongs()` 调用 `renderSongs` 函数。
...
你应该使用 `remove()` 方法从 DOM 中删除 `resetButton`。
