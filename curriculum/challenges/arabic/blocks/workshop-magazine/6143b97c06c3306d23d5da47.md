...
داخل عنصر `article`، أنشئ عنصر `h3` مع تعيين `class` إلى `list-title` والنص `A Brief History`. أسفله، أنشئ عنصر `p` بالنص `Of the Curriculum`. ثم أنشئ عنصر `ul` مع الصنف `lists`.

# --hints--

يجب أن تنشئ عنصر `h3` داخل عنصر `article`.

```js
assert.exists(document.querySelector('article h3'));
```

يجب أن تنشئ عنصر `p` داخل عنصر `article`.

```js
assert.exists(document.querySelector('article p'));
```

يجب أن تنشئ عنصر `ul` داخل عنصر `article`.
