...
بعد ذلك، أضف ثلاثة عناصر `td` مع النصوص التالية (بالترتيب): `$750` و `$600` و `$475`. اعطي عنصر `td` الثالث سمة `class` تسمى `current`.

# --hints--

يجب أن يحتوي عنصر `tr` الرابع الخاص بك على عنصر `th`.

```js
assert(document.querySelectorAll('table')?.[1]?.querySelector('tbody')?.querySelectorAll('tr')?.[3]?.querySelector('th'));
```

يجب أن يحتوي عنصر `th` الخاص بك على النص `Total Liabilities`.
...
يجب أن تغلف النص `Liabilities` في عنصر `span`.
