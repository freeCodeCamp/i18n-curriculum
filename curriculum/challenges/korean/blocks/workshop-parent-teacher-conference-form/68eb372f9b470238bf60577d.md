...
이제 라디오 버튼이 선택되었을 때 어떤 일이 일어나는지 정의할 차례입니다.

라디오 버튼이 선택되었을 때만 내부 원을 스타일링하므로 선택자는 `.contact-method-radio-btn:checked::before`이어야 합니다.

규칙에서 `transform`을 `translate(3px, 3px) scale(1)`로, `background-color`을 `lightgreen`로 설정하세요.

`transform`는 내부 원을 약간 이동시키고 `scale(1)`으로 전체 크기로 확대하며, 연두색 배경이 원을 채워 옵션이 선택되었음을 보여주어 보이게 만듭니다.

# --hints--

`.contact-method-radio-btn:checked::before` 가상 요소를 대상으로 해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

`transform` 속성을 `translate(3px, 3px) scale(1)`로 설정하세요.
...
`background-color` 속성을 `lightgreen`로 설정하세요.
