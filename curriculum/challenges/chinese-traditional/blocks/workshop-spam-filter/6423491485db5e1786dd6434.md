...
你的最終正則表達式將查找類似 `dear friend` 的字符串。 聲明一個 `dearRegex` 併爲其分配一個與字符串 `dear friend` 匹配的正則表達式。 記住不區分大小寫，並將其添加到你的 `denyList` 數組中。

# --hints--

你應該聲明一個 `dearRegex` 變量。

```js
assert.exists(dearRegex);
```

你的 `dearRegex` 變量應該被分配一個正則表達式。

```js
assert.instanceOf(dearRegex, RegExp);
```

你的 `dearRegex` 應該與 `dear friend` 匹配。
...
你的 `dearRegex` 應該不區分大小寫。

```js
assert.include(dearRegex.flags, 'i');
```

你的 `denyList` 數組應該包含 `dearRegex`。
