...
`.arm` 요소를 선택하고, `width`의 `30%`, `height`의 `60%`, 그리고 시계 방향으로 `background`에서 시작해 `90deg`에서 끝나는 선형 그라디언트의 `gray`를 `rgb(209, 210, 199)`에 지정하세요.
...
`.arm`에 `width`의 `--fcc-expected--`를 지정하세요. 위치는 `--fcc-actual--`입니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

`.arm`에 `height`의 `--fcc-expected--`를 지정하세요. 위치는 `--fcc-actual--`입니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

`.arm`에 `background`의 `linear-gradient(90deg, gray, rgb(209, 210, 199))`를 지정하세요.
