...
1. `#main-doc` 要素の中に、クラスが `main-section` の `section` 要素がいくつかあります。最低5つ必要です
1. 各 `.main-section` の最初の要素は `header` 要素で、そのセクションのトピックを説明するテキストが含まれている必要があります
1. クラスが `main-section` の各 `section` 要素には、含まれる各 `header` のテキストに対応する `id` も必要です。スペースはアンダースコアに置き換えられている必要があります（例：「JavaScript and Java」というヘッダーを含むセクションには対応する `id="JavaScript_and_Java"` が必要です）
...
1. さらに、navbar にはクラスが `nav-link` のリンク（`a`）要素が含まれている必要があります。クラス `main-section` の各要素に対して1つずつ必要です
1. `header` 要素は `#navbar` の中にあり、navbar のリンク（`a`）要素より前に配置されている必要があります
1. クラスが `nav-link` の各要素には、対応する `section` 内の `header` テキストに対応するテキストが含まれている必要があります（例：「Hello world」というセクション/ヘッダーがあれば、navbar にも「Hello world」というテキストを含む要素が必要です）
...
クラスが `main-section` の `section` 要素を最低5つ用意してください。
...
クラスが `nav-link` の `a` 要素を最低1つ用意してください。
