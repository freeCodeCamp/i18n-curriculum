...
你需要重新渲染歌曲，突出顯示它並設置播放按鈕的可訪問文本，因爲歌曲列表將會改變。

調用 `renderSongs` 函數並傳入 `userData?.songs` 數組作爲參數，這將顯示修改後的播放列表。

之後，調用 `highlightCurrentSong` 函數突出顯示當前歌曲（如果有），並調用 `setPlayButtonAccessibleText` 函數更新播放按鈕的可訪問文本。

# --hints--

你不應修改現有的 `deleteSong` 函數及其內容。
...
你應該使用 `userData?.songs` 調用 `renderSongs` 函數。
