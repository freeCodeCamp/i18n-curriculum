...
Da Sie das Objekt, das die Ausnahme verursacht, im Ausnahmeobjekt speichern, können Sie es sinnvoll nutzen.

Fügen Sie nach Ihrem bestehenden `print`-Aufruf `print(f'Unable to add {e.obj}: {type(e.obj)}')` hinzu.

Solche Nachrichten können äußerst hilfreich sein, wenn Sie versuchen, Ihren Code zu debuggen.

# --hints--

Sie sollten `print(f'Unable to add {e.obj}: {type(e.obj)}')` nach Ihrem bestehenden `print(f'Media Error: {e}')`-Aufruf haben.
