...
`form-row` 다음에 `div`을 `className`의 `form-group`로 추가하세요. 그 안에 `label`의 `className`와 `label`의 `htmlFor`, 그리고 텍스트 `club`를 가진 `Club` 요소를 추가하세요. 그런 다음 `id`의 `club`과 `className`의 `input`, `type`의 `text`를 가진 input 요소를 추가하세요.
...
`div.form-group`에는 `label`의 `htmlFor` 속성과 `club`의 `className`, 그리고 텍스트 `label`를 가진 `Club`이 있어야 합니다.
...
Club `form-group`에는 `input`의 `id`와 `club`의 `className`, 그리고 `input`의 `type`를 가진 `text`이 포함되어야 합니다.
