...
你需要找出用戶正在添加到購物車的產品。 記住數組有一個 `.find()` 方法。 在你的 `addItem` 函數中，聲明一個 `product` 變量，並將調用 `products` 數組上的 `.find()` 方法的結果賦值給它。

對於 `.find()` 的回調函數，傳遞一個函數，該函數接受一個參數 `item` ，並返回 `item` 的 `id` 屬性是否嚴格等於傳遞給 `addItem` 的 `id` 參數。

# --hints--

你應該在 `addItem` 函數中聲明一個 `product` 變量。
...
該回調函數應返回 `item` 的 `id` 屬性是否嚴格等於傳遞給 `addItem` 的 `id` 參數。
...
你應該將 `.find()` 方法的返回值分配給 `product` 變量。
