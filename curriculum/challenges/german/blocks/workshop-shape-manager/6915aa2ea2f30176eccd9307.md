...
Use the `oninput` event on `shapeTypeSelect` and set it to `handleShapeSelect`. This will make the function run each time the shape selection changes.

After doing that, you should be able to see the input fields of each shape. But the calculations won't work yet, and that's what you will handle next.
