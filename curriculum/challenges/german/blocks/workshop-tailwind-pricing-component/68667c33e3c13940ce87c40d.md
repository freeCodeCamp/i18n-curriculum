...
Das `span`, das die Dauer enthält, benötigt ebenfalls etwas Styling. Fügen Sie daher die Klassen `text-base`, `font-medium` und `text-fuchsia-300` hinzu.
...
Ihr `span`-Element sollte die Klasse `text-fuchsia-300` haben.
