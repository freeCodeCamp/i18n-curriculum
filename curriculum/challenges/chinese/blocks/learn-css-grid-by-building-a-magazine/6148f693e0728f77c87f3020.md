...
创建第三个 `@media` 查询，对应 `only screen` 且 `max-width` 为 `550px`。 在其中，创建一个 `.hero-title` 选择器，将 `font-size` 设置为 `6rem`；一个 `.hero-subtitle, .author, .quote, .list-title` 选择器，`font-size` 设置为 `1.8rem`；一个 `.social-icons` 选择器，`font-size` 设置为 `2rem`；一个 `.text` 选择器，`font-size` 设置为 `1.6rem`。

# --hints--

你应该有一个新的 `@media` 查询，对应 `only screen`，`max-width` 为 `550px`。 这应该在你的前两个媒体查询之后。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

你的新 `@media` 规则应该有一个 `.hero-title` 选择器、一个 `.hero-subtitle, .author, .quote, .list-title` 选择器、一个 `.social-icons` 选择器和一个 `.text` 选择器。 这些选择器应按此顺序排列。
