...
في هذه الورشة، ستتدرب على العمل مع الخاصيتين `aria-live` و `aria-label` من خلال بناء تطبيق ملاحظات يمكن الوصول إليه.

تم توفير معظم ترميز HTML وكل CSS لك. ستتضمن الخطوات الأولى إضافة الترميز المتبقي.

إذا نظرت إلى المعاينة، سترى نصًا يخبرك بالنقر على البطاقة لتحريرها. لكن إذا حاولت ذلك، فلن يعمل. لجعل عنصر `div` قابلاً للتحرير، يمكنك استخدام الخاصية `contenteditable` بهذه الطريقة:

```html
<div contenteditable="true"></div>
```

في وسم `div` الافتتاحي، أضف الخاصية `contenteditable` وضع قيمتها على `"true"`. الآن يجب أن تتمكن من النقر على العنصر وتحرير النص.

# --hints--

يجب أن يحتوي وسم `div` الافتتاحي على الخاصية `contenteditable`.

```js
assert.isTrue(document.getElementById("note").hasAttribute("contenteditable"));
```

يجب أن يحتوي عنصر `div` على الخاصية `contenteditable` مع تعيين قيمتها إلى `"true"`.
