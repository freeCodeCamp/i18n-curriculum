...
Jaro距离是两个字符串之间相似性的度量。 两个字符串的Jaro距离越大，它们就越相似。 这个距离是被归一化的，其中`0`表示它们毫无相似性，而`1`表示两个字符串完全匹配。

**定义**

给定两个字符串 \\(s_1\\) 和 \\(s_2\\) ，它们的Jaro距离是：

\begin{align}d_j = \begin{cases}0& & \text{如果 }m=0 \\ \\{\frac {1}{3}}\left({\frac {m}{|s_{1}|}}+{\frac {m}{|s_{2}|}}+{\frac {m-t}{m}}\right)& & \text{否则}\end{cases}\end{align}
...
每一个 \\(s_1\\) 中的字符都被和它在 \\(s_2\\) 中对应的字符相比较。 *换位字符* 的数量就是 <em>匹配（但在字符串中顺序不同）字符</em> 的数量除以 2。
