...
`apply_discount` のために `FixedAmountDiscount` メソッドを実装してください。このメソッドは、商品価格から固定額を差し引き、新しい価格を返す必要があります。

# --hints--

`FixedAmountDiscount` クラスには `apply_discount` メソッドが必要です。
...
`apply_discount` メソッドは `product.price - self.amount` を返す必要があります。
