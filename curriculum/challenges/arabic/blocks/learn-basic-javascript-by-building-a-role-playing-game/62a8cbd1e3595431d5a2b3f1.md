...
يجب أن تخبر اللاعب بالسلاح الذي اشتراه. بين السطرين اللذين كتبتهما للتو، استخدم `let` لتهيئة متغير جديد اسمه `newWeapon`. اجعله يساوي `weapons`.

# --hints--

يجب أن تحتوي دالة `buyWeapon` على متغير اسمه `newWeapon`.

```js
assert.match(buyWeapon.toString(), /newWeapon/);
```

يجب أن تستخدم `let` لإعلان `newWeapon`.

```js
assert.match(code, /let\s+newWeapon/);
```

يجب تهيئة `newWeapon` ليحمل قيمة `weapons`. لا تنس الفاصلة المنقوطة.

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

يجب إعلان `newWeapon` قبل تعديل `text`.
