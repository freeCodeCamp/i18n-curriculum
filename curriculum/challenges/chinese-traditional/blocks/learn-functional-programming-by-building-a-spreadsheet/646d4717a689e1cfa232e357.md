...
就像你對你的 `highPrecedence()` 函式（程式）所做的那樣，你的 `evalFormula()` 函式（程式）需要確保它已經評估並替換了所有內容。

使用三元運算子檢查 `functionExpanded` 是否與原始字串 `x` 相等。如果相等，傳回 `functionExpanded`，否則傳回再次以 `functionExpanded` 和 `cells` 作為引數呼叫 `evalFormula()` 的結果。

# --hints--

你的 `evalFormula` 函式（程式）應該使用 `return` 關鍵字。
