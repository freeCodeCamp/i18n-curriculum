...
創建一個新的 `.calories-info` 選擇器，並為它添加一個 `display` 屬性，設定為 `flex`。同時為它添加一個 `justify-content` 屬性，設定為 `space-between`，以及一個 `align-items` 屬性，設定為 `flex-end`。

# --hints--

你應該創建 `.calories-info` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info'));
```

你的 `.calories-info` 選擇器應該有一個 `display` 屬性設為 `flex`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info')?.display === 'flex');
```

你的 `.calories-info` 選擇器應該有一個 `justify-content` 屬性設為 `space-between`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info')?.justifyContent === 'space-between');
```

你的 `.calories-info` 選擇器應該有一個 `align-items` 屬性設定為 `flex-end`。
