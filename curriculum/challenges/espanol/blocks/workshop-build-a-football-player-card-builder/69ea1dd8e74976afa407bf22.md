...
A continuación, crea un tipo `Position` con el valor `typeof POSITIONS[number]`. Esto descompone el arreglo `POSITIONS` en una unión de sus literales de cadena individuales, haciendo que se vea como `"GK" | "CB" | "LB" | ...` y así sucesivamente.

# --hints--

Debes crear un alias de tipo llamado `Position`.
...
Tu tipo `Position` debe establecerse como `typeof POSITIONS[number]`.
