...
Geordnete Listen beginnen mit einem öffnenden `<ol>`-Element, gefolgt von beliebig vielen `<li>`-Elementen. Schließlich werden geordnete Listen mit dem `</ol>`-Tag geschlossen.
...
würde eine nummerierte Liste von `Garfield` und `Sylvester` erstellen.

# --instructions--

Erstellen Sie eine geordnete Liste der drei Dinge, die Katzen am meisten hassen.

# --hints--

Sie sollten eine geordnete Liste für `Top 3 things cats hate:` haben.
...
Sie sollten eine ungeordnete Liste für `Things cats love:` haben.
...
Sie sollten nur ein `ul`-Element haben.
...
Sie sollten drei `li`-Elemente innerhalb Ihres `ol`-Elements haben.
...
Ihr `ol`-Element sollte ein schließendes Tag haben.
...
Ihr `li`-Element sollte ein schließendes Tag haben.
...
Die `li`-Elemente in Ihrer ungeordneten Liste sollten nicht leer sein.
...
Die `li`-Elemente in Ihrer geordneten Liste sollten nicht leer sein.
