...
當使用者點擊 `Roll the dice` 按鈕時，應該產生五個隨機的骰子點數並顯示在螢幕上。 

建置邏輯，使點擊 `rollDiceBtn` 時產生五個介於 1 和 6（含）之間的隨機數，將 `diceValuesArr` 設定為僅包含這五個數字，並依序在 `listOfAllDice` 元素中顯示這些數字。

# --hints--

當你的 `rollDiceBtn` 被點擊時，你的 `diceValuesArr` 陣列應該包含五個元素。
...
當你的 `rollDiceBtn` 被第二次點擊時，你的 `diceValuesArr` 陣列應該只包含五個新的元素。
...
你的每個 `.die` 元素都應該包含來自 `diceValuesArr` 陣列的對應值。
