...
Зараз `songsHTML` є масивом. Якщо ви спробуєте відобразити його як є, то побачите пісні, розділені комами. Це не є бажаним результатом, бо ви хочете показати пісні у вигляді списку. Щоб це виправити, потрібно об’єднати масив у один рядок, використовуючи метод <dfn>join()</dfn>.

Метод `join()` використовується для конкатенації всіх елементів масиву в один рядок. Він приймає необов’язковий параметр, який називається `separator`, і використовується для розділення кожного елемента масиву. Наприклад:
...
Зчепіть метод `join()` до вашого методу `map()` і передайте порожній рядок як роздільник.

Щоб зчепити кілька методів разом, ви можете викликати метод `join()` на результаті методу `map()`. Наприклад:
...
Ви повинні додати `join("")` до існуючого коду.
