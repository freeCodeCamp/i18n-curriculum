...
Sasa unahitaji kusukuma `product` ndani ya safu ya data ya `items` ya gari la mkokoteni. Kumbuka kutumia neno kuu la `this`.

# --hints--

Unapaswa kuita njia ya `push` kwenye safu ya data ya `items`.
...
Kumbuka unahitaji kutumia neno kuu la `this` kupata upatikanaji wa safu ya data ya `items`.
...
Unapaswa `push` variable ya `product` kwenye safu ya data ya `items`.
