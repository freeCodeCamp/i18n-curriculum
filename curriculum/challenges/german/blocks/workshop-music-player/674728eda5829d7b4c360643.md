...
Fügen Sie ein drittes Objekt mit den folgenden Eigenschaften und Werten hinzu:
...
Ihr `allSongs` Array sollte als dritten Wert ein Objekt enthalten.

```js
assert.isObject(allSongs[2]);
```

Das dritte Objekt in Ihrem `allSongs` Array sollte eine `id` Eigenschaft mit dem Wert der Zahl `2` haben.

```js
assert.equal(allSongs[2].id, 2);
```

Das dritte Objekt in Ihrem `allSongs` Array sollte eine `title` Eigenschaft mit dem Wert des Strings `"Still Learning"` haben.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

Das dritte Objekt in Ihrem `allSongs` Array sollte eine `artist` Eigenschaft mit dem Wert des Strings `"Quincy Larson"` haben.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

Das dritte Objekt in Ihrem `allSongs` Array sollte eine `duration` Eigenschaft mit dem Wert des Strings `"3:51"` haben.

```js
assert.equal(allSongs[2].duration, "3:51");
```

Das dritte Objekt in Ihrem `allSongs` Array sollte eine `src` Eigenschaft mit dem Wert des Strings `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"` haben.
