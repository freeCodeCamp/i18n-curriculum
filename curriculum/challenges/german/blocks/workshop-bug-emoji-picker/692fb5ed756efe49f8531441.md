...
Implementieren Sie innerhalb der `Bee`-Klasse die `render`-Methode, die mit dem `override`-Schlüsselwort als Präfix versehen ist, um die `innerText`-Eigenschaft des `emojiElement` mit dem Wert der `emoji`-Eigenschaft zu aktualisieren.

# --hints--

Ihre `Bee`-Klasse sollte eine `render`-Methode haben, die das `override`-Schlüsselwort verwendet.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

Sie sollten `this.emojiElement.innerText` innerhalb der `this.emoji`-Methode von `render` auf `Bee` setzen.
