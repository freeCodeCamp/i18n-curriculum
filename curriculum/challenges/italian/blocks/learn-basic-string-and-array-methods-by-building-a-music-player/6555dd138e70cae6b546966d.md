...
Devi ottenere la canzone attualmente in riproduzione o la prima canzone nella playlist. Per farlo, crea una costante `song` e usa l'operatore OR (`||`) per assegnarla alla canzone corrente di `userData`, **oppure** alla prima canzone nell'array `userData?.songs`.

Non dimenticare di usare il concatenamento opzionale.

# --hints--

Non dovresti modificare `setPlayButtonAccessibleText` esistente e il suo contenuto.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Dovresti consultare `userData?.currentSong` o `userData?.songs[0]`.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

Dovresti assegnare `userData?.currentSong || userData?.songs[0]` a una costante `song`.
