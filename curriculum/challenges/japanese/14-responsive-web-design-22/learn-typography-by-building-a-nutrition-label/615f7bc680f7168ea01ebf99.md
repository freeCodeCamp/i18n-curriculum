...
`.no-divider` 要素の下には仕切り線を別途追加する必要が出てきました。

1 つ目の `.no-divider` 要素の後には `.divider` があります。 2 つ目の `.no-divider` 要素の後に、もう 1 つの `.divider` を 作成してください。

# --hints--

`.daily-value` の要素の最後に新しい `div` を作成する必要があります。
