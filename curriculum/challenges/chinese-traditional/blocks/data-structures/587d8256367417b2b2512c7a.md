...
在這個挑戰中，你將定義兩個方法、`findMin` 和 `findMax`。這些方法應該傳回二元搜尋樹中所持有的最小值和最大值（目前不用擔心將值添加到樹中，我們已經在背景中添加了一些）。如果你卡住了，可以反思二元搜尋樹必須成立的不變條件：每個左子樹的值都小於或相等於其父節點，而每個右子樹的值都大於或相等於其父節點。假設我們的樹只能儲存整數值。如果樹是空的，任一方法都應該傳回 `null`。
...
`findMin` 和 `findMax` 方法在空樹時應傳回 `null`。
