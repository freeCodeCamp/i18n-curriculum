...
يسمح لك المحدِّد الزائف `::first-letter` باستهداف الحرف الأول في محتوى النص لعنصر ما.

أنشئ محدِّد `.first-paragraph::first-letter` واضبط خاصية `font-size` على `6rem`. وأضف أيضًا خاصية `color` بقيمة `orangered` لجعله بارزًا.

# --hints--

يجب أن يكون لديك محدِّد `.first-paragraph::first-letter`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

يجب أن يحتوي محدِّد `.first-paragraph::first-letter` على خاصية `font-size` مضبوطة على `6rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

يجب أن يحتوي محدِّد `.first-paragraph::first-letter` على خاصية `color` مضبوطة على `orangered`.
