...
다음 섹션에서는 사용자가 호텔에 대한 평가를 남길 수 있는 기능을 제공합니다.

`fieldset` 요소를 새로 추가하고 그 안에 `legend` 요소를 중첩하세요. `legend`에는 `Ratings`라는 텍스트가 있어야 합니다.

`legend` 요소 아래에 `label` 요소를 추가하고 텍스트는 `How was the service?`로 하세요. `for` 속성은 `"service"`로 설정해야 합니다.
