...
`請問 (qǐng wèn)`, `我也是 (wǒ yě shì)`, `叫什麼 (jiào shén me)`, `你呢 (nǐ ne)`
...
`Liu Ming: 你好 (nǐ hǎo)，我叫劉明 (wǒ jiào liú míng)。`

`Anna: 我是數據分析師 (wǒ shì shù jù fēn xī shī)。BLANK？`

`Liu Ming: 我是 (wǒ shì) UI 設計師 (shè jì shī)。很高興認識你 (hěn gāo xìng rèn shì nǐ)，Anna。`
...
If you want to ask a question, don't forget to first introduce the question politely with `請問 (qǐng wèn)` and then ask it.

In order to know a person's name, you can use `叫什麼 (jiào shén me)`.
