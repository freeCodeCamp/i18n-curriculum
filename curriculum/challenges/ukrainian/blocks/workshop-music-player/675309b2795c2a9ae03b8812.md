...
Зверніть увагу, що обкладинка альбому в HTML і пісні в масиві `userData.songs` змінилися. Ми замінили оригінальні пісні на коротші, які ви можете використати для тестування свого застосунку.

Всі основні функції тепер на місці. Єдина проблема — наступна пісня не починає автоматично відтворюватися, коли закінчується поточна.

Щоб це виправити, можна налаштувати слухача подій, який буде визначати, коли поточна пісня закінчується. Слухач подій `"ended"` підходить для цього. Він спрацьовує, коли відтворення медіаконтенту досягає кінця.

Додайте слухача подій до елемента `audio`, який слухає подію `"ended"` і передайте референс на функцію `playNextSong`. Таким чином проєкт музичного плеєра буде завершено!

# --hints--

Ви повинні додати метод `addEventListener()` до вашої змінної `audio`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

Слухач подій, який ви використали для змінної `audio`, має слухати подію `"ended"`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

Ви повинні передати референс на `playNextSong` до вашого слухача подій `"ended"`.
