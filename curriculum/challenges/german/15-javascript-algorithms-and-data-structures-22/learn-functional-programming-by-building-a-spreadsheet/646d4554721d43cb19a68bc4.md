...
Deine `apply`-Funktion sollte eine implizite Rückgabe verwenden.
...
Deine `apply`-Funktion sollte auf das `spreadsheetFunctions`-Objekt zugreifen.
...
Deine `apply`-Funktion sollte auf die Eigenschaft des `spreadsheetFunctions`-Objekt zuweisen, die dem `fn`-Wert entspricht.
...
Deine `apply`-Funktion sollte die `.toLowerCase()`-Methode auf `fn` in der Eigenschaft access aufrufen.
