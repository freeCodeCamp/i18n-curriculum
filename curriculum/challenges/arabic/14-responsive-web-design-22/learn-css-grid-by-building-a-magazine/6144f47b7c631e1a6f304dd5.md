...
أحد الخيارات هو خاصية `grid-column`، التي هي اختصار لـ `grid-column-start` و `grid-column-end`. تخبر خاصية `grid-column` عنصر ال grid بالـ grid line الذي يبدأ وينتهي عنده.

قم بإنشاء قاعدة `.heading` وتعيين خاصية `grid-column` إلى `2 / 3`. هذا سيخبر عنصر `.heading` للبدء عند grid line 2 والانتهاء عند grid line 3.

# --hints--

يجب أن يكون لديك منتقي `.heading`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.heading'));
```

يجب أن يكون لمنتقي `.heading` الخاص بك الخاصية `grid-column` بقيمة `2 / 3`.
