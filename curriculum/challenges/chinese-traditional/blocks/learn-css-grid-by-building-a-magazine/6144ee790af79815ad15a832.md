...
現在你已準備好開始整理網格佈局。 CSS Grid offers a two-dimensional grid-based layout, allowing you to center items horizontally and vertically while still retaining control to do things like overlap elements.

首先創建一個 `main` 選擇器，並將 `display` 屬性設置爲 `grid`。
...
你的 `main` 選擇器應該有一個 `display` 屬性，設置爲 `grid`。
