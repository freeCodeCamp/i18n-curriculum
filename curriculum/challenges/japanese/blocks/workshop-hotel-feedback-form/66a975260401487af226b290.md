...
`option` をデフォルトで選択状態にするには、選択したい `selected` 要素に `option` 属性を追加できます。

`selected` 属性を使った例を示します。

```html
<option selected value="amazing">Amazing</option>
```

`select` 要素の中で、`selected` 要素に `option` の値で `"excellent"` 属性を追加してください。

# --hints--

`option` の値を持つ `"excellent"` 要素には `selected` 属性が必要です。
