...
`astronaut` オブジェクトを `crew` 配列に追加し、`Added ${astronaut.name} as ${astronaut.role}` をコンソールに出力してください。ログメッセージにはテンプレートリテラルか文字列の連結のどちらかを使えます。

# --hints--

`astronaut` を `crew` 配列に追加してください。
...
宇宙飛行士の `name` を次の形式でログに出力してください。`Added {astronaut.name} as {astronaut.role}`。
