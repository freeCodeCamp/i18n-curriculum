...
이제 텍스트 일부의 색상을 변경해 보겠습니다.

`style` 요소의 `h2`를 변경하면 됩니다.

요소 텍스트 색상을 담당하는 속성은 `color` 스타일 속성입니다.

`h2` 요소의 텍스트 색상을 파란색으로 설정하는 방법은 다음과 같습니다.

```html
<h2 style="color: blue;">CatPhotoApp</h2>
```

인라인 `style` 선언은 `;`로 끝내는 것이 좋은 습관임을 참고하세요.

# --instructions--

`h2` 요소의 스타일을 변경하여 텍스트 색상이 빨간색이 되게 하세요.

# --hints--

`h2` 요소에 `style` 선언이 있어야 합니다.

```js
assert.exists(document.querySelector('h2').getAttribute('style'));
```

`h2` 요소의 color가 `red`로 설정되어야 합니다.
