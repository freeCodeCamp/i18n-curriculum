...
等级徽章仍然显示硬编码的文本 `GOLD`。你需要用动态表达式替换它，以显示正确的等级。

调用 `getPlayerTier()` 并传入 `player.overallRating` 来获取等级字串，然后对结果调用 `.toUpperCase()` 以大写形式显示。别忘了用花括弧包裹整个表达式。

# --hints--

你应该在 `getPlayerTier(player.overallRating).toUpperCase()` 内使用 `.card-tier-badge`。

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

`.card-tier-badge` 元素应该为评分为 98 的玩家显示 `ELITE`。
