...
بعض الكتب تحتوي على `"Unknown"` كسنة لها. سنتعامل مع هذه الحالة الخاصة أولًا.

داخل الحلقة، تحقق مما إذا كانت `book.year` تساوي السلسلة النصية `"Unknown"`. ثم تحقق مما إذا كانت `grouped["Unknown"]` غير موجودة بعد وابدأها كمصفوفة فارغة. أضف `book` إلى `grouped["Unknown"]`.

أخيرًا، استخدم `continue` لتخطي بقية جسم الحلقة. جملة `continue` تقفز مباشرة إلى التكرار التالي.

# --hints--

يجب أن تتحقق مما إذا كانت `book.year` تساوي `"Unknown"`.

```js
assert.match(groupByDecade.toString(), /book\.year\s*===?\s*("|')Unknown\1/);
```

يجب أن تتحقق مما إذا كانت `grouped["Unknown"]` غير موجودة بعد وابدأها كمصفوفة فارغة.
...
يجب أن تضيف `book` إلى `grouped["Unknown"]`.

```js
assert.match(groupByDecade.toString(), /grouped\s*\[\s*("|')Unknown\1\s*\]\s*\.push\s*\(\s*book\s*\)/);
```

يجب أن تستخدم `continue` لتخطي إلى التكرار التالي بعد التعامل مع السنة غير المعروفة.
