...
Debe dar a `.penguin-body` un `width` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

Debe dar a `.penguin-body` una `height` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

Debe dar a `.penguin-body` un `background` de `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)`.
