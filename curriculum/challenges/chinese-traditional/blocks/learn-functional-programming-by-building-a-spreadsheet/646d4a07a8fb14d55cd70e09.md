...
為你的 `spreadsheetFunctions` 添加一個 `someeven` 屬性－使用 `.some()` 方法來檢查陣列中是否有任何元素是偶數。

# --hints--

你的 `spreadsheetFunctions` 物件應該有一個 `someeven` 屬性。

```js
assert.property(spreadsheetFunctions, "someeven");
```

你的 `someeven` 屬性應該是一個函式（程式）。

```js
assert.isFunction(spreadsheetFunctions.someeven);
```

你的 `someeven` 函式應該在陣列中有些元素是偶數時傳回 `true`。

```js
assert.isTrue(spreadsheetFunctions.someeven([1, 2, 3]));
```

如果陣列中沒有任何元素是偶數，你的 `someeven` 函式應該傳回 `false`。
