...
ستحتاج إلى دالة تُحدّث الـ DOM فعليًا. أعلن عن دالة `updateResultsList`.

ابدأ بأن تمسح الـ HTML من عنصر `resultList`.

# --hints--

يجب أن تُعرّف دالة `updateResultsList`.

```js
assert.isFunction(updateResultsList);
```

يجب أن تُعيّن دالة `updateResultsList` الخاصية `resultList.innerHTML` إلى سلسلة نصية فارغة.
