...
이제 `lunchCalories` 변수를 선언하고, `getCaloriesFromInputs`를 인수로 하여 `lunchNumberInputs`을 호출한 값을 할당하세요.

# --hints--

`calculateCalories` 함수에는 `lunchCalories` 변수가 있어야 합니다.

```js
assert.match(calculateCalories.toString(), /lunchCalories\s*=/);
```

`getCaloriesFromInputs`를 `lunchNumberInputs` 인수로 호출해야 합니다.

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*lunchNumberInputs\s*\)/);
```

`getCaloriesFromInputs`의 결과를 `lunchCalories`에 할당해야 합니다.
