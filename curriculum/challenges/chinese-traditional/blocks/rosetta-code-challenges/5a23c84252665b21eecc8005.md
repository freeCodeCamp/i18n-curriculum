...
像 Shell sort 一樣，Comb Sort 增加用於比較和交換的間距。

將間隙除以 $(1-e^{-\\varphi})^{-1} \approx 1.247330950103979$ 效果最佳，但 1.3 可能更實用。

某些實作品在間距小於某個數量時會使用插入排序。
...
撰寫一個函式（程式），使用 Comb sort 對給定的陣列進行排序。
...
`combSort([25, 32, 12, 7, 20])` 應該傳回一個陣列。

```js
assert(Array.isArray(combSort([25, 32, 12, 7, 20])));
```

`combSort([25, 32, 12, 7, 20])` 應該傳回 `[7, 12, 20, 25, 32]`。

```js
assert.deepEqual(combSort([25, 32, 12, 7, 20]), [7, 12, 20, 25, 32]);
```

`combSort([38, 45, 35, 8, 13])` 應該傳回 `[8, 13, 35, 38, 45]`。

```js
assert.deepEqual(combSort([38, 45, 35, 8, 13]), [8, 13, 35, 38, 45]);
```

`combSort([43, 36, 20, 34, 24])` 應該傳回 `[20, 24, 34, 36, 43]`。

```js
assert.deepEqual(combSort([43, 36, 20, 34, 24]), [20, 24, 34, 36, 43]);
```

`combSort([12, 33, 26, 18, 1, 16, 38])` 應該傳回 `[1, 12, 16, 18, 26, 33, 38]`。
...
`combSort([3, 39, 48, 16, 1, 4, 29])` 應該傳回 `[1, 3, 4, 16, 29, 39, 48]`。
