...
2. Ви повинні реалізувати `planRestock(pantry, shipment)` функцію, яка порівнює поточний запас із вхідною партією та повертає масив дій у формі `{ type, item }`, де `type` — одне з `"restock"`, `"discard"` або `"donate"`, а `item` — оброблений об’єкт партії.
   ...
   - Якщо у елемента партії `qty` дорівнює `0` або менше, тип дії має бути `"discard"`, незалежно від того, чи є цей елемент у запасах.
   - Інакше, якщо `sku` елемента партії вже існує у запасах, тип дії має бути `"restock"`.
   - Інакше (якщо `sku` елемента партії відсутній у запасах), тип дії має бути `"donate"`.
