...
Wakati `target` ni tupu, diski inapaswa kuhamishwa lazima kutoka `source` kwenda `target`.

Baada ya tamko la `forward`, ongeza tamko la `if` ili kukagua kama `rods[target]` ni tupu. Ikiwa ni tupu, badilisha `forward` kuwa `True`.

# --hints--

Unapaswa kuwa na tamko la `if` ili kukagua kama `rods[target]` ni tupu, na weka thamani ya `True` kwa `forward`.
