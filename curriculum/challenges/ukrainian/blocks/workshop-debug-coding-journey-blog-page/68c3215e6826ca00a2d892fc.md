...
Останньою зміною буде текст е-пошти всередині нижнього колонтитула. У попередніх уроках і практичних заняттях ви навчалися працювати з `mailto` посиланням ось так:

```html
<p>Email me at <a href="mailto:janedoe@email.com">janedoe@email.com</a></p>
```

Почніть з того, що обгорніть `camperbot@blog.io` адресу е-пошти в елемент посилання. Потім додайте атрибут `href` до елемента посилання і встановіть його значення рівним `mailto:camperbot@blog.io`.

З цією останньою зміною ви успішно вирішили всі проблеми на сторінці блогу!

# --hints--

Ваша `camperbot@blog.io` адреса е-пошти має бути обгорнута в елемент посилання.
...
Ваш елемент посилання має мати атрибут `href` зі значенням `mailto:camperbot@blog.io`.
