...
자세히 보면 현재 재생 중인 노래가 재생 목록에서 강조 표시되지 않아 어떤 노래가 재생 중인지 알 수 없습니다. 재생 중인 노래를 강조 표시하는 함수를 만들어 이 문제를 고칠 수 있습니다.

화살표 구문을 사용해 `highlightCurrentSong` 함수를 만드세요. 함수 내부에서 `querySelectorAll`를 사용해 `.playlist-song` 요소를 가져와 `playlistSongElements` 상수에 할당하세요.

# --hints--

`const`를 사용해 `highlightCurrentSong` 함수를 만들어야 합니다.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong`는 함수여야 합니다.

```js
assert.isFunction(highlightCurrentSong)
```

`highlightCurrentSong` 함수는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

`playlistSongElements` 함수 내부에 `highlightCurrentSong` 상수를 만들어야 합니다.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

`querySelectorAll()` 메서드를 사용해 `.playlist-song` 요소를 선택하고 `playlistSongElements` 상수에 할당해야 합니다.
