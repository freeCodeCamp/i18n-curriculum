...
在你的 `forumCategory` 函式（程式）內，創建一個新的 `let` 變數，命名為 `selectedCategory`，並指定一個空物件。這將用來儲存每個分類的分類名稱和類別名稱。

# --hints--

你應該使用 `let` 來宣告名為 `selectedCategory` 的變數。 

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

你應該將一個空的物件指定給 `selectedCategory` 變數。
