...
Adicione mais dois objetos ao seu `instrumentsArr` para representar os instrumentos restantes de madeiras, clarinete e oboé.

# --hints--

Você deve ter `{ category: "woodwinds", instrument: "Clarinet", price: 200 }` dentro do seu `instrumentsArr`.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

Você deve ter `{ category: "woodwinds", instrument: "Oboe", price: 4000 }` dentro do seu `instrumentsArr`.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

Você ainda deve ter `{ category: "woodwinds", instrument: "Flute", price: 500 }` dentro do seu `instrumentsArr`.
