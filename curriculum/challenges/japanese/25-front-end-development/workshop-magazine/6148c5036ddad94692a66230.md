...
これで、新しいアイコン要素を追加すると自動配置アルゴリズムが実行されます。 ただし、アルゴリズムのデフォルトでは、新しい列の幅は `auto`となり、今ある列と一致しません。

この動作は `grid-auto-columns` プロパティを使用して上書きできます。 `.social-icons` セレクターの `grid-auto-columns` プロパティを `1fr` に設定してください。

# --hints--

`.social-icons` セレクターの `grid-auto-columns` プロパティを `1fr` に設定する必要があります。
