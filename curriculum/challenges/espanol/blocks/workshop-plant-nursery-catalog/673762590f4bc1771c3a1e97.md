...
Antes de continuar, elimina las llamadas `console.log` de tu código.

# --hints--

No deberías tener `console.log(removePlant(ballerina))` en tu código.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*removePlant\s*\(\s*ballerina\s*\)\s*\)/);
```

No deberías tener `console.log(sellPlants(ballerina, "small", 10));` en tu código.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

No deberías tener `console.log(catalog)` en tu código.
