...
الآن ستُظهر أن دالتك تعمل. في نهاية برنامجك، استدعِ دالة `shortest_path` مع `adj_matrix` و`start_node` بقيمة `0` و`target_node` بقيمة `5` كمعلمات.

بهذا، تكتمل خوارزمية أقصر مسار!

# --hints--

يجب أن تستدعي دالة `shortest_path` بثلاث معلمات: `adj_matrix`، و`0`، و`5`.
