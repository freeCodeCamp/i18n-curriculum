...
創建一個名為 `filteredItems` 的變數，並將使用 `filter()` 方法篩選 `items` 陣列的結果指定給它。將一個匿名函式作為引數傳遞給 `filter()` 方法，該匿名函式接受 `item` 作為引數。在匿名函式內，目前只需傳回 `item`。

# --hints--

你應該創建一個名為 `filteredItems` 的變數。

```js
assert.match(code, /(const|let|var)\s+filteredItems\s*=/);
```

你應該在 `items` 陣列上使用 `filter()` 方法。

```js
assert.match(code, /items\.filter\s*\(/);
```

你的 `filter()` 方法應該接受一個匿名函式，該函式以 `item` 作為引數並傳回 `item`。
