...
在 `tbody` 中，添加一個 `tr`，並將 `class` 設置爲 `total`。 在其中，添加一個帶有文本 `Total Net Worth` 的 `th`，並將 `Net Worth` 包裹在 `span` 中，並將 `class` 設置爲 `sr-only`。

然後添加三個 `td` 元素，給第三個元素添加一個設置爲 `current` 的 `class`， 並給每個元素添加文本： `$-171`、`$136`、`$334`。
