...
それでは、90から99までの数字を見ていきましょう。

スペイン語で90は `noventa` と呼ばれます。

これらの数字は、十の位の単語 `noventa`（90）に、接続詞 `y`（「と」）を続け、さらに一の位の数字（1～9）を付けて作られます。
...
この範囲で91の数字 `noventa y uno` は特別で、男性名詞の前では `noventa y un` に変わります。例えば：

`Noventa y un años` - 91歳。
