...
Всередині `map()` додайте інструкцію `return` з backticks, у якій ви інтерполюєте всі елементи, відповідальні за відображення деталей пісні.

Усередині backticks створіть елемент `li` з атрибутом `id`, рівним `song-${song.id}`, та атрибутом `class`, рівним `playlist-song`.

# --hints--

Ви повинні додати інструкцію `return` з порожньою парою backticks.

```js
assert.match(code, /return\s*`/)
```

Ви повинні створити тег `li` всередині backticks.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

Ваш відкриваючий тег `li` має мати атрибут `id`, встановлений у `song-${song.id}`.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

Ваш відкриваючий тег `li` має мати атрибут `class`, встановлений у `playlist-song`.
