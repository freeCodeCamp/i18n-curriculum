...
Per il `span` all'interno dell'elemento `p` che contiene il testo `/month`, assegna le classi `text-base`, `font-medium` e `text-gray-500`. Questo lo distinguerà dal prezzo vero e proprio rendendo il testo più chiaro.

# --hints--

Il tuo elemento `span` dovrebbe avere la classe `text-base`.
...
Il tuo elemento `span` dovrebbe avere la classe `font-medium`.
...
Il tuo elemento `span` dovrebbe avere la classe `text-gray-500`.
