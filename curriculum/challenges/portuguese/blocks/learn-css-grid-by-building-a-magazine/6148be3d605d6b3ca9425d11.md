...
A legenda também precisa ser estilizada. Crie um seletor `.hero-subtitle` e dê a ele a propriedade `font-size` com `2.4rem`, a propriedade `color` definida com `orangered` e a propriedade `text-align` definida com `center`.

# --hints--

Você deve ter um seletor `.hero-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

O seletor `.hero-subtitle` deve ter a propriedade `font-size` definida como `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

O seletor `.hero-subtitle` deve ter a propriedade `color` definida como `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

O seletor `.hero-subtitle` deve ter a propriedade `text-align` definida como `center`.
