...
Kila `tab` inawakilisha moja ya vifungo katika orodha ya tabo. Mtumizi anapaswa kuwa na uwezo wa kubofya tabo ili kuonyesha paneli inayolingana na kuficha zingine.

Katika moduli za awali, ulijifunza jinsi ya kuambatisha msikilizaji wa tukio la `"click"` kwa kila kitufe katika `NodeList` kama ifuatavyo:
...
Tumia njia ya `addEventListener` kuambatisha msikilizaji wa tukio la `"click"` kwa kila kitufe cha `tab`. Hoja ya pili ya njia ya `addEventListener` inapaswa kuwa kitendakazi cha mshale cha tupu `() => {}` kwa sasa.

# --hints--

Unapaswa kuwa na `tab.addEventListener('click', () => {})` ndani ya mzunguko wa `forEach`.
