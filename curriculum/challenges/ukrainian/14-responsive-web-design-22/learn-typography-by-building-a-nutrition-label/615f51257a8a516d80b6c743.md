...
Створіть новий елемент `div` під елементом `.large` та надайте йому атрибут `class` зі значенням `calories-info`.

# --hints--

Створіть інший елемент `div`.

```js
assert(document.querySelectorAll('div')?.length === 4);
```

Новий `div` повинен мати атрибут `class` зі значенням `calories-info`.

```js
assert(document.querySelector('.label')?.lastElementChild?.classList?.contains('calories-info'));
```

Новий `div` повинен йти після елемента `.large`.
