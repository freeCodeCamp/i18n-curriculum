...
向你的 `setPlayerCards` 函數添加一個名爲 `arr` 的新參數，併爲其分配一個默認值 `players`。 請記住，你在 `175` 行從 `myFavoriteFootballTeam` 對象中解構了 `players` 變量。

# --hints--

你的 `setPlayerCards` 函數應該採用 `arr` 參數。

```js
assert.match(code, /const\s+setPlayerCards\s*=\s*\(\s*arr/);
```

你的 `arr` 參數應該有一個默認值 `players`。
