...
イベントリスナーのアロー関数内で、`if`を追加して`userData?.currentSong`が`null`かどうかを確認してください。

`if`ブロック内で、`playSong()`配列の最初の曲の`id`を引数にして`userData?.songs`関数を呼び出してください。これにより、プレイリストの最初の曲が最初に再生されます。
...
`playSong`ブロック内で`userData?.songs[0].id`を引数にして`if`関数を呼び出してください。
