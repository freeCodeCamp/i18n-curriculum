...
In the game of Golf, each hole has a `par`, meaning, the average number of `strokes` a golfer is expected to make in order to sink the ball in the hole to complete the play. 根據你實際 `strokes` 與 `par` 的差距，會有不同的暱稱來描述成績。
