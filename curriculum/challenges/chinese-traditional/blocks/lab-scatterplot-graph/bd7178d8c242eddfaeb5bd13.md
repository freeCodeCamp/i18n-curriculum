...
在這個專案中你將使用來自 `https://cdn.freecodecamp.org/curriculum/labs/data/scatterplot/cyclist-data.json` 的資料來建置散點圖。

測試要求使用 D3 axis 屬性來產生軸，該屬性會自動在軸上產生刻度。這些刻度是通過 D3 測試所必需的，因為它們的位置用來判斷圖表元素的對齊。

**目標：** 完成以下使用者故事並通過所有測試以完成實驗。

**使用者故事：**

1. 你的圖表應該有一個標題，並對應 `id="title"`。
1. 你的圖表應該有一個 `g` 元素的 x 軸，並對應有 `id="x-axis"`。
1. 你的圖表應該有一個 `g` 元素的 y 軸，並且對應有 `id="y-axis"`。
1. 兩個軸都應包含多個刻度標籤，每個標籤都有對應的 `class="tick"`。
1. 你的圖表應該為每個資料點有一個 `circle` 元素，並帶有對應的 `class="dot"` 來顯示資料。
1. 每個 `.dot` 都應該有包含 `year` 和 `time` 值的 `data-xvalue` 和 `data-yvalue` 屬性。對於 `data-xvalue`，整數（完整年份）或 Date 物件皆可用於測試評估。對於 `data-yvalue`（分鐘），請使用 Date 物件。
1. `.dot` 元素的 `data-xvalue` 屬性應該與提供的資料順序相符。
1. `.dot` 元素的 `data-yvalue` 屬性應該與提供的資料順序相符。
1. 每個 `.dot` 元素應與 x 軸上的對應值對齊。
1. 每個 `.dot` 元素應與 y 軸上的對應值對齊。
1. y 軸上應該有多個刻度標籤，時間格式為 `%M:%S`。
1. x 軸上應該有多個刻度標籤顯示年份。
1. x 軸標籤的範圍應該在實際 x 軸 `data` 的範圍內。
1. y 軸標籤的範圍應該在實際 y 軸 `data` 的範圍內。
1. 應該有一個包含描述性文字且具有 `id="legend"` 的圖例。
1. 你應該能將滑鼠移到某個區域上，並看到一個對應的 `id="tooltip"` 工具提示，顯示該區域的更多資訊。
1. 你的工具提示應該有一個 `data-year` 屬性，該屬性對應於作用中區域的 `data-xvalue`。
...
圖表應該有一個 `id` 為 `title` 的元素來包含標題。

```js
assert.exists(document.getElementById('title'));
```

圖表應該有一個帶有對應 `id="x-axis"` 的 `x-axis`。
...
圖表應該有一個帶有對應 `id="y-axis"` 的 `y-axis`。
...
每個點的 `data-xvalue` 和 `data-yvalue` 應該在實際資料的範圍內且符合正確的資料格式。對於 `data-xvalue`，整數（完整年份）或 Date 物件在測試評估時是可接受的。對於 `data-yvalue`（分鐘），請使用 Date 物件。
...
`data-xvalue` 和其對應的點應與 x 軸上的對應點/值對齊。
...
`data-yvalue` 和其對應的點應與 y 軸上的對應點/值對齊。
...
你可以在 y 軸上看到多個以 `%M:%S` 時間格式顯示的刻度標籤。
...
當暫留在某個區域時，你的散點圖應該有一個對應的 `id="tooltip"` 工具提示，顯示該區域的更多資訊。
...
工具提示應該有一個 `data-year` 屬性，該屬性對應於作用中區域的 `data-xvalue`。
