...
Com base no diálogo, qual frase mostra o uso correto de `going to` com base em evidências presentes?
...
Esta frase usa `will` para uma ação futura provável, que não tem o sentido de uma decisão tomada com base em evidências presentes.
...
Aqui, `will` é usado para uma ação futura baseada em uma situação atual, mas `going to` seria mais apropriado já que a decisão parece baseada na expertise atual da equipe.
...
Esta frase usa `will` para uma decisão espontânea (`calling the designer`), o que é apropriado, mas não ilustra o uso de `going to` baseado em evidências presentes.
