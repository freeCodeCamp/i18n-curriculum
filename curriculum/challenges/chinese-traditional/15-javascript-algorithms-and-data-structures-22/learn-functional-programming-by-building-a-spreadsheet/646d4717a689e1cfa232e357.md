...
就像你對 `highPrecedence()` 函數所做的那樣，你的 `evalFormula()` 函數需要確保它已評估並替換了所有內容。

使用三元組檢查 `functionExpanded` 是否等於原始字符串 `x`。 如果是，則返回 `functionExpanded`，否則返回使用 `functionExpanded` 和 `cells` 作爲參數再次調用 `evalFormula()` 的結果。

# --hints--

你的 `evalFormula` 函數應該使用 `return` 關鍵字。
