...
أنشئ عبارة `if` للتحقق مما إذا كان كائن `allCategories` يحتوي على خاصية `id`. تذكر، يمكنك استخدام طريقة `hasOwnProperty()` لهذا الغرض.

# --hints--

يجب أن تحتوي على عبارة `if` تتحقق مما إذا كان كائن `allCategories` يحتوي على خاصية `id`
