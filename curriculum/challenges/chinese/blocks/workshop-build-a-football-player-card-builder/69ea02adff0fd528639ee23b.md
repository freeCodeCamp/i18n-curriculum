...
接下来，创建一个 `loadPlayer` 函数，返回类型为 `PlayerData`。在函数内部，添加一个 `try...catch` 语法，其中 `error` 是 `catch` 块的参数。其他部分暂时保持为空。

# --hints--

你应该创建一个名为 `loadPlayer` 的函数。
...
你的 `loadPlayer` 函数应该有一个返回类型为 `PlayerData`。
...
你的 `loadPlayer` 函数应该包含一个 `try...catch` 块。
