...
لإيقاف الأغنية نهائيًا، استخدم طريقة `pause()` على المتغير `audio`. `pause()` هي طريقة من واجهة برمجة تطبيقات صوت الويب لإيقاف ملفات الموسيقى مؤقتًا.

# --hints--

يجب أن تزيل دالة `pauseSong` الصنف `playing` من `playButton`.
...
يجب أن تستدعي دالة `pauseSong` طريقة `pause()` على `audio`.
