...
在 `verify_card_number` 函数的第二个 `for` 循环下方，创建一个名为 `total` 的变量，并将奇数和偶数之和的值赋给它。 将 `total` 打印到控制台。

# --hints--

`verify_card_number` 函数中应该有 `total = sum_of_odd_digits + sum_of_even_digits`。
...
你应该在 `verify_card_number` 函数中包含 `print(total)`。
