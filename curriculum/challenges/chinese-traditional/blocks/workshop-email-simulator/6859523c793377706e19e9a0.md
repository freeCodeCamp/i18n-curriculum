...
現在為電子郵件添加時間戳。在 Email 類別的 `__init__` 方法中，創建一個 `timestamp` 屬性並指定當前時間給它，以自動設定電子郵件建立的時間戳。

這有助於追蹤訊息何時被發送和接收。

# --hints--

你應該在 Email 的 `self.timestamp` 方法中有一個 `__init__`，並將 `datetime.datetime.now()` 指定給它。
