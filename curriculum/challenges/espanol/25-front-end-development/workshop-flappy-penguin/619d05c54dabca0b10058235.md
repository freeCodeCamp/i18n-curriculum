...
Cree una propiedad CSS personalizada llamada `--penguin-face`, y configúrela en `white`.
...
Debe dar a `:root` una propiedad `--penguin-face`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--penguin-face'));
```

Debe dar a `:root` una propiedad `--penguin-face` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.
