...
使用類別選擇器，為你的 `.cat-mouth-line-left` 元素設定 `position` 為 `absolute`、`top` 為 `88px` 和 `left` 為 `74px`。這會將它移動到正確的位置。

# --hints--

你應該有一個 `.cat-mouth-line-left` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left'))
```

你的 `.cat-mouth-line-left` 選擇器應該有一個設定為 `absolute` 的 `position` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.position === 'absolute')
```

你的 `.cat-mouth-line-left` 選擇器應該有一個 `top` 屬性設定為 `88px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.top === '88px')
```

你的 `.cat-mouth-line-left` 選擇器應該有一個 `left` 屬性設定為 `74px`。
