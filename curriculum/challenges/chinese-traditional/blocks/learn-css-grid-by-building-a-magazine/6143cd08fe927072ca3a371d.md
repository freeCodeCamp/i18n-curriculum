...
在你的 `.image-wrapper` 元素中，給第二個 `img` 元素添加一個值爲 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png` 的 `src`、一個值爲 `image of a calculator project` 的 `alt`、一個值爲 `lazy` 的 `loading` 屬性、一個值爲 `image-2` 的 `class`、一個值爲 `400` 的 `width` 屬性和一個值爲 `400` 的 `height` 屬性。

# --hints--

你的第一個 `img` 元素應該有一個 `src` 屬性，值爲 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`。
