...
爲了處理當前歌曲的信息並跟蹤其播放時間，創建 `currentSong` 和 `songCurrentTime` 屬性。 將值分別設置爲 `null` 和 `0`。

# --hints--

你不應修改現有的 `userData` 對象及其內容。

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

你的 `userData` 對象應該有一個設置爲 `null` 的 `currentSong` 鍵。

```js
assert.propertyVal(userData, "currentSong", null)
```

你的 `userData` 對象應該有一個設置爲 `0` 的 `songCurrentTime` 鍵。
