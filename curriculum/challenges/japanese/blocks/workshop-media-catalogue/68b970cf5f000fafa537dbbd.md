...
例外を引き起こしているオブジェクトを例外オブジェクトに保存しているので、それを活用できます。

既存の`print`呼び出しの後に`print(f'Unable to add {e.obj}: {type(e.obj)}')`を追加してください。

この種のメッセージは、コードをデバッグしようとしているときに非常に役立ちます。

# --hints--

既存の`print(f'Unable to add {e.obj}: {type(e.obj)}')`呼び出しの後に`print(f'Media Error: {e}')`があるはずです。
