...
سو دوكو (تعني باليابانية *مكان الأرقام*) هو اسم لمفهوم لغز شائع. أصله غير واضح، لكن يجب نسب الفضل إلى ليونهارد أويلر الذي اخترع فكرة لغز مشابهة وأكثر صعوبة تُسمى المربعات اللاتينية. الهدف من ألغاز سو دوكو هو ملء الفراغات (أو الأصفار) في شبكة 9 في 9 بحيث يحتوي كل صف وعمود وصندوق 3 في 3 على كل الأرقام من 1 إلى 9. أدناه مثال على شبكة بداية لغز نموذجية وشبكة الحل الخاصة بها.
...
لغز سو دوكو المصمم جيدًا له حل وحيد ويمكن حله بالمنطق، رغم أنه قد يكون من الضروري استخدام طرق "التخمين والاختبار" لاستبعاد الخيارات (وهناك جدل كبير حول هذا). تعقيد البحث يحدد صعوبة اللغز؛ المثال أعلاه يُعتبر سهلًا لأنه يمكن حله بالاستنتاج المباشر البسيط.

مصفوفة `puzzlesArr` تحتوي على سلاسل نصية لألغاز سو دوكو مختلفة الصعوبة، لكن جميعها لها حلول فريدة.

بحل جميع الألغاز في `puzzlesArr`، احسب مجموع الأرقام الثلاثية الموجودة في الزاوية العلوية اليسرى من كل شبكة حل؛ على سبيل المثال، 483 هو الرقم الثلاثي الموجود في الزاوية العلوية اليسرى من شبكة الحل أعلاه.
...
يجب أن تُرجع الدالة `suDoku(testPuzzles1)` رقمًا.

```js
assert(typeof suDoku(_testPuzzles1) === 'number');
```

يجب أن تُرجع الدالة `suDoku(testPuzzles1)` القيمة `1190`.

```js
assert.strictEqual(suDoku(_testPuzzles1), 1190);
```

يجب أن تُرجع الدالة `suDoku(testPuzzles2)` القيمة `24702`.
