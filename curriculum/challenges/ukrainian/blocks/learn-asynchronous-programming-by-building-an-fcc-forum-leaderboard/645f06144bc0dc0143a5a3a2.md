...
Кожен допис на форумі міститиме список зображень аватарів користувачів, які представляють усіх учасників розмови з цієї теми.

Почніть зі створення стрілкової функції з назвою `avatars`, з двома параметрами `posters` та `users`.

# --hints--

У вас має бути порожня стрілкова функція з назвою `avatars`.

```js
assert.match(code, /const\s+avatars\s*=\s*\([\s\S]*\)\s*=>\s*{\s*}/);
```

У функції з назвою `posters` мають бути параметри з іменами `users` та `avatars`.
