...
Hamisha sifa na thamani ya `background-color` kutoka `.fb1` kwenda `.fb1b`.

# --hints--

Unapaswa kuondoa `background-color` kutoka `.fb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb1")?.backgroundColor);
```

Unapaswa kuongeza `background-color` ya `--building-color4` kwenye `.fb1b`.
