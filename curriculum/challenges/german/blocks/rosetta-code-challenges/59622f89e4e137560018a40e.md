...
title: Hofstadter-Figure-Figure-Folgen
challengeType: 1
forumTopicId: 302286
...
Die Hofstadter-Figure-Figure-Folgen $R_n$ und $S_n$ sind definiert durch
...
Die Folge $R_n$ ist durch die Rekursionsgleichung $R_n = R_{n-1} + S_{n-1}$ definiert, während $S_n$ als Folge positiver ganzer Zahlen definiert ist, die nicht in der Folge $R_n$ enthalten sind.

# --instructions--

Erstellen Sie zwei Funktionen mit den Namen `ffr` und `ffs`, die für jeden Index `R(n)` jeweils `S(n)` bzw. `n` zurückgeben. Beachten Sie, dass die Hofstadter-Figure-Figure-Folgen 1-indiziert sind, mit $R_1 = 1$ und $S_1 = 2$.

Es darf kein maximaler Wert für `n` angenommen werden.
...
`ffr` sollte eine ganze Zahl zurückgeben.

```js
assert(Number.isInteger(ffr(1)));
```

`ffs` sollte eine ganze Zahl zurückgeben.
