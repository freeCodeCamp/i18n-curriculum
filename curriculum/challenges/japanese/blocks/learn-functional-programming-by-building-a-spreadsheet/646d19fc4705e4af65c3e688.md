...
あなたの `addCharacters` 関数は最終的に文字の範囲を返します。セルIDの配列を返すようにしたいです。`.map()` の呼び出しに `charRange()` メソッドをチェーンしてください。まだコールバック関数は渡さないでください。

# --hints--

`.map()` の呼び出しに `charRange()` をチェーンする必要があります。
