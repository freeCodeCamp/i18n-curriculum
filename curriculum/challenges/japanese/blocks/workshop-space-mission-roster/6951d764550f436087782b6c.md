...
スクリプトに`console.log()`の呼び出しを追加して`EVAReadySquad`配列を検証しましたが、その呼び出しはもう必要ありません。今後のステップでターミナルが散らからないように、現在の`for`ループを削除して、`EVAReadySquad`の宇宙飛行士をコンソールに記録する処理をなくしましょう。

# --hints--

`for`の宇宙飛行士をコンソールに記録する`EVAReadySquad`ループを削除してください。ループ全体のブロックを削除する必要があります。
