...
Kitendakazi cha `parseCard` kitachukua mfuatano wa herufi wa orodha ghafi na kurudisha kitu kilicho na muundo. Kabla ya kuunda kitendakazi chako, weka njia ya kuona matokeo yake unavyofanya kazi.

Fanya wito wa `parseCard` ukiwa na `rawCatalogCards[2]` kama hoja, weka thamani ya matokeo katika kigezo kinachoitwa `cardResult`, na andika `cardResult` kwenye konsoli.

Kwa sasa `parseCard` haina mwili, kwa hivyo inarudisha `undefined`. Unapoongeza msimbo kwenye kitendakazi katika hatua zinazofuata, matokeo yaliyoandikwa yatakuonyesha hasa kile kitendakazi kinachorudisha kila hatua.

# --hints--

Unapaswa kufanya wito wa `parseCard` ukiwa na `rawCatalogCards[2]` kama hoja.

```js
assert.match(__helpers.removeJSComments(code), /parseCard\s*\(\s*rawCatalogCards\s*\[\s*2\s*\]\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya wito wa `parseCard` ukiwa na `rawCatalogCards[2]` katika kigezo kinachoitwa `cardResult`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+cardResult\s*=\s*parseCard\s*\(\s*rawCatalogCards\s*\[\s*2\s*\]\s*\)\s*;?/);
```

Unapaswa kuandika `cardResult` kwenye konsoli.
