...
現在，檢查給定數字 `num` 是否已經存在於當前3x3方格的單元格中。

用 `if` 語句替換 `pass`，該語句檢查數獨板當前單元格中的數字是否等於 `num` 。 如果是，則從 `if` 主體返回 `False`，表明該數字不是一個有效的選擇。

# --hints--

你應該刪除 `pass` 並創建一個 `if` 語句，檢查數獨板的當前單元格是否等於 `num` 。
