...
Під цим додайте три елементи `td` і надайте їм такий текст у порядку: `$200`, `$300` і `$400`. Третьому елементу `td` додайте атрибут `class`, встановлений у `current`.
...
Текст `Annual anticipated expenses, such as payroll.` слід обгорнути в елемент `span`.
