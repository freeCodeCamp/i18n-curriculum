...
Você deve ter uma variável chamada `invalid_records` no final do seu loop `for` dentro da função `validate`.
...
Sua variável `invalid_records` deve ter o valor de `find_invalid_records(**dictionary)`.
