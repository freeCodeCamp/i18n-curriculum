...
في دالتك `elemValue`، أعلنت صراحة عن دالة تسمى `inner` وأعدتها. مع ذلك، بما أنك تستخدم صيغة الدالة السهمية، يمكنك إعادة دالة ضمنيًا. على سبيل المثال:

```js
const curry = soup => veggies => {};
```

`curry` هي دالة تأخذ معلمة `soup` وتُعيد دالة تأخذ معلمة `veggies`. باستخدام هذه الصيغة، حدّث دالتك `addCharacters` لتُعيد دالة فارغة تأخذ معلمة `character2`.

# --hints--

يجب أن تستخدم دالتك `addCharacters` إعادة ضمنية.
...
يجب أن تُعيد دالتك `addCharacters` دالة سهمية تحتوي على معلمة `character2`.
...
يجب أن تكون الدالة السهمية الداخلية فارغة.
