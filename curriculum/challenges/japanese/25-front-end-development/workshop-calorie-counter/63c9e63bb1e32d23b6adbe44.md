...
When you need to lower case a string, you can use the <dfn>toLowerCase()</dfn> method. このメソッドは、呼び出し元の文字列の値を小文字に変換したものを返します。
...
ではまず、`output.innerHTML` の文字列内に `span` 要素を追加してください。 Create that, and give it a `class` attribute set to the `surplusOrDeficit` variable. Your `surplusOrDeficit` variable should be converted to lower case using the `toLowerCase()` method.

`span` 要素内にはまだテキストを入れないでください。

# --hints--

テンプレートリテラル内に `span` 要素を作成してください。
