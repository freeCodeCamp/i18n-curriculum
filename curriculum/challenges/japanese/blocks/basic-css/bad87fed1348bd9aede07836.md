...
例えば、要素の背景色を `green` にしたい場合は、`style` 要素の中に次のように書きます。
...
`silver-background` というクラスを作成し、`background-color` に `silver` を代入してください。このクラスを `div` 要素に割り当ててください。

# --hints--

`div` 要素に `silver-background` クラスが付いているはずです。
...
`silver-background` というクラスが `style` 要素内に定義されていて、`silver` の値が `background-color` プロパティに代入されているはずです。
