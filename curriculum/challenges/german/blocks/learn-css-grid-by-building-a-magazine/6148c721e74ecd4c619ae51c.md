...
Dein `.text`-Element ist kein CSS-Gitter, du kannst aber Spalten innerhalb eines Elements ohne die Verwendung von Gitter erstellen, indem du die `column-width`-Eigenschaft verwendest.

Gib deinem `.text`-Selektor eine `column-width`-Eigenschaft mit dem Wert `25rem`.

# --hints--

Dein `.text`-Selektor sollte eine `column-width`-Eigenschaft mit dem Wert `25rem` haben.
