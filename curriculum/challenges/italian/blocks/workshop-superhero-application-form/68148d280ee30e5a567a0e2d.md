...
Rimuovi il testo `Hello` all’interno di `label`, poi crea un `input` di tipo `checkbox` e un `value` di `power`.

Infine, mostra i valori dell’array accanto a ogni casella di spunta creando un elemento `span` subito prima della chiusura di `label` con un contenuto testuale di `{power}`.

# --hints--

Dovresti rimuovere il testo `Hello` dall’elemento `label`.

```js
assert.notMatch(code, /hello/i)
```

Dovresti creare un `input` con il suo `type` impostato su `checkbox` e un `value` impostato su `power` come primo figlio dell’etichetta.

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

Dovresti creare un elemento `span` con `{power}` come contenuto testuale come ultimo figlio dell’etichetta.
