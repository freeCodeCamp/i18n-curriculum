...
创建一个 `body` 选择器，并为其指定一个设置为 `sans-serif` 的 `font-family` 属性，以及一个设置为 `#0a0a23` 的 `color` 属性。
...
你的 `body` 选择器应该有一个 `font-family` 属性，设置为 `sans-serif`。
...
你的 `body` 选择器应将 `color` 属性设置为 `#0a0a23`。
