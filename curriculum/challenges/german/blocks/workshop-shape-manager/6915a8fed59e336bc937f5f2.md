...
Die letzte Gruppe von Elementen, die Sie auswählen sollen, sind die Ergebnis-Elemente. Verwenden Sie Ihre `getElement`-Funktion, um das Element mit den IDs `result-text` und `result-card` zu erhalten. Beide sollten `HTMLElement` als Typ verwenden.

Denken Sie daran, dass Sie die Variablen `resultText` und `resultCard` bereits deklariert haben, sodass Sie ihnen nur noch Werte zuweisen müssen.

# --hints--

Sie sollten der Variablen `resultText` einen Wert zuweisen.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` sollte das `result-text`-Element auswählen.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

Sie sollten `HTMLElement` als Typ des `getElement` verwenden, mit dem Sie das `result-text`-Element auswählen.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

Sie sollten der Variablen `resultCard` einen Wert zuweisen.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` sollte das `result-card`-Element auswählen.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

Sie sollten `HTMLElement` als Typ des `getElement` verwenden, mit dem Sie das `result-card`-Element auswählen.
