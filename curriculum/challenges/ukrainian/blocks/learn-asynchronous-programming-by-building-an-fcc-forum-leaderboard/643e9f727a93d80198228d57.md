...
Щоб відобразити заголовок теми, додайте `p` елемент всередину першого `td` елемента.

Між тегами абзацу додайте вставлений вираз, який містить змінну `title`. Потім додайте клас з назвою `"post-title"` всередину відкриваючого тегу абзацу.

# --hints--

У вас має бути `p` елемент всередині першого `td` елемента.

```js
assert(/return\s*`\s*<tr>\s*<td>\s*\S*\s*<p.*>\s*.*\s*<\/p>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```

У відкриваючому `"post-title"` тегу має бути клас з назвою `p`.
...
Змінна `title` має бути між тегами абзацу.
