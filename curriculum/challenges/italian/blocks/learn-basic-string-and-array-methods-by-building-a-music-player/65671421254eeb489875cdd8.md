...
All'interno della funzione di callback, usa il metodo `removeAttribute()` per rimuovere l'attributo `"aria-current"`. Questo eliminerà l'attributo per ciascuna delle canzoni.

# --hints--

Non dovresti modificare il `forEach()` esistente e il suo contenuto.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

Dovresti usare il metodo `removeAttribute()` su `songEl` e passare `"aria-current"`.
