...
为你的 `constraints` 字典添加另一个密钥 `gender`。按照你在前面步骤中编写的表达式格式，查看 `gender` 是否为字串。然后，使用 `and` 操作符检查小写的 `gender` 是否在 `('male', 'female')` 中。

# --hints--

你的 `constraints` 字典应包含一个密钥 `gender`。
...
你的 `constraints` 字典中的 `gender` 密钥应具有值 `isinstance(gender, str) and gender.lower() in ('male', 'female')`。
