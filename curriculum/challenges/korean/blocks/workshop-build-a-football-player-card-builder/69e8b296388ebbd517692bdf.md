...
DEF와 PHY 입력 모두에 `value`와 `onChange` 속성을 추가하세요.

DEF의 경우 `value`를 `player.def`로 설정하고 `onChange`를 `setPlayer`를 `player`를 펼쳐서 호출하며 `def`를 `Number(e.target.value)`로 설정하는 화살표 함수로 만드세요.

PHY의 경우 `value`를 `player.phy`로 설정하고 `onChange`를 `setPlayer`를 `player`를 펼쳐서 호출하며 `phy`를 `Number(e.target.value)`로 설정하는 화살표 함수로 만드세요.

이 단계로 모든 입력이 상태에 연결되어 각 필드마다 실시간 미리보기가 업데이트됩니다!

# --hints--

DEF `input`는 플레이어의 `value` 상태와 같은 `def`를 가져야 합니다.
...
DEF `input`는 `onChange`를 `setPlayer`를 펼쳐 호출하며 `player`를 `def`로 설정하는 `Number(e.target.value)` 핸들러를 가져야 합니다.
...
PHY `input`는 플레이어의 `value` 상태와 같은 `phy`를 가져야 합니다.
...
PHY `input`는 `onChange`를 `setPlayer`를 펼쳐 호출하며 `player`를 `phy`로 설정하는 `Number(e.target.value)` 핸들러를 가져야 합니다.
