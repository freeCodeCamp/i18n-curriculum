...
Crie uma função chamada `setPlayButtonAccessibleText`. Dentro da função, defina o atributo `aria-label` do botão de reprodução para `Play` seguido de um espaço e do título da música atual. Se `userData.currentSong` for `null`, defina-o como `Play`.

# --hints--

Você deve ter uma função chamada `setPlayButtonAccessibleText`.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

Sua função `setPlayButtonAccessibleText` deve definir o atributo `aria-label` do botão de reprodução para `Play` seguido de um espaço e do título da música atual.
...
```
 ...
 Quando `userData.currentSong` for `null`, `setPlayButtonAccessibleText` deve definir o atributo `aria-label` do botão de reprodução para `Play`.
