...
アニメーションシーケンスが完了するまでの時間を設定するには、`animation-duration` プロパティを使用します。 時間は秒 (`s`) またはミリ秒 (`ms`) のいずれかで指定する必要があります。 `.wheel` セレクターの `animation-duration` プロパティを `10s` に設定してください。

# --hints--

`.wheel` セレクターの `animation-name` プロパティを `wheel` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.wheel')?.animationName === 'wheel');
```

`.wheel` セレクターの `animation-duration` プロパティを `10s` に設定する必要があります。
