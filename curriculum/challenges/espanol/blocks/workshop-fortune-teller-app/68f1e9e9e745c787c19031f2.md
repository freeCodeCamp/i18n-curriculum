...
Dentro de la declaración `if`, obtén el elemento `cardTitle` y asigna su `textContent` a `foundCard.name`. Haz lo mismo para el elemento `description` asignando su `textContent` a `foundCard.desc`.

# --hints--

Debes asignar el `cardTitle` `textContent` a `foundCard.name`. No olvides usar la palabra clave `this`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

Debes asignar el `description` `textContent` a `foundCard.desc`. No olvides usar la palabra clave `this`.
