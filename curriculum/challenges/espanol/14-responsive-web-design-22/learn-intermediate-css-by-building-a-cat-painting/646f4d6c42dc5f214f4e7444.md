...
Tu selector `.cat-whisker-right-top` debe tener una propiedad `position` establecido en `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.position === 'absolute')
```

Tu selector `.cat-whisker-right-top` debe tener una propiedad `top` establecido en `120px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.top === '120px')
```

Tu selector `.cat-whisker-right-top` debe tener una propiedad `left` establecido en `109px`.
