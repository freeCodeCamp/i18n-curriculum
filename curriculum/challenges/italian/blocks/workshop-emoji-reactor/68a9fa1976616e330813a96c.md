...
Ora fai un po' di spazio, rimuovi tutte le variabili dei pulsanti e il `eventListener` che hai creato per ciascuno.

# --hints--

La variabile `happyBtn` non dovrebbe esistere.
...
La variabile `confusedBtn` non dovrebbe esistere.
...
La variabile `sadBtn` non dovrebbe esistere.
...
La variabile `lovingBtn` non dovrebbe esistere.
