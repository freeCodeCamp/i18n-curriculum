...
Елемент `textarea` працює як елемент `input` типу `text`, але має додаткову перевагу — може приймати багаторядковий текст, а також ініціальне число рядків і стовпців тексту.

Користувачі зможуть зареєструватися з біографією. Додайте `label` з текстом `Provide a bio:` в кінці `fieldset`. Додайте елемент `textarea` всередину елемента `label`. Зверніть увагу, що `textarea` потребує кінцевого тегу.

# --hints--

Ви повинні додати елемент `label` в кінці третього `fieldset`, після існуючих елементів `label`.
...
Ви повинні вкласти елемент `textarea` всередину `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea'));
```

Ви повинні надати елементу `textarea` відкриваючий і кінцевий теги.
