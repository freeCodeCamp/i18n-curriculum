...
이제 세 번째 `section`에 스타일을 지정할 차례입니다. `text` 속성에 `text-with-images`과 `class` 값이 있어서 이미 `.text` 규칙에서 스타일을 상속받고 있다는 점을 참고하세요.

`.text-with-images` 선택자를 만들고 `display` 속성을 `grid`로 설정하세요.

# --hints--

`.text-with-images` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

`.text-with-images` 선택자에 `display` 속성이 `grid`로 설정되어야 합니다.
