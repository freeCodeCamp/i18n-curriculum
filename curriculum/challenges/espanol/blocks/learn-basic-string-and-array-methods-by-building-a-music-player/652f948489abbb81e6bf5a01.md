...
En este proyecto aprenderás métodos básicos de cadenas y arreglos construyendo una aplicación de reproductor de música. Podrás reproducir, pausar, saltar y mezclar canciones.

El HTML y CSS de este proyecto te han sido proporcionados, para que puedas centrarte en el JavaScript.

Comienza accediendo a los elementos `#playlist-songs`, `#play` y `#pause` con el método `getElementById()`. Asígnales a las variables `playlistSongs`, `playButton` y `pauseButton`, respectivamente.


# --hints--

Debes tener una variable llamada `playlistSongs`. 

```js
assert.isDefined(playlistSongs);
```

Debes usar `document.getElementById()` para obtener el elemento `#playlist-songs`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

Debes asignar el elemento `#playlist-songs` a la variable `playlistSongs`. 

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

Debes tener una variable llamada `playButton`. 

```js
assert.isDefined(playButton);
```

Debes usar `document.getElementById()` para obtener el elemento `#play`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

Debes asignar el elemento `#play` a la variable `playButton`. 

```js
assert.equal(playButton, document.getElementById('play'));
```

Debes tener una variable llamada `pauseButton`. 

```js
assert.isDefined(pauseButton);
```

Debes usar `document.getElementById()` para obtener el elemento `#pause`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

Debes asignar el elemento `#pause` a la variable `pauseButton`. 
