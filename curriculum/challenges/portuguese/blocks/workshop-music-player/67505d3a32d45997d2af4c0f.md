...
Crie uma função chamada `playPreviousSong`.Dentro da sua nova função, use o valor retornado pela função `getPreviousSong` para encontrar a música a ser reproduzida.Quando `getPreviousSong()` estiver `undefined`, toque a primeira música da sua playlist.Caso contrário, toque a música antes da música atual que está tocando.

# --hints--

Você deve ter uma função chamada `playPreviousSong`.

```js
assert.isFunction(playPreviousSong);
```

A função `playPreviousSong` deve tocar a música anterior à música atual que está tocando na playlist. 
...
Quando `getPreviousSong()` está `undefined`, a função `playPreviousSong` deve tocar a primeira música na playlist. 
