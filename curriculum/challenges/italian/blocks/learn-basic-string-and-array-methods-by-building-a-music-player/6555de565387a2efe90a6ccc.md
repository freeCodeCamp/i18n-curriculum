...
La funzione `setPlayButtonAccessibleText` imposterà l'attributo `aria-label` sulla canzone corrente, o sulla prima canzone nella playlist. E se la playlist è vuota, imposta `aria-label` su `"Play"`.

Usa il metodo `setAttribute` sull'elemento `playButton` per impostare un attributo chiamato `"aria-label"`. Usando un ternario, imposta il valore dell'attributo su `Play ${song.title}` o `"Play"` se `song?.title` non è disponibile.

Non dimenticare che qui serve l'interpolazione di stringa, quindi devi usare i backtick.

# --hints--

Non dovresti modificare la funzione `setPlayButtonAccessibleText` esistente e il suo contenuto.
...
Dovresti usare il metodo `setAttribute()` su `playButton`.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` dovrebbe essere il primo valore del tuo metodo `setAttribute()`.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

Il tuo metodo `setAttribute` dovrebbe avere ``song?.title ? `Play ${song.title}` : "Play"`` come secondo argomento.
