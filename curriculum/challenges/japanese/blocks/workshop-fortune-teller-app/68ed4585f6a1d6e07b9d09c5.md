...
`fetchCardsData` メソッドの中で、`response` という名前の変数を宣言し、次のURLを使って `fetch` 関数を呼び出して代入してください：`` `${CDN_URL}/card_data.json` ``。

`await` の呼び出しの前に `fetch` キーワードを追加し、全体の処理を `try...catch` ブロックで囲んでください。

# --hints--

`response` メソッドの中で `fetchCardsData` 変数を宣言する必要があります。
...
`response` 変数は、``await fetch`${CDN_URL}/card_data.json` `` `await fetch` を呼び出して代入してください。
...
`fetch` の呼び出しは `try...catch` ブロックで囲み、`error` を `catch` パラメータとして使ってください。
