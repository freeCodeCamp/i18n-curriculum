...
在你的第一個 `p` 元素中，添加一個 `span` 元素。 爲 `span` 賦予 `product-count` 類和 `id` 爲 `product-count-for-id${id}`。 然後，在 span 之後，將 `name` 變量的文本賦予 `p` 元素。
...
你的 `span` 元素應該具有 `product-count` 的 `class`。
...
你的 `span` 元素應該具有 `product-count-for-id${id}` 的 `id`。
...
你的第一個 `p` 元素應該包含 `name` 變量的文本。 這應該在 span 之外。
