...
Додайте третій елемент `p` в кінці елемента `.text` та надайте йому такий текст:
...
Ви повинні створити третій елемент `p` в елементі `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 3);
```

Ваш третій елемент `p` повинен мати наданий текст.
