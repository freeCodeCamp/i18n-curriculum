...
Verifica se il predicato (secondo argomento) è <dfn>truthy</dfn> su tutti gli elementi di una collezione (primo argomento).

In altre parole, ti viene dato un array di oggetti. Il predicato `pre` sarà una proprietà dell'oggetto e devi restituire `true` se il suo valore è `truthy`. Altrimenti, restituisci `false`.

In JavaScript, i valori `truthy` sono valori che si traducono in `true` quando valutati in un contesto booleano.

Ricorda, puoi consultare le proprietà degli oggetti sia tramite notazione a punto sia tramite notazione `[]`.
