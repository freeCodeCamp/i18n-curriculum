...
Für den öffnenden `a`-Tag setzen Sie das `target`-Attribut auf `"_blank"`. Dann setzen Sie das `href`-Attribut auf `${forumTopicUrl}${slug}/${id}`.

Und mit diesen Änderungen ist Ihr freeCodeCamp-Forum-Leaderboard-Projekt jetzt abgeschlossen!

# --hints--

Ihr öffnender Anker-Tag sollte ein `target`-Attribut haben, das auf `"_blank"` gesetzt ist.
...
Ihr öffnender Anker-Tag sollte ein `href`-Attribut haben, das auf `${forumTopicUrl}${slug}/${id}` gesetzt ist.
