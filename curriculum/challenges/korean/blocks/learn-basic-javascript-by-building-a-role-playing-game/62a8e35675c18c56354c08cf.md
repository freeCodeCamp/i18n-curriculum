...
이제 `attack` 함수를 만들 수 있습니다. 먼저 `text` 메시지를 `"The <monster name> attacks."`로 업데이트하고, `<monster name>`를 몬스터 이름으로 바꾸세요. 이때 연결 연산자를 사용할 수 있다는 점을 기억하세요.

# --hints--

점 표기법을 사용해 `innerText`의 `text` 속성에 접근하세요.

```js
assert.match(attack.toString(), /text\.innerText/);
```

문자열 `"The "`를 `innerText`의 `text` 속성에 할당하세요.

```js
assert.match(attack.toString(), /text\.innerText\s*=\s*('|")The \1/);
```

연결 연산자를 사용해 `monsters[fighting].name` 값을 문자열 `"The "`에 더하세요.

```js
assert.match(attack.toString(), /text\.innerText\s*=\s*('|")The \1\s*\+\s*monsters\s*\[\s*fighting\s*\]\s*\.name/);
```

연결 연산자를 사용해 문자열 `" attacks."`를 `monsters[fighting].name` 문자열에 더하세요.
