...
أنشئ `div` جديدًا أسفل العنصر `.large` وأعطه خاصية `class` بقيمة `calories-info`.
...
يجب أن يحتوي `div` الجديد على خاصية `class` بقيمة `calories-info`.

```js
assert(document.querySelector('.label')?.lastElementChild?.classList?.contains('calories-info'));
```

يجب أن يأتي `div` الجديد بعد العنصر `.large`.
