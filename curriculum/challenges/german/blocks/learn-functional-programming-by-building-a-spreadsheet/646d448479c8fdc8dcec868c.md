...
Deklarieren Sie eine `functionCall`-Variable und weisen Sie ihr diesen regulären Ausdruck zu: `/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

Dieser Ausdruck sucht nach Funktionsaufrufen wie `sum(1, 4)`.

# --hints--

Sie sollten eine `functionCall`-Variable deklarieren.
...
Sie sollten `const` verwenden, um Ihre `functionCall`-Variable zu deklarieren.
...
Sie sollten `functionCall` den bereitgestellten regulären Ausdruck zuweisen.
