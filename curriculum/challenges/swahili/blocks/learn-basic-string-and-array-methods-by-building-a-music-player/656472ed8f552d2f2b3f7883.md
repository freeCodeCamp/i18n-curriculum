...
Ongeza kipengele cha `else` kushughulikia muda wa sasa wa kuchezwa kwa wimbo. Hii inakuwezesha kuendelea na wimbo uliopo mahali uliposimama.
...
Haupaswi kubadilisha tamko la `if` lililopo na yaliyomo yake.
...
Unapaswa kuongeza kipengele cha `else` kwenye kipengele cha `if` kilichopo.
...
Unapaswa kuweka `audio.currentTime` kuwa `userData?.songCurrentTime` ndani ya kipengele cha `else` cha tamko lako la `if`.
