...
Con `type="password"` puedes usar el atributo `pattern` para definir una expresión regular que la contraseña debe coincidir para ser considerada válida.

Añade un atributo `pattern` al elemento de `input` de la contraseña para requerir la coincidencia con: `[a-z0-5]{8,}`
...
Debes dar al elemento `input` de la contraseña un atributo `pattern`.
...
Debes eliminar el atributo `minlength` del elemento de `input` de la contraseña.
