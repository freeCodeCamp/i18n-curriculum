...
Для другого `fieldset` ви хочете, щоб текст `input` і `label` з’являвся в одному рядку.

Почніть з того, що надайте елементам `input` у другому `fieldset` клас `inline`.

# --hints--

Ви повинні надати першому `input` клас `inline`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

Ви повинні надати другому `input` клас `inline`.
