...
`isSpam` キーワードとアロー構文を使って `const` 関数を作成してください。この関数は単一のパラメータ `msg` を受け取り、今は暗黙的に `false` を返すようにしてください。

# --hints--

`const` を使って `isSpam` 変数を宣言してください。

```js
assert.match(code, /const\s+isSpam\s*=/);
```

アロー構文を使って `isSpam` に関数を代入してください。

```js
assert.match(code, /const\s+isSpam\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`isSpam` 関数は単一のパラメータ `msg` を持つべきです。

```js
assert.match(code, /const\s+isSpam\s*=\s*(\(\s*msg\s*\)|msg)/);
```

`isSpam` 関数は暗黙的に `false` を返すべきです。
