...
Erstellen Sie ein `button`-Element mit der Klasse `playlist-song-info`. Fügen Sie innerhalb des `button` ein `span`-Element mit der Klasse `playlist-song-title` hinzu, und interpolieren Sie dann `song.title` als Text.

# --hints--

Sie sollten ein `button` mit der Klasse `playlist-song-info` erstellen.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

Sie sollten ein `span`-Element innerhalb Ihres `button`-Elements erstellen.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

Ihr `span`-Element sollte die Klasse `playlist-song-title` haben.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

Sie sollten `song.title` als Inhalt Ihres `span`-Elements interpolieren.
