...
Em seguida, você precisa verificar se o valor retornado pela função `parseInt()` é um número ou não.
...
Além disso, como mencionamos no passo 1, estamos considerando apenas números positivos, então devemos adicionar uma terceira condição à declaração `if` para verificar se o número é menor que 0 (ou seja, números negativos).
