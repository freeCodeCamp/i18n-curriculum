...
仍在 `elif` 子句中，将 `expenses_from_category` 迭代器传递给 `print_expenses` 调用。

# --hints--

你应该在 `elif` 子句中调用 `print_expenses()` 并将 `expenses_from_category` 作为参数传递。
...
你的 `print_expenses()` 调用应该在 `expenses_from_category` 变量之后。
