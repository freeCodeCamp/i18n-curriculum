...
前のレッスンでは、フィルターされた結果の新しい配列を返すために使われる `filter()` メソッドの使い方を学びました。
...
2つのパラメータを持つ `getBooksByAuthor` 関数を作成してください。1つは本のオブジェクトの配列、もう1つは著者の文字列です。

この関数は、特定の著者の本を含む配列を返す必要があります。

# --hints--

`getBooksByAuthor` 関数を作成してください。

```js
assert.isFunction(getBooksByAuthor);
```

`getBooksByAuthor` 関数は2つのパラメータを持つべきです。

```js
assert.lengthOf(getBooksByAuthor, 2);
```

`getBooksByAuthor` 関数は配列を返すべきです。

```js
assert.isArray(getBooksByAuthor(library, ''));
```

`getBooksByAuthor` 関数はどの著者に対しても正しい数の本を返すべきです。
