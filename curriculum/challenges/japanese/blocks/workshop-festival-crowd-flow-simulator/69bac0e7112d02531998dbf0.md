...
`handleGateAtTick` 関数の最後のタスクは、現在のティックのオーバーフローを処理することです。

もしオーバーフローが存在する場合（`result.overflow > 0`）、次のことをしてください：

- 次のメッセージをログに記録します `"Overflow of " + result.overflow + " attendees. Rerouting..."`。
- `rerouteOverflow()` 関数を `gates`、`gate`、`tickIndex`、および `result.overflow` で呼び出してオーバーフローをルーティングし直します。

# --hints--

もしオーバーフローが存在する場合（`result.overflow > 0`）、まずメッセージ `"Overflow of " + result.overflow + " attendees. Rerouting..."` をログに記録してください。
...
もしオーバーフローが存在する場合（`result.overflow > 0`）、`rerouteOverflow()` を `gates`、`gate`、`tickIndex`、および `result.overflow` で呼び出してオーバーフローをルーティングし直してください。
