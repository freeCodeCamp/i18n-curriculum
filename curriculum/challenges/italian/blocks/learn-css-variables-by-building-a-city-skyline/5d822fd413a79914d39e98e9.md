...
Per iniziare a ottimizzare il tuo codice, sposta prima le proprietà e i valori `position` e `top` da `.foreground-buildings` a `.background-buildings`, poi modifica il selettore `.background-buildings` esistente in modo che includa anche `.foreground-buildings`, così gli stili verranno applicati a entrambi. Puoi usare una virgola (`,`) per separare i selettori in questo modo: `selector1, selector2`.
...
Dovresti aggiungere la proprietà `position` di `absolute` a `.background-buildings, .foreground-buildings`.
...
Dovresti aggiungere la proprietà `top` di `0` a `.background-buildings, .foreground-buildings`.
...
Dovresti usare una virgola per usare sia i selettori `.foreground-buildings` che `.background-buildings` nella stessa dichiarazione di stile.
...
Non dovresti avere un selettore `.background-buildings` separato.
