...
就像一般函式（程式）一樣，箭頭函式也可以傳回返回值。 

以下是一個箭頭函式傳回兩個數字相乘結果的範例：
...
創建一個名為 `addTwoNumbers` 的新變數並指定一個箭頭函式。這個箭頭函式應該接受兩個名為 `num1` 和 `num2` 的參數。在函式主體內，傳回將 `num1` 和 `num2` 相加的運算式。

在 `addTwoNumbers` 函式（程式）下方，添加一個主控台述語。在該主控台述語內，呼叫 `addTwoNumbers` 函式（程式），並傳入數字 `3` 和 `4` 作為引數。
...
你的 `addTwoNumbers` 函式（程式）應該使用箭頭函式語法。

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

你的 `addTwoNumbers` 函式應該有兩個名為 `num1` 和 `num2` 的參數。

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

你的 `addTwoNumbers` 函式（程式）應該傳回將 `num1` 和 `num2` 相加的表達式、運算式。

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

你應該有一個 `console.log(addTwoNumbers(3, 4));` 述語。
