...
Crie um seletor `.lists` e defina a propriedade `list-style-type` como `none`. Isto eliminará os pontos da lista de itens.

# --hints--

Você deve ter um seletor `.lists`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists'));
```

O seletor `.lists` deve ter a propriedade `list-style-type` definida como `none`.
