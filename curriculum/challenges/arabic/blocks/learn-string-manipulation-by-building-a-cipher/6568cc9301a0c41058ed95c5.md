...
الآن، استبدل `new_char` بـ `encrypted_text`. كما عدّل استدعاء `print()` إلى `print('char:', char, 'encrypted text:', encrypted_text)` ليعكس هذا التغيير.

# --hints--

يجب أن تستبدل `new_char` بـ `encrypted_text` داخل حلقة `for` الخاصة بك.
...
يجب أن تحول استدعاء `print()` إلى `print('char:', char, 'encrypted text:', encrypted_text)` داخل حلقة `for` الخاصة بك.
