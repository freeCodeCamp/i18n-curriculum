...
この2語のフレーズは、提案に賛成するときに使います。最初の単語の最初の文字は大文字です。

# --explanation--

`Good idea` は提案に賛成や承認を表すために使います。議論の中で役立つ提案を認めるときによく使われます。例えば：

- **プロジェクトマネージャー:** `Let's set up a shared document for tracking progress.`

- **従業員:** `Good idea. That will help everyone stay updated.` - この人は、共有ドキュメントがコミュニケーションを改善すると賛成しています。
