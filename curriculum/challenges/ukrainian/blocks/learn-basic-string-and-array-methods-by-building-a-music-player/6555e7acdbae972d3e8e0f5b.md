...
Додайте слухача події `"click"` до елемента `shuffleButton`. Щоб функція запустилася, передайте функцію `shuffle`.

**Примітка**: У цьому конкретному слухачі події функція зворотного виклику не потрібна. Також не потрібно викликати функцію `shuffle`, просто передайте її ідентифікатор.

# --hints--

Ви повинні додати метод `addEventListener()` до `shuffleButton`.
...
Ви повинні передати `shuffle` як друге значення вашого слухача події `"click"`.
