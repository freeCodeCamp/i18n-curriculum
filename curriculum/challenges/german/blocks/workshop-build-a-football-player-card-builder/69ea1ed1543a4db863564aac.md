...
Ändern Sie den Wert des Schlüssels `position` in den abgeleiteten `Position`.

# --hints--

Ihr `PlayerData`-Interface-`position`-Feld sollte den `Position`-Typ anstelle von `string` verwenden.
