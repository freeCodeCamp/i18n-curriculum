...
`else` 함수에 `buyWeapon` 문장을 추가하세요. 그 문장 안에서 `text.innerText`를 `"You do not have enough gold to buy a weapon."`와 같게 설정하세요.

# --hints--

`else` 함수에 `buyWeapon` 문장을 추가해야 합니다.

```js
assert.match(buyWeapon.toString(), /else/);
```

`else` 문장은 `if` 문장 다음에 와야 합니다.
...
`text.innerText`를 `"You do not have enough gold to buy a weapon."`로 설정해야 합니다.

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You do not have enough gold to buy a weapon.\1/);
```

`else` 문장은 `text.innerText`를 `"You do not have enough gold to buy a weapon."`로 설정해야 합니다.
