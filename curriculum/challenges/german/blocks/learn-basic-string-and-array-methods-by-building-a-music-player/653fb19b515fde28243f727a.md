...
Um die Songs in der Benutzeroberfläche (User Interface) anzuzeigen, müssen Sie eine Funktion erstellen.

Verwenden Sie die Syntax der Pfeilfunktion, um eine `renderSongs`-Funktion zu erstellen, die `array` als Parameter entgegennimmt.

# --hints--

Sie sollten `const` verwenden, um eine leere Funktion mit dem Namen `renderSongs` zu erstellen.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs` sollte eine Funktion sein.

```js
assert.isFunction(renderSongs)
```

Ihre `renderSongs`-Funktion sollte die Syntax der Pfeilfunktion verwenden.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

Ihre `renderSongs`-Funktion sollte `array` als Parameter annehmen.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

Ihre `renderSongs`-Funktion sollte leer sein.
