...
Il selettore `#years` dovrebbe avere un proprietà `display` impostata su `flex`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('display') === 'flex');
```

Il selettore `#years` dovrebbe avere una proprietà `justify-content` impostata su `flex-end`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('justify-content') === 'flex-end');
```

Il selettore `#years` dovrebbe avere una proprietà `position` impostata su `sticky`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('position') === 'sticky');
```

Il selettore `#years` dovrebbe avere una proprietà `top` impostata a `0`.
