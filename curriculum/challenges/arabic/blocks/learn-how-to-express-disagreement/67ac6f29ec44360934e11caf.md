---
id: 67ac6f29ec44360934e11caf
title: المهمة 18
challengeType: 22
dashedName: task-18
lang: en-US
---

<!-- (Audio) Bob: A comfortable desk and a chair can make a big difference. -->

# --instructions--

استمع إلى الصوت وأكمل الجملة أدناه.

# --fillInTheBlank--

## --sentence--

`A comfortable BLANK and a BLANK can make a big difference.`

## --blanks--

`desk`

### --feedback--

هذا قطعة أثاث ذات سطح مستوٍ، تُستخدم للعمل أو الدراسة.

---

`chair`

### --feedback--

هذا قطعة أثاث للجلوس، عادةً مع مسند للظهر.

# --explanation--

يذكر Bob كلًا من `desk` و`chair` لأنهما ضروريان لمساحة عمل مريحة وفعالة. يوفر المكتب مساحة للعمل، بينما يدعم الكرسي المريح الوضعية ويمنع الانزعاج أثناء الجلوس لساعات طويلة.

# --scene--

```json
{
  "setup": {
    "background": "company1-boardroom.png",
    "characters": [
      {
        "character": "Bob",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_11-1.mp3",
      "startTime": 1,
      "startTimestamp": 26.86,
      "finishTimestamp": 30.5
    }
  },
  "commands": [
    {
      "character": "Bob",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Bob",
      "startTime": 1,
      "finishTime": 4.64,
      "dialogue": {
        "text": "A comfortable desk and a chair can make a big difference.",
        "align": "center"
      }
    },
    {
      "character": "Bob",
      "opacity": 0,
      "startTime": 5.14
    }
  ]
}
```
