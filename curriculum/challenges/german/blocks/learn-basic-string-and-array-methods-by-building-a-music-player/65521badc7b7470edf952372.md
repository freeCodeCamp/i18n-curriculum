...
Definieren Sie eine `pauseSong`-Funktion mit dem Schlüsselwort `const` und der Syntax einer Pfeilfunktion. Die Funktion soll keine Parameter haben.

# --hints--

Sie sollten `const` verwenden, um eine leere Funktion mit dem Namen `pauseSong` zu erstellen.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` sollte eine Funktion sein.

```js
assert.isFunction(pauseSong)
```

Ihre `pauseSong`-Funktion sollte eine Pfeil-Syntax verwenden.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Ihre `pauseSong`-Funktion sollte leer sein.
