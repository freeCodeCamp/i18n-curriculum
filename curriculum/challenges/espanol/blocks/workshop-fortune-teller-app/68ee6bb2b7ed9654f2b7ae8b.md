...
Agrega otro event listener al objeto global `document` que llame a la función `showFortune`. Dentro del callback, incluye un parámetro `e` de tipo `Event`, y pasa `e` como argumento al método `showFortune`.

`showFortune` manejará los eventos de clic en las cartas, como leer el ID de la carta clickeada desde el DOM y mostrar los detalles de la fortuna correspondiente. Lo crearás en los próximos pasos.

# --hints--

Debes asignar un event listener `click` al objeto global `document`.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Tu event listener debe tener un parámetro callback llamado `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

Tu parámetro callback debe tener el tipo `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

Dentro del callback, debes llamar a la función `showFortune`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

Dentro de la función `showFortune`, debes pasar el argumento `e`.
