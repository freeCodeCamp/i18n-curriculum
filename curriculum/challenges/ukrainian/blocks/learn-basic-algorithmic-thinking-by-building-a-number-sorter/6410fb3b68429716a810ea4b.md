...
Сортування вибором базується на відстеженні індексу найменшого значення в масиві. Оголосіть змінну `minIndex` і призначте їй значення `i` — це гарантує, що якщо ваше поточне значення є найменшим, воно буде обміняне саме з собою і не змінить позицію. Вам потрібно буде мати можливість переназначати значення `minIndex` під час ітерування по масиву.

Потім напишіть ще один цикл `for`, використовуючи `j` як ітератор. Цей цикл має починатися з індексу після `i` і ітерувати по залишку масиву.

# --hints--

Ви повинні використовувати `let` для оголошення `minIndex`.
...
Ви повинні призначити `minIndex` значення `i`.
...
У вас має бути вкладений цикл `for`.
...
Ваш цикл `for` має ініціалізувати `j` значенням `i + 1`.
...
Ваш цикл `for` має ітерувати по залишку масиву.
...
Ваш цикл `for` має збільшувати `j`.
