...
Ahora necesitas definir las propiedades que tendrán todos los bugs.

Dentro de la clase `Bug`, declara una propiedad llamada `emoji` con el tipo `T` usando el operador de afirmación de asignación definitiva (`!`). Esto le indica a TypeScript que la propiedad se asignará más adelante.

# --hints--

Tu clase `Bug` debe tener una propiedad llamada `emoji`.
...
Tu propiedad `emoji` debe tener el tipo `T`.
...
Tu propiedad `emoji` debe usar el operador de afirmación de asignación definitiva (`!`).
