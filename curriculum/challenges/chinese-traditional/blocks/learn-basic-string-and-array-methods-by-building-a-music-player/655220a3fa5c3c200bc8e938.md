...
使用 `classList` 和 `remove()` 方法將 `playing` 類別從 `playButton` 中移除，因為此時歌曲將會暫停。

要最終暫停歌曲，請對 `audio` 變數使用 `pause()` 方法。`pause()` 是 Web Audio API 用於暫停音樂檔案的方法。

# --hints--

你不應該修改現有的 `pauseSong` 函式（程式）及其內容。

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

你應該使用 `classList` 屬性和 `remove()` 方法，從 `playButton` 移除類別 `"playing"`。

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

你應該在你的 `audio` 變數上使用 `pause()` 方法。
