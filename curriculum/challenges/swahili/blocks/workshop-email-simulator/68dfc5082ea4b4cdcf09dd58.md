...
Baada ya kupata kielezo halisi, tengeneza taarifa ya if ili kukagua kama `actual_index` ni kidogo kuliko sifuri au ni kubwa au sawa na urefu wa orodha ya `self.emails`. Ikiwa ni hivyo, chapisha ujumbe `Invalid email number.\n` na tumia `return` kutoka njia.
...
Ikiwa kielezo si halali, unapaswa kuchapisha ujumbe `Invalid email number.\n` na tumia `return` kutoka njia.
