...
`parseCatalog` actualmente devuelve un arreglo vacío. Para llenarlo, agrega un ciclo `for` dentro de la función que itere sobre `rawCards`. Dentro del ciclo, llama a `parseCard` en cada elemento y agrega el resultado a `catalog`.

# --hints--

`parseCatalog(rawCatalogCards).length` debería ser `19`.

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

`parseCatalog` debería usar un ciclo `for` para iterar sobre `rawCards`.

```js
assert.match(parseCatalog.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*rawCards\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Debes llamar a `parseCard` dentro del ciclo.
