...
`ul` 요소 안에 여섯 개의 `li` 요소를 만드세요. 각 `h4` 요소에 `class`가 `list-subtitle`로 설정된 `p` 요소와 `li` 요소를 추가하세요.

그다음 `h4`과 `p` 요소에 다음 텍스트 내용을 순서대로 넣으세요. 각 `h4`는 콜론 왼쪽에 있는 내용을, 각 `p`는 오른쪽에 있는 내용을 사용합니다:
...
`ul` 요소에 여섯 개의 `li` 요소가 있어야 합니다.

```js
assert.lengthOf(document.querySelectorAll('.lists li'), 6);
```

새로 만든 각 `li` 요소에는 `h4`와 `p` 요소가 있어야 합니다.
...
첫 번째 `h4`에는 텍스트 `V1 - 2014`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

첫 번째 `p`에는 텍스트 `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`가 있어야 합니다.
...
두 번째 `h4`에는 텍스트 `V2 - 2015`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[1]?.innerText, 'V2 - 2015');
```

두 번째 `p`에는 텍스트 `We added interactive algorithm challenges.`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll('.lists li p')?.[1]?.innerText, 'We added interactive algorithm challenges.');
```

세 번째 `h4`에는 텍스트 `V3 - 2015`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[2]?.innerText, 'V3 - 2015');
```

세 번째 `p`에는 텍스트 `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`가 있어야 합니다.
...
네 번째 `h4`에는 텍스트 `V4 - 2016`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[3]?.innerText, 'V4 - 2016');
```

네 번째 `p`에는 텍스트 `We expanded the curriculum to 3 certifications, including Front-End, Back-End, and Data Visualization. They each had 10 required projects, but only the Front-End section had its own challenges. For the other certs, we were still using external resources like Node School.`가 있어야 합니다.
...
다섯 번째 `h4`에는 텍스트 `V5 - 2017`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[4]?.innerText, 'V5 - 2017');
```

다섯 번째 `p`에는 텍스트 `We added the back-end and data visualization challenges.`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll('.lists li p')?.[4]?.innerText, 'We added the back-end and data visualization challenges.');
```

여섯 번째 `h4`에는 텍스트 `V6 - 2018`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[5]?.innerText, 'V6 - 2018');
```

여섯 번째 `p`에는 텍스트 `We launched 6 new certifications to replace our old ones. This was the biggest curriculum improvement to date.`가 있어야 합니다.
...
여섯 개의 `h4` 요소 각각에 `list-subtitle` 클래스를 지정하세요.
