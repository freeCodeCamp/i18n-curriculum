...
`buyHealth`、`buyWeapon`、`goTown` という名前の空の関数を3つ作成してください。

# --hints--

`buyHealth` は `function` キーワードで宣言してください。

```js
assert.match(code, /function\s+buyHealth/);
```

`buyWeapon` は `function` キーワードで宣言してください。

```js
assert.match(code, /function\s+buyWeapon/);
```

`goTown` は `function` キーワードで宣言してください。
