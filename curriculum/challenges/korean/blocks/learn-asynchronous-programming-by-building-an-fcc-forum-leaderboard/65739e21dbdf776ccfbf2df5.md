...
`fetch` 호출에서 오류가 발생하면 `catch` 블록이 이를 처리할 것입니다.

`catch` 블록 안에 `console.log`를 추가하여 `err` 매개변수를 기록하세요.

또한 `console.log(data);` 호출에서 반환되는 값을 이해했으니 `try` 블록에서 `fetch`를 제거하세요.

# --hints--

`console.log`를 사용하여 `err` 변수를 기록해야 합니다.

```js
assert.match(code, /\s*console\.log\(\s*err\s*\)/);
```

`console.log`는 `catch` 블록 안에 있어야 합니다.

```js
assert.match(code, /\s*try\s*{[^}]*\s*}\s*catch\s*\(\s*err\s*\)\s*{[^}]*\s*console\.log\(\s*err\s*\)[^}]*}/);
```

`console.log(data);`를 `try` 블록에서 제거해야 합니다.
