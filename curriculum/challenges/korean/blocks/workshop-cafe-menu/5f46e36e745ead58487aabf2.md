...
이제 상단 여백이 잘 보입니다. 메뉴 하단의 주소 아래 공간이 메뉴 상단과 `h1` 요소 위 공간보다 약간 더 큽니다.

주소 `p` 요소 아래 기본 여백 공간을 줄이려면 `address`라는 클래스 선택자를 만들고 `5px` 속성에 `margin-bottom` 값을 사용하세요.
...
`.address` 선택자에 `margin-bottom` 속성을 `5px`으로 설정해야 합니다.
