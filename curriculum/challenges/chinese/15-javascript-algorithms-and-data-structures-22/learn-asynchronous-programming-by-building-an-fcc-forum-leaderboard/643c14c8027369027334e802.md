...
你想要将响应主体作为 JSON 对象获取。 你的 `res` 变量的 `.json()` 方法返回一个 Promise，这意味着你需要 `await` 它。

创建一个名为 `data` 的常量，并将其赋值为 `await res.json()` 的结果。

# --hints--

你应该有一个名为 `data` 的 `const` 变量。

```js
assert.match(code, /const\s+data/);
```

你应该将 `await res.json()` 分配给 `data` 变量。
