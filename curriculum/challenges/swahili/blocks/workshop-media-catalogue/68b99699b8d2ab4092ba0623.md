...
Baada ya usemi wa `if` katika njia yako ya `__str__`, tengeneza vigezo viwili vinavyoitwa `movies` na `series` na weka thamani yake wito wa njia ya `get_movies` na wito wa njia ya `get_tv_series`, mtawalia.

# --hints--

Unapaswa kuwa na kigezo kinachoitwa `movies` katika njia yako ya `__str__`.
...
Unaweka thamani ya `self.get_movies()` kwa kigezo chako cha `movies`.
...
Unapaswa kuwa na kigezo kinachoitwa `series` katika njia yako ya `__str__`.
...
Unaweka thamani ya `self.get_tv_series()` kwa kigezo chako cha `series`.
