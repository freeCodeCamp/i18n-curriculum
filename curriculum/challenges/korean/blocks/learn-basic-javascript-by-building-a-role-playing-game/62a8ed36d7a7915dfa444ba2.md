...
`healthText.innerText`와 `monsterHealthText.innerText`을 `health`와 `monsterHealth`과 같게 업데이트하세요.

# --hints--

`healthText.innerText`를 업데이트해야 합니다.

```js
assert.match(attack.toString(), /healthText\.innerText/);
```

`healthText.innerText`를 `health`로 설정해야 합니다.

```js
assert.match(attack.toString(), /healthText\.innerText\s*=\s*health/);
```

`monsterHealthText.innerText`를 업데이트해야 합니다.

```js
assert.match(attack.toString(), /monsterHealthText\.innerText/);
```

`monsterHealthText.innerText`를 `monsterHealth`로 설정해야 합니다.
