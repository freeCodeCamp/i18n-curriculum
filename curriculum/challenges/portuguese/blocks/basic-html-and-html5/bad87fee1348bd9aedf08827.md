...
Listas não ordenadas começam com um elemento `<ul>` de abertura, seguido por qualquer número de elementos `<li>`.Finalmente, listas não ordenadas são encerradas com um `</ul>`.
