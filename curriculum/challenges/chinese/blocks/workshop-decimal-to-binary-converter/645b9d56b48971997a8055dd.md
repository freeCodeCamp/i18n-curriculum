...
在 `if` 语句的主体内，将 `result` 的 `innerText` 属性设置为等于字符串 `"0"`。 然后，使用早期的 `return` 语句尽早退出该函数。

# --hints--

你应该将 `result` 的 `innerText` 属性设置为等于字符串 `"0"`。

```js
assert.match(String(decimalToBinary), /if\s*\(\s*input\s*===?\s*0\s*\)\s*\{\s*result\.innerText\s*=\s*('|"|`)\s*0\s*\1/);
```

在设置 `result` 元素的 `innerText` 之后，你应该使用早期的 `return` 语句来尽早退出该函数。
