...
Kombinieren Sie nun alle Zeilen zu einem CSV-String, den Sie exportieren oder teilen können.

Deklarieren Sie nach der ersten Schleife `let csv = header`. Fügen Sie eine weitere `for`-Schleife hinzu, um jede Zeile mit einem Zeilenumbruch an `csv` anzuhängen: `csv = csv + "\n" + rows[i]`. Geben Sie dann `csv` zurück. Protokollieren Sie anschließend unterhalb der Funktion `exportToCSV(catalog)`, um die formatierte Ausgabe zu überprüfen.

# --hints--

`exportToCSV` sollte einen String zurückgeben.

```js
assert.isString(exportToCSV(catalog));
```

Die erste Zeile des zurückgegebenen Strings sollte `"Title,Author,Year,Location"` sein.

```js
assert.strictEqual(exportToCSV(catalog).split("\n")[0], "Title,Author,Year,Location");
```

Die Ausgabe sollte `"King, Stephen"` enthalten (in Anführungszeichen, um das Komma zu handhaben).

```js
assert.include(exportToCSV(catalog), '"King, Stephen"');
```

Sie sollten `exportToCSV(catalog)` in der Konsole protokollieren.
