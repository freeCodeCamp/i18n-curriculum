...
Su `allSongs` array debería tener dos valores.

```js
assert.equal(allSongs.length, 2);
```

Su `allSongs` array debería tener un objeto como su segundo valor.

```js
assert.isObject(allSongs[1]);
```

El segundo objeto en su `allSongs` array debería tener una propiedad `id` establecida al número `1`.

```js
assert.equal(allSongs[1].id, 1);
```

El segundo objeto en su `allSongs` array debería tener una propiedad `title` establecida a la cadena `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

El segundo objeto en su `allSongs` array debería tener una propiedad `artist` establecida a la cadena `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

El segundo objeto en su `allSongs` array debería tener una propiedad `duration` establecida a la cadena `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

El segundo objeto en su `allSongs` array debería tener una propiedad `src` establecida a la cadena `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
