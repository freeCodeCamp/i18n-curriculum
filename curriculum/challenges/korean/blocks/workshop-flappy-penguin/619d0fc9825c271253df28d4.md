...
`.blush` 요소를 선택하고 `width`의 `15%`, `height`의 `10%`, `background-color`의 `pink`를 지정하세요.
...
`.blush`에 `width`의 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾을 수 있습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.width, '15%');
```

`.blush`에 `height`의 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾을 수 있습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.height, '10%');
```

`.blush`에 `background-color`의 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾을 수 있습니다.
