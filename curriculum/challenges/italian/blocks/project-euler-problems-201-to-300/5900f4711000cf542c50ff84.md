---
id: 5900f4711000cf542c50ff84
title: 'Problema 261: somme quadrate pivotali'
challengeType: 1
forumTopicId: 301910
dashedName: problem-261-pivotal-square-sums
---

# --description--

Chiamiamo un numero intero positivo $k$ un pivot quadrato, se esiste una coppia di numeri interi $m > 0$ e $n ≥ k$, tale che la somma dei ($m + 1$) quadrati consecutivi fino a $k$ sia uguale alla somma dei $m$ quadrati consecutivi da ($n + 1$) in poi:

$${(k - m)}^2 + \ldots + k^2 = {(n + 1)}^2 + \ldots + {(n + m)}^2$$

Alcuni piccoli pivot quadrati sono

$$\begin{align}
  & \mathbf{4}: 3^2 + \mathbf{4}^2 = 5^2 \\\\
  & \mathbf{21}: {20}^2 + \mathbf{21}^2 = {29}^2 \\\\
  & \mathbf{24}: {21}^2 + {22}^2 + {23}^2 + \mathbf{24}^2 = {25}^2 + {26}^2 + {27}^2 \\\\
  & \mathbf{110}: {108}^2 + {109}^2 + \mathbf{110}^2 = {133}^2 + {134}^2 \\\\
\end{align}$$

Trova la somma di tutti i pivot quadrati distinti $≤ {10}^{10}$.

# --hints--

`pivotalSquareSums()` dovrebbe restituire `238890850232021`.

```js
assert.strictEqual(pivotalSquareSums(), 238890850232021);
```

# --seed--

## --seed-contents--

```js
function pivotalSquareSums() {

  return true;
}

pivotalSquareSums();
```

# --solutions--

```js
// solution required
```
