...
Agora é hora de aplicar o que você aprendeu sobre arrow functions no seu projeto de player de música. 

Comece removendo as funções `printGreeting`, `printMessage` e `addTwoNumbers`. Além disso, remova todas as declarações de console e chamadas de função. 

# --hints--

Você não deve ter uma função `printGreeting`.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

Você não deve ter uma função `printMessage`.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

Você não deve ter uma função `addTwoNumbers`.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

Você não deve ter uma chamada de função `printGreeting();`.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

Você não deve ter uma chamada de função `printMessage('freeCodeCamp');`.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

Você não deve ter uma instrução de console `console.log(addTwoNumbers(3,4));`.
