...
Füge eine Anfangsgröße von `3` Reihen und `30` Spalten hinzu.

# --hints--

Du solltest der `textarea` ein `rows`-Attribut mit dem Wert `3` zuweisen.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

Du solltest der `textarea` ein `cols`-Attribut mit dem Wert `30` zuweisen.
