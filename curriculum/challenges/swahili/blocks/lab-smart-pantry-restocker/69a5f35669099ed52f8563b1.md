...
2. Unapaswa kutekeleza kitendakazi `planRestock(pantry, shipment)` kinacholinganishwa pantri ya sasa na mzigo unaokuja na kurudisha safu ya vitendo kwa aina ya `{ type: "restock" | "discard" | "donate", item }`.

3. Unapaswa kutekeleza kitendakazi `groupByZone(actions)` kinachokusanya vitendo katika maeneo ya kuhifadhi kulingana na sifa ya `zone` ya kila kipengee.
