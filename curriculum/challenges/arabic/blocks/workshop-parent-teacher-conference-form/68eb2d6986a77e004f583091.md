...
حان الوقت الآن لتخصيص أزرار الاختيار. افتراضيًا، أزرار الاختيار تظهر بمظهر المتصفح الأصلي. تريد إزالة هذا المظهر لتتمكن من إنشاء مظهر مخصص.

اختر العنصر الذي يحمل الصنف `contact-method-radio-btn`. عيّن الخاصية `appearance` إلى `none`.

# --hints--

يجب أن تستهدف العنصر الذي يحمل الصنف `contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

عيّن الخاصية `appearance` إلى القيمة `none`.
