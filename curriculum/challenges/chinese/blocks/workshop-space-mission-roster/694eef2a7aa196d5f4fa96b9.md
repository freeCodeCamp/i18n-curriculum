...
调用 `getEVAReadyCrew()` 并传入你的 `updatedSquad` 花名册，将结果保存到一个名为 `EVAReadySquad` 的新建变量中。然后，使用 `for` 循环记录 `EVAReadySquad` 数组中每个宇航员的 `name`。

# --hints--

你应该使用 `updatedSquad` 调用 `getEVAReadyCrew()` 并将结果保存到名为 `EVAReadySquad` 的新建变量中。
...
你应该使用 `for` 循环迭代 `EVAReadySquad` 数组并日志每个宇航员的 `name`。
