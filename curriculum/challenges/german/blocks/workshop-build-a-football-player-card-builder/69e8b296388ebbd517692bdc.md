...
Fügen Sie dem SHO `value` ein `player.sho`-Attribut mit dem Wert `input` hinzu. Setzen Sie dann dessen `onChange`-Attribut auf eine Pfeilfunktion, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `sho` auf `Number(e.target.value)` gesetzt ist.

# --hints--

Ihr SHO `input` sollte ein `value` haben, das dem aktuellen `sho`-Wert des Spielers entspricht.
...
Ihr SHO `input` sollte einen `onChange`-Handler haben, der `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `sho` auf `Number(e.target.value)` gesetzt ist.
