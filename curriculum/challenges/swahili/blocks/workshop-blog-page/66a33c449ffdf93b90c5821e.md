...
Ndani ya sehemu ya mawasiliano, utataka kuonyesha taarifa za mawasiliano za mwandishi wa blogu. Utatumia kipengele cha <dfn>address</dfn> kwa hili.

Kipengele cha `address` kinatumika kuwakilisha taarifa za mawasiliano za mtu au shirika.

Hapa kuna mfano unaotumia kipengele cha `address` kwa anwani ya kimwili. Kipengele cha `br` kinatumika hapa kuunda mapumziko ya mstari kati ya maandishi.
...
Chini ya kipengele chako cha `h2`, ongeza kipengele cha `address`.

# --hints--

Msimbo wako unapaswa kuwa na lebo ya ufunguzi ya `address`.

```js
assert.match(code, /<address>/i);
```

Msimbo wako unapaswa kuwa na lebo ya kufunga ya `address`.

```js
assert.match(code, /<\/address>/i);
```

Kipengele chako cha `address` kinapaswa kuwa ndani ya kipengele chako cha `section`.
