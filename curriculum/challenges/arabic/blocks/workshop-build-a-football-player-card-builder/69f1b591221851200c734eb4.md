...
انتقل إلى العنصر الثاني `div`، وأضف `label` بـ `className` تساوي `label`، و`htmlFor` تساوي `overallRating`، ونص `Overall`، و`input` بـ `id` يساوي `overallRating`، و`className` تساوي `input`، ونوع `number`.

# --hints--

يجب أن يحتوي ثاني `form-group` في الصف على `label` بـ `className` تساوي `label` و`htmlFor` تساوي `overallRating`.
...
يجب أن يحتوي `label` الخاص بتقييمك العام على النص `Overall`.
...
يجب أن يحتوي ثاني `form-group` في الصف على `input` مع `id` يساوي `overallRating` و`type` يساوي `number`.
