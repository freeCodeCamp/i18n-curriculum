...
title: Кола заданого радіуса через дві точки
challengeType: 1
forumTopicId: 302231
...
Дано дві точки на площині та радіус. Зазвичай можна побудувати два кола заданого радіуса, що проходять через ці точки.

**Винятки:**

<ul>
  <li>Радіус нуль слід вважати таким, що ніколи не описує кола (окрім випадку, коли точки співпадають).</li>
  <li>Якщо точки співпадають, то можна побудувати нескінченну кількість кіл, які проходять через цю точку, якщо тільки радіус не дорівнює нулю, тоді кола зводяться до точки.</li>
  <li>Якщо точки утворюють діаметр, поверніть одне коло.</li>
  <li>Якщо точки занадто далеко одна від одної, кола побудувати неможливо.</li>
</ul>

# --instructions--

Реалізуйте функцію, яка приймає дві точки та радіус і повертає два кола, що проходять через ці точки. Для кожного кола надайте координати центру, округлені до чотирьох десяткових знаків. Поверніть кожну координату у вигляді масиву, а координати — у вигляді масиву масивів.

**Для крайніх випадків поверніть наступне:**

<ul>
  <li>Якщо точки лежать на діаметрі, поверніть одну точку. Якщо радіус також дорівнює нулю, поверніть <code>"Radius Zero"</code>.</li>
  <li>Якщо точки співпадають, поверніть <code>"Coincident point. Infinite solutions"</code>.</li>
  <li>Якщо точки розташовані далі, ніж діаметр, поверніть <code>"No intersection. Points further apart than circle diameter"</code>.</li>
</ul>
