...
Надайте новим `div` елементам ці `width` та `height` значення: `3%` і `10%` для `.bb4a`, `80%` і `5%` для `.bb4b`, а також `100%` і `85%` для `.bb4c`.

# --hints--

Ви маєте надати `.bb4a` `width` зі значенням `3%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.width, "3%");
```

Ви маєте надати `.bb4a` `height` зі значенням `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.height, "10%");
```

Ви маєте надати `.bb4b` `width` зі значенням `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.width, "80%");
```

Ви маєте надати `.bb4b` `height` зі значенням `5%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.height, "5%");
```

Ви маєте надати `.bb4c` `width` зі значенням `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4c")?.width, "100%");
```

Ви маєте надати `.bb4c` `height` зі значенням `85%`.
