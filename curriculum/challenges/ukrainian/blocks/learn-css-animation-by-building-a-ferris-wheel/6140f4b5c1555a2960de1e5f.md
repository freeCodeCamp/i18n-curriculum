...
Створіть ще одне правило `@keyframes` з ім’ям `cabins`. Використайте ті самі властивості, що й у вашому `@keyframes wheel`, скопіювавши обидва правила `0%` і `100%`, але встановіть властивість `transform` селектора `100%` у значення `rotate(-360deg)`.

# --hints--

У вас має бути правило `@keyframes`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.length === 2);
```

Ваше нове правило `@keyframes` має називатися `cabins`.
...
Селектор `100%` має йти після селектора `0%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '100%')
```

Селектор `100%` має мати властивість `transform`, встановлену в `rotate(-360deg)`.
