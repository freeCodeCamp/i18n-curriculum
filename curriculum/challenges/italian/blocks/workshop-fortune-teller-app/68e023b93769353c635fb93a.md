...
Ora, all'interno del template literal della funzione `renderCard`, crea un elemento `div`. All'interno di `div`, aggiungi un elemento `h2` che produce in output `drawingType`, e un elemento `figure` con la classe `card_container ${isReversed ? "reversed-card" : ""}` e l'attributo `data-id="${shortName}"`.

# --hints--

Dovresti avere un elemento `div` nel template literal della funzione `renderCard`.
...
Dovresti avere un elemento `h2` che interpola `drawingType` all'interno dell'elemento `div`.
...
Dovresti avere un elemento `figure` all'interno dell'elemento `div`.
...
Il tuo elemento `figure` dovrebbe avere la classe `card_container` e usare un ternario per aggiungere condizionatamente `reversed-card`.
...
Il tuo elemento `figure` dovrebbe avere un attributo `data-id` impostato su `${shortName}`.
