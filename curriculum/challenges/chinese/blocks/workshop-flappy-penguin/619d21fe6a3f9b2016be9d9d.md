...
选择 `class` 为 `left` 的 `.foot` 元素，将其放置在父元素左侧 `25%` 的位置。 然后，选择 `class` 为 `right` 的 `.foot` 元素，将其放置在父元素右侧 `25%` 的位置。

# --hints--

你应该有一个 `.foot.left` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.foot.left', '.left.foot']));
```

你应该将 `.foot.left` 的 `left` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.foot.left', '.left.foot'])?.left, '25%');
```

你应该有一个 `.foot.right` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.foot.right', '.right.foot']));
```

你应该将 `.foot.right` 的 `right` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
