...
حان الوقت الآن لتعيين التبويب الذي تم النقر عليه كمحدد.

استخدم طريقة `setAttribute` على الـ `tab` لتعيين الخاصية `aria-selected` إلى `"true"`.

# --hints--

عند نقر `tab`، يجب تعيين الخاصية `aria-selected` للتبويب الذي تم النقر عليه إلى `"true"`.
