...
`.sort()` へのコールバックは数値を返す必要があります。その数値によって `a` と `b` の要素の並び順が決まります：

- 数値が負の場合、`a` を `b` の前に並べます。
- 数値が正の場合、`b` を `a` の前に並べます。
- 数値がゼロの場合、`a` と `b` の順序は変更しません。

数値を昇順（小さい順から大きい順）に並べたいことを考慮して、上記のロジックで正しく並べ替えられるように、`a` と `b` を使った単一の引き算の計算を返してください。

# --hints--

コールバック関数は明示的な `return` を使うべきです。

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*\(\s*a\s*,\s*b\s*\)\s*=>\s*{\s*return/);
```

コールバック関数は `a - b` を返すべきです。
