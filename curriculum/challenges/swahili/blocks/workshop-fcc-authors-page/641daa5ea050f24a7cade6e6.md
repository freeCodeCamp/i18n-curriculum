...
Ikiwa hali hii itatimizwa, zima kitufe kwa kuweka sifa yake ya `disabled` kuwa `true`. Pia, weka `textContent` ya kitufe kuwa `"No more data to load"`.
...
Unapaswa kuweka sifa ya `disabled` ya `loadMoreBtn` kuwa `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Unapaswa kuweka `textContent` ya `loadMoreBtn` kuwa `"No more data to load"`.
