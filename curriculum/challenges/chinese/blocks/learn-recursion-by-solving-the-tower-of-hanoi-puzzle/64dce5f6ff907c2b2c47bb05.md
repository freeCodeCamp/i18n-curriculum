...
当 `target` 为空时，圆盘必须从 `source` 移动到 `target`。

在 `forward` 声明之后，添加 `if` 语句来检查 `rods[target]` 是否为空。 如果是，请将 `forward` 更改为 `True`。

# --hints--

你应该有一个 `if` 语句来检查 `rods[target]` 是否为空，并将 `True` 分配给 `forward`。
