...
不要担心底部的空间，稍后当你为建筑物顶部的元素添加一些高度时，所有内容都会向下移动。

给 `.fb1c` 添加一个 `repeating-linear-gradient`，方向为 `90deg`，从 `0%` 到 `10%` 处的颜色为 `--building-color4`，从 `10%` to `15%` 处的颜色为 `transparent`。

# --hints--

你应该给 `.fb1c` 一个值为 `repeating-linear-gradient` 的 `background`。
...
你应该在从 `0%` 到 `10%` 处使用第一种颜色 `--building-color4`。
...
你应该在从 `10%` 到 `15%` 处使用第二种颜色 `transparent`。
