...
Ndani ya darasa la vitu la `Spider`, tengeneza kijenzi kinachopokea kigezo kimoja `emojiElement` cha aina `HTMLParagraphElement`.

Ndani ya kijenzi, fanya wito wa kitendakazi cha `super` ukiwa na kigezo `emojiElement` ili kuitisha kijenzi cha darasa la vitu la mzazi `Bug`. Kisha, weka sifa ya `emoji` kuwa thamani ya mfuatano wa herufi `"🕷️"`.

# --hints--

Darasa lako la vitu la `Spider` linapaswa kuwa na kijenzi.
...
Kijenzi chako cha `Spider` kinapaswa kuwa na kigezo kinachoitwa `emojiElement` cha aina `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Kijenzi chako cha `Spider` kinapaswa kuita `super` ukiwa na kigezo `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?super\s*\(\s*emojiElement\s*\)/);
```

Kijenzi chako cha `Spider` kinapaswa kuweka `this.emoji` kuwa `"🕷️"`.
