...
为 `for` 添加一个循环，遍历 `catalog`。在循环内部，声明 `const entry = catalog[i]`。构建一个由四个字段用逗号分隔的行字串，并将其推入 `rows`。

字串字段在输出中需要字面双引号字符（例如 `"King, Stephen"`），以防字段内的逗号破坏 CSV 格式。由于 `year` 是数字类型，不需要双引号。
