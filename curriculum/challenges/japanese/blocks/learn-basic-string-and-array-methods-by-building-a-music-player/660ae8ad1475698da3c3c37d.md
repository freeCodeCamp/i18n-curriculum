...
名前付きアロー関数式を呼び出すには、その名前で関数を参照できます。

```js
exampleArrowFunction();
```

`printGreeting` 関数定義の下に、関数を呼び出してください。コンソールを開いて出力を確認しましょう。

# --hints--

`printGreeting` 関数を呼び出す必要があります。

```js
assert.match(code, /printGreeting\(\);?/);
```

`printGreeting` 関数の呼び出しは、`printGreeting` 関数定義の下に置いてください。

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

`printGreeting` 関数の呼び出しは、コンソールに `Hello there!` を出力するはずです。
