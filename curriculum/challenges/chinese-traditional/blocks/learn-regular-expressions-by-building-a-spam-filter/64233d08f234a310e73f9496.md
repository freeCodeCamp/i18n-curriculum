...
你的下一個正規表達式將會匹配像 `stock alert` 這樣的字串。宣告一個 `stockRegex` 變數並指定一個能匹配字串 `stock alert` 的正規表達式。記得要使其不區分大小寫。

也將它添加到你的 `denyList` 陣列中。

# --hints--

你應該使用 `const` 來宣告你的 `stockRegex` 變數。

```js
assert.match(code, /const\s+stockRegex\s*=/);
```

你的 `stockRegex` 變數應該指定一個正規表達式。
