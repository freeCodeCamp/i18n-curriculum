...
将你的新 `getVariance` 函数添加到 `calculate` 函数里，并更新相应的 HTML 元素。

# --hints--

你的 `calculate` 函数应该有一个 `variance` 变量，其值为 `getVariance(numbers)`。

```js
assert.match(calculate.toString(), /variance\s*=\s*getVariance\(\s*numbers\s*\)/);
```

你应该将 `#variance` 元素的 `textContent` 设置为 `variance` 变量的值。
