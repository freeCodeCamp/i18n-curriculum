...
まだクリックされていないリンクのデフォルトの色は通常青です。すでにページから訪問されたリンクのデフォルトの色は通常紫です。

リンクが訪問済みかどうかに関係なく`footer`のリンクを同じ色にするには、アンカー要素（`a`）の型セレクターを使い、`black`プロパティに`color`の値を設定してください。
