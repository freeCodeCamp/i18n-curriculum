...
`object-fit` プロパティは、要素をコンテナ内でどのように配置するかをブラウザーに指示します。この場合、`cover` は画像をコンテナに合わせて埋め込み、アスペクト比を変えずに必要に応じてトリミングします。
...
`img` セレクターには `object-fit` プロパティが `cover` に設定されているはずです。
