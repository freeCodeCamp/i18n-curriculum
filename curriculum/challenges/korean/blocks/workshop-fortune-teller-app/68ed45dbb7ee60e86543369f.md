...
데이터를 가져온 후, 응답을 변수에 저장하고 JSON으로 파싱하세요. 파싱한 데이터에 `Deck` 타입을 지정한 다음 `cards` 속성에 할당하세요. `catch` 블록 안에서는 이 메시지로 오류를 반드시 기록하세요: `"Error fetching cards:", error`.

# --hints--

`data` 안에 `fetchCardsData`라는 변수를 선언하세요.
...
`data.cards`를 `this.cards`에 저장하세요.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

`catch` 블록에서는 `console.error()`를 사용해 `"Error fetching cards:", error` 메시지로 오류를 기록해야 합니다.
