...
يمكنك فعليًا تبسيط هذا المنطق قليلًا. باستخدام الإرجاع الضمني للدالة السهمية، يمكنك إرجاع قيمة طريقة `.reduce()` مقسومة على طول المصفوفة مباشرةً، دون الحاجة إلى تعيين أي متغيرات.

حدّث دالة `getMean` كما هو موضح أعلاه.

# --hints--

يجب إزالة إعلان المتغير `sum`.

```js
assert.notMatch(getMean.toString(), /sum\s*=/);
```

يجب إزالة إعلان المتغير `mean`.

```js
assert.notMatch(getMean.toString(), /mean\s*=/);
```

لا يجب تغيير المنطق داخل طريقة `reduce`.
...
يجب قسمة نتيجة طريقة `reduce` على طول المصفوفة.
...
يجب استخدام صيغة الإرجاع الضمني لإرجاع نتيجة `reduce` مقسومة على طول المصفوفة مباشرةً.
