...
يمكنك الآن تنسيق محتوى عنصر `.heading` باستخدام شبكة CSS.

تُستخدم دالة CSS `repeat()` لتكرار قيمة بدلاً من كتابتها يدويًا، وهي مفيدة لتخطيطات الشبكة. على سبيل المثال، تعيين خاصية `grid-template-columns` إلى `repeat(20, 200px)` سينشئ 20 عمودًا بعرض `200px` لكل منها.

أعطِ عنصر `.heading` الخاص بك خاصية `grid-template-columns` مضبوطة على `repeat(2, 1fr)` لإنشاء عمودين بعرض متساوٍ.

# --hints--

يجب أن يحتوي محدِّد `.heading` على خاصية `grid-template-columns` مضبوطة على `repeat(2, 1fr)`.
