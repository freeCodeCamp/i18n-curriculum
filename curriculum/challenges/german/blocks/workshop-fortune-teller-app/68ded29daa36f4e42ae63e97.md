...
Schließen Sie die Funktion ab, indem Sie die `forEach`-Array-Methode verwenden, um das Argument elements zu durchlaufen. Fügen Sie innerhalb der Callback-Funktion jedem Element die Klasse hidden hinzu, indem Sie die `.add()`-Methode von `classList` verwenden.

# --hints--

Sie sollten eine `forEach`-Methode am Parameter `elements` haben.
...
Ihre forEach-Methode sollte einen Parameter namens `el` haben.
...
Innerhalb der forEach-Methode sollten Sie die Klasse hidden mit der `classList.add`-Methode hinzufügen.
