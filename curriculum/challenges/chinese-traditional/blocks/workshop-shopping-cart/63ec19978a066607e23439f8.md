...
現在你需要取得你的兩個按鈕。繼續這個樣式，取得 `#cart-btn` 和 `#clear-cart-btn` 元素。分別將它們儲存在名為 `cartBtn` 和 `clearCartBtn` 的變數中。

# --hints--

你應該使用 `document.getElementById()` 來取得 `#cart-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)cart-btn\1\s*\)/);
```

你應該將 `#cart-btn` 元素指定給名為 `cartBtn` 的變數。請記得使用 `const` 來宣告該變數。

```js
assert.match(code, /const\s+cartBtn\s*=\s*document\.getElementById\(\s*('|"|`)cart-btn\1\s*\)/);
```

你應該使用 `document.getElementById()` 來取得 `#clear-cart-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)clear-cart-btn\1\s*\)/);
```

你應該將 `#clear-cart-btn` 元素指定到名為 `clearCartBtn` 的變數。請記得使用 `const` 來宣告該變數。
