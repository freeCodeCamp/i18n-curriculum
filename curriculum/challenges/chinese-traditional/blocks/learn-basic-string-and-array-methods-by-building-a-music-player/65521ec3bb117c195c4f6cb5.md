...
要在歌曲暫停時儲存當前時間，請將 `userData` 物件的 `songCurrentTime` 設定為 `audio` 變數的 `currentTime`。

*注意*：你不應該在此步驟使用選用串鏈，因為 `userData.songCurrentTime` 在此時不會是 `null` 或 `undefined`。
...
你應該將 `audio.currentTime` 指定到 `userData.songCurrentTime`。
