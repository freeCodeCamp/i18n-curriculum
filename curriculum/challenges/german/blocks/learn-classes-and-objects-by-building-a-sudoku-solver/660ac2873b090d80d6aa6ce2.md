...
Erstellen Sie nun eine `for`-Schleife, um die Zeilen im Spielfeld zu durchlaufen. Verwenden Sie `row` als Schleifenvariable.

# --hints--

Sie sollten eine `for`-Schleife erstellen, die `self.board` durchläuft.
...
Sie sollten `row` als Schleifenvariable verwenden.
