...
Você precisa posicionar o círculo interno e torná-lo inicialmente invisível.

Ainda mirando `.contact-method-radio-btn::before`, defina `transform` como `translate(3px, 3px) scale(0)`. A função `translate` move o círculo levemente e `scale(0)` o reduz a tamanho zero para que ele fique oculto por padrão. 

Além disso, adicione uma `transition` de `all 0.3s ease-in` para criar uma animação suave quando ela aparecer. Isso significa que qualquer alteração de estilo será animada ao longo de 0,3 segundos, começando devagar e acelerando.

# --hints--

Você deve continuar a direcionar o pseudo-elemento `.contact-method-radio-btn::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Defina a propriedade `transform` como `translate(3px, 3px) scale(0)`.
...
Defina a propriedade `transition` para `all 0.3s ease-in`.
