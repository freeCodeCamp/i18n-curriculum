...
你的正则表达式应该匹配整个单词，而不是部分单词。 也就是说，你不想匹配 `hands-free money management`。

为此，首先检查模式前后的空格。 你可以使用元字符 `\s` 来执行此操作，它将匹配空格、制表符和换行符。

# --hints--

你的 `freeRegex` 应该使用 `\s` 令牌。

```js
assert.match(freeRegex.source, /\\s/);
```

你的 `freeRegex` 应该在模式的开头和结尾寻找空格。
