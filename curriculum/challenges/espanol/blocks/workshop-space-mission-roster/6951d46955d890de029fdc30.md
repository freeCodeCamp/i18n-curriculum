...
Anteriormente habías agregado una llamada a `console.log()` en tu función `swapCrewMembers` para validar su comportamiento, y ahora la llamada ya no es necesaria. Para evitar que el terminal se llene en los próximos pasos, elimina el `for` actual que registra en la consola.

# --hints--

Elimina el `for` loop de la función `swapCrewMembers`. Debes borrar todo el bloque del loop.
