...
Функція `apply` повертає функцію електронної таблиці, але фактично не застосовує її. Оновіть `apply`, щоб викликати функцію. Передайте результат виклику `toNumberList` з `args` як аргументом.

# --hints--

Функція `apply` має викликати функцію `spreadsheetFunctions[fn.toLowerCase()]`.
