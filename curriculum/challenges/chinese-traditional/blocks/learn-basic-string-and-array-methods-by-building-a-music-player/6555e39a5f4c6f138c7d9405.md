...
在前面的步驟中，你學習瞭如何使用 `sort()` 方法按字母順序對歌曲進行排序。 回調函數的另一個用例是隨機化數組。

隨機化項目數組的一種方法是從 `Math.random()` 中減去 `0.5`，這會產生正數或負數的隨機值。 這使得比較結果成爲正值和負值的混合，從而導致元素的隨機排序。
...
在 `userData?.songs` 數組上使用 `sort()` 方法。 將回調傳遞給方法，並返回 `Math.random() - 0.5` 的結果。
