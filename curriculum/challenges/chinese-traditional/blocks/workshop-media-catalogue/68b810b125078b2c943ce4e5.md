...
在 `TVSeries` 類別中，創建一個 `__str__` 方法，該方法傳回一個字串，格式如下：`title (year) - S seasons, N episodes, duration min avg, director`，其中 `title`、`year`、`S`、`N`、`duration` 和 `director` 應分別被 `title`、`year`、`seasons`、`total_episodes`、`duration` 和 `director` 屬性取代。

# --hints--

你的 `TVSeries` 類別應該有一個 `__str__` 方法。
...
你的 `__str__` 方法應該傳回格式為 `title (year) - S seasons, N episodes, duration min avg, director` 的字串。
