...
在 `path` 变量之后，添加一个 `console.log()` 调用，使用一个模板字面量输出：

```md
\n{startNode}-{nodeNo} distance: {distances[nodeNo]}\nPath: {path}
```

例如，如果 `startNode` 是 `0`，而 `nodeNo` 是 `3`，输出应如下所示：
...
你应该在 `console.log()` 循环内有一个 `for...of`。
...
`console.log()` 应该使用包含 `${startNode}-${nodeNo} distance: ${distances[nodeNo]}` 的模板字面量。
