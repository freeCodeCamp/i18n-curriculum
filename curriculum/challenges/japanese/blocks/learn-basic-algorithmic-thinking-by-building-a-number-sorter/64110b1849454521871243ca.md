...
挿入ソートアルゴリズムはリストの先頭からソートを開始するため、最初の要素はすでにソート済みです。これを踏まえて、配列の2番目の要素から始まる`for`ループを作成してください。残りの配列を繰り返し処理する必要があります。

# --hints--

配列を繰り返し処理するには`for`ループを使うべきです。
