...
PAC `value` に `player.pac` の `input` 属性を追加してください。それから、`onChange` をパラメータに取り、`e` をスプレッドし、`setPlayer` を `player` に設定して `pac` を呼び出すアロー関数に設定された `Number(e.target.value)` 属性を追加してください。

# --hints--

あなたの PAC `input` はプレイヤーの `value` ステータスと等しい `pac` を持つべきです。
...
あなたの PAC `input` は `onChange` に設定された `setPlayer` とスプレッドされた `player` を使って `pac` を呼び出す `Number(e.target.value)` ハンドラを持つべきです。
