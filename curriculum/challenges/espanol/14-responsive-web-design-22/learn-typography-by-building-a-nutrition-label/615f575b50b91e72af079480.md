...
Cree un nuevo selector `.left-container p` configurando el margen superior e inferior en `-5px`, y el margen izquierdo y derecho en `-2px`. También ajusta `font-size` a `2em` y `font-weight` a `700`.

# --hints--

Debería tener un nuevo selector `.left-container p`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

Su nuevo selector `.left-container p` debe tener una propiedad `margin` establecida en `-5px -2px`.
...
Su nuevo selector `.left-container p` debe tener una propiedad `font-size` establecida en `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

Su nuevo selector `.left-container p` debe tener una propiedad `font-weight` establecida en `700`.
