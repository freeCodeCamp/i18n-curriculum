...
Eleza kitendakazi cha `playSong` kinachopokea kigezo kimoja ambacho kitawakilisha kitambulisho cha kipekee cha wimbo unaotaka kuucheza.

# --hints--

Unapaswa kuwa na kitendakazi tupu kinachoitwa `playSong`.

```js
assert.isFunction(playSong);
```

Kitendakazi chako cha `playSong` kinapaswa kupokea kigezo kimoja.
