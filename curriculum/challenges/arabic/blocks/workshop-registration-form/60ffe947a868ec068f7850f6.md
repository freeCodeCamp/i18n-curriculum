...
لجعل زر الإرسال يبدو أكثر تميزًا، امنحه `background-color` بقيمة `#3b3b4f`، و`border-color` بقيمة `white`.

# --hints--

يجب أن تعطي زر الإرسال `background-color` بقيمة `#3b3b4f`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.backgroundColor, 'rgb(59, 59, 79)');
```

يجب أن تعطي زر الإرسال `border-color` بقيمة `white`.
