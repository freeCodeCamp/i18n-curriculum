...
Verknüpfen Sie die entsprechenden Formularelemente mit ihren `label`-Elementen.

Verwenden Sie `profile-picture`, `age`, `referrer` und `bio` als Werte für die jeweiligen `id`-Attribute.

# --hints--

Das erste `input`-Element sollte ein `id` von `profile-picture` haben.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[0]?.matches('#profile-picture'))
```

Das zweite `input`-Element sollte ein `id` von `age` haben.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[1]?.matches('#age'))
```

Das `select`-Element sollte ein `id` von `referrer` haben.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) select')?.matches('#referrer'))
```

Das `textarea`-Element sollte ein `id` von `bio` haben.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) textarea')?.matches('#bio'))
```

Das erste `label`-Element sollte ein `for`-Attribut mit dem Wert `profile-picture` haben.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[0]?.matches('label[for="profile-picture"]'))
```

Das zweite `label`-Element sollte ein `for`-Attribut mit dem Wert `age` haben.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[1]?.matches('label[for="age"]'))
```

Das dritte `label`-Element sollte ein `for`-Attribut mit dem Wert `referrer` haben.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[2]?.matches('label[for="referrer"]'))
```

Das vierte `label`-Element sollte ein `for`-Attribut mit dem Wert `bio` haben.
