...
`fetch` の呼び出しでエラーが発生した場合には、`catch` ブロックがそれを処理します。

`catch` ブロックの中に `console.log` を追加して、仮引数 `err` を出力しましょう。

また、`fetch` の呼び出しで何が返ってくるか確認できたので、`try` ブロックの `console.log(data);` は削除してください。

# --hints--

`console.log` を使用して、変数 `err` をログ出力してください。

```js
assert.match(code, /\s*console\.log\(\s*err\s*\)/);
```

`console.log` は `catch` ブロック内に置いてください。

```js
assert.match(code, /\s*try\s*{[^}]*\s*}\s*catch\s*\(\s*err\s*\)\s*{[^}]*\s*console\.log\(\s*err\s*\)[^}]*}/);
```

`try` ブロックから `console.log(data);` を取り除いてください。
