...
Declare una variable `currValue` y asígnale el valor en `array[i]`. Luego, declare una variable `j` y asígnale `i - 1`. Tu variable `j` debe ser reasignable.

# --hints--

Debes declarar una variable `currValue` con `const`.
...
Debes asignarle a `currValue` el valor en `array[i]`.
...
Debes declarar una variable `j` con `let`.
...
Debes asignarle a `j` el valor de `i - 1`.
