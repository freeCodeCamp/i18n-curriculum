...
Наразі всі градієнти, які ви створили, йшли зверху вниз — це напрямок за замовчуванням. Ви можете вказати інший напрямок, додавши його перед кольорами, ось так:
...
Заповніть `.bb3`, використовуючи `repeating-linear-gradient`. Використайте `90deg` для напрямку, свій `building-color3` для перших двох кольорів та `window-color3` на `15%` для третього.

Якщо ви не вказуєте відстань для кольору, він використовуватиме розумні значення. В цьому разі першими двома кольорами за замовчуванням будуть `0%` та `7.5%`, оскільки вони починаються з `0%`, а `7.5%` становить половину `15%`, отже не потрібно їх встановлювати.

# --hints--

Надайте `background` до `.bb3`, використовуючи `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb3")?.background, "repeating-linear-gradient");
```

Використайте `90deg` для напрямку в першому аргументі `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb3")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Не встановлюйте відсоток для першого кольору.

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle(".bb3")?.getPropVal('background', true), "var(--building-color3)0%");
```

Не встановлюйте відсоток для другого кольору.
