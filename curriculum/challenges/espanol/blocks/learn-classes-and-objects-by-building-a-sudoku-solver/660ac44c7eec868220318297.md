...
Modifica la comprensión de `row_str` para que dé una cadena solo cuando el elemento no sea cero, y un carácter asterisco en caso contrario.

# --hints--

La comprensión de listas asignada a la variable `row_str` debe llamar a `str()` en cada elemento `i` en `row` si `i` es verdadero, y debe evaluarse como `'*'` de lo contrario.
