...
Before the `${name}` expression, add a new embedded expression. Dentro dessa expressão, use um operador ternário para verificar se `isCaptain` está definido como verdadeiro (true). If so, return `"(Captain)"` otherwise return an empty string.
