...
應該將 `.beak.top` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.width, '20%');
```

應該將 `.beak.top` 的 `top` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.top, '60%');
```

應該將 `.beak.top` 的 `left` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
