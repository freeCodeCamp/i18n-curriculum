...
3 つ目の `@media` クエリを作成し、`only screen` かつ `max-width` が `550px` の場合を対象にしてください。 その中で、`.hero-title` セレクターの `font-size` を `6rem`、`.hero-subtitle, .author, .quote, .list-title` セレクターの `font-size` を `1.8rem`、`.social-icons` セレクターの `font-size` を `2rem`、`.text` セレクターの `font-size` を `1.6rem` に設定してください。
