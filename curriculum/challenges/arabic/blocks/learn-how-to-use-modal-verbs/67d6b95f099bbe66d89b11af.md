...
`critical`، `performance`، `overwhelmed`، `might`، `have`، `deployment`، و`depends on`.
...
هذا الفعل الناقص يعبر عن الاحتمالية.
...
تشير هذه الكلمة إلى جعل البرنامج متاحًا للمستخدمين.
...
تعني هذه العبارة أن شيئًا ما يتأثر بشيء آخر.
...
تصف هذه الكلمة مشكلات خطيرة يجب إصلاحها فورًا.
...
يُظهر هذا الفعل الضرورة — شيء يجب القيام به.
...
تشير هذه الكلمة إلى مدى كفاءة عمل شيء ما.
...
تعني هذه الكلمة وجود الكثير مما يجب التعامل معه.
