...
두 개의 `div` 요소를 만드세요. 첫 번째는 `cat-left-inner-ear` 클래스를 가진 `.cat-left-ear` 요소 안에, 두 번째는 `cat-right-inner-ear` 클래스를 가진 `.cat-right-ear` 요소 안에 있어야 합니다.

# --hints--

`cat-left-ear` 클래스를 가진 기존 `div` 요소는 변경하지 마세요.

```js
assert(document.querySelectorAll('div.cat-left-ear').length === 1);
```

`cat-right-ear` 클래스를 가진 기존 `div` 요소는 변경하지 마세요.
