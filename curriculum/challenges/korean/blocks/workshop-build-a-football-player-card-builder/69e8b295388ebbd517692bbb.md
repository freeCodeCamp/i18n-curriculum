...
`card-image-wrap` 요소 뒤에 `div`의 `className`를 가진 `card-name-strip`을 추가하세요. 그 안에 `span`의 `className`와 텍스트 `card-name`를 가진 `PELE`를 추가하세요.

# --hints--

`div`의 `className`를 가진 `card-name-strip`을 추가해야 합니다.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

`span` 안에 `className`의 `card-name`를 가진 `.card-name-strip`을 추가해야 합니다.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

`.card-name` 스팬에는 텍스트 `PELE`가 있어야 합니다.
