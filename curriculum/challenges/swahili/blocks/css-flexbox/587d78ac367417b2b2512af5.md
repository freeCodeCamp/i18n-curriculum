...
title: Tumia sifa ya flex-direction kuunda safu katika ingizo la tweet
challengeType: 0
...
Ingizo la tweet `header` na `footer` lilitumia sifa ya `flex-direction` awali kwa thamani ya safu (row). Vivyo hivyo, vitu vilivyo ndani ya kipengele cha `.profile-name` vitafanya kazi vizuri ikiwa vimepangwa kama safu (column).

# --instructions--

Ongeza sifa ya CSS `flex-direction` kwenye kipengele cha `.profile-name` cha kichwa na weka thamani kuwa `column`.
...
Kipengele cha `.profile-name` kinapaswa kuwa na sifa ya `flex-direction` iliyowekwa kuwa `column`.
