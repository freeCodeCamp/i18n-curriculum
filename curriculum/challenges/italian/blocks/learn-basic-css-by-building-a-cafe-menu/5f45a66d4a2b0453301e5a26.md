...
Spostati sull'elemento `footer` e imposta tutto il testo con un valore `14px` per la dimensione del carattere tipografico.

# --hints--

Dovresti avere un selettore `footer`.
...
Il tuo selettore `footer` dovrebbe essere sotto il tuo commento.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Dovresti impostare la proprietà `font-size` su `14px`.
...
Il tuo elemento `footer` dovrebbe avere un id `font-size` di `14px`.
