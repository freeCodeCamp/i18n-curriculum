...
You should create an `if` statement with `(taskData.length)` as the condition. Zur Erinnerung, `0` ist ein fehlerhafter Wert.

```js
assert.match(code, /if\s*\(\s*taskData\.length\s*\)\s*\{\s*/)
```

Du solltest die `updateTaskContainer`-Funktion in deiner `if`-Anweisung aufrufen.
