...
بناءً على الحوار، أي جملة تُظهر الاستخدام الصحيح لـ `going to` بناءً على الأدلة الحالية؟
...
تستخدم هذه الجملة `will` لفعل مستقبلي محتمل، لكنها تفتقر إلى الإحساس بقرار اتُخذ بناءً على الأدلة الحالية.
...
هنا، تُستخدم `will` لفعل مستقبلي بناءً على وضع حالي، لكن `going to` ستكون أكثر ملاءمة لأن القرار يبدو مستندًا إلى خبرة الفريق الحالية.
...
تستخدم هذه الجملة `will` لاتخاذ قرار عفوي (`calling the designer`)، وهذا مناسب، لكنها لا توضح استخدام `going to` بناءً على دليل حالي.
