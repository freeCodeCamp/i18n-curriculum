...
为了用数据填充论坛排行榜，你需要从 API 请求数据。 这被称为<dfn>异步</dfn>操作，这意味着任务独立于主程序流执行。 

你可以使用 `async` 关键字创建一个异步函数，该函数会返回一个 Promise。
...
使用 `async` 关键字来创建一个名为 `fetchData` 的异步箭头函数。

# --hints--

你应该使用 `async` 关键字创建一个名为 `fetchData` 的异步箭头函数。
