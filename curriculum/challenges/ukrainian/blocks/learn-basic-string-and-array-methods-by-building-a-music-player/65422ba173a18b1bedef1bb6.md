...
У стрілковій функції слухача події додайте `if`, щоб перевірити, чи `userData?.currentSong` є `null`.

Всередині блоку `if` викличте функцію `playSong()` з ідентифікатором `id` першої пісні в масиві `userData?.songs`. Це гарантує, що перша пісня в плейлисті буде відтворена першою.
...
Ви маєте викликати функцію `playSong` з `userData?.songs[0].id` всередині вашого блоку `if`.
