...
Dentro del método `initializeEventListeners`, adjunta un listener de evento `click` a `singleCardBtn`. Dentro del manejador del evento, llama al método `singleCardSelected`. recuerda obtener el `singleCardBtn` desde `elements`.

Crearás este método en los próximos pasos.

# --hints--

Debes adjuntar un evento `click` al elemento `singleCardBtn`. No olvides que aquí necesitas la palabra clave `this`.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Dentro del callback del evento, debes llamar al método `singleCardSelected`.
