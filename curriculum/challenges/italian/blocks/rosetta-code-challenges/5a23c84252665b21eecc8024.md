...
title: Puzzle con i nomi degli stati
challengeType: 1
forumTopicId: 302323
...
**State name** è un <em>puzzle di parole</em> in cui prendi le lettere di due nomi di stati degli Stati Uniti e le riorganizzi per formare i nomi di due stati diversi degli Stati Uniti.


# --instructions--

Scrivi una funzione per risolvere la sfida dato un array di nomi reali di stati degli Stati Uniti e per nomi di stati fittizi.

La funzione dovrebbe restituire un array. Ogni elemento dovrebbe essere un oggetto in questa forma: `{"from":[],"to":[]}`. L'array `from` dovrebbe contenere i nomi originali e l'array `to` dovrebbe contenere i nomi risultanti.
