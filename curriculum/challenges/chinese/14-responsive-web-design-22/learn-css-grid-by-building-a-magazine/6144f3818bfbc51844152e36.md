...
将每个已定义的 `grid-template-columns` 属性值放在 `minmax` 函数中，使用每个值作为第二个参数。 第一个参数应分别为 `2rem`、`min-content` 和 `2rem`。

# --hints--

你的 `main` 选择器应该有一个 `grid-template-columns` 属性设置为 `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`。
