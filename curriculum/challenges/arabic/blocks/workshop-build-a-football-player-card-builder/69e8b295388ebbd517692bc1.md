...
الآن بعد أن عرّفت واجهة `PlayerData`، حدّث مكوِّن `PlayerCard` ليقبل خاصية `player` من نوع `PlayerData` لتتطابق مع واجهة `PlayerData`.

تذكّر أنه يمكنك تفكيك الخصائص بهذه الطريقة:
...
يجب أن تحدّث مكوِّن `PlayerCard` ليقبل خاصية `player` من نوع `PlayerData`.
