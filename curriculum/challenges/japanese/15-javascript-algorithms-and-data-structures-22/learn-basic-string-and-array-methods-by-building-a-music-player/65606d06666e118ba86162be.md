...
次のキーと値を使って、2 つ目のオブジェクトを追加してください。
...
`allSongs` 配列内に要素が 2 つ必要です。

```js
assert.equal(allSongs.length, 2);
```

配列 `allSongs` の 2 番目の要素はオブジェクトであるべきです。

```js
assert.isObject(allSongs[1]);
```

配列 `allSongs` 内の 2 番目のオブジェクトは、`id` プロパティが数値の `1` に設定されているようにしてください。

```js
assert.equal(allSongs[1].id, 1);
```

配列 `allSongs` 内の 2 番目のオブジェクトは、`title` プロパティが文字列の `"Can't Stay Down"` に設定されているようにしてください。

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

配列 `allSongs` 内の 2 番目のオブジェクトは、`artist` プロパティが文字列の `"Quincy Larson"` に設定されているようにしてください。

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

配列 `allSongs` 内の 2 番目のオブジェクトは、`duration` プロパティが文字列の `"4:15"` に設定されているようにしてください。

```js
assert.equal(allSongs[1].duration, "4:15");
```

配列 `allSongs` 内の 2 番目のオブジェクトは、`src` プロパティが文字列の `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"` に設定されているようにしてください。
