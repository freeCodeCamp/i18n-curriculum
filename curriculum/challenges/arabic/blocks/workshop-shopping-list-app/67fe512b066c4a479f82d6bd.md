...
داخل عنصر `li`، فوق نص `item`، أضف عنصر `input` مع تعيين الخاصية `type` إلى `checkbox`. أيضًا، عيّن الخاصية `onChange` إلى دالة مجهولة تستدعي `toggleItem()` مع `item` كمعلمة.

أخيرًا، غلف عنصر `input` ونص `item` داخل عنصر `label` لتحسين إمكانية الوصول.

# --hints--

يجب أن تضيف عنصر `input` آخر مع تعيين الخاصية `type` إلى `checkbox`.
...
يجب أن يحتوي عنصر `input` الجديد على الخاصية `onChange` المعينة إلى دالة مجهولة تستدعي `toggleItem()` مع `item` كمعلمة.
...
يجب أن يُغلف عنصر `input` الجديد ونص `item` داخل عنصر `label`.
