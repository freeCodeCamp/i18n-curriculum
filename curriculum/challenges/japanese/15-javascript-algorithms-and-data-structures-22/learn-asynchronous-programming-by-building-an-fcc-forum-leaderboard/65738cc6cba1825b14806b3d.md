...
次に、`const` の変数 `forumCategoryUrl` を作成し、文字列 `"https://forum.freecodecamp.org/c/"` を代入してください。

その下にもう 1 つ `const` 変数 `avatarUrl` を作成し、文字列 `"https://sea1.discourse-cdn.com/freecodecamp"` を代入してください。

# --hints--

`forumCategoryUrl` という変数を `const` で宣言してください。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=/);
```

`"https://forum.freecodecamp.org/c/"` を変数 `forumCategoryUrl` に代入してください。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=\s*('|")https:\/\/forum\.freecodecamp\.org\/c\/\1\s*/);
```

`avatarUrl` という変数を `const` で宣言してください。

```js
assert.match(code, /const\s+avatarUrl\s*=/);
```

`"https://sea1.discourse-cdn.com/freecodecamp"` を変数 `avatarUrl` に代入してください。
