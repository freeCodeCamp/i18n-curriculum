...
Fügen Sie einen `else`-Block hinzu, um die aktuelle Wiedergabezeit des Songs zu verwalten. So können Sie den aktuellen Song an der Stelle fortsetzen, an der er pausiert wurde.

Setzen Sie innerhalb des `else`-Blocks die `currentTime`-Eigenschaft des `audio`-Objekts auf den Wert, der in `userData?.songCurrentTime` gespeichert ist.
...
Sie sollten einen `else`-Block zum bestehenden `if`-Block hinzufügen.
...
Sie sollten `audio.currentTime` innerhalb des `userData?.songCurrentTime`-Blocks Ihrer `else`-Anweisung auf `if` setzen.
