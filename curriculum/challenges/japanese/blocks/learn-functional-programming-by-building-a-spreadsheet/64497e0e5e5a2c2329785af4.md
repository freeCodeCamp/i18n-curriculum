...
スプレッドシートソフトウェアは通常、セルの先頭に`=`を使って計算を行うことを示し、スプレッドシートの関数を評価します。

`&&`演算子を使って、`if`文に2つ目の条件を追加し、`value`の最初の文字が`"="`であるかどうかもチェックしてください。

# --hints--

`&&`演算子を使って、`if`文に2つ目の条件を追加し、`value`の最初の文字が`"="`であるかどうかもチェックしてください。`[0]`、`.startsWith()`、または`.charAt(0)`を使うことができます。
