...
前の曲を取得するには、`1`の`currentSongIndex`から`userData?.songs`を引き、その結果を定数`previousSong`に代入してください。その後、`playSong`関数を呼び出し、`previousSong.id`を引数として渡してください。
...
`userData?.songs[currentSongIndex - 1]`を`previousSong`の定数に代入してください。
...
`playSong`関数を`previousSong.id`で呼び出してください。
