...
Створіть останній елемент `p` в кінці елемента `.text` та надайте йому такий текст:
...
Додайте шостий елемент `p` до елемента `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 7);
```

Шостий елемент `p` повинен мати наданий текст.
