...
當用戶嘗試將十進制數 `5` 轉換爲二進制時，你將顯示動畫，因此你需要在 `checkUserInput()` 函數中添加檢查。

使用 `if` 語句檢查 `numberInput` 的 `value` 屬性是否等於數字 `5`。 請記住在將字符串與 `5` 進行比較之前使用 `parseInt()` 函數將字符串轉換爲數字。 暫時將 `if` 語句留空。

# --hints--

你應該在 `checkUserInput()` 函數中現有的 `if` 語句之後添加第二個 `if` 語句。

```js
assert.match(String(checkUserInput), /if\s*\(\s*!numberInput\s*\.\s*value\s*\|\|\s*isNaN\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)[\s\S]+if\s*\(/);
```

你應該在新的 `if` 語句的條件中使用 `parseInt()` 函數。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(/);
```

你應該檢查 `numberInput` 的 `value` 屬性是否等於 `5`。
