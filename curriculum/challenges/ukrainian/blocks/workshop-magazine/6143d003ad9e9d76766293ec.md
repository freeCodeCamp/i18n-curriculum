...
У вашому елементі `.image-quote` вкладіть елементи `hr`, `p` та другий елемент `hr`. Додайте елементу `p` множину `class`, встановлену в `quote`, та текст `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
