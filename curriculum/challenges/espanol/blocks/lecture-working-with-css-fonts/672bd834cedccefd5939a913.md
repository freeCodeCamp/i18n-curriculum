...
Dentro de las llaves, necesitarás incluir descriptores para personalizar tu fuente. Veamos algunos de los más comúnmente usados.  The font-family descriptor specifies the name that you will use throughout the stylesheet to refer to that font. Por ejemplo, digamos que defines esta regla `@font-face rule`. Tiene el descriptor `font-family` definido y su valor es `MyCustomFont`:
