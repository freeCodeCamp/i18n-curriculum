...
Um schließlich die Berechnungen anzuzeigen, durchlaufen Sie das `propertyInputs`-Objekt mit einer `for...of`-Schleife mit `Object.entries()`. Sie können den Schlüssel ignorieren, indem Sie vor `input` in der Schleifendeklaration ein Komma setzen.

# --hints--

Sie sollten eine `for...of`-Schleife erstellen, die jeden Eintrag aus `Object.entries(propertyInputs)` destrukturiert, indem sie den Schlüssel überspringt und nur `input` behält.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Ihre `for...of`-Schleife sollte `Object.entries(propertyInputs)` durchlaufen.
