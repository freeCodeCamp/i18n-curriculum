...
Innerhalb Ihrer `while`-Schleife sollten Sie `currentTickQueue` mit dem Dekrementoperator `--` dekrementieren und `processed` mit dem Inkrementoperator `++` inkrementieren.
