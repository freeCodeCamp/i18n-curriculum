...
`PlayerData` 介面目前只描述了基本的玩家資訊。你需要將它擴充為卡片所需的其他屬性。

在介面內，新增一個型別為 `imageUrl` 的 `string`，以及六個型別為 `pac` 的數值屬性：`sho`、`pas`、`dri`、`def`、`phy` 和 `number`。

# --hints--

你的 `PlayerData` 介面應該有一個型別為 `imageUrl` 的 `string`。
...
你的 `PlayerData` 介面應該有一個型別為 `pac` 的 `number`。
...
你的 `PlayerData` 介面應該有一個型別為 `sho` 的 `number`。
...
你的 `PlayerData` 介面應該有一個型別為 `pas` 的 `number`。
...
你的 `PlayerData` 介面應該有一個型別為 `dri` 的 `number`。
...
你的 `PlayerData` 介面應該有一個型別為 `phy` 的 `number`。
