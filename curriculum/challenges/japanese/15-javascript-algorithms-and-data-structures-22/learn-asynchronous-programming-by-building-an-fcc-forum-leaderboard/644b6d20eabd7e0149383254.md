...
`forumCategory` 関数の中に、`let` を使用して `selectedCategory` という名前の変数を作成し、空のオブジェクトを代入してください。 これは各カテゴリーのカテゴリー名とクラス名を格納するために使います。

# --hints--

`selectedCategory` という名前の変数の宣言には `let` を使用してください。

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

変数 `selectedCategory` には空のオブジェクトを割り当ててください。
