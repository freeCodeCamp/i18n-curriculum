...
為了讓導覽按鈕看起來更像典型的按鈕，請移除錨點標籤的底線。

然後，創建一個新的選擇器，標的為導覽列表元素，當游標暫留在它們上方時，背景色和文字色會互換，且游標會變成指標。

# --hints--

你應該使用現有的 `li > a` 選擇器將 `text-decoration` 設定為 `none`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

你應該使用 `nav > ul > li:hover` 或 `nav li:hover` 選擇器來設定元素在暫留時的樣式。
...
你應該為暫留的 `li` 元素設定 `background-color` 為 `#dfdfe2`。
...
你應該為暫留的 `li` 元素設定 `color` 為 `#1b1b32`。
...
你應該為暫留的 `li` 元素設定 `cursor` 為 `pointer`。
