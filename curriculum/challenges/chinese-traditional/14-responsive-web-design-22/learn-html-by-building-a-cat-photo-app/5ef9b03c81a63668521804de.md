...
Both radio buttons should have a `name` attribute. 請確保在開始標籤的名稱後面要有一個空格，且所有的屬性名稱前面也要有一個空格。
...
Both radio buttons should have a `type` attribute. 請檢查在開始標籤的名稱後面要有一個空格，且/或所有的屬性名稱前面也要有一個空格。
...
Both radio buttons should have a `name` attribute with the value `indoor-outdoor`. 你可能落下了這個值或者有拼寫錯誤。 所有屬性的值都應該放在引號中。
