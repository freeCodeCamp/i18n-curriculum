...
现在，创建 `QuadraticEquation` 类的实例来表示 \\( 11x^2 - x + 1 = 0 \\)。

将新实例分配给 `quadr_eq` 变量，然后打印该变量。 注意：此时，方程的字符串表示形式中缺少二阶项。

# --hints--

你应该改声明一个 `quadr_eq` 变量，并将使用 `11`，`-1` 和 `1` 作为参数实例化的 `QuadraticEquation` 类的实例分配给该变量。
