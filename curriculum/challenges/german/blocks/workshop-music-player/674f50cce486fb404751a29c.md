...
Da Sie jetzt wissen, wie Sie den nächsten abzuspielenden Song finden, erstellen Sie eine Funktion namens `playNextSong`. Innerhalb der `playNextSong`-Funktion erstellen Sie eine `if`-Anweisung, um zu prüfen, ob das `currentSong` von `userData` strikt gleich `null` ist. Damit wird überprüft, ob im `userData`-Objekt gerade kein Song abgespielt wird.

Wenn die Bedingung wahr ist, rufen Sie die `playSong`-Funktion mit dem `id` des ersten Songs im `userData.songs`-Array als Argument auf.

Fügen Sie dann am Ende des `if`-Blocks eine `return`-Anweisung hinzu.

# --hints--

Sie sollten eine Funktion namens `playNextSong` haben.

```js
assert.isFunction(playNextSong);
```

Wenn das `currentSong` von `userData` strikt gleich `null` ist, sollte Ihre `playNextSong`-Funktion die `playSong`-Funktion mit dem `id` des ersten Songs im `userData.songs`-Array als Argument aufrufen.
...
Sie sollten am Ende des `if`-Blocks eine `return`-Anweisung haben.
