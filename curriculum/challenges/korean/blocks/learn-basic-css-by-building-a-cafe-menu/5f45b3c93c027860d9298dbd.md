...
마우스가 링크 위에 올려졌을 때 링크의 속성을 변경하려면 `a:hover { propertyName: propertyValue; }`처럼 생긴 <dfn>의사 선택자</dfn>를 사용하세요.

사용자가 푸터 `Visit our website` 링크 위에 마우스를 올렸을 때 색상을 `brown`로 변경하세요.

# --hints--

`a:hover` 의사 선택자를 사용하세요.
...
`a:hover`에 `color`인 `brown`이 있어야 합니다.
