...
En el bucle anterior, `callable()` es una función incorporada que devuelve `True` si el objeto que se le pasa puede ser llamado como una función o método, y `False` en caso contrario. Al verificar `not callable(getattr(person, attr))`, el bucle omite los métodos y solo imprime los atributos de datos como `name` y `age`.
