...
يجب عليك إضافة عنصر `div` داخل `.eye.left`، ولكن وجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.eye.left > div')?.length ?? 0, 1);
```

يجب عليك إضافة عنصر `div` داخل `.eye.right`، ولكن وجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.eye.right > div')?.length ?? 0, 1);
```

يجب عليك إعطاء أول عنصر `div` الـ `class` بقيمة `eye-lid`.
