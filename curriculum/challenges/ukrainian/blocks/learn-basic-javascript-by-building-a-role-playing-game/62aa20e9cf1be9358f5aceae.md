...
Додайте інструкцію `else` до першої інструкції `if` всередині вашої функції `attack()`. В інструкції `else` використайте оператор `+=`, щоб додати текст `" You miss."` у кінець `text.innerText`.

# --hints--

Ви повинні додати блок `else` після вашого блоку `if (isMonsterHit())`.
...
Ви повинні додати текст `" You miss."` у кінець `text.innerText`. Пам’ятайте використовувати складене присвоєння і переконайтеся, що перед словом `You` є пробіл.
