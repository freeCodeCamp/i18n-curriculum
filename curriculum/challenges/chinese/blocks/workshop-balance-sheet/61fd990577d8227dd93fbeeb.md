...
在第四个 `tr` 元素中，添加一个 `th` 元素，文本为 `Total Liabilities`。 将文本 `Liabilities` 包裹在 `span` 元素中，并将 `class` 属性设置为 `sr-only`。

接着，添加三个 `td` 元素，文本分别为（按顺序）：`$750`、`$600`、`$475`。 给第三个 `td` 元素添加一个 `class` 属性，设置为 `current`。
...
你应该将文本 `Liabilities` 包裹在 `span` 元素中。
