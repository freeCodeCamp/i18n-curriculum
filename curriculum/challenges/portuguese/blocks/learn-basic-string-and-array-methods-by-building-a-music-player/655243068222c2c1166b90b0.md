...
Dentro da função `playNextSong`, crie uma declaração `if` para verificar se a `currentSong` de `userData` é estritamente igual a `null`. Isso verificará se não há nenhuma música tocando atualmente no objeto `userData`. 

Se a condição for verdadeira, chame a função `playSong` com o `id` da primeira música no array `userData?.songs` como argumento.

# --hints--

Você não deve modificar a função `playNextSong` existente e seu conteúdo.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Você deve criar uma declaração `if` com a condição `userData?.currentSong === null`.
...
Você deve chamar a função `playSong` com `userData?.songs[0].id` dentro da declaração `if`.
