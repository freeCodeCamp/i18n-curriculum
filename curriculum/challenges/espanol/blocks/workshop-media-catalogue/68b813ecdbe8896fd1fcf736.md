...
Usa el método `add` de `catalogue` para agregar `series1` a la lista `items`.

Luego, crea otra variable `series2`. Asígnale otra instancia de `TVSeries` y agrega `series2` también al catálogo.

# --hints--

Debes usar el método `add` de `catalogue` pasando `series1`.
...
Debes crear una variable llamada `series2` y asignarle una instancia de `TVSeries`.
...
Debes usar el método `add` de `catalogue` pasando `series2`.
