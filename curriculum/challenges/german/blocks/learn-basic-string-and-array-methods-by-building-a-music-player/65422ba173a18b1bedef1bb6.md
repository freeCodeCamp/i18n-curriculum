...
Fügen Sie innerhalb der Pfeilfunktion des Ereignis-Listeners eine `if` hinzu, um zu prüfen, ob `userData?.currentSong` `null` ist.

Rufen Sie innerhalb des `if`-Blocks die `playSong()`-Funktion mit dem `id` des ersten Songs im `userData?.songs`-Array auf. So wird sichergestellt, dass der erste Song in der Wiedergabeliste zuerst abgespielt wird.
...
Sie sollten die `playSong`-Funktion mit `userData?.songs[0].id` innerhalb Ihres `if`-Blocks aufrufen.
