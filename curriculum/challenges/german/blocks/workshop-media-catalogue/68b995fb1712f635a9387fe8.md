...
Erstellen Sie eine weitere Methode namens `get_tv_series` und lassen Sie sie eine Liste mit allen Instanzen von `TVSeries` zurückgeben, die Sie in `self.items` finden können.

# --hints--

Ihre `MediaCatalogue`-Klasse sollte eine Methode namens `get_tv_series` haben.
...
Ihre `get_tv_series`-Methode sollte eine Liste zurückgeben, die alle Elemente in `self.items` enthält, die Instanzen der `TVSeries`-Klasse sind.
