...
Lenga vipengele vyote vya `label` ndani ya vipengele vya `.info`, na weka `width` yao kuwa `10%`, na yafanye yasichukue chini ya `55px`.
...
Unapaswa kuwapa vipengele vya `label` `width` ya `10%`.
...
Unapaswa kuwapa vipengele vya `label` `min-width` ya `55px`.
