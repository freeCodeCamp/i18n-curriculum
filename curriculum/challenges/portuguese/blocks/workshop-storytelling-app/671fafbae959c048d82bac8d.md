...
Modifique o listener de evento `scaryStoryBtn` para chamar a função `displayStory` com o argumento `"scary"`. Além disso, adicione um listener de evento `click` aos botões `funnyStoryBtn` e `adventureStoryBtn` que chamam a função `displayStory` com os argumentos `"funny"` e `"adventure"` respectivamente.

Com isso, você completou com sucesso o workshop.

# --hints--

Quando você clicar no `scaryStoryBtn`, o parágrafo `#result` deve exibir a história de terror `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` com uma cor de borda `#ee4b2b`.
...
Quando você clicar no `funnyStoryBtn`, o parágrafo `#result` deve exibir a história engraçada com uma cor de borda `#f1be32`.
...
Quando você clicar no `adventureStoryBtn`, o parágrafo `#result` deve exibir a história de aventura com uma cor de borda `#acd157`.
