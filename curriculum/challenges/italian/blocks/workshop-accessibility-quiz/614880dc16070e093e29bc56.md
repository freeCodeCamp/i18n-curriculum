...
Sostituisci il margine superiore degli elementi `h2` con `60px` di spaziatura interna superiore.

# --hints--

Dovresti dare al `h2` una `margin-top` di `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h2')?.marginTop, '0px');
```

Dovresti dare al `h2` una `padding-top` di `60px`.
