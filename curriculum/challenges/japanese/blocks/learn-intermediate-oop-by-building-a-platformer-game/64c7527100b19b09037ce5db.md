...
`draw` メソッド内で、`"#acd157"` を `ctx.fillStyle` に代入してください。

その下で、`ctx.fillRect` メソッドを呼び出し、`x` と `y` の座標、および `width` と `height` のプロパティを渡してください。各プロパティの前に `this` を含めるのを忘れないでください。

# --hints--

`"#acd157"` を `ctx.fillStyle` に代入する必要があります。

```js
assert.match(code, /ctx\.fillStyle\s*=\s*('|")#acd157\1\s*;?/);
```

正しい引数を渡して `ctx.fillRect` メソッドを呼び出してください。
