...
Dein `spreadsheetFunctions`-Objekt sollte eine `random`-Eigenschaft enthalten.

```js
assert.property(spreadsheetFunctions, "random");
```

Deine `random`-Eigenschaft sollte eine Funktion sein.

```js
assert.isFunction(spreadsheetFunctions.random);
```

Deine `random`-Funktion sollte eine zufällige Zahl zwischen den ersten beiden Zahlen im Array zurückgeben.
...
Deine `random`-Funktion sollte eine ganze Zahl (Integer) zurückgeben.
