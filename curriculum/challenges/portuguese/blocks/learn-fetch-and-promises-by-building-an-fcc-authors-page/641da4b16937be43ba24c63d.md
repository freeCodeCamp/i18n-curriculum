...
Encadeie outro `.then()` ao método `.then()` existente. Desta vez, passe `data` como o parâmetro para a função de callback. Para o callback, use chaves porque você terá mais de uma expressão. Dentro da função de callback, registre `data` no console usando console.log() para ver a aparência dos dados.
...
Seu primeiro `.then()` não deve ter ponto e vírgula.
...
Seu segundo `.then()` não deve ter ponto e vírgula após ele.
