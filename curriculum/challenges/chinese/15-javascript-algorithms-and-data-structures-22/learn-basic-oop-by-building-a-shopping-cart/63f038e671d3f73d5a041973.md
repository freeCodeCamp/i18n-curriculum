...
你应该做的第一件事是检查 `items` 数组是否为空。 如果是，则向用户显示 `alert` 和文本 `Your shopping cart is already empty`，然后从函数返回。

请记住 `0` 是一个假值，因此你可以使用 `!` 运算符来检查数组是否为空。

显示 alert 后，从函数返回以停止执行。

# --hints--

你应该创建一个 `if` 语句，使用否定运算符检查 `items` 数组是否为空。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

你的 `if` 语句应该向用户显示 `alert` 和文本 `Your shopping cart is already empty`。
