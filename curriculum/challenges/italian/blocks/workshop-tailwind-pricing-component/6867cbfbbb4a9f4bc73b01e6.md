...
I marker degli elementi della lista devono avere un po' più di spazio. Quindi, assegna a ogni `span` negli elementi della lista le classi `text-green-700` e `mr-2`.

Inoltre, aggiungi l'attributo `aria-hidden` a ciascuno degli elementi `span`.

# --hints--

I tuoi elementi `span` dovrebbero avere la classe `text-green-700`.
