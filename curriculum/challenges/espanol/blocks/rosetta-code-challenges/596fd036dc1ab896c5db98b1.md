...
title: Convierte segundos a duración compuesta
challengeType: 1
forumTopicId: 302236
...
Demuestra que pasa los siguientes tres casos de prueba:

<div style='font-size:115%; font-weight: bold;'>Test Cases</div>

Introduce númeroNúmero de salida
| ------------ | ------------------------------------- |
7259<code>2 hr, 59 sec</code>
86400<code>1 d</code>
6000000<code>9 sem, 6 d, 10 hr, 40 min</code>
...
UnidadSufijo usado en OutputConversión
| ------ | --------------------- | --------------------- |
semana<code>wk</code>1 semana = 7 días
día<code>d</code>1 día = 24 horas
hora<code>hr</code>1 hora = 60 minutos
minuto<code>min</code>1 minuto = 60 segundos
segundo<code>sec</code>| ---                   |
...
`convertSeconds` debería ser una función.

```js
assert(typeof convertSeconds === 'function');
```

`convertSeconds(7259)` debería devolver `2 hr, 59 sec`.

```js
assert.equal(convertSeconds(testCases[0]), results[0]);
```

`convertSeconds(86400)` debería devolver `1 d`.

```js
assert.equal(convertSeconds(testCases[1]), results[1]);
```

`convertSeconds(6000000)` debería devolver `9 wk, 6 d, 10 hr, 40 min`.
