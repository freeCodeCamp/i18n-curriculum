...
此时，你只想显示那些不是起始节点且可从起始节点到达的节点的结果。

添加一个条件式，查看 `node_no` 是否相等于 `start_node` **或** `distances[node_no]` 是否相等于 `INF`。

如果任一条件为真，使用 `continue` 跳过循环的下一次迭代。
...
你的 `if` 语句应查看 `node_no` 是否相等于 `start_node` 或者 `distances[node_no]` 是否相等于 `INF`。
