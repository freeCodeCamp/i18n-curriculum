...
Crie um loop `while` vazio que continue enquanto houver participantes esperando (`currentTickQueue > 0`) e o portão ainda tiver capacidade disponível (`processed < gate.capacity`).

# --hints--

A condição do seu loop `while` deve ser `currentTickQueue > 0` e `processed < gate.capacity`.
