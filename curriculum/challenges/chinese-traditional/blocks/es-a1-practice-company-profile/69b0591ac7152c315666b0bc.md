...
這句話是真還是假：當 Sebastián 問 `¿Cuál es su sitio web?` 時，他是在詢問公司的網站。
...
Sebastián 正在詢問公司的 `sitio web`，意思是「網站」。
...
`¿Cuál...?` 用來詢問特定資訊或身分識別。 例如：

- `¿Cuál es tu nombre?` - 你叫什麼名字？

- `¿Cuál es tu número de teléfono?` - 你的電話號碼是多少？

在對話中，`¿Cuál es su sitio web?` 請求一項特定的資訊：網站。

`Su` 指的是公司（「它的」），而 `es` 是第三人稱單數，因為主詞是 `su sitio web`。
