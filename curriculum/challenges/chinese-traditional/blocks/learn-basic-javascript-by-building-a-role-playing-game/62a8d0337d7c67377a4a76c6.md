...
到目前為止，每次更新 `text.innerText` 時，舊的文字都會被清除。這次，使用 `+=` 運算子將文字添加到 `text.innerText` 的末尾。

為你的字串添加 `" In your inventory you have: "` - 包含開頭和結尾的空格。

# --hints--

你應該為你的 `text.innerText` 添加一個新行。
...
你應該使用 `+=` 運算子為 `text.innerText` 添加內容。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*\+=\s*/)
```

你應該將字串 `" In your inventory you have: "` 添加到 `text.innerText` 的結尾。注意空格！
