...
在這個遊戲階段，你可以擲骰子、做出選擇並進行幾輪遊戲。然而，你應該注意到遊戲沒有結束，且有無限輪數。根據規則，應該總共進行 6 輪，然後遊戲以最終分數結束。 

在使用者選擇分數後執行你的邏輯，應該檢查是否已經進行了 6 輪。如果是，則在 500 毫秒後顯示包含使用者最終分數的警示。 

# --hints--

如果已經進行了 6 輪，你應該向使用者顯示警示。
