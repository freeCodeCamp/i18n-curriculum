...
將 `card_number` 最後四位數字的順序反轉，方法是使用步長為 `-1` 的切片。你可以對起始和結束索引使用負數或正數索引。

# --hints--

你應該在 `verify_card_number` 函式（程式）中有 `card_number_reversed = card_number[-1:-5:-1]`。預期 `--fcc-actual--` 與 `--fcc-expected--` 相等。
