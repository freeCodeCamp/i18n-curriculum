...
Per il tag di apertura `a`, imposta l'attributo `target` su `"_blank"`. Poi, imposta l'attributo `href` su `${forumTopicUrl}${slug}/${id}`. 

E con queste modifiche, il tuo progetto della classifica del forum freeCodeCamp è ora completo!

# --hints--

Il tuo tag àncora di apertura dovrebbe avere un attributo `target` impostato su `"_blank"`.
...
Il tuo tag àncora di apertura dovrebbe avere un attributo `href` impostato su `${forumTopicUrl}${slug}/${id}`.
