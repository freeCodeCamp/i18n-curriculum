...
接著，添加一個 `useEffect` 掛鉤，當 `player` 狀態改變時會執行。在裡面，使用箭頭函式並添加一個 `try..catch`，其 `error` 為 `catch` 區塊的參數。

# --hints--

你應該在 `useEffect` 裡呼叫 `FootballPlayerCard`。
...
你的 `useEffect` 應該有 `[player]` 作為它的相依陣列。

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

你的 `useEffect` 回呼應包含一個 `try...catch` 區塊，該區塊的 `error` 參數為 `catch`。
