...
1. 该表单应包含一个 id 为 `position` 的 `select` 元素，允许用户选择职位。
...
1. 为 `input` 和 `textarea` 元素添加 `:focus` 伪类，以在聚焦时更改它们的边框颜色并移除默认轮廓。
...
1. 为 `input` 元素添加 `:first-of-type` 伪类，以不同方式样式化第一个输入字段。（例如，圆角）。
...
该表单应包含一个 id 为 `position` 的 `select` 元素以及一些 `option` 元素。
...
为 `input` 元素添加 `:first-of-type` 伪类，以不同方式样式化第一个输入字段。
