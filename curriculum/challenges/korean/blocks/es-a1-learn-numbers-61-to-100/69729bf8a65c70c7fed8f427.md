...
이전 섹션에서 스페인어 숫자 `sesenta` (60)에 대해 배웠습니다.

이제 61부터 69까지의 숫자를 배울 차례입니다.

이 숫자들은 10의 자리 숫자 `sesenta` (60)에 접속사 `y` ("그리고")와 1부터 9까지의 일의 자리 숫자를 붙여 만듭니다.
...
숫자 `sesenta y uno` (61)은 이 범위에서 독특한데, 남성 명사 앞에서는 `sesenta y un`로 변합니다. 예를 들어:

`Sesenta y un años` - 61년.
