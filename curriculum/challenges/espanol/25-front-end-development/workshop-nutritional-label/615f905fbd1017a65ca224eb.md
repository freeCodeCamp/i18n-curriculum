...
Agregue un divisor mediano después de su `.daily-value` elemento. Debajo de ese nuevo divisor, cree un elemento `p` con el atributo `class` establecido a `note`.

Dé al elemento `p` el siguiente texto:
...
Deberías crear un nuevo `div` después de tu elemento `.daily-value`.

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

Tu nuevo `div` debe tener el `class` establecido en `divider medium`.
...
Deberías crear un elemento `p` después de tu nuevo elemento `div`.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'p');
```

Tu nuevo elemento `p` debe tener el `class` establecido en `note`.
