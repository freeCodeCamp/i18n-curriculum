...
CSS를 시작하려면 `*`를 포함해 모든 요소와 `::before`, `::after` 의 가상 선택자를 대상으로 CSS 규칙을 표준화하세요.

`padding`와 `margin` 속성을 둘 다 `0`로 설정하고 `box-sizing` 속성은 `border-box`로 설정하세요.
