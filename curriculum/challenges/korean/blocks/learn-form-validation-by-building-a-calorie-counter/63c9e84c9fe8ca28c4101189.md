...
이제 `p` 요소를 만들고 텍스트로 `budgetCalories Calories Budgeted`을 넣으세요. 보간을 사용해 `budgetCalories`를 적절한 변수로 바꾸세요.

이 요소는 `hr` 요소 다음에 와야 합니다.
...
`p` 요소는 텍스트로 `${budgetCalories} Calories Budgeted`을 가져야 합니다.
