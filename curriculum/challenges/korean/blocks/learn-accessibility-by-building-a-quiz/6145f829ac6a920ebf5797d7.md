...
이 프로젝트의 마지막 두 개 의미론적 HTML 요소는 `footer`와 `address` 요소입니다. `footer` 요소는 페이지와 관련된 콘텐츠 모음을 담는 컨테이너이고, `address` 요소는 페이지 작성자의 연락처 정보를 담는 컨테이너입니다.

`main` 요소 다음에 `footer` 요소 하나를 추가하고, 그 안에 `address` 요소 하나를 중첩하세요.

# --hints--

`footer` 요소 다음에 `main` 요소 하나를 추가해야 합니다.

```js
assert.exists(document.querySelector('main + footer'));
```

`address` 요소 안에 `footer` 요소 하나를 중첩해야 합니다.
