...
請記住，在步驟 1 中你選擇了 `Load More Authors` 按鈕並將其分配給 `loadMoreBtn`。

使用 `addEventListener` 將 `"click"` 事件監聽器添加到 `loadMoreBtn`。 此外，傳遞對 `fetchMoreAuthors` 函數的引用，該函數在每次單擊按鈕時運行。

之後，當你單擊按鈕時，你應該會看到另外 8 位作者。
...
你應該在 `loadMoreBtn` 變量上調用 `addEventListener()` 方法。
...
你的事件監聽器應該以 `fetchMoreAuthors` 作爲要運行的函數。
