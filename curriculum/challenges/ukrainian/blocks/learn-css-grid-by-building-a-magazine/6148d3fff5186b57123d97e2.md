...
Цитата не є справжньою цитатою без правильних лапок. Ви можете додати їх за допомогою CSS псевдоселекторів.

Створіть селектор `.quote::before` і встановіть властивість `content` у значення `"` з пробілом після нього.

Також створіть селектор `.quote::after` і встановіть властивість `content` у значення `"` з пробілом перед ним.

# --hints--

У вас має бути селектор `.quote::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

Ваш селектор `.quote::before` має мати властивість `content`, встановлену в `'" '`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content?.match(/\"\\"\s\"/));
```

У вас має бути селектор `.quote::after`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

Ваш селектор `.quote::after` має мати властивість `content`, встановлену в `' "'`.
