...
Не хвилюйтеся через пробіл унизу, все буде опущено вниз пізніше, коли ви додасте висоту елементу вгорі будівлі.

Додайте `repeating-linear-gradient` до `.fb1c` з кутом `90deg`, ваш `--building-color4` від `0%` до `10%` і `transparent` від `10%` до `15%`.
