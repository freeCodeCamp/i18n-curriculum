...
Il selettore `.text` dovrebbe avere una proprietà`font-size` impostata a `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize === '1.8rem');
```

Il selettore `.text` dovrebbe avere una proprietà `letter-spacing` impostata a `0.6px`.
