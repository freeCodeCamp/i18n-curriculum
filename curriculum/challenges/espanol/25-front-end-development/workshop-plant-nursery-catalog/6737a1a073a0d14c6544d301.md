...
Declare una variable denominada `plantsSet` y asígnele una llamada a la función `displayPlantsSet`.

Luego registre `plantsSet` en la consola. Necesitará abrir la consola del navegador para ver su conjunto.


# --hints--

Debe tener una variable denominada `plantsSet`.

```js
assert.exists(plantsSet);
```

Debe llamar a `displayPlantsSet` y asignarlo a `plantsSet`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

Debe registrar `plantsSet` en la consola.
