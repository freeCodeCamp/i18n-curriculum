...
調用 `valid_in_square()` 方法（以 `row`、`col` 和 `num` 作爲參數）檢查該數字是否適合插入到指定的 3x3 正方形中，並將結果分配給變量 `valid_in_square`。

# --hints--

你應該聲明一個變量 `valid_in_square` 並給它賦值 `self.valid_in_square(row, col, num)` 。
