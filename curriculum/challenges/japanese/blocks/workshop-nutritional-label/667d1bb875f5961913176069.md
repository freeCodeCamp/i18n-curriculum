...
プレビューでテキスト `8g` が中央に配置されているのに気づきましたか。テキスト `span` を含む `Total Fat` 要素とテキスト `8g` を、整列のためにさらに `span` 要素にネストしてください。

# --hints--

コード `span` の周りに `<span class="bold">Total Fat</span> 8g` 要素があるはずです。
