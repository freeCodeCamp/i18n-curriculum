...
Si quieres eliminar un elemento de una lista, puedes usar el método `remove()`. Aquí tienes un ejemplo de cómo usar el método `remove()` para eliminar un número duplicado de una lista `numbers`:
