...
- `V4 - 2016`: `Expandimos el currículo a 3 certificaciones, que incluyen Front End, Back End y Visualización de Datos. Cada uno de ellos tenía 10 proyectos requeridos, pero solo la sección de Front End tenía sus propios desafíos. Para las otras certificaciones, aún estábamos utilizando recursos externos como Node School.`
- `V5 - 2017`: `We added the back end and data visualization challenges.`
- `V6 - 2018`: `Lanzamos 6 nuevas certificaciones para reemplazar las antiguas. Esta fue la mayor mejora del currículo hasta la fecha.`

# --hints--

Su elemento `ul` debe tener seis elementos `li`.

```js
assert(document.querySelectorAll('.lists li')?.length === 6);
```

Cada uno de sus nuevos elementos `li` debe tener un elemento `h4` y `p`.
...
Su primer `h4` debe tener el texto `V1 - 2014`.

```js
assert(document.querySelectorAll('.lists li h4')?.[0]?.innerText === 'V1 - 2014');
```

Tu primera `p` debe tener el texto `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
Su segundo `h4` debe tener el texto `V2 - 2015`.

```js
assert(document.querySelectorAll('.lists li h4')?.[1]?.innerText === 'V2 - 2015');
```

Su segundo `p` debe tener el texto `We added interactive algorithm challenges.`

```js
assert(document.querySelectorAll('.lists li p')?.[1]?.innerText === 'We added interactive algorithm challenges.');
```

Su tercer `h4` debe tener el texto `V3 - 2015`.

```js
assert(document.querySelectorAll('.lists li h4')?.[2]?.innerText === 'V3 - 2015');
```

Su tercera `p` debe tener el texto `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`
...
Su cuarto `h4` debe tener el texto `V4 - 2016`.

```js
assert(document.querySelectorAll('.lists li h4')?.[3]?.innerText === 'V4 - 2016');
```

Tu cuarto `p` debería tener el texto `We expanded the curriculum to 3 certifications, including Front End, Back End, and Data Visualization. Cada uno tenía 10 proyectos requeridos, pero solo la sección de Front End tenía sus propios desafíos. Para las otras certificaciones, aún estábamos utilizando recursos externos como Node School.`
...
Tu quinto `h4` debe tener el texto `V5 - 2017`.

```js
assert(document.querySelectorAll('.lists li h4')?.[4]?.innerText === 'V5 - 2017');
```

`Su quinto <code>p` debe tener el texto `We added the back end and data visualization challenges.`
...
Tu sexto `p` debería tener el texto `We launched 6 new certifications to replace our old ones. Esta fue la mayor mejora del currículo hasta la fecha.`
