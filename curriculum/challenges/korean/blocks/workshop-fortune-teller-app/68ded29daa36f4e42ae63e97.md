...
`forEach` 배열 메서드를 사용해 elements 인수를 순회하면서 함수를 마무리하세요. 콜백 함수 내부에서 `.add()`의 `classList` 메서드를 사용해 각 요소에 hidden 클래스를 추가하세요.

# --hints--

`forEach` 매개변수에 `elements` 메서드가 붙어 있어야 합니다.
...
forEach 메서드는 `el`라는 매개변수를 가져야 합니다.
...
forEach 메서드 내부에서 `classList.add` 메서드를 사용해 hidden 클래스를 추가해야 합니다.
