...
يجب عليك استخدام منتقي `.eye.left`.

```js
assert.match(code, /\.eye\.left\s*\{/);
```

يجب أن تعطي عنصر `.eye.left` خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye.left')?.left, '25%');
```

يجب عليك استخدام منتقي `.eye.right`.

```js
assert.match(code, /\.eye\.right\s*\{/);
```

يجب أن تعطي عنصر `.eye.right` خاصية `right` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
