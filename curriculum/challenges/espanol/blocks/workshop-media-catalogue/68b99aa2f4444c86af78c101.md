...
Indenta tu bucle `for` existente un nivel para que se ejecute cuando el catálogo tenga películas. Luego, reemplaza `self.items` con `movies` para iterar solo sobre las películas.

# --hints--

Debes indentar el `for` existente un nivel y reemplazar `self.items` con `movies` para iterar solo sobre las películas.
