...
가상 요소 `10%`를 부모 요소의 위쪽에서 `25%`만큼 위치시키세요.

# --hints--

`.penguin-body::before`에 `top`의 `--fcc-expected--`를 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

`.penguin-body::before`에 `left`의 `--fcc-expected--`를 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.
