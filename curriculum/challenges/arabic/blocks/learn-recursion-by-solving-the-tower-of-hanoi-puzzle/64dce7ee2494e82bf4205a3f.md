...
الحالة الأخرى التي يجب عليك فيها نقل القرص بالضرورة من `source` إلى `target` هي عندما لا تكون قائمة `source` فارغة وقرصها الأخير أصغر من القرص الأخير في `target`.

أضف عبارة `elif` للتحقق من هذا الشرط. ثم عيّن المتغير `forward` إلى `True` إذا تحقق الشرط.

# --hints--

يجب أن تحتوي على عبارة `elif` للتحقق مما إذا كانت `rods[source]` غير فارغة و`rods[source][-1] < rods[target][-1]`.
...
يجب تعيين `forward` إلى `True` داخل عبارة `elif` الجديدة. لا تغير التعيينات السابقة لـ `forward`.
