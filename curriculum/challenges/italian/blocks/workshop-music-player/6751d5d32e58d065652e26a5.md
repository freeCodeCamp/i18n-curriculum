...
Ora devi aggiungere l'attributo `aria-current` alla canzone attualmente in riproduzione.

Controlla se `songToHighlight` esiste. In tal caso, usa il metodo `setAttribute` su `songToHighlight` e passa `"aria-current"` e `"true"` come primo e secondo argomento.

# --hints--

`highlightCurrentSong` dovrebbe aggiungere l'attributo `aria-current` alla canzone attualmente in riproduzione e rimuovere l'attributo dalla canzone riprodotta in precedenza.
