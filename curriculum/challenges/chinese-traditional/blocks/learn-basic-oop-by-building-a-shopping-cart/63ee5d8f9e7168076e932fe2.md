...
在你的 `h2` 元素之後，創建兩個 `p` 元素。為第一個元素添加 `class` 為 `dessert-price`，並將文字設為美元符號 `"$"`，後面接著 `price` 變數的值。為第二個元素添加 `class` 為 `product-category`，並將文字設為 `"Category: "`，後面接著 `category` 變數的值。
