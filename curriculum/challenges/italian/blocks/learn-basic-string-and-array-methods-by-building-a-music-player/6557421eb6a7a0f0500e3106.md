...
All'interno dell'elemento button nella funzione `renderSongs`, aggiungi un attributo `onclick`. Per il valore, chiama la funzione `deleteSong` e interpolare `song.id`.

# --hints--

Dovresti aggiungere un attributo `onclick` al pulsante di eliminazione e passare `"deleteSong(${song.id})"`.
