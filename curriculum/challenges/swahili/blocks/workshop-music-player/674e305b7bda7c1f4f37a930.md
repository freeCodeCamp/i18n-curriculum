...
Kabla hujaanza kufanya kazi ya kucheza wimbo unaofuata na uliotangulia, unahitaji kupata kielezo cha kila wimbo katika sifa ya `songs` ya `userData`. Ili kufanya hivyo unaweza kutumia njia ya safu ya data `indexOf()`, ambayo hurudisha kielezo cha kwanza ambacho kipengele fulani kinaweza kupatikana katika safu ya data, au `-1` ikiwa kipengele hakipo.
...
Tengeneza kitendakazi kinachoitwa `getCurrentSongIndex` kinachorudisha kielezo ambacho `userData.currentSong` kinaweza kupatikana katika `userData.songs` na `-1` wakati wimbo wa sasa ni `null`.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Kitendakazi chako cha `getCurrentSongIndex` kinapaswa kurudisha kielezo ambacho `userData.currentSong` kinaweza kupatikana katika `userData.songs`.
...
Kitendakazi chako cha `getCurrentSongIndex` kinapaswa kurudisha `-1` wakati `userData.currentSong` ni `null`.
