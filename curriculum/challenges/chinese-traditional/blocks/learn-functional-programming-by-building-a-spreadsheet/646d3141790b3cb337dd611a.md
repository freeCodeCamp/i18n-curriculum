...
回呼函式應該為正規表達式中的每個捕獲群組帶有一個參數。在你的情況中，`rangeRegex` 有四個捕獲群組：第一個字母、第一組數字、第二個字母和第二組數字。

為你的回呼函式添加四個參數以對應那些捕獲群組：`char1`、`num1`、`char2` 和 `num2`。`char` 是 `character` 的縮寫。

# --hints--

你的回呼函式（程式）應該將 `char1` 作為第二個參數。
