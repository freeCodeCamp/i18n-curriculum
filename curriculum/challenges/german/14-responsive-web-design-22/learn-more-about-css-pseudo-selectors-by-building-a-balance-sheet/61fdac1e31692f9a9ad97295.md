...
Die `clip-path`-Eigenschaft bestimmt die Form, die die `clip`-Eigenschaft annehmen soll. Setze die `clip-path`-Eigenschaft auf den Wert `inset(50%)`, wodurch innerhalb des Elements der clip-path zu einem Rechteck geformt wird.
