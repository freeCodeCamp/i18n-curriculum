...
The `controls` attribute enables users to manage audio playback, including pausing or resuming playback. `controls` 屬性是一個布爾屬性，可添加到元素中以啓用內置播放控制。 如果省略，則不會顯示任何控件。
