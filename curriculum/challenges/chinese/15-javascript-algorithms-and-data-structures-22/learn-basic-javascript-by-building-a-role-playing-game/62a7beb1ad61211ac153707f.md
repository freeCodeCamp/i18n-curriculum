...
现在，添加一行，将 `button2` 的文本更新为 `"Buy weapon (30 gold)"`，并将 `button3` 的文本更新为 `"Go to town square"`。

# --hints--

你应该使用点符号来访问 `button2` 的 `innerText` 属性。

```js
assert.match(code, /button2\.innerText/);
```

你不应使用 `let` 或 `const` 来访问 `button2` 的 `innerText` 属性。

```js
assert.notMatch(code, /(let|const)\s+button2\.innerText/);
```

你应该将 `button2` 的 `innerText` 属性更新为 `"Buy weapon (30 gold)"`。
...
你应该使用点符号来访问 `button3` 的 `innerText` 属性。

```js
assert.match(code, /button3\.innerText/);
```

你不应使用 `let` 或 `const` 来访问 `button3` 的 `innerText` 属性。

```js
assert.notMatch(code, /(let|const)\s+button3\.innerText/);
```

你应该将 `button3` 的 `innerText` 属性更新为 `"Go to town square"`。
