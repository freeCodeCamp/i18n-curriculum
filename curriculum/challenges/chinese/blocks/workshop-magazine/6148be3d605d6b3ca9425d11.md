...
副标题也需要设置样式。 创建一个 `.hero-subtitle` 选择器，给他一个设置为 `2.4rem` 的 `font-size` 属性、一个设置为 `orangered` 的 `color` 属性和一个设置为 `center` 的 `text-align` 属性。

# --hints--

你应该有一个 `.hero-subtitle` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

你的 `.hero-subtitle` 选择器应该有一个 `font-size` 属性，设置为 `2.4rem`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize, '2.4rem');
```

你的 `.hero-subtitle` 选择器应该有一个 `color` 属性，设置为 `orangered`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color, 'orangered');
```

你的 `.hero-subtitle` 选择器应该有一个 `text-align` 属性，设置为 `center`。
