...
正規表現には、バックスラッシュ (`\`) に続く文字からなる特殊なシーケンスが含まれることがあります。これらのシーケンスは特別な意味を持ちます。例えば、`\d` は10進数の桁にマッチします。
...
文字 `p` の後には、`patient_id` に数字の連続が必要です。したがって、正規表現パターンを文字 `p` に続けて特殊なシーケンス `\d` を含むように変更してください。

# --hints--

正規表現パターンを `p\d` に変更してください。
