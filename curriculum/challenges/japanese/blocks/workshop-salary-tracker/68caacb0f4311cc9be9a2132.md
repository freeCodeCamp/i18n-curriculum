...
これから`salary`セッターのコーディングにフォーカスします。`salary`ゲッターの後に、`salary`プロパティのシンプルなセッターを作成してください。このセッターは、メソッドに渡された引数の値を`self._salary`に設定します。その後、`Salary updated to $`に続けて新しい給与を出力し、句点を付けてください。

新しい給与の検証は次のいくつかのステップで行います。

# --hints--

あなたの`Employee`クラスには、`salary`でデコレートされた`@salary.setter`メソッドが必要です。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].has_decorators("salary.setter")`)) })
```

あなたの`salary`セッターは、引数として渡された値を`self._salary`に設定する必要があります。
...
あなたの`salary`セッターは、`Salary updated to $`に続けて新しい給与を出力し、句点を付けてください。
