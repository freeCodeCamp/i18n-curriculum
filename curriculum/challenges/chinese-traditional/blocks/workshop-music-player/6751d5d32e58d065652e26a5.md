...
現在，你需要為目前播放的歌曲添加 `aria-current` 屬性。

檢查是否存在 `songToHighlight`。如果存在，對 `songToHighlight` 使用 `setAttribute` 方法，並將 `"aria-current"` 和 `"true"` 作為第一和第二個引數傳入。 

# --hints--

`highlightCurrentSong` 應該為目前播放的歌曲添加 `aria-current` 屬性，並從先前播放的歌曲移除該屬性。
