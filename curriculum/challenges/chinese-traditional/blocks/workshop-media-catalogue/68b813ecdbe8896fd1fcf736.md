...
使用 `catalogue` 的 `add` 方法將 `series1` 附加到 `items` 列表。

然後，創建另一個變數 `series2`。將另一個 `TVSeries` 實體指定給它，並將 `series2` 也附加到目錄中。

# --hints--

你應該使用 `catalogue` 的 `add` 方法，並傳入 `series1`。
...
你應該創建一個名為 `series2` 的變數，並將一個 `TVSeries` 的實體指定給它。
...
你應該使用 `catalogue` 的 `add` 方法，並傳入 `series2`。
