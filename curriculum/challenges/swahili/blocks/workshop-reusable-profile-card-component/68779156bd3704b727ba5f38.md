...
Kama unavyokumbuka kutoka mafundoni ya awali, unaweza kutumia njia ya `map` kurudia kupitia vitu tofauti na kuonyesha sehemu kwa njia ya mabadiliko kama hii:
...
Tumia njia ya `map()` kuonyesha `Card` moja kutoka kwenye safu ya data. Pitisha `name`, `title`, na `bio` kama vigezo vinavyopitishwa kwa sehemu ya `Card`.

# --hints--

Unapaswa kutumia njia ya `map()` kurudia kupitia safu ya data ya `profiles`.

```js
assert.match(code, /profiles\.map\s*\(/);
```

Unapaswa kupitisha vigezo vinavyopitishwa vya `name`, `title`, na `bio` kutoka kwenye kitu cha `profile`.
