...
由於用戶可以隨機播放和刪除播放列表中的歌曲，因此你需要創建 `allSongs` 數組的副本而不改變原始數組。 這時 `spread` 操作符就派上用場了。

擴展運算符（`...`）允許你將一個數組中的所有元素複製到另一個數組中。 它還可用於將多個數組連接成一個。 在下面的例子中，`arr1` 和 `arr2` 都已擴展到 `combinedArr`：
...
在 `userData` 對象內部創建 `songs` 屬性。 對於值，將 `allSongs` 展開到數組中。

# --hints--

你的 `userData` 對象應該有一個名爲 `songs` 的鍵。

```js
assert.property(userData, "songs");
```

你的 `songs` 屬性應具有 `[...allSongs]` 的值。
