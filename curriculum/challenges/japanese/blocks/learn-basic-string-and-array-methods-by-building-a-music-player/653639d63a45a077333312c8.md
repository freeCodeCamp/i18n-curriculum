...
曲を再生する前に、最初から始まるようにする必要があります。これは、`currentTime`オブジェクトの`audio`プロパティを使うことで実現できます。

`if`文を追加して、`userData?.currentSong`が偽値であるか、または`userData?.currentSong.id`が厳密に`song.id`と等しくないかを確認してください。この条件は、現在曲が再生されていないか、再生しようとしている曲が現在の曲と異なるかをチェックします。

`if`ブロック内で、`currentTime`オブジェクトの`audio`プロパティを`0`に設定してください。

# --hints--

`if`の条件で`userData?.currentSong === null || userData?.currentSong.id !== song.id`文を作成してください。オプショナルチェーンを忘れずに含めてください。
...
`audio.currentTime`ブロック内で`0`を`if`に設定してください。
