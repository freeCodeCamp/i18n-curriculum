...
Agora, no arquivo JavaScript, crie uma variável `happyBtn`.

Use `querySelector` para armazenar uma referência ao elemento `#happy-btn` na variável `happyBtn`.

# --hints--

Você deve ter uma variável chamada `happyBtn`.

```js
assert.exists(happyBtn);
```

A variável `happyBtn` deve conter uma referência ao elemento `#happy-btn`.
