...
Dentro do elemento `div`, chame a função `avatars` e passe os argumentos `posters` e `users`.

Agora você deve ver os avatares exibidos na página.

# --hints--

Você deve chamar a função chamada `avatars`.
...
A chamada da sua função deve ter os argumentos `posters` e `users`.
