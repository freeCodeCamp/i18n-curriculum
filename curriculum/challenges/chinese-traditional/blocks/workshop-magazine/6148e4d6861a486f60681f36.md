...
為 `.image-wrapper` 選擇器添加 `grid-template-columns` 屬性並設定為 `2fr 1fr`，以及添加 `grid-template-rows` 屬性並設定為 `repeat(3, min-content)`。這將使我們的網格列根據內容調整高度，但行則根據容器保持固定寬度。

# --hints--

你的 `.image-wrapper` 選擇器應該有一個 `grid-template-columns` 屬性設定為 `2fr 1fr`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

你的 `.image-wrapper` 選擇器應該有一個 `grid-template-rows` 屬性設定為 `repeat(3, min-content)`。
