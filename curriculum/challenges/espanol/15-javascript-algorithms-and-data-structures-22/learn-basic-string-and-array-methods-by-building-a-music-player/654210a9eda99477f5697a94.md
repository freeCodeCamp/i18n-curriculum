...
Crea un elemento `button` con la clase `playlist-song-info`. Dentro del `button`, agrega un elemento `span` con la clase `playlist-song-title`, luego intercala `song.title` como el texto.

# --hints--

Debes crear un `button` con la clase `playlist-song-info`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

Debes crear un elemento `span` dentro de tu elemento `button`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

Tu elemento `span` debe tener la clase `playlist-song-title`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

Debes intercalar `song.title` como el contenido de tu elemento `span`.
