...
Ora che il layout della rivista è completato, devi renderlo responsivo.

Inizia con una query `@media` per `only screen` con una `max-width` di `720px`. All’interno, crea un selettore `.image-wrapper` e assegna a esso una proprietà `grid-template-columns` di `1fr`.

Questo farà sì che le tre immagini si riducano a una colonna su schermi più piccoli.

# --hints--

Dovresti avere una nuova regola `@media` per `only screen and (max-width: 720px)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

La tua nuova regola `@media` dovrebbe avere un selettore `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

Il tuo nuovo selettore `.image-wrapper` dovrebbe avere una proprietà `grid-template-columns` di `1fr`.
