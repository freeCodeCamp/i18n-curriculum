...
いい感じです！次は前景の建物に進みましょう！`.fb1` の建物を3つのセクションに分けるために、その中に3つの新しい `div` 要素をネストしてください。順番に `fb1a`、`fb1b`、`fb1c` のクラスを付けてください。

# --hints--

`div` の中に3つの `.fb1` 要素を追加してください。

```js
assert.lengthOf(document.querySelector("div.fb1")?.children, 3);
```

最初の新しい `div` に `fb1a` のクラスを付けてください。

```js
assert.exists(document.querySelector("div.fb1 > div.fb1a"));
```

2番目の新しい `div` に `fb1b` のクラスを付けてください。

```js
assert.exists(document.querySelector("div.fb1 > div.fb1b"));
```

3番目の新しい `div` に `fb1c` のクラスを付けてください。

```js
assert.exists(document.querySelector("div.fb1 > div.fb1c"));
```

新しい `div` 要素は `.fb1a + .fb1b + .fb1c` の順番で配置してください。
