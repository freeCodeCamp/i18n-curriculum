...
Inserisci ciascuno dei tuoi valori già definiti della proprietà `grid-template-columns` in una funzione `minmax`, usando ogni valore come secondo argomento. Il primo argomento dovrebbe essere `2rem`, `min-content` e `2rem` rispettivamente.

# --hints--

Il selettore `main` dovrebbe avere una proprietà `grid-template-columns` impostata su `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
