...
次に再生する曲の見つけ方がわかったので、`playNextSong` という名前の関数を作成してください。`playNextSong` 関数の中で、`if` 文を作成し、`currentSong` の `userData` が `null` と厳密に等しいかどうかをチェックしてください。これは `userData` オブジェクトに現在再生中の曲がないかどうかを確認します。

もし条件が真であれば、`playSong` 配列の最初の曲の `id` を引数として `userData.songs` 関数を呼び出してください。

次に、`if` ブロックの最後に `return` 文を追加してください。

# --hints--

`playNextSong` という名前の関数があるはずです。

```js
assert.isFunction(playNextSong);
```

`currentSong` の `userData` が `null` と厳密に等しい場合、`playNextSong` 関数は `playSong` 配列の最初の曲の `id` を引数として `userData.songs` 関数を呼び出すべきです。
...
`if` ブロックの最後に `return` 文があることを確認してください。
