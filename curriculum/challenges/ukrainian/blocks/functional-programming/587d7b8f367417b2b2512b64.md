...
title: Реалізувати метод filter на прототипі
challengeType: 1
forumTopicId: 301231
...
Ви можете багато дізнатися про метод `filter`, якщо реалізуєте власну його версію. Рекомендується використовувати цикл `for` або `Array.prototype.forEach()`.

# --instructions--

Напишіть власний `Array.prototype.myFilter()`, який має працювати точно так само, як `Array.prototype.filter()`. Не слід використовувати вбудований метод `filter`. Екземпляр `Array` можна отримати у методі `myFilter` за допомогою `this`.
