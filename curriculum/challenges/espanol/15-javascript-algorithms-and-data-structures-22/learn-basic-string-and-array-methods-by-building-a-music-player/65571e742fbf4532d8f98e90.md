...
Necesitas volver a renderizar las canciones, resaltarlo y ajustar el texto accesible del botón de reproducción ya que la lista de canciones cambiará.

Llama a la función `renderSongs` y pasa el arreglo `userData?.songs` como argumento, esto muestra la lista de reproducción modificada.

Después de eso, llamar a la función `highlightCurrentSong` para resaltar la canción actual si hay alguna también y la función `setPlayButtonAccessibleText` para actualizar el texto accesible del botón.

# --hints--

No debe modificar la función `deleteSong` y su contenido.
...
Debes llamar a la función `renderSongs` con `userData?.songs`.
