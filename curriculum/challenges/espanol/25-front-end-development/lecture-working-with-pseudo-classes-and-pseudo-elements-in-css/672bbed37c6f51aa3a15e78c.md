...
¿Qué son los pseudo-elementos y cómo funcionan?
...
En el CSS, utilizaremos la posición absoluta y el pseudo-elemento `::before` para agregar una estrella antes del texto del botón `Learn More`. Aprenderá más sobre posicionamiento absoluto en videos futuros.
...
En el pseudo-elemento `::after`, la propiedad `transition` se utiliza para animar cambios durante 0.3 segundos con un efecto de suavizado, creando una transformación suave y gradual en lugar de una abrupta. Aprenderás más sobre la propiedad `transition` en videos futuros.
...
Eso es lo que hace la propiedad `transform`: te permite rotar, inclinar, escalar o trasladar un elemento en una dirección particular. Aprenderás más sobre eso en videos futuros.
...
En este video de la conferencia, hemos cubierto solo algunos pseudo-elementos. Pero hay muchos más como `::placeholder`, `::spelling-error` y `::selection` que te animo a explorar por tu cuenta.
