...
Crea un selettore `.author-name a:hover` e dagli una proprietà `background-color` impostata su `#306203`.

Questo creerà un effetto al passaggio del mouse solo per l'elemento `a` all'interno del `.author-name`, mostrando il verde originale di freeCodeCamp come sfondo.

# --hints--

Dovresti avere un selettore `.author-name a:hover`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

Il tuo selettore `.author-name a:hover` dovrebbe avere una proprietà `background-color` impostata su `#306203`.
