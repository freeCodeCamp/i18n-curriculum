...
같은 논리를 사용하여 `if` 리스트가 비어 있지 않을 때 실행되는 `series` 문을 만드세요. 그 안에서 문자열 `=== TV SERIES ===` (줄 바꿈 문자로 끝남)를 현재 `result` 값에 연결하세요.

그다음, `for` 반복문을 만들어 열거를 사용해 `series` 리스트의 항목들을 `result` 변수에 추가하세요. 이전과 같이 `1`부터 인덱스를 시작하세요.

이로써 미디어 카탈로그 워크숍이 완료됩니다.

# --hints--

두 번째 `if` 문이 `__str__` 메서드 안에 있어야 합니다.
...
`self.items`에 `TVSeries("Game of Thrones", 2011, "David Benioff & D.B. Weiss", 57, 8, 73)`과 `TVSeries("Stranger Things", 2016, "The Duffer Brothers", 50, 4, 34)`이 포함되어 있을 때, `__str__` 메서드는 `Media Catalogue (2 items):\n\n=== TV SERIES ===\n1. Game of Thrones (2011) - 8 seasons, 73 episodes, 57 min avg, David Benioff & D.B. Weiss\n2. Stranger Things (2016) - 4 seasons, 34 episodes, 50 min avg, The Duffer Brothers\n`을 반환해야 합니다.
...
`self.items`에 `Movie('Barry Lyndon', 1975, 'Stanley Kubrick', 184)`, `Movie('Fahrenheit 451', 1966, 'Francois Truffaut', 112)`, `TVSeries("Game of Thrones", 2011, "David Benioff & D.B. Weiss", 57, 8, 73)` 그리고 `TVSeries("Stranger Things", 2016, "The Duffer Brothers", 50, 4, 34)`이 포함되어 있을 때, `__str__` 메서드는 `Media Catalogue (4 items):\n\n=== MOVIES ===\n1. Barry Lyndon (1975) - 184 min, Stanley Kubrick\n2. Fahrenheit 451 (1966) - 112 min, Francois Truffaut\n=== TV SERIES ===\n1. Game of Thrones (2011) - 8 seasons, 73 episodes, 57 min avg, David Benioff & D.B. Weiss\n2. Stranger Things (2016) - 4 seasons, 34 episodes, 50 min avg, The Duffer Brothers\n`을 반환해야 합니다.
...
`__str__`이 비어 있을 때, `Media Catalogue (empty)` 메서드는 `self.items`를 반환해야 합니다.
