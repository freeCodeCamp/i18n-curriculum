...
Al igual que las funciones regulares, las funciones flecha pueden aceptar múltiples parámetros. 

Aquí tienes un ejemplo de una función flecha nombrada con un parámetro: 
...
Si la función solo tiene un parámetro, puedes omitir los paréntesis alrededor de la lista de parámetros de esta manera:
...
Crea una nueva función flecha nombrada llamada `printMessage` que tenga un parámetro llamado `org`. Dentro del cuerpo de esa función, agrega una sentencia de consola. Dentro de esa sentencia de consola, agrega el literal de plantilla `${org} es increíble!`. 

Debajo de tu función `printMessage`, llama a la función y pasa la cadena `"freeCodeCamp"` como argumento.

Abre la consola para ver el resultado. 

# --hints--

Debes tener una función `printMessage`.

```js
assert.isFunction(printMessage);
```

Debes usar la sintaxis de función flecha.

```js
assert.match(code, /(let|const)\s+printMessage\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Tu función `printMessage` debe tener un parámetro llamado `org`.
...
Tu función `printMessage` debe tener una sentencia de consola.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

Tu sentencia de consola debe incluir el literal de plantilla `${org} es increíble!`.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

Debes llamar a la función `printMessage` y pasar la cadena `"freeCodeCamp"`.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

Tu llamada a la función debe estar debajo de la definición de tu función `printMessage`.
...
Tu llamada a la función `printMessage` debería mostrar `freeCodeCamp es increíble!` en la consola. 
