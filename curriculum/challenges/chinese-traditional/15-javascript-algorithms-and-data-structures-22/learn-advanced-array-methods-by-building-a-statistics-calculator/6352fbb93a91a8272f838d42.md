...
下一步是對每一個差值進行平方運算。 要對一個值進行平方運算，你可以使用 `**` 運算符。 例如，`3 ** 2` 將返回 `9`。

聲明一個 `squaredDifferences` 變量，並給它賦值 `differences.map()`。 在回調中，返回 `el` 的平方值。

# --hints--

你應該有一個 `squaredDifferences` 變量。

```js
assert.match(getVariance.toString(), /squaredDifferences\s*=/);
```

你的 `squaredDifferences` 變量應該使用 `differences.map()` 方法。

```js
assert.match(getVariance.toString(), /squaredDifferences\s*=\s*differences\.map\(/);
```

你的 `squaredDifferences` 變量應該使用回調函數中的 `el` 參數。

```js
assert.match(getVariance.toString(), /squaredDifferences\s*=\s*differences\.map\(\s*function\s*\(\s*el\s*\)/);
```

你的 `map` 回調應該返回 `el` 的平方值。
