...
Erstellen Sie innerhalb Ihres `ul`-Elements sechs `li`-Elemente. Fügen Sie jedem Ihrer `h4`-Elemente ein `class`-Element mit einem `list-subtitle`, das auf `p` gesetzt ist, und ein `li`-Element hinzu.

Geben Sie dann den `h4`- und `p`-Elementen den folgenden Textinhalt in der Reihenfolge, wobei jedes `h4` den linken Teil vor dem Doppelpunkt verwendet und jedes `p` den rechten Teil:
...
Ihr `ul`-Element sollte sechs `li`-Elemente enthalten.

```js
assert.lengthOf(document.querySelectorAll('.lists li'), 6);
```

Jedes Ihrer neuen `li`-Elemente sollte ein `h4`- und ein `p`-Element haben.
...
Ihr erstes `h4` sollte den Text `V1 - 2014` enthalten.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

Ihr erstes `p` sollte den Text `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.` enthalten.
...
Ihr zweites `h4` sollte den Text `V2 - 2015` enthalten.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[1]?.innerText, 'V2 - 2015');
```

Ihr zweites `p` sollte den Text `We added interactive algorithm challenges.` enthalten.

```js
assert.equal(document.querySelectorAll('.lists li p')?.[1]?.innerText, 'We added interactive algorithm challenges.');
```

Ihr drittes `h4` sollte den Text `V3 - 2015` enthalten.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[2]?.innerText, 'V3 - 2015');
```

Ihr drittes `p` sollte den Text `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).` enthalten.
...
Ihr viertes `h4` sollte den Text `V4 - 2016` enthalten.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[3]?.innerText, 'V4 - 2016');
```

Ihr viertes `p` sollte den Text `We expanded the curriculum to 3 certifications, including Front-End, Back-End, and Data Visualization. They each had 10 required projects, but only the Front-End section had its own challenges. For the other certs, we were still using external resources like Node School.` enthalten.
...
Ihr fünftes `h4` sollte den Text `V5 - 2017` enthalten.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[4]?.innerText, 'V5 - 2017');
```

Ihr fünftes `p` sollte den Text `We added the back-end and data visualization challenges.` enthalten.

```js
assert.equal(document.querySelectorAll('.lists li p')?.[4]?.innerText, 'We added the back-end and data visualization challenges.');
```

Ihr sechstes `h4` sollte den Text `V6 - 2018` enthalten.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[5]?.innerText, 'V6 - 2018');
```

Ihr sechstes `p` sollte den Text `We launched 6 new certifications to replace our old ones. This was the biggest curriculum improvement to date.` enthalten.
...
Ihre sechs `h4`-Elemente sollten jeweils die Klasse `list-subtitle` haben.
