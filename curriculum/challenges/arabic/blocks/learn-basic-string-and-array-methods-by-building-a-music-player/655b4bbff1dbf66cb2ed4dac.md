...
استخدم جملة `if` للتحقق مما إذا كان `nextSongExists` موجودًا، ثم نفذ استدعاء الدالة `playNextSong()` داخل كتلة الـ `if`. سيؤدي هذا إلى تشغيل الأغنية التالية تلقائيًا عند انتهاء الأغنية الحالية.

# --hints--

يجب أن تنشئ جملة `if` بشرط `nextSongExists`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*/)
```

يجب أن تنفذ استدعاء الدالة `playNextSong` داخل جملة الـ `if`.
