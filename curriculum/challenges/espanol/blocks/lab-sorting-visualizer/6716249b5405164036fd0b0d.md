...
1. Debes tener una función llamada `isOrdered` que reciba dos enteros y devuelva un booleano que indique si el primer entero es menor o igual que el segundo.
...
1. Para cada paso de ordenamiento, debes usar `highlightCurrentEls` para resaltar los dos números que se están comparando y en el siguiente paso intercambiarlos usando `swapElements`.
...
Tu función `highlightCurrentEls` debe dar a los descendientes del elemento especificado, ubicados en el índice dado y en el siguiente índice, un borde que sea `dashed`, `red` y con un ancho a tu elección.
