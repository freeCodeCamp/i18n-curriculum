...
使用相同的語法，查詢 `#lunch` 元素中的 `number` 輸入，並將它們指定給 `lunchNumberInputs`。

# --hints--

你應該宣告一個 `lunchNumberInputs` 變數。

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=/);
```

你應該使用 `#lunch input[type='number']` 呼叫 `document.querySelectorAll()`。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你應該將 `document.querySelectorAll()` 呼叫的結果指定給 `lunchNumberInputs`。
