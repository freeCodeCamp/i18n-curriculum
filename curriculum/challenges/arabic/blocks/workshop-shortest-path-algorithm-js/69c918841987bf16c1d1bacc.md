...
الآن كرر التكرار عبر عقد الهدف لعرض النتائج لكل واحدة منها.

أضف حلقة `for...of` تتكرر عبر `targets` باستخدام `nodeNo` كمتغير للحلقة. اترك جسم الحلقة فارغًا في الوقت الحالي.

# --hints--

يجب أن تنشئ حلقة `for...of` بالمتغير `nodeNo` يتكرر عبر `targets`.
