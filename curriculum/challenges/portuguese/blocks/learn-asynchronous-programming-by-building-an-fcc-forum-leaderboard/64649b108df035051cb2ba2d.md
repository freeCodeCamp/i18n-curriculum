...
Para a abertura da tag `a`, defina o atributo `target` como `"_blank"`.Então, defina o atributo `href` para `${forumTopicUrl}${slug}/${id}`. 

E com essas mudanças, seu projeto do placar do fórum freeCodeCamp está agora completo!

# --hints--

Sua tag de âncora de abertura deve ter um atributo `target` definido como `"_blank"`.
...
Sua tag de âncora de abertura deve ter um atributo `href` definido como `${forumTopicUrl}${slug}/${id}`.
