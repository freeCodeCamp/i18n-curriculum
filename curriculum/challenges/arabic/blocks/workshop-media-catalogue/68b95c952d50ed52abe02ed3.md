...
أضف إلى صنف `MediaError` طريقة `__init__` بالمعلمات التالية: `self`، `message`، و `obj`.

داخلها، استدعِ `super().__init__(message)` لضمان عمل آليات تخزين وعرض رسالة الخطأ، الموجودة مسبقًا في صنف `Exception`، بشكل صحيح.

بعد ذلك، عيّن `obj` إلى `self.obj`.

# --hints--

يجب أن يحتوي صنف `MediaError` على طريقة `__init__` بالمعلمات `self`، `message`، و `obj`.
...
يجب استدعاء `super().__init__(message)` داخل طريقة `__init__` الخاصة بك.
...
يجب تعيين `obj` إلى `self.obj` داخل طريقة `__init__` الخاصة بك.
