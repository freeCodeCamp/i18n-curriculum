...
أنشئ عنصر `a` آخر داخل العنصر الثاني `li`. اجعله يحمل خاصية `href` بقيمة `#` والنص `Widgets`.

# --hints--

يجب أن تنشئ عنصر `a` داخل العنصر الثاني من القائمة.

```js
assert.lengthOf(document.querySelectorAll('li a'), 2);
```

يجب أن تحمل خاصية `href` في عنصر `a` القيمة `#`.

```js
assert.equal(document.querySelectorAll('a')[1]?.getAttribute('href'), '#');
```

يجب أن يحتوي عنصر `a` على النص `Widgets`.
