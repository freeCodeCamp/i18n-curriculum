...
استخدم الدالة `minmax` لجعل أعمدتك متجاوبة على أي جهاز. تأخذ الدالة `minmax` معلمتين، الأولى هي الحد الأدنى والثانية هي الحد الأقصى. يمكن أن تكون هذه القيم طولًا، نسبة مئوية، `fr`، أو حتى كلمة مفتاحية مثل `max-content`.

لف كل قيمة من القيم التي عرّفتها مسبقًا في خاصية `grid-template-columns` داخل دالة `minmax`، مستخدمًا كل قيمة كالمعلمة الثانية. يجب أن تكون المعلمة الأولى على التوالي `2rem`، `min-content`، و `2rem`.

# --hints--

يجب أن يحتوي محدد `main` على خاصية `grid-template-columns` مضبوطة على `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
