...
Quando clicchi una canzone della tua playlist vuoi riprodurre quella canzone. Per farlo dovrai aggiungere un listener per l'evento `click` su ogni elemento `button` che è un figlio di un elemento `song`.

Completa la callback del tuo `forEach` per farlo. All'interno della callback `addEventListener`, chiama `playSong(n)`, dove `n` è l'id della canzone che puoi ottenere dall'attributo `id` di `song`.

Nota che ogni `song` ha un attributo `id` di `song-n`: dovrai prendere la parte `n` e convertirla in un numero prima di passarla a `playSong()`.


# --hints--

Dovresti aggiungere un listener per l'evento `click` su ogni elemento `button` che è un figlio di un elemento `.playlist-song` e chiamare `playSong` con il `id` della canzone corrispondente.
