...
Додайте вашому `conversionTable` властивість `gram` і призначте їй об’єкт із властивістю `cup`, встановленою в `1/240`, властивістю `ounce`, встановленою в `0.0353`, і властивістю `teaspoon`, встановленою в `0.2`.

# --hints--

Ваш `conversionTable` має мати властивість `gram`.

```js
assert.property(conversionTable, "gram");
```

Властивість `gram` має бути об’єктом.

```js
assert.isObject(conversionTable.gram);
```

Об’єкт `gram` має мати властивість `cup`.

```js
assert.property(conversionTable.gram, "cup");
```

Властивість `cup` має бути встановлена в `1/240`.

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

Об’єкт `gram` має мати властивість `ounce`.

```js
assert.property(conversionTable.gram, "ounce");
```

Властивість `ounce` має бути встановлена в `0.0353`.

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

Об’єкт `gram` має мати властивість `teaspoon`.

```js
assert.property(conversionTable.gram, "teaspoon");
```

Властивість `teaspoon` має бути встановлена в `0.2`.
