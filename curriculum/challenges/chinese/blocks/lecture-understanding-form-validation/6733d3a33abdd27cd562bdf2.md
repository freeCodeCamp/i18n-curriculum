...
另一个常见的使用 `e.preventDefault()` 方法的例子与表单提交有关。默认情况下，提交表单会将数据发送到服务器并重新加载页面。使用 `e.preventDefault()` 可以阻止这种情况发生。
...
当你需要更好地控制用户如何与页面交互时，阻止默认行为非常有用，但重要的是要考虑无障碍：你的自定义行为应提供与默认行为相同的功能。
