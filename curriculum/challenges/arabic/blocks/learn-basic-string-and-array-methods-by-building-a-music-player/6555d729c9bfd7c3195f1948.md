...
تُعيّن `textContent` نص العقدة وتسمح لك بتعيين أو استرجاع محتوى النص لعنصر HTML.
...
يمكنك استخدام معامل ثلاثي لتعيين قيمة محتوى النص بشكل شرطي. إليك مثالًا على تعيين نتيجة معامل ثلاثي إلى متغير:

```js
const example = condition ? "I'm true" : "I'm false";
```

استخدم معامل ثلاثي للتحقق مما إذا كانت `currentTitle` تُقيّم إلى قيمة صحيحة. إذا كانت كذلك، عيّن `playingSong.textContent` إلى `currentTitle`. وإلا، عيّنها إلى سلسلة فارغة.

ثم أدناه، استخدم عامل `ternary` للتحقق مما إذا كانت قيمة `currentArtist` صحيحة. إذا كانت كذلك، عيّن `songArtist.textContent` إلى `currentArtist`. وإلا، عيّنها إلى سلسلة فارغة.

# --hints--

يجب أن تربط خاصية `textContent` بـ `playingSong`.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

يجب استخدام العامل `ternary` لتعيين خاصية `textContent` في `playingSong` إلى `currentTitle` أو `""`.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

يجب أن تربط خاصية `textContent` بـ `songArtist`.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

يجب استخدام العامل `ternary` لتعيين خاصية `textContent` لـ `songArtist` إلى `currentArtist` أو `""`.
