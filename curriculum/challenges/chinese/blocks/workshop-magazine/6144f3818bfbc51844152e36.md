...
将 `grid-template-columns` 属性的每个已定义的值放在 `minmax` 函数中作为第二个参数。 第一个参数应分别为 `2rem`、`min-content` 和 `2rem`。

# --hints--

你的 `main` 选择器应该有一个 `grid-template-columns` 属性。值为 `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`。
