...
Sie sollten eine flache Kopie des `crew`-Arrays mit der `slice()`-Methode erstellen. Benennen Sie das neue Array `updatedCrew`.

Denken Sie daran, dass das Aufrufen von `slice()` ohne Argumente eine Kopie des gesamten Arrays zurückgibt:
...
Sie sollten die `slice()`-Methode verwenden, um das Eingabe-`crew`-Array zu kopieren und das Ergebnis in einer neuen Variablen namens `updatedCrew` zu speichern.
