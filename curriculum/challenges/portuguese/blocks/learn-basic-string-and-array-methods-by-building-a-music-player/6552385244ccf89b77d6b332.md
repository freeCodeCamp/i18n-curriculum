...
Você precisa trabalhar em tocar a próxima música e a música anterior. Para isso, você precisará de uma função `playNextSong` e `playPreviousSong`.

Use `const` e a sintaxe de arrow para criar uma função vazia `playNextSong`.

# --hints--

Você deve usar `const` para criar uma função vazia chamada `playNextSong`.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` deve ser uma função.

```js
assert.isFunction(playNextSong)
```

Sua função `playNextSong` deve usar uma sintaxe de arrow.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Sua função `playNextSong` deve estar vazia.
