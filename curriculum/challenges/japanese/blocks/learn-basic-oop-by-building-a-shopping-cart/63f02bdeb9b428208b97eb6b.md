...
あなたの`calculateTaxes`メソッドは、`taxRate`の値（パーセントに変換するために100で割ったもの）に`amount`パラメータを乗算した値を返す必要があります。

わかりやすくするために、`taxRate / 100`の計算は丸括弧で囲んでください。

# --hints--

`taxRate`を100で割る必要があります。`calculateTaxes`メソッド内で行ってください。`this`キーワードを使うことを忘れないでください。
...
`this.taxRate / 100`の計算は丸括弧で囲むべきです。
...
`amount`パラメータに、100で割った`taxRate`を乗算する必要があります。`calculateTaxes`メソッド内で行ってください。
