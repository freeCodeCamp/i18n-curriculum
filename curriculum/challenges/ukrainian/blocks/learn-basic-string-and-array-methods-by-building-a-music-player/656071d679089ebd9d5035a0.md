...
Оскільки користувачі зможуть перемішувати та видаляти пісні з плейлиста, вам потрібно створити копію масиву `allSongs` без зміни оригіналу. Тут стане в пригоді оператор `spread`.

Оператор розширення (`...`) дозволяє скопіювати всі елементи з одного масиву в інший. Його також можна використовувати для об’єднання кількох масивів в один. У прикладі нижче `arr1` і `arr2` були розширені в `combinedArr`:
...
У об’єкті `userData` створіть властивість `songs`. Для значення розширте `allSongs` у масив.

# --hints--

Ваш об’єкт `userData` має містити ключ `songs`.

```js
assert.property(userData, "songs");
```

Властивість `songs` має мати значення `[...allSongs]`.
