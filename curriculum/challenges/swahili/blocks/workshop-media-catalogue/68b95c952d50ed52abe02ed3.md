...
Toa darasa la `MediaError` njia ya `__init__` yenye vigezo vifuatavyo: `self`, `message`, na `obj`.

Ndani yake, fanya wito wa `super().__init__(message)` ili kuhakikisha mifumo ya kuhifadhi na kuonyesha ujumbe wa kosa, ambao tayari upo katika darasa la `Exception`, inaweza kufanya kazi ipasavyo.

Baada ya hapo, weka thamani ya `obj` kwa `self.obj`.

# --hints--

Darasa lako la `MediaError` linapaswa kuwa na njia ya `__init__` yenye vigezo `self`, `message`, na `obj`.
...
Unapaswa kufanya wito wa `super().__init__(message)` ndani ya njia yako ya `__init__`.
...
Unapaswa kuweka thamani ya `obj` kwa `self.obj` ndani ya njia yako ya `__init__`.
