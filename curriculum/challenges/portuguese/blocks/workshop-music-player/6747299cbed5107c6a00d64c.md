...
O restante das músicas foi adicionado ao array `allSongs` para você.

Nas lições anteriores, você aprendeu sobre a <dfn>Web Audio API</dfn> e como usá-la para tocar músicas. Todos os navegadores modernos suportam a Web Audio API, que permite gerar e processar áudio em aplicações web.

Crie uma variável chamada `audio` e defina-a como igual a `new Audio()`. Isso criará um novo elemento HTML5 `audio`.

# --hints--

Você deve ter uma variável chamada `audio`.

```js
assert.isDefined(audio);
```

Você deve atribuir uma nova instância do objeto `Audio` à variável chamada `audio`.
