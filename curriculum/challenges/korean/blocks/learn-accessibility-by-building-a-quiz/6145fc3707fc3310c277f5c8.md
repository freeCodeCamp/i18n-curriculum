...
페이지 스타일링으로 돌아갑니다. 내비게이션 바 안의 리스트 요소를 선택하고, 다음 스타일을 적용하세요:
...
`nav li` 또는 `nav > ul > li` 선택자 중 하나를 사용하세요.
...
`li` 요소에 `margin`를 `0 0.2rem`로 지정하세요.
...
`li` 요소에 `padding`를 `0.2rem`로 지정하세요.
...
`li` 요소에 `display`를 `block`로 지정하세요.
