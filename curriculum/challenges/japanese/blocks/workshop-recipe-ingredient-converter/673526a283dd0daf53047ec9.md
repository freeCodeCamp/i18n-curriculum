...
これで最初のカリー化された関数を作成する必要があります。カリー化された関数とは、別の関数を返す関数のことです。例えば次のようになります。
...
まず`convertQuantity`関数を宣言してください。パラメータは1つの`fromUnit`で、関数本体は空にしてください。

# --hints--

`convertQuantity`関数を宣言してください。

```js
assert.isFunction(convertQuantity)
```

`convertQuantity`関数は`fromUnit`パラメータを持つべきです。

```js
assert.match(convertQuantity?.toString(), /\(fromUnit\)/);
```

`convertQuantity`関数は空にしてください。
