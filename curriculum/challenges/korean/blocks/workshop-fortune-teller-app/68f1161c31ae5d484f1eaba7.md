...
이제 `showFortune`라는 메서드를 하나 더 추가하세요. 매개변수로 `e`을 전달하고 타입은 `Event`로 지정하세요. 또한 `newReading` 메서드를 만들고 지금은 비워 두세요.

# --hints--

`showFortune`라는 메서드를 선언해야 합니다.
...
`showFortune` 메서드는 `e` 매개변수를 가지고 타입은 `Event`이어야 합니다.
