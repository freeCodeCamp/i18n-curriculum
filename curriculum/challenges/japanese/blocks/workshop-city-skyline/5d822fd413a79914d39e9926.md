...
`.fb3-window` 要素に `width` の `25%`、`height` の `80%` を与え、`--window-color1` 変数を `background-color` の値として使ってください。

# --hints--

`.fb3-window` に `width` の `25%` を与える必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

`.fb3-window` に `height` の `80%` を与える必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

`.fb3-window` に `background-color` の `--window-color1` を与える必要があります。
