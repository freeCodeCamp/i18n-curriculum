...
Adicione um event listener para o evento `click` em `scaryStoryBtn` e passe a função `displayStory`. Então, clique no seu botão e veja a saída no console.

# --hints--

Você deve adicionar um event listener para o evento `click` em `scaryStoryBtn` passando a função `displayStory`.
