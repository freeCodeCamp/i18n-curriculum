...
Як бачите, тепер ваша множина повністю порожня. Видаліть обидва виклики `clear` і `has` зі свого коду.

# --hints--

У вашому коді не повинно бути `plantsSet.clear()`.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

У вашому коді не повинно бути `plantsSet.has(ballerina)`.
