...
다음으로, DOM에서 요소를 숨기는 함수를 만들어야 합니다. `hideElements` 함수를 선언하고 `...elements`을 `HTMLElement[]` 타입의 매개변수로 받으세요.

이 함수는 CSS `hidden` 클래스를 사용하며, 앱 전반에서 UI 요소의 가시성을 토글하는 데 사용됩니다.

# --hints--

`hideElements`라는 이름의 함수를 선언했어야 합니다.
...
`hideElements` 함수는 `...elements`라는 이름의 매개변수를 `HTMLElement[]` 타입으로 가져야 합니다.
