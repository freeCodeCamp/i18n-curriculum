...
`fightSlime`와 `fightBeast`이라는 이름의 빈 함수를 두 개 더 만드세요. 이 함수들은 곧 나올 `cave` 객체에서 사용될 것입니다.

# --hints--

`fightSlime`는 함수여야 합니다.

```js
assert.isFunction(fightSlime);
```

`fightBeast`는 함수여야 합니다.

```js
assert.isFunction(fightBeast);
```

`fightSlime`는 비어 있어야 합니다.

```js
assert.match(fightSlime.toString(), /function fightSlime\(\s*\) \{\}/);
```

`fightBeast`는 비어 있어야 합니다.
