...
Вам потрібна функція, яка фактично оновлює DOM. Оголосіть `updateResultsList` функцію.

Спочатку функція має очистити HTML у `resultList` елементі.

# --hints--

Ви маєте визначити `updateResultsList` функцію.

```js
assert.isFunction(updateResultsList);
```

Ваша `updateResultsList` функція має встановити `resultList.innerHTML` у порожній рядок.
