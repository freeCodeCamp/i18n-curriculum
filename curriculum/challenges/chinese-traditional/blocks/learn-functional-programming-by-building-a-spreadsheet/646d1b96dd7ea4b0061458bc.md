...
你可以將函式（程式）<dfn>reference</dfn> 作為回呼參數傳遞。函式（程式）引用是沒有小括弧的小括號的函式（程式）名稱。例如：
...
這裡的 `.map()` 方法會呼叫 `myFunc` 函式，並傳遞 `.map()` 回呼所接收的相同引數。第一個引數是陣列在目前迭代的值，因此 `newArray` 將會是 `[value: 1, value: 2, value: 3]`。

將你的 `elemValue` 函式的引用作為回呼傳遞給你的 `.map()` 方法。

# --hints--

你不應該呼叫你的 `elemValue` 函式（程式）。
...
你應該將 `elemValue` 的引用作為回呼傳遞給你的 `.map()` 方法。
