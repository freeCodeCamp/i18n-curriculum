...
在你的 `allCategories` 對象中，添加一個新的鍵 `299`，它的值爲空對象。

在該對象中，添加一個名爲 `category` 的鍵，其字符串值爲 `"Career Advice"`。 在該屬性下面，再添加一個名爲 `className` 的鍵，其值爲字符串 `"career"`。

# --hints--

你的 `allCategories` 對象應該有一個 `299` 屬性。
