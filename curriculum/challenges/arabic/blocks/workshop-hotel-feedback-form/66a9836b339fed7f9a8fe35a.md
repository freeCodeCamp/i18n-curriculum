...
أضف إلى عنصر `textarea` الخاص بك خاصيتي `id` و `name` بقيمة `"comments"`.

عيّن قيمة الخاصية `cols` إلى `30` وقيمة الخاصية `rows` إلى `10`.

# --hints--

يجب أن يحتوي عنصر `textarea` الخاص بك على خاصية `id` بقيمة `"comments"`.

```js
assert.exists(document.querySelector('textarea#comments'));
```

يجب أن يحتوي عنصر `textarea` الخاص بك على خاصية `name` بقيمة `"comments"`.

```js
assert.exists(document.querySelector('textarea[name="comments"]'));
```

يجب أن يحتوي عنصر `textarea` الخاص بك على خاصية `cols` بقيمة `"30"`.

```js
assert.exists(document.querySelector('textarea[cols="30"]'));
```

يجب أن يحتوي عنصر `textarea` الخاص بك على خاصية `rows` بقيمة `"10"`.
