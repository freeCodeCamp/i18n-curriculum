...
請注意，HTML 中的專輯封面和 `userData.songs` 陣列中的歌曲已經更改。我們已將原始歌曲替換為較短的歌曲，供你用來測試你的應用程式。

所有核心功能現在都已就緒。唯一的問題是當目前播放的歌曲結束時，下一首歌曲不會自動播放。 

為了解決這個問題，你可以設置一個事件監聽器來偵測目前播放的歌曲何時結束。`"ended"` 事件監聽器適合用於此。當媒體播放到結束時，它會被觸發。

為你的 `audio` 元素添加一個監聽 `"ended"` 事件的監聽器，並傳入對 `playNextSong` 函式的引用。這樣音樂播放器專案就完成了！

# --hints--

你應該將 `addEventListener()` 方法串鏈到你的 `audio` 變數。

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

你在 `audio` 變數上使用的事件監聽器應該監聽 `"ended"` 事件。

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

你應該將對 `playNextSong` 的引用傳入你的 `"ended"` 事件監聽器。
