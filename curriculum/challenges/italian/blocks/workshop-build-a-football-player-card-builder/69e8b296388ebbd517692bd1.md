...
Aggiungi un altro `div` con il `className` di `form-group`. Al suo interno, aggiungi un elemento `label` con il `className` di `label`, un `htmlFor` di `imageUrl` e il testo `Image URL`. Poi aggiungi un elemento `input` con un `id` di `imageUrl`, un `className` di `input` e un `type` di `text`.

# --hints--

Dovresti avere un elemento `div` con il `className` di `form-group`.
...
Il tuo `div.form-group` dovrebbe avere un `label` con un attributo `htmlFor` di `imageUrl`, un `className` di `label` e il testo `Image URL`.
...
Il tuo URL dell'immagine `form-group` dovrebbe contenere un `input` con un `id` di `imageUrl`, `className` di `input` e un `type` di `text`.
