...
`currentWeapon` を代入した後で、`text.innerText` を設定しましょう。文字列 `"You sold a "`、次に `currentWeapon`、次に文字列 `"."` を、連結演算子で連結して設定してください。

# --hints--

`text.innerText` に代入演算子を使用してください。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=/);
```

`text.innerText` に `"You sold a "` を追加してください。 スペースに注意してください。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

文字列 `"You sold a "` に、`currentWeapon` の値を連結してください。 連結演算子を使用して、同じ行で連結してください。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a\s\1\s*\+\s*currentWeapon/);
```

`currentWeapon` の値に文字列 `"."` を連結してください。 連結演算子を使用して、同じ行で連結してください。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1\s*\+\s*currentWeapon\s+\+\s+('|")\.\2/);
```

`text.innerText` が適切な文字列に更新される必要があります。
