...
العنصران النهائيان الدلاليان في HTML لهذا المشروع هما العنصران `footer` و `address`. عنصر `footer` هو حاوية لمجموعة من المحتوى المرتبط بالصفحة، وعنصر `address` هو حاوية لمعلومات الاتصال بمؤلف الصفحة.

بعد عنصر `main`، أضف عنصر `footer` واحد، وقم بالتسكين لعنصر `address` واحد بداخله.

# --hints--

يجب إضافة عنصر `footer` واحد بعد عنصر `main`.

```js
assert.exists(document.querySelector('main + footer'));
```

يجب تسكين عنصر `address` واحد داخل عنصر `footer`.
