...
Após o elemento `h2`, crie dois elementos `p`.Dê ao primeiro um `class` de `dessert-price` e defina o texto para um cifrão `"$"` seguido do valor da variável `price`.Dê ao segundo um `class` de `product-category` e o texto `"Category: "` seguido pelo valor da variável `category`.

# --hints--

Você deve criar dois elementos `p`.
...
Os elementos `p` devem vir depois do elemento `h2`.
...
O primeiro elemento `p` deve ter uma `class` definida como `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Seu primeiro elemento `p` deve ter o símbolo de dólar `"$"` seguido pelo valor da variável `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

O segundo elemento `p` deve ter uma `class` definida como `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Seu segundo elemento `p` deve conter o texto `"Category: "` seguido pelo valor da variável `category`.
