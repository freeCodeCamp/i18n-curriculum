...
قم بتوسيط كل النص أفقيًا داخل عنصر `address`، وإضافة بعض من padding.

# --hints--

يجب عليك استخدام منتقي العناصر `address`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

يجب عليك إعطاء `address` خاصية `text-align` بقيمة `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

يجب عليك إعطاء `address` خاصية `padding-top` بقيمة `1px`.
...
يجب عليك إعطاء `address` خاصية `padding-right` بقيمة `1px`.
...
يجب عليك إعطاء `address` خاصية `padding-bottom` بقيمة `1px`.
...
يجب عليك إعطاء `address` خاصية `padding-left` بقيمة `1px`.
