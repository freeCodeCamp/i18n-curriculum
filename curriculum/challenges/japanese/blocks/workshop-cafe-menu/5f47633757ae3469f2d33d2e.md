...
ページプレビューの幅を狭くすると、左側のテキストの一部が次の行に折り返され始めるのに気づきます。これは、左側の`p`要素の幅がスペースの`50%`しか取れないためです。

右側の価格は文字数がかなり少ないので、`flavor`クラスを`width`から`75%`に、priceクラスを`width`から`25%`に更新してください。

# --hints--

`width`セレクターで`75%`プロパティを`.flavor`に設定してください。
...
`width`セレクターで`25%`プロパティを`.price`に設定してください。
