...
1. Tu `#house` debe tener una `position` establecida como `relative` para que sus hijos puedan ser posicionados con respecto a ella.
1. Tu `#house` debe tener un ancho de `500px` y una altura de `400px`. No establezcas valores mínimos ni máximos.
...
1. Todos los hijos inmediatos de la `#house` deben tener una `position` de `absolute`.
...
`#window-1` debe tener la propiedad `position` establecida como `absolute`.

```js
assert.equal(getComputedStyle(document.querySelector("#window-1"))?.getPropertyValue("position"), "absolute");
```

Debes apuntar a `#window-1` y establecer su propiedad `width`.
...
Debes apuntar a `#window-1` y establecer su propiedad `height`.
...
Debes apuntar a `#window-1` y establecer su propiedad `border`.
...
Debes apuntar a `#window-1` y establecer su propiedad `background-color`.
...
`#window-2` debe tener la propiedad `position` establecida como `absolute`.

```js
assert.equal(getComputedStyle(document.querySelector("#window-2"))?.getPropertyValue("position"), "absolute");
```

Debes apuntar a `#window-2` y establecer su propiedad `width`.
...
Debes apuntar a `#window-2` y establecer su propiedad `height`.
...
Debes apuntar a `#window-2` y establecer su propiedad `border`.
...
Debes apuntar a `#window-2` y establecer su propiedad `background-color`.
