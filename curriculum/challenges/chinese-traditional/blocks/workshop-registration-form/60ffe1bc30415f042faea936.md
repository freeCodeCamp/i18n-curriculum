...
為了讓 `input` 和 `textarea` 元素與背景主題融合，請將它們的 `background-color` 設為 `#0a0a23`。然後，為它們添加一個顏色為 `#0a0a23` 的 `1px`、`solid` 邊框。

# --hints--

你應該使用逗號分隔的元素選擇器來選取 `input` 和 `textarea` 元素。
...
你應該為 `input` 和 `textarea` 元素設定 `background-color` 為 `#0a0a23`。
...
你應該為 `input` 和 `textarea` 元素設定一個顏色為 `#0a0a23`、寬度為 `1px`、樣式為 `solid` 的邊框。
