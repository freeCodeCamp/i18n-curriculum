...
給 `#years` 一個 `margin` 爲 `0 -2px`，和一個 `padding` 爲 `0.5rem calc(1.25rem + 2px) 0.5rem 0`。

# --hints--

你的 `#years` 選擇器應該有一個 `margin` 屬性設置爲 `0 -2px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('margin') === '0px -2px');
```

你的 `#years` 選擇器應該有一個 `padding` 屬性設置爲 `0.5rem calc(1.25rem + 2px) 0.5rem 0`。
