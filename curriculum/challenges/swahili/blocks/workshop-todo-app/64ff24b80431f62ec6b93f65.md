...
Ondoa safu ya data `myTaskArr` na msimbo wote wa `localStorage` kwa sababu hautaendelea kuhitaji tena.

# --hints--

Unapaswa kuondoa `myTaskArr` na msimbo wote unaohusiana na `localStorage` ambao umejifunza hivi karibuni.
...
Unapaswa kuondoa rejea zote zilizobaki za `myTaskArr` mahali popote kwenye msimbo.

```js
assert.notMatch(code, /myTaskArr/);
```

Unapaswa kuondoa rejea yoyote ya `localStorage.getItem` kwa kipengee cha `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Unapaswa kuondoa rejea zote zilizobaki za `getTaskArrObj` mahali popote kwenye msimbo.
