...
Sifa ya `gap` ni njia fupi ya kuweka thamani ya `column-gap` na `row-gap` kwa wakati mmoja. Ikipewa thamani moja, huweka `column-gap` na `row-gap` zote kwa thamani hiyo. Ikipewa thamani mbili, huweka `row-gap` kwa thamani ya kwanza na `column-gap` kwa thamani ya pili.

Toa kichaguzi cha `.image-wrapper` sifa ya `gap` iliyowekwa kuwa `2rem`.

# --hints--

Kipengele chako cha `.image-wrapper` kinapaswa kuwa na sifa ya `gap` iliyowekwa kuwa `2rem`.
