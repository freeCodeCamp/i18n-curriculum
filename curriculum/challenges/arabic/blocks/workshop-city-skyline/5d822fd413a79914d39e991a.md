...
لننتقل إلى البناء التالي! قم بالتسكين لعناصر `div` اثنين داخل `.fb2` وأعطهما أصنافًا `fb2a` و `fb2b` بهذا الترتيب.

# --hints--

يجب أن تضيف عنصرين `div` داخل `.fb2`.

```js
assert.lengthOf(document.querySelectorAll("div.fb2 > div"), 2);
```

يجب أن تعطي أول عنصر `div` جديد صنفًا `fb2a`.
...
يجب أن تعطي ثاني عنصر `div` جديد صنفًا `fb2b`.
