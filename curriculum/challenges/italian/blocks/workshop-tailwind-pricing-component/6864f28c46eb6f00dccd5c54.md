...
Spostati sul `h2` all'interno del **piano Listener**, assegnagli la classe di utilità `text-lg` per ingrandirlo, `font-semibold` per renderlo più in grassetto e `text-indigo-600` per dargli un bellissimo colore indaco.

# --hints--

Il tuo elemento `h2` dovrebbe avere la classe `text-lg`.
...
Il tuo elemento `h2` dovrebbe avere la classe `font-semibold`.
...
Il tuo elemento `h2` dovrebbe avere la classe `text-indigo-600`.
