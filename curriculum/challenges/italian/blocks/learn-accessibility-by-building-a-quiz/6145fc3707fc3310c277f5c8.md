...
Torniamo a definire lo stile della pagina. Seleziona gli elementi lista all'interno della barra di navigazione e applica loro i seguenti stili:
...
Dovresti usare il selettore `nav li` oppure `nav > ul > li`.
...
Dovresti dare agli elementi `li` un `margin` di `0 0.2rem`.
