...
수식에서 셀 범위를 일치시킬 수 있어야 합니다. 셀 범위는 `A1:B12` 또는 `A3:A25`처럼 보일 수 있습니다. 이러한 패턴을 일치시키려면 정규 표현식을 사용할 수 있습니다.

`rangeRegex` 변수를 선언하고 `A`부터 `J`까지(스프레드시트의 열 범위)를 일치시키는 정규 표현식을 할당하는 것부터 시작하세요. 이를 위해 문자 클래스를 포함한 캡처 그룹을 사용하세요.

# --hints--

`rangeRegex` 함수 다음에 `idToText` 변수를 선언해야 합니다.
...
`const` 변수를 선언할 때 `rangeRegex`를 사용하세요.
...
`rangeRegex`은 캡처 그룹을 사용해야 합니다.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(.*\)\/\s*;?/);
```

`rangeRegex`은 캡처 그룹 안에 문자 클래스를 사용해야 합니다.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[.*\]\)\/\s*;?/);
```

`rangeRegex`은 `A`부터 `J`까지 일치시키기 위해 문자 클래스를 사용해야 합니다.
