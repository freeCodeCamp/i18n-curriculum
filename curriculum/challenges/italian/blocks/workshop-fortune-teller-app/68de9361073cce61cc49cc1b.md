...
In una lezione precedente, hai imparato che l'inferenza di tipo è la capacità di TypeScript di determinare e assegnare automaticamente i tipi a variabili, valori di ritorno di funzioni ed espressioni in base al loro utilizzo e contesto.

In TypeScript, i generici rendono più flessibili funzioni, classi e tipi. I generici permettono di creare variabili di tipo, eliminando la necessità di definire esplicitamente i tipi ogni volta. Questo consente alla stessa funzione di funzionare in modo sicuro con tipi diversi.

Ecco un esempio semplice di generici:
...
Le funzioni possono usare due o più generici. `T` è usato per convenzione, e si possono aggiungere altre variabili di tipo come `S` secondo necessità.
...
Crea una funzione `getElement` che prende `selector` di tipo `string` come parametro. Deve anche prendere un tipo generico `T` che estende `HTMLElement` e avere un tipo di ritorno `T`.

# --hints--

Devi avere una funzione freccia chiamata `getElement`.
...
La tua funzione `getElement` deve prendere un parametro chiamato `selector` di tipo `string`.
...
Il parametro di tipo generico `T` deve estendere il tipo `HTMLElement`.
...
La tua funzione `getElement` deve avere un tipo di ritorno `T`.
