...
È il momento di verificare che la ricerca funzioni. Stephen King ha sei libri nel catalogo.

Chiama `findByAuthor(catalog, "king")` e assegna il risultato a una variabile chiamata `kingBooks`. Registra `kingBooks.length` per confermare il conteggio.

Poi usa un loop `for` per registrare ogni risultato. Usa un letterale template per includere il titolo e l'anno, come `` `${kingBooks[i].title} (${kingBooks[i].year})` ``.

# --hints--

`kingBooks` dovrebbe avere una lunghezza di `6`.

```js
assert.strictEqual(kingBooks.length, 6);
```

Dovresti registrare `kingBooks.length` nella console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*kingBooks\.length\s*\)/);
```

Dovresti usare un loop `for` per registrare il titolo e l'anno di ogni libro.
