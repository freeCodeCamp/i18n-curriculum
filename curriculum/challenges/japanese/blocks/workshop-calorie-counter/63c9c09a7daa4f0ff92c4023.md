...
同じ構文を使って、`number` 要素内の `#lunch` 入力をクエリし、それらを `lunchNumberInputs` に代入してください。

# --hints--

`lunchNumberInputs` 変数を宣言する必要があります。

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=/);
```

`document.querySelectorAll()` を使って `#lunch input[type='number']` を呼び出してください。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` の呼び出し結果を `lunchNumberInputs` に代入してください。
