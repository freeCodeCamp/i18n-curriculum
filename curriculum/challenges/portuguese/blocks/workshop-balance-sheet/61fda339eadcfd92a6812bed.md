...
The `span[class~="sr-only"]` selector will select any `span` element whose `class` _includes_ `sr-only`. Crie esse seletor e dê a ele uma propriedade `border` definida como `0`.
