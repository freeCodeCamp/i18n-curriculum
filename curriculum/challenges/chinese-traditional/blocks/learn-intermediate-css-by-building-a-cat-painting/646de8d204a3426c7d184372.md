...
那些看起來像眼睛，但你仍然可以讓它們更好。這就是為什麼你創建了兩個內部眼睛的 `div` 元素。

使用類別選擇器，為你的 `.cat-left-inner-eye` 元素設定寬度為 `10px` 和高度為 `20px`。同時，為它設定背景顏色為 `#fff`。

# --hints--

你應該有一個 `.cat-left-inner-eye` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye'))
```

你的 `.cat-left-inner-eye` 選擇器應該將 `width` 設定為 `10px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.width === '10px')
```

你的 `.cat-left-inner-eye` 選擇器應該設定 `height` 為 `20px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.height === '20px')
```

你的 `.cat-left-inner-eye` 選擇器應該設定 `background-color` 為 `#fff`。
