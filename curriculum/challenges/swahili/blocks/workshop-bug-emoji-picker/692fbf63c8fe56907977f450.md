...
Ifuatayo, fafanua kitu kinachoitwa `bugMap` chenye aina ya `Record<string, Bug<string>>` na weka thamani yake kuwa kitu tupu kwa sasa.

# --hints--

Unapaswa kuwa na thabiti inayoitwa `bugMap`.
...
`bugMap` yako inapaswa kuwa na maelezo ya aina ya `Record<string, Bug<string>>`.
...
`bugMap` yako inapaswa kuanzishwa kuwa kitu tupu.
