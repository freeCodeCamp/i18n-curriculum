...
Створіть один фінальний `@media` запит для `only screen` з `max-width` типом `420px`. Всередині створіть `.hero-title` селектор з `font-size` властивістю, встановленою в `4.5rem`.
...
Ваш новий `@media` запит має містити `.hero-title` селектор.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText === '.hero-title');
```

Ваш `.hero-title` селектор має мати `font-size` властивість, встановлену в `4.5rem`.
