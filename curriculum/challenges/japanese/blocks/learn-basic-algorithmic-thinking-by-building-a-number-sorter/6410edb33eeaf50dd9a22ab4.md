...
`if` の条件が true のとき、2つの要素を入れ替えて、より大きい要素を配列の末尾に「バブルアップ」させる必要があります。

これを行うには、`temp` 変数を宣言して `array[j]` の値を代入してください。次に `array[j]` に `array[j + 1]` の値を代入します。最後に `array[j + 1]` に `temp` の値を代入してください。

# --hints--

`if` 文の中で `temp` 変数を宣言する必要があります。
...
`array[j]` に `array[j + 1]` の値を代入してください。
