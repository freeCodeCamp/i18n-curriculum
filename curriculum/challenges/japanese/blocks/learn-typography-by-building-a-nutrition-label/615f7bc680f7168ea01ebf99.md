...
今度は、`.no-divider` 要素の下に別々の区切りを追加する必要があります。

最初の `.no-divider` 要素の後には `.divider` があります。2番目の `.divider` 要素の後にもう1つ `.no-divider` を作成してください。

# --hints--

`div` 要素の最後に新しい `.daily-value` を作成してください。
