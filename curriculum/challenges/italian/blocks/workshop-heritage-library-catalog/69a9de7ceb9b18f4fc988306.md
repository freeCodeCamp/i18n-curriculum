...
All'interno del loop, verifica se l'autore della voce corrente (convertito in minuscolo) include `searchTerm`. Il metodo `.includes()` restituisce `true` se una stringa contiene un'altra stringa come sottostringa. Se la condizione è vera, inserisci la voce in `results`. Dopo il loop, restituisci `results`.

# --hints--

`findByAuthor(catalog, "king").length` dovrebbe essere `6`.

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

`findByAuthor(catalog, "asimov").length` dovrebbe essere `2`.

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

`findByAuthor(catalog, "xyz").length` dovrebbe essere `0`.
