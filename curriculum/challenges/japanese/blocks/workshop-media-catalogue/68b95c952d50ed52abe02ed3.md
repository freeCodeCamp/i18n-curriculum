...
`MediaError` クラスに、`__init__`、`self`、`message` のパラメータを持つ `obj` メソッドを作成してください。

その中で、`super().__init__(message)` クラスにすでに存在するエラーメッセージの保存と表示の仕組みが正しく動作するように、`Exception` を呼び出してください。

その後、`obj` に `self.obj` を代入してください。

# --hints--

`MediaError` クラスには、`__init__`、`self`、`message` のパラメータを持つ `obj` メソッドが必要です。
...
`super().__init__(message)` メソッドの中で `__init__` を呼び出す必要があります。
...
`obj` メソッドの中で `self.obj` に `__init__` を代入してください。
