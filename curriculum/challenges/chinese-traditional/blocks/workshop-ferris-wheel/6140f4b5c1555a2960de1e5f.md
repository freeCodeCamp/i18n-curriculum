...
創建另一個名為 `cabins` 的 `@keyframes` 規則。使用與你的 `@keyframes wheel` 相同的屬性，複製 `0%` 和 `100%` 規則，但將 `100%` 選擇器的 `transform` 屬性設為 `rotate(-360deg)`。
...
你的 `0%` 選擇器應該有一個設定為 `rotate(0deg)` 的 `transform` 屬性。
...
你的 `100%` 選擇器應該有一個設定為 `rotate(-360deg)` 的 `transform` 屬性。
