...
1. 由于测试需要重新赋值，`getWeather` 函数和 `showWeather` 函数都不应使用 `const` 声明。

**注意：** 由于测试需要重新赋值，`getWeather` 函数和 `showWeather` 函数都不应使用 `const` 声明。此外，测试完成需要时间，只要你在控制台看到 `// running tests`，说明测试正在执行中。
...
`getWeather` 函数和 `showWeather` 函数都不应使用 `const` 声明。
