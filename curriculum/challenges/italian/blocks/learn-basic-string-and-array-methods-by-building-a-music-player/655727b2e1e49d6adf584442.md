...
Se la playlist è vuota, devi creare un elemento `resetButton` e un testo per esso. Questo pulsante apparirà solo se la playlist è vuota.

`createElement()` è un metodo DOM che puoi usare per creare dinamicamente un elemento con JavaScript. Per usare `createElement()`, lo chiami e poi passi il nome del tag come stringa:
...
All’interno della tua istruzione `if`, dichiara una costante `resetButton`, poi usa `createElement()` per creare un `"button"`.
...
Dovresti usare il metodo `createElement` per creare un `"button"`.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

Dovresti assegnare il nuovo elemento `button` creato a una costante `resetButton`.
