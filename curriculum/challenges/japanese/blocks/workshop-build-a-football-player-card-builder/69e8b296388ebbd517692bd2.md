...
Player Info セクションはこれで完成です。Player Info セクションの閉じる `div` の後、そして `form-panel` の内側に、新しい `div` を追加してください。その中に、`p` の `className` とテキスト `form-section-title` を持つ `Player Stats` 要素を追加します。次に、`div` の `className` を持つ `stats-grid` を追加してください。stats グリッドは今は空のままにしておきます。

# --hints--

あなたの `div.form-panel` には Player Stats セクション用の2つ目の `div` を含める必要があります。
...
Player Stats セクションには、`p` の `className` とテキスト `form-section-title` を持つ `Player Stats` を含める必要があります。
...
Player Stats セクションには、`div` の `className` を持つ `stats-grid` を含める必要があります。
