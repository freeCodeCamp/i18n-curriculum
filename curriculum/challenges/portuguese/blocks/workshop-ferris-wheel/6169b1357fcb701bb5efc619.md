...
Crie um novo seletor de `25%` entre os seletores `0%` e `50%`.Dê a esse novo seletor a propriedade `background-color` definida como `yellow`.

# --hints--

Para fazer isso, crie uma nova regra `25%` dentro da sua regra `@keyframes cabins`.
...
O elemento `25%` deve estar entre os seletores `0%` e `50%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText , '25%');
```

O seletor `25%` deve ter a propriedade `background-color` definida como `yellow`.
