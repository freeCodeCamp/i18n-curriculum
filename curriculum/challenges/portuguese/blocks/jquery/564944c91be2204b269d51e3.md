...
<a href="https://www.freecodecamp.org/news/html-elements-explained-what-are-html-tags/#em-element" target="_blank" rel="noopener noreferrer nofollow">Veja nosso artigo de notícias sobre &lt;em&gt;</a> para aprender a diferença entre `<i>` e `<em>` e seus usos.

Observe que, embora a tag `<i>` tenha sido tradicionalmente usada para enfatizar texto, ela desde então foi adotada para uso como uma tag para ícones. A tag `<em>` agora é amplamente aceita como a tag para ênfase. Ambos funcionarão para esse desafio.
