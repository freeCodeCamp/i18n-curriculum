...
Per i due elementi `p` che hai appena aggiunto, aggiungi `dessert` come valore dell'attributo `class` del primo elemento `p` e il valore `price` come attributo `class` del secondo elemento `p`.
