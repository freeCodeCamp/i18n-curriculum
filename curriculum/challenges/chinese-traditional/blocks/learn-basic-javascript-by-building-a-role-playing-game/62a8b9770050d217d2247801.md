...
`location["button text"]` 是一個有三個元素的陣列。將 `button1.innerText` 的指派改為 `location["button text"][0]`，它表現該陣列的第一個元素。

# --hints--

你應該存取 `location` 參數中 `"button text"` 屬性的第一個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*0\s*\]/);
```

你應該將 `button1.innerText` 屬性設為 `location` 參數中 `"button text"` 屬性的第一個元素。
