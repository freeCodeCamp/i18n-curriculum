...
Dopo il tuo loop while, devi inserire il valore corrente. Ricorda che il loop termina quando `j` esce dai limiti dell'array oppure quando il valore a `j` è inferiore al tuo valore corrente.

Usa l'operatore di assegnazione per inserire il tuo valore corrente nell'indice corretto.

# --hints--

Dovresti assegnare `currValue` all'indice `j + 1`.
