...
在 `.cat-whiskers-left` 元素内部，创建三个具有类 `cat-whisker-left-top`、`cat-whisker-left-middle` 和 `cat-whisker-left-bottom` 的 `div` 元素。

# --hints--

你不应该更改现有的类为 `cat-whiskers-left` 的 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers-left'), 1);
```

你应该在 `.cat-whiskers-left` 元素内创建三个 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers-left div'), 3);
```

第一个 `div` 元素在 `.cat-whiskers-left` 元素之内，应具有类 `cat-whisker-left-top`。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-left div')[0].classList.contains('cat-whisker-left-top'));
```

第二个 `div` 元素在 `.cat-whiskers-left` 元素内，应具有类 `cat-whisker-left-middle`。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-left div')[1].classList.contains('cat-whisker-left-middle'));
```

第三个 `div` 元素在 `.cat-whiskers-left` 元素之内，应具有类 `cat-whisker-left-bottom`。
