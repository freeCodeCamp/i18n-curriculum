...
使用 `minmax` 函式（程式）使你的行在任何裝置上都能響應。`minmax` 函式（程式）接受兩個引數，第一個是最小值，第二個是最大值。這些值可以是長度、百分比、`fr`，甚至是像 `max-content` 這樣的關鍵字。

將你已定義的每個 `grid-template-columns` 屬性的值包裹在 `minmax` 函式中，並使用每個值作為第二個引數。第一個引數應分別為 `2rem`、`min-content` 和 `2rem`。

# --hints--

你的 `main` 選擇器應該有一個 `grid-template-columns` 屬性，設定為 `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`。
