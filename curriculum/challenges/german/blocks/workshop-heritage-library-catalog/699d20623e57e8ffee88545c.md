...
Jetzt erstellen Sie `groupByDecade`, das den Katalog nach Jahrzehnten organisiert. Jeder Schlüssel wird ein String wie `"1970s"` sein, und jeder Wert ein Array von Büchern aus diesem Zeitraum.

Erstellen Sie eine Funktion namens `groupByDecade`, die `catalog` als Parameter erhält. Deklarieren Sie darin `const grouped = {}`. Fügen Sie eine `for`-Schleife hinzu, um `catalog` zu durchlaufen, und deklarieren Sie innerhalb der Schleife `const book = catalog[i]`.

# --hints--

Sie sollten eine Funktion namens `groupByDecade` mit einem `catalog`-Parameter haben.
...
Sie sollten eine Variable namens `grouped` deklarieren, die auf ein leeres Objekt gesetzt ist.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+grouped\s*=\s*\{\s*\}/);
```

Sie sollten eine `for`-Schleife verwenden, um `catalog` zu durchlaufen.

```js
assert.match(groupByDecade.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Sie sollten eine Variable namens `book` deklarieren, die auf den aktuellen Katalogeintrag gesetzt ist.
