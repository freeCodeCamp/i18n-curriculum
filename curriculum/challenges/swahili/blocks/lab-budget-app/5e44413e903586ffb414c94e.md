...
title: Jenga programu ya bajeti
challengeType: 27
...
Katika maabara hii, utajenga programu rahisi ya bajeti inayofuatilia matumizi katika makundi tofauti na inaweza kuonyesha asilimia ya matumizi kwa chati.

**Lengo:** Timiza hadithi za watumizi zilizo hapa chini na upite vipimo vyote ili kumaliza maabara.

**Hadithi za watumizi:**

1. Unapaswa kuwa na darasa la vitu `Category` ambalo linakubali jina kama hoja.
1. Darasa la `Category` linapaswa kuwa na sifa ya kipengele cha mfano `ledger` ambayo ni orodha, na ina orodha ya miamala.
1. Darasa la `Category` linapaswa kuwa na njia zifuatazo:

   - Njia ya `deposit` inayokubali kiasi na maelezo ya hiari. Ikiwa hakuna maelezo yanayotolewa, inapaswa kuwa mfuatano wa herufi tupu. Njia hii inapaswa kuongeza kitu kwenye orodha ya `ledger` kwa njia ya `{'amount': amount, 'description': description}`.
   - Njia ya `withdraw` inayokubali kiasi na maelezo ya hiari (kawaida mfuatano wa herufi tupu). Njia hii inapaswa kuhifadhi `ledger` kiasi kilichopitishwa kama nambari hasi, na inapaswa kurudisha `True` ikiwa uondoaji ulifanikiwa na `False` vinginevyo.
   - Njia ya `get_balance` inayorudisha salio la sasa la kundi kulingana na `ledger`.
   - Njia ya `transfer` inayokubali kiasi na mfano mwingine wa `Category`, inachukua kiasi na maelezo `Transfer to [Destination]`, kisha kuweka kiasi kwenye kundi jingine na maelezo `Transfer from [Source]`, ambapo `[Destination]` na `[Source]` zinapaswa kubadilishwa na majina ya makundi ya marudio na chanzo. Njia hii inapaswa kurudisha `True` wakati uhamisho unafanikishwa, na `False` vinginevyo.
   - Njia ya `check_funds` inayokubali kiasi na kurudisha `False` ikiwa kiasi kinazidi salio au `True` vinginevyo. Njia hii lazima itumike na njia za `withdraw` na `transfer`.

1. Wakati kitu cha `Category` kinachapishwa, kinapaswa:  
   - Onyesha mstari wa kichwa wa herufi 30 na jina la kundi likiwa katikati kati ya herufi za `*`.  
   - Orodhesha kila kipengele cha `ledger` kwa maelezo ya herufi hadi 23 upande wa kushoto na kiasi upande wa kulia (nafasi mbili za desimali, herufi 7 kwa jumla).  
   - Onyesha mstari wa mwisho wa `Total: [balance]`, ambapo `[balance]` inapaswa kubadilishwa na jumla ya kundi.
...
1. Unapaswa kuwa na kitendakazi nje ya darasa la `Category` kinachoitwa `create_spend_chart(categories)` kinachopokea orodha ya makundi na kurudisha mfuatano wa herufi wa chati ya nguzo. Ili kujenga chati:

   - Anza na kichwa `Percentage spent by category`.
   - Hesabu asilimia kutoka tu kwa uondoaji na si kwa amana. Asilimia inapaswa kuwa asilimia ya kiasi kilichotumika kwa kila kundi kwa jumla ya matumizi yote ya makundi yote (kuzungushwa chini hadi nambari 10 inayofuatana).
   - Weka lebo ya mhimili wa y kutoka `100` hadi `0` kwa hatua za 10.
   - Tumia herufi za `o` kwa nguzo.
   - Jumuisha mstari wa usawa umbali wa nafasi mbili baada ya nguzo ya mwisho.
   - Andika majina ya makundi wima chini ya nguzo.

   Kitendakazi hiki kitajaribiwa na makundi hadi manne.

   Hakikisha nafasi za matokeo zinafanana kabisa na mfano:
...
KUMBUKA: fungua konsoli ya kivinjari kwa F12 kuona matokeo ya vipimo kwa undani zaidi.

# --hints--

Njia ya `deposit` inapaswa kuunda kitu maalum katika sifa ya mfano ya ledger.
...
Njia ya `withdraw` inapaswa kuunda kitu maalum katika sifa ya mfano ya `ledger`.
...
Kupiga wito wa njia ya `transfer` kwenye kitu cha kundi kunapaswa kuunda kipengele maalum cha ledger katika kitu hicho cha kundi.
...
Njia ya `transfer` inapaswa kuongeza salio la kitu cha kundi kilichopitishwa kama hoja.
...
Njia ya `transfer` inapaswa kuunda kipengele maalum cha ledger katika kitu cha kundi kilichopitishwa kama hoja.
...
Njia ya `check_funds` inapaswa kurudisha `False` ikiwa kiasi kilichopitishwa kwa njia ni kikubwa kuliko salio la kundi.
...
Njia ya `check_funds` inapaswa kurudisha `True` ikiwa kiasi kilichopitishwa si kikubwa kuliko salio la kundi.
...
Kuchapisha mfano wa `Category` kunapaswa kutoa uwakilishi tofauti wa mfuatano wa herufi wa kitu.
...
Kichwa juu ya chati ya `create_spend_chart` kinapaswa kusema `Percentage spent by category`.
...
Chati ya `create_spend_chart` inapaswa kuwa na asilimia sahihi upande wa kushoto.
...
Urefu wa kila nguzo kwenye chati ya `create_spend_chart` unapaswa kuzungushwa chini hadi nambari 10 inayofuatana.
...
Kila mstari katika chati ya `create_spend_chart` unapaswa kuwa na urefu sawa. Nguzo za makundi tofauti zinapaswa kutenganishwa kwa nafasi mbili, na nafasi mbili za ziada baada ya nguzo ya mwisho.
...
`create_spend_chart` inapaswa kuonyesha mstari wa usawa chini ya nguzo. Kutumia herufi tatu za `-` kwa kila kundi, na kwa jumla kwenda herufi mbili zaidi baada ya nguzo ya mwisho.
...
Chati ya `create_spend_chart` haipaswi kuwa na tabia ya mstari mpya mwishoni.
...
Chati ya `create_spend_chart` inapaswa kuwa na kila jina la kundi limeandikwa wima chini ya nguzo. Kila mstari unapaswa kuwa na urefu sawa, kila kundi linapaswa kutenganishwa kwa nafasi mbili, na nafasi mbili za ziada baada ya kundi la mwisho.
...
`create_spend_chart` inapaswa kuchapisha uwakilishi tofauti wa chati. Hakikisha nafasi zote ni sahihi. Fungua konsoli ya kivinjari kwa F12 kwa maelezo zaidi.
