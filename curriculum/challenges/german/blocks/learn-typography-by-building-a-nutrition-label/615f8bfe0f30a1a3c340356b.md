...
Erstellen Sie ein weiteres `p`-Element, geben Sie ihm den Text `Calcium 260mg 20%`. Richten Sie `20%` rechtsbündig aus. Erstellen Sie darunter ein `p`-Element mit dem Text `Iron 8mg 45%` und richten Sie `45%` rechtsbündig aus.

# --hints--

Sie sollten zwei neue `p`-Elemente am Ende Ihres `.daily-value`-Elements erstellen.
...
Ihr erstes neues `p`-Element sollte den Text `Calcium 260mg 20%` enthalten.
...
Ihr erstes neues `p`-Element sollte ein `span`-Element enthalten.
...
Ihr erstes `span`-Element sollte nicht fett sein.
...
Ihr erstes `span`-Element sollte den Text `20%` umschließen.
