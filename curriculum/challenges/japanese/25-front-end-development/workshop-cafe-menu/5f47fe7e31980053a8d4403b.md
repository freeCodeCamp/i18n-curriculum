...
より簡単な方法として、マイナスの値の上部マージンを `img` 要素に追加して、画像を現在の位置から引き上げることもできます。 マイナスの値は `-` を値の前につけることで作成します。 To complete this workshop, go ahead and use a negative top margin of `25px` in the `img` type selector.
