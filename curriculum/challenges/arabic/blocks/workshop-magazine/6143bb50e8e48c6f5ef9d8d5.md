...
داخل عنصر `aside`، أنشئ عنصرين `img`، وعنصر `blockquote`، وعنصر `img` ثالث. عيّن للعنصر `blockquote` صنفًا `class` بقيمة `image-quote`.

# --hints--

يجب أن تنشئ ثلاثة عناصر `img` داخل عنصر `aside`.

```js
assert.lengthOf(document.querySelectorAll('aside img'), 3);
```

يجب أن تنشئ عنصر `blockquote` داخل عنصر `aside`.
