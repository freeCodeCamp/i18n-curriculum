...
Erstellen Sie eine `DiscountEngine`-Instanz und speichern Sie sie in der Variablen `engine`. Berechnen Sie danach den besten Preis mit der `calculate_best_price`-Methode und speichern Sie ihn in einer Variablen namens `best_price`.

# --hints--

Sie sollten eine `DiscountEngine`-Instanz mit der `strategies`-Liste erstellen.
...
Sie sollten `engine.calculate_best_price(product, user_tier)` aufrufen und das Ergebnis in `best_price` speichern.
