...
Il selettore `h1 .flex` dovrebbe avere una proprietà `display` con il valore `flex`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1 .flex')?.getPropertyValue('display') === 'flex');
```

Il selettore `h1 .flex` dovrebbe avere una proprietà `flex-direction` con il valore `column-reverse`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1 .flex')?.getPropertyValue('flex-direction') === 'column-reverse');
```

Il selettore `h1 .flex` dovrebbe avere una proprietà `gap` con il valore `1rem`.
