...
Scrivi una funzione per risolvere \\(Ax = b\\) usando l'eliminazione gaussiana e poi la sostituzione all'indietro.

\\(A\\) è una matrice \\(n \\times n\\). Inoltre, \\(x\\) e \\(b\\) sono vettori \\(n\\) per 1.

Per migliorare la precisione, usa il pivot parziale e la scalatura.
