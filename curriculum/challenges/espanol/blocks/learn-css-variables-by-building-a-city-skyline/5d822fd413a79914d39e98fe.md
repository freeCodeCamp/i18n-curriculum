...
Cree y agregue las siguientes propiedades a `.bb2a`:
...
Después de agregar estas, puede ver cómo un borde grueso en un elemento le proporciona algunos ángulos donde se encuentran dos lados. Vas a usar ese borde inferior como la parte superior del edificio.
...
Debe dar a `.bb2a` una `height` de `5vw`.
...
Debe dar a `.bb2a` un `border-right` de `1vw solid #999`.
