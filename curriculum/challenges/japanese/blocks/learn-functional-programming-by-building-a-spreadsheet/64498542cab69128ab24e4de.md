...
セルの文字に正しくマッチした後、`rangeRegex` はセル番号にもマッチする必要があります。シート内のセル番号は `1` から `99` までの範囲です。

文字のキャプチャグループの後にキャプチャグループを追加してください。新しいキャプチャグループは1桁または2桁の数字にマッチする必要があります。最初の数字は `1` から `9` の範囲で、2番目の数字は `0` から `9` の範囲です。2番目の数字は省略可能です。

# --hints--

`rangeRegex` に2つ目のキャプチャグループを追加してください。
...
2つ目のキャプチャグループは文字クラスを持つべきです。

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[.*\]\??\)\/\s*;?/);
```

2つ目のキャプチャグループは2つの文字クラスを持つべきです。

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[.*\]\[.*\]\??\)\/\s*;?/);
```

最初の新しい文字クラスは `1` から `9` の数字にマッチするべきです。
...
2つ目の新しい文字クラスは `0` から `9` の数字にマッチするべきです。
