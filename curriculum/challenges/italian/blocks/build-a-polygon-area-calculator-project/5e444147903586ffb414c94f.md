...
title: Costruisci un progetto calcolatore di area di un poligono
challengeType: 23
...
Quando viene creato un oggetto Rectangle, dovrebbe essere inizializzato con gli attributi `width` e `height`. La classe dovrebbe contenere anche i seguenti metodi:
...
- `get_picture`: restituisce una stringa che rappresenta la forma usando righe di '\*'. Il numero di righe dovrebbe essere uguale all’altezza e il numero di '\*' in ogni riga dovrebbe essere uguale alla larghezza. Alla fine di ogni riga ci dovrebbe essere un a capo (`\n`). Se la larghezza o l’altezza sono maggiori di 50, questa dovrebbe restituire la stringa: `'Too big for picture.'`.
- `get_amount_inside`: prende un’altra forma (quadrato o rettangolo) come argomento. Restituisce quante volte la forma passata potrebbe entrare dentro la forma (senza rotazioni). Per esempio, un rettangolo con larghezza 4 e altezza 8 potrebbe contenere due quadrati con lato 4.

Inoltre, se un’istanza di `Rectangle` viene rappresentata come stringa, dovrebbe apparire così: `'Rectangle(width=5, height=10)'`.

## Classe Square

La classe `Square` dovrebbe essere una sottoclasse di `Rectangle`. Quando viene creato un oggetto `Square`, viene passato un solo lato. Il metodo `__init__` dovrebbe memorizzare la lunghezza del lato sia negli attributi `width` che `height` della classe `Rectangle`.

La classe `Square` dovrebbe poter consultare i metodi della classe `Rectangle` ma dovrebbe contenere anche un metodo `set_side`. Se un’istanza di `Square` viene rappresentata come stringa, dovrebbe apparire così: `'Square(side=9)'`.
