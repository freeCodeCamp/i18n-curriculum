...
你的 `decimalToBinary` 函數運行良好，但存在一個問題 - 由於你的 `while` 循環中的條件，它僅適用於大於 `0` 的數字。 如果你嘗試將 `0` 轉換爲二進制，則不會向頁面添加任何內容。

爲了解決這個問題，請添加一個 `if` 語句來檢查 `input` 是否等於 `0`。 暫時將 `if` 語句的主體保留爲空。

# --hints--

你應該添加一個 `if` 語句來檢查 `input` 是否等於 `0`。
