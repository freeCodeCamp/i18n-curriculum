...
Kipengele cha `address` hakihitaji kuwa na eneo la kijiografia la kimwili. Kinaweza kutumika kutoa kiungo unganisho kwa somo.

Funga kiungo kuzunguka maandishi ya `freeCodeCamp`, na weka eneo lake kuwa `https://freecodecamp.org`.

# --hints--

Unapaswa kuongeza kipengele cha `a`.

```js
assert.exists(document.querySelector('address > a'));
```

Unapaswa kumpa kipengele cha `a` sifa ya kipengele ya `href` yenye thamani ya `https://freecodecamp.org`.

```js
assert.equal(document.querySelector('address > a')?.getAttribute('href'), 'https://freecodecamp.org');
```

Unapaswa kufunga kipengele cha `a` kuzunguka maandishi yaliyopo ya `freeCodeCamp`.
