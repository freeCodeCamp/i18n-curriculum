...
Usa il metodo `filter()` per rimuovere l'oggetto canzone che corrisponde al parametro `id` dall'array `userData?.songs`.

Il metodo `filter` mantiene solo gli elementi di un array che soddisfano la funzione di callback passata:
...
Usa il metodo `filter()` su `userData?.songs`. Passa `song` come parametro della funzione freccia di callback e usa il return implicito per verificare se `song.id` è strettamente diverso da `id`. Assegna tutto questo a `userData.songs`.

*Nota*: non dovresti usare il concatenamento opzionale quando assegni il risultato di `userData?.songs.filter` a `userData.songs` perché l'array `allSongs` non sarà `undefined` o `null` a quel punto.

# --hints--

Non dovresti modificare la funzione `deleteSong` esistente.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Dovresti usare il metodo `filter()` su `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

La funzione di callback del tuo metodo `filter()` dovrebbe usare la sintassi freccia e avere un parametro `song`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

Il tuo metodo filter dovrebbe restituire implicitamente `song.id !== id`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

Dovresti assegnare il metodo `filter()` che hai usato su `userData?.songs` a `userData.songs`.
