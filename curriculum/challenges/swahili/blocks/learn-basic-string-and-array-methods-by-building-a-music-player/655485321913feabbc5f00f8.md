...
Ongeza msikilizaji wa tukio la `"click"` kwenye kipengele cha `previousButton`, kisha pita `playPreviousSong` kama hoja ya pili.

# --hints--

Unapaswa kuita njia ya `addEventListener()` kwenye kigezo chako cha `previousButton`.
...
Unapaswa kupitisha `playPreviousSong` kama hoja ya pili ya njia yako ya `addEventListener`.
