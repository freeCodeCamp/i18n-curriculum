...
الآن يجب أن تُرجع دالة `applyFunction` نتيجة. أرجع نتيجة استدعاء طريقة `.replace()` على `str2`. مرر تعبيرك النمطي `functionCall` واستدعاء راجع فارغ.

# --hints--

يجب أن تُرجع دالة `applyFunction` نتيجة استدعاء طريقة `.replace()` على `str2`.
...
يجب أن تمرر `functionCall` كالمعلمة الأولى لاستدعاء `.replace()`.
...
يجب أن تمرر دالة سهمية فارغة كالمعلمة الثانية لاستدعاء `.replace()`.
