...
Estamos usando el `input` de correo electrónico que incluye validación incorporada para verificar validaciones básicas como si la entrada incluye el signo arroba (`@`).
...
Informa el estado inválido de inmediato, en lugar de esperar a que enviemos el formulario. Pero aún usa el mensaje predeterminado. Esto se debe a que el método `reportValidity()` solo le dice al navegador que la entrada es inválida. El navegador sigue eligiendo cómo mostrar por qué es inválida. Ahí es donde entra el método `setCustomValidity()`.
...
Este método acepta un mensaje de error personalizado, que se muestra al usuario. Como resultado, verás el mensaje de error personalizado `You must use a company email address that ends in @sampleCompany.com`.
...
Hay varias propiedades útiles que todas tienen un valor booleano de `true` o `false`.

Algunas de estas propiedades útiles que puedes explorar por tu cuenta serían la propiedad `valueMissing`, que es `true` cuando un campo de entrada requerido queda vacío, o la propiedad `patternMismatch`, que es `true` si el valor no coincide con el patrón de expresión regular especificado.
