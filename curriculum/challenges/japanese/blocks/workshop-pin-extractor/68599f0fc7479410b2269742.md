...
関数の引数を `poems` に更新し、関数の現在の内容すべての周りに `for` ループを作成して、`poems` を反復し、`poem` をループ変数として使用してください。

# --hints--

`pin_extractor` 関数には `poems` という名前のパラメータが必要です。
...
関数の既存の本体を新しい `for` ループの中に移動してください。
