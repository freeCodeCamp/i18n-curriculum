...
包含剩下的引數 `chosenCard.name_short` 和 `chosenCard.img`。然後在 `renderCard` 函式外，將 `multipleCard` 元素的 `innerHTML` 重設為空字串。

# --hints--

你應該將 `chosenCard.name_short` 作為引數傳入 `renderCard`。
...
你應該將 `chosenCard.img` 作為最後一個引數傳入 `renderCard`。
...
你應該將 `this.elements.multipleCard.innerHTML` 重設為空字串。
