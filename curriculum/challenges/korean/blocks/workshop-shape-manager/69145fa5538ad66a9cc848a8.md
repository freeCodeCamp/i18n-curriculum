...
`updateResult` 함수는 드롭다운에서 선택한 도형 유형을 읽고, 해당 입력 값을 모아 `shape` 객체를 생성한 뒤, `calculateArea()` 함수에 전달해 영역을 계산하고, 계산된 결과를 `resultText` 요소에 표시합니다.

함수 내부에서 선택한 도형을 확인하고 `calculateArea` 함수에 전달할 올바른 객체를 만드세요.

도형이 `"circle"`라면, 타입과 반지름 입력의 숫자 값을 포함하는 객체를 `calculateArea` 함수에 호출하세요.

# --hints--

`updateResult` 함수는 도형이 원일 때 결과 텍스트를 올바르게 업데이트해야 합니다.
