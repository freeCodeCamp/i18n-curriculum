...
Você precisa re-renderizar as músicas, destacá-las e definir o texto acessível do botão de reprodução já que a lista de músicas vai mudar.

Chame a função `renderSongs` e passe o array `userData?.songs` como argumento, isso exibe a playlist modificada.

Depois disso, chame a função `highlightCurrentSong` para destacar a música atual se houver alguma e a função `setPlayButtonAccessibleText` para atualizar o texto acessível do botão de reprodução.

# --hints--

Você não deve modificar a função `deleteSong` existente nem seu conteúdo.
...
Você deve chamar a função `renderSongs` com `userData?.songs`.
