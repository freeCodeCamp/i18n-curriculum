...
標量乘法工作正常。 現在，將 `v5` 的賦值修改爲點積 `v1 * v2`。 另外，更新 `print` 調用。

在執行此操作之前，請隨意嘗試並將 `v1` 乘以無效類型的對象，以查看控制檯上打印的錯誤消息。


# --hints--

你應該更新 `v5` 的分配，將其分配給標量積 `v1 * v2`。

```js
({ test: () => assert(runPython(`_Node(_code).find_variable("v5").is_equivalent("v5 = v1 * v2")`)) })
```

你應該更新你的 `print` 調用以打印 f 字符串 `f'v1 * v2 = {v5}'`。
