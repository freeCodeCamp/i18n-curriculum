...
`textContent` 设置节点的文本并允许你设置或检索 HTML 元素的文本内容。
...
你可以使用三元运算符有条件地设置文本内容值。 以下是将三元运算符的结果分配给变量的示例：

```js
const example = condition ? "I'm true" : "I'm false";
```

使用三元运算符检查 `currentTitle` 是否计算为真值。 如果是，则将 `playingSong.textContent` 设置为 `currentTitle`。 否则，将其设置为空字符串。

然后在下面使用 `ternary` 运算符来检查 `currentArtist` 是否为真。 如果是，则将 `songArtist.textContent` 设置为 `currentArtist`。 否则，将其设置为空字符串。 

# --hints--

你应该将 `textContent` 属性链接到 `playingSong`。

```js
assert.match(code, /playingSong\.textContent\s*/)
```

你应该使用 `ternary` 运算符将 `playingSong` 的 `textContent` 属性设置为 `currentTitle` 或 `""`。

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

你应该将 `textContent` 属性链接到 `songArtist`。

```js
assert.match(code, /songArtist\.textContent\s*/)
```

你应该使用 `ternary` 运算符将 `songArtist` 的 `textContent` 属性设置为 `currentArtist` 或 `""`。
