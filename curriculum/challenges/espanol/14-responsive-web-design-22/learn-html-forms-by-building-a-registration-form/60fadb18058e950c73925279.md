...
Puedes utilizar tanto el valor `none` o `0` para eliminar el `border`.

```js
assert.match(new __helpers.CSSHelp(document).getStyle('fieldset')?.border, /(none)|(0px)|(medium)/);
```

Debes agregar un `padding` de `2rem` a la parte superior e inferior de cada `fieldset`.
...
Debes eliminar el `padding` izquierdo y derecho de cada `fieldset`.
