...
Tu `Cholesterol` `span` debería tener el atributo `class` establecido en `bold`.
...
Tu `0%` `span` debería tener el atributo `class` establecido en `bold`.
...
Tu `Cholesterol` `span` y tu texto `0mg` deberían estar envueltos en un `span`.
