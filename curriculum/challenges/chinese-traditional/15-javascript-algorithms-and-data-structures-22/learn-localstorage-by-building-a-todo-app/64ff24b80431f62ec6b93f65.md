...
刪除 `myTaskArr` 數組和 `localStorage` 的所有代碼，因爲你不再需要它們。

# --hints--

你應該刪除 `myTaskArr` 以及你剛剛學到的與 `localStorage` 相關的所有代碼。
...
你應該移除代碼中任何地方對 `myTaskArr` 的引用。

```js
assert.notMatch(code, /myTaskArr/);
```

你應該移除對 `"data"` 元素 `localStorage.getItem` 的任何引用。

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

你應該從代碼中移除對 `localStorage.clear()` 的任何引用。

```js
assert.notMatch(code, /localStorage\.clear\(\s*\)\s*;?/);
```

你應該移除代碼中任何地方對 `getTaskArrObj` 的引用。
