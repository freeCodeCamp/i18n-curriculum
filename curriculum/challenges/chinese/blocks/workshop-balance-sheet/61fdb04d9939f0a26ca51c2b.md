...
是时候为表格标题设置样式了。 创建一个 `h1` 选择器。 给它一个 `max-width` 属性，设置为 `37.25rem`；一个 `margin` 属性，设置为 `0 auto`；以及一个`padding` 属性，设置为 `1.5rem 1.25rem`。
...
你的 `h1` 选择器应该有一个 `max-width` 属性，设置为 `37.25rem`。
...
你的 `h1` 选择器应该有一个 `margin` 属性，设置为 `0 auto`。
...
你的 `h1` 选择器应该有一个 `padding` 属性，设置为 `1.5rem 1.25rem`。
