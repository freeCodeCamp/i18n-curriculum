...
Encapsule tudo dentro do elemento `.label` em um novo elemento `header`.

# --hints--

Você deve criar um novo elemento `header`.

```js
assert(document.querySelector('header'));
```

O elemento `header` deve estar dentro do elemento `.label`.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

Os elementos `h1`, `div` e `p` devem estar dentro do novo elemento `header`.
