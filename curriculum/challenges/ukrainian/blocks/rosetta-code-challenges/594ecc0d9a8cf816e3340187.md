...
Число таксі (визначення, яке тут використовується) — це додатне ціле число, яке можна подати як суму двох додатних кубів більш ніж одним способом.

Перше число таксі — це `1729`, яке дорівнює:

1<sup>3</sup> + 12<sup>3</sup> та

9<sup>3</sup> + 10<sup>3</sup>.

Числа таксі також відомі як:

<ul>
  <li>числа таксі</li>
  <li>числа taxi-cab</li>
  <li>числа taxi cab</li>
  <li>числа Гарді-Рамануджана</li>
</ul>

# --instructions--

Напишіть функцію, яка повертає найменші `n` чисел таксі. Для кожного числа таксі покажіть саме число та його складові куби.
...
`taxicabNumbers` має повертати масив.

```js
assert(typeof taxicabNumbers(2) === 'object');
```

`taxicabNumbers` має повертати масив чисел.

```js
assert(typeof taxicabNumbers(100)[0] === 'number');
```

`taxicabNumbers(4)` має повертати [1729, 4104, 13832, 20683].

```js
assert.deepEqual(taxicabNumbers(4), res4);
```

`taxicabNumbers(25)` має повертати [1729, 4104, 13832, 20683, 32832, 39312, 40033, 46683, 64232, 65728, 110656, 110808, 134379, 149389, 165464, 171288, 195841, 216027, 216125, 262656, 314496, 320264, 327763, 373464, 402597]

```js
assert.deepEqual(taxicabNumbers(25), res25);
```

`taxicabNumbers(39)` числа від 20 до 29 мають бути [314496,320264,327763,373464,402597,439101,443889,513000,513856].
