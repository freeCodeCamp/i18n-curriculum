...
Sie möchten den Warenkorb nur löschen, wenn der Benutzer die Eingabeaufforderung bestätigt. Erstellen Sie eine `if`-Anweisung, die überprüft, ob der Benutzer die Eingabeaufforderung bestätigt hat.

Setzen Sie in der `if`-Anweisung die `items`-Eigenschaft wieder auf ein leeres Array und anschließend die `total`-Eigenschaft auf `0`.

# --hints--

Sie sollten eine `if`-Anweisung erstellen, die überprüft, ob der Benutzer die Eingabeaufforderung bestätigt hat. Denken Sie daran, dass Sie die Wahrhaftigkeit von `isCartCleared` direkt überprüfen können.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

Ihre `if`-Anweisung sollte die `items`-Eigenschaft wieder auf ein leeres Array setzen.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

Ihre `if`-Anweisung sollte die `total`-Eigenschaft auf `0` setzen.
