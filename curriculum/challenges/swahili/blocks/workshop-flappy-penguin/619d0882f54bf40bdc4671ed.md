...
Mahali panapofaa, badilisha thamani za sifa kwa `--penguin-face` yako.

# --hints--

Unapaswa kumpa `.face` `background-color` ya `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Unapaswa kumpa `.chin` `background-color` ya `var(--penguin-face)`.
