...
Agora é hora de testar o botão de pausa.

Adicione um listener de evento `"click"` ao elemento `pauseButton`, depois passe `pauseSong` como o segundo argumento do listener de evento. Esta é a função que o listener de evento irá executar.

Teste seu app clicando primeiro no botão de play seguido pelo botão de pause. Você deve ver que tudo está funcionando conforme o esperado.

# --hints--

Você deve chamar o método `addEventListener()` na sua variável `pauseButton`.
...
Você deve passar `pauseSong` como o segundo argumento do seu método `addEventListener`.
