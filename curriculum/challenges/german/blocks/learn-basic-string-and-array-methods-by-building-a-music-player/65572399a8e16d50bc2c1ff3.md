...
Wenn eine Übereinstimmung vorliegt, setzen Sie `userData.currentSong` auf `null` und `userData.songCurrentTime` auf `0`.

Rufen Sie danach die `pauseSong()`-Funktion auf, um die Wiedergabe zu stoppen, und die `setPlayerDisplay()`-Funktion, um die Anzeige des Players zu aktualisieren.

# --hints--

Sie sollten die vorhandene `if`-Anweisung und deren Inhalt nicht verändern.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\?\.id\s*===\s*id\s*\)\s*\{\s*.*\s*.*\s*.*\s*.*\s*\}/)
```

Sie sollten `userData.currentSong` auf `null` setzen.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\?\.id\s*===\s*id\s*\)\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*.*\s*.*\s*\}/)
```

Sie sollten `userData.songCurrentTime` auf `0` setzen.
