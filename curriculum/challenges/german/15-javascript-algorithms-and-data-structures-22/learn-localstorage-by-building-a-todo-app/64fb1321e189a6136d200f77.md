...
Du solltest `titleInput.value` auf `currentTask.title` setzen.

```js
assert.match(editTask.toString(), /titleInput\.value\s*=\s*currentTask\.title\s*;?/)
```

Du solltest `dateInput.value` auf `currentTask.date` setzen.

```js
assert.match(editTask.toString(), /dateInput\.value\s*=\s*currentTask\.date\s*;?/)
```

Du solltest `descriptionInput.value` auf `currentTask.description` setzen.
