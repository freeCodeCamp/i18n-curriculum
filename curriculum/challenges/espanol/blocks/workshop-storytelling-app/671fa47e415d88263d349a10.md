...
A continuación, comenzarás a trabajar en el `JavaScript`. Para eso, comienza enlazando el archivo `script.js` a tu HTML.

Crea un elemento `script` con su atributo `src` apuntando al archivo `script.js`. Por ejemplo: 
...
Debes crear un elemento `script`.

```js
assert.lengthOf(document.querySelectorAll('script'), 1);
```

Tu elemento `script` debe tener un atributo `src` configurado a `script.js`.
