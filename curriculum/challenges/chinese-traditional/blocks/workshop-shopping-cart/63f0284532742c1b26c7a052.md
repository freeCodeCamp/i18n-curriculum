...
為你的 `forEach` 方法添加回呼函式。它應該帶有一個 `btn` 參數。然後，在回呼中，為 `btn` 添加一個事件監聽器。事件監聽器應該監聽 `click` 事件，並且應該帶有另一個帶有 `event` 參數的回呼。第二個回呼應該是空的。

# --hints--

你應該使用箭頭語法為 `forEach` 方法添加一個帶有 `btn` 參數的回呼函式（程式）。

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(\s*(\(\s*btn\s*\)|btn)\s*=>\s*(?:\{|\s*)/);
```

你應該為你的 `btn` 變數添加一個事件監聽器。
...
你應該為事件監聽器添加一個空的回呼函式。記得給它一個 `event` 參數。
