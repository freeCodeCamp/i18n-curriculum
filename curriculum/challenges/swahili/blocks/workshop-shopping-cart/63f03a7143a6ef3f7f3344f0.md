...
Unataka kufuta kikapu tu ikiwa mtumizi atathibitisha onyo. Unda usemi wa `if` unaothibitisha kama mtumizi amethibitisha onyo.

Katika usemi wa `if`, weka sifa ya `items` kurudi kuwa safu ya data tupu, kisha weka sifa ya `total` kuwa `0`.

# --hints--

Unapaswa kuunda usemi wa `if` unaothibitisha kama mtumizi amethibitisha onyo. Kumbuka unaweza kuangalia uhalisia wa `isCartCleared` moja kwa moja.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

Usemi wako wa `if` unapaswa kuweka sifa ya `items` kurudi kuwa safu ya data tupu.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

Usemi wako wa `if` unapaswa kuweka sifa ya `total` kuwa `0`.
