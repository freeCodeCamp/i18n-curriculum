...
`It was mentioned...` is also a structure in the **passive voice**, where the focus is on the idea being shared, not on who said it. Por ejemplo:

`It was mentioned that the update improved security.` – This tells you the idea without naming the speaker.

The structure `it's` + comparative adjective + `to` + verb is used to show how something is done in contrast with something else. Por ejemplo:
