...
Crea una regola `.image-1, .image-3` e assegna a essa una proprietà `grid-column` impostata su `1 / -1`. Questo permetterà alla prima e alla terza immagine di estendersi per tutta la larghezza della griglia.

# --hints--

Dovresti avere un selettore `.image-1, .image-3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

Il tuo selettore `.image-1, .image-3` dovrebbe avere una proprietà `grid-column` impostata su `1 / -1`.
