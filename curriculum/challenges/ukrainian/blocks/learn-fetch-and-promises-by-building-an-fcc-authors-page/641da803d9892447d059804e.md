...
Тепер створіть тег зображення і надайте йому `class` `"user-img"`. Використайте інтерполяцію рядків, щоб встановити атрибут `src` у значення `image`, яке ви деструктурували раніше. Встановіть атрибут `alt` у `author`, після чого додайте текст `"avatar"`. Переконайтеся, що між змінною `author` і словом `"avatar"` є пробіл, наприклад, `"Quincy Larson avatar"`.
