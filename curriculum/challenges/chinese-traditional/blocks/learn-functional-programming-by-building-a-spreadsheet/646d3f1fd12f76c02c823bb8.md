...
你有一個小錯誤。 `arg1` 和 `arg2` 是字符串，而不是數字。 `infixToFunction['+']("1", "2")` 將返回 `12`，這在數學上並不正確。

將每個 `infixToFunction[operator]` 參數包裝在 `parseFloat()` 調用中。

# --hints--

你應該將 `arg1` 包裝在 `parseFloat()` 調用中。
