...
Далі створіть конструктор для класу `Bug`, який приймає один параметр з іменем `emojiElement` типу `HTMLParagraphElement`. Усередині конструктора призначте параметр `emojiElement` властивості `emojiElement` класу.

# --hints--

Ваш клас `Bug` має мати конструктор.
...
Ваш конструктор `Bug` має мати параметр з іменем `emojiElement` типу `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Ваш конструктор `Bug` має призначати `emojiElement` `this.emojiElement`.
