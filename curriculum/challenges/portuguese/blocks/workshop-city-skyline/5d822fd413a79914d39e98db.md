...
Dê aos novos edifícios as propriedades `width` e `height` com os valores: `10%` e `50%` para `.bb2`, `10%` e `55%` para `.bb3`, `11%` e `58%` para `.bb4`.Você usará quase todas as unidades baseadas em porcentagem e alguns flexbox para este projeto, então, tudo será completamente responsivo.
