...
今、`v1` と `v2` の代入を変更してください。これらを `R3Vector` のインスタンスに変え、`z=1` と `z=2` をそれぞれ `v1` と `v2` に渡してください。

# --hints--

`v1` の代入を `R3Vector` のインスタンスに変更する必要があります。

```js
({ test: () => assert(runPython(`_Node(_code).find_variable("v1").is_equivalent("v1 = R3Vector(x=2, y=3, z=1)")`)) })
```

`v2` の代入を `R3Vector` のインスタンスに変更する必要があります。
