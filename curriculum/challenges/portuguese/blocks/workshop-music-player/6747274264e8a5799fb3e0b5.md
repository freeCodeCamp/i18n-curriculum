...
Adicione um segundo objeto com as seguintes chaves e valores:
...
Seu array `allSongs` deve ter dois valores.

```js
assert.equal(allSongs.length, 2);
```

Seu array `allSongs` deve ter um objeto como seu segundo valor.

```js
assert.isObject(allSongs[1]);
```

O segundo objeto no seu array `allSongs` deve ter uma propriedade `id` definida com o número `1`.

```js
assert.strictEqual(allSongs[1].id, 1);
```

O segundo objeto no seu array `allSongs` deve ter uma propriedade `title` definida com a string `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

O segundo objeto no seu array `allSongs` deve ter uma propriedade `artist` definida com a string `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

O segundo objeto no seu array `allSongs` deve ter uma propriedade `duration` definida como a string `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

O segundo objeto no seu array `allSongs` deve ter uma propriedade `src` definida para a string `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
