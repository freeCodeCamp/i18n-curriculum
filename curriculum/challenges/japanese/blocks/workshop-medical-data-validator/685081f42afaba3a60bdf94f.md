...
次に、`age`が整数であることを確認したいです。そこで、`age`辞書にもう1つのキー`constraints`を追加してください。その値には、`isinstance`と`age`を引数として渡して`int`を呼び出してください。

# --hints--

あなたの`constraints`辞書には、キー`age`があるはずです。
...
あなたの`age`辞書の`constraints`キーは、`isinstance(age, int)`の値を持つべきです。
