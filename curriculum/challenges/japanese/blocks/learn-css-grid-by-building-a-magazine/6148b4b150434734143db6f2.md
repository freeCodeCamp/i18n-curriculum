...
`grid-column` プロパティは、要素がどの列から始まりどの列で終わるかを決定することを覚えておいてください。グリッドの列数が不明な場合でも、要素を最後の列で止めたいことがあります。その場合、終了列に `-1` を使うことができます。

`.hero` セレクターを作成し、`grid-column` プロパティに `1 / -1` を設定してください。これにより、要素がグリッドの全幅にまたがるようになります。
...
`.hero` セレクターには、`grid-column` プロパティが `1 / -1` に設定されているはずです。
