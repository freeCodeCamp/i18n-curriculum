...
Katika hatua za awali, ulijifunza jinsi ya kutumia njia ya `sort()` kupanga nyimbo kwa mpangilio wa alfabeti. Matumizi mengine ya kitendakazi cha mwitiko ni kugeuza safu ya data kuwa ya mfuatano wa nasibu.

Njia moja ya kugeuza safu ya data kuwa ya nasibu ni kutoa `0.5` kutoka `Math.random()` ambayo huleta thamani za nasibu zenye kuwa chanya au hasi. Hii hufanya matokeo ya kulinganisha kuwa mchanganyiko wa thamani chanya na hasi, na kusababisha mpangilio wa nasibu wa vipengele.
...
Tumia njia ya `sort()` kwenye safu ya data ya `userData?.songs`. Pitia kitendakazi cha mwitiko kwa njia hiyo, na rudisha matokeo ya `Math.random() - 0.5`.


# --hints--

Haupaswi kubadilisha kitendakazi cha `shuffle` kilichopo.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

Unapaswa kutumia njia ya `sort()` kwenye `userData?.songs`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

Njia yako ya `sort()` inapaswa kuwa na kitendakazi cha mwitiko kinachotumia sintaksia ya mshale.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

Kitendakazi cha mwitiko cha njia yako ya `sort()` kinapaswa kurudisha `Math.random() - 0.5`.
