...
بعد حلقة `for` المسكنة، وجدت أصغر قيمة. يجب عليك تبديلها مع القيمة الحالية.

كما فعلت في فرز الفقاعات، استخدم متغير `temp` لاستخراج القيمة عند `array[i]`، ثم بدّل القيم عند `array[i]` و`array[minIndex]`.

# --hints--

بعد حلقة `for` المسكنة، يجب إعلان متغير `temp`.
...
يجب تعيين `array[minIndex]` إلى `array[i]`.
...
يجب تعيين `temp` إلى `array[minIndex]`.
