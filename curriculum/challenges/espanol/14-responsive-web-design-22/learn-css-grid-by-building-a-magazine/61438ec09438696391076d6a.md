...
Below your `.heading` element, create a new `section` element with the `class` set to `text`. Dentro de eso, crea un elemento `p` con la `class` establecida en `first-paragraph` y el siguiente texto:
...
Tu nuevo elemento `section` debe tener `class` establecido en `text`.
...
Su nuevo elemento `p` debe tener la `class` establecida en `first-paragraph`.
