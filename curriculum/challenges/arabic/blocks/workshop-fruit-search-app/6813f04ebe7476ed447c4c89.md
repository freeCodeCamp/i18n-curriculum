...
بعد ذلك، استدعِ `.json()` على الاستجابة واحتفظ بالنتيجة في متغير اسمه `data`. بما أنك لا تزال داخل دالة `async`، يجب استخدام `await` قبل `response.json()`.

# --hints--

يجب استدعاء `json()` على `response` وتخزينها في متغير `data`.
