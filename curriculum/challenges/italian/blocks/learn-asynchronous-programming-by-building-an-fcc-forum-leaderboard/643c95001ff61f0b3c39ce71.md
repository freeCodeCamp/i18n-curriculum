...
All'interno del metodo `map`, destruttura le seguenti prop dall'oggetto `item`:
...
Dovresti destrutturare le prop `id`, `title`, `views`, `posts_count`, `slug`, `posters`, `category_id`, `bumped_at` dall'oggetto `item`.
