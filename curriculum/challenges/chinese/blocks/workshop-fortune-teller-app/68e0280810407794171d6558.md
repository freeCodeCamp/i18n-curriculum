...
现在，添加一个带有类 `div` 的 `img-loader`。之后，创建一个 `img` 元素，并使用 `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}` 为图像设置一个条件式 URL。

# --hints--

你的 `div` 元素内应该有一个 `figure` 元素。

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

你的 `div` 应该有一个类 `img-loader`。

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

你的 `img` 元素内应该有一个 `figure` 元素。

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

当提供 `img` 时，`CDN_URL` 元素应使用 `img` 和 `src` 作为其 `img`，否则回退到 `LOCAL_DEFAULT_IMG`。
...
你的 `img` 元素应具有类 `card-img` 和 `hidden`。
