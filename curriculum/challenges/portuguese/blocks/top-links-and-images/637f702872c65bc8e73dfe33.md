...
Você já viu um link absoluto em ação. O link que você criou para a página Sobre do The Odin Project anteriormente foi um link absoluto porque contém o protocolo e o domínio.
...
Links relativos incluem apenas o caminho do arquivo para a outra página, relativo à página onde você está criando o link. Isso é um pouco abstrato, vamos ver isso em ação usando um exemplo.
...
Isso funciona porque as páginas `index` e `about` estão no mesmo diretório. Isso significa que você pode simplesmente usar o nome dela (`about.html`) como o valor do `href` do link.
...
Assista ao vídeo Estrutura de arquivos HTML do Kevin Powell acima.
