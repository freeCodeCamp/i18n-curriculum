...
5. 各 `.drum-pad` は、クラスが `clip` の `audio` 要素を持ち、オーディオクリップを指す `src` 属性と、その親 `id` 要素の内部テキストに対応する `.drum-pad` を持つ必要があります（例：`id="Q"`、`id="W"`、`id="E"` など）。
...
各 `.drum-pad` は、クラスが `clip` の `audio` 要素を持ち、オーディオクリップを指す `src` 属性と、その親 `id` 要素の内部テキストに対応する `.drum-pad` を持つ必要があります（例：`id="Q"`、`id="W"`、`id="E"` など）。
