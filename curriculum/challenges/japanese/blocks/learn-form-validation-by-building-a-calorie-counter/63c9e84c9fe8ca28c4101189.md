...
今、`p` 要素を作成し、テキストに `budgetCalories Calories Budgeted` を設定してください。補間を使って `budgetCalories` を適切な変数に置き換えます。

これは `hr` 要素の後に配置してください。
...
`p` 要素のテキストは `${budgetCalories} Calories Budgeted` にしてください。
