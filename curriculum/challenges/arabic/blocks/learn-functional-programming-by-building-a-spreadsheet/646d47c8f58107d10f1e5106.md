...
المعلمة الأولى لاستدعاء `evalFormula` يجب أن تكون محتويات الخلية (التي خزنتها في `value`). مع ذلك، تبدأ المحتويات بحرف `=` لتفعيل الدالة، لذا يجب أن تمرر الجزء الفرعي من `value` بدءًا من الفهرس `1`.

# --hints--

يجب أن تمرر `value` كالمعلمة الأولى لاستدعاء `evalFormula()`.
...
يجب أن تستدعي الطريقة `.slice()` على المعلمة `value`.
...
يجب أن تمرر الرقم `1` كمعلمة لاستدعاء `.slice()`.
