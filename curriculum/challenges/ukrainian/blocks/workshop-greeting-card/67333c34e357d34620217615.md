...
Створіть селектор для елементів `section` і надайте їм:

- властивість `margin` зі значенням `20px auto`,
- множину `max-width` зі значенням `600px`,
- властивість `background-color` зі значенням `whitesmoke`,
- властивість `padding` зі значенням `20px`,
- властивість `border-radius` зі значенням `10px`

# --hints--

Ви маєте мати селектор `section`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle("section"));
```

Селектор `section` повинен мати властивість `margin` зі значенням `20px auto`.
...
Селектор `section` повинен мати властивість `max-width` зі значенням `600px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("max-width"), "600px");
```

Селектор `section` повинен мати властивість `background-color` зі значенням `whitesmoke`.
...
Селектор `section` повинен мати властивість `padding` зі значенням `20px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("padding"), "20px");
```

Селектор `section` повинен мати властивість `border-radius` зі значенням `10px`.
