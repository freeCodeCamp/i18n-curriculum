...
이제 공간을 만들고, 모든 버튼 변수를 제거하며 각 버튼에 대해 만든 `eventListener`도 삭제하세요.

# --hints--

`happyBtn` 변수는 존재하지 않아야 합니다.
