...
Para alcançar o resultado de "enviar a bolha para cima", você precisa verificar se o elemento atual é maior que o próximo elemento.Você pode fazer isso acessando o `array` em `j` e em `j+1`.

Crie uma condição `if`, que verifica se o elemento atual é maior que o próximo.

# --hints--

Você não deve remover a instrução do `console.log`.
...
Você deve usar uma instrução `if` no laço aninhado.
...
A instrução `if` deve verificar se `array[j]` é maior do que `array[j+1]`.
