...
Після вашого останнього елемента `.divider` створіть елемент `p` і додайте йому текст `Total Fat 8g 10%`. Потім обгорніть текст `Total Fat` в один елемент `span`, текст `10%` — в інший, і надайте кожному клас `bold`.
