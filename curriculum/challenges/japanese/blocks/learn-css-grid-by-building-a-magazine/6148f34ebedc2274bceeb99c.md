...
雑誌のレイアウトが完成したので、レスポンシブにする必要があります。

`@media` に対して `only screen` の `max-width` を使った `720px` クエリから始めてください。その中に `.image-wrapper` セレクターを作成し、`grid-template-columns` の `1fr` プロパティを与えます。

これにより、小さい画面で3つの画像が1列に折りたたまれます。

# --hints--

`@media` に対する新しい `only screen and (max-width: 720px)` ルールがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

新しい `@media` ルールには `.image-wrapper` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

新しい `.image-wrapper` セレクターには `grid-template-columns` の `1fr` プロパティがあるはずです。
