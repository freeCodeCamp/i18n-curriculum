...
بالعودة إلى صنف `MediaCatalogue`، أضف إليه سلسلة توثيق تقول `A catalogue that can store different types of media items.`

# --hints--

يجب أن تضيف سلسلة التوثيق `A catalogue that can store different types of media items.` إلى صنف `MediaCatalogue` الخاص بك.
