...
<blockquote>rgba significa:<br>  r = red (rojo)<br>  g = green (verde)<br>  b = blue (azul)<br>  a = alpha/nivel de opacidad</blockquote>

Los valores RGB pueden variar de 0 a 255.  The alpha value can range from 1, which is fully opaque or a solid color, to 0, which is fully transparent or clear. `rgba()` es ideal para usar en este caso, ya que te permite ajustar la opacidad. Esto significa que no tienes que bloquear completamente el fondo.
