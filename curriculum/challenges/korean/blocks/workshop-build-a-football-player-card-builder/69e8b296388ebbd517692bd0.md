...
`form-row` 뒤에 `form-group`의 `className`를 가진 `div`를 추가하세요. 그 안에 `label`의 `className`, `club`의 `htmlFor`, 텍스트 `Club`를 가진 `label` 요소를 추가하세요. 그런 다음 `club`의 `id`, `input`의 `className`, `text`의 `type`를 가진 input 요소를 추가하세요.
...
`div.form-group`에는 `club`의 `htmlFor` 속성, `label`의 `className`, 텍스트 `Club`를 가진 `label`가 있어야 합니다.
...
Club용 `form-group`에는 `club`의 `id`, `input`의 `className`, `text`의 `type`를 가진 `input`가 포함되어야 합니다.
