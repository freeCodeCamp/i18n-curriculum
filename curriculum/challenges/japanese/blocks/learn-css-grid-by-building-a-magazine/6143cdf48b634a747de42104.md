...
`.image-wrapper` 要素内の3番目の `img` 要素に、`src` の `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`、`alt` の `four people working on code`、`loading` の `lazy` 属性、`class` に設定された `image-3`、`width` に設定された `600` 属性、`height` に設定された `400` 属性を付けてください。

# --hints--

3番目の `img` 要素には `src` に設定された `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg` が必要です。
