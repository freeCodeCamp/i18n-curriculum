...
前後の曲の再生機能に取り掛かります。 そのためには、関数 `playNextSong` と `playPreviousSong` が必要です。

`const` とアロー構文を使用して、空の関数 `playNextSong` を作成してください。

# --hints--

`const` を使用して、`playNextSong` という名前の空の関数を作成してください。

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` は関数でなければなりません。
