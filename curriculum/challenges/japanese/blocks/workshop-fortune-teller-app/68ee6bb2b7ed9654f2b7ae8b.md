...
グローバルな `document` オブジェクトに、`showFortune` 関数を呼び出す別のイベントリスナーを追加してください。コールバックの中で、`e` 型のパラメータ `Event` を含め、`e` メソッドに `showFortune` を引数として渡してください。

`showFortune` はカードのクリックイベントを処理します。例えば、クリックされたカードのIDをDOMから読み取り、一致するカードの運勢の詳細を表示します。これからのステップで作成します。

# --hints--

グローバルな `click` に `document` イベントリスナーを代入してください。

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

イベントリスナーのコールバックには `e` という名前のパラメータを持たせてください。

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

コールバックのパラメータは `Event` 型である必要があります。

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

コールバックの中で `showFortune` 関数を呼び出してください。

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

`showFortune` 関数の中で、引数 `e` を渡してください。
