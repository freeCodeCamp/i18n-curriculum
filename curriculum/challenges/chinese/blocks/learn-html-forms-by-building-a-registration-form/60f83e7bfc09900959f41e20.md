...
某些 `type` 属性值带有内置表单验证功能。 例如，`type="email"` 要求值必须是有效的电子邮件地址。

给密码 `input` 元素添加自定义验证，添加一个 `minlength` 属性，其值为 `8`。 这样做可以防止提交少于 8 个字符的输入。

# --hints--

你应该给密码 `input` 元素添加一个 `minlength` 属性。

```js
assert.notEqual(document.querySelector('input[type="password"]')?.minLength, -1);
```

你应该将 `minlength` 属性设置为 `8`。
