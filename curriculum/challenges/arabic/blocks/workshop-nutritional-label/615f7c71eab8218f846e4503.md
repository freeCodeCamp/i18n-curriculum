...
بعد آخر `.divider`، أنشئ عنصر `p` آخر يحتوي على النص `Trans Fat 0g`. اجعل كلمة `Trans` مائلة بتغليفها داخل عنصر `i`. أعطِ عنصر `p` الجديد الخاصية `class` بقيمة `indent no-divider`. غلف النص `Trans Fat 0g` داخل عنصر `span` لمحاذاته.
