...
Se premi il pulsante precedente quando non sta suonando nessuna canzone, partirà la prima canzone. Per risolvere questo problema, aggiungi un'istruzione `if` all'inizio della funzione `playPreviousSong`. Controlla se la canzone corrente è `null` e interrompi l'esecuzione della funzione con un'istruzione `return`.

# --hints--

Dovresti aggiungere un'istruzione `if` che verifica se `userData.currentSong === null` e interrompe l'esecuzione della funzione con un'istruzione `return` all'interno della tua funzione `playPreviousSong`.
