...
最后，就像你对其他函数所做的那样，更新 `calculate` 函数，使其包含标准差逻辑。

恭喜！ 你的项目完成了。

# --hints--

你的 `calculate` 函数应该将 `standardDeviation` 变量设置为 `getStandardDeviation(numbers)` 的结果。

```js
assert.match(calculate.toString(), /standardDeviation\s*=\s*getStandardDeviation\(\s*numbers\s*\)/);
```

你应该将 `#standardDeviation` 元素的 `textContent` 更新为 `standardDeviation` 变量的值。
