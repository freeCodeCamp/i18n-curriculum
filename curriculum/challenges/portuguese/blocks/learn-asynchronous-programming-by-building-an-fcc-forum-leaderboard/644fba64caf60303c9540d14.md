...
Após o atributo `href`, defina o atributo `class` para a constante chamada `linkClass`. 

Após o atributo `class`, defina o atributo `target` como `"_blank"`. 

Por fim, coloque a constante `linkText` entre as tags de âncora para exibir o texto no link.

# --hints--

Você deve ter um atributo `class` depois do atributo `href`.
...
Você deve definir o valor do atributo `class` para a constante chamada `linkClass`.
...
Você deve ter um atributo `target` depois do atributo `class`.
...
Você deve definir o valor do atributo `target` para `"_blank"`.
...
Você deve colocar a constante `linkText` entre as tags de âncora.
