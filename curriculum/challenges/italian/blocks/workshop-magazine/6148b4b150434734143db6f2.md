...
Ricorda che la proprietà `grid-column` determina da quale colonna un elemento inizia e a quale colonna finisce. Può capitare di non sapere quante colonne avrà la tua griglia, ma di voler far terminare un elemento all’ultima colonna. Per farlo, puoi usare `-1` per la colonna finale.

Crea un selettore `.hero` e dagli una proprietà `grid-column` impostata su `1 / -1`. Questo dirà all’elemento di estendersi per tutta la larghezza della griglia.

# --hints--

Dovresti avere un selettore `.hero`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero'));
```

Il tuo selettore `.hero` dovrebbe avere una proprietà `grid-column` impostata su `1 / -1`.
