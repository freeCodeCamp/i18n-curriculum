...
在 `for` 迴圈內，宣告一個變數 `row_str` 並指定一個列表推導式，該推導式對 `row` 進行迭代，將 `row` 中的每個項目 `i` 轉換成字串。為此使用 `str()` 函式（程式）。
...
你應該將一個對 `row` 進行迭代的列表推導式指定給 `row_str` 變數。
...
指定給 `row_str` 變數的列表推導式應該使用 `i` 作為迭代變數。
...
指定給 `row_str` 變數的列表推導式應該對 `row` 中的每個項目 `i` 呼叫 `str()`。
