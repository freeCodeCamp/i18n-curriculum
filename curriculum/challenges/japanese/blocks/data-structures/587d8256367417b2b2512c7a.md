...
このチャレンジでは、`findMin` と `findMax` の2つのメソッドを定義します。これらのメソッドは、二分探索木に保持されている最小値と最大値を返す必要があります（今は木に値を追加することは気にしなくて大丈夫です。背景でいくつか追加されています）。もし行き詰まったら、二分探索木に必ず成り立つ不変条件を思い出してください：各左のサブツリーは親以下であり、各右のサブツリーは親以上であるとします。また、木は整数値のみをストアできるとしましょう。木が空の場合、どちらのメソッドも `null` を返す必要があります。
...
二分探索木には `findMin` というメソッドがあるはずです。
...
二分探索木には `findMax` というメソッドがあるはずです。
...
`findMin` と `findMax` のメソッドは空の木の場合 `null` を返すべきです。
