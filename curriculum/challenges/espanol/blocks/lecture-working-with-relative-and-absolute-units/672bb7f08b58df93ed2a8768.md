...
Para la clase `para`, establecemos el `tamaño de fuente` a `20px` y el `margen inferior` a `1.5em`.  This means that the margin will be 1.5 times the font size of the paragraph element. `1.5em` results in 30 pixels of margin at the bottom of the paragraph. También hemos establecido un `bordet` de `2px rojo sólido` para que puedas ver mejor los márgenes. 
