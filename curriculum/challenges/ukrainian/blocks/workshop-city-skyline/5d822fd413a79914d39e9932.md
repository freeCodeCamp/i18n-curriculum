...
Переходимо до наступної будівлі! Це зелена на передньому плані. Додайте їй `repeating-linear-gradient` з вашим кольором будівлі від `0%` до `5%`, а `transparent` від `5%` до `10%`.

# --hints--

Ви повинні додати `.fb5` властивість `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

Ви повинні додати `background` властивість `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Ви повинні додати `repeating-linear-gradient` перший колір `--building-color2` від `0%` до `5%`.
...
Ви повинні додати `repeating-linear-gradient` другий колір `transparent` від `5%` до `10%`.
