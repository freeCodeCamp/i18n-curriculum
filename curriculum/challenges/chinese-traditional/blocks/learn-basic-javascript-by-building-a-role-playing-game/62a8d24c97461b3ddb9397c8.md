...
一旦玩家擁有最強大的武器，你就可以給他們出售舊武器的能力。

在外層的 `else` 述語中，將 `button2.innerText` 設定為 `"Sell weapon for 15 gold"`。同時將 `button2.onclick` 設定為函式名稱 `sellWeapon`。

# --hints--

你應該設定 `button2.innerText` 的值。

```js
assert.match(buyWeapon.toString(), /button2\.innerText/);
```

你應該將 `button2.innerText` 的值設為 `"Sell weapon for 15 gold"`。

```js
assert.match(buyWeapon.toString(), /button2\.innerText\s*=\s*('|")Sell weapon for 15 gold\1\s*;?/);
```

你應該設定 `button2.onclick` 的值。

```js
assert.match(buyWeapon.toString(), /button2\.onclick/);
```

你應該將 `button2.onclick` 的值設為 `sellWeapon`。
