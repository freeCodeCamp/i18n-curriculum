...
Alama ya tiki katika kila `span` mwanzoni mwa kipengee cha orodha ni ya mapambo. Ongeza `aria-hidden` kwa vipengele hivi vya `span` ili kuzificha kwa wasomaji wa skrini.

Pia, weka madarasa `text-green-700` na `mr-2` kwa kila kipengee cha `span`.

# --hints--

Vipengele vyako viwili vya `span` vinapaswa kuwa na sifa ya `aria-hidden`.
...
Vipengele vyako viwili vya `span` vinapaswa kuwa na darasa la `text-green-700`.
...
Vipengele vyako viwili vya `span` vinapaswa kuwa na darasa la `mr-2`.
