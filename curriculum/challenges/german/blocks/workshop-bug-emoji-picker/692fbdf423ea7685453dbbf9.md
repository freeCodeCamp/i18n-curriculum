...
Sie werden nun beginnen, auf die benötigten DOM-Elemente zuzugreifen.

Holen Sie das Absatz-Element mit der ID `bug-emoji` und speichern Sie es in einer Konstanten namens `bugEmojiElement`. Verwenden Sie den `!`-Non-Null-Assertion-Operator, um sicherzustellen, dass das Element nicht `null` ist.

# --hints--

Sie sollten eine Konstante namens `bugEmojiElement` haben.
...
Ihre `bugEmojiElement` sollte das Element mit der ID `bug-emoji` mithilfe von `document.querySelector` mit einem Typ-Argument von `HTMLParagraphElement` abfragen.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

Ihre `bugEmojiElement` sollte den Non-Null-Assertion-Operator (`!`) verwenden.
