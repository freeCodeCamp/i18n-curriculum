...
将有心形表情符号的 `div` 放置在距离其父级元素顶部 `22.5px`、左侧 `12px` 的位置。

# --hints--

你应该将 `.shirt div` 的 `top` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt div')?.top, '22.5px');
```

你应该将 `.shirt div` 的 `left` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
