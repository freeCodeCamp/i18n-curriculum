...
Sie müssen die Karte erneut zufällig und umgedreht machen. Deklarieren Sie eine Variable namens `isReversed` und `card`. Für die erste rufen Sie die `Math.random`-Methode auf und vergleichen sie mit `< 0.5`. Für die zweite rufen Sie `getRandomItem` auf und übergeben `this.cards`.

# --hints--

Sie sollten eine Variable namens `isReversed` deklarieren.
...
Ihre `isReversed`-Variable sollte `Math.random()` verwenden, um zufällig zu entscheiden, ob die Karte umgedreht ist.
...
Sie sollten eine Variable namens `card` deklarieren.
...
Ihre `card`-Variable sollte durch den Aufruf der `getRandomItem`-Methode mit `this.cards` als Argument zugewiesen werden.
