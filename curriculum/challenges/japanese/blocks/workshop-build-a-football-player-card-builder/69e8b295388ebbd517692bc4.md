...
ティアバッジにはまだハードコードされたテキスト `GOLD` が表示されています。正しいティアが表示されるように、動的な式に置き換える必要があります。

`getPlayerTier()` を使って `player.overallRating` を呼び出し、ティアの文字列を取得してください。その結果に対して `.toUpperCase()` を呼び出し、大文字で表示します。式全体を波括弧で囲むのを忘れないでください。

# --hints--

`getPlayerTier(player.overallRating).toUpperCase()` の中で `.card-tier-badge` を使うべきです。

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

`.card-tier-badge` 要素は、レーティングが98のプレイヤーに対して `ELITE` を表示する必要があります。
