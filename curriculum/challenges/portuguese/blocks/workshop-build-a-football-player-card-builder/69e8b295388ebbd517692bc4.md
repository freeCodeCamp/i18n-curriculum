...
O selo de nível ainda mostra o texto fixo `GOLD`. Você precisa substituí-lo por uma expressão dinâmica para que o nível correto apareça.

Faça uma chamada a `getPlayerTier()` com `player.overallRating` para obter a string do nível, depois faça uma chamada a `.toUpperCase()` no resultado para exibi-lo em maiúsculas. Não esqueça de envolver toda a expressão em chaves.

# --hints--

Você deve usar `getPlayerTier(player.overallRating).toUpperCase()` dentro de `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

O elemento `.card-tier-badge` deve exibir `ELITE` para um jogador com avaliação 98.
