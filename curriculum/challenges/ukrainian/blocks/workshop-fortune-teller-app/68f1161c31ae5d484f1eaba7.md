...
Тепер додайте ще один метод з назвою `showFortune`. Передайте `e` як параметр і вкажіть для нього тип `Event`. Також створіть метод `newReading` і поки що залиште його порожнім.

# --hints--

Ви повинні оголосити метод з назвою `showFortune`.
...
Ваш метод `showFortune` має мати параметр `e` з типом `Event`.
...
Ви повинні оголосити метод з назвою `newReading`.
