...
يجب أن يكون لديك منتقي `.quote`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote'));
```

يجب أن يكون لمنتقي `.quote` الخاص بك الخاصية `color` بقيمة `#00beef`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.color === 'rgb(0, 190, 239)');
```

يجب أن يكون لمنتقي `.quote` الخاص بك خاصية `font-size` بقيمة `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize === '2.4rem');
```

يجب أن يكون لمنتقي `.quote` الخاص بك خاصية `text-align` بقيمة `center`.
