...
Sie benötigen eine Spalte für Text und eine Spalte für Bilder. Geben Sie dem `.text-with-images`-Selektor eine `grid-template-columns`-Eigenschaft mit dem Wert `1fr 2fr`. Setzen Sie außerdem die `column-gap`-Eigenschaft auf `3rem`, um mehr Abstand zwischen den Spalten zu schaffen.

# --hints--

Ihr `.text-with-images`-Selektor sollte eine `grid-template-columns`-Eigenschaft mit dem Wert `1fr 2fr` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns, '1fr 2fr');
```

Ihr `.text-with-images`-Selektor sollte eine `column-gap`-Eigenschaft mit dem Wert `3rem` haben.
