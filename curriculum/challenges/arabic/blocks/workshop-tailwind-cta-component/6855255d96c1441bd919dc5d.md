...
انتقل إلى عنصر `span` الذي يحتوي على النص `Soundflow`، يجب أن يكون النص فيه بأحرف كبيرة.

إليك مثال يوضح كيفية استخدام صنف الأدوات `uppercase`:

```html
<p class="uppercase">Quincy Larson</p>
```

أضف الصنف `uppercase` إلى عنصر `span`.

# --hints--

يجب أن يحتوي عنصر `span` على صنف الأدوات `uppercase`.
