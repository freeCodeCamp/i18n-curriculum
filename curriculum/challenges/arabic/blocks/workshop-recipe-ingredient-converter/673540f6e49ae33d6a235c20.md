...
ستحتاج إلى الاحتفاظ بقائمة الوحدات التي يدعمها تطبيقك.

أعلن عن مصفوفة `units`، وأعطها القيم النصية `"cup"`، `"gram"`، `"ounce"`، و`"teaspoon"`.

# --hints--

يجب أن تعلن عن متغير `units`.

```js
assert.isDefined(units);
```

يجب أن يكون متغير `units` مصفوفة.

```js
assert.isArray(units);
```

يجب أن تحتوي مصفوفة `units` على أربعة عناصر.

```js
assert.lengthOf(units, 4);
```

يجب أن تتضمن مصفوفة `units` السلسلة النصية `"cup"`.

```js
assert.include(units, "cup");
```

يجب أن تتضمن مصفوفة `units` السلسلة النصية `"gram"`.

```js
assert.include(units, "gram");
```

يجب أن تتضمن مصفوفة `units` السلسلة النصية `"ounce"`.

```js
assert.include(units, "ounce");
```

يجب أن تتضمن مصفوفة `units` السلسلة النصية `"teaspoon"`.
