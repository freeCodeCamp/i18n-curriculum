---
id: 5f356ed60a5decd94ab66986
title: Step 18
challengeType: 0
dashedName: step-18
---

# --description--

CSS에서 주석은 이렇게 생겼습니다:

```css
/* comment here */
```

스타일 시트에서 `background-color` 속성과 값이 포함된 줄을 주석 처리하세요. 그러면 `#menu` 요소에만 스타일이 적용되는 효과를 볼 수 있습니다. 이렇게 하면 배경이 다시 흰색이 됩니다.

# --hints--

CSS에서 `background-color: burlywood;` 줄을 주석 처리해야 합니다.

```js
assert.match(code, /\/\*\s*background-color:\s*burlywood;?\s*\*\//i);
```


`body`은 흰색 배경이어야 합니다.

```js
const body = document.querySelector("body");
const compStyles = window.getComputedStyle(body);
const backgroundColor = compStyles?.getPropertyValue('background-color');
assert.equal(backgroundColor, "rgba(0, 0, 0, 0)")
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Cafe Menu</title>
    <link href="styles.css" rel="stylesheet"/>
  </head>
  <body>
    <div id="menu">
      <main>
        <h1>CAMPER CAFE</h1>
        <p>Est. 2020</p>
        <section>
          <h2>Coffee</h2>
        </section>
      </main>
    </div>
  </body>
</html>
```

```css
body {
--fcc-editable-region--
  background-color: burlywood;
--fcc-editable-region--
}

h1, h2, p {
  text-align: center;
}

#menu {
  width: 300px;
}
```

