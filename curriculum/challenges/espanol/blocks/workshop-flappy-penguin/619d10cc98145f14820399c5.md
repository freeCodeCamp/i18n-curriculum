...
Debajo del elemento `.blush.right`, agregue dos elementos `div` cada uno con una `class` de `beak`. Además, asigna al primer elemento `.beak` una `class` de `top`, y al segundo elemento `.beak` un <code >`class` de `bottom`.
