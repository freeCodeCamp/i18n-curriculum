...
Du solltest eine `toNumberList`-Variable deklarieren.
...
Du solltest `const` verwenden, um deine `toNumberList`-Variable zu deklarieren.
...
Deine `toNumberList`-Variable sollte eine Arrow-Funktion sein.
...
Deine `toNumberList`-Funktion sollte einen `args`-Parameter haben.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

Deine `toNumberList`-Funktion sollte eine implizite Rückgabe verwenden.
...
Deine `toNumberList`-Funktion sollte das Ergebnis des Aufrufs der `.split()`-Methode von `args` zurückgeben.
...
Du solltest die `.map()`-Methode mit der `.split()`-Methode verketten.
...
Du solltest einen Verweis auf `parseFloat` als Callback an `.map()` übergeben.
