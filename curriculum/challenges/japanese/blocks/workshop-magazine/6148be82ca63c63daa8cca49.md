...
`.author` セレクターを作成し、`font-size` プロパティを `2rem` に設定し、`font-family` プロパティを `Raleway` に設定して、`sans-serif` をフォールバックとして指定してください。
...
`.author` セレクターには `font-size` プロパティが `2rem` に設定されているはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize, '2rem');
```

`.author` セレクターには `font-family` プロパティが `Raleway` に設定され、`sans-serif` がフォールバックとして指定されているはずです。
