...
在你的 `inner` 函式中，傳回以 `character + num` 作為引數呼叫 `idToText` 的結果。

# --hints--

你的 `inner` 函式（程式）應該使用明確的傳回。
...
你的 `inner` 函式應該傳回呼叫你的 `idToText` 函式的結果。
...
你應該將 `character + num` 作為引數傳遞給你的 `idToText` 函式（程式）。
