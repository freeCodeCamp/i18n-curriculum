...
استهدف العنصر `.back-mountain`، واضبط `width` و`height` إلى `300px`. ثم اضبط `background` إلى تدرج لوني خطي يبدأ من `rgb(203, 241, 228)` وينتهي عند `rgb(47, 170, 255)`.

# --hints--

يجب أن تستخدم المحدِّد `.back-mountain`.

```js
assert.match(code, /\.back-mountain\s*\{/);
```

يجب أن تعطي `.back-mountain` `width` بقيمة `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

يجب أن تعطي `.back-mountain` `height` بقيمة `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

يجب أن تعطي `.back-mountain` `background` بقيمة `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`.
