...
Gegeben ist das Array `arr`. Durchlaufen Sie es und entfernen Sie jedes Element beginnend mit dem ersten Element (dem Index 0), bis die Funktion `func` `true` zurückgibt, wenn das durchlaufene Element an sie übergeben wird.

Geben Sie dann den Rest des Arrays zurück, sobald die Bedingung erfüllt ist, andernfalls soll `arr` als leeres Array zurückgegeben werden.
