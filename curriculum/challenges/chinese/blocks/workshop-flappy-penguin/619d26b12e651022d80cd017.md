...
选择 `.arm` 元素，将它们的 `width` 设置为 `30%`，`height` 设置为 `60%`，`background` 设置为线性渐变，顺时针方向 `90deg`，以 `gray` 处开始，以 `rgb(209, 210, 199)` 结束。
...
你应该将 `.arm` 的 `width` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

你应该将 `.arm` 的 `height` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

你应该将 `.arm` 的 `background` 设置为 `linear-gradient(90deg, gray, rgb(209, 210, 199))`。
