...
同じセレクター構文を使って、`th` が `class` のテーブル行内の `total` 要素をターゲットにしてください。テキストを左揃えにし、パディングを `0.5rem 0 0.25rem 0.5rem` に設定してください。

# --hints--

`tr[class="total"] th` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"] th'));
```

`tr[class="total"] th` セレクターには、`text-align` プロパティを `left` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"] th')?.getPropertyValue('text-align') === 'left');
```

`tr[class="total"] th` セレクターには、`padding` プロパティを `0.5rem 0 0.25rem 0.5rem` に設定してください。
