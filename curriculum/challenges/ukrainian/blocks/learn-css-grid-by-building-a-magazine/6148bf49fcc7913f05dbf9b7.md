...
Створіть селектор `.author-name a:hover` і надайте йому множину властивостей `background-color`, встановлену в `#306203`.

Це створить ефект наведення лише для елемента `a` всередині `.author-name`, показуючи оригінальний зелений колір freeCodeCamp на фоні.

# --hints--

У вас має бути селектор `.author-name a:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

Ваш селектор `.author-name a:hover` має мати властивість `background-color`, встановлену в `#306203`.
