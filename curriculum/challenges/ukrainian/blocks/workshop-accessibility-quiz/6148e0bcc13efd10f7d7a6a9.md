...
Встановіть `footer` колір фону `#2a2a40` і використайте гнучкий контейнер, щоб горизонтально вирівняти текст по центру.

# --hints--

Ви маєте використати селектор елемента `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

Ви маєте задати `footer` ідентифікатор `background-color` зі значенням `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

Ви маєте задати `footer` множину `display` зі значенням `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Ви маєте задати `footer` множину `justify-content` зі значенням `center`.
