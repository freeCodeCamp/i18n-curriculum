...
لا يزال هناك نوعان من المقارنات التي يجب تنفيذها. تُستدعى الطريقة `__le__` عند استخدام عامل `<=` لمقارنة كائنين.

عرّف طريقة `__le__` بمعلمتين `self` و `other` واجعلها تُرجع العكس لـ `self > other`.

# --hints--

يجب أن تعرف طريقة `__le__` داخل صنف `R2Vector`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__le__")`)) })
```

يجب أن تحتوي طريقة `__le__` على معلمتين، `self` و `other`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__le__").has_args("self, other")`)) })
```

يجب أن تُرجع طريقة `__le__` العكس لـ `self > other`.
