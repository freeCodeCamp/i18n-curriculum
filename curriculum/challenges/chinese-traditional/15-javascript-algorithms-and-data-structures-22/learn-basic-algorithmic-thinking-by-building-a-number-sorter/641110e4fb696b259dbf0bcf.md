...
對於這個算法，你會想要使用 `while` 循環。 這個循環需要兩個條件：

- 首先，程序不應運行超出數組的開頭（通過 `j` 訪問）。
- 其次，當找到比當前值小的值時，循環不應該繼續運行。

爲了防止無限循環，請在你的循環中遞減 `j`。
...
你的 `while` 循環的第一個條件應該檢查 `j` 的值是否大於或等於 `0`。
...
你的 `while` 循環應該有一個條件來檢查 `array[j]` 的值是否大於 `currValue`。
