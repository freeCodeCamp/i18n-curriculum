...
関数 `goTown` を、関数 `goStore` の上に移動させてください。 次に、関数 `goStore` の内容を関数 `goTown` にコピー＆ペーストしてください。

# --hints--

関数 `goTown` は関数 `goStore` よりも前に置く必要があります。
...
関数 `goTown` は関数 `goStore` と同じ内容である必要があります。
