...
Dentro del arreglo `allSongs`, crea un objeto con las siguientes propiedades y valores:
...
Tu arreglo `allSongs` debe tener un valor.

```js
assert.equal(allSongs.length, 1);
```

Tu arreglo `allSongs` debe tener un objeto como su primer valor.

```js
assert.isObject(allSongs[0]);
```

Tu arreglo `allSongs` debe tener un objeto con una propiedad `id` establecida al número `0`.

```js
assert.strictEqual(allSongs[0].id, 0);
```

Tu arreglo `allSongs` debe tener un objeto con una propiedad `title` establecida a la cadena `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Tu arreglo `allSongs` debe tener un objeto con una propiedad `artist` establecida a la cadena `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Tu arreglo `allSongs` debe tener un objeto con una propiedad `duration` establecida a la cadena `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Tu arreglo `allSongs` debe tener un objeto con una propiedad `src` establecida a la cadena `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
