...
`setPlayerCards` 関数を `players.filter()` の引数で呼び出してください。

`filter` メソッドの中で、`player` というパラメータを持つコールバック関数を追加し、`player.nickname` が `null` でないかを暗黙的に返してください。

# --hints--

`nickname` の場合は、`setPlayerCards` 関数を呼び出す必要があります。

```js
assert.match(code, /\s*case\s*('|"|`)\s*nickname\s*\1\s*:\s*setPlayerCards\(/)
```

`players.filter()` を `setPlayerCards` の呼び出しに渡してください。

```js
assert.match(code, /\s*case\s*('|"|`)\s*nickname\s*\1\s*:\s*setPlayerCards\(\s*players\.filter\(/)
```

`players.filter()` の呼び出しにはアローコールバック関数を渡してください。
...
`filter()` のコールバックは `player.nickname` が `null` でないかをチェックした結果を暗黙的に返す必要があります。
