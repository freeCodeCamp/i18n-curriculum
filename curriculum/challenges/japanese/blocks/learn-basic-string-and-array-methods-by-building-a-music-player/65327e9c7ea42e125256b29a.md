...
残りの曲を`allSongs`配列に追加しました。

次に、<dfn>Web Audio API</dfn>について学び、それを使って曲を再生する方法を説明します。すべての最新ブラウザはWeb Audio APIをサポートしており、ウェブアプリケーションで音声を生成および処理できます。

`const`を使って、`audio`という名前の変数を作成し、`new Audio()`と等しく設定してください。これにより、新しいHTML5の`audio`要素が作成されます。

# --hints--

`new`キーワードを使って、`Audio`オブジェクトのインスタンスを作成してください。

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

`Audio`オブジェクトを`audio`という定数に代入してください。
