...
Перше, що слід зробити, — перевірити, чи порожній масив `items`. Якщо так, відобразіть користувачу `alert` з текстом `Your shopping cart is already empty`, а потім поверніться з функції.

Пам’ятайте, що `0` — це хибне значення, тому ви можете використати оператор `!` для перевірки, чи масив порожній.

Після відображення повідомлення поверніться з функції, щоб зупинити виконання.

# --hints--

Ви повинні створити інструкцію `if`, яка перевіряє, чи порожній масив `items`, використовуючи оператор заперечення.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

Ваша інструкція `if` має відобразити користувачу `alert` з текстом `Your shopping cart is already empty`.
...
Ваша інструкція `if` має повертати результат із функції.
