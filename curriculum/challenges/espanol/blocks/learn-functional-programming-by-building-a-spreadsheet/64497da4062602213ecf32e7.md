...
Porque el evento `change` se está activando en un elemento `input`, el elemento tendrá una propiedad `value` que representa el valor actual de la entrada.

Asigna la propiedad `value` del `element` a una nueva variable llamada `value`, y usa `.replace()` para eliminar todos los espacios en blanco.

# --hints--

Debes declarar una variable `value` después de tu variable `element`.

```js
assert.match(code, /const\s+update\s*=\s*(\(\s*event\s*\)|event)\s*=>\s*\{\s*const\s+element\s*=\s*event\.target\s*;?\s*(?:const|let|var)\s+value/);
```

Debes usar `const` para declarar tu variable `value`.

```js
assert.match(code, /const\s+update\s*=\s*(\(\s*event\s*\)|event)\s*=>\s*\{\s*const\s+element\s*=\s*event\.target\s*;?\s*const\s+value/);
```

Debes asignar la propiedad `value` del `element` a tu variable `value`.
...
Debes llamar al método `.replace()` en la propiedad `value` del `element`.
...
Debes pasar una expresión regular para coincidir con espacios en blanco a tu método `.replace()`. Usa la clase de carácter `\s`.
...
Debes hacer que tu expresión regular sea global.
...
Debes pasar una cadena vacía como tu segundo argumento al método `.replace()`.
