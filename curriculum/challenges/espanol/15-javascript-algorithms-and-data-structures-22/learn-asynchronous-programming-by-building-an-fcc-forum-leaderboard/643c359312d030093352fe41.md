...
Ahora es momento de mostrar los datos en la página.

Comienza creando una función flecha llamada `showLatestPosts`, que toma un único parámetro `data`.

# --hints--

Debes tener una función flecha llamada `showLatestPosts`.

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

Tu función `showLatestPosts` debe tener un parámetro `data`.

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

`showLatestPosts` debe definirse.

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts` debe ser una función.
