...
現在當你添加新的圖示元素時，自動配置演算法將會啟動。然而，該演算法預設新的行寬為 `auto`，這將不符合你目前的行。

你可以用 `grid-auto-columns` 屬性來改寫這個設定。為 `.social-icons` 選擇器添加一個 `grid-auto-columns` 屬性，並設定為 `1fr`。

# --hints--

你的 `.social-icons` 選擇器應該有一個 `grid-auto-columns` 屬性設定為 `1fr`。
