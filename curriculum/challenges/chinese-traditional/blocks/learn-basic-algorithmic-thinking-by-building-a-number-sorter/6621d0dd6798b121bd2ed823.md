...
更新你的 `.map()` 回呼以呼叫 `Number()` 函式（程式）。將 `dropdown.value` 傳遞給該函式（程式）呼叫。

打開主控台分頁以查看你的 `inputValues` 現在是一個數字陣列。

# --hints--

你應該在你的 `.map()` 回呼中呼叫 `Number()` 函式（程式）。 
...
你應該將 `dropdown.value` 傳遞給你的 `Number()` 呼叫。
