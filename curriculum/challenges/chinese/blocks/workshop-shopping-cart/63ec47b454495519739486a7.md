...
你需要在你的超文本标记语言中显示可用的产品。首先使用加法赋值操作符，将一个空的模板字串添加到 `dessertCards` 变量的 `innerHTML` 属性。

# --hints--

你应该访问 `dessertCards` 变量的 `innerHTML` 属性。

```js
assert.match(code, /dessertCards\.innerHTML/);
```

你应该对 `innerHTML` 属性使用加法赋值操作符。

```js
assert.match(code, /dessertCards\.innerHTML\s*\+=\s*/);
```

你应该为 `innerHTML` 属性添加一个空的模板字串。
