...
Usando la sintaxis de flecha completa la función de devolución de llamada `forEach`. Dentro del cuerpo de la función de devolución de llamada, usa una asignación de suma para establecer el `innerHTML` de `tasksContainer` a comillas vacías.

# --hints--

No debes alterar el existente `taskData.forEach()` y los valores que desestructuraste.
...
Debes usar la sintaxis de flecha y adjuntar `innerHTML` a `tasksContainer`.

```js
assert.match(code, /taskData\.forEach\(\s*\(\s*\{.*\}\s*\)\s*=>\s*(\s*\{?\(\s*tasksContainer\.innerHTML\s*\)\s*\}?)?/)
```

Debes usar la asignación de suma para establecer el `innerHTML` de `tasksContainer` a un par vacío de comillas.
