...
在你的 `tbody` 元素中，添加四個 `tr` 元素。 將前三個的 `class` 屬性設置爲 `data`，第四個的 `class` 屬性設置爲 `total`。

# --hints--

你的 `tbody` 元素應該有四個 `tr` 元素。
...
你的前三個 `tr` 元素應將 `class` 屬性設置爲 `data`。
...
你的第四個 `tr` 元素應將 `class` 屬性設置爲 `total`。
