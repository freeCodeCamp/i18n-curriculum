...
2つのパラメータ、`rangeFromString` と `num1` を受け取る `num2` アロー関数を宣言してください。この関数は、`range` と `num1` を引数として `num2` を呼び出した結果を暗黙的に返す必要があります。

安全のため、`num1` と `num2` を `range` に渡す際に整数に変換してください。

# --hints--

`rangeFromString` の後に `rangeRegex` 変数を宣言する必要があります。
...
`const` 変数を宣言するには `rangeFromString` を使うべきです。
...
`rangeFromString` 変数はアロー関数であるべきです。
...
`rangeFromString` 関数の最初のパラメータは `num1` であるべきです。
...
`rangeFromString` 関数の2番目のパラメータは `num2` であるべきです。
...
`rangeFromString` 関数は暗黙の返却を使うべきです。
...
`rangeFromString` 関数は `range` 関数を呼び出した結果を返すべきです。
...
`parseInt` を `num1` を引数にして呼び出し、その結果を `range` の呼び出しに渡すべきです。

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

`parseInt` を `num2` を引数にして呼び出し、その結果を `range` の呼び出しに渡すべきです。
