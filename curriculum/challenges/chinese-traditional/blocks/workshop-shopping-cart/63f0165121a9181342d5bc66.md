...
對於你的 truthy 表達式，移除 `undefined` 後，你需要更新 `currentProductCountSpan` 的 `textContent`，使其為 `currentProductCount` 後接一個 `x`。使用模板字面值來完成此操作。

# --hints--

你應該從你的 truthy 表達式中移除 `undefined`。
...
你應該存取 `currentProductCountSpan` 的 `textContent` 屬性。
...
你應該使用模板字面量語法來更新 `textContent` 為 `${currentProductCount}x`。
