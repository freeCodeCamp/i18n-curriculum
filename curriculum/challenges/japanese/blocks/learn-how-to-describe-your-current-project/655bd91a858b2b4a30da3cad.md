...
`make pictures` は、データの文脈では、データの発見を示すための視覚的な表現やグラフを作成することを指します。情報を示すためにレポート作成でよく使われる方法です。
...
この単語はデータの視覚的表現を作成するプロセスを指します。`-ing` で終わります。
...
この単語の複数形は、ボブ と彼のチームが準備している文書やプレゼンテーションを指します。
