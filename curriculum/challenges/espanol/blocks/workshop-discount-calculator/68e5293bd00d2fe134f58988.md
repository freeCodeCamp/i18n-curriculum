...
Devuelve `True` si `product.price * 0.9` es mayor que `self.amount`, de lo contrario devuelve `False`. Esto asegura que el descuento no supere el 10% del precio original, lo cual sería poco realista.
...
Tu método `is_applicable` debe tener una indicación de tipo de retorno de `-> bool`.
