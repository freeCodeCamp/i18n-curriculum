...
同じパターンに従って、`number`、`#dinner`、`#snacks` 要素の中から `#exercise` 入力をクエリしてください。前の2つと同じ命名規則で変数に代入してください。

# --hints--

`dinnerNumberInputs` 変数を宣言してください。

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

`document.querySelectorAll()` を使って `#dinner input[type='number']` を呼び出してください。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` の呼び出し結果を `dinnerNumberInputs` に代入してください。
