...
L'immagine che hai aggiunto non è centrata orizzontalmente come l'intestazione `Coffee` sopra di essa. Gli elementi `img` sono "simili" agli elementi in linea.

Per far comportare l'immagine come elementi a livello di blocco come le intestazioni, crea un selettore di tipo `img`. Imposta la proprietà `display` su `block` e usa le proprietà `margin-left` e `margin-right` per centrarla orizzontalmente.
...
Dovresti impostare la proprietà `margin-left` su `auto`.
...
Dovresti impostare la proprietà `margin-right` su `auto`.
...
Il tuo elemento `img` dovrebbe avere un `margin-left` e `margin-right` di `auto`.
