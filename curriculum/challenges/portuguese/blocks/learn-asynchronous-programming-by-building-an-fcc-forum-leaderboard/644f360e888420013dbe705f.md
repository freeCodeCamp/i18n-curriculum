...
Dentro da sua cláusula `else`, atribua a string `"general"` a `selectedCategory.className`.

Abaixo disso, atribua a string `"General"` a `selectedCategory.category`.

Por fim, atribua o número `1` a `selectedCategory.id`.

# --hints--

Você deve atribuir a string `"general"` a `selectedCategory.className`.

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

Você deve atribuir a string `"General"` a `selectedCategory.category`.

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

Você deve atribuir o número `1` a `selectedCategory.id`.
