...
Додайте блок `else` до інструкції `if`. В межах блоку `else` викличте функцію `getCurrentSongIndex()` та призначте її до константи під назвою `currentSongIndex`.

# --hints--

Не змінюйте наявну інструкцію `if`.
...
Додайте `else` до наявної інструкції `if`.
...
Встановіть константу `currentSongIndex` на `getCurrentSongIndex()` в межах блоку `else`.
