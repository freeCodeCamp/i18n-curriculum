...
Зв’яжіть відповідні елементи форми та їхні `label` елементи разом.

Використовуйте `profile-picture`, `age`, `referrer` і `bio` як значення для відповідних `id` атрибутів.

# --hints--

Перший `input` елемент має мати `id` зі значенням `profile-picture`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[0]?.matches('#profile-picture'))
```

Другий `input` елемент має мати `id` зі значенням `age`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[1]?.matches('#age'))
```

`select` елемент має мати `id` зі значенням `referrer`.

```js
assert(document.querySelector('fieldset:nth-of-type(3) select')?.matches('#referrer'))
```

`textarea` елемент має мати `id` зі значенням `bio`.

```js
assert(document.querySelector('fieldset:nth-of-type(3) textarea')?.matches('#bio'))
```

Перший `label` елемент має мати атрибут `for` зі значенням `profile-picture`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[0]?.matches('label[for="profile-picture"]'))
```

Другий `label` елемент має мати атрибут `for` зі значенням `age`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[1]?.matches('label[for="age"]'))
```

Третій `label` елемент має мати атрибут `for` зі значенням `referrer`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[2]?.matches('label[for="referrer"]'))
```

Четвертий `label` елемент має мати атрибут `for` зі значенням `bio`.
