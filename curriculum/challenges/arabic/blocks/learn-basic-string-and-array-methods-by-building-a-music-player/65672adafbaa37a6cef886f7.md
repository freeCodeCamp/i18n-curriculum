...
الآن بعد أن أنشأت الزر، عليك تعيين نص له. للقيام بذلك، يجب استخدام طريقة `createTextNode()` من DOM.

تُستخدم طريقة `createTextNode()` لإنشاء عقدة نصية. لاستخدامها، استدعها ومرر النص كسلسلة نصية:
...
استخدم طريقة `createTextNode()` لإنشاء نص `"Reset Playlist"`، ثم عيّنه إلى ثابت `resetText`.
...
يجب استخدام `document.createTextNode()` لإنشاء نص `"Reset Playlist"`.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

يجب تعيين عقدة النص التي أنشأتها إلى ثابت `resetText`.
