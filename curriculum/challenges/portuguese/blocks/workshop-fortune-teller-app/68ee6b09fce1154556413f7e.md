...
Dentro do método `initializeEventListeners`, anexe um listener de evento `click` ao `singleCardBtn`. Dentro do manipulador do evento, faça a chamada do método `singleCardSelected`. Lembre-se de obter o `singleCardBtn` a partir do `elements`.

Você criará esse método nos próximos passos.

# --hints--

Você deve anexar um evento `click` ao elemento `singleCardBtn`. Não esqueça que aqui você precisa da palavra-chave `this`.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Dentro do callback do evento, você deve fazer a chamada do método `singleCardSelected`.
