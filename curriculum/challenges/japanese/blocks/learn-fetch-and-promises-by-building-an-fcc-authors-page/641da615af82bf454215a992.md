...
`.forEach()` メソッドに空のコールバック関数を渡してください。コールバックの最初のパラメータには、`author`、`image`、`url`、`bio` の項目を分割代入してください。

2番目のパラメータには `index` を渡してください。これは各著者の位置を表し、後でページネーションに役立ちます。
...
コールバック関数の最初のパラメータとして `author`、`image`、`url`、`bio` を分割代入してください。波括弧の中に入れるのを忘れないでください。
...
コールバック関数の2番目のパラメータとして `index` を分割代入してください。これは他のパラメータを入れた波括弧の外に置く必要があります。

```js
assert.match(code, /authors\.forEach\(\s*\(\s*\{.*\}\s*,\s*index\s*\)/)
```

コールバック関数はアロー構文を使ってください。
