...
接下来，在控制台记录文本 `"Quotients: "`，后面依次是逗号和 `quotients` 数组。 同时记录文本 `"Remainders: "`，后跟逗号和 `remainders` 数组。

# --hints--

你应该将文本 `"Quotients: "` 和 `quotients` 数组记录到控制台，以逗号分隔。

```js
assert.match(String(decimalToBinary), /console\.log\(\s*('|"|`)Quotients:\s*\1\s*,\s*quotients\s*\)/i);
```

你应该将文本 `"Remainders: "` 和 `remainders` 数组记录到控制台，以逗号分隔。
