...
Rimuovi la proprietà e il valore `background-color` da `.bb4` e aggiungili alle tre nuove sezioni `.bb4a`, `.bb4b` e `.bb4c`, in modo che solo le sezioni siano compilate.

# --hints--

Dovresti rimuovere `background-color` da `.bb4`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb4")?.backgroundColor);
```

Dovresti dare a `.bb4a` una `background-color` di `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.backgroundColor.trim(), "var(--building-color4)");
```

Dovresti dare a `.bb4b` una `background-color` di `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.backgroundColor.trim(), "var(--building-color4)");
```

Dovresti dare a `.bb4c` una `background-color` di `--building-color4`.
