...
`legend` のテキストをより目立たせるには、スタイルを設定する必要があります。

`fieldset legend` 要素を選択してください。`font-size` を `1.3rem` に、`font-weight` を `600` に設定してください。

`font-weight` はテキストの太さを制御します。

`font-weight` の一般的な値は次の通りです：
...
`font-weight` プロパティを `600` に設定してください。
