...
Create a new variable in `:root` named `window-color2` with a value of `#8cd9b3`. 这将被用作该建筑物的间色。

# --hints--

你应该在 `:root` 中创建一个名为 `window-color2` 的新属性变量。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--window-color2'));
```

你应该给 `window-color2` 一个值为 `#8cd9b3`。
