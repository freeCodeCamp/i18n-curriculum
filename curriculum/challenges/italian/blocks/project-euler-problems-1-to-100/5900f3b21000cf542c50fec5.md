...
title: 'Problema 70: permutazione del toziente'
challengeType: 1
forumTopicId: 302183
...
La funzione toziente di Euler, ${\phi}(n)$ (a volte chiamata funzione phi), serve a determinare il numero di numeri positivi minori o uguali a `n` che sono relativamente primi a `n`. Per esempio, poiché 1, 2, 4, 5, 7 e 8 sono tutti minori di nove e relativamente primi a nove, ${\phi}(9) = 6$. Il numero 1 è considerato relativamente primo a ogni numero positivo, quindi ${\phi}(1) = 1$.

Curiosamente, ${\phi}(87109) = 79180$, e si può vedere che 87109 è una permutazione di 79180.

Trova il valore di `n`, 1 &lt; `n` &lt; `limit`, per cui ${\phi}(n)$ è una permutazione di `n` e il rapporto $\displaystyle\frac{n}{{\phi}(n)}$ produce un minimo.
