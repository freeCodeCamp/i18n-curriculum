...
爲了增加頁面的可訪問性，`role` 屬性可用於向輔助技術指示頁面上元素背後的用途。 `role` 屬性是 _網絡無障礙倡議_（WAI）的一部分，並接受預設值。

爲每個 `section` 元素添加 `region` 角色。

# --hints--

你應爲第一個 `section` 元素添加 `region` 角色。

```js
assert.equal(document.querySelectorAll('section')?.[0]?.getAttribute('role'), 'region');
```

你應爲第二個 `section` 元素添加 `region` 角色。

```js
assert.equal(document.querySelectorAll('section')?.[1]?.getAttribute('role'), 'region');
```

你應爲第三個 `section` 元素添加 `region` 角色。
