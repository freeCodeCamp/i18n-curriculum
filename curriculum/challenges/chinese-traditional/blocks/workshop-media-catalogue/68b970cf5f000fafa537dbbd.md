...
由於你將導致例外的物件儲存在例外物件中，你可以對它進行一些使用。

在你現有的 `print` 呼叫之後，為你的程式碼添加 `print(f'Unable to add {e.obj}: {type(e.obj)}')`。

這種類型的訊息在你嘗試偵錯你的程式碼時非常有用。

# --hints--

你應該在現有的 `print(f'Media Error: {e}')` 呼叫之後加入 `print(f'Unable to add {e.obj}: {type(e.obj)}')`。
