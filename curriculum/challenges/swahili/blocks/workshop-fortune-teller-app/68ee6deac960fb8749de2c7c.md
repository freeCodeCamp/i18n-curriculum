...
Jumuisha hoja zilizobaki `chosenCard.name_short` na `chosenCard.img`. Kisha nje ya kitendakazi cha `renderCard`, weka upya `multipleCard` ya kipengele cha `innerHTML` kuwa mfuatano wa herufi tupu.

# --hints--

Unapaswa kupitisha `chosenCard.name_short` kama hoja kwa `renderCard`.
...
Unapaswa kupitisha `chosenCard.img` kama hoja ya mwisho kwa `renderCard`.
...
Unapaswa kuweka upya `this.elements.multipleCard.innerHTML` kuwa mfuatano wa herufi tupu.
