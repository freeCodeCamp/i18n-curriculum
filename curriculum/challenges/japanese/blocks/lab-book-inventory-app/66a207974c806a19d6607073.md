...
1. テキストが`Book Inventory`の`h1`要素が必要です。
1. `Title`、`Author`、`Category`、`Status`、`Rate`という名前の列を持つ`table`要素が必要です。
1. テーブルには少なくとも4行あり、最初の行は列の見出しで、残りは本の情報で埋められている必要があります。
1. テーブル本体内の各行には、`read`、`to-read`、または`in-progress`のいずれかのクラスが付いている必要があります。
1. `Status`列の`td`要素には、`status`の`class`で囲まれた`span`要素が含まれており、そのテキストは行のクラスに応じて`Read`、`To Read`、または`In Progress`です。
1. `Rate`列の`td`要素には、3つの空の`span`要素をラップする`rate`の`class`を持つ`span`要素が含まれている必要があります。
1. `.read`行内に配置された`.rate`要素には、個人評価に応じて`one`、`two`、または`three`のいずれかの値を持つ追加のクラスが必要です。この値は`rate`の後に続きます。
1. `read`、`to-read`、`in-progress`のクラスを持つ要素をターゲットにする3つの属性セレクターを作成し、それらの`background-image`プロパティに任意の`linear-gradient`を設定してください。
1. 各`span`要素の`display`プロパティを`inline-block`に設定してください。
1. `status`クラスを持つ`span`要素の子孫である`to-read`クラスの`tr`要素をターゲットにする属性セレクターを使用し、それらの`border`および`background-image`プロパティを設定してください。
1. `status`クラスを持つ`span`要素の子孫である`read`クラスの`tr`要素をターゲットにする属性セレクターを使用し、それらの`border`および`background-image`プロパティを設定してください。
1. `status`クラスを持つ`span`要素の子孫である`in-progress`クラスの`tr`要素をターゲットにする属性セレクターを使用し、それらの`border`および`background-image`プロパティを設定してください。
1. `status`クラスの`span`要素と、クラス値が`rate`で始まる`span`要素をターゲットにする属性セレクターを使用し、それらの`height`、`width`、および`padding`プロパティを設定してください。
1. `span`要素の直下の子であり、`class`値が`rate`で始まる`span`要素をターゲットにする属性セレクターを使用し、それらの`border`、`border-radius`、`margin`、`height`、`width`、および`background-color`プロパティを設定してください。
1. `span`要素の子孫のうち、`class`値に`one`という単語を含む最初の要素をターゲットにする属性セレクターを使用し、その`background-image`プロパティに`linear-gradient`を設定してください。
1. `span`要素の子孫のうち、`class`値に`two`という単語を含む最初の2つの要素をターゲットにする属性セレクターを使用し、それらの`background-image`プロパティに`linear-gradient`を設定してください。
1. `span`要素の子孫で、`class`値に`three`という単語を含む3つの`span`要素をターゲットにする属性セレクターを使用し、それらの`background-image`プロパティに`linear-gradient`を設定してください。

# --hints--

テキストが`Book Inventory`の`h1`要素が必要です。
...
`Status`列の`td`要素には`span`要素が含まれている必要があります。
...
`Rate`列の`td`要素には`span`要素が含まれている必要があります。
...
`.read`行内に配置された`.rate`要素には、`rate`クラスの後に個人評価に応じて`one`、`two`、または`three`のいずれかの値を持つ追加のクラスが必要です。
...
`status`クラスの`span`要素と、クラス値が`span`で始まる`rate`要素をターゲットにする属性セレクターを使用し、それらの`height`プロパティを設定してください。
...
`status`クラスの`span`要素と、クラス値が`span`で始まる`rate`要素をターゲットにする属性セレクターを使用し、それらの`width`プロパティを設定してください。
...
`status`クラスの`span`要素と、クラス値が`span`で始まる`rate`要素をターゲットにする属性セレクターを使用し、それらの`padding`プロパティを設定してください。
...
`span`要素の直下の子であり、`class`値が`rate`で始まる`span`要素をターゲットにする属性セレクターが必要です。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('span[class^="rate"] > span'));
```

`span`要素の直下の子であり、`class`値が`rate`で始まる`span`要素をターゲットにする属性セレクターを使用し、それらの`border`プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('span[class^="rate"] > span')?.getPropVal('border'));
```

`span`要素の直下の子であり、`class`値が`rate`で始まる`span`要素をターゲットにする属性セレクターを使用し、それらの`border-radius`プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('span[class^="rate"] > span')?.getPropVal('border-radius'));
```

`span`要素の直下の子であり、`class`値が`rate`で始まる`span`要素をターゲットにする属性セレクターを使用し、それらの`margin`プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('span[class^="rate"] > span')?.getPropVal('margin'));
```

`span`要素の直下の子であり、`class`値が`rate`で始まる`span`要素をターゲットにする属性セレクターを使用し、それらの`height`プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('span[class^="rate"] > span')?.getPropVal('height'));
```

`span`要素の直下の子であり、`class`値が`rate`で始まる`span`要素をターゲットにする属性セレクターを使用し、それらの`width`プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('span[class^="rate"] > span')?.getPropVal('width'));
```

`span`要素の直下の子であり、`class`値が`rate`で始まる`span`要素をターゲットにする属性セレクターを使用し、それらの`background-color`プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('span[class^="rate"] > span')?.getPropVal('background-color'));
```

`span`要素の子孫のうち、`class`値に`one`という単語を含む最初の要素をターゲットにする属性セレクターが必要です。
...
`span`要素の子孫のうち、`class`値に`one`という単語を含む最初の要素をターゲットにする属性セレクターを使用し、その`background-image`プロパティに`linear-gradient`を設定してください。
...
`span`要素の子孫のうち、`class`値に`two`という単語を含む最初の2つの要素をターゲットにする属性セレクターが必要です。
...
`span`要素の子孫のうち、`class`値に`two`という単語を含む最初の2つの要素をターゲットにする属性セレクターを使用し、それらの`background-image`プロパティに`linear-gradient`を設定してください。
