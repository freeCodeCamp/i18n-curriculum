...
プロジェクトのロジックを準備するために、`const` とアロー構文を使って `sortInputArray` 関数を宣言してください。1つの `event` パラメータを受け取る必要があります。

# --hints--

`sortInputArray` 関数を `const` で宣言してください。

```js
assert.match(code, /const\s+sortInputArray\s*=/);
```

`sortInputArray` は関数である必要があります。

```js
assert.isFunction(sortInputArray);
```

`sortInputArray` は1つの `event` パラメータを受け取る必要があります。

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)/);
```

`sortInputArray` はアロー構文を使う必要があります。

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)\s*=>/);
```

`sortInputArray` 関数は空である必要があります。
