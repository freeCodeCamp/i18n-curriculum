...
È il momento di spostare del codice dalla funzione `move()` alla funzione `make_allowed_move()`. Sposta il codice annidato all'interno della prima istruzione `if` (eccetto la prima chiamata `print()`) nella tua nuova funzione. Fai molta attenzione all'indentazione.  
Non dimenticare di rimuovere la parola chiave `pass`.

# --hints--

Dovresti spostare il codice annidato all'interno della prima istruzione `if` (eccetto la prima chiamata `print()`) nella tua nuova funzione.
