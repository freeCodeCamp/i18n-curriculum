...
`.one` の CSS ルールを更新して、新しい `red` クラスを対象にしてください。

# --hints--

コードに `.one` クラスセレクターが含まれていないことを確認してください。

```js
assert(!new __helpers.CSSHelp(document).getStyle('.one'));
```

クラス `red` を対象にするには、クラスセレクターを使う必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.red'));
```

`.red` の CSS ルールには、`background-color` プロパティが `rgb(0, 0, 0)` に設定されている必要があります。
