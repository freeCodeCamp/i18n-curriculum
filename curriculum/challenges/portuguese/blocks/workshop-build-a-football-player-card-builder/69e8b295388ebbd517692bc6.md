...
A foto do jogador é carregada a partir de uma URL fixa. Substitua o valor do atributo `src` `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` por `player.imageUrl`.

Também substitua o texto fixo `alt` por um valor dinâmico usando um template literal com `player.name` e o texto `headshot`.

Note que quando você usa uma expressão JavaScript como valor de atributo em TSX, você usa chaves `{}` em vez de aspas. Por exemplo `src={player.imageUrl}`, não `src="{player.imageUrl}"`.

# --hints--

Você deve substituir a URL fixa da imagem por `player.imageUrl` como o atributo `src`.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

Você deve substituir o texto fixo `alt` por um template literal usando `player.name` e o texto `headshot`.
