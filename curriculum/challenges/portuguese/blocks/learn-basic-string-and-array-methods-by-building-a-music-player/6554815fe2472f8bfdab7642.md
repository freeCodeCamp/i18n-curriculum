...
Use `const` e a sintaxe de arrow para criar uma função vazia `playPreviousSong`.

# --hints--

Você deve usar `const` para criar uma função vazia chamada `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` deve ser uma função.

```js
assert.isFunction(playPreviousSong)
```

Sua função `playPreviousSong` deve usar uma sintaxe de arrow.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Sua função `playPreviousSong` deve estar vazia.
