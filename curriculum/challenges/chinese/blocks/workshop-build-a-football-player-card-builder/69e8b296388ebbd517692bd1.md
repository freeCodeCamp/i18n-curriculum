...
再添加一个 `div`，其 `className` 为 `form-group`。在其中，添加一个 `label` 元素，其 `className` 为 `label`，`htmlFor` 为 `imageUrl`，以及文本 `Image URL`。然后添加一个 `input` 元素，其 `id` 为 `imageUrl`，`className` 为 `input`，以及 `type` 为 `text`。
