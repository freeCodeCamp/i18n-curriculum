...
Inside the `.btn-container`, create three buttons for each type of story. 为每个按钮赋予一个 `btn` 类，并为每个按钮赋予一个 `scary-btn`、`funny-btn` 和 `adventure-btn` 的 `id` 类。 每个按钮的文本应分别为 `Scary Story`、`Funny Story` 和 `Adventure Story`。

# --hints--

在 `.btn-container` 元素内，你应该有三个按钮。
...
你应该有一个按钮，其 `id` 为 `scary-btn`，文本为 `Scary Story`。
...
你应该有一个按钮，其 `id` 为 `funny-btn`，文本为 `Funny Story`。
...
你应该有一个按钮，其 `id` 为 `adventure-btn`，文本为 `Adventure Story`。
