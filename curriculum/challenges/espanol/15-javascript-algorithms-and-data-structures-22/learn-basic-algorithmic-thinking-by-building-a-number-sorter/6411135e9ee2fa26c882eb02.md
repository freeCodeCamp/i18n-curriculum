...
En cada iteración de tu `while` loop, está encontrando un elemento que es más grande que tu valor actual. Necesitas mover ese elemento a la derecha para hacer espacio para tu valor actual.

Hazlo asignando el valor en el índice `j` al siguiente índice.

# --hints--

Antes de decrementar `j`, asigna el valor en `j` al índice `j + 1`.
