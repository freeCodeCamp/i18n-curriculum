...
이제 코드를 일부 통합할 수 있습니다. `goTown` 함수 안의 코드를 복사해서 `update` 함수에 붙여넣으세요. 그런 다음 `goTown`와 `goStore` 함수 안의 모든 코드를 제거하세요.

# --hints--

`update` 함수는 `button1.innerText`를 `"Go to store"`로 설정해야 합니다.

```js
assert.match(update.toString(), /button1\.innerText\s*=\s*('|")Go to store\1/);
```

`update` 함수는 `button2.innerText`를 `"Go to cave"`로 설정해야 합니다.

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*('|")Go to cave\1/);
```

`update` 함수는 `button3.innerText`를 `"Fight dragon"`로 설정해야 합니다.

```js
assert.match(update.toString(), /button3\.innerText\s*=\s*('|")Fight dragon\1/);
```

`update` 함수는 `button1.onclick`를 `goStore`로 설정해야 합니다.

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*goStore/);
```

`update` 함수는 `button2.onclick`를 `goCave`로 설정해야 합니다.

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*goCave/);
```

`update` 함수는 `button3.onclick`를 `fightDragon`로 설정해야 합니다.

```js
assert.match(update.toString(), /button3\.onclick\s*=\s*fightDragon/);
```

`update` 함수는 `text.innerText`를 `"You are in the town square. You see a sign that says \"Store\"."`로 설정해야 합니다.

```js
assert.match(update.toString(), /text\.innerText\s*=\s*"You are in the town square. You see a sign that says \\"Store\\"\."/);
```

`goTown` 함수는 비워 두어야 합니다.

```js
assert.match(goTown.toString(), /function goTown\(\s*\) \{\}/);
```

`goStore` 함수는 비워 두어야 합니다.
