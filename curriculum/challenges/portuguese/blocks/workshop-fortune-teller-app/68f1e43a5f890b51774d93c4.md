...
Dentro do método `showFortune` declare uma variável chamada `target` e atribua-a a `e.target` e crie outra declaração `if` vazia.

# --hints--

Você deve declarar uma variável chamada `target` dentro do `showFortune`.
...
Sua variável `target` deve ser atribuída a `e.target`.
...
Você deve ter uma instrução `if` vazia logo após sua variável `target`.
