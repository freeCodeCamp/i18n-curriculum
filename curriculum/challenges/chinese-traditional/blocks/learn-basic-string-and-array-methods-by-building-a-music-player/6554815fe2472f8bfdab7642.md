...
使用 `const` 和箭頭語法來創建一個空的 `playPreviousSong` 函式（程式）。

# --hints--

你應該使用 `const` 來創建一個名為 `playPreviousSong` 的空函式（程式）。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` 應該是一個函式（程式）。

```js
assert.isFunction(playPreviousSong)
```

你的 `playPreviousSong` 函式應該使用箭頭語法。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `playPreviousSong` 函式（程式）應該是空的。
