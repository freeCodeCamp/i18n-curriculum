...
`cardElement` という名前の変数を宣言し、`card_container` クラスを使って要素を取得してください。`closest` から `target` セレクターを使って取得します。

# --hints--

`cardElement` の中で `showFortune` という名前の変数を宣言してください。
...
`cardElement` は `target?.closest` を使って `.card_container` を見つけるべきです。
...
`.card_container` に引数として `closest` を渡してください。
