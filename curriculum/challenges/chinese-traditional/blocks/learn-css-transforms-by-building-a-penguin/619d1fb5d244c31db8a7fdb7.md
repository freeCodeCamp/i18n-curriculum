...
將標的 `.foot` 元素設定為 `width` 為 `15%`、`height` 為 `30%`，以及 `background-color` 為 `orange`。
...
你應該給 `.foot` 一個 `width` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.width, '15%');
```

你應該給 `.foot` 一個 `height` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.height, '30%');
```

你應該給 `.foot` 一個 `background-color` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。
