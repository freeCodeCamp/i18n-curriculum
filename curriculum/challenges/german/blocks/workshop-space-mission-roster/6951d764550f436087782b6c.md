...
Sie haben einen `console.log()`-Aufruf zu Ihrem Skript hinzugefügt, um Ihr `EVAReadySquad`-Array zu validieren, und dieser Aufruf wird jetzt nicht mehr benötigt. Um zu verhindern, dass das Terminal in den nächsten Schritten unübersichtlich wird, entfernen Sie bitte die aktuelle `for`-Schleife, die `EVAReadySquad`-Astronauten in der Konsole ausgibt.

# --hints--

Entfernen Sie die `for`-Schleife, die `EVAReadySquad`-Astronauten in der Konsole ausgibt. Sie sollten den gesamten Schleifenblock löschen.
