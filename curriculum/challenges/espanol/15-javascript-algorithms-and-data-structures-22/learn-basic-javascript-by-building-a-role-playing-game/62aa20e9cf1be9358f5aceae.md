...
Agrega una declaración `else` a la primera declaración `if` dentro de tu función `attack()`. En la declaración `else`, usa el operador `+=` para agregar el texto `" You miss."` al final de `text.innerText`.

# --hints--

Debes agregar un bloque `else` después de tu bloque `if (isMonsterHit())`.
...
Debes agregar el texto `" You miss."` al final de `text.innerText`. Recuerda usar la asignación compuesta y asegúrate de dejar un espacio antes de la palabra `You`.
