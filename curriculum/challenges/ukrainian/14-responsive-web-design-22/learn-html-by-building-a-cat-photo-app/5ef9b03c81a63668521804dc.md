...
Ось приклад перемикача з опцією `cat`:
...
Перед полем тексту додайте перемикач з опцією, встановленою як:
...
Your new `input` element should be above the existing `input` with `type` attribute set to `text`. Вони знаходяться в неправильному порядку.
