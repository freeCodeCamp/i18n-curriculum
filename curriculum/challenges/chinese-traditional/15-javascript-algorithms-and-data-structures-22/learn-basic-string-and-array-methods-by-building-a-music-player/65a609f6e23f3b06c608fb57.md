...
`sort()` 方法接受定義排序順序的比較回調函數。

在此示例中，第一個條件 `(a.name < b.name)` 檢查第一個水果的名稱是否小於第二個水果的名稱。 如果是這樣，則第一個水果排在第二個水果之前。

字符串按字典順序進行比較，這意味着它們逐個字符進行比較。 例如，`"Apples"` 小於 `"Bananas"`，因爲按字母表順序，`"A"` 位於 `"B"` 之前。

此示例返回數字的原因是因爲 `sort()` 方法期望返回一個數字。 如果返回負數，則第一項將排在第二項之前。
...
在回調函數內部，添加 `if` 語句來檢查 `a.title` 是否小於 `b.title`。 如果是，則返回 `-1`。
...
你的 `if` 語句應檢查 `a.title` 是否小於 `b.title`。

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

如果 `a.title` 小於 `b.title`，則 `if` 語句應返回 `-1`。
