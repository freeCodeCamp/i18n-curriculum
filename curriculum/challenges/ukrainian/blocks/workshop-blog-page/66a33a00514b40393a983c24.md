...
Для третього допису в блозі вам потрібно додати ще один елемент `article`.

Всередині елемента `article` додайте елемент `h3` з текстом `Mr. Whiskers' First Birthday Party`.

Під вашим елементом `h3` додайте два абзаци тексту lorem ipsum.

# --hints--

У вас має бути третій елемент `article` всередині елемента `section` з множиною `id`, встановленою на `posts`.
...
У вас має бути елемент `h3` всередині елемента `article` з текстом `Mr. Whiskers' First Birthday Party`.
