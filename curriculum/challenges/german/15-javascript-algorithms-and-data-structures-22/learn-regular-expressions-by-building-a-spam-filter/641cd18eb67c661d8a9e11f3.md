...
Um das Projekt zu beginnen, verwende die `.getElementById()`-Methode, um die `#message-input`-, `#result`- und `#check-message-btn`-Elemente aus dem HTML-Dokument abzurufen und sie den Variablen `messageInput`, `result` bzw. `checkMessageButton` zuzuweisen.

# --hints--

Du solltest `const` verwenden, um eine `messageInput`-Variable zu deklarieren.
...
Du solltest `const` verwenden, um eine `result`-Variable zu deklarieren.
...
Du solltest `const` verwenden, um eine `checkMessageButton`-Variable zu deklarieren.
