...
Seleccione el elemento `.penguin-head`, y dele un `width` que sea la mitad de su padre, y un `height` de `45%`. Luego, establece el `background` en un degradado lineal a `45deg` comenzando en `gray`, y terminando en `rgb(239, 240, 228)`.
...
Debe dar a `.penguin-head` un `height` de `45%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

Debe dar a `.penguin-head` un `background` de `linear-gradient(45deg, gray, rgb(239, 240, 228))`.
