...
`map()` 方法用於遍歷數組並返回一個新數組。 當你想根據現有數組的值創建新數組時，它很有用。 舉個例子：
...
請注意，`map()` 方法接受函數作爲參數。 這被稱爲<dfn>回調函數</dfn>，它是作爲參數傳遞給另一個函數的函數。 在上面的例子中，回調函數是 `(number) => number * 2`，它在 `numbers` 數組中的每個元素上運行。 然後，`map()` 方法返回一個包含結果的新數組。

將回調函數傳遞給 `map()` 方法。 回調函數應該以 `song` 作爲參數，使用箭頭函數語法，並且具有空函數體。

# --hints--

你的 `map()` 方法的回調函數應該使用箭頭函數語法。

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

你的 `map()` 方法在其回調函數中應該有一個 `song` 參數。
