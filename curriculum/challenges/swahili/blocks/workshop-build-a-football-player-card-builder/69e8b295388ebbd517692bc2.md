...
Katika JSX na TSX, unapitisha usemi wa kihesabu wa JavaScript kama thamani za sifa ya kipengele kwa kutumia mabano ya mviringo. Herufi za kiolezo hutumia alama za kupigia na `${}` kwa uingizaji wa thamani. Kwa mfano:

```tsx
<div className={`prefix-${someVariable}`}>
```

Kifuniko cha kadi `className` kwa sasa kimeandikwa moja kwa moja kama `card-wrapper tier-gold`. Badilisha `gold` kwa thamani inayobadilika kwa kutumia herufi za kiolezo zinazofanya wito wa `getPlayerTier` na `player.overallRating`. Hii itarejea darasa la vitu la tier sahihi kutoka kwa CSS, kulingana na kiwango cha jumla cha mchezaji.

# --hints--

Unapaswa kutumia herufi za kiolezo na `getPlayerTier(player.overallRating)` kwa `className` ya `.card-wrapper`.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

Kipengele cha `.card-wrapper` kinapaswa kuwa na darasa la vitu la `tier-elite` wakati kiwango cha mchezaji ni 98.
