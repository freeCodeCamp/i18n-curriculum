...
當二次項係數為正時，拋物線有一個最小點並向上開口，或稱為向上凹。相反地，當二次項係數為負時，拋物線有一個最大點並向下開口，或稱為向下凹。
...
宣告一個 `concavity` 變數，並根據拋物線的凹凸性指定其為字串 `'upwards'` 或 `'downwards'`。同時，宣告一個名為 `min_max` 的變數，並根據頂點是最小值還是最大值分別指定其為字串 `'min'` 或 `'max'`。

最後，將字典添加到傳回兩個鍵 `'min_max'` 和 `'concavity'`，其值分別為 `min_max` 和 `concavity`。

# --hints--

你的 `analyze` 方法應該傳回一個包含四個鍵的字典，分別是 `'x'`、`'y'`、`'min_max'` 和 `'concavity'`，以及對應的值 `x`、`y`、`min_max` 和 `concavity`。
