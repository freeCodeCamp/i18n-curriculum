...
Benennen Sie innerhalb des Funktionskörpers die Variablen `text` und `shift` jeweils in `message` und `offset` um.

# --hints--

Sie sollten alle Vorkommen von `text` in `message` umbenennen.
...
Sie sollten `shift` in `offset` umbenennen.
