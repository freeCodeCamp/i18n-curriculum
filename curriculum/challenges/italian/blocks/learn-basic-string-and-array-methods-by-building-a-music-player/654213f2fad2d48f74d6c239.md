...
Crea un altro elemento `button` con la classe `playlist-song-delete` e l'attributo `aria-label` impostato su `Delete` interpolato con `song.title`. Per il contenuto dell'icona di eliminazione, incolla il seguente SVG:
...
Dovresti creare un elemento `button` con la classe `playlist-song-delete` come primo attributo e valore.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

Il tuo elemento button dovrebbe avere un attributo `aria-label` con il valore `Delete ${song.title}` come secondo attributo e valore.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

Dovresti incollare il `svg` fornito come contenuto del tuo elemento `button`.
