...
`.catch()`の中で、`console.error()`を削除し、`innerHTML`の`authorContainer`に`p` `class`を持つ`"error-msg"`要素をテキスト`"There was an error loading the authors"`で設定してください。
...
`innerHTML`の`authorContainer`にアクセスし、`p`要素を設定してください。`p`要素はテンプレートリテラルで囲むのを忘れないでください。

```js
assert.match(code, /authorContainer\.innerHTML\s*=\s*(`|"|')\s*<p.*>.*<\/p>\s*\1/)
```

`p`要素にはクラス`"error-msg"`を設定してください。

```js
assert.match(code, /(`|"|')\s*<p\s+class\s*=\s*("|')error-msg\2>/)
```

`p`要素にはテキスト`"There was an error loading the authors"`を設定してください。
