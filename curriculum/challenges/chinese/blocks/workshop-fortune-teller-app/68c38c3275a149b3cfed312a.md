...
在本次工作坊中，你将继续通过创建一个算命应用来练习使用 TypeScript。

首先声明一个名为 `Card` 的接口。在 `Card` 接口中，添加以下属性：`name`、`name_short` 和 `value`。`name` 和 `name_short` 属性的类型应设置为 `string`，而 `value` 应该设置为 `string | number` 的并集类型。
...
你的 `Card` 接口应该有一个类型为 `name` 的 `string` 属性。
...
你的 `Card` 接口应该有一个类型为 `name_short` 的 `string` 属性。
...
你的 `Card` 接口应该有一个类型为 `value` 和 `string` 并集的 `number` 属性。
