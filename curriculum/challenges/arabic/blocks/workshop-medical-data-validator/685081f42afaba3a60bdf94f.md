...
بعد ذلك، تريد التحقق من أن `age` عدد صحيح. أضف مفتاحًا آخر `age` إلى قاموس `constraints`. لقيمته، نفذ استدعاء `isinstance` مع تمرير `age` و `int` كمعلمتين له.

# --hints--

يجب أن يحتوي قاموس `constraints` على مفتاح `age`.
...
يجب أن يكون للمفتاح `age` في قاموس `constraints` القيمة الناتجة عن `isinstance(age, int)`.
