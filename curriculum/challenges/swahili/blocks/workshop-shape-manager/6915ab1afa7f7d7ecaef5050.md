...
Ili kuonyesha hatimaye mahesabu, rudia kupitia kitu cha `propertyInputs` kwa kutumia mzunguko wa `for...of` na `Object.entries()`. Unaweza kupuuza ufunguo kwa kutumia koma kabla ya `input` katika tamko la mzunguko.

# --hints--

Unapaswa kuunda mzunguko wa `for...of` unaovunjua kila kipengee kutoka `Object.entries(propertyInputs)` kwa kupita ufunguo na kuhifadhi tu `input`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Mzunguko wako wa `for...of` unapaswa kurudia juu ya `Object.entries(propertyInputs)`.
