...
`div`の`className`を持つ`card-stats`を追加してください。その中に、`div`の`className`を持つ`stat-col`を追加します。その中に、`div`の`className`を持つ`stat-row`要素を3つ追加してください。

# --hints--

`div`の後に、`className`の`card-stats`を持つ`.card-name-strip`を追加してください。

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

`div`の中に、`className`の`stat-col`を持つ`.card-stats`を追加してください。

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

`div`の中に、`className`の`stat-row`を持つ`.stat-col`要素を3つ追加してください。
