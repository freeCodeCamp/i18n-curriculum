...
Далі, між ім’ям і зображенням автора та рештою деталей на картці майже немає відокремлення. Роздільник надасть карткам авторів чітку візуальну ієрархію.

Додайте елемент `div` над біографією автора і надайте йому атрибут `class` зі значенням `"purple-divider"`.
...
Ви повинні створити елемент `div` перед вашим елементом `p`.

```js
await __helpers.retryingTest(() => document.querySelector('p')?.previousElementSibling?.tagName === 'DIV', "'div' element not found");
```

Ваш елемент `div` має мати множину `class` зі значенням `"purple-divider"`.
