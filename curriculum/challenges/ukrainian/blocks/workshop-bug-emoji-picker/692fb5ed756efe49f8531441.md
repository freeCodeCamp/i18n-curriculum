...
У класі `Bee` реалізуйте метод `render`, префіксований ключовим словом `override`, щоб оновити властивість `innerText` об’єкта `emojiElement` значенням властивості `emoji`.

# --hints--

Ваш клас `Bee` має містити метод `render`, який використовує ключове слово `override`.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

Ви маєте присвоїти `this.emojiElement.innerText` значення `this.emoji` всередині методу `render` класу `Bee`.
