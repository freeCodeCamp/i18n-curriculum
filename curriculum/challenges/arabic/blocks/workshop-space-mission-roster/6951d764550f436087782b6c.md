...
أضفت استدعاء `console.log()` إلى السكربت الخاص بك للتحقق من مصفوفة `EVAReadySquad`، والآن لم يعد هذا الاستدعاء ضروريًا. لمنع وحدة التحكم من التزاحم في الخطوات القادمة، احذف حلقة `for` الحالية التي تسجل رواد الفضاء في `EVAReadySquad` إلى وحدة التحكم.

# --hints--

احذف حلقة `for` التي تسجل رواد الفضاء في `EVAReadySquad` إلى وحدة التحكم. يجب حذف كتلة الحلقة بالكامل.
