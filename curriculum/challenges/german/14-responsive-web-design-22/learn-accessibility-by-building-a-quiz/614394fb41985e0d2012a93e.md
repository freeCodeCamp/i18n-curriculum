...
Die Fragennummern sind nicht beschreibend genug. Dies gilt insbesondere für sehbehinderte Nutzer. Eine Möglichkeit, ein solches Problem zu umgehen, ohne dem Element sichtbaren Text hinzufügen zu müssen, besteht darin, Text hinzuzufügen, den nur ein Screen-Reader lesen kann.

Füge ein `span` Element mit einer `class` von `sr-only` zu jedem der `h3` Elemente hinzu.
