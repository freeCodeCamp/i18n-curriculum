...
Actualiza la asignación de `employee_info` para concatenar también `employee_age` al final, usando el operador `+`.

Una vez que lo hayas hecho, verás un `TypeError` en el terminal. En el siguiente paso, trabajarás en corregirlo.

# --hints--

Debes actualizar la asignación de `employee_info` para concatenar `employee_age` al final.
