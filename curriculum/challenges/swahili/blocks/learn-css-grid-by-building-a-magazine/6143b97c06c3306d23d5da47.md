...
Ndani ya kipengele chako cha `article`, tengeneza kipengele cha `h3` chenye `class` iliyowekwa kuwa `list-title` na maandishi ya `A Brief History`. Chini ya hapo, tengeneza kipengele cha `p` chenye maandishi ya `Of the Curriculum`. Kisha tengeneza kipengele cha `ul` chenye darasa la vitu la `lists`.

# --hints--

Unapaswa kutengeneza kipengele cha `h3` ndani ya kipengele chako cha `article`.

```js
assert.exists(document.querySelector('article h3'));
```

Unapaswa kutengeneza kipengele cha `p` ndani ya kipengele chako cha `article`.

```js
assert.exists(document.querySelector('article p'));
```

Unapaswa kutengeneza kipengele cha `ul` ndani ya kipengele chako cha `article`.
...
Kipengele chako kipya cha `h3` kinapaswa kuwa na `class` iliyowekwa kuwa `list-title`.

```js
assert(document.querySelector('article h3')?.className === 'list-title');
```

Kipengele chako kipya cha `h3` kinapaswa kuwa na maandishi ya `A Brief History`.

```js
assert(document.querySelector('article h3')?.innerText === 'A Brief History');
```

Kipengele chako kipya cha `p` kinapaswa kuwa na maandishi ya `Of the Curriculum`.

```js
assert(document.querySelector('article p')?.innerText === 'Of the Curriculum');
```

Kipengele chako kipya cha `ul` kinapaswa kuwa na `class` iliyowekwa kuwa `lists`.
