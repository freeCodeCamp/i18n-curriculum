...
داخل دالة `getEVAReadyCrew`، نفذ استدعاء `sortByPriorityDescending()` مع المصفوفة المحلية `eligible` لترتيبها قبل إرجاعها.

# --hints--

يجب استدعاء `sortByPriorityDescending()` مع المصفوفة المحلية `eligible`.
