...
為了讓第一個區段看起來更行內，只針對 `.info` 元素內的 `input` 元素，並將它們的 `width` 設為 `50%`，且將它們的文字靠左對齊。

# --hints--

你應該使用 `.info input` 或 `.info > input` 選擇器。
...
你應該給 `input` 元素設定 `text-align` 為 `left`。
