...
在你的 `article` 元素中，创建一个 `h3` 元素，将 `class` 设置为 `list-title`，文本为 `A Brief History`。 在此之下，创建一个 `p` 元素，文本为 `Of the Curriculum`。 然后创建一个类为 `lists` 的 `ul` 元素。
...
你的新 `h3` 元素应将 `class` 设置为 `list-title`。
