...
Erstellen Sie eine Funktion namens `isSelect`, die einen einzelnen Parameter `element` vom Typ `EventTarget | null` entgegennimmt. Diese Funktion soll prüfen, ob das ausgewählte Element ein `<select>` HTML-Element ist.

# --hints--

Sie sollten eine Funktion namens `isSelect` haben.
...
Ihre `isSelect`-Funktion sollte einen Parameter namens `element` vom Typ `EventTarget | null` haben.
...
Ihre `isSelect`-Funktion sollte einen Rückgabetyp von `element is HTMLSelectElement` haben.
