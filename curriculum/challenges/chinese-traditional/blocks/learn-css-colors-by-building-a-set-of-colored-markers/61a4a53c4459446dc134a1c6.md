...
現在你知道了 `linear-gradient` 函數和色標的基本原理，你可以用它們來使馬克筆看起來更加真實。

在 `linear-gradient` 函數中，將 `gradientDirection` 設置爲 `180deg`。

# --hints--

你的 `.red` CSS 規則應該有一個 `background` 屬性，設置爲 `linear-gradient(180deg, rgb(255, 0, 0) 75%, rgb(0, 255, 0), rgb(0, 0, 255))`。
