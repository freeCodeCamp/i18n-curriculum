...
接下來，添加一個 `if` 語句，檢查 `collisionDetectionRules` 數組中的每個規則是否爲真。 確保爲此使用 `every` 方法。

在 `if` 語句的主體內，將數字 0 分配給玩家的 `y` 速度，然後跟上 `return` 語句。

# --hints--

你應該有一個 if 語句，使用 `every` 方法來檢查 `collisionDetectionRules` 數組中的每個規則是否爲真。

```js
assert.match(code, /if\s*\(\s*collisionDetectionRules\.every\(\s*(?:\(\s*(.+)\s*\)\s*=>\s*\1|([^\s()]+)\s*=>\s*\2)\s*\)\s*\)\s*\{\s*/);
```

你應該將數字 `0` 分配給玩家的 `y` 速度，然後在 `if` 語句主體內添加 `return` 語句。
