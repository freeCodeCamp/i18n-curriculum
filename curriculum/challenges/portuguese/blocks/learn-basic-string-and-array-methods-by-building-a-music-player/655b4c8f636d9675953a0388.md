...
Se não houver próxima música na playlist, use o bloco `else` para redefinir a chave `currentSong` de `userData` para `null` e sua propriedade `songCurrentTime` para `0`.
...
Você deve usar a notação de ponto para definir a propriedade `currentSong` de `userData` como `null` no bloco `else` da sua declaração `if`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

Você deve usar a notação de ponto para definir a propriedade `songCurrentTime` de `userData` para `0` no bloco `else` do seu `if` statement.
