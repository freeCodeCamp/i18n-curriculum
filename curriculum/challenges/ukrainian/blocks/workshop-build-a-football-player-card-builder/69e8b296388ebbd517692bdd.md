...
Додайте атрибут `value` зі значенням `player.pas` до PAS `input`. Потім додайте атрибут `onChange`, який дорівнює стрілковій функції, що приймає `e` як параметр і викликає `setPlayer` з розгортанням `player` та `pas`, встановленим у `Number(e.target.value)`.

# --hints--

Ваш PAS `input` має мати `value`, рівний значенню ігрової статистики `pas`.
...
Ваш PAS `input` має мати обробник `onChange`, який викликає `setPlayer` з розгортанням `player` та `pas`, встановленим у `Number(e.target.value)`.
