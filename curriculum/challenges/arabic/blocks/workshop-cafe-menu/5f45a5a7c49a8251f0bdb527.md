...
باستخدام نفس محدِّد النمط في الخطوة السابقة، اجعل حجم الخط للعناصر والأسعار أكبر باستخدام القيمة `18px`.

# --hints--

يجب تعيين خاصية `font-size` إلى `18px`.
...
يجب استخدام محدِّد `.item p` الموجود.
...
عناصر `p` الخاصة بك المسكنة داخل عناصر `.item` يجب أن يكون لها `font-size` بقيمة `18px`.
