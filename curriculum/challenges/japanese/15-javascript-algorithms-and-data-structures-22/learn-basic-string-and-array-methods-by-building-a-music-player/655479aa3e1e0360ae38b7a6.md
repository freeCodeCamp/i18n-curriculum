...
次に、プレイリスト内の次の曲を取得する必要があります。 そのためには、現在の曲のインデックスを取得し、それに `1` を加えます。

では `nextSong` という定数を作成し、`userData?.songs[currentSongIndex + 1]` を代入してください。

最後に、`playSong` 関数を呼び出し、引数として `nextSong.id` を渡してください。

# --hints--

既存の `if` 文、その `else` ブロック、および内容を変更しないでください。
...
定数 `nextSong` に `userData?.songs[currentSongIndex + 1]` を代入してください。
...
関数 `playSong` に `nextSong.id` を渡して呼び出してください。
