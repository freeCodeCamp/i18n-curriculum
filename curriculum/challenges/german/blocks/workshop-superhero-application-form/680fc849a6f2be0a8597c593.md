...
Erstellen Sie nun die Zustandsvariablen und Setter für `powerSource` und `powers`. `powerSource` sollte einen Anfangswert als leeren String haben, und `powers` sollte einen Anfangswert als leeres Array haben.

# --hints--

Sie sollten Array-Destrukturierung verwenden, um eine `powerSource`-Zustandsvariable und einen `setPowerSource`-Setter zu setzen.

```js
assert.match(code, /(const|let|var)\s+\[\s*powerSource\s*,\s*setPowerSource\s*\]/);
```

Ihre `powerSource` und `setPowerSource` sollten den `useState`-Hook verwenden.
...
Ihr `useState`-Hook für `powerSource` sollte einen Anfangswert als leeren String haben.
...
Sie sollten Array-Destrukturierung verwenden, um eine `powers`-Zustandsvariable und einen `setPowers`-Setter zu setzen.

```js
assert.match(code, /(const|let|var)\s+\[\s*powers\s*,\s*setPowers\s*\]/);
```

Ihre `powers` und `setPowers` sollten den `useState`-Hook verwenden.
...
Ihr `useState`-Hook für `powers` sollte einen Anfangswert als leeres Array haben.
