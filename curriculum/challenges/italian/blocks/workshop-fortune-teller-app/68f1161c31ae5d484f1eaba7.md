...
Ora aggiungi un altro metodo chiamato `showFortune`. Passa come parametro `e` e assegna a questo il tipo `Event`. Crea anche un metodo `newReading` e lascialo vuoto per ora.

# --hints--

Dovresti dichiarare un metodo chiamato `showFortune`.
...
Il tuo metodo `showFortune` dovrebbe avere un parametro `e` con il tipo `Event`.
...
Dovresti dichiarare un metodo chiamato `newReading`.
