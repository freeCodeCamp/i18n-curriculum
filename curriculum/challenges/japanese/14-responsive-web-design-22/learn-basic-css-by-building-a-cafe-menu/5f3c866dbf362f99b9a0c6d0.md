...
上記のセレクターを使用し、`display` プロパティと値 `inline-block` を追加して、`p` 要素がインライン要素のように動作するようにしてください。
...
`display` プロパティを `inline-block` に設定する必要があります。
...
`.item p` セレクターの `display` プロパティを `inline-block` に設定する必要があります。
