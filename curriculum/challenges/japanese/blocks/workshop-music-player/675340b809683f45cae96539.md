...
曲が再生されていないときに前の曲ボタンを押すと、最初の曲が再生されます。これを修正するには、`if` 関数の先頭に `playPreviousSong` 文を追加してください。現在の曲が `null` かどうかを確認し、`return` 文で関数の実行を停止してください。

# --hints--

`if` 関数内に、`userData.currentSong === null` をチェックして関数の実行を `return` 文で停止する `playPreviousSong` 文を追加する必要があります。
