...
Щоб нарешті запрацювала функція localStorage, змініть ініціалізацію стану з `defaultPlayer` на `loadPlayer`. Це гарантує, що стан починається з будь-яких доступних збережених даних.

Тепер протестуйте функцію localStorage. Все, що ви введете, має зберігатися після оновлення попереднього перегляду або перезавантаження сторінки.

Ви також можете змінити посилання на зображення `src` на `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png`, щоб побачити зміну зображення в реальному часі. Просто переконайтеся, що ви змінили ім’я гравця на Maradona.

Отже, ваш проєкт конструктора картки футболіста завершено!

# --hints--

Ваш `useState` має бути ініціалізований `loadPlayer` замість `defaultPlayer`.
