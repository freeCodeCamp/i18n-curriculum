...
为了提供退出程序的方法，请使用另一个 `elif` 子句来检查 `choice` 是否等于字符串 `'5'`。

在新的 `elif` 子句中，打印字符串 `'Exiting the program.'` 以表明程序正在终止其执行。

# --hints--

你应该创建一个 `elif` 子句来检查是否 `choice == '5'`。
...
你应该在新的 `elif` 子句中打印 `'Exiting the program.'`。
