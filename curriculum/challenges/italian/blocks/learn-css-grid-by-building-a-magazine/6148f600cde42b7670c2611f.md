...
L'area di testo in basso verrà visualizzata in una singola colonna su schermi più piccoli del valore specificato.

# --hints--

Dovresti creare una nuova `@media` query per `only screen and (max-width: 600px)`. Dovrebbe trovarsi sotto la `@media` query precedente.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

La nuova `@media` query dovrebbe avere un selettore `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

Il nuovo selettore `.text-with-images` dovrebbe avere una proprietà `grid-template-columns` con un valore di `1fr`.
