...
All'interno del loop `for`, dichiara una variabile `row_str` e assegnale una list comprehension che itera su `row` e trasforma ogni elemento `i` in `row` in una stringa. Usa la funzione `str()` per farlo.

# --hints--

Dovresti dichiarare una variabile `row_str`.
...
Dovresti assegnare una list comprehension che itera su `row` alla variabile `row_str`.
...
La list comprehension assegnata alla variabile `row_str` dovrebbe usare `i` come variabile di iterazione.
...
La list comprehension assegnata alla variabile `row_str` dovrebbe chiamare `str()` su ogni elemento `i` in `row`.
