...
Перед початком симуляції вам потрібен об’єкт, щоб відстежувати, скільки відвідувачів обробляє кожні ворота за всі поділки. Викличте `initializeThroughput()` з `gates` і призначте результат змінній з ім’ям `throughputSummary`.

# --hints--

Ви повинні викликати `initializeThroughput()` з `gates` і призначити результат змінній з ім’ям `throughputSummary`.
...
Ваша функція `simulateFestival` має викликати функцію `initializeThroughput()` з `gates`.
