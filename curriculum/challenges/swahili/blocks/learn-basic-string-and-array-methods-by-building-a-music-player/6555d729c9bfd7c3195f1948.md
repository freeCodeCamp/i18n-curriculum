...
`textContent` huweka maandishi ya nodi na hukuruhusu kuweka au kupata maudhui ya maandishi ya kipengee cha HTML.
...
Unaweza kutumia opereta ya ternary kuweka thamani ya maudhui ya maandishi kwa masharti. Hapa kuna mfano wa kuweka matokeo ya opereta ya ternary katika kigezo:

```js
const example = condition ? "I'm true" : "I'm false";
```

Tumia opereta ya ternary kuangalia kama `currentTitle` inatathmini kuwa thamani ya kweli. Ikiwa ni hivyo, weka `playingSong.textContent` kuwa `currentTitle`. Vinginevyo, weka kuwa mfuatano wa herufi tupu.

Kisha chini ya hapo, tumia opereta ya `ternary` kuangalia kama `currentArtist` ni thamani ya kweli. Ikiwa ni hivyo, weka `songArtist.textContent` kuwa `currentArtist`. Vinginevyo, weka kuwa mfuatano wa herufi tupu.

# --hints--

Unapaswa kuunganisha sifa ya `textContent` na `playingSong`.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

Unapaswa kutumia opereta ya `ternary` kuweka sifa ya `textContent` ya `playingSong` kuwa `currentTitle` au `""`.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

Unapaswa kuunganisha sifa ya `textContent` na `songArtist`.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

Unapaswa kutumia opereta ya `ternary` kuweka sifa ya `textContent` ya `songArtist` kuwa `currentArtist` au `""`.
