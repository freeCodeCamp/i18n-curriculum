...
下一個價格方案卡是 **Premium Plan**。

為高級方案的外覆 `div` 添加類別 `bg-gray-950`、`text-white`、`ring-2`、`ring-fuchsia-500` 和 `p-8`。

# --hints--

你的高級方案的外包 `div` 元素應該有類別 `bg-gray-950`。
...
你的高級方案的外覆 `div` 元素應該有類別 `text-white`。
...
你的高級方案的外覆 `div` 元素應該有類別 `ring-fuchsia-500`。
