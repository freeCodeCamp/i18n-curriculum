...
按照<a href="https://www.freecodecamp.org/news/how-to-set-up-a-github-oauth-application/" target="_blank" rel="noopener noreferrer nofollow">這些說明</a>從 GitHub 獲取你的 *客戶端 ID 和密鑰*。 Set the homepage URL to your homepage (**not the project code's URL**), and set the callback URL to the same homepage URL with `/auth/github/callback` appended to the end. 將客戶端 ID 和你的客戶端密碼保存到你的項目的 `.env`，作爲 `GITHUB_CLIENT_ID` 和 `GITHUB_CLIENT_SECRET`。
