...
`lin_eq` の係数を自由に変更して、出力がどのように変わるか確認してください。

その後、`print(solver(lin_eq))` の呼び出しを削除し、`solver()` を引数にして `quadr_eq` を呼び出した結果を出力してください。

# --hints--

コード内に `print(solver(lin_eq))` があってはいけません。

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

`solver(quadr_eq)` を出力してください。
