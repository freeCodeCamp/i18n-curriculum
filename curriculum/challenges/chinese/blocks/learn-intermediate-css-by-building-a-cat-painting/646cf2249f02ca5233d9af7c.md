...
使用类选择器，为 `.cat-right-ear` 元素的左右边框分别设置 `35px solid transparent`。 另外，将底部边框设置为 `70px solid #5e5e5e`。
