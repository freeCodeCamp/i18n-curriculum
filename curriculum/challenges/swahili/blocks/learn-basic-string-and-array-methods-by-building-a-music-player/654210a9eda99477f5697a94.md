...
Unda kipengele cha `button` chenye darasa la vitu la `playlist-song-info`. Ndani ya `button`, ongeza kipengele cha `span` chenye darasa la vitu la `playlist-song-title`, kisha weka `song.title` kama maandishi.

# --hints--

Unapaswa kuunda `button` chenye darasa la vitu la `playlist-song-info`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

Unapaswa kuunda kipengele cha `span` ndani ya kipengele chako cha `button`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

Kipengele chako cha `span` kinapaswa kuwa na darasa la vitu la `playlist-song-title`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

Unapaswa kuweka `song.title` kama maudhui ya kipengele chako cha `span`.
