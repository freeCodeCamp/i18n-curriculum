...
仍然在 `Bug` 类中，声明一个名为 `emojiElement` 的属性，类型为 `HTMLParagraphElement`。该属性将保存一个对显示错误表情符号的超文本标记语言段落元素的引用。

# --hints--

你的 `Bug` 类应该有一个名为 `emojiElement` 的属性。
...
你的 `emojiElement` 属性应该具有 `HTMLParagraphElement` 类型。
...
你的 `emojiElement` 属性应该使用确定赋值断言操作符（`!`）。
