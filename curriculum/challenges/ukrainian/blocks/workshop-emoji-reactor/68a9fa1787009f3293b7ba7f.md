...
Ви, мабуть, помітили, що ваш код стає досить повторюваним! Існує кращий спосіб працювати з кількома схожими елементами.

Замість того, щоб вибирати кожну кнопку окремо, використайте `querySelectorAll(".emoji-btn")`, щоб вибрати всі кнопки з класом `emoji-btn` одночасно. Збережіть результат у змінну з назвою `btns`.
...
Ви повинні використати `querySelectorAll`, щоб вибрати всі кнопки з емодзі.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

Ви повинні зберегти результат у змінну з назвою `btns`.
...
Змінна `btns` має містити всі 4 кнопки з емодзі.
