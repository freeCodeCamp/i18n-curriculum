...
Ahora el algoritmo de auto-colocación entrará en acción cuando agregues un nuevo elemento de ícono. Sin embargo, el algoritmo por defecto asigna al nuevo ancho de columna el valor `auto`, que no coincidirá con las columnas actuales.

Puedes anular esto utilizando la propiedad `grid-auto-columns`. Dale al selector `.social-icons` una propiedad `grid-auto-columns` establecida en `1fr`.

# --hints--

Tu selector `.social-icons` debe tener una propiedad `grid-auto-columns` establecida en `1fr`.
