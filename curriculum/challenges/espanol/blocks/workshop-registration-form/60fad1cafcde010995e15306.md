...
Con los envíos de formularios, es útil y una buena práctica proporcionar a cada elemento envíable un atributo `name`. Este atributo se usa para identificar el elemento en el envío del formulario.

Excepto para las dos entradas de `radio` (que ya has nombrado), dale a cada elemento envíable un atributo `name` único de tu elección.

# --hints--

Debe darle al `input` que espera un nombre un atributo `name`. _P.D. Yo habría elegido `first-name`_

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(1) > label:nth-child(1) > input')?.name);
```

Debe darle al `input` que espera un apellido un atributo `name`. _P.D. Yo habría elegido `last-name`_

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(1) > label:nth-child(2) > input')?.name);
```

Debe darle al `email` un atributo `name`. _P.D. Yo habría elegido `email`_

```js
assert.isNotEmpty(document.querySelector('input[type="email"]')?.name);
```

Debe darle al `password` un atributo `name`. _P.D. Yo habría elegido `password`_

```js
assert.isNotEmpty(document.querySelector('input[type="password"]')?.name);
```

Debe darle al `checkbox` un atributo `name`. _P.D. Yo habría elegido `terms`_

```js
assert.isNotEmpty(document.querySelector('input[type="checkbox"]')?.name);
```

Debe darle al `file` un atributo `name`. _P.D. Yo habría elegido `file`_

```js
assert.isNotEmpty(document.querySelector('input[type="file"]')?.name);
```

Debe darle al `number` un atributo `name`. _P.D. Yo habría elegido `age`_

```js
assert.isNotEmpty(document.querySelector('input[type="number"]')?.name);
```

Debe darle al elemento `select` un atributo `name`. _P.D. Yo habría elegido `referrer`_

```js
assert.isNotEmpty(document.querySelector('select')?.name);
```

Debe darle al elemento `textarea` un atributo `name`. _P.D. Yo habría elegido `bio`_
