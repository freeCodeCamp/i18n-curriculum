...
在 `.heading` 元素下方，創建一個新的 `section` 元素，並將 `class` 設置爲 `text`。 在其中，創建一個 `p` 元素，將 `class` 設置爲 `first-paragraph`，並添加以下文本：
...
你的新 `section` 元素應將 `class` 設置爲 `text`。
...
你的新 `p` 元素應將 `class` 設置爲 `first-paragraph`。
