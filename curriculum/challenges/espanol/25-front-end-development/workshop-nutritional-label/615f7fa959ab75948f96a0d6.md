...
Debajo de su último elemento `p`, agrega otro elemento `p` con el texto `Dietary Fiber 4g`. Asigna al elemento `p` la `class` necesaria para indentarlo y eliminar el borde divisorio. Luego crea un divisor debajo de ese elemento `p`.
...
Tu nuevo elemento `p` debe tener el texto `Dietary Fiber 4g`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Dietary Fiber[\s|\n]+4g/));
```

Tu nuevo elemento `p` debe tener el atributo `class` establecido como `indent no-divider`.
