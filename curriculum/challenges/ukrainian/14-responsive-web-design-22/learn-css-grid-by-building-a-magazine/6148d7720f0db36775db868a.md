...
You will need to have a column for text and a column for images. Надайте селектору `.text-with-images` властивість `grid-template-columns` зі значенням `1fr 2fr`. Also set the `column-gap` property to `3rem` to provide more spacing between the columns.
