...
Тепер ви готові почати створювати макет сітки. У попередніх уроках ви дізналися, що CSS Grid пропонує двовимірний макет на основі сітки, який дозволяє центрувати елементи по горизонталі та вертикалі, при цьому зберігаючи контроль для таких речей, як накладання елементів.

Почніть зі створення селектора `main` і надайте йому властивість `display`, встановлену в `grid`.

# --hints--

У вас має бути селектор `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Ваш селектор `main` повинен мати властивість `display`, встановлену в `grid`.
