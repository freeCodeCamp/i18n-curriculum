...
Dentro de la función `getElement`, usa `querySelector` con los genéricos y guarda el resultado en una variable. Añade una sentencia `if` para verificar si el elemento no existe y, en ese caso, lanza un error con el mensaje `Element not found: ${error message here}`. Si el elemento existe, devuélvelo.

# --hints--

Debes declarar una variable llamada `el` dentro de la función `getElement`.
...
Tu variable `el` debe asignarse con el valor `document.querySelector<T>(selector)`.
...
Debes tener una sentencia if que verifique si `el` no se encontró.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

Debes lanzar un nuevo Error con el mensaje `Element not found: ${selector}` dentro de tu sentencia `if`.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

Debes devolver la variable `el` dentro de la función getElement.
