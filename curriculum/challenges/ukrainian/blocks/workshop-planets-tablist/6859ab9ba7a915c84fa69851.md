...
Для другого елемента `tabpanel` додайте атрибут `id` зі значенням `"panel-saturn"` та атрибут `aria-labelledby` зі значенням `"tab-saturn"`.

Для третього елемента `tabpanel` додайте атрибут `id` зі значенням `"panel-mars"` та атрибут `aria-labelledby` зі значенням `"tab-mars"`.

# --hints--

Ваш другий елемент `tabpanel` має мати атрибут `id`.

```js
assert.isTrue(document.querySelectorAll('[role="tabpanel"]')[1].hasAttribute("id"));
```

Ваш другий атрибут `id` має мати значення `"panel-saturn"`.

```js
assert.equal(document.querySelectorAll('[role="tabpanel"]')[1].getAttribute("id"), "panel-saturn");
```

Ваш другий елемент `tabpanel` має мати атрибут `aria-labelledby`.

```js
assert.isTrue(document.querySelectorAll('[role="tabpanel"]')[1].hasAttribute("aria-labelledby"));
```

Ваш другий атрибут `aria-labelledby` має мати значення `"tab-saturn"`.

```js
assert.equal(document.querySelectorAll('[role="tabpanel"]')[1].getAttribute("aria-labelledby"), "tab-saturn");
``` 

Ваш третій елемент `tabpanel` має мати атрибут `id`.

```js
assert.isTrue(document.querySelectorAll('[role="tabpanel"]')[2].hasAttribute("id"));
```

Ваш третій атрибут `id` має мати значення `"panel-mars"`.

```js
assert.equal(document.querySelectorAll('[role="tabpanel"]')[2].getAttribute("id"), "panel-mars");
```

Ваш третій елемент `tabpanel` має мати атрибут `aria-labelledby`.

```js
assert.isTrue(document.querySelectorAll('[role="tabpanel"]')[2].hasAttribute("aria-labelledby"));
```

Ваш третій атрибут `aria-labelledby` має мати значення `"tab-mars"`.
