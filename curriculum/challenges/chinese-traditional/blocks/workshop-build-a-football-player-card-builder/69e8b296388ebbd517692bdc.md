...
為 SHO `input` 添加一個 `value` 屬性，屬性值為 `player.sho`。接著將其 `onChange` 屬性設為一個箭頭函式，該函式以 `e` 作為參數，並呼叫 `setPlayer`，傳入展開的 `player` 和 `sho`，其中 `sho` 設為 `Number(e.target.value)`。

# --hints--

你的 SHO `input` 應該有一個 `value`，其值等於當前玩家的 `sho` 狀態。
...
你的 SHO `input` 應該有一個 `onChange` 處理常式，該處理常式呼叫 `setPlayer`，並傳入展開的 `player` 和 `sho`，其中 `sho` 設為 `Number(e.target.value)`。
