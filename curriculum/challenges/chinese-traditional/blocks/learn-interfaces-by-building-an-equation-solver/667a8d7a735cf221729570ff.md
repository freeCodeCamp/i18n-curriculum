...
為當方程式為二次方程式時添加另一個 `case`。使用與 `QuadraticEquation` 的 `analyze` 方法傳回的格式相同的字典。

然後，將 `details_list` 指定為一個包含兩個字串的列表，格式分別為 `concavity = <concavity>` 和 `<min_max> = (<x>, <y>)`。將 `<x>` 和 `<y>` 格式化為顯示 3 位小數。

最後，在 `match`/`case` 區塊之後，迭代 `details_list` 並將每個項目添加到 `output_string` 的當前值。確保每個字串項目以換行字元結尾。此處不要使用任何額外的格式選項。
