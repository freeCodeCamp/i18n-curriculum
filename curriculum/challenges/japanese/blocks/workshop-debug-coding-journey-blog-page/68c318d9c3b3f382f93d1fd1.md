...
ページのメインタイトルの下に、各ブログ投稿へのリンクを含むナビゲーションセクションがあります。

このセクションはナビゲーションのランドマークとしての役割を示すために、`nav` 要素で囲む必要があります。
...
`<h2>Navigation</h2>` は `nav` 要素の中に配置してください。
...
追加の `ul`、`li`、または `h2` 要素を追加しないでください。
