...
Aktualisieren Sie Ihren `.map()` Callback, sodass er die `Number()` Funktion aufruft. Übergeben Sie `dropdown.value` an diesen Funktionsaufruf.

Öffnen Sie den Tab Konsole, um zu sehen, dass Ihr `inputValues` jetzt ein Array von Zahlen ist.

# --hints--

Sie sollten die `Number()` Funktion in Ihrem `.map()` Callback aufrufen.
...
Sie sollten `dropdown.value` an Ihren `Number()` Aufruf übergeben.
