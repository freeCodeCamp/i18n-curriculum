...
أضف خاصية `ounce` إلى `conversionTable`، وعيّن لها كائنًا يحتوي على خاصية `cup` بقيمة `0.125`، وخاصية `gram` بقيمة `28.35`، وخاصية `teaspoon` بقيمة `6`.

# --hints--

يجب أن يحتوي كائن `conversionTable` على خاصية `ounce`.

```js
assert.property(conversionTable, "ounce");
```

يجب أن تكون خاصية `ounce` كائنًا.

```js
assert.isObject(conversionTable.ounce);
```

يجب أن يحتوي كائن `ounce` على خاصية `cup`.

```js
assert.property(conversionTable.ounce, "cup");
```

يجب تعيين خاصية `cup` إلى القيمة `0.125`.

```js
assert.strictEqual(conversionTable.ounce?.cup, 0.125);
```

يجب أن يحتوي كائن `ounce` على خاصية `gram`.

```js
assert.property(conversionTable.ounce, "gram");
```

يجب تعيين خاصية `gram` إلى القيمة `28.35`.

```js
assert.strictEqual(conversionTable.ounce?.gram, 28.35);
```

يجب أن يحتوي كائن `ounce` على خاصية `teaspoon`.

```js
assert.property(conversionTable.ounce, "teaspoon");
```

يجب تعيين خاصية `teaspoon` إلى القيمة `6`.
