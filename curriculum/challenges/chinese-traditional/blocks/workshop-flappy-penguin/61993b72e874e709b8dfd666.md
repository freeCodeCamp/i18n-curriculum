...
將標的 `.penguin-head` 元素的 `width` 設為其父元素的一半，並將 `height` 設為 `45%`。接著，將 `background` 設為從 `gray` 開始、以 `45deg` 角度的線性漸層，並以 `rgb(239, 240, 228)` 結束。

# --hints--

你應該使用 `.penguin-head` 選擇器。

```js
assert.match(code, /\.penguin-head\s*\{/);
```

你應該給 `.penguin-head` 一個 `width` 為 `50%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

你應該給 `.penguin-head` 一個 `height` 為 `45%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

你應該給 `.penguin-head` 一個 `background` 為 `linear-gradient(45deg, gray, rgb(239, 240, 228))`。
