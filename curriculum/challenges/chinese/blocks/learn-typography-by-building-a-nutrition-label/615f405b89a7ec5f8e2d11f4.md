...
为新的 `.divider` 创建一个选择器，并将 `border-bottom` 属性设置为 `1px solid #888989`。 再给它一个 `2px` 的上下外边距。 它不应该有任何左右外边距。
...
你的 `.divider` 选择器应该有一个 `border-bottom` 属性，设置为 `1px solid #888989`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.divider')?.borderBottom === '1px solid rgb(136, 137, 137)');
```

你的 `.divider` 选择器应该有一个 `margin` 属性，设置为 `2px 0`。
