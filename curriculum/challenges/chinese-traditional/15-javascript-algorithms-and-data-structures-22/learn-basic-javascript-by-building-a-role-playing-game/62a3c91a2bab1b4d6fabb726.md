...
現在創建一個 `goCave` 函數，將 `"Going to cave."` 打印到控制檯。

# --hints--

你應該使用 `function` 關鍵字來聲明 `goCave`。
...
你的代碼中應該有一個 `console.log("Going to cave.");` 行。

```js
assert.match(code, /console\.log\(\s*('|")Going to cave\.\1\s*\)/);
```

你的 `console.log("Going to cave.");` 行應該位於你的 `goCave` 函數內。
