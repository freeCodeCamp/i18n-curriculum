...
2. Cuando se crea un objeto `Rectangle`, debe inicializarse con atributos `width` y `height`. La clase debe contener también los siguientes métodos:
...
5. Cuando se crea un objeto `Square`, debe inicializarse con una sola longitud de lado. El método `__init__` debe almacenar la longitud del lado en ambos atributos `width` y `height` de la clase `Rectangle`.
...
El método `.get_picture()` debe devolver la cadena `Too big for picture.` si los atributos `width` o `height` son mayores que `50`.
