...
Poi, nell'elemento checkbox `input`, aggiungi un attributo `checked` e impostalo su `isChecked`. Anche se non è strettamente necessario impostare l'attributo `checked`, è una buona pratica assicurarsi che ogni casella di spunta rifletta lo stato attuale dell'array `selectedItems`.

# --hints--

Dovresti aggiungere un attributo `checked` all'elemento checkbox `input`.
...
Dovresti impostare l'attributo `checked` su `isChecked`.
