...
بعد ذلك، ستبدأ العمل على `JavaScript`. للقيام بذلك، ابدأ بربط ملف `script.js` بصفحة HTML الخاصة بك.

أنشئ عنصر `script` مع خاصية `src` تشير إلى ملف `script.js`. على سبيل المثال:
...
يجب أن تنشئ عنصر `script`.

```js
assert.lengthOf(document.querySelectorAll('script'), 1);
```

يجب أن يحتوي عنصر `script` على خاصية `src` مضبوطة على `script.js`.
