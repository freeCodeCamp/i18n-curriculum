...
雖然這個表達式會符合 `1 hundred dollars`，但不會符合 `1  hundred  dollars` 或 `10 dollars`。

垃圾訊息會並且一定會找到方法利用你偵測中的漏洞。是時候改進你的 regex。

將第一個字面空白取代為 `\s*` 運算式。`\s` 字元類別會比對空白字元，例如空格、製表符和換行。`*` 量詞表示「比對前一個字元 0 次或多次」。

將第二個字元空白取代為 `\s+`。`+` 量詞表示「至少匹配前一個字元一次」。

# --hints--

你的 `dollarRegex` 不得使用字面空格。

```js
assert.notMatch(dollarRegex.source, / /);
```

你的 `dollarRegex` 必須允許在美元數量前有任意數量的空格，且在其後有一個或多個空格。
