...
Тепер потрібно знову додати атрибут до пісні, яка зараз відтворюється.

Створіть інструкцію `if` з умовою `songToHighlight`. Для інструкції, використайте `setAttribute` на `songToHighlight`, щоб передати `"aria-current"` та `"true"` як перший та другий аргументи.

# --hints--

Створіть інструкцію `if` з умовою `songToHighlight`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

Використайте метод `setAttribute()` на `songToHighlight` у своєї інструкції `if`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

Передайте `"aria-current"` та `"true"` як перший та другий аргумент методу `setAttribute`.
