...
Para la etiqueta de apertura `a`, establece el atributo `target` a `"_blank"`. Luego, ajusta el atributo `href` a `${forumTopicUrl}${slug}/${id}`.

Y con esos cambios, ¡tu proyecto de tabla de clasificación de foros freeCodeCamp ya está completado!

# --hints--

Tu etiqueta de anclaje debe tener un atributo `target` establecido en `"_blank"`.
...
Tu etiqueta de anclaje debe tener un atributo `href` establecido en `${forumTopicUrl}${slug}/${id}`.
