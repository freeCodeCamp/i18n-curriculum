...
你需要使用 `remove()` 從 DOM 中刪除任務，並使用 `splice()` 從 `taskData` 數組中刪除任務。

`splice()` 是一個數組方法，它通過刪除、替換或添加指定索引處的元素來修改數組，同時返回被刪除的元素。 它最多可以接受三個參數：第一個是必須的起始索引，第二個是需要刪除的項目數，第三個是可選的替換元素。 這是一個示例：
...
使用 `remove()` 方法從 DOM 中刪除 `buttonEl` 的 `parentElement`。 然後使用 `splice()` 從 `taskData` 數組中刪除該任務。 將 `dataArrIndex` 和 `1` 作爲 `splice()` 的參數傳入。

`dataArrIndex` 是開始的索引，`1` 是要刪除的項目數。

# --hints--

你不應更改 `dataArrIndex` 語句。
