...
在正則表達式樣式中，*lookaround* 是一種斷言，用來匹配某個樣式但不消耗字串中的字元。一種 lookaround 是 lookbehind，可以是正向或負向。它們分別以 `(?<=...)` 和 `(?<!...)` 表示。
...
在上述範例中，樣式 `(?<=l)a` 包含正向回顧，用來匹配只有在前面有 `l` 時的 `a` 字元。在範例的最後一行，樣式 `(?<!l)a` 包含負向回顧，用來匹配只有在前面**沒有** `l` 時的 `a` 字元。請注意，在這兩種情況中，回顧中包含的字元都不會被消耗。

修改你的正則表達式樣式以使用負向回顧，讓字元 `1` 僅在前面沒有數字時才被取代。

# --hints--

你應該修改作為第一個引數傳遞給 `re.sub()` 呼叫的樣式，使用負向前瞻。
