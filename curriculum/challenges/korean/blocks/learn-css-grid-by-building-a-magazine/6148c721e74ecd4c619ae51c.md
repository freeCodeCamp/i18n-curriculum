...
`.text` 요소는 CSS Grid가 아니지만, Grid를 사용하지 않고도 `column-width` 속성을 사용해 요소 내에 열을 만들 수 있습니다.

`.text` 선택자에 `column-width` 속성을 `25rem`로 설정하세요.

# --hints--

`.text` 선택자에 `column-width` 속성을 `25rem`로 설정해야 합니다.
