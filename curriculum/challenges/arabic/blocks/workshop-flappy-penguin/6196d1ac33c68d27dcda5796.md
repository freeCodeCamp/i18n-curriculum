...
دوّر العنصر `.back-mountain` بزاوية `45deg` في اتجاه عقارب الساعة. ثم، أعطه خاصية `left` بقيمة `110px`، وخاصية `top` بقيمة `225px`.

# --hints--

يجب استخدام خاصية `transform` لتدوير العنصر.
...
يجب أن تعطي `.back-mountain` خاصية `transform` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
...
يجب أن تعطي `.back-mountain` خاصية `left`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.left);
```

يجب أن تعطي `.back-mountain` خاصية `left` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.left, '110px');
```

يجب أن تعطي `.back-mountain` خاصية `top`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.top);
```

يجب أن تعطي `.back-mountain` خاصية `top` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
