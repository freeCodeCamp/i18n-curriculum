...
Тепер, коли створено сеттер `salary`, оновіть `__init__`, щоб викликати його замість прямого призначення `self._salary`. Змініть `self._salary = Employee._base_salaries[level]` на `self.salary = Employee._base_salaries[level]`.

# --hints--

Ваш метод `__init__` має використовувати `self.salary = Employee._base_salaries[level]` замість `self._salary = Employee._base_salaries[level]`.
