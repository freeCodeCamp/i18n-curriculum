...
Hoja ya kwanza kwa wito wako wa `evalFormula` inapaswa kuwa maudhui ya seli (ambayo ulihifadhi katika `value`). Hata hivyo, maudhui huanza na herufi ya `=` ili kuanzisha kitendakazi, kwa hivyo unahitaji kupitisha sehemu ndogo ya mfuatano wa herufi wa `value` kuanzia kielezo cha `1`.

# --hints--

Unapaswa kupitisha `value` kama hoja ya kwanza kwa wito wako wa `evalFormula()`.
...
Unapaswa kuita njia ya `.slice()` kwenye hoja ya `value`.
...
Unapaswa kupitisha nambari `1` kama hoja kwa wito wako wa `.slice()`.
