...
Declare uma variável `functionCall` e atribua a ela esta expressão regular: `/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

Essa expressão examinará chamadas de função, como `sum(1, 4)`.

# --hints--

Você deve declarar uma variável `functionCall`.
...
Você deve usar `const` para declarar a variável `functionCall`.
...
Você deve atribuir à função `functionCall` a expressão regular fornecida.
