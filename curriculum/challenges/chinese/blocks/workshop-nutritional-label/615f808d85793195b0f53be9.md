...
在最后一个 `.divider` 之后创建另一个 `p` 元素，并为其指定文本 `Total Sugars 12g`。 为该 `p` 元素分配缩进和移除底部边框所需的 `class` 值。 然后在新的 `p` 元素下方创建另一个 `.divider`。

# --hints--

你应该在 `.daily-value` 元素末尾创建一个新的 `p` 和 `div` 元素。
