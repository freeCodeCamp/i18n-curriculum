...
Якщо стрілкова функція повертає простий вираз, ви можете пропустити ключове слово `return` та фігурні дужки `{}`. Це називається implicit return(неявнє повернення).

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

Якщо стрілкова функція містить кілька рядків коду в тілі функції, потрібно використовувати ключове слово `return` та фігурні дужки `{}`.
...
Перетворіть або оновіть функцію `addTwoNumbers`, щоб видалити ключове слово `return` та фігурні дужки `{}`. Натомість функція `addTwoNumbers` повинна використовувати неявне повернення.

Відкрийте консоль, щоб переконатися, що ви все ще отримуєте правильний вивід.

# --hints--

Перероблена функція `addTwoNumbers` має використовувати неявне повернення.
