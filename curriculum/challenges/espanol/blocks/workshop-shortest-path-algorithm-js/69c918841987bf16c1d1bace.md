...
Ahora que sabes que el nodo es accesible y no es el inicio, formatea su ruta en una cadena legible.

El arreglo `paths[nodeNo]` contiene la secuencia de índices de nodos visitados para llegar a `nodeNo`. Puedes convertir eso en una cadena legible para humanos como `0 -> 2 -> 3` usando el método `.join()`.

Dentro del ciclo `for...of`, después de la instrucción `if` con `continue`, declara una variable llamada `path` y asígnale el valor `paths[nodeNo].join(' -> ')`.

# --hints--

Deberías tener una variable llamada `path` dentro del ciclo `for...of`.
...
Tu variable `path` debería tener el valor `paths[nodeNo].join(' -> ')`.
