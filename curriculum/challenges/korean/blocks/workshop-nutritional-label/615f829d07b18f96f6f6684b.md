...
이 구분선을 만드는 장점은 각각에 특정 클래스를 적용해 개별적으로 스타일을 지정할 수 있다는 점입니다. 마지막 `double-indent`에 `class`를 `.divider`에 추가하세요.

# --hints--

마지막 `.divider` 요소에 `double-indent`를 `class`에 추가해야 합니다. 기존 값은 제거하지 마세요.
