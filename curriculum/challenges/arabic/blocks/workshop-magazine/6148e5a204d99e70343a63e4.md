...
خاصية `gap` هي طريقة مختصرة لتعيين قيمة `column-gap` و`row-gap` في نفس الوقت. إذا أعطيت قيمة واحدة، فإنها تعيّن كلًا من `column-gap` و`row-gap` إلى تلك القيمة. وإذا أعطيت قيمتين، فإنها تعيّن `row-gap` إلى القيمة الأولى و`column-gap` إلى القيمة الثانية.

أعطِ المحدِّد `.image-wrapper` خاصية `gap` بقيمة `2rem`.

# --hints--

يجب أن يحتوي عنصر `.image-wrapper` على خاصية `gap` بقيمة `2rem`.
