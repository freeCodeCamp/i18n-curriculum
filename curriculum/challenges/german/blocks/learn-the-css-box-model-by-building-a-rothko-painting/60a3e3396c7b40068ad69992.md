...
Die Ecken jedes Rechtecks sind noch zu scharf.

Runden Sie jede Ecke des `.one`-Elements mit 9 Pixeln ab, indem Sie die `border-radius`-Eigenschaft verwenden.

# --hints--

Sie sollten die `border-radius`-Eigenschaft auf `9px` setzen.
...
Ihr `.one`-Element sollte einen `border-radius`-Wert von `9px` haben.
