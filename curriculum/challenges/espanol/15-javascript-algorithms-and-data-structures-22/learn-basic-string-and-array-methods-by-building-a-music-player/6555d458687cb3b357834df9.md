...
Acceda a las propiedades `userData?.currentSong?.title` y `userData?.currentSong?.artist` y asígnelas a las variables `currentTitle` y `currentArtist` respectivamente.

# --hints--

Debes acceder al `title` del `currentSong` del objeto `userData`. No olvides utilizar el encadenamiento opcional.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

Debes asignar el `title` del `currentSong` a una constante `currentTitle`.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

Debes acceder al `artist` del `currentSong` del objeto `userData`. No olvides utilizar el encadenamiento opcional.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

Debes asignar el `artist` del `currentSong` a una constante `currentArtist`.
