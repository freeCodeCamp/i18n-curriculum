...
Usando un selector de clase, da a tu elemento `.cat-right-eye` un ancho de `30px` y una altura de `40px`. Además, dale un color de fondo de `#000`.

# --hints--

Debes tener un selector `.cat-right-eye`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-right-eye'))
```

Tu selector `.cat-right-eye` debe tener un ancho establecido en `30px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.width, '30px')
```

Tu selector `.cat-right-eye` debe tener una altura establecida en `40px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.height, '40px')
```

Tu selector `.cat-right-eye` debe tener un `background-color` establecido en `#000`.
