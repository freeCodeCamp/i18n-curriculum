...
Después de tu elemento `h2`, crea dos elementos `p`. Dale al primero una `clase` de `dessert-price`, y establece el texto a un signo de dólar `"$"` seguido por el valor de la variable `price`. Dale al segundo una `clase` de `product-category`, y el texto `"Categoría: "` seguido por el valor de la variable `category`.

# --hints--

Debe crear dos elementos `p`.
...
Tu primer elemento `p` debe tener una `clase` de `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Tu primer elemento `p` debe tener el signo de dólar `"$"` seguido por el valor de la variable `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Tu segundo elemento `p` debe tener una `clase` de `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Tu segundo elemento `p` debe tener el texto `"Categoría: "` seguido por el valor de la variable `category`.
