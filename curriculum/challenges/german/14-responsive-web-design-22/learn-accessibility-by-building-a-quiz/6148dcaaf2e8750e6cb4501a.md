...
Lass die Absatzelemente mit den folgenden CSS-Eigenschaften als höhere Priorität erscheinen:
...
Du solltest den `p`-Element-Selektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('p'));
```

Du solltest dem `p`-Element ein `margin-top` von `5px` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('p')?.marginTop, '5px');
```

Du solltest dem `p` Element ein `padding-left` von `15px` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('p')?.paddingLeft, '15px');
```

Du solltest dem `p`-Element eine `font-size` von `20px` geben.
