...
Додайте вашим `tr.total` елементам ефект наведення, який змінює фон на `#99c9ff`.

# --hints--

У вас має бути селектор `tr.total:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total:hover'));
```

Ваш селектор `tr.total:hover` має мати властивість `background-color`, встановлену в `#99c9ff`.
