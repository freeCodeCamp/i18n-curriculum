...
創建一個名為 `linkClass` 的常數並指定一個模板字串。在該模板字串內，為你的 `category ${selectedCategory.className}` 添加值。 

這些類別名稱將用於為錨點元素套用樣式。

# --hints--

你應該有一個名為 `linkClass` 的常數。

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

你應該將模板字串指定給你的 `linkClass` 變數。

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

你應該在模板字串中擁有 `category ${selectedCategory.className}` 的值。
