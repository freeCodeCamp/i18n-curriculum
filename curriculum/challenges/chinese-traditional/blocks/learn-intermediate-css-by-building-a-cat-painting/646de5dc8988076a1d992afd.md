...
為了讓左眼看起來像眼睛，給它一個 `60%` 的邊框半徑。另外，使用 `transform` 屬性，將它旋轉 `25` 度。

# --hints--

你的 `.cat-left-eye` 選擇器應該有一個 `border-radius` 屬性設定為 `60%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.borderRadius === '60%')
```

你的 `.cat-left-eye` 選擇器應該有一個設定為 `rotate(25deg)` 的 `transform` 屬性。
