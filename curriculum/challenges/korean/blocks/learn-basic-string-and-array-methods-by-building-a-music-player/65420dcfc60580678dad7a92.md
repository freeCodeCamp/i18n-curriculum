...
`map()` 안에 노래 세부 정보를 표시하는 모든 요소를 보간할 백틱이 있는 `return` 문장을 추가하세요.

백틱 안에 `li`인 `id` 속성과 `song-${song.id}`인 `class` 속성을 가진 `playlist-song` 요소를 만드세요.

# --hints--

빈 백틱 쌍이 있는 `return` 문장을 추가하세요.
...
열리는 `li` 태그에 `id`로 설정된 `song-${song.id}` 속성이 있어야 합니다.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

열리는 `li` 태그에 `class`로 설정된 `playlist-song` 속성이 있어야 합니다.
