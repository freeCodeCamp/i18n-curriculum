...
`displayStory` 関数の中で、`storyObj` オブジェクトに `genre` パラメータと一致するプロパティがあるか確認し、選択されたジャンルのストーリーを `#result` 段落のテキストコンテンツに設定してください。

また、`storyContainer` のボーダー色を選択されたジャンルの `borderColor` プロパティに設定してください。

`displayStory` 関数を `scary` ジャンルで呼び出してテストしてください。ボーダー色は `#ee4b2b` で、ストーリーは `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` であるはずです。

# --hints--

`displayStory` 関数を `"scary"` ジャンルで呼び出す必要があります。

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

`displayStory("scary")` を呼び出すと、ボーダー色は `#ee4b2b` になるはずです。
...
`displayStory("scary")` を呼び出すと、表示されるストーリーは `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` になるはずです。
