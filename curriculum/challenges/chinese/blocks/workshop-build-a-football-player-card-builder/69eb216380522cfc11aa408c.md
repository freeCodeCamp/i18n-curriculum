...
`PAC` 代表速度，`SHO` 表现射门，`PAS` 表现传球。

数字放在 `stat-value` 范围内，属性放在 `stat-label` 范围内。

# --hints--

第一个 `.stat-row` 应该有一个 `.stat-value` 为 `97`，以及一个 `.stat-label` 为 `PAC`。
...
第二个 `.stat-row` 应该有一个 `.stat-value` 为 `98`，以及一个 `.stat-label` 为 `SHO`。
...
第三个 `.stat-row` 应该有一个 `.stat-value` 为 `83`，以及一个 `.stat-label` 为 `PAS`。
