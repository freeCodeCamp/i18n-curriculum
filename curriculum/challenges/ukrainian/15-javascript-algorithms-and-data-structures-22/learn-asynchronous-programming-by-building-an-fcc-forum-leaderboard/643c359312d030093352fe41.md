...
Тепер настав час показати дані на сторінці.

Почніть зі створення стрілкової функції під назвою `showLatestPosts`, яка приймає параметр `data`.

# --hints--

Ви повинні мати стрілкову функцію під назвою `showLatestPosts`.

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

Функція `showLatestPosts` повинна мати параметр `data`.

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

Необхідно визначити `showLatestPosts`.
