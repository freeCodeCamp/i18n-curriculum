...
Спроєктуйте функцію ящика касового апарата `checkCashRegister()`, яка приймає ціну покупки як перший аргумент (`price`), оплату як другий аргумент (`cash`) і готівку в ящику (`cid`) як третій аргумент.

`cid` — це двовимірний масив, що містить доступну валюту.

Функція `checkCashRegister()` завжди має повертати об’єкт із ключем `status` та ключем `change`.

Поверніть `{status: "INSUFFICIENT_FUNDS", change: []}`, якщо готівки в ящику менше, ніж здачі, або якщо неможливо видати точну здачу.

Поверніть `{status: "CLOSED", change: [...]}` із готівкою в ящику як значенням для ключа `change`, якщо вона дорівнює здачі.

Інакше поверніть `{status: "OPEN", change: [...]}`, із здачею у монетах і банкнотах, відсортованою від найбільшої до найменшої, як значення ключа `change`.

<table><tbody><tr><th>Одиниця валюти</th><th>Сума</th></tr><tr><td>Пенні</td><td>$0.01 (PENNY)</td></tr><tr><td>Нікель</td><td>$0.05 (NICKEL)</td></tr><tr><td>Дайм</td><td>$0.1 (DIME)</td></tr><tr><td>Квартал</td><td>$0.25 (QUARTER)</td></tr><tr><td>Долар</td><td>$1 (ONE)</td></tr><tr><td>П’ять доларів</td><td>$5 (FIVE)</td></tr><tr><td>Десять доларів</td><td>$10 (TEN)</td></tr><tr><td>Двадцять доларів</td><td>$20 (TWENTY)</td></tr><tr><td>Сто доларів</td><td>$100 (ONE HUNDRED)</td></tr></tbody></table>

Нижче наведено приклад масиву готівки в ящику:
