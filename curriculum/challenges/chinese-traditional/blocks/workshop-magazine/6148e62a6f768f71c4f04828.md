...
`place-items` 屬性可用於同時設置 `align-items` 和 `justify-items` 的值。 `place-items` 屬性採用一個或兩個值。 如果提供了一個值，則它同時用於 `align-items` 和 `justify-items` 屬性。 如果提供了兩個值，則第一個值用於 `align-items` 屬性，第二個值用於 `justify-items` 屬性。

給 `.image-wrapper` 選擇器一個 `place-items` 屬性，設置爲 `center`。

# --hints--

你的 `.image-wrapper` 選擇器應該有一個 `place-items` 屬性，設置爲 `center`。
