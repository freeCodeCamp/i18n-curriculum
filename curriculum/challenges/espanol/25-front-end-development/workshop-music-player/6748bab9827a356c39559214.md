...
Utiliza el método `classList` y `remove()` para quitar la clase `playing` del `playButton`, ya que la canción se pausará en este punto.

Para finalmente pausar la canción, usa el método `pause()` en la variable `audio`. `pause()` es un método del Web Audio API para pausar archivos de música.
