...
2 つ目の `fieldset` については、`input` 要素と `label` のテキストを同じラインに表示したいですね。

まず、2 つ目の `fieldset` 内の `input` 要素に `inline` というクラスを設定しましょう。

# --hints--

1 つ目の `input` にクラス `inline` を設定してください。

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

2 つ目の `input` にクラス `inline` を設定してください。
