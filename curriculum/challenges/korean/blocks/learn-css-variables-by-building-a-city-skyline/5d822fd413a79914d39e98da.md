---
id: 5d822fd413a79914d39e98da
title: 18단계
challengeType: 0
dashedName: step-18
---

# --description--

첫 번째 건물이 이제 꽤 좋아 보입니다. `div` 컨테이너 안에 세 개의 새로운 `.background-buildings` 요소를 중첩하고, 차례대로 `bb2`, `bb3`, `bb4` 클래스를 부여하세요. 이들은 배경에 추가할 세 개의 건물이 됩니다.

# --hints--

`div` 클래스를 가진 `bb2`를 만들어야 합니다.

```js
assert.exists(document.querySelector('div.bb2'));
```

`div` 클래스를 가진 `bb3`를 만들어야 합니다.

```js
assert.exists(document.querySelector('div.bb3'));
```

`div` 클래스를 가진 `bb4`를 만들어야 합니다.

```js
assert.exists(document.querySelector('div.bb4'));
```

새로운 `div` 요소 3개를 만들어야 합니다.

```js
assert.equal(document.querySelectorAll('div')?.length, 9);
```

이 `div` 요소들을 `.background-buildings` 요소 안에 배치해야 합니다.

```js
assert.equal(document.querySelector('div.background-buildings')?.children?.length, 4);
```

요소들을 올바른 순서로 배치해야 합니다.

```js
function __t(a, b) {
  return [...document.querySelector(a)?.nextElementSibling?.classList]?.includes(b);
}
assert(__t('div.bb1','bb2') && __t('div.bb2','bb3') && __t('div.bb3','bb4'));
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">    
  <head>
    <meta charset="UTF-8">
    <title>City Skyline</title>
    <link href="styles.css" rel="stylesheet" />   
  </head>

  <body>
    <div class="background-buildings">
      <div class="bb1">
        <div class="bb1a"></div>
        <div class="bb1b"></div>
        <div class="bb1c"></div>
        <div class="bb1d"></div>
      </div>
--fcc-editable-region--

--fcc-editable-region--
    </div>
  </body>
</html>
```

```css
* {
  border: 1px solid black;
  box-sizing: border-box;
}

body {
  height: 100vh;
  margin: 0;
  overflow: hidden;
}

.background-buildings {
  width: 100%;
  height: 100%;
}

.bb1 {
  width: 10%;
  height: 70%;
  display: flex;
  flex-direction: column;
  align-items: center;
  --building-color1: #aa80ff;
}

.bb1a {
  width: 70%;
  height: 10%;
  background-color: var(--building-color1);
}

.bb1b {
  width: 80%;
  height: 10%;
  background-color: var(--building-color1);
}

.bb1c {
  width: 90%;
  height: 10%;
  background-color: var(--building-color1);
}

.bb1d {
  width: 100%;
  height: 70%;
  background-color: var(--building-color1);
}
    
```
