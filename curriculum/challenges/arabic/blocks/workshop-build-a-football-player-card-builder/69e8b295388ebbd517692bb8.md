...
أسفل عنصر `div` الذي يحمل `className` بقيمة `card-tier-badge`، أضف عنصر `div` يحمل `className` بقيمة `card-club` والنص `Santos FC`.

# --hints--

يجب إضافة عنصر `div` يحمل `className` بقيمة `card-club` أسفل `.card-tier-badge`.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

يجب أن يحتوي عنصر `.card-club` على النص `Santos FC`.
