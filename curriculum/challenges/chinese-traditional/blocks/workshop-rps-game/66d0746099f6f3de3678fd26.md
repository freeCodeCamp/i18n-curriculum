...
現在是測試你的 `showResults` 函式（程式）的時候了。 

首先建立一個名為 `rockBtn` 的變數，並將 id 為 `rock-btn` 的按鈕的值指定給它。

然後，創建一個名為 `paperBtn` 的變數，並將 id 為 `paper-btn` 的按鈕的值指定給它。

最後，創建一個名為 `scissorsBtn` 的變數，並將 id 為 `scissors-btn` 的按鈕指定給它。

# --hints--

你應該有一個名為 `rockBtn` 的變數。

```js
assert.isNotNull(rockBtn);
```

你的 `rockBtn` 變數應該指定到 id 為 `rock-btn` 的按鈕。

```js
assert.equal(rockBtn, document.getElementById("rock-btn"));
```

你應該有一個名為 `paperBtn` 的變數。

```js
assert.isNotNull(paperBtn);
```

你的 `paperBtn` 變數應該指定到 id 為 `paper-btn` 的按鈕。

```js
assert.equal(paperBtn, document.getElementById("paper-btn"));
```

你應該有一個名為 `scissorsBtn` 的變數。

```js
assert.isNotNull(scissorsBtn);
```

你的 `scissorsBtn` 變數應該指定到 id 為 `scissors-btn` 的按鈕。
