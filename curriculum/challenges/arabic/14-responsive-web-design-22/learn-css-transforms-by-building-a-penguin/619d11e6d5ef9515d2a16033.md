...
يجب عليك استخدام منتقي `.beak.top`.

```js
assert.match(code, /\.beak\.top\s*\{/);
```

يجب أن يحتوي منتقي `.beak.top` على خاصية `width` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.width, '20%');
```

يجب أن يحتوي منقي `.beak.top` على خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.top, '60%');
```

يجب أن يحتوي منتقي `.beak.top` على خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
