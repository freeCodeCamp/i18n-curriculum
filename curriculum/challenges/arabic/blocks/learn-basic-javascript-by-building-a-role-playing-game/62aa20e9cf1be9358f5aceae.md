...
أضف عبارة `else` إلى أول عبارة `if` داخل دالتك `attack()`. في عبارة `else`، استخدم العامل `+=` لإضافة النص `" You miss."` إلى نهاية `text.innerText`.

# --hints--

يجب أن تضيف كتلة `else` بعد كتلة `if (isMonsterHit())`.
...
يجب أن تضيف النص `" You miss."` إلى نهاية `text.innerText`. تذكر استخدام التعيين المركب وتأكد من وجود مسافة قبل كلمة `You`.
