...
为你的 `if...else` 语句添加一个 `elif` 语句并查看 `seat_type` 是否相等于 `Gold`。在 `elif` 语句的体内，将 `service_charges` 的值更新为 `3`。

在 `if...elif..else` 语句下方，使用 `print()` 调用打印一条消息，显示 `Service charges:`，后跟更新后的 `service_charges` 值。然后，在终端查看输出。
