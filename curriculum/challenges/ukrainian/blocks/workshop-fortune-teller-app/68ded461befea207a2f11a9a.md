...
Наразі у вас є три функції: `getElement`, `hideElements` та `showElements`.
Тепер вам потрібна функція, яка допоможе отримати випадкову вдачу.

Оголосіть функцію з іменем `getRandomItem`, яка приймає масив `items` як аргумент. Використайте дженерики для цієї функції.

Всередині функції використовуйте `Math.random()` та `Math.floor()`, щоб вибрати випадкову вдачу з масиву `items`.
Не забудьте повернути вибрану вдачу.

# --hints--

Ви повинні оголосити функцію з іменем `getRandomItem`.
...
Ваша функція `getRandomItem` має використовувати параметр типу дженерика `T`.
...
Ваша функція `getRandomItem` має мати параметр з іменем `items` типу `T[]`.
...
Функція `getRandomItem` має мати тип повернення `T`.
...
Ви повинні оголосити змінну з іменем `index` всередині функції `getRandomItem`.
...
Змінна `index` має використовувати `Math.floor()` та `Math.random()` з `items.length`, щоб отримати випадковий індекс.
...
Ваша функція `getRandomItem` має повертати `items[index]`.
