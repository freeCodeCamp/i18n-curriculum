...
title: Crea un pulsante bootstrap come elemento di blocco
challengeType: 0
forumTopicId: 16810
...
Questo pulsante sarebbe largo solo quanto la parola `Submit`.

<button class='btn btn-default'>Submit</button>

Facendoli diventare elementi di blocco con l'aggiunta della classe `btn-block`, il tuo pulsante si allargherà per occupare tutto lo spazio orizzontale disponibile nella pagina e gli elementi che seguono fluiranno su una "nuova riga" sotto il blocco.

```html
<button class="btn btn-default btn-block">Submit</button>
```

Questo pulsante occuperebbe il 100% della larghezza disponibile.

<button class='btn btn-default btn-block'>Submit</button>

Nota che questi pulsanti devono comunque avere la classe `btn`.

Aggiungi la classe `btn-block` di Bootstrap al tuo pulsante Bootstrap.
...
Tutti i tuoi elementi `button` dovrebbero avere i tag di chiusura.
