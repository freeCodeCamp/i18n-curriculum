...
Ви хочете порівняти два вектори, лише коли вони належать до одного класу. Для цього створіть інструкцію `if`, яка перевіряє, чи `self` і `other` не належать до одного класу, і в такому випадку повертає `NotImplemented`.
# --hints--

Ви повинні створити інструкцію `if`, яка перевіряє, чи `self` і `other` не належать до одного класу.
...
Ви повинні повернути `NotImplemented` з тіла вашої `if`.
