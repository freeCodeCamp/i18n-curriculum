...
Damit auf die Daten eines Formulars in der `action`-Attribut, angegebenen Position zugegriffen werden kann, musst du dem Textfeld ein `name`-Attribut zuweisen und ihm einen Wert verleihen, um die übermittelten Daten zu repräsentieren.
...
Dein `input`-Element sollte ein `name`-Attribut mit dem Wert `catphotourl` enthalten.  Du hast entweder den Wert weggelassen oder einen Tippfehler gemacht. Denke daran, dass Attributwerte zwischen Anführungszeichen geschrieben werden sollten.
