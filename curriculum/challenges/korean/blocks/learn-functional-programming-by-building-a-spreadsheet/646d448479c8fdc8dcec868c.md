...
`functionCall` 변수를 선언하고, 이 정규 표현식을 `/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i` 할당하세요.

이 표현식은 `sum(1, 4)` 같은 함수 호출을 찾습니다.

# --hints--

`functionCall` 변수를 선언해야 합니다.
...
`const`를 사용해 `functionCall` 변수를 선언하세요.
...
제공된 정규 표현식을 `functionCall`에 할당해야 합니다.
