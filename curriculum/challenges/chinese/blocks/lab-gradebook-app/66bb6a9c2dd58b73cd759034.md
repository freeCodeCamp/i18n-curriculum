...
1. 你应该有一个名为 `getGrade` 的函数，它接受一个学生分数作为参数，并返回一个表现字母等级的字串，基于该分数。以下是分数及其对应的字母等级：
...
1. 你应该有一个名为 `studentMsg` 的函数，它接受一个分数数组和一个学生分数作为参数。该函数应返回具有以下格式的字符串：
    ...
    将 `average-goes-here` 替换为总分数的平均值，将 `grade-goes-here` 替换为学生的等级。
    使用 `getAverage` 获取平均分数，使用 `getGrade` 获取学生的等级。
