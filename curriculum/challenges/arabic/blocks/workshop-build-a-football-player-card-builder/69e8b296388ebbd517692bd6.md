...
النموذج الآن مكتمل. لجعل بطاقة المعاينة تتحدث أثناء الكتابة، تحتاج إلى ربط المُدخَلات بحالة `player`.

ابدأ بمُدخَل الاسم `input`، أضف إليه خاصية `value` بقيمة `player.name`. ثم أضف خاصية `onChange` مع دالة سهمية تأخذ `e` كمعلمة وتستدعي `setPlayer` مع نسخ من `player` وتعيين `name` إلى `e.target.value`. لا تنسَ وضع النسخ وتعيين قيمة `name` داخل كائن.

# --hints--

يجب ربط قيمة `value` لمُدخَل الاسم بحقل `name` في حالة اللاعب.
...
يجب أن يحتوي مُدخَل الاسم على معالِج `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

يجب أن يقوم معالِج `onChange` باستدعاء `setPlayer` بحيث يتضمن استدعاء `setPlayer` كائنًا ينشر `player` ويضبط `name` إلى `e.target.value`.
