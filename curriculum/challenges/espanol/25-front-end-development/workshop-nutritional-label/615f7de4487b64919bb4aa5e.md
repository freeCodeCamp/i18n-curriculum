...
Después de su último `.divider`, cree un nuevo elemento `p` con el texto `Cholesterol 0mg 0%`. Luego, envuelva el texto `Colesterol` en un elemento `span`, `0%` en otro, y asigne a cada uno de ellos una clase de `bold`.

Finalmente, anide el elemento `span` que contiene el texto `Colesterol` junto con el texto `0mg`, en un elemento `span` adicional para alineación.
...
Su nuevo elemento `p` debe tener el texto `Colesterol 0mg 0%`.
...
Su nuevo elemento `p` debe tener exactamente tres elementos `span`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelectorAll('span')?.length === 3);
```

El texto `Colesterol` debe estar anidado en un `span`.
...
El elemento `span` que contiene el texto `Colesterol` debe tener su atributo `class` configurado como `bold`.
...
El texto `0%` debe estar anidado en un `span`.
...
El elemento `span` que contiene el texto `0%` debe tener su atributo `class` configurado como `bold`.
...
El elemento `span` que contiene el texto `Colesterol` y el texto `0mg` deben estar envueltos juntos en otro elemento `span`.
