...
El ciclo está completo. Devuelve `grouped` desde la función.

Luego, debajo de la función, llama a `groupByDecade(catalog)` y asigna el resultado a una variable llamada `byDecade`. Registra `byDecade` para inspeccionar el resultado agrupado, y deberías ver un objeto con claves como `"1950s"`, `"1960s"`, `"1970s"`, y así sucesivamente, cada una apuntando a un arreglo de libros de esa década.

# --hints--

`groupByDecade` debería devolver un objeto.

```js
assert.isObject(groupByDecade(catalog));
```

`byDecade["1980s"].length` debería ser `5`.

```js
assert.strictEqual(groupByDecade(catalog)["1980s"].length, 5);
```

`byDecade["Unknown"].length` debería ser `1`.

```js
assert.strictEqual(groupByDecade(catalog)["Unknown"].length, 1);
```

Deberías asignar el resultado de `groupByDecade(catalog)` a una variable llamada `byDecade`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+byDecade\s*=\s*groupByDecade\s*\(\s*catalog\s*\)/);
```

Deberías registrar `byDecade` en la consola.
