...
Crea e aggiungi le seguenti proprietà a `.bb2a`:
...
Dopo averle aggiunte, puoi vedere come un bordo spesso su un elemento crea degli angoli dove si incontrano due lati. Userai quel bordo inferiore come la parte superiore dell'edificio.

# --hints--

Dovresti dare a `.bb2a` una `margin` di `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.margin, "auto");
```

Dovresti dare a `.bb2a` una `width` di `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.width, "5vw");
```

Dovresti dare a `.bb2a` una `height` di `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.height, "5vw");
```

Dovresti dare a `.bb2a` una `border-top` di `1vw solid #000`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderTop, "1vw solid rgb(0, 0, 0)");
```

Dovresti dare a `.bb2a` una `border-bottom` di `1vw solid #000`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderBottom, "1vw solid rgb(0, 0, 0)");
```

Dovresti dare a `.bb2a` una `border-left` di `1vw solid #999`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderLeft, "1vw solid rgb(153, 153, 153)");
```

Dovresti dare a `.bb2a` una `border-right` di `1vw solid #999`.
