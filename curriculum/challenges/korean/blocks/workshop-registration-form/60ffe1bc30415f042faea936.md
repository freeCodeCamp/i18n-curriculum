...
`input`와 `textarea` 요소가 배경 테마와 어우러지도록, `background-color`를 `#0a0a23`로 설정하세요. 그런 다음, `1px`, `solid` 테두리를 `#0a0a23` 색상으로 지정하세요.

# --hints--

쉼표로 구분된 요소 선택자를 사용해 `input`와 `textarea` 요소를 선택하세요.
...
`input`와 `textarea` 요소에 `background-color`를 `#0a0a23`로 지정하세요.
...
`input`와 `textarea` 요소에 `1px`, `solid` 테두리를 `#0a0a23` 색상으로 지정하세요.
