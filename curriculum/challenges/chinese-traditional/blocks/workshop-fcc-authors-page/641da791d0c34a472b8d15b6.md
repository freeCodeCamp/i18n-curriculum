...
現在 `authorDataArr` 與你剛才在主控台記錄的 `data` 相同。將 `authorDataArr` 記錄到主控台以確認這一點。

在你的 `console.log()` 述語中，為第一個引數添加文字 `"Author Data Array:"`，為第二個引數添加 `authorDataArr`。使用逗號將文字與 `authorDataArr` 分開。
