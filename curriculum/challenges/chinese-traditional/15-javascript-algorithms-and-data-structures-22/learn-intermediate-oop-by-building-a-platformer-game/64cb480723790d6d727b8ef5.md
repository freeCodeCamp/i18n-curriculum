...
在 `else if` 語句中，添加 `forEach` 循環來遍歷 `checkpoints` 數組。 使用 `checkpoint` 作爲回調函數的參數名。

在 `forEach` 回調中，使用加法賦值運算符在 checkpoints 數組的 `x` 位置加 5。
...
你應該在 `forEach` 回調函數中使用 `checkpoint` 參數。
...
你應該使用加法賦值運算符給 `checkpoint` 的 `x` 位置添加 5。
