...
Щоб відтворювати пісню щоразу, коли користувач натискає її, додайте атрибут `onclick` до першого елемента кнопки. В межах `onclick` викличте функцію `playSong` з `song.id`.

Не забудьте, що тут потрібно інтерполювати зі знаком долара.

# --hints--

Додайте атрибут `onclick` зі значенням `playSong(${song.id})` до елемента `button`.
