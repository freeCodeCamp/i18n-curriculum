...
`.map()` 関数がカリー化された `num => charRange(...).map(...)` 呼び出しから返された `addCharacters` 関数の参照を受け取るようになったので、要素を正しく繰り返し処理し、それぞれの要素を `n` としてその関数に渡します。

`match` パラメータを使っていないことに気づくでしょう。JavaScript では、使わないパラメータに慣例として接頭辞にアンダースコア `_` を付けることがよくあります。パラメータを空にしてもよいですが `(, char1)`、将来の可読性のためにパラメータ名を付けておくほうがわかりやすい場合が多いです。

`match` パラメータにアンダースコアを接頭辞として付けてください。

# --hints--

`match` パラメータにアンダースコアを接頭辞として付けるべきです。
