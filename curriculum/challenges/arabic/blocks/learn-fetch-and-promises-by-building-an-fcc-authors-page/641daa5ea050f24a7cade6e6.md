...
إذا تحقق هذا الشرط، عطّل الزر من خلال ضبط الخاصية `disabled` إلى `true`. أيضًا، عيّن الخاصية `textContent` للزر إلى `"No more data to load"`.
...
يجب تعيين خاصية `disabled` للزر `loadMoreBtn` إلى `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

يجب تعيين الخاصية `textContent` للزر `loadMoreBtn` إلى القيمة `"No more data to load"`.
