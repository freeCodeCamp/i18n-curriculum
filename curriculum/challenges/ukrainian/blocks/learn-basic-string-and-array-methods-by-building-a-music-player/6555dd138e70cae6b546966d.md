...
Вам потрібно отримати пісню, яка зараз відтворюється, або першу пісню у плейлисті. Для цього створіть константу `song` і використайте оператор OR (`||`), щоб призначити їй поточну пісню `userData`, **або** першу пісню в масиві `userData?.songs`.

Не забудьте використати опціональний ланцюжок.

# --hints--

Вам не слід змінювати існуючий `setPlayButtonAccessibleText` та його вміст.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Вам слід отримати доступ до `userData?.currentSong` або `userData?.songs[0]`.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

Вам слід призначити `userData?.currentSong || userData?.songs[0]` константі `song`.
