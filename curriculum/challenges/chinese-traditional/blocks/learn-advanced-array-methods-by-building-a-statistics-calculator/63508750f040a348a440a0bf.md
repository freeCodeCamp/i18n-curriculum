...
要顯示 `mean` 的值，你的應用程式已準備好一個 `#mean` 元素。

使用 `.querySelector` 找到該元素，然後將其 `.textContent` 設定為 `mean` 的值。

# --hints--

你的 `calculate` 函式應該使用 `document.querySelector()`。

```js
assert.lengthOf(calculate.toString().match(/document\.querySelector\(/g), 2);
```

你的 `.querySelector()` 應該以 `#mean` 元素為標的。

```js
assert.match(calculate.toString(), /document\.querySelector\(\s*('|")#mean\1\s*\)/);
```

你的 `calculate` 函式應該將 `#mean` 元素的 `.textContent` 設定為 `mean` 的值。
