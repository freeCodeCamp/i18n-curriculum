...
そのブール値の式の下に、プレイヤーの`x`位置がプラットフォームの`x`位置とプラットフォームの幅の合計からプレイヤーの幅の3分の1を引いた値以下であるかをチェックする別のブール値の式を追加してください。プレイヤーの幅の3分の1を計算するには除算演算子（`/`）を使ってください。

# --hints--

プレイヤーの`x`位置がプラットフォームの`x`位置とプラットフォームの幅の合計からプレイヤーの幅を`3`で割った値以下であるかをチェックするブール値の式があるはずです。
