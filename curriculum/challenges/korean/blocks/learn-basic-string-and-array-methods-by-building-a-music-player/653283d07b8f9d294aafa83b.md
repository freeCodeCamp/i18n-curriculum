...
표시된 노래를 재생하는 기능을 구현할 시간입니다.

`playSong`을 사용해 `const` 함수를 정의하세요. 이 함수는 재생하려는 노래의 고유 식별자를 나타내는 `id` 매개변수를 받아야 합니다.

# --hints--

빈 함수를 만들려면 `const`을 사용해 `playSong`라는 이름으로 만드세요.
