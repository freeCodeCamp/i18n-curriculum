...
In questo momento il `songsHTML` è un array. Se provassi a mostrarlo così com’è, vedresti le canzoni separate da virgole. Questo non è il risultato desiderato perché vuoi mostrare le canzoni come una lista. Per risolvere questo, dovrai unire l’array in un’unica stringa usando il metodo <dfn>join()</dfn>.

Il metodo `join()` serve a concatenare tutti gli elementi di un array in un’unica stringa. Prende un parametro opzionale chiamato `separator` che serve a separare ogni elemento dell’array. Per esempio:
...
Collega il metodo `join()` al tuo metodo `map()` e passa una stringa vuota come separatore.

Per concatenare più metodi insieme, puoi chiamare il metodo `join()` sul risultato del metodo `map()`. Per esempio:
...
Dovresti aggiungere `join("")` al codice esistente.
