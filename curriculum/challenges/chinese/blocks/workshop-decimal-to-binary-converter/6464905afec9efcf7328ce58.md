...
接下来，使用 `.forEach()` 方法循环遍历 `animationData` 数组。 对于 `.forEach()` 方法的回调函数，传入 `obj` 作为参数，但回调函数体暂时留空。

# --hints--

你应该使用 `.forEach()` 方法循环遍历 `animationData` 数组。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\(/);
```

你应该将 `obj` 作为参数传递给 `.forEach()` 方法的回调函数。
