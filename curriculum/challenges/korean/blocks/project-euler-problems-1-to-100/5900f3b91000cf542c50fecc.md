...
소수의 합으로 10을 정확히 다섯 가지 방법으로 나타낼 수 있습니다:
...
소수의 합으로 `n`가지 이상의 방법으로 나타낼 수 있는 가장 작은 값은 무엇인가요?

# --hints--

`primeSummations(5)`은 숫자를 반환해야 합니다.

```js
assert(typeof primeSummations(5) === 'number');
```

`primeSummations(5)`는 `11`을 반환해야 합니다.

```js
assert.strictEqual(primeSummations(5), 11);
```

`primeSummations(100)`는 `31`를 반환해야 합니다.

```js
assert.strictEqual(primeSummations(100), 31);
```

`primeSummations(1000)`는 `53`을 반환해야 합니다.

```js
assert.strictEqual(primeSummations(1000), 53);
```

`primeSummations(5000)`는 `71`를 반환해야 합니다.
