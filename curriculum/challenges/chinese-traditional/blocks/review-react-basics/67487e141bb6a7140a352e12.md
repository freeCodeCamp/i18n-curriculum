...
- 匯出讓組件可以被其他檔案使用。匯入則允許檔案使用其他地方匯出的組件。
- React 中有兩種匯出類型：預設匯出和具名匯出。

**預設匯出**

- 一個檔案只能有一個預設匯出。這適用於主要包含單一組件的檔案。
- 你可以這樣將組件匯出為預設匯出：
...
- 具名匯出允許一個檔案分享多個組件。與預設匯出不同，匯入時必須使用與匯出時相同的名稱。
- 你可以這樣從同一檔案匯出多個組件：
...
- 要匯入具名匯出，使用大括號並寫出正確的匯出名稱：

```jsx
import { Cat, Dog } from './Animals';
```

- 你也可以在匯入時使用 `as` 關鍵字重新命名具名匯出：
...
- 匯入混合匯出時，預設匯出放在前面（不加大括號），具名匯出放在後面（加大括號）：
