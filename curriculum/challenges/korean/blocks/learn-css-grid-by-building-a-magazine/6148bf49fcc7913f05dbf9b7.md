...
`.author-name a:hover` 선택자를 만들고 `background-color` 속성을 `#306203`로 설정하세요.

이렇게 하면 `a` 안에 있는 `.author-name` 요소에만 호버 효과가 생기며, 배경에 원래 freeCodeCamp 녹색이 표시됩니다.

# --hints--

`.author-name a:hover` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

`.author-name a:hover` 선택자에 `background-color` 속성을 `#306203`로 설정해야 합니다.
