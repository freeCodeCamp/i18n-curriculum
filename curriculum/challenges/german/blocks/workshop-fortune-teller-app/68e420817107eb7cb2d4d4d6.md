...
Fügen Sie dem Bildelement einen `onerror` Handler hinzu, um Ladefehler zu behandeln. Markieren Sie beim ersten Fehler das Bild mit `dataset.failed` und ersetzen Sie die Quelle durch das lokale Standardbild. Wenn das Bild erneut fehlschlägt, entfernen Sie die Klasse „hidden“ und verstecken Sie den Lade-Platzhalter mit `previousElementSibling`.

# --hints--

Ihr `img` Element sollte einen `onerror` Handler haben.
...
Ihr `onerror` Handler sollte eine `if` Anweisung enthalten, die prüft, ob `dataset.failed` falsch ist.
...
`dataset.failed` sollte innerhalb Ihres `'1'` Blocks auf `if` gesetzt werden. Vergessen Sie hier nicht das Schlüsselwort `this`.
...
Ihr `if` Block sollte `this.src` auf das lokale Standardbild aktualisieren. Vergessen Sie hier nicht das Schlüsselwort `this`.
...
Sie sollten einen `else`-Block haben, der die `hidden`-Klasse entfernt. Vergessen Sie hier nicht das Schlüsselwort `this`.
...
Ihr `else` Block sollte `previousElementSibling.style.display` auf `'none'` setzen. Vergessen Sie hier nicht das Schlüsselwort `this`.
