...
Crea otro elemento `label` (en una nueva linea) al final de tu `HTMLString`. Este `label` debe tener el texto `Entry # Calories`, usando tu sintaxis literal de plantillas para reemplazar `#` con el valor de `entryNumber`, y el atributo `for` configurado en `X-#-calories`, donde `X` es el `value` de `entryDropdown` y `#` es el valor de `entryNumber`.

# --hints--

Debes tener dos elementos `label` en tu `HTMLString`.
...
Tu nuevo elemento `label` debe estar en una nueva línea.
...
El nuevo elemento `label` debe estar después del elemento `input`.
...
Tu nuevo elemento `label` debe tener un atributo `for` asignado a `${entryDropdown.value}-${entryNumber}-calories`.
...
Tu nuevo elemento `label` debe tener el texto `Entry ${entryNumber} Calories`.
...
No debes modificar los elementos previamente existentes.
