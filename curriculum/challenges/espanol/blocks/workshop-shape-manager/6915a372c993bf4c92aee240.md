...
Ahora, crea una función `initializeApp` y déjala vacía por ahora.

Esta función inicializará toda la aplicación una vez que el DOM esté cargado. Así que es la función que pronto pasarás a un evento `DOMContentLoaded` para que la aplicación funcione.

# --hints--

Debes crear una función `initializeApp`.
