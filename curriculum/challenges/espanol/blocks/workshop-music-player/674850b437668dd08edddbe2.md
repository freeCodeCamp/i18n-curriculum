...
En los pasos anteriores construiste la funcionalidad para reproducir una canción. Ahora necesitas añadir la funcionalidad al botón de reproducción.

Utiliza el método `addEventListener()` en `playButton` y pasa un evento `"click"` como primer argumento. Por ahora, usa un callback que invoque `playSong(0)` como segundo argumento.

Luego, pruébalo haciendo clic en el botón de reproducción.


# --hints--

Debes agregar un escuchador de eventos para el evento `click` al `playButton` y pasar una función de callback que llame a `playSong(0)` como segundo argumento al método `addEventListener`.
