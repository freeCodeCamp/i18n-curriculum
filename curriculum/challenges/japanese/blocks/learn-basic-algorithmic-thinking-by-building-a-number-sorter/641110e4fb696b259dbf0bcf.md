...
このアルゴリズムでは、`while` ループを使う必要があります。このループには2つの条件が必要です。

- まず、配列の先頭（`j` でアクセス）を超えて実行されないこと。
- 次に、現在の値より小さい値を見つけた後はループが実行されないこと。

無限ループを防ぐために、ループ内で `j` をデクリメントしてください。
...
`while` ループの最初の条件は、`j` の値が `0` 以上であることをチェックしてください。
...
`while` ループには、`array[j]` の値が `currValue` より大きいことをチェックする条件が必要です。
...
`while` ループはループ内で `j` をデクリメントしてください。
