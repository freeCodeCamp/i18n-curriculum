...
Infine, dovresti renderizzare di nuovo le canzoni, aggiornare il testo accessibile del pulsante play e rimuovere il pulsante reset dalla playlist. Devi anche rimuovere il `resetButton` dal DOM.

Chiama la funzione `renderSongs()` con `sortSongs()` come argomento per renderizzare di nuovo le canzoni in ordine alfabetico.

Chiama la funzione `setPlayButtonAccessibleText()` per aggiornare il testo accessibile del pulsante play.

Rimuovi il pulsante reset dalla playlist chiamando il metodo `remove()` sulla variabile `resetButton`.

**Nota**: Ora puoi provare a rimuovere tutte le canzoni per vedere cosa succede.

# --hints--

Non dovresti modificare l'event listener esistente e il suo contenuto.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Dovresti chiamare la funzione `renderSongs` con `sortSongs()`.
...
Dovresti usare il metodo `remove()` per rimuovere il `resetButton` dal DOM.
