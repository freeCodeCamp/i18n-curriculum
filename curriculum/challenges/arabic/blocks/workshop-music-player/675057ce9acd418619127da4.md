...
الآن، أنشئ دالة باسم `getPreviousSong`. اجعلها تُرجع الأغنية الموجودة في `userData.songs` عند `getCurrentSongIndex()` ناقص واحد، أو `undefined` إذا كانت الأغنية الحالية هي الأولى في قائمة التشغيل.

# --hints--

يجب أن يكون لديك دالة باسم `getPreviousSong`.

```js
assert.isFunction(getPreviousSong);
```

يجب أن تستدعي دالتك `getPreviousSong` الدالة `getCurrentSongIndex`.

```js
assert.match(getPreviousSong.toString(), /getCurrentSongIndex\s*\(\s*\)/);
```

يجب أن تُرجع دالتك `getPreviousSong` الأغنية التي تسبق الأغنية الحالية.
...
يجب أن تُرجع دالتك `getPreviousSong` القيمة `undefined` عندما تكون الأغنية الحالية هي الأولى في قائمة التشغيل.
