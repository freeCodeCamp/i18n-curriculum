...
`results` の長さが `2` の場合の別のケースを追加してください。今回は、`result_list` に `x1 = <root1>` と `x2 = <root2>` の形式の2つの文字列を含むリストを代入してください。再び、正の符号と負の符号の両方を表示するようにしてください。
...
既存の `case` ブロックの後に、`2` パターンの新しい `case` を作成してください。
...
新しい `x1 = <root1>` の本体内で、`x2 = <root2>` と `result_list` の形式の2つの文字列を含むリストを `case` に代入してください。結果には正の符号と負の符号の両方を表示してください。
