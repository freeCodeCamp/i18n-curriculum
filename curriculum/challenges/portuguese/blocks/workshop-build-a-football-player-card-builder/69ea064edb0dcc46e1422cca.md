...
Agora você vai adicionar a lógica para salvar em `localStorage` usando `useEffect`.

Acessar ou salvar dados em `localStorage` é considerado um efeito colateral no React, então você deve usar `useEffect` para essa lógica.

Comece desestruturando `useEffect` do React, assim como foi feito com `useState`.

# --hints--

Você deve desestruturar `useEffect` de `React`.
