...
要自定义头像的大小，你可以将其设置为 `30`。 

首先创建一个名为 `avatar` 的常量。 然后使用 `replace` 方法对 `user.avatar_template` 进行处理并将结果赋值给它。

对于 `replace` 方法，使用 `/{size}/` 作为第一个参数，使用数字 `30` 作为第二个参数。
...
你应该将 `user.avatar_template.replace()` 赋值给你的 `avatar` 常量。

```js
assert(code.match(/const\s+avatar\s*=\s*user\.avatar_template\.replace\(\s*.*\s*\)/));
```

你应该在 `replace` 方法中将第一个参数设为 `/{size}/`，第二个参数设为数字 `30`。
