...
Надайте новим будівлям властивості `width` та `height`: `10%` та `50%` до `.bb2`, `10%` та `55%` до `.bb3` й `11%` та `58%` до `.bb4`. You will be using almost all percent based units and some flexbox for this project, so everything will be completely responsive.
