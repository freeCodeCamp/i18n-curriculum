...
Adicione dois elementos `span` com o `className` de `stat-value` e `stat-label` respectivamente a cada `div.stat-row`.

# --hints--

Cada `.stat-row` deve conter um `span` com `className` de `stat-value` e um `span` com `className` de `stat-label`.
