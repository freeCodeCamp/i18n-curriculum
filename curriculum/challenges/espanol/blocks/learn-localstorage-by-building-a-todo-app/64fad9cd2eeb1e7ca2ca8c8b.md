...
Usar la sintaxis de flecha para crear una función `updateTaskContainer`. Luego, mueve `taskData.forEach()` y su contenido del event listener `submit` de `taskForm` a la función recientemente creada.

# --hints--

Debes usar `const` y la sintaxis de flecha para crear una función `updateTaskContainer`.

```js
assert.match(code, /const\s+updateTaskContainer\s*=\s*\(\s*\)\s*=>\s*\{/)
```

Debes mover `taskData.forEach()` y su contenido dentro de la función `updateTaskContainer()`.
