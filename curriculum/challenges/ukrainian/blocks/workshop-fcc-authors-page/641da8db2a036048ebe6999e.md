...
Всередині функції `fetchMoreAuthors` встановіть змінні `startingIndex` і `endingIndex` рівними `+= 8`.
...
Ви повинні встановити змінну `startingIndex` рівною `+=8`.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Ви повинні встановити змінну `endingIndex` рівною `+=8`.
