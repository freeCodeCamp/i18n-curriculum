...
Mientras que los elementos `ul`/`li` son ideales para proporcionar viñetas para los elementos de la lista, los botones de radio no las necesitan. Puedes controlar cómo se ven las viñetas con la propiedad `list-style`. Por ejemplo, puedes convertir tus viñetas en círculos con lo siguiente:
...
Elimina el estilo predeterminado de los elementos `.answers-list` configurando su estilo en `none`, y elimina el relleno de la lista desordenada.
...
Debes dar a `.answers-list` un `list-style` de `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.answers-list')?.listStyle, 'none');
```

Debes dar a `.answers-list` un `padding` de `0`.
