...
`Works fine` means that something is functioning properly, with no issues. 例如：

`The program works fine.` – This means the program is running without any problems.

`Weird` describes something that is strange or not normal. `Acts weird` means that the behavior is unusual or not what is expected. 例如：
