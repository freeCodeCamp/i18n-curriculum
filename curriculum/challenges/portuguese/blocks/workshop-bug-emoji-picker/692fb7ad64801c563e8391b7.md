...
Crie uma subclasse `Spider` que estenda a classe `Bug` com o parâmetro de tipo genérico definido como `string`.

# --hints--

Você deve ter uma classe chamada `Spider`.
...
Sua classe `Spider` deve estender `Bug`.
...
Sua classe `Spider` deve estender `Bug` com um argumento de tipo `string`.
