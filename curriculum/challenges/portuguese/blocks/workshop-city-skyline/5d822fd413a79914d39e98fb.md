...
As transições gradientes muitas vezes mudam gradualmente de uma cor para outra.Quando é necessária uma mudança mais brusca, a transição pode ser feita com uma parada brusca, assim:
...
Você deve usar `--window-color2` de `6%` a `9%`.
...
No `.bb2b`, adicione um `linear-gradient` em transição de `--building-color2` de `0%` a `6%` e `--window-color2` de `6%` a `9%`.
