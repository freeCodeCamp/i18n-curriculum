...
Crea una regla `.lists li` para orientar los elementos de lista dentro de tu elemento `.lists`. Dale una propiedad `margin-bottom` establecida en `1.5rem`.

# --hints--

Debes tener un selector `.lists li`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

Tu selector `.lists li` debe tener una propiedad `margin-bottom` establecida en `1.5rem`.
