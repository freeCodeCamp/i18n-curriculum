...
Пам’ятайте, що властивість `grid-column` визначає, з яких колонок починається і на яких закінчується елемент. Іноді ви можете не знати, скільки колонок буде у вашій сітці, але хочете, щоб елемент закінчувався на останній колонці. Для цього можна використати `-1` для кінцевої колонки.

Створіть селектор `.hero` і надайте йому властивість `grid-column`, встановлену в `1 / -1`. Це скаже елементу зайняти всю ширину сітки.

# --hints--

У вас має бути селектор `.hero`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero'));
```

Ваш селектор `.hero` має мати властивість `grid-column`, встановлену в `1 / -1`.
