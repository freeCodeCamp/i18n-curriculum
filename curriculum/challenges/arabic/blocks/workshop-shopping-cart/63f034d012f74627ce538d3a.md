...
أخيرًا، حدّث خاصية `display` في كائن `style` للمتغير `cartContainer` إلى ثلاثي آخر يتحقق مما إذا كانت `isCartShowing` صحيحة. إذا كانت كذلك، عيّن خاصية `display` إلى `"block"`، وإلا عيّنها إلى `"none"`.

الآن يجب أن تتمكن من رؤية سلة التسوق الخاصة بك وإضافة عناصر إليها.
