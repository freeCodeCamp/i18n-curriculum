...
كما ذُكر سابقًا، عندما يحرر المستخدم الملاحظة ثم ينقر خارج الملاحظة، يجب أن تظهر رسالة تُفيد بأن الملاحظة حُفظت.

لتحقيق ذلك، عيّن قيمة `statusEl.textContent` إلى السلسلة النصية `"Note saved successfully!"`.

الآن عند اختبارك، يجب أن ترى تلك الرسالة تظهر على الشاشة.

# --hints--

عندما يحرر المستخدم ملاحظة، يجب تحديث رسالة الحالة لتُشير إلى أن الملاحظة حُفظت.
