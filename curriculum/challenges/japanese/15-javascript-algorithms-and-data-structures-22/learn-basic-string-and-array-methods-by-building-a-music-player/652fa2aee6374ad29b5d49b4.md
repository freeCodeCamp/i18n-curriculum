...
同様に、HTML に用意されている `#next`、`#previous`、`#shuffle` の要素にアクセスします。

それぞれ `nextButton`、`previousButton`、`shuffleButton` という名前の変数に代入してください。

# --hints--

`nextButton` という名前の変数が必要です。
...
`#next` の要素を変数 `nextButton` に代入してください。

```js
assert.equal(nextButton, document.getElementById('next'));
```

`previousButton` という名前の変数が必要です。
...
`#previous` の要素を変数 `previousButton` に代入してください。

```js
assert.equal(previousButton, document.getElementById('previous'));
```

`shuffleButton` という名前の変数が必要です。
...
`#shuffle` の要素を変数 `shuffleButton` に代入してください。
