...
Il selettore `table caption` dovrebbe avere una proprietà `color` impostata su `#356eaf`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('color') === 'rgb(53, 110, 175)');
```

Il selettore `table caption` dovrebbe avere una proprietà `font-size` impostata a `1.3em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('font-size') === '1.3em');
```

Il selettore `table caption` dovrebbe avere una proprietà `font-weight` impostata su `normal`.
