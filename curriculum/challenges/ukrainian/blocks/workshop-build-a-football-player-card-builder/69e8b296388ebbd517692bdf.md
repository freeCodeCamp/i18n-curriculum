...
Додайте атрибути `value` і `onChange` до обох вхідних даних DEF і PHY.

Для DEF встановіть `value` у `player.def` і `onChange` у стрілкову функцію, яка викликає `setPlayer` з розгортанням `player` і `def`, встановленим у `Number(e.target.value)`.

Для PHY встановіть `value` у `player.phy` і `onChange` у стрілкову функцію, яка викликає `setPlayer` з розгортанням `player` і `phy`, встановленим у `Number(e.target.value)`.

З цим кроком усі вхідні дані тепер підключені до стану, і живий перегляд оновлюватиметься для кожного поля!

# --hints--

Ваш DEF `input` має мати `value`, рівний показнику `def` гравця.
...
Ваш DEF `input` має мати обробник `onChange`, який викликає `setPlayer` з розгортанням `player` і `def`, встановленим у `Number(e.target.value)`.
...
Ваш PHY `input` має мати `value`, рівний показнику `phy` гравця.
...
Ваш PHY `input` має мати обробник `onChange`, який викликає `setPlayer` з розгортанням `player` і `phy`, встановленим у `Number(e.target.value)`.
