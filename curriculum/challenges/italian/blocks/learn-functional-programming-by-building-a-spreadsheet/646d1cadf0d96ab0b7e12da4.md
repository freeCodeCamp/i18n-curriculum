...
Poiché `elemValue` restituisce una funzione, la tua funzione `addCharacters` alla fine restituisce un array di riferimenti a funzioni. Vuoi che il metodo `.map()` esegua la funzione interna della tua funzione `elemValue`, il che significa che devi chiamare `elemValue` invece di farne solo riferimento. Passa `num` come argomento alla tua funzione `elemValue`.

<!-- TODO: Explain further? -->

# --hints--

Dovresti chiamare `elemValue()` nel tuo metodo `.map()`.
...
Dovresti passare `num` alla tua chiamata `elemValue()`.
