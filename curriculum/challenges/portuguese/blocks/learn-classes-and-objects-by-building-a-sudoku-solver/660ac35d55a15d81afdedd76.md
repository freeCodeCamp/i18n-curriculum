...
Dentro do loop `for`, declare uma variável `row_str` e atribua a ela uma list comprehension que itera sobre `row` e transforma cada item `i` em `row` em uma string.Use a função `str()` para isso.

# --hints--

Você deve declarar uma variável `row_str`.
...
Você deve atribuir uma list comprehension que itere sobre `row` à variável `row_str`.
...
A compreensão de lista atribuída à variável `row_str` deve usar `i` como variável de iteração.
...
A list comprehension atribuída à variável `row_str` deve chamar `str()` em cada item `i` em `row`.
