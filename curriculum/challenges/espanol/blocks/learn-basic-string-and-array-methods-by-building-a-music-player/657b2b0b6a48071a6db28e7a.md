...
Observe que el método `map()` toma una función como argumento. Esto se llama una <dfn>función de devolución de llamada</dfn>, que es una función que se pasa a otra función como argumento.  In the example above, the callback function is `(number) => number * 2`, and it's run on each element in the `numbers` array. El método `map()` luego devuelve un nuevo array con los resultados.
