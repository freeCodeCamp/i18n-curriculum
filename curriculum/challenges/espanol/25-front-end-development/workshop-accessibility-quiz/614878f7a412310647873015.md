...
Ordena el `header`, usando _Flexbox_ para crear espacio entre los elementos secundarios y centrarlos verticalmente.

# --hints--

Debes dar al `header` una propiedad `justify-content` de `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

Debes dar al `header` una propiedad `align-items` de `center`.
