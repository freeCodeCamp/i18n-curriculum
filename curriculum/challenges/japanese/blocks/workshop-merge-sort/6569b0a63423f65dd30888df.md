...
`pass` キーワードを削除してください。`while` ループのコードブロック内で、`left_part[left_array_index]` を `array[sorted_index]` に代入してください。

# --hints--

ループの本体には `array[sorted_index] = left_part[left_array_index]` があるはずです。ループの本体にある `pass` キーワードを削除するのを忘れないでください。
