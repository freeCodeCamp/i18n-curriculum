...
接下來，使用 `classList` 屬性和 `add()` 方法將 `"playing"` 類添加到 `playButton` 元素。 這將在 CSS 文件中查找類 `"playing"` 並將其添加到 `playButton` 元素。

要最終播放歌曲，請對 `audio` 變量使用 `play()` 方法。 `play()` 是來自網絡音頻 API 的用於播放 mp3 文件的方法。

# --hints--

你應該使用 `classList` 屬性和 `add` 方法將類 `"playing"` 添加到 `playButton`。

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

你應該在 `audio` 變量上使用 `play()` 方法。
