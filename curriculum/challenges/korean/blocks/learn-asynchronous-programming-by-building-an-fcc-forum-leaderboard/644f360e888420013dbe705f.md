...
`else` 절 안에서 문자열 `"general"`을 `selectedCategory.className`에 할당하세요.

그 아래에 문자열 `"General"`을 `selectedCategory.category`에 할당하세요.

마지막으로 숫자 `1`를 `selectedCategory.id`에 할당하세요.

# --hints--

문자열 `"general"`을 `selectedCategory.className`에 할당해야 합니다.

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

문자열 `"General"`을 `selectedCategory.category`에 할당해야 합니다.

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

숫자 `1`을 `selectedCategory.id`에 할당해야 합니다.
