...
`handleShapeSelect` 関数の中で、`.preventDefault()` の `e` メソッドを呼び出し、入力フィールドをクリアするために `clearInputFields()` 関数を呼び出してください。

# --hints--

`.preventDefault()` に対して `e` を呼び出す必要があります。

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

`clearInputFields` 関数を呼び出す必要があります。
