...
Unapaswa kutoa kitufe chako cha radio cha kawaida ukubwa na umbo.

Bado ukilenga `.contact-method-radio-btn`, weka `width` kuwa `20px`, `height` kuwa `20px`, `border-radius` kuwa `50%`, na `border` kuwa `2px solid gray`.

# --hints--

Unapaswa kuendelea kulenga darasa la `.contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Weka sifa ya `width` kuwa `20px`.
...
Weka sifa ya `border` kuwa `2px solid gray`.
