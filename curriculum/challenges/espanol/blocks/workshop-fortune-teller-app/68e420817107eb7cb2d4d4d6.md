...
Agrega un manejador `onerror` al elemento de imagen para manejar fallos de carga. En el primer fallo, márcalo usando `dataset.failed` y reemplaza la fuente con la imagen predeterminada local. Si la imagen falla de nuevo, elimina la clase hidden y oculta el marcador de carga usando `previousElementSibling`.

# --hints--

Tu elemento `img` debe tener un manejador `onerror`.
...
Tu manejador `onerror` debe contener una sentencia `if` que verifique si `dataset.failed` es falso.
...
`dataset.failed` debe establecerse en `'1'` dentro de tu bloque `if`. No olvides la palabra clave `this` aquí.
...
Tu bloque `if` debe actualizar `this.src` a la imagen predeterminada local. No olvides la palabra clave `this` aquí.
...
Debes tener un bloque `else` que elimine la clase `hidden`. No olvides la palabra clave `this` aquí.
...
Tu bloque `else` debe establecer `previousElementSibling.style.display` a `'none'`. No olvides la palabra clave `this` aquí.
