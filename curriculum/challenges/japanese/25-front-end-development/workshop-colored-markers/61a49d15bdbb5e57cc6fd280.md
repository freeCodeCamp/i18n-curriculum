...
色経由点 (color-stops) を利用すると、グラデーション軸に沿って色がどう配置されるかを微調整できます。 They are used in the `linear-gradient` function to specify where a color ends and the transition to the next color begins. Either a percentage or a length value can be used to define the color-stop position.
