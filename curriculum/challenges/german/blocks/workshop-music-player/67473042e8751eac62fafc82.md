...
Definieren Sie eine `playSong`-Funktion, die einen einzelnen Parameter entgegennimmt, der die eindeutige Kennung des Songs darstellt, den Sie abspielen möchten.

# --hints--

Sie sollten eine leere Funktion mit dem Namen `playSong` haben.

```js
assert.isFunction(playSong);
```

Ihre `playSong`-Funktion sollte einen einzelnen Parameter entgegennehmen.
