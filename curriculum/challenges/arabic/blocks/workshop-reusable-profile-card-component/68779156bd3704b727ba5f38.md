...
كما تتذكر من الدروس السابقة، يمكنك استخدام الطريقة `map` للتكرار خلال الكائنات المختلفة وعرض المكوِّنات ديناميكيًا بهذا الشكل:
...
استخدم الطريقة `map()` لعرض `Card` واحد من المصفوفة. مرر الخصائص `name` و `title` و `bio` إلى مكوِّن `Card`.

# --hints--

يجب أن تستخدم الطريقة `map()` للتكرار خلال مصفوفة `profiles`.

```js
assert.match(code, /profiles\.map\s*\(/);
```

يجب أن تمرر الخصائص `name` و `title` و `bio` من كائن `profile`.
