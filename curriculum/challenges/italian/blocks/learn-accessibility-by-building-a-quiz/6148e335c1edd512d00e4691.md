...
Cliccando sui collegamenti di navigazione, la viewport dovrebbe saltare alla sezione corrispondente. Tuttavia, questo salto può risultare disorientante per alcune persone.

Seleziona tutti gli elementi e imposta il `scroll-behavior` su `smooth`.
...
Dovresti dare a `*` una `scroll-behavior` di `smooth`.
