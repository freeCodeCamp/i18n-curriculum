...
يجب عليك استخدام منتقي `.back-mountain`.

```js
assert.match(code, /\.back-mountain\s*\{/);
```

يجب عليك إعطاء `.back-mountain` خاصية `width` بقيمة `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

يجب عليك إعطاء `.back-mountain` خاصية `height` بقيمة `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

يجب عليك إعطاء `.back-mountain` خاصية `background` من `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`.
