...
Quieres agregar el mismo degradado a las siguientes dos secciones. En lugar de hacer eso, crea un nuevo selector de clase llamado `bb1-window` y mueve las propiedades `height` y `background` y sus valores de `.bb1a` al nuevo selector de clase.

# --hints--

Debes crear un nuevo selector de clase llamado `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

Debes mover la propiedad `height` y su valor de `.bb1a` a `.bb1-window`.
...
Debes mover la propiedad `background` y su valor de `.bb1a` a `.bb1-window`.
...
No debes mover la propiedad `background-color` de `.bb1a`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

No debes mover la propiedad `width` de `.bb1a`.
