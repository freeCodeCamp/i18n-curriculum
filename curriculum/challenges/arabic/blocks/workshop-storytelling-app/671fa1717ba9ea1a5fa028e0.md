...
أنشئ عنصر `main` مع صنف `story-container`. داخل عنصر `.story-container`، أنشئ `div` مع صنف `btn-container`.

# --hints--

يجب أن يكون لديك عنصر `main` مع صنف `story-container`.

```js
assert.exists(document.querySelector('main.story-container'));
```

داخل عنصر `.story-container`، يجب أن يكون لديك `div` مع صنف `btn-container`.
