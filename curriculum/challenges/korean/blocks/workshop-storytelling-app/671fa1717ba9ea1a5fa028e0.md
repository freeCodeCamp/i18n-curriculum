...
클래스가 `main`인 `story-container` 요소를 만드세요. `.story-container` 요소 안에 클래스가 `div`인 `btn-container`를 만드세요.

# --hints--

클래스가 `main`인 `story-container` 요소가 있어야 합니다.

```js
assert.exists(document.querySelector('main.story-container'));
```

`.story-container` 요소 안에 클래스가 `div`인 `btn-container`가 있어야 합니다.
