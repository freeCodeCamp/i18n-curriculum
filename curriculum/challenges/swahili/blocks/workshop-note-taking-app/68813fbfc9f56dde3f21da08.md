...
Wakati mtumizi anapohariri kumbukumbu na kisha kubonyeza tab au kubofya nje ya kadi, basi ujumbe wa hali unapaswa kuonyeshwa ukimjulisha mtumizi kuwa kumbukumbu imehifadhiwa kwa mafanikio.

Ili kuanza, tengeneza kipengele cha `div` chenye sifa ya kipengele ya `id` iliyowekwa kuwa `"status"`.

# --hints--

Unapaswa kuwa na kipengele cha `div` cha pili.

```js
assert.lengthOf(document.querySelectorAll("div"), 2);
```

Kipengele chako cha `div` cha pili kinapaswa kuwa na sifa ya kipengele ya `id`.

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

Kipengele chako cha `div` cha pili kinapaswa kuwa na sifa ya kipengele ya `id` iliyowekwa kuwa `"status"`.
