...
你的正則表達式應匹配整個單詞，而不是部分單詞。 也就是說，你不想匹配 `hands-free money management`。

要做到這一點，先檢查模式前後是否有空格。 你可以使用元字符 `\s`，它將匹配空格、製表符和換行符。

# --hints--

你的 `freeRegex` 應該使用 `\s` 標記。

```js
assert.match(freeRegex.source, /\\s/);
```

你的 `freeRegex` 應該在模式的開頭和結尾查找空格。
