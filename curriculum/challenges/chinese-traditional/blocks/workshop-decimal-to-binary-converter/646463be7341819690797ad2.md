...
當使用者嘗試將十進位數字 `5` 轉換為二進位時，你會顯示動畫，所以你需要在你的 `checkUserInput()` 函式中為此添加檢查。

使用 `if` 述語來檢查 `numberInput` 的 `value` 屬性是否相等於數字 `5`。請記得使用 `parseInt()` 函式（程式）將字串轉換成數字，然後再與 `5` 比較。暫時將 `if` 述語留空。

# --hints--

你應該在現有的 `if` 述語之後，為你的 `checkUserInput()` 函式添加第二個 `if` 述語。

```js
assert.match(String(checkUserInput), /if\s*\(\s*!numberInput\s*\.\s*value\s*\|\|\s*isNaN\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)[\s\S]+if\s*\(/);
```

你應該在新的 `if` 述語的條件中使用 `parseInt()` 函式（程式）。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(/);
```

你應該檢查 `numberInput` 的 `value` 屬性是否相等於 `5`。
