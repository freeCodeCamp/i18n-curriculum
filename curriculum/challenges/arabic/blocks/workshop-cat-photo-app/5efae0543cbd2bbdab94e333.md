...
يجب أن يحتوي عنصر `figure` على وسم بداية. وسم البداية له هذه الصيغة النحوية: `<elementName>`.

```js
assert.lengthOf(document.querySelectorAll('figure'), 2);
```

يجب أن يحتوي عنصر `figure` على وسم الإغلاق. وسم الإغلاق يحتوي على `/` مباشرة بعد الحرف `<`.
