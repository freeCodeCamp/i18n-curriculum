...
`--penguin-skin` という名前のカスタム CSS 変数を作成し、`gray` に設定してください。それから、関連するすべてのプロパティ値をそれに置き換えてください。

# --hints--

`:root` に `--penguin-skin` プロパティを設定してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

`--penguin-skin` に `gray` の値を設定する必要がありますが、`--fcc-actual--` が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

`.penguin-head` に `background` プロパティを `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))` に設定してください。
...
`.penguin-body::before` に `background-color` プロパティを `var(--penguin-skin)` に設定する必要がありますが、`--fcc-actual--` が見つかりました。
...
`.arm` に `background` プロパティを `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))` に設定してください。
