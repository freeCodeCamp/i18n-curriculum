...
Настав час перемістити частину коду з функції `move()` до функції `make_allowed_move()`. Перемістіть код, вкладений у першу інструкцію `if` (окрім першого виклику `print()`), до вашої нової функції. Уважно слідкуйте за відступами.
Не забудьте видалити ключове слово `pass`.

# --hints--

Ви маєте перемістити код, вкладений у першу інструкцію `if` (окрім першого виклику `print()`), до вашої нової функції.
