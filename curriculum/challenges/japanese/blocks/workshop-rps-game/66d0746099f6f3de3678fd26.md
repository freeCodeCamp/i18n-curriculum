...
いよいよ`showResults`関数をテストする時間です。

まず、`rockBtn`という変数を作成し、idが`rock-btn`のボタンの値を代入してください。

次に、`paperBtn`という変数を作成し、idが`paper-btn`のボタンの値を代入してください。

最後に、`scissorsBtn`という変数を作成し、idが`scissors-btn`のボタンの値を代入してください。

# --hints--

`rockBtn`という変数があるはずです。

```js
assert.isNotNull(rockBtn);
```

`rockBtn`変数にはidが`rock-btn`のボタンが代入されているはずです。

```js
assert.equal(rockBtn, document.getElementById("rock-btn"));
```

`paperBtn`という変数があるはずです。

```js
assert.isNotNull(paperBtn);
```

`paperBtn`変数にはidが`paper-btn`のボタンが代入されているはずです。

```js
assert.equal(paperBtn, document.getElementById("paper-btn"));
```

`scissorsBtn`という変数があるはずです。

```js
assert.isNotNull(scissorsBtn);
```

`scissorsBtn`変数にはidが`scissors-btn`のボタンが代入されているはずです。
