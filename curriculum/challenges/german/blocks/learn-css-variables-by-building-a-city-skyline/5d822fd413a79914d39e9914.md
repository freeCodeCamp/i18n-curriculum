...
Verschieben Sie die `background-color`-Eigenschaft und ihren Wert von `.fb1` zu `.fb1b`.

# --hints--

Sie sollten `background-color` aus `.fb1` entfernen.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb1")?.backgroundColor);
```

Sie sollten eine `background-color` von `--building-color4` zu `.fb1b` hinzufügen.
