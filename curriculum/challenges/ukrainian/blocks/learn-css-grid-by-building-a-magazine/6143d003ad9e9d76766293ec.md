...
У вашому елементі `.image-quote` вкладіть елемент `hr`, елемент `p` та другий елемент `hr`. Додайте елементу `p` множину `class`, встановлену в `quote`, та текст `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.

# --hints--

Ви повинні додати два елементи `hr` до вашого елемента `.image-quote`.

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

Ви повинні додати елемент `p` до вашого елемента `.image-quote`.

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

Ваші дочірні елементи `.image-quote` мають бути в правильному порядку.
...
Ваш новий елемент `p` має мати множину `class`, встановлену в `quote`.

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Ваш новий елемент `p` має містити текст `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
