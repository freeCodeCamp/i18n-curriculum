...
`timeAgo` 関数では、現在の日時と最後のアクティビティの日時の差を計算します。 これで、トピックに動きがあってからどのくらいの時間が経過したか表示できるようになります。

下記の要件を満たすように、`timeAgo` 関数を完成させてください。

- If the amount of minutes that have passed is less than `60`, return the string `"xm ago"`. `x` は分数を表す。
- If the amount of hours that have passed is less than `24`, return the string `"xh ago"`. `x` は時間数を表す。
- Otherwise, return the string `"xd ago"`. `x` は日数を表す。

時間差の計算に役立つ式をいくつか紹介します。
