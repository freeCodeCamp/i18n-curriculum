...
Aggiorna la tua callback `.map()` per chiamare la funzione `Number()`. Passa `dropdown.value` a quella chiamata di funzione.

Apri la scheda Console per vedere che il tuo `inputValues` è ora un array di numeri.

# --hints--

Dovresti chiamare la funzione `Number()` nella tua callback `.map()`. 
...
Dovresti passare `dropdown.value` alla tua chiamata `Number()`.
