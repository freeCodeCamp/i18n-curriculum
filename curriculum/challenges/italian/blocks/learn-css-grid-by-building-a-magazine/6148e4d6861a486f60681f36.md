...
Le immagini dovrebbero essere disposte in un layout a due colonne e tre righe.

Dai al selettore `.image-wrapper` una proprietà `grid-template-columns` impostata su `2fr 1fr` e una proprietà `grid-template-rows` impostata su `repeat(3, min-content)`. Questo farà sì che le righe della nostra griglia si adattino in altezza in base al contenuto, mentre le colonne manterranno una larghezza fissa basata sul contenitore.

# --hints--

Il tuo selettore `.image-wrapper` dovrebbe avere una proprietà `grid-template-columns` impostata su `2fr 1fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns === '2fr 1fr');
```

Il tuo selettore `.image-wrapper` dovrebbe avere una proprietà `grid-template-rows` impostata su `repeat(3, min-content)`.
