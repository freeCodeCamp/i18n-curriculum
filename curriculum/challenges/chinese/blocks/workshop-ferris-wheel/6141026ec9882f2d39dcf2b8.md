...
在 `0%` 和 `100%` 选择器之间，添加一个 `50%` 选择器。 这将适用于动画周期的中间阶段。 将 `background-color` 设置为 `purple`。

# --hints--

你应该在 `@keyframes cabins` 规则中创建一个新的 `50%` 选择器。
