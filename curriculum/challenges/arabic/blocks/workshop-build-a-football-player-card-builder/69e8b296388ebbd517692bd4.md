...
أضف عنصرين `div` إضافيين، كل منهما يحمل `className` بقيمة `form-group` داخل `stats-grid`، واحد لـ PAS والآخر لـ DRI.

بالنسبة لـ PAS، أضف `label` مع `htmlFor` بقيمة `pas` والنص `PAS`، ثم أضف مُدخَلًا مع `id` بقيمة `pas` و`type` من نوع `number`.

وبالنسبة لـ DRI، أضف `label` مع `htmlFor` بقيمة `dri` والنص `DRI`، ثم أضف مُدخَلًا مع `id` بقيمة `dri` و`type` من نوع `number`.

# --hints--

يجب أن تضيف عنصرين `div` إضافيين، كل منهما يحمل `className` بقيمة `form-group` داخل عنصر `div` الذي يحمل `className` بقيمة `stats-grid`.
...
يجب أن يحتوي عنصر `div` الثالث الذي يحمل `className` بقيمة `form-group` على `label` مع خاصية `htmlFor` بقيمة `pas`، و`className` بقيمة `label`، ونص `PAS`.
...
يجب أن يحتوي عنصر `div` الثالث الذي يحمل `className` بقيمة `form-group` على `input` يحمل `className` بقيمة `input`، و`id` بقيمة `pas`، و`type` بقيمة `number`.
...
يجب أن يحتوي عنصر `div` الرابع الذي يحمل `className` بقيمة `form-group` على `label` مع خاصية `htmlFor` بقيمة `dri`، و`className` بقيمة `label`، ونص `DRI`.
...
يجب أن يحتوي العنصر الرابع من نوع `div` الذي يحمل `className` بقيمة `form-group` على `input` يحمل `className` بقيمة `input`، و`id` بقيمة `dri`، و`type` بقيمة `number`.
