...
Sahihisha hoja ya kitendakazi kuwa `poems`, kisha tengeneza mzunguko wa `for` unaozunguka maudhui yote ya sasa ya kitendakazi unaorudia juu ya `poems` na kutumia `poem` kama kigezo cha mzunguko.

# --hints--

Kitendakazi cha `pin_extractor` kinapaswa kuwa na kigezo kinachoitwa `poems`.
...
Unapaswa kuwa na mzunguko wa `for` unaorudia juu ya `poems`.
...
Mzunguko unapaswa kuwa na `poem` kama kigezo cha mzunguko.
...
Unapaswa kuhamisha mwili uliopo wa kitendakazi ndani ya mzunguko mpya wa `for`.
