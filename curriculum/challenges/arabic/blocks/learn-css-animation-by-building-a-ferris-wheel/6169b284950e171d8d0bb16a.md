...
أخيرًا، أنشئ محدِّدًا جديدًا بنسبة `75%` بين محدِّدَي `50%` و `100%`. امنح هذا المحدِّد الجديد خاصية `background-color` بقيمة `yellow`.

بهذا، تصبح حركتك أكثر سلاسة وتكتمل عجلة فيريس الخاصة بك.

# --hints--

يجب أن تنشئ محدِّدًا جديدًا بنسبة `75%` في قاعدة `@keyframes cabins` الخاصة بك.
...
يجب أن يكون محدِّد `75%` بين محدِّدَي `50%` و `100%`.
