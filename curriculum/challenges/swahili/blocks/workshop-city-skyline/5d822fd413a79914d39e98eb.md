...
Mchoro wa mji unaanza kuonekana. Jaza sifa ya `background-color` ya majengo ya mbele. Tumia tofauti yako ya `--building-color1` kujaza `.fb3` na `.fb4`, `--building-color2` kwa `.fb5`, `--building-color3` kwa `.fb2` na `.fb6`, na `--building-color4` kwa `.fb1`.

# --hints--

Unapaswa kumpa `.fb1` sifa ya `background-color` kwa kutumia `--building-color4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb1')?.getPropVal('background-color', true), 'var(--building-color4)');
```

Unapaswa kumpa `.fb2` sifa ya `background-color` kwa kutumia `--building-color3`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb2')?.getPropVal('background-color', true), 'var(--building-color3)');
```

Unapaswa kumpa `.fb3` sifa ya `background-color` kwa kutumia `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb3')?.getPropVal('background-color', true), 'var(--building-color1)');
```

Unapaswa kumpa `.fb4` sifa ya `background-color` kwa kutumia `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.getPropVal('background-color', true), 'var(--building-color1)');
```

Unapaswa kumpa `.fb5` sifa ya `background-color` kwa kutumia `--building-color2`.
