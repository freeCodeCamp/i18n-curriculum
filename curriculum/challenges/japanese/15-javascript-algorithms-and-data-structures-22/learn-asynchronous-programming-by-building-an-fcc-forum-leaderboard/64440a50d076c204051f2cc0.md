...
閲覧数をより読みやすい形式に変換する関数を作成します。 例えば、閲覧数が `1000` の場合 `1k`、`100,000` の場合 `100k` と表示しましょう。

関数 `viewCount` を作成し、仮引数 `views` を持たせてください。 `views` が `1000` 以上の場合、`views` の値を `1000` で割った数に `k` という文字を付加した文字列を返してください。 `views / 1000` の値は整数に切り捨ててください。

それ以外の場合は、`views` の値を返してください。

例えば、`views` が `1000` の場合、戻り値は文字列 `"1k"` となります。

# --hints--

`viewCount` という関数が必要です。

```js
assert.isFunction(viewCount);
```

`viewCount` 関数には `views` というパラメーターが必要です。

```js
assert.match(viewCount.toString(), /\(?views\)?/);
```

`viewCount` 関数は、`views` が `1000` の場合、文字列 `"1k"` を返す必要があります。

```js
assert.strictEqual(viewCount(1000), "1k");
```

`viewCount` 関数は、`views` が `100000` の場合、文字列 `"100k"` を返す必要があります。

```js
assert.strictEqual(viewCount(100000), "100k");
```

`viewCount` 関数は、`views` が `1000` 以上の場合、正しい文字列を返す必要があります。
...
`viewCount` 関数は、`views` が `200` の場合、`views` を返す必要があります。

```js
assert.strictEqual(viewCount(200), 200);
```

`viewCount` 関数は、`views` が `1000` 未満の場合、`views` を返す必要があります。
