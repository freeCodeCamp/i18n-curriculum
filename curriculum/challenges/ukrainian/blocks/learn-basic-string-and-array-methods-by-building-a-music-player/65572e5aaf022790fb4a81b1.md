...
Тепер настав час додати функціонал скидання до `resetButton`. Це поверне пісні у плейлист при натисканні.

Додайте слухача події click до змінної `resetButton`. Передайте функцію зворотного виклику у вигляді стрілкової функції, поки що залиште її порожньою.

# --hints--

Ви повинні використати метод `addEventListener()` на `resetButton`.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

Ваш слухач події `resetButton` має слухати подію `"click"`.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

Функція зворотного виклику вашого слухача події має бути стрілковою функцією з порожніми фігурними дужками.
