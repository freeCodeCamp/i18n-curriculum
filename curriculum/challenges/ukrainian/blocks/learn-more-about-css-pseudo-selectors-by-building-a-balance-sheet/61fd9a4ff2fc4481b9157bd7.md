...
До третьої таблиці додайте `caption` з текстом `Net Worth` та встановіть заголовок і тіло таблиці.

# --hints--

Ваш третій елемент `table` повинен мати елемент `caption`.

```js
assert(document.querySelectorAll('table')?.[2]?.children?.[0]?.localName === 'caption');
```

Елемент `caption` повинен мати текст `Net Worth`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('caption')?.textContent === 'Net Worth');
```

Ваш третій елемент `table` повинен мати елемент `thead`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('thead'));
```

Ваш третій елемент `table` повинен мати елемент `tbody`.
