...
如果你用數字列表測試表單，就會看到平均值顯示在頁面上。 不過，這只是因爲 freeCodeCamp 的 iframe 有特殊設置。 通常情況下，當提交表單時，提交事件會觸發頁面的刷新。

要解決這個問題，在 `onsubmit` 屬性中的 `calculate();` 調用後添加 `return false;`。

# --hints--

你的 `onsubmit` 屬性應該有一個 `return false;` 語句。

```js
assert.match(document.querySelector("form").getAttribute("onsubmit"), /return\s+false\s*;?/);
```

你的 `onsubmit` 屬性應該依舊調用 `calculate()`。

```js
assert.match(document.querySelector("form").getAttribute("onsubmit"), /calculate\(\s*\)/);
```

您應該在調用 `calculate()` 之後返回 `false`。
