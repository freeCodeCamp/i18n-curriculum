...
使用 `forEach` 方法循环遍历 `playlistSongElements`。 

`forEach` 方法用于循环遍历数组并对数组的每个元素执行函数。 例如，假设你有一个数字数组，并且你想要将每个数字记录到控制台。
...
对 `playlistSongElements` 使用 `forEach` 方法。 传入 `songEl` 作为参数并使用箭头语法添加一个空的回调。

# --hints--

你应该将 `forEach()` 链接到 `playlistSongElements`。

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

你的 `forEach()` 方法应该有一个 `songEl` 参数并使用箭头语法。

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

你的 `forEach` 应该有一对空的花括号。
