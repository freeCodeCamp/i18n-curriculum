...
Declare uma função `toNumberList` que recebe um parâmetro `args` e retorna implicitamente o resultado da divisão do `args` por vírgulas. Então encadeie um método `map` ao seu método `split` e passe `parseFloat` como argumento para o método `map`.

# --hints--

Você deve declarar uma variável `toNumberList`.
...
Você deve usar `const` para declarar a variável `toNumberList`.
...
A variável `toNumberList` deve ser uma arrow function.
...
A função `toNumberList` deve ter um parâmetro `args`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

A função `toNumberList` deve usar um retorno implícito.
...
A função `toNumberList` deve retornar o resultado da chamada do método `.split()` em `args`.
...
Você deve dividir `args` no caractere `","`.
...
Você deve encadear o método `.map()` ao método `.split()`.
...
Você deve passar uma referência a função `parseFloat` como a função de callback de `.map()`.
