...
بعد ذلك، أضف عبارة `else if` للتحقق مما إذا تم الضغط على المفتاح الأيسر وإذا كانت قيمة `isCheckpointCollisionDetectionActive` صحيحة.

داخل هذا الشرط، أضف حلقة `forEach` للتكرار خلال مصفوفة `platforms`.

داخل الحلقة، استخدم معامل التعيين بالإضافة لإضافة 5 إلى موضع `x` للمنصة.

# --hints--

يجب أن يكون لديك شرط يتحقق مما إذا تم الضغط على المفتاح الأيسر وإذا كانت قيمة `isCheckpointCollisionDetectionActive` صحيحة.
...
يجب أن تضيف حلقة `forEach` تقوم بالتكرار خلال مصفوفة `platforms`.

```js
assert.match(code, /else\s+if\s*\(.*\)\s*{\s*platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

يجب أن تستخدم معامل التعيين بالإضافة لإضافة 5 إلى موضع `x` للمنصة.
