...
在 `form` 元素內，為你的 `form` 添加一個 `label` 元素。你的 `label` 元素應該有文字 `Search for fruits:` 和一個 `htmlFor` 屬性，屬性值為 `"search-input"`。

# --hints--

你應該在 `form` 元素內有一個 `label` 元素。
...
`label` 元素應該有一個 `htmlFor` 屬性，其值為 `"search-input"`。
...
你的 `label` 元素應該有文字 `Search for fruits:`。
