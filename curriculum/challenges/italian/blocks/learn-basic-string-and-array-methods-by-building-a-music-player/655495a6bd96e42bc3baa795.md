...
Adesso devi mostrare il titolo e l'artista della canzone corrente nel display del lettore. Usa `const` e la sintassi a freccia per creare una funzione `setPlayerDisplay` vuota.

# --hints--

Dovresti usare `const` per creare una funzione vuota chiamata `setPlayerDisplay`.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` dovrebbe essere una funzione.

```js
assert.isFunction(setPlayerDisplay)
```

La tua funzione `setPlayerDisplay` dovrebbe usare la sintassi a freccia.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

La tua funzione `setPlayerDisplay` dovrebbe essere vuota.
