...
Il selettore `table` dovrebbe avere una proprietà `width` impostata al `100%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('width') === '100%');
```

Il selettore `table` dovrebbe avere una proprietà `position` impostata su `relative`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('position') === 'relative');
```

Il selettore `table` dovrebbe avere una proprietà `margin-top` impostata su `3rem`.
