...
title: 트윗 임베드에서 align-items 속성 사용하기
challengeType: 0
...
마지막 도전 과제에서는 `align-items` 속성을 소개하고 예시를 보여드렸습니다. 이 속성은 몇몇 트윗 임베드 요소에 적용하여 내부의 플렉스 항목들을 정렬할 수 있습니다.

# --instructions--

헤더의 `align-items` 요소에 CSS 속성 `.follow-btn`을 추가하세요. 값을 `center`로 설정하세요.

# --hints--

`.follow-btn`가 페이지에 렌더링되어야 합니다. 광고 차단기 같은 확장 기능은 반드시 꺼두세요.
...
`.follow-btn` 요소에 `align-items` 속성을 값 `center`로 설정해야 합니다.
