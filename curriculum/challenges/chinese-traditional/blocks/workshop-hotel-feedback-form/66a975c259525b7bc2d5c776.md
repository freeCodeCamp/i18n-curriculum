...
你的酒店反饋表還應允許用戶對食物進行評分。

首先添加一個 `label` 元素，其文本爲 `How was the food?`。 該 `label` 元素應將 `for` 屬性設置爲 `"food"`。

在你的 `label` 元素下方，添加一個 `select` 元素，`id` 和 `name` 設置爲 `"food"`。

# --hints--

你應將 `label` 元素的 `for` 屬性設置爲 `"food"`。

```js
assert.exists(document.querySelector('label[for="food"]'));
```

你的 `label` 應包含文本 `How was the food?`。

```js
assert.strictEqual(document.querySelector('label[for="food"]')?.textContent.trim(), 'How was the food?');
```

你應將 `label` 下方的 `select` 元素的 `id` 設置爲 `"food"`。

```js
assert.exists(document.querySelector('label + select#food'));
```

你的 `select` 元素應將 `name` 屬性設置爲 `"food"`。
