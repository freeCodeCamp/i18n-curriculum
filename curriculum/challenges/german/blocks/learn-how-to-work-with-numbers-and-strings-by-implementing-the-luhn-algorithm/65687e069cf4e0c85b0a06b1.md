...
Fügen Sie außerhalb der `if`-Anweisung `number` zu `sum_of_even_digits` hinzu. Entfernen Sie außerdem den `print`-Aufruf.

# --hints--

Sie sollten `sum_of_even_digits += number` innerhalb der `for`-Schleife haben.
...
Sie sollten `print(number)` nicht innerhalb der `for`-Schleife haben.
...
Sie sollten `sum_of_even_digits += number` nicht innerhalb der `if`-Anweisung haben.
