...
爲數字 `417` 添加一個新的鍵，值爲空對象。

在該對象中，添加一個屬性，其鍵爲 `category`，字符串值爲 `"freeCodeCamp Support"`。

在該屬性下，添加另一個名爲 `className` 的鍵，其字符串值爲 `"support"`。

# --hints--

你的 `allCategories` 對象應該有一個 `417` 屬性。
