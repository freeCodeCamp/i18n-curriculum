...
`profiles` 配列に2つのオブジェクトを追加してください — 1つは Tiffany 用、もう1つは Doug 用です。
...
`map()` のロジックは変更せず、3つのカードすべてをレンダリングしてください。

これで、再利用可能なプロフィールカードコンポーネントのプロジェクトは完成です！

# --hints--

正確に3つの `Card` コンポーネントをレンダリングしてください。
