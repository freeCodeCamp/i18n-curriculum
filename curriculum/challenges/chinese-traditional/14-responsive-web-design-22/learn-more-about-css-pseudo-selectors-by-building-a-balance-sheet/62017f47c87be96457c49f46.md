...
給 `span[class~="sr-only"]` 選擇器中的每個屬性一個 `!important` 關鍵字。 不要更改任何值。

# --hints--

`span[class~="sr-only"]` 選擇器應該將 `border` 屬性設置爲 `0 !important`。
...
`span[class~="sr-only"]` 選擇器應該將 `clip` 屬性設置爲 `rect(1px, 1px, 1px, 1px) !important`。
...
`span[class~="sr-only"]` 選擇器應該將 `clip-path` 屬性設置爲 `inset(50%) !important` .
...
你的 `span[class~="sr-only"]` 選擇器應該將 `height` 屬性設置爲 `1px !important`。
...
你的 `span[class~="sr-only"]` 選擇器應該將 `width` 屬性設置爲 `1px !important`。
...
你的 `span[class~="sr-only"]` 選擇器應該將 `position` 屬性設置爲 `absolute !important`。
...
你的 `span[class~="sr-only"]` 選擇器應該將 `overflow` 屬性設置爲 `hidden !important`。
...
你的 `span[class~="sr-only"]` 選擇器應該將 `white-space` 屬性設置爲 `nowrap !important`。
...
你的 `span[class~="sr-only"]` 選擇器應該將 `padding` 屬性設置爲 `0 !important`。
...
你的 `span[class~="sr-only"]` 選擇器應該將 `margin` 屬性設置爲 `-1px !important`。
