...
Njia ya `.extend()` inakuwezesha kuongeza vipengele kutoka kwenye iterable mwishoni mwa orodha:
...
Tengeneza kifungu cha `else` na tumia njia ya `.extend()` kuongeza njia ya nodi ya sasa kwenye njia ya nodi jirani.

# --hints--

Unapaswa kutengeneza kifungu cha `else` baada ya tamko lako lililopangwa ndani la `if`.
...
Unapaswa kuwa na `paths[node].extend(paths[current])` katika kifungu chako cha `else`.
