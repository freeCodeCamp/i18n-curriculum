...
クラスセレクターを使って `.cat-whisker-right-bottom` 要素をターゲットにし、`position` の `absolute`、`top` の `134px`、`left` の `109px` で位置を調整してください。
 

# --hints--

`.cat-whisker-right-bottom` セレクターが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom'))
```

`.cat-whisker-right-bottom` セレクターには `position` に設定された `absolute` プロパティが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.position === 'absolute')
```

`.cat-whisker-right-bottom` セレクターには `top` に設定された `134px` プロパティが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.top === '134px')
```

`.cat-whisker-right-bottom` セレクターには `left` に設定された `109px` プロパティが必要です。
