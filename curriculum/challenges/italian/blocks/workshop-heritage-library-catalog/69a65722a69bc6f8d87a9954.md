...
Concludiamo con un riepilogo. Registra in console `catalog.length` e `Object.keys(byDecade).length` per vedere il totale dei libri e i gruppi per decennio.

Dichiara `let oldestYear = Infinity` e `let newestYear = 0`. Usa un loop `for` su `catalog`: per ogni elemento in cui l'anno non è `"Unknown"`, aggiorna `oldestYear` se l'anno è minore e `newestYear` se è maggiore. Registra in console entrambi i valori.

Hai completato il workshop Heritage Library Catalog!

# --hints--

Dovresti registrare `catalog.length` nella console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*catalog\.length\s*\)/);
```

Dovresti registrare `Object.keys(byDecade).length` nella console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*Object\.keys\s*\(\s*byDecade\s*\)\.length\s*\)/);
```

Dovresti usare un loop `for` su `catalog` per trovare gli anni più vecchi e più recenti.

```js
assert.match(__helpers.removeJSComments(code), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

L'anno più vecchio nel catalogo dovrebbe essere `850`.

```js
assert.strictEqual(oldestYear, 850);
```

L'anno più recente nel catalogo dovrebbe essere `2011`.

```js
assert.strictEqual(newestYear, 2011);
```

Dovresti registrare `oldestYear` nella console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*oldestYear\s*\)/);
```

Dovresti registrare `newestYear` nella console.
