...
使用相同的语法，在 `#lunch` 元素中查询 `number` 输入并将其分配给 `lunchNumberInputs`。

# --hints--

你应该声明一个 `lunchNumberInputs` 变量。

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=/);
```

你应该使用 `#lunch input[type='number']` 调用 `document.querySelectorAll()`。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你应该将 `document.querySelectorAll()` 调用的结果分配给 `lunchNumberInputs`。
