...
最後，為你的程式碼添加一個 `else` 語句，列印允許的移動並呼叫 `make_allowed_move`。嘗試找出正確的引數。
...
你應該在 `else` 區塊中有 `print(f'Move {i + 1} allowed between {source} and {target}')`。
...
你應該在 `else` 區塊中有 `make_allowed_move(source, target)`。
