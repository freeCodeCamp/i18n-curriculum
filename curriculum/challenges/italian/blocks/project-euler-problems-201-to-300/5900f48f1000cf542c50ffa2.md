---
id: 5900f48f1000cf542c50ffa2
title: 'Problema 291: primi di Panaitopol'
challengeType: 1
forumTopicId: 301943
dashedName: problem-291-panaitopol-primes
---

# --description--

Un numero primo $p$ si chiama primo di Panaitopol se $p = \frac{x^4 - y^4}{x^3 + y^3}$ per alcuni numeri interi positivi $x$ e $y$.

Trova quanti primi di Panaitopol sono minori di $5 × {10}^{15}$.

# --hints--

`panaitopolPrimes()` dovrebbe restituire `4037526`.

```js
assert.strictEqual(panaitopolPrimes(), 4037526);
```

# --seed--

## --seed-contents--

```js
function panaitopolPrimes() {

  return true;
}

panaitopolPrimes();
```

# --solutions--

```js
// solution required
```
