...
Als Nächstes müssen Sie den aktuellen Songtitel und den Künstler im Player-Display anzeigen.

Greifen Sie zunächst auf die `#player-song-title`- und `#player-song-artist`-Elemente zu, die für die Anzeige des Songtitels bzw. des Künstlers verantwortlich sind, und speichern Sie sie in den Variablen `playingSong` bzw. `songArtist`.

# --hints--

Sie sollten eine Variable mit dem Namen `playingSong` haben.

```js
assert.isDefined(playingSong);
```

Ihre `playingSong`-Variable sollte den Wert des `#player-song-title`-Elements haben.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

Sie sollten eine Variable mit dem Namen `songArtist` haben.
...
Ihre `songArtist`-Variable sollte den Wert des `#player-song-artist`-Elements haben.
