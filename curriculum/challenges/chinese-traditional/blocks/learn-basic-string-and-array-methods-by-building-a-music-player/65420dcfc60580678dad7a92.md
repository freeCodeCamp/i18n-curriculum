...
在 `map()` 中，添加一個帶有反引號的 `return` 語句，你將在其中插入負責顯示歌曲詳細信息的所有元素。

在反引號內，創建一個 `li` 元素，其 `id` 屬性爲 `song-${song.id}`，`class` 屬性爲 `playlist-song`。

# --hints--

你應該添加一個帶有一對空反引號的 `return` 語句。
...
你的打開 `li` 標籤應將 `id` 屬性設置爲 `song-${song.id}`。

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

你的打開 `li` 標籤應將 `class` 屬性設置爲 `playlist-song`。
