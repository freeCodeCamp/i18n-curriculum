...
حان الوقت لتنفيذ الطريقة `solve`. عندما يكون المميز في المعادلة التربيعية سالبًا، لا تحتوي المعادلة على حلول حقيقية أو جذور.

داخل الطريقة `solve`، أنشئ جملة `if` للتحقق مما إذا كان المميز سالبًا. إذا كان كذلك، أرجع قائمة فارغة.

# --hints--

يجب أن تنشئ جملة `if` للتحقق مما إذا كانت `self.delta` أقل من `0`.
...
يجب أن تُرجع قائمة فارغة من جملة `if` الجديدة.
