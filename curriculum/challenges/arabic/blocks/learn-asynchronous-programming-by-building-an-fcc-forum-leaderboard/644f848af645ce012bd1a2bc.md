...
أنشئ ثابتًا اسمه `linkText` وقم بتعيين قيمة `selectedCategory.category` له. سيعرض هذا اسم الفئة في عنصر وسم الرابط.

# --hints--

يجب أن يكون لديك ثابت اسمه `linkText`.

```js
assert(code.match(/const\s+linkText\s*=/));
```

يجب تعيين `selectedCategory.category` إلى متغير `linkText` الخاص بك.
