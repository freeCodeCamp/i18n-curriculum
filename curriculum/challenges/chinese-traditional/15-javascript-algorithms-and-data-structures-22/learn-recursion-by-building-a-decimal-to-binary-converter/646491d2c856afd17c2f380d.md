...
由於你在 `addElDelay` 中存儲了每個動畫幀的時間，因此你可以將該值與 `setTimeout()` 一起使用來設置將元素添加到 DOM 的延遲。

在 `.forEach()` 方法的回調函數主體內，添加一個 `setTimeout()` 函數。 傳入一個空的回調函數作爲第一個參數，並傳入 `obj.addElDelay` 作爲第二個參數。

# --hints--

你應該在 `.forEach()` 方法的回調函數主體中添加 `setTimeout()` 函數。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\([\s\S]+\{\s*setTimeout\(/);
```

你應該將一個空的回調函數作爲第一個參數傳遞給 `setTimeout()` 函數。
