...
コンピューターが数値を保存し処理する方法のため、小数を含む計算では奇妙な動作が起こることがあります。例えば、`0.1 + 0.2` は `0.3` と等しくありません。これは、コンピューターが小数を2進分数として保存しており、一部の2進分数は正確に10進分数で表せないためです。

計算結果の数値をきれいにしたいです。計算式を丸括弧で囲み（`return` の文は含めないでください）、その上で `.toFixed()` メソッドを呼び出してください。`.toFixed()` メソッドには数値 `2` を引数として渡してください。これにより、小数点以下2桁に丸められ、文字列が返されます。
...
計算式に対して `.toFixed()` メソッドを呼び出してください。
...
`.toFixed()` メソッドには数値 `2` を引数として渡してください。
