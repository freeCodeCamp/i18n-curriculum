...
Zurück in Ihrem Ereignis-Listener müssen Sie den Text des `result`-Elements aktualisieren. Sie können einen ternären Operator verwenden, um diese Aufgabe zu erledigen.

Hier ist ein Beispiel, wie Sie das Ergebnis eines ternären Operators dem Textinhalt eines Elements zuweisen:

```js
el.textContent = condition ? "Use this text if the condition is true" : "Use this text if the condition is false";
```

Nach der `if`-Anweisung verwenden Sie einen ternären Operator, um die Wahrheit des Aufrufs von `isSpam()` mit `messageInput.value` als Argument zu überprüfen. Wenn wahr, setzen Sie die `textContent`-Eigenschaft des `result`-Elements auf `"Oh no! This looks like a spam message."`. Andernfalls setzen Sie sie auf `"This message does not seem to contain any spam."`.

Setzen Sie dann die `value`-Eigenschaft des `messageInput`-Elements auf einen leeren String.

# --hints--

Sie sollten den Zuweisungsoperator verwenden, um die `textContent`-Eigenschaft des `result`-Elements zu setzen.
...
Sie sollten die `isSpam()`-Funktion nach dem Zuweisungsoperator `=` und vor dem ternären Operator `?` aufrufen.
...
Sie sollten die ternäre Syntax verwenden, um die Wahrheit von `isSpam(messageInput.value)` zu überprüfen.
...
Der wahre Ausdruck Ihres ternären Operators sollte die `textContent`-Eigenschaft des `result`-Elements auf `"Oh no! This looks like a spam message."` setzen.
...
Der falsche Ausdruck Ihres ternären Operators sollte die `textContent`-Eigenschaft des `result`-Elements auf `"This message does not seem to contain any spam."` setzen.
...
Nach Ihrem ternären Operator setzen Sie die `value`-Eigenschaft des `messageInput`-Elements auf einen leeren String.
