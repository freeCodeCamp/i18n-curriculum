...
لتخزين الوقت الحالي للأغنية عند إيقافها مؤقتًا، عيّن `songCurrentTime` في كائن `userData` إلى `currentTime` في المتغير `audio`.

*ملاحظة*: لا يجب استخدام التسلسل الاختياري في هذه الخطوة لأن `userData.songCurrentTime` لن يكون `null` أو `undefined` في هذه المرحلة.

# --hints--

لا تعدل دالة `pauseSong` الحالية ومحتواها.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

عيّن `audio.currentTime` إلى `userData.songCurrentTime`.
