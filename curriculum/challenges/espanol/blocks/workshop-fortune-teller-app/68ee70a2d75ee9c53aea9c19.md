...
Dentro de `map`, devuelve la función `renderCard` pasando `type`, `isReversed`, `name_short` y `img` como argumentos, y añade el método `join` sobre `map` con una cadena vacía.

# --hints--

Debes devolver la función `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

Debes pasar `type` como primer argumento en la función `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

Debes pasar `isReversed` como segundo argumento en la función `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?isReversed\s*/);
```

Debes pasar `card.name_short` como argumento en la función `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*/);
```

Debes pasar `card.img` como argumento en la función `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

Debes adjuntar `join("")` al método `map`.
