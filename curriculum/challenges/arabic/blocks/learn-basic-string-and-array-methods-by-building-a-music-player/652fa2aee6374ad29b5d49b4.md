...
احصل على العناصر `#next` و `#previous` و `#shuffle` من ملف HTML.

عيّنها إلى متغيرات باسم `nextButton` و `previousButton` و `shuffleButton` على التوالي.

# --hints--

يجب أن يكون لديك متغير اسمه `nextButton`.

```js
assert.isDefined(nextButton);
```

استخدم `document.getElementById()` للحصول على العنصر `#next`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

عيّن العنصر `#next` إلى المتغير `nextButton`.

```js
assert.equal(nextButton, document.getElementById('next'));
```

يجب أن يكون لديك متغير اسمه `previousButton`.

```js
assert.isDefined(previousButton);
```

استخدم `document.getElementById()` للحصول على العنصر `#previous`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

عيّن العنصر `#previous` إلى المتغير `previousButton`.

```js
assert.equal(previousButton, document.getElementById('previous'));
```

يجب أن يكون لديك متغير اسمه `shuffleButton`.

```js
assert.isDefined(shuffleButton);
```

استخدم `document.getElementById()` للحصول على العنصر `#shuffle`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

عيّن العنصر `#shuffle` إلى المتغير `shuffleButton`.
