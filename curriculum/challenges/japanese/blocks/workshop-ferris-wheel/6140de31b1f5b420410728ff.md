...
今度は `@keyframes wheel` ルールに `100%` セレクターを付けてください。その中で、`transform` を `rotate(360deg)` に設定します。こうすることで、アニメーションが完全な回転を完了します。

# --hints--

`@keyframes wheel` ルールには `100%` セレクターを付ける必要があります。
...
`100%` セレクターには `transform` プロパティを `rotate(360deg)` に設定してください。
