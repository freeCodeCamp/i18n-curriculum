...
لجعل نص `legend` أكثر بروزًا، عليك تنسيقه.

اختر عنصر `fieldset legend`. عيّن `font-size` إلى `1.3rem` و`font-weight` إلى `600`.

`font-weight` يتحكم في عِرض النص.

القيم الشائعة لـ `font-weight` هي:

- `normal` → وزن عادي (افتراضي)
- `bold` → نص عريض
- `lighter` → أخف من العنصر الأصل
- `bolder` → أعرض من العنصر الأصل
- قيم رقمية مثل:
  - `100` (رفيع)
  - `400` (عادي)
  - `700` (عريض)
...
يجب أن يكون لديك محدِّد `fieldset legend`.

```js
assert(new __helpers.CSSHelp(document).getStyle('fieldset legend'));
```

عيّن خاصية `font-size` إلى `1.3rem`.
...
عيّن خاصية `font-weight` إلى `600`.
