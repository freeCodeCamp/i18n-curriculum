...
У JSX і TSX ви передаєте JavaScript-вирази як значення атрибутів, використовуючи фігурні дужки. Шаблонні літерали використовують поділки і `${}` для інтерполяції. Наприклад:

```tsx
<div className={`prefix-${someVariable}`}>
```

Обгортка картки `className` наразі жорстко закодована як `card-wrapper tier-gold`. Замініть `gold` на динамічне значення, використовуючи шаблонний літерал, який викликає `getPlayerTier` з `player.overallRating`. Це посилатиметься на правильний клас tier із CSS, залежно від загального рейтингу гравця.

# --hints--

Ви повинні використати шаблонний літерал з `getPlayerTier(player.overallRating)` для `className` `.card-wrapper`.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

Елемент `.card-wrapper` має мати клас `tier-elite`, коли рейтинг гравця дорівнює 98.
