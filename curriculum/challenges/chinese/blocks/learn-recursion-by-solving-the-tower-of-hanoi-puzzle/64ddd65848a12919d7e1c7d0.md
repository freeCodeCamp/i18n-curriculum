...
现在是时候将一些代码从 `move()` 函数移动到 `make_allowed_move()` 函数了。 将嵌套在第一个 `if` 语句内的代码（第一个 `print()` 调用除外）移动到新函数中。 密切注意缩进。 不要忘记删除 `pass` 关键字。

# --hints--

你应该将嵌套在第一个 `if` 语句内的代码（第一个 `print()` 调用除外）移动到新函数中。
