...
ضع عنصر `.penguin-head` على بعد `10%` من الأعلى، و`25%` من اليسار بالنسبة لعنصره الأصل.

# --hints--

يجب أن تعطي `.penguin-head` خاصية `top`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.top);
```

يجب أن تعطي `.penguin-head` خاصية `top` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.top, '10%');
```

يجب أن تعطي `.penguin-head` خاصية `left`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.left);
```

يجب أن تعطي `.penguin-head` خاصية `left` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
