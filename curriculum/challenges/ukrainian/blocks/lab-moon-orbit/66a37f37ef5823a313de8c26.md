...
14. Селектор `.earth` повинен мати колір фону та `border-radius` зі значенням `50%`.

15. Селектор `.moon` повинен мати колір фону та `border-radius` зі значенням `50%`.

16. Визначте анімацію `@keyframes orbit`, що обертатиме елемент `.orbit` на 360 градусів навколо центру. Застосуйте цю анімацію до елемента `.orbit`, задавши тривалість `5` секунд, лінійну часову функцію та нескінченну кількість ітерацій.
...
У межах `div` з класом зі значенням `space` має бути `div` з `class` зі значенням `earth`.
...
Елемент `.orbit` має бути після елемента `.earth`.
