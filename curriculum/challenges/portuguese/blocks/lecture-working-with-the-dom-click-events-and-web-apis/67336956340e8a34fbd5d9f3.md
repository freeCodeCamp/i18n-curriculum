...
Agora, vamos dar uma olhada em outro exemplo. O HTML para este exemplo é um elemento `div` com o texto `freeCodeCamp is Awesome`. O CSS transforma o `div` em um retângulo e oculta qualquer coisa que saia da viewport à esquerda ou à direita. O código JavaScript move o retângulo `2px` para a direita a cada chamada de `requestAnimationFrame(animate)` com a função `animate` como callback:
