...
한 가지 더 있습니다. `Load More Authors` 버튼을 계속 클릭해서 더 이상 불러올 데이터가 없고 텍스트가 `"No more data to load"`으로 바뀌었을 때도 커서 값은 여전히 `pointer`입니다. 왜 커서 값을 대신 `not-allowed`으로 바꾸지 않을까요?
...
점 표기법으로 `style`의 `loadMoreBtn` 속성에 접근해야 합니다.

```js
assert.match(code, /loadMoreBtn\.style/)
```

점 표기법으로 `cursor` 속성에서 `style`에 접근해야 합니다.

```js
assert.match(code, /loadMoreBtn\.style\.cursor/)
```

`cursor` 속성의 값을 `"not-allowed"`으로 설정해야 합니다.
