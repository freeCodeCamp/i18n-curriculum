...
各 `.eye` 要素の中に、`div` の `class` を持つ `eye-lid` を追加してください。

# --hints--

`div` の中に `.eye.left` 要素を1つ追加する必要がありますが、`--fcc-actual--` が見つかりました。

```js
assert.equal(document.querySelectorAll('.eye.left > div')?.length ?? 0, 1);
```

`div` の中に `.eye.right` 要素を1つ追加する必要がありますが、`--fcc-actual--` が見つかりました。

```js
assert.equal(document.querySelectorAll('.eye.right > div')?.length ?? 0, 1);
```

最初の新しい `div` に `class` の `eye-lid` を付けてください。

```js
assert.exists(document.querySelector('.eye.left > div.eye-lid'));
```

2番目の新しい `div` に `class` の `eye-lid` を付けてください。
