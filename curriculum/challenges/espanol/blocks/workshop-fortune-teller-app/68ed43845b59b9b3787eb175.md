...
Agrega un `constructor` dentro de la clase. En el constructor, necesitas usar los elementos que creaste antes para seleccionar los elementos DOM correspondientes usando la función `getElement`.

Por ahora, maneja `singleCardBtn` y `singleCard`. Para `singleCardBtn`, pasa `#btn-single-card`, y para `singleCard`, pasa el `.single_card`.

# --hints--

La clase `Game` debería tener un `constructor`.
...
Tu constructor debería asignar `this.elements`.
...
`singleCardBtn` debería establecerse usando `getElement` con `'#btn-single-card'`.
...
`singleCard` debería establecerse usando `getElement` con `'.single_card'`.
