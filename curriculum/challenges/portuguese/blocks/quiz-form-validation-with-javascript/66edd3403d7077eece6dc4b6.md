...
Retorna `false` sem mostrar nenhuma mensagem de erro para o usuário.
...
Qual ação não faz com que um formulário HTML seja verificado quanto a erros?
...
O que significa quando `patternMismatch` é `true`?
...
O valor não corresponde ao atributo `pattern`.

### --question--

#### --text--

O que acontece se `reportValidity()` for chamado, mas um campo falhar na validação?
...
Qual ação dispara a validação embutida do formulário pelo navegador?
...
Ele envia os dados do formulário para o servidor especificado na ação do formulário.
