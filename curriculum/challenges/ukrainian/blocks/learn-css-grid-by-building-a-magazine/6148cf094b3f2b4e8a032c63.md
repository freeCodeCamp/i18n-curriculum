...
Псевдо-селектор `::first-letter` дозволяє націлитися на першу літеру у текстовому вмісті елемента.

Створіть селектор `.first-paragraph::first-letter` і встановіть властивість `font-size` у значення `6rem`. Також додайте йому властивість `color`, встановлену у `orangered`, щоб він виділявся.

# --hints--

У вас має бути селектор `.first-paragraph::first-letter`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

Ваш селектор `.first-paragraph::first-letter` має мати властивість `font-size`, встановлену у `6rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

Ваш селектор `.first-paragraph::first-letter` має мати властивість `color`, встановлену у `orangered`.
