...
你需要维护一个你的应用程序支持的单位列表。

声明一个 `units` 数组，并赋予它字串值 `"cup"`、`"gram"`、`"ounce"` 和 `"teaspoon"`。
...
你的 `units` 数组应包含字串 `"cup"`。

```js
assert.include(units, "cup");
```

你的 `units` 数组应包含字串 `"gram"`。

```js
assert.include(units, "gram");
```

你的 `units` 数组应包含字串 `"ounce"`。

```js
assert.include(units, "ounce");
```

你的 `units` 数组应包含字串 `"teaspoon"`。
