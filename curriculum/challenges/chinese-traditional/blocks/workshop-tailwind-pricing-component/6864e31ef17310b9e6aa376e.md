...
網格項目距離頁面左右邊緣太近，所以你應該調整間距。

將類別 `max-w-6xl` 指定給 `div`，以設定固定寬度為 `72rem` 並防止項目在頁面上過度延伸。

由於那會將所有項目推向左側，請為 `div` 添加 `mx-auto` 以水平置中項目。

# --hints--

你的 `div` 元素應該有 `max-w-6xl` 類別。
