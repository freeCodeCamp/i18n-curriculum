...
在 `.foreground-buildings` 中巢狀放置六個 `div` 元素，並依序為它們添加 `fb1` 到 `fb6` 的類別。 "fb" 代表 "foreground building"。這些將是前景的另外六棟建築。

# --hints--

你應該創建一個帶有 `fb1` 類別的新的 `div`。

```js
assert.exists(document.querySelector('div.fb1'));
```

你應該創建一個帶有 `fb2` 類別的新的 `div`。

```js
assert.exists(document.querySelector('div.fb2'));
```

你應該創建一個帶有 `fb3` 類別的新的 `div`。

```js
assert.exists(document.querySelector('div.fb3'));
```

你應該創建一個新的 `div`，並且類別為 `fb4`。

```js
assert.exists(document.querySelector('div.fb4'));
```

你應該創建一個新的 `div`，其類別為 `fb5`。

```js
assert.exists(document.querySelector('div.fb5'));
```

你應該創建一個帶有 `fb6` 類別的新 `div`。

```js
assert.exists(document.querySelector('div.fb6'));
```

你應該將這些新的 `div` 元素放置在 `.foreground-buildings` 元素內。
