...
`attack` 関数内で、`monsterHealth` 変数を更新するコード行を見つけて、`if` 関数を呼び出す条件付きの `isMonsterHit` ブロック内に配置してください。
...
`if` 文は条件内で `isMonsterHit` を呼び出す必要があります。

```js
assert.match(attack.toString(), /if\s*\(\s*isMonsterHit\s*\(\s*\)/)
```

`monsterHealth -= weapons[currentWeaponIndex].power + Math.floor(Math.random() * xp) + 1` を `if` ブロックに配置してください。
