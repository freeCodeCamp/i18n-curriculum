...
У інструкції `if` деструктуруйте `className` та `category` з об’єкта `allCategories[id]`.

# --hints--

Ви повинні деструктурувати `className` та `category` з об’єкта `allCategories[id]`. Властивості можуть бути в будь-якому порядку.
