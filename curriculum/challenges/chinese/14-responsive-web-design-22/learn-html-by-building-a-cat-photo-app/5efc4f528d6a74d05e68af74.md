...
You should add a new `label` element after the checkbox element. 确保它有一个开始标签和一个结束标签。
...
Your `label` element should have a `for` attribute with the value `loving`.  应该是 `--fcc-expected--`，而不是 `--fcc-actual--`。
