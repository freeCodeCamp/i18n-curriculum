...
Erstellen Sie ein weiteres `p`-Element unter Ihrem `.first-paragraph`-Element und geben Sie ihm den folgenden Text:
...
Sie sollten ein zweites `p`-Element innerhalb Ihres `.text`-Elements erstellen.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 2);
```

Ihr zweites `p`-Element sollte den angegebenen Text enthalten.
