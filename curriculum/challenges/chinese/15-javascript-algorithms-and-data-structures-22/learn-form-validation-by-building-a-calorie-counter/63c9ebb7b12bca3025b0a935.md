...
是时候进行另一次循环了。 创建一个 `for...of` 循环，其中有一个名为 `container` 的变量，以遍历 `inputContainers` 数组。

在循环内部，将 `container` 的 `innerHTML` 属性设置为空字符串。 这将清除该输入容器的所有内容。

# --hints--

你的 `for...of` 循环应该遍历 `inputContainers` 数组。 循环的变量名应该是 `container`。
...
你的 `for...of` 循环应将 `container` 的 `innerHTML` 属性设置为空字符串。
