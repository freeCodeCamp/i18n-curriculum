...
Entferne die `differences`, `squaredDifferences` und `sumSquaredDifferences`-Variablen (und deren Werte). Deklariere eine `variance`-Variable und weise ihr den Wert von `array.reduce()` zu. Gib für den Callback deine `acc`- und `el`-Standardparameter ein, aber lass den Funktionskörper vorerst leer. Denke daran, den Ausgangswert auf `0` zu setzen.

# --hints--

Du solltest die `differences`-Variablen entfernen.

```js
assert.notMatch(getVariance.toString(), /differences\s*=/);
```

Du solltest die `squaredDifferences`-Variablen entfernen.

```js
assert.notMatch(getVariance.toString(), /squaredDifferences\s*=/);
```

Du solltest die `sumSquaredDifferences`-Variablen entfernen.

```js
assert.notMatch(getVariance.toString(), /sumSquaredDifferences\s*=/);
```

Du solltest eine `variance`-Variable haben.

```js
assert.match(getVariance.toString(), /variance\s*=/);
```

Deine `variance`-Variable sollte die `array.reduce()`-Methode verwenden.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(/);
```

Deine `variance`-Variable sollte die `acc`- und `el`-Parameter in der Callback-Funktion verwenden.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)/);
```

Deine `reduce`-Callback sollte eine leere Funktion sein.
