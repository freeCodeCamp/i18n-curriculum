...
`useMemo()` 接受兩個引數：一個傳回你想要記憶的值的函式，以及一個依賴的陣列。只有當一個或多個依賴改變時，記憶的值才會重新計算。
...
將 `filteredItems` 設為等於 `useMemo()`，並傳入一個帶有大括號的匿名函式（程式）。在大括號內，使用 `return` 述語明確地傳回你現有的篩選邏輯。並且，將 `query` 作為相依陣列中唯一的相依項添加。這確保篩選操作只會在 `query` 變更時重新執行。

# --hints--

你應該將 `filteredItems` 設定為呼叫 `useMemo()` 掛鉤的結果。
