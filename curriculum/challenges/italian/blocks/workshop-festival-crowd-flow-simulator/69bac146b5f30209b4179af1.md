...
Ora puoi creare una funzione per simulare il festival. Crea una funzione vuota chiamata `simulateFestival` con i parametri `gates` e `timeBlock`.

# --hints--

Dovresti creare una funzione chiamata `simulateFestival`.

```js
assert.isFunction(simulateFestival);
```

La tua funzione `simulateFestival` dovrebbe avere i parametri `gates` e `timeBlock`.
