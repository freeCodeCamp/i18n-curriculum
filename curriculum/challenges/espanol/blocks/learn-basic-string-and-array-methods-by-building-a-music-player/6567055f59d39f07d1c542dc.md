...
A continuación, necesitarás actualizar la lista de reproducción en tu documento HTML para mostrar las canciones.

Asigna `songsHTML` a la propiedad `innerHTML` del elemento `playlistSongs`. Esto insertará el elemento `li` que acabas de crear en el elemento `ul` en el archivo `HTML`.

# --hints--

Debes usar la propiedad `innerHTML` para añadir `songsHTML` a `playlistSongs`.
