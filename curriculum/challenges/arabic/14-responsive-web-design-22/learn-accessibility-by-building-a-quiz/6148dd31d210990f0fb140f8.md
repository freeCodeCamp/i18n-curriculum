...
من المفيد رؤية الحدود الافتراضية حول عناصر `fieldset` أثناء التطوير. ومع ذلك، قد لا يكون هذا هو الأسلوب الذي تريده.

قم بإزالة border و bottom padding من عناصر `.question`.

# --hints--

يجب عليك استخدام منتقي `.question`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

يجب أن تعطي `.question` خاصية `border` بقيمة `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

يجب عليك إعطاء `.question` خاصية `padding-bottom` بقيمة `0`.
