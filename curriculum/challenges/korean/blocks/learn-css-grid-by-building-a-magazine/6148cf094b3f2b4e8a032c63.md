...
`::first-letter` 의사선택자를 사용하면 요소의 텍스트 내용에서 첫 글자를 선택할 수 있습니다.

`.first-paragraph::first-letter` 선택자를 만들고 `font-size` 속성을 `6rem` 으로 설정하세요. 또한 눈에 띄게 하려면 `color` 속성을 `orangered` 로 설정하세요.

# --hints--

`.first-paragraph::first-letter` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

`.first-paragraph::first-letter` 선택자에 `font-size` 속성이 `6rem` 로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

`.first-paragraph::first-letter` 선택자에 `color` 속성이 `orangered` 로 설정되어야 합니다.
