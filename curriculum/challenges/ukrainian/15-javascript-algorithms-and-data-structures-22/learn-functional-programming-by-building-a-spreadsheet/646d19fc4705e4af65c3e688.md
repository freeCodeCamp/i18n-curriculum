...
Функція `addCharacters` зрештою повертає діапазон символів. Ви хочете, щоб вона повертала масив ідентифікаторів клітинок. З’єднайте метод `.map()` з викликом `charRange()`. Ще не передавайте функцію зворотного виклику.

# --hints--

Створіть ланцюжок з `.map()` до виклику `charRange()`.
...
Нічого не передавайте у виклик `.map()`.
