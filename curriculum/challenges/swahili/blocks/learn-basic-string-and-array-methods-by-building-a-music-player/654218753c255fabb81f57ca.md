...
Sasa unahitaji kuita kitendakazi cha `renderSongs` na kupitisha `userData?.songs` ili kuonyesha wimbo katika UI.

Uunganishaji wa mfululizo (`?.`) husaidia kuzuia makosa wakati wa kupata sifa zilizopangwa ndani ambazo zinaweza kuwa null au hazijafafanuliwa. Kwa mfano:
...
Ita kitendakazi cha `renderSongs` kwa sifa ya `songs` ya `userData`. Hii itaonyesha nyimbo katika orodha ya nyimbo.

# --hints--

Unapaswa kuita kitendakazi chako cha `renderSongs` na `userData?.songs`.
