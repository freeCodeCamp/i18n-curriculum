...
それぞれの `span` に、`className` と `stat-value` の `stat-label` を持つ `div.stat-row` 要素を2つ追加してください。

# --hints--

各 `.stat-row` は、`span` の `className` を持つ `stat-value` と、`span` の `className` を持つ `stat-label` を含む必要があります。
