...
既存の`repeating-linear-gradient`の下に`.fb1c`を追加してください。`--building-color4`から`0%`までの`10%`と、`--window-color4`から`10%`までの`90%`を使います。これにより、最後に追加したグラデーションの背後が埋まります。
...
`repeating-linear-gradient`から`--building-color4`まで、最初の色が`0%`の`10%`を追加してください。
...
`--window-color4`から`10%`まで、2番目の色が`90%`のものを使ってください。
