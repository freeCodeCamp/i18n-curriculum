...
Il badge del livello mostra ancora il testo hardcoded `GOLD`. Devi sostituirlo con un'espressione dinamica in modo che venga mostrato il livello corretto.

Chiama `getPlayerTier()` con `player.overallRating` per ottenere la stringa del livello, poi chiama `.toUpperCase()` sul risultato per visualizzarlo in maiuscolo. Non dimenticare di racchiudere l'intera espressione tra parentesi graffe.

# --hints--

Dovresti usare `getPlayerTier(player.overallRating).toUpperCase()` all'interno di `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

L'elemento `.card-tier-badge` dovrebbe mostrare `ELITE` per un giocatore con un punteggio di 98.
