...
最后一组要选择的元素是结果元素。使用你的 `getElement` 函数获取 id 为 `result-text` 和 `result-card` 的元素。两者都应该使用 `HTMLElement` 作为类型。

记住你已经为它们声明了 `resultText` 和 `resultCard` 变量，所以你只需要赋值即可。

# --hints--

你应该为 `resultText` 变量赋值。

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` 应该选择 `result-text` 元素。

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

你应该使用 `HTMLElement` 作为选择 `getElement` 元素时使用的 `result-text` 的类型。

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

你应该为 `resultCard` 变量赋值。

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` 应该选择 `result-card` 元素。

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

你应该使用 `HTMLElement` 作为选择 `getElement` 元素时使用的 `result-card` 的类型。
