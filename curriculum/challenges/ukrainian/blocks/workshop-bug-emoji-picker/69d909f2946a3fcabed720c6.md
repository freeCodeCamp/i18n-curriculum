...
Тепер, коли перевірка `if` на місці, використайте `e.target.value`, щоб знайти відповідну помилку в `bugMap` і викликати її метод `render`. Після цього ви зможете вибирати помилки та одразу бачити результат.

На цьому ваше практичне заняття з вибору емодзі помилок завершено!

# --hints--

Ви повинні викликати метод `render` на відповідній помилці з `bugMap`.
