...
在你的 `.heading` 元素下方，創建一個新的 `section` 元素，並將 `class` 設定為 `text`。在其中，創建一個 `p` 元素，並將 `class` 設定為 `first-paragraph`，內容為以下文字：
