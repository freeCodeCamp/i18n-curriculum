...
Dé a su `conversionTable` una propiedad `cup`, y asígnale un objeto con una propiedad `gram` establecida a `240`, una propiedad `ounce` establecida a `8.0`, y una propiedad `teaspoon` establecida a `48`.

# --hints--

Su `conversionTable` debería tener una propiedad `cup`.

```js
assert.property(conversionTable, "cup");
```

Su propiedad `cup` debería ser un objeto.

```js
assert.isObject(conversionTable.cup);
```

Su objeto `cup` debería tener una propiedad `gram`.

```js
assert.property(conversionTable.cup, "gram");
```

Su propiedad `gram` debería estar establecida en `240`.

```js
assert.strictEqual(conversionTable.cup?.gram, 240);
```

Su objeto `cup` debería tener una propiedad `ounce`.

```js
assert.property(conversionTable.cup, "ounce");
```

Su propiedad `ounce` debería estar establecida en `8.0`.

```js
assert.strictEqual(conversionTable.cup?.ounce, 8.0);
```

Su objeto `cup` debería tener una propiedad `teaspoon`.

```js
assert.property(conversionTable.cup, "teaspoon");
```

Su propiedad `teaspoon` debería estar establecida en `48`.
