...
Crea una nueva función flecha nombrada llamada `printMessage` que tenga un parámetro llamado `org`. Dentro del cuerpo de esa función, agrega una sentencia de consola. Dentro de esa sentencia de consola, agrega el literal de plantilla `${org} is awesome!`.
...
Tu sentencia de consola debe incluir el literal de plantilla `${org} is awesome!`.
...
Tu llamada a la función `printMessage` debería mostrar `freeCodeCamp is awesome!` en la consola.
