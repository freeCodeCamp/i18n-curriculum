...
في هذا المشروع ستستخدم البيانات من `https://cdn.freecodecamp.org/curriculum/labs/data/bar-chart/GDP-data.json` لبناء مخطط بالأعمدة.

تتطلب الاختبارات توليد المحاور باستخدام خاصية المحور في D3، التي تُولد تلقائيًا علامات دقيقة على طول المحور. هذه العلامات الدقيقة ضرورية لاجتياز اختبارات D3 لأن مواقعها تُستخدم لتحديد محاذاة العناصر المرسومة.

**الهدف:** حقق قصص المستخدم أدناه واجتز جميع الاختبارات لإكمال المختبر.

**قصص المستخدم:**

1. يجب أن يحتوي مخططك على عنوان مع `id="title"` المقابل.
1. يجب أن يحتوي مخططك على عنصر `g` لمحور x مع `id="x-axis"` المقابل.
1. يجب أن يحتوي مخططك على عنصر `g` لمحور y مع `id="y-axis"` المقابل.
1. يجب أن يحتوي كلا المحورين على عدة مسميات علامات دقيقة، كل منها مع `class="tick"` المقابل.
1. يجب أن يحتوي مخططك على عنصر `rect` لكل نقطة بيانات مع `class="bar"` المقابل لعرض البيانات.
1. يجب أن يحتوي كل `.bar` على الخاصيتين `data-date` و `data-gdp` اللتين تحتويان على قيم `date` و `GDP`.
1. يجب أن تتطابق خاصيات `data-date` لعناصر `.bar` مع ترتيب البيانات المقدمة.
1. يجب أن تتطابق خاصيات `data-gdp` لعناصر `.bar` مع ترتيب البيانات المقدمة.
1. يجب أن تمثل ارتفاعات عناصر `.bar` بدقة قيمة `GDP` المقابلة للبيانات.
1. يجب أن تتوافق خاصية `data-date` وعنصر `.bar` المقابل معها مع القيمة المقابلة على محور x.
1. يجب أن تتوافق خاصية `data-gdp` وعنصر `.bar` المقابل معها مع القيمة المقابلة على محور y.
1. يجب أن تتمكن من تحريك مؤشر الفأرة فوق منطقة ورؤية تلميح مع `id="tooltip"` المقابل يعرض مزيدًا من المعلومات عن المنطقة.
1. يجب أن يحتوي التلميح على خاصية `data-date` التي تتوافق مع `data-date` للمنطقة النشطة.
...
يجب أن يحتوي المخطط على عنصر يحمل `id` بقيمة `title` ليضم العنوان.

```js
assert.exists(document.getElementById('title'));
```

يجب أن يحتوي المخطط على عنصر `g` يمثل المحور الأفقي مع معرف مطابق `x-axis`.

```js
assert.isNotEmpty(document.querySelector('g#x-axis'))
```

يجب أن يحتوي المخطط على عنصر `g` للمحور y-axis مع معرف مطابق `y-axis`.

```js
assert.isNotEmpty(document.querySelector('g#y-axis'))
```

يجب أن يحتوي محور x على عدة مسميات علامات دقيقة، كل منها مع `class="tick"` المقابل.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

يجب أن يحتوي محور y على عدة مسميات علامات دقيقة، كل منها مع `class="tick"` المقابل.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

يجب أن يحتوي مخططك على عنصر `rect` لكل نقطة بيانات مع `class="bar"` المقابل لعرض البيانات.

```js
assert.lengthOf(document.querySelectorAll('rect.bar'), GDPDataJson.data.length);
```

يجب أن يحتوي كل عمود على الخاصيتين `data-date` و `data-gdp` اللتين تحتويان على قيم التاريخ وGDP.
...
يجب أن تتطابق خاصيات `data-date` لعناصر الأعمدة مع ترتيب البيانات المقدمة.
...
يجب أن تتطابق خاصيات `data-gdp` لعناصر الأعمدة مع ترتيب البيانات المقدمة.
...
يجب أن يمثل ارتفاع كل عنصر عمود بدقة قيمة GDP المقابلة للبيانات.
...
يجب أن تتوافق خاصية `data-date` وعنصر العمود المقابل معها مع القيمة المقابلة على محور x. يمكنك استخدام `d3.scaleTime()` لإنشاء مقياس زمني لمحور x.
...
يجب أن تتوافق خاصية `data-gdp` وعنصر العمود المقابل معها مع القيمة المقابلة على محور y. يمكنك استخدام `d3.scaleLinear()` لإنشاء مقياس خطي لمحور y.
...
عند تحريك مؤشر الفأرة فوق منطقة، يجب أن يحتوي مخطط الأعمدة على تلميح مع `id="tooltip"` المقابل يعرض مزيدًا من المعلومات عن المنطقة.
...
يجب أن يحتوي التلميح على خاصية `data-date` التي تتوافق مع `data-date` للمنطقة النشطة.
