...
باستخدام قائمة محددات (`selector1, selector2`) امنح كلًا من العنصرين `#controls` و `#stats` حدودًا (`border`) بقيمة `1px solid #0a0a23`، ولون نص `#0a0a23`، و`5px` من الحشو (`padding`).

# --hints--

يجب أن تحتوي على قائمة محدِّدات `#controls, #stats`.
...
يجب أن يحتوي محدِّد `#controls, #stats` على حدود `border` بقيمة `1px solid #0a0a23`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#controls, #stats')?.border, '1px solid rgb(10, 10, 35)');
```

يجب أن يحتوي محدِّد `#controls, #stats` على `color` بقيمة `#0a0a23`.
...
يجب أن يحتوي محدِّد `#controls, #stats` على حشو بمقدار `5px`.
