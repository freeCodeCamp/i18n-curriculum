...
Wenn Sie sich das HTML ansehen, werden Sie feststellen, dass das zweite und dritte Panel ausgeblendet sind. Dies geschieht mit dem `hidden`-Attribut. Das erste Panel ist standardmäßig sichtbar. Wenn ein Benutzer auf einen Tab klickt, sollten alle Panels ausgeblendet werden.

Verwenden Sie die `forEach`-Methode, um durch die `panels` zu iterieren und das `hidden`-Attribut bei jedem Panel auf `true` zu setzen.

# --hints--

Sie sollten durch die `panels` iterieren und bei jedem Panel das `hidden`-Attribut auf `true` setzen.
