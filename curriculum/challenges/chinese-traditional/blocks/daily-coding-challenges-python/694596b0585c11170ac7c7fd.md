...
給定一個考試成績（數字）陣列，根據以下對照表以字母等級的形式傳回平均成績：
...
透過將陣列中所有分數相加並除以分數總數來計算平均值。

# --hints--

`get_average_grade([92, 91, 90, 94, 89, 93])` 應該傳回 `"A-"`。
...
`get_average_grade([84, 89, 85, 100, 91, 88, 79])` 應該傳回 `"B+"`。
...
`get_average_grade([63, 69, 65, 66, 71, 64, 65])` 應該傳回 `"D"`。
...
`get_average_grade([97, 98, 99, 100, 96, 97, 98, 99, 100])` 應該傳回 `"A+"`。
...
`get_average_grade([75, 100, 88, 79, 80, 78, 64, 60])` 應該傳回 `"C+"`。
...
`get_average_grade([45, 48, 50, 52, 100, 54, 56, 58, 59])` 應該傳回 `"F"`。
