...
Agora, substitua o loop `for` e a declaração `if` que você adicionou na etapa anterior por uma declaração `if` que usa a função embutida `any()`.

# --hints--

A condição do seu novo `if` deve ser uma chamada para `any()`.
...
Você deve passar uma expressão geradora como argumento para sua chamada `any()`.
...
A expressão geradora passada para `any()` deve iterar sobre `args`.
...
Sua declaração `if` deve verificar se algum dos argumentos em `args` não é uma instância de `int` ou `float`.
...
Você deve usar a string fornecida para lançar um `TypeError` dentro do seu novo comando `if`.
