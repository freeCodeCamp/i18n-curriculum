...
재생 목록에 다음 노래가 없으면 `else` 블록을 사용해 `currentSong`의 `userData` 키를 `null`로, 그리고 그 `songCurrentTime` 속성을 `0`로 재설정하세요.
