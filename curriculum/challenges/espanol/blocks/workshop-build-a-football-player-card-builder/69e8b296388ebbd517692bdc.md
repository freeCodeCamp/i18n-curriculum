...
Agrega un atributo `value` de `player.sho` al SHO `input`. Luego asigna su atributo `onChange` a una función flecha que toma `e` como parámetro y llama a `setPlayer` con un spread de `player` y `sho` establecido en `Number(e.target.value)`.

# --hints--

Tu SHO `input` debe tener un `value` igual a la estadística `sho` del jugador actual.
...
Tu SHO `input` debe tener un manejador `onChange` que llame a `setPlayer` con un spread de `player` y `sho` establecido en `Number(e.target.value)`.
