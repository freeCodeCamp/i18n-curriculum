...
Abaixo do elemento `h1`, adicione um elemento `h2` com um `id` definido como `"subtitle"`. Você pode escrever qualquer texto que desejar para o `h2` e verá as alterações na janela de visualização. Para interagir com o exemplo, será necessário ativar o editor interativo.
...
**NOTA**: Alguns CSS foram fornecidos para você neste exemplo interativo. Não se preocupe em tentar entender o código CSS porque você aprenderá mais sobre isso em lições futuras. Mas se quiser ver a cor do texto mudar para azul, ative o editor interativo, clique na aba `styles.css` e altere o `color: red;` para `color: blue;`. 
...
**NOTA**: Alguns CSS foram fornecidos para você neste exemplo interativo. Não se preocupe em tentar entender o código CSS porque você aprenderá mais sobre isso em lições futuras. Mas se você quiser mudar a cor das primeiras e terceiras caixas, ative o editor interativo e clique na aba `styles.css` e mude o `background-color: red;` para `background-color: black;`.
