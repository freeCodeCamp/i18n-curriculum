...
色が変わりませんね。 変数を使用する箇所で「`var(--variable-name, fallback-value)`」のように 2 つ目の値を渡して、変数にフォールバック値を追加しましょう。 これで、変数に問題がある場合そのプロパティはフォールバック値を使うようになります。 `.bb2` の `background-color` の設定に、フォールバック値として `green` を追加してください。
