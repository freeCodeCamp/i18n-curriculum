...
Dentro de tu elemento `.image-quote`, anida un elemento `hr`, un elemento `p` y un segundo elemento `hr`. Asigna al elemento `p` una `class` establecida en `quote` y el texto `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.

# --hints--

Debes añadir dos elementos `hr` a tu elemento `.image-quote`.

```js
assert.lengthOf(document.querySelectorAll('.image-quote hr'), 2);
```

Debes añadir un elemento `p` a tu elemento `.image-quote`.

```js
assert.lengthOf(document.querySelectorAll('.image-quote p'), 1);
```

Los hijos de tu `.image-quote` deben estar en el orden correcto.
...
Tu nuevo elemento `p` debe tener una `class` establecida en `quote`.

```js
assert.isTrue(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Tu nuevo elemento `p` debe tener el texto `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
