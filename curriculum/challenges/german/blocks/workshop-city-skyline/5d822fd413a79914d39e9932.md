...
Weiter zum nächsten Gebäude! Es ist das grüne im Vordergrund. Geben Sie ihm eine `repeating-linear-gradient` mit Ihrer Gebäude-Farbe von `0%` bis `5%` und `transparent` von `5%` bis `10%`.

# --hints--

Sie sollten `.fb5` eine `background`-Eigenschaft geben.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

Sie sollten dem `background` eine `repeating-linear-gradient` geben.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Sie sollten dem `repeating-linear-gradient` eine erste Farbe von `--building-color2` von `0%` bis `5%` geben.
...
Sie sollten dem `repeating-linear-gradient` eine zweite Farbe von `transparent` von `5%` bis `10%` geben.
