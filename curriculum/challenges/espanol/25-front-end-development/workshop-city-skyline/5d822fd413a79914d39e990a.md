...
Mueve las propiedades y valores de `display`, `flex-direction` y `align-items` de `.bb1` a la nueva clase `building-wrap`.

# --hints--

Debes eliminar `display` de `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.display);
```

Debes mover `display` con un valor de `flex` a `.building-wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.display, "flex");
```

Debes eliminar `flex-direction` de `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.flexDirection);
```

Debes mover `flex-direction` con un valor de `column` a `.building-wrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".building-wrap")?.flexDirection, "column");
```

Debes eliminar `align-items` de `.bb1`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb1")?.alignItems);
```

Debes mover `align-items` con un valor de `center` a `.building-wrap`.
