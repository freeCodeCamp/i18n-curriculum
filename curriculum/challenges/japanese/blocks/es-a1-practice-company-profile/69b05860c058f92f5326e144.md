...
この練習タスクはリスニング理解力をチェックします。会社について言及されている重要な事実にフォーカスしてください。
...
よく聞いてください。その数字はモノローグの別のことを指しています。
...
モノローグでは2つの異なる数字が言及されています。1つは従業員数を指し、もう1つは部署数を指しています。これらの詳細を理解するのに役立つ重要な語彙と構文を復習しましょう。例えば：

- `Tiene setenta y tres empleados.` - 従業員が73人います。
- `Tiene ocho departamentos.` - 部署が8つあります。

`Tiene` は所有を示す動詞で、モノローグに基づくと、その後に続く数字が従業員数または部署数の数量を指定しています。
