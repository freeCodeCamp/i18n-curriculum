...
Цикл завершено. Поверніть `grouped` з функції.

Потім, під функцією, викличте `groupByDecade(catalog)` і призначте результат змінній з ім’ям `byDecade`. Виведіть у консоль `byDecade`, щоб перевірити згрупований результат, і ви побачите об’єкт із ключами на кшталт `"1950s"`, `"1960s"`, `"1970s"` тощо, кожен з яких вказує на масив книг з того десятиліття.

# --hints--

`groupByDecade` має повертати об’єкт.

```js
assert.isObject(groupByDecade(catalog));
```

`byDecade["1980s"].length` має бути `5`.

```js
assert.strictEqual(groupByDecade(catalog)["1980s"].length, 5);
```

`byDecade["Unknown"].length` має бути `1`.

```js
assert.strictEqual(groupByDecade(catalog)["Unknown"].length, 1);
```

Ви маєте призначити результат `groupByDecade(catalog)` змінній з ім’ям `byDecade`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+byDecade\s*=\s*groupByDecade\s*\(\s*catalog\s*\)/);
```

Ви маєте вивести `byDecade` у консоль.
