...
如果用戶擲出三個相同的點數和另外兩個相同的點數，這叫作葫蘆（或滿堂紅，英文爲 full house）。 聲明一個接受單個參數的 `detectFullHouse` 函數。 函數調用時將傳遞 `diceValuesArr` 數組。

你的 `detectFullHouse` 函數應檢查用戶是否擲出了三個相同的點數和兩個相同的點數。 如果是這樣，它應更新第三個單選按鈕以正確的屬性顯示得分 `25`。 無論如何，它應該始終更新最後一個單選按鈕以顯示分數 0，並具​​有正確的屬性。

不要忘記在擲骰子時調用你的新函數。

# --hints--

你應該有一個 `detectFullHouse` 函數。

```js
assert.isFunction(detectFullHouse);
```

你的 `detectFullHouse` 函數應該有一個參數。

```js
assert.match(code, /(?:var|let|const)\s+detectFullHouse\s*=\s*(\(\s*\w+\s*\)|\s*\w+\s*)\s*=>\s*|function\s+detectFullHouse\s*\(\s*\w+\s*\)/);
```

當擲出一個葫蘆時，你的 `detectFullHouse` 函數應啓用第三個單選按鈕，將其值設爲 `25`，並設置第三個 `span` 顯示文本 `, score = 25`。
...
當沒有擲出葫蘆時，你的 `detectFullHouse` 函數應啓用最後一個單選按鈕，將其值設爲 `0`，並設置最後一個 `span` 顯示文本 `, score = 0`。
...
當點擊 `rollDiceBtn` 時，你應該調用你的 `detectFullHouse` 函數。
