...
Aggiungi un terzo oggetto con le seguenti proprietà e valori:
...
Il tuo array `allSongs` dovrebbe avere un oggetto come terzo valore.

```js
assert.isObject(allSongs[2]);
```

Il terzo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `id` impostata sul numero `2`.

```js
assert.equal(allSongs[2].id, 2);
```

Il terzo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `title` impostata sulla stringa `"Still Learning"`.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

Il terzo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `artist` impostata sulla stringa `"Quincy Larson"`.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

Il terzo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `duration` impostata sulla stringa `"3:51"`.

```js
assert.equal(allSongs[2].duration, "3:51");
```

Il terzo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `src` impostata sulla stringa `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`.
