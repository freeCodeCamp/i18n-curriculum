...
Después de tu bucle `for` anidado, has encontrado el valor más pequeño. Necesitas intercambiarlo con el valor actual.

Como hiciste en tu ordenación de burbuja, usa una variable `temp` para extraer el valor en `array[i]`, luego intercambia los valores en `array[i]` y `array[minIndex]`.

# --hints--

Después de tu bucle `for` anidado, deberías declarar una variable `temp`.
...
Deberías asignar `array[i]` a `temp`.
...
Deberías asignar `array[minIndex]` a `array[i]`.
...
Deberías asignar `temp` a `array[minIndex]`.
