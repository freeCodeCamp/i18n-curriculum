...
Verwenden Sie nun das `onload`-Ereignis, um die `hidden`-Klasse zu entfernen, wenn das Bild geladen wird, und verstecken Sie den Ladeplatzhalter mithilfe seines `previousElementSibling`.

# --hints--

Ihr `img`-Element sollte einen `onload`-Handler haben.
...
Ihr `onload`-Handler sollte `classList.remove` verwenden, um die `hidden`-Klasse zu entfernen. Vergessen Sie nicht, dass Sie hier das Schlüsselwort `this` benötigen.
...
Ihr `onload`-Handler sollte `previousElementSibling.style.display` auf `'none'` setzen. Vergessen Sie nicht, dass Sie dafür das Schlüsselwort `this` benötigen.
