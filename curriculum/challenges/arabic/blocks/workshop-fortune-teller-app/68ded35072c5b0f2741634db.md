...
تحتاج إلى دالة أخرى تعرض العناصر في DOM. مثل الدالة السابقة، أنشئ دالة باسم `showElements`. هذه المرة استخدم `classList.remove("hidden")`.

# --hints--

يجب أن يكون لديك دالة باسم `showElements`.
...
يجب أن تحتوي دالة `showElements` على معلمة باسم `...elements` من النوع `HTMLElement[]`.
...
يجب أن تحتوي دالة `showElements` على طريقة `forEach` مرتبطة بمعلمة `elements`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

يجب أن تأخذ طريقة `forEach` معلمة باسم `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

يجب إزالة الصنف `hidden` باستخدام طريقة `classList.remove` داخل طريقة `forEach`.
