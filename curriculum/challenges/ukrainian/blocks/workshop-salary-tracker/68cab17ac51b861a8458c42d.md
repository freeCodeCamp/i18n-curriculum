...
Нарешті, тепер, коли ви завершили сеттер `salary`, ви можете викликати його всередині сеттера `level`.

Змініть лінію `self._salary = Employee._base_salaries[new_level]` так, щоб вона викликала сеттер `salary` замість прямої зміни атрибута `_salary`.

На цьому практичне заняття зі слідкування за зарплатою завершене.

# --hints--

Ви повинні встановити `self.salary` у `Employee._base_salaries[new_level]` всередині сеттера `level`.
