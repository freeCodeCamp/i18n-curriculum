...
선택 정렬은 배열에서 가장 작은 값의 인덱스를 추적하는 데 의존합니다. `minIndex` 변수를 선언하고 `i`로 설정하세요 — 이렇게 하면 현재 값이 가장 작을 경우 자기 자신과 교환되어 이동하지 않습니다. 배열을 순차적으로 반복하면서 `minIndex`의 값을 다시 할당할 수 있어야 합니다.

그다음, `for` 반복문을 하나 더 작성하세요. `j`를 반복자로 사용합니다. 이 반복문은 `i` 다음 인덱스에서 시작해 배열의 나머지 부분을 순차적으로 반복해야 합니다.

# --hints--

`let`를 사용해 `minIndex`을 선언하세요.
...
`for` 반복문은 배열의 나머지 부분을 순차적으로 반복해야 합니다.
