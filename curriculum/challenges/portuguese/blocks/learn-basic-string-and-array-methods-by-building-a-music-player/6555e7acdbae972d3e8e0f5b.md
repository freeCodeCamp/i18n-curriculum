...
Adicione um listener de evento `"click"` ao elemento `shuffleButton`.Para a função executar, passe a função `shuffle`.

**Nota**: Você não precisa de um callback dentro deste listener de evento específico.Você também não precisa chamar a função `shuffle`, apenas passe seu identificador.

# --hints--

Você deve encadear o método `addEventListener()` ao `shuffleButton`.
...
Você deve passar `shuffle` como o segundo valor do seu listener de evento `"click"`.
