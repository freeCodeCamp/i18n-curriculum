...
`showLatestPosts()` 関数を組み立てていく際の変更が見えるように、この関数を呼び出しましょう。

`try` ブロックの最後で、`showLatestPosts()` 関数を呼び出して、実引数として `data` を渡してください。

# --hints--

`showLatestPosts()` を `try` ブロックの末尾で呼び出してください。

```js
assert.match(code, /\s*showLatestPosts\(.*\)\s*;?\s*/);
```

`showLatestsPosts()` の引数として `data` を渡してください。
