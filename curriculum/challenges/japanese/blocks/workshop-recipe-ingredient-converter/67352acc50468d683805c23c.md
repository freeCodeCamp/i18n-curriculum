...
上記の構文に従って、`convertQuantity` 関数を更新し、別の関数を返すようにしてください。この内部関数は `toUnit` 引数を受け取り、空の状態にしてください。

# --hints--

あなたの `convertQuantity` 関数は関数を返す必要があります。

```js
assert.isFunction(convertQuantity());
```

返される関数は `toUnit` パラメータを持つ必要があります。
