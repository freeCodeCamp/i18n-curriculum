...
أنشئ محدِّد `.author` وأعطه خاصية `font-size` بقيمة `2rem` وخاصية `font-family` بقيمة `Raleway` مع نظام بديل `sans-serif`.
...
يجب أن يحتوي محدِّد `.author` على خاصية `font-family` بقيمة `Raleway` مع نظام بديل `sans-serif`.
