...
You should add a new `label` element after the checkbox element. 確保它有一個開始標籤和一個結束標籤。
...
Your `label` element should have a `for` attribute with the value `loving`.  應該是 `--fcc-expected--`，而不是 `--fcc-actual--`。
