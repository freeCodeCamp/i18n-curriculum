...
استخدم صيغة السهم لإكمال دالة الاستدعاء الراجع `forEach`. داخل جسم دالة الاستدعاء الراجع، استخدم تعيين الجمع لتعيين `innerHTML` الخاص بـ `tasksContainer` إلى علامات دقيقة فارغة.

# --hints--

لا تغير في `taskData.forEach()` الموجود والقيم التي فككتها.
...
استخدم صيغة السهم واربط `innerHTML` بـ `tasksContainer`.

```js
assert.match(code, /taskData\.forEach\(\s*\(\s*\{.*\}\s*\)\s*=>\s*(\s*\{?\(\s*tasksContainer\.innerHTML\s*\)\s*\}?)?/)
```

استخدم تعيين الجمع لتعيين `innerHTML` الخاص بـ `tasksContainer` إلى زوج فارغ من علامات دقيقة.
