...
Додайте атрибут `value` зі значенням `player.overallRating` до Overall `input`. Після цього додайте атрибут `onChange`, який буде стрілковою функцією, що приймає `e` як параметр і викликає `setPlayer` з розгортанням `player` та `overallRating`, встановленим у `Number(e.target.value)`.

Оскільки `overallRating` є числом, використайте `Number()` для перетворення рядкового значення з вхідних даних.

# --hints--

Ваш Overall `input` має мати `value`, рівний поточному загальному рейтингу гравця.
...
Ваш Overall `input` має мати атрибут `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

`onChange` вашого Overall `input` має викликати `setPlayer` з розгортанням `player` та `overallRating`, встановленим у `Number(e.target.value)`.
