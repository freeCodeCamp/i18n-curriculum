...
Перемістіть `.fb4` відносно поточного розташування, додавши `position` зі значенням `relative` та `left` зі значенням `10%`. Зробіть те саме для `.fb5`, але використайте `right` замість `left`. Це прикриє решту білого простору між будівлями.
