...
创建一个新的 `.calories-info` 选择器，并将其 `display` 属性设置为 `flex`。 再给它一个 `justify-content` 属性，设置为 `space-between`，以及一个 `align-items` 属性，设置为 `flex-end`。
...
你的 `.calories-info` 选择器应该将 `justify-content` 属性设置为 `space-between`。
