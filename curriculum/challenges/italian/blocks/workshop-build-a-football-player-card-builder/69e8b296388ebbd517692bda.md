...
Aggiungi un attributo `value` di `player.imageUrl` all'URL dell'immagine `input`. Poi imposta l'attributo `onChange` su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `imageUrl` impostato su `e.target.value`.

# --hints--

Il tuo URL dell'immagine `input` dovrebbe avere un `value` uguale all'URL dell'immagine del giocatore.
...
Il tuo URL dell'immagine `input` dovrebbe avere un gestore `onChange` che chiama `setPlayer` con uno spread di `player` e `imageUrl` impostato su `e.target.value`.
