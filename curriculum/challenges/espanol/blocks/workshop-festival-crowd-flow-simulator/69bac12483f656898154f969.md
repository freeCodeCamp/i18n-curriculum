...
Usa un `for...in` para iterar por cada puerta en el objeto `summary` de entrada y registra el total de asistentes procesados.

Para cada puerta, registra un mensaje con este formato: `gate + ": " + summary[gate] + " attendees processed"`.

Como ejemplo, para un objeto `summary` de entrada definido como `{ North: 5, East: 3, South: 4, West: 2 }`, deberías registrar:
...
Debes recorrer cada elemento en el objeto `summary` y registrar el siguiente mensaje una vez por elemento: `element + ": " + summary[element] + " attendees processed`.
