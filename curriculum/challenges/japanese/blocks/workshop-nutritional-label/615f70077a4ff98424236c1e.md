...
新しい `.indent` セレクターを作成し、`margin-left` プロパティを `1em` に設定してください。

# --hints--

新しい `.indent` セレクターがあるはずです。

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('.indent'));
```

`.indent` セレクターには `margin-left` プロパティが `1em` に設定されているはずです。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.indent')?.marginLeft, '1em');
```

`.indent` セレクターは他のマージンプロパティではなく、`margin-left` プロパティのみを設定してください。
