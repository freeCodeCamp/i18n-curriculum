...
أنشئ محدِّدًا جديدًا بنسبة `25%` بين محدِّدات `0%` و `50%`. اجعل الخاصية `background-color` لهذا المحدِّد الجديد تساوي `yellow`.

# --hints--

يجب أن تنشئ محدِّدًا جديدًا بنسبة `25%` داخل قاعدة `@keyframes cabins` الخاصة بك.
...
يجب أن يكون محدِّد `25%` بين محدِّدات `0%` و `50%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText , '25%');
```

يجب أن يحتوي محدِّد `25%` على خاصية `background-color` مضبوطة على `yellow`.
