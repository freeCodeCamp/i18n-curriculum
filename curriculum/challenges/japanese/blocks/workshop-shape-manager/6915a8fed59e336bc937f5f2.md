...
最後に選択する要素のセットは結果要素です。`getElement` 関数を使って、IDが `result-text` と `result-card` の要素を取得してください。どちらも `HTMLElement` をタイプとして使う必要があります。

すでに `resultText` と `resultCard` の変数を宣言しているので、代入するだけで大丈夫です。

# --hints--

`resultText` 変数に値を代入してください。

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` は `result-text` 要素を選択するはずです。

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

`HTMLElement` を使って `getElement` 要素を選択するときは、`result-text` をタイプとして使う必要があります。

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

`resultCard` 変数に値を代入してください。

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` は `result-card` 要素を選択するはずです。

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

`HTMLElement` を使って `getElement` 要素を選択するときは、`result-card` をタイプとして使う必要があります。
