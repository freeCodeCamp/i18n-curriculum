...
У блоці `try` створіть змінну `saved`. Для значення використайте метод `getItem()` для отримання `STORAGE_KEY`. Одразу під цією змінною створіть інструкцію `if` з умовою `saved`. Залиште блок `if` поки що порожнім.

# --hints--

Ваша функція `loadPlayer` має викликати `localStorage.getItem` з `STORAGE_KEY` всередині блоку `try`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Результат `localStorage.getItem` слід зберегти у змінній з назвою `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

У блоці `try` має бути інструкція `if` з умовою `saved`.
