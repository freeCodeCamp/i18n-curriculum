...
素晴らしいです！datetime のフォーマットを練習したので、コードの一番下にある `current_time` 変数と print 文を削除してください。次のステップで `Email` クラスにタイムスタンプを統合します。

# --hints--

コードに `current_time = datetime.datetime.now()")` という文があってはいけません。
...
コードに `strftime()` を使った練習用の print 文があってはいけません。
