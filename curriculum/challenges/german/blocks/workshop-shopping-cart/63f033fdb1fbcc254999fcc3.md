...
Ihr Warenkorb ist derzeit auf der Webseite nicht sichtbar. Um ihn sichtbar zu machen, fügen Sie zunächst einen Event Listener zur `cartBtn`-Variablen hinzu, der auf das Klick-Ereignis hört. Die Callback-Funktion benötigt keine Parameter.

# --hints--

Sie sollten einen Event Listener zur `cartBtn`-Variablen hinzufügen.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(/);
```

Sie sollten auf ein `click`-Ereignis an der `cartBtn`-Variablen hören.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

Sie sollten dem Event Listener eine leere Callback-Funktion (mit Pfeil-Syntax) hinzufügen. Denken Sie daran, dass sie keine Parameter benötigt.
