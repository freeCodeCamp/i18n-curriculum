...
يجب عليك إعطاء `.fb5` وظيفية `repeating-linear-gradient` ثانية في `background` الخاصية.
...
يجب أن تعطي `repeating-linear-gradient` اتجاه `90deg`.
...
يجب عليك إضافة `repeating-linear-gradient` يحمل لون أولي `--building-color2` من `0%` إلى `12%`.
...
يجب عليك إضافة `repeating-linear-gradient` يحمل لون ثاني `--window-color2` من `12%` إلى `44%`.
