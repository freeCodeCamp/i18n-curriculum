...
لتوفير طريقة للخروج من البرنامج، استخدم بند `elif` آخر للتحقق مما إذا كانت قيمة `choice` تساوي السلسلة النصية `'5'`.

داخل بند `elif` الجديد، اطبع السلسلة النصية `'Exiting the program.'` لتوضح أن البرنامج ينهي تنفيذه.

# --hints--

يجب أن تنشئ بند `elif` يتحقق مما إذا كانت `choice == '5'`.
...
يجب أن تطبع `'Exiting the program.'` في بند `elif` الجديد.
