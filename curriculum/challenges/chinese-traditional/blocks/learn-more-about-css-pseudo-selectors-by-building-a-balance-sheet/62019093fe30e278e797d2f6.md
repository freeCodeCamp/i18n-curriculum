...
`[attribute="value"]` 選擇器標的任何具有特定屬性值的元素。創建一個 `tr[class="total"]` 選擇器，以專門標的你的具有 `total` 類別的 `tr` 元素。為它設定底部邊框為 `4px double #0a0a23` 並使字型加粗。

# --hints--

你應該有一個 `tr[class="total"]` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"]'));
```

你的 `tr[class="total"]` 選擇器應該有一個設定為 `4px double #0a0a23` 的 `border-bottom` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"]')?.getPropertyValue('border-bottom') === '4px double rgb(10, 10, 35)');
```

你的 `tr[class="total"]` 選擇器應該有設定 `font-weight` 屬性為 `bold`。
