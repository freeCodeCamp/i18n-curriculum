...
Escribe una directiva `@each` que recorre la lista: `blue, black, red` y asigna cada variable a una clase `.color-bg`, donde la parte `color` cambia para cada elemento al color correspondiente. Además, cada clase debe definir la propiedad `background-color` igual al color correspondiente.
