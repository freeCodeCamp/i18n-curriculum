...
`label` 要素の後、テンプレート文字列の新しい行に `input` 要素を作成してください。`type` 属性を `text` に、`placeholder` 属性を `Name` に、そして `id` 属性を `for` 要素の `label` 属性と一致するように設定してください。
...
新しい行に `input` 要素を追加してください。

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>Entry\s\$\{entryNumber\}\sName<\/label>\n\s*<input/);
```

`input` 要素には `type` 属性を `text` に設定してください。
...
`input` 要素には `placeholder` 属性を `Name` に設定してください。
...
`input` 要素には `id` 属性を `${entryDropdown.value}-${entryNumber}-name` に設定してください。
