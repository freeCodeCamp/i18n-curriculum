---
id: 69c918841987bf16c1d1baba
title: ステップ 2
challengeType: 1
dashedName: step-2
---

# --description--

グラフの隣接行列を表すには2次元配列が必要です。各値は2つのノード間の辺の重み（距離）を表します。`INF` の値は直接の辺がないことを意味します。

`adjMatrix` という名前の別の変数を作成し、次の値を持つ2次元配列を代入してください。

```js
[0, 5, 3, INF, 11, INF],
[5, 0, 1, INF, INF, 2],
[3, 1, 0, 1, 5, INF],
[INF, INF, 1, 0, 9, 3],
[11, INF, 5, 9, 0, INF],
[INF, 2, INF, 3, INF, 0]
```

# --hints--

`adjMatrix` という名前の変数を作成してください。

```js
assert.isDefined(adjMatrix);
```

`adjMatrix` は6行6列の2次元配列にしてください。

```js
assert.isArray(adjMatrix);
assert.lengthOf(adjMatrix, 6);
adjMatrix.forEach(row => assert.lengthOf(row, 6));
```

`adjMatrix` には正しい値を代入してください。

```js
assert.deepEqual(adjMatrix[0], [0, 5, 3, INF, 11, INF]);
assert.deepEqual(adjMatrix[1], [5, 0, 1, INF, INF, 2]);
assert.deepEqual(adjMatrix[2], [3, 1, 0, 1, 5, INF]);
assert.deepEqual(adjMatrix[3], [INF, INF, 1, 0, 9, 3]);
assert.deepEqual(adjMatrix[4], [11, INF, 5, 9, 0, INF]);
assert.deepEqual(adjMatrix[5], [INF, 2, INF, 3, INF, 0]);
```

# --seed--

## --seed-contents--

```js
const INF = Infinity;
--fcc-editable-region--

--fcc-editable-region--
```
