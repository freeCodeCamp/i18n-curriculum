...
Dovresti dare a `.foot` una proprietà `top` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.top, '85%');
```

Dovresti dare a `.foot` una proprietà `border-radius` con il valore `--fcc-expected--` invece di `--fcc-actual--`.
