...
Aggiungi un'istruzione `if` per verificare che nessuna canzone sia in riproduzione controllando che `userData.currentSong` sia falsy.
All'interno del blocco `if`, imposta la proprietà `currentTime` dell'oggetto `audio` su `0`.

# --hints--

La tua funzione `playSong` dovrebbe impostare la proprietà `currentTime` dell'oggetto `audio` su `0` quando nessuna canzone è in riproduzione.
