...
Your `input` element does not have a `placeholder` attribute. 请检查在开始标签的名称后面要有一个空格，且/或所有的属性名称前面也要有一个空格。

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('placeholder'));
```

Your `input` element should have a `placeholder` attribute with the value `cat photo URL`.  你可能落下了这个值或者有拼写错误。 所有属性的值都应该放在引号中。
