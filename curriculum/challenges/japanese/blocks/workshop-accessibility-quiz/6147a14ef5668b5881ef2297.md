...
ナビゲーションボタンをより一般的なボタンのように見せるために、アンカーの下線を削除してください。

次に、ナビゲーションリスト要素を対象とする新しいセレクターを作成し、カーソルがホバーしたときに背景色と文字色が入れ替わり、カーソルがポインターになるようにしてください。

# --hints--

既存の `li > a` セレクターを使って `text-decoration` を `none` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

ホバー時のスタイルには `nav > ul > li:hover` または `nav li:hover` セレクターを使うべきです。
...
ホバーした `li` 要素に `background-color` を `#dfdfe2` に設定してください。
...
ホバーした `li` 要素に `color` を `#1b1b32` に設定してください。
...
ホバーした `li` 要素に `cursor` を `pointer` に設定してください。
