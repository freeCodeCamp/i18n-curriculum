...
`setPlayButtonAccessibleText` 函式會將 `aria-label` 屬性設定為目前的歌曲，或播放列表中的第一首歌曲。如果播放列表為空，則會將 `aria-label` 設定為 `"Play"`。

對 `playButton` 元素使用 `setAttribute` 方法來設定名為 `"aria-label"` 的屬性。使用三元運算子，將屬性值設定為 `Play ${song.title}`，如果 `song?.title` 不存在則設定為 `"Play"`。

別忘了這裡需要字串插值，所以你需要使用反引號（backticks）。

# --hints--

你不應該修改現有的 `setPlayButtonAccessibleText` 函式（程式）及其內容。
...
你應該在 `playButton` 上使用 `setAttribute()` 方法。

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`setAttribute()` 方法的第一個值應該是 `"aria-label"`。

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

你的 `setAttribute` 方法應該將 `song?.title ? `Play ${song.title}` : "Play"` 作為第二個引數。
