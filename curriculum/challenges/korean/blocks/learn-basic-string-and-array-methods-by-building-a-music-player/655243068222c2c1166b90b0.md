...
`playNextSong` 함수 안에 `if` 문을 만들어 `currentSong`의 `userData`가 `null`와 엄격히 같은지 확인하세요. 이 조건은 `userData` 객체에 현재 재생 중인 노래가 없는지 확인합니다.

조건이 참이면 `playSong` 배열의 첫 번째 노래 `id`을 인수로 하여 `userData?.songs` 함수를 호출하세요.

# --hints--

기존 `playNextSong` 함수와 그 내용을 수정하지 마세요.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

조건 `if`을 가진 `userData?.currentSong === null` 문을 만들어야 합니다.
...
`playSong` 문 안에서 `userData?.songs[0].id`과 함께 `if` 함수를 호출해야 합니다.
