...
你也將在此方法中更新 HTML。 將 `cartSubTotal` 的 `textContent` 設置爲 `subTotal` 的值，固定爲小數點後 2 位。 使用模板字面量語法將美元符號添加到值的開頭。

# --hints--

你應該訪問 `cartSubTotal` 元素的 `textContent` 屬性。

```js
assert.match(cart.calculateTotal.toString(), /cartSubTotal\.textContent/);
```

你應該在 `subTotal` 變量上調用 `.toFixed()` 方法，並將 `2` 作爲參數傳遞。
...
你應該將 `cartSubTotal` 元素的 `textContent` 設置爲你的模板字符串。
