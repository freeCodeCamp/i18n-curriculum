...
Тепер слід додати відступ до контейнера. Встановіть `padding` у `10px 20px`. Таким чином, ви встановите відступ `10px` зверху і знизу, а `20px` зліва і справа.

# --hints--

Вам потрібно націлитися на клас `.container`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.container'));
```

Встановіть властивість `padding` у `10px 20px`.
