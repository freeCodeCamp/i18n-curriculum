...
`card-rating` div の下に、`div` の `className` を持ち、文字列 `card-position` を含む `ST` を追加してください。これはストライカーを表します。

# --hints--

`div` div の下に、`className` の `card-position` を持つ `.card-rating` を追加する必要があります。

```js
assert.exists(document.querySelector('.card-rating + .card-position'));
```

`.card-position` 要素には文字列 `ST` を含めてください。
