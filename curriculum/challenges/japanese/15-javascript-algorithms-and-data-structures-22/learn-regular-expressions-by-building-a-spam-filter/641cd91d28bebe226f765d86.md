...
`checkMessageButton` に、`click` イベントを検知するイベントリスナーを設定してください。 また、空のコールバック関数を渡してください。

# --hints--

`checkMessageButton` 要素に対して `.addEventListener()` を呼び出してください。

```js
assert.match(code, /checkMessageButton\.addEventListener\(/);
```

`.addEventListener()` メソッドは `"click"` イベント型を持っている必要があります。

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,/);
```

`.addEventListener()` メソッドには空のコールバック関数が必要です。
