...
Dai a `.fb2a` un `width` di `100%` e a `.fb2b` un `width` di `100%` e `height` di `75%`.

# --hints--

Dovresti dare a `.fb2a` un `width` di `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.width, "100%");
```

Dovresti dare a `.fb2b` un `width` di `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2b")?.width, "100%");
```

Dovresti dare a `.fb2b` un `height` di `75%`.
