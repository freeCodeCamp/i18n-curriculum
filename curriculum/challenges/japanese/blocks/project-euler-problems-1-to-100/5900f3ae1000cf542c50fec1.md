...
次の形の2次ディオファントス方程式を考えます：

<div style='text-align: center;'>x<sup>2</sup> – Dy<sup>2</sup> = 1</div>

例えば、D=13 のとき、最小の解は 649<sup>2</sup> – 13×180<sup>2</sup> = 1 です。

D が平方数の場合、正の整数解は存在しないと仮定できます。

D = {2, 3, 5, 6, 7} に対して最小の x の解を求めると、次のようになります：
...
したがって、D ≤ 7 の場合の `x` における最小解を考えると、最大の `x` は D=5 のときに得られます。

D ≤ `n` の範囲で、`x` の最小解において `x` の最大値が得られる D の値を求めてください。
