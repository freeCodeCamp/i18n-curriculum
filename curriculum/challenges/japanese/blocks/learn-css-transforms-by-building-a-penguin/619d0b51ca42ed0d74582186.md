...
`.eye` 要素をターゲットにして、`width` を `15%`、`height` を `17%`、`background-color` を `black` に設定してください。
...
`.eye` に `width` を `--fcc-expected--` に設定するべきです。場所は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.width, '15%');
```

`.eye` に `height` を `--fcc-expected--` に設定するべきです。場所は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.height, '17%');
```

`.eye` に `background-color` を `--fcc-expected--` に設定するべきです。場所は `--fcc-actual--` です。
