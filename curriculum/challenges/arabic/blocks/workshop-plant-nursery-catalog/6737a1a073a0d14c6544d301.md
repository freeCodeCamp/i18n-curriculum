...
أعلن عن متغير باسم `plantsSet` وقم بتعيين استدعاء الدالة `displayPlantsSet` له.

ثم سجّل `plantsSet` في وحدة التحكم. ستحتاج إلى فتح وحدة تحكم المتصفح لرؤية المجموعة الخاصة بك.


# --hints--

يجب أن يكون لديك متغير باسم `plantsSet`.

```js
assert.exists(plantsSet);
```

يجب أن تستدعي `displayPlantsSet` وتعيّن نتيجتها إلى `plantsSet`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

يجب أن تسجل `plantsSet` في وحدة التحكم.
