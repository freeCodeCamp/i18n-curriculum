...
Un'opzione è la proprietà `grid-column`, che è la proprietà shorthand per `grid-column-start` e `grid-column-end`. La proprietà `grid-column` indica all'elemento in corrispondenza di quale linea della griglia iniziare e terminare.

Crea una regola `.heading` e imposta la proprietà `grid-column` su `2 / 3`. Questa regola dirà all'elemento `.heading` di iniziare dalla linea 2 della griglia e terminare alla linea 3.
...
Il selettore `.heading` dovrebbe avere una proprietà `grid-column` impostata su `2 / 3`.
