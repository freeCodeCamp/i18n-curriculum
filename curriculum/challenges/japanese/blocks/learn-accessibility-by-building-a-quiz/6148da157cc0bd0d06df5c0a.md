...
入力ボックス同士を揃えるために、`input`要素内のすべての`label`と`.info`要素を対象とする新しいルールセットを作成し、`display`プロパティを`inline-block`に設定してください。

また、`label`要素のテキストを右揃えにしてください。

# --hints--

`.info > label, .info > input`または`.info label, .info input`セレクターのいずれかを使うべきです。
...
`input`と`label`要素に`display`を`inline-block`で指定するべきです。
...
`label`要素に`text-align`を`right`で指定するべきです。
