...
在 `while` 循環中，`array[sorted_index]` 賦值爲 `right_part[right_array_index]`。

# --hints--

在循環體中應該有 `array[sorted_index] = right_part[right_array_index]`。 不要忘記移除循環體中的 `pass` 關鍵字。
