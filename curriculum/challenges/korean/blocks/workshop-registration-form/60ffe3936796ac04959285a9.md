...
현재 `input` 또는 `textarea` 요소에 입력하면 텍스트가 보이지 않습니다. 또한, 높이가 너무 작아 사용하기 어렵습니다.

`color`를 `#ffffff`로 설정하고, `min-height`를 `2em`로 설정하여 이 문제를 고치세요.

# --hints--

`color`를 `#ffffff`로 설정해야 합니다.
