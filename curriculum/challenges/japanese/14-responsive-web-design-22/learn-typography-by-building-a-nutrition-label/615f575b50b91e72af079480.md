...
新しく `.left-container p` セレクターが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

新しい `.left-container p` セレクターの `margin` プロパティを `-5px -2px` に設定する必要があります。
...
新しい `.left-container p` セレクターの `font-size` プロパティを `2em` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

新しい `.left-container p` セレクターの `font-weight` プロパティを `700` に設定する必要があります。
