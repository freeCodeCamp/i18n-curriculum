...
为 PAS 和 DRI 各添加两个 `div` 元素，每个元素内包含一个 `className` 为 `form-group` 的 `stats-grid`。

对于 PAS 的，添加一个 `label`，其 `htmlFor` 属性为 `pas`，文本为 `PAS`，然后添加一个 `id` 属性为 `pas`，`type` 属性为 `number` 的输入框。

对于 DRI 的，添加一个 `label`，其 `htmlFor` 属性为 `dri`，文本为 `DRI`，然后添加一个 `id` 属性为 `dri`，`type` 属性为 `number` 的输入框。

# --hints--

你应该添加两个更多的 `div` 元素，每个元素内包含一个 `className` 属性为 `form-group` 的 `div`，该 `div` 位于 `className` 的 `stats-grid` 内。
...
你的第三个 `div` 元素，位于 `className` 的 `form-group` 内，应包含一个 `label`，其 `htmlFor` 属性为 `pas`，一个 `className` 属性为 `label`，以及文本 `PAS`。
...
你的第三个 `div` 元素，位于 `className` 的 `form-group` 内，应包含一个 `input`，其 `className` 属性为 `input`，一个 `id` 属性为 `pas`，以及一个 `type` 属性为 `number`。
...
你的第四个 `div` 元素，位于 `className` 的 `form-group` 内，应包含一个 `label`，其 `htmlFor` 属性为 `dri`，一个 `className` 属性为 `label`，以及文本 `DRI`。
...
你的第四个 `div` 元素，位于 `className` 的 `form-group` 内，应包含一个 `input`，其 `className` 属性为 `input`，一个 `id` 属性为 `dri`，以及一个 `type` 属性为 `number`。
