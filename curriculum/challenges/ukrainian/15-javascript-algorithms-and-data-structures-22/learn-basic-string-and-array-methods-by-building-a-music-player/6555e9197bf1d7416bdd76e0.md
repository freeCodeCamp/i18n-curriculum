...
Настав час реалізувати функціональність видалення для списку відтворення. Це керуватиме видаленням пісні зі списку відтворення, оброблятиме інші пов’язані дії під час видалення пісні та створюватиме кнопку `Reset Playlist`.

Використайте `const` та стрілковий синтаксис, щоб створити порожню функцію `deleteSong` та передати `id` як параметр.

# --hints--

Використайте `const`, щоб створити порожню функцію під назвою `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` має бути функцією.

```js
assert.isFunction(deleteSong)
```

Функція `deleteSong` повинна використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Функція `deleteSong` має приймати параметр `id`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

Функція `deleteSong` має бути порожньою.
