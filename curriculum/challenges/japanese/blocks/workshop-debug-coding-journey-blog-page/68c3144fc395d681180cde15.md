...
ページ上の見出しの使い方にまだいくつか問題があるようです。最初の `div` 要素を見ると、サブ見出しとして `h4` 要素が2つ使われています。これらは `h3` 要素の下のレベルなので、サブ見出しには `h2` 要素を使うほうが良いです。

2つの `h4` 要素を `h3` 要素に変更してください。

# --hints--

テキストが `h4` の `Early Challenges` 要素は `h3` 要素にする必要があります。
...
テキストが `h4` の `Breakthroughs` 要素は `h3` 要素にする必要があります。
