...
플레이어의 유일한 무기가 부서지지 않게 하세요. 논리 AND 연산자는 두 조건이 모두 참인지 확인합니다.

<dfn>논리 AND</dfn> 연산자 `&&`를 사용해 `if` 문장에 두 번째 조건을 추가하세요. 플레이어의 무기는 `inventory.length`가 (`!==`) 1과 같지 않을 때만 부서져야 합니다.

두 조건이 있는 `if` 문장의 예는 다음과 같습니다:
...
논리 AND 연산자를 사용해 `inventory.length`가 `1`와 같지 않은지 확인하세요.
