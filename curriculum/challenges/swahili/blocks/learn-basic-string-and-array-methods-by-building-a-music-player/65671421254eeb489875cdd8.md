...
Ndani ya kitendakazi cha mwitiko, tumia njia ya `removeAttribute()` kuondoa sifa ya kipengele ya `"aria-current"`. Hii itaondoa sifa ya kipengele kwa kila moja ya nyimbo.

# --hints--

Haupaswi kubadilisha `forEach()` iliyopo na yaliyomo yake.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

Unapaswa kutumia njia ya `removeAttribute()` kwenye `songEl` na kupitisha `"aria-current"`.
