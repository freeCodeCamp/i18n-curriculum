...
بما أن `associatedPanel` هو `id`، فذلك يسمح لك باستعلام المستند والعثور على عنصر اللوحة المقابل.

أنشئ متغيرًا جديدًا اسمه `panel` وقم بتعيين قيمته إلى `document.getElementById(associatedPanel)`.

# --hints--

يجب أن يكون لديك متغير اسمه `panel` يحتوي على عنصر اللوحة المرتبط بالتبويب الذي تم النقر عليه.
