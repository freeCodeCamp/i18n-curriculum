...
title: "المشكلة 12: عدد مثلثي قابل للقسمة بشكل كبير"
challengeType: 1
forumTopicId: 301746
...
تُولد متتالية الأعداد المثلثية بجمع الأعداد الطبيعية. إذًا العدد المثلثي السابع هو 1 + 2 + 3 + 4 + 5 + 6 + 7 = 28. ستكون أول عشرة حدود:

<div style='text-align: center;'>1, 3, 6, 10, 15, 21, 28, 36, 45, 55, ...</div>

دعنا ندرج عوامل أول سبعة أعداد مثلثية:
...
نرى أن 28 هو أول عدد مثلثي له أكثر من خمسة عوامل.

ما قيمة أول عدد مثلثي له أكثر من `n` عاملًا؟

# --hints--

يجب أن تُرجع الدالة `divisibleTriangleNumber(5)` عددًا.

```js
assert.isNumber(divisibleTriangleNumber(5));
```

يجب أن تُرجع الدالة `divisibleTriangleNumber(5)` العدد 28.

```js
assert.strictEqual(divisibleTriangleNumber(5), 28);
```

يجب أن تُرجع الدالة `divisibleTriangleNumber(23)` العدد 630.

```js
assert.strictEqual(divisibleTriangleNumber(23), 630);
```

يجب أن تُرجع الدالة `divisibleTriangleNumber(167)` العدد 1385280.

```js
assert.strictEqual(divisibleTriangleNumber(167), 1385280);
```

يجب أن تُرجع الدالة `divisibleTriangleNumber(374)` العدد 17907120.

```js
assert.strictEqual(divisibleTriangleNumber(374), 17907120);
```

يجب أن تُرجع الدالة `divisibleTriangleNumber(500)` العدد 76576500.
