...
`userData` 객체에 `songs` 속성을 만들고 그 값을 `allSongs` 배열로 설정하세요.

그다음, 현재 노래 정보를 처리하고 재생 시간을 추적하기 위해 `currentSong`와 `songCurrentTime` 속성을 만드세요. 각각의 값을 `null`와 `0`로 설정하세요.

# --hints--

`userData` 객체는 `songs` 키를 가지고 있어야 하며, 그 값은 `allSongs` 배열이어야 합니다.

```js
assert.propertyVal(userData, "songs", allSongs);
```

`userData` 객체는 `currentSong` 키를 가지고 있어야 하며, 그 값은 `null`이어야 합니다.

```js
assert.propertyVal(userData, "currentSong", null);
```

`userData` 객체는 `songCurrentTime` 키를 가지고 있어야 하며, 그 값은 `0`이어야 합니다.
