...
在第一個 `tr` 中，添加一個 `th` 元素，文本爲 `Loans The outstanding balance on our startup loan.`。 將除 `Loans` 之外的文本包裹在 `span` 元素內，並將 `class` 設置爲 `description`。

在其下方添加三個 `td` 元素，並按順序爲它們提供以下文本：`$500`、`$250` 和 `$0`。 給第三個 `td` 添加一個 `class`，設置爲 `current`。
...
你的 `th` 元素應包含文本 `Loans The outstanding balance on our startup loan.`。
...
你應該將文本 `The outstanding balance on our startup loan.` 包裹在 `span` 元素中。
