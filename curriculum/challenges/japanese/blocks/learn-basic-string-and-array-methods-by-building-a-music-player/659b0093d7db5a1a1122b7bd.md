...
現在の曲のインデックスを取得するには、<dfn>indexOf()</dfn> メソッドを使えます。`indexOf()` 配列メソッドは、指定した要素が配列内で最初に見つかるインデックスを返します。要素が存在しない場合は `-1` を返します。
...
`getCurrentSongIndex` 関数の中で、`userData?.songs.indexOf()` を返してください。`indexOf()` 引数には `userData?.currentSong` を設定してください。

# --hints--

`userData?.songs.indexOf()` 関数の中で `getCurrentSongIndex` を返す必要があります。

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

`userData?.currentSong` メソッドに `indexOf()` を渡す必要があります。
