...
Нарешті, поверніть мінімальне значення зі списку `prices` за допомогою функції `min()`. Це дасть вам найкращу (найнижчу) ціну після врахування всіх застосовних знижок.

# --hints--

Ваш метод `calculate_best_price` має повертати мінімальне значення зі списку `prices` за допомогою функції `min`.
