...
أولًا، ستجد موقع بوابة محددة داخل مصفوفة `gates`.

استخدم الطريقة `indexOf()` على `gates` لتحديد عنصر `currentGate` وعيّن النتيجة إلى متغير باسم `currentIndex`.

إليك تذكير بكيفية القيام بذلك:
...
يجب أن تستخدم `indexOf()` على مصفوفة `gates` للوصول إلى فهرس `currentGate` وتعيينه إلى متغير باسم `currentIndex`. يجب تنفيذ ذلك في سطر واحد.
