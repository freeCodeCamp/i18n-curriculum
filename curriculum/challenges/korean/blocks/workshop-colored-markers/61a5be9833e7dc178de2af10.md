...
더 좋아 보이지만, 녹색 마커의 아래쪽 가장자리가 더 어두워져서 입체감이 좀 더 생겨야 합니다.

같은 `linear-gradient` 함수에서 빨강 `11`, 초록 `6C`, 파랑 `31` 값을 가진 16진수 색상 코드를 세 번째 색상 인수로 추가하세요.

# --hints--

`.green` CSS 규칙에 `background` 속성을 `linear-gradient(180deg, #55680D, #71F53E, #116C31)`로 설정해야 합니다.
