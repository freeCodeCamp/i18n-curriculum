...
プレイリストの曲をクリックすると、その曲を再生したくなります。そのためには、`click` 要素の子である各 `button` 要素に対して `song` イベントのイベントリスナーを追加する必要があります。

`forEach` のコールバックを完成させてください。`addEventListener` のコールバック内で、`playSong(n)` を呼び出してください。ここで `n` は `id` の `song` 属性から取得できる曲のIDです。

各 `song` には `id` の `song-n` 属性があることに注意してください。`n` 部分を取得し、`playSong()` に渡す前に数値に変換する必要があります。


# --hints--

`click` 要素の子である各 `button` 要素に対して `.playlist-song` イベントのイベントリスナーを追加し、対応する曲の `playSong` を使って `id` を呼び出してください。
