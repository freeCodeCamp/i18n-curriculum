...
`Set` 物件允許你儲存任何型別的唯一值。你可以使用 `Set` 建構式來創建一個空集合：

```js
const mySet = new Set();
``` 

創建一個名為 `displayPlantsSet` 的函式（程式），並使其傳回一個新的 `Set` 物件。 

# --hints--

你應該有一個名為 `displayPlantsSet` 的函式（程式）。

```js
assert.isFunction(displayPlantsSet);
```

你的 `displayPlantsSet` 函式（程式）應該傳回一個空的 `Set` 物件。
