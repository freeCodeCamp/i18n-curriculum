...
드롭다운 메뉴에서 카테고리를 선택하면 악기 카드가 올바르게 필터링되어 페이지에 표시되지만, 카드 사이에 있는 쉼표를 없애야 합니다.

`instrumentCards`가 반환하는 배열을 연결하여 해결하세요. 이렇게 하면 음악 악기 필터가 완성됩니다.
...
`instrumentCards` 함수가 반환하는 배열을 연결해야 합니다. 이벤트 리스너는 수정하지 마세요.
