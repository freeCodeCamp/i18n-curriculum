...
次の曲と前の曲を再生する処理に取りかかる前に、`songs`の`userData`プロパティ内の各曲のインデックスを取得する必要があります。そのために、配列のメソッドである`indexOf()`を使うことができます。これは、指定した要素が配列内で最初に見つかるインデックスを返し、要素が存在しない場合は`-1`を返します。
...
`getCurrentSongIndex`という名前の関数を作成してください。この関数は、現在の曲が`userData.currentSong`のときに、`userData.songs`が`-1`と`null`の中で見つかるインデックスを返します。

# --hints--

`getCurrentSongIndex`という名前の関数を用意してください。

```js
assert.isFunction(getCurrentSongIndex);
```

`getCurrentSongIndex`関数は、`userData.currentSong`が`userData.songs`の中で見つかるインデックスを返す必要があります。
...
`getCurrentSongIndex`関数は、`-1`が`userData.currentSong`の場合に`null`を返す必要があります。
