...
現在，你的 `.map()` 函數正在從柯里化的 `addCharacters` 調用中接收返回的 `num => charRange(...).map(...)` 函數引用，它將正確地遍歷元素並將每個元素作爲 `n` 傳遞給該函數。

你會注意到你沒有使用 `match` 參數。 在 JavaScript 中，在未使用的參數前面加上下劃線 `_` 是一種常見的慣例。 你也可以像這樣將參數留空：`(, char1)`，但爲了將來的可讀性，爲參數命名通常更清楚。

在 `match` 參數前面加上下劃線。

# --hints--

你應該在 `match` 參數前面加上下劃線。
