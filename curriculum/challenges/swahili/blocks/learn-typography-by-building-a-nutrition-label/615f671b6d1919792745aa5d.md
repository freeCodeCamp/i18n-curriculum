...
Unda kipengele kipya cha `div` chini ya kipengele chako cha `.medium`. Kipekee sifa ya kipengele ya `class` iwe sawa na `daily-value small-text`. Ndani ya `div` hii mpya, ongeza kipengele cha `p` chenye maandishi ya `% Daily Value *`, na weka sifa ya kipengele ya `class` kuwa `bold right`.
...
Kipengele chako kipya cha `div` kinapaswa kuwa na sifa ya kipengele ya `class` iliyowekwa kuwa `daily-value small-text`.
...
Kipengele chako kipya cha `div` kinapaswa kuwa na kipengele cha `p`.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.localName === 'p');
```

Kipengele chako kipya cha `p` kinapaswa kuwa na maandishi ya `% Daily Value *`.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.textContent === '% Daily Value *');
```

Kipengele chako kipya cha `p` kinapaswa kuwa na sifa ya kipengele ya `class` iliyowekwa kuwa `bold right`.
