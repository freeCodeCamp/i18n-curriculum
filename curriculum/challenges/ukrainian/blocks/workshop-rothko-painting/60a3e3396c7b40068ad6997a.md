...
Напишіть нове правило, яке націлюється на `.one` і встановіть його `width` на 425 пікселів.

# --hints--

У вас має бути селектор `.one`.
...
Ви повинні встановити властивість `width` у значення `425px`.
...
Ваш елемент `.one` має мати значення `width` рівне `425px`.
