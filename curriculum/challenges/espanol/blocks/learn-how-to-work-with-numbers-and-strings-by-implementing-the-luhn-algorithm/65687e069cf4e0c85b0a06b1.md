...
Fuera de la estructura `if`, añade `number` a `sum_of_even_digits`. Además, elimina la llamada a `print`.

# --hints--

Debes tener `sum_of_even_digits += number` dentro del ciclo `for`.
...
No deberías tener `print(number)` dentro del ciclo `for`.
...
No deberías tener `sum_of_even_digits += number` dentro de la estructura `if`.
