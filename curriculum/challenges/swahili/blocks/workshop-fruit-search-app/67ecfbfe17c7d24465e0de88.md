...
Ndani ya kipengele cha `form`, baada ya lebo, ongeza ingizo lenye `id` ya `search-input` na `type` ya `search`.

# --hints--

Unapaswa kuwa na `input` ndani ya kipengele cha `form` chenye `id` ya `search-input`.
...
`input` yako inapaswa kuwa na thamani ya `type` ya `search`.
