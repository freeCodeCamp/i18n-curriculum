...
记住数组的索引从零开始的，这意味着数组的最后一个元素的下标是：`length - 1`（数组的长度 -1）。 我们这个循环的 <dfn>条件</dfn>是`i < arr.length`，当`i`的值为 长度 -1 的时候循环就停止了。 在这个例子中，最后一个循环是 `i === 4`，也就是说，当 `i` 的值等于 `arr.length - 1` 时，结果输出 `6`。  Then `i` increases to `5`, and the loop terminates because `i < arr.length` is `false`.
