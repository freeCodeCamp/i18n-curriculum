...
創建一個新的 `.left-container p` 選擇器，將上下邊距設為 `-5px`，左右邊距設為 `-2px`。同時將 `font-size` 設為 `2em`，`font-weight` 設為 `700`。
...
你的新 `.left-container p` 選擇器應該有一個 `margin` 屬性設定為 `-5px -2px`。
...
你的新 `.left-container p` 選擇器應該有一個 `font-size` 屬性設定為 `2em`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

你的新 `.left-container p` 選擇器應該有一個 `font-weight` 屬性設定為 `700`。
