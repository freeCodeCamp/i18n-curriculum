---
id: 677fac4406da40165a058179
title: Завдання 42
challengeType: 19
dashedName: task-42
lang: en-US
---

<!-- READING -->

# --instructions--

Прочитайте текст і дайте відповідь на запитання нижче.

# --questions--

## --text--

Команда підтримки опублікувала короткий посібник з усунення проблем із завантаженням:

`Having boot issues?`

`Troubleshooting computer issues can feel overwhelming, but understanding them can make the process easier. Here's a simple step-by-step checklist for diagnosing and fixing computer boot problems:`

`1. Check the power connection: If the power cable isn't securely connected, the computer won't turn on.`

`2. Inspect the BIOS: If the BIOS is corrupted, the system might fail to boot. You may need to reset or update it.`

`3. Examine the hard drive: If the hard drive is failing, the operating system won't load. Run diagnostic tools to confirm.`

`4. Update drivers: If the software crashes, it could be due to outdated or incompatible drivers. Always keep drivers updated to prevent this.`

`5. Watch out for memory leaks: If the application has memory leaks, it can slow down or crash the system. Look for signs like freezing or lagging.`

`Following these steps can help you identify the root cause and get things running smoothly again.`

Який перший крок у діагностиці комп’ютера, який не вмикається?

## --answers--

Перевірте підключення живлення.

---

Перевірте BIOS.

### --feedback--

Перевірка BIOS — це другий крок після того, як переконалися, що підключення живлення надійне.

---

Запустіть діагностичні інструменти.

### --feedback--

Діагностичні інструменти згадуються для жорстких дисків, але перший крок — перевірити живлення.

---

Оновіть драйвери.

### --feedback--

Оновлення драйверів пропонуються пізніше для виправлення збоїв програмного забезпечення, а не проблем із завантаженням.

## --video-solution--

1

# --explanation--

Посібник починається з розгляду `power connection`, підкреслюючи, що `if the power cable isn't securely connected, the computer won't turn on`. Цей базовий крок є критично важливим, щоб виключити прості причини перед переходом до складніших діагностик.

Контрольний список свідомо починається тут, бо проблеми з живленням є загальними і легко вирішуються.

Зосередьтеся на частині, де посібник пов’язує симптом (`computer won't turn on`) із рішенням (`check the power connection`), демонструючи важливість починати з базового усунення несправностей.
