---
id: 67cac4bf5c233918f5312fd5
title: Aufgabe 34
challengeType: 22
dashedName: task-34
lang: en-US
---

<!-- (audio) Sophie: Thanks for pointing that out. I noticed that SecureSoft has more integration options with other tools. Is SafeGuard as flexible? -->

# --instructions--

Hören Sie sich das Audio an und vervollständigen Sie den Satz unten.

# --fillInTheBlank--

## --sentence--

`Thanks for pointing that out. I noticed that SecureSoft has more integration options with other tools. Is SafeGuard BLANK?`

## --blanks--

`as flexible`

### --feedback--

Dieser zweiteilige Ausdruck vergleicht die Flexibilität von zwei Dingen.

# --explanation--

`As flexible` bedeutet, dass Sie die Flexibilität von zwei Dingen vergleichen, um zu sehen, ob sie in dieser Hinsicht gleich sind. Zum Beispiel:  

`This jacket is as flexible as the one I bought last week.` – In diesem Fall vergleichen Sie, wie anpassungsfähig beide Jacken sind, und sagen, dass sie das gleiche Maß an Flexibilität haben.

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Sophie",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_13-1.mp3",
      "startTime": 1,
      "startTimestamp": 49.48,
      "finishTimestamp": 56.22
    }
  },
  "commands": [
    {
      "character": "Sophie",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sophie",
      "startTime": 1,
      "finishTime": 5.65,
      "dialogue": {
        "text": "Thanks for pointing that out. I noticed that SecureSoft has more integration options with other tools.",
        "align": "center"
      }
    },
    {
      "character": "Sophie",
      "startTime": 6.39,
      "finishTime": 7.47,
      "dialogue": {
        "text": "Is Safeguard as flexible?",
        "align": "center"
      }
    },
    {
      "character": "Sophie",
      "opacity": 0,
      "startTime": 8.07
    }
  ]
}
```
