...
ワークショップの残りのステップでは、保存されたメモのメッセージを動的に表示するコードを追加し、現在のメモをコンソールに記録します。

まず、`noteEl` という変数を作成し、`id` の `note` を持つ要素をドキュメントからクエリして取得した結果を代入してください。

次に、`statusEl` という別の変数を作成し、`id` の `status` を持つ要素をドキュメントからクエリして取得した結果を代入してください。

# --hints--

`noteEl` の `id` を持つ要素をドキュメントからクエリして取得した結果を代入する `note` という変数が必要です。

```js
assert.equal(noteEl, document.getElementById("note"));
``` 

`statusEl` の `id` を持つ要素をドキュメントからクエリして取得した結果を代入する `status` という変数が必要です。
