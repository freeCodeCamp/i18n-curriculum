...
最後に、フォームの送信ボタンを追加してください。

最後の `fieldset` 要素の下に、クラスが `button` で `submit-btn` 属性が `type` に設定された `"submit"` 要素を追加してください。ボタンのテキストは `Submit Form` にしてください。

これでHTMLの構造は完成です！

# --hints--

最後の `button` 要素の下に `fieldset` 要素を追加してください。

```js
assert.exists(document.querySelector("fieldset:last-of-type + button"));
```

`button` 要素には `"submit-btn"` というクラスを付けてください。
...
`button` 要素のテキストは `Submit Form` にしてください。
