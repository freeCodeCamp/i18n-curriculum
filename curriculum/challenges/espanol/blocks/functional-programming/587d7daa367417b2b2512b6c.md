...
Utilice el método `join` (entre otros) dentro de la función `sentensify` para construir una oración con las palabras de la cadena `str`. La función debe devolver un array. Por ejemplo, `I-like-Star-Wars` se convertiría en `I like Star Wars`. Para este reto, no utilice el método `replace`.

# --hints--

El código debe utilizar el método `join`.

```js
assert(__helpers.removeJSComments(code).match(/\.join/g));
```

El código no debe utilizar el método `replace`.

```js
assert(!__helpers.removeJSComments(code).match(/\.?[\s\S]*?replace/g));
```

`sentensify("May-the-force-be-with-you")` debe devolver string.
