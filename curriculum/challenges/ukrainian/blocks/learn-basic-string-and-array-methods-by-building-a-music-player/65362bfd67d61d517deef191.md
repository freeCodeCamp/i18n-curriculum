...
Метод <dfn>find()</dfn> повертає перший елемент у масиві, який задовольняє умови, визначені у наданій функції зворотного виклику. Якщо жоден елемент не відповідає умові, метод повертає `undefined`.

У наведеному нижче прикладі метод `find()` використовується, щоб знайти перше число, більше за 25:
...
Використайте `const`, щоб створити змінну з ім’ям `song` і призначити їй результат виклику методу `find()` на масиві `userData?.songs`. Використайте `song` як параметр функції зворотного виклику `find()` і перевірте, чи `song.id` строго дорівнює `id`.

Це ітеруватиме по масиву `userData?.songs`, шукаючи пісню, яка відповідає `id`, переданому у функцію `playSong`.

# --hints--

Ви повинні використати метод `find` на `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

Ваш метод `find` має мати `song` як параметр стрілкової функції зворотного виклику.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

Ваш метод `find` має використовувати сувору рівність, щоб перевірити, чи `song.id` дорівнює `id`.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

Ваш метод `find` має бути призначений константі `song`.
