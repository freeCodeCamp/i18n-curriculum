...
开始添加一个 `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap` 字体样式表 `link` 元素，一个 `https://use.fontawesome.com/releases/v5.8.2/css/all.css` FontAwesome 样式表 `link` 元素，还有一个 `./styles.css` 样式表 `link` 元素。
...
你的三个 `link` 元素都应该有一个值为 `stylesheet` 的 `rel` 属性。
...
有一个链接元素应将 `href` 设置为 `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap`。
...
有一个链接元素应将 `href` 设置为 `https://use.fontawesome.com/releases/v5.8.2/css/all.css`。
