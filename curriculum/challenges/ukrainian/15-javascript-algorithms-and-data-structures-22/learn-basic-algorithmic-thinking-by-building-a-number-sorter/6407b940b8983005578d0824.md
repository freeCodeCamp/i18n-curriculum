...
Вам потрібно отримати значення з елементів `select`. Ці значення наразі будуть рядками, та ви перетворите їх на числа.

Використовуйте функцію `map` для ітерування через масив. Передайте функцію зворотного виклику до `map`, яка приймає параметр `dropdown` та повертає `dropdown.value`.

# --hints--

Приєднайте метод `map` одразу на масиві.

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.getElementsByClassName\(\s*('|"|`)values-dropdown\1\s*\)\s*\]\s*\.map\(/);
```

Функція зворотного виклику `map` має використати синтаксис стрілкової функції.

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.getElementsByClassName\(\s*('|"|`)values-dropdown\1\s*\)\s*\]\s*\.map\s*\(\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

Функція зворотного виклику `map` має приймати параметр `dropdown`.
...
Функція зворотного виклику `map` має неявно повернути `dropdown.value`.
