...
接下来，编写一个 `if` 语句来查看 `prevToggleItem` 是否不严格相等于 `toggleItem`。如果它们不相等，向控制台记录字串 `New toggleItem function`。然后，将 `prevToggleItem` 设为等于 `toggleItem`。

# --hints--

你应该编写一个 `if` 语句来查看 `prevToggleItem` 是否不严格相等于 `toggleItem`。

```js
assert.match(code, /if\s*\(\s*prevToggleItem\s*!==\s*toggleItem\s*\)/);
```

如果条件满足，你应该在 `if` 语句内将字串 `New toggleItem function` 记录到控制台。
...
如果条件满足，你应该在 `if` 语句内将 `prevToggleItem` 设置为与 `toggleItem` 相等。
