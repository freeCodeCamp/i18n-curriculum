...
次に、`Position` という型を `typeof POSITIONS[number]` の値で作成してください。これは `POSITIONS` 配列を個々の文字列リテラルのユニオンに展開し、実質的に `"GK" | "CB" | "LB" | ...` のように見えるようにします。

# --hints--

`Position` という型エイリアスを作成してください。
...
`Position` 型は `typeof POSITIONS[number]` に設定する必要があります。
