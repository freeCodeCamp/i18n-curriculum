...
Du solltest `.shirt div` ein `top` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt div')?.top, '22.5px');
```

Du solltest `.shirt div` ein `left` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.
