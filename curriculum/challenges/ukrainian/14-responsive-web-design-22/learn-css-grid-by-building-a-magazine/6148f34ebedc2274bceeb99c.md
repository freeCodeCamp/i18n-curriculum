...
Макет журналу готовий, тож його потрібно зробити адаптивним.
...
Ви повинні мати нове правило `@media` для `only screen and (max-width: 720px)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

Ваше нове правило `@media` повинне мати селектор `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

Ваш новий селектор `.image-wrapper` повинен мати властивість `grid-template-columns` зі значенням `1fr`.
