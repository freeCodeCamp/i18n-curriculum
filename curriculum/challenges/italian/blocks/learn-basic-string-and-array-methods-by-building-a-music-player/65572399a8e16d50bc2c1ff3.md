...
Se c'è una corrispondenza, imposta `userData.currentSong` su `null` e `userData.songCurrentTime` su `0`.

Dopodiché, chiama la funzione `pauseSong()` per fermare la riproduzione e la funzione `setPlayerDisplay()` per aggiornare la visualizzazione del lettore.
