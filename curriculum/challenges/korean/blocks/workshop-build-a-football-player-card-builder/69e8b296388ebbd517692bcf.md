...
`div`의 `className` `form-row`를 추가하세요. 그 안에 각각 `div`의 `className`를 가진 두 개의 `form-group` 요소를 추가하세요.

첫 번째 `div` 요소는 `label`의 `className` `label`, `htmlFor`의 `position` 속성, 텍스트 `Position`, 그리고 `input`의 `id`를 가진 `position`, `className`의 `input`, `type`의 `text`를 가져야 합니다.

# --hints--

Player Info 섹션에는 `div`의 `className`를 가진 `form-row`가 있어야 합니다.
...
`div.form-row`에는 각각 `div`의 `className`를 가진 두 개의 `form-group` 요소가 있어야 합니다.
...
행의 첫 번째 `form-group`에는 `label`의 `className`를 가진 `label`와 `htmlFor`의 `position`가 있어야 합니다.
...
position `label`에는 텍스트 `Position`가 있어야 합니다.
...
행의 첫 번째 `form-group`에는 `input`의 `className`를 가진 `input`, `id`의 `position`, 그리고 `type`의 `text`가 있어야 합니다.
