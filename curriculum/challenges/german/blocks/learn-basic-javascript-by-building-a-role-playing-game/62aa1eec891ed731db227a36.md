...
Wenn du das Spiel in seinem aktuellen Zustand spielst, wirst du möglicherweise einen Fehler bemerken. Wenn dein `xp` hoch genug ist, wird die `getMonsterAttackValue` Funktion eine negative Zahl zurückgeben, die deine Gesundheit im Kampf gegen ein Monster tatsächlich erhöht! You can fix this issue by using a <dfn>ternary operator</dfn> to ensure negative values are not returned.

The ternary operator is a conditional operator and can be used as a one-line `if-else` statement. Die Syntax lautet: `condition ? expressionIfTrue : expressionIfFalse`.
...
Ändere in `getMonsterAttackValue` `return hit` zu einem ternären Operator, der `hit` zurückgibt, wenn `hit` größer ist als `0` oder `0` zurückgibt, wenn dies nicht der Fall ist.
