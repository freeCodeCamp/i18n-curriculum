...
Concatena un altro metodo `.then()` al metodo `.then()` esistente. Stavolta passa `data` come parametro per la funzione callback. For the callback, use curly braces because you will have more than one expression. All'interno della funzione callback, mostra `data` sulla console per vedere che aspetto ha.
