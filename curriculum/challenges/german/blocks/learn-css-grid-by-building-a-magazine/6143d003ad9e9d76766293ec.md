...
Verschachteln Sie innerhalb Ihres `.image-quote`-Elements ein `hr`-Element, ein `p`-Element und ein zweites `hr`-Element. Geben Sie dem `p`-Element ein `class`, das auf `quote` gesetzt ist, und den Text `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.

# --hints--

Sie sollten zwei `hr`-Elemente zu Ihrem `.image-quote`-Element hinzufügen.

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

Sie sollten ein `p`-Element zu Ihrem `.image-quote`-Element hinzufügen.

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

Ihre `.image-quote`-Kinder sollten in der richtigen Reihenfolge sein.
...
Ihr neues `p`-Element sollte ein `class` haben, das auf `quote` gesetzt ist.

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Ihr neues `p`-Element sollte den Text `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.` haben.
