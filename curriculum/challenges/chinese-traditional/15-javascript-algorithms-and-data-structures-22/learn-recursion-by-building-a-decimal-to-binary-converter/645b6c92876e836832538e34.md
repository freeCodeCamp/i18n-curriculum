...
接下來，在控制檯記錄文本 `"Quotients: "`，後面依次是逗號和 `quotients` 數組。 同時記錄文本 `"Remainders: "`，後跟逗號和 `remainders` 數組。

# --hints--

你應該將文本 `"Quotients: "` 和 `quotients` 數組記錄到控制檯，以逗號分隔。

```js
assert.match(String(decimalToBinary), /console\.log\(\s*('|"|`)Quotients:\s*\1\s*,\s*quotients\s*\)/i);
```

你應該將文本 `"Remainders: "` 和 `remainders` 數組記錄到控制檯，以逗號分隔。
