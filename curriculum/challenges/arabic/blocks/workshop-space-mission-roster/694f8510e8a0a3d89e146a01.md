...
لإنشاء أجزاء بحجم `n` من `array` دون تغييره، يمكنك دمج حلقة `for` مع طريقة `slice()` كما يلي:
...
- ينشئ مصفوفة فارغة باسم `result`

- يمر عبر `array` الأصلية في خطوات بحجم `n`

- ينشئ قطعًا من `array` بحجم `n` ويدفعها إلى `result` باستخدام `slice()`

لإكمال دالة `chunkCrew`، يجب عليك:

- إنشاء مصفوفة باسم `chunks`

- المرور عبر `crew` بخطوات بحجم `size`، وإنشاء ودفع أجزاء من `crew` إلى `chunks` باستخدام `slice()`

- إرجاع مصفوفة `chunks`

# --hints--

يجب إنشاء مصفوفة فارغة باسم `chunks`.
...
يجب استخدام حلقة `for` لبناء الأجزاء.
...
داخل حلقة `for`، يجب استدعاء `push()` على مصفوفة `chunks` لإدخال كل جزء مقطوع (يجب إنشاؤه باستدعاء `slice()` على مصفوفة `crew`).
...
بعد الحلقة، يجب إرجاع مصفوفة `chunks`.
...
يجب أن تُرجع دالة `chunkCrew` أجزاءً بطول `size` من المصفوفة المدخلة `crew` دون تعديلها.
