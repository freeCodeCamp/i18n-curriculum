...
添加一个 `firsttwo` 属性，该属性接收一个 `nums` 参数，并按顺序返回 `nums` 数组的前两个元素。 然后添加一个 `lasttwo` 属性，它按顺序返回 `nums` 数组的最后两个元素。

# --hints--

你的 `spreadsheetFunctions` 对象应该具有 `firsttwo` 属性。

```js
assert.property(spreadsheetFunctions, "firsttwo");
```

你的 `firsttwo` 属性应该是一个函数。

```js
assert.isFunction(spreadsheetFunctions.firsttwo);
```

你的 `firsttwo` 函数应该返回数组的前两个数字。

```js
assert.deepEqual(spreadsheetFunctions.firsttwo([1, 2, 3, 4, 5, 6]), [1, 2]);
```

你的 `spreadsheetFunctions` 对象应该具有 `lasttwo` 属性。

```js
assert.property(spreadsheetFunctions, "lasttwo");
```

你的 `lasttwo` 属性应该是一个函数。

```js
assert.isFunction(spreadsheetFunctions.lasttwo);
```

你的 `lasttwo` 函数应该返回数组的最后两个数字。
