...
每當你點擊播放按鈕時，你的播放列表中的第一首歌曲將會一直播放。若要修正此問題，請在事件監聽器的箭頭函式中，將 `playSong(0)` 替換為一個 `if` 述語，用來檢查 `userData.currentSong` 是否為 `null`。

在 `if` 區塊內，使用 `userData.songs` 陣列中第一首歌曲的 `id` 呼叫 `playSong` 函式（程式）。

# --hints--

你應該修改事件監聽器的回呼，以便在 `userData.currentSong` 為 `null` 時，使用 `userData.songs` 陣列中第一首歌曲的 `id` 來呼叫 `playSong` 函式（程式）。
