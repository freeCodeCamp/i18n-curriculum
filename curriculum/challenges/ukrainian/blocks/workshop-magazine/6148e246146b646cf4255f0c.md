...
Створіть селектор `.list-title, .list-subtitle` і встановіть властивість `color` у значення `#00beef`.

# --hints--

У вас має бути селектор `.list-title, .list-subtitle`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'));
```

Ваш селектор `.list-title, .list-subtitle` повинен мати властивість `color`, встановлену в `#00beef`.
