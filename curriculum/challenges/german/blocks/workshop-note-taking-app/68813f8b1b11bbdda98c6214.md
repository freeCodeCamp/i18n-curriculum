...
Da ein `div`-Element für die Notiz verwendet wird, verstehen Screenreader dessen Zweck nicht. Sie können das beheben, indem Sie ein `aria-label`-Attribut hinzufügen. Dies verbessert die Barrierefreiheit und gibt diesem `div`-Element eine Bedeutung.

Fügen Sie in Ihrem öffnenden `div`-Tag ein `aria-label`-Attribut hinzu und setzen Sie dessen Wert auf `"Note editor"`.

# --hints--

Ihr öffnender `div`-Tag sollte das `aria-label`-Attribut haben.

```js
assert.isTrue(document.getElementById("note").hasAttribute("aria-label"));
```

Ihr öffnender `div`-Tag sollte das `aria-label`-Attribut mit dem Wert `"Note editor"` haben.
