...
1. 양식에는 사용자의 전체 이름을 입력할 수 있도록 타입이 `text`이고 id가 `name`인 `input` 요소가 포함되어야 합니다.  
1. 사용자의 이메일 주소를 입력할 수 있도록 타입이 `email`이고 id가 `email`인 또 다른 `input` 요소가 있어야 합니다.  
1. 양식에는 사용자가 직무를 선택할 수 있도록 id가 `position`인 `select` 요소가 포함되어야 합니다.  
1. `fieldset` 요소에 `radio-group` 클래스가 있어야 합니다.  
1. `.radio-group` 안에는 타입이 `radio`인 `input` 요소 세트가 있어야 하며, 관련 레이블이 있어야 하고 (예: 풀타임, 파트타임) 그룹 `name`은 `availability`이어야 합니다.  
1. 메시지를 입력할 수 있도록 id가 `message`인 `textarea` 요소가 있어야 합니다.  
1. 모든 `input`, `select`, `textarea` 요소는 `label` 요소와 연결되어야 합니다.  
1. 양식을 제출할 수 있도록 타입이 `submit`인 `button` 요소가 있어야 합니다.  
1. `input`과 `textarea` 요소에 `:focus` 가상 클래스를 추가하여 포커스 시 테두리 색상이 바뀌고 기본 윤곽선이 제거되도록 하세요.  
...
1. 라디오 버튼이 선택되었을 때 테두리 색상, 배경색, 박스 그림자가 추가되도록 `.radio-group input[type="radio"]`에 `:checked` 가상 클래스를 사용하세요.  
1. 라디오 버튼이 선택되었을 때 관련 `label`의 텍스트 색상이 바뀌도록 라디오 버튼에 `:checked` 가상 클래스를 사용하세요.  
1. 첫 번째 입력 필드를 다르게 스타일링하려면 `input` 요소에 `:first-of-type` 가상 클래스를 추가하세요. (예: 둥근 모서리)
...
양식에는 사용자의 전체 이름을 입력할 수 있도록 타입이 `text`이고 id가 `name`인 `input` 요소가 포함되어야 합니다.

```js
assert.exists(document.querySelector("div.container > form input#name[type='text']"));
```

사용자의 이메일 주소를 입력할 수 있도록 타입이 `email`이고 id가 `email`인 또 다른 `input` 요소가 있어야 합니다.

```js
assert.exists(document.querySelector("div.container > form input#email[type='email']"));
```

양식에는 일부 `option` 요소가 포함된 id가 `position`인 `select` 요소가 있어야 합니다.
...
메시지를 입력할 수 있도록 id가 `message`인 `textarea` 요소가 있어야 합니다.

```js
assert.isNotEmpty(document.querySelectorAll("div.container > form textarea#message"));
```

모든 `input`, `select`, `textarea` 요소는 `label` 요소와 연결되어야 합니다.
...
`input`과 `textarea` 요소에 `:focus` 가상 클래스를 추가하여 포커스 시 테두리 색상이 바뀌도록 하세요. 주어진 순서대로 리스트 선택자를 사용하세요.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('input:focus, textarea:focus').getPropertyValue('border-color'));
```

`input`과 `textarea` 요소에 `:focus` 가상 클래스를 사용하여 포커스 시 기본 윤곽선이 제거되도록 하세요.
...
첫 번째 입력 필드를 다르게 스타일링하려면 `input` 요소에 `:first-of-type` 가상 클래스를 추가하세요.
