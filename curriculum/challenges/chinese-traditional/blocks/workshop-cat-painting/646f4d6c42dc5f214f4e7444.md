...
現在，你要將右側的鬍鬚移到合適的位置。 使用類選擇器選擇 `.cat-whisker-right-top` 元素，設置其 `position` 爲 `absolute`、`top` 爲 `120px`、`left` 爲 `109px`。
