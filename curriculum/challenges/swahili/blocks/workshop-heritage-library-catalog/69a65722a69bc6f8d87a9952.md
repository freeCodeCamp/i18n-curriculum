...
JSON (JavaScript Object Notation) ni muundo wa kawaida wa kushiriki data kati ya mifumo. `JSON.stringify` hubadilisha thamani ya JavaScript kuwa mfuatano wa herufi ulio katika muundo wa JSON.

Anza kwa kuondoa wito mbili za `validateEntry` console.log, kwani zilihitajika tu kwa ajili ya majaribio. Kisha tengeneza kitendakazi kinachoitwa `exportToJSON` kinachopokea `catalog` kama kigezo. Rudisha `JSON.stringify(catalog, null, 2)`. Hoja ya kwanza ni data ya kubadilisha. Hoja ya pili (`null`) ina maana hakuna uchujaji maalum. Hoja ya tatu (`2`) huongeza nafasi mbili za kuingiza, na kufanya matokeo yawe rahisi kusoma.

Chini ya kitendakazi, andika `exportToJSON(catalog.slice(0, 2))` ili kuonyesha awali matokeo ya JSON kwa maingizo mawili ya kwanza.

# --hints--

Unapaswa kuondoa wito za `validateEntry` console.log.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry/);
```

`exportToJSON` inapaswa kurudisha mfuatano wa herufi.

```js
assert.isString(exportToJSON(catalog));
```

`JSON.parse(exportToJSON(catalog))` inapaswa kutoa safu ya data halali.

```js
assert.isArray(JSON.parse(exportToJSON(catalog)));
```

Unapaswa kutumia `JSON.stringify` na `null` na `2` kama hoja ya pili na ya tatu.

```js
assert.match(exportToJSON.toString(), /JSON\.stringify\s*\(\s*catalog\s*,\s*null\s*,\s*2\s*\)/);
```

Unapaswa kuandika `exportToJSON(catalog.slice(0, 2))` kwenye konsoli.
