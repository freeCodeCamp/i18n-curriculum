...
يجب أن يحتفظ مشغل الموسيقى الخاص بك بسجل الأغاني، والأغنية الحالية التي تُشغل، ووقت الأغنية الحالية. للقيام بذلك، تحتاج إلى إنشاء كائن لتخزين هذه المعلومات.

ابدأ بتعريف متغير جديد اسمه `userData` وقم بتعيين كائن فارغ له.

# --hints--

يجب أن يكون لديك متغير اسمه `userData`.

```js
assert.isDefined(userData);
```

يجب أن تعيّن كائنًا فارغًا إلى متغير `userData`.
