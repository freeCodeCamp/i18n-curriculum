...
يجب أن يكون لديك استعلام `@media` جديد لـ `only screen` مع `max-width` بقيمة `550px`. يجب أن يأتي هذا بعد استعلاميك السابقين.
