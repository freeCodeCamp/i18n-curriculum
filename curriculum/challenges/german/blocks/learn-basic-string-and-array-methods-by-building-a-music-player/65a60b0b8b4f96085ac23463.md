...
Im Beispiel, wenn `a.name` gleich `b.name` ist, gibt die Funktion `0` zurück. Das bedeutet, dass sich nichts ändert und die Reihenfolge von `a` und `b` gleich bleibt.
...
Unter Ihren `if`-Anweisungen geben Sie die Zahl `0` zurück, um die Reihenfolge der beiden Elemente unverändert zu lassen.

# --hints--

Sie sollten die Zahl `0` unter Ihren `if`-Anweisungen zurückgeben.
