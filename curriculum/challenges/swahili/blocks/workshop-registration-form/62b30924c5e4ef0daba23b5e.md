...
Mpaka wa kipengele cha mwisho cha `fieldset` unaonekana kidogo kuwa hauna mpangilio mzuri. 

Katika mafundisho yaliyopita, ulijifunza jinsi ya kufanya kazi na darasa la bandia la `last-of-type` CSS kama ifuatavyo: 

```css
p:last-of-type { }
```

Hilo litatumia kuchagua kipengele cha mwisho cha `p`.

Tengeneza kichaguzi kipya kinacholenga kipengele cha mwisho cha `fieldset` na weka `border-bottom` yake kuwa `none`.

# --hints--

Unaweza kutumia darasa la bandia la `:last-of-type` kuchagua kipengele cha mwisho tu cha aina fulani.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type'));
```

`fieldset:last-of-type` yako inapaswa kuwa na `border-bottom` iliyowekwa kama `none`.
