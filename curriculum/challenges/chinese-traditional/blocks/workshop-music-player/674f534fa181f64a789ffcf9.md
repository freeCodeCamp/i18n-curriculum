...
如果 `userData.currentSong` 不是 `null`，你需要找到下一首要播放的歌曲。請記得你可以使用 `getNextSong` 來達成這個目的。

完成你的 `playNextSong` 函式（程式），使其播放播放列表中的下一首歌曲。如果目前的歌曲是最後一首，將 `userData.currentSong` 設為 `null`，`userData.songCurrentTime` 設為零，並呼叫 `pauseSong` 函式（程式）。

# --hints--

當 `userData` 的 `currentSong` 為真值且不是播放列表中的最後一首歌曲時，`playNextSong` 函式（程式）應該播放緊接在目前歌曲之後的歌曲。 
...
當 `userData` 的 `currentSong` 是播放列表的最後一首歌曲時，`playNextSong` 函式應該將 `userData.currentSong` 設為 `null`。
...
當 `userData` 的 `currentSong` 是播放列表的最後一首歌時，`playNextSong` 函式（程式）應該將 `userData.songCurrentTime` 設定為 `0`。
...
當 `userData` 的 `currentSong` 是播放列表的最後一首歌曲時，`playNextSong` 函式應該呼叫 `pauseSong` 函式。
...
當 `userData` 的 `currentSong` 相等於 `null` 時，你的 `playNextSong` 函式（程式）仍然應該以 `userData.songs` 陣列中第一首歌曲的 `id` 作為引數呼叫 `playSong` 函式（程式）。
