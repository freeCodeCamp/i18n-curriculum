...
Quando você precisar converter uma string para letras minúsculas, pode usar o método <dfn>toLowerCase()</dfn>.Este método retorna o valor da string que chamou convertido para letras minúsculas.
...
A string de `output.innerHTML` precisará de um elemento `span`.Crie isso e dê a ele um atributo `class` definido pela variável `surplusOrDeficit`.Sua variável `surplusOrDeficit` deve ser convertida para letras minúsculas usando o método `toLowerCase()`.
