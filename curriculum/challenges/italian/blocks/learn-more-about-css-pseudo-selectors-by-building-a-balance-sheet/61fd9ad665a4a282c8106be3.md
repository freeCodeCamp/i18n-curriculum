...
All'interno del tuo `thead`, crea un elemento `tr`. Al suo interno, aggiungi un `td` e tre elementi `th`. All'interno di ciascuno degli elementi `th`, aggiungi un elemento `span` con l'attributo `class` impostato su `sr-only` e il seguente testo, in ordine: `2019`, `2020` e `2021`.

# --hints--

Il tuo elemento `thead` dovrebbe avere un elemento `tr`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('thead')?.children?.[0]?.localName === 'tr');
```

Il tuo elemento `tr` dovrebbe avere un elemento `td` come primo figlio.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('tr')?.children?.[0]?.localName === 'td');
```

Il tuo elemento `tr` dovrebbe avere tre elementi `th`, dopo l'elemento `td`.
...
Ognuno dei tuoi nuovi elementi `span` dovrebbe avere l'attributo `class` impostato su `sr-only`.
