...
Setzen Sie Ihre `password` Variablendeklaration und die folgende `for` Schleife in eine `while` Schleife. Verwenden Sie `True` als Bedingung für Ihre neue Schleife.

# --hints--

Sie sollten eine `while True` Schleife erstellen, die Ihre bestehende `password` Deklaration und `for` Schleife umschließt.
