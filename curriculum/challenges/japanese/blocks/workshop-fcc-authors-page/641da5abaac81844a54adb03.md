...
次は、ページに著者を表示する役割を持つ関数を作成する時間です。

まず、`displayAuthors` という名前の関数を作成し、`authors` パラメータを受け取るようにしてください。関数内で `authors` 配列を繰り返し処理し、各著者のカードを `authorContainer` 要素に追加してください。
...
- `authors` 配列を繰り返すために、どのタイプのループでも使えます。例：`for`、`forEach`、`map` など。
- ループ内でカードを `innerHTML` に追加するには `authorContainer` プロパティを使ってください。また、マークアップを追加するにはテンプレートリテラル構文を使う必要があります。
...
`displayAuthors` は関数である必要があります。

```js
assert.isFunction(displayAuthors)
```

`displayAuthors` 関数は `authors` パラメータを受け取る必要があります。
...
`displayAuthors` 関数が著者の配列で呼び出されたとき、各著者のカードを `#authorContainer` 要素に追加する必要があります。
