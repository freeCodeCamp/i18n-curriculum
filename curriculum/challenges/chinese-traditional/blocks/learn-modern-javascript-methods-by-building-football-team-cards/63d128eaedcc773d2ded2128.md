...
你的 `myFavoriteFootballTeam.players` 陣列應該有兩個值。
...
你的 `myFavoriteFootballTeam.players` 陣列的第二個值應該是一個物件。
...
你的 `name` 屬性應該設定為 `"Sergio Batista"`。
...
你的 `position` 屬性應該設定為 `"midfielder"`。
