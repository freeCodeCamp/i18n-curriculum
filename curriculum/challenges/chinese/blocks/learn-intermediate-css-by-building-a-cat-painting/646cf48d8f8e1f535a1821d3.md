...
将右耳移动到位，其 `position` 属性为 `absolute`，`top` 为 `-26px`，`left` 为 `163px`。

# --hints--

你的 `.cat-right-ear` 选择器应该有一个 `position` 属性，设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.position === 'absolute')
```

你的 `.cat-right-ear` 选择器应该有一个 `top` 属性，设置为 `-26px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.top === '-26px')
```

你的 `.cat-right-ear` 选择器应该有一个 `left` 属性，设置为 `163px`。
