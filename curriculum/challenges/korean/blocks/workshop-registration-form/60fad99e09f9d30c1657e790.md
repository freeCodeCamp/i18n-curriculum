...
`form` 요소를 `margin` 값의 `0 auto`으로 가운데 정렬하세요. 그런 다음 최대 너비를 `500px`, 최솟값 너비를 `300px`로 고정하세요. 그 범위 내에서는 `width` 값을 가진 `60vw`를 허용하세요.

# --hints--

`form` 요소를 스타일링하려면 `form` 선택자를 사용하세요.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('form'));
```

`form`에 `margin` 값을 가진 `0 auto`을 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.margin, '0px auto');
```

`form`에 `max-width` 값을 가진 `500px`을 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.maxWidth, '500px');
```

`form`에 `min-width` 값을 가진 `300px`을 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.minWidth, '300px');
```

`form`에 `width` 값을 가진 `60vw`을 지정하세요.
