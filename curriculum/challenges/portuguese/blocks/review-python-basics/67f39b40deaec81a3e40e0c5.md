...
- **Definição**: Como você se lembra do JavaScript, strings são imutáveis o que significa que você não pode alterá-las depois que elas foram criadas. Em Python, você pode usar aspas simples ou duplas.  It is recommended to choose a rule and stick with it:
...
The start position represents the index where the extraction should begin. A posição de parada é onde o slice deve terminar. Esta posição não é inclusiva. A posição do passo representa o intervalo para incrementar no fatiamento. Aqui estão alguns exemplos:
...
- **Declaração `elif`**: Estas são condições que vêm depois de uma declaração `if`. Um bloco `elif` é executado somente se todas as condições anteriores avaliarem para `False` e sua própria condição avaliar para `True`.
