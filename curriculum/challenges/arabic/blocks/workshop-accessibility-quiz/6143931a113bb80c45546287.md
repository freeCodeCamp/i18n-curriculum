...
ثم، داخل كل عنصر `div.question-block`، أضف عنصرًا واحدًا من نوع `h3` بنص أرقام متزايدة، بدءًا من `1`، وعنصرًا واحدًا من نوع `fieldset` مع صنف `question`.
...
يجب أن تعطي أول عنصر `div` جديد صنفًا هو `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[0]?.className, 'question-block');
```

يجب أن تعطي ثاني عنصر `div` جديد صنفًا هو `question-block`.
