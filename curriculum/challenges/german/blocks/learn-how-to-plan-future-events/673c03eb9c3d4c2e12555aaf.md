---
id: 673c03eb9c3d4c2e12555aaf
title: Aufgabe 17
challengeType: 22
dashedName: task-17
lang: en-US
---
<!-- (Audio) Jake: Our main goal is to minimize disruption while strengthening our defenses. -->

# --instructions--

Hören Sie sich die Audioaufnahme an und vervollständigen Sie den folgenden Satz.

# --fillInTheBlank--

## --sentence--

`Our main goal is to minimize disruption while BLANK our defenses.`

## --blanks--

`strengthening`

### --feedback--

Dieses Wort bezeichnet den Prozess, etwas stärker oder sicherer zu machen. Es endet auf `-ing`.

# --explanation--

`Strengthening` beschreibt den fortlaufenden Prozess, etwas stärker zu machen. Zum Beispiel:  

`We are strengthening our defenses to improve security.` – Das bedeutet, aktiv daran zu arbeiten, die Verteidigung besser und sicherer zu machen.

# --scene--

```json
{
  "setup": {
    "background": "company2-boardroom.png",
    "characters": [
      {
        "character": "Jake",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_5-1.mp3",
      "startTime": 1,
      "startTimestamp": 20.32,
      "finishTimestamp": 24.72
    }
  },
  "commands": [
    {
      "character": "Jake",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Jake",
      "startTime": 1,
      "finishTime": 5.4,
      "dialogue": {
        "text": "Our main goal is to minimize disruption while strengthening our defenses.",
        "align": "center"
      }
    },
    {
      "character": "Jake",
      "opacity": 0,
      "startTime": 5.9
    }
  ]
}
```
