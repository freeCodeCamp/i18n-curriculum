...
接下来，你将需要使用 `document.querySelector` 来获取 `.start-screen` 和 `.checkpoint-screen` 元素。

将它们分别存储在名为 `startScreen` 和 `checkpointScreen` 的 `const` 变量中。
...
你应该获取具有 `start-screen` 的 `class` 的元素。 不要忘记在 `document.querySelector` 方法中包含 `.`。

```js
assert.match(code, /document\.querySelector\(\s*('|"|`)\.start-screen\1\s*\)/);
```

你应该使用 `const` 声明名为 `startScreen` 的变量，并将 `.start-screen` 元素分配给它。

```js
assert.match(code, /const\s+startScreen\s*=\s*document\.querySelector\(\s*('|"|`)\.start-screen\1\s*\)/);
```

你应该使用 `document.querySelector()` 方法来获取 `.checkpoint-screen` 元素。

```js
assert.match(code, /document\.querySelector\(\s*('|"|`)\.checkpoint-screen\1\s*\)/);
```

你应该使用 `const` 声明名为 `checkpointScreen` 的变量，并将 `.checkpoint-screen` 元素分配给它。
