...
創建一個名為 `highlightCurrentSong` 的函式（程式）。在該函式（程式）內，檢查是否存在具有 `aria-current="true"` 的 `.playlist-song` 元素。如果存在，使用 `removeAttribute` 方法移除該元素的 `aria-current` 屬性。

# --hints--

你應該有一個名為 `highlightCurrentSong` 的函式（程式）。

```js
assert.isFunction(highlightCurrentSong)
```

你的 `highlightCurrentSong` 函式應該使用 `removeAttribute` 方法，從具有 `aria-current="true"` 的 `.playlist-song` 元素中移除 `aria-current` 屬性（如果該元素存在）。
