...
أنشئ محدِّد `.cabin`. عيّن `background-color` إلى `red`، و`width` إلى `20%`، و`height` إلى `20%`.

# --hints--

يجب أن يكون لديك محدِّد `.cabin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cabin'));
```

يجب أن يحتوي محدِّد `.cabin` على خاصية `background-color` مضبوطة على `red`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cabin')?.backgroundColor, 'red');
```

يجب أن يحتوي محدِّد `.cabin` على خاصية `width` مضبوطة على `20%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cabin')?.width, '20%');
```

يجب أن يحتوي محدِّد `.cabin` على خاصية `height` مضبوطة على `20%`.
