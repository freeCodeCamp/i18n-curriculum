...
Crie uma função chamada `highlightCurrentSong`. Dentro da função, verifique se existe um elemento `.playlist-song` com `aria-current="true"`. Se for o caso, use o método `removeAttribute` para remover o atributo `aria-current` dele.

# --hints--

Você deve ter uma função chamada `highlightCurrentSong`.

```js
assert.isFunction(highlightCurrentSong)
```

Sua função `highlightCurrentSong` deve usar o método `removeAttribute` para remover o atributo `aria-current` do elemento `.playlist-song` com `aria-current="true"` se esse elemento existir.
