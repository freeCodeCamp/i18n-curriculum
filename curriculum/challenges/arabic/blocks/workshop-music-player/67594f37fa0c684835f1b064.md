...
عندما تنقر على أغنية في قائمة التشغيل تريد تشغيل تلك الأغنية. لذلك تحتاج إلى إضافة مستمع حدث لحدث `click` على كل عنصر `button` وهو طفل لعنصر `song`.

أكمل استدعاء الراجعة في `forEach` للقيام بذلك. داخل استدعاء الراجعة لـ `addEventListener`، نفذ استدعاء `playSong(n)`، حيث `n` هو معرف الأغنية الذي يمكنك الحصول عليه من خاصية `id` لعنصر `song`.

لاحظ أن كل عنصر `song` لديه خاصية `id` على شكل `song-n`: تحتاج إلى استخراج الجزء `n` وتحويله إلى رقم قبل تمريره إلى `playSong()`.


# --hints--

يجب إضافة مستمع حدث لحدث `click` على كل عنصر `button` وهو طفل لعنصر `.playlist-song` وتنفيذ استدعاء `playSong` باستخدام `id` للأغنية المقابلة.
