...
الشيء التالي الذي ستعرضه هو التفاصيل البيوغرافية عن المؤلف. يمكنك فعل ذلك باستخدام الخاصية `bio`.

أضف عنصر فقرة مع خاصية `class` بقيمة `"bio"`، ثم أدخل `bio` داخل عنصر الفقرة.
...
يجب أن تدخل `bio` في عنصر `p` الخاص بك.
