...
Da das Magazinlayout nun fertig ist, müssen Sie es responsiv machen.

Beginnen Sie mit einer `@media`-Abfrage für `only screen` mit einer `max-width` von `720px`. Erstellen Sie darin einen `.image-wrapper`-Selektor und geben Sie ihm eine `grid-template-columns`-Eigenschaft von `1fr`.

Dadurch werden die drei Bilder auf kleineren Bildschirmen zu einer Spalte zusammengeklappt.

# --hints--

Sie sollten eine neue `@media`-Regel für `only screen and (max-width: 720px)` haben.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

Ihre neue `@media`-Regel sollte einen `.image-wrapper`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

Ihr neuer `.image-wrapper`-Selektor sollte eine `grid-template-columns`-Eigenschaft von `1fr` haben.
