---
id: 657e2c02a685f4ec9ec951ff
title: 任務 38
challengeType: 22
dashedName: task-38
lang: en-US
---

<!-- (audio) Anna: You have a one-hour lunch break around noon every day. Bring your lunch or go to one of the cafés nearby. -->

# --description--

`Lunch break` 是工作日中專門用來吃午餐的時間段。例如：

`I usually take my lunch break at 1:00 PM.`－這表示你通常在下午 1:00 用午餐並休息。

`Noon` 是指下午 12:00，正午。 例如：

`We have our team meeting at noon.`－這表示團隊會議在 12:00 PM 開始。

# --fillInTheBlank--

## --sentence--

`You have a one-hour BLANK break around BLANK every day. BLANK your lunch or go to one of the BLANK nearby.`

## --blanks--

`lunch`

### --feedback--

這是一餐通常在一天中午間食用。

---

`noon`

### --feedback--

這是指中午或 12 PM 的術語。

---

`Bring`

### --feedback--

這個動詞表示帶著某物。第一個字母為大寫。

---

`cafés`

### --feedback--

這個字的複數形式取用的是你可以購買和享用食物的地方。別忘了重音符號（`é`）。

# --scene--

```json
{
  "setup": {
    "background": "interview-room3.png",
    "characters": [
      {
        "character": "Anna",
        "position": {"x":50,"y":15,"z":1.2},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "2.1-2.mp3",
      "startTime": 1,
      "startTimestamp": 32.41,
      "finishTimestamp": 37.48
    }
  },
  "commands": [
    {
      "character": "Anna",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Anna",
      "startTime": 1,
      "finishTime": 3.66,
      "dialogue": {
        "text": "You have a one-hour lunch break around noon every day.",
        "align": "center"
      }
    },
    {
      "character": "Anna",
      "startTime": 3.82, 
      "finishTime": 6.07,
      "dialogue": {
        "text": "Bring your lunch or go to one of the cafés nearby.",
        "align": "center"
      }
    },
    {
      "character": "Anna",
      "opacity": 0,
      "startTime": 6.57
    }
  ]
}
```
