...
1. Debes tener una función llamada `generateArray` que use la función `generateElement` para devolver un arreglo que contenga cinco números enteros aleatorios.
1. Debes tener una función llamada `generateContainer` que cree y devuelva un elemento `div` vacío.
1. Debes tener una función llamada `fillArrContainer` que reciba un elemento HTML como primer argumento y un arreglo como segundo argumento.
1. La función `fillArrContainer` debe recibir un elemento como su primer parámetro y un arreglo de enteros como su segundo parámetro, luego debe llenar el elemento con cinco elementos `span`, donde cada `span` muestre uno de los enteros del arreglo.
1. Debes tener una función llamada `isOrdered` que reciba dos enteros y devuelva un booleano indicando si el primer entero es menor o igual que el segundo.
1. Debes tener una función llamada `swapElements` que reciba un arreglo de enteros y un índice numérico.
1. La función `swapElements` debe modificar el arreglo en el lugar intercambiando el elemento en el índice pasado y el elemento siguiente si `isOrdered` devuelve `false`.
1. Debes tener una función llamada `highlightCurrentEls` que reciba un elemento HTML y un índice numérico.
1. La función `highlightCurrentEls` debe establecer el `border` del hijo del elemento dado en el índice indicado y el hijo inmediatamente después del índice, para que tengan un estilo `dashed`, color `red` y un ancho a tu elección.
1. Cuando hagas clic en `#generate-btn` debes usar la función `fillArrContainer` para llenar `#starting-array` con cinco elementos `span`, cada uno con un número aleatorio como texto. Si hay otros elementos presentes, deben eliminarse de `#array-container`.
1. Debes implementar el algoritmo Bubble Sort para que, después de hacer clic en `#sort-btn`, `#array-container` contenga un elemento `div` por cada uno de los pasos requeridos por el algoritmo Bubble Sort para ordenar el arreglo inicial, incluyendo el `div` que representa el arreglo inicial y un `div` que representa el arreglo ordenado. Las funciones que has creado hasta ahora pueden ser útiles aquí.
1. Cada `div` debe contener cinco elementos `span`, representando el arreglo en su estado actual de ordenamiento.
1. Después de hacer clic en `#sort-btn`, `#starting-array` debe representar el paso inicial con el arreglo original y los dos primeros enteros resaltados.
1. Para cada paso de ordenamiento, debes usar `highlightCurrentEls` para resaltar los dos números que se están comparando y luego intercambiarlos en el siguiente paso usando `swapElements`.
...
Tu función `highlightCurrentEls` debe dar a los descendientes del elemento especificado, ubicados en el índice dado y en el índice siguiente, un borde que sea `dashed`, `red` y con un ancho a tu elección.
