...
Ndani ya kitendakazi cha `displayStory`, angalia kama kitu cha `storyObj` kina sifa inayolingana na kigezo cha `genre` na weka maudhui ya maandishi ya aya ya `#result` kuwa hadithi ya aina iliyochaguliwa.

Pia, weka rangi ya mpaka ya `storyContainer` kuwa sifa ya `borderColor` ya aina iliyochaguliwa.

Jaribu hili kwa kuita kitendakazi cha `displayStory` na aina ya `scary`. Rangi ya mpaka inapaswa kuwa `#ee4b2b` na hadithi inapaswa kuwa `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.

# --hints--

Unapaswa kuita kitendakazi cha `displayStory` na aina ya `"scary"`.

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

Unapoitisha `displayStory("scary")`, rangi ya mpaka inapaswa kuwa `#ee4b2b`.
...
Unapoitisha `displayStory("scary")`, hadithi inayotolewa inapaswa kuwa `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.
