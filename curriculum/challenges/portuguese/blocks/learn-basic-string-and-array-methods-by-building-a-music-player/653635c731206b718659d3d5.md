...
Dentro da função `playSong`, defina a propriedade `audio.src` igual a `song.src`.Isso informa ao elemento de áudio onde encontrar os dados de áudio para a música selecionada.
...
Você não deve modificar a função `playSong` existente e seu conteúdo.
...
Você deve definir `audio.src` para `song.src`.
...
Você deve definir `audio.title` para `song.title`.
