...
Тепер ви почнете процес заміни поля введення позиції на вибір.

Змініть поле введення на елемент `select` з ідентифікатором `position`, `className` зі значенням `input` та `value` зі значенням `player.position`.

# --hints--

Ви маєте змінити поле введення позиції `input` на елемент `select` з `id` зі значенням `position` та `className` зі значенням `input`.
...
Ваше поле введення позиції `select` має мати `value` зі значенням `player.position`.
