...
En el tiempo `Present Perfect`, se usan `for` y `since` para hablar de la duración del tiempo y puntos de inicio. 

`For` indica cuánto tiempo ha estado ocurriendo algo. Por ejemplo, `for five years` o `for two months`.

`Since` indica cuándo comenzó algo. Por ejemplo, `since 2020` o `since last week`.
...
Esta preposición expresa la duración de una acción o situación.
