...
Nach Ihrer letzten Trennlinie erstellen Sie ein weiteres `p`-Element mit dem Text `Protein 3g`. Verwenden Sie die notwendigen Klassen, um den unteren Rahmen zu entfernen, und ein `span`, um den `Protein` fett zu machen. Wickeln Sie dann den Text `Protein 3g` einschließlich des neuen `span`-Elements in ein neues `span`-Element ein.

Erstellen Sie nach diesem Element eine große Trennlinie.
...
Ihr neues `div` sollte das `class`-Attribut mit dem Wert `divider large` haben.
