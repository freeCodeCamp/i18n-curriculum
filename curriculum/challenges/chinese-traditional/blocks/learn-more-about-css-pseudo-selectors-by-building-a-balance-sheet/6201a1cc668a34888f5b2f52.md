...
選取 `class` 設為 `data` 的 `tr` 元素。為它們添加 `linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)` 的背景影像。
...
你的 `tr.data` 選擇器應該將 `background-image` 屬性設為 `linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)`。
