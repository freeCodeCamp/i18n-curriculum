...
Sotto il titolo principale della pagina, c’è una sezione di navigazione che contiene collegamenti a ogni post del blog.

Questa sezione dovrebbe essere racchiusa in un elemento `nav` per indicarne lo scopo come punto di riferimento di navigazione.

# --hints--

La tua pagina dovrebbe avere un elemento `nav`.
...
Il tuo `<h2>Navigation</h2>` dovrebbe essere all’interno dell’elemento `nav`.
...
Il tuo elemento `ul` dovrebbe essere all’interno dell’elemento `nav`.
...
I tuoi elementi `li` dovrebbero essere all’interno dell’elemento `ul`.
...
Non dovresti aggiungere altri elementi `ul`, `li` o `h2`.
