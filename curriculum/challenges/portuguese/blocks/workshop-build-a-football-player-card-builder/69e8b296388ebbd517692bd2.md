...
A seção de informações do jogador está agora completa. Após o fechamento do `div` da seção de informações do jogador e ainda dentro do `form-panel`, adicione um novo `div`. Dentro dele, adicione um elemento `p` com o `className` do `form-section-title` e o texto `Player Stats`. Depois, adicione um `div` com o `className` do `stats-grid`. Deixe a grade de estatísticas vazia por enquanto.

# --hints--

Seu `div.form-panel` deve conter um segundo `div` para a seção de estatísticas do jogador.
...
A seção de estatísticas do jogador deve conter um `p` com o `className` do `form-section-title` e o texto `Player Stats`.
...
A seção de estatísticas do jogador deve conter um `div` com o `className` do `stats-grid`.
