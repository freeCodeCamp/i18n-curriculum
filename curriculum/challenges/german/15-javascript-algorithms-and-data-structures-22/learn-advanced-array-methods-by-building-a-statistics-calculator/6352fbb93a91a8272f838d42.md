...
The next step is to square each of the differences. Um einen Wert zu quadrieren, kannst du den `**`-Operator verwenden. For example, `3 ** 2` would return `9`.

Declare a `squaredDifferences` variable, and assign it the value of `differences.map()`. Gib für den Callback den Wert von `el` zum Quadrat zurück.
