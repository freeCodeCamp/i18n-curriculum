...
爲了存儲歌曲暫停時的當前時間，請將 `userData` 對象的 `songCurrentTime` 設置爲 `audio` 變量的 `currentTime`。

*注意*：你不應該在此步驟中使用可選鏈接，因爲此時 `userData.songCurrentTime` 將不會是 `null` 或 `undefined`。
...
你應該將 `audio.currentTime` 分配給 `userData.songCurrentTime`。
