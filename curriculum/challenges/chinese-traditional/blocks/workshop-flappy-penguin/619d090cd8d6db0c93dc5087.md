...
在 `.chin` 元素下方，添加兩個 `div` 元素，每個元素的 `class` 為 `eye`。此外，為第一個 `.eye` 元素添加 `class` 為 `left`，為第二個 `.eye` 元素添加 `class` 為 `right`。

# --hints--

你應該在 `.penguin-head` 中添加兩個 `div` 元素。預期有 --fcc-expected-- `.penguin-head > div` 元素，但實際找到 --fcc-actual--。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 5);
```

你應該給第一個新的 `div` 一個 `class` 為 `eye`。

```js
assert.exists(document.querySelector('.penguin-head > div.eye'));
```

你應該給第二個新的 `div` 一個 `class` 為 `eye`。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div.eye'), 2);
```

你應該給第一個新的 `div` 一個 `class` 為 `left`。

```js
assert.exists(document.querySelector('.penguin-head > div.eye.left'));
```

你應該給第二個新的 `div` 一個 `class` 為 `right`。

```js
assert.exists(document.querySelector('.penguin-head > div.eye.right'));
```

你應該將 `div.eye.left` 放在 `div.chin` 之後。

```js
assert.exists(document.querySelector('.chin + .eye.left'));
```

你應該將 `div.eye.right` 放在 `div.eye.left` 之後。
