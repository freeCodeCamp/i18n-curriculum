...
1. 각 `tr` 요소는 다섯 개의 `td` 요소를 포함해야 합니다:
   ...
   - `td`는 `div` 클래스를 가진 `avatar-container` 요소를 포함하며, `avatars` 함수가 `posters`과 `users`을 인수로 호출하여 반환한 이미지를 포함합니다.
...
`img` 함수가 반환한 문자열의 각 `avatars` 요소는 게시자의 `alt` 속성 값을 가진 `name` 텍스트를 가져야 합니다.
...
`td`가 반환한 문자열의 각 표 행의 두 번째 `showLatestPosts` 요소는 `avatars` 함수가 `posters`과 `users`를 인수로 호출하여 반환한 이미지를 `div` 클래스를 가진 `avatar-container` 요소 안에 중첩하여 포함해야 합니다.
