...
你仍然没有匹配 `free money`，因为你还需要匹配字符串的结尾。

与 `^` 锚点一样，你可以使用 `$` 锚点来匹配字符串的末尾。

更新正则表达式以匹配字符串的末尾或空格，就像对字符串的开头所做的那样。

# --hints--

你的 `freeRegex` 应该使用第二个非捕获组。

```js
assert.lengthOf(freeRegex.source.match(/\(\?:/g), 2);
```

你的 `freeRegex` 应该使用非捕获组来匹配 `\s` 或 `$`。

```js
assert.match(freeRegex.source, /\(\?:(\$\|\\s|\\s\|\$)\)/);
```

你的 `freeRegex` 应该与 `it's free money time` 匹配。

```js
assert.match("it's free money time", freeRegex);
```

你的 `freeRegex` 应该与 `free money time` 匹配。

```js
assert.match('free money time', freeRegex);
```

你的 `freeRegex` 应该与 `it's free money` 匹配。

```js
assert.match("it's free money", freeRegex);
```

你的 `freeRegex` 应该与 `free money` 匹配。

```js
assert.match('free money', freeRegex);
```

你的 `freeRegex` 不应匹配 `hands-free money time`。

```js
assert.notMatch('hands-free money', freeRegex);
```

你的 `freeRegex` 不应匹配 `free money-management`。
