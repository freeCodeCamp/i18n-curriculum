...
現在你的 `applyFunction` 需要傳回結果。傳回對 `str2` 呼叫 `.replace()` 方法的結果。傳入你的 `functionCall` 正規表示式和一個空的回呼。

# --hints--

你的 `applyFunction` 函式應該傳回對 `str2` 呼叫 `.replace()` 方法的結果。
...
你應該將 `functionCall` 作為第一個引數傳遞給你的 `.replace()` 呼叫。
...
你應該將一個空的箭頭函式作為第二個引數傳遞給你的 `.replace()` 呼叫。
