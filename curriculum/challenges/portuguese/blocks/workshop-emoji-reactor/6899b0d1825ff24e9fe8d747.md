...
Abaixo do elemento `h1`, adicione um elemento `p` com a classe `description`.

Dentro do elemento `p` escreva o texto `Click on the buttons below to rate your emotions.`.

# --hints--

Você deve criar um novo elemento `p` abaixo do elemento `h1`.

```js
assert.exists(document.querySelector("h1 + p"));
```

O novo elemento `p` deve ter uma classe `description`.

```js
assert.equal(document.querySelector("h1 + p")?.className, "description");
```

O novo elemento `p` deve conter o texto `Click on the buttons below to rate your emotions.`.
