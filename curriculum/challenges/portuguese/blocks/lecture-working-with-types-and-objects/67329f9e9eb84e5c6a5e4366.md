...
Quando você usa a propriedade `length` em um primitivo string, o JavaScript envolve temporariamente o primitivo string em um objeto string para realizar a operação. É por isso que você pode usar a propriedade `length` e os diferentes métodos como `repeat()`, `concat()` e `slice()`.  These types of methods and properties are referred to as instance methods, instance properties, and static methods. Você aprenderá em detalhes como isso funciona em módulos futuros.
