...
ユーザーが入力した内容を取得するために、入力に`onChange`属性を追加してください。`e`を引数に取るアロー関数に設定します。関数内で、入力の現在の値を保持する`query`をセッター関数に渡して、`e.target.value`の状態を更新してください。例を示します。
...
`onChange`ハンドラが定義されていることを確認してください。
...
`onChange`ハンドラは`setQuery()`を`e.target.value`で呼び出す必要があります。
