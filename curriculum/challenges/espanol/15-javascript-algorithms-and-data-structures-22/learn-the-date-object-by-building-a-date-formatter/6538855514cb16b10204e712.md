...
Crea una variable `const` llamada `minutes` y asígnale los minutos de `date` con el método `.getMinutes()`.

# --hints--

Debes usar `const` para declarar una variable `minutes`.

```js
assert.match(code, /const\s+minutes\s*=/);
```

Debes asignar `date.getMinutes()` a tu variable `minutes`.
