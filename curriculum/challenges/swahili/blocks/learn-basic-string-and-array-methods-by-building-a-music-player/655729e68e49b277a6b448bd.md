...
Sasa umeunda `resetButton`, unahitaji kuweka thamani ya sifa ya kipengele `id` na `aria-label`. JavaScript inatoa vigezo `id` na `ariaLabel` unavyohitaji kutumia kwa hili.

Kwa mfano, `element.id` itaweka sifa ya kipengele `id`, na `element.ariaLabel` itaweka sifa ya kipengele `aria-label`. Zote mbili zinakubali thamani zao kama mfuatano wa herufi.

Weka sifa ya kipengele `id` ya kipengele `resetButton` kuwa `"reset"` na sifa yake ya kipengele `aria-label` kuwa `"Reset playlist"`.

# --hints--

Haupaswi kubadilisha tamko la `if` lililopo na yaliyomo ndani yake.
...
Unapaswa kutumia `resetButton.id` kuunda sifa ya kipengele `id` yenye jina `"reset"` kwa `resetButton`.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

Unapaswa kutumia `resetButton.ariaLabel` kuunda sifa ya kipengele `"aria-label"` yenye jina `"Reset playlist"` kwa `resetButton`.
