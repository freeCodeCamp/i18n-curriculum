...
بعد ذلك، تحتاج إلى التحقق مما إذا كانت قائمة التشغيل فارغة. إذا كانت كذلك، يجب إعادة تعيين كائن `userData` إلى حالته الأصلية.

استخدم جملة `if` للتحقق مما إذا كان طول `userData?.songs` يساوي `0`.

# --hints--

يجب أن تنشئ جملة `if` فارغة بشرط `userData?.songs.length === 0`.
