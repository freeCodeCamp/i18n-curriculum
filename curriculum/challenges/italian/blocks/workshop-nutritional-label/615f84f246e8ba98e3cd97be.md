...
Sotto il tuo elemento `.double-indent`, aggiungi un nuovo elemento `p` con il testo `Includes 10g Added Sugars 20%`. Il tuo nuovo elemento `p` dovrebbe anche essere indentato doppiamente e non avere bordo inferiore. Usa un `span` per rendere il `20%` in grassetto e allineato a destra.
...
Il tuo nuovo elemento `p` dovrebbe avere l'attributo `class` impostato su `double-indent no-divider`.
...
Il tuo elemento `span` dovrebbe avere l'attributo `class` impostato su `bold`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.firstElementChild?.classList?.contains('bold'));
```

Il tuo elemento `span` dovrebbe racchiudere il testo `20%`.
