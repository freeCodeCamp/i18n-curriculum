...
في الخطوات السابقة، تعلمت كيف تستخدم الطريقة `sort()` لترتيب الأغاني أبجديًا. استخدام آخر لدالة الاستدعاء الراجع هو عشوائية ترتيب المصفوفة.

طريقة واحدة لعشوائية ترتيب عناصر المصفوفة هي طرح `0.5` من `Math.random()` التي تُنتج قيمًا عشوائية إما موجبة أو سالبة. هذا يجعل نتيجة المقارنة مزيجًا من القيم الموجبة والسالبة، مما يؤدي إلى ترتيب عشوائي للعناصر.
...
استخدم الطريقة `sort()` على مصفوفة `userData?.songs`. مرر دالة استدعاء راجع للطريقة، وأرجع نتيجة `Math.random() - 0.5`.


# --hints--

لا تعدل الدالة `shuffle` الموجودة.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

يجب أن تستخدم الطريقة `sort()` على `userData?.songs`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

يجب أن تحتوي طريقتك `sort()` على دالة استدعاء راجع تستخدم صيغة السهم.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

يجب أن تُرجع دالة الاستدعاء الراجع في طريقتك `sort()` القيمة `Math.random() - 0.5`.
