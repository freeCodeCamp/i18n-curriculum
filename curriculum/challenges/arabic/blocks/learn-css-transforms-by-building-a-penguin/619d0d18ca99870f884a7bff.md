...
داخل كل عنصر `.eye`، أضف `div` مع `class` اسمه `eye-lid`.

# --hints--

يجب أن تضيف عنصر `div` واحد داخل `.eye.left`، لكن وُجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.eye.left > div')?.length ?? 0, 1);
```

يجب أن تضيف عنصر `div` واحد داخل `.eye.right`، لكن وُجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.eye.right > div')?.length ?? 0, 1);
```

يجب أن تعطي أول `div` جديد `class` اسمه `eye-lid`.

```js
assert.exists(document.querySelector('.eye.left > div.eye-lid'));
```

يجب أن تعطي ثاني `div` جديد `class` اسمه `eye-lid`.
