...
アプリが対応する単位のリストを保持する必要があります。

`units` 配列を宣言し、文字列の値 `"cup"`、`"gram"`、`"ounce"`、`"teaspoon"` を与えてください。
...
`units` 変数は配列である必要があります。

```js
assert.isArray(units);
```

`units` 配列は4つの要素を持つべきです。

```js
assert.lengthOf(units, 4);
```

`units` 配列には文字列 `"cup"` を含めてください。

```js
assert.include(units, "cup");
```

`units` 配列には文字列 `"gram"` を含めてください。

```js
assert.include(units, "gram");
```

`units` 配列には文字列 `"ounce"` を含めてください。

```js
assert.include(units, "ounce");
```

`units` 配列には文字列 `"teaspoon"` を含めてください。
