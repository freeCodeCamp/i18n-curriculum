...
Erstellen Sie am Ende Ihres `p`-Elements ein letztes `.text`-Element und geben Sie ihm den folgenden Text:
...
Sie sollten dem `p`-Element ein sechstes `.text`-Element hinzufügen.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 7);
```

Ihr sechstes `p`-Element sollte den vorgegebenen Text enthalten.
