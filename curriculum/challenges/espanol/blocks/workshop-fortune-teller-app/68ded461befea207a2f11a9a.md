...
Hasta ahora, tienes tres funciones: `getElement`, `hideElements` y `showElements`.
Ahora necesitas una función que te ayude a obtener una fortuna aleatoria.

Declara una función llamada `getRandomItem` que reciba un arreglo de `items` como argumento. Usa genéricos para esta función.

Dentro de la función, usa `Math.random()` y `Math.floor()` para seleccionar una fortuna aleatoria del arreglo `items`.
No olvides devolver la fortuna seleccionada.

# --hints--

Debes declarar una función llamada `getRandomItem`.
...
Tu función `getRandomItem` debe usar el parámetro de tipo genérico `T`.
...
Tu función `getRandomItem` debe tener un parámetro llamado `items` con el tipo `T[]`.
...
Tu función `getRandomItem` debe tener un tipo de retorno `T`.
...
Debes declarar una variable llamada `index` dentro de la función `getRandomItem`.
...
La variable `index` debe usar `Math.floor()` y `Math.random()` con `items.length` para producir un índice aleatorio.
...
Tu función `getRandomItem` debe devolver `items[index]`.
