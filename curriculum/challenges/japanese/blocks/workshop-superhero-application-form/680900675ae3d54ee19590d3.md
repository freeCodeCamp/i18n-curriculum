...
入力を埋めずにフォームを送信できることに気づきましたか？

それを防ぐために、`disabled` 属性を追加して、`heroName`、`realName`、または `powerSource` が偽の場合、あるいは `powers` の長さが `0` の場合に送信ボタンを無効にしてください。

これで、何かが入力されていないとボタンが無効になります。スーパーヒーローの情報を入力して、リーグに参加するために送信してください。

これで、スーパーヒーローの申請フォームが完成です。

# --hints--

ボタンの既存の内容は変更しないでください。

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2/)
```

`button` 要素には `disabled` プロパティが必要です。

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

`button`、`heroName`、`realName` のいずれかが false の場合、または `powerSource` の長さが `powers` の場合に、`0` を無効にしてください。
