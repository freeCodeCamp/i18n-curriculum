...
تنتقل التدرجات اللونية غالبًا تدريجيًا من لون إلى آخر. عندما يتطلب الأمر تغييرًا أكثر حدة، يمكن إجراء الانتقال بتوقف حاد مثل هذا:
...
أضف `linear-gradient` إلى `.bb2b` يستخدم `--building-color2` من `0%` إلى `6%` و`--window-color2` من `6%` إلى `9%`.

# --hints--

يجب أن تعطي `.bb2b` خاصية `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

يجب أن تستخدم `linear-gradient` على الخاصية `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

يجب أن تستخدم `--building-color2` من `0%` إلى `6%`.
...
يجب أن تستخدم `--window-color2` من `6%` إلى `9%`.
...
يجب أن تحتوي `.bb2b` على `linear-gradient` ينتقل من `--building-color2` عند `0%` إلى `6%`، ومن `--window-color2` عند `6%` إلى `9%`.
