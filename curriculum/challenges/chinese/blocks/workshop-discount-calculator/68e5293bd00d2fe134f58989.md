...
现在为 `FixedAmountDiscount` 实现 `apply_discount` 方法。该方法应从产品价格中减去固定金额并返回新价格。

# --hints--

你的 `FixedAmountDiscount` 类应有一个 `apply_discount` 方法。
...
你的 `apply_discount` 方法应返回 `product.price - self.amount`。
