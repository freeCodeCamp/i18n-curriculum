...
Ndani ya kitendakazi, pata rejea za vipengele vya HTML vinavyohusika na kuonyesha kichwa cha wimbo na msanii.

Pata vipengele vya `#player-song-title` na `#player-song-artist` kwa njia ya `getElementById()`. Weka thamani zao katika vigezo `playingSong` na `songArtist` mtawalia.

# --hints--

Haupaswi kubadilisha kitendakazi cha `setPlayerDisplay` kilichopo na yaliyomo ndani yake.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#player-song-title`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Unapaswa kuweka kipengele cha `#player-song-title` katika kigezo `playingSong`. Usisahau kutumia `const` kutangaza kigezo hicho.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#player-song-artist`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Unapaswa kuweka kipengele cha `#player-song-artist` katika kigezo `songArtist`. Usisahau kutumia `const` kutangaza kigezo hicho.
