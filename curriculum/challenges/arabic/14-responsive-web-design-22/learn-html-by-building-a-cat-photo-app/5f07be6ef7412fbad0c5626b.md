...
Your `section` element should have an opening tag. Opening tags تكتب هكذا: `<elementName>`.

```js
assert(document.querySelector('section'));
```

Your `section` element should have a closing tag. Closing tags يجب ان تتبع بـ `/` مباشرة بعد رمز `<`.
