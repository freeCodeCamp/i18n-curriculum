...
عرّف دالة `playSong` تأخذ معلمة واحدة تمثل المعرف الفريد للأغنية التي تريد تشغيلها.

# --hints--

يجب أن يكون لديك دالة فارغة باسم `playSong`.

```js
assert.isFunction(playSong);
```

يجب أن تأخذ دالة `playSong` معلمة واحدة.
