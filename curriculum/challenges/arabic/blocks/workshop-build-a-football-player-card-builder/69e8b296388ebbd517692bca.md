...
يُعرض `FootballPlayerCard` حاليًا البطاقة باستخدام `defaultPlayer` ثابت. لجعل التطبيق تفاعليًا، تحتاج إلى إدارة بيانات اللاعب في حالة React بحيث تؤدي التحديثات إلى إعادة العرض.

داخل مكوِّن `FootballPlayerCard`، أنشئ متغير حالة باسم `player` مع دالة تعيين `setPlayer` التي ستحتفظ ببيانات اللاعب الحالية. ابدأ هذه الحالة بـ `defaultPlayer`، وحدد أن الحالة تستخدم النوع العام `PlayerData` ليعرف TypeScript شكل كائن اللاعب.

بعد ذلك، حدّث الإرجاع لتمرير متغير الحالة `player` كخاصية إلى `PlayerCard`.

# --hints--

يجب أن تستخدم التفكيك من المصفوفة لتعيين متغير حالة `player` ودالة تعيين `setPlayer`.

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

يجب أن يستخدم كل من `player` و `setPlayer` الخطاف `useState`.
...
يجب تهيئة `useState` بـ `defaultPlayer` وأن يكون له النوع العام `PlayerData`.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

يجب أن يكون لـ `player` القيمة الخاصة بخاصية `player` الممررة إلى `PlayerCard`.
