...
为 `label` 元素添加一个 `for` 属性，其值为 `X-#-name`，其中 `X` 是 `entryDropdown` 元素的 `value`，而 `#` 是 `entryNumber` 的值。 请记住，HTML 属性应该用双引号引起来。

# --hints--

你的 `label` 元素应将 `for` 属性设置为 `${entryDropdown.value}-${entryNumber}-name`。

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>/)
```

你的 `label` 元素应具有与之前相同的文本。
