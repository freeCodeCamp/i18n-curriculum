...
`article` 요소 안에 `h3`가 `class`로 설정된 `list-title` 요소를 만들고, 텍스트는 `A Brief History`로 하세요. 그 아래에 텍스트가 `p`인 `Of the Curriculum` 요소를 만드세요. 그리고 클래스가 `ul`인 `lists` 요소를 만드세요.

# --hints--

`h3` 요소 안에 `article` 요소를 만들어야 합니다.

```js
assert.exists(document.querySelector('article h3'));
```

`p` 요소 안에 `article` 요소를 만들어야 합니다.

```js
assert.exists(document.querySelector('article p'));
```

`ul` 요소 안에 `article` 요소를 만들어야 합니다.

```js
assert.exists(document.querySelector('article ul'));
```

`article` 요소 안에 있는 요소들이 올바른 순서여야 합니다.
