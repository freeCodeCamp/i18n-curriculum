...
Erstellen Sie als Nächstes eine Unterklasse namens `Bee`, die die `Bug`-Klasse mit dem generischen Typ-Parameter `string` erweitert.

# --hints--

Sie sollten eine Klasse namens `Bee` haben.
...
Ihre `Bee`-Klasse sollte `Bug` erweitern.
...
Ihre `Bee`-Klasse sollte `Bug` mit einem Typ-Argument von `string` erweitern.
