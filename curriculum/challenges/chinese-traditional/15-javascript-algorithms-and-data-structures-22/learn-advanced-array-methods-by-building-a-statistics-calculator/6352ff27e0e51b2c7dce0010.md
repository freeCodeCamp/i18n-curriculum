...
將你的新 `getVariance` 函數添加到 `calculate` 函數裏，並更新相應的 HTML 元素。

# --hints--

你的 `calculate` 函數應該有一個 `variance` 變量，其值爲 `getVariance(numbers)`。

```js
assert.match(calculate.toString(), /variance\s*=\s*getVariance\(\s*numbers\s*\)/);
```

你應該將 `#variance` 元素的 `textContent` 設置爲 `variance` 變量的值。
