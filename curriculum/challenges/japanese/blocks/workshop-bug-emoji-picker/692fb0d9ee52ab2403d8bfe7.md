...
次に、`Bug` クラスのコンストラクタを作成してください。このコンストラクタは `emojiElement` 型の `HTMLParagraphElement` という名前のパラメータを1つ受け取ります。コンストラクタ内で、`emojiElement` パラメータをクラスの `emojiElement` プロパティに代入してください。

# --hints--

あなたの `Bug` クラスにはコンストラクタが必要です。
...
あなたの `Bug` コンストラクタには `emojiElement` 型の `HTMLParagraphElement` という名前のパラメータが必要です。

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

あなたの `Bug` コンストラクタは `emojiElement` を `this.emojiElement` に代入する必要があります。
