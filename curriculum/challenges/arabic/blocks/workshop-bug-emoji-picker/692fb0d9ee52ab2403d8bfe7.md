...
بعد ذلك، أنشئ دالة بناء لـ `Bug` تأخذ معلمة واحدة باسم `emojiElement` من نوع `HTMLParagraphElement`. داخل دالة البناء، عيّن المعلمة `emojiElement` إلى الخاصية `emojiElement` في الصنف.

# --hints--

يجب أن يحتوي صنف `Bug` على دالة بناء.
...
يجب أن تحتوي دالة البناء في `Bug` على معلمة باسم `emojiElement` من نوع `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

يجب أن تعيّن دالة البناء في `Bug` القيمة `emojiElement` إلى `this.emojiElement`.
