...
Dentro del bucle `while`, asigna `right_part[right_array_index]` a `array[sorted_index]`.

# --hints--

Deberías tener `array[sorted_index] = right_part[right_array_index]` en el cuerpo del ciclo. No olvides eliminar la palabra clave `pass` en el cuerpo del ciclo.
...
No debe haber ninguna palabra clave `pass` en el ciclo `while`
