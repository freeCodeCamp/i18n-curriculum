...
Necesitarás probar tu nueva función curried. Declara una variable `servingsResult` y asígnale el resultado de ajustar desde una cantidad base de 4 a una porción de 6.

Registra esa variable en la consola.

# --hints--

Deberías declarar una variable `servingsResult`.

```js
assert.isDefined(servingsResult);
```

Tu variable `servingsResult` debería tener el resultado de llamar a `adjustQuantity(4)(6)`.

```js
assert.strictEqual(servingsResult, 24);
```

Deberías registrar tu valor `servingsResult`.
