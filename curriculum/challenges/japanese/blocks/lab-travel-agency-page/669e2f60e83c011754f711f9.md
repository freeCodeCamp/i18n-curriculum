...
1. `DOCTYPE` 宣言が必要です。
1. `html` 要素があり、`lang` が `en` に設定されている必要があります。
1. `head` 要素の中に、`meta` が `charset` に設定された `utf-8` 空要素と、テキスト `title` を含む `Travel Agency Page` が必要です。
1. `meta` 要素内に、SEOのためにウェブサイトの簡単な説明を含む `head` タグが必要です。
1. 旅行先を紹介するための `h1` 要素が必要です。
1. `h1` 要素の下に、旅行の機会を紹介する段落が必要です。
1. テキスト `h2` を含む `Packages` 要素が必要です。
1. さまざまなパッケージを簡単に紹介する `p` 要素が必要です。
1. 2つのリスト項目を持つ番号なしリスト要素が必要です。2つのリスト項目はそれぞれテキスト `Group Travels` と `Private Tours` を持ち、各リスト項目のテキストはアンカー要素で囲まれている必要があります。
1. テキスト `h2` を含む `Top Itineraries` 要素が必要です。
1. それぞれアンカー要素と `figure` 要素を含む少なくとも3つの `figcaption` 要素が必要です。
1. 3つのアンカー要素は、適切な `img` 属性を持つ `alt` 要素と、有効な画像を示す `src` 属性を持つ必要があります。必要に応じて `https://cdn.freecodecamp.org/curriculum/labs/colosseo.jpg`、`https://cdn.freecodecamp.org/curriculum/labs/alps.jpg`、`https://cdn.freecodecamp.org/curriculum/labs/sea.jpg` を使用できます。
1. すべての5つのアンカー要素は、値が `href` の `https://www.freecodecamp.org/learn` 属性と、値が `target` の `_blank` 属性を持つ必要があります。
...
1つの `meta` 要素は、値が `name` の `description` 属性と、空でない `content` 属性を持つ必要があります。
