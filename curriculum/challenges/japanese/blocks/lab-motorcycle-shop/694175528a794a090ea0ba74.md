...
このラボでは、すべてのHTMLとCSSが提供されています。TypeScriptを使ってバイクショップのページの機能を完成させてください。

**目的:** 以下のユーザーストーリーを満たし、すべてのテストに合格してラボを完了させてください。

**ユーザーストーリー:**

1. `Category`という名前の`type`を作成してください。この`type`は次の可能な値を含みます：`'Sport'`、`'Cruiser'`、`'Touring'`、`'Dirt'`、`'Adventure'`、`'Naked'`、`'Electric'`。
2. 次のプロパティを持つ`interface`という名前の`Motorcycle`を作成してください：`id`（型は`string`）、`name`（型は`string`）、`manufacturer`（型は`string`）、`category`（型は`Category`）、`price`（型は`number`）、`image_url`（型は`string`）、`created_at`（型は`Date`）、`description`（型は`string`）、およびyear（型は`number`）。
3. `fetchMotorcycles`という名前の関数を作成してください。この関数は`Promise`を返し、`Motorcycle`オブジェクトの配列を解決します。
4. `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`エンドポイントから`Motorcycle`データを読み込んでください。
5. `renderMotorcycleCard`という名前の関数を作成してください。この関数は単一の`Motorcycle`オブジェクトをパラメータとして受け取り、バイクカードのフォーマットされたHTML文字列を含む文字列を返します。
   1. `renderMotorCycleCard`関数は`Motorcycle`型の`motorcycle`というパラメータを受け取る必要があります。
   2. `renderMotorCycleCard`関数は`string`型を返す必要があります。
   3. 各バイクカードコンポーネントには、有効な画像と`motorcycle-card-image-container`クラスを持つ`img`要素が必要です。
   4. 各バイクカードコンポーネントには、年をプレーンテキストで表示する`motorcycle-card-year-badge`クラスの要素が必要です。
   5. 各バイクカードコンポーネントには、バイクの名前をプレーンテキストで表示する`motorcycle-card-title`クラスの要素が必要です。
   6. 各バイクカードコンポーネントには、バイクの製造元の名前をプレーンテキストで表示する`motorcycle-card-manufacturer`クラスの要素が必要です。
   7. 各バイクカードコンポーネントには、バイクのカテゴリをプレーンテキストで表示する`motorcycle-card-category`クラスの要素が必要です。
   8. 各バイクカードコンポーネントには、バイクの説明をプレーンテキストで表示する`motorcycle-card-description`クラスの要素が必要です。
   9. 各バイクカードコンポーネントには、バイクの価格をプレーンテキストで表示する`motorcycle-card-price`クラスの要素が必要です。
   10. 各バイクカードコンポーネントには、各エンジンの馬力を表示する`motorcycle-card-engine`クラスの要素が必要です。
6. `MotorcycleGalleryApp`という名前のクラスを作成してください。
   1. このクラスには、`Motorcycle`オブジェクトの配列型に静的に型付けされた`private`配列の`allMotorcycles`が必要です。
   2. このクラスには、idが`motorcycle-grid`の要素内に`allMotorcycles`変数に相当するバイクカードコンポーネントをレンダリングする`renderMotorcycles`という名前の関数が必要です。
7. 表示される`Motorcycle`要素の数は、idが`results-number`の要素内に表示される必要があります。
8. フィルターは`input`イベントで更新される必要があります。

# --hints--

`Category`という名前の`type`を用意してください。
...
`Category`型は正しい値のユニオン型に設定してください。
...
`Motorcycle`という名前の`interface`を用意してください。
...
`Motorcycle`インターフェースには指定されたすべてのプロパティを含めてください。
...
`fetchMotorcycles`という名前の関数を用意してください。

```js
assert.isFunction(fetchMotorcycles);
```

`https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`エンドポイントを使用してください。
...
`fetchMotorcycles`から25個の`Motorcycle`オブジェクトのPromiseを返してください。
...
`renderMotorcycleCard`という名前の関数を用意してください。

```js
assert.isFunction(renderMotorcycleCard);
```

`renderMotorcycle`関数は`Motorcycle`型の静的に型付けされたパラメータを受け取る必要があります。
...
`renderMotorcycleCard`関数は明示的に`string`型を返す必要があります。
...
`renderMotorcycleCard`関数は`string`を返す必要があります。
...
`MotorcycleGalleryApp`という名前のクラスを用意してください。
...
`MotorcycleGalleryApp`には`allMotorcycles`という名前の配列が必要です。
...
`allMotorcycles`プロパティは`private`である必要があります。
...
`allMotorcycles`プロパティは`Motorcycle[]`に静的に型付けされている必要があります。
...
`MotorcycleGalleryApp`には`renderMotorcycles`という名前の関数が必要です。
...
`renderMotorcycles`関数は`motorcycle-card-image-container`クラスを持つ25個の要素をレンダリングする必要があります。
...
`renderMotorcycles`関数は`motorcycle-card-year-badge`クラスを持つ25個の要素をレンダリングする必要があります。
...
`renderMotorcycles`関数は`motorcycle-card-title`クラスを持つ25個の要素をレンダリングする必要があります。
...
`renderMotorcycles`関数は`motorcycle-card-manufacturer`クラスを持つ25個の要素をレンダリングする必要があります。
...
`renderMotorcycles`関数は`motorcycle-card-category`クラスを持つ25個の要素をレンダリングする必要があります。
...
`renderMotorcycles`関数は`motorcycle-card-description`クラスを持つ25個の要素をレンダリングする必要があります。
...
`renderMotorcycles`関数は`motorcycle-card-price`クラスを持つ25個の要素をレンダリングする必要があります。
...
`renderMotorcycles`関数は`motorcycle-card-engine`クラスを持つ25個の要素をレンダリングする必要があります。
