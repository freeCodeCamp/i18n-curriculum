...
在你的 `.calories-info` 元素下方，添加一個 `div`，其 `class` 屬性設為 `divider medium`。

# --hints--

你應該在你的 `.label` 元素中創建一個新的 `div`。

```js
assert(document.querySelectorAll('.label > div')?.length === 3)
```

你的新 `div` 應該將 `class` 屬性設為 `divider medium`。這個 div 應該是你 `.label` 元素中的最後一個元素。
