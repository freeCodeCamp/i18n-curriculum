...
title: بناء مشروع آلة حاسبة للوقت
challengeType: 23
...
اكتب دالة باسم `add_time` تأخذ معلمتين ضروريتين ومعلمة اختيارية واحدة:

- وقت بداية بصيغة الساعة 12 (ينتهي بـ AM أو PM)
- مدة زمنية تشير إلى عدد الساعات والدقائق
- (اختياري) يوم بداية الأسبوع، غير حساس لحالة الأحرف

يجب أن تضيف الدالة مدة الوقت إلى وقت البداية وتُرجع النتيجة.

إذا كانت النتيجة في اليوم التالي، يجب أن تظهر `(next day)` بعد الوقت. إذا كانت النتيجة بعد أكثر من يوم، يجب أن تظهر `(n days later)` بعد الوقت، حيث "n" هو عدد الأيام التي تلي.

إذا أعطيت الدالة معلمة يوم بداية الأسبوع الاختيارية، يجب أن يعرض الناتج يوم الأسبوع للنتيجة. يجب أن يظهر يوم الأسبوع في الناتج بعد الوقت وقبل عدد الأيام التي تلي.

فيما يلي بعض الأمثلة على الحالات المختلفة التي يجب أن تتعامل معها الدالة. انتبه جيدًا للمسافات وعلامات الترقيم في النتائج.
...
لا تستورد أي مكتبات بايثون. افترض أن أوقات البداية صحيحة. ستكون الدقائق في مدة الوقت عددًا صحيحًا أقل من 60، لكن الساعات يمكن أن تكون أي عدد صحيح.

ملاحظة: افتح وحدة التحكم في المتصفح بالضغط على F12 لرؤية إخراج أكثر تفصيلاً للاختبارات.

# --hints--
استدعاء `add_time('3:30 PM', '2:12')` يجب أن يُرجع `'5:42 PM'`.
...
استدعاء `add_time('11:55 AM', '3:12')` يجب أن يُرجع `'3:07 PM'`.
...
من المتوقع أن ينتهي الوقت بـ `'(next day)'` عندما يكون في اليوم التالي.
...
من المتوقع أن يتغير الفترة من `AM` إلى `PM` عند `12:00`.
...
استدعاء `add_time('2:59 AM', '24:00')` يجب أن يُرجع `'2:59 AM (next day)'`.
...
استدعاء `add_time('11:59 PM', '24:05')` يجب أن يُرجع `'12:04 AM (2 days later)'`.
...
استدعاء `add_time('8:16 PM', '466:02')` يجب أن يُرجع `'6:18 AM (20 days later)'`.
...
من المتوقع أن يُرجع إضافة `0:00` الوقت الأصلي.
...
استدعاء `add_time('3:30 PM', '2:12', 'Monday')` يجب أن يُرجع `'5:42 PM, Monday'`.
...
استدعاء `add_time('2:59 AM', '24:00', 'saturDay')` يجب أن يُرجع `'2:59 AM, Sunday (next day)'`.
...
استدعاء `add_time('11:59 PM', '24:05', 'Wednesday')` يجب أن يُرجع `'12:04 AM, Friday (2 days later)'`.
...
استدعاء `add_time('8:16 PM', '466:02', 'tuesday') ` يجب أن يُرجع `'6:18 AM, Monday (20 days later)'`.
