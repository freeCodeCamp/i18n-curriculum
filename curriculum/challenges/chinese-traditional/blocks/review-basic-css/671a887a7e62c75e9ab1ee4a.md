...
- **CSS 規則的基本剖析**： CSS 規則由兩個主要部分組成：選擇器和聲明塊。 選擇器是 CSS 中使用的一種模式，用於識別和針對特定 HTML 元素設計樣式。 A declaration block applies a set of styles for a given selector or selectors.
