...
现在你可以开始将函数解析器应用到 `evalFormula` 逻辑了。 声明一个 `functionExpanded` 变量，并将使用 `cellExpanded` 字符串调用 `applyFunction` 的结果分配给它。

# --hints--

你应该声明一个 `functionExpanded` 变量。
...
你应该将 `cellExpanded` 传递给 `applyFunction` 调用。
