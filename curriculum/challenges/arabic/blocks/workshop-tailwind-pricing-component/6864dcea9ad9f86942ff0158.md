...
حان الوقت للعمل على عنصر `div` الذي يلف البطاقات الثلاث الخاصة بالتسعير. لترتيب البطاقات، يمكنك استخدام فئات شبكة Tailwind CSS المساعدة.

إليك كيفية تعيين عرض `grid` لعنصر باستخدام Tailwind:
...
عيّن صنف `grid` لعنصر `div` الذي يلف بطاقات التسعير.

# --hints--

يجب أن يحتوي عنصر `div` على الصنف `grid`.
