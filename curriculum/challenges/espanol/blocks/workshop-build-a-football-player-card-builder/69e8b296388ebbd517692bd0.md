...
Después del `form-row`, agrega un `div` con el `className` de `form-group`. Dentro de él, agrega un elemento `label` con el `className` de `label`, un `htmlFor` de `club` y el texto `Club`. Luego, agrega un elemento input con un `id` de `club`, un `className` de `input` y un `type` de `text`.
...
Tu `div.form-group` debería tener un `label` con un atributo `htmlFor` de `club`, un `className` de `label` y el texto `Club`.
...
Tu Club `form-group` debería contener un `input` con un `id` de `club`, un `className` de `input` y un `type` de `text`.
