...
Per calcolare una radice con esponente, come $\sqrt[n]{x}$, puoi usare un esponente invertito $x^{1/n}$. JavaScript ha una funzione integrata `Math.pow()` che può essere usata per calcolare potenze.

Ecco la sintassi base per la funzione `Math.pow()`:

```js
Math.pow(base, exponent);
```

Ecco un esempio di come calcolare la radice quadrata di `4`:
...
La tua variabile `standardDeviation` dovrebbe usare la funzione `Math.pow()`.

```js
assert.match(getStandardDeviation.toString(), /standardDeviation\s*=\s*Math\.pow\(/);
```

La tua funzione `Math.pow()` dovrebbe avere una base `variance` e un esponente `1/2`.
