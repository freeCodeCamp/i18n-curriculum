...
Kufuata muundo uleule, fanya hoja ya utafutaji kwa ingizo lako la `number` katika vipengele vya `#dinner`, `#snacks`, na `#exercise`. Weka thamani zao katika vigezo ukifuata mpangilio wa majina wa hatua mbili zilizopita.

# --hints--

Unapaswa kutangaza kigezo cha `dinnerNumberInputs`.

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

Unapaswa kufanya wito wa `document.querySelectorAll()` kwa `#dinner input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya wito wako wa `document.querySelectorAll()` katika `dinnerNumberInputs`.
...
Unapaswa kutangaza kigezo cha `snacksNumberInputs`.

```js
assert.match(calculateCalories.toString(), /snacksNumberInputs\s*=/);
```

Unapaswa kufanya wito wa `document.querySelectorAll()` kwa `#snacks input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#snacks input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya wito wako wa `document.querySelectorAll()` katika `snacksNumberInputs`.
...
Unapaswa kutangaza kigezo cha `exerciseNumberInputs`.

```js
assert.match(calculateCalories.toString(), /exerciseNumberInputs\s*=/);
```

Unapaswa kufanya wito wa `document.querySelectorAll()` kwa `#exercise input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#exercise input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya wito wako wa `document.querySelectorAll()` katika `exerciseNumberInputs`.
