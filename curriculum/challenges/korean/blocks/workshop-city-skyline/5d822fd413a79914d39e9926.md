...
`.fb3-window` 요소에 `width`의 `25%`과 `height`의 `80%`을 주고, `--window-color1` 변수를 `background-color` 값으로 사용하세요.

# --hints--

`.fb3-window`에 `width`의 `25%`을 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

`.fb3-window`에 `height`의 `80%`을 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

`.fb3-window`에 `background-color`의 `--window-color1`을 주어야 합니다.
