...
Setzen Sie das Muster fort und erstellen Sie ein `fieldset` für `dinner` mit denselben verschachtelten Elementen.

# --hints--

Sie sollten ein drittes `fieldset`-Element in Ihrem `form` erstellen.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]);
```

Ihr drittes `fieldset`-Element sollte nach Ihrem zweiten `fieldset`-Element kommen.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.previousElementSibling?.tagName, "FIELDSET");
```

Ihr drittes `fieldset`-Element sollte ein `id` mit dem Wert `dinner` haben.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.id, "dinner");
```

Ihr drittes `fieldset`-Element sollte ein `legend`-Element enthalten.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]?.querySelector('legend'));
```

Ihr neues `legend`-Element sollte den Text `Dinner` haben.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.querySelector('legend')?.innerText, "Dinner");
```

Ihr drittes `fieldset`-Element sollte ein `div`-Element enthalten.
