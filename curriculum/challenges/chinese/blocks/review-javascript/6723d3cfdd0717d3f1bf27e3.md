...
- **`prompt()` 方法**：`window` 的这种方法用于通过对话的形式从用户处获取信息。 该方法接受两个参数。 第一个是将出现在对话中的信息，通常是提示用户输入信息。 第二个是默认值，是可选的，将作为填充输入字段的初始值。
...
- **`Object.hasOwn()` 方法**：这是查看一个对象是否拥有某个自身（非继承）属性的现代推荐方式。语法是 `Object.hasOwn(object, propertyName)`。如果属性存在于对象上，则返回 `true`，如果不存在则返回 `false` —— 无论该属性的值是什么。这使得它比检查 `if (obj.prop)` 更安全，因为属性值可能是 `0`、`false`、`null` 或 `undefined`。
...
- **定义**：JavaScript 中的类用于定义用于创建对象和封装数据的蓝图。类包含一个构造函数，这是一个特殊的方法，当从类中创建新对象时会自动调用。它用于初始化对象的属性。这里使用 `this` 关键字来引用类的当前实例。在构造函数下面，你可以定义所谓的方法。方法是在类内部定义的函数，用于对类的数据或状态执行操作。它们用于定义类的实例可以执行的行为。
