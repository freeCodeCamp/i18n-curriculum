...
Створіть селектор `.author` і надайте йому множину властивостей `font-size`, встановлену в `2rem`, а також властивість `font-family`, встановлену в `Raleway` із запасним варіантом `sans-serif`.

# --hints--

У вас має бути селектор `.author`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author'));
```

Ваш селектор `.author` має мати властивість `font-size`, встановлену в `2rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

Ваш селектор `.author` має мати властивість `font-family`, встановлену в `Raleway` із запасним варіантом `sans-serif`.
