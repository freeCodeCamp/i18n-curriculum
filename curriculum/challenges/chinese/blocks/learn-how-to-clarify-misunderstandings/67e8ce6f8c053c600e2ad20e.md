...
- **学生：** `Is encapsulation just about hiding code?`

- **导师：** `Not exactly, it's also about organizing and protecting it.` —— 这展示了如何使用 `not exactly` 来温和地纠正一个语句，同时添加更多细节。

`Inheritance` 是面向对象编程中的一个关键概念。它允许一个类（子类）继承另一个类（基类）的属性和方法。这支持代码重用，意味着你不必在多个地方重写相同的逻辑，同时也使扩展或构建现有功能性变得更容易。例如：
