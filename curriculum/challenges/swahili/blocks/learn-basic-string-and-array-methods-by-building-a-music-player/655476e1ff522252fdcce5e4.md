...
Ongeza kipengele cha `else` kwenye usemi wa `if`. Ndani ya kipengele cha `else`, fanya wito wa kitendakazi cha `getCurrentSongIndex()` na weka thamani yake katika thabiti iitwayo `currentSongIndex`.

# --hints--

Haupaswi kubadilisha usemi wa `if` uliopo.
...
Unapaswa kuongeza `else` kwenye usemi wa `if` uliopo.
...
Unapaswa kuweka thabiti ya `currentSongIndex` kuwa `getCurrentSongIndex()` ndani ya kipengele cha `else`.
