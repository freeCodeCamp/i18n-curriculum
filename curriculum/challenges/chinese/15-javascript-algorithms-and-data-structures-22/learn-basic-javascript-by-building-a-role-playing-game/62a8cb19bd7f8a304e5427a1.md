...
现在更新 `goldText` 元素以显示 `gold` 的新值，并更新 `text` 元素以显示 `"You now have a new weapon."`。

# --hints--

你应该将 `goldText` 元素的 `innerText` 属性更新为 `gold`。

```js
assert.match(buyWeapon.toString(), /goldText\.innerText\s*=\s*gold/);
```

你应该将 `text` 元素的 `innerText` 属性更新为 `"You now have a new weapon."`。
