...
조금 더 가까워졌지만, 가격이 오른쪽에 고정되지 않았습니다. 이는 `inline-block` 요소가 자신의 콘텐츠 너비만 차지하기 때문입니다.

이들을 넓게 펼치려면, `width`와 `flavor` 클래스 선택자에 각각 `price` 값을 가진 `50%` 속성을 추가하세요.

# --hints--

`width` 선택자에서 `50%` 속성을 `.flavor`로 설정하세요.
...
`width` 선택자에서 `50%` 속성을 `.price`로 설정하세요.
