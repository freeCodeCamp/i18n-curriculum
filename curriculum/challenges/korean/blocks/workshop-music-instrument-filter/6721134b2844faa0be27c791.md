...
나머지 악기들은 `instrumentsArr` 배열에 추가되어 있습니다.

이제 두 변수를 선언하고 `selectContainer`에는 HTML 드롭다운을, `productsContainer`에는 `products-container` 클래스를 가진 요소를 각각 할당하세요.

# --hints--

`selectContainer` 변수가 있어야 합니다.

```js
assert.exists(selectContainer);
```

`selectContainer` 변수에는 `select` 요소의 값이 할당되어야 합니다.

```js
assert.strictEqual(selectContainer, document.querySelector("select"));
```

`productsContainer` 변수가 있어야 합니다.

```js
assert.exists(productsContainer);
```

`productsContainer` 변수에는 `.products-container` 요소의 값이 할당되어야 합니다.
