...
フォームでは、ユーザーが1日のカロリー予算を表す数字を入力できるようにします。

`label` 要素を作成し、`for` 属性に `budget` を設定し、テキストに `Budget` を入れてください。その後、`input` 要素を作成し、`id` を `budget` に設定してください。

# --hints--

`label` の中に `form` 要素を作成してください。

```js
assert.exists(document.querySelector('form label'));
```

`label` 要素には `for` 属性を `budget` に設定してください。

```js
assert.equal(document.querySelector('form label')?.getAttribute('for'), 'budget');
```

`label` 要素にはテキスト `Budget` を入れてください。

```js
assert.equal(document.querySelector('form label')?.innerText, 'Budget');
```

`input` の中に `form` 要素を作成してください。
...
`input` 要素には `id` を `budget` に設定してください。
