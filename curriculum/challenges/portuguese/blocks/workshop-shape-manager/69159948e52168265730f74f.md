...
Você precisa de uma forma segura de acessar o elemento select e proteger contra valores `null`. Para isso, crie uma constante `val` com o valor `e.currentTarget` e tipo `HTMLSelectElement` usando a palavra-chave `as`.

Abaixo da constante, use uma declaração `if` para verificar se `val` é falsy negando-o. No bloco `if`, retorne `target value not found`.

# --hints--

Você deve ter uma constante chamada `val`.

```js
assert.match(code, /const\s+val/)
```

Sua constante `val` deve ter um valor `e.currentTarget` tipado como `HTMLSelectElement` usando a palavra-chave `as`.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

Você deve ter uma declaração `if` que verifica se `val` é null.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

Sua declaração `if` deve retornar `target value not found`.
