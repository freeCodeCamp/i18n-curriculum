...
CSV（逗號分隔值）是一種用於試算表和資料匯入的格式。每一行代表一筆資料，欄位以逗號分隔。

首先移除 `console.log(exportToJSON(...))` 呼叫，因為它只用於測試。接著創建一個名為 `exportToCSV` 的函式（程式），並帶有 `catalog` 作為參數。在函式內部，宣告 `const header = "Title,Author,Year,Location"` 和 `const rows = []`。

# --hints--

你應該移除 `exportToJSON` 的主控台輸出呼叫。

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

你應該有一個名為 `exportToCSV` 的函式（程式），帶有 `catalog` 參數。
...
你應該宣告一個名為 `header` 的變數，值為 `"Title,Author,Year,Location"`。

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

你應該宣告一個名為 `rows` 的變數，並設為空陣列。
