...
创建一个名为 `--penguin-skin` 的自定义 CSS 变量，并将其设置为 `gray`。 然后，用它替换所有相关属性值。

# --hints--

你应该给 `:root` 添加一个 `--penguin-skin` 属性。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

你应该将 `--penguin-skin` 的值设置为 `gray`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

你应该将 `.penguin-head` 的 `background` 设置为 `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`。
...
你应该将 `.penguin-body::before` 的 `background-color` 设置为 `var(--penguin-skin)`，而不是 `--fcc-actual--`。
...
你应该将 `.arm` 的 `background` 设置为 `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`。
