...
ربما لاحظت أن `EVAChunks` هي في الأساس مصفوفة تحتوي على مصفوفات بداخلها، أو مصفوفة ثنائية الأبعاد (2D). يمكن استخدام حلقة `for` مسكنة لتسجيل البيانات من مصفوفة ثنائية الأبعاد:
...
في المثال أعلاه، `rootArray` هي المصفوفة الرئيسية، و`rootArray[i]` هي مصفوفة فرعية داخل المصفوفة الرئيسية، و`rootArray[i][j]` هو كائن واحد داخل تلك المصفوفة الفرعية، و`.property` تصل إلى قيمة في ذلك الكائن.

استخدم حلقة `for` مسكنة للتكرار خلال `EVAChunks` وتسجيل `name` لكل رائد فضاء في كل جزء. تأكد من:

- استخدام `EVAChunks` كمصفوفة الجذر
- تسجيل `Chunk ${i+1}:` بين الأجزاء

# --hints--

يجب أن تستخدم حلقة `for` مسكنة للتكرار خلال `EVAChunks` وتسجيل `name` لكل رائد فضاء في كل جزء. سجّل `Chunk ${i+1}:` بين الأجزاء.
