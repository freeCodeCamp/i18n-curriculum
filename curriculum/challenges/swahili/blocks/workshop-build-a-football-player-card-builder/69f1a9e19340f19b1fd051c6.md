...
Ongeza vipengele viwili vya `span` vyenye `className` ya `stat-value` na `stat-label` mtawaliwa kwa kila `stat-row`.

# --hints--

Kila `.stat-row` katika `.stat-col` ya pili inapaswa kuwa na `span` yenye darasa la `stat-value` na `span` yenye darasa la `stat-label`.
