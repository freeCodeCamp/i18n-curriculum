...
ページに著者の名前を表示するには、2番目の `displayAuthors` メソッド内で `.then()` 関数を呼び出す必要があります。しかしその前に、空の `authorDataArr` 配列に著者データを代入する必要があります。

まず、`console.log()` 文を削除してください。それから、`data` を `authorDataArr` 変数に代入してください。
