...
Msimbo wa mtihani wa `kingBooks` ulibaini kuwa `findByAuthor` inafanya kazi ipasavyo. Kabla ya kuendelea na `groupByDecade`, safisha matokeo ya konsoli kwa kuondoa yale yaliyopo.

Ondoa mstari wa `const kingBooks = ...`, `console.log(kingBooks.length)`, na mzunguko wa `for` unaoonyesha vichwa vya vitabu na miaka.

# --hints--

Kigezo cha `kingBooks`, tamko la `console.log(kingBooks.length)`, na mzunguko wa `for` vyote vinapaswa kuondolewa.
