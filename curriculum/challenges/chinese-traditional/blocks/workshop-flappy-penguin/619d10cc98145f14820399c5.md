...
在 `.blush.right` 元素下方，添加兩個 `div` 元素，每個元素都有一個 `class` 為 `beak`。此外，為第一個 `.beak` 元素添加一個 `class` 為 `top`，為第二個 `.beak` 元素添加一個 `class` 為 `bottom`。

# --hints--

你應該在 `.penguin-head` 中添加兩個 `div` 元素。預期有 --fcc-expected-- 個 `div` 元素，但實際找到 --fcc-actual-- 個。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 9);
```

你應該給第一個新的 `div` 一個 `class` 為 `beak`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

你應該給第二個新的 `div` 一個 `class` 為 `beak`。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div.beak'), 2);
```

你應該給第一個新的 `div` 一個 `class` 為 `top`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

你應該給第二個新的 `div` 一個 `class` 為 `bottom`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak.bottom'));
```

你應該將 `div.beak.top` 放在 `div.blush.right` 之後。

```js
assert.exists(document.querySelector('.blush.right + .beak.top'));
```

你應該將 `div.beak.bottom` 放在 `div.beak.top` 之後。
