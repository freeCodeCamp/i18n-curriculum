...
現在呼叫 `showElements` 函式，並將 `multipleCard`、`fortuneContainer` 和 `text` 作為屬性傳入。

# --hints--

你應該在 `showElements` 方法中呼叫 `multipleCardSelected` 函式。
...
你應該將 `text` 作為引數傳入 `showElements` 函式。
