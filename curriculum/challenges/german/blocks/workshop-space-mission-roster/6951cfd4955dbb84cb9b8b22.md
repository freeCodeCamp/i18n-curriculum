...
Sie hatten zuvor einen `console.log()`-Aufruf in Ihre `addCrewMember`-Funktion eingefügt, um deren Verhalten zu überprüfen, und dieser Aufruf wird jetzt nicht mehr benötigt. Um zu verhindern, dass das Terminal in den nächsten Schritten unübersichtlich wird, entfernen Sie bitte den aktuellen `console.log()`-Aufruf.

# --hints--

Entfernen Sie den `console.log()`-Aufruf aus Ihrer `addCrewMember`-Funktion. Sie sollten die gesamte Zeile löschen.
