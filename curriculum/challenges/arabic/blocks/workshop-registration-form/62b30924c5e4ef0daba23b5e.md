...
حدود آخر عنصر `fieldset` تبدو غير مناسبة قليلاً.

في الدروس السابقة، تعلمت كيفية العمل مع فئة CSS المزيفة `last-of-type` بهذا الشكل:

```css
p:last-of-type { }
```

هذا سيختار آخر عنصر `p`.

أنشئ محددًا جديدًا يستهدف آخر عنصر `fieldset` واضبط `border-bottom` على `none`.
...
يجب أن يكون لـ `fieldset:last-of-type` الخاص بك `border-bottom` مضبوطًا على `none`.
