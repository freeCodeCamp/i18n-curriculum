...
`for` 반복문 안에서 변수 `row_str`을 선언하고, `row`를 반복하며 `i` 안의 각 항목 `row`을 문자열로 변환하는 리스트 내포를 할당하세요. 이를 위해 `str()` 함수를 사용하세요.

# --hints--

변수 `row_str`을 선언해야 합니다.
...
변수 `row`에 `row_str`를 반복하는 리스트 내포를 할당해야 합니다.
...
변수 `row_str`에 할당된 리스트 내포는 `i`을 반복 변수로 사용해야 합니다.
...
변수 `row_str`에 할당된 리스트 내포는 `str()` 안의 각 항목 `i`에 `row`를 호출해야 합니다.
