...
Ahora, dentro del template literal de la función `renderCard`, crea un elemento `div`. Dentro del `div`, añade un elemento `h2` que muestre `drawingType`, y un elemento `figure` con la clase `card_container ${isReversed ? "reversed-card" : ""}` y el atributo `data-id="${shortName}"`.

# --hints--

Deberías tener un elemento `div` en el template literal de la función `renderCard`.
...
Deberías tener un elemento `h2` que interpole `drawingType` dentro del elemento `div`.
...
Deberías tener un elemento `figure` dentro del elemento `div`.
...
Tu elemento `figure` debería tener la clase `card_container` y usar un ternario para añadir condicionalmente `reversed-card`.
...
Tu elemento `figure` debería tener un atributo `data-id` con valor `${shortName}`.
