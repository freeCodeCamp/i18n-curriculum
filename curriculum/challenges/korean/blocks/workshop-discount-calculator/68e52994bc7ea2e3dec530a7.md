...
워크숍을 완료하려면, f-string을 사용해 두 자리 소수점으로 최저가를 출력하는 마지막 출력문을 추가하세요. 형식은 `Best price for [product.name] for [user_tier] user: $[best_price]`이며 `[product.name]`, `[user_tier]`, `[best_price]`는 실제 변수 값으로 교체되어야 합니다.

예시 출력은 `Best price for Wireless Buds for Premium user: $93.00`일 수 있습니다. 가격에 두 자리 소수점이 있는 점을 주목하세요.

이로써 할인 계산기 워크숍이 완료됩니다!

# --hints--

출력 형식은 `Best price for {product.name} for {user_tier} user: ${best_price:.2f}`와 일치해야 합니다.
