...
ビュー数をより読みやすい形式に変換する関数が必要です。例えば、ビュー数が`1000`の場合は`1k`と表示し、ビュー数が`100,000`の場合は`100k`と表示する必要があります。

`viewCount`という関数を作成し、`views`というパラメータを持たせてください。もし`views`が`1000`以上であれば、`views`の値を`1000`で割ったものに`k`の文字を付けた文字列を返してください。`views / 1000`は小数点以下を切り捨てて整数にしてください。

そうでなければ、`views`の値を返してください。

例えば、`views`が`1000`の場合、戻り値は文字列`"1k"`となるはずです。

# --hints--

`viewCount`という関数を作成してください。

```js
assert.isFunction(viewCount);
```

`viewCount`関数は`views`というパラメータを持つべきです。

```js
assert.match(viewCount.toString(), /\(?views\)?/);
```

`viewCount`関数は`"1k"`が`views`の場合、文字列`1000`を返すべきです。

```js
assert.strictEqual(viewCount(1000), "1k");
```

`viewCount`関数は`"100k"`が`views`の場合、文字列`100000`を返すべきです。

```js
assert.strictEqual(viewCount(100000), "100k");
```

`viewCount`関数は`views`が`1000`以上の場合、正しい文字列を返すべきです。
...
`viewCount`関数は`views`が`views`の場合、`200`を返すべきです。

```js
assert.strictEqual(viewCount(200), 200);
```

`viewCount`関数は`views`が`views`未満の場合、`1000`を返すべきです。
