...
داخل الدالة `fetchMoreAuthors`، عرّف المتغيرين `startingIndex` و `endingIndex` ليزيد كل منهما بمقدار `+= 8`.
...
يجب أن تعرّف المتغير `startingIndex` ليزيد بمقدار `+=8`.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

يجب أن تعرّف المتغير `endingIndex` ليزيد بمقدار `+=8`.
