...
此時，你應該能在點擊卡片後看到每張卡片顯示的運勢。剩下唯一要做的就是讓「新的占卜」按鈕能正常運作。

在 `newReading` 方法中，呼叫 `showElements()` 和 `hideElements()`。

# --hints--

你應該在 `showElements` 方法中呼叫 `newReading` 函式。

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

你應該在 `hideElements` 方法中呼叫 `newReading` 函式。
