...
空の`.reduce()`コールバック内で、変数`difference`を宣言し、`el`から`mean`を引いた値に設定してください。次に、`squared`変数を宣言し、`difference`の`2`乗の値に設定してください。最後に、`acc`と`squared`の合計を返してください。

# --hints--

`reduce`コールバックには`difference`変数が必要です。

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

`difference`変数は`el`から`mean`を引いた値に設定してください。
...
`reduce`コールバックには`squared`変数が必要です。
...
`squared`変数は`difference`の2乗の値に設定してください。
...
`reduce`コールバックは`acc`と`squared`の合計を返してください。
...
`0`の初期値は`.reduce()`メソッドから削除しないでください。
