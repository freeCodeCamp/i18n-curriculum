...
Настав час почати реалізовувати функціональність для відтворення відображених пісень.

Визначте функцію `playSong`, яка прийматиме один параметр, що представлятиме унікальний ідентифікатор пісні, яку ви хочете відтворити.

# --hints--

У вас має бути порожня функція з назвою `playSong`.

```js
assert.isFunction(playSong);
```

Ваша функція `playSong` має приймати один параметр.
