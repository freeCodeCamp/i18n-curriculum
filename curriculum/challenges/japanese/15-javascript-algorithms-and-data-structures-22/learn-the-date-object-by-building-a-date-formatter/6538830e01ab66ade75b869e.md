...
`Date` オブジェクトには、さまざまな形式の日付と時刻を取得できるたくさんのメソッドがあります。

その 1 つが `.getDate()` メソッドです。これは、その日付 (日時) の「日」を表す `1` から `31` の数値を返します。 例えば、次のように記述できます。
...
では `const` を使って、`day` という名前の変数を作成してください。そしてその変数に、`date` 変数の「日」を `.getDate()` メソッドで取得して、代入してください。
