...
最後，在第二個 `label` 後的新行上，創建另一個 `input` 元素。 將此 `type` 屬性設置爲 `number`，將 `min` 屬性設置爲 `0`（以確保不能添加負卡路里），將 `placeholder` 屬性設置爲 `Calories`，並將 `id` 屬性設置爲與第二個 `label` 元素的 `for` 屬性相匹配。

# --hints--

你的 `HTMLString` 中應該有兩個 `input` 元素。
...
你的新 `input` 元素應將 `type` 屬性設置爲 `number`。
...
你的 `input` 元素應將 `placeholder` 屬性設置爲 `Calories`。
...
你的 `input` 元素應將 `id` 屬性設置爲 `${entryDropdown.value}-${entryNumber}-calories`。
