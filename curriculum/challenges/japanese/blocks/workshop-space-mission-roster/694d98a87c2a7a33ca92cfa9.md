...
`for` ループを使って、`name` 配列にいるすべての宇宙飛行士の `updatedCrew` をログに出力してください。ループの後、`updatedCrew` 配列を返して `swapCrewMembers` 関数を完成させてください。

# --hints--

`for` ループを使って `updatedCrew` 配列を繰り返し処理し、各宇宙飛行士の `name` をログに出力してください。
...
宇宙飛行士の名前をログに出力した後、`updatedCrew` 配列を返してください。
