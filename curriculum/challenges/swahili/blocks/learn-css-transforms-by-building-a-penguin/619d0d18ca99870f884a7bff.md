...
Unapaswa kuongeza kipengele kimoja cha `div` ndani ya `.eye.left`, lakini ulibaini `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.eye.left > div')?.length ?? 0, 1);
```

Unapaswa kuongeza kipengele kimoja cha `div` ndani ya `.eye.right`, lakini ulibaini `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.eye.right > div')?.length ?? 0, 1);
```

Unapaswa kumpa `div` mpya ya kwanza `class` ya `eye-lid`.

```js
assert.exists(document.querySelector('.eye.left > div.eye-lid'));
```

Unapaswa kumpa `div` mpya ya pili `class` ya `eye-lid`.
