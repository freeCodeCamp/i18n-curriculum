...
Тепер викличте функцію `showElements` і передайте `multipleCard`, `fortuneContainer` та `text` як пропи.

# --hints--

Ви повинні викликати функцію `showElements` всередині методу `multipleCardSelected`.
...
Ви повинні передати `multipleCard` як перший аргумент функції `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

Ви повинні передати `fortuneContainer` як аргумент функції `showElements`.
...
Ви повинні передати `text` як аргумент функції `showElements`.
