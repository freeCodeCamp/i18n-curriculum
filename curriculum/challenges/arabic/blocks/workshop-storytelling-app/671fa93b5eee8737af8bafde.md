...
أنشئ دالة باسم `displayStory`. داخل الدالة، سجّل `"You clicked the button"` في وحدة التحكم.

# --hints--

يجب أن يكون لديك دالة باسم `displayStory`.

```js
assert.isFunction(displayStory);
```

يجب أن تسجل `"You clicked the button"` في وحدة التحكم داخل دالة `displayStory`.
