...
`forumCategory` 関数の中で、`let` という名前の新しい `selectedCategory` 変数を作成し、空のオブジェクトを代入してください。これは各カテゴリのカテゴリ名とクラス名を保存するために使います。

# --hints--

`let` を使って `selectedCategory` という名前の変数を宣言してください。

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

空のオブジェクトを `selectedCategory` 変数に代入してください。
