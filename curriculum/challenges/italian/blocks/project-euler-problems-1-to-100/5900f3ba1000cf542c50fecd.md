...
Sia ${p}(n)$ il numero di modi diversi in cui `n` monete possono essere separate in pile. Per esempio, cinque monete possono essere separate in pile in esattamente sette modi diversi, quindi ${p}(5) = 7$.
...
Trova il valore minimo di `n` per cui ${p}(n)$ è divisibile per `divisor`.
