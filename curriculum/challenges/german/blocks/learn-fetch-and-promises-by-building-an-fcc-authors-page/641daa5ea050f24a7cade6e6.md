...
Wenn diese Bedingung erfüllt ist, deaktivieren Sie den Button, indem Sie seine `disabled`-Eigenschaft auf `true` setzen. Setzen Sie außerdem die `textContent` des Buttons auf `"No more data to load"`.
...
Sie sollten die `disabled`-Eigenschaft von `loadMoreBtn` auf `true` setzen.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Sie sollten die `textContent` von `loadMoreBtn` auf `"No more data to load"` setzen.
