...
Declare uma arrow function chamada `rangeFromString`, que receba dois parâmetros, `num1` e `num2`.A função deve retornar implicitamente o resultado da chamada de `range`, com `num1` e `num2` como argumentos.

Por segurança, faça o parsing de `num1` e `num2` para números inteiros conforme você os passa para `range`.

# --hints--

Você deve declarar uma variável `rangeFromString` após a função `rangeRegex`.
...
Você deve usar `const` para declarar a variável `rangeFromString`.
...
A variável `rangeFromString` deve ser uma arrow function.
...
A função `rangeFromString` deve ter `num1` como primeiro parâmetro.
...
A função `rangeFromString` deve ter `num2` como segundo parâmetro.
...
A função `rangeFromString` deve usar um retorno implícito.
...
A função `rangeFromString` deve retornar o resultado da chamada da função `range`.
...
Você deve chamar `parseInt` com `num1` como argumento e passar o resultado para a chamada de `range`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

Você deve chamar `parseInt` com `num2` como argumento e passar o resultado para a chamada de `range`.
