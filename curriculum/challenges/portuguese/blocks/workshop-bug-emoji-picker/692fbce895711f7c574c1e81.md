...
Agora, dentro da função `isSelect`, retorne um booleano indicando se o `element` é uma instância de `HTMLSelectElement`. Use o operador `instanceof` para realizar essa verificação.

# --hints--

Sua função `isSelect` deve retornar `element instanceof HTMLSelectElement`.
