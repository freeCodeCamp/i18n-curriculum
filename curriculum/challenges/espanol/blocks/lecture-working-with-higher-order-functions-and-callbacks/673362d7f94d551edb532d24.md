...
Hay muchas formas diferentes de ordenar tus datos en programación. En esta lección, nos enfocaremos en el método incorporado `sort` en JavaScript.
...
## Manejo de valores `undefined` y espacios vacíos

También es importante entender cómo el método `sort` maneja los valores `undefined` y los espacios vacíos en un arreglo.

Cuando el método `sort` encuentra un valor `undefined`, no pasa ese valor a la función de comparación. En cambio, los valores `undefined` se mueven automáticamente al final del arreglo.

Aquí hay un ejemplo que muestra cómo `sort` maneja un arreglo con un valor `undefined`:
...
Observa que `undefined` se coloca al final del arreglo ordenado, después de todos los demás elementos.

De manera similar, cuando el método `sort` encuentra un espacio vacío en un arreglo disperso, preserva ese espacio vacío y lo mueve al final del arreglo. Si hay tanto valores `undefined` como espacios vacíos, los valores `undefined` aparecerán primero, seguidos por los espacios vacíos al final.

Aquí hay un ejemplo que demuestra este comportamiento:
...
En este ejemplo, las cadenas `"apple"` y `"banana"` se ordenan alfabéticamente primero. Luego, el valor `undefined` se coloca después de ellas. Finalmente, el espacio vacío se preserva al final del arreglo.
