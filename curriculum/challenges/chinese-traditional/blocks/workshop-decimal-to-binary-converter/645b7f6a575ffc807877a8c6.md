...
你的 `decimalToBinary` 函式（程式）運作良好，但有一個問題－因為你在 `while` 迴圈中的條件，它只適用於大於 `0` 的數字。如果你嘗試將 `0` 轉換成二進位，頁面上將不會新增任何內容。

為了解決這個問題，為你的程式碼添加一個 `if` 述語來檢查 `input` 是否相等於 `0`。目前先將 `if` 述語的主體留空。

# --hints--

你應該添加一個 `if` 述語來檢查 `input` 是否相等於 0。
