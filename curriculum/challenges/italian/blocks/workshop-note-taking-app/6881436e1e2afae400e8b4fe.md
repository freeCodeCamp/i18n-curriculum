...
Al momento, ogni volta che l’utente modifica una nota e clicca all’esterno della nota, la variabile `currentContent` viene aggiornata. Ma cosa succede se l’utente mette il focus sulla nota e decide di uscire senza fare modifiche?

In questo caso, puoi aggiungere una condizione per gestire questo caso.

Sopra la riga `currentContent = newContent;`, aggiungi un’istruzione `if` per verificare se `currentContent` è uguale a `newContent`. Se è così, esci dalla funzione.

# --hints--

Dovresti aggiungere un’istruzione `if` per verificare se `currentContent` è uguale a `newContent`. Se è così, esci dalla funzione.
