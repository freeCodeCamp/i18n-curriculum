...
حاليًا تُرجع دالة `range` مصفوفة بالطول الصحيح، لكن جميع القيم هي قيمة `start`. لإصلاح هذا، اربط طريقة `.map()` بطريقة `.fill()` الخاصة بك.

مرر إلى طريقة `.map()` دالة استدعاء راجع تأخذ `element` و`index` كمعلمات وتُرجع مجموع هاتين المعلمتين.
...
يجب أن تُرجع دالة الاستدعاء الراجع في `.map()` ضمنيًا مجموع `element` و`index`.
