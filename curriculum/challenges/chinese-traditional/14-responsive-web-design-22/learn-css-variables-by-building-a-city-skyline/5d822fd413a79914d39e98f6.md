...
在這裏，它會在元素的 `0%` 和 `20%` 之間從 `color1` 過渡到 `color2`，然後其餘部分過渡到 `color3`。 將 `80%` 添加到 `.bb1d` 漸變的 `--building-color1` 顏色中，以便你可以看到它的實際效果。

# --hints--

你應該將值 `80%` 添加到 `.bb1d` 的 `linear-gradient` 中的 `--building-color1` 顏色。
