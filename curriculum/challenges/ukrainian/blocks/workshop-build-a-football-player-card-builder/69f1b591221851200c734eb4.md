...
Перейшовши до другого `div` елемента, додайте `label` з `className` `label`, `htmlFor` з `overallRating`, текст `Overall` та `input` з `id`, встановленим на `overallRating`, `className` з `input` і типом `number`.

# --hints--

Ваш другий `form-group` у рядку має містити `label` з `className` `label` і `htmlFor` `overallRating`.
...
Ваш загальний рейтинг `label` має містити текст `Overall`.
...
Ваш другий `form-group` у рядку має містити `input` з `id` `overallRating` і `type` `number`.
