...
`select` 要素から値を取得する必要があります。これらの値は現在文字列であり、数値に変換します。

配列を繰り返すには `map` 関数を使ってください。`map` にコールバック関数を渡し、その関数は `dropdown` パラメータを受け取り `dropdown.value` を返します。

# --hints--

配列に対して `map` メソッドを直接チェーンすることをお勧めします。

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.getElementsByClassName\(\s*('|"|`)values-dropdown\1\s*\)\s*\]\s*\.map\(/);
```

`map` コールバック関数はアロー関数構文を使うべきです。

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.getElementsByClassName\(\s*('|"|`)values-dropdown\1\s*\)\s*\]\s*\.map\s*\(\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`map` コールバック関数は `dropdown` パラメータを受け取るべきです。
...
`map` コールバック関数は `dropdown.value` を暗黙的に返すべきです。
