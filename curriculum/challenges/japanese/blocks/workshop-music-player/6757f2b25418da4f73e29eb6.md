...
次に、プレイヤーの表示に現在の曲のタイトルとアーティスト名を表示する必要があります。

まずは、曲のタイトルとアーティスト名を表示する役割を持つ `#player-song-title` と `#player-song-artist` の要素にアクセスし、それぞれを変数 `playingSong` と `songArtist` に格納してください。

# --hints--

`playingSong` という名前の変数があるはずです。

```js
assert.isDefined(playingSong);
```

`playingSong` 変数には `#player-song-title` 要素の値が入っているはずです。

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

`songArtist` という名前の変数があるはずです。

```js
assert.isDefined(songArtist);
```

`songArtist` 変数には `#player-song-artist` 要素の値が入っているはずです。
