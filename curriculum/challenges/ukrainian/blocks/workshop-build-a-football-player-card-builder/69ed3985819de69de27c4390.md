...
Тепер оновіть `FootballPlayerCard`, щоб передавати `defaultPlayer` до `PlayerCard` замість рядкового об’єкта.

# --hints--

Ви повинні мати `defaultPlayer` як значення пропа `player`, який передається до `PlayerCard`.
