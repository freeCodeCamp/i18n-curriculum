...
Erstellen Sie einen `.list-title, .list-subtitle`-Selektor und setzen Sie die `color`-Eigenschaft auf `#00beef`.

# --hints--

Sie sollten einen `.list-title, .list-subtitle`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'))
```

Ihr `.list-title, .list-subtitle`-Selektor sollte eine `color`-Eigenschaft haben, die auf `#00beef` gesetzt ist.
