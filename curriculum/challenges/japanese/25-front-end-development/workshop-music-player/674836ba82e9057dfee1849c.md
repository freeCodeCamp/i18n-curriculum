...
Then, set `userData.currentSong` to `song`, and to finally play the song, use the `play()` method on the `audio` variable. `play()` は Web Audio API のメソッドで、mp3 ファイルを再生できます。
