...
終於！你來到了最後一棟建築！為你的建築添加一個方向為 `90deg` 的重複漸層。從 `0%` 到 `10%` 使用建築顏色，從 `10%` 到 `30%` 使用 `transparent`。

# --hints--

你應該在 `background` 屬性的 `.fb6` 中添加 `repeating-linear-gradient`。
...
你應該將 `repeating-linear-gradient` 的第一個顏色設為從 `0%` 到 `10%` 的 `--building-color3`。
...
你應該給 `repeating-linear-gradient` 在 `10%` 到 `30%` 之間添加第二個顏色 `transparent`。
