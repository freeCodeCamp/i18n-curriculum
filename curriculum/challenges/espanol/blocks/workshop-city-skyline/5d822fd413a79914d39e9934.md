...
¡Finalmente! ¡Has llegado al último edificio! Agrega un degradado repetido con una dirección de `90deg`. Usa el color del edificio desde el `0%` hasta el `10%` y `transparente` desde el `10%` hasta el `30%`.

# --hints--

Debe agregar un `repeating-linear-gradient` a `.fb6` en la propiedad de fondo.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

Debe darle al `repeating-linear-gradient` una dirección de `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Debe darle al `repeating-linear-gradient` un primer color de `--building-color3` desde `0%` hasta `10%`.
...
Debe darle al `repeating-linear-gradient` un segundo color de `transparente` desde `10%` hasta `30%`.
