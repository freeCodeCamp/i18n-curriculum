...
返回到你的 `attack` 函数 - 在 `else if` 块内，创建另一个 `if` 和 `else` 语句。 如果玩家正在与龙战斗（`fighting` 将是 `2`），则调用 `winGame` 函数。 将 `defeatMonster()` 调用移至 `else` 块。

对于这一步，你需要使用严格相等（`===`）运算符来检查 `fighting` 是否等于 `2`。 


# --hints--

你应该在 `else if` 块中添加一个新的 `if` 语句。
...
你的新 `if` 语句应该检查 `fighting` 是否严格等于 `2`。
...
你的 `else` 块应该调用 `defeatMonster` 函数。
