...
أنشئ استعلام `@media` آخر لـ`only screen` مع `max-width` بقيمة `600px`. بداخله، أنشئ قاعدة `.text-with-images` وأعطها خاصية `grid-template-columns` بقيمة `1fr`.

سيؤدي هذا إلى دمج مساحة النص السفلية في عمود واحد على الشاشات الأصغر.

# --hints--

يجب أن تنشئ استعلام `@media` جديد لـ`only screen and (max-width: 600px)`. يجب أن يكون هذا أسفل استعلام `@media` السابق.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

يجب أن يحتوي استعلام `@media` الجديد على محدِّد `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

يجب أن يحتوي محدِّد `.text-with-images` الجديد على خاصية `grid-template-columns` بقيمة `1fr`.
