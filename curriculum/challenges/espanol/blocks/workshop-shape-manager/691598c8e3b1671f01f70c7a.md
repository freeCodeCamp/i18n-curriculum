...
Dentro de la función `handleShapeSelect`, llama al método `.preventDefault()` en `e` y llama a la función `clearInputFields()` para limpiar los campos de entrada.

# --hints--

Debes llamar a `.preventDefault()` en `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

Debes llamar a la función `clearInputFields`.
