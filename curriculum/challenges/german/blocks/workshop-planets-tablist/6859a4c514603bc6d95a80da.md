...
Jetzt ist es an der Zeit, die Tab-Panels mit ihren jeweiligen Tabs zu verknüpfen.

Fügen Sie für das erste `tabpanel`-Element das `id`-Attribut mit dem Wert `"panel-earth"` und ein `aria-labelledby`-Attribut mit dem Wert `"tab-earth"` hinzu.

# --hints--

Ihr erstes `tabpanel`-Element sollte ein `id`-Attribut haben.

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("id"));
```

Ihr `id`-Attribut sollte den Wert `"panel-earth"` haben.

```js
assert.equal(document.querySelector('[role="tabpanel"]').getAttribute("id"), "panel-earth");
```

Ihr erstes `tabpanel`-Element sollte ein `aria-labelledby`-Attribut haben.

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("aria-labelledby"));
```

Ihr `aria-labelledby`-Attribut sollte den Wert `"tab-earth"` haben.
