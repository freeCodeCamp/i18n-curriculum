...
次に、`caption` 要素をヘッダーのように見えるようにスタイル設定してください。`table caption` セレクターを作成します。テキストの色を `#356eaf`、サイズを `1.3em`、フォントの太さを通常に設定してください。

# --hints--

`table caption` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption'));
```

`table caption` セレクターには `color` プロパティを `#356eaf` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('color') === 'rgb(53, 110, 175)');
```

`table caption` セレクターには `font-size` プロパティを `1.3em` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('font-size') === '1.3em');
```

`table caption` セレクターには `font-weight` プロパティを `normal` に設定してください。
