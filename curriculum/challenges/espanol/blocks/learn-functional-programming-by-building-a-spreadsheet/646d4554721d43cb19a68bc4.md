...
El parámetro `fn` será pasado el nombre de una función, como `"SUM"`. Actualiza `apply` para devolver implícitamente la función desde su objeto `spreadsheetFunctions` usando la variable `fn` como clave para el acceso al objeto.

Recuerda que `fn` puede no contener una cadena en minúsculas, así que necesitarás convertirla en una cadena en minúsculas.

# --hints--

Tu función `apply` debe usar un retorno implícito.
...
Tu función `apply` debe acceder al objeto `spreadsheetFunctions`.
...
Tu función `apply` debe acceder a la propiedad del objeto `spreadsheetFunctions` que coincida con el valor `fn`.
...
Tu función `apply` debe llamar al método `.toLowerCase()` en `fn` en el acceso a la propiedad.
