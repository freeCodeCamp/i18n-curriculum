...
Das nächste `input` ist ein Kontrollkästchen, um alle Superheldenkräfte zu markieren, die auf den Benutzer zutreffen.

Erstellen Sie ein `label`-Element mit einem `className` von `section` und `column` sowie dem Text `List your powers (select all that apply):`.

# --hints--

Sie sollten ein `label`-Element mit einem `className` von `section` und `column` erstellen.
...
Ihr `label`-Element sollte den Text `List your powers (select all that apply):` enthalten.
