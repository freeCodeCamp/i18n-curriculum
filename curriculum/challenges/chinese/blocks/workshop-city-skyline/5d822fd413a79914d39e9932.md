...
构建下一栋建筑！ 这是前景中的绿色建筑。 将其背景设置为 `repeating-linear-gradient`，从 `0%` 到 `5%` 使用其建筑颜色，从 `5%` 到 `10%` 使用 `transparent`。

# --hints--

你应该给 `.fb5` 一个 `background` 属性。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

你应该给 `background` 一个 `repeating-linear-gradient`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

你应该将从 `0%` 到 `5%` 的 `--building-color2` 设置为 `repeating-linear-gradient` 的第一种颜色。
...
你应该将从 `5%` 到 `10%` 的 `transparent` 设置为 `repeating-linear-gradient` 的第二种颜色。
