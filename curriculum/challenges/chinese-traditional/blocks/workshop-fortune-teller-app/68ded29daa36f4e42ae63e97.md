...
使用 `forEach` 陣列方法來迭代 elements 引數，完成這個函式。在回呼函式內，使用 `.add()` 的 `classList` 方法，為每個元素添加 hidden 類別。

# --hints--

你應該在 `forEach` 參數上附加一個 `elements` 方法。
...
在 forEach 方法內，應該使用 `classList.add` 方法添加 hidden 類別。
