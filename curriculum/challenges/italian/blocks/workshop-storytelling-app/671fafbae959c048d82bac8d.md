...
Modifica il listener dell'evento `scaryStoryBtn` per chiamare la funzione `displayStory` con l'argomento `"scary"`. Inoltre, aggiungi un listener dell'evento `click` ai pulsanti `funnyStoryBtn` e `adventureStoryBtn` che chiamano la funzione `displayStory` con gli argomenti `"funny"` e `"adventure"` rispettivamente.

Con questo, hai completato con successo il workshop.

# --hints--

Quando clicchi il `scaryStoryBtn`, il paragrafo `#result` dovrebbe mostrare la storia spaventosa `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` con un bordo di colore `#ee4b2b`.
...
Quando clicchi il `funnyStoryBtn`, il paragrafo `#result` dovrebbe mostrare la storia divertente con un bordo di colore `#f1be32`.
...
Quando clicchi il `adventureStoryBtn`, il paragrafo `#result` dovrebbe mostrare la storia d'avventura con un bordo di colore `#acd157`.
