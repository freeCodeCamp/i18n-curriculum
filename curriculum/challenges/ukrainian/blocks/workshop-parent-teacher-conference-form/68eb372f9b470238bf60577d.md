...
Тепер настав час визначити, що відбувається, коли радіокнопка вибрана.

Ви стилізуєте внутрішнє коло лише тоді, коли радіокнопка вибрана, тому ваш селектор має бути `.contact-method-radio-btn:checked::before`.

У правилі встановіть `transform` у `translate(3px, 3px) scale(1)` і `background-color` у `lightgreen`.

`transform` трохи зсуває внутрішнє коло і збільшує його до повного розміру за допомогою `scale(1)`, а фон світло-зеленого кольору заповнює коло, щоб показати, що опція вибрана, роблячи його видимим.

# --hints--

Ви маєте націлитися на псевдоелемент `.contact-method-radio-btn:checked::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

Встановіть властивість `transform` у `translate(3px, 3px) scale(1)`.
...
Встановіть властивість `background-color` у `lightgreen`.
