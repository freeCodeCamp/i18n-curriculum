...
在先前的課程中，你學會了如何使用 `map()` 方法來呈現一個選項列表，如下所示：
...
請記住，`key` 必須始終是唯一的。它有助於 React 識別哪些項目已更改、被添加或移除。

在第一個 `option` 之後，使用箭頭函式（arrow function）以 `source` 作為參數，遍歷 `powerSourceOptions` 陣列。

然後在 `map()` 方法內，創建一個 `option` 元素，其 `key` 為 `source`，`value` 為 `source`。最後，使用 `{source}` 來顯示下拉選單中每個電源的名稱。

# --hints--

你應該遍歷 `powerSourceOptions`。

```js
assert.match(code, /\{\s*powerSourceOptions\s*\.\s*map\s*\(/)
```

你應該將 `source` 作為 map 的參數傳入。

```js
assert.match(code, /\{\s*powerSourceOptions\s*\.\s*map\s*\(\s*(source|\(\s*source\s*\))\s*=>/)
```

你應該創建一個 `option` 元素，其 `key` 為 `source`，`value` 為 `source`。
...
對於最後 6 個 `option` 元素，每個文字內容應對應 `powerSourceOptions` 陣列中的一個電源選項。 
