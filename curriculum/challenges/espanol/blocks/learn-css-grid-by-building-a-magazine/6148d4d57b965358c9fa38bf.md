...
Crea un selector `.text-with-images` y establece la propiedad `display` a `grid`.
...
Tu selector `.text-with-images` debe tener una propiedad `display` establecida en `grid`.
