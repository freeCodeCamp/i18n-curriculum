...
داخل `tbody`، أضف `tr` مع تعيين `class` إلى `total`. ضمنه، أضف `th` بالنص `Total Net Worth`، ولفّ `Net Worth` داخل `span` مع تعيين `class` إلى `sr-only`.
...
يجب أن تلتف النص `Net Worth` داخل عنصر `span`.
