...
Не соромтеся змінювати коефіцієнти `lin_eq`, щоб побачити, як змінюється вивід.

Потім видаліть свій виклик `print(solver(lin_eq))` та надрукуйте результат виклику `solver()` з `quadr_eq` як аргументом.

# --hints--

Код не повинен містити `print(solver(lin_eq))`.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

Надрукуйте `solver(quadr_eq)`.
