...
يجب أن تصف خاصية الصورة `alt` محتوى الصورة. يعلن قارئ الشاشة النص البديل بدلًا من الصور. إذا تعذّر تحميل الصورة، يُعرض هذا النص بدلًا منها.

لإكمال المشروع، أضف خاصية `alt` إلى جميع صور القطط التسع لوصفها. استخدم قيمة لا تقل عن خمسة أحرف لكل منها.


# --hints--

يجب أن تحتوي جميع عناصر `img` التسعة على خاصية `alt` بنص يصف كل صورة، مع استخدام كل وصف على الأقل `5` أحرف.
