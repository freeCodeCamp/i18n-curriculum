...
Ifuatayo, ondoa `width` na `height` kutoka `.bb2a`, na badilisha `border-left` na `border-right` kutumia `5vw` badala ya `1vw`. Kipengele sasa kitakuwa na ukubwa sifuri na mipaka itakutana katikati.

# --hints--

Unapaswa kuondoa `width` kutoka `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.width);
```

Unapaswa kuondoa `height` kutoka `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.height);
```

Unapaswa kubadilisha `border-left` kutumia `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid rgb(153, 153, 153)");
```

Unapaswa kubadilisha `border-right` kutumia `5vw`.
