...
Setze `monsterHealth` auf `monsterHealth` minus die Stärke der aktuellen Waffe des Spielers.

Remember that you can access the power of the player's current weapon using `weapons[currentWeaponIndex].power`.

# --hints--

Du solltest eine zusammengesetzte Zuweisung verwenden, um die `monsterHealth`-Variable zu ändern.
