...
在 `Bee` 類別中，創建一個建構式，該建構式接受一個型別為 `emojiElement` 的單一參數 `HTMLParagraphElement`。

在建構式內，使用 `super` 參數呼叫 `emojiElement` 函式，以喚起父類別 `Bug` 的建構式。接著，將 `emoji` 屬性設定為字串值 `"🐝"`。
