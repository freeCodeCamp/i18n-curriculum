...
Dein `h2`-Element sollte ein öffnendes Tag haben. Opening tags have this syntax: `<elementName>`.

```js
assert(document.querySelectorAll('h2').length === 2);
```

Dein `h2`-Element sollte ein schließendes Tag haben. Closing tags have a `/` just after the `<` character.
