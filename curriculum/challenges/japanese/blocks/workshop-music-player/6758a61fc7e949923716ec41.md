...
次に、`querySelectorAll` メソッドを使って `.playlist-song` 要素を取得し、`songs` という名前の変数にストアしてください。

その後、`forEach` に対して `songs` メソッドを呼び出してください。メソッドのパラメータとして `song` を使うコールバック関数を渡します。波括弧は今は空のままにしてください。

# --hints--

`songs` という名前の変数があるはずです。

```js
assert.isDefined(songs);
```

すべての `.playlist-song` 要素のコレクションを `songs` 変数に代入してください。

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

`forEach` に対して `songs` メソッドを呼び出し、メソッドのパラメータとして `song` を使うコールバックを渡してください。
