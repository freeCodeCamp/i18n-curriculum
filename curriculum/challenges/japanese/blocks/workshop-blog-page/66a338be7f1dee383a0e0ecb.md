...
2つ目のブログ投稿には、もう1つ`article`要素を追加する必要があります。

`article`要素の中に、`h3`のテキストを持つ`Mr. Whiskers' First Bath`要素を追加してください。

`h3`要素の下に、2つの段落のロレム・イプサムテキストを追加してください。

# --hints--

`article`要素の中に、`section`に設定された`id`を持つ2つ目の`posts`要素があるはずです。
...
`h3`要素の中に、`article`のテキストを持つ`Mr. Whiskers' First Bath`要素があるはずです。
...
最初の段落要素にはロレム・イプサムテキストがあるはずです。スペルや句読点の誤りがないか再確認してください。
...
2つ目の段落要素にもロレム・イプサムテキストがあるはずです。スペルや句読点の誤りがないか再確認してください。
