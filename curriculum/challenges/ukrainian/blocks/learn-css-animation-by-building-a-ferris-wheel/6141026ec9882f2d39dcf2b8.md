...
Між селекторами `0%` і `100%` додайте селектор `50%`. Він застосовуватиметься посередині циклу анімації. Встановіть властивість `background-color` у значення `purple`.

# --hints--

Ви маєте створити новий селектор `50%` у вашому правилі `@keyframes cabins`.
...
Ваш селектор `50%` має бути між селекторами `0%` і `100%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '50%');
```

Ваш селектор `50%` має мати властивість `background-color`, встановлену в `purple`.
