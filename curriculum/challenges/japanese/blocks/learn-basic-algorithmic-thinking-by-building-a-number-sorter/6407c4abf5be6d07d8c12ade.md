...
配列の各要素に対して処理を行うには、配列を繰り返すためのメソッドを使ってください。

`forEach()` メソッドを使い、`num` と `i` をパラメータに取る空のコールバックを渡してください。

# --hints--

`.forEach()` パラメータを繰り返すには `array` メソッドを使うべきです。

```js
assert.match(code, /array\.forEach\(/);
```

`.forEach()` メソッドはアロー構文を使ったコールバック関数を取るべきです。

```js
assert.match(code, /array\.forEach\s*\(\s*\(.*\)\s*=>/);
```

コールバック関数は `num` と `i` の2つのパラメータを取るべきです。
