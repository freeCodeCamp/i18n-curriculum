...
`try` 블록 안에서 현재 플레이어 데이터를 저장 키와 문자열화한 플레이어 객체를 사용해 `localStorage`를 호출하여 `localStorage.setItem`에 저장하세요.

그리고 `catch` 블록 안에서는 문자열 `Failed to save player data:`와 `error` 매개변수를 차례로 콘솔에 기록하세요.

# --hints--

`useEffect` try 블록에서는 `localStorage.setItem`과 문자열화한 `STORAGE_KEY` 객체를 사용해 `player`를 호출해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

`useEffect` catch 블록에서는 `'Failed to save player data:'`과 `error`를 콘솔에 기록해야 합니다.
