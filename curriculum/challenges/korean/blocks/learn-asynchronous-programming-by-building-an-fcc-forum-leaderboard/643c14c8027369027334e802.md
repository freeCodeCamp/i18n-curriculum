...
응답 본문을 JSON 객체로 가져오고 싶습니다. `.json()` 변수의 `res` 메서드는 프로미스를 반환하므로 `await` 해야 합니다.

`data`라는 상수를 만들고 `await res.json()` 값을 할당하세요.

# --hints--

`const`라는 `data` 변수가 있어야 합니다.

```js
assert.match(code, /const\s+data/);
```

`await res.json()` 변수에 `data`을 할당해야 합니다.
