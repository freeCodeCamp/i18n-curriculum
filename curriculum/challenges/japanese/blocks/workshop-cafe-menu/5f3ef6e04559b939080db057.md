...
メニューのすべての`4`側は同じ内部の間隔なので、4つのプロパティを削除して、値が`padding`の単一の`20px`プロパティを使ってください。

# --hints--

`padding-left`プロパティを削除してください。

```js
assert.notMatch(code, /padding-left/i);
```

`padding-right`プロパティを削除してください。

```js
assert.notMatch(code, /padding-right/i);
```

`padding-top`プロパティを削除してください。

```js
assert.notMatch(code, /padding-top/i);
```

`padding-bottom`プロパティを削除してください。
...
`.menu`要素は`padding`の値を`20px`にしてください。
