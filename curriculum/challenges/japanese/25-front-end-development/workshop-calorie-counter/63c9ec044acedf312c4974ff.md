...
ループが完了したら、`budgetNumberInput` もクリアする必要があります。 `budgetNumberInput` の `value` プロパティを空の文字列に設定してください。

# --hints--

`clearForm` 関数で、`budgetNumberInput` の要素の `value` プロパティにアクセスする必要があります。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value/);
```

`clearForm` 関数では、`budgetNumberInput` の要素の `value` プロパティを空の文字列に設定してください。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value\s*=\s*('|"|`)\1/);
```

`for` ループの後で `budgetNumberInput` に変更を加えてください。
