...
`if` 文の中で、プレイヤーの `x` の速度を `0` に、プレイヤーの `y` の速度を `0` に設定してください。

その下に、`return` 文を追加してください。

# --hints--

プレイヤーの `x` の速度を `0` に設定する必要があります。

```js
assert.match(movePlayer.toString(), /player\.velocity\.x\s*=\s*0\s*;?/);
```

プレイヤーの `y` の速度を `0` に設定する必要があります。

```js
assert.match(movePlayer.toString(), /player\.velocity\.y\s*=\s*0\s*;?/);
```

`return` 文の中に `if` 文があるはずです。
