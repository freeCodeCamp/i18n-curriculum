...
だいぶ良くなってきましたが、緑のマーカーの下端をもう少し暗くして、立体感を出す必要があります。

同じ`linear-gradient`関数内で、赤が`11`、緑が`6C`、青が`31`の値となる16進カラーコードを3番目の色の引数として追加してください。

# --hints--

`.green`のCSSルールには、`background`プロパティを`linear-gradient(180deg, #55680D, #71F53E, #116C31)`に設定してください。
