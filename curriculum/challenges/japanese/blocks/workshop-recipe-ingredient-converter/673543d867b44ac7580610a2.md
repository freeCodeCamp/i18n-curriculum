...
このステップでは、`updateResultsList` 関数を完成させます。

次に、`units` 配列を繰り返し処理してください。各値について、現在の単位（`unitToConvert.value`）と等しくないかどうかを確認します。

条件が真の場合、既存の単位を現在の単位に変換し、次の形式で `resultList` に新しいリスト項目を追加してください：

`ingredient: quantity unit`

たとえば、入力が `flour`、`5`、`gram` の場合、結果は次のようになります：
...
ボタンをクリックすると、`updateResultsList` 関数が正しく DOM を更新する必要があります。
