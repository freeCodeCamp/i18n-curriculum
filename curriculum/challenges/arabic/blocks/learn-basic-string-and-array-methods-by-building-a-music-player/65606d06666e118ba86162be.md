...
أضف كائنًا ثانيًا بالمفاتيح والقيم التالية:
...
يجب أن تحتوي مصفوفة `allSongs` على قيمتين.

```js
assert.equal(allSongs.length, 2);
```

يجب أن تحتوي مصفوفة `allSongs` على كائن كقيمتها الثانية.

```js
assert.isObject(allSongs[1]);
```

يجب أن يحتوي الكائن الثاني في مصفوفة `allSongs` على خاصية `id` بقيمة الرقم `1`.

```js
assert.equal(allSongs[1].id, 1);
```

يجب أن يحتوي الكائن الثاني في مصفوفة `allSongs` على خاصية `title` بقيمة السلسلة النصية `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

يجب أن يحتوي الكائن الثاني في مصفوفة `allSongs` على خاصية `artist` بقيمة السلسلة النصية `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

يجب أن يحتوي الكائن الثاني في مصفوفة `allSongs` على خاصية `duration` بقيمة السلسلة النصية `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

يجب أن يحتوي الكائن الثاني في مصفوفة `allSongs` على خاصية `src` بقيمة السلسلة النصية `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
