...
Füge nach deinem `.daily-value`-Element eine mittlere Trennlinie hinzu. Erstelle unter dieser neuen Trennlinie ein `p`-Element mit dem `class`-Attribut, das auf `note` gesetzt ist.

Gib dem `p`-Element den folgenden Text:
...
Du solltest nach deinem `.daily-value`-Element ein neues `div` erstellen.

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

Dein neues `div` sollte die `class` auf `divider medium` gesetzt haben.
...
Du solltest nach deinem neuen `div`-Element ein `p`-Element erstellen.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'p');
```

Dein neues `p`-Element sollte `class` auf `note` gesetzt haben.
