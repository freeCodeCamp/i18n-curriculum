...
The <dfn>find()</dfn> method retrieves the first element within an array that fulfills the conditions specified in the provided callback function. If no element satisfies the condition, the method returns `undefined`.
...
Використайте метод `find` на `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

Параметром стрілкової функції зворотного виклику методу `find` має бути `song`.
