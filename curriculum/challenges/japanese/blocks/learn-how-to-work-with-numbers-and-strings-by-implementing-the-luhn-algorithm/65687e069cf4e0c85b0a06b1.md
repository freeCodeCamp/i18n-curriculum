...
`if` 文の外側に、`number` を `sum_of_even_digits` に追加してください。また、`print` の呼び出しを削除してください。

# --hints--

`sum_of_even_digits += number` ループの中に `for` があるはずです。
...
`print(number)` ループの中に `for` があってはいけません。
...
`sum_of_even_digits += number` 文の中に `if` があってはいけません。
