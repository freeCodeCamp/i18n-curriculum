...
在此步驟中，你將完成 `updateResultsList` 函式（程式）。 

現在對 `units` 陣列進行迭代。對每個值，檢查它是否不相等於目前的單位（`unitToConvert.value`）。

如果條件為真，將現有單位轉換為當前單位，並以以下格式將新的列表項目添加到 `resultList`：

`ingredient: quantity unit`

例如，如果輸入是 `flour`、`5` 和 `gram`，結果應該是：
...
當點擊按鈕時，你的 `updateResultsList` 函式（程式）應該正確更新 DOM。
