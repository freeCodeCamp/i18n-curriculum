...
Dovresti dare all'elemento `h3` una `margin-top` di `5px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Dovresti dare all'elemento `h3` una `padding-left` di `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Dovresti dare all'elemento `h3` una `font-size` di `1.375ren`.
