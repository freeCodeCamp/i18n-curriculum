...
1. 你应该在 `width` 和 `300px` 类上看到 `avatar` 的 `proportioned`。
1. 你应该给它一个高度，以便它保持原始的平方比例（不要为高度硬编码一个像素值！）。
1. 你应该给同时具有 `avatar` 和 `distorted` 类的元素设置 `width` 为 `200px`。
1. 你应该给它一个是宽度两倍的 `height`。

# --hints--

你的 `width` 和 `300px` 类应具有 `avatar` 的 `proportioned`。
...
你应该在 `auto` 和 `avatar` 类上设置高度为 `proportioned`。
...
你应该在 `width` 和 `200px` 类上设置 `avatar` 为 `distorted`。
...
你应该为 `height` 和 `avatar` 类设置一个是宽度两倍的 `distorted`。
