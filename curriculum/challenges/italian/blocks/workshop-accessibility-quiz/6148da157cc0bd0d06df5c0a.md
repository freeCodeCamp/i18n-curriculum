...
Per allineare i riquadri di input tra loro, crea un nuovo insieme di regole che abbia come obiettivo tutti gli elementi `input` e `label` all’interno di un elemento `.info` e imposta la proprietà `display` su `inline-block`.

Allinea inoltre il testo dell’elemento `label` a destra.
...
Dovresti dare agli elementi `input` e `label` una proprietà `display` con valore `inline-block`.
...
Dovresti dare all’elemento `label` una proprietà `text-align` con valore `right`.
