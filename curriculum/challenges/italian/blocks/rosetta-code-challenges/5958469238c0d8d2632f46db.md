...
Dati numeri interi non negativi `m` e `n`, genera tutte le combinazioni di dimensione `m` degli interi da `0` (zero) a `n-1` in ordine ordinato (ogni combinazione è ordinata e l'intera tabella è ordinata).
