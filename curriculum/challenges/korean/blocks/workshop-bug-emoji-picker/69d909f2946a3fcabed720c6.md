...
이제 `if` 검사가 준비되었으니, `e.target.value`을 사용해 `bugMap`에서 해당 버그를 찾아 그 `render` 메서드를 호출하세요. 그러면 버그를 선택하고 바로 결과를 확인할 수 있습니다.

이로써 Bug Emoji Picker 워크숍이 완료됩니다!

# --hints--

`render`에서 해당 버그에 `bugMap` 메서드를 호출해야 합니다.
