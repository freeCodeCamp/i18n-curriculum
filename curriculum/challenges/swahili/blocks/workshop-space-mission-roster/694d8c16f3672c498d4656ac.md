...
Kama ilivyo awali, anza kwa kuthibitisha ingizo ndani ya kitendakazi cha `swapCrewMembers`. Hasa, lazima uthibitishe nambari za wanasayansi wa anga wanaobadilishwa.

Kama mojawapo ya yafuatayo ni kweli:

- `fromIndex` ni hasi

- `fromIndex` ni kubwa au sawa na `crew.length`

- `toIndex` ni hasi

- `toIndex` ni kubwa au sawa na `crew.length`

basi, unapaswa kuandika `"Invalid crew indices"` kwenye konsoli na kuita `return` kutoka kwa kitendakazi.

Unaweza kutumia mlinganishi wa AU `||` kuangalia masharti mengi kwa wakati mmoja:
...
Unapaswa kuthibitisha nambari za wanasayansi kwa kutumia tamko la `if`.
...
Unapaswa kwanza kuthibitisha nambari za wanasayansi na, ikiwa si sahihi, andika ujumbe wa `"Invalid crew indices"` kabla ya kutoka kwenye kitendakazi kwa kutumia `return`.
...
Mara moja baada ya kuandika ujumbe katika uthibitisho, itaje `return` kutoka mapema kwenye kitendakazi.
...
Ukipata ingizo sahihi, kitendakazi chako cha `swapCrewMembers` hakipaswi kuandika ujumbe lolote la kosa wala kutoka mapema.
