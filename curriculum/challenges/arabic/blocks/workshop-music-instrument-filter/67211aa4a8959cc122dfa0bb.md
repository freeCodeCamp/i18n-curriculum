...
عدّل استدعاء `console.log` ليُسجل القيمة المختارة من القائمة المنسدلة.

يمكنك الوصول إلى القيمة الحالية لعنصر `select` باستخدام خاصيته `.value`. على سبيل المثال:
...
يجب أن تسجل القيمة المختارة من القائمة المنسدلة عند حدوث `change` على `selectContainer`.
