...
`elemValue` は関数を返すため、`addCharacters` 関数は最終的に関数の参照の配列を返します。`.map()` メソッドは `elemValue` 関数の内部関数を実行する必要があるので、参照ではなく `elemValue` を呼び出す必要があります。`num` 関数の引数として `elemValue` を渡してください。

<!-- TODO: Explain further? -->

# --hints--

`elemValue()` を `.map()` メソッド内で呼び出してください。
