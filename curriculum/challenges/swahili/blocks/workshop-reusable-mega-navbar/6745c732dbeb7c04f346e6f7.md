...
Unda kipengele cha `ul` ndani ya kipengele cha `nav`. Ndani ya kipengele hicho cha `ul`, unda vitu vitatu vya orodha.

# --hints--

Unapaswa kuunda kipengele cha `ul` ndani ya kipengele chako cha `nav`.

```js
assert.exists(document.querySelector('nav ul'));
```

Unapaswa kuunda vipengele vitatu vya `li` ndani ya kipengele chako cha `ul`.
