...
أخيرًا، بعد أن أكملت مُعيّن `salary`، يمكنك استدعاؤه داخل مُعيّن `level`.

عدّل السطر `self._salary = Employee._base_salaries[new_level]` ليُستدعى مُعيّن `salary` بدلًا من تعديل الخاصية `_salary` مباشرةً.

بهذا تكون ورشة عمل متتبع الراتب مكتملة.

# --hints--

يجب أن تضبط `self.salary` على `Employee._base_salaries[new_level]` داخل مُعيّن `level`.
