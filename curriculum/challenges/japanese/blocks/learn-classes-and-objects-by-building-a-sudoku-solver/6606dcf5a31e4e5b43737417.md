...
`valid_in_col()` メソッドを `col` と `num` を引数にして呼び出し、指定された列に挿入する数値が有効かどうかを確認し、その結果を変数 `valid_in_col` に代入してください。

# --hints--

変数 `valid_in_col` を宣言し、`self.valid_in_col(col, num)` を代入してください。
