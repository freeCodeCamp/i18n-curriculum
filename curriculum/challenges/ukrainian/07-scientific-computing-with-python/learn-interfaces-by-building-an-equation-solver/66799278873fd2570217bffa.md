...
Додайте інший `case`, для випадку коли довжина `results` є `1`. У цьому випадку призначте `result_list` список, що містить рядок у форматі `x = <root>`, де `<root>` є рішення рівняння. Відформатуйте рядок так, щоб для розв’язку відображалися як додатний, так і від’ємний знак.
...
Створіть новий `case` із шаблоном `1` після наявного блоку `case`.
...
Призначте список, що містить `f'x = {results[0]:+}'` до `result_list` у тілі нового `case`.
