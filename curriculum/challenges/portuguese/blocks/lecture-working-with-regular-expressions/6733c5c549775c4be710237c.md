...
Boas notícias! Nosso `replaceAll()` funcionou exatamente como queríamos: ele substituiu todas as ocorrências de `freecodecamp` em minúsculas pela versão em camelcase adequada.
...
Vamos atualizar nosso código para usar isso: vamos limpar nossas chamadas de `replaceAll` já que sabemos que funcionam:
