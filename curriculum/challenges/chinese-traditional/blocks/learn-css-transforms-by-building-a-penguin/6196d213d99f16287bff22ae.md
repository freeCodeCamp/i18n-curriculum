...
要完成背景，請在 `.back-mountain` 元素之後立即建立一個新的 `div` 元素，並為它添加 `sun` 類別。

# --hints--

你應該為你的 `body` 添加一個新的 `div` 元素。預期有 `--fcc-expected--` 個 `div` 元素，但實際找到 `--fcc-actual--` 個。

```js
assert.equal(document.querySelectorAll('body > div')?.length, 5);
```

你應該為新的 `div` 元素指定一個 `class` 為 `sun`。

```js
assert.include(document.querySelector('div:not(.back-mountain, .left-mountain, .penguin, .ground)')?.className, 'sun');
```

你應該將新的 `div` 元素緊接在 `.back-mountain` 元素之後放置。
