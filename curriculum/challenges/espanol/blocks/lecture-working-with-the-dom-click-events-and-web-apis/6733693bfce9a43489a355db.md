...
El evento `DOMContentLoaded` se activa cuando todo en el documento HTML ha sido cargado y analizado. Si tienes hojas de estilo externas o imágenes, el evento `DOMContentLoaded` no esperará a que estas se carguen. Solo esperará a que se cargue el HTML.
...
Ahora, echemos un vistazo a otro ejemplo usando el evento `DOMContentLoaded`. En este ejemplo, tenemos una imagen dentro del HTML. Para actualizar la imagen, podemos crear una función que cambie los atributos `src` y `alt` de la imagen:
