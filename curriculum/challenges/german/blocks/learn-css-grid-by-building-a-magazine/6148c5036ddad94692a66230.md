...
Jetzt wird der Auto-Placement-Algorithmus aktiviert, wenn Sie ein neues Icon-Element hinzufügen. Allerdings setzt der Algorithmus standardmäßig die neue Spaltenbreite auf `auto`, was nicht zu Ihren aktuellen Spalten passt.

Sie können dies mit der `grid-auto-columns`-Eigenschaft überschreiben. Geben Sie dem `.social-icons`-Selektor eine `grid-auto-columns`-Eigenschaft mit dem Wert `1fr`.

# --hints--

Ihr `.social-icons`-Selektor sollte eine `grid-auto-columns`-Eigenschaft mit dem Wert `1fr` haben.
