...
Infine, aggiorna la tua funzione interna per restituire una terza funzione. Anche questa terza funzione dovrebbe accettare un singolo argomento chiamato `quantity` e avere un corpo vuoto.

# --hints--

La tua seconda funzione dovrebbe restituire un'altra funzione.

```js
assert.isFunction(convertQuantity()());
```

La tua funzione più interna dovrebbe avere un parametro `quantity`.

```js
assert.match(convertQuantity()()?.toString(), /\(quantity\)/);
```

La tua funzione più interna dovrebbe essere vuota.
