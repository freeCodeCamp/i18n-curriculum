...
`handleShapeSelect` 함수 안에서 `.preventDefault()`에 `e` 메서드를 호출하고, 입력 필드를 지우기 위해 `clearInputFields()` 함수를 호출하세요.

# --hints--

`.preventDefault()`에 `e`를 호출해야 합니다.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

`clearInputFields` 함수를 호출해야 합니다.
