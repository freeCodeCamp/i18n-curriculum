...
이전 수업에서 `map()` 메서드를 사용해 옵션 목록을 렌더링하는 방법을 배웠습니다:
...
`key`는 항상 고유해야 한다는 점을 기억하세요. 이것은 React가 어떤 항목이 변경되었고, 추가되거나 제거되었는지 식별하는 데 도움을 줍니다.

첫 번째 `option` 다음에는 화살표 함수를 사용해 `powerSourceOptions` 배열을 `source` 매개변수로 매핑하세요.

그다음 `map()` 메서드 안에서 `option` 요소를 만들고 `key`를 `source`로, `value`를 `source`으로 설정하세요. 마지막으로 `{source}`을 사용해 드롭다운에 각 전원 공급 장치의 이름을 표시하세요.

# --hints--

`powerSourceOptions`을 매핑해야 합니다.

```js
assert.match(code, /\{\s*powerSourceOptions\s*\.\s*map\s*\(/)
```

map의 매개변수로 `source`을 전달해야 합니다.

```js
assert.match(code, /\{\s*powerSourceOptions\s*\.\s*map\s*\(\s*(source|\(\s*source\s*\))\s*=>/)
```

`option`를 `key`로, `source`를 `value`로 하는 `source` 요소를 만들어야 합니다.
...
마지막 여섯 개 `option` 요소 각각의 텍스트 내용은 `powerSourceOptions` 배열에 있는 전원 공급 옵션 중 하나와 일치해야 합니다.
