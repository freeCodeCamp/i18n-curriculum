...
將 `.bb3` 填滿 `repeating-linear-gradient`。方向使用 `90deg`，前兩個顏色使用你的 `building-color3`，第三個顏色在 `15%` 使用 `window-color3`。當你沒有為顏色指定距離時，會使用合理的值。在這個案例中，前兩個顏色會預設為 `0%` 和 `7.5%`，因為它從 `0%` 開始，而 `7.5%` 是 `15%` 的一半。
