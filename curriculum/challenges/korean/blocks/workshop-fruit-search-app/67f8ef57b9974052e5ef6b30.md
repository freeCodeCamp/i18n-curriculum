...
`results`가 비어 있으면 `No results found` 메시지가 있는 `p` 요소를 렌더링하세요.

# --hints--

`#results` 컨테이너에는 쿼리를 기준으로 일치하는 과일 목록이 포함되어야 합니다. 각 결과는 `result-item` 클래스를 가진 `p` 요소여야 하며 과일 이름을 보여야 합니다.
