...
Erstellen Sie eine `if` Statement, um zu überprüfen, ob das `allCategories` Objekt eine `id` Eigenschaft hat. Denken Sie daran, dass Sie die `hasOwnProperty()` Methode dafür verwenden können.

# --hints--

Sie sollten ein `if` Statement haben, das prüft, ob das `allCategories` Objekt eine `id` Eigenschaft hat
