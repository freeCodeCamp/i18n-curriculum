...
노래가 일시 중지될 시점이므로 `classList`에서 `remove()` 클래스를 `playing`와 `playButton` 메서드를 사용해 제거하세요.

마지막으로 노래를 일시 중지하려면 `pause()` 변수에 `audio` 메서드를 사용하세요. `pause()`는 음악 파일을 일시 중지하는 Web Audio API의 메서드입니다.

# --hints--

`pauseSong` 함수는 `playing`에서 `playButton` 클래스를 제거해야 합니다.
...
`pauseSong` 함수는 `pause()`에서 `audio` 메서드를 호출해야 합니다.
