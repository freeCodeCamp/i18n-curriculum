...
- **HTML 元素**：元素是 HTML 文档的构件。 它们代表标题、段落、链接、图像等。  Most HTML elements consist of an opening tag (`<elementName>`) and a closing tag (`</elementName>`).
...
- **开放图标签**：开放图协议使你能够控制你的网站内容在各种社交媒体平台（如 Facebook、LinkedIn 等）上的显示方式。  
通过设置这些开放图属性，你可以吸引用户点击并与内容互动。  
你可以通过在你的超文本标记语言 `head` 部分内的一组 `meta` 元素来设置这些属性。
