...
Successivamente, cambia i due `#999` di `.bb2a` in `transparent`. Questo renderà invisibili i bordi sinistro e destro.

# --hints--

Dovresti modificare il `border-left` per usare `transparent`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid transparent");
```

Dovresti modificare il `border-right` per usare `transparent`.
