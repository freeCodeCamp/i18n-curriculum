...
Seleziona tutti gli elementi `label` all'interno di elementi `.info` e imposta la loro proprietà `width` sul `10%` e fallo in modo che non occupino meno di `55px`.
...
Dovresti dare agli elementi `label` una proprietà `width` del `10%`.
...
Dovresti dare agli elementi `label` una proprietà `min-width` di `55px`.
