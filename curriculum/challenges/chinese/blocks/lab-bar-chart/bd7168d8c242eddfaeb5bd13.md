...
在本项目中，你将使用 `https://cdn.freecodecamp.org/curriculum/labs/data/bar-chart/GDP-data.json` 中的数据来构建一个柱状图。

测试要求使用 D3 的 axis 属性生成坐标轴，该属性会自动在坐标轴上生成刻度线。这些刻度线对于通过 D3 测试是必须的，因为它们的位置用于确定图形元素的对齐。
...
1. 你的图表应有一个 `g` 元素的 x 轴，并带有对应的 `id="x-axis"`。
1. 你的图表应有一个 `g` 元素的 y 轴，并带有对应的 `id="y-axis"`。
1. 两个坐标轴都应包含多个刻度标签，每个标签都有对应的 `class="tick"`。
1. 你的图表应为每个数据点有一个 `rect` 元素，并带有显示数据的对应 `class="bar"`。
1. 每个 `.bar` 应具有属性 `data-date` 和 `data-gdp`，其中包含 `date` 和 `GDP` 的值。
1. `.bar` 元素的 `data-date` 属性应与提供的数据顺序匹配。
1. `.bar` 元素的 `data-gdp` 属性应与提供的数据顺序匹配。
1. 每个 `.bar` 元素的高度应准确表现数据对应的 `GDP`。
1. `data-date` 属性及其对应的 `.bar` 元素应与 x 轴上的对应值对齐。
1. `data-gdp` 属性及其对应的 `.bar` 元素应与 y 轴上的对应值对齐。
1. 你应能将鼠标悬停在某个区域，并看到带有对应 `id="tooltip"` 的工具提示，显示该区域的更多信息。
1. 你的工具提示应有一个 `data-date` 属性，对应于活动区域的 `data-date`。
...
柱状条元素的 `data-date` 属性应与提供的数据顺序匹配。
...
`data-date` 属性及其对应的柱状条元素应与 x 轴上的对应值对齐。你可以使用 `d3.scaleTime()` 来为 x 轴创建时间比例尺。
...
`data-gdp` 属性及其对应的柱状条元素应与 y 轴上的对应值对齐。你可以使用 `d3.scaleLinear()` 来为 y 轴创建线性比例尺。
...
当鼠标悬停在某个区域时，你的柱状图应显示带有对应 `id="tooltip"` 的工具提示，显示该区域的更多信息。
...
工具提示应有一个 `data-date` 属性，对应于活动区域的 `data-date`。
