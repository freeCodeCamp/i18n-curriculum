...
En éste caso, ya lo hemos añadido para ti "detrás de escena".

El elemento `i` se utilizó originalmente para transformar en cursiva otros elementos, pero ahora es usado comúnmente para los íconos. Puedes añadir las clases de Font Awesome al elemento `i` para transformarlo en un ícono, por ejemplo:
...
Usa Font Awesome para añadir un ícono `thumbs-up` a tu botón "Like", dándole un elemento `i` con las clases `fas` y `fa-thumbs-up`. Asegúrate de dejar el texto `Like` junto al icono.

# --hints--

Deberías añadir un elemento `i` con las clases `fas` y `fa-thumbs-up`.
...
Tu ícono `fa-thumbs-up` debería estar ubicado dentro del botón "Like".
...
Tu elemento `i` debería estar anidado dentro de tu elemento `button`.
...
Tu ícono debería tener una etiqueta de cierre.
