...
Aufgrund der Art und Weise, wie Computer Zahlen speichern und mit ihnen rechnen, können Berechnungen mit Dezimalzahlen zu seltsamen Ergebnissen führen. Zum Beispiel ist `0.1 + 0.2` nicht gleich `0.3`. Das liegt daran, dass Computer Dezimalzahlen als binäre Brüche speichern, und einige binäre Brüche können nicht exakt als Dezimalbrüche dargestellt werden.

Wir möchten das Ergebnis Ihrer Berechnung bereinigen. Fassen Sie Ihre Berechnung in runde Klammern (ohne die `return`-Anweisung!) und rufen Sie die `.toFixed()`-Methode darauf auf. Übergeben Sie der `.toFixed()`-Methode die Zahl `2` als Argument. Dadurch wird die Zahl auf zwei Dezimalstellen gerundet und als String zurückgegeben.

# --hints--

Sie sollten Ihre Berechnung in runde Klammern fassen.
...
Sie sollten die `.toFixed()`-Methode auf Ihre Berechnung aufrufen.
...
Sie sollten der `.toFixed()`-Methode die Zahl `2` als Argument übergeben.
