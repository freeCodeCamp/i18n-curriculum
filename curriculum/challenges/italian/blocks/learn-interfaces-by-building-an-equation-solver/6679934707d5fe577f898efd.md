...
Aggiungi un altro caso per quando la lunghezza di `results` è `2`. Questa volta, assegna a `result_list` una lista contenente due stringhe con il formato `x1 = <root1>` e `x2 = <root2>`. Ancora una volta, fai in modo che la soluzione mostri sia i segni positivi sia quelli negativi.
...
Non dovresti modificare i blocchi `case` esistenti.
...
Dovresti creare un nuovo `case` con il pattern `2` dopo il blocco `case` esistente.
...
Dovresti assegnare una lista contenente due stringhe con il formato `x1 = <root1>` e `x2 = <root2>` a `result_list` all’interno del corpo del nuovo `case`. Mostra sia i segni positivi sia quelli negativi per i risultati.
