...
Створіть функцію з назвою `setPlayButtonAccessibleText`. Усередині функції встановіть атрибут `aria-label` кнопки відтворення в `Play`, після чого додайте пробіл і назву поточної пісні. Якщо `userData.currentSong` дорівнює `null`, встановіть його в `Play`.

# --hints--

У вас має бути функція з назвою `setPlayButtonAccessibleText`.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

Ваша функція `setPlayButtonAccessibleText` має встановлювати атрибут `aria-label` кнопки відтворення в `Play`, після чого додати пробіл і назву поточної пісні.
...
Коли `userData.currentSong` дорівнює `null`, `setPlayButtonAccessibleText` має встановлювати атрибут `aria-label` кнопки відтворення в `Play`.
