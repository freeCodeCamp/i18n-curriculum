...
`if` メソッドの `__str__` 文の後に、`movies` と `series` という名前の2つの変数を作成し、それぞれ `get_movies` メソッドの呼び出しと `get_tv_series` メソッドの呼び出しを代入してください。

# --hints--

`movies` メソッドに `__str__` という名前の変数があるはずです。
...
`self.get_movies()` 変数に `movies` を代入します。
...
`series` メソッドに `__str__` という名前の変数があるはずです。
...
`self.get_tv_series()` 変数に `series` を代入します。
