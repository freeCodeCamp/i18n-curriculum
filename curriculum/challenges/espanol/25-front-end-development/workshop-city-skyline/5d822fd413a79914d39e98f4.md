...
Los gradientes pueden usar tantos colores como desees, de esta manera:
...
Agrega un `linear-gradient` a `.bb1d` con `naranja` como primer color, `--building-color1` como segundo, y `--window-color1` como tercero. Recuerda usar el gradiente en la propiedad `background`.

# --hints--

Debes usar el `background` en `.bb1d`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background);
```

Debes dar a la propiedad `background` un `linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'linear-gradient');
```

Debes usar `naranja` como el primer color en el `linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.getPropVal('background', true), 'linear-gradient(orange');
```

Debes usar `--building-color1` como el segundo color en el `linear-gradient`.
...
Debes usar `--window-color1` como el tercer color en el `linear-gradient`.
