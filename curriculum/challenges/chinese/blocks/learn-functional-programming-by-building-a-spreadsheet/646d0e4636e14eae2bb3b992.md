...
现在更新 `addCharacters` 链中最内层的函数，以隐式返回使用 `character1` 和 `character2` 作为参数调用 `charRange()` 的结果。
...
你应该将 `character1` 作为第一个参数传递给 `charRange()` 调用。
...
你应该将 `character2` 作为第二个参数传递给 `charRange()` 调用。
