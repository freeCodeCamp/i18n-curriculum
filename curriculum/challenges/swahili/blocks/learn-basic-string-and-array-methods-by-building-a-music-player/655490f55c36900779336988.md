...
Rudia kupitia `playlistSongElements` kwa kutumia njia ya `forEach`. 

Njia ya `forEach` hutumika kurudia kupitia safu ya data na kutekeleza kitendakazi kwa kila kipengele cha safu ya data. Kwa mfano, fikiria una safu ya nambari na unataka kuandika kila nambari kwenye konsoli.
...
Tumia njia ya `forEach` kwenye `playlistSongElements`. Pitia `songEl` kama kigezo na tumia sintaksia ya mshale kuongeza wito wa kurudisha usio na kitu.

# --hints--

Unapaswa kuunganisha `forEach()` na `playlistSongElements`.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

Njia yako ya `forEach()` inapaswa kuwa na kigezo cha `songEl` na tumia sintaksia ya mshale.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

`forEach` yako inapaswa kuwa na mabano ya mviringo yasiyo na kitu.
