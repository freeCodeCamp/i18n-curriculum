...
最后，在你的 `p` 元素之后，创建一个 `button` 元素。 给它一个与变量 `id` 的值相同的 `id`，一个 `class` 为 `btn add-to-cart-btn`，并使用 `"Add to cart"` 作为文本。
...
你的 `button` 元素应该有一个 `id` 设置为 `id` 变量的值。

```js
assert.equal(document.querySelector('.dessert-card button')?.id, '1');
```

你的 `button` 元素应该有一个 `class` 为 `btn add-to-cart-btn`。
