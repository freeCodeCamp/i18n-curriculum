...
使用類選擇器，給你的 `.cat-nose` 元素左右各設置一個 `15px solid transparent` 的邊框。 還要給它一個底部邊框，樣式爲 `20px solid #442c2c`。
