...
對於 `.fb2a`，為其添加 `border-bottom` 為 `10vh solid var(--building-color3)`，以及 `border-left` 和 `border-right` 為 `1vw solid transparent`。這次的邊框技巧將創建一個梯形形狀。

# --hints--

你應該給 `.fb2a` 一個 `border-bottom`，值為 `10vh solid var(--building-color3)`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderBottom.trim(), "10vh solid var(--building-color3)");
```

你應該為 `.fb2a` 設定一個 `border-left`，值為 `1vw solid transparent`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderLeft, "1vw solid transparent");
```

你應該給 `.fb2a` 一個 `border-right`，設定為 `1vw solid transparent`。
