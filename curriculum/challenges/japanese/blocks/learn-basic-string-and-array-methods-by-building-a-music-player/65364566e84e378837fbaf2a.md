...
前のステップでは曲を再生する機能を作成しました。次は、再生ボタンに現在の曲をクリックしたときに再生する機能を追加する必要があります。

`addEventListener()` メソッドを使い、最初の引数に `"click"` イベントを渡し、2番目の引数にはアロー構文で空のコールバック関数を渡してください。例：`() => {}`。

# --hints--

`addEventListener()` 変数に対して `playButton` メソッドを呼び出す必要があります。

```js
assert.match(code, /playButton\.addEventListener\(/)
```

イベントリスナーは `"click"` イベントを監視するようにしてください。

```js
assert.match(code, /playButton\.addEventListener\(\s*('|"|`)click\1/)
```

イベントリスナーに空のコールバックを渡すにはアロー構文を使いましょう。空の波括弧も忘れずに用意してください。
