...
曲の現在の再生時間を扱う`else`ブロックを追加してください。これにより、一時停止した地点から現在の曲を再開できます。

`else`ブロック内で、`currentTime`オブジェクトの`audio`プロパティを`userData.songCurrentTime`に保存されている値に設定してください。

# --hints--

`playSong`関数は、現在曲が再生中の場合に`currentTime`オブジェクトの`audio`プロパティを`userData.songCurrentTime`に保存されている値に設定する必要があります。
