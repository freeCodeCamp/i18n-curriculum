...
Attualmente la tua funzione `range` restituisce un array con la lunghezza corretta, ma tutti i valori sono il valore di `start`. Per risolvere questo, concatena il metodo `.map()` al tuo metodo `.fill()`.

Passa al metodo `.map()` una callback che prende `element` e `index` come parametri e restituisce la somma di questi parametri.
...
La tua callback `.map()` dovrebbe usare un return implicito.
...
La tua callback `.map()` dovrebbe restituire implicitamente la somma di `element` e `index`.
