...
Es kann bei der Entwicklung von Nutzen sein, den Standardrand um die `fieldset`-Elemente zu sehen. Aber vielleicht ist es nicht der Stil, den du dir vorgestellt hast.

Entferne den Rand und die untere Padding-Einheit der `.question`-Elemente.

# --hints--

Du solltest den `.question`-Selektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

Du solltest `.question` eine `border` von `none` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

Du solltest `.question` ein `padding-bottom` von `0` geben.
