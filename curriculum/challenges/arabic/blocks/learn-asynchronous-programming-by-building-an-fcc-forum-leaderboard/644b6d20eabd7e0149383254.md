...
داخل دالة `forumCategory`، أنشئ متغيرًا جديدًا باستخدام `let` باسم `selectedCategory` وقم بتعيين كائن فارغ له. سيُستخدم هذا لتخزين اسم الفئة واسم الصنف لكل فئة.

# --hints--

يجب استخدام `let` لإعلان متغير باسم `selectedCategory`.

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

يجب تعيين كائن فارغ إلى المتغير `selectedCategory`.
