...
Declare uma variável `rangeExpanded` e atribua a ela o resultado da chamada do método `.replace()` do parâmetro `x`.Passe a variável `rangeRegex` como o argumento.

# --hints--

Você deve declarar uma variável `rangeExpanded`.
...
Você deve passar `rangeRegex` como o primeiro argumento para `.replace()`.
