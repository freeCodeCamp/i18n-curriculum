...
Ora aggiungi un `div` con il `className` di `form-group`. Al suo interno, aggiungi un `label` con il `className` di `label` e un attributo `htmlFor` di `name`. Sotto di esso, aggiungi un `input` con un `id` impostato su `name`, un `className` di `input` e un attributo `type` di `text`.
