...
このプロジェクトの最後の2つのセマンティックHTML要素は`footer`要素と`address`要素です。`footer`要素はページに関連するコンテンツの集合を格納するコンテナで、`address`要素はページの作成者の連絡先情報を格納するコンテナです。

`main`要素の後に`footer`要素を1つ追加し、その中に`address`要素を1つネストしてください。

# --hints--

`footer`要素の後に`main`要素を1つ追加してください。

```js
assert.exists(document.querySelector('main + footer'));
```

`address`要素の中に`footer`要素を1つネストしてください。
