...
Fügen Sie ein `value`-Attribut von `player.overallRating` zum Gesamt-`input` hinzu. Danach fügen Sie ein `onChange`-Attribut hinzu, das auf eine Pfeilfunktion gesetzt ist, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `overallRating` auf `Number(e.target.value)` gesetzt ist.

Da `overallRating` eine Zahl ist, verwenden Sie `Number()`, um den String-Wert aus der Eingabe zu konvertieren.

# --hints--

Ihr Gesamt-`input` sollte ein `value` haben, das dem aktuellen Gesamtwert des Spielers entspricht.
...
Ihr Gesamt-`input` sollte ein `onChange`-Attribut haben.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Der `onChange` Ihres Gesamt-`input` sollte `setPlayer` mit einer Ausbreitung von `player` aufrufen, wobei `overallRating` auf `Number(e.target.value)` gesetzt ist.
