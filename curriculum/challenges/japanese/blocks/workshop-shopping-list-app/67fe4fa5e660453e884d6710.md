...
`toggleItem` という名前の関数を作成し、`item` を引数として受け取ってください。

関数の中では、今はとりあえず `setSelectedItems()` 関数を呼び出してください。選択された項目を切り替えるロジックは後で実装します。

# --hints--

`toggleItem` を引数に取る `item` という名前の関数があるはずです。
...
`setSelectedItems()` 関数の中で `toggleItem()` 関数を呼び出す必要があります。
