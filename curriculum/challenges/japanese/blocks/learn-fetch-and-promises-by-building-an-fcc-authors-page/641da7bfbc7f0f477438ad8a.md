...
では `displayAuthors` 関数を呼び出します。 繰り返しになりますが、すべての著者データを一度に追加したくはありません。 代わりに、変数 `startingIndex` と `endingIndex` を使って著者データの一部を抽出しましょう。 これに最適なのが配列メソッドの `.slice()` です。

まず、`authorDataArr` を表示しているコンソールログ文を削除してください。 そして `authorDataArr` 配列と `.slice()` を使って、`displayAuthors` 関数を呼び出してください。 `startingIndex` 変数を開始位置、`endingIndex` 変数を終了位置として使用してください。
