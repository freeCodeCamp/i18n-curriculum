...
在 `is_valid` 方法里，调用 `valid_in_row()` 方法（以 `row` 和 `num` 作为参数）来检查数字是否可以插入到指定行，并将结果赋值给变量 `valid_in_row` 。
 记住使用 `self` 来引用当前实例的方法。

# --hints--

你应该声明一个变量 `valid_in_row` 并给它赋值 `self.valid_in_row(row, num)` 。
