...
依照相同的樣式，將你的 `#add-entry` 元素指定給 `addEntryButton`，將你的 `#clear` 元素指定給 `clearButton`，以及將你的 `#output` 元素指定給 `output`。

# --hints--

你應該宣告一個名為 `addEntryButton` 的變數。

```js
assert.isDefined(addEntryButton);
```

你應該使用 `document.getElementById()` 來取得 `#add-entry` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")add-entry\1\s*\)/g);
```

你應該將 `#add-entry` 元素指定給 `addEntryButton`。

```js
assert.deepEqual(addEntryButton, document.getElementById('add-entry'));
```

你應該宣告一個名為 `clearButton` 的變數。

```js
assert.isDefined(clearButton);
```

你應該使用 `document.getElementById()` 來取得 `#clear` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")clear\1\s*\)/g);
```

你應該將 `#clear` 元素指定給 `clearButton`。

```js
assert.deepEqual(clearButton, document.getElementById('clear'));
```

你應該宣告一個名為 `output` 的變數。
...
你應該使用 `document.getElementById()` 來取得 `#output` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")output\1\s*\)/g);
```

你應該將 `#output` 元素指定給 `output`。
