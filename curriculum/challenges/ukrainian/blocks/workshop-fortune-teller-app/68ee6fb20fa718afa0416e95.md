...
Як і для попереднього `multipleCard`, знову отримайте елемент `multipleCard` і призначте його `innerHTML`, перебираючи `DrawingType`. Використайте `Object.values()` і викличте на ньому метод `map`, передаючи `type` як аргумент.

# --hints--

Ви повинні призначити `multipleCard.innerHTML`, викликавши `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

Ви повинні передати `DrawingType` методу `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

Ви повинні викликати метод `map` на `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Ваш метод `map` має мати параметр з іменем `type`.
