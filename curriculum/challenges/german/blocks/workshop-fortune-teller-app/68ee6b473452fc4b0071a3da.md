...
Machen Sie nun dasselbe für `multipleCardsBtn` und `newReadingBtn`, indem Sie einen Klick-Ereignis-Listener an `multipleCardsBtn` anhängen, der `multipleCardSelected` aufruft, und einen weiteren an `newReadingBtn`, der `newReading` aufruft.

Sie werden die beiden Methoden in den kommenden Schritten erstellen.

# --hints--

Sie sollten ein `click`-Ereignis an das `multipleCardsBtn`-Element anhängen.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Innerhalb des `multipleCardsBtn`-Callback-Ereignisses sollten Sie die `multipleCardSelected`-Methode aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

Sie sollten ein `click`-Ereignis an das `newReadingBtn`-Element anhängen.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Innerhalb des `newReadingBtn`-Callback-Ereignisses sollten Sie die `newReading`-Methode aufrufen.
