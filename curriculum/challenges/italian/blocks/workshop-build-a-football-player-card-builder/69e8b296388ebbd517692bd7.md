...
Ora inizierai il processo per trasformare l'input posizione in un select.

Cambia l'input in un elemento `select` con id `position`, un `className` di `input` e un `value` di `player.position`.

# --hints--

Dovresti cambiare il `input` posizione in un elemento `select` con un `id` di `position` e `className` di `input`.
...
Il tuo `select` posizione dovrebbe avere un `value` di `player.position`.
