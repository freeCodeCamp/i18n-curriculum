...
Тепер вам потрібно додати окремі роздільники під елементами `.no-divider`.

Ваш перший елемент `.no-divider` має `.divider` після нього. Створіть ще один `.divider` після другого елемента `.no-divider`.
