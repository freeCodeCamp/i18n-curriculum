...
Du solltest `text.innerText` auf `"You already have the most powerful weapon!"` setzen.

```js
assert.match(buyWeapon.toString(), /('|")You already have the most powerful weapon!\1/);
```

Du solltest deinen `text.innerText` zu `"You already have the most powerful weapon!"` innerhalb deiner äußeren `else`-Anweisung ändern.
