...
クラスセレクターを使って、`.cat-nose` 要素に左右それぞれ `15px solid transparent` のボーダーを付けてください。また、下側には `20px solid #442c2c` のボーダーを付けてください。

# --hints--

`.cat-nose` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-nose'));
```

`.cat-nose` セレクターには `border-left` プロパティを `15px solid transparent` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderLeft, '15px solid transparent');
```

`.cat-nose` セレクターには `border-right` プロパティを `15px solid transparent` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderRight, '15px solid transparent');
```

`.cat-nose` セレクターには `border-bottom` プロパティを `20px solid #442c2c` に設定してください。
