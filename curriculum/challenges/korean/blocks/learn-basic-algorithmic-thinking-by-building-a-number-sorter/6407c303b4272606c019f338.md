...
정렬된 숫자로 화면을 업데이트하는 함수를 만들어야 합니다. 화살표 구문을 사용해 단일 `updateUI` 매개변수를 받는 `array` 함수를 선언하는 것부터 시작하세요.

즉시 반환값이 없는 알고리즘을 작성할 예정이므로 `array`의 대체 수단을 빈 배열로 설정하세요. 대체 수단이 있는 함수 예시는 다음과 같습니다.
...
`const`를 사용해 `updateUI` 변수를 선언하세요.
...
`updateUI`는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+updateUI\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`updateUI`는 단일 `array` 매개변수를 받아야 합니다.

```js
assert.match(code, /const\s+updateUI\s*=\s*\(?\s*array/);
```

`updateUI`는 `array`에 대해 빈 배열을 대체 수단으로 가져야 합니다.
