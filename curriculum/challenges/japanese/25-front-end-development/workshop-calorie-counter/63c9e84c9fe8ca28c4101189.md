...
では、`budgetCalories Calories Budgeted` というテキストを持つ `p` 要素を作成し、補間を使用して、`budgetCalories` の部分を適切な変数に置き換えてください。
...
`p` 要素は `${budgetCalories} Calories Budgeted` というテキストを持つようにしてください。
