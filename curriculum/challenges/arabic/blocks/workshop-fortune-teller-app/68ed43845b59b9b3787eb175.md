...
أضف `constructor` داخل الصنف. في دالة البناء، تحتاج إلى استخدام العناصر التي أنشأتها سابقًا لاختيار عناصر DOM المقابلة باستخدام الدالة `getElement`.

في الوقت الحالي، تعامل مع `singleCardBtn` و `singleCard`. بالنسبة لـ `singleCardBtn`، مرر `#btn-single-card`، وبالنسبة لـ `singleCard`، مرر `.single_card`.

# --hints--

يجب أن يحتوي صنف `Game` على `constructor`.
...
يجب أن تُعيّن دالة البناء الخاص بك `this.elements`.
...
يجب تعيين `singleCardBtn` باستخدام `getElement` مع `'#btn-single-card'`.
...
يجب تعيين `singleCard` باستخدام `getElement` مع `'.single_card'`.
