...
Використовуючи той самий синтаксис інтерполяції, додайте другий елемент `p` із текстом `consumedCalories Calories Consumed` та третій із текстом `exerciseCalories Calories Burned`. Не забудьте замінити змінні `consumedCalories` та `exerciseCalories` на відповідні значення.
