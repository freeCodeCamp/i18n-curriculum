...
為每個列表項目內的 `span` 元素指定類別 `text-green-500` 和 `mr-2`，使標記變成綠色並將它們稍微推離文字。

此外，也像你對第一個定價方案的 `span` 元素所做的那樣，為它們指定 `aria-hidden` 屬性。

# --hints--

你的 `span` 元素應該有 `text-green-500` 類別。
