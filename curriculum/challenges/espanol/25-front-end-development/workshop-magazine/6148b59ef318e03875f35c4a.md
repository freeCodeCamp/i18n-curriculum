...
Crea un selector de `img` y dale una propiedad `width` establecida en `100%`, y una propiedad `object-fit` establecida en `cover`.

La propiedad `object-fit` le indica al navegador cómo posicionar el elemento dentro de su contenedor. En este caso, `cover` ajustará la imagen para llenar el contenedor, recortando lo necesario para evitar cambiar la proporción.
...
Tu selector `img` debe tener una propiedad `object-fit` establecida en `cover`.
