...
Далі оновіть об’єкт вхідних даних `throughputSummary`, щоб відобразити, скільки відвідувачів було успішно оброблено цими воротами на поточній поділці. Зокрема, візьміть значення `processed` у `result` і додайте його до елемента в `throughputSummary` за індексом `gate.id`.

Цей крок гарантує, що підсумок пропускної здатності накопичує відвідувачів за кілька поділок.

# --hints--

Ви повинні додати `result.processed` до `throughputSummary[gate.id]`.
...
Ваша функція `handleGateAtTick` має накопичувати оброблених відвідувачів за кілька поділок.
...
Ваша функція `handleGateAtTick` має оновлювати пропускну здатність лише для конкретних воріт, які обробляються.
