...
الآن، عيّن للصنف `span` داخل عنصر `p` الأصناف `text-base` و`font-medium` و`text-gray-500`.

# --hints--

يجب أن يحتوي `span` داخل عنصر `p` على الصنف `text-base`.
...
يجب أن يحتوي `span` داخل عنصر `p` على الصنف `font-medium`.
...
يجب أن يحتوي `span` داخل عنصر `p` على الصنف `text-gray-500`.
