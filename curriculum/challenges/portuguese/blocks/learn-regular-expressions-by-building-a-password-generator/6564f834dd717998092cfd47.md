...
Após seu novo comentário, escreva um loop `for` para iterar sobre a lista `constraints`.Use `constraint` e `pattern` como as variáveis do loop.

# --hints--

Você deve escrever um loop `for` para iterar sobre a lista `constraints`.
...
Seu loop `for` deve usar `constraint` e `pattern` como as variáveis do loop para iterar sobre a lista `constraints`.
