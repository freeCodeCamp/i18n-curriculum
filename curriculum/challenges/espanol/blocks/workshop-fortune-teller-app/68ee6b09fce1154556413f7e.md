...
Dentro del método `initializeEventListeners`, adjunta un manejador de evento `click` al `singleCardBtn`. Dentro del manejador del evento, llama al método `singleCardSelected`. Recuerda obtener el `singleCardBtn` de `elements`.
