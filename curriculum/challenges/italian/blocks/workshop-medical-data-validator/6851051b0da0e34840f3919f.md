...
`age` non dovrebbe essere solo un intero, dovrebbe essere un intero positivo maggiore o uguale a `18`.

Usando l'operatore `and`, aggiungi una seconda espressione al valore della chiave `age` per verificarlo.

# --hints--

La chiave `age` del tuo dizionario `constraints` dovrebbe avere il valore `isinstance(age, int) and age >= 18`.
