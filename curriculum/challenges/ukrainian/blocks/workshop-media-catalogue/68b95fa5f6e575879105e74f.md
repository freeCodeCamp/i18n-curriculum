...
Тепер, коли клас `MediaError` завершено, змініть інструкцію `if` у методі `add`. Замініть `TypeError` на `MediaError` з тим самим повідомленням.

Також передайте `media_item` як другий аргумент після повідомлення. Це дозволить зберегти елемент, що спричинив помилку, в об’єкті виключення.

# --hints--

Ви повинні викликати `MediaError` з повідомленням `Only Movie or TVSeries instances can be added`, коли `media_item` не є екземпляром `Movie` або його підкласів.
...
Ви повинні передати `media_item` як другий аргумент до `MediaError`.
...
Ви не повинні викликати `MediaError`, коли `media_item` є дійсним елементом.
...
Ваш метод `add` не повинен викликати `TypeError`.
