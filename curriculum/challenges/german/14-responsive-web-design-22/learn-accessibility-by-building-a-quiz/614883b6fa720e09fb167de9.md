...
Füge die Padding-Einheiten oben und links von den `.info`-Elementen hinzu und setze die anderen Werte auf `0`.

# --hints--

Du solltest den `.info` Selektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.info'));
```

Du solltest `.info` ein `padding-top` von mindestens `1px` geben.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('.info')?.paddingTop?.replace(/\D+/, '')), 1);
```

Du solltest `.info` ein `padding-right` von `0` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingRight, '0px');
```

Du solltest `.info` ein `padding-bottom` von `0` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingBottom, '0px');
```

Du solltest `.info` ein `padding-left` von mindestens `1px` geben.
