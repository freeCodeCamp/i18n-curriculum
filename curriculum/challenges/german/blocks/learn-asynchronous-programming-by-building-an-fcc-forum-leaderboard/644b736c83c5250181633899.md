...
Erstellen Sie eine `if`-Anweisung, um zu überprüfen, ob das `allCategories`-Objekt eine Eigenschaft von `id` besitzt. Denken Sie daran, dass Sie dafür die `hasOwnProperty()`-Methode verwenden können.

# --hints--

Sie sollten eine `if`-Anweisung haben, die überprüft, ob das `allCategories`-Objekt eine Eigenschaft von `id` besitzt
