...
Знову потрібно зробити карту випадковою та перевернутою. Оголосіть змінні з іменами `isReversed` та `card`. Для першої викличте метод `Math.random` і порівняйте його з `< 0.5`. Для другої викличте `getRandomItem` і передайте `this.cards`.

# --hints--

Ви повинні оголосити змінну з іменем `isReversed`.
...
Ваша змінна `isReversed` має використовувати `Math.random()` для випадкового визначення, чи карта перевернута.
...
Ви повинні оголосити змінну з іменем `card`.
...
Вашій змінній `card` має бути призначено значення в результаті виклику методу `getRandomItem` з аргументом `this.cards`.
