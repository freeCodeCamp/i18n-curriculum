...
Надсилання форми з вибраною опцією не відправлятиме дійсне значення на сервер. Тому всім елементам `option` потрібно надати атрибут `value`. Без нього текстовий вміст елементів `option` буде надіслано на сервер.

Першому `option` надайте `value` зі значенням `""`, а наступним елементам `option` надайте атрибути `value` від `1` до `4`.

# --hints--

Надайте першому елементу `option` атрибут `value` зі значенням `""`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

Надайте другому елементу `option` атрибут `value` зі значенням `1`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

Надайте третьому елементу `option` атрибут `value` зі значенням `2`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

Надайте четвертому елементу `option` атрибут `value` зі значенням `3`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

Надайте п’ятому елементу `option` атрибут `value` зі значенням `4`.
