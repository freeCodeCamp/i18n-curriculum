...
Додайте обробник `onerror` до елемента зображення, щоб обробляти помилки завантаження. При першій помилці позначте це за допомогою `dataset.failed` і замініть джерело на локальне зображення за замовчуванням. Якщо зображення знову не завантажується, видаліть клас hidden і сховайте заповнювач завантаження за допомогою `previousElementSibling`.

# --hints--

Ваш елемент `img` має мати обробник `onerror`.
...
Ваш обробник `onerror` має містити інструкцію `if`, яка перевіряє, чи є `dataset.failed` хибним.
...
`dataset.failed` має бути встановлений у `'1'` всередині блоку `if`. Не забудьте ключове слово `this` тут.
...
Ваш блок `if` має оновити `this.src` на локальне зображення за замовчуванням. Не забудьте ключове слово `this` тут.
...
У вас має бути блок `else`, який видаляє клас `hidden`. Не забудьте про ключове слово `this` тут.
...
Ваш блок `else` має встановити `previousElementSibling.style.display` у `'none'`. Не забудьте ключове слово `this` тут.
