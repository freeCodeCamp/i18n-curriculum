...
Golomb 的自描述序列（$G(n)$）是唯一一個非遞減的自然數序列，使得 $n$ 在序列中恰好出現 $G(n)$ 次。前幾個 $n$ 的 $G(n)$ 值為
...
你被給予 $G({10}^3) = 86$、$G({10}^6) = 6137$。

你也被給予 $\sum G(n^3) = 153\\,506\\,976$，其中 $1 ≤ n < {10}^3$。

求 $\sum G(n^3)$，其中 $1 ≤ n < {10}^6$。

# --hints--

`golombsSequence()` 應該傳回 `56098610614277016`。
