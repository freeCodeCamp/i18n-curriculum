...
Las esquinas de cada rectángulo siguen siendo demasiado pronunciadas.

Redondea cada esquina del elemento `.one` por 9 píxeles, utilizando la propiedad `border-radius`.

# --hints--

Debes establecer la propiedad `border-radius` en `9px`.
...
El elemento `.one` debe tener un valor de `border-radius` de `9px`.
