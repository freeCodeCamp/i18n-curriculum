...
두 번째 `textarea` 요소 안에 `div.answer` 요소를 중첩하고, 행과 열의 수를 설정하세요.

# --hints--

두 번째 `textarea` 요소 안에 `div.answer` 요소를 중첩해야 합니다.

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

`textarea`에 숫자 값을 가진 `rows` 속성을 지정해야 합니다.
...
`textarea`에 숫자 값을 가진 `cols` 속성을 지정해야 합니다.
