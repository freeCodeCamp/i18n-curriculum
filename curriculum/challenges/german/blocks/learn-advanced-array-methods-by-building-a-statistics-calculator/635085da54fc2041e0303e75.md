...
Schließlich müssen Sie den Wert von `mean` zurückgeben.

# --hints--

Ihre `getMean`-Funktion sollte das Schlüsselwort `return` verwenden.
...
Ihre `getMean`-Funktion sollte den Wert von `mean` zurückgeben.
...
Ihre `return`-Anweisung sollte nach der Deklaration der `mean`-Variablen stehen.
