...
Ndani ya usemi wa `if`, pata kipengele cha `cardTitle` na weka `textContent` yake kuwa `foundCard.name`. Fanya vivyo hivyo kwa kipengele cha `description` kwa kuweka `textContent` yake kuwa `foundCard.desc`.

# --hints--

Unapaswa kuweka `cardTitle` `textContent` kuwa `foundCard.name`. Usisahau kutumia neno kuu la `this`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

Unapaswa kuweka `description` `textContent` kuwa `foundCard.desc`. Usisahau kutumia neno kuu la `this`.
