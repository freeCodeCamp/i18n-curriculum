...
Per iniziare il tuo CSS, normalizza le regole CSS selezionando tutti gli elementi con `*`, inclusi i pseudo-selettori `::before` e `::after`.

Imposta le proprietà `padding` e `margin` entrambe su `0` e imposta la proprietà `box-sizing` su `border-box`.
...
Il tuo selettore `*, ::before, ::after` dovrebbe avere una proprietà `padding` impostata su `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.padding, '0px');
```

Il tuo selettore `*, ::before, ::after` dovrebbe avere una proprietà `margin` impostata su `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.margin, '0px');
```

Il tuo selettore `*, ::before, ::after` dovrebbe avere una proprietà `box-sizing` impostata su `border-box`.
