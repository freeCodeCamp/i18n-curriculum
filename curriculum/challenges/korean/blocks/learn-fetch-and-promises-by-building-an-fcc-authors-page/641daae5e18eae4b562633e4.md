...
`.catch()` 내부에서 `console.error()`를 제거하고 `innerHTML`의 `authorContainer`를 `p` 요소로 설정하세요. 이 요소에는 `class` `"error-msg"`와 텍스트 `"There was an error loading the authors"`가 포함되어야 합니다.
...
`console.error`와 그 텍스트를 제거해야 합니다.

```js
assert.notMatch(code, /console\.error\(\s*`There\s+was\s+an\s+error:\s+\$\{err\}`\s*\)\s*;?/)
```

`innerHTML`의 `authorContainer`에 접근하여 `p` 요소로 설정하세요. `p` 요소를 템플릿 리터럴로 감싸는 것을 잊지 마세요.

```js
assert.match(code, /authorContainer\.innerHTML\s*=\s*(`|"|')\s*<p.*>.*<\/p>\s*\1/)
```

`p` 요소에 `"error-msg"` 클래스를 지정해야 합니다.

```js
assert.match(code, /(`|"|')\s*<p\s+class\s*=\s*("|')error-msg\2>/)
```

`p` 요소에 텍스트 `"There was an error loading the authors"`를 넣어야 합니다.
