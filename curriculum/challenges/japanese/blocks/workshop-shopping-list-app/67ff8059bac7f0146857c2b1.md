...
今アプリを使うと、アイテムをチェックまたはチェック解除したり、検索バーに入力したりするたびに `toggleItem()` 関数が再作成されているのがわかります。

パフォーマンスを改善するために、`useCallback()` フックを使って `toggleItem()` 関数をメモ化またはキャッシュできます。そうすると、依存関係が変わったときだけ関数が再作成されます。

まず、ファイルの先頭で React から `useCallback()` フックを分割代入してください。

# --hints--

ファイルの先頭で `useCallback` から `React` を分割代入する必要があります。すでに使っている他のフックを削除していないか確認してください。
