...
To style the submit button, you can use an _attribute_ selector, which selects an element based on the given attribute value. Aquí hay un ejemplo:
...
Debes utilizar un selector de atributos `input[type="submit"]` para dar estilo al botón de envío.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="submit"]'));
```

Debes dar al botón de envío un `display` de `block`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.display, 'block');
```

Debes dar al botón de envío un `width` de `60%`.
