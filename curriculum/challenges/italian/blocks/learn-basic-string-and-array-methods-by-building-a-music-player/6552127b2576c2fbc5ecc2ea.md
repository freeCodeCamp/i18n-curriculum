...
Questo garantisce che la canzone attualmente in riproduzione continui a suonare quando si clicca il pulsante play.
...
Dovresti aggiungere un blocco `else` alla tua istruzione `if`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{\s*playSong\(\s*userData\?\.songs\s*\[\s*0\s*\]\s*\.id\s*\)\s*;?\s*\}\s*else\s*\{/)
```

Dovresti chiamare la funzione `playSong` con `userData?.currentSong.id` nel blocco `else` della tua istruzione `if`.
