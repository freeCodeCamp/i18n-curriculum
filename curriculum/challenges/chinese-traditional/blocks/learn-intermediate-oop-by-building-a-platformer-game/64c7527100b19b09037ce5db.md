...
在 `draw` 方法內，將 `"#acd157"` 指定給 `ctx.fillStyle`。

在那之下，呼叫 `ctx.fillRect` 方法，並傳入 `x` 和 `y` 座標，以及 `width` 和 `height` 屬性。記得在每個屬性前加上 `this`。

# --hints--

你應該將 `"#acd157"` 指定給 `ctx.fillStyle`。

```js
assert.match(code, /ctx\.fillStyle\s*=\s*('|")#acd157\1\s*;?/);
```

你應該呼叫 `ctx.fillRect` 方法並傳入正確的引數。
