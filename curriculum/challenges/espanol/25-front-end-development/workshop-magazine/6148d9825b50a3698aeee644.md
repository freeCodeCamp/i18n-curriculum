...
Crea un selector `.lists` y establece la propiedad `list-style-type` en `none`. Esto eliminará los puntos de las viñetas en los elementos de la lista.

# --hints--

Debes tener un selector `.lists`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.lists'));
```

Tu selector `.lists` debe tener una propiedad `list-style-type` establecida en `none`.
