...
Використайте наступні властивості CSS, щоб елементи `h3` відтворювались з вищим пріоритетом:
...
Використайте селектор елемента `h3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3'));
```

Надайте елементу `h3` властивість `margin-top` зі значенням `5px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Надайте елементу `h3` властивість `padding-left` зі значенням `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Надайте елементу `h3` властивість `font-size` зі значенням `1.375ren`.
