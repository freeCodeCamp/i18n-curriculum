...
Cree un nuevo selector `.indent` y asígnele una propiedad `margin-left` configurada en `1em`.
...
Su selector `.indent` debería tener una propiedad `margin-left` configurada en `1em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.indent')?.marginLeft, '1em');
```

Su selector `.indent` solo debería establecer la propiedad `margin-left`, no otras propiedades de margen.
