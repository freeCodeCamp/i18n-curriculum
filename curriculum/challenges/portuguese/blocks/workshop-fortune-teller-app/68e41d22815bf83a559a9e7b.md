...
Agora, use o `onload` evento para remover a classe `hidden` quando a imagem carregar e esconda o marcador de posição de carregamento usando seu `previousElementSibling`.

# --hints--

Seu elemento `img` deve ter um manipulador `onload`.
...
Seu manipulador `onload` deve usar `classList.remove` para remover a classe `hidden`. Não esqueça que você precisa da palavra-chave `this` aqui.
...
Seu manipulador `onload` deve definir `previousElementSibling.style.display` para `'none'`. Não esqueça que você precisa da palavra-chave `this` para fazer isso.
