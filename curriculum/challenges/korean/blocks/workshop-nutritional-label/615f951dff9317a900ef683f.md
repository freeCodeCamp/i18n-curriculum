...
`.note` 선택자에 좌우 패딩을 `8px`로 주고, 상하 패딩은 제거하세요. 또한 `text-indent` 속성을 `-8px`으로 설정하세요.

이 마지막 변경으로 영양 레이블이 완성됩니다!

# --hints--

`.note` 선택자에 `padding` 속성을 `0 8px`으로 설정해야 합니다.
...
`.note` 선택자에 `text-indent` 속성을 `-8px`으로 설정해야 합니다.
