...
テーブルのボディ内の`td`要素をターゲットにするセレクターを作成してください。ビューポートいっぱいに幅を設定し、最小値と最大値を`4rem`にしてください。この方法で幅が固定されます。`width`を指定すると、要素がコンテナに合わせて縮む場合があります。

# --hints--

`tbody td`セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('tbody td'));
```

`tbody td`セレクターには、`width`プロパティを`100vw`に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('tbody td')?.getPropertyValue('width') === '100vw');
```

`tbody td`セレクターには、`min-width`プロパティを`4rem`に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('tbody td')?.getPropertyValue('min-width') === '4rem');
```

`tbody td`セレクターには、`max-width`プロパティを`4rem`に設定してください。
