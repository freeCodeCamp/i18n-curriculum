...
Sasa tengeneza mzunguko wa `for` unaorudia juu ya `self.strategies`. Kwa kila `strategy`, angalia kama inafaa kwa kutumia njia ya `is_applicable`. Ikiwa inafaa, hesabu bei iliyopunguzwa kwa kutumia `apply_discount` na uhifadhi katika kigezo kinachoitwa `discounted`. Kisha, ongeza bei iliyopunguzwa kwenye orodha ya `prices`.

# --hints--

Unapaswa kutengeneza mzunguko wa `for` unaorudia juu ya `self.strategies`.
...
Ndani ya mzunguko, tamko lako la `if` linapaswa kuwa `if strategy.is_applicable(product, user_tier):`.
...
Ikiwa mkakati unafaa, unapaswa kuita `apply_discount` na kuongeza matokeo kwenye `prices`.
