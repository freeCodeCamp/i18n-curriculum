...
Kama vile vitendakazi vya kawaida, vitendakazi vya mshale vinaweza kupokea vigezo vingi. 

Hapa kuna mfano wa kitendakazi cha mshale chenye jina na kigezo kimoja: 
...
Ikiwa kitendakazi kina kigezo kimoja tu, unaweza kuondoa mabano ya kawaida yanayozunguka orodha ya vigezo kama hivi:
...
Tengeneza kitendakazi kipya cha mshale chenye jina `printMessage` chenye kigezo kimoja kinachoitwa `org`. Ndani ya mwili wa kitendakazi hicho, ongeza tamko la konsoli. Ndani ya tamko hilo la konsoli, ongeza mfuatano wa herufi wa template literal `${org} is awesome!`. 

Chini ya kitendakazi chako cha `printMessage`, fanya wito wa kitendakazi hicho na pita mfuatano wa herufi `"freeCodeCamp"` kama hoja.

Fungua konsoli ili kuona matokeo. 

# --hints--

Unapaswa kuwa na kitendakazi cha `printMessage`.

```js
assert.isFunction(printMessage);
```

Unapaswa kutumia sintaksia ya kitendakazi cha mshale.

```js
assert.match(code, /(let|const)\s+printMessage\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Kitendakazi chako cha `printMessage` kinapaswa kuwa na kigezo kinachoitwa `org`.
...
Kitendakazi chako cha `printMessage` kinapaswa kuwa na tamko la konsoli.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

Tamko lako la konsoli linapaswa kujumuisha mfuatano wa herufi wa template literal `${org} is awesome!`.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

Unapaswa kuita kitendakazi cha `printMessage` na kupitisha mfuatano wa herufi `"freeCodeCamp"`.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

Wito wa kitendakazi chako unapaswa kuwa chini ya ufafanuzi wa kitendakazi chako cha `printMessage`.
...
Wito lako la kitendakazi cha `printMessage` linapaswa kutoa `freeCodeCamp is awesome!` kwenye konsoli. 
