...
Terminate the `while` loop by removing the current node from the `unvisited` list. 请注意缩进。

# --hints--

你应该在你的 `for` 循环后面调用 `.remove()` 方法删除 `unvisited` 的当前节点。
