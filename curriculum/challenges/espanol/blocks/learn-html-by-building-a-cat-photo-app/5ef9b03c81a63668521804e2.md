...
Tu elemento `input` del checkbox debe tener una etiqueta de apertura, pero no una etiquete de cierre.

```js
assert.isTrue(document.querySelector('fieldset > input') && !code.match(/<\/input\>/g));
```

Solo debiste de haber añadido un elemento input para tu checkbox. Elimine cualquier extra.

```js
assert.lengthOf(document.querySelectorAll('fieldset > input'), 1);
```

Tu nuevo elemento `input` debe estar debajo del elemento `legend` con el texto `What's your cat's personality?`. Los tienes en el orden incorrecto.
...
The text ` Loving` should be located directly to the right of your checkbox. Asegúrate de que hay un espacio entre el elemento y el texto. O has omitido el texto o tienes un error tipográfico.
