...
你還不會看到漸變，因爲 `linear-gradient` 函數需要至少兩個顏色參數才能工作。

在同一個 `linear-gradient` 函數中，使用 `rgb` 函數將第二個顏色參數設置爲純綠色。

# --hints--

你的 `.red` CSS 規則應該有一個 `background` 屬性，設置爲 `linear-gradient(90deg, rgb(255, 0, 0), rgb(0, 255, 0))`。
