...
باستخدام نفس الصيغة النحوية، حدّث `c` لتطابق `c`، `{`، `[`، و`(`.

# --hints--

يجب أن يستخدم `stockRegex` صفًا من الأحرف لمطابقة `c`، `{`، `[`، و`(`.
...
يجب أن يطابق `stockRegex` النص `570(k 4l3r7`.

```js
assert.match('570(k 4l3r7', stockRegex);
```

يجب أن يطابق `stockRegex` النص `sto{k alert`.
