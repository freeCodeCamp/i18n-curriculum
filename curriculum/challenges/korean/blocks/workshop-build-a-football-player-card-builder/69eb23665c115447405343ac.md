...
각 `stat-row`에는 각각 `span`와 `className`의 `stat-value`를 가진 두 개의 `stat-label` 요소가 있어야 합니다.

`spans` 요소 안의 `div` 요소에는 다음이 있어야 합니다:

- `99`와 `DRI`
- `41`와 `DEF`
- `75`와 `PHY`

`DRI`는 드리블링, `DEF`는 수비, `PHY`는 체격을 나타냅니다.

# --hints--

두 번째 `.stat-row`의 첫 번째 `.stat-col`에 `99` 값을, `DRI` 레이블을 설정하세요.
...
두 번째 `.stat-row`의 두 번째 `.stat-col`에 `41` 값을, `DEF` 레이블을 설정하세요.
...
두 번째 `.stat-row`의 세 번째 `.stat-col`에 `75` 값을, `PHY` 레이블을 설정하세요.
