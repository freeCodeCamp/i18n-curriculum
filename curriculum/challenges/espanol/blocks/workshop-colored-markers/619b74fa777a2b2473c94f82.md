---
id: 619b74fa777a2b2473c94f82
title: Paso 40
challengeType: 0
dashedName: step-40
---

# --description--

Actualiza el selector de clase CSS `.two` para que seleccione la nueva clase `green`. Y actualiza el selector de clase `.three` para que seleccione la nueva clase `blue`.

# --hints--

Tu código ya no debería tener un selector de clases `.two`.

```js
assert.notExists(new __helpers.CSSHelp(document).getStyle('.two'));
```

Debes usar un selector de clase para seleccionar la clase `green`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.green'));
```

Tu regla CSS `.green` debe tener una propiedad `background-color` establecida en `rgb(0, 0, 0)`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.green')?.backgroundColor, 'rgb(0, 0, 0)');
```

Tu código ya no debería tener un selector de clases `.three`.

```js
assert.notExists(new __helpers.CSSHelp(document).getStyle('.three'));
```

Debes usar un selector de clase para seleccionar la clase `blue`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.blue'));
```

Tu regla CSS `.blue` debe tener una propiedad `background-color` establecida en `rgb(0, 0, 0)`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.blue')?.backgroundColor, 'rgb(0, 0, 0)');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Colored Markers</title>
    <link rel="stylesheet" href="styles.css">
  </head>
  <body>
    <h1>CSS Color Markers</h1>
    <div class="container">
      <div class="marker red">
      </div>
      <div class="marker green">
      </div>
      <div class="marker blue">
      </div>
    </div>
  </body>
</html>
```

```css
h1 {
  text-align: center;
}

.container {
  background-color: rgb(255, 255, 255);
  padding: 10px 0;
}

.marker {
  width: 200px;
  height: 25px;
  margin: 10px auto;
}

--fcc-editable-region--
.red {
  background-color: rgb(255, 0, 0);
}

.two {
  background-color: rgb(0, 0, 0);
}

.three {
  background-color: rgb(0, 0, 0);
}
--fcc-editable-region--

```
