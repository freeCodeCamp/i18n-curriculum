...
Тепер час стилізувати третій розділ (`section`). Зауважте, що він має значення `text` та `text-with-images` для атрибута `class`, що означає, що він вже успадковує стилі з правила `.text`.

Створіть селектор `.text-with-images` та встановіть властивість `display` на `grid`.

# --hints--

Ви повинні мати селектор `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

Ваш селектор `.text-with-images` повинен мати властивість `display` зі значенням `grid`.
