...
在你的第一个 `p` 元素中，添加一个 `span` 元素。 为 `span` 赋予 `product-count` 类和 `id` 为 `product-count-for-id${id}`。 然后，在 span 之后，将 `name` 变量的文本赋予 `p` 元素。

# --hints--

你的第一个 `p` 元素应该有一个 `span` 元素。
...
你的 `span` 元素应该具有 `product-count` 的 `class`。
...
你的 `span` 元素应该具有 `product-count-for-id${id}` 的 `id`。
...
你的第一个 `p` 元素应该包含 `name` 变量的文本。 这应该在 span 之外。
