...
由於你正在建構式內呼叫 `fetchCardsData` 和 `initializeEventListeners`，現在需要在類別內定義它們。將兩者都創建為 `private` 方法。確保 `fetchCardsData` 是非同步的，因為它將控制 API 呼叫。

# --hints--

你應該有一個 `private` 方法，名稱為 `fetchCardsData`。
...
你應該有一個 `private` 方法，名稱為 `initializeEventListeners`。
