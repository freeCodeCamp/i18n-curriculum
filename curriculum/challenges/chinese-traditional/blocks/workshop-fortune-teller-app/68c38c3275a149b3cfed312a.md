...
在這個工作坊中，你將繼續練習使用 TypeScript，建置一個占卜應用程式。

首先宣告一個名為 `Card` 的介面。在 `Card` 介面中，新增以下屬性：`name`、`name_short` 和 `value`。`name` 和 `name_short` 屬性的型別應設定為 `string`，而 `value` 則應設定為 `string | number` 的聯集型別。
...
你的 `Card` 介面應該有一個型別為 `name` 的 `string` 屬性。
...
你的 `Card` 介面應該有一個型別為 `name_short` 的 `string` 屬性。
...
你的 `Card` 介面應該有一個型別為 `value` 和 `string` 聯集的 `number` 屬性。
