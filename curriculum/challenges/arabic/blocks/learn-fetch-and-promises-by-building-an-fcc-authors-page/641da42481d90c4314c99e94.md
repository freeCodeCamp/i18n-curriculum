...
تُعيد طريقة `fetch()` وعدًا (`Promise`) وهو كائن عنصر نائب سيُنجز إذا نجح طلبك، أو يُرفض إذا فشل طلبك.

إذا تم الوفاء بـ `Promise`، فإنه يحل إلى كائن `Response`، ويمكنك استخدام طريقة `.then()` للوصول إلى `Response`.
...
يجب ألا تضع فاصلة منقوطة بعد طريقة `.then()`.
