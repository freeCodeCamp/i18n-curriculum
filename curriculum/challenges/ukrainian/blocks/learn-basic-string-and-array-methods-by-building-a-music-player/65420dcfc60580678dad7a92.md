...
Всередину `map()` додайте інструкцію `return` із зворотними лапками, де ви будете інтерполювати всі елементи, відповідальні за відображення деталей пісні.

У зворотних галочках створіть елемент `li` з `id` атрибутом `song-${song.id}` та `class` атрибутом `playlist-song`.

# --hints--

Ви повинні додати інструкцію `return` із порожньою парою зворотних лапок.

```js
assert.match(code, /return\s*`/)
```

Ви повинні створити теґ `li` всередині зворотних лапок.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

Початковий тег `li` повинен мати атрибут `id` зі значенням `song-${song.id}`.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

Початковий тег `li` повинен мати атрибут `class` зі значенням `playlist-song`.
