...
يجب أن يكون لديك query للـ `@media` جديد لـ `only screen` مع `max-width` من `550px`. يجب أن يأتي هذا بعد الاثنتين السابقتين.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

يجب أن تحتوي قاعدتك `@media` الجديدة على منتقى `.hero-title` ومنتقى `.hero-subtitle, .author, .quote, .list-title` ومنتقي `.social-icons` ومنتقى `.text`. هذه الـ selectors يجب أن تكون بهذا الترتيب.
...
يجب أن يكون لمنتقي `.hero-title` الخاص بك خاصية `font-size` بقيمة `6rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[0]?.style?.fontSize === '6rem');
```

يجب أن يكون لدى منتقي `.hero-subtitle, .author, .quote, .list-title` خاصية `font-size` بقيمة `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[1]?.style?.fontSize === '1.8rem');
```

يجب أن يكون لمنتقي `.social-icons` الخاص بك الخاصية `font-size` بقيمة `2rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize === '2rem');
```

يجب أن يكون لمنتقي `.text` الخاص بك خاصية `font-size` بقيمة `1.6rem`.
