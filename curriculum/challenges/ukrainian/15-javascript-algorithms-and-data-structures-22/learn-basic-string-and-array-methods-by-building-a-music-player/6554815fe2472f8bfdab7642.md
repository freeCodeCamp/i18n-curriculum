...
Використайте `const`, щоб створити порожню функцію під назвою `playPreviousSong`.
...
Функція `playPreviousSong` повинна використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Функція `playPreviousSong` має бути порожньою.
