...
다음 단계는 포럼 데이터를 표시할 표를 만드는 것입니다.

구조 분해 할당 아래에 `return` 키워드를 추가하고 그 뒤에 템플릿 리터럴 세트를 추가하세요. 템플릿 리터럴 안에는 표 행 `tr` 요소를 추가하세요.

# --hints--

템플릿 리터럴 세트를 반환해야 합니다.

```js
assert.match(code, /return\s*`[\s\S]*`/);
```

템플릿 리터럴 안에는 빈 `tr` 요소가 포함되어야 합니다.
