...
为 DRI 输入添加一个 `value` 属性 `player.dri`。然后添加一个 `onChange` 属性，设置为一个箭头函数，该函数以 `e` 作为参数，并调用 `setPlayer`，传入展开的 `player` 和 `dri`，其中 dri 设置为 `Number(e.target.value)`。

# --hints--

你的 DRI `input` 应该有一个 `value`，其值等于玩家的 `dri` 属性。
...
你的 DRI `input` 应该有一个 `onChange` 处理器，该处理器调用 `setPlayer`，传入展开的 `player` 和 `dri`，其中 dri 设置为 `Number(e.target.value)`。
