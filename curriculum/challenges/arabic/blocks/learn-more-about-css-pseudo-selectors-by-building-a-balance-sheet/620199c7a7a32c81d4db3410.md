...
أضف إلى عناصر `tr.total` تأثير المرور بالمؤشر / التأثير المرئي يغير الخلفية إلى `#99c9ff`.

# --hints--

يجب أن يكون لديك محدِّد `tr.total:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total:hover'));
```

يجب أن يحتوي محدِّد `tr.total:hover` على خاصية `background-color` مضبوطة على `#99c9ff`.
