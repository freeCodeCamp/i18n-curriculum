...
Ongeza darasa jipya la `bb1-window` kwa vipengele vya `.bb1a`, `.bb1b`, na `.bb1c`. Hii itawafanyia kazi mwelekeo wa rangi.

# --hints--

Haupaswi kuondoa darasa la `bb1a`.

```js
assert.exists(document.querySelector('div.bb1a'));
```

Unapaswa kuongeza darasa la `bb1-window` kwa kipengele cha `.bb1a`.

```js
assert.exists(document.querySelector('div.bb1a.bb1-window'));
```

Haupaswi kuondoa darasa la `bb1b`.

```js
assert.exists(document.querySelector('div.bb1b'));
```

Unapaswa kuongeza darasa la `bb1-window` kwa kipengele cha `.bb1b`.

```js
assert.exists(document.querySelector('div.bb1b.bb1-window'));
```

Haupaswi kuondoa darasa la `bb1c`.

```js
assert.exists(document.querySelector('div.bb1c'));
```

Unapaswa kuongeza darasa la `bb1-window` kwa kipengele cha `.bb1c`.

```js
assert.exists(document.querySelector('div.bb1c.bb1-window'));
```

Haupaswi kubadilisha kipengele cha `.bb1d`.
