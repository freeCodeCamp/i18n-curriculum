...
أضف محدِّد `50%` بين محدِّدَي `0%` و `100%`. سيُطبق هذا في منتصف دورة الحركة. عيّن `background-color` إلى `purple`.

# --hints--

يجب أن تنشئ محدِّد `50%` جديد في قاعدة `@keyframes cabins` الخاصة بك.
...
يجب أن يكون محدِّد `50%` بين محدِّدَي `0%` و `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '50%');
```

يجب أن يحتوي محدِّد `50%` على خاصية `background-color` مضبوطة على `purple`.
