...
أزحِ مسافة بادئة واحدة إلى داخل حلقة `for` الحالية لتعمل فقط عندما يحتوي الكتالوج على أفلام. ثم استبدل `self.items` بـ `movies` للتكرار على الأفلام فقط.

# --hints--

يجب أن تزيح مسافة بادئة واحدة إلى داخل حلقة `for` الحالية وتستبدل `self.items` بـ `movies` للتكرار على الأفلام فقط.
...
عندما يحتوي `self.items` على `Movie('Dances with Wolves', 1990, 'Kevin Costner', 224)` و `Movie('Annie Hall', 1977, 'Woody Allen', 93)`، يجب أن تُرجع طريقة `__str__` القيمة `Media Catalogue (2 items):\n\n=== MOVIES ===\n1. Dances with Wolves (1990) - 224 min, Kevin Costner\n2. Annie Hall (1977) - 93 min, Woody Allen\n`.
...
عندما يحتوي `self.items` على `Movie('Barry Lyndon', 1975, 'Stanley Kubrick', 184)` و `Movie('Fahrenheit 451', 1966, 'Francois Truffaut', 112)`، يجب أن تُرجع طريقة `__str__` القيمة `Media Catalogue (2 items):\n\n=== MOVIES ===\n1. Barry Lyndon (1975) - 184 min, Stanley Kubrick\n2. Fahrenheit 451 (1966) - 112 min, Francois Truffaut\n`.
...
يجب أن تُرجع طريقة `__str__` القيمة `Media Catalogue (empty)` عندما يكون `self.items` فارغًا.
