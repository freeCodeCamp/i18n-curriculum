...
在每個新的 `th` 元素中，嵌套一個 `span` 元素，其中 `class` 設置爲 `sr-only year`。 給它們以下文本（按順序）：`2019`、`2020` 和 `2021`。

爲第三個 `th` 元素添加 `class` 屬性，設置爲 `current`。

將 `td` 元素留空。 此元素的存在只是爲了確保表格具有四列布局並將標題與正確的列相關聯。
...
你的每個新的 `span` 元素都應該有一個設置爲 `sr-only year` 的 `class` 屬性。
...
你的第三個 `th` 元素應將 `class` 屬性設置爲 `current`。
