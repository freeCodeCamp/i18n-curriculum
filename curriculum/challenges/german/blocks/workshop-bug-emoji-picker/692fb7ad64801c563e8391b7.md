...
Erstellen Sie eine Unterklasse `Spider`, die die Klasse `Bug` mit dem generischen Typ-Parameter `string` erweitert.

# --hints--

Sie sollten eine Klasse namens `Spider` haben.
...
Ihre `Spider`-Klasse sollte `Bug` erweitern.
...
Ihre `Spider`-Klasse sollte `Bug` mit einem Typ-Argument von `string` erweitern.
