...
现在为 `PremiumUserDiscount` 实现 `apply_discount` 方法。这应为高级用户应用 20％ 折扣。

返回 `product.price * 0.8`（表现原价的 80%，即打 8 折）。

# --hints--

你的 `PremiumUserDiscount` 类应有一个 `apply_discount` 方法。
...
你的 `apply_discount` 方法应返回 `product.price * 0.8`。
...
你的 `apply_discount` 方法应应用 20％ 折扣（原价的 80％）。
