...
Ainda dentro do terceiro elemento `li`, crie um elemento `ul` para o popup. Dê a ele um `className` de `sub-menu` e um `aria-label` de `Apps`.

# --hints--

Você deve criar um elemento `ul` dentro do seu terceiro elemento `li`.

```js
assert.exists(document.querySelector('li ul'));
```

Você deve definir o atributo `className` do seu elemento `ul` para `sub-menu`.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

Você deve definir o atributo `aria-label` do seu elemento `ul` para `Apps`.
