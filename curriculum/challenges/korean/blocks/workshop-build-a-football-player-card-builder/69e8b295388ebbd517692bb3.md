...
클래스 `div`가 있는 `card-wrapper` 요소가 카드의 바깥 테두리와 잘린 모양을 만듭니다.

이제 `div` 바로 안에 `card-wrapper`의 `className`인 내부 `card`를 추가하세요. 이렇게 하면 카드에 어두운 배경이 생깁니다.

# --hints--

`div` 요소 안에 `className`의 `card`인 `.card-wrapper`를 추가해야 합니다.
