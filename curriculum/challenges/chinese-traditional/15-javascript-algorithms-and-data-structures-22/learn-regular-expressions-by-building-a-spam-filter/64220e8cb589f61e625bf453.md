...
在你的數字和 `dollars` 文本之間，你想要捕捉位值。

使用 `|` 標記允許數字與 `dollars` 之間的數字爲 `hundred`、`thousand`、`million` 或 `billion`。

# --hints--

你的 `dollarRegex` 應該使用 `|` 標記。

```js
assert.match(dollarRegex.source, /\|/);
```

你的 `dollarRegex` 應該有三個 `|` 標記。

```js
assert.lengthOf(dollarRegex.source.match(/\|/g), 3);
```

你的 `dollarRegex` 應使用 `|` 標記來匹配 `hundred`、`thousand`、`million`、或`billion`。
