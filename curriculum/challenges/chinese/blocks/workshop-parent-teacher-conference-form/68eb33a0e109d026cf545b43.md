...
仍然定位 `.contact-method-radio-btn::before`，将 `transform` 设置为 `translate(3px, 3px) scale(0)`。`translate` 函数稍微移动圆圈，`scale(0)` 将其缩小到零大小，因此默认情况下它是隐藏的。 

另外，为了创建平滑的动画效果，当它出现时，为 `transition` 添加 `all 0.3s ease-in`。这意味着任何样式变化都会在 0.3 秒内动画显示，开始时缓慢然后加速。

# --hints--

你应该继续定位 `.contact-method-radio-btn::before` 伪元素。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

将 `transform` 属性设置为 `translate(3px, 3px) scale(0)`。
...
将 `transition` 属性设置为 `all 0.3s ease-in`。
