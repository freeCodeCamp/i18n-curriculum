...
さらにそのオブジェクトの下に、以下のプロパティを持つ新しいオブジェクトを作成してください:
...
`myFavoriteFootballTeam.players` 配列に 2 つの値を追加してください。
