...
Sasa baada ya kuunda `salary` setter, sahihisha `__init__` ili iite badala ya kuweka thamani moja kwa moja kwa `self._salary`. Badilisha `self._salary = Employee._base_salaries[level]` kuwa `self.salary = Employee._base_salaries[level]`.

# --hints--

Njia yako ya `__init__` inapaswa kutumia `self.salary = Employee._base_salaries[level]` badala ya `self._salary = Employee._base_salaries[level]`.
