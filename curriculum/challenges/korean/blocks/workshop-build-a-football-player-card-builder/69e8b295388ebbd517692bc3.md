...
JSX에서 중괄호가 JavaScript 표현식을 JSX 안에 직접 임베드할 수 있게 해준다는 것을 기억하세요. 이것이 프로프스에서 동적 데이터를 표시하는 방법입니다. TSX에서도 동일하게 적용됩니다.

카드 헤더는 여전히 하드코딩된 값을 보여줍니다. `98` 안의 텍스트 `card-rating`를 `player.overallRating`로 바꾸고, `ST` 안의 `card-position`를 `player.position`로 바꾸세요.

# --hints--

하드코딩된 `98`를 `player.overallRating` 안에서 `.card-rating`로 바꾸세요.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

하드코딩된 `ST`를 `player.position` 안에서 `.card-position`로 바꾸세요.
