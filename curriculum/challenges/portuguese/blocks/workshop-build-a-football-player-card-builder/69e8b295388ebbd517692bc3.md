...
Lembre-se do JSX que as chaves permitem incorporar qualquer expressão JavaScript diretamente dentro do JSX. É assim que você exibe dados dinâmicos das props. Você faz o mesmo no TSX.

O cabeçalho do cartão ainda mostra valores fixos. Substitua o texto `98` dentro de `card-rating` por `player.overallRating`, e substitua `ST` dentro de `card-position` por `player.position`.

# --hints--

Você deve substituir o `98` fixo por `player.overallRating` em `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

Você deve substituir o `ST` fixo por `player.position` em `.card-position`.
