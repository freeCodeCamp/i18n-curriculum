...
与えられた長方形は *m* × *n* の正方形で構成されています。*m* と *n* の両方が奇数でない場合、正方形の辺に沿って長方形を切り分ける経路を作ることができ、その結果、長方形は2つの連結した同じ形の部分に分割されます（一方の部分を180°回転させた後に同じ形になります）。2 × 2 と 4 × 3 の長方形に対するすべての経路は以下に示されています。
...
<div style="width: 100%; text-align: center;">
  <svg xmlns="https://www.w3.org/2000/svg" xmlns:xlink="https://www.w3.org/1999/xlink" width="520" height="170" aria-hidden="true" alt="Diagram showing the possible paths for 2 by 2 and 4 by 3 rectangles">
    <style>
.diagram-g { 塗りつぶしなし; 線の色: #ccc }
      .diagram-s, .diagram-s2 { 塗りつぶし: #bff; 線の色: 黒; 塗りつぶしの不透明度: .4 }
      .diagram-s2 { 塗りつぶし: #fbf }
      .diagram-d { 線の色: 黒; 塗りつぶしなし}
    </style>
    <defs>
      <g id="diagram-m">
        <g id="diagram-h4">
...
*m* × *n* の長方形を切り分ける異なる方法の数を計算する関数を書いてください。

# --hints--

`cutRectangle` は関数である必要があります。

```js
assert(typeof cutRectangle == 'function');
```

`cutRectangle(2, 2)` は数値を返す必要があります。

```js
assert(typeof cutRectangle(2, 2) == 'number');
```

`cutRectangle(2, 2)` は `2` を返す必要があります。

```js
assert.equal(cutRectangle(2, 2), 2);
```

`cutRectangle(4, 3)` は `9` を返す必要があります。

```js
assert.equal(cutRectangle(4, 3), 9);
```

`cutRectangle(4, 4)` は `22` を返す必要があります。

```js
assert.equal(cutRectangle(4, 4), 22);
```

`cutRectangle(8, 3)` は `53` を返す必要があります。

```js
assert.equal(cutRectangle(8, 3), 53);
```

`cutRectangle(7, 4)` は `151` を返す必要があります。
