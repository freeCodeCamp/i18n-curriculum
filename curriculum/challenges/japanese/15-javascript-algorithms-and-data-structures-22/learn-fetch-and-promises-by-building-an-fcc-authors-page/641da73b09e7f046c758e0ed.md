...
著者名を画面上に表示させるには、2 番目の `.then()` メソッドの中で `displayAuthors` 関数を呼び出す必要があります。 しかしその前に、いま空の配列が入っている `authorDataArr` に、著者データを代入しましょう。

まず、`console.log()` 文を削除してください。 次に、変数 `authorDataArr` に `data` を代入してください。
