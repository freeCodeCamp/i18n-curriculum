...
لعرض الحسابات أخيرًا، قم بالتكرار على كائن `propertyInputs` باستخدام حلقة `for...of` مع `Object.entries()`. يمكنك تجاهل المفتاح باستخدام فاصلة قبل `input` في إعلان الحلقة.

# --hints--

يجب أن تنشئ حلقة `for...of` تقوم بتفكيك كل مدخل من `Object.entries(propertyInputs)` بتخطي المفتاح والاحتفاظ بـ `input` فقط.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

يجب أن تتكرر حلقة `for...of` الخاصة بك على `Object.entries(propertyInputs)`.
