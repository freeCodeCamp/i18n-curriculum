...
Rectangleオブジェクトが作成されるとき、`width`と`height`属性で初期化される必要があります。このクラスには以下のメソッドも含まれているべきです：

- `set_width`
- `set_height`
- `get_area`: 面積（`width * height`）を返します
- `get_perimeter`: 周囲長（`2 * width + 2 * height`）を返します
- `get_diagonal`: 対角線（`(width ** 2 + height ** 2) ** .5`）を返します
- `get_picture`: '\*'の行を使って形状を表す文字列を返します。行数は高さと同じで、各行の'\*'の数は幅と同じです。各行の最後には改行（`\n`）があります。幅または高さが50より大きい場合は、文字列`'Too big for picture.'`を返します。
- `get_amount_inside`: もう一つの形（正方形または長方形）を引数に取ります。回転なしで渡された形が何回この形の中に収まるかを返します。例えば、幅4、高さ8の長方形は、辺が4の正方形を2つ収めることができます。

さらに、`Rectangle`のインスタンスが文字列として表される場合、次のように見えるべきです：`'Rectangle(width=5, height=10)'`。

## Squareクラス

`Square`クラスは`Rectangle`のサブクラスであるべきです。`Square`オブジェクトが作成されるとき、1つの辺の長さが渡されます。`__init__`メソッドは、`width`クラスの`height`と`Rectangle`属性の両方に辺の長さを保存する必要があります。

`Square`クラスは`Rectangle`クラスのメソッドにアクセスできますが、`set_side`メソッドも含むべきです。`Square`のインスタンスが文字列として表される場合、次のように見えるべきです：`'Square(side=9)'`。

さらに、`set_width`クラスの`set_height`と`Square`メソッドは幅と高さの両方を設定する必要があります。
...
`Square`クラスは`Rectangle`クラスのサブクラスであるべきです。
...
`Square`クラスは`Rectangle`クラスとは別のクラスであるべきです。
...
正方形オブジェクトは`Square`クラスと`Rectangle`クラスのインスタンスであるべきです。
...
`Rectangle(3, 6)`の文字列表現は`'Rectangle(width=3, height=6)'`であるべきです。
...
`Square(5)`の文字列表現は`'Square(side=5)'`であるべきです。
...
`Rectangle(3, 6).get_area()`は`18`を返すべきです。
...
`Square(5).get_area()`は`25`を返すべきです。
...
`Rectangle(3, 6).get_perimeter()`は`18`を返すべきです。
...
`Square(5).get_perimeter()`は`20`を返すべきです。
...
`Rectangle(3, 6).get_diagonal()`は`6.708203932499369`を返すべきです。
...
`Square(5).get_diagonal()`は`7.0710678118654755`を返すべきです。
...
`Rectangle`クラスのインスタンスは新しい値を設定した後、異なる文字列表現を持つべきです。
...
`Square`クラスのインスタンスは`.set_side()`を使って新しい値を設定した後、異なる文字列表現を持つべきです。
...
`Square`クラスのインスタンスは`.set_width()`または`set_height()`を使って新しい値を設定した後、異なる文字列表現を持つべきです。
...
`.get_picture()`メソッドは`Rectangle`インスタンスの異なる文字列表現を返すべきです。
...
`.get_picture()`メソッドは`Square`インスタンスの異なる文字列表現を返すべきです。
...
`.get_picture()`メソッドは、`'Too big for picture.'`または`width`属性が`height`より大きい場合、文字列`50`を返すべきです。
...
`Rectangle(15,10).get_amount_inside(Square(5))`は`6`を返すべきです。
...
`Rectangle(4,8).get_amount_inside(Rectangle(3, 6))`は`1`を返すべきです。
...
`Rectangle(2,3).get_amount_inside(Rectangle(3, 6))`は`0`を返すべきです。
