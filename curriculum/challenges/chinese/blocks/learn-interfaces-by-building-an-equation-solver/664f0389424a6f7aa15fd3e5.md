...
另一种断言是前行断言。 正向和负向前行断言分别用 `(?=...)` 和 `(?!...)` 表示。 它们用于匹配跟随特定字符序列的表达式，但不会被消耗：
...
在上面的例子中，表达式 `a(?=t)` 包含一个正向前行断言，仅当后面跟着 `t` 时才用于匹配 `a` 字符。 在示例的最后一行中，表达式 `a(?!t)` 包含一个负向前行断言，仅当 `a` 后面 **not** 跟着 `t` 字符时，它才用于匹配。 同样，在这两种情况下，前行断言中包含的字符都不会被消耗。

向你的正则表达式添加一个正向前行断言，以便仅当字符 `x` 后跟字符 `1` 时才替换该字符。

# --hints--

你应该使用正向前行断言修改正则表达式，以便仅当字符 `1` 后跟 `x` 时才替换该字符。 不要从你的表达式中删除负向后行断言。
