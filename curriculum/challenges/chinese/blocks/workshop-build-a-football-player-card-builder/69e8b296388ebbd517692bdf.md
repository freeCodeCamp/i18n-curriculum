...
为 DEF 和 PHY 两个输入都添加 `value` 和 `onChange` 属性。

对于 DEF，将 `value` 设置为 `player.def`，将 `onChange` 设置为一个箭头函数，该函数调用 `setPlayer`，并使用展开的 `player`，同时将 `def` 设置为 `Number(e.target.value)`。

对于 PHY，将 `value` 设置为 `player.phy`，将 `onChange` 设置为一个箭头函数，该函数调用 `setPlayer`，并使用展开的 `player`，同时将 `phy` 设置为 `Number(e.target.value)`。

通过这一步，所有输入现在都连接到状态，实时预览将针对每个字段更新！

# --hints--

你的 DEF `input` 应该有一个 `value`，其值等于玩家的 `def` 属性。
...
你的 DEF `input` 应该有一个 `onChange` 处理器，该处理器调用 `setPlayer`，并使用展开的 `player`，同时将 `def` 设置为 `Number(e.target.value)`。
...
你的 PHY `input` 应该有一个 `value`，其值等于玩家的 `phy` 属性。
...
你的 PHY `input` 应该有一个 `onChange` 处理器，该处理器调用 `setPlayer`，并使用展开的 `player`，同时将 `phy` 设置为 `Number(e.target.value)`。
