...
Antes de eliminar una canción, necesitas verificar si la canción se está reproduciendo actualmente. Si es así, necesitas pausar la canción y reproducir la siguiente canción en la lista de reproducción. 

Usa una declaración `if` para comprobar si el `userData?.currentSong?.id` es igual al `id` de la canción que quieres eliminar. 

# --hints--

Debes crear una declaración `if` vacía con la condición `userData?.currentSong?.id === id`.
