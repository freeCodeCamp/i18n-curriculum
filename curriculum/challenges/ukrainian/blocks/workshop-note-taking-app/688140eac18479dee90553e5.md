...
Оскільки ви використовуєте інший `div` елемент, потрібно зробити його більш доступним для читачів екрана.

У попередньому уроці ви дізналися про атрибут `aria-live`, який використовується для створення живої області на сторінці. Це дозволить користувачам читачів екрана автоматично отримувати сповіщення, коли вміст живої області змінюється, без необхідності вручну фокусуватися на ній або взаємодіяти з нею.

У вашому `#status` елементі додайте атрибут `aria-live` і встановіть його значення на `"polite"`.

Значення `polite` повідомить читачам екрана чекати, доки поточне оголошення не завершиться або користувач не припинить вводити текст, перш ніж оголосити оновлення.

# --hints--

Ваш `#status` елемент повинен мати атрибут `aria-live`.

```js
assert.isTrue(document.getElementById("status").hasAttribute("aria-live"));
```

Ваш `#status` елемент повинен мати атрибут `aria-live`, встановлений у `"polite"`.
