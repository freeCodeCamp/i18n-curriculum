...
أنشئ محدِّدًا جديدًا يستهدف عناصر `.card-links a` إذا كانت قد زُرت سابقًا. اضبط الخاصية `color` على `crimson`.

# --hints--

يجب أن يكون لديك محدِّد `.card-links a:visited`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:visited"));
```

يجب أن يحتوي محدِّد `.card-links a:visited` على الخاصية `color` مضبوطة على `crimson`.
