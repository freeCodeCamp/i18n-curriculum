...
تُستدعى الطريقة `__gt__` ضمنيًا عند استخدام معامل `>` لمقارنة كائن مع شيء آخر.

بعد الطريقة `__lt__`، نفذ الطريقة `__gt__` بنفس الطريقة التي نفذت بها `__lt__`. انتبه لاستخدام المعامل المناسب.

# --hints--

يجب تعريف طريقة `__gt__` داخل صنف `R2Vector`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__gt__")`)) })
```

يجب أن تحتوي طريقتك `__lt__` على معلمتين، `self` و `other`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__gt__").has_args("self, other")`)) })
```

يجب إنشاء جملة `if` تتحقق مما إذا كان `self` و `other` لا ينتميان إلى نفس الصنف.
...
يجب إرجاع `NotImplemented` من جسم جملة `if`.
...
يجب أن تُرجع الطريقة `__gt__` نتيجة مقارنة معيار النسخة الحالية مع معيار `other` باستخدام معامل أكبر من.
