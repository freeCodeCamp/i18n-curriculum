...
两个单选按钮都应该有一个 `value` 属性。 确保在开始标签的名称后面有一个空格，且所有的属性名称前面也要有空格。
...
两个单选按钮都应该有 `type` 属性。 确保在开始标签的名称后面有一个空格，且所有的属性名称前面也要有空格。
...
`Indoor` 单选按钮的 `value` 属性应设置为 `indoor`。 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
...
`Outdoor` 单选按钮的 `value` 属性应设置为 `outdoor`。 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
