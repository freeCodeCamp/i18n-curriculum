...
Kwa chaguo-msingi, vipengele vya `label` ni ndani ya mstari. Ili kuvitengeneza kwa wima (isipokuwa lebo zako za kitufe cha radio), tumia darasa la bandia la `:not()`.

Darasa la bandia la `:not()` linapingana na uchaguzi. Hapa, linachagua vipengele vyote vya `label` ambavyo havina darasa la vitu la `contact-method`.

Chagua `label:not(.contact-method)`, kisha weka sifa ya `display` kuwa `block` na sifa ya `margin` kuwa `10px 0`.

# --hints--

Unapaswa kuwa na kichaguzi cha `label:not(.contact-method)`.
...
Weka sifa ya `margin` kuwa `10px 0`.
