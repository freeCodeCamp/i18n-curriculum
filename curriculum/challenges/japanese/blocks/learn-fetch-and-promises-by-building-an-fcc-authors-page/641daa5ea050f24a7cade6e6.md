...
この条件が満たされた場合、ボタンの`disabled`プロパティを`true`に設定して無効化してください。また、ボタンの`textContent`を`"No more data to load"`に設定してください。
...
`disabled`の`loadMoreBtn`プロパティを`true`に設定してください。

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

`textContent`の`loadMoreBtn`を`"No more data to load"`に設定してください。
