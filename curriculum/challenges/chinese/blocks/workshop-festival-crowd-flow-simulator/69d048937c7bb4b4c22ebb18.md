...
现在是时候让参与者通过门口了。在 `while` 循环内：

- 将 `currentTickQueue` 减少 1，表示一名参与者已通过。使用递减操作符（`--`）。
- 将 `processed` 增加 1，以跟踪处理了多少参与者。使用递增操作符（`++`）。

# --hints--

在你的 `while` 循环内，你应该使用递减操作符 `currentTickQueue` 来减少 `--`，并使用递增操作符 `processed` 来增加 `++`。
