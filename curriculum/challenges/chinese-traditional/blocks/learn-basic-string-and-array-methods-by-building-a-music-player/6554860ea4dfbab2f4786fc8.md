...
如果仔細檢查，你會發現當前播放的歌曲未在播放列表中突出顯示，因此你實際上不知道正在播放哪首歌曲。 你可以通過創建一個函數來突出顯示正在播放的任何歌曲來解決此問題。

使用箭頭語法創建一個 `highlightCurrentSong` 函數。 在函數內部，使用 `querySelectorAll` 獲取 `.playlist-song` 元素並分配給 `playlistSongElements` 常量。

# --hints--

你應該使用 `const` 來創建 `highlightCurrentSong` 函數。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` 應該是一個函數。

```js
assert.isFunction(highlightCurrentSong)
```

你的 `highlightCurrentSong` 函數應該使用箭頭語法。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

你應該在 `highlightCurrentSong` 函數內創建一個 `playlistSongElements` 常量。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

你應該使用 `querySelectorAll()` 方法選擇 `.playlist-song` 元素並將其分配給 `playlistSongElements` 常量。
