...
Gli ultimi valori hardcoded sono le sei statistiche del giocatore. Sostituisci i primi tre insiemi di numeri hardcoded con la corrispondente proprietà `player`:
...
Dovresti sostituire il valore `97` per `PAC` con `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

Dovresti sostituire il valore `98` per `SHO` con `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

Dovresti sostituire il valore `83` per `PAS` con `player.pas`.
