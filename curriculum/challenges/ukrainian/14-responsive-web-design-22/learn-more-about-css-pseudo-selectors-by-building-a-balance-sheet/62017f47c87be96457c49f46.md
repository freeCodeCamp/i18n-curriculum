...
Замість того, щоб постійно перевіряти, чи ви не переписали попередні властивості, можна використати ключове слово `!important`, щоб переконатися, що ці властивості завжди застосовуються, незалежно від порядку чи специфіки.

Надайте кожній властивості в своєму селекторі `span[class~="sr-only"]` ключове слово `!important`. Не змінюйте жодних значень.

# --hints--

Ваш селектор `span[class~="sr-only"]` повинен мати властивість `border` зі значенням `0 !important`.
...
Ваш селектор `span[class~="sr-only"]` повинен мати властивість `clip` зі значенням `rect(1px, 1px, 1px, 1px) !important`.
...
Ваш селектор `span[class~="sr-only"]` повинен мати властивість `clip-path` зі значенням `inset(50%) !important`.
