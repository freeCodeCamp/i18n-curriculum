...
Jetzt können Sie die Gesamtanzahl der Elemente auf der Webseite aktualisieren. Weisen Sie den Wert der `cart`-Objekt-`.getCounts()`-Methode der `textContent`-Eigenschaft der `totalNumberOfItems`-Variablen zu.

# --hints--

Sie sollten auf die `textContent`-Eigenschaft der `totalNumberOfItems`-Variablen zugreifen.

```js
assert.match(code, /totalNumberOfItems\s*\.\s*textContent/)
```

Sie sollten den Wert der `cart`-Objekt-`.getCounts()`-Methode der `textContent`-Eigenschaft der `totalNumberOfItems`-Variablen zuweisen.
