...
残りの引数 `chosenCard.name_short` と `chosenCard.img` を含めてください。それから `renderCard` 関数の外で、`multipleCard` 要素の `innerHTML` を空の文字列にリセットしてください。

# --hints--

`chosenCard.name_short` を `renderCard` の引数として渡す必要があります。
...
`chosenCard.img` を `renderCard` の最後の引数として渡す必要があります。
...
`this.elements.multipleCard.innerHTML` を空の文字列にリセットする必要があります。
