...
Щоб отримати індекс для поточної пісні, ви можете скористатися методом <dfn>indexOf()</dfn>. Метод масиву `indexOf()` повертає перший індекс, після якого можна знайти заданий елемент у масиві, або `-1`, якщо елемент відсутній.
...
У функції `getCurrentSongIndex` поверніть `userData?.songs.indexOf()`. Для аргументу `indexOf()` встановіть його до `userData?.currentSong`.

# --hints--

Поверніть `userData?.songs.indexOf()` в межах функції `getCurrentSongIndex`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

Передайте `userData?.currentSong` у метод `indexOf()`.
