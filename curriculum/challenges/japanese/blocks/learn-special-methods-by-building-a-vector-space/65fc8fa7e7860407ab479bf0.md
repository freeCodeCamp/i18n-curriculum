...
掛け算をテストする時です。新しい変数`v5`を宣言して、スカラー乗算`v1 * 3`を代入してください。
次に、`print`関数を呼び出して、次のf文字列を渡してください：`f'v1 * 3 = {v5}'`。

# --hints--

新しい変数`v5`を宣言して、スカラー乗算`v1 * 3`を代入してください。

```js
({ test: () => assert(runPython(`_Node(_code).find_variable("v5").is_equivalent("v5 = v1 * 3")`)) })
```

f文字列`f'v1 * 3 = {v5}'`を出力してください。
