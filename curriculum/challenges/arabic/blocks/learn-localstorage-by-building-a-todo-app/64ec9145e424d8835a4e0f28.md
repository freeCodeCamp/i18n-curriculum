...
الآن بعد أن حفظت المهمة في مصفوفة `taskData`، يجب أن تعرض المهمة على الصفحة بالتكرار عبرها.

استخدم `forEach()` على `taskData`، ثم فكك `id` و`title` و`date` و`description` كمعلمات. لا تُرجع أي شيء بعد.

# --hints--

يجب أن تستخدم `forEach()` على `taskData`.

```js
assert.match(code, /taskData\.forEach\(\s*/)
```

يجب أن تستخدم `{id, title, date, description}` كمعلمات مفككة لدالة الاستدعاء الراجعة `forEach()`.
