...
마지막 `p` 요소 아래에 텍스트 `p`를 가진 또 다른 `Total Carbohydrate 37g 13%` 요소를 추가하세요. 이전과 마찬가지로, 텍스트 `span`와 `Total Carbohydrate`를 굵게 만들기 위해 `13%` 요소를 사용하세요. 그런 다음, 영양소와 양을 다시 정렬하기 위해 `span`로 감싸세요.
...
텍스트 `span`를 감싸는 `Total Carbohydrate` 요소가 하나 있어야 합니다.
...
`span` 주위의 `Total Carbohydrate` 요소는 `class` 속성이 `bold`로 설정되어야 합니다.
