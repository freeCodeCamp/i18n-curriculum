...
Настав час почати реалізовувати функціонал для відтворення відображених пісень.

Визначте `playSong` функцію, використовуючи `const`. Функція має приймати `id` параметр, який представлятиме унікальний ідентифікатор пісні, яку ви хочете відтворити.

# --hints--

Ви маєте використати `const` для створення пустої функції з ім’ям `playSong`.

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

`playSong` має бути функцією.

```js
assert.isFunction(playSong)
```

Ваша `playSong` функція має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Ваша `playSong` функція має приймати `id` параметр.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

Ваша `playSong` функція має бути порожньою.
