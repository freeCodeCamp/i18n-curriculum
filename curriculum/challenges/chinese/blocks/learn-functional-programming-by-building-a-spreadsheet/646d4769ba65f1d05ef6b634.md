...
现在你的 `update()` 函数实际上可以评估公式。 请记住，你编写了 `if` 条件来检查函数是否被调用。

在 `if` 语句中，将 `element` 的 `value` 设置为 `evalFormula()` 函数的结果。 暂时不要传递任何参数。

# --hints--

你应该在 `if` 块中更新 `element` 的 `value` 属性。
...
你应该将调用 `evalFormula()` 函数的结果分配给 `value` 属性。
