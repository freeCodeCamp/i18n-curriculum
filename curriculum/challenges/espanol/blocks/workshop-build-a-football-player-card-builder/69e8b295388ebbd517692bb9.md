...
El encabezado de la tarjeta está completo. Ahora necesitas agregar la sección de la foto del jugador.

Después del cierre de `div` de `card-header` y aún dentro del `div` con `className` de `card`, agrega un `div` con el `className` de `card-image-wrap`. Déjalo vacío por ahora. Agregarás la imagen en el siguiente paso.

# --hints--

Debes agregar un `div` con el `className` de `card-image-wrap`.
