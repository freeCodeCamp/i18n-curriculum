...
Implementa il metodo `is_applicable` per `PremiumUserDiscount`. Questo metodo dovrebbe verificare se l'utente è un membro premium.

Restituisci `True` se `user_tier.lower()` è uguale a `'premium'`, altrimenti restituisci `False`. Usare `.lower()` garantisce che il controllo non sia sensibile al caso.

# --hints--

La tua classe `PremiumUserDiscount` dovrebbe avere un metodo `is_applicable`.
...
Il tuo metodo `is_applicable` dovrebbe restituire `True` quando `user_tier.lower()` è uguale a `'premium'`.
...
Il tuo metodo `is_applicable` dovrebbe restituire `False` quando `user_tier` non è `'premium'`.
