...
Inside the `.btn-container`, create three buttons for each type of story. Dale a cada botón una clase de `btn` y un `id` de `scary-btn`, `funny-btn` y `adventure-btn`. El texto de cada botón debe ser `Scary Story`, `Funny Story`, y `Adventure Story` respectivamente.
