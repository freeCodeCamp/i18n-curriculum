...
高级定价卡应当是页面上最突出的一张卡片。此外，它是唯一带有 `Most popular` 徽章的卡片。为了使这张卡片及其徽章更加突出，你可以使用 Tailwind 的相对定位和缩放工具类。

以下是如何使用 Tailwind CSS 通过缩放使元素变大的方法：
...
你将在接下来的步骤中处理定位。现在，为包装器 `div` 赋值类 `scale-105`，使其比其他定价卡稍大一点。

# --hints--

你的高级计划的包装器 `div` 元素应具有类 `scale-105`。
