...
حدّث الآن `FootballPlayerCard` لتمرير `defaultPlayer` إلى `PlayerCard` بدلاً من الكائن المضمَّن.

# --hints--

يجب أن تكون `defaultPlayer` هي قيمة الخاصية `player` الممررة إلى `PlayerCard`.
