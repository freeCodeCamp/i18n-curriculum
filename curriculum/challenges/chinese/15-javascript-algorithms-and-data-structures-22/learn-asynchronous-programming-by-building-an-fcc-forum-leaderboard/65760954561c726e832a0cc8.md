...
在你的 `allCategories` 对象中，添加一个新的键 `299`，它的值为空对象。

在该对象中，添加一个名为 `category` 的键，其字符串值为 `"Career Advice"`。 在该属性下面，再添加一个名为 `className` 的键，其值为字符串 `"career"`。

# --hints--

你的 `allCategories` 对象应该有一个 `299` 属性。
