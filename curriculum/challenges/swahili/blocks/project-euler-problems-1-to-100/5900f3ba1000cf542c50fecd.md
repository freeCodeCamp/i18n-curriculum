...
title: 'Zoezi la 78: Mgawanyo wa sarafu'
challengeType: 1
forumTopicId: 302191
...
Acha ${p}(n)$ wakilishe idadi ya njia tofauti ambazo sarafu `n` zinaweza kugawanywa katika vikundi. Kwa mfano, sarafu tano zinaweza kugawanywa katika vikundi kwa njia saba tofauti kabisa, kwa hiyo ${p}(5) = 7$.
...
Tafuta thamani ndogo kabisa ya `n` ambayo ${p}(n)$ inagawanyika kwa `divisor`.

# --hints--

`coinPartitions(7)` inapaswa kurudisha nambari.

```js
assert(typeof coinPartitions(7) === 'number');
```

`coinPartitions(7)` inapaswa kurudisha `5`.

```js
assert.strictEqual(coinPartitions(7), 5);
```

`coinPartitions(10000)` inapaswa kurudisha `599`.

```js
assert.strictEqual(coinPartitions(10000), 599);
```

`coinPartitions(100000)` inapaswa kurudisha `11224`.

```js
assert.strictEqual(coinPartitions(100000), 11224);
```

`coinPartitions(1000000)` inapaswa kurudisha `55374`.
