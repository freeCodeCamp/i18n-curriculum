...
يجب عليك استخدام منتقي `.eye`.

```js
assert.match(code, /\.eye\s*\{/);
```

بداخل عنصر `.eye` يجب أن تكون خاصية `width` بقيمة `--fcc-expected--`، ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.width, '15%');
```

بداخل عنصر `.eye` يجب أن تكون خاصية `height` بقيمة `--fcc-expected--`، ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.height, '17%');
```

بداخل عنصر `.eye` يجب أن تكون خاصية `background-color` بقيمة `--fcc-expected--`، ولكن وجدت بقيمة `--fcc-actual--`.
