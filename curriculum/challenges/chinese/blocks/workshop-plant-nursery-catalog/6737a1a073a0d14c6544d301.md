...
声明一个名为 `plantsSet` 的变量，并赋值为对 `displayPlantsSet` 函数的调用。

然后将 `plantsSet` 记录到控制台。你需要打开浏览器控制台才能看到你的集合。


# --hints--

你应该有一个名为 `plantsSet` 的变量。

```js
assert.exists(plantsSet);
```

你应该调用 `displayPlantsSet` 并将其赋值给 `plantsSet`。

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

你应该将 `plantsSet` 记录到控制台日志。
