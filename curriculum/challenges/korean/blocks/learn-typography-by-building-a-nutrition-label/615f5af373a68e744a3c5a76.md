...
`.calories-info span` 선택자를 만들고, `font-size` 속성을 `2.4em`로, `font-weight` 속성을 `700`로 설정하세요.

# --hints--

`.calories-info span` 선택자를 만들어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span'));
```

`.calories-info span` 선택자의 `font-size` 속성은 `2.4em`로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span')?.fontSize === '2.4em');
```

`.calories-info span` 선택자의 `font-weight` 속성은 `700`로 설정되어야 합니다.
