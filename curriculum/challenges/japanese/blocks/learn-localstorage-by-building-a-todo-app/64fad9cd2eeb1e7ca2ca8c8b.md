...
アロー構文を使って`updateTaskContainer`関数を作成してください。それから`taskData.forEach()`の`taskForm`イベントリスナーから`submit`とその内容を新しく作成した関数に移動してください。

# --hints--

`const`とアロー構文を使って`updateTaskContainer`関数を作成することをお勧めします。

```js
assert.match(code, /const\s+updateTaskContainer\s*=\s*\(\s*\)\s*=>\s*\{/)
```

`taskData.forEach()`とその内容を`updateTaskContainer()`関数に移動してください。
