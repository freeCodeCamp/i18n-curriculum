...
يجب أن يحتوي منتقي `.foot` على خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.top, '85%');
```

يجب أن يحتوي منتقي `.foot` على خاصية `border-radius` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
