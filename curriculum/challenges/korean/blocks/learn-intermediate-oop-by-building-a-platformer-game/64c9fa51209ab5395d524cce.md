...
그 불리언 표현식 아래에, 플레이어의 `x` 위치가 플랫폼의 `x` 위치에서 플레이어 너비의 절반을 뺀 값보다 크거나 같은지 확인하는 또 다른 불리언 표현식을 추가하세요. 플레이어 너비의 절반을 계산하려면 나눗셈 연산자(`/`)를 사용하세요.

# --hints--

플레이어의 `x` 위치가 플랫폼의 `x` 위치에서 플레이어 너비를 `2`로 나눈 값을 뺀 것보다 크거나 같은지 확인하는 불리언 표현식이 있어야 합니다.
