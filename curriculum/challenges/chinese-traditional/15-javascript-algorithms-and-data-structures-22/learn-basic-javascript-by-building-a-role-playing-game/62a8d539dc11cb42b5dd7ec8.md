...
在上一個項目中，你學習瞭如何使用 `shift()` 方法從數組中刪除第一個元素，如下所示：
...
使用 `shift()` 方法從 `inventory` 數組中獲取第一個元素並將其分配給 `currentWeapon` 變量。

# --hints--

你的 `sellWeapon` 函數應該使用 `shift()` 方法。

```js
assert.match(sellWeapon.toString(), /shift\(\s*\)/);
```

你應該在 `inventory` 數組上使用 `shift()` 方法。

```js
assert.match(sellWeapon.toString(), /inventory\.shift\(\s*\)/);
```

你應該將 `inventory.shift()` 的值分配給你的 `currentWeapon` 變量。
