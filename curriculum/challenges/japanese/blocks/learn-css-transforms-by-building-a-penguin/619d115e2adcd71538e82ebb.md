...
`.beak` 要素をターゲットにして、`height` を `10%` に、`background-color` を `orange` に設定し、すべての角に `50%` の半径を付けてください。
...
`.beak` に `height` を `--fcc-expected--` に設定するべきです。場所は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.height, '10%');
```

`.beak` に `background-color` を `--fcc-expected--` に設定するべきです。場所は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.backgroundColor, 'orange');
```

`.beak` に `border-radius` を `--fcc-expected--` に設定するべきです。場所は `--fcc-actual--` です。
