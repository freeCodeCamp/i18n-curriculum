...
宣告一個名為 `cardId` 的變數，並使用 `data-id` 方法從 `cardElement` 取得 `getAttribute()` 屬性的值，指定給該變數。

# --hints--

你應該在 `cardId` 內宣告一個名為 `showFortune` 的變數。
...
你的 `cardId` 變數應該透過呼叫 `cardElement.getAttribute("data-id")` 來指定。
