...
- **Властивість `width`**: Ця властивість вказує ширину елемента. If you do not specify a width, then the default is set to `auto`. Це означає, що елемент заповнить всю ширину свого материнського компонента.
