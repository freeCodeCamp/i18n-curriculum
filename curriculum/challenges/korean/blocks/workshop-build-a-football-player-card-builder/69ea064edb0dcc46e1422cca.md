...
이제 `localStorage`을 사용해 `useEffect`에 저장하는 로직을 추가하세요.

React에서 `localStorage`에 데이터를 접근하거나 저장하는 것은 부작용으로 간주되므로, 이 로직에는 `useEffect`를 사용해야 합니다.

`useEffect`에서 했던 것처럼 React에서 `useState`를 구조 분해하여 시작하세요.

# --hints--

`useEffect`에서 `React`를 구조 분해해야 합니다.
