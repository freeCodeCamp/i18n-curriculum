...
メールの読み取りと削除のために、`User` クラスに2つのメソッドを追加してください：`read_email` と `delete_email` です。どちらも `index` パラメータを受け取り、`self.inbox` の対応するメソッドを呼び出す必要があります。

# --hints--

`read_email` クラスに `User` という名前のメソッドを定義し、`self` と `index` をパラメータとして受け取る必要があります。
...
`read_email` メソッドは `self.inbox.read_email(index)` を呼び出すべきです。
...
`delete_email` クラスに `User` という名前のメソッドを定義し、`self` と `index` をパラメータとして受け取る必要があります。
...
`delete_email` メソッドは `self.inbox.delete_email(index)` を呼び出すべきです。
