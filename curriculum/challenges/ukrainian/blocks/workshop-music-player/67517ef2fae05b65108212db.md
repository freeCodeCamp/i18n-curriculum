...
Вам потрібно отримати `id` поточної пісні, що відтворюється. Для цього ви можете використати `userData.currentSong?.id`.

Використайте `getElementById()`, щоб вибрати поточну пісню: передайте шаблонний рядок, щоб додати префікс `id` до `song-`. Призначте це змінній з ім’ям `songToHighlight`.

# --hints--

Вам не слід змінювати існуючу функцію `highlightCurrentSong` та її вміст.
...
Вам слід використати `document.getElementById()` і передати `` `song-${userData.currentSong?.id}` ``.

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Ви повинні призначити свій `getElementById()` змінній `songToHighlight`.
