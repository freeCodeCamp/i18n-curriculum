...
داخل صنف `Bee`، أنشئ دالة بناء تأخذ معلمة واحدة `emojiElement` من نوع `HTMLParagraphElement`.

داخل دالة البناء، استدعِ الدالة `super` مع المعلمة `emojiElement` لاستدعاء دالة البناء الخاصة بالصنف الأب `Bug`. ثم عيّن الخاصية `emoji` إلى القيمة النصية `"🐝"`.

# --hints--

يجب أن يحتوي صنف `Bee` على دالة بناء.
...
يجب أن تحتوي دالة البناء في `Bee` على معلمة باسم `emojiElement` من نوع `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Bee[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

يجب أن تستدعي دالة البناء في `Bee` الدالة `super` مع المعلمة `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /super\s*\(\s*emojiElement\s*\)/);
```

يجب أن تعيّن دالة البناء في `Bee` الخاصية `this.emoji` إلى `"🐝"`.
