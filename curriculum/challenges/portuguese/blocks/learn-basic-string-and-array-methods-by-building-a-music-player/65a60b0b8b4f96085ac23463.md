...
No exemplo, se `a.name` for igual a `b.name`, então a função retorna `0`. Isso significa que nada muda e a ordem de `a` e `b` permanece a mesma.
...
Abaixo das suas declarações `if`, retorne o número `0` para deixar a ordem dos dois elementos inalterada.

# --hints--

Você deve retornar o número `0` abaixo dos seus comandos `if`.
