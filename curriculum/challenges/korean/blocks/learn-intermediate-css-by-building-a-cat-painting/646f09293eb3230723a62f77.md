...
클래스 선택자로 `.cat-mouth-line-right` 요소에 접근한 다음, `position`의 `absolute`와 `top`의 `88px`, 그리고 `left`의 `91px`를 사용해 올바른 위치로 이동하세요.

# --hints--

`.cat-mouth-line-right` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-right'))
```

`.cat-mouth-line-right` 선택자에는 `position`으로 설정된 `absolute` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-right')?.position === 'absolute')
```

`.cat-mouth-line-right` 선택자에는 `top`으로 설정된 `88px` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-right')?.top === '88px')
```

`.cat-mouth-line-right` 선택자에는 `left`으로 설정된 `91px` 속성이 있어야 합니다.
