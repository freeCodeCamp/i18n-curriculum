...
Debes añadir un nuevo elemento `label` después del elemento casilla de verificación. Make sure it has both an opening and closing tag.
...
Tu elemento `label` debe tener un atributo `for` con el valor `loving`.  Expected `--fcc-expected--`, but found `--fcc-actual--`.
