...
Ongeza kipengele cha nanga chenye maandishi `Read our terms and conditions` kabla ya `label` iliyoongezwa hivi karibuni. Weka `href` kuwa:

```md
https://www.freecodecamp.org/news/terms-of-service/
```

Kisha ongeza maandishi `I accept the terms and conditions` mara moja baada ya kipengele cha ingizo katika lebo.

# --hints--

Unapaswa kuwa na kipengele cha `a` moja kwa moja kabla ya `label` chenye maandishi `Read our terms and conditions`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.innerText.trim().replaceAll(/\s+/g, ' '), /Read our terms and conditions\.?/i);
```

Kipengele cha `a` kinapaswa kuwa na `href` ya `https://www.freecodecamp.org/news/terms-of-service/`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.href, /https:\/\/www\.freecodecamp\.org\/news\/terms-of-service\/?/);
```

Unapaswa kuwa na kipengele cha `input` ndani ya `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) + a + label > input'));
```

Ndani ya `label`, mara moja baada ya kipengele cha `input` unapaswa kuwa na maandishi `I accept the terms and conditions`.
