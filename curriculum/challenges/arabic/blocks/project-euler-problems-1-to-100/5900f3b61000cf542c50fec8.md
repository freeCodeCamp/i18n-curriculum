...
title: "المشكلة 73: عد الكسور في مدى"
challengeType: 1
forumTopicId: 302186
...
يمكن ملاحظة وجود `3` كسور بين $\frac{1}{3}$ و $\frac{1}{2}$.

كم عدد الكسور التي تقع بين $\frac{1}{3}$ و $\frac{1}{2}$ في مجموعة الكسور الفعلية المختزلة المرتبة لـ `d` ≤ `limit`؟

# --hints--

`countingFractionsInARange(8)` يجب أن تُرجع رقمًا.

```js
assert(typeof countingFractionsInARange(8) === 'number');
```

`countingFractionsInARange(8)` يجب أن تُرجع `3`.

```js
assert.strictEqual(countingFractionsInARange(8), 3);
```

`countingFractionsInARange(1000)` يجب أن تُرجع `50695`.

```js
assert.strictEqual(countingFractionsInARange(1000), 50695);
```

`countingFractionsInARange(6000)` يجب أن تُرجع `1823861`.

```js
assert.strictEqual(countingFractionsInARange(6000), 1823861);
```

`countingFractionsInARange(12000)` يجب أن تُرجع `7295372`.
