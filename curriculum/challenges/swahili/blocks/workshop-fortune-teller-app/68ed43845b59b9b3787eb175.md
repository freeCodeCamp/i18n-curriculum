...
Ongeza `constructor` ndani ya darasa la vitu. Katika kijenzi, unahitaji kutumia vipengele ulivyoviumba awali kuchagua vipengele vinavyolingana vya DOM kwa kutumia kitendakazi cha `getElement`.

Kwa sasa, shughulikia `singleCardBtn` na `singleCard`. Kwa `singleCardBtn`, pita `#btn-single-card`, na kwa `singleCard`, pita `.single_card`.

# --hints--

Darasa la `Game` linapaswa kuwa na `constructor`.
...
Kijenzi chako kinapaswa kuweka thamani ya `this.elements`.
...
`singleCardBtn` inapaswa kuwekwa kwa kutumia `getElement` na `'#btn-single-card'`.
...
`singleCard` inapaswa kuwekwa kwa kutumia `getElement` na `'.single_card'`.
