...
마지막으로 하드코딩된 값은 6개의 플레이어 스탯입니다. 처음 세 세트의 하드코딩된 숫자를 해당 `player` 속성으로 바꾸세요.
