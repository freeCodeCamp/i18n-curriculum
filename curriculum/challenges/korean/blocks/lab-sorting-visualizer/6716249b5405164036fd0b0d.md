...
1. `1`부터 `100`까지(양 끝 포함)의 임의의 정수를 반환하는 `generateElement`라는 이름의 함수가 있어야 합니다.
1. `generateElement` 함수를 사용하여 다섯 개의 임의의 정수를 포함하는 배열을 반환하는 `generateArray`라는 이름의 함수가 있어야 합니다.
1. 비어 있는 `div` 요소를 생성하여 반환하는 `generateContainer`라는 이름의 함수가 있어야 합니다.
1. HTML 요소를 첫 번째 인수로, 배열을 두 번째 인수로 받는 `fillArrContainer`라는 이름의 함수가 있어야 합니다.
1. `fillArrContainer` 함수는 요소를 첫 번째 매개변수로, 정수 배열을 두 번째 매개변수로 받은 다음, 다섯 개의 `span` 요소로 해당 요소를 채워야 하며, 각 `span`은 배열의 정수 중 하나를 표시합니다.
1. 두 개의 정수를 받아 첫 번째 정수가 두 번째 정수보다 작거나 같은지를 나타내는 불리언을 반환하는 `isOrdered`라는 이름의 함수가 있어야 합니다.
1. 정수 배열과 숫자 인덱스를 받는 `swapElements`라는 이름의 함수가 있어야 합니다.
1. `swapElements` 함수는 `isOrdered`가 `false`를 반환하면 전달된 인덱스의 요소와 그다음 요소를 서로 바꾸어 배열을 제자리에서 수정해야 합니다.
1. HTML 요소와 숫자 인덱스를 받는 `highlightCurrentEls`라는 이름의 함수가 있어야 합니다.
1. `highlightCurrentEls` 함수는 주어진 인덱스에 있는 해당 요소의 자식과 그 인덱스 바로 다음 자식의 `border`를 `dashed` 스타일, `red` 색상, 그리고 원하는 너비로 설정해야 합니다.
1. `#generate-btn`을 클릭하면 `fillArrContainer` 함수를 사용하여 `#starting-array`를 다섯 개의 `span` 요소로 채워야 하며, 각 요소는 임의의 숫자를 텍스트로 가집니다. 다른 요소가 있다면 `#array-container`에서 제거해야 합니다.
1. `#sort-btn`을 클릭한 후 `#array-container`가, 초기 배열을 나타내는 `div`와 정렬된 배열을 나타내는 `div`를 포함하여, Bubble Sort 알고리즘이 초기 배열을 정렬하는 데 필요한 각 단계에 해당하는 `div` 요소를 담도록 Bubble Sort 알고리즘을 구현해야 합니다. 지금까지 만든 함수들이 여기서 유용할 수 있습니다.
1. 각 `div`에는 다섯 개의 `span` 요소가 있어야 하며, 정렬되는 현재 상태의 배열을 나타냅니다.
1. `#sort-btn`을 클릭한 후 `#starting-array`는 초기 배열과 처음 두 정수가 강조 표시된 시작 단계를 나타내야 합니다.
1. 각 정렬 단계마다 `highlightCurrentEls`를 사용하여 비교되는 두 숫자를 강조 표시하고, 다음 단계에서 `swapElements`를 사용하여 서로 바꿔야 합니다.
