...
title: 'Problema 78: Particiones de moneda'
challengeType: 1
forumTopicId: 302191
...
Sea ${p}(n)$ represente al número de diferentes maneras en las que `n` monedas puedan separarse en pilas. Por ejemplo, cinco monedas pueden separarse en pilas en exactamente siete maneras diferentes, así que ${p}(5) = 7$.
...
Encuentra el valor menor de `n` para el cual ${p}(n)$ es divisible para `divisor`.

# --hints--

`coinPartitions(7)` debería devolver un número.

```js
assert(typeof coinPartitions(7) === 'number');
```

`coinPartitions(7)` debería devolver `5`.

```js
assert.strictEqual(coinPartitions(7), 5);
```

`coinPartitions(10000)` debería devolver `599`.

```js
assert.strictEqual(coinPartitions(10000), 599);
```

`coinPartitions(100000)` debería devolver `11224`.

```js
assert.strictEqual(coinPartitions(100000), 11224);
```

`coinPartitions(1000000)` debe devolver `55374`.
