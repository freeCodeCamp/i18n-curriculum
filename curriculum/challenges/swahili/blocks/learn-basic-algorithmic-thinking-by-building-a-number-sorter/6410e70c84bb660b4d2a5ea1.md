...
Ili kufanikisha matokeo ya "bubble up", unahitaji kukagua kama kipengele cha sasa ni kikubwa kuliko kipengele kinachofuata. Unaweza kufanya hivi kwa kupata upatikanaji wa `array` katika `j` na `j+1`.

Tengeneza hali ya `if` inayokagua kama kipengele cha sasa ni kikubwa kuliko kipengele kinachofuata.

# --hints--

Haupaswi kuondoa tamko lako la `console.log`.
...
Unapaswa kutumia tamko la `if` katika mzunguko wako ulio pangwa ndani.
...
Tamko lako la `if` linapaswa kukagua kama `array[j]` ni kubwa kuliko `array[j+1]`.
...
Tamko lako la `if` linapaswa kuwa tupu.
