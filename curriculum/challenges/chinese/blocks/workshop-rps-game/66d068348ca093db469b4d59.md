...
研讨会的下一部分是创建将显示结果的功能性。

首先创建一个名为 `playerScoreSpanElement` 的变量，并将其赋值为 id 为 `player-score` 的元素。

然后创建一个名为 `computerScoreSpanElement` 的变量，并将其赋值为 id 为 `computer-score` 的元素。

最后，创建一个名为 `roundResultsMsg` 的变量，并将其赋值为 id 为 `results-msg` 的元素的值。

# --hints--

你应该有一个名为 `playerScoreSpanElement` 的变量。

```js
assert.isNotNull(playerScoreSpanElement);
```

你应该将 id 为 `player-score` 的元素的值赋值给 `playerScoreSpanElement`。

```js
assert.equal(playerScoreSpanElement, document.getElementById("player-score"));
```

你应该有一个名为 `computerScoreSpanElement` 的变量。

```js
assert.isNotNull(computerScoreSpanElement);
```

你应该将 id 为 `computer-score` 的元素的值赋值给 `computerScoreSpanElement`。

```js
assert.equal(computerScoreSpanElement, document.getElementById("computer-score"));
```

你应该有一个名为 `roundResultsMsg` 的变量。

```js
assert.isNotNull(roundResultsMsg);
```

你应该将 id 为 `results-msg` 的元素的值赋值给 `roundResultsMsg`。
