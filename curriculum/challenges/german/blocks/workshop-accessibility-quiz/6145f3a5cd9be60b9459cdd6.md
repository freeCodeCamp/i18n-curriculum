...
Verknüpfen Sie das erste `label`-Element mit dem `select`-Element und geben Sie dem `select`-Element ein `name`-Attribut.

# --hints--

Sie sollten dem `label`-Element ein `for`-Attribut geben.

```js
assert.notEmpty(document.querySelector('.question-block > label')?.htmlFor);
```

Sie sollten dem `select`-Element ein `id`-Attribut geben.

```js
assert.notEmpty(document.querySelector('.answer > select')?.id);
```

Sie sollten dem `select`-Element ein `id`-Attribut geben, das mit dem `for`-Attribut des `label`-Elements übereinstimmt.

```js
assert.equal(document.querySelector('.answer > select')?.id, document.querySelector('.question-block > label')?.htmlFor);
```

Sie sollten dem `select`-Element ein `name`-Attribut geben.
