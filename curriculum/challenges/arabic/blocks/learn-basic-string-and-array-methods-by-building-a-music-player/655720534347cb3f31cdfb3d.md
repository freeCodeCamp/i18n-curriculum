...
قبل حذف أغنية، يجب التحقق مما إذا كانت الأغنية تُشغل حاليًا. إذا كانت كذلك، يجب إيقاف الأغنية مؤقتًا وتشغيل الأغنية التالية في قائمة التشغيل.

استخدم جملة `if` للتحقق مما إذا كان `userData?.currentSong?.id` يساوي `id` الخاص بالأغنية التي تريد حذفها.

# --hints--

يجب إنشاء جملة `if` فارغة مع الشرط `userData?.currentSong?.id === id`.
