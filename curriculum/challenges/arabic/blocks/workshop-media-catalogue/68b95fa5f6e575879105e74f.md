...
الآن بعد أن اكتمل صنف `MediaError`، عدل جملة `if` في طريقة `add`. استبدل `TypeError` بـ `MediaError` مع نفس الرسالة.

أيضًا، مرر `media_item` كالمعلمة الثانية بعد الرسالة. هذا سيسمح لك بتخزين العنصر الذي تسبب في الخطأ داخل كائن الاستثناء.

# --hints--

يجب أن ترفع `MediaError` مع الرسالة `Only Movie or TVSeries instances can be added` عندما لا يكون `media_item` مثيلًا لـ `Movie` أو لأحد أصنافها الفرعية.
...
يجب أن تمرر `media_item` كالمعلمة الثانية إلى `MediaError`.
...
لا يجب أن ترفع `MediaError` عندما يكون `media_item` عنصرًا صالحًا.
...
يجب ألا ترفع طريقة `add` الخاصة بك `TypeError`.
