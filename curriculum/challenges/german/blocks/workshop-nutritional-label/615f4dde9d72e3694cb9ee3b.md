...
Aktualisieren Sie nun Ihren `h1`-Selektor zu `header h1`, um gezielt Ihr `h1`-Element innerhalb Ihres neuen `header` anzusprechen.

# --hints--

Sie sollten einen `header h1`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('header h1'));
```

Sie sollten keinen `h1`-Selektor haben.

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('h1'));
```

Sie sollten keine Eigenschaften im Selektor ändern.
