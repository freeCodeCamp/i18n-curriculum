...
Ihre `idToText`-Funktion gibt derzeit ein `input`-Element zurück. Aktualisieren Sie sie so, dass sie das `value` dieses `input`-Elements zurückgibt.

# --hints--

Sie sollten die `value`-Eigenschaft des Rückgabewerts der `.find()`-Methode zurückgeben.
