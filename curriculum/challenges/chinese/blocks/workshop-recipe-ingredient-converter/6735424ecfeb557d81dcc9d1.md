...
在此步骤中，为 `recipeForm` 添加一个 `submit` 事件监听器，以便在点击 `Convert` 按钮时调用 `updateResultsList()` 函数。

为 `recipeForm` 添加一个 `submit` 事件监听器。当表单提交时，调用 `event.preventDefault()` 来阻止默认行为，然后调用 `updateResultsList()`。

这将允许你通过填写表单并按下 `Convert` 来通过 UI 触发函数，对于在下一步添加转换逻辑时进行调试非常有用。

# --hints--

通过为 `recipeForm` 添加一个 `"submit"` 事件监听器，使点击 `Convert` 按钮时运行 `updateResultsList()`，该监听器调用 `event.preventDefault()` 以阻止默认的表单行为。
