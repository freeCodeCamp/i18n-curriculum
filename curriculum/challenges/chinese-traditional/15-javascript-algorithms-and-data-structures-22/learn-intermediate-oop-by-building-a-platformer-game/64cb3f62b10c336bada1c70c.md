...
在動畫函數中，你需要將每個 `checkpoints` 繪製到畫布上。

添加一個 `forEach` 循環，迭代 `checkpoints` 數組。

在回調函數內部，添加一個 `checkpoint` 參數，並在函數主體上調用每個 `checkpoint` 上的 `draw` 方法。

# --hints--

你應該有一個 `forEach` 循環，用於迭代 `checkpoints` 數組。

```js
assert.match(code, /\bcheckpoints\b\s*\.\s*forEach\s*\(/s);
```

你應該在 `forEach` 回調函數中有一個 `checkpoint` 參數。

```js
assert.match(code, /\s*checkpoints\s*\.\s*forEach\s*\(\s*(\(\s*checkpoint\s*\)|checkpoint)\s*=>\s*{?/s);
```

你應該在 `forEach` 回調函數內的每個 `checkpoint` 上調用 `draw` 方法。
