...
Jetzt ist es an der Zeit zu definieren, was passiert, wenn die Optionsschaltfläche aktiviert ist.

Sie gestalten nur den inneren Kreis, wenn die Optionsschaltfläche ausgewählt ist, daher sollte Ihr Selektor `.contact-method-radio-btn:checked::before` sein.

Legen Sie in der Regel `transform` auf `translate(3px, 3px) scale(1)` und `background-color` auf `lightgreen` fest.

Das `transform` verschiebt den inneren Kreis leicht und skaliert ihn mit `scale(1)` auf volle Größe, und ein hellgrüner Hintergrund füllt den Kreis mit Farbe, um zu zeigen, dass die Option ausgewählt ist, wodurch sie sichtbar wird.

# --hints--

Sie sollten das Pseudoelement `.contact-method-radio-btn:checked::before` ansprechen.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

Setzen Sie die Eigenschaft `transform` auf `translate(3px, 3px) scale(1)`.
...
Setzen Sie die Eigenschaft `background-color` auf `lightgreen`.
