...
Ora fai lo stesso per `multipleCardsBtn` e `newReadingBtn` allegando un listener per l'evento click a `multipleCardsBtn` che chiama `multipleCardSelected`, e un altro a `newReadingBtn` che chiama `newReading`.

Creerai i due metodi nei passaggi successivi.

# --hints--

Dovresti allegare un evento `click` all'elemento `multipleCardsBtn`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

All'interno del callback per l'evento `multipleCardsBtn`, dovresti chiamare il metodo `multipleCardSelected`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

Dovresti allegare un evento `click` all'elemento `newReadingBtn`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

All'interno del callback per l'evento `newReadingBtn`, dovresti chiamare il metodo `newReading`.
