...
関数の<dfn>参照</dfn>をコールバックのパラメータとして渡すことができます。関数参照とは、丸括弧なしの関数名のことです。例えば：
...
ここでの`.map()`メソッドは、`myFunc`関数を呼び出し、`.map()`コールバックが受け取るのと同じ引数を渡します。最初の引数は現在の反復の配列の値なので、`newArray`は`[value: 1, value: 2, value: 3]`になります。

`elemValue`関数の参照を、`.map()`メソッドのコールバックとして渡してください。

# --hints--

`elemValue`関数を呼び出してはいけません。
...
`elemValue`の参照を、`.map()`メソッドのコールバックとして渡してください。
