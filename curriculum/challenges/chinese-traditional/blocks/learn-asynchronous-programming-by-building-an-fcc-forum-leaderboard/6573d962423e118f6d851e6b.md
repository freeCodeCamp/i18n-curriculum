...
接著，使用 `getElementById()` 方法存取 `#posts-container` 元素。將它指定給一個名為 `postsContainer` 的新常數。

# --hints--

你應該使用 `const` 來宣告一個 `postsContainer` 變數。

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

你應該使用 `document.getElementById()` 來取得 `#posts-container` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

你應該將 `#posts-container` 元素指定到你的 `postsContainer` 變數。
