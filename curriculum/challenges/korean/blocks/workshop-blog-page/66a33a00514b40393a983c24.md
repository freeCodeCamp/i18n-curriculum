...
세 번째 블로그 게시물에는 또 다른 `article` 요소를 추가해야 합니다.

`article` 요소 안에 `h3` 요소를 추가하고 `Mr. Whiskers' First Birthday Party` 텍스트를 넣으세요.

`h3` 요소 아래에 두 개의 lorem ipsum 단락을 추가하세요.

# --hints--

`article` 요소 안에 세 번째 `section` 요소가 있어야 하며 `id`은 `posts`로 설정되어야 합니다.
...
`h3` 요소 안에 `article` 요소가 있어야 하며 텍스트는 `Mr. Whiskers' First Birthday Party`이어야 합니다.
