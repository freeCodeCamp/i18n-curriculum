...
尋找 *sum to one hundred* 拼圖的解法。

在十進位數字字串 **123456789** 的任意數字前添加數學運算子 **+** 或 **─**（加號或減號），使得結果的數學表達式加總為特定的和（在這個經典範例中為 **100**）。
...
撰寫一個以數字作為參數的函式（程式）。該函式應傳回一個陣列，該陣列包含所有給定數字的解法。解法應該是表述運算式的字串。例如：「1+23-456+78-9」。在傳回之前先對陣列進行排序。
...
`sumTo100(199)` 應該傳回一個陣列。

```js
assert(Array.isArray(sumTo100(199)));
```

`sumTo100(199)` 應該傳回 `["-1+2-3+45+67+89", "123-4+5+6+78-9", "123-4+56+7+8+9"]`。
...
`sumTo100(209)` 應該傳回 `["1+234+56+7-89"]`。

```js
assert.deepEqual(sumTo100(209), ['1+234+56+7-89']);
```

`sumTo100(243)` 應該傳回 `["-1-234+567-89", "-12+345+6-7-89", "123+45+6+78-9"]`。
...
`sumTo100(197)` 應該傳回 `["1-2-3+45+67+89", "12+34-5+67+89", "123+4-5+6+78-9"]`。
