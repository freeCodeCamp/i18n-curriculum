...
次に、プレイリストの次の曲を取得する必要があります。そのためには、現在の曲のインデックスを取得してから、`1` を加えます。

`nextSong` という定数を作成し、`userData?.songs[currentSongIndex + 1]` を代入してください。

最後に、`playSong` 関数を呼び出し、`nextSong.id` を引数として渡してください。

# --hints--

既存の `if` 文、その `else` ブロック、および内容は変更しないでください。
...
`userData?.songs[currentSongIndex + 1]` を `nextSong` 定数に代入してください。
...
`playSong` 関数を `nextSong.id` で呼び出してください。
