...
最後，創建最後一個 `if` 述語來驗證 `duration` 是否不大於 `0`，並引發帶有訊息 `Duration must be positive` 的 `ValueError`。

# --hints--

當 `duration` 不是正數時，你應該引發一個帶有訊息 `Duration must be positive` 的 `ValueError`。
...
當 `duration` 是有效數字時，你不應該引發 `ValueError`。
