...
O loop está completo. Retorne `grouped` da função.

Depois, abaixo da função, faça a chamada `groupByDecade(catalog)` e atribua o resultado a uma variável chamada `byDecade`. Registre `byDecade` no console para inspecionar o resultado agrupado, e você deverá ver um objeto com chaves como `"1950s"`, `"1960s"`, `"1970s"` e assim por diante, cada uma apontando para um array de livros daquela década.

# --hints--

`groupByDecade` deve retornar um objeto.

```js
assert.isObject(groupByDecade(catalog));
```

`byDecade["1980s"].length` deve ser `5`.

```js
assert.strictEqual(groupByDecade(catalog)["1980s"].length, 5);
```

`byDecade["Unknown"].length` deve ser `1`.

```js
assert.strictEqual(groupByDecade(catalog)["Unknown"].length, 1);
```

Você deve atribuir o resultado de `groupByDecade(catalog)` a uma variável chamada `byDecade`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+byDecade\s*=\s*groupByDecade\s*\(\s*catalog\s*\)/);
```

Você deve registrar `byDecade` no console.
