...
Ongeza sifa za `value` na `onChange` kwa ingizo zote mbili za DEF na PHY.

Kwa DEF, weka `value` kuwa `player.def` na `onChange` kuwa kitendakazi cha mshale kinachofanya wito kwa `setPlayer` kwa kueneza `player` na `def` kuwekwa kuwa `Number(e.target.value)`.

Kwa PHY, weka `value` kuwa `player.phy` na `onChange` kuwa kitendakazi cha mshale kinachofanya wito kwa `setPlayer` kwa kueneza `player` na `phy` kuwekwa kuwa `Number(e.target.value)`.

Kwa hatua hii, ingizo zote sasa zimeunganishwa na hali na onyesho la moja kwa moja litasasishwa kwa kila uwanja!

# --hints--

`input` yako ya DEF inapaswa kuwa na `value` sawa na takwimu za `def` za mchezaji.
...
`input` yako ya DEF inapaswa kuwa na mshughulikiaji wa `onChange` unaofanya wito kwa `setPlayer` kwa kueneza `player` na `def` kuwekwa kuwa `Number(e.target.value)`.
...
`input` yako ya PHY inapaswa kuwa na `value` sawa na takwimu za `phy` za mchezaji.
...
`input` yako ya PHY inapaswa kuwa na mshughulikiaji wa `onChange` unaofanya wito kwa `setPlayer` kwa kueneza `player` na `phy` kuwekwa kuwa `Number(e.target.value)`.
