...
Da `elemValue` eine Funktion zurückgibt, gibt Ihre `addCharacters`-Funktion letztlich ein Array von Funktionsreferenzen zurück. Sie möchten, dass die `.map()`-Methode die innere Funktion Ihrer `elemValue`-Funktion ausführt, was bedeutet, dass Sie `elemValue` aufrufen müssen, anstatt nur darauf zu verweisen. Übergeben Sie `num` als Argument an Ihre `elemValue`-Funktion.

<!-- TODO: Explain further? -->

# --hints--

Sie sollten `elemValue()` in Ihrer `.map()`-Methode aufrufen.
...
Sie sollten `num` an Ihren `elemValue()`-Aufruf übergeben.
