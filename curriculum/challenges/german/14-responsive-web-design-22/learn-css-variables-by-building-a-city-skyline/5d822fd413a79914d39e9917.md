...
Füge einen `repeating-linear-gradient` zu `.fb1c` unterhalb des vorhandenen hinzu; verwende deine `--building-color4` von `0 %` bis `10 %` und `--window-color4` von <0 >10 %</code> und `90 %`. Dies wird hinter dem zuletzt hinzugefügten Farbverlauf hinzugefügt.

# --hints--

Du solltest den ersten `repeating-linear-gradient` nicht verändern.
...
Du solltest einen `repeating-linear-gradient` mit einer ersten Farbe von `--building-color4` von `0%` bis `10%`.
...
Du solltest eine zweite Farbe von `--window-color4` von `10%` bis `90%` verwenden.
