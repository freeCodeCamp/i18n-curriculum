...
`resetText`を`resetButton`要素の子として追加し、`resetButton`も`playlistSongs`要素の子として追加する必要があります。そのために使う`appendChild()`メソッドがあります。

`appendChild()`はノードや要素を別の要素の子として追加できます。以下の例では、「Click me」というテキストがボタンに付けられます。
...
`appendChild()`を使って`resetText`を`resetButton`要素に、`resetButton`を`playlistSongs`要素に付けてください。

# --hints--

`appendChild()`で`resetButton`メソッドを使うべきです。

```js
assert.match(code, /resetButton\.appendChild\(/)
```

最初の`resetText`の値として`appendChild()`を渡してください。

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

`appendChild()`で`playlistSongs`メソッドを使うべきです。

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

2番目の`resetButton`の値として`appendChild()`を渡してください。
