...
還有兩個錯誤需要修正。首先，如果你創建一個標題只有空格的任務，即使標題是必填的，也會創建一個空的任務。

透過在函式（程式）開始時加入對 `!titleInput.value.trim()` 的檢查來修正。如果沒有標題，顯示帶有文字 `Please provide a title` 的警告，然後 `return`。

# --hints--

你應該在 `addOrUpdateTask` 函式中加入一個條件為 `!titleInput.value.trim()` 的 `if` 述語。

```js
assert.match(code, /if\s*\(\s*!titleInput\.value\.trim\(\s*\)\s*\)\s*/)
```

你應該在你的 `if` 區塊中呼叫 `alert("Please provide a title")`。
