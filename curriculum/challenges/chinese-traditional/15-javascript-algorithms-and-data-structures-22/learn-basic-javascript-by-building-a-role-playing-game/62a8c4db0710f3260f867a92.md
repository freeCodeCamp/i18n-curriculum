...
如果玩家沒有足夠的金幣來購買健康怎麼辦？ 你應該使用 `if` 語句來檢查玩家是否有足夠的金幣來購買健康。

在上一個項目中，你學習瞭如何使用 `if` 語句，如下所示：
...
首先將 `buyHealth` 函數中的所有代碼放在 `if` 語句內。 對於 `if` 語句條件，檢查 `gold` 是否大於或等於 `10`。

# --hints--

你的 `buyHealth` 函數應該有一個 `if` 語句。

```js
assert.match(buyHealth.toString(), /if/);
```

你的 `if` 語句應檢查 `gold` 是否大於或等於 `10`。

```js
assert.match(buyHealth.toString(), /if\s*\(\s*gold\s*>=\s*10\s*\)/);
```

所有 `buyHealth` 代碼都應位於 `if` 語句內。
