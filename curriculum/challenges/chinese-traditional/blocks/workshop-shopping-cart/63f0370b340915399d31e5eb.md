...
你也將更新此方法中的 HTML。將 `cartSubTotal` 的 `textContent` 設為 `subTotal` 的值，並固定為 2 位小數。使用模板字面值語法將美元符號添加到該值的開頭。

# --hints--

你應該存取 `cartSubTotal` 元素的 `textContent` 屬性。

```js
assert.match(cart.calculateTotal.toString(), /cartSubTotal\.textContent/);
```

你應該在 `subTotal` 變數上呼叫 `.toFixed()` 方法，並傳入 `2` 作為引數。
...
你應該將 `cartSubTotal` 元素的 `textContent` 設定為你的模板字串。
