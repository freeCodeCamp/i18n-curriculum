...
最後の`p`要素の下に、テキスト`p`を持つ別の`Total Carbohydrate 37g 13%`要素を追加してください。前回と同様に、テキスト`span`と`Total Carbohydrate`を太字にするために`13%`要素を使ってください。そして、栄養素と量を再び整列のために`span`で囲んでください。
...
新しい`p`要素にはテキスト`Total Carbohydrate 37g 13%`が含まれている必要があります。
...
テキスト`span`を囲む`Total Carbohydrate 37g`要素が1つ必要です。
...
テキスト`span`を囲む`Total Carbohydrate`要素が1つ必要です。
...
`span`の周りの`Total Carbohydrate`要素には、属性`class`が`bold`に設定されている必要があります。
