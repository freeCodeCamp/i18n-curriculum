...
你可以看到區段頂部的硬色彩變化。將此區段的漸層型別從 `linear-gradient` 更改為 `repeating-linear-gradient`。這將使你的漸層的四種顏色重複，直到到達元素底部；為你產生一些條紋，並且省去你必須添加大量元素來創建它們的麻煩。

# --hints--

你應該將 `.bb2b` 的 `background` 屬性從使用 `linear-gradient` 改為使用 `repeating-linear-gradient`。
