...
添加 `else` 塊。 在 `else` 塊內，使用當前播放歌曲的 `id` 作爲參數調用 `playSong` 函數。

這確保了當點擊播放按鈕時當前播放的歌曲將繼續播放。
