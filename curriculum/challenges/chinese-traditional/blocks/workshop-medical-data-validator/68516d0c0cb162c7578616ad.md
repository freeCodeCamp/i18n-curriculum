...
為你的 `constraints` 字典添加最後一個鍵 `last_visit_id`。對於它的值，使用 `isinstance` 來驗證 `last_visit_id` 是否為字串。

# --hints--

你的 `constraints` 字典應該有一個鍵 `last_visit_id`。
...
你的 `constraints` 字典中的 `last_visit_id` 鍵應該具有 `isinstance(last_visit_id, str)` 的值。
