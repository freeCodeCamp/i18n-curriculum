...
Jetzt wird es bunt. Geben Sie dem `body` eine `background-color` von `brown` und dem `.card` ebenfalls eine `background-color` von `white`.

# --hints--

Das `body`-Element sollte eine `background-color` von `brown` haben.
...
Sie sollten einen `.card`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card"));
```

Das `.card`-Element sollte eine `background-color` von `white` haben.
