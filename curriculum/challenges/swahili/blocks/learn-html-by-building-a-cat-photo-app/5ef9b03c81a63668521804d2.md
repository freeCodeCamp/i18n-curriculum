...
Kipengele chako cha `ol` kinafaa kuwa na tagi ya kufungua. Opening tags have this syntax: `<elementName>`.

```js
assert(document.querySelector('ol'));
```

Kipengele chako cha `ol` kinapaswa kuwa na tagi ya kufunga. Closing tags have a `/` just after the `<` character.
