...
Дайте елементу `.sun` `width` і `height` зі значенням `200px`, а також `background-color` зі значенням `yellow`.
...
Ви повинні дати `.sun` `width` зі значенням `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.width, '200px');
```

Ви повинні дати `.sun` `height` зі значенням `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.height, '200px');
```

Ви повинні дати `.sun` `background-color` зі значенням `yellow`.
