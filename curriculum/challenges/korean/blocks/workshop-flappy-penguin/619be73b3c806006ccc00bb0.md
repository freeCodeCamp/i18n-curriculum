...
가상 요소에 부모 너비의 절반인 `width`를, `height`의 `45%`를, 그리고 `background-color`의 `gray`를 지정하세요.

# --hints--

`.penguin-body::before`에 `width`인 `50%`를 지정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.width, '50%');
```

`.penguin-body::before`에 `height`인 `45%`를 지정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.height, '45%');
```

`.penguin-body::before`에 `background-color`인 `--fcc-expected--`를 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.
