...
`div`の`className`を持つ別の`form-group`を追加してください。その中に、`label`の`className`、`label`の`htmlFor`、およびテキスト`imageUrl`を持つ`Image URL`要素を追加します。次に、`input`の`id`、`imageUrl`の`className`、および`input`の`type`を持つ`text`要素を追加してください。

# --hints--

`div`の`className`を持つ`form-group`要素があるはずです。
...
`div.form-group`には、`label`の`htmlFor`属性、`imageUrl`の`className`、およびテキスト`label`を持つ`Image URL`があるはずです。
...
画像URLの`form-group`には、`input`の`id`、`imageUrl`の`className`、および`input`の`type`を持つ`text`が含まれているはずです。
