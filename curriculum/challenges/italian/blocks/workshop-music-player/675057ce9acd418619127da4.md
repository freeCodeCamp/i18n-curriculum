...
Ora, crea una funzione chiamata `getPreviousSong`. Deve restituire la canzone che si trova a `getCurrentSongIndex()` meno uno in `userData.songs`, oppure `undefined` se la canzone corrente è la prima nella playlist.

# --hints--

Dovresti avere una funzione chiamata `getPreviousSong`.

```js
assert.isFunction(getPreviousSong);
```

La tua funzione `getPreviousSong` dovrebbe chiamare `getCurrentSongIndex`.
...
La tua funzione `getPreviousSong` dovrebbe restituire la canzone precedente a quella corrente.
...
La tua funzione `getPreviousSong` dovrebbe restituire `undefined` quando la canzone corrente è la prima nella playlist.
