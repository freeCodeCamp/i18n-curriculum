...
第一个可以用超过 `n` 种方式表示为素数之和的值是多少？

# --hints--

`primeSummations(5)` 应该返回一个数字。

```js
assert(typeof primeSummations(5) === 'number');
```

`primeSummations(5)` 应该返回 `11`。

```js
assert.strictEqual(primeSummations(5), 11);
```

`primeSummations(100)` 应该返回 `31`。

```js
assert.strictEqual(primeSummations(100), 31);
```

`primeSummations(1000)` 应该返回 `53`。

```js
assert.strictEqual(primeSummations(1000), 53);
```

`primeSummations(5000)` 应该返回 `71`。
