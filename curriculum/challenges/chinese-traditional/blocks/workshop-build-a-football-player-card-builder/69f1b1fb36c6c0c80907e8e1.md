...
在 `preview-panel` 元素內，添加一個 `p` 元素，該元素是 `className` 的 `preview-label`，並包含文字 `Live Preview`，另一個 `p` 元素，是 `className` 的 `preview-hint`，並包含文字 `Updates as you type`，還有一個 `div`，其 `className` 設為模板字串 `preview-box tier-${getPlayerTier(player.overallRating)}`。

在最後一個 `div` 中，放入 `PlayerCard` 組件及其 prop。

# --hints--

你的 `div.preview-panel` 元素應包含一個 `p`，其 `className` 為 `preview-label`，並包含文字 `Live Preview`。
...
你的 `div.preview-panel` 應包含一個 `p`，其 `className` 為 `preview-hint`，並包含文字 `Updates as you type`。
...
你的 `div.preview-panel` 應包含一個 `div`，其 `className` 為 `preview-box`，並有一個以 `className` 為前置式的動態層級 `tier-`。
...
你的 `PlayerCard` 應該被渲染在 `className` 為 `preview-box` 的 `div` 元素內。
