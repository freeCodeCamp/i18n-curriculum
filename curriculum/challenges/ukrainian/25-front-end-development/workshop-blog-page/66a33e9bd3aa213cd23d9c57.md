...
В останньому кроці потрібно додати номер телефону і адресу електронної пошти автора блогу.

Додайте абзац з текстом `Phone: 555-555-5555` до елемента `address`.

Під цим абзацом додайте ще один абзац з текстом `Email: fake@email.com`.

Це були останні зміни. Сторінка блогу тепер завершена.

# --hints--

Ви повинні мати два елементи `p` в межах елемента `address`.
...
Перший елемент `p` повинен містити текст `Phone: 555-555-5555`.
...
Другий елемент `p` повинен містити текст `Email: fake@email.com`.
