...
Der andere Text wurde verschoben. Bringen Sie ihn in Position, indem Sie dem `.first-paragraph::first-letter`-Selektor eine `float`-Eigenschaft mit dem Wert `left` und eine `margin-right`-Eigenschaft mit dem Wert `1rem` zuweisen.

# --hints--

Ihr `.first-paragraph::first-letter`-Selektor sollte eine `float`-Eigenschaft mit dem Wert `left` haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float === 'left');
```

Ihr `.first-paragraph::first-letter`-Selektor sollte eine `margin-right`-Eigenschaft mit dem Wert `1rem` haben.
