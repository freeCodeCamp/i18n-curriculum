...
Dovresti utilizzare il selettore `.shirt`.

```js
assert.match(code, /\.shirt\s*\{/);
```

Dovresti dare a `.shirt` una proprietà `font-size` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontSize, '25px');
```

Dovresti dare a `.shirt` una proprietà `font-family` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontFamily, 'Helvetica, sans-serif');
```

Dovresti dare a `.shirt` una proprietà `font-weight` di `--fcc-expected--` invece di `--fcc-actual--`.
