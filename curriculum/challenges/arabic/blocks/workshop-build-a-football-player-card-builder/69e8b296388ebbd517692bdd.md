...
أضف خاصية `value` بقيمة `player.pas` إلى `input` الخاص بـ PAS. ثم أضف خاصية `onChange` مع دالة سهمية تأخذ `e` كمعلمة وتستدعي `setPlayer` مع نسخ من `player` وتعيين `pas` إلى `Number(e.target.value)`.

# --hints--

يجب أن يحتوي `input` الخاص بـ PAS على خاصية `value` تساوي قيمة `pas` للاعب.
...
يجب أن يحتوي `input` الخاص بـ PAS على معالِج `onChange` يستدعي `setPlayer` مع نسخ من `player` وتعيين `pas` إلى `Number(e.target.value)`.
