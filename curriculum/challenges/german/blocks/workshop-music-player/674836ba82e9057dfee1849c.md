...
Verwenden Sie als Nächstes die `classList`-Eigenschaft und die `add()`-Methode, um die `playing`-Klasse zum `playButton`-Element hinzuzufügen. Dadurch wird in der CSS-Datei nach der Klasse `playing` gesucht und sie dem `playButton`-Element hinzugefügt.

Setzen Sie dann `userData.currentSong` auf `song`, und um das Lied schließlich abzuspielen, verwenden Sie die `play()`-Methode auf der `audio`-Variablen. `play()` ist eine Methode der Web-Audio-API zum Abspielen einer mp3-Datei.

# --hints--

Sie sollten die `playing`-Klasse zum `playButton`-Element innerhalb Ihrer `playSong`-Funktion hinzufügen.
...
Sie sollten `userData.currentSong` innerhalb Ihrer `song`-Funktion auf `playSong` setzen.
...
Sie sollten die `play()`-Methode auf der `audio`-Variablen innerhalb Ihrer `playSong`-Funktion aufrufen.
