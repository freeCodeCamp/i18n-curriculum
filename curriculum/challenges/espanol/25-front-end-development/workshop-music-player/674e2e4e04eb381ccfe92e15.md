...
Ahora es el momento de probar el botón de pausa.

Agregue un `"click"` event listener al elemento `pauseButton`, luego pase `pauseSong` como el segundo argumento del evento listener. Esta es la función que ejecutará el event listener.

Prueba tu aplicación haciendo clic primero en el botón de reproducción y luego en el botón de pausa. Deberías ver que todo funciona como se espera.

# --hints--

Deberías llamar al método `addEventListener()` en tu variable `pauseButton`.

```js
assert.match(__helpers.removeJSComments(code), /pauseButton\.addEventListener\(/)
```

Tu event listener debería escuchar un evento `"click"`.

```js
assert.match(__helpers.removeJSComments(code), /pauseButton\.addEventListener\(\s*('|"|`)click\1/)
```

Deberías pasar `pauseSong` como el segundo argumento de tu método `addEventListener`.
