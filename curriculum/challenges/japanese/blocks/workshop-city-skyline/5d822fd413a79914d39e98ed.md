...
`.fb4` の位置を、`position` の `relative` と `left` の `10%` を追加して、現在の位置から移動させてください。`.fb5` に対しても同様に行いますが、`right` の代わりに `left` を使ってください。これで建物の間の残りの白いスペースを覆うことができます。

# --hints--

`.fb4` に `position` の `relative` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.position, 'relative');
```

`.fb4` に `left` の `10%` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.left, '10%');
```

`.fb5` に `position` の `relative` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb5')?.position, 'relative');
```

`.fb5` に `right` の `10%` を設定してください。
