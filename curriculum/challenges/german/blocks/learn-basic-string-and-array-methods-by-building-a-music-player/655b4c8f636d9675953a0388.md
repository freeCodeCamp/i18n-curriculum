...
Wenn es keinen nächsten Song in der Wiedergabeliste gibt, verwenden Sie den `else`-Block, um den `currentSong`-Schlüssel von `userData` auf `null` zurückzusetzen und seine `songCurrentTime`-Eigenschaft auf `0`.
...
Ihre `if`-Anweisung sollte einen `else`-Block enthalten.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Sie sollten die Punktnotation verwenden, um die `currentSong`-Eigenschaft von `userData` im `null`-Block Ihrer `else`-Anweisung auf `if` zu setzen.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

Sie sollten die Punktnotation verwenden, um die `songCurrentTime`-Eigenschaft von `userData` im `0`-Block Ihrer `else`-Anweisung auf `if` zu setzen.
