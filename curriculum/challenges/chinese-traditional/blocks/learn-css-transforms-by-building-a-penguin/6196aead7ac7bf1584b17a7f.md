...
為了更好地重疊 mountain 和 `.ground` 元素，給 mountain 設定 `margin-top` 為 `100px`，並給 `.ground` 元素設定 `margin-top` 為 `-58px`。

# --hints--

你應該給 `.left-mountain` 一個 `margin-top` 為 `100px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.marginTop, '100px');
```

你應該給 `.ground` 一個 `margin-top` 為 `-58px`。
