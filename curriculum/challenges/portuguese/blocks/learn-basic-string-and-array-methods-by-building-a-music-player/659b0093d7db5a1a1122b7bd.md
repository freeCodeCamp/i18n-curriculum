...
Para obter o índice da música atual, você pode usar o método <dfn>indexOf()</dfn>.O método de array `indexOf()` retorna o primeiro índice em que um determinado elemento pode ser encontrado no array, ou `-1` se o elemento não estiver presente.
...
Dentro da sua função `getCurrentSongIndex`, retorne `userData?.songs.indexOf()`.Para o argumento `indexOf()`, defina-o como `userData?.currentSong`.

# --hints--

Você deve retornar `userData?.songs.indexOf()` dentro da sua função `getCurrentSongIndex`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

Você deve passar `userData?.currentSong` para o método `indexOf()`.
