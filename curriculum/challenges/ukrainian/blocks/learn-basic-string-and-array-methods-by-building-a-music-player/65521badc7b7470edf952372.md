...
Тепер потрібно попрацювати над паузою для пісні, яка зараз відтворюється.

Визначте `pauseSong` функцію, використовуючи ключове слово `const` та синтаксис стрілкової функції. Функція не повинна приймати жодних параметрів.

# --hints--

Ви повинні використати `const` для створення порожньої функції з ім’ям `pauseSong`.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` має бути функцією.

```js
assert.isFunction(pauseSong)
```

Ваша функція `pauseSong` має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Ваша функція `pauseSong` має бути порожньою.
