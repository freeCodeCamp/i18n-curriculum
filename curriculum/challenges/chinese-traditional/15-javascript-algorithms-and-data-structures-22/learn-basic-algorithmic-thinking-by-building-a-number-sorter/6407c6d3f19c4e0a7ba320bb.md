...
在你的 `sortInputArray()` 函數中，調用你的 `updateUI()` 函數，並將 `inputValues` 作爲參數傳遞。

現在你應該可以點擊 `Sort` 按鈕，並在 `Output` 部分看到輸入的數組。

# --hints--

你應該在 `sortInputArray()` 函數中調用 `updateUI()`。

```js
assert.match(sortInputArray.toString(), /updateUI\(/);
```

你應該將 `inputValues` 作爲參數傳遞給 `updateUI()`。
