...
今、`span` 要素の中の `p` に、クラス `text-base`、`font-medium`、`text-gray-500` を代入してください。

# --hints--

`span` 要素の中の `p` はクラス `text-base` を持つべきです。
...
`span` 要素の中の `p` はクラス `font-medium` を持つべきです。
...
`span` 要素の中の `p` はクラス `text-gray-500` を持つべきです。
