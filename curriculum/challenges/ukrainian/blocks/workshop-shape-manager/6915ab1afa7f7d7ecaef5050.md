...
Щоб нарешті показати обчислення, ітеруйте над об’єктом `propertyInputs` за допомогою циклу `for...of` з `Object.entries()`. Ви можете ігнорувати ключ, використавши кому перед `input` в оголошенні циклу.

# --hints--

Ви маєте створити цикл `for...of`, який деструктурує кожен запис з `Object.entries(propertyInputs)`, пропускаючи ключ і залишаючи лише `input`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Ваш цикл `for...of` має ітерувати над `Object.entries(propertyInputs)`.
