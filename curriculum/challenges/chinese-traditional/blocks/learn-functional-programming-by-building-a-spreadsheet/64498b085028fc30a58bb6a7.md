...
宣告一個函式（程式）`elemValue`，該函式接受一個 `num` 參數。該函式應該是空的。

# --hints--

你應該在你的 `rangeFromString()` 函式後宣告一個 `elemValue` 變數。
