...
O método `sort()` aceita uma função de callback de comparação que define a ordem de classificação. 

Neste exemplo, a primeira condição `(a.name < b.name)` verifica se o nome da primeira fruta é menor que o nome da segunda fruta.Se for assim, a primeira fruta é ordenada antes da segunda fruta.

Strings são comparadas lexicograficamente o que significa que elas são comparadas caractere por caractere.Por exemplo, `"Apples"` é menor que `"Bananas"` porque `"A"` vem antes de `"B"` no alfabeto.

A razão pela qual este exemplo está retornando números é porque o método `sort()` espera que um número seja retornado.Se você retornar um número negativo, o primeiro item será ordenado antes do segundo item. 
...
Dentro da sua função de callback, adicione uma declaração `if` para verificar se `a.title` é menor que `b.title`.Se for o caso, retorne `-1`. 

# --hints--

Você deve ter uma declaração `if` dentro da sua função callback.

```js
assert.match(code, /if\s*\(/);
```

Sua declaração `if` deve verificar se `a.title` é menor que `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
``` 

Sua declaração `if` deve retornar `-1` se `a.title` for menor que `b.title`.
