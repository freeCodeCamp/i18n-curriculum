...
为 PAS `value` 添加一个 `player.pas` 属性，属性值为 `input`。然后添加一个 `onChange` 属性，属性值为一个箭头函数，该函数以 `e` 作为参数，并调用 `setPlayer`，传入 `player` 的展开和 `pas` 设置为 `Number(e.target.value)`。

# --hints--

你的 PAS `input` 应该有一个 `value`，其值等于玩家的 `pas` 属性。
...
你的 PAS `input` 应该有一个 `onChange` 处理器，该处理器调用 `setPlayer`，传入 `player` 的展开和 `pas` 设置为 `Number(e.target.value)`。
