...
Unahitaji kuwa na uwezo wa kulinganisha anuwai za seli katika usemi wa kihesabu. Anuwai za seli zinaweza kuonekana kama `A1:B12` au `A3:A25`. Unaweza kutumia usemi wa kawaida kulinganisha mifumo hii.

Anza kwa kutangaza tofauti ya `rangeRegex` na weka thamani yake kuwa usemi wa kawaida unaolingana na `A` hadi `J` (anuwai ya safu katika lahajedwali lako). Tumia kundi la kunasa lenye darasa la herufi kufanikisha hili.

# --hints--

Unapaswa kutangaza tofauti ya `rangeRegex` baada ya kitendakazi chako cha `idToText`.
...
Unapaswa kutumia `const` kutangaza tofauti yako ya `rangeRegex`.
...
Tofauti yako ya `rangeRegex` inapaswa kuwa usemi wa kawaida.
...
`rangeRegex` inapaswa kutumia kundi la kunasa.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(.*\)\/\s*;?/);
```

`rangeRegex` inapaswa kutumia darasa la herufi ndani ya kundi la kunasa.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[.*\]\)\/\s*;?/);
```

`rangeRegex` inapaswa kutumia darasa la herufi kulinganisha `A` hadi `J`.
