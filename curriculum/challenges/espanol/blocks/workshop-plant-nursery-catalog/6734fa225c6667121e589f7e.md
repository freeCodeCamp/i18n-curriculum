...
Tu función `sellPlants` está generando un `TypeError` porque está intentando leer una propiedad de un objeto que ha sido eliminado del catálogo. Por lo tanto, necesitarás encargarte del caso en el que una clave no se encuentra en tu mapa `catalog`. Para eso, el método `has` es muy útil. Devuelve un booleano que indica si la clave especificada existe en el mapa o no.

Modifica tu función `sellPlants` para que devuelva `Artículo no encontrado.` cuando el catálogo no contenga la planta pasada a la función.

# --hints--

Tu función `sellPlants` debería llamar al método `has` en `catalog`.
...
`sellPlants(ballerina, "pequeño", 10)` debería devolver `Artículo no encontrado.`.

```js
assert.equal(sellPlants(ballerina, "small", 10), "Item not found.")
```

<code>sellPlants(hidcote, "grande", 95)` debería devolver `No hay suficientes macetas de tamaño grande para Lavandula angustifolia 'Hidcote'. Solo quedan 18.</code>.
...
`sellPlants(hidcote, "grande", 5)` debería devolver `Catálogo actualizado con éxito.` y restar `5` de la propiedad `grande` del objeto asociado con la clave `hidcote` en el catálogo.
