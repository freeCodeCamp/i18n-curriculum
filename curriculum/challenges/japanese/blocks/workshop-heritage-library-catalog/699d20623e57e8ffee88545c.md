...
これから`groupByDecade`を作成します。これはカタログを年代ごとに整理します。各キーは`"1970s"`のような文字列で、各値はその時代の本の配列になります。

`groupByDecade`をパラメータとして受け取る`catalog`という名前の関数を作成してください。その中で`const grouped = {}`を宣言します。`for`を繰り返すために`catalog`ループを追加し、ループ内で`const book = catalog[i]`を宣言してください。

# --hints--

`groupByDecade`パラメータを持つ`catalog`という名前の関数があるはずです。
