...
Ni wakati wa kupamba kitufe cha kuwasilisha.

Chagua kipengele chenye darasa la `submit-btn`. Weka `cursor` kuwa `pointer` (kuonyesha kinaweza kubofyiwa), `background-color` kuwa `royalblue`, `color` kuwa `whitesmoke`, `border` kuwa `none`, `border-radius` kuwa `6px`, na `padding` kuwa `12px 20px`.

# --hints--

Unapaswa kulenga kipengele chenye darasa la `submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Weka sifa ya `cursor` kuwa `pointer`.
...
Weka sifa ya `background-color` kuwa `royalblue`.
...
Weka sifa ya `color` kuwa `whitesmoke`.
...
Weka sifa ya `border-radius` kuwa `6px`.
...
Weka sifa ya `padding` kuwa `12px 20px`.
