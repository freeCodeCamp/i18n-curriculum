...
CSS変数を作成するには、名前の前にハイフンを2つ付けて、その値を次のように代入するだけです。

```css
--penguin-skin: gray;
```

これで`--penguin-skin`という名前の変数が作成され、`gray`の値が代入されます。これをCSSの他の場所で使って、他のプロパティの値をグレーに変更できます。

# --instructions--

`penguin`クラス内で、`--penguin-skin`という名前の変数を作成し、`gray`の値を代入してください。

# --hints--

`penguin`クラスは`--penguin-skin`変数を宣言し、`gray`に代入する必要があります。
