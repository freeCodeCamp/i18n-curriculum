...
بعد `form-group` الخاص بـ DRI، أضف عنصرين `div` إضافيين لكل منهما `className` بقيمة `form-group` داخل `stats-grid`، واحد لـ DEF والآخر لـ PHY.

بالنسبة لـ DEF، أضف `label` مع `htmlFor` بقيمة `def` والنص `DEF`، ثم أضف مُدخَلًا مع `id` بقيمة `def` و`type` من نوع `number`.

وبالنسبة لـ PHY، أضف مسمى مع `htmlFor` بقيمة `phy` والنص `PHY`، ثم أضف مُدخَلًا مع `id` بقيمة `phy` و`type` من نوع `number`.

# --hints--

يجب أن تضيف مجموعة أخرى من عنصرين `div` لكل منهما `className` بقيمة `form-group` داخل عنصر `div` الذي يحمل `className` بقيمة `stats-grid`.
...
يجب أن يحتوي عنصر `div` الخامس الذي يحمل `className` بقيمة `form-group` على مسمى `label` مع خاصية `htmlFor` بقيمة `def`، و`className` بقيمة `label`، ونص `DEF`.
...
يجب أن يحتوي عنصر `div` الخامس الذي يحمل `className` بقيمة `form-group` على مُدخَل `input` مع `className` بقيمة `input`، و`id` بقيمة `def`، و`type` من نوع `number`.
...
يجب أن يحتوي عنصر `div` السادس الذي يحمل `className` بقيمة `form-group` على مسمى `label` مع خاصية `htmlFor` بقيمة `phy`، و`className` بقيمة `label`، ونص `PHY`.
...
يجب أن يحتوي عنصر `div` السادس الذي يحمل `className` بقيمة `form-group` على مُدخَل `input` مع `className` بقيمة `input`، و`id` بقيمة `phy`، و`type` من نوع `number`.
