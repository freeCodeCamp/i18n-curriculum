...
`playPreviousSong` 関数内に、条件が `if` の `userData?.currentSong === null` 文を追加してください。これは現在曲が再生されていないかどうかをチェックします。再生されていなければ、`return` を使って関数を終了してください。

`else` ブロック内に、`currentSongIndex` という名前の定数を作成し、`getCurrentSongIndex()` を代入してください。

# --hints--

既存の `playPreviousSong` 関数とその内容は変更しないでください。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

`if` 関数内に、条件が `userData?.currentSong === null` の `playPreviousSong` 文があるはずです。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

`return` 文のブロック内に `if` があるはずです。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

`if` 文には `else` ブロックが必要です。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

`getCurrentSongIndex` 文の `currentSongIndex` ブロック内で `else` を呼び出し、`if` に代入してください。
