...
クラス `playlist-song-artist` を持つ `span` 要素を作成してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

クラス `playlist-song-artist` を持つ `span` 要素は、内容として `song.artist` を挿入してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

クラス `playlist-song-duration` を持つ `span` 要素を作成してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

クラス `playlist-song-duration` を持つ `span` 要素は、内容として `song.duration` を挿入してください。
