...
وسّط عناصر `h1` و `p` بإعطائها `margin` بقيمة `1em auto`. ثم، قم بمحاذاة نصها في `center` أيضًا.

# --hints--

استخدم محدِّد عناصر مفصول بفواصل لتنسيق العناصر `h1` و `p`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

يجب استخدام `margin` بقيمة `1em auto` لمركزة العنصرين `h1` و `p`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

استخدم خاصية `text-align` بقيمة `center` لمركزة نصوص العناصر `h1` و `p`.
