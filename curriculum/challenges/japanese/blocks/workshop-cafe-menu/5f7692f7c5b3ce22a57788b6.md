...
スタイリングを完成させるために、該当するクラス名 `flavor` と `price` を残りのすべての `p` 要素に追加してください。
...
`.flavor` 要素は、テキストが `p`、`French Vanilla`、`Caramel Macchiato`、`Pumpkin Spice`、`Hazelnut` の `Mocha` 要素であるはずです。
...
`.price` 要素は、テキストが `p`、`3.00`、`3.75`、`3.50`、`4.00` の `4.50` 要素であるはずです。
