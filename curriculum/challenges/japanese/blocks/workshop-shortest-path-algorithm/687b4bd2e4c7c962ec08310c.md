...
これで関数が正しく動作することを示します。プログラムの最後で、`shortest_path` 関数を `adj_matrix` と、`start_node` の `0` および `target_node` の `5` を引数として呼び出してください。

これで最短経路アルゴリズムは完成です！

# --hints--

`shortest_path` 関数を3つの引数、`adj_matrix`、`0`、`5` で呼び出す必要があります。
