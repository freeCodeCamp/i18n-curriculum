...
现在是时候为你的第三个 `section` 设置样式了。 注意，它具有值是 `text` 和 `text-with-images` 的 `class` 属性，这意味着它已经从你的 `.text` 规则继承了样式。

创建一个 `.text-with-images` 选择器并将 `display` 属性设置为 `grid`。

# --hints--

你应该有一个 `.text-with-images` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

你的 `.text-with-images` 选择器应该有一个 `display` 属性，设置为 `grid`。
