...
في استدعاء راجع مستمع الحدث الخاص بك، نفذ طريقة `.addItem()` لكائن `cart`، ومرر `event.target.id`. تذكر أن `id` هنا ستكون سلسلة نصية، لذا عليك تحويلها إلى رقم.

مرر مصفوفة `products` كالمعلمة الثانية.

# --hints--

يجب أن يستدعي استدعاء راجع مستمع الحدث طريقة `.addItem()` لكائن `cart`.

```js
assert.match(code, /cart\.addItem\(/);
```

يجب استدعاء طريقة `.addItem()` مع `event.target.id` كالمعلمة الأولى. تذكر تحويل `id` إلى رقم باستخدام `Number()`.

```js
assert.match(code, /cart\.addItem\(\s*Number\(\s*event\.target\.id\s*\)\s*/);
```

يجب استدعاء طريقة `.addItem()` مع مصفوفة `products` كالمعلمة الثانية.
