...
`freeRegex`처럼, `stockRegex`을 업데이트하여 `e`와 `o` 문자를 문자 클래스로 바꿔서 글자와 해당 숫자에 맞게 만드세요.

# --hints--

`stockRegex`은 문자 클래스 하나를 사용해 글자 `e`와 숫자 `3`에 맞춰야 합니다.

```js
assert.match(stockRegex.source, /\[(e3|3e)\]/);
```

`stockRegex`은 문자 클래스 하나를 사용해 글자 `o`와 숫자 `0`에 맞춰야 합니다.
...
`stockRegex`은 `st0ck al3rt`과 일치해야 합니다.
