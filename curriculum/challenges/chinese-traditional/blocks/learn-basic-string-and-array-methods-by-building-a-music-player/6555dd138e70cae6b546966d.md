...
你需要取得目前正在播放的歌曲或播放列表中的第一首歌曲。為此，創建一個 `song` 常數，並使用 OR 運算子（`||`）將其設為 `userData` 的當前歌曲，**或** `userData?.songs` 陣列中的第一首歌曲。

別忘了使用選用串鏈。

# --hints--

你不應該修改現有的 `setPlayButtonAccessibleText` 及其內容。

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

你應該存取 `userData?.currentSong` 或 `userData?.songs[0]`。

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

你應該將 `userData?.currentSong || userData?.songs[0]` 指定給一個 `song` 常數。
