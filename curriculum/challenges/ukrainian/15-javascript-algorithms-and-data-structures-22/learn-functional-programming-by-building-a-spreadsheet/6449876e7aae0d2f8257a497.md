...
Оголосіть стрілкову функцію `rangeFromString`, яка приймає два параметри: `num1` та `num2`. Функція має неявно повернути результат виклику `range` з `num1` та `num2` як аргументами.

Для безпеки розберіть `num1` та `num2` на цілі числа, як ви передаєте їх у `range`.

# --hints--

Оголосіть змінну `rangeFromString` після `rangeRegex`.
...
Використайте `const`, щоб оголосити змінну `rangeFromString`.
...
Змінна `rangeFromString` має бути стрілковою функцією.
...
Функція `rangeFromString` повинна мати `num1` як перший параметр.
...
Функція `rangeFromString` повинна мати `num2` як другий параметр.
...
Функція `rangeFromString` має використати неявне повернення.
...
Функція `rangeFromString` має повернути результат виклику функції `range`.
...
Викличте `parseInt` з `num1` як аргументом та передайте результат до виклику `range`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

Викличте `parseInt` з `num2` як аргументом та передайте результат до виклику `range`.
