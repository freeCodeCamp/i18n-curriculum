...
`@media`에 대해 `only screen`의 `max-width`를 가진 또 다른 `600px` 쿼리를 만드세요. 그 안에 `.text-with-images` 규칙을 만들고 `grid-template-columns` 값을 가진 `1fr` 속성을 주세요.

이렇게 하면 작은 화면에서 하단 텍스트 영역이 한 열로 축소됩니다.

# --hints--

이전 `@media` 쿼리 아래에 `only screen and (max-width: 600px)`에 대한 새로운 `@media` 쿼리를 만들어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

새 `@media` 쿼리는 `.text-with-images` 선택자를 가져야 합니다.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

새 `.text-with-images` 선택자는 `grid-template-columns` 값을 가진 `1fr` 속성을 가져야 합니다.
