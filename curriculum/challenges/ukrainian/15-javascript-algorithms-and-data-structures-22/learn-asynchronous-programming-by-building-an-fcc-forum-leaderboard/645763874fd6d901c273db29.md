...
В межах першого елемента `td` додайте вбудований вираз `${}`. Усередині цього виразу викличте функцію `forumCategory` з аргументом `category_id`.

Тепер ви повинні побачити категорію показаною під кожною темою публікації.

# --hints--

Викличте функцію `forumCategory` з аргументом `category_id`
