...
Отримайте елементи `#next`, `#previous` та `#shuffle` з файлу HTML.

Призначте їх змінним з іменами `nextButton`, `previousButton` та `shuffleButton` відповідно.
...
Використайте `document.getElementById()`, щоб отримати елемент `#next`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

Призначте елемент `#next` до змінної `nextButton`.
...
Використайте `document.getElementById()`, щоб отримати елемент `#previous`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

Призначте елемент `#previous` до змінної `previousButton`.
...
Використайте `document.getElementById()`, щоб отримати елемент `#shuffle`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

Призначте елемент `#shuffle` до змінної `shuffleButton`.
