...
在动画函数中，你需要将每个 `checkpoints` 绘制到画布上。

添加一个 `forEach` 循环，迭代 `checkpoints` 数组。

在回调函数内部，添加一个 `checkpoint` 参数，并在函数主体上调用每个 `checkpoint` 上的 `draw` 方法。

# --hints--

你应该有一个 `forEach` 循环，用于迭代 `checkpoints` 数组。

```js
assert.match(code, /\bcheckpoints\b\s*\.\s*forEach\s*\(/s);
```

你应该在 `forEach` 回调函数中有一个 `checkpoint` 参数。

```js
assert.match(code, /\s*checkpoints\s*\.\s*forEach\s*\(\s*(\(\s*checkpoint\s*\)|checkpoint)\s*=>\s*{?/s);
```

你应该在 `forEach` 回调函数内的每个 `checkpoint` 上调用 `draw` 方法。
