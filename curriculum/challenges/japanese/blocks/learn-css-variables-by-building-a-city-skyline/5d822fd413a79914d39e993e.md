...
最後に、メディアクエリの`:root`セレクター内で、4つの`--window-color`変数すべてを`#777`を使うように再定義してください。完了したら、ウィンドウのサイズを変更して、昼から夜に変わる様子を確認しましょう。

変数は主に色に使われますが、ここではそのように使いました。しかし、変数にはどんな値でも設定でき、どんなプロパティにも使えます。あなたのプロジェクトは素晴らしい仕上がりです！

# --hints--

`--window-color1`を`#777`の値で追加してください。
...
`--window-color2`を`#777`の値で追加してください。
...
`--window-color3`を`#777`の値で追加してください。
...
`--window-color4`を`#777`の値で追加してください。
