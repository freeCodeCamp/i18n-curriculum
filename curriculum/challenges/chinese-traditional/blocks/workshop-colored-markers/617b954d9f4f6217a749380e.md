...
現在，在馬克筆和它們所在的 `container` 元素的邊緣之間添加更多的垂直空間。

在 `.container` CSS 規則中，使用簡寫 `padding` 屬性添加上下內邊距，各爲 `10px`，並將左右內邊距設置爲 `0`。 這類似於你之前使用的簡寫 `margin` 屬性。
...
你的 `.container` CSS 規則應該有一個 `padding` 屬性，設置爲 `10px 0`。
