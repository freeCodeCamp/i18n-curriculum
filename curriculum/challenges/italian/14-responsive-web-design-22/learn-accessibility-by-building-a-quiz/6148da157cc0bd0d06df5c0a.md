...
Per allineare le caselle di input tra loro, crea un nuovo set di regole che seleziona tutti gli elementi `input` e `label` all'interno di un elemento `.info` e imposta la proprietà `display` su `inline-block`.

Inoltre allinea il testo dell'elemento `label` sulla destra.
...
Dovresti dare agli elementi `input` e `label` una proprietà `display` con il valore `inline-block`.
...
Dovresti dare all'elemento `label` una proprietà `text-align` con il valore `right`.
