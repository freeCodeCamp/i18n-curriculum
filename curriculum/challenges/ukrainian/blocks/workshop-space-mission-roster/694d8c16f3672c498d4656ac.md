...
Як і раніше, почніть із валідації вхідних даних всередині функції `swapCrewMembers`. Зокрема, потрібно перевірити індекси астронавтів, яких потрібно поміняти місцями.

Якщо виконується будь-яка з наступних умов:

- `fromIndex` є від’ємним

- `fromIndex` більший або дорівнює `crew.length`

- `toIndex` є від’ємним

- `toIndex` більший або дорівнює `crew.length`

тоді слід вивести в консоль `"Invalid crew indices"` і викликати `return`, щоб вийти з функції.

Ви можете використати логічний оператор АБО `||`, щоб перевірити кілька умов одночасно:
...
Ви повинні перевіряти індекси за допомогою інструкції `if`.
...
Спочатку перевірте індекси, і якщо вони недійсні, виведіть повідомлення `"Invalid crew indices"` перед тим, як вийти з функції за допомогою `return`.
...
Одразу після виведення повідомлення у перевірці валідації викличте `return`, щоб рано вийти з функції.
...
Якщо вхідні дані дійсні, ваша функція `swapCrewMembers` не повинна виводити повідомлення про помилки і не повинна виходити раніше часу.
