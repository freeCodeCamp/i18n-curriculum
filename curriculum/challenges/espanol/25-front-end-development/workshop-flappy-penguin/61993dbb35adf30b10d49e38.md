...
Seleccione el elemento `.penguin-body`, y asígnele un `ancho` de `53%`, y una `altura` de `45%`. Luego, establezca el `fondo` a un gradiente lineal en `45deg`, `rgb(134, 133, 133)` desde `0%`, `rgb(234, 231, 231)` desde `25%`, y `blanco` desde `67%`.
...
Debe dar a `.penguin-body` un `ancho` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

Debe dar a `.penguin-body` una `altura` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

Debe dar a `.penguin-body` un `fondo` de `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, blanco 67%)`.
