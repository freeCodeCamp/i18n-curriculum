...
最後，在你的 `50%` 和 `100%` 選擇器之間創建一個新的 `75%` 選擇器。為這個新的選擇器添加一個 `background-color` 屬性，並設定為 `yellow`。

這樣一來，你的動畫會更加流暢，你的摩天輪也完成了。

# --hints--

你應該在你的 `@keyframes cabins` 規則中創建一個新的 `75%` 選擇器。
