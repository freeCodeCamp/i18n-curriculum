...
2番目の `textarea` 要素の中に `div.answer` 要素を1つネストし、その行数と列数を設定してください。

# --hints--

2番目の `textarea` 要素の中に `div.answer` 要素を1つネストしてください。

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

`textarea` に数値の `rows` 属性を付けてください。
