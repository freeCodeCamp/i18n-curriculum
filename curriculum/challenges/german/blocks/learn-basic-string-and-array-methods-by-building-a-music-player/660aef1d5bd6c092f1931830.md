...
Genau wie reguläre Funktionen können auch Pfeilfunktionen Werte zurückgeben.

Hier ist ein Beispiel für eine Pfeilfunktion, die das Ergebnis der Multiplikation von zwei Zahlen zurückgibt:
...
Erstellen Sie eine neue Variable namens `addTwoNumbers` und weisen Sie ihr eine Pfeilfunktion zu. Diese Pfeilfunktion soll zwei Parameter namens `num1` und `num2` annehmen. Geben Sie im Funktionskörper den Ausdruck zurück, der `num1` und `num2` addiert.

Fügen Sie unter der `addTwoNumbers`-Funktion eine Konsolenausgabe hinzu. Rufen Sie in dieser Konsolenausgabe die `addTwoNumbers`-Funktion auf und übergeben Sie die Zahlen `3` und `4` als Argumente.

Öffnen Sie die Konsole, um die Ausgabe zu sehen.
...
Ihre `addTwoNumbers`-Funktion sollte die Syntax einer Pfeilfunktion verwenden.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Ihre `addTwoNumbers`-Funktion sollte zwei Parameter namens `num1` und `num2` haben.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

Ihre `addTwoNumbers`-Funktion sollte den Ausdruck zurückgeben, der `num1` und `num2` addiert.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

Sie sollten eine `console.log(addTwoNumbers(3, 4));`-Anweisung haben.
