...
Ora puoi definire lo stile del layout della tua griglia. CSS Grid è simile a Flexbox perché ha una proprietà speciale sia per gli elementi genitori sia per quelli figli.

In questo caso, il tuo elemento genitore è l'elemento `main`. Imposta il contenuto in modo che abbia un layout a tre colonne aggiungendo una proprietà `grid-template-columns` con un valore `1fr 94rem 1fr`. Questo creerà tre colonne dove la colonna centrale è larga `94rem`, e la prima e l'ultima colonna occupano entrambe 1 frazione dello spazio rimanente nel contenitore griglia.
