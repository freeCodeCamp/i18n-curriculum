...
ホテルのフィードバックフォームには、食事の評価をユーザーができるようにしてください。

まず、`label`というテキストを持つ`How was the food?`要素を追加してください。その`label`要素には、`for`属性を`"food"`に設定してください。

`label`要素の下に、`select`と`id`が`name`に設定された`"food"`要素を追加してください。

# --hints--

`label`要素に`for`属性が`"food"`に設定されているはずです。

```js
assert.exists(document.querySelector('label[for="food"]'));
```

`label`には`How was the food?`というテキストがあるはずです。

```js
assert.strictEqual(document.querySelector('label[for="food"]')?.textContent.trim(), 'How was the food?');
```

`select`の下に`id`が`"food"`に設定された`label`要素があるはずです。

```js
assert.exists(document.querySelector('label + select#food'));
```

`select`要素には`name`属性が`"food"`に設定されているはずです。
