...
Crea un elemento `div` con la classe `task`. Usa le template string per impostare l'attributo `id` del `div` al `id` che hai destrutturato dai dati dell'attività.

# --hints--

Dovresti creare un elemento `div` con la classe `task`.
...
Il tuo elemento `div` dovrebbe avere un attributo `id` con valore `${id}`.
