...
你的新 `p` 元素應該有一個 `class` 爲 `quote`。

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

你的新 `p` 元素應包含文本 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`。
