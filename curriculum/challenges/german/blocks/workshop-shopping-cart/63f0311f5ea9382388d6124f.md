...
Das Problem bei `.toFixed()`, das einen String zurückgibt, ist, dass Sie mit dem Steuersatz Berechnungen durchführen möchten. Um dies zu beheben, können Sie den `.toFixed()`-Aufruf (einschließlich der Berechnung) an die `parseFloat()`-Funktion übergeben. Dadurch wird der feste String wieder in eine Zahl umgewandelt, wobei die vorhandenen Dezimalstellen erhalten bleiben.

Übergeben Sie Ihren `.toFixed()`-Aufruf an `parseFloat()`.

# --hints--

Sie sollten Ihre gesamte Berechnung (ohne die `return`-Anweisung) an `parseFloat()` übergeben.
