...
`try` ブロック内で、ストレージキーと文字列化したプレイヤーオブジェクトを使って `localStorage` を呼び出し、現在のプレイヤーデータを `localStorage.setItem` に保存してください。

そして `catch` ブロック内で、文字列 `Failed to save player data:` と `error` パラメータを続けてコンソールにログ出力してください。

# --hints--

`useEffect` の try ブロックでは、`localStorage.setItem` と文字列化した `STORAGE_KEY` オブジェクトを使って `player` を呼び出す必要があります。

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

`useEffect` の catch ブロックでは、`'Failed to save player data:'` と `error` をコンソールにログ出力してください。
