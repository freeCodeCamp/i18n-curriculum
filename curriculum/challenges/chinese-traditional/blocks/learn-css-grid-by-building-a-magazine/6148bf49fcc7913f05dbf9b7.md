...
創建 `.author-name a:hover` 選擇器，並為它設定 `background-color` 屬性為 `#306203`。

這將只為 `.author-name` 內的 `a` 元素創建暫留效果，並在背景中顯示原始的 freeCodeCamp 綠色。

# --hints--

你應該有一個 `.author-name a:hover` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

你的 `.author-name a:hover` 選擇器應該設定 `background-color` 屬性為 `#306203`。
