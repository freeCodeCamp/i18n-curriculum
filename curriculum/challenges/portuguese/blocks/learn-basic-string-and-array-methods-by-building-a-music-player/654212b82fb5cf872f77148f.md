...
Dentro do elemento `button`, crie mais dois elementos `span`. 

O primeiro elemento `span` deve ter uma `class` de `playlist-song-artist`. Entre as tags `span`, adicione `${song.artist}`. 

O segundo elemento `span` deve ter um `class` de `playlist-song-duration`. Entre as tags `span`, adicione `${song.duration}`. 

# --hints--

Você não deve modificar o elemento `button` existente e seu conteúdo.
...
Você deve criar um elemento `span` com a classe `playlist-song-artist`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

Seu elemento `span` com a classe `playlist-song-artist` deve interpolar `song.artist` como seu conteúdo.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

Você deve criar um elemento `span` com a classe `playlist-song-duration`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

Seu elemento `span` com a classe `playlist-song-duration` deve interpolar `song.duration` como seu conteúdo.
