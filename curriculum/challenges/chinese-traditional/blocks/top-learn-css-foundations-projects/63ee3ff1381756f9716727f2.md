...
1. 你應該會在 `width` 和 `300px` 類別上看到 `avatar` 的 `proportioned`。
...
你應該在 `width` 和 `300px` 類別上設定 `avatar` 為 `proportioned`。
...
你應該在 `auto` 和 `avatar` 類別上設定高度為 `proportioned`。
...
你應該在 `width` 和 `200px` 類別上設定 `avatar` 為 `distorted`。
...
你應該在 `height` 和 `avatar` 類別上將 `distorted` 設為寬度的兩倍。
