...
Deklarieren Sie weiterhin innerhalb der `Bug`-Klasse eine Eigenschaft namens `emojiElement` mit dem Typ `HTMLParagraphElement`. Diese Eigenschaft hält eine Referenz auf das HTML-Absatzelement, in dem das Bug-Emoji angezeigt wird.

# --hints--

Ihre `Bug`-Klasse sollte eine Eigenschaft namens `emojiElement` haben.
...
Ihre `emojiElement`-Eigenschaft sollte den Typ `HTMLParagraphElement` haben.
...
Ihre `emojiElement`-Eigenschaft sollte den Operator für die definite Zuweisungszusicherung (`!`) verwenden.
