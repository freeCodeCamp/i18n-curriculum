...
當歌曲顯示在頁面上時，它應該顯示每首歌曲的標題、藝術家、時長以及刪除按鈕。

在接下來的幾個步驟中，你將學習如何使用 <dfn>map()</dfn> 方法爲每首歌曲添加新的 HTML。 下一步將更詳細地解釋該方法。

首先使用 `const` 聲明一個名爲 `songsHTML` 的變量，並將其分配給 `array.map()`。

# --hints--

你應該有一個名爲 `songsHTML` 的 `const` 變量。

```js
assert.match(code, /const\s+songsHTML/);
```

你應該將 `array.map()` 分配給你的 `songsHTML`。
