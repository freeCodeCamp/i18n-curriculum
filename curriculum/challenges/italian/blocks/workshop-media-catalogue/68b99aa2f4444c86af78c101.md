...
Rientra il tuo loop `for` esistente di un livello in modo che venga eseguito solo quando il catalogo contiene film. Poi, sostituisci `self.items` con `movies` per iterare solo sui film.

# --hints--

Dovresti rientrare il loop `for` esistente di un livello e sostituire `self.items` con `movies` per iterare solo sui film.
