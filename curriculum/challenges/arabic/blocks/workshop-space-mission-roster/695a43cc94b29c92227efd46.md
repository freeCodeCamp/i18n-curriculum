...
أضفت استدعاء `console.log()` إلى السكربت للتحقق من مصفوفة `EVAChunks`، والآن لم يعد هذا الاستدعاء ضروريًا. للحفاظ على وحدة التحكم نظيفة للخطوات التالية، احذف كتلة `for loop` التي تسجل البيانات من `EVAChunks`.

# --hints--

يجب حذف كتلة `for loop` التي تسجل البيانات من `EVAChunks`. احذف الكتلة كاملة التي تحتوي على كلا الحلقتين.
