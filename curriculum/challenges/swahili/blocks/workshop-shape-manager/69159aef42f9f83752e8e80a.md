...
Sasa, fanya wito wa kitendakazi cha `chooseShape()` kwa `val.value` kuonyesha sehemu za ingizo zinazofaa, kisha fanya wito wa `updateResult()` kuhesabu upya na kuonyesha eneo.

# --hints--

Unapaswa kufanya wito wa kitendakazi cha `chooseShape` kwa `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

Unapaswa kufanya wito wa kitendakazi cha `updateResult()`.
