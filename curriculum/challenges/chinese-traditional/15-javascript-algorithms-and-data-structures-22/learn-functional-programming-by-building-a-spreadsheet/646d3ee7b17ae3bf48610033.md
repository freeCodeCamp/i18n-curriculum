...
`infixToFunction[operator]` 返回一個函數。 直接調用該函數，傳遞 `arg1` 和 `arg2` 作爲參數。

# --hints--

你的回調函數應該返回調用 `infixToFunction[operator]` 的結果。
...
你應該將 `arg1` 作爲第一個參數傳遞給 `infixToFunction[operator]` 調用。
...
你應該將 `arg2` 作爲第二個參數傳遞給 `infixToFunction[operator]` 調用。
