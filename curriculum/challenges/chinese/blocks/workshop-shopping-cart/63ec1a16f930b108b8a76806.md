...
接下来是获取你的总数。 获取 `#total-items`、`#subtotal`、`#taxes` 和 `#total` 元素。 将它们分别存储在名为 `totalNumberOfItems`、`cartSubTotal`、`cartTaxes` 和 `cartTotal` 的变量中。

# --hints--

你应该使用 `document.getElementById()` 来获取 `#total-items` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)total-items\1\s*\)/);
```

你应该将 `#total-items` 元素分配给名为 `totalNumberOfItems` 的变量。 记住使用 `const` 来声明变量。

```js
assert.match(code, /const\s+totalNumberOfItems\s*=\s*document\.getElementById\(\s*('|"|`)total-items\1\s*\)/);
```

你应该使用 `document.getElementById()` 来获取 `#subtotal` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)subtotal\1\s*\)/);
```

你应该将 `#subtotal` 元素分配给名为 `cartSubTotal` 的变量。 记住使用 `const` 来声明变量。

```js
assert.match(code, /const\s+cartSubTotal\s*=\s*document\.getElementById\(\s*('|"|`)subtotal\1\s*\)/);
```

你应该使用 `document.getElementById()` 来获取 `#taxes` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)taxes\1\s*\)/);
```

你应该将 `#taxes` 元素分配给名为 `cartTaxes` 的变量。 记住使用 `const` 来声明变量。

```js
assert.match(code, /const\s+cartTaxes\s*=\s*document\.getElementById\(\s*('|"|`)taxes\1\s*\)/);
```

你应该使用 `document.getElementById()` 来获取 `#total` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)total\1\s*\)/);
```

你应该将 `#total` 元素分配给名为 `cartTotal` 的变量。 记住使用 `const` 来声明变量。
