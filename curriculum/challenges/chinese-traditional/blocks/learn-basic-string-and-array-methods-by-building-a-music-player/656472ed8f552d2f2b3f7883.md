...
為你的程式碼添加一個 `else` 區塊來控制歌曲當前的播放時間。這讓你可以從暫停的點繼續播放當前歌曲。

在 `else` 區段內，將 `audio` 物件的 `currentTime` 屬性設定為儲存在 `userData?.songCurrentTime` 的值。
...
你應該在 `if` 述語的 `else` 區塊中將 `audio.currentTime` 設定為 `userData?.songCurrentTime`。
