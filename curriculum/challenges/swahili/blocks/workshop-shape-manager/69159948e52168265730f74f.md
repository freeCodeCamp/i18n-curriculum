...
Unahitaji njia ya kupata kwa usalama kipengele cha select na kulinda dhidi ya thamani za `null`. Kwa ajili hiyo, tengeneza thabiti ya `val` yenye thamani `e.currentTarget` na uiweke aina `HTMLSelectElement` kwa kutumia neno kuu la `as`.

Chini ya thabiti hiyo, tumia tamko la `if` kuangalia kama `val` ni batili kwa kuikataa. Katika sehemu ya `if`, rudisha `target value not found`.

# --hints--

Unapaswa kuwa na thabiti iitwayo `val`.

```js
assert.match(code, /const\s+val/)
```

Thabiti yako ya `val` inapaswa kuwa na thamani ya `e.currentTarget` yenye aina `HTMLSelectElement` kwa kutumia neno kuu la `as`.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

Unapaswa kuwa na tamko la `if` linaloangalia kama `val` ni null.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

Tamko lako la `if` linapaswa kurudisha `target value not found`.
