...
ステップ1で`Load More Authors`ボタンを選択し、`loadMoreBtn`に代入したことを覚えていますか。

`addEventListener`を使って`"click"`に`loadMoreBtn`イベントリスナーを追加してください。また、ボタンがクリックされるたびに実行する`fetchMoreAuthors`関数の参照を渡してください。

その後、ボタンをクリックするとさらに8人の著者が表示されるはずです。
...
`addEventListener()`変数に対して`loadMoreBtn`メソッドを呼び出す必要があります。
...
イベントリスナーは実行する関数として`fetchMoreAuthors`を受け取る必要があります。
