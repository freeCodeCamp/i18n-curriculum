...
`#stats` 要素と同様に、`#monsterStats` 要素には `span` 要素が 2 つ必要です。 それらの要素にクラス `stat` を設定し、1 つ目の要素のテキストを `Monster Name:` に、2 つ目のテキストを `Health:` にしてください。 各テキストの後ろに、 空の `span` 要素が 1 つネストされている `strong` 要素を追加してください。 Give the first inner `span` element an `id` of `monsterName` and the second inner `span` element an `id` of `monsterHealth`.
...
両方の新しい `span` 要素には `stat` という値の `class` が必要です。
...
`strong` と `span` 要素はテキストの後ろに置く必要があります。
