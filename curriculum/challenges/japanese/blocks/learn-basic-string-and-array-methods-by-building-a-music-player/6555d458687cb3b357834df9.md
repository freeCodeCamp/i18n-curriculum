...
`userData?.currentSong?.title` と `userData?.currentSong?.artist` のプロパティにアクセスし、それぞれ `currentTitle` と `currentArtist` の変数に代入してください。

# --hints--

`title` オブジェクトから `currentSong` の `userData` にアクセスしてください。オプショナルチェーンを使うのを忘れないでください。

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

`title` の `currentSong` を `currentTitle` 定数に代入してください。

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

`artist` オブジェクトから `currentSong` の `userData` にアクセスしてください。オプショナルチェーンを使うのを忘れないでください。

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

`artist` の `currentSong` を `currentArtist` 定数に代入してください。
