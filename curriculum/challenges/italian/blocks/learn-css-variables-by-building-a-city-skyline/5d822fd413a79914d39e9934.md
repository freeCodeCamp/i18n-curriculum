...
Dovresti impostare al `repeating-linear-gradient` una direzione `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Dovresti impostare al `repeating-linear-gradient` un primo colore `--building-color3` da `0%` a `10%`.
...
Dovresti impostare al `repeating-linear-gradient` un secondo colore `transparent` da `10%` a `30%`.
