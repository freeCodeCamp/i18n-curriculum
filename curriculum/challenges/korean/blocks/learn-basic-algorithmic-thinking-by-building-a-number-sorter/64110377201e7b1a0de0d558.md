...
`sortedValues` 변수를 `selectionSort` 대신 `bubbleSort`을 호출한 결과로 업데이트하세요.

# --hints--

`sortedValues` 변수에 `selectionSort` 배열을 인수로 하여 `inputValues`를 호출한 값을 할당해야 합니다.

```js
assert.match(code, /const\s+sortedValues\s*=\s*selectionSort\s*\(\s*inputValues\s*\)/);
```

코드에서 `bubbleSort`를 호출하면 안 됩니다.
