...
`PlayerCard` 内で参照されている `FootballPlayerCard` に、`player` プロップを受け取り、以下のプロパティを持つオブジェクトを渡してください。
...
`player` 内の `PlayerCard` に `FootballPlayerCard` プロップを渡す必要があります。
...
`player` プロップには `name` プロパティがあり、その値は `'PELE'` である必要があります。
...
`player` プロップには `overallRating` プロパティがあり、その値は `98` である必要があります。
...
`player` プロップには `position` プロパティがあり、その値は `'ST'` である必要があります。
...
`player` プロップには `club` プロパティがあり、その値は `'Santos FC'` である必要があります。
...
`player` プロップには Pele の画像 URL を持つ `imageUrl` プロパティが必要です。
...
`player` プロップには `pac` プロパティがあり、その値は `97` である必要があります。
...
`player` プロップには `sho` プロパティがあり、その値は `98` である必要があります。
...
`player` プロップには `pas` プロパティがあり、その値は `83` である必要があります。
...
`player` プロップには `dri` プロパティがあり、その値は `99` である必要があります。
...
`player` プロップには `def` プロパティがあり、その値は `41` である必要があります。
...
`player` プロップには `phy` プロパティがあり、その値は `75` である必要があります。
