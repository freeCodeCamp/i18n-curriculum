...
`.push()`를 사용해 `quotient` 변수를 `quotients` 배열에 추가하세요. 또한 `remainder` 변수를 `remainders` 배열에 추가하세요.

# --hints--

`.push()` 반복문 안에서 `quotients` 배열에 `while` 메서드를 사용하세요.

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*quotients\.push\(/);
```

`.push()` 배열에 `quotient`를 추가하려면 `quotients` 메서드를 사용하세요.

```js
assert.match(String(decimalToBinary), /quotients\.push\(\s*quotient\s*\)\s*;?/);
```

`.push()` 반복문 안에서 `remainders` 배열에 `while` 메서드를 사용하세요.

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*remainders\.push\(/);
```

`.push()` 배열에 `remainder`를 추가하려면 `remainders` 메서드를 사용하세요.
