...
حدّث `healthText.innerText` و `monsterHealthText.innerText` ليصبحا مساويين لـ `health` و `monsterHealth`.

# --hints--

يجب تحديث `healthText.innerText`.

```js
assert.match(attack.toString(), /healthText\.innerText/);
```

يجب تعيين `healthText.innerText` إلى `health`.

```js
assert.match(attack.toString(), /healthText\.innerText\s*=\s*health/);
```

يجب تحديث `monsterHealthText.innerText`.

```js
assert.match(attack.toString(), /monsterHealthText\.innerText/);
```

يجب تعيين `monsterHealthText.innerText` إلى `monsterHealth`.
