...
في هذه المرحلة، تريد فقط عرض النتائج للعقد التي ليست عقدة البداية ويمكن الوصول إليها منها.

أضف شرطًا يتحقق مما إذا كان `node_no` يساوي `start_node` **أو** إذا كان `distances[node_no]` يساوي `INF`.

إذا تحقق أي من الشرطين، استخدم `continue` لتخطي التكرار التالي في الحلقة.

# --hints--

يجب أن تضيف عبارة شرطية داخل الحلقة.
...
يجب أن تتحقق عبارة `if` مما إذا كان `node_no` يساوي `start_node` *أو* إذا كان `distances[node_no]` يساوي `INF`.
...
يجب أن تستخدم `continue` لتخطي التكرار التالي في الحلقة.
