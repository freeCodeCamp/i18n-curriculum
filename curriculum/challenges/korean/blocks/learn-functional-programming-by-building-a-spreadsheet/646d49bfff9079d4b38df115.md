...
`has2` 속성을 추가하세요. 이 속성은 `nums` 배열에 `2`가 값으로 있는지 여부를 반환해야 하며, `increment` 속성은 모든 값이 1씩 증가된 `nums`를 반환해야 합니다.

# --hints--

`spreadsheetFunctions` 객체에 `has2` 속성이 있어야 합니다.
