...
Você pode melhorar a legibilidade e a manutenibilidade do código refatorando o listener do evento `submit` em duas funções separadas.A primeira função pode ser usada para adicionar os valores de entrada a `taskData`, enquanto a segunda função pode ser responsável por adicionar as tarefas ao DOM.

Use a sintaxe de seta para criar uma função `addOrUpdateTask`.Então mova a variável `dataArrIndex`, o objeto `taskObj` e a declaração `if` para dentro da função `addOrUpdateTask`.
