...
入力やテキストエリアに入力されたテキストと、そのプレースホルダーが見えるように、色を設定してください。

`input, input::placeholder, textarea` を選択し、`color` を `whitesmoke` に設定してください。

# --hints--

`input`、`input::placeholder`、`textarea` を対象にしてください。
