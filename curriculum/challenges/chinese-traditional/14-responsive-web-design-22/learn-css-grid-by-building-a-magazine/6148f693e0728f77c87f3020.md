...
你應該有一個新的 `@media` 查詢，對應 `only screen` 且 `max-width` 爲 `550px`。 這應該在你的前兩個媒體查詢之後。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

你的新 `@media` 規則應該有一個 `.hero-title` 選擇器、一個 `.hero-subtitle, .author, .quote, .list-title` 選擇器、一個 `.social-icons` 選擇器和一個 `.text` 選擇器。 這些選擇器應按此順序排列。
