...
Su Doku (japanisch für *number place*) ist der Name eines beliebten Rätselkonzepts. Sein Ursprung ist unklar, aber die Anerkennung gebührt Leonhard Euler, der eine ähnliche und wesentlich schwierigere Rätselidee namens Lateinische Quadrate erfand. Das Ziel von Su Doku-Rätseln ist es jedoch, die Leerstellen (oder Nullen) in einem 9 mal 9 Raster so zu ersetzen, dass jede Zeile, jede Spalte und jedes 3 mal 3 Feld jede der Ziffern von 1 bis 9 enthält. Unten sehen Sie ein Beispiel für ein typisches Start-Rätselraster und dessen Lösungsraster.
...
Ein gut konstruiertes Su Doku-Rätsel hat eine eindeutige Lösung und kann durch Logik gelöst werden, obwohl es manchmal notwendig sein kann, „Raten und Testen“-Methoden anzuwenden, um Optionen auszuschließen (darüber gibt es viele unterschiedliche Meinungen). Die Komplexität der Suche bestimmt die Schwierigkeit des Rätsels; das obige Beispiel gilt als einfach, weil es durch direkte, klare Schlussfolgerungen gelöst werden kann.

Das `puzzlesArr`-Array enthält verschiedene Su Doku-Rätselstrings mit unterschiedlichem Schwierigkeitsgrad, aber alle mit eindeutigen Lösungen.

Indem Sie alle Rätsel in `puzzlesArr` lösen, finden Sie die Summe der dreistelligen Zahlen, die in der oberen linken Ecke jedes Lösungsrasters stehen; zum Beispiel ist 483 die dreistellige Zahl, die in der oberen linken Ecke des obigen Lösungsrasters steht.
