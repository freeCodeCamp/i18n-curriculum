...
`kingBooks` のテストコードで `findByAuthor` が正しく動作することが確認できました。`groupByDecade` に進む前に、コンソールの出力を整理して削除してください。

`const kingBooks = ...` の行、`console.log(kingBooks.length)`、および本のタイトルと年を表示している `for` のループを削除してください。

# --hints--

`kingBooks` 変数、`console.log(kingBooks.length)` 文、`for` のループはすべて削除する必要があります。
