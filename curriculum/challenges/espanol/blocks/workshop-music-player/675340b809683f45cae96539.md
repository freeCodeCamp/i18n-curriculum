...
Si presionas el botón anterior cuando no se está reproduciendo una canción, se reproducirá la primera canción. Para solucionar esto, agrega una declaración `if` al inicio de la función `playPreviousSong`. Verifica si la canción actual es `null` y detén la ejecución de la función con una declaración `return`.

# --hints--

Deberías agregar una declaración `if` que verifique si `userData.currentSong === null` y detenga la ejecución de la función con una declaración `return` dentro de tu función `playPreviousSong`.
