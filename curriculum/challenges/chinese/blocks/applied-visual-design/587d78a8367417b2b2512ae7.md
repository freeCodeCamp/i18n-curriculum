...
给 id 为 `ball1` 和 `ball2` 的元素分别添加 `animation-timing-function` 属性，将 `#ball1` 设为 `linear`，将 `#ball2` 设为 `ease-out`。 注意观察这些元素在动画过程中的移动差异，但由于它们共享相同的 2 秒 `animation-duration`，它们会同时结束。
