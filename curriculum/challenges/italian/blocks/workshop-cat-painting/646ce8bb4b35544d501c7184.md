...
All'interno dell'elemento `.cat-ears`, crea due elementi `div` con le classi `cat-left-ear` e `cat-right-ear` rispettivamente.
...
Dovresti creare due elementi `div` all'interno dell'elemento `.cat-ears`.

```js
assert.lengthOf(document.querySelectorAll('.cat-ears div'), 2);
```

Il primo elemento `div` dovrebbe avere la classe `cat-left-ear`.

```js
assert.isTrue(document.querySelectorAll('.cat-ears div')[0]?.classList.contains('cat-left-ear'));
```

Il secondo elemento `div` dovrebbe avere la classe `cat-right-ear`.
