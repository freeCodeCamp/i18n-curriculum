...
`map()` 메서드는 배열을 순차적으로 반복하며 새로운 배열을 반환하는 데 사용됩니다. 기존 배열의 값을 기반으로 새 배열을 만들고 싶을 때 유용합니다. 예를 들어:
...
`map()` 메서드는 함수를 인수로 받는다는 점에 주목하세요. 이것을 <dfn>콜백 함수</dfn>라고 하며, 다른 함수에 인수로 전달되는 함수입니다. 위 예제에서 콜백 함수는 `(number) => number * 2`이고, `numbers` 배열의 각 요소에 대해 실행됩니다. 그런 다음 `map()` 메서드는 결과를 담은 새 배열을 반환합니다.

`map()` 메서드에 콜백 함수를 전달하세요. 콜백 함수는 `song`를 매개변수로 받고, 화살표 함수 구문을 사용하며, 함수 본문은 비워 두세요.

# --hints--

`map()` 메서드의 콜백 함수는 화살표 함수 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

`map()` 메서드의 콜백 함수에는 `song` 매개변수가 있어야 합니다.
