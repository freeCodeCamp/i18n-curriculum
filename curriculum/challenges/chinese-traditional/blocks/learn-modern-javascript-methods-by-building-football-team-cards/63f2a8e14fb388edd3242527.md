...
為你的 `case` 添加一個新的 `"defender"`，該 `case` 會檢查 `player.position` 是否相等於 `"defender"`，並遵循前一步驟的相同樣式。

# --hints--

你應該在你的 `switch` 述語中為 `"defender"` 添加一個新的 `case`。

```js
assert.match(code, /\s*case\s*('|"|`)\s*defender\s*\1\s*:\s*/)
```

在你的 `"defender"` `case` 中，你應該呼叫 `setPlayerCards` 函式（程式）。
...
你的 `filter()` 回呼應該隱式地傳回檢查 `player.position` 是否為 `"defender"` 的結果。
