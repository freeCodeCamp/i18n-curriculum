...
請記住，`grid-column` 屬性決定元素開始和結束的行。當你不確定你的網格會有多少行，但你想讓元素停止在最後一行時，可以將結束行設為 `-1`。

創建一個 `.hero` 選擇器並為它設定一個 `grid-column` 屬性，值為 `1 / -1`。這會告訴該元素跨越整個網格的寬度。
...
你的 `.hero` 選擇器應該有一個 `grid-column` 屬性設定為 `1 / -1`。
