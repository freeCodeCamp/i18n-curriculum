...
现在你需要缩小这些元素的尺寸。 Give your `span[class~="sr-only"]` selector a `width` and `height` property set to `1px`.

# --hints--

`span[class~="sr-only"]` 选择器应该有一个 `width` 属性设置为 `1px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('width') === '1px');
```

`span[class~="sr-only"]` 选择器应该有一个 `height` 属性设置为 `1px`。
