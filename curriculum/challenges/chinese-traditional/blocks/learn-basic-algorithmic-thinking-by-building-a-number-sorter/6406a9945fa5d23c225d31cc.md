...
為了在撰寫程式碼時測試它，請將事件監聽器掛載到你的 `sortButton` 元素。它應該監聽 `"click"` 事件，並將 `sortInputArray` 作為回呼。

# --hints--

你應該在你的 `sortButton` 元素上呼叫 `.addEventListener()` 方法。
...
你的 `.addEventListener()` 方法應該將 `sortInputArray` 作為回呼。請記得傳遞函式的引用，而不是函式的呼叫。
