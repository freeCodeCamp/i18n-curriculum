...
使用以下規則完成函式，修改傳遞給函式的物件。

-   你的函式必須始終傳回整個 `records` 物件。
-   如果 `value` 是空字串，則從相簿中刪除指定的 `prop` 屬性。
-   如果 `prop` 不是 `tracks` 且 `value` 不是空字串，則將該 `value` 指定給該專輯的 `prop`。
-   如果 `prop` 是 `tracks` 且 `value` 不是空字串，但專輯沒有 `tracks` 屬性，則創建一個空陣列並將 `value` 添加到其中。
-   如果 prop 是 `tracks` 且 `value` 不是空字串，將 `value` 添加到專輯現有的 `tracks` 陣列末端。

**注意：** 測試中使用了 `recordCollection` 物件的複本。你不應該直接修改 `recordCollection` 物件。
