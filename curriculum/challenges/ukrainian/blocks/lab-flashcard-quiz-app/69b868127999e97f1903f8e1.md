...
title: Створіть застосунок для тестування з картками
challengeType: 25
...
У цій лабораторній роботі ви створите застосунок, який відображає та зберігає дані карток для запам’ятовування, які можна буде пізніше отримати.

**Мета:** Виконайте наведені нижче історії користувача та пройдіть усі тести, щоб завершити лабораторну роботу.

**Історія користувача:**

1) Ви повинні мати HTML-елемент з ідентифікатором `flashcard`.  
2) Ви повинні мати інтерфейс з назвою `FlashCard`.  
3) Інтерфейс `FlashCard` повинен містити такі властивості `questionText` і `questionAnswer`, обидві типу `string`.  
4) Ви повинні мати колекцію елементів `FlashCard` з назвою `currentCards`.  
5) Коли на елемент `#flashcard` натискають, картка повинна отримати клас `flipped`.  
6) Ви повинні мати елемент з ідентифікатором `delete-btn`.  
7) Коли на елемент `#delete-btn` натискають, він повинен видалити поточну картку та відобразити дані попередньої картки.  
8) Ви повинні створити форму введення з ідентифікатором `entry-form`, щоб мати змогу додавати більше карток до колекції `currentCards` при відправленні.  
9) Два елементи всередині форми повинні мати ідентифікатори `front-text` і `back-text` відповідно.  
10) Ви повинні створити та викликати `InvalidUserInputError`, коли текст питання або відповідь на питання порожні у формі введення.

# --hints--

Ви повинні мати HTML-елемент з ідентифікатором `flashcard`.
...
Ви повинні мати інтерфейс `FlashCard`.
...
Інтерфейс `FlashCard` повинен мати властивість `questionText` типу `string`.
...
Інтерфейс `FlashCard` повинен мати властивість `questionAnswer` типу `string`.
...
Ви повинні мати колекцію елементів `FlashCard` з назвою `currentCards` типу `FlashCard[]`.
...
Коли на елемент `#flashcard` натискають уперше, елемент повинен отримати клас `flipped`.
...
Ви повинні мати елемент з ідентифікатором `delete-btn`.
...
Коли на `delete-btn` натискають, елемент картки повинен бути видалений з колекції `currentCards`.
...
Коли на `delete-btn` натискають, має відобразитися попередні дані картки.
...
Ви повинні створити форму введення з ідентифікатором `entry-form`.
...
У формі повинні бути два елементи `textarea` з ідентифікаторами `front-text` і `back-text` відповідно.
...
Повинно бути викинуто `InvalidUserInputError`, коли текст питання або відповідь на питання порожні у формі введення.
