...
All'interno della funzione `playSong`, imposta la proprietà `audio.src` uguale a `song.src`. Questo indica all'elemento audio dove trovare i dati audio per la canzone selezionata.
...
Non dovresti modificare la funzione `playSong` esistente e il suo contenuto.
...
Dovresti impostare `audio.src` su `song.src`.
...
Dovresti impostare `audio.title` su `song.title`.
