...
| Referências | Referências fortes | Referências fracas |
| Iteração | Suporta iteração com `forEach` e laços | Não dá suporte à iteração |
| Métodos e Propriedades | `add()`, `delete()`, `has()`, `keys()`, `values()`, `size` e mais | Somente `add()`, `delete()` e `has()` |
| Caso de uso | Coleção de propósito geral de valores únicos e eliminação de duplicatas de arrays | Acompanhamento eficiente da memória das referências de objetos |
