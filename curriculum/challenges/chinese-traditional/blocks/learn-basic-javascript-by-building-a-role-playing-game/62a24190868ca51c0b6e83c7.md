...
目前，使用 `display` 屬性隱藏你的 `#monsterStats` 元素。不要更改其他任何樣式。

# --hints--

你應該有一個 `#monsterStats` 選擇器。
...
你的 `#monsterStats` 選擇器應該有一個 `display` 屬性為 `none`。
...
你的 `#monsterStats` 選擇器應該有 `1px solid #0a0a23` 的 `border`。
...
你的 `#monsterStats` 選擇器應該有 `5px` 的內距。
...
你的 `#monsterStats` 選擇器應該有 `color` 為 `#ffffff`。
...
你的 `#monsterStats` 選擇器應該有 `background-color` 為 `#c70d0d`。
