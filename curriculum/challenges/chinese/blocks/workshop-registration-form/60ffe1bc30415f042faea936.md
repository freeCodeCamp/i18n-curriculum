...
设置 `input` 和 `textarea` 元素的 `background-color` 为 `#0a0a23`，使其更好地融入背景。 然后设置其边框为 `1px`、`solid`，边框颜色为 `#0a0a23`。

# --hints--

你应该使用逗号分隔的选择器列表来选择 `input` 和 `textarea` 元素。
...
你应该给 `input` 和 `textarea` 元素一个 `background-color`，值为 `#0a0a23`。
...
你应该给 `input` 和 `textarea` 元素一个 `1px`、`solid` 的边框， 颜色是 `#0a0a23`。
