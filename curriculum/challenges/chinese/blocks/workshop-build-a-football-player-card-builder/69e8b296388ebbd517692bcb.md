...
`FootballPlayerCard` 目前只渲染了最基本的卡片。要构建完整的页面，请添加一个 `div` 元素，`className` 为 `page`。

在该 `div` 内，添加一个 `header` 元素，`className` 为 `header`。在 `header` 内，添加一个 `div`，`className` 为 `header-inner`。在 `header-inner` 元素内，分别添加两段文本 `Football Card Builder` 和 `Customize your player card`。这两段文本还应分别具有 `className` 的 `header-title` 和 `header-subtitle`。

在闭合的 `header` 元素之后，但仍在 `className` 为 `page` 的 `div` 内，移入 `PlayerCard` 组件及其 prop，作为临时占位符，直到添加主布局。

# --hints--

你的 `FootballPlayerCard` 应该渲染一个具有 `className` 的 `div`，className 为 `page`。
...
你的 `div`，其 `className` 为 `page`，应包含一个具有 `className` 的 `header`，className 为 `header`。
...
你的 `header` 应包含一个具有 `className` 的 `div`，className 为 `header-inner`。
...
你的 `header-inner` 元素应包含一个具有 `className` 的 `p`，className 为 `header-title`，以及文本 `Football Card Builder`。
...
你的 `header-inner` 元素应包含一个具有 `className` 的 `p`，className 为 `header-subtitle`，以及文本 `Customize your player card`。
...
你的 `PlayerCard` 组件应渲染在 `div.page` 内。
