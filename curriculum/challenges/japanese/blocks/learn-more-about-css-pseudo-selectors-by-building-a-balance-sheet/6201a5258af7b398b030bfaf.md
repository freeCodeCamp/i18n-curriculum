...
ドル金額が現在ずれています。`td` 要素内の `tr.data` 要素を対象にするセレクターを作成してください。テキストを縦方向に上揃え、横方向に右揃えにし、パディングを `0.3rem 0.25rem 0` に設定してください。

# --hints--

`tr.data td` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td'));
```

`tr.data td` セレクターには `vertical-align` プロパティを `top` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td')?.getPropertyValue('vertical-align') === 'top');
```

`tr.data td` セレクターには `text-align` プロパティを `right` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td')?.getPropertyValue('text-align') === 'right');
```

`tr.data td` セレクターには `padding` プロパティを `0.3rem 0.25rem 0` に設定してください。
