...
Usa el evento `oninput` en `shapeTypeSelect` y configúralo a `handleShapeSelect`. Esto hará que la función se ejecute cada vez que cambie la selección de la forma.

Después de hacer eso, deberías poder ver los campos de entrada de cada forma. Pero los cálculos aún no funcionarán, y eso es lo que manejarás a continuación.

# --hints--

Debes configurar `shapeTypeSelect.oninput` a `handleShapeSelect`.
