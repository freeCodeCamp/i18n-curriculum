...
最後に、現在のノードの結果を出力してください。

出力には2つの情報を表示する必要があります。`start_node` から `node_no` までの距離と、そこに至るまでの経路です。

`f-string` を使って、距離と完全な経路の両方を次の形式で表示してください。
...
- `[starting node]` は `start_node` です
- `[node number]` は `node_no` です
- `[distance]` は `start_node` から `node_no` までの距離です
- `[path]` は `node_no` に至るまでの経路です

# --hints--

`print(f'\n{start_node}-{node_no} distance: {distances[node_no]}\nPath: {path}')` を使って正しい出力を表示してください。
