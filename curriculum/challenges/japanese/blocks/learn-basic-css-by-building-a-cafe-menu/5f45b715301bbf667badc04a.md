...
メニューのテキスト `CAMPER CAFE` は、メニューの下部にある住所のスペースとは異なる上部のスペースがあります。これはブラウザが `h1` 要素にデフォルトの上マージンを設定しているためです。

`h1` 要素の上マージンを `0` に変更して、すべての上マージンを取り除いてください。
...
`h1` 要素は `margin-top` を `0` に設定してください。
