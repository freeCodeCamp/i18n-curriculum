...
Якщо в списку відтворення немає наступної пісні, скористайтеся блоком `else`, щоб скинути ключ `currentSong` з `userData` на `null`, а її властивість `songCurrentTime` на `0`.

# --hints--

Не змінюйте наявну інструкцію `if` та її вміст.
...
Використайте точкову нотацію, щоб встановити властивість `currentSong` з `userData` на `null` в межах блоку `else` інструкції `if`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

Використайте точкову нотацію, щоб встановити властивість `songCurrentTime` з `userData` на `0` в межах блоку `else` інструкції `if`.
