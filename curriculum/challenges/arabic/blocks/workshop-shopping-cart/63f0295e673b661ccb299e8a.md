...
تحتاج إلى طريقة للوصول إلى العدد الإجمالي للعناصر في العربة. أفضل طريقة لذلك هي إضافة طريقة أخرى إلى صنف `ShoppingCart` بدلاً من الوصول مباشرة إلى مصفوفة `items`.

أضف طريقة `getCounts` إلى صنف `ShoppingCart`. يجب أن تُرجع عدد العناصر في مصفوفة `items`.

# --hints--

يجب أن يحتوي صنف `ShoppingCart` على طريقة `getCounts`.
...
يجب أن تُرجع طريقة `getCounts` عدد العناصر في مصفوفة `items`.
