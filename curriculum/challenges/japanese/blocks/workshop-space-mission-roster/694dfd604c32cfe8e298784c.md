...
ミッションコントロールから、`squad` 配列内の2人の宇宙飛行士の位置を入れ替えるよう依頼されました。具体的には、インデックス `2` と `5` にいるメンバーです。`swapCrewMembers()` 関数を使って入れ替えを行い、結果を `updatedSquad` という新しい変数に保存してください。

# --hints--

`swapCrewMembers()` を使って、インデックス `squad` と `2` にいる `5` メンバーを入れ替え、結果の配列を `updatedSquad` という新しい変数に保存してください。
