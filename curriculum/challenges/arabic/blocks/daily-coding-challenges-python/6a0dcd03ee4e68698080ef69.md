...
أعطِ مصفوفة ثنائية الأبعاد حيث تحتوي كل مصفوفة داخلية (بهذا الترتيب) على اسم فكرة، تقدير متفائل، تقدير واقعي، وتقدير متشائم (بالأيام)، أرجع مصفوفة بأسماء الأفكار مرتبة حسب الوقت المتوقع لإكمالها، الأقصر أولاً.

احسب الوقت المتوقع لإكمال كل فكرة باستخدام الصيغة التالية:

- `expected = ((optimistic + 4 * realistic + pessimistic) / 6) * length of idea name`

# --hints--

`analyze_ideas([["Add logging", 2, 5, 15], ["SEO optimization", 4, 8, 20], ["Fix bug", 1, 3, 5]])` يجب أن تُرجع `["Fix bug", "Add logging", "SEO optimization"]`.
...
`analyze_ideas([["Dark mode", 1, 3, 8], ["Real-time collaboration feature", 6, 12, 20], ["Add tooltip", 1, 2, 4]])` يجب أن تُرجع `["Add tooltip", "Dark mode", "Real-time collaboration feature"]`.
...
`analyze_ideas([["Update user profile page", 3, 7, 14], ["Add pagination", 2, 5, 10], ["Add tags", 2, 3, 6], ["Fix login bug", 1, 4, 8]])` يجب أن تُرجع `["Add tags", "Fix login bug", "Add pagination", "Update user profile page"]`.
...
`analyze_ideas([["Migrate database", 14, 25, 40], ["Add chat assistant", 8, 15, 24], ["Redesign onboarding flow", 3, 7, 13], ["Add language support", 6, 11, 18]])` يجب أن تُرجع `["Redesign onboarding flow", "Add language support", "Add chat assistant", "Migrate database"]`.
