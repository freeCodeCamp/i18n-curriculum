...
أنشئ محدِّد `.flex-container` واضبط خاصية `display` على القيمة `flex`.

# --hints--

يجب أن يكون لديك محدِّد `.flex-container`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.flex-container'));
```

اضبط خاصية `display` على القيمة `flex` داخل محدِّد `.flex-container`.
