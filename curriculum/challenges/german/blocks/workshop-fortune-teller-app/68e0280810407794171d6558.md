...
Fügen Sie nun ein `div` mit der Klasse `img-loader` hinzu. Erstellen Sie danach ein `img`-Element und geben Sie ihm eine bedingte URL für das Bild mit ``${img ? `${CDN_URL}/${img}` : LOCAL_DEFAULT_IMG}``.

# --hints--

Sie sollten ein `div`-Element innerhalb Ihres `figure`-Elements haben.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

Ihr `div` sollte die Klasse `img-loader` haben.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

Sie sollten ein `img`-Element innerhalb Ihres `figure`-Elements haben.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

Das `img`-Element sollte `CDN_URL` und `img` für sein `src` verwenden, wenn `img` bereitgestellt wird, und auf `LOCAL_DEFAULT_IMG` zurückgreifen, wenn dies nicht der Fall ist.
...
Ihr `img`-Element sollte die Klassen `card-img` und `hidden` haben.
