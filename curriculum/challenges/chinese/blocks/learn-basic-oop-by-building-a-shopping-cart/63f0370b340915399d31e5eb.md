...
你也将在此方法中更新 HTML。 将 `cartSubTotal` 的 `textContent` 设置为 `subTotal` 的值，固定为小数点后 2 位。 使用模板字面量语法将美元符号添加到值的开头。

# --hints--

你应该访问 `cartSubTotal` 元素的 `textContent` 属性。

```js
assert.match(cart.calculateTotal.toString(), /cartSubTotal\.textContent/);
```

你应该在 `subTotal` 变量上调用 `.toFixed()` 方法，并将 `2` 作为参数传递。
...
你应该将 `cartSubTotal` 元素的 `textContent` 设置为你的模板字符串。
