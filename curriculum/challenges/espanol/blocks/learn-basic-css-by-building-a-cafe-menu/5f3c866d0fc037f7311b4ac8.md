...
Para distribuirlos, añade una propiedad `width` a los selectores de clase `flavor` y `price` que tengan un valor de `50%` cada uno.

# --hints--

Debes establecer la propiedad `width` a `50%` en tu selector `.flavor`.
...
Debes establecer la propiedad `width` a `50%` en tu selector `.price`.
