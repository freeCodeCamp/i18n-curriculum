...
`rawData` 配列には、`sku|name|qty|expires|zone` という形式のパイプ区切り文字列が含まれており、`zone` は省略可能です。

**ユーザーストーリー:**

1. 文字列の配列を受け取り、`{ sku, name, qty, expires, zone }` プロパティを持つオブジェクトの配列を返す`parseShipment(rawData)` 関数を実装してください。

   - 入荷品の`sku`が重複している場合は無視してください。
   - `zone` セグメントが提供されていない場合は、`"general"` をデフォルトで代入してください。
   - `qty` の値は数値に変換してください。

2. 現在のパントリーと入荷品を比較し、`{ type, item }` 形式のアクションの配列を返す`planRestock(pantry, shipment)` 関数を実装してください。`type` は `"restock"`、`"discard"`、または `"donate"` のいずれかで、`item` はパース済みの入荷品オブジェクトです。

   `pantry` パラメータは、パース済みの入荷品アイテムと同じ形状のオブジェクトの配列です（`{ sku, name, qty, expires, zone }`）。

- 入荷品の`qty`が`0`以下の場合は、パントリーに存在するかどうかに関わらず、アクションの`type`は`"discard"`にしてください。  
- それ以外で、入荷品の`sku`がすでにパントリーに存在する場合は、アクションの`type`は`"restock"`にしてください。  
- それ以外（入荷品の`sku`がパントリーに存在しない場合）は、アクションの`type`は`"donate"`にしてください。

3. 各アイテムの`zone` プロパティに基づいてアクションを保管ゾーンごとにグループ化する`groupByZone(actions)` 関数を実装してください。関数は、ゾーン名をキー、該当ゾーンのアクション配列を値とするオブジェクトを返す必要があります。例えば、アクションに`"fridge"` と `"pantry"` のゾーンが含まれている場合、結果は `{ fridge: [...], pantry: [...] }` のようになります。
...
`groupByZone` 関数は、正しい内容と数でアクションをグループ化する必要があります。
