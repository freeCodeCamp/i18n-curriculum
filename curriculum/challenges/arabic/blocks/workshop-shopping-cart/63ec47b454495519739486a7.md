...
تحتاج إلى عرض المنتجات المتاحة في HTML الخاص بك. ابدأ باستخدام معامل التعيين بالإضافة لإضافة سلسلة نصية فارغة من نوع template literal إلى خاصية `innerHTML` للمتغير `dessertCards`.

# --hints--

يجب أن تصل إلى خاصية `innerHTML` للمتغير `dessertCards`.

```js
assert.match(code, /dessertCards\.innerHTML/);
```

يجب أن تستخدم معامل التعيين بالإضافة على خاصية `innerHTML`.

```js
assert.match(code, /dessertCards\.innerHTML\s*\+=\s*/);
```

يجب أن تضيف سلسلة نصية فارغة من نوع template literal إلى خاصية `innerHTML`.
