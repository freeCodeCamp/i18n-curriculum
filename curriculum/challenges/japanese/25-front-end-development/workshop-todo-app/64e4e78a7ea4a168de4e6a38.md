...
以前のプロジェクトで、`el.classList.add()` と `el.classList.remove()` を使用して、要素からクラスを追加および削除する方法を学びました。 Another method to use with the `classList` property is the `toggle` method.
...
イベントリスナーのコールバック関数は、`classList.toggle()` メソッドを使って `taskForm` 要素の `hidden` クラスを切り替える必要があります。 Refer back to the example provided in the description.
