...
`.getHours()` メソッドは、`0` から `23` までの数値を返します。 この数値は日付 (日時) の「時」を表します。`0` は午前 0 時 (深夜) 、`23` は午後 11 時に対応します。

では `const` を使って、`hours` という名前の変数を作成してください。そしてその変数に、`date` 変数の「時」を `.getHours()` メソッドで取得して、代入してください。
