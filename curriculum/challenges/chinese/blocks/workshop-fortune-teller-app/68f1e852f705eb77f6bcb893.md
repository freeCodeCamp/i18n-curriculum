...
声明一个名为 `cardId` 的变量，并使用 `data-id` 方法从 `cardElement` 获取 `getAttribute()` 属性的值赋值给它。

# --hints--

你应该在 `cardId` 内声明一个名为 `showFortune` 的变量。
...
你的 `cardId` 变量应该通过调用 `cardElement.getAttribute("data-id")` 来赋值。
