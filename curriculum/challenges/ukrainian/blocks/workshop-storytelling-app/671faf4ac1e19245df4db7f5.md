...
Всередині функції `displayStory` перевірте, чи має об’єкт `storyObj` властивість, що відповідає параметру `genre`, і встановіть текстовий вміст абзацу `#result` на історію вибраного жанру.

Також встановіть колір кордону `storyContainer` на властивість `borderColor` вибраного жанру.

Перевірте це, викликавши функцію `displayStory` з жанром `scary`. Колір кордону має бути `#ee4b2b`, а історія — `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.

# --hints--

Ви маєте викликати функцію `displayStory` з жанром `"scary"`.

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

Коли ви викликаєте `displayStory("scary")`, колір кордону має бути `#ee4b2b`.
...
Коли ви викликаєте `displayStory("scary")`, відображена історія має бути `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.
