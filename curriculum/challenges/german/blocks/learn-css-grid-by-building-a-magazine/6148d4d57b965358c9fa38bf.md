...
Jetzt ist es Zeit, Ihr drittes `section` zu stylen. Beachten Sie, dass es die `text` und `text-with-images` Werte für das `class`-Attribut hat, was bedeutet, dass es bereits die Styles von Ihrer `.text`-Regel erbt.

Erstellen Sie einen `.text-with-images`-Selektor und setzen Sie die `display`-Eigenschaft auf `grid`.

# --hints--

Sie sollten einen `.text-with-images`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

Ihr `.text-with-images`-Selektor sollte eine `display`-Eigenschaft mit dem Wert `grid` haben.
