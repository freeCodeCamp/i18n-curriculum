...
`PlayerData` インターフェースが定義されたので、`PlayerCard` コンポーネントを更新して、`player` インターフェースに合わせて `PlayerData` 型の `PlayerData` プロップを受け取るようにしてください。
...
`PlayerCard` を更新して、`player` 型の `PlayerData` プロップを受け取るようにしてください。
