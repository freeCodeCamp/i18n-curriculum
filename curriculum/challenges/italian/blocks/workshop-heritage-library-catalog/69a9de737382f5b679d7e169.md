...
Per trovare gli autori corrispondenti, controlla ogni voce del catalogo una per una.

Aggiungi un loop `for` vuoto all'interno di `findByAuthor` per iterare su `catalog`. Aggiungerai il corpo del loop nel passaggio successivo.
