...
要用資料填充論壇排行榜，你需要從 API 請求資料。這稱為一種 <dfn>非同步</dfn> 操作，表示任務會獨立於主程式流程執行。 

你可以使用 `async` 關鍵字來創建非同步函式（程式），它會傳回一個 promise。
...
使用 `async` 關鍵字來創建一個名為 `fetchData` 的非同步箭頭函式（程式）。

# --hints--

你應該使用 `async` 關鍵字來創建一個名為 `fetchData` 的非同步箭頭函式（程式）。
