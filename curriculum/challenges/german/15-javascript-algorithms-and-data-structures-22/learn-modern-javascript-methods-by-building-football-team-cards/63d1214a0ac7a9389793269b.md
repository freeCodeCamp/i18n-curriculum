...
Erstelle innerhalb dieses `players`-Array ein neues Objekt mit den folgenden Eigenschaften:
...
Dein `myFavoriteFootballTeam.players`-Array sollte den Wert eines Objekts haben.
...
Deine `name`-Eigenschaft sollte auf `"Sergio Almirón"` eingerichtet sein.
...
Deine `position`-Eigenschaft sollte auf `"forward"` eingerichtet sein.
...
Deine `number`-Eigenschaft sollte auf `1` eingerichtet sein.
