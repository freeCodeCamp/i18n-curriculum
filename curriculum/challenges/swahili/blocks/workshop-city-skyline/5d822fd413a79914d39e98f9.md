...
Mpe `.bb2b` `width` na `height` ya `100%` ili ijaze kontena la jengo. Baadaye utaongeza kitu juu kidogo.

# --hints--

Unapaswa kumpa `.bb2b` `width` ya `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2b')?.width, '100%');
```

Unapaswa kumpa `.bb2b` `height` ya `100%`.
