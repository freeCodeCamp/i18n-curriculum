...
在 `showFortune` 方法内声明一个名为 `target` 的变量，并将其赋值为 `e.target`，然后创建另一个空的 `if` 语句。

# --hints--

你应该在 `target` 内声明一个名为 `showFortune` 的变量。
