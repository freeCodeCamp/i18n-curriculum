...
要准备你的项目逻辑，使用 `const` 和箭头语法声明一个 `sortInputArray` 函数。 它应该采用单个 `event` 参数。

# --hints--

你应该用 `const` 声明一个 `sortInputArray` 函数。

```js
assert.match(code, /const\s+sortInputArray\s*=/);
```

你的 `sortInputArray` 应该是一个函数。

```js
assert.isFunction(sortInputArray);
```

你的 `sortInputArray` 应该采用单个 `event` 参数。

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)/);
```

你的 `sortInputArray` 函数应该使用箭头语法。

```js
assert.match(code, /const\s+sortInputArray\s*=\s*(\(\s*event\s*\)|event)\s*=>/);
```

你的 `sortInputArray` 函数应该是空的。
