...
일반 함수처럼 화살표 함수도 값을 반환할 수 있습니다.

다음은 두 숫자를 곱한 결과를 반환하는 화살표 함수 예시입니다:
...
`addTwoNumbers`라는 새 변수를 만들고 화살표 함수를 할당하세요. 이 화살표 함수는 `num1`과 `num2`라는 두 매개변수를 받아야 합니다. 함수 본문 안에서 `num1`과 `num2`를 더하는 표현식을 반환하세요.

`addTwoNumbers` 함수 아래에 콘솔 문을 추가하세요. 그 콘솔 문 안에서 `addTwoNumbers` 함수를 호출하고 숫자 `3`과 `4`을 인수로 전달하세요.

출력을 확인하려면 콘솔을 열어보세요.

# --hints--

`addTwoNumbers`라는 함수가 있어야 합니다.

```js
assert.isFunction(addTwoNumbers);
```

`addTwoNumbers` 함수는 화살표 함수 구문을 사용해야 합니다.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

`addTwoNumbers` 함수는 `num1`와 `num2`이라는 두 매개변수를 가져야 합니다.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

`addTwoNumbers` 함수는 `num1`와 `num2`를 더하는 표현식을 반환해야 합니다.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

`console.log(addTwoNumbers(3, 4));` 문이 있어야 합니다.
