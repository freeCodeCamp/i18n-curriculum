...
使用相同的語法，在 `#lunch` 元素中查詢 `number` 輸入並將其分配給 `lunchNumberInputs`。

# --hints--

你應該聲明一個 `lunchNumberInputs` 變量。

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=/);
```

你應該使用 `#lunch input[type='number']` 調用 `document.querySelectorAll()`。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你應該將 `document.querySelectorAll()` 調用的結果分配給 `lunchNumberInputs`。
