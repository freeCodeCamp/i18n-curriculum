...
输出变得相当难以阅读，因此你应该用分别打印每一行的循环替换 `print(graph.create_trajectory())`。 

# --hints--

移除 `print(graph.create_trajectory())` 调用。
...
你应该创建一个循环来遍历 `graph.create_trajectory()` 。
...
你应该在 `graph.create_trajectory()` 中打印每一行。
