...
對於你的假值表達式，你需要向 `productsContainer` 添加新的 HTML。 首先刪除 `undefined`，然後使用加法賦值運算符和模板字面量語法將 `div` 添加到 `productsContainer` 的 `innerHTML` 屬性中，並將 `class` 設置爲 `product`，將 `id` 設置爲 `dessert${id}`。

# --hints--

你應該從虛假表達式中刪除 `undefined`。
...
你應該使用加法賦值運算符將 HTML 添加到 `productsContainer`。 請記住，HTML 位於 `innerHTML` 屬性中。
...
你應該使用模板字面量語法將 HTML 添加到 `productsContainer`。

```js
assert.match(code, /productsContainer\.innerHTML\s*\+=\s*`/);
```

你應該將 `div` 添加到 `productsContainer`。
