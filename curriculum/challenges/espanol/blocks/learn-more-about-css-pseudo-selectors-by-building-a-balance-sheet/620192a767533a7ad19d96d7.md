...
Tu selector `tr.total td` debe tener una propiedad de alineación de `text-align` establecida en `right`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td')?.getPropertyValue('text-align') === 'right');
```

Tu selector `tr.total td` debe tener una propiedad de `padding` establecida en `0 0.25rem`.
