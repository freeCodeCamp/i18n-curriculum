...
如果 `num` 不在該行中，則表達式計算結果爲 `True`，表示該數字可以有效插入。

如果 `num` 在該行中，則表達式計算結果爲 `False`，並且插入違反規則。

在 `valid_in_row` 方法體內的表達式前面添加 `return` 關鍵字，以便檢查數字的有效性。

# --hints--

你應該返回 `num not in self.board[row]` 。
