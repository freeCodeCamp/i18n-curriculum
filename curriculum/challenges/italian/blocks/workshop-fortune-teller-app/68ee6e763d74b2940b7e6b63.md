...
L'app della cartomante supporta il render di più carte, quindi devi avere un metodo che possa aiutarti a renderizzare più carte. Dichiara un metodo chiamato `multipleCardSelected`.

All'interno del metodo, chiama la funzione `hideElements` e passa le proprietà `singleCard`, `singleCardBtn`, `multipleCardsBtn` e `headerTitle`. Non dimenticare che devi consultarle da `this.elements`.

# --hints--

Dovresti dichiarare un metodo `multipleCardSelected`.
...
Dovresti chiamare la funzione `hideElements` all'interno del metodo `multipleCardSelected`.
...
Dovresti passare `singleCard` come primo argomento alla funzione `hideElements`.
...
Dovresti passare `singleCardBtn` come argomento alla funzione `hideElements`.
...
Dovresti passare `multipleCardsBtn` come argomento alla funzione `hideElements`.
...
Dovresti passare `headerTitle` come argomento alla funzione `hideElements`.
