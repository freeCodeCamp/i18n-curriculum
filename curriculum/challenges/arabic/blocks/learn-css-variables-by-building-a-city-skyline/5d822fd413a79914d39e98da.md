...
يبدو المبنى الأول جيدًا الآن. قم بتسكين ثلاثة عناصر `div` جديدة داخل حاوية `.background-buildings` وأعطها الأصناف `bb2` و `bb3` و `bb4` بهذا الترتيب. ستكون هذه ثلاثة مبانٍ إضافية للخلفية.

# --hints--

يجب إنشاء عنصر `div` بصنف `bb2`.

```js
assert.exists(document.querySelector('div.bb2'));
```

يجب إنشاء عنصر `div` بصنف `bb3`.

```js
assert.exists(document.querySelector('div.bb3'));
```

يجب إنشاء عنصر `div` بصنف `bb4`.

```js
assert.exists(document.querySelector('div.bb4'));
```

يجب إنشاء 3 عناصر `div` جديدة.

```js
assert.equal(document.querySelectorAll('div')?.length, 9);
```

يجب وضع هذه العناصر `div` داخل عنصر `.background-buildings`.

```js
assert.equal(document.querySelector('div.background-buildings')?.children?.length, 4);
```

يجب وضع العناصر بالترتيب الصحيح.
