...
Kwa kuwa umehamisha kulinganisha ndani ya sharti la `while`, unaweza kuondoa tamko lako lote la `if`.

# --hints--

Haupaswi tena kuwa na tamko la `if`.

```js
assert.notMatch(__helpers.removeJSComments(code), /if\s*\(\s*done\s*===\s*count\s*\)/);
```

Haupaswi tena kuweka `continueLoop` kuwa `false`.
