...
**Nota:** Si desea omitir uno de los argumentos al método `connect`, pasas `null` en su lugar.

# --instructions--

El editor de código tiene las funciones `mapStateToProps()` y `mapDispatchToProps()` y un nuevo componente React llamado `Presentacional`. Conecta este componente a Redux con el método `connect` del objeto global `ReactRedux`, y llámelo inmediatamente en el componente `Presentacional`. Asigna el resultado a un nuevo `const` llamado `ConnectedComponent` que representa el componente conectado. Eso es todo, ¡ahora estás conectado a Redux! Intenta cambiar cualquiera de los argumentos de `connect` a `null` y observa los resultados de la prueba.

# --hints--

El componente `Presentacional` debe renderizarse.
...
El componente `Presentational` debe recibir un prop `messages` a través de `connect`.
...
El componente `Presentational` debe recibir un prop `submitNewMessage` a través de `connect`.
