...
O pseudosseletor `::first-letter` permite que você tenha como alvo a primeira letra do conteúdo de texto de um elemento.
...
Você deve ter um seletor `.first-paragraph::first-letter`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

O seletor `.first-paragraph::first-letter` deve ter a propriedade `font-size` definida como `6rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize, '6rem');
```

O seletor `.first-paragraph::first-letter` deve ter a propriedade `color` definida como `orangered`.
