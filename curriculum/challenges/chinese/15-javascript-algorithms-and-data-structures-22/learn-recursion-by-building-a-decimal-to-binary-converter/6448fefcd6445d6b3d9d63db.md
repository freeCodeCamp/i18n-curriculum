...
每当事件触发事件监听器时，就会自动创建一个事件对象。 你并不总是需要使用这个对象，就像上面的 `click` 处理程序一样，但它对于访问有关触发事件的信息很有用。

首先，将 `e` 作为参数传递给回调函数。 请记住，`e` 是事件对象的通用参数名称。 接下来，将 `e` 记录到回调函数主体中的控制台。
