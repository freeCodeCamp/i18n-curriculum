...
Тепер слід збільшити розмір шрифту для всіх `label` елементів.

Виберіть `label` елемент і встановіть його `font-size` у значення `1.2rem`.

# --hints--

Вам потрібно націлитися на `label` елемент.

```js
assert(new __helpers.CSSHelp(document).getStyle('label'));
```

Встановіть властивість `font-size` у значення `1.2rem`.
