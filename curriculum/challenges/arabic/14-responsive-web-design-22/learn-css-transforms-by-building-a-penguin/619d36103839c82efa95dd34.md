...
يجب أن يحتوي عنصر `.penguin` على خاصية `transition-duration` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionDuration, '1s');
```

يجب أن يحتوي عنصر `.penguin` على خاصية `transition-timing-function` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionTimingFunction, 'ease-in-out');
```

يجب أن يحتوي عنصر `.penguin` على خاصية `transition-delay` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
