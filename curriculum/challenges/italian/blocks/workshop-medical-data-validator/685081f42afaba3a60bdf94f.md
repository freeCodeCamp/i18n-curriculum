...
Ora vuoi verificare che `age` sia un numero intero. Quindi aggiungi un'altra chiave `age` al dizionario `constraints`. Per il suo valore, chiama `isinstance` passando `age` e `int` come argomenti.

# --hints--

Il tuo dizionario `constraints` dovrebbe avere una chiave `age`.
...
La chiave `age` del tuo dizionario `constraints` dovrebbe avere il valore `isinstance(age, int)`.
