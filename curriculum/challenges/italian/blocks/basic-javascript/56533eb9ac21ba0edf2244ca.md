...
title: Usare gli oggetti per le ricerche
challengeType: 1
forumTopicId: 18373
...
Gli oggetti possono essere considerati come una struttura di archiviazione chiave/valore, simile a un dizionario. Se hai dati tabellari, puoi usare un oggetto per cercare valori invece di un'istruzione `switch` o di una catena `if/else`. Questo è particolarmente utile quando sai che i tuoi dati di input sono limitati a un certo intervallo.

Ecco un esempio di un oggetto articolo:
...
Converti l'istruzione switch in un oggetto chiamato `lookup`. Usalo per cercare `val` e assegna la stringa associata alla variabile `result`.
...
`phoneticLookup("")` dovrebbe essere uguale a `undefined`
