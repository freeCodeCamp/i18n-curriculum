...
`bb4-window` クラスに `width` の `18%` を、`height` の `90%` を与え、`--window-color4` 変数を `background-color` として追加してください。

# --hints--

`.bb4-window` に `width` の `18%` を与える必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

`.bb4-window` に `height` の `90%` を与える必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

`.bb4-window` に `background-color` の `--window-color4` を与える必要があります。
