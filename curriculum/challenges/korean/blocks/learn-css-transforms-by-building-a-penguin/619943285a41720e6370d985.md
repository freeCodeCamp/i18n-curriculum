...
`.penguin-body` 요소를 부모의 위쪽에서 `40%`만큼, 왼쪽에서 `23.5%`만큼 위치시키세요.

# --hints--

`.penguin-body`에 `top` 속성을 지정해야 합니다.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top);
```

`.penguin-body`에 `top` 속성을 `--fcc-expected--` 값으로 지정해야 하지만 `--fcc-actual--` 값을 찾았습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top, '40%');
```

`.penguin-body`에 `left` 속성을 지정해야 합니다.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.left);
```

`.penguin-body`에 `left` 속성을 `--fcc-expected--` 값으로 지정해야 하지만 `--fcc-actual--` 값을 찾았습니다.
