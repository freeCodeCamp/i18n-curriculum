...
これは、`style`要素の`h2`を変更することでできます。

要素のテキストの色を決めるプロパティは`color`スタイルプロパティです。

`h2`要素のテキストの色を青に設定する方法は次の通りです。

```html
<h2 style="color: blue;">CatPhotoApp</h2>
```

インライン`style`の宣言は`;`で終わらせるのが良い習慣です。

# --instructions--

`h2`要素のスタイルを変更して、テキストの色を赤にしてください。

# --hints--

`h2`要素には`style`の宣言が必要です。

```js
assert.exists(document.querySelector('h2').getAttribute('style'));
```

`h2`要素のcolorは`red`に設定してください。

```js
assert.strictEqual(document.querySelector('h2').style.color, 'red');
```

`style`の宣言は`;`で終わらせてください。
