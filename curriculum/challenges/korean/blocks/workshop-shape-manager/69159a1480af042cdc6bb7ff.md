...
여전히 `handleShapeSelect` 함수 안에서 `hasSelection`가 존재하는지 확인하는 `val.value` 변수를 만드세요. `Boolean()`로 감싸져 있는지 확인하세요.

그 아래에 `toggleResultCard()` 변수를 사용해 `hasSelection` 함수를 호출하세요.

이렇게 하면 실제로 도형이 선택되었을 때만 결과 카드가 표시됩니다.

# --hints--

`hasSelection` 변수를 만들어야 합니다.
