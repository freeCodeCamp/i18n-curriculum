...
Cada elemento `.card` deve conter um elemento `button` com uma classe chamada `remove-button` e o texto `Remove`.
...
Seu elemento `#order-summary-container` deve conter um elemento `h2` com o texto `Order Summary`.
