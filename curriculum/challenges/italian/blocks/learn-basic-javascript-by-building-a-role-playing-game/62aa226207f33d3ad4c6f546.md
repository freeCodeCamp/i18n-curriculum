...
Usa l'operatore `+=` per aggiungere `" Your <weapon> breaks."`, con uno spazio davanti a `Your`, alla fine di `text.innerText`. Sostituisci `<weapon>` con l'ultimo elemento dell'array `inventory` usando `inventory.pop()`, che rimuoverà l'ultimo elemento dall'array E lo restituirà in modo che appaia nella tua stringa.
...
Dovresti aggiungere `" Your "`, con uno spazio prima e dopo, a `text.innerText`.
...
Dovresti aggiungere `" breaks."`, con uno spazio davanti, alla fine della tua stringa.
