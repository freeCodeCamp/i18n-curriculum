...
Debes crear un nuevo selector `50%` en la regla `@keyframes cabins`.
...
Tu selector `50%` debe encontrarse entre los selectores `0%` y `100%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '50%');
```

Tu selector `50%` debe tener la propiedad `background-color` establecida en `purple`.
