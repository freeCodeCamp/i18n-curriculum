...
Ahora actualice su selector `h1` para que sea `header h1` y apunte específicamente al elemento `h1` dentro de su nuevo `header`.

# --hints--

Debe tener un selector `header h1`.

```js
assert(new __helpers.CSSHelp(document).getStyle('header h1'));
```

No debe tener un selector `h1`.

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('h1'));
```

No debe cambiar ninguna propiedad en el selector.
