...
Para mostrar las canciones en la IU (Interfaz de Usuario), necesitas crear una función.

Utiliza la sintaxis de la función flecha para crear una función `renderSongs` que tome `array` como su parámetro.

# --hints--

Debes usar `const` para crear una función vacía llamada `renderSongs`.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs` debe ser una función.

```js
assert.isFunction(renderSongs)
```

Tu función `renderSongs` debe usar la sintaxis de función flecha.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

Tu función `renderSongs` debe tomar `array` como un parámetro.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

Tu función `renderSongs` debe estar vacía.
