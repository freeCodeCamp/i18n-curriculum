...
Sie sollten auch an einer Funktion arbeiten, die die Eingabefelder löscht.

Erstellen Sie eine `clearInputFields`-Funktion. Verwenden Sie darin `querySelectorAll`, um alle Eingabefelder zu erhalten, und durchlaufen Sie diese mit `forEach()`. Setzen Sie innerhalb der `forEach()` den Wert jedes `input` auf einen leeren String.

# --hints--

Sie sollten eine `clearInputFields`-Funktion erstellen.

```js
assert.isFunction(clearInputFields)
```

Ihre `clearInputFields`-Funktion sollte jedes Eingabefeld auf einen leeren String setzen.
