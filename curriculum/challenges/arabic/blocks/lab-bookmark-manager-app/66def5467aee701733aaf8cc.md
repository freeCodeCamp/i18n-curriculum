...
title: بناء تطبيق مدير العلامات المرجعية
challengeType: 25
...
لهذا المختبر، تم توفير كل من HTML وCSS لك. ستستخدم JavaScript لإكمال تطبيق مدير العلامات المرجعية بحيث يمكنه تخزين مجموعة من العلامات المرجعية في المخزن المحلي وقراءة البيانات منه.

**الهدف:** إكمال قصص المستخدم أدناه واجتياز جميع الاختبارات لإتمام المختبر.

**قصص المستخدم:**

1. يجب أن يكون لديك دالة `getBookmarks` تُعيد مصفوفة `bookmarks` المخزنة في المخزن المحلي. إذا لم يتم تعيين مفتاح `bookmarks` بعد، أو لم يحتوي على مصفوفة صالحة من العلامات المرجعية، يجب أن تُعيد دالة `getBookmarks` مصفوفة فارغة.
1. يجب أن يكون مفتاح `bookmarks` المخزن في المخزن المحلي مصفوفة من كائنات العلامات المرجعية. يجب أن يحتوي كل كائن علامة مرجعية على ثلاثة مفاتيح: `name` و`category` و`url`.
1. يجب أن يكون لديك دالة باسم `displayOrCloseForm` تقوم بالتبديل بين خاصية `hidden` على `#main-section` و`#form-section`.
1. عند النقر على `#add-bookmark-button`، يجب تحديث النص الداخلي لـ `.category-name` ليكون قيمة الخيار المحدد من `#category-dropdown` واستدعاء `displayOrCloseForm` لعرض قسم النموذج وإخفاء القسم الرئيسي.
1. عند النقر على `#close-form-button`، يجب تشغيل دالتك لإخفاء قسم النموذج وعرض القسم الرئيسي.
1. عند النقر على `#add-bookmark-button-form`، يجب تحديث مفتاح `bookmarks` المخزن في المخزن المحلي بإضافة كائن علامة مرجعية إلى نهاية المصفوفة. يجب أن يحتوي الكائن على `name` بقيمة مُدخَل `#name`، و`category` بقيمة الخيار المحدد من قائمة الفئات، و`url` بقيمة مُدخَل `#url`.
1. بمجرد تحديث مفتاح `bookmarks`، يجب إعادة تعيين قيمة `#name` و`#url` إلى سلسلة فارغة قبل تشغيل دالتك لإخفاء قسم النموذج وعرض القسم الرئيسي.
1. يجب أن يكون لديك دالة باسم `displayOrHideCategory` تقوم بالتبديل بين خاصية `hidden` على `#main-section` و`#bookmark-list-section`.
1. عند النقر على `#view-category-button`، يجب تحديث النص الداخلي لـ `.category-name` ليكون قيمة الخيار المحدد من `#category-dropdown`، وتعديل المحتوى الداخلي لـ `#category-list` وفقًا لقصص المستخدم أدناه، واستدعاء دالة `displayOrHideCategory`.
1. إذا لم يكن لدى أي من العلامات المرجعية في المخزن المحلي الفئة المحددة، يجب تعيين المحتوى الداخلي لـ `#category-list` إلى عنصر `p` يحتوي على النص `No Bookmarks Found`.
1. إذا كان لدى علامة مرجعية واحدة أو أكثر في المخزن المحلي الفئة المحددة، أضف زر اختيار لكل علامة مرجعية إلى المحتوى الداخلي لـ `#category-list`. امنح كل زر اختيار `id` و`value` مساويين لاسم العلامة المرجعية، و`name` موحد لجميع الأزرار.
1. يجب أن يحتوي كل زر اختيار على مسمى مطابق يحتوي على عنصر وسم الرابط مع اسم العلامة المرجعية و`href` مضبوط على عنوان URL الخاص بالعلامة.
1. عند النقر على `#close-list-button`، يجب تشغيل دالتك لإخفاء قسم `#bookmark-list-section` وعرض القسم الرئيسي.
1. عند النقر على `#delete-bookmark-button`، يجب حذف العلامة المرجعية المطابقة لزر الاختيار المحدد والفئة المناسبة من المخزن المحلي وتحديث قائمة العلامات المرجعية المعروضة.
...
يجب أن يكون لديك دالة `getBookmarks`.

```js
assert.isFunction(getBookmarks);
```

يجب أن تُعيد دالة `getBookmarks` مصفوفة.

```js
assert.isArray(getBookmarks());
```

يجب أن تُعيد دالة `getBookmarks` مفتاح `bookmarks` من `localStorage`.
...
عندما لا يكون مفتاح `bookmarks` معينًا في `localStorage` أو تكون المصفوفة فارغة، يجب أن تُعيد دالة `getBookmarks` مصفوفة فارغة.
...
عندما لا يحتوي مفتاح `bookmarks` في `localStorage` على مصفوفة صالحة من كائنات العلامات المرجعية، يجب أن تُعيد دالة `getBookmarks` مصفوفة فارغة.
...
يجب أن يكون لديك دالة باسم `displayOrCloseForm`.

```js
assert.isFunction(displayOrCloseForm);
```

يجب أن تقوم دالة `displayOrCloseForm` بالتبديل بين خاصية `hidden` على `#main-section` و`#form-section`.
...
عند النقر على `#add-bookmark-button`، يجب تحديث النص الداخلي لـ `.category-name` ليكون قيمة الخيار المحدد من `#category-dropdown`.
...
عند النقر على `#add-bookmark-button`، يجب استدعاء `displayOrCloseForm` لعرض قسم النموذج وإخفاء القسم الرئيسي.
...
عند النقر على `#close-form-button`, يجب استدعاء `displayOrCloseForm` لإخفاء قسم النموذج وعرض القسم الرئيسي.
...
عند النقر على `#add-bookmark-button-form`، يجب تحديث مفتاح `bookmarks` المخزن في المخزن المحلي بإضافة كائن إلى نهاية المصفوفة. يجب أن يحتوي الكائن المضاف على `name` بقيمة مُدخَل `#name`، و`category` بقيمة الخيار المحدد من قائمة الفئات، و`url` بقيمة مُدخَل `#url`.
...
عند النقر على `#add-bookmark-button-form`، يجب إعادة تعيين قيمة `#name` و`#url` إلى سلسلة فارغة.
...
عند النقر على `#add-bookmark-button-form`، يجب تشغيل `displayOrCloseForm` لإخفاء قسم النموذج وعرض القسم الرئيسي.
...
يجب أن يكون لديك دالة باسم `displayOrHideCategory`.

```js
assert.isFunction(displayOrHideCategory);
```

يجب أن تقوم دالة `displayOrHideCategory` بالتبديل بين خاصية `hidden` على `#main-section` و`#bookmark-list-section`.
...
عند النقر على `#view-category-button`، يجب تحديث النص الداخلي لـ `.category-name` ليكون قيمة الخيار المحدد من `#category-dropdown`.
...
عند النقر على `#view-category-button`، يجب إضافة عنصر `p` يحتوي على النص `No Bookmarks Found` إلى المحتوى الداخلي لـ `#category-list` إذا لم يكن لدى أي من العلامات المرجعية في المخزن المحلي الفئة المحددة.
...
عند النقر على `#view-category-button`، يجب تعديل المحتوى الداخلي لعنصر `#category-list` بإضافة زر اختيار. يجب أن يحتوي زر الاختيار على خاصيتي `id` و`value` مساويتين لاسم العلامة المرجعية لكل علامة في الفئة المحددة. بالإضافة إلى ذلك، يجب أن يكون لكل زر اختيار خاصية `name` موحدة.
...
يجب أن يحتوي كل زر اختيار مضاف إلى المحتوى الداخلي لـ `#category-list` على مسمى مطابق يحتوي على عنصر وسم الرابط مع اسم العلامة المرجعية و`href` مضبوط على عنوان URL الخاص بالعلامة.
...
يجب أن يحتوي كل عنصر `label` على عنصر وسم الرابط مع اسم العلامة المرجعية كنص، و`href` مضبوط على عنوان URL الخاص بالعلامة.
...
عند النقر على `#close-list-button`، يجب إخفاء قسم `#bookmark-list-section` وعرض القسم الرئيسي.
...
عند النقر على `#close-list-button` ثم فتح أي فئة، يجب أن يحتوي `#category-list` فقط على البيانات المتعلقة بالفئة المحددة، دون تكرار المدخلات.
...
عند النقر على `#delete-bookmark-button`، يجب حذف العلامة المرجعية المطابقة لزر الاختيار المحدد والفئة المناسبة من المخزن المحلي وتحديث قائمة العلامات المرجعية المعروضة.
