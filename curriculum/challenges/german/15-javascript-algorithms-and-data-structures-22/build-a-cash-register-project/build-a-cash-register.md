...
title: Erstelle eine Registrierkasse
challengeType: 14
forumTopicId: 16012
...
Hier erstellst du eine App für eine Registrierkasse, die dem Kunden Rückgeld herausgibt, basierend auf dem Preis des Produktes, dem Bargeldbetrag, den der Kunde gezahlt hat und der Menge an Bargeld im Geldfach. Du musst dem Kunden auch verschiedene Nachrichten in verschiedenen Szenarien anzeigen, B. wenn der Kunde zu wenig Bargeld einzahlt oder die Kassenschublade nicht genug Bargeld enthält, um das Wechselgeld herauszugeben.
...
**Ziel:** Erstelle eine App, die eine ähnliche Funktionalität wie <a href="https://cash-register.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://cash-register.freecodecamp.rocks</a> aufweist.

**User Stories:**

1. Du sollst ein `input` Elemet mit einer `id` von `"cash"` haben.
1. Du sollst ein `div`, `span` oder `p` Element mit einer `id` von `"change-due"` haben.
1. Du sollst ein `button` Element mit einer `id` von `"purchase-btn"` haben.
...
Du solltest ein `input` -Element mit einer `id` von `cash` haben.
