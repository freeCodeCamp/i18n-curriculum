...
이 워크숍에서는 재생 목록에서 노래를 재생, 일시 정지, 건너뛸 수 있는 음악 플레이어 앱을 만듭니다.

이 프로젝트의 HTML과 CSS는 제공되었으니 JavaScript에 포커스를 맞추세요.

먼저 `#playlist-songs`, `#play`, `#pause` 요소에 접근하여 각각 `playlistSongs`, `playButton`, `pauseButton` 변수에 할당하세요.
...
`#playlist-songs` 요소를 `playlistSongs` 변수에 할당해야 합니다. 
...
`#play` 요소를 `playButton` 변수에 할당해야 합니다. 
...
`#pause` 요소를 `pauseButton` 변수에 할당해야 합니다. 
