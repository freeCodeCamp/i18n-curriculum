...
最後に、もしエラーが発生して著者データの読み込みに失敗したらどうしますか？その場合は、UIにエラーを表示する必要があります。まさに`.catch()`メソッドはエラー処理のためのものです。

`.catch()`の中で、`console.error()`を削除し、`innerHTML`の`authorContainer`を`p`という`class`属性を持つ`"error-msg"`要素に設定し、テキストは`"There was an error loading the authors"`にしてください。
...
`console.error`とそのテキストを削除してください。

```js
assert.notMatch(code, /console\.error\(\s*`There\s+was\s+an\s+error:\s+\$\{err\}`\s*\)\s*;?/)
```

`innerHTML`の`authorContainer`にアクセスし、それを`p`要素に設定してください。`p`要素はテンプレートリテラルで囲むのを忘れないでください。

```js
assert.match(code, /authorContainer\.innerHTML\s*=\s*(`|"|')\s*<p.*>.*<\/p>\s*\1/)
```

`p`要素には`"error-msg"`クラスを付けてください。

```js
assert.match(code, /(`|"|')\s*<p\s+class\s*=\s*("|')error-msg\2>/)
```

`p`要素には`"There was an error loading the authors"`というテキストを設定してください。
