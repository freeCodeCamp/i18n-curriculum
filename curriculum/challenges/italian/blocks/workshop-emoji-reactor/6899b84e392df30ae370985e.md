...
Ora, nel file JavaScript, crea una variabile `happyBtn`.

Usa `querySelector` per archiviare un riferimento all'elemento `#happy-btn` nella variabile `happyBtn`.

# --hints--

Dovresti avere una variabile chiamata `happyBtn`.

```js
assert.exists(happyBtn);
```

La variabile `happyBtn` dovrebbe contenere un riferimento all'elemento `#happy-btn`.
