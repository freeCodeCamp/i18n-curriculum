...
title: 트윗 임베드에서 행을 만들기 위해 flex-direction 속성 적용
challengeType: 0
...
트윗 임베드 예제의 `header`와 `footer`에는 `flex-direction` 속성을 사용해 행으로 배열할 수 있는 자식 항목들이 있습니다. 이 속성은 CSS에 자식 요소들을 수평으로 정렬하라고 지시합니다.

# --instructions--

`flex-direction` CSS 속성을 `header`와 `footer` 모두에 추가하고 값을 `row`로 설정하세요.
...
`header`에는 `flex-direction` 속성이 `row`로 설정되어야 합니다.

```js
assert.match(code, /header\s*?{[^}]*?flex-direction:\s*?row;/g);
```

`footer`에는 `flex-direction` 속성이 `row`로 설정되어야 합니다.
