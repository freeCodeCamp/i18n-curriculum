...
现在屏幕上显示了歌曲列表，最好按照标题的字母顺序对它们进行排序。 你可以手动更新 `allSongs` 数组，但 JavaScript 有一个可以使用的数组方法，称为 <dfn>sort()</dfn>。

`sort()` 方法将数组的元素转换为字符串，并根据 `UTF-16` 编码中的值对其进行排序。
...
在 `sortSongs` 函数内，将 `sort()` 方法添加到 `userData?.songs`。

# --hints--

你应该将 `sort()` 方法添加到 `userData?.songs`。
