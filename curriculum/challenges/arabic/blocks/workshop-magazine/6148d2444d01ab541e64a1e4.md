...
أنشئ محدِّد `.quote`. أضف له خاصية `color` بقيمة `#00beef`، وخاصية `font-size` بقيمة `2.4rem`، وخاصية `text-align` بقيمة `center`.

# --hints--

يجب أن يكون لديك محدِّد `.quote`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote'));
```

يجب أن يحتوي محدِّد `.quote` على خاصية `color` بقيمة `#00beef`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.color, 'rgb(0, 190, 239)');
```

يجب أن يحتوي محدِّد `.quote` على خاصية `font-size` بقيمة `2.4rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize, '2.4rem');
```

يجب أن يحتوي محدِّد `.quote` على خاصية `text-align` بقيمة `center`.
