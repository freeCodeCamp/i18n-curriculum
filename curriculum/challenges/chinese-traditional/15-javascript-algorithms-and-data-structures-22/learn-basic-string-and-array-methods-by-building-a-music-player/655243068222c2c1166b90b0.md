...
在 `playNextSong` 函數內部，創建一個 `if` 語句來檢查 `userData` 的 `currentSong` 是否嚴格等於 `null`。 這將檢查 `userData` 對象中是否沒有正在播放的歌曲。

如果條件爲真，則使用 `userData?.songs` 數組中第一首歌曲的 `id` 作爲參數調用 `playSong` 函數。

# --hints--

你不應修改現有的 `playNextSong` 函數及其內容。

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

你應該創建一個帶有條件 `userData?.currentSong === null` 的 `if` 語句。
...
你應該在 `if` 語句內使用 `userData?.songs[0].id` 調用 `playSong` 函數。
