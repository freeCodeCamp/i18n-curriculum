...
Після перевірки `"Unknown"` додайте логіку групування за десятиліттями для книг з числовими роками. Оголосіть `const decade = Math.floor(book.year / 10) * 10`. Наприклад, `Math.floor(1977 / 10)` дає `197`, помножене на `10`, дає `1970`.

Потім оголосіть `const decadeKey =` `` `${decade}s` `` to produce a string like `"1970s"`.

Використовуйте дужкову нотацію, щоб згрупувати книгу: перевірте, чи `grouped[decadeKey]` ще не існує, і ініціалізуйте його як порожній масив, потім додайте `book` у нього. Тут потрібна дужкова нотація, бо ключ зберігається у змінній.

# --hints--

Ви повинні оголосити змінну з іменем `decade`, встановлену в `Math.floor(book.year / 10) * 10`.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+decade\s*=\s*Math\.floor\s*\(\s*book\.year\s*\/\s*10\s*\)\s*\*\s*10/);
```

Ви повинні оголосити змінну з іменем `decadeKey`, яка містить `decade` і літеру `"s"`.
...
Ви повинні перевірити, чи `grouped[decadeKey]` ще не існує, і ініціалізувати його як порожній масив.
...
Ви повинні додати `book` у `grouped[decadeKey]`.
