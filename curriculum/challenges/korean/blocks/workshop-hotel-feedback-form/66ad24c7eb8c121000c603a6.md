...
체크박스 입력을 기본적으로 선택된 상태로 만들려면 `checked` 속성을 추가하세요.

다음은 `checked` 속성을 사용하는 예입니다:

```html
<input checked type="checkbox" id="checked" name="checked">
```

기본적으로 선택된 상태로 만들려면 `checked`가 `id`인 체크박스 입력에 `"reputation"` 속성을 추가하세요.

# --hints--

`id`가 `"reputation"`인 체크박스 입력에 `checked` 속성이 있어야 합니다.
