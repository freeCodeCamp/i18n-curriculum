...
Um die Absenden-Schaltfläche deutlicher hervorzuheben, geben Sie ihr eine `background-color` von `#3b3b4f` und eine `border-color` von `white`.

# --hints--

Sie sollten der Absenden-Schaltfläche eine `background-color` von `#3b3b4f` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.backgroundColor, 'rgb(59, 59, 79)');
```

Sie sollten der Absenden-Schaltfläche eine `border-color` von `white` geben.
