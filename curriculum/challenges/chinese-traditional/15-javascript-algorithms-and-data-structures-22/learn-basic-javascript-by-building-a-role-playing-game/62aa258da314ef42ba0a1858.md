...
創建兩個新函數，分別命名爲 `pickTwo` 和 `pickEight`。

在這兩個函數內部，調用 `pick()` 函數，並根據函數名稱傳遞 `2` 或 `8` 作爲參數。

# --hints--

你應該使用 `function` 關鍵字聲明 `pickTwo`。

```js
assert.match(code, /function\s+pickTwo\s*/);
```

你應該使用 `function` 關鍵字聲明 `pickEight`。

```js
assert.match(code, /function\s+pickEight\s*/);
```

`pickTwo` 應該調用 `pick` 函數。

```js
assert.match(pickTwo.toString(), /pick\(/);
```

`pickTwo` 應該調用 `pick` 函數，並傳遞 `2` 作爲參數。

```js
assert.match(pickTwo.toString(), /pick\(\s*2\s*\)/);
```

`pickEight` 應該調用 `pick` 函數。

```js
assert.match(pickEight.toString(), /pick\(/);
```

`pickEight` 應該調用 `pick` 函數，並傳遞 `8` 作爲參數。
