...
Innerhalb der `for`-Schleife deklarieren Sie eine Variable `row_str` und weisen ihr eine List Comprehension zu, die über `row` iteriert und jedes Element `i` in `row` in einen String umwandelt. Verwenden Sie dafür die Funktion `str()`.

# --hints--

Sie sollten eine Variable `row_str` deklarieren.
...
Sie sollten der Variable `row` eine List Comprehension zuweisen, die über `row_str` iteriert.
...
Die List Comprehension, die der Variable `row_str` zugewiesen wird, sollte `i` als Iterationsvariable verwenden.
...
Die List Comprehension, die der Variable `row_str` zugewiesen wird, sollte für jedes Element `str()` in `i` die Funktion `row` aufrufen.
