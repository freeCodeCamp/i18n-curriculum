...
`linear-gradient` 関数とカラーストップの基本がわかったので、それらを使ってマーカーをよりリアルに見せることができます。

`linear-gradient` 関数内で、`gradientDirection` を `180deg` に設定してください。

# --hints--

`.red` の CSS ルールには、`background` プロパティが `linear-gradient(180deg, rgb(255, 0, 0) 75%, rgb(0, 255, 0), rgb(0, 0, 255))` に設定されている必要があります。
