...
Тепер можна стилізувати вміст елемента `.heading` за допомогою сітки CSS.

CSS-функцію `repeat()` використовують для повторення значення, а не для написання його вручну, що корисне для сітки. Наприклад, встановлення властивості `grid-template-columns` на `repeat(20, 200px)` створить 20 стовпчиків, кожен шириною `200px`.

Надайте елементу `.heading` властивість `grid-template-columns` зі значенням `repeat(2, 1fr)`, щоб створити два стовпчики однакової ширини.

# --hints--

Cелектор `.heading` повинен мати властивість `grid-template-columns` зі значенням `repeat(2, 1fr)`.
