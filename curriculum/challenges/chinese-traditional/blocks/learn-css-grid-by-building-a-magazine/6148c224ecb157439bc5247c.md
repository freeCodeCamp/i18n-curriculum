...
CSS Grid 的預設設定會根據需要創建額外的列，這與 Flexbox 不同。為 `.social-icons` 選擇器添加一個 `grid-template-columns` 屬性，並設定為 `repeat(5, 1fr)`，以將圖示排列在單一列中。

# --hints--

你的 `.social-icons` 選擇器應該有一個 `grid-template-columns` 屬性設定為 `repeat(5, 1fr)`。
