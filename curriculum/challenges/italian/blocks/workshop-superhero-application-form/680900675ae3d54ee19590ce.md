...
Ci saranno sei caselle di spunta, quindi è meglio creare anche un array per loro e scorrerlo per mostrarle come caselle di spunta. Per questo, ti è stato fornito un array `powersOptions`.

Usa una funzione freccia per scorrere l'array `powersOptions` usando il parametro `power`. All'interno del map, crea un elemento `label` con un `key` di `power` e un testo di `Hello`.

Dopo di che, dovresti vedere sei testi `Hello`. Compila gli elementi dell'array nel passaggio successivo.

# --hints--

Dovresti scorrere `powersOptions`.

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\(/)
```

Dovresti usare `power` come parametro del tuo map.

```js
assert.match(code, /{\s*powersOptions\s*\.\s*map\s*\(\s*(power|\(\s*power\s*\))\s*=>/)
```

Dovresti creare un elemento `label` con un `key` di `power`.

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\([\s\S]*<\s*label\s+key\s*=\s*\{\s*power\s*\}\s*>[\s\S]*<\s*\/\s*label\s*>/)
```

Il tuo elemento `label` dovrebbe avere il testo `Hello`.
