...
Nel tuo oggetto `userData`, crea una proprietà `songs` e imposta il suo valore sull'array `allSongs`.

Poi, per gestire le informazioni della canzone corrente e tracciare il suo tempo di riproduzione, crea le proprietà `currentSong` e `songCurrentTime`. Imposta i valori rispettivamente su `null` e `0`.

# --hints--

Il tuo oggetto `userData` dovrebbe avere una chiave `songs` impostata sull'array `allSongs`.

```js
assert.propertyVal(userData, "songs", allSongs);
```

Il tuo oggetto `userData` dovrebbe avere una chiave `currentSong` impostata su `null`.

```js
assert.propertyVal(userData, "currentSong", null);
```

Il tuo oggetto `userData` dovrebbe avere una chiave `songCurrentTime` impostata su `0`.
