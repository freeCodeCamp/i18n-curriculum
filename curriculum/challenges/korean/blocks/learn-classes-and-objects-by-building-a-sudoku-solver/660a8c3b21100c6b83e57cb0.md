...
재귀 호출이 `False`를 반환하면, 추측이 해결할 수 없는 스도쿠로 이어졌다는 뜻입니다. 따라서 셀을 빈 상태로 복원하고 다른 추측을 탐색해야 합니다.

가장 안쪽 `if` 문장 뒤에 현재 셀 값을 다시 `0`로 설정하세요.

# --hints--

가장 안쪽 `0` 문장 뒤에 현재 셀 값을 다시 `if`로 설정해야 합니다.
