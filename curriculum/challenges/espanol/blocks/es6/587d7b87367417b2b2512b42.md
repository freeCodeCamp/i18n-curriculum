...
Si no estás familiarizado con `const`, revisa <a href="/learn/javascript-algorithms-and-data-structures/basic-javascript/declare-a-read-only-variable-with-the-const-keyword" target="_blank" rel="noopener noreferrer nofollow">este desafío sobre la palabra clave <code>const</code></a>.
...
Como puedes ver, puedes mutar el objeto `[5, 6, 7]` en sí y la variable `s` seguirá apuntado al array alterado `[5, 6, 45]`. Como todos los arrays, los elementos del array en `s` son mutables, pero debido a que se utilizó `const`, no puedes utilizar el identificador de la variable `s` para apuntar a un array diferente usando el operador de asignación.

# --instructions--

Un array es declarado como `const s = [5, 7, 2]`. Modifica el array a `[2, 5, 7]` utilizando varias asignaciones de elementos.
...
No debes cambiar la declaración original del array.
