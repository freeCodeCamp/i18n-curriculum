...
All'interno di `map`, restituisci la funzione `renderCard` passando `type`, `isReversed`, `name_short` e `img` come argomenti, e aggiungi il metodo `join` su `map` con una stringa vuota.

# --hints--

Dovresti restituire la funzione `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

Dovresti passare `type` come primo argomento nella funzione `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

Dovresti passare `isReversed` come secondo argomento nella funzione `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?isReversed\s*/);
```

Dovresti passare `card.name_short` come argomento nella funzione `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*/);
```

Dovresti passare `card.img` come argomento nella funzione `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

Dovresti allegare `join("")` al metodo `map`.
