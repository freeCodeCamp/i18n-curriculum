...
Jetzt muss Ihre `applyFunction` ein Ergebnis zurückgeben. Geben Sie das Ergebnis des Aufrufs der `.replace()`-Methode auf `str2` zurück. Übergeben Sie Ihren `functionCall`-Regex und ein leeres Callback.

# --hints--

Ihre `applyFunction`-Funktion sollte das Ergebnis des Aufrufs der `.replace()`-Methode auf `str2` zurückgeben.
...
Sie sollten `functionCall` als erstes Argument an Ihren `.replace()`-Aufruf übergeben.
...
Sie sollten eine leere Pfeilfunktion als zweites Argument an Ihren `.replace()`-Aufruf übergeben.
