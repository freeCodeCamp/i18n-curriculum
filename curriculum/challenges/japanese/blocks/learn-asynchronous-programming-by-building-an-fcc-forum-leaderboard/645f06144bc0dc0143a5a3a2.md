...
各投稿には、そのトピックの会話に参加しているすべてのユーザーを表す、アバター画像のリストが含まれます。

ではまず、`avatars` という名前のアロー関数を作成し、`posters` と `users` という 2 つの仮引数を持たせてください。

# --hints--

空のアロー関数 `avatars` が必要です。

```js
assert.match(code, /const\s+avatars\s*=\s*\([\s\S]*\)\s*=>\s*{\s*}/);
```

関数 `avatars` には、`posters` と `users` という仮引数が必要です。
