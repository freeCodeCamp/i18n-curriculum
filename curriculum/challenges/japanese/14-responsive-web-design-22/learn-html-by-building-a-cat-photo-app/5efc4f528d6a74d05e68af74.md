...
You should add a new `label` element after the checkbox element. 開始タグと終了タグが両方あることを確認してください。
...
Your `label` element should have a `for` attribute with the value `loving`.  期待される値は `--fcc-expected--` ですが、見つかった値は `--fcc-actual--` でした。
