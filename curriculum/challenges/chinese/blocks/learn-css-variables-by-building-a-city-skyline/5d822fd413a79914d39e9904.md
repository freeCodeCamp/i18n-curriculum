...
给 `.bb3` 添加 `repeating-linear-gradient`。 将方向设置为 `90deg`，`building-color3` 作为前两种颜色，在 `15%` 处采用 `window-color3` 作为第三种颜色。

当你未指定颜色的距离时，它将使用合理的默认值。 在这种情况下，前两个颜色会默认设置为 `0%` 和 `7.5%`，因为它从 `0%` 开始，而 `7.5%` 是 `15%` 的一半，因此你无需手动设置它们。
