...
對於 `p` 元素內含有文字 `/month` 的 `span`，為它添加 `text-base`、`font-medium` 和 `text-gray-500` 類別。這將使該文字較價格本身更為淡化，以便區分。

# --hints--

你的 `span` 元素應該有 `text-base` 類別。
...
你的 `span` 元素應該有 `text-gray-500` 類別。
