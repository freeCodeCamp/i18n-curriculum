...
Usa la función `minmax` para hacer que tus columnas sean responsive en cualquier dispositivo. La función `minmax` toma dos argumentos, el primero siendo el valor mínimo y el segundo el máximo. Estos valores podrían ser una longitud, porcentaje, `fr`, o incluso una palabra clave como `max-content`.

Envuelve cada uno de tus valores ya definidos de la propiedad `grid-template-columns` en una función `minmax`, usando cada valor como el segundo argumento. El primer argumento debería ser `2rem`, `min-content`, y `2rem` respectivamente.

# --hints--

Tu selector `main` debe tener una propiedad `grid-template-columns` establecida en `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
