...
現在 `b()` 已經執行，將其從調用堆棧中彈出。 然後，將對 `a()` 的模擬調用更新爲以下內容：`"a(): returns 'freeCodeCamp ' + 'is awesome!'"`。

# --hints--

`callStack` 應該有一個字符串元素。
...
`callStack` 中的字符串應該是字符串 `"a(): returns 'freeCodeCamp ' + 'is awesome!'"`。
