...
Em um módulo anterior, você aprendeu que pode criar suas próprias exceções personalizadas herdando da classe interna `Exception`:
...
Crie uma classe chamada `MediaError`. Faça-a herdar da classe `Exception` e adicione a ela uma docstring dizendo `Custom exception for media-related errors.`.

# --hints--

Você deve criar uma classe chamada `MediaError`.
...
Sua classe `MediaError` deve herdar de `Exception`.
...
Você deve adicionar a docstring `Custom exception for media-related errors.` à sua classe `MediaError`.
