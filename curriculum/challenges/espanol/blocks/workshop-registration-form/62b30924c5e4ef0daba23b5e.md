...
El borde del último elemento `fieldset` se ve un poco fuera de lugar. 

En lecciones anteriores, aprendiste cómo trabajar con la pseudo-clase CSS `last-of-type` así: 

```css
p:last-of-type { }
```

Esto seleccionará el último elemento `p`.

Cree un nuevo selector que apunte al último elemento `fieldset` y establezca su `border-bottom` a `none`.

# --hints--

Puedes utilizar la pseudo-clase `:last-of-type` para seleccionar solo el último elemento de un determinado tipo.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type'));
```

Tu `fieldset:last-of-type` debe tener `border-bottom` establecido como `none`.
