...
既然你知道要重新引导到哪个闸口，就可以移动溢出的参与者。

获取 `overflowAmount` 并将其添加到下一个闸口在相同 `tickIndex` 的队列中。

具体来说，你必须：

- 使用 `nextGateIndex` 访问下一个闸口。
- 访问它的 `queue`。
- 将 `overflowAmount` 添加到位置 `tickIndex` 的值中。

# --hints--

你的 `rerouteOverflow` 函数应该将 `overflowAmount` 添加到下一个闸口 `tickIndex` 中 `queue` 位置的值。
