...
你應該告訴玩家他們購買了什麼武器。 在你剛剛寫的兩行之間，使用 `let` 初始化一個名爲 `newWeapon` 的新變量。 將其設置爲等於 `weapons`。

# --hints--

你的 `buyWeapon` 函數應該有一個名爲 `newWeapon` 的變量。

```js
assert.match(buyWeapon.toString(), /newWeapon/);
```

你應該使用 `let` 來聲明 `newWeapon`。

```js
assert.match(code, /let\s+newWeapon/);
```

`newWeapon` 應初始化爲具有 `weapons` 的值。 不要忘記分號。

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

在修改 `text` 之前，應先聲明 `newWeapon`。
