...
Metti in ordine l'elemento `header` utilizzando _Flexbox_ per aggiungere spazio tra gli elementi figli e centrarli verticalmente.

Quindi, fissa l'elemento `header` in cima al viewport.

# --hints--

Dovresti dare all'elemento `header` una proprietà `justify-content` con il valore `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

Dovresti dare all'elemento `header` una proprietà `align-items` con il valore `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

Dovresti dare all'elemento `header` una proprietà `position` con il valore `fixed`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.position, 'fixed');
```

Dovresti dare all'elemento `header` una proprietà `top` con il valore `0`.
