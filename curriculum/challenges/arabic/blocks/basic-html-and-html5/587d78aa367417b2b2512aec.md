...
title: تعريف رأس وجسم مستند HTML
challengeType: 0
...
يمكنك إضافة مستوى آخر من التنظيم في مستند HTML الخاص بك داخل وسم `html` باستخدام العنصرين `head` و `body`. أي ترميز يحتوي على معلومات عن صفحتك يجب أن يوضع داخل وسم `head`. ثم أي ترميز يحتوي على محتوى الصفحة (ما يُعرض للمستخدم) يجب أن يوضع داخل وسم `body`.

عناصر البيانات الوصفية، مثل `link` و `meta` و `title` و `style`، عادةً ما توضع داخل عنصر `head`.

إليك مثال على تخطيط صفحة:
...
حرر الترميز بحيث يحتوي على `head` و `body`. يجب أن يحتوي عنصر `head` فقط على `title`، ويجب أن يحتوي عنصر `body` فقط على `h1` و `p`.

# --hints--

يجب أن يكون هناك عنصر `head` واحد فقط في الصفحة.
...
يجب أن يكون هناك عنصر `body` واحد فقط في الصفحة.
...
يجب أن يكون عنصر `head` تابعًا لعنصر `html`.
...
يجب أن يكون عنصر `body` تابعًا لعنصر `html`.
...
يجب أن يحيط عنصر `head` بعنصر `title`.
...
يجب أن يحيط عنصر `body` بكل من العنصرين `h1` و `p`.
