...
معطى شبكة 3x3 مع رقم مفقود واحد (مُمثل بـ `0`)، أرجع الرقم المفقود الذي يُكمل المربع السحري، أو `"impossible"` إذا لم يوجد رقم صالح.

المربع السحري هو شبكة حيث مجموع كل صف وعمود وقطر يساوي نفس الرقم.

# --hints--

`solveMagicSquare([[2, 7, 6], [9, 0, 1], [4, 3, 8]])` يجب أن تُرجع `5`.

```js
assert.equal(solveMagicSquare([[2, 7, 6], [9, 0, 1], [4, 3, 8]]), 5);
```

`solveMagicSquare([[0, 14, 12], [18, 10, 2], [8, 6, 16]])` يجب أن تُرجع `4`.

```js
assert.equal(solveMagicSquare([[0, 14, 12], [18, 10, 2], [8, 6, 16]]), 4);
```

`solveMagicSquare([[12, 17, 16], [19, 0, 10], [14, 13, 18]])` يجب أن تُرجع `"impossible"`.

```js
assert.equal(solveMagicSquare([[12, 17, 16], [19, 0, 10], [14, 13, 18]]), "impossible");
```

`solveMagicSquare([[15, 35, 31], [43, 27, 11], [23, 19, 0]])` يجب أن تُرجع `39`.

```js
assert.equal(solveMagicSquare([[15, 35, 31], [43, 27, 11], [23, 19, 0]]), 39);
```

`solveMagicSquare([[26, 41, 14], [47, 35, 0], [32, 29, 44]])` يجب أن تُرجع `"impossible"`.
