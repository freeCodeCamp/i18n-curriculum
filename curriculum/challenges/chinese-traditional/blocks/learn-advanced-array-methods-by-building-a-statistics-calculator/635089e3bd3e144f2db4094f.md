...
如果你用一組數字測試你的表單，你應該會看到平均值顯示在頁面上。然而，這只因為 freeCodeCamp 的 `iframe` 有特殊設定才會有效。通常，當表單被提交時，`event` 會觸發頁面重新整理。

為了解決此問題，請在 `onsubmit` 屬性中你的 `calculate();` 呼叫後添加 `return false;`。

# --hints--

你的 `onsubmit` 屬性應該包含 `return false;` 述語。

```js
assert.match(document.querySelector("form").getAttribute("onsubmit"), /return\s+false\s*;?/);
```

你的 `onsubmit` 屬性仍然應該呼叫 `calculate()`。

```js
assert.match(document.querySelector("form").getAttribute("onsubmit"), /calculate\(\s*\)/);
```

你在呼叫 `calculate()` 之後應該傳回 `false`。
