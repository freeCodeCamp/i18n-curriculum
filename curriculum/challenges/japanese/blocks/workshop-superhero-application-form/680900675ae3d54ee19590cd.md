...
次の `input` は、ユーザーに当てはまるすべてのスーパーヒーローの能力をマークするためのチェックボックスです。

`label` と `className` の `section` を持ち、テキストが `column` の `List your powers (select all that apply):` 要素を作成してください。

# --hints--

`label` と `className` の `section` を持つ `column` 要素を作成してください。
...
`label` 要素にはテキスト `List your powers (select all that apply):` を含める必要があります。
