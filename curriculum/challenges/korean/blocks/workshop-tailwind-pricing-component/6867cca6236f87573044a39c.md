...
마지막으로 가족 요금제의 CTA 버튼에 스타일을 지정하세요.

클래스 `block`, `rounded-md`, `bg-indigo-600`, `px-4`, `py-2`, `text-center`, `font-semibold`, `text-white`, `hover:bg-indigo-700`를 지정하세요.

이로써 Tailwind 가격 컴포넌트 워크숍이 완료됩니다!

# --hints--

앵커 요소에 클래스 `bg-indigo-600`가 있어야 합니다.
...
앵커 요소에 클래스 `text-center`가 있어야 합니다.
...
앵커 요소에 클래스 `text-white`가 있어야 합니다.
...
앵커 요소에 클래스 `font-semibold`가 있어야 합니다.
...
앵커 요소에 클래스 `hover:bg-indigo-700`가 있어야 합니다.
...
앵커 요소에 클래스 `rounded-md`가 있어야 합니다.
