...
Ahora que se creó el setter `salary`, actualiza `__init__` para que lo llame en lugar de asignar directamente a `self._salary`. Cambia `self._salary = Employee._base_salaries[level]` por `self.salary = Employee._base_salaries[level]`.

# --hints--

Tu método `__init__` debe usar `self.salary = Employee._base_salaries[level]` en lugar de `self._salary = Employee._base_salaries[level]`.
