...
檢查 `length` 是否正在使用你的 `isEven` 函數。 如果是，則返回 `middle` 索引處的數字與其後一個數字的平均值。 如果是奇數，則返回 `middle` 索引處的數字 - 你需要將 `middle` 值向上舍入。
...
如果 `length` 爲偶數，則應返回 `middle` 索引處的數字和其後的數字的平均值。
...
如果 `length` 爲奇數，則應返回 `middle` 索引處的值。
