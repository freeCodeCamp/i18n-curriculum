...
`Est. 2020`에 `established` 클래스 선택자를 만들고 `font-style` 값을 가진 `italic` 속성을 지정하여 이탤릭체로 만드세요.

# --hints--

`.established` 선택자가 있어야 합니다.
...
`font-style` 속성을 `italic`로 설정해야 합니다.
...
`.established` 선택자는 `font-style` 속성을 `italic`로 설정해야 합니다.
