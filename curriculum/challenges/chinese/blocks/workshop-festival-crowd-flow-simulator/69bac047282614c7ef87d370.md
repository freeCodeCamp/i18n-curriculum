...
为了帮助追踪参与者如何通过系统，向控制台记录以下消息：`overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`。你可以使用字串串接或模板字面值。

# --hints--

你的 `rerouteOverflow` 函数应该向控制台记录以下消息 `overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`。
