...
Kommen wir nun zum `Most popular`-Abzeichen. Sie müssen es in der oberen rechten Ecke der Karte platzieren, dafür können Sie einige der Positionierungs-Utility-Klassen von Tailwind verwenden.

So würden Sie CSS-Positionierung in Tailwind verwenden:
...
Weisen Sie dem Wrapper `relative` die Klasse `div` zu und `absolute` dem `div` mit dem Text `Most popular`.

# --hints--

Ihr Wrapper-Element `div` für den Premium-Plan sollte die Klasse `relative` haben.
...
Ihr `div`-Element mit dem Text `Most popular` sollte die Klasse `absolute` haben.
