...
بعد أن تمسح `multipleCard` مباشرةً، داخل طريقة `singleCardSelected`، نفذ استدعاء الدالة `showElements` ومرر `singleCard` كالمعلمة الأولى و`fortuneContainer` كالمعلمة الثانية.

# --hints--

يجب أن تنفذ استدعاء الدالة `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

يجب أن تمرر `elements.singleCard` كالمعلمة الأولى.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

يجب أن تمرر `elements.fortuneContainer` كالمعلمة الثانية.
