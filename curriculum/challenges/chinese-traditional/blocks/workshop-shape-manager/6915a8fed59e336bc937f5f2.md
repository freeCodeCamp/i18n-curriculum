...
最後一組要選取的元素是結果元素。使用你的 `getElement` 函式取得 id 為 `result-text` 和 `result-card` 的元素。兩者都應該使用 `HTMLElement` 作為型別。

記得你已經宣告了 `resultText` 和 `resultCard` 變數，所以你只需要為它們指定值。

# --hints--

你應該為 `resultText` 變數指定一個值。

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` 應該選取 `result-text` 元素。

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

你應該使用 `HTMLElement` 作為你用來選取 `getElement` 元素的 `result-text` 的型別。

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

你應該為 `resultCard` 變數指定一個值。

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` 應該選取 `result-card` 元素。

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

你應該使用 `HTMLElement` 作為你用來選取 `getElement` 元素的 `result-card` 的型別。
