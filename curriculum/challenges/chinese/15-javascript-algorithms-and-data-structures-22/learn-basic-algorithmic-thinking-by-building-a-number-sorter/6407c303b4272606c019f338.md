...
你需要一个函数来更新显示排序的数字。 首先使用箭头语法声明一个采用单个 `array` 参数的 `updateUI` 函数。

因为你将编写不会立即具有返回值的算法，所以将 `array` 的后备值设置为空数组。 以下是具有备用值的函数示例：
...
`updateUI` 应该接受一个 `array` 参数。

```js
assert.match(code, /const\s+updateUI\s*=\s*\(?\s*array/);
```

`updateUI` 应该为 `array` 设置一个空数组作为默认值。
