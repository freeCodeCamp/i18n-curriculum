...
ESTの日時文字列が与えられたとき、時間に12時間を加えた日時を文字列として出力してください。タイムゾーンはそのまま保持してください。

例の入力: `"March 6 2009 7:30pm EST"`

例の出力: `"March 7 2009 7:30am EST"`

# --hints--

`add12Hours` は関数である必要があります。

```js
assert(typeof add12Hours === 'function');
```

`add12Hours(dateString)` は文字列を返す必要があります。

```js
assert(typeof add12Hours('January 17 2017 11:43am EST') === 'string');
```

`add12Hours("January 17 2017 11:43am EST")` は `"January 17 2017 11:43pm EST"` を返す必要があります。
...
日付の変わり目を処理する必要があります。`add12Hours("March 6 2009 7:30pm EST")` は `"March 7 2009 7:30am EST"` を返す必要があります。

```js
assert(add12Hours('March 6 2009 7:30pm EST') === 'March 7 2009 7:30am EST');
```

うるう年の月の変わり目を処理する必要があります。`add12Hours("February 29 2004 9:15pm EST")` は `"March 1 2004 9:15am EST"` を返す必要があります。

```js
assert(add12Hours('February 29 2004 9:15pm EST') === 'March 1 2004 9:15am EST');
```

平年の月の変わり目を処理する必要があります。`add12Hours("February 28 1999 3:15pm EST")` は `"March 1 1999 3:15am EST"` を返す必要があります。

```js
assert(add12Hours('February 28 1999 3:15pm EST') === 'March 1 1999 3:15am EST');
```

年の変わり目を処理する必要があります。`add12Hours("December 31 2020 1:45pm EST")` は `"January 1 2021 1:45am EST"` を返す必要があります。
