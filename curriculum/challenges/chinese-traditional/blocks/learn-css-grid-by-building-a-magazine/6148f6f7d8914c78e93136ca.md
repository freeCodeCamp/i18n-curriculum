...
為 `only screen` 創建一個最終的 `@media` 查詢，`max-width` 為 `420px`。在其中，為 `.hero-title` 選擇器創建一個 `font-size` 屬性，設定為 `4.5rem`。

恭喜！你的雜誌現在已完成。

# --hints--

你應該有一個新的 `@media` 查詢，為 `only screen and (max-width: 420px)`。這應該是 `@media` 查詢列表中的最後一個查詢。
...
你的 `.hero-title` 選擇器應該有一個 `font-size` 屬性設定為 `4.5rem`。
