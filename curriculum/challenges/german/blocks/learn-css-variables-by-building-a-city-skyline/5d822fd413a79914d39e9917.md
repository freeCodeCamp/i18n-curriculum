...
Fügen Sie einen `repeating-linear-gradient` unter dem vorhandenen `.fb1c` hinzu; verwenden Sie Ihren `--building-color4` von `0%` bis `10%` und `--window-color4` von `10%` bis `90%`. Dadurch wird der Bereich hinter dem zuletzt hinzugefügten Verlauf ausgefüllt.
