...
除了 `Go Premium` CTA，其他內容都在 `div` 中。這使得使用 CSS grid 成為排列所有內容的好選項。

為你的 `div` 添加類別 `grid`、`grid-rows-[auto_auto_auto_1fr]` 和 `gap-y-2`，該 `div` 包裹著 `h2`、`p` 和 `ul` 元素。

# --hints--

包裹 `h2`、`p` 和 `ul` 元素的 `div` 元素應該具有類別 `grid`。
...
包裹 `h2`、`p` 和 `ul` 元素的 `div` 元素應該有類別 `grid-rows-[auto_auto_auto_1fr]`。
...
包裹 `h2`、`p` 和 `ul` 元素的 `div` 元素應該有類別 `gap-y-2`。
