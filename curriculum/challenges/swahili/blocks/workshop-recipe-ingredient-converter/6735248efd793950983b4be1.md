...
Weka sifa ya `conversionTable` kwa `ounce` yako, na weka thamani yake kuwa kitu chenye sifa ya `cup` iliyowekwa kuwa `0.125`, sifa ya `gram` iliyowekwa kuwa `28.35`, na sifa ya `teaspoon` iliyowekwa kuwa `6`.

# --hints--

Kitu chako cha `conversionTable` kinapaswa kuwa na sifa ya `ounce`.

```js
assert.property(conversionTable, "ounce");
```

Sifa yako ya `ounce` inapaswa kuwa kitu.

```js
assert.isObject(conversionTable.ounce);
```

Kitu chako cha `ounce` kinapaswa kuwa na sifa ya `cup`.

```js
assert.property(conversionTable.ounce, "cup");
```

Sifa yako ya `cup` inapaswa kuwekwa kuwa `0.125`.

```js
assert.strictEqual(conversionTable.ounce?.cup, 0.125);
```

Kitu chako cha `ounce` kinapaswa kuwa na sifa ya `gram`.

```js
assert.property(conversionTable.ounce, "gram");
```

Sifa yako ya `gram` inapaswa kuwekwa kuwa `28.35`.

```js
assert.strictEqual(conversionTable.ounce?.gram, 28.35);
```

Kitu chako cha `ounce` kinapaswa kuwa na sifa ya `teaspoon`.

```js
assert.property(conversionTable.ounce, "teaspoon");
```

Sifa yako ya `teaspoon` inapaswa kuwekwa kuwa `6`.
