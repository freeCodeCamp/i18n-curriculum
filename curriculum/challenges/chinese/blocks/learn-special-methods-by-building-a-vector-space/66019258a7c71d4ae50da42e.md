...
仍然有两个可能的比较需要实现。`__le__` 方法在使用 `<=` 操作符比较两个对象时被调用。

定义一个带有 `self` 和 `other` 参数的 `__le__` 方法，并使其返回 `self > other` 的相反值。

# --hints--

你应该在 `R2Vector` 类中定义一个 `__le__` 方法。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__le__")`)) })
```

你的 `__le__` 方法应该有两个参数，`self` 和 `other`。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__le__").has_args("self, other")`)) })
```

你的 `__le__` 方法应返回 `self > other` 的相反结果。
