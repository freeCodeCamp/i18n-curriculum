...
No primeiro, adicione um `label` com o `className` de `label`, um `htmlFor` de `pac` e o texto `PAC`, depois uma entrada com um `id` de `pac`, um `className` de `input` e um `type` de `number`.
