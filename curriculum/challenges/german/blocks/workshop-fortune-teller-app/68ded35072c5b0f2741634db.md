...
Sie benötigen eine weitere Funktion, die Elemente im DOM anzeigt. Ähnlich wie bei der letzten Funktion erstellen Sie eine Funktion namens `showElements`. Verwenden Sie diesmal `classList.remove("hidden")`.

# --hints--

Sie sollten eine Funktion namens `showElements` haben.
...
Ihre `showElements`-Funktion sollte einen Parameter namens `...elements` mit dem Typ `HTMLElement[]` haben.
...
Ihre `showElements`-Funktion sollte eine `forEach`-Methode haben, die an den `elements`-Parameter angehängt ist.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Ihre `forEach`-Methode sollte einen Parameter namens `el` annehmen.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Sie sollten die `hidden`-Klasse mit der `classList.remove`-Methode innerhalb der `forEach`-Methode entfernen.
