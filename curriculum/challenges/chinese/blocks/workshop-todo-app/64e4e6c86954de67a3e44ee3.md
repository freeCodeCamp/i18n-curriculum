...
你需要使用 `getElementById()` 方法访问更多元素。 这次你需要 `close-task-form-btn`、`add-or-update-task-btn` 和 `cancel-btn` 元素。 将它们保存在变量 `closeTaskFormBtn`、`addOrUpdateTaskBtn` 和 `cancelBtn` 中。

# --hints--

你应该使用 `getElementById()` 来访问 `close-task-form-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)close\-task\-form\-btn\1\s*\)\s*;?/)
```

你应该将 `close-task-form-btn` 元素分配给变量 `closeTaskFormBtn`。 不要忘记使用 `const` 来声明变量。

```js
assert.match(code, /const\s+closeTaskFormBtn\s*=\s*document\.getElementById\(\s*('|"|`)close\-task\-form\-btn\1\s*\)\s*;?/)
```

你应该使用 `getElementById()` 来访问 `add-or-update-task-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)add\-or\-update\-task\-btn\1\s*\)\s*;?/)
```

你应该将 `add-or-update-task-btn` 元素分配给变量 `addOrUpdateTaskBtn`。 不要忘记使用 `const` 来声明变量。

```js
assert.match(code, /const\s+addOrUpdateTaskBtn\s*=\s*document\.getElementById\(\s*('|"|`)add\-or\-update\-task\-btn\1\s*\)\s*;?/)
```

你应该使用 `getElementById()` 来访问 `cancel-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)cancel\-btn\1\s*\)\s*;?/)
```

你应该将 `cancel-btn` 元素分配给变量 `cancelBtn`。 不要忘记使用 `const` 来声明变量。
