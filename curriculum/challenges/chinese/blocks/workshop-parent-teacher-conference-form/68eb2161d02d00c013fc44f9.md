...
为了使 `legend` 文本更突出，你需要对其进行样式设置。

选择 `fieldset legend` 元素。将其 `font-size` 设置为 `1.3rem`，`font-weight` 设置为 `600`。
