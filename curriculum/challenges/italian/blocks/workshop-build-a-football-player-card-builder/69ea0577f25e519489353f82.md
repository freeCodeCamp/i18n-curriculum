...
Nel blocco `catch`, registra nella console la stringa `Failed to load player data, using defaults:` seguita dal parametro `error`.

Poi, all'esterno di `catch` ma ancora all'interno della funzione `loadPlayer`, restituisci `defaultPlayer`.

Tutto questo caricherebbe i dati del giocatore, li popolerebbe nell'interfaccia utente e caricherebbe l'oggetto `defaultPlayer` se il processo fallisce.

# --hints--

Il tuo blocco `catch` dovrebbe registrare `'Failed to load player data, using defaults:', error` nella console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

La tua funzione `loadPlayer` dovrebbe restituire `defaultPlayer` come valore di fallback.
