...
تعيد الدالة `all` القيمة `True` إذا كانت كل العناصر من القابل للتكرار الممرر إليها صحيحة، و`False` خلاف ذلك:
...
مرر القائمة `[isinstance(i, str) for i in medications]` إلى الدالة `all` لتتأكد من أن كل عنصر فيها هو سلسلة نصية.

# --hints--

يجب تعديل قيمة المفتاح `medications` بتمرير `[isinstance(i, str) for i in medications]` إلى الدالة `all`.
