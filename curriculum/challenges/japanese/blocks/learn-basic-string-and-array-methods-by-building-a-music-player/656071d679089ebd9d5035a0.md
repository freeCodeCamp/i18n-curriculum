...
ユーザーがプレイリストの曲をシャッフルしたり削除したりできるようにするため、元の`allSongs`配列を変更せずにコピーを作成する必要があります。ここで`spread`演算子が役立ちます。

スプレッド演算子（`...`）を使うと、ある配列のすべての要素を別の配列にコピーできます。また、複数の配列を1つに連結することも可能です。以下の例では、`arr1`と`arr2`の両方が`combinedArr`にスプレッドされています。
...
`userData`オブジェクトの中に`songs`プロパティを作成してください。値には`allSongs`を配列にスプレッドしてください。

# --hints--

あなたの`userData`オブジェクトには`songs`というキーがあるはずです。

```js
assert.property(userData, "songs");
```

あなたの`songs`プロパティの値は`[...allSongs]`であるべきです。
