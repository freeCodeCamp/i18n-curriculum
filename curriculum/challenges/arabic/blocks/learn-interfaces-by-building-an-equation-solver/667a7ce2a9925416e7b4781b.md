...
تُتيح مطابقة النمط الهيكلي التحقق من أن الموضوع له بنية محددة. بالإضافة إلى ذلك، تربط الأسماء في النمط بعناصر الموضوع. على سبيل المثال:
...
عدّل بناء `match`/`case` ليطابق `results` بدلاً من `len(results)`. ثم عدّل كل `case` لاستخدام قائمة تحتوي على العدد المناسب من العناصر. استخدم `x` للحالة التي تحتوي فيها القائمة على عنصر واحد، و`x1` و`x2` للحالة التي تحتوي فيها القائمة على عنصرين.

أخيرًا، عدّل سلاسل f لاستخدام أسماء المتغيرات المستخدمة في كل `case`.

# --hints--

يجب تعديل جملة `match` لاستخدام `results` كقيمة للموضوع.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("results")`)) })
```

يجب تعديل أول `case` لاستخدام النمط `[]`.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_cases()[0].find_case_pattern().is_equivalent("[]")`)) })
```

لا يجب تعديل جسم أول `case`.
...
يجب تعديل ثاني `case` لاستخدام النمط `[x]`.
...
يجب تعديل سلسلة f الموجودة داخل `result_list` لاستخدام `x` بدلًا من `result[0]`.
...
يجب تعديل ثالث `case` لاستخدام قائمة تحتوي على `x1` و`x2` كنمط.
...
يجب تعديل سلاسل f الموجودة داخل `result_list` لاستخدام المتغيرات المرتبطة من نمطك.
