...
Для останньої частини гри вам потрібно створити алгоритм, який перевіряє наявність стрейту. Малий стрейт — це коли чотири кубики мають послідовні значення в будь-якому порядку (наприклад, у кидку `41423` ми маємо `1234`), що дає результат у `30` очки. Великий стрейт — це коли всі п’ять кубиків мають послідовні значення в будь-якому порядку (наприклад, у кидку `35124` ми маємо `12345`), що дає результат у `40` очок.

Оголосіть функцію `checkForStraights`, яка приймає масив чисел. Якщо користувач отримує великий стрейт, оновіть п’яту радіокнопку зі значенням `40`. Якщо користувач отримує малий стрейт, оновіть четверту радіокнопку зі значенням `30`. У будь-якому разі завжди оновлюйте останню радіокнопку, щоб відобразити значення 0 з правильними атрибутами.

Викличте вашу функцію `checkForStraights` при натисканні на `rollDiceBtn`, щоб завершити вашу гру з кубиками!

# --hints--

Ваша змінна `checkForStraights` має бути функцією.

```js
assert.isFunction(checkForStraights);
```

Якщо випав малий стрейт, ваша функція `checkForStraights` має увімкнути четверту радіокнопку, встановити значення `30`, оновити відображений текст на `, score = 30` і залишити п’яту радіокнопку вимкненою.
...
Якщо випав великий стрейт, ваша функція `checkForStraights` має увімкнути четверту кнопку, встановити значення `30`, оновити відображений текст на `, score = 30`. Крім того, функція має увімкнути п’яту радіокнопку, встановити значення `40` і оновити відображений текст на `, score = 40`.
...
Якщо стрейт не випав, ваша функція `checkForStraights` не має увімкнути четверту або п’яту радіокнопку.
...
Якщо стрейт не випав, ваша функція `checkForStraights` має увімкнути фінальну радіокнопку, встановити значення `0` і оновити відображений текст на `, score = 0`.
...
Ви маєте викликати функцію `checkForStraights` при натисканні на `rollDiceBtn`.
