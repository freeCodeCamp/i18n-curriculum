...
`card-stats` div 내부에서, 첫 번째 `stat-col` 다음에 자식 요소 두 개를 더 추가해야 합니다.

먼저, 두 통계 열 사이에 얇은 세로선을 렌더링하기 위해 `stat-divider`의 `className`를 가진 자체 종료 `div`를 추가하고, 두 번째로 `stat-col`의 `className`를 가진 `div`를 추가하세요. 이 `div`는 각각 `stat-row`의 `className`를 가진 세 개의 div 요소를 포함합니다.

# --hints--

첫 번째 `.stat-col` 다음에 `stat-divider`의 `className`를 가진 자체 종료 `div`를 추가하세요.

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

`.stat-divider` 다음에 `stat-col`의 `className`를 가진 두 번째 `div`를 추가하세요.

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

두 번째 `.stat-col`는 각각 `stat-row`의 `className`를 가진 세 개의 `div` 요소를 포함해야 합니다.
