...
Aquí tienes un ejemplo de un elemento `input` con un marcador de posición establecido en `Ex. Jane Doe`:
...
Your `input` element does not have a `placeholder` attribute. Comprueba que hay un espacio después del nombre de la etiqueta de apertura y/o que hay espacios antes de todos los nombres de los atributos.

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('placeholder'));
```

Your `input` element should have a `placeholder` attribute with the value `cat photo URL`.  Has omitido el valor o tienes un error de escritura. Recuerda que los valores de los atributos deben estar entre comillas.
