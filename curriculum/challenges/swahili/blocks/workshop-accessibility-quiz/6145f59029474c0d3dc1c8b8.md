...
Kama ilivyo kwa vipengele vingine vya `input` na `label`, unganisha `textarea` na kipengele chake kinacholingana cha `label`, na umpe sifa ya kipengele ya `name`.

# --hints--

Unapaswa kumpa kipengele cha `textarea` sifa ya kipengele ya `id`.

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id);
```

Huwezi kujumuisha nafasi katika `id` kwa ajili ya kipengele cha `textarea`.

```js
assert.match(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id, /^\S+$/);
```

Lazima uchague `id` tofauti kwa kipengele cha `textarea` kwa sababu kipengele kingine kinatumia `id` hiyo.
...
Unapaswa kumpa kipengele cha `label` sifa ya kipengele ya `for` inayolingana na `id` ya kipengele cha `textarea`.
...
Unapaswa kumpa kipengele cha `textarea` sifa ya kipengele ya `name`.

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.name);
```

Unapaswa kutumia angalau mhusika mmoja asiye nafasi katika sifa ya kipengele ya `name` kwa ajili ya `textarea`.
