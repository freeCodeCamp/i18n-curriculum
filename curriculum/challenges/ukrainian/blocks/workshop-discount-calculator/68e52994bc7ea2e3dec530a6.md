...
Тепер давайте перевіримо повну систему знижок! Додайте інструкцію `if`, яка перевіряє, чи `__name__ == '__main__'`, і перемістіть `product = Product('Wireless Mouse', 50.0)` всередину неї.

Після цього, всередині блоку `if`, створіть змінну `user_tier` і встановіть її в рядок `Premium`. Потім створіть змінну `strategies`, встановлену як список стратегій, що містить `PercentageDiscount(10)`, `FixedAmountDiscount(5)` і `PremiumUserDiscount()`.
...
Ви повинні перемістити `product = Product('Wireless Mouse', 50.0)` всередину блоку `if`.
...
Ви повинні встановити `user_tier` в рядок `Premium` всередині блоку `if`.
...
Ви повинні створити список з іменем `strategies`, що містить `PercentageDiscount(10)`, `FixedAmountDiscount(5)` і `PremiumUserDiscount()`.
