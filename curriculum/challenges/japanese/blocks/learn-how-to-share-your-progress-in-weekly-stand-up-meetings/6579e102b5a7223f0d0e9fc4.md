---
id: 6579e102b5a7223f0d0e9fc4
title: タスク 52
challengeType: 22
dashedName: task-52
lang: en-US
---

<!-- (Audio) Bob: I agree with you. It might help to have a clear style guide for each language. -->

# --description--

単語 `each` は、グループ内の各項目を個別に考えたものすべてを指します。グループ内の個々の注意や区別を強調するためによく使われます。たとえば、`Each team member has a specific role` はチームの全員がそれぞれ独自の役割を持っていることを強調しています。

# --fillInTheBlank--

## --sentence--

`I BLANK with you. It might help to have a BLANK style guide for BLANK language.`

## --blanks--

`agree`

### --feedback--

これはボブがソフィーの提案に同意していることを示しています。

---

`clear`

### --feedback--

理解しやすく正確なガイドの必要性を強調しています。

---

`each`

### --feedback--

彼らが使うすべてのプログラミング言語を個別に指しています。

# --scene--

```json
{
  "setup": {
    "background": "company1-boardroom.png",
    "characters": [
      {
        "character": "Bob",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "6.2-3.mp3",
      "startTime": 1,
      "startTimestamp": 20.08,
      "finishTimestamp": 23.6
    }
  },
  "commands": [
    {
      "character": "Bob",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Bob",
      "startTime": 1,
      "finishTime": 4.52,
      "dialogue": {
        "text": "I agree with you. It might help to have a clear style guide for each language.",
        "align": "center"
      }
    },
    {
      "character": "Bob",
      "opacity": 0,
      "startTime": 5.02
    }
  ]
}
```
