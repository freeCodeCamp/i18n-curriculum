...
Sie können endlich damit beginnen, Ihre Logik mit der Benutzeroberfläche zu verbinden. Sie müssen das DOM abfragen, um einige Elemente anhand ihrer ID zu erhalten. Holen Sie die Elemente mit den IDs `ingredient`, `quantity`, `unit`, `servings`, `recipe-form` und `result-list` und weisen Sie sie den Variablen `ingredientName`, `ingredientQuantity`, `unitToConvert`, `numberOfServings`, `recipeForm` und `resultList` zu.

# --hints--

Sie sollten eine `ingredientName`-Variable definieren.

```js
assert.isDefined(ingredientName);
```

Ihre `ingredientName`-Variable sollte das Element sein, das von `.getElementById()` mit der ID `ingredient` zurückgegeben wird.

```js
assert.strictEqual(ingredientName, document.getElementById('ingredient'));
```

Sie sollten eine `ingredientQuantity`-Variable definieren.

```js
assert.isDefined(ingredientQuantity);
```

Ihre `ingredientQuantity`-Variable sollte das Element sein, das von `.getElementById()` mit der ID `quantity` zurückgegeben wird.

```js
assert.strictEqual(ingredientQuantity, document.getElementById('quantity'));
```

Sie sollten eine `unitToConvert`-Variable definieren.

```js
assert.isDefined(unitToConvert);
```

Ihre `unitToConvert`-Variable sollte das Element sein, das von `.getElementById()` mit der ID `unit` zurückgegeben wird.

```js
assert.strictEqual(unitToConvert, document.getElementById('unit'));
```

Sie sollten eine `numberOfServings`-Variable definieren.

```js
assert.isDefined(numberOfServings);
```

Ihre `numberOfServings`-Variable sollte das Element sein, das von `.getElementById()` mit der ID `servings` zurückgegeben wird.

```js
assert.strictEqual(numberOfServings, document.getElementById('servings'));
```

Sie sollten eine `recipeForm`-Variable definieren.

```js
assert.isDefined(recipeForm);
```

Ihre `recipeForm`-Variable sollte das Element sein, das von `.getElementById()` mit der ID `recipe-form` zurückgegeben wird.

```js
assert.strictEqual(recipeForm, document.getElementById("recipe-form"));
```

Sie sollten eine `resultList`-Variable definieren.

```js
assert.isDefined(resultList);
```

Ihre `resultList`-Variable sollte das Element sein, das von `.getElementById()` mit der ID `result-list` zurückgegeben wird.
