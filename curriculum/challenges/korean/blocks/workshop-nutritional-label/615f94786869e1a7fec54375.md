...
`.note` 선택자를 만들고 폰트 크기를 `0.6rem`로 설정하세요. 또한 위쪽과 아래쪽 여백을 `5px`로 설정하고 왼쪽과 오른쪽 여백은 제거하세요.

# --hints--

새로운 `.note` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.note'));
```

`.note` 선택자에 `font-size` 속성이 `0.6rem`로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.note')?.fontSize === '0.6rem');
```

`.note` 선택자에 `margin` 속성이 `5px 0`로 설정되어야 합니다.
