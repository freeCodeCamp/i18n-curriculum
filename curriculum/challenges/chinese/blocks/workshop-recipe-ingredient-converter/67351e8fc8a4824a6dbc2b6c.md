...
为你的 `conversionTable` 添加一个 `cup` 属性，并赋值一个对象，该对象的 `gram` 属性设置为 `240`，`ounce` 属性设置为 `8.0`，`teaspoon` 属性设置为 `48`。

# --hints--

你的 `conversionTable` 应该有一个 `cup` 属性。
...
你的 `cup` 对象应有一个 `teaspoon` 属性。

```js
assert.property(conversionTable.cup, "teaspoon");
```

你的 `teaspoon` 属性应设置为 `48`。
