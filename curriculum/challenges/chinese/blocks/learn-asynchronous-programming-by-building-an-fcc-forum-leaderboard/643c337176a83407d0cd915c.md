...
要查看数据结果，可以在 `try` 块中将 `data` 变量打印到控制台。 

在 `fetchData` 定义的下面调用这个函数，然后打开控制台查看结果。

# --hints--

你应该使用 `console.log` 将 `data` 变量记录到控制台。

```js
assert.match(code, /\s*console\.log\(\s*data\s*\)/);
```

你的 `console.log` 应该放在 `try` 块里面。

```js
assert.match(code, /\s*try\s*{[\s\S]*\s*console\.log\(\s*data\s*\)[\s\S]*}\s*catch\s*\(\s*err\s*\)\s*{\s*}/);
```

你应该在定义 `fetchData()` 函数之后，再调用 `fetchData()` 函数。
