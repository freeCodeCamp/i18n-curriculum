...
次に、ラジオボタンがチェックされたときに表示される内側の円を作成してください。これには`::before`疑似要素を使います。

`::before`のような疑似要素は、実際の要素の前に追加のコンテンツを挿入できます。これは装飾目的でよく使われます。

`.contact-method-radio-btn::before`疑似要素をターゲットにして、`display`を`block`に、`content`を`" "`に、`width`を`10px`に、`height`を`10px`に、`border-radius`を`50%`に設定してください。

# --hints--

`.contact-method-radio-btn::before`セレクターがあるはずです。
...
`border-radius`プロパティを`50%`に設定してください。
