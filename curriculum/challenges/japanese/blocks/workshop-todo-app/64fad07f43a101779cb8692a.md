...
コードの可読性と保守性を高めるために、`submit` イベントリスナーを2つの別々の関数にリファクタリングできます。最初の関数は入力値を `taskData` に追加するために使い、2つ目の関数はタスクをDOMに追加する役割を担います。

アロー構文を使って `addOrUpdateTask` 関数を作成してください。次に `dataArrIndex` 変数、`taskObj` オブジェクト、`if` 文を `addOrUpdateTask` 関数に移動してください。

# --hints--

`const` とアロー構文を使って `addOrUpdateTask` 関数を作成してください。

```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

`dataArrIndex` 変数を `addOrUpdateTask` 関数に移動してください。
...
`taskObj` オブジェクトを `addOrUpdateTask` 関数に移動してください。
...
条件 `if` を持つ `dataArrIndex === -1` 文を `addOrUpdateTask` 関数に移動してください。
