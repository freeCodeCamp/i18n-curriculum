...
在嵌套的 `for` 循环之后，你找到了最小的值。 您需要将其与当前值交换。

就像你在冒泡排序中所做的那样，使用 `temp` 变量提取 `array[i]` 的值，然后交换 `array[i]` 和 `array[minIndex]` 的值。

# --hints--

在嵌套的 `for` 循环之后，你应声明一个 `temp` 变量。
...
你应该将 `array[minIndex]` 分配给 `array[i]`。
