...
`.sr-only` のテキストはまだ表示されています。スクリーンリーダーだけが読み取れるようにテキストを視覚的に隠す一般的なパターンがあります。

このパターンは、次のCSSプロパティを設定することです：
