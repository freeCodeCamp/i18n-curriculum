...
Створіть селектор `a` і надайте йому множину властивостей `text-decoration`, встановлену в `none`, а також множину властивостей `color`, встановлену в `linen`.
...
Ваш селектор `a` має мати властивість `text-decoration`, встановлену в `none`.

```js
assert(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration === 'none');
```

Ваш селектор `a` має мати властивість `color`, встановлену в `linen`.
