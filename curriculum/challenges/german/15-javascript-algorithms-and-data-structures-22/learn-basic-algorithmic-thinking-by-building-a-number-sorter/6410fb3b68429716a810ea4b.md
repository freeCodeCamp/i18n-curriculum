...
Beim Selectionsort wird der Index des kleinsten Wertes im Array verfolgt. Deklariere eine `minIndex`-Variable und setze sie auf `i` - dies stellt sicher, dass dein aktueller Wert, wenn er der kleinste ist, mit sich selbst ausgetauscht und nicht verschoben wird. Du musst in der Lage sein, den Wert von `minIndex` neu zuzuweisen, während du das Array durchläufst.

Schreibe dann eine weitere `for`-Schleife, wobei du `j` als Iterator verwendest. Diese Schleife muss beim Index nach `i` beginnen und den Rest des Arrays durchlaufen.

# --hints--

Du solltest `let` verwenden, um `minIndex` zu deklarieren.
...
Du solltest `minIndex` auf `i` setzen.
...
Du solltest eine verschachtelte `for`-Schleife haben.
...
Deine `for`-Schleife sollte `j` mit `i + 1` initialisieren.
...
Deine `for`-Schleife sollte den Rest des Arrays durchlaufen.
...
Deine `for`-Schleife sollte `j` erhöhen.
