...
يمكن أن تحتوي التعبيرات النمطية على متتاليات خاصة تتكون من شرطة مائلة عكسية (`\`) تليها حرف. لهذه المتتاليات معنى خاص. على سبيل المثال، `\d` تطابق رقمًا عشريًا.
...
بعد الحرف `p`، يجب أن يحتوي `patient_id` على متتالية من الأرقام. لذا، عدل نمط التعبير النمطي ليشمل الحرف `p` متبوعًا بالمتتالية الخاصة `\d`.

# --hints--

يجب تعديل نمط التعبير النمطي ليكون `p\d`.
