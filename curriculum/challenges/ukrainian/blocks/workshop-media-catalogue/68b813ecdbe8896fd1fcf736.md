...
Використайте метод `add` об’єкта `catalogue`, щоб додати `series1` до списку `items`.

Потім створіть іншу змінну `series2`. Призначте їй ще один екземпляр `TVSeries` і також додайте `series2` до каталогу.

# --hints--

Ви повинні використати метод `add` об’єкта `catalogue`, передаючи `series1`.
...
Ви повинні створити змінну з іменем `series2` і призначити їй екземпляр `TVSeries`.
...
Ви повинні використати метод `add` об’єкта `catalogue`, передаючи `series2`.
