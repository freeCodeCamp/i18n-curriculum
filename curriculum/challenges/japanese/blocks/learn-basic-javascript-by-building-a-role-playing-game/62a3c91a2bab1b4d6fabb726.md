...
`goCave` 関数を作成して、`"Going to cave."` をコンソールに出力してください。

# --hints--

`function` を宣言するには `goCave` キーワードを使う必要があります。
...
コードには `console.log("Going to cave.");` 行が必要です。

```js
assert.match(code, /console\.log\(\s*('|")Going to cave\.\1\s*\)/);
```

`console.log("Going to cave.");` 行は `goCave` 関数の中にあるべきです。
