...
上記は `id` が `example` でないすべての `div` 要素を選択します。

`.daily-value p` セレクターを変更して `.no-divider` の要素を除外してください。

# --hints--

`.daily-value p:not(.no-divider)` セレクターが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p:not(.no-divider)'));
```

`.daily-value p` セレクターが無いようにしてください。

```js
assert(!new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

`.daily-value p:not(.no-divider)` セレクター内のプロパティを変更しないでください。
