...
Ongeza kitu cha pili chenye funguo na thamani zifuatazo:
...
Safu yako ya `allSongs` inapaswa kuwa na thamani mbili.

```js
assert.equal(allSongs.length, 2);
```

Safu yako ya `allSongs` inapaswa kuwa na kitu kama thamani yake ya pili.

```js
assert.isObject(allSongs[1]);
```

Kitu cha pili katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `id` iliyowekwa kwa nambari `1`.

```js
assert.strictEqual(allSongs[1].id, 1);
```

Kitu cha pili katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `title` iliyowekwa kwa mfuatano wa herufi `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

Kitu cha pili katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `artist` iliyowekwa kwa mfuatano wa herufi `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

Kitu cha pili katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `duration` iliyowekwa kwa mfuatano wa herufi `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

Kitu cha pili katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `src` iliyowekwa kwa mfuatano wa herufi `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
