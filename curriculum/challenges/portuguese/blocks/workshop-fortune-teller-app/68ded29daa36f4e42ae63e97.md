...
Finalize a função usando o método `forEach` do array para iterar sobre o argumento elements. Dentro da função de callback, adicione a classe hidden a cada elemento usando o método `.add()` de `classList`.

# --hints--

Você deve ter um método `forEach` anexado ao parâmetro `elements`.
...
Seu método forEach deve receber um parâmetro de callback.
...
Dentro do método forEach, você deve adicionar a classe hidden usando o método `classList.add`.
