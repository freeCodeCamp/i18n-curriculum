...
現在你需要縮小這些元素的尺寸。 Give your `span[class~="sr-only"]` selector a `width` and `height` property set to `1px`.

# --hints--

`span[class~="sr-only"]` 選擇器應該有一個 `width` 屬性設置爲 `1px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('width') === '1px');
```

`span[class~="sr-only"]` 選擇器應該有一個 `height` 屬性設置爲 `1px`。
