...
Ahora que la interfaz `PlayerData` está definida, actualiza el componente `PlayerCard` para que acepte una prop `player` tipada como `PlayerData` para que coincida con la interfaz `PlayerData`.

Recuerda que puedes desestructurar las props así:
...
Debes actualizar `PlayerCard` para que acepte una prop `player` tipada como `PlayerData`.
