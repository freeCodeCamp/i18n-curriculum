...
Definisci una funzione `pauseSong` usando la parola chiave `const` e la sintassi della funzione freccia. La funzione non deve avere parametri.

# --hints--

Dovresti usare `const` per creare una funzione vuota chiamata `pauseSong`.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` dovrebbe essere una funzione.

```js
assert.isFunction(pauseSong)
```

La tua funzione `pauseSong` dovrebbe usare la sintassi della funzione freccia.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

La tua funzione `pauseSong` dovrebbe essere vuota.
