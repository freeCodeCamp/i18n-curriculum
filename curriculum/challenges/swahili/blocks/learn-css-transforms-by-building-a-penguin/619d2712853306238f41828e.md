...
Tengeneza vigezo maalum vya CSS vinavyoitwa `--penguin-skin`, na viweke kuwa `gray`. Kisha, badilisha thamani zote zinazohusiana za sifa kwa kutumia vigezo hivyo.

# --hints--

Unapaswa kumpa `:root` sifa ya `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

Unapaswa kumpa `--penguin-skin` thamani ya `gray`, lakini umeipata `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

Unapaswa kumpa `.penguin-head` sifa ya `background` yenye thamani `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
Unapaswa kumpa `.arm` sifa ya `background` yenye thamani `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
