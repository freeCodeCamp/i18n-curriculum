...
Debajo de tu elemento `.text`, crea un nuevo elemento `section` y asígnale una `clase` de `text text-with-images`. Dentro de eso, crea un elemento `article` con una `clase` establecida a `brief-history`, y un elemento `aside` con la `clase` establecida a `image-wrapper`.
...
Tu nuevo elemento `section` debe venir después de tu elemento `.text`.
...
Tu nuevo elemento `section` debe tener un elemento `article`.

```js
assert.exists(document.querySelector('.text-with-images article'));
```

Tu nuevo elemento `section` debe tener un elemento `aside`.

```js
assert.exists(document.querySelector('.text-with-images aside'));
```

El elemento `article` debe venir antes del elemento `aside`.
