...
このプロジェクトを完成させるには、`clearButton` ボタンにイベントリスナーを追加してください。ボタンがクリックされたときに、`clearForm` 関数を呼び出す必要があります。
...
`"click"` の最初の引数として `.addEventListener()` を渡してください。

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

`clearForm` の2番目の引数として `.addEventListener()` を渡してください。
