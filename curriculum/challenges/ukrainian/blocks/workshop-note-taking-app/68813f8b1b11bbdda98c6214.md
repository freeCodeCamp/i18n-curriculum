...
Оскільки для нотатки використовується елемент `div`, читачі екрана не зможуть зрозуміти його призначення. Ви можете виправити це, додавши атрибут `aria-label`. Це допоможе з доступністю та надасть значення цьому елементу `div`. 

У вашому відкриваючому тегу `div` додайте атрибут `aria-label` і встановіть його значення на `"Note editor"`.

# --hints--

Ваш відкриваючий тег `div` повинен мати атрибут `aria-label`.

```js
assert.isTrue(document.getElementById("note").hasAttribute("aria-label"));
```

Ваш відкриваючий тег `div` повинен мати атрибут `aria-label`, встановлений у `"Note editor"`.
