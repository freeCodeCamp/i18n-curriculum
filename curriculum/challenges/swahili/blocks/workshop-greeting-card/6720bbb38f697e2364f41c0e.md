...
Sasa ni wakati wa rangi. Toa `body` `background-color` ya `brown` na pia mpa `.card` `background-color` ya `white`.

# --hints--

Kipengele cha `body` kinapaswa kuwa na `background-color` ya `brown`.
...
Unapaswa kuwa na kichaguzi cha `.card`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card"));
```

Kipengele cha `.card` kinapaswa kuwa na `background-color` ya `white`.
