...
Метод `sort()` приймає функцію зворотного виклику порівняння, яка визначає порядок сортування.

У цьому прикладі перша умова `(a.name < b.name)` перевіряє, чи назва першого фрукта менша за назву другого фрукта. Якщо так, перший плід сортується перед другим.

Рядки порівнюються лексикографічно, тобто вони порівнюються символ за символом. Наприклад, `"Apples"` менше, ніж `"Bananas"`, оскільки `"A"` стоїть перед `"B"` у алфавіті.

Причина, чому цей приклад повертає числа, полягає в тому, що метод `sort()` очікує, щоб число було повернено. Якщо ви повертаєте від’ємне число, перший елемент сортується перед другим.
...
У функцію зворотного виклику додайте інструкцію `if`, щоб перевірити, чи `a.title` є меншим за `b.title`. Якщо так, поверніть `-1`.

# --hints--

Функція зворотного виклику повинна містити інструкцію `if`.

```js
assert.match(code, /if\s*\(/);
```

Інструкція `if` повинна перевіряти, чи `a.title` менше `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

Інструкція `if` має повернути `-1`, якщо `a.title` менше `b.title`.
