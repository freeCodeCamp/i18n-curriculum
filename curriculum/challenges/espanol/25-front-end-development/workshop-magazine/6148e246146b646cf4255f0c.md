...
Crea un selector `.list-title, .list-subtitle` y establece la propiedad `color` a `#00beef`.

# --hints--

Debes tener un selector `.list-title, .list-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'))
```

Tu selector `.list-title, .list-subtitle` debe tener una propiedad `color` establecida en `#00beef`.
