...
**目标：**构建一个功能类似于 <a href="https://roman-numeral-converter.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://roman-numeral-converter.freecodecamp.rocks</a> 的应用。

**用户需求：**

1. 你应该有一个 `input` 元素，其中 `id` 为 `"number"`。
1. 你应该有一个 `button` 元素，其中包含一个 `id` 元素 `"convert-btn"`。
1. 你应该有一个 `div`、`span` 或 `p` 元素，以及一个 `id` 为 `output`。
1. 当你单击 `#convert-btn` 元素而不在 `#number` 元素中输入值时，`#output` 元素应该包含文本 `"Please enter a valid number"`。
1. 当 `#number` 元素包含数字 `-1` 且单击 `#convert-btn` 元素时，`#output` 元素应包含文本 `"Please enter a number greater than or equal to 1"`。
1. 当 `#number` 元素包含数字 `4000` 或更大，并且单击 `#convert-btn` 元素时，`#output` 元素应包含文本 `"Please enter a number less than or equal to 3999"`。
1. 当 `#number` 元素包含数字 `9` 且单击 `#convert-btn` 元素时，`#output` 元素应包含文本 `"IX"`。
1. 当 `#number` 元素包含数字 `16`，并且 `#convert-btn` 元素被点击时，`#output` 元素应该包含文本 `"XVI"`。
1. 当 `#number` 元素包含数字 `649`，并且 `#convert-btn` 元素被点击时，`#output` 元素应该包含文本 `"DCXLIX"`。
1. 当 `#number` 元素包含数字 `1023`，并且 `#convert-btn` 元素被点击时，`#output` 元素应该包含文本 `"MXXIII"`。
1. 当 `#number` 元素包含数字 `3999`，并且 `#convert-btn` 元素被点击时，`#output` 元素应该包含文本 `"MMMCMXCIX"`。
