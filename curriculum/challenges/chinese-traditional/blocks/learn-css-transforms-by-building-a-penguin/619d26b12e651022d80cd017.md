...
將標的 `.arm` 元素設定為 `width` 為 `30%`、`height` 為 `60%`，並將 `background` 設為從順時針方向 `90deg` 的線性漸層，起始色為 `gray`，結束色為 `rgb(209, 210, 199)`。
...
你應該給 `.arm` 一個 `width` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

你應該給 `.arm` 一個 `height` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

你應該給 `.arm` 一個 `background` 為 `linear-gradient(90deg, gray, rgb(209, 210, 199))`。
