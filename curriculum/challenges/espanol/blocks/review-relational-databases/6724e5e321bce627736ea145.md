...
- **Diagramas de Relaciones de Entidad (ERDs)**: Visualiza cómo las entidades (tablas) se relacionan entre sí en un esquema de base de datos.
...
- **Cláusula WHERE**: Filtra los resultados según condiciones. Usa operadores de comparación como `<`, `=`, `>`, etc.
...
- **`ALTER TABLE ADD COLUMN` con Restricción**: Esta instrucción se usa para agregar una columna con una restricción a una tabla existente.
...
- **`ALTER TABLE ADD COLUMN` con clave foránea**: Esta sentencia se usa para agregar una columna de clave foránea que referencia a otra tabla.
...
- **Sentencia `UPDATE`**: Esta sentencia se usa para actualizar datos existentes en una tabla según una condición.
...
- **Sentencia `FULL OUTER JOIN`**: Esta unión devuelve todas las filas de ambas tablas, incluyendo las filas que no coinciden de cualquiera de las dos tablas.
...
- **Declaración `SELF JOIN`**: Esta unión se usa para unir una tabla consigo misma y comparar filas dentro de la misma tabla.
...
- **PostgreSQL**: De código abierto, avanzado y extensible. Soporta tipos de datos personalizados y programación del lado del servidor.
...
- **Help flags**: Many commands support `--help` for quick help information.
  - Alternativa a las páginas manuales para referencia rápida
...
- **Double bracket syntax**: Use `[[ ]]` for conditional testing and pattern matching.
  ...
  - Devuelve el estado de salida 0 (verdadero) o 1 (falso) según el resultado de la prueba
...
- **String comparison operators**: Compare strings using various operators within `[[ ]]`.
  ...
  - `!=` (no igual): Prueba si dos cadenas son diferentes
  - `<` (menor lexicográficamente): comparación de cadenas en orden alfabético
...
- **Numeric comparison operators**: Compare numbers using specific numeric operators.
  ...
  - `-le` (less than or equal): comparación numérica de menor o igual que
...
- **Assignment operators**: Modify variables using arithmetic assignment operators.
  ...
  - `+=` (sumar y asignar): Añade valor a la variable
  ...
  - `%=` (módulo y asignar): Asigna a la variable el resto
...
- **Conditional (ternary) operator**: Use `condition ? true_value : false_value` syntax.
  - Proporciona una forma concisa de asignar valores basándose en condiciones
...
- **Arrays**: Store multiple values in a single variable.
  ...
  - El indexado de arrays comienza en 0
...
- **Random numbers**: Generate random values using the `$RANDOM` variable.
  ...
  - Agrega 1 para evitar cero: `$(( RANDOM % 75 + 1 ))`
...
- **Random array access**: Use random numbers to access array elements randomly.
  ...
  - Usa un índice aleatorio para acceder a los elementos del array
...
- **Script execution methods**: Multiple ways to run bash scripts:
  ...
  - **`bash scriptname.sh`**: Ejecuta con el intérprete de shell bash.
...
## Permisos de Archivo y Ejecución de Scripts

- **Error de permiso denegado**: Al usar `./scriptname.sh`, puede que recibas "permission denied" si el archivo no tiene permisos de ejecución.
...
- **2NF (Second Normal Form)**
  ...
  - No hay **dependencias parciales**: cada atributo no clave debe depender de toda la clave primaria compuesta.
  - Ejemplo: Divide la tabla `orders` en `order_header` y `order_items` para evitar atributos que dependan solo de una parte de la clave.
...
- **3NF (Third Normal Form)**
  ...
  - Ejemplo: Mueve `city_postal_code` a una tabla `cities` en lugar de almacenarlo con cada pedido.
...
### Recuperando Múltiples Columnas
...
Es una vulnerabilidad de seguridad web donde los atacantes insertan código SQL malicioso en campos de entrada para manipular la base de datos.
...
1. **Usa Sentencias Preparadas**: Estas separan el código SQL de los datos, previniendo inyecciones. Aquí tienes un ejemplo (Node.js con pg):
...
**Nota**: Nunca otorgues derechos de administrador a cuentas de aplicación.
...
- Múltiples consultas pequeñas son más lentas que una consulta optimizada.
...
Para configurar la dirección de correo electrónico del usuario, puedes ejecutar el siguiente comando:
...
- **Estrategias de ramificación**: Tu rama `main` es la rama predeterminada y generalmente es bastante estable. Por eso, es mejor crear nuevas ramas a partir de ella para elementos como correcciones de errores, nuevas funcionalidades u otros trabajos diversos.
- **Conflictos de fusión**: Esto ocurre cuando Git intenta fusionar automáticamente cambios de diferentes ramas pero no puede decidir qué cambios conservar. Esto suele suceder cuando hay cambios conflictivos en la misma parte del archivo.

## Cinco estados para un archivo rastreado por Git

- **"Untracked"**: Esto significa que el archivo es nuevo en el repositorio y que Git no lo ha "visto" antes.
- **"Modificado"**: Este archivo existía en el commit anterior y tiene cambios que no han sido confirmados.
- **"Ignored"**: Probablemente no verás archivos ignorados en Git, pero tu IDE podría tener un indicador para ellos. Los archivos ignorados están excluidos de las operaciones de Git, generalmente porque están incluidos en el archivo `.gitignore`.
...
- **Subiendo Repositorios Locales a GitHub**: Si tienes un proyecto local en tu computadora, puedes subir ese repositorio a GitHub. Aquí tienes un resumen paso a paso del proceso:
...
- **Firmar Commits con Claves SSH**: Para firmar con una clave SSH, que es una función relativamente nueva en GitHub, primero tendrás que subir la clave a tu cuenta de GitHub. Luego necesitarás configurar el modo de firma para que git use SSH:
