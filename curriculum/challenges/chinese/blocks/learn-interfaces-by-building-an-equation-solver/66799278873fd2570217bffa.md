...
当 `results` 的长度为 `1` 时，添加另一个 `case`。  In this case, assign to `result_list` a list containing a string with the format `x = <root>`, where `<root>` is the solution of the equation. 格式化字符串，以便解决方案同时显示正号和负号。
...
你应该在现有的 `case` 块之后创建一个具有模式 `1` 的新 `case`。
...
你应该在新的 `case` 主体内分配一个包含 `f'x = {results[0]:+}'` 到 `result_list` 的列表。
