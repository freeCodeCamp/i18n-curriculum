...
Dentro del ciclo `for`, declara una variable `row_str` y asígnale una comprensión de listas que itere sobre `row` y convierta cada elemento `i` en `row` en una cadena. Usa la función `str()` para eso.

# --hints--

Debes declarar una variable `row_str`.
...
Debes asignar una comprensión de lista que itere sobre `row` a la variable `row_str`.
...
La comprensión de listas asignada a la variable `row_str` debe usar `i` como variable de iteración.
...
La comprensión de listas asignada a la variable `row_str` debe llamar a `str()` en cada elemento `i` de `row`.
