...
Можна швидко отримати всі клітинки зі сторінки, отримавши елемент `#container` за його `id` та отримавши властивість `children` результату. Передайте це у виклик `evalFormula()` як другий параметр.

# --hints--

Для другого параметра виклику `evalFormula()` викличте метод `.getElementById()` об’єкта `document`.
...
Передайте `container` як аргумент до виклику `.getElementById()`.
...
Отримайте доступ до властивості `children` результату виклику `.getElementById()`.
