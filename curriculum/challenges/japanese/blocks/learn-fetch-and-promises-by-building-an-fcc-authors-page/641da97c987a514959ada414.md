...
前と同じように、著者データの一部を使って`displayAuthors`関数を呼び出してください。

関数を呼び出した後に`Load More Authors`ボタンをクリックしても動作しません。これは、まだボタンに`click`イベントリスナーを追加していないからです。次にそれを行います。
...
`slice()`メソッドの最初の引数は`startingIndex`である必要があります。
...
`slice()`メソッドの2番目の引数は`endingIndex`である必要があります。
