...
Додайте атрибут `value` зі значенням `player.pac` до PAC `input`. Потім додайте атрибут `onChange`, який дорівнює стрілковій функції, що приймає `e` як параметр і виконує виклик `setPlayer` з розгортанням `player` та `pac`, встановленим у `Number(e.target.value)`.

# --hints--

Ваш PAC `input` має мати `value`, рівний показнику `pac` гравця.
...
Ваш PAC `input` має мати обробник `onChange`, який викликає `setPlayer` з розгортанням `player` та `pac`, встановленим у `Number(e.target.value)`.
