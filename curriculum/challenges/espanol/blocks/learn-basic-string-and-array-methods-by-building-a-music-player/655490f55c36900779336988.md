...
Repite a través de los elementos `playlistSongElements` con un método `forEach`.

El método `forEach` se usa para recorrer un array y realizar una función en cada elemento del array. Por ejemplo, supongamos que tienes un array de números y quieres registrar cada número en la consola.
...
Usa el método `forEach` en `playlistSongElements`. Pasa `songEl` como parámetro y usa la sintaxis de flecha para añadir en un callback vacío.

# --hints--

Debe encadenar `forEach()` a `playlistSongElements`.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

Tu método `forEach()` debe tener un parámetro `songEl` y usar sintaxis de flecha.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

Tu `forEach` debe tener un par vacío de llaves.
