...
في دالتك `elemValue`، أعلن عن دالة تسمى `inner` تأخذ معلمة `character`.

ثم أعد دالتك `inner`.

# --hints--

يجب أن تعلن عن متغير `inner`.
...
يجب أن تستخدم `const` لإعلان متغير `inner`.
...
يجب أن يكون متغير `inner` دالة سهمية.
...
يجب أن تحتوي دالتك `inner` على `character` كمعلمة وحيدة.
