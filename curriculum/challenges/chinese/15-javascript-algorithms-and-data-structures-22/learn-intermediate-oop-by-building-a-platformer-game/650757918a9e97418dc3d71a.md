...
你需要做的最后一件事是添加 `else if` 语句。

你的条件应该检查玩家的 `x` 位置是否大于或等于检查点的 `x` 位置并且小于或等于检查点的 `x` 位置加上 `40`。

在 `else if` 语句的主体内，你需要调用 `showCheckpointScreen` 函数并传入字符串 `"You reached a checkpoint!"` 作为参数。

恭喜！ 你已完成平台游戏项目！

# --hints--

你应该添加一个 `else if` 子句来检查玩家的 `x` 位置是否大于或等于检查点的 `x` 位置，并且小于或等于检查点的 `x` 位置加 `40`。
...
你应该调用 `showCheckpointScreen` 函数并传入 “You reached a checkpoint!” 作为参数。
