...
Crea un altro metodo chiamato `get_tv_series` e fallo restituire una lista con tutte le istanze di `TVSeries` che riesci a trovare in `self.items`.

# --hints--

La tua classe `MediaCatalogue` dovrebbe avere un metodo chiamato `get_tv_series`.
...
Il tuo metodo `get_tv_series` dovrebbe restituire una lista contenente tutti gli elementi in `self.items` che sono istanze della classe `TVSeries`.
