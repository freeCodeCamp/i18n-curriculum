...
將另一個物件添加到 `animationData` 陣列。你的新物件應該有屬性 `inputVal` 和 `addElDelay`，分別設定為 `1` 和 `2000`。請記得將 `animationData` 陣列視為堆疊，並將新物件添加到堆疊頂端。

# --hints--

`animationData` 陣列的長度應該為 `3`。
