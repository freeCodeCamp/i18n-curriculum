...
Ви помітили, що можна відправити форму, не заповнивши вхідні дані?

Щоб цього уникнути, додайте атрибут `disabled`, щоб вимкнути кнопку відправки, якщо `heroName`, `realName` або `powerSource` є хибними, або якщо довжина `powers` дорівнює `0`.

Тепер кнопка буде вимкнена, якщо щось не заповнене. Заповніть форму інформацією про свого супергероя і відправте її, щоб приєднатися до ліги.

Таким чином, ваша форма застосунку супергероя завершена.

# --hints--

Ви не повинні змінювати існуючий вміст кнопки.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2/)
```

Ваш елемент `button` має мати властивість `disabled`.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

Ви повинні вимкнути `button`, якщо будь-який з `heroName`, `realName` та `powerSource` є хибним, або якщо довжина `powers` дорівнює `0`.
