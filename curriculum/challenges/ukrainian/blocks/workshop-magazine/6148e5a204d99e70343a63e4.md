...
Властивість `gap` — це скорочений спосіб одночасно встановити значення `column-gap` і `row-gap`. Якщо вказано одне значення, воно встановлює `column-gap` і `row-gap` обом у це значення. Якщо вказано два значення, перше встановлює `row-gap`, а друге — `column-gap`.

Задайте селектору `.image-wrapper` властивість `gap` зі значенням `2rem`.

# --hints--

Ваш елемент `.image-wrapper` повинен мати властивість `gap`, встановлену в `2rem`.
