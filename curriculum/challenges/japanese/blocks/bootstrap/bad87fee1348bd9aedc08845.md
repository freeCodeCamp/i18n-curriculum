...
Font Awesome は便利なアイコンのライブラリです。これらのアイコンはウェブフォントまたはベクターグラフィックとして扱われます。アイコンはフォントと同じように扱われ、ピクセル単位でサイズを指定でき、親の HTML 要素のフォントサイズを引き継ぎます。

# --instructions--

Font Awesome を使って、info ボタンに `info-circle` アイコンを、delete ボタンに `trash` アイコンを追加してください。

**注意:** このチャレンジを完了するには、`i` または `span` 要素のどちらかを使うことができます。

# --hints--

`<i class="fas fa-info-circle"></i>` ボタン要素の中に `info` を追加してください。
...
`<i class="fas fa-trash"></i>` ボタン要素の中に `delete` を追加してください。
...
それぞれの `i` 要素には終了タグが必要で、`<i class="fas fa-thumbs-up"></i>` は `like` ボタン要素の中にあります。
