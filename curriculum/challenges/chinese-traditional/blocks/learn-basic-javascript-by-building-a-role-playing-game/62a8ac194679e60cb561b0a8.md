...
為你的空 `button text` 陣列指定三個字串成員。使用在 `goTown` 函式（程式）中被指定給按鈕 `innerText` 屬性的三個字串。請記得陣列值以逗號分隔。

# --hints--

你的 `button text` 陣列應該有三個元素。

```js
assert.lengthOf(locations[0]["button text"], 3);
```

你的 `button text` 陣列應該有三個字串。
...
`button text` 陣列中的第一個值應該是 `"Go to store"`。

```js
assert.equal(locations[0]["button text"][0], "Go to store");
```

`button text` 陣列中的第二個值應該是 `"Go to cave"`。

```js
assert.equal(locations[0]["button text"][1], "Go to cave");
```

`button text` 陣列中的第三個值應該是 `"Fight dragon"`。
