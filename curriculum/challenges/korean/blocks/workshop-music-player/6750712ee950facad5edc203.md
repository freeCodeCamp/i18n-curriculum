...
플레이어를 조금 사용해 보면 몇 가지 버그가 있다는 것을 알 수 있습니다. 먼저, 노래를 재생한 후 일시 정지하고 이전 버튼이나 다음 버튼을 누르면 노래가 처음부터 시작되지 않습니다.

이 문제를 고치기 위해 `start` 함수에 `playSong`라는 두 번째 매개변수를 추가하세요. 기본값은 `true`로 설정하세요.

# --hints--

`playSong` 함수에는 `start`라는 두 번째 매개변수가 있어야 하며, 기본값은 `true`여야 합니다.
