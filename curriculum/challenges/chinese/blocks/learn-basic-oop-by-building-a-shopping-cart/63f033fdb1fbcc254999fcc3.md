...
你的购物车目前未显示在网页上。 为了使其可见，首先向 `cartBtn` 变量添加一个事件监听器，监听点击事件。 回调不需要任何参数。

# --hints--

你应该向 `cartBtn` 变量添加一个事件监听器。

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(/);
```

你应该监听 `cartBtn` 变量上的 `click` 事件。

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

你应该向事件监听器添加一个空的回调函数（使用箭头语法）。 请记住，它不需要任何参数。
