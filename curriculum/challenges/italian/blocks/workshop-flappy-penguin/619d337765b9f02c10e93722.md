...
Per il terzo e il quarto waypoint, ripeti ancora una volta il pattern `transform`.

# --hints--

Dovresti dare al waypoint `30%` un `transform` di `rotate(110deg) scaleX(-1)`.
...
Dovresti dare al waypoint `40%` un `transform` di `rotate(130deg) scaleX(-1)`.
