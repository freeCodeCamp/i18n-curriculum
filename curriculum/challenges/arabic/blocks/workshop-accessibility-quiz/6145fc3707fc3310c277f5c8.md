...
عد إلى تنسيق الصفحة. اختر عناصر القائمة داخل شريط التنقل، وأعطها الأنماط التالية:
...
يجب أن تستخدم محدِّد `nav li` أو `nav > ul > li`.
...
يجب أن تعطي عناصر `li` خاصية `color` بالقيمة `#dfdfe2`.
...
يجب أن تعطي عناصر `li` خاصية `margin` بالقيمة `0 0.2rem`.
...
يجب أن تعطي عناصر `li` خاصية `padding` بالقيمة `0.2rem`.
...
يجب أن تعطي عناصر `li` خاصية `display` بالقيمة `block`.
