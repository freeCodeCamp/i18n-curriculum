...
كما في السابق، حدّث متغير `sortedValues` ليكون ناتج `insertionSort` بدلًا من `selectionSort`.

# --hints--

يجب استدعاء `insertionSort` مع مصفوفة `inputValues` الخاصة بك.

```js
assert.match(code, /const\s+sortedValues\s*=\s*insertionSort\s*\(\s*inputValues\s*\)/);
```

لا يجب استدعاء `selectionSort` في شفرتك.
