...
在小螢幕上，導覽列中的無序列表會溢出螢幕右側。

透過使用 *Flexbox* 來包裹 `ul` 內容以修正此問題。接著，設定以下 CSS 屬性以正確對齊文字：
