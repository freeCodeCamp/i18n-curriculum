...
Die letzte Änderung betrifft den E-Mail-Text in der Fußzeile. In früheren Lektionen und Workshops haben Sie gelernt, wie Sie mit dem `mailto` Link so arbeiten:

```html
<p>Email me at <a href="mailto:janedoe@email.com">janedoe@email.com</a></p>
```

Beginnen Sie damit, die `camperbot@blog.io` E-Mail-Adresse in ein Ankerelement einzuschließen. Fügen Sie dann dem Ankerelement das `href` Attribut hinzu und setzen Sie es auf `mailto:camperbot@blog.io`.

Mit dieser letzten Änderung haben Sie alle Probleme auf der Blogseite erfolgreich behoben!

# --hints--

Ihre `camperbot@blog.io` E-Mail-Adresse sollte in ein Ankerelement eingeschlossen sein.
...
Ihr Ankerelement sollte ein `href` Attribut mit dem Wert `mailto:camperbot@blog.io` haben.
