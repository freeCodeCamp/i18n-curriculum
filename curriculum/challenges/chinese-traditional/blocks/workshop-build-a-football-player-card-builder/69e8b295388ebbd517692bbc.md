...
現在新增一個 `div`，其 `className` 為 `card-stats`。在裡面，新增一個 `div`，其 `className` 為 `stat-col`。在那裡，新增三個 `div` 元素，每個都有 `className` 為 `stat-row`。

# --hints--

你應該在 `div` 後新增一個 `className`，其 `card-stats` 為 `.card-name-strip`。

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

你應該在 `div` 裡新增一個 `className`，其 `stat-col` 為 `.card-stats`。

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

你應該在 `div` 裡新增三個 `className` 元素，每個都有 `stat-row` 為 `.stat-col`。
