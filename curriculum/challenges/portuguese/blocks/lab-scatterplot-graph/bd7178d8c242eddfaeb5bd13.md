...
title: Construir um gráfico de dispersão
challengeType: 25
...
Neste projeto você vai usar os dados de `https://cdn.freecodecamp.org/curriculum/labs/data/scatterplot/cyclist-data.json` para construir um gráfico de dispersão.

Os testes requerem que os eixos sejam gerados usando a propriedade de eixos do D3, que geram automaticamente marcações ao longo do eixo. Essas marcações são exigidas para passar nos testes do D3 porque suas posições são usadas para determinar o alinhamento dos elementos do gráfico.

**Objetivo:** Cumprir as user stories abaixo e fazer todos os testes passarem para completar o laboratório.

**Histórias de Usuário:**

1. Seu gráfico deve ter um título com um correspondente `id="title"`.
1. Seu gráfico deve ter um elemento `g` para o eixo x com um `id="x-axis"` correspondente.
1. Seu gráfico deve ter um elemento `g` do eixo y com um `id="y-axis"` correspondente.
1. Ambos os eixos devem conter múltiplos rótulos de marcação, cada um com uma `class="tick"`.
1. Seu gráfico deve ter um elemento `circle` para cada ponto de dados com uma `class="dot"` correspondente exibindo os dados.
1. Cada `.dot` deve ter os atributos `data-xvalue` e `data-yvalue` contendo os valores `year` e `time`. Para `data-xvalue`, inteiros (anos completos) ou objetos Date são aceitáveis para avaliação do teste. Para `data-yvalue` (minutos), use objetos Date.
1. Os `data-xvalue` atributos dos elementos `.dot` devem corresponder à ordem dos dados fornecidos.
1. Os `data-yvalue` atributos dos elementos `.dot` devem corresponder à ordem dos dados fornecidos.
...
1. Seu tooltip deve ter um atributo `data-year` que corresponda ao `data-xvalue` da área ativa.
...
O gráfico deve ter um elemento com o `id` `title` para conter o título.

```js
assert.exists(document.getElementById('title'));
```

O gráfico deve ter um `x-axis` com um correspondente `id="x-axis"`.
...
O gráfico deve ter um `y-axis` com um `id="y-axis"` correspondente.
...
O eixo x deve conter múltiplos rótulos de marcação, cada um com o correspondente `class="tick"`.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

O eixo y deve conter múltiplos rótulos de tick, cada um com o correspondente `class="tick"`.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

Seu gráfico deve ter pontos, que cada um tenha uma classe `dot`, que representam os dados sendo plotados.

```js
assert.isNotEmpty(document.querySelectorAll('circle.dot'));
```

Cada ponto deve ter as propriedades `data-xvalue` e `data-yvalue` contendo seus respectivos valores x e y.
...
O `data-xvalue` e o `data-yvalue` de cada ponto devem estar dentro do intervalo dos dados reais e no formato correto de dados. Para `data-xvalue`, inteiros (anos completos) ou objetos Date são aceitáveis para avaliação do teste. Para `data-yvalue` (minutos), use objetos Date.
...
O `data-xvalue` e seu ponto correspondente devem alinhar com o ponto/valor correspondente no eixo x.
...
Você pode ver múltiplos rótulos de ticks no eixo y com o formato de tempo `%M:%S`.
...
Você pode ver uma legenda contendo texto descritivo que tem `id="legend"`.
...
Ao passar o mouse sobre uma área, seu gráfico de dispersão deve ter um tooltip correspondente com `id="tooltip"` que exibe mais informações sobre a área.
...
O tooltip deve ter um atributo `data-year` que corresponda ao `data-xvalue` da área ativa.
