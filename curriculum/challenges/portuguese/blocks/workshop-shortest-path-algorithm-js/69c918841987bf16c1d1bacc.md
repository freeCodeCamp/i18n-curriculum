...
Agora faça um loop pelos nós alvo para exibir os resultados de cada um.

Adicione um loop `for...of` que itere sobre `targets` usando `nodeNo` como variável do loop. Deixe o corpo vazio por enquanto.

# --hints--

Você deve criar um loop `for...of` com a variável `nodeNo` iterando sobre `targets`.
