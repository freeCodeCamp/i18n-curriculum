...
أنشئ وأضف الخصائص التالية إلى `.bb2a`:
...
بعد إضافتها، سترى كيف أن الحدود السميكة على عنصر تعطيك زوايا حيث يلتقي جانبان. ستستخدم تلك الحدود السفلية كقمة للمبنى.

# --hints--

يجب أن تعطي `.bb2a` هامشًا (`margin`) بقيمة `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.margin, "auto");
```

يجب أن تعطي `.bb2a` عرضًا (`width`) بقيمة `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.width, "5vw");
```

يجب أن تعطي `.bb2a` ارتفاعًا (`height`) بقيمة `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.height, "5vw");
```

يجب أن تعطي `.bb2a` حدًا علويًا (`border-top`) بقيمة `1vw solid #000`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderTop, "1vw solid rgb(0, 0, 0)");
```

يجب أن تعطي `.bb2a` حدًا سفليًا (`border-bottom`) بقيمة `1vw solid #000`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderBottom, "1vw solid rgb(0, 0, 0)");
```

يجب أن تعطي `.bb2a` حدًا أيسرًا (`border-left`) بقيمة `1vw solid #999`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2a')?.borderLeft, "1vw solid rgb(153, 153, 153)");
```

يجب أن تعطي `.bb2a` حدًا أيمنًا (`border-right`) بقيمة `1vw solid #999`.
