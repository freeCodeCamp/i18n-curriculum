...
起始位置表述提取應該開始的索引。停止位置是切片應該結束的地方。此位置不包含在內。步進位置表述切片遞增的間隔。以下是一些範例：
...
- **`strip()`**：此字串方法傳回一個字串的複本，並移除指定的前置和後置字元（如果沒有傳入引數，則移除前置和後置的空白字元）。
...
- **`startswith(prefix)`**：此字串方法傳回一個布林值，表示字串是否以指定的前置式開始：
...
- **`endswith(suffix)`**：此字串方法傳回一個布林值，表示字串是否以指定的後綴結尾：
...
- **`find()`**：此字串方法傳回子字串首次出現的位置索引。如果找不到，則傳回 `-1`：
...
- **`capitalize()`**：此字串方法傳回一個新的字串，該字串的第一個字元為大寫，其他字元為小寫：
...
- **`isupper()`**：此字串方法會在字串中所有字母皆為大寫時傳回 `True`，否則傳回 `False`：
...
- **`islower()`**：此字串方法會在字串中所有字母皆為小寫時傳回 `True`，否則傳回 `False`：
...
- **`title()`**：此字串方法傳回一個新字串，其中每個字的第一個字母皆為大寫：
...
- **`maketrans()`**：這個字串方法用於創建一個一對一字元對應的轉換表。它通常與 `translate()` 方法一起使用，該方法將該轉換表套用到字串並傳回翻譯後的結果。
...
- **`float()` 函式（程式）**：你可以使用這個函式將整數轉換為浮點數。
...
- **`int()` Function**：你可以使用此函式將浮點數轉換為整數。
...
- **短路運算**：`and` 和 `or` 運算子被稱為短路運算子。短路運算表示 Python 從左到右檢查值，並在確定最終結果後立即停止。
...
- **切片列表**：切片是使用切片運算子 `:` 來存取列表部分內容的概念。要切片一個從索引 `1` 開始且在索引 `3` 之前結束的列表，你可以使用以下語法：
...
- **sorted()**：一個內建函式，會傳回一個新的排序後列表，而不是修改原始列表：
...
- 如果 `count()` 方法中指定的項目在元組中完全不存在，則返回值將是 `0`：
...
- 如果沒有引數傳遞給 `count()` 方法，Python 將會傳回 `TypeError`。
...
- **差集運算子 (`-`)**：差集運算子 `-` 傳回一個新集合，該集合包含第一個集合中不在另一個集合內的元素。
...
- **IndexError**：如果你存取一個在列表或其他序列（例如 tuple 和字串）中不存在的索引，就會得到 `IndexError`。例如，在 `Hello world` 字串中，最後一個字元的索引是 `10`。如果你繼續這樣存取字元，`greet = "hello world"; print(greet[11])`，你會得到帶有訊息 `IndexError: string index out of range` 的錯誤。
...
- **`try...except`**：這用於執行可能引發例外的程式碼區塊。`try` 區塊是你預期可能發生錯誤的地方，而 `except` 區塊會在指定的錯誤被引發時執行。以下是一個範例：
  ...
  你也可以串鏈多個 `except` 區塊，以便控制更多型別的例外：
...
## 例外信號
  ...
  以下是你如何使用 `login` 函式與 `InvalidCredentialsError` 例外的範例：
...
- **在物件上呼叫方法**：你可以在物件上呼叫方法來執行動作或取得資訊。
...
- **間接呼叫 dunder 方法**：你不需要直接呼叫 dunder 方法。相反地，當發生某些操作時，Python 會自動呼叫它們。這些操作包括：
  ...
  - **像加法、減法、乘法、除法及其他的算術操作行為**。`__add__()` 用於加法，`__sub__()` 用於減法，`__mul__()` 用於乘法，`__truediv__()` 用於除法。
...
- **類別**：用於創建物件的藍圖。每個從類別創建的物件都有定義資料的屬性和決定物件行為的方法。
...
- **`super()` 函式**：一個讓你在類別對該方法有不同實作或擴充該方法時，**呼叫**父類別方法而不重複程式碼的函式。
