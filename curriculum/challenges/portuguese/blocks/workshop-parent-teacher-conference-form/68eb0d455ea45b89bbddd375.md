...
Agora, você deve adicionar algum espaçamento interno ao container. Defina `padding` como `10px 20px`. Dessa forma, você define um espaçamento interno de `10px` na parte superior e inferior e `20px` na esquerda e direita.
...
Defina a propriedade `padding` para `10px 20px`.
