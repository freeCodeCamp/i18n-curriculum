...
Fügen Sie dem input ein `onChange`-Attribut hinzu, um zu erfassen, was der Benutzer eingibt. Setzen Sie es auf eine Pfeilfunktion, die `e` als Argument erhält. Aktualisieren Sie innerhalb der Funktion den `query`-Status, indem Sie `e.target.value` an die Setter-Funktion übergeben, die den aktuellen Wert der Eingabe enthält. Hier ist ein Beispiel:
...
Sie sollten sicherstellen, dass der `onChange`-Handler definiert ist.
...
Ihr `onChange`-Handler sollte einen `e`-Parameter haben.
...
Ihr `onChange`-Handler sollte `setQuery()` mit `e.target.value` aufrufen.
