...
Você deve definir uma nova variável de propriedade chamada `--building-color4`.

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color4'));
```

Você deve definir `--building-color4` com um valor de `#538cc6` no seletor `:root`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color4').trim(), '#538cc6');
```

Você deve adicionar `background-color: var(--building-color4)` ao seletor `.bb4`.
