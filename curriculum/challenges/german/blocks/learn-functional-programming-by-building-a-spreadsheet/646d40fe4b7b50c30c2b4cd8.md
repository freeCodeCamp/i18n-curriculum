...
Ihre `infixEval`-Funktion wird nur die erste Multiplikations- oder Divisionsoperation auswerten, weil `regex` nicht global ist. Das bedeutet, Sie sollten einen rekursiven Ansatz verwenden, um den gesamten String auszuwerten.

Wenn `infixEval` keine Übereinstimmungen findet, gibt es den Wert `str` unverändert zurück. Verwenden Sie einen ternären Ausdruck, um zu prüfen, ob `str2` gleich `str` ist. Falls ja, geben Sie `str` zurück, andernfalls das Ergebnis des Aufrufs von `highPrecedence()` mit `str2`.

# --hints--

Ihre `highPrecedence`-Funktion sollte das Schlüsselwort `return` verwenden.
...
Sie sollten das Schlüsselwort `return` mit einer Bedingung verwenden, um zu prüfen, ob `str` gleich `str2` ist.
...
Sie sollten die ternäre Syntax mit Ihrer `return`-Anweisung verwenden.
...
Wenn die ternäre Bedingung wahr ist, sollten Sie `str` zurückgeben.
...
Wenn die ternäre Bedingung falsch ist, sollten Sie das Ergebnis des Aufrufs von `highPrecedence()` zurückgeben.
...
Sie sollten `str2` an Ihren `highPrecedence()`-Aufruf übergeben.
