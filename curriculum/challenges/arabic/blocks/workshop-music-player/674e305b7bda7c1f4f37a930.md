...
قبل أن تبدأ العمل على تشغيل الأغنية التالية والسابقة، تحتاج إلى الحصول على فهرس كل أغنية في خاصية `songs` داخل `userData`. للقيام بذلك، يمكنك استخدام طريقة المصفوفة `indexOf()`، التي تُرجع أول فهرس يمكن العثور فيه على عنصر معين في المصفوفة، أو `-1` إذا لم يكن العنصر موجودًا.
...
أنشئ دالة باسم `getCurrentSongIndex` تُرجع الفهرس الذي يمكن العثور فيه على `userData.currentSong` داخل `userData.songs`، وترجع `-1` عندما تكون الأغنية الحالية `null`.

# --hints--

يجب أن يكون لديك دالة باسم `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

يجب أن تُرجع دالة `getCurrentSongIndex` الفهرس الذي يمكن العثور فيه على `userData.currentSong` داخل `userData.songs`.
...
يجب أن تُرجع دالة `getCurrentSongIndex` القيمة `-1` عندما تكون `userData.currentSong` تساوي `null`.
