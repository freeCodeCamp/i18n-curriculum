...
Усередині шаблонного рядка створіть елемент `div` з множиною `id`, встановленою за допомогою методу масиву `index` `.forEach()`. Не забудьте використати інтерполяцію рядків для цього.

Також додайте `class` типу `"user-card"` до `div`.
...
Ви повинні створити елемент `div` з класом `"user-card"`.
...
Ваш елемент `div` має мати множину `id` зі значенням `index`. Не забудьте використати інтерполяцію рядків для встановлення `id`.
...
Ваш елемент `div` має бути порожнім.
