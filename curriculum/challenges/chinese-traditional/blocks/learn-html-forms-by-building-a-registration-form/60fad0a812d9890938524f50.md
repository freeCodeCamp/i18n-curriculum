...
爲了讓學員知道在他們的簡歷中應該寫些什麼，使用了 `placeholder` 屬性。 `placeholder` 接受一個文本值，該值在用戶開始鍵入之前一直顯示。

給 `textarea` 添加一個 `placeholder`，值爲 `I like coding on the beach...`。

# --hints--

你應該給 `textarea` 添加一個 `placeholder` 屬性。
