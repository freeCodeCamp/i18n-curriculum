...
Finaliza la función usando el método `forEach` del arreglo para iterar sobre el argumento elements. Dentro de la función callback, agrega la clase hidden a cada elemento usando el método `.add()` de `classList`.

# --hints--

Deberías tener un método `forEach` adjunto al parámetro `elements`.
...
Tu método forEach debe tomar un parámetro llamado `el`.
...
Dentro del método forEach, debes agregar la clase hidden usando el método `classList.add`.
