...
곱셈 또는 나눗셈에 일치하는 정규 표현식이 있으니, 이제 그 표현식을 평가할 수 있습니다.

`str2` 변수를 선언하고 `infixEval`와 `str`를 인수로 하여 `regex`을 호출한 결과를 할당하세요.
...
`regex` 호출의 두 번째 인수로 `infixEval`을 전달해야 합니다.
