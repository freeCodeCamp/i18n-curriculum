...
在你的 `attack` 函式中，找到更新 `monsterHealth` 變數的程式碼行，並將它放入一個 `if` 區塊中，該區塊的條件是呼叫 `isMonsterHit` 函式。
...
你的 `if` 述語應該在條件中呼叫 `isMonsterHit`。

```js
assert.match(attack.toString(), /if\s*\(\s*isMonsterHit\s*\(\s*\)/)
```

你應該將你的 `monsterHealth -= weapons[currentWeaponIndex].power + Math.floor(Math.random() * xp) + 1` 放入你的 `if` 區塊。
