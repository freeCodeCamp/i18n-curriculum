...
現在創建一個 `fightDragon` 函式（程式），將 `"Fighting dragon."` 列印到主控台。

# --hints--

你應該使用 `function` 關鍵字來宣告 `fightDragon`。

```js
assert.match(code, /function\s+fightDragon/);
```

`fightDragon` 應該是一個函式（程式）。

```js
assert.isFunction(fightDragon);
```

你的程式碼中應該有一行 `console.log("Fighting dragon.");`。

```js
assert.match(code, /console\.log\(\s*('|")Fighting dragon\.\1\s*\)/);
```

你的 `console.log("Fighting dragon.");` 行應該放在你的 `fightDragon` 函式中。
