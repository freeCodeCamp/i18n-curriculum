...
في `evalFormula`، أعلن عن دالة سهمية `idToText` تأخذ معلمة `id`.

يجب أن تُرجع دالة `idToText` نتيجة استدعاء `.find()` على مصفوفة `cells` مع دالة استدعاء راجع تأخذ معلمة `cell` وتُرجع `cell.id === id`.

يجب أن تستخدم كلتا الدالتين عوائد ضمنية.

# --hints--

يجب أن تعلن عن متغير `idToText` داخل دالة `evalFormula`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*(?:const|let|var)\s+idToText/);
```

يجب أن تستخدم `const` لإعلان متغير `idToText`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText/);
```

يجب أن يكون متغير `idToText` دالة سهمية.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

يجب أن تحتوي دالة `idToText` على معلمة `id`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>/);
```

يجب أن تُرجع دالة `idToText` نتيجة استدعاء طريقة `.find()` على مصفوفة `cells`. يجب أن تستخدم دالة الاستدعاء الراجع عائدًا ضمنيًا.
...
يجب أن تُرجع دالة الاستدعاء الراجع ما إذا كان `cell.id` يساوي `id` بدقة.
