...
Для умови інструкції `if` перевірте, чи `target` не є екземпляром `HTMLElement`. Потім всередині неї додайте `return`.

# --hints--

Ви маєте перевірити, що у вашій інструкції `if` є умова `!(target instanceof  HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

У вашій інструкції `return` має бути `if`.
