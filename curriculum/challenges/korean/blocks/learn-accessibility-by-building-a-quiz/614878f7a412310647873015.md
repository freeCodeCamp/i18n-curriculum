...
자식 요소들 사이에 공간을 만들고 수직으로 가운데 정렬하도록 _Flexbox_를 사용해 `header`을 정리하세요.

# --hints--

`header`에 `justify-content` 값을 가진 `space-between`를 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

`header`에 `align-items` 값을 가진 `center`를 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

`header`에 `top` 값을 가진 `0`를 지정하세요.
