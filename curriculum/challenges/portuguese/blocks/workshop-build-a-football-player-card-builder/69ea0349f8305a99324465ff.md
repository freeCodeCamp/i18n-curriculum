...
No bloco `try`, crie uma variável `saved`. Para o valor, use o método `getItem()` para recuperar `STORAGE_KEY`. Logo abaixo dessa variável, crie uma declaração `if` com a condição `saved`. Deixe o bloco `if` vazio por enquanto.

# --hints--

Sua função `loadPlayer` deve fazer uma chamada a `localStorage.getItem` com `STORAGE_KEY` dentro do bloco `try`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

O resultado de `localStorage.getItem` deve ser armazenado em uma variável chamada `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

O bloco `try` deve ter uma declaração `if` com a condição `saved`.
