...
Per creare chunk di dimensione `n` da un `array` senza modificarlo, puoi combinare un loop `for` con il metodo `slice()` in questo modo: 
...
- crea un array vuoto chiamato `result`

- itera sull'array originale `array` a intervalli di dimensione `n`

- crea chunk da `array` di dimensione `n` e li inserisce in `result` usando `slice()`

Per completare la funzione `chunkCrew`, devi: 

- creare un array chiamato `chunks`

- iterare su `crew` a intervalli di dimensione `size`, creando e inserendo chunk da `crew` in `chunks` usando `slice()`

- restituire l'array `chunks`

# --hints--

Dovresti creare un array vuoto chiamato `chunks`.
...
Dovresti usare un loop `for` per costruire i chunk.
...
All'interno del loop `for`, dovresti chiamare `push()` sull'array `chunks` per inserire ogni chunk estratto (che deve essere creato chiamando `slice()` sull'array `crew`).
...
Dopo il loop, dovresti restituire l'array `chunks`.
...
La tua funzione `chunkCrew` dovrebbe restituire chunk di lunghezza `size` dall'array di input `crew` senza modificarlo.
