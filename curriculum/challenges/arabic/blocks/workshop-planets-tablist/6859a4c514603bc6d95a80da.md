...
حان الوقت الآن لربط لوحات التبويب بالتبويبات الخاصة بها.

لأول عنصر `tabpanel`، أضف الخاصية `id` بالقيمة `"panel-earth"` والخاصية `aria-labelledby` بالقيمة `"tab-earth"`.

# --hints--

يجب أن يحتوي أول عنصر `tabpanel` على خاصية `id`.

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("id"));
```

يجب أن تكون قيمة الخاصية `id` هي `"panel-earth"`.

```js
assert.equal(document.querySelector('[role="tabpanel"]').getAttribute("id"), "panel-earth");
```

يجب أن يحتوي أول عنصر `tabpanel` على خاصية `aria-labelledby`.

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("aria-labelledby"));
```

يجب أن تكون قيمة الخاصية `aria-labelledby` هي `"tab-earth"`.
