...
يجب يحتوي منتقي `table caption` الخاص بك على `position` بقيمة `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('position') === 'absolute');
```

يجب يحتوي منتقي `table caption` الخاص بك على `top` بقيمة `-2.25rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('top') === '-2.25rem');
```

يجب يحتوي منتقي `table caption` الخاص بك على `left` بقيمة `0.5rem`.
