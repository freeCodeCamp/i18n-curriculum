...
CSV (Comma-Separated Values) è un formato usato per fogli di calcolo e importazione dati. Ogni riga rappresenta un elemento con i campi separati da virgole.

Inizia rimuovendo la chiamata `console.log(exportToJSON(...))`, dato che serviva solo per i test. Poi crea una funzione chiamata `exportToCSV` che prende `catalog` come parametro. All’interno, dichiara `const header = "Title,Author,Year,Location"` e `const rows = []`.

# --hints--

Dovresti rimuovere la chiamata `exportToJSON` console.log.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

Dovresti avere una funzione chiamata `exportToCSV` con un parametro `catalog`.
...
Dovresti dichiarare una variabile chiamata `header` con valore `"Title,Author,Year,Location"`.

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

Dovresti dichiarare una variabile chiamata `rows` impostata a un array vuoto.
