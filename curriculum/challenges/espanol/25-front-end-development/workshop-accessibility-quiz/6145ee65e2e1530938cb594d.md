...
La sección final de este cuestionario contendrá una lista desplegable y un cuadro de texto.

Comience anidando un `div` con una `class` de `formrow`, y anide cuatro elementos `div` dentro de él, alternando sus atributos `class` con `question-block` y `answer`.

# --hints--

Debe añadir un elemento `div` dentro de la sección final.

```js
assert.exists(document.querySelector('section:nth-of-type(3) > div'));
```

Debe dar el primer `div` una `class` de `formrow`.

```js
assert.equal(document.querySelector('section:nth-of-type(3) > div')?.className, 'formrow');
```

Debe colocar `div.formrow` después del elemento `h2`.

```js
assert.exists(document.querySelector('section:nth-of-type(3) > h2 + div.formrow'));
```

Debe anidar cuatro elementos `div` dentro de `div.formrow`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(3) > div.formrow > div')?.length, 4);
```

Debe dar al primer `div` anidado una `class` de `question-block`.

```js
assert.equal(document.querySelector('section:nth-of-type(3) > div.formrow > div:nth-of-type(1)')?.className, 'question-block');
```

Debe dar al segundo `div` anidado una `class` de `answer`.

```js
assert.equal(document.querySelector('section:nth-of-type(3) > div.formrow > div:nth-of-type(2)')?.className, 'answer');
```

Debe dar al tercer `div` anidado una `class` de `question-block`.

```js
assert.equal(document.querySelector('section:nth-of-type(3) > div.formrow > div:nth-of-type(3)')?.className, 'question-block');
```

Debe dar al cuarto `div` anidado una `class` de `answer`.
