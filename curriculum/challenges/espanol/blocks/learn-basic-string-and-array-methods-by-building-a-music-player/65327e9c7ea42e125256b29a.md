...
Hemos agregado el resto de las canciones al arreglo `allSongs` para ti.

A continuación, aprenderás sobre la <dfn>API de Audio Web</dfn> y cómo usarla para reproducir canciones. Todos los navegadores modernos son compatibles con la API de Audio Web, que te permite generar y procesar audio en aplicaciones web.

Usa `const` para crear una variable llamada `audio` y asígnale `new Audio()`. Esto creará un nuevo elemento `audio` en HTML5.

# --hints--

Debes usar la palabra clave `new` para crear una instancia del objeto `Audio`.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

Deberías asignar el objeto `Audio` a una constante llamada `audio`.
