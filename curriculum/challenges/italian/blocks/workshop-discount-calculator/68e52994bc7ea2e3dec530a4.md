...
Ora crea un loop `for` che itera su `self.strategies`. Per ogni `strategy`, verifica se è applicabile usando il metodo `is_applicable`. Se lo è, calcola il prezzo scontato usando `apply_discount` e salvalo in una variabile chiamata `discounted`. Poi, aggiungi il prezzo scontato alla lista `prices`.

# --hints--

Dovresti creare un loop `for` che itera su `self.strategies`.
...
All’interno del loop, la tua istruzione `if` dovrebbe essere `if strategy.is_applicable(product, user_tier):`.
...
Se una strategia è applicabile, dovresti chiamare `apply_discount` e aggiungere il risultato a `prices`.
