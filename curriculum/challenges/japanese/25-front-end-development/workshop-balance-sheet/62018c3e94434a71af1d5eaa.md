...
テーブルのボディ内にある `td` 要素を選択するセレクターを作成してください。 その幅がビューポートの幅全体に広がるよう設定し、最小の幅も最大の幅も `4rem` に設定してください。 `width` を特定の値に設定してもコンテナーの幅に応じて要素が縮小してしまうのに対し、このアプローチであれば、幅が確実に固定されます。
