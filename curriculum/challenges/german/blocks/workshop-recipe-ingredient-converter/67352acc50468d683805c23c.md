...
Aktualisieren Sie nun entsprechend der obigen Syntax Ihre `convertQuantity`-Funktion, sodass sie eine zweite Funktion zurückgibt. Diese innere Funktion soll ein `toUnit`-Argument akzeptieren und leer sein.

# --hints--

Ihre `convertQuantity`-Funktion sollte eine Funktion zurückgeben.

```js
assert.isFunction(convertQuantity());
```

Ihre zurückgegebene Funktion sollte einen `toUnit`-Parameter haben.

```js
assert.match(convertQuantity()?.toString(), /\(toUnit\)/);
```

Ihre zurückgegebene Funktion sollte leer sein.
