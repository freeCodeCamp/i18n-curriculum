...
要獲取上一首歌曲，請從 `userData?.songs` 的 `currentSongIndex` 中減去 `1`，並將其分配給常數 `previousSong`。 之後，調用 `playSong` 函數並傳遞 `previousSong.id` 作爲參數。
...
你應該將 `userData?.songs[currentSongIndex - 1]` 分配給 `previousSong` 常量。
...
你應該使用 `previousSong.id` 調用 `playSong` 函數。
