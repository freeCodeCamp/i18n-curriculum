...
A sintaxe é diferente em apenas um ponto. O valor importado, `add`, não está cercado por chaves (`{}`). `add` aqui é simplesmente um nome de variável para o que quer que seja a exportação padrão do arquivo `math_functions.js`. Você pode usar qualquer nome ao importar algo que foi exportado como padrão.
