...
بمجرد التأكد من العثور على عقدة صالحة، علّمها بأنها تمت زيارتها حتى لا تعالجها الخوارزمية مرة أخرى.

بعد جملة `if` التي تحتوي على الشرط `current === -1`، عيّن `visited[current]` إلى `true`.

# --hints--

يجب أن تعيّن `visited[current]` إلى `true`.
