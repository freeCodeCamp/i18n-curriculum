...
在這個項目中，你將通過構建平臺遊戲來學習中級面向對象編程原理。 所有的 HTML 和 CSS 都已爲你提供。

首先使用 `document.getElementById()` 獲取 `#start-btn` 和 `#canvas` 元素。

將它們分別存儲在名爲 `startBtn` 和 `canvas` 的 `const` 變量中。
