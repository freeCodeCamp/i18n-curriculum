...
前のステップと同じスタイルセレクターを使用して、商品と価格のフォントサイズをより大きくするために `18px` という値を設定してください。

# --hints--

`font-size` プロパティを `18px` に設定する必要があります。
...
`.item` 要素内にネストされている `p` 要素には値が `18px` の `font-size` が必要です。
