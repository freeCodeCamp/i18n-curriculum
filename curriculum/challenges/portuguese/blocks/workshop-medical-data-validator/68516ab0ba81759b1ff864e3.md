...
A função `all` retorna `True` se todos os elementos do iterável passado para ela forem verdadeiros e `False` caso contrário:
...
Você deve modificar o valor da sua chave `medications` passando `[isinstance(i, str) for i in medications]` para a função `all`.
