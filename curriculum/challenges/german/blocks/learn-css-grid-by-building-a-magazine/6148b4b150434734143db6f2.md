...
Denken Sie daran, dass die `grid-column`-Eigenschaft bestimmt, an welchen Spalten ein Element beginnt und endet. Es kann Situationen geben, in denen Sie nicht genau wissen, wie viele Spalten Ihr Grid haben wird, aber Sie möchten, dass ein Element an der letzten Spalte endet. Dafür können Sie `-1` für die Endspalte verwenden.

Erstellen Sie einen `.hero`-Selektor und geben Sie ihm eine `grid-column`-Eigenschaft mit dem Wert `1 / -1`. Das weist das Element an, die volle Breite des Grids einzunehmen.

# --hints--

Sie sollten einen `.hero`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero'));
```

Ihr `.hero`-Selektor sollte eine `grid-column`-Eigenschaft mit dem Wert `1 / -1` haben.
