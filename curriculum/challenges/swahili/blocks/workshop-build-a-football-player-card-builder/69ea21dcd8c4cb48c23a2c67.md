...
Ongeza sifa ya kipengele `onChange` iliyowekwa kuwa kitendakazi cha mshale kinachopokea `e` kama kigezo chake na kufanya wito wa `setPlayer` kwa kueneza `player` na `position` iliyowekwa kuwa `e.target.value`, ikibadilishwa kuwa `Position`.

# --hints--

Nafasi yako `select` inapaswa kuwa na mshughulikiaji `onChange` anayefanya wito wa `setPlayer` kwa kueneza `player` na kusahihisha `position`.
