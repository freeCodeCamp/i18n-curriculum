...
선 가장자리의 회색을 확인하세요. 이 가장자리들은 <dfn>테두리</dfn>라고 합니다. 요소의 각 면은 서로 다른 색일 수도 있고 모두 같은 색일 수도 있습니다.

`hr` 요소의 모든 가장자리를 `border-color` 속성을 사용해 배경색과 같게 만드세요.

# --hints--

`border-color` 속성을 `brown`로 설정하세요.
...
`hr` 요소는 `border-color`가 `brown`이어야 합니다.
