...
将 `.line` 选择器的 `position` 属性设置为 `absolute`、`left` 属性设置为 `50%`、`top` 属性设置为 `50%`。

# --hints--

你的 `.line` 选择器应该有一个 `position` 属性，设置为 `absolute`。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.line')?.position, 'absolute');
```

你的 `.line` 选择器应该有一个 `left` 属性，设置为 `50%`。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.line')?.left, '50%');
```

你的 `.line` 选择器应该有一个 `top` 属性，设置为 `50%`。
