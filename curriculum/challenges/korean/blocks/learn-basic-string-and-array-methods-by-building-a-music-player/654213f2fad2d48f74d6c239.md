...
클래스가 `button`이고 `playlist-song-delete` 속성이 `aria-label`에 `Delete`가 보간된 값으로 설정된 또 다른 `song.title` 요소를 만드세요. 삭제 아이콘의 내용으로 다음 SVG를 붙여넣으세요:
...
첫 번째 속성과 값으로 클래스가 `button`인 `playlist-song-delete` 요소를 만들어야 합니다.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

버튼 요소에는 두 번째 속성과 값으로 `aria-label` 속성이 `Delete ${song.title}`인 것이 있어야 합니다.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

제공된 `svg`를 `button` 요소의 내용으로 붙여넣어야 합니다.
