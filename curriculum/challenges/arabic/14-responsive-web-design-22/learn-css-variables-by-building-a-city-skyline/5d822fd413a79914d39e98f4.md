...
أضف `linear-gradient` إلى `.bb1d` مع `orange` كاللون الأول، و`--building-color1` كالثاني و `--window-color1` كثالث. تذكر أن تستخدم معامل التدرج على خاصية `background`.

# --hints--

يجب عليك استخدام `background` على `.bb1d`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background);
```

يجب أن تعطي الخاصية `background` وظيفة `linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'linear-gradient');
```

يجب عليك استخدام `orange` كاللون الأول في `linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.getPropVal('background', true), 'linear-gradient(orange');
```

يجب عليك استخدام `--building-color1` كاللون الثاني في `linear-gradient`.
...
يجب عليك استخدام `--window-color1` كلون ثالث في `linear-gradient`.
