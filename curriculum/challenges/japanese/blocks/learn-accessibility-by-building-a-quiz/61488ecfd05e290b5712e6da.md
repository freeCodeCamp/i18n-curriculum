...
最初のセクションをよりインライン風に見せるために、`input` 要素内の `.info` 要素だけをターゲットにして、それらの `width` を `50%` に設定し、テキストを左揃えにしてください。

# --hints--

`.info input` または `.info > input` のセレクターを使うべきです。
...
`input` 要素に `width` を `50%` に設定してください。
...
`input` 要素に `text-align` を `left` に設定してください。
