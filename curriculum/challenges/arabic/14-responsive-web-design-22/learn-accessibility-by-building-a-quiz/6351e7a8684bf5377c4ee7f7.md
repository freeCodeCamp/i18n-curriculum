...
مع أنّه غير مطلوب لعناصر `label` التي بداخلها `input`، لا يزال من أفضل الممارسات ربط `label` ربطاً صريحاً بعنصره `input`.

الآن، أضف سمة `for` لكل من 4 عناصر `label` التي تربط `label` برز `input` المقابل نوعه راديو (radio).

# --hints--

يجب أن تعطي أول `label` سمة `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

يجب عليك إعطاء أول `label` سمة `for` التي تطابق `id` لعنصره `input`.
...
يجب أن تعطي ثاني `label` سمة `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

يجب عليك إعطاء ثاني `label` سمة `for` التي تطابق `id` لعنصره `input`.
...
يجب أن تعطي ثالث `label` سمة `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

يجب عليك إعطاء ثالث `label` سمة `for` التي تطابق `id` لعنصره `input`.
...
يجب أن تعطي رابع `label` سمة `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

يجب عليك إعطاء رابع `label` سمة `for` التي تطابق `id` لعنصره `input`.
