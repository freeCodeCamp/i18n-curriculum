...
现在你已准备好开始整理网格布局。 CSS Grid offers a two-dimensional grid-based layout, allowing you to center items horizontally and vertically while still retaining control to do things like overlap elements.

首先创建一个 `main` 选择器，并将 `display` 属性设置为 `grid`。
...
你的 `main` 选择器应该有一个 `display` 属性，设置为 `grid`。
