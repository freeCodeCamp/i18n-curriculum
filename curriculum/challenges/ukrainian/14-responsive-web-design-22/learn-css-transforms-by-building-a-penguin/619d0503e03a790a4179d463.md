...
Розташуйте елемент `.chin` на `25%` зверху та `5%` зліва від батьківського елемента. Потім задайте верхнім кутам радіус `70%`, а нижнім кутам — радіус `100%`.

# --hints--

Ви повинні надати `.chin` властивість `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Ви повинні надати `.chin` властивість `top` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Ви повинні надати `.chin` властивість `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Ви повинні надати `.chin` властивість `left` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Ви повинні надати `.chin` властивість `border-radius` зі значенням `70% 70% 100% 100%`.
