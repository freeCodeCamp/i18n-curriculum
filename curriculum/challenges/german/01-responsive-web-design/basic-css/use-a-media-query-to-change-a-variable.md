...
title: Verwende eine Medienabfrage (Media Query), um eine Variable zu ändern
challengeType: 0
...
Ändere im `:root` Selektor der Media Query `--penguin-size` so, dass sie neu definiert wird und einen Wert von `200px` erhält. Definiere außerdem `--penguin-skin` neu und weise den Wert `black` zu. Verändere dann die Größe der Vorschau, um diese Änderung in Aktion zu sehen.
