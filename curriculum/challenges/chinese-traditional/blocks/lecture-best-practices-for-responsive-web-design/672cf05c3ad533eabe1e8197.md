...
Media features describe specific characteristics of the user agent, output device, or environment. Let's talk about some of the most commonly used media features. The `width` and `height` refers to the viewport width and height, and are often used with `min-` or `max-` prefixes for range queries. 這是一個示例：
...
`aspect-ratio` describes the ratio between the width and height of the viewport. 這是一個示例：
...
The `orientation` feature indicates whether the device is in landscape or portrait orientation. 這是一個示例：
...
The `resolution` feature describes the resolution of the output device in dots per inch (dpi) or dots per centimeter (dpcm). 這是一個示例：
...
The `hover` feature tests whether the primary input mechanism can hover over elements. 這是一個示例：
...
The `prefers-color-scheme` feature detects if the user has requested a light or dark color theme. 這是一個示例：
