...
بما أن جميع الجوانب الأربعة (`4`) للقائمة لها نفس التباعد الداخلي، احذف الخصائص الأربعة واستخدم خاصية `padding` واحدة بقيمة `20px`.

# --hints--

يجب حذف خاصية `padding-left`.

```js
assert(!code.match(/padding-left/i));
```

يجب حذف خاصية `padding-right`.

```js
assert(!code.match(/padding-right/i));
```

يجب حذف خاصية `padding-top`.

```js
assert(!code.match(/padding-top/i));
```

يجب حذف خاصية `padding-bottom`.

```js
assert(!code.match(/padding-bottom/i));
```

يجب تعيين خاصية `padding` إلى `20px`.
...
يجب أن يحتوي عنصر `.menu` على قيمة `padding` تساوي `20px`.
