...
건물들이 너무 떨어져 있습니다. `div` 요소 상단에 빈 `.background-buildings` 요소 두 개를 추가하고, 하단에도 두 개를 추가하며, `.bb3`와 `.bb4` 사이에 하나를 더 추가해 건물들을 서로 더 가까이 모으세요. 이렇게 하면 컨테이너 전체에 고르게 배치되어 건물들이 중앙 쪽으로 더 가까워집니다.
