...
Erstellen Sie nach Ihrem letzten `.divider`-Element ein `p`-Element und geben Sie ihm den Text `Total Fat 8g 10%`. Wickeln Sie dann den Text `Total Fat` in ein `span`-Element, den Text `10%` in ein weiteres, und geben Sie beiden jeweils die Klasse `bold`.
...
Ihr neues `p`-Element sollte den Text `Total Fat 8g 10%` enthalten.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText?.match(/Total Fat[\s|\n]+8g[\s|\n]+10%/));
```

Ihr `p`-Element sollte zwei `span`-Elemente enthalten.
