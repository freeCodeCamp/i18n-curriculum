...
从 `.fb3` 中移除 `background-color` 属性和值，并将它们添加到 `.fb3a` 和 `.fb3b`。

# --hints--

你应该从 `.fb3` 中删除 `background-color`。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

你应该给 `.fb3a` 一个值为 `--building-color1` 的 `background-color`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

你应该给 `.fb3b` 一个值为 `--building-color1` 的 `background-color`。
