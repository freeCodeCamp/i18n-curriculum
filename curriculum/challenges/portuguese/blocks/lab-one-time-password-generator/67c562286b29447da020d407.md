...
Neste laboratório, você irá gerar um OTP (Senha Única) de 6 dígitos e exibí-lo para o usuário. O OTP expirará após 5 segundos, e um novo OTP será gerado quando o usuário clicar no botão `"Generate New OTP"`.
...
O `.container` deve conter um elemento `h1` com o ID `otp-title` com o texto `"OTP Generator"`.
...
Inicialmente, o `h2` dentro de `.container` deve exibir a mensagem `"Click 'Generate OTP' to get a code"`.
...
O `.container` deve conter um elemento `button` com o ID `generate-otp-button` e o texto `"Generate OTP"`.
...
O botão `"Generate OTP"` deve ser desabilitado enquanto o cronômetro regressivo estiver em execução.
...
O botão `"Generate OTP"` deve ser habilitado assim que o cronômetro regressivo atingir 0.
...
Quando o temporizador regressivo atingir `0`, você deve exibir a mensagem `OTP expired. Click the button to generate a new OTP.` Clique no botão para gerar um novo OTP.`.
