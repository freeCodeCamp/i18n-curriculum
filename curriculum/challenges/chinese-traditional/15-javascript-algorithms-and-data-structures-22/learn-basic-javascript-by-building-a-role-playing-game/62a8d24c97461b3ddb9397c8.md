...
一旦玩家擁有最強大的武器，你就可以賦予他們出售舊武器的能力。

在外部 `else` 語句中，將 `button2.innerText` 設置爲 `"Sell weapon for 15 gold"`。 還將 `button2.onclick` 設置爲函數名稱 `sellWeapon`。

# --hints--

你應該設置 `button2.innerText` 的值。

```js
assert.match(buyWeapon.toString(), /button2\.innerText/);
```

你應該將 `button2.innerText` 的值設置爲 `"Sell weapon for 15 gold"`。

```js
assert.match(buyWeapon.toString(), /button2\.innerText\s*=\s*('|")Sell weapon for 15 gold\1\s*;?/);
```

你應該設置 `button2.onclick` 的值。

```js
assert.match(buyWeapon.toString(), /button2\.onclick/);
```

你應該將 `button2.onclick` 的值設置爲 `sellWeapon`。
