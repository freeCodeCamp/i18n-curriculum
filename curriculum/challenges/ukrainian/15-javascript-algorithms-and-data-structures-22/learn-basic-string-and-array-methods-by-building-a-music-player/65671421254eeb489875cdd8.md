...
У функції зворотного виклику використайте метод `removeAttribute()`, щоб видалити атрибут `"aria-current"`. Це видалить атрибут для кожної з пісень.

# --hints--

Не змінюйте наявний `forEach()` та його вміст.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

Використайте метод `removeAttribute()` на `songEl` та передайте `"aria-current"`.
