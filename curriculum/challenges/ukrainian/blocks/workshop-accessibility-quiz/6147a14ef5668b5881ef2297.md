...
Щоб кнопки навігації виглядали більше як звичайні кнопки, видаліть підкреслення з тегів посилань.

Потім створіть новий селектор, який націлюється на елементи списку навігації, щоб коли курсор наводиться на них, змінювався колір фону та тексту, а курсор ставав вказівником.

# --hints--

Ви повинні використати існуючий селектор `li > a`, щоб встановити `text-decoration` у `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

Ви повинні використати або селектор `nav > ul > li:hover`, або `nav li:hover`, щоб стилізувати елементи при наведенні.
...
Ви повинні надати елементам `li` при наведенні `background-color` зі значенням `#dfdfe2`.
...
Ви повинні надати елементам `li` при наведенні `color` зі значенням `#1b1b32`.
...
Ви повинні надати елементам `li` при наведенні `cursor` зі значенням `pointer`.
