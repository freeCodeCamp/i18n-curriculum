...
如果符合此條件，將按鈕的 `disabled` 屬性設為 `true` 以停用按鈕。並且，將按鈕的 `textContent` 設為 `"No more data to load"`。
...
你應該將 `loadMoreBtn` 的 `disabled` 屬性設為 `true`。

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

你應該將 `loadMoreBtn` 的 `textContent` 設為 `"No more data to load"`。
