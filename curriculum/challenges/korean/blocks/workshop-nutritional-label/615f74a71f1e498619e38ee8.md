...
`% Daily Value *`와 `Saturated Fat 1g 5%` 요소 아래의 테두리가 레이블 전체 너비만큼 확장되지 않았습니다. 이 두 요소에 대해 `no-divider`를 `class`에 추가하세요.

# --hints--

텍스트가 `p`인 `% Daily Value *` 요소에 `no-divider`를 `class` 속성에 추가하세요. 기존 클래스는 제거하지 마세요.
...
텍스트가 `p`인 `Saturated Fat 1g 5%` 요소에 `no-divider`를 `class` 속성에 추가하세요. 기존 클래스는 제거하지 마세요.
