...
7. Елемент `.earth` має використовувати позиціювання `absolute`. Position the center of the `.earth` element at the halfway point of its parent on both the vertical (top) and horizontal (left) axes. After that, shift the `.earth` element back by half its own width and height, to center it within its parent, the `.space` element.
...
10.  Елемент `.orbit` має використовувати позиціювання `absolute`. Its bottom right corner should be at the center of the `.space` element using a `transform` property that shifts it by `-50%` on both the vertical and horizontal axes.

11. The orbit path for the moon around the Earth should be a circle.

12. Елемент `.moon` має використовувати позиціювання `absolute`, а також мати ширину і висоту зі значенням `30px`. The `.moon` element should position itself at the top of the `.orbit` element and be centered horizontally.
