...
Um die Gestaltung der Buttons abzuschließen, fügen Sie jedem `a`-Element die Utility-Klasse `font-semibold` hinzu, damit deren Text fetter dargestellt wird. Außerdem fügen Sie die Klasse `rounded` hinzu, um die scharfen Kanten der Buttons zu entfernen.
