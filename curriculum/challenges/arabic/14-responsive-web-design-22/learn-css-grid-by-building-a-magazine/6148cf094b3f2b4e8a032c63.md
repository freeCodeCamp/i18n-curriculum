...
قم بإنشاء منتقي `.first-paragraph::first-letter` وتعيين خاصية `font-size` إلى `6rem`. أعطيه أيضا خاصية `color` بقيمة `orangered` لجعله بارزا.

# --hints--

يجب أن يكون لديك منتقي `.first-paragraph::first-letter`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

يجب أن يكون لمنتقي `.first-paragraph::first-letter` الخاص بك خاصية `font-size` بقيمة `6rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

يجب أن يكون لمنتقي `.first-paragraph::first-letter` الخاص بك خاصية `color` بقيمة `orangered`.
