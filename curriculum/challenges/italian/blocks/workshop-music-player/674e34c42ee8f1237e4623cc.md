...
Aggiungi un blocco `else`. All'interno del blocco `else`, chiama la funzione `playSong` con l'argomento `id` della canzone attualmente in riproduzione.

Questo garantisce che la canzone attualmente in riproduzione continui a suonare quando viene cliccato il pulsante play.

# --hints--

Quando viene cliccato il `playButton` e `userData.currentSong` non è `null`, dovresti chiamare la funzione `playSong` con l'argomento `id` della canzone attualmente in riproduzione.
...
Dovresti comunque chiamare la funzione `playSong` con l'argomento `id` della prima canzone nella tua playlist quando viene cliccato il `playButton` e `userData.currentSong` è `null`,
