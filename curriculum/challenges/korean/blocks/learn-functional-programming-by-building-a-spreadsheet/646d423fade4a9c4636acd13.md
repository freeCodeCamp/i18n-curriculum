...
먼저 우선순위가 높은 연산자를 처리해야 합니다. `noHigh` 변수를 선언하고, `highPrecedence()`를 인수로 하여 `str`을 호출한 결과를 할당하세요.
