...
Em seguida, não há muita separação entre o nome, a imagem de cada autor e o resto dos detalhes do card. Um divisor dará aos cards de autor uma hierarquia visual clara.

Adicione um elemento `div` acima da biografia do autor e dê a ele um atributo `class` com o valor `"purple-divider"`.
...
Você deve criar um elemento `div` antes do elemento `p`.

```js
await __helpers.retryingTest(() => document.querySelector('p')?.previousElementSibling?.tagName === 'DIV', "'div' element not found");
```

Seu elemento `div` deve ter o `class` definido como `"purple-divider"`.
