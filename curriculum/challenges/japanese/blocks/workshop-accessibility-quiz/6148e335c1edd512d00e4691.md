...
ナビゲーションリンクをクリックすると、ビューポートが該当するセクションにジャンプします。しかし、このジャンプは一部のユーザーにとって混乱を招く場合があります。

すべての要素を選択し、`scroll-behavior` を `smooth` に設定してください。
...
`*` に `scroll-behavior` を `smooth` に設定してください。
