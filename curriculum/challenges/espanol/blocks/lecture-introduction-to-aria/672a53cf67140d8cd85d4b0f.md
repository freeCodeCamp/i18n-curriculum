...
Un rol ARIA define el propósito de un elemento dentro de un sitio web o una app web. Aquí tienes un ejemplo de cómo asignar el rol `button` a un elemento `div`.

```html
<div role="button">Click Me</div>
```

Al hacer esto indicas a la tecnología de asistencia que el elemento es un botón. Sin embargo, los roles no proporcionan ninguna funcionalidad. Solo darle a este `div` un `role` de `button` no hará que actúe como un botón. Para que se vea y se comporte como un botón, necesitarías usar CSS y JavaScript para lograr el resultado deseado.
...
Usa WAI-ARIA solo cuando HTML no sea suficiente, y no olvides probar con tecnologías de asistencia como lectores de pantalla, o que personas con discapacidades prueben tu trabajo. Además, asegúrate de que los estados y propiedades de WAI-ARIA se actualicen en tiempo real con el contenido. Evita usar ARIA en exceso, ya que puede resultar confuso.
...
Concéntrate en cómo WAI-ARIA mejora la accesibilidad de los elementos HTML.
