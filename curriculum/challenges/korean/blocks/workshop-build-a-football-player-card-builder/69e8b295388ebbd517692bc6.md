...
플레이어 사진은 하드코딩된 URL에서 불러옵니다. `src` 속성 값 `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg`을 `player.imageUrl`로 바꾸세요.

또한 하드코딩된 `alt` 텍스트를 `player.name`와 텍스트 `headshot`를 사용한 템플릿 리터럴로 동적 값으로 바꾸세요.

TSX에서 JavaScript 표현식을 속성 값으로 사용할 때는 따옴표 대신 중괄호 `{}`를 사용한다는 점을 기억하세요. 예를 들어 `src={player.imageUrl}`는 맞지만 `src="{player.imageUrl}"`는 아닙니다.

# --hints--

하드코딩된 이미지 URL을 `player.imageUrl`로 `src` 속성에 바꾸세요.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

하드코딩된 `alt` 텍스트를 `player.name`와 텍스트 `headshot`를 사용한 템플릿 리터럴로 바꾸세요.
