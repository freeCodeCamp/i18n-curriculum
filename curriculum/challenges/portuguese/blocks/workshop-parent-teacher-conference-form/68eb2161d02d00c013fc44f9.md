...
Para tornar o texto do `legend` mais destacado, você precisa estilizar ele.

Selecione o elemento `fieldset legend`. Defina seu `font-size` para `1.3rem` e seu `font-weight` para `600`.

`font-weight` controla a espessura do texto. 

Valores comuns de `font-weight` são:

- `normal` → Peso regular (padrão)
- `bold` → Texto em negrito
- `lighter` → Mais claro que o elemento pai
...
Você deve ter um seletor `fieldset legend`.

```js
assert(new __helpers.CSSHelp(document).getStyle('fieldset legend'));
```

Defina a propriedade `font-size` para `1.3rem`.
...
Defina a propriedade `font-weight` para `600`.
