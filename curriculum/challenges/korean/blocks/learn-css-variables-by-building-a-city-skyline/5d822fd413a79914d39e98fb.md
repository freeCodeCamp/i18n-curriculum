...
그라디언트 전환은 보통 한 색상에서 다른 색상으로 점진적으로 변합니다. 더 급격한 변화가 필요할 때는 다음과 같이 딱 끊기는 전환을 만들 수 있습니다:
...
`linear-gradient`를 `.bb2b`에 추가하고, `--building-color2`에서 `0%`까지 `6%`를 사용하며 `--window-color2`에서 `6%`까지 `9%`를 사용하세요.

# --hints--

`.bb2b`에 `background` 속성을 지정하세요.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

`linear-gradient`에 `background`를 사용하세요.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

`--building-color2`에서 `0%`까지 `6%`를 사용하세요.
...
`--window-color2`에서 `6%`까지 `9%`를 사용하세요.
...
`.bb2b`에는 `linear-gradient`에서 `--building-color2`까지 `0%`에서 전환되고, `6%`에서 `--window-color2`까지 `6%`에서 전환되는 `9%`가 있어야 합니다.
