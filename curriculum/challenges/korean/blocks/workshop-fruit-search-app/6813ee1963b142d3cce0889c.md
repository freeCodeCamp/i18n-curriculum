...
`try` 블록 안에서 `fetch` URL을 사용해 `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` 함수를 호출하고, 결과를 `response`라는 변수에 저장하세요. `async` 함수 안에 있으므로 `await` 전에 `fetch`를 사용해야 합니다.

# --hints--

`fetch`를 사용해 `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}`을 호출하고, 결과를 `response` 변수에 저장하세요.
