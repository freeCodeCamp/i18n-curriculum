...
En tu archivo de JavaScript, selecciona el elemento `.story-container` y guárdalo en una variable llamada `storyContainer`.

# --hints--

Debes tener una variable `storyContainer`.

```js
assert.isDefined(storyContainer);
```

Debes seleccionar el elemento `.story-container` usando el método `querySelector` y guardarlo en la variable `storyContainer`.
