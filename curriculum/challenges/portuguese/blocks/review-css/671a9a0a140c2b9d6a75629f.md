...
- **O que é CSS?**: Cascading Style Sheets (CSS) é uma linguagem de folhas de estilo usada para aplicar estilos a elementos HTML. O CSS é usado para cores, imagens de fundo, layouts e muito mais.
- **Anatomia básica de uma regra CSS**: Uma regra CSS é composta por duas partes principais: um seletor e um bloco de declaração. Um seletor é um padrão usado no CSS para identificar e direcionar elementos HTML específicos para estilização. Um bloco de declaração aplica um conjunto de estilos para um seletor ou seletores.
- **Elemento `meta name="viewport"`**: Este elemento `meta` dá ao navegador instruções sobre como controlar as dimensões e o escalonamento da página em diferentes dispositivos, especialmente em celulares e tablets.
- **Estilos padrão do navegador**: Cada elemento HTML terá estilos padrão do navegador aplicados a ele. Isso geralmente inclui itens como margens e preenchimentos padrão.
...
- **Combinador Descendente (` `)&nbsp;\*um único espaço entre seletores**: Este combinador é usado para direcionar elementos que são descendentes de um elemento pai especificado.
...
- **Elementos inline**: Elementos inline ocupam apenas a largura necessária e não começam em uma nova linha. Esses elementos fluem dentro do conteúdo, permitindo que texto e outros elementos inline apareçam ao lado deles. Elementos inline comuns são os elementos `span`, `a` e `img`.
- **Elementos block**: Elementos block ocupam toda a largura disponível por padrão, estendendo-se pela largura do seu container. Alguns elementos block comuns são `div`, `p` e `section`.
- **Elementos inline-block**: Você pode definir um elemento como `inline-block` usando a propriedade `display`. Esses elementos se comportam como inline, mas podem ter `width` e `height` definidos como elementos block.
...
- **Especificidade do CSS inline**: O CSS inline tem a maior especificidade porque é aplicado diretamente ao elemento. Ele sobrescreve qualquer CSS interno ou externo. O valor de especificidade para estilos inline é (1, 0, 0, 0).
- **Especificidade do CSS interno**: O CSS interno é definido dentro de um elemento `style` na seção `head` do documento HTML. Ele tem especificidade menor que estilos inline. Tanto o CSS interno quanto o externo compartilham o mesmo nível de especificidade, que é determinado pelos seus seletores, não pela localização.
- **Especificidade do CSS externo**: O CSS externo é vinculado via um elemento `link` na seção `head` e escrito em arquivos `.css` separados. Assim como o CSS interno, sua especificidade é determinada pelos seletores usados. Quando duas regras têm especificidade igual, a ordem no código determina o vencedor: a regra que aparece depois no documento tem precedência. O CSS externo oferece a melhor manutenção para projetos maiores.
- **Seletor universal (`*`)**: É um tipo especial de seletor CSS que corresponde a qualquer elemento no documento. É frequentemente usado para aplicar um estilo a todos os elementos da página, útil para resetar ou normalizar estilos entre navegadores. O seletor universal tem o menor valor de especificidade de qualquer seletor. Ele contribui com 0 para todas as partes do valor de especificidade (0, 0, 0, 0).
- **Seletores de tipo**: Esses seletores miram elementos com base no nome da tag. Eles têm especificidade relativamente baixa comparada a outros seletores. O valor de especificidade para um seletor de tipo é (0, 0, 0, 1).
- **Seletores de classe**: São definidos por um ponto (`.`) seguido do nome da classe. O valor de especificidade para um seletor de classe é (0, 0, 1, 0). Isso significa que seletores de classe podem sobrescrever seletores de tipo, mas podem ser sobrescritos por seletores de ID e estilos inline.
- **Seletores de ID**: São definidos por um símbolo de hash (`#`) seguido do nome do ID. Seletores de ID têm especificidade muito alta, maior que seletores de tipo e classe, mas menor que estilos inline. O valor de especificidade para um seletor de ID é (0, 1, 0, 0).
- **Palavra-chave `!important`**: Usada para dar a uma regra de estilo a maior prioridade, permitindo que ela sobrescreva quaisquer outras declarações para uma propriedade. Quando usada, força o navegador a aplicar o estilo especificado, independentemente da especificidade de outros seletores. Deve-se ter cuidado ao usar `!important` porque pode tornar seu CSS mais difícil de manter e depurar.
- **Algoritmo de cascata**: Algoritmo usado para decidir quais regras CSS aplicar quando múltiplos estilos miram o mesmo elemento. Garante que os estilos mais apropriados sejam usados, com base em um conjunto de regras bem definidas.
- **Herança no CSS**: Processo pelo qual estilos são passados de elementos pai para seus filhos. A herança permite definir estilos em um nível mais alto na árvore do documento e aplicá-los a múltiplos elementos sem especificá-los explicitamente para cada um.
...
- **Modo escuro**: Recurso especial em aplicações web que permite trocar o esquema de cores claro padrão por um esquema escuro. Você deve usar cores dessaturadas no modo escuro. Cores dessaturadas são menos intensas e têm nível de saturação menor.
- **Breadcrumbs**: Auxílio de navegação que mostra ao usuário onde ele está na hierarquia do site. É melhor posicionar os breadcrumbs no topo da página para que os usuários os encontrem facilmente. Também é importante garantir que os breadcrumbs sejam grandes o suficiente para serem lidos com facilidade, mas não tão grandes a ponto de ocupar muito espaço na página.
- **Componente card**: Seu componente card deve ter design simples, sem poluição visual ou excesso de informações. Para mídias, escolha imagens e vídeos de alta qualidade para melhorar a experiência do usuário.
- **Scroll infinito**: Padrão de design que carrega mais conteúdo conforme o usuário rola a página para baixo. Você deve considerar usar um botão de carregar mais, pois dá ao usuário controle sobre quando quer ver mais conteúdo. Também pode adicionar um botão de voltar para que usuários possam retornar à página anterior sem precisar rolar até o topo.
- **Modal dialog**: Tipo de pop-up que aparece sobre o conteúdo existente da página. Normalmente, o conteúdo de fundo recebe uma sobreposição de cor escura para ajudar o usuário a focar no conteúdo do modal. Também é sempre uma boa ideia permitir que o usuário clique fora do modal para fechá-lo. Ao usar o elemento HTML `dialog`, você obtém muitas funcionalidades e benefícios de acessibilidade embutidos.
- **Indicação de progresso para registro em formulário**: Forma de mostrar aos usuários o quanto eles avançaram em um processo. Pode ser usado em formulários, registros e processos de configuração. Seu design deve ser simples, fácil de encontrar e permitir voltar a passos anteriores.
- **Carrinho de compras**: Espaço onde usuários veem os itens que já selecionaram em uma plataforma de comércio eletrônico. Seus carrinhos devem estar sempre visíveis para o usuário, usar um ícone comum como carrinho, bolsa ou cesta, e ter um botão claro de call-to-action para os usuários prosseguirem para o checkout.

## Ferramentas Comuns de Design

- **Figma**: Ferramenta baseada na nuvem especializada em design de interface do usuário e experiência do usuário (UI/UX). Permite que equipes de design e desenvolvimento colaborem de qualquer lugar, oferecendo recursos embutidos como design vetorial, layout automático, sistema de comentários e feedback, entre outros.
- **Sketch**: Ferramenta de design popular usada por sua interface intuitiva e simplicidade, ideal para desenvolvedores que querem criar protótipos rapidamente. Também é amplamente usada por designers para tarefas como criação de UIs, ícones e layouts web.
- **Adobe XD**: Ferramenta vetorial de design e prototipagem para UI/UX, conhecida pela integração perfeita com outros apps Adobe como Photoshop, Illustrator e After Effects.
- **Canva**: Ferramenta que permite criar uma ampla variedade de conteúdos visuais, incluindo pôsteres, fotos de capa, apresentações, vídeos curtos e mais. Seu design simples e amigável a torna ideal para iniciantes.

## Unidades Absolutas

- **`px` (Pixels)**: Unidade absoluta fixa de medida no CSS, que oferece controle preciso sobre dimensões. Isso significa que 1px é sempre igual a 1/96 de polegada.
- **`in` (Polegada)**: Unidade absoluta igual a 96px.
- **`cm` (Centímetros)**: Unidade absoluta igual a 25.2/64 de polegada.
- **`mm` (Milímetros)**: Unidade absoluta igual a 1/10 de centímetro.
- **`q` (Quarto de milímetro)**: Unidade absoluta igual a 1/40 de centímetro.
- **`pc` (Picas)**: Unidade absoluta igual a 1/6 de polegada.
- **`pt` (Pontos)**: Unidade absoluta igual a 1/72 de polegada.
...
- **Pseudo-classes de localização**: Essas pseudo-classes são usadas para estilizar links e elementos que são alvos dentro do documento atual.
- **Pseudo-classe `:any-link`**: Combinação das pseudo-classes `:link` e `:visited`. Corresponde a qualquer elemento âncora com atributo `href`, independentemente de ter sido visitado ou não.
- **Pseudo-classe `:link`**: Permite selecionar todos os links não visitados em uma página. Pode ser usada para estilizar links de forma diferente antes do usuário clicar.
- **Pseudo-classe `:local-link`**: Seleciona links que apontam para o mesmo documento. Útil para diferenciar links internos de externos.
- **Pseudo-classe `:visited`**: Seleciona um link que o usuário já visitou.
- **Pseudo-classe `:target`**: Usada para aplicar estilos a um elemento que é o alvo de um fragmento de URL.
...
- **Pseudo-classes funcionais**: Pseudo-classes funcionais permitem selecionar elementos com base em condições ou relacionamentos mais complexos. Diferente das pseudo-classes regulares que selecionam elementos com base em um estado (por exemplo, `:hover`, `:focus`), as pseudo-classes funcionais aceitam argumentos.
- **Pseudo-classe `:is()`**: Essa pseudo-classe recebe uma lista de seletores (por exemplo, `ol`, `ul`) e seleciona um elemento que corresponda a um dos seletores da lista.
- **Pseudo-classe `:where()`**: Essa pseudo-classe recebe uma lista de seletores (por exemplo, `ol`, `ul`) e seleciona um elemento que corresponda a um dos seletores da lista. A diferença entre `:is()` e `:where()` é que esta última terá especificidade 0.
- **Pseudo-classe `:has()`**: Essa pseudo-classe é frequentemente chamada de seletor `"parent"` porque permite estilizar elementos que contenham elementos filhos especificados na lista de seletores.
...
- **Definição de teoria das cores**: Estudo de como as cores interagem entre si e como afetam nossa percepção. Abrange relações de cores, harmonia de cores e impacto psicológico das cores.
- **Cores primárias**: São amarelo, azul e vermelho, as matizes fundamentais das quais todas as outras cores derivam.
- **Cores secundárias**: Resultam da mistura em partes iguais de duas cores primárias. Verde, laranja e roxo são exemplos de cores secundárias.
- **Cores terciárias**: Resultam da combinação de uma cor primária com uma cor secundária vizinha. Amarelo-verde, azul-verde e azul-violeta são exemplos de cores terciárias.
- **Cores quentes**: Incluem vermelhos, laranjas e amarelos, evocando sensações de conforto, calor e aconchego.
- **Cores frias**: Incluem azuis, verdes e roxos, evocando sensações de calma, serenidade e profissionalismo.
- **Círculo cromático**: Diagrama circular que mostra como as cores se relacionam. É uma ferramenta essencial para designers porque ajuda a selecionar combinações de cores.
- **Esquemas de cores análogos**: Criam experiências coesas e suaves. Contêm cores análogas, que são adjacentes no círculo cromático.
- **Esquemas de cores complementares**: Criam alto contraste e impacto visual. Suas cores estão localizadas em extremos opostos do círculo cromático.
- **Esquema de cores triádico**: Possui cores vibrantes, feitas de cores aproximadamente equidistantes entre si. Se conectadas, formam um triângulo equilátero no círculo cromático.
- **Esquema monocromático**: Todas as cores derivam da mesma cor base ajustando sua luminosidade, escuridão e saturação. Evoca sensação de unidade e harmonia, mantendo contraste.
...
- **Função `rgb()`**: RGB significa Red, Green e Blue, as cores primárias da luz. Essas três cores são combinadas em diferentes intensidades para criar uma ampla gama de cores. A função `rgb()` permite definir cores usando o modelo RGB.
- **Função `rgba()`**: Adiciona um quarto valor, alpha, que controla a transparência da cor.
- **Função `hsl()`**: HSL significa Hue, Saturation e Lightness — três componentes chave que definem uma cor.
- **Função `hsla()`**: Adiciona um quarto valor, alpha, que controla a opacidade da cor.
- **Hexadecimal**: Código hex (abreviação de código hexadecimal) é uma string de seis caracteres usada para representar cores no modelo RGB. "Hex" refere-se ao sistema numérico base 16, que usa dígitos de 0 a 9 e letras de A a F.
...
- **Definição**: Esta propriedade determina como os itens flexíveis são quebrados dentro de um container flexível para caber no espaço disponível. `flex-wrap` pode assumir três valores possíveis: `nowrap`, `wrap` e `wrap-reverse`.
- **`flex-wrap: nowrap;`**: Valor padrão. Os itens flexíveis não serão quebrados em uma nova linha, mesmo que sua largura exceda a do container.
- **`flex-wrap: wrap;`**: Esta propriedade fará com que os itens sejam quebrados quando excederem a largura do container.
- **`flex-wrap: wrap-reverse;`**: Esta propriedade quebra os itens flexíveis em ordem reversa.
- **Propriedade `flex-flow`**: Propriedade abreviada para `flex-direction` e `flex-wrap`.
...
- **Fontes web safe**:
  - Fontes sans-serif são comumente usadas para desenvolvimento web porque não têm pequenos "pés" ou linhas nas extremidades dos caracteres, facilitando a leitura na tela. Alguns exemplos de fontes sans-serif web safe são: Arial, Verdana e Trebuchet MS.
  - Em contraste, fontes serif têm pequenos "pés" nas extremidades dos caracteres, sendo comumente usadas para impressão tradicional. Fontes serif web safe incluem: Times New Roman e Georgia.
...
- **Grades fluidas**: Usam unidades relativas como porcentagens em vez de unidades fixas como pixels, permitindo que o conteúdo redimensione e se reorganize conforme o tamanho da tela.
- **Imagens flexíveis**: São configuradas para redimensionar dentro de seus elementos contêineres, garantindo que não ultrapassem seus containers em telas menores.
...
- **`--property-name`**: É o nome da propriedade personalizada que você está definindo. Como todas as propriedades personalizadas, deve começar com dois traços.
...
- **Fallbacks**: Ao usar a propriedade personalizada, você pode fornecer um valor fallback usando a função `var()`, assim como faria com propriedades personalizadas padrão.
