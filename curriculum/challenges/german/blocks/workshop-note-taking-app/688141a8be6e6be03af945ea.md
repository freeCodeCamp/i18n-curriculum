...
Sie sollten `let` verwenden, um die Variable `currentContent` zu erstellen.

```js
assert.match(code, /let\s+currentContent/);
```

Ihre `currentContent`-Variable sollte ein String sein.

```js
assert.isString(currentContent);
```

Ihre `currentContent`-Variable sollte mit einem leeren String initialisiert werden.
