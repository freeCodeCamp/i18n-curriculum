...
Your `head` element should have an opening tag. Las etiquetas de apertura tienen esta sintaxis: `<elementName>`.

```js
assert(code.match(/\<head\>/));
```

Your `head` element should have a closing tag. Las etiquetas de cierre tienen una `/` inmediatamente después del carácter `<`.
