...
title: construir um gráfico de barras
challengeType: 25
...
Neste projeto você vai usar os dados do `https://cdn.freecodecamp.org/curriculum/labs/data/bar-chart/GDP-data.json` para construir um gráfico de barras.

Os testes exigem que os eixos sejam gerados usando a propriedade axis do D3, que gera automaticamente ticks ao longo do eixo. Esses ticks são necessários para passar nos testes do D3 porque suas posições são usadas para determinar o alinhamento dos elementos do gráfico.

**Objetivo:** Atenda às user stories abaixo e faça todos os testes passarem para completar o laboratório.

**User stories:**

1. Seu gráfico deve ter um título com um `id="title"` correspondente.
1. Seu gráfico deve ter um elemento `g` para o eixo x com um `id="x-axis"` correspondente.
1. Seu gráfico deve ter um elemento `g` para o eixo y com um `id="y-axis"` correspondente.
1. Ambos os eixos devem conter múltiplos rótulos de ticks, cada um com um `class="tick"` correspondente.
1. Seu gráfico deve ter um elemento `rect` para cada ponto de dado com um `class="bar"` correspondente exibindo os dados.
1. Cada `.bar` deve ter os atributos `data-date` e `data-gdp` contendo valores `date` e `GDP`.
1. Os atributos `.bar` dos elementos `data-date` devem corresponder à ordem dos dados fornecidos.
1. Os atributos `.bar` dos elementos `data-gdp` devem corresponder à ordem dos dados fornecidos.
1. A altura de cada elemento `.bar` deve representar com precisão o `GDP` correspondente dos dados.
1. O atributo `data-date` e seu elemento `.bar` correspondente devem alinhar-se com o valor correspondente no eixo x.
1. O atributo `data-gdp` e seu elemento `.bar` correspondente devem alinhar-se com o valor correspondente no eixo y.
1. Você deve conseguir passar o mouse sobre uma área e ver um tooltip com um `id="tooltip"` correspondente que exibe mais informações sobre a área.
1. Seu tooltip deve ter um atributo `data-date` que corresponda ao `data-date` da área ativa.
...
O gráfico deve ter um elemento com o `id` de `title` para conter o título.

```js
assert.exists(document.getElementById('title'));
```

O gráfico deve ter um elemento `g` para o eixo x com um id `x-axis` correspondente.

```js
assert.isNotEmpty(document.querySelector('g#x-axis'))
```

O gráfico deve ter um elemento `g` para o eixo y com um id `y-axis` correspondente.

```js
assert.isNotEmpty(document.querySelector('g#y-axis'))
```

O eixo x deve conter múltiplos rótulos de ticks, cada um com o `class="tick"` correspondente.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

O eixo y deve conter múltiplos rótulos de ticks, cada um com o `class="tick"` correspondente.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

Seu gráfico deve ter um elemento `rect` para cada ponto de dado com um `class="bar"` correspondente exibindo os dados.

```js
assert.lengthOf(document.querySelectorAll('rect.bar'), GDPDataJson.data.length);
```

Cada barra deve ter os atributos `data-date` e `data-gdp` contendo valores de data e GDP.
...
Os atributos `data-date` dos elementos barra devem corresponder à ordem dos dados fornecidos.
...
Os atributos `data-gdp` dos elementos barra devem corresponder à ordem dos dados fornecidos.
...
A altura de cada elemento barra deve representar com precisão o GDP correspondente dos dados.
...
O atributo `data-date` e seu elemento barra correspondente devem alinhar-se com o valor correspondente no eixo x. Você pode usar `d3.scaleTime()` para criar uma escala de tempo para o eixo x.
...
O atributo `data-gdp` e seu elemento barra correspondente devem alinhar-se com o valor correspondente no eixo y. Você pode usar `d3.scaleLinear()` para criar uma escala linear para o eixo y.
...
Ao passar o mouse sobre uma área, seu gráfico de barras deve ter um tooltip com um `id="tooltip"` correspondente que exibe mais informações sobre a área.
...
O tooltip deve ter um atributo `data-date` que corresponda ao `data-date` da área ativa.
