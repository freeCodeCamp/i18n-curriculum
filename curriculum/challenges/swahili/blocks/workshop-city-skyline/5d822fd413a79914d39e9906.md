...
Jengo linalofuata litakuwa na sehemu tatu. Panga ndani vipengele vitatu vya `div` ndani ya `.bb4`. Wape madarasa ya `bb4a`, `bb4b` na `bb4c` kwa mpangilio huo.
...
Unapaswa kumpa `div` mpya la kwanza darasa la `bb4a`.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

Unapaswa kumpa `div` mpya la pili darasa la `bb4b`.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

Unapaswa kumpa `div` mpya la tatu darasa la `bb4c`.
