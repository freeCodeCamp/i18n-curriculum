...
Necesitas obtener la canción que se está reproduciendo actualmente o la primera canción de la lista de reproducción. Para hacer eso, crea una constante `song` y usa el operador OR (`||`) para asignarla a la canción actual de `userData`, **o** la primera canción en el arreglo `userData?.songs`.
...
Deberías asignar `userData?.currentSong || userData?.songs[0]` a una constante `song`.
