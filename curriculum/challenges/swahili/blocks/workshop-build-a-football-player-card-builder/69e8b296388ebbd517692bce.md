...
Sasa ongeza `div` yenye `className` ya `form-group`. Ndani yake, ongeza `label` ya `className` ya `label` na sifa ya kipengele ya `htmlFor` ya `name`. Chini yake ongeza `input` yenye `id` iliyowekwa kuwa `name`, `className` ya `input`, na sifa ya kipengele ya `type` ya `text`.
