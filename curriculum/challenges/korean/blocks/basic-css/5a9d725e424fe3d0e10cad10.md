...
title: 여러 요소를 한 번에 바꾸기 위해 CSS 변수 사용하기
challengeType: 0
...
<dfn>CSS 변수</dfn>는 단 하나의 값만 바꿔도 여러 CSS 스타일 속성을 한꺼번에 바꿀 수 있는 강력한 방법입니다.

아래 지침을 따라 세 가지 값만 바꿔도 여러 요소의 스타일이 어떻게 바뀌는지 확인하세요.

# --instructions--

`penguin` 클래스에서 `black` 값을 `gray`로, `gray` 값을 `white`로, `yellow` 값을 `orange`로 바꾸세요.
...
`penguin` 클래스는 `--penguin-belly` 변수를 선언하고 `white`로 할당해야 합니다.

```js
assert.match(code, /.penguin\s*?{[\s\S]*--penguin-belly\s*?:\s*?white\s*?;[\s\S]*}/gi);
```

`penguin` 클래스는 `--penguin-beak` 변수를 선언하고 `orange`로 할당해야 합니다.
