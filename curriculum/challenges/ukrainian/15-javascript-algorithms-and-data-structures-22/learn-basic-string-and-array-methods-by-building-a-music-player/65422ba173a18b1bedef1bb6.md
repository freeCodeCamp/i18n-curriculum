...
В межах блоку `if` викличте функцію `playSong()` із `id` першої пісні в масиві `userData?.songs`. Це забезпечить відтворення першої пісні зі списку відтворення.

# --hints--

Створіть інструкцію `if` з умовою `userData?.currentSong === null`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{/);
```

Викличте функцію `playSong` з `userData?.songs[0].id` всередині блока `if`.
