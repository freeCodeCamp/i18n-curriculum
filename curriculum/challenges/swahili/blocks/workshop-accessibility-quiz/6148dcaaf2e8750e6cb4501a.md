...
Fanya vipengele vya `h3` viwe na kipaumbele cha juu zaidi, kwa kutumia mali zifuatazo za CSS:
...
Unapaswa kutumia kichaguzi cha kipengele cha `h3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3'));
```

Unapaswa kumpa kipengele cha `h3` vigezo vya `margin-top` vya `5px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Unapaswa kumpa kipengele cha `h3` vigezo vya `padding-left` vya `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Unapaswa kumpa kipengele cha `h3` vigezo vya `font-size` vya `1.375ren`.
