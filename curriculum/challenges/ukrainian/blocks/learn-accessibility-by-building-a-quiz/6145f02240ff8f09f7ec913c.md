...
В межах елементів `div.question-block` вкладіть один елемент `label` і додайте пов’язане з `CSS` питання до тексту `label`.

# --hints--

Ви повинні вкласти один елемент `label` у перший елемент `div.question-block`.

```js
assert.exists(document.querySelectorAll('.formrow > .question-block')?.[0]?.querySelector('label'));
```

Ви повинні вкласти один елемент `label` у другий елемент `div.question-block`.
