...
Katika kipengele cha tatu na cha mwisho cha `section`, utaunda muundo sawa wa HTML kama ulivyofanya katika sehemu iliyopita.

Kwanza, ndani ya sehemu ya mwisho, ongeza kipengele cha `h2` chenye maandishi ya `Importance of Building a Reputation`.

Kisha, chini ya kipengele cha `h2`, ongeza kipengele cha nukuu ya kipengele chenye sifa ya `cite` iliyowekwa kuwa `https://www.freecodecamp.org/news/learn-to-code-book/`.

Mwishowe, panga ndani vipengele vitatu vya aya ndani ya kipengele cha nukuu ya kipengele na maandishi yafuatayo, kwa mpangilio:
...
Hongera! Kwa hili umemaliza warsha hii.

# --hints--

Unapaswa kuwa na kipengele cha `h2` kilichopangwa ndani ya sehemu ya tatu.

```js
assert.exists(document.querySelector('main > section:nth-of-type(3) > h2'));
```

Kipengele chako cha `h2` kinapaswa kuwa na maandishi ya `Importance of Building a Reputation`.
...
Unapaswa kuwa na kipengele cha `blockquote` kilichopangwa ndani ya sehemu ya tatu.
...
Sifa yako ya `cite` ya kipengele cha `blockquote` inapaswa kuwa na thamani ya `https://www.freecodecamp.org/news/learn-to-code-book/`.
...
Unapaswa kuwa na vipengele vitatu vya `p` vilivyopangwa ndani ya kipengele cha `blockquote` katika sehemu ya tatu.
...
Kipengele chako cha kwanza cha `p` kinapaswa kuwa na maandishi ya `Share short video demos of your projects.`
...
Kipengele chako cha pili cha `p` kinapaswa kuwa na maandishi ya `Keep applying to speak at bigger and bigger conferences.`
...
Kipengele chako cha tatu cha `p` kinapaswa kuwa na maandishi ya `Hang out at hackerspaces and help people who are even newer to coding than you.`
