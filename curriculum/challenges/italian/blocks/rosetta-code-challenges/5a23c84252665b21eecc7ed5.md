...
title: Il problema del cavaliere
challengeType: 1
forumTopicId: 302297
...
Problema del tour del cavaliere: hai una scacchiera `width` \* `height` vuota, tranne per un singolo cavaliere posizionato su una casella. Il cavaliere deve eseguire una sequenza di mosse legali che porti a visitare ogni casella della scacchiera esattamente una volta. Nota che *non* è necessario che il tour sia "chiuso"; cioè, il cavaliere non deve necessariamente terminare a una mossa di distanza dalla posizione di partenza.

# --instructions--

Scrivi una funzione che prenda `width` e `height` come parametri e restituisca il numero di posizioni iniziali da cui è possibile completare l'attività descritta sopra.
...
`knightTour(6, 6)` dovrebbe restituire `36`.

```js
assert.equal(knightTour(6, 6), 36);
```

`knightTour(5, 6)` dovrebbe restituire `30`.

```js
assert.equal(knightTour(5, 6), 30);
```

`knightTour(4, 6)` dovrebbe restituire `12`.

```js
assert.equal(knightTour(4, 6), 12);
```

`knightTour(7, 3)` dovrebbe restituire `10`.

```js
assert.equal(knightTour(7, 3), 10);
```

`knightTour(8, 6)` dovrebbe restituire `48`.
