...
게임의 마지막 부분에서는 스트레이트가 있는지 확인하는 알고리즘을 만들어야 합니다. 작은 스트레이트는 네 개의 주사위가 어떤 순서로든 연속된 값을 가질 때입니다(예: `41423` 굴림에서 `1234`가 있을 때) 점수는 `30`점입니다. 큰 스트레이트는 다섯 개의 주사위가 어떤 순서로든 연속된 값을 가질 때입니다(예: `35124` 굴림에서 `12345`가 있을 때) 점수는 `40`점입니다.

숫자 배열을 매개변수로 받는 `checkForStraights` 함수를 선언하세요. 사용자가 큰 스트레이트를 얻으면 다섯 번째 라디오 버튼을 `40` 점수로 업데이트하세요. 사용자가 작은 스트레이트를 얻으면 네 번째 라디오 버튼을 `30` 점수로 업데이트하세요. 어떤 경우든 마지막 라디오 버튼은 항상 점수 0을 표시하도록 올바른 속성으로 업데이트해야 합니다.

주사위 게임을 완료하려면 `checkForStraights`가 클릭될 때 `rollDiceBtn` 함수를 호출하세요!

# --hints--

`checkForStraights` 변수는 함수여야 합니다.

```js
assert.isFunction(checkForStraights);
```

작은 스트레이트가 나오면 `checkForStraights` 함수가 네 번째 라디오 버튼을 활성화하고, 값을 `30`으로 설정하며, 표시된 텍스트를 `, score = 30`로 업데이트하고 다섯 번째 라디오 버튼은 비활성 상태로 두어야 합니다.
...
큰 스트레이트가 나오면 `checkForStraights` 함수가 네 번째 버튼을 활성화하고, 값을 `30`으로 설정하며, 표시된 텍스트를 `, score = 30`로 업데이트해야 합니다. 추가로 다섯 번째 라디오 버튼도 활성화하고, 값을 `40`으로 설정하며, 표시된 텍스트를 `, score = 40`로 업데이트해야 합니다.
...
스트레이트가 없으면 `checkForStraights` 함수가 네 번째나 다섯 번째 라디오 버튼을 활성화하지 않아야 합니다.
...
스트레이트가 없으면 `checkForStraights` 함수가 마지막 라디오 버튼을 활성화하고, 값을 `0`로 설정하며, 표시된 텍스트를 `, score = 0`로 업데이트해야 합니다.
...
`checkForStraights`가 클릭될 때 `rollDiceBtn` 함수를 호출해야 합니다.
