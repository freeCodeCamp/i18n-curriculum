...
أضف خاصيتي `value` و `onChange` إلى كل من مُدخَلي DEF و PHY.

بالنسبة لـ DEF، عيّن `value` إلى `player.def` و`onChange` إلى دالة سهمية تستدعي `setPlayer` مع نشر `player` وتعيين `def` إلى `Number(e.target.value)`.

بالنسبة لـ PHY، عيّن `value` إلى `player.phy` و`onChange` إلى دالة سهمية تستدعي `setPlayer` مع نشر `player` وتعيين `phy` إلى `Number(e.target.value)`.

مع هذه الخطوة، أصبحت كل المُدخَلات متصلة بالحالة الآن، وستُحدَّث المعاينة الحية لكل حقل!

# --hints--

يجب أن يحتوي `input` الخاص بـ DEF على `value` يساوي قيمة `def` للاعب.
...
يجب أن يحتوي `input` الخاص بـ DEF على معالِج `onChange` يستدعي `setPlayer` مع نشر `player` وضبط `def` على `Number(e.target.value)`.
...
يجب أن يحتوي `input` الخاص بـ PHY على `value` تساوي قيمة `phy` الخاصة باللاعب.
...
يجب أن يحتوي `input` الخاص بـ PHY على معالِج `onChange` يستدعي `setPlayer` مع نشر `player` وضبط `phy` على `Number(e.target.value)`.
