...
title: Costruisci un convertitore di valuta
challengeType: 25
...
1. Il tuo componente `CurrencyConverter` dovrebbe renderizzare un elemento `input` per accettare l'importo da convertire.
2. Il tuo elemento `input` dovrebbe accettare numeri.
3. Il tuo componente `CurrencyConverter` dovrebbe renderizzare due elementi `select` per scegliere la valuta da cui convertire e quella in cui convertire.
4. Il tuo elemento `select` dovrebbe includere opzioni per **almeno** `USD`, `EUR`, `GBP` e `JPY`. Puoi usare qualsiasi tasso di cambio, purché non ci sia una corrispondenza uno a uno tra le valute. Ecco alcuni esempi di mappature buone e cattive:
...
5. Il tuo componente `CurrencyConverter` dovrebbe memorizzare il calcolo degli importi convertiti per la valuta **da cui** in modo che un cambiamento nell'opzione `select` **a cui** non ricalcoli gli importi convertiti.
6. Il tuo componente `CurrencyConverter` dovrebbe renderizzare un elemento che mostra l'importo convertito nel formato `XX.XX CCC`, dove `XX.XX` è l'importo convertito e `CCC` è il codice della valuta.
7. L'importo convertito dovrebbe essere arrotondato a due cifre decimali.

# --hints--

Dovresti esportare un componente `CurrencyConverter`.
...
Dovresti avere un elemento `input[type="number"]` per accettare l'importo da convertire.
...
Dovresti avere due elementi `select`.
...
Gli elementi `select` dovrebbero avere opzioni per almeno `USD`, `EUR`, `GBP` e `JPY`.
...
Cambiare il valore del primo elemento `select` dovrebbe causare il ricalcolo degli importi convertiti.
...
Cambiare il valore del secondo elemento `select` non dovrebbe causare il ricalcolo degli importi convertiti.
...
Cambiare il valore del primo elemento `select` dovrebbe mostrare il nuovo importo convertito.
...
Cambiare il valore del secondo elemento `select` dovrebbe mostrare il nuovo importo convertito e la valuta.
...
L'importo convertito dovrebbe essere mostrato nel formato `XX.XX CCC`, dove `XX.XX` è l'importo convertito arrotondato a due cifre decimali e `CCC` è il codice della valuta.
...
L'importo convertito dovrebbe essere diverso dall'importo di input.
