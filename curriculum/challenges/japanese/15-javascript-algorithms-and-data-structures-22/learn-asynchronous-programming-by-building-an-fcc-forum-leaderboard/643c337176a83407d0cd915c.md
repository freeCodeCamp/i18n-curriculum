...
結果を見るため、`try` ブロックの中で変数 `data` をコンソールに出力してください。

そして、`fetchData` の定義の下でこの関数を呼び出してください。コンソールを開いて結果を確認しましょう。

# --hints--

`console.log` を使って、変数 `data` をコンソールにログ出力してください。

```js
assert.match(code, /\s*console\.log\(\s*data\s*\)/);
```

`console.log` は `try` ブロック内にある必要があります。

```js
assert.match(code, /\s*try\s*{[\s\S]*\s*console\.log\(\s*data\s*\)[\s\S]*}\s*catch\s*\(\s*err\s*\)\s*{\s*}/);
```

`fetchData()` 関数が定義された後に、`fetchData()` 関数を呼び出してください。
