...
이제 작성자 데이터를 UI에 채우는 함수를 만드세요. 이 함수는 두 번째 `.then()` 메서드 안에서 호출할 것입니다.

`displayAuthors`를 매개변수로 받는 빈 화살표 함수 `authors`을 만드세요.
...
빈 함수를 만들 때 `const`을 사용해 `displayAuthors`라는 이름으로 만드세요.

```js
assert.match(code, /const\s+displayAuthors\s*=\s*/)
```

`displayAuthors`는 함수여야 합니다.

```js
assert.isFunction(displayAuthors)
```

`displayAuthors` 함수는 화살표 구문을 사용해야 합니다.
