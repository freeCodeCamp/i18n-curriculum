...
Dentro do método `singleCardSelected`, recupere o elemento `singleCard` e atualize seu `innerHTML` chamando a função `renderCard`. Passe `"Click the card and reveal the fortune"` como primeiro argumento e `isReversed` como segundo argumento.

# --hints--

Você deve atualizar `this.elements.singleCard.innerHTML` chamando a função `renderCard`.
...
Sua chamada `renderCard` deve ter `"Click the card and reveal the fortune"` como primeiro argumento.
...
Você deve passar `isReversed` como segundo argumento para `renderCard`.
