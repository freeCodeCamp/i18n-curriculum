...
在 `div` 裡面，為 PAS 和 DRI 各添加兩個帶有 `className` 為 `form-group` 的 `stats-grid` 元素。

對於 PAS，添加一個帶有 `label` 為 `htmlFor` 且文字為 `pas` 的 `PAS`，然後添加一個 `id` 為 `pas` 且 `type` 為 `number` 的輸入框。

對於 DRI，添加一個帶有 `label` 為 `htmlFor` 且文字為 `dri` 的 `DRI`，然後添加一個 `id` 為 `dri` 且 `type` 為 `number` 的輸入框。

# --hints--

你應該在 `div` 的 `className` 的 `form-group` 裡面，為 PAS 和 DRI 各添加兩個帶有 `div` 為 `className` 的 `stats-grid` 元素。
...
你的第三個 `div` 的 `className` 的 `form-group` 元素應該有一個帶有 `label` 屬性為 `htmlFor`、`pas` 為 `className`，以及文字為 `label` 的 `PAS`。
...
你的第三個 `div` 的 `className` 的 `form-group` 元素應該有一個帶有 `input` 為 `className`、`input` 為 `id`，以及 `pas` 為 `type` 的 `number`。
...
你的第四個 `div` 的 `className` 的 `form-group` 元素應該有一個帶有 `label` 屬性為 `htmlFor`、`dri` 為 `className`，以及文字為 `label` 的 `DRI`。
...
你的第四個 `div` 的 `className` 的 `form-group` 元素應該有一個帶有 `input` 為 `className`、`input` 為 `id`，以及 `dri` 為 `type` 的 `number`。
