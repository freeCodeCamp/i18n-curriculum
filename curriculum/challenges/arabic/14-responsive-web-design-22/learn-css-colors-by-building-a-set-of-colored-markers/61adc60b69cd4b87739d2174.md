...
يجب عليك استخدام منتقي class (.) لاستهداف فئة `sleeve`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.sleeve'));
```

يجب أن تكون لقاعدة CSS الخاصة بك `.sleeve` خاصية `width` بقيمة `110px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.sleeve')?.width === '110px');
```

يجب أن تكون لقاعدة CSS الخاصة بك `.sleeve` خاصية `height` بقيمة `25px`.
