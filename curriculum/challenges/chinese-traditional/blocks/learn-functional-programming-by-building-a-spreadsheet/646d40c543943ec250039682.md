...
既然你已經有一個用來匹配乘法或除法的正規表達式，你就可以計算該表達式。

宣告一個 `str2` 變數，並將呼叫 `infixEval` 並以 `str` 和 `regex` 作為引數的結果指定給它。
...
你應該將 `regex` 作為第二個引數傳遞給你的 `infixEval` 呼叫。
