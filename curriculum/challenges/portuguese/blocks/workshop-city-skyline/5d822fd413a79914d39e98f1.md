...
Você deve criar um seletor de classe chamado `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

Você deve mover a propriedade `height` e o valor de `.bb1a` para `.bb1-window`.
...
Você deve mover a propriedade `background` e o valor de `.bb1a` para `.bb1-window`.
...
Você naõ deve mover a propriedade `background-color` de `.bb1a`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

Você não deve mover a `width` de `.bb1a`.
