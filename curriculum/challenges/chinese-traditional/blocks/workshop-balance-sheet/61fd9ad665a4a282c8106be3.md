...
在你的 `thead` 中，創建一個 `tr` 元素。在該元素中，添加一個 `td` 和三個 `th` 元素。在每個 `th` 元素中，添加一個 `span` 元素，並將 `class` 設定為 `sr-only`，依序加入以下文字：`2019`、`2020` 和 `2021`。
...
你的每個新的 `span` 元素都應該將 `class` 屬性設為 `sr-only`。
