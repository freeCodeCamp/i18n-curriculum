...
`fightDragon` 関数を作成して、`"Fighting dragon."` をコンソールに出力してください。

# --hints--

`function` を宣言するには `fightDragon` キーワードを使う必要があります。

```js
assert.match(code, /function\s+fightDragon/);
```

`fightDragon` は関数であるべきです。

```js
assert.isFunction(fightDragon);
```

コードには `console.log("Fighting dragon.");` 行が必要です。

```js
assert.match(code, /console\.log\(\s*('|")Fighting dragon\.\1\s*\)/);
```

`console.log("Fighting dragon.");` 行は `fightDragon` 関数の中にあるべきです。
