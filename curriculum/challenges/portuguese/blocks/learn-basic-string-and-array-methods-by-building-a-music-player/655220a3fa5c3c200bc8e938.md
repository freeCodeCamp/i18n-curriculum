...
Use `classList` e o método `remove()` para remover a classe `playing` do `playButton`, já que a música será pausada neste ponto.

Para finalmente pausar a música, use o método `pause()` na variável `audio`. `pause()` é um método da Web Audio API para pausar arquivos de música.

# --hints--

Você não deve modificar a função `pauseSong` existente nem seu conteúdo.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

Você deve usar a propriedade `classList` e o método `remove()` para remover a classe `"playing"` do `playButton`.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

Você deve usar o método `pause()` na sua variável `audio`.
