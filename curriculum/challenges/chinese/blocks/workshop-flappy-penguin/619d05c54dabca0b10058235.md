...
目前，`.face` 和 `.chin` 具有相同的 `background-color`。

创建一个名为 `--penguin-face` 的自定义 CSS 属性，并将其设置为 `white`。
