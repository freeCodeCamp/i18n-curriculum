...
接著，撰寫一個 `if` 述語來檢查 `prevToggleItem` 是否不嚴格相等於 `toggleItem`。如果它們不相等，則在主控台記錄字串 `New toggleItem function`。然後，將 `prevToggleItem` 設定為 `toggleItem`。

# --hints--

你應該撰寫一個 `if` 述語來檢查 `prevToggleItem` 是否不嚴格相等於 `toggleItem`。

```js
assert.match(code, /if\s*\(\s*prevToggleItem\s*!==\s*toggleItem\s*\)/);
```

如果條件符合，你應該在 `if` 述語內將字串 `New toggleItem function` 記錄到主控台。
...
如果條件符合，你應該在 `if` 述語內將 `prevToggleItem` 設定為與 `toggleItem` 相等。
