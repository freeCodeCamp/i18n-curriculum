...
将你的 `getMode()` 函数添加到你的 `calculate` 逻辑，并更新相应的 HTML 元素。

# --hints--

你的 `calculate` 函数应该有一个 `mode` 变量，其值为 `getMode(numbers)`。

```js
assert.match(calculate.toString(), /mode\s*=\s*getMode\(\s*numbers\s*\)\s*;?/);
```

你的 `calculate` 函数应该查询 DOM 获得 `#mode` 元素，并将 `textContent` 设置为 `mode`。
