...
Usando `const` y la sintaxis de flecha para crear una función vacía llamada `shuffle`.

Esta función es responsable de mezclar las canciones en la lista de reproducción y realizar actualizaciones de gestión de estado necesarias después de la mezcla.

# --hints--

Debes usar `const` para crear una función vacía llamada `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

`shuffle` debe ser una función.

```js
assert.isFunction(shuffle)
```

Tu función `shuffle` debe usar una sintaxis de flecha y no debe tomar un parámetro.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

Tu función `shuffle` debe estar vacía.
