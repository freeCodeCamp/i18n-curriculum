...
在 `else` 區段中，將 `audio` 物件的 `currentTime` 屬性設定為儲存在 `userData.songCurrentTime` 的值。

# --hints--

當有正在播放的歌曲時，你的 `playSong` 函式（程式）應該將 `audio` 物件的 `currentTime` 屬性設定為儲存在 `userData.songCurrentTime` 的值。
