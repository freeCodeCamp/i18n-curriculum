...
在研讨会的最后部分，你将专注于创建赢家消息和重置按钮功能性。

首先创建一个名为 `winnerMsgElement` 的变量，用于保存 id 为 `winner-msg` 的元素。 

然后，创建一个名为 `optionsContainer` 的变量，用于保存类名为 `options-container` 的元素。 

最后，创建一个名为 `resetGameBtn` 的变量，用于保存 id 为 `reset-game-btn` 的元素。 

# --hints--

你应该有一个 `winnerMsgElement` 变量。

```js
assert.isNotNull(winnerMsgElement);
```

你的 `winnerMsgElement` 变量应保存 id 为 `winner-msg` 的元素。

```js
assert.equal(winnerMsgElement, document.getElementById("winner-msg"));
```

你应该有一个 `optionsContainer` 变量。

```js
assert.isNotNull(optionsContainer);
```

你的 `optionsContainer` 变量应保存具有 `options-container` 类的元素。

```js
assert.equal(optionsContainer, document.querySelector(".options-container"));
```

你应该有一个 `resetGameBtn` 变量。

```js
assert.isNotNull(resetGameBtn);
```

你的 `resetGameBtn` 变量应保存 id 为 `reset-game-btn` 的元素。
