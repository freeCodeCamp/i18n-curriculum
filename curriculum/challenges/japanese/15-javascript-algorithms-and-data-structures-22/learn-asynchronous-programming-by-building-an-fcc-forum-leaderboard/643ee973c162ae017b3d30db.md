...
`Activity` の列に表示するデータには、各トピックの `bumped_at` プロパティを使います。このデータは ISO 8601 形式のタイムスタンプになっています。 このデータを処理して、トピックに動きがあってからどのくらいの時間が経過したかを表示します。

新しい関数 `timeAgo` を作成し、仮引数 `time` を持たせてください。

`timeAgo` 関数の中では、`currentTime` と `lastPost` という 2 つの変数を作成し、それぞれ `new Date()` と `new Date(time)` を代入してください。

`lastPost` はトピックに最後の動きがあった日時、`currentTime` は現在の日時を表します。
