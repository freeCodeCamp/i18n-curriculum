...
開く `a` タグに対して、`target` 属性を `"_blank"` に設定してください。次に、`href` 属性を `${forumTopicUrl}${slug}/${id}` に設定します。

これらの変更を加えれば、freeCodeCamp フォーラムのリーダーボードプロジェクトは完成です！

# --hints--

開くアンカータグには `target` 属性が `"_blank"` に設定されている必要があります。
...
開くアンカータグには `href` 属性が `${forumTopicUrl}${slug}/${id}` に設定されている必要があります。
