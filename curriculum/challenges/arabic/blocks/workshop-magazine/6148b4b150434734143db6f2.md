...
تذكّر أن خاصية `grid-column` تحدد الأعمدة التي يبدأ وينتهي عندها العنصر. قد تواجه أحيانًا حالة لا تعرف فيها عدد الأعمدة في الشبكة، لكنك تريد أن يتوقف العنصر عند العمود الأخير. للقيام بذلك، يمكنك استخدام `-1` للعمود النهائي.

أنشئ محدد `.hero` وأعطه خاصية `grid-column` بقيمة `1 / -1`. هذا سيجعل العنصر يمتد على كامل عرض الشبكة.

# --hints--

يجب أن يكون لديك محدد `.hero`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero'));
```

يجب أن يحتوي محدد `.hero` على خاصية `grid-column` بقيمة `1 / -1`.
