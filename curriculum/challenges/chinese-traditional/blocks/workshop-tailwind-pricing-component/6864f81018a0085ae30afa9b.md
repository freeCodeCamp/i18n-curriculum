...
列表項目開頭每個 `span` 中的勾選符號是裝飾用的。為這些 `span` 元素添加 `aria-hidden`，以將它們從螢幕閱讀器中隱藏。

此外，為每個 `span` 元素指定 `text-green-700` 和 `mr-2` 類別。

# --hints--

你的兩個 `span` 元素應該有 `aria-hidden` 屬性。
...
你的兩個 `span` 元素應該有 `text-green-700` 類別。
