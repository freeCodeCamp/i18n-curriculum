...
이제 `PlayerCard` 컴포넌트를 `main`의 `className` 요소인 `main` 안에 감싸세요.

`main` 요소 안에 `div`의 `className`인 `layout`를 추가하세요. `layout` 안에는 각각 `div`과 `className`의 `form-panel`인 두 개의 별도 `preview-panel` 요소를 추가하세요.

# --hints--

`FootballPlayerCard`은 `main` 안에 `className`의 `main`인 `div.page` 요소를 렌더링해야 합니다.
...
`main.main`은 `div`의 `className`인 `layout`를 포함해야 합니다.
...
`layout`는 `div`의 `className`인 `form-panel`를 포함해야 합니다.
...
`layout`은 `div`의 `className`인 `preview-panel`를 포함해야 합니다.
