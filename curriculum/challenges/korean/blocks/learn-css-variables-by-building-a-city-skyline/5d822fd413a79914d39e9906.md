...
다음 건물은 세 개의 섹션을 가질 것입니다. `div` 안에 세 개의 `.bb4` 요소를 중첩하세요. 순서대로 `bb4a`, `bb4b`, `bb4c` 클래스를 부여하세요.

# --hints--

`div` 안에 세 개의 `.bb4` 요소를 추가하세요.

```js
assert.equal(document.querySelector("div.bb4")?.children?.length, 3);
```

첫 번째 새 `div`에 `bb4a` 클래스를 부여하세요.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

두 번째 새 `div`에 `bb4b` 클래스를 부여하세요.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

세 번째 새 `div`에 `bb4c` 클래스를 부여하세요.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

새 `div` 요소들을 `.bb4a + .bb4b + .bb4c` 순서로 배치하세요.
