...
在 `try` 區塊中，創建一個 `saved` 變數。對於該變數的值，使用 `getItem()` 方法來取得 `STORAGE_KEY`。就在該變數下方，建立一個條件為 `if` 的 `saved` 述語。暫時將 `if` 區塊留空。

# --hints--

你的 `loadPlayer` 函式應該在 `localStorage.getItem` 區塊中呼叫 `STORAGE_KEY` 並帶入 `try`。

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

`localStorage.getItem` 的結果應該儲存在名為 `saved` 的變數中。

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

`try` 區塊應該有一個條件為 `if` 的 `saved` 述語。
