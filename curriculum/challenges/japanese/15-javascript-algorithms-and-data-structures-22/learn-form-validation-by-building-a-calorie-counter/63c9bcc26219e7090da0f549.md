...
プログラミングでは、`null` は値が存在しないことを表します。 ここでは、ユーザーが無効な入力をした場合にアラートで通知した上で、`null` を返して関数が失敗したことを示すようにします。

では `if` ブロック内で、変数 `isError` を `true` に設定した後、`null` を返してください。

# --hints--

`alert` の後、`isError` を `true` に設定してください。
...
`isError` を変更した後、値 `null` を `return` してください。
