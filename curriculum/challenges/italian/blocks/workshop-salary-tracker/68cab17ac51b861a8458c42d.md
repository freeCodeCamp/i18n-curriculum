...
Finalmente, ora che hai completato il setter `salary`, puoi chiamarlo all'interno del setter `level`.

Modifica la riga `self._salary = Employee._base_salaries[new_level]` in modo che chiami il setter `salary` invece di modificare direttamente l'attributo `_salary`.

Con questo il workshop sul tracciatore di stipendio è completo.

# --hints--

Dovresti impostare `self.salary` su `Employee._base_salaries[new_level]` all'interno del setter `level`.
