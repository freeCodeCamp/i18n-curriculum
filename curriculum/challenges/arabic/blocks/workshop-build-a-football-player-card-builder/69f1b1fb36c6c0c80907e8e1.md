...
داخل عنصر `preview-panel`، أضف عنصر `p` مع `className` بقيمة `preview-label` والنص `Live Preview`، وعنصر `p` آخر مع `className` بقيمة `preview-hint` والنص `Updates as you type`، وعنصر `div` مع `className` مضبوط على التعبير النصي القالب `preview-box tier-${getPlayerTier(player.overallRating)}`.

داخل آخر عنصر `div`، انقل مكوِّن `PlayerCard` وخصائصه.

# --hints--

يجب أن يحتوي عنصر `div.preview-panel` على عنصر `p` مع `className` بقيمة `preview-label` والنص `Live Preview`.
...
يجب أن يحتوي عنصر `div.preview-panel` على عنصر `p` مع `className` بقيمة `preview-hint` والنص `Updates as you type`.
...
يجب أن يحتوي عنصر `div.preview-panel` على عنصر `div` مع `className` بقيمة `preview-box` و`className` ديناميكي للدرجة يبدأ بـ `tier-`.
...
يجب عرض مكوِّن `PlayerCard` داخل عنصر `div` الذي يحمل `className` بقيمة `preview-box`.
