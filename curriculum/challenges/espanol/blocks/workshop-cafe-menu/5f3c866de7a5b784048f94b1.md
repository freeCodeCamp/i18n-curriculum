...
Eso es más o menos lo que quieres, pero ahora sería bueno que el sabor y el precio estuvieran en la misma línea. Los elementos `p` son elementos <dfn>block-level</dfn>, por lo que ocupan todo el ancho de su elemento padre.
...
Para hacer eso, comienza añadiendo un atributo `class` con el valor `item` al primer elemento `article` bajo del encabezado `Coffee`.
