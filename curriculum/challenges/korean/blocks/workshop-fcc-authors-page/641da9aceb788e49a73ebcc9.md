...
1단계에서 `Load More Authors` 버튼을 선택하고 `loadMoreBtn`에 할당한 것을 기억하세요.

`addEventListener`를 사용해 `"click"`에 `loadMoreBtn` 이벤트 리스너를 추가하세요. 또한, 버튼을 클릭할 때마다 실행할 `fetchMoreAuthors` 함수의 참조를 전달하세요.

그 후 버튼을 클릭하면 8명의 저자가 더 보여야 합니다.
...
`addEventListener()` 변수에서 `loadMoreBtn` 메서드를 호출해야 합니다.
...
이벤트 리스너는 실행할 함수로 `fetchMoreAuthors`를 받아야 합니다.
