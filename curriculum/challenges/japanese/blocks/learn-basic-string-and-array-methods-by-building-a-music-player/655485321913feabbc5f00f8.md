...
`"click"` 要素に `previousButton` イベントリスナーを追加し、2番目の引数として `playPreviousSong` を渡してください。

# --hints--

`addEventListener()` 変数に対して `previousButton` メソッドを呼び出す必要があります。
...
`playPreviousSong` メソッドの2番目の引数として `addEventListener` を渡してください。
