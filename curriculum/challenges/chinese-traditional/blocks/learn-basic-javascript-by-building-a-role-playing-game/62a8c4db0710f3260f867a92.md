...
如果玩家沒有足夠的金幣購買生命值，該怎麼辦？你應該使用 `if` 述語來檢查玩家是否有足夠的金幣購買生命值。 

在先前的專案中，你學會了如何使用像這樣的 `if` 述語：
...
首先將你所有在 `buyHealth` 函式（程式）中的程式碼放入一個 `if` 述語中。對於 `if` 述語的條件，檢查 `gold` 是否大於或相等於 `10`。

# --hints--

你的 `buyHealth` 函式應該有一個 `if` 述語。

```js
assert.match(buyHealth.toString(), /if/);
```

你的 `if` 述語應該檢查 `gold` 是否大於或相等於 `10`。

```js
assert.match(buyHealth.toString(), /if\s*\(\s*gold\s*>=\s*10\s*\)/);
```

你所有的 `buyHealth` 程式碼都應該放在 `if` 述語內。
