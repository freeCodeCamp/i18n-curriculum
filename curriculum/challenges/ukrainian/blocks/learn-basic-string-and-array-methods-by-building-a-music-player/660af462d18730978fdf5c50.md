...
Тепер настав час застосувати те, що ви дізналися про стрілкові функції, у вашому проєкті музичного плеєра.

Почніть із видалення функцій `printGreeting`, `printMessage` і `addTwoNumbers`. Також видаліть усі інструкції console та виклики функцій.

# --hints--

У вас не повинно бути функції `printGreeting`.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

У вас не повинно бути функції `printMessage`.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

У вас не повинно бути функції `addTwoNumbers`.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

У вас не повинно бути виклику функції `printGreeting();`.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

У вас не повинно бути виклику функції `printMessage('freeCodeCamp');`.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

У вас не повинно бути інструкції console `console.log(addTwoNumbers(3,4));`.
