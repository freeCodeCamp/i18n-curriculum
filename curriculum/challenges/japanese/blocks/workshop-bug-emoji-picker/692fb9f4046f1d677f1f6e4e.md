...
`isSelect` という名前の関数を作成してください。この関数は `element` 型の単一のパラメータ `EventTarget | null` を受け取ります。この関数は、選択された要素が `<select>` HTML 要素かどうかをチェックします。

# --hints--

`isSelect` という名前の関数があるはずです。
...
`isSelect` 関数には `element` 型の `EventTarget | null` という名前のパラメータが必要です。
...
`isSelect` 関数の戻り値の型は `element is HTMLSelectElement` であるべきです。
