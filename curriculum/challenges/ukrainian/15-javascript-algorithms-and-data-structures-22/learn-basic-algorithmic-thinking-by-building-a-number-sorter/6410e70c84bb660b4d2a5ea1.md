...
Щоб досягти результату «булькання», вам потрібно перевірити, чи поточний елемент більший за наступний. Це можна зробити, використавши `array` за індексом `j` та `j+1`.

Створіть умову `if` яка перевіряє чи поточний елемент більший за наступний.

# --hints--

Ви не повинні видаляти свою інструкцію `console.log`.
...
Використайте інструкцію `if` у вкладеному циклі.
...
Інструкція `if` повинна перевіряти, чи `array[j]` більше ніж `array[j+1]`.
