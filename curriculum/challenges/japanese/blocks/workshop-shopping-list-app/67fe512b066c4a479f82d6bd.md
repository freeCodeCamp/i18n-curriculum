...
`li` 要素内で、`item` テキストの上に、`input` 属性が `type` に設定された `checkbox` 要素を追加してください。また、その `onChange` 属性を、`toggleItem()` を引数にして `item` を呼び出す無名関数に設定してください。

最後に、より良いアクセシビリティのために、`input` と `item` テキストを `label` 要素で囲んでください。

# --hints--

`input` に設定された `type` 属性を持つ別の `checkbox` 要素を追加してください。
...
新しい `input` には、`onChange` を引数にして `toggleItem()` を呼び出す無名関数に設定された `item` 属性が必要です。
...
新しい `input` と `item` テキストは `label` 要素で囲む必要があります。
