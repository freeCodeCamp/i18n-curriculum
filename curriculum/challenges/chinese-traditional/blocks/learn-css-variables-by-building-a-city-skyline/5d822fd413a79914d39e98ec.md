...
透過在 `.foreground-buildings` 元素的頂部和底部各添加兩個空的 `div` 元素，以及在 `.fb2` 和 `.fb3` 之間再添加一個，將建築物再次擠在一起。

# --hints--

你應該為你的 `.foreground-buildings` 添加兩個作為第一個子元素的 `div` 元素。
...
你應該為你的 `.foreground-buildings` 添加兩個作為最後子元素的 `div` 元素。
