...
إذا كان الشرط الذي أضفته صحيحًا، فهذا يعني أن العقدة الحالية هي أفضل خيار غير مُزار حتى الآن ويجب تحديث المتغيرات لتعكس ذلك.

داخل كتلة `if`، حدّث `min_distance` إلى `distances[node_no]` واضبط `current` على `node_no`.

# --hints--

يجب تحديث المتغير `min_distance` إلى `distances[node_no]`.
