...
`FootballPlayerCard` は現在、固定された `defaultPlayer` でカードをレンダリングしています。アプリをインタラクティブにするには、Reactの状態でプレイヤーデータを管理し、更新がレンダリングの再実行を引き起こすようにする必要があります。

`FootballPlayerCard` コンポーネント内で、現在のプレイヤーデータを保持する `player` という状態変数と、そのセッター `setPlayer` を作成してください。この状態は `defaultPlayer` で初期化し、TypeScriptがプレイヤーオブジェクトの形状を認識できるように、ジェネリックとして `PlayerData` 型を指定してください。

その後、戻り値を更新して、`player` 状態変数を `PlayerCard` にプロップとして渡してください。

# --hints--

配列の分割代入を使って、`player` 状態変数と `setPlayer` セッターを設定してください。

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

`player` と `setPlayer` は `useState` フックを使うべきです。
...
`useState` は `defaultPlayer` で初期化し、ジェネリック型として `PlayerData` を指定してください。

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

`player` を `player` プロップの値として `PlayerCard` に渡す必要があります。
