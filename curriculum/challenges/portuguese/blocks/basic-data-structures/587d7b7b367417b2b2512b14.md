...
Como arrays podem ser alterados, ou *mutados*, a qualquer momento, não há garantia sobre onde um dado específico estará em um determinado array, ou se esse elemento ainda existe. Felizmente, o JavaScript nos fornece outro método embutido, `indexOf()`, que nos permite verificar rápida e facilmente a presença de um elemento em um array. `indexOf()` recebe um elemento como parâmetro e, quando chamado, retorna a posição, ou índice, desse elemento, ou `-1` se o elemento não existir no array.
