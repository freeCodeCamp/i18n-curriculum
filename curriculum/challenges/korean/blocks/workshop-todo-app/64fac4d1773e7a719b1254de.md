...
`hidden`에서 `taskForm` 클래스 토글하는 기존 코드를 제거하고 대신 `reset` 함수를 호출하세요. 이렇게 하면 입력 필드가 지워지고 사용자가 추가된 작업을 볼 수 있도록 폼 모달도 숨겨집니다.

# --hints--

`hidden`에서 `taskForm` 클래스를 토글하는 코드를 제거해야 합니다.
