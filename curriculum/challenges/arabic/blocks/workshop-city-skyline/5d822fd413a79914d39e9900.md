...
بعد ذلك، غيّر القيمتين `#999` في `.bb2a` إلى `transparent`. هذا سيجعل الحدود اليسرى واليمنى غير مرئية.

# --hints--

يجب أن تغيّر `border-left` لاستخدام `transparent`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid transparent");
```

يجب أن تغيّر `border-right` لاستخدام `transparent`.
