...
Dovresti dare a `.fb6` un secondo `repeating-linear-gradient` nella proprietà `background`.
...
Dovresti dare al secondo `repeating-linear-gradient` un primo colore di `--building-color3` dallo `0%` al `10%`.
...
Dovresti dare al secondo `repeating-linear-gradient` un secondo colore di `--window-color3` dal `10%` al `30%`.
