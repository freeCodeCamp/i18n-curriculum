...
你需要将 `resetText` 作为子元素添加到 `resetButton` 元素中，还将 `resetButton` 作为子元素添加到 `playlistSongs` 元素中。 为此，有一个 `appendChild()` 方法可供使用。

`appendChild()` 允许你添加一个节点或元素作为另一个元素的子元素。 在下面的例子中，文本“Click me”将附加到按钮：
...
使用 `appendChild()` 将 `resetText` 附加到 `resetButton` 元素，并使用 `resetButton` 附加到 `playlistSongs` 元素。

# --hints--

你应该在 `resetButton` 上使用 `appendChild()` 方法。

```js
assert.match(code, /resetButton\.appendChild\(/)
```

你应该传入 `resetText` 作为第一个 `appendChild()` 的值。

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

你应该在 `playlistSongs` 上使用 `appendChild()` 方法。

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

你应该传入 `resetButton` 作为第二个 `appendChild()` 的值。
