...
أخيرًا، عدل المعلمة اللونية الثالثة في دالة `linear-gradient`، التي هي حاليًا أزرق نقي.

حدّث دالة `rgb` بحيث تكون قيمة الأحمر `162`، وقيمة الأخضر `27`، وقيمة الأزرق `27`.

# --hints--

يجب أن تحتوي قاعدة CSS `.red` على خاصية `background` مضبوطة على `linear-gradient(180deg, rgb(122, 74, 14) 0%, rgb(245, 62, 113) 50%, rgb(162, 27, 27) 100%)`.
