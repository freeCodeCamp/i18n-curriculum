...
الآن، استدعِ الدالة `chooseShape()` مع `val.value` لعرض حقول الإدخال الصحيحة، ثم استدعِ `updateResult()` لإعادة حساب وعرض المساحة.

# --hints--

يجب استدعاء الدالة `chooseShape` مع `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

يجب استدعاء الدالة `updateResult()`.
