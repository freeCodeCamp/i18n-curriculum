...
Під елементом `.eye.right` додайте два елементи `div`, які мають `class` зі значенням `blush`. Також надайте першому елементу `.blush` властивість `class` зі значенням `left`, а другому елементу `.blush` — властивість `class` зі значенням `right`.
...
Першому новому `div` надайте `class` зі значенням `blush`.

```js
assert.exists(document.querySelector('.penguin-head > div.blush'));
```

Другому новому `div` надайте `class` зі значенням `blush`.
