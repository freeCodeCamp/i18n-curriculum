...
Come prima, dovresti validare il tuo input. In particolare, se `size` è minore di `1`, dovresti registrare `"Chunk size must be >= 1"` nella console e poi chiamare `return` per uscire dalla funzione.

# --hints--

Se l'input `size` non è valido, dovresti registrare il seguente messaggio di errore nella console, `"Chunk size must be >= 1"`.
...
Dopo aver registrato il messaggio di errore, dovresti chiamare immediatamente `return` per uscire anticipatamente dalla funzione.
...
La tua funzione `chunkCrew` dovrebbe superare la validazione senza errori quando `size` è maggiore o uguale a 1.
