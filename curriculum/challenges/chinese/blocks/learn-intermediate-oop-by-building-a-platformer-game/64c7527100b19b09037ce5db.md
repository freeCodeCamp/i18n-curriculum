...
在 `draw` 方法内部，将 `"#acd157"` 分配给 `ctx.fillStyle`。

下面，调用 `ctx.fillRect` 方法并传入 `x` 和 `y` 坐标以及 `width` 和 `height` 属性。 请记住在每个属性前包含 `this`。

# --hints--

你应该将 `"#acd157"` 分配给 `ctx.fillStyle`。

```js
assert.match(code, /ctx\.fillStyle\s*=\s*('|")#acd157\1\s*;?/);
```

你应该调用 `ctx.fillRect` 方法并传递正确的参数。
