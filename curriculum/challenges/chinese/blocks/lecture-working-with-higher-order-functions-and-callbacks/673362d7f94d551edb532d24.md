...
这是因为 `sort` 方法会将元素转换为字串，然后比较它们的 UTF-16 码元序列。
...
理解 `sort` 方法如何处理数组中的 `undefined` 值和空槽也很重要。

当 `sort` 方法遇到 `undefined` 值时，它不会将该值传递给比较函数。相反，`undefined` 值会自动移动到数组末尾。

下面是一个示例，展示了 `sort` 如何处理包含 `undefined` 值的数组：
...
注意 `undefined` 被放置在排序后数组的末尾，位于所有其他元素之后。

同样，当 `sort` 方法遇到稀疏数组中的空槽时，它会保留该空槽并将其移动到数组末尾。如果数组中同时存在 `undefined` 值和空槽，`undefined` 值会先出现，空槽则排在最末尾。
...
在这个示例中，字串 `"apple"` 和 `"banana"` 首先按字母顺序排序。然后 `undefined` 值被放置在它们之后。最后，空槽被保留在数组的最末尾。
