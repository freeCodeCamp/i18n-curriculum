...
无障碍树是辅助技术（如屏幕阅读器）用来解释和与网页内容交互的结构。
...
- **`display: none;`**：使用 `display: none;` 意味着屏幕阅读器和其他辅助技术无法访问该内容，因为它不包含在无障碍树中。因此，只有当你想完全从视觉呈现和无障碍中移除内容时，才应使用此方法。
- **`visibility: hidden;`**：该属性和值会在视觉上隐藏内容，但保持其在文档流中，这意味着它仍然占据页面空间。这些元素也不会被屏幕阅读器读取，因为它们已从无障碍树中移除。
...
在上述示例中，如果用户未在设备上设置减少动画的偏好，则启用平滑滚动。
...
使用占位符文本对无障碍性不利。用户经常将占位符文本误认为是实际的输入值，认为输入框中已经有值。
