...
Nei moduli precedenti, hai imparato come iterare su un `NodeList` in questo modo:
...
Usa il metodo `forEach` per iterare su `tabs`. Per la funzione di callback, usa una funzione freccia che prende un singolo parametro, `tab`. 

# --hints--

Dovresti avere un `tabs.forEach(tab => {})` nel tuo codice.
