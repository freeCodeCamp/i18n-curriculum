...
2<sup>15</sup> = 32768 且其數字和為 3 + 2 + 7 + 6 + 8 = 26。

2<sup><code>exponent</code></sup> 這個數字的各位數字和是多少？

# --hints--

`powerDigitSum(15)` 應該傳回一個數字。

```js
assert.isNumber(powerDigitSum(15));
```

`powerDigitSum(15)` 應該傳回 26。

```js
assert.strictEqual(powerDigitSum(15), 26);
```

`powerDigitSum(128)` 應該傳回 166。

```js
assert.strictEqual(powerDigitSum(128), 166);
```

`powerDigitSum(1000)` 應該傳回 1366。
