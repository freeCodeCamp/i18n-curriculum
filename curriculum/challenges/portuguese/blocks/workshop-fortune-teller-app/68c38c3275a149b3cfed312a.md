...
Nesta oficina, você vai continuar praticando o uso do TypeScript construindo um app de previsão do futuro.

Comece declarando uma interface chamada `Card`. Dentro da interface `Card`, adicione as seguintes propriedades: `name`, `name_short` e `value`. As propriedades `name` e `name_short` devem ter os tipos definidos como `string` e a `value` deve ter um tipo união definido como `string | number`.

# --hints--

Você deve declarar uma interface chamada `Card`.
...
Sua interface `Card` deve ter uma propriedade `name` do tipo `string`.
...
Sua interface `Card` deve ter uma propriedade `name_short` do tipo `string`.
...
Sua interface `Card` deve ter uma propriedade `value` com um tipo união de `string` e `number`.
