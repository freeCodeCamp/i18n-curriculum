...
獲取你的 `#budget` 元素並將其分配給 `budgetNumberInput`，以及你的 `#entry-dropdown` 元素並將其分配給 `entryDropdown`。

# --hints--

你應該聲明一個名爲 `budgetNumberInput` 的變量。

```js
assert.isDefined(budgetNumberInput);
```

你應該使用 `document.getElementById()` 來獲取 `#budget` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")budget\1\s*\)/g);
```

你應該將 `#budget` 元素存儲在名爲 `budgetNumberInput` 的變量中。

```js
assert.equal(budgetNumberInput, document.getElementById('budget'));
```

你應該聲明一個名爲 `entryDropdown` 的變量。

```js
assert.isDefined(entryDropdown);
```

你應該使用 `document.getElementById()` 來獲取 `#entry-dropdown` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")entry-dropdown\1\s*\)/g);
```

你應該將 `#entry-dropdown` 元素存儲在名爲 `entryDropdown` 的變量中。
