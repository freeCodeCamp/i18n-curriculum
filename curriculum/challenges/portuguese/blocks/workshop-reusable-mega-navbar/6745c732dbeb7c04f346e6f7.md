...
Crie um elemento `ul` dentro do elemento `nav`.Dentro desse elemento `ul`, crie três itens de lista.

# --hints--

Você deve criar um elemento `ul` dentro do seu elemento `nav`.

```js
assert.exists(document.querySelector('nav ul'));
```

Você deve criar três elementos `li` dentro do seu elemento `ul`.
