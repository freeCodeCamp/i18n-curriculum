...
向 `pauseButton` 元素添加 `"click"` 事件監聽器，然後傳入 `pauseSong` 作爲事件監聽器的第二個參數。 這是事件監聽器將運行的函數。

通過首先單擊播放按鈕然後單擊暫停按鈕來測試你的應用。 你應該看到一切都按預期進行。

# --hints--

你應該在 `pauseButton` 變量上調用 `addEventListener()` 方法。
...
你應該將 `pauseSong` 作爲 `addEventListener` 方法的第二個參數傳入。
