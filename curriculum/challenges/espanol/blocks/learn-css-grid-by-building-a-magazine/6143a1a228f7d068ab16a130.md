...
Debajo de tu elemento `blockquote`, añade otro elemento `p` con el siguiente texto:
...
Debes añadir un cuarto elemento `p` a tu elemento `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 5);
```

Tu nuevo elemento `p` debe venir después de tu elemento `blockquote`.
