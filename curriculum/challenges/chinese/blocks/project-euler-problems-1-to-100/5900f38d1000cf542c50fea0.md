...
分数<sup><sub>九十八分之四十九</sub></sup>是好奇馏分，如在试图简化它可能会错误地认为<sup><sub>九十八分之四十九</sub></sup> = <sup><sub>4/8，</sub></sup>这是正确的，则通过取消787-9获得一个没有经验的数学家。

我们应考虑馏分喜欢， <sup><sub>五十○分之三十○=</sub></sup> <sup><sub>3/5，</sub></sup>是微不足道的例子。
...
`digitCancellingFractions()` 应该返回一个数字。

```js
assert(typeof digitCancellingFractions() === 'number');
```

`digitCancellingFractions()` 应该返回 100。
