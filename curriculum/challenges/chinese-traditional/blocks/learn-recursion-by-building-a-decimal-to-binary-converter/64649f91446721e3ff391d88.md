...
最後，使用字串插值將段落元素的文字設定為 `decimalToBinary(${currVal})`，其中 `currVal` 是目前物件的 `inputVal` 屬性。完成後，透過在數字輸入欄位輸入數字 `5` 並點擊 `Convert` 按鈕來測試你的程式碼。

# --hints--

你應該使用字串插值來設定段落元素的文字為 `decimalToBinary(${obj.inputVal})`。
