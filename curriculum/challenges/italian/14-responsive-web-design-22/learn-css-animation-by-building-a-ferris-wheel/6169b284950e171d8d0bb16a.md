...
Dovresti creare un nuovo selettore `75%` nella regola `@keyframes cabins`.
...
Il selettore `75%` dovrebbe essere compreso tra i selettori `50%` e `100%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText === '75%');
```

Il selettore `75%` dovrebbe avere una proprietà `background-color` impostata su `yellow`.
