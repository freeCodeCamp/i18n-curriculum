...
Questa proprietà accetta come primo valore `row` o `column` con un secondo valore opzionale `dense`. `grid-auto-flow` usa un algoritmo di posizionamento automatico per regolare il layout della griglia. Impostarlo su `column` dirà all'algoritmo di creare nuove colonne per il contenuto, se necessario. Il valore `dense` consente all'algoritmo di tornare indietro e riempire i buchi nella griglia con elementi più piccoli, il che può far apparire gli elementi non in ordine.

Per il selettore `.social-icons`, imposta la proprietà `grid-auto-flow` su `column`.

# --hints--

Il selettore `.social-icons` dovrebbe avere una proprietà `grid-auto-flow` impostata su `column`.
