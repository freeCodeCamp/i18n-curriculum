...
要調用命名箭頭函數表達式，可以通過其名稱引用該函數。

```js
exampleArrowFunction();
```

在 `printGreeting` 函數定義下方，調用該函數。 打開控制檯查看輸出。

# --hints--

你應該調用你的 `printGreeting` 函數。

```js
assert.match(code, /printGreeting\(\);?/);
```

你的 `printGreeting` 函數調用應位於 `printGreeting` 函數定義下方。

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

你的 `printGreeting` 函數調用應該在控制檯輸出 `Hello there!`。
