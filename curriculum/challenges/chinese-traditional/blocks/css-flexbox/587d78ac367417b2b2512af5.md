...
title: 將 `flex-direction` 屬性應用於在 Tweet 嵌入中創建行。
challengeType: 0
...
推文嵌入的 `header` 和 `footer` 先前使用了 `flex-direction` 屬性並設定為列值。同樣地，`.profile-name` 元素內的條款以欄方式堆疊會運作良好。

# --instructions--

為你的標頭的 `.profile-name` 元素添加 CSS 屬性 `flex-direction`，並將值設為 `column`。
...
`.profile-name` 元素應該設定 `flex-direction` 屬性為 `column`。
