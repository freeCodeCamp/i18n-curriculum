...
Erstellen Sie im `try`-Block eine `saved`-Variable. Verwenden Sie für den Wert die `getItem()`-Methode, um `STORAGE_KEY` abzurufen. Direkt unter dieser Variable erstellen Sie eine `if`-Anweisung mit der Bedingung `saved`. Lassen Sie den `if`-Block vorerst leer.

# --hints--

Ihre `loadPlayer`-Funktion sollte `localStorage.getItem` mit `STORAGE_KEY` innerhalb des `try`-Blocks aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Das Ergebnis von `localStorage.getItem` sollte in einer Variable namens `saved` gespeichert werden.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Der `try`-Block sollte eine `if`-Anweisung mit der Bedingung `saved` enthalten.
