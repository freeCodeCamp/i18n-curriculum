...
1. Você deve definir uma constante de objeto chamada `projectStatus` com as três chaves: `PENDING`, `SUCCESS` e `FAILURE`. Cada status deve ser atribuído a um objeto com uma chave `description` com os valores `Pending Execution`, `Executed Successfully` e `Execution Failed`, respectivamente.
...
Chamar `updateProjectStatus(projectStatus.SUCCESS)` em `new ProjectIdea("Fitness Tracker App", "An app that tracks user workouts, diet, and sleep patterns.")` deve atualizar o status para `{ description: 'Executed Successfully' }`.
...
Chamar `updateProjectStatus(projectStatus.FAILURE)` em `new ProjectIdea("Breakfast Chef Robot", "A robot that can follow a given list of instructions and prepare breakfast for the user and let them know through their phone.")` deve atualizar o status para `{ description: 'Execution Failed' }`.
...
Chamar `updateProjectStatus(projectStatus.SUCCESS)` em `new ProjectIdea("Online Used Video Games Store", "An online platform where users can buy second hand physical copies of video games from other users.")` deve atualizar o status para `{ description: 'Executed Successfully' }`.
...
Quando você fixa `new ProjectIdea("Smart Home System", "An integrated system to control lighting, temperature, and security devices remotely.")` em `new ProjectIdeaBoard("Tech Projects Board")`, `techProjects.formatToString()` deve retornar `Tech Projects Board has 1 idea(s)\nSmart Home System (Pending Execution) - An integrated system to control lighting, temperature, and security devices remotely.\n`.
