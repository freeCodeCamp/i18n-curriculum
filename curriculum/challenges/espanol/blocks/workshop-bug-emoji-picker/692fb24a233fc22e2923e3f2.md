...
Después del constructor, crea un método abstracto `render` que tenga un tipo de retorno `void`. Lo usarás en las subclases para actualizar el emoji que se muestra.

# --hints--

Tu clase `Bug` debe tener un método abstracto `render`.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

Tu método `render` debe tener un tipo de retorno `void`.
