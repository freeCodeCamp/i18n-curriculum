...
Debes tener cinco elementos `.flavor`.

```js
assert.lengthOf(document.querySelectorAll('.flavor'), 5);
```

Debes tener cinco elementos `.price`.

```js
assert.lengthOf(document.querySelectorAll('.price'), 5);
```

Tus elementos `.flavor` deben ser los mismos elementos `p` con los textos `French Vanilla`, `Caramel Macchiato`, `Pumpkin Spice`, `Hazelnut` y `Mocha`.
...
Tus elementos `.price` deben ser los mismos elementos `p` con los textos `3.00`, `3.75`, `3.50`, `4.00` y `4.50`.
