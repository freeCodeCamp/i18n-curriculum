...
`move()` 関数から `make_allowed_move()` 関数へコードを移動する時が来ました。最初の `if` 文の中にネストされたコード（最初の `print()` 呼び出しを除く）を新しい関数に移してください。インデントに特に注意してください。
`pass` キーワードを削除するのを忘れないでください。

# --hints--

最初の `if` 文の中にネストされたコード（最初の `print()` 呼び出しを除く）を新しい関数に移す必要があります。
