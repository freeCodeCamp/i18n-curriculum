...
다음으로, `const` 변수 하나를 `forumCategoryUrl`라고 선언하고 문자열 `"https://forum.freecodecamp.org/c/"`를 할당하세요.

그 아래에 또 다른 `const` 변수 하나를 `avatarUrl`라고 선언하고 문자열 `"https://sea1.discourse-cdn.com/freecodecamp"`를 할당하세요.

# --hints--

`forumCategoryUrl`를 사용해 `const` 변수를 선언하세요.

```js
assert.match(code, /const\s+forumCategoryUrl\s*=/);
```

`"https://forum.freecodecamp.org/c/"` 변수에 `forumCategoryUrl`를 할당하세요.

```js
assert.match(code, /const\s+forumCategoryUrl\s*=\s*('|")https:\/\/forum\.freecodecamp\.org\/c\/\1\s*/);
```

`avatarUrl`를 사용해 `const` 변수를 선언하세요.

```js
assert.match(code, /const\s+avatarUrl\s*=/);
```

`"https://sea1.discourse-cdn.com/freecodecamp"` 변수에 `avatarUrl`를 할당하세요.
