...
title: Tweet埋め込みでflex-directionプロパティを使って行を作成する
challengeType: 0
...
Tweet埋め込みの例にある`header`と`footer`には、`flex-direction`プロパティを使って行として配置できる子要素があります。これはCSSに子要素を水平方向に揃えるよう指示します。

# --instructions--

`flex-direction`のCSSプロパティを`header`と`footer`の両方に追加し、値を`row`に設定してください。
...
`header`には`flex-direction`プロパティを`row`に設定する必要があります。

```js
assert.match(code, /header\s*?{[^}]*?flex-direction:\s*?row;/g);
```

`footer`には`flex-direction`プロパティを`row`に設定する必要があります。
