...
Nel tuo file JavaScript, seleziona l'elemento `.story-container` e archivialo in una variabile chiamata `storyContainer`.

# --hints--

Dovresti avere una variabile `storyContainer`.

```js
assert.isDefined(storyContainer);
```

Dovresti selezionare l'elemento `.story-container` usando il metodo `querySelector` e archiviarlo nella variabile `storyContainer`.
