...
Зараз усі три факти видимі одночасно. Але має бути видимий лише один факт одночасно.

Щоб досягти цього, потрібно встановити атрибут `hidden` на другому та третьому елементах `tabpanel`.

# --hints--

Ваш другий елемент `tabpanel` має мати атрибут `hidden`.

```js
assert.isTrue(document.querySelectorAll('div[role="tabpanel"]')[1].hasAttribute("hidden"));
```

Ваш третій елемент `tabpanel` має мати атрибут `hidden`.
