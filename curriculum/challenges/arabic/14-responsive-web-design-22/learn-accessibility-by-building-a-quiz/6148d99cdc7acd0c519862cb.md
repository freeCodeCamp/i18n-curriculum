...
استهدف جميع عناصر `label` داخل عناصر `.info`، وعيّن الـ `width` إلى `10%`, واجعله لا ياخذ أقل من `55px`.

# --hints--

يجب عليك استخدام منتقي `.info label` أو `.info > label`.
...
يجب عليك إعطاء `label` الـ `width` بقيمة `10%`.
...
يجب عليك إعطاء `label` الـ `min-width` بقيمة `55px`.
