...
Tu selector `tr[class="total"] th` debe tener una propiedad `text-align` establecida en `left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"] th')?.getPropertyValue('text-align') === 'left');
```

Tu selector `tr[class="total"] th` debe tener una propiedad `padding` establecida en `0.5rem 0 0.25rem 0.5rem`.
