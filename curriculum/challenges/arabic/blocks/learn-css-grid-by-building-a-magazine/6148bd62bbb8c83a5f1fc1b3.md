...
أنشئ محدِّد `.hero-title` وأعطه خاصية `text-align` بقيمة `center`، وخاصية `color` بقيمة `orangered`، وخاصية `font-size` بقيمة `8rem`.

# --hints--

يجب أن يكون لديك محدِّد `.hero-title`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

يجب أن يحتوي محدِّد `.hero-title` على خاصية `text-align` بقيمة `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign === 'center');
```

يجب أن يحتوي محدِّد `.hero-title` على خاصية `color` بقيمة `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color === 'orangered');
```

يجب أن يحتوي محدِّد `.hero-title` على خاصية `font-size` بقيمة `8rem`.
