...
Centra orizzontalmente tutto il testo all'interno dell'elemento `address` e aggiungi un po' di spaziatura interna.

# --hints--

Dovresti usare il selettore dell'elemento `address`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

Dovresti impostare a `address` un `text-align` di `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Dovresti impostare a `address` un `padding-top` di almeno `1px`.
...
Dovresti impostare a `address` un `padding-right` di almeno `1px`.
...
Dovresti impostare a `address` un `padding-bottom` di almeno `1px`.
...
Dovresti impostare a `address` un `padding-left` di almeno `1px`.
