...
이제 `start` 다음에 `if` 문장에 `userData.currentSong === null` 매개변수를 두 번째 조건으로 추가하려면 **OR** 연산자를 사용하세요. 


# --hints--

`start` 다음에 `if` 문장에 `userData.currentSong === null` 매개변수를 두 번째 조건으로 추가하려면 **OR** 연산자를 사용하세요. 
