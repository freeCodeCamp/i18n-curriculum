...
接著進入下一個建築物！是前景中綠色的那個。為它使用你的建築顏色從 `0%` 到 `5%` 的 `repeating-linear-gradient`，以及從 `5%` 到 `10%` 的 `transparent`。

# --hints--

你應該給 `.fb5` 添加 `background` 屬性。
...
你應該將 `repeating-linear-gradient` 的第一個顏色設為從 `0%` 到 `5%` 的 `--building-color2`。
...
你應該給 `repeating-linear-gradient` 在 `5%` 到 `10%` 之間添加第二個 `transparent` 顏色。
