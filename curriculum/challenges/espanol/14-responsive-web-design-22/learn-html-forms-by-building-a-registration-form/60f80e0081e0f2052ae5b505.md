...
Specifying the `type` attribute of an `input` element is important for the browser to know what kind of data it should expect. Si el `type` no está especificado, el navegador utilizará por defecto `text`.
