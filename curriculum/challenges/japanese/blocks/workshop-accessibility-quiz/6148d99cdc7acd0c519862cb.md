...
`label` 要素内のすべての `.info` 要素を対象にし、それらの `width` を `10%` に設定し、`55px` より小さくならないようにしてください。

# --hints--

`.info label` または `.info > label` セレクターのいずれかを使うべきです。
...
`label` 要素に `width` を `10%` に設定してください。
...
`label` 要素に `min-width` を `55px` に設定してください。
