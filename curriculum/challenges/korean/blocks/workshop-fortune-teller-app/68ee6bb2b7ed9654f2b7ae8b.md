...
글로벌 `document` 객체에 `showFortune` 함수를 호출하는 또 다른 이벤트 리스너를 추가하세요. 콜백 안에는 `e` 타입의 매개변수 `Event`를 포함하고, `e` 메서드에 `showFortune`를 인수로 전달하세요.

`showFortune`는 카드 클릭 이벤트를 처리하며, 클릭한 카드 ID를 DOM에서 읽고 일치하는 카드 운세 세부 정보를 표시하는 역할을 합니다. 이 함수는 다음 단계에서 만들게 됩니다.

# --hints--

글로벌 `click`에 `document` 이벤트 리스너를 할당해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

이벤트 리스너의 콜백 매개변수 이름은 `e`이어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

콜백 매개변수의 타입은 `Event`이어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

콜백 안에서 `showFortune` 함수를 호출해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

`showFortune` 함수 안에서 인수 `e`를 전달해야 합니다.
