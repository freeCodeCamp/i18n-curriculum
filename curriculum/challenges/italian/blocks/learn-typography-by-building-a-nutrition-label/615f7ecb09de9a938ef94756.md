...
Sotto il tuo ultimo elemento `p`, aggiungi un altro elemento `p` con il testo `Total Carbohydrate 37g 13%`. Come prima, usa elementi `span` per rendere il testo `Total Carbohydrate` e `13%` in grassetto. Poi, avvolgi di nuovo il nutriente e la quantità in un `span` per l'allineamento.
...
Il tuo nuovo elemento `p` dovrebbe avere tre elementi `span`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelectorAll('span')?.length === 3);
```

Un elemento `span` dovrebbe avvolgere il testo `Total Carbohydrate 37g`.
...
Un elemento `span` dovrebbe avvolgere il testo `Total Carbohydrate`.
...
L'elemento `span` intorno a `Total Carbohydrate` dovrebbe avere l'attributo `class` impostato su `bold`.
...
Un elemento `span` dovrebbe avvolgere il testo `13%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.lastElementChild?.innerText === '13%');
```

L'elemento `span` intorno a `13%` dovrebbe avere l'attributo `class` impostato su `bold`.
