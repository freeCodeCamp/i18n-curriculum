...
訪問 `userData?.currentSong?.title` 和 `userData?.currentSong?.artist` 屬性並將它們分別分配給 `currentTitle` 和 `currentArtist` 變量。

# --hints--

你應該從 `userData` 對象訪問 `currentSong` 的 `title`。 不要忘記使用可選鏈接。

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

你應該將 `currentSong` 的 `title` 分配給 `currentTitle` 常量。

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

你應該從 `userData` 對象訪問 `currentSong` 的 `artist`。 不要忘記使用可選鏈接。

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

你應該將 `currentSong` 的 `artist` 分配給 `currentArtist` 常量。
