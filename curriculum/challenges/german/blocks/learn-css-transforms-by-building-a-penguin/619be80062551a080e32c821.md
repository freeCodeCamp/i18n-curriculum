...
Runden Sie den Kamm ab, indem Sie den unteren Ecken des Pseudoelements einen Radius von `100%` geben und die oberen Ecken bei `0%` belassen.

# --hints--

Sie sollten die Eigenschaft `border-radius` verwenden, um den Kamm abzurunden.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

Sie sollten `.penguin-body::before` eine `border-radius` von `0% 0% 100% 100%` geben.
