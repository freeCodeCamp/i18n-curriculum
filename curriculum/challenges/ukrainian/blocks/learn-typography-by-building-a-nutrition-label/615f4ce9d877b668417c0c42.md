...
Обгорніть увесь вміст всередині елемента `.label` у новий елемент `header`.

# --hints--

Ви повинні створити новий елемент `header`.

```js
assert(document.querySelector('header'));
```

Ваш елемент `header` має бути всередині вашого елемента `.label`.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

Ваші елементи `h1`, `div` і `p` мають бути всередині вашого нового елемента `header`.
