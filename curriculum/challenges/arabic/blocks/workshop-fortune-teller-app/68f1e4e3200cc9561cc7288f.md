...
بالنسبة لشرط جملة `if`، تحقق مما إذا كان `target` ليس مثيلاً لـ `HTMLElement`. ثم أضف داخلها `return`.

# --hints--

يجب أن يحتوي شرط جملة `if` على التعبير `!(target instanceof HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

يجب أن تحتوي جملة `if` على `return` بداخلها.
