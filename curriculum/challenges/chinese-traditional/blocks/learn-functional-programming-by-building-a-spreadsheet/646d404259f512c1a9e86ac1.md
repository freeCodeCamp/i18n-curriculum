...
# --description--
在你的 `highPrecedence` 函式中，使用 `const` 宣告一個變數並指定一個正規表達式，用來檢查傳入 `str` 參數的字串是否符合一個數字後接 `*` 或 `/` 運算子再接另一個數字的樣式。

你的函式應該傳回一個布林值。請記得你可以使用 `test()` 方法來達成這個目的。

# --hints--

你應該在你的 `highPrecedence` 函式中宣告一個變數來表示你的正規表達式。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*(?:const|let|var)\s+\w+/);
```

你應該使用 `const` 來宣告你的 regex 變數。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+\w+/);
```

你的 regex 變數應該包含一個正規表達式。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+\w+\s*=\s*\//);
```

你的 `highPrecedence` 函式（程式）應該傳回一個布林值。

```js
assert.isBoolean(highPrecedence("12*2"));
```

你的 `highPrecedence` 函式（程式）應該正確檢查字串是否符合數字後接 `*` 或 `/` 運算子再接另一個數字的樣式。
