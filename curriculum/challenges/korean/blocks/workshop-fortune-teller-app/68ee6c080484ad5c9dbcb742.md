...
`singleCardSelected`라는 메서드를 만드세요. 그 안에서 `hideElements` 함수를 호출하고 다음 프로프들을 전달하세요: `singleCardBtn`, `multipleCardsBtn`, `multipleCard`, `text`, 그리고 `headerTitle`.

이 프로프들 모두에 `this.elements`를 사용해 접근해야 한다는 점을 기억하세요.

# --hints--

`singleCardSelected`라는 이름의 메서드를 정의해야 합니다.
...
`singleCardSelected` 안에서 `hideElements` 함수를 호출해야 합니다.
...
`singleCardBtn`을 `hideElements` 함수에 전달해야 합니다.
...
`multipleCardsBtn`을 `hideElements` 함수에 전달해야 합니다.
...
`multipleCard`을 `hideElements` 함수에 전달해야 합니다.
...
`text`을 `hideElements` 함수에 전달해야 합니다.
...
`headerTitle`을 `hideElements` 함수에 전달해야 합니다.
