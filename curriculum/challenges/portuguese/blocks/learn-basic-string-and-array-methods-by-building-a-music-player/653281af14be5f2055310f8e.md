...
Seu reprodutor de música deve acompanhar as músicas, a música atual que está tocando e o tempo da música atual.Para fazer isso, você precisará criar um objeto para armazenar essas informações.

Comece usando a palavra-chave `let` para declarar uma nova variável chamada `userData` e atribuir a ela um objeto vazio.

# --hints--

Você deve usar a palavra-chave `let` para criar a variável `userData`.

```js
assert.match(code, /let\s+userData\s*=/)
```

Você deve atribuir um objeto vazio à sua variável `userData`.
