...
Al cambiar el margen inferior (`margin-bottom`) a `5px` se ve genial. Sin embargo, ahora el espacio entre la opción del menú `Cinnamon Roll` y el segundo elemento `hr` no coincide con el espacio entre el elemento `hr` superior y el título `Coffee`.
