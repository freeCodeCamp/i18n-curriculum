...
Fügen Sie `value` und `onChange` Attribute zu beiden Eingaben DEF und PHY hinzu.

Für DEF setzen Sie `value` auf `player.def` und `onChange` auf eine Pfeilfunktion, die `setPlayer` mit einer Spread-Notation von `player` aufruft und `def` auf `Number(e.target.value)` setzt.

Für PHY setzen Sie `value` auf `player.phy` und `onChange` auf eine Pfeilfunktion, die `setPlayer` mit einer Spread-Notation von `player` aufruft und `phy` auf `Number(e.target.value)` setzt.

Mit diesem Schritt sind nun alle Eingaben mit dem State verbunden, und die Live-Vorschau wird für jedes Feld aktualisiert!

# --hints--

Ihr DEF `input` sollte einen `value` haben, der dem `def` Wert des Spielers entspricht.
...
Ihr DEF `input` sollte einen `onChange` Handler haben, der `setPlayer` mit der Spread-Notation von `player` aufruft und `def` auf `Number(e.target.value)` setzt.
...
Ihr PHY `input` sollte einen `value` haben, der dem `phy` Wert des Spielers entspricht.
...
Ihr PHY `input` sollte einen `onChange` Handler haben, der `setPlayer` mit der Spread-Notation von `player` aufruft und `phy` auf `Number(e.target.value)` setzt.
