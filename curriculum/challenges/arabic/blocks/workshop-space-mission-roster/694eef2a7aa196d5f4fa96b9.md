...
استدعِ الدالة `getEVAReadyCrew()` مع قائمة `updatedSquad` وخزن النتيجة في متغير جديد اسمه `EVAReadySquad`. ثم استخدم حلقة `for` لتسجيل `name` لكل رائد فضاء في مصفوفة `EVAReadySquad`.

# --hints--

يجب استدعاء `getEVAReadyCrew()` مع `updatedSquad` وتخزين النتيجة في متغير جديد اسمه `EVAReadySquad`.
...
يجب استخدام حلقة `for` للتكرار خلال مصفوفة `EVAReadySquad` وتسجيل `name` لكل رائد فضاء.
