...
No seu objeto `userData`, crie uma propriedade `songs` e defina seu valor para o array `allSongs`.

Então, para gerenciar as informações da música atual e acompanhar seu tempo de reprodução, crie as propriedades `currentSong` e `songCurrentTime`. Defina os valores para `null` e `0`, respectivamente.

# --hints--

Seu objeto `userData` deve ter uma chave `songs` definida para o array `allSongs`.

```js
assert.propertyVal(userData, "songs", allSongs);
```

Seu objeto `userData` deve ter uma chave `currentSong` definida como `null`.

```js
assert.propertyVal(userData, "currentSong", null);
```

Seu objeto `userData` deve ter uma chave `songCurrentTime` definida como `0`.
