...
Agora que a função `displayStory` está funcionando corretamente, é hora de adicionar a funcionalidade aos botões. 

Antes disso, remova a chamada da função `displayStory("scary")`.

# --hints--

Você deve remover a chamada da função `displayStory("scary")`.
