...
現在，為你的內容添加一個帶有 `img-loader` 類別的 `div`。接著，創建一個 `img` 元素，並使用 ``${img ? `${CDN_URL}/${img}` : LOCAL_DEFAULT_IMG}`` 來為圖片設定條件式的 URL。

# --hints--

你的 `div` 元素應該在 `figure` 元素內。

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

你的 `div` 應該有一個類別 `img-loader`。

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

你應該在 `img` 元素內有一個 `figure` 元素。

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

`img` 元素應該使用 `CDN_URL` 和 `img` 作為其 `src`，當 `img` 有提供時，若沒有則回退使用 `LOCAL_DEFAULT_IMG`。
...
你的 `img` 元素應該有類別 `card-img` 和 `hidden`。
