...
Usa un bucle `for` para registrar el `name` de cada astronauta en el arreglo `updatedCrew`. Después del bucle, devuelve el arreglo `updatedCrew` para completar tu función `swapCrewMembers`.

# --hints--

Debes usar un ciclo `for` para iterar a través del arreglo `updatedCrew` y registrar el `name` de cada astronauta. 
...
Debes devolver el arreglo `updatedCrew` después de registrar los nombres de sus astronautas.
