...
Per concludere, imposta un testo di `Calendar` per il primo tag àncora, `Chat` per il secondo e `Email` per il terzo.

Con questo, il tuo mega componente barra di navigazione è completo!

# --hints--

Dovresti impostare un testo di `Calendar` per il primo elemento àncora.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Dovresti impostare un testo di `Chat` per il secondo elemento àncora.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Dovresti impostare un testo di `Email` per il terzo elemento àncora.
