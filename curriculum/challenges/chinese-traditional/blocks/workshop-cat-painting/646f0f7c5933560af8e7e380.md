...
在 `.cat-whiskers-left` 元素內，創建三個具有類別 `cat-whisker-left-top`、`cat-whisker-left-middle` 和 `cat-whisker-left-bottom` 的 `div` 元素。

# --hints--

你不應該更改具有類別 `cat-whiskers-left` 的現有 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers-left'), 1);
```

你應該在你的 `.cat-whiskers-left` 元素內創建三個 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers-left div'), 3);
```

`.cat-whiskers-left` 元素內的第一個 `div` 元素應該具有 `cat-whisker-left-top` 類別。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-left div')[0].classList.contains('cat-whisker-left-top'));
```

`.cat-whiskers-left` 元素內的第二個 `div` 元素應該有 `cat-whisker-left-middle` 類別。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-left div')[1].classList.contains('cat-whisker-left-middle'));
```

位於 `.cat-whiskers-left` 元素內的第三個 `div` 元素應該具有 `cat-whisker-left-bottom` 類別。
