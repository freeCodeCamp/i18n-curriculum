...
`const` とアロー構文を使って空の `playPreviousSong` 関数を作成してください。

# --hints--

空の関数 `const` を作成するには `playPreviousSong` を使う必要があります。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` は関数である必要があります。

```js
assert.isFunction(playPreviousSong)
```

`playPreviousSong` 関数はアロー構文を使う必要があります。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

`playPreviousSong` 関数は空である必要があります。
