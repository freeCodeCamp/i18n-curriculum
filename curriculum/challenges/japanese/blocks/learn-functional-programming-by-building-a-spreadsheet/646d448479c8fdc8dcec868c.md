...
`functionCall` 変数を宣言し、この正規表現を代入してください：`/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

この式は `sum(1, 4)` のような関数呼び出しを探します。

# --hints--

`functionCall` 変数を宣言してください。
...
`const` を使って `functionCall` 変数を宣言してください。
...
`functionCall` に指定された正規表現を代入してください。
