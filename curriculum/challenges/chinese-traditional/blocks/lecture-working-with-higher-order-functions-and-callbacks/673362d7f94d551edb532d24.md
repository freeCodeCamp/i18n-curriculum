...
這是因為 `sort` 方法會將元素轉換成字串，然後比較它們的 UTF-16 編碼單元序列。
...
了解 `sort` 方法如何處理陣列中的 `undefined` 值和空槽也很重要。

當 `sort` 方法遇到 `undefined` 值時，不會將該值傳給比較函式。相反地，`undefined` 值會自動移到陣列的末端。

以下是一個範例，展示 `sort` 如何處理含有 `undefined` 值的陣列：
...
注意 `undefined` 被放置在排序後陣列的最後面，位於所有其他元素之後。

同樣地，當 `sort` 方法遇到稀疏陣列中的空槽時，會保留該空槽並將它移到陣列末端。如果同時存在 `undefined` 值和空槽，`undefined` 值會先出現，空槽則會在最末端。
...
在這個範例中，字串 `"apple"` 和 `"banana"` 先按字母順序排序。接著 `undefined` 值被放在它們之後。最後，空槽被保留在陣列的最末端。
