...
Para tocar a música sempre que o usuário clicar nela, adicione um atributo `onclick` ao primeiro elemento button.Dentro do `onclick`, chame a função `playSong` com `song.id`.

Não esqueça que você precisa interpolar com o sinal de cifrão aqui.

# --hints--

Você deve adicionar um atributo `onclick` com o valor `playSong(${song.id})` ao elemento `button`.
