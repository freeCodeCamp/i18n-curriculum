...
All'interno della funzione `playNextSong`, crea un'istruzione `if` per verificare se lo `currentSong` di `userData` è strettamente uguale a `null`. Questo controllerà se non c'è nessuna canzone in riproduzione nell'oggetto `userData`.

Se la condizione è vera, chiama la funzione `playSong` con lo `id` della prima canzone nell'array `userData?.songs` come argomento.

# --hints--

Non dovresti modificare la funzione `playNextSong` esistente e il suo contenuto.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Dovresti creare un'istruzione `if` con la condizione `userData?.currentSong === null`.
...
Dovresti chiamare la funzione `playSong` con `userData?.songs[0].id` all'interno dell'istruzione `if`.
