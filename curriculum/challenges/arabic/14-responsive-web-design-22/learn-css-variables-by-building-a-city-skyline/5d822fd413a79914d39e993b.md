...
ملاحظة: ستحتاج إلى التمرير خارج المنطقة القابلة للتحرير لنسخ الـ class.

# --hints--

لا يجب عليك حذف التعريف `.sky` الحالي.
...
يجب عليك نسخ التعريف `.sky` الحالي في استعلام الوسائل(media query).
