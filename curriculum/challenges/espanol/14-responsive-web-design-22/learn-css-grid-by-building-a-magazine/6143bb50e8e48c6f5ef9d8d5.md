...
Dentro de su elemento `aside`, crear dos elementos `img` un elemento `blockquote`, y un tercer elemento `img`. Give the `blockquote` element a `class` set to `image-quote`.
