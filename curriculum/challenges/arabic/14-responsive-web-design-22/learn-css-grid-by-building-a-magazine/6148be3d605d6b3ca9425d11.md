...
يجب أن يكون لديك منتقي `.hero-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

يجب أن يكون لمنتقي `.hero-subtitle` الخاص بك خاصية `font-size` بقيمة `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

يجب أن يكون لمنتقي `.hero-subtitle` الخاص بك خاصية `color` بقيمة `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

يجب أن يكون لمنتقي `.hero-subtitle` الخاص بك خاصية `text-align` بقيمة `center`.
