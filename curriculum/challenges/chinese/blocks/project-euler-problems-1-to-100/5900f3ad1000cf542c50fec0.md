...
无限连分数可以写成，$\\sqrt{2} = \[1; (2)]$ 表示 2 无限重复。以类似的方式，$\\sqrt{23} = \[4; (1, 3, 1, 8)]$。事实证明，平方根的连分数部分值序列提供了最佳的有理近似。让我们考虑 $\\sqrt{2}$ 的收敛分数。
...
因此，$\\sqrt{2}$ 的前十个收敛分数的序列是：

$1, \\dfrac{3}{2}, \\dfrac{7}{5}, \\dfrac{17}{12}, \\dfrac{41}{29}, \\dfrac{99}{70}, \\dfrac{239}{169}, \\dfrac{577}{408}, \\dfrac{1393}{985}, \\dfrac{3363}{2378}, ...$

最令人惊讶的是重要的数学常量，$e = \[2; 1, 2, 1, 1, 4, 1, 1, 6, 1, ... , 1, 2k, 1, ...]$。`e` 的前十个收敛序列项是：

$2, 3, \\dfrac{8}{3}, \\dfrac{11}{4}, \\dfrac{19}{7}, \\dfrac{87}{32}, \\dfrac{106}{39}, \\dfrac{193}{71}, \\dfrac{1264}{465}, \\dfrac{1457}{536}, ...$

第 10 个 `convergent` 分子中的数字之和是 $1 + 4 + 5 + 7 = 17$。

查找 `e` 连分数的第 `n`<sup>th</sup> 收敛分数的分子中数字的和。

# --hints--

`convergentsOfE(10)` 应该返回一个数字。

```js
assert(typeof convergentsOfE(10) === 'number');
```

`convergentsOfE(10)` 应该返回 `17`。

```js
assert.strictEqual(convergentsOfE(10), 17);
```

`convergentsOfE(30)` 应该返回 `53`。

```js
assert.strictEqual(convergentsOfE(30), 53);
```

`convergentsOfE(50)` 应该返回 `91`。

```js
assert.strictEqual(convergentsOfE(50), 91);
```

`convergentsOfE(70)` 应该返回 `169`。

```js
assert.strictEqual(convergentsOfE(70), 169);
```

`convergentsOfE(100)` 应该返回 `272`。
