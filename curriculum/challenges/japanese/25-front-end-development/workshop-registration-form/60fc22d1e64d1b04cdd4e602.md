...
`.inline` 要素の右側の `margin` を `0.5em` に設定して、`label` のテキストとの間にスペースを追加してください。 また、他のマージンは `0` に設定してください。

# --hints--

`.inline` の要素の `margin-top` を `0` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginTop, '0px');
```

`.inline` の要素の `margin-right` を `0.5em` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginRight, '0.5em');
```

`.inline` の要素の `margin-bottom` を `0` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginBottom, '0px');
```

`.inline` の要素の `margin-left` を `0` に設定する必要があります。
