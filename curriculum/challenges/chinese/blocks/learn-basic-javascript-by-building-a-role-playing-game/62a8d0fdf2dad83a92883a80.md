...
一旦玩家拥有了最好的武器，他们就不能购买另一个。 将 `buyWeapon` 函数中的所有代码包装在另一个 `if` 语句中。 条件应该检查 `currentWeaponIndex` 是否小于 `3`（最后一个武器的索引）。

# --hints--

你应该有一个新的 `if` 语句来检查 `currentWeaponIndex` 是否小于 `3`。

```js
assert.match(buyWeapon.toString(), /currentWeaponIndex\s*\<\s*3/)
```

你现有的 `if` 语句应位于新的 `if` 语句内。
