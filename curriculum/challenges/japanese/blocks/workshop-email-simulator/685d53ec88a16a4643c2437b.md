...
次に、メール一覧にタイムスタンプも表示しましょう。`__str__`クラスの`Email`メソッドを更新して、件名の後にタイムスタンプを含めてください。

return文を修正して、`'%Y-%m-%d %H:%M'`で区切った末尾に` | Time: `形式のタイムスタンプを含めてください。
...
ここで、`status`はメールのステータス、`sender`は送信者の名前、`subject`はメールの件名、`time`は`'%Y-%m-%d %H:%M'`形式です。

# --hints--

`__str__`メソッドを更新して、フォーマット済みのタイムスタンプを含める必要があります。
