...
كائن `Set` يسمح لك بتخزين قيم فريدة من أي نوع. يمكنك إنشاء مجموعة فارغة باستخدام دالة البناء `Set`:

```js
const mySet = new Set();
``` 

أنشئ دالة باسم `displayPlantsSet` واجعلها تُرجع كائن `Set` جديد.

# --hints--

يجب أن يكون لديك دالة باسم `displayPlantsSet`.

```js
assert.isFunction(displayPlantsSet);
```

يجب أن تُرجع دالة `displayPlantsSet` كائن `Set` فارغ.
