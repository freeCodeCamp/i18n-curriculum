...
Lenga vipengele vya `.arm`, na uwape `width` ya `30%`, `height` ya `60%`, na `background` ya mwelekeo wa mstari wa mviringo (linear gradient) kwa `90deg` kuanzia kwa mwelekeo wa saa, kuanzia `gray`, na kumalizika `rgb(209, 210, 199)`.
...
Unapaswa kumpa `.arm` `width` ya `--fcc-expected--`, iliyopatikana `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

Unapaswa kumpa `.arm` `height` ya `--fcc-expected--`, iliyopatikana `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

Unapaswa kumpa `.arm` `background` ya `linear-gradient(90deg, gray, rgb(209, 210, 199))`.
