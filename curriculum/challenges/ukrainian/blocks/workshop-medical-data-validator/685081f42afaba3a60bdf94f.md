...
Далі ви хочете перевірити, що `age` є цілим числом. Тож додайте ще один ключ `age` до словника `constraints`. Для його значення викличте `isinstance`, передаючи `age` і `int` як аргументи.

# --hints--

Ваш словник `constraints` має містити ключ `age`.
...
Ключ `age` вашого словника `constraints` має мати значення `isinstance(age, int)`.
