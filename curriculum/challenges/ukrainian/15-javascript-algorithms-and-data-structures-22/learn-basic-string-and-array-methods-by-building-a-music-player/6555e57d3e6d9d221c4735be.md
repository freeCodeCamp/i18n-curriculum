...
Коли натиснуто кнопку перемішування, ви бажаєте встановити `currentSong` у нічого, а `songCurrentTime` на `0`.

Встановіть `userData.currentSong` на `null` та `userData.songCurrentTime` на `0`.

*Примітка*: використайте необов’язковий ланцюжок для цього кроку, оскільки ви явно встановлюєте властивості `currentSong` та `songCurrentTime` на `null` та `0` відповідно.

# --hints--

Встановіть `userData.currentSong` на `null`.

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

Встановіть `userData.songCurrentTime` на `0`.
