...
如果用户点击 `Discard` 按钮，你需要关闭显示 `Cancel` 和 `Discard` 按钮的模式，然后隐藏表单模式。

为 `discardBtn` 添加一个点击事件监听器，然后对 `confirmCloseDialog` 变量使用 `close()` 方法。 另外，使用 `classList` 在 `taskForm` 上切换类 `hidden`，这样表单模式也会关闭。
