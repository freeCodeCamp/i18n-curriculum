...
获取 ID 为 `bug-emoji` 的段落元素，并将其保存到名为 `bugEmojiElement` 的常量中。使用 `!` 非空断言操作符来断言该元素不是 `null`。

# --hints--

你应该有一个名为 `bugEmojiElement` 的常量。
...
你的 `bugEmojiElement` 应该使用带有 `bug-emoji` 类型参数的 `document.querySelector` 来查询 ID 为 `HTMLParagraphElement` 的元素。

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

你的 `bugEmojiElement` 应该使用非空断言操作符（`!`）。
