...
بعد ذلك، أنشئ مصفوفة لتخزين كل الأغاني.

أنشئ مصفوفة `allSongs` فارغة.

# --hints--

يجب أن يكون لديك متغير اسمه `allSongs`.

```js
assert.isDefined(allSongs)
```

يجب تعيين مصفوفة إلى متغير `allSongs` الخاص بك.

```js
assert.isArray(allSongs)
```

يجب أن تكون مصفوفاتك فارغة.
