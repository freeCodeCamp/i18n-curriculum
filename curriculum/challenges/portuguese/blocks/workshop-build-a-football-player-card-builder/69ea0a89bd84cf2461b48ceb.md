...
Dentro do bloco `try`, salve os dados atuais do jogador em `localStorage` fazendo uma chamada a `localStorage.setItem` com a chave de armazenamento e o objeto do jogador convertido em string.

E no bloco `catch`, registre no console a string `Failed to save player data:` seguida do parâmetro `error`.

# --hints--

Seu bloco `useEffect` try deve fazer uma chamada a `localStorage.setItem` com `STORAGE_KEY` e o objeto `player` convertido em string.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

Seu bloco `useEffect` catch deve registrar no console `'Failed to save player data:'` e `error`.
