...
Geben Sie die Anzahl aller Permutationen des angegebenen Strings zurück, die keine aufeinanderfolgenden Buchstaben mit Wiederholungen enthalten. Gehen Sie davon aus, dass alle Zeichen im angegebenen String jeweils einzigartig sind.

Zum Beispiel sollte `aab` 2 zurückgeben, da es insgesamt 6 Permutationen gibt (`aab`, `aab`, `aba`, `aba`, `baa`, `baa`), aber nur 2 davon (`aba` und `aba`) keine Wiederholung desselben Buchstabens (in diesem Fall `a`) hintereinander haben.
