...
Per correggere l'errore, crea una funzione freccia `handlePowersChange` con un parametro `e`.

All'interno della funzione, destruttura `value` e `checked` da `e.target` per ottenere il valore della casella di spunta e se è selezionata o meno.

# --hints--

Dovresti creare una funzione freccia `handlePowersChange` con un parametro `e`.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

Dovresti destrutturare `value` e `checked` da `e.target`.
