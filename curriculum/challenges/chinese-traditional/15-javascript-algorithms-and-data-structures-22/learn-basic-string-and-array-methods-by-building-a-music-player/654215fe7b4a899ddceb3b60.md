...
現在 `songsHTML` 是一個數組。 如果你嘗試按原樣顯示，你將看到用逗號分隔的歌曲。 這不是期望的結果，因爲你想以列表形式顯示歌曲。 要解決這個問題，你需要使用 <dfn>join()</dfn> 方法將數組連接成一個字符串。

`join()` 方法用於將數組的所有元素連接成一個字符串。 它採用一個可選參數 `separator`，用於分隔數組的每個元素。 例如：
...
將 `join()` 方法鏈接到 `map()` 方法，並傳入一個空字符串作爲分隔符。

要將多個方法鏈接在一起，你可以在 `map()` 方法的結果上調用 `join()` 方法。 例如：
