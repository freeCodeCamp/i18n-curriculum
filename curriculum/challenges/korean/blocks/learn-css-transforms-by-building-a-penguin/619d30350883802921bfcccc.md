...
이제 CSS 애니메이션을 사용해 펭귄이 손을 흔들게 만드세요.

새로운 `@keyframes`를 `wave`라는 이름으로 정의하세요.

# --hints--

새로운 `@keyframes` 규칙을 정의해야 합니다.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getCSSRules('keyframes'));
```

`@keyframes` 규칙에 `--fcc-expected--`라는 이름을 지정해야 하는데 `--fcc-actual--`가 발견되었습니다.
