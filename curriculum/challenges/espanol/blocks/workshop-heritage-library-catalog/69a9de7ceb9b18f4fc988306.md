...
Dentro del ciclo, verifica si el autor de la entrada actual (convertido a minúsculas) incluye `searchTerm`. El método `.includes()` devuelve `true` si una cadena contiene a otra cadena como subcadena. Si la condición es verdadera, agrega la entrada a `results`. Después del ciclo, devuelve `results`.

# --hints--

`findByAuthor(catalog, "king").length` debería ser `6`.

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

`findByAuthor(catalog, "asimov").length` debería ser `2`.

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

`findByAuthor(catalog, "xyz").length` debería ser `0`.
