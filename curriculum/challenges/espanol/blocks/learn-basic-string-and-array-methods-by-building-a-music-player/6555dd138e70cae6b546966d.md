...
Necesitas obtener la canción que se está reproduciendo actualmente o la primera canción de la lista de reproducción. Para hacer eso, crea una constante `canción` y usa el operador OR (`||`) para asignarla a la canción actual de `userData`, **o** la primera canción en el arreglo `userData?.songs`.

No olvides utilizar el encadenamiento opcional.

# --hints--

No deberías modificar el `setPlayButtonAccessibleText` existente y su contenido.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Deberías acceder a `userData?.currentSong` o `userData?.songs[0]`.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

Deberías asignar `userData?.currentSong || userData?.songs[0]` a una constante `canción`.
