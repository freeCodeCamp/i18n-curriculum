...
1. フォームには、ユーザーのフルネームを入力するためのタイプ `input` の `text` 要素と、ID `name` が含まれていること。
1. ユーザーのメールアドレスを入力するためのタイプ `input` の `email` 要素と、ID `email` の別の要素があること。
...
1. `:focus` と `input` 要素に `textarea` 疑似クラスを追加し、フォーカス時にボーダーの色を変え、デフォルトのアウトラインを削除すること。
...
フォームには、ユーザーのフルネームを入力するためのタイプ `input` の `text` 要素と、ID `name` が含まれていること。

```js
assert.exists(document.querySelector("div.container > form input#name[type='text']"));
```

ユーザーのメールアドレスを入力するためのタイプ `input` の `email` 要素と、ID が `email` の別の要素があること。
