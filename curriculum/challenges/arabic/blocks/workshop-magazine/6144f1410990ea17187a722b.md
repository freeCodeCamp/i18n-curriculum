...
يمكنك الآن تنسيق تخطيط الشبكة الخاصة بك. تشبه شبكة CSS Grid خاصية Flexbox في أنها تحتوي على خاصية مميزة لكل من العنصر الأب والعناصر الفرعية.

في هذه الحالة، العنصر الأب هو عنصر `main`. اجعل المحتوى يتبع تخطيطًا بثلاثة أعمدة بإضافة خاصية `grid-template-columns` بقيمة `1fr 94rem 1fr`. هذا سيُنشئ ثلاثة أعمدة حيث يكون العمود الأوسط بعرض `94rem`، والعمودان الأول والأخير يشغلان كل منهما كسرًا واحدًا من المساحة المتبقية في حاوية الشبكة.

# --hints--

يجب أن يحتوي قسم `main` على خاصية `grid-template-columns` مضبوطة على `1fr 94rem 1fr`.
