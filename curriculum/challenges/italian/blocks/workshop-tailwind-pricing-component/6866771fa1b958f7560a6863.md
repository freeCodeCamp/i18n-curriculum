...
Questo sposta il testo del badge, ma non esattamente dove vuoi. La magia sta nelle classi di utilità `right-[num]`, `left-[num]`, `top-[num]` e `bottom-[num]`.
...
Puoi anche indicare valori negativi anteponendo un trattino:
...
Aggiungi le classi `-top-3` e `right-3` al badge `Most popular` `div`.

# --hints--

Il tuo elemento `div` con il testo `Most popular` dovrebbe avere la classe `-top-3`.
...
Il tuo elemento `div` con il testo `Most popular` dovrebbe avere la classe `right-3`.
