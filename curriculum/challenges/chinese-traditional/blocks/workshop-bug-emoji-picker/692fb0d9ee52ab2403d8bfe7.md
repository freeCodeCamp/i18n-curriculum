...
接著，為 `Bug` 類別創建一個建構式，該建構式接受一個名為 `emojiElement`、型別為 `HTMLParagraphElement` 的參數。在建構式內，將 `emojiElement` 參數指定給該類別的 `emojiElement` 屬性。
...
你的 `Bug` 建構式應該有一個名為 `emojiElement`、型別為 `HTMLParagraphElement` 的參數。

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

你的 `Bug` 建構式應該將 `emojiElement` 指定給 `this.emojiElement`。
