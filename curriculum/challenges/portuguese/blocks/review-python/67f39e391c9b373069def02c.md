...
- **Declaração de variáveis**: Para declarar uma variável, comece com o nome da variável seguido do operador de atribuição (`=`) e logo após o valor. Isto pode ser um número, string, boolean, etc. Aqui estão alguns exemplos:
...
- **`try...except`**: Isso é usado para executar um bloco de código que pode gerar uma exceção. O bloco `try` é onde você antecipa que um erro pode ocorrer, enquanto o bloco `except` captura uma exceção especificada e é executado se esse erro especificado for gerado. Aqui está um exemplo: Você também pode encadear múltiplos blocos `except` para que possa tratar mais tipos de exceções:
