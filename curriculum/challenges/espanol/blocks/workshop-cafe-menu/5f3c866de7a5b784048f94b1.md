...
Para ponerlos en la misma línea, necesitas aplicar algo de estilo a los elementos `p` para que se comporten más como elementos <dfn>en línea</dfn>.

Para hacer eso, comienza agregando un atributo `class` con el valor `item` al primer elemento `article` bajo el encabezado `Coffee`.
...
Debes tener solo un elemento con la clase `item`.
