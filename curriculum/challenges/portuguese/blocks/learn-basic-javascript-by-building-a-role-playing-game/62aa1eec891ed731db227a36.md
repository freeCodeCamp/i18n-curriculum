...
Se você jogar o jogo na situação atual, poderá notar um erro. Se a `xp` for alta o suficiente, a função `getMonsterAttackValue` retornará um número negativo, que, na verdade, vai adicionar vida ao seu total quando lutar com um monstro! You can fix this issue by using a <dfn>ternary operator</dfn> to ensure negative values are not returned.

The ternary operator is a conditional operator and can be used as a one-line `if-else` statement. A sintaxe é: `condition ? expressionIfTrue : expressionIfFalse`.
