...
Aktualisieren Sie nun Ihre innerste Funktion in der `addCharacters`-Kette so, dass sie implizit das Ergebnis des Aufrufs von `charRange()` mit `character1` und `character2` als Argumente zurückgibt.

# --hints--

Ihre innerste Funktion sollte eine implizite Rückgabe verwenden.
...
Ihre innerste Funktion sollte das Ergebnis des Aufrufs von `charRange()` zurückgeben.
...
Sie sollten `character1` als erstes Argument an Ihren `charRange()`-Aufruf übergeben.
...
Sie sollten `character2` als zweites Argument an Ihren `charRange()`-Aufruf übergeben.
