...
给新选择器一个 `bold` 字体，宽度为 `4.5rem`，文本向右对齐。

# --hints--

你应该有一个 `#years span[class]` 选择器。
...
你的 `#years span[class]` 选择器应该有一个 `font-weight` 属性，设置为 `bold`。
...
你的 `#years span[class]` 选择器应该有一个 `width` 属性，设置为 `4.5rem`。
...
你的 `#years span[class]` 选择器应该有一个 `text-align` 属性，设置为 `right`。
