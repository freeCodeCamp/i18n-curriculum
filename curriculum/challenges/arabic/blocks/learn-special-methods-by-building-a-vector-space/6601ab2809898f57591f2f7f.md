...
الآن، عدل تعيينات `v1` و `v2`. حوّلها إلى مثيلات من `R3Vector` ومرر `z=1` و `z=2` إلى `v1` و `v2` على التوالي.

# --hints--

يجب تعديل تعيين `v1` ليكون مثيلًا من `R3Vector`.

```js
({ test: () => assert(runPython(`_Node(_code).find_variable("v1").is_equivalent("v1 = R3Vector(x=2, y=3, z=1)")`)) })
```

يجب تعديل تعيين `v2` ليكون مثيلًا من `R3Vector`.
