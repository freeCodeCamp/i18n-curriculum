...
Dentro da classe `Bee`, crie um construtor que receba um único parâmetro `emojiElement` do tipo `HTMLParagraphElement`.

Dentro do construtor, faça a chamada da função `super` com o parâmetro `emojiElement` para invocar o construtor da classe pai `Bug`. Depois, defina a propriedade `emoji` com o valor string `"🐝"`.

# --hints--

Sua classe `Bee` deve ter um construtor.
...
O construtor da sua `Bee` deve ter um parâmetro chamado `emojiElement` do tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Bee[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

O construtor da sua `Bee` deve fazer a chamada `super` com o parâmetro `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /super\s*\(\s*emojiElement\s*\)/);
```

O construtor da sua `Bee` deve definir `this.emoji` como `"🐝"`.
