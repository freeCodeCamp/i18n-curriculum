...
使用 `for` 循环记录 `updatedCrew` 数组中每个宇航员的 `name`。循环结束后，`return` `updatedCrew` 数组以完成你的 `swapCrewMembers` 函数。

# --hints--

你应该使用 `for` 循环来迭代 `updatedCrew` 数组并记录每个宇航员的 `name`。 
...
你应该在记录其宇航员姓名的 `Log` 后返回 `updatedCrew` 数组。
