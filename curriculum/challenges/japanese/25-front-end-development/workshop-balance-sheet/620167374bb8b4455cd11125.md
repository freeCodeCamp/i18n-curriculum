...
表の見出しの最後の部分は、年の表示です。 `#years` セレクターを作成して、フレックスボックスを有効にしてください。 コンテンツをフレックスコンテナーの主軸方向の末尾に寄せ、スクロールした時にこの要素が画面に貼りつく (sticky) ようにしてください。 その際コンテナーの上端で固定されるように、`top: 0` を設定してください。
