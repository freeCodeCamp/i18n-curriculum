...
首先添加一个 `return`，后面是模板字面量。 在模板字面量中添加一个 `img` 元素。 

在 `img` 标签中，添加一个 `src` 属性，其值为 `${userAvatarUrl}`。 对于 `alt` 属性，添加 `${user.name}` 的值。
...
你应该将 `src` 属性的值设置为名为 `userAvatarUrl` 的常量。
...
你应该将 `alt` 属性的值设置为名为 `user.name` 的常量。
