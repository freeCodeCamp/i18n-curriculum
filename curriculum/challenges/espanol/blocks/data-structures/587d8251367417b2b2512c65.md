...
Al igual que nuestro método `remove(element)`, que <a href="/learn/coding-interview-prep/data-structures/remove-elements-from-a-linked-list" target="_blank" rel="noopener noreferrer nofollow">cubrirnos en una lección anterior</a>, necesitamos tener cuidado de no dejar huérfana el resto de nuestra lista cuando eliminamos el nodo en nuestro método `removeAt(index)`. Mantenemos nuestros nodos contiguos asegurándonos de que el nodo que tiene referencia al nodo eliminado tenga una referencia al siguiente nodo.
