...
Sua função está completa agora. Teste-a novamente registrando `genParentheses(2)` em vez de `genParentheses(1)`.

# --hints--

Você deve registrar `genParentheses(2)`.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*genParentheses\s*\(\s*2\s*\)\s*\)/);
```

Você não deve ter `console.log(genParentheses(1))` no seu código.
