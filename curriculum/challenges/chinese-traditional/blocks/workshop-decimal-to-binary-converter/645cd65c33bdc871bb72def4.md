...
雖然這是一個簡單的範例，但它展示了呼叫堆疊如何逐步執行你的程式碼並呼叫多個函式（程式）。

現在是進入遞迴的時候，並了解呼叫堆疊如何融入其中。

移除你的 `callStack` 陣列、`a()`、`b()` 和 `c()` 函式，以及 `console.log()` 述語。

# --hints--

你應該從你的程式碼中移除 `callStack` 陣列。

```js
assert.notMatch(code, /(var|let|const)\s+callStack\s*=\s*\[\s*\]/);
```

你應該從你的程式碼中移除 `a()`、`b()` 和 `c()` 函式（程式）。
