...
تذكر من JSX أن الأقواس المتعرجة تتيح لك تضمين أي تعبير JavaScript مباشرة داخل JSX. هذه هي الطريقة لعرض البيانات الديناميكية من الخصائص. تفعل نفس الشيء في TSX.

لا يزال القسم العلوي للبطاقة يعرض قيمًا ثابتة. استبدل النص `98` داخل `card-rating` بـ `player.overallRating`، واستبدل `ST` داخل `card-position` بـ `player.position`.

# --hints--

يجب استبدال القيمة الثابتة `98` بـ `player.overallRating` في `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

يجب استبدال القيمة الثابتة `ST` بـ `player.position` في `.card-position`.
