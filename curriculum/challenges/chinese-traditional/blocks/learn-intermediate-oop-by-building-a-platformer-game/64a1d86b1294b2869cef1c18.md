...
目前，當玩家跳起時，有可能會超過畫布的高度。 

為了修正該問題，你需要添加一個條件來阻止玩家跌落超過畫布的高度。

創建一個空的 `if` 述語，用來檢查玩家的 `y` 位置、高度和 `y` 速度的總和是否小於或相等於畫布的高度。 

# --hints--

你的 `update()` 方法應該有一個 `if` 述語。
...
你的 `if` 述語條件應該檢查玩家的 `y` 位置、身高和 `y` 速度的總和是否小於或相等於畫布的高度。
