...
Ahora comenzarás a referenciar los elementos DOM necesarios.

Obtén el elemento párrafo con el ID `bug-emoji` y guárdalo en una constante llamada `bugEmojiElement`. Usa el operador de aserción no nula `!` para asegurar que el elemento no es `null`.

# --hints--

Deberías tener una constante llamada `bugEmojiElement`.
...
Tu `bugEmojiElement` debe consultar el elemento con el ID `bug-emoji` usando `document.querySelector` con un argumento de tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

Tu `bugEmojiElement` debe usar el operador de aserción no nula (`!`).
