...
`ul`/`li` 要素はリスト項目の箇条書きに最適ですが、ラジオボタンには必要ありません。`list-style` プロパティで箇条書きの見た目を制御できます。例えば、次のようにして箇条書きを丸に変えることができます。
...
`.answers-list` 項目のデフォルトスタイルを `none` に設定して削除し、番号なしリストのパディングも削除してください。

# --hints--

`.answers-list` セレクターを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.answers-list'));
```

`.answers-list` に `list-style` を `none` に設定するべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.answers-list')?.listStyle, 'none');
```

`.answers-list` に `padding` を `0` に設定するべきです。
