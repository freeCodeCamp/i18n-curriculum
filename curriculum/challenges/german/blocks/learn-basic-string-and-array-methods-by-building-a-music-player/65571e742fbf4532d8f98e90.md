...
Sie müssen die Songs neu rendern, sie hervorheben und den zugänglichen Text des Abspielknopfs setzen, da sich die Songliste ändern wird.

Rufen Sie die `renderSongs`-Funktion auf und übergeben Sie das `userData?.songs`-Array als Argument, damit die geänderte Wiedergabeliste angezeigt wird.

Rufen Sie danach die `highlightCurrentSong`-Funktion auf, um den aktuellen Song hervorzuheben, falls vorhanden, und die `setPlayButtonAccessibleText`-Funktion, um den zugänglichen Text des Abspielknopfs zu aktualisieren.

# --hints--

Sie sollten die bestehende `deleteSong`-Funktion und deren Inhalt nicht verändern.
...
Sie sollten die `renderSongs`-Funktion mit `userData?.songs` aufrufen.
