...
在 `animate` 函數中，你需要將每個平臺繪製到畫布上。

添加一個 `forEach` 循環，迭代 `platforms` 數組。

在回調函數內部，添加一個 `platform` 參數，並在函數主體中調用每個 `platform` 上的 `draw` 方法。

# --hints--

你應該有一個 `forEach` 循環，用於遍歷 `platforms` 數組。

```js
assert.match(code, /platforms\.forEach\(\s*(\(.*\)|[^\s()]+)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

你應該向回調函數添加 `platform` 參數。

```js
assert.match(code, /platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

你應該在每個 `platform` 上調用 `draw` 方法。
