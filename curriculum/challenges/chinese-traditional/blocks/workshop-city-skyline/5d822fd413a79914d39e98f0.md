...
在前面的模塊中，你瞭解了 CSS 中的漸變是跨元素距離在顏色之間過渡的一種方式。 它們應用於 `background` 屬性，語法如下所示：
...
在示例中，`color1` 在頂部是純色，`color2` 在底部是純色，而在它們之間均勻地從一個顏色過渡到下一個顏色。 在 `.bb1a` 中，在 `background-color` 屬性下面添加 `background` 屬性。 將其設置爲 `linear-gradient` 類型的漸變，使用 `--building-color1` 作爲其第一種顏色，使用 `--window-color1` 作爲第二種顏色。
