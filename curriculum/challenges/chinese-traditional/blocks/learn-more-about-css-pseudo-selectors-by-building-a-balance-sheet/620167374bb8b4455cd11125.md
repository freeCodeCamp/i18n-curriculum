...
表格標題的最後一部分是年份。 創建一個 `#years` 選擇器，並啓用 flexbox。 將內容對齊到 flex 方向的末尾，並使元素具有粘性。 使用 `top: 0` 將其固定到其容器的頂部。
