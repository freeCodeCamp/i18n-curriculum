...
では、`button2` のテキストを `"Buy weapon (30 gold)"` に更新するコードと、`button3` のテキストを `"Go to town square"` に更新するコードも追加してください。

# --hints--

ドット記法を使用して、`button2` の `innerText` プロパティにアクセスしてください。

```js
assert.match(code, /button2\.innerText/);
```

`button2` の `innerText` プロパティにアクセスする際に `let` や `const` を使用する必要はありません。

```js
assert.notMatch(code, /(let|const)\s+button2\.innerText/);
```

`button2` の `innerText` プロパティを `"Buy weapon (30 gold)"` に更新してください。
...
ドット記法を使用して、`button3` の `innerText` プロパティにアクセスしてください。

```js
assert.match(code, /button3\.innerText/);
```

`button3` の `innerText` プロパティにアクセスする際に `let` や `const` を使用する必要はありません。

```js
assert.notMatch(code, /(let|const)\s+button3\.innerText/);
```

`button3` の `innerText` プロパティを `"Go to town square"` に更新してください。
