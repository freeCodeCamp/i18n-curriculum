...
`section` 要素のセレクターを作成し、次のようにしてください：

- `margin` プロパティを `20px auto` に設定する、
- `max-width` を `600px` に設定する、
- `background-color` プロパティを `whitesmoke` に設定する、
- `padding` プロパティを `20px` に設定する、
- `border-radius` プロパティを `10px` に設定する。

# --hints--

`section` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle("section"));
```

`section` セレクターは `margin` プロパティを `20px auto` に設定してください。
...
`section` セレクターは `max-width` プロパティを `600px` に設定してください。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("max-width"), "600px");
```

`section` セレクターは `background-color` プロパティを `whitesmoke` に設定してください。
