...
1. 你應該建置一個網頁，顯示至少三張撲克牌。
1. 你應該有一個 ID 為 `playing-cards` 的 `main` 元素。
1. 在你的 `#playing-cards` 元素內，應該至少有三個 `div` 元素，每個都有 `card` 類別。
1. 在每個 `.card` 元素內，應該有三個 `div` 元素，第一個有 `left` 類別，第二個有 `middle` 類別，第三個有 `right` 類別。
1. 你的 `#playing-cards` 元素應該使用 flexbox 將其子元素水平置中，允許換行，並在它們之間放置 `20px` 的間距。
1. 你的每個 `.card` 元素應該使用 flexbox，並以 `space-between` 來調整其子元素的排列。
1. 你的每個 `.left` 元素應該使用 flexbox 項目屬性，使其在父容器的起始位置對齊。
1. 你的每個 `.middle` 元素應該使用 flexbox 項目屬性，使其在父容器的中間對齊。
1. 你的每個 `.right` 元素應該使用 flexbox 項目屬性，使其在父容器的結束位置對齊。
1. 你的每個 `.middle` 元素應該使用 flexbox 以欄(column)方式顯示其子元素。
