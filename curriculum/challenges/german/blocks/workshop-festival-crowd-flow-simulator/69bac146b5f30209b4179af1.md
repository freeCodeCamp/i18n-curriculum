...
Jetzt können Sie eine Funktion zum Simulieren des Festivals erstellen. Erstellen Sie eine leere Funktion namens `simulateFestival` mit den Parametern `gates` und `timeBlock`.

# --hints--

Sie sollten eine Funktion namens `simulateFestival` erstellen.

```js
assert.isFunction(simulateFestival);
```

Ihre `simulateFestival`-Funktion sollte die Parameter `gates` und `timeBlock` haben.
