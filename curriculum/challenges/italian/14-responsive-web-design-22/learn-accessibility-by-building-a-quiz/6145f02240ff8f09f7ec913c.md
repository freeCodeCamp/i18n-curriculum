...
All'interno degli elementi `div.question-block`, annida un elemento `label` e aggiungi una domanda relativa al `CSS` al testo dell'elemento `label`.
...
Dovresti dare al primo elemento `label` del contenuto di testo.

```js
assert.isAtLeast(document.querySelectorAll('.formrow > .question-block')?.[0]?.querySelector('label')?.textContent?.length, 1);
```

Dovresti dare al secondo elemento `label` del contenuto di testo.
