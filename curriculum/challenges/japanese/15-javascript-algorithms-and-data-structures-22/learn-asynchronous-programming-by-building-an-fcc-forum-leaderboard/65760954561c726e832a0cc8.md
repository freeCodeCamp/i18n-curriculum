...
`allCategories` オブジェクト内に、数値の `299` を新しいキーとして追加して、値を空のオブジェクトにしてください。

そのオブジェクト内に、`category` というキーと `"Career Advice"` という文字列の値を持つプロパティを追加してください。 そのプロパティの下にもう 1 つ、`className` というキーと `"career"` という文字列の値を追加してください。

# --hints--

`allCategories` オブジェクトには `299` というプロパティが必要です。

```js
assert.property(allCategories, '299');
```

`299` の値はオブジェクトであるべきです。

```js
assert.isObject(allCategories['299'])
```

`299` オブジェクトには `category` というプロパティが必要です。

```js
assert.property(allCategories['299'], 'category');
```

`category` プロパティを文字列 `"Career Advice"` に設定してください。

```js
assert.equal(allCategories["299"].category, 'Career Advice');
```

`299` オブジェクトには `className` プロパティが必要です。

```js
assert.property(allCategories['299'], 'className');
```

`className` プロパティを文字列 `"career"` にしてください。
