...
Agora que você tem a lista de músicas exibida na tela, seria bom ordená-las em ordem alfabética pelo título. Você poderia atualizar manualmente o array `allSongs`, mas o JavaScript tem um método de array que você pode usar chamado <dfn>sort()</dfn>.

O método `sort()` converte os elementos de um array em strings e os ordena no local com base em seus valores na codificação `UTF-16`.
...
Dentro da sua função `sortSongs`, adicione o método `sort()` em `userData?.songs`. 

# --hints--

Você deve adicionar o método `sort()` em `userData?.songs`.
