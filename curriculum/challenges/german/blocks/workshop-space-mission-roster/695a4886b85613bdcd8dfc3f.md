...
Erstellen Sie zuerst eine flache Kopie des Eingabe-Arrays `crew` mit `slice()` und weisen Sie diese einer Variablen namens `sorted` zu.

# --hints--

Sie sollten die `slice()`-Methode verwenden, um das Eingabe-Array `crew` in eine neue Variable namens `sorted` zu kopieren.
