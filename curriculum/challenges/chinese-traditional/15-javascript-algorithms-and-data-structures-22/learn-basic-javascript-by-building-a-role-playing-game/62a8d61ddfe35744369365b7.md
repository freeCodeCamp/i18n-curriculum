...
在 `currentWeapon` 之後，使用連接運算符將 `text.innerText` 設置爲字符串 `"You sold a "`，然後是 `currentWeapon`，然後是字符串 `"."`。

# --hints--

你應該將賦值運算符與 `text.innerText` 一起使用。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=/);
```

你應該將 `"You sold a "` 添加到 `text.innerText`。 間距很重要。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

你應該將 `currentWeapon` 的值添加到 `"You sold a "` 字符串中。 使用連接運算符在同一行執行此操作。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a\s\1\s*\+\s*currentWeapon/);
```

你應該將字符串 `"."` 添加到 `currentWeapon` 的值中。 使用連接運算符在同一行執行此操作。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1\s*\+\s*currentWeapon\s+\+\s+('|")\.\2/);
```

你的 `text.innerText` 應該更新爲正確的字符串。
