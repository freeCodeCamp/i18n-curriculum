...
Додайте `for` цикл над `catalog`. Усередині циклу оголосіть `const entry = catalog[i]`. Побудуйте рядок із чотирьох полів, розділених комами, і додайте його до `rows`.

Рядкові поля повинні містити буквальні подвійні лапки у вихідних даних (наприклад, `"King, Stephen"`), щоб коми всередині значень не порушували формат CSV. Оскільки `year` — це число, лапки не потрібні.

# --hints--

Ви повинні використати `for` цикл, щоб ітерувати над `catalog`.

```js
assert.match(exportToCSV.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Кожен рядок має містити поля запису, розділені комами.

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

Рядкові поля слід обгорнути в подвійні лапки.
