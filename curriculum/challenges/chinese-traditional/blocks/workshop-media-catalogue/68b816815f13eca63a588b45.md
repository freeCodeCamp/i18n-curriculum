...
Docstring 是放在類別或函式（程式）中第一個述語的字串，用來為該類別或函式（程式）提供文件說明。Docstring 通常以三個雙引號括起：
...
完成你的 `Movie` 類別，為它添加以下說明字串：`Parent class representing a movie.`

# --hints--

你應該為你的 `Movie` 類別添加 docstring `Parent class representing a movie.`。
