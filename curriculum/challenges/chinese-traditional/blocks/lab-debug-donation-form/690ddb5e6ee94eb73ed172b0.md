...
你的工作是修正所有錯誤，使表單能正確運作並讓所有使用者都能存取。完成以下使用者故事中的項目，然後點擊「Check Your Code」以檢查你是否修正了所有錯誤。
...
4. 你應該使用 `label` 上的 `input` 屬性和 `for` 上相符的 `label` 屬性，將每個 `id` 元素與其對應的 `input` 元素關聯起來。
...
你應該使用 `input` 和 `label` 屬性將每個 `for` 元素與 `id` 元素關聯起來。
