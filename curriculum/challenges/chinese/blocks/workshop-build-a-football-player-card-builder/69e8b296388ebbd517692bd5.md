...
对于 DEF，添加一个 `htmlFor` 为 `def`、文本为 `DEF` 的 `label`，然后添加一个 `id` 为 `def`、`type` 为 `number` 的输入框。

对于 PHY，添加一个 `htmlFor` 为 `phy`、文本为 `PHY` 的标签，然后添加一个 `id` 为 `phy`、`type` 为 `number` 的输入框。
...
你的第五个 `div` 元素的 `className` 的 `form-group` 应该有一个 `label`，它有 `htmlFor` 属性为 `def`，`className` 为 `label`，以及文本 `DEF`。
...
你的第五个 `div` 元素的 `className` 的 `form-group` 应该有一个 `input`，它有 `className` 为 `input`，`id` 为 `def`，以及 `type` 为 `number`。
...
你的第六个 `div` 元素的 `className` 的 `form-group` 应该有一个 `label`，它有 `htmlFor` 属性为 `phy`，`className` 为 `label`，以及文本 `PHY`。
...
你的第六个 `div` 元素的 `className` 的 `form-group` 应该有一个 `input`，它有 `className` 为 `input`，`id` 为 `phy`，以及 `type` 为 `number`。
