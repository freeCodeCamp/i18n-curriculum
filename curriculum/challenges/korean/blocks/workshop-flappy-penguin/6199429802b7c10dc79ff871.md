...
`.penguin-head` 요소를 부모의 위쪽에서 `10%`만큼, 왼쪽에서 `25%`만큼 위치시키세요.

# --hints--

`.penguin-head`에 `top` 속성을 지정해야 합니다.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.top);
```

`.penguin-head`에 `top` 속성을 `--fcc-expected--` 값으로 지정해야 하지만, `--fcc-actual--` 값을 찾았습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.top, '10%');
```

`.penguin-head`에 `left` 속성을 지정해야 합니다.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.left);
```

`.penguin-head`에 `left` 속성을 `--fcc-expected--` 값으로 지정해야 하지만, `--fcc-actual--` 값을 찾았습니다.
