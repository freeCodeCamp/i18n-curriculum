...
创建一个 `dataArrIndex` 变量。 要获取其值，请使用 `taskData` 上的 `findIndex()` 方法。 将 `item` 作为参数传递给其回调函数，并检查 `item` 的 `id` 是否等于 `buttonEl` 的 `parentElement` 的 `id`。
...
你的箭头函数回调应该检查 `item.id === buttonEl.parentElement.id`。
