...
أخيرًا، حدّث الدالة الداخلية لتُعيد دالة ثالثة. يجب أن تقبل هذه الدالة الثالثة معلمة واحدة باسم `quantity`، وأن يكون جسمها فارغًا.

# --hints--

يجب أن تُعيد دالتك الثانية دالة أخرى.

```js
assert.isFunction(convertQuantity()());
```

يجب أن تحتوي دالتك الأعمق على معلمة `quantity`.

```js
assert.match(convertQuantity()()?.toString(), /\(?quantity\)?/);
```

يجب أن تكون دالتك الأعمق فارغة.
