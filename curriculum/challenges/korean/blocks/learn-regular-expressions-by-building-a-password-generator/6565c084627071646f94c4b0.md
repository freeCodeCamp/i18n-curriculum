...
마지막으로 `for` 반복문 뒤에 `if` 문을 만들어 `count`가 `4`와 같은지 확인하고 `while` 문을 사용해 `break` 반복문을 탈출하세요.

# --hints--

`if` 반복문 뒤에 `count`가 `4`와 같은지 확인하는 `for` 문을 만들어야 합니다.
...
새로운 `break` 안에 `if`을 사용해 `while` 반복문을 탈출하세요.
