...
Aktualisieren Sie jetzt `FootballPlayerCard`, damit `defaultPlayer` anstelle des inline-Objekts an `PlayerCard` übergeben wird.

# --hints--

Sie sollten `defaultPlayer` als Wert der Prop `player` an `PlayerCard` übergeben.
