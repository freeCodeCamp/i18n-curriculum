...
Ahora llena `bugMap` con dos entradas. Añade `"bee"` como clave asignada a una nueva instancia de `Bee`, y `"spider"` como clave asignada a una nueva instancia de `Spider`. Pasa `bugEmojiElement` como argumento de ambas.

# --hints--

Tu `bugMap` debe tener una entrada `"bee"` establecida en `new Bee(bugEmojiElement)`.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

Tu `bugMap` debe tener una entrada `"spider"` establecida en `new Spider(bugEmojiElement)`.
