...
Буде шість прапорців, тому найкраще створити для них масив і спроєктувати їх, щоб відобразити як прапорці. Для цього вам надано масив `powersOptions`.

Використайте стрілкову функцію, щоб спроєктувати масив `powersOptions` з параметром `power`. Усередині `key` створіть елемент `label` з ідентифікатором `power` і текстом `Hello`.

Після цього ви побачите шість текстів `Hello`. Ви заповните елементи масиву на наступному кроці.

# --hints--

Ви повинні спроєктувати `powersOptions`.

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\(/)
```

Використайте `power` як параметр вашого map.

```js
assert.match(code, /{\s*powersOptions\s*\.\s*map\s*\(\s*(power|\(\s*power\s*\))\s*=>/)
```

Створіть елемент `label` з `key` `power`.

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\([\s\S]*<\s*label\s+key\s*=\s*\{\s*power\s*\}\s*>[\s\S]*<\s*\/\s*label\s*>/)
```

Ваш елемент `label` повинен містити текст `Hello`.
