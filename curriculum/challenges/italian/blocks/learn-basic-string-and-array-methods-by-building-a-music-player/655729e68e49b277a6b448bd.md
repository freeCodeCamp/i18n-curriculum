...
Ora che hai creato il `resetButton`, devi assegnargli un `id` e un `aria-label` attributi. JavaScript fornisce le proprietà `id` e `ariaLabel` che devi usare per questo.

Per esempio, `element.id` imposta un attributo `id`, e `element.ariaLabel` imposta un attributo `aria-label`. Entrambi accettano i loro valori come stringa.

Imposta l'attributo `id` dell'elemento `resetButton` su `"reset"` e il suo attributo `aria-label` su `"Reset playlist"`.

# --hints--

Non dovresti modificare l'istruzione `if` esistente e il suo contenuto.
...
Dovresti usare `resetButton.id` per creare un attributo `id` chiamato `"reset"` per il `resetButton`.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

Dovresti usare `resetButton.ariaLabel` per creare un attributo `"aria-label"` chiamato `"Reset playlist"` per il `resetButton`.
