...
如果你嘗試輸入消息 `free money`，你會注意到它與表達式不匹配！ 這是因爲 `\s` 不匹配文本的開頭或結尾。

要匹配文本的開頭，可以使用 `^` 錨點。 這斷言你的模式匹配從整個字符串的開頭開始。

將你的第一個 `\s` 字符替換爲一個非捕獲組，該組匹配 `\s` 或 `^`。

# --hints--

你的 `freeRegex` 應該使用一個非捕獲組。

```js
assert.match(freeRegex.source, /\(\?:/);
```

你的 `freeRegex` 應該使用非捕獲組匹配 `\s` 或 `^`。
