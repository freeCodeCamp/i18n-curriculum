...
Тепер настав час пов’язати панелі вкладок із відповідними вкладками.

Для першого елемента `tabpanel` додайте атрибут `id` зі значенням `"panel-earth"` та атрибут `aria-labelledby` зі значенням `"tab-earth"`.

# --hints--

Ваш перший елемент `tabpanel` має містити атрибут `id`.

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("id"));
```

Ваш атрибут `id` має мати значення `"panel-earth"`.

```js
assert.equal(document.querySelector('[role="tabpanel"]').getAttribute("id"), "panel-earth");
```

Ваш перший елемент `tabpanel` має містити атрибут `aria-labelledby`.

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("aria-labelledby"));
```

Ваш атрибут `aria-labelledby` має мати значення `"tab-earth"`.
