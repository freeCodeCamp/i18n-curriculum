...
Añade un tercer objeto con las siguientes propiedades y valores:
...
El tercer objeto en tu array `allSongs` debe tener una propiedad `id` establecida en el número `2`.

```js
assert.equal(allSongs[2].id, 2);
```

El tercer objeto en tu array `allSongs` debe tener una propiedad `title` establecida en la cadena `"Still Learning"`.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

El tercer objeto en tu array `allSongs` debe tener una propiedad `artist` establecida en la cadena `"Quincy Larson"`.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

El tercer objeto en tu array `allSongs` debe tener una propiedad `duration` establecida en la cadena `"3:51"`.

```js
assert.equal(allSongs[2].duration, "3:51");
```

El tercer objeto en tu array `allSongs` debe tener una propiedad `src` establecida en la cadena `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`.
