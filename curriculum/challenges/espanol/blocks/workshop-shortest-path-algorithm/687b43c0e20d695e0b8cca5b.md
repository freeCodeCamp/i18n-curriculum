...
En este punto, solo quieres mostrar resultados para nodos que no sean el nodo inicial y que sean accesibles desde él.

Agrega una condicional que verifique si `node_no` es igual a `start_node` **o** si `distances[node_no]` es igual a `INF`.

Si alguna de las condiciones es verdadera, usa `continue` para saltar a la siguiente iteración del ciclo.

# --hints--

Debes agregar una sentencia condicional dentro del ciclo.
...
Tu sentencia `if` debe verificar si `node_no` es igual a `start_node` *o* si `distances[node_no]` es igual a `INF`.
...
Debes usar `continue` para saltar a la siguiente iteración del ciclo.
