...
為 `for` 添加一個 `catalog` 迴圈。在迴圈內宣告 `const entry = catalog[i]`。建置一個由四個欄位以逗號分隔的列字串，並將其推入 `rows`。

字串欄位在輸出時需要加上字面上的雙引號字元（例如 `"King, Stephen"`），以避免欄位內的逗號破壞 CSV 格式。由於 `year` 是數字，故不需要加雙引號。
