...
声明一个名为 `cardElement` 的变量，并使用 `card_container` 类来获取该元素。使用 `closest` 选择器从 `target` 中获取它。

# --hints--

你应该在 `cardElement` 内声明一个名为 `showFortune` 的变量。
...
你的 `cardElement` 应该使用 `target?.closest` 来查找 `.card_container`。
...
你应该将 `.card_container` 作为参数传递给 `closest`。
