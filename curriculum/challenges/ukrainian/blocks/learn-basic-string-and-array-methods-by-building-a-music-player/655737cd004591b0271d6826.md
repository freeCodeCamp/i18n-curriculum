...
Щоб скинути плейлист до його початкового стану, розпакуйте `allSongs` у масив і призначте його `userData.songs`.

*Примітка*: не слід використовувати опціональний ланцюжок для `userData.songs`, оскільки на цьому етапі пісня не буде `null` або `undefined`.

# --hints--

Не змінюйте існуючий слухач подій і його вміст.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*\)\s*;?/)
```

Ви повинні призначити `[...allSongs]` `userData.songs`.
