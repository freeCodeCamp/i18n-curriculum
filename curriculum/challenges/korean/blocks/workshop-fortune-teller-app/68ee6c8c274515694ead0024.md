...
운세는 무작위여야 하므로 무작위 운세를 선택하는 도우미 함수가 필요합니다. `chosenCard` 메서드 안에 `singleCardSelected`라는 변수를 선언하세요. `getRandomItem` 메서드를 사용해 무작위 카드를 얻고 `cards`를 인수로 전달하세요.

# --hints--

`chosenCard` 안에 `singleCardSelected`라는 변수를 선언해야 합니다.
...
`chosenCard` 변수는 `getRandomItem(this.cards)`를 호출하여 할당해야 합니다.
...
`this.cards`에 인수로 `getRandomItem`를 전달해야 합니다.
