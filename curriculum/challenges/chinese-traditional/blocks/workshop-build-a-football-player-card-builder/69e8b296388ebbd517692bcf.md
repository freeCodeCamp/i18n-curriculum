...
接著為 `div` 添加一個 `className` 的 `form-row`。在裡面，加入兩個具有 `div` 為 `className` 的 `form-group` 元素。

第一個 `div` 元素應該有一個 `label`，其 `className` 為 `label`，一個 `htmlFor` 屬性為 `position`，一個文字 `Position`，以及一個 `input`，其 `id` 為 `position`，`className` 為 `input`，`type` 為 `text`。

# --hints--

你的 Player Info 區段應該包含一個 `div`，其 `className` 為 `form-row`。
...
你的 `div.form-row` 應該包含兩個 `div` 元素，每個都有 `className` 為 `form-group`。
...
你在列中的第一個 `form-group` 應該有一個 `label`，其 `className` 為 `label`，以及一個 `htmlFor` 為 `position`。
...
你在列中的第一個 `form-group` 應該有一個 `input`，其 `className` 為 `input`，`id` 為 `position`，以及 `type` 為 `text`。
