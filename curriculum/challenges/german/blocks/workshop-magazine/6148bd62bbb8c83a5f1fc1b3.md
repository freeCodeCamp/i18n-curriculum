...
Erstellen Sie einen `.hero-title`-Selektor und geben Sie ihm eine `text-align`-Eigenschaft mit dem Wert `center`, eine `color`-Eigenschaft mit dem Wert `orangered` und eine `font-size`-Eigenschaft mit dem Wert `8rem`.

# --hints--

Sie sollten einen `.hero-title`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

Ihr `.hero-title`-Selektor sollte eine `text-align`-Eigenschaft mit dem Wert `center` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign, 'center');
```

Ihr `.hero-title`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `orangered` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color, 'orangered');
```

Ihr `.hero-title`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `8rem` haben.
