...
title: 'Problema 51: Reemplazos de dígitos primos'
challengeType: 1
forumTopicId: 302162
...
Al reemplazar el 1er dígito del número de 2 dígitos \*3, resulta que seis de los nueve valores posibles: 13, 23, 43, 53, 73 y 83, son todos primos.

Al reemplazar el 3er y 4to dígito de 56\*\*3 con el mismo dígito, este número de 5 dígitos es el primer ejemplo que tiene siete números primos entre los diez números generados, dando lugar a la familia: 56003, 56113, 56333, 56443, 56663, 56773 y 56993. En consecuencia, 56003, siendo el primer miembro de esta familia, es el número primo más pequeño con esta propiedad.

Encuentra el número primo más pequeño que, al reemplazar parte del número (no necesariamente dígitos adyacentes) con el mismo dígito, forma parte de una familia de valores `n` primos.

# --hints--

`primeDigitReplacements(6)` debería devolver un número.

```js
assert(typeof primeDigitReplacements(6) === 'number');
```

`primeDigitReplacements(6)` debería devolver `13`.

```js
assert.strictEqual(primeDigitReplacements(6), 13);
```

`primeDigitReplacements(7)` debería devolver `56003`.

```js
assert.strictEqual(primeDigitReplacements(7), 56003);
```

`primeDigitReplacements(8)` debería devolver `121313`.
