...
2番目の`legend`要素の中に、テキスト`What's your cat's personality?`を持つ`fieldset`要素を追加してください。

# --hints--

2番目の`fieldset`要素を削除したか、開始タグまたは終了タグが欠けています。
...
`legend`要素にはテキスト`What's your cat's personality?`が必要です。テキストを省略したか、タイプミスがあります。
