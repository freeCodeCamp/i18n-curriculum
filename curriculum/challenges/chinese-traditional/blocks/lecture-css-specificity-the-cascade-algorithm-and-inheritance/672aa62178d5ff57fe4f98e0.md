...
在這個範例中，第一個段落會是紅色，因為 ID 選擇器的特異性比類別和型別選擇器高。`.example-para` 元素會是綠色，而其他段落元素會是藍色。
...
這是一個使用 `*` 選擇器將所有元素的顏色設為紅色的範例。不過你不會看到任何紅色元素，因為有 ID 和型別選擇器覆寫了這些樣式，這凸顯了 `*` 選擇器的低特異性。
:::interactive_editor
...
內部 CSS 是定義在 HTML 文件 `head` 區段的 `style` 元素中。它的特異性比行內樣式低。內部和外部 CSS 具有相同的特異性層級——特異性由所使用的選擇器決定，而不是 CSS 定義的位置。當兩條規則的特異性相同時，文件中後出現的規則會勝出。因此，如果外部樣式表在內部 `style` 區塊之後被連結，外部規則會優先；如果在之前連結，則內部規則勝出。
...
在此範例中，文字將會是藍色，除非套用行內樣式或更具特異性的選擇器。

外部 CSS 是透過 `head` 區段中的 `link` 元素來連結，並寫在獨立的 `.css` 檔案中。像內部 CSS 一樣，外部樣式的特異性由所使用的選擇器決定。外部 CSS 為較大型專案提供最佳的維護性。
...
在此範例中，文字顏色定義在 `styles.css` 檔案中，除非被更具體的選擇器或行內樣式覆寫，否則將會套用。
