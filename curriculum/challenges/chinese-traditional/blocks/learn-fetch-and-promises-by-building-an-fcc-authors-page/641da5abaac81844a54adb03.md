...
現在你將創建一個函式來填充 UI 的作者資料。你會在第二個 `.then()` 方法中呼叫這個函式。

創建一個名為 `displayAuthors` 的空箭頭函式（程式），該函式以 `authors` 作為參數。
...
你應該使用 `const` 來創建一個名為 `displayAuthors` 的空函式（程式）。
