...
Додайте вашому `conversionTable` властивість `cup` і призначте їй об’єкт із властивістю `gram`, встановленою в `240`, властивістю `ounce`, встановленою в `8.0`, і властивістю `teaspoon`, встановленою в `48`.

# --hints--

Ваш `conversionTable` має мати властивість `cup`.

```js
assert.property(conversionTable, "cup");
```

Властивість `cup` має бути об’єктом.

```js
assert.isObject(conversionTable.cup);
```

Ваш об’єкт `cup` має мати властивість `gram`.

```js
assert.property(conversionTable.cup, "gram");
```

Властивість `gram` має бути встановлена в `240`.

```js
assert.strictEqual(conversionTable.cup?.gram, 240);
```

Ваш об’єкт `cup` має мати властивість `ounce`.

```js
assert.property(conversionTable.cup, "ounce");
```

Властивість `ounce` має бути встановлена в `8.0`.

```js
assert.strictEqual(conversionTable.cup?.ounce, 8.0);
```

Ваш об’єкт `cup` має мати властивість `teaspoon`.

```js
assert.property(conversionTable.cup, "teaspoon");
```

Властивість `teaspoon` має бути встановлена в `48`.
