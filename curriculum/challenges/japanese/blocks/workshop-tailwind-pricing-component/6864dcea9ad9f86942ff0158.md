...
3つの料金カードを包む`div`要素に取り組む時です。カードを配置するには、Tailwind CSSのグリッドユーティリティクラスを使うことができます。

Tailwindで要素に`grid`の表示を代入する方法は次の通りです。
...
料金カードを包む`grid`要素に`div`のクラスを代入してください。

# --hints--

`div`要素には`grid`のクラスを付ける必要があります。
