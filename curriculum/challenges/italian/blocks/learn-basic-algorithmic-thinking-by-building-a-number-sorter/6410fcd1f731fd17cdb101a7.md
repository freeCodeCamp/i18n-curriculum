...
All’interno del tuo loop `for` annidato, aggiungi una chiamata `console.log()` per controllare i valori di `array`, `array[j]` e `array[minIndex]` a ogni iterazione. Puoi cliccare il pulsante `Sort` per vedere come il tuo algoritmo sta attraversando l’array.

Poi scrivi un’istruzione `if` che verifichi se il valore in `array[j]` è più piccolo del valore in `array[minIndex]`. Se lo è, imposta `minIndex` su `j`.

# --hints--

Dovresti avere una chiamata `console.log()` all’interno del tuo loop `for` annidato.
...
Dovresti passare `array` come primo argomento a `console.log()`.
...
Dovresti passare `array[j]` come secondo argomento a `console.log()`.
...
Dovresti passare `array[minIndex]` come terzo argomento a `console.log()`.
...
Dovresti avere un’istruzione `if` dopo la tua chiamata `console.log()`.
...
Dovresti avere una condizione nella tua istruzione `if` che verifica se `array[j]` è minore di `array[minIndex]`.
...
La tua istruzione `if` dovrebbe impostare `minIndex` su `j`.
