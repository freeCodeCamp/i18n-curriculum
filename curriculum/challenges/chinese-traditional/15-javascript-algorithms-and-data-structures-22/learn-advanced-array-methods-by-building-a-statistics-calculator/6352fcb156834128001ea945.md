...
聲明一個 `sumSquaredDifferences` 變量，並給它賦值 `squaredDifferences.reduce()`。 在回調中，返回 `acc` 和 `el` 的和。 記住要將初始值設置爲 `0`。

# --hints--

你應該有一個 `sumSquaredDifferences` 變量。

```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=/);
```

你的 `sumSquaredDifferences` 變量應該使用 `squaredDifferences.reduce()` 方法。

```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(/);
```

你的 `sumSquaredDifferences` 變量應該使用回調函數中的 `acc` 和 `el` 參數。

```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)/);
```

你的 `reduce` 回調應該返回 `acc` 和 `el` 的和。
