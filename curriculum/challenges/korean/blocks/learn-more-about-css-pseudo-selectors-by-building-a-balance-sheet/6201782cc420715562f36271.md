...
`#years` 요소 안의 텍스트를 `#years span[class]` 선택자를 만들어 스타일링하세요. `span[class]` 구문은 `span` 속성이 설정된 모든 `class` 요소를 속성 값과 상관없이 선택합니다.

새 선택자에 `bold` 폰트, 너비 `4.5rem`, 그리고 오른쪽 정렬된 텍스트를 지정하세요.

# --hints--

`#years span[class]` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]'));
```

`#years span[class]` 선택자에 `font-weight` 속성이 `bold`으로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]')?.getPropertyValue('font-weight') === 'bold');
```

`#years span[class]` 선택자에 `width` 속성이 `4.5rem`으로 설정되어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]')?.getPropertyValue('width') === '4.5rem');
```

`#years span[class]` 선택자에 `text-align` 속성이 `right`으로 설정되어야 합니다.
