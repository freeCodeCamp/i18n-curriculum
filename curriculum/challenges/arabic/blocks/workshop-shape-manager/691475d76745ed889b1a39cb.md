...
يجب أن تعمل أيضًا على دالة تقوم بمسح حقول الإدخال.

أنشئ دالة `clearInputFields`. بداخلها، استخدم `querySelectorAll` للحصول على كل حقول الإدخال وكرر عليها باستخدام `forEach()`. داخل `forEach()`، اجعل قيمة كل `input` سلسلة نصية فارغة.

# --hints--

يجب أن تنشئ دالة `clearInputFields`.

```js
assert.isFunction(clearInputFields)
```

يجب أن تجعل دالة `clearInputFields` كل حقل إدخال سلسلة نصية فارغة.
