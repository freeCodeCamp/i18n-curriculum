...
Dê à sua `conversionTable` uma propriedade `ounce` e atribua a ela um objeto com uma propriedade `cup` definida como `0.125`, uma propriedade `gram` definida como `28.35` e uma propriedade `teaspoon` definida como `6`.

# --hints--

Seu objeto `conversionTable` deve ter uma propriedade `ounce`.

```js
assert.property(conversionTable, "ounce");
```

Sua propriedade `ounce` deve ser um objeto.

```js
assert.isObject(conversionTable.ounce);
```

Seu objeto `ounce` deve ter uma propriedade `cup`.

```js
assert.property(conversionTable.ounce, "cup");
```

A sua propriedade `cup` deve ser definida como `0.125`.

```js
assert.strictEqual(conversionTable.ounce?.cup, 0.125);
```

Seu objeto `ounce` deve ter uma propriedade `gram`.

```js
assert.property(conversionTable.ounce, "gram");
```

A sua propriedade `gram` deve ser configurada para `28.35`.

```js
assert.strictEqual(conversionTable.ounce?.gram, 28.35);
```

Seu objeto `ounce` deve ter uma propriedade `teaspoon`.

```js
assert.property(conversionTable.ounce, "teaspoon");
```

A sua propriedade `teaspoon` deve ser configurada para `6`.
