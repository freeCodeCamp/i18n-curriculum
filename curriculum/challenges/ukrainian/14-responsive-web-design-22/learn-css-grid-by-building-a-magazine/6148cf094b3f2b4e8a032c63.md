...
Створіть селектор `.first-paragraph::first-letter` та встановіть властивість `font-size` на `6rem`. Також надайте йому властивість `color` зі значенням `orangered`, щоб виділятися.

# --hints--

Ви повинні мати селектор `.first-paragraph::first-letter`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

Ваш селектор `.first-paragraph::first-letter` повинен мати властивість `font-size` зі значенням `6rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

Ваш селектор `.first-paragraph::first-letter` повинен мати властивість `color` зі значенням `orangered`.
