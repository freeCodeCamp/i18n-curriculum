...
`:not()` 伪选择器用于定位与选择器不匹配的所有元素——在这种情况下，我们希望定位没有 `sr-only` 类的任何 `span` 元素。 这可以确保之前的 `span[class~="sr-only"]` 类规则不会被覆盖。

# --hints--

应该有一个 `span:not(.sr-only)` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)'));
```

`span:not(.sr-only)` 选择器应该有一个 `font-weight` 属性设置为 `normal`。
