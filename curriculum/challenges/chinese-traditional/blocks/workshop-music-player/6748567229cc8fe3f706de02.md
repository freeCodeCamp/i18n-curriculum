...
定義一個不帶任何參數的 `pauseSong` 函式（程式）。在你新的函式（程式）中，為了儲存歌曲暫停時的當前時間，將 `userData` 物件的 `songCurrentTime` 設定為 `audio` 變數的 `currentTime`。


# --hints--

你應該有一個 `pauseSong` 函式（程式）。

```js
assert.isFunction(pauseSong);
```

你的 `pauseSong` 函式（程式）應該將 `userData` 物件的 `songCurrentTime` 設定為 `audio` 變數的 `currentTime`。
