...
现在你需要准备好使用用户添加的新产品来更新显示。 声明一个 `currentProductCount` 变量，并为其分配与 `product` 的 `id` 匹配的 `totalCountPerProduct` 对象的属性值。

# --hints--

你应该在 `addItem` 函数中声明一个 `currentProductCount` 变量。
...
你应该使用 `const` 来声明 `currentProductCount`。
...
你应该将 `totalCountPerProduct[product.id]` 的值分配给 `currentProductCount`。
