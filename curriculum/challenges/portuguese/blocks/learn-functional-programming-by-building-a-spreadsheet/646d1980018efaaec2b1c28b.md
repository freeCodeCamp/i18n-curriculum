...
Use a mesma sintaxe da função `addCharacters` para atualizar a função `elemValue`.Ela não deve mais declarar `inner`, mas retornar a função implicitamente.

# --hints--

A função `elemValue` deve usar um retorno implícito.
...
A função `elemValue` deve retornar implicitamente uma arrow function com um parâmetro `character`.
...
A arrow function interna deve usar um retorno implícito.
...
A arrow function interna deve retornar o resultado da chamada de `idToText()` com `character + num` como argumento.
