...
Füge unter dem `.eye.right`-Element zwei `div`-Elemente mit einer `class` von `blush` hinzu. Gib außerdem dem ersten `.blush` Element eine `class` von `left`, und dem zweiten `.blush` Element eine `class` von `right`.
...
Du solltest dem ersten neuen `div` eine `class` von `blush` geben.

```js
assert.exists(document.querySelector('.penguin-head > div.blush'));
```

Du solltest dem zweiten neuen `div` eine `class` von `blush` geben.
...
Du solltest `.blush.right` nach `.blush.left` platzieren.
