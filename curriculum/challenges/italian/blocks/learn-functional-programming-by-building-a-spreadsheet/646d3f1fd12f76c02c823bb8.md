...
Hai un piccolo bug. `arg1` e `arg2` sono stringhe, non numeri. `infixToFunction['+']("1", "2")` restituirebbe `12`, il che non è matematicamente corretto.

Avvolgi ciascuno dei tuoi argomenti `infixToFunction[operator]` in una chiamata `parseFloat()`.

# --hints--

Dovresti avvolgere `arg1` in una chiamata `parseFloat()`.
...
Dovresti avvolgere `arg2` in una chiamata `parseFloat()`.
