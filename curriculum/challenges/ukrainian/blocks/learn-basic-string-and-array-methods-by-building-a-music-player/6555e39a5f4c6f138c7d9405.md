...
У попередніх кроках ви навчились працювати з методом `sort()` для сортування пісень в алфавітному порядку. Іншим випадком використання функції зворотного виклику є випадкове перемішування масиву.

Один зі способів випадково перемішати масив елементів — це відняти `0.5` від `Math.random()`, що дає випадкові значення, які можуть бути як додатними, так і від’ємними. Це робить результат порівняння сумішшю додатних і від’ємних значень, що призводить до випадкового порядку елементів.
...
Використайте метод `sort()` для масиву `userData?.songs`. Передайте методу функцію зворотного виклику і поверніть результат `Math.random() - 0.5`.


# --hints--

Вам не слід змінювати існуючу функцію `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

Використайте метод `sort()` для `userData?.songs`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

Ваш метод `sort()` має мати функцію зворотного виклику, яка використовує стрілковий синтаксис.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

Функція зворотного виклику вашого методу `sort()` має повертати `Math.random() - 0.5`.
