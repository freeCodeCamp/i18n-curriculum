...
`.daily-value` 요소 뒤에 중간 구분선을 추가하세요. 그 새 구분선 아래에 `p` 속성이 `class`로 설정된 `note` 요소를 만드세요.
...
`div` 요소 뒤에 새 `.daily-value`를 만들어야 합니다.

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

새 `div`에는 `class`이 `divider medium`로 설정되어야 합니다.
