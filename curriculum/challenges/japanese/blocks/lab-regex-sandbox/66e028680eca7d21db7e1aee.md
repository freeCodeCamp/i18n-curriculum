...
1. `#pattern`、`#test-string`、`#test-btn`、`#result` の各要素にアクセスし、それぞれを変数 `regexPattern`、`stringToTest`、`testButton`、`testResult` に保存してください。
1. `#i` と `#g` のフラグチェックボックスにアクセスし、それぞれを変数 `caseInsensitiveFlag` と `globalFlag` に保存してください。
1. チェックされたフラグチェックボックスからフラグを含む文字列を返す `getFlags` という名前の関数を用意してください。どのチェックボックスもチェックされていない場合は空の文字列を返してください。
1. `testButton` ボタンをクリックしたときに、正規表現パターンがテスト文字列にマッチした場合、マッチしたテキストをハイライトしてください。ハイライトするには、`stringToTest` の `innerHTML` を更新し、各マッチを `highlight` クラスの `span` 要素で囲まれた同じテキストに置き換えてください。CSSはすでに追加されています。
1. マッチがある場合、マッチしたテキストを `#result` 内に表示してください。複数のマッチがある場合は、各マッチしたテキストをカンマとスペースで区切ってください。
1. マッチがない場合は、`#result` 内に `no match` というテキストを表示してください。
