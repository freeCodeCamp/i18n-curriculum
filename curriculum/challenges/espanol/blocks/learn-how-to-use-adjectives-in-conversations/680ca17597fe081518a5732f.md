...
This word means to request or buy something, usually officially or formally. Termina con `-ed`.
...
`Last time` refers to the most recent event. Por ejemplo:

`Last time we updated the server, we had some downtime.` – This indicates during the previous update.

`Order` means to officially request goods or equipment. Its past form is `ordered`. Por ejemplo:

`The IT team ordered new laptops for the developers.` – This means they placed a request for laptops.

`Double that` means twice the previous quantity. Por ejemplo:
