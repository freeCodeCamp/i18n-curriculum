...
Declare uma variável chamada `cardElement` e use a classe `card_container` para obter o elemento. Use o seletor `closest` para buscá-lo dentro do `target`.

# --hints--

Você deve declarar uma variável chamada `cardElement` dentro do `showFortune`.
...
Sua `cardElement` deve usar `target?.closest` para buscar o `.card_container`.
...
Você deve passar `.card_container` como argumento para `closest`.
