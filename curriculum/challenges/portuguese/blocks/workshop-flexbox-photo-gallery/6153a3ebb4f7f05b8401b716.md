...
O atributo `alt` da imagem deve descrever o conteúdo da imagem. Leitores de tela anunciam o texto alternativo no lugar das imagens. Se a imagem não puder ser carregada, esse texto é apresentado no lugar da imagem.

Para completar o projeto, adicione um atributo `alt` a todas as nove imagens de gatos para descrevê-las. Use um valor de pelo menos 5 caracteres de tamanho para cada um.


# --hints--

Todos os nove elementos `img` devem ter um atributo `alt` com texto descrevendo cada imagem, com cada descrição usando pelo menos `5` letras.
