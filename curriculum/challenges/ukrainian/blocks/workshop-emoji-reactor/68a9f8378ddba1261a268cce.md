...
Ви можете зробити код більш багаторазовим.

Створіть функцію з назвою `updateCount`, яка прийматиме елемент кнопки як параметр.

Ця функція має містити всю логіку оновлення лічильника кнопки: знаходження елемента лічильника, розбір поточного значення, перевірку, чи досягнуто максимуму, збільшення значення та оновлення відображення.

Потім оновіть функцію зворотного виклику вашого слухача подій, щоб вона викликала `updateCount(happyBtn)` замість того, щоб містити логіку рядковим кодом.

# --hints--

Ви маєте створити функцію з назвою `updateCount`.

```js
assert.isFunction(updateCount);
```

Функція `updateCount` має приймати параметр кнопки.

```js
assert.equal(updateCount.length, 1);
```

Функція `updateCount` має коректно працювати з будь-яким елементом кнопки.
...
Ваш слухач подій має викликати `updateCount` з кнопкою як аргументом.
