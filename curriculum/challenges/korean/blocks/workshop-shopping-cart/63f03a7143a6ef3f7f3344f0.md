...
사용자가 프롬프트를 확인했을 때만 장바구니를 지우고 싶습니다. 사용자가 프롬프트를 확인했는지 검사하는 `if` 문장을 만드세요.

`if` 문장에서는 `items` 속성을 빈 배열로 다시 설정한 다음, `total` 속성을 `0`로 설정하세요.

# --hints--

사용자가 프롬프트를 확인했는지 검사하는 `if` 문장을 만들어야 합니다. `isCartCleared`의 진릿값을 직접 확인할 수 있다는 점을 기억하세요.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

`if` 문장은 `items` 속성을 빈 배열로 다시 설정해야 합니다.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

`if` 문장은 `total` 속성을 `0`로 설정해야 합니다.
