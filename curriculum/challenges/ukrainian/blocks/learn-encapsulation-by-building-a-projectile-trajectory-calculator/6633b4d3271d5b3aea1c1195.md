...
Тепер настав час для графіка. Створіть метод `create_trajectory` і замініть останній виклик `print` у кінці вашого коду на `print(graph.create_trajectory())`.

Першим кроком цього нового методу зробіть локальну копію координат, але де всі значення округлені до цілих чисел. Збережіть цю нову версію координат у змінній з ім’ям `rounded_coords` і поверніть цю змінну.

# --hints--

Ви повинні визначити метод з ім’ям `create_trajectory`.
...
Ви повинні замінити `print(graph.create_coordinates_table())` на `print(graph.create_trajectory())`.
...
Функція має повертати `rounded_coords`.
...
Змінна `rounded_coords` має містити округлені координати.
