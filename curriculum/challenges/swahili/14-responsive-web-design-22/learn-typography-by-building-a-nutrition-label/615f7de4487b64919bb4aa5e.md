...
Baada ya `.divider` yako ya mwisho, unda kipengele kipya cha `p` chenye maandishi `Cholesterol 0mg 0%`. Then, wrap the text `Cholesterol` in a `span` element, `0%` in another, and give each of them a class of `bold`.
