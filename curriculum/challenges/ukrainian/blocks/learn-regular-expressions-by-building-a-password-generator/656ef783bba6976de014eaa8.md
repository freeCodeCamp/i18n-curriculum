...
Помістіть ваше оголошення змінної `password` та наступний цикл `for` всередину циклу `while`. Використовуйте `True` як умову для нового циклу.

# --hints--

Вам слід створити цикл `while True`, який охоплюватиме ваше існуюче оголошення `password` та цикл `for`.
