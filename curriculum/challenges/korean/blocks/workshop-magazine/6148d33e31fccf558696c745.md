...
인용문 텍스트를 더 돋보이게 하려면 `.quote` 선택자에 `font-family` 속성을 `Raleway`로 설정하고 대체 수단으로 `sans-serif`을 지정하세요.

# --hints--

`.quote` 선택자에 `font-family` 속성을 `Raleway`로 설정하고 대체 수단으로 `sans-serif`을 지정하세요.
