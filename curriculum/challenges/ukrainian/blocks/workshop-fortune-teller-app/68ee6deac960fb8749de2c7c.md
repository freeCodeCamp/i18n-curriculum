...
Додайте решту аргументів `chosenCard.name_short` і `chosenCard.img`. Потім поза межами функції `renderCard` скиньте `multipleCard` елемента `innerHTML` до порожнього рядка.

# --hints--

Ви повинні передати `chosenCard.name_short` як аргумент у `renderCard`.
...
Ви повинні передати `chosenCard.img` як останній аргумент у `renderCard`.
...
Ви повинні скинути `this.elements.multipleCard.innerHTML` до порожнього рядка.
