...
داخل `map()`، أضف جملة `return` مع علامات backticks حيث ستُدرج كل العناصر المسؤولة عن عرض تفاصيل الأغنية.

داخل علامات backticks، أنشئ عنصر `li` مع خاصية `id` بقيمة `song-${song.id}` وخاصية `class` بقيمة `playlist-song`.

# --hints--

يجب أن تضيف جملة `return` مع زوج فارغ من علامات backticks.

```js
assert.match(code, /return\s*`/)
```

يجب أن تنشئ وسم `li` داخل علامات backticks.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

يجب أن يحتوي وسم `li` الافتتاحي على خاصية `id` مضبوطة على `song-${song.id}`.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

يجب أن يحتوي وسم `li` الافتتاحي على خاصية `class` مضبوطة على `playlist-song`.
