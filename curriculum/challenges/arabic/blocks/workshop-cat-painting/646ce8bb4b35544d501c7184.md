...
داخل العنصر `.cat-ears`، أنشئ عنصرين `div` يحملان الصنفين `cat-left-ear` و `cat-right-ear` على التوالي.

# --hints--

لا تغير عنصر `div` الموجود الذي يحمل الصنف `cat-ears`.

```js
assert.lengthOf(document.querySelectorAll('div.cat-ears'), 1);
```

أنشئ عنصرين `div` داخل العنصر `.cat-ears`.

```js
assert.lengthOf(document.querySelectorAll('.cat-ears div'), 2);
```

يجب أن يحمل أول عنصر `div` الصنف `cat-left-ear`.

```js
assert.isTrue(document.querySelectorAll('.cat-ears div')[0]?.classList.contains('cat-left-ear'));
```

يجب أن يحمل ثاني عنصر `div` الصنف `cat-right-ear`.
