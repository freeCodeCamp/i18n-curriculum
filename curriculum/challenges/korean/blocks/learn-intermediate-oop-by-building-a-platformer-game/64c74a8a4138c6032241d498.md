...
속성 이름과 값이 같을 때는 단축 속성 구문을 사용할 수 있습니다. 이 구문을 사용하면 속성 값이 속성 이름과 같을 경우 값을 생략할 수 있습니다.
...
생성자 안에 `this.position`를 추가하고 `x`과 `y` 좌표를 가진 객체를 할당하세요. 단축 속성 구문을 사용해야 합니다.

# --hints--

`this.position` 속성이 있어야 합니다.

```js
assert.match(Platform.toString(), /this\.position\s*;?/);
```

`this.position` 속성은 `x`와 `y` 좌표를 가진 객체여야 합니다. 단축 속성 구문을 사용하세요.
