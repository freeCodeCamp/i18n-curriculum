...
Ni muhimu kuona mpaka chaguo-msingi karibu na vipengele vya `fieldset`, wakati wa uundaji. Walakini, inaweza kuwa sio mtindo unaotaka.

Ondoa mpaka na sehemu ya chini kwenye vipengele vya `.question`.

# --hints--

Unapaswa kutumia kichaguzi cha `.question`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

Unapaswa kuipa `.question` `border` ya `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

Unapaswa kuipa `.question` `padding-bottom` ya `0`.
