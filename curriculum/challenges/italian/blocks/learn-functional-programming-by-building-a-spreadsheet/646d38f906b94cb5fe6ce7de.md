...
La tua `addCharacters(char1)` restituisce anche una funzione, che a sua volta restituisce un'altra funzione. Devi effettuare un'altra chiamata di funzione per consultare quel riferimento alla funzione più interna per la callback `.map()`. JavaScript ti permette di invocare immediatamente le funzioni restituite:

```js
myFunc(1)("hi");
```

Invoca immediatamente la funzione restituita dalla tua chiamata `addCharacters(char1)` e passa `char2` come argomento.

# --hints--

Dovresti concatenare una chiamata di funzione alla tua chiamata `addCharacters(char1)`.
...
Dovresti passare `char2` come argomento alla tua chiamata di funzione concatenata.
