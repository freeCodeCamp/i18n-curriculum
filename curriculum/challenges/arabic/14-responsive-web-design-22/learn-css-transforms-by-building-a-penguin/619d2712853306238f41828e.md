...
يجب عليك إعطاء عنصر `:root` خاصية الـ `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

خاصية `--penguin-skin` يجب أن تحمل قيمة `gray`، ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

يجب عليك إعطاء `.penguin-head` خاصية `background` من`linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
يجب أن يحتوي عنصر الزائف `.penguin-body::before` على خاصية `background-color` بقيمة `var(--penguin-skin)`, ولكن وجدت بقيمة `--fcc-actual--`.
...
يجب عليك إعطاء `.arm` خاصية `background` من`linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
