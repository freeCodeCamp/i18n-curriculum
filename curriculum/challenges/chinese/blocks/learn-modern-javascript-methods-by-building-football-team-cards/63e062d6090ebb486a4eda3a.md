...
向你的 `setPlayerCards` 函数添加一个名为 `arr` 的新参数，并为其分配一个默认值 `players`。 请记住，你在 `175` 行从 `myFavoriteFootballTeam` 对象中解构了 `players` 变量。

# --hints--

你的 `setPlayerCards` 函数应该采用 `arr` 参数。

```js
assert.match(code, /const\s+setPlayerCards\s*=\s*\(\s*arr/);
```

你的 `arr` 参数应该有一个默认值 `players`。
