...
次は、**Listenerプラン**の要素のスタイリングに進みましょう。

プランの外側の`div`に、`bg-gray-100`の周りに太いアウトラインを作るために`ring-1`、`div`のクラスを付け、さらにそのアウトラインに暗い色を付けるために`ring-gray-300`のクラスを付けてください。

# --hints--

`div`要素には`bg-gray-100`のクラスを付けてください。
...
`div`要素には`ring-gray-300`のクラスを付けてください。
