...
在嵌套的 `for` 循環之後，你找到了最小的值。 您需要將其與當前值交換。

就像你在冒泡排序中所做的那樣，使用 `temp` 變量提取 `array[i]` 的值，然後交換 `array[i]` 和 `array[minIndex]` 的值。

# --hints--

在嵌套的 `for` 循環之後，你應聲明一個 `temp` 變量。
...
你應該將 `array[minIndex]` 分配給 `array[i]`。
