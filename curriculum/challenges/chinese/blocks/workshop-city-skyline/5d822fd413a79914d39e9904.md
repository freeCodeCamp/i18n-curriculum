...
到目前为止，你创建的所有渐变都是从上到下，这是默认方向。 你可以像下面这样通过在颜色之前添加方向来指定另一个方向：
...
给 `.bb3` 添加 `repeating-linear-gradient`。 将方向设置为 `90deg`，`building-color3` 作为前两种颜色，在 `15%` 处采用 `window-color3` 作为第三种颜色。  When you don't specify a distance for a color, it will use the values that make sense. 在这个例子中，前两种颜色将默认为在 `0%` 处和 `7.5%` 处，因为它从 `0%` 开始，而 `7.5%` 是 `15%` 的一半。

# --hints--

你应该将`.bb3` 的`background`设置为 `repeating-linear-gradient`。
...
你应该在 `15%` 处使用 `--window-color3` 作为第三种颜色。
