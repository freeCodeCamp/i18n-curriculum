...
이제 같은 방법으로 오른쪽 눈을 작업하세요.

클래스 선택자를 사용해 `.cat-right-eye` 요소에 너비 `30px`와 높이 `40px`를 지정하세요. 또한 배경색을 `#000`로 설정하세요.

# --hints--

`.cat-right-eye` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-right-eye'))
```

`.cat-right-eye` 선택자에 `width`가 `30px`로 설정되어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.width, '30px')
```

`.cat-right-eye` 선택자에 `height`가 `40px`로 설정되어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.height, '40px')
```

`.cat-right-eye` 선택자에 `background-color`가 `#000`로 설정되어야 합니다.
