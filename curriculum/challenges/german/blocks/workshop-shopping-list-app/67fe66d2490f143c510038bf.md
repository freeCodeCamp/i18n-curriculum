...
Fügen Sie dann im Kontrollkästchen `input` ein `checked`-Attribut hinzu und setzen Sie es auf `isChecked`. Obwohl es nicht zwingend erforderlich ist, das `checked`-Attribut zu setzen, ist es eine gute Praxis, um sicherzustellen, dass jedes Kontrollkästchen den aktuellen Zustand des `selectedItems`-Arrays widerspiegelt.

# --hints--

Sie sollten dem Kontrollkästchen `checked` ein `input`-Attribut hinzufügen.
...
Sie sollten das `checked`-Attribut auf `isChecked` setzen.
