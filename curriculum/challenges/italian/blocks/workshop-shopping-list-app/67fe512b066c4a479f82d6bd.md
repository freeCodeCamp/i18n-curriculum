...
All'interno dell'elemento `li`, sopra il testo `item`, aggiungi un elemento `input` con l'attributo `type` impostato su `checkbox`. Inoltre, imposta il suo attributo `onChange` su una funzione anonima che chiama `toggleItem()` con `item` come argomento.

Infine, racchiudi `input` e il testo `item` in un elemento `label` per una migliore accessibilità.

# --hints--

Dovresti aggiungere un altro elemento `input` con l'attributo `type` impostato su `checkbox`.
...
Il tuo nuovo `input` dovrebbe avere un attributo `onChange` impostato su una funzione anonima che chiama `toggleItem()` con `item` come argomento.
...
Il tuo nuovo `input` e il testo `item` dovrebbero essere racchiusi in un elemento `label`.
