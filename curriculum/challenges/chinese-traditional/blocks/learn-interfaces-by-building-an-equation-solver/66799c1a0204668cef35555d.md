...
f-strings 也讓你可以使用 `.nf` 格式說明符，為你的數值資料設定特定的精確度，其中 `n` 是要顯示的小數位數。

在 `result_list` 內包含的 f-string 的大括號中，撰寫在 `:+` 後顯示 3 位小數所需的格式說明符。

# --hints--

你應該將 `case 1` 區塊中包含的 `result_list` 字串修改為 `f'x = {results[0]:+.3f}'`。
...
你應該修改 `case 2` 區塊中 `result_list` 所包含的字串，使結果以 `3` 位小數顯示。
