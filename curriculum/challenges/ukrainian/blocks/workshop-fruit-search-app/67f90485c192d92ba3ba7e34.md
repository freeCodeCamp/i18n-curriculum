...
Всередині функції зворотного виклику `useEffect` додайте умову `if`, щоб перевірити, чи `query.trim() === ''`. Це перевіряє, чи є `query` порожнім рядком або містить лише пробіли. Якщо так, викличте `setResults([])` і поверніться раніше.

# --hints--

Ви повинні перевірити, чи `query.trim() === ''`.
...
Якщо `query.trim() === ''` є істинним, викличте `setResults()` з порожнім масивом, а потім використайте ключове слово `return` після цього.
