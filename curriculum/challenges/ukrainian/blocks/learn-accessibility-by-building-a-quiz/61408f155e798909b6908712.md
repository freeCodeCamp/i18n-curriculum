...
Щоб увімкнути навігацію на сторінці, додайте невпорядкований список із трьома такими елементами списку:

- `INFO`
- `HTML`
- `CSS`

Текст елементів списку має бути обгорнутий у теги посилань.

# --hints--

Ви повинні вкласти один елемент `ul` всередину `nav`.

```js
assert.equal(document.querySelectorAll('nav > ul')?.length, 1);
```

Ви повинні вкласти три елементи `li` всередину елемента `ul`.

```js
assert.equal(document.querySelectorAll('nav > ul > li')?.length, 3);
```

Ви повинні вкласти один елемент `a` всередину кожного елемента `li`.
