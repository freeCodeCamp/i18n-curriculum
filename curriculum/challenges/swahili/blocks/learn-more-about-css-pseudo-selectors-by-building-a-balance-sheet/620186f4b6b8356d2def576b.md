...
Ifuatayo unahitaji kupamba vipengele vyako vya `caption` ili viweze kuonekana zaidi kama vichwa. Unda kichaguzi cha `table caption`. Weka maandishi ya rangi ya `#356eaf`, ukubwa wa `1.3em`, na uzito wa kawaida.

# --hints--

Unapaswa kuwa na kichaguzi cha `table caption`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption'));
```

Kichaguzi chako cha `table caption` kinapaswa kuwa na sifa ya `color` iliyowekwa kuwa `#356eaf`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('color') === 'rgb(53, 110, 175)');
```

Kichaguzi chako cha `table caption` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `1.3em`.
