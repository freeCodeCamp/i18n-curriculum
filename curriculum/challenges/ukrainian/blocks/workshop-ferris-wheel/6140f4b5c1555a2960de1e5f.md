...
Створіть ще одне правило `@keyframes` з іменем `cabins`. Використайте ті самі властивості, що й у вашому `@keyframes wheel`, скопіювавши правила `0%` та `100%`, але встановіть властивість `transform` селектора `100%` у значення `rotate(-360deg)`.
...
Нове правило `@keyframes` має називатися `cabins`.
...
Нове правило `@keyframes` має йти після правила `@keyframes wheel`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.name, 'cabins');
```

Не слід змінювати ім’я правила `@keyframes wheel`.
