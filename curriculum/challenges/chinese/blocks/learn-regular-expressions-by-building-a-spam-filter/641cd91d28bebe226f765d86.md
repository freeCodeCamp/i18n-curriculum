...
将事件监听器附加到你的 `checkMessageButton`，监听 `click` 事件。 给它一个空的回调函数。

# --hints--

你应该在你的 `checkMessageButton` 元素上调用 `.addEventListener()`。

```js
assert.match(code, /checkMessageButton\.addEventListener\(/);
```

你的 `.addEventListener()` 方法应该使用 `"click"` 事件类型。

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,/);
```

你的 `.addEventListener()` 方法应该有一个空的回调函数。
