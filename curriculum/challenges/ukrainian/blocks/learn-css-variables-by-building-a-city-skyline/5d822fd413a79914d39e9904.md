...
До цього моменту всі градієнти, які ви створювали, йшли зверху вниз — це напрямок за замовчуванням. Ви можете вказати інший напрямок, додавши його перед кольорами ось так:
...
Заповніть `.bb3` `repeating-linear-gradient`. Використайте `90deg` для напрямку, ваш `building-color3` для перших двох кольорів і `window-color3` на `15%` для третього.

Якщо ви не вказуєте відстань для кольору, буде використано значення, які мають сенс. У цьому випадку перші два кольори за замовчуванням будуть `0%` і `7.5%`, бо градієнт починається з `0%`, а `7.5%` — це половина `15%`, тому їх не потрібно встановлювати.

# --hints--

Ви повинні надати `.bb3` `background` за допомогою `repeating-linear-gradient`.
...
Ви не повинні встановлювати відсоток для першого кольору.

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle(".bb3")?.getPropVal('background', true), "var(--building-color3)0%");
```

Ви не повинні встановлювати відсоток для другого кольору.
