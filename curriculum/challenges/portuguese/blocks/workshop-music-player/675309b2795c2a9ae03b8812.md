...
Observe que a arte do álbum no HTML e as músicas no array `userData.songs` foram alteradas.Trocamos as músicas originais por versões mais curtas que você pode usar para testar seu app.

Todas as funcionalidades principais estão agora no lugar.O único problema agora é que a próxima música não toca automaticamente quando a música que está tocando termina. 

Para corrigir isso, você pode configurar um listener de evento que detectará quando a música que está tocando terminar.O listener de evento `"ended"` é apropriado para isso.Ele é disparado quando a reprodução de uma mídia chega ao fim.

Adicione um event listener ao elemento `audio` que escute o evento `"ended"` e passe uma referência para a função `playNextSong`.Com isso o projeto do player de música está completo!

# --hints--

Você deve encadear o método `addEventListener()` à sua variável `audio`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

O listener de evento que você usou na sua variável `audio` deve escutar um evento `"ended"`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

Você deve passar uma referência para `playNextSong` no seu listener de evento `"ended"`.
