...
title: "Usa l'assegnazione destrutturante per assegnare variabili da oggetti annidati"
challengeType: 1
forumTopicId: 301214
...
Puoi usare gli stessi principi delle due lezioni precedenti per destrutturare valori da oggetti annidati.
...
Ecco come estrarre i valori delle proprietà di un oggetto e assegnarli a variabili con lo stesso nome:

```js
const { johnDoe: { age, email }} = user;
```

Ecco invece come puoi assegnare i valori delle proprietà di un oggetto a variabili con nomi diversi:
...
Sostituisci le due assegnazioni con un'assegnazione destrutturante equivalente. Dovrebbe comunque assegnare alle variabili `lowToday` e `highToday` i valori di `today.low` e `today.high` dall'oggetto `LOCAL_FORECAST`.
