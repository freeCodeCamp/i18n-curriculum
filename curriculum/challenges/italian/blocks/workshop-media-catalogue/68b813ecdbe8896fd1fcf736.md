...
Usa il metodo `add` di `catalogue` per aggiungere `series1` alla lista `items`.

Poi, crea un'altra variabile `series2`. Assegna un'altra istanza di `TVSeries` e aggiungi anche `series2` al catalogo.

# --hints--

Dovresti usare il metodo `add` di `catalogue` passando `series1`.
...
Dovresti creare una variabile chiamata `series2` e assegnarle un'istanza di `TVSeries`.
...
Dovresti usare il metodo `add` di `catalogue` passando `series2`.
