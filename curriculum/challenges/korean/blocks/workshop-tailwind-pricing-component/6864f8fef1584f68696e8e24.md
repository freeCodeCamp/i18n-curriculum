...
이제 `Start listening` CTA로 이동하여 제대로 된 버튼으로 만드세요.

유틸리티 클래스 `bg-indigo-100`, `px-4`, `py-2`, `text-center`, `font-semibold`, `text-indigo-700`, `hover:bg-indigo-200`를 할당하세요.

# --hints--

앵커 요소에 `bg-indigo-100` 클래스를 추가하세요.
...
앵커 요소에 `font-semibold` 클래스를 추가하세요.
...
앵커 요소에 `text-indigo-700` 클래스를 추가하세요.
...
앵커 요소에 `hover:bg-indigo-200` 클래스를 추가하세요.
