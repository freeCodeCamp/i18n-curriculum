...
如果 `fetch` 呼叫發生錯誤，`catch` 區塊會控制代碼。 

在 `catch` 區塊中，為你的 `console.log` 添加用來記錄 `err` 參數。

此外，既然你已經了解從 `fetch` 呼叫傳回的是什麼，現在請從你的 `try` 區塊中移除 `console.log(data);`。

# --hints--

你應該使用 `console.log` 來記錄 `err` 變數。

```js
assert.match(code, /\s*console\.log\(\s*err\s*\)/);
```

你的 `console.log` 應該在 `catch` 區塊內。

```js
assert.match(code, /\s*try\s*{[^}]*\s*}\s*catch\s*\(\s*err\s*\)\s*{[^}]*\s*console\.log\(\s*err\s*\)[^}]*}/);
```

你應該從 `try` 區塊中移除 `console.log(data);`。
