...
曲がページに表示されるときは、各曲のタイトル、アーティスト、再生時間、および削除ボタンが表示される必要があります。

次のいくつかのステップでは、<dfn>map()</dfn> メソッドを使って各曲の新しいHTMLを追加する方法を学びます。このメソッドについては次のステップで詳しく説明します。

まず、`const` を使って `songsHTML` という名前の変数を宣言し、`array.map()` を代入してください。

# --hints--

`const` という変数 `songsHTML` を用意してください。

```js
assert.match(code, /const\s+songsHTML/);
```

`array.map()` を `songsHTML` に代入してください。
