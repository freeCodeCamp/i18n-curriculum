...
Потім додайте виклик `print()` всередині тіла `if`, передаючи наступний f-рядок: `f'Solved puzzle:\n{gameboard}'`.


# --hints--

Ви повинні створити інструкцію `if`, яка перевіряє, чи виклик методу `solver()` з об’єкта `gameboard` повертає `True`.
...
Ви повинні вивести `f'Solved puzzle:\n{gameboard}'` всередині нового тіла `if`.
