...
Crie um loop `for` para iterar sobre `targets_to_print` e imprimir a seguinte f-string: `f'\n{start}-{node} distance: {distances[node]}\nPath: {" -> ".join(paths[node])}'`.

# --hints--

Você deve criar um loop `for` para iterar sobre `targets_to_print`. Use `node` como a variável do loop.
...
Você deve imprimir a string fornecida dentro do seu novo loop `for`.
