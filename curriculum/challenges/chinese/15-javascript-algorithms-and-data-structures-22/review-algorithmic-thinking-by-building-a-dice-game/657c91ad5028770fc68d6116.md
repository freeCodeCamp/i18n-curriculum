...
当用户点击 `Roll the dice` 按钮时，应该生成五个随机骰子数字并显示在屏幕上。

构建逻辑，使得单击 `rollDiceBtn` 会生成 1 到 6 之间的五个随机数，将 `diceValuesArr` 设置为仅包含这五个数字，并在 `listOfAllDice` 元素中按顺序显示这些数字。

# --hints--

当你的 `rollDiceBtn` 被点击时，你的 `diceValuesArr` 数组应该包含五个元素。
...
当第二次单击 `rollDiceBtn` 时，`diceValuesArr` 数组应该只包含五个新元素。
...
每个 `.die` 元素都应包含来自 `diceValuesArr` 数组的相应值。
