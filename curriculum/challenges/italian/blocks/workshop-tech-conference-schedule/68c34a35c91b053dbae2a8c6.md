...
Un altro valore per l'attributo `scope` è `row`, che indica che una cella di intestazione è un'intestazione per l'intera riga.

All'interno del tuo elemento `th`, aggiungi un attributo `scope` con valore `row`.

# --hints--

Il tuo elemento `th` dovrebbe avere un attributo `scope` con valore `row`.
