...
Atualmente, tanto filmes quanto séries de TV são exibidos todos juntos quando você imprime o catálogo. Nos próximos passos, você cuidará de filtrar os itens em duas categorias para exibi-los separadamente.

Dentro da classe `MediaCatalogue`, crie um método chamado `get_movies` e faça com que ele retorne uma lista contendo apenas instâncias da classe pai `Movie` que podem ser encontradas em `self.items`.

Use a abordagem que preferir para fazer isso, mas lembre-se de usar a função `type` já que `isinstance` não será capaz de discriminar entre classes pai e filho.

# --hints--

Sua classe `MediaCatalogue` deve ter um método chamado `get_movies`.
...
Seu método `get_movies` deve retornar uma lista contendo todos os itens em `self.items` que são instâncias da classe `Movie`.
