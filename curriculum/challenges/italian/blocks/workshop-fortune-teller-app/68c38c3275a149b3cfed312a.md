...
In questo workshop continuerai a esercitarti con TypeScript costruendo un'app di lettura della fortuna.

Inizia dichiarando un'interfaccia chiamata `Card`. All'interno dell'interfaccia `Card`, aggiungi le seguenti proprietà: `name`, `name_short` e `value`. Le proprietà `name` e `name_short` devono avere i tipi impostati su `string` mentre `value` deve avere un tipo unione impostato su `string | number`.

# --hints--

Dovresti dichiarare un'interfaccia chiamata `Card`.
...
La tua interfaccia `Card` dovrebbe avere una proprietà `name` di tipo `string`.
...
La tua interfaccia `Card` dovrebbe avere una proprietà `name_short` di tipo `string`.
...
La tua interfaccia `Card` dovrebbe avere una proprietà `value` con un tipo unione di `string` e `number`.
