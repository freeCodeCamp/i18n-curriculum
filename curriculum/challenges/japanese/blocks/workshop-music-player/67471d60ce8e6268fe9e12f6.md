...
このワークショップでは、プレイリストから曲を再生、一時停止、スキップできるミュージックプレーヤーアプリを作成します。

このプロジェクトのHTMLとCSSは用意されているので、JavaScriptにフォーカスしてください。

まず、`#playlist-songs`、`#play`、`#pause`の要素にアクセスし、それぞれ`playlistSongs`、`playButton`、`pauseButton`という変数に代入してください。
...
`#playlist-songs`の要素を`playlistSongs`という変数に代入してください。
...
`#play`の要素を`playButton`という変数に代入してください。
...
`#pause`の要素を`pauseButton`という変数に代入してください。
