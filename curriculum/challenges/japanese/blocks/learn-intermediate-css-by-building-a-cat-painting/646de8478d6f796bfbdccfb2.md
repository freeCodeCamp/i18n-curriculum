...
右目を目のように見せるために、`60%` のボーダー半径を与えてください。また、`transform` プロパティを使って、`-25` 度回転させてください。

# --hints--

`.cat-right-eye` セレクターには、`border-radius` プロパティを `60%` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.borderRadius === '60%')
```

`.cat-right-eye` セレクターには、`transform` プロパティを `rotate(-25deg)` に設定してください。
