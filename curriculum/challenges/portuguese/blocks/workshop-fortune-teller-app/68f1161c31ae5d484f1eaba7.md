...
Agora adicione outro método chamado `showFortune`. Como parâmetro, passe `e` e dê a ele o tipo `Event`. Também crie um método `newReading` e deixe-o vazio por enquanto.

# --hints--

Você deve declarar um método chamado `showFortune`.
...
Seu método `showFortune` deve ter um parâmetro `e` com o tipo `Event`.
...
Você deve declarar um método chamado `newReading`.
