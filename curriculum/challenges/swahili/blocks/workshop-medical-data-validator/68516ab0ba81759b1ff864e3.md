...
Kitendakazi cha `all` hurejesha `True` ikiwa vipengele vyote kutoka kwenye iterable iliyotumwa kwako ni vya kweli, na `False` vinginevyo:
...
Tuma orodha ya `[isinstance(i, str) for i in medications]` kwa kitendakazi cha `all` ili kuhakikisha kila kipengele ndani yake ni mfuatano wa herufi.

# --hints--

Unapaswa kubadilisha thamani ya ufunguo wako wa `medications` kwa kutuma `[isinstance(i, str) for i in medications]` kwa kitendakazi cha `all`.
