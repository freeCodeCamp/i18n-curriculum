...
`onChange` 속성을 추가하세요. 이 속성은 `e`을 매개변수로 받고, `setPlayer`을 펼친 것과 `player`를 `position`로 설정한 값을 `e.target.value`로 형변환하여 `Position`를 호출하는 화살표 함수여야 합니다.

# --hints--

`select` 위치에는 `onChange` 핸들러가 있어야 하며, 이 핸들러는 `setPlayer`을 펼쳐서 `player`를 호출하고 `position`을 업데이트해야 합니다.
