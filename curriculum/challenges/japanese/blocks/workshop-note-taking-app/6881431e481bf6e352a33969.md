...
前に説明したように、ユーザーがノートを編集してからノートの外をクリックすると、ノートが保存されたことを示すメッセージが表示される必要があります。

これを実現するには、`statusEl.textContent` を文字列 `"Note saved successfully!"` に設定してください。

これでテストすると、そのメッセージが画面に表示されるはずです。

# --hints--

ユーザーがノートを編集したとき、ステータスメッセージはノートが保存されたことを示すように更新される必要があります。
