...
Ongeza kitu cha tatu chenye sifa na thamani zifuatazo:
...
Safu yako ya `allSongs` inapaswa kuwa na kitu kama thamani yake ya tatu.

```js
assert.isObject(allSongs[2]);
```

Kitu cha tatu katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `id` iliyowekwa kwa nambari `2`.

```js
assert.equal(allSongs[2].id, 2);
```

Kitu cha tatu katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `title` iliyowekwa kwa mfuatano wa herufi `"Still Learning"`.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

Kitu cha tatu katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `artist` iliyowekwa kwa mfuatano wa herufi `"Quincy Larson"`.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

Kitu cha tatu katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `duration` iliyowekwa kwa mfuatano wa herufi `"3:51"`.

```js
assert.equal(allSongs[2].duration, "3:51");
```

Kitu cha tatu katika safu yako ya `allSongs` kinapaswa kuwa na sifa ya `src` iliyowekwa kwa mfuatano wa herufi `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`.
