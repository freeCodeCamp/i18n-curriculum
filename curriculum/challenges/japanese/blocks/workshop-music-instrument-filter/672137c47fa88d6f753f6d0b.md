...
現在、`instrumentCards` 関数は楽器オブジェクトの配列を返しているので、楽器カードをページに表示するにはもう少しステップが必要です。

関数を修正して、楽器カードを表示するためのHTMLコードを含む文字列の配列を返すようにしてください。各文字列は`instruments` 配列のオブジェクトに対応し、文字列はこの形式`<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`である必要があります。
...
`instrumentCards("all")` は楽器カードを表す文字列の配列を返すべきで、それぞれの形式は`<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`です。
...
`instrumentCards("woodwinds")` は楽器カードを表す文字列の配列を返すべきで、それぞれの形式は`<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`です。
...
`instrumentCards("brass")` は楽器カードを表す文字列の配列を返すべきで、それぞれの形式は`<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`です。
...
`instrumentCards("percussion")` は楽器カードを表す文字列の配列を返すべきで、それぞれの形式は`<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`です。
...
`instrumentCards("all")` はページ上のすべての楽器の楽器カードを表す10個の文字列の配列を返すべきです。
...
`instrumentCards("woodwinds")` はページ上の木管楽器の楽器カードを表す3個の文字列の配列を返すべきです。
...
`instrumentCards("brass")` はページ上の金管楽器の楽器カードを表す3個の文字列の配列を返すべきです。
...
`instrumentCards("percussion")` はページ上の打楽器の楽器カードを表す4個の文字列の配列を返すべきです。
