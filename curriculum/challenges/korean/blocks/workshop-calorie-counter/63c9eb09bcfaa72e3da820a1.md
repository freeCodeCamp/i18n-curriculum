...
모든 입력 컨테이너를 가져와야 합니다. `inputContainers` 변수를 선언하고, 클래스가 `input-container`인 모든 요소를 쿼리한 값으로 할당하세요.

# --hints--

`inputContainers` 함수 안에 `clearForm` 변수를 선언해야 합니다.

```js
assert.match(clearForm.toString(), /inputContainers\s*=/);
```

클래스가 `querySelectorAll`인 모든 요소를 가져오려면 `input-container` 메서드를 사용하세요.

```js
assert.match(clearForm.toString(), /document\.querySelectorAll\(\s*('|")\.input-container\1\s*\)/);
```

`querySelectorAll` 변수에 `inputContainers` 메서드의 값을 할당해야 합니다.
