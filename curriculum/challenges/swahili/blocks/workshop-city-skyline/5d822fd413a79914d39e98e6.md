...
Panga ndani vipengele sita vya `div` ndani ya `.foreground-buildings` na uwape madarasa ya `fb1` hadi `fb6` kwa mpangilio huo. "fb" inasimama kwa "jengo la mbele". Hivi vitakuwa majengo sita zaidi ya mbele.

# --hints--

Unapaswa kuunda `div` mpya yenye darasa la `fb1`.

```js
assert.exists(document.querySelector('div.fb1'));
```

Unapaswa kuunda `div` mpya yenye darasa la `fb2`.

```js
assert.exists(document.querySelector('div.fb2'));
```

Unapaswa kuunda `div` mpya yenye darasa la `fb3`.

```js
assert.exists(document.querySelector('div.fb3'));
```

Unapaswa kuunda `div` mpya yenye darasa la `fb4`.

```js
assert.exists(document.querySelector('div.fb4'));
```

Unapaswa kuunda `div` mpya yenye darasa la `fb5`.

```js
assert.exists(document.querySelector('div.fb5'));
```

Unapaswa kuunda `div` mpya yenye darasa la `fb6`.

```js
assert.exists(document.querySelector('div.fb6'));
```

Unapaswa kuweka vipengele hivi vipya vya `div` ndani ya kipengele cha `.foreground-buildings`.
