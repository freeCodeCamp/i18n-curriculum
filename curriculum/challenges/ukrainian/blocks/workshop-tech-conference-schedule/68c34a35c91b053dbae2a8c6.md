...
Інше значення для атрибута `scope` — це `row`, яке вказує, що клітинка заголовка є заголовком для всього свого рядка.

Всередині вашого елемента `th` додайте атрибут `scope` зі значенням `row`.

# --hints--

Ваш елемент `th` має мати атрибут `scope` зі значенням `row`.
