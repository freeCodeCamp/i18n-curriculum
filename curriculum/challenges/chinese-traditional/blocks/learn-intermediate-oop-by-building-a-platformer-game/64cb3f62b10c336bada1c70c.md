...
在 `animate` 函式中，你需要將每個 `checkpoints` 繪製到畫布上。

為你的程式碼添加一個 `forEach` 迴圈，迭代 `checkpoints` 陣列。

在回呼函式中，為你的回呼函式添加一個 `checkpoint` 參數，並在函式主體中對每個 `checkpoint` 呼叫 `draw` 方法。

# --hints--

你應該有一個 `forEach` 迴圈來迭代 `checkpoints` 陣列。

```js
assert.match(code, /\bcheckpoints\b\s*\.\s*forEach\s*\(/s);
```

你應該在 `forEach` 回呼函式內有一個 `checkpoint` 參數。

```js
assert.match(code, /\s*checkpoints\s*\.\s*forEach\s*\(\s*(\(\s*checkpoint\s*\)|checkpoint)\s*=>\s*{?/s);
```

你應該在每個 `checkpoint` 上呼叫 `draw` 方法，並將其放在 `forEach` 回呼函式內。
