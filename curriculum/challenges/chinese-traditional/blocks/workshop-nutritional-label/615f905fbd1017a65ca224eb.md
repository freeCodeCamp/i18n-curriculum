...
在你的 `.daily-value` 元素後添加一個中等分隔線。在該新的分隔線下方，創建一個 `p` 元素，並將 `class` 屬性設為 `note`。
