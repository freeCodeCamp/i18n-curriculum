...
Tornando al tuo event listener, devi aggiornare il testo dell'elemento `result`. Puoi usare un operatore ternario per svolgere questa attività.
...
Dopo l'istruzione `if`, usa un operatore ternario per verificare la veridicità della chiamata a `isSpam()` con `messageInput.value` come argomento. Se è vero, imposta la proprietà `textContent` sull'elemento `result` a `"Oh no! This looks like a spam message."`. Altrimenti, impostala a `"This message does not seem to contain any spam."`.
...
Dovresti chiamare la funzione `isSpam()` dopo l'operatore di assegnazione `=` e prima dell'operatore ternario `?`.
...
Dovresti usare la sintassi ternaria per verificare la veridicità di `isSpam(messageInput.value)`.
...
L'espressione vera del tuo ternario dovrebbe impostare la proprietà `textContent` dell'elemento `result` a `"Oh no! This looks like a spam message."`.
...
L'espressione falsa del tuo ternario dovrebbe impostare la proprietà `textContent` dell'elemento `result` a `"This message does not seem to contain any spam."`.
...
Dopo il tuo ternario, imposta la proprietà `value` sull'elemento `messageInput` a una stringa vuota.
