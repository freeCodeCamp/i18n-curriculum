...
Deklariere eine `rangeFromString`-Arrow-Funktion, die zwei Parameter verwendet: `num1` und `num2`. Die Funktion sollte implizit das Ergebnis des Aufrufs von `range` mit `num1` und `num2` als Argumente zurückgeben.

Um sicher zu gehen, solltest du `num1` und `num2` in ganze Zahlen zerlegen, wenn du sie an `range` übergibst.

# --hints--

Du solltest eine `rangeFromString`-Variable nach deinem `rangeRegex` deklarieren.
...
Du solltest `const` verwenden, um deine `rangeFromString`-Variable zu deklarieren.
...
Deine `rangeFromString`-Variable sollte eine Funktion sein.
...
Deine `rangeFromString`-Funktion sollte `num1` als ersten Parameter enthalten.
...
Deine `rangeFromString`-Funktion sollte `num2` als zweiten Parameter enthalten.
...
Deine `rangeFromString`-Funktion sollte eine implizite Rückgabe verwenden.
...
Deine `rangeFromString`-Funktion sollte das Ergebnis des Aufrufs deiner `range`-Funktion zurückgeben.
...
Du solltest `parseInt` mit `num1` als Argument aufrufen und das Ergebnis dem `range`-Aufruf übergeben.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

Du solltest `parseInt` mit `num2` als Argument aufrufen und das Ergebnis dem `range`-Aufruf übergeben.
