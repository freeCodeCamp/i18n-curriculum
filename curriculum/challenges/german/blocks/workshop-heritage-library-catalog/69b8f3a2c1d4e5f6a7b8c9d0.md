...
`parseCatalog` gibt derzeit ein leeres Array zurück. Um es zu füllen, fügen Sie eine `for`-Schleife innerhalb der Funktion hinzu, die `rawCards` durchläuft. Rufen Sie innerhalb der Schleife `parseCard` für jedes Element auf und fügen Sie das Ergebnis in `catalog` ein.

# --hints--

`parseCatalog(rawCatalogCards).length` sollte `19` sein.

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

`parseCatalog` sollte eine `for`-Schleife verwenden, um `rawCards` zu durchlaufen.

```js
assert.match(parseCatalog.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*rawCards\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Sie sollten `parseCard` innerhalb der Schleife aufrufen.
