...
Você deve dar aos seus botões de opção personalizados um tamanho e formato.

Ainda direcionando `.contact-method-radio-btn`, defina `width` para `20px`, `height` para `20px`, `border-radius` para `50%` e `border` para `2px solid gray`.
...
Defina a propriedade `width` para `20px`.
...
Defina a propriedade `height` para `20px`.
...
Defina a propriedade `border` para `2px solid gray`.
