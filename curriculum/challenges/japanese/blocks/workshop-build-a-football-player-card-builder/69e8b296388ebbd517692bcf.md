...
`div` の `className` の `form-row` を追加してください。その中に、`div` の `className` を持つ `form-group` 要素を2つ追加します。

最初の `div` 要素には、`label` の `className` の `label`、`htmlFor` の `position` 属性、テキストの `Position`、および `input` の `id`、`position` の `className`、`input` の `type` を持つ `text` が必要です。

# --hints--

Player Info セクションには、`div` の `className` を持つ `form-row` が含まれている必要があります。
...
`div.form-row` には、`div` の `className` を持つ `form-group` 要素が2つ含まれている必要があります。
...
行の最初の `form-group` には、`label` の `className` を持つ `label` と、`htmlFor` の `position` が必要です。
...
位置の `label` には、テキスト `Position` が必要です。
...
行の最初の `form-group` には、`input` の `className` を持つ `input`、`id` の `position`、および `type` の `text` が必要です。
