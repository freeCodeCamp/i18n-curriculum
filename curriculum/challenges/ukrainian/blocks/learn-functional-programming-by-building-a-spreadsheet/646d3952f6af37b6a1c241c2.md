...
Тепер, коли функція `.map()` отримує повернене посилання на функцію `num => charRange(...).map(...)` від карруваних викликів `addCharacters`, вона належним чином буде ітерувати по елементах та передасть кожен елемент як `n` до цієї функції.

Ви помітите, що не ви не використаєте свій параметр `match`. У JavaScript це загальна умовність додавати перед невикористаним параметром знак підкреслення `_`. Ви також можете залишити параметр порожнім, як от: `(, char1)`, але часто зрозуміліше надати ім’я параметру для читання у майбутньому.

Префіксніти свій параметр `match` підкресленням.

# --hints--

Ви повинні префікснути свій параметр `match` підкресленням.
