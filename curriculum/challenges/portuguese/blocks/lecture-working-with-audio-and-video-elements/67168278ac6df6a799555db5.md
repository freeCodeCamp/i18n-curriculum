...
Para incluir conteúdo de áudio na sua página web, você pode usar o elemento `audio` com o atributo `src` apontando para o local do seu arquivo de áudio:

```html
<audio src="CrystalizeThatInnerChild.mp3"></audio>
```

Se você tentar executar este exemplo, verá que nada aparece na página.No entanto, se você inspecionar a página com as ferramentas de desenvolvedor, verá que o elemento `audio` está de fato na página.Se você quiser ver o player de áudio na página, então você pode adicionar o elemento `audio` com o atributo `controls`:
...
Além dos atributos `src` e `controls`, existem vários outros atributos que aprimoram a funcionalidade do elemento `audio`.O atributo `loop` é um atributo booleano que faz o áudio ser reproduzido continuamente.Aqui está um exemplo de uso do atributo `loop` para tocar uma das músicas de Quincy Larson intitulada "Can't stay down":
...
Quando a música chegar ao fim, ela voltará a tocar novamente desde o início.Outro atributo que você pode usar é o atributo `muted`.Quando presente no elemento `audio`, este atributo booleano iniciará o áudio em estado mudo.Aqui está um exemplo de uso do atributo `muted`:
...
Quando você iniciar a música no navegador, você não ouvirá nada.Para ouvir a música você precisará clicar no ícone de volume.
...
Este exemplo também está usando o atributo `width` para definir a largura em 620 pixels para que o vídeo se ajuste melhor na tela.
