...
`addCharacters` devuelve una función, así que querrás llamarla. Pasa `char1` como argumento.

# --hints--

Debe llamar a su función `addCharacters()` en su método `.map()`.
...
Debes pasar `char1` como argumento a tu llamada `addCharacters()`.
