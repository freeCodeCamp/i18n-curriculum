...
为你的代码添加一个 `console.log(currentContent);`。 

要测试你的更改，尝试编辑该笔记然后点击笔记外部。你现在应该能在控制台看到编辑后的笔记。 

# --hints--

你应该在事件监听器内部，在将 `newContent` 赋值给 `currentContent` 之后，添加 `console.log(currentContent);`。
