...
`He's` can be a short form for `he has` or `he is`. You can understand which one it is by looking at the context. When `he's` is followed by a verb in the past participle (like `been`, `done`, `seen`), it means `he has`. 例えば、次のように使います。
