...
- **Usando o depurador embutido do Python (`pdb`)**: Python fornece um módulo `pdb` para depuração. Ele faz parte da biblioteca padrão do Python, então está sempre disponível para uso. Com o `pdb`, você pode definir um trace com a função `set_trace()` para começar a executar o código passo a passo e inspecionar variáveis de forma interativa.
