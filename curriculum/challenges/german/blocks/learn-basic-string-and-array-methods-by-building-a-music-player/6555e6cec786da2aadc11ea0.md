...
Sie sollten die Songs auch erneut rendern, das aktuell abgespielte Lied pausieren, die Anzeige des Players einstellen und den zugänglichen Text des Wiedergabe-Buttons erneut setzen.

Rufen Sie die `renderSongs`-Funktion auf und übergeben Sie `userData?.songs` als Argument. Rufen Sie außerdem die `pauseSong`-, `setPlayerDisplay`- und `setPlayButtonAccessibleText`-Funktionen auf.

# --hints--

Sie sollten die bestehende `shuffle`-Funktion und deren Inhalt nicht verändern.
