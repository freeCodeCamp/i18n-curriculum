...
إذا نظرت إلى HTML، سترى أن اللوحين الثاني والثالث مخفيان. يُنجز ذلك باستخدام الخاصية `hidden`. اللوح الأول ظاهر افتراضيًا. عندما ينقر المستخدم على تبويب، يجب إخفاء كل اللوحات.

استخدم الطريقة `forEach` للتكرار على `panels` واضبط الخاصية `hidden` على كل لوح لتصبح `true`.

# --hints--

يجب أن تتكرر على `panels` وتضبط الخاصية `hidden` لكل لوح لتصبح `true`.
