...
Genau wie bei einer regulären Funktion können Sie Argumente an eine Pfeilfunktion übergeben.
...
Hat eine Pfeilfunktion nur einen einzelnen Parameter, können die runden Klammern um den Parameter weggelassen werden.
...
Schreiben Sie die `myConcat`-Funktion, die den Inhalt von `arr2` an `arr1` anhängt, so um, dass die Funktion die Syntax einer Pfeilfunktion verwendet.
