...
Щоб контейнер виділявся, слід додати множину `box-shadow` зі значенням `0 5px 15px black`.

Це додає тінь навколо контейнера. Значення відповідно контролюють горизонтальне зміщення, вертикальне зміщення, радіус розмиття та колір.

# --hints--

Слід націлитися на клас `.container`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.container'));
```

Слід встановити властивість `box-shadow` у значення `0 5px 15px black`.
