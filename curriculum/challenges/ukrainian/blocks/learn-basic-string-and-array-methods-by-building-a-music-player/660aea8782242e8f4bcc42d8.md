...
Як і звичайні функції, стрілкові функції можуть приймати кілька параметрів.

Ось приклад іменованої стрілкової функції з одним параметром:
...
Якщо функція має лише один параметр, можна опустити круглі дужки навколо списку параметрів ось так:
...
Створіть нову іменовану стрілкову функцію з назвою `printMessage`, яка має один параметр з ім’ям `org`. Усередині тіла цієї функції додайте інструкцію console. У цій інструкції console використайте шаблонний рядок `${org} is awesome!`.

Під вашою функцією `printMessage` викличте цю функцію, передавши рядок `"freeCodeCamp"` як аргумент.

Відкрийте консоль, щоб побачити результат.

# --hints--

Ви повинні мати функцію `printMessage`.

```js
assert.isFunction(printMessage);
```

Використайте синтаксис стрілкової функції.

```js
assert.match(code, /(let|const)\s+printMessage\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Ваша функція `printMessage` має мати параметр з ім’ям `org`.
...
Ваша функція `printMessage` має містити інструкцію console.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

Ваша інструкція console має включати шаблонний рядок `${org} is awesome!`.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

Ви повинні викликати функцію `printMessage` і передати рядок `"freeCodeCamp"`.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

Виклик функції має бути розташований під визначенням функції `printMessage`.
...
Виклик функції `printMessage` має вивести в консоль `freeCodeCamp is awesome!`.
