...
title: 트윗 임베드에서 flex-direction 속성으로 열 만들기
challengeType: 0
...
트윗 임베드 `header`와 `footer`는 이전에 `flex-direction` 속성을 row 값으로 사용했습니다. 마찬가지로 `.profile-name` 요소 안의 항목들은 열로 쌓으면 잘 작동할 것입니다.

# --instructions--

헤더의 `flex-direction` 요소에 CSS 속성 `.profile-name`를 추가하고 값을 `column`으로 설정하세요.
...
`.profile-name` 요소에 `flex-direction` 속성을 `column`으로 설정해야 합니다.
