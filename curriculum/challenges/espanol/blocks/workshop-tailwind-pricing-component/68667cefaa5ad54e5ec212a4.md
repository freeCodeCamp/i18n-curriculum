...
Pasando al elemento de la lista, agrega las clases `mt-6`, `space-y-2`, `text-sm` y `text-fuchsia-100` al `ul` para espaciar los elementos de la lista y hacer que se vean más atractivos.

# --hints--

Tu elemento `ul` debe tener la clase `mt-6`.
...
Tu elemento `ul` debe tener la clase `space-y-2`.
...
Tu elemento `ul` debe tener la clase `text-sm`.
...
Tu elemento `ul` debe tener la clase `text-fuchsia-100`.
