...
Agora, vamos estilizar a terceira `section`.Observe que ela tem os valores de `text` e `text-with-images` para o atributo `class`, o que significa que já está herdando os estilos da regra `.text`.

Crie um seletor `.text-with-images` e defina a propriedade `display` como `grid`.

# --hints--

Você deve ter um seletor `.text-with-images`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

O seletor `.text-with-images` deve ter a propriedade `display` definida como `grid`.
