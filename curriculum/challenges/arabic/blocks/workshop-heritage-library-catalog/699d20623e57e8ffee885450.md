...
تمت إضافة بقية بطاقات الكتالوج. لاحظ الثلاث مدخلات قرب النهاية التي تحتوي على حقول مفقودة؛ ستتعامل معها في خطوة لاحقة.

ستأخذ الدالة `parseCard` سلسلة كتالوج خام وتُرجع كائنًا منظمًا بأربع خصائص: `title` و`author` و`year` و`location`. في الوقت الحالي، أنشئ دالة `parseCard` بمعلمة اسمها `rawString`.

# --hints--

يجب أن يكون لديك دالة باسم `parseCard`.

```js
assert.isFunction(parseCard);
```

يجب أن تكون `parseCard` دالة تحتوي على معلمة اسمها `rawString`.
