...
O elemento `address` não precisa conter um local geográfico físico. Ele pode ser utilizado para fornecer um link para o assunto.

Envolva o texto `freeCodeCamp` usando um link e defina sua localização como `https://freecodecamp.org`.

# --hints--

Você deve adicionar um elemento `a`.

```js
assert.exists(document.querySelector('address > a'));
```

Você deve dar ao elemento `a` um atributo `href` com o valor `https://freecodecamp.org`.

```js
assert.equal(document.querySelector('address > a')?.getAttribute('href'), 'https://freecodecamp.org');
```

Você deve aplicar o elemento `a` em torno do texto `freeCodeCamp`.
