...
أعلن دالة `toNumberList` التي تأخذ معلمة `args` وتُرجع ضمنيًا نتيجة تقسيم `args` حسب الفواصل. ثم اربط طريقة `map` بطريقة `split` ومرر `parseFloat` كمعلمة لطريقة `map`.

# --hints--

يجب إعلان متغير `toNumberList`.
...
يجب استخدام `const` لإعلان متغير `toNumberList`.
...
يجب أن يكون متغير `toNumberList` دالة سهمية.
...
يجب أن تحتوي دالة `toNumberList` على معلمة `args`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

يجب أن تستخدم دالة `toNumberList` إرجاعًا ضمنيًا.
...
يجب أن تُرجع دالة `toNumberList` نتيجة استدعاء طريقة `.split()` على `args`.
...
يجب ربط طريقة `.map()` بطريقة `.split()`.
...
يجب تمرير مرجع إلى `parseFloat` كاستدعاء راجع لطريقة `.map()`.
