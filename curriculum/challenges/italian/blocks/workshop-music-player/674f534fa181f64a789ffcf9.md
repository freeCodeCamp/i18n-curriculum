...
Se `userData.currentSong` non è `null` devi trovare la canzone successiva da riprodurre. Ricorda che puoi usare `getNextSong` per questo.

Completa la tua funzione `playNextSong` in modo che riproduca la canzone successiva nella playlist. Se la canzone corrente è l’ultima, imposta `userData.currentSong` su `null`, `userData.songCurrentTime` a zero e chiama la funzione `pauseSong`.

# --hints--

Quando `currentSong` di `userData` è truthy e non è l’ultima canzone della playlist, la funzione `playNextSong` dovrebbe riprodurre la canzone successiva a quella corrente.
...
Quando `currentSong` di `userData` è l’ultima canzone della playlist, la funzione `playNextSong` dovrebbe impostare `userData.currentSong` su `null`.
...
Quando `currentSong` di `userData` è l’ultima canzone della playlist, la funzione `playNextSong` dovrebbe impostare `userData.songCurrentTime` su `0`.
...
Quando `currentSong` di `userData` è l’ultima canzone della playlist, la funzione `playNextSong` dovrebbe chiamare la funzione `pauseSong`.
...
Quando `currentSong` di `userData` è uguale a `null`, la tua funzione `playNextSong` dovrebbe comunque chiamare la funzione `playSong` con `id` della prima canzone nell’array `userData.songs` come argomento.
