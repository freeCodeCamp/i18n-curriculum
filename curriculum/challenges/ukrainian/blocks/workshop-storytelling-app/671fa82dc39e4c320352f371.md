...
Щоб зберігати різні типи історій, створіть об’єкт з назвою `storyObj`. Об’єкт має містити три властивості: `scary`, `funny` та `adventure`. Значення кожної властивості має бути об’єктом із властивостями `story` та `borderColor`.

Призначте наступні значення окремим об’єктам:
...
У вас має бути об’єкт `storyObj`.

```js
assert.isObject(storyObj);
```

Ваш об’єкт `storyObj` має містити три властивості: `scary`, `funny` та `adventure`.

```js
assert.containsAllKeys(storyObj, ['scary', 'funny', 'adventure']);
```

Кожна властивість має містити властивості `story` та `borderColor`.
...
Властивість `story` для `scary` має мати рядкове значення `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.
...
Властивість `borderColor` для `scary` має мати рядкове значення `"#ee4b2b"`.

```js
assert.equal(storyObj.scary.borderColor, "#ee4b2b");
```

Властивість `story` для `funny` має мати рядкове значення `"During a camping trip, Mark decided to show off his culinary skills by cooking dinner over an open fire. However, his attempt caused him to burn the dinner as well as his eyebrows off."`.
...
Властивість `borderColor` для `funny` має мати рядкове значення `"#f1be32"`.

```js
assert.equal(storyObj.funny.borderColor, "#f1be32");
```

Властивість `story` для `adventure` має мати рядкове значення `"Lost in the heart of the Amazon rain forest, Sarah and Jake stumbled upon an ancient temple. They braved deadly traps and encountered strange wildlife, all while deciphering cryptic clues left behind by a mysterious civilization."`.
...
Властивість `borderColor` для `adventure` має мати рядкове значення `"#acd157"`.
