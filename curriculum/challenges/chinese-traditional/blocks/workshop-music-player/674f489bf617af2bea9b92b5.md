...
現在，創建一個名為 `getNextSong` 的函式（程式）。讓它傳回在 `userData.songs` 中位於 `getCurrentSongIndex()` 加 1 的歌曲，或者如果目前的歌曲是播放列表中的最後一首，則傳回 `undefined`。

# --hints--

你應該有一個名為 `getNextSong` 的函式（程式）。

```js
assert.isFunction(getNextSong);
```

你的 `getNextSong` 函式應該呼叫 `getCurrentSongIndex`。
...
你的 `getNextSong` 函式（程式）應該傳回當前歌曲的下一首歌曲。
...
當目前歌曲是播放列表中的最後一首時，你的 `getNextSong` 函式應該傳回 `undefined`。
