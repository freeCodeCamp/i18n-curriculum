...
title: Спроєктуйте сторінку вибору функції
challengeType: 25
...
1. На вашій сторінці має бути елемент `h1` з текстом `Feature Selection`.
2. На вашій сторінці має бути елемент `div` з класом `feature-card-container`.
3. На вашій сторінці має бути щонайменше два елементи `label`, кожен з класом `feature-card`, всередині `div`.
4. Кожен із ваших елементів `label` має містити текст мітки та елемент `input` типу `checkbox`.
5. Створіть селектор, який націлюється на прапорці, і застосуйте такі стилі:
    - Всі ваші елементи прапорців мають бути встановлені в `appearance: none;` у вашому CSS.
    - Всі ваші прапорці мають мати кордон із шириною, кольором і стилем на ваш вибір.
6. Коли прапорець вибрано, у центрі прапорця має відображатися галочка `✓`.
7. Коли прапорець вибрано, колір фону прапорця має змінюватися на колір на ваш вибір.
8. Коли прапорець вибрано, колір кордону прапорця має змінюватися на колір на ваш вибір.

**Примітка:** Обов’язково підключіть ваш CSS-файл у HTML.
...
Ваш `h1` має містити текст `Feature Selection`.

```js
assert.equal(document.querySelector('h1').innerText.trim(), `Feature Selection`);
```

У вас має бути елемент `div` з класом `feature-card-container`.

```js
assert.exists(document.querySelector('div.feature-card-container'));
```

У вас має бути щонайменше два елементи `label` всередині `.feature-card-container`.
...
Кожен елемент `label` всередині `.feature-card-container` має мати клас `feature-card`.
...
Всередині кожного елемента `label` має бути текст мітки.
...
Всередині кожного елемента label має бути один елемент `input` типу `checkbox`.
...
У вас має бути селектор для прапорців.
...
Ваші прапорці мають бути встановлені в `appearance: none`.
...
Ваші прапорці мають мати кордон із шириною на ваш вибір.
...
Ваші прапорці мають мати кордон із кольором на ваш вибір.
...
Ваші прапорці мають мати кордон із стилем на ваш вибір.
...
Коли прапорці вибрано, у центрі прапорця має бути галочка `✓`.
...
Коли прапорець вибрано, колір фону прапорця має змінюватися на колір на ваш вибір.
...
Коли прапорець вибрано, колір кордону прапорця має змінюватися на колір на ваш вибір.
