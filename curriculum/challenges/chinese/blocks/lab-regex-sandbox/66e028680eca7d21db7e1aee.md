...
1. 你应该有一个名为 `getFlags` 的函数，该函数返回一个包含已选中标记复选框标记的字串，如果没有选中任何标记复选框，则返回空字串。
1. 当你点击 `testButton` 按钮时，如果正则表达式模式匹配测试字串，匹配的文本应被高亮显示。为高亮匹配项，更新 `stringToTest` 的 `innerHTML`，使每个匹配项被一个带有 `highlight` 类的 `span` 元素包围。注意相关 CSS 已经为你添加好了。
1. 当有匹配时，匹配的文本应显示在 `#result` 内。如果有多个匹配，每个匹配文本之间用逗号和空格分隔。
1. 当没有匹配时，`#result` 内应显示文本 `no match`。
