...
Asigne al selector `.nota` un relleno izquierdo y derecho de `8px`, eliminando el relleno superior e inferior. También establezca la propiedad `text-indent` a `-8px`.

Con estos últimos cambios, ¡su etiqueta de nutrición está completa!

# --hints--

El selector `.nota` debe tener una propiedad `padding` establecida a `0 8px`.
...
El selector `.nota` debe tener una propiedad `text-indent` establecida a `-8px`.
