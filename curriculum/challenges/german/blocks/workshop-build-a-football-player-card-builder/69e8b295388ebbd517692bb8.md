...
Unter dem `div`-Element mit dem `className` von `card-tier-badge` fügen Sie ein `div` mit `className` von `card-club` und dem Text `Santos FC` hinzu.

# --hints--

Sie sollten ein `div` mit dem `className` von `card-club` unter `.card-tier-badge` hinzufügen.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

Das `.card-club`-Element sollte den Text `Santos FC` enthalten.
