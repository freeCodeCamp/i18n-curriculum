...
Kommen wir zum letzten `fieldset`. Was, wenn Sie einem Benutzer erlauben wollten, ein Profilbild hochzuladen?

Nun, der `input`-Typ `file` ermöglicht genau das. Fügen Sie ein `label` mit dem Text `Upload a profile picture: ` hinzu und verschachteln Sie ein `input`, das einen Datei-Upload akzeptiert.

# --hints--

Sie sollten ein `label` mit dem Text `Upload a profile picture: ` hinzufügen.

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label')?.innerText, /Upload a profile picture:/i);
```

Sie sollten ein `input`-Element innerhalb des `label`-Elements verschachteln.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label > input'));
```

Sie sollten dem `input`-Element ein `type` von `file` geben.
