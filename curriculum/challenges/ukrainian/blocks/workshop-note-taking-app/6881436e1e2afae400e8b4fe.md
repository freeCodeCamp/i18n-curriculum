...
Зараз, щоразу коли користувач редагує нотатку і натискає поза межами нотатки, змінна `currentContent` оновлюється. Але що робити у випадках, коли користувач фокусується на нотатці і вирішує вийти, не вносячи жодних змін?

У такій ситуації ви можете додати умову, щоб обробити цей випадок.

Понад рядком `currentContent = newContent;` додайте інструкцію `if`, щоб перевірити, чи дорівнює `currentContent` `newContent`. Якщо так, поверніться.

# --hints--

Ви повинні додати інструкцію `if`, щоб перевірити, чи дорівнює `currentContent` `newContent`. Якщо так, поверніться.
