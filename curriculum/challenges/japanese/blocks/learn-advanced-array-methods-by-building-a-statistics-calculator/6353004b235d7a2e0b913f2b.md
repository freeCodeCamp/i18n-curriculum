...
根指数、例えば $\sqrt[n]{x}$ を計算するには、逆数の指数 $x^{1/n}$ を使うことができます。JavaScript には指数を計算するための組み込み `Math.pow()` 関数があります。

`Math.pow()` 関数の基本的な構文は次の通りです：
...
`standardDeviation` 変数を宣言し、`Math.pow()` 関数を使って $variance^{1/2}$ の値を代入してください。

# --hints--

`standardDeviation` 変数を用意してください。

```js
assert.match(getStandardDeviation.toString(), /standardDeviation\s*=/);
```

`standardDeviation` 変数は `Math.pow()` 関数を使う必要があります。

```js
assert.match(getStandardDeviation.toString(), /standardDeviation\s*=\s*Math\.pow\(/);
```

`Math.pow()` 関数は `variance` を底、`1/2` を指数にしてください。
