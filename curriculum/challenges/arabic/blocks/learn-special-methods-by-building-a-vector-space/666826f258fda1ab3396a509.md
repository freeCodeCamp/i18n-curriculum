...
الآن، اطبع نتيجة وصول الخاصية `x` في `v1` باستخدام عامل النقطة ودالة `getattr()` المدمجة.

سترى أن `__getattribute__` يُستدعى في الحالتين.

# --hints--

يجب أن تطبع الخاصية `x` في `v1` باستخدام عامل النقطة ودالة `getattr()`.
