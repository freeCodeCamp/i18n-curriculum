...
دالتك `addCharacters` تُرجع في النهاية نطاقًا من الأحرف. تريدها أن تُرجع مصفوفة من معرفات الخلايا. اربط طريقة `.map()` باستدعاء `charRange()` الخاص بك. لا تمرر دالة استدعاء راجع بعد.

# --hints--

يجب أن تربط `.map()` باستدعاء `charRange()` الخاص بك.
...
لا يجب أن تمرر أي شيء إلى استدعاء `.map()` الخاص بك.
