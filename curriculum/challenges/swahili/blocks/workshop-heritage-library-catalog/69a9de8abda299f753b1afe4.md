...
Ongeza mzunguko wa `for` juu ya `catalog`. Ndani ya mzunguko, tangaza `const entry = catalog[i]`. Tengeneza mfuatano wa mstari wenye sehemu nne zilizoachwa kwa koma na uweke ndani ya `rows`.

Sehemu za mfuatano wa herufi zinahitaji alama za nukuu maradufu halisi katika matokeo (mfano `"King, Stephen"`), ili koma zilizo ndani ya thamani zisivunje muundo wa CSV. Kwa kuwa `year` ni nambari, haizihitaji.
...
Kila mstari unapaswa kuwa na sehemu za kipengee zimetenganishwa kwa koma.

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

Sehemu za mfuatano wa herufi zinapaswa kufunikwa na alama za nukuu maradufu.
