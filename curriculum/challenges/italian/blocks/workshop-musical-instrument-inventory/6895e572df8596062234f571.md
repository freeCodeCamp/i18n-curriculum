...
Infine, fai lo stesso per `instrument_2`. Chiama il metodo `play()` su `instrument_2`, e poi stampa il risultato della chiamata a `get_fact()` su `instrument_2`.

Questo completerà il tuo workshop di strumenti musicali!

# --hints--

Dovresti chiamare il metodo `play()` sull'istanza `instrument_2`.
...
Dovresti chiamare `get_fact()` su `instrument_2` e stampare il risultato.
