...
أخيرًا، ستحتاج إلى إرجاع الصورة الخاصة بصورة المستخدم الرمزية.

ابدأ بإضافة `return` متبوعة بمجموعة من القوالب الحرفية. داخل القوالب الحرفية، أضف عنصر `img`.

داخل وسم `img`، أضف خاصية `src` بقيمة `${userAvatarUrl}`. ولخاصية `alt`، أضف قيمة `${user.name}`.
...
يجب أن يحتوي القالب الحرفي على عنصر `img`.
...
يجب أن يحتوي وسم `img` على خاصية `src`.
...
يجب تعيين قيمة خاصية `src` إلى الثابت المسمى `userAvatarUrl`.
...
يجب أن تضيف خاصية `alt` بعد خاصية `src`.
...
يجب تعيين قيمة خاصية `alt` إلى الثابت المسمى `user.name`.
