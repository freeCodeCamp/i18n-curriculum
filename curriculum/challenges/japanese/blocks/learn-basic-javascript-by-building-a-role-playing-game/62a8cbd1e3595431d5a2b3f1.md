...
プレイヤーがどの武器を買ったかを伝える必要があります。先ほど書いた2行の間に、`let`を使って`newWeapon`という新しい変数を初期化してください。これを`weapons`と等しく設定します。

# --hints--

あなたの`buyWeapon`関数には`newWeapon`という名前の変数が必要です。
...
`newWeapon`は`weapons`の値で初期化する必要があります。セミコロンを忘れないでください。

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

`newWeapon`は`text`を変更する前に宣言してください。
