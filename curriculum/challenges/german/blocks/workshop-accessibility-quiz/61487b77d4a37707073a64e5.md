...
Wenn die Bildschirmbreite klein ist, wird der Textinhalt des `h1` nicht wie vorgesehen umbrochen. Richten Sie den Text des `h1`-Elements zentriert aus.

Geben Sie dann dem `main` einen Innenabstand, sodass die Kopfzeile des `Student Info`-Abschnitts vollständig sichtbar ist.

# --hints--

Sie sollten dem `h1` einen `text-align` von `center` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

Sie sollten einen `main`-Selektor hinzufügen, um das `main`-Element anzusprechen.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Sie sollten dem `main` ein `padding-top` von mindestens `25px` geben.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

Sie sollten nur den Wert von `padding-top` ändern.
