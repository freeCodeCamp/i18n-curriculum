...
Antes de continuar, remova as chamadas `console.log` do seu código.

# --hints--

Você não deve ter `console.log(removePlant(ballerina))` no seu código.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*removePlant\s*\(\s*ballerina\s*\)\s*\)/);
```

Você não deve ter `console.log(sellPlants(ballerina, "small", 10));` no seu código.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

Você não deve ter `console.log(catalog)` no seu código.
