...
Kuwasilisha fomu ukiwa umechagua chaguo hakutatumia thamani yenye maana kwa seva. Kwa sababu hiyo, kila kipengele cha `option` kinahitaji sifa ya kipengele ya `value`. Bila sifa hii ya kipengele, maudhui ya maandishi ya `option` yatawasilishwa kwa seva.

Toa `option` ya `value` kwa `""` ya kwanza, na sifa za kipengele za `option` kwa vipengele vya `value` vinavyofuata kutoka `1` hadi `4`.

# --hints--

Unapaswa kutoa `option` ya `value` kwa `""` ya kwanza.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

Unapaswa kutoa `option` ya `value` kwa `1` ya pili.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

Unapaswa kutoa `option` ya `value` kwa `2` ya tatu.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

Unapaswa kutoa `option` ya `value` kwa `3` ya nne.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

Unapaswa kutoa `option` ya `value` kwa `4` ya tano.
