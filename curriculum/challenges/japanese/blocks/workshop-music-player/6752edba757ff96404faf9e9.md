...
`setPlayButtonAccessibleText` という名前の関数を作成してください。関数内で、再生ボタンの `aria-label` 属性を `Play` に続けてスペースと現在の曲のタイトルに設定してください。もし `userData.currentSong` が `null` なら、`Play` に設定してください。

# --hints--

`setPlayButtonAccessibleText` という名前の関数があるはずです。

```js
assert.isFunction(setPlayButtonAccessibleText);
```

`setPlayButtonAccessibleText` 関数は、再生ボタンの `aria-label` 属性を `Play` に続けてスペースと現在の曲のタイトルに設定する必要があります。
...
```
 ...
 `userData.currentSong` が `null` の場合、`setPlayButtonAccessibleText` は再生ボタンの `aria-label` 属性を `Play` に設定する必要があります。
