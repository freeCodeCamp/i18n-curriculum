...
تمامًا مثل الدوال العادية، يمكن للدوال السهمية قبول عدة معلمات.

إليك مثال على دالة سهمية مسماة تحتوي على معلمة واحدة:
...
إذا كانت الدالة تحتوي على معلمة واحدة فقط، يمكنك حذف الأقواس العادية حول قائمة المعلمات هكذا:
...
أنشئ دالة سهمية مسماة جديدة باسم `printMessage` تحتوي على معلمة واحدة تسمى `org`. داخل جسم تلك الدالة، أضف بيانًا في وحدة التحكم. داخل ذلك البيان، أضف النص القالب `${org} is awesome!`.

أسفل دالتك `printMessage`، استدعِ الدالة ومرر السلسلة النصية `"freeCodeCamp"` كمعلمة.

افتح وحدة التحكم لترى النتيجة.

# --hints--

يجب أن يكون لديك دالة `printMessage`.
...
يجب أن تحتوي دالة `printMessage` على معلمة تسمى `org`.
...
يجب أن تحتوي دالة `printMessage` على بيان في وحدة التحكم.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

يجب أن يتضمن بيان وحدة التحكم النص القالب `${org} is awesome!`.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

يجب أن تستدعي دالة `printMessage` وتمرر السلسلة النصية `"freeCodeCamp"`.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

يجب أن يكون استدعاء الدالة أسفل تعريف دالة `printMessage`.
...
يجب أن يعرض استدعاء دالة `printMessage` النص `freeCodeCamp is awesome!` في وحدة التحكم.
