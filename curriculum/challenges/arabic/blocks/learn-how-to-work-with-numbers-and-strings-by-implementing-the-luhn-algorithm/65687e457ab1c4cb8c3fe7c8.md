...
عدّل استدعاء `verify_card_number` بحيث إذا أعاد `True`، اطبع `'VALID!'` في وحدة التحكم. وإلا، اطبع `'INVALID!'`.

# --hints--

يجب أن يحتوي الدالة `main` على `if verify_card_number(translated_card_number):`.
...
يجب أن يحتوي البند `if` على `print('VALID!')`.
...
يجب أن يحتوي البند `else` على `print('INVALID!')`.
