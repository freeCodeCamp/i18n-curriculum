...
对于递归情况，添加 `else` 语句并 `return` 调用 `decimalToBinary()` 的结果。 传入 `input` 除以 `2` 并使用 `Math.floor()` 四舍五入作为参数。

# --hints--

你应该将 `if` 语句转换为 `if...else` 语句。

```js
assert.match(String(decimalToBinary), /\{\s*return\s+('|"|`)\1\s*;?\s*\}\s*else\s*\{/);
```

在 `else` 语句主体中，你应该 `return` 调用 `decimalToBinary` 的结果。

```js
assert.match(String(decimalToBinary), /else\s*\{\s*return\s+decimalToBinary\(/);
```

你应该将 `input` 除以 `2` 并使用 `Math.floor()` 向下舍入的结果作为 `decimalToBinary` 的参数传入。
