...
Ti servirà una colonna per il testo e una colonna per le immagini. Dai al selettore `.text-with-images` una proprietà `grid-template-columns` impostata su `1fr 2fr`. Imposta anche la proprietà `column-gap` su `3rem` per creare più spazio tra le colonne.

# --hints--

Il tuo selettore `.text-with-images` dovrebbe avere una proprietà `grid-template-columns` impostata su `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

Il tuo selettore `.text-with-images` dovrebbe avere una proprietà `column-gap` impostata su `3rem`.
