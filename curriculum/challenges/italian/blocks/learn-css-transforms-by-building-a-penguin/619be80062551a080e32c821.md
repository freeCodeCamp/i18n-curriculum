...
Arrotonda la cresta, dando ai bordi inferiori del pseudo-elemento un raggio di `100%`, lasciando i bordi superiori a `0%`.

# --hints--

Dovresti usare la proprietà `border-radius` per arrotondare la cresta.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

Dovresti dare a `.penguin-body::before` una `border-radius` di `0% 0% 100% 100%`.
