...
在先前的專案中，你學會了如何使用串接運算子將變數插入到字串中，如下所示：
...
將字串 `"You now have a new weapon."` 更新為 `"You now have a "`，後面接新武器的名稱，並記得以句號結尾。


# --hints--

你應該更新 `text.innerText` 指派，使其以字串 `"You now have a "` 開頭。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1/);
```

你應該使用串接運算子將 `newWeapon` 添加到 `text.innerText` 字串的結尾。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1\s*\+\s*newWeapon/);
```

你應該使用串接運算子來讓你的 `text.innerText` 字串以 `.` 結尾。
