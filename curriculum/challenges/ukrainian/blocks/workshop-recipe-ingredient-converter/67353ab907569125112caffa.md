...
Тепер настав час використовувати свої функції. Оголосіть нову `processIngredient` функцію, яка приймає чотири аргументи: `baseQuantity`, `baseUnit`, `newUnit` і `newServings`.

Ваша функція має відкоригувати порції для базової кількості до нових порцій, конвертувати цю відкориговану кількість з базової одиниці у нову одиницю та повернути конвертовану кількість у вигляді рядка з двома десятковими знаками.

Якщо застрягнете, зверніться до прикладів викликів функції, які ви записали. Ви можете змінювати значення в обох цих прикладах, щоб дослідити.

# --hints--

Ви повинні оголосити `processIngredient` функцію.

```js
assert.isFunction(processIngredient);
```

Ваша `processIngredient` функція має мати `baseQuantity` як перший параметр.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity/);
```

Ваша `processIngredient` функція має мати `baseUnit` як другий параметр.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit/);
```

Ваша `processIngredient` функція має мати `newUnit` як третій параметр.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit/);
```

Ваша `processIngredient` функція має мати `newServings` як четвертий параметр.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit,\s*newServings\)/);
```

Ваша `processIngredient` функція має повертати рядок.

```js
assert.isString(processIngredient(10, "gram", "cup", 5));
```

Ваша `processIngredient` функція має повертати правильне значення.
