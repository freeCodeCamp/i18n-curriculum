...
La sezione Player Info è ora completa. Dopo la chiusura di `div` della sezione Player Info e ancora all'interno di `form-panel`, aggiungi un nuovo `div`. Al suo interno, aggiungi un elemento `p` con il `className` di `form-section-title` e il testo `Player Stats`. Poi aggiungi un `div` con il `className` di `stats-grid`. Lascia vuota per ora la griglia delle statistiche.

# --hints--

Il tuo `div.form-panel` dovrebbe contenere un secondo `div` per la sezione Player Stats.
...
La sezione Player Stats dovrebbe contenere un `p` con il `className` di `form-section-title` e il testo `Player Stats`.
...
La sezione Player Stats dovrebbe contenere un `div` con il `className` di `stats-grid`.
