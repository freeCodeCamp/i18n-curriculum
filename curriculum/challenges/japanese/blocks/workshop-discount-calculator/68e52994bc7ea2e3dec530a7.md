...
ワークショップを完了するには、f-string を使って小数点以下2桁で最良価格を表示する最後の出力文を追加してください。フォーマットは `Best price for [product.name] for [user_tier] user: $[best_price]` で、`[product.name]`、`[user_tier]`、`[best_price]` は実際の変数の値に置き換えてください。

出力例は `Best price for Wireless Buds for Premium user: $93.00` のようになります。価格が小数点以下2桁であることに注意してください。

これで割引計算ワークショップは完了です！

# --hints--

出力は `Best price for {product.name} for {user_tier} user: ${best_price:.2f}` のフォーマットに一致している必要があります。
