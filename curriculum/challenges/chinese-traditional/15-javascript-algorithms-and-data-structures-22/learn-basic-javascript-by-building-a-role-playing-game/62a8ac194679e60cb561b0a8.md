...
爲空的 `button text` 數組提供三個字符串元素。 在 `goTown` 函數中使用分配給按鈕 `innerText` 屬性的三個字符串。 請記住，數組值以逗號分隔。

# --hints--

你的 `button text` 數組應該包含三個元素。

```js
assert.lengthOf(locations[0]["button text"], 3);
```

你的 `button text` 數組應該有三個字符串。
...
`button text` 數組中的第一個值應該是 `"Go to store"`。

```js
assert.equal(locations[0]["button text"][0], "Go to store");
```

`button text` 數組中的第二個值應該是 `"Go to cave"`。

```js
assert.equal(locations[0]["button text"][1], "Go to cave");
```

`button text` 數組中的第三個值應該是 `"Fight dragon"`。
