...
Agora é hora de exibir os dados na página.

Comece criando uma função arrow chamada `showLatestPosts`, que recebe um único parâmetro `data`.

# --hints--

Você deve ter uma função arrow chamada `showLatestPosts`.

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

Sua função `showLatestPosts` deve ter um parâmetro `data`.

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

`showLatestPosts` deve ser definido.

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts` deve ser uma função.
