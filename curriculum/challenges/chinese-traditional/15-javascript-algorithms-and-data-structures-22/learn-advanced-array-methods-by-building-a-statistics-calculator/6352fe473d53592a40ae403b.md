...
在你的空 `.reduce()` 回調中，聲明一個變量 `difference`，將它設置爲 `el` 減去 `mean` 的值。 然後聲明一個 `squared` 變量，並將其值設置爲 `difference` 的 `2` 次方。 最後，返回 `acc` 加上 `squared` 的值。

# --hints--

你的 `reduce` 函數應該有一個 `difference` 變量。

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

你的 `difference` 變量應該被設置爲 `el` 減去 `mean` 的值。
...
你的 `squared` 變量應該被設置爲 `difference` 的二次方的值。
...
你的 `reduce` 回調應該返回 `acc` 加上 `squared` 的和。
