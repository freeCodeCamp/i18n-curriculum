...
现在是时候为播放列表实现删除功能了。 这将管理从播放列表中删除歌曲，处理删除歌曲时的其他相关操作，并创建一个 `Reset Playlist` 按钮。

使用 `const` 和箭头语法创建一个空的 `deleteSong` 函数并传入 `id` 作为参数。

# --hints--

你应该使用 `const` 创建一个名为 `deleteSong` 的空函数。
