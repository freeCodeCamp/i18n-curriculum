...
Ndani ya njia ya `showFortune` tangaza kigezo kinachoitwa `target` na weka thamani yake kuwa `e.target` na tengeneza taarifa nyingine ya if isiyo na maudhui.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `target` ndani ya `showFortune`.
...
Kigezo chako cha `target` kinapaswa kupewa thamani ya `e.target`.
...
Unapaswa kuwa na taarifa tupu ya `if` mara baada ya kigezo chako cha `target`.
