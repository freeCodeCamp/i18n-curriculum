...
Ifuatayo, unahitaji kuonyesha kichwa cha wimbo wa sasa na msanii kwenye onyesho la mchezaji. Tumia `const` na sintaksia ya mshale kuunda kitendakazi tupu cha `setPlayerDisplay`.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi tupu chenye jina `setPlayerDisplay`.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` inapaswa kuwa kitendakazi.

```js
assert.isFunction(setPlayerDisplay)
```

Kitendakazi chako cha `setPlayerDisplay` kinapaswa kutumia sintaksia ya mshale.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

Kitendakazi chako cha `setPlayerDisplay` kinapaswa kuwa tupu.
