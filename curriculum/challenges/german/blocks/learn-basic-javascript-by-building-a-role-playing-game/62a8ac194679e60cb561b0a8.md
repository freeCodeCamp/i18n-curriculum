...
Geben Sie Ihrem leeren `button text`-Array drei String-Elemente. Verwenden Sie die drei Strings, die den Button-`innerText`-Eigenschaften in der `goTown`-Funktion zugewiesen werden. Denken Sie daran, dass Array-Werte durch Kommas getrennt sind.

# --hints--

Ihr `button text`-Array sollte drei Elemente enthalten.

```js
assert.lengthOf(locations[0]["button text"], 3);
```

Ihr `button text`-Array sollte drei Strings enthalten.
