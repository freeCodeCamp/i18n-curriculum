...
تسمح طريقة `reduce` بمزيد من الأشكال العامة لمعالجة ال array ، ومن الممكن إظهار أن كلاً من `filter` و `map` يمكن اشتقاقهما كتطبيقات خاصة لـ `reduce`. تتكرر طريقة `reduce` علي كل عنصر في array وتقوم بإرجاع قيمة واحدة (أي string او رقم او object او array). يتم تحقيق ذلك من خلال الـ callback function التي يتم استدعاؤها في كل تكرار.
...
في مثال آخر ، انظر كيف يمكن إرجاع object يحتوي على أسماء المستخدمين كخصائص بأعمارهم كقيم.
...
يحتوي المتغير `watchList` على array من ال objects التي تحتوي على معلومات عن عدة أفلام. استخدم `reduce` للعثور على متوسط تصنيف IMDB للأفلام التي اخرجها `Christopher Nolan`. استرجع من التحديات السابقة كيفية عمل `filter` للبيانات و تطبيق `map` عليها لسحب ما تحتاج إليه. قد تحتاج إلى إنشاء متغيرات أخرى، وإرجاع متوسط التقييم من دالة `getRating`. لاحظ أن قيم التصنيف يتم حفظها كـ strings في ال object ويجب تحويلها إلى أرقام قبل استخدامها في أي عمليات رياضية.
