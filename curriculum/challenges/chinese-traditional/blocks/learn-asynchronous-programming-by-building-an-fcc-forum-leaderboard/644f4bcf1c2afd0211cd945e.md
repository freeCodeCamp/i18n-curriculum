...
每個分類都會有一個 URL 指向 [freeCodeCamp 論壇](https://forum.freecodecamp.org/) 上該分類。

創建一個名為 `url` 的常數，並指定一個模板字串。在該模板字串內，放入 `${forumCategoryUrl}${selectedCategory.className}/${id}` 的值。
...
你應該將模板字串指定給你的 `url` 變數。

```js
assert.match(code, /const\s+url\s*=\s*`[\s\S]*`/);
```

你應該在你的模板字串中擁有 `${forumCategoryUrl}${selectedCategory.className}/${id}` 的值。 
