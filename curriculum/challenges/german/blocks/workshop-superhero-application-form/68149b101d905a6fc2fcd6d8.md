...
Unterhalb der destrukturierten `value` und `checked` aktualisieren Sie die Liste der ausgewählten Kräfte.

Rufen Sie die `setPowers`-Funktion auf, die ein Array-Argument erwartet. Verwenden Sie einen ternären Operator, um zu prüfen, ob `checked` wahr ist. Falls ja, breiten Sie das bestehende `powers`-Array in einem neuen Array aus und fügen Sie `value` hinzu. Falls nicht, filtern Sie `value` aus `powers` mit `powers.filter(p => p !== value)` heraus.

# --hints--

Sie sollten einen ternären Operator verwenden, um zu prüfen, ob `checked` wahr ist. Falls ja, breiten Sie das bestehende `powers` und `value` in einem Array aus. Falls nicht, filtern Sie `value` aus `powers` heraus.
