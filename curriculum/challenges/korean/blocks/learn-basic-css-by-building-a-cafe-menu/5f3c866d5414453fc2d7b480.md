...
기존 coffee/price 쌍 아래에, 각 `article` 요소 안에 두 개의 중첩된 `p` 요소를 포함하여 다음 커피와 가격을 추가하세요:
...
각 `article` 요소는 두 개의 `p` 요소를 가져야 합니다.
...
첫 번째 `article` 요소는 `p` 요소를 가지고, 텍스트는 `French Vanilla` 와 `3.00` 이어야 합니다.
...
두 번째 `article` 요소는 `p` 요소를 가지고, 텍스트는 `Caramel Macchiato` 와 `3.75` 이어야 합니다.
...
세 번째 `article` 요소는 `p` 요소를 가지고, 텍스트는 `Pumpkin Spice` 와 `3.50` 이어야 합니다.
...
네 번째 `article` 요소는 `p` 요소를 가지고, 텍스트는 `Hazelnut` 와 `4.00` 이어야 합니다.
...
다섯 번째 `article` 요소는 `p` 요소를 가지고, 텍스트는 `Mocha` 와 `4.50` 이어야 합니다.
