...
Sasa fanya wito wa kitendakazi cha `showElements` na pita `multipleCard`, `fortuneContainer`, na `text` kama vigezo.

# --hints--

Unapaswa kufanya wito wa kitendakazi cha `showElements` ndani ya njia ya `multipleCardSelected`.
...
Unapaswa kupitisha `multipleCard` kama hoja ya kwanza kwa kitendakazi cha `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

Unapaswa kupitisha `fortuneContainer` kama hoja kwa kitendakazi cha `showElements`.
...
Unapaswa kupitisha `text` kama hoja kwa kitendakazi cha `showElements`.
