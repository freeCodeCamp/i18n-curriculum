...
Agora você vai começar a construir o formulário. Dentro do `div` com o `className` de `form-panel`, adicione um `div` para envolver a seção "Player Info". Dentro desse novo `div`, adicione `p` com o `className` de `form-section-title` e o texto `Player Info`.

Você continuará adicionando campos a essa seção nos próximos passos.

# --hints--

Seu `div.form-panel` deve conter um `div` para envolver a seção Player Info.
...
O `div` dentro do `div.form-panel` deve conter um `p` com o `className` de `form-section-title` e o texto `Player Info`.
