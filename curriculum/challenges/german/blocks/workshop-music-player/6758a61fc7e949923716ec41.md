...
Verwenden Sie nun die `querySelectorAll`-Methode, um die `.playlist-song`-Elemente anzusprechen und speichern Sie sie in einer Variablen namens `songs`.

Rufen Sie dann die `forEach`-Methode auf `songs` auf. Übergeben Sie eine Callback-Funktion, die `song` als Parameter für die Methode verwendet. Lassen Sie die geschweiften Klammern vorerst leer.

# --hints--

Sie sollten eine Variable namens `songs` haben.

```js
assert.isDefined(songs);
```

Sie sollten die Sammlung aller `.playlist-song`-Elemente Ihrer `songs`-Variablen zuweisen.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

Sie sollten die `forEach`-Methode auf `songs` aufrufen und eine Callback-Funktion übergeben, die `song` als Parameter für die Methode verwendet.
