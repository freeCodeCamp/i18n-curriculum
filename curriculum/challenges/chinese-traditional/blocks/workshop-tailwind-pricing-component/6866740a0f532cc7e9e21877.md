...
讓我們繼續到 `Most popular` 徽章。你需要將它放置在卡片的右上角，因此你可以使用 Tailwind 的一些定位工具類別來達成。
...
將類別 `relative` 指定到外覆 `div`，並將 `absolute` 指定到帶有文字 `Most popular` 的 `div`。

# --hints--

你的高級方案的外覆 `div` 元素應該有 `relative` 類別。
...
你的 `div` 元素中帶有文字 `Most popular` 應該有類別 `absolute`。
