...
1. Wenn nach dem Durchsuchen des ganzen Arrays der Wert nicht vorhanden ist, gib `false` zurück (Das Array wurde durchsucht und der Wert befindet sich nicht im Array).
...
Der Funktion wird ein sortierter Array mit Integern sowie ein Zielwert übergeben. Es wird ein Array zurückgegeben, das (in Reihenfolge) den mittleren Wert enthält, den du bei jeder Halbierung des ursprünglichen Arrays gefunden hast, bis du den Zielwert gefunden hast. Der Zielwert sollte das letzte Element des zurückgegebenen Arrays sein. Wenn der Wert nicht gefunden wurde, gib den String `Value Not Found` zurück.
