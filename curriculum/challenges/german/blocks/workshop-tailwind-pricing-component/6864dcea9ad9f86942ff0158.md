...
Es ist Zeit, am `div`-Element zu arbeiten, das die drei Preiskarten umschließt. Um die Karten anzuordnen, können Sie die Grid-Utility-Klassen von Tailwind CSS verwenden.

So weisen Sie einem Element mit Tailwind eine Anzeige von `grid` zu:
...
Weisen Sie dem `div`-Element, das die Preiskarten umschließt, die Klasse `grid` zu.
