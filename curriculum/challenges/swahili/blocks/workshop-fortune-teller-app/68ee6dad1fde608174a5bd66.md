...
Ndani ya njia ya `singleCardSelected`, chukua kipengele cha `singleCard` na sasisha `innerHTML` kwa kuita kitendakazi cha `renderCard`. Pitia `"Click the card and reveal the fortune"` kama hoja ya kwanza, na `isReversed` kama hoja ya pili.

# --hints--

Unapaswa kusasisha `this.elements.singleCard.innerHTML` kwa kuita kitendakazi cha `renderCard`.
...
Wito wako wa `renderCard` unapaswa kuwa na `"Click the card and reveal the fortune"` kama hoja ya kwanza.
...
Unapaswa kupitisha `isReversed` kama hoja ya pili kwa `renderCard`.
