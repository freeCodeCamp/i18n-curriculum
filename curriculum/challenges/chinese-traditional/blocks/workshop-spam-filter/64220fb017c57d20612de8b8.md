...
現在你已經有你的捕獲群組，你可以將整個樣式標記為選用的比對。`?` 量詞比對前一個字元或群組出現零次或一次。舉例來說，正規表達式 `/colou?r/` 同時比對 `color` 和 `colour`，因為 `u` 是選用的。
...
你的 `(hundred|thousand|million|billion)` 捕獲群組應該是選用的。

```js
assert.match(dollarRegex.source, /\(hundred\|thousand\|million\|billion\)\?/);
```

你的 `dollarRegex` 仍然應該匹配 `1 million dollars`。
