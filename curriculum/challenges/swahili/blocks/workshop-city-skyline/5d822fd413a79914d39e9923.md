...
Ondoa sifa na thamani ya `background-color` kutoka `.fb3`, na ziweke katika `.fb3a` na `.fb3b`.
...
Unapaswa kumpa `.fb3a` sifa ya `background-color` yenye thamani ya `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

Unapaswa kumpa `.fb3b` sifa ya `background-color` yenye thamani ya `--building-color1`.
