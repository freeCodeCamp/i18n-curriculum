...
새로운 `getVariance` 함수를 `calculate` 함수에 추가하고, 해당 HTML 요소를 업데이트하세요.

# --hints--

`calculate` 함수에는 `variance` 값을 가진 `getVariance(numbers)` 변수가 있어야 합니다.

```js
assert.match(calculate.toString(), /variance\s*=\s*getVariance\(\s*numbers\s*\)/);
```

`textContent` 요소의 `#variance`를 `variance` 변수 값으로 설정하세요.
