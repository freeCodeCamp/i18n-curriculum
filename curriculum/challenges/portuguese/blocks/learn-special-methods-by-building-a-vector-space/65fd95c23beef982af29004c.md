...
Você quer comparar dois vetores, somente quando eles pertencem à mesma classe.Para isso, crie uma declaração `if` que verifique se `self` e `other` não pertencem à mesma classe e retorne `NotImplemented` nesse caso.
# --hints--
