...
All'interno della funzione `playPreviousSong`, aggiungi un'istruzione `if` con una condizione `userData?.currentSong === null`. Questo controllerà se attualmente non c'è nessuna canzone in riproduzione. Se non ce n'è, esci dalla funzione usando un `return`.

All'interno del blocco `else`, crea una costante chiamata `currentSongIndex` e assegnale `getCurrentSongIndex()`.

# --hints--

Non dovresti modificare la funzione `playPreviousSong` esistente e il suo contenuto.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Dovresti avere un'istruzione `if` con la condizione `userData?.currentSong === null` all'interno della tua funzione `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

Dovresti avere `return` all'interno del blocco della tua istruzione `if`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

La tua istruzione `if` dovrebbe avere un blocco `else`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

Dovresti chiamare `getCurrentSongIndex` e assegnarla a `currentSongIndex` all'interno del blocco `else` della tua istruzione `if`.
