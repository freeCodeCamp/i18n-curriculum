...
الآن بعد أن أصبح لديك الهيكل الأساسي للتطبيق، حان الوقت لإضافة بعض العناصر وتشغيل وظيفة البحث.

أنشئ مصفوفة وعيّنها لمتغير باسم `items`. ضمن المصفوفة، أضف السلاسل النصية `Apples`، `Bananas`، `Strawberries`، `Blueberries`، `Mangoes`، `Pineapple`، `Lettuce`، `Broccoli`، `Paper Towels`، و`Dish Soap`. ستكون هذه قائمة العناصر التي ستُعرض في التطبيق.
...
يجب أن تحتوي مصفوفة `items` على السلاسل النصية `Apples`، `Bananas`، `Strawberries`، `Blueberries`، `Mangoes`، `Pineapple`، `Lettuce`، `Broccoli`، `Paper Towels`، و`Dish Soap`.
