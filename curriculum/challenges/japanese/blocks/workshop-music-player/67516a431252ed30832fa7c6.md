...
今度は、`start` の後にある `if` 文に、`userData.currentSong === null` パラメータを2つ目の条件として **OR** 演算子で追加してください。 


# --hints--

`start` の後にある `if` 文に、`userData.currentSong === null` パラメータを2つ目の条件として **OR** 演算子で追加する必要があります。 
