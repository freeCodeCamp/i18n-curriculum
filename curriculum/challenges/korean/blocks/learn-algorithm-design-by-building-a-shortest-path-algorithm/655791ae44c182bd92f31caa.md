...
`.remove()` 메서드는 인수로 전달된 첫 번째 일치하는 요소를 목록에서 제거합니다.
...
현재 노드를 `while` 목록에서 제거하여 `unvisited` 루프를 종료하세요. 들여쓰기에 주의하세요.

# --hints--

`.remove()` 루프 후에 현재 노드를 `unvisited`에서 제거하려면 `for` 메서드를 사용하세요.
