...
`@media` at-rule 有一個名為 `prefers-reduced-motion` 的媒體特性，用來根據使用者的偏好設定 CSS。它可以接受以下其中一個值：
...
將設定 `scroll-behavior: smooth` 的樣式規則包裹在具有媒體特性 `prefers-reduced-motion` 且值為 `no-preference` 的 `@media` 介面規則中。
