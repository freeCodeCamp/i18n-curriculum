...
이제 점수와 라운드 결과 메시지를 업데이트할 시간입니다.

매개변수 `showResults`를 가진 `userOption` 함수를 만드세요.

`showResults` 함수 안에서, 라운드 결과로 `roundResultsMsg`를 업데이트해야 합니다.

그다음, 플레이어와 컴퓨터의 업데이트된 점수를 보여주기 위해 `playerScoreSpanElement`와 `computerScoreSpanElement`도 업데이트해야 합니다.

여기서 순서가 중요하다는 점을 기억하세요. 먼저 `roundResultsMsg`를 업데이트하고, 그다음 `playerScoreSpanElement`, 마지막으로 `computerScoreSpanElement`를 업데이트해야 합니다. 왜냐하면 `roundResultsMsg`가 점수를 결정하는 데 사용되기 때문입니다.

# --hints--

`showResults`라는 함수가 있어야 합니다.

```js
assert.isFunction(showResults);
```

`showResults` 함수는 `userOption`라는 매개변수를 가져야 합니다.

```js
assert.match(code, /showResults\s*(?:=\s*)?(?:\(\s*)?userOption(?:\s*\))?\s*/);
```

`showResults` 함수는 라운드 결과로 `roundResultsMsg`를 업데이트해야 합니다.
...
`showResults` 함수는 컴퓨터의 업데이트된 점수를 보여주기 위해 `computerScoreSpanElement`를 업데이트해야 합니다.
...
`showResults` 함수는 플레이어의 업데이트된 점수를 보여주기 위해 `playerScoreSpanElement`를 업데이트해야 합니다.
