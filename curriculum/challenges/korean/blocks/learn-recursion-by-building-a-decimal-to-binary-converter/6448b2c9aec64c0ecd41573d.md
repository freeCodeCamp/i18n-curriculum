...
`checkUserInput` 함수 안에서 `console.log()` 문장을 제거하세요. 그런 다음 `decimalToBinary` 함수를 호출하고 `value`의 `numberInput`를 인수로 전달하세요. 또한 입력을 숫자로 변환하려면 `parseInt()` 함수를 사용해야 합니다.

# --hints--

`console.log()` 함수에서 `checkUserInput` 문장을 제거해야 합니다.

```js
assert.notMatch(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*console\.log\((\s|.)*\)/);
```

`decimalToBinary` 함수 본문 내 `if` 문장 뒤에 `checkUserInput` 함수를 호출해야 합니다.

```js
assert.match(String(checkUserInput), /(?:window\.|globalThis\.)?alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*decimalToBinary\(/);
```

`parseInt()`의 `value`를 숫자로 변환하려면 `numberInput`을 사용하고, 그 값을 `decimalToBinary` 함수에 인수로 전달하세요.
