...
新しい行で、加算代入演算子(`+=`)を使って、文字列`" You attack it with your <weapon>."`を`text`の値に追加してください。`<weapon>`はプレイヤーの現在の武器に置き換えます。さらに、この行のテキストは先頭にスペースがあるので、正しく表示されます。

# --hints--

新しい行で、ドット記法を使って`innerText`の`text`プロパティにアクセスしてください。
