...
Antes de comenzar la simulación, necesitas un objeto para registrar cuántos asistentes procesa cada puerta durante todos los ticks. Llama a `initializeThroughput()` con `gates` y asigna el resultado a una variable llamada `throughputSummary`.

# --hints--

Debes llamar a `initializeThroughput()` con `gates` y asignar el resultado a una variable llamada `throughputSummary`.
...
Tu función `simulateFestival` debe llamar a la función `initializeThroughput()` con `gates`.
