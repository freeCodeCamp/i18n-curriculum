...
Come vedi, ora il tuo insieme è completamente vuoto. Rimuovi entrambe le chiamate `clear` e `has` dal tuo codice.

# --hints--

Non dovresti avere `plantsSet.clear()` nel tuo codice.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

Non dovresti avere `plantsSet.has(ballerina)` nel tuo codice.
