...
你的 `addCharacters(char1)` 也傳回一個函式（程式），該函式又傳回另一個函式（程式）。你需要再呼叫另一個函式（程式），才能存取該最內層函式（程式）引用，以用於 `.map()` 回呼。JavaScript 允許你立即喚起傳回的函式（程式）：

```js
myFunc(1)("hi");
```

立即喚起從你的 `addCharacters(char1)` 呼叫傳回的函式，並將 `char2` 作為引數傳入。

# --hints--

你應該將函式（程式）呼叫串鏈到你的 `addCharacters(char1)` 呼叫。
