...
Você deve anexar um `addEventListener` ao botão `scissorsBtn`. O ouvinte de evento deve receber um evento `"click"` e uma função de callback que chama a função `showResults` com o argumento `"Scissors"`.
