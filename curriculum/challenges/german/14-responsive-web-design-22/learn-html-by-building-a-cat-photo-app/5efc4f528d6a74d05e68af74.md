...
Du solltest nach dem Kontrollkästchen ein neues `label`-Element hinzufügen. Make sure it has both an opening and closing tag.
...
Dein `label`-Element sollte ein `for`-Attribut mit dem Wert `loving` enthalten.  Expected `--fcc-expected--`, but found `--fcc-actual--`.
