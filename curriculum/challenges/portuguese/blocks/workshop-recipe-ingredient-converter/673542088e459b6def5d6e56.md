...
Você vai precisar de uma função que realmente atualize o DOM. Declare uma função `updateResultsList`.

Para começar, a função deve limpar o HTML do elemento `resultList`.

# --hints--

Você deve definir uma função `updateResultsList`.

```js
assert.isFunction(updateResultsList);
```

Sua função `updateResultsList` deve definir o `resultList.innerHTML` como uma string vazia.
