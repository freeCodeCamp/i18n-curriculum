...
回到 `initializeApp` 函数，这里你将把之前创建的内联类型和 `getElement()` 函数投入使用，通过获取应用所需的 DOM 元素。

在 `initializeApp` 函数内部，通过调用 `shapeTypeSelect` 并传入 `getElement()` 来重新赋值 `"shape-type"`，然后使用 `as` 关键字将结果转换为 `HTMLSelectElement`。

# --hints--

你应该用合适的值重新赋值 `shapeTypeSelect`。

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

你应该调用 `getElement` 函数并传入 `shape-type`，然后赋值给 `shapeTypeSelect`。

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

你应该使用 `getElement` 关键字调用 `HTMLSelectElement`，并将 `shapeTypeSelect` 作为 `as` 的类型。
