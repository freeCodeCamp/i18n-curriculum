...
Agregaste una llamada a `console.log()` en tu script para validar tu arreglo `EVAChunks`, y ahora la llamada ya no es necesaria. Para mantener la terminal limpia para los siguientes pasos, elimina el bloque de `for loop` que registra datos de `EVAChunks`.

# --hints--

Debes eliminar el bloque de `for loop` que registra datos de `EVAChunks`. Borra todo el bloque que contiene ambos loops.
