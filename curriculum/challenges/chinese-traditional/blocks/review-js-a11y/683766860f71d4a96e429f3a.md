...
- **`aria-haspopup` 屬性**：此狀態用於表示互動元素在啟動時會觸發彈出元素。你只能在彈出元素具有以下角色之一時使用 `aria-haspopup` 屬性：`menu`、`listbox`、`tree`、`grid` 或 `dialog`。`aria-haspopup` 的值必須是這些角色之一，或是 `true`（等同於 `menu`）。
