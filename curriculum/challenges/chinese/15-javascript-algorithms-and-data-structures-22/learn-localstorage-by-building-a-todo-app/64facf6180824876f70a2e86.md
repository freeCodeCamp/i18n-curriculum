...
创建一个 `if` 语句来检查 `formInputsContainValues` 是否为真。 如果 `formInputsContainValues` 为 true，表明有变化，请在 `confirmCloseDialog` 上使用 `showModal()` 方法。 否则，如果没有变化，则调用 `reset()` 函数清除输入字段并隐藏表单模式。

# --hints--

你应该创建一个带有条件 `formInputsContainValues` 的 `if` 语句。

```js
assert.match(code, /if\s*\(\s*formInputsContainValues\s*\)\s*\{/)
```

`if` 语句的 `if` 块应包含 `confirmCloseDialog.showModal();`。
...
你应该在 `if` 语句的 `else` 块中调用 `reset()` 函数。
