...
Los elementos de la cuadrícula están demasiado cerca de los bordes izquierdo y derecho de la página, así que debes trabajar en el espaciado.

Asigna la clase `max-w-6xl` al `div` para establecer un ancho fijo de `72rem` y evitar que los elementos se estiren demasiado a lo largo de la página.

Como eso empujará todos hacia la izquierda, agrega `mx-auto` al `div` para centrar los elementos horizontalmente.

# --hints--

Tu elemento `div` debe tener la clase `max-w-6xl`.
...
Tu elemento `div` debe tener la clase `mx-auto`.
