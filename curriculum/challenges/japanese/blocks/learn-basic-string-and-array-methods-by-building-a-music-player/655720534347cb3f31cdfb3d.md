...
曲を削除する前に、その曲が現在再生中かどうかを確認する必要があります。再生中であれば、曲を一時停止してプレイリストの次の曲を再生してください。

`if` 文を使って、`userData?.currentSong?.id` が削除したい曲の `id` と等しいかどうかを確認してください。

# --hints--

条件 `if` を持つ空の `userData?.currentSong?.id === id` 文を作成してください。
