...
Invoque `getEVAReadyCrew()` com sua lista `updatedSquad` e armazene o resultado em uma nova variável chamada `EVAReadySquad`. Então, use um `for` loop para registrar o `name` de cada astronauta no array `EVAReadySquad`.

# --hints--

Você deve fazer a chamada de `getEVAReadyCrew()` com `updatedSquad` e armazenar o resultado em uma nova variável chamada `EVAReadySquad`.
...
Você deve usar um `for` loop para iterar através do array `EVAReadySquad` e registrar o `name` de cada astronauta.
