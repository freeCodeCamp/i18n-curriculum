...
Kutumia `const` na sintaksia ya mshale kuunda kitendakazi tupu kinachoitwa `shuffle`.

Kitendakazi hiki kinahusika na kuchanganya nyimbo katika orodha ya nyimbo na kufanya masasisho muhimu ya usimamizi wa hali baada ya kuchanganya.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi tupu kinachoitwa `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

`shuffle` inapaswa kuwa kitendakazi.

```js
assert.isFunction(shuffle)
```

Kitendakazi chako cha `shuffle` kinapaswa kutumia sintaksia ya mshale na kisichopokea kigezo.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

Kitendakazi chako cha `shuffle` kinapaswa kuwa tupu.
