...
이제 이 요소들의 크기를 줄여야 합니다. `span[class~="sr-only"]` 선택자에 `width`과 `height` 속성을 `1px`으로 설정하세요.

# --hints--

`span[class~="sr-only"]` 선택자에는 `width` 속성을 `1px`으로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('width') === '1px');
```

`span[class~="sr-only"]` 선택자에는 `height` 속성을 `1px`으로 설정해야 합니다.
