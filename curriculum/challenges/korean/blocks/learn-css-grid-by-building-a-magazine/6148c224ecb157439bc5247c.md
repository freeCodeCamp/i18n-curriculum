...
CSS Grid의 기본 설정은 Flexbox와 달리 필요에 따라 추가 행을 만듭니다. 아이콘을 한 줄로 배열하려면 `.social-icons` 선택자에 `grid-template-columns` 속성을 `repeat(5, 1fr)`로 설정하세요.

# --hints--

`.social-icons` 선택자에 `grid-template-columns` 속성을 `repeat(5, 1fr)`로 설정해야 합니다.
