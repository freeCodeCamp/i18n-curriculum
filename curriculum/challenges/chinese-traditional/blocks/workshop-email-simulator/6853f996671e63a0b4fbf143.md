...
為你的 `User` 類別添加兩個方法：`read_email` 和 `delete_email`，用於讀取和刪除電子郵件。兩者都應該接受一個 `index` 參數，並呼叫 `self.inbox` 上相應的方法。

# --hints--

你應該在 `User` 類別中定義一個名為 `read_email` 的方法，該方法接受 `self` 和 `index` 作為參數。
...
`read_email` 方法應該呼叫 `self.inbox.read_email(index)`。
...
你應該在 `User` 類別中定義一個名為 `delete_email` 的方法，該方法接受 `self` 和 `index` 作為參數。
...
`delete_email` 方法應該呼叫 `self.inbox.delete_email(index)`。
