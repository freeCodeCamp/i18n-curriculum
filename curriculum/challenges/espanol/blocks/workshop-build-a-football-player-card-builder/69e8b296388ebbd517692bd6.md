...
El formulario ya está completo. Para que la tarjeta de vista previa se actualice mientras escribes, necesitas conectar las entradas al estado `player`.

Comenzando con el nombre `input`, añade un atributo `value` con valor `player.name`. Luego agrega un atributo `onChange` configurado con una función flecha que toma `e` como parámetro y llama a `setPlayer` con una copia de `player` y `name` establecido en `e.target.value`. No olvides poner tanto la copia como el valor de `name` dentro de un objeto.

# --hints--

Tu entrada para el nombre `value` debe estar vinculada al campo `name` del estado player.
...
Tu entrada para el nombre debe tener un manejador `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

El manejador `onChange` debe llamar a `setPlayer` con `setPlayer` teniendo un objeto que copia `player` y establece `name` en `e.target.value`.
