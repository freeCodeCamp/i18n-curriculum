...
Em alguns casos, as cartas precisam ser invertidas, pois nem sempre estão na posição correta.

Declare uma variável chamada `isReversed` dentro do método `singleCardSelected`. Use o método `Math.random()` para gerar um valor booleano aleatório. Para garantir que o valor fique dentro dos limites, compare-o com `0.5`.
