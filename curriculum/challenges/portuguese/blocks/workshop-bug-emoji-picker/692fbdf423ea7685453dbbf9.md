...
Agora você vai começar a referenciar os elementos DOM necessários.

Pegue o elemento de parágrafo com o id `bug-emoji` e armazene-o em uma constante chamada `bugEmojiElement`. Use o operador de asserção non-null `!` para garantir que o elemento não é `null`.

# --hints--

Você deve ter uma constante chamada `bugEmojiElement`.
...
Seu `bugEmojiElement` deve consultar o elemento com o id `bug-emoji` usando `document.querySelector` com um argumento de tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

Seu `bugEmojiElement` deve usar o operador de asserção non-null (`!`).
