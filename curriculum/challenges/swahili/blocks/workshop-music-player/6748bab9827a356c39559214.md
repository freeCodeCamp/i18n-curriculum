...
Tumia `classList` na `remove()` njia kuondoa darasa la vitu la `playing` kutoka kwa `playButton`, kwa sababu wimbo utaachwa kwa wakati huu.

Ili kuacha wimbo kabisa, tumia njia ya `pause()` kwenye mabadiliko ya `audio`. `pause()` ni njia ya Web Audio API kwa kuacha mafaili ya muziki.

# --hints--

Kitendakazi chako cha `pauseSong` kinapaswa kuondoa darasa la vitu la `playing` kutoka kwa `playButton`.
...
Kitendakazi chako cha `pauseSong` kinapaswa kuita njia ya `pause()` kwenye `audio`.
