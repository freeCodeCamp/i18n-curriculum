...
現在の再生時間 (位置) を処理するため、`else` ブロックを追加してください。 これにより、現在の曲を一時停止した再生位置から再開できるようにします。

`else` ブロック内では、`audio` オブジェクトの `currentTime` プロパティを、`userData?.songCurrentTime` に格納されている値に設定してください。

# --hints--

既存の `if` 文とその中身は変更しないでください。
...
既存の `if` ブロックの後に `else` ブロックを追加してください。
...
`if` 文の `else` ブロック内で、`audio.currentTime` を `userData?.songCurrentTime` に設定してください。
