...
Puedes agregar varios gradientes a un elemento separándolos con una coma (`,`) de esta manera:
...
Agrega un `repeating-linear-gradient` a `.fb1c` debajo del que está; utiliza tu `--building-color4` de `0%` a `10%` y `--window-color4` de `10%` a `90%`. Esto se llenará detrás del gradiente que agregaste último.

# --hints--

No debes alterar el primer `repeating-linear-gradient`.
...
Debes agregar un `repeating-linear-gradient` con un primer color de `--building-color4` de `0%` a `10%`.
...
Debes usar un segundo color de `--window-color4` de `10%` a `90%`.
