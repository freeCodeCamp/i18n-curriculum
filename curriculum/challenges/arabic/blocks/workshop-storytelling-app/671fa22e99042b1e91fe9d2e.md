...
داخل `.btn-container`، أنشئ ثلاث أزرار لكل نوع من القصة. أعط كل زر صنف `btn` و`id` بقيم `scary-btn`، `funny-btn`، و`adventure-btn`. يجب أن يكون نص كل زر هو `Scary Story`، `Funny Story`، و`Adventure Story` على التوالي.

# --hints--

داخل عنصر `.btn-container`، يجب أن يكون لديك ثلاثة أزرار.

```js
assert.lengthOf(document.querySelectorAll('.btn-container > button') ,3);
```

يجب أن تحمل جميع الأزرار الثلاثة صنفًا `btn`.

```js
assert.equal(document.querySelectorAll('.btn-container > .btn').length, 3);
```

يجب أن يكون لديك زر يحمل `id` بقيمة `scary-btn` ونصه `Scary Story`.
...
يجب أن يكون لديك زر يحمل `id` بقيمة `funny-btn` ونصه `Funny Story`.
...
يجب أن يكون لديك زر يحمل `id` بقيمة `adventure-btn` ونصه `Adventure Story`.
