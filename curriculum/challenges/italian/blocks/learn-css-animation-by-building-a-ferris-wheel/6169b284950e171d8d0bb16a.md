...
Così la tua animazione sarà molto più fluida e la tua ruota panoramica sarà completa.
...
Il tuo selettore `75%` dovrebbe essere tra i selettori `50%` e `100%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText === '75%');
```

Il tuo selettore `75%` dovrebbe avere una proprietà `background-color` impostata su `yellow`.
