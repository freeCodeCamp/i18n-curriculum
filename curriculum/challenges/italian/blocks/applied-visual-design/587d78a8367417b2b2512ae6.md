...
title: Anima più elementi a velocità variabili
challengeType: 0
...
Nella sfida precedente, hai modificato le velocità di animazione di due elementi animati in modo simile cambiando le loro regole `@keyframes`. Puoi ottenere lo stesso risultato manipolando la `animation-duration` di più elementi.

Nell'animazione che gira nell'editor di codice, ci sono tre stelle nel cielo che scintillano alla stessa velocità in un loop continuo. Per farle scintillare a velocità diverse, puoi impostare la proprietà `animation-duration` su valori diversi per ogni elemento.

# --instructions--

Imposta la `animation-duration` degli elementi con le classi `star-1`, `star-2` e `star-3` rispettivamente su 1s, 0.9s e 1.1s.
