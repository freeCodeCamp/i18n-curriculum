...
메모에 `div` 요소가 사용되고 있어서 화면 읽기 프로그램이 그 목적을 이해하지 못합니다. `aria-label` 속성을 추가하면 이를 고칠 수 있습니다. 이렇게 하면 접근성이 향상되고 이 `div` 요소에 의미를 부여할 수 있습니다.

시작 `div` 태그에 `aria-label` 속성을 추가하고 그 값을 `"Note editor"`로 설정하세요.

# --hints--

시작 `div` 태그에는 `aria-label` 속성이 있어야 합니다.

```js
assert.isTrue(document.getElementById("note").hasAttribute("aria-label"));
```

시작 `div` 태그에는 `aria-label` 속성을 `"Note editor"`로 설정해야 합니다.
