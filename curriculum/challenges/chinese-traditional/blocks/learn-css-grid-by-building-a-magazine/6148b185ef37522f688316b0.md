...
現在你可以使用 CSS Grid 來設計 `.heading` 元素的內容樣式。

CSS 的 `repeat()` 函式（程式）用於重複一個值，而不是手動寫出，對於網格佈局非常有幫助。<dfn>例如</dfn>，將 `grid-template-columns` 屬性設定為 `repeat(20, 200px)` 將會創建 20 個寬度為 `200px` 的行。

為你的 `.heading` 元素添加 `grid-template-columns` 屬性並設為 `repeat(2, 1fr)`，以生成兩個寬度相等的行。

# --hints--

你的 `.heading` 選擇器應該有一個 `grid-template-columns` 屬性設定為 `repeat(2, 1fr)`。
