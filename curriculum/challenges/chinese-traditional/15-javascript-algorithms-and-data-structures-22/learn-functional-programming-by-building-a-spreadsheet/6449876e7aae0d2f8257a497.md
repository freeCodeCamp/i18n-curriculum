...
聲明一個 `rangeFromString` 箭頭函數，它接受兩個參數，`num1` 和 `num2`。 該函數應隱式返回以 `num1` 和 `num2` 作爲參數調用 `range` 的結果。

爲了安全起見，在將 `num1` 和 `num2` 傳遞到 `range` 時，請將它們解析爲整數。

# --hints--

你應該在 `rangeRegex` 之後聲明一個 `rangeFromString` 變量。
...
你應該使用 `const` 來聲明你的 `rangeFromString` 變量。
...
你的 `rangeFromString` 變量應該是一個箭頭函數。
...
你的 `rangeFromString` 函數應該將 `num1` 作爲第一個參數。
...
你的 `rangeFromString` 函數應該將 `num2` 作爲第二個參數。
...
你的 `rangeFromString` 函數應該返回調用 `range` 函數的結果。
...
你應該使用 `num1` 作爲參數調用 `parseInt`，並將結果傳遞給 `range` 調用。

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

你應該使用 `num2` 作爲參數調用 `parseInt`，並將結果傳遞給 `range` 調用。
