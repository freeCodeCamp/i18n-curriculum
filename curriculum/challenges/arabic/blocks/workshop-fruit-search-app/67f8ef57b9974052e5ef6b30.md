...
داخل عنصر `#results`، استخدم معامل ثلاثي للتحقق مما إذا كان طول مصفوفة `results` أكبر من صفر. إذا كان كذلك، استعرض العناصر واعرض كل واحد في عنصر `p` مع صنف `result-item`. تأكد من تضمين خاصية `key` مع كل فقرة أيضًا.

إذا كانت `results` فارغة، اعرض عنصر `p` مع الرسالة `No results found`.

# --hints--

يجب أن تحتوي الحاوية `#results` على قائمة من الفواكه المطابقة بناءً على الاستعلام. يجب أن يكون كل نتيجة عنصر `p` مع صنف `result-item` ويعرض اسم الفاكهة.
...
يجب أن يحتوي عنصر `#results` على عنصر `p` بالنص `No results found` عندما تكون مصفوفة `results` فارغة.
