...
Para crear fragmentos de tamaño `n` a partir de un `array` sin modificarlo, puedes combinar un ciclo `for` con el método `slice()` de la siguiente manera: 
...
- crea un arreglo vacío llamado `result`

- itera a través del `array` original en pasos de tamaño `n`

- crea fragmentos de `array` de tamaño `n` y los agrega a `result` usando `slice()`

Para completar la función `chunkCrew`, debes: 

- crea un arreglo llamado `chunks`

- itera a través de `crew` en pasos de tamaño `size`, creando y agregando fragmentos de `crew` a `chunks` usando `slice()`

- devuelve el arreglo `chunks`

# --hints--

Debes crear un arreglo vacío llamado `chunks`.
...
Debes usar un bucle `for` para construir fragmentos.
...
Dentro del ciclo `for`, debes llamar a `push()` en el arreglo `chunks` para insertar cada fragmento cortado (que debe ser creado llamando a `slice()` en el arreglo `crew`).
...
Después del ciclo, debes devolver el arreglo `chunks`.
...
Tu función `chunkCrew` debe devolver fragmentos de longitud `size` del arreglo de entrada `crew` sin modificarlo.
