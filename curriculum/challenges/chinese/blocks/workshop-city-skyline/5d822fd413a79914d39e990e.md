...
将 `bb4-window` 类的 `width` 设置为 `18%`，`height` 设置为 `90%`，并将`background-color` 的值设为变量`--window-color4` 。

# --hints--

你应该给 `.bb4-window` 添加一个值为 `18%` 的 `width` 。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

你应该给 `.bb4-window` 添加一个值为 `90%` 的 `height` 。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

你应该给 `.bb4-window` 添加一个值为 `--window-color4` 的 `background-color` 。
