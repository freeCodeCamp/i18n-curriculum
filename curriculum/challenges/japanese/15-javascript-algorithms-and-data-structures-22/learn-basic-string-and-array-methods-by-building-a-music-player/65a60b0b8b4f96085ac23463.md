...
例では、`a.name` が `b.name` と等しい場合、関数は `0` を返すようになっています。 これは並べ替えを行わず、`a` と `b` の順序を同じままにすることを意味します。
...
では `if` 文の後に、2 つの要素の順序を変えないように数値 `0` を返す処理を追加してください。

# --hints--

`if` 文の後に数値 `0` を返す処理が必要です。
