...
下一個建置將有三個區段。在 `.bb4` 中巢狀三個 `div` 元素。依序為它們指定 `bb4a`、`bb4b` 和 `bb4c` 類別。

# --hints--

你應該在 `.bb4` 中添加三個 `div` 元素。

```js
assert.equal(document.querySelector("div.bb4")?.children?.length, 3);
```

你應該給第一個新的 `div` 一個 `bb4a` 的類別。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你應該給第二個新的 `div` 一個 `bb4b` 的類別。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你應該給第三個新的 `div` 一個 `bb4c` 的類別。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

你應該以此順序放置新的 `div` 元素 `.bb4a + .bb4b + .bb4c`。
