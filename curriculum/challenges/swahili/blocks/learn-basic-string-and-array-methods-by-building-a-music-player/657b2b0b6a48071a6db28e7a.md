...
Njia ya `map()` hutumika kurudia kupitia safu ya data na kurudisha safu mpya ya data. Ni msaada wakati unataka kuunda safu mpya ya data kulingana na thamani za safu iliyopo. Kwa mfano:
...
Angalia kwamba njia ya `map()` inachukua kitendakazi kama hoja. Hii huitwa <dfn>kitendakazi cha mwitiko</dfn>, ambacho ni kitendakazi kinachopitishwa kwa kitendakazi kingine kama hoja. Katika mfano hapo juu, kitendakazi cha mwitiko ni `(number) => number * 2`, na kinaendeshwa kwa kila kipengele katika safu ya `numbers`. Njia ya `map()` kisha hurudisha safu mpya ya data yenye matokeo.

Pitia kitendakazi cha mwitiko kwa njia ya `map()`. Kitendakazi cha mwitiko kinapaswa kuchukua `song` kama kigezo, tumia sintaksia ya kitendakazi cha mshale, na kuwa na mwili tupu.

# --hints--

Kitendakazi cha mwitiko cha njia yako ya `map()` kinapaswa kutumia sintaksia ya kitendakazi cha mshale.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

Njia yako ya `map()` inapaswa kuwa na kigezo cha `song` katika kitendakazi chake cha mwitiko.
