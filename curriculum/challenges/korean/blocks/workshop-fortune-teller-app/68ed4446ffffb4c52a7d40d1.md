...
생성자 안에서 `fetchCardsData()`와 `initializeEventListeners()`를 `this` 키워드를 사용해 호출하세요. 이렇게 하면 클래스가 초기화될 때 실행됩니다.

`fetchCardsData` 메서드는 API에서 타로 카드 데이터를 불러오는 역할을 하며, `initializeEventListeners`는 버튼 클릭 핸들러를 설정합니다. 두 메서드는 다음 단계에서 만들게 됩니다.

# --hints--

`fetchCardsData()` 메서드를 `constructor` 안에서 호출해야 합니다.
...
생성자 안에서 `initializeEventListeners()` 메서드를 호출해야 합니다.
