...
산맥 효과를 내기 위해, `div` 바로 뒤에 새 `.left-mountain`를 만들고 새 `div`에 `class`의 `back-mountain`를 지정하여 또 다른 산을 추가하세요.

# --hints--

`div` 안에 새 `body`를 추가해야 합니다.

```js
assert.equal(document.querySelectorAll('body > div')?.length, 4);
```

`div`에 `class`의 `back-mountain`를 지정해야 합니다.

```js
assert.include(document.querySelector('div:not(.left-mountain, .ground, .penguin)')?.className, 'back-mountain');
```

`.back-mountain`을 `.left-mountain` 뒤에 배치해야 합니다.
