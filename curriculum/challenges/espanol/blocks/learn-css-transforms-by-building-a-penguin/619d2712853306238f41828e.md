...
Crea una variable CSS personalizada llamada `--penguin-skin`, y establecela en `gray`. Luego, reemplaza todos los valores relevantes de las propiedades con ella.

# --hints--

Debes darle a `:root` una propiedad `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

Debes darle a `--penguin-skin` un valor de `gray`, pero se encontró `--fcc-actual--`.
...
Debes darle a `.penguin-body::before` un `background-color` de `var(--penguin-skin)`, pero se encontró `--fcc-actual--`.
