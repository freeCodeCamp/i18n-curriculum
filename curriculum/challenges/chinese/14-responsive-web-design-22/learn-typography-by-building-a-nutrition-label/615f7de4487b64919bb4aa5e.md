...
在最后一个 `.divider` 之后，使用文本 `Cholesterol 0mg 0%` 创建一个新的 `p` 元素。 然后，将文本 `Cholesterol` 放在一个 `span` 元素中，将 `0%` 放在另一个元素中，并为每个元素设置 `bold` 类。
...
新的 `p` 元素应包含文本 `Cholesterol 0mg 0%`。
...
你的 `Cholesterol` `span` 应该有 `class` 属性设置为 `bold`。
...
你的 `0%` `span` 应该有 `class` 属性设置为 `bold`。
...
你的 `Cholesterol` `span` 和 `0mg` 文本应该被包裹在 `span` 中。
