...
Verkette ein weiteres `.then()` mit der bestehenden `.then()`-Methode. Übergib diesmal `data` als Parameter für die Callback-Funktion. For the callback, use curly braces because you will have more than one expression. Schreibe innerhalb deiner Callback-Funktion `data` auf die Konsole, um zu sehen, wie es aussieht.
