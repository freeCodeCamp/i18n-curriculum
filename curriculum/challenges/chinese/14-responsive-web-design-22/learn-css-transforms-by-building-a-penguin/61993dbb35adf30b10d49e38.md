...
应该使用 `.penguin-body` 选择器。

```js
assert.match(code, /\.penguin-body\s*\{/);
```

`.penguin-body` 的 `width` 应该是 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

`.penguin-body` 的 `height` 应该是 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

`.penguin-body` 的 `background` 的值应该为 `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)`。
