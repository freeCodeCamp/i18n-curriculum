...
現在你可以看到 `toggleItem()` 函式定義首次設定或變更的時間，你也應該記錄函式在不同呈現中保持相同的時間。

為你剛寫的 `if` 述語添加一個 `else` 述語。在 `else` 區塊內，將字串 `Current toggleItem function` 輸出到主控台。

# --hints--

你應該為你在上一步撰寫的 `if` 述語添加一個 `else` 述語。

```js
assert.match(code, /if\s*\(\s*prevToggleItem\s*!==\s*toggleItem\s*\)\s*{.*}\s*else\s*{/si);
```

你應該在 `else` 述語內將字串 `Current toggleItem function` 記錄到主控台。
