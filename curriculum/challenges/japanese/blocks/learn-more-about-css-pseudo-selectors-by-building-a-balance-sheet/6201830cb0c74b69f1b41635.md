...
テーブルがコンテナいっぱいに広がるように、`width` プロパティを `100%` に設定してください。その後、相対位置にして、上に `3rem` のマージンをつけてください。

# --hints--

`table` セレクターには `width` プロパティを `100%` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('width') === '100%');
```

`table` セレクターには `position` プロパティを `relative` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('position') === 'relative');
```

`table` セレクターには `margin-top` プロパティを `3rem` に設定してください。
