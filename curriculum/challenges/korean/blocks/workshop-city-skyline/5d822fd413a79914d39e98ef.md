...
`:root`에 `--window-color1`이라는 새 변수를 만들고 `black` 값을 할당하세요. 이것은 보라색 건물의 보조 색상이 될 것입니다.

# --hints--

`:root`에 `--window-color1`라는 새 변수를 만들어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--window-color1'));
```

속성 변수 `--window-color1`에 `black` 값을 할당해야 합니다.
