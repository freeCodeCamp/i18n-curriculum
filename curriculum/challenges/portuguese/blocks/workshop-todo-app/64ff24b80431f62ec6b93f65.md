...
Remova o array `myTaskArr` e todo o código para `localStorage` porque você não precisa mais deles.

# --hints--

Você deve remover `myTaskArr` e todo o código relacionado a `localStorage` que você acabou de aprender.
...
Você deve remover quaisquer referências restantes a `myTaskArr` em qualquer lugar do código.

```js
assert.notMatch(code, /myTaskArr/);
```

Você deve remover qualquer referência a `localStorage.getItem` para o item `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Você deve remover quaisquer referências restantes a `getTaskArrObj` em qualquer lugar no código.
