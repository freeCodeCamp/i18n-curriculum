...
El mismo patrón se aplica a los otros tres campos. Añade verificaciones equivalentes para `author`, `year` y `location`.

Ten en cuenta que `isValid` se acumula a lo largo de las cuatro verificaciones: si algún campo falla, se convierte en `false` y permanece `false` incluso si las demás verificaciones pasan. Esto significa que la función siempre inspecciona cada campo en lugar de detenerse en el primer problema.

# --hints--

`validateEntry(catalog[0])` debería devolver `true`.

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` debería devolver `false`.

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` debería devolver `false`.

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` debería devolver `false` porque la ubicación es `"Unknown"`.
