...
`async`/`await` 建構於 promises 之上，使撰寫和閱讀非同步程式碼更容易。當你在函式前加上 `async` 關鍵字，表示該函式將永遠傳回一個 `Promise`。`await` 關鍵字讓你可以在繼續執行下一行程式碼之前，等待一個 `Promise` 被決議，但它只能用在非同步函式內或模組的頂層區塊。以下是一個範例來說明 `async`/`await` 的運作方式：
