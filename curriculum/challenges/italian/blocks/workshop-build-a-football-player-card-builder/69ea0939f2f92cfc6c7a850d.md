...
Poi, aggiungi un hook `useEffect` che venga eseguito ogni volta che cambia lo stato `player`. Al suo interno, usa una funzione freccia e aggiungi un `try..catch` con `error` come parametro del blocco `catch`.

# --hints--

Dovresti chiamare `useEffect` all'interno di `FootballPlayerCard`.
...
Il tuo `useEffect` dovrebbe avere `[player]` come array di dipendenze.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

La callback `useEffect` dovrebbe contenere un blocco `try...catch` con `error` come parametro del blocco `catch`.
