...
أضف كتلة `else`. داخل كتلة `else`، استدعِ الدالة `playSong` مع `id` الأغنية التي تُشغل حاليًا كمعلمة.

هذا يضمن استمرار تشغيل الأغنية الحالية عند نقر زر التشغيل.

# --hints--

عند نقر زر `playButton` وكون `userData.currentSong` ليس `null`، يجب استدعاء الدالة `playSong` مع `id` الأغنية التي تُشغل حاليًا.
...
يجب أيضًا استدعاء الدالة `playSong` مع `id` أول أغنية في قائمة التشغيل عند نقر زر `playButton` وكون `userData.currentSong` يساوي `null`.
