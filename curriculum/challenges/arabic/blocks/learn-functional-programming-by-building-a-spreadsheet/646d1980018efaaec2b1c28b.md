...
استخدم نفس الصيغة النحوية لدالتك `addCharacters` لتحديث دالتك `elemValue`. يجب ألا تعلن عن `inner` بعد الآن، بل يجب أن تُرجع الدالة ضمنيًا.

# --hints--

يجب أن تستخدم دالتك `elemValue` إرجاعًا ضمنيًا.
...
يجب أن تُرجع دالتك `elemValue` ضمنيًا دالة سهمية تحتوي على معلمة `character`.
...
يجب أن تستخدم دالتك السهمية الداخلية إرجاعًا ضمنيًا.
...
يجب أن تُرجع دالتك السهمية الداخلية نتيجة استدعاء `idToText()` مع `character + num` كمعلمة.
