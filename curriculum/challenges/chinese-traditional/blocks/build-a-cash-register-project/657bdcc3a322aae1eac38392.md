...
在這裡你將建置一個收銀機應用程式，根據商品的價格、顧客提供的現金金額以及現金抽屜中的現金金額，將找零傳回給顧客。你還需要在不同情況下向使用者顯示不同的訊息，例如當顧客提供的現金不足或現金抽屜中沒有足夠的現金找零時。

在 `script.js` 檔案中，你已經獲得了 `price` 和 `cid` 變數。`price` 變數是該項目的價格，而 `cid` 變數是現金抽屜中的現金，這是一個列出現金抽屜中可用貨幣的二維陣列。

你還需要添加的另一個變數是 `cash` 變數，它是顧客為該項目提供的現金金額，該金額是透過頁面上的 `input` 元素提供的。

如果你想用不同的 `price` 和 `cid` 值來測試你的應用程式，請確保使用 `let` 關鍵字宣告它們，以便我們的測試可以重新指定它們。

你的應用程式應該根據商品的價格、顧客提供的現金金額，以及抽屜中的現金金額顯示不同的訊息：

- `"Status: INSUFFICIENT_FUNDS"`：如果 `cash-in-drawer` 少於應找的零錢，或你無法傳回正確的零錢。
- `"Status: CLOSED"`：如果 `cash-in-drawer` 與應找零金額相等。
- `"Status: OPEN"`：如果 `cash-in-drawer` 大於應找零金額且你可以找零，找零金額以硬幣和鈔票從高到低排序。
...
**目標：** 建置一個在功能上類似於 <a href="https://cash-register.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://cash-register.freecodecamp.rocks</a> 的應用程式。

**使用者故事：**

1. 你應該有一個 `input` 元素，其 `id` 為 `"cash"`。
1. 你應該有一個 `div`、`span` 或 `p` 元素，其 `id` 為 `"change-due"`。
1. 你應該有一個 `button` 元素，其 `id` 為 `"purchase-btn"`。
1. 當 `#cash` 元素中的值小於 `price` 時，應該出現一個警示，顯示文字 `"Customer does not have enough money to purchase the item"`。
1. 當 `#cash` 元素中的值與 `price` 相等時，`#change-due` 元素中的值應該是 `"No change due - customer paid with exact cash"`。
1. 當 `price` 是 `19.5` 時，`#cash` 元素中的值是 `20`，`cid` 是 `[["PENNY", 1.01], ["NICKEL", 2.05], ["DIME", 3.1], ["QUARTER", 4.25], ["ONE", 90], ["FIVE", 55], ["TEN", 20], ["TWENTY", 60], ["ONE HUNDRED", 100]]`，並且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: OPEN QUARTER: $0.5"`。
1. 當 `price` 是 `3.26` 時，`#cash` 元素中的值是 `100`，`cid` 是 `[["PENNY", 1.01], ["NICKEL", 2.05], ["DIME", 3.1], ["QUARTER", 4.25], ["ONE", 90], ["FIVE", 55], ["TEN", 20], ["TWENTY", 60], ["ONE HUNDRED", 100]]`，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: OPEN TWENTY: $60 TEN: $20 FIVE: $15 ONE: $1 QUARTER: $0.5 DIME: $0.2 PENNY: $0.04"`。
1. 當 `price` 是 `19.5` 時，`#cash` 元素中的值是 `20`，`cid` 是 `[["PENNY", 0.01], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 0], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]`，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: INSUFFICIENT_FUNDS"`。
1. 當 `price` 是 `19.5` 時，`#cash` 元素中的值是 `20`，`cid` 是 `[["PENNY", 0.01], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 1], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]`，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: INSUFFICIENT_FUNDS"`。
1. 當 `price` 是 `19.5` 時，`#cash` 元素中的值是 `20`，`cid` 是 `[["PENNY", 0.5], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 0], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]`，並且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: CLOSED PENNY: $0.5"`。
...
你應該有一個 `id` 為 `"change-due"` 的 `div`、`span` 或 `p` 元素。
...
你應該有一個 `button` 元素，其 `id` 為 `"purchase-btn"`。
...
當 `price` 是 `20`，`#cash` 元素中的值是 `10`，且點擊 `#purchase-btn` 元素時，應該出現一個警告，文字為 `"Customer does not have enough money to purchase the item"`。
...
當 `#cash` 元素中的值小於 `price`，且點擊 `#purchase-btn` 元素時，應該出現一個警告，顯示文字 `"Customer does not have enough money to purchase the item"`。
...
當 `price` 是 `11.95`，`#cash` 元素中的值是 `11.95`，並且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"No change due - customer paid with exact cash"`。
...
當 `#cash` 元素中的值與 `price` 相等，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"No change due - customer paid with exact cash"`。
...
當 `price` 是 `19.5` 時，`#cash` 元素中的值是 `20`，`cid` 是 `[["PENNY", 1.01], ["NICKEL", 2.05], ["DIME", 3.1], ["QUARTER", 4.25], ["ONE", 90], ["FIVE", 55], ["TEN", 20], ["TWENTY", 60], ["ONE HUNDRED", 100]]`，並且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: OPEN QUARTER: $0.5"`。
...
當 `price` 是 `3.26` 時，`#cash` 元素中的值是 `100`，`cid` 是 `[["PENNY", 1.01], ["NICKEL", 2.05], ["DIME", 3.1], ["QUARTER", 4.25], ["ONE", 90], ["FIVE", 55], ["TEN", 20], ["TWENTY", 60], ["ONE HUNDRED", 100]]`，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: OPEN TWENTY: $60 TEN: $20 FIVE: $15 ONE: $1 QUARTER: $0.5 DIME: $0.2 PENNY: $0.04"`。
...
當 `price` 小於 `#cash` 元素中的值、抽屜中現金總額 `cid` 大於應找零金額、各面額金額允許找零，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應為 `"Status: OPEN"`，並以由高到低排序的硬幣和鈔票顯示所需找零金額。
...
當 `price` 是 `19.5` 時，`#cash` 元素中的值是 `20`，`cid` 是 `[["PENNY", 0.01], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 0], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]`，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: INSUFFICIENT_FUNDS"`
...
當 `price` 小於 `#cash` 元素中的值且抽屜中的現金總額（`cid`）不足以涵蓋應找的零錢時，購買不應繼續進行。當在這些條件下點擊 `#purchase-btn` 時，`#change-due` 元素應顯示 `"Status: INSUFFICIENT_FUNDS"`。
...
當 `price` 是 `19.5` 時，`#cash` 元素中的值是 `20`，`cid` 是 `[["PENNY", 0.01], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 1], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]`，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: INSUFFICIENT_FUNDS"`。
...
當 `price` 小於 `#cash` 元素中的值，且抽屜中的現金總額 `cid` 大於應找零金額，但各面額金額使得無法傳回所需找零時，當點擊 `#purchase-btn` 元素，`#change-due` 元素中的值應為 `"Status: INSUFFICIENT_FUNDS"`
...
當 `price` 是 `19.5` 時，`#cash` 元素中的值是 `20`，`cid` 是 `[["PENNY", 0.5], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 0], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]`，並且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應該是 `"Status: CLOSED PENNY: $0.5"`。
...
當 `price` 小於 `#cash` 元素中的值時，抽屜中的現金總額 `cid` 與應找零金額相等，且點擊 `#purchase-btn` 元素時，`#change-due` 元素中的值應為 `"Status: CLOSED"`，並且應找零的硬幣和鈔票按從高到低的順序排列。
