...
Після вашого останнього `.divider` створіть ще один `p` елемент з текстом `Trans Fat 0g`. Виділіть курсивом слово `Trans`, обгорнувши його в `i` елемент. Додайте новому `p` елементу атрибут `class`, встановлений у `indent no-divider`. Обгорніть `Trans Fat 0g` у `span` елемент для вирівнювання.
...
Ваш новий `p` елемент має містити текст `Trans Fat 0g`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText === 'Trans Fat 0g');
```

Ваш новий елемент `p` повинен мати атрибут `class`, призначений значенню `indent no-divider`.
...
Ваш `span` елемент має обгорнути текст `Trans Fat 0g`.
