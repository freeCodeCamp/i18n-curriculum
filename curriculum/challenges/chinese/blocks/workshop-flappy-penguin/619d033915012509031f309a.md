...
在 `.face.right` 元素下方，添加一个 `div` 元素，并将其 `class` 设置为 `chin`。

# --hints--

你应该在 `.penguin-head` 内添加一个 `div` 元素。预期的 --fcc-expected-- `.penguin-head > div` 元素，但实际找到的是 --fcc-actual--。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 3);
```

你应该将 `div` 的 `class` 设置为 `chin`。

```js
assert.exists(document.querySelector('.penguin-head > div.chin'));
```

你应该将 `div` 放在 `.face.right` 元素下面。
