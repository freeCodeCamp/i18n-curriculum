...
Nella tua funzione `elemValue`, dichiara una funzione chiamata `inner` che prende un parametro `character`.

Poi, restituisci la tua funzione `inner`.

# --hints--

Dovresti dichiarare una variabile `inner`.
...
Dovresti usare `const` per dichiarare la tua variabile `inner`.
...
La tua variabile `inner` dovrebbe essere una funzione freccia.
...
La tua funzione `inner` dovrebbe avere `character` come unico parametro.
...
La tua funzione `inner` dovrebbe essere vuota.
...
Dovresti restituire esplicitamente la tua funzione `inner`.
