...
이제 `Email` 클래스에서 만든 `mark_as_read` 메서드를 테스트하세요. 기존 `Email` 객체 `email_obj`에 이 메서드를 사용해 상태를 변경하세요. 이메일을 읽음으로 표시한 후, 변경 사항을 확인하려면 `read`의 `email_obj` 속성을 출력하세요.
...
`mark_as_read` 메서드를 호출한 후 `email_obj`의 `read` 속성을 출력해야 합니다.
