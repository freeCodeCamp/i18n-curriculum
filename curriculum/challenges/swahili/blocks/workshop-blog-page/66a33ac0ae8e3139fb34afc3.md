...
Sasa umeishaongeza machapisho yote ya blogu, jaribu kubofya kiungo cha `Posts` na utagundua kuwa ukurasa unaruka hadi sehemu ya `Posts`.

Sehemu ya mwisho ya kuongeza kwenye ukurasa wako wa blogu itakuwa sehemu ya mawasiliano.

Chini ya kipengele cha `main`, ongeza kipengele cha `footer`.

# --hints--

Kipengele chako cha `footer` kinapaswa kuwa na lebo ya kufungua.

```js
assert.match(code, /<footer\>/);
```

Kipengele chako cha `footer` kinapaswa kuwa na lebo ya kufunga.

```js
assert.match(code, /<\/footer\>/);
```

Kipengele chako cha `footer` kinapaswa kuwa ndani ya kipengele chako cha `body`.
