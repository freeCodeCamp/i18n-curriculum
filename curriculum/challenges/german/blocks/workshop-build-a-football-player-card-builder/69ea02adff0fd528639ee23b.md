...
Erstellen Sie als Nächstes eine `loadPlayer`-Funktion mit `PlayerData` als Rückgabetyp. Fügen Sie darin eine `try...catch`-Syntax mit `error` als Parameter des `catch`-Blocks hinzu. Lassen Sie alles andere vorerst leer.

# --hints--

Sie sollten eine Funktion namens `loadPlayer` erstellen.
...
Ihre `loadPlayer`-Funktion sollte den Rückgabetyp `PlayerData` haben.
...
Ihre `loadPlayer`-Funktion sollte einen `try...catch`-Block enthalten.
