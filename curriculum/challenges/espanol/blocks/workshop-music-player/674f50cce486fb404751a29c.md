...
Ahora que sabes cómo encontrar la próxima canción para reproducir, crea una función llamada `playNextSong`. Dentro de la función `playNextSong`, crea una declaración `if` para verificar si el `currentSong` de `userData` es estrictamente igual a `null`. Esto verificará si no hay una canción actual reproduciéndose en el objeto `userData`.

Si la condición es verdadera, llama a la función `playSong` con el `id` de la primera canción en el array `userData.songs` como argumento.

Luego agrega una sentencia `return` al final del bloque `if`.

# --hints--

Deberías tener una función llamada `playNextSong`.

```js
assert.isFunction(playNextSong);
```

Cuando el `currentSong` de `userData` es estrictamente igual a `null`, tu función `playNextSong` debería llamar a la función `playSong` con el `id` de la primera canción en el array `userData.songs` como argumento.
...
Debes tener una sentencia `return` al final del bloque `if`.
