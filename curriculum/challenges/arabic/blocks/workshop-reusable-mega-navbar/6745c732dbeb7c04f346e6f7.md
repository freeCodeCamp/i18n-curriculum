...
أنشئ عنصر `ul` داخل عنصر `nav`. داخل ذلك العنصر `ul`، أنشئ ثلاثة عناصر قائمة.

# --hints--

يجب أن تنشئ عنصر `ul` داخل عنصر `nav` الخاص بك.

```js
assert.exists(document.querySelector('nav ul'));
```

يجب أن تنشئ ثلاثة عناصر `li` داخل عنصر `ul` الخاص بك.
