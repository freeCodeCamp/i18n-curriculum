...
Você provavelmente já viu um atributo `alt` em uma tag `img` em outros desafios. O texto `alt` descreve o conteúdo da imagem e fornece uma alternativa em texto para ela.Um atributo `alt` ajuda nos casos onde a imagem não carrega ou não pode ser vista pelo usuário.Os mecanismos de busca também a utilizam para compreender o que uma imagem contém para a incluir nos resultados de pesquisa.Aqui está um exemplo:
...
O Camper Cat é tanto um ninja do código quanto um ninja de verdade. Ele está construindo um site para compartilhar seu conhecimento.A foto de perfil que ele quer usar mostra suas habilidades e deve ser apreciada por todos os visitantes do site.Adicione um atributo `alt` na tag `img`, que explique que o Camper Cat está fazendo karatê. (O `src` da imagem não aponta para um arquivo real, então você deve ver o texto do `alt` na exibição.)
