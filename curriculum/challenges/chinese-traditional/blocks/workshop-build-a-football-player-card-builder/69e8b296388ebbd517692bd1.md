...
再新增一個 `className` 為 `form-group` 的 `div`。在其中，新增一個 `label` 元素，其 `className` 為 `label`、`htmlFor` 為 `imageUrl`，以及文字 `Image URL`。接著新增一個 `input` 元素，其 `id` 為 `imageUrl`、`className` 為 `input`，以及 `type` 為 `text`。
