...
マウスカーソルを合わせた時のリンクのプロパティを変更するには、`a:hover { propertyName: propertyValue; }` というような<dfn>疑似セレクター</dfn>を使用します。

フッターの `Visit our website` というリンクにユーザーがカーソルを合わせた時に、その色が `brown` に変わるようにしてください。

# --hints--

`a:hover` という疑似セレクターを使用する必要があります。
...
`color` プロパティを `brown` に設定する必要があります。
...
`a:hover` の `color` は `brown` である必要があります。
