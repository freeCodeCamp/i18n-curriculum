...
نعود إلى الدالة `initializeApp`، هنا ستستخدم الأنواع المضمَّنة والدالة `getElement()` التي أنشأتها سابقًا للحصول على عناصر DOM اللازمة للتطبيق.

داخل الدالة `initializeApp`، أعد تعيين `shapeTypeSelect` باستدعاء `getElement()` مع القيمة `"shape-type"`، ثم استخدم الكلمة المفتاحية `as` لتحويل النتيجة إلى `HTMLSelectElement`.

# --hints--

يجب أن تعيد تعيين `shapeTypeSelect` بالقيمة المناسبة.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

يجب أن تستدعي الدالة `getElement` مع `shape-type` وتعيّن النتيجة إلى `shapeTypeSelect`.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

يجب أن تستدعي `getElement` مع `HTMLSelectElement` كنوع لـ `shapeTypeSelect` باستخدام الكلمة المفتاحية `as`.
