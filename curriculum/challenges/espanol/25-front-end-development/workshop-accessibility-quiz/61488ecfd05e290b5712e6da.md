...
Para hacer que la primera sección parezca más en línea, apunta solo a los elementos `input` dentro de los elementos `.info`, y establece su propiedad `width` en `50%`, y alinea su texto a la izquierda.

# --hints--

Debes usar ya sea el selector `.info input` o `.info > input`.
...
Debes dar a los elementos `input` una propiedad `width` de `50%`.
...
Debes dar a los elementos `input` una alineación de texto `text-align` de `left`.
