...
`.face` 要素に `width` の `60%`、`height` の `70%`、および `background-color` の `white` を設定してください。

# --hints--

`.face` セレクターを使う必要があります。

```js
assert.match(code, /\.face\s*\{/);
```

`.face` に `width` プロパティを設定する必要があります。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.width);
```

`.face` に `width` の `--fcc-expected--` を設定する必要がありますが、`--fcc-actual--` が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.width, '60%');
```

`.face` に `height` プロパティを設定する必要があります。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.height);
```

`.face` に `height` の `--fcc-expected--` を設定する必要がありますが、`--fcc-actual--` が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.height, '70%');
```

`.face` に `background-color` プロパティを設定する必要があります。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.backgroundColor);
```

`.face` に `background-color` の `--fcc-expected--` を設定する必要がありますが、`--fcc-actual--` が見つかりました。
