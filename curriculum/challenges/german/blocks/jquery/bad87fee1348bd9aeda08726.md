...
Du solltest dein `$(document).ready(function() {` am Anfang deines `script`-Elements belassen.

```js
assert(code.match(/\$\(document\)\.ready\(function\(\)\s?\{/g));
```

Du solltest das abschließende `})` der Funktion `document.ready` intakt lassen.

```js
assert(code.match(/.*\s*\}\);/g));
```

Du solltest den schließenden Tag deines `script`-Elements intakt lassen.
