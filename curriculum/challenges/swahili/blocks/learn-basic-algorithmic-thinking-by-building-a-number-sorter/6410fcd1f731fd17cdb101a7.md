...
Ndani ya mzunguko wako uliopangwa ndani wa `for`, ongeza wito wa `console.log()` ili ukague thamani za `array`, `array[j]`, na `array[minIndex]` katika kila mzunguko. Unaweza kubofya kitufe cha `Sort` kuona jinsi algoriti yako inavyopitia safu ya data.
...
Unapaswa kuwa na wito wa `console.log()` ndani ya mzunguko wako uliopangwa ndani wa `for`.
...
Unapaswa kupitisha `array` kama hoja ya kwanza kwa `console.log()`.
...
Unapaswa kupitisha `array[j]` kama hoja ya pili kwa `console.log()`.
...
Unapaswa kupitisha `array[minIndex]` kama hoja ya tatu kwa `console.log()`.
...
Unapaswa kuwa na usemi wa `if` baada ya wito wako wa `console.log()`.
...
Unapaswa kuwa na hali katika usemi wako wa `if` inayokagua kama `array[j]` ni ndogo kuliko `array[minIndex]`.
...
Usemi wako wa `if` unapaswa kuweka `minIndex` kuwa `j`.
