...
Setzen Sie `font-size` auf `1.1rem`, `display` auf `block` und `margin` auf `auto`, um die Schaltfläche zu zentrieren.

# --hints--

Sie sollten weiterhin die `.submit-btn`-Klasse ansprechen.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Setzen Sie die `font-size`-Eigenschaft auf `1.1rem`.
...
Setzen Sie die `margin`-Eigenschaft auf `auto`.
