...
Die Schleife ist abgeschlossen. Geben Sie `grouped` aus der Funktion zurück.

Rufen Sie dann unterhalb der Funktion `groupByDecade(catalog)` auf und weisen Sie das Ergebnis einer Variablen namens `byDecade` zu. Protokollieren Sie `byDecade`, um das gruppierte Ergebnis zu überprüfen. Sie sollten ein Objekt mit Schlüsseln wie `"1950s"`, `"1960s"`, `"1970s"` und so weiter sehen, die jeweils auf ein Array von Büchern aus diesem Jahrzehnt verweisen.

# --hints--

`groupByDecade` sollte ein Objekt zurückgeben.

```js
assert.isObject(groupByDecade(catalog));
```

`byDecade["1980s"].length` sollte `5` sein.

```js
assert.strictEqual(groupByDecade(catalog)["1980s"].length, 5);
```

`byDecade["Unknown"].length` sollte `1` sein.

```js
assert.strictEqual(groupByDecade(catalog)["Unknown"].length, 1);
```

Sie sollten das Ergebnis von `groupByDecade(catalog)` einer Variablen namens `byDecade` zuweisen.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+byDecade\s*=\s*groupByDecade\s*\(\s*catalog\s*\)/);
```

Sie sollten `byDecade` in der Konsole protokollieren.
