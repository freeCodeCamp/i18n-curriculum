...
最後に、文字列の補間を使って段落要素のテキストを `decimalToBinary(${currVal})` に設定してください。ここで `currVal` は現在のオブジェクトの `inputVal` プロパティです。この後、数値入力に `5` を入力し、`Convert` ボタンをクリックしてコードをテストしてください。

# --hints--

段落要素のテキストを `decimalToBinary(${obj.inputVal})` に設定するには文字列の補間を使う必要があります。
