...
Declare a `currValue` variable and assign it the value at `array[i]`. Deklariere dann eine `j`-Variable und weise ihr `i - 1` zu. Deine `j`-Variable sollte neu zuweisbar sein.

# --hints--

Du solltest eine `currValue`-Variable mit `const` deklarieren.
...
Du solltest eine `j`-Variable mit Hilfe von `let` deklarieren.
...
Du solltest `j` den Wert `i - 1` zuweisen.
