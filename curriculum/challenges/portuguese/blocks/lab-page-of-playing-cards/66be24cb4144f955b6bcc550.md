...
1. Você deve construir uma página que exiba pelo menos três cartas de baralho.
1. Você deve ter um elemento `main` com um ID `playing-cards`.
1. Dentro do seu elemento `#playing-cards`, você deve ter pelo menos três elementos `div`, cada um com a classe `card`.
1. Dentro de cada elemento `.card`, você deve ter três elementos `div`, o primeiro com a classe `left`, o segundo com a classe `middle` e o terceiro com a classe `right`.
1. Seu elemento `#playing-cards` deve usar flexbox para centralizar horizontalmente seus filhos, permitir que eles quebrem a linha e colocar um espaço de `20px` entre eles.
1. Cada um dos seus elementos `.card` deve usar flexbox para justificar seus filhos usando `space-between`.
1. Cada um dos seus elementos `.left` deve usar propriedades de item do flexbox para se alinhar no início do container pai.
1. Cada um dos seus elementos `.middle` deve usar propriedades de item do flexbox para se alinhar no centro do container pai.
1. Cada um dos seus elementos `.right` deve usar propriedades de item do flexbox para se alinhar no final do container pai.
1. Cada um dos seus elementos `.middle` deve usar flexbox para exibir seus filhos em uma coluna.
