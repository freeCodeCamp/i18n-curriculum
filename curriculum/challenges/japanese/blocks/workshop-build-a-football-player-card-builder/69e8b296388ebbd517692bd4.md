...
`div`の中に`className`の`form-group`を持つ`stats-grid`要素を2つ追加してください。1つはPAS用、もう1つはDRI用です。

PAS用には、`label`の`htmlFor`を持ちテキストが`pas`の`PAS`を追加し、その後に`id`の`pas`と`type`の`number`を持つ入力を追加してください。

DRI用には、`label`の`htmlFor`を持ちテキストが`dri`の`DRI`を追加し、その後に`id`の`dri`と`type`の`number`を持つ入力を追加してください。

# --hints--

`div`の`className`の`form-group`の中に、`div`の`className`を持つ`stats-grid`要素をさらに2つ追加してください。
...
`div`の`className`の3番目の`form-group`要素には、`label`の`htmlFor`属性を持つ`pas`、`className`の`label`、そしてテキスト`PAS`を追加してください。
...
`div`の`className`の3番目の`form-group`要素には、`input`の`className`を持つ`input`、`id`の`pas`、そして`type`の`number`を追加してください。
...
`div`の`className`の4番目の`form-group`要素には、`label`の`htmlFor`属性を持つ`dri`、`className`の`label`、そしてテキスト`DRI`を追加してください。
...
`div`の`className`の4番目の`form-group`要素には、`input`の`className`を持つ`input`、`id`の`dri`、そして`type`の`number`を追加してください。
