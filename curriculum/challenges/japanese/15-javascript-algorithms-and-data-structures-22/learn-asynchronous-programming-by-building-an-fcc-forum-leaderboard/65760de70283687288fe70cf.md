...
数値の `417` を新しいキーとして追加して、値を空のオブジェクトにしてください。

そのオブジェクト内に、`category` というキーと `"freeCodeCamp Support"` という文字列の値を持つプロパティを追加してください。

そのプロパティの下にもう 1 つ、`className` というキーと `"support"` という文字列の値を追加してください。

# --hints--

`allCategories` オブジェクトには `417` というプロパティが必要です。

```js
assert.property(allCategories, '417');
```

`417` の値はオブジェクトであるべきです。

```js
assert.isObject(allCategories['417'])
```

`417` オブジェクトには `category` というプロパティが必要です。

```js
assert.property(allCategories['417'], 'category');
```

`category` プロパティを文字列 `"freeCodeCamp Support"` に設定してください。

```js
assert.equal(allCategories["417"].category, 'freeCodeCamp Support');
```

`417` オブジェクトには `className` というプロパティが必要です。

```js
assert.property(allCategories['417'], 'className');
```

`className` プロパティを文字列 `"support"` に設定してください。
