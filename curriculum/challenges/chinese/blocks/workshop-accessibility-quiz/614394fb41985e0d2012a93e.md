...
问题编号的描述性不够。 对视障用户来说尤其如此。 解决这个问题的方法之一是在元素中添加只有屏幕阅读器才能读取的文本，而无需在元素中添加可见文本。

在每个 `h3` 元素的数字前面嵌套一个带有 `class` 为 `sr-only` 的 `span` 元素。
...
你应该给第一个 `span` 元素一个 `class`，值为 `sr-only`。
