...
داخل كائن `allCategories`، أضف مفتاحًا جديدًا للرقم `299` بقيمة كائن فارغ.

داخل ذلك الكائن، أضف خاصية بمفتاح `category` وقيمة سلسلة نصية `"Career Advice"`. أسفل تلك الخاصية، أضف مفتاحًا آخر يسمى `className` بقيمة سلسلة نصية `"career"`.

# --hints--

يجب أن يحتوي كائن `allCategories` على خاصية `299`.

```js
assert.property(allCategories, '299');
```

يجب أن تكون قيمة `299` كائنًا.

```js
assert.isObject(allCategories['299'])
```

يجب أن يحتوي كائن `299` على خاصية `category`.

```js
assert.property(allCategories['299'], 'category');
```

يجب تعيين خاصية `category` إلى السلسلة النصية `"Career Advice"`.

```js
assert.equal(allCategories["299"].category, 'Career Advice');
```

يجب أن يحتوي كائن `299` على خاصية `className`.

```js
assert.property(allCategories['299'], 'className');
```

يجب تعيين خاصية `className` إلى السلسلة النصية `"career"`.
