...
现在更新 `button2.innerText` 和 `button3.innerText`，分别分配 `"button text"` 数组的第二个和第三个值。

# --hints--

你应该访问 `location` 参数的 `"button text"` 属性的第二个元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*1\s*\]/);
```

你应该将 `button2.innerText` 属性设置为 `location` 参数的 `"button text"` 属性的第二个元素。

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*location\s*\[\s*('|")button text\1\s*\]\s*\[\s*1\s*\]/);
```

你应该访问 `location` 参数的 `"button text"` 属性的第三个元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*2\s*\]/);
```

你应该将 `button3.innerText` 属性设置为 `location` 参数的 `"button text"` 属性的第三个元素。
