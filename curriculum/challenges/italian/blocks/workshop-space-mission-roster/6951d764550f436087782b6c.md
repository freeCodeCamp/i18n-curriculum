...
Hai aggiunto una chiamata `console.log()` al tuo script per convalidare il tuo array `EVAReadySquad`, e ora la chiamata non è più necessaria. Per evitare che il terminale si riempia nei passaggi successivi, procedi ed elimina il loop `for` attuale che registra `EVAReadySquad` astronauti nella console.

# --hints--

Elimina il loop `for` che registra `EVAReadySquad` astronauti nella console. Dovresti eliminare l'intero blocco del loop.
