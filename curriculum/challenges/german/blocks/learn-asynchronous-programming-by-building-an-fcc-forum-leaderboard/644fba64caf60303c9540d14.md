...
Nach dem `href` Attribut, setzen Sie das `class` Attribut auf die Konstante namens `linkClass`.

Nach dem `class` Attribut setzen Sie das `target` Attribut auf `"_blank"`.

Zuletzt, platzieren Sie die `linkText` Konstante zwischen den Anker-Tags, um den Text im Link anzuzeigen.

# --hints--

Sie sollten ein `class` Attribut hinter dem `href` Attribut haben.
...
Sie sollten den Wert des `class` Attributs auf die Konstante namens `linkClass` setzen.
...
Sie sollten die `linkText` Konstante zwischen den Anker-Tags platzieren.
