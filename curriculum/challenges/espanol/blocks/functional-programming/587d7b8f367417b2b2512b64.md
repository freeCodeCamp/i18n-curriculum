...
`[23, 65, 98, 5, 13].myFilter(item => item % 2)` debería ser igual a `[23, 65, 5, 13]`.
...
`["naomi", "quincy", "camperbot"].myFilter(element => element === "naomi")` debería devolver `["naomi"]`.
...
`[1, 1, 2, 5, 2].myFilter((element, index, array) => array.indexOf(element) === index)` debería devolver `[1, 2, 5]`.
