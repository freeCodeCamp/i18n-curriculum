...
الآن بعد أن أصلحت الأخطاء البرمجية وأصبحت ألوان المباني صحيحة، يمكنك إزالة قيم النظام البديل في الموضعين اللذين استُخدما فيهما. تابع وافعل ذلك الآن.

# --hints--

يجب إزالة النظام البديل في `background-color` من `.bb2`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2')?.getPropVal('background-color', true), 'var(--building-color2)');
```

يجب إزالة النظام البديل في `background-color` من `.bb3`.
