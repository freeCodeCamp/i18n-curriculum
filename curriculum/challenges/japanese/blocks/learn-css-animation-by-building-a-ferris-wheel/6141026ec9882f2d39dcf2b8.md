...
`0%` と `100%` セレクターの間に `50%` セレクターを追加してください。これはアニメーションサイクルの途中で適用されます。`background-color` を `purple` に設定してください。

# --hints--

`50%` ルール内に新しい `@keyframes cabins` セレクターを作成してください。
...
`50%` セレクターは `0%` と `100%` セレクターの間にあるべきです。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '50%');
```

`50%` セレクターには `background-color` プロパティを `purple` に設定してください。
