...
Lass die `h3`Elemente durch die folgenden CSS-Eigenschaften höher priorisiert erscheinen:
...
Du solltest den `h3`-Element-Selektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3'));
```

Du solltest beim `h3`-Element `margin-top` auf `5px` stellen.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Du solltest beim `h3`-Element `padding-left` auf `15px` stellen.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Du solltest beim `h3`-Element die `font-size` auf `1.375ren` stellen.
