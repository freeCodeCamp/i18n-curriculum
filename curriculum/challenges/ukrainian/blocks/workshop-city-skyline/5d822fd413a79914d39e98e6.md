...
Вкладіть шість елементів `div` всередину `.foreground-buildings` і надайте їм класи від `fb1` до `fb6` у цьому порядку. "fb" означає "foreground building" (будівля на передньому плані). Це будуть ще шість будівель для переднього плану.

# --hints--

Ви повинні створити новий `div` з класом `fb1`.

```js
assert.exists(document.querySelector('div.fb1'));
```

Ви повинні створити новий `div` з класом `fb2`.

```js
assert.exists(document.querySelector('div.fb2'));
```

Ви повинні створити новий `div` з класом `fb3`.

```js
assert.exists(document.querySelector('div.fb3'));
```

Ви повинні створити новий `div` з класом `fb4`.

```js
assert.exists(document.querySelector('div.fb4'));
```

Ви повинні створити новий `div` з класом `fb5`.

```js
assert.exists(document.querySelector('div.fb5'));
```

Ви повинні створити новий `div` з класом `fb6`.

```js
assert.exists(document.querySelector('div.fb6'));
```

Ви повинні розмістити ці нові елементи `div` всередині елемента `.foreground-buildings`.
...
Ви повинні розмістити нові елементи `div` у правильному порядку.
