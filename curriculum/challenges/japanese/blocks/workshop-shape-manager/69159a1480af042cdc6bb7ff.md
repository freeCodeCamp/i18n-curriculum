...
まだ`handleShapeSelect`関数の中にいて、`hasSelection`が存在するかどうかをチェックする`val.value`変数を作成してください。必ず`Boolean()`で囲んでください。

その下で、`toggleResultCard()`変数を使って`hasSelection`関数を呼び出してください。

これにより、実際に図形が選択されている場合にのみ結果カードが表示されます。

# --hints--

`hasSelection`変数を作成してください。
