...
En JSX y TSX, pasas expresiones de JavaScript como valores de atributos usando llaves. Las plantillas literales usan backticks y `${}` para la interpolación. Por ejemplo:

```tsx
<div className={`prefix-${someVariable}`}>
```

El contenedor de la tarjeta `className` está actualmente codificado como `card-wrapper tier-gold`. Reemplaza `gold` con un valor dinámico usando una plantilla literal que llame a `getPlayerTier` con `player.overallRating`. Esto hará referencia a la clase de nivel correcta del CSS, basada en la calificación general del jugador.

# --hints--

Debes usar una plantilla literal con `getPlayerTier(player.overallRating)` para el `className` de `.card-wrapper`.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

El elemento `.card-wrapper` debe tener la clase `tier-elite` cuando la calificación del jugador sea 98.
