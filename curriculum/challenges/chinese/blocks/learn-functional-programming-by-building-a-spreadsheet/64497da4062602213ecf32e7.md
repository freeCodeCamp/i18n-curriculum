...
你应该对 `element` 的 `value` 属性调用 `.replace()` 方法。
...
你应该传递一个正则表达式来将空格匹配到你的 `.replace()` 方法。 使用 `\s` 字符类。
