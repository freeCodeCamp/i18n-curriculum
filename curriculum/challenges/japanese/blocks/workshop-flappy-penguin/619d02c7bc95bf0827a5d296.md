...
`.face` クラスを持つ `right` 要素をターゲットにして、その親要素の右側に `5%` で配置してください。

# --hints--

`.face.right` セレクターを使うべきです。

```js
assert.match(code, /\.face\.right\s*\{/);
```

`.face.right` に `right` プロパティを与えるべきです。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face.right')?.right);
```

`.face.right` に `right` を `--fcc-expected--` に設定するべきですが、`--fcc-actual--` が見つかりました。
