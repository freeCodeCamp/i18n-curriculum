...
Unapaswa kuongeza vipengele viwili vya `div` ndani ya `.penguin-head`. Ilitarajiwa vipengele vya `div`, lakini vilipatikana --fcc-actual--.

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 9);
```

Unapaswa kumpa `div` mpya ya kwanza `class` ya `beak`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

Unapaswa kumpa `div` mpya ya pili `class` ya `beak`.

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div.beak'), 2);
```

Unapaswa kumpa `div` mpya ya kwanza `class` ya `top`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

Unapaswa kumpa `div` mpya ya pili `class` ya `bottom`.
