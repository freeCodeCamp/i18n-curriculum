...
现在是时候使用你的 `handleToggleVisibility` 函数了。 

在之前的课程中，你学习了 React 的合成事件系统以及如何使用诸如 `onClick` 和 `onSubmit` 之类的属性。

请记住，你可以将函数引用传递给 `onClick` 属性，如下所示：
...
更新现有的 `button` 元素以添加 `onClick` 属性并传入 `handleToggleVisibility` 函数引用。 

现在，当你单击按钮时，你将看到段落文本的可见性在页面上的隐藏和显示之间切换。 

# --hints--

当单击 `toggle-button` 时，屏幕上应出现消息 `"I love freeCodeCamp!"`。
