...
在你的 `.calories-info` 元素中，创建一个 `div` 元素。 将该 `div` 元素的 `class` 属性设置为 `left-container`。 在新创建的 `div` 元素中，创建一个带有文本 `Amount per serving` 的 `h2` 元素。 将 `h2` 元素的 `class` 属性设置为 `bold small-text`。

# --hints--

你应该在 `.calories-info` 元素中创建一个新的 `div` 元素。

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.localName === 'div');
```

你的新的 `div` 元素应该将 `class` 设置为 `left-container`。

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.classList.contains('left-container'));
```

你应该在 `.left-container` 元素中创建一个新的 `h2` 元素。

```js
assert(document.querySelector('.left-container')?.children?.[0]?.localName === 'h2');
```

你的新的 `h2` 元素应该有一个 `class` 属性，设置为 `bold small-text`。
...
你的新的 `h2` 元素应包含文本 `Amount per serving`。
