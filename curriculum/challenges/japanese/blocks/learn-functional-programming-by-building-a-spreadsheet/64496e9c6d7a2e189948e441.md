...
これでスプレッドシートの関数を使い始めることができます。まずは`update`のアロー関数を宣言してください。`event`のパラメータを受け取るようにします。
...
`const`を使って`update`の変数を宣言してください。

```js
assert.match(code, /const\s+update/);
```

`update`の変数は関数である必要があります。

```js
assert.isFunction(update);
```

`update`の関数は`event`のパラメータを受け取る必要があります。

```js
assert.match(code, /const\s+update\s*=\s*(\(\s*event\s*\)|event)\s*=>/);
```

`update`の関数は空である必要があります。
