...
仅当输入字段中存在一些文本时，才应该向用户显示 `Cancel` 和 `Discard` 按钮。

首先删除 `confirmCloseDialog.showModal();` 行。

然后在 `closeTaskFormBtn` 事件监听器内，编写一些逻辑以查看 `titleInput`、`dateInput` **或** `descriptionInput` 字段中是否有值。

如果这些输入字段中有任何一个有值，则在 `confirmCloseDialog` 上使用 `showModal()` 方法。 否则，如果没有变化，则调用 `reset()` 函数清除输入字段并隐藏表单模式。

# --hints--

如果任何一个输入字段有值，则应该显示确认对话框。 否则，应该调用 `reset` 函数。
