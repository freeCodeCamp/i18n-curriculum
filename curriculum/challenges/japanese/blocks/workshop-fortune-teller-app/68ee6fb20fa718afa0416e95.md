...
前の `multipleCard` と同様に、再度 `multipleCard` 要素を取得し、`innerHTML` を繰り返し処理して `DrawingType` を設定してください。`Object.values()` を使い、`map` を引数として渡して `type` メソッドを呼び出してください。

# --hints--

`multipleCard.innerHTML` を呼び出して `Object.values` に代入してください。

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

`DrawingType` メソッドに `Object.values` を渡してください。

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

`map` の `Object.values` メソッドを呼び出してください。

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

`map` メソッドは `type` という名前のパラメータを持つべきです。
