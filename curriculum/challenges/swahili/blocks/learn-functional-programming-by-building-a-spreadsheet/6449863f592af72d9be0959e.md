...
Baada ya `rangeRegex` kupata `:`, inahitaji kutafuta muundo huo huo wa herufi na nambari kama ilivyokuwa awali.

Nakili makundi yako mawili ya kukamata yaliyopo na ubandike baada ya nukta ya wima.

# --hints--

Unapaswa kuongeza kundi la tatu la kukamata kwenye `rangeRegex`, baada ya nukta ya wima.
...
Kundi lako la tatu la kukamata linapaswa kutumia darasa la herufi.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[.*\]\)/);
```

Kundi lako la tatu la kukamata linapaswa kulingana na herufi `A` hadi `J`.
...
Unapaswa kuongeza kundi la nne la kukamata kwenye `rangeRegex`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(.*\)/);
```

Kundi lako la nne la kukamata linapaswa kulingana na nambari moja au mbili.
