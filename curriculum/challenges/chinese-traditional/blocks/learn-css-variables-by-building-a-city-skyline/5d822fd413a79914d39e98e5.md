...
你想讓 `.foreground-buildings` 容器直接位於 `.background-buildings` 元素的正上方。為它設定 `width` 和 `height` 為 `100%`，將 `position` 設為 `absolute`，並將 `top` 設為 `0`。這會使它與 body 大小相同，並將起點移動到左上角。

# --hints--

你應該使用 `.foreground-buildings` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.foreground-buildings'));
```

你應該為 `.foreground-buildings` 元素設定 `width` 為 `100%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.width, '100%');
```

你應該給 `.foreground-buildings` 元素一個 `height` 為 `100%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.height, '100%');
```

你應該為 `.foreground-buildings` 元素指定 `position` 為 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.position, 'absolute');
```

你應該給 `.foreground-buildings` 元素一個 `top` 為 `0`。
