...
여러 `padding` 속성으로 메뉴의 내용과 가장자리 사이에 공간을 줄 수 있습니다.

`menu` 클래스에 `padding-left`와 같은 값인 `padding-right`를 `20px`로 지정하세요.

# --hints--

`padding-left` 속성을 `20px`로 설정해야 합니다.
...
`padding-right` 속성을 `20px`로 설정해야 합니다.
...
`.menu` 요소에 `padding-left`가 `20px`이어야 합니다.
...
`.menu` 요소에 `padding-right`가 `20px`이어야 합니다.
