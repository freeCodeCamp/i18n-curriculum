...
Створіть функцію з іменем `playPreviousSong`. Усередині нової функції використайте значення, яке повертає функція `getPreviousSong`, щоб знайти пісню для відтворення. Коли `getPreviousSong()` дорівнює `undefined`, відтворюйте першу пісню у вашому плейлисті. Інакше відтворюйте пісню перед поточною, що відтворюється.

# --hints--

У вас має бути функція з іменем `playPreviousSong`.

```js
assert.isFunction(playPreviousSong);
```

Функція `playPreviousSong` має відтворювати пісню перед поточною, що відтворюється у плейлисті.
...
Коли `getPreviousSong()` дорівнює `undefined`, функція `playPreviousSong` має відтворювати першу пісню у плейлисті.
