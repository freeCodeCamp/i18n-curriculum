...
`.key.black--key::after`に対して、`width`を`32px`に、`height`を`100px`に設定してください。

# --hints--

`.key.black--key::after`には`width`プロパティを`32px`に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.key.black--key::after')?.width, '32px');
```

`.key.black--key::after`には`height`プロパティを`100px`に設定してください。
