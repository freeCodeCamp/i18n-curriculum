...
創建一個 `.social-icons` 選擇器。為它設定 `display` 屬性為 `grid`，以及 `font-size` 屬性為 `3rem`。

# --hints--

你應該有一個 `.social-icons` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

你的 `.social-icons` 選擇器應該有一個設定為 `grid` 的 `display` 屬性。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

你的 `.social-icons` 選擇器應該設定 `font-size` 屬性為 `3rem`。
