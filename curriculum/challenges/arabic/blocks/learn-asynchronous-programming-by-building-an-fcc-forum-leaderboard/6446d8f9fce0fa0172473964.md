...
تم إكمال بقية كائن `allCategories` من أجلك.

في الخطوات القليلة التالية، ستنشئ دالة لاسترجاع اسم الفئة من كائن `allCategories`.

ابدأ بإنشاء دالة سهمية باسم `forumCategory`، مع استخدام `id` كاسم للمعلمة.

# --hints--

يجب إنشاء دالة فارغة باسم `forumCategory`.

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

يجب أن تحتوي دالة `forumCategory` على معلمة باسم `id`.
