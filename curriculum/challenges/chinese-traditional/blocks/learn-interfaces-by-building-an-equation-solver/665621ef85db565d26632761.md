...
在你的新函式（程式）中，創建一個 `if` 述語，檢查 `equation` 是否不是 `Equation` 類別的實體，並使用字串 `'Argument must be an Equation object'` 引發 `TypeError` 以提供訂製訊息。

# --hints--

你應該在你的 `solver` 函式（程式）中創建一個 `if` 述語來檢查 `equation` 是否不是 `Equation` 類別的實體。
...
你應該在新的 `if` 述語中使用提供的字串引發 `TypeError`。
