...
A árvore de acessibilidade é uma estrutura usada por tecnologias assistivas, como leitores de tela, para interpretar e interagir com o conteúdo de uma página.
...
- **`display: none;`**: Usar `display: none;` significa que leitores de tela e outras tecnologias assistivas não terão acesso a esse conteúdo, pois ele não está incluído na árvore de acessibilidade. Portanto, é importante usar esse método apenas quando quiser remover completamente o conteúdo tanto da apresentação visual quanto da acessibilidade.
- **`visibility: hidden;`**: Essa propriedade e valor escondem o conteúdo visualmente, mas o mantêm no fluxo do documento, ou seja, ele ainda ocupa espaço na página. Esses elementos também não serão mais lidos por leitores de tela porque terão sido removidos da árvore de acessibilidade.
- **Classe CSS `.sr-only`**: Essa é uma técnica comum usada para esconder visualmente o conteúdo enquanto o mantém acessível para leitores de tela.
...
No exemplo acima, a rolagem suave é habilitada se o usuário não tiver definido uma preferência por movimento reduzido no dispositivo.
...
Usar texto de marcador de posição não é ideal para acessibilidade. Frequentemente, os usuários confundem o texto do marcador de posição com um valor real de entrada e pensam que já há um valor no campo.
