...
對於遞歸情況，添加 `else` 語句並 `return` 調用 `decimalToBinary()` 的結果。 傳入 `input` 除以 `2` 並使用 `Math.floor()` 四捨五入作爲參數。

# --hints--

你應該將 `if` 語句轉換爲 `if...else` 語句。

```js
assert.match(String(decimalToBinary), /\{\s*return\s+('|"|`)\1\s*;?\s*\}\s*else\s*\{/);
```

在 `else` 語句主體中，你應該 `return` 調用 `decimalToBinary` 的結果。

```js
assert.match(String(decimalToBinary), /else\s*\{\s*return\s+decimalToBinary\(/);
```

你應該將 `input` 除以 `2` 並使用 `Math.floor()` 向下舍入的結果作爲 `decimalToBinary` 的參數傳入。
