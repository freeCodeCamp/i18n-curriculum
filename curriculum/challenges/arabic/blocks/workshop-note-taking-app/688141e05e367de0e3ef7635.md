...
في الوقت الحالي، يُهيأ `currentContent` بقيمة سلسلة نصية فارغة. لكن النتيجة المطلوبة هي أن يحمل `currentContent` قيمة نص الملاحظة الحالية.

في درس سابق، تعلمت كيف تتعامل مع حدث `DOMContentLoaded` بهذه الطريقة:
...
عندما يُحمّل كل شيء في مستند HTML ويُحلل، ستتمكن من الوصول إلى `noteEl` بأمان.

ابدأ بإرفاق طريقة `addEventListener` إلى كائن `window`. يجب أن تكون المعلمة الأولى لطريقة `addEventListener` هي حدث `"DOMContentLoaded"`. يجب أن تكون المعلمة الثانية دالة سهمية. داخل جسم الدالة السهمية، عيّن قيمة `noteEl.textContent` إلى `currentContent`.

# --hints--

يجب أن تُرفق طريقة `addEventListener` إلى كائن `window`.

```js
assert.match(code, /window\.addEventListener/);
```

يجب أن يستمع مستمع الحدث إلى حدث `"DOMContentLoaded"`.

```js
assert.match(code, /window\.addEventListener\(['"]DOMContentLoaded['"]\,/);
```

داخل جسم الدالة السهمية، عيّن قيمة `noteEl.textContent` إلى `currentContent`.
