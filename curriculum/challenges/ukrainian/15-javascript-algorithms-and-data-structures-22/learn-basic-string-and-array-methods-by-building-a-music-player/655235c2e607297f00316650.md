...
Далі скористайтеся властивістю `classList` та методом `add()`, щоб додати клас `"playing"` до елемента `playButton`. Він буде шукати по клас `"playing"` у файлі CSS та додасть його до елемента `playButton`.

Щоб нарешті відтворити пісню, скористайтеся методом `play()` на змінній `audio`. `play()` — це метод API вебаудіо для відтворення файлу mp3.

# --hints--

Використайте властивість `classList` та метод `add`, щоб додати клас `"playing"` до `playButton`.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

Використайте метод `play()` на змінній `audio`.
