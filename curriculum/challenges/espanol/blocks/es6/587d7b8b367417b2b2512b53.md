...
El método `constructor` es un método especial para crear e inicializar un objeto creado con una clase. Aprenderás más en la sección de Programación Orientada a Objetos de la Certificación en Algoritmos JavaScript y Estructura de Datos.
...
`Vegetable` debe ser una `class` con un método `constructor` definido.
...
`Vegetable` debe ser capaz de ser inicializada.
