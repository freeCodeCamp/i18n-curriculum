...
`button` 要素内に、さらに 2 つ `span` 要素を作成してください。

最初の `span` 要素に `playlist-song-artist` という `class` を設定してください。 `span` タグの間に `${song.artist}` を追加してください。

2 番目の `span` 要素には `playlist-song-duration` という `class` を設定してください。 `span` タグの間に `${song.duration}` を追加してください。

# --hints--

`button` 要素の既存のコンテンツは変更しないでください。
...
`playlist-song-artist` のクラスを持つ `span` 要素を作成してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

`playlist-song-artist` のクラスを持つ `span` 要素には、テキストコンテンツとして `song.artist` を文字列補間で挿入してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

`playlist-song-duration` のクラスを持つ `span` 要素を作成してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

`playlist-song-duration` のクラスを持つ `span` 要素には、テキストコンテンツとして `song.duration` を文字列補間で挿入してください。
