...
اكتمل التكرار. أرجع `grouped` من الدالة.

ثم، أسفل الدالة، استدعِ `groupByDecade(catalog)` وعيّن النتيجة إلى متغير اسمه `byDecade`. سجّل `byDecade` لفحص النتيجة المجمعة، ويجب أن ترى كائنًا يحتوي على مفاتيح مثل `"1950s"`، `"1960s"`، `"1970s"`، وهكذا، كل مفتاح يشير إلى مصفوفة من الكتب من ذلك العقد.

# --hints--

يجب أن تُرجع `groupByDecade` كائنًا.

```js
assert.isObject(groupByDecade(catalog));
```

يجب أن تكون قيمة `byDecade["1980s"].length` هي `5`.

```js
assert.strictEqual(groupByDecade(catalog)["1980s"].length, 5);
```

يجب أن تكون قيمة `byDecade["Unknown"].length` هي `1`.

```js
assert.strictEqual(groupByDecade(catalog)["Unknown"].length, 1);
```

يجب أن تعيّن نتيجة `groupByDecade(catalog)` إلى متغير اسمه `byDecade`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+byDecade\s*=\s*groupByDecade\s*\(\s*catalog\s*\)/);
```

يجب أن تسجّل `byDecade` في وحدة التحكم.
