...
Dovresti dare al primo `div` un attributo `class` con il valore `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.className, 'info');
```

Dovresti dare al secondo `div` un attributo `class` con il valore `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.className, 'info');
```

Dovresti dare al terzo `div` un attributo `class` con il valore `info`.
...
Dovresti annidare un elemento `input` all'interno del primo `div`, dopo `label`.
...
Dovresti annidare un elemento `input` all'interno del secondo `div`, dopo `label`.
...
Dovresti annidare un elemento `input` all'interno del terzo `div`, dopo `label`.
