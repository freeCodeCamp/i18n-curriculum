...
よく見ると、現在再生中の曲がプレイリストでハイライトされていないため、どの曲が再生されているのかわかりません。再生中の曲をハイライトする関数を作成してこれを修正しましょう。

アロー構文を使って `highlightCurrentSong` 関数を作成してください。関数内で `querySelectorAll` を使って `.playlist-song` 要素を取得し、`playlistSongElements` 定数に代入してください。

# --hints--

`const` を使って `highlightCurrentSong` 関数を作成してください。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` は関数である必要があります。

```js
assert.isFunction(highlightCurrentSong)
```

`highlightCurrentSong` 関数はアロー構文を使うべきです。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

`playlistSongElements` 定数を `highlightCurrentSong` 関数内で作成してください。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

`querySelectorAll()` メソッドを使って `.playlist-song` 要素を選択し、`playlistSongElements` 定数に代入してください。
