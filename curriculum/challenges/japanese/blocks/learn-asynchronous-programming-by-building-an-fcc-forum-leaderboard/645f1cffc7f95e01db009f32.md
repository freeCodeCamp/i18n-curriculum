...
次のステップは、`posters` 配列をループして、すべてのアバターを取得することです。

まず、`return` キーワードに続けて `posters.map()` を追加してください。コールバック関数には、`poster` というパラメータを追加します。

# --hints--

`posters.map()` を返す必要があります。

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

コールバック関数には `poster` という名前のパラメータが必要です。
