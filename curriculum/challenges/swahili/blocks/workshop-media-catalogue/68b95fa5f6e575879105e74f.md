...
Sasa darasa la `MediaError` limekamilika, badilisha usemi wa `if` katika njia ya `add`. Badilisha `TypeError` na `MediaError` ukitumia ujumbe uleule.

Pia, pita `media_item` kama hoja ya pili baada ya ujumbe. Hii itakuwezesha kuhifadhi kipengee kinachosababisha kosa katika kitu cha exception.

# --hints--

Unapaswa kuinua `MediaError` yenye ujumbe `Only Movie or TVSeries instances can be added` wakati `media_item` si mfano wa `Movie` au darasa lake ndogo.
...
Unapaswa kupitisha `media_item` kama hoja ya pili kwa `MediaError`.
...
Hupaswi kuinua `MediaError` wakati `media_item` ni kipengee halali.
...
Njia yako ya `add` haipaswi kuinua `TypeError`.
