...
令 ${p}(n)$ 表述 `n` 個硬幣可以分成堆的不同方式數量。 例如，五個硬幣可以分成堆的方式正好有七種，所以 ${p}(5) = 7$。
...
找出最小的 `n` 值，使得 ${p}(n)$ 可被 `divisor` 整除。

# --hints--

`coinPartitions(7)` 應該傳回一個數字。

```js
assert(typeof coinPartitions(7) === 'number');
```

`coinPartitions(7)` 應該傳回 `5`。

```js
assert.strictEqual(coinPartitions(7), 5);
```

`coinPartitions(10000)` 應該傳回 `599`。

```js
assert.strictEqual(coinPartitions(10000), 599);
```

`coinPartitions(100000)` 應該傳回 `11224`。

```js
assert.strictEqual(coinPartitions(100000), 11224);
```

`coinPartitions(1000000)` 應該傳回 `55374`。
