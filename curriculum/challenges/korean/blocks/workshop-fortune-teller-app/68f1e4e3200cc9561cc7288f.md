...
`if` 문장의 조건으로 `target`이 `HTMLElement`의 인스턴스가 아닌지 확인하세요. 그런 다음 그 안에 `return`을 추가하세요.

# --hints--

`if` 문장의 조건에 `!(target instanceof  HTMLElement)`가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

`return` 문장 안에 `if`이 있어야 합니다.
