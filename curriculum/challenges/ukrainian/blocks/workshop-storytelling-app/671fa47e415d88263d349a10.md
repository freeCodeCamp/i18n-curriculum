...
Далі ви почнете працювати над `JavaScript`. Для цього спочатку додайте посилання на файл `script.js` у ваш HTML.

Створіть елемент `script` з атрибутом `src`, який вказує на файл `script.js`. Наприклад: 
...
Ви повинні створити елемент `script`.

```js
assert.lengthOf(document.querySelectorAll('script'), 1);
```

Ваш елемент `script` має мати атрибут `src`, встановлений у `script.js`.
