...
回想一下，每次循環運行時，`input` 都是 `input` 的前一個值除以 `2` 後的商，向下舍入。 最終，`input` 小於 `1`，循環停止運行。

你只需一步即可完成此操作。

在 `while` 循環內，將 `input` 設置爲等於 `input` 除以 `2` 的商。 另外，請記住使用 `Math.floor()` 將商向下舍入。

# --hints--

你應該在 `while` 循環內設置 `input` 的值。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=/);
```

你應該使用 `Math.floor()` 將商向下舍入。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=\s*Math\.floor\(/);
```

你應該在 `while` 循環內將 `input` 設置爲等於 `input` 除以 `2` 的商（向下舍入）。
