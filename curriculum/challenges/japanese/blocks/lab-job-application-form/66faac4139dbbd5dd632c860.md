...
1. フォームには、ユーザーのフルネームを入力するためのタイプ `input`、ID `name` の要素が含まれていること。
1. ユーザーのメールアドレスを入力するためのタイプ `input`、ID `email` の別の要素があること。
...
`input` と `textarea` 要素に `:focus` 疑似クラスを使い、フォーカス時にデフォルトのアウトラインを削除すること。
