...
كما ترى، يُعرض `series1` بنفس تنسيق كائن `Movie` لأنه يرث طريقة `__str__` الخاصة بالأصل. في الخطوة التالية، ستتجاوز طريقة `__str__` لتوفر تمثيلًا مخصصًا كسلسلة نصية.

في الوقت الحالي، احذف السطر `print(series1)` من شفرتك.

# --hints--

يجب ألا يكون لديك `print(series1)` في شفرتك
