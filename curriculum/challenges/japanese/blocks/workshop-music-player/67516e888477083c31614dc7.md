...
よく見ると、現在再生中の曲がプレイリストでハイライトされていないため、どの曲が再生されているのかわかりません。再生中の曲をハイライトする関数を作成して、この問題を解決しましょう。

`highlightCurrentSong` という名前の関数を作成してください。関数内で、`.playlist-song` を持つ `aria-current="true"` 要素が存在するか確認します。存在する場合は、`removeAttribute` メソッドを使って、その要素から `aria-current` 属性を削除してください。

# --hints--

`highlightCurrentSong` という名前の関数を作成しているはずです。

```js
assert.isFunction(highlightCurrentSong)
```

`highlightCurrentSong` 関数は、`removeAttribute` を持つ `aria-current` 要素が存在する場合、その要素から `.playlist-song` 属性を削除するために `aria-current="true"` メソッドを使う必要があります。
