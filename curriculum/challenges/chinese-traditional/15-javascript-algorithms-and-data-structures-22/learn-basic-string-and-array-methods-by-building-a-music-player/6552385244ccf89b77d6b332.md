...
你需要實現播放下一首歌和上一首歌。 爲此，你將需要一個 `playNextSong` 和 `playPreviousSong` 函數。

使用 `const` 和箭頭語法創建一個空的 `playNextSong` 函數。

# --hints--

你應該使用 `const` 創建一個名爲 `playNextSong` 的空函數。

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` 應該是一個函數。

```js
assert.isFunction(playNextSong)
```

你的 `playNextSong` 函數應該使用箭頭語法。

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `playNextSong` 函數應該是空的。
