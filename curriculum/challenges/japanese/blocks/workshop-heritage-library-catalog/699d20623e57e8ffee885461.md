...
同じパターンを他の3つのフィールドにも適用してください。`author`、`year`、`location` に対しても同様のチェックを追加しましょう。

`isValid` は4つのチェックすべてにわたって累積します。どれかのフィールドが失敗すると `false` になり、残りのチェックが通っても `false` のままです。つまり、この関数は最初の問題で止まらず、常にすべてのフィールドを検査します。

# --hints--

`validateEntry(catalog[0])` は `true` を返すべきです。

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` は `false` を返すべきです。

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` は `false` を返すべきです。

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` は `false` を返すべきです。なぜなら場所が `"Unknown"` だからです。
