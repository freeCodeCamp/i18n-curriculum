...
Тепер замініть решту жорстко закодованих чисел на такі:
...
Ви повинні замінити жорстко закодоване `99` для значення `DRI` на `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

Ви повинні замінити жорстко закодоване `41` для значення `DEF` на `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

Ви повинні замінити жорстко закодоване `99` для значення `PHY` на `player.phy`.
