...
title: "applica la proprietà flex-direction per creare una colonna nell'incorporamento del tweet"
challengeType: 0
...
L'incorporamento del tweet `header` e `footer` ha usato prima la proprietà `flex-direction` con valore row. Allo stesso modo, gli elementi all'interno dell'elemento `.profile-name` funzionerebbero bene impilati come colonna.
...
L'elemento `.profile-name` dovrebbe avere la proprietà `flex-direction` impostata su `column`.
