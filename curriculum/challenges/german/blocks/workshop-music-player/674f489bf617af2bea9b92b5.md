...
Erstellen Sie nun eine Funktion namens `getNextSong`. Sie soll das Lied zurückgeben, das sich an der Position `getCurrentSongIndex()` plus eins in `userData.songs` befindet, oder `undefined`, wenn das aktuelle Lied das letzte in der Wiedergabeliste ist.

# --hints--

Sie sollten eine Funktion namens `getNextSong` haben.

```js
assert.isFunction(getNextSong);
```

Ihre `getNextSong`-Funktion sollte `getCurrentSongIndex` aufrufen.
...
Ihre `getNextSong`-Funktion sollte das Lied zurückgeben, das neben dem aktuellen Lied steht.
...
Ihre `getNextSong`-Funktion sollte `undefined` zurückgeben, wenn das aktuelle Lied das letzte in der Wiedergabeliste ist.
