...
Зверніть увагу, що обкладинка альбому в HTML та пісні в масиві `userData.songs` змінилися. Ми замінили оригінальні пісні на коротші, які можна використати для тестування додатка на наступних етапах.

Далі вам потрібно перевірити, чи є наступна пісня для відтворення. Отримайте індекс поточної пісні, викликавши функцію `getCurrentSongIndex()`, та збережіть його в константі `currentSongIndex`.

Після цього створіть константу `nextSongExists`, яка містить булеве значення `true` або `false` залежно від того, чи існує наступна пісня.

# --hints--

Не змінюйте наявний слухач подій `ended` та його вміст.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Створіть константу `currentSongIndex` та встановіть її на виклик функції `getCurrentSongIndex`.
...
Перевірте, чи існує наступна пісня, порівнюючи `userData.songs.length` та `currentSongIndex` і встановити її до константи `nextSongExists`. Якщо останній індекс масиву пісень (`userData.songs.length - 1`) більший за currentSongIndex, це означає, що є наступна пісня.
