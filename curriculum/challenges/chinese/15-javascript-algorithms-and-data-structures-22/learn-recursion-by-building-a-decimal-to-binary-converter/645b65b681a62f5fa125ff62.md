...
使用 `.push()` 将 `quotient` 变量附加到 `quotients` 数组。 另外，将 `remainder` 变量附加到 `remainders` 数组。

# --hints--

你应该在 `while` 循环中的 `quotients` 数组上使用 `.push()` 方法。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*quotients\.push\(/);
```

你应该使用 `.push()` 方法将 `quotient` 附加到 `quotients` 数组。

```js
assert.match(String(decimalToBinary), /quotients\.push\(\s*quotient\s*\)\s*;?/);
```

你应该在 `while` 循环中的 `remainders` 数组上使用 `.push()` 方法。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*remainders\.push\(/);
```

你应该使用 `.push()` 方法将 `remainder` 附加到 `remainders` 数组。
