...
爲 `only screen` 創建另一個 `@media` 查詢，其中 `max-width` 爲 `600px`。 在其中，創建一個 `.text-with-images` 規則，併爲其設置一個值爲 `1fr` 的 `grid-template-columns` 屬性。

這會將你的底部文本區域在較小屏幕上摺疊成單列。

# --hints--

你應該爲 `only screen and (max-width: 600px)` 創建一個新的 `@media` 查詢。 這應該在你之前的 `@media` 查詢下面。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText, 'only screen and (max-width: 600px)');
```

你的新 `@media` 查詢應該有一個 `.text-with-images` 選擇器。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText, '.text-with-images');
```

你的新 `.text-with-images` 選擇器應該有一個 `grid-template-columns` 屬性，其值爲 `1fr`。
