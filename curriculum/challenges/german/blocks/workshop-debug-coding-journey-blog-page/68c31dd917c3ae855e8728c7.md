...
Für den letzten Teil des Workshops sind einige Änderungen im Kontaktabschnitt am unteren Rand erforderlich.

Die erste Änderung besteht darin, den Kontaktabschnitt in ein `footer`-Element einzuschließen. Dies hilft Screenreadern, diesen Abschnitt als Fußzeile der Seite zu erkennen.

# --hints--

Ihre Seite sollte ein `footer`-Element enthalten.
...
Ihr `footer`-Element sollte ein `h3`-Element mit dem Text `Contact Me` enthalten.
...
Ihr `footer`-Element sollte ein `p`-Element mit dem Text `Email me at camperbot@blog.io` enthalten.
