...
Оголосіть стрілкову функцію `rangeFromString`, яка приймає два параметри, `num1` і `num2`. Функція має неявно повертати результат виклику `range` з аргументами `num1` і `num2`.

Щоб бути впевненим, аналізуйте `num1` і `num2` як цілі числа, коли передаєте їх у `range`.

# --hints--

Ви маєте оголосити змінну `rangeFromString` після вашої `rangeRegex`.
...
Ви маєте використовувати `const` для оголошення змінної `rangeFromString`.
...
Ваша змінна `rangeFromString` має бути стрілковою функцією.
...
Ваша функція `rangeFromString` має мати `num1` як перший параметр.
...
Ваша функція `rangeFromString` має мати `num2` як другий параметр.
...
Ваша функція `rangeFromString` має використовувати неявне повернення.
...
Ваша функція `rangeFromString` має повертати результат виклику вашої функції `range`.
...
Ви маєте викликати `parseInt` з аргументом `num1` і передати результат у виклик `range`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

Ви маєте викликати `parseInt` з аргументом `num2` і передати результат у виклик `range`.
