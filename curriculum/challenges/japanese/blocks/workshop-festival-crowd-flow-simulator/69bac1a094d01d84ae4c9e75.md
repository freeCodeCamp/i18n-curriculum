...
シミュレーション中に現在処理されている目盛りを追跡するために、`tickIndex` という名前の変数を作成し、初期値として `0` を代入してください。

この変数はシミュレーション内のすべての目盛りをループ処理し、各ゲートが正しい順序で処理されるようにするために使います。

# --hints--

`tickIndex` という名前の変数を作成し、`0` の値を代入してください。
