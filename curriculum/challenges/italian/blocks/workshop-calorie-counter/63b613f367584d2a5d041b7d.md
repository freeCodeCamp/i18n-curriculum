...
Prendi il tuo elemento `#budget` e assegnalo a `budgetNumberInput`, e il tuo elemento `#entry-dropdown` e assegnalo a `entryDropdown`.
...
Dovresti usare `document.getElementById()` per ottenere l'elemento `#budget`.

```js
assert.match(code, /document\.getElementById\(\s*('|")budget\1\s*\)/g);
```

Dovresti archiviare l'elemento `#budget` in una variabile chiamata `budgetNumberInput`.
...
Dovresti usare `document.getElementById()` per ottenere l'elemento `#entry-dropdown`.

```js
assert.match(code, /document\.getElementById\(\s*('|")entry-dropdown\1\s*\)/g);
```

Dovresti archiviare l'elemento `#entry-dropdown` in una variabile chiamata `entryDropdown`.
