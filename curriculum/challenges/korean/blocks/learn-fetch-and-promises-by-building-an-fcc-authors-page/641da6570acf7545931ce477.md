...
이제 구조 분해한 데이터로 페이지의 HTML을 만들기 시작할 시간입니다. 복합 대입 연산자(`+=`)와 `innerHTML` 속성을 조합해 이 작업을 할 수 있습니다.

콜백 함수 안에서 복합 대입 연산자를 사용해 `innerHTML`의 `authorContainer`에 빈 템플릿 리터럴을 추가하세요.
...
`innerHTML`의 `authorContainer`에 접근해야 합니다.

```js
assert.match(code, /authorContainer\.innerHTML\s*/)
```

복합 대입을 사용해 `innerHTML`의 `authorContainer`에 빈 템플릿 리터럴을 추가해야 합니다.
