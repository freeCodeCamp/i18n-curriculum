...
Badilisha ngazi ya stack ya kipengele cha `.penguin` ili ionekane mbele ya kipengele cha `.ground`, na kipa `position` ya `relative`.

# --hints--

Unapaswa kutumia sifa ya `z-index` kubadilisha ngazi ya msururu.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex);
```

Unapaswa kumpa kipengele cha `.penguin` `z-index` ya `4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex, '4');
```

Unapaswa kumpa `.penguin` sifa ya `position`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin')?.position);
```

Unapaswa kumpa `.penguin` `position` ya `relative`.
