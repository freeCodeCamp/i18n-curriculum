...
داخل صنف `Bee`، نفذ طريقة `render`، مسبوقة بكلمة `override`، لتحديث خاصية `innerText` في `emojiElement` بقيمة الخاصية `emoji`.

# --hints--

يجب أن يحتوي صنف `Bee` على طريقة `render` تستخدم كلمة `override`.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

يجب تعيين `this.emojiElement.innerText` إلى `this.emoji` داخل طريقة `render` في `Bee`.
