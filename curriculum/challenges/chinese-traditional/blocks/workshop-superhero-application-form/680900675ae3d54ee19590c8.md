...
接下來，使用 `section` 和 `column` 中的 `className` 以及文本 `How did you get your powers?` 創建 `label`。 在 `label` 內，創建一個 `select` 元素。
...
你的 `label` 元素應該包含文本 `How did you get your powers?`。
