...
残りの楽曲はこちらで `allSongs` 配列に追加しました。

次は <dfn>Web Audio API</dfn> と、それを使って楽曲を再生する方法について学びます。 主要な最新のブラウザーはすべて Web Audio API をサポートしており、ウェブアプリケーション内で音声を生成および処理できます。

では、`const` を使って `audio` という名前の変数を作成し、そこに `new Audio()` を代入してください。 これにより、新しい HTML5 `audio` 要素が作成されます。

# --hints--

`new` キーワードを使用して `Audio` オブジェクトのインスタンスを作成してください。

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

`Audio` オブジェクトを `audio` という名前の定数に割り当ててください。
