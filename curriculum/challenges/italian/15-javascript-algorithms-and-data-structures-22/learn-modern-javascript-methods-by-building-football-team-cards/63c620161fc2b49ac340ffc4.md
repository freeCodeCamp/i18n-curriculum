...
In this project, you will build a set of football team cards and learn about nested objects, object destructuring, and default parameters. Tutto l'HTML e il CSS per questo progetto ti è già stato fornito.

Start by accessing the `id` called `"team"` from the HTML document and storing it in a `const` variable called `teamName`.

Ricorda che puoi usare il metodo `getElementById` per questo.
