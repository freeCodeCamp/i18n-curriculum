...
定义一个不带参数的 `pauseSong` 函数。在你的新建函数中，为了保存歌曲暂停时的当前时间，将 `userData` 对象的 `songCurrentTime` 设置为 `audio` 变量的 `currentTime`。
...
你的 `pauseSong` 函数应将 `userData` 对象的 `songCurrentTime` 设置为 `audio` 变量的 `currentTime`。
