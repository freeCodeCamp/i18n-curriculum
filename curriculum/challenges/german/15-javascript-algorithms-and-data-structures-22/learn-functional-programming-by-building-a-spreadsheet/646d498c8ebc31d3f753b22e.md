...
Dein `spreadsheetFunctions`-Object sollte eine `firsttwo`-Eigenschaft enthalten.

```js
assert.property(spreadsheetFunctions, "firsttwo");
```

Deine `firsttwo`-Eigenschaft sollte eine Funktion sein.

```js
assert.isFunction(spreadsheetFunctions.firsttwo);
```

Deine `firsttwo`-Funktion sollte die ersten beiden Zahlen des Arrays zurückgeben.

```js
assert.deepEqual(spreadsheetFunctions.firsttwo([1, 2, 3, 4, 5, 6]), [1, 2]);
```

Dein `spreadsheetFunctions`-Objekt sollte eine `lasttwo`-Eigenschaft haben.

```js
assert.property(spreadsheetFunctions, "lasttwo");
```

Deine `lasttwo`-Eigenschaft sollte eine Funktion sein.

```js
assert.isFunction(spreadsheetFunctions.lasttwo);
```

Deine `lasttwo`-Funktion sollte die letzten beiden Zahlen des Arrays zurückgeben.
