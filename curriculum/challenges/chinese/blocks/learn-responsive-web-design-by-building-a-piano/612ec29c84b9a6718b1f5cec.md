...
对于新的 `@media` 规则，将 `#piano` 的 `width` 设置为 `675px`，并将 `.keys` 的 `width` 设置为 `633px`。

有了它，你的钢琴就完成了！

# --hints--

你的第二个 `@media` 规则应该有一个 `#piano` 选择器。
...
你的新的 `#piano` 选择器的 `width` 应为 `675px`。
