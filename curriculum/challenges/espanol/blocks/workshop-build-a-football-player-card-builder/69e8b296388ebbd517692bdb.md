...
Agrega un atributo `value` de `player.pac` al PAC `input`. Luego agrega un atributo `onChange` configurado como una función flecha que toma `e` como parámetro y llama a `setPlayer` con un spread de `player` y `pac` establecido en `Number(e.target.value)`.

# --hints--

Tu PAC `input` debe tener un `value` igual a la estadística `pac` del jugador.
...
Tu PAC `input` debe tener un manejador `onChange` que llame a `setPlayer` con un spread de `player` y `pac` establecido en `Number(e.target.value)`.
