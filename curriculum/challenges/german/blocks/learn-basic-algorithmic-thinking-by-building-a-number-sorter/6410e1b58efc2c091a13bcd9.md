...
Fügen Sie zu Debugging-Zwecken einen `console.log()`-Aufruf in Ihre innere Schleife ein. Übergeben Sie ihm die Argumente `array`, `array[j]` und `array[j+1]`.

# --hints--

Sie sollten einen `console.log()`-Aufruf in Ihrer inneren Schleife verwenden.
...
Sie sollten `array[j+1]` als drittes Argument an `console.log()` übergeben.
