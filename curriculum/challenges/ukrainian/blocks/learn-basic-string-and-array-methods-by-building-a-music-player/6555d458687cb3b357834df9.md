...
Отримайте доступ до проп `userData?.currentSong?.title` і `userData?.currentSong?.artist` та призначте їх відповідно змінним `currentTitle` і `currentArtist`.

# --hints--

Ви повинні отримати доступ до проп `title` об’єкта `currentSong` з об’єкта `userData`. Не забудьте використати опціональний ланцюжок.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

Ви повинні призначити проп `title` об’єкта `currentSong` константі `currentTitle`.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

Ви повинні отримати доступ до проп `artist` об’єкта `currentSong` з об’єкта `userData`. Не забудьте використати опціональний ланцюжок.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

Ви повинні призначити проп `artist` об’єкта `currentSong` константі `currentArtist`.
