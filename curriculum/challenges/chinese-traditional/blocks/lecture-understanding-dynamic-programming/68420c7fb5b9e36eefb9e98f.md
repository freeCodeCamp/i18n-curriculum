...
僅對 `n=5`，我們進行了 9 次函式呼叫，而實際只需要 5 次獨特的計算。隨著 `n` 增長，這種冗餘會呈指數爆炸——`climb_stairs(30)` 將需要數百萬次函式呼叫。時間複雜度變成 `O(2^n)`，使其對較大數值的 `n` 來說既低效又不實用。
...
- **實際影響**：`climb_stairs(30)` 從數百萬次函式呼叫降至約 30 次獨特子問題計算
