...
تمامًا مثل الدوال العادية، يمكن للدوال السهمية إرجاع قيم.

إليك مثال على دالة سهمية تُرجع نتيجة الضرب بين رقمين:
...
أنشئ متغيرًا جديدًا اسمه `addTwoNumbers` وقم بتعيين دالة سهمية له. يجب أن تأخذ هذه الدالة السهمية معلمتين اسميهما `num1` و `num2`. داخل جسم الدالة، أرجع التعبير الذي يجمع بين `num1` و `num2`.

أسفل دالة `addTwoNumbers`، أضف تعبيرًا في وحدة التحكم. داخل هذا التعبير، استدعي الدالة `addTwoNumbers` ومرر الأرقام `3` و `4` كمعلمات.

افتح وحدة التحكم لرؤية الناتج.

# --hints--

يجب أن يكون لديك دالة اسمها `addTwoNumbers`.

```js
assert.isFunction(addTwoNumbers);
```

يجب أن تستخدم دالة `addTwoNumbers` صيغة الدالة السهمية.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

يجب أن تحتوي دالة `addTwoNumbers` على معلمتين اسميهما `num1` و `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

يجب أن تُرجع دالة `addTwoNumbers` التعبير الذي يجمع بين `num1` و `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

يجب أن يكون لديك التعبير `console.log(addTwoNumbers(3, 4));`.
