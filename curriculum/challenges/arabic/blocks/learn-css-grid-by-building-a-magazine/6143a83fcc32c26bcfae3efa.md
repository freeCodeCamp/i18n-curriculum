...
أسفل عنصر `.text` الخاص بك، أنشئ عنصر `section` جديد وأعطه `class` بقيمة `text text-with-images`. بداخله، أنشئ عنصر `article` مع تعيين `class` إلى `brief-history`، وعنصر `aside` مع تعيين `class` إلى `image-wrapper`.

# --hints--

يجب أن تنشئ عنصر `section` جديد.

```js
assert(document.querySelectorAll('section')?.length === 3)
```

يجب أن يأتي عنصر `section` الجديد بعد عنصر `.text` الخاص بك.

```js
assert(document.querySelectorAll('section')?.[2]?.previousElementSibling?.className === 'text')
```

يجب أن يكون للعنصر `section` الجديد `class` بقيمة `text text-with-images`.

```js
assert(document.querySelectorAll('section')?.[2]?.className === 'text text-with-images')
```

يجب أن يحتوي عنصر `section` الجديد على عنصر `article`.

```js
assert.exists(document.querySelector('.text-with-images article'));
```

يجب أن يحتوي عنصر `section` الجديد على عنصر `aside`.

```js
assert.exists(document.querySelector('.text-with-images aside'));
```

يجب أن يأتي عنصر `article` قبل عنصر `aside`.

```js
assert(document.querySelector('.text-with-images article')?.nextElementSibling?.localName === 'aside');
```

يجب أن يكون لعنصر `article` الخاص بك `class` بقيمة `brief-history`.

```js
assert(document.querySelector('.text-with-images article')?.className === 'brief-history');
```

يجب أن يكون لعنصر `aside` الخاص بك `class` بقيمة `image-wrapper`.
