...
JavaScript 中的 `Math` 物件包含數學常數和函式的靜態屬性與方法。其一是 `Math.random()`，它會產生從 `0`（包含）到 `1`（不包含）的隨機數。另一個是 `Math.floor()`，它會將給定的數字向下取整到最接近的整數。

使用這些，你可以在一個範圍內產生隨機數。 例如，這會產生介於 `1` 和 `5` 之間的隨機數：`Math.floor(Math.random() * 5) + 1;`。

依照此樣式，使用加法運算子（`+`）將介於 `1` 和 `xp` 值之間的隨機數添加到你的 `monsterHealth -= weapons[currentWeaponIndex].power`。

# --hints--

你應該使用你已經寫好的相同 `monsterHealth` 行。

```js
assert.lengthOf(attack.toString().match(/monsterHealth/g), 1);
```

你應該為 `weapons[currentWeaponIndex].power` 值添加。
