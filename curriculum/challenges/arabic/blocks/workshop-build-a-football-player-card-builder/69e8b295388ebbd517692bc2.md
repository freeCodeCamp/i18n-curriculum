...
في JSX و TSX، تمرر تعبيرات JavaScript كقيم للخاصيات باستخدام الأقواس المتعرجة. تستخدم القوالب الحرفية علامات دقيقة و`${}` للاستيفاء. على سبيل المثال:

```tsx
<div className={`prefix-${someVariable}`}>
```

الخاصية `className` لعنصر تغليف البطاقة محددة حاليًا كـ `card-wrapper tier-gold`. استبدل `gold` بقيمة ديناميكية باستخدام قالب نصي يستدعي الدالة `getPlayerTier` مع `player.overallRating`. هذا سيشير إلى صنف المستوى الصحيح من CSS بناءً على التقييم العام للاعب.

# --hints--

يجب أن تستخدم قالبًا حرفيًا مع `getPlayerTier(player.overallRating)` لخاصية `className` في `.card-wrapper`.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

يجب أن يحتوي عنصر `.card-wrapper` على الصنف `tier-elite` عندما يكون تقييم اللاعب 98.
