...
曲を一時停止した時に現在の再生時間を保存するには、`userData` オブジェクトの `songCurrentTime` に、変数 `audio` の `currentTime` を代入してください。

*注*: この時点で `userData.songCurrentTime` が `null` または `undefined` である可能性はないため、このステップではオプショナルチェインを使うべきではありません。
...
`userData.songCurrentTime` に `audio.currentTime` を代入してください。
