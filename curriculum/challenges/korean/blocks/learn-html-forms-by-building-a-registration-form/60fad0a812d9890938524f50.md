...
Campers가 자기소개에 무엇을 넣을지 감을 잡을 수 있도록 `placeholder` 속성을 사용합니다. `placeholder`은 텍스트 값을 받아 사용자가 입력을 시작할 때까지 표시됩니다.

`textarea`에 `placeholder` 속성으로 `I like coding on the beach...`를 주세요.

# --hints--

`textarea`에 `placeholder` 속성을 주어야 합니다.

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

`placeholder`에 `I like coding on the beach...` 값을 주어야 합니다.
