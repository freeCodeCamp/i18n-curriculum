...
CSV (Comma-Separated Values) ni muundo unaotumika kwa lahajedwali na import ya data. Kila mstari unawakilisha kipengele kimoja na sehemu zake zimegawanyika kwa koma.

Anza kwa kuondoa wito la `console.log(exportToJSON(...))`, kwani lilihitajika tu kwa ajili ya majaribio. Kisha tengeneza kitendakazi kinachoitwa `exportToCSV` kinachopokea `catalog` kama kigezo. Ndani yake, tangaza `const header = "Title,Author,Year,Location"` na `const rows = []`.

# --hints--

Unapaswa kuondoa wito la `exportToJSON` console.log.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

Unapaswa kuwa na kitendakazi kinachoitwa `exportToCSV` chenye kigezo `catalog`.
...
Unapaswa kutangaza kigezo kinachoitwa `header` chenye thamani `"Title,Author,Year,Location"`.

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

Unapaswa kutangaza kigezo kinachoitwa `rows` kilichowekwa kuwa safu ya data tupu.
