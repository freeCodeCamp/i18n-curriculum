...
O próximo passo é percorrer o array `posters` para obter todos os avatares deles. 

Comece adicionando a palavra-chave `return` seguida de `posters.map()`. Para a função de callback, adicione um parâmetro chamado `poster`.

# --hints--

Você deve retornar `posters.map()`.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

Sua função de callback deve ter um parâmetro chamado `poster`.
