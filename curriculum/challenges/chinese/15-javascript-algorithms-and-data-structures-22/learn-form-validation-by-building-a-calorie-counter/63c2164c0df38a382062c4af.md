...
每个条目都会有一个文本输入作为条目名称，还有一个数字输入作为卡路里。 要获取条目数量，你可以通过文本输入进行查询。 请注意，你不能通过数字输入进行查询，因为你有一个用于用户卡路里预算的额外数字输入。

将字符串 `input[type="text"]` 传递给 `querySelectorAll()` 方法。 请记住，你需要对字符串使用单引号，以便可以在其中使用双引号。
