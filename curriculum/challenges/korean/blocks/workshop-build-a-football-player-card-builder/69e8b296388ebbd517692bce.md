...
이제 `div`의 `className`를 가진 `form-group`를 추가하세요. 그 안에 `label`의 `className`인 `label`와 `htmlFor`의 `name` 속성을 추가하세요. 그 아래에 `input`으로 설정된 `id`를 가진 `name`와 `className`의 `input`, 그리고 `type`의 `text` 속성을 추가하세요.

# --hints--

`div`의 `className`를 가진 `form-group`가 있어야 합니다.
...
`form-group` 안에 `label`의 `className`와 `label`의 `htmlFor` 속성을 가진 `name`가 있어야 합니다.
...
`form-group` 안에 `input`의 `id`, `name`의 `className`, 그리고 `input`의 `type`를 가진 `text`가 있어야 합니다.
