...
最初のセクションがより整列して見えるようにするために、`.info` 要素内の `input` 要素のみを選択し、`width` を `50%` に設定し、テキストを左側に揃えてください。

# --hints--

`.info input` または `.info > input` のいずれかのセレクターを使用する必要があります。
...
`input` 要素の `width` を `50%` に設定する必要があります。
...
`input` 要素の `text-align` を `left` に設定する必要があります。
