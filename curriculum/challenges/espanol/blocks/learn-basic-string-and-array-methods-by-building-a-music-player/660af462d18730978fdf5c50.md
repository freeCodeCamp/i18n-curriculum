...
Ahora es el momento de aplicar lo que has aprendido sobre las funciones flecha a tu proyecto de reproductor de música. 

Comienza eliminando las funciones `printGreeting`, `printMessage`, y `addTwoNumbers`. Además, elimina todas las declaraciones de consola y llamadas a funciones. 

# --hints--

No deberías tener una función `printGreeting`.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

No deberías tener una función `printMessage`.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

No deberías tener una función `addTwoNumbers`.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

No deberías tener un llamado a función `printGreeting();`.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

No deberías tener un llamado a función `printMessage('freeCodeCamp');`.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

No deberías tener una declaración de consola `console.log(addTwoNumbers(3,4));`.
