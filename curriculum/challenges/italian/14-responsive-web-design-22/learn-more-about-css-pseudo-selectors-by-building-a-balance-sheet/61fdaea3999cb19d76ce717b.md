...
Il selettore `span[class~="sr-only"]` dovrebbe avere una proprietà `width` con il valore `1px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('width') === '1px');
```

Il selettore `span[class~="sr-only"]` dovrebbe avere una proprietà `height` con il valore `1px`.
