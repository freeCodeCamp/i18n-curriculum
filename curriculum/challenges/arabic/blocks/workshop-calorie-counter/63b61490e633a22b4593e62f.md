...
باتباع نفس النمط، عيّن عنصر `#add-entry` إلى `addEntryButton`، وعنصر `#clear` إلى `clearButton`، وعنصر `#output` إلى `output`.

# --hints--

يجب إعلان متغير باسم `addEntryButton`.

```js
assert.isDefined(addEntryButton);
```

يجب استخدام `document.getElementById()` للحصول على عنصر `#add-entry`.

```js
assert.match(code, /document\.getElementById\(\s*('|")add-entry\1\s*\)/g);
```

يجب تعيين عنصر `#add-entry` إلى `addEntryButton`.

```js
assert.deepEqual(addEntryButton, document.getElementById('add-entry'));
```

يجب إعلان متغير باسم `clearButton`.

```js
assert.isDefined(clearButton);
```

يجب استخدام `document.getElementById()` للحصول على عنصر `#clear`.

```js
assert.match(code, /document\.getElementById\(\s*('|")clear\1\s*\)/g);
```

يجب تعيين عنصر `#clear` إلى `clearButton`.

```js
assert.deepEqual(clearButton, document.getElementById('clear'));
```

يجب إعلان متغير باسم `output`.
...
يجب استخدام `document.getElementById()` للحصول على عنصر `#output`.

```js
assert.match(code, /document\.getElementById\(\s*('|")output\1\s*\)/g);
```

يجب تعيين عنصر `#output` إلى `output`.
