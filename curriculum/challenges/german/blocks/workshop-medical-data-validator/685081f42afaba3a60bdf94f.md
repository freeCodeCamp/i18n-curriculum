...
Als Nächstes möchten Sie überprüfen, ob `age` eine ganze Zahl ist. Fügen Sie also einen weiteren Schlüssel `age` zum `constraints`-Wörterbuch hinzu. Rufen Sie für dessen Wert `isinstance` auf und übergeben Sie `age` und `int` als Argumente.

# --hints--

Ihr `constraints`-Wörterbuch sollte einen Schlüssel `age` enthalten.
...
Der `age`-Schlüssel Ihres `constraints`-Wörterbuchs sollte den Wert `isinstance(age, int)` haben.
