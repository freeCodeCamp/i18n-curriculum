...
マウスがリンクの上にホバーしたときに、そのプロパティを変更するには、`a:hover { propertyName: propertyValue; }` のような<dfn>疑似セレクター</dfn>を使います。

ユーザーがフッターの `Visit our website` リンクにホバーしたとき、その色を `brown` に変更してください。

# --hints--

`a:hover` 疑似セレクターを使う必要があります。
...
あなたの `a:hover` は `color` の `brown` を持つべきです。
