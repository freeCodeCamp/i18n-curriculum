...
Defina o nível de pilha do elemento da montanha de modo que permaneça diretamente atrás do elemento `.ground`.
...
Você deve definir a propriedade `z-index` como `2`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.zIndex, '2');
```

Você não deve alterar o `z-index` do elemento `.ground`.
