...
Перш ніж довіряти проаналізованим даним, почніть із валідації обов’язкових полів.

Створіть функцію з іменем `validateEntry`, яка приймає `entry` як параметр. Оголосіть `let isValid = true`.

Додайте перевірку валідації для `title`: встановіть множину `isValid = false`, якщо `title` відсутній, має хибне значення або дорівнює `"Unknown"`.

Поверніть `isValid`. Перевірки для інших полів ви додасте на наступних кроках.

# --hints--

`validateEntry` має повертати булеве значення.

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })` має повертати `true`.

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})` має повертати `false`.

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })` має повертати `false`.

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })` має повертати `false`.
