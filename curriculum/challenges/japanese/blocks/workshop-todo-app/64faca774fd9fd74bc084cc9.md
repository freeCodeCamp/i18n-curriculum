...
入力フィールドにテキストがある場合にのみ、`Cancel` と `Discard` のボタンをユーザーに表示してください。

まず、`confirmCloseDialog.showModal();` の行を削除してください。

次に、`closeTaskFormBtn` のイベントリスナー内で、`titleInput`、`dateInput`、または `descriptionInput` のいずれかのフィールドに値があるかどうかをチェックするロジックを書いてください。

これらの入力フィールドのいずれかに値があれば、`showModal()` に対して `confirmCloseDialog` メソッドを使ってください。そうでなければ、変更がない場合は `reset()` 関数を呼び出して入力フィールドをクリアし、フォームモーダルを非表示にしてください。

# --hints--

入力フィールドのいずれかに値があれば、確認ダイアログが表示されるべきです。そうでなければ、`reset` 関数が呼び出されるべきです。
