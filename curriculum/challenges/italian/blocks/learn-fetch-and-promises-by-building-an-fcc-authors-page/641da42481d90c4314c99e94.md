...
Concatena il metodo `.then()` alla chiamata di `fetch`. All'interno del metodo `.then()`, aggiungi una funzione callback con `res` come parametro, poi mostra `res` sulla console in modo da poter vedere l'oggetto `Response`. Open your browser console and expand the `Response` object to see what it contains.
