...
你應該在 `background` 屬性中將 `repeating-linear-gradient` 添加到 `.fb6` 中。
...
你應該爲 `repeating-linear-gradient` 設置從 `0%` 到 `10%` 是第一種顏色 `--building-color3`。
...
你應該爲 `repeating-linear-gradient` 設置從 `10%` 到 `30%` 是第二種顏色 `transparent`。
