...
عنصرك الجديد `p` يجب أن يكون ملفوفا داخل عنصرك `footer`. يجب أن يكون عندك عنصر `p` واحد بالضبط.
...
عنصرك الجديد `a` يجب أن يكون ملفوفا داخل عنصرك `p`. يجب أن يكون عندك عنصر `a` واحد بالضبط.
...
يجب أن يحتوي عنصر `a` الجديد على نص `Visit our website`.

```js
assert(document.querySelector("footer > p > a")?.innerText === "Visit our website");
```

يجب ربط عنصر `a` الجديد الخاص بك بـ `https://www.freecodecamp.org`. تذكر أن عناصر `a` تستخدم خاصية `href` لإنشاء رابط.
