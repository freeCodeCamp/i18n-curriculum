...
接著，為作者頁面添加一個在 freeCodeCamp News 的聯結。

新增一個帶有 `class` 屬性且值為 `"author-link"` 的錨點元素，將 `url` 插入作為 `href` 屬性的值，並將 `target` 設為 `"_blank"`。錨點元素的文字為插入 `author`，後接文字 `"'s author page"`。例如，`"Quincy Larson's author page"`。
