...
`label` 요소 안에 있는 모든 `.info` 요소를 선택하고, 그들의 `width`를 `10%`로 설정하며, 최소한 `55px`만큼의 공간을 차지하도록 만드세요.

# --hints--

`.info label` 선택자나 `.info > label` 선택자 중 하나를 사용하세요.
...
`label` 요소에 `min-width`를 `55px`로 지정하세요.
