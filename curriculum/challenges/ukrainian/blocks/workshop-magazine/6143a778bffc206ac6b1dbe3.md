...
Створіть один фінальний `p` елемент у кінці вашого `.text` елемента та додайте йому наступний текст:
...
Ви повинні додати шостий `p` елемент до `.text` елемента.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 7);
```

Ваш шостий `p` елемент має містити наданий текст.
