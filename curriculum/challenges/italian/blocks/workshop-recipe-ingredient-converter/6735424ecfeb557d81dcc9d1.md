...
In questo passaggio, allega un listener per l'evento `submit` a `recipeForm` in modo che quando si clicca il pulsante `Convert`, venga chiamata la funzione `updateResultsList()`.

Allega un listener per l'evento `submit` a `recipeForm`. Quando il modulo viene inviato, chiama `event.preventDefault()` per fermare il comportamento predefinito, e poi chiama `updateResultsList()`.

Questo ti permetterà di attivare la funzione tramite l'interfaccia utente compilando il modulo e premendo `Convert`, utile per eseguire il debug mentre aggiungi la logica di conversione nel passaggio successivo.

# --hints--

Fai in modo che cliccando il pulsante `Convert` venga eseguita `updateResultsList()` aggiungendo un listener per l'evento `"submit"` a `recipeForm` che chiama `event.preventDefault()` per fermare il comportamento predefinito del modulo.
