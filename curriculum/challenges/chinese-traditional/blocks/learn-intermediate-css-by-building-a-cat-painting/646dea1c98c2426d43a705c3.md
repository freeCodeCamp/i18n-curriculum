...
將左側內眼移動到位置，使用 `position` 屬性為 `absolute`，`top` 為 `8px`，`left` 為 `2px`。同時，為它設定 `60%` 的邊框半徑並旋轉 `10` 度。

# --hints--

你的 `.cat-left-inner-eye` 選擇器應該有一個設定為 `absolute` 的 `position` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.position === 'absolute')
```

你的 `.cat-left-inner-eye` 選擇器應該有一個 `top` 屬性設定為 `8px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.top === '8px')
```

你的 `.cat-left-inner-eye` 選擇器應該有一個設定為 `2px` 的 `left` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.left === '2px')
```

你的 `.cat-left-inner-eye` 選擇器應該有一個 `border-radius` 屬性設定為 `60%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.borderRadius === '60%')
```

你的 `.cat-left-inner-eye` 選擇器應該有一個 `transform` 屬性設定為 `10deg`。
