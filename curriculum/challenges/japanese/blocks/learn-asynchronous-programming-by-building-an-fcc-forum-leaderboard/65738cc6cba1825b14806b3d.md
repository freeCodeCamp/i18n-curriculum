...
次に、`const` 型の変数 `forumCategoryUrl` を作成し、文字列 `"https://forum.freecodecamp.org/c/"` を代入してください。

その下に、別の `const` 型の変数 `avatarUrl` を作成し、文字列 `"https://sea1.discourse-cdn.com/freecodecamp"` を代入してください。

# --hints--

`forumCategoryUrl` 型の変数を `const` で宣言してください。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=/);
```

`"https://forum.freecodecamp.org/c/"` 変数に `forumCategoryUrl` を代入してください。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=\s*('|")https:\/\/forum\.freecodecamp\.org\/c\/\1\s*/);
```

`avatarUrl` 型の変数を `const` で宣言してください。

```js
assert.match(code, /const\s+avatarUrl\s*=/);
```

`"https://sea1.discourse-cdn.com/freecodecamp"` 変数に `avatarUrl` を代入してください。
