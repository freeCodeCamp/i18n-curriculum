...
Du solltest die Methode `canvas.getContext("2d")` verwenden.

```js
assert.match(code, /canvas\.getContext\(\s*('|"|`)2d\1\s*\)/);
```

Du solltest `const` verwenden, um die Variable namens `ctx` zu deklarieren und ihr `canvas.getContext("2d")` zuzuweisen.
