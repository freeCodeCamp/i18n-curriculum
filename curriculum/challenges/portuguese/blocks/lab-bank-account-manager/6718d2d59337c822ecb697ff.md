...
4. Se o valor for menor ou igual a `0`, o método `deposit` deve retornar `"Deposit amount must be greater than zero."`.
...
6. Se o valor a ser sacado for menor ou igual a `0` ou maior que o saldo atual, o método `withdraw` deve retornar `"Insufficient balance or invalid amount."`.


7. Você deve definir um método chamado `checkBalance` que retorne o saldo atual no formato `"Current balance: $[balance]"`.
...
"`BankAccount.deposit(100)` deve retornar `"Successfully deposited $100. New balance: $100"` Novo saldo: $100`.
...
`BankAccount.deposit(-50)` deve retornar `"Deposit amount must be greater than zero."`.
...
`BankAccount.deposit(0)` deve retornar `"Deposit amount must be greater than zero."`.
...
Quando o saldo da conta é `100`, `BankAccount.withdraw(150)` deve retornar `"Insufficient balance or invalid amount."`.
...
`BankAccount.withdraw(-50)` deve retornar `"Insufficient balance or invalid amount."`.
...
`BankAccount.withdraw(0)` deve retornar `"Insufficient balance or invalid amount."`.
...
Quando o saldo da conta é `200`, `BankAccount.withdraw(150)` deve retornar `"Successfully withdrew $150. New balance: $50"` Novo saldo: $50`.
...
Quando o saldo da conta é `200`, `BankAccount.checkBalance()` deve retornar `"Current balance: $200"`.
...
Quando você depositar `10`, `35`, `90`, o método `listAllDeposits` deve retornar `"Deposits: 10,35,90"`.
