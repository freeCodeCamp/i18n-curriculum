...
Utahitaji kurekebisha kwa idadi ya sehemu zinazohesabiwa. Hii ni fursa nzuri ya kutumia kitendakazi kingine kilichopangwa.

Tangaza kitendakazi cha `adjustForServings` kinachopokea hoja ya `baseQuantity`. Kitendakazi hicho kinapaswa kurudisha kitendakazi cha pili kinachopokea hoja ya `newServings`.

Kitendakazi cha ndani kinapaswa kurudisha matokeo ya kuzidisha `baseQuantity` na `newServings`.

# --hints--

Unapaswa kutangaza kitendakazi cha `adjustForServings`.

```js
assert.isFunction(adjustForServings);
```

Kitendakazi chako cha `adjustForServings` kinapaswa kupokea hoja ya `baseQuantity`.

```js
assert.match(adjustForServings?.toString(), /\(?baseQuantity\)?/);
```

Kitendakazi chako cha `adjustForServings` kinapaswa kurudisha kitendakazi.

```js
assert.isFunction(adjustForServings?.());
```

Kitendakazi chako cha ndani kinapaswa kuwa na kigezo cha `newServings`.

```js
assert.match(adjustForServings?.()?.toString(), /\(?newServings\)?/);
```

Kitendakazi chako cha ndani kinapaswa kurudisha matokeo ya kuzidisha `baseQuantity` na `newServings`.
