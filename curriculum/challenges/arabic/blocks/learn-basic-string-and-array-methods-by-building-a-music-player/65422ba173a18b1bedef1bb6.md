...
داخل الدالة السهمية لمستمع الحدث، أضف جملة `if` للتحقق مما إذا كانت `userData?.currentSong` تساوي `null`.

داخل كتلة `if`، استدعِ الدالة `playSong()` مع `id` أول أغنية في مصفوفة `userData?.songs`. هذا يضمن تشغيل أول أغنية في قائمة التشغيل أولًا.
...
يجب أن تستدعي الدالة `playSong` مع `userData?.songs[0].id` داخل كتلة `if` الخاصة بك.
