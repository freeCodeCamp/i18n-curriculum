...
在 `Bee` 類別中，實作 `render` 方法，並以 `override` 關鍵字作為前置式，將 `innerText` 的 `emojiElement` 屬性更新為 `emoji` 屬性的值。

# --hints--

你的 `Bee` 類別應該有一個使用 `render` 關鍵字的 `override` 方法。

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

你應該在 `this.emojiElement.innerText` 的 `this.emoji` 方法中將 `render` 設定為 `Bee`。
