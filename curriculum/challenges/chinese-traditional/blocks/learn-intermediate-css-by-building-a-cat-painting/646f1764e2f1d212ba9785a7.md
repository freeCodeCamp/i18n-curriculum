...
使用類別選擇器，將 `.cat-whisker-left-bottom` 移動到位置，並設定 `position` 為 `absolute`、`top` 為 `134px`、`left` 為 `52px`。

# --hints--

你應該有一個 `.cat-whisker-left-bottom` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom'))
```

你的 `.cat-whisker-left-bottom` 選擇器應該有一個設定為 `absolute` 的 `position` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.position === 'absolute')
```

你的 `.cat-whisker-left-bottom` 選擇器應該有一個 `top` 屬性設定為 `134px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.top === '134px')
```

你的 `.cat-whisker-left-bottom` 選擇器應該有一個設定為 `52px` 的 `left` 屬性。
