...
Наприкінці, потрібно буде повернути зображення для аватара користувача.

Для початку додайте `return`, а потім набор шаблонних літералів. Усередині шаблонних літералів додайте елемент `img`.

Всередині теґа `img` додайте атрибут `src` зі значенням `${userAvatarUrl}`. Для атрибута `alt` додайте значення `${user.name}`.

# --hints--

Поверніть шаблонний літерал.
...
Шаблонний літерал повинен містити елемент `img`.
...
Теґ `img` повинен містити атрибут `src`.
...
Встановіть значення атрибута `src` на константу під назвою `userAvatarUrl`.
...
Ви повинні мати атрибут `alt` після атрибута `src`.
...
Встановіть значення атрибута `alt` на константу під назвою `user.name`.
