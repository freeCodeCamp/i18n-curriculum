...
你的 `input` 元素需要一些附加属性。 将其 `type` 设置为 `number` 以仅允许数字输入，将 `min` 属性设置为 `0` 以仅允许正数，并将 `placeholder` 设置为 `Daily calorie budget`。

最后，将 `input` 元素标记为 `required`。
...
你的 `input` 元素应将 `placeholder` 属性设置为 `Daily calorie budget`。
