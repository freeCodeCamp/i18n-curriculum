...
Folgen Sie dem gleichen Muster und führen Sie eine Abfrage für Ihre `number` Eingaben in den `#dinner`, `#snacks` und `#exercise` Elementen durch. Weisen Sie sie Variablen zu, die dem Namensschema der vorherigen beiden folgen.

# --hints--

Sie sollten eine `dinnerNumberInputs` Variable deklarieren.

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

Sie sollten `document.querySelectorAll()` mit `#dinner input[type='number']` aufrufen.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Sie sollten das Ergebnis Ihres `document.querySelectorAll()` Aufrufs `dinnerNumberInputs` zuweisen.
...
Sie sollten eine `snacksNumberInputs` Variable deklarieren.

```js
assert.match(calculateCalories.toString(), /snacksNumberInputs\s*=/);
```

Sie sollten `document.querySelectorAll()` mit `#snacks input[type='number']` aufrufen.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#snacks input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Sie sollten das Ergebnis Ihres `document.querySelectorAll()` Aufrufs `snacksNumberInputs` zuweisen.
...
Sie sollten eine `exerciseNumberInputs` Variable deklarieren.

```js
assert.match(calculateCalories.toString(), /exerciseNumberInputs\s*=/);
```

Sie sollten `document.querySelectorAll()` mit `#exercise input[type='number']` aufrufen.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#exercise input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Sie sollten das Ergebnis Ihres `document.querySelectorAll()` Aufrufs `exerciseNumberInputs` zuweisen.
