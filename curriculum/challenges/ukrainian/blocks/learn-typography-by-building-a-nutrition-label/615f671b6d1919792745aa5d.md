...
Створіть новий елемент `div` під вашим елементом `.medium`. Додайте йому атрибут `class` зі значенням `daily-value small-text`. Всередині цього нового `div` додайте елемент `p` з текстом `% Daily Value *` і встановіть атрибут `class` зі значенням `bold right`.

# --hints--

Ви маєте створити новий елемент `div` після вашого елемента `.medium`.
...
Ваш новий `div` має мати атрибут `class` зі значенням `daily-value small-text`.
