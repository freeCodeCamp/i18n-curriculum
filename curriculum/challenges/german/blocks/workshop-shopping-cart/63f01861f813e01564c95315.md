...
Fügen Sie innerhalb Ihres `div` zwei `p`-Elemente hinzu. Setzen Sie den Text des zweiten `p`-Elements auf den Wert der `price`-Variablen.

# --hints--

Sie sollten zwei `p`-Elemente innerhalb Ihres `div`-Elements hinzufügen.
...
Ihr zweites `p`-Element sollte den Text der `price`-Variablen haben.
