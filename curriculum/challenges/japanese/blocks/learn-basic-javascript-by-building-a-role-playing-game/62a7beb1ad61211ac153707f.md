...
今、`button2` のテキストを `"Buy weapon (30 gold)"` に更新し、`button3` のテキストを `"Go to town square"` に更新する行を追加してください。

# --hints--

`innerText` の `button2` プロパティにアクセスするにはドット記法を使うべきです。

```js
assert.match(code, /button2\.innerText/);
```

`let` の `const` プロパティにアクセスするのに `innerText` や `button2` を使うべきではありません。

```js
assert.notMatch(code, /(let|const)\s+button2\.innerText/);
```

`innerText` の `button2` プロパティを `"Buy weapon (30 gold)"` に更新するべきです。

```js
assert.match(code, /button2\.innerText\s*=\s*('|")Buy weapon \(30 gold\)\1/);
```

`innerText` 関数内で `goStore` プロパティを更新するべきです。

```js
assert.match(goStore.toString(), /button2\.innerText\s*=\s*('|")Buy weapon \(30 gold\)\1/);
```

`innerText` の `button3` プロパティにアクセスするにはドット記法を使うべきです。

```js
assert.match(code, /button3\.innerText/);
```

`let` の `const` プロパティにアクセスするのに `innerText` や `button3` を使うべきではありません。

```js
assert.notMatch(code, /(let|const)\s+button3\.innerText/);
```

`innerText` の `button3` プロパティを `"Go to town square"` に更新するべきです。
