...
Створіть новий селектор `.left-container p`, встановивши верхнє та нижнє поле на `-5px`, а ліве та праве поле на `-2px`. Також встановіть `font-size` у значення `2em` і `font-weight` у значення `700`.

# --hints--

У вас має бути новий селектор `.left-container p`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

Ваш новий селектор `.left-container p` має мати властивість `margin`, встановлену в `-5px -2px`.
...
Ваш новий селектор `.left-container p` має мати властивість `font-size`, встановлену в `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

Ваш новий селектор `.left-container p` має мати властивість `font-weight`, встановлену в `700`.
