...
Fügen Sie die verbleibenden Argumente `chosenCard.name_short` und `chosenCard.img` ein. Setzen Sie dann außerhalb der `renderCard`-Funktion das `multipleCard` des `innerHTML`-Elements auf einen leeren String zurück.

# --hints--

Sie sollten `chosenCard.name_short` als Argument an `renderCard` übergeben.
...
Sie sollten `chosenCard.img` als letztes Argument an `renderCard` übergeben.
...
Sie sollten `this.elements.multipleCard.innerHTML` auf einen leeren String zurücksetzen.
