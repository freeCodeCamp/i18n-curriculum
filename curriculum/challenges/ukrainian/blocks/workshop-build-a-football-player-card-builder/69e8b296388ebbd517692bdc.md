...
Додайте атрибут `value` зі значенням `player.sho` до SHO `input`. Потім встановіть його атрибут `onChange` у стрілкову функцію, яка приймає `e` як параметр і викликає `setPlayer` з розгортанням `player` та `sho`, встановленим у `Number(e.target.value)`.

# --hints--

Ваш SHO `input` має мати `value`, рівний поточному значенню статистики `sho` гравця.
...
Ваш SHO `input` має мати обробник `onChange`, який викликає `setPlayer` з розгортанням `player` та `sho`, встановленим у `Number(e.target.value)`.
