...
Sua classe `ShoppingCart` precisa da capacidade de adicionar itens. Crie um método `addItem` vazio, que receba dois parâmetros: `id` e `products`. Criar um método pode ser assim:
...
O primeiro parâmetro, `id`, é o `id` do produto que o usuário adicionou ao carrinho. O segundo parâmetro, `products`, é um array de instâncias `Dessert`. Usando um parâmetro em vez de referenciar diretamente seu array `products` existente, esse método será mais flexível caso você queira adicionar listas adicionais de produtos no futuro.

# --hints--

Sua classe `ShoppingCart` deve ter um método `addItem`.
...
Seu método `addItem` deve receber dois parâmetros: `id` e `products`.
...
Seu método `addItem` deve estar vazio.
