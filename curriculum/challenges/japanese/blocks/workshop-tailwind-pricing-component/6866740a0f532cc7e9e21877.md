...
`Most popular` バッジに進みましょう。カードの右上隅に配置する必要があるので、Tailwindの位置指定ユーティリティクラスを使うことができます。

TailwindでCSSの位置指定を使う方法は次の通りです。
...
ラッパー`relative`にクラス`div`を代入し、テキスト`absolute`がある`div`に`Most popular`を代入してください。

# --hints--

プレミアムプラン用のラッパー`div`要素にはクラス`relative`を付けてください。
...
テキスト`div`がある`Most popular`要素にはクラス`absolute`を付けてください。
