...
Створіть функцію з іменем `isSelect`, яка приймає один параметр `element` типу `EventTarget | null`. Ця функція перевірятиме, чи є вибраний елемент HTML-елементом `<select>`.

# --hints--

У вас має бути функція з іменем `isSelect`.
...
Ваша функція `isSelect` має мати параметр з іменем `element` типу `EventTarget | null`.
...
Ваша функція `isSelect` має повертати значення типу `element is HTMLSelectElement`.
