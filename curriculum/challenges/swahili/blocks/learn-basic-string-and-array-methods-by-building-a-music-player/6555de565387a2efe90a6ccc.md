...
Kitendakazi cha `setPlayButtonAccessibleText` kitaweka sifa ya kipengele ya `aria-label` kwa wimbo wa sasa, au kwa wimbo wa kwanza katika orodha ya nyimbo. Na ikiwa orodha ya nyimbo ni tupu, kitaweka `aria-label` kuwa `"Play"`.

Tumia njia ya `setAttribute` kwenye kipengele cha `playButton` kuweka sifa ya kipengele iitwayo `"aria-label"`. Kwa kutumia kikokotoo cha masharti cha pande tatu, weka thamani ya sifa ya kipengele kuwa `Play ${song.title}` au `"Play"` ikiwa `song?.title` haipatikani.

Usisahau unahitaji uingizaji wa thamani hapa, kwa hivyo unahitaji kutumia alama za kupigia (backticks).

# --hints--

Haupaswi kubadilisha kitendakazi cha `setPlayButtonAccessibleText` kilichopo na yaliyomo ndani yake.
...
Unapaswa kutumia njia ya `setAttribute()` kwenye `playButton`.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` inapaswa kuwa thamani ya kwanza ya njia yako ya `setAttribute()`.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

Njia yako ya `setAttribute` inapaswa kuwa na ``song?.title ? `Play ${song.title}` : "Play"`` kama hoja ya pili.
