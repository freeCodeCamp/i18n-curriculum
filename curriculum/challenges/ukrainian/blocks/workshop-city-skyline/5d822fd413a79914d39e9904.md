...
Заповніть `.bb3` `repeating-linear-gradient`. Використовуйте `90deg` для напрямку, ваш `building-color3` для перших двох кольорів і `window-color3` на `15%` для третього. Якщо ви не вказуєте відстань для кольору, буде використано значення, які мають сенс. У цьому випадку перші два кольори за замовчуванням будуть `0%` і `7.5%`, бо починається з `0%`, а `7.5%` — це половина `15%`.
