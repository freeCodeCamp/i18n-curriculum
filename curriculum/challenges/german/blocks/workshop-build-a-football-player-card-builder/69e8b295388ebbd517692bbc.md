...
Fügen Sie nun ein `div` mit dem `className` von `card-stats` hinzu. Darin fügen Sie ein `div` mit dem `className` von `stat-col` ein. Darin fügen Sie drei `div`-Elemente ein, jeweils mit dem `className` von `stat-row`.

# --hints--

Sie sollten ein `div` mit dem `className` von `card-stats` nach `.card-name-strip` hinzufügen.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

Sie sollten ein `div` mit dem `className` von `stat-col` innerhalb von `.card-stats` hinzufügen.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

Sie sollten drei `div`-Elemente hinzufügen, jeweils mit dem `className` von `stat-row` innerhalb von `.stat-col`.
