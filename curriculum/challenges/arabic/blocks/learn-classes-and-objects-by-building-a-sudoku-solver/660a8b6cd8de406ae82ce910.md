...
قم بالتسكين لجملة `if` داخل جملة `if` الحالية. استخدم استدعاءً متكررًا لـ `solver()` كشرط لجملة `if`، وأرجع `True` من جسم جملة `if` الجديدة.

# --hints--

يجب أن تسكن جملة `if` داخل جملة `if` الحالية وتستخدم `self.solver()` كشرط.
...
يجب أن تُرجع `True` من جملة `if` الجديدة.
