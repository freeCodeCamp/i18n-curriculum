...
使用類選擇器，將 `.cat-whisker-left-top` 元素移動到位，將其 `position` 設置爲 `absolute`，`top` 設置爲 `120px`，`left` 設置爲 `52px`。
