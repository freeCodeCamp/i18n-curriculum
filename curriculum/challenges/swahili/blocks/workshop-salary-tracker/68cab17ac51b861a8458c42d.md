...
Mwishowe, sasa baada ya kumaliza setter ya `salary`, unaweza kuitumia ndani ya setter ya `level`.

Badilisha mstari wa `self._salary = Employee._base_salaries[new_level]` ili uitumie setter ya `salary` badala ya kubadilisha moja kwa moja sifa ya kipengele ya `_salary`.

Kwa hivyo warsha ya mfuatilia mshahara imekamilika.

# --hints--

Unapaswa kuweka `self.salary` kuwa `Employee._base_salaries[new_level]` ndani ya setter ya `level`.
