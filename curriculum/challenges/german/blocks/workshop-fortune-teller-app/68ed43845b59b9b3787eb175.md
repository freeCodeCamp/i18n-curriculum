...
Fügen Sie eine `constructor` innerhalb der Klasse hinzu. Im Konstruktor müssen Sie die zuvor erstellten Elemente verwenden, um die entsprechenden DOM-Elemente mit der `getElement`-Funktion auszuwählen.

Behandeln Sie vorerst `singleCardBtn` und `singleCard`. Für `singleCardBtn` übergeben Sie `#btn-single-card`, und für `singleCard` übergeben Sie `.single_card`.

# --hints--

Die `Game`-Klasse sollte einen `constructor` haben.
...
Ihr Konstruktor sollte `this.elements` zuweisen.
...
`singleCardBtn` sollte mit `getElement` und `'#btn-single-card'` gesetzt werden.
...
`singleCard` sollte mit `getElement` und `'.single_card'` gesetzt werden.
