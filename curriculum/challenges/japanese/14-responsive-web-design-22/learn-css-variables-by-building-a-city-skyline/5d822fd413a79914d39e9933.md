...
`.fb5` の `background` プロパティに、2 つ目の `repeating-linear-gradient` を追加する必要があります。
...
2 つ目の `repeating-linear-gradient` に `90deg` の方向を指定する必要があります。
...
2 つ目の `repeating-linear-gradient` で最初の色 `--building-color2` を `0%` から `12%` まで設定する必要があります。
...
2 つ目の `repeating-linear-gradient` で 2 番目の色 `--window-color2` を `12%` から `44%` まで設定する必要があります。
