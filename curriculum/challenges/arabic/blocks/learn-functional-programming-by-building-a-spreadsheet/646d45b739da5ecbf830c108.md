...
دالتك `apply` تُرجع دالة جدول البيانات، لكنها لا تُنفذها فعليًا. حدّث `apply` لاستدعاء الدالة. مرّر نتيجة استدعاء `toNumberList` مع `args` كمعلمة.

# --hints--

يجب أن تستدعي دالتك `apply` الدالة `spreadsheetFunctions[fn.toLowerCase()]`.
...
يجب أن تمرر استدعاء `toNumberList()` إلى استدعاء `spreadsheetFunctions[fn.toLowerCase()]` الخاص بك.
...
يجب أن تمرر `args` إلى استدعاء `toNumberList()`.
