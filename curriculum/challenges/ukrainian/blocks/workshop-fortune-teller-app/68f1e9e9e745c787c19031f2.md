...
Всередині інструкції `if` отримайте елемент `cardTitle` і встановіть його `textContent` у значення `foundCard.name`. Зробіть те саме для елемента `description`, встановивши його `textContent` у значення `foundCard.desc`.

# --hints--

Ви повинні встановити `cardTitle` `textContent` у `foundCard.name`. Не забудьте використати ключове слово `this`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

Ви повинні встановити `description` `textContent` у `foundCard.desc`. Не забудьте використати ключове слово `this`.
