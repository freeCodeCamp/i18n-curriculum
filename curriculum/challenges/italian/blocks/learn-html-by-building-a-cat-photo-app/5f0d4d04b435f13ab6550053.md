...
L'elemento `legend` dovrebbe avere un tag di apertura. Opening tags have this syntax: `<elementName>`.
...
L'elemento `legend` dovrebbe avere un tag di chiusura. Closing tags have a `/` just after the `<` character.
