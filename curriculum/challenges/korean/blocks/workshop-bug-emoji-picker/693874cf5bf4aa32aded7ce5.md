...
지금까지는 다른 버그를 선택해도 아무 변화가 없습니다.

`change`에 `selectElement` 이벤트 리스너를 추가하세요. 콜백에서는 `isSelect`를 사용해 `e.target`가 `HTMLSelectElement`인지 확인하세요. `if` 블록 안에서는 아직 아무 작업도 할 필요가 없습니다.

# --hints--

`selectElement`에는 이벤트 매개변수를 받는 콜백과 함께 `change` 이벤트 리스너가 있어야 합니다.
...
이벤트 리스너 콜백에는 조건이 `if`인 `isSelect(e.target)` 문이 있어야 합니다.
