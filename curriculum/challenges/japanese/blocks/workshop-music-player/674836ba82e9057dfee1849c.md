...
次に、`classList` プロパティと `add()` メソッドを使って、`playing` 要素に `playButton` クラスを追加してください。これにより、CSS ファイル内の `playing` クラスが探され、`playButton` 要素に追加されます。

その後、`userData.currentSong` を `song` に設定し、最後に曲を再生するために `play()` 変数に対して `audio` メソッドを使ってください。`play()` は mp3 ファイルを再生するためのウェブオーディオ API のメソッドです。

# --hints--

`playing` 関数内で `playButton` 要素に `playSong` クラスを追加してください。
...
`userData.currentSong` 関数内で `song` を `playSong` に設定してください。
...
`play()` 関数内で `audio` 変数に対して `playSong` メソッドを呼び出してください。
