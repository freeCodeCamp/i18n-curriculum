...
次に、ユーザーが入力欄に文字を入力したときに、APIから果物の名前を取得するようにアプリを作成します。

まず、`useEffect` フックを追加して、`query` の状態が変わるたびに実行されるようにしてください。`useEffect` の最初の引数にはアロー関数を渡し、依存関係の配列には `query` を含めることを忘れないでください。

# --hints--

`useEffect` フックを使うべきです。
...
`query` フックの依存関係の配列に `useEffect` を含めるべきです。
