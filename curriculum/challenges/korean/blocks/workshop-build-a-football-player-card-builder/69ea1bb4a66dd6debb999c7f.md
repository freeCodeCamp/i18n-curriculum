...
이제 선수 위치 선택을 처리할 차례입니다. 입력을 select로 리팩터링한 다음, 유효한 축구 위치들의 배열을 select 옵션 값으로 매핑하세요.

먼저, `POSITIONS` 배열을 `GK`, `CB`, `LB`, `RB`, `CDM`, `CM`, `CAM`, `LW`, `RW`, `ST`, `CF` 문자열 값들로 만드세요. 닫는 대괄호 바로 뒤에 `as const`를 추가해 TypeScript가 일반 `string[]` 배열 타입 대신 정확한 리터럴 값들의 읽기 전용 튜플로 타입을 좁히도록 하세요.

이렇게 하면 TypeScript가 어떤 문자열이 유효한지 정확히 알게 되어, 단순히 문자열 배열임을 아는 것보다 더 구체적으로 처리합니다.
...
`POSITIONS` 배열에는 필요한 모든 축구 위치 문자열이 포함되어야 하며, `as const`을 사용해 타입을 읽기 전용 문자열 리터럴로 좁히세요.
