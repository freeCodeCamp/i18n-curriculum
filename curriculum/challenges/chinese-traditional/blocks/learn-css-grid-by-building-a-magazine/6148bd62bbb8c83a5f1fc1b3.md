...
創建一個 `.hero-title` 選擇器，並為它設定 `text-align` 屬性為 `center`、`color` 屬性為 `orangered` 以及 `font-size` 屬性為 `8rem`。
