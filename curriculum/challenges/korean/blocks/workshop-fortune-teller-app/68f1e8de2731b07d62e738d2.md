...
`foundCard`라는 또 다른 변수를 선언하고, `find()`에서 `cards` 메서드를 호출한 결과를 할당하세요. `card`을 콜백 매개변수로 전달하고, 엄격한 동등성으로 `card.name_short`와 `cardId`를 비교해 일치하는 카드를 찾으세요.

# --hints--

`foundCard` 내부에 `showFortune`라는 변수를 선언해야 합니다.
...
`foundCard`는 올바른 콜백과 함께 `find`에서 `this.cards`를 호출한 결과를 할당해야 합니다.
...
`find` 콜백은 `card`라는 매개변수를 가져야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

콜백 내부에서 `card.name_short`과 `cardId`를 비교해야 합니다.
