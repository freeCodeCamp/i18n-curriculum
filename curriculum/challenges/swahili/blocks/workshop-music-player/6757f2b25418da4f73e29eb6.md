...
Ifuatayo, unahitaji kuonyesha kichwa cha wimbo wa sasa na msanii kwenye onyesho la mchezaji.

Kama hatua ya kwanza, pata upatikanaji wa vipengele vya `#player-song-title` na `#player-song-artist`, vinavyohusika na kuonyesha kichwa cha wimbo na msanii, na vihifadhi katika vigezo `playingSong` na `songArtist`, mtawalia.

# --hints--

Unapaswa kuwa na kigezo kinachoitwa `playingSong`.

```js
assert.isDefined(playingSong);
```

Kigezo chako cha `playingSong` kinapaswa kuwa na thamani ya kipengele cha `#player-song-title`.

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

Unapaswa kuwa na kigezo kinachoitwa `songArtist`.
...
Kigezo chako cha `songArtist` kinapaswa kuwa na thamani ya kipengele cha `#player-song-artist`.
