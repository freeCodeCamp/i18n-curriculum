...
Під останнім елементом `p` створіть ще один елемент `p` з текстом `Sodium 160mg 7%`. Розмістіть `Sodium` та `7%` у власних елементах `span` з класом `bold`, як ви робили з іншими.

Потім додайте ще один елемент `span` навколо `Sodium 160mg` знову з метою центрування.
...
Новий елемент `p` повинен мати текст `Sodium 160mg 7%`.
...
Елемент `span` повинен обгортати текст `Sodium 160mg`.
