...
`allCategories` 物件的其餘部分已為你完成。

在接下來的幾個步驟中，你將創建一個函式（程式）來從 `allCategories` 物件中取得分類名稱。

首先建立一個名為 `forumCategory` 的箭頭函式（arrow function），並以 `id` 作為參數名稱。

# --hints--

你需要創建一個名為 `forumCategory` 的空函式（程式）。

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

你應該在 `forumCategory` 函式中有一個名為 `id` 的參數。
