...
title: "المشكلة 79: اشتقاق رمز المرور"
challengeType: 1
forumTopicId: 302192
...
طريقة أمان شائعة تُستخدم في الخدمات المصرفية عبر الإنترنت هي طلب ثلاثة أحرف عشوائية من رمز المرور. على سبيل المثال، إذا كان رمز المرور هو `531278`، قد يطلبون الحرف الثاني والثالث والخامس؛ والرد المتوقع سيكون: `317`.

المصفوفات `keylog1` و`keylog2` و`keylog3` تحتوي على خمسين محاولة تسجيل دخول ناجحة.

بما أن الأحرف الثلاثة تُطلب دائمًا بالترتيب، حلل المصفوفة لتحديد أقصر رمز مرور سري ممكن بطول غير معروف.

# --hints--

يجب أن تُرجع الدالة `passcodeDerivation(keylog1)` رقمًا.
...
يجب أن تُرجع الدالة `passcodeDerivation(keylog1)` القيمة `531278`.
...
يجب أن تُرجع الدالة `passcodeDerivation(keylog2)` القيمة `1230567`.
...
يجب أن تُرجع الدالة `passcodeDerivation(keylog3)` القيمة `73162890`.
