...
Kuendelea na kontena kuu, mpe darasa la vitu la `container` sifa ya `width` kuwa `80%` na sifa ya `max-width` kuwa `600px`. Pia, ongeza sifa ya `border-radius` kuwa `10px` na weka `margin` kuwa `20px auto` ili kuifanya iwe katikati kwa usawa.

# --hints--

Unapaswa kulenga darasa la vitu la `container`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.container'));
```

Weka sifa ya `width` kuwa `80%`.
...
Weka sifa ya `max-width` kuwa `600px`.
...
Weka sifa ya `border-radius` kuwa `10px`.
...
Weka sifa ya `margin` kuwa `20px auto`.
