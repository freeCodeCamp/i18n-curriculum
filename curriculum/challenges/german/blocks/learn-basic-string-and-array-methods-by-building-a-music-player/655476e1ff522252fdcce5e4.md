...
Fügen Sie einen `else`-Block zur `if`-Anweisung hinzu. Rufen Sie innerhalb des `else`-Blocks die `getCurrentSongIndex()`-Funktion auf und weisen Sie sie einer Konstanten mit dem Namen `currentSongIndex` zu.

# --hints--

Sie sollten die bestehende `if`-Anweisung nicht ändern.
...
Sie sollten einen `else` zur bestehenden `if`-Anweisung hinzufügen.
...
Sie sollten die Konstante `currentSongIndex` innerhalb des `getCurrentSongIndex()`-Blocks auf `else` setzen.
