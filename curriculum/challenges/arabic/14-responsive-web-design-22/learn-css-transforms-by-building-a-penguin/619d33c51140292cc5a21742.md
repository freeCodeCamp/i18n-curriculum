...
بداخل عنصر `.arm.left` يجب أن تكون خاصية `animation-name` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationName, 'wave');
```

يجب أن يحتوي عنصر `.arm.left` على خاصية `animation-duration` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationDuration, '3s');
```

يجب أن يحتوي عنصر `.arm.left` على خاصية `animation-iteration-count` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationIterationCount, 'infinite');
```

يجب أن يحتوي عنصر `.arm.left` على خاصية `animation-timing-function` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
