...
`h1` と `p` の要素を、`margin` の `1em auto` を指定して中央に配置してください。そして、それらのテキストも `center` に揃えてください。

# --hints--

`h1` と `p` の要素をスタイル指定するには、カンマ区切りの要素セレクターを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

`margin` と `1em auto` の要素を中央に配置するには、`h1` の `p` を使うべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

`text-align` と `center` のテキストを中央に揃えるには、`h1` の `p` を使うべきです。
