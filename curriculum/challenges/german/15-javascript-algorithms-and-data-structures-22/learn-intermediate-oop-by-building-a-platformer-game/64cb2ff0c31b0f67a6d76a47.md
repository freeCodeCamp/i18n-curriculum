...
Die letzte Methode, die du zur `CheckPoint`-Klasse hinzufügen musst, ist die `claim`-Methode.

Inside the `claim` method, assign `0` to the `width` and `height` properties of the `CheckPoint` instance.

Weise unter diesen Eigenschaften `Infinity` der `y`-Position zu.

Lastly, assign `true` to the `claimed` property.

# --hints--

Deine `CheckPoint`-Klasse sollte eine `claim`-Methode haben.
...
Du solltest `Infinity` der `y`-Position zuweisen.
