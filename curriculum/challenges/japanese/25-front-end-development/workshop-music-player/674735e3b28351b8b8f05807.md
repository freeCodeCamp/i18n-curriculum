...
The `find()` method retrieves the first element within an array that fulfills the conditions specified in the provided callback function. 条件を満たす要素が無い場合は `undefined` を返します。
