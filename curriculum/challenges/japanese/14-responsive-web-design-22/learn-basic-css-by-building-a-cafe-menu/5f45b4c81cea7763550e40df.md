...
すでに使用しているテーマカラー (黒と茶色) に合わせて、訪問されたことがあるリンクの色を `black` に変更し、リンクがクリックされている最中の色には `brown` を使用してください。

# --hints--

リンクが `visited` (訪問済み) の場合の `color` プロパティを `black` に設定する必要があります。
...
リンクが `active` (クリック中) である場合の `color` プロパティを `brown` に設定する必要があります。
