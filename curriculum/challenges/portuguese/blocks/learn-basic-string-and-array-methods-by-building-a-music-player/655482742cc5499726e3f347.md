...
Dentro da função `playPreviousSong`, adicione uma declaração `if` com a condição `userData?.currentSong === null`. Isso verificará se atualmente não há nenhuma música tocando. Se não houver, saia da função usando um `return`.

Dentro do bloco `else`, crie uma constante chamada `currentSongIndex` e atribua a ela `getCurrentSongIndex()`.

# --hints--

Você não deve alterar a função `playPreviousSong` existente e seu conteúdo.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Você deve ter uma declaração `if` com a condição `userData?.currentSong === null` dentro da sua função `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

Você deve ter `return` dentro do bloco do seu `if` statement.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

Sua declaração `if` deve ter um bloco `else`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

Você deve chamar `getCurrentSongIndex` e atribuí-lo a `currentSongIndex` dentro do bloco `else` da sua declaração `if`.
