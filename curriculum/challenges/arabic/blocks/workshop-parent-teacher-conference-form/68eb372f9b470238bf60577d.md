...
حان الوقت الآن لتعريف ما يحدث عند تحديد زر الاختيار.

أنت تقوم بتنسيق الدائرة الداخلية فقط عند اختيار زر الاختيار، لذا يجب أن يكون المحدِّد الخاص بك هو `.contact-method-radio-btn:checked::before`.

في القاعدة، عيّن الخاصية `transform` إلى `translate(3px, 3px) scale(1)` والخاصية `background-color` إلى `lightgreen`.

تُحرّك الخاصية `transform` الدائرة الداخلية قليلاً وتُكبرها إلى الحجم الكامل باستخدام `scale(1)`، ويملأ خلفية بلون lightgreen الدائرة لتُظهر أن الخيار محدد، مما يجعلها مرئية.

# --hints--

يجب أن تستهدف العنصر المزيف `.contact-method-radio-btn:checked::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

عيّن الخاصية `transform` إلى `translate(3px, 3px) scale(1)`.
...
عيّن الخاصية `background-color` إلى `lightgreen`.
