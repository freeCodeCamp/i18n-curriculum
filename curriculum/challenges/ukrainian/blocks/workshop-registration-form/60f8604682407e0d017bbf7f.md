...
До кожного відповідного елемента `label` та одразу після елемента `input` додайте пробіл і такий текст:
