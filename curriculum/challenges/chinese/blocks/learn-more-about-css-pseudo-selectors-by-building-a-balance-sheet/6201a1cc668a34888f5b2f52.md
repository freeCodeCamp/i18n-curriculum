...
选择 `class` 属性为 `data` 的 `tr` 元素。 给他们一个 `linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)` 的背景图片。
...
你的 `tr.data` 选择器应该将 `background-image` 属性设置为 `linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)`。
