...
O elemento `ol` deve ter uma tag de abertura. Opening tags have this syntax: `<elementName>`.

```js
assert(document.querySelector('ol'));
```

O elemento `ol` deve ter uma tag de fechamento. Closing tags have a `/` just after the `<` character.
