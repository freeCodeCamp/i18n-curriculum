...
Зображення мають бути розміщені у макеті з двома колонками та трьома рядками.

Дайте селектору `.image-wrapper` властивість `grid-template-columns` зі значенням `2fr 1fr` та властивість `grid-template-rows` зі значенням `repeat(3, min-content)`. Це дозволить нашим рядкам сітки змінювати висоту залежно від вмісту, а колонки залишатимуться фіксованої ширини відповідно до контейнера.

# --hints--

Ваш селектор `.image-wrapper` має мати властивість `grid-template-columns` зі значенням `2fr 1fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

Ваш селектор `.image-wrapper` має мати властивість `grid-template-rows` зі значенням `repeat(3, min-content)`.
