...
플레이어가 라운드에서 이기면 `playerScore`만큼 `1`을 업데이트하고 메시지 `"Player wins! [player's choice] beats [computer's choice]"`을 반환하세요.

컴퓨터와 플레이어가 같은 선택을 하면 메시지 `"It's a tie! Both chose [player's choice]"`를 반환하세요.

컴퓨터가 라운드에서 이기면 `computerScore`만큼 `1`를 업데이트하고 메시지 `"Computer wins! [computer's choice] beats [player's choice]"`을 반환하세요.
...
`getRoundResults` 함수는 누가 라운드에서 이겼는지에 따라 올바른 메시지를 반환해야 합니다. 아무도 이기지 못하면 무승부 메시지를 반환하세요.
