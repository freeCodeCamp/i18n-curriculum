...
Il centro di controllo ti ha chiesto di scambiare le posizioni di due astronauti nel tuo array `squad`, in particolare i membri agli indici `2` e `5`. Usa la tua funzione `swapCrewMembers()` per effettuare lo scambio e archivia il risultato in una nuova variabile chiamata `updatedSquad`.

# --hints--

Dovresti usare `swapCrewMembers()` per scambiare i membri `squad` agli indici `2` e `5` e archiviare l'array risultante in una nuova variabile chiamata `updatedSquad`.
