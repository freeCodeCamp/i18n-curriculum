...
既然你從玩家身上扣除了生命值，你需要檢查玩家的 `health` 是否小於或相等於 `0`。如果是，呼叫 `lose` 函式（程式）。

# --hints--

你的 `pick` 函式應該在 `else` 主體中巢狀包含 `if` 述語。
...
你的 `if` 述語應該檢查玩家的 `health` 是否小於或相等於 `0`。

```js
assert.match(pick.toString(), /if\s*\(\s*health\s*<=\s*0\s*\)/);
```

你在 `else` 主體中的 `if` 述語應該呼叫 `lose` 函式（程式）。
