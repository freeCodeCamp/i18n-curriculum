...
داخل الدالة `playNextSong`، أنشئ جملة `if` للتحقق مما إذا كانت قيمة `currentSong` في `userData` تساوي تمامًا `null`. هذا سيتحقق مما إذا لم تكن هناك أغنية حالية تُشغل في كائن `userData`.

إذا تحقق الشرط، نفذ استدعاء الدالة `playSong` مع `id` أول أغنية في مصفوفة `userData?.songs` كمعلمة.

# --hints--

لا تعدل الدالة `playNextSong` الحالية أو محتواها.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

يجب أن تنشئ جملة `if` بالشرط `userData?.currentSong === null`.
...
يجب أن تستدعي الدالة `playSong` مع `userData?.songs[0].id` داخل جملة الـ `if`.
