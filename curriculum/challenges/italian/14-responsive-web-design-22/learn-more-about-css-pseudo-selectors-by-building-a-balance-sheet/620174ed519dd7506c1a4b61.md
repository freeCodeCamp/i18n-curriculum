...
Gli elementi `#years` dovrebbero avere una proprietà `color` impostata su `#fff`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('color') === 'rgb(255, 255, 255)');
```

Gli elementi `#years` dovrebbero avere una proprietà `background-color` impostata su `#0a0a23`.
