...
Innerhalb der Funktion erhalten Sie Referenzen auf die HTML-Elemente, die für die Anzeige des Songtitels und des Künstlers verantwortlich sind.

Greifen Sie mit der `#player-song-title`-Methode auf die `#player-song-artist`- und `getElementById()`-Elemente zu. Weisen Sie sie jeweils den Variablen `playingSong` und `songArtist` zu.

# --hints--

Sie sollten die bestehende `setPlayerDisplay`-Funktion und deren Inhalt nicht verändern.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Sie sollten `document.getElementById()` verwenden, um das `#player-song-title`-Element zu erhalten.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Sie sollten das `#player-song-title`-Element der Variablen `playingSong` zuweisen. Vergessen Sie nicht, `const` zu verwenden, um die Variable zu deklarieren.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Sie sollten `document.getElementById()` verwenden, um das `#player-song-artist`-Element zu erhalten.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Sie sollten das `#player-song-artist`-Element der Variablen `songArtist` zuweisen. Vergessen Sie nicht, `const` zu verwenden, um die Variable zu deklarieren.
