...
제출 버튼이 더 뚜렷하게 보이도록 `background-color`에 `#3b3b4f`를, 그리고 `border-color`에 `white`를 지정하세요.

# --hints--

제출 버튼에 `background-color`에 `#3b3b4f`를 지정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.backgroundColor, 'rgb(59, 59, 79)');
```

제출 버튼에 `border-color`에 `white`를 지정해야 합니다.
