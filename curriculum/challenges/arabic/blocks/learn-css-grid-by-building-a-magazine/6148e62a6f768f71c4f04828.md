...
يمكن استخدام خاصية `place-items` لضبط قيمتي `align-items` و `justify-items` في نفس الوقت. تأخذ خاصية `place-items` قيمة واحدة أو قيمتين. إذا تم توفير قيمة واحدة، تُستخدم لكل من خاصيتي `align-items` و `justify-items`. إذا تم توفير قيمتين، تُستخدم القيمة الأولى لخاصية `align-items` والقيمة الثانية لخاصية `justify-items`.

أعطِ المحدِّد `.image-wrapper` خاصية `place-items` بقيمة `center`.

# --hints--

يجب أن يحتوي المحدِّد `.image-wrapper` على خاصية `place-items` بقيمة `center`.
