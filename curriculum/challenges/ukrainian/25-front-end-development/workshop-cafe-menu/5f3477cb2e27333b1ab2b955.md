...
Now you need to link the `styles.css` file, so the styles will be applied again. Додайте елемент `link` до елемента `head`. Give it a `rel` attribute with the value of `"stylesheet"` and a `href` attribute with the value of `"styles.css"`.
...
Не змінюйте наявний елемент `head`. Make sure you did not delete the closing tag.
