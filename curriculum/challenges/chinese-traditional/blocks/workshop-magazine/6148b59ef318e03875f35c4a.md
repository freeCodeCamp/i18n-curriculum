...
創建一個 `img` 選擇器，將其 `width` 屬性設置爲 `100%`、`object-fit` 屬性設置爲 `cover`。

`object-fit` 屬性告訴瀏覽器如何在其容器中定位元素。 在這種情況下，`cover` 會將圖像設置爲填充容器，並根據需要進行裁剪以避免更改寬高比。
...
你的 `img` 選擇器應該有一個 `width` 屬性，設置爲 `100%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('img')?.width, '100%');
```

你的 `img` 選擇器應該有一個 `object-fit` 屬性，設置爲 `cover`。
