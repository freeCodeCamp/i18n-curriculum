...
Тепер настав час для графіку. Створіть метод `create_trajectory` та замініть останній виклик `print` у нижній частині коду на `print(graph.create_trajectory())`.

As the first step of this new method, make a local copy of the coordinates but where all the values are rounded to integers. Збережіть цю нову версію координат у змінній під назвою `rounded_coords` та поверніть цю змінну.
...
Замініть `print(graph.create_coordinates_table())` на `print(graph.create_trajectory())`.
...
Функція має повернути `rounded_coords`.
