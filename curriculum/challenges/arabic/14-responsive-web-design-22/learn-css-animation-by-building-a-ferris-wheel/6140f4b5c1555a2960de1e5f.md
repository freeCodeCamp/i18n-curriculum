...
قاعدة `@keyframes` الجديدة الخاصة بك يجب أن تسمى `cabins`.
...
يجب أن يأتي منتقي `100%` الخاص بك بعد منتقي `0%` الخاص بك.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '100%')
```

يجب أن يكون لمنتقي `100%` الخاص بك خاصية `transform` بقيمة `rotate(-360deg)`.
