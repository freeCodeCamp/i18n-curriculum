...
Quando o usuário executar o programa, ele exibirá o prompt `Enter ISBN and length: `.</dfn> O usuário pode inserir o código ISBN que deseja validar no formato `ISBN,length`. O código ISBN não deve conter hífens, seguido pelo seu comprimento (`10` ou `13`), separados por vírgula.
...
1. Quando o usuário não inserir um valor separado por vírgulas, ele deve ver a mensagem `Enter comma-separated values.` no console, e o programa deve ser encerrado.
1. Mesmo que o usuário insira um valor não numérico para o comprimento, o programa deve tratar o `ValueError` sem travar.
1. Quando o usuário inserir um valor não numérico para o comprimento, ele deve ver a mensagem `Length must be a number.` no console, e o programa deve ser encerrado.
...
1. Quando o usuário inserir um código ISBN incorreto com caracteres diferentes de números, ele deverá ver a mensagem `Invalid character was found.` no console.
1. Quando o usuário inserir `1530051126,10`, ele deverá ver a mensagem `Valid ISBN Code.`
1. Quando o usuário inserir `9781530051120,13`, ele deverá ver a mensagem `Valid ISBN Code.`
