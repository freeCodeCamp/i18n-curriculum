...
もう一つあります。`Load More Authors`ボタンをデータがもう読み込めなくなるまでクリックし続けて、テキストが`"No more data to load"`に変わっても、カーソルの値はまだ`pointer`のままです。なぜカーソルの値を`"not-allowed"`に変えないのでしょうか？

`style` ボタンの `Load More Authors` プロパティにアクセスして、`cursor` を `"not-allowed"` に設定してください。
...
`style` の `loadMoreBtn` プロパティにドット記法でアクセスしてください。

```js
assert.match(code, /loadMoreBtn\.style/)
```

`cursor` プロパティから `style` にドット記法でアクセスしてください。

```js
assert.match(code, /loadMoreBtn\.style\.cursor/)
```

`cursor` プロパティの値を `"not-allowed"` に設定してください。
