...
`h2, h3, h4, h5, h6` 선택자를 만드세요. `font-family` 속성을 `Raleway`로 설정하고 대체 수단으로 `sans-serif`을 지정하세요.

# --hints--

`h2, h3, h4, h5, h6` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h2, h3, h4, h5, h6'));
```

`h2, h3, h4, h5, h6` 선택자에 `font-family` 속성을 `Raleway`로 설정하고 대체 수단으로 `sans-serif`을 지정해야 합니다.
