...
你的杂志将有三个主要部分。 你已经在 `main` 规则中设置了整体布局，但你可以在子规则中调整布局。

一种选择是 `grid-column` 属性，它是 `grid-column-start` 和 `grid-column-end` 的简写。 `grid-column` 属性告诉网格项从哪条网格线开始和结束。

创建 `.heading` 规则，并将 `grid-column` 属性设置为 `2 / 3`。 这将告诉 `.heading` 元素从网格线 2 开始并在网格线 3 结束。
...
你的 `.heading` 选择器应该有一个 `grid-column` 属性，设置为 `2 / 3`。
