...
Встановіть `font-size` у `1.1rem`, `display` у `block` і `margin` у `auto`, щоб центровано розмістити кнопку.

# --hints--

Ви маєте продовжувати націлюватися на клас `.submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Встановіть властивість `font-size` у `1.1rem`.
...
Встановіть властивість `margin` у `auto`.
