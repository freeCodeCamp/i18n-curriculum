...
У цьому проєкті ви навчитеся основним методам рядків та масивів, створивши застосунок музичного плеєра. Ви зможете відтворювати, призупиняти, пропускати та перемішувати пісні.

Вам надано HTML та CSS для цього проєкту, щоб ви могли зосередитися на JavaScript.

Почніть із доступу до елементів `#playlist-songs`, `#play` та `#pause` за допомогою методу `getElementById()`. Призначте їх змінним `playlistSongs`, `playButton` та `pauseButton` відповідно.
...
Призначте елемент `#playlist-songs` до змінної `playlistSongs`.
...
Призначте елемент `#play` до змінної `playButton`.
...
Призначте елемент `#pause` до змінної `pauseButton`.
