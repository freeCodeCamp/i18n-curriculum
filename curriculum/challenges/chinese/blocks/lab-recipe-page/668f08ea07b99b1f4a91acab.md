...
1. 你应该有一个 `html` 元素，且 `lang` 属性设置为 `en`。
1. 你应该有一个 `head` 元素，包含一个带有你菜谱名称的 `title` 元素，以及一个 `meta` 元素，其 `charset` 属性设置为 `UTF-8`。
1. 你应该有一个 `body` 元素。
1. 你应该只有一个带有你菜谱名称的 `h1` 元素。
1. 你应该在 `h1` 元素下方有一个介绍菜谱的 `p` 元素。
1. 你应该有一个 `h2` 元素，文本为 `Ingredients`，用于配料部分。
1. 你应该在第一个 `h2` 元素下方有一个无序列表（`ul` 元素），其中至少包含四个列出配料的列表项（`li` 元素）。
1. 你应该有第二个 `h2` 元素，文本为 `Instructions`，用于说明部分。
1. 你应该在第二个 `h2` 元素下方有一个有序列表（`ol` 元素），其中至少包含四个按顺序列出菜谱步骤的列表项。
1. 你应该有一个 `img` 元素，其 `src` 属性设置为有效图像，且有一个描述图像的 `alt` 属性。如果你愿意，可以使用 `https://cdn.freecodecamp.org/curriculum/labs/recipe.jpg`。
...
你的 `html` 元素应该有一个闭合标签。闭合标签在 `<` 字符后紧跟一个 `/`。
...
你的 `body` 元素应该有一个闭合标签。闭合标签在 `<` 字符后紧跟一个 `/`。

```js
assert.match(code, /<\/\s*body\s*>/i);
```

你应该只有一个带有你菜谱名称的 `h1` 元素。
