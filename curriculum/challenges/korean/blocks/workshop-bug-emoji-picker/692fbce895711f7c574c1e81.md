...
이제 `isSelect` 함수 안에서 `element`이 `HTMLSelectElement`의 인스턴스인지 여부를 나타내는 불리언을 반환하세요. 이 확인을 수행하려면 `instanceof` 연산자를 사용하세요.

# --hints--

`isSelect` 함수는 `element instanceof HTMLSelectElement`를 반환해야 합니다.
