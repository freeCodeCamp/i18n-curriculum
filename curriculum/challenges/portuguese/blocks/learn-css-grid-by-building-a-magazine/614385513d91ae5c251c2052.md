...
Adicione um elemento `title` com o texto `Magazine`, um elemento `link` para a folha de estilos da fonte `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap`, um `link` para a folha de estilos FontAwesome `https://use.fontawesome.com/releases/v5.8.2/css/all.css` e um `link` para sua folha de estilos `./styles.css`.
...
Seu código deve ter um elemento `title`.
...
Seu elemento `title` deve estar dentro do seu elemento `head`.
...
O projeto deve ter o título `Magazine`. Lembre-se, maiúsculas, minúsculas e a ortografia são importantes para o título.
