...
ブートストラップを使うので、見出し要素を中央揃えにして見た目を良くしましょう。やることは、`text-center`要素に`h2`クラスを追加するだけです。

複数のクラスを同じ要素に追加する場合は、スペースで区切ってください。例えば次のようにします。
...
`h2`要素は`text-center`クラスを適用して中央揃えにしてください。

```js
assert.isTrue(document.querySelector('h2')?.classList?.contains('text-center'));
```

`h2`要素には引き続き`red-text`クラスが付いているはずです。
