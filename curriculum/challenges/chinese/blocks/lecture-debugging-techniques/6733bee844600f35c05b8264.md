...
在上一课中，你学习了如何在程序中抛出异常。在本课中，我们将看看如何在 `try...catch...finally` 块中优雅地处理这些错误。

`try` 块用于包裹可能抛出误差的代码。它作为一个安全空间来尝试可能失败的操作。

`catch` 块捕捉并处理在 `try` 块中发生的误差。你可以在 `catch` 内使用 `Error` 对象来检查出了什么问题。

`finally` 块在 `try` 和 `catch` 块之后运行，无论是否发生错误。它通常用于清理任务，比如关闭文件或释放资源。

下面是使用 `try...catch` 块的一个示例：
...
我们在 `try` 块中调用该函数。由于函数调用抛出了错误，它会被 `catch` 块捕获，并且错误信息会显示在控制台中。
...
想想 `try...catch...finally` 语句中块的顺序，以及每个块处理的任务类型。
