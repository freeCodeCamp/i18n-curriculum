...
Ви маєте задати розмір і форму своїм кастомним радіокнопкам.

Продовжуючи націлюватися на `.contact-method-radio-btn`, встановіть `width` у `20px`, `height` у `20px`, `border-radius` у `50%` і `border` у `2px solid gray`.

# --hints--

Ви маєте продовжувати націлюватися на клас `.contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Встановіть властивість `width` у `20px`.
...
Встановіть властивість `height` у `20px`.
...
Встановіть властивість `border-radius` у `50%`.
...
Встановіть властивість `border` у `2px solid gray`.
