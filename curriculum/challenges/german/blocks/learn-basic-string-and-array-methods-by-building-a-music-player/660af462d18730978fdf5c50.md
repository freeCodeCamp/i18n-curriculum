...
Jetzt ist es an der Zeit, das Gelernte über Pfeilfunktionen in Ihrem Musikplayer-Projekt anzuwenden.

Beginnen Sie damit, die `printGreeting`, `printMessage` und `addTwoNumbers` Funktionen zu entfernen. Entfernen Sie außerdem alle Konsolenanweisungen und Funktionsaufrufe.

# --hints--

Sie sollten keine `printGreeting` Funktion haben.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

Sie sollten keine `printMessage` Funktion haben.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

Sie sollten keine `addTwoNumbers` Funktion haben.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

Sie sollten keinen `printGreeting();` Funktionsaufruf haben.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

Sie sollten keinen `printMessage('freeCodeCamp');` Funktionsaufruf haben.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

Sie sollten keine `console.log(addTwoNumbers(3,4));` Konsolenanweisung haben.
