...
- **Declaración de variables**: Para declarar una variable, se empieza con el nombre de la variable seguido del operador de asignación (`=`) y luego el valor. Esto puede ser un número, cadena, booleano, etc. Aquí hay algunos ejemplos:
...
- Los nombres de las variables sólo pueden comenzar con una letra o un guion bajo (_) y no un número.
  ...
  - Los nombres de variables con múltiples palabras se separan por guiones bajos. Por ejemplo, `snake_case`.
...
- **`strip()`**: Este método de cadena devuelve una copia de la cadena con los caracteres especificados al inicio y al final eliminados (si no se pasa ningún argumento al método, elimina los espacios en blanco al inicio y al final).
...
- **`find()`**: Este método de cadena devuelve el índice de la primera aparición de una subcadena. Si no se encuentra ninguna, entonces se devuelve `-1`:
...
- **`isupper()`**: Este método de cadena devuelve `True` si todas las letras en la cadena están en mayúsculas y `False` en caso contrario:
...
- **`islower()`**: Este método de cadena devuelve `True` si todas las letras en la cadena están en minúsculas y `False` si no es así:
...
- **`maketrans()`**: Este método de cadena se usa para crear una tabla de mapeos de caracteres 1 a 1 para la traducción. A menudo se usa con el método `translate()` que aplica esa tabla a una cadena y devuelve el resultado traducido.
...
- **Función `float()`**: Puedes usar esta función para convertir un entero en un float.
...
- **`int()` Function**: Puedes usar esta función para convertir un float en un entero.
...
- **Definición**: Las funciones son piezas reutilizables de código que reciben entradas (argumentos) y devuelven una salida. Para llamar a una función, necesitas referenciar el nombre de la función seguido de un conjunto de paréntesis:
...
- **Cortocircuito**: Los operadores `and` y `or` son conocidos como operadores de cortocircuito. El cortocircuito significa que Python verifica los valores de izquierda a derecha y se detiene tan pronto como determina el resultado final.
...
- Si el elemento especificado en el método `count()` no está presente en la tupla, entonces el valor devuelto será `0`:
...
- El argumento obligatorio `stop` es un entero (no inclusivo) que representa el punto final para la secuencia de números que se está generando. Aquí tienes un ejemplo de cómo usar la función `range()`:
...
- **Operador de diferencia (`-`)**: El operador de diferencia `-` devuelve un nuevo conjunto con los elementos del primer conjunto que no están en el otro conjunto.
...
- **IndexError**: Obtendrás un `IndexError` si accedes a un índice que no existe en una lista u otras secuencias como tupla y cadena. Por ejemplo, en una cadena `Hello world`, el índice del último carácter es `10`. Si accedes a un carácter de esta manera, `greet = "hello world"; print(greet[11])`, obtendrás un error con el mensaje `IndexError: string index out of range`.
...
- **`try...except`**: Esto se usa para ejecutar un bloque de código que podría generar una excepción. El bloque `try` es donde anticipas que podría ocurrir un error, mientras que el bloque `except` captura una excepción especificada y se ejecuta si se genera ese error. Aquí tienes un ejemplo:
...
- **operaciones aritméticas como suma, resta, multiplicación, división y otras**. Se llama a `__add__()` para suma, a `__sub__()` para resta, a `__mul__()` para multiplicación y a `__truediv__()` para división.
...
- **Clases**: La estructura para crear objetos. Cada objeto creado a partir de una clase tiene atributos que definen datos y métodos que determinan los comportamientos del objeto.
...
### Ejemplo del módulo Python `heapq`
