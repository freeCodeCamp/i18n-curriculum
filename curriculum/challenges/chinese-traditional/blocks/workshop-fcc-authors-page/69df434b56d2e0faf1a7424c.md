...
現在你的 fCC Authors 頁面已經完全運作，讓我們重構它以提升可讀性，改用 `async` 和 `await` 來取代使用串鏈方式的 `then`。

回想一下，為了使用 `await` 運算子等待一個回傳 `Promise` 的函式，你需要將該函式呼叫包裹在 `async` 函式中（如果它不在模組的主體中）。

由於你的 `fetch` 呼叫既不在非同步函式中，也不在模組的主體中，因此在修正之前無法使用 `await`。

將 `fetch` 述語和整個串鏈的 then 與 catch 方法包成一個新的非同步箭頭函式，命名為 `initialFetch`，且不帶任何引數。
...
`initialFetch` 應該是一個函式。

```js
assert.isFunction(initialFetch)
```

你應該使用 `const` 來創建一個 `initialFetch` 函式。

```js
assert.match(code, /const\s+initialFetch\s*=\s*/)
```

`initialFetch` 應該是一個非同步函式。
...
你的 `initialFetch` 函式不應該帶有任何參數。

```js
assert.match(code, /const\s+initialFetch\s*=\s*.*\s*\(\s*\)\s*/)
```

你的 `initialFetch` 函式應該使用箭頭語法。

```js
assert.match(code, /const\s+initialFetch\s*=\s*.*\s*\(.*\)\s*=>\s*/)
```

你的 `initialFetch` 函式不應該是空的。
...
你的 `initialFetch` 函式應該包含 `fetch` 述語和串鏈方法。
