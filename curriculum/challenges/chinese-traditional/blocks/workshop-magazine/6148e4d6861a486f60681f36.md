...
給 `.image-wrapper` 選擇器一個設置爲 `2fr 1fr` 的 `grid-template-columns` 屬性和一個設置爲 `repeat(3, min-content)` 的 `grid-template-rows` 屬性。 這將使我們的網格行可以根據內容調整高度，但列可以根據容器保持固定的寬度。

# --hints--

你的 `.image-wrapper` 選擇器應該有一個 `grid-template-columns` 屬性，設置爲 `2fr 1fr`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

你的 `.image-wrapper` 選擇器應該有一個 `grid-template-rows` 屬性，設置爲 `repeat(3, min-content)`。
