...
- 會傳回錯誤的情況：
  - 如果提供給函式（程式）的問題**太多**。限制是**五個**，超過將傳回：
    `'Error: Too many problems.'`
  - 該函式將接受的適當運算子是**加法**和**減法**。乘法和除法將傳回錯誤。此項未提及的其他運算子則不需要測試。傳回的錯誤為：
    `"Error: Operator must be '+' or '-'."`
  - 每個數字（運算元）應該只包含數字。否則，函式將傳回：
    `'Error: Numbers must only contain digits.'`
  - 每個運算元（即運算子兩側的數字）寬度最多為四位數。否則，將傳回錯誤字串：
...
`arithmetic_arranger(["24 + 85215", "3801 - 2", "45 + 43", "123 + 49"])` 應該傳回 `'Error: Numbers cannot be more than four digits.'`。
