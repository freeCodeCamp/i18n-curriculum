...
다음으로, `case`에 대한 `"forward"` 절을 추가하세요.

그 `case` 안에서 `setPlayerCards` 함수를 `players.filter()` 인수와 함께 호출하세요.

`filter()` 메서드 안에 `player` 매개변수를 가진 콜백 함수를 추가하여 `player.position`가 `"forward"`와 같은지 확인하세요.

마지막으로, `break` 함수 호출 아래에 `setPlayerCards` 문장을 추가하세요.

# --hints--

`case` 문장 안에 `"forward"`에 대한 새로운 `switch`을 추가해야 합니다.

```js
assert.match(code, /\s*case\s*('|"|`)\s*forward\s*\1\s*:\s*/)
```

`"forward"` `case` 안에서 `setPlayerCards` 함수를 호출해야 합니다.

```js
assert.match(code, /\s*case\s*('|"|`)\s*forward\s*\1\s*:\s*setPlayerCards\(/)
```

`players.filter()` 호출에 `setPlayerCards`를 전달해야 합니다.
