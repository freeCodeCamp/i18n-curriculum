...
داخل الدالة `handleShapeSelect`، نفذ استدعاء الطريقة `.preventDefault()` على `e`، واستدعِ الدالة `clearInputFields()` لمسح حقول الإدخال.

# --hints--

يجب استدعاء `.preventDefault()` على `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

يجب استدعاء الدالة `clearInputFields`.
