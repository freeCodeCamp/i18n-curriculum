...
现在，你需要以一种即使对于 `0/10` 或 `5/10` 也能正常工作的方式获取 `10/10` 文本中的第一个数字。

解析 `.count` 元素的文本内容以提取当前数字。

将计数从字串转换为数字。

用你的新日志替换之前的 `console.log(countEl.textContent)`。

以此格式记录当前计数：`console.log("Current count:", currCount)`。

提示：记住你可以使用 `split` 方法将字串分割成子字串数组。
...
你应该以适用于各种可能值的方式将当前计数作为数字记录到日志中。
