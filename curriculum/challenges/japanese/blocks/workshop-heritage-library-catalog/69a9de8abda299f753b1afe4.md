...
`for`を繰り返す`catalog`ループを追加してください。ループ内で`const entry = catalog[i]`を宣言します。4つのフィールドをカンマで区切った行の文字列を作成し、`rows`に追加してください。

文字列フィールドは出力に二重引用符の文字（例：`"King, Stephen"`）が必要です。これは値の中のカンマがCSV形式を壊さないようにするためです。`year`は数値なので、二重引用符は不要です。

# --hints--

`for`を繰り返すには`catalog`ループを使うべきです。

```js
assert.match(exportToCSV.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

各行はエントリーのフィールドをカンマで区切る必要があります。

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

文字列フィールドは二重引用符で囲む必要があります。
