...
Erstelle unter deinem letzen `p`-Element ein weiteres `p`-Element mit dem Text `Sodium 160mg 7%`. Put `Sodium` and `7%` each in their own `span` with a class of a `bold` like you did with the others.
...
Das `span` um `Sodium` sollte das `class`-Attribut auf `bold` gesetzt haben.
...
Das `span`-Element um `7%` sollte das `class`-Attribut auf `bold` gesetzt haben.
