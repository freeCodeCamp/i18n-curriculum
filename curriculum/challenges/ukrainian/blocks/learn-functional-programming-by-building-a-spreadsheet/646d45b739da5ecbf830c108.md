...
Ваша функція `apply` повертає функцію електронної таблиці, але фактично її не застосовує. Оновіть `apply`, щоб викликати цю функцію. Передайте результат виклику `toNumberList` з `args` як аргумент.

# --hints--

Ваша функція `apply` має викликати функцію `spreadsheetFunctions[fn.toLowerCase()]`.
...
Ви маєте передати виклик `toNumberList()` у виклик `spreadsheetFunctions[fn.toLowerCase()]`.
...
Ви маєте передати `args` у виклик `toNumberList()`.
