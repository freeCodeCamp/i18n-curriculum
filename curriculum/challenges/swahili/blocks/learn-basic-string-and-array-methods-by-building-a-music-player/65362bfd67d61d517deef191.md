...
Njia ya <dfn>find()</dfn> inapata kipengele cha kwanza ndani ya safu ya data kinachokidhi masharti yaliyotolewa katika kitendakazi cha mwitiko kilichotolewa. Ikiwa hakuna kipengele kinachokidhi sharti, njia hiyo inarudisha `undefined`.

Katika mfano hapa chini, njia ya `find()` inatumika kupata nambari ya kwanza kubwa zaidi ya 25:
...
Tumia `const` kuunda kigezo kinachoitwa `song` na weka thamani ya matokeo ya wito wa njia ya `find()` kwenye safu ya data ya `userData?.songs`. Tumia `song` kama kigezo cha wito wa kurudisha wa `find()` na angalia kama `song.id` ni sawa kabisa na `id`.

Hii itapita kupitia safu ya data ya `userData?.songs`, ikitafuta wimbo unaolingana na `id` uliotumwa katika kitendakazi cha `playSong`.

# --hints--

Unapaswa kutumia njia ya `find` kwenye `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

Njia yako ya `find` inapaswa kuwa na `song` kama kigezo cha kitendakazi cha mshale cha wito wa kurudisha.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

Njia yako ya `find` inapaswa kutumia usawa mkali kuangalia kama `song.id` ni sawa na `id`.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

Njia yako ya `find` inapaswa kupewa thamani ya kigezo thabiti cha `song`.
