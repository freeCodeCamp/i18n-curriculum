...
Ogni post del forum includerà una lista di immagini avatar degli utenti che rappresentano tutti gli utenti che partecipano alla conversazione su quell’argomento.

Inizia creando una funzione freccia chiamata `avatars`, con due parametri chiamati `posters` e `users`.

# --hints--

Dovresti avere una funzione freccia vuota chiamata `avatars`.

```js
assert.match(code, /const\s+avatars\s*=\s*\([\s\S]*\)\s*=>\s*{\s*}/);
```

Dovresti avere i parametri chiamati `posters` e `users` nella funzione chiamata `avatars`.
