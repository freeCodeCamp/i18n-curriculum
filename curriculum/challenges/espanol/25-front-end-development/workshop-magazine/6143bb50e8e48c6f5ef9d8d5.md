...
Dentro de tu elemento `aside`, crea dos elementos `img`, un elemento `blockquote`, y un tercer elemento `img`. Asigna al elemento `blockquote` una `class` establecida en `image-quote`.

# --hints--

Debes crear tres elementos `img` dentro de tu elemento `aside`.

```js
assert.lengthOf(document.querySelectorAll('aside img'), 3);
```

Debes crear un elemento `blockquote` dentro de tu elemento `aside`.

```js
assert.exists(document.querySelector('aside blockquote'));
```

Tu elemento `blockquote` debe tener una `class` establecida en `image-quote`.

```js
assert.isTrue(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

Tus nuevos elementos deben estar en el orden correcto.
