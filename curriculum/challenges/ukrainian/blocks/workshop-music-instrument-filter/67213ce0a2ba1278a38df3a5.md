...
Видаліть свій `console.log` з обробника події та встановіть внутрішній HTML `productsContainer` як результат виклику `instrumentCards` з вибраною опцією категорії.

Потім вибирайте різні опції з випадаючого списку і дивіться результат у вікні попереднього перегляду.
...
Коли опція випадаючого меню змінюється, ви повинні встановити внутрішній HTML `productsContainer` як результат виклику функції `instrumentCards` з вибраною опцією як аргументом.
...
Ви повинні видалити `console.log(instrumentCards(selectContainer.value))` з функції зворотного виклику вашого обробника події.
