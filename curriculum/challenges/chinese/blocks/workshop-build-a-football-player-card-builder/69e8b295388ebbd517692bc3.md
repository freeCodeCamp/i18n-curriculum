...
还记得 JSX 中，括弧状的花括弧让你可以直接在 JSX 内嵌入任何 JavaScript 表达式。这就是如何显示来自 props 的动态数据。在 TSX 中你也同样这么做。

卡片头部仍然显示硬编码的值。将 `98` 内的文本 `card-rating` 替换为 `player.overallRating`，并将 `ST` 内的 `card-position` 替换为 `player.position`。

# --hints--

你应该将 `98` 中硬编码的 `player.overallRating` 替换为 `.card-rating`。

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

你应该将 `ST` 中硬编码的 `player.position` 替换为 `.card-position`。
