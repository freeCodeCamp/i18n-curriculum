...
Um Ihre `simulateFestival`-Funktion abzuschließen, zeigen Sie eine Zusammenfassung der Simulationsergebnisse an. Rufen Sie `printSummary()` mit Ihrem lokalen `throughputSummary`-Objekt auf.

# --hints--

Sie sollten `printSummary()` mit `throughputSummary` aufrufen.
