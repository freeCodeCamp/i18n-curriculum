...
Ukikagua kwa makini, utaona wimbo unaochezwa sasa hauangaziwi kwenye orodha ya nyimbo, kwa hivyo hufahamu ni wimbo gani unaochezwa. Unaweza kurekebisha hili kwa kuunda kitendakazi cha kuangazia wimbo wowote unaochezwa.

Ukitegemea sintaksia ya mshale, unda kitendakazi cha `highlightCurrentSong`. Ndani ya kitendakazi hicho, tumia `querySelectorAll` kuchukua kipengele cha `.playlist-song` na weka thamani yake katika thabiti ya `playlistSongElements`.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi cha `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` inapaswa kuwa kitendakazi.

```js
assert.isFunction(highlightCurrentSong)
```

Kitendakazi chako cha `highlightCurrentSong` kinapaswa kutumia sintaksia ya mshale.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

Unapaswa kuunda thabiti ya `playlistSongElements` ndani ya kitendakazi chako cha `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

Unapaswa kutumia njia ya `querySelectorAll()` kuchagua kipengele cha `.playlist-song` na kuweka thamani yake katika thabiti ya `playlistSongElements`.
