...
A função `apply` está retornando a função de planilha, mas sem aplicá-la.Atualize `apply` para que chame a função.Passe o resultado da chamada de `toNumberList`, com `args` como um argumento.

# --hints--

A função `apply` deve acessar a função `spreadsheetFunctions[fn.toLowerCase()]`.
...
Você deve passar uma chamada de `toNumberList()` para a chamada de `spreadsheetFunctions[fn.toLowerCase()]`.
...
Você deve passar `args` para a chamada de `toNumberList()`.
