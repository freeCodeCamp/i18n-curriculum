...
당신의 `calculateTaxes` 메서드는 `taxRate` 값을 (백분율로 변환하기 위해 100으로 나눈 후) `amount` 매개변수와 곱한 값을 반환해야 합니다.

명확하게 하기 위해 `taxRate / 100` 계산을 소괄호로 감싸세요.

# --hints--

`taxRate`를 100으로 나누어야 합니다. `calculateTaxes` 메서드에서 이 작업을 하세요. `this` 키워드를 사용하는 것을 잊지 마세요.
...
`this.taxRate / 100` 계산을 소괄호로 감싸야 합니다.
...
`amount` 매개변수에 `taxRate`를 100으로 나눈 값을 곱해야 합니다. `calculateTaxes` 메서드에서 이 작업을 하세요.
