...
Agrega dos elementos `span` con el `className` de `stat-value` y `stat-label` respectivamente a cada `stat-row`.

# --hints--

Cada `.stat-row` en el segundo `.stat-col` debe contener un `span` con la clase `stat-value` y un `span` con la clase `stat-label`.
