...
Горизонтально центрируйте весь текст всередині елемента `address` та додайте відступи.
...
Ви повинні задати `address` відступ `text-align` зі значенням `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Ви повинні задати `address` відступ `padding-top` не менше ніж `1px`.
...
Ви повинні задати `address` відступ `padding-right` не менше ніж `1px`.
...
Ви повинні задати `address` відступ `padding-bottom` не менше ніж `1px`.
...
Ви повинні задати `address` відступ `padding-left` не менше ніж `1px`.
