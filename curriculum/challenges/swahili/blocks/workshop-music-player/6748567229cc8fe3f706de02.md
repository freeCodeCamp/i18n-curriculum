...
Sasa unahitaji kufanya kazi ya kusitisha wimbo unaocheza kwa sasa.

Tengeneza kitendakazi cha `pauseSong` ambacho hakichukui vigezo vyovyote. Ndani ya kitendakazi chako kipya, ili kuhifadhi wakati wa sasa wa wimbo unapositishwa, weka `songCurrentTime` ya kitu cha `userData` kuwa `currentTime` ya kigezo cha `audio`.


# --hints--

Unapaswa kuwa na kitendakazi cha `pauseSong`.

```js
assert.isFunction(pauseSong);
```

Kitendakazi chako cha `pauseSong` kinapaswa kuweka `songCurrentTime` ya kitu cha `userData` kuwa `currentTime` ya kigezo cha `audio`.
