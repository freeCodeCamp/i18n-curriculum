...
Du solltest einen `#controls, #stats`-Selektorliste haben.
...
Dein `#controls, #stats`-Selektor sollte einen `border` von `1px solid #0a0a23` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#controls, #stats')?.border, '1px solid rgb(10, 10, 35)');
```

Dein `#controls, #stats`-Selektor sollte einen `color` von `#0a0a23` haben.
