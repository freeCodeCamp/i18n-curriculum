...
Bis jetzt ändert sich nichts, wenn Sie einen anderen Bug auswählen.

Fügen Sie einen `change`-Ereignislistener zu `selectElement` hinzu. Verwenden Sie im Callback `isSelect`, um zu prüfen, ob `e.target` ein `HTMLSelectElement` ist. Im `if`-Block müssen Sie noch nichts tun.

# --hints--

Ihr `selectElement` sollte einen `change`-Ereignislistener mit einem Callback haben, der einen Ereignis-Parameter entgegennimmt.
...
Der Callback Ihres Ereignislisteners sollte eine `if`-Anweisung mit der Bedingung `isSelect(e.target)` enthalten.
