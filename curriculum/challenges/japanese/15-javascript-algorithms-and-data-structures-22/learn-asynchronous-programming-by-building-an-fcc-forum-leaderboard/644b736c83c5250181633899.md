...
`allCategories` オブジェクトが `id` に該当するプロパティを持っているかチェックする `if` 文を作成してください。 これには `hasOwnProperty()` メソッドを使うことを思い出してください。

# --hints--

`if` 文を作成して、`allCategories` オブジェクトが `id` に該当するプロパティを持っているかを確認しましょう
