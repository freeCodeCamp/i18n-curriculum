...
Ora, usa l'evento `onload` per rimuovere la classe `hidden` quando l'immagine viene caricata e nascondi il segnaposto di caricamento usando il suo `previousElementSibling`.

# --hints--

Il tuo elemento `img` dovrebbe avere un gestore `onload`.
...
Il tuo gestore `onload` dovrebbe usare `classList.remove` per rimuovere la classe `hidden`. Non dimenticare che qui ti serve la parola chiave `this`.
...
Il tuo gestore `onload` dovrebbe impostare `previousElementSibling.style.display` su `'none'`. Non dimenticare che per farlo ti serve la parola chiave `this`.
