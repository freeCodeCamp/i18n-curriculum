...
如果仔细检查，你会发现当前播放的歌曲未在播放列表中突出显示，因此你实际上不知道正在播放哪首歌曲。 你可以通过创建一个函数来突出显示正在播放的任何歌曲来解决此问题。

使用箭头语法创建一个 `highlightCurrentSong` 函数。 在函数内部，使用 `querySelectorAll` 获取 `.playlist-song` 元素并分配给 `playlistSongElements` 常量。

# --hints--

你应该使用 `const` 来创建 `highlightCurrentSong` 函数。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` 应该是一个函数。

```js
assert.isFunction(highlightCurrentSong)
```

你的 `highlightCurrentSong` 函数应该使用箭头语法。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

你应该在 `highlightCurrentSong` 函数内创建一个 `playlistSongElements` 常量。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

你应该使用 `querySelectorAll()` 方法选择 `.playlist-song` 元素并将其分配给 `playlistSongElements` 常量。
