...
A continuación, accede al elemento `#posts-container` utilizando el método `getElementById()`. asígnalo a una nueva constante llamada `postsContainer`.

# --hints--

Debes utilizar `const` para declarar una variable `postsContainer`.

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

Debes utilizar `document.getElementById()` para obtener el elemento `#posts-container`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

Debes asignar el elemento `#posts-container` a tu variable `postsContainer`.
