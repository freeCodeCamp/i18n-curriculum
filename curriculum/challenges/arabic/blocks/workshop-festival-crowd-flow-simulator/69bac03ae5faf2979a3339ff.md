...
الآن بعد أن عرفت البوابة التي تعيد توجيه الحضور المتجاوزين إليها، يمكنك نقلهم.

خذ قيمة `overflowAmount` وأضفها إلى قائمة الانتظار للبوابة التالية عند نفس `tickIndex`.

على وجه التحديد، يجب عليك:

- الوصول إلى البوابة التالية باستخدام `nextGateIndex`.
- الوصول إلى قائمة الانتظار الخاصة بها `queue`.
- إضافة قيمة `overflowAmount` إلى القيمة في الموضع `tickIndex`.

# --hints--

يجب أن تضيف دالة `rerouteOverflow` قيمة `overflowAmount` إلى القيمة عند `tickIndex` في قائمة الانتظار `queue` للبوابة التالية.
