...
Ersetzen Sie, wo relevant, Eigenschaftswerte durch Ihre `--penguin-face`-Variable.

# --hints--

Sie sollten `.face` eine `background-color` von `var(--penguin-face)` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Sie sollten `.chin` eine `background-color` von `var(--penguin-face)` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Sie sollten `var(--penguin-face)` nicht im `.penguin-body`-Selektor verwenden.
