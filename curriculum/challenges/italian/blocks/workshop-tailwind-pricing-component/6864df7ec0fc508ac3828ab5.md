...
Dal CSS tradizionale, hai imparato che assegnare a un elemento un display `grid` non fa nulla finché non specifichi le righe e le colonne della griglia. Funziona così anche in Tailwind.

In Tailwind, per assegnare una riga e una colonna della griglia, usi le rispettive classi di utilità in questo modo:
...
Assegna al wrapper `div` le classi di utilità `grid-cols-1` e `md:grid-cols-3`.

La classe `grid-cols-1` dispone i tre elementi in una singola colonna, impilandoli verticalmente. Su schermi medi e più grandi, `md:grid-cols-3` cambia il layout in tre colonne, mostrando gli elementi affiancati.

# --hints--

Il tuo elemento `div` dovrebbe avere la classe `grid-cols-1`.
...
Il tuo elemento `div` dovrebbe avere la classe `md:grid-cols-3`.
