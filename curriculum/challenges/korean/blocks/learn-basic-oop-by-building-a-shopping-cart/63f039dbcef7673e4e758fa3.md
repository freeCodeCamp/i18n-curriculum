...
브라우저에는 사용자에게 확인 메시지를 표시하는 내장 `confirm()` 함수가 있습니다. `confirm()`은 사용자에게 표시할 메시지인 문자열을 받습니다. 사용자가 확인하면 `true`를 반환하고, 취소하면 `false`를 반환합니다.

변수 `isCartCleared`를 선언하고 문자열 `confirm()`와 함께 `"Are you sure you want to clear all items from your shopping cart?"`를 호출한 값을 할당하세요.

# --hints--

`const`를 사용해 `isCartCleared` 변수를 선언하세요.
...
문자열 `Are you sure you want to clear all items from your shopping cart?`을 `confirm()` 함수에 전달하세요.

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(\s*('|"|`)Are you sure you want to clear all items from your shopping cart\?\1\s*\)/);
```

`confirm()` 함수의 값을 `isCartCleared` 변수에 할당하세요.
