...
Sheria yako ya `@keyframes` inapaswa kuitwa `cabins`.
...
Sheria yako ya `@keyframes` inapaswa kufuata sheria yako ya `@keyframes wheel`.
...
Sheria yako ya `@keyframes cabins` inapaswa kuwa na kiteuzi `0%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[0]?.keyText === '0%');
```

Kichaguzi chako cha `0%` kinafaa kuwa na sifa ya `transform` iliyowekwa kuwa `rotate(0deg)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[0]?.style?.transform === 'rotate(0deg)');
```

Sheria yako ya `@keyframes cabins` inapaswa kuwa na kichaguzi `100%`.
