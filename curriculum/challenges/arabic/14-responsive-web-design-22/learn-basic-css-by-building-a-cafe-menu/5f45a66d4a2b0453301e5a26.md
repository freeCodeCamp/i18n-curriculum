...
بالانتقال للأسفل إلى عنصر `footer` اجعل كل النص له قيمة `14px` لحجم الخط (font size).

# --hints--

يجب أن يكون لديك منتقي `footer`.
...
يجب أن يكون منتقي `footer` الخاص بك تحت تعليقك.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

يجب عليك تعيين خاصية `font-size` إلى `14px`.
...
يجب أن يكون للعنصر `footer` الخاص بك `font-size` بقيمة `14px`.
