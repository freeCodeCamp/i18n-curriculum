...
Aggiungi un'ultima chiave `last_visit_id` al dizionario `constraints`. Per il suo valore, usa `isinstance` per verificare che `last_visit_id` sia una stringa.

# --hints--

Il tuo dizionario `constraints` dovrebbe avere una chiave `last_visit_id`.
...
La chiave `last_visit_id` del tuo dizionario `constraints` dovrebbe avere il valore di `isinstance(last_visit_id, str)`.
