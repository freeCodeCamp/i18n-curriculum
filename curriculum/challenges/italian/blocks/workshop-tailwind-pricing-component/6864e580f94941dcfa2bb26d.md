...
Ora dovresti lavorare sul posizionamento degli elementi all'interno della scheda per il piano.

Per farlo, imposta il contenitore per usare il display a griglia e specifica una dimensione personalizzata per la riga usando le parentesi quadre nella tua classe Tailwind.

Ecco come puoi impostare una riga della griglia personalizzata usando unità frazionarie:
...
Dai al contenitore esterno `div` per il piano le classi di utilità `grid` e `grid-rows-[1fr_auto]`.

# --hints--

Il tuo elemento `div` dovrebbe avere la classe `grid`.
...
Il tuo elemento `div` dovrebbe avere la classe `grid-rows-[1fr_auto]`.
