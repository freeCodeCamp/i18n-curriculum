...
`.fb4-window` 要素に `width` の `30%`、`height` の `10%`、および `border-radius` の `50%` を与えてください。これらはこの建物の丸い窓を作ります。

# --hints--

`.fb4-window` に `width` の `30%` を与える必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

`.fb4-window` に `height` の `10%` を与える必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

`.fb4-window` に `border-radius` の `50%` を与える必要があります。
