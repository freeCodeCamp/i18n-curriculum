...
现在的问题是，一旦预览刷新或你重新加载页面，为玩家卡添加的所有内容都会消失。

将所有输入保存到 `localStorage` 中可以解决这个问题。为此，首先创建一个 `STORAGE_KEY` 常量，其值为字串 `football_player_card`。

# --hints--

你应该声明一个 `STORAGE_KEY` 变量。
...
你的 `STORAGE_KEY` 应该有字串 `football_player_card` 作为它的值。
