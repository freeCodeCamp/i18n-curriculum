...
在你最後一個 `.divider` 之後，創建另一個帶有文字 `Trans Fat 0g` 的 `p` 元素。將字 `Trans` 用 `i` 元素包裹以斜體顯示。為新的 `p` 元素設定 `class` 屬性為 `indent no-divider`。將 `Trans Fat 0g` 包裹在 `span` 元素中以便對齊。
