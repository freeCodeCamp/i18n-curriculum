...
Ora `authorDataArr` è uguale a `data`, che hai visualizzato nella console in precedenza. Visualizza nella console `authorDataArr` per averne conferma.

Inside your `console.log()` statement, add the text `"Author Data Array:"` as the first argument and `authorDataArr` as the second argument. Usa la virgola per separare il testo da `authorDataArr`.
