...
Usa la proprietà `filter` per `blur` il dipinto inserendo `2px` nell'elemento `.canvas`.

Ecco un esempio di regola che aggiunge un `blur` di 3px:
...
Dovresti impostare la proprietà `filter` su `blur(2px)`.
...
Il tuo elemento `.canvas` dovrebbe avere un valore `filter` di `blur(2px)`.
