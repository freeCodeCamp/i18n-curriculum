...
Acesse as propriedades `userData?.currentSong?.title` e `userData?.currentSong?.artist` e atribua-as às variáveis `currentTitle` e `currentArtist` respectivamente.

# --hints--

Você deve acessar o `title` de `currentSong` do objeto `userData`. Não esqueça de usar encadeamento opcional.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

Você deve atribuir o `title` do `currentSong` a uma constante `currentTitle`.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

Você deve acessar o `artist` de `currentSong` a partir do objeto `userData`. Não esqueça de usar optional chaining.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

Você deve atribuir o `artist` do `currentSong` a uma constante `currentArtist`.
