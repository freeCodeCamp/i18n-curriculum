...
Цільте всі `label` елементи всередині `.info` елементів, і встановіть їхній `width` у `10%`, а також зробіть так, щоб вони не займали менше ніж `55px`.

# --hints--

Ви повинні використати або селектор `.info label`, або `.info > label`.
...
Ви повинні надати `label` елементам `width` зі значенням `10%`.
...
Ви повинні надати `label` елементам `min-width` зі значенням `55px`.
