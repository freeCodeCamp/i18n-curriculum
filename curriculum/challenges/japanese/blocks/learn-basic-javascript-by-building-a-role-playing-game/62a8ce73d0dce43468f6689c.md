...
前のプロジェクトでは、連結演算子を使って変数を文字列に挿入する方法を学びました。例えば次のようにです：
...
文字列 `"You now have a new weapon."` を `"You now have a "` に更新し、新しい武器の名前を続けて、文の最後には必ず句点を付けてください。


# --hints--

`text.innerText` の代入を、文字列 `"You now have a "` で始まるように更新してください。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1/);
```

連結演算子を使って、`newWeapon` 文字列の末尾に `text.innerText` を追加してください。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1\s*\+\s*newWeapon/);
```

連結演算子を使って、`text.innerText` 文字列の末尾を `.` で終わらせてください。
