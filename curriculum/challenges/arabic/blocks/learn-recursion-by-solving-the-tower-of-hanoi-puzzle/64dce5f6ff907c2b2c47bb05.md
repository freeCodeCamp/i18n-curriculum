...
عندما يكون `target` فارغًا، يجب نقل القرص بالضرورة من `source` إلى `target`.

بعد الإعلان عن `forward`، أضف جملة `if` للتحقق مما إذا كان `rods[target]` فارغًا. إذا كان كذلك، غيّر قيمة `forward` إلى `True`.

# --hints--

يجب أن تحتوي على جملة `if` للتحقق مما إذا كان `rods[target]` فارغًا، وتعيين `True` إلى `forward`.
