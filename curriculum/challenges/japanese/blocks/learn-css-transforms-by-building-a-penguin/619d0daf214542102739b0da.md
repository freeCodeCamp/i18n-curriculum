...
`.eye-lid` 要素をターゲットにして、`width` を `150%`、`height` を `100%`、`background-color` を `--penguin-face` にしてください。

# --hints--

`.eye-lid` セレクターを使うべきです。

```js
assert.match(code, /\.eye-lid\s*\{/);
```

`.eye-lid` に `width` を `--fcc-expected--` に設定してください。場所は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

`.eye-lid` に `height` を `--fcc-expected--` に設定してください。場所は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.height, '100%');
```

`.eye-lid` に `background-color` を `var(--penguin-face)` に設定してください。
