...
title: 'Problema 39: Triángulos rectángulos enteros'
challengeType: 1
forumTopicId: 302054
...
Si `p` es el perímetro de un triángulo rectángulo con lados de longitud entera, {a,b,c}, hay exactamente tres soluciones para p = 120.
...
`intRightTriangles(500)` debería devolver un número.

```js
assert(typeof intRightTriangles(500) === 'number');
```

`intRightTriangles(500)` debería devolver 420.

```js
assert(intRightTriangles(500) == 420);
```

`intRightTriangles(800)` debería devolver 720.

```js
assert(intRightTriangles(800) == 720);
```

`intRightTriangles(900)` debería devolver 840.

```js
assert(intRightTriangles(900) == 840);
```

`intRightTriangles(1000)` debería devolver 840.
