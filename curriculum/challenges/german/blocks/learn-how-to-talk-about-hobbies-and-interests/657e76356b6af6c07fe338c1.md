...
Im `Present Perfect`-Tempus werden `for` und `since` verwendet, um Zeitdauer und Anfangspunkte zu besprechen.

`For` gibt an, wie lange etwas schon andauert. Zum Beispiel `for five years` oder `for two months`.

`Since` gibt an, wann etwas begonnen hat. Zum Beispiel `since 2020` oder `since last week`.
...
Diese Präposition drückt die Dauer einer Handlung oder Situation aus.
