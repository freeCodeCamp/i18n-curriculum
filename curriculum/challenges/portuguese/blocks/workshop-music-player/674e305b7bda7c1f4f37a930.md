...
Antes de começar a trabalhar em tocar a próxima e a música anterior, você precisa obter o índice de cada música na propriedade `songs` de `userData`. Para fazer isso você pode usar o método de array `indexOf()`, que retorna o primeiro índice no qual um determinado elemento pode ser encontrado no array ou `-1` se o elemento não estiver presente.
...
Crie uma função chamada `getCurrentSongIndex` que retorne o índice em que `userData.currentSong` pode ser encontrado em `userData.songs` e `-1` quando a música atual for `null`.

# --hints--

Você deve ter uma função chamada `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Sua função `getCurrentSongIndex` deve retornar o índice em que `userData.currentSong` pode ser encontrado em `userData.songs`.
