...
In un modulo precedente, hai imparato che puoi creare le tue eccezioni personalizzate ereditando dalla classe integrata `Exception`:
...
Crea una classe chiamata `MediaError`. Fallo ereditare dalla classe `Exception` e aggiungi una docstring che dica `Custom exception for media-related errors.`.

# --hints--

Dovresti creare una classe chiamata `MediaError`.
...
La tua classe `MediaError` dovrebbe ereditare da `Exception`.
...
Dovresti aggiungere la docstring `Custom exception for media-related errors.` alla tua classe `MediaError`.
