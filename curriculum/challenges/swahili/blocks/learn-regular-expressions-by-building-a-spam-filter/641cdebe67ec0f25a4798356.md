...
Kitendakazi chako cha mwitiko kinapaswa kuwa na tamko la `if`.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

Tamko lako la `if` linapaswa kukagua kama `value` ya `messageInput` ni mfuatano wa herufi tupu.
...
Tamko lako la `if` linapaswa kuonyesha tahadhari kwa mtumizi yenye ujumbe `"Please enter a message."`.
...
Tamko lako la `if` linapaswa kutoka katika utekelezaji wa kitendakazi.
