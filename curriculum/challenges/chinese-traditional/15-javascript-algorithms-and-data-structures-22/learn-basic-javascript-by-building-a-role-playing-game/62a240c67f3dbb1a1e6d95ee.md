...
爲 `#game` 提供 `500px` 的最大寬度和 `400px` 的最大高度。 將 `background-color` 設置爲 `#ffffff`，將 `color` 設置爲 `#ffffff`。

使用邊距使其居中，將頂部邊距設置爲 `30px`，底部邊距設置爲 `0px`，左側和右側邊距設置爲 `auto`。
...
你的 `#game` 選擇器應該具有 `500px` 的 `max-width`。
...
你的 `#game` 選擇器應該具有 `400px` 的 `max-height`。
...
你的 `#game` 選擇器應該具有 `#ffffff` 的 `background-color`。
...
你的 `#game` 選擇器應該具有 `#ffffff` 的 `color`。
...
你的 `#game` 選擇器應將 `margin-top` 設置爲 `30px`。
...
你的 `#game` 選擇器應將 `margin-left` 設置爲 `auto`。
...
你的 `#game` 選擇器應將 `margin-right` 設置爲 `auto`。
...
你的 `#game` 選擇器應將 `margin-bottom` 設置爲 `0px`。
