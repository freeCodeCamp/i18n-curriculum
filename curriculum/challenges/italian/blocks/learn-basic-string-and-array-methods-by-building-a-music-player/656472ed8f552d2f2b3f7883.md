...
Aggiungi un blocco `else` per gestire il tempo di riproduzione corrente della canzone. Questo ti permette di riprendere la canzone corrente dal punto in cui è stata messa in pausa.
...
Dovresti aggiungere un blocco `else` al blocco `if` esistente.
...
Dovresti impostare `audio.currentTime` su `userData?.songCurrentTime` all'interno del blocco `else` della tua istruzione `if`.
