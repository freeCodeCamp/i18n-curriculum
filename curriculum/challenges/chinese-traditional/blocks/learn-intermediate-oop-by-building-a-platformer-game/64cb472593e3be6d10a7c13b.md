...
在你的條件中，為你的 `checkpoints` 陣列添加一個 `forEach` 迴圈來迭代。將 `checkpoint` 用作回呼函式的參數名稱。

在 `forEach` 回呼中，使用減法指定運算子從 checkpoints 的 `x` 位置中減去 5。
...
你應該使用減法指派運算子，從 `checkpoint` 的 `x` 位置中減去 5。
