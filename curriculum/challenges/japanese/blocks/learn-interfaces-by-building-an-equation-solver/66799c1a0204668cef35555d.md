...
f-stringsでは、`.nf` 形式指定子を使って数値データの特定の精度を設定できます。ここで`n`は表示する10進数の桁数です。

`result_list`内のf-stringsの波括弧の中に、`3`のすぐ後に`:+`の10進数の桁数を表示するために必要な形式指定子を書いてください。

# --hints--

`result_list`ブロック内の`case 1`に含まれる文字列を`f'x = {results[0]:+.3f}'`に変更してください。
...
`result_list`ブロック内の`case 2`に含まれる文字列を、結果が`3`の10進数の桁数で表示されるように変更してください。
