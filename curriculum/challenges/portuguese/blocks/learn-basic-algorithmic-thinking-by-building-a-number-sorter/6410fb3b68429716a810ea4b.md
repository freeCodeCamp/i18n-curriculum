...
A ordenação de seleção depende do acompanhamento do índice do menor valor do array.Declare uma variável `minIndex` e defina-a como `i` – isso garante que, se o valor atual for o menor, ele será trocado por ele mesmo e não será movido.Você precisará ser capaz de reatribuir o valor de `minIndex` ao percorrer o array.

Em seguida, escreva outro laço `for` usando a variável `j` para realizar a iteração.Esse laço precisa começar no índice após `i` e percorrer o resto do array.

# --hints--

Você deve usar `let` para declarar `minIndex`.
...
Você deve definir `minIndex` como `i`.
...
Você deve usar um laço `for` aninhado.
...
O laço `for` deve inicializar `j` como `i + 1`.
...
O laço `for` deve percorrer o resto do array.
