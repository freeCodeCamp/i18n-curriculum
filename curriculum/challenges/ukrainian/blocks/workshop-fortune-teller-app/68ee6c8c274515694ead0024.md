...
Прогноз має бути випадковим, тому потрібна допоміжна функція для вибору випадкового прогнозу. Оголосіть змінну з іменем `chosenCard` всередині методу `singleCardSelected`. Використайте метод `getRandomItem` для отримання випадкової карти та передайте `cards` як аргумент.

# --hints--

Ви повинні оголосити змінну з іменем `chosenCard` всередині `singleCardSelected`.
...
Вашій змінній `chosenCard` має бути призначено значення через виклик `getRandomItem(this.cards)`.
...
Ви повинні передати `this.cards` як аргумент до `getRandomItem`.
