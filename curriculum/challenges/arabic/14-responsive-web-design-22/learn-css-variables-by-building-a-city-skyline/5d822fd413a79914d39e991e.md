...
يجب عليك إعطاء `.fb2-window` خاصية `width` بقيمة `22%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

يجب عليك إعطاء `.fb2-window` خاصية `height` بقيمة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

يجب عليك إعطاء `.fb2-window` خاصية `background-color` بقيمة `--window-color3`.
