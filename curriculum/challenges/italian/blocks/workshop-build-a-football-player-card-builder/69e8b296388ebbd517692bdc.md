...
Aggiungi un attributo `value` di `player.sho` al SHO `input`. Poi imposta il suo attributo `onChange` su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `sho` impostato su `Number(e.target.value)`.

# --hints--

Il tuo SHO `input` dovrebbe avere un `value` uguale alla statistica `sho` del giocatore corrente.
...
Il tuo SHO `input` dovrebbe avere un gestore `onChange` che chiama `setPlayer` con uno spread di `player` e `sho` impostato su `Number(e.target.value)`.
