...
Declare uma variável `arg_list`.Dê a ele o valor de uma list comprehension que itera sobre `key` e `val` para cada par chave-valor em `vars(self).items()` e calcula a f-string `f'{key}={val}'` em cada iteração.

# --hints--

Você deve declarar uma variável `arg_list`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__repr__").has_variable("arg_list")`)) })
```

Você deve atribuir uma list comprehension que itere sobre `vars(self).items()` para `arg_list`.
...
A list comprehension atribuída a `arg_list` deve usar `key` e `val` para iterar sobre `vars(self).items()`.
...
A compreensão de lista atribuída a `arg_list` deve calcular a string `f'{key}={val}'` para cada par chave-valor em `vars(self).items()`.
