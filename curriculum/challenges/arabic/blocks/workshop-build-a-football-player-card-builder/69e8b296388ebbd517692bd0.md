...
بعد `form-row`، أضف عنصر `div` مع `className` بقيمة `form-group`. بداخله، أضف عنصر `label` مع `className` بقيمة `label`، و`htmlFor` بقيمة `club`، والنص `Club`. ثم أضف عنصر مُدخَل مع `id` بقيمة `club`، و`className` بقيمة `input`، و`type` بقيمة `text`.
...
يجب أن يحتوي `div.form-group` على `label` مع خاصية `htmlFor` بقيمة `club`، و`className` بقيمة `label`، والنص `Club`.
...
يجب أن يحتوي `form-group` الخاص بـ Club على عنصر `input` مع `id` بقيمة `club`، و`className` بقيمة `input`، و`type` بقيمة `text`.
