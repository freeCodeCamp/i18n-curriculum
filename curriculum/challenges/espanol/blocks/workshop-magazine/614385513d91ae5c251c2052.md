...
Comienza agregando un elemento `link` para la hoja de estilos de la fuente `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap`, un `link` para la hoja de estilos FontAwesome `https://use.fontawesome.com/releases/v5.8.2/css/all.css` y un `link` para tu hoja de estilos `./styles.css`.
