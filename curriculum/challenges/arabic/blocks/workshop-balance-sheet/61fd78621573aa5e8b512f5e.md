...
في عنصر `tr` الثالث، أضف عنصر `th` بالنص `Savings Funds set aside for emergencies.`. لف هذا النص، باستثناء `Savings `، داخل عنصر `span` مع تعيين خاصية `class` إلى `description`.

بعد ذلك، أضف ثلاثة عناصر `td` بالنصوص التالية (بالترتيب): `$500`، `$650`، `$728`. اجعل خاصية `class` في عنصر `td` الثالث تساوي `current`.
...
يجب أن يحتوي عنصر `span` على خاصية `class` مع القيمة `description`.
