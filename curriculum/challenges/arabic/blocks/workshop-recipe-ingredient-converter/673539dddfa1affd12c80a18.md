...
ستحتاج إلى اختبار دالتك الجديدة ذات التكوين التدريجي. أعلن عن متغير `servingsResult`، وقم بتعيين نتيجة تعديل الكمية من 4 كأساس إلى 6 حصص.

سجل هذا المتغير في وحدة التحكم.

# --hints--

يجب أن تعلن عن متغير `servingsResult`.

```js
assert.isDefined(servingsResult);
```

يجب أن يحتوي متغير `servingsResult` على نتيجة استدعاء `adjustQuantity(4)(6)`.

```js
assert.strictEqual(servingsResult, 24);
```

يجب أن تسجل قيمة `servingsResult` في وحدة التحكم.
