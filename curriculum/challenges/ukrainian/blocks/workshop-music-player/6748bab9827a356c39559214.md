...
Використайте методи `classList` і `remove()` для видалення класу `playing` з `playButton`, оскільки пісня буде призупинена в цей момент.

Щоб остаточно призупинити пісню, використайте метод `pause()` на змінній `audio`. `pause()` — це метод Web Audio API для призупинення музичних файлів.

# --hints--

Ваша функція `pauseSong` має видалити клас `playing` з `playButton`.
...
Ваша функція `pauseSong` має викликати метод `pause()` на `audio`.
