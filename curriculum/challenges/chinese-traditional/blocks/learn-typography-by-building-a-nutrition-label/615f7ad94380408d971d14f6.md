...
`:not` 偽選擇器可用來選取所有不符合指定 CSS 規則的元素。 
...
上述選取所有沒有 `id` 為 `example` 的 `div` 元素。

修改你的 `.daily-value p` 選擇器以排除 `.no-divider` 元素。

# --hints--

你應該有一個 `.daily-value p:not(.no-divider)` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p:not(.no-divider)'));
```

你不應該有 `.daily-value p` 選擇器。

```js
assert(!new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

你不應該更改 `.daily-value p:not(.no-divider)` 選擇器中的屬性。
