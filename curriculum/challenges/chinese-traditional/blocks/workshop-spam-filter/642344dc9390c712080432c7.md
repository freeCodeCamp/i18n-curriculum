...
與 `freeRegex` 類似，更新 `stockRegex` 以將 `e` 和 `o` 字符替換爲字符類，以匹配字母和相應的數字。

# --hints--

你的 `stockRegex` 應該使用字符類來匹配字母 `e` 和數字 `3`。

```js
assert.match(stockRegex.source, /\[(e3|3e)\]/);
```

你的 `stockRegex` 應該使用字符類來匹配字母 `o` 和數字 `0`。
...
你的 `stockRegex` 應該與 `st0ck al3rt` 匹配。
