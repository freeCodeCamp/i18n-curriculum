...
正如你在之前的课程中学到的，当用户修改某些输入元素的值时，会触发 `change` 事件。你希望在从下拉菜单中选择新值时能够随时更新你的页面。为此，向 `selectContainer` 添加一个针对 `change` 事件的事件监听器。

目前，在回调内，将字串 `this is a test` 记录到控制台。然后，从你的下拉菜单中选择不同的选项，并查看控制台以测试事件监听器是否正常工作。

# --hints--

你应该为 `selectContainer` 添加 `change` 事件监听器，并在传递给该方法的回调中记录字串 `this is a test`。
