...
Mara tu baada ya orodha ya `terms`, tengeneza mzunguko wa `for` na tumia njia ya `.items()` kupita juu ya funguo na thamani zilizohifadhiwa katika sifa ya kipengele ya `coefficients`. Tumia `n` na `coefficient` kama vigezo vya mzunguko.

Ndani ya mzunguko, tengeneza tamko la `if` linalokagua kama msingi wa mzunguko wa sasa una thamani ya uongo na ruka mzunguko huo ikiwa ni hivyo. Hii ni kwa sababu hutaki kuwakilisha misingi yenye thamani ya sifuri.

# --hints--

Unapaswa kutengeneza mzunguko wa `for` unaorudia juu ya `coefficients.items()`.
...
Mzunguko wako wa `for` unapaswa kutumia `n` na `coefficient` kupita juu ya `coefficients.items()`.
...
Unapaswa kutengeneza tamko la `if` kukagua kama `coefficient` ina thamani ya uongo ndani ya mzunguko wako wa `for`.
...
Unapaswa kutumia neno kuu la `continue` ndani ya tamko lako jipya la `if`.
...
Mzunguko wako wa `for` unapaswa kuwekwa mara tu baada ya tamko la `terms`.
