...
Dovresti dare a `.blush` un valore `width` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.width, '15%');
```

Dovresti dare a `.blush` un valore `height` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.height, '10%');
```

Dovresti dare a `.blush` un valore `background-color` di `--fcc-expected--` invece di `--fcc-actual--`.
