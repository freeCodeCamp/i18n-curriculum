...
`.penguin-body` 要素を親要素の上から `40%`、左から `23.5%` の位置に配置してください。

# --hints--

`.penguin-body` に `top` プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top);
```

`.penguin-body` に `top` プロパティを `--fcc-expected--` に設定する必要がありますが、現在は `--fcc-actual--` になっています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top, '40%');
```

`.penguin-body` に `left` プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.left);
```

`.penguin-body` に `left` プロパティを `--fcc-expected--` に設定する必要がありますが、現在は `--fcc-actual--` になっています。
