...
title: Fibonacci-n-Schritt-Zahlenfolgen
challengeType: 1
forumTopicId: 302267
...
Diese Zahlenfolgen sind eine Erweiterung der gewöhnlichen Fibonacci-Folge, bei der:
...
<ol>
  ...
  <li>Für allgemeines $n>2$ haben wir die Fibonacci-$n$-Schritt-Folge – $F_k^n$; mit Anfangswerten der ersten $n$ Werte der $(n-1)$'ten Fibonacci-$n$-Schritt-Folge $F_k^{n-1}$; und der $k$'te Wert dieser $n$'ten Folge ist $F_k^n = \sum_{i=1}^{(n)} {F_{k-i}^{(n)}}$</li>
</ol>

Für kleine Werte von $n$ werden manchmal griechische Zahlpräfixe verwendet, um jede Folge einzeln zu benennen.

Fibonacci-$n$-Schritt-Folgen:

| $n$ | Name der Folge | Werte                                                  |
| --- | -------------- | ----------------------------------------------------- |
...
Verwandte Folgen können erzeugt werden, indem die Anfangswerte geändert werden: Die Lucas-Folge summiert die zwei vorherigen Werte wie die Fibonacci-Folge für $n=2$, verwendet aber $\[2, 1]$ als Anfangswerte.

# --instructions--

Schreiben Sie eine Funktion, um Fibonacci-$n$-Schritt-Zahlenfolgen und Lucas-Folgen zu erzeugen. Der erste Parameter ist $n$. Der zweite Parameter ist die Anzahl der zurückzugebenden Elemente. Der dritte Parameter gibt an, ob die Fibonacci-Folge oder die Lucas-Folge ausgegeben werden soll. Wenn der Parameter `"f"` ist, geben Sie die Fibonacci-Folge zurück, und wenn er `"l"` ist, geben Sie die Lucas-Folge zurück. Die Folgen müssen als Array zurückgegeben werden.
