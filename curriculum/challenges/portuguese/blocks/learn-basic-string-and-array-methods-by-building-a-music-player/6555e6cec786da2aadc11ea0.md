...
Você também deve re-renderizar as músicas, pausar a música que está tocando, configurar a exibição do player e definir novamente o texto acessível do botão de reprodução.

Chame a função `renderSongs` e passe `userData?.songs` como argumento.Além disso, chame as funções `pauseSong`, `setPlayerDisplay` e `setPlayButtonAccessibleText`.

# --hints--

Você não deve modificar a função `shuffle` existente e seu conteúdo.
