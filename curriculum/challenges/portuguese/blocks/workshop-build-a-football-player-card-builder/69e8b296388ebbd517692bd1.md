...
Adicione outro `div` com o `className` de `form-group`. Dentro dele, adicione um elemento `label` com o `className` de `label`, um `htmlFor` de `imageUrl` e o texto `Image URL`. Em seguida, adicione um elemento `input` com o `id` de `imageUrl`, um `className` de `input` e um `type` de `text`.

# --hints--

Você deve ter um elemento `div` com o `className` de `form-group`.
...
Seu `div.form-group` deve ter um `label` com um atributo `htmlFor` de `imageUrl`, um `className` de `label` e o texto `Image URL`.
...
Sua URL da imagem `form-group` deve conter um `input` com um `id` de `imageUrl`, `className` de `input` e um `type` de `text`.
