...
Перейшовши до елемента `span` з текстом `Soundflow`, текст у ньому потрібно зробити у верхньому регістрі.

Ось приклад, який показує, як використати утиліту `uppercase`:

```html
<p class="uppercase">Quincy Larson</p>
```

Додайте клас `uppercase` до елемента `span`.

# --hints--

Ваш елемент `span` має мати утилітний клас `uppercase`.
