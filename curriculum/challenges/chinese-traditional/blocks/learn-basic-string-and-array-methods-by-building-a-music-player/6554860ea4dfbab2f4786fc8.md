...
如果你仔細查看，你會發現目前正在播放的歌曲並未在播放列表中凸顯標示，所以你無法真正知道哪首歌曲正在播放。你可以透過建立一個函式，來凸顯標示任何正在播放的歌曲，從而解決這個問題。

使用箭頭語法，創建一個 `highlightCurrentSong` 函式（程式）。在函式（程式）內，使用 `querySelectorAll` 取得 `.playlist-song` 元素並指定給一個 `playlistSongElements` 常數。

# --hints--

你應該使用 `const` 來創建一個 `highlightCurrentSong` 函式（程式）。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` 應該是一個函式（程式）。

```js
assert.isFunction(highlightCurrentSong)
```

你的 `highlightCurrentSong` 函式應該使用箭頭語法。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

你應該在你的 `highlightCurrentSong` 函式（程式）內創建一個 `playlistSongElements` 常數。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

你應該使用 `querySelectorAll()` 方法來選取 `.playlist-song` 元素，並將其指定給 `playlistSongElements` 常數。
