...
All'interno della callback `useEffect`, aggiungi una condizione `if` per verificare se `query.trim() === ''`. Questo controlla se `query` è una stringa vuota o contiene solo spazi. In tal caso, chiama `setResults([])` e termina subito.

# --hints--

Dovresti verificare se `query.trim() === ''`.
...
Se `query.trim() === ''` è true, chiama `setResults()` con un array vuoto, poi usa la parola chiave `return` subito dopo.
