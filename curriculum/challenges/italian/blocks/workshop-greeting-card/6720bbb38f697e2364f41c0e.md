...
Ora è il momento di aggiungere un po’ di colore. Dai al `body` un `background-color` di `brown` e dai anche al `.card` un `background-color` di `white`.

# --hints--

L’elemento `body` dovrebbe avere un `background-color` di `brown`.
...
Dovresti avere un selettore `.card`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card"));
```

L’elemento `.card` dovrebbe avere un `background-color` di `white`.
