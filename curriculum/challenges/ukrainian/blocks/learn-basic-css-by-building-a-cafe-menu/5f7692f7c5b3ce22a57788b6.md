...
Щоб завершити стилізацію, додайте відповідні імена класів `flavor` та `price` до всіх решти елементів `p`.

# --hints--

У вас має бути п’ять елементів `.flavor`.

```js
assert.lengthOf(document.querySelectorAll('.flavor'), 5);
```

У вас має бути п’ять елементів `.price`.

```js
assert.lengthOf(document.querySelectorAll('.price'), 5);
```

Ваші елементи `.flavor` мають бути вашими елементами `p` з текстом `French Vanilla`, `Caramel Macchiato`, `Pumpkin Spice`, `Hazelnut` та `Mocha`.
...
Ваші елементи `.price` мають бути вашими елементами `p` з текстом `3.00`, `3.75`, `3.50`, `4.00` та `4.50`.
