...
現在你的 `applyFunction` 需要返回一個結果。 返回對 `str2` 調用 `.replace()` 方法的結果。 傳遞你的 `functionCall` 正則表達式和一個空的回調。

# --hints--

你的 `applyFunction` 函數應該返回在 `str2` 上調用 `.replace()` 方法的結果。
...
你應該將 `functionCall` 作爲第一個參數傳遞給 `.replace()` 調用。
...
你應該將一個空的箭頭函數作爲第二個參數傳遞給 `.replace()` 調用。
