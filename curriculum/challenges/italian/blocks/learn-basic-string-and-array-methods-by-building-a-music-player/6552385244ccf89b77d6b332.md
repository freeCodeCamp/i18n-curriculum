...
Devi lavorare sulla riproduzione della canzone successiva e di quella precedente. Per questo, ti serviranno una funzione `playNextSong` e una funzione `playPreviousSong`.

Usa `const` e la sintassi a freccia per creare una funzione `playNextSong` vuota.

# --hints--

Dovresti usare `const` per creare una funzione vuota chiamata `playNextSong`.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` dovrebbe essere una funzione.

```js
assert.isFunction(playNextSong)
```

La tua funzione `playNextSong` dovrebbe usare la sintassi a freccia.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

La tua funzione `playNextSong` dovrebbe essere vuota.
