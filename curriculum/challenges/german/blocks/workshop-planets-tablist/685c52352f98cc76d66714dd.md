...
Jetzt ist es Zeit, den angeklickten Tab als ausgewählt zu markieren.

Verwenden Sie die `setAttribute`-Methode auf dem `tab`, um das `aria-selected`-Attribut auf `"true"` zu setzen.

# --hints--

Wenn ein `tab` angeklickt wird, sollte das `aria-selected`-Attribut des angeklickten Tabs auf `"true"` gesetzt werden.
