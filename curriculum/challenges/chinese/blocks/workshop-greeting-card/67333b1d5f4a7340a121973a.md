...
为你的 `.card-links a` 元素添加一个聚焦时的选择器。将 `outline` 属性设置为 `2px solid yellow`。

# --hints--

你应该有一个 `.card-links a:focus` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:focus"));
```

`.card-links a:focus` 选择器应具有设置为 `2px solid yellow` 的 `outline` 属性。
