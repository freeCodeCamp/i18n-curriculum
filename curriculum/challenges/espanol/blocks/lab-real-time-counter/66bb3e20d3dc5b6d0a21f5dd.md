...
6. Cuando el elemento `#text-input` contiene el texto `I am learning a new language and it is called c++.` el elemento `#char-count` debe contener el texto `"Character Count: 50/50"`. **NOTA**: Aunque el atributo `maxlength` lograría la funcionalidad correcta, este laboratorio requiere una solución solo con JavaScript.
7. El usuario no debería poder ingresar más de `50` caracteres en el elemento `textarea`. Cuando el conteo de caracteres llegue a `50`, cualquier entrada extra debería recortarse automáticamente y el texto `Character Count: 50/50` debería mostrarse en `red`.
...
Cuando el conteo de caracteres es `50`, el texto debe mostrarse en `red`.
...
Si el recuento de caracteres es mayor o igual a `50`, la entrada extra debe ser recortada.
