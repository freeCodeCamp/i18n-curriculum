...
通過兩個 `.map()` 調用和一個 `.reduce()` 調用，你將創建額外的數組，並且迭代的次數比需要的更多。 你應該將所有的邏輯移到 `.reduce()` 調用中，以節省時間和內存。

刪除 `differences`、`squaredDifferences` 和 `sumSquaredDifferences` 變量（以及它們的值）。 聲明一個 `variance` 變量，並給它賦值 `array.reduce()`。 對於回調，傳入標準的 `acc` 和 `el` 參數，但是讓函數主體部分爲空。 不要忘記將初始值設置爲 `0`。

# --hints--

你應該刪除 `differences` 變量。

```js
assert.notMatch(getVariance.toString(), /differences\s*=/);
```

你應該刪除 `squaredDifferences` 變量。

```js
assert.notMatch(getVariance.toString(), /squaredDifferences\s*=/);
```

你應該刪除 `sumSquaredDifferences` 變量。
