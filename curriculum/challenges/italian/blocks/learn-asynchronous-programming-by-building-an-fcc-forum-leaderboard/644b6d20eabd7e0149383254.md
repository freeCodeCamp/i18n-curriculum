...
All'interno della tua funzione `forumCategory`, crea una nuova variabile `let` chiamata `selectedCategory` e assegnale un oggetto vuoto. Questo servirà per archiviare il nome della categoria e il nome della classe per ogni categoria.

# --hints--

Dovresti usare `let` per dichiarare una variabile chiamata `selectedCategory`. 

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

Dovresti assegnare un oggetto vuoto alla variabile `selectedCategory`.
