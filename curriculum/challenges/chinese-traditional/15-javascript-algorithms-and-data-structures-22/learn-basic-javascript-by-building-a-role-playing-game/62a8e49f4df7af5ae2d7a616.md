...
將 `monsterHealth` 設置爲 `monsterHealth` 減去玩家當前武器的威力。

請記住，你可以使用 `weapons[currentWeaponIndex].power` 訪問玩家當前武器的威力。

# --hints--

你應該使用複合賦值來修改 `monsterHealth` 變量。

```js
assert.match(attack.toString(), /monsterHealth\s*-=/);
```

你應該使用帶有 `currentWeaponIndex` 的括號表示法來訪問 `weapons`。

```js
assert.match(attack.toString(), /weapons\s*\[\s*currentWeaponIndex\s*\]/);
```

你應該使用點符號來訪問 `weapons[currentWeaponIndex]` 的 `power` 屬性。

```js
assert.match(attack.toString(), /weapons\s*\[\s*currentWeaponIndex\s*\]\s*\.power/);
```

你應該從 `monsterHealth` 中減去當前武器的 `power`。
