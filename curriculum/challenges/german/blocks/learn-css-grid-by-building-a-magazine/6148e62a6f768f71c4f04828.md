...
Die Eigenschaft `place-items` kann verwendet werden, um die Werte von `align-items` und `justify-items` gleichzeitig festzulegen. Die Eigenschaft `place-items` nimmt einen oder zwei Werte an. Wenn ein Wert angegeben wird, wird dieser sowohl für die Eigenschaften `align-items` als auch `justify-items` verwendet. Wenn zwei Werte angegeben werden, wird der erste Wert für die Eigenschaft `align-items` und der zweite Wert für die Eigenschaft `justify-items` verwendet.

Geben Sie dem Selektor `.image-wrapper` eine Eigenschaft `place-items`, die auf `center` gesetzt ist.

# --hints--

Ihr Selektor `.image-wrapper` sollte eine Eigenschaft `place-items` haben, die auf `center` gesetzt ist.
