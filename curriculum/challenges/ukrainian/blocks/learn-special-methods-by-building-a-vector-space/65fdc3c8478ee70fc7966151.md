...
Так само як ви робили раніше для методу `__eq__`, створіть інструкцію `if`, яка перевіряє, чи `self` та `other` не належать до одного класу та поверніть `NotImplemented`.
