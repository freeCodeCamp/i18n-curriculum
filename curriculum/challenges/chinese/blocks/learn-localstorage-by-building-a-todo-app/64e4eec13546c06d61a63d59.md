...
你需要确定添加到 `taskData` 数组的任务是否已经存在。 如果任务不存在，你将把它添加到数组中。 如果确实存在，你将更新它。 为了实现这一点，你可以使用 <dfn>findIndex()</dfn> 方法。

`findIndex()` 数组方法查找并返回数组中满足提供的测试回调函数指定的条件的第一个元素的索引。 如果没有找到这样的元素，该方法返回 `-1`。 回调应该返回一个真值来表明已经找到匹配元素，否则返回一个假值。
...
使用 `const` 声明一个名为 `dataArrIndex` 的变量，并为其赋值 `taskData.findIndex()`。 对于 `findIndex()` 方法，传入以 `item` 为参数的箭头函数。 

在箭头函数中，检查 `item` 的 `id` 属性是否严格等于 `currentTask` 的 `id` 属性。

# --hints--

你应该使用 `const` 声明 `dataArrIndex` 变量并将其设置为 `taskData.findIndex()`。

```js
assert.match(code, /const\s+dataArrIndex\s*=\s*taskData\.findIndex\(/)
```

你应该传入 `item` 作为箭头函数回调的参数。 不要使用花括号。

```js
assert.match(code, /const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)/)
```

你的箭头函数回调应该检查是否 `item.id === currentTask.id`。
