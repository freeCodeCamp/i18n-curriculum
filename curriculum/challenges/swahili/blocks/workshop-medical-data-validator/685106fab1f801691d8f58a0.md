...
Ongeza ufunguo mwingine `gender` katika kamusi ya `constraints`. Kufuatia muundo wa usemi wa kihesabu uliouandika katika hatua zilizopita, hakikisha kwamba `gender` ni mfuatano wa herufi. Kisha, tumia kigeuzi cha `and` kuangalia kwamba herufi ndogo ya `gender` iko ndani ya `('male', 'female')`.

# --hints--

Kamusi yako ya `constraints` inapaswa kuwa na ufunguo `gender`.
...
Ufunguo wa `gender` katika kamusi yako ya `constraints` unapaswa kuwa na thamani ya `isinstance(gender, str) and gender.lower() in ('male', 'female')`.
