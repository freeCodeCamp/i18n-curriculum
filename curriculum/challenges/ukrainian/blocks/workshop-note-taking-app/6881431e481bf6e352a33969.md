...
Як згадувалося раніше, коли користувач редагує нотатку, а потім натискає поза межами нотатки, має з’явитися повідомлення про те, що нотатку збережено.

Щоб це зробити, встановіть `statusEl.textContent` у рядок `"Note saved successfully!"`.

Тепер, коли ви протестуєте це, на екрані має з’явитися це повідомлення.

# --hints--

Коли користувач редагує нотатку, повідомлення про стан має оновлюватися, щоб вказати, що нотатку збережено.
