...
Puoi scrivere testo citato direttamente all’interno dell’elemento di citazione a blocchi, come hai fatto nel primo elemento `section`. In alternativa, puoi racchiuderlo in uno o più elementi paragrafo. Questo è utile quando il testo ha più paragrafi, ma vuoi mantenerli all’interno dello stesso elemento di citazione a blocchi.

Ecco un esempio con due paragrafi:
...
Nella seconda sezione, all’interno dell’elemento di citazione a blocchi esistente, aggiungi quattro elementi `p` con i seguenti testi, in ordine:
...
Dovresti avere quattro elementi `p` annidati dentro l’elemento `blockquote` all’interno della seconda sezione.
...
Il tuo primo elemento `p` dovrebbe avere il testo `So much of getting a job is who you know.`
...
Il tuo secondo elemento `p` dovrebbe avere il testo `It's OK to be an introvert, but you do need to push your boundaries.`
...
Il tuo terzo elemento `p` dovrebbe avere il testo `Create GitHub, Twitter, LinkedIn, and Discord accounts.`
...
Il tuo quarto elemento `p` dovrebbe contenere il testo `Go to tech meetups and conferences. Travel if you have to.`
