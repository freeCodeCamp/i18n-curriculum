...
لتخصيص حجم الصورة الرمزية، يمكنك تعيينه إلى القيمة `30`.

ابدأ بإنشاء ثابت يسمى `avatar`. ثم عيّن له نتيجة استخدام الطريقة `replace` على `user.avatar_template`.

بالنسبة للطريقة `replace`، استخدم `/{size}/` كالمعلمة الأولى والرقم `30` كالمعلمة الثانية.

# --hints--

يجب أن يكون لديك ثابت يسمى `avatar`.

```js
assert(code.match(/const\s+avatar\s*=/));
```

يجب أن تعيّن `user.avatar_template.replace()` إلى الثابت `avatar` الخاص بك.

```js
assert(code.match(/const\s+avatar\s*=\s*user\.avatar_template\.replace\(\s*.*\s*\)/));
```

يجب أن يكون لديك `/{size}/` كالمعلمة الأولى والرقم `30` كالمعلمة الثانية للطريقة `replace`.
