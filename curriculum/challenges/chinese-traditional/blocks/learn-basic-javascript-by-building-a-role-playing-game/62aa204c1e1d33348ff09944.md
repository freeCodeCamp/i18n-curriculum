...
在你的`attack`函數中，找到更新 `monsterHealth` 變量的代碼行，並將其放置在調用 `isMonsterHit` 函數作爲條件的 `if` 代碼塊中。
...
你的 `if` 語句應該在條件中調用 `isMonsterHit`。

```js
assert.match(attack.toString(), /if\s*\(\s*isMonsterHit\s*\(\s*\)/)
```

你應該將 `monsterHealth -= weapons[currentWeaponIndex].power + Math.floor(Math.random() * xp) + 1` 放入 `if` 塊中。
