...
Das erste Argument für deinen `evalFormula`-Aufruf muss der Inhalt der Zelle sein (den du im `value` gespeichert hast). Allerdings beginnt der Inhalt mit einem `=`-Zeichen, um die Funktion auszulösen. Du musst also den Substring von `value` übergeben, der bei Index `1` beginnt.

# --hints--

Du solltest deinem `evalFormula()`-Aufruf `value` als erstes Argument übergeben.
...
Du solltest die `.slice()`-Methode für das `value`-Argument übergeben.
...
Du solltest deinem `.slice()`-Aufruf die Zahl `1` als Argument übergeben.
