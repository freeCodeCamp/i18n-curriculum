...
你需要取得目前正在播放歌曲的 `id`。為此，你可以使用 `userData?.currentSong?.id`。

使用 `getElementById()` 取得目前播放歌曲的 `id`，然後使用模板字面值為它加上 `song-` 的前置式。將它指定給常數 `songToHighlight`。

# --hints--

你不應該修改現有的 `highlightCurrentSong` 函式（程式）及其內容。
...
你應該使用 `document.getElementById()` 並傳入 ``song-${userData?.currentSong?.id}``。

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

你應該將你的 `getElementById()` 指定給一個 `songToHighlight` 常數。
