...
每当你点击播放按钮时，你的播放列表中的第一首歌曲总是会播放。为了解决这个问题，在事件监听器的箭头函数内，用一个检查 `userData.currentSong` 是否为 `null` 的 `if` 语句替换 `playSong(0)`。

在 `if` 块内，使用 `userData.songs` 数组中第一首歌曲的 `id` 调用 `playSong` 函数。

# --hints--

你应该修改事件监听器的回调，使其在 `userData.currentSong` 为 `null` 时，调用 `playSong` 函数并传入 `userData.songs` 数组中第一首歌曲的 `id`。
