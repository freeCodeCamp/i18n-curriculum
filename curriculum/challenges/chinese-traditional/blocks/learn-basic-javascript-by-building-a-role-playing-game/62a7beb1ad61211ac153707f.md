...
現在，添加一行，將 `button2` 的文本更新爲 `"Buy weapon (30 gold)"`，並將 `button3` 的文本更新爲 `"Go to town square"`。

# --hints--

你應該使用點符號來訪問 `button2` 的 `innerText` 屬性。

```js
assert.match(code, /button2\.innerText/);
```

你不應使用 `let` 或 `const` 來訪問 `button2` 的 `innerText` 屬性。

```js
assert.notMatch(code, /(let|const)\s+button2\.innerText/);
```

你應該將 `button2` 的 `innerText` 屬性更新爲 `"Buy weapon (30 gold)"`。
...
你應該使用點符號來訪問 `button3` 的 `innerText` 屬性。

```js
assert.match(code, /button3\.innerText/);
```

你不應使用 `let` 或 `const` 來訪問 `button3` 的 `innerText` 屬性。

```js
assert.notMatch(code, /(let|const)\s+button3\.innerText/);
```

你應該將 `button3` 的 `innerText` 屬性更新爲 `"Go to town square"`。
