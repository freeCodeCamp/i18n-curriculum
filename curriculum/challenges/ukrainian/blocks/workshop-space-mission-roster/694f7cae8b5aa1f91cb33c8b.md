...
Як і раніше, слід виконати валідацію вхідних даних. Зокрема, якщо `size` менше за `1`, потрібно вивести `"Chunk size must be >= 1"` у консоль, а потім викликати `return`, щоб вийти з функції.

# --hints--

Якщо вхідні дані `size` недійсні, слід вивести в консоль таке повідомлення про помилку, `"Chunk size must be >= 1"`.
...
Після виведення повідомлення про помилку потрібно негайно викликати `return`, щоб раніше завершити функцію.
...
Ваша функція `chunkCrew` має проходити валідацію без помилок, коли `size` більший або дорівнює 1.
