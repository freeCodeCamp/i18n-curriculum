...
`.author` 선택자를 만들고 `font-size` 속성을 `2rem`로 설정하며 `font-family` 속성을 `Raleway`로 설정하되 대체 수단으로 `sans-serif`를 지정하세요.
...
`.author` 선택자에 `font-size` 속성을 `2rem`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

`.author` 선택자에 `font-family` 속성을 `Raleway`로 설정하되 대체 수단으로 `sans-serif`를 지정해야 합니다.
