...
`forumCategory` 함수 안에 `let` 변수 하나를 새로 선언하고 `selectedCategory`라는 이름을 붙인 뒤 빈 객체를 할당하세요. 이 객체는 각 카테고리의 카테고리 이름과 클래스 이름을 저장하는 데 사용됩니다.

# --hints--

`let`를 사용해 `selectedCategory`라는 변수를 선언하세요.

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

빈 객체를 `selectedCategory` 변수에 할당하세요.
