...
在 `if` 语句内，获取 `cardTitle` 元素并将其 `textContent` 设置为 `foundCard.name`。对 `description` 元素执行相同操作，将其 `textContent` 设置为 `foundCard.desc`。

# --hints--

你应该将 `cardTitle` 的 `textContent` 设置为 `foundCard.name`。别忘了使用 `this` 关键字。

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

你应该将 `description` 的 `textContent` 设置为 `foundCard.desc`。别忘了使用 `this` 关键字。
