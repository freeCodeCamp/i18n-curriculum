...
重要的是要记住，`aria-hidden` 只会将内容从辅助技术（如屏幕阅读器）中隐藏。如果内容应该对所有人隐藏，那么你不应该使用 `aria-hidden` 来隐藏它。例如，当前折叠的汉堡菜单必须对所有键盘用户隐藏，而不仅仅是屏幕阅读器用户。在这种情况下，你可以将菜单的 CSS `display` 属性设置为 `none`，以在折叠时将其从渲染页面和无障碍树中移除。
...
**注意**：此交互示例包含 Font Awesome CDN，因此你可以在预览窗口中看到齿轮图标。要查看预览，你需要启用交互式编辑器。
...
一般来说，页面上所有可用的内容和功能也应该对使用辅助技术的人可用。`aria-hidden` 的使用场景非常有限，主要应限制在通过移除纯装饰性或重复信息来让屏幕阅读器用户的体验更清晰。不要使用 `aria-hidden` 来隐藏你认为屏幕阅读器用户不感兴趣的内容。屏幕阅读器用户应有权访问页面上的所有信息。
