...
今度は `happyBtn` 要素にイベントリスナーを追加する必要があります。

ボタンがクリックされたときにアプリがコンソールに `Button clicked!` をログするように、イベントリスナーとそのコールバックを書いてください。

# --hints--

`happyBtn` がクリックされたとき、`Button clicked!` がコンソールにログされるはずです。
