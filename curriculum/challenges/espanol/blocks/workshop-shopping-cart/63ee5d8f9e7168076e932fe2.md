...
Después de tu elemento `h2`, crea dos elementos `p`. Dale al primero una `class` de `dessert-price` y establece el texto con un signo de dólar `"$"` seguido del valor de la variable `price`. Dale al segundo una `class` de `product-category` y el texto `"Category: "` seguido del valor de la variable `category`.
...
Tu segundo elemento `p` debe tener el texto `"Category: "` seguido del valor de la variable `category`.
