...
في استدعاء `forEach`، غلّف التعيين على اليمين `totalCountPerProduct[dessert.id] || 0` بأقواس عادية `()` لضمان التقييم الصحيح، ثم زد القيمة بواحد.
...
يجب غلاف `totalCountPerProduct[dessert.id] || 0` بأقواس عادية `()` قبل زيادتها بواحد.
