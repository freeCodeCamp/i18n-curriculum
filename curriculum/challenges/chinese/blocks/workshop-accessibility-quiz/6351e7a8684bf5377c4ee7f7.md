...
尽管对于带有嵌套 `input` 的 `label` 元素来说不是必需的，但最佳实践仍然是将 `label` 与其对应的 `input` 元素显式链接。

现在，给你的四个 `label` 中的每一个添加一个 `for` 属性，链接到相应的单选 `input` 的 `label`。

# --hints--

你应该给第一个 `label` 一个 `for` 属性。

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

你应该给第一个 `label` 一个与其 `input` 元素的 `id` 相匹配的 `for` 属性。
