...
Crie uma declaração `if` para verificar se o objeto `allCategories` possui uma propriedade `id`. Lembre-se, você pode usar o método `hasOwnProperty()` para isso.

# --hints--

Você deve ter uma declaração `if` que verifica se o objeto `allCategories` possui uma propriedade `id`
