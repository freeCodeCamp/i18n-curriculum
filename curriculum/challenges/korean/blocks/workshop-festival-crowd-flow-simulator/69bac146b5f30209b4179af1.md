...
이제 축제를 시뮬레이션하는 함수를 만들 수 있습니다. `simulateFestival`라는 이름의 빈 함수를 만들고 매개변수로 `gates`과 `timeBlock`를 지정하세요.

# --hints--

`simulateFestival`라는 이름의 함수를 만들어야 합니다.

```js
assert.isFunction(simulateFestival);
```

`simulateFestival` 함수에는 `gates`와 `timeBlock` 매개변수가 있어야 합니다.
