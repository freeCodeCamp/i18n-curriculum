...
將 `parseInt(numberInput.value)` 的所有實例替換爲 `inputInt`。

# --hints--

你應該將 `isNaN(parseInt(numberInput.value))` 替換爲 `isNaN(inputInt)`，並將 `parseInt(numberInput.value) < 0` 替換爲 `inputInt < 0`。
...
你應該將 `parseInt(numberInput.value) === 5` 替換爲 `inputInt === 5`。
...
你應該將 `decimalToBinary(parseInt(numberInput.value))` 替換爲 `decimalToBinary(inputInt)`。
...
你的 `checkUserInput()` 函數應該只包含一個 `parseInt(numberInput.value)` 實例。
