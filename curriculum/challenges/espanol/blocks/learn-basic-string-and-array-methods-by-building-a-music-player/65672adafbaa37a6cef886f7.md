...
Ahora que has creado el botón, necesitas asignarle un texto. Para hacer esto, necesitas usar el método `createTextNode()` de DOM.

El método `createTextNode()` se utiliza para crear un nodo de texto. Para usarlo, lo llamas y pasas el texto como una cadena:
...
Usa el método `createTextNode()` para crear un texto `"Reset Playlist"`, luego asígnalo a una constante `resetText`.
...
Deberías usar `document.createTextNode()` para crear un texto `"Reset Playlist"`.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

Deberías asignar tu nodo de texto creado a una constante `resetText`.
