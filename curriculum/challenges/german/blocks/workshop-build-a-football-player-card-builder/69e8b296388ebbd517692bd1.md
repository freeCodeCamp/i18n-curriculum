...
Fügen Sie ein weiteres `div` mit dem `className` von `form-group` hinzu. Darin fügen Sie ein `label`-Element mit dem `className` von `label`, einem `htmlFor` von `imageUrl` und dem Text `Image URL` ein. Dann fügen Sie ein `input`-Element mit einem `id` von `imageUrl`, einem `className` von `input` und einem `type` von `text` hinzu.
