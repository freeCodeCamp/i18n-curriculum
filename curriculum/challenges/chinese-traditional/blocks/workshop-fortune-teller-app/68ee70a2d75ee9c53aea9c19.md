...
在 `map` 裡，透過傳入 `renderCard`、`type`、`isReversed` 和 `name_short` 作為引數，回傳 `img` 函式，並在 `join` 上使用 `map` 方法，並傳入空字串。
