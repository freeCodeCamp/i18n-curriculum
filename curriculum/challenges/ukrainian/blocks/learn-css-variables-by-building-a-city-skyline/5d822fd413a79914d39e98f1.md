...
Ви хочете додати той самий градієнт до наступних двох розділів. Замість цього створіть новий клас-селектор з назвою `bb1-window` і перенесіть властивості та значення `height` і `background` з `.bb1a` до нового клас-селектора.
...
Ви повинні перенести властивість і значення `height` з `.bb1a` до `.bb1-window`.
...
Ви повинні перенести властивість і значення `background` з `.bb1a` до `.bb1-window`.
...
Ви не повинні переносити властивість `background-color` з `.bb1a`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

Ви не повинні переносити властивість `width` з `.bb1a`.
