...
如果用户掷出三个相同的点数和另外两个相同的点数，这叫作葫芦（或满堂红，英文为 full house）。 声明一个接受单个参数的 `detectFullHouse` 函数。 函数调用时将传递 `diceValuesArr` 数组。

你的 `detectFullHouse` 函数应检查用户是否掷出了三个相同的点数和两个相同的点数。 如果是这样，它应更新第三个单选按钮以正确的属性显示得分 `25`。 无论如何，它应该始终更新最后一个单选按钮以显示分数 0，并具​​有正确的属性。

不要忘记在掷骰子时调用你的新函数。

# --hints--

你应该有一个 `detectFullHouse` 函数。

```js
assert.isFunction(detectFullHouse);
```

你的 `detectFullHouse` 函数应该有一个参数。

```js
assert.match(code, /(?:var|let|const)\s+detectFullHouse\s*=\s*(\(\s*\w+\s*\)|\s*\w+\s*)\s*=>\s*|function\s+detectFullHouse\s*\(\s*\w+\s*\)/);
```

当掷出一个葫芦时，你的 `detectFullHouse` 函数应启用第三个单选按钮，将其值设为 `25`，并设置第三个 `span` 显示文本 `, score = 25`。
...
当没有掷出葫芦时，你的 `detectFullHouse` 函数应启用最后一个单选按钮，将其值设为 `0`，并设置最后一个 `span` 显示文本 `, score = 0`。
...
当点击 `rollDiceBtn` 时，你应该调用你的 `detectFullHouse` 函数。
