...
Dé al selector `.image-wrapper` una propiedad `grid-template-columns` con un valor establecido en `2fr 1fr` y una propiedad `grid-template-rows` con un valor establecido en `repeat(3, min-content)`. Esto dará a nuestras filas de la cuadrícula un ajuste en altura según el contenido, pero las columnas permanecerán con un ancho fijo basado en el contenedor.

# --hints--

Tu selector `.image-wrapper` debe tener una propiedad `grid-template-columns` establecida en `2fr 1fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

Tu selector `.image-wrapper` debe tener una propiedad `grid-template-rows` establecida en `repeat(3, min-content)`.
