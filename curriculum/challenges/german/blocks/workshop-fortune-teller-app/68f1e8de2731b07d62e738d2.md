...
Deklarieren Sie eine weitere Variable namens `foundCard` und weisen Sie ihr das Ergebnis zu, das durch den Aufruf der `find()`-Methode auf `cards` entsteht. Übergeben Sie `card` als Callback-Parameter und vergleichen Sie `card.name_short` mit `cardId`, um die passende Karte mit strikter Gleichheit zu finden.

# --hints--

Sie sollten eine Variable namens `foundCard` innerhalb von `showFortune` deklarieren.
...
Ihre `foundCard` sollte zugewiesen werden, indem Sie `find` auf `this.cards` mit dem korrekten Callback aufrufen.
...
Der `find`-Callback sollte einen Parameter namens `card` haben.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

Innerhalb des Callbacks sollte `card.name_short` mit `cardId` verglichen werden.
