...
給 `.fb4-window` 元素設定 `width` 為 `30%`、`height` 為 `10%`，以及 `border-radius` 為 `50%`。這些將為這個建築物製作一些圓形視窗。

# --hints--

你應該給 `.fb4-window` 一個 `width` 為 `30%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

你應該給 `.fb4-window` 一個 `height` 為 `10%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

你應該為 `.fb4-window` 設定 `border-radius` 為 `50%`。
