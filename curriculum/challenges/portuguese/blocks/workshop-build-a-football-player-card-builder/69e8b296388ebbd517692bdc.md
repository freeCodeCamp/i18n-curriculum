...
Adicione um atributo `value` com valor `player.sho` ao SHO `input`. Depois defina seu atributo `onChange` como uma arrow function que recebe `e` como parâmetro e faz uma chamada a `setPlayer` com um spread de `player` e `sho` definido como `Number(e.target.value)`.

# --hints--

Seu SHO `input` deve ter um `value` igual ao atributo `sho` do jogador atual.
...
Seu SHO `input` deve ter um manipulador `onChange` que chama `setPlayer` com um spread de `player` e `sho` definido como `Number(e.target.value)`.
