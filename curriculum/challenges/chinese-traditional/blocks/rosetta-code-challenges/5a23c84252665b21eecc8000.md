...
給定一個值的列表和一組指向該值列表的整數索引，任務是對給定索引處的值進行排序，但保留那些不在排序索引集合中的值。

讓你的函式（程式）能處理以下的值列表和索引集合：
...
`sortDisjoint` 應該是一個函式（程式）。

```js
assert(typeof sortDisjoint == 'function');
```

`sortDisjoint([7, 6, 5, 4, 3, 2, 1, 0], [6, 1, 7])` 應該傳回一個陣列。

```js
assert(Array.isArray(sortDisjoint([7, 6, 5, 4, 3, 2, 1, 0], [6, 1, 7])));
```

`sortDisjoint([7, 6, 5, 4, 3, 2, 1, 0], [6, 1, 7])` 應該傳回 `[7, 0, 5, 4, 3, 2, 1, 6]`。
...
`sortDisjoint([7, 6, 5, 4, 3, 2, 1, 0], [1, 2, 5, 6])` 應該傳回 `[7, 1, 2, 4, 3, 5, 6, 0]`。
...
`sortDisjoint([8, 7, 6, 5, 4, 3, 2, 1], [6, 1, 7])` 應該傳回 `[8, 1, 6, 5, 4, 3, 2, 7]`。
...
`sortDisjoint([8, 7, 6, 5, 4, 3, 2, 1], [1, 3, 5, 6])` 應該傳回 `[8, 2, 6, 3, 4, 5, 7, 1]`。
...
`sortDisjoint([6, 1, 7, 1, 3, 5, 6], [6, 1, 5, 4])` 應該傳回 `[6, 1, 7, 1, 3, 5, 6]`。
