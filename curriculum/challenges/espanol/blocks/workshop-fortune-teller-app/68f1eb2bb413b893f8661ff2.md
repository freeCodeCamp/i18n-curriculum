...
En este punto, deberías poder ver la fortuna que muestra cada carta al hacer clic en una carta. Lo único que queda por hacer es hacer que funcione el botón de nueva lectura.

Dentro del método `newReading`, llama a `showElements()` y `hideElements()`.

# --hints--

Debes llamar a la función `showElements` dentro del método `newReading`.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

Debes llamar a la función `hideElements` dentro del método `newReading`.
