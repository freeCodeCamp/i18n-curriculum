...
Crea un altro elemento `p` dopo il tuo ultimo `.divider` e assegnagli il testo `Total Sugars 12g`. Assegna a quell'elemento `p` i valori `class` necessari per indentarlo e rimuovere il bordo inferiore. Poi crea un altro `.divider` sotto il tuo nuovo elemento `p`.
