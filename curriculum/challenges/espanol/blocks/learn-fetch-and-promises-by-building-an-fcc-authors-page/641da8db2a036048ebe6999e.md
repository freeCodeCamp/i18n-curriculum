...
Dentro de la función `fetchMoreAuthors`, establece las variables `startingIndex` y `endingIndex` a `+= 8` cada una.
...
Debes establecer la variable `startingIndex` a `+=8`.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Debes establecer la variable `endingIndex` a `+=8`.
