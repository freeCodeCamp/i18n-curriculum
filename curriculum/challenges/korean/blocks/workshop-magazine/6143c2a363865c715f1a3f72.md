...
`.image-wrapper` 요소 안에서, 첫 번째 `img` 요소에 `src` 값을 가진 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png` 속성, `alt` 값을 가진 `image of the quote machine project` 속성, `class` 값을 가진 `image-1` 속성, `loading` 값으로 설정된 `lazy` 속성, `width` 값을 가진 `600` 속성, 그리고 `height` 값을 가진 `400` 속성을 주세요.

# --hints--

첫 번째 `img` 요소에 `src` 값으로 설정된 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png` 속성이 있어야 합니다.
