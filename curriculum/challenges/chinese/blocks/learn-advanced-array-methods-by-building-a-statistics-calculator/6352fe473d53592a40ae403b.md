...
在你的空 `.reduce()` 回调中，声明一个变量 `difference`，将它设置为 `el` 减去 `mean` 的值。 然后声明一个 `squared` 变量，并将其值设置为 `difference` 的 `2` 次方。 最后，返回 `acc` 加上 `squared` 的值。

# --hints--

你的 `reduce` 函数应该有一个 `difference` 变量。

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

你的 `difference` 变量应该被设置为 `el` 减去 `mean` 的值。
...
你的 `squared` 变量应该被设置为 `difference` 的二次方的值。
...
你的 `reduce` 回调应该返回 `acc` 加上 `squared` 的和。
