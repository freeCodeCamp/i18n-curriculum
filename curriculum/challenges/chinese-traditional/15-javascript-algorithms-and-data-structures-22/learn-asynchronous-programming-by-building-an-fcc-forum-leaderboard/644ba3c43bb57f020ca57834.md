...
現在，你需要在 `selectedCategory` 對象中添加 `className` 和 `category` 屬性。

首先將 `className` 變量賦值給 `selectedCategory.className`。 然後將 `category` 變量賦值給 `selectedCategory.category`。

# --hints--

你應使用點符號來選中 `selectedCategory` 對象的 `className` 屬性。

```js
assert.match(code, /selectedCategory\.className/);
```

你應該將 `allCategories` 對象的 `className` 屬性賦值給 `selectedCategory` 對象的 `className` 屬性。

```js
assert.match(code, /selectedCategory\.className\s*=\s*className/);
```

你應該使用點符號來選中 `selectedCategory` 對象的 `category` 屬性。

```js
assert.match(code, /selectedCategory\.category/);
```

你應該將 `allCategories` 對象的 `category` 屬性賦值給 `selectedCategory` 對象的 `category` 屬性。
