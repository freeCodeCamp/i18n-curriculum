...
Sasa unahitaji kutangaza sifa ambazo hitilafu zote zitakuwa nazo.

Ndani ya darasa la `Bug`, tangaza sifa iitwayo `emoji` yenye aina ya `T` ukitumia kiendeshaji cha uthibitisho wa mgawo wa thamani (`!`). Hii inaambia TypeScript kwamba sifa hiyo itapewa thamani baadaye.

# --hints--

Darasa lako la `Bug` linapaswa kuwa na sifa iitwayo `emoji`.
...
Sifa yako ya `emoji` inapaswa kuwa na aina ya `T`.
...
Sifa yako ya `emoji` inapaswa kutumia kiendeshaji cha uthibitisho wa mgawo wa thamani (`!`).
