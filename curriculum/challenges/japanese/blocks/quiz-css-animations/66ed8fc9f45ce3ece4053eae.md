...
この `@keyframes` アットルールはアニメーション要素に何をしますか？
...
以下の CSS の `@keyframes` アットルールにおける `50%` は何を表しますか？
...
CSS アニメーションが多すぎると、検索エンジンのランキングが低下したり、まったく表示されなくなる場合があります。
...
`prefers-reduced-motion` メディアクエリ内で、どの宣言がトランジションを無効にしますか？
