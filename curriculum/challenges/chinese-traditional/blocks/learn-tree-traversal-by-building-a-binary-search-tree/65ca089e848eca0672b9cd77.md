...
現在，回到 `_delete` 方法，你必須使用 `_min_value` 輔助函數來選擇後繼者。

在 `elif` 塊之後添加 `_min_value` 調用，並將 `node.right` 作爲參數傳遞。 將其分配給當前節點鍵。

# --hints--

你應該在 `elif` 塊之後將 `self._min_value(node.right)` 分配給 `node.key`。
