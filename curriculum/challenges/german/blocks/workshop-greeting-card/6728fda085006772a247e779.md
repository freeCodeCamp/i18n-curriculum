...
Geben Sie `.card` eine `max-width` von `400px`, eine `padding` von `40px` an allen Seiten und eine `margin` von `0` für oben und unten sowie `auto` für links und rechts (verwenden Sie die Kurzschreibweise-Eigenschaft).

# --hints--

`.card` sollte eine `max-width` von `400px` haben.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("max-width"), "400px");
```

`.card` sollte eine `padding` von `40px` haben.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("padding"), "40px");
```

`.card` sollte eine `margin` von `0 auto` haben.
