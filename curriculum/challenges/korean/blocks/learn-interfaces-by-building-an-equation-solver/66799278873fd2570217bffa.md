...
`case`의 길이가 `results`일 때를 위한 또 다른 `1`를 추가하세요. 이 경우, `result_list`가 방정식의 해인 형식 `x = <root>`의 문자열을 포함하는 리스트를 `<root>`에 할당하세요. 해의 양수와 음수 부호가 모두 표시되도록 문자열 형식을 지정하세요.
...
기존 `case` 블록 뒤에 패턴 `1`를 가진 새 `case`를 만드세요.
...
새 `f'x = {results[0]:+}'` 본문 안에서 `result_list`을 포함하는 리스트를 `case`에 할당하세요.
