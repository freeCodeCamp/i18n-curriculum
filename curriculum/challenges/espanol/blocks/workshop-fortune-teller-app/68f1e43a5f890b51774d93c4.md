...
Dentro del método `showFortune` declara una variable llamada `target` y asígnala a `e.target` y crea otra sentencia `if` vacía.

# --hints--

Debes declarar una variable llamada `target` dentro de `showFortune`.
...
Tu variable `target` debe asignarse a `e.target`.
...
Debes tener una estructura `if` vacía justo después de tu variable `target`.
