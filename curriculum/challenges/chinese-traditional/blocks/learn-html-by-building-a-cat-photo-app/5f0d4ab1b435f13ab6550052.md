...
You should only add one opening `fieldset` tag. 請刪除任何多餘的。

```js
assert(document.querySelectorAll('fieldset').length === 2);
```

你的新 `fieldset` 元素應該有一個結束標籤。 結束標籤在 `<` 字符之後有一個 `/`。

```js
assert(code.match(/<\/fieldset>/g).length >= 2);
```

You should only add one closing `fieldset` tag. 請刪除任何多餘的。
...
Your new `fieldset` element should be below the existing `fieldset` element. 你把它們的順序寫錯了。
