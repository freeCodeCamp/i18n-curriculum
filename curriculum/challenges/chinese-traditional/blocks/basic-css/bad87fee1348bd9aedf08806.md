...
在包含你的 `red-text` 類別的同一個 `<style>` 標籤內，為 `p` 元素創建一個條目，並將 `font-size` 設定為 16 像素（`16px`）。

# --hints--

在 `style` 標記之間，給 `p` 元素設定 `font-size` 為 `16px`。瀏覽器和文字縮放應該設為 100%。
