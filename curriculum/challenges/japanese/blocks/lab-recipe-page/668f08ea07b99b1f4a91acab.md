...
1. `lang` が `en` に設定された `html` 要素が必要です。
1. レシピ名を含む `title` 要素と、`charset` 属性が `UTF-8` に設定された `meta` 要素を含む `head` 要素が必要です。
1. `body` 要素が必要です。
1. レシピ名を含む `h1` 要素が必要です。
1. `h1` の下にレシピを紹介する `p` 要素が必要です。
1. 材料セクションのテキスト `Ingredients` を含む `h2` 要素が1つ必要です。
1. 最初の `h2` 要素の下に、材料をリストアップした少なくとも4つのリスト項目（`li` 要素）を含む番号なしリスト（`ul` 要素）が必要です。
1. 手順セクションのテキスト `Instructions` を含む2番目の `h2` 要素が必要です。
1. 2番目の `h2` の下に、レシピのステップを順番にリストアップした少なくとも4つのリスト項目を含む番号付きリスト（`ol` 要素）が必要です。
1. 有効な画像が設定された `src` 属性を持つ `img` 要素が1つ必要です。必要に応じて `https://cdn.freecodecamp.org/curriculum/labs/recipe.jpg` を使用でき、画像を説明する `alt` 属性も必要です。
