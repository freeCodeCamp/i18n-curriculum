...
選擇 `class` 爲 `top` 的 `.beak` 元素，將其 `width` 設置爲 `20%`，並放置在父元素頂部 `60%` 左側 `40%` 的位置。
...
你應該將 `.beak.top` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.width, '20%');
```

你應該將 `.beak.top` 的 `top` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.top, '60%');
```

你應該將 `.beak.top` 的 `left` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
