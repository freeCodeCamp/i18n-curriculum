...
title: Definire head e body di un documento HTML
challengeType: 0
...
Puoi aggiungere un altro livello di organizzazione nel tuo documento HTML all’interno dei tag `html` con gli elementi `head` e `body`. Qualsiasi markup con informazioni sulla tua pagina andrebbe inserito nel tag `head`. Poi qualsiasi markup con il contenuto della pagina (quello che viene mostrato all’utente) andrebbe inserito nel tag `body`.

Gli elementi di metadati, come `link`, `meta`, `title` e `style`, di solito vanno all’interno dell’elemento `head`.

Ecco un esempio della struttura di una pagina:
...
Modifica il markup in modo che ci siano un `head` e un `body`. L’elemento `head` dovrebbe includere solo il `title`, e l’elemento `body` dovrebbe includere solo `h1` e `p`.

# --hints--

Dovrebbe esserci un solo elemento `head` nella pagina.
...
Dovrebbe esserci un solo elemento `body` nella pagina.
...
L’elemento `body` dovrebbe racchiudere sia gli elementi `h1` che `p`.
