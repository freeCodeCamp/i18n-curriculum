...
Оголосіть ще одну змінну з іменем `foundCard` і призначте їй результат виклику методу `find()` на `cards`. Передайте `card` як параметр функції зворотного виклику, а потім порівняйте `card.name_short` з `cardId` за допомогою суворої рівності, щоб знайти відповідну карту.

# --hints--

Ви повинні оголосити змінну з іменем `foundCard` всередині `showFortune`.
...
Вашій `foundCard` має бути призначено результат виклику `find` на `this.cards` з правильним параметром функції зворотного виклику.
...
Функція зворотного виклику `find` має мати параметр з іменем `card`.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

Всередині функції зворотного виклику `card.name_short` має бути порівняно з `cardId`.
