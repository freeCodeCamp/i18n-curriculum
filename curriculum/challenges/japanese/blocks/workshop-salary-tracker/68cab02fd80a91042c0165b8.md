...
既存の`if`文の後に、新しい給与が現在のレベルの基本給より低い場合のための別の`if`文を作成してください。初期化中に`AttributeError`が発生しないように、`hasattr(self, '_level')`を使ってください。

`if`文の中で、`Salary must be higher than minimum salary $`というメッセージに続けて現在のレベルの基本給とピリオドを付けて、`ValueError`を発生させてください。

# --hints--

`salary`セッターの中に2つ目の`if`文があるはずです。
...
新しい給与が現在のレベルの基本給より低い場合、`Salary must be higher than minimum salary $`というメッセージに続けて現在のレベルの基本給とピリオドを付けて、`ValueError`を発生させる必要があります。
...
`new_salary`が現在の給与より高い場合は、例外を発生させてはいけません。
