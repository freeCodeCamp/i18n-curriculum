...
A questo punto dovresti riuscire a vedere la fortuna che ogni carta mostra una volta che clicchi su una carta. L’unica cosa che resta da fare è far funzionare il pulsante per una nuova lettura.

All’interno del metodo `newReading`, chiama `showElements()` e `hideElements()`.

# --hints--

Dovresti chiamare la funzione `showElements` all’interno del metodo `newReading`.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

Dovresti chiamare la funzione `hideElements` all’interno del metodo `newReading`.
