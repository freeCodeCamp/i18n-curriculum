...
単語 `"Arg!"` は引用符で囲む必要があります。引用符をエスケープする以外に、文字列内に引用符を含める別の方法があります。

文字列 `"The monster screams Arg! as it dies. You gain experience points and find gold."` の周りの二重引用符を単一引用符 `'` に変更し、次に `"Arg!"` の周りに二重引用符を追加してください。

# --hints--

文字列の周りには単一引用符を使うべきです。

```js
assert.match(code, /'The monster screams "?Arg!"? as it dies. You gain experience points and find gold.'/);
```

`"Arg!"` の周りには二重引用符を使うべきです。
