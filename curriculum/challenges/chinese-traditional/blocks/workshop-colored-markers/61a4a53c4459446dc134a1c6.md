...
既然你已經了解了 `linear-gradient` 函式和色彩停點的基本運作方式，你就可以使用它們讓標記看起來更逼真。

在 `linear-gradient` 函式中，將 `gradientDirection` 設定為 `180deg`。

# --hints--

你的 `.red` CSS 規則應該有一個 `background` 屬性，設定為 `linear-gradient(180deg, rgb(255, 0, 0) 75%, rgb(0, 255, 0), rgb(0, 0, 255))`。
