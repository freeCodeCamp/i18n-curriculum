...
在第二個 `tr` 中，添加一個 `th` 元素，文本爲 `Expenses Annual anticipated expenses, such as payroll.`。 將除 `Expenses` 之外的文本包裝在 `span` 元素內，並將 `class` 設置爲 `description`。

在其下方添加三個 `td` 元素，並按順序爲它們提供以下文本：`$200`、`$300` 和 `$400`。 給第三個 `td` 添加一個 `class`，設置爲 `current`。
...
你的 `th` 元素應包含文本 `Expenses Annual anticipated expenses, such as payroll.`。
...
你應該將文本 `Annual anticipated expenses, such as payroll.` 包裹在 `span` 元素中。
