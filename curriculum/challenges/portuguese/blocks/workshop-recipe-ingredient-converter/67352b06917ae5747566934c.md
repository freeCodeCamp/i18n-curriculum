...
Finalmente, atualize sua função interna para retornar uma terceira função.Esta terceira função também deve aceitar um único argumento chamado `quantity` e ter um corpo vazio.

# --hints--

Sua segunda função deve retornar mais uma função.

```js
assert.isFunction(convertQuantity()());
```

Sua função mais interna deve ter um parâmetro `quantity`.

```js
assert.match(convertQuantity()()?.toString(), /\(quantity\)/);
```

Sua função mais interna deve estar vazia.
