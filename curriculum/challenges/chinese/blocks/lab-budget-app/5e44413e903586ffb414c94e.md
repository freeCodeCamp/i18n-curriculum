...
1. 你应该有一个位于 `Category` 类外部的函数，名为 `create_spend_chart(categories)`，它接受一个类别列表并返回一个条形图字符串。构建图表时：

   - 以标题 `Percentage spent by category` 开始。
   - 仅从提款计算百分比，不包括存款。百分比应为每个类别支出金额占所有类别总支出的百分比（向下取整到最接近的 10）。
   - y 轴从 `100` 标记到 `0`，步长为 10。
   - 使用 `o` 字符表示条形。
   - 在最后一个栏杆后两格处包含一条水平线。
   - 在条形图下方垂直书写类别名称。
