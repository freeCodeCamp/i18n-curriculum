...
下一個 `input` 是一個核取方塊，用來標記所有適用於使用者的超級英雄能力。

創建一個 `label` 元素，其 `className` 為 `section` 和 `column`，並且文字為 `List your powers (select all that apply):`。

# --hints--

你應該創建一個 `label` 元素，其 `className` 為 `section` 和 `column`。
...
你的 `label` 元素應該有文字 `List your powers (select all that apply):`。
