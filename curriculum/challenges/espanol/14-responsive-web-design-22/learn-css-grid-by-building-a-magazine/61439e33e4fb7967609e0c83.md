...
Después de los tres elementos `p` dentro de su elemento `.text`, cree un elemento `blockquote`. Within that, add an `hr` element, a `p` element with the `class` set to `quote`, and a second `hr` element.
