...
Crea una clausola `elif` per il caso `n == 1`. All’interno della clausola `elif`, crea una f-string contenente il coefficiente seguito direttamente da un `x` minuscolo e aggiungila alla lista `terms`.

# --hints--

Dovresti creare una clausola `elif` per verificare se `n` è uguale a `1`.
...
Dovresti aggiungere `f'{coefficient}x'` alla lista `terms` all’interno della tua nuova clausola `elif`.
