...
CSS의 `position` 속성의 다음 옵션은 `absolute`로, 이는 요소를 부모 컨테이너를 기준으로 위치를 정합니다. `relative` 위치와는 다르게 요소를 문서의 일반적인 흐름에서 제거하여 주변 항목을 무시하게 됩니다. CSS 오프셋 속성 (top 또는 bottom 그리고 left 또는 right) 을 사용하여 위치를 조절합니다.
