...
Ili kurejesha orodha ya nyimbo katika hali yake ya awali, sambaza `allSongs` ndani ya safu ya data na weka thamani yake kwa `userData.songs`.

*Kumbuka*: Hupaswi kutumia uunganishaji wa mfululizo kwa `userData.songs` kwa sababu wimbo hautakuwa `null` au `undefined` wakati huu.

# --hints--

Hupaswi kubadilisha msikilizaji wa tukio uliopo na yaliyomo yake.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*\)\s*;?/)
```

Unapaswa weka thamani ya `[...allSongs]` kwa `userData.songs`.
