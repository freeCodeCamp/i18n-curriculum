...
`setPlayerCards` 함수를 `players.filter()` 인수와 함께 호출하세요.

`filter` 메서드 안에 `player`라는 매개변수를 가진 콜백 함수를 추가하고 `player.nickname`가 `null`가 아님을 암묵적으로 반환하세요.

# --hints--

`nickname` 경우에는 `setPlayerCards` 함수를 호출해야 합니다.
...
`filter()` 콜백은 `player.nickname`가 `null`가 아님을 확인한 결과를 암묵적으로 반환해야 합니다.
