...
在前一個核取方塊後添加一個最後的核取方塊，其 `id` 屬性值為 `energetic`。`name` 屬性應與前一個核取方塊相同。

同時在新的核取方塊右側添加一個 `label` 元素，文字為 `Energetic`。確保將該 `label` 元素與新的核取方塊關聯。
...
你的新核取方塊應該有一個 `id` 屬性，值為 `energetic`，以及一個 `name` 屬性，值為 `personality`。請檢查開啟標記名稱後是否有空格，和／或所有屬性名稱前是否有空格。
...
在你新的核取方塊右側，應該有一個 `label` 元素，裡面有文字 `Energetic`。
