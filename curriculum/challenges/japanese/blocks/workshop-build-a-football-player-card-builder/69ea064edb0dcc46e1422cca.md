...
これから `localStorage` に保存するロジックを `useEffect` を使って追加します。

Reactでは `localStorage` のデータにアクセスしたり保存したりすることは副作用とみなされるため、このロジックには `useEffect` を使う必要があります。

まずは `useEffect` と同様に、Reactから `useState` を分割代入してください。

# --hints--

`useEffect` から `React` を分割代入してください。
