---
id: 6904850796159a43815677a5
title: Task 20
challengeType: 19
dashedName: task-20
lang: zh-CN
---

<!-- (Audio) Liu Ming: 認識 (rèn shi) -->

# --description--

Liu Ming is saying "to know someone" in Chinese.

# --instructions--

Listen to the audio and answer the question below.

# --questions--

## --text--

What word does Liu Ming use to say "to know someone"?

## --answers--

`學習 (xué xí)`

### --feedback--

This means "to study".

---

`認識 (rèn shi)`

---

`開發 (kāi fā)`

### --feedback--

This means "to develop".

---

`分析 (fēn xī)`

### --feedback--

This means "to analyze", but Liu Ming is expressing knowing someone, not analyzing them.

## --video-solution--

2

# --explanation--

`認識 (rèn shi)` means "to meet someone for the first time" or "to get to know someone". It's used when people meet and get acquainted. For example:

`很高興認識你 (hěn gāo xìng rèn shi nǐ)！` – Nice to meet you!

# --scene--

```json
{
  "setup": {
    "background": "company1-breakroom.png",
    "characters": [
      {
        "character": "Liu Ming",
        "position": {
          "x": 50,
          "y": 17,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "ZH_A1_greetings_and_introductions_warm_up.mp3",
      "startTime": 1,
      "startTimestamp": 43.3,
      "finishTimestamp": 44.15
    }
  },
  "commands": [
    {
      "character": "Liu Ming",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Liu Ming",
      "startTime": 1,
      "finishTime": 1.85,
      "dialogue": {
        "text": "認識 (rèn shi)",
        "align": "center"
      }
    },
    {
      "character": "Liu Ming",
      "opacity": 0,
      "startTime": 2.35
    }
  ]
}
```
