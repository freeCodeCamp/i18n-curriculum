...
對於你的 `timeAgo` 函式（程式），你會想要計算目前時間與主題上次活動時間之間的差異。這將允許你顯示自主題有任何活動以來經過了多少時間。

完成符合以下需求的 `timeAgo` 函式（程式）。

- 如果經過的分鐘數少於 `60`，傳回字串 `"xm ago"`。`x` 將表述分鐘數。
- 如果經過的時數少於 `24`，傳回字串 `"xh ago"`。`x` 將表述小時數。
- 否則，傳回字串 `"xd ago"`。`x` 將表述天數。
...
當已過的分鐘數是 `30` 分鐘時，你應該傳回字串 `"30m ago"`。
