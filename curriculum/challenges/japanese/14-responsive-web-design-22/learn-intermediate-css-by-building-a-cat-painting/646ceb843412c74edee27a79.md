...
クラスセレクターを使用して、`.cat-right-ear` の要素の `height` プロパティと `width` プロパティを `100px` に設定してください。 Set the `background-color` to `white`. 左右の境界線は `35px solid blue` に、 上下の境界線は `35px solid red` に設定してください。
