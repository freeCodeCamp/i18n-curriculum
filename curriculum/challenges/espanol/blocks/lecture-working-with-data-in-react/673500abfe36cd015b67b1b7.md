...
Los enfoques más comunes para usar renderizado condicional incluyen usar sentencias `if`, el operador ternario (`?:`) y el operador lógico AND (`&&`).
...
Otro patrón común para el renderizado condicional es usar el operador lógico AND (`&&`). Esto es particularmente útil cuando quieres renderizar algo, o nada, basado en una condición:
...
Piensa en los métodos discutidos en la lección para manejar el renderizado condicional en React.
...
¿Qué será renderizado por el siguiente código si `message` es una cadena vacía?
...
Considera cómo funciona el operador lógico AND (`&&`) con valores falsy en JSX.
