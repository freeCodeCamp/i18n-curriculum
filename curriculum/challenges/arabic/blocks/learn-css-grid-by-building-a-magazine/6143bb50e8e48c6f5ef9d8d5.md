...
داخل عنصر `aside`، أنشئ عنصرين `img`، وعنصر `blockquote`، وعنصر `img` ثالث. عيّن للعنصر `blockquote` صنفًا (`class`) باسم `image-quote`.

# --hints--

يجب أن تنشئ ثلاثة عناصر `img` داخل عنصر `aside` الخاص بك.

```js
assert(document.querySelectorAll('aside img')?.length === 3);
```

يجب أن تنشئ عنصر `blockquote` داخل عنصر `aside` الخاص بك.

```js
assert.exists(document.querySelector('aside blockquote'));
```

يجب أن يحتوي عنصر `blockquote` على `class` مضبوط إلى `image-quote`.

```js
assert(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

يجب أن تكون العناصر الجديدة بالترتيب الصحيح.
