...
Crea una regola `.lists li` per selezionare gli elementi della lista all'interno del tuo elemento `.lists`. Imposta la sua proprietà `margin-bottom` su `1.5rem`.

# --hints--

Dovresti avere un selettore `.lists li`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

Il tuo selettore `.lists li` dovrebbe avere una proprietà `margin-bottom` impostata su `1.5rem`.
