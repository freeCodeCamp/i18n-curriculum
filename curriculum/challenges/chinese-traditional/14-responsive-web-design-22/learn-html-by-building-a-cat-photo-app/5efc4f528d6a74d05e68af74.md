...
你應該在複選框元素後添加一個新的 `label` 元素。 Make sure it has both an opening and closing tag.
...
你的 `label` 元素應該有一個值爲 `loving` 的 `for` 屬性。  Expected `--fcc-expected--`, but found `--fcc-actual--`.
