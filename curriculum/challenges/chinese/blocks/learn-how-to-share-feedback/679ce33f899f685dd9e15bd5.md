...
`Anything` is commonly used in questions to ask if there is any information, object, or detail related to the topic. It does not specify a particular thing. 例如：

`Did she say anything about the deadline?` - This asks if she provided any information about the deadline.

`Anything` is also used in negative sentences. 例如：

`I didn't hear anything about the meeting.` - The speaker has no information about the meeting.

However, in affirmative sentences, `something` is usually used instead of `anything`. 例如：
