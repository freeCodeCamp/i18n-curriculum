...
Erstellen Sie auf die gleiche Weise wie zuvor für die `__eq__`-Methode eine `if`-Anweisung, die überprüft, ob `self` und `other` nicht zur gleichen Klasse gehören, und geben Sie `NotImplemented` zurück.
...
Sie sollten `NotImplemented` aus dem Körper Ihrer `if` zurückgeben.
