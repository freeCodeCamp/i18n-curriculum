...
你需要一种方法来响应用户更改下拉菜单中的形状选择。为此，创建一个带有 `handleShapeSelect` 参数且参数类型为 `e` 的 `Event` 函数。

该函数将通过清除所有输入、阻止默认行为、验证选择、根据是否选择了形状显示或隐藏结果卡、显示所选形状的相应输入字段以及重新计算面积来处理下拉菜单的更改。

目前先将函数留空。

# --hints--

你应该创建一个 `handleShapeSelect` 函数。

```js
assert.isFunction(handleShapeSelect)
```

你的 `handleShapeSelect` 函数应有一个类型为 `e` 的 `Event` 参数。
