...
Finalmente, necesitas manejar el método `catch`. Primero, anida la sentencia `fetch`, la sentencia `json` y la llamada a displayAuthors dentro de un bloque `try` dentro de `initialFetch`. Luego, modifica el método encadenado `catch` para que sea un bloque catch según el ejemplo que se muestra a continuación.
...
Si lo has hecho correctamente, el bloque `try` contendrá tres líneas de código y el bloque `catch` contendrá solo una línea de código. Además, el `SyntaxError` se resolverá y la app debería funcionar normalmente.

¡Date una palmada en la espalda! Has aprendido a hacer que tu código asíncrono sea más legible usando los operadores `async` y `await`. Ahora tu página de autores está completa.
...
Debes crear un bloque `try/catch`

```js
assert.match(code, /try\s*\{[\s\S]*?\}\s*catch\s*\([^)]*\)\s*\{/);
```

El `catch` ya no debe ser un método encadenado

```js
assert.notMatch(code, /\.catch/);
```

El bloque `try` debe contener las tres sentencias que obtienen los datos, crean el `json` y luego lo muestran.
...
La página de autores de fCC debería renderizarse correctamente
