...
Pata upatikanaji wa mali za `userData?.currentSong?.title` na `userData?.currentSong?.artist` na weka thamani zao katika vigezo vya `currentTitle` na `currentArtist` mtawalia.

# --hints--

Unapaswa kupata upatikanaji wa `title` wa `currentSong` kutoka kwa kitu cha `userData`. Usisahau kutumia uunganishaji wa mfululizo.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

Unapaswa kuweka thamani ya `title` ya `currentSong` katika thabiti ya `currentTitle`.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

Unapaswa kupata upatikanaji wa `artist` wa `currentSong` kutoka kwa kitu cha `userData`. Usisahau kutumia uunganishaji wa mfululizo.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

Unapaswa kuweka thamani ya `artist` ya `currentSong` katika thabiti ya `currentArtist`.
