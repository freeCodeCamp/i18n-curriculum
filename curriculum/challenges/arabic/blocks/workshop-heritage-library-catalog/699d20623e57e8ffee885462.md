...
حان الوقت لرؤية المُحقق يكتشف مشكلة حقيقية. `catalog[0]` هو "From a Buick 8" لستيفن كينغ، الذي يحتوي على جميع الحقول الأربعة مملوءة. `catalog[16]` هو "Unknown Title"، الذي يحتوي على حقل المؤلف فارغ.

سجل `validateEntry(catalog[0])` و `validateEntry(catalog[16])`. يجب أن تُرجع الأولى `true`؛ والثانية يجب أن تُرجع `false`.

# --hints--

يجب أن تسجل `validateEntry(catalog[0])` في وحدة التحكم.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

يجب أن تسجل `validateEntry(catalog[16])` في وحدة التحكم.
