...
内側の円を配置し、最初は見えないようにする必要があります。

引き続き `.contact-method-radio-btn::before` をターゲットにして、`transform` を `translate(3px, 3px) scale(0)` に設定してください。`translate` 関数は円を少し動かし、`scale(0)` はサイズをゼロに縮小してデフォルトで隠します。

また、`transition` に `all 0.3s ease-in` を追加して、表示時にスムーズなアニメーションを作成してください。これにより、スタイルの変更が0.3秒かけてゆっくり始まり、だんだん速くなるアニメーションになります。

# --hints--

引き続き `.contact-method-radio-btn::before` 疑似要素をターゲットにしてください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

`transform` プロパティを `translate(3px, 3px) scale(0)` に設定してください。
...
`transition` プロパティを `all 0.3s ease-in` に設定してください。
