...
現在你需要將該屬性添加回目前正在播放的歌曲。

使用條件為 `songToHighlight` 的 `if` 述語。對該述語，對 `songToHighlight` 使用 `setAttribute`，並將 `"aria-current"` 和 `"true"` 作為第一和第二個引數傳入。

# --hints--

你應該使用條件 `songToHighlight` 創建一個 `if` 述語。

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

你應該在你的 `if` 述語內對 `songToHighlight` 使用 `setAttribute()` 方法。

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

你應該將 `"aria-current"` 和 `"true"` 作為 `setAttribute` 方法的第一和第二個引數傳入。
