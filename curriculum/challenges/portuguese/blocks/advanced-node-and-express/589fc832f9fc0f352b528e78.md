...
Agora, o client terá todas as informações necessárias para exibir corretamente a contagem de usuário atual e anunciar quando um usuário se conectar ou desconectar! Para lidar com este evento no lado do cliente, devemos escutar por `'user'`, então atualizar a contagem atual de usuários usando jQuery para alterar o texto de `#num-users` para `'{NUMBER} usuários online'`, assim como adicionar um `<li>` à lista não ordenada com id `messages` com `'{NAME} {entrou/saiu} do chat.'`.
...
Envie sua página quando você achar que ela está certa. Se você estiver encontrando erros, pode conferir <a href="https://forum.freecodecamp.org/t/advanced-node-and-express/567135/3#announce-new-users-10" target="_blank" rel="noopener noreferrer nofollow">o projeto concluído até este ponto</a>.
