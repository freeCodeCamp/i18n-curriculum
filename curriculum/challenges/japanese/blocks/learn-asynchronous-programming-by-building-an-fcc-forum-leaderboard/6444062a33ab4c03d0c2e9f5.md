...
最後の投稿からの経過時間を表示するには、`timeAgo` 関数を呼び出し、引数として `bumped_at` 変数を渡してください。この関数呼び出しは最後の `td` 要素の中に配置します。

これらの変更を行ったら、テーブルを横にスクロールして `Activity` 列に新しい値が表示されているのを確認してください。

# --hints--

最後の `timeAgo` 要素で、引数に `bumped_at` を指定して `td` 関数を呼び出す必要があります。
