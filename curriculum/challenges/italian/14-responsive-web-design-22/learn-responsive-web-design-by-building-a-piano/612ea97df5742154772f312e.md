...
Il selettore `.key.black--key::after` dovrebbe avere una proprietà `position` impostata su `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.key.black--key::after')?.position === 'absolute');
```

Il selettore `.key.black--key::after` dovrebbe avere una proprietà `left` impostata a `-18px`.
