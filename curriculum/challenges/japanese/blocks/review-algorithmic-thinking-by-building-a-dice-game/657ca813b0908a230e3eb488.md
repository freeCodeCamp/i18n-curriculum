...
ゲームの各ラウンドで、ユーザーは最大3回までサイコロを振ることができます。ユーザーが`rollDiceBtn`をクリックしたときにすでに3回振っている場合、ブラウザはスコアを選択する必要があることを示す`alert()`を表示するべきです。そうでなければ、現在の動作のようにサイコロを振り、`rolls`変数をインクリメントしてください。

# --hints--

イベントリスナーはサイコロを振る処理を行うべきです。
...
サイコロが3回振られている場合、ブラウザはアラートを表示するべきです。
