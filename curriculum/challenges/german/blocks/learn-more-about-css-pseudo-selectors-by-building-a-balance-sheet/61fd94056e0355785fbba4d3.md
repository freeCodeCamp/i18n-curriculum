...
Fügen Sie innerhalb des zweiten `tr` ein `th`-Element mit dem Text `Expenses Annual anticipated expenses, such as payroll.` hinzu. Wickeln Sie diesen Text, außer `Expenses `, in ein `span`-Element mit dem `class`-Attribut, das auf `description` gesetzt ist.
