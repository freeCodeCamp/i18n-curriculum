...
Rufen Sie die `.map()`-Methode bei Ihrem `rangeFromString()`-Aufruf auf und übergeben Sie eine Referenz auf `addCharacters` als Callback-Funktion.

# --hints--

Sie sollten die `.map()`-Methode bei Ihrem `rangeFromString()`-Aufruf aufrufen.
...
Sie sollten eine Referenz auf `addCharacters` als Callback an Ihre `.map()`-Methode übergeben.
