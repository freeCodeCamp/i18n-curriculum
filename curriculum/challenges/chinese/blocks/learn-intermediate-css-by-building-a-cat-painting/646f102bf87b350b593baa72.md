...
在 `.cat-whiskers-right` 元素内，创建 3 个 `div` 元素，类分别为 `cat-whisker-right-top`、`cat-whisker-right-middle` 和 `cat-whisker-right-bottom`。

# --hints--

你不应该更改已有的类为 `cat-whiskers-right` 的 `div` 元素。

```js
assert(document.querySelectorAll('div.cat-whiskers-right').length === 1)
```

你应该在你的 `.cat-whiskers-right` 元素内部创建三个 `div` 元素。

```js
assert(document.querySelectorAll('.cat-whiskers-right div').length === 3)
```

位于 `.cat-whiskers-right` 元素内的第一个 `div` 元素应具有类 `cat-whisker-right-top`。

```js
assert(document.querySelectorAll('.cat-whiskers-right div')[0].classList.contains('cat-whisker-right-top'))
```

位于 `.cat-whiskers-right` 元素内的第二个 `div` 元素应该有类 `cat-whisker-right-middle`。

```js
assert(document.querySelectorAll('.cat-whiskers-right div')[1].classList.contains('cat-whisker-right-middle'))
```

位于 `.cat-whiskers-right` 元素内的第三个 `div` 元素应该有类 `cat-whisker-right-bottom`。
