...
Crie outro elemento `p` abaixo do divisor grande. Dê ao elemento `p` o texto `Vitamin D 2mcg 10%`.

O elemento `p` contém apenas texto, Você pode encapsular a porcentagem em um elemento `span` para que seja considerada uma entidade separada do resto do texto e movida para a direita.
...
O novo elemento `p` deve ter o texto `Vitamin D 2mcg 10%`.
...
O texto do elemento `span` não deve estar em negrito.
