...
제품이 이미 장바구니에 있는지 여부에 따라 `addItem` 메서드의 동작을 변경해야 합니다. 현재 제품이 이미 장바구니에 있는지 확인하는 삼항 연산자를 만드세요. 구문 오류를 피하기 위해 참과 거짓 표현식 모두에 `undefined`을 사용하세요.

# --hints--

`currentProductCount`가 `1`보다 큰지 확인해야 합니다.
...
`currentProductCount > 1` 조건에 삼항 연산자를 사용해야 합니다.
