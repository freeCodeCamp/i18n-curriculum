...
為了在所有這些檢查之後驗證數字是否有效，呼叫 `all()` 函式並傳入包含 `valid_in_row`、`valid_in_col` 和 `valid_in_square` 的列表。並且，傳回 `all()` 呼叫的結果。

# --hints--

你應該傳回一個對 `all()` 函式的呼叫，並傳入一個包含 `valid_in_row`、`valid_in_col` 和 `valid_in_square` 的列表。 
