...
Ahora que la verificación `if` está en su lugar, usa `e.target.value` para buscar el error correspondiente en `bugMap` y llama a su método `render`. Después de eso, deberías poder seleccionar los errores y ver el resultado de inmediato.

Con eso, ¡tu taller de selector de emojis de errores está completo!

# --hints--

Debes llamar al método `render` en el error correspondiente de `bugMap`.
