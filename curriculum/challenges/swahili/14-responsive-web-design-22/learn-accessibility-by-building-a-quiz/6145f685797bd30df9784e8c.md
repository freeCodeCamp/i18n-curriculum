...
Usisahau kuipa `form` yako kitufe cha kuwasilisha na maandishi `Send`.
...
Unapaswa kukipa kitufe cha kuwasilisha `type` ya `submit`.

```js
assert.exists(document.querySelector('button[type="submit"]') || document.querySelector('form > input[type="submit"]'));
```

Kitufe cha kuwasilisha kinapaswa kuonyesha maandishi `Send`.
