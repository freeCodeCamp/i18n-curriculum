...
같은 보간 구문을 사용하여 텍스트가 `p`인 두 번째 `consumedCalories Calories Consumed` 요소와 텍스트가 `exerciseCalories Calories Burned`인 세 번째 요소를 추가하세요. `consumedCalories`와 `exerciseCalories` 변수를 적절한 값으로 바꾸는 것도 잊지 마세요.

# --hints--

템플릿 리터럴에 두 번째 `p` 요소를 추가해야 합니다.
...
두 번째 `p` 요소는 텍스트가 `${consumedCalories} Calories Consumed`이어야 합니다.
...
세 번째 `p` 요소는 텍스트가 `${exerciseCalories} Calories Burned`이어야 합니다.
