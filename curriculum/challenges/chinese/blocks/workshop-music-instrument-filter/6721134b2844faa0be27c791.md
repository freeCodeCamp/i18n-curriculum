...
剩余的记录仪已为你添加到 `instrumentsArr` 数组中。

现在，声明两个变量 `selectContainer` 和 `productsContainer`，并分别将它们赋值为你的超文本标记语言下拉菜单和具有类 `products-container` 的元素。

# --hints--

你应该有一个 `selectContainer` 变量。

```js
assert.exists(selectContainer);
```

你的 `selectContainer` 变量应具有你的 `select` 元素的值。

```js
assert.strictEqual(selectContainer, document.querySelector("select"));
```

你应该有一个 `productsContainer` 变量。

```js
assert.exists(productsContainer);
```

你的 `productsContainer` 变量应具有你的 `.products-container` 元素的值。
