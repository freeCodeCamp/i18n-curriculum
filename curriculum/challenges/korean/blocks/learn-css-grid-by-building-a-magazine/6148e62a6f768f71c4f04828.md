...
`place-items` 속성은 `align-items`과 `justify-items` 값을 동시에 설정하는 데 사용할 수 있습니다. `place-items` 속성은 하나 또는 두 개의 값을 받습니다. 하나의 값이 제공되면 `align-items`와 `justify-items` 속성 모두에 사용됩니다. 두 개의 값이 제공되면 첫 번째 값은 `align-items` 속성에, 두 번째 값은 `justify-items` 속성에 사용됩니다.

`.image-wrapper` 선택자에 `place-items` 속성을 `center`으로 설정하세요.

# --hints--

`.image-wrapper` 선택자에 `place-items` 속성을 `center`으로 설정해야 합니다.
