...
Dentro da declaração `if`, obtenha o elemento `cardTitle` e defina seu `textContent` para `foundCard.name`. Faça o mesmo para o elemento `description`, definindo seu `textContent` para `foundCard.desc`.

# --hints--

Você deve definir o `cardTitle` `textContent` para `foundCard.name`. Não esqueça de usar a palavra-chave `this`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

Você deve definir o `description` `textContent` para `foundCard.desc`. Não esqueça de usar a palavra-chave `this`.
