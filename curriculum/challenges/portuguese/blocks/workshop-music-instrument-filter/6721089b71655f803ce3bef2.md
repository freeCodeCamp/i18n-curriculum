...
Agora adicione quatro opções ao seu dropdown, `All`, `Woodwinds`, `Brass` e `Percussion`, nesta ordem. Para cada opção, use a opção em minúsculas para especificar o atributo `id` e o atributo `value`.

# --hints--

Você deve ter quatro elementos `option` no seu elemento `select`.

```js
assert.lengthOf(document.querySelectorAll("select>option"), 4);
```

Seu primeiro elemento `option` deve ter o texto `All`.

```js
assert.equal(document.querySelectorAll("select>option")[0].innerText,"All");
```

Sua opção `All` deve ter um atributo `id` e um atributo `value` com o valor `all`.
...
Seu segundo elemento `option` deve ter o texto `Woodwinds`.

```js
assert.equal(document.querySelectorAll("select>option")[1].innerText,"Woodwinds");
```

Sua opção `Woodwinds` deve ter um atributo `id` e um atributo `value` com o valor `woodwinds`.
...
Seu terceiro elemento `option` deve ter o texto `Brass`.

```js
assert.equal(document.querySelectorAll("select>option")[2].innerText,"Brass");
```

Sua opção `Brass` deve ter um atributo `id` e um atributo `value` com o valor `brass`.
...
Seu quarto elemento `option` deve ter o texto `Percussion`.

```js
assert.equal(document.querySelectorAll("select>option")[3].innerText,"Percussion");
```

Sua opção `Percussion` deve ter um atributo `id` e um atributo `value` com o valor `percussion`.
