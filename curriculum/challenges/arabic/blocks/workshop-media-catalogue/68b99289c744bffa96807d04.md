...
حاليًا، تُعرض الأفلام والمسلسلات التلفزيونية معًا عند طباعة الكتالوج. في الخطوات التالية، ستتولى تصفية العناصر إلى فئتين لعرضهما بشكل منفصل.

داخل صنف `MediaCatalogue`، أنشئ طريقة باسم `get_movies` واجعلها تُرجع قائمة تحتوي فقط على مثيلات الصنف الأصل `Movie` الموجودة في `self.items`.

استخدم الطريقة التي تفضلها للقيام بذلك، لكن تذكر استخدام الدالة `type` لأن `isinstance` لن تتمكن من التمييز بين الأصناف الأصلية والفرعية.

# --hints--

يجب أن يحتوي صنف `MediaCatalogue` على طريقة باسم `get_movies`.
...
يجب أن تُرجع طريقة `get_movies` قائمة تحتوي على كل العناصر في `self.items` التي هي مثيلات لصنف `Movie`.
