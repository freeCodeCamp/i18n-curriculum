...
لتشغيل الأغنية في أي وقت ينقر المستخدم عليها، أضف خاصية `onclick` إلى أول عنصر زر. داخل `onclick`، استدعِ الدالة `playSong` مع `song.id`.

لا تنسَ أنك تحتاج إلى استخدام التضمين مع علامة الدولار هنا.

# --hints--

يجب أن تضيف خاصية `onclick` بالقيمة `playSong(${song.id})` إلى عنصر `button`.
