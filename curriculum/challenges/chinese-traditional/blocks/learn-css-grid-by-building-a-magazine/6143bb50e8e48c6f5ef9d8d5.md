...
在你的 `aside` 元素中，創建兩個 `img` 元素、一個 `blockquote` 元素，以及第三個 `img` 元素。為 `blockquote` 元素設定一個 `class`，值為 `image-quote`。

# --hints--

你應該在你的 `aside` 元素中創建三個 `img` 元素。

```js
assert(document.querySelectorAll('aside img')?.length === 3);
```

你應該在你的 `aside` 元素中創建一個 `blockquote` 元素。

```js
assert.exists(document.querySelector('aside blockquote'));
```

你的 `blockquote` 元素應該設定 `class` 為 `image-quote`。
