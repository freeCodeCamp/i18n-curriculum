...
在你的 `else` 子句中，將字符串 `"general"` 賦值給`selectedCategory.className`。

在其下面，將字符串 `"General"` 賦值給 `selectedCategory.category`。

最後，將數字 `1` 賦值給 `selectedCategory.id`。

# --hints--

你應該將字符串 `"general"` 賦值給 `selectedCategory.className`。

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

你應該將字符串 `"General"` 賦值給 `selectedCategory.category`。

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

你應該將數字 `1` 賦值給 `selectedCategory.id`。
