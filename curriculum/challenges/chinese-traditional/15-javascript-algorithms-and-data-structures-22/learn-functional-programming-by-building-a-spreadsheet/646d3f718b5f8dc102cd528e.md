...
既然你可以評估數學表達式，那麼你需要考慮運算順序。 聲明一個採用 `str` 參數的 `highPrecedence` 函數。

# --hints--

你應該聲明一個 `highPrecedence` 變量。

```js
assert.match(code, /(?:var|let|const)\s+highPrecedence/);
```

你應該使用 `const` 來聲明你的 `highPrecedence` 變量。

```js
assert.match(code, /const\s+highPrecedence/);
```

你的 `highPrecedence` 變量應該是一個函數。

```js
assert.isFunction(highPrecedence);
```

你的 `highPrecedence` 函數應該使用箭頭語法。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

你的 `highPrecedence` 函數應該有一個 `str` 參數。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>/);
```

你的 `highPrecedence` 函數應該是空的。
