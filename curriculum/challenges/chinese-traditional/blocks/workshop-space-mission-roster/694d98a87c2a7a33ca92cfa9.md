...
使用 `for` 迴圈記錄 `updatedCrew` 陣列中每個太空人的 `name`。迴圈結束後，傳回 `updatedCrew` 陣列以完成你的 `swapCrewMembers` 函式（程式）。

# --hints--

你應該使用 `for` 迴圈來迭代 `updatedCrew` 陣列並記錄每個太空人的 `name`。 
...
你應該在記錄其太空人名稱後傳回 `updatedCrew` 陣列。
