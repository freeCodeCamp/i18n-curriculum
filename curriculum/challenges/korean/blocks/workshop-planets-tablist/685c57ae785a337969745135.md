...
마지막 단계에서는 관련된 탭을 클릭할 때 패널이 보이도록 만들어야 합니다. `hidden`의 `panel` 속성을 `false`로 설정하면 됩니다.

어떤 탭이든 클릭하면 해당 패널 내용이 표시되는 것을 확인하세요.

이 마지막 변경으로 워크숍이 완료되었습니다!

# --hints--

탭을 클릭할 때 패널의 `hidden` 속성을 `false`로 설정해야 합니다.
