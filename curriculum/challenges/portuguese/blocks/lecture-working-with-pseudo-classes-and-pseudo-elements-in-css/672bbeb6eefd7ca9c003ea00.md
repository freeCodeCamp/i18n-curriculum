...
Elementos vazios, ou seja, elementos sem filhos além de espaços em branco, também estão incluídos na árvore do documento.É por isso que existe a pseudo-classe `:empty` para direcionar elementos vazios.Por exemplo, este código HTML tem dois itens de lista vazios:
...
Com a pseudo-classe `:empty`, você pode estilizar os itens de lista vazios de forma diferente:
...
Aqui está um exemplo em HTML de uma tabela de lista de preços de frutas:
...
Aqui está o CSS usando a pseudo-classe `:nth-child` para direcionar as células da tabela com base nas posições ímpares e pares:
...
Usar as pseudo-classes `:first-child` e `:last-child` selecionará tanto `Item 1` e `Item 3` neste HTML:
...
Para mostrar como a pseudo-classe `:only-child` funciona, aqui está um exemplo em HTML com dois elementos `div` separados:
...
Usar a pseudo-classe `:only-child` garante que apenas o elemento `div` com um único filho seja selecionado:
...
No HTML abaixo, `:first-of-type` e `:last-of-type` se aplicam ao primeiro elemento e ao último elemento dentro do elemento `section`:
...
No HTML abaixo, há dois elementos `div` sendo que um deles possui um único elemento:
...
Aqui está o CSS que se aplica apenas ao primeiro container:
