...
Iterieren Sie nun über die Zielknoten, um die Ergebnisse für jeden einzelnen anzuzeigen.

Fügen Sie eine `for...of`-Schleife hinzu, die über `targets` iteriert und `nodeNo` als Schleifenvariable verwendet. Lassen Sie den Schleifenkörper vorerst leer.

# --hints--

Sie sollten eine `for...of`-Schleife mit der Variablen `nodeNo` erstellen, die über `targets` iteriert.
