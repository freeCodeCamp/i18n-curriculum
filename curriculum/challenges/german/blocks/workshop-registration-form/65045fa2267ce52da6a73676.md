...
Befolgen Sie bewährte Vorgehensweisen zur Barrierefreiheit, indem Sie die `input`-Elemente und die `label`-Elemente im zweiten `fieldset` verlinken.

Verwenden Sie `personal-account` und `business-account` als Werte für die jeweiligen `id`-Attribute.

# --hints--

Das erste `input`-Element sollte ein `id` mit dem Wert `personal-account` haben.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[0]?.matches('#personal-account'))
```

Das zweite `input`-Element sollte ein `id` mit dem Wert `business-account` haben.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[1]?.matches('#business-account'))
```

Das erste `label`-Element sollte ein `for`-Attribut mit dem Wert `personal-account` haben.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) label')?.[0]?.matches('label[for="personal-account"]'))
```

Das zweite `label`-Element sollte ein `for`-Attribut mit dem Wert `business-account` haben.
