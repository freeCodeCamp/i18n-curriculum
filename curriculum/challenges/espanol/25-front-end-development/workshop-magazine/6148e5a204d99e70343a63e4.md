...
La propiedad `gap` es una forma abreviada de establecer el valor de `column-gap` y `row-gap` al mismo tiempo. Si se le da un valor, establece tanto `column-gap` como `row-gap` a ese valor. Si se le dan dos valores, establece `row-gap` al primer valor y `column-gap` al segundo.

Dar al selector `.image-wrapper` una propiedad `gap` establecida en `2rem`.

# --hints--

Tu elemento `.image-wrapper` debe tener una propiedad `gap` establecida en `2rem`.
