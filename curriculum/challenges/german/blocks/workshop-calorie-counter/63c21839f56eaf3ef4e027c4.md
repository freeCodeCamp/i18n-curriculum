...
Erstellen Sie ein weiteres `label`-Element (in einer neuen Zeile) am Ende Ihres `HTMLString`. Dieses `label` sollte den Text `Entry # Calories` enthalten, wobei Sie Ihre Template-Literal-Syntax verwenden, um `#` durch den Wert von `entryNumber` zu ersetzen, und das `for`-Attribut auf `X-#-calories` setzen, wobei `X` das `value` von `entryDropdown` ist und `#` der Wert von `entryNumber`.

# --hints--

Sie sollten zwei `label`-Elemente in Ihrem `HTMLString` haben.
...
Ihr neues `label`-Element sollte in einer neuen Zeile stehen.
...
Ihr neues `label`-Element sollte nach Ihrem `input`-Element kommen.
...
Ihr neues `label`-Element sollte ein `for`-Attribut mit dem Wert `${entryDropdown.value}-${entryNumber}-calories` haben.
...
Ihr neues `label`-Element sollte den Text `Entry ${entryNumber} Calories` enthalten.
...
Sie sollten Ihre vorhandenen Elemente nicht verändern.
