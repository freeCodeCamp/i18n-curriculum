...
在新的一行，使用加法指定運算子(`+=`)，將字串 `" You attack it with your <weapon>."` 添加到 `text` 值，並將 `<weapon>` 替換為玩家目前的武器。此外，請記得這行文字以空格開頭，以確保正確顯示。 

# --hints--

你應該使用點記法在新行存取 `text` 的 `innerText` 屬性。
...
你應該對 `text.innerText` 使用複合指派。

```js
assert.match(attack.toString(), /text\.innerText\s*\+=/);
```

你應該將字串 `" You attack it with your "` 添加到 `text.innerText` 的值中。請記得空格很重要。

```js
assert.match(attack.toString(), /text\.innerText\s*\+=\s*('|") You attack it with your \1/);
```

你應該使用串接運算子將當前武器添加到字串。你可以使用 `weapons[currentWeaponIndex].name` 取得當前武器。
