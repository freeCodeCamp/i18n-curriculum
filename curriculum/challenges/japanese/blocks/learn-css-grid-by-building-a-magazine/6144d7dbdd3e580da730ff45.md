...
`body` セレクターを作成してください。`font-family` プロパティを `Baskervville` に設定し、フォールバックとして `serif` を指定してください。`color` プロパティを `linen` に、`background-color` プロパティを `rgb(20, 30, 40)` に設定してください。
...
`body` セレクターには、`font-family` プロパティが `Baskervville` に設定されており、フォールバックとして `serif` が指定されているはずです。
...
`body` セレクターには、`color` プロパティが `linen` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.color === 'linen');
```

`body` セレクターには、`background-color` プロパティが `rgb(20, 30, 40)` に設定されているはずです。
