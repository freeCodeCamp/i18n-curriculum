...
Devi aggiungere `resetText` come figlio dell'elemento `resetButton`, e anche `resetButton` come figlio dell'elemento `playlistSongs`. Per questo, c'è un metodo `appendChild()` da usare.

`appendChild()` ti permette di aggiungere un nodo o un elemento come figlio di un altro elemento. Nell'esempio qui sotto, il testo "Click me" sarebbe allegato al pulsante:
...
Usa `appendChild()` per allegare `resetText` all'elemento `resetButton`, e `resetButton` all'elemento `playlistSongs`.

# --hints--

Dovresti usare il metodo `appendChild()` su `resetButton`.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

Dovresti passare `resetText` come valore del tuo primo `appendChild()`.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

Dovresti usare il metodo `appendChild()` su `playlistSongs`.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

Dovresti passare `resetButton` come valore del tuo secondo `appendChild()`.
