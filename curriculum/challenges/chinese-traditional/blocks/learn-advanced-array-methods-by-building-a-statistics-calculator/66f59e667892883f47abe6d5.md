...
還有另一種撰寫 `forEach` 的方式。你可以使用表達式主體 `() =>`，而不是為回呼使用區塊主體 `() => {}`。

你必須將 `if...else` 述語轉換成表達式。將該表達式寫成三元運算子，並為三元運算子使用單一指派。

```js
assignment = condition ? exprIfTrue : exprIfFalse
```

將 `forEach` 回呼轉換為使用表達式主體，並用三元運算子取代述語。

# --hints--

你的函式（程式）仍然應該正確地遞增 `counts` 變數。 
