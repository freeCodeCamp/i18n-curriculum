...
Für zusätzliche Kontrolle über das Layout Ihres Inhalts können Sie ein CSS Grid innerhalb eines CSS Grids verwenden.

Setzen Sie die `display`-Eigenschaft Ihres `.heading`-Selektors auf `grid`.

# --hints--

Ihr `.heading`-Selektor sollte eine `display`-Eigenschaft mit dem Wert `grid` haben.
