...
Щоб відобразити пісні в інтерфейсі користувача (UI), потрібно створити функцію.

Використайте синтаксис стрілкової функції, щоб створити `renderSongs` функцію, яка прийматиме `array` як параметр.

# --hints--

Ви повинні використати `const`, щоб створити порожню функцію з ім’ям `renderSongs`.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs` має бути функцією.

```js
assert.isFunction(renderSongs)
```

Ваша функція `renderSongs` має використовувати синтаксис стрілкової функції.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

Ваша функція `renderSongs` має приймати `array` як параметр.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

Ваша функція `renderSongs` має бути порожньою.
