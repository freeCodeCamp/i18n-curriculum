...
そのブール値の式の下に、プレイヤーの`x`位置がプラットフォームの`x`位置からプレイヤーの幅の半分を引いた値以上かどうかをチェックする別のブール値の式を追加してください。プレイヤーの幅の半分を計算するには、除算演算子（`/`）を使ってください。

# --hints--

プレイヤーの`x`位置がプラットフォームの`x`位置からプレイヤーの幅を`2`で割った値を引いた値以上かどうかをチェックするブール値の式があるはずです。
