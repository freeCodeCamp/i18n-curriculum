...
현재 재생 중인 노래 또는 재생 목록의 첫 번째 노래를 가져와야 합니다. 이를 위해 `song` 상수를 만들고 OR 연산자(`||`)를 사용해 `userData`의 현재 노래 또는 `userData?.songs` 배열의 첫 번째 노래로 설정하세요.

옵셔널 체이닝을 사용하는 것도 잊지 마세요.

# --hints--

기존 `setPlayButtonAccessibleText`와 그 내용을 수정하면 안 됩니다.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

`userData?.currentSong` 또는 `userData?.songs[0]`에 접근해야 합니다.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

`userData?.currentSong || userData?.songs[0]`을 `song` 상수에 할당해야 합니다.
