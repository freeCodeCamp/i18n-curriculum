...
한 가지 더 있습니다. `Load More Authors` 버튼을 더 이상 불러올 데이터가 없을 때까지 계속 클릭하면 텍스트가 `"No more data to load"`으로 바뀌는데, 커서 값은 여전히 `pointer`입니다. 왜 커서 값을 대신 `not-allowed`으로 바꾸지 않을까요?

`style` 버튼의 `Load More Authors` 속성에 접근해서 `cursor`을 `"not-allowed"`로 설정하세요.

이렇게 하면 작성자 페이지가 완성됩니다!
...
점 표기법으로 `style`의 `loadMoreBtn` 속성에 접근하세요.

```js
assert.match(code, /loadMoreBtn\.style/)
```

점 표기법으로 `cursor` 속성에서 `style`에 접근하세요.

```js
assert.match(code, /loadMoreBtn\.style\.cursor/)
```

`cursor` 속성의 값을 `"not-allowed"`으로 설정하세요.
