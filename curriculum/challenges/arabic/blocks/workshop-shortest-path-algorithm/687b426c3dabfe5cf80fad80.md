...
بعد أن عرّفت العقد التي تريد عرض النتائج لها، يجب أن تقوم بالتكرار عليها.

أضف حلقة `for` تقوم بالتكرار على كل `node_no` في قائمة `targets`. أضف `pass` كعنصر نائب داخل جسم الحلقة.

# --hints--

يجب أن تنشئ حلقة `for` تستخدم `node_no` للتكرار على `targets`.
