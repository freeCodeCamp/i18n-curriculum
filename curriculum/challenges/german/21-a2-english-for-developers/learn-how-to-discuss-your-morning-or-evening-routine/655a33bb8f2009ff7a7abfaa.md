...
`May` alleine, schlägt Möglichkeiten oder Erlaubnis vor. `May be` suggeriert eine Möglichkeit oder Potenzialität.

`Maybe` ist ein Adverb, das Unsicherheit oder Möglichkeit ausdrückt. Zum Beispiel:

`It may be raining outside.` (schlägt die Möglichkeit von Regen vor) `Maybe I will go out later.` (drückt Unsicherheit aus raus zu gehen) `May I ask a question?` (bittet um die Erlaubnis zu sprechen)
