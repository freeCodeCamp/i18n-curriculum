...
`infixToFunction[operator]` 返回一个函数。 直接调用该函数，传递 `arg1` 和 `arg2` 作为参数。

# --hints--

你的回调函数应该返回调用 `infixToFunction[operator]` 的结果。
...
你应该将 `arg1` 作为第一个参数传递给 `infixToFunction[operator]` 调用。
...
你应该将 `arg2` 作为第二个参数传递给 `infixToFunction[operator]` 调用。
