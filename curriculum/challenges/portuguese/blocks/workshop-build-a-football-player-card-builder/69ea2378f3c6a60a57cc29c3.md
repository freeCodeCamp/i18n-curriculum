...
Agora faça um map no array `POSITIONS` usando uma arrow function que recebe cada string de posição como `pos` e retorna um elemento `option`. A option deve ter um `key` igual a `pos` e o valor também `pos`. Além disso, exiba `pos` como o texto de cada `option` gerado.

Sinta-se à vontade para usar qualquer coisa como parâmetro do seu map, não precisa ser `pos`.

# --hints--

Seu `select` de posição deve conter elementos `option` mapeados do array `POSITIONS`.
...
O `value` de cada option deve corresponder ao texto exibido e ser uma posição válida.
