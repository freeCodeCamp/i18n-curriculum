...
`return` の呼び出し内の `filteredItems.map()` 文の前に、`isChecked` という変数を作成し、`includes()` メソッドを使って現在の `item` が `selectedItems` 配列に含まれているかどうかを確認してください。

次に、`style` 要素に `li` プロップを追加し、三項演算子を使って `textDecoration` が true の場合は `line-through` プロパティを `isChecked` に、false の場合は `none` に設定してください。三項演算子は JavaScript の式なので、波括弧で囲む必要があることを忘れないでください。

たとえば、シンプルなコンポーネントで段落の色を条件付きで設定するために三項演算子を使う方法は次の通りです。
...
`isChecked` 配列の `includes()` メソッドを使って現在の `selectedItems` が配列に含まれているかどうかを確認する `item` という変数を作成してください。
...
`style` プロップには、`textDecoration` が true の場合は `line-through` プロパティを `isChecked` に、false の場合は `none` に設定する三項演算子を含めてください。
