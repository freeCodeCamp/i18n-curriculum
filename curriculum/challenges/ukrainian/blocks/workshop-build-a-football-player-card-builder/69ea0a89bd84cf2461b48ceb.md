...
Всередині блоку `try` збережіть поточні дані гравця у `localStorage`, викликавши `localStorage.setItem` з ключем сховища та рядковим поданням об’єкта гравця.

А в блоці `catch` виведіть у консоль рядок `Failed to save player data:`, а потім параметр `error`.

# --hints--

Ваш блок try `useEffect` має викликати `localStorage.setItem` з `STORAGE_KEY` та рядковим поданням об’єкта `player`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

Ваш блок catch `useEffect` має вивести у консоль `'Failed to save player data:'` та `error`.
