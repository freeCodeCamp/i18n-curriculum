...
Zum Schluss müssen Sie das Bild für das Benutzeravatar zurückgeben.

Beginnen Sie damit, ein `return` gefolgt von einem Satz Template-Literalen hinzuzufügen. Fügen Sie innerhalb der Template-Literale ein `img`-Element hinzu.

Fügen Sie innerhalb des `img`-Tags ein `src`-Attribut mit dem Wert `${userAvatarUrl}` hinzu. Für das `alt`-Attribut fügen Sie einen Wert von `${user.name}` hinzu.
...
Sie sollten ein Template-Literal zurückgeben.
...
Sie sollten ein `img`-Element innerhalb des Template-Literals haben.
...
Sie sollten ein `src`-Attribut innerhalb des `img`-Tags haben.
...
Sie sollten den Wert für das `src`-Attribut auf die Konstante mit dem Namen `userAvatarUrl` setzen.
...
Sie sollten ein `alt`-Attribut nach dem `src`-Attribut haben.
...
Sie sollten den Wert des `alt`-Attributs auf die Konstante mit dem Namen `user.name` setzen.
