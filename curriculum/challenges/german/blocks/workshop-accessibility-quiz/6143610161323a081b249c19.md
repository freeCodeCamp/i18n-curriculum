...
Füllen Sie den Inhalt des Quiz aus, indem Sie unterhalb von `#student-info` drei `div`-Elemente mit einem `class` von `info` hinzufügen.

Verschachteln Sie dann innerhalb jedes `div` jeweils ein `label`-Element und ein `input`-Element.

# --hints--

Sie sollten drei `div`-Elemente unterhalb des `h2#student-info`-Elements verschachteln.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.length, 3);
```

Dem ersten `div` sollten Sie einen `class` von `info` geben.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.className, 'info');
```

Dem zweiten `div` sollten Sie einen `class` von `info` geben.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.className, 'info');
```

Dem dritten `div` sollten Sie einen `class` von `info` geben.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.className, 'info');
```

Sie sollten ein `label`-Element innerhalb des ersten `div` verschachteln.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.querySelectorAll('label')?.length, 1);
```

Sie sollten ein `input`-Element innerhalb des ersten `div`, nach dem `label`, verschachteln.
...
Sie sollten ein `label`-Element innerhalb des zweiten `div` verschachteln.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.querySelectorAll('label')?.length, 1);
```

Sie sollten ein `input`-Element innerhalb des zweiten `div`, nach dem `label`, verschachteln.
...
Sie sollten ein `label`-Element innerhalb des dritten `div` verschachteln.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.querySelectorAll('label')?.length, 1);
```

Sie sollten ein `input`-Element innerhalb des dritten `div`, nach dem `label`, verschachteln.
