...
Después de la verificación `"Unknown"`, agrega la lógica para agrupar por década los libros con años numéricos. Declara `const decade = Math.floor(book.year / 10) * 10`. Por ejemplo, `Math.floor(1977 / 10)` da `197`, multiplicado por `10` da `1970`.

Luego declara `const decadeKey =` `` `${decade}s` `` to produce a string like `"1970s"`.

Usa la notación de corchetes para agrupar el libro: verifica si `grouped[decadeKey]` no existe aún y inicialízalo como un arreglo vacío, luego agrega `book` a él. Aquí se requiere la notación de corchetes porque la clave está almacenada en una variable.

# --hints--

Debes declarar una variable llamada `decade` asignada a `Math.floor(book.year / 10) * 10`.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+decade\s*=\s*Math\.floor\s*\(\s*book\.year\s*\/\s*10\s*\)\s*\*\s*10/);
```

Debes declarar una variable llamada `decadeKey` que incluya `decade` y la letra `"s"`.
...
Debes verificar si `grouped[decadeKey]` no existe aún y inicializarlo como un arreglo vacío.
...
Debes agregar `book` a `grouped[decadeKey]`.
