...
이제 `powerSource`와 `powers`에 대한 상태 변수와 설정 함수를 만드세요. `powerSource`는 초기값이 빈 문자열이어야 하고, `powers`는 초기값이 빈 배열이어야 합니다.

# --hints--

배열 구조 분해를 사용해 `powerSource` 상태 변수와 `setPowerSource` 설정 함수를 만드세요.

```js
assert.match(code, /(const|let|var)\s+\[\s*powerSource\s*,\s*setPowerSource\s*\]/);
```

`powerSource`와 `setPowerSource`는 `useState` 훅을 사용해야 합니다.
...
`useState` 훅을 사용해 `powerSource`의 초기값을 빈 문자열로 설정하세요.
...
배열 구조 분해를 사용해 `powers` 상태 변수와 `setPowers` 설정 함수를 만드세요.

```js
assert.match(code, /(const|let|var)\s+\[\s*powers\s*,\s*setPowers\s*\]/);
```

`powers`와 `setPowers`는 `useState` 훅을 사용해야 합니다.
...
`useState` 훅을 사용해 `powers`의 초기값을 빈 배열로 설정하세요.
