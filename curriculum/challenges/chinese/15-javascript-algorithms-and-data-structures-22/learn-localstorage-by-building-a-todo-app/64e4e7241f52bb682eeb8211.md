...
你需要从 HTML 文件中获取的最后一组元素是 `date-input` 和 `description-input` 元素。 将它们分别保存在变量 `dateInput` 和 `descriptionInput` 中。

# --hints--

你应该使用 `getElementById()` 来访问 `date-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)date\-input\1\s*\)\s*;?/)
```

你应该将 `date-input` 元素分配给变量 `dateInput`。 不要忘记使用 `const` 来声明变量。

```js
assert.match(code, /const\s+dateInput\s*=\s*document\.getElementById\(\s*('|"|`)date\-input\1\s*\)\s*;?/)
```

你应该使用 `getElementById()` 来访问 `description-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)description\-input\1\s*\)\s*;?/)
```

你应该将 `description-input` 元素分配给变量 `descriptionInput`。 不要忘记使用 `const` 来声明变量。
