...
Modifica la chiamata `verify_card_number` in modo che, se restituisce `True`, venga stampato `'VALID!'` nella console. Altrimenti, stampa `'INVALID!'`.

# --hints--

Dovresti avere `if verify_card_number(translated_card_number):` all'interno della funzione `main`.
...
Dovresti avere `print('VALID!')` all'interno dell'istruzione `if`.
...
Dovresti avere `print('INVALID!')` all'interno della clausola `else`.
