...
Dovresti aggiungere due elementi`hr` all'elemento `.image-quote`.

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

Dovresti aggiungere un elemento `p` all'elemento `.image-quote`.

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

Gli elementi figli di `.image-quote` dovrebbero essere nell'ordine corretto.
...
Il nuovo elemento `p` dovrebbe avere `class` impostato su `quote`.

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

Il nuovo elemento `p` dovrebbe avere il testo `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
