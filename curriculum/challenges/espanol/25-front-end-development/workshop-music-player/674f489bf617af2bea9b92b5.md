...
Ahora, crea una función llamada `getNextSong`. Haz que regrese la canción que se puede encontrar en `getCurrentSongIndex()` más uno en `userData.songs`, o `undefined` si la canción actual es la última en la lista de reproducción.

# --hints--

Debes tener una función llamada `getNextSong`.

```js
assert.isFunction(getNextSong);
```

Tu función `getNextSong` debe llamar a `getCurrentSongIndex`.
...
Tu función `getNextSong` debe devolver la canción siguiente a la canción actual.
...
Tu función `getNextSong` debe devolver `undefined` cuando la canción actual es la última en la lista de reproducción.
