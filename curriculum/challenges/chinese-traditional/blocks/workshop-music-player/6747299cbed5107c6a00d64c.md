...
其餘的歌曲已為你添加到 `allSongs` 陣列中。

在之前的課程中，你學習了 <dfn>Web Audio API</dfn> 以及如何使用它來播放歌曲。所有現代瀏覽器都支援 Web Audio API，讓你能在網頁應用程式中產生和處理音訊。

創建一個名為 `audio` 的變數並將其設定為 `new Audio()`。這將創建一個新的 HTML5 `audio` 元素。
...
你應該將 `Audio` 物件的新實體指定給名為 `audio` 的變數。
