...
Geben Sie nun innerhalb der `isSelect`-Funktion einen Boolean zurück, der angibt, ob das `element` eine Instanz von `HTMLSelectElement` ist. Verwenden Sie den `instanceof`-Operator, um diese Überprüfung durchzuführen.

# --hints--

Ihre `isSelect`-Funktion sollte `element instanceof HTMLSelectElement` zurückgeben.
