...
f 字符串还允许你使用 `.nf` 格式说明符为数值数据设置特定的精度，其中 `n` 是需要显示的小数位数。

在 `result_list` 中包含的 f 字符串的花括号内，写入在 `:+` 之后紧接着显示 `3` 个十进制数字所需的格式说明符。

# --hints--

你应该将 `case 1` 块中的 `result_list` 中包含的字符串修改为 `f'x = {results[0]:+.3f}'`。
...
你应该修改 `case 2` 块中 `result_list` 中包含的字符串，以便结果以 `3` 个十进制数字显示。
