...
Verwende nach deinem `currentWeapon` den Verkettungsoperator, um `text.innerText` auf den String `"You sold a "`, then `currentWeapon`, then the string `"."` zu setzen.

# --hints--

Du solltest den Zuweisungsoperator mit `text.innerText` verwenden.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=/);
```

Du solltest `"You sold a "` to `text.innerText` hinzufügen. Die Abstände sind wichtig.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

Du solltest den Wert von `currentWeapon` zum `"You sold a "`-String hinzufügen. Verwende den Verkettungsoperator, um dies in derselben Zeile zu tun.
