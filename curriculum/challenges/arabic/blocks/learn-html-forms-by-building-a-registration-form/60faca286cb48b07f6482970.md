...
إرسال النموذج مع اختيار خيار لن يرسل قيمة مفيدة إلى الخادم. لذلك، يجب إعطاء كل `option` خاصية `value`. بدونها، سيُرسل محتوى النص الخاص بـ `option` إلى الخادم.

أعطِ أول `option` خاصية `value` بقيمة `""`، وأعطِ عناصر `option` التالية خاصيات `value` من `1` إلى `4`.
