...
이전 단계에서 노래를 재생하는 기능을 구현했습니다. 이제 재생 버튼에 기능을 추가해야 합니다.

`addEventListener()`에서 `playButton` 메서드를 사용하고 첫 번째 인수로 `"click"` 이벤트를 전달하세요. 지금은 두 번째 인수로 `playSong(0)`를 호출하는 콜백을 사용하세요.

그다음 재생 버튼을 클릭하여 테스트하세요.


# --hints--

`click`에 `playButton` 이벤트에 대한 이벤트 리스너를 추가하고 `playSong(0)` 메서드의 두 번째 인수로 `addEventListener`를 호출하는 콜백 함수를 전달해야 합니다.
