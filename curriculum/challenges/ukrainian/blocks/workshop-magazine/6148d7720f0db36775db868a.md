...
Вам потрібно мати колонку для тексту та колонку для зображень. Додайте селектору `.text-with-images` властивість `grid-template-columns` зі значенням `1fr 2fr`. Також встановіть властивість `column-gap` зі значенням `3rem`, щоб забезпечити більший відступ між колонками.

# --hints--

Ваш селектор `.text-with-images` має мати властивість `grid-template-columns` зі значенням `1fr 2fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns, '1fr 2fr');
```

Ваш селектор `.text-with-images` має мати властивість `column-gap` зі значенням `3rem`.
