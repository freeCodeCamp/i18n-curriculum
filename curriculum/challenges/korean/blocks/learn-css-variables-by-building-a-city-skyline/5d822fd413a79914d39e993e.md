...
마지막으로, 미디어 쿼리의 `:root` 선택자 안에서 네 개의 `--window-color` 변수를 모두 `#777`를 사용하도록 다시 정의하세요. 완료하면 창 크기를 조절해서 낮에서 밤으로 변하는 모습을 확인하세요.

변수는 주로 색상에 사용되며, 여기서도 그렇게 사용했습니다. 하지만 변수는 어떤 값이든 가질 수 있고 어떤 속성에도 사용할 수 있습니다. 프로젝트가 아주 멋지게 보입니다!

# --hints--

값이 `--window-color1`인 `#777`을 추가하세요.
...
값이 `--window-color2`인 `#777`를 추가하세요.
...
값이 `--window-color3`인 `#777`을 추가하세요.
...
값이 `--window-color4`인 `#777`를 추가하세요.
