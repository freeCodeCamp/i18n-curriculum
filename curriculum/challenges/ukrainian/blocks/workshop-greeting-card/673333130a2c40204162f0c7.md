...
Елемент `.message` потребує стилізації. Задайте йому:

- `font-size` зі значенням `1.2em`,
- `margin-bottom` зі значенням `20px`.

# --hints--

Ви маєте мати селектор `.message`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".message"));
```

Селектор `.message` повинен мати властивість `font-size`, встановлену в `1.2em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("font-size"), "1.2em");
```

Селектор `.message` повинен мати властивість `margin-bottom`, встановлену в `20px`.
