...
أنشئ عنصر `label` آخر (في سطر جديد) في نهاية `HTMLString`. يجب أن يحتوي هذا `label` على النص `Entry # Calories`، مستخدمًا صيغة القالب النصي لاستبدال `#` بقيمة `entryNumber`، وأن تكون خاصية `for` مضبوطة إلى `X-#-calories`، حيث أن `X` هي `value` لـ `entryDropdown` و `#` هي قيمة `entryNumber`.
...
يجب أن يحتوي عنصر `label` الجديد على خاصية `for` بقيمة `${entryDropdown.value}-${entryNumber}-calories`.
