...
Dovresti assegnare a `:root` una proprietà `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

Dovresti dare a `--penguin-skin` il valore `gray` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

Dovresti dare a `.penguin-head` una proprietà `background` di `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
Dovresti dare a `.penguin-body::before` una proprietà `background-color` con il valore `var(--penguin-skin)` invece di `--fcc-actual--`.
...
Dovresti dare a `.arm` una proprietà `background` con il valore `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
