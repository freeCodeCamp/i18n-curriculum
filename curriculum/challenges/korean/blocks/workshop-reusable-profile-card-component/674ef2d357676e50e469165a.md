...
`Card` 컴포넌트를 사용하려면 `export` 키워드를 사용해 `App` 함수형 컴포넌트를 만드세요.

`App` 컴포넌트 안에서 빈 문자열을 포함한 소괄호 한 쌍을 반환하세요.

# --hints--

`App` 함수형 컴포넌트를 내보내야 합니다.

```js
assert.isFunction(window.index.App);
```

`App` 컴포넌트는 빈 문자열이 들어있는 소괄호 한 쌍을 반환해야 합니다.
