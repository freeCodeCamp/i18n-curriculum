...
ضع العنصر `.chin` بحيث يكون على بعد `25%` من الأعلى، و`5%` من اليسار بالنسبة لوالده. ثم، أعطِ الزوايا العلوية انحناءً مقداره `70%`، والزوايا السفلية انحناءً مقداره `100%`.

# --hints--

يجب أن تعطي `.chin` خاصية `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

يجب أن تكون قيمة `top` لـ `.chin` هي `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

يجب أن تعطي `.chin` خاصية `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

يجب أن تكون قيمة `left` لـ `.chin` هي `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

يجب أن تعطي `.chin` خاصية `border-radius` بقيمة `70% 70% 100% 100%`.
