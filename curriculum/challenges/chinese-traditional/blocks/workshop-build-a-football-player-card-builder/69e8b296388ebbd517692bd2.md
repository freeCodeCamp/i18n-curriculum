...
玩家資訊區塊現在已完成。在玩家資訊區塊的結尾 `div` 之後，且仍在 `form-panel` 內，新增一個 `div`。在其中，新增一個帶有 `className` 的 `p` 元素，該 `className` 為 `form-section-title`，並包含文字 `Player Stats`。接著新增一個帶有 `className` 的 `div`，該 `className` 為 `stats-grid`。目前先將統計資料網格留空。

# --hints--

你的 `div.form-panel` 應該包含第二個 `div`，用於玩家統計區塊。
...
玩家統計區塊應包含一個帶有 `className` 的 `p`，該 `className` 為 `form-section-title`，並包含文字 `Player Stats`。
...
玩家統計區塊應包含一個帶有 `className` 的 `div`，該 `className` 為 `stats-grid`。
