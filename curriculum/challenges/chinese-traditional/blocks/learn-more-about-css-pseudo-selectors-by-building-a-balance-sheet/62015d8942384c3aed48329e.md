...
你將你的表格包裹在一個 section 元素中－現在你可以為你的表格設定邊框樣式。創建一個 `section` 選擇器，並將 `max-width` 屬性設為 `40rem` 以實現響應式設計。將 `margin` 屬性設為 `0 auto` 以置中，並將 `border` 屬性設為 `2px solid #d0d0d5`。
