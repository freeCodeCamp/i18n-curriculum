...
Utiliza la misma variable que el `background-color` de las clases`.bb1b`, `.bb1c`, y `.bb1d` para completar el resto del edificio.

# --hints--

El `background-color` del elemento `.bb1b` debe estar establecido.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1b')?.backgroundColor)
```

Debes utilizar `var(--building-color1)` para establecer el `background-color` del elemento `.bb1b`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1b')?.getPropVal('background-color', true), 'var(--building-color1)');
```

El `background-color` del elemento `.bb1c` debe estar establecido.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1c')?.backgroundColor)
```

Debes utilizar `var(--building-color1)` para establecer el `background-color` del elemento `.bb1c`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1c')?.getPropVal('background-color', true), 'var(--building-color1)');
```

El `background-color` del elemento `.bb1d` debe estar establecido.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1d')?.backgroundColor)
```

Debes utilizar `var(--building-color1)` para establecer el `background-color` del elemento `.bb1d`.
