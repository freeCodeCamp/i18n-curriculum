...
現在再生中の曲に属性を追加する必要があります。

条件 `if` を使った `songToHighlight` 文を作成してください。その文の中で、`setAttribute` に対して `songToHighlight` を使い、`"aria-current"` と `"true"` をそれぞれ最初と2番目の引数として渡してください。

# --hints--

条件 `if` を使った `songToHighlight` 文を作成してください。

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

`setAttribute()` 文の中で `songToHighlight` に対して `if` メソッドを使うべきです。

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

`"aria-current"` メソッドの最初と2番目の引数として `"true"` と `setAttribute` を渡すべきです。
