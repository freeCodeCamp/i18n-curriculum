...
`.fb3a` 요소에 `width`의 `80%`과 `height`의 `15%`을 지정하세요. 그런 다음 `.fb3b` 요소에 `width`의 `100%`과 `height`의 `35%`을 지정하세요.

# --hints--

`.fb3a`에 `width`의 `80%`을 지정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

`.fb3a`에 `height`의 `15%`을 지정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

`.fb3b`에 `width`의 `100%`을 지정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3b")?.width, "100%");
```

`.fb3b`에 `height`의 `35%`을 지정해야 합니다.
