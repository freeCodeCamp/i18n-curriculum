...
نفذ خوارزمية واحدة أو أكثر لحساب دالة جاما (في المجال الحقيقي فقط).

يمكن تعريف دالة جاما كما يلي:
...
يجب أن تكون `gamma` دالة.

```js
assert(typeof gamma == 'function');
```

يجب أن تُرجع `gamma(.1)` رقمًا.

```js
assert(typeof gamma(0.1) == 'number');
```

يجب أن تُرجع `gamma(.1)` القيمة `9.513507698668736`.

```js
assert.equal(round(gamma(0.1)), round(9.513507698668736));
```

يجب أن تُرجع `gamma(.2)` القيمة `4.590843711998803`.

```js
assert.equal(round(gamma(0.2)), round(4.590843711998803));
```

يجب أن تُرجع `gamma(.3)` القيمة `2.9915689876875904`.

```js
assert.equal(round(gamma(0.3)), round(2.9915689876875904));
```

يجب أن تُرجع `gamma(.4)` القيمة `2.218159543757687`.

```js
assert.equal(round(gamma(0.4)), round(2.218159543757687));
```

يجب أن تُرجع `gamma(.5)` القيمة `1.7724538509055159`.
