...
Lembre-se de que arrays têm indexação baseada em zero, o que significa que o último índice do array é de `length - 1`.Nossa condição para este loop é `i < arr.length`, que interrompe o loop quando `i` é igual a `length`.Nesse caso a última iteração é `i === 4`, ou seja, quando `i` se tornar igual a `arr.length - 1` e exibir `6` no console.Então `i` aumenta para `5`, e o loop termina porque `i < arr.length` é `false`.
