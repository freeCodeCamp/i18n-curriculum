...
تقبل الطريقة `sort()` دالة استدعاء راجع للمقارنة تُحدد ترتيب الفرز.

في هذا المثال، الشرط الأول `(a.name < b.name)` يتحقق مما إذا كان اسم الفاكهة الأولى أقل من اسم الفاكهة الثانية. إذا كان كذلك، تُفرز الفاكهة الأولى قبل الفاكهة الثانية.

تُقارن السلاسل النصية ترتيبًا معجميًا، أي تُقارن حرفًا بحرف. على سبيل المثال، `"Apples"` أقل من `"Bananas"` لأن `"A"` تأتي قبل `"B"` في الأبجدية.

سبب إرجاع هذا المثال لأرقام هو أن الطريقة `sort()` تتوقع إرجاع رقم. إذا أرجعت رقمًا سالبًا، تُفرز العنصر الأول قبل العنصر الثاني.
...
داخل دالة الاستدعاء الراجع، أضف جملة `if` للتحقق مما إذا كان `a.title` أقل من `b.title`. إذا كان كذلك، أرجع `-1`.

# --hints--

يجب أن تحتوي دالة الاستدعاء الراجع على جملة `if`.

```js
assert.match(code, /if\s*\(/);
```

يجب أن تتحقق جملة `if` مما إذا كان `a.title` أقل من `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
``` 

يجب أن تُرجع جملة `if` القيمة `-1` إذا كان `a.title` أقل من `b.title`.
