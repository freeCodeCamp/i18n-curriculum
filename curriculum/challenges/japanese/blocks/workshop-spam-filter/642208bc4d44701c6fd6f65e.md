...
ドルの値は1桁以上の場合があります。これに対応するために、`+` 量指定子を使うことができます。これは1回以上の連続した出現にマッチします。例えば、正規表現 `/a+/` は1回以上連続した `a` 文字にマッチします。

1回以上連続した数字にマッチするように正規表現を更新してください。

# --hints--

`dollarRegex` には `+` 量指定子を使うべきです。

```js
assert.match(dollarRegex.source, /\+/);
```

`dollarRegex` には `+` 量指定子を `[0-9]` 文字クラスに使うべきです。

```js
assert.match(dollarRegex.source, /\[0-9\]\+/);
```

`dollarRegex` はまだ `"100 dollars"` にマッチするはずです。
...
`dollarRegex` はまだ `"3 dollars"` にマッチするはずです。
...
`dollarRegex`は大文字小文字を区別しない設定にしてください。
