...
Erstellen Sie nun eine Methode namens `calculate_best_price` in Ihrer `DiscountEngine`-Klasse. Diese Methode soll `self`, `product` und `user_tier` als Parameter übernehmen und einen `float` zurückgeben.

Verwenden Sie vorerst einfach `pass` im Methodenrumpf. Die Logik implementieren Sie in den nächsten Schritten.

# --hints--

Ihre `DiscountEngine`-Klasse sollte eine Methode namens `calculate_best_price` haben.
...
Ihre `calculate_best_price`-Methode sollte die Parameter `self`, `product: Product` und `user_tier: str` haben.
...
Ihre `calculate_best_price`-Methode sollte einen Rückgabetyp-Hinweis von `-> float` haben.
