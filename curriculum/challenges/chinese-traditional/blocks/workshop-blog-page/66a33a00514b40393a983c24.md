...
對於第三篇博文，需要添加另一個 `article` 元素。

在 `article` 元素中，添加一個 `h3` 元素，其文本爲 `Mr. Whiskers' First Birthday Party`。

在 `h3` 元素下方，添加兩段 lorem ipsum 文本。

# --hints--

在 `section` 元素中，你應該有第三個 `article` 元素，`id` 設置爲 `posts`。
...
在 `article` 元素中，你應該有一個 `h3` 元素，其文本爲 `Mr. Whiskers' First Birthday Party`。
