...
사용자가 드롭다운에서 도형 선택을 변경할 때 반응할 방법이 필요합니다. 이를 위해 `handleShapeSelect` 함수를 `e` 매개변수와 `Event` 타입으로 만드세요.

이 함수는 모든 입력을 지우고, 기본 동작을 막고, 선택을 검증하며, 도형이 선택되었는지에 따라 결과 카드를 보여주거나 숨기고, 선택한 도형에 맞는 입력 필드를 표시하며, 영역을 다시 계산하는 작업을 처리할 것입니다.

함수는 지금은 비워 두세요.

# --hints--

`handleShapeSelect` 함수를 만들어야 합니다.

```js
assert.isFunction(handleShapeSelect)
```

`handleShapeSelect` 함수는 `e` 매개변수를 `Event` 타입으로 가져야 합니다.
