...
Agrega un atributo `value` de `player.imageUrl` a la URL de la imagen `input`. Luego asigna el atributo `onChange` a una función flecha que toma `e` como parámetro y llama a `setPlayer` con una expansión de `player` y `imageUrl` establecido en `e.target.value`.

# --hints--

Tu URL de imagen `input` debe tener un `value` igual a la URL de la imagen del jugador.
...
Tu URL de imagen `input` debe tener un manejador `onChange` que llame a `setPlayer` con una expansión de `player` y `imageUrl` establecido en `e.target.value`.
