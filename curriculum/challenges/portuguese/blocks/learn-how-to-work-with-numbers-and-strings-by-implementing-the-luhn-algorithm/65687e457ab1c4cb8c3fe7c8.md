...
Ajuste a chamada `verify_card_number` de forma que, se ela retornar `True`, imprima `'VALID!'` no console.Caso contrário, imprima `'INVALID!'`.

# --hints--

Você deve ter `if verify_card_number(translated_card_number):` dentro da função `main`.
...
Você deve ter `print('VALID!')` dentro da declaração `if`.
...
Você deve ter `print('INVALID!')` dentro da cláusula `else`.
