...
title: Asignar el valor de una variable a otra
challengeType: 1
forumTopicId: 418265
...
Lo anterior declara una variable `myVar` sin valor, luego le asigna el valor `5`. A continuación, una variable llamada `myNum` se declara sin valor. Entonces, el contenido de `myVar` (que es `5`) es asignado a la variable `myNum`. Ahora `myNum` también tiene el valor `5`.
