...
أضف تدرجًا متكررًا آخر إلى هذا المبنى؛ اجعله مثل الذي أضفته للتو، لكن لا تضف اتجاه `90deg` واستخدم لون النافذة بدلًا من اللونين `transparent`.

# --hints--

يجب أن تُعطى `.fb6` خاصية `background` تحتوي على `repeating-linear-gradient` ثانية.
...
يجب أن تعطي التدرج الخطي المتكرر الثاني (`repeating-linear-gradient`) اللون الأول `--building-color3` من `0%` إلى `10%`.
...
يجب أن تعطي التدرج الثاني من نوع `repeating-linear-gradient` لونًا ثانيًا هو `--window-color3` من `10%` إلى `30%`.
