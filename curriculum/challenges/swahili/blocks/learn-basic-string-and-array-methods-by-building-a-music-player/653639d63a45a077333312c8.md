...
Kabla ya kuanza kuimba wimbo, unahitaji kuhakikisha unaanza kutoka mwanzo. Hii inaweza kufanikishwa kwa kutumia sifa ya `currentTime` kwenye kitu cha `audio`.

Ongeza tamko la `if` ili ukague kama `userData?.currentSong` ni batili *AU* kama `userData?.currentSong.id` si sawa kabisa na `song.id`. Hali hii itakagua kama hakuna wimbo unaoimbwa kwa sasa au kama wimbo wa sasa ni tofauti na ule unaotarajiwa kuimbwa.

Ndani ya blokki ya `if`, weka sifa ya `currentTime` ya kitu cha `audio` kuwa `0`.

# --hints--

Unapaswa kuunda tamko la `if` lenye hali ya `userData?.currentSong === null || userData?.currentSong.id !== song.id`. Usisahau kujumuisha uunganishaji wa mfululizo.
...
Unapaswa kuweka `audio.currentTime` kuwa `0` ndani ya blokki yako ya `if`.
