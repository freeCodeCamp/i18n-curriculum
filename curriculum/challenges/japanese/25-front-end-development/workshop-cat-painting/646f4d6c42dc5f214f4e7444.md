...
次は右側のひげを適切な位置に移動しましょう。 Use class selector to target the `.cat-whisker-right-top` element and give it a `position` of `absolute`, a `top` of `120px`, and a `left` of `109px`.

# --hints--

`.cat-whisker-right-top` セレクターが必要です。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top'));
```

`.cat-whisker-right-top` セレクターの `position` プロパティを `absolute` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.position, 'absolute');
```

`.cat-whisker-right-top` セレクターの `top` プロパティを `120px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.top, '120px');
```

`.cat-whisker-right-top` セレクターの `left` プロパティを `109px` に設定してください。
