...
你将会<a href="https://colab.research.google.com/github/freeCodeCamp/boilerplate-cat-and-dog-image-classifier/blob/master/fcc_cat_dog.ipynb" target="_blank" rel="noopener noreferrer nofollow">使用 Google Colaboratory 进行此项目的开发</a>。
...
对于本次挑战，你将完成代码以分类狗和猫的图像。你将使用 TensorFlow 2.0 和 Keras 创建一个卷积神经网络，该网络能够至少以 63% 的准确率正确分类猫和狗的图像。（如果你能达到 70% 的准确率，将获得额外加分！）
...
现在轮到你了！正确设置此单元格中的每个变量。（它们不应再相等于 `None`。）
...
对于 `*_data_gen` 变量，使用 `flow_from_directory` 方法。  Pass in the batch size, directory, target size (`(IMG_HEIGHT, IMG_WIDTH)`), class mode, and anything else required. `test_data_gen` will be the trickiest one. 对于 `test_data_gen`，确保将 `shuffle=False` 传递给 `flow_from_directory` 方法。  This will make sure the final predictions stay in the order that our test expects. 对于 `test_data_gen`，观察目录结构也很有帮助。
...
你不需要对这个单元格做任何操作。`train_data_gen` 的创建方式与之前相同，但使用了新的 `train_image_generator`。然后，使用不同的变体对同一张图像绘制了五次。
...
在此单元格中，获取每个测试图像（来自 `test_data_gen`）是狗还是猫的概率。`probabilities` 应该是一个整数列表。 
