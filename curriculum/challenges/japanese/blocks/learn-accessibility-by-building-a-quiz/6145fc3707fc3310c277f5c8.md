...
ページのスタイリングに戻ります。ナビゲーションバー内のリスト要素を選択し、次のスタイルを適用してください。
...
`nav li` または `nav > ul > li` のセレクターを使うべきです。
...
`li` 要素に `color` を `#dfdfe2` で指定してください。
...
`li` 要素に `margin` を `0 0.2rem` で指定してください。
...
`li` 要素に `padding` を `0.2rem` で指定してください。
...
`li` 要素に `display` を `block` で指定してください。
