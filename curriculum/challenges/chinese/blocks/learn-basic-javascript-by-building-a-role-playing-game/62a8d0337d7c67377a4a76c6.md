...
到目前为止，每次更新 `text.innerText` 时，旧文本都会被删除。 这次，使用 `+=` 运算符将文本添加到 `text.innerText` 的末尾。

添加字符串 `" In your inventory you have: "` - 包括开头和结尾的空格。

# --hints--

你应该添加一个带有 `text.innerText` 的新行。
...
你应该使用 `+=` 运算符将其添加到 `text.innerText`。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*\+=\s*/)
```

你应该将字符串 `" In your inventory you have: "` 添加到 `text.innerText` 的末尾。 注意空格！
