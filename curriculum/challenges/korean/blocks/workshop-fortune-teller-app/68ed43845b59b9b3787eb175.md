...
클래스 안에 `constructor`를 추가하세요. 생성자에서, 이전에 만든 요소들을 사용해 `getElement` 함수를 이용해 해당 DOM 요소들을 선택해야 합니다.

지금은 `singleCardBtn`와 `singleCard`를 처리하세요. `singleCardBtn`에는 `#btn-single-card`를 전달하고, `singleCard`에는 `.single_card`를 전달하세요.

# --hints--

`Game` 클래스는 `constructor`를 가져야 합니다.
...
생성자에서는 `this.elements`를 할당해야 합니다.
...
`singleCardBtn`는 `getElement`와 함께 `'#btn-single-card'`를 사용해 설정해야 합니다.
...
`singleCard`는 `getElement`와 함께 `'.single_card'`를 사용해 설정해야 합니다.
