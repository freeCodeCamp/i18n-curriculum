...
Successivamente, crea una sottoclasse chiamata `Bee` che estende la classe `Bug` con il parametro di tipo generico impostato su `string`.

# --hints--

Dovresti avere una classe chiamata `Bee`.
...
La tua classe `Bee` dovrebbe estendere `Bug`.
...
La tua classe `Bee` dovrebbe estendere `Bug` con un argomento di tipo `string`.
