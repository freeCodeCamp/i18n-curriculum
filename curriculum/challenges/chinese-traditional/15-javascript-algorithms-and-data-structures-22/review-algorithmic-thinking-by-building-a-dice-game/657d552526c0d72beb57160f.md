...
在遊戲的這個階段，你可以擲骰子，做出選擇並進行幾輪遊戲。 然而，你應該注意，遊戲沒有結束，並且有無限的輪次。 根據規則，總共應該有六輪遊戲，然後以最終得分結束。

當用戶選擇分數後，運行你的邏輯，你應該檢查是否已經進行了6輪。 如果是這樣，在500毫秒後顯示用戶的最終得分警報。

# --hints--

如果已經進行了六輪，你應該向用戶顯示一個警報。
