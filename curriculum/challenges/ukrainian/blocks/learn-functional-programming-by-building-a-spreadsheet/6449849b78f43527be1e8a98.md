...
Вам потрібно вміти знаходити діапазони клітинок у формулі. Діапазони клітинок можуть виглядати як `A1:B12` або `A3:A25`. Ви можете використати регулярний вираз, щоб знайти ці шаблони.

Почніть з оголошення змінної `rangeRegex` і призначте їй регулярний вираз, який знаходить `A` до `J` (діапазон стовпців у вашій таблиці). Використайте групу захоплення з класом символів для цього.

# --hints--

Ви повинні оголосити змінну `rangeRegex` після вашої функції `idToText`.
...
Ви повинні використати `const` для оголошення змінної `rangeRegex`.
...
Ваша змінна `rangeRegex` має бути регулярним виразом.
...
Ваш `rangeRegex` має використовувати групу захоплення.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(.*\)\/\s*;?/);
```

Ваш `rangeRegex` має використовувати клас символів у групі захоплення.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[.*\]\)\/\s*;?/);
```

Ваш `rangeRegex` має використовувати клас символів, щоб знаходити `A` до `J`.
