...
10 以下的質數總和是 2 + 3 + 5 + 7 = 17。

找出所有小於 `n` 的質數總和。

# --hints--

`primeSummation(17)` 應該傳回一個數字。

```js
assert.isNumber(primeSummation(17));
```

`primeSummation(17)` 應該傳回 41。

```js
assert.strictEqual(primeSummation(17), 41);
```

`primeSummation(2001)` 應該傳回 277050。

```js
assert.strictEqual(primeSummation(2001), 277050);
```

`primeSummation(140759)` 應該傳回 873608362。

```js
assert.strictEqual(primeSummation(140759), 873608362);
```

`primeSummation(2000000)` 應該傳回 142913828922。
