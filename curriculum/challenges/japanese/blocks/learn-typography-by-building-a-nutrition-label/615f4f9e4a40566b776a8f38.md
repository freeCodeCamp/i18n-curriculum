...
新しい`.large`セレクターを作成し、`height`プロパティを`10px`に設定してください。また、`.large, .medium`セレクターを作成し、`background-color`プロパティを`black`に設定してください。

# --hints--

新しい`.large`セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.large'));
```

`.large`セレクターには`height`プロパティが`10px`に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.large')?.height === '10px');
```

新しい`.large, .medium`セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.large, .medium'));
```

`.large, .medium`セレクターには`background-color`プロパティが`black`に設定されているはずです。
