...
回調函數應該爲正則表達式中的每個捕獲組提供一個參數。 在你的情況下，`rangeRegex` 有四個捕獲組：第一個字母、第一個數字、第二個字母和第二個數字。

爲回調函數提供另外四個參數以匹配這些捕獲組：`char1`、`num1`、`char2` 和 `num2`。 `char` 是 `character` 的縮寫。

# --hints--

你的回調函數應該有 `char1` 作爲第二個參數。
