...
讓我們以總結作結。將 `catalog.length` 和 `Object.keys(byDecade).length` 記錄到主控台，以查看書籍總數和年代群組。

宣告 `let oldestYear = Infinity` 和 `let newestYear = 0`。使用 `for` 迴圈遍歷 `catalog`：對於每個年份不是 `"Unknown"` 的項目，如果年份較小則更新 `oldestYear`，如果較大則更新 `newestYear`。將兩個值都記錄出來。

你已完成 Heritage Library Catalog 工作坊！
