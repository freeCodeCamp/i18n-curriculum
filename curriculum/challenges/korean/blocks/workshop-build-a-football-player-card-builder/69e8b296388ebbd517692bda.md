...
Image URL `value`에 `player.imageUrl` 속성 `input`을 추가하세요. 그런 다음 `onChange` 속성을 매개변수로 `e`를 받고 `setPlayer`를 펼친 것과 `player`를 `imageUrl`로 설정하여 `e.target.value`를 호출하는 화살표 함수로 설정하세요.

# --hints--

Image URL `input`에는 플레이어 이미지 URL과 같은 `value`이 있어야 합니다.
...
Image URL `input`에는 `onChange`로 설정된 `setPlayer`와 `player`를 펼쳐서 `imageUrl`를 호출하는 `e.target.value` 핸들러가 있어야 합니다.
