...
編集した変更内容をコンソールで確認できると便利です。

`console.log(currentContent);` を追加してください。

変更をテストするには、ノートを編集してからノートの外をクリックしてみてください。編集したノートがコンソールに表示されるはずです。

# --hints--

`console.log(currentContent);` を `newContent` に代入した後、イベントリスナーの中に `currentContent` を追加してください。
