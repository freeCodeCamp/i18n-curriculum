...
創建一個 `if` 述語，檢查來自 `gameboard` 物件的 `solver()` 方法呼叫是否傳回 `True`。

然後，在 `if` 主體內添加一個 `print()` 呼叫，傳入以下 f-string：`f'Solved puzzle:\n{gameboard}'`。


# --hints--

你應該創建一個 `if` 述語，檢查來自 `gameboard` 物件的 `solver()` 方法呼叫是否傳回 `True`。
...
你應該在新的 `if` 主體內列印 `f'Solved puzzle:\n{gameboard}'`。
