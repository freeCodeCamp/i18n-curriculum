...
У блоці інструкції `if` поверніть об’єкт, який розгортає окремо `defaultPlayer` і `JSON.parse(saved)`. Переконайтеся, що між ними стоїть кома, оскільки це два окремі об’єкти, які об’єднуються в один.

# --hints--

Ваш блок `if (saved)` має повертати об’єкт, який розгортає як `defaultPlayer`, так і проаналізовані збережені дані.
