...
لإعادة قائمة التشغيل إلى حالتها الأصلية، وزع `allSongs` في مصفوفة وعيّنها إلى `userData.songs`.

*ملاحظة*: لا يجب استخدام التسلسل الاختياري لـ `userData.songs` لأن الأغنية لن تكون `null` أو `undefined` في هذه المرحلة.

# --hints--

لا يجب تعديل مستمع الحدث الحالي ومحتواه.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*\)\s*;?/)
```

يجب تعيين `[...allSongs]` إلى `userData.songs`.
