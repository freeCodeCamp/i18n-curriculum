...
最后，让你的正则表达式匹配整个单词（就像你对 `freeRegex` 所做的那样）。

# --hints--

你的 `stockRegex` 应该使用非捕获组。

```js
assert.match(stockRegex.source, /\(\?:/);
```

你的 `stockRegex` 应该使用非捕获组来匹配 `\s` 或 `^`。

```js
assert.match(stockRegex.source, /\(\?:(\^\|\\s|\\s\|\^)\)/);
```

你的 `stockRegex` 应该使用第二个非捕获组。
...
你的 `stockRegex` 应该与 `it's stock alert time` 匹配。

```js
assert.match("it's stock alert time", stockRegex);
```

你的 `stockRegex` 应该与 `stock alert time` 匹配。

```js
assert.match('stock alert time', stockRegex);
```

你的 `stockRegex` 应该与 `it's stock alert` 匹配。

```js
assert.match("it's stock alert", stockRegex);
```

你的 `stockRegex` 应该与 `stock alert` 匹配。

```js
assert.match('stock alert', stockRegex);
```

你的 `stockRegex` 不应匹配 `hands-stock alert time`。

```js
assert.notMatch('hands-stock alert', stockRegex);
```

你的 `stockRegex` 不应匹配 `stock alert-management`。
