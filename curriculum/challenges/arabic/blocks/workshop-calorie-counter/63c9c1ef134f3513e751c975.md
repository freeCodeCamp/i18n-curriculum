...
باتباع نفس النمط، أعلن متغيرات لمُدخَلات `number` في عناصر `#dinner` و `#snacks` و `#exercise`. عيّن لها استدعاءات `getCaloriesFromInputs` المناسبة.

# --hints--

يجب أن تحتوي دالة `calculateCalories` على متغير `dinnerCalories`.

```js
assert.match(calculateCalories.toString(), /dinnerCalories\s*=/);
```

يجب استدعاء `getCaloriesFromInputs` مع `dinnerNumberInputs` كمعلمة.

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*dinnerNumberInputs\s*\)/);
```

يجب تعيين نتيجة `getCaloriesFromInputs` إلى `dinnerCalories`.

```js
assert.match(calculateCalories.toString(), /dinnerCalories\s*=\s*getCaloriesFromInputs\s*\(\s*dinnerNumberInputs\s*\)/);
```

يجب أن تحتوي دالة `calculateCalories` على متغير `snacksCalories`.

```js
assert.match(calculateCalories.toString(), /snacksCalories\s*=/);
```

يجب استدعاء `getCaloriesFromInputs` مع `snacksNumberInputs` كمعلمة.

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*snacksNumberInputs\s*\)/);
```

يجب تعيين نتيجة `getCaloriesFromInputs` إلى `snacksCalories`.

```js
assert.match(calculateCalories.toString(), /snacksCalories\s*=\s*getCaloriesFromInputs\s*\(\s*snacksNumberInputs\s*\)/);
```

يجب أن تحتوي دالة `calculateCalories` على متغير `exerciseCalories`.

```js
assert.match(calculateCalories.toString(), /exerciseCalories\s*=/);
```

يجب استدعاء `getCaloriesFromInputs` مع `exerciseNumberInputs` كمعلمة.

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*exerciseNumberInputs\s*\)/);
```

يجب تعيين نتيجة `getCaloriesFromInputs` إلى `exerciseCalories`.
