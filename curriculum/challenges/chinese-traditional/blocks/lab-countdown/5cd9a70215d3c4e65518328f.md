...
要完成此實驗，你必須使用遞迴來建造結果，且不得使用迴圈（`for`、`while`）、`Array.from()` 或陣列迭代方法（`forEach`、`map`、`filter`、`reduce`）。每次呼叫都應該產生自己的結果陣列（不要使用全域的變數來儲存倒數）。
...
5. 你必須使用遞迴（該函式應該呼叫自身），且不得使用任何形式的迴圈（`for`、`while`、`Array.from()`，或像 `forEach`、`map`、`filter`、`reduce` 這類高階方法）。
...
你的程式碼不應依賴任何類型的迴圈（`for`、`while`、`Array.from()` 或高階函式如 `forEach`、`map`、`filter` 和 `reduce`）。
