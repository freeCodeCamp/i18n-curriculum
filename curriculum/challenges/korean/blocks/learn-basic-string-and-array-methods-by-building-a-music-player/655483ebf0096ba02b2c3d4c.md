...
이전 노래를 가져오려면 `1`의 `currentSongIndex`에서 `userData?.songs`을 빼서 상수 `previousSong`에 할당하세요. 그다음 `playSong` 함수를 호출하고 `previousSong.id`를 인수로 전달하세요.
...
`userData?.songs[currentSongIndex - 1]`을 `previousSong` 상수에 할당해야 합니다.
...
`playSong`을 사용하여 `previousSong.id` 함수를 호출해야 합니다.
