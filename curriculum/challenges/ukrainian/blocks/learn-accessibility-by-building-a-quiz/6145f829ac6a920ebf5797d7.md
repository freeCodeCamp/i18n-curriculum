...
Два фінальні семантичні HTML-елементи для цього проєкту — це елементи `footer` та `address`. Елемент `footer` є контейнером для колекції вмісту, що стосується сторінки, а елемент `address` — контейнером для контактної інформації автора сторінки.

Після елемента `main` додайте один елемент `footer` і вкладіть у нього один елемент `address`.
...
Ви повинні вкласти один елемент `address` всередину елемента `footer`.
