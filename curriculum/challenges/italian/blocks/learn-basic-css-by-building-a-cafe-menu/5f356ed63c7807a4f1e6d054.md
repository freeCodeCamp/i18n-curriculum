...
Puoi utilizzare un selettore di `id` per selezionare uno specifico elemento `div`. Un selettore di `id` si definisce inserendo un simbolo `#` subito prima del valore dell'attributo `id` dell'elemento. Ad esempio, se un elemento ha un `id` con valore `cat` allora puoi selezionare quell'elemento in questo modo:
