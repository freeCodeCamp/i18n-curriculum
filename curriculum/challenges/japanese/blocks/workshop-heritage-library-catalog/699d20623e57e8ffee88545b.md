...
検索が正しく動作するか確認する時間です。Stephen King の本はカタログに6冊あります。

`findByAuthor(catalog, "king")`を呼び出して、その結果を`kingBooks`という変数に代入してください。`kingBooks.length`をログに出力して、件数を確認しましょう。

次に、`for`ループを使って各結果をログに出力してください。テンプレートリテラルを使い、タイトルと年を `` `${kingBooks[i].title} (${kingBooks[i].year})` `` のように含めてください。

# --hints--

`kingBooks`の長さは`6`であるべきです。

```js
assert.strictEqual(kingBooks.length, 6);
```

`kingBooks.length`をコンソールにログ出力してください。

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*kingBooks\.length\s*\)/);
```

各本のタイトルと年をログに出力するには、`for`ループを使うべきです。
