...
Después de tu llamada a `print()`, necesitas filtrar los gastos e imprimir la lista filtrada. Declara una variable `expenses_from_category` y asígnale una llamada a `filter_expenses_by_category` pasando `expenses` y `category` como argumento.

# --hints--

Debes declarar una variable llamada `expenses_from_category` en tu cláusula `elif`.
...
Debes llamar a `filter_expenses_by_category()` pasando `expenses` y `category` como argumentos y asignarlo a tu variable `expenses_from_category`.
...
Tu variable `expenses_from_category` debe ir después de la llamada a `print()`.
