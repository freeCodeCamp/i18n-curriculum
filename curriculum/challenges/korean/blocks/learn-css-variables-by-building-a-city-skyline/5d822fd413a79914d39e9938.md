...
`sky` 클래스에 `radial-gradient`를 주세요. `#ffcf33`에서 `0%`까지 `20%`를, `#ffff66`에 `21%`를, 그리고 `#bbeeff`에 `100%`를 사용하세요. 이렇게 하면 배경에 원형 그라디언트가 추가되어 태양이 됩니다.

# --hints--

`.sky`에 `radial-gradient` 속성으로 `background`를 주어야 합니다.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".sky")?.background, "radial-gradient");
```

`radial-gradient`에 첫 번째 색상으로 `#ffcf33`를 주어야 합니다.

```js
assert.match(new __helpers.CSSHelp(document).getStyle(".sky")?.background, /radial-gradient\(rgb\(255, 207, 51\)( 0%)?, rgb\(255, 207, 51\) 20%/);
```

`radial-gradient`에 두 번째 색상으로 `#ffff66`를 `21%`에 주어야 합니다.
...
`radial-gradient`에 세 번째 색상으로 `#bbeeff`를 `100%`에 주어야 합니다.
