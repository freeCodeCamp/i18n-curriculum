...
داخل عنصر `.image-wrapper`، أعطِ أول عنصر `img` خاصية `src` بقيمة `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`، وخاصية `alt` بقيمة `image of the quote machine project`، وخاصية `class` بقيمة `image-1`، وخاصية `loading` بقيمة `lazy`، وخاصية `width` بقيمة `600`، وخاصية `height` بقيمة `400`.
