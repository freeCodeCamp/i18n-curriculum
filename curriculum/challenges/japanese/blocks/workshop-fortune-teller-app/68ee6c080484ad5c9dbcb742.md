...
`singleCardSelected` という名前のメソッドを作成してください。その中で `hideElements` 関数を呼び出し、次のプロップを渡してください：`singleCardBtn`、`multipleCardsBtn`、`multipleCard`、`text`、および `headerTitle`。

これらすべてには `this.elements` を使ってアクセスする必要があります。

# --hints--

`singleCardSelected` という名前のメソッドを定義してください。
...
`singleCardSelected` の中で `hideElements` 関数を呼び出す必要があります。
...
`singleCardBtn` を `hideElements` 関数に渡してください。
...
`multipleCardsBtn` を `hideElements` 関数に渡してください。
...
`multipleCard` を `hideElements` 関数に渡してください。
...
`text` を `hideElements` 関数に渡してください。
...
`headerTitle` を `hideElements` 関数に渡してください。
