...
Cuando tu condición `if` es verdadera, necesitas intercambiar los dos elementos, "haciendo burbujear" el elemento más grande hacia el final del arreglo.

Para hacer esto, declara una variable `temp` y asígnale el valor de `array[j]`. Luego, asigna a `array[j]` el valor de `array[j + 1]`. Finalmente, asigna a `array[j + 1]` el valor de `temp`.

# --hints--

Dentro de tu sentencia `if`, debes declarar una variable `temp`.
...
Debes asignar a `temp` el valor de `array[j]`.
...
Debes asignar a `array[j]` el valor de `array[j + 1]`.
...
Debes asignar a `array[j + 1]` el valor de `temp`.
