...
Aggiungi un loop (`for`) su `catalog`. All'interno del loop, dichiara `const entry = catalog[i]`. Costruisci una stringa di riga con i quattro campi separati da virgole e inseriscila in `rows`.

I campi stringa devono avere caratteri letterali di virgolette doppie nell'output (ad esempio `"King, Stephen"`), così le virgole all'interno dei valori non rompono il formato CSV. Poiché `year` è un numero, non ne ha bisogno.
...
Ogni riga dovrebbe avere i campi della voce separati da virgole.

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

I campi stringa dovrebbero essere racchiusi tra virgolette doppie.
