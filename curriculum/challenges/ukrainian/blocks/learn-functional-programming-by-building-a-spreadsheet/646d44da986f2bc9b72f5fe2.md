...
Оголосіть `toNumberList` функцію, яка приймає `args` параметр і неявно повертає результат розбиття `args` за комами. Потім додайте ланцюжок виклику `map` методу до вашого `split` методу і передайте `parseFloat` як аргумент методу `map`.

# --hints--

Ви повинні оголосити `toNumberList` змінну.
...
Ви повинні використовувати `const` для оголошення вашої `toNumberList` змінної.
...
Ваша `toNumberList` змінна має бути стрілковою функцією.
...
Ваша `toNumberList` функція має мати `args` параметр.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

Ваша `toNumberList` функція має використовувати неявне повернення.
...
Ваша `toNumberList` функція має повертати результат виклику методу `.split()` у `args`.
...
Ви повинні розбити `args` за символом `","`.
...
Ви повинні додати ланцюжок виклику методу `.map()` до методу `.split()`.
...
Ви повинні передати референс на `parseFloat` як функцію зворотного виклику до `.map()`.
