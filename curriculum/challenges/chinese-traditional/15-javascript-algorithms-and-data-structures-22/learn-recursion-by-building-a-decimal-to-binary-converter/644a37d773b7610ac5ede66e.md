...
現在你已經熟悉了二進制數，現在是時候完成構建函數來爲你執行轉換了。 你將首先從一個更簡單的解決方案開始，然後將其重構爲遞歸解決方案。

首先，你需要創建一些數組來存儲在以下步驟中執行的除法的輸入和結果。 這些將使我們更容易瞭解十進制到二進制的轉換是如何進行的。

從 `decimalToBinary` 函數中刪除 `return` 語句。 然後，聲明名爲 `inputs`、`quotients` 和 `remainders` 的變量，併爲每個變量分配一個空數組。

# --hints--

你應該從 `decimalToBinary` 函數中刪除 `return` 語句。

```js
assert.notMatch(String(decimalToBinary), /return\s*('|"|`)(\s|\d)*\1\s*;?\s*\}$/);
```

你應該聲明一個名爲 `inputs` 的變量。
