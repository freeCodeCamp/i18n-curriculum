...
Defina `font-size` para `1.1rem`, `display` para `block` e `margin` para `auto` para centralizar o botão.

# --hints--

Você deve continuar a direcionar a classe `.submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Defina a propriedade `font-size` para `1.1rem`.
...
Defina a propriedade `margin` como `auto`.
