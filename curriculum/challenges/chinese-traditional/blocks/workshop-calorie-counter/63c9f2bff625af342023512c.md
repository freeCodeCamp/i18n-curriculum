...
To complete this workshop, add an event listener to the `clearButton` button. 當單擊按鈕時，它應該調用 `clearForm` 函數。

恭喜！ 你的項目完成了。

# --hints--

你應該向 `clearButton` 按鈕添加一個事件監聽器。

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(/);
```

你應該將 `"click"` 作爲第一個參數傳遞給 `.addEventListener()`。

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

你應該將 `clearForm` 作爲第二個參數傳遞給 `.addEventListener()`。
