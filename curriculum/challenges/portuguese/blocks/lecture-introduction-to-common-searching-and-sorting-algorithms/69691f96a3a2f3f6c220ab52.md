...
title: O que é busca binária e como ela difere da busca linear?
challengeType: 19
...
Buscar em uma lista de itens é uma ocorrência comum em ciência da computação. Existem dois algoritmos principais que você deve conhecer quando se trata de busca: busca linear e busca binária.
...
Ela continua dividindo as partes restantes da lista em metades até que o valor alvo seja encontrado. Se o valor alvo não estiver na lista, retorna `-1`.
...
Caso contrário, verificamos se o valor no ponto médio é menor que o alvo. Se for, atualizamos o índice `low` para ser o índice do meio mais um. Isso significa que vamos buscar na metade direita da lista.

Por fim, se nenhuma das outras condições for `true`, atualizamos o índice `high` para ser o índice do meio menos um. Isso significa que vamos buscar na metade esquerda da lista.
