...
Тестовий код `kingBooks` підтвердив, що `findByAuthor` працює правильно. Перед тим як перейти до `groupByDecade`, очистіть вихідні дані в консолі, видаливши їх.

Видаліть лінію `const kingBooks = ...`, `console.log(kingBooks.length)` та цикл `for`, які відображають назви книг і роки.

# --hints--

Змінну `kingBooks`, інструкцію `console.log(kingBooks.length)` і цикл `for` слід видалити.
