...
나머지 노래들은 `allSongs` 배열에 추가되어 있습니다.

이전 수업에서 <dfn>Web Audio API</dfn>와 이를 사용해 노래를 재생하는 방법을 배웠습니다. 모든 최신 브라우저는 Web Audio API를 지원하며, 이를 통해 웹 애플리케이션에서 오디오를 생성하고 처리할 수 있습니다.

`audio`이라는 변수를 만들고 `new Audio()`와 같게 설정하세요. 이렇게 하면 새로운 HTML5 `audio` 요소가 생성됩니다.

# --hints--

`audio`라는 변수가 있어야 합니다.

```js
assert.isDefined(audio);
```

`Audio`라는 변수에 `audio` 객체의 새 인스턴스를 할당해야 합니다.
