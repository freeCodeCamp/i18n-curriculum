...
Nach der `if`-Anweisung in Ihrer `__str__`-Methode erstellen Sie zwei Variablen mit den Namen `movies` und `series` und weisen ihnen jeweils einen Aufruf der `get_movies`-Methode und der `get_tv_series`-Methode zu.

# --hints--

Sie sollten eine Variable namens `movies` in Ihrer `__str__`-Methode haben.
...
Sie weisen `self.get_movies()` Ihrer `movies`-Variable zu.
...
Sie sollten eine Variable namens `series` in Ihrer `__str__`-Methode haben.
...
Sie weisen `self.get_tv_series()` Ihrer `series`-Variable zu.
