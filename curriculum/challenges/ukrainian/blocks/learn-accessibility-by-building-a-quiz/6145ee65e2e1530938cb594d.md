...
Фінальний розділ цього тесту міститиме випадаючий список і текстове поле.

Почніть із вкладення `div` з `class` типу `formrow`, і всередині нього вкладіть чотири елементи `div`, чергуючи їхні атрибути `class` між `question-block` та `answer`.

# --hints--

Ви повинні додати один елемент `div` всередину фінального `section`.

```js
assert.exists(document.querySelector('section:nth-of-type(3) > div'));
```

Ви повинні надати першому `div` атрибут `class` зі значенням `formrow`.
