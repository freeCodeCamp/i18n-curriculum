...
بعد ذلك، تريد توسيط العنصر `#menu` أفقيًا. يمكنك فعل ذلك بتعيين خاصيتي `margin-left` و`margin-right` إلى `auto`. فكر في الهامش كمساحة غير مرئية حول العنصر. باستخدام هاتين الخاصيتين، وسّط عنصر `#menu` داخل عنصر `body`.

# --hints--

يجب تعيين خاصية `margin-left` إلى `auto`.
...
يجب تعيين خاصية `margin-right` إلى `auto`.
...
يجب تعيين خاصيتي `margin-left` و`margin-right` لعنصر `#menu` إلى `auto`.
