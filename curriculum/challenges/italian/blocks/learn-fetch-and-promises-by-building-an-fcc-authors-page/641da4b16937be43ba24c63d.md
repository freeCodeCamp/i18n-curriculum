...
Concatena un altro `.then()` al metodo `.then()` esistente. Questa volta, passa `data` come parametro per la funzione di callback. Per la callback, usa le parentesi graffe perché avrai più di un'espressione. All'interno della tua funzione di callback, stampa `data` nella console per vedere com'è fatto.
...
Il tuo secondo metodo `.then()` dovrebbe stampare `data` nella console.
