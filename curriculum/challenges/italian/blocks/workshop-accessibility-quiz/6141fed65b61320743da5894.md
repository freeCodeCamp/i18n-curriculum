...
Per aumentare l'accessibilità della pagina, l'attributo `role` può essere usato per indicare lo scopo di un elemento sulla pagina alle tecnologie assistive. L'attributo `role` fa parte della _Web Accessibility Initiative_ (WAI) e accetta valori preimpostati.

Assegna a ciascuno degli elementi `section` il ruolo `region`.
