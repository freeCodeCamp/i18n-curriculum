...
سيؤدي هذا إلى إنشاء تأثير Hover فقط لعنصر `a` داخل `.author-name`، مع إظهار لون freeCodeCamp الأخضر الأصلي في الخلفية.

# --hints--

يجب أن يكون لديك منتقي `.author-name a:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

منتقي `.author-name a:hover` يجب ان يحتوي علي خاصية `background-color` بقيمة `#306203`.
