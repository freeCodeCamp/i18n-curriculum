...
Para corrigir o erro, crie uma função arrow `handlePowersChange` com um parâmetro `e`.

Dentro da função, desestruture `value` e `checked` de `e.target` para obter o valor da checkbox e se ela está marcada ou não.

# --hints--

Você deve criar uma função `handlePowersChange` com um parâmetro `e`.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

Você deve desestruturar `value` e `checked` de `e.target`.
