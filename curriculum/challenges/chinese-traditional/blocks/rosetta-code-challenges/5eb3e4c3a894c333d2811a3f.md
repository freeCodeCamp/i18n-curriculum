...
`SHA-2` 系列是比 `SHA-1` 更強大的替代方案。它們之間的主要差異在於雜湊值的長度。  
也就是說，`SHA-1` 提供較短的程式碼，且獨特組合的可能性較少。`SHA-2` 或 `SHA-256` 則產生較長且更複雜的雜湊值，具有更多的可能性。

# --instructions--

研究實作細節並撰寫一個以字串作為參數並傳回使用 `SHA-256` 的雜湊值的函式。
...
`SHA256("Rosetta code")` 應該傳回一個字串。

```js
assert(typeof SHA256('Rosetta code') === 'string');
```

`SHA256("Rosetta code")` 應該傳回 `"764faf5c61ac315f1497f9dfa542713965b785e5cc2f707d6468d7d1124cdfcf"`。
...
`SHA256("SHA-256 Hash")` 應該傳回 `"bee8c0cabdcf8c7835f40217dd35a8b0dba9134520e633f1c57285f35ca7ee3e"`。
...
`SHA256("implementation")` 應該傳回 `"da31012c40330e7e21538e7dd57503b16e8a0839159e96137090cccc9910b171"`。
...
`SHA256("algorithm")` 應該傳回 `"b1eb2ec8ac9f31ff7918231e67f96e6deda83a9ff33ed2c67443f1df81e5ed14"`。
...
`SHA256("language")` 應該傳回 `"a4ef304ba42a200bafd78b046e0869af9183f6eee5524aead5dcb3a5ab5f8f3f"`。
