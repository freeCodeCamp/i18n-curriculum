...
その後、`if`ブロックの中で、変数`user_tier`を作成し、文字列`Premium`を代入してください。次に、`strategies`、`PercentageDiscount(10)`、`FixedAmountDiscount(5)`、`PremiumUserDiscount()`を含む戦略のリストを変数`strategies`として作成してください。
...
`strategies`、`PercentageDiscount(10)`、`FixedAmountDiscount(5)`、`PremiumUserDiscount()`を含むリストを作成してください。
