...
在 `oninput` 上使用 `shapeTypeSelect` 事件並將其設為 `handleShapeSelect`。這樣每次形狀選擇改變時，函式就會執行。

完成後，你應該能看到每個形狀的輸入欄位。但計算功能還無法運作，接下來你將處理這部分。

# --hints--

你應該將 `shapeTypeSelect.oninput` 設為 `handleShapeSelect`。
