...
Aggiungi all'oggetto un'ultima proprietà chiamata `text`. Dare a questa proprietà lo stesso valore stringa di quello assegnato a `text.innerText` nella funzione `goTown`.

# --hints--

Il primo valore di `locations` dovrebbe avere una proprietà `text`.

```js
assert.isDefined(locations[0]["text"]);
```

La proprietà `text` dovrebbe essere una stringa.
