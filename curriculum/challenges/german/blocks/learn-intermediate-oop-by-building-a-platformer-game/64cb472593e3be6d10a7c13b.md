...
Inside your condition, add a `forEach` loop to iterate through the `checkpoints` array. Verwende `checkpoint` als Parameternamen für die Callback-Funktion.
...
Du solltest einen `checkpoint`-Parameter innerhalb deiner `forEach`-Callbackfunktion haben.
