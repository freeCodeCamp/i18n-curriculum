...
現在、`songsHTML` は配列です。このまま表示しようとすると、曲がカンマで区切られて表示されます。これは望ましい結果ではありません。曲をリストとして表示したいからです。これを修正するには、<dfn>join()</dfn> メソッドを使って配列を1つの文字列に結合する必要があります。

`join()` メソッドは、配列のすべての要素を1つの文字列に連結するために使われます。オプションのパラメータとして `separator` を受け取り、これは配列の各要素を区切るために使われます。例えば：
...
`join()` メソッドを `map()` メソッドにチェーンし、区切り文字として空文字列を渡してください。

複数のメソッドをチェーンするには、`join()` メソッドの結果に対して `map()` メソッドを呼び出せばよいです。例えば：
