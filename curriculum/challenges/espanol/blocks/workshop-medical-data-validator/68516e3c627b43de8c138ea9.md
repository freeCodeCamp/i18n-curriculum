...
La clave `last_visit_id` de tu diccionario `constraints` debe tener el valor de `isinstance(last_visit_id, str) and re.fullmatch('v\d+', last_visit_id, re.IGNORECASE)`.
