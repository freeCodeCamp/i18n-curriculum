...
在最後一個 `.divider` 之後，創建另一個 `p` 元素，其中包含文本 `Trans Fat 0g`。 通過將單詞 `Trans` 包裝在 `i` 元素中，將其變爲斜體。 將新的 `p` 元素的 `class` 屬性設置爲 `indent no-divider`。 將 `Trans Fat 0g` 放在一個 `span` 元素中，以對齊它。
