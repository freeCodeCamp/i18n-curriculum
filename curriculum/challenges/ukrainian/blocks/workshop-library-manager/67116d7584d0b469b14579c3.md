...
У попередніх уроках ви навчились працювати з методом `filter()`, який використовується для повернення нового масиву відфільтрованих результатів.
...
Створіть функцію `getBooksByAuthor` з двома параметрами — масивом об’єктів книг і рядком з ім’ям автора.

Функція має повертати масив, що містить книги певного автора.

# --hints--

Ви повинні створити функцію `getBooksByAuthor`.

```js
assert.isFunction(getBooksByAuthor);
```

Ваша функція `getBooksByAuthor` має мати два параметри.

```js
assert.lengthOf(getBooksByAuthor, 2);
```

Ваша функція `getBooksByAuthor` має повертати масив.

```js
assert.isArray(getBooksByAuthor(library, ''));
```

Ваша функція `getBooksByAuthor` має повертати правильну кількість книг для будь-якого з авторів.
