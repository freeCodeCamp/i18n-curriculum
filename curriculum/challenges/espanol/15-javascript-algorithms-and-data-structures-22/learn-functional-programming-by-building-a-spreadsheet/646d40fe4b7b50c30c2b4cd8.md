...
Tu función `infixEval` solo evaluará la primera operación de multiplicación o división, porque el `regex` no es global. Esto significa que querrás usar una aproximación recursiva para evaluar la cadena completa.

Si `infixEval` no encuentra coincidencias, devolverá el valor de `str` tal cual. Usando una expresión ternaria, verifica si `str2` es igual a `str`. Si lo es, devuelve `str`, de lo contrario devuelve el resultado de llamar a `highPrecedence()` en `str2`.

# --hints--

Tu función `highPrecedence` debe usar la palabra clave `return`.
...
Debes usar la palabra clave `return` con una condición para verificar si `str` es igual a `str2`.
...
Debes usar la sintaxis ternaria con tu declaración `return`.
...
Si la condición ternaria es verdadera, debes devolver `str`.
...
Si la condición ternaria es falsa, debes devolver el resultado de llamar a `highPrecedence()`.
...
Debes pasar `str2` a tu llamada `highPrecedence()`.
