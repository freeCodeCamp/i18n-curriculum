...
기존 `for` 반복문을 한 단계 들여쓰기하여 카탈로그에 영화가 있을 때 실행되도록 하세요. 그런 다음 `self.items`을 `movies`로 바꿔 영화만 순회하도록 하세요.

# --hints--

기존 `for` 반복문을 한 단계 들여쓰기하고 `self.items`를 `movies`로 바꿔 영화만 순회하세요.
...
`self.items`에 `Movie('Barry Lyndon', 1975, 'Stanley Kubrick', 184)`과 `Movie('Fahrenheit 451', 1966, 'Francois Truffaut', 112)`이 포함되어 있으면 `__str__` 메서드는 `Media Catalogue (2 items):\n\n=== MOVIES ===\n1. Barry Lyndon (1975) - 184 min, Stanley Kubrick\n2. Fahrenheit 451 (1966) - 112 min, Francois Truffaut\n`을 반환해야 합니다.
