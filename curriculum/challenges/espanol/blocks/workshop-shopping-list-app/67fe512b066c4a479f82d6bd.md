...
Dentro del elemento `li`, encima del texto `item`, agrega un elemento `input` con el atributo `type` configurado como `checkbox`. También, establece su atributo `onChange` en una función anónima que llame a `toggleItem()` con `item` como argumento.

Finalmente, envuelve el `input` y el texto `item` en un elemento `label` para mejorar la accesibilidad.

# --hints--

Debes agregar otro elemento `input` con el atributo `type` configurado como `checkbox`.
...
El nuevo `input` debe tener un atributo `onChange` configurado en una función anónima que llame a `toggleItem()` con `item` como argumento.
...
El nuevo `input` y el texto `item` deben estar envueltos en un elemento `label`.
