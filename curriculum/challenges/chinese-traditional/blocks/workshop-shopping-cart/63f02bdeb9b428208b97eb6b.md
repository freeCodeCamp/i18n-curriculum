...
你的 `calculateTaxes` 方法應該傳回 `taxRate`（除以 100，以轉換成百分比）乘以 `amount` 參數的值。

為了清楚起見，將 `taxRate / 100` 計算包在小括弧中。

# --hints--

你應該在你的 `calculateTaxes` 方法中將 `taxRate` 除以 100。記得使用 `this` 關鍵字。
...
你應該將 `this.taxRate / 100` 的計算包在小括弧中。
...
你應該在你的 `calculateTaxes` 方法中將 `amount` 參數乘以 `taxRate` 除以 100。
