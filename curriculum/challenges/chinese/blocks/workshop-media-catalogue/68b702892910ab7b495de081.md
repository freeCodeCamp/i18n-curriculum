...
你需要一种合适的方式来直观表现目录。所以在你的 `__str__` 类中创建一个 `MediaCatalogue` 方法。

如果 `items` 列表为空，确保 `__str__` 方法返回 `Media Catalogue (empty)`。

# --hints--

你的 `__str__` 类中应该有一个 `MediaCatalogue` 方法。
...
当 `__str__` 为空时，你的 `Media Catalogue (empty)` 方法应返回 `self.items`。
...
你的 `__str__` 方法**不应**在 `Media Catalogue (empty)` **不**为空时返回 `self.items`。
