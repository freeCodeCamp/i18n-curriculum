...
添加另一个情况，当 `results` 的长度为 `2` 时。  This time, assign `result_list` a list containing two strings with the format `x1 = <root1>` and `x2 = <root2>`. 再次，使解决方案同时显示正号和负号。
...
你应该在现有的 `case` 块之后创建一个具有模式 `2` 的新 `case`。
...
你应该在你的新 `case` 体内将一个包含两个字串的列表赋值给 `result_list`，字串格式为 `x1 = <root1>` 和 `x2 = <root2>`。结果应显示正负号。
