...
Jetzt ist es Zeit, Ihre Funktionen zu verwenden. Deklarieren Sie eine neue `processIngredient`-Funktion, die vier Argumente akzeptiert: `baseQuantity`, `baseUnit`, `newUnit` und `newServings`.

Ihre Funktion soll die Portionen für die Basis-Menge an die neuen Portionen anpassen, diese angepasste Menge von der Basiseinheit in die neue Einheit umrechnen und die umgerechnete Menge als String mit zwei Dezimalstellen zurückgeben.

Wenn Sie nicht weiterkommen, sehen Sie sich die Beispielaufrufe Ihrer Funktion an, die Sie protokolliert haben. Sie können die Werte in beiden Beispielen ändern, um zu experimentieren.

# --hints--

Sie sollten eine `processIngredient`-Funktion deklarieren.

```js
assert.isFunction(processIngredient);
```

Ihre `processIngredient`-Funktion sollte `baseQuantity` als ersten Parameter haben.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity/);
```

Ihre `processIngredient`-Funktion sollte `baseUnit` als zweiten Parameter haben.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit/);
```

Ihre `processIngredient`-Funktion sollte `newUnit` als dritten Parameter haben.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit/);
```

Ihre `processIngredient`-Funktion sollte `newServings` als vierten Parameter haben.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit,\s*newServings\)/);
```

Ihre `processIngredient`-Funktion sollte einen String zurückgeben.

```js
assert.isString(processIngredient(10, "gram", "cup", 5));
```

Ihre `processIngredient`-Funktion sollte den korrekten Wert zurückgeben.
