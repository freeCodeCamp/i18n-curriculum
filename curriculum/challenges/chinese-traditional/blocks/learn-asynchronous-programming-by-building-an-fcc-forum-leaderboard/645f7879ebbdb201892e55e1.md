...
首先建立一個名為 `userAvatarUrl` 的常數。然後使用三元運算子來檢查 `avatar` 是否以 `"/user_avatar/"` 開頭。 

如果是這樣，使用 `concat` 方法將 `avatar` 串接到 `avatarUrl`。否則傳回 `avatar`。

這將確保頭像 URL 無論是相對還是絕對 URL 都能正確形成。

# --hints--

你應該有一個名為 `userAvatarUrl` 的常數。 

```js
assert(code.match(/const\s+userAvatarUrl\s*=/));
```

你的 `ternary` 應該檢查 `avatar` 是否以 `"/user_avatar/"` 開頭，並傳回 `avatarUrl.concat(avatar)` 或 `avatar`。
