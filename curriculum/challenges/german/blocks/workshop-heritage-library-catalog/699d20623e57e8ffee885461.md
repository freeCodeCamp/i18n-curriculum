...
Dasselbe Muster gilt für die anderen drei Felder. Fügen Sie entsprechende Prüfungen für `author`, `year` und `location` hinzu.

Beachten Sie, dass `isValid` sich über alle vier Prüfungen hinweg aufsummiert: Wenn ein Feld fehlschlägt, wird es zu `false` und bleibt `false`, selbst wenn die übrigen Prüfungen bestehen. Das bedeutet, die Funktion überprüft immer jedes Feld, anstatt beim ersten Problem abzubrechen.

# --hints--

`validateEntry(catalog[0])` sollte `true` zurückgeben.

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` sollte `false` zurückgeben.

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` sollte `false` zurückgeben.

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` sollte `false` zurückgeben, weil der Ort `"Unknown"` ist.
