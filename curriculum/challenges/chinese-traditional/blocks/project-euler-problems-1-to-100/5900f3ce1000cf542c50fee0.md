...
第一個已知超過一百萬位數的質數是在 1999 年被發現的，它是一個形如 $2^{6972593} − 1$ 的梅森質數；它正好包含 2,098,960 位數。隨後，其他形如 $2^p − 1$ 的梅森質數也被發現，且包含更多位數。

然而，在 2004 年發現了一個巨大的非 Mersenne 質數，包含 2,357,207 個數字：$28433 × 2^{7830457} + 1$。

找出該非梅森質數以 $multiplier × 2^{power} + 1$ 表單表示時的最後 10 位數字。

# --hints--

`largeNonMersennePrime(19, 6833086)` 應該傳回一個字串。

```js
assert(typeof largeNonMersennePrime(19, 6833086) === 'string');
```

`largeNonMersennePrime(19, 6833086)` 應該傳回字串 `3637590017`。

```js
assert.strictEqual(largeNonMersennePrime(19, 6833086), '3637590017');
```

`largeNonMersennePrime(27, 7046834)` 應該傳回字串 `0130771969`。

```js
assert.strictEqual(largeNonMersennePrime(27, 7046834), '0130771969');
```

`largeNonMersennePrime(6679881, 6679881)` 應該傳回字串 `4455386113`。

```js
assert.strictEqual(largeNonMersennePrime(6679881, 6679881), '4455386113');
```

`largeNonMersennePrime(28433, 7830457)` 應該傳回字串 `8739992577`。
