...
問題編號的描述不夠清楚。這對於視障使用者尤其如此。解決此類問題的一種方法，是在不必為元素添加可見文字的情況下，僅添加螢幕閱讀器能讀取的文字。

在每個 `h3` 元素中的數字前，巢狀一個具有 `sr-only` `class` 的 `span` 元素。

# --hints--

你應該在第一個 `h3` 元素內添加一個 `span` 元素。

```js
assert.exists(document.querySelector('h3 > span'));
```

你應該為第一個 `span` 元素指定一個 `class` 為 `sr-only`。

```js
assert.equal(document.querySelector('span')?.className, 'sr-only');
```

你應該在第二個 `h3` 元素中添加一個 `span` 元素。

```js
assert.exists(document.querySelectorAll('h3 > span')[1]);
```

你應該給第二個 `span` 元素一個 `class` 為 `sr-only`。

```js
assert.equal(document.querySelectorAll('span')[1]?.className, 'sr-only');
```

你應該在第一個 `h3` 元素中數字 1 之前添加 `span` 元素。

```js
assert.equal(document.querySelector('span')?.nextSibling?.textContent.trim(), '1');
```

你應該在第二個 `h3` 元素中數字 2 之前添加 `span` 元素。
