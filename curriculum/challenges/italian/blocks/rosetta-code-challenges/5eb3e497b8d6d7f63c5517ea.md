...
title: Cercare in una lista di record
challengeType: 1
forumTopicId: 385315
...
Un record è composto da attributi che descrivono un'entità. Ogni attributo ha un nome e un valore. Per esempio, una persona può avere un attributo `age` con un valore di 25. Un'operazione importante su una lista di record è trovare un record con un valore specifico di un attributo.

# --instructions--

Scrivi una funzione che prende una stringa come parametro. La funzione dovrebbe restituire l'indice dell'elemento in `list` per cui il valore dell'attributo `name` corrisponde alla stringa data.
