...
Poiché gli utenti potranno mescolare ed eliminare canzoni dalla playlist, dovrai creare una copia dell'array `allSongs` senza modificare l'originale. Qui entra in gioco l'operatore `spread`.

L'operatore di espansione (`...`) ti permette di copiare tutti gli elementi da un array in un altro. Può anche essere usato per concatenare più array in uno solo. Nell'esempio qui sotto, sia `arr1` che `arr2` sono stati espansi in `combinedArr`:
...
All'interno dell'oggetto `userData` crea una proprietà `songs`. Per il valore, espandi `allSongs` in un array.

# --hints--

Il tuo oggetto `userData` dovrebbe avere una chiave chiamata `songs`.

```js
assert.property(userData, "songs");
```

La tua proprietà `songs` dovrebbe avere un valore pari a `[...allSongs]`.
