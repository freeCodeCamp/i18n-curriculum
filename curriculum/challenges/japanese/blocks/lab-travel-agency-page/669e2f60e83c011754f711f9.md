...
1. `head` 要素の中に、`charset` が `utf-8` に設定された `meta` 空要素と、テキスト `Travel Agency Page` を含む `title` が必要です。
1. `head` 要素内に、SEOのためにウェブサイトの簡単な説明を含む `meta` タグが必要です。
1. 旅行先を紹介するための `h1` 要素が必要です。
1. `h1` 要素の下に、旅行の機会を紹介する段落が必要です。
1. テキスト `Packages` を含む `h2` 要素が必要です。
...
1. テキスト `Top Itineraries` を含む `h2` 要素が必要です。
1. それぞれアンカー要素と `figcaption` 要素を含む少なくとも3つの `figure` 要素が必要です。
1. 3つのアンカー要素は、適切な `alt` 属性を持つ `img` 要素と、有効な画像を示す `src` 属性を持つ必要があります。必要に応じて `https://cdn.freecodecamp.org/curriculum/labs/colosseo.jpg`、`https://cdn.freecodecamp.org/curriculum/labs/alps.jpg`、`https://cdn.freecodecamp.org/curriculum/labs/sea.jpg` を使用できます。
1. すべての5つのアンカー要素は、値が `https://www.freecodecamp.org/learn` の `href` 属性と、値が `_blank` の `target` 属性を持つ必要があります。
