...
Tabellensoftware verwendet in der Regel `=` am Anfang einer Zelle, um anzuzeigen, dass eine Berechnung verwendet werden soll, und dass Tabellenfunktionen ausgewertet werden sollen.

Use the `&&` operator to add a second condition to your `if` statement that also checks if the first character of `value` is `"="`.

# --hints--

You should use the `&&` operator to add a second condition to your `if` statement that also checks if the first character of `value` is `"="`. Du kannst `[0]`, `.startsWith()` oder `.charAt(0)` verwenden.
