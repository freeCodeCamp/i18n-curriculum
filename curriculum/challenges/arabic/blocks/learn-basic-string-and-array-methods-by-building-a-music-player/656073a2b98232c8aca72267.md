...
للتعامل مع معلومات الأغنية الحالية وتتبع وقت تشغيلها، أنشئ خاصيتي `currentSong` و`songCurrentTime`. عيّن القيم إلى `null` و`0` على التوالي.

# --hints--

لا تعدل كائن `userData` الحالي ومحتواه.

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

يجب أن يحتوي كائن `userData` على مفتاح `currentSong` بقيمة `null`.

```js
assert.propertyVal(userData, "currentSong", null)
```

يجب أن يحتوي كائن `userData` على مفتاح `songCurrentTime` بقيمة `0`.
