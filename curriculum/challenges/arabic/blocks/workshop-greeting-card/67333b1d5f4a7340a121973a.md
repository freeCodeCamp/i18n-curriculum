...
أضف محدِّدًا جديدًا يستهدف عناصر `.card-links a` عند وجود تركيز عليها. عيّن الخاصية `outline` إلى `2px solid yellow`.

# --hints--

يجب أن يكون لديك محدِّد `.card-links a:focus`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:focus"));
```

يجب أن يحتوي محدِّد `.card-links a:focus` على الخاصية `outline` مع القيمة `2px solid yellow`.
