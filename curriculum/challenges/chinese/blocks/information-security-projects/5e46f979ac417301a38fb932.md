...
你将使用我们的 Ona 公式化代码<a href="https://app.ona.com/?autostart=true#https://github.com/freeCodeCamp/boilerplate-port-scanner" target="_blank" rel="noopener noreferrer nofollow">来完成这个项目</a>。
...
在 `port_scanner.py` 文件中，创建一个名为 `get_open_ports` 的函数，该函数接受一个 `target` 参数和一个 `port_range` 参数。`target` 可以是 URL 或 IP 地址。`port_range` 是一个包含两个数字的列表，表示要查看的端口范围的起始和结束端口号。
