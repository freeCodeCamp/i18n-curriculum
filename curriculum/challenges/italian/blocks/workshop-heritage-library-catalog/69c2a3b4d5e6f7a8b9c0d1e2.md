...
Il codice di test `kingBooks` ha confermato che `findByAuthor` funziona correttamente. Prima di passare a `groupByDecade`, pulisci l'output della console rimuovendolo.

Rimuovi la riga `const kingBooks = ...`, `console.log(kingBooks.length)` e il loop `for` che mostra i titoli e gli anni dei libri.

# --hints--

La variabile `kingBooks`, l'istruzione `console.log(kingBooks.length)` e il loop `for` devono essere tutti rimossi.
