...
使用三元運算子將你的 `.hasOwnProperty()` 呼叫轉換成條件。若該物件具有該屬性，傳回以 `fn` 和 `args` 作為引數呼叫 `apply` 的結果。否則，傳回 `match`。
...
如果三元條件為真，你的回呼函式（程式）應該傳回呼叫 `apply()` 的結果。
