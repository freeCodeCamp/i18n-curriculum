...
В межах другого `tr` додайте елемент `th` із текстом `Expenses Annual anticipated expenses, such as payroll.`. Wrap that text, except for `Expenses`, within a `span` element with the `class` set to `description`.

Додайте під ним три елементи `td` та надайте їм такий текст, за порядком: `$200`, `$300` та `$400`. Третьому елементу `td` потрібно надати `class` зі значенням `current`.
...
Ваш елемент `th` повинен мати текст `Expenses Annual anticipated expenses, such as payroll.`.
...
Ви повинні обгорнути текст `Annual anticipated expenses, such as payroll.` в елемент `span`.
