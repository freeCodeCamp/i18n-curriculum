...
أنشئ متغيرًا آخر باسم `movie2` وقم بتعيين كائن من نوع `Movie` تم إنشاؤه باستخدام المعلمات التي تختارها. ثم أضف `movie2` إلى الكتالوج.

# --hints--

يجب أن يكون لديك متغير باسم `movie2`.
...
يجب تعيين نسخة من `Movie` إلى المتغير `movie2`.
...
يجب أن يحتوي كتلة `try` على `catalogue.add(movie2)`.
