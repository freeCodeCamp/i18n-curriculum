...
2番目の `tabpanel` 要素に、値が `"panel-saturn"` の `id` 属性と、値が `"tab-saturn"` の `aria-labelledby` 属性を追加してください。

3番目の `tabpanel` 要素に、値が `"panel-mars"` の `id` 属性と、値が `"tab-mars"` の `aria-labelledby` 属性を追加してください。
