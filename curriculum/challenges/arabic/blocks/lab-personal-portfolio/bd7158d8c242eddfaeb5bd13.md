...
2. يجب أن يحتوي قسم الترحيب على عنصر `h1` يتضمّن نصًّا.
3. يجب أن يحتوي معرض الأعمال على قسم مشاريع يحمل `id` بقيمة `project-section`.
4. يجب أن يحتوي قسم المشاريع على عنصر واحد على الأقل يحمل `class` بقيمة `project-tile` لاستيعاب مشروع.
5. يجب أن يحتوي قسم المشاريع على رابط واحد على الأقل يؤدي إلى مشروع.
6. يجب أن يحتوي معرض الأعمال على شريط تنقل يحمل id بقيمة `navbar`.
7. يجب أن يحتوي شريط التنقل على رابط واحد على الأقل يمكنك النقر عليه للانتقال إلى أقسام مختلفة من الصفحة.
8. يجب أن يحتوي معرض الأعمال على رابط يحمل id بقيمة `profile-link`، يفتح ملفك الشخصي على GitHub أو freeCodeCamp في علامة تبويب جديدة.
9. يجب أن يحتوي معرض الأعمال على استعلام وسائط واحد على الأقل.
10. يجب أن يساوي ارتفاع قسم الترحيب ارتفاع إطار العرض.
11. يجب أن يبقى شريط التنقل دائمًا في أعلى إطار العرض.

**ملاحظة:** تأكد من ربط ورقة الأنماط الخاصة بك في ملف HTML وتطبيق CSS الخاص بك.
...
يجب أن يحتوي معرض أعمالك على قسم "مشاريع" بـ `id` يساوي `project-section`.
...
يجب أن يحتوي معرض أعمالك على عنصر واحد على الأقل بصنف `project-tile`.
...
يجب أن يحتوي عنصر `#project-section` على عنصر `a` واحد على الأقل.
