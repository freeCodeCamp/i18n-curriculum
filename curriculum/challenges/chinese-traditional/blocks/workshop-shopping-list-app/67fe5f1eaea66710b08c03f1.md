...
現在你將編寫一些邏輯來劃掉列表中任何已選取的項目。

首先，因為這個 `filteredItems.map()` 呼叫內的所有內容都是隱式傳回的，請將其中所有程式碼用大括號包起來，並明確傳回 `li` 元素。

例如，這裡示範如何使用一個簡單的組件來呈現一個名字列表：
...
你應該將程式碼包裹在 `filteredItems.map()` 呼叫的花括號內。
