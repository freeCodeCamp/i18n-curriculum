...
`while` ループ内で次の3つの文字列 `'3. Show total expenses'`、`'4. Filter expenses by category'`、`'5. Exit'` を出力して、他のメニューオプションを表示してください。`print()` の呼び出しは順番に追加し続けてください。

# --hints--

`print('3. Show total expenses')` ループ内に `while` があるはずです。
...
`print('4. Filter expenses by category')` ループ内に `while` があるはずです。
...
`print('5. Exit')` ループ内に `while` があるはずです。
