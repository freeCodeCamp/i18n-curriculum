...
创建另一个 `p` 元素，给它添加文本 `Calcium 260mg 20%`。 将 `20%` 向右对齐。 在其下方，创建一个带有文本 `Iron 8mg 45%` 的 `p` 元素，将 `45%` 对齐到右侧。
...
你的第一个新的 `p` 元素应包含文本 `Calcium 260mg 20%`。
