...
Geben Sie Ihrem `conversionTable` eine `gram`-Eigenschaft und weisen Sie ihr ein Objekt zu, das eine `cup`-Eigenschaft mit dem Wert `1/240`, eine `ounce`-Eigenschaft mit dem Wert `0.0353` und eine `teaspoon`-Eigenschaft mit dem Wert `0.2` enthält.

# --hints--

Ihr `conversionTable` sollte eine `gram`-Eigenschaft haben.

```js
assert.property(conversionTable, "gram");
```

Ihre `gram`-Eigenschaft sollte ein Objekt sein.

```js
assert.isObject(conversionTable.gram);
```

Ihr `gram`-Objekt sollte eine `cup`-Eigenschaft haben.

```js
assert.property(conversionTable.gram, "cup");
```

Ihre `cup`-Eigenschaft sollte auf `1/240` gesetzt sein.

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

Ihr `gram`-Objekt sollte eine `ounce`-Eigenschaft haben.

```js
assert.property(conversionTable.gram, "ounce");
```

Ihre `ounce`-Eigenschaft sollte auf `0.0353` gesetzt sein.

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

Ihr `gram`-Objekt sollte eine `teaspoon`-Eigenschaft haben.

```js
assert.property(conversionTable.gram, "teaspoon");
```

Ihre `teaspoon`-Eigenschaft sollte auf `0.2` gesetzt sein.
