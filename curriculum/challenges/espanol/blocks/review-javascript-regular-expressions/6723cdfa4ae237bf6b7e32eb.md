...
- **Bandera `u`**: Esta bandera amplía la funcionalidad de una expresión regular para permitir que coincida con caracteres especiales unicode. La bandera `u` te da acceso a clases especiales como `Extended_Pictographic` para coincidir con la mayoría de los emojis. También existe una bandera `v`, que amplía aún más la funcionalidad de la coincidencia unicode.
- **Bandera `y`**: El modificador sticky se comporta de manera muy similar al modificador global, pero con algunas excepciones. La más importante es que una expresión regular global comenzará desde lastIndex y buscará en el resto de la cadena otra coincidencia, pero una expresión regular sticky devolverá null y restablecerá lastIndex a 0 si no hay una coincidencia inmediata en el lastIndex anterior.
- **Bandera `s`**: El modificador de línea única permite que un carácter comodín, representado por un `.` en regex, coincida con saltos de línea, tratando efectivamente la cadena como una sola línea de texto.
