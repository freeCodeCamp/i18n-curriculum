...
該進行另一個迴圈了。使用名為 `container` 的變數創建一個 `for...of` 迴圈，以迭代 `inputContainers` 陣列。

在迴圈內，將 `container` 的 `innerHTML` 屬性設為空字串。這將清除該輸入容器的所有內容。

# --hints--

你的 `for...of` 迴圈應該要迭代 `inputContainers` 陣列。迴圈的變數名稱應該是 `container`。
...
你的 `for...of` 迴圈應該將 `container` 的 `innerHTML` 屬性設為空字串。
