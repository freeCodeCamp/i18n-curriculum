...
Per il secondo elemento `tabpanel`, aggiungi l'attributo `id` con il valore `"panel-saturn"` e un attributo `aria-labelledby` con il valore `"tab-saturn"`.

Per il terzo elemento `tabpanel`, aggiungi l'attributo `id` con il valore `"panel-mars"` e un attributo `aria-labelledby` con il valore `"tab-mars"`.

# --hints--

Il tuo secondo elemento `tabpanel` dovrebbe avere un attributo `id`.

```js
assert.isTrue(document.querySelectorAll('[role="tabpanel"]')[1].hasAttribute("id"));
```

Il tuo secondo attributo `id` dovrebbe avere il valore `"panel-saturn"`.

```js
assert.equal(document.querySelectorAll('[role="tabpanel"]')[1].getAttribute("id"), "panel-saturn");
```

Il tuo secondo elemento `tabpanel` dovrebbe avere un attributo `aria-labelledby`.

```js
assert.isTrue(document.querySelectorAll('[role="tabpanel"]')[1].hasAttribute("aria-labelledby"));
```

Il tuo secondo attributo `aria-labelledby` dovrebbe avere il valore `"tab-saturn"`.

```js
assert.equal(document.querySelectorAll('[role="tabpanel"]')[1].getAttribute("aria-labelledby"), "tab-saturn");
``` 

Il tuo terzo elemento `tabpanel` dovrebbe avere un attributo `id`.

```js
assert.isTrue(document.querySelectorAll('[role="tabpanel"]')[2].hasAttribute("id"));
```

Il tuo terzo attributo `id` dovrebbe avere il valore `"panel-mars"`.

```js
assert.equal(document.querySelectorAll('[role="tabpanel"]')[2].getAttribute("id"), "panel-mars");
```

Il tuo terzo elemento `tabpanel` dovrebbe avere un attributo `aria-labelledby`.

```js
assert.isTrue(document.querySelectorAll('[role="tabpanel"]')[2].hasAttribute("aria-labelledby"));
```

Il tuo terzo attributo `aria-labelledby` dovrebbe avere il valore `"tab-mars"`.
