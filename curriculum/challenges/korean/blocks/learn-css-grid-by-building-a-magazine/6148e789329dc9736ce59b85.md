...
`.image-1, .image-3` 규칙을 만들고 `grid-column` 속성을 `1 / -1`로 설정하세요. 이렇게 하면 첫 번째와 세 번째 이미지가 그리드의 전체 너비를 차지하게 됩니다.

# --hints--

`.image-1, .image-3` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

`.image-1, .image-3` 선택자에 `grid-column` 속성을 `1 / -1`로 설정해야 합니다.
