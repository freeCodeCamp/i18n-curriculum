...
為完成工作坊，為你的程式碼添加最後一個列印述語，使用 f-string 並以此格式顯示兩位小數的最佳價格：`Best price for [product.name] for [user_tier] user: $[best_price]`，其中 `[product.name]`、`[user_tier]` 和 `[best_price]` 應該取代為實際的變數值。

範例輸出可能是：`Best price for Wireless Buds for Premium user: $93.00`。請注意價格的小數點後兩位數。
