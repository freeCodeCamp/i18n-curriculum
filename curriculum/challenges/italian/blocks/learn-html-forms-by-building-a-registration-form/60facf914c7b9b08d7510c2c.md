...
Il `textarea` appare troppo piccolo. Per dargli una dimensione iniziale, puoi aggiungere gli attributi `rows` e `cols`.

Aggiungi una dimensione iniziale di `3` righe e `30` colonne.

# --hints--

Dovresti dare al `textarea` un attributo `rows` con valore `3`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

Dovresti dare al `textarea` un attributo `cols` con valore `30`.
