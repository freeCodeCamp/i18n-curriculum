...
次のステップは、プレイヤーとコンピューターのスコアを管理する変数を作成することです。

`playerScore` という変数を作成し、値を `0` で初期化してください。

次に、`computerScore` という変数を作成し、値を `0` で初期化してください。

これらの変数はゲームの進行中に値が変わるため、`let` を使って宣言する必要があります。

# --hints--

`playerScore` という変数があるはずです。

```js
assert.isNotNull(playerScore);
```

`let` 変数を宣言するには `playerScore` を使うべきです。

```js
assert.match(code, /let\s+playerScore/);
```

`playerScore` は `0` で初期化されているはずです。

```js
assert.strictEqual(playerScore, 0);
```

`computerScore` という変数があるはずです。

```js
assert.isNotNull(computerScore);
```

`let` 変数を宣言するには `computerScore` を使うべきです。

```js
assert.match(code, /let\s+computerScore/);
```

`computerScore` は `0` で初期化されているはずです。
