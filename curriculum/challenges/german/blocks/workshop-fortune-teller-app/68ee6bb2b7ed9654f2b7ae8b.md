...
Fügen Sie dem globalen `document`-Objekt einen weiteren Ereignis-Listener hinzu, der die `showFortune`-Funktion aufruft. Fügen Sie im Callback einen Parameter `e` vom Typ `Event` hinzu und übergeben Sie `e` als Argument an die `showFortune`-Methode.

`showFortune` wird Klick-Ereignisse auf Karten verarbeiten, zum Beispiel die ID der angeklickten Karte aus dem DOM lesen und die passenden Kartenglücksdetails anzeigen. Sie werden diese Funktion in den kommenden Schritten erstellen.

# --hints--

Sie sollten dem globalen `click` einen `document`-Ereignis-Listener zuweisen.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Ihr Ereignis-Listener sollte einen Callback-Parameter namens `e` haben.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

Ihr Callback-Parameter sollte den Typ `Event` haben.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

Innerhalb des Callbacks sollten Sie die `showFortune`-Funktion aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

Innerhalb der `showFortune`-Funktion sollten Sie das Argument `e` übergeben.
