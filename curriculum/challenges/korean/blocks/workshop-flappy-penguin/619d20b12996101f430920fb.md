...
펭귄의 부리와 발은 같은 `color`를 공유합니다.

새로운 사용자 정의 CSS 변수를 `--penguin-picorna`라는 이름으로 만들고, 관련된 모든 속성 값을 이 변수로 바꾸세요.

# --hints--

`:root`에 `--penguin-picorna` 속성을 지정해야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-picorna'));
```

`--penguin-picorna`에 `orange` 값을 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-picorna', true), 'orange');
```

`.beak`에 `background-color`가 `var(--penguin-picorna)`여야 하지만 `--fcc-actual--`가 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.getPropVal('background-color', true), 'var(--penguin-picorna)');
```

`.foot`에 `background-color`가 `var(--penguin-picorna)`여야 하지만 `--fcc-actual--`가 발견되었습니다.
