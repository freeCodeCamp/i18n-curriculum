...
在開發過程中，查看 `fieldset` 默認邊框很有幫助。 然而，它們使內容間的距離過遠。

刪除 `border`，然後給 `fieldset` 的頂部和底部添加 `2rem` 的內邊距。 確保移除了左側和右側的 `padding`。
...
你應該爲每個 `fieldset` 添加 `2rem` 的頂部和底部 `padding`。
