...
كما تعلمت بالفعل، تُستخدم طريقة `delete` لحذف عنصر من الخريطة. تحتوي المجموعات على طريقة `delete` يمكنك استخدامها لحذف عنصر منها أيضًا

أحيانًا قد تحتاج إلى حذف كل العناصر من خريطة أو مجموعة. تقوم طريقة `clear` بهذا العمل بالضبط.

نفذ طريقة `clear` على `plantsSet` لحذف كل العناصر منه.

# --hints--

يجب أن تنفذ طريقة `clear` على `plantsSet`.
