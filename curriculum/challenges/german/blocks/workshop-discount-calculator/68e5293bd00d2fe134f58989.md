...
Implementieren Sie nun die `apply_discount`-Methode für `FixedAmountDiscount`. Diese Methode soll den festen Betrag vom Produktpreis abziehen und den neuen Preis zurückgeben.

# --hints--

Ihre `FixedAmountDiscount`-Klasse sollte eine `apply_discount`-Methode haben.
...
Ihre `apply_discount`-Methode sollte `product.price - self.amount` zurückgeben.
