...
Altere o texto do elemento `h1` abaixo do `Hello World` para `CatPhotoApp` e veja as mudanças na pré-visualização do navegador.
...
O elemento `h1` deve ter uma tag de abertura. Opening tags have this syntax: `<elementName>`.

```js
assert(document.querySelector('h1'));
```

O elemento `h1` deve ter uma tag de fechamento. As tags de fechamento têm um `/` logo após o caractere `<`.
