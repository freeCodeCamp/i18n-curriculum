...
Ni wakati wa kupamba sehemu ya mwisho ya jarida - picha.

Picha zimefungwa na kipengele cha `aside` kinachotumia darasa la vitu la `image-wrapper`, kwa hivyo tengeneza kichaguzi cha `.image-wrapper`. Weka sifa ya `display` kuwa `grid`.
...
Kichaguzi chako cha `.image-wrapper` kinapaswa kuwa na sifa ya `display` iliyowekwa kuwa `grid`.
