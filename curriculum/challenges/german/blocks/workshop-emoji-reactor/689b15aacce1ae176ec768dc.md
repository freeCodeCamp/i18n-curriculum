...
Jetzt müssen Sie die erste Zahl im `0/10`-Text ermitteln, und zwar so, dass es auch für `5/10` oder `10/10` funktioniert.

Analysieren Sie den Textinhalt des `.count`-Elements, um die aktuelle Zahl zu extrahieren.

Konvertieren Sie die Zählung von einem String in eine Zahl.

Ersetzen Sie das vorherige `console.log(countEl.textContent)` durch Ihre neue Ausgabe.

Geben Sie die aktuelle Zählung in folgendem Format aus: `console.log("Current count:", currCount)`.

Hinweis: Denken Sie daran, dass Sie die `split`-Methode verwenden können, um einen String in ein Array von Teilstrings zu teilen.

# --hints--

Sie sollten die aktuelle Zählung als Zahl ausgeben.
...
Sie sollten die aktuelle Zählung als Zahl ausgeben, und zwar so, dass es für die verschiedenen möglichen Werte funktioniert.
