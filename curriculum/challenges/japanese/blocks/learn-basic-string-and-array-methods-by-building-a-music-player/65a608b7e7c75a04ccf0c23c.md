...
画面に曲のリストが表示されたので、タイトルのアルファベット順に並べ替えると良いでしょう。`allSongs` 配列を手動で更新することもできますが、JavaScriptには <dfn>sort()</dfn> という配列メソッドが用意されています。

`sort()` メソッドは配列の要素を文字列に変換し、`UTF-16` エンコーディングの値に基づいてその場で並べ替えます。
...
`sortSongs` 関数の中で、`sort()` に `userData?.songs` メソッドを追加してください。

# --hints--

`sort()` に `userData?.songs` メソッドを追加してください。
