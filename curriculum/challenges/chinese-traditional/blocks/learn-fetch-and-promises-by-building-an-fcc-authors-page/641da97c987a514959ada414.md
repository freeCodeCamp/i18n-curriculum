...
現在，像之前一樣，使用部分作者數據調用 `displayAuthors` 函數。

如果調用該函數後單擊 `Load More Authors` 按鈕，它將不起作用。 這是因爲你仍然必須向按鈕添加 `click` 事件監聽器。 你接下來會這麼做。
...
`slice()` 方法的第一個參數應該是 `startingIndex`。
