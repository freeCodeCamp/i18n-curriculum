...
Die Grid-Elemente sind zu nah an den linken und rechten Seitenrändern der Seite, daher sollten Sie am Abstand arbeiten.

Weisen Sie dem `max-w-6xl` die Klasse `div` zu, um eine feste Breite von `72rem` festzulegen und zu verhindern, dass sich die Elemente zu weit über die Seite erstrecken.

Da das alle nach links schiebt, fügen Sie dem `mx-auto` `div` hinzu, um die Elemente horizontal zu zentrieren.

# --hints--

Ihr `div`-Element sollte die Klasse `max-w-6xl` haben.
...
Ihr `div`-Element sollte die Klasse `mx-auto` haben.
