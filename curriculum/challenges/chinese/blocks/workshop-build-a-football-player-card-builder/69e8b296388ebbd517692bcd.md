...
现在你将开始创建形式。在 `div` 内，带有 `className` 的 `form-panel` 中，添加一个 `div` 来包裹“Player Info”部分。在那个新的 `div` 内，添加带有 `className` 的 `p`，className 的类型是 `form-section-title`，文本是 `Player Info`。

你将在接下来的步骤中继续为该部分添加字段。

# --hints--

你的 `div.form-panel` 应该包含一个 `div` 来包裹 Player Info 部分。
...
`div.form-panel` 内的 `div` 应该包含一个带有 `className` 的 `p`，className 的类型是 `form-section-title`，文本是 `Player Info`。
