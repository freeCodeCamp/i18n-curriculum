...
創建一個 `if` 述語來檢查鄰近節點的距離（處理過的元組中的第二項）加上 `current` 的距離是否小於目前已知的鄰近節點距離（處理過的元組中的第一項）。 

使用 `pass` 關鍵字暫時填充 `if` 的主體。

# --hints--

你應該有一個 `if` 述語來檢查 `distance + distances[current]` 是否小於 `distances[node]`。
