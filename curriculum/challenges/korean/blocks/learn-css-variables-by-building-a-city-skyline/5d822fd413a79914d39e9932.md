...
다음 건물로 갑니다! 앞에 있는 초록색 건물입니다. `repeating-linear-gradient`에서 `0%`까지 건물 색상으로 `5%`를 주고, `transparent`에서 `5%`까지 `10%`를 주세요.

# --hints--

`.fb5`에 `background` 속성을 주어야 합니다.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

`background`에 `repeating-linear-gradient`를 주어야 합니다.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

`repeating-linear-gradient`에 `--building-color2`에서 `0%`까지 첫 번째 색상 `5%`를 주어야 합니다.
...
`repeating-linear-gradient`에 `transparent`에서 `5%`까지 두 번째 색상 `10%`를 주어야 합니다.
