...
В межах функції отримайте посилання на елементи HTML, відповідальні за відображення назви пісні та виконавця.

Отримайте доступ до елементів `#player-song-title` та `#player-song-artist` за допомогою методу `getElementById()`. Призначте їх змінним `playingSong` та `songArtist` відповідно.

# --hints--

Не змінюйте наявну функцію `setPlayerDisplay` та її вміст.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Використайте `document.getElementById()`, щоб отримати елемент `#player-song-title`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Призначте елемент `#player-song-title` до змінної `playingSong`. Не забудьте використати `const` для оголошення змінної.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Використайте `document.getElementById()`, щоб отримати елемент `#player-song-artist`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Призначте елемент `#player-song-artist` до змінної `songArtist`. Не забудьте використати `const` для оголошення змінної.
