...
我們已經為你將剩下的歌曲添加到 `allSongs` 陣列中。

接下來，你將學習 <dfn>Web Audio API</dfn> 以及如何使用它來播放歌曲。所有現代瀏覽器都支援 Web Audio API，讓你能在網頁應用程式中產生和處理音訊。

使用 `const` 創建一個名為 `audio` 的變數，並將它設定為 `new Audio()`。這將創建一個新的 HTML5 `audio` 元素。

# --hints--

你應該使用 `new` 關鍵字來創建 `Audio` 物件的實體。

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

你應該將 `Audio` 物件指定給名為 `audio` 的常數。
