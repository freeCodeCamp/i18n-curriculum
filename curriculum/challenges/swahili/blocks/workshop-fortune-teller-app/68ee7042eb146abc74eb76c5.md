...
Tena, unahitaji kufanya kadi kuwa ya nasibu na iliyogeuzwa. Tangaza kinachoitwa `isReversed` na `card`. Kwa ya kwanza, fanya wito wa njia ya `Math.random` na linganisha na `< 0.5`. Kwa ya pili, fanya wito wa `getRandomItem` na pita `this.cards`.

# --hints--

Unapaswa kutangaza kinachoitwa `isReversed`.
...
Kipengele chako cha `isReversed` kinapaswa kutumia `Math.random()` kuamua kwa nasibu kama kadi imegeuzwa.
...
Unapaswa kutangaza kinachoitwa `card`.
...
Kipengele chako cha `card` kinapaswa kupewa thamani kwa kufanya wito wa njia ya `getRandomItem` na `this.cards` kama hoja.
