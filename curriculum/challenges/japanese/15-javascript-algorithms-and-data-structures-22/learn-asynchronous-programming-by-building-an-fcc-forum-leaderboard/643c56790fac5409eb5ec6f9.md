...
`showLatestPosts()` 関数に戻り、分割代入を使って、`data` オブジェクトからプロパティ `topic_list` と `users` を取り出してください。

# --hints--

You should use `const` to destructure `topic_list` and `users` from the `data` object.
