...
Agora faça um pouco de espaço, remova todas as variáveis de botão e o `eventListener` que você criou para cada um.

# --hints--

A variável `happyBtn` não deve existir.
...
A variável `confusedBtn` não deve existir.
...
A variável `sadBtn` não deve existir.
...
A variável `lovingBtn` não deve existir.
