...
Agora que você sabe para qual portão redirecionar, pode mover os participantes em overflow.

Pegue o `overflowAmount` e adicione-o à fila do próximo portão no mesmo `tickIndex`.

Especificamente, você deve:

- Acessar o próximo portão usando `nextGateIndex`.
- Acessar seu `queue`.
- Adicionar o `overflowAmount` ao valor na posição `tickIndex`.

# --hints--

Sua função `rerouteOverflow` deve adicionar `overflowAmount` ao valor em `tickIndex` no `queue` do próximo portão.
