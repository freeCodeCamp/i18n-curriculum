...
`textarea` 要素は `input` 要素の `text` タイプのように動作しますが、複数行のテキストを受け取ることができ、初期のテキスト行数と列数を指定できるという利点があります。

ユーザーは自己紹介文を登録できるようになります。`label` の最後にテキスト `Provide a bio:` を持つ `fieldset` を追加してください。`textarea` 要素の中に `label` 要素を追加してください。`textarea` は終了タグが必要なことに注意してください。

# --hints--

3番目の `label` の最後に、既存の `fieldset` 要素の後で `label` 要素を追加してください。

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4)'));
```

`label` にテキスト `Provide a bio:` を設定してください。

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label:nth-child(4)')?.innerText, /Provide a bio/);
```

`textarea` の中に `label` 要素をネストしてください。

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea'));
```

`textarea` 要素には開始タグと終了タグを付けてください。
