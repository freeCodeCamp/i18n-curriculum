...
使用你的 `updatedSquad` 名單喚起 `getEVAReadyCrew()`，並將結果儲存在名為 `EVAReadySquad` 的新變數中。然後，使用 `for` 迴圈記錄 `EVAReadySquad` 陣列中每位太空人的 `name`。

# --hints--

你應該使用 `updatedSquad` 來呼叫 `getEVAReadyCrew()`，並將結果儲存在名為 `EVAReadySquad` 的新變數中。
...
你應該使用 `for` 迴圈來迭代 `EVAReadySquad` 陣列並記錄每個太空人的 `name`。
