...
Após o último elemento `.divider`, crie um elemento `p` e dê a ele o texto `Total Fat 8g 10%`. Then, wrap the text `Total Fat` in one `span` element, the text `10%` in another, and give them each a class of `bold`.
...
O novo elemento `p` deve ter o texto `Total Fat 8g 10%`.
