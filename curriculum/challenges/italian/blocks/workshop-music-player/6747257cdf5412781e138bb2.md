...
Il tuo array `allSongs` dovrebbe avere un valore.
...
Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `id` impostata sul numero `0`.

```js
assert.strictEqual(allSongs[0].id, 0);
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `title` impostata sulla stringa `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `artist` impostata sulla stringa `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `duration` impostata sulla stringa `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Il tuo array `allSongs` dovrebbe avere un oggetto con una proprietà `src` impostata sulla stringa `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
