...
داخل الدالة `displayStory`، تحقق مما إذا كان كائن `storyObj` يحتوي على خاصية تطابق معلمة `genre` واضبط محتوى نص الفقرة `#result` على قصة النوع المختار.

أيضًا، اضبط لون حدود `storyContainer` إلى الخاصية `borderColor` للنمط المختار.

اختبر ذلك باستدعاء الدالة `displayStory` مع النوع `scary`. يجب أن يكون لون الحدود `#ee4b2b` والقصة هي `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.

# --hints--

يجب أن تستدعي الدالة `displayStory` مع النوع `"scary"`.

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

عند استدعاء `displayStory("scary")`، يجب أن يكون لون الحدود `#ee4b2b`.
...
عند استدعاء `displayStory("scary")`، يجب أن تُعرض القصة `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.
