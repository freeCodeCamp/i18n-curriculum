...
Параметр `fn` отримає ім’я функції, наприклад `"SUM"`. Оновіть `apply`, щоб неявно повертати функцію з вашого об’єкта `spreadsheetFunctions`, використовуючи змінну `fn` як ключ для доступу до об’єкта.

Пам’ятайте, що `fn` може не містити рядок у нижньому регістрі, тому вам потрібно буде конвертувати його у рядок нижнього регістру.

# --hints--

Ваша функція `apply` має використовувати неявне повернення.
...
Ваша функція `apply` має отримувати доступ до об’єкта `spreadsheetFunctions`.
...
Ваша функція `apply` має отримувати доступ до властивості об’єкта `spreadsheetFunctions`, що відповідає значенню `fn`.
...
Ваша функція `apply` має викликати метод `.toLowerCase()` на `fn` у доступі до властивості.
