...
Ainda dentro da função `playSong`, defina a propriedade `audio.src` igual a `song.src`. Isso informa ao elemento de áudio onde encontrar os dados de áudio para a música selecionada.

Além disso, defina a propriedade `audio.title` igual a `song.title`. Isto informa ao elemento de áudio o que exibir como título da música.

# --hints--

Você deve definir a propriedade `audio.src` igual a `song.src` dentro da sua função `playSong`.
...
Você deve definir a propriedade `audio.title` igual a `song.title` dentro da sua função `playSong`.
