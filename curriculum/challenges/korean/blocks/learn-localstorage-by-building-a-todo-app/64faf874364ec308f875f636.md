...
`remove()`를 사용해 DOM에서 작업을 제거하고 `taskData`를 사용해 `splice()` 배열에서도 제거해야 합니다.

`splice()`는 지정한 인덱스에서 요소를 제거, 교체 또는 추가하면서 제거된 요소를 반환하는 배열 메서드입니다. 최대 세 개의 인수를 받을 수 있는데, 첫 번째는 시작할 필수 인덱스, 두 번째는 제거할 항목 수, 세 번째는 선택적 교체 요소입니다. 예시는 다음과 같습니다:
...
`remove()` 메서드를 사용해 DOM에서 `parentElement`의 `buttonEl`를 제거하세요. 그런 다음 `splice()`를 사용해 `taskData` 배열에서 작업을 제거하세요. `dataArrIndex`의 인수로 `1`와 `splice()`을 전달하세요.

`dataArrIndex`는 시작할 인덱스이고 `1`는 제거할 항목 수입니다.

# --hints--

`dataArrIndex` 문장은 변경하지 마세요.
