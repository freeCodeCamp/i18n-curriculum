...
あなたの `apply` 関数はスプレッドシート関数を返していますが、実際には適用していません。`apply` を更新して関数を呼び出してください。`toNumberList` を引数として `args` を呼び出した結果を渡してください。

# --hints--

あなたの `apply` 関数は `spreadsheetFunctions[fn.toLowerCase()]` 関数を呼び出す必要があります。
...
`toNumberList()` の呼び出しに `spreadsheetFunctions[fn.toLowerCase()]` の呼び出しを渡す必要があります。
...
`args` の呼び出しに `toNumberList()` を渡す必要があります。
