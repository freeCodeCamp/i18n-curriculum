...
`isSelect`라는 이름의 함수를 만들고, `element` 타입의 단일 매개변수 `EventTarget | null`을 받으세요. 이 함수는 선택한 요소가 `<select>` HTML 요소인지 확인할 것입니다.

# --hints--

`isSelect`라는 이름의 함수가 있어야 합니다.
...
`isSelect` 함수는 `element` 타입의 `EventTarget | null`라는 매개변수를 가져야 합니다.
...
`isSelect` 함수는 `element is HTMLSelectElement` 타입의 반환값을 가져야 합니다.
