...
현재 재생 중인 노래의 `id`를 가져와야 합니다. 이를 위해 `userData?.currentSong?.id`를 사용할 수 있습니다.

`getElementById()`를 사용해 현재 재생 중인 노래의 `id`를 가져온 다음, 템플릿 리터럴로 `song-`를 접두사로 붙이세요. 그리고 이를 상수 `songToHighlight`에 할당하세요.

# --hints--

기존 `highlightCurrentSong` 함수와 그 내용을 수정하지 마세요.
...
`document.getElementById()`를 사용하고 `` `song-${userData?.currentSong?.id}` ``를 전달해야 합니다.

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

`getElementById()`를 `songToHighlight` 상수에 할당해야 합니다.
