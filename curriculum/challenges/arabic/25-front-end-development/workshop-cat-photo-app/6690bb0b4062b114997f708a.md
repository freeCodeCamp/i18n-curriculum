...
Your `main` element should have an opening tag. تكتب علامات الفتح هكذا: `<elementName>`.

```js
assert.exists(document.querySelector('main'));
```

Your `main` element should have a closing tag. Closing tags يجب ان تتبع بـ `/` مباشرة بعد رمز `<`.
...
Your `main` element's opening tag should be above the `h1` element. لقد وضعتهم بترتيب خاطئ.
...
Your `main` element's closing tag should be below the `p` element. لقد وضعتهم بترتيب خاطئ.
