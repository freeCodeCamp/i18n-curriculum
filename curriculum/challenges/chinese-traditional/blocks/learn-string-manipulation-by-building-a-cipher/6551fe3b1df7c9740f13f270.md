...
你應該聲明一個名爲 `alphabet` 的變量。 注意將變量名放在行首。

```js
assert.match(code, /^alphabet\s*=/m)
```

你應該將字符串 `'abcdefghijklmnopqrstuvwxyz'` 分配給你的 `alphabet` 變量。 請記住使用單引號或雙引號來括住字符串。
