...
Erstellen Sie als Nächstes ein Array, um alle Songs zu speichern.

Erstellen Sie ein leeres `allSongs`-Array.

# --hints--

Sie sollten eine Variable namens `allSongs` haben.

```js
assert.isDefined(allSongs)
```

Sie sollten Ihrer Variable `allSongs` ein Array zuweisen.
