...
你需要從 HTML 文件中獲取的最後一組元素是 `date-input` 和 `description-input` 元素。 將它們分別保存在變量 `dateInput` 和 `descriptionInput` 中。

# --hints--

你應該使用 `getElementById()` 來訪問 `date-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)date\-input\1\s*\)\s*;?/)
```

你應該將 `date-input` 元素分配給變量 `dateInput`。 不要忘記使用 `const` 來聲明變量。

```js
assert.match(code, /const\s+dateInput\s*=\s*document\.getElementById\(\s*('|"|`)date\-input\1\s*\)\s*;?/)
```

你應該使用 `getElementById()` 來訪問 `description-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)description\-input\1\s*\)\s*;?/)
```

你應該將 `description-input` 元素分配給變量 `descriptionInput`。 不要忘記使用 `const` 來聲明變量。
