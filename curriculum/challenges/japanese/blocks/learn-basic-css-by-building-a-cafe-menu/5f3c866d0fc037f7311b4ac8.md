...
だいぶ近づきましたが、価格が右側に固定されていませんでした。これは`inline-block`要素がコンテンツの幅だけしか取らないためです。

それらを広げるには、`width`と`flavor`のクラスセレクターに、それぞれ`price`の値を持つ`50%`プロパティを追加してください。

# --hints--

`width`セレクターで`50%`プロパティを`.flavor`に設定してください。
...
`width`セレクターで`50%`プロパティを`.price`に設定してください。
