...
接下來是取得你的總計。取得 `#total-items`、`#subtotal`、`#taxes` 和 `#total` 項目。分別將它們儲存在名為 `totalNumberOfItems`、`cartSubTotal`、`cartTaxes` 和 `cartTotal` 的變數中。

# --hints--

你應該使用 `document.getElementById()` 來取得 `#total-items` 項目。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)total-items\1\s*\)/);
```

你應該將 `#total-items` 元素指定給名為 `totalNumberOfItems` 的變數。請記得使用 `const` 來宣告該變數。

```js
assert.match(code, /const\s+totalNumberOfItems\s*=\s*document\.getElementById\(\s*('|"|`)total-items\1\s*\)/);
```

你應該使用 `document.getElementById()` 來取得 `#subtotal` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)subtotal\1\s*\)/);
```

你應該將 `#subtotal` 元素指定給名為 `cartSubTotal` 的變數。記得使用 `const` 來宣告該變數。

```js
assert.match(code, /const\s+cartSubTotal\s*=\s*document\.getElementById\(\s*('|"|`)subtotal\1\s*\)/);
```

你應該使用 `document.getElementById()` 來取得 `#taxes` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)taxes\1\s*\)/);
```

你應該將 `#taxes` 元素指定給名為 `cartTaxes` 的變數。請記得使用 `const` 來宣告該變數。

```js
assert.match(code, /const\s+cartTaxes\s*=\s*document\.getElementById\(\s*('|"|`)taxes\1\s*\)/);
```

你應該使用 `document.getElementById()` 來取得 `#total` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)total\1\s*\)/);
```

你應該將 `#total` 元素指定給名為 `cartTotal` 的變數。請記得使用 `const` 來宣告該變數。
