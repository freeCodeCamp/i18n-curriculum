...
Assim como funções regulares, funções arrow podem aceitar múltiplos parâmetros. 

Aqui está um exemplo de uma função arrow nomeada com um parâmetro: 
...
Se a função tiver apenas um parâmetro, você pode omitir os parênteses ao redor da lista de parâmetros assim:
...
Crie uma nova função arrow nomeada chamada `printMessage` que tenha um parâmetro chamado `org`. Dentro do corpo dessa função, adicione uma instrução console. Dentro dessa instrução do console, adicione o template literal `${org} is awesome!`. 

Abaixo da sua função `printMessage`, chame a função e passe a string `"freeCodeCamp"` como argumento.

Abra o console para ver o resultado. 

# --hints--

Você deve ter uma função `printMessage`.

```js
assert.isFunction(printMessage);
```

Você deve usar a sintaxe de função arrow.

```js
assert.match(code, /(let|const)\s+printMessage\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Sua função `printMessage` deve ter um parâmetro chamado `org`.
...
Sua função `printMessage` deve ter uma instrução de console.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

Sua declaração no console deve incluir o template literal `${org} is awesome!`.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

Você deve chamar a função `printMessage` e passar a string `"freeCodeCamp"`.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

A chamada da sua função deve estar abaixo da definição da função `printMessage`.
...
A chamada da sua função `printMessage` deve exibir `freeCodeCamp is awesome!` no console. 
