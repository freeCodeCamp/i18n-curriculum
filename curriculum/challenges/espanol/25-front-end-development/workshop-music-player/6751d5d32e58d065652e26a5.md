...
Ahora, necesitas añadir el atributo `aria-current` a la canción que se está reproduciendo actualmente.

Comprueba si `songToHighlight` existe. Si es así, utiliza el método `setAttribute` en `songToHighlight` y pasa `"aria-current"` y `"true"` como primer y segundo argumentos.

# --hints--

`highlightCurrentSong` debería añadir el atributo `aria-current` a la canción que se está reproduciendo actualmente y eliminar el atributo de la canción reproducida anteriormente.
