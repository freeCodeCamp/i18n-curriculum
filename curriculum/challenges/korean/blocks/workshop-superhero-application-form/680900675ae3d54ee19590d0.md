...
오류를 고치려면 `handlePowersChange` 화살표 함수에 `e` 매개변수를 만드세요.

함수 내부에서 `value`에서 `checked`와 `e.target`를 구조 분해하여 체크박스의 값과 선택 여부를 가져오세요.

# --hints--

`handlePowersChange` 매개변수를 가진 `e` 함수를 만들어야 합니다.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

`value`에서 `checked`와 `e.target`를 구조 분해해야 합니다.
