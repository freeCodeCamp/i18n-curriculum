---
id: 5d822fd413a79914d39e98d1
title: ステップ 9
challengeType: 0
dashedName: step-9
---

# --description--

`.background-buildings` 要素に `width` の `height` と `100%` を設定して、親要素である `body` の幅と高さいっぱいにしてください。

# --hints--

正しい要素を選択するには `background-buildings` クラスを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.background-buildings'));
```

`.background-buildings` 要素には `width` の `100%` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.background-buildings')?.width, '100%');
```

`.background-buildings` 要素には `height` の `100%` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.background-buildings')?.height, '100%');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">    
  <head>
    <meta charset="UTF-8">
    <title>City Skyline</title>
    <link href="styles.css" rel="stylesheet" />
  </head>

  <body>
    <div class="background-buildings"></div>
  </body>
</html>
```

```css
* {
  border: 1px solid black;
  box-sizing: border-box;
}

body {
  height: 100vh;
  margin: 0;
  overflow: hidden;
}
--fcc-editable-region--


--fcc-editable-region--
    
```
