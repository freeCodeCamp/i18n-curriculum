...
创建一个名为 `setPlayButtonAccessibleText` 的函数。在函数内部，将播放按钮的 `aria-label` 属性设置为 `Play`，后跟一个空格和当前歌曲的标题。如果 `userData.currentSong` 为 `null`，则将其设置为 `Play`。

# --hints--

你应该有一个名为 `setPlayButtonAccessibleText` 的函数。

```js
assert.isFunction(setPlayButtonAccessibleText);
```

你的 `setPlayButtonAccessibleText` 函数应将播放按钮的 `aria-label` 属性设置为 `Play`，后跟一个空格和当前歌曲的标题。
...
```
 ...
 当 `userData.currentSong` 为 `null` 时，`setPlayButtonAccessibleText` 应将播放按钮的 `aria-label` 属性设置为 `Play`。
