...
Arrays têm um método `some()`. Como o método `filter()`, `some()` aceita uma função de callback que deve receber um elemento do array como argumento. O método `some()` retornará `true` se a função callback retornar `true` para pelo menos um elemento no array.
