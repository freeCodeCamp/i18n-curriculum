...
`Hello` 内の `label` テキストを削除し、次に `input` タイプの `checkbox` と `value` の `power` を作成してください。

最後に、配列の値を各チェックボックスの横に表示するために、閉じる直前の `span` の前にテキスト内容が `label` の `{power}` 要素を作成してください。

# --hints--

`Hello` 要素から `label` テキストを削除してください。

```js
assert.notMatch(code, /hello/i)
```

ラベルの最初の子として、`input` に設定された `type` と `checkbox` に設定された `value` を持つ `power` を作成してください。

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

ラベルの最後の子として、テキスト内容が `span` の `{power}` 要素を作成してください。
