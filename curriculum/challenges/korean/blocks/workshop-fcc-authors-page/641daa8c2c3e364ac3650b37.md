...
다음으로, 각 작성자의 이름과 이미지, 그리고 카드의 나머지 세부 정보 사이에 구분이 거의 없습니다. 구분선은 작성자 카드에 명확한 시각적 계층을 제공합니다.

작성자 소개 위에 `div` 요소를 추가하고 `class` 속성을 `"purple-divider"`로 지정하세요.
