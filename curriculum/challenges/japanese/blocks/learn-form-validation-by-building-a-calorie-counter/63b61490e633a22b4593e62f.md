...
同じパターンに従って、`#add-entry` 要素を `addEntryButton` に、`#clear` 要素を `clearButton` に、`#output` 要素を `output` に代入してください。

# --hints--

`addEntryButton` という変数を宣言してください。

```js
assert.isDefined(addEntryButton);
```

`document.getElementById()` を使って `#add-entry` 要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|")add-entry\1\s*\)/g);
```

`#add-entry` 要素を `addEntryButton` に代入してください。

```js
assert.deepEqual(addEntryButton, document.getElementById('add-entry'));
```

`clearButton` という変数を宣言してください。

```js
assert.isDefined(clearButton);
```

`document.getElementById()` を使って `#clear` 要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|")clear\1\s*\)/g);
```

`#clear` 要素を `clearButton` に代入してください。

```js
assert.deepEqual(clearButton, document.getElementById('clear'));
```

`output` という変数を宣言してください。
...
`document.getElementById()` を使って `#output` 要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|")output\1\s*\)/g);
```

`#output` 要素を `output` に代入してください。
