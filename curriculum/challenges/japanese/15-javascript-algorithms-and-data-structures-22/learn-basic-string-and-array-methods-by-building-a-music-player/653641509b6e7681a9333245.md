...
現在再生中の曲 (cuurentSong) と、`playButton` の要素の見た目を更新しましょう。

`userData` オブジェクトの `currentSong` プロパティに `song` を代入してください。

*注*: この時点で `userData.currentSong` が `null` または `undefined` である可能性はないため、このステップではオプショナルチェイン演算子 `?.` を使うべきではありません。

# --hints--

`userData.currentSong` に `song` を代入してください。
