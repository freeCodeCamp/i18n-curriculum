...
この建物の窓にセカンダリーカラーを塗ってください。また、窓に少しスペースを作るために`margin`に`10%`を追加してください。

# --hints--

`.fb4-window`に`background-color`の`--window-color1`を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.backgroundColor.trim(), "var(--window-color1)");
```

`.fb4-window`に`margin`の`10%`を設定してください。
