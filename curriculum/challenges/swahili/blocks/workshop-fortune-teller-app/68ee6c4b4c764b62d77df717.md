...
Katika baadhi ya matukio, kadi zinahitaji kugeuzwa, kwa sababu si kila mara ziko katika nafasi sahihi.

Tangaza kigezo kinachoitwa `isReversed` ndani ya njia ya `singleCardSelected`. Tumia njia ya `Math.random()` kuunda thamani ya kweli au siyo kweli isiyopangwa. Ili kuhakikisha thamani inabaki ndani ya mipaka, linganisha na `0.5`.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `isReversed`.
...
Kigezo chako cha `isReversed` kinapaswa kutumia `Math.random()` kuamua kwa bahati kama kadi imegeuzwa.
