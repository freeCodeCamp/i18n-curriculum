...
Asigna a tu tercer elemento `th` el atributo `class` con el valor `current`.
...
Cada uno de tus nuevos elementos `span` debe tener el atributo `class` con el valor `sr-only year`.
