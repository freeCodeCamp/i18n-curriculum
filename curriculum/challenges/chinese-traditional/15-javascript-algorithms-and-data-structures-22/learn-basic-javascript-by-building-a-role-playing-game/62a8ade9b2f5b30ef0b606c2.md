...
向名爲 `text` 的對象添加一個最終屬性。 賦予此屬性與在 `goTown` 函數中分配給 `text.innerText` 的字符串值相同的字符串值。

# --hints--

你的第一個 `locations` 值應該具有 `text` 屬性。
...
你的 `text` 屬性應該具有值 `"You are in the town square. You see a sign that says \"Store\"."`
