...
Deberías tener `array[sorted_index] = left_part[left_array_index]` en el cuerpo del bucle. No olvides eliminar la palabra clave `pass` en el cuerpo del bucle.
...
No deberías tener `pass` en el bucle `while`.
