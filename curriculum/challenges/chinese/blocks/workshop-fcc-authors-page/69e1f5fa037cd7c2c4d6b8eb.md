...
最后，你需要处理 `catch` 方法。首先，将 `fetch` 语句、`json` 语句和 displayAuthors 调用嵌套到 `try` 中的一个 `initialFetch` 块里。然后，按照下面的示例，将链式的 `catch` 方法修改为 catch 块。
...
如果你做对了，`try` 块中将包含三行代码，`catch` 块中将只包含一行代码。同时，`SyntaxError` 会被解析，应用程序应该能正常运行。

为自己鼓掌！你已经学会了如何使用 `async` 和 `await` 操作符让你的异步代码更易读！现在你的作者页面已经完成。
...
`try` 块应包含获取数据、创建 `json` 并显示它的三条语句
