...
استخدم حلقة `for` للتكرار على كل رقم في قائمة `odd_digits`. انقل استدعاء `print` من الخطوة السابقة إلى داخل حلقة `for`، وغيّره لطباعة كل رقم.

# --hints--

يجب أن تستخدم حلقة `for` للتكرار على `odd_digits`.
...
يجب أن يحتوي جسم حلقة `for` على `--fcc-expected--`.
