...
El selector `.calories-info span` debe tener una propiedad `font-size` establecida en `2.4em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span')?.fontSize === '2.4em');
```

El selector `.calories-info span` debe tener una propiedad `font-weight` establecida en `700`.
