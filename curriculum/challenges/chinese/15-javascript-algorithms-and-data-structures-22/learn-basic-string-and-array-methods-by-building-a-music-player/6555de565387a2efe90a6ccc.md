...
`setPlayButtonAccessibleText` 函数将 `aria-label` 属性设置为当前歌曲，或者播放列表中的第一首歌曲。 如果播放列表为空，则将 `aria-label` 设置为 `"Play"`。

在 `playButton` 元素上使用 `setAttribute` 方法设置名为 `"aria-label"` 的属性。 使用三元组，将属性值设置为 `Play ${song.title}` 或 `"Play"`（如果 `song?.title` 不可用）。

不要忘记这里需要字符串插值，所以你需要使用反引号。

# --hints--

你不应修改现有的 `setPlayButtonAccessibleText` 函数及其内容。
...
你应该在 `playButton` 上使用 `setAttribute()` 方法。

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` 应该是 `setAttribute()` 方法的第一个值。

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

你的 `setAttribute` 方法应该有 ``song?.title ? `Play ${song.title}` : "Play"`` 作为第二个参数。
