...
残りのHTMLはすでに用意されています。プレミアムプランとファミリープランのカードも含まれています。では、Tailwind CSSを使ってHTML要素のスタイルを設定しましょう。

まず、ページ全体に薄いグレーの背景を設定するために、`body`要素に`bg-gray-100`というクラスを代入してください。

# --hints--

body要素には`bg-gray-100`というクラスを付けてください。
