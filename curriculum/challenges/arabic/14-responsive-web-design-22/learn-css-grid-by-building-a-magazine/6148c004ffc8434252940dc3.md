...
يجب أن يكون لديك منتقي `.social-icons`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

يجب أن يكون لمنتقي `.social-icons` الخاص بك الخاصية `display` بقيمة `grid`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display === 'grid');
```

يجب أن يكون لمنتقي `.social-icons` الخاص بك الخاصية `font-size` بقيمة `3rem`.
