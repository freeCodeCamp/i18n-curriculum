...
Cuando haces clic en una canción de la lista de reproducción quieres reproducir esa canción. Para eso necesitarás agregar un escuchador de eventos para el evento `click` en cada elemento `button` que es un hijo de un elemento `song`.

Completa el callback de tu `forEach` para hacer eso. Dentro del callback de `addEventListener`, llama a `playSong(n)`, donde `n` es el id de la canción que puedes obtener del atributo `id` de `song`.

Nota que cada `song` tiene un atributo `id` de `song-n`: necesitarás obtener la parte `n` y convertirla en un número antes de pasarla a `playSong()`.


# --hints--

Deberías agregar un escuchador de eventos para el evento `click` en cada elemento `button` que es un hijo de un elemento `.playlist-song` y llamar a `playSong` con el `id` de la canción correspondiente.
