...
In the block of the `if` statement, return an object spreading both `defaultPlayer` and `JSON.parse(saved)` separately. Make sure they're both separated by a comma, as they are two separate objects being merged into one.
