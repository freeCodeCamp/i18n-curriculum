...
Usando a mesma sintaxe, atualize `c` para que corresponda a `c`, `{`, `[` e `(`.

# --hints--

`stockRegex` deve usar uma classe de caractere que corresponda a `c`, `{`, `[` e `(`.
...
`stockRegex` deve corresponder a `570(k 4l3r7`.

```js
assert.match('570(k 4l3r7', stockRegex);
```

`stockRegex` deve corresponder a `sto{k alert`.
