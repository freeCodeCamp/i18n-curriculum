...
通常の関数と同様に、アロー関数も複数の引数を受け取ることができます。

仮引数を 1 つ持つ名前付きアロー関数の例がこちらです。
...
関数の仮引数が 1 つだけの場合は、引数リストの周りの丸括弧を次のように省略できます。
...
では、`printMessage` という名前のアロー関数を新たに作成し、1 つの仮引数 `org` を持たせてください。 その関数の中にコンソール文を追加してください。 そのコンソール文の括弧内に、テンプレートリテラルを使用して `${org} is awesome!` を追加してください。

`printMessage` 関数の下で関数を呼び出して、文字列 `"freeCodeCamp"` を引数として渡してください。

コンソールを開いて結果を確認してください。

# --hints--

`printMessage` 関数が必要です。
...
`printMessage` 関数には `org` という仮引数が必要です。
...
`printMessage` 関数内にコンソール文を持たせてください。

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

コンソール文にはテンプレートリテラル `${org} is awesome!` を含める必要があります。

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

`printMessage` 関数を呼び出し、文字列 `"freeCodeCamp"` を引数として渡してください。

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

関数呼び出しは `printMessage` 関数の定義より後に記述してください。
