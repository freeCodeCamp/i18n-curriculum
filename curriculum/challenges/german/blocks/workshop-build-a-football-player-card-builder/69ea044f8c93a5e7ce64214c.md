...
Geben Sie im Block der `if`-Anweisung ein Objekt zurück, das sowohl `defaultPlayer` als auch `JSON.parse(saved)` jeweils separat spreadet. Achten Sie darauf, dass sie durch ein Komma getrennt sind, da es sich um zwei separate Objekte handelt, die zu einem zusammengeführt werden.

# --hints--

Ihr `if (saved)`-Block sollte ein Objekt zurückgeben, das sowohl `defaultPlayer` als auch die geparsten gespeicherten Daten spreadet.
