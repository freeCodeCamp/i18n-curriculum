...
عنصران semantic HTML نهائيان لهذا المشروع هما `footer` و `address`. عنصر `footer` هو حاوية لمجموعة من المحتويات المرتبطة بالصفحة، وعنصر `address` هو حاوية لمعلومات الاتصال لكاتب الصفحة.

بعد عنصر `main`، أضف عنصر `footer` وادمج عنصر `address` واحد داخله.

# --hints--

يجب عليك إضافة عنصر `footer` واحد بعد عنصر `main`.

```js
assert.exists(document.querySelector('main + footer'));
```

يجب عليك دمج عنصر `address` واحد داخل عنصر `footer`.
