...
احذف `orange` من التدرج اللوني `.bb1d` وغيّر القيمة من `80%` إلى `50%`. سيجعل هذا `--building-color1` صلبًا للنصف العلوي، ثم ينتقل إلى `--window-color1` للنصف السفلي.

# --hints--

يجب حذف `orange` من `linear-gradient`.

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'orange');
```

يجب تغيير لون `linear-gradient` الأول الآن ليبدأ الانتقال عند `50%`.
