...
다음으로, 플레이어 화면에 현재 노래 제목과 아티스트를 표시해야 합니다. `const`와 화살표 구문을 사용해 빈 `setPlayerDisplay` 함수를 만드세요.

# --hints--

빈 함수를 만들려면 `const`를 사용하세요. 함수 이름은 `setPlayerDisplay`입니다.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay`는 함수여야 합니다.

```js
assert.isFunction(setPlayerDisplay)
```

`setPlayerDisplay` 함수는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

`setPlayerDisplay` 함수는 비어 있어야 합니다.
