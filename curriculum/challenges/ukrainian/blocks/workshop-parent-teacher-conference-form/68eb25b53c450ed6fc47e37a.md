...
За замовчуванням елементи `label` є рядковими. Щоб розташувати їх вертикально (окрім міток ваших радіокнопок), використовуйте псевдоклас `:not()`.

Псевдоклас `:not()` заперечує вибір. Тут він вибирає всі елементи `label`, які не мають класу `contact-method`.

Виберіть `label:not(.contact-method)`, потім встановіть `display` у `block` і `margin` у `10px 0`.

# --hints--

У вас має бути селектор `label:not(.contact-method)`.

```js
assert(new __helpers.CSSHelp(document).getStyle('label:not(.contact-method)'));
```

Встановіть властивість `display` у `block`.
...
Встановіть властивість `margin` у `10px 0`.
