...
Devi posizionare il cerchio interno e renderlo inizialmente invisibile.

Continuando a puntare `.contact-method-radio-btn::before`, imposta `transform` su `translate(3px, 3px) scale(0)`. La funzione `translate` sposta leggermente il cerchio, e `scale(0)` lo riduce a dimensioni zero in modo che sia nascosto per impostazione predefinita.

Inoltre, aggiungi una `transition` di `all 0.3s ease-in` per creare un’animazione fluida quando appare. Questo significa che qualsiasi modifica di stile verrà animata in 0,3 secondi, iniziando lentamente e accelerando.

# --hints--

Dovresti continuare a puntare al pseudo-elemento `.contact-method-radio-btn::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Imposta la proprietà `transform` su `translate(3px, 3px) scale(0)`.
...
Imposta la proprietà `transition` su `all 0.3s ease-in`.
