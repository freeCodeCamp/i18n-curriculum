...
在 `button` 元素内，再创建两个 `span` 元素。

第一个 `span` 元素应该具有 `playlist-song-artist` 的 `class`。 在 `span` 标签之间添加 `${song.artist}`。

第二个 `span` 元素应该具有 `playlist-song-duration` 的 `class`。 在 `span` 标签之间添加 `${song.duration}`。

# --hints--

你不应修改现有的 `button` 元素及其内容。
...
你应该创建一个带有类 `playlist-song-artist` 的 `span` 元素。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

你的具有 `playlist-song-artist` 类的 `span` 元素应该插入 `song.artist` 作为其内容。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

你应该创建一个带有类 `playlist-song-duration` 的 `span` 元素。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

你的具有 `playlist-song-duration` 类的 `span` 元素应该插入 `song.duration` 作为其内容。
