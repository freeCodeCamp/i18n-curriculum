...
الآن استدعِ الدالة `showElements` ومرر `multipleCard` و`fortuneContainer` و`text` كخصائص.

# --hints--

يجب أن تستدعي الدالة `showElements` داخل الطريقة `multipleCardSelected`.
...
يجب أن تمرر `multipleCard` كالمعلمة الأولى إلى الدالة `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

يجب أن تمرر `fortuneContainer` كمعلمة إلى الدالة `showElements`.
...
يجب أن تمرر `text` كمعلمة إلى الدالة `showElements`.
