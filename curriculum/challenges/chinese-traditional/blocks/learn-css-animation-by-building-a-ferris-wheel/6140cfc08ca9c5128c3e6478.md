...
將 `.line` 選擇器的 `position` 屬性設為 `absolute`，`left` 屬性設為 `50%`，以及 `top` 屬性設為 `50%`。

# --hints--

你的 `.line` 選擇器應該有一個設定為 `absolute` 的 `position` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.line')?.position === 'absolute');
```

你的 `.line` 選擇器應該有一個設定為 `50%` 的 `left` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.line')?.left === '50%');
```

你的 `.line` 選擇器應該有一個設定為 `50%` 的 `top` 屬性。
