...
`background-color`から`.fb2`へ`.fb2b`のプロパティと値を移動して、コンテナではなくセクションだけに色を付けてください。

# --hints--

`background-color`から`.fb2`のプロパティを削除してください。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb2")?.backgroundColor);
```

`.fb2b`に`background-color`の`--building-color3`を設定してください。
