...
Debes tener un selector `tr.total td`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td'));
```

Tu selector `tr.total td` debe tener una propiedad `text-align` establecida en `right`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td')?.getPropertyValue('text-align') === 'right');
```

Tu selector `tr.total td` debe tener una propiedad `padding` establecida en `0 0.25rem`.
