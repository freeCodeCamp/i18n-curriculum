...
Sarà utile avere una funzione per mostrare un riepilogo del throughput durante una simulazione. Crea una funzione vuota chiamata `printSummary` con un parametro `summary`.

# --hints--

Dovresti creare una funzione chiamata `printSummary`.

```js
assert.isFunction(printSummary);
```

La tua funzione `printSummary` dovrebbe avere un parametro `summary`.
