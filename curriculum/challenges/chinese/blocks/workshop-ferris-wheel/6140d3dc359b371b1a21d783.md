...
该把座舱安置在摩天轮周围了。 选择第一个 `.cabin` 元素。 将 `right` 属性设置为 `-8.5%`、`top` 属性设置为 `50%`。
...
你的 `.cabin:nth-of-type(1)` 选择器应该有一个 `right` 属性，设置为 `-8.5%`。
...
你的 `.cabin:nth-of-type(1)` 选择器应该有一个 `top` 属性，设置为 `50%`。
