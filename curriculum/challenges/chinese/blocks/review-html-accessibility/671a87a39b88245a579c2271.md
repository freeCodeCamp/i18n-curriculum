...
- **正确的标题级别结构**：你应该使用正确的标题级别来为你的内容创建一个逻辑结构。这有助于使用辅助技术的人理解你网站的内容。
- **无障碍与表格**：使用表格时，你应该使用 `th` 元素来定义头部单元格，使用 `td` 元素来定义数据单元格。这有助于使用辅助技术的人理解表格的结构。通过 `caption` 元素，你可以为表格写标题（或标题），这样用户，尤其是使用辅助技术的用户，可以快速理解表格的目的和内容。你应该将 `caption` 元素放在 `table` 元素的起始标签之后。这样，屏幕阅读器和其他辅助技术可以通过在读取内容之前宣布标题来提供更多上下文。
- **输入框关联标签的重要性**：你应该使用 `label` 元素将标签与表单输入关联起来。这有助于使用辅助技术的人理解输入的用途。
- **`span` 元素与无障碍**：`span` 元素是一个通用的内联容器，用于包裹内容。它通常用于对一段文本进行不同于周围文本的样式，或者包裹文本并赋予一个 `id`，以便其他元素（例如 ARIA 属性）可以引用。
- **良好 `alt` 文本的重要性**：你应该使用 `alt` 属性为图像提供文本备选。这有助于使用辅助技术的人理解图像的内容。
- **良好链接文本的重要性**：你应该使用描述性的链接文本，帮助用户理解链接的目的。这有助于使用辅助技术的人理解链接的用途。
- **使音频和视频内容无障碍的最佳实践**：你应该为音频和视频内容提供字幕和文字记录，使其对听力障碍者可访问。你还应该为视频内容提供音频描述，使其对视觉障碍者可访问。
- **`tabindex` 属性**：用于使元素可聚焦，并定义使用键盘导航时的相对顺序。重要的是，绝不要使用大于 0 的 `tabindex` 值。你应该使用 0 或 -1。
...
- **窗口角色**：这类角色定义了子窗口，如弹出模态对话。 这些角色包括 `alertdialog` 和 `dialog`。
- **抽象角色**：这类角色有助于组织文档。 因此，你应该知道它们的存在，但不应该在网站或网络应用程序中使用它们。

- **`aria-label` 和 `aria-labelledby` 属性**：这些属性用于为元素定义一个程序化（或可访问的）名称，帮助使用辅助技术（例如屏幕阅读器）的人理解该元素的用途。当元素的视觉标签是图像或符号而非文本时，它们经常被使用。`aria-label` 允许你直接在属性中定义名称，而 `aria-labelledby` 允许你引用页面上已有的文本。
