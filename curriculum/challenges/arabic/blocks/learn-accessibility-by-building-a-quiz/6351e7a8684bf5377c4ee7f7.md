...
على الرغم من أن ربط عنصر `label` بعنصر `input` المتداخل ليس ضروريًا، إلا أنه من الأفضل ربط `label` بعنصر `input` المقابل له بشكل صريح.

أضف الآن خاصية `for` لكل من عناصر `label` الأربعة لربط كل `label` بعنصر `input` الخاص به من نوع الراديو.

# --hints--

يجب أن تعطي أول `label` خاصية `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

يجب أن تعطي أول `label` خاصية `for` تطابق `id` الخاص بعنصر `input` الخاص به.
...
يجب أن تعطي ثاني `label` خاصية `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

يجب أن تعطي ثاني `label` خاصية `for` تطابق `id` الخاص بعنصر `input` الخاص به.
...
يجب أن تعطي ثالث `label` خاصية `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

يجب أن تعطي ثالث `label` خاصية `for` تطابق `id` الخاص بعنصر `input` الخاص به.
...
يجب أن تعطي رابع `label` خاصية `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

يجب أن تعطي رابع `label` خاصية `for` تطابق `id` الخاص بعنصر `input` الخاص به.
