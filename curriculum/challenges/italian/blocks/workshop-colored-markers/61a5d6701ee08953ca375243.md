...
Come secondo argomento colore, passa la funzione `hsl` con i valori `223` per la tonalità, `90%` per la saturazione e `60%` per la luminosità.

# --hints--

La tua regola CSS `.blue` dovrebbe avere una proprietà `background` impostata su `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%))`.
