...
title: Construir um Aplicativo de Orçamento
challengeType: 27
...
1. Você deve ter uma classe `Category` que aceita um nome como argumento.
...
1. Quando um objeto `Category` for impresso, ele deve:  
   - Exiba uma linha de título de 30 caracteres com o nome da categoria centralizado entre os caracteres `*`.  
   - Liste cada entrada do `ledger` com até 23 caracteres da sua descrição alinhados à esquerda e o valor alinhado à direita (duas casas decimais, máximo de 7 caracteres).  
   - Mostre uma linha final `Total: [balance]`, onde `[balance]` deve ser substituído pelo total da categoria.
...
1. Você deve ter uma função fora da classe `Category` chamada `create_spend_chart(categories)` que retorna uma string de gráfico de barras.Para construir o gráfico:

   - Comece com o título `Percentage spent by category`.
   - Calcule percentuais apenas a partir de saques e não de depósitos. O percentual deve ser o percentual do valor gasto para cada categoria em relação ao total gasto em todas as categorias (arredondado para o múltiplo de 10 mais próximo).
   ...
   Certifique-se de corresponder exatamente ao espaçamento da saída do exemplo:
