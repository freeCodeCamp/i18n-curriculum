...
Unapobofya kitufe cha kuchanganya, unataka kuweka `currentSong` kuwa tupu na `songCurrentTime` kuwa `0`.

Weka `userData.currentSong` kuwa `null` na `userData.songCurrentTime` kuwa `0`.

*Kumbuka*: Haufai kutumia uunganishaji wa mfululizo kwa hatua hii kwa sababu unakuwa unaweka wazi mali za `currentSong` na `songCurrentTime` kuwa `null` na `0` mtawalia.

# --hints--

Unapaswa kuweka `userData.currentSong` kuwa `null`.

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

Unapaswa kuweka `userData.songCurrentTime` kuwa `0`.
