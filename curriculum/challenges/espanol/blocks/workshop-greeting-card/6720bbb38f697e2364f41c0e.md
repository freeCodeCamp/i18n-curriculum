...
Ahora es momento de añadir un poco de color. Dale al `body` un `background-color` de `brown` y también dale al `.card` un `background-color` de `white`.

# --hints--

El elemento `body` debería tener un `background-color` de `brown`.
...
Deberías tener un selector `.card`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card"));
```

El elemento `.card` debería tener un `background-color` de `white`.
