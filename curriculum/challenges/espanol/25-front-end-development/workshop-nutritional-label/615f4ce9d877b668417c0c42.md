...
Envuelve todo dentro del elemento `.label` en un nuevo elemento `header`.

# --hints--

Debe crear un nuevo elemento `header`.

```js
assert(document.querySelector('header'));
```

El elemento `header` debe estar dentro del elemento `.label`.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

Los elementos `h1`, `div`, y `p` deben estar dentro del nuevo elemento `header`.
