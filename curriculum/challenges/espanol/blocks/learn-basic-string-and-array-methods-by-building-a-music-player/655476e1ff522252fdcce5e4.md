...
Añade un bloque `else` a la declaración `if`. Dentro del bloque `else`, llama a la función `getCurrentSongIndex()` y asígnalo a una constante nombrada `currentSongIndex`.

# --hints--

No debes modificar la declaración `if` existente.
...
Deberías añadir un `else` a la declaración `if` existente.
...
Deberías establecer la constante `currentSongIndex` en `getCurrentSongIndex()` dentro del bloque `else`.
