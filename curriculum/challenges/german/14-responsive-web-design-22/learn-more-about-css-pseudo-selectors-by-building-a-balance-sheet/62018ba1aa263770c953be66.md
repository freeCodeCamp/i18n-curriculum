...
Dein `table caption`-Selektor sollte die `position`-Eigenschaft auf `absolute` gesetzt haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('position') === 'absolute');
```

Dein `table caption`-Selektor sollte die `top`-Eigenschaft auf `-2.25rem` gesetzt haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('top') === '-2.25rem');
```

Dein `table caption`-Selektor sollte die `left`-Eigenschaft auf `0.5rem` gesetzt haben.
