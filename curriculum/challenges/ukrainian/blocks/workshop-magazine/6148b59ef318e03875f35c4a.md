...
Створіть селектор `img` і надайте йому множину властивостей `width` зі значенням `100%` та `object-fit` зі значенням `cover`.

Властивість `object-fit` вказує браузеру, як розташувати елемент у межах його контейнера. У цьому випадку `cover` встановить зображення так, щоб воно заповнювало контейнер, обрізаючись за потреби, щоб не змінювати співвідношення сторін.

# --hints--

У вас має бути селектор `img`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('img'));
```

Ваш селектор `img` має мати властивість `width` зі значенням `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('img')?.width, '100%');
```

Ваш селектор `img` має мати властивість `object-fit` зі значенням `cover`.
