...
在 `if` 述語之後，使用三元運算子來檢查以 `messageInput.value` 作為引數呼叫 `isSpam()` 的真值。如果為真，將 `result` 元素的 `textContent` 屬性設為 `"Oh no! This looks like a spam message."`。否則，將其設為 `"This message does not seem to contain any spam."`

然後將 `messageInput` 元素的 `value` 屬性設為空字串。

# --hints--

你應該使用指定運算子來設值 `result` 元素的 `textContent` 屬性。
...
你應該在指定運算子 `=` 之後且 `?` 三元運算子之前呼叫 `isSpam()` 函式（程式）。
...
你應該使用三元語法來檢查 `isSpam(messageInput.value)` 的真值。
...
你的三元運算式的真值表達式應該將 `result` 元素的 `textContent` 屬性設為 `"Oh no! This looks like a spam message."`。
...
你的三元運算式的假值表達式應該將 `result` 元素的 `textContent` 屬性設為 `"This message does not seem to contain any spam."`。
...
在你的三元運算子之後，將 `messageInput` 元素上的 `value` 屬性設為空字串。
