...
A continuación, utiliza la propiedad `classList` y el método `add()` para agregar la clase `"playing"` al elemento `playButton`. Esto buscará la clase `"playing"` en el archivo CSS y la agregará al elemento `playButton`.

Para finalmente reproducir la canción, utiliza el método `play()` en la variable `audio`. `play()` es un método de la API de audio web para reproducir un archivo mp3.

# --hints--

Debe utilizar la propiedad `classList` y el método `add` para agregar la clase `"playing"` a `playButton`.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

Debe utilizar el método `play()` en la variable `audio`.
