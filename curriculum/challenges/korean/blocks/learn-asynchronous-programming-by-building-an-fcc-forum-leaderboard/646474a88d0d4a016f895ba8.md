...
남은 단계에서는 사용자 아바타를 표시하는 기능을 추가하세요.

두 번째 `td` 요소 안에 `div` 요소를 추가하고, 클래스 이름을 `"avatar-container"`로 지정하세요.

# --hints--

두 번째 `div` 요소 안에 `td` 요소가 있어야 합니다.
...
열리는 `div` 태그에 클래스 속성으로 `"avatar-container"` 값을 지정하세요.
