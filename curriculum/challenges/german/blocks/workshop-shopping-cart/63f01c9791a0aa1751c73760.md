...
Ihre `ShoppingCart`-Klasse benötigt noch weitere Funktionen, aber zuerst müssen Sie den bisher geschriebenen Code testen können. Sie müssen ein neues `ShoppingCart`-Objekt <dfn>instanziieren</dfn> und es einer Variablen zuweisen. Hier ist ein Beispiel für das Instanziieren der `Computer`-Klasse aus früheren Beispielen:

```js
const myComputer = new Computer();
```

Deklarieren Sie eine `cart`-Variable und weisen Sie ihr ein neues `ShoppingCart`-Objekt zu. Beachten Sie die Verwendung des Schlüsselworts `new` beim Instanziieren des Objekts.

# --hints--

Sie sollten `const` verwenden, um eine `cart`-Variable zu deklarieren.

```js
assert.match(code, /const\s+cart\s*=/);
```

Sie sollten das Schlüsselwort `new` verwenden, um ein neues `ShoppingCart`-Objekt zu instanziieren.

```js
assert.match(code, /new\s+ShoppingCart\s*\(\s*\)/);
```

Sie sollten Ihr neues `ShoppingCart`-Objekt der `cart`-Variablen zuweisen.
