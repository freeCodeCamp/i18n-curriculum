...
`PlayerData` 接口目前只描述了基本的玩家简介。你需要用卡片所需的剩余属性来扩展它。

在接口内部，添加一个类型为 `imageUrl` 的 `string`，以及六个数值型属性：`pac`、`sho`、`pas`、`dri`、`def` 和 `phy`，它们的类型均为 `number`。

# --hints--

你的 `PlayerData` 接口应该有一个类型为 `imageUrl` 的 `string`。
...
你的 `PlayerData` 接口应该有一个类型为 `pac` 的 `number`。
...
你的 `PlayerData` 接口应该有一个类型为 `sho` 的 `number`。
...
你的 `PlayerData` 接口应该有一个类型为 `pas` 的 `number`。
...
你的 `PlayerData` 接口应该有一个类型为 `dri` 的 `number`。
...
你的 `PlayerData` 接口应该有一个类型为 `phy` 的 `number`。
