...
預設情況下，像圓鈕這類的表單元素可能不會與它們的標籤完美對齊，特別是當標籤文字較高或有不同字型設定時。

要將圓鈕與其標籤垂直對齊，你可以使用 `vertical-align` 屬性。此屬性控制行內或行內區塊元素與周圍文字的垂直排列方式。

現在，將 `vertical-align` 設為 `bottom` 用於圓鈕。你可以嘗試其他值，例如 `middle` 或 `top`，來查看它們如何影響對齊，但 `bottom` 通常適用於標籤旁的圓鈕。

# --hints--

你應該繼續以 `.contact-method-radio-btn` 類別為標的。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

將 `vertical-align` 屬性設為 `bottom`。
