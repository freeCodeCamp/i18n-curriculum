...
Luego, agrega un enlace a la página del autor en freeCodeCamp News.

Agrega un elemento de anclaje con un atributo `class` de `"author-link"`, interpola `url` como el valor para el atributo `href`, y configura `target` a `"_blank"`. Para el texto del elemento de anclaje, interpola `author` seguido por el texto `"'s author page"`. Por ejemplo, `"Quincy Larson's author page"`.
...
Debes crear un elemento de anclaje.
...
Tu elemento de anclaje debe tener la clase `"author-link"`.

```js
assert.include(document.querySelector('a')?.className, "author-link");
```

Debes interpolar `url` como el valor del atributo `href` de tu ancla.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), authorDataArr[0].url);
```

Debes establecer el atributo `target` de tu elemento de anclaje a `"_blank"`.

```js
assert.equal(document.querySelector('a')?.getAttribute('target'), '_blank');
```

Debes interpolar `author` seguido por el texto `"'s author page"` dentro de tu elemento de anclaje.
