...
إذا كان هناك تطابق، عيّن `userData.currentSong` إلى `null` و`userData.songCurrentTime` إلى `0`.

بعد ذلك، استدعِ الدالة `pauseSong()` لإيقاف التشغيل والدالة `setPlayerDisplay()` لتحديث عرض المشغل.

# --hints--

لا تعدل جملة `if` الموجودة ومحتواها.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\?\.id\s*===\s*id\s*\)\s*\{\s*.*\s*.*\s*.*\s*.*\s*\}/)
```

عيّن `userData.currentSong` إلى `null`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\?\.id\s*===\s*id\s*\)\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*.*\s*.*\s*\}/)
```

عيّن `userData.songCurrentTime` إلى `0`.
...
استدعِ الدالة `setPlayerDisplay`.
