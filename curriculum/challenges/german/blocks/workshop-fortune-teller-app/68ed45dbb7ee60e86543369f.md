...
Nachdem Sie die Daten abgerufen haben, speichern Sie die Antwort in einer Variablen und parsen Sie sie zu JSON. Geben Sie den geparsten Daten den Typ `Deck` und weisen Sie sie dann der `cards`-Eigenschaft zu. Stellen Sie im `catch`-Block sicher, dass Sie den Fehler mit dieser Nachricht protokollieren: `"Error fetching cards:", error`.

# --hints--

Sie sollten eine Variable namens `data` innerhalb von `fetchCardsData` deklarieren.
...
Ihre `data`-Variable sollte den Typ `Deck` haben.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

Ihre `data`-Variable sollte durch Parsen der Antwort als JSON zugewiesen werden.
...
Sie sollten `data.cards` in `this.cards` speichern.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Ihr `catch`-Block sollte `console.error()` verwenden, um den Fehler mit der Nachricht `"Error fetching cards:", error` zu protokollieren.
