...
ブラウザーには組み込みの `alert()` 関数があり、これを使ってユーザーにポップアップメッセージを表示することができます。 表示するメッセージは `alert()` 関数の引数として渡されます。

`if` ブロック内で `alert()` 関数を呼び出して、ユーザーにメッセージを表示しましょう。テンプレートリテラルを使用して、`"Invalid Input: "` の後に続けて `invalidInputMatch` 配列内の最初の値を表示してください。

# --hints--

`if` ブロック内で `alert()` 関数を呼び出してください。

```js
assert.match(getCaloriesFromInputs.toString(), /if\s*\(\s*invalidInputMatch\s*\)\s*\{\s*(?:window\.|globalThis\.)?alert\(/);
```

テンプレートリテラルを使用して、`alert()` 関数に `"Invalid Input: "` というメッセージを渡してください。
...
テンプレートリテラルを使用して、`"Invalid Input: "` というテキストの後に `invalidInputMatch` 配列の最初の要素を表示してください。
