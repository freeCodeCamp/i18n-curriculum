...
عيّن قيمة الخاصية `msg` إلى السلسلة النصية التالية:
...
يجب أن يحتوي كائن التحريك في أعلى المكدس على الخاصية `msg`.
...
يجب أن تعيّن قيمة الخاصية `msg` إلى `"decimalToBinary(1) returns '1' (base case) and gives that value to the stack below. Then it pops off the stack."`
