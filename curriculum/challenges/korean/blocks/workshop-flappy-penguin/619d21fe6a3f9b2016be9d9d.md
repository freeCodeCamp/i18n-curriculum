...
`.foot` 요소를 `class`인 `left`로 선택하고, 부모의 왼쪽에 `25%` 위치시키세요. 그런 다음 `.foot` 요소를 `class`인 `right`로 선택하고, 부모의 오른쪽에 `25%` 위치시키세요.

# --hints--

`.foot.left` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.foot.left', '.left.foot']));
```

`.foot.left`에 `left`인 `--fcc-expected--`을 지정해야 하지만 `--fcc-actual--`이(가) 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.foot.left', '.left.foot'])?.left, '25%');
```

`.foot.right` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.foot.right', '.right.foot']));
```

`.foot.right`에 `right`인 `--fcc-expected--`을 지정해야 하지만 `--fcc-actual--`이(가) 발견되었습니다.
