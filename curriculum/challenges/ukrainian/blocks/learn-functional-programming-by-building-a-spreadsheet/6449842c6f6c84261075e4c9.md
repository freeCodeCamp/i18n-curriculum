...
У вашій `evalFormula` оголосіть `idToText` стрілкову функцію, яка приймає `id` параметр.

Ваша `idToText` функція має повертати результат виклику `.find()` на масиві `cells` з функцією зворотного виклику, яка приймає `cell` параметр і повертає `cell.id === id`.

Обидві ваші функції мають використовувати неявні повернення.

# --hints--

Ви маєте оголосити `idToText` змінну у вашій `evalFormula` функції.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*(?:const|let|var)\s+idToText/);
```

Ви маєте використовувати `const` для оголошення вашої `idToText` змінної.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText/);
```

Ваша `idToText` змінна має бути стрілковою функцією.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

Ваша `idToText` функція має мати `id` параметр.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>/);
```

Ваша `idToText` функція має повертати результат виклику методу `.find()` на вашому масиві `cells`. Ваша функція зворотного виклику має використовувати неявне повернення.
...
Ваша функція зворотного виклику має повертати, чи `cell.id` строго дорівнює `id`.
