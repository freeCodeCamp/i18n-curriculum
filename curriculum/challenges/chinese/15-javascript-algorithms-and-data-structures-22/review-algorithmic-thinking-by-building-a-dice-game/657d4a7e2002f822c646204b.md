...
用户做出选择后，应该能够保留该分数并点击 `keepScoreBtn` 进入下一轮。

当该按钮被点击时，你应该找到被选中的单选项并获取其值和值属性。 如果用户已选择一个选项，调用你的函数来更新分数，重置单选项，并将该值和值添加到分数历史中。

如果用户未选择选项，则显示一个警告提醒他们这样做。

# --hints--

当你的 `keepScoreBtn` 被点击时，分数应在 `totalScoreElement` 文本中更新。
...
当你的 `keepScoreBtn` 被点击时，分数历史应被更新。
...
当你的 `keepScoreBtn` 被点击时，未选择任何选项时，你应向用户显示一个警告。
