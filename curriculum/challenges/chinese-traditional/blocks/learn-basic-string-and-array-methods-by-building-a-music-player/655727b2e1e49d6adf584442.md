...
如果播放列表是空的，你需要創建一個 `resetButton` 元素和對應的文字。只有當播放列表是空的時候，這個按鈕才會顯示。

`createElement()` 是一個 DOM 方法，你可以使用 JavaScript 動態創建元素。要使用 `createElement()`，你需要呼叫它，然後傳入頁籤名稱作為字串：
...
在你的 `if` 述語中，宣告一個 `resetButton` 常數，然後使用 `createElement()` 來創建一個 `"button"`。
...
你應該使用 `createElement` 方法來創建一個 `"button"`。

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

你應該將你新創建的 `button` 元素指定給一個 `resetButton` 常數。
