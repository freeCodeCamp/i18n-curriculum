...
创建一个 `if` 语句，检查从 `gameboard` 对象调用 `solver()` 方法是否返回 `True`。

然后，在 `if` 主体内添加一个 `print()` 调用，传递以下 f 字符串：`f'Solved puzzle:\n{gameboard}'`。


# --hints--

你应该创建一个 `if` 语句来检查从 `gameboard` 对象调用 `solver()` 方法是否返回 `True`。
...
你应该在新的 `if` 正文中打印 `f'Solved puzzle:\n{gameboard}'` 。
