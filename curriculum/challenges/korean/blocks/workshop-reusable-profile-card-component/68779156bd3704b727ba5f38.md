...
이전 수업에서 기억하듯이, `map` 메서드를 사용해 다양한 객체를 순차적으로 반복하며 컴포넌트를 동적으로 렌더링할 수 있습니다:
...
`map()` 메서드를 사용해 배열에서 단일 `Card`를 렌더링하세요. `name`, `title`, 그리고 `bio`를 `Card` 컴포넌트에 프로프스로 전달하세요.

# --hints--

`map()` 메서드를 사용해 `profiles` 배열을 순차적으로 반복해야 합니다.

```js
assert.match(code, /profiles\.map\s*\(/);
```

`name` 객체에서 `title`, `bio`, 그리고 `profile` 프로프스를 전달해야 합니다.
