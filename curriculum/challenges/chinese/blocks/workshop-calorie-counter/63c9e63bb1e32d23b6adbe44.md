...
当你需要将字符串转换为小写时，可以使用 <dfn>toLowerCase()</dfn> 方法。 此方法返回转换为小写的调用字符串值。
...
你的 `output.innerHTML` 字符串将需要一个 `span` 元素。 创建它，并赋予它一个 `class` 属性，并将其设置为 `surplusOrDeficit` 变量。 你的 `surplusOrDeficit` 变量应使用 `toLowerCase()` 方法转换为小写。

暂时不要为 `span` 添加任何文本。

# --hints--

你应该在模板文字中创建一个 `span` 元素。
...
你的 `span` 应将 `class` 属性设置为 `surplusOrDeficit.toLowerCase()`。
