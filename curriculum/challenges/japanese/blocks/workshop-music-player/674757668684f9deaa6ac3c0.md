...
まだ`playSong`関数の中にいて、`audio.src`プロパティを`song.src`に設定してください。これにより、オーディオ要素が選択された曲のオーディオデータの場所を認識します。

また、`audio.title`プロパティを`song.title`に設定してください。これにより、オーディオ要素が曲のタイトルとして何を表示するかが指定されます。

# --hints--

`audio.src`関数の中で`song.src`プロパティを`playSong`に設定する必要があります。
...
`audio.title`関数の中で`song.title`プロパティを`playSong`に設定する必要があります。
