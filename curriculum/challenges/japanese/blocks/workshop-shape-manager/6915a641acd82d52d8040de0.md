...
`initializeApp` 関数に戻り、ここで先ほど作成したインライン型と `getElement()` 関数を使って、アプリに必要な DOM 要素を取得します。

`initializeApp` 関数内で、`shapeTypeSelect` を `getElement()` で呼び出して `"shape-type"` に再代入し、その結果を `as` キーワードで `HTMLSelectElement` にキャストしてください。

# --hints--

適切な値で `shapeTypeSelect` を再代入してください。

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

`getElement` 関数を `shape-type` で呼び出し、その結果を `shapeTypeSelect` に代入してください。

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

`getElement` を `HTMLSelectElement` の型として `shapeTypeSelect` に `as` キーワードを使って呼び出してください。
