...
你可以通過將 `submit` 事件監聽器重構爲兩個獨立的函數來增強代碼的可讀性和可維護性。 第一個函數可用於將輸入值添加到 `taskData`，而第二個函數可用於將任務添加到 DOM。

使用箭頭語法創建 `addOrUpdateTask` 函數。 然後將 `dataArrIndex` 變量、`taskObj` 對象和 `if` 語句移入 `addOrUpdateTask` 函數中。

# --hints--

你應該使用 `const` 和箭頭語法來創建 `addOrUpdateTask` 函數。

```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

你應該將 `dataArrIndex` 變量移入 `addOrUpdateTask` 函數中。
...
你應該將 `taskObj` 對象移入 `addOrUpdateTask` 函數中。
...
你應該將帶有條件 `dataArrIndex === -1` 的 `if` 語句移到 `addOrUpdateTask` 函數中。
