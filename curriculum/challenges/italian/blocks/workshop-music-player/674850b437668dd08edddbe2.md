...
Nei passaggi precedenti hai costruito la funzionalità per riprodurre una canzone. Ora devi aggiungere la funzionalità al pulsante play.

Usa il metodo `addEventListener()` su `playButton` e passa un evento `"click"` come primo argomento. Per ora, usa una funzione di callback che chiama `playSong(0)` come secondo argomento.

Poi, testalo cliccando sul pulsante play.


# --hints--

Dovresti aggiungere un listener per l'evento `click` a `playButton` e passare una funzione di callback che chiama `playSong(0)` come secondo argomento al metodo `addEventListener`.
