...
ようこそ！これから30から39までの数字を学びます。

スペイン語で30は `treinta` と呼ばれます。

31から39までの数字は、10の位の言葉 `treinta`（30）に、接続詞 `y`（「と」）を続け、その後に1から9までの数字をつけて作られます。
...
この範囲で数字 `treinta y uno`（31）は特別で、男性名詞の前では `treinta y un` に変わります。例えば：

`Treinta y un años` - 31歳。
