...
El otro texto se ha desplazado fuera de lugar. Muévelo a la posición correcta dando al selector `.first-paragraph::first-letter` una propiedad `float` establecida en `left` y una propiedad `margin-right` establecida en `1rem`.

# --hints--

Tu selector `.first-paragraph::first-letter` debe tener una propiedad `float` establecida en `left`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

Tu selector `.first-paragraph::first-letter` debe tener una propiedad `margin-right` establecida en `1rem`.
