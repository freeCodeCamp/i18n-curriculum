...
Ili kuonyesha kichwa cha mada, ongeza kipengele cha `p` ndani ya kipengele cha kwanza cha `td`.

Kati ya lebo za aya, ongeza usemi wa kihesabu uliyoingizwa unaoonyesha tofauti ya `title`. Kisha ongeza darasa la vitu linaloitwa `"post-title"` ndani ya lebo ya kufungua ya aya.

# --hints--

Unapaswa kuwa na kipengele cha `p` ndani ya kipengele cha kwanza cha `td`.

```js
assert(/return\s*`\s*<tr>\s*<td>\s*\S*\s*<p.*>\s*.*\s*<\/p>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```

Unapaswa kuwa na darasa la vitu linaloitwa `"post-title"` ndani ya lebo ya kufungua ya `p`.
...
Unapaswa kuwa na tofauti ya `title` kati ya lebo za aya.
