...
你的 `decimalToBinary` 函数运行良好，但存在一个问题 - 由于你的 `while` 循环中的条件，它仅适用于大于 `0` 的数字。 如果你尝试将 `0` 转换为二进制，则不会向页面添加任何内容。

为了解决这个问题，请添加一个 `if` 语句来检查 `input` 是否等于 `0`。 暂时将 `if` 语句的主体保留为空。

# --hints--

你应该添加一个 `if` 语句来检查 `input` 是否等于 `0`。
