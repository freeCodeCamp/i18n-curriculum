...
对于你的假值表达式，你需要向 `productsContainer` 添加新的 HTML。 首先删除 `undefined`，然后使用加法赋值运算符和模板字面量语法将 `div` 添加到 `productsContainer` 的 `innerHTML` 属性中，并将 `class` 设置为 `product`，将 `id` 设置为 `dessert${id}`。 

# --hints--

你应该从虚假表达式中删除 `undefined`。
...
你应该使用加法赋值运算符将 HTML 添加到 `productsContainer`。 请记住，HTML 位于 `innerHTML` 属性中。
...
你应该使用模板字面量语法将 HTML 添加到 `productsContainer`。

```js
assert.match(code, /productsContainer\.innerHTML\s*\+=\s*`/);
```

你应该将 `div` 添加到 `productsContainer`。
