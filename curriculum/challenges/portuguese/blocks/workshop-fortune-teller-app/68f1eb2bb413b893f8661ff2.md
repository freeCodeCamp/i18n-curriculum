...
Neste ponto, você já deve conseguir ver a previsão que cada carta mostra ao clicar nela. A única coisa que falta é fazer o botão de nova leitura funcionar.

Dentro do método `newReading`, faça a chamada das funções `showElements()` e `hideElements()`.

# --hints--

Você deve chamar a função `showElements` dentro do método `newReading`.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

Você deve chamar a função `hideElements` dentro do método `newReading`.
