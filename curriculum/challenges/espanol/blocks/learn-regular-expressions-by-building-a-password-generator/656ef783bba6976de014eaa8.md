...
Coloca la declaración de tu variable `password` y el siguiente ciclo `for` dentro de un ciclo `while`. Usa `True` como la condición para tu nuevo ciclo.

# --hints--

Debes crear un bucle `while True` que englobe tu declaración existente de `password` y el bucle `for`.
