...
Alla fine della tua funzione, restituisci i dati che hai calcolato così possono essere usati all'esterno della funzione.

Restituisci sia la lista dei percorsi più brevi `distances` dal `start_node` a tutti gli altri nodi sia la lista dei `paths` che portano a ogni nodo

# --hints--

Dovresti avere un'istruzione `return` alla fine della tua funzione.
...
Dovresti restituire sia `distances` sia `paths`.
