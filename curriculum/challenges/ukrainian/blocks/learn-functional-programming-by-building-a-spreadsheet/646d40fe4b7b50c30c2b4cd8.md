...
Ваша `infixEval` функція оцінить лише першу операцію множення або ділення, тому що `regex` не є глобальним. Це означає, що вам слід використати рекурсивний підхід для оцінки всього рядка.

Якщо `infixEval` не знайде жодних співпадінь, вона поверне значення `str` без змін. Використовуючи тернарний вираз, перевірте, чи `str2` дорівнює `str`. Якщо так, поверніть `str`, інакше поверніть результат виклику `highPrecedence()` на `str2`.

# --hints--

Ваша `highPrecedence` функція має використовувати ключове слово `return`.
...
Ви маєте використовувати ключове слово `return` з умовою, щоб перевірити, чи `str` дорівнює `str2`.
...
Ви маєте використовувати тернарний синтаксис у вашій `return` інструкції.
...
Якщо тернарна умова істинна, ви маєте повернути `str`.
...
Якщо тернарна умова хибна, ви маєте повернути результат виклику `highPrecedence()`.
...
Ви маєте передати `str2` у ваш виклик `highPrecedence()`.
