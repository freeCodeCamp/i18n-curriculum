...
أنشئ دالة باسم `isSelect` تأخذ معلمة واحدة `element` من النوع `EventTarget | null`. تتحقق هذه الدالة مما إذا كان العنصر المحدد هو عنصر HTML من نوع `<select>`.

# --hints--

يجب أن يكون لديك دالة باسم `isSelect`.
...
يجب أن تحتوي دالة `isSelect` على معلمة باسم `element` من النوع `EventTarget | null`.
...
يجب أن تكون نوع القيمة المرجعة في دالة `isSelect` هو `element is HTMLSelectElement`.
