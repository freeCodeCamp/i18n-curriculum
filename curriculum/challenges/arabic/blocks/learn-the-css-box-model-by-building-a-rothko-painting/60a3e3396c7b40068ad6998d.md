...
أنشئ قاعدة تستهدف كلًا من `.one` و `.two` وزد تأثير `blur` الخاص بهما بمقدار 1 بكسل.

# --hints--

يجب أن تحتوي على قائمة محددات `.one, .two`.
...
يجب تعيين خاصية `filter` إلى `blur(1px)`.
...
يجب أن يحتوي عنصر `.one` على قيمة `filter` تساوي `blur(1px)`.
...
يجب أن يحتوي عنصر `.two` على قيمة `filter` تساوي `blur(1px)`.
