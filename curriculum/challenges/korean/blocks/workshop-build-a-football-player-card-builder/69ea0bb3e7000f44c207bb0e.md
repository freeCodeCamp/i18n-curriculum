...
localStorage 기능이 제대로 작동하도록 상태 초기화를 `defaultPlayer`에서 `loadPlayer`로 변경하세요. 이렇게 하면 상태가 사용 가능한 저장된 데이터로 시작합니다.

이제 localStorage 기능을 테스트해 보세요. 입력한 모든 내용은 미리보기 새로 고침이나 페이지를 다시 불러온 후에도 유지되어야 합니다.

또한 이미지 `src` 링크를 `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png`로 변경하여 이미지가 실시간으로 바뀌는 것을 확인할 수 있습니다. 단, 선수 이름은 반드시 Maradona로 변경하세요.

이로써 축구 선수 카드 빌더 프로젝트가 완성되었습니다!

# --hints--

`useState`는 `loadPlayer` 대신 `defaultPlayer`로 초기화되어야 합니다.
