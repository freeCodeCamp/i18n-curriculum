...
Passiamo all’edificio successivo! È quello verde in primo piano. Dagli un `repeating-linear-gradient` con il colore del tuo edificio da `0%` a `5%`, e `transparent` da `5%` a `10%`.

# --hints--

Dovresti dare a `.fb5` una proprietà `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

Dovresti dare a `background` una `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Dovresti dare a `repeating-linear-gradient` un primo colore `--building-color2` da `0%` a `5%`.
...
Dovresti dare a `repeating-linear-gradient` un secondo colore `transparent` da `5%` a `10%`.
