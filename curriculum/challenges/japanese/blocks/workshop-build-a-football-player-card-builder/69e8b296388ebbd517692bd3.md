...
`stats-grid`の中に、`div`の`className`を持つ`form-group`要素を2つ追加してください。

1つ目には、`label`の`className`、`label`の`htmlFor`、テキスト`pac`を持つ`PAC`を追加し、その後に`id`の`pac`、`className`の`input`、`type`の`number`を持つ入力を追加してください。

2つ目には、`label`のテキストと`htmlFor`の`sho`を持つ`SHO`を追加し、その後に`id`の`sho`と`type`の`number`を持つ入力を追加してください。

# --hints--

`div`の`className`の`form-group`の中に、`div`の`className`を持つ`stats-grid`要素が2つあるはずです。
...
最初の`div`の`className`の`form-group`要素には、`label`の`htmlFor`属性、`pac`の`className`、テキスト`label`を持つ`PAC`があるはずです。
...
最初の`div`の`className`の`form-group`要素には、`input`の`className`、`input`の`id`、`pac`の`type`を持つ`number`があります。
...
2つ目の`div`の`className`の`form-group`要素には、`label`の`htmlFor`属性、`sho`の`className`、テキスト`label`を持つ`SHO`があります。
...
2つ目の`div`の`className`の`form-group`要素には、`input`の`id`、`sho`の`className`、`input`の`type`を持つ`number`があります。
