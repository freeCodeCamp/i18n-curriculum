...
現在更新你在 `addCharacters` 串鏈中最內層的函式，讓它隱喻地傳回以 `character1` 和 `character2` 作為引數呼叫 `charRange()` 的結果。
...
你最內層的函式（程式）應該傳回呼叫 `charRange()` 的結果。
...
你應該將 `character1` 作為第一個引數傳遞給你的 `charRange()` 呼叫。
...
你應該將 `character2` 作為第二個引數傳遞給你的 `charRange()` 呼叫。
