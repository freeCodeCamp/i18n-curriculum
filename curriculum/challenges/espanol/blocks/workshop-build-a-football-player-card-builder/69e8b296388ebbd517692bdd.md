...
Agrega un atributo `value` de `player.pas` al PAS `input`. Luego agrega un atributo `onChange` configurado como una función flecha que toma `e` como parámetro y llama a `setPlayer` con un spread de `player` y `pas` establecido en `Number(e.target.value)`.

# --hints--

Tu PAS `input` debe tener un `value` igual a la estadística `pas` del jugador.
...
Tu PAS `input` debe tener un manejador `onChange` que llame a `setPlayer` con un spread de `player` y `pas` establecido en `Number(e.target.value)`.
