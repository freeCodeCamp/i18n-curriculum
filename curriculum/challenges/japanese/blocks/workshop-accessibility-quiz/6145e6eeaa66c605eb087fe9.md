...
すべてのラジオ `id`に `input` を追加して、ラベルとリンクできるようにしてください。最初のものには `id` の `q1-a1` を付けてください。残りには、それぞれ `id`、`q1-a2`、`q2-a1` の `q2-a2` を付けてください。

# --hints--

最初の `input` 要素には `id` の `q1-a1` を付ける必要があります。

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[0]?.id, "q1-a1");
```

2番目の `input` 要素には `id` の `q1-a2` を付ける必要があります。

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[1]?.id, "q1-a2");
```

3番目の `input` 要素には `id` の `q2-a1` を付ける必要があります。

```js
assert.equal(document.querySelectorAll('ul.answers-list > li > label > input')?.[2]?.id, "q2-a1");
```

4番目の `input` 要素には `id` の `q2-a2` を付ける必要があります。
