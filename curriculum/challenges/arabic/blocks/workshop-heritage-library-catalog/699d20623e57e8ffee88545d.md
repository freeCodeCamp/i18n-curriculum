...
بعد التحقق من `"Unknown"`، أضف منطق تجميع العقود للكتب التي تحتوي على سنوات رقمية. أعلن `const decade = Math.floor(book.year / 10) * 10`. على سبيل المثال، `Math.floor(1977 / 10)` يعطي `197`، مضروبًا في `10` يعطي `1970`.

ثم أعلن `const decadeKey =` `` `${decade}s` `` لإنتاج سلسلة نصية مثل `"1970s"`.

استخدم ترميز الأقواس لتجميع الكتاب: تحقق مما إذا كان `grouped[decadeKey]` غير موجود بعد وابدأه كمصفوفة فارغة، ثم أضف `book` إليه. ترميز الأقواس مطلوب هنا لأن المفتاح مخزن في متغير.

# --hints--

يجب أن تعلن متغيرًا باسم `decade` يعادل `Math.floor(book.year / 10) * 10`.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+decade\s*=\s*Math\.floor\s*\(\s*book\.year\s*\/\s*10\s*\)\s*\*\s*10/);
```

يجب أن تعلن متغيرًا باسم `decadeKey` يتضمن `decade` والحرف `"s"`.
...
يجب أن تتحقق مما إذا كان `grouped[decadeKey]` غير موجود بعد وابدأه كمصفوفة فارغة.
...
يجب أن تضيف `book` إلى `grouped[decadeKey]`.
