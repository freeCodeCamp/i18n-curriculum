...
One major gotcha is that all the code for server components remains on the server, and doesn't get shipped to the browser. Isso significa que você não pode usar React hooks com eles e eles não têm acesso a Web APIs ou listeners de eventos do navegador. Então como você pode adicionar interatividade?
