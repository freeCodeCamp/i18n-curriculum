...
對於新的 `@media` 規則，將 `#piano` 的 `width` 設為 `675px`，並將 `.keys` 的 `width` 設為 `633px`。

這樣，你的鋼琴就完成了！

# --hints--

你的第二個 `@media` 規則應該有一個 `#piano` 選擇器。
...
你的新 `#piano` 選擇器應該有 `675px` 的 `width`。
