...
Parece que todavía hay algunos problemas con el uso de los encabezados en la página. Si miras el primer elemento `div`, hay dos elementos `h4` usados como subencabezados. Una mejor práctica sería usar elementos `h3` para estos subencabezados, ya que están un nivel por debajo del elemento `h2`.

Cambia los dos elementos `h4` a elementos `h3`.

# --hints--

Tu elemento `h4` con el texto `Early Challenges` debería ser un elemento `h3`.
...
Tu elemento `h4` con el texto `Breakthroughs` debería ser un elemento `h3`.
