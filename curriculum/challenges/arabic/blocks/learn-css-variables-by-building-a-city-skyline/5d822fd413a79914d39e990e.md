...
أعطِ الصنف `bb4-window` قيمة `width` تساوي `18%`، و`height` تساوي `90%`، وأضف متغير `--window-color4` كـ `background-color`.

# --hints--

يجب أن تعطي `.bb4-window` قيمة `width` تساوي `18%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

يجب أن تعطي `.bb4-window` قيمة `height` تساوي `90%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

يجب أن تعطي `.bb4-window` قيمة `background-color` تساوي `--window-color4`.
