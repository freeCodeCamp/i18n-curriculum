...
Ahora que tienes la lista de canciones mostrada en la pantalla, sería bueno ordenarlas en orden alfabético por título. Podrías actualizar manualmente el arreglo `allSongs`, pero JavaScript tiene un método para arreglos que puedes usar llamado <dfn>sort()</dfn>.

El método `sort()` convierte elementos de un arreglo en cadenas y los ordena en el lugar basado en sus valores en la codificación `UTF-16`.
...
Dentro de tu función `sortSongs`, agrega el método `sort()` a `userData?.songs`.

# --hints--

Debes agregar el método `sort()` a `userData?.songs`.
