...
Während der Entwicklung ist es hilfreich, die `fieldset` Standardrahmen zu sehen. Allerdings wirken sie, als ob der Inhalt zu stark getrennt wäre.

Entfernen Sie den `border` und fügen Sie `2rem` Innenabstand nur oben und unten bei jedem `fieldset` hinzu. Achten Sie darauf, den `padding` links und rechts zu entfernen.

# --hints--

Sie können entweder den Wert `none` oder `0` verwenden, um den `border` zu entfernen.

```js
assert.match(new __helpers.CSSHelp(document).getStyle('fieldset')?.border, /(none)|(0px)|(medium)/);
```

Sie sollten `padding` von `2rem` oben und unten bei jedem `fieldset` hinzufügen.
...
Sie sollten den `padding` links und rechts bei jedem `fieldset` entfernen.
