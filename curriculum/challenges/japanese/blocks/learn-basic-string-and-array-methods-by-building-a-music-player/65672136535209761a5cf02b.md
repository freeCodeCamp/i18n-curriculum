...
ユーザーが曲をクリックするとその曲を再生できるようにするため、最初のボタン要素に `onclick` 属性を追加してください。 `onclick` 属性の値では、`song.id` を渡して `playSong` 関数を呼び出してください。

ここでは $ 記号を使った文字列補間が必要なことを忘れないでください。

# --hints--

`button` 要素に `playSong(${song.id})` という値を持つ `onclick` 属性を追加してください。
