...
Da Sie nun die Liste der Songs auf dem Bildschirm sehen, wäre es schön, sie alphabetisch nach Titel zu sortieren.

Beginnen Sie damit, eine Pfeilfunktion namens `sortSongs` zu erstellen.

# --hints--

Sie sollten eine Funktion namens `sortSongs` haben.

```js
assert.isFunction(sortSongs);
```

Ihre `sortSongs`-Funktion sollte die Syntax einer Pfeilfunktion verwenden.
