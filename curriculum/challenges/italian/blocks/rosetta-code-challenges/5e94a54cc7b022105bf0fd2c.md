...
Dato un testo e un numero intero n, restituisci le n parole più comuni nel file (insieme al numero delle loro occorrenze) in ordine decrescente di frequenza.

# --instructions--

Scrivi una funzione per contare le occorrenze di ogni parola e restituisci le n parole più comuni insieme al numero delle loro occorrenze in ordine decrescente di frequenza.

La funzione dovrebbe restituire un array bidimensionale con ciascuno degli elementi nella seguente forma: `[word, freq]`. `word` dovrebbe essere la versione minuscola della parola e `freq` il numero che indica il conteggio.

La funzione dovrebbe restituire un array vuoto se non viene fornita alcuna stringa.

La funzione dovrebbe essere case insensitive, per esempio, le stringhe "Hello" e "hello" dovrebbero essere trattate allo stesso modo.

Puoi considerare parole che contengono caratteri speciali come underscore, trattini, apostrofi, virgole, ecc., come parole distinte.
...
`wordFrequency("Apple App apply aPP aPPlE", 3)` dovrebbe restituire `[['app', 2], ['apple', 2], ['apply', 1]]` o `[['apple', 2], ['app', 2], ['apply', 1]]`
