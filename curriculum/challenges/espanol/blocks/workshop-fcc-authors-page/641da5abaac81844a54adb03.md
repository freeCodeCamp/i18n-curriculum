...
Ahora, es momento de crear la función que será responsable de mostrar los autores en la página. 

Comienza creando una función llamada `displayAuthors` que tome un parámetro `authors`. Tu función debería luego recorrer el array `authors` y agregar una tarjeta para cada autor al elemento `#authorContainer`. 

La tarjeta debe tener la siguiente estructura:
...
- Puedes usar cualquier tipo de ciclo para iterar sobre el arreglo `authors`. Ej. `for`, `forEach`, `map`, etc.
- Deberías usar la propiedad `innerHTML` para agregar la tarjeta al `authorContainer` dentro del bucle. También necesitarás usar la sintaxis de literales de plantilla para agregar el marcado.
...
`displayAuthors` debe ser una función.

```js
assert.isFunction(displayAuthors)
```

Tu función `displayAuthors` debe tomar un parámetro `authors`.
...
Cuando se llama a la función `displayAuthors` con un array de autores, debería agregar una tarjeta para cada autor al elemento `#authorContainer`.
