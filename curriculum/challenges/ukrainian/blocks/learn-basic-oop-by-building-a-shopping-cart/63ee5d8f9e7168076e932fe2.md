...
Після вашого елемента `h2` створіть два елементи `p`. Першому надайте `class` зі значенням `dessert-price` і встановіть текст у вигляді знака долара `"$"`, за яким йде значення змінної `price`. Другому надайте `class` зі значенням `product-category`, а текст — `"Category: "`, за яким йде значення змінної `category`.
...
Ваші елементи `p` мають йти після елемента `h2`.
...
Перший елемент `p` має мати `class` зі значенням `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Перший елемент `p` має містити знак долара `"$"`, за яким йде значення змінної `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Другий елемент `p` має мати `class` зі значенням `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Другий елемент `p` має містити текст `"Category: "`, за яким йде значення змінної `category`.
