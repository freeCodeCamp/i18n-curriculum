...
`#game`에 최대 너비 `500px`과 최대 높이 `400px`를 지정하세요. `background-color`는 `#ffffff`로, `color`는 `#ffffff`로 설정하세요.

여백을 사용해 위쪽 여백은 `30px`, 아래쪽 여백은 `0px`, 좌우 여백은 `auto`로 설정해 가운데 정렬하세요.

마지막으로 네 면 모두에 `10px`의 패딩을 주세요.
...
`#game` 선택자에는 `max-width`가 `500px`로 설정되어야 합니다.
...
`#game` 선택자에는 `max-height`가 `400px`로 설정되어야 합니다.
...
`#game` 선택자에는 `background-color`가 `#ffffff`로 설정되어야 합니다.
...
`#game` 선택자에는 `color`가 `#ffffff`로 설정되어야 합니다.
...
`#game` 선택자에는 `margin-top`가 `30px`로 설정되어야 합니다.
...
`#game` 선택자에는 `margin-left`가 `auto`로 설정되어야 합니다.
...
`#game` 선택자에는 `margin-right`가 `auto`로 설정되어야 합니다.
...
`#game` 선택자에는 `margin-bottom`가 `0px`로 설정되어야 합니다.
...
`#game` 선택자에는 네 면 모두에 `10px`의 패딩이 있어야 합니다.
