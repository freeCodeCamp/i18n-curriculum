...
此时，应用的行为应该没有改变。代码仍然从 CDN 获取一个文件，然后将检索到的数据处理成 `JSON`。完成后，它将这些数据储存在 `authorDataArr` 变量中，然后调用 `displayAuthors`。

修改对 `fetch` 的调用，使其使用 `await` 操作符。将结果储存在一个名为 `const` 的新 `res` 变量中。这个新变量应在 `initialFetch` 内局部定义。
