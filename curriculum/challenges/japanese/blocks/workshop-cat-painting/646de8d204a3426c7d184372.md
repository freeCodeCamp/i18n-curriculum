...
それらは目のように見えますが、もっと良くすることができます。だからこそ、2つの内側の目の `div` 要素を作成しました。

クラスセレクターを使って、`.cat-left-inner-eye` 要素に幅 `10px` と高さ `20px` を設定してください。また、背景色を `#fff` にしてください。

# --hints--

`.cat-left-inner-eye` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye'));
```

`.cat-left-inner-eye` セレクターには `width` を `10px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.width, '10px');
```

`.cat-left-inner-eye` セレクターには `height` を `20px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.height, '20px');
```

`.cat-left-inner-eye` セレクターには `background-color` を `#fff` に設定してください。
