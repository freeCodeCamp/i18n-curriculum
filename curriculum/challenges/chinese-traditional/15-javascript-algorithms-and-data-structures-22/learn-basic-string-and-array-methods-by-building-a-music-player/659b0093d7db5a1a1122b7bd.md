...
要獲取當前歌曲的索引，可以使用 <dfn>indexOf()</dfn> 方法。 `indexOf()` 數組方法返回在數組中找到給定元素的第一個索引，如果元素不存在，則返回 `-1`。
...
在你的 `getCurrentSongIndex` 函數中，返回 `userData?.songs.indexOf()`。 對於 `indexOf()` 參數，將其設置爲 `userData?.currentSong`。

# --hints--

你應該在 `getCurrentSongIndex` 函數內返回 `userData?.songs.indexOf()`。

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

你應該將 `userData?.currentSong` 傳遞給 `indexOf()` 方法。
