...
Finora, non succede nulla quando selezioni un bug diverso.

Aggiungi un listener per l'evento `change` a `selectElement`. Nella callback, usa `isSelect` per verificare se `e.target` è un `HTMLSelectElement`. Non devi ancora fare nulla all'interno del blocco `if`.

# --hints--

Il tuo `selectElement` dovrebbe avere un listener per l'evento `change` con una callback che prende un parametro evento.
...
La callback del tuo listener per l'evento dovrebbe avere un'istruzione `if` con la condizione `isSelect(e.target)`.
