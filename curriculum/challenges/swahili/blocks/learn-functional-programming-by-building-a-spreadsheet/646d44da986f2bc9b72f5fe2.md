...
Tangaza kitendakazi cha `toNumberList` kinachopokea kigezo cha `args` na kinarejesha matokeo ya kugawanya `args` kwa koma bila ya kuonyesha moja kwa moja. Kisha ungana njia ya `map` na njia yako ya `split` na pita `parseFloat` kama hoja kwa njia ya `map`.

# --hints--

Unapaswa kutangaza kigezo cha `toNumberList`.
...
Unapaswa kutumia `const` kutangaza kigezo chako cha `toNumberList`.
...
Kigezo chako cha `toNumberList` kinapaswa kuwa kitendakazi cha mshale.
...
Kitendakazi chako cha `toNumberList` kinapaswa kuwa na kigezo cha `args`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

Kitendakazi chako cha `toNumberList` kinapaswa kutumia kurejesha bila ya kuonyesha moja kwa moja.
...
Kitendakazi chako cha `toNumberList` kinapaswa kurejesha matokeo ya wito wa njia ya `.split()` ya `args`.
...
Unapaswa kugawanya `args` kwa herufi ya `","`.
...
Unapaswa kuunganisha njia ya `.map()` na njia ya `.split()`.
...
Unapaswa kupitisha rejea ya `parseFloat` kama wito wa kurudisha kwa `.map()`.
