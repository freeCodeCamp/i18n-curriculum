...
`evalFormula` の呼び出しで最初の引数には、セルの内容（`value` に保存したもの）を渡す必要があります。ただし、内容は関数を起動するために `=` 文字で始まっているので、`value` のインデックス `1` から始まる部分文字列を渡す必要があります。

# --hints--

`value` の呼び出しで最初の引数として `evalFormula()` を渡してください。
...
`.slice()` 引数に対して `value` メソッドを呼び出してください。
...
`1` の呼び出しに引数として数値 `.slice()` を渡してください。
