...
Deine `.catch()`-Methode sollte `console.error()` verwenden, um `err` auf der Konsole mit dem Text \``` `There was an error: ${err}` `` zu protokollieren. Denke daran, Backticks zu verwenden.
