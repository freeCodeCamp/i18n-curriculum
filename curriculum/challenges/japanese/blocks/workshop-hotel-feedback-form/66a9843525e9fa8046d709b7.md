...
ホテルのフィードバックフォームのワークショップの最後のステップでは、フォームに送信ボタンを追加する必要があります。

送信ボタンの使い方は、これまでのレッスンで学んでいます。

`button` 要素を追加し、`type` 属性を `"submit"` に設定し、テキスト内容を `Submit` にしてください。

これで、ホテルのフィードバックフォームが完成です！

# --hints--

フォームの中に `button` があるはずです。

```js
assert.exists(document.querySelector('form button'));
```

ボタンは `type` の `"submit"` に設定してください。

```js
assert.exists(document.querySelector('button[type="submit"]'));
```

ボタンのテキスト内容は `Submit` にしてください。
