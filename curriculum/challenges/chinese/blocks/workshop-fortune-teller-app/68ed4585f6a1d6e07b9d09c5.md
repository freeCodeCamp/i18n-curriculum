...
在 `fetchCardsData` 方法内，声明一个名为 `response` 的变量，并通过调用 `fetch` 函数，传入以下 URL 赋值：`` `${CDN_URL}/card_data.json` ``。

在 `await` 调用前添加 `fetch` 关键字，并将整个逻辑包裹在 `try...catch` 块中。

# --hints--

你应该在 `response` 方法内声明一个 `fetchCardsData` 变量。
...
你的 `response` 变量应通过调用 `await fetch` 并传入 URL `` `${CDN_URL}/card_data.json` `` 赋值。
...
你的 `fetch` 调用应包裹在 `try...catch` 块中，`error` 作为 `catch` 参数。
