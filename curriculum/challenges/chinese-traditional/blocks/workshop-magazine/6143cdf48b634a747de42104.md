...
在你的 `.image-wrapper` 元素中，給第三個 `img` 元素添加一個值爲 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg` 的 `src`、一個值爲 `four people working on code` 的 `alt`、一個值爲 `lazy` 的 `loading` 屬性、一個值爲 `image-3` 的 `class`、一個值爲 `600` 的 `width` 屬性和一個值爲 `400` 的 `height` 屬性。

# --hints--

你的第三個 `img` 元素應該有一個 `src` 屬性，值爲 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`。
...
你的第三個 `img` 元素應該有一個 `alt`，值爲 `four people working on code`。
...
你的第三個 `img` 元素應該有一個 `loading` 屬性，值爲 `lazy`。
...
你的第三個 `img` 元素應該有一個 `class`，值爲 `image-3`。
...
你的第三個 `img` 元素應該有一個 `width` 屬性，值爲 `600`。
...
你的第三個 `img` 元素應該有一個 `height` 屬性，值爲 `400`。
