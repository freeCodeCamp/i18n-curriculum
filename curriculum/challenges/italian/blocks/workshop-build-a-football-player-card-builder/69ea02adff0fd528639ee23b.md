...
Successivamente, crea una funzione `loadPlayer` con `PlayerData` come tipo di ritorno. Al suo interno, aggiungi una sintassi `try...catch` con `error` come parametro del blocco `catch`. Lascia tutto il resto vuoto per ora.

# --hints--

Dovresti creare una funzione chiamata `loadPlayer`.
...
La tua funzione `loadPlayer` dovrebbe avere un tipo di ritorno `PlayerData`.
...
La tua funzione `loadPlayer` dovrebbe contenere un blocco `try...catch`.
