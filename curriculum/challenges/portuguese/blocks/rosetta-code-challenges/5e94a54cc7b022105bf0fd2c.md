...
A função deve retornar um array 2D com cada um dos elementos na seguinte forma: `[word, freq]`. `word` deve ser a versão em minúsculas da palavra e `freq` o número que indica a contagem.
