...
Modifica tu función `sellPlants` para que devuelva `Item not found.` cuando el catálogo no contenga la planta pasada a la función.
...
`sellPlants(hidcote, "large", 5)` debería devolver `Catalog successfully updated.` y restar `5` de la propiedad `large` del objeto asociado con la clave `hidcote` en el catálogo.
