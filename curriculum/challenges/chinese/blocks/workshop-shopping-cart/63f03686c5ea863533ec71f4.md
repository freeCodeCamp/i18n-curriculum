...
声明一个变量 `tax`，并为其分配调用新 `.calculateTaxes()` 方法的值，同时传递 `subTotal` 作为参数。

# --hints--

使用 `const` 声明一个名为 `tax` 的变量。
...
将调用新的 `.calculateTaxes()` 方法的值分配给 `tax` 变量，并将 `subTotal` 作为参数传递。 请记住使用 `this` 关键字。
