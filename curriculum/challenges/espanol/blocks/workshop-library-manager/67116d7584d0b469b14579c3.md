...
En lecciones anteriores, aprendiste cómo trabajar con el método `filter()`, que se usa para devolver un nuevo arreglo con los resultados filtrados.
...
Crea una función `getBooksByAuthor` con dos parámetros: un array con los objetos de libro y una cadena con el autor.

La función debe devolver un array que contenga los libros por un autor en particular.

# --hints--

Debes crear una función `getBooksByAuthor`.

```js
assert.isFunction(getBooksByAuthor);
```

Tu función `getBooksByAuthor` debe tener dos parámetros.

```js
assert.lengthOf(getBooksByAuthor, 2);
```

Tu función `getBooksByAuthor` debe devolver un array.

```js
assert.isArray(getBooksByAuthor(library, ''));
```

Tu función `getBooksByAuthor` debe devolver el número correcto de libros para cualquiera de los autores.
