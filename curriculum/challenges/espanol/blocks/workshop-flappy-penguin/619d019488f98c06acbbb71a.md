...
Corrija esto agregando una `class` de `left` al primer elemento `.face`, y una `class` de `right` al segundo elemento `.face`.

# --hints--

Debe agregar una `class` de `left` al primer elemento `.face`.

```js
assert.include(document.querySelector('.face:nth-of-type(1)').className, 'left');
```

Debe agregar una `class` de `right` al segundo elemento `.face`.
