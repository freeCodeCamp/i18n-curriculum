...
Bisher haben Sie drei Funktionen: `getElement`, `hideElements` und `showElements`.
Jetzt benötigen Sie eine Funktion, die Ihnen hilft, ein zufälliges Orakel zu erhalten.

Deklarieren Sie eine Funktion namens `getRandomItem`, die ein Array von `items` als Argument annimmt. Verwenden Sie Generics für diese Funktion.

Verwenden Sie innerhalb der Funktion `Math.random()` und `Math.floor()`, um ein zufälliges Orakel aus dem `items`-Array auszuwählen.
Vergessen Sie nicht, das ausgewählte Orakel zurückzugeben.

# --hints--

Sie sollten eine Funktion namens `getRandomItem` deklarieren.
...
Ihre `getRandomItem`-Funktion sollte den generischen Typ-Parameter `T` verwenden.
...
Ihre `getRandomItem`-Funktion sollte einen Parameter namens `items` mit dem Typ `T[]` haben.
...
Ihre `getRandomItem`-Funktion sollte einen Rückgabetyp von `T` haben.
...
Sie sollten eine Variable namens `index` innerhalb der `getRandomItem`-Funktion deklarieren.
...
Die `index`-Variable sollte `Math.floor()` und `Math.random()` mit `items.length` verwenden, um einen zufälligen Index zu erzeugen.
...
Ihre `getRandomItem`-Funktion sollte `items[index]` zurückgeben.
