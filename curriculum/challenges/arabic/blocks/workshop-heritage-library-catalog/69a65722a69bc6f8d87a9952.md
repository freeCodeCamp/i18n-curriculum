...
JSON (JavaScript Object Notation) هو تنسيق قياسي لمشاركة البيانات بين الأنظمة. تقوم الدالة `JSON.stringify` بتحويل قيمة JavaScript إلى سلسلة نصية بتنسيق JSON.

ابدأ بإزالة استدعائي console.log الخاصين بـ `validateEntry`، إذ لم تكن مطلوبة إلا للاختبار. ثم أنشئ دالة باسم `exportToJSON` تأخذ `catalog` كمعلمة. أعد `JSON.stringify(catalog, null, 2)`. الوسيطة الأولى هي البيانات المراد تحويلها. والوسيطة الثانية (`null`) تعني عدم وجود ترشيح مخصص. والثالثة (`2`) تضيف مسافة بادئة من فراغين، مما يجعل المخرجات قابلة للقراءة.

أسفل الدالة، سجّل `exportToJSON(catalog.slice(0, 2))` لمعاينة ناتج JSON لأول مدخلين.

# --hints--

يجب إزالة استدعاءات وحدة التحكم `validateEntry`.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry/);
```

يجب أن تُرجع دالة `exportToJSON` سلسلة نصية.

```js
assert.isString(exportToJSON(catalog));
```

يجب أن ينتج `JSON.parse(exportToJSON(catalog))` مصفوفة صحيحة.

```js
assert.isArray(JSON.parse(exportToJSON(catalog)));
```

يجب استخدام `JSON.stringify` مع `null` و `2` كوسيطين الثاني والثالث.

```js
assert.match(exportToJSON.toString(), /JSON\.stringify\s*\(\s*catalog\s*,\s*null\s*,\s*2\s*\)/);
```

يجب تسجيل `exportToJSON(catalog.slice(0, 2))` في وحدة التحكم.
