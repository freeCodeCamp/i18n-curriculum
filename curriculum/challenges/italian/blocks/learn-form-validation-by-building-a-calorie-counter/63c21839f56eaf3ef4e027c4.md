...
Crea un altro elemento `label` (su una nuova riga) alla fine del tuo `HTMLString`. Questo `label` dovrebbe contenere il testo `Entry # Calories`, usando la sintassi dei template literal per sostituire `#` con il valore di `entryNumber`, e l'attributo `for` impostato su `X-#-calories`, dove `X` è il `value` di `entryDropdown` e `#` è il valore di `entryNumber`.

# --hints--

Dovresti avere due elementi `label` nel tuo `HTMLString`.
...
Il tuo nuovo elemento `label` dovrebbe essere su una nuova riga.
...
Il tuo nuovo elemento `label` dovrebbe venire dopo il tuo elemento `input`.
...
Il tuo nuovo elemento `label` dovrebbe avere un attributo `for` impostato su `${entryDropdown.value}-${entryNumber}-calories`.
...
Il tuo nuovo elemento `label` dovrebbe contenere il testo `Entry ${entryNumber} Calories`.
...
Non dovresti modificare i tuoi elementi esistenti.
