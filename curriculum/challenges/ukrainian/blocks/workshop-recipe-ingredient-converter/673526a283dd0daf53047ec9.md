...
Тепер вам потрібно створити свою першу карировану функцію. Пам’ятайте, що карирована функція — це функція, яка повертає іншу функцію, ось так:
...
Почніть з оголошення `convertQuantity` функції. Вона має мати один параметр `fromUnit` і бути порожньою.

# --hints--

Ви повинні оголосити `convertQuantity` функцію.

```js
assert.isFunction(convertQuantity)
```

Ваша `convertQuantity` функція має мати параметр `fromUnit`.

```js
assert.match(convertQuantity?.toString(), /\(fromUnit\)/);
```

Ваша `convertQuantity` функція має бути порожньою.
