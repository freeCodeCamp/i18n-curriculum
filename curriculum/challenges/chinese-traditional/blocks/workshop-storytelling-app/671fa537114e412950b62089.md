...
在你的 JavaScript 檔案中，選取 `.story-container` 元素並將其儲存在名為 `storyContainer` 的變數中。

# --hints--

你應該有一個 `storyContainer` 變數。

```js
assert.isDefined(storyContainer);
```

你應該使用 `querySelector` 方法選取 `.story-container` 元素，並將其儲存在 `storyContainer` 變數中。
