...
أضف عنصر وسم الرابط مع `class` `"author-link"`، وادمج `url` كقيمة لخاصية `href`، وعيّن `target` إلى `"_blank"`. لنص عنصر وسم الرابط، ادمج `author` متبوعًا بالنص `"'s author page"`. على سبيل المثال: `"Quincy Larson's author page"`.
...
يجب أن تستبدل `url` بقيمة الخاصية `href` في عنصر وسم الرابط.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), authorDataArr[0].url);
```

يجب تعيين الخاصية `target` في عنصر وسم الرابط إلى القيمة `"_blank"`.

```js
assert.equal(document.querySelector('a')?.getAttribute('target'), '_blank');
```

يجب أن تُدرج `author` متبوعة بالنص `"'s author page"` داخل عنصر وسم الرابط الخاص بك.
