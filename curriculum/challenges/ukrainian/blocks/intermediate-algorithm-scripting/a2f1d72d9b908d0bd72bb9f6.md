...
Заповніть конструктор об’єкта наведеними нижче методами:
...
Запустіть тести, щоб побачити очікувані вихідні дані для кожного методу. Ці методи мають бути єдиним доступним способом взаємодії з об’єктом. Кожен тест оголосить новий екземпляр `Person` як `new Person('Bob', 'Ross')`.
...
Ви не повинні переназначати параметр `first`.

```js
assert.notMatch(code, /\bfirst\s*=\s*/);
```

Ви не повинні переназначати параметр `last`.

```js
assert.notMatch(code, /\blast\s*=\s*/);
```

Не слід додавати властивості. `Object.keys(Person).length` завжди має повертати 6.
...
Ви повинні мати змогу створити свій об’єкт `Person`.
...
Ваш об’єкт `Person` не повинен мати властивість `firstName`.
...
Ваш об’єкт `Person` не повинен мати властивість `lastName`.
