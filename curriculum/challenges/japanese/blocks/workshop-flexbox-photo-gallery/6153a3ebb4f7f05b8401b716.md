...
`alt` 属性は画像の内容を説明する必要があります。スクリーンリーダーは画像の代わりに代替テキストを読み上げます。画像が読み込めない場合は、このテキストが画像の代わりに表示されます。

プロジェクトを完成させるには、9つの猫の画像すべてに`alt` 属性を追加して説明してください。各属性の値は少なくとも5文字以上にしてください。


# --hints--

9つの`img` 要素すべてに、画像を説明するテキストを含む`alt` 属性が必要です。各説明は少なくとも`5` 文字以上で書いてください。
