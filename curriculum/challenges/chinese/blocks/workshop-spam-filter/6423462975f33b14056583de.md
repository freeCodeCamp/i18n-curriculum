...
字符类可以容纳两个以上的字符。 将你的 `a` 字符替换为与 `a`、`@` 和 `4` 匹配的字符类。

# --hints--

你的 `stockRegex` 应该使用字符类来匹配 `a`、`@` 和 `4`。
...
你的 `stockRegex` 应该与 `stock @lert` 匹配。

```js
assert.match('stock @lert', stockRegex);
```

你的 `stockRegex` 应该与 `stock 4lert` 匹配。

```js
assert.match('stock 4lert', stockRegex);
```

你的 `stockRegex` 应该与 `570ck 4l3r7` 匹配。
