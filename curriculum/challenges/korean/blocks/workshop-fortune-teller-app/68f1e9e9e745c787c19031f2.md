...
`if` 문장 안에서 `cardTitle` 요소를 가져와 그 `textContent`를 `foundCard.name`로 설정하세요. `description` 요소에 대해서도 `textContent`를 `foundCard.desc`로 설정하는 작업을 똑같이 하세요.

# --hints--

`cardTitle` `textContent`를 `foundCard.name`로 설정해야 합니다. `this` 키워드를 사용하는 것을 잊지 마세요.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

`description` `textContent`를 `foundCard.desc`로 설정해야 합니다. `this` 키워드를 사용하는 것을 잊지 마세요.
