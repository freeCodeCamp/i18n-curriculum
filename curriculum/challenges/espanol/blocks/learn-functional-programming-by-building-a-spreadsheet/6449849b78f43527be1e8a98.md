...
Necesitas poder coincidir rangos de celdas en una fórmula. Los rangos de celdas pueden verse como `A1:B12` o `A3:A25`. Puedes usar una expresión regular para coincidir con estos patrones.

Empieza declarando una variable `rangeRegex` y asígnale una expresión regular que coincida con `A` hasta `J` (el rango de columnas en tu hoja de cálculo). Usa un grupo de captura con una clase de caracteres para lograr esto.

# --hints--

Debe declarar una variable `rangeRegex` después de tu función `idToText`.
...
Debes usar `const` para declarar tu variable `rangeRegex`.
...
Tu variable `rangeRegex` debe ser una expresión regular.
...
Tu `rangeRegex` debe usar un grupo de captura.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(.*\)\/\s*;?/);
```

Tu `rangeRegex` debe usar una clase de caracteres en el grupo de captura.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[.*\]\)\/\s*;?/);
```

Tu `rangeRegex` debe usar una clase de caracteres para coincidir con `A` hasta `J`.
