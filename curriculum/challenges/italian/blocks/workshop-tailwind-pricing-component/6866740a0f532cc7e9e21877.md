...
Ora passiamo al badge `Most popular`. Devi posizionarlo nell'angolo in alto a destra della card, quindi puoi usare alcune delle classi di utilità per il posizionamento di Tailwind per farlo.

Ecco come useresti il posizionamento CSS in Tailwind:
...
Assegna la classe `relative` al wrapper `div` e `absolute` al `div` con il testo `Most popular`.

# --hints--

Il tuo elemento wrapper `div` per il piano premium dovrebbe avere la classe `relative`.
...
Il tuo elemento `div` con il testo `Most popular` dovrebbe avere la classe `absolute`.
