...
在你的 `select` 元素内，你需要创建选项供用户选择作为他们的电源。 `select` 应该有下面列出的七个选项：
...
第一个选项（`Select one`）的 `value` 属性应为一个空字串（即 `value=''`）。这在用户界面中充当占位符。

# --hints--

你应该在你的 `select` 元素内创建第一个带有文本 `Select one` 的 `option` 元素。
...
你应该给第一个 `option` 元素一个值为空字串的 `value` 属性。
...
你应该在你的 `select` 元素内创建第二个带有文本 `Bitten by a strange creature` 的 `option` 元素。
...
你应该在你的 `select` 元素内创建第三个带有文本 `Radioactive exposure` 的 `option` 元素。
...
你应该在你的 `select` 元素内创建第四个带有文本 `Science experiment` 的 `option` 元素。
...
你应该在你的 `select` 元素内创建第五个带有文本 `Alien heritage` 的 `option` 元素。
...
你应该在你的 `select` 元素内创建第六个带有文本 `Ancient artifact discovery` 的 `option` 元素。
...
你应该在你的 `select` 元素中创建第七个带有文本 `Other` 的 `option` 元素。
