...
أضف خاصية `value` بقيمة `player.dri` إلى مُدخَل DRI. ثم أضف خاصية `onChange` مضبوطة على دالة سهمية تأخذ `e` كمعلمة وتستدعي `setPlayer` مع نشر `player` وضبط `dri` على `Number(e.target.value)`.

# --hints--

يجب أن يحتوي `input` الخاص بـ DRI على `value` تساوي قيمة `dri` للاعب.
...
يجب أن يحتوي `input` الخاص بـ DRI على معالِج `onChange` يستدعي `setPlayer` مع نشر `player` وضبط `dri` على `Number(e.target.value)`.
