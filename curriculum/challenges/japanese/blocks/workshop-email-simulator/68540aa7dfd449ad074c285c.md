...
これから受信トレイの機能を実演します。Ramyは受信トレイを確認し、最初のメールを読み、最初のメールを削除してから、変更を確認するために再度受信トレイをチェックします。

`main` 関数内でメールを送信した後、次のコードを追加してください。

- Ramyに`check_inbox`メソッドを使って受信トレイを確認させてください。
- Ramyに最初のメールを読ませてください。
- Ramyに最初のメールを削除させてください。
- Ramyにもう一度受信トレイを確認させてください。

これでメールシミュレーターは完成です！

# --hints--

Ramy のメールを表示するには `ramy.check_inbox()` を呼び出してください。
...
最初のメールを読むには `ramy.read_email(1)` を呼び出してください。
...
最初のメールを削除するには `ramy.delete_email(1)` を呼び出してください。
...
更新された受信トレイを表示するには再度 `ramy.check_inbox()` を呼び出してください。
