...
漸層過渡通常會從一種顏色逐漸變化到另一種顏色。當需要更突兀的變化時，過渡可以像這樣以硬停止方式進行：
...
為你的 `.bb2b` 添加一個 `linear-gradient`，該漸層從 `0%` 到 `6%` 使用 `--building-color2`，從 `6%` 到 `9%` 使用 `--window-color2`。

# --hints--

你應該給 `.bb2b` 一個 `background` 屬性。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

你應該在 `background` 上使用 `linear-gradient`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

你應該使用 `--building-color2` 從 `0%` 到 `6%`。
...
`.bb2b` 應該有一個 `linear-gradient`，從 `0%` 到 `6%` 由 `--building-color2` 過渡，從 `6%` 到 `9%` 由 `--window-color2` 過渡。
