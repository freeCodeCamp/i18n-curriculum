...
あなたの `conversionTable` に `gram` プロパティを追加し、その値として `cup` プロパティが `1/240` に設定されているオブジェクト、`ounce` プロパティが `0.0353` に設定されているオブジェクト、そして `teaspoon` プロパティが `0.2` に設定されているオブジェクトを代入してください。

# --hints--

あなたの `conversionTable` には `gram` プロパティが必要です。

```js
assert.property(conversionTable, "gram");
```

あなたの `gram` プロパティはオブジェクトである必要があります。

```js
assert.isObject(conversionTable.gram);
```

あなたの `gram` オブジェクトには `cup` プロパティが必要です。

```js
assert.property(conversionTable.gram, "cup");
```

あなたの `cup` プロパティは `1/240` に設定してください。

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

あなたの `gram` オブジェクトには `ounce` プロパティが必要です。

```js
assert.property(conversionTable.gram, "ounce");
```

あなたの `ounce` プロパティは `0.0353` に設定してください。

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

あなたの `gram` オブジェクトには `teaspoon` プロパティが必要です。

```js
assert.property(conversionTable.gram, "teaspoon");
```

あなたの `teaspoon` プロパティは `0.2` に設定してください。
