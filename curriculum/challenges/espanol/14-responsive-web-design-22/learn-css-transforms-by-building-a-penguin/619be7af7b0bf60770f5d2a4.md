...
Debes dar a `.penguin-body::before` un `top` de `--fcc-expected--`, pero encontrado `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

Debes dar a `.penguin-body::before` un `left` de `--fcc-expected--`, pero encontró `--fcc-actual--`.
