...
`.author-name a:hover` セレクターを作成し、`background-color` プロパティに `#306203` を設定してください。

これにより、`a` 内の `.author-name` 要素にだけホバーエフェクトが適用され、背景に元の freeCodeCamp グリーンが表示されます。

# --hints--

`.author-name a:hover` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

`.author-name a:hover` セレクターには `background-color` プロパティを `#306203` に設定してください。
