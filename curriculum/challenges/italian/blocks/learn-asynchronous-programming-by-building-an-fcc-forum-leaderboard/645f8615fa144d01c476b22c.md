...
Infine, dovrai restituire l'immagine per l'avatar utente.

Inizia aggiungendo un `return` seguito da un insieme di template literal. All'interno dei template literal, aggiungi un elemento `img`.

All'interno del tag `img`, aggiungi un attributo `src` con il valore `${userAvatarUrl}`. Per l'attributo `alt`, aggiungi un valore `${user.name}`.
...
Dovresti restituire un template literal.
...
Dovresti avere un elemento `img` all'interno del template literal.
...
Dovresti avere un attributo `src` all'interno del tag `img`.
...
Dovresti impostare il valore per l'attributo `src` alla costante chiamata `userAvatarUrl`.
...
Dovresti avere un attributo `alt` dopo l'attributo `src`.
...
Dovresti impostare il valore dell'attributo `alt` alla costante chiamata `user.name`.
