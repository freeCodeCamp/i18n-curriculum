...
Dopo aver recuperato i dati, archivia la risposta in una variabile e analizzala in JSON. Dai ai dati analizzati il tipo `Deck` e poi assegnali alla proprietà `cards`. All'interno del blocco `catch`, assicurati di registrare l'errore con questo messaggio: `"Error fetching cards:", error`.

# --hints--

Dovresti dichiarare una variabile chiamata `data` all'interno di `fetchCardsData`.
...
La tua variabile `data` dovrebbe avere il tipo `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

La tua variabile `data` dovrebbe essere assegnata analizzando la risposta come JSON.
...
Dovresti archiviare `data.cards` in `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Il tuo blocco `catch` dovrebbe usare `console.error()` per registrare l'errore con il messaggio `"Error fetching cards:", error`.
