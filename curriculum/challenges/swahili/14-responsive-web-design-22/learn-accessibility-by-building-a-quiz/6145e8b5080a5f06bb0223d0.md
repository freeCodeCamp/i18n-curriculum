...
Vipe vipengele vya `label` maandishi ya hivi kwamba `input` huja kabla ya maandishi. Kisha, vipe vipengele vya `input` `value` inayolingana na maandishi.
...
Unapaswa kuipa `label` ya pili maandishi `False`.
...
Unapaswa kuipa `label` ya tatu maandishi ya `True` au `False`.

```js
assert.include(['True', 'False'], document.querySelectorAll('ul.answers-list > li > label')?.[2]?.textContent?.trim());
```

Unapaswa kuipa `label` ya nne maandishi ya `True`.
...
Unapaswa kuipa `label` ya nne maandishi `False`.
...
Unapaswa kuipa `input` ya kwanza `value` inayolingana na maudhui ya maandishi ya `label`.
...
Unapaswa kuipa `input` ya pili `value` inayolingana na maudhui ya maandishi ya `label`.
...
Unapaswa kuipa `input` ya tatu `value` inayolingana na maudhui ya maandishi ya `label`.
...
Unapaswa kuipa `input` ya nne `value` inayolingana na maudhui ya maandishi ya `label`.
