...
Crea otro elemento `button` con la clase `playlist-song-delete` y el atributo `aria-label` establecido en `Delete` interpolado con `song.title`. Para el contenido del icono de borrado, pega el siguiente SVG:
...
Debe crear un elemento `button` con la clase `playlist-song-delete` como su primer atributo y valor.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

Tu elemento button debe tener un atributo `aria-label` con el valor `Delete ${song.title}` como su segundo atributo y valor.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

Debe pegar el `svg` proporcionado como el contenido de su elemento `button`.
