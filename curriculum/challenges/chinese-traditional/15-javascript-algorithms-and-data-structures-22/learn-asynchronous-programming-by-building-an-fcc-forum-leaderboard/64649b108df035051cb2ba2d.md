...
對於 `a` 開始標籤，將 `target` 屬性設置爲 `"_blank"`。 然後，將 `href` 屬性設置爲 `${forumTopicUrl}${slug}/${id}`。

有了這些更改，你的 freeCodeCamp 論壇排行榜項目現在已經完成了！

# --hints--

你的開始錨標籤應該將 `target` 屬性設置爲 `"_blank"`。
...
你的開始錨標籤應該設置一個 `href` 屬性，其值爲 `${forumTopicUrl}${slug}/${id}`。
