...
En otro ejemplo, vea cómo un objeto puede ser devuelto, conteniendo los nombres de los usuarios como propiedades con sus edades como valores.
...
La variable `watchList` contiene un arreglo de objetos con información sobre varias películas. Utiliza `reduce` para encontrar la calificación media en IMDB de las películas dirigidas por `Christopher Nolan`. Recuerda de desafíos anteriores filtrar los datos `filter` y mapear sobre ellos `map` para extraer lo que necesitas. Puede que necesites crear otras variables y devolver la calificación media con la función `getRating`. Ten en cuenta que los valores de valoración se guardan como cadenas en el objeto y necesitan ser convertidos en números antes de ser utilizados en cualquier operación matemática.
...
El código debe utilizar el método `reduce`.
