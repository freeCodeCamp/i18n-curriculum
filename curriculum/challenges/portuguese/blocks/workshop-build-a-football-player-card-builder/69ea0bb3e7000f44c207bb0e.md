...
Para fazer o recurso localStorage funcionar finalmente, altere a inicialização do estado de `defaultPlayer` para `loadPlayer`. Isso garante que o estado comece com quaisquer dados salvos disponíveis.

Agora teste o recurso localStorage. Tudo o que você digitar deve permanecer após a pré-visualização ser atualizada ou você recarregar a página.

Você também pode mudar o link da imagem `src` para `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` para ver a imagem mudar em tempo real. Apenas certifique-se de alterar o nome do jogador para Maradona.

Com isso, seu projeto de criador de cartão de jogador de futebol está completo!

# --hints--

Seu `useState` deve ser inicializado com `loadPlayer` em vez de `defaultPlayer`.
