...
使用相同的语法，将 `button2` 和 `button3` 的 `onclick` 属性分别设置为 `goCave` 和 `fightDragon`。

完成后，打开控制台并尝试单击项目上的按钮。
...
你不应使用 `let` 或 `const` 来分配 `button2.onclick`。

```js
assert.notMatch(code, /(let|const)\s+button2\.onclick/);
```

你应该将 `goCave` 函数引用分配给 `button2.onclick`。 确保不要调用该函数。
...
你不应使用 `let` 或 `const` 来分配 `button3.onclick`。

```js
assert.notMatch(code, /(let|const)\s+button3\.onclick/);
```

你应该将 `fightDragon` 函数引用分配给 `button3.onclick`。 确保不要调用该函数。
