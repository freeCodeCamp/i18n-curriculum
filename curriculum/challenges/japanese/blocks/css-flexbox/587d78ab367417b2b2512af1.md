...
title: ツイート埋め込みにFlexのスーパーパワーを追加する
challengeType: 0
...
右側に実例として使うツイートの埋め込みがあります。いくつかの要素は別のレイアウトにしたほうが見栄えが良くなります。前回のチャレンジでは`display: flex`を示しました。ここではツイート埋め込みのいくつかのコンポーネントにそれを追加して、配置を調整し始めます。

# --instructions--

以下のすべての項目にCSSプロパティ`display: flex`を追加してください。セレクターはすでにCSSで設定されています：

`header`、ヘッダーの`.profile-name`、ヘッダーの`.follow-btn`、ヘッダーの`h3`と`h4`、`footer`、そしてフッターの`.stats`。

# --hints--

あなたの`.follow-btn`はページ上にレンダリングされているはずです。広告ブロッカーなどの拡張機能はオフにしてください。
...
あなたの`header`には`display`プロパティが`flex`に設定されている必要があります。
...
あなたの`footer`には`display`プロパティが`flex`に設定されている必要があります。
...
あなたの`h3`には`display`プロパティが`flex`に設定されている必要があります。
...
あなたの`h4`には`display`プロパティが`flex`に設定されている必要があります。
...
あなたの`.profile-name`には`display`プロパティが`flex`に設定されている必要があります。
...
あなたの`.follow-btn`には`display`プロパティが`flex`に設定されている必要があります。
...
あなたの`.stats`には`display`プロパティが`flex`に設定されている必要があります。
