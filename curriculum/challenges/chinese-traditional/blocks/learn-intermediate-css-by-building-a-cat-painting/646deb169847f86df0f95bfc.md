...
使用類別選擇器，為你的 `.cat-right-inner-eye` 元素設定寬度為 `10px` 和高度為 `20px`。同時，為它設定背景顏色為 `#fff`。
