...
الخطوة التالية هي إنشاء متغير يسمى `currentWeapon`.
...
استخدم الكلمة المفتاحية `let` لإنشاء متغير باسم `currentWeapon`. لا تُعيّن له قيمة بعد.

# --hints--

يجب استخدام `let` لإعلان متغير `currentWeapon`.
