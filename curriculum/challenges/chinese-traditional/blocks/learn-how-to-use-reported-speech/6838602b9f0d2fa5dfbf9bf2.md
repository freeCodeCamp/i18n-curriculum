...
`Unusually` is an adverb used to describe something that happens more or less than what is normal or expected. On the other hand, `unusual` is an adjective that describes something that is not common or normal. 例如：
...
A `query` is a question or request sent to a database to get information or perform an action. 例如：
