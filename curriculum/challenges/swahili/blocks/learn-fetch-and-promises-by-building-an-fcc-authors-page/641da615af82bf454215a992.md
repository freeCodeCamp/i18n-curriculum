...
Pitia kitendakazi cha mwitiko tupu kwa njia ya `.forEach()`. Kwa kigezo cha kwanza cha kitendakazi cha mwitiko, gawanisha vitu vya `author`, `image`, `url`, na `bio`.

Kwa kigezo cha pili, pitia `index`. Hii itawakilisha nafasi ya kila mwandishi, na itakuwa na manufaa kwa ugawaji ukurasa baadaye.
...
Unapaswa kugawanisha `author`, `image`, `url`, na `bio` kama kigezo cha kwanza cha kitendakazi chako cha mwitiko. Usisahau kuziweka ndani ya mabano ya mviringo.
...
Unapaswa kugawanisha `index` kama kigezo cha pili cha kitendakazi chako cha mwitiko. Lazima iwe nje ya mabano ya mviringo uliyoweka mengine ndani yake.

```js
assert.match(code, /authors\.forEach\(\s*\(\s*\{.*\}\s*,\s*index\s*\)/)
```

Kitendakazi chako cha mwitiko kinapaswa kutumia sintaksia ya mshale.
