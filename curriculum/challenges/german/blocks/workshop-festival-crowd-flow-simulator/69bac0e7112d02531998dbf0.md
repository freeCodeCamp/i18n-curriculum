...
Die letzte Aufgabe in Ihrer `handleGateAtTick`-Funktion besteht darin, jeden Überlauf für den aktuellen Tick zu behandeln.

Wenn ein Überlauf vorliegt (`result.overflow > 0`), sollten Sie:

- Die folgende Nachricht `"Overflow of " + result.overflow + " attendees. Rerouting..."` protokollieren.
- Den Überlauf umleiten, indem Sie Ihre `rerouteOverflow()`-Funktion mit `gates`, `gate`, `tickIndex` und `result.overflow` aufrufen.

# --hints--

Wenn ein Überlauf vorliegt (`result.overflow > 0`), sollten Sie zuerst die Nachricht `"Overflow of " + result.overflow + " attendees. Rerouting..."` protokollieren.
...
Wenn ein Überlauf vorliegt (`result.overflow > 0`), sollten Sie den Überlauf umleiten, indem Sie `rerouteOverflow()` mit `gates`, `gate`, `tickIndex` und `result.overflow` aufrufen.
