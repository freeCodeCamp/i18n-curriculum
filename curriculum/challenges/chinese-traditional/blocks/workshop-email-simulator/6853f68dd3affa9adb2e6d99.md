...
太好了！既然你已經練習過 datetime 格式化，請從你的程式碼底部移除 `current_time` 變數和列印述語。我們將在下一步將時間戳記整合到 `Email` 類別中。

# --hints--

你的程式碼中不應該有述語 `current_time = datetime.datetime.now()")`。
...
你不應該在你的程式碼中使用帶有 `strftime()` 的列印述語。
