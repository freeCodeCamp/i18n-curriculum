...
JSON（JavaScript 物件表示法）是一種在系統間共享資料的標準格式。`JSON.stringify` 將 JavaScript 值轉換成 JSON 格式的字串。

首先移除兩個 `validateEntry` 的主控台輸出呼叫，因為它們只是用來測試。接著創建一個名為 `exportToJSON` 的函式，該函式接受 `catalog` 作為參數。傳回 `JSON.stringify(catalog, null, 2)`。第一個引數是要轉換的資料。第二個引數（`null`）表示不使用自定過濾。第三個（`2`）加入兩個空格的縮排，使輸出更易讀。

在函式下方，將 `exportToJSON(catalog.slice(0, 2))` 輸出到主控台，以預覽前兩筆資料的 JSON 輸出。

# --hints--

你應該移除 `validateEntry` 的主控台輸出呼叫。
...
`JSON.parse(exportToJSON(catalog))` 應該產生有效的陣列。

```js
assert.isArray(JSON.parse(exportToJSON(catalog)));
```

你應該使用 `JSON.stringify`，並將 `null` 和 `2` 作為第二和第三個引數。

```js
assert.match(exportToJSON.toString(), /JSON\.stringify\s*\(\s*catalog\s*,\s*null\s*,\s*2\s*\)/);
```

你應該將 `exportToJSON(catalog.slice(0, 2))` 輸出到主控台。
