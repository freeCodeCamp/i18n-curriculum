...
En este momento, la sección `footer` tiene un encabezado `h3`. Pero debería ser un encabezado `h2` para mantener una jerarquía de encabezados adecuada. 

Cambia el `h3` a un `h2`.

# --hints--

Tu elemento `footer` debe contener un elemento `h2` en lugar de un elemento `h3`.
