...
Ми додали решту пісень до масиву `allSongs` для вас.

Далі ви дізнаєтеся про <dfn>веб аудіо API</dfn> і як його використовувати для відтворення пісень. Усі сучасні браузери підтримують Веб Аудіо API, який дозволяє створювати та обробляти аудіо у вебзастосунках.

Використайте `const`, щоб створити змінну під назвою `audio` та встановіть для неї значення `new Audio()`. Це створить новий елемент HTML5 `audio`.

# --hints--

Використайте ключове слово `new`, щоб створити екземпляр об’єкта `Audio`.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

Призначте об’єкт `Audio` до константи під назвою `audio`.
