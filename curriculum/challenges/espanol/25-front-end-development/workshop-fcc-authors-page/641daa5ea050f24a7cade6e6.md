...
Si se cumple esta condición, deshabilita el botón estableciendo su propiedad `disabled` en `true`. También, establece el `textContent` del botón a `"No más datos para cargar"`.
...
Deberías establecer la propiedad `disabled` de `loadMoreBtn` en `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Deberías establecer el `textContent` de `loadMoreBtn` a `"No más datos para cargar"`.
