...
アバターのサイズの値を `30` にカスタマイズしましょう。

まず、`avatar` と言う名前の const 変数を作成してください。 その変数に、`user.avatar_template` に `replace` メソッドを使用した結果を代入してください。

`replace` メソッドでは、1 番目の実引数に `/{size}/`、2 番目の実引数に数値の `30` を使用してください。

# --hints--

You should have a constant called `avatar`.
