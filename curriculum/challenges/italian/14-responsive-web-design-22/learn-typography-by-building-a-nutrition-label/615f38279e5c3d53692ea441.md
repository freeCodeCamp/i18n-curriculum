...
Il nuovo elemento `div` dovrebbe avere l'attributo `class` con il valore `label`.

```js
assert(document.querySelector('div')?.classList?.contains('label'));
```

Gli elementi `h1` e `p` dovrebbero trovarsi all'interno del nuovo elemento `.label`.
