...
アロー関数が単純な式を返す場合、`return` キーワードおよび波括弧 `{}` を省略できます。 これは暗黙のリターン (implicit return) と呼ばれます。

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

アロー関数の関数本体が複数行のコードを持つ場合には、`return` キーワードと波括弧 `{}` を使用する必要があります。
...
では `addTwoNumbers` 関数をリファクタリング (動作を変えないようにコードを改善すること) して、`return` キーワードと波括弧 `{}` を削除しましょう。 `addTwoNumbers` 関数は、代わりに暗黙のリターンを使用する必要があります。

コンソールを開いて、正しい出力が得られていることを確認してください。

# --hints--

リファクタリングした `addTwoNumbers` 関数では、暗黙のリターンを使用する必要があります。
