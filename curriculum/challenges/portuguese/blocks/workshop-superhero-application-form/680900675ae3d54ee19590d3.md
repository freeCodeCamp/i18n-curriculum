...
Você percebeu que pode enviar o formulário sem preencher os inputs?

Para evitar isso, adicione o atributo `disabled` para desabilitar o botão de envio se `heroName`, `realName` ou `powerSource` forem falsy, ou se o tamanho de `powers` for `0`.

Agora, o botão será desabilitado se algo não estiver preenchido. Preencha o formulário com suas informações de super-herói e envie-o para entrar na liga.

Com isso, seu formulário de inscrição para super-herói está completo.

# --hints--

Você não deve modificar o conteúdo existente do botão.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2/)
```

Seu elemento `button` deve ter uma propriedade `disabled`.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

Você deve desabilitar o `button` se algum de `heroName`, `realName` e `powerSource` for falso ou se o tamanho de `powers` for `0`.
