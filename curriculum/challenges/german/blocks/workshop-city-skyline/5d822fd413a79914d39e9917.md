...
Sie können einem Element mehrere Verläufe hinzufügen, indem Sie diese mit einem Komma (`,`) trennen, so wie hier:
...
Fügen Sie einen `repeating-linear-gradient` zu `.fb1c` unter dem bereits vorhandenen hinzu; verwenden Sie Ihren `--building-color4` von `0%` bis `10%` und `--window-color4` von `10%` bis `90%`. Dadurch wird der Bereich hinter dem zuletzt hinzugefügten Verlauf ausgefüllt.

# --hints--

Sie sollten den ersten `repeating-linear-gradient` nicht verändern.
...
Nach dem ersten `,` sollte ein Komma (`repeating-linear-gradient`) stehen.
...
Sie sollten einen `repeating-linear-gradient` mit einer ersten Farbe von `--building-color4` von `0%` bis `10%` hinzufügen.
...
Sie sollten eine zweite Farbe von `--window-color4` von `10%` bis `90%` verwenden.
