...
Setzen Sie innerhalb der `playSong`-Funktion die `audio.src`-Eigenschaft gleich `song.src`. Damit wird dem Audioelement mitgeteilt, wo es die Audiodaten für den ausgewählten Song findet.
...
Sie sollten die vorhandene `playSong`-Funktion und deren Inhalt nicht verändern.
...
Sie sollten `audio.src` auf `song.src` setzen.
...
Sie sollten `audio.title` auf `song.title` setzen.
