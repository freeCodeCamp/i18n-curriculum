...
Adicione o evento `DOMContentLoaded` ao documento raiz e passe `initializeApp` como a função de callback para executar. Isso garante que toda a inicialização aconteça somente depois que o DOM estiver completamente carregado.

# --hints--

Você deve adicionar o evento `DOMContentLoaded` ao `document` raiz.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

Você deve referenciar `initializeApp` como a função a ser executada no seu listener de evento.
