...
حدد للعنصر `#game` عرضًا أقصى قدره `500px` وارتفاعًا أقصى قدره `400px`. اضبط `background-color` على `#ffffff` و`color` على `#ffffff`.

استخدم الهوامش لمركزة العنصر عبر تعيين الهامش العلوي إلى `30px`، والهامش السفلي إلى `0px`، والهامش الأيسر والأيمن إلى `auto`.

أخيرًا، أعطه حشوًا قدره `10px` على جميع الجوانب الأربعة.

# --hints--

يجب أن يكون لديك محدِّد `#game`.
...
يجب أن يحتوي محدِّد `#game` على `max-width` بقيمة `500px`.
...
يجب أن يحتوي محدِّد `#game` على `max-height` بقيمة `400px`.
...
يجب أن يحتوي محدِّد `#game` على `background-color` بقيمة `#ffffff`.
...
يجب أن يحتوي محدِّد `#game` على `color` بقيمة `#ffffff`.
...
يجب أن يحتوي محدِّد `#game` على `margin-top` مضبوطًا على `30px`.
...
يجب أن يحتوي محدِّد `#game` على `margin-left` مضبوطًا على `auto`.
...
يجب أن يحتوي محدِّد `#game` على `margin-right` مضبوطًا على `auto`.
...
يجب أن يحتوي محدِّد `#game` على `margin-bottom` مضبوطًا على `0px`.
...
يجب أن يحتوي محدِّد `#game` على حشو قدره `10px` على جميع الجوانب.
