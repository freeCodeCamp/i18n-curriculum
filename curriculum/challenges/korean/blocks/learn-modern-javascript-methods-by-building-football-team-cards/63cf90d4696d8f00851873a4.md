...
`team` 속성 아래에 네 개의 속성 키-값 쌍을 추가하세요.

1. `sport`이라는 키와 `"Football"`라는 문자열 값을 추가하세요.  
2. `year`이라는 키와 `1986`라는 숫자 값을 추가하세요.  
3. `isWorldCupWinner`라는 키와 `true`로 설정된 불리언 값을 추가하세요.  
4. `headCoach`라는 새 키와 빈 객체 값을 추가하세요.

# --hints--

`myFavoriteFootballTeam` 객체에 `sport` 속성이 있어야 합니다.

```js
assert.property(myFavoriteFootballTeam, 'sport');
```

`sport` 속성은 `Football`로 설정되어야 합니다.

```js
assert.equal(myFavoriteFootballTeam.sport, 'Football');
```

`myFavoriteFootballTeam` 객체에 `year` 속성이 있어야 합니다.
