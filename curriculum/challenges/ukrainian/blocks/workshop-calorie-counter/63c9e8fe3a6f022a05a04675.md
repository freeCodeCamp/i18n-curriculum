...
Використовуючи той самий синтаксис інтерполяції, додайте другий `p` елемент із текстом `consumedCalories Calories Consumed` і третій із текстом `exerciseCalories Calories Burned`. Не забудьте замінити свої змінні `consumedCalories` та `exerciseCalories` на відповідні значення.

# --hints--

Ви повинні додати другий `p` елемент до свого шаблонного рядка.
...
Ваш другий `p` елемент має бути на новій лінії.
...
Ваш другий `p` елемент має містити текст `${consumedCalories} Calories Consumed`.
...
Ви повинні додати третій `p` елемент до свого шаблонного рядка.
...
Ваш третій `p` елемент має містити текст `${exerciseCalories} Calories Burned`.
