...
أخيرًا، في محدِّد `:root` لاستعلام الوسائط، أعد تعريف جميع المتغيرات الأربعة `--window-color` لاستخدام القيمة `#777`. عند الانتهاء، غيّر حجم النافذة وراقب كيف تتحول من النهار إلى الليل.

تُستخدم المتغيرات أساسًا مع الألوان، وهكذا استخدمتها هنا. لكنها يمكن أن تأخذ أي قيمة وتُستخدم على أي خاصية. مشروعك يبدو رائعًا!

# --hints--

يجب أن تضيف `--window-color1` بقيمة `#777`.
...
يجب أن تضيف `--window-color2` بقيمة `#777`.
...
يجب أن تضيف `--window-color3` بقيمة `#777`.
...
يجب أن تضيف `--window-color4` بقيمة `#777`.
