...
이제 이미지 태그를 만들고 `class` `"user-img"`를 지정하세요. 문자열 보간을 사용해 `src` 속성을 앞서 구조 분해한 `image`로 설정하세요. `alt` 속성은 `author` 다음에 텍스트 `"avatar"`가 오도록 설정하세요. `author` 변수와 단어 `"avatar"` 사이에는 공백이 있어야 합니다. 예를 들어 `"Quincy Larson avatar"`처럼 하세요.
