...
Усередині вашого циклу `while`:

- Виведіть у консоль повідомлення `"\nTick " + (tickIndex + 1)`.
- Пройдіться циклом по кожних воротах у масиві `gates` і викличте `handleGateAtTick()` з такими аргументами:
  - `gates`: повний масив вхідних даних воріт.
  - `gate`: поточний об’єкт воріт.
  - `tickIndex`: поточний такт.
  - `throughputSummary`: локальний об’єкт, що відстежує загальну кількість відвідувачів, оброблених для кожних воріт.
- Збільшіть `tickIndex` на 1, щоб перейти до наступного такту.

# --hints--

Ви маєте вивести у консоль таке повідомлення `"\nTick " + (tickIndex + 1)`.
...
Після виведення у консоль, пройдіться циклом по кожних воротах у масиві `gates` і викличте `handleGateAtTick()` з `gates`, `gate`, `tickIndex` та `throughputSummary`.
...
Після вашого циклу по масиву `gates` збільшіть `tickIndex` на 1.
