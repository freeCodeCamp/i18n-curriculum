...
Du solltest den `.blush.left`-Selektor verwenden.

```js
assert.match(code, /\.blush\.left\s*\{/);
```

Du solltest `.blush.left` eine `left` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush.left')?.left, '15%');
```

Du solltest den `.blush.right`-Selektor verwenden.

```js
assert.match(code, /\.blush\.right\s*\{/);
```

Du solltest `.blush.right` eine `right` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.
