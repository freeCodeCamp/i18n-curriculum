...
Оскільки користувачі зможуть перемішувати та видаляти пісні зі списку відтворення, вам потрібно буде створити копію масиву `allSongs`, не змінюючи оригінал. Ось тут і стане в пригоді оператор `spread` (розгортання).

Оператор розгортання (`...`) дозволяє копіювати всі елементи з одного масиву в інший. Його також можна використовувати для об’єднання кількох масивів в один. У наведеному нижче прикладі `arr1` та `arr2` були розгорнуті в `combinedArr`:
...
Всередині об’єкта `userData` створіть властивість `songs`. Для значення, розгорніть `allSongs` в масив.

# --hints--

Об’єкт `userData` повинен мати ключ під назвою `songs`.

```js
assert.property(userData, "songs");
```

Властивість `songs` повинна мати значення `[...allSongs]`.
