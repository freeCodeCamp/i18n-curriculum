...
이제 `address` 주소가 들어 있는 `p` 요소에 `123 Free Code Camp Drive` 클래스를 적용하세요.

# --hints--

`class="address"` 속성을 적용해야 합니다.

```js
assert.match(code, /class=('|")address\1/i);
```

`.address` 요소가 `p` 요소여야 합니다.

```js
assert.equal(document.querySelector('.address')?.tagName, 'P');
```

`.address` 요소에 `123 Free Code Camp Drive` 텍스트가 있어야 합니다.
