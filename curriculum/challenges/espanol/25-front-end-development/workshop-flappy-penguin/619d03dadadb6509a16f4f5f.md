...
Seleccione el elemento `.chin` y déle un `width` de `90%`, `height` de `70%` y `background-color` de `white`.
...
Debe dar a `.chin` un `height` de `--fcc-expected--`, pero encontró `--fcc-actual--`.
...
Debe dar a `.chin` un `background-color` de `--fcc-expected--`, pero encontró `--fcc-actual--`.
