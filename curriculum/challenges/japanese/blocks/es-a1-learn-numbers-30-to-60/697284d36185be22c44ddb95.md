...
30台の数字がわかったので、次は40台に進みましょう。

スペイン語で40は `cuarenta` と呼ばれます。

41から49までの数字は、10の位の単語 `cuarenta`（40）に接続詞 `y`（「と」）をつけ、その後に1から9の数字を続けて作ります。
...
この範囲で数字 `cuarenta y uno`（41）は特別で、男性名詞の前では `cuarenta y un` に変わります。例えば：

`Cuarenta y un años` - 41歳。
