...
`Bee` 클래스 안에 `render` 키워드로 접두사된 `override` 메서드를 구현하여 `innerText`의 `emojiElement` 속성을 `emoji` 속성의 값으로 업데이트하세요.

# --hints--

`Bee` 클래스에는 `render` 키워드를 사용한 `override` 메서드가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

`this.emojiElement.innerText`의 `this.emoji` 메서드 안에서 `render`을 `Bee`으로 설정해야 합니다.
