...
Unapaswa kuongeza kipengele kimoja cha `p` ndani ya `.shirt`, lakini ulibaini `--fcc-actual--`.

```js
assert.lengthOf(document.querySelectorAll('.shirt > p'), 1);
```

Unapaswa kumpa kipengele cha `p` maudhui ya `I CSS`, lakini ulibaini `--fcc-actual--`.
