...
Crear otro elemento `label` (en una nueva línea) al final de su `HTMLString`. Este `label` debe tener el texto `Entry # Calories`, usando tu sintaxis literal de plantillas para reemplazar `#` con el valor de `entryNumber`, y el atributo `for` configurado en `X-#-calories`, donde `X` es el `value` de `entryDropdown` y `#` es el valor de `entryNumber`.
