...
帶有類別 `div` 的 `card-wrapper` 元素創建了卡片的外部邊框和裁剪形狀。

現在直接在 `div` 裡面添加內部的 `card-wrapper`，並給它 `className` 為 `card`。這樣卡片就有了深色背景。

# --hints--

你應該在 `div` 元素內添加一個帶有 `className` 為 `card` 的 `.card-wrapper`。
