...
HTML의 앨범 아트와 `userData.songs` 배열의 노래가 바뀐 것을 확인하세요. 원래 노래 대신 앞으로 단계에서 앱을 테스트할 수 있도록 더 짧은 노래로 교체했습니다.

다음으로, 재생할 다음 노래가 있는지 확인해야 합니다. `getCurrentSongIndex()` 함수를 호출해 현재 노래 인덱스를 가져와 `currentSongIndex` 상수에 저장하세요.

그다음, 다음 노래가 존재하는지에 따라 불리언 값 `nextSongExists` 또는 `true`를 담은 `false` 상수를 만드세요.

# --hints--

기존 `ended` 이벤트 리스너와 그 내용을 수정하지 마세요.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

`currentSongIndex` 상수를 만들고 `getCurrentSongIndex` 함수를 호출한 결과로 설정하세요.
...
다음 노래가 있는지 `userData.songs.length`와 `currentSongIndex`를 비교해 확인하고 `nextSongExists` 상수에 저장하세요. songs 배열의 마지막 인덱스(`userData.songs.length - 1`)가 currentSongIndex보다 크면 다음 노래가 있다는 뜻입니다.
