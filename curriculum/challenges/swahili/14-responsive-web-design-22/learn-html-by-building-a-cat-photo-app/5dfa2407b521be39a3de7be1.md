...
Your anchor (`a`) element does not have a `target` attribute. Hakikisha kuwa kuna nafasi baada ya jina la tagi inayofungua na/au kuna nafasi kabla ya majina yote ya sifa.

```js
assert(document.querySelector('a').hasAttribute('target'));
```

The value of the `target` attribute should be `_blank`. Aidha umesahau maandishi, au kuna makosa ya maandishi. Kumbuka kwamba thamani za sifa zinapaswa kuzungukwa na alama za nukuu.
