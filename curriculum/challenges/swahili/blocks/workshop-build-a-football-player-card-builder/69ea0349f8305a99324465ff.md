...
Katika blokki ya `try`, tengeneza kigezo cha `saved`. Kwa thamani, tumia njia ya `getItem()` kupata `STORAGE_KEY`. Moja kwa moja chini ya kigezo hicho, tengeneza tamko la `if` lenye sharti la `saved`. Acha blokki ya `if` kuwa tupu kwa sasa.

# --hints--

Kitendakazi chako cha `loadPlayer` kinapaswa kuita `localStorage.getItem` na `STORAGE_KEY` ndani ya blokki ya `try`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Matokeo ya `localStorage.getItem` yanapaswa kuhifadhiwa katika kigezo kinachoitwa `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Blokki ya `try` inapaswa kuwa na tamko la `if` lenye sharti la `saved`.
