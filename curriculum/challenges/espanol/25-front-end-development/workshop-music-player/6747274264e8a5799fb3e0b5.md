...
Agrega un segundo objeto con las siguientes claves y valores:
...
Tu arreglo `allSongs` debería tener dos valores.

```js
assert.equal(allSongs.length, 2);
```

Tu arreglo `allSongs` debería tener un objeto como su segundo valor.

```js
assert.isObject(allSongs[1]);
```

El segundo objeto en tu arreglo `allSongs` debería tener una propiedad `id` establecida en el número `1`.

```js
assert.strictEqual(allSongs[1].id, 1);
```

El segundo objeto en tu arreglo `allSongs` debería tener una propiedad `title` establecida en la cadena `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

El segundo objeto en tu arreglo `allSongs` debería tener una propiedad `artist` establecida en la cadena `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

El segundo objeto en tu arreglo `allSongs` debería tener una propiedad `duration` establecida en la cadena `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

El segundo objeto en tu arreglo `allSongs` debería tener una propiedad `src` establecida en la cadena `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
