...
title: 在 Tweet Embed 中使用 `justify-content` 屬性
challengeType: 0
...
上一個挑戰展示了 `justify-content` 屬性的範例。對於推文嵌入，這個屬性可以用來對齊 `.profile-name` 元素中的項目。

# --instructions--

為你的標頭的 `.profile-name` 元素添加 CSS 屬性 `justify-content`，並將值設為上一個挑戰中的任一選項。
...
`.profile-name` 元素的 `justify-content` 屬性應設定為以下任一值：`center`、`flex-start`、`flex-end`、`space-between`、`space-around` 或 `space-evenly`。
