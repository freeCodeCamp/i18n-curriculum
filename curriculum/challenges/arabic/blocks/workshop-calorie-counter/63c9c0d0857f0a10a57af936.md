...
باتباع نفس النمط، استعلم عن مُدخَلات `number` في عناصر `#dinner` و`#snacks` و`#exercise`. عيّنها إلى متغيرات باتباع نظام التسمية المستخدم في المتغيرين السابقين.

# --hints--

يجب إعلان متغير `dinnerNumberInputs`.

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

يجب استدعاء `document.querySelectorAll()` مع `#dinner input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

يجب تعيين نتيجة استدعاء `document.querySelectorAll()` إلى `dinnerNumberInputs`.
...
يجب إعلان متغير `snacksNumberInputs`.

```js
assert.match(calculateCalories.toString(), /snacksNumberInputs\s*=/);
```

يجب استدعاء `document.querySelectorAll()` مع `#snacks input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#snacks input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

يجب تعيين نتيجة استدعاء `document.querySelectorAll()` إلى `snacksNumberInputs`.
...
يجب إعلان متغير `exerciseNumberInputs`.

```js
assert.match(calculateCalories.toString(), /exerciseNumberInputs\s*=/);
```

يجب استدعاء `document.querySelectorAll()` مع `#exercise input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#exercise input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

يجب تعيين نتيجة استدعاء `document.querySelectorAll()` إلى `exerciseNumberInputs`.
