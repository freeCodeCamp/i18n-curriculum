...
エラーを修正するには、`handlePowersChange` のアロー関数を `e` のパラメータで作成してください。

関数内で、チェックボックスの値とチェックされているかどうかを取得するために、`value` から `checked` と `e.target` を分割代入してください。

# --hints--

`handlePowersChange` のパラメータを持つ `e` 関数を作成する必要があります。

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

`value` から `checked` と `e.target` を分割代入する必要があります。
