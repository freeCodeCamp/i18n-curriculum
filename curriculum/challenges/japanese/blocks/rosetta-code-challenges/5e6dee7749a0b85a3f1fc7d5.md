...
Lucas-Lehmerテスト：$p$が奇素数のとき、メルセンヌ数 $2^p-1$ は、$2^p-1$ が $S(p-1)$ を割り切る場合に限り素数です。ここで $S(n+1)=(S(n))^2-2$、$S(1)=4$ です。

# --instructions--

与えられたメルセンヌ数が素数かどうかを返す関数を書いてください。

# --hints--

`lucasLehmer` は関数であるべきです。

```js
assert(typeof lucasLehmer == 'function');
```

`lucasLehmer(11)` はブール値を返すべきです。

```js
assert(typeof lucasLehmer(11) == 'boolean');
```

`lucasLehmer(11)` は `false` を返すべきです。

```js
assert.equal(lucasLehmer(11), false);
```

`lucasLehmer(15)` は `false` を返すべきです。

```js
assert.equal(lucasLehmer(15), false);
```

`lucasLehmer(13)` は `true` を返すべきです。

```js
assert.equal(lucasLehmer(13), true);
```

`lucasLehmer(17)` は `true` を返すべきです。

```js
assert.equal(lucasLehmer(17), true);
```

`lucasLehmer(19)` は `true` を返すべきです。

```js
assert.equal(lucasLehmer(19), true);
```

`lucasLehmer(21)` は `false` を返すべきです。
