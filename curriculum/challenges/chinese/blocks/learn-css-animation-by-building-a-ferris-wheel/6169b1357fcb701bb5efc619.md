...
在 `0%` 和 `50%` 选择器之间创建一个新的 `25%` 选择器。 给这个新的选择器添加一个 `background-color` 属性，设置为 `yellow`。

# --hints--

你应该在 `@keyframes cabins` 规则中创建一个新的 `25%` 选择器。
