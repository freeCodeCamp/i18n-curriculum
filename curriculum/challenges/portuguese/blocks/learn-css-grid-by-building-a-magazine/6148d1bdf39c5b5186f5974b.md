...
Crie um seletor `hr` e dê a ele uma propriedade `margin` definida como `1.5rem 0`.

# --hints--

Você deve ter um seletor `hr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('hr'));
```

O seletor `hr` deve ter a propriedade `margin` definida como `1.5rem 0`.
