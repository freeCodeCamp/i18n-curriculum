...
從 HTML 文件訪問 `#next`、`#previous` 和 `#shuffle` 元素。

將它們分別分配給名爲 `nextButton`、`previousButton` 和 `shuffleButton` 的變量。

# --hints--

你應該有一個名爲 `nextButton` 的變量。

```js
assert.isDefined(nextButton);
```

你應該使用 `document.getElementById()` 來獲取 `#next` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

你應該將 `#next` 元素分配給變量 `nextButton`。

```js
assert.equal(nextButton, document.getElementById('next'));
```

你應該有一個名爲 `previousButton` 的變量。

```js
assert.isDefined(previousButton);
```

你應該使用 `document.getElementById()` 來獲取 `#previous` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

你應該將 `#previous` 元素分配給變量 `previousButton`。

```js
assert.equal(previousButton, document.getElementById('previous'));
```

你應該有一個名爲 `shuffleButton` 的變量。

```js
assert.isDefined(shuffleButton);
```

你應該使用 `document.getElementById()` 來獲取 `#shuffle` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

你應該將 `#shuffle` 元素分配給變量 `shuffleButton`。
