...
Se você quiser adicionar mais ícones sociais, mas mantê-los na mesma linha, você precisa atualizar `grid-template-columns` para criar colunas adicionais.Como alternativa, você pode usar a propriedade `grid-auto-flow`.

Esta propriedade aceita `row` ou `column` como o primeiro valor, com um segundo valor opcional de `dense`. `grid-auto-flow` usa um algoritmo de posicionamento automático para ajustar o layout da grade.Defini-lo como `column` dirá ao algoritmo para criar colunas para o conteúdo conforme necessário.O valor `dense` permite que o algoritmo reveja seus passos e preencha os espaços na grade com itens menores, o que pode fazer com que os itens apareçam fora de ordem.

Para o seletor `.social-icons`, defina a propriedade `grid-auto-flow` como `column`.

# --hints--

O seletor `.social-icons` deve ter a propriedade `grid-auto-flow` definida como `column`.
