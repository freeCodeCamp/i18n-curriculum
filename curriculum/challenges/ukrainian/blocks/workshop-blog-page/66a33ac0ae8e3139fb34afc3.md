...
Ви додали всі дописи, тож спробуйте натиснути на посилання `Posts` і побачите, що сторінка перейде до розділу `Posts`.

Останній компонент, який потрібно додати до блогу, — це розділ з контактною інформацією.

Додайте елемент `footer` під елементом `main`.

# --hints--

Елемент `footer` повинен мати початковий тег.

```js
assert.match(code, /<footer\>/);
```

Елемент `footer` повинен мати кінцевий тег.

```js
assert.match(code, /<\/footer\>/);
```

Елемент `footer` має бути в межах елемента `body`.
