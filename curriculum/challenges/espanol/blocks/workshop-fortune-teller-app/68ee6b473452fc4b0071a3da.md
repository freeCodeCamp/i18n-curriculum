...
Ahora haz lo mismo para `multipleCardsBtn` y `newReadingBtn` adjuntando un listener de evento click a `multipleCardsBtn` que llame a `multipleCardSelected`, y otro a `newReadingBtn` que llame a `newReading`.

Crearás los dos métodos en los próximos pasos.

# --hints--

Debes adjuntar un evento `click` al elemento `multipleCardsBtn`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Dentro del evento callback `multipleCardsBtn`, debes llamar al método `multipleCardSelected`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

Debes adjuntar un evento `click` al elemento `newReadingBtn`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Dentro del evento callback `newReadingBtn`, debes llamar al método `newReading`.
