...
Der Wert der `currentWeaponIndex` Variable entspricht einem Index im ` weapons ` Array. The player starts with a `"stick"`, since `currentWeaponIndex` starts at `0` and `weapons[0]` is the `"stick"` weapon.
