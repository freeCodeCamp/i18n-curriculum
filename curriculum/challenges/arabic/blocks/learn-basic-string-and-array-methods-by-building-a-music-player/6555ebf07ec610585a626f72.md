...
استخدم الطريقة `filter()` لإزالة كائن الأغنية الذي يطابق معلمة `id` من مصفوفة `userData?.songs`.

تحافظ طريقة `filter` على العناصر فقط في المصفوفة التي تحقق دالة الاستدعاء الراجع الممررة إليها:
...
استخدم الطريقة `filter()` على `userData?.songs`. مرر `song` كمعلمة لدالة الاستدعاء الراجع السهمية واستخدم الإرجاع الضمني للتحقق مما إذا كان `song.id` لا يساوي `id` تمامًا. عيّن كل ذلك إلى `userData.songs`.

*ملاحظة*: لا يجب استخدام التسلسل الاختياري عند تعيين نتيجة `userData?.songs.filter` إلى `userData.songs` لأن مصفوفة `allSongs` لن تكون `undefined` أو `null` في تلك النقطة.

# --hints--

لا يجب تعديل الدالة `deleteSong` الموجودة.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

يجب استخدام الطريقة `filter()` على `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

يجب أن تستخدم دالة الاستدعاء الراجع لطريقة `filter()` صيغة السهم وأن تحتوي على معلمة `song`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

يجب أن تُرجع طريقة الفلترة ضمنيًا `song.id !== id`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

يجب تعيين نتيجة استخدام الطريقة `filter()` على `userData?.songs` إلى `userData.songs`.
