...
Використайте метод `filter()`, щоб видалити об’єкт пісні, який відповідає параметру `id`, з масиву `userData?.songs`.

Метод `filter` зберігає лише ті елементи масиву, які задовольняють функцію зворотного виклику, передану йому:
...
Використайте метод `filter()` на `userData?.songs`. Передайте `song` як параметр стрілкової функції зворотного виклику та використайте неявне повернення, щоб перевірити, чи `song.id` строго не дорівнює `id`. Призначте все це `userData.songs`.

*Примітка*: Не слід використовувати опціональний ланцюжок при призначенні результату `userData?.songs.filter` у `userData.songs`, оскільки масив `allSongs` на той момент не буде `undefined` або `null`.

# --hints--

Не слід змінювати існуючу функцію `deleteSong`.
...
Функція зворотного виклику вашого методу `filter()` має бути стрілковою та мати параметр `song`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

Ваш метод filter має неявно повертати `song.id !== id`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

Слід призначити метод `filter()`, який ви використали на `userData?.songs`, у `userData.songs`.
