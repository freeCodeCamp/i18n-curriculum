...
已经提供了卡片的默认图像和用于发起 API 请求的 URL，你只需要创建一些变量来储存它们。

创建一个 `CDN_URL` 变量并将其设置为 `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`。同时创建一个 `defaultImg` 变量，值为 `new URL("default.svg", CDN_URL + "/")`，以及一个 `LOCAL_DEFAULT_IMG` 变量，值为 `defaultImg`。

# --hints--

你应该创建一个名为 `CDN_URL` 的变量。
...
你的 `CDN_URL` 变量应该设置为 `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"`。
...
你的 `defaultImg` 变量应设置为 `new URL("default.svg", CDN_URL + "/")`。
...
你应该创建一个名为 `LOCAL_DEFAULT_IMG` 的变量。
...
你的 `LOCAL_DEFAULT_IMG` 变量应该设置为 `defaultImg`。
