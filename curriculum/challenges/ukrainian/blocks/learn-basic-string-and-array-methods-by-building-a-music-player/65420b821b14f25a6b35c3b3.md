...
Коли пісні відображаються на сторінці, має показуватися назва, виконавець, тривалість кожної пісні та кнопка видалення.

У наступних кількох кроках ви навчитеся додавати новий HTML для кожної пісні за допомогою методу <dfn>map()</dfn>. Цей метод буде детальніше пояснений у наступному кроці.

Почніть із використання `const` для оголошення змінної з іменем `songsHTML` і призначте їй `array.map()`.

# --hints--

У вас має бути змінна `const` з назвою `songsHTML`.

```js
assert.match(code, /const\s+songsHTML/);
```

Ви маєте призначити `array.map()` своїй `songsHTML`.
