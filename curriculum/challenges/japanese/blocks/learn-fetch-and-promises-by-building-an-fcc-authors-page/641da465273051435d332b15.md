...
`GET` リクエストで取得したデータは最初は使えません。データを使えるようにするには、`.json()` オブジェクトの `Response` メソッドを使って JSON に解析します。ブラウザのコンソールで `Prototype` オブジェクトの `Response` を展開すると、そこに `.json()` メソッドが見えます。

`console.log(res)` を削除し、代わりに暗黙的に `res.json()` を返してください。
