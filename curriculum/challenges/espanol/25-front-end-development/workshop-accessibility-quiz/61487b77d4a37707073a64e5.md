...
Cuando el ancho de la pantalla es pequeño, el elemento `h1` no ajusta su contenido de texto como debería. Alinea el texto para el elemento `h1` en el centro.

Luego, da al elemento `main` un padding tal que el encabezado de la sección `Student Info` se vea completamente.

# --hints--

Debes dar al `h1` un `text-align` de `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

Debes añadir un selector `main` para dirigir al elemento `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Debes dar al `main` un `padding-top` de al menos `25px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

Solo debes cambiar el valor de `padding-top`.
