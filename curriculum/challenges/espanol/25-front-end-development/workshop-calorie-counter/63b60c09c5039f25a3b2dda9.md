...
Crea otro elemento `div`. Dentro de él, anida un `button` para `submit` el formulario. Este botón debe tener el texto `Calculate Remaining Calories`.

Luego agrega un `button` con el `id` configurado en `clear` para limpiar el formulario (no olvides darle un atributo `type` que evite que lo envíe). Este botón necesita el texto `Clear`.
