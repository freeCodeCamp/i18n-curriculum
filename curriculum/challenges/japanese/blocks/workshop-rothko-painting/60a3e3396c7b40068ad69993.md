...
`border-radius` セレクターの `.two` プロパティを使って、左上の半径と右下の半径を `8px` に、右上の半径と左下の半径を `10px` に設定してください。

# --hints--

`border-radius` プロパティを `8px 10px` に設定する必要があります。
...
`.two` 要素は `border-radius` が `8px 10px` であるべきです。
