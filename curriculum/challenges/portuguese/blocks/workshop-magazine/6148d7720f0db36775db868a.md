...
Você precisará ter uma coluna para texto e outra para imagens. Dê ao seletor `.text-with-images` uma propriedade `grid-template-columns` definida como `1fr 2fr`. Também defina a propriedade `column-gap` como `3rem` para fornecer mais espaçamento entre as colunas.

# --hints--

O seletor `.text-with-images` deve ter a propriedade `grid-template-columns` definida como `1fr 2fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns, '1fr 2fr');
```

O seletor `.text-with-images` deve ter a propriedade `column-gap` definida como `3rem`.
