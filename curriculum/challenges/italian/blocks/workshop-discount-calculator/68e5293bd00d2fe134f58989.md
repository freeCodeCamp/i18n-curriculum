...
Ora implementa il metodo `apply_discount` per `FixedAmountDiscount`. Questo metodo dovrebbe sottrarre l'importo fisso dal prezzo del prodotto e restituire il nuovo prezzo.

# --hints--

La tua classe `FixedAmountDiscount` dovrebbe avere un metodo `apply_discount`.
...
Il tuo metodo `apply_discount` dovrebbe restituire `product.price - self.amount`.
