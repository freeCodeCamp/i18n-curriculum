...
يجب عليك إضافة عدد`--fcc-expected--` من عناصر `div` إلى `.penguin-head`، ولكن وجدت `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 2);
```

بداخل عنصر `div` الأول يجب أن تكون خاصية `class` بقيمة `face`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.include(document.querySelector('.penguin-head > div:nth-of-type(1)')?.className, 'face');
```

بداخل عنصر `div` الثاني يجب أن تكون خاصية `class` بقيمة `face`, ولكن وجدت بقيمة `--fcc-actual--`.
