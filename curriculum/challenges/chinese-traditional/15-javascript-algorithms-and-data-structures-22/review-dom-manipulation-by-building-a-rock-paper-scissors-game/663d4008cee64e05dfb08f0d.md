...
完成 `showResults` 函數。 `playerScoreSpanElement` 和 `computerScoreSpanElement` 應該更新以顯示玩家和電腦的最新得分。

`roundResultsMsg` 也應該更新爲該輪比賽的結果。

**提示：**

- 記得你學過如何使用 `innerText` 屬性來更新元素的文本內容。
- 你可以使用 `getRoundResults` 函數來獲取比賽結果。

# --hints--

你的 `showResults` 函數應該使用回合結果更新 `roundResultsMsg`。
...
你的 `showResults` 函數應該更新 `computerScoreSpanElement` 以顯示更新後的電腦得分。
...
你的 `showResults` 函數應該更新 `playerScoreSpanElement` 以顯示玩家的最新得分。
