...
现在你已经有了应用程序的基本结构，是时候添加一些项目并让搜索功能正常运行了。

创建一个数组并将其分配给名为 `items` 的变量。 在数组中，添加字符串 `Apples`、`Bananas`、`Strawberries`、`Blueberries`、`Mangoes`、`Pineapple`、`Lettuce`、`Broccoli`、`Paper Towels` 和 `Dish Soap`。 这将是应用程序中显示的项目列表。
...
你的 `items` 数组应包含字符串 `Apples`、`Bananas`、`Strawberries`、`Blueberries`、`Mangoes`、`Pineapple`、`Lettuce`、`Broccoli`、`Paper Towels` 和 `Dish Soap`。
