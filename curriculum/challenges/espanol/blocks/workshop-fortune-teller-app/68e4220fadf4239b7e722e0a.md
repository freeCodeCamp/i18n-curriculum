...
Como la aplicación de fortuna solo tiene tres momentos: pasado, presente y futuro, declara un enum llamado `DrawingType` con tres propiedades: `Past`, `Present` y `Future`. Cada propiedad debe tener un valor de cadena que corresponda a su nombre.

# --hints--

Deberías tener `enum` llamado `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Tu enum debe tener una propiedad `Past` con un valor de cadena `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Tu enum debe tener una propiedad `Present` con un valor de cadena `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Tu enum debe tener una propiedad `Future` con un valor de cadena `future`.
