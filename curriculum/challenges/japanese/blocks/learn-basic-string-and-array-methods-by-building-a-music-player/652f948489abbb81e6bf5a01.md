...
このプロジェクトでは、音楽プレーヤーアプリを作りながら基本的な文字列と配列のメソッドを学びます。曲の再生、一時停止、スキップ、シャッフルができるようになります。

このプロジェクトのHTMLとCSSは用意されているので、JavaScriptにフォーカスしてください。

まず、`#playlist-songs`、`#play`、`#pause`の要素を`getElementById()`メソッドでアクセスしてください。それぞれ変数`playlistSongs`、`playButton`、`pauseButton`に代入してください。


# --hints--

`playlistSongs`という変数を用意してください。

```js
assert.isDefined(playlistSongs);
```

`document.getElementById()`を使って`#playlist-songs`要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

`#playlist-songs`要素を変数`playlistSongs`に代入してください。

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

`playButton`という変数を用意してください。

```js
assert.isDefined(playButton);
```

`document.getElementById()`を使って`#play`要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

`#play`要素を変数`playButton`に代入してください。

```js
assert.equal(playButton, document.getElementById('play'));
```

`pauseButton`という変数を用意してください。

```js
assert.isDefined(pauseButton);
```

`document.getElementById()`を使って`#pause`要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

`#pause`要素を変数`pauseButton`に代入してください。
