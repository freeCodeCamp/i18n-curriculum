...
为 SHO `value` 添加一个 `player.sho` 属性，属性名为 `input`。然后将它的 `onChange` 属性设置为一个箭头函数，该函数以 `e` 作为参数，并调用 `setPlayer`，传入 `player` 的展开和 `sho` 设置为 `Number(e.target.value)`。

# --hints--

你的 SHO `input` 应该有一个 `value`，其值等于当前玩家的 `sho` 属性。
...
你的 SHO `input` 应该有一个 `onChange` 处理程序，该处理程序调用 `setPlayer`，传入 `player` 的展开和 `sho` 设置为 `Number(e.target.value)`。
