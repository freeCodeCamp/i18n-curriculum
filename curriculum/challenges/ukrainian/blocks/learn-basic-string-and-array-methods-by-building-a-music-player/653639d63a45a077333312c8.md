...
Перед тим як відтворити пісню, потрібно переконатися, що вона починається з початку. Це можна зробити за допомогою властивості `currentTime` об’єкта `audio`.

Додайте інструкцію `if`, щоб перевірити, чи є `userData?.currentSong` хибним *АБО* чи `userData?.currentSong.id` строго не дорівнює `song.id`. Ця умова перевірить, чи наразі не відтворюється жодна пісня або якщо поточна пісня відрізняється від тієї, що збирається відтворюватися.

Всередині блоку `if` встановіть властивість `currentTime` об’єкта `audio` у значення `0`.

# --hints--

Ви повинні створити інструкцію `if` з умовою `userData?.currentSong === null || userData?.currentSong.id !== song.id`. Не забудьте включити опціональний ланцюжок.
...
Ви повинні встановити `audio.currentTime` у `0` всередині вашого блоку `if`.
