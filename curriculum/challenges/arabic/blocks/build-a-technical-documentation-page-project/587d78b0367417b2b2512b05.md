...
**الهدف:** بناء تطبيق يشبه وظيفيًا <a href="https://technical-documentation-page.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://technical-documentation-page.freecodecamp.rocks</a>. **لا تنسخ هذا المشروع التجريبي**.

**قصص المستخدم:**

1. يجب أن ترى عنصر `main` مع `id="main-doc"` المقابل، والذي يحتوي على المحتوى الرئيسي للصفحة (التوثيق الفني)
1. داخل عنصر `#main-doc`، يجب أن ترى عدة عناصر `section`، كل منها يحمل صنف `main-section`. يجب أن يكون هناك خمسة على الأقل
1. أول عنصر داخل كل `.main-section` يجب أن يكون عنصر `header`، يحتوي على نص يصف موضوع ذلك القسم.
1. يجب أن يحتوي كل عنصر `section` الذي يحمل صنف `main-section` على `id` يتوافق مع نص كل `header` بداخله. يجب استبدال أي فراغات بشرطات سفلية (مثلاً القسم الذي يحتوي على العنوان "JavaScript and Java" يجب أن يكون له `id="JavaScript_and_Java"` المقابل)
1. يجب أن تحتوي عناصر `.main-section` على ما لا يقل عن عشرة عناصر `p` إجمالاً (ليس لكل منها)
1. يجب أن تحتوي عناصر `.main-section` على ما لا يقل عن خمسة عناصر `code` إجمالاً (ليس لكل منها)
1. يجب أن تحتوي عناصر `.main-section` على ما لا يقل عن خمسة عناصر `li` إجمالاً (ليس لكل منها)
1. يجب أن ترى عنصر `nav` مع `id="navbar"` المقابل
1. يجب أن يحتوي عنصر شريط التنقل على عنصر `header` واحد يحتوي على نص يصف موضوع التوثيق الفني
1. بالإضافة إلى ذلك، يجب أن يحتوي شريط التنقل على عناصر رابط (`a`) تحمل صنف `nav-link`. يجب أن يكون هناك واحد لكل عنصر يحمل صنف `main-section`
1. يجب أن يأتي عنصر `header` في `#navbar` قبل أي عناصر رابط (`a`) في شريط التنقل
1. يجب أن يحتوي كل عنصر يحمل صنف `nav-link` على نص يتوافق مع نص `header` داخل كل `section` (مثلاً إذا كان لديك قسم/عنوان "Hello world"، يجب أن يحتوي شريط التنقل على عنصر يحمل النص "Hello world")
1. عند النقر على عنصر في شريط التنقل، يجب أن تنتقل الصفحة إلى القسم المقابل في عنصر `#main-doc` (مثلاً إذا نقرت على عنصر `.nav-link` يحتوي على النص "Hello world"، تنتقل الصفحة إلى عنصر `section` بهذا المعرف، ويحتوي على العنوان المقابل)
1. على الأجهزة ذات الحجم العادي (أجهزة اللابتوب، أجهزة سطح المكتب)، يجب أن يظهر العنصر الذي يحمل `id="navbar"` على الجانب الأيسر من الشاشة ويكون مرئيًا دائمًا للمستخدم
1. يجب أن يستخدم التوثيق الفني استعلام وسائط واحد على الأقل

نفذ قصص المستخدم واجتز جميع الاختبارات أدناه لإكمال هذا المشروع. أضف أسلوبك الشخصي. برمجة سعيدة!

**ملاحظة:** تأكد من إضافة `<link rel="stylesheet" href="styles.css">` في ملف HTML لربط ورقة الأنماط وتطبيق CSS الخاص بك

# --hints--

يجب أن يكون لديك عنصر `main` يحمل `id` بقيمة `main-doc`.
...
يجب أن يكون لديك خمسة عناصر `section` على الأقل تحمل صنف `main-section`.
...
يجب أن تكون كل عناصر `.main-section` من نوع `section`.
...
يجب أن يكون لديك خمسة عناصر `.main-section` على الأقل تكون عناصر تابعة لـ `#main-doc`.
...
يجب أن يكون أول عنصر تابع لكل `.main-section` هو عنصر `header`.
...
لا يجب أن يكون أي من عناصر `header` فارغًا.
...
يجب أن يكون لكل عناصر `.main-section` معرف `id`.
...
يجب أن يكون لكل `.main-section` معرف `id` يطابق نص أول عنصر تابع له، مع استبدال أي فراغات في نص الطفل بشرطات سفلية (`_`) للمعرف.
...
يجب أن يكون لديك ما لا يقل عن 10 عناصر `p` (إجمالاً) داخل عناصر `.main-section`.
...
يجب أن يكون لديك ما لا يقل عن خمسة عناصر `code` تابعة لعناصر `.main-section`.
...
يجب أن يكون لديك ما لا يقل عن خمسة عناصر `li` تابعة لعناصر `.main-section`.
...
يجب أن يكون لديك عنصر `nav` يحمل `id` بقيمة `navbar`.
...
يجب أن يحتوي `#navbar` على عنصر `header` واحد فقط بداخله.
...
يجب أن يكون لديك عنصر `a` واحد على الأقل يحمل صنف `nav-link`.
...
يجب أن تكون كل عناصر `.nav-link` من نوع رابط (`a`).
...
يجب أن تكون كل عناصر `.nav-link` داخل `#navbar`.
...
يجب أن يكون عدد عناصر `.nav-link` مساويًا لعدد عناصر `.main-section`.
...
يجب أن يأتي عنصر `header` في `#navbar` قبل أي عناصر رابط (`a`) أخرى في `#navbar`.
...
يجب أن يحتوي كل عنصر `.nav-link` على نص يطابق نص القسم العلوي `header` في قسمه `section` المرتبط (على سبيل المثال، إذا كان لديك قسم/قسم علوي بعنوان "Hello world"، يجب أن يكون في شريط التنقل `#navbar` عنصر `.nav-link` يحمل النص "Hello world").
...
يجب أن يحتوي كل `.nav-link` على خاصية `href` تربطه بـ `.main-section` المقابل (مثلاً إذا نقرت على عنصر `.nav-link` يحتوي على النص "Hello world"، تنتقل الصفحة إلى عنصر `section` بهذا المعرف).
...
يجب أن يكون `#navbar` دائمًا على الحافة اليسرى من النافذة.
...
يجب أن يستخدم مشروع التوثيق الفني استعلام وسائط واحد على الأقل.
