...
لعرض عنوان الموضوع، أضف عنصر `p` داخل أول عنصر `td`.

بين وسمَي الفقرة، أضف تعبيرًا مضمنًا يحتوي على المتغير `title`. ثم أضف صنفًا اسمه `"post-title"` داخل وسم الفقرة الافتتاحي.

# --hints--

يجب أن يكون لديك عنصر `p` داخل أول عنصر `td`.

```js
assert(/return\s*`\s*<tr>\s*<td>\s*\S*\s*<p.*>\s*.*\s*<\/p>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```

يجب أن يكون لديك صنف اسمه `"post-title"` داخل وسم `p` الافتتاحي.
...
يجب أن يكون المتغير `title` بين وسمَي الفقرة.
