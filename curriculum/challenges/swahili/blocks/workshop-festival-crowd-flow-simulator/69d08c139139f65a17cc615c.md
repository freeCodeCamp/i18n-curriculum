...
Ndani ya mzunguko wako wa `while`:

- Andika ujumbe, `"\nTick " + (tickIndex + 1)`.
- Pitia kila lango katika safu ya data ya `gates` na fanya wito wa `handleGateAtTick()` ukitumia hoja hizi:
  - `gates`: safu kamili ya ingizo la milango.
  - `gate`: kitu cha lango la sasa.
  - `tickIndex`: tiketi ya sasa.
  - `throughputSummary`: kitu cha ndani kinachofuatilia jumla ya washiriki waliopitiwa kwa kila lango.
- Ongeza `tickIndex` kwa 1 ili uende kwenye tiketi inayofuata.

# --hints--

Unapaswa kuandika ujumbe ufuatao kwenye konsoli, `"\nTick " + (tickIndex + 1)`.
...
Baada ya kuandika kwenye konsoli, unapaswa kupitia kila lango katika safu ya data ya `gates` na kufanya wito wa `handleGateAtTick()` ukitumia `gates`, `gate`, `tickIndex`, na `throughputSummary`.
...
Baada ya mzunguko wako kupitia safu ya data ya `gates`, unapaswa kuongeza `tickIndex` kwa 1.
