...
إذا لم تكن `userData.currentSong` تساوي `null`، عليك إيجاد الأغنية التالية للتشغيل. تذكر أنه يمكنك استخدام `getNextSong` لهذا الغرض.

أكمل دالة `playNextSong` بحيث تشغل الأغنية التالية في قائمة التشغيل. إذا كانت الأغنية الحالية هي الأخيرة، عيّن `userData.currentSong` إلى `null`، و`userData.songCurrentTime` إلى صفر، واستدعِ دالة `pauseSong`.

# --hints--

عندما تكون `currentSong` في `userData` صحيحة وليست الأغنية الأخيرة في قائمة التشغيل، يجب أن تشغل دالة `playNextSong` الأغنية التي تلي الأغنية الحالية.
...
عندما تكون `currentSong` في `userData` هي الأغنية الأخيرة في قائمة التشغيل، يجب أن تعيّن دالة `playNextSong` قيمة `userData.currentSong` إلى `null`.
...
عندما تكون `currentSong` في `userData` هي آخر أغنية في قائمة التشغيل، يجب أن تُعيّن الدالة `playNextSong` القيمة `0` للخاصية `userData.songCurrentTime`.
...
عندما تكون `currentSong` في `userData` هي الأغنية الأخيرة في قائمة التشغيل، يجب أن تستدعي دالة `playNextSong` دالة `pauseSong`.

```js
assert.match(playNextSong.toString(), /pauseSong\s*\(\s*\)/);
```

عندما تكون `currentSong` في `userData` تساوي `null`، يجب أن تستدعي دالة `playNextSong` دالة `playSong` مع `id` أول أغنية في مصفوفة `userData.songs` كمعلمة.
