...
اعطي عنصر `.quote` النص `The entire curriculum should be a series of projects`.

# --hints--

يجب عليك إنشاء عنصر `blockquote` جديد داخل عنصر `.text` الخاص بك.

```js
assert.exists(document.querySelector('.text blockquote'));
```

عنصر `blockquote` الخاص بك يجب أن يأتي بعد العناصر الثلاثة `p` الخاصة بك.

```js
assert(document.querySelector('.text')?.children?.[3]?.localName === 'blockquote');
```

يجب أن يحتوي عنصر `blockquote` الخاص بك على عنصرين `hr`.
...
يجب أن يكون العناصر الأطفال في `blockquote` بالترتيب الصحيح.
...
عنصر `p` الجديد الخاص بك يجب أن يحتوي على `class` يسمي `quote`.

```js
assert(document.querySelector('.text blockquote p')?.className === 'quote');
```

عنصر `p` الجديد الخاص بك يجب أن يحتوي على النص `The entire curriculum should be a series of projects`.
