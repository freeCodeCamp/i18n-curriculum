...
Il centro di controllo ha bisogno di un metodo per copiare gli astronauti idonei per EVA da un equipaggio. Crea una funzione vuota chiamata `getEVAReadyCrew` che accetta un parametro `crew`.

# --hints--

Dovresti creare una funzione chiamata `getEVAReadyCrew`.

```js
assert.isFunction(getEVAReadyCrew);
```

La tua funzione `getEVAReadyCrew` dovrebbe avere un parametro `crew`.
