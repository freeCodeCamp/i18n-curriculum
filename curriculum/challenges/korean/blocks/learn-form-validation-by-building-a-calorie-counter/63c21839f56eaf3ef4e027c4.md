...
`label`의 끝에 새 줄로 또 다른 `HTMLString` 요소를 만드세요. 이 `label`는 `Entry # Calories` 텍스트를 가져야 하며, 템플릿 리터럴 구문을 사용해 `#`를 `entryNumber` 값으로 바꾸고, `for` 속성은 `X-#-calories`로 설정하세요. 여기서 `X`은 `value`의 `entryDropdown`이고 `#`은 `entryNumber`의 값입니다.

# --hints--

`label` 안에 `HTMLString` 요소가 두 개 있어야 합니다.
...
새 `label` 요소는 `input` 요소 뒤에 와야 합니다.
...
새 `label` 요소는 `for` 속성이 `${entryDropdown.value}-${entryNumber}-calories`로 설정되어야 합니다.
...
새 `label` 요소는 `Entry ${entryNumber} Calories` 텍스트를 가져야 합니다.
