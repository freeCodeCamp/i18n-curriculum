...
既然你已經創建了 `resetButton`，你需要為它指定 `id` 和 `aria-label` 屬性。JavaScript 提供了你需要使用的 `id` 和 `ariaLabel` 屬性。

例如，`element.id` 會設定一個 `id` 屬性，而 `element.ariaLabel` 會設定一個 `aria-label` 屬性。它們兩者都接受字串作為其值。

將 `resetButton` 元素的 `id` 屬性設為 `"reset"`，並將其 `aria-label` 屬性設為 `"Reset playlist"`。
...
你應該使用 `resetButton.id` 為 `resetButton` 創建一個名為 `"reset"` 的 `id` 屬性。

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

你應該使用 `resetButton.ariaLabel` 來為 `resetButton` 創建一個名為 `"Reset playlist"` 的 `"aria-label"` 屬性。
