...
给定两个 *集合* A 和 *B*，计算 $(A \\setminus B) \\cup (B \\setminus A).$ 即枚举属于 *A* 或属于 *B* 但不同时属于两者的元素。 这个集称为 *A* 和 *B* 的对称差。 换句话说：$(A \\cup B) \\setminus (A \\cap B)$（至少在 *A* 或 *B* 中的项目，减去同时在 *A* 和 *B* 中的项目）。
