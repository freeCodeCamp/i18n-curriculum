...
Sin embargo, puedes usar el carácter `?` para cambiarlo a una coincidencia perezosa. `"titanic"` comparado con la expresión regular ajustada `/t[a-z]*?i/` devuelve `["ti"]`.
...
La variable `result` debe ser un arreglo que contenga `<h1>`
