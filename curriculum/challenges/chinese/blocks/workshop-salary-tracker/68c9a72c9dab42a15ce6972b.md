...
现在你将为 `_level` 属性创建一个 setter。创建一个名为 `level` 的方法，带有参数 `self` 和 `new_level`。使用 `@level.setter` 装饰该方法。

在该方法内，将 `self._level` 设置为 `new_level`。

# --hints--

你的 `Employee` 类应有一个带有参数 `self` 和 `new_level` 的 `level` 方法。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("level")[1].has_args("self, new_level")`)) })
```

你的 `level` 方法应该用 `@level.setter` 装饰。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("level")[1].has_decorators("level.setter")`)) })
```

你的 `level` 方法应将 `self._level` 设置为 `new_level`。
