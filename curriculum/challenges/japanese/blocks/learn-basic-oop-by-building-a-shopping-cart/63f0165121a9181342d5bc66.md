...
真偽値を返す式では、`undefined`を削除し、`textContent`の`currentProductCountSpan`を`currentProductCount`に続けて`x`に更新する必要があります。そのためにテンプレートリテラルを使ってください。

# --hints--

真偽値を返す式から`undefined`を削除してください。
...
`textContent`の`currentProductCountSpan`プロパティにアクセスしてください。
...
テンプレートリテラル構文を使って`textContent`を`${currentProductCount}x`に更新してください。
