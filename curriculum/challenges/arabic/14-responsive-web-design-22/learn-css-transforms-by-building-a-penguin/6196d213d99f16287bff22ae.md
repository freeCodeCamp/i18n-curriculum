...
يجب عليك إضافة `div` جديد إلى `body`. متوقع قيمة `--fcc-expected--` داخل عنصر `div`، لكن وجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('body > div')?.length, 5);
```

يجب عليك إعطاء عنصر `div` الـ `class` بقيمة `sun`.

```js
assert.include(document.querySelector('div:not(.back-mountain, .left-mountain, .penguin, .ground)')?.className, 'sun');
```

يجب عليك وضع عنصر `div` جديد بعد عنصر `.back-mountain`.
