...
Nachdem Sie den Button erstellt haben, müssen Sie ihm einen Text zuweisen. Dazu müssen Sie die `createTextNode()`-Methode des DOM verwenden.

Die `createTextNode()`-Methode wird verwendet, um einen Textknoten zu erstellen. Um sie zu verwenden, rufen Sie sie auf und übergeben den Text als String:

```js
document.createTextNode("your text")
```

Sie können ihn auch einer Variablen zuweisen:

```js
const myText = document.createTextNode("your text")
```

Verwenden Sie die `createTextNode()`-Methode, um einen `"Reset Playlist"`-Text zu erstellen, und weisen Sie ihn dann einer `resetText`-Konstanten zu.
...
Sie sollten `document.createTextNode()` verwenden, um einen `"Reset Playlist"`-Text zu erstellen.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

Sie sollten Ihren erstellten Textknoten einer `resetText`-Konstanten zuweisen.
