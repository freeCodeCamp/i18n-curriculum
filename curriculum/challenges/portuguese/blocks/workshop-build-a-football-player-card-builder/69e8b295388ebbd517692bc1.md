...
Agora que a interface `PlayerData` está definida, atualize o componente `PlayerCard` para aceitar uma prop `player` tipada como `PlayerData` para corresponder à interface `PlayerData`.

Lembre-se de que você pode desestruturar props assim:
...
Você deve atualizar `PlayerCard` para aceitar uma prop `player` tipada como `PlayerData`.
