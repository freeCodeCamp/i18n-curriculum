...
Aggiungi un listener per l'evento `"click"` all'elemento `previousButton`, poi passa `playPreviousSong` come secondo argomento.

# --hints--

Dovresti chiamare il metodo `addEventListener()` sulla tua variabile `previousButton`.
...
Dovresti passare `playPreviousSong` come secondo argomento del tuo metodo `addEventListener`.
