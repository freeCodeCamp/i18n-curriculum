...
Para fins de depuração, adicione uma chamada a `console.log()` no laço interno. Passe ali os argumentos de `array`, `array[j]` e `array[j+1]`.

# --hints--

Você deve usar uma chamada a `console.log()` no laço interno.
...
Você deve passar `array[j+1]` como o terceiro argumento para `console.log()`.
