...
`all` 関数は、渡されたイテラブルのすべての要素が真であれば `True` を返し、そうでなければ `False` を返します。
...
リスト `[isinstance(i, str) for i in medications]` を `all` 関数に渡して、そのすべての要素が文字列であることを確認してください。

# --hints--

`medications` キーの値を、`[isinstance(i, str) for i in medications]` を `all` 関数に渡すことで変更してください。
