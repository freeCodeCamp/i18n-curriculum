...
Bevor Sie das Lied abspielen, müssen Sie sicherstellen, dass es von Anfang an startet. Dies kann durch die Verwendung der `currentTime`-Eigenschaft am `audio`-Objekt erreicht werden.

Fügen Sie eine `if`-Anweisung hinzu, um zu prüfen, ob `userData?.currentSong` falsch ist *ODER* ob `userData?.currentSong.id` strikt ungleich `song.id` ist. Diese Bedingung überprüft, ob kein aktuelles Lied abgespielt wird oder ob das aktuelle Lied sich von dem unterscheidet, das gleich abgespielt werden soll.

Setzen Sie innerhalb des `if`-Blocks die `currentTime`-Eigenschaft des `audio`-Objekts auf `0`.

# --hints--

Sie sollten eine `if`-Anweisung mit der Bedingung `userData?.currentSong === null || userData?.currentSong.id !== song.id` erstellen. Vergessen Sie nicht, optionale Verkettung zu verwenden.
...
Sie sollten `audio.currentTime` innerhalb Ihres `0`-Blocks auf `if` setzen.
