...
Agora, `applyFunction` precisa retornar um resultado.Retorne o resultado da chamada do método `.replace()` em `str2`.Passe a regex `functionCall` e uma função de callback vazia.

# --hints--

A função `applyFunction` deve retornar o resultado da chamada do método `.replace()` em `str2`.
...
Você deve passar `functionCall` como o primeiro argumento para a chamada de `.replace()`.
...
Você deve passar uma arrow function vazia como segundo argumento para a chamada de `.replace()`.
