...
`self.items`에 `Movie('Dances with Wolves', 1990, 'Kevin Costner', 224)`과 `Movie('Annie Hall', 1977, 'Woody Allen', 93)`이 포함되어 있으면 `__str__` 메서드는 `Media Catalogue (2 items):\n\n=== MOVIES ===\n1. Dances with Wolves (1990) - 224 min, Kevin Costner\n2. Annie Hall (1977) - 93 min, Woody Allen\n`을 반환해야 합니다.
...
`__str__` 메서드는 `Media Catalogue (empty)`이 비어 있을 때 `self.items`을 반환해야 합니다.
