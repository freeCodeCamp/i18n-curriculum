...
选择 `class` 为 `left` 的 `.blush` 元素，将其放置在父元素左侧 `15%` 的位置。 然后，选择 `class` 为 `right` 的 `.blush` 元素，将其放置在父元素右侧 `15%` 的位置。

# --hints--

你应该有一个 `.blush.left` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.blush.left', '.left.blush']));
```

你应该将 `.blush.left` 的 `left` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.blush.left', '.left.blush'])?.left, '15%');
```

你应该有一个 `.blush.right` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.blush.right', '.right.blush']));
```

你应该将 `.blush.right` 的 `right` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
