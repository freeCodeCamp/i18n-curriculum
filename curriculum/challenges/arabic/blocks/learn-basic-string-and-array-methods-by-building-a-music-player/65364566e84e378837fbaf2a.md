...
في الخطوات السابقة، بنيت وظيفة تشغيل أغنية. الآن عليك إضافة الوظيفة إلى زر التشغيل ليشغل الأغنية الحالية عند النقر عليه.

استخدم الطريقة `addEventListener()` ومرر حدث `"click"` كالمعلمة الأولى ودالة استدعاء راجع فارغة بصيغة السهم كالمعلمة الثانية، مثل `() => {}`.

# --hints--

يجب استدعاء الطريقة `addEventListener()` على المتغير `playButton` الخاص بك.
...
استخدم صيغة السهم لتمرير دالة استدعاء راجع فارغة إلى مستمع الحدث. لا تنسَ أنك تحتاج أيضًا إلى زوج فارغ من الأقواس المتعرجة.
