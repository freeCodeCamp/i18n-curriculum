...
下一步是計算每個元素和平均數相差多少。 聲明一個 `differences` 變量，並給它賦值 `array.map()`。 在回調中，返回 `el` 減去 `mean` 的值。

# --hints--

你的 `getVariance` 函數應該聲明一個 `differences` 變量。

```js
assert.match(getVariance.toString(), /differences\s*=/);
```

你的 `differences` 變量應該使用 `array.map()` 方法。

```js
assert.match(getVariance.toString(), /differences\s*=\s*array\.map\(/);
```

你的 `differences` 變量應使用回調函數中的 `el` 參數。

```js
assert.match(getVariance.toString(), /differences\s*=\s*array\.map\(\s*function\s*\(\s*el\s*\)/);
```

你的 `map` 回調應該返回 `el` 減去 `mean` 的值。
