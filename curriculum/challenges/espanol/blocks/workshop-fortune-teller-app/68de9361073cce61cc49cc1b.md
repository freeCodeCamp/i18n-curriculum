...
En una clase anterior, aprendiste que la inferencia de tipos es la capacidad de TypeScript para determinar y asignar automáticamente tipos a variables, valores de retorno de funciones y expresiones según su uso y contexto.

En TypeScript, los genéricos hacen que las funciones, clases y tipos sean más flexibles. Los genéricos permiten la creación de variables de tipo, eliminando la necesidad de definir explícitamente los tipos cada vez. Esto permite que la misma función funcione de forma segura con diferentes tipos.

Aquí tienes un ejemplo simple de genéricos:
...
Las funciones pueden usar dos o más genéricos. `T` se usa por convención, y se pueden agregar variables de tipo adicionales como `S` según sea necesario.
...
Crea una función `getElement` que tome `selector` de tipo `string` como parámetro. También debe tomar un tipo genérico `T` que extienda `HTMLElement` y tener un tipo de retorno `T`.

# --hints--

Debes tener una función flecha llamada `getElement`.
...
Tu función `getElement` debe tomar un parámetro llamado `selector` de tipo `string`.
...
El parámetro de tipo genérico `T` debe extender el tipo `HTMLElement`.
...
Tu función `getElement` debe tener un tipo de retorno `T`.
