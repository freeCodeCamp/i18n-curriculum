...
Geben Sie in Ihrer `inner`-Funktion das Ergebnis des Aufrufs von `idToText` mit `character + num` als Argument zurück.

# --hints--

Ihre `inner`-Funktion sollte eine explizite Rückgabe verwenden.
...
Ihre `inner`-Funktion sollte das Ergebnis des Aufrufs Ihrer `idToText`-Funktion zurückgeben.
...
Sie sollten `character + num` als Argument an Ihre `idToText`-Funktion übergeben.
