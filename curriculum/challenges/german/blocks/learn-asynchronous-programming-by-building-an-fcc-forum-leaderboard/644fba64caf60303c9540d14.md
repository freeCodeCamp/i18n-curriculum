...
Nach dem `href`-Attribut setzen Sie das `class`-Attribut auf die Konstante mit dem Namen `linkClass`. 

Nach dem `class`-Attribut setzen Sie das `target`-Attribut auf `"_blank"`. 

Zum Schluss platzieren Sie die Konstante `linkText` zwischen den Anker-Tags, damit der Text im Link angezeigt wird.

# --hints--

Sie sollten ein `class`-Attribut nach dem `href`-Attribut haben.
...
Sie sollten den Wert des `class`-Attributs auf die Konstante mit dem Namen `linkClass` setzen.
...
Sie sollten ein `target`-Attribut nach dem `class`-Attribut haben.
...
Sie sollten den Wert des `target`-Attributs auf `"_blank"` setzen.
...
Sie sollten die Konstante `linkText` zwischen den Anker-Tags platzieren.
