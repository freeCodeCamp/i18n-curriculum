...
Para restablecer la lista de reproducción a su estado original, distribuya `allSongs` en un array y asígnelo a `userData.songs`.

*Nota*: No debe utilizar el encadenamiento opcional en `userData.songs` porque la canción no será `null` ni `undefined` en este punto.
...
Debe asignar `[...allSongs]` a `userData.songs`.
