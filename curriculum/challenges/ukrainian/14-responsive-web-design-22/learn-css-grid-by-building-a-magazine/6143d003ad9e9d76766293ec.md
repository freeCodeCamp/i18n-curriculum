...
Вкладіть елемент `hr`, елемент `p` та другий елемент `hr` в межах елемента `.image-quote`. Надайте `class` зі значенням `quote` та текст `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.` до елемента `p`.
