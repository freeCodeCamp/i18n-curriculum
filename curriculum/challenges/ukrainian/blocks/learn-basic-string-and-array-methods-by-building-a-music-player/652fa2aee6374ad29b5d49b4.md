...
Отримайте доступ до елементів `#next`, `#previous` і `#shuffle` з HTML-файлу.

Призначте їх відповідним змінним з іменами `nextButton`, `previousButton` і `shuffleButton`.

# --hints--

У вас має бути змінна з назвою `nextButton`.

```js
assert.isDefined(nextButton);
```

Використовуйте `document.getElementById()` для отримання елемента `#next`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

Призначте елемент `#next` змінній `nextButton`.

```js
assert.equal(nextButton, document.getElementById('next'));
```

У вас має бути змінна з назвою `previousButton`.

```js
assert.isDefined(previousButton);
```

Використовуйте `document.getElementById()` для отримання елемента `#previous`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

Призначте елемент `#previous` змінній `previousButton`.

```js
assert.equal(previousButton, document.getElementById('previous'));
```

У вас має бути змінна з назвою `shuffleButton`.

```js
assert.isDefined(shuffleButton);
```

Використовуйте `document.getElementById()` для отримання елемента `#shuffle`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

Призначте елемент `#shuffle` змінній `shuffleButton`.
