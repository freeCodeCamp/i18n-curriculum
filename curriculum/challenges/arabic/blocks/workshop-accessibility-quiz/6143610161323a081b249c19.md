...
املأ محتوى الاختبار القصير، أسفل `#student-info`، أضف ثلاثة عناصر `div` مع `class` باسم `info`.

ثم، داخل كل `div` قم بتسكين عنصر `label` واحد وعنصر `input` واحد.

# --hints--

يجب أن تسكن ثلاثة عناصر `div` أسفل العنصر `h2#student-info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.length, 3);
```

يجب أن تعطي أول `div` `class` باسم `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.className, 'info');
```

يجب أن تعطي ثاني `div` `class` باسم `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.className, 'info');
```

يجب أن تعطي ثالث `div` `class` باسم `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.className, 'info');
```

يجب أن تسكن عنصر `label` واحد داخل أول `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.querySelectorAll('label')?.length, 1);
```

يجب أن تسكن عنصر `input` واحد داخل أول `div`، بعد `label`.
...
يجب أن تسكن عنصر `label` واحد داخل ثاني `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.querySelectorAll('label')?.length, 1);
```

يجب أن تسكن عنصر `input` واحد داخل ثاني `div`، بعد `label`.
...
يجب أن تسكن عنصر `label` واحد داخل ثالث `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.querySelectorAll('label')?.length, 1);
```

يجب أن تسكن عنصر `input` واحد داخل ثالث `div`، بعد `label`.
