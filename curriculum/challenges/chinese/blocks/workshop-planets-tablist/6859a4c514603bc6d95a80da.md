...
现在是将标签（页）面板与其各自标签（页）关联的时候了。 

为第一个 `tabpanel` 元素添加 `id` 属性，值为 `"panel-earth"`，并添加 `aria-labelledby` 属性，值为 `"tab-earth"`。

# --hints--

你的第一个 `tabpanel` 元素应具有 `id` 属性。

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("id"));
```

你的 `id` 属性应具有值 `"panel-earth"`。

```js
assert.equal(document.querySelector('[role="tabpanel"]').getAttribute("id"), "panel-earth");
```

你的第一个 `tabpanel` 元素应具有 `aria-labelledby` 属性。

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("aria-labelledby"));
```

你的 `aria-labelledby` 属性应具有值 `"tab-earth"`。
