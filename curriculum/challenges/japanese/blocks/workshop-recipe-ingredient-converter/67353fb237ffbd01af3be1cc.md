...
ついにロジックをユーザーインターフェースに連携させ始めることができます。IDでいくつかの要素を取得するためにDOMをクエリする必要があります。`ingredient`、`quantity`、`unit`、`servings`、`recipe-form`、`result-list`のIDを持つ要素を取得し、それぞれ`ingredientName`、`ingredientQuantity`、`unitToConvert`、`numberOfServings`、`recipeForm`、`resultList`の変数に代入してください。

# --hints--

`ingredientName`という変数を定義してください。

```js
assert.isDefined(ingredientName);
```

`ingredientName`変数は、`.getElementById()`の`ingredient` IDで返される要素にしてください。

```js
assert.strictEqual(ingredientName, document.getElementById('ingredient'));
```

`ingredientQuantity`という変数を定義してください。

```js
assert.isDefined(ingredientQuantity);
```

`ingredientQuantity`変数は、`.getElementById()`の`quantity` IDで返される要素にしてください。

```js
assert.strictEqual(ingredientQuantity, document.getElementById('quantity'));
```

`unitToConvert`という変数を定義してください。

```js
assert.isDefined(unitToConvert);
```

`unitToConvert`変数は、`.getElementById()`の`unit` IDで返される要素にしてください。

```js
assert.strictEqual(unitToConvert, document.getElementById('unit'));
```

`numberOfServings`という変数を定義してください。

```js
assert.isDefined(numberOfServings);
```

`numberOfServings`変数は、`.getElementById()`の`servings` IDで返される要素にしてください。

```js
assert.strictEqual(numberOfServings, document.getElementById('servings'));
```

`recipeForm`という変数を定義してください。

```js
assert.isDefined(recipeForm);
```

`recipeForm`変数は、`.getElementById()`の`recipe-form` IDで返される要素にしてください。

```js
assert.strictEqual(recipeForm, document.getElementById("recipe-form"));
```

`resultList`という変数を定義してください。

```js
assert.isDefined(resultList);
```

`resultList`変数は、`.getElementById()`の`result-list` IDで返される要素にしてください。
