...
Dai al selettore `.text` una proprietà `font-size` impostata su `1.8rem` e una proprietà `letter-spacing` impostata su `0.6px`.

# --hints--

Il tuo selettore `.text` dovrebbe avere una proprietà `font-size` impostata su `1.8rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize, '1.8rem');
```

Il tuo selettore `.text` dovrebbe avere una proprietà `letter-spacing` impostata su `0.6px`.
