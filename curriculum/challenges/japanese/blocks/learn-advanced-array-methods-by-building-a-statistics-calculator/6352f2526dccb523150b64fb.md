...
`calculate` 関数に範囲を計算して表示するロジックを追加してください。

# --hints--

`calculate` 関数は `range` の結果を `getRange(numbers)` 変数に設定する必要があります。

```js
assert.match(calculate.toString(), /range\s*=\s*getRange\(\s*numbers\s*\)\s*;?/);
```

`calculate` 関数は `#range` 変数を `textContent` 要素の `range` に設定する必要があります。
