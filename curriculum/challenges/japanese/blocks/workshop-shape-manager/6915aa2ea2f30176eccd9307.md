...
`oninput`の`shapeTypeSelect`イベントを使い、それを`handleShapeSelect`に設定してください。これにより、形状の選択が変わるたびに関数が実行されます。

これを行った後、各形状の入力フィールドが見えるはずです。しかし、計算はまだ動作しないので、次にそれを処理します。

# --hints--

`shapeTypeSelect.oninput`を`handleShapeSelect`に設定してください。
