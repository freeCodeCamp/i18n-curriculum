...
Le transizioni a gradiente spesso cambiano gradualmente da un colore a un altro. Quando è necessaria una variazione più netta, la transizione può essere fatta con un'interruzione brusca come questa:
...
Aggiungi un `linear-gradient` a `.bb2b` che usa `--building-color2` da `0%` a `6%` e `--window-color2` da `6%` a `9%`.

# --hints--

Dovresti dare a `.bb2b` una proprietà `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

Dovresti usare un `linear-gradient` sul `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

Dovresti usare `--building-color2` da `0%` a `6%`.
...
Dovresti usare `--window-color2` da `6%` a `9%`.
...
`.bb2b` dovrebbe avere una `linear-gradient` che transita da `--building-color2` a `0%` a `6%`, e `--window-color2` a `6%` a `9%`.
