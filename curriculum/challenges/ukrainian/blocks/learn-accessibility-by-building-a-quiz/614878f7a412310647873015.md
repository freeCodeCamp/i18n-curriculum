...
Приберіть безлад у `header`, використовуючи _гнучкий контейнер_ для розміщення пробілів між дочірніми елементами та їх вертикального центрування.

# --hints--

Ви маєте надати `header` `justify-content` зі значенням `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

Ви маєте надати `header` `align-items` зі значенням `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

Ви маєте надати `header` `top` зі значенням `0`.
