...
在最後一個 `.divider` 之後，使用文本 `Cholesterol 0mg 0%` 創建一個新的 `p` 元素。 然後，將文本 `Cholesterol` 放在一個 `span` 元素中，將 `0%` 放在另一個元素中，併爲每個元素設置 `bold` 類。

最後，將包含文本 `Cholesterol` 和文本 `0mg` 的 `span` 元素嵌套到另一個 `span` 元素中，以便對齊。
...
你的 `Cholesterol` `span` 應該有 `class` 屬性設置爲 `bold`。
...
你的 `0%` `span` 應該有 `class` 屬性設置爲 `bold`。
...
你的 `Cholesterol` `span` 和 `0mg` 文本應該被包裹在 `span` 中。
