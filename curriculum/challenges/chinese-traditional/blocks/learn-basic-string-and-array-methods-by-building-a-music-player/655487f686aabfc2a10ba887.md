...
在你開始實作播放下一首和上一首歌曲之前，你需要取得 `userData` 的 `songs` 屬性中每首歌曲的索引。 

首先建立一個名為 `getCurrentSongIndex` 的箭頭函式（程式）。

# --hints--

你應該有一個 `getCurrentSongIndex` 函式（程式）。

```js
assert.isFunction(getCurrentSongIndex);
```

你的 `getCurrentSongIndex` 函式應該使用箭頭語法。
