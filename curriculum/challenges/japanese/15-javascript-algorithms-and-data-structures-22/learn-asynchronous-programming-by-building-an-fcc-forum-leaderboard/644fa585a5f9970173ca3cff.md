...
const の変数 `linkClass` を作成し、テンプレートリテラルを割り当ててください。 そのテンプレートリテラルには `category ${selectedCategory.className}` という値を入れてください。

このクラス名は、のちほどアンカー要素にスタイルを適用するために使います。

# --hints--

`linkClass` という定数が必要です。

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

変数 `linkClass` にテンプレートリテラルを割り当ててください。

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

テンプレートリテラル内に `category ${selectedCategory.className}` の値を含める必要があります。
