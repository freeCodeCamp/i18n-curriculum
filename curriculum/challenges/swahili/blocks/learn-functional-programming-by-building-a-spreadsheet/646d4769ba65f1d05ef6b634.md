...
Sasa kitendakazi chako cha `update()` kinaweza kutathmini usemi wa kihesabu. Kumbuka kwamba uliandika sharti la `if` ili kuangalia kwamba kitendakazi kilikuwa kimeitwa.

Ndani ya tamko la `if`, weka thamani ya sifa ya `value` ya `element` kuwa matokeo ya kitendakazi chako cha `evalFormula()`. Usipitishe hoja yoyote bado.

# --hints--

Unapaswa kusahihisha sifa ya `value` ya `element` ndani ya kipande chako cha `if`.
...
Unapaswa kuweka thamani ya sifa ya `value` kuwa matokeo ya wito wa kitendakazi chako cha `evalFormula()`.
...
Hupaswi kupitisha hoja yoyote kwa wito lako la `evalFormula()`.
