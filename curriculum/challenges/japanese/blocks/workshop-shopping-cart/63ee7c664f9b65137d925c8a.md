...
あなたの `ShoppingCart` クラスにはアイテムを追加する機能が必要です。2つのパラメータ、`addItem` と `id` を受け取る空の `products` メソッドを作成してください。メソッドの作成は次のようになります。
...
最初のパラメータ `id` は、ユーザーがカートに追加した商品の `id` です。2つ目のパラメータ `products` は `Dessert` インスタンスの配列です。既存の `products` 配列を直接参照する代わりにパラメータを使うことで、将来的に追加の製品リストを追加したい場合でも、このメソッドはより柔軟になります。

# --hints--

あなたの `ShoppingCart` クラスには `addItem` メソッドが必要です。
...
あなたの `addItem` メソッドは2つのパラメータ、`id` と `products` を受け取る必要があります。
