...
將左眼移動到位置，使用 `position` 屬性為 `absolute`，`top` 為 54px，`left` 為 39px。

# --hints--

你的 `.cat-left-eye` 選擇器應該有一個 `position` 屬性設定為 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.position === 'absolute')
```

你的 `.cat-left-eye` 選擇器應該有一個 `top` 屬性設定為 `54px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.top === '54px')
```

你的 `.cat-left-eye` 選擇器應該有一個設定為 `39px` 的 `left` 屬性。
