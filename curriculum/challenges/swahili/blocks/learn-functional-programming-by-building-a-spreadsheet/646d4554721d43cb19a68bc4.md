...
Kigezo cha `fn` kitapokelewa jina la kitendakazi, kama `"SUM"`. Sasisha `apply` ili kurudisha moja kwa moja kitendakazi kutoka kwenye kitu cha `spreadsheetFunctions` kwa kutumia tofauti ya `fn` kama ufunguo wa upatikanaji wa kitu hicho.

Kumbuka kuwa `fn` huenda isijumuishe mfuatano wa herufi ndogo, kwa hivyo utahitaji kuibadilisha kuwa mfuatano wa herufi ndogo.

# --hints--

Kitendakazi chako cha `apply` kinapaswa kutumia kurudisha moja kwa moja.
...
Kitendakazi chako cha `apply` kinapaswa kupata upatikanaji wa kitu cha `spreadsheetFunctions`.
...
Kitendakazi chako cha `apply` kinapaswa kupata sifa ya kitu cha `spreadsheetFunctions` inayolingana na thamani ya `fn`.
...
Kitendakazi chako cha `apply` kinapaswa kuita njia ya `.toLowerCase()` kwenye `fn` katika upatikanaji wa sifa.
