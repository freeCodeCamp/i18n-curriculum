...
分数 $\frac{n}{d}$ を考えます。ここで `n` と `d` は正の整数です。もし `n` &lt; `d` かつ最大公約数 ${HCF}(n, d) = 1$ であれば、それは既約真分数と呼ばれます。

`d` ≤ 8 の既約真分数の集合を大きさの昇順に並べると、次のようになります：
...
$\frac{1}{3}$ と $\frac{1}{2}$ の間には `3` 個の分数があることがわかります。

`d` ≤ `limit` の既約真分数の昇順に並べた集合で、$\frac{1}{3}$ と $\frac{1}{2}$ の間にいくつの分数があるでしょうか？
