...
사용자가 한 숫자를 세 번, 다른 숫자를 두 번 굴리면 이를 풀 하우스라고 합니다. 단일 인수를 받는 `detectFullHouse` 함수를 선언하세요. 이 함수는 호출될 때 `diceValuesArr` 배열을 전달받습니다.

`detectFullHouse` 함수는 사용자가 한 숫자를 세 번, 다른 숫자를 두 번 굴렸는지 확인해야 합니다. 그렇다면 세 번째 라디오 버튼을 올바른 속성으로 점수 `25`를 표시하도록 업데이트해야 합니다. 그렇지 않더라도 마지막 라디오 버튼을 올바른 속성으로 점수 0을 표시하도록 항상 업데이트해야 합니다.

주사위를 굴릴 때 새 함수를 호출하는 것을 잊지 마세요.

# --hints--

`detectFullHouse` 함수를 선언해야 합니다.

```js
assert.isFunction(detectFullHouse);
```

`detectFullHouse` 함수는 매개변수를 가져야 합니다.

```js
assert.match(code, /(?:var|let|const)\s+detectFullHouse\s*=\s*(\(\s*\w+\s*\)|\s*\w+\s*)\s*=>\s*|function\s+detectFullHouse\s*\(\s*\w+\s*\)/);
```

풀 하우스를 굴렸을 때 `detectFullHouse` 함수는 세 번째 라디오 버튼을 활성화하고, 값을 `25`로 설정하며, 세 번째 `span`에 텍스트 `, score = 25`를 표시해야 합니다.
...
풀 하우스를 굴리지 않았을 때 `detectFullHouse` 함수는 마지막 라디오 버튼을 활성화하고, 값을 `0`로 설정하며, 마지막 `span`에 텍스트 `, score = 0`를 표시해야 합니다.
...
`detectFullHouse` 함수를 `rollDiceBtn`가 클릭될 때 호출해야 합니다.
