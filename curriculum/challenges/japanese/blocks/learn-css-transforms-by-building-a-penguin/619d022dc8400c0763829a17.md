...
`.face` クラスを持つ `left` 要素をターゲットにして、その親要素の左側に `5%` で配置してください。

# --hints--

`.face.left` セレクターを使うべきです。

```js
assert.match(code, /\.face\.left\s*\{/);
```

`.face.left` に `left` プロパティを与えるべきです。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face.left')?.left);
```

`.face.left` に `left` を `--fcc-expected--` に設定するべきですが、`--fcc-actual--` が見つかりました。
