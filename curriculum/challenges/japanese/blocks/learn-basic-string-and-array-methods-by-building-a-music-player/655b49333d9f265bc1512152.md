...
HTMLのアルバムアートと`userData.songs`配列の曲が変わっていることに気づいてください。元の曲を短い曲に入れ替えましたので、これからのステップでアプリのテストに使えます。

次に、再生する次の曲があるかどうかを確認する必要があります。`getCurrentSongIndex()`関数を呼び出して現在の曲のインデックスを取得し、`currentSongIndex`定数に保存してください。

その後、次の曲が存在するかどうかに応じて、ブール値`nextSongExists`または`true`を含む`false`定数を作成してください。

# --hints--

既存の`ended`イベントリスナーとその内容は変更しないでください。

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

`currentSongIndex`定数を作成し、`getCurrentSongIndex`関数の呼び出しに設定してください。
...
`userData.songs.length`と`currentSongIndex`を比較して次の曲が存在するか確認し、`nextSongExists`定数に設定してください。曲の配列（`userData.songs.length - 1`）の最後のインデックスが現在の曲のインデックスより大きければ、次の曲があることを意味します。
