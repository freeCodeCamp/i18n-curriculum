...
بعد جلب البيانات، خزّن الاستجابة في متغير وحلّلها إلى JSON. أعطِ البيانات المحللة النوع `Deck`، ثم عيّنها إلى الخاصية `cards`. داخل كتلة `catch`، تأكد من تسجيل الخطأ مع هذه الرسالة: `"Error fetching cards:", error`.

# --hints--

يجب إعلان متغير باسم `data` داخل `fetchCardsData`.
...
يجب أن يكون للمتغير `data` النوع `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

يجب تعيين المتغير `data` عن طريق تحليل الاستجابة كـ JSON.
...
يجب تخزين `data.cards` في `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

يجب أن تستخدم كتلة `catch` دالة `console.error()` لتسجيل الخطأ مع الرسالة `"Error fetching cards:", error`.
