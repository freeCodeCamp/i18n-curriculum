...
Метод `map()` використовується для ітерування над масивом та повертає новий масив. Це корисно, коли ви хочете створити новий масив на основі значень теперішнього масиву. Наприклад:
...
Зверніть увагу, що метод `map()` приймає функцію як аргумент. Це називається <dfn>функцією зворотного виклику</dfn>, тобто функцією, яка передається іншій функції як аргумент. У наведеному вище прикладі функція зворотного виклику — `(number) => number * 2`, і вона виконується на кожному елементі в масиві `numbers`. Потім метод `map()` повертає новий масив з результатами.

Передайте функцію зворотного виклику методу `map()`. Функція зворотного виклику має приймати `song` як параметр, використовувати синтаксис стрілкової функції та мати порожнє тіло.

# --hints--

Функція зворотного виклику методу `map()` має використовувати синтаксис стрілкової функції.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

Метод `map()` повинен мати параметр `song` у функції зворотного виклику.
