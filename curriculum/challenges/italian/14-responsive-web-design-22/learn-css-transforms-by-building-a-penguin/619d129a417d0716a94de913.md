...
Dovresti dare a `.beak.bottom` una proprietà `width` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.width, '16%');
```

Dovresti dare a `.beak.bottom` una proprietà `top` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.top, '65%');
```

Dovresti dare a `.beak.bottom` una proprietà `left` con il valore `--fcc-expected--` invece di `--fcc-actual--`.
