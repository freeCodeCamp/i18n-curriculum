...
次は表示された曲を再生するための機能を実装します。

`const` を使って `playSong` 関数を定義してください。 この関数は、再生したい曲の一意の識別子を表す `id` を引数に取るようにしてください。

# --hints--

`const` を使用して、空の関数 `playSong` を宣言してください。

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

`playSong` は関数でなければなりません。

```js
assert.isFunction(playSong)
```

`playSong` 関数にはアロー構文を使用してください。

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

`playSong` 関数に仮引数 `id` を持たせてください。

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

`playSong` 関数は空にしてください。
