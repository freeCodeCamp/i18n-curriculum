...
املأ في `.bb3` بعملية `repeating-linear-gradient`. استخدم `90deg` للاتجاه، وأستخدم متغيرات `building-color3` كلونين الأولين، و `window-color3` في `15%` كالون الثالث. عندما لا تحدد مسافة للون، سوف يُستخدم القيم المنطقية. في هذه الحالة، سيكون أول لونين افتراضيين إلى `0%` و `7.5%` لأنها تبدأ عند `0%` و`7.5%` هي نصف `15%`.

# --hints--

داخل `.bb3` غيّر خاصية `background` باستخدام `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb3")?.background, "repeating-linear-gradient");
```

يجب عليك استخدام `90deg` للاتجاه في الحِجَّة الأولى من `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb3")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

يجب عليك استخدام `--building-color3` للونين الأولين.
...
يجب عليك استخدام `--window-color3` عند `15%` للون الثالث.
