...
Tengeneza hoja ya utafutaji ya tatu ya `@media` kwa ajili ya `only screen` yenye `max-width` ya `550px`. Ndani yake, tengeneza kichaguzi cha `.hero-title` chenye `font-size` iliyowekwa kuwa `6rem`, kichaguzi cha `.hero-subtitle, .author, .quote, .list-title` chenye `font-size` iliyowekwa kuwa `1.8rem`, kichaguzi cha `.social-icons` chenye `font-size` iliyowekwa kuwa `2rem`, na kichaguzi cha `.text` chenye `font-size` iliyowekwa kuwa `1.6rem`.

# --hints--

Unapaswa kuwa na hoja mpya ya utafutaji ya `@media` kwa ajili ya `only screen` yenye `max-width` ya `550px`. Hii inapaswa kuja baada ya mbili zako zilizotangulia.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

Kanuni yako mpya ya `@media` inapaswa kuwa na kichaguzi cha `.hero-title`, kichaguzi cha `.hero-subtitle, .author, .quote, .list-title`, kichaguzi cha `.social-icons`, na kichaguzi cha `.text`. Vichaguzi hivi vinapaswa kuwa kwa mpangilio huu.
...
Kichaguzi chako cha `.hero-title` kinapaswa kuwa na `font-size` iliyowekwa kuwa `6rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[0]?.style?.fontSize === '6rem');
```

Kichaguzi chako cha `.hero-subtitle, .author, .quote, .list-title` kinapaswa kuwa na `font-size` iliyowekwa kuwa `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[1]?.style?.fontSize === '1.8rem');
```

Kichaguzi chako cha `.social-icons` kinapaswa kuwa na `font-size` iliyowekwa kuwa `2rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize === '2rem');
```

Kichaguzi chako cha `.text` kinapaswa kuwa na `font-size` iliyowekwa kuwa `1.6rem`.
