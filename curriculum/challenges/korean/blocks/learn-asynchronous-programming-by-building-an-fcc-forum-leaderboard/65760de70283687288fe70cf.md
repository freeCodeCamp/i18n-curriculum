...
숫자 `417`에 대한 새 키를 빈 객체 값으로 추가하세요.

그 객체 안에 `category` 키와 `"freeCodeCamp Support"` 문자열 값을 가진 속성을 추가하세요.

그 속성 아래에 `className`라는 또 다른 키를 `"support"` 문자열 값으로 추가하세요.

# --hints--

`allCategories` 객체에 `417` 속성이 있어야 합니다.
