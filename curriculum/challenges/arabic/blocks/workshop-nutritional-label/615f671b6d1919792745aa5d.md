...
أنشئ عنصر `div` جديدًا أسفل عنصر `.medium` الخاص بك. امنحه خاصية `class` بقيمة `daily-value small-text`. داخل هذا العنصر الجديد `div`، أضف عنصر `p` يحتوي على النص `% Daily Value *`، واضبط خاصية `class` له على `bold right`.

# --hints--

يجب أن تنشئ عنصر `div` جديدًا بعد عنصر `.medium` الخاص بك.
...
يجب أن يحتوي عنصر `div` الجديد على خاصية `class` بقيمة `daily-value small-text`.
...
يجب أن يحتوي عنصر `p` الجديد على النص `% Daily Value *`.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.textContent === '% Daily Value *');
```

يجب أن يحتوي عنصر `p` الجديد على خاصية `class` بقيمة `bold right`.
