...
`.blush` 要素をターゲットにして、`width` を `15%`、`height` を `10%`、`background-color` を `pink` にしてください。
...
`.blush` に `width` を `--fcc-expected--` に設定してください。場所は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.width, '15%');
```

`.blush` に `height` を `--fcc-expected--` に設定してください。場所は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.height, '10%');
```

`.blush` に `background-color` を `--fcc-expected--` に設定してください。場所は `--fcc-actual--` です。
