...
Das `alt`-Bildattribut sollte den Bildinhalt beschreiben. Screenreader geben den alternativen Text anstelle von Bildern aus. Wenn das Bild nicht geladen werden kann, wird dieser Text anstelle des Bildes angezeigt.

Um das Projekt abzuschließen, fügen Sie allen neun Ihrer Katzenbilder ein `alt`-Attribut hinzu, das sie beschreibt. Verwenden Sie für jede Beschreibung einen Wert mit mindestens fünf Zeichen.


# --hints--

Alle neun Ihrer `img`-Elemente sollten ein `alt`-Attribut mit einem Text haben, der jedes Bild beschreibt, wobei jede Beschreibung mindestens `5` Buchstaben enthält.
