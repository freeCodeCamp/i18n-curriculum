...
`card` div 안에 `div`의 `className`를 가진 `card-header`을 추가하세요. 이 섹션에는 선수 평점, 포지션, 티어 배지, 클럽 이름이 들어갑니다.

# --hints--

`div`의 `className`를 가진 `card-header` 안에 `div`의 `className`를 가진 `.card`를 추가해야 합니다.
