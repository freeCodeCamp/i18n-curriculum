...
`multipleCardsBtn` に `newReadingBtn` を呼び出すクリックイベントリスナーを、`multipleCardsBtn` に `multipleCardSelected` を呼び出す別のクリックイベントリスナーをそれぞれ追加して、`newReadingBtn` と `newReading` に対して同じことをしてください。

これらの2つのメソッドは今後のステップで作成します。

# --hints--

`click` 要素に `multipleCardsBtn` イベントを追加してください。

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

`multipleCardsBtn` のコールバックイベント内で `multipleCardSelected` メソッドを呼び出してください。

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

`click` 要素に `newReadingBtn` イベントを追加してください。

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

`newReadingBtn` のコールバックイベント内で `newReading` メソッドを呼び出してください。
