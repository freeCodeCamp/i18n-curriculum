...
Neste workshop, você criará uma página de revista usando um comunicado de imprensa de uma versão anterior do currículo do freeCodeCamp.

Comece adicionando um elemento `link` para a folha de estilos da fonte `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap`, um `link` para a folha de estilos FontAwesome `https://use.fontawesome.com/releases/v5.8.2/css/all.css` e um `link` para sua folha de estilos `./styles.css`.
