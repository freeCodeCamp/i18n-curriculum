...
现在你的客户将拥有所有必要的信息，以正确显示当前用户数并在用户连接或断开时进行公告！要在客户侧处理此事件，我们应监听 `'user'`，然后使用 jQuery 通过更改 `#num-users` 的文本为 `'{NUMBER} users online'` 来更新当前用户数，同时向 id 为 `messages` 的无序列表追加一个 `<li>`，内容为 `'{NAME} has {joined/left} the chat.'`。
...
当你认为页面已经正确时提交。如果遇到误差，你可以查看 <a href="https://forum.freecodecamp.org/t/advanced-node-and-express/567135/3#announce-new-users-10" target="_blank" rel="noopener noreferrer nofollow">到目前为止完成的项目</a>。

# --hints--

`'user'` 事件应该和 `username` 、 `currentUsers` 、 `connected` 一起发送。
