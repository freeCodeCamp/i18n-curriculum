---
id: 5900f4de1000cf542c50fff1
title: 'Problem 370: Geometrische Dreiecke'
challengeType: 1
forumTopicId: 302032
dashedName: problem-370-geometric-triangles
---

# --description--

Definieren wir ein geometrisches Dreieck als ein ganzzahliges Dreieck mit den Seitenlängen $a ≤ b ≤ c$, sodass seine Seiten eine geometrische Folge bilden, also $b^2 = a \times c$ gilt.

Ein Beispiel für ein solches geometrisches Dreieck ist das Dreieck mit den Seiten $a = 144$, $b = 156$ und $c = 169$.

Es gibt $861\\,805$ geometrische Dreiecke mit einem $\text{Umfang} ≤ {10}^6$.

Wie viele geometrische Dreiecke existieren mit einem $\text{Umfang} ≤ 2,5 \times {10}^{13}$?

# --hints--

`geometricTriangles()` sollte `41791929448408` zurückgeben.

```js
assert.strictEqual(geometricTriangles(), 41791929448408);
```

# --seed--

## --seed-contents--

```js
function geometricTriangles() {

  return true;
}

geometricTriangles();
```

# --solutions--

```js
// solution required
```
