...
Dove rilevante, sostituisci i valori delle proprietà con la tua variabile `--penguin-face`.

# --hints--

Dovresti dare a `.face` una `background-color` di `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Dovresti dare a `.chin` una `background-color` di `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Non dovresti usare `var(--penguin-face)` nel selettore `.penguin-body`.
