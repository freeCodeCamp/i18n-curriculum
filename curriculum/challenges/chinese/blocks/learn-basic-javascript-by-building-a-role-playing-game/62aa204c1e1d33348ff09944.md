...
在你的`attack`函数中，找到更新 `monsterHealth` 变量的代码行，并将其放置在调用 `isMonsterHit` 函数作为条件的 `if` 代码块中。
...
你的 `if` 语句应该在条件中调用 `isMonsterHit`。

```js
assert.match(attack.toString(), /if\s*\(\s*isMonsterHit\s*\(\s*\)/)
```

你应该将 `monsterHealth -= weapons[currentWeaponIndex].power + Math.floor(Math.random() * xp) + 1` 放入 `if` 块中。
