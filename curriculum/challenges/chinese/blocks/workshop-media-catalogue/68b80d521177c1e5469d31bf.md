...
现在 `__init__` 方法已完成，在 `try` 块内创建一个名为 `series1` 的变量。为它赋值一个传入你选择的参数的 `TVSeries` 实例。然后，将 `series1` 打印到控制台。

# --hints--

你应该在 `try` 块内将一个 `TVSeries` 实例赋值给名为 `series1` 的变量。
