...
これまで学んだアロー関数についての知識を、音楽プレーヤープロジェクトに適用する時が来ました。

まずは `printGreeting`、`printMessage`、`addTwoNumbers` の関数を削除してください。また、すべてのコンソールの文と関数の呼び出しも削除してください。

# --hints--

`printGreeting` の関数は存在してはいけません。

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

`printMessage` の関数は存在してはいけません。

```js
assert.notMatch(code, /const printMessage = org => {/);
```

`addTwoNumbers` の関数は存在してはいけません。

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

`printGreeting();` の関数呼び出しは存在してはいけません。

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

`printMessage('freeCodeCamp');` の関数呼び出しは存在してはいけません。

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

`console.log(addTwoNumbers(3,4));` のコンソール文は存在してはいけません。
