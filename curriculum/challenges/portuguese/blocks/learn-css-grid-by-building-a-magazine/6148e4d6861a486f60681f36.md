...
As imagens devem estar dentro de um layout de duas colunas e três linhas.
...
O seletor `.image-wrapper` deve ter a propriedade `grid-template-columns` definida como `2fr 1fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns === '2fr 1fr');
```

O seletor `.image-wrapper` deve ter a propriedade `grid-template-rows` definida como `repeat(3, min-content)`.
