...
Ahora, vas a usar animaciones CSS para hacer que el pingüino salude.

Define un nuevo `@keyframes` llamado `wave`.

# --hints--

Debes definir una nueva regla `@keyframes`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getCSSRules('keyframes'));
```

Debes darle al nombre de la regla `@keyframes` el valor de `--fcc-expected--`, pero se encontró `--fcc-actual--`.
