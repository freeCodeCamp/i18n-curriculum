...
Ndani ya kitendakazi cha `playSong`, weka sifa ya `audio.src` sawa na `song.src`. Hii inaeleza kipengee cha sauti mahali pa kupata data ya sauti kwa wimbo ulioteuliwa.

Pia, weka sifa ya `audio.title` sawa na `song.title`. Hii inaeleza kipengee cha sauti nini kiwe kinavyoonyeshwa kama kichwa cha wimbo.

# --hints--

Haupaswi kubadilisha kitendakazi cha `playSong` kilichopo na yaliyomo ndani yake.
...
Unapaswa kuweka `audio.src` kuwa `song.src`.
...
Unapaswa kuweka `audio.title` kuwa `song.title`.
