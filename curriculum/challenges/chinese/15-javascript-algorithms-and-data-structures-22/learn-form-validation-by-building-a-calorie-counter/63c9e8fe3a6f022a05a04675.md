...
使用相同的插值语法，添加第二个 `p` 元素，其文本为 `consumedCalories Calories Consumed`，并添加第三个元素，其文本为 `exerciseCalories Calories Burned`。 请记住用适当的值替换 `consumedCalories` 和 `exerciseCalories` 变量。
...
你的第二个 `p` 元素应包含文本 `${consumedCalories} Calories Consumed`。
