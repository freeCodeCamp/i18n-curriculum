...
- `overflow`: el número de asistentes que quedaron en la fila después del procesamiento (que es `currentTickQueue` después del bucle).

Debes devolver directamente el objeto así:
...
Debes devolver un objeto con las propiedades `processed` y `overflow`. Devuelve directamente el objeto sin asignarlo a una variable.
...
La propiedad `processed` del objeto que devuelvas debe tener el valor `processed`.
...
La propiedad `overflow` del objeto que devuelvas debe tener el valor `currentTickQueue`.
