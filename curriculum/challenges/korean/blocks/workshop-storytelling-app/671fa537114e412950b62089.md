...
JavaScript 파일에서 `.story-container` 요소를 선택해 `storyContainer`라는 변수에 저장하세요.

# --hints--

`storyContainer` 변수가 있어야 합니다.

```js
assert.isDefined(storyContainer);
```

`.story-container` 메서드를 사용해 `querySelector` 요소를 선택하고 `storyContainer` 변수에 저장해야 합니다.
