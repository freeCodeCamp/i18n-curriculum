...
在之前的课程中，你已经了解到类型推导是 TypeScript 根据变量、函数返回值和表达式的使用情况与上下文，自动确定并赋予类型的能力。

在 TypeScript 中，泛型使函数、类和类型更加灵活。泛型允许创建类型变量，避免每次都显式定义类型。这使得同一个函数能够安全地处理不同的类型。
...
函数可以使用两个或更多泛型。`T` 是惯例使用的类型变量，必要时可以添加额外的类型变量，比如 `S`。
...
创建一个名为 `getElement` 的函数，它接受类型为 `string` 的 `selector` 作为参数。它还应接受一个继承自 `HTMLElement` 的泛型类型 `T`，并且返回类型为 `T`。

# --hints--

你应该有一个名为 `getElement` 的箭头函数。
...
你的 `getElement` 函数应该接受一个名为 `selector`、类型为 `string` 的参数。
