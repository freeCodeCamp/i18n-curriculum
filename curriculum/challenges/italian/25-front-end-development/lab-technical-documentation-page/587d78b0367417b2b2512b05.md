...
1. Dentro l'elemento `#main-doc`, puoi aggiungere svariati elementi `section`, ognuno con una classe di valore `main-section`. There should be a minimum of five.
...
1. Ogni elemento `section` con la classe `main-section` dovrebbe avere un `id` che corrisponde al testo di ogni elemento `header` al suo interno. Any spaces should be replaced with underscores (e.g. The section that contains the header "JavaScript and Java" should have a corresponding `id="JavaScript_and_Java"`).
...
1. In aggiunta, l'elemento navbar dovrebbe contenere dei link (elementi `a`) con la classe `nav-link`. There should be one for every element with the class `main-section`.
