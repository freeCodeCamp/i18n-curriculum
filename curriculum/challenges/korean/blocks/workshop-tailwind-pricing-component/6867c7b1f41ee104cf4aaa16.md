...
이제 `grid`를 사용해 래퍼 `div` 안의 항목들을 정렬하세요.

래퍼 `grid`에 클래스 `grid-rows-[1fr_auto]`, `gap-6`, 그리고 `div`를 추가하세요.

# --hints--

가족 요금제용 래퍼 `div` 요소에는 클래스 `grid`가 있어야 합니다.
...
가족 요금제용 래퍼 `div` 요소에는 클래스 `grid-rows-[1fr_auto]`가 있어야 합니다.
...
가족 요금제용 래퍼 `div` 요소에는 클래스 `gap-6`가 있어야 합니다.
