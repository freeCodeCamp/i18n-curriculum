...
在最后一个 `.divider` 之后，使用文本 `Cholesterol 0mg 0%` 创建一个新的 `p` 元素。 然后，将文本 `Cholesterol` 放在一个 `span` 元素中，将文本 `0%` 放在另一个中，并为他们设置 `bold` 类。

最后，将包含文本 `Cholesterol` 和文本 `0mg` 的 `span` 元素嵌套到另一个 `span` 元素中，以便对齐。
...
你的新的 `p` 元素应包含文本 `Cholesterol 0mg 0%`。
...
包含文本 `Cholesterol` 的 `span` 元素应该将 `class` 属性设置为 `bold`。
...
包含文本 `0%` 的 `span` 元素应该将 `class` 属性设置为 `bold`。
...
包含文本 `Cholesterol` 的 `span` 元素和文本 `0mg` 应该共同包含在另一个 `span` 元素中。
