...
残りのクルーは、次のデータを持つ`remainingCrew`という名前の配列で作成されています。

| id | name | 役割 | isEVAEligible | 優先度 |
| -- | ---- | ---- | ------------- | -------- |
...
`remainingCrew`配列をループして、`squad`関数を使い、各宇宙飛行士を`addCrewMember()`に追加してください。

# --hints--

`for`の各宇宙飛行士に対して`addCrewMember()`を呼び出すには、`remainingCrew`ループを使うべきです。
