...
Endlich, da Sie den `salary` Setter abgeschlossen haben, können Sie ihn nun innerhalb des `level` Setters aufrufen.

Ändern Sie die Zeile `self._salary = Employee._base_salaries[new_level]` so, dass sie den `salary` Setter aufruft, anstatt direkt das `_salary` Attribut zu ändern.

Damit ist der Gehalts-Tracker-Workshop abgeschlossen.

# --hints--

Sie sollten `self.salary` im `Employee._base_salaries[new_level]` Setter auf `level` setzen.
