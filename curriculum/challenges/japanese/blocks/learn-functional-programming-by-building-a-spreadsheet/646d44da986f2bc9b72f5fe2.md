...
`toNumberList` 関数を宣言し、`args` パラメータを受け取り、`args` をカンマで分割した結果を暗黙的に返してください。その後、`map` メソッドを `split` メソッドにチェーンし、`parseFloat` メソッドの引数として `map` を渡してください。

# --hints--

`toNumberList` 変数を宣言してください。
...
`const` を使って `toNumberList` 変数を宣言してください。
...
`toNumberList` 関数は `args` パラメータを持つべきです。

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

`toNumberList` 関数は暗黙の返り値を使うべきです。
...
`toNumberList` 関数は `.split()` の `args` メソッドを呼び出した結果を返すべきです。
...
`.map()` メソッドを `.split()` メソッドにチェーンしてください。
...
`parseFloat` への参照を `.map()` のコールバックとして渡してください。
