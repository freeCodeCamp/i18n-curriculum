...
Функція `setPlayButtonAccessibleText` встановить атрибут `aria-label` на поточну пісню або на першу пісню у списку відтворення. І якщо список відтворення порожній, він встановлює `aria-label` на `"Play"`.

Використайте метод `setAttribute` на елементі `playButton`, щоб встановити атрибут під назвою `"aria-label"`. Використовуючи потрійний, встановіть значення атрибута на `Play ${song.title}` або `"Play"`, якщо `song?.title` недоступний.

Не забувайте, що тут потрібна інтерполяція рядків, тому потрібно використати зворотні дужки.

# --hints--

Ви не повинні змінювати наявну функцію `setPlayButtonAccessibleText` та її вміст.
...
Використайте метод `setAttribute()` на `playButton`.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` має бути першим значенням методу `setAttribute()`.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

Метод `setAttribute` повинен мати ``song?.title ? `Play ${song.title}` : "Play"`` як другий аргумент.
