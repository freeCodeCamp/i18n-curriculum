...
DRI 입력에 `value` 속성으로 `player.dri`을 추가하세요. 그런 다음 `onChange`을 매개변수로 받고 `e`를 펼친 상태로 `setPlayer`를 호출하며 `player`을 `dri`로 설정하는 화살표 함수로 `Number(e.target.value)` 속성을 추가하세요.

# --hints--

DRI `input`는 플레이어의 `value` 스탯과 같은 `dri`를 가져야 합니다.
...
DRI `input`는 `onChange`로 설정된 `setPlayer`와 `player`를 펼친 상태로 `dri`를 호출하는 `Number(e.target.value)` 핸들러를 가져야 합니다.
