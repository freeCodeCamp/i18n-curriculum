...
Під вашим елементом `blockquote` додайте ще один елемент `p` з таким текстом:
...
Ви повинні додати четвертий елемент `p` до вашого елемента `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 5);
```

Ваш новий елемент `p` має йти після вашого елемента `blockquote`.

```js
assert(document.querySelectorAll('.text p')?.[4]?.previousElementSibling?.localName === 'blockquote');
```

Ваш новий елемент `p` повинен містити наданий текст.
