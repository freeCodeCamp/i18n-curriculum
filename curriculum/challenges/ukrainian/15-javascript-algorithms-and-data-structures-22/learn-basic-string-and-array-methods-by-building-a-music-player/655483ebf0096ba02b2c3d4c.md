...
Щоб отримати попередню пісню, відніміть `1` від `currentSongIndex` з `userData?.songs` та призначте її до константи `previousSong`. Після цього, викличте функцію `playSong` та передайте `previousSong.id` як аргумент.
...
Призначте `userData?.songs[currentSongIndex - 1]` до константи `previousSong`.
...
Викличте функцію `playSong` з `previousSong.id`.
