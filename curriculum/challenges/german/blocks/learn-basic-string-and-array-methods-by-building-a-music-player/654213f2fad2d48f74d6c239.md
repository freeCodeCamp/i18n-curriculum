...
Erstellen Sie ein weiteres `button`-Element mit der Klasse `playlist-song-delete` und dem Attribut `aria-label`, das auf `Delete` interpoliert mit `song.title` gesetzt ist. Für den Inhalt des Löschen-Symbols fügen Sie das folgende SVG ein:
...
Sie sollten ein `button`-Element mit der Klasse `playlist-song-delete` als erstes Attribut und Wert erstellen.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

Ihr Button-Element sollte ein Attribut `aria-label` mit dem Wert `Delete ${song.title}` als zweites Attribut und Wert haben.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

Sie sollten das bereitgestellte `svg` als Inhalt Ihres `button`-Elements einfügen.
