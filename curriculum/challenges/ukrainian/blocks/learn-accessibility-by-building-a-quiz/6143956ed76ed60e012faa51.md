...
У першому елементі `span` додайте текст `Question`.

У другому елементі `span` додайте текст `Question`.

# --hints--

Ви повинні додати першому елементу `span` текст `Question`.

```js
assert.equal(document.querySelector('h3 > span')?.textContent, 'Question');
```

Ви повинні додати другому елементу `span` текст `Question`.
