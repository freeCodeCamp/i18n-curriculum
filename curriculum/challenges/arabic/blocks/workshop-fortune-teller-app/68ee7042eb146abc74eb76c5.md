...
مرة أخرى، تحتاج إلى جعل البطاقة عشوائية ومعكوسة. أعلن عن متغيرين باسم `isReversed` و `card`. للمتغير الأول، استدعِ طريقة `Math.random` وقارنه بـ `< 0.5`. للثاني، استدعِ `getRandomItem` ومرر `this.cards`.

# --hints--

يجب إعلان متغير باسم `isReversed`.
...
يجب أن يستخدم متغير `isReversed` دالة `Math.random()` ليقرر عشوائيًا ما إذا كانت البطاقة معكوسة.
...
يجب تعيين متغير `card` عن طريق استدعاء طريقة `getRandomItem` مع تمرير `this.cards` كمعلمة.
