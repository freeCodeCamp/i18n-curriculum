...
اختر كل عناصر `input` و`textarea` و`select`، واجعلها تملأ العرض الكامل لعناصر الأب الخاصة بها.

أضف أيضًا `10px` من الـ `margin` إلى أعلى العناصر المختارة. اجعل بقية الهوامش تساوي `0`.

# --hints--

يجب أن تستخدم محدد عناصر مفصول بفواصل لاختيار عناصر `input` و`textarea` و`select`.
...
يجب أن تضبط خاصية `width` على `100%`.
...
يجب أن تضبط خاصية `margin-top` على `10px`.
...
يجب أن تضبط خاصية `margin-bottom` على `0`.
...
يجب أن تضبط خاصية `margin-left` على `0`.
...
يجب أن تضبط خاصية `margin-right` على `0`.
