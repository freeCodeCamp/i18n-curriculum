...
다른 `input` 및 `label` 요소와 마찬가지로 `textarea`를 해당 `label` 요소에 연결하고 `name` 속성을 부여하세요.

# --hints--

`textarea` 요소에 `id` 속성을 부여해야 합니다.

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id);
```

`id` 요소의 `textarea`에는 공백을 포함할 수 없습니다.

```js
assert.match(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id, /^\S+$/);
```

다른 요소가 이미 그 `id`을 사용 중이므로 `textarea` 요소에는 다른 `id`를 선택해야 합니다.
...
`label` 요소의 `for`와 일치하는 `id` 속성을 `textarea` 요소에 부여해야 합니다.
...
`textarea` 요소에 `name` 속성을 부여해야 합니다.

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.name);
```

`name`의 `textarea` 속성에는 최소한 하나 이상의 공백이 아닌 문자를 사용해야 합니다.
