...
Dentro de la clase `Bee`, crea un constructor que reciba un solo parámetro `emojiElement` de tipo `HTMLParagraphElement`. 

Dentro del constructor, llama a la función `super` con el parámetro `emojiElement` para invocar el constructor de la clase padre `Bug`. Luego, asigna la propiedad `emoji` con el valor de cadena `"🐝"`.

# --hints--

Tu clase `Bee` debe tener un constructor.
...
Tu constructor `Bee` debe tener un parámetro llamado `emojiElement` de tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Bee[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Tu constructor `Bee` debe llamar a `super` con el parámetro `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /super\s*\(\s*emojiElement\s*\)/);
```

Tu constructor `Bee` debe asignar `this.emoji` a `"🐝"`.
