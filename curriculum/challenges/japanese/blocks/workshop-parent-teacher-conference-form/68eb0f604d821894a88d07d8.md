...
コンテナを目立たせるために、`box-shadow` を `0 5px 15px black` に設定してください。

これによりコンテナの周りに影の効果が追加されます。値はそれぞれ水平方向のオフセット、垂直方向のオフセット、ぼかしの半径、色を制御します。
...
`box-shadow` プロパティを `0 5px 15px black` に設定してください。
