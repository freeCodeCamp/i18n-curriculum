...
Sasa unaweza kufanya kazi kwenye sikio la kulia la paka. Tayari una HTML yake.

Ukitegemea kichaguzi cha darasa la vitu, mpe kipengele cha `.cat-right-ear` mpaka wa kushoto na wa kulia wa `35px solid transparent` kila mmoja. Pia, weka mpaka wa chini kuwa `70px solid #5e5e5e`.
...
Kichaguzi chako cha `.cat-right-ear` kinapaswa kuwa na sifa ya `border-left` iliyowekwa kuwa `35px solid transparent`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderLeft, '35px solid transparent')
```

Kichaguzi chako cha `.cat-right-ear` kinapaswa kuwa na sifa ya `border-right` iliyowekwa kuwa `35px solid transparent`.
