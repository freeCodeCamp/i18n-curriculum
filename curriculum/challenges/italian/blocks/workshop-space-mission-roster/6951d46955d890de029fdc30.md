...
Avevi precedentemente aggiunto una chiamata `console.log()` alla tua funzione `swapCrewMembers` per validarne il comportamento, e ora quella chiamata non è più necessaria. Per evitare che il terminale si riempia nei passaggi successivi, procedi a eliminare il loop `for` attualmente presente che scrive nella console.

# --hints--

Elimina il loop `for` dalla funzione `swapCrewMembers`. Devi eliminare l'intero blocco del loop.
