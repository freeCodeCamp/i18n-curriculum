...
これからフォームの作成を始めます。`div`の`className`内にある`form-panel`の中に、「Player Info」セクションを囲む`div`を追加してください。その新しい`div`の中に、`p`の`className`を持つ`form-section-title`とテキスト`Player Info`を追加します。

このセクションには次のステップでさらにフィールドを追加していきます。

# --hints--

`div.form-panel`には「Player Info」セクションを囲む`div`を含める必要があります。
...
`div`内の`div.form-panel`には、`p`の`className`を持ち、テキスト`form-section-title`を含む`Player Info`を入れてください。
