...
Dovresti dare ai tuoi pulsanti di opzione personalizzati una dimensione e una forma.

Continuando a puntare `.contact-method-radio-btn`, imposta `width` su `20px`, `height` su `20px`, `border-radius` su `50%` e `border` su `2px solid gray`.

# --hints--

Dovresti continuare a puntare alla classe `.contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Imposta la proprietà `width` su `20px`.
...
Imposta la proprietà `height` su `20px`.
...
Imposta la proprietà `border` su `2px solid gray`.
