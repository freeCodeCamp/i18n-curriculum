...
A saída está ficando bastante difícil de ler, então você deve substituir o `print(graph.create_trajectory())` por um loop que imprime cada linha separadamente. 

# --hints--

Remova a chamada `print(graph.create_trajectory())`.
...
Você deve ter um loop que itere sobre `graph.create_trajectory()`.
...
Você deve imprimir cada linha em `graph.create_trajectory()`.
