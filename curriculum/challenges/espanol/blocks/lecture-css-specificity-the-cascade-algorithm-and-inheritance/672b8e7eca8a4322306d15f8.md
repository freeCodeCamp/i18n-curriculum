...
Aquí hay un ejemplo de cómo usar el selector universal para establecer el `margin` y el `padding` para todo el documento HTML:
...
El selector universal `*` establece el color del texto a `blue` para todos los elementos.

Sin embargo, el selector de tipo `p` sobrescribe esto estableciendo el color del texto a `red` específicamente para los elementos `p`.

Si un elemento tiene la clase `highlight`, el selector de clase toma precedencia, cambiando el color del texto a `green`.

Finalmente, el selector de ID `#unique`, que tiene la especificidad más alta, sobrescribirá a todos los demás, estableciendo el color del texto a `purple`.
