...
Залишок об’єкта `allCategories` вже заповнено для вас.

У наступних кількох кроках ви створите функцію для отримання назви категорії з об’єкта `allCategories`.

Почніть зі створення стрілкової функції з ім’ям `forumCategory`, де `id` буде ім’ям параметра.

# --hints--

Вам потрібно створити порожню функцію з ім’ям `forumCategory`.

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

У функції `id` має бути параметр з ім’ям `forumCategory`.
