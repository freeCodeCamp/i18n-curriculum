...
Submeter um formulário com uma opção selecionada não enviaria um valor útil para o servidor. Por causa disso, cada elemento `option` precisa de um atributo `value`. Sem esse atributo, o conteúdo de texto do `option` será enviado ao servidor.

Dê ao primeiro `option` um `value` de `""` e aos elementos `option` subsequentes atributos `value` de `1` a `4`.

# --hints--

Você deve dar à primeira `option` um `value` de `""`.
