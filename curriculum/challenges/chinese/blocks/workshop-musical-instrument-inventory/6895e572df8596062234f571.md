...
最后，对 `instrument_2` 执行相同操作。调用 `instrument_2` 上的 `play()` 方法，然后打印调用 `instrument_2` 上的 `get_fact()` 的结果。

这将完成你的音乐记录仪工作坊！

# --hints--

你应该在 `instrument_2` 实例上调用 `play()` 方法。
...
你应该在 `instrument_2` 上调用 `get_fact()` 并打印结果。
