...
Vamos ver o que acontece quando criamos uma classe que torna o texto rosa e, em seguida, aplicamos ela a um elemento.Nossa classe irá *sobrescrever* a propriedade CSS `color: green;` do elemento `body`?
...
Seu `<style>` deve ter uma classe CSS `pink-text` que altera a `color`.
