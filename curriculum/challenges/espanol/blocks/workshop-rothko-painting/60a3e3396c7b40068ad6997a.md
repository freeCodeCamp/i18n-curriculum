...
Escribe una nueva regla que apunte a `.one` y establece su propiedad `width` a 425 píxeles.

# --hints--

Debes tener un selector `.one`.
...
Debes establecer la propiedad `width` en `425px`.
...
Tu elemento `.one` debería tener un valor de `width` de `425px`.
