...
因爲 `elemValue` 返回一個函數，所以你的 `addCharacters` 函數最終返回一個函數引用數組。 你希望 `.map()` 方法運行 `elemValue` 函數的內部函數，這意味着你需要調用 `elemValue` 而不是引用它。 將 `num` 作爲參數傳遞給 `elemValue` 函數。

<!-- TODO: Explain further? -->

# --hints--

你應該在 `.map()` 方法中調用 `elemValue()`。
