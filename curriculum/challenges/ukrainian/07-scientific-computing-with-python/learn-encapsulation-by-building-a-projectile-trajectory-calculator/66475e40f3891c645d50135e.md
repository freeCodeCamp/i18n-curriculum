...
`matrix_list` — це список списків, кожен елемент має координату `(x, y)`. Використайте список координат у `rounded_coords`, щоб змінити елементи у `matrix_list` за координатами в списку на символ у змінній `PROJECTILE`. Пам’ятайте, що графік координат має `(0, 0)` у нижньому лівому куті.
...
Метод має повернути правильне значення для різних екземплярів.
