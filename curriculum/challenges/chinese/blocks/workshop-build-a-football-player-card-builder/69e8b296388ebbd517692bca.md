...
`FootballPlayerCard` 当前使用固定的 `defaultPlayer` 来渲染卡片。为了让应用具有交互性，你需要在 React 状态中管理玩家数据，以便更新时能够触发重新渲染。

在 `FootballPlayerCard` 组件内部，创建一个名为 `player` 的状态变量和一个名为 `setPlayer` 的设置函数，用来保存当前的玩家数据。用 `defaultPlayer` 初始化这个状态，并指定该状态使用 `PlayerData` 类型作为通用类型，这样 TypeScript 就知道玩家对象应该是什么结构。

之后，更新 return，将 `player` 状态变量作为属性传递给 `PlayerCard`。

# --hints--

你应该使用数组解构来设置一个 `player` 状态变量和一个 `setPlayer` 设置函数。

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

你的 `player` 和 `setPlayer` 应该使用 `useState` 钩子。
...
你的 `useState` 应该用 `defaultPlayer` 初始化，并且有 `PlayerData` 作为它的通用类型。

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

你应该将 `player` 作为 `player` 属性的值传递给 `PlayerCard`。
