...
現在你將創建一個函數來使用作者數據填充 UI。 你將在第二個 `.then()` 方法中調用此函數。

創建一個名爲 `displayAuthors` 的空箭頭函數，以 `authors` 作爲參數。
