...
Agora você precisa de uma função que ajude a obter uma fortuna aleatória.

Declare uma função chamada `getRandomItem` que receba um array de `items` como argumento. Use genéricos para essa função.

Dentro da função, use `Math.random()` e `Math.floor()` para selecionar uma fortuna aleatória do array `items`.
Não esqueça de retornar a fortuna selecionada.
