...
在 `li` 元素内，在 `item` 文本上方，添加一个 `input` 元素，其 `type` 属性设置为 `checkbox`。同时，将其 `onChange` 属性设置为一个调用 `toggleItem()` 并以 `item` 作为参数的匿名函数。

最后，将 `input` 和 `item` 文本包裹在 `label` 元素中以提高无障碍性。

# --hints--

你应该添加另一个 `input` 元素，其 `type` 属性设置为 `checkbox`。
...
你的新建 `input` 应该有一个 `onChange` 属性，设置为一个调用 `toggleItem()` 并以 `item` 作为参数的匿名函数。
...
你的新建 `input` 和 `item` 文本应包裹在一个 `label` 元素中。
