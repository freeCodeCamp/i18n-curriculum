...
لتكن `f(k, n)` = $e^\frac{k}{n} - 1$، لكل الأعداد الصحيحة غير السالبة `k`.

من اللافت أن `f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3.1415926… ≈ π.

في الواقع، هذا هو أفضل تقريب لـ π بالشكل `f(a, 200) + f(b, 200) + f(c, 200) + f(d, 200)`.

لتكن `almostPi(n)` = a<sup>2</sup> + b<sup>2</sup> + c<sup>2</sup> + d<sup>2</sup> حيث a و b و c و d تقلل الخطأ: $\lvert f(a,n) + f(b,n) + f(c,n) + f(d,n) - \Pi\rvert$

معطى لديك أن `almostPi(200)` = 6<sup>2</sup> + 75<sup>2</sup> + 89<sup>2</sup> + 226<sup>2</sup> = 64658.

# --hints--

يجب أن تكون `almostPi` دالة.

```js
assert.isFunction(almostPi);
```

يجب أن تُرجع `almostPi` رقمًا.

```js
assert.strictEqual(typeof almostPi(10), 'number');
```

يجب أن تُرجع `almostPi(29)` القيمة `1208`.

```js
assert.strictEqual(almostPi(29), 1208);
```

يجب أن تُرجع `almostPi(50)` القيمة `4152`.

```js
assert.strictEqual(almostPi(50), 4152);
```

يجب أن تُرجع `almostPi(200)` القيمة `64658`.
