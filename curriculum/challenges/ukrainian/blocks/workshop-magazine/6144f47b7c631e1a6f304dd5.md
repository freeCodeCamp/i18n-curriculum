...
У вашому журналі буде три основні розділи. Ви вже встановили загальний макет у правилі `main`, але можете налаштувати розташування у дочірніх правилах.

Одним із варіантів є властивість `grid-column`, яка є скороченням для `grid-column-start` та `grid-column-end`. Властивість `grid-column` вказує елементу сітки, з якої лінії сітки починатися і на якій закінчуватися.

Створіть правило `.heading` і встановіть властивість `grid-column` у значення `2 / 3`. Це скаже елементу `.heading` починатися з лінії сітки 2 і закінчуватися на лінії сітки 3.

# --hints--

Ви повинні мати селектор `.heading`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.heading'));
```

Ваш селектор `.heading` має мати властивість `grid-column`, встановлену в `2 / 3`.
