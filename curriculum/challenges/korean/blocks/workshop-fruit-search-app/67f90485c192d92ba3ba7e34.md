...
`useEffect` 콜백 안에 `if` 조건을 추가해 `query.trim() === ''`인지 확인하세요. 이는 `query`이 빈 문자열이거나 공백만 포함하는지 검사합니다. 그렇다면 `setResults([])`를 호출하고 조기 반환하세요.

# --hints--

`query.trim() === ''`인지 확인해야 합니다.
...
`query.trim() === ''`가 true라면, 빈 배열을 인수로 `setResults()`를 호출한 뒤 `return` 키워드를 사용하세요.
