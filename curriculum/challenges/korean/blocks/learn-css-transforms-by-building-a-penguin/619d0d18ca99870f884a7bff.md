...
각 `.eye` 요소 안에 `div`의 `class`를 가진 `eye-lid`을 추가하세요.

# --hints--

`div` 안에 `.eye.left` 요소를 하나 추가해야 하지만 `--fcc-actual--`를 찾았습니다.

```js
assert.equal(document.querySelectorAll('.eye.left > div')?.length ?? 0, 1);
```

`div` 안에 `.eye.right` 요소를 하나 추가해야 하지만 `--fcc-actual--`를 찾았습니다.

```js
assert.equal(document.querySelectorAll('.eye.right > div')?.length ?? 0, 1);
```

첫 번째 새 `div`에 `class`의 `eye-lid`을 지정해야 합니다.

```js
assert.exists(document.querySelector('.eye.left > div.eye-lid'));
```

두 번째 새 `div`에 `class`의 `eye-lid`을 지정해야 합니다.
