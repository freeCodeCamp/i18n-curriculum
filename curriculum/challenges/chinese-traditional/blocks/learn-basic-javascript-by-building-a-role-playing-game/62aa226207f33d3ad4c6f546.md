...
使用 `+=` 運算子，將 `" Your <weapon> breaks."`（`Your` 前面有一個空格）添加到 `text.innerText` 的結尾。使用 `inventory.pop()` 將 `<weapon>` 替換為 `inventory` 陣列中的最後一個項目，該方法會移除陣列中的最後一個項目並傳回它，使其出現在你的字串中。

# --hints--

你應該使用複合指定運算子來修改 `text.innerText`。
