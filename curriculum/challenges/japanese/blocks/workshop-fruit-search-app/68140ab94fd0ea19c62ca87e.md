...
`timeoutId` 変数の下に、`clearTimeout(timeoutId)` を使ってタイムアウトをクリアするクリーンアップ関数を返してください。これにより、複数の遅延フェッチが重なるのを防げます。クリーンアップ関数は、暗黙の戻り値を持つアロー関数にしてください。

これで Fruit Search アプリが完成です！`apple` や `pear` のような果物の名前を入力して、結果が表示されるのを試してみてください。

# --hints--

`clearTimeout(timeoutId)` フックの最後に `useEffect` を呼び出すアロー関数を返す必要があります。
