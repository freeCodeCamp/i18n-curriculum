...
Тепер додайте `div` з `className` `card-stats`. Всередині нього додайте `div` з `className` `stat-col`. Усередині цього додайте три елементи `div`, кожен з `className` `stat-row`.

# --hints--

Ви повинні додати `div` з `className` `card-stats` після `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

Ви повинні додати `div` з `className` `stat-col` всередині `.card-stats`.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

Ви повинні додати три елементи `div`, кожен з `className` `stat-row` всередині `.stat-col`.
