...
使用 `forEach` 方法迴圈遍歷 `playlistSongElements`。 

`forEach` 方法用於遍歷陣列並對陣列中的每個元素執行函式（程式）。例如，假設你有一個數字陣列，並且你想將每個數字記錄到主控台。
...
對 `playlistSongElements` 使用 `forEach` 方法。傳入 `songEl` 作為參數，並使用箭頭語法添加一個空的回呼。

# --hints--

你應該將 `forEach()` 串鏈到 `playlistSongElements`。

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

你的 `forEach()` 方法應該有一個 `songEl` 參數並使用箭頭語法。

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

你的 `forEach` 應該有一對空的花括號。
