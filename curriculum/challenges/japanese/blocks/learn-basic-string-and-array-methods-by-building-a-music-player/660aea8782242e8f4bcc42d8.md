...
通常の関数と同様に、アロー関数も複数のパラメータを受け取ることができます。

以下は、1つのパラメータを持つ名前付きアロー関数の例です。
...
関数にパラメータが1つだけの場合、パラメータリストの丸括弧を省略することができます。このように書きます。
...
`printMessage` という名前の新しい名前付きアロー関数を作成してください。この関数は `org` という名前のパラメータを1つ持ちます。その関数の本体内にコンソール文を追加してください。そのコンソール文の中にテンプレートリテラル `${org} is awesome!` を入れてください。

`printMessage` 関数の下に、その関数を呼び出し、文字列 `"freeCodeCamp"` を引数として渡してください。

結果を見るためにコンソールを開いてください。

# --hints--

`printMessage` 関数があるはずです。
...
`printMessage` 関数は `org` というパラメータを持つべきです。
...
`printMessage` 関数にはコンソール文があるべきです。

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

コンソール文にはテンプレートリテラル `${org} is awesome!` を含めるべきです。

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

`printMessage` 関数を呼び出し、文字列 `"freeCodeCamp"` を渡すべきです。

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

関数の呼び出しは `printMessage` 関数定義の下にあるべきです。
...
`printMessage` 関数の呼び出しはコンソールに `freeCodeCamp is awesome!` を出力するべきです。
