...
Ongeza sifa ya kipengele ya `value` ya `player.overallRating` kwa `input` ya jumla. Baada ya hapo, ongeza sifa ya kipengele ya `onChange` iliyowekwa kuwa kitendakazi cha mshale kinachopokea `e` kama kigezo chake na kufanya wito wa `setPlayer` kwa kueneza `player` na `overallRating` kuwekwa kuwa `Number(e.target.value)`.

Kwa kuwa `overallRating` ni nambari, tumia `Number()` kubadilisha thamani ya mfuatano wa herufi kutoka kwa ingizo.

# --hints--

`input` yako ya jumla inapaswa kuwa na `value` sawa na kiwango cha jumla cha mchezaji wa sasa.
...
`onChange` ya `input` yako ya jumla inapaswa kufanya wito wa `setPlayer` kwa kueneza `player` na `overallRating` kuwekwa kuwa `Number(e.target.value)`.
