...
Тепер переходимо до значка `Most popular`. Його потрібно розмістити у верхньому правому куті картки, тому ви можете використати деякі утиліти позиціонування Tailwind для цього.

Ось як можна застосувати позиціонування CSS у Tailwind:
...
Призначте клас `relative` обгортці `div` і `absolute` елементу `div` з текстом `Most popular`.

# --hints--

Ваш обгортковий елемент `div` для преміум-плану має мати клас `relative`.
...
Ваш елемент `div` з текстом `Most popular` має мати клас `absolute`.
