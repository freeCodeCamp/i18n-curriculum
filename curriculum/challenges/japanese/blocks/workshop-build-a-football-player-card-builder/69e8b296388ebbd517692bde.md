...
DRIの入力に`value`属性として`player.dri`を追加してください。次に、`onChange`をパラメータに取り、`e`のスプレッドと`setPlayer`を`player`に設定して`dri`を呼び出すアロー関数を設定した`Number(e.target.value)`属性を追加してください。

# --hints--

あなたのDRIの`input`は、プレイヤーの`value`ステータスと等しい`dri`を持つ必要があります。
...
あなたのDRIの`input`は、`onChange`に設定された`setPlayer`と`player`のスプレッドを使って`dri`を呼び出す`Number(e.target.value)`ハンドラを持つ必要があります。
