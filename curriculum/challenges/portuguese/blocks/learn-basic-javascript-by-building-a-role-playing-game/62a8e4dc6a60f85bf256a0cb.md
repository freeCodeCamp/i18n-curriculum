...
O objeto `Math` em JavaScript contém propriedades e métodos estáticos para constantes e funções matemáticas.Um deles é `Math.random()`, que gera um número aleatório de `0` (inclusivo) a `1` (exclusivo).Outra é `Math.floor()`, que arredonda um número qualquer para o inteiro mais próximo abaixo dele.

Ao usá-las, você pode gerar um número aleatório dentro de um intervalo.Por exemplo, isso gera um número aleatório entre `1` e `5`: `Math.floor(Math.random() * 5) + 1;`.

Seguindo este padrão, use o operador de adição (`+`) para adicionar um número aleatório entre `1` e o valor de `xp` ao seu `monsterHealth -= weapons[currentWeaponIndex].power`.
...
Você deve adicionar ao valor `weapons[currentWeaponIndex].power`.
...
Você deve adicionar o resultado de `Math.floor(Math.random() * xp) + 1` ao resultado de `weapons[currentWeaponIndex].power`.
