...
Então, atualize sua chamada `updateUI` para passar `sortedValues` como argumento.
...
`updateUI` deve ser chamado com `sortedValues` como argumento.

```js
assert.match(code, /updateUI\s*\(\s*sortedValues\s*\)/);
```

`updateUI` não deve ser chamado com `inputValues` como argumento.
