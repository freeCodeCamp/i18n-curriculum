...
Il resto dell'oggetto `allCategories` è stato completato per te.

Nei prossimi passaggi, creerai una funzione per recuperare il nome della categoria dall'oggetto `allCategories`.

Inizia creando una funzione freccia chiamata `forumCategory`, con `id` come nome del parametro.

# --hints--

Devi creare una funzione vuota chiamata `forumCategory`.

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

Dovresti avere un parametro chiamato `id` nella funzione `forumCategory`.
