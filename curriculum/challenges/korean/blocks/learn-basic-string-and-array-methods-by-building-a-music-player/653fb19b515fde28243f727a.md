...
UI(사용자 인터페이스)에 노래를 표시하려면 함수를 만들어야 합니다.

화살표 함수 구문을 사용해 `renderSongs` 함수를 만들고 `array`을 매개변수로 받으세요.

# --hints--

빈 함수를 만들려면 `const`를 사용해 `renderSongs`라는 이름의 함수를 만드세요.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs`는 함수여야 합니다.

```js
assert.isFunction(renderSongs)
```

`renderSongs` 함수는 화살표 함수 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

`renderSongs` 함수는 `array`을 매개변수로 받아야 합니다.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

`renderSongs` 함수는 비어 있어야 합니다.
