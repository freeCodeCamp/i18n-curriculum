...
`sticky` 定位是 `relative` 定位和 `fixed` 定位的混合体。 它允许元素根据滚动位置**粘**到其包含元素或视口中的特定位置。

将 `.cat-head` 的 `position` 属性值改为 `sticky`，将 `top` 设置为 `0`，然后删除 `left` 及其值。

**注意**：要了解 `sticky` 的工作原理，你必须在`.cat-head` 之前和之后放置一些文本。 如果你在此之后向下滚动，你会看到 `.cat-head` 卡在顶部并保留在那里。
