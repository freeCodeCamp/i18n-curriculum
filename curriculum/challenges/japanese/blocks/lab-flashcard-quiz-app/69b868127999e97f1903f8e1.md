...
このラボでは、フラッシュカードのデータを表示および保存し、後で取り出せるアプリを作成します。

**目的:** 以下のユーザーストーリーを満たし、すべてのテストを通過してラボを完了してください。

**ユーザーストーリー:**

1) idが`flashcard`のHTML要素が必要です。  
2) `FlashCard`というインターフェースが必要です。  
3) `FlashCard`インターフェースには、`questionText`と`questionAnswer`という両方とも`string`型のプロパティが含まれている必要があります。  
4) `FlashCard`という名前の`currentCards`要素のコレクションが必要です。  
5) `#flashcard`要素がクリックされたとき、カードに`flipped`クラスが付与される必要があります。  
6) idが`delete-btn`の要素が必要です。  
7) `#delete-btn`要素がクリックされたとき、現在のカードが削除され、前のカードのデータが表示される必要があります。  
8) idが`entry-form`の入力フォームを作成し、送信時に`currentCards`コレクションにフラッシュカードを追加できるようにしてください。  
9) フォーム内の2つの要素は、それぞれidが`front-text`と`back-text`である必要があります。  
10) 入力フォームの質問テキストまたは質問の答えが空の場合、`InvalidUserInputError`を作成して呼び出してください。

# --hints--

idが`flashcard`のHTML要素が必要です。
...
`FlashCard`インターフェースには、`questionText`という`string`型のプロパティが必要です。
...
`FlashCard`インターフェースには、`questionAnswer`という`string`型のプロパティが必要です。
...
`FlashCard`要素のコレクションで、`currentCards`という名前のものが必要で、型は`FlashCard[]`です。
...
`#flashcard`要素が最初にクリックされたとき、その要素に`flipped`クラスが付与される必要があります。
...
`delete-btn`がクリックされたとき、`currentCards`コレクションからフラッシュカード要素が削除されます。
...
`delete-btn`がクリックされたとき、前のフラッシュカードのデータが表示されます。
...
idが`entry-form`の入力フォームを作成してください。
...
フォーム内に、idが`textarea`と`front-text`の2つの`back-text`要素が必要です。
...
入力フォームの質問テキストまたは質問の答えが空の場合、`InvalidUserInputError`をスローしてください。
