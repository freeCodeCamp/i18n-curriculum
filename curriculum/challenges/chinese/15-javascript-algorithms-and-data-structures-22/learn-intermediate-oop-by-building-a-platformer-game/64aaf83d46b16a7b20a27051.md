...
在 `startGame` 函数中，你需要显示 `canvas` 元素并隐藏 `startScreen` 容器。

使用 `canvas.style.display` 将显示值更改为 `"block"`。

下面，使用 `startScreen.style.display` 将显示值更改为 `"none"`。

# --hints--

你应该使用点符号来访问 `canvas` 的 `style` 属性。

```js
assert.match(startGame.toString(), /canvas\.style/);
```

你应该使用点符号来访问 `style` 属性的 `display` 属性。

```js
assert.match(startGame.toString(), /canvas\.style\.display/);
```

你应该将 `canvas` `display` 属性设置为 `"block"`。

```js
assert.match(startGame.toString(), /canvas\.style\.display\s*=\s*('|")block\1/);
```

你应该使用点符号来访问 `startScreen` 的 `style` 属性。
...
你应该将 `startScreen` `display` 属性设置为 `"none"`。
