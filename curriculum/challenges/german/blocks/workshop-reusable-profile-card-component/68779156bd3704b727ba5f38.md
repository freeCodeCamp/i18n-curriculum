...
Wie Sie sich aus früheren Lektionen erinnern, können Sie die `map`-Methode verwenden, um die verschiedenen Objekte zu durchlaufen und die Komponenten dynamisch so zu rendern:
...
Verwenden Sie die `map()`-Methode, um ein einzelnes `Card` aus dem Array zu rendern. Übergeben Sie die `name`, `title` und `bio` als Props an die `Card`-Komponente.

# --hints--

Sie sollten die `map()`-Methode verwenden, um das `profiles`-Array zu durchlaufen.

```js
assert.match(code, /profiles\.map\s*\(/);
```

Sie sollten die Props `name`, `title` und `bio` vom `profile`-Objekt übergeben.
