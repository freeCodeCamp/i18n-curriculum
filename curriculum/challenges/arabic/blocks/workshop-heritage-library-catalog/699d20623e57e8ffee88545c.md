...
الآن ستبني `groupByDecade`، التي تنظم الكتالوج حسب العقد. كل مفتاح سيكون سلسلة نصية مثل `"1970s"` وكل قيمة ستكون مصفوفة من الكتب من تلك الفترة.

أنشئ دالة باسم `groupByDecade` تأخذ `catalog` كمعلمة. بداخلها، أعلن `const grouped = {}`. أضف حلقة `for` للتكرار على `catalog`، وداخل الحلقة أعلن `const book = catalog[i]`.

# --hints--

يجب أن يكون لديك دالة باسم `groupByDecade` مع معلمة `catalog`.
...
يجب أن تعلن عن متغير باسم `grouped` وتعيّنه إلى كائن فارغ.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+grouped\s*=\s*\{\s*\}/);
```

يجب أن تستخدم حلقة `for` للتكرار على `catalog`.

```js
assert.match(groupByDecade.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

يجب أن تعلن عن متغير باسم `book` وتعيّنه إلى العنصر الحالي من الكتالوج.
