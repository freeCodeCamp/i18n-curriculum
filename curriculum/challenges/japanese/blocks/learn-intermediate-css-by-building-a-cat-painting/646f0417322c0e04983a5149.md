...
クラスセレクターを使って、`.cat-mouth-line-left` 要素に `position` の `absolute`、`top` の `88px`、`left` の `74px` を与えてください。これで右側の位置に移動します。

# --hints--

`.cat-mouth-line-left` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left'))
```

`.cat-mouth-line-left` セレクターには `position` に設定された `absolute` プロパティがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.position === 'absolute')
```

`.cat-mouth-line-left` セレクターには `top` に設定された `88px` プロパティがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.top === '88px')
```

`.cat-mouth-line-left` セレクターには `left` に設定された `74px` プロパティがあるはずです。
