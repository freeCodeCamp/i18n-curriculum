...
使用類別選擇器，將 `.cat-whisker-left-top` 元素移動到位置，並設定 `position` 為 `absolute`、`top` 為 120px、`left` 為 52px。

# --hints--

你應該有一個 `.cat-whisker-left-top` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top'));
```

你的 `.cat-whisker-left-top` 選擇器應該有一個設定為 `absolute` 的 `position` 屬性。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.position, 'absolute');
```

你的 `.cat-whisker-left-top` 選擇器應該有一個 `top` 屬性設定為 `120px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.top, '120px');
```

你的 `.cat-whisker-left-top` 選擇器應該有一個設定為 `52px` 的 `left` 屬性。
