...
將空的回呼函式傳遞給 `.forEach()` 方法。對回呼的第一個參數，解構 `author`、`image`、`url` 和 `bio` 項目。 

對於第二個參數，傳入 `index`。這將表述每個作者的位置，並且對於後續分頁會很有用。
...
你應該將 `author`、`image`、`url` 和 `bio` 解構為你的回呼函式的第一個參數。別忘了將它們放在大括號內。
...
你應該將 `index` 解構為你的回呼函式的第二個參數。它必須放在你放其他參數的大括號外面。
