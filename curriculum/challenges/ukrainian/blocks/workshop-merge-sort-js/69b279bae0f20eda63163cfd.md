...
Нарешті, ви повинні скопіювати всі залишкові значення з правої половини у `array`.

Для цього додайте `while` цикл, який працюватиме, доки `rightArrayIndex` менше за `rightPart.length`, призначатиме `rightPart[rightArrayIndex]` у `array[sortedIndex]` і збільшуватиме як `rightArrayIndex`, так і `sortedIndex`.

# --hints--

Ви повинні створити `while` цикл з умовою `rightArrayIndex < rightPart.length`.
...
Тіло циклу має переміщувати `rightPart[rightArrayIndex]` у `array[sortedIndex]`.
...
Після присвоєння слід збільшити як `rightArrayIndex`, так і `sortedIndex`.
