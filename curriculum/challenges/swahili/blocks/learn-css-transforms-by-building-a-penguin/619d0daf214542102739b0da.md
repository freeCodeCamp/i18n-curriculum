...
Lenga vipengele vya `.eye-lid`, na uwape `width` ya `150%`, `height` ya `100%`, na `background-color` ya `--penguin-face`.
...
Unapaswa kumpa `.eye-lid` `width` ya `--fcc-expected--`, iliyopatikana `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

Unapaswa kumpa `.eye-lid` `height` ya `--fcc-expected--`, iliyopatikana `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.height, '100%');
```

Unapaswa kumpa `.eye-lid` `background-color` ya `var(--penguin-face)`.
