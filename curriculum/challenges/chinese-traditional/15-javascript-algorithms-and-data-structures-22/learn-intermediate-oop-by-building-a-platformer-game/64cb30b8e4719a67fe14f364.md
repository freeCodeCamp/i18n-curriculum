...
使用 `const` 創建一個名爲 `checkpointPositions` 的新數組。
...
你應該有一個 `checkpointPositions` 數組。

```js
assert.isArray(checkpointPositions);
```

`checkpointPositions` 數組內應該有三個對象。

```js
assert.lengthOf(checkpointPositions, 3);
```

你應該有一個對象，其 `x` 屬性設置爲 `1170`，`y` 屬性設置爲 `proportionalSize(80)`，`z` 屬性設置爲 `1`。

```js
assert.deepStrictEqual(checkpointPositions[0], { x: 1170, y: proportionalSize(80), z: 1 });
```

你應該有一個對象，其 `x` 屬性設置爲 `2900`，`y` 屬性設置爲 `proportionalSize(330)`，`z` 屬性設置爲 `2`。

```js
assert.deepStrictEqual(checkpointPositions[1], { x: 2900, y: proportionalSize(330), z: 2 });
```

你應該有一個對象，其 `x` 屬性設置爲 `4800`，`y` 屬性設置爲 `proportionalSize(80)`，`z` 屬性設置爲 `3`。
