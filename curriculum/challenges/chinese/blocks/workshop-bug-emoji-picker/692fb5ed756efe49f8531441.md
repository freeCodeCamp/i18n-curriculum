...
在 `Bee` 类中，实现一个以 `render` 关键字开头的 `override` 方法，用 `innerText` 属性的值更新 `emojiElement` 的 `emoji` 属性。

# --hints--

你的 `Bee` 类应该有一个使用 `render` 关键字的 `override` 方法。

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

你应该在 `this.emojiElement.innerText` 的 `this.emoji` 方法中将 `render` 设置为 `Bee`。
