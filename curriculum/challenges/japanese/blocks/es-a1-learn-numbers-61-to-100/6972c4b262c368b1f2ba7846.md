...
これから80から89までの数字を学びます。

スペイン語で80は `ochenta` と呼ばれます。

これらの数字は、10の位の単語 `ochenta`（80）に、接続詞の `y`（「と」）を続け、最後に1から9までの数字を付けて作られます。
...
この範囲で81の数字 `ochenta y uno` は特別で、男性名詞の前では `ochenta y un` に変わります。例えば：

`Ochenta y un años` - 81歳。
