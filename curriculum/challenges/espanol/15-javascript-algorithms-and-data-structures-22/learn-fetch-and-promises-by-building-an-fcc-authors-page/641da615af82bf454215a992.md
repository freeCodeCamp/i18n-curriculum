...
Pasa una función de devolución de llamada vacía al método `.forEach()`. Para el primer parámetro de la devolución de llamada, desestructura los elementos `author`, `image`, `url` y `bio`.

Para el segundo parámetro, pasa `index`. Esto representará la posición de cada autor y será útil para la paginación más tarde.
...
Debes desestructurar `author`, `image`, `url` y `bio` como el primer parámetro de tu función de devolución de llamada. No olvides ponerlos dentro de llaves.
...
Deberías desestructurar `index` como el segundo parámetro de tu función de devolución de llamada. Tiene que estar fuera de las llaves en las que pones los otros.

```js
assert.match(code, /authors\.forEach\(\s*\(\s*\{.*\}\s*,\s*index\s*\)/)
```

Tu función de devolución de llamada debe usar una sintaxis de flecha.
