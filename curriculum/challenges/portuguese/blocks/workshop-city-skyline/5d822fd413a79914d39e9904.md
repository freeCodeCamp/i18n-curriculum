...
Preencha `.bb3` com `repeating-linear-gradient`. Use `90deg` para a direção, com `building-color3` para as duas primeiras cores e `window-color3` a `15%` para a terceira.  When you don't specify a distance for a color, it will use the values that make sense. Neste caso, as duas primeiras cores estarão por padrão entre `0%` e `7.5%`, porque começam em `0%` e `7.5%` é metade dos `15%`.
