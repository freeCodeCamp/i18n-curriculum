...
قبل المتابعة، احذف استدعاءات `console.log` من شفرتك.

# --hints--

لا يجب أن تحتوي شفرتك على `console.log(removePlant(ballerina))`.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*removePlant\s*\(\s*ballerina\s*\)\s*\)/);
```

لا يجب أن تحتوي شفرتك على `console.log(sellPlants(ballerina, "small", 10));`.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

لا يجب أن تحتوي شفرتك على `console.log(catalog)`.
