...
إذا أردت إضافة المزيد من أيقونات التواصل الاجتماعي، مع إبقائها في نفس الصف، يجب تحديث `grid-template-columns` لإنشاء أعمدة إضافية. كبديل، يمكنك استخدام الخاصية `grid-auto-flow`.

تأخذ هذه الخاصية إما `row` أو `column` كقيمة أولى، مع قيمة ثانية اختيارية هي `dense`. تستخدم `grid-auto-flow` خوارزمية التوزيع التلقائي لضبط تخطيط الشبكة. تعيينها إلى `column` يخبر الخوارزمية بإنشاء أعمدة جديدة للمحتوى حسب الحاجة. تسمح القيمة `dense` للخوارزمية بالعودة وملء الفراغات في الشبكة بعناصر أصغر، مما قد يؤدي إلى ظهور العناصر بترتيب غير متسلسل.

للمحدِّد `.social-icons`، عيّن الخاصية `grid-auto-flow` إلى `column`.

# --hints--

يجب أن يحتوي المحدِّد `.social-icons` على خاصية `grid-auto-flow` مضبوطة على `column`.
