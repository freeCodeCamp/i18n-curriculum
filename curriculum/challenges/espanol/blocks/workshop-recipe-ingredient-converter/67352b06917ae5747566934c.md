...
Finalmente, actualiza tu función interna para devolver una tercera función. Esta tercera función también debe aceptar un argumento único llamado `quantity`, y tener un cuerpo vacío.

# --hints--

Tu segunda función debe devolver otra función más.

```js
assert.isFunction(convertQuantity()());
```

Tu función más interna debe tener un parámetro `quantity`.

```js
assert.match(convertQuantity()()?.toString(), /\(quantity\)/);
```

Tu función más interna debe estar vacía.
