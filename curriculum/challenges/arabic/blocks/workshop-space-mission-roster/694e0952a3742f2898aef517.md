...
يتطلب مركز التحكم طريقة لنسخ رواد الفضاء المؤهلين لـ EVA من الطاقم. أنشئ دالة فارغة باسم `getEVAReadyCrew` تستقبل معلمة `crew`.

# --hints--

يجب أن تنشئ دالة باسم `getEVAReadyCrew`.

```js
assert.isFunction(getEVAReadyCrew);
```

يجب أن تحتوي دالة `getEVAReadyCrew` على معلمة `crew`.
