...
عندما تكون قيمة `forward` هي `False`، يجب تحريك القرص في الاتجاه المعاكس. اكتب بند `else` لذلك. اطبع الحركة وغيّر محتوى القوائم وفقًا لذلك.

# --hints--

يجب أن يكون لديك بند `else` لتحريك الأقراص في الاتجاه المعاكس.

```js
({ test: () => assert.match(code, /rods\s*\[\s*target\s*\]\s*\.append\s*\(\s*rods\s*\[\s*source\s*\]\s*\.pop\s*\(\s*\)\s*\)\s+else\s*:/) })
```

يجب أن يحتوي بند `else` على `print(f'Moving disk {rods[target][-1]} from {target} to {source}')`.
...
يجب إزالة العنصر الأخير من `rods[target]`.

```js
({ test: () => assert.match(code, /rods\s*\[\s*target\s*\]\s*\.pop\s*\(\s*\)/) })
```

يجب أن يحتوي شفرتك على `rods[source].append(rods[target].pop())`.
