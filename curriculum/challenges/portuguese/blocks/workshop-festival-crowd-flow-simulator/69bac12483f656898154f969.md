...
Use um loop `for...in` para iterar através de cada portão no objeto `summary` de entrada e registre o total de participantes processados.

Para cada portão, registre uma mensagem neste formato: `gate + ": " + summary[gate] + " attendees processed"`.

Como exemplo, para um objeto `summary` de entrada definido como `{ North: 5, East: 3, South: 4, West: 2 }`, você deve registrar:
...
Você deve iterar através de cada elemento no objeto `summary` e registrar a seguinte mensagem uma vez por elemento: `element + ": " + summary[element] + " attendees processed`.
