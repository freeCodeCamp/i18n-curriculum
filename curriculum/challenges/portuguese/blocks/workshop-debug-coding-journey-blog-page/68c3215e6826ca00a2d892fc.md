...
A última alteração será no texto do email dentro do rodapé. Em lições e workshops anteriores, você aprendeu como trabalhar com o link `mailto` assim:

```html
<p>Email me at <a href="mailto:janedoe@email.com">janedoe@email.com</a></p>
```

Comece envolvendo o endereço de email `camperbot@blog.io` dentro de um elemento anchor. Em seguida, adicione o atributo `href` ao elemento anchor e defina-o igual a `mailto:camperbot@blog.io`.

Com essa última alteração, você resolveu com sucesso todos os problemas na página do blog!

# --hints--

Seu endereço de email `camperbot@blog.io` deve estar dentro de um elemento anchor.
...
Seu elemento anchor deve ter um atributo `href` com um valor de `mailto:camperbot@blog.io`.
