...
Erstellen Sie nun in der JavaScript-Datei eine `happyBtn`-Variable.

Verwenden Sie `querySelector`, um eine Referenz auf das `#happy-btn`-Element in der `happyBtn`-Variable zu speichern.

# --hints--

Sie sollten eine Variable mit dem Namen `happyBtn` haben.

```js
assert.exists(happyBtn);
```

Die `happyBtn`-Variable sollte eine Referenz auf das `#happy-btn`-Element enthalten.
