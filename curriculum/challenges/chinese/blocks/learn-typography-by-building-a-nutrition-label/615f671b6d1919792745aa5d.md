...
在你的 `.medium` 元素下方创建一个新的 `div` 元素。 给它一个 `class` 属性，设置为 `daily-value small-text`。 在这个新的 `div` 中，添加一个带有文本 `% Daily Value *` 的 `p` 元素，并将 `class` 属性设置为 `bold right`。
...
你的新的 `div` 应该将 `class` 属性设置为 `daily-value small-text`。
