...
`str2` 변수를 선언하고 `infixEval()`와 `noHigh`를 인수로 하여 `infix`을 호출한 결과를 할당하세요.
...
`str2`에 `infixEval()`을 호출한 결과를 할당해야 합니다.
...
`noHigh`를 `infixEval()`의 첫 번째 인수로 전달하세요.
...
`infix`를 `infixEval()`의 두 번째 인수로 전달하세요.
