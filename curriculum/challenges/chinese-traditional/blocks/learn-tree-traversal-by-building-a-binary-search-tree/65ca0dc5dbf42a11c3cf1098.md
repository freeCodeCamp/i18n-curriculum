...
將 `pass` 替換爲 `if` 語句，檢查當前節點 (`node`) 是否不爲空。 然後，以 `node.left` 和 `result` 作爲參數遞歸調用 `_inorder_traversal`。

# --hints--

你應該從 `_inorder_traversal` 方法中刪除現有的 `pass` 關鍵字。
...
你應該在 `if` 塊內的 `node.left` 上調用 `_inorder_traversal`。
