...
`.image-wrapper` 요소 안에서 두 번째 `img` 요소에 `src` 값을 가진 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`를, `alt` 값을 가진 `image of a calculator project`를, `loading`로 설정된 `lazy` 속성을, `class`로 설정된 `image-2`를, `width`로 설정된 `400` 속성을, 그리고 `height`로 설정된 `400` 속성을 각각 부여하세요.

# --hints--

두 번째 `img` 요소에 `src`로 설정된 `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`가 있어야 합니다.
