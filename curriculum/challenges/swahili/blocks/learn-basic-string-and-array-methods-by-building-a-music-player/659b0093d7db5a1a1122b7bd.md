...
Ili kupata kielezo cha wimbo wa sasa, unaweza kutumia njia ya <dfn>indexOf()</dfn>. Njia ya safu ya data `indexOf()` inarudisha kielezo cha kwanza ambacho kipengele fulani kinaweza kupatikana katika safu ya data, au `-1` ikiwa kipengele hakipo.
...
Ndani ya kitendakazi chako cha `getCurrentSongIndex`, rudisha `userData?.songs.indexOf()`. Kwa hoja ya `indexOf()`, iweka kuwa `userData?.currentSong`.

# --hints--

Unapaswa kurudisha `userData?.songs.indexOf()` ndani ya kitendakazi chako cha `getCurrentSongIndex`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

Unapaswa kupitisha `userData?.currentSong` katika njia ya `indexOf()`.
