...
Füge unter deinem `p`-Element ein weiteres `p`-Element mit dem Text `Dietary Fiber 4g` hinzu. Weise dem `p`-Element die `class` zu, die notwendig ist, um es einzurücken und den Trennungsrahmen zu entfernen. Erstelle dann einen Trenner unterhalb dieses `p`-Elements.
...
Dein neues `p`-Element sollte den Text `Dietary Fiber 4g` enthalten.
