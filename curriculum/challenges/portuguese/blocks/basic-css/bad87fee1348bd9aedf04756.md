...
**Nota:** Não importa a ordem em que as classes são listadas no elemento HTML.

No entanto, a ordem das declarações de `class` na seção `<style>` é o que importa. A última declaração sempre terá prioridade sobre a anterior. Como `.blue-text` é declarado em segundo lugar, ele substitui os atributos de `.pink-text`.
