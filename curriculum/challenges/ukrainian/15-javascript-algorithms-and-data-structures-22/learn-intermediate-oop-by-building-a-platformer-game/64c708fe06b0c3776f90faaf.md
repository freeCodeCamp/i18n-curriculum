...
Додайте клаузу `else`, яка призначає число `0` швидкості `x` гравця.

# --hints--

Додайте інструкцію `else` до функції `animate`.

```js
assert.match(animate.toString(), /\}\s*else\s*\{/);
```

Інструкція `else` має йти після інструкції `else if`.
...
Додайте умову `else`, яка призначає число `0` швидкості `x` гравця.
