...
结构模式匹配是一种 Python 构造，它支持将模式与主题值进行匹配，该主题值在 `match` 关键字后指定：
...
每个模式都在 `case` 语句后指定。 如果匹配，则运行 `case` 块内的代码。

使用 `match`/`case` 语法检查 `results` 的长度。 如果长度为 `0`，则将包含字符串 `'No real roots'` 的列表分配给名为 `result_list` 的变量。

# --hints--

你应该使用 `len(results)` 作为主题值来创建 `match`/`case` 构造。

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("len(results)")`)) })
```

你应该创建一个具有模式 `0` 的新 `case`。
...
你应该在 `case` 主体内分配一个包含 `'No real roots'` 到 `result_list` 的列表。
