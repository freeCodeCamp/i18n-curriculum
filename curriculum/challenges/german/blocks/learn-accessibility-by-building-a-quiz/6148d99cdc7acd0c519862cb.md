...
Zielen Sie auf alle `label`-Elemente innerhalb von `.info`-Elementen ab, und setzen Sie deren `width` auf `10%`, sodass sie nicht weniger als `55px` einnehmen.

# --hints--

Sie sollten entweder den `.info label`- oder den `.info > label`-Selektor verwenden.
...
Sie sollten den `label`-Elementen eine `width` von `10%` geben.
...
Sie sollten den `label`-Elementen eine `min-width` von `55px` geben.
