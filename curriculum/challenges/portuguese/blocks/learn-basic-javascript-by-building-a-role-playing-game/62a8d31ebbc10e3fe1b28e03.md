...
Os jogadores não deverão poder vender a única arma que têm. Dentro da função `sellWeapon`, adicione uma instrução `if` com uma condição que verifique se o tamanho do array `inventory` é maior que `1`.

# --hints--

A função `sellWeapon` deve ter uma instrução `if`.

```js
assert.match(sellWeapon.toString(), /if/);
```

A instrução `if` deve verificar se `inventory.length` é maior que `1`.
