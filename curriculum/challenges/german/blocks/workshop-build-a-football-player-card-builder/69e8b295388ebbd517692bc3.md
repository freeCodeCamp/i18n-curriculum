...
Erinnern Sie sich aus JSX, dass geschweifte Klammern es Ihnen ermöglichen, jeden JavaScript-Ausdruck direkt in JSX einzubetten. So zeigen Sie dynamische Daten aus Props an. Dasselbe machen Sie in TSX.

Die Kopfzeile der Karte zeigt noch fest codierte Werte. Ersetzen Sie den Text `98` innerhalb von `card-rating` durch `player.overallRating` und ersetzen Sie `ST` innerhalb von `card-position` durch `player.position`.

# --hints--

Sie sollten den fest codierten `98` durch `player.overallRating` in `.card-rating` ersetzen.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

Sie sollten den fest codierten `ST` durch `player.position` in `.card-position` ersetzen.
