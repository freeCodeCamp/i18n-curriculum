...
在 `HTMLString` 末尾創建另一個 `label` 元素（在新行上）。 這個 `label` 應該包含文本 `Entry # Calories`，使用模板文字語法將 `#` 替換爲 `entryNumber` 的值，並將 `for` 屬性設置爲 `X-#-calories`，其中 `X` 是 `entryDropdown` 的 `value`，而 `#` 是 `entryNumber` 的值。

# --hints--

你的 `HTMLString` 中應該有兩個 `label` 元素。
...
你的新 `label` 元素應將 `for` 屬性設置爲 `${entryDropdown.value}-${entryNumber}-calories`。
...
你的新 `label` 元素應該包含文本 `Entry ${entryNumber} Calories`。
