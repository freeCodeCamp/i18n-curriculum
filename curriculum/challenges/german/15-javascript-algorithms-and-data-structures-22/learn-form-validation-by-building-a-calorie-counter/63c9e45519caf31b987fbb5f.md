...
Deine `getCaloriesFromInputs`-Funktion setzt das globale Fehler-Flag auf `true`, wenn eine ungültige Eingabe erkannt wird. Füge eine `if`-Anweisung zu deiner `calculateCalories`-Funktion hinzu, die den Wahrheitsgehalt deines globalen Fehler-Flags prüft, und wenn er wahrheitsgemäß ist, verwende `return`, um die Funktionsausführung zu beenden.
