...
`.message` 要素にスタイルを適用する必要があります。次のようにしてください：

- `font-size` を `1.2em` に設定し、
- `margin-bottom` を `20px` に設定してください。

# --hints--

`.message` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".message"));
```

`.message` セレクターには `font-size` プロパティを `1.2em` に設定してください。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("font-size"), "1.2em");
```

`.message` セレクターには `margin-bottom` プロパティを `20px` に設定してください。
