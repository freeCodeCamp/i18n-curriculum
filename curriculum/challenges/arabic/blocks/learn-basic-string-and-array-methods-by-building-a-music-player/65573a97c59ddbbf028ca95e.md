...
أخيرًا، يجب أن تعرض الأغاني مرة أخرى، وتحدّث النص القابل للوصول لزر التشغيل، وتزيل زر إعادة التعيين من قائمة التشغيل. كما تحتاج إلى إزالة `resetButton` من DOM.

استدعِ الدالة `renderSongs()` مع `sortSongs()` كمعلمة لعرض الأغاني مرة أخرى بترتيب أبجدي.

استدعِ الدالة `setPlayButtonAccessibleText()` لتحديث النص القابل للوصول لزر التشغيل.

أزل زر إعادة التعيين من قائمة التشغيل باستدعاء الطريقة `remove()` على المتغير `resetButton`.

**ملاحظة**: يمكنك الآن تجربة إزالة كل الأغاني لترى ماذا يحدث.

# --hints--

لا تعدل مستمع الحدث الموجود ومحتواه.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

يجب أن تستدعي الدالة `renderSongs` مع `sortSongs()`.
...
يجب أن تستدعي الدالة `setPlayButtonAccessibleText`.
...
يجب أن تستخدم الطريقة `remove()` لإزالة `resetButton` من DOM.
