...
因為 `elemValue` 傳回一個函式（程式），你的 `addCharacters` 函式最終會傳回一個函式引用的陣列。你想讓 `.map()` 方法執行 `elemValue` 函式的內部函式，這表示你需要呼叫 `elemValue` 而不是取用它。將 `num` 作為引數傳遞給你的 `elemValue` 函式。

<!-- TODO: Explain further? -->

# --hints--

你應該在你的 `.map()` 方法中呼叫 `elemValue()`。
