...
創建一個名爲 `linkClass` 的常量，併爲其賦值一個模板字面量。 在該模板字面量中，添加 `category ${selectedCategory.className}` 的值。

這些類名將用於爲錨元素應用樣式。

# --hints--

你應該有一個名爲 `linkClass` 的常量。

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

你應該爲 `linkClass` 變量賦值一個模板字面量。

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

你應該在模板字面量中使用 `category ${selectedCategory.className}` 的值。
