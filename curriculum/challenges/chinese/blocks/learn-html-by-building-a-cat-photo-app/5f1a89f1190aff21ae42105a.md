...
All three checkboxes should have a `value` attribute. 检查开始标签的名称后面是否有空格和/或所有属性名称之前是否有空格。
...
The `value` attribute of the `Loving` checkbox should be set to `loving`. 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
...
The `value` attribute of the `Lazy` checkbox should be set to `lazy`. 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
...
The `value` attribute of the `Energetic` checkbox should be set to `energetic`. 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
