...
플레이어의 체력을 빼앗았으니, 플레이어의 `health`가 `0`보다 작거나 같은지 확인하세요. 그렇다면 `lose` 함수를 호출하세요.

# --hints--

`pick` 함수 안에 `if` 문장이 `else` 본문에 중첩되어 있어야 합니다.
...
`if` 문장은 플레이어의 `health`가 `0`보다 작거나 같은지 확인해야 합니다.

```js
assert.match(pick.toString(), /if\s*\(\s*health\s*<=\s*0\s*\)/);
```

`if` 문장은 `else` 본문에서 `lose` 함수를 호출해야 합니다.
