...
Додайте текст `Question` до першого елемента `span`.

Додайте текст `Question` до другого елемента `span`.

# --hints--

Надайте першому елементу `span` текст `Question`.

```js
assert.equal(document.querySelector('h3 > span')?.textContent, 'Question');
```

Надайте другому елементу `span` текст `Question`.
