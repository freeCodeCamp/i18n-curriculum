...
Il log `byDecade` ha svolto il suo scopo. Rimuovi `console.log(byDecade)` dal tuo codice.

Poi, per rendere più facile la scansione degli elementi del catalogo, crea una funzione chiamata `renderEntry` che prende `entry` come parametro. Lascia vuoto il corpo della funzione per ora. Sotto la funzione, registra `renderEntry(catalog[0])`.

# --hints--

`renderEntry` dovrebbe essere una funzione.

```js
assert.isFunction(renderEntry);
```

`renderEntry` dovrebbe essere una funzione con un parametro `entry`.
...
La chiamata `console.log(byDecade)` dovrebbe essere rimossa.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*byDecade\s*\)/);
```

Dovresti registrare `renderEntry(catalog[0])`.
