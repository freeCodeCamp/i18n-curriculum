...
لجعل القسم (section) الأول يبدو أكثر خطياً (inline)، استهدف فقط عناصر `input` داخل عناصر `.info`، و عيّن `width` إلى `50%` و قم بمحاذاة نصهم على اليسار ( left-align their text).

# --hints--

يجب عليك استخدام منتقي `.info input` أو `.info > input`.
...
يجب عليك إعطاء `input` خاصية `width` بقيمة `50%`.
...
يجب عليك إعطاء `input` خاصية `text-align` بقيمة `left`.
