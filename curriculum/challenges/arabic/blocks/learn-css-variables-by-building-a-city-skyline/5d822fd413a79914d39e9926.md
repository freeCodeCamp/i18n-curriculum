...
أعطِ عناصر `.fb3-window` `width` بقيمة `25%`، و`height` بقيمة `80%`، واستخدم متغير `--window-color1` كقيمة لـ `background-color`.

# --hints--

يجب أن تعطي `.fb3-window` `width` بقيمة `25%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

يجب أن تعطي `.fb3-window` `height` بقيمة `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

يجب أن تعطي `.fb3-window` `background-color` بقيمة `--window-color1`.
