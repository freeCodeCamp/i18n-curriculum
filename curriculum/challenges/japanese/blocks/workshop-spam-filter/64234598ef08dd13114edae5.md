...
次に、`s` と `t` の文字をそれぞれ `5` と `7` にも一致するように更新してください。

# --hints--

`stockRegex` は文字クラスを使って、文字 `s` と数字 `5` に一致させる必要があります。

```js
assert.match(stockRegex.source, /\[(s5|5s)\]/);
```

`stockRegex` は文字クラスを使って、文字 `t` と数字 `7` に一致させる必要があります。
...
`stockRegex` は `570ck al3r7` に一致させてください。
