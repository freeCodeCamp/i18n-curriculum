...
Sasa tangaza mabadiliko wa `remainingCalories`, na weka thamani yake kuwa matokeo ya kutoa `consumedCalories` kutoka `budgetCalories` na kuongeza `exerciseCalories`.

# --hints--

Kitendakazi chako cha `calculateCalories` kinapaswa kuwa na mabadiliko wa `remainingCalories`.

```js
assert.match(calculateCalories.toString(), /remainingCalories\s*=/);
```

Unapaswa kupata thamani ya `budgetCalories - consumedCalories + exerciseCalories`.
...
Unapaswa kuweka thamani ya `budgetCalories - consumedCalories + exerciseCalories` kwa `remainingCalories`.
