...
Al igual que las funciones normales, las funciones flecha pueden devolver valores.

Aquí hay un ejemplo de una función flecha que devuelve el resultado de multiplicar dos números:
...
Crea una nueva variable llamada `addTwoNumbers` y asígnale una función flecha. Esta función flecha debe tomar dos parámetros llamados `num1` y `num2`. Dentro del cuerpo de la función, devuelve la expresión de sumar `num1` y `num2`.

Debajo de la función `addTwoNumbers`, agrega una declaración de consola. Dentro de esa declaración de consola, llama a la función `addTwoNumbers` y pasa los números `3` y `4` como argumentos.
...
Tu función `addTwoNumbers` debe usar la sintaxis de funciones flecha.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Tu función `addTwoNumbers` debe tener dos parámetros llamados `num1` y `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

Tu función `addTwoNumbers` debe devolver la expresión de sumar `num1` y `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

Debes tener una declaración `console.log(addTwoNumbers(3, 4));`.
