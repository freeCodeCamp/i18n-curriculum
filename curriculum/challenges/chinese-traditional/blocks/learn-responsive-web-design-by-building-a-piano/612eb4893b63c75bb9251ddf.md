...
通過創建 `.logo` 選擇器開始設置 logo 樣式。 將 `width` 設置爲 `200px`、 `position` 設置爲 `absolute`、`top` 設置爲 `23px`。
