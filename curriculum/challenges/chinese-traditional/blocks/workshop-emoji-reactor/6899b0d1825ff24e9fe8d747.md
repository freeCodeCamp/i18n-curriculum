...
在 `h1` 元素下方，為你的 `p` 元素添加類別 `description`。

在 `p` 元素內撰寫文字 `Click on the buttons below to rate your emotions.`

# --hints--

你應該在 `h1` 元素下方創建一個新的 `p` 元素。

```js
assert.exists(document.querySelector("h1 + p"));
```

新的 `p` 元素應該有一個 `description` 的類別。

```js
assert.equal(document.querySelector("h1 + p")?.className, "description");
```

新的 `p` 元素應該有 `Click on the buttons below to rate your emotions.` 的文字。
