...
이제 `+=` 연산자를 사용해 `" In your inventory you have: "` 문자열과 `inventory` 내용을 `text.innerText`에 더하세요. `" In your inventory you have: "` 문자열의 앞뒤 공백도 반드시 포함하세요.

# --hints--

다른 `text.innerText` 줄을 추가해야 합니다.
...
두 번째 `" In your inventory you have: "` 줄에 `text.innerText` 문자열을 더해야 합니다. 공백이 중요합니다.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*\+=\s*('|") In your inventory you have: \1/);
```

두 번째 `inventory` 줄 끝에 `text.innerText` 값을 더하려면 연결 연산자를 사용하세요.
