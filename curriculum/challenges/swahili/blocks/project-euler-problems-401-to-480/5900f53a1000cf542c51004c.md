...
Acha `f(k, n)` = $e^\frac{k}{n} - 1$, kwa nambari kamili zisizo hasi zote `k`.

Kwa kushangaza, `f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3.1415926… ≈ π.

Kwa kweli, ni makadirio bora zaidi ya π ya aina `f(a, 200) + f(b, 200) + f(c, 200) + f(d, 200)`.

Acha `almostPi(n)` = a<sup>2</sup> + b<sup>2</sup> + c<sup>2</sup> + d<sup>2</sup> kwa a, b, c, d wanaopunguza makosa: $\lvert f(a,n) + f(b,n) + f(c,n) + f(d,n) - \Pi\rvert$

Umepatiwa `almostPi(200)` = 6<sup>2</sup> + 75<sup>2</sup> + 89<sup>2</sup> + 226<sup>2</sup> = 64658.

# --hints--

`almostPi` inapaswa kuwa kitendakazi.

```js
assert.isFunction(almostPi);
```

`almostPi` inapaswa kurudisha nambari.

```js
assert.strictEqual(typeof almostPi(10), 'number');
```

`almostPi(29)` inapaswa kurudisha `1208`.

```js
assert.strictEqual(almostPi(29), 1208);
```

`almostPi(50)` inapaswa kurudisha `4152`.

```js
assert.strictEqual(almostPi(50), 4152);
```

`almostPi(200)` inapaswa kurudisha `64658`.
