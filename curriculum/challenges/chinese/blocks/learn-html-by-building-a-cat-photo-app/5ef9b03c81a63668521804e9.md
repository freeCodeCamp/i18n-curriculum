...
你的锚元素（`a`）应该有一个闭合标签。 结束标签在 `<` 字符之后有一个 `/`。
...
你的锚元素（`a`）应该有一个 `href` 属性，其值为 `https://www.freecodecamp.org`。 你要么遗漏了属性/值，要么拼写有误。
...
嵌套锚元素（`a`）后，浏览器中唯一可见的 `p` 元素内容应该是 `No Copyright - freeCodeCamp.org`。 仔细检查 `p` 和嵌套锚元素的文本、空格或标点符号。
