...
Para solucionar eso, puedes configurar un oyente de eventos que detectará cuando la canción que está sonando termine. El oyente de eventos `"ended"` es apropiado para esto. Se activa cuando la reproducción de un medio alcanza el final.
...
El oyente de eventos que usaste en tu variable `audio` debería escuchar un evento `"ended"`.
