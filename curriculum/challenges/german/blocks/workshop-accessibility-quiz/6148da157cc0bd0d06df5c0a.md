...
Um die Eingabefelder zueinander auszurichten, erstellen Sie ein neues Regelwerk, das alle `input` und `label` Elemente innerhalb eines `.info` Elements anspricht, und setzen Sie die `display`-Eigenschaft auf `inline-block`.

Richten Sie außerdem den Text des `label`-Elements rechtsbündig aus.

# --hints--

Sie sollten entweder einen `.info > label, .info > input`- oder `.info label, .info input`-Selektor verwenden.
...
Sie sollten den `input`- und `label`-Elementen eine `display` von `inline-block` geben.
...
Sie sollten dem `label`-Element eine `text-align` von `right` geben.
