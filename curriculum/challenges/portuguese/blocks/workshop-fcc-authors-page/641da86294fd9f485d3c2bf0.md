...
Adicione um elemento anchor com um atributo `class` de `"author-link"`, interpolar `url` como o valor para o atributo `href` e definir `target` para `"_blank"`. Para o texto do elemento anchor, interpolar `author` seguido do texto `"'s author page"`. Por exemplo, `"Quincy Larson's author page"`.
...
Seu elemento anchor deve ter a classe `"author-link"`.
...
Você deve definir o atributo `target` do seu elemento anchor para `"_blank"`.

```js
assert.equal(document.querySelector('a')?.getAttribute('target'), '_blank');
```

Você deve interpolar `author` seguido do texto `"'s author page"` dentro do seu elemento anchor.
