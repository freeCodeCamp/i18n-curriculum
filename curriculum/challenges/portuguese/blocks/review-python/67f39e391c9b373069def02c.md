...
- **Declaração de variáveis**: Para declarar uma variável, comece com o nome da variável seguido do operator de atribuição (`=`) e em seguida o valor. Isto pode ser um número, string, boolean, etc. Aqui estão alguns exemplos:
...
A posição inicial representa o índice onde a extração deve começar. A posição de parada é onde o slice deve terminar. Essa posição não é inclusiva. A posição do passo representa o intervalo para incrementar no fatiamento. Aqui estão alguns exemplos:
