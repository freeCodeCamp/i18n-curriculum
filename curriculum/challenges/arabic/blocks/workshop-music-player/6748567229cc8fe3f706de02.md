...
الآن عليك العمل على إيقاف الأغنية التي تُشغل حاليًا مؤقتًا.

عرّف دالة `pauseSong` التي لا تأخذ معلمات. داخل دالتك الجديدة، لتخزين الوقت الحالي للأغنية عند إيقافها مؤقتًا، اضبط الخاصية `songCurrentTime` في كائن `userData` على قيمة `currentTime` من المتغير `audio`.


# --hints--

يجب أن يكون لديك دالة `pauseSong`.

```js
assert.isFunction(pauseSong);
```

يجب أن تضبط دالة `pauseSong` الخاصية `songCurrentTime` في كائن `userData` على قيمة `currentTime` من المتغير `audio`.
