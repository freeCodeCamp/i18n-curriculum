...
同じ補間構文を使って、テキスト `consumedCalories Calories Consumed` を持つ 2 つ目の `p` 要素と、テキスト `exerciseCalories Calories Burned` を持つ 3 つ目の `p` 要素を追加してください。 Remember to replace your `consumedCalories` and `exerciseCalories` variables with the appropriate values.
