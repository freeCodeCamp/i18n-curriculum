...
다음 `input`은 사용자가 가진 모든 슈퍼히어로 능력을 표시하는 체크박스입니다.

`label`와 `className`의 `section`를 가진 `column` 요소를 만들고, 텍스트는 `List your powers (select all that apply):`로 하세요.

# --hints--

`label`와 `className`의 `section`를 가진 `column` 요소를 만들어야 합니다.
...
`label` 요소에는 `List your powers (select all that apply):` 텍스트가 있어야 합니다.
