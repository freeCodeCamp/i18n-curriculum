...
Sempre que você clicar no botão de play, a primeira música da sua playlist sempre será reproduzida.Para corrigir isso, dentro da função arrow do event listener, substitua `playSong(0)` por uma declaração `if` que verifica se `userData.currentSong` é `null`.

Dentro do bloco `if`, chame a função `playSong` com o `id` da primeira música no array `userData.songs`.

# --hints--

Você deve modificar o callback do event listener para chamar a função `playSong` com o `id` da primeira música no array `userData.songs` quando `userData.currentSong` for `null`.
