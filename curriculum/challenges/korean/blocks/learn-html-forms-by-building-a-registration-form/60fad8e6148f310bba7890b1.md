...
`h1`와 `p` 요소에 `margin` 값을 가진 `1em auto`를 지정하여 가운데 정렬하세요. 그런 다음, `center` 안의 텍스트도 정렬하세요.

# --hints--

쉼표로 구분된 요소 선택자를 사용해 `h1`와 `p` 요소를 스타일링하세요.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

`margin`와 `1em auto` 요소를 가운데 정렬하려면 `h1` 값을 가진 `p`을 사용하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

`text-align`와 `center` 텍스트를 가운데 정렬하려면 `h1` 값을 가진 `p`을 사용하세요.
