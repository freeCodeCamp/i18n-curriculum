...
最後，你應該再次呈現歌曲，更新播放按鈕的可訪問文本，並從播放列表中刪除重置按鈕。 你還需要從 DOM 中刪除 `resetButton`。

調用 `renderSongs()` 函數並以 `sortSongs()` 作爲參數，再次按字母順序呈現歌曲。

調用 `setPlayButtonAccessibleText()` 函數來更新播放按鈕的可訪問文本。

通過調用 `resetButton` 變量上的 `remove()` 方法從播放列表中刪除重置按鈕。

**注意**：現在你可以嘗試刪除所有歌曲，看看會發生什麼。
...
你應該使用 `sortSongs()` 調用 `renderSongs` 函數。
...
你應該使用 `remove()` 方法從 DOM 中刪除 `resetButton`。
