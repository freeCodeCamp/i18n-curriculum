...
リンクが実際に訪問されたときにリンクのプロパティを変更するには、`a:visited { propertyName: propertyValue; }`のような<dfn>疑似セレクター</dfn>を使います。

ユーザーがリンクを訪問したときに、フッターの`Visit our website`リンクの色を`grey`に変更してください。

# --hints--

`a:visited`疑似セレクターを使うべきです。
...
`a:visited`は`color`の`grey`を持つべきです。
