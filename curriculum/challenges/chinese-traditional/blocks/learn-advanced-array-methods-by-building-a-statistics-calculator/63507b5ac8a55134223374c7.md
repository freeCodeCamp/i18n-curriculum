...
首先，`calculate` 函式需要找到在 `#numbers` 輸入欄中輸入的數字。為此，使用 `.querySelector` 來定位輸入欄，然後使用 `.value` 屬性取得輸入的數字。

將此儲存在 `value` 變數中。

# --hints--

你的 `calculate` 函式（程式）應該有一個 `value` 變數。

```js
assert.match(calculate.toString(), /value/);
```

你的 `calculate` 函式（程式）應該以 `"#numbers"` 作為引數來呼叫 `document.querySelector()`。

```js
assert.match(calculate.toString(), /document\.querySelector\(\s*("|')#numbers\1\s*\)/)
```

你應該存取 `document.querySelector()` 呼叫的 `value` 屬性。

```js
assert.match(calculate.toString(), /document\.querySelector\(\s*("|')#numbers\1\s*\)\.value/);
```

你應該將 `value` 屬性指定到你的 `value` 變數。
