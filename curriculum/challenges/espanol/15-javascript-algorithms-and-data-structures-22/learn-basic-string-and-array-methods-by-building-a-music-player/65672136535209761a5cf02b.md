...
Para reproducir la canción cada vez que el usuario hace clic en ella, agrega un atributo `onclick` al primer elemento de botón. Dentro del `onclick`, llama a la función `playSong` con `song.id`.

No olvides que necesitas interpolar con el signo de dólar aquí.

# --hints--

Debes agregar un atributo `onclick` con el valor `playSong(${song.id})` al elemento `button`.
