...
Image URL `value` に `player.imageUrl` の `input` 属性を追加してください。次に、`onChange` 属性を、`e` をパラメータに取り、`setPlayer` をスプレッドし、`player` を `imageUrl` に設定して `e.target.value` を呼び出すアロー関数に設定してください。

# --hints--

Image URL `input` はプレイヤーの画像URLと等しい `value` を持つ必要があります。
...
Image URL `input` は `onChange` を呼び出し、`setPlayer` をスプレッドし、`player` を `imageUrl` に設定する `e.target.value` ハンドラを持つ必要があります。
