...
Declara otra variable llamada `foundCard` y asígnale el resultado de llamar al método `find()` sobre `cards`. Pasa `card` como parámetro callback y compara `card.name_short` con `cardId` para encontrar la carta que coincida usando igualdad estricta.

# --hints--

Debes declarar una variable llamada `foundCard` dentro de `showFortune`.
...
Tu `foundCard` debe asignarse llamando a `find` sobre `this.cards` con el callback correcto.
...
El callback `find` debe tener un parámetro llamado `card`.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

Dentro del callback, `card.name_short` debe compararse con `cardId`.
