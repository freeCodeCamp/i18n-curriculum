...
Тепер настав час додати функціональність скидання до `resetButton`. Це поверне пісні в список відтворення коли натиснуто.

Додайте слухач події клацання до змінної `resetButton`. Передайте зворотний виклик використовуючи стрілковий синтаксис та поки що залиште його порожнім.

# --hints--

Використайте метод `addEventListener()` на `resetButton`.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

Слухач подій `resetButton` повинен слухати по подію `"click"`.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

Функція зворотного виклику слухача подій має використовувати стрілковий синтаксис та мати порожню пару фігурних дужок.
