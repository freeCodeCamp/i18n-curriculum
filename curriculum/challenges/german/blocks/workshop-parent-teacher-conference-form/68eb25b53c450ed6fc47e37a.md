...
Standardmäßig sind `label`-Elemente inline. Um sie vertikal anzuordnen (außer bei Ihren Optionsschaltflächen-Bezeichnungen), verwenden Sie die `:not()`-Pseudoklasse.

Die `:not()`-Pseudoklasse negiert eine Auswahl. Hier wählt sie alle `label`-Elemente aus, die nicht die Klasse `contact-method` haben.

Wählen Sie `label:not(.contact-method)` aus, und setzen Sie dann `display` auf `block` und `margin` auf `10px 0`.

# --hints--

Sie sollten einen `label:not(.contact-method)`-Selektor haben.
...
Setzen Sie die `margin`-Eigenschaft auf `10px 0`.
