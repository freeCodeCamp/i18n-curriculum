...
當 `inventory` 長度不超過一時，使用 `else` 語句運行。 將 `text.innerText` 設置爲 `"Don't sell your only weapon!"`。

# --hints--

你的 `sellWeapon` 函數應該有一個 `else` 語句。

```js
assert.match(sellWeapon.toString(), /else/);
```

你應該將 `text.innerText` 設置爲 `"Don't sell your only weapon!"`。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")Don't sell your only weapon!\1/);
```

你的新 `text.innerText` 行應該位於你的 `else` 語句中。
