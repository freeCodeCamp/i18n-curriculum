...
Ongeza msikilizaji mwingine wa tukio kwa kitu cha `document` cha ulimwengu wote kinachofanya wito kwa kitendakazi cha `showFortune`. Ndani ya wito wa kurudisha, jumuisha kigezo cha `e` cha aina ya `Event`, na pita `e` kama hoja kwa njia ya `showFortune`.

`showFortune` itashughulikia matukio ya kubofya kadi, kama vile kusoma kitambulisho cha kadi iliyobofyiwa kutoka DOM na kuonyesha maelezo ya bahati ya kadi inayolingana. Utaifanya katika hatua zinazofuata.

# --hints--

Unapaswa kuweka msikilizaji wa tukio wa `click` kwa `document` ya ulimwengu wote.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Msikilizaji wako wa tukio unapaswa kuwa na kigezo cha wito wa kurudisha kinachoitwa `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

Kigezo chako cha wito wa kurudisha kinapaswa kuwa na aina ya `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

Ndani ya wito wa kurudisha, unapaswa kuita kitendakazi cha `showFortune`

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

Ndani ya kitendakazi cha `showFortune`, unapaswa kupita hoja ya `e`.
