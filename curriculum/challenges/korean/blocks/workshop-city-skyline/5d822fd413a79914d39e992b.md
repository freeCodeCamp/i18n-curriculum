...
`--building-color1` 변수를 `background-color`의 `.fb4b` 속성 값으로 추가하세요. 그런 다음 `background-color`에서 `.fb4`을 제거하세요.

# --hints--

`background-color`에서 `.fb4`를 제거해야 합니다.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb4")?.backgroundColor);
```

`.fb4b`에 `background-color` 값을 가진 `--building-color1`를 지정해야 합니다.
