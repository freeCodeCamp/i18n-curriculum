...
最後的變更將會是在頁腳內的電子郵件文字。在之前的課程和工作坊中，你已經學會如何使用像這樣的 `mailto` 聯結：

```html
<p>Email me at <a href="mailto:janedoe@email.com">janedoe@email.com</a></p>
```

首先將 `camperbot@blog.io` 電子郵件位址包裹在錨點元素中。然後，為錨點元素添加 `href` 屬性並將其設定為 `mailto:camperbot@blog.io`。

透過最後的變更，你已成功決議部落格頁面中的所有問題！

# --hints--

你的 `camperbot@blog.io` 電子郵件位址應該包裹在一個錨點元素內。
...
你的錨點元素應該有一個 `href` 屬性，值為 `mailto:camperbot@blog.io`。
