...
`On another note` is used to introduce a new topic in a conversation. Por ejemplo:

`On another note, have you seen the new project updates?` – This means the speaker is changing the subject.

`To touch base on something` means to make contact or have a brief discussion about a topic. Por ejemplo:
