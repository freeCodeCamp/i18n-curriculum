...
Agrega otro degradado repetido a este edificio; hazlo igual que el que acabas de agregar, excepto que no agregues la dirección `90deg` y utiliza tu color de ventana en lugar de los dos colores `transparent`.

# --hints--

Deberías darle a `.fb6` un segundo `repeating-linear-gradient` en la propiedad `background`.
...
Le debes dar al segundo `repeating-linear-gradient` un primer color de `--building-color3` desde el `0%` hasta el `10%`.
...
Le debes dar al segundo `repeating-linear-gradient` un segundo color de `--window-color3` desde el `10%` hasta el `30%`.
