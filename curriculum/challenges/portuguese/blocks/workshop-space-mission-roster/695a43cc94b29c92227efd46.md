...
Você adicionou uma chamada `console.log()` ao seu script para validar seu array `EVAChunks`, e agora a chamada não é mais necessária. Para manter o terminal limpo para os próximos passos, exclua o bloco de `for loop` que registra dados de `EVAChunks`.

# --hints--

Você deve remover o bloco de `for loop` que registra dados de `EVAChunks`. Exclua o bloco inteiro contendo ambos os loops.
