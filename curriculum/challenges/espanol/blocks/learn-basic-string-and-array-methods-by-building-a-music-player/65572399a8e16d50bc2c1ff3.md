...
Si hay una coincidencia, establece `null` en `userData.currentSong` y `0` en `userData.songCurrentTime`. 

Después de eso, llama a la función `pauseSong()` para detener la reproducción y la función `setPlayerDisplay()` para actualizar la visualización del reproductor.
