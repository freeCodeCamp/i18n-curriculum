...
`async`/`await`，基于 promises，使编写和读取异步的代码更容易。当你在函数前加上 `async` 关键字时，表示该函数总是返回一个 `Promise`。`await` 关键字允许你等待一个 `Promise` 解析后再执行下一行代码，它只能在异步函数内部或模块的顶层主体中使用。下面是一个示例来说明 `async`/`await` 的工作原理：
