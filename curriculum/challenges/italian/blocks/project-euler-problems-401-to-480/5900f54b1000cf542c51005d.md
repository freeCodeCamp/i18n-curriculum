...
Sia $S(n) = \displaystyle\sum_{p = 1}^n \sum_{k = 1}^n {(a_k + b_k)}^p {(b_k + c_k)}^p {(c_k + a_k)}^p$ per tutti i numeri interi $p$, $k$ tali che $1 ≤ p, k ≤ n$.

Curiosamente, $S(n)$ è sempre un numero intero. Per esempio, $S(4) = 51\\,160$.
