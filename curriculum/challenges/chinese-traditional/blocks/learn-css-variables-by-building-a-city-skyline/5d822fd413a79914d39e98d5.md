...
將 `.bb1` 元素轉換成彈性盒子（flexbox）父元素來居中你建築物。 使用 `flex-direction` 和 `align-items` 屬性來居中子項。
...
你應該給 `.bb1` 元素添加一個設置爲 `flex` 的 `display`。
...
你應該給 `.bb1` 元素添加一個設置爲 `column` 的 `flex-direction`。
...
你應該給 `.bb1` 元素添加一個設置爲 `center` 的 `align-items`。
