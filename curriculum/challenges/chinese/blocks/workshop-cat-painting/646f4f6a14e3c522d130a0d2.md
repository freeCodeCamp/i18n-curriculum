...
使用类选择器来定位 `.cat-whisker-right-middle` 元素，然后通过设置 `position` 为 `absolute`、`top` 为 `127px` 以及 `left` 为 `109px` 来将右中间的胡须移动到正确的位置。

# --hints--

你应该有一个 `.cat-whisker-right-middle` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle'));
```

你的 `.cat-whisker-right-middle` 选择器应该有一个 `position` 属性，设置为 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.position, 'absolute');
```

你的 `.cat-whisker-right-middle` 选择器应该有一个 `top` 属性，设置为 `127px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.top, '127px');
```

你的 `.cat-whisker-right-middle` 选择器应有一个 `left` 属性，设置为 `109px`。
