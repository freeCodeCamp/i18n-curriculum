...
当处理属性名称和值相同的对象时，可以使用简写属性名称语法。 如果属性值与属性名称相同，则此语法允许你省略属性值。
...
在构造函数中，添加 `this.position` 并为其分配一个具有 `x` 和 `y` 坐标的对象。 确保使用简写属性语法。

# --hints--

你应该有一个 `this.position` 属性。

```js
assert.match(Platform.toString(), /this\.position\s*;?/);
```

`this.position` 属性应该是一个具有 `x` 和 `y` 坐标的对象。 确保使用简写属性语法。
