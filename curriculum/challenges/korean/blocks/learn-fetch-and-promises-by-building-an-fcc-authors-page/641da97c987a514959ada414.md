...
이제 이전과 마찬가지로 author 데이터의 일부를 사용해 `displayAuthors` 함수를 호출하세요.

함수를 호출한 후 `Load More Authors` 버튼을 클릭해도 작동하지 않습니다. 그 이유는 버튼에 `click` 이벤트 리스너를 아직 추가하지 않았기 때문입니다. 다음에 그 작업을 하세요.
...
`slice()` 메서드의 첫 번째 인수는 `startingIndex` 이어야 합니다.
...
`slice()` 메서드의 두 번째 인수는 `endingIndex` 이어야 합니다.
