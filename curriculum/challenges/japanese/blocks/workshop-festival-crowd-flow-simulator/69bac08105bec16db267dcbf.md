...
次に、次のメッセージをコンソールに記録してください：`gate.queue[tickIndex] + " attendees arriving."`。

# --hints--

あなたの`handleGateAtTick`関数は、次のメッセージをコンソールに記録する必要があります：`gate.queue[tickIndex] + " attendees arriving."`。
...
あなたの`handleGateAtTick`関数は、指定されたtickIndexのゲートのキューに基づいて正しい参加者数を記録する必要があります。
