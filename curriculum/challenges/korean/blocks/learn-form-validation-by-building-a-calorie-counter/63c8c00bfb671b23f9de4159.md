...
입력이 유효한지 확인해야 합니다. `invalidInputMatch` 변수를 선언하고, `isInvalidInput`를 인수로 하여 `currVal` 함수를 호출한 결과를 할당하세요.

# --hints--

`invalidInputMatch` 변수를 선언해야 합니다.

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=/);
```

`isInvalidInput` 변수에 `invalidInputMatch`를 호출한 결과를 할당해야 합니다.

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(/);
```

`currVal`에 `isInvalidInput`를 인수로 전달해야 합니다.
