...
你的表單需要某個地方來顯示結果。 添加一個空的 `div` 元素併爲其指定 `id` `output`，以及 `class` 值 `output` 和 `hide`。

# --hints--

你應該在 `form` 後添加一個 `div` 元素。

```js
assert.exists(document.querySelector('.container > div'));
```

你的新 `div` 元素應將 `id` 設置爲 `output`。

```js
assert.equal(document.querySelector('.container > div')?.id, 'output');
```

你的新 `div` 元素應具有 `output` 和 `hide` 的 `class` 值。
