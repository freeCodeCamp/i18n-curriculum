...
Jetzt ist es an der Zeit, die Ereignis-Listener hinzuzufügen, die für den Aufruf der `movePlayer`-Funktion verantwortlich sein werden.

Beginne, indem du ein `addEventListener` zum globalen `window`-Objekt hinzufügst.
...
Du musst dem globalen `window`-Objekt ein `addEventListener` hinzufügen.
