...
次に、クラス名が `img-loader` の `div` を追加してください。その後、`img` 要素を作成し、画像のURLを ``${img ? `${CDN_URL}/${img}` : LOCAL_DEFAULT_IMG}`` を使って条件付きで設定してください。

# --hints--

`div`要素の中に`figure`要素があるはずです。

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

`div`にはクラス`img-loader`が付いているはずです。

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

`img`要素の中に`figure`要素があるはずです。

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

`img`要素は、`CDN_URL`が提供されている場合は`img`と`src`を`img`として使い、提供されていない場合は`LOCAL_DEFAULT_IMG`にフォールバックします。
...
`img`要素にはクラス`card-img`と`hidden`が付いているはずです。
