...
在 `startGame` 函數中，你需要顯示 `canvas` 元素並隱藏 `startScreen` 容器。

使用 `canvas.style.display` 將顯示值更改爲 `"block"`。

下面，使用 `startScreen.style.display` 將顯示值更改爲 `"none"`。

# --hints--

你應該使用點符號來訪問 `canvas` 的 `style` 屬性。
...
你應該將 `canvas` `display` 屬性設置爲 `"block"`。

```js
assert.match(startGame.toString(), /canvas\.style\.display\s*=\s*('|")block\1/);
```

你應該使用點符號來訪問 `startScreen` 的 `style` 屬性。
...
你應該將 `startScreen` `display` 屬性設置爲 `"none"`。
