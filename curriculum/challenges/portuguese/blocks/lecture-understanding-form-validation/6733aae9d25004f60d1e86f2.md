...
Estamos usando o `input` de e-mail que já vem com validação embutida para verificar validações básicas, como se a entrada inclui o sinal de arroba (`@`).
...
Ele informa o estado inválido imediatamente, em vez de esperar que enviemos o formulário. Mas ainda está usando a mensagem padrão. Isso acontece porque o método `reportValidity()` apenas avisa ao navegador que a entrada é inválida. O navegador ainda escolhe como mostrar o motivo da invalidez. É aí que o método `setCustomValidity()` entra.
...
Existem várias propriedades úteis que todas possuem um valor booleano de `true` ou `false`.

Algumas dessas propriedades úteis que você pode explorar por conta própria são a propriedade `valueMissing`, que é `true` quando um campo de entrada obrigatório fica vazio, ou a propriedade `patternMismatch`, que é `true` se o valor não corresponder ao padrão especificado pela expressão regular.
