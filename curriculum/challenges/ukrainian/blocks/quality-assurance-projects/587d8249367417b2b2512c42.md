...
Створіть повноцінний застосунок на JavaScript, функціонально схожий на цей: <a href="https://issue-tracker.freecodecamp.rocks/" target="_blank" rel="noopener noreferrer nofollow">https://issue-tracker.freecodecamp.rocks/</a>. Робота над цим проєктом передбачає написання коду з використанням одного з наступних методів:

-   Клонуйте <a href="https://github.com/freeCodeCamp/boilerplate-project-issuetracker/" target="_blank" rel="noopener noreferrer nofollow">цей репозиторій GitHub</a> і завершіть проєкт локально.
-   Використайте конструктор сайту на власний розсуд, щоб завершити проєкт. Переконайтеся, що ви зберегли всі файли з нашого репозиторію GitHub.
...
Надайте власний проєкт, а не посилання на приклад.

```js
  assert(!/.*\/issue-tracker\.freecodecamp\.rocks/.test(code));
```

Ви можете надіслати запит `POST` до `/api/issues/{projectname}` із даними форми, що містять обов’язкові поля `issue_title`, `issue_text`, `created_by` і, за бажанням, `assigned_to` та `status_text`.
...
Якщо ви надішлете запит `POST` до `/api/issues/{projectname}` без необхідних полів, то повернеться помилка `{ error: 'required field(s) missing' }`.
...
Якщо надісланий запит `PUT` до `/api/issues/{projectname}` не містить `_id`, то поверненим значенням буде `{ error: 'missing _id' }`.
...
Якщо запит `PUT`, надісланий до `/api/issues/{projectname}`, не містить оновлювальні поля, поверненим значенням буде `{ error: 'no update field(s) sent', '_id': _id }`. При іншій помилці поверненим значенням буде `{ error: 'could not update', '_id': _id }`.
...
Ви можете надіслати запит `DELETE` до `/api/issues/{projectname}` із `_id`, щоб видалити проблему. Якщо `_id` не надіслано, то поверненим значенням буде `{ error: 'missing _id' }`. Якщо все успішно, поверненим значенням буде `{ result: 'successfully deleted', '_id': _id }`. При помилці поверненим значенням буде `{ error: 'could not delete', '_id': _id }`.
