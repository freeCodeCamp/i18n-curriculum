...
`random` 속성을 만드세요. 이 속성은 배열의 처음 두 숫자를 사용해 무작위 정수를 생성해야 합니다. 이 숫자의 범위는 두 숫자 중 첫 번째 숫자(포함)부터 두 숫자의 합 바로 전까지입니다. 계산에는 `Math.floor()`과 `Math.random()` 메서드를 사용하세요.

# --hints--

`spreadsheetFunctions` 객체에 `random` 속성이 있어야 합니다.
...
`random` 함수는 배열의 처음 두 숫자 사이에서 무작위 숫자를 반환해야 합니다.
...
`random` 함수는 정수(whole number)를 반환해야 합니다.
