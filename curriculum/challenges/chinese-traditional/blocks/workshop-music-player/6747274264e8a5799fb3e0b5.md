...
你的 `allSongs` 陣列應該有兩個值。

```js
assert.equal(allSongs.length, 2);
```

你的 `allSongs` 陣列的第二個值應該是一個物件。

```js
assert.isObject(allSongs[1]);
```

你的 `allSongs` 陣列中的第二個物件應該有一個 `id` 屬性，並設定為數字 `1`。

```js
assert.strictEqual(allSongs[1].id, 1);
```

你 `allSongs` 陣列中的第二個物件應該有一個 `title` 屬性，其值設定為字串 `"Can't Stay Down"`。

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

你 `allSongs` 陣列中的第二個物件應該有一個 `artist` 屬性，其值設定為字串 `"Quincy Larson"`。

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

你的 `allSongs` 陣列中的第二個物件應該有一個 `duration` 屬性，其值設定為字串 `"4:15"`。

```js
assert.equal(allSongs[1].duration, "4:15");
```

你 `allSongs` 陣列中的第二個物件應該有一個 `src` 屬性，該屬性設定為字串 `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`。
