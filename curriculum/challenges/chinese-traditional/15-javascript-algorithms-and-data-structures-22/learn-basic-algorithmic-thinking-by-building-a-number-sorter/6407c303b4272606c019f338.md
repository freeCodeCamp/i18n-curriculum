...
你需要一個函數來更新顯示排序的數字。 首先使用箭頭語法聲明一個採用單個 `array` 參數的 `updateUI` 函數。

因爲你將編寫不會立即具有返回值的算法，所以將 `array` 的後備值設置爲空數組。 以下是具有備用值的函數示例：
...
`updateUI` 應該接受一個 `array` 參數。

```js
assert.match(code, /const\s+updateUI\s*=\s*\(?\s*array/);
```

`updateUI` 應該爲 `array` 設置一個空數組作爲默認值。
