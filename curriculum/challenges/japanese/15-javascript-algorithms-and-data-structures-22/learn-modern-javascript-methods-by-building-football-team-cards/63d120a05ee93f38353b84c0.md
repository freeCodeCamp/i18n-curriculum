...
`headCoach` プロパティの下に、`players` という名前のキーと空の配列の値を持つプロパティを追加してください。

# --hints--

`myFavoriteFootballTeam` オブジェクトに `players` プロパティを追加してください。

```js
assert.property(myFavoriteFootballTeam, 'players');
```

`players` の値は配列にしてください。

```js
assert.isArray(myFavoriteFootballTeam.players)
```

配列 `players` は空にしてください。
