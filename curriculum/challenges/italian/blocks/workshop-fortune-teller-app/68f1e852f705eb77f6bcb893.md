...
Dichiara una variabile chiamata `cardId` e assegnale il valore dell'attributo `data-id` da `cardElement` usando il metodo `getAttribute()`.

# --hints--

Dovresti dichiarare una variabile chiamata `cardId` all'interno di `showFortune`.
...
La tua variabile `cardId` dovrebbe essere assegnata chiamando `cardElement.getAttribute("data-id")`.
...
Dovresti usare il metodo `getAttribute` su `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

Dovresti passare `'data-id'` a `getAttribute`.
