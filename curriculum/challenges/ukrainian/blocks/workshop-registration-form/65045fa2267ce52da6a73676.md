...
Дотримуйтесь найкращих практик доступності, пов’язуючи елементи `input` та елементи `label` у другому `fieldset`.

Використовуйте `personal-account` і `business-account` як значення для відповідних атрибутів `id`.

# --hints--

Перший елемент `input` має мати `id` `personal-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[0]?.matches('#personal-account'))
```

Другий елемент `input` має мати `id` `business-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) input')?.[1]?.matches('#business-account'))
```

Перший елемент `label` має мати атрибут `for` зі значенням `personal-account`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(2) label')?.[0]?.matches('label[for="personal-account"]'))
```

Другий елемент `label` має мати атрибут `for` зі значенням `business-account`.
