...
`html` 선택자를 만들고 `font-size` 속성을 `62.5%`로 설정하세요. 이렇게 하면 웹 페이지의 기본 폰트 크기가 10px로 설정됩니다(브라우저 기본값은 16px입니다).

이렇게 하면 나중에 `rem` 단위를 다루기 더 쉬워집니다. `2rem`는 20px가 되기 때문입니다.
...
`html` 선택자에 `font-size` 속성을 `62.5%`로 설정하세요.
