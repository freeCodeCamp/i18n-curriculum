...
あなたの`conversionTable`に`cup`プロパティを付けて、`gram`プロパティが`240`に設定されているオブジェクト、`ounce`プロパティが`8.0`に設定されているオブジェクト、`teaspoon`プロパティが`48`に設定されているオブジェクトを代入してください。

# --hints--

あなたの`conversionTable`には`cup`プロパティが必要です。

```js
assert.property(conversionTable, "cup");
```

あなたの`cup`プロパティはオブジェクトである必要があります。

```js
assert.isObject(conversionTable.cup);
```

あなたの`cup`オブジェクトには`gram`プロパティが必要です。

```js
assert.property(conversionTable.cup, "gram");
```

あなたの`gram`プロパティは`240`に設定してください。

```js
assert.strictEqual(conversionTable.cup?.gram, 240);
```

あなたの`cup`オブジェクトには`ounce`プロパティが必要です。

```js
assert.property(conversionTable.cup, "ounce");
```

あなたの`ounce`プロパティは`8.0`に設定してください。

```js
assert.strictEqual(conversionTable.cup?.ounce, 8.0);
```

あなたの`cup`オブジェクトには`teaspoon`プロパティが必要です。

```js
assert.property(conversionTable.cup, "teaspoon");
```

あなたの`teaspoon`プロパティは`48`に設定してください。
