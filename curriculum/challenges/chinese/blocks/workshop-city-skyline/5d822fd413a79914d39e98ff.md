...
接下来，从 `.bb2a` 中删除 `width` 和 `height`，并将 `border-left` 和 `border-right` 更改为使用 `5vw` 而不是 `1vw`。 该元素现在的大小将是零，并且边框将在中间聚集在一起。
