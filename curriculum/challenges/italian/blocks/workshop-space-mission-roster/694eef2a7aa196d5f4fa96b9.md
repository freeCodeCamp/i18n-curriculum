...
Invoca `getEVAReadyCrew()` con il tuo roster `updatedSquad` e archivia il risultato in una nuova variabile chiamata `EVAReadySquad`. Poi, usa un loop `for` per registrare il `name` di ogni astronauta nell'array `EVAReadySquad`.

# --hints--

Dovresti chiamare `getEVAReadyCrew()` con `updatedSquad` e archiviare il risultato in una nuova variabile chiamata `EVAReadySquad`.
...
Dovresti usare un loop `for` per iterare su l'array `EVAReadySquad` e registrare il `name` di ogni astronauta.
