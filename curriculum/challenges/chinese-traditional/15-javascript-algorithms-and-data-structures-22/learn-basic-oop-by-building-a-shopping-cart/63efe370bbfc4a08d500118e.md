...
你現在有一個小錯誤。 當你嘗試訪問一個對象的屬性而該屬性不存在時，你會得到 `undefined`。 這意味着如果甜點在 `totalCountPerProduct` 對象中不存在，你會嘗試將 `1` 添加到 `undefined`，這將導致 `NaN`。

要解決這個問題，可以使用 `||` 運算符，如果不存在則將值設爲 `0`。 將右邊的 `totalCountPerProduct[dessert.id]` 括在括號中，並在表達式末尾添加 `|| 0`。

# --hints--

你應該將右邊的 `totalCountPerProduct[dessert.id]` 括在括號中。
