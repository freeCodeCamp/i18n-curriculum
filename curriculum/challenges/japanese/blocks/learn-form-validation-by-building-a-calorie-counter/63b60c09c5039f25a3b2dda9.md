...
もう一つの `div` 要素を作成してください。その中に `button` をネストして `submit` フォームを作成します。このボタンには `Calculate Remaining Calories` というテキストを付けてください。

次に、`button` を追加し、`id` を `clear` に設定してフォームをクリアできるようにします（フォームの送信を防ぐ `type` 属性を付けるのを忘れないでください）。このボタンには `Clear` というテキストが必要です。
