...
`img` 선택자를 만들고 `width` 속성을 `100%`로, `object-fit` 속성을 `cover`로 설정하세요.

`object-fit` 속성은 브라우저가 요소를 컨테이너 내에서 어떻게 배치할지 알려줍니다. 이 경우 `cover`는 이미지가 컨테이너를 채우도록 설정하며, 종횡비를 변경하지 않도록 필요에 따라 자릅니다.
...
`img` 선택자에 `object-fit` 속성을 `cover`로 설정해야 합니다.
