...
Фінальне завдання у вашій функції `handleGateAtTick` — обробити будь-яке переповнення для поточного тіку.

Якщо існує будь-яке переповнення (`result.overflow > 0`), ви повинні:

- Зареєструвати наступне повідомлення `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
- Перенаправити переповнення, викликавши вашу функцію `rerouteOverflow()` з `gates`, `gate`, `tickIndex` та `result.overflow`.

# --hints--

Якщо існує будь-яке переповнення (`result.overflow > 0`), спочатку слід зареєструвати повідомлення `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
...
Якщо існує будь-яке переповнення (`result.overflow > 0`), слід перенаправити переповнення, викликавши `rerouteOverflow()` з `gates`, `gate`, `tickIndex` та `result.overflow`.
