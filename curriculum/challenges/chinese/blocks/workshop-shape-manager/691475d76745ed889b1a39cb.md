...
创建一个 `clearInputFields` 函数。在其中，使用 `querySelectorAll` 获取所有输入字段，并用 `forEach()` 对它们进行循环。在 `forEach()` 内，将每个 `input` 的值设置为空字串。

# --hints--

你应该创建一个 `clearInputFields` 函数。

```js
assert.isFunction(clearInputFields)
```

你的 `clearInputFields` 函数应该将每个输入字段设置为空字串。
