...
Оскільки застосунок fortune має лише три моменти: минуле, теперішнє та майбутнє, оголосіть enum з назвою `DrawingType` з трьома властивостями: `Past`, `Present` та `Future`. Кожна властивість має мати рядкове значення, що відповідає її імені.

# --hints--

Ви повинні мати enum з назвою `enum` з іменем `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Ваш enum має мати властивість `Past` з рядковим значенням `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Ваш enum має мати властивість `Present` з рядковим значенням `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Ваш enum має мати властивість `Future` з рядковим значенням `future`.
