...
Dovresi usare il selettore `.foot.left`.

```js
assert.match(code, /\.foot\.left\s*\{/);
```

Dovresti dare a `.foot.left` una proprietà `left` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot.left')?.left, '25%');
```

Dovresi usare il selettore `.foot.right`.

```js
assert.match(code, /\.foot\.right\s*\{/);
```

Dovresti dare a `.foot.right` una proprietà `right` di `--fcc-expected--` invece di `--fcc-actual--`.
