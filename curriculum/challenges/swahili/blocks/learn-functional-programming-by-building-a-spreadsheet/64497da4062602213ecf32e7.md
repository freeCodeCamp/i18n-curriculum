...
Kwa sababu tukio la `change` linaanzishwa kwenye kipengele cha `input`, kipengele hicho kitakuwa na sifa ya `value` inayowakilisha thamani ya sasa ya ingizo.

Weka thamani ya sifa ya `value` ya `element` kwenye kigezo kipya kinachoitwa `value`, na tumia `.replace()` kuondoa nafasi zote tupu.

# --hints--

Unapaswa kutangaza kigezo cha `value` baada ya kigezo chako cha `element`.

```js
assert.match(code, /const\s+update\s*=\s*(\(\s*event\s*\)|event)\s*=>\s*\{\s*const\s+element\s*=\s*event\.target\s*;?\s*(?:const|let|var)\s+value/);
```

Unapaswa kutumia `const` kutangaza kigezo chako cha `value`.

```js
assert.match(code, /const\s+update\s*=\s*(\(\s*event\s*\)|event)\s*=>\s*\{\s*const\s+element\s*=\s*event\.target\s*;?\s*const\s+value/);
```

Unapaswa kuweka thamani ya sifa ya `value` ya `element` kwenye kigezo chako cha `value`.
...
Unapaswa kuita njia ya `.replace()` kwenye sifa ya `value` ya `element`.
...
Unapaswa kupitisha usemi wa kawaida unaolingana na nafasi tupu kwenye njia yako ya `.replace()`. Tumia darasa la herufi la `\s`.
...
Unapaswa kufanya usemi wako wa kawaida kuwa wa kimataifa (global).
...
Unapaswa kupitisha mfuatano wa herufi tupu kama hoja yako ya pili kwenye njia ya `.replace()`.
