...
클래스를 사용하면 여러 HTML 요소에 동일한 CSS 스타일을 적용할 수 있습니다. 첫 번째 `red-text` 요소에 `p` 클래스를 적용해 확인하세요.
...
첫 번째 `p` 요소에는 `red-text` 클래스가 있어야 합니다.
