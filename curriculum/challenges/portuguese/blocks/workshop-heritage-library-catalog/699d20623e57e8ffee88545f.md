...
O log `byDecade` cumpriu seu propósito. Remova `console.log(byDecade)` do seu código.

Depois, para facilitar a visualização das entradas do catálogo, crie uma função chamada `renderEntry` que recebe `entry` como parâmetro. Deixe o corpo da função vazio por enquanto. Abaixo da função, faça um log de `renderEntry(catalog[0])`.

# --hints--

`renderEntry` deve ser uma função.

```js
assert.isFunction(renderEntry);
```

`renderEntry` deve ser uma função com um parâmetro `entry`.
...
A chamada `console.log(byDecade)` deve ser removida.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*byDecade\s*\)/);
```

Você deve fazer um log de `renderEntry(catalog[0])`.
