...
Erstellen Sie nach Ihrem `h2`-Element zwei `p`-Elemente. Geben Sie dem ersten eine `class` von `dessert-price` und setzen Sie den Text auf ein Dollarzeichen `"$"` gefolgt vom Wert der `price`-Variablen. Geben Sie dem zweiten eine `class` von `product-category` und den Text `"Category: "` gefolgt vom Wert der `category`-Variablen.

# --hints--

Sie sollten zwei `p`-Elemente erstellen.
...
Ihre `p`-Elemente sollten nach Ihrem `h2`-Element kommen.
...
Ihr erstes `p`-Element sollte eine `class` von `dessert-price` haben.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Ihr erstes `p`-Element sollte das Dollarzeichen `"$"` gefolgt vom Wert der `price`-Variablen haben.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Ihr zweites `p`-Element sollte eine `class` von `product-category` haben.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Ihr zweites `p`-Element sollte den Text `"Category: "` gefolgt vom Wert der `category`-Variablen haben.
