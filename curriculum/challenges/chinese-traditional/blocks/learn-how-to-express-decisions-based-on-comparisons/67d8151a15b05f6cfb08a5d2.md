...
`That way` is used to explain the result or benefit of a decision or action. 例如：
...
`While` is used to indicate that two things are happening at the same time. 例如：
