...
最後，使用字符串插值將段落元素的文本設置爲 `decimalToBinary(${currVal})`，其中 `currVal` 是當前對象的 `inputVal` 屬性。 此後，通過在數字輸入中輸入數字 `5` 並單擊 `Convert` 按鈕來測試你的代碼。

# --hints--

你應該使用字符串插值將段落元素的文本設置爲 `decimalToBinary(${obj.inputVal})`。
