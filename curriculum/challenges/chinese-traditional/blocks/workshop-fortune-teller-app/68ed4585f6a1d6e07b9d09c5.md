...
在 `fetchCardsData` 方法內，宣告一個名為 `response` 的變數，並透過呼叫 `fetch` 函式，使用以下 URL 指定其值：`` `${CDN_URL}/card_data.json` ``。

在 `await` 呼叫前加上 `fetch` 關鍵字，並將整個邏輯包裹在 `try...catch` 區塊中。

# --hints--

你應該在 `response` 方法內宣告一個 `fetchCardsData` 變數。
...
你的 `response` 變數應該透過呼叫 `await fetch`，並使用 URL `` `${CDN_URL}/card_data.json` `` 指定其值。
...
你的 `fetch` 呼叫應該包裹在 `try...catch` 區塊中，並以 `error` 作為 `catch` 參數。
