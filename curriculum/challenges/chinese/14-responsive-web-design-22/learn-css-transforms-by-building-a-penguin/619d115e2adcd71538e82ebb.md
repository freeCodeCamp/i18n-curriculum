...
应该设置 `.beak` 的 `height` 为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.height, '10%');
```

应该将 `.beak` 的 `background-color` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.backgroundColor, 'orange');
```

应该将 `.beak` 的 `border-radius` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
