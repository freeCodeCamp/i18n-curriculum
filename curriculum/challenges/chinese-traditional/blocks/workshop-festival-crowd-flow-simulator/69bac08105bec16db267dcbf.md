...
接著，將以下訊息記錄到主控台：`gate.queue[tickIndex] + " attendees arriving."`。

# --hints--

你的 `handleGateAtTick` 函式應該將以下訊息記錄到主控台：`gate.queue[tickIndex] + " attendees arriving."`。
...
你的 `handleGateAtTick` 函式應該根據指定 tickIndex 時門口的佇列，記錄正確的參加者人數。
