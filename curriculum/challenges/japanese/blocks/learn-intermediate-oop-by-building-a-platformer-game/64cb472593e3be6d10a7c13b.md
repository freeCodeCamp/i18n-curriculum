...
条件の中に、`forEach` ループを追加して `checkpoints` 配列を繰り返し処理してください。コールバック関数のパラメータ名には `checkpoint` を使ってください。

`forEach` コールバックの中で、代入演算子の減算代入を使って checkpoints の `x` 位置から 5 を引いてください。

# --hints--

`forEach` 配列を繰り返し処理する `checkpoints` ループが必要です。
...
`checkpoint` の `x` 位置から 5 を引くために減算代入演算子を使ってください。
