...
바퀴 주위에 캐빈을 배치할 시간입니다. 첫 번째 `.cabin` 요소를 선택하세요. `right` 속성을 `-8.5%`로, `top` 속성을 `50%`로 설정하세요.

# --hints--

`.cabin:nth-of-type(1)` 선택자가 있어야 합니다.
...
`.cabin:nth-of-type(1)` 선택자에 `right` 속성이 `-8.5%`로 설정되어 있어야 합니다.
...
`.cabin:nth-of-type(1)` 선택자에 `top` 속성이 `50%`로 설정되어 있어야 합니다.
