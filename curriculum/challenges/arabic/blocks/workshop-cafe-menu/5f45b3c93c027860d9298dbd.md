...
تُغيّر خصائص الرابط عندما يحوم مؤشر الفأرة فوقه باستخدام <dfn>محدد زائف</dfn> يشبه `a:hover { propertyName: propertyValue; }`.

غيّر لون رابط تذييل `Visit our website` إلى اللون `brown` عندما يحوم المستخدم فوقه.

# --hints--

يجب أن تستخدم المحدد الزائف `a:hover`.
...
يجب أن تضبط خاصية `color` على `brown`.
...
يجب أن يكون لـ `a:hover` الخاص بك خاصية `color` بقيمة `brown`.
