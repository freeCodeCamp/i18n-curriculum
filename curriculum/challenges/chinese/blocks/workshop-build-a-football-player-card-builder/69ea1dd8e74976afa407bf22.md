...
接下来，创建一个类型 `Position`，其值为 `typeof POSITIONS[number]`。这会将 `POSITIONS` 数组解包成其各个字面字串的并集，实际上使它看起来像 `"GK" | "CB" | "LB" | ...` 等等。

# --hints--

你应该创建一个名为 `Position` 的类型别名。
...
你的 `Position` 类型应设置为 `typeof POSITIONS[number]`。
