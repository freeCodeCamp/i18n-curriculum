...
在第一個 `tr` 中，為你的 `th` 元素添加文字 `Loans The outstanding balance on our startup loan.`。將該文字中除 `Loans ` 之外的部分包裹在 `class` 設為 `description` 的 `span` 元素內。

在其下方添加三個 `td` 元素，並依序給予它們以下文字：`$500`、`$250` 和 `$0`。將第三個 `td` 元素的 `class` 設為 `current`。
...
你的 `th` 元素應該有文字 `Loans The outstanding balance on our startup loan.`
...
你應該將 `The outstanding balance on our startup loan.` 的文字包裹在 `span` 元素中。
