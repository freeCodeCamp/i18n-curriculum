...
Modifica la comprensione `row_str` per restituire una stringa solo quando l'elemento non è zero, e un carattere asterisco altrimenti.

# --hints--

La list comprehension assegnata alla variabile `row_str` dovrebbe chiamare `str()` su ogni elemento `i` in `row` se `i` è vero, e dovrebbe valutare a `'*'` altrimenti.
