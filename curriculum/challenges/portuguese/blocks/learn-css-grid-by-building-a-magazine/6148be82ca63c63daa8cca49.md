...
Você deve ter um seletor `.author`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author'));
```

O seletor `.author` deve ter a propriedade `font-size` definida como `2rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

O seletor `.author` deve ter a propriedade `font-family` definida como `Raleway` com `sans-serif` de alternativa.
