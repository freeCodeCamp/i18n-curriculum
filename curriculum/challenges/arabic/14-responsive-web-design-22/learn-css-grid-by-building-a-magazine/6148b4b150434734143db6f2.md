...
قم بإنشاء منتقي `.hero` وإعطائه الخاصية `grid-column` بقيمة `1 / -1`. هذا سيخبر العنصر بامتداد العرض الكامل للـ grid.

# --hints--

يجب أن يكون لديك منتقي `.hero`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero'));
```

يجب أن يكون لمنتقي `.hero` الخاص بك الخاصية `grid-column` بقيمة `1 / -1`.
