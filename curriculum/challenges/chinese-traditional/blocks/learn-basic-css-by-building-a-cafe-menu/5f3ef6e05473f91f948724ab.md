...
你可以更改文字的 `font-family`，使其看起來與瀏覽器的預設字型不同。每個瀏覽器都有一些可用的常用字型。

透過為你的 `body` 添加一個值為 `sans-serif` 的 `font-family` 屬性，來更改所有的文字。這是一種相當常見且非常易讀的字型。

# --hints--

你應該將 `font-family` 屬性設定為 `sans-serif`。

```js
const hasFontFamily = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style['font-family'] === 'sans-serif');
```

你的 `body` 應該有 `sans-serif` 的 `font-family`。
