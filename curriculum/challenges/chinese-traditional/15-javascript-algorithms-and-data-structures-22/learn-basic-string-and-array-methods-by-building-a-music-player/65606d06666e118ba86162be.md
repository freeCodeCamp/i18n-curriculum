...
你的 `allSongs` 數組應該有兩個值。

```js
assert.equal(allSongs.length, 2);
```

你的 `allSongs` 數組應該有一個對象作爲其第二個值。

```js
assert.isObject(allSongs[1]);
```

`allSongs` 數組中的第二個對象應將 `id` 屬性設置爲數字 `1`。

```js
assert.equal(allSongs[1].id, 1);
```

`allSongs` 數組中的第二個對象應將 `title` 屬性設置爲字符串 `"Can't Stay Down"`。

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

`allSongs` 數組中的第二個對象應將 `artist` 屬性設置爲字符串 `"Quincy Larson"`。

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

`allSongs` 數組中的第二個對象應將 `duration` 屬性設置爲字符串 `"4:15"`。

```js
assert.equal(allSongs[1].duration, "4:15");
```

`allSongs` 數組中的第二個對象應將 `src` 屬性設置爲字符串 `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`。
