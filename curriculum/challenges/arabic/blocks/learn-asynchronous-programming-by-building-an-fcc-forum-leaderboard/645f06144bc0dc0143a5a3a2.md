...
كل منشور في المنتدى سيشمل قائمة بصور رموز المستخدمين التي تمثل جميع المستخدمين المشاركين في المحادثة حول ذلك الموضوع.

ابدأ بإنشاء دالة سهمية تسمى `avatars`، مع معلمتين تسمى `posters` و `users`.

# --hints--

يجب أن يكون لديك دالة سهمية فارغة تسمى `avatars`.

```js
assert.match(code, /const\s+avatars\s*=\s*\([\s\S]*\)\s*=>\s*{\s*}/);
```

يجب أن تحتوي الدالة المسماة `avatars` على المعلمتين المسميتين `posters` و `users`.
