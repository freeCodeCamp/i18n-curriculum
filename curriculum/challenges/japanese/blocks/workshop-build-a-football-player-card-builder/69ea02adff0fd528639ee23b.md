...
次に、戻り値の型が `loadPlayer` の `PlayerData` 関数を作成してください。その中に、`try...catch` ブロックのパラメータとして `error` を持つ `catch` 構文を追加してください。その他はすべて空のままにしてください。

# --hints--

`loadPlayer` という名前の関数を作成してください。
...
`loadPlayer` 関数の戻り値の型は `PlayerData` にしてください。
...
`loadPlayer` 関数の中に `try...catch` ブロックを含めてください。
