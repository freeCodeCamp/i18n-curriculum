...
La funzione CSS `repeat()` viene utilizzata per ripetere un valore, invece di scriverlo manualmente ed è utile per i layout della griglia. Ad esempio, impostare la proprietà `grid-template-columns` su `repeat(20, 200px)` creerebbe 20 colonne, ciascuna ampia `200px`.

Dai all'elemento `.heading` una proprietà `grid-template-columns` impostata su `repeat(2, 1fr)` per creare due colonne di uguale larghezza.

# --hints--

Il selettore `.heading` dovrebbe avere una proprietà `grid-template-columns` impostata su `repeat(2, 1fr)`.
