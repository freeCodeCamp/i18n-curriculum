...
Перш ніж грати пісню, потрібно переконатися, що вона починається спочатку. Цього можна досягти використав властивість `currentTime` на об'єкті `audio`.

Додайте інструкцію `if`, щоб перевірити, чи `userData?.currentSong` є фальшивим *АБО* якщо `userData?.currentSong.id` строго не дорівнює `song.id`. Ця умова перевіряє, чи не відтворюється поточна пісня, або поточна пісня відрізняється від тієї, що збирається відтворитися.

Inside `if` block, set the `currentTime` property of the `audio` object to `0`.

# --hints--

You should create an `if` statement with the condition `userData?.currentSong === null || userData?.currentSong.id !== song.id`. Не забудьте включити необов’язковий ланцюжок.
...
Ви повинні встановити `audio.currentTime` на `0` у своєму блоці `if`.
