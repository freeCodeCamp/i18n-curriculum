...
データを扱い始めるには、別の`.then()`メソッドを使う必要があります。

既存の`.then()`メソッドにもう一つ`.then()`をチェーンしてください。今回は、コールバック関数のパラメータとして`data`を渡します。コールバックには複数の式があるので波括弧を使ってください。コールバック関数内で、`data`をコンソールにログ出力して、どのようなものか確認しましょう。
