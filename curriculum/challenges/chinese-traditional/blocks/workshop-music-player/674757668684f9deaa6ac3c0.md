...
仍在 `playSong` 函式中，將 `audio.src` 屬性設定為 `song.src`。這告訴音訊元素在哪裡找到所選歌曲的音訊資料。

此外，將 `audio.title` 屬性設定為與 `song.title` 相等。這會告訴音訊元素要顯示什麼作為歌曲的標題。

# --hints--

你應該在你的 `playSong` 函式（程式）內將 `audio.src` 屬性設為與 `song.src` 相等。
...
你應該在你的 `playSong` 函式中將 `audio.title` 屬性設為與 `song.title` 相等。
