...
接下来是最后一个定价项，**Family Plan**。

首先，为计划的包装器 `div` 添加类 `bg-gray-100`、`ring-1`、`ring-gray-300`、`p-8` 和 `rounded-xl`。

# --hints--

你的家庭计划的包装器 `div` 元素应具有类 `bg-gray-100`。
...
你的家庭计划的包装器 `div` 元素应具有类 `ring-gray-300`。
...
你的家庭计划的包装器 `div` 元素应具有类 `rounded-xl`。
