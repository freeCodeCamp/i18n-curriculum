...
ボタンのテキストと同じパターンに従って、3つのボタンの`onclick`の代入を`"button functions"`配列の1番目、2番目、3番目の値に更新してください。

# --hints--

`"button functions"`パラメータの`location`プロパティの最初の要素にアクセスする必要があります。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

`button1.onclick`パラメータの`"button functions"`プロパティの最初の要素を`location`プロパティに代入する必要があります。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

`"button functions"`パラメータの`location`プロパティの2番目の要素にアクセスする必要があります。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

`button2.onclick`パラメータの`"button functions"`プロパティの2番目の要素を`location`プロパティに代入する必要があります。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

`"button functions"`パラメータの`location`プロパティの3番目の要素にアクセスする必要があります。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*2\s*\]/);
```

`button3.onclick`パラメータの`"button functions"`プロパティの3番目の要素を`location`プロパティに代入する必要があります。
