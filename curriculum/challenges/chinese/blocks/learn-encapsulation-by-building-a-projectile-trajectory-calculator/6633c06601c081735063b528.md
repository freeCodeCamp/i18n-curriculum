...
最后，修改你在类之外编写的代码，并将其整合到一个名为 `projectile_helper` 的小工具函数中，该函数接收速度、高度和角度的所需值，并将弹丸的详细信息、坐标表和轨迹图依次打印到终端。

使用你选择的值调用 `projectile_helper` 一次。 

# --hints--

你应该在全局范围内创建 `projectile_helper`。
...
你应该至少调用一次 `projectile_helper` 函数。
