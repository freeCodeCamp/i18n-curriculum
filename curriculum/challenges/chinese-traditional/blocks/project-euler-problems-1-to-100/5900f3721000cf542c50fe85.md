...
因此，前 10 個自然數的平方和與和的平方之差為 3025 − 385 = 2640。

找出前 `n` 個自然數的平方和與和的平方之間的差異。

# --hints--

`sumSquareDifference(10)` 應該傳回一個數字。

```js
assert.isNumber(sumSquareDifference(10));
```

`sumSquareDifference(10)` 應該傳回 2640。

```js
assert.strictEqual(sumSquareDifference(10), 2640);
```

`sumSquareDifference(20)` 應該傳回 41230。

```js
assert.strictEqual(sumSquareDifference(20), 41230);
```

`sumSquareDifference(100)` 應該傳回 25164150。
