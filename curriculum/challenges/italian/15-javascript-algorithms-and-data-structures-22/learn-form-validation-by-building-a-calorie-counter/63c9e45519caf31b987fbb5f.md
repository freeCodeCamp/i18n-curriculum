...
Your `getCaloriesFromInputs` function will set the global error flag `isError` to `true` if an invalid input is detected. Aggiungi un'istruzione `if` alla funzione `calculateCalories` che controlla se il flag di errore globale è truthy e se lo è usa `return` per terminare l'esecuzione della funzione.
