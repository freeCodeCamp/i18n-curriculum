...
أسفل عنصر الاقتباس المقتبس، أضف عنصر فقرة يحتوي على النص `&mdash;Quincy Larson, How to Learn to Code and Get a Developer Job [Full Book]`.

`&mdash;` هو كيان HTML يمثل الشرطة المائلة الطويلة `—`.

# --hints--

يجب أن يكون لديك عنصر `p` أسفل عنصر `blockquote`.

```js
assert.exists(document.querySelector('section:first-of-type > blockquote + p'));
```

يجب أن يحتوي عنصر `p` الخاص بك على النص `&mdash;Quincy Larson, How to Learn to Code and Get a Developer Job [Full Book]`.
