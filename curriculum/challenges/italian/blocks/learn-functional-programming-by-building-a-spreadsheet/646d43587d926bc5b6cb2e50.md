...
Dichiara una variabile `str2` e assegnale il risultato della chiamata a `infixEval()` con `noHigh` e `infix` come argomenti.
...
Dovresti assegnare a `str2` il risultato della chiamata a `infixEval()`.
...
Dovresti passare `noHigh` come primo argomento a `infixEval()`.
...
Dovresti passare `infix` come secondo argomento a `infixEval()`.
