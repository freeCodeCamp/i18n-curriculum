...
Du wirst feststellen, dass du deinen `match`-Parameter nicht verwendest. In JavaScript ist es üblich, einem nicht verwendeten Parameter einen Unterstrich `_` voranzustellen. Du könntest den Parameter auch leer lassen, etwa so: `(, char1)`, aber es ist oft übersichtlicher, den Parameter zu benennen, damit er in Zukunft lesbar ist.

Stelle deinem `match`-Parameter einen Unterstrich voran.

# --hints--

Du solltest deinem `match`-Parameter einen Unterstrich voranstellen.
