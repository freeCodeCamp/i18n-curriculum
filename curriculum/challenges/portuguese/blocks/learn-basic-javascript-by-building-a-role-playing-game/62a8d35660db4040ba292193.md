...
Dentro da instrução `if`, defina `gold` como `15` a mais que o valor atual. Também atualize `goldText.innerText` com o novo valor.

# --hints--

Você deve usar a atribuição composta para aumentar `gold` em `15`.

```js
assert.match(sellWeapon.toString(), /gold\s*\+=\s*15/);
```

Você deve definir `goldText.innerText` como `gold`.
