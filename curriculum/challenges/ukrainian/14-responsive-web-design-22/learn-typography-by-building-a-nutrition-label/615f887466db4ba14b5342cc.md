...
Слідом за цим елементом створіть великий роздільник.
...
Ваш новий елемент `p` повинен мати текст `Protein 3g`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Protein[\s|\n]+3g/));
```

Ваш новий елемент `p` повинен мати атрибут `class` зі значенням `no-divider`.
