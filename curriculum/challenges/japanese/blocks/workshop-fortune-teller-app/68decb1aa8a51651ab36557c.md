...
`getElement` 関数の中で、ジェネリクスを使って `querySelector` を呼び出し、その結果を変数にストアしてください。要素が存在しない場合に備えて、`if` 文を追加し、存在しなければメッセージ `Element not found: ${error message here}` でエラーをスローしてください。要素が存在する場合は、それを返してください。

# --hints--

`el` 関数の中で `getElement` という名前の変数を宣言してください。
...
`el` 変数には `document.querySelector<T>(selector)` の値を代入してください。
