...
All'interno del blocco `try`, salva i dati correnti del giocatore in `localStorage` chiamando `localStorage.setItem` con la chiave di archiviazione e l'oggetto giocatore convertito in stringa.

E nel blocco `catch`, registra nella console la stringa `Failed to save player data:` seguita dal parametro `error`.

# --hints--

Il tuo blocco `useEffect` try dovrebbe chiamare `localStorage.setItem` con `STORAGE_KEY` e l'oggetto `player` convertito in stringa.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

Il tuo blocco `useEffect` catch dovrebbe registrare nella console `'Failed to save player data:'` e `error`.
