...
Kama Flexbox, kwa kutumia CSS Grid unaweza kupanga maudhui ya vipande vya gridi kwa `align-items` na `justify-items`. `align-items` itapanga vipengele vidogo kando ya mhimili wa safu, na `justify-items` itapanga vipengele vidogo kando ya mhimili wa mstari.

Toa kichaguzi cha `.social-icons` sifa ya `align-items` iliyowekwa kuwa `center`.

# --hints--

Kichaguzi chako cha `.social-icons` kinapaswa kuwa na sifa ya `align-items` iliyowekwa kuwa `center`.
