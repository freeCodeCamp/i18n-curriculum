...
Il segno di spunta in ogni `span` all'inizio di un elemento della lista è decorativo. Aggiungi `aria-hidden` a questi elementi `span` per nasconderli ai lettori di schermo.

Inoltre, assegna le classi `text-green-700` e `mr-2` a ciascuno degli elementi `span`.

# --hints--

I tuoi due elementi `span` dovrebbero avere l'attributo `aria-hidden`.
...
I tuoi due elementi `span` dovrebbero avere la classe `text-green-700`.
...
I tuoi due elementi `span` dovrebbero avere la classe `mr-2`.
