...
Ni wakati wa kuanza kufanya kazi kwenye vifungo vingine. Vipengele vilivyobaki vya `button` vimeongezwa kwako katika faili ya HTML.

Kwa kila kitufe kipya (`#confused-btn`, `#sad-btn` na `#loving-btn`), chagua kwa kutumia `querySelector` na kitambulisho chake, kisha ongeza msikilizaji wa tukio la kubofya unaofanya wito wa `updateCount` na kitufe hicho.

Tumia majina ya vigezo `confusedBtn`, `sadBtn`, `lovingBtn`.

# --hints--

Unapaswa kuunda kigezo kinachoitwa `confusedBtn`.

```js
assert.exists(confusedBtn);
```

Unapaswa kuhifadhi kipengele cha `#confused-btn` katika kigezo cha `confusedBtn`.

```js
assert.equal(confusedBtn, document.querySelector('#confused-btn'));
```

Unapaswa kuunda kigezo kinachoitwa `sadBtn`.

```js
assert.exists(sadBtn);
```

Unapaswa kuhifadhi kipengele cha `#sad-btn` katika kigezo cha `sadBtn`.

```js
assert.equal(sadBtn, document.querySelector('#sad-btn'));
```

Unapaswa kuunda kigezo kinachoitwa `lovingBtn`.

```js
assert.exists(lovingBtn);
```

Unapaswa kuhifadhi kipengele cha `#loving-btn` katika kigezo cha `lovingBtn`.

```js
assert.equal(lovingBtn, document.querySelector('#loving-btn'));
```

Kipengele cha `#confused-btn` kinapaswa kuwa na msikilizaji wa tukio la kubofya unaofanya wito wa `updateCount`.
...
Kipengele cha `#sad-btn` kinapaswa kuwa na msikilizaji wa tukio la kubofya unaofanya wito wa `updateCount`.
...
Kipengele cha `#loving-btn` kinapaswa kuwa na msikilizaji wa tukio la kubofya unaofanya wito wa `updateCount`.
