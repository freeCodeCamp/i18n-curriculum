...
在你的 `.text` 元素內的三個 `p` 元素之後，創建一個 `blockquote` 元素。在該元素內，為你的 `blockquote` 添加一個 `hr` 元素、一個 `class` 設為 `quote` 的 `p` 元素，以及第二個 `hr` 元素。

給 `.quote` 元素賦予文字 `The entire curriculum should be a series of projects`。

# --hints--

你應該在你的 `.text` 元素中創建一個新的 `blockquote` 元素。

```js
assert.exists(document.querySelector('.text blockquote'));
```

你的 `blockquote` 元素應該位於你的三個 `p` 元素之後。
...
你的新 `p` 元素應該將 `class` 設定為 `quote`。

```js
assert(document.querySelector('.text blockquote p')?.className === 'quote');
```

你的新 `p` 元素應該有文字 `The entire curriculum should be a series of projects`。
