...
其他文字已被移位。將 `.first-paragraph::first-letter` 選擇器設定 `float` 屬性為 `left`，並設定 `margin-right` 屬性為 `1rem`，即可將其移回正確位置。

# --hints--

你的 `.first-paragraph::first-letter` 選擇器應該有一個設定為 `left` 的 `float` 屬性。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

你的 `.first-paragraph::first-letter` 選擇器應該有一個 `margin-right` 屬性設定為 `1rem`。
