...
`sortedValues` 変数を `selectionSort` ではなく `bubbleSort` を呼び出した結果に更新してください。

# --hints--

`sortedValues` 変数には、`selectionSort` 配列を引数にして `inputValues` を呼び出した値を代入する必要があります。

```js
assert.match(code, /const\s+sortedValues\s*=\s*selectionSort\s*\(\s*inputValues\s*\)/);
```

コード内で `bubbleSort` を呼び出してはいけません。
