...
Вам потрібно розташувати внутрішнє коло та зробити його ініціально невидимим.

Продовжуючи цілитися в `.contact-method-radio-btn::before`, встановіть `transform` у `translate(3px, 3px) scale(0)`. Функція `translate` трохи переміщує коло, а `scale(0)` зменшує його до нульового розміру, щоб воно було приховане за замовчуванням.

Також додайте `transition` зі значенням `all 0.3s ease-in`, щоб створити плавну анімацію при появі. Це означає, що будь-яка зміна стилю анімуватиметься протягом 0.3 секунди, починаючи повільно і прискорюючись.

# --hints--

Ви повинні продовжувати цілитися в псевдоелемент `.contact-method-radio-btn::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Встановіть властивість `transform` у `translate(3px, 3px) scale(0)`.
...
Встановіть властивість `transition` у `all 0.3s ease-in`.
