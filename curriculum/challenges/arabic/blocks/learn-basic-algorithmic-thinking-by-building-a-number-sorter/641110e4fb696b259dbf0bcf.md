...
لهذه الخوارزمية، ستحتاج إلى استخدام حلقة `while`. هذه الحلقة تحتاج إلى شرطين:

- أولًا، يجب ألا تعمل بعد بداية المصفوفة (التي تم الوصول إليها باستخدام `j`).
- ثانيًا، يجب ألا تستمر الحلقة بعد أن تجد قيمة أصغر من القيمة الحالية.

لتجنب حلقة لا نهائية، قلل قيمة `j` داخل الحلقة.
...
يجب أن تحتوي حلقة `while` على الشرط الأول الذي يتحقق من أن قيمة `j` أكبر من أو تساوي `0`.
...
يجب أن تستخدم حلقة `while` عامل AND.
...
يجب أن تحتوي حلقة `while` على شرط يتحقق من أن قيمة `array[j]` أكبر من `currValue`.
...
يجب أن تقلل حلقة `while` قيمة `j` داخل الحلقة.
