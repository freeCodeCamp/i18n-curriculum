...
`currentWeapon` の後に、連結演算子を使って `text.innerText` に文字列 `"You sold a "`、次に `currentWeapon`、そして文字列 `"."` を代入してください。

# --hints--

`text.innerText` には代入演算子を使う必要があります。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=/);
```

`"You sold a "` に `text.innerText` を追加してください。スペースの有無が重要です。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

`currentWeapon` 文字列に `"You sold a "` の値を追加してください。同じ行で連結演算子を使います。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a\s\1\s*\+\s*currentWeapon/);
```

`"."` の値に文字列 `currentWeapon` を追加してください。同じ行で連結演算子を使います。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1\s*\+\s*currentWeapon\s+\+\s+('|")\.\2/);
```

`text.innerText` は正しい文字列に更新されているはずです。
