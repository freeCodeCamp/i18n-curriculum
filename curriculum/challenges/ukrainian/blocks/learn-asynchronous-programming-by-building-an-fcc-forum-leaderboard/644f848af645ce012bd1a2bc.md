...
Створіть константу з назвою `linkText` і призначте їй значення `selectedCategory.category`. Це відобразить назву категорії в елементі посилання.

# --hints--

У вас має бути константа з назвою `linkText`.

```js
assert(code.match(/const\s+linkText\s*=/));
```

Ви повинні призначити `selectedCategory.category` вашій змінній `linkText`.
