...
`className`が`form-group`の`div`をもう1つ追加してください。その中に、`className`が`label`、`htmlFor`が`imageUrl`、テキストが`Image URL`の`label`要素を追加してください。次に、`id`が`imageUrl`、`className`が`input`、`type`が`text`の`input`要素を追加してください。
