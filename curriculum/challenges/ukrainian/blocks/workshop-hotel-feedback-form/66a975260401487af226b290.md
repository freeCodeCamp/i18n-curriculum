...
Щоб опція була вибрана за замовчуванням, ви можете додати атрибут `selected` до відповідного елемента `option`.

Ось приклад використання атрибута `selected`:

```html
<option selected value="amazing">Amazing</option>
```

В межах елемента `select` додайте атрибут `selected` до елемента `option` зі значенням `"excellent"`.

# --hints--

Елемент `option` зі значенням `"excellent"` повинен мати атрибут `selected`.
