...
在 headCoach 对象内部，添加一个属性，其键为 `coachName`，字符串值为 `"Carlos Bilardo"`。 在该属性下方，添加另一个名为 `matches` 的键，其数字值为 `7`。

# --hints--

你的 `headCoach` 对象应该具有 `coachName` 属性。

```js
assert.property(myFavoriteFootballTeam.headCoach, 'coachName');
```

你的 `coachName` 属性应设置为 `"Carlos Bilardo"`。

```js
assert.equal(myFavoriteFootballTeam.headCoach.coachName, 'Carlos Bilardo');
```

你的 `headCoach` 对象应该具有 `matches` 属性。

```js
assert.property(myFavoriteFootballTeam.headCoach, 'matches');
```

你的 `matches` 属性应该是一个数字。
