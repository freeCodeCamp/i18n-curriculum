...
Denken Sie daran, dass Sie Destrukturierung verwenden können, um mehrere Werte aus einem Array oder Objekt in einer einzigen Anweisung zu extrahieren.

Dekonstruieren Sie für den ersten Parameter Ihrer Callback-Funktion die Eigenschaften `name`, `id`, `price` und `category` aus der übergebenen `Dessert`-Instanz.

# --hints--

Sie sollten Destrukturierung verwenden, um die Parameter `name`, `id`, `price` und `category` zu deklarieren. Für diesen Test ist die Reihenfolge wichtig.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

Ihre Destrukturierung sollte der erste Parameter der Callback-Funktion sein.
