...
新しい `.divider` のセレクターを作成し、`border-bottom` プロパティを `1px solid #888989` に設定してください。また、上下のマージンを `2px` に設定し、左右のマージンは設定しないでください。

# --hints--

`.divider` セレクターを作成する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.divider'));
```

`.divider` セレクターには `border-bottom` プロパティを `1px solid #888989` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.divider')?.borderBottom === '1px solid rgb(136, 137, 137)');
```

`.divider` セレクターには `margin` プロパティを `2px 0` に設定してください。
