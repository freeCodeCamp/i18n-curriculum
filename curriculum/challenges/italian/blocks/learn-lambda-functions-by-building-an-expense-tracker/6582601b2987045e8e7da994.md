...
Dopo la tua chiamata `print()`, devi filtrare le spese e stampare la lista filtrata. Dichiara una variabile `expenses_from_category` e assegnale una chiamata a `filter_expenses_by_category` passando `expenses` e `category` come argomento.

# --hints--

Dovresti dichiarare una variabile chiamata `expenses_from_category` nella tua clausola `elif`.
...
Dovresti chiamare `filter_expenses_by_category()` passando `expenses` e `category` come argomenti e assegnarla alla tua variabile `expenses_from_category`.
...
La tua variabile `expenses_from_category` dovrebbe venire dopo la chiamata `print()`.
