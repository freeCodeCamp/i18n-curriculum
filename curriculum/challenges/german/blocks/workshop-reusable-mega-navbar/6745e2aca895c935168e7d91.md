...
Um alles abzuschließen, setzen Sie einen Text von `Calendar` für das erste Ankerelement, `Chat` für das zweite und `Email` für das dritte.

Damit ist Ihre Mega-Navigationsleisten-Komponente fertig!

# --hints--

Sie sollten einen Text von `Calendar` für das erste Ankerelement setzen.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Sie sollten einen Text von `Chat` für das zweite Ankerelement setzen.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Sie sollten einen Text von `Email` für das dritte Ankerelement setzen.
