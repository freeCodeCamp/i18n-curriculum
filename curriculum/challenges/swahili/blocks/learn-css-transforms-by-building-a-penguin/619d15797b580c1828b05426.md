...
Unapaswa kuongeza kipengele cha `div` ndani ya `div.shirt`. Inatarajiwa kipengele cha `--fcc-expected--` `div`, kilichopatikana `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.shirt > div')?.length, 1);
```

Unapaswa kumpa `div` mpya maudhui ya `💜`, lakini ilipatikana `--fcc-actual--`.
