...
Novamente, você precisa fazer o cartão ser aleatório e invertido. Declare uma variável chamada `isReversed` e `card`. Para a primeira, faça a chamada do método `Math.random` e compare com `< 0.5`. Para a segunda, faça a chamada `getRandomItem` e passe `this.cards`.

# --hints--

Você deve declarar uma variável chamada `isReversed`.
...
Sua variável `isReversed` deve usar `Math.random()` para decidir aleatoriamente se o cartão está invertido.
...
Você deve declarar uma variável chamada `card`.
...
Sua variável `card` deve ser atribuída pela chamada do método `getRandomItem` com `this.cards` como argumento.
