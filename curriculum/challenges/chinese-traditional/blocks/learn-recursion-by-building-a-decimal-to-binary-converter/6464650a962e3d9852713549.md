...
如果 `numberInput` 的 `value` 相等於 `5`，則呼叫 `showAnimation()` 函式，然後提前 `return`。

# --hints--

你應該在第二個 `if` 述語的主體中呼叫 `showAnimation()` 函式（程式）。

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

在呼叫 `showAnimation()` 函式（程式）後，你應該使用 `return` 關鍵字來提前離開 `checkUserInput()` 函式（程式）。
