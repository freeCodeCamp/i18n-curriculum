...
데이터 결과를 확인하려면 `data` 블록 안에서 `try` 변수를 콘솔에 출력하세요.

`fetchData` 정의 아래에서 함수를 호출하고 콘솔을 열어 결과를 확인하세요.

# --hints--

`console.log` 변수를 콘솔에 출력하려면 `data`를 사용하세요.

```js
assert.match(code, /\s*console\.log\(\s*data\s*\)/);
```

`console.log`는 `try` 블록 안에 있어야 합니다.

```js
assert.match(code, /\s*try\s*{[\s\S]*\s*console\.log\(\s*data\s*\)[\s\S]*}\s*catch\s*\(\s*err\s*\)\s*{\s*}/);
```

`fetchData()` 함수가 정의된 후에 `fetchData()` 함수를 호출해야 합니다.
