...
Damit ist ein grundlegendes Styling für den Abstand der Elementkarte erledigt, einschließlich des CTA-Buttons. Für den restlichen inneren Inhalt müssen Sie alles in einem Grid anordnen, indem Sie das innere `div` ansprechen.

Weisen Sie den Klassen `grid`, `grid-rows-[auto_auto_auto_1fr]` und `gap-y-2` das innere `div` zu.

# --hints--

Ihr inneres `div`-Element für den Familienplan sollte die Klasse `grid` haben.
...
Ihr inneres `div`-Element für den Familienplan sollte die Klasse `grid-rows-[auto_auto_auto_1fr]` haben.
...
Ihr inneres `div`-Element für den Familienplan sollte die Klasse `gap-y-2` haben.
