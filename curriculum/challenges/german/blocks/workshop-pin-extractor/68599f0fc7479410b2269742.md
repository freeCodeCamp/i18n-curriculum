...
Aktualisieren Sie das Argument der Funktion zu `poems`, und erstellen Sie dann eine `for`-Schleife um den gesamten aktuellen Inhalt der Funktion, die über `poems` iteriert und `poem` als Schleifenvariable verwendet.

# --hints--

Die `pin_extractor`-Funktion sollte einen Parameter namens `poems` haben.
...
Sie sollten eine `for`-Schleife haben, die über `poems` iteriert.
...
Die Schleife sollte `poem` als Schleifenvariable haben.
...
Sie sollten den bestehenden Funktionskörper in die neue `for`-Schleife verschieben.
