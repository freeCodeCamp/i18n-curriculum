...
2차원에 주어진 점들 중 가장 가까운 두 점을 찾는 함수를 작성하세요.

직관적인 해결책은 $O(n^2)$ 알고리즘(이를 *브루트포스 알고리즘*이라고 부를 수 있음)이며, 인덱스를 사용한 의사 코드는 다음과 같습니다:
...
더 나은 알고리즘은 재귀적 분할 정복 방식을 기반으로 하며, $O(n\log n)$입니다. 의사 코드는 다음과 같습니다:
...
입력으로는 `Point` 객체들의 배열이 인수로 주어지며, `x`과 `y` 멤버가 숫자로 설정되어 있습니다. 가장 가까운 두 점 쌍에 대해 `distance`과 `pair` 키:값 쌍을 포함하는 객체를 반환하세요.

예를 들어 `getClosestPair`에 `points` 배열을 입력하면:
...
**참고:** `pair` 배열을 `x` 값 기준으로 오름차순 정렬하세요.
...
`getClosestPair`는 함수여야 합니다.

```js
assert(typeof getClosestPair === 'function');
```

`getClosestPair(points1).distance`는 `0.0894096443343775`이어야 합니다.

```js
assert.equal(getClosestPair1_().distance, answer1.distance);
```

`getClosestPair(points1).pair`는 `[ { x: 7.46489, y: 4.6268 }, { x: 7.46911, y: 4.71611 } ]`이어야 합니다.
...
`getClosestPair(points2).distance`는 `65.06919393998976`이어야 합니다.

```js
assert.equal(getClosestPair2_().distance, answer2.distance);
```

`getClosestPair(points2).pair`는 `[ { x: 37134, y: 1963 }, { x: 37181, y: 2008 } ]`이어야 합니다.
...
`getClosestPair(points3).distance`는 `6754.625082119658`이어야 합니다.

```js
assert.equal(getClosestPair3_().distance, answer3.distance);
```

`getClosestPair(points3).pair`는 `[ { x: 46817, y: 64975 }, { x: 48953, y: 58567 } ]`이어야 합니다.
