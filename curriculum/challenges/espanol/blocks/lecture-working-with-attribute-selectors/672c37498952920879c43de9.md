...
En este caso, cualquier elemento `p` con un atributo `data-lang` establecido en francés (`fr`) tendrá el color de su texto cambiado a azul. Esto te permite identificar y estilizar rápidamente secciones de contenido basadas en la información del idioma almacenada en el atributo `data-lang`.
