...
要在使用者介面（UI）中顯示歌曲，你需要創建一個函式（程式）。

使用箭頭函式語法來創建一個 `renderSongs` 函式（程式），該函式（程式）以 `array` 作為其參數。

# --hints--

你應該使用 `const` 來創建一個名為 `renderSongs` 的空函式（程式）。

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs` 應該是一個函式（程式）。

```js
assert.isFunction(renderSongs)
```

你的 `renderSongs` 函式（程式）應該使用箭頭函式語法。

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

你的 `renderSongs` 函式（程式）應該將 `array` 作為參數。

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

你的 `renderSongs` 函式應該是空的。
