...
你的 `allSongs` 数组应该有一个值。

```js
assert.equal(allSongs.length, 1);
```

你的 `allSongs` 数组应该有一个对象作为其第一个值。

```js
assert.isObject(allSongs[0]);
```

你的 `allSongs` 数组应该有一个对象，其 `id` 属性设置为数字 `0`。

```js
assert.strictEqual(allSongs[0].id, 0);
```

你的 `allSongs` 数组应该有一个对象，其 `title` 属性设置为字符串 `"Scratching The Surface"`。

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

你的 `allSongs` 数组应该有一个对象，其 `artist` 属性设置为字符串 `"Quincy Larson"`。

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

你的 `allSongs` 数组应该有一个对象，其 `duration` 属性设置为字符串 `"4:25"`。

```js
assert.equal(allSongs[0].duration, "4:25");
```

你的 `allSongs` 数组应该有一个对象，其 `src` 属性设置为字符串 `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`。
