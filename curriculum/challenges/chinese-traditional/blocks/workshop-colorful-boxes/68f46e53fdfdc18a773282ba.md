...
在你的 `styles.css` 檔案中，創建一個 `.box2` 選擇器並為它添加 `background` 屬性，值為 `#f4a261`。

另外，為你的元素添加值為 `0` 的 `order` 屬性。
