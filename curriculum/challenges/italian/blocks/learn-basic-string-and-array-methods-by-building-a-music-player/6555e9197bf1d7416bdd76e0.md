...
È ora di implementare una funzionalità per eliminare dalla playlist. Questa gestirà la rimozione di una canzone dalla playlist, si occuperà di altre azioni correlate quando una canzone viene eliminata e creerà un pulsante `Reset Playlist`.

Usa `const` e la sintassi a freccia per creare una funzione `deleteSong` vuota e passa `id` come parametro.

# --hints--

Dovresti usare `const` per creare una funzione vuota chiamata `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` dovrebbe essere una funzione.

```js
assert.isFunction(deleteSong)
```

La tua funzione `deleteSong` dovrebbe usare la sintassi a freccia.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

La tua funzione `deleteSong` dovrebbe prendere un parametro `id`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

La tua funzione `deleteSong` dovrebbe essere vuota.
