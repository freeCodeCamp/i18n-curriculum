...
All'interno del loop `while`, assegna `right_part[right_array_index]` a `array[sorted_index]`.

# --hints--

Dovresti avere `array[sorted_index] = right_part[right_array_index]` nel corpo del loop. Non dimenticare di rimuovere la parola chiave `pass` nel corpo del loop.
...
Non dovrebbe esserci la parola chiave `pass` nel loop `while`
