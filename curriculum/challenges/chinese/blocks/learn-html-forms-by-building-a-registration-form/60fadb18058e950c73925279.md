...
在开发过程中，查看 `fieldset` 默认边框很有帮助。 然而，它们使内容间的距离过远。

删除 `border`，然后给 `fieldset` 的顶部和底部添加 `2rem` 的内边距。 确保移除了左侧和右侧的 `padding`。
...
你应该为每个 `fieldset` 添加 `2rem` 的顶部和底部 `padding`。
