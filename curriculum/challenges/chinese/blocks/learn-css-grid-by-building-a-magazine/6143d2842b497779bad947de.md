...
开始写 CSS，通过使用 `*`，包括 `::before` 和 `::after` 伪选择器，定位所有元素来规范 CSS 规则，。

将 `padding` 和 `margin` 属性均设置为 `0`，并将 `box-sizing` 属性设置为 `border-box`。
