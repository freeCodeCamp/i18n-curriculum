...
在 `try` 块之前，创建一个 `MediaCatalogue` 实例并将其赋值给名为 `catalogue` 的变量。

然后，使用 `add` 方法将 `movie1` 追加到目录中。最后将 `catalogue` 打印到控制台。

# --hints--

你应该在你的 `try` 块之前将一个 `MediaCatalogue` 实例赋值给名为 `catalogue` 的变量。
...
你应该在你的 `try` 块中有 `catalogue.add(movie1)`。
