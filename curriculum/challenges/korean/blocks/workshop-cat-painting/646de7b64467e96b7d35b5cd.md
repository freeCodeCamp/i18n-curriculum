...
오른쪽 눈을 `position`의 `absolute` 속성, `top`의 `54px` 속성, `left`의 `134px` 속성으로 위치시키세요.

# --hints--

`.cat-right-eye` 선택자에 `position` 속성을 `absolute`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.position, 'absolute')
```

`.cat-right-eye` 선택자에 `top` 속성을 `54px`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.top, '54px')
```

`.cat-right-eye` 선택자에 `left` 속성을 `134px`로 설정하세요.
