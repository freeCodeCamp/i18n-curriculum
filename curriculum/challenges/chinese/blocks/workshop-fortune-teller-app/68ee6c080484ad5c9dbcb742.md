...
创建一个名为 `singleCardSelected` 的方法。在其中，调用 `hideElements` 函数，并传入以下属性：`singleCardBtn`、`multipleCardsBtn`、`multipleCard`、`text` 和 `headerTitle`。

记住你必须使用 `this.elements` 来访问它们全部。

# --hints--

你应该定义一个名为 `singleCardSelected` 的方法。
...
在 `singleCardSelected` 内，你应该调用 `hideElements` 函数。
...
你应该将 `singleCardBtn` 传递给 `hideElements` 函数。
...
你应该将 `multipleCardsBtn` 传递给 `hideElements` 函数。
...
你应该将 `multipleCard` 传递给 `hideElements` 函数。
...
你应该将 `headerTitle` 传递给 `hideElements` 函数。
