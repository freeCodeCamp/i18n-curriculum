...
`select` 要素（その中の `option` 要素も含む）を、テキスト `label` を持つ `How did you hear about us?` 要素の中にネストしてください。テキストは `select` 要素の前に配置します。

# --hints--

`select` 要素だけを `label` 要素の中にネストしてください。

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(3) > select'));
```

`label` 要素にテキスト `How did you hear about us?` を設定してください。

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(3)')?.innerText?.trim(), 'How did you hear about us?');
```

テキストは `select` 要素の前に置いてください。
...
`option` 要素の中に `select` 要素が5つあるはずです。
