...
現在你可以開始將你的函式解析邏輯套用到字串。宣告一個名為 `applyFunction` 的函式，該函式帶有一個 `str` 參數。

# --hints--

你應該宣告一個 `applyFunction` 變數。

```js
assert.match(code, /(?:var|let|const)\s+applyFunction\s*=/);
```

你應該使用 `const` 來宣告你的 `applyFunction` 變數。

```js
assert.match(code, /const\s+applyFunction\s*=/);
```

你的 `applyFunction` 變數應該是一個函式（程式）。

```js
assert.isFunction(applyFunction);
```

你的 `applyFunction` 函式（程式）應該使用箭頭語法。

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

你的 `applyFunction` 函式應該有一個 `str` 參數。

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>/);
```

你的 `applyFunction` 應該是空的。
