...
在 `li` 元素內、`item` 文字上方，為你的 `input` 元素添加 `type` 屬性並設為 `checkbox`。同時，將它的 `onChange` 屬性設為一個匿名函式，該函式會以 `item` 作為引數呼叫 `toggleItem()`。

最後，將 `input` 和 `item` 文字包裹在 `label` 元素中，以提升存取性。

# --hints--

你應該為你的表單添加另一個 `input` 元素，並將 `type` 屬性設為 `checkbox`。
...
你的新 `input` 應該有一個 `onChange` 屬性，設定為一個匿名函式，該函式呼叫 `toggleItem()` 並以 `item` 作為引數。
...
你的新 `input` 和 `item` 文字應該包裹在 `label` 元素中。
