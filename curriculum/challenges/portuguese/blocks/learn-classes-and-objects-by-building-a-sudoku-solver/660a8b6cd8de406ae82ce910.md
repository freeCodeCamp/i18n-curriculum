...
Aninhe uma declaração `if` dentro do `if` atual.Para a condição `if`, use uma chamada recursiva para `solver()` e retorne `True` do novo corpo do `if`.

# --hints--

Você deve aninhar uma declaração `if` dentro da declaração `if` atual e usar `self.solver()` como condição.
...
Você deve retornar `True` do seu novo `if` statement.
