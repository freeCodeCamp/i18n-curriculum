...
Jetzt fügen Sie die Logik zum Speichern in `localStorage` mit `useEffect` hinzu.

Das Zugreifen auf oder Speichern von Daten in `localStorage` gilt in React als Nebeneffekt, daher sollten Sie `useEffect` für diese Logik verwenden.

Beginnen Sie damit, `useEffect` aus React zu destrukturieren, genau wie es mit `useState` gemacht wurde.

# --hints--

Sie sollten `useEffect` aus `React` destrukturieren.
