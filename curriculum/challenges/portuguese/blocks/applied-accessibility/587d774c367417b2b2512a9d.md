...
**Nota:** Para imagens com legenda, você ainda pode querer incluir o texto `alt`, pois isso ajuda os mecanismos de busca a catalogar o conteúdo da imagem.

# --instructions--

O Camper Cat codificou o esqueleto da página do blog do site dele.Ele está planejando adicionar uma quebra visual entre os dois artigos com uma imagem decorativa de uma espada de samurai.Adicione um atributo `alt` à tag `img` e defina-o como uma string vazia. (Observe que o `src` da imagem não aponta para um arquivo real - não se preocupe que não apareçam espadas na exibição.)
