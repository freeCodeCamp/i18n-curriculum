...
title: Abgerundete Ecken mit border-radius hinzufügen
challengeType: 0
...
Ihr Katzenfoto hat derzeit scharfe Ecken. Wir können diese Ecken mit einer CSS-Eigenschaft namens `border-radius` abrunden.

# --instructions--

Sie können einen `border-radius` in Pixeln angeben. Geben Sie Ihrem Katzenfoto einen `border-radius` von `10px`.

**Hinweis:** Für diese Aufgabe gibt es mehrere mögliche Lösungen. Zum Beispiel können Sie `border-radius` entweder der `.thick-green-border`-Klasse oder der `.smaller-image`-Klasse hinzufügen.

# --hints--

Ihr Bild-Element sollte die Klasse `thick-green-border` haben.

```js
assert.isTrue(document.querySelector('img').classList.contains('thick-green-border'));
```

Ihr Bild sollte einen Rahmenradius von `10px` haben.
