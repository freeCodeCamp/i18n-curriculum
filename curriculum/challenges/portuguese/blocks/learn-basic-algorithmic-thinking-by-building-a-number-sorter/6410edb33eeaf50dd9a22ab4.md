...
Quando a condição de `if` for verdadeira, você precisará trocar os dois elementos, "subindo a bolha" do maior elemento em direção ao final do array.

Para fazer isso, declare uma variável `temp` e atribua a ela o valor de `array[j]`.Em seguida, atribua a `array[j]` o valor de `array[j + 1]`.Por fim, atribua a `array[j + 1]` o valor de `temp`.
