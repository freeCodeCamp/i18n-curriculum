...
Es importante recordar que `aria-hidden` solo oculta el contenido de la tecnología de asistencia, como los lectores de pantalla. Si el contenido debe estar oculto para todos, entonces no debes usar `aria-hidden` para ocultarlo. Por ejemplo, un menú hamburguesa que está colapsado debe estar oculto para todos los usuarios de teclado, no solo para los usuarios de lectores de pantalla. En este caso, podrías establecer la propiedad CSS `display` en `none` en el menú para eliminarlo de la página renderizada y del árbol de accesibilidad cuando esté colapsado.
...
**NOTA**: Este ejemplo interactivo incluye el CDN de Font Awesome para que puedas ver el ícono de engranaje mostrado en la ventana de vista previa. Para ver las vistas previas, necesitarás habilitar el editor interactivo.
...
En estos tres casos, el elemento ya está oculto del árbol de accesibilidad, por lo que el atributo `aria-hidden` no es necesario.
...
En general, todo el contenido y la funcionalidad disponibles en la página también deberían estar disponibles para las personas que usan tecnología de asistencia. El uso de `aria-hidden` es muy limitado y debería restringirse principalmente a hacer la experiencia más limpia para los usuarios de lectores de pantalla al eliminar información puramente decorativa o duplicada. No uses `aria-hidden` para ocultar contenido que creas que los usuarios de lectores de pantalla podrían querer. Los usuarios de lectores de pantalla merecen tener acceso a toda la información en la página.
