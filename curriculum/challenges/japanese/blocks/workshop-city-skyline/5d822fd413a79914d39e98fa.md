...
`:root` に `window-color2` という新しい変数を作成し、値を `#8cd9b3` にしてください。これはこの建物のセカンダリーカラーとして使われます。

# --hints--

`window-color2` の中に `:root` という新しいプロパティ変数を作成してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--window-color2'));
```

`window-color2` に `#8cd9b3` の値を与えてください。
