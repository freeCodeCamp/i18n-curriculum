...
الآن، كل الحقائق الثلاثة مرئية في نفس الوقت. لكن يجب أن تكون حقيقة واحدة فقط مرئية في كل مرة.

لتحقيق ذلك، تحتاج إلى تعيين الخاصية `hidden` على العنصرين الثاني والثالث من نوع `tabpanel`.

# --hints--

يجب أن يحتوي العنصر الثاني من نوع `tabpanel` على الخاصية `hidden`.

```js
assert.isTrue(document.querySelectorAll('div[role="tabpanel"]')[1].hasAttribute("hidden"));
```

يجب أن يحتوي العنصر الثالث من نوع `tabpanel` على الخاصية `hidden`.
