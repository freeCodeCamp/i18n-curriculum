...
다음으로, `label`와 `className`의 `section`을 가진 `column`를 만들고, 텍스트 `How did you get your powers?`를 넣으세요. `label` 안에 `select` 요소를 만드세요.

# --hints--

`label`와 `className`의 `section`을 가진 `column` 요소를 만들어야 합니다.
...
`label` 요소에는 텍스트 `How did you get your powers?`가 있어야 합니다.
...
`select` 요소 안에 `label` 요소를 만들어야 합니다.
