...
你的 `allSongs` 数组应该有两个值。

```js
assert.equal(allSongs.length, 2);
```

你的 `allSongs` 数组应该有一个对象作为其第二个值。

```js
assert.isObject(allSongs[1]);
```

`allSongs` 数组中的第二个对象应将 `id` 属性设置为数字 `1`。

```js
assert.equal(allSongs[1].id, 1);
```

`allSongs` 数组中的第二个对象应将 `title` 属性设置为字符串 `"Can't Stay Down"`。

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

`allSongs` 数组中的第二个对象应将 `artist` 属性设置为字符串 `"Quincy Larson"`。

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

`allSongs` 数组中的第二个对象应将 `duration` 属性设置为字符串 `"4:15"`。

```js
assert.equal(allSongs[1].duration, "4:15");
```

`allSongs` 数组中的第二个对象应将 `src` 属性设置为字符串 `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`。
