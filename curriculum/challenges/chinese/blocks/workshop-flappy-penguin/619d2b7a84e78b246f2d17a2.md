...
选择 `class` 为 `left` 的 `.arm` 元素，并将其放置在父元素顶部 `35%` 左侧 `5%` 的位置。 然后，选择 `class` 为 `right` 的 `.arm` 元素，并将其放置在父元素顶部 `0%` 右侧 `-5%` 的位置。
