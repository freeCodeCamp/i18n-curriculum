...
Si no hay una siguiente canción en la lista de reproducción, utiliza el bloque `else` para reiniciar la clave `currentSong` de `userData` a `null`, y su propiedad `songCurrentTime` a `0`.

# --hints--

No debes modificar la instrucción `if` existente ni su contenido.
...
Debes usar la notación de puntos para establecer la propiedad `currentSong` de `userData` a `null` en el bloque `else` de tu declaración `if`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

Debes usar la notación de puntos para establecer la propiedad `songCurrentTime` de `userData` a `0` en el bloque `else` de tu declaración `if`.
