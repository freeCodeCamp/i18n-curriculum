...
Aggiungi un attributo `value` con valore `player.pac` al PAC `input`. Poi aggiungi un attributo `onChange` impostato su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `pac` impostato su `Number(e.target.value)`.

# --hints--

Il tuo PAC `input` dovrebbe avere un `value` uguale alla statistica `pac` del giocatore.
...
Il tuo PAC `input` dovrebbe avere un gestore `onChange` che chiama `setPlayer` con uno spread di `player` e `pac` impostato su `Number(e.target.value)`.
