...
你的代码应该有一个 `img` 元素。 You may have removed the `img` element or you have not surrounded the `src` attribute's value with quotes.
...
Your `img` element's `src` attribute should be set to '`https://cdn.freecodecamp.org/curriculum/cat-photo-app/relaxing-cat.jpg`'. 你要么遗漏了 URL，要么拼写有误。 The case of the URL is important.
