...
잘했습니다! 이제 전경 건물로 넘어가세요! `.fb1` 건물을 세 개의 섹션으로 만들기 위해 그 안에 세 개의 새로운 `div` 요소를 중첩하세요. 순서대로 `fb1a`, `fb1b`, `fb1c` 클래스를 각각 부여하세요.

# --hints--

`div` 안에 세 개의 `.fb1` 요소를 추가하세요.

```js
assert.equal(document.querySelector("div.fb1")?.children?.length, 3);
```

첫 번째 새 `div`에 `fb1a` 클래스를 부여하세요.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1a"));
```

두 번째 새 `div`에 `fb1b` 클래스를 부여하세요.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1b"));
```

세 번째 새 `div`에 `fb1c` 클래스를 부여하세요.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1c"));
```

새로운 `div` 요소들을 `.fb1a + .fb1b + .fb1c` 순서로 배치하세요.
