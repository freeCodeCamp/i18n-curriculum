...
Ambas as suas funções devem usar retornos implícitos.
...
Sua função `idToText` deve retornar o resultado da chamada do método `.find()` no seu array `cells`. A função de callback deve usar um retorno implícito.
...
Sua função `idToText` deve usar um retorno implícito.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(/);
```

Você deve passar uma função callback para o seu método `.find()`. Use a sintaxe de arrow.
