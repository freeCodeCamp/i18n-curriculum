...
Встановіть колір фону `footer` на `#2a2a40` і використайте гнучкий контейнер, щоб горизонтально центровано розмістити текст.
...
Ви повинні задати `footer` властивість `background-color` зі значенням `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

Ви повинні задати `footer` властивість `display` зі значенням `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Ви повинні задати `footer` властивість `justify-content` зі значенням `center`.
