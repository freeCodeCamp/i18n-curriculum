...
`while` ループ内で、`right_part[right_array_index]` に `array[sorted_index]` を代入してください。

# --hints--

ループの本体には `array[sorted_index] = right_part[right_array_index]` があるはずです。ループの本体から `pass` キーワードを削除するのを忘れないでください。
...
`pass` ループ内に `while` キーワードがあってはいけません。
