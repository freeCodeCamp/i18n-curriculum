...
- 导出使组件可以被其他文件使用。导入允许一个文件使用在其他地方导出的组件。
- React 中有两种导出类型：默认导出和命名导出。

**默认导出**

- 一个文件只能有一个默认导出。它适合主要包含单个组件的文件。
- 你可以这样将组件导出为默认导出：
...
- 命名导出允许一个文件共享多个组件。与默认导出不同，导入时必须使用导出时的确切名称。
- 你可以这样从同一个文件导出多个组件：
...
- 导入混合导出时，默认导出放在前面（不带花括弧），命名导出放在后面（带花括弧）：
