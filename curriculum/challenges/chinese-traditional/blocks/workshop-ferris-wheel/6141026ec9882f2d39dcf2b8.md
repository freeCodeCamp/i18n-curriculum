...
在 `0%` 和 `100%` 選擇器之間，添加一個 `50%` 選擇器。 這將適用於動畫週期的中間階段。 將 `background-color` 設置爲 `purple`。

# --hints--

你應該在 `@keyframes cabins` 規則中創建一個新的 `50%` 選擇器。
