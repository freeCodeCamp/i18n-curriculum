...
Successivamente, rimuovi `width` e `height` da `.bb2a` e modifica `border-left` e `border-right` per usare `5vw` invece di `1vw`. L'elemento avrà ora dimensione zero e i bordi si uniranno al centro.

# --hints--

Dovresti rimuovere `width` da `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.width);
```

Dovresti rimuovere `height` da `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.height);
```

Dovresti modificare `border-left` per usare `5vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid rgb(153, 153, 153)");
```

Dovresti modificare `border-right` per usare `5vw`.
