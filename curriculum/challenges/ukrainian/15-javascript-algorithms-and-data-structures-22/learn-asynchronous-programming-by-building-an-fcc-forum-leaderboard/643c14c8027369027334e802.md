...
Ви хочете отримати тіло відповіді як об’єкт JSON. Метод `.json()` змінної `res` повертає проміс, тобто потрібно буде `await`(чекати) її.

Створіть константу під назвою `data` та призначте до неї значення `await res.json()`.
...
Призначте `await res.json()` до змінної `data`.
