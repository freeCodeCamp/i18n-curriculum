...
Un ordinamento per selezione si basa sul tenere traccia dell'indice del valore più piccolo nell'array. Dichiara una variabile `minIndex` e impostala su `i` — questo garantisce che se il valore corrente è il più piccolo, verrà scambiato con se stesso e non spostato. Dovrai poter riassegnare il valore di `minIndex` mentre iteri sull'array.

Poi, scrivi un altro loop `for`, usando `j` come iteratore. Questo loop deve iniziare dall'indice successivo a `i` e iterare sul resto dell'array.

# --hints--

Dovresti usare `let` per dichiarare `minIndex`.
...
Dovresti impostare `minIndex` su `i`.
...
Dovresti avere un loop `for` annidato.
...
Il tuo loop `for` dovrebbe inizializzare `j` a `i + 1`.
...
Il tuo loop `for` dovrebbe iterare sul resto dell'array.
...
Il tuo loop `for` dovrebbe incrementare `j`.
