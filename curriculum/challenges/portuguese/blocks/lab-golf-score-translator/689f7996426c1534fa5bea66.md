...
1. `golfScore` deve retornar `"Eagle"` se `strokes` for menor ou igual a `par` menos `2`.
...
1. `golfScore` deve retornar `"Go Home!"` se `strokes` for maior ou igual a `par` mais `3`.
