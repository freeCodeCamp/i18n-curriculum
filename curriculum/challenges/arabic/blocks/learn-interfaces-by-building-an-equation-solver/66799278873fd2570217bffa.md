...
أضف `case` آخر عندما يكون طول `results` هو `1`. في هذه الحالة، عيّن إلى `result_list` قائمة تحتوي على سلسلة نصية بالشكل `x = <root>`، حيث `<root>` هو حل المعادلة. نسق السلسلة النصية بحيث يظهر كل من الإشارة الموجبة والسالبة للحل.

# --hints--

لا تعدل قيمة الموضوع في جملة `match` الخاصة بك.
...
أنشئ `case` جديد بالنمط `1` بعد كتلة `case` الموجودة.
...
عيّن قائمة تحتوي على `f'x = {results[0]:+}'` إلى `result_list` داخل جسم `case` الجديد.
