...
`.formrow` 要素に上マージンと左右のパディングを設定してください。その他のパディングの値は `0` にしてください。

次に、すべての `input` 要素のフォントサイズを大きくしてください。

# --hints--

`.formrow` 要素をターゲットにするには `.formrow` セレクターを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.formrow'));
```

`.formrow` に少なくとも `margin-top` の `1px` を与えるべきです。
...
`.formrow` に `padding-top` の `0` を与えるべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingTop, '0px');
```

`.formrow` に少なくとも `padding-right` の `1px` を与えるべきです。
...
`.formrow` に `padding-bottom` の `0` を与えるべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingBottom, '0px');
```

`.formrow` に少なくとも `padding-left` の `1px` を与えるべきです。
...
`input` 要素をターゲットにするには `input` セレクターを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input'));
```

`input` に `font-size` より大きい `13px` を与えるべきです。
