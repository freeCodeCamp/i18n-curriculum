...
다음으로, 재생 목록에서 다음 노래를 가져와야 합니다. 이를 위해 현재 노래의 인덱스를 얻은 다음 `1`을 더해야 합니다.

`nextSong`라는 상수를 만들고 `userData?.songs[currentSongIndex + 1]`를 할당하세요.

마지막으로, `playSong` 함수를 호출하고 `nextSong.id`를 인수로 전달하세요.

# --hints--

기존 `if` 문장과 그 `else` 블록 및 내용을 수정하지 마세요.
...
`userData?.songs[currentSongIndex + 1]`를 `nextSong` 상수에 할당해야 합니다.
...
`playSong` 함수를 `nextSong.id`와 함께 호출해야 합니다.
