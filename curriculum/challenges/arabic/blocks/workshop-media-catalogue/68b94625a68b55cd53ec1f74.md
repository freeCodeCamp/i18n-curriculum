...
يمكن الوصول إلى docstrings عبر الخاصية `__doc__`، التي تُعيّن إلى `None` افتراضيًا.

تحقق من ذلك بطباعة `movie1.__doc__` و `series1.__doc__`. لاحظ أن الخاصية `__doc__` لا تُورث لأي صنف فرعي.

# --hints--

يجب طباعة `movie1.__doc__`.
