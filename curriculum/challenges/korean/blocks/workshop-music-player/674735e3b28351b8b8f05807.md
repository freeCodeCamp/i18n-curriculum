...
`find()` 메서드는 제공된 콜백 함수에서 지정한 조건을 충족하는 배열 내 첫 번째 요소를 가져옵니다. 조건을 만족하는 요소가 없으면 이 메서드는 `undefined`을 반환합니다.
...
`playSong` 함수 내에서 `find()` 배열에 `userData.songs` 메서드를 사용해 `id` 함수에 전달된 `id`과 엄격히 같은 `playSong`를 가진 노래를 검색하세요. `find` 호출 결과를 `song`이라는 변수에 할당하세요.

# --hints--

`playSong` 함수는 `find`에서 `userData.songs` 메서드를 호출해야 합니다.
...
`find` 호출은 `id` 함수에 전달된 `id`과 엄격히 같은 `playSong`를 가진 노래를 반환해야 합니다.
...
`find` 호출 결과를 `song`라는 변수에 할당해야 합니다.
