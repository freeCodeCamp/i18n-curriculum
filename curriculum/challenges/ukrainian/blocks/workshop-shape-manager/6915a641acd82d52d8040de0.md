...
Повертаючись до функції `initializeApp`, тут ви використаєте рядкові типи та функцію `getElement()`, яку створили раніше, отримуючи потрібні DOM-елементи для застосунку.

Всередині функції `initializeApp` переназначте `shapeTypeSelect`, викликавши `getElement()` з `"shape-type"`, а потім використайте ключове слово `as`, щоб привести результат до `HTMLSelectElement`.

# --hints--

Ви повинні переназначити `shapeTypeSelect` відповідним значенням.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

Ви повинні викликати функцію `getElement` з `shape-type` і призначити це `shapeTypeSelect`.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

Ви повинні викликати `getElement` з `HTMLSelectElement` як тип `shapeTypeSelect`, використовуючи ключове слово `as`.
