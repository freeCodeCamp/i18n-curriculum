...
`fn` 매개변수에는 `"SUM"` 같은 함수 이름이 전달됩니다. `apply` 변수를 객체 접근 키로 사용하여 `spreadsheetFunctions` 객체에서 함수를 암묵적으로 반환하도록 `fn`를 업데이트하세요.

`fn`가 소문자 문자열이 아닐 수도 있으니, 소문자 문자열로 변환해야 한다는 점을 기억하세요.

# --hints--

`apply` 함수는 암묵적 반환을 사용해야 합니다.
...
`apply` 함수는 `spreadsheetFunctions` 객체에 접근해야 합니다.
...
`apply` 함수는 `spreadsheetFunctions` 값과 일치하는 `fn` 객체의 속성에 접근해야 합니다.
...
`apply` 함수는 속성 접근에서 `.toLowerCase()`의 `fn` 메서드를 호출해야 합니다.
