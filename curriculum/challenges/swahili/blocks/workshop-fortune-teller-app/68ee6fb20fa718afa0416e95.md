...
Kama ulivyofanya kwa `multipleCard` iliyopita, pata tena kipengele cha `multipleCard` na weka `innerHTML` kwa kuzunguka kupitia `DrawingType`. Tumia `Object.values()` na fanya wito wa njia ya `map` juu yake, ukipitisha `type` kama hoja.

# --hints--

Unapaswa kuweka thamani ya `multipleCard.innerHTML` kwa kufanya wito wa `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

Unapaswa kupitisha `DrawingType` kwa njia ya `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

Unapaswa kufanya wito wa njia ya `map` juu ya `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Njia yako ya `map` inapaswa kuwa na kigezo kinachoitwa `type`.
