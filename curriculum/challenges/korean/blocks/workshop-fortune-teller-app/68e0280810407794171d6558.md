...
이제 `div`를 클래스 `img-loader`와 함께 추가하세요. 그다음 `img` 요소를 만들고 `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}`를 사용해 이미지에 조건부 URL을 지정하세요.

# --hints--

`div` 요소 안에 `figure` 요소가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

`div`에는 클래스 `img-loader`가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

`img` 요소 안에 `figure` 요소가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

`img` 요소는 `CDN_URL`가 제공될 때 `img`과 `src`를 `img`로 사용하고, 제공되지 않을 때는 `LOCAL_DEFAULT_IMG`로 대체해야 합니다.
...
`img` 요소에는 클래스 `card-img`와 `hidden`가 있어야 합니다.
