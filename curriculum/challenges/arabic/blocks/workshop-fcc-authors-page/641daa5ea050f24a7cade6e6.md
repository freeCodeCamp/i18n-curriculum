...
إذا تحقق هذا الشرط، عطّل الزر بتعيين الخاصية `disabled` إلى `true`. أيضًا، عيّن الخاصية `textContent` للزر إلى `"No more data to load"`.
...
يجب تعيين خاصية `disabled` للعنصر `loadMoreBtn` إلى `true`.
