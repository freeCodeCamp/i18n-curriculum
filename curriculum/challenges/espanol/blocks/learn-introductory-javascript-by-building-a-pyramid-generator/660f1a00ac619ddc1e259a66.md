...
Por ahora, usarás el operador <dfn>menor que</dfn> (`<`). Esto te permite verificar si el valor a la izquierda es menor que el valor a la derecha. Por ejemplo, `count < 3` evaluaría a `true` si `count` es `2`, y a `false` si `count` es `4`.
...
Tu ciclo `for` debe usar `i < count` como condición.
