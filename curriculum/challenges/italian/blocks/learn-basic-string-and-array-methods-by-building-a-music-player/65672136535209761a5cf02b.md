...
Per far partire la canzone ogni volta che l’utente ci clicca sopra, aggiungi un attributo `onclick` al primo elemento button. All’interno di `onclick`, chiama la funzione `playSong` con `song.id`.

Non dimenticare che qui devi interpolare con il simbolo del dollaro.

# --hints--

Dovresti aggiungere un attributo `onclick` con valore `playSong(${song.id})` all’elemento `button`.
