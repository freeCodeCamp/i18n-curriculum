...
Додайте блок `else` до інструкції `if`. Усередині блоку `else` викличте функцію `getCurrentSongIndex()` і призначте її константі з ім’ям `currentSongIndex`.

# --hints--

Ви не повинні змінювати існуючу інструкцію `if`.
...
Ви повинні додати `else` до існуючої інструкції `if`.
...
Ви повинні призначити константі `currentSongIndex` значення `getCurrentSongIndex()` всередині блоку `else`.
