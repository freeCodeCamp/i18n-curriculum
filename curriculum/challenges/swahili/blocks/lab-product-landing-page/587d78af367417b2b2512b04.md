...
title: Jenga ukurasa wa kutua wa bidhaa
challengeType: 25
...
1. Ukurasa wako wa kutua wa bidhaa unapaswa kuwa na kipengele cha `header` chenye `id="header"` inayolingana.
2. Unapaswa kuwa na picha ndani ya kipengele cha `header` chenye `id="header-img"` inayolingana (Nembo itakuwa picha nzuri hapa).
3. Ndani ya kipengele cha `#header`, unapaswa kuwa na kipengele cha `nav` chenye `id="nav-bar"` inayolingana.
4. Unapaswa kuwa na angalau vipengele vitatu vinavyoweza kubofywa ndani ya kipengele cha `nav`, kila kikiwa na darasa la vitu la `nav-link`.
5. Unapobofya kitufe cha `.nav-link` kilicho ndani ya kipengele cha `nav`, unapaswa kupelekwa kwenye sehemu inayolingana ya ukurasa wa kutua.
6. Unapaswa kuwa na video ya bidhaa iliyojumuishwa na `id="video"`.
7. Ukurasa wako wa kutua unapaswa kuwa na kipengele cha `form` chenye `id="form"` inayolingana.
8. Ndani ya fomu, kunapaswa kuwepo na sehemu ya ingizo ya `input` yenye `id="email"` ambapo unaweza kuingiza anwani ya barua pepe.
9. Sehemu ya ingizo ya `#email` inapaswa kuwa na maandishi ya kiwakilishi ili kuwajulisha watumizi ni kwa ajili gani sehemu hiyo.
10. Sehemu ya ingizo ya `#email` inapaswa kutumia uthibitisho wa HTML5 kuthibitisha kuwa maandishi yaliyoingizwa ni anwani ya barua pepe.
11. Ndani ya fomu, kunapaswa kuwepo na kitufe cha kuwasilisha cha `input` chenye `id="submit"` inayolingana.
12. Unapobofya kipengele cha `#submit`, barua pepe inapaswa kuwasilishwa kwenye ukurasa wa statiki (tumia URL hii ya mfano: `https://www.freecodecamp.org/email-submit`).
13. Navbar inapaswa daima kuwa juu ya eneo la kuona.
14. Ukurasa wako wa kutua wa bidhaa unapaswa kuwa na angalau media query moja.
15. Ukurasa wako wa kutua wa bidhaa unapaswa kutumia CSS flexbox angalau mara moja.
...
Thamani ya `#header-img` ya `src` yako inapaswa kuwa URL halali (inaanza na `http`).
...
`#email` yako inapaswa kutumia uthibitisho wa HTML5 kwa kuweka `type` yake kuwa `email`.
...
`#form` yako inapaswa kuwa na sifa ya kipengele ya `action` yenye thamani ya `https://www.freecodecamp.org/email-submit`.
...
`#nav-bar` yako inapaswa daima kuwa juu ya eneo la kuona.
