...
قم بتسكين عنصر `select` (مع عناصر `option` الخاصة به) داخل عنصر `label` مع النص `How did you hear about us?`. يجب أن يأتي النص قبل عنصر `select`.

# --hints--

يجب أن تسكن فقط عنصر `select` داخل عنصر `label`.
...
يجب أن تضع النص قبل عنصر `select`.
...
يجب أن يحتوي عنصر `select` على 5 عناصر `option`.
