...
Il selettore `.cabin` dovrebbe avere una proprietà `position` con il valore `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cabin')?.position === 'absolute');
```

Il selettore `.cabin` dovrebbe avere una proprietà `border` con il valore `2px solid`.
