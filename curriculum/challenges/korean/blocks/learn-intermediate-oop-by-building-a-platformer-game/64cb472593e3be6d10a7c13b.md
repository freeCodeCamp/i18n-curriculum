...
조건문 안에 `forEach` 반복문을 추가하여 `checkpoints` 배열을 순차적으로 반복하세요. 콜백 함수의 매개변수 이름으로 `checkpoint`를 사용하세요.

`forEach` 콜백 안에서 대입 연산자 중 뺄셈 할당 연산자를 사용해 checkpoints의 `x` 위치에서 5를 빼세요.

# --hints--

`forEach` 반복문이 `checkpoints` 배열을 순차적으로 반복해야 합니다.
...
`checkpoint` 매개변수가 `forEach` 콜백 함수 안에 있어야 합니다.
...
뺄셈 할당 연산자를 사용해 `checkpoint`의 `x` 위치에서 5를 빼야 합니다.
