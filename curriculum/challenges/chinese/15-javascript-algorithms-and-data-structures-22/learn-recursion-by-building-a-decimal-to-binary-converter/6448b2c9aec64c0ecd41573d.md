...
在 `checkUserInput` 函数中，删除 `console.log()` 语句。 然后，调用 `decimalToBinary` 函数并将 `numberInput` 的 `value` 作为参数传入。 另外，请确保使用 `parseInt()` 函数将输入转换为数字。

# --hints--

你应该从 `checkUserInput` 函数中删除 `console.log()` 语句。

```js
assert.notMatch(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*console\.log\((\s|.)*\)/);
```

你应该在 `checkUserInput` 函数主体内的 `if` 语句之后调用 `decimalToBinary` 函数。

```js
assert.match(String(checkUserInput), /(?:window\.|globalThis\.)?alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*decimalToBinary\(/);
```

你应该使用 `parseInt()` 将 `numberInput` 的 `value` 转换为数字，并将其作为参数传递给 `decimalToBinary` 函数。
