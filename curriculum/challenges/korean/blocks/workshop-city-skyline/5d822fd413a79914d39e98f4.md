...
그라디언트는 이렇게 원하는 만큼 많은 색상을 사용할 수 있습니다:
...
`linear-gradient`에 `.bb1d`를 첫 번째 색상, `orange`를 두 번째, `--building-color1`를 세 번째 색상으로 하여 `--window-color1`을 추가하세요. 그라디언트를 `background` 속성에 사용하는 것을 잊지 마세요.

# --hints--

`background`에 `.bb1d`를 사용하세요.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background);
```

`background` 속성에 `linear-gradient`를 지정하세요.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'linear-gradient');
```

`orange`에서 첫 번째 색상으로 `linear-gradient`를 사용하세요.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.getPropVal('background', true), 'linear-gradient(orange');
```

`--building-color1`에서 두 번째 색상으로 `linear-gradient`를 사용하세요.
...
`--window-color1`에서 세 번째 색상으로 `linear-gradient`를 사용하세요.
