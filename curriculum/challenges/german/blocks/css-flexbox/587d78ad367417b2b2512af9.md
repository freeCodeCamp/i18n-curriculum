...
title: Die Eigenschaft align-items im Tweet-Embed verwenden
challengeType: 0
...
Die letzte Aufgabe hat die Eigenschaft `align-items` eingeführt und ein Beispiel gegeben. Diese Eigenschaft kann auf einige Elemente des Tweet-Embeds angewendet werden, um die Flex-Elemente darin auszurichten.

# --instructions--

Fügen Sie die CSS-Eigenschaft `align-items` zum `.follow-btn`-Element der Kopfzeile hinzu. Setzen Sie den Wert auf `center`.

# --hints--

Ihr `.follow-btn` sollte auf der Seite gerendert werden. Stellen Sie sicher, dass Sie alle Erweiterungen wie Werbeblocker deaktivieren.
...
Das `.follow-btn`-Element sollte die Eigenschaft `align-items` mit dem Wert `center` haben.
