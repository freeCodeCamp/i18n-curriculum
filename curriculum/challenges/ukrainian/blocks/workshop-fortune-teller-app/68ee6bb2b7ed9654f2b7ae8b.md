...
Додайте ще одного слухача подій до глобального об’єкта `document`, який викликає функцію `showFortune`. Усередині функції зворотного виклику додайте параметр `e` типу `Event` і передайте `e` як аргумент методу `showFortune`.

`showFortune` оброблятиме події натискання на картку, наприклад, зчитування ідентифікатора натиснутої картки з DOM і відображення відповідних деталей передбачення картки. Ви створите цю функцію в наступних кроках.

# --hints--

Ви повинні призначити слухача подій `click` глобальному об’єкту `document`.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Ваш слухач подій має мати параметр функції зворотного виклику з ім’ям `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

Параметр функції зворотного виклику має бути типу `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

Всередині функції зворотного виклику ви маєте викликати функцію `showFortune`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

У функції `showFortune` ви маєте передати аргумент `e`.
