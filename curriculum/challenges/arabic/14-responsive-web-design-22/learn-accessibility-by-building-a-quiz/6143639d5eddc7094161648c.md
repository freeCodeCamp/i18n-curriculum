...
يتم ذلك من خلال إعطاء الـ `label` السمة `for`، والتي تحتوي على `id` بقيمة الـ `input`.

سيأخذ هذا القسم اسم الطالب، وعنوان البريد الإلكتروني، وتاريخ الميلاد. قم بإعطاء عناصر الـ `label` سمات `for` مناسبة، وكذلك محتوى النص. ثم قم بربط عناصر `input` إلى عناصر الـ `label` المقابلة لها.

# --hints--

يجب أن تعطي أول `label` سمة `for` مناسبة.

```js
assert.isAtLeast(document.querySelectorAll('label')?.[0]?.htmlFor?.length, 1);
```

يجب أن تعطي ثاني `label` سمة `for` مناسبة.

```js
assert.isAtLeast(document.querySelectorAll('label')?.[1]?.htmlFor?.length, 1);
```

يجب أن تعطي ثالث `label` سمة `for` مناسبة.

```js
assert.isAtLeast(document.querySelectorAll('label')?.[2]?.htmlFor?.length, 1);
```

يجب عليك إعطاء أول عنصر `label` محتوى نصي مناسب.

```js
assert.isAtLeast(document.querySelectorAll('label')?.[0]?.textContent?.length, 1);
```

يجب عليك إعطاء ثاتي عنصر `label` محتوى نصي مناسب.

```js
assert.isAtLeast(document.querySelectorAll('label')?.[1]?.textContent?.length, 1);
```

يجب عليك إعطاء ثالث عنصر `label` محتوى نصي مناسب.

```js
assert.isAtLeast(document.querySelectorAll('label')?.[2]?.textContent?.length, 1);
```

يجب أن تعطي عنصر `input` الاول، السمة `id` التي تطابق سمة `for` للـ `label` الاول.

```js
assert.equal(document.querySelectorAll('input')?.[0]?.id, document.querySelectorAll('label')?.[0]?.htmlFor);
```

يجب أن تعطي عنصر `input` الثاني، السمة `id` التي تطابق سمة `for` للـ `label` الثاني.

```js
assert.equal(document.querySelectorAll('input')?.[1]?.id, document.querySelectorAll('label')?.[1]?.htmlFor);
```

يجب أن تعطي عنصر `input` الثالث، السمة `id` التي تطابق سمة `for` للـ `label` الثالث.

```js
assert.equal(document.querySelectorAll('input')?.[2]?.id, document.querySelectorAll('label')?.[2]?.htmlFor);
```

يجب ألا تستخدم نفس سمة الـ `id` لأكثر من عنصر `input`.
...
يجب ألا تستخدم نفس سمة الـ `for` لأكثر من عنصر `label`.
