...
Para lograr el resultado "ascendente", debes verificar si el elemento actual es más grande que el siguiente elemento. Puede hacer esto accediendo al `array` en `j` y `j+1`.

Crea una condición `if` que compruebe si el elemento actual es más grande que el siguiente elemento.

# --hints--

No debes eliminar tu instrucción `console.log`.
...
Debes usar una instrucción `if` en tu bucle anidado.
...
Tu instrucción `if` debe verificar si el `array[j]` es más grande que el `array[j+1]`.
