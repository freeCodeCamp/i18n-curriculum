...
使用箭頭函式語法創建一個 `fetchMoreAuthors` 函式（程式）。暫時不要在裡面放任何東西。請確保你使用大括號，因為你會在函式（程式）內有多於一個的表達式。
...
你的 `fetchMoreAuthors` 函式（程式）應該使用箭頭語法。

```js
assert.match(code, /const\s+fetchMoreAuthors\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `fetchMoreAuthors` 函式（程式）應該是空的。
