...
In this lab, you will build a freeCodeCamp forum leaderboard that displays the latest topics, users, and replies from the [freeCodeCamp forum](https://forum.freecodecamp.org/). O HTML, CSS e parte do JS foram fornecidos para você. Sinta-se à vontade para explorá-los.
...
1. Cada elemento `tr` deve conter cinco elementos `td`:
   ...
   - um `td` contendo o número de respostas ao post. *Dica:* use `posts_count - 1`.
...
O terceiro elemento `td` de cada linha da tabela da string retornada por `showLatestPosts` deve conter o número de respostas ao post. *Dica:* use `posts_count - 1`.
