...
Geben Sie Ihrem `conversionTable` schließlich ein `teaspoon`-Objekt mit einer `cup`-Eigenschaft, die auf `1/48` gesetzt ist, einer `gram`-Eigenschaft, die auf `5` gesetzt ist, und einer `ounce`-Eigenschaft, die auf `0.167` gesetzt ist.

# --hints--

Ihr `conversionTable` sollte eine `teaspoon`-Eigenschaft haben.

```js
assert.property(conversionTable, "teaspoon");
```

Ihre `teaspoon`-Eigenschaft sollte ein Objekt sein.

```js
assert.isObject(conversionTable.teaspoon);
```

Ihr `teaspoon`-Objekt sollte eine `cup`-Eigenschaft haben.

```js
assert.property(conversionTable.teaspoon, "cup");
```

Ihre `cup`-Eigenschaft sollte auf `1/48` gesetzt sein.

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

Ihr `teaspoon`-Objekt sollte eine `gram`-Eigenschaft haben.

```js
assert.property(conversionTable.teaspoon, "gram");
```

Ihre `gram`-Eigenschaft sollte auf `5` gesetzt sein.

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

Ihr `teaspoon`-Objekt sollte eine `ounce`-Eigenschaft haben.

```js
assert.property(conversionTable.teaspoon, "ounce");
```

Ihre `ounce`-Eigenschaft sollte auf `0.167` gesetzt sein.
