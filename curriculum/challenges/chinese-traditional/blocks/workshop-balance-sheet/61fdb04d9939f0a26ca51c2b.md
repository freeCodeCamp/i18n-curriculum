...
是時候為你的表格標題設計樣式了。創建一個 `h1` 選擇器。為它添加一個 `max-width` 屬性，設定為 `37.25rem`，一個 `margin` 屬性，設定為 `0 auto`，以及一個 `padding` 屬性，設定為 `1.5rem 1.25rem`。
...
你的 `h1` 選擇器應該設定 `max-width` 屬性為 `37.25rem`。
...
你的 `h1` 選擇器應該有一個 `margin` 屬性設定為 `0 auto`。
...
你的 `h1` 選擇器應該設定 `padding` 屬性為 `1.5rem 1.25rem`。
