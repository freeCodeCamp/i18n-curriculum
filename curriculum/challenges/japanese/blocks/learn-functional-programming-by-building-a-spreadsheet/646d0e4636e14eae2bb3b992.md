...
`addCharacters` チェーンの最も内側の関数を更新して、`charRange()` と `character1` を引数として `character2` を呼び出した結果を暗黙的に返すようにしてください。

# --hints--

最も内側の関数は暗黙の return を使う必要があります。
...
最も内側の関数は `charRange()` を呼び出した結果を返す必要があります。
...
`character1` の呼び出しに `charRange()` を最初の引数として渡してください。
...
`character2` の呼び出しに `charRange()` を2番目の引数として渡してください。
