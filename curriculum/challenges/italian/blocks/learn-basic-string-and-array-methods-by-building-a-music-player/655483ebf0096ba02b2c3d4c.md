...
Per ottenere la canzone precedente, sottrai `1` dal `currentSongIndex` di `userData?.songs` e assegnalo alla costante `previousSong`. Dopo, chiama la funzione `playSong` e passa `previousSong.id` come argomento.
...
Dovresti assegnare `userData?.songs[currentSongIndex - 1]` a una costante `previousSong`.
...
Dovresti chiamare la funzione `playSong` con `previousSong.id`.
