...
المستطيل المعطى مكوّن من مربعات بأبعاد *m* × *n*. إذا لم يكن *m* و *n* كلاهما فرديين، فيمكن قطع مسار عبر المستطيل على طول حواف المربعات بحيث ينقسم المستطيل إلى قطعتين متصلتين لهما نفس الشكل (بعد تدوير إحدى القطعتين 180°). جميع هذه المسارات للمستطيلات 2 × 2 و 4 × 3 موضحة أدناه.
...
<div style="width: 100%; text-align: center;">
  <svg xmlns="https://www.w3.org/2000/svg" xmlns:xlink="https://www.w3.org/1999/xlink" width="520" height="170" aria-hidden="true" alt="Diagram showing the possible paths for 2 by 2 and 4 by 3 rectangles">
    <style>
.diagram-g { لا تعبئة; حد: #ccc }
      .diagram-s, .diagram-s2 { تعبئة: #bff; حد: أسود; شفافية التعبئة: .4 }
      .diagram-s2 { تعبئة: #fbf }
      .diagram-d { حد: أسود; لا تعبئة }
    </style>
    <defs>
      <g id="diagram-m">
        <g id="diagram-h4">
...
اكتب دالة تحسب عدد الطرق المختلفة لقص مستطيل بأبعاد *m* × *n*.

# --hints--

يجب أن تكون `cutRectangle` دالة.

```js
assert(typeof cutRectangle == 'function');
```

يجب أن تُرجع `cutRectangle(2, 2)` رقمًا.

```js
assert(typeof cutRectangle(2, 2) == 'number');
```

يجب أن تُرجع `cutRectangle(2, 2)` القيمة `2`.

```js
assert.equal(cutRectangle(2, 2), 2);
```

يجب أن تُرجع `cutRectangle(4, 3)` القيمة `9`.

```js
assert.equal(cutRectangle(4, 3), 9);
```

يجب أن تُرجع `cutRectangle(4, 4)` القيمة `22`.

```js
assert.equal(cutRectangle(4, 4), 22);
```

يجب أن تُرجع `cutRectangle(8, 3)` القيمة `53`.

```js
assert.equal(cutRectangle(8, 3), 53);
```

يجب أن تُرجع `cutRectangle(7, 4)` القيمة `151`.
