...
Agora é hora de definir a aba clicada para ser selecionada.

Use o método `setAttribute` no `tab` para definir o atributo `aria-selected` como `"true"`. 

# --hints--

Quando um `tab` é clicado, o atributo `aria-selected` do `tab` clicado deve ser definido como `"true"`.
