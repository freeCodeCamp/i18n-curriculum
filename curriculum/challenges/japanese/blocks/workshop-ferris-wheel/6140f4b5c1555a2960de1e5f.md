...
`@keyframes` という名前の別のルールを `cabins` で作成してください。`@keyframes wheel` と同じプロパティを使い、`0%` と `100%` の両方のルールをコピーしますが、`transform` セレクターの `100%` プロパティは `rotate(-360deg)` に設定してください。
...
新しい `@keyframes` ルールは `cabins` という名前にしてください。
...
新しい `@keyframes` ルールは `@keyframes wheel` ルールの後に配置してください。
...
`0%` セレクターには `transform` プロパティが `rotate(0deg)` に設定されている必要があります。
...
`100%` セレクターには `transform` プロパティが `rotate(-360deg)` に設定されている必要があります。
