...
Тепер ви додасте логіку для збереження у `localStorage` за допомогою `useEffect`.

Доступ або збереження даних у `localStorage` вважається побічним ефектом у React, тому для цієї логіки слід використовувати `useEffect`.

Почніть із деструктуризації `useEffect` з React, так само як це було зроблено з `useState`.

# --hints--

Ви повинні деструктуризувати `useEffect` з `React`.
