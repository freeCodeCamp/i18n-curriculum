...
Adicione outro listener de evento ao objeto global `document` que faça a chamada da função `showFortune`. Dentro do callback, inclua um parâmetro `e` do tipo `Event` e passe `e` como argumento para o método `showFortune`.

`showFortune` vai lidar com eventos de clique no cartão, como ler o id do cartão clicado no DOM e exibir os detalhes da sorte correspondente ao cartão. Você vai criá-lo nos próximos passos.

# --hints--

Você deve atribuir um listener de evento `click` ao objeto global `document`.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Seu listener de evento deve ter um parâmetro de callback chamado `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

O parâmetro do seu callback deve ter o tipo `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

Dentro do callback, você deve chamar a função `showFortune`

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

Dentro da função `showFortune`, você deve passar o argumento `e`.
