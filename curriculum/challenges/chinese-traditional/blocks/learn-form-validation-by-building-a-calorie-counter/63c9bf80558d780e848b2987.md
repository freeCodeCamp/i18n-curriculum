...
宣告一個 `breakfastNumberInputs` 變數，並將呼叫 `document.querySelectorAll()` 並使用選擇器 `#breakfast input[type='number']` 的結果指定給它。這將傳回位於 `#breakfast` 元素中的任何 `number` 輸入。

# --hints--

你應該宣告一個 `breakfastNumberInputs` 變數。

```js
assert.match(calculateCalories.toString(), /breakfastNumberInputs\s*=/);
```

你應該使用 `#breakfast input[type='number']` 呼叫 `document.querySelectorAll()`。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#breakfast input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你應該將 `document.querySelectorAll()` 呼叫的結果指定給 `breakfastNumberInputs`。
