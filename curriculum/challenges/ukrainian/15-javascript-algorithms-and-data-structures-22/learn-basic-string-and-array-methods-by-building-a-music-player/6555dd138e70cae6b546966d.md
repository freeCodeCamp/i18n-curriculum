...
Вам потрібно отримати пісню, яка зараз відтворюється, або першу пісню в списку відтворення. Для цього створіть константу `song` та використайте оператор OR(АБО) (`||`), щоб встановіть її на поточну пісню `userData`, **або** на першу пісню в масиві `userData?.songs`.

Не забудьте використати необов’язковий ланцюжок.

# --hints--

Не змінюйте наявну `setPlayButtonAccessibleText` та її вміст.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Ви повинні використати `userData?.currentSong` або `userData?.songs[0]`.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

Призначте `userData?.currentSong || userData?.songs[0]` до константи `song`.
