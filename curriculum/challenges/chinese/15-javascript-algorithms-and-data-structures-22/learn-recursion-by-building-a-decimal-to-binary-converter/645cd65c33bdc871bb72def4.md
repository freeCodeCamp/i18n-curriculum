...
虽然这是一个简单的例子，但它演示了调用堆栈如何逐步执行代码并调用多个函数。

现在是时候进入递归，看看调用堆栈是如何发挥作用的。

删除 `callStack` 数组、`a()`、`b()` 和 `c()` 函数以及 `console.log()` 语句。

# --hints--

你应该从代码中删除 `callStack` 数组。

```js
assert.notMatch(code, /(var|let|const)\s+callStack\s*=\s*\[\s*\]/);
```

你应该从代码中删除 `a()`、`b()` 和 `c()` 函数。
