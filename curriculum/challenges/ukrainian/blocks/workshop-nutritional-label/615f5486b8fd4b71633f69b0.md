...
Під вашим елементом `.small-text` створіть новий елемент `p` з текстом `Calories`. Також під елементом `.left-container` створіть новий елемент `span` з текстом `230`.

# --hints--

У вашому елементі `p` має бути новий елемент `.calories-info`.

```js
assert(document.querySelector('.calories-info')?.querySelector('p'))
```

У вашому елементі `span` має бути новий елемент `.calories-info`.

```js
assert(document.querySelector('.calories-info')?.querySelector('span'))
```

Ваш елемент `p` має йти після елемента `.small-text`.

```js
assert(document.querySelector('.small-text')?.nextElementSibling?.localName === 'p');
```

Ваш елемент `span` має йти після елемента `.left-container`.

```js
assert(document.querySelector('.left-container')?.nextElementSibling?.localName ==='span');
```

Ваш елемент `p` має містити текст `Calories`.

```js
assert(document.querySelector('.small-text')?.nextElementSibling?.innerText === 'Calories');
```

Ваш елемент `span` має містити текст `230`.
