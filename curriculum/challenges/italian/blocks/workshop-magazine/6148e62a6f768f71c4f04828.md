...
La proprietà `place-items` può essere usata per impostare contemporaneamente i valori `align-items` e `justify-items`. La proprietà `place-items` accetta uno o due valori. Se viene fornito un solo valore, viene usato sia per la proprietà `align-items` sia per la proprietà `justify-items`. Se vengono forniti due valori, il primo viene usato per la proprietà `align-items` e il secondo per la proprietà `justify-items`.

Imposta la proprietà `.image-wrapper` su `place-items` per il selettore `center`.

# --hints--

Il tuo selettore `.image-wrapper` dovrebbe avere una proprietà `place-items` impostata su `center`.
