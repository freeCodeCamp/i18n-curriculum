...
Wählen Sie den `#result` Absatz aus und speichern Sie ihn in einer Variablen namens `resultParagraph`. Sie werden diese Variable verwenden, um die ausgewählte Geschichte anzuzeigen.

# --hints--

Sie sollten den `#result` Absatz mit der `getElementById` Methode auswählen und in der `resultParagraph` Variablen speichern.
