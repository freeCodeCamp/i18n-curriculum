...
Acesse os elementos `#next`, `#previous` e `#shuffle` do arquivo HTML.

Atribua-os a variáveis chamadas `nextButton`, `previousButton` e `shuffleButton`, respectivamente.

# --hints--

Você deve ter uma variável chamada `nextButton`.

```js
assert.isDefined(nextButton);
```

Você deve usar `document.getElementById()` para obter o elemento `#next`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

Você deve atribuir o elemento `#next` à variável `nextButton`. 

```js
assert.equal(nextButton, document.getElementById('next'));
```

Você deve ter uma variável chamada `previousButton`.

```js
assert.isDefined(previousButton);
```

Você deve usar `document.getElementById()` para obter o elemento `#previous`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

Você deve atribuir o elemento `#previous` à variável `previousButton`. 

```js
assert.equal(previousButton, document.getElementById('previous'));
```

Você deve ter uma variável chamada `shuffleButton`.

```js
assert.isDefined(shuffleButton);
```

Você deve usar `document.getElementById()` para obter o elemento `#shuffle`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

Você deve atribuir o elemento `#shuffle` à variável `shuffleButton`. 
