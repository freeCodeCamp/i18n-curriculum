...
修改 `scaryStoryBtn` 事件监听器以使用参数 `"scary"` 调用 `displayStory` 函数。 另外，向 `funnyStoryBtn` 和 `adventureStoryBtn` 按钮添加 `click` 事件监听器，分别使用参数 `"funny"` 和 `"adventure"` 调用 `displayStory` 函数。

至此，你已成功完成研讨会。

# --hints--

当你点击 `scaryStoryBtn` 时，`#result` 段落应显示恐怖故事 `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`，并且边框颜色为 `#ee4b2b`。
...
当你单击 `funnyStoryBtn` 时，`#result` 段落应显示有趣的故事，边框颜色为 `#f1be32`。
...
当你单击 `adventureStoryBtn` 时，`#result` 段落应显示边框颜色为 `#acd157` 的冒险故事。
