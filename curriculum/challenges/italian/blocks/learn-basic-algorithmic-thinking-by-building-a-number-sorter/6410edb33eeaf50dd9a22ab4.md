...
Quando la tua condizione `if` è vera, devi scambiare i due elementi, "facendo risalire" l'elemento più grande verso la fine dell'array.

Per farlo, dichiara una variabile `temp` e assegnale il valore di `array[j]`. Poi assegna a `array[j]` il valore di `array[j + 1]`. Infine, assegna a `array[j + 1]` il valore di `temp`.

# --hints--

All'interno della tua istruzione `if`, dovresti dichiarare una variabile `temp`.
...
Dovresti assegnare a `temp` il valore di `array[j]`.
...
Dovresti assegnare a `array[j]` il valore di `array[j + 1]`.
...
Dovresti assegnare a `array[j + 1]` il valore di `temp`.
