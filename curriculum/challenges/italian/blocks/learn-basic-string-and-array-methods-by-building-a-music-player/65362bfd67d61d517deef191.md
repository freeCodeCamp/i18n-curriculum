...
Il metodo <dfn>find()</dfn> recupera il primo elemento all'interno di un array che soddisfa le condizioni specificate nella funzione di callback fornita. Se nessun elemento soddisfa la condizione, il metodo restituisce `undefined`.

Nell'esempio qui sotto, il metodo `find()` viene usato per trovare il primo numero maggiore di 25:
...
Usa `const` per creare una variabile chiamata `song` e assegnarle il risultato della chiamata al metodo `find()` sull'array `userData?.songs`. Usa `song` come parametro della callback `find()` e verifica se `song.id` è strettamente uguale a `id`.

Questo itererà sull'array `userData?.songs`, cercando una canzone che corrisponda a `id` passato alla funzione `playSong`.

# --hints--

Dovresti usare il metodo `find` su `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

Il tuo metodo `find` dovrebbe avere `song` come parametro della sua funzione freccia di callback.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

Il tuo metodo `find` dovrebbe usare l'uguaglianza stretta per verificare se `song.id` è uguale a `id`.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

Il tuo metodo `find` dovrebbe essere assegnato a una costante `song`.
