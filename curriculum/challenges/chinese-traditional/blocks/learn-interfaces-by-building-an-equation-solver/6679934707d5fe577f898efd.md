...
為 `results` 的長度為 `2` 時添加另一個情況。這次，將 `result_list` 指定為包含兩個字串的列表，格式為 `x1 = <root1>` 和 `x2 = <root2>`。同樣，使解法顯示正負號。
...
你應該在現有的 `case` 區塊後面創建一個樣式為 `2` 的新 `case`。
...
你應該在你的新 `case` 主體內，將一個包含兩個字串的列表指定給 `result_list`，字串格式為 `x1 = <root1>` 和 `x2 = <root2>`。結果需同時顯示正負號。
