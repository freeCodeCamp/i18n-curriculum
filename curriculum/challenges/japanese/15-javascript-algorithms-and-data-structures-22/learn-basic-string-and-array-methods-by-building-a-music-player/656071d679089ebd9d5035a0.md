...
ユーザーがプレイリストから曲をシャッフルしたり削除したりできるようにするために、元の `allSongs` 配列を変更 (ミューテート) せずにコピーを作成する必要があります。 このようなケースでは `spread` 演算子が役に立ちます。

スプレッド演算子 (`...`) を使うと、1 つの配列から別の配列へ、すべての要素をコピーできます。 また、複数の配列を 1 つに結合するためにも使用できます。 下記の例では、`arr1` と `arr2` の両方が `combinedArr` へとスプレッドされています。
...
では、`userData` オブジェクトの中に `songs` プロパティを作成してください。 その値として、`allSongs` を別の配列へとスプレッドしてください。

# --hints--

`userData` オブジェクトには `songs` というキーが必要です。

```js
assert.property(userData, "songs");
```

`songs` プロパティに `[...allSongs]` という値が必要です。
