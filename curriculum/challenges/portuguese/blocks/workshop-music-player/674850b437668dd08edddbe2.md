...
Nos passos anteriores você desenvolveu a funcionalidade para tocar uma música. Agora você precisa adicionar a funcionalidade ao botão de play.

Use o método `addEventListener()` em `playButton` e passe um evento `"click"` como primeiro argumento. Por enquanto, use um callback que chame `playSong(0)` como o segundo argumento.

Então, teste clicando no botão de play.


# --hints--

Você deve adicionar um event listener para o evento `click` no `playButton` e passar uma função callback que chama `playSong(0)` como o segundo argumento do método `addEventListener`.
