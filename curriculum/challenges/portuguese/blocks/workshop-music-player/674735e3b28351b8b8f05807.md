...
O método `find()` recupera o primeiro elemento dentro de um array que atende às condições especificadas na função callback fornecida. Se nenhum elemento satisfizer a condição, o método retorna `undefined`.
