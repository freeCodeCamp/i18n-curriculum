...
In früheren Modulen haben Sie gelernt, wie man ein `NodeList` so durchläuft:
...
Verwenden Sie die `forEach`-Methode, um das `tabs` zu durchlaufen. Für die Callback-Funktion verwenden Sie eine Pfeilfunktion, die einen einzelnen Parameter, `tab`, annimmt.

# --hints--

Sie sollten eine `tabs.forEach(tab => {})` in Ihrem Code haben.
