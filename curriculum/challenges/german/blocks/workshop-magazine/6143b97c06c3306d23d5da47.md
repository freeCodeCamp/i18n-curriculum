...
Erstellen Sie innerhalb Ihres `article`-Elements ein `h3`-Element mit dem `class`, das auf `list-title` gesetzt ist, und dem Text `A Brief History`. Darunter erstellen Sie ein `p`-Element mit dem Text `Of the Curriculum`. Erstellen Sie anschließend ein `ul`-Element mit der Klasse `lists`.

# --hints--

Sie sollten ein `h3`-Element innerhalb Ihres `article`-Elements erstellen.

```js
assert.exists(document.querySelector('article h3'));
```

Sie sollten ein `p`-Element innerhalb Ihres `article`-Elements erstellen.

```js
assert.exists(document.querySelector('article p'));
```

Sie sollten ein `ul`-Element innerhalb Ihres `article`-Elements erstellen.

```js
assert.exists(document.querySelector('article ul'));
```

Ihre Elemente innerhalb des `article`-Elements sollten in der richtigen Reihenfolge sein.
...
Ihr neues `h3`-Element sollte das `class` auf `list-title` gesetzt haben.

```js
assert.equal(document.querySelector('article h3')?.className, 'list-title');
```

Ihr neues `h3`-Element sollte den Text `A Brief History` haben.

```js
assert.equal(document.querySelector('article h3')?.innerText, 'A Brief History');
```

Ihr neues `p`-Element sollte den Text `Of the Curriculum` haben.

```js
assert.equal(document.querySelector('article p')?.innerText, 'Of the Curriculum');
```

Ihr neues `ul`-Element sollte das `class` auf `lists` gesetzt haben.
