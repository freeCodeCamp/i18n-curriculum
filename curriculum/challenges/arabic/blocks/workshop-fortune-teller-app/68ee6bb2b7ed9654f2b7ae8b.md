...
أضف مستمع حدث آخر إلى كائن `document` العالمي يستدعي الدالة `showFortune`. ضمن الاستدعاء الراجع، أدرج معلمة `e` من نوع `Event`، ومرر `e` كمعلمة إلى طريقة `showFortune`.

تتعامل `showFortune` مع أحداث النقر على البطاقة، مثل قراءة معرف البطاقة التي نُقرت من DOM وعرض تفاصيل الحظ المطابقة للبطاقة. ستنشئها في الخطوات القادمة.

# --hints--

يجب تعيين مستمع حدث `click` إلى كائن `document` العالمي.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

يجب أن يحتوي مستمع الحدث على معلمة استدعاء راجع باسم `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

يجب أن تكون معلمة الاستدعاء الراجع من النوع `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

داخل الاستدعاء الراجع، يجب استدعاء الدالة `showFortune`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

داخل الدالة `showFortune`، يجب تمرير المعلمة `e`.
