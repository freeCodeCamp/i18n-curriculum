...
All three checkboxes should have a `value` attribute. 檢查開始標籤的名稱後面是否有空格和/或所有屬性名稱之前是否有空格。
...
The `value` attribute of the `Loving` checkbox should be set to `loving`. 你要麼遺漏了屬性值，要麼拼寫有誤。 記住，屬性值應該放在引號中。
...
The `value` attribute of the `Lazy` checkbox should be set to `lazy`. 你要麼遺漏了屬性值，要麼拼寫有誤。 記住，屬性值應該放在引號中。
...
The `value` attribute of the `Energetic` checkbox should be set to `energetic`. 你要麼遺漏了屬性值，要麼拼寫有誤。 記住，屬性值應該放在引號中。
