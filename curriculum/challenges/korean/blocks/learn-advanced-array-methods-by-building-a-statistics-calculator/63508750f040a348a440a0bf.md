...
`mean`의 값을 표시하려면, 앱에 `#mean` 요소가 준비되어 있어야 합니다.

`.querySelector`를 사용해 그 요소를 찾고, `.textContent`을 `mean` 값으로 설정하세요.

# --hints--

`calculate` 함수는 `document.querySelector()`를 사용해야 합니다.

```js
assert.lengthOf(calculate.toString().match(/document\.querySelector\(/g), 2);
```

`.querySelector()`는 `#mean` 요소를 대상으로 해야 합니다.

```js
assert.match(calculate.toString(), /document\.querySelector\(\s*('|")#mean\1\s*\)/);
```

`calculate` 함수는 `#mean` 요소의 `.textContent`을 `mean` 값으로 설정해야 합니다.
