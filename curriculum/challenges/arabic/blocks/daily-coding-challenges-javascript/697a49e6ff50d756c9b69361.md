...
معطاة مصفوفة من أوقات النهاية لسباق التزلج الريفي، حوّلها إلى أوقات متأخرة عن الفائز.

- الأوقات المعطاة هي سلاسل نصية بصيغة `"H:MM:SS"`.
- الأوقات المعطاة مرتبة من الأسرع إلى الأبطأ.
- يجب أن يكون وقت الفائز (الأسرع) مساويًا لـ `"0"`.
- يجب أن يظهر كل وقت آخر الوقت المتأخر عن الفائز، بصيغة `"+M:SS"`.

على سبيل المثال، إذا كانت المعطيات `["1:25:32", "1:26:10", "1:27:05"]`، أرجع `["0", "+0:38", "+1:33"]`.

# --hints--

`getRelativeResults(["1:25:32", "1:26:10", "1:27:05"])` يجب أن تُرجع `["0", "+0:38", "+1:33"]`.

```js
assert.deepEqual(getRelativeResults(["1:25:32", "1:26:10", "1:27:05"]), ["0", "+0:38", "+1:33"]);
```

`getRelativeResults(["1:00:01", "1:00:05", "1:00:10"])` يجب أن تُرجع `["0", "+0:04", "+0:09"]`.

```js
assert.deepEqual(getRelativeResults(["1:00:01", "1:00:05", "1:00:10"]), ["0", "+0:04", "+0:09"]);
```

`getRelativeResults(["1:10:06", "1:10:23", "1:10:48", "1:12:11"])` يجب أن تُرجع `["0", "+0:17", "+0:42", "+2:05"]`.

```js
assert.deepEqual(getRelativeResults(["1:10:06", "1:10:23", "1:10:48", "1:12:11"]), ["0", "+0:17", "+0:42", "+2:05"]);
```

`getRelativeResults(["0:49:13", "0:49:15", "0:50:14", "0:51:30", "0:51:58", "0:52:16", "0:53:12", "0:53:31", "0:56:19", "1:02:20"])` يجب أن تُرجع `["0", "+0:02", "+1:01", "+2:17", "+2:45", "+3:03", "+3:59", "+4:18", "+7:06", "+13:07"]`.
...
`getRelativeResults(["2:01:15", "2:10:45", "2:10:53", "2:11:04", "2:11:55", "2:13:27", "2:14:30", "2:15:10"])` يجب أن تُرجع `["0", "+9:30", "+9:38", "+9:49", "+10:40", "+12:12", "+13:15", "+13:55"]`.
