...
Justo después de limpiar `multipleCard`, dentro del método `singleCardSelected`, llama a la función `showElements` y pasa `singleCard` como primer argumento y `fortuneContainer` como segundo argumento.

# --hints--

Debes llamar a la función `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

Debes pasar `elements.singleCard` como primer argumento.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

Debes pasar `elements.fortuneContainer` como segundo argumento.
