...
`font` 元素是一个废弃的元素，用于设置文本的字体大小和颜色。以下是一个 `font` 元素的示例。
...
`center` 元素是另一个废弃的元素，用于在其容器内水平居中内容。以下是包含文本和段落元素的 `center` 元素示例。

启用交互式编辑器，并为 `<p>Another example text.</p>` 添加 `center` 标签，以便在页面上居中显示。
...
启用交互式编辑器，为 `Some other text` 添加 `big` 标签，并在预览窗口中查看更改。 
...
请记住，字体大小应始终通过 CSS 设置，因此你不应在现代超文本标记语言中使用此元素。
...
这是一个包含导航区元素的 `header` 元素示例。

启用交互式编辑器，并在 `nav` 内添加 `<a href="#">Products</a>`，然后在预览窗口中查看更改。
