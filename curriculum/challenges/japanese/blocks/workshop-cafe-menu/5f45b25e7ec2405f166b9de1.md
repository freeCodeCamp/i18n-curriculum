...
リンクが訪問済みの場合にリンクのプロパティを変更するには、`a:visited { propertyName: propertyValue; }` のような<dfn>疑似セレクター</dfn>を使います。

ユーザーがリンクを訪問したときに、フッターの `Visit our website` リンクの色を `grey` に変更してください。

# --hints--

`a:visited` 疑似セレクターを使う必要があります。
...
`a:visited` は `color` が `grey` である必要があります。
