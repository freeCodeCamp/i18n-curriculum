...
次に、`classList` プロパティと `add()` メソッドを使用して、`playButton` の要素に `"playing"` クラスを追加してください。 これにより、CSS ファイルにあるクラス `"playing"` のスタイルが `playButton` の要素に適用されます。

その後、曲を再生するために変数 `audio` の `play()` メソッドを使用してください。 `play()` は Web Audio API のメソッドで、mp3 ファイルを再生できます。

# --hints--

変数 `playButton` にクラス `"playing"` を追加するために、`classList` プロパティと `add` メソッドを使用してください。

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

変数 `audio` の `play()` メソッドを使用してください。
