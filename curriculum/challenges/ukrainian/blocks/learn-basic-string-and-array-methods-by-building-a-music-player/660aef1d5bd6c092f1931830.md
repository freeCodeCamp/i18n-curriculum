...
Як і звичайні функції, стрілкові функції можуть повертати значення.
...
Створіть нову змінну під назвою `addTwoNumbers` та призначте до неї стрілкову функцію. Ця стрілкова функціяповинна приймати два параметри: `num1` та `num2`. Усередині тіла функції поверніть вираз додавання `num1` та `num2`.

Під функцією `addTwoNumbers` додайте консольну інструкцію. У цієї консольної інструкції викличте функцію `addTwoNumbers` і передайте числа `3` та `4` як аргументи.
...
Функція `addTwoNumbers` повинна використовувати синтаксис стрілкових функцій.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Функція `addTwoNumbers` повинна містити два параметри: `num1` та `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

Функція `addTwoNumbers` має повернути вираз додавання `num1` та `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

Код повинен мати інструкцію `console.log(addTwoNumbers(3, 4));`.
