...
Python は三項構文を使って `if`/`else` 条件式を簡潔に書く方法を提供しています。

```py
val_1 if condition else val_2
```

上の式は `val_1` が true の場合 `condition` に評価され、それ以外の場合は `val_2` に評価されます。

`print` の呼び出しを削除し、`targets_to_print` ループの後に `while` という変数を作成してください。三項構文を使って、`[target]` が真の場合は `target` を、そうでなければ `graph` を代入してください。


# --hints--

`print` の呼び出しを削除してください。

```js
({ test: () => assert.isFalse( /print\s*\(\s*f("|')Unvisited:\s*\{\s*unvisited\s*\}\\nDistances:\s\{\s*distances\s*\}\\nPaths:\s\{\s*paths\s*\}\1\s*\)/.test(code)) })
```

`targets_to_print` ループの後に `while` という変数を作成してください。
...
三項構文を使って、`[target]` が真の場合は `target` を、そうでなければ `graph` を `targets_to_print` 変数に代入してください。
