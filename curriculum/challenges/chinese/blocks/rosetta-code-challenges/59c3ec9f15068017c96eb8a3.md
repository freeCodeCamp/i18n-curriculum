...
Farey 序列 <code>F<sub>n</sub></code> 的阶数为 `n`，是介于 `0` 和 `1` 之间的完全约分分数序列，这些分数在最简形式下的分母小于或等于 `n`，并按大小递增顺序排列。
...
编写一个返回 `n` 阶 Farey 序列的函数。 该函数应该有一个参数，即 `n`。 它应该将序列作为数组返回。
...
`farey(3)` 应该返回 `['0/1','1/3','1/2','2/3','1/1']`

```js
assert.deepEqual(farey(3),['0/1', '1/3', '1/2', '2/3', '1/1']);
```

`farey(4)` 应该返回 `['0/1','1/4','1/3','1/2','2/3','3/4','1/1']`

```js
assert.deepEqual(farey(4), ['0/1', '1/4', '1/3', '1/2', '2/3', '3/4', '1/1']);
```

`farey(5)` 应该返回 `['0/1','1/5','1/4','1/3','2/5','1/2','3/5','2/3','3/4','4/5','1/1']`
