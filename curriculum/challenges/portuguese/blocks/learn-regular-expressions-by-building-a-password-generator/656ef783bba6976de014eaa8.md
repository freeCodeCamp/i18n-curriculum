...
Coloque a declaração da sua variável `password` e o seguinte loop `for` dentro de um loop `while`.Use `True` como condição para seu novo loop.

# --hints--

Você deve criar um loop `while True` envolvendo sua declaração existente de `password` e o loop `for`.
