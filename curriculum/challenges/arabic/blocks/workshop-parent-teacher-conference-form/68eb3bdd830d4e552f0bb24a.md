...
أضف أخيرًا تأثير المرور بالمؤشر / التأثير المرئي إلى زر الإرسال لتحسين تفاعل المستخدم.

اختر `.submit-btn:hover` واضبط خاصية `background-color` على `midnightblue`.

تهانينا على إتمام هذه الورشة!

# --hints--

يجب أن تستهدف فئة مزيفة `.submit-btn:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.submit-btn:hover'));
```

اضبط خاصية `background-color` على `midnightblue`.
