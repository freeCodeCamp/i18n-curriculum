...
是时候创建标签和输入了。 在表单中，创建一个 `label`，其中包含文本 `Hero Name` 和一个类型为 `text` 的 `input` 元素。

将 `Hero Name` 的 `input` 连接到 `heroName` 状态变量，方法是将 `value` 属性设置为 `heroName`。
...
你的 `label` 元素应该以 `Hero Name` 作为其文本。
...
你应该在 `label` 元素内创建一个类型为 `text` 的 `input` 元素。
...
你的 `input` 元素的值应设置为 `heroName` 状态变量。
