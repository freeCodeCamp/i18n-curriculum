...
Quindi dai agli elementi `h4` e `p` il seguente contenuto di testo, in ordine, usando per ogni `h4` ciò che si trova sul lato sinistro dei due punti, e per ogni `p` ciò che è a destra:
...
L'elemento `ul` dovrebbe avere sei elementi `li`.

```js
assert.lengthOf(document.querySelectorAll('.lists li'), 6);
```

Ciascuno dei nuovi elementi `li` dovrebbe avere un elemento `h4` e un `p`.
...
Il primo `h4` dovrebbe avere il testo `V1 - 2014`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

Il primo `p` dovrebbe avere il testo `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
Il secondo `h4` dovrebbe avere il testo `V2 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[1]?.innerText, 'V2 - 2015');
```

Il secondo `p` dovrebbe avere il testo `We added interactive algorithm challenges.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[1]?.innerText, 'We added interactive algorithm challenges.');
```

Il terzo `h4` dovrebbe avere il testo `V3 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[2]?.innerText, 'V3 - 2015');
```

Il terzo `p` dovrebbe avere il testo `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`
...
Il quarto `h4` dovrebbe avere il testo `V4 - 2016`.
...
Ognuno dei sei elementi `h4` dovrebbe avere la classe `list-subtitle`.
