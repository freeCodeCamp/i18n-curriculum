...
Agora tente adicionar o objeto `ballerina` ao seu `catalogSet` dentro da sua função pela segunda vez.Você verá que nada acontece, porque o objeto já está armazenado no seu set e elementos duplicados não são permitidos em sets.

Se quiser, você pode verificar o número de elementos no seu conjunto usando a propriedade `size`, da mesma forma que fez antes para o seu mapa.

# --hints--

Você deve `add` o objeto `ballerina` uma segunda vez ao seu conjunto dentro da função `displayPlantsSet`.
