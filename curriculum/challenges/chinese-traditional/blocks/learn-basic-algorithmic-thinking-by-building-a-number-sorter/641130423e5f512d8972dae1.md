...
注意 `10` 這個數字是如何被放置在陣列的開頭。這是因為 `.sort()` 的預設行為是將數字值轉換成字串，並依字母順序排序。而 `"10"` 在字母順序上排在 `"2"` 之前。

為了解決這個問題，你可以將回呼函式傳遞給 `.sort()` 方法。回呼函式有兩個參數－對你來說，使用 `a` 和 `b`。`a` 和 `b` 的參數表述將被排序的陣列中的數值。

暫時將函式（程式）、函數（數學）留空。

# --hints--

你應該將回呼函式傳遞給 `.sort()` 方法。請記得使用箭頭語法。

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

回呼函式（程式）應該有兩個參數，`a` 和 `b`。
