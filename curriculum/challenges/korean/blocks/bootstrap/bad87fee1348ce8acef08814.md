...
title: btn-danger로 위험한 동작을 사용자에게 경고하기
challengeType: 0
forumTopicId: 18375
...
부트스트랩에는 버튼용으로 미리 정의된 여러 색상이 있습니다. `btn-danger` 클래스는 고양이 사진 삭제와 같은 파괴적인 동작을 수행하는 버튼임을 사용자에게 알릴 때 사용할 버튼 색상입니다.

텍스트가 `Delete`인 버튼을 만들고 `btn-danger` 클래스를 지정하세요.

이 버튼들은 여전히 `btn`와 `btn-block` 클래스를 필요로 한다는 점을 기억하세요.

# --hints--

텍스트가 `button`인 새 `Delete` 요소를 만들어야 합니다.
...
모든 부트스트랩 버튼에는 `btn`와 `btn-block` 클래스가 있어야 합니다.

```js
assert.lengthOf(document.querySelectorAll('button.btn-block.btn'),3);
```

새 버튼에는 `btn-danger` 클래스가 있어야 합니다.
