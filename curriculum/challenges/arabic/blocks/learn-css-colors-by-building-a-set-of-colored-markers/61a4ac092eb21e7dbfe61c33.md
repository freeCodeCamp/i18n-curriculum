...
عدّل المعلمة اللونية الثانية في دالة `linear-gradient`، التي هي حاليًا أخضر نقي.

حدّث دالة `rgb` بحيث تكون قيمة الأحمر `245`، وقيمة الأخضر `62`، وقيمة الأزرق `113`.

# --hints--

يجب أن تحتوي قاعدة CSS `.red` على خاصية `background` مضبوطة على `linear-gradient(180deg, rgb(122, 74, 14) 0%, rgb(245, 62, 113) 50%, rgb(0, 0, 255) 100%)`.
