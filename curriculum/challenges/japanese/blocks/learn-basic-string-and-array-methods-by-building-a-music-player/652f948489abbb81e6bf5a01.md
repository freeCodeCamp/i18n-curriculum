...
このプロジェクトでは音楽プレイヤーアプリを作成しながら、基礎的な文字列・配列メソッドを学習します。 曲を再生、一時停止、スキップ、シャッフルすることができるアプリを作成します。

このプロジェクトで使う HTML と CSS のコードはあらかじめ用意してありますので、JavaScript に集中できます。

`getElementById()` メソッドを使って、`#playlist-songs`、`#play`、そして `#pause` の要素にアクセスするところから始めましょう。 それぞれの要素を、変数 `playlistSongs`、`playButton`、`pauseButton` に割り当ててください。


# --hints--

`playlistSongs` という名前の変数を作成する必要があります。

```js
assert.isDefined(playlistSongs);
```

`document.getElementById()` を使って `#playlist-songs` の要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

`#playlist-songs` の要素を変数 `playlistSongs` に割り当ててください。

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

`playButton` という名前の変数を作成する必要があります。

```js
assert.isDefined(playButton);
```

`document.getElementById()` を使って `#play` の要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

`#play` の要素を変数 `playButton` に割り当ててください。

```js
assert.equal(playButton, document.getElementById('play'));
```

`pauseButton` という名前の変数を作成する必要があります。

```js
assert.isDefined(pauseButton);
```

`document.getElementById()` を使って `#pause` の要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

`#pause` の要素を変数 `pauseButton` に割り当ててください。
