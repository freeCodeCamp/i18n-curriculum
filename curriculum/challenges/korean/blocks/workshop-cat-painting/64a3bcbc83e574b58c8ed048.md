...
이전 단계에서 했던 것처럼 자손 선택자를 사용해 `div` 요소 안에 있는 세 개의 `.cat-whiskers-right` 요소를 선택하세요. `width` 값을 가진 `40px`, `height` 값을 가진 `1px`, `background-color` 값을 가진 `#000`를 각각 지정하세요.

# --hints--

`.cat-whiskers-right div` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-right div'))
```

`.cat-whiskers-right div` 선택자에는 `width`로 설정된 `40px`가 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-right div')?.width, '40px')
```

`.cat-whiskers-right div` 선택자에는 `height`로 설정된 `1px`가 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-right div')?.height, '1px')
```

`.cat-whiskers-right div` 선택자에는 `background-color`로 설정된 `#000`가 있어야 합니다.
