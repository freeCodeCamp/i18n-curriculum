...
É hora de implementar uma funcionalidade de exclusão para a playlist.Isso gerenciaria a remoção de uma música da playlist, lidaria com outras ações relacionadas quando uma música fosse excluída e criaria um botão `Reset Playlist`.

Use `const` e a sintaxe de arrow para criar uma função vazia `deleteSong` e passar `id` como parâmetro.

# --hints--

Você deve usar `const` para criar uma função vazia chamada `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` deve ser uma função.

```js
assert.isFunction(deleteSong)
```

Sua função `deleteSong` deve usar uma sintaxe de arrow.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Sua função `deleteSong` deve receber um parâmetro `id`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

Sua função `deleteSong` deve estar vazia.
