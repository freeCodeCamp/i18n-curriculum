...
在 `.penguin-body` 元素中，添加兩個 `div` 元素，每個元素的 `class` 爲 `foot`。 給第一個 `.foot` 一個 `class` 爲 `left`，給第二個 `.foot` 一個 `class` 爲 `right`。

# --hints--

應該在 `.penguin-body` 中添加兩個 `div` 元素。 應該是 `--fcc-expected--` `div` 元素，而不是 `--fcc-actual--`。

```js
assert.equal(document.querySelectorAll('.penguin-body > div')?.length, 2);
```

你應該給第一個新的 `div` 一個 `class` 爲 `foot`。

```js
assert.exists(document.querySelector('.penguin-body > div.foot'));
```

你應該給第二個新的 `div` 一個 `class` 爲 `foot`。

```js
assert.equal(document.querySelectorAll('.penguin-body > div.foot')?.length, 2);
```

其中一個 `div` 的 `class` 應該包含 `left`。

```js
assert.exists(document.querySelector('.penguin-body > div.foot.left'));
```

另一個 `div` 的 `class` 應該包含 `right`。
