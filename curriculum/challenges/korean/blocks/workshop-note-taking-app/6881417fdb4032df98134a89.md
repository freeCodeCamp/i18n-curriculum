...
워크숍의 나머지 단계는 저장된 메모 메시지를 동적으로 표시하는 코드와 현재 메모를 콘솔에 기록하는 코드를 추가하는 작업입니다.

먼저 `noteEl`라는 변수를 만들고 `id`의 `note` 속성을 가진 요소를 문서에서 쿼리한 결과를 할당하세요.

그다음 `statusEl`라는 또 다른 변수를 만들고 `id`의 `status` 속성을 가진 요소를 문서에서 쿼리한 결과를 할당하세요.

# --hints--

`noteEl`의 `id` 속성을 가진 요소를 문서에서 쿼리한 결과를 할당하는 `note` 변수가 있어야 합니다.

```js
assert.equal(noteEl, document.getElementById("note"));
``` 

`statusEl`의 `id` 속성을 가진 요소를 문서에서 쿼리한 결과를 할당하는 `status` 변수가 있어야 합니다.
