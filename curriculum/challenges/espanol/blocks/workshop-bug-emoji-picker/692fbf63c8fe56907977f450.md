...
A continuación, define un objeto llamado `bugMap` con el tipo `Record<string, Bug<string>>` y asígnale por ahora un objeto vacío.

# --hints--

Deberías tener una constante llamada `bugMap`.
...
Tu `bugMap` debe tener una anotación de tipo `Record<string, Bug<string>>`.
...
Tu `bugMap` debe inicializarse con un objeto vacío.
