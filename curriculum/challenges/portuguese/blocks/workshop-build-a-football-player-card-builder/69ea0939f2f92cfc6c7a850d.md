...
Em seguida, adicione um hook `useEffect` que seja executado sempre que o estado `player` mudar. Dentro dele, use uma arrow function e adicione um `try..catch` com `error` como parâmetro do bloco `catch`.

# --hints--

Você deve fazer a chamada `useEffect` dentro do `FootballPlayerCard`.
...
Seu `useEffect` deve ter `[player]` como seu array de dependências.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

O callback `useEffect` deve conter um bloco `try...catch` com `error` como parâmetro do bloco `catch`.
