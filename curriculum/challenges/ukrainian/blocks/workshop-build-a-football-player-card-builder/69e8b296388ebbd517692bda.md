...
Додайте атрибут `value` зі значенням `player.imageUrl` до URL зображення `input`. Потім встановіть атрибут `onChange` у стрілкову функцію, яка приймає `e` як параметр і виконує виклик `setPlayer` з розгортанням `player` та `imageUrl`, встановленим у `e.target.value`.

# --hints--

Ваш URL зображення `input` має містити атрибут `value`, рівний URL зображення гравця.
...
Ваш URL зображення `input` має містити обробник атрибуту `onChange`, який викликає `setPlayer` з розгортанням `player` та `imageUrl`, встановленим у `e.target.value`.
