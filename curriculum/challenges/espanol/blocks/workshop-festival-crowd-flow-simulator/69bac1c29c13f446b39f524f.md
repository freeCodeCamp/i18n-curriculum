...
Ahora debes implementar el ciclo principal de la simulación. Este ciclo asegurará que cada compuerta se procese en cada tic de la simulación hasta que se completen todos los tics.

Crea un ciclo `while` vacío que continúe mientras `tickIndex` sea menor que `maxTicks`.

# --hints--

Debes crear un ciclo `while` vacío con la condición `(tickIndex < maxTicks)`.
