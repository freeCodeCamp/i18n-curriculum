...
Seu seletor `.cat-right-ear` deve ter uma propriedade `border-top-left-radius` definida como `90px`. Não esqueça de adicionar um ponto e vírgula.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderTopLeftRadius === '90px')
```

Seu seletor `.cat-right-ear` deve ter uma propriedade `border-top-right-radius` definida como `10px`. Não esqueça de adicionar um ponto e vírgula.
