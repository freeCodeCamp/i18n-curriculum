...
在程式設計中，`null` 是用來表述值的缺失。在這種情況下，如果使用者輸入無效，你想要提醒他們，然後傳回 `null` 以顯示該函式（程式）已失敗。

仍然在你的 `if` 區塊中，將 `isError` 設為 `true` 並傳回 `null`。

# --hints--

在你的 `alert` 之後，你應該將 `isError` 設定為 `true`。
...
在你修改 `isError` 之後，你應該 `return` 值 `null`。
