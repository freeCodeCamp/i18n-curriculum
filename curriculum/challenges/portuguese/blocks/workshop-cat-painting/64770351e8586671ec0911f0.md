...
O posicionamento `sticky` é um híbrido dos posicionamentos `relative` e `fixed`. Ele permite que um elemento **grude** em uma posição específica dentro do seu elemento contêiner ou viewport, com base na posição do scroll.

Altere o valor da propriedade `position` de `.cat-head` para `sticky`, defina `top` como `0` e remova `left` e seu valor.

**Nota**: Para ver como `sticky` funciona, você precisa colocar alguns textos antes e depois do seu elemento `.cat-head`. Se você rolar para baixo depois disso, verá que `.cat-head` fica preso na parte superior e permanece lá.
