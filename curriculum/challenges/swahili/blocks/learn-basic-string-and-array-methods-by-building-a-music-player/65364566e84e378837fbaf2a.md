...
Katika hatua zilizopita ujenzi wa utendakazi wa kuimba wimbo ulikamilika. Sasa unahitaji kuongeza utendakazi kwa kitufe cha kucheza ili kiweze kuimba wimbo wa sasa wakati kinapobofyanwa.

Tumia njia ya `addEventListener()` na pita tukio la `"click"` kama hoja ya kwanza na kitendakazi cha mwitiko tupu chenye sintaksia ya mshale kama hoja ya pili, kwa mfano `() => {}`.

# --hints--

Unapaswa kuita njia ya `addEventListener()` kwenye kigezo chako cha `playButton`.
...
Unapaswa kutumia sintaksia ya mshale kupitisha kitendakazi cha mwitiko tupu kwenye msikilizaji wako wa tukio. Usisahau pia unahitaji mabano ya mviringo tupu.
