...
Modifica las dos llamadas a `print` para imprimir la norma de tus vectores y verifica que el método `norm()` funcione correctamente.

# --hints--

Debes imprimir `v1.norm()`.

```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(v1.norm())")`)) })
```

Debes imprimir `v2.norm()`.
