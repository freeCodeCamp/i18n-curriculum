...
`DiscountEngine` のインスタンスを作成し、変数 `engine` に保存してください。その後、`calculate_best_price` メソッドを使って最適な価格を計算し、`best_price` という名前の変数に保存してください。

# --hints--

`DiscountEngine` リストを使って `strategies` のインスタンスを作成してください。
...
`engine.calculate_best_price(product, user_tier)` を呼び出し、その結果を `best_price` に保存してください。
