...
你最内层的柯里的函数将包含转换该值的逻辑。

首先，使用 `fromUnit` 参数访问你的 `conversionTable` 对象，然后使用 `toUnit` 参数访问嵌套对象。

返回将该转换值与 `quantity` 值相乘的结果。

# --hints--

你的函数应返回 `convertQuantity("gram")("cup")(1)` 的正确转换结果。

```js
assert.strictEqual(convertQuantity("gram")("cup")(1), 1/240)
```

你的函数应返回 `convertQuantity("teaspoon")("ounce")(6)` 的正确转换结果
