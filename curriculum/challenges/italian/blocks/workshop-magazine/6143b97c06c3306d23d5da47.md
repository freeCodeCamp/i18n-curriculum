...
All'interno del tuo elemento `article`, crea un elemento `h3` con la proprietà `class` impostata su `list-title` e il testo `A Brief History`. Sotto di esso, crea un elemento `p` con il testo `Of the Curriculum`. Poi crea un elemento `ul` con la classe `lists`.

# --hints--

Dovresti creare un elemento `h3` all'interno del tuo elemento `article`.

```js
assert.exists(document.querySelector('article h3'));
```

Dovresti creare un elemento `p` all'interno del tuo elemento `article`.

```js
assert.exists(document.querySelector('article p'));
```

Dovresti creare un elemento `ul` all'interno del tuo elemento `article`.

```js
assert.exists(document.querySelector('article ul'));
```

I tuoi elementi all'interno dell'elemento `article` dovrebbero essere nel corretto ordine.
...
Il tuo nuovo elemento `h3` dovrebbe avere la proprietà `class` impostata su `list-title`.

```js
assert.equal(document.querySelector('article h3')?.className, 'list-title');
```

Il tuo nuovo elemento `h3` dovrebbe avere il testo `A Brief History`.

```js
assert.equal(document.querySelector('article h3')?.innerText, 'A Brief History');
```

Il tuo nuovo elemento `p` dovrebbe avere il testo `Of the Curriculum`.

```js
assert.equal(document.querySelector('article p')?.innerText, 'Of the Curriculum');
```

Il tuo nuovo elemento `ul` dovrebbe avere la proprietà `class` impostata su `lists`.
