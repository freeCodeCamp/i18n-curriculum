...
È ora di definire lo stile degli elementi `fieldset` per dare loro un bordo distinto.

Seleziona l'elemento `fieldset`. Imposta la sua proprietà `border` su `1px solid gray`, `border-radius` su `5px`, `margin` su `20px 0` e `padding` su `20px`.

# --hints--

Dovresti puntare all'elemento `fieldset`.
...
Imposta la proprietà `margin` su `20px 0`.
...
Imposta la proprietà `padding` su `20px`.
