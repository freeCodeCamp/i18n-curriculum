...
Aunque no es obligatorio para los elementos `label` con un `input` anidado, sigue siendo una buena práctica enlazar explícitamente un `label` con su elemento `input` correspondiente.

Ahora, añade un atributo `for` a cada uno de tus cuatro `label`s que vincule el `label` con su correspondiente `input` de radio.

# --hints--

Debes darle al primer `label` un atributo `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

Debes darle al primer `label` un atributo `for` que coincida con el `id` de su elemento `input`.
...
Debes darle al segundo `label` un atributo `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

Debes darle al segundo `label` un atributo `for` que coincida con el `id` de su elemento `input`.
...
Debes darle al tercer `label` un atributo `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

Debes darle al tercer `label` un atributo `for` que coincida con el `id` de su elemento `input`.
...
Debes darle al cuarto `label` un atributo `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

Debes darle al cuarto `label` un atributo `for` que coincida con el `id` de su elemento `input`.
