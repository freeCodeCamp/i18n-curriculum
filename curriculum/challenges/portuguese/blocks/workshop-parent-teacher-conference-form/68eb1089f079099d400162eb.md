...
Você adicionou uma classe `center` a alguns elementos no HTML. Agora, você deve definir essa classe para centralizar esses elementos.

Aponte para a classe `center` e centralize os elementos que a possuem com uma propriedade `text-align` definida como `center`.

# --hints--

Você deve direcionar os elementos com a classe `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.center'));
```

Você deve definir a propriedade `text-align` para `center`.
