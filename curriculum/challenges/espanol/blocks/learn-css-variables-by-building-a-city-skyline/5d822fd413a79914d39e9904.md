...
Completa `.bb3` con un `repeating-linear-gradient`. Usa `90deg` para la dirección, tu `building-color3` para los dos primeros colores, y `window-color3` al `15%` para el tercero.

Cuando no especificas una distancia para un color, usará los valores que tienen sentido. En este caso, los dos primeros colores predeterminados serán `0%` y `7.5%` porque comienza en `0%` y `7.5%` es la mitad del `15%`, por lo que no es necesario configurarlos.
