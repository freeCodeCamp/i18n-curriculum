...
既存の`for`ループを1段階インデントして、カタログに映画があるときに実行されるようにしてください。それから、`self.items`を`movies`に置き換えて、映画だけを繰り返してください。

# --hints--

既存の`for`ループを1段階インデントし、`self.items`を`movies`に置き換えて、映画だけを繰り返してください。
...
`__str__`メソッドは`Media Catalogue (empty)`が空の場合、`self.items`を返すべきです。
