...
使用 `.push()` 將 `quotient` 變數附加到 `quotients` 陣列。也將 `remainder` 變數附加到 `remainders` 陣列。

# --hints--

你應該在你的 `while` 迴圈中，對 `quotients` 陣列使用 `.push()` 方法。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*quotients\.push\(/);
```

你應該使用 `.push()` 方法將 `quotient` 附加到 `quotients` 陣列。

```js
assert.match(String(decimalToBinary), /quotients\.push\(\s*quotient\s*\)\s*;?/);
```

你應該在你的 `while` 迴圈中，對 `remainders` 陣列使用 `.push()` 方法。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{[\s\S]*remainders\.push\(/);
```

你應該使用 `.push()` 方法將 `remainder` 附加到 `remainders` 陣列。
