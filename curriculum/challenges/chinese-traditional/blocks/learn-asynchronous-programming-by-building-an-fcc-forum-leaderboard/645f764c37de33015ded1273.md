...
若要自訂頭像的大小，你可以將它設定為 `30`。 

首先建立一個名為 `avatar` 的常數。然後將使用 `replace` 方法於 `user.avatar_template` 上的結果指定給它。

對於 `replace` 方法，第一個引數使用 `/{size}/`，第二個引數使用數字 `30`。
...
你應該將 `user.avatar_template.replace()` 指定給你的 `avatar` 常數。

```js
assert(code.match(/const\s+avatar\s*=\s*user\.avatar_template\.replace\(\s*.*\s*\)/));
```

你應該為 `replace` 方法的第一個引數使用 `/{size}/`，為第二個引數使用數字 30。
