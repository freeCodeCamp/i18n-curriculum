...
在 `allSongs` 陣列中，創建一個具有以下屬性和值的物件：
...
你的 `allSongs` 陣列應該有一個值。

```js
assert.equal(allSongs.length, 1);
```

你的 `allSongs` 陣列的第一個值應該是一個物件。

```js
assert.isObject(allSongs[0]);
```

你的 `allSongs` 陣列應該包含一個 `id` 屬性設為數字 `0` 的物件。

```js
assert.equal(allSongs[0].id, 0);
```

你的 `allSongs` 陣列應該包含一個物件，其 `title` 屬性設定為字串 `"Scratching The Surface"`。

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

你的 `allSongs` 陣列應該包含一個具有 `artist` 屬性且該屬性設定為字串 `"Quincy Larson"` 的物件。

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

你的 `allSongs` 陣列應該包含一個具有 `duration` 屬性且該屬性設定為字串 `"4:25"` 的物件。

```js
assert.equal(allSongs[0].duration, "4:25");
```

你的 `allSongs` 陣列應該包含一個物件，其 `src` 屬性設定為字串 `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`。
