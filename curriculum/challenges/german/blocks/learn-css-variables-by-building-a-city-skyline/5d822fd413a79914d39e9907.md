...
Geben Sie den neuen `div`-Elementen diese `width`- und `height`-Werte: `3%` und `10%` zu `.bb4a`, `80%` und `5%` zu `.bb4b` sowie `100%` und `85%` zu `.bb4c`.

# --hints--

Sie sollten `.bb4a` eine `width` von `3%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.width, "3%");
```

Sie sollten `.bb4a` eine `height` von `10%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.height, "10%");
```

Sie sollten `.bb4b` eine `width` von `80%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.width, "80%");
```

Sie sollten `.bb4b` eine `height` von `5%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.height, "5%");
```

Sie sollten `.bb4c` eine `width` von `100%` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4c")?.width, "100%");
```

Sie sollten `.bb4c` eine `height` von `85%` geben.
