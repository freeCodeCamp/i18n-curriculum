...
먼저 삭제하려는 작업의 인덱스를 찾아야 합니다.

`dataArrIndex` 변수를 선언하고 `taskData` 배열에서 `findIndex()` 메서드를 사용해 값을 설정하세요. 화살표 콜백 함수의 매개변수로 `item`를 전달하고, 콜백 안에서 `item`의 `id`가 `buttonEl`의 `parentElement`의 `id`와 같은지 확인하세요.

# --hints--

`deleteTask` 함수는 변경하지 마세요.
