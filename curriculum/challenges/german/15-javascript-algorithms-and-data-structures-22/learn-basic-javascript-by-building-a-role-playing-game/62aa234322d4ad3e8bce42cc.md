...
Wir wollen nicht, dass die einzige Waffe eines Spielers kaputt geht. Der logische AND-Operator prüft, ob zwei Aussagen wahr sind.

Verwende den <dfn>logischen AND</dfn>-Operator `&&`, um eine zweite Bedingung zu deiner `if`-Anweisung hinzuzufügen. Die Waffe des Spielers sollte nur kaputt gehen, wenn `inventory.length` ungleich (`!==`) eins ist.
...
Du solltest den logischen AND-Operator verwenden, um zu prüfen, ob `inventory.length` ungleich `1` ist.
