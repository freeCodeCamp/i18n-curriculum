...
Player Info 섹션이 이제 완성되었습니다. Player Info 섹션의 닫는 `div` 뒤이면서 `form-panel` 안에 새로운 `div`를 추가하세요. 그 안에 `p`의 `className`와 텍스트 `form-section-title`가 있는 `Player Stats` 요소를 추가하세요. 그런 다음 `div`의 `className`가 있는 `stats-grid`를 추가하세요. stats 그리드는 지금은 비워 두세요.

# --hints--

`div.form-panel` 안에 Player Stats 섹션을 위한 두 번째 `div`가 있어야 합니다.
...
Player Stats 섹션에는 `p`의 `className`와 텍스트 `form-section-title`가 있는 `Player Stats`가 있어야 합니다.
...
Player Stats 섹션에는 `div`의 `className`가 있는 `stats-grid`가 있어야 합니다.
