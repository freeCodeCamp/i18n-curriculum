...
Para finalizar, defina um texto `Calendar` para a primeira tag anchor, `Chat` para a segunda e `Email` para a terceira.

Com isso, seu componente mega navbar está completo!

# --hints--

Você deve definir um texto de `Calendar` para o primeiro elemento anchor.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Você deve definir um texto de `Chat` para o segundo elemento anchor.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Você deve definir um texto de `Email` para o terceiro elemento anchor.
