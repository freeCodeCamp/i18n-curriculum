...
Остання множина елементів для вибору — це елементи результату. Використайте свою `getElement` функцію, щоб отримати елемент з ідентифікаторами `result-text` та `result-card`. Обидва мають використовувати `HTMLElement` як тип.

Пам’ятайте, що ви вже оголосили змінні `resultText` та `resultCard` для них, тож потрібно лише призначити їм значення.

# --hints--

Ви повинні призначити значення змінній `resultText`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` має вибрати елемент `result-text`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

Ви повинні використовувати `HTMLElement` як тип `getElement`, який ви використовуєте для вибору елемента `result-text`.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

Ви повинні призначити значення змінній `resultCard`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` має вибрати елемент `result-card`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

Ви повинні використовувати `HTMLElement` як тип `getElement`, який ви використовуєте для вибору елемента `result-card`.
