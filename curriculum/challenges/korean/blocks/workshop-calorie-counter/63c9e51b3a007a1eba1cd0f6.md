...
이제 `remainingCalories` 변수를 선언하고, `consumedCalories`에서 `budgetCalories`을 빼고 `exerciseCalories`을 더한 값을 할당하세요.

# --hints--

`calculateCalories` 함수 안에 `remainingCalories` 변수가 있어야 합니다.

```js
assert.match(calculateCalories.toString(), /remainingCalories\s*=/);
```

`budgetCalories - consumedCalories + exerciseCalories` 값을 찾아야 합니다.
...
`budgetCalories - consumedCalories + exerciseCalories` 값을 `remainingCalories`에 할당해야 합니다.
