...
在你的 `sortInputArray()` 函式中，呼叫你的 `updateUI()` 函式並將 `inputValues` 作為引數傳遞。

你現在應該能夠點擊 `Sort` 按鈕，並在 `Output` 區段看到輸入的陣列。

# --hints--

你應該在你的 `sortInputArray()` 函式中呼叫 `updateUI()`。

```js
assert.match(sortInputArray.toString(), /updateUI\(/);
```

你應該將 `inputValues` 作為引數傳遞給 `updateUI()`。
