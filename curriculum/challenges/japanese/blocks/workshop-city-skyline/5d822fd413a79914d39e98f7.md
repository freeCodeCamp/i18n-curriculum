...
`orange` グラデーションから `.bb1d` を削除し、`80%` を `50%` に変更してください。これにより、上半分は `--building-color1` が単色になり、下半分は `--window-color1` に移行します。

# --hints--

`orange` から `linear-gradient` を削除してください。

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'orange');
```

現在の最初の `linear-gradient` の色を `50%` で移行するように変更してください。
