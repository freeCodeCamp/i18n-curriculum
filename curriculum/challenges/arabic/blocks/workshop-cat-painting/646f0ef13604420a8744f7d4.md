...
داخل العنصر `.cat-whiskers`، أنشئ عنصرين `div` بالصنفين `cat-whiskers-left` و `cat-whiskers-right`.

# --hints--

لا تغير عنصر `div` الموجود بالصنف `cat-whiskers`
...
يجب أن يكون العنصر الأول `div` داخل `.cat-whiskers` بالصنف `cat-whiskers-left`.

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers div')[0].classList.contains('cat-whiskers-left'));
```

يجب أن يكون العنصر الثاني `div` داخل `.cat-whiskers` بالصنف `cat-whiskers-right`.
