...
以前、`console.log()` 関数の動作を検証するために `swapCrewMembers` 呼び出しを追加しましたが、現在はその呼び出しは不要になりました。今後のステップでターミナルが散らからないように、コンソールにログを出力している現在の `for` ループを削除してください。

# --hints--

`for` 関数から `swapCrewMembers` ループを削除してください。ループ全体のブロックを削除する必要があります。
