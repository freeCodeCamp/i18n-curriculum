...
Dentro do `map`, retorne a função `renderCard` passando `type`, `isReversed`, `name_short` e `img` como argumentos, e adicione o método `join` no `map` com uma string vazia.

# --hints--

Você deve retornar a função `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

Você deve passar `type` como primeiro argumento na função `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

Você deve passar `isReversed` como segundo argumento na função `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?isReversed\s*/);
```

Você deve passar `card.name_short` como argumento na função `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*/);
```

Você deve passar `card.img` como argumento na função `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

Você deve anexar `join("")` no método `map`.
