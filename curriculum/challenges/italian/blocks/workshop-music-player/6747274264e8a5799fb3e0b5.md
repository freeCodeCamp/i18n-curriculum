...
Aggiungi un secondo oggetto con le seguenti chiavi e valori:
...
Il tuo array `allSongs` dovrebbe avere due valori.

```js
assert.equal(allSongs.length, 2);
```

Il tuo array `allSongs` dovrebbe avere un oggetto come secondo valore.

```js
assert.isObject(allSongs[1]);
```

Il secondo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `id` impostata sul numero `1`.

```js
assert.strictEqual(allSongs[1].id, 1);
```

Il secondo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `title` impostata sulla stringa `"Can't Stay Down"`.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

Il secondo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `artist` impostata sulla stringa `"Quincy Larson"`.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

Il secondo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `duration` impostata sulla stringa `"4:15"`.

```js
assert.equal(allSongs[1].duration, "4:15");
```

Il secondo oggetto nel tuo array `allSongs` dovrebbe avere una proprietà `src` impostata sulla stringa `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
