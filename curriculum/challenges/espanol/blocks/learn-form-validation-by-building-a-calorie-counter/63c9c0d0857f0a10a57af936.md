...
Siguiendo el mismo patrón, busca tus entradas de `número` en los elementos `#cena`, `#bocadillos` y `#ejercicio`. Asígnales a variables siguiendo el esquema de nombres de los dos anteriores.

# --hints--

Debes declarar una variable `dinnerNumberInputs`.

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

Debes llamar a `document.querySelectorAll()` con `#dinner input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Debes asignar el resultado de tu llamada `document.querySelectorAll()` a `dinnerNumberInputs`.
...
Debes declarar una variable `snacksNumberInputs`.

```js
assert.match(calculateCalories.toString(), /snacksNumberInputs\s*=/);
```

Debes llamar a `document.querySelectorAll()` con `#snacks input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#snacks input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Debes asignar el resultado de tu llamada `document.querySelectorAll()` a `snacksNumberInputs`.
...
Debes declarar una variable `exerciseNumberInputs`.

```js
assert.match(calculateCalories.toString(), /exerciseNumberInputs\s*=/);
```

Debes llamar a `document.querySelectorAll()` con `#exercise input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#exercise input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Debes asignar el resultado de tu llamada `document.querySelectorAll()` a `exerciseNumberInputs`.
