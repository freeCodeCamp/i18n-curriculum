---
id: 66069719acca7d9b59499ce9
title: المهمة 22
challengeType: 22
dashedName: task-22
lang: en-US
---

<!-- (Audio) Brian: After that, you should read the methods and functions associated with the class. These can explain how to interact with the class and what you can achieve using it. -->

# --description--

تعني `Achieve` النجاح في الوصول إلى هدف أو إكمال مهمة. في البرمجة، يمكن أن تشير إلى ما يمكنك إنجازه باستخدام صنف أو طريقة. مثل قول: `With this code, I can achieve a fast calculation.`

# --fillInTheBlank--

## --sentence--

`After that, you should read the BLANK and functions BLANK with the class. These can BLANK how to interact with the class and what you can BLANK using it.`

## --blanks--

`methods`

### --feedback--

هي دالة أو إجراء محدد يُعد جزءًا من صنف في البرمجة.

---

`associated`

### --feedback--

تشير إلى الأشياء المرتبطة أو المتصلة، مثل الطرق والدوال التي تُعد جزءًا من صنف.

---

`explain`

### --feedback--

تعني هذه الكلمة الشرح أو التوضيح. هنا، المقصود بها توضيح كيفية استخدام الصنف.

---

`achieve`

### --feedback--

تعني النجاح في إكمال أو تحقيق هدف. في هذا السياق، تشير إلى ما يمكنك إنجازه باستخدام الصنف.

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Brian",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "7.3-3.mp3",
      "startTime": 1,
      "startTimestamp": 16.2,
      "finishTimestamp": 24.46
    }
  },
  "commands": [
    {
      "character": "Brian",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Brian",
      "startTime": 1,
      "finishTime": 4.58,
      "dialogue": {
        "text": "After that, you should read the methods and functions associated with the class.",
        "align": "center"
      }
    },
    {
      "character": "Brian",
      "startTime": 4.96,
      "finishTime": 9.26,
      "dialogue": {
        "text": "These can explain how to interact with the class and what you can achieve using it.",
        "align": "center"
      }
    },
    {
      "character": "Brian",
      "opacity": 0,
      "startTime": 9.76
    }
  ]
}
```
