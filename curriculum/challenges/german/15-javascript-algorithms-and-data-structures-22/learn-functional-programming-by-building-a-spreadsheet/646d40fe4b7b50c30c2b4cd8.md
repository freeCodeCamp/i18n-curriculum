...
Deine `infixEval`-Funktion wertet nur die erste Multiplikations- oder Divisionsoperation aus, weil `regex` nicht global ist. Das bedeutet, dass du einen rekursiven Ansatz verwenden solltest, um den ganzen String auszuwerten.

Findet `infixEval` keine Übereinstimmungen, gibt er den `str`-Wert so zurück, wie er ist. Überprüfe mit Hilfe eines ternären Ausdrucks, ob `str2` gleich `str` ist. Wenn ja, gib `str` zurück, andernfalls gib das Ergebnis des Aufrufs von `highPrecedence()` auf `str2` zurück.
...
Du solltest das `return`-Schlüsselwort mit einer Bedingung verwenden, um zu überprüfen, ob `str` gleich `str2` ist.
...
Du solltest die ternäre Syntax mit deiner `return`-Anweisung verwenden.
...
Wenn die ternäre Bedingung "wahr" ist, solltest du `str` zurückgeben.
...
Wenn die ternäre Bedingung "falsch" ist, solltest du das Ergebnis des Aufrufs von `highPrecedence()` zurückgeben.
...
Du solltest deinem `highPrecedence()`-Aufruf `str2` übergeben.
