...
The behavior of the `addItem` method needs to change if the product is already in the cart or not. 創建一個三元運算符來檢查當前產品是否已經在購物車中。 使用 `undefined` 作爲真假表達式以避免語法錯誤。

# --hints--

你應該檢查 `currentProductCount` 是否大於 `1`。
...
你應該使用三元運算符與 `currentProductCount > 1` 條件。
