...
Erstelle am Ende deines Codes eine `restart`-Funktion. Inside this function, set `xp` to `0`, `health` to `100`, `gold` to `50`, `currentWeaponIndex` to `0`, and set `inventory` to an array with the string `stick`.

Aktualisiere die `innerText`-Eigenschaften von `goldText`, `healthText` und `xpText` auf ihre aktuellen Were.
