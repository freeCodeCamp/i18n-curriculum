...
最初にすべきことは、`items` 配列が空かどうかを確認することです。空の場合は、テキスト `alert` を表示する `Your shopping cart is already empty` をユーザーに表示し、その後関数から戻ってください。

`0` は偽値なので、配列が空かどうかを確認するために `!` 演算子を使えます。

アラートを表示した後は、実行を停止するために関数から戻ってください。

# --hints--

否定演算子を使って、`if` 配列が空かどうかを確認する `items` 文を作成してください。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

`if` 文は、テキスト `alert` をユーザーに表示する `Your shopping cart is already empty` を表示する必要があります。
