...
`input` 要素は `size` 属性を持つことができます。この属性は、ユーザーが入力中に表示される文字数を定義します。`size` の値は 0 より大きい非負整数である必要があります。`size` が指定されていない場合、または無効な値が指定されている場合、入力はブラウザによって設定されたデフォルトの幅になります。
