...
Jetzt ist es Zeit, die Reset-Funktionalität zum `resetButton` hinzuzufügen. Dadurch werden die Songs in der Wiedergabeliste beim Klicken wieder angezeigt.

Fügen Sie der `resetButton`-Variablen einen Klick-Ereignis-Listener hinzu. Übergeben Sie eine Callback-Funktion mit Pfeilsyntax und lassen Sie sie vorerst leer.

# --hints--

Sie sollten die `addEventListener()`-Methode auf `resetButton` verwenden.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

Ihr `resetButton`-Ereignis-Listener sollte auf ein `"click"`-Ereignis hören.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

Die Callback-Funktion Ihres Ereignis-Listeners sollte Pfeilsyntax verwenden und ein leeres Paar geschweifter Klammern enthalten.
