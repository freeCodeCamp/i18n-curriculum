...
使用 `const` 關鍵字和箭頭語法創建一個 `isSpam` 函式（程式）。該函式應該接受一個單一參數 `msg`，並且目前隱式傳回 `false`。

# --hints--

你應該使用 `const` 來宣告一個 `isSpam` 變數。

```js
assert.match(code, /const\s+isSpam\s*=/);
```

你應該使用箭頭語法來指定 `isSpam` 一個函式（程式）。

```js
assert.match(code, /const\s+isSpam\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

你的 `isSpam` 函式（程式）應該只有一個 `msg` 參數。

```js
assert.match(code, /const\s+isSpam\s*=\s*(\(\s*msg\s*\)|msg)/);
```

你的 `isSpam` 函式（程式）應該隱喻地傳回 `false`。
