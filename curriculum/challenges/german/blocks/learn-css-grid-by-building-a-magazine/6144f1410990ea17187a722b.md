...
Jetzt können Sie das Layout Ihres Grids gestalten. CSS Grid ähnelt Flexbox darin, dass es eine spezielle Eigenschaft sowohl für das Elternelement als auch für die Kindelemente gibt.

In diesem Fall ist Ihr Elternelement das `main`-Element. Legen Sie das Layout so fest, dass der Inhalt drei Spalten hat, indem Sie eine `grid-template-columns`-Eigenschaft mit dem Wert `1fr 94rem 1fr` hinzufügen. Dadurch entstehen drei Spalten, wobei die mittlere Spalte `94rem` breit ist und die erste sowie die letzte Spalte jeweils 1 Bruch des verbleibenden Platzes im Grid-Container einnehmen.

# --hints--

Ihr `main`-Abschnitt sollte eine `grid-template-columns`-Eigenschaft mit dem Wert `1fr 94rem 1fr` haben.
