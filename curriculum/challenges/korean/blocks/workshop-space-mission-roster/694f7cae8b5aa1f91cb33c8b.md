...
이전과 마찬가지로 입력에 대해 유효성 검사를 해야 합니다. 구체적으로, `size`가 `1`보다 작으면 `"Chunk size must be >= 1"`를 콘솔에 기록하고 함수 종료를 위해 `return`를 호출해야 합니다.

# --hints--

입력 `size`가 유효하지 않으면 다음 오류 메시지 `"Chunk size must be >= 1"`를 콘솔에 기록해야 합니다.
...
오류 메시지를 기록한 후에는 즉시 함수 조기 종료를 위해 `return`를 호출해야 합니다.
...
`chunkCrew` 함수는 `size`가 1 이상일 때 오류 없이 유효성 검사를 통과해야 합니다.
