...
Para finalmente tocar a música, use o método `play()` na variável `audio`. `play()` é um método da web audio API para reproduzir um arquivo mp3.

# --hints--

Você deve usar a propriedade `classList` e o método `add` para adicionar a classe `"playing"` ao `playButton`.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

Você deve usar o método `play()` na variável `audio`.
