...
All'interno del metodo `showFortune` dichiara una variabile chiamata `target` e assegnala a `e.target` e crea un'altra istruzione if vuota.

# --hints--

Dovresti dichiarare una variabile chiamata `target` all'interno di `showFortune`.
...
La tua variabile `target` dovrebbe essere assegnata a `e.target`.
...
Dovresti avere un'istruzione `if` vuota subito dopo la tua variabile `target`.
