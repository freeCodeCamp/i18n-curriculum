...
Seleziona tutti gli elementi `label` all'interno degli elementi `.info`, imposta il loro `width` su `10%` e fai in modo che non occupino meno di `55px`.

# --hints--

Dovresti usare il selettore `.info label` oppure `.info > label`.
...
Dovresti dare agli elementi `label` un `width` di `10%`.
...
Dovresti dare agli elementi `label` un `min-width` di `55px`.
