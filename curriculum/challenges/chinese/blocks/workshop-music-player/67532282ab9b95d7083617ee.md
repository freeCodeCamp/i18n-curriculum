...
在 `playNextSong` 函数内，调用 `setPlayerDisplay`、`highlightCurrentSong` 和 `setPlayButtonAccessibleText` 函数，以在播放列表结束时正确更新播放器。

# --hints--

你应该在你的 `playNextSong` 函数的 `else` 语句中调用 `setPlayerDisplay`。
...
你应该在你的 `playNextSong` 函数的 `else` 语句中调用 `highlightCurrentSong`。
...
你应该在你的 `playNextSong` 函数的 `else` 语句中调用 `setPlayButtonAccessibleText`。
