...
現在你需要將這些元素縮小。為你的 `span[class~="sr-only"]` 選擇器添加 `width` 和 `height` 屬性，並將其設為 `1px`。

# --hints--

你的 `span[class~="sr-only"]` 選擇器應該設定 `width` 屬性為 `1px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('width') === '1px');
```

你的 `span[class~="sr-only"]` 選擇器應該設定 `height` 屬性為 `1px`。
