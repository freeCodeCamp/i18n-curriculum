...
使用 `const` 來創建一個名為 `checkpointPositions` 的新陣列。 
...
你應該有一個 `checkpointPositions` 陣列。

```js
assert.isArray(checkpointPositions);
```

你應該在 `checkpointPositions` 陣列中有三個物件。

```js
assert.lengthOf(checkpointPositions, 3);
```

你應該有一個物件，其 `x` 屬性設為 `1170`，`y` 屬性設為 `proportionalSize(80)`，以及 `z` 屬性設為 `1`。

```js
assert.deepStrictEqual(checkpointPositions[0], { x: 1170, y: proportionalSize(80), z: 1 });
```

你應該有一個物件，其 `x` 屬性設為 `2900`、`y` 屬性設為 `proportionalSize(330)`，以及 `z` 屬性設為 `2`。

```js
assert.deepStrictEqual(checkpointPositions[1], { x: 2900, y: proportionalSize(330), z: 2 });
```

你應該有一個物件，其 `x` 屬性設為 `4800`，`y` 屬性設為 `proportionalSize(80)`，以及 `z` 屬性設為 `3`。
