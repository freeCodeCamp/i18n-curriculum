...
最後に、送信ボタンは上の`fieldset`から離して配置し、幅が`300px`を下回らないように調整してください。

`margin`のプロパティを変更して、上下に`1em`を含め、左右のマージンは`auto`のままにしてください。その後、幅を上記のように設定してください。

# --hints--

`width`のプロパティは変更しないでください。`min-width`のプロパティを使ってください。
...
既存の`margin`のプロパティを使って、上下に`1em`を含めてください。
