...
在之前的步骤中，你构建了播放歌曲的功能性。现在你需要为播放按钮添加功能性。

在 `playButton` 上使用 `addEventListener()` 方法，并将 `"click"` 事件作为第一个参数传入。现在，使用一个回调作为第二个参数，该回调调用 `playSong(0)`。
...
你应该为 `playButton` 添加一个 `click` 事件监听器，并将调用 `playSong(0)` 的回调函数作为第二个参数传递给 `addEventListener` 方法。
