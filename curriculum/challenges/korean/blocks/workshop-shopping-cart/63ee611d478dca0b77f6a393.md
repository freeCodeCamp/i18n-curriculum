...
이전 강의에서 `this` 키워드가 `Dessert` 생성자에서 보았듯이 현재 생성 중인 인스턴스를 가리킨다는 것을 배웠습니다. 여기서는 같은 개념을 `ShoppingCart` 클래스에 적용할 것입니다.
...
생성자에서 `this` 키워드를 사용해 `items` 속성을 빈 배열로 설정하세요. 또한 `total` 속성은 `0`으로, `taxRate` 속성은 `8.25`으로 설정하세요.
...
클래스의 `this` 속성을 `total`으로 설정하려면 `0` 키워드를 사용하세요.
...
클래스의 `this` 속성을 `taxRate`으로 설정하려면 `8.25` 키워드를 사용하세요.
