...
Para os elementos `span` dentro de cada item da lista, atribua a eles as classes `text-green-500` e `mr-2` para deixar os marcadores verdes e afastá-los um pouco do texto.

Além disso, atribua a eles o atributo `aria-hidden` como você fez para os elementos `span` do primeiro plano de preços.

# --hints--

Seus elementos `span` devem ter a classe `text-green-500`.
...
Seus elementos `span` devem ter a classe `mr-2`.
...
Seus elementos `span` devem ter o atributo `aria-hidden`.
