...
Sasa umehifadhi zoezi katika safu ya data ya `taskData`, unapaswa kuonyesha zoezi hilo kwenye ukurasa kwa kuzungusha kupitia safu hiyo.

Tumia `forEach()` kwenye `taskData`, kisha gawanisha `id`, `title`, `date`, `description` kama vigezo. Usirudishe chochote bado.

# --hints--

Unapaswa kutumia `forEach()` kwenye `taskData`.

```js
assert.match(code, /taskData\.forEach\(\s*/)
```

Unapaswa kutumia `{id, title, date, description}` kama vigezo vilivygawanywa vya kitendakazi cha mwitiko cha `forEach()`.
