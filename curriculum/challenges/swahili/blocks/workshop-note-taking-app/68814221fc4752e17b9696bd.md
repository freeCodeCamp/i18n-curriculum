...
Wakati mtumizi anapobofya kwenye noti, kufanya mabadiliko kisha kubofya nje ya noti hiyo, utahitaji njia ya kupata toleo hilo jipya lililorekebishwa. Unaweza kufanikisha hili kwa kutumia tukio la `blur`. Tukio hili hufanyika wakati kipengele kinapopoteza makini. Hapa kuna mfano:
...
Unganisha njia ya `addEventListener` kwa kwenye kigezo cha `noteEl`. Hoja ya kwanza inapaswa kuwa tukio la `"blur"` na hoja ya pili inapaswa kuwa kitendakazi cha mshale. Ndani ya mwili wa kitendakazi hicho cha mshale, tumia `const` kuunda kigezo kipya kinachoitwa `newContent` na weka thamani yake kuwa `noteEl.innerHTML`.

`innerHTML` inatumiwa hapa kusaidia kuhifadhi nafasi na muundo wa noti.

# --hints--

Unapaswa kuunganisha njia ya `addEventListener` kwa kwenye kigezo cha `noteEl`.

```js
assert.match(code, /noteEl\.addEventListener/);
```

Njia yako ya `addEventListener` inapaswa kuwa na hoja ya kwanza ya tukio la `"blur"`.

```js
assert.match(code, /noteEl\.addEventListener\(['"]blur['"],/);
```

Ndani ya mwili wa kitendakazi cha mshale, unapaswa kutumia `const` kuunda kigezo kipya kinachoitwa `newContent` na weka thamani yake kuwa `noteEl.innerHTML`.
