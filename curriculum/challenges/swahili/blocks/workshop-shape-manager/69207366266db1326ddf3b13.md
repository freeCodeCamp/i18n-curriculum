...
Add the `DOMContentLoaded` event to the root document and pass in `initializeApp` as the callback function to run. This ensures all initialization happens only after the DOM is fully loaded.
