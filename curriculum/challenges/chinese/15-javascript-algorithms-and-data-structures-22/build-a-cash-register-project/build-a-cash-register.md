...
**目标：**构建一个功能类似于 <a href="https://cash-register.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://cash-register.freecodecamp.rocks</a> 的应用。

**用户需求：**

1. 你应该有一个 `id` 为 `"cash"` 的 `input` 元素。
1. 你应该有一个 `div`、`span` 或 `p` 元素，`id` 为 `"change-due"`。
1. 你应该有一个 `button` 元素，`id` 为 `"purchase-btn"`。
1. 当 `#cash` 元素中的值小于 `price`，应该提示 `"Customer does not have enough money to purchase the item"`。
1. 当 `#cash` 元素中的值等于 `price` 时，`#change-due` 元素中的值应该是 `"No change due - customer paid with exact cash"`。
1. 当 `price` 为 `19.5` 时，`#cash` 元素的值为 `20`，`cid` 的值为 `[["PENNY", 1.01], ["NICKEL", 2.05], ["DIME", 3.1], ["QUARTER", 4.25], ["ONE", 90], ["FIVE", 55], ["TEN", 20], ["TWENTY", 60], ["ONE HUNDRED", 100]]`，当 `#purchase-btn` 元素被点击后，`#change-due` 元素的值应该是 `"Status: OPEN QUARTER: $0.5"`。
1. 当 `price` 为 `3.26` 时，`#cash` 元素的值为 `100`，`cid` 的值为 `[["PENNY", 1.01], ["NICKEL", 2.05], ["DIME", 3.1], ["QUARTER", 4.25], ["ONE", 90], ["FIVE", 55], ["TEN", 20], ["TWENTY", 60], ["ONE HUNDRED", 100]]`，当 `#purchase-btn` 元素被点击后，`#change-due` 元素的值应该是 `"Status: OPEN TWENTY: $60 TEN: $20 FIVE: $15 ONE: $1 QUARTER: $0.5 DIME: $0.2 PENNY: $0.04"`。
1. 当 `price` 为 `19.5` 时，`#cash` 元素的值为 `20`， `cid` 的值为 `[["PENNY", 0.01], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 0], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]` ，当 `#purchase-btn` 元素被点击后，`#change-due` 元素的值应该是 `"Status: INSUFFICIENT_FUNDS"`。
1. 当 `price` 为 `19.5` 时，`#cash` 元素的值为 `20`， `cid` 的值为 `[["PENNY", 0.01], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 1], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]`，当 `#purchase-btn` 元素被点击后，`#change-due` 元素的值应该是 `"Status: INSUFFICIENT_FUNDS"`。
1. 当 `price` 为 `19.5` 时，`#cash` 元素的值为 `20`， `cid` 的值为 `[["PENNY", 0.5], ["NICKEL", 0], ["DIME", 0], ["QUARTER", 0], ["ONE", 0], ["FIVE", 0], ["TEN", 0], ["TWENTY", 0], ["ONE HUNDRED", 0]]`，当 `#purchase-btn` 元素被点击后，`#change-due` 元素的值应该是 `"Status: CLOSED PENNY: $0.5"`。
...
当 `price` 为 `20`、`#cash` 元素中的值为 `10` 且单击 `#purchase-btn` 元素时，应出现一条警告，其中显示文字 `"Customer does not have enough money to purchase the item"`。
...
当 `#cash` 元素中的值小于 `price` 并且点击 `#purchase-btn` 元素时，应该出现一个警告，其中显示文字 `"Customer does not have enough money to purchase the item"`。
...
当 `price` 为 `11.95` 时，`#cash` 元素中的值为 `11.95`，且点击 `#purchase-btn` 元素时，`#change-due` 元素中的值应为 `"No change due - customer paid with exact cash"`。
...
当 `#cash` 元素中的值等于 `price`，并且点击 `#purchase-btn` 元素时，`#change-due` 元素中的值应为 `"No change due - customer paid with exact cash"`。
