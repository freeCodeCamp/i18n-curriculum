---
id: 5f35e5c44359872a137bd98f
title: 第 24 步
challengeType: 0
dashedName: step-24
---

# --description--

由於咖啡館主要販售的產品是咖啡，你可以使用咖啡豆的影像作為頁面背景。

移除 `body` 型別選擇器內的註解及其內容。之後，為你的 `background-image` 屬性添加並將其值設為 `url(https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg)`。

# --hints--

你應該移除被註解掉的 `background-color` 屬性。

```js
assert.notMatch(code, /\/\*\s*background-color:\s*burlywood;?\s*\*\//i);
```

你的 `body` 選擇器不應該有任何註解。

```js
assert.notMatch(code, /body\s*{\s*\/\*/i);
```

你應該將 `background-image` 屬性設為 `url(https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg)`。

```js
const hasBackground = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style['background-image'] === `url("https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg")`)
assert.isTrue(hasBackground)
```

你的 `body` 元素應該有咖啡豆背景影像。

```js
const bodyBackground = new __helpers.CSSHelp(document).getStyle('body')?.getPropertyValue('background-image');

assert.equal(bodyBackground, `url("https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg")`);
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Cafe Menu</title>
    <link href="styles.css" rel="stylesheet"/>
  </head>
  <body>
    <div class="menu">
      <main>
        <h1>CAMPER CAFE</h1>
        <p>Est. 2020</p>
        <section>
          <h2>Coffee</h2>
        </section>
      </main>
    </div>
  </body>
</html>
```

```css
body {
--fcc-editable-region--
  /*
  background-color: burlywood;
  */
--fcc-editable-region--
}

h1, h2, p {
  text-align: center;
}

.menu {
  width: 80%;
  background-color: burlywood;
  margin-left: auto;
  margin-right: auto;
}
```

