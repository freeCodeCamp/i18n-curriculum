...
更新 `healthText.innerText` 和 `monsterHealthText.innerText` 使其等於 `health` 和 `monsterHealth`。

# --hints--

你應該更新 `healthText.innerText`。

```js
assert.match(attack.toString(), /healthText\.innerText/);
```

你應該將 `healthText.innerText` 設置爲 `health`。

```js
assert.match(attack.toString(), /healthText\.innerText\s*=\s*health/);
```

你應該更新 `monsterHealthText.innerText`。

```js
assert.match(attack.toString(), /monsterHealthText\.innerText/);
```

你應該將 `monsterHealthText.innerText` 設置爲 `monsterHealth`。
