...
1. Sie sollten ein `div`-Element mit einem `id` von `drum-machine` haben, das alle anderen Elemente enthält.

2. Innerhalb des `#drum-machine`-Elements sollten Sie ein weiteres `div` mit einem `id` von `pad-bank` haben.

3. Innerhalb des `#drum-machine`-Elements sollten Sie ein `p`-Element mit einem `id` von `display` haben.

4. Innerhalb Ihres `#pad-bank`-Elements sollten Sie neun anklickbare Drum-Pad-Elemente haben, jeweils mit einer Klasse von `drum-pad`, einem einzigartigen `id`, das den Audioclip beschreibt, den das Drum-Pad auslösen wird, und einem inneren Text, der einer der folgenden Tasten auf der Tastatur entspricht: `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`. Die Drum-Pads MÜSSEN in dieser Reihenfolge sein.

5. Jedes `.drum-pad` sollte ein `audio`-Element haben, das eine Klasse von `clip`, ein `src`-Attribut, das auf einen Audioclip zeigt, und ein `id` hat, das dem inneren Text seines übergeordneten `.drum-pad`-Elements entspricht (z. B. `id="Q"`, `id="W"`, `id="E"` usw.).

6. Wenn Sie auf ein `.drum-pad`-Element klicken, sollte der Audioclip, der in seinem untergeordneten Audioelement enthalten ist, ausgelöst werden.

7. Wenn Sie die Auslösetaste drücken, die mit jedem `.drum-pad` verknüpft ist, sollte der Audioclip, der in seinem untergeordneten Audioelement enthalten ist, ausgelöst werden (z. B. sollte das Drücken der `Q`-Taste das Drum-Pad auslösen, das den String `Q` enthält, das Drücken der `W`-Taste sollte das Drum-Pad auslösen, das den String `W` enthält usw.).

8. Wenn ein `.drum-pad` ausgelöst wird, sollten Sie einen String anzeigen, der den zugehörigen Audioclip als inneren Text des `#display`-Elements beschreibt (jeder String muss einzigartig sein).

Einige Audiosamples, die Sie für Ihre Schlagzeugmaschine verwenden können, finden Sie unter `https://cdn.freecodecamp.org/curriculum/drum/<fileName>`, wobei `<fileName>` wie folgt ist:

| Schlagzeugname | `fileName` |
| --- | --- |
...
Sie sollten ein `div`-Element mit einem `id` von `drum-machine` haben, das alle anderen Elemente enthält.

```js
assert.isNotNull(document.querySelector('div#drum-machine'));
```

Innerhalb des `#drum-machine`-Elements sollten Sie ein weiteres `div` mit einem `id` von `pad-bank` haben.

```js
assert.isNotNull(document.querySelector('#drum-machine div#pad-bank'));
```

Innerhalb des `#drum-machine`-Elements sollten Sie ein `p`-Element mit einem `id` von `display` haben.

```js
assert.isNotNull(document.querySelector('#drum-machine p#display'));
```

Innerhalb Ihres `#pad-bank`-Elements sollten Sie neun anklickbare Drum-Pad-Elemente haben, jeweils mit einer Klasse von `drum-pad`.

```js
assert.lengthOf(document.querySelectorAll('#pad-bank .drum-pad'), 9);
```

Jedes `.drum-pad` sollte ein `button`-Element sein.
...
Jedes `.drum-pad` sollte einen der folgenden Buchstaben als `innerText` haben, in der Reihenfolge: `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`.
...
Jedes `.drum-pad` sollte ein `audio`-Element haben, das eine Klasse von `clip`, ein `src`-Attribut, das auf einen Audioclip zeigt, und ein `id` hat, das dem inneren Text seines übergeordneten `.drum-pad`-Elements entspricht (z. B. `id="Q"`, `id="W"`, `id="E"` usw.).
...
Wenn Sie auf ein `.drum-pad`-Element klicken, sollte der Audioclip, der in seinem untergeordneten Audioelement enthalten ist, ausgelöst werden.
...
Wenn Sie eine der Tasten Q, W, E, A, S, D, Z, X, C auf Ihrer Tastatur drücken, sollte das entsprechende `audio`-Element den entsprechenden Sound abspielen.
...
Wenn ein `.drum-pad` ausgelöst wird, sollten Sie einen String anzeigen, der den zugehörigen Audioclip als inneren Text des `#display`-Elements beschreibt (jeder String muss einzigartig sein).
