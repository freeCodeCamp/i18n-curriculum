...
Você precisa exibir os produtos disponíveis no seu HTML. Comece usando o operador de atribuição de adição para adicionar uma string vazia de template literal à propriedade `innerHTML` da variável `dessertCards`.

# --hints--

Você deve acessar a propriedade `innerHTML` da variável `dessertCards`.

```js
assert.match(code, /dessertCards\.innerHTML/);
```

Você deve usar o operador de atribuição de adição na propriedade `innerHTML`.

```js
assert.match(code, /dessertCards\.innerHTML\s*\+=\s*/);
```

Você deve adicionar uma string vazia de template literal à propriedade `innerHTML`.
