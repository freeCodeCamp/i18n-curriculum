...
Crea un selettore `.list-title, .list-subtitle` e imposta la proprietà `color` su `#00beef`.

# --hints--

Dovresti avere un selettore `.list-title, .list-subtitle`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'));
```

Il tuo selettore `.list-title, .list-subtitle` dovrebbe avere una proprietà `color` impostata su `#00beef`.
