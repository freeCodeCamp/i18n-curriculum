...
在函式中，取得負責顯示歌曲標題和歌手的 HTML 元素的引用。

使用 `getElementById()` 方法存取 `#player-song-title` 和 `#player-song-artist` 元素。分別將它們指定給變數 `playingSong` 和 `songArtist`。

# --hints--

你不應該修改現有的 `setPlayerDisplay` 函式（程式）及其內容。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

你應該使用 `document.getElementById()` 來取得 `#player-song-title` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

你應該將 `#player-song-title` 元素指定給變數 `playingSong`。別忘了使用 `const` 來宣告該變數。

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

你應該使用 `document.getElementById()` 來取得 `#player-song-artist` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

你應該將 `#player-song-artist` 元素指定給變數 `songArtist`。別忘了使用 `const` 來宣告該變數。
