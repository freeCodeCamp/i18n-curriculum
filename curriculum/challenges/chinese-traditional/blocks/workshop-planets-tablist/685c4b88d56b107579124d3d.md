...
如果你查看 HTML，你會看到第二和第三個面板被隱藏。這是使用 `hidden` 屬性完成的。第一個面板預設是可見的。當使用者點擊分頁時，所有面板都應該被隱藏。

使用 `forEach` 方法來迴圈瀏覽 `panels`，並將每個面板的 `hidden` 屬性設為 `true`。 

# --hints--

你應該對 `panels` 進行迴圈，並將每個面板的 `hidden` 屬性設為 `true`。
