...
Функція `idToText` наразі повертає елемент `input`. Оновіть його, щоб повернути `value` того елемента `input`.

# --hints--

Поверніть властивість `value` значення, яке повертає метод `.find()`.
