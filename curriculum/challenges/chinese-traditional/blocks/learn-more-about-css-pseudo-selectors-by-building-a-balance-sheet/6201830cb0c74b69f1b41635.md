...
爲確保表格填充其容器，將 `width` 設置爲 `100%` 。然後將其相對定位併爲其提供 `3rem` 的上邊距。

# --hints--

你的 `table` 選擇器應該有一個 `width` 屬性，設置爲 `100%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('width') === '100%');
```

你的 `table` 選擇器應該有一個 `position` 屬性，設置爲 `relative`。

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('position') === 'relative');
```

你的 `table` 選擇器應該有一個 `margin-top` 屬性，設置爲 `3rem`。
