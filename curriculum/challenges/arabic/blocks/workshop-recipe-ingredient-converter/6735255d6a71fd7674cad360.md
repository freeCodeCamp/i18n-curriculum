...
أخيرًا، أضف إلى `conversionTable` الخاص بك كائن `teaspoon` يحتوي على خاصية `cup` بقيمة `1/48`، وخاصية `gram` بقيمة `5`، وخاصية `ounce` بقيمة `0.167`.

# --hints--

يجب أن يحتوي `conversionTable` الخاص بك على خاصية `teaspoon`.

```js
assert.property(conversionTable, "teaspoon");
```

يجب أن تكون خاصية `teaspoon` كائنًا.

```js
assert.isObject(conversionTable.teaspoon);
```

يجب أن يحتوي كائن `teaspoon` على خاصية `cup`.

```js
assert.property(conversionTable.teaspoon, "cup");
```

يجب تعيين خاصية `cup` إلى `1/48`.

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

يجب أن يحتوي كائن `teaspoon` على خاصية `gram`.

```js
assert.property(conversionTable.teaspoon, "gram");
```

يجب تعيين خاصية `gram` إلى `5`.

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

يجب أن يحتوي كائن `teaspoon` على خاصية `ounce`.

```js
assert.property(conversionTable.teaspoon, "ounce");
```

يجب تعيين خاصية `ounce` إلى `0.167`.
