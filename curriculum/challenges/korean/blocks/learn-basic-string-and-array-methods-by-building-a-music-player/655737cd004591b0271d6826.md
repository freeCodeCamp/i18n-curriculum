...
재생목록을 원래 상태로 재설정하려면 `allSongs`를 배열로 펼쳐서 `userData.songs`에 할당하세요.

*참고*: 이 시점에서 노래가 `userData.songs`나 `null`가 아니므로 `undefined`에 선택적 체이닝을 사용하면 안 됩니다.

# --hints--

기존 이벤트 리스너와 그 내용을 수정하면 안 됩니다.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*\)\s*;?/)
```

`[...allSongs]`를 `userData.songs`에 할당해야 합니다.
