...
`map()` の中に、曲の詳細を表示するためのすべての要素を埋め込むバッククォート付きの `return` 文を追加してください。

バッククォートの中に、`li` の `id` 属性と `song-${song.id}` の `class` 属性を持つ `playlist-song` 要素を作成してください。

# --hints--

空のバッククォートペアを持つ `return` 文を追加してください。
...
開始の `li` タグには `id` に設定された `song-${song.id}` 属性が必要です。

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

開始の `li` タグには `class` に設定された `playlist-song` 属性が必要です。
