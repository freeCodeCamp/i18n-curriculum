...
لا تزال داخل صنف `Bug`، أعلن عن خاصية باسم `emojiElement` من النوع `HTMLParagraphElement`. ستحتفظ هذه الخاصية بمرجع إلى عنصر الفقرة في HTML حيث سيُعرض رمز التعبير الخاص بالخطأ البرمجي.

# --hints--

يجب أن يحتوي صنف `Bug` على خاصية باسم `emojiElement`.
...
يجب أن تكون خاصية `emojiElement` من النوع `HTMLParagraphElement`.
...
يجب أن تستخدم خاصية `emojiElement` عامل تحقق التعيين القطعي (`!`).
