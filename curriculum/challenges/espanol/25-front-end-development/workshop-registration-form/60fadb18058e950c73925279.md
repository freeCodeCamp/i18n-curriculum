...
Durante el desarrollo, es útil ver los bordes predeterminados del `fieldset`. Sin embargo, hacen que el contenido parezca demasiado separado.

Elimina el `border` y agrega `2rem` de padding solo en la parte superior e inferior de cada `fieldset`. Asegúrate de eliminar el `padding` de los lados izquierdo y derecho.

# --hints--

Puedes usar un valor de `none` o `0` para eliminar el `border`.

```js
assert.match(new __helpers.CSSHelp(document).getStyle('fieldset')?.border, /(none)|(0px)|(medium)/);
```

Debes añadir un `padding` de `2rem` en la parte superior e inferior de cada `fieldset`.
...
Debes eliminar el `padding` de los lados izquierdo y derecho de cada `fieldset`.
