...
Далі потрібно відобразити поточну назву пісні та виконавця на дисплеї плеєра. Використайте `const` та стрілковий синтаксис, щоб створити порожню `setPlayerDisplay` функцію.

# --hints--

Ви повинні використати `const`, щоб створити порожню функцію з ім’ям `setPlayerDisplay`.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` має бути функцією.

```js
assert.isFunction(setPlayerDisplay)
```

Ваша `setPlayerDisplay` функція має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

Ваша `setPlayerDisplay` функція має бути порожньою.
