...
Restituisci `true` se la stringa data è un palindrome. Altrimenti, restituisci `false`.

Un <dfn>palindrome</dfn> è una parola o una frase che si legge allo stesso modo sia da sinistra a destra sia da destra a sinistra, ignorando punteggiatura, maiuscole/minuscole e spazi.

**Nota:** Dovrai rimuovere **tutti i caratteri non alfanumerici** (punteggiatura, spazi e simboli) e trasformare tutto nello stesso caso (minuscolo o maiuscolo) per poter verificare i palindrome.

Passeremo stringhe con formati diversi, come `racecar`, `RaceCar` e `race CAR` tra gli altri.

Passeremo anche stringhe con simboli speciali, come `2A3*3a2`, `2A3 3a2` e `2_A3*3#A2`.

# --hints--

`palindrome("eye")` dovrebbe restituire un booleano.
