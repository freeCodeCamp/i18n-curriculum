...
Für Ihren truthy-Ausdruck, bei dem Sie `undefined` entfernen, müssen Sie die `textContent` der `currentProductCountSpan` so aktualisieren, dass sie der `currentProductCount` gefolgt von einem `x` ist. Verwenden Sie dazu ein Template Literal.

# --hints--

Sie sollten `undefined` aus Ihrem truthy-Ausdruck entfernen.
...
Sie sollten auf die `textContent`-Eigenschaft von `currentProductCountSpan` zugreifen.
...
Sie sollten die Syntax für Template Literals verwenden, um die `textContent` so zu aktualisieren, dass sie `${currentProductCount}x` ist.
