...
讓我們看看驗證器如何抓出真正的問題。`catalog[0]` 是 Stephen King 的《From a Buick 8》，其中所有四個欄位都有填寫。`catalog[16]` 是《Unknown Title》，作者欄位是空白的。

將 `validateEntry(catalog[0])` 和 `validateEntry(catalog[16])` 記錄到主控台。第一個應該傳回 `true`；第二個應該傳回 `false`。

# --hints--

你應該將 `validateEntry(catalog[0])` 記錄到主控台。

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

你應該將 `validateEntry(catalog[16])` 記錄到主控台。
