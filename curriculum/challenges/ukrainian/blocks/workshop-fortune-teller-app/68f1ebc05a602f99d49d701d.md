...
Тепер всередині `showElements` передайте ці аргументи: `singleCardBtn`, `multipleCardsBtn` і `headerTitle`. Пам’ятайте, що ви отримуєте їх із `elements` і отримуєте доступ за допомогою ключового слова `this`.

# --hints--

Ви повинні передати `singleCardBtn` як перший аргумент функції `showElements`.
...
Ви повинні передати `multipleCardsBtn` як другий аргумент функції `showElements`.
...
Ви повинні передати `headerTitle` як аргумент функції `showElements`.
