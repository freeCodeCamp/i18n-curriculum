...
Fügen Sie eine `if`-Anweisung hinzu, um zu überprüfen, ob `foundCard` existiert, bevor Sie die Elemente mit dessen Daten oder Inhaltsblock aktualisieren.

# --hints--

Sie sollten eine `if`-Anweisung mit der Bedingung `foundCard` deklarieren.
