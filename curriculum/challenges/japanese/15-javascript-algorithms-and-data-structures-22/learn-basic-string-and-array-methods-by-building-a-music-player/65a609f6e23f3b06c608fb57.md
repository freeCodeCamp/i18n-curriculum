...
`sort()` メソッドは、ソート順序を定義する比較コールバック関数を受け取ります。

下記の例の最初の条件 `(a.name < b.name)` は、最初の果物の名前が 2 番目の果物の名前より「小さい」かどうかをチェックしています。 当てはまる場合、最初の果物を 2 番目の果物の前に並べる処理となっています。

文字列は辞書のように、文字ごとに比較されます。 例えば `"Apples"` は `"Bananas"` より「小さい」と見なされます。アルファベット順で `"A"` が `"B"` より前に来るためです。

そして `sort()` メソッドは比較コールバック関数から数値が返されることを期待しているため、この例でも数字を返しています。 比較コールバック関数から負の数を返すことにより、最初のアイテムが 2 番目のアイテムの前に並べられます。
...
ではコールバック関数の中に、`a.title` が `b.title` より小さいかどうかをチェックする `if` 文を追加してください。 当てはまる場合、`-1` を返してください。

# --hints--

コールバック関数の中に `if` 文が必要です。

```js
assert.match(code, /if\s*\(/);
```

`if` 文では `a.title` が `b.title` より小さいことをチェックしてください。

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

`if` 文で、`a.title` が `b.title` より小さい場合は `-1` を返してください。
