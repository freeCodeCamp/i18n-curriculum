...
title: Створіть проєкт калькулятора площі багатокутника
challengeType: 23
...
Коли створюється об’єкт Rectangle, він має ініціалізуватися атрибутами `width` і `height`. Клас також повинен містити такі методи:
...
- `get_picture`: Повертає рядок, який представляє форму за допомогою рядків із символів '\*'. Кількість рядків має дорівнювати висоті, а кількість '\*' у кожному рядку — ширині. В кінці кожного рядка має бути новий рядок (`\n`). Якщо ширина або висота більші за 50, цей метод має повертати рядок: `'Too big for picture.'`.
- `get_amount_inside`: Приймає іншу форму (квадрат або прямокутник) як аргумент. Повертає кількість разів, скільки передана форма може поміститися всередині цієї форми (без обертання). Наприклад, прямокутник шириною 4 і висотою 8 може вмістити два квадрати зі сторонами 4.

Крім того, якщо екземпляр `Rectangle` представлений у вигляді рядка, він має виглядати так: `'Rectangle(width=5, height=10)'`.

## Клас Square

Клас `Square` має бути підкласом `Rectangle`. Коли створюється об’єкт `Square`, передається довжина однієї сторони. Метод `__init__` має зберігати довжину сторони в обох атрибутах `width` і `height` класу `Rectangle`.

Клас `Square` має мати доступ до методів класу `Rectangle`, але також повинен містити метод `set_side`. Якщо екземпляр `Square` представлений у вигляді рядка, він має виглядати так: `'Square(side=9)'`.

Крім того, методи `set_width` і `set_height` у класі `Square` мають встановлювати і ширину, і висоту.
...
Примітка: відкрийте консоль браузера за допомогою F12, щоб побачити більш детальний вивід тестів.
