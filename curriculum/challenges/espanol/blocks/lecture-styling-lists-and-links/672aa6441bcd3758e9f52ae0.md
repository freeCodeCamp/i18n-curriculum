...
Para aplicar algo de espaciado a la parte inferior de cada elemento de lista, puedes usar la propiedad `margin-bottom` así:
...
Otra forma de espaciar los elementos de lista sería usando la propiedad `line-height`.

La propiedad `line-height` ajusta el espaciado vertical entre las líneas de texto dentro de un único elemento de lista.
...
Si los elementos de lista tienen múltiples líneas de texto, la `line-height` afectará el espaciado entre esas líneas, pero no ajusta directamente el espaciado entre los elementos de lista separados.

Para controlar el espaciado entre elementos individuales de lista, se utilizarían propiedades como `margin` o `padding`.

Usando la misma lista desordenada de antes, aquí hay un ejemplo de aplicar `line-height` a los elementos de lista:
...
En este ejemplo, `line-height: 2;` establece que la altura de línea sea el doble del tamaño de la fuente, creando más espacio vertical dentro de cada elemento de lista.
...
Esta propiedad afecta el espacio dentro del `border` del elemento.
...
Esta propiedad afecta el `border` del elemento, no el espacio fuera de él.
...
Añade `15px` de espacio entre el contenido y el `border` de cada elemento de lista.

### --feedback--

Esta regla afecta el espacio fuera del `border` de cada elemento de lista.
...
¿Qué controla la propiedad `line-height` en CSS?

## --answers--

El `width` del contenido.

### --feedback--

Esto es controlado por la propiedad `width`, no por `line-height`.

---

El espacio dentro del `border` de un elemento.

### --feedback--

Esto es controlado por propiedades como `padding`, no por `line-height`.
...
El espacio fuera del `border` de un elemento.

### --feedback--

Esto es controlado por propiedades como `margin`, no por `line-height`.
