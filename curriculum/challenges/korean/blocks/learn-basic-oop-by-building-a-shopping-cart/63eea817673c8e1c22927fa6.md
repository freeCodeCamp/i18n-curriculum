...
`const`와 구조 분해를 사용해 `name`에서 `price`과 `product` 변수를 추출하세요.

# --hints--

`name`와 `price` 변수를 얻으려면 구조 분해를 사용하세요.
...
`const`와 `name` 변수를 선언하려면 `price`를 사용하세요.
...
`name`에서 `price`와 `product` 변수를 얻으려면 구조 분해를 사용하세요.
