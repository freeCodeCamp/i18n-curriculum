...
Annida un'istruzione `if` all'interno dell'attuale `if`. Per la condizione `if`, usa una chiamata ricorsiva a `solver()` e restituisci `True` dal nuovo corpo `if`.

# --hints--

Dovresti annidare un'istruzione `if` all'interno dell'attuale istruzione `if` e usare `self.solver()` come condizione.
...
Dovresti restituire `True` dalla tua nuova istruzione `if`.
