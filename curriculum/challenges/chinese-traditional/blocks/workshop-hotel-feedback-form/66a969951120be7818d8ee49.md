...
表單的下一部分將提供使用者為飯店留下評分的功能。 

首先為你的表單添加一個新的 `fieldset` 元素，並在其中巢狀一個 `legend` 元素。`legend` 應該包含文字 `Ratings`。

在 `legend` 元素下方，添加一個帶有文字 `How was the service?` 的 `label` 元素。`for` 屬性應設為 `"service"`。
