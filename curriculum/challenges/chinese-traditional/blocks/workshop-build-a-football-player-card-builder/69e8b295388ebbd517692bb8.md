...
在具有 `div` 為 `className` 的 `card-tier-badge` 元素下方，添加一個具有 `div` 為 `className` 且文字為 `card-club` 的 `Santos FC`。

# --hints--

你應該在 `div` 下方添加一個具有 `className` 為 `card-club` 的 `.card-tier-badge`。

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

`.card-club` 元素應該包含文字 `Santos FC`。
