...
أنشئ محدِّدًا جديدًا `.left-container p` واضبط هامش الأعلى والأسفل على `-5px`، وهامش اليسار واليمين على `-2px`. اضبط أيضًا `font-size` على `2em` و`font-weight` على `700`.

# --hints--

يجب أن يكون لديك محدِّد جديد `.left-container p`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

يجب أن يحتوي محدِّد `.left-container p` الجديد على خاصية `margin` مضبوطة على `-5px -2px`.
...
يجب أن يحتوي محدِّد `.left-container p` الجديد على خاصية `font-size` مضبوطة على `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

يجب أن يحتوي محدِّد `.left-container p` الجديد على خاصية `font-weight` مضبوطة على `700`.
