...
Виходячи з діалогу, яке речення демонструє правильне використання `going to` на основі наявних теперішніх доказів?
...
У цьому реченні `will` використовується для ймовірної майбутньої дії, що не передає відчуття рішення, прийнятого на основі теперішніх доказів.
...
Тут `will` використовується для майбутньої дії, що базується на поточній ситуації, але `going to` був би доречнішим, оскільки рішення, здається, ґрунтується на теперішньому досвіді команди.
...
У цьому реченні `will` використовується для спонтанного рішення (`calling the designer`), що є доречним, але воно не ілюструє використання `going to` на основі теперішніх доказів.
