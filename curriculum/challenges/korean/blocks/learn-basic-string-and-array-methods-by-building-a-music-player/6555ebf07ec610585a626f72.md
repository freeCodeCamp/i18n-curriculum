...
`filter()` 메서드를 사용해 `id` 매개변수와 일치하는 노래 객체를 `userData?.songs` 배열에서 제거하세요.

`filter` 메서드는 전달된 콜백 함수 조건을 만족하는 배열 요소만 유지합니다:
...
`filter()`에 `userData?.songs` 메서드를 사용하세요. 화살표 함수 콜백의 매개변수로 `song`를 전달하고 암묵적 반환을 사용해 `song.id`가 `id`와 엄격히 같지 않은지 확인하세요. 이 모든 것을 `userData.songs`에 할당하세요.

*참고*: `userData?.songs.filter`의 결과를 `userData.songs`에 할당할 때는 선택적 체이닝을 사용하지 마세요. 그 시점에 `allSongs` 배열이 `undefined` 또는 `null`가 아니기 때문입니다.

# --hints--

기존 `deleteSong` 함수는 수정하지 마세요.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

`filter()`에 `userData?.songs` 메서드를 사용하세요.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

`filter()` 메서드의 콜백 함수는 화살표 구문을 사용하고 `song` 매개변수를 가져야 합니다.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

filter 메서드는 `song.id !== id`를 암묵적으로 반환해야 합니다.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

`filter()`에 사용한 `userData?.songs` 메서드를 `userData.songs`에 할당하세요.
