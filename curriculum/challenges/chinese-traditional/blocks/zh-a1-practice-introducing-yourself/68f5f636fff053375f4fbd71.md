...
`您好 (nín hǎo)`, `設計師 (shè jì shī)`, `新加坡人 (xīn jiā pō rén)`, and `我是 (wǒ shì)`.
...
Liu Ming first greets politely with `您好 (nín hǎo)`, then uses `我是 (wǒ shì)` to introduce his name. He also introduces his nationality as `新加坡人 (xīn jiā pō rén)` and his profession as `設計師 (shè jì shī)`.
