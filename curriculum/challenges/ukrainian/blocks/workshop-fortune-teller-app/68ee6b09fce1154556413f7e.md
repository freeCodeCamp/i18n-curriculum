...
Всередині методу `initializeEventListeners` приєднайте обробник події `click` до `singleCardBtn`. У обробнику події викличте метод `singleCardSelected`. Не забудьте отримати ідентифікатор `singleCardBtn` з `elements`.

Цей метод ви створите у наступних кроках.

# --hints--

Ви повинні приєднати подію `click` до елемента `singleCardBtn`. Не забувайте, що тут потрібне ключове слово `this`.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

У функції зворотного виклику події слід викликати метод `singleCardSelected`.
