...
O elemento `.text` não é uma grade do CSS, mas você pode criar colunas dentro de um elemento sem usar o Grid usando a propriedade `column-width`.

Dê ao seletor `.text` uma propriedade `column-width` definida como `25rem`.

# --hints--

O seletor `.text` deve ter a propriedade `column-width` definida como `25rem`.
