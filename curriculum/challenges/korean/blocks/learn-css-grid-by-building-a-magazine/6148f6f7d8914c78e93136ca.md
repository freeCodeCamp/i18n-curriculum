...
`@media`에 대해 `only screen`의 `max-width`를 가진 마지막 `420px` 쿼리를 하나 만드세요. 그 안에 `.hero-title`로 설정된 `font-size` 속성을 가진 `4.5rem` 선택자를 만드세요.

축하합니다! 이제 잡지가 완성되었습니다.

# --hints--

`@media`에 대한 새로운 `only screen and (max-width: 420px)` 쿼리가 있어야 합니다. 이것이 `@media` 쿼리 목록에서 마지막 쿼리여야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

새로운 `@media` 쿼리는 `.hero-title` 선택자를 가져야 합니다.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText === '.hero-title');
```

`.hero-title` 선택자는 `font-size`로 설정된 `4.5rem` 속성을 가져야 합니다.
