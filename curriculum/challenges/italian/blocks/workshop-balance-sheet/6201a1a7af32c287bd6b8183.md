...
Seleziona i tuoi elementi `td` con il valore `class` della `current` e imposta il carattere tipografico in corsivo.

# --hints--

Dovresti avere un selettore `td.current`.
...
Il tuo selettore `td.current` dovrebbe avere la proprietà `font-style` impostata su `italic`.
