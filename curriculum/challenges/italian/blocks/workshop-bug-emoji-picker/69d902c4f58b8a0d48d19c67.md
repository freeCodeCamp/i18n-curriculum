...
Ora popola `bugMap` con due elementi. Aggiungi `"bee"` come chiave mappata a una nuova istanza di `Bee` e `"spider"` come chiave mappata a una nuova istanza di `Spider`. Passa `bugEmojiElement` come argomento di entrambe.

# --hints--

Il tuo `bugMap` dovrebbe avere un elemento `"bee"` impostato su `new Bee(bugEmojiElement)`.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

Il tuo `bugMap` dovrebbe avere un elemento `"spider"` impostato su `new Spider(bugEmojiElement)`.
