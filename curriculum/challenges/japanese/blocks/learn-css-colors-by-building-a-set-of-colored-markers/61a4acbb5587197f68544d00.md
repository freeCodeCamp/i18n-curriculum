...
最後に、現在純粋な青である`linear-gradient`関数の3番目の色の引数を変更してください。

`rgb`関数を更新して、赤の値が`162`、緑の値が`27`、青の値が`27`になるようにしてください。

# --hints--

`.red`のCSSルールには、`background`プロパティが`linear-gradient(180deg, rgb(122, 74, 14) 0%, rgb(245, 62, 113) 50%, rgb(162, 27, 27) 100%)`に設定されている必要があります。
