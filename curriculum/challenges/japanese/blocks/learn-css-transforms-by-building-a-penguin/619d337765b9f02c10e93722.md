...
3番目と4番目のウェイポイントに対して、`transform` パターンをもう一度繰り返してください。

# --hints--

`30%` ウェイポイントには `transform` の `rotate(110deg) scaleX(-1)` を設定してください。
...
`40%` ウェイポイントには `transform` の `rotate(130deg) scaleX(-1)` を設定してください。
