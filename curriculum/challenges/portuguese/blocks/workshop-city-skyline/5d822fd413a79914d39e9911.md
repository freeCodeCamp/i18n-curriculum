...
Você deve adicionar três elementos `div` a `.fb1`.

```js
assert.lengthOf(document.querySelector("div.fb1")?.children, 3);
```

Você deve dar à primeira nova `div` a classe `fb1a`.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1a"));
```

Você deve dar à segunda nova `div` a classe `fb1b`.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1b"));
```

Você deve dar à terceira nova `div` a classe `fb1c`.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1c"));
```

Você deve colocar os novos elementos `div` na ordem `.fb1a + .fb1b + .fb1c`.
