...
國際銀行帳號（IBAN）是一種國際公認的跨國界識別銀行帳戶的方式，可降低傳輸錯誤的風險。

<abbr title="International Bank Account Number">IBAN</abbr> 由最多 34 個字母數字字元組成：
...
檢查碼可啟用銀行帳號的合理性檢查，以在提交交易之前確認其完整性。

# --instructions--

撰寫一個函式，該函式以 IBAN 字串作為參數。如果它有效，則傳回 true。否則，傳回 false。
...
`isValid("GB82 WEST 1234 5698 7654 32")` 應該傳回一個布林值。

```js
assert(typeof isValid('GB82 WEST 1234 5698 7654 32') == 'boolean');
```

`isValid("GB82 WEST 1234 5698 7654 32")` 應該傳回 `true`。

```js
assert.equal(isValid('GB82 WEST 1234 5698 7654 32'), true);
```

`isValid("GB82 WEST 1.34 5698 7654 32")` 應該傳回 `false`。

```js
assert.equal(isValid('GB82 WEST 1.34 5698 7654 32'), false);
```

`isValid("GB82 WEST 1234 5698 7654 325")` 應該傳回 `false`。

```js
assert.equal(isValid('GB82 WEST 1234 5698 7654 325'), false);
```

`isValid("GB82 TEST 1234 5698 7654 32")` 應該傳回 `false`。

```js
assert.equal(isValid('GB82 TEST 1234 5698 7654 32'), false);
```

`isValid("SA03 8000 0000 6080 1016 7519")` 應該傳回 `true`。
