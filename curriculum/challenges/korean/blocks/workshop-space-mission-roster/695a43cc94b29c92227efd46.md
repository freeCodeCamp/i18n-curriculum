...
스크립트에 `console.log()` 호출을 추가해 `EVAChunks` 배열을 검증했는데, 이제 이 호출이 더 이상 필요하지 않습니다. 다음 단계를 위해 터미널을 깔끔하게 유지하려면 `for loop`에서 데이터를 기록하는 `EVAChunks` 블록을 삭제하세요.

# --hints--

`for loop`에서 데이터를 기록하는 `EVAChunks` 블록을 제거해야 합니다. 두 루프가 모두 포함된 전체 블록을 삭제하세요.
