...
Ahora que tu función `.map()` está recibiendo la referencia de función devuelta `num => charRange(...).map(...)` de las llamadas curriadas de `addCharacters`, iterará correctamente sobre los elementos y pasará cada elemento como `n` a esa función.
