...
まず、既存の画像の下に新しい画像を追加してください。新しい画像の`src`属性を`https://cdn.freecodecamp.org/curriculum/cat-photo-app/running-cats.jpg`に設定します。

この画像がスマートフォンの画面幅とぴったり同じになると良いですね。

幸いなことに、ブートストラップを使えば、画像に`img-responsive`クラスを追加するだけで済みます。これを行うと、画像がページの幅にぴったり合うはずです。
...
新しい画像は古い画像の下にあり、`img-responsive`クラスを持っている必要があります。

```js
assert.isTrue(document.querySelectorAll('img')?.[1]?.classList?.contains('img-responsive'));
```

新しい画像に`smaller-image`クラスは付けないでください。

```js
assert.isFalse(document.querySelectorAll('img')?.[1]?.classList?.contains('smaller-image'));
```

新しい画像の`src`は`https://cdn.freecodecamp.org/curriculum/cat-photo-app/running-cats.jpg`である必要があります。
