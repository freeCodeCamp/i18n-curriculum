...
正如你从之前的课程中回忆的那样，你可以使用 `map` 方法来迭代不同的对象并动态渲染组件，如下所示：
...
使用 `map()` 方法从数组中渲染单个 `Card`。将 `name`、`title` 和 `bio` 作为 props 传递给 `Card` 组件。

# --hints--

你应该使用 `map()` 方法来迭代 `profiles` 数组。

```js
assert.match(code, /profiles\.map\s*\(/);
```

你应该从 `profile` 对象传递 `name`、`title` 和 `bio` 属性。
