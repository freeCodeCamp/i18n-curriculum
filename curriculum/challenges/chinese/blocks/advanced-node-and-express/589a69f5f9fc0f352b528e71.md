...
设置你的 GitHub 认证的最后一步是创建策略本身。`passport-github@~1.1.0` 已经作为依赖被添加，所以在你的 `auth.js` 文件中以 `GitHubStrategy` 的方式引入它，代码如下：`const GitHubStrategy = require('passport-github').Strategy;`。不要忘记引入并配置 `dotenv` 来使用你的环境变量。
...
当你认为页面已经正确时提交。如果遇到误差，你可以<a href="https://forum.freecodecamp.org/t/advanced-node-and-express/567135#implementation-of-social-authentication-ii-4" target="_blank" rel="noopener noreferrer nofollow">查看到目前为止完成的项目</a>。

# --hints--

应该添加 `passport-github` 依赖。
