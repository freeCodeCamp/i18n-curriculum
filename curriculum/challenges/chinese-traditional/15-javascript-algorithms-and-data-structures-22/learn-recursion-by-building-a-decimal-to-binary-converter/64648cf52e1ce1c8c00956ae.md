...
首先，將 `result` 的 `innerText` 屬性設置爲 `"Call Stack Animation"`。

# --hints--

你應該將 `result` 的 `innerText` 屬性設置爲字符串 `"Call Stack Animation"`。

```js
assert.match(String(showAnimation), /result\.innerText\s*=\s*('|"|`)Call Stack Animation\1/i);
```

調用 `showAnimation` 函數後，`result` 的 `innerText` 屬性應爲 `"Call Stack Animation"`。
