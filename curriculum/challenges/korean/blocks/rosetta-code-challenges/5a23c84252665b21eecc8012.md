...
정수 배열에 대해 스투지 정렬을 수행하는 함수를 작성하세요. 함수는 정렬된 배열을 반환해야 합니다.

스투지 정렬 알고리즘은 다음과 같습니다:
...
`stoogeSort`는 함수여야 합니다.

```js
assert(typeof stoogeSort == 'function');
```

`stoogeSort([25, 32, 12, 7, 20])`은 배열을 반환해야 합니다.

```js
assert(Array.isArray(stoogeSort([25, 32, 12, 7, 20])));
```

`stoogeSort([25, 32, 12, 7, 20])`는 `[7, 12, 20, 25, 32]`을 반환해야 합니다.

```js
assert.deepEqual(stoogeSort([25, 32, 12, 7, 20]), [7, 12, 20, 25, 32]);
```

`stoogeSort([38, 45, 35, 8, 13])`는 `[8, 13, 35, 38, 45]`를 반환해야 합니다.

```js
assert.deepEqual(stoogeSort([38, 45, 35, 8, 13]), [8, 13, 35, 38, 45]);
```

`stoogeSort([43, 36, 20, 34, 24])`는 `[20, 24, 34, 36, 43]`을 반환해야 합니다.

```js
assert.deepEqual(stoogeSort([43, 36, 20, 34, 24]), [20, 24, 34, 36, 43]);
```

`stoogeSort([12, 33, 26, 18, 1, 16, 38])`는 `[1, 12, 16, 18, 26, 33, 38]`를 반환해야 합니다.
...
`stoogeSort([3, 39, 48, 16, 1, 4, 29])`는 `[1, 3, 4, 16, 29, 39, 48]`을 반환해야 합니다.
