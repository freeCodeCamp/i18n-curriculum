...
Jetzt sollten Sie den inneren Kreis erstellen, der erscheint, wenn die Optionsschaltfläche aktiviert ist. Dafür verwenden Sie das `::before` Pseudoelement.

Ein Pseudoelement wie `::before` ermöglicht es Ihnen, zusätzlichen Inhalt vor dem eigentlichen Element einzufügen. Dies wird oft zu dekorativen Zwecken genutzt.

Richten Sie das `.contact-method-radio-btn::before` Pseudoelement aus, und setzen Sie eine `display` von `block`, eine `content` von `" "`, eine `width` von `10px`, eine `height` von `10px` und eine `border-radius` von `50%`.

# --hints--

Sie sollten einen `.contact-method-radio-btn::before` Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Setzen Sie die `display` Eigenschaft auf `block`.
...
Setzen Sie die `content` Eigenschaft auf `" "`.
...
Setzen Sie die `width` Eigenschaft auf `10px`.
...
Setzen Sie die `height` Eigenschaft auf `10px`.
...
Setzen Sie die `border-radius` Eigenschaft auf `50%`.
