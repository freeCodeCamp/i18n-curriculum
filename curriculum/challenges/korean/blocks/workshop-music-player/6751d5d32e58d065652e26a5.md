...
이제 현재 재생 중인 노래에 `aria-current` 속성을 추가해야 합니다.

`songToHighlight`이 존재하는지 확인하세요. 존재한다면 `setAttribute`에서 `songToHighlight` 메서드를 사용하고 `"aria-current"`와 `"true"`를 첫 번째와 두 번째 인수로 전달하세요.

# --hints--

`highlightCurrentSong`은 현재 재생 중인 노래에 `aria-current` 속성을 추가하고 이전에 재생된 노래에서는 해당 속성을 제거해야 합니다.
