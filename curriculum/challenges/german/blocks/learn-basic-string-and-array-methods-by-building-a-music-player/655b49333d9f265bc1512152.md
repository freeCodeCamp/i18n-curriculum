...
Beachten Sie, dass das Albumcover im HTML und die Songs im `userData.songs`-Array geändert wurden. Wir haben die ursprünglichen Songs gegen kürzere ausgetauscht, die Sie in den kommenden Schritten zum Testen Ihrer App verwenden können.

Als Nächstes müssen Sie prüfen, ob ein nächster Song zum Abspielen vorhanden ist. Rufen Sie den aktuellen Song-Index durch einen Aufruf der `getCurrentSongIndex()`-Funktion ab und speichern Sie ihn in einer `currentSongIndex`-Konstanten.

Erstellen Sie danach eine `nextSongExists`-Konstante, die den Boolean-Wert `true` oder `false` enthält, je nachdem, ob der nächste Song existiert.

# --hints--

Sie sollten den bestehenden `ended`-Ereignislistener und dessen Inhalt nicht verändern.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Sie sollten eine `currentSongIndex`-Konstante erstellen und sie auf den Aufruf der `getCurrentSongIndex`-Funktion setzen.
...
Sie sollten prüfen, ob ein nächster Song existiert, indem Sie `userData.songs.length` und `currentSongIndex` vergleichen, und das Ergebnis in einer `nextSongExists`-Konstanten speichern. Wenn der letzte Index des Songs-Arrays (`userData.songs.length - 1`) größer als currentSongIndex ist, bedeutet das, dass ein nächster Song vorhanden ist.
