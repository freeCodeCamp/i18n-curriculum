...
要最終顯示計算結果，請使用帶有 `propertyInputs` 的 `for...of` 迴圈來迭代 `Object.entries()` 物件。你可以在迴圈宣告中，在 `input` 前加逗號來忽略鍵。

# --hints--

你應該建立一個 `for...of` 迴圈，透過解構每個來自 `Object.entries(propertyInputs)` 的項目，跳過鍵只保留 `input`。

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

你的 `for...of` 迴圈應該要迭代 `Object.entries(propertyInputs)`。
