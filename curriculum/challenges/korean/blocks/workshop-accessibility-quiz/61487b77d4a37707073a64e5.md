...
화면 너비가 작을 때 `h1`가 텍스트 내용을 제대로 줄 바꿈하지 않습니다. `h1` 요소의 텍스트를 가운데 정렬하세요.

그다음 `main`에 패딩을 주어 `Student Info` 섹션 헤더가 완전히 보이도록 하세요.

# --hints--

`h1`에 `text-align`의 `center`를 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

`main` 요소를 선택하려면 `main` 선택자를 추가해야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

`main`에 최소 `padding-top`의 `25px`을 주어야 합니다.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

`padding-top` 값만 변경하세요.
