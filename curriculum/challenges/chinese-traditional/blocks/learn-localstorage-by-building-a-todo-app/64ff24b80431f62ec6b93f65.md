...
移除 `myTaskArr` 陣列和所有 `localStorage` 的程式碼，因為你不再需要它們。

# --hints--

你應該移除 `myTaskArr` 和所有你剛學到的與 `localStorage` 相關的程式碼。
...
你應該移除程式碼中任何剩餘的對 `myTaskArr` 的引用。

```js
assert.notMatch(code, /myTaskArr/);
```

你應該移除對 `localStorage.getItem` 中項目 `"data"` 的任何引用。

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

你應該移除程式碼中任何剩餘的對 `getTaskArrObj` 的引用。
