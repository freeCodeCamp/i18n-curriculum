...
你有一个轻微的错误。`arg1` 和 `arg2` 是字串，不是数字。`infixToFunction['+']("1", "2")` 会返回 `12`，这在数学上是不正确的。

将每个 `infixToFunction[operator]` 参数包装在 `parseFloat()` 调用中。

# --hints--

你应该将 `arg1` 包装在 `parseFloat()` 调用中。
