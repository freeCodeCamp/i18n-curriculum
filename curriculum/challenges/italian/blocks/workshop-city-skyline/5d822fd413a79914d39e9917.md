...
Puoi aggiungere gradienti multipli a un elemento separandoli con una virgola (`,`) in questo modo:
...
Aggiungi un `repeating-linear-gradient` a `.fb1c` sotto quello già presente; usa il tuo `--building-color4` da `0%` a `10%` e `--window-color4` da `10%` a `90%`. Questo riempirà lo sfondo dietro l'ultimo gradiente aggiunto.

# --hints--

Non dovresti modificare il primo `repeating-linear-gradient`.
...
Dovresti avere una virgola (`,`) dopo il primo `repeating-linear-gradient`.
...
Dovresti aggiungere un `repeating-linear-gradient` con un primo colore `--building-color4` da `0%` a `10%`.
...
Dovresti usare un secondo colore `--window-color4` da `10%` a `90%`.
