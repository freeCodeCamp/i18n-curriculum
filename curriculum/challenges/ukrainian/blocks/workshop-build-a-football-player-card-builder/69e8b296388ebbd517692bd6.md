...
Форма тепер завершена. Щоб картка попереднього перегляду оновлювалася під час введення, потрібно зв’язати вхідні дані зі станом `player`.

Починаючи з імені `input`, додайте до нього атрибут `value` зі значенням `player.name`. Потім додайте атрибут `onChange`, встановлений у стрілкову функцію, яка приймає `e` як параметр і викликає `setPlayer` з розгортанням `player` та `name`, встановленим у `e.target.value`. Не забудьте помістити і розгортання, і встановлення значення `name` в об’єкт.

# --hints--

Ваше поле введення імені `value` має бути прив’язане до поля стану player `name`.
...
Ваше поле введення імені має мати обробник `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Обробник `onChange` має викликати `setPlayer` з `setPlayer`, що містить об’єкт, який розгортає `player` і встановлює `name` у `e.target.value`.
