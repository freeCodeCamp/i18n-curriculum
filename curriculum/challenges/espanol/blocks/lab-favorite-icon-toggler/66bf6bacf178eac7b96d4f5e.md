...
**Nota:** Asegúrate de enlazar tu archivo JavaScript en tu HTML. (Ej. `<script src="script.js"></script>`)
...
Cuando se hace clic en el elemento `button`, y contiene la clase `filled`, debes eliminar la clase `filled` del elemento `button` y cambiar el innerHTML del elemento `button` a `&#9825;`.
...
Cuando se hace clic en el elemento `button`, y no contiene la clase `filled`, debes agregar la clase `filled` al elemento `button` y cambiar el `innerHTML` del elemento `button` a `&#10084;`.
