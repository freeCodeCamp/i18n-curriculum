...
El `--property-name` es el nombre de la propiedad personalizada que estás definiendo. Como todas las propiedades personalizadas, debe comenzar con dos guiones.
...
En este ejemplo, estamos definiendo una propiedad personalizada `--main-color` como un valor de `color`, estableciéndola para no heredar y dándole un valor inicial de `#3498db`.
...
Esto asegura que, incluso si la propiedad `--main-color` no está definida o es inválida, el botón aún tendrá un color de fondo.
...
La regla CSS `@property` se considera ampliamente disponible en todos los principales motores de navegadores modernos (como Chrome, Edge, Opera, Firefox y Safari) desde principios de 2024. Si quieres soportar navegadores más antiguos, necesitas proporcionar CSS de respaldo como se explicó arriba.
