...
雖然 `ul`/`li` 元素非常適合為列表項目提供項目符號，但你的圓鈕不需要它們。你可以使用 `list-style` 屬性來控制項目符號的外觀。例如，你可以使用以下方式將項目符號變成圓圈：
...
透過將 `.answers-list` 項目的樣式設為 `none`，移除其預設樣式，並移除無序列表的內距。
