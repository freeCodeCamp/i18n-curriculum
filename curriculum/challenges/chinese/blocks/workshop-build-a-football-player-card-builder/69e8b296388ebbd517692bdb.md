...
为 PAC `input` 添加一个 `value` 属性，属性值为 `player.pac`。然后添加一个 `onChange` 属性，属性值为一个箭头函数，该函数以 `e` 作为参数，并调用 `setPlayer`，传入展开的 `player` 和 `pac`，其中 `pac` 设置为 `Number(e.target.value)`。

# --hints--

你的 PAC `input` 应该有一个 `value`，其值等于玩家的 `pac` 属性。
...
你的 PAC `input` 应该有一个 `onChange` 处理程序，该处理程序调用 `setPlayer`，传入展开的 `player` 和 `pac`，其中 `pac` 设置为 `Number(e.target.value)`。
