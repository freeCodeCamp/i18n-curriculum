...
你是否注意到无需填写任何内容即可提交表单？

要停止这种情况，请添加 `disabled` 属性，以便在 `heroName`、`realName` 或 `powerSource` 为假时，或者 `powers` 的长度为 `0` 时禁用提交按钮。

现在，如果没有填写任何内容，按钮将被禁用。 在表格中填写你的超级英雄信息并提交以加入联盟。
...
你的 `button` 元素应该具有 `disabled` 属性。

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

如果 `heroName`、`realName` 和 `powerSource` 中的任何一个为假，或者 `powers` 的长度为 `0`，则应禁用 `button`。
