...
لضمان ظهور النص المُدخل في المدخلات ومساحات النص، وكذلك عناصر النائب الخاصة بها، عيّن لونها.

اختر `input, input::placeholder, textarea` وعيّن `color` إلى `whitesmoke`.

# --hints--

يجب أن تستهدف `input`، و`input::placeholder`، و`textarea`.
