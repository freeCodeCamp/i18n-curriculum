...
前のプロジェクトでは、Promiseが解決された後にロジックを実行するために`fetch()`の`.then()`メソッドを使いました。今回は`await`キーワードを使って、`fetch()`メソッドの非同期の性質を扱います。

`await`キーワードはPromiseが解決されるのを待ち、その結果を返します。
...
`try`ブロックの中で、`res`という定数を作成し、`await fetch()`を代入してください。`fetch`の呼び出しには`forumLatest`変数を渡してください。

# --hints--

`res`ブロックの中に`try`という定数があるはずです。

```js
assert.match(code, /\s*const\s+res/)
```

`await fetch()`変数に`res`を代入してください。

```js
assert.match(code, /\s*const\s+res\s*=\s*await\s+fetch\(\s*(.*)\s*\)/);
```

`forumLatest`の呼び出しに`fetch`定数を渡してください。
