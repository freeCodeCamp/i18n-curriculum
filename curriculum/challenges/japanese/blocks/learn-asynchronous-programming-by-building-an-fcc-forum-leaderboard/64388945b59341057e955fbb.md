...
フォーラムのリーダーボードにデータを表示するには、APIからデータをリクエストする必要があります。これは<dfn>非同期</dfn>操作と呼ばれ、メインプログラムの流れとは独立してタスクが実行されることを意味します。

`async` キーワードを使って非同期関数を作成できます。この関数はPromiseを返します。
