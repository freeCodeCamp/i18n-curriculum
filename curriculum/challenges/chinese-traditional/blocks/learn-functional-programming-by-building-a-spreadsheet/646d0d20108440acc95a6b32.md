...
在你的 `elemValue` 函數中，你明確聲明瞭一個名爲 `inner` 的函數並返回它。 但是，由於你使用箭頭語法，因此可以隱式返回一個函數。 舉個例子：

```js
const curry = soup => veggies => {};
```

`curry` 是一個接受 `soup` 參數的函數，並返回一個接受 `veggies` 參數的函數。 使用此語法，更新你的 `addCharacters` 函數以返回一個採用 `character2` 參數的空函數。

# --hints--

你的 `addCharacters` 函數應該使用隱式返回。
...
你的 `addCharacters` 函數應該返回一個具有 `character2` 參數的箭頭函數。
