...
为 `.note` 选择器提供 `8px` 的左右内边距，移除上下内边距。 再将 `text-indent` 属性设置为 `-8px`。

通过这些最后的更改，你的营养成分表就完成了！

# --hints--

你的 `.note` 选择器应该有一个 `padding` 属性，设置为 `0 8px`。
...
你的 `.note` 选择器应该有一个 `text-indent` 属性，设置为 `-8px`。
