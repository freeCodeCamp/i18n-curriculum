...
Erstellen Sie als Nächstes einen Typ `Position` mit dem Wert `typeof POSITIONS[number]`. Dieser entpackt das `POSITIONS`-Array in eine Vereinigung seiner einzelnen String-Literale, wodurch es effektiv wie `"GK" | "CB" | "LB" | ...` und so weiter aussieht.

# --hints--

Sie sollten einen Typalias namens `Position` erstellen.
...
Ihr `Position`-Typ sollte auf `typeof POSITIONS[number]` gesetzt werden.
