...
지금 앱을 사용하면, 항목을 체크하거나 체크 해제할 때, 또는 검색창에 입력할 때마다 `toggleItem()` 함수가 새로 생성되는 것을 볼 수 있습니다.

성능을 개선하려면 `useCallback()` 훅을 사용해 `toggleItem()` 함수를 메모이제이션하거나 캐시할 수 있습니다. 그러면 React는 의존성이 변경될 때만 함수를 다시 생성합니다.

먼저, 파일 상단에서 React로부터 `useCallback()` 훅을 구조 분해하세요.

# --hints--

파일 상단에서 `useCallback`로부터 `React`를 구조 분해해야 합니다. 이미 사용 중인 다른 훅을 제거하지 않도록 주의하세요.
