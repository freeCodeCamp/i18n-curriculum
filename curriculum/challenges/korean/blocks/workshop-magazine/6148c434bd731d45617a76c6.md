...
더 많은 소셜 아이콘을 추가하되 같은 행에 유지하려면 `grid-template-columns`을 업데이트해 추가 열을 만들어야 합니다. 대안으로 `grid-auto-flow` 속성을 사용할 수 있습니다.

이 속성은 첫 번째 값으로 `row` 또는 `column`를 받고, 두 번째 값으로는 선택적으로 `dense`를 받습니다. `grid-auto-flow`는 자동 배치 알고리즘을 사용해 그리드 레이아웃을 조정합니다. `column`로 설정하면 알고리즘이 필요에 따라 콘텐츠용 새 열을 만듭니다. `dense` 값은 알고리즘이 그리드의 빈 공간을 작은 항목으로 채우도록 하여 항목이 순서와 다르게 나타날 수 있습니다.

`.social-icons` 선택자에 대해 `grid-auto-flow` 속성을 `column`으로 설정하세요.

# --hints--

`.social-icons` 선택자에는 `grid-auto-flow` 속성이 `column`으로 설정되어야 합니다.
