...
`if` 문장 블록에서 `defaultPlayer`과 `JSON.parse(saved)`를 각각 펼쳐서 반환하세요. 두 개의 별도 객체가 하나로 병합되므로 반드시 쉼표로 구분하세요.

# --hints--

`if (saved)` 블록은 `defaultPlayer`와 파싱된 저장된 데이터를 모두 펼친 객체를 반환해야 합니다.
