...
지금 문제는 미리보기가 새로 고침되거나 페이지를 다시 불러오면 플레이어 카드에 추가한 모든 내용이 사라진다는 점입니다.

모든 입력값을 `localStorage`에 저장하면 이를 방지할 수 있습니다. 이를 위해 먼저 값으로 `football_player_card` 문자열을 가진 `STORAGE_KEY` 상수를 생성하세요.

# --hints--

`STORAGE_KEY` 변수를 선언해야 합니다.
...
`STORAGE_KEY`는 값으로 `football_player_card` 문자열을 가져야 합니다.
