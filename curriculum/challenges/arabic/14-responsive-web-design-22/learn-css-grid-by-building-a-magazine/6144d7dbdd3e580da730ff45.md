...
منتقي `body` يجب أن يكون لديه خاصية `font-family` بقيمة `Baskervville`، مع fallback بقيمة `serif`.
...
يجب أن يكون لمنتقي `body` الخاص بك خاصية `color` بقيمة `linen`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.color === 'linen');
```

منتقي `body` يجب أن يكون لديه خاصية `background-color` بقيمة `rgb(20, 30, 40)`.
