...
`if` 문장 본문 안에서 `innerText`의 `result` 속성을 문자열 `"0"`와 같게 설정하세요. 그런 다음, 함수 실행을 조기에 종료하기 위해 조기 `return` 문장을 사용하세요.

# --hints--

`innerText`의 `result` 속성을 문자열 `"0"`와 같게 설정해야 합니다.

```js
assert.match(String(decimalToBinary), /if\s*\(\s*input\s*===?\s*0\s*\)\s*\{\s*result\.innerText\s*=\s*('|"|`)\s*0\s*\1/);
```

`innerText` 요소의 `result`를 설정한 후, 함수 실행을 조기에 종료하기 위해 조기 `return` 문장을 사용하세요.
