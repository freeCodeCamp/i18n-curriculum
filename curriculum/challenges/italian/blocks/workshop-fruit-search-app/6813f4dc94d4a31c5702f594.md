...
Ora che hai i dati dall'API, aggiorna lo stato `results` chiamando `setResults` e passando un array che contiene solo la proprietà `name` di ogni frutto nell'array `data`. Usa il metodo `map()` per questo.

# --hints--

Dovresti aggiornare `setResults` con `data.map(fruit => fruit.name)`.
