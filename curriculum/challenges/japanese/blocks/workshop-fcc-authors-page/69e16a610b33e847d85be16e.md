...
この時点で、アプリの動作は変わっていないはずです。コードは依然としてCDNからファイルを取得し、取得したデータを`JSON`に変換しています。それが完了すると、そのデータを`authorDataArr`変数に保存し、`displayAuthors`を呼び出します。

`fetch`の呼び出しを修正して、`await`演算子を使うようにしてください。結果は`const`という新しい`res`変数に保存します。この新しい変数は`initialFetch`の中でローカルに定義してください。
...
`SyntaxError`が見えるかもしれませんが、今は無視してかまいません。
