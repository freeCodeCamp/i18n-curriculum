...
`DiscountEngine` 인스턴스를 생성하여 변수 `engine`에 저장하세요. 그다음 `calculate_best_price` 메서드를 사용해 최적 가격을 계산하고 `best_price`라는 변수에 저장하세요.

# --hints--

`DiscountEngine` 리스트로 `strategies` 인스턴스를 생성해야 합니다.
...
`engine.calculate_best_price(product, user_tier)`를 호출하고 결과를 `best_price`에 저장해야 합니다.
