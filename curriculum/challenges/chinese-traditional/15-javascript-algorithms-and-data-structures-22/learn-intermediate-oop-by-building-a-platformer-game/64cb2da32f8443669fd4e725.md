...
下一步是將 `width` 和 `height` 添加到 `CheckPoint` 類。

`width` 和 `height` 應分別爲 `proportionalSize(40)` 和 `proportionalSize(70)`。

# --hints--

你應該在 `CheckPoint` 類中有一個 `width` 屬性。
...
你應該在 `CheckPoint` 類中有一個 `height` 屬性。
...
你應該將 `width` 屬性分配給 `proportionalSize(40)`。
...
你應該將 `height` 屬性分配給 `proportionalSize(70)`。
