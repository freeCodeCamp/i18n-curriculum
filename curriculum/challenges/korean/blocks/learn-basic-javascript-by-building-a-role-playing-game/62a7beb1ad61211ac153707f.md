...
이제 `button2`의 텍스트를 `"Buy weapon (30 gold)"`로, `button3`의 텍스트를 `"Go to town square"`로 업데이트하는 줄을 추가하세요.

# --hints--

점 표기법을 사용해 `innerText`의 `button2` 속성에 접근하세요.

```js
assert.match(code, /button2\.innerText/);
```

`let`의 `const` 속성에 접근할 때 `innerText`나 `button2`를 사용하지 마세요.

```js
assert.notMatch(code, /(let|const)\s+button2\.innerText/);
```

`innerText`의 `button2` 속성을 `"Buy weapon (30 gold)"`로 업데이트하세요.
...
점 표기법을 사용해 `innerText`의 `button3` 속성에 접근하세요.

```js
assert.match(code, /button3\.innerText/);
```

`let`의 `const` 속성에 접근할 때 `innerText`나 `button3`를 사용하지 마세요.

```js
assert.notMatch(code, /(let|const)\s+button3\.innerText/);
```

`innerText`의 `button3` 속성을 `"Go to town square"`로 업데이트하세요.
