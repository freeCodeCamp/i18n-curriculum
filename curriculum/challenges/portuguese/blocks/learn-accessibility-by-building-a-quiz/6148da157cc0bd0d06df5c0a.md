...
Para alinhar as caixas de entrada entre si, crie um novo conjunto de regras que selecione todos os elementos `input` e `label` dentro de um elemento `.info` e defina a propriedade `display` como `inline-block`.

Além disso, alinhe o texto do elemento `label` à direita.

# --hints--

Você deve usar um seletor `.info > label, .info > input` ou `.info label, .info input`.
...
Você deve dar aos elementos `input` e `label` um `display` de `inline-block`.
...
Você deve dar ao elemento `label` um `text-align` com o valor `right`.
