...
Il primo argomento per la tua chiamata `evalFormula` deve essere il contenuto della cella (che hai archiviato in `value`). Tuttavia, il contenuto inizia con un carattere `=` per attivare la funzione, quindi devi passare la sottostringa di `value` a partire dall'indice `1`.

# --hints--

Dovresti passare `value` come primo argomento alla tua chiamata `evalFormula()`.
...
Dovresti passare il numero `1` come argomento alla tua chiamata `.slice()`.
