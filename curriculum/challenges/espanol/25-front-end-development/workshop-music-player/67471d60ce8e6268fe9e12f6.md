...
En este taller crearás una aplicación de reproductor de música capaz de reproducir, pausar y saltar canciones de una lista de reproducción.

El HTML y CSS de este proyecto han sido proporcionados para ti, por lo que puedes centrarte en el JavaScript.

Comienza accediendo a los elementos `#playlist-songs`, `#play` y `#pause` y asígnalos a las variables `playlistSongs`, `playButton` y `pauseButton`, respectivamente.


# --hints--

Deberías tener una variable llamada `playlistSongs`.

```js
assert.isDefined(playlistSongs);
```

Deberías asignar el elemento `#playlist-songs` a la variable `playlistSongs`.

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

Deberías tener una variable llamada `playButton`.

```js
assert.isDefined(playButton);
```

Deberías asignar el elemento `#play` a la variable `playButton`.

```js
assert.equal(playButton, document.getElementById('play'));
```

Deberías tener una variable llamada `pauseButton`.

```js
assert.isDefined(pauseButton);
```

Deberías asignar el elemento `#pause` a la variable `pauseButton`.
