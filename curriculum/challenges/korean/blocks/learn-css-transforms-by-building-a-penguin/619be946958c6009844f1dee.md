...
`--fcc-expected--`에 `div` `.penguin-head` 요소를 추가해야 하지만 `--fcc-actual--`를 찾았습니다.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 2);
```

첫 번째 `div`에 `class`인 `face`를 지정해야 하지만 `--fcc-actual--`를 찾았습니다.

```js
assert.include(document.querySelector('.penguin-head > div:nth-of-type(1)')?.className, 'face');
```

두 번째 `div`에 `class`인 `face`를 지정해야 하지만 `--fcc-actual--`를 찾았습니다.
