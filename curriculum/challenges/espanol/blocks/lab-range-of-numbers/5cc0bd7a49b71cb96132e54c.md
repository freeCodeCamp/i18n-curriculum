...
- No uses bucles ni métodos de arreglo (`for`, `while`, `Array.from`, `forEach`, `map`, `filter`, `reduce`)
...
1. Tu función debe usar recursión llamándose a sí misma. No debe usar ninguna sintaxis de bucle (`for`, `while`, `Array.from()`, o funciones de orden superior como `forEach`, `map`, `filter` o `reduce`).
...
Tu código no debe usar ninguna sintaxis de bucle (`for`, `while`, `Array.from()` o funciones de orden superior como `forEach`, `map`, `filter` o `reduce`).
