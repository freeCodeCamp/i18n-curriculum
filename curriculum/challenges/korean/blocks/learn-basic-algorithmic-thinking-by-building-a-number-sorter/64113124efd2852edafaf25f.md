...
`.sort()`에 대한 콜백은 숫자를 반환해야 합니다. 이 숫자는 `a`과 `b` 요소를 정렬하는 방식을 결정합니다:

- 숫자가 음수이면 `a`을 `b`보다 앞에 정렬합니다.
- 숫자가 양수이면 `b`를 `a`보다 앞에 정렬합니다.
- 숫자가 0이면 `a`과 `b`의 순서를 변경하지 않습니다.

숫자를 오름차순(작은 것에서 큰 것 순)으로 정렬하려는 점을 염두에 두고, 위의 논리에 맞게 숫자를 올바르게 정렬할 수 있도록 `a`과 `b`을 사용한 단일 뺄셈 계산을 반환하세요.

# --hints--

콜백 함수는 명시적인 `return`을 사용해야 합니다.

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*\(\s*a\s*,\s*b\s*\)\s*=>\s*{\s*return/);
```

콜백 함수는 `a - b`를 반환해야 합니다.
