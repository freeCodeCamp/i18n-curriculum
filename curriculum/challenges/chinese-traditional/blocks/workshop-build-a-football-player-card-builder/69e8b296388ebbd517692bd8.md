...
為整體 `input` 添加一個 `value` 屬性，屬性值為 `player.overallRating`。接著，添加一個 `onChange` 屬性，該屬性設為一個箭頭函式，該函式以 `e` 作為參數，並呼叫 `setPlayer`，傳入展開的 `player` 和 `overallRating`，其中 `overallRating` 設為 `Number(e.target.value)`。

由於 `overallRating` 是數字，請使用 `Number()` 將輸入的字串值轉換。

# --hints--

你的整體 `input` 應該有一個 `value`，其值等於目前玩家的整體評分。
...
你的整體 `input` 應該有一個 `onChange` 屬性。

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

你的整體 `input` 的 `onChange` 應該呼叫 `setPlayer`，並傳入展開的 `player`，以及 `overallRating` 設為 `Number(e.target.value)`。
