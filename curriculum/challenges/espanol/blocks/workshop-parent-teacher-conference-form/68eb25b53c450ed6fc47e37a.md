...
Por defecto, los elementos `label` son en línea. Para hacer que se apilen verticalmente (excepto tus etiquetas de botones de opción), usa la pseudo-clase `:not()`.

La pseudo-clase `:not()` niega una selección. Aquí, selecciona todos los elementos `label` que no tienen la clase `contact-method`.

Selecciona `label:not(.contact-method)`, luego establece `display` en `block` y `margin` en `10px 0`.

# --hints--

Debes tener un selector `label:not(.contact-method)`.
...
Establece la propiedad `margin` en `10px 0`.
