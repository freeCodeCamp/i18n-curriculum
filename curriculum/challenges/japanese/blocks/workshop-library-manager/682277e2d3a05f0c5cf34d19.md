...
`console.log()` 関数の下に `getBooksByAuthor()` を追加してください。

`console.log()` の中で、`getBooksByAuthor()` 関数を `library` と `"Arvid Kahl"` を引数として呼び出してください。

これで、特定の著者のすべての本がコンソールに表示されるはずです。

# --hints--

`console.log(getBooksByAuthor(library, "Arvid Kahl"))` があるはずです。
