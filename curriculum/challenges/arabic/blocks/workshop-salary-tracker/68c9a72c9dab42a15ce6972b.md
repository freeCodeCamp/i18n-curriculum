...
الآن ستنشئ مُعيّنًا (setter) للخاصية `_level`. أنشئ طريقة باسم `level` بمعلمتين `self` و `new_level`. زيّن الطريقة بـ `@level.setter`.

داخل الطريقة، عيّن `self._level` إلى `new_level`.

# --hints--

يجب أن يحتوي صنف `Employee` على طريقة `level` بمعلمتين `self` و `new_level`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("level")[1].has_args("self, new_level")`)) })
```

يجب أن تُزيّن طريقة `level` بـ `@level.setter`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("level")[1].has_decorators("level.setter")`)) })
```

يجب أن تُعيّن طريقة `level` الخاصية `self._level` إلى `new_level`.
