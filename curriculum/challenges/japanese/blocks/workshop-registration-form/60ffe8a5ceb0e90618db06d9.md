...
送信ボタンをフォームの他の部分とより調和させるために、他のフィールドと同じ`height`を付けてください（`2em`）。また、`font-size`を`1.1rem`に増やしてください。

# --hints--

送信ボタンに`height`として`2em`を付ける必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.height, '2em');
```

送信ボタンに`font-size`として`1.1rem`を付ける必要があります。
