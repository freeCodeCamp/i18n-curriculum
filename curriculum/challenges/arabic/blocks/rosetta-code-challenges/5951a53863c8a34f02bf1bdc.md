...
زوّد دالة لإيجاد أقرب نقطتين بين مجموعة نقاط معطاة في بعدين.

الحل المباشر هو خوارزمية ذات تعقيد $O(n^2)$ (يمكننا تسميتها *خوارزمية القوة الغاشمة*)؛ يمكن أن يكون الشيفرة الزائفة (باستخدام الفهارس) ببساطة:

<pre><strong>bruteForceClosestPair</strong> لـ P(1), P(2), ... P(N)
...
خوارزمية أفضل تعتمد على نهج التقسيم والتغلب التكراري، وتعقيدها $O(n\log n)$، ويمكن أن يكون الشيفرة الزائفة كما يلي:

<pre><strong>closestPair</strong> لـ (xP, yP)
  حيث xP هي P(1) .. P(N) مرتبة حسب إحداثي x، و
  yP هي P(1) .. P(N) مرتبة حسب إحداثي y (ترتيب تصاعدي)
<strong>if</strong> N ≤ 3 <strong>then</strong>
  <strong>return</strong> أقرب نقاط من xP باستخدام خوارزمية القوة الغاشمة
<strong>else</strong>
  xL ← نقاط xP من 1 إلى ⌈N/2⌉
  xR ← نقاط xP من ⌈N/2⌉+1 إلى N
  xm ← xP(⌈N/2⌉)<sub>x</sub>
  yL ← { p ∈ yP : p<sub>x</sub> ≤ xm }
  yR ← { p ∈ yP : p<sub>x</sub> > xm }
  (dL, pairL) ← closestPair لـ (xL, yL)
  (dR, pairR) ← closestPair لـ (xR, yR)
...
بالنسبة للمدخلات، توقع أن تكون المعلمة مصفوفة من كائنات `Point` التي تحتوي على أعضاء `x` و `y` بقيم أرقام. أرجع كائنًا يحتوي على أزواج المفتاح:القيمة لـ `distance` و `pair` (زوج أقرب نقطتين).

على سبيل المثال، `getClosestPair` مع مصفوفة المدخلات `points`:
...
**ملاحظة:** رتب مصفوفة `pair` حسب قيم `x` بترتيب تصاعدي.
...
يجب أن تكون `getClosestPair` دالة.

```js
assert(typeof getClosestPair === 'function');
```

يجب أن تكون قيمة `getClosestPair(points1).distance` هي `0.0894096443343775`.

```js
assert.equal(getClosestPair1_().distance, answer1.distance);
```

يجب أن تكون قيمة `getClosestPair(points1).pair` هي `[ { x: 7.46489, y: 4.6268 }, { x: 7.46911, y: 4.71611 } ]`.
...
يجب أن تكون قيمة `getClosestPair(points2).distance` هي `65.06919393998976`.

```js
assert.equal(getClosestPair2_().distance, answer2.distance);
```

يجب أن تكون قيمة `getClosestPair(points2).pair` هي `[ { x: 37134, y: 1963 }, { x: 37181, y: 2008 } ]`.
...
يجب أن تكون قيمة `getClosestPair(points3).distance` هي `6754.625082119658`.

```js
assert.equal(getClosestPair3_().distance, answer3.distance);
```

يجب أن تكون قيمة `getClosestPair(points3).pair` هي `[ { x: 46817, y: 64975 }, { x: 48953, y: 58567 } ]`.
