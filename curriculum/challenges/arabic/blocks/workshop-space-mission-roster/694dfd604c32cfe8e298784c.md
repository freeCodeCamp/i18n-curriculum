...
طلب مركز التحكم منك تبديل مواضع رائدَي فضاء في مصفوفة `squad`، تحديدًا العضوين عند الفهارس `2` و `5`. استخدم دالة `swapCrewMembers()` لتنفيذ التبديل واحفظ النتيجة في متغير جديد اسمه `updatedSquad`.

# --hints--

يجب استخدام دالة `swapCrewMembers()` لتبديل أعضاء `squad` عند الفهارس `2` و `5` وحفظ المصفوفة الناتجة في متغير جديد اسمه `updatedSquad`.
