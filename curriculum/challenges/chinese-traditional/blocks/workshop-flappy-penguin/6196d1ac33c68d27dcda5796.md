...
將 `.back-mountain` 元素順時針旋轉 `45deg`。 然後，將其 `left` 屬性設置爲 `110px`，`top` 屬性設置爲 `225px`。
...
你應該將 `.back-mountain` 的 `transform` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
...
你應該將 `.back-mountain` 的 `left` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
...
你應該將 `.back-mountain` 的 `top` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
