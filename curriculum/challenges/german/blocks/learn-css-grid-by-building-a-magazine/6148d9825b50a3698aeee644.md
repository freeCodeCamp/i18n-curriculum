...
Erstelle einen `.lists`-Selektor und setze die `list-style-type`-Eigenschaft auf `none`. Dies wird die Aufzählungspunkte vor den Listenelementen entfernen.

# --hints--

Du solltest einen `.lists`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists'));
```

Dein `.lists`-Selektor sollte eine `list-style-type`-Eigenschaft mit dem Wert `none` haben.
