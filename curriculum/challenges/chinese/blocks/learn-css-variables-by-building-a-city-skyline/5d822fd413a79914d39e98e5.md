...
你需要的是 `.foreground-buildings` 容器直接位于 `.background-buildings` 元素之上。 为其指定 `width` 和 `height` 为 `100%`，将 `position` 设置为 `absolute`，并将 `top` 设置为 `0`。 这将使其与 body 大小相同，并将其起点移动到左上角。
