...
在第二個 `fieldset` 元素內為 `legend` 元素添加文字 `What's your cat's personality?`。

# --hints--

你要麼刪除了第二個 `fieldset` 元素，要麼它缺少開啟標籤或關閉標籤。
...
你的 `legend` 元素應該有一個開啟標籤。開啟標籤具有此語法：`<elementName>`。
...
`legend` 元素應該有文字 `What's your cat's personality?`。你要麼遺漏了文字，要麼有拼寫錯誤。
