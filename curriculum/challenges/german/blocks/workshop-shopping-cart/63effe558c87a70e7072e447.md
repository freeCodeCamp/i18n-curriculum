...
Das Verhalten der `addItem`-Methode muss sich ändern, je nachdem, ob das Produkt bereits im Warenkorb ist oder nicht. Erstellen Sie einen ternären Operator, der überprüft, ob das aktuelle Produkt bereits im Warenkorb ist. Verwenden Sie `undefined` sowohl für den wahren als auch für den falschen Ausdruck, um einen Syntaxfehler zu vermeiden.

# --hints--

Sie sollten prüfen, ob `currentProductCount` größer als `1` ist.
...
Sie sollten einen ternären Operator mit Ihrer `currentProductCount > 1`-Bedingung verwenden.
...
Sie sollten `undefined` als wahren Ausdruck verwenden.
...
Sie sollten `undefined` als falschen Ausdruck verwenden.
