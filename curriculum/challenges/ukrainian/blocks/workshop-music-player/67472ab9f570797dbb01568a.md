...
Почніть із оголошення нової змінної з назвою `userData` і призначте їй порожній об’єкт.

# --hints--

У вас має бути змінна з назвою `userData`.

```js
assert.isDefined(userData);
```

Ви маєте призначити порожній об’єкт змінній `userData`.
