...
Дотримуючись найкращих практик для вхідних даних форми, надайте кожному `input` відповідний `type` та атрибут `name`. Потім додайте першому `input` атрибут `placeholder`.

# --hints--

Ви повинні надати першому `input` атрибут `type` зі значенням `text`.

```js
assert.equal(document.querySelectorAll('input')?.[0]?.type, 'text');
```

Ви повинні надати другому `input` атрибут `type` зі значенням `email`.

```js
assert.equal(document.querySelectorAll('input')?.[1]?.type, 'email');
```

Ви повинні надати третьому `input` атрибут `type` зі значенням `date`.
