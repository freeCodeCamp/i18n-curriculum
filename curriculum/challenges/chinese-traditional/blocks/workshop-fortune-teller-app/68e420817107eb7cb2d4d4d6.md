...
為圖片元素添加一個 `onerror` 處理常式來處理載入失敗。在第一次失敗時，使用 `dataset.failed` 標記它，並將來源替換為區域的預設圖片。如果圖片再次失敗，移除 hidden 類別並使用 `previousElementSibling` 隱藏載入佔位符。

# --hints--

你的 `img` 元素應該有一個 `onerror` 處理常式。
...
你的 `onerror` 處理常式應該包含一個 `if` 述語來檢查 `dataset.failed` 是否為假值。
...
`dataset.failed` 應該在你的 `'1'` 區塊內被設定為 `if`。這裡別忘了使用 `this` 關鍵字。
...
你的 `if` 區塊應該將 `this.src` 更新為區域的預設圖片。這裡別忘了使用 `this` 關鍵字。
...
你應該有一個 `else` 區塊，移除 `hidden` 類別。這裡別忘了使用 `this` 關鍵字。
...
你的 `else` 區塊應該將 `previousElementSibling.style.display` 設為 `'none'`。這裡別忘了使用 `this` 關鍵字。
