...
1. `playing-cards`というIDを持つ`main`要素を用意してください。
1. `#playing-cards`要素の中に、クラスが`card`の`div`要素を少なくとも3つ用意してください。
1. 各`.card`要素の中に、クラスがそれぞれ`left`、`middle`、`right`の3つの`div`要素を用意してください。
1. `#playing-cards`要素はflexboxを使って子要素を水平方向に中央揃えにし、折り返しを許可し、要素間に`20px`の間隔を設けてください。
1. 各`.card`要素はflexboxを使って子要素を`space-between`で配置してください。
1. 各`.left`要素はflexboxのアイテムプロパティを使って親コンテナの開始位置に揃えてください。
1. 各`.middle`要素はflexboxのアイテムプロパティを使って親コンテナの中央に揃えてください。
1. 各`.right`要素はflexboxのアイテムプロパティを使って親コンテナの終了位置に揃えてください。
1. 各`.middle`要素はflexboxを使って子要素を縦方向（カラム）に表示してください。
