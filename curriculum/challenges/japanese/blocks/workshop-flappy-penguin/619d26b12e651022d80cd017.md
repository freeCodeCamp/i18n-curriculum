...
`.arm` 要素をターゲットにして、`width` の `30%`、`height` の `60%`、そして時計回りで `background` から始まり `90deg` で終わる線形グラデーションの `gray` を `rgb(209, 210, 199)` に設定してください。
...
`.arm` に `width` の `--fcc-expected--` を設定するべきです。場所は `--fcc-actual--` にあります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

`.arm` に `height` の `--fcc-expected--` を設定するべきです。場所は `--fcc-actual--` にあります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

`.arm` に `background` の `linear-gradient(90deg, gray, rgb(209, 210, 199))` を設定するべきです。
