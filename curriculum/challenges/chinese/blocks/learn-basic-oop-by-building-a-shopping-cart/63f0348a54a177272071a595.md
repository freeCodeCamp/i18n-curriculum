...
现在将三元表达式的结果分配给 `showHideCartSpan` 变量的 `textContent`，该表达式检查 `isCartShowing` 是否为真。 如果是，则将 `textContent` 设置为 `"Hide"`，否则将其设置为 `"Show"`。

# --hints--

你应该对 `showHideCartSpan` 变量的 `textContent` 属性使用赋值运算符。
...
如果 `isCartShowing` 为真，则应将 `showHideCartSpan` 变量的 `textContent` 设置为 `"Hide"`。

```js
assert.match(code, /showHideCartSpan\s*\.\s*textContent\s*=\s*isCartShowing\s*\?\s*('|"|`)Hide\1\s*:\s*/)
```

如果 `isCartShowing` 为假，则应将 `showHideCartSpan` 变量的 `textContent` 设置为 `"Show"`。
