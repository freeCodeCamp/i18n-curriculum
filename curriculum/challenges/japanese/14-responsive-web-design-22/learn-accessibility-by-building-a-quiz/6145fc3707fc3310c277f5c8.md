...
ページのスタイリングに戻りましょう。 ナビゲーションバー内のリスト要素を指定し、以下のスタイルを加えてください:
...
`nav li` または `nav > ul > li` のいずれかのセレクターを使用する必要があります。
...
`li` 要素の `color` の値を `#dfdfe2` に設定する必要があります。
...
`li` 要素の `margin` の値を `0 0.2rem` に設定する必要があります。
...
`li` 要素の `padding` の値を `0.2rem` に設定する必要があります。
...
`li` 要素の `display` の値を `block` に設定する必要があります。
