...
Kabla hujaanza kufanya kazi ya kucheza wimbo unaofuata na uliopita, unahitaji kupata kielezo cha kila wimbo katika sifa ya `songs` ya `userData`.

Anza kwa kuunda kitendakazi cha mshale kinachoitwa `getCurrentSongIndex`.

# --hints--

Unapaswa kuwa na kitendakazi cha `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Kitendakazi chako cha `getCurrentSongIndex` kinapaswa kutumia sintaksia ya mshale.
