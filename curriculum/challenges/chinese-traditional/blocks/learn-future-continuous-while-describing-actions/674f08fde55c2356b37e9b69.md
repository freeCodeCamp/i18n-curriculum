...
`Going to` is a phrase used to describe actions that will happen in the future or intentions someone has. 例如：

`She is going to start her new job next week.` - It means that she plans to begin her new job in the future.

`Online` means being connected to the internet or using internet-based tools. 例如：
