...
添加另一個布爾表達式，檢查玩家的 `position.y` 是否大於或等於檢查點的 `position.y`。

在該語句下方，添加另一個布爾表達式，檢查玩家的 `position.y` 加上玩家的 `height` 是否小於或等於檢查點的 `position.y` 加上檢查點的 `height`。

在該語句下方，添加 `isCheckpointCollisionDetectionActive` 變量。

# --hints--

你應該有一個布爾表達式來檢查玩家的 `position.y` 是否大於或等於檢查點的 `position.y`
...
你應該有一個布爾表達式，檢查玩家的 `position.y` 加上玩家的身高是否小於或等於檢查點的 `position.y` 加上檢查點的高度。
...
你應該將 `isCheckpointCollisionDetectionActive` 添加爲 `checkpointDetectionRules` 數組的最後一項。
