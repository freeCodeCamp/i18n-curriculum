...
在 `card-rating` div 下方，添加一个 `div`，其 `className` 为 `card-position`，并带有文本 `ST`，表示击球手。

# --hints--

你应该在 `div` div 下方添加一个 `className`，其 `card-position` 为 `.card-rating`。

```js
assert.exists(document.querySelector('.card-rating + .card-position'));
```

`.card-position` 元素应包含文本 `ST`。
