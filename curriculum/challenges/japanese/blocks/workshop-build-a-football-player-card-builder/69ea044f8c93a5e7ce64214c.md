...
`if` 文のブロック内で、`defaultPlayer` と `JSON.parse(saved)` の両方を別々にスプレッドしたオブジェクトを返してください。これらは2つの別々のオブジェクトを1つにマージしているので、必ずカンマで区切ってください。

# --hints--

`if (saved)` ブロックは、`defaultPlayer` と解析された保存データの両方をスプレッドしたオブジェクトを返す必要があります。
