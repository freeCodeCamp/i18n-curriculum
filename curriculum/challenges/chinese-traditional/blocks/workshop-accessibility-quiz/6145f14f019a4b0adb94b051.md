...
第一個 `option` 元素的 `value` 爲 `""`，文本爲 `Select an option`。 第二個 `option` 的 `value` 爲 `yes`，文本爲 `Yes`。 第三個 `option` 的 `value` 爲 `no`，文本爲 `No`。
...
你應將第二個 `option` 元素的 `value` 設置爲 `yes`。
...
你應將第三個 `option` 元素的 `value` 設置爲 `no`。
