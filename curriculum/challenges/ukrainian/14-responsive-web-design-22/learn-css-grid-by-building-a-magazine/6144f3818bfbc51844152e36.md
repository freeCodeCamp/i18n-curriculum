...
Використовуйте функцію `minmax`, щоб стовпчики були адаптивними на будь-якому пристрої. Функція `minmax` приймає два аргументи: перший — мінімальне значення, а другий — максимальне. Цими значеннями можуть бути одиниці довжини, відсотки, `fr` або навіть ключове слово, як-от `max-content`.

Обгорніть кожне з вже визначених значень властивості `grid-template-columns` в функцію `minmax`, використовуючи кожне значення як другий аргумент. Перший аргумент повинен бути `2rem`, `min-content` та `2rem` відповідно.

# --hints--

Ваш селектор `main` повинен мати властивість `grid-template-columns` зі значенням `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
