...
Adicione um atributo `value` com valor `player.pac` ao PAC `input`. Depois adicione um atributo `onChange` definido como uma arrow function que recebe `e` como parâmetro e chama `setPlayer` com um spread de `player` e `pac` definido como `Number(e.target.value)`.

# --hints--

Seu PAC `input` deve ter um `value` igual ao status `pac` do jogador.
...
Seu PAC `input` deve ter um manipulador `onChange` que chama `setPlayer` com um spread de `player` e `pac` definido como `Number(e.target.value)`.
