...
텍스트의 `font-family`를 변경해서 브라우저 기본 폰트와 다르게 보이게 할 수 있습니다. 각 브라우저에는 공통으로 사용할 수 있는 폰트가 있습니다.

`body`의 모든 텍스트를 `font-family` 속성을 `sans-serif` 값으로 추가해서 변경하세요. 이 폰트는 꽤 흔하고 읽기 쉽습니다.

# --hints--

`font-family` 속성을 `sans-serif`로 설정하세요.

```js
const hasFontFamily = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style['font-family'] === 'sans-serif');
```

`body`에는 `font-family`가 `sans-serif`이어야 합니다.
