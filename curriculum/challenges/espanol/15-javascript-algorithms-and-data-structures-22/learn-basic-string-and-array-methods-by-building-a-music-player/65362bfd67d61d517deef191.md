...
El método <dfn>find()</dfn> recupera el primer elemento dentro de un arreglo que cumple las condiciones especificadas en la función callback proporcionada. Si ningún elemento satisface la condición, el método devuelve `undefined`.

En el ejemplo a continuación, el método `find()` se utiliza para encontrar el primer número mayor que 25:
...
Usa `const` para crear una variable llamada `song` y asígnale el resultado de la llamada al método `find()` en el arreglo `userData?.songs`. Usa `song` como el parámetro de la función callback de `find()` y verifica si `song.id` es estrictamente igual a `id`.

Esto iterará a través del arreglo `userData?.songs`, buscando una canción que corresponda al `id` pasado a la función `playSong`.

# --hints--

Debes usar el método `find` en `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

Tu método `find` debe tener `song` como el parámetro de su función de flecha callback.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

Tu método `find` debe usar igualdad estricta para verificar si `song.id` es igual a `id`.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

Tu método `find` debe estar asignado a una constante `song`.
