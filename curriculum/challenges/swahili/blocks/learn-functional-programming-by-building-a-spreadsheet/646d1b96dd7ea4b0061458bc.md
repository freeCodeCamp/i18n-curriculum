...
Unaweza kupitisha rejea ya kitendakazi kama kigezo cha wito wa kurudisha. Rejea ya kitendakazi ni jina la kitendakazi bila mabano ya kawaida. Kwa mfano:
...
Njia ya `.map()` hapa itaita kitendakazi cha `myFunc`, ikipitisha hoja zile zile ambazo wito wa kurudisha wa `.map()` unazipokea. Hoja ya kwanza ni thamani ya safu ya data katika mzunguko wa sasa, kwa hivyo `newArray` itakuwa `[value: 1, value: 2, value: 3]`.

Pitia rejea ya kitendakazi chako cha `elemValue` kama wito wa kurudisha kwa njia yako ya `.map()`.

# --hints--

Haupaswi kuita kitendakazi chako cha `elemValue`.
...
Unapaswa kupitisha rejea ya `elemValue` kama wito wa kurudisha kwa njia yako ya `.map()`.
