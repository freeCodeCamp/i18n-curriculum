...
In the previous lecture, you learned about the <dfn>Web Audio API</dfn> and how to use it to play songs. Усі сучасні браузери підтримують Веб Аудіо API, який дозволяє створювати та обробляти аудіо у вебзастосунках.

Create a variable named `audio` and set it equal to `new Audio()`. Це створить новий елемент HTML5 `audio`.
