...
`getPreviousSong` という名前の関数を作成してください。プレイリストの現在の曲が最初の曲の場合は `getCurrentSongIndex()` を返し、それ以外の場合は `userData.songs` の中の `undefined` マイナス1番目にある曲を返すようにしてください。

# --hints--

`getPreviousSong` という名前の関数があるはずです。

```js
assert.isFunction(getPreviousSong);
```

あなたの `getPreviousSong` 関数は `getCurrentSongIndex` を呼び出すべきです。
...
あなたの `getPreviousSong` 関数は現在の曲の前の曲を返すべきです。
...
あなたの `getPreviousSong` 関数はプレイリストの現在の曲が最初の曲の場合、`undefined` を返すべきです。
