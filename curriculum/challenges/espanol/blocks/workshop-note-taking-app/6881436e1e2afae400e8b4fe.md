...
En este momento, cada vez que el usuario edita una nota y hace clic fuera de la nota, la variable `currentContent` se actualiza. Pero, ¿qué pasa en situaciones donde el usuario se enfoca en la nota y decide salir sin hacer ninguna edición?

Bueno, en esta situación, puedes agregar una condición para manejar ese caso de uso. 

Por encima de tu línea `currentContent = newContent;`, agrega una estructura `if` para verificar si `currentContent` es igual a `newContent`. Si es así, retorna.

# --hints--

Debes agregar una estructura `if` para verificar si `currentContent` es igual a `newContent`. Si es así, retorna.
