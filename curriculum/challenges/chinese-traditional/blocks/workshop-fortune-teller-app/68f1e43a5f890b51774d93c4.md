...
在 `showFortune` 方法內宣告一個名為 `target` 的變數，並將它指定為 `e.target`，然後建立另一個空的 if 述語。

# --hints--

你應該在 `target` 內宣告一個名為 `showFortune` 的變數。
