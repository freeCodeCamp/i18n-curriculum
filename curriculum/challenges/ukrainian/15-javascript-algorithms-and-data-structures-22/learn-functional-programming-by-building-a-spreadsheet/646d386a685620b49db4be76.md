...
Викличте метод `.map()` на виклику `rangeFromString()`, передавши посилання на `addCharacters` як функцію зворотного виклику.

# --hints--

Викличте метод `.map()` на виклику `rangeFromString()`.
...
Передайте посилання на `addCharacters` як зворотний виклик до методу `.map()`.
