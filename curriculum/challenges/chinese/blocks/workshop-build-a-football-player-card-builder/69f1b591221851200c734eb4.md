...
移动到第二个 `div` 元素，添加一个 `label`，其 `className` 为 `label`，一个 `htmlFor`，其 `overallRating` 为 `Overall` 的文本，以及一个 `input`，其 `id` 设置为 `overallRating`，一个 `className`，其 `input` 为 ，类型为 `number`。

# --hints--

你行中的第二个 `form-group` 应该有一个 `label`，其 `className` 为 `label`，并且有一个 `htmlFor`，其 `overallRating` 为 。
...
你行中的第二个 `form-group` 应该有一个 `input`，其 `id` 为 `overallRating`，并且有一个 `type`，其 `number` 为 。
