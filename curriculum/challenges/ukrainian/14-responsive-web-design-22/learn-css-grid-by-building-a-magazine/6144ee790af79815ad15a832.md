...
Тепер ви готові почати складати сітку. CSS-сітка пропонує двовимірний макет на основі сітки, що дозволяє розташовувати елементи за центром горизонтально та вертикально, зберігаючи при цьому контроль, щоб виконувати такі дії, як накладання елементів.

Спочатку створіть селектор `main` та надайте йому властивість `display` зі значенням `grid`.

# --hints--

Ви повинні мати селектор `main`.

```js
assert(new __helpers.CSSHelp(document).getStyle('main'));
```

Ваш селектор `main` повинен мати властивість `display` зі значенням `grid`.
