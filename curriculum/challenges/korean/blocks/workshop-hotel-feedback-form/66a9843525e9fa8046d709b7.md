...
호텔 피드백 폼 워크숍의 마지막 단계에서는 폼에 제출 버튼을 추가해야 합니다.

이전에 수업에서 제출 버튼을 다루는 방법을 배운 것을 기억하세요.

`button` 요소를 추가하고 `type` 속성을 `"submit"`로 설정하며 텍스트 내용은 `Submit`로 설정하세요.

이렇게 하면 호텔 피드백 폼이 완성됩니다!

# --hints--

폼 안에 `button`가 있어야 합니다.

```js
assert.exists(document.querySelector('form button'));
```

버튼은 `type`가 `"submit"`로 설정되어야 합니다.

```js
assert.exists(document.querySelector('button[type="submit"]'));
```

버튼의 텍스트 내용은 `Submit`이어야 합니다.
