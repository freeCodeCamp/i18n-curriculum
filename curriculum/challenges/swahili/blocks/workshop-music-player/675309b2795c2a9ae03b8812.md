...
Tafadhali angalia kuwa picha ya albamu katika HTML na nyimbo katika safu ya data ya `userData.songs` zimebadilika. Tumebadilisha nyimbo za awali na kuweka nyimbo fupi ambazo unaweza kutumia kujaribu programu yako.

Kazi zote kuu sasa zipo mahali pake. Tatizo pekee sasa ni kwamba nyimbo inayocheza haianzi kucheza moja kwa moja nyimbo inayofuata baada ya ile inayocheza sasa kumalizika.

Ili kurekebisha hilo, unaweza kuweka msikilizaji wa tukio ambao utagundua wakati nyimbo inayocheza sasa itakapomalizika. Msikilizaji wa tukio wa `"ended"` ni sahihi kwa hili. Unawashwa wakati uchezaji wa media unafikia mwisho.

Ongeza msikilizaji wa tukio kwa kipengele cha `audio` ambacho husikiliza tukio la `"ended"` na upitie rejea ya kitendakazi cha `playNextSong`. Kwa hivyo mradi wa mchezaji wa muziki utakuwa umekamilika!

# --hints--

Unapaswa kuunganisha njia ya `addEventListener()` kwa kigezo chako cha `audio`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

Msikilizaji wa tukio uliotumia kwa kigezo chako cha `audio` unapaswa kusikiliza tukio la `"ended"`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

Unapaswa kupitisha rejea ya `playNextSong` kwa msikilizaji wa tukio wa `"ended"`.
