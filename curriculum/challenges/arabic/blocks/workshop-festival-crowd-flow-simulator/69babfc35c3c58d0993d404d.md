...
لإكمال دالة `processGateFlow`، أرجع كائنًا يحتوي على خاصيتين:

- `processed`: عدد الحضور الذين تمكن البوابة من معالجتهم خلال هذه الدورة.
- `overflow`: عدد الحضور المتبقين في الطابور بعد المعالجة (وهو `currentTickQueue` بعد الحلقة).

يجب أن تُرجع الكائن مباشرةً على هذا النحو:
...
يجب أن تُرجع كائنًا بخاصيتي `processed` و `overflow`. أرجع الكائن مباشرةً دون تعيينه لمتغير.
...
يجب أن تحتوي خاصية `processed` في الكائن المرجع على القيمة `processed`.
...
يجب أن تحتوي خاصية `overflow` في الكائن المرجع على القيمة `currentTickQueue`.
