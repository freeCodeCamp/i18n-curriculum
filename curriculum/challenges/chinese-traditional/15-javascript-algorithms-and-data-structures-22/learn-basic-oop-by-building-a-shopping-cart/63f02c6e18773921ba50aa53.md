...
鑑於計算機存儲和處理數字的方式，涉及小數的計算可能會出現一些奇怪的行爲。 例如，`0.1 + 0.2` 不等於 `0.3`。 這是因爲計算機將十進制數存儲爲二進制分數，而某些二進制分數不能精確地表示爲十進制分數。

我們想清理你計算的數字結果。 將你的計算括在括號中（不要包含 `return` 語句！）並在其上調用 `.toFixed()` 方法。 將數字 `2` 作爲參數傳遞給 `.toFixed()` 方法。 這會將數字四捨五入到小數點後兩位並返回一個字符串。
