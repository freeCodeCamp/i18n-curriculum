...
Далі додайте інструкцію `else if`, щоб перевірити, чи була натиснута ліва клавіша, і чи `isCheckpointCollisionDetectionActive` є істинним.

Всередині цієї умови додайте цикл `forEach`, щоб ітерувати по масиву `platforms`.

Усередині циклу використайте оператор додавання з присвоєнням, щоб додати 5 до позиції `x` платформи.

# --hints--

У вас має бути умова, яка перевіряє, чи була натиснута ліва клавіша, і чи `isCheckpointCollisionDetectionActive` є істинним.
...
Ви маєте додати цикл `forEach`, який ітерує по масиву `platforms`.

```js
assert.match(code, /else\s+if\s*\(.*\)\s*{\s*platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

Використайте оператор додавання з присвоєнням, щоб додати 5 до позиції `x` платформи.
