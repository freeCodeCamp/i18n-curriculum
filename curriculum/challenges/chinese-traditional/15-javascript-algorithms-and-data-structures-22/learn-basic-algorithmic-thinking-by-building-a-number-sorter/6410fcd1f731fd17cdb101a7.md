...
在你的嵌套 `for` 循環中，添加 `console.log()` 調用來檢查每次迭代中的 `array`，`array[j]` 和 `array[minIndex]` 的值。 你可以點擊 `Sort` 按鈕查看你的算法如何遍歷數組。

然後寫一個 `if` 語句來檢查 `array[j]` 的值是否小於 `array[minIndex]` 的值。 如果是，請將 `minIndex` 設置爲 `j`。

# --hints--

你的嵌套 `for` 循環中應該有一個 `console.log()` 調用。
...
你應該將 `array` 作爲第一個參數傳遞給 `console.log()`。
...
你應該將 `array[j]` 作爲第二個參數傳遞給 `console.log()`。
...
你應該將 `array[minIndex]` 作爲第三個參數傳遞給 `console.log()`。
...
在你的 `console.log()` 調用之後應該有一個 `if` 語句。
...
你應該在你的 `if` 語句中添加一個條件，以檢查 `array[j]` 是否小於 `array[minIndex]`。
