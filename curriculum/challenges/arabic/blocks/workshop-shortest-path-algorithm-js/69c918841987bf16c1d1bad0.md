...
يجب أن تُعيد الدالة البيانات المحسوبة حتى يتمكن المستدعون من استخدامها برمجيًا.

بعد حلقة `for...of`، أضف عبارة `return` تُعيد مصفوفة تحتوي على `distances` و `paths`، بهذا الترتيب.

# --hints--

يجب أن تُعيد `[distances, paths]` بعد حلقة `for...of`.
