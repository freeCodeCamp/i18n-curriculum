...
Dentro del primer elemento `td`, agrega una expresión incrustada `${}`. Dentro de esa expresión, llama a la función `forumCategory` con el argumento de `category_id`.

Ahora deberías ver una categoría mostrada debajo de cada tema de publicación.

# --hints--

Debes llamar a la función `forumCategory` con el argumento de `category_id`.
