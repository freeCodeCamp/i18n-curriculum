...
방금 추가한 두 번째 `text.innerText` 문자열 끝에 연결 연산자를 사용해 `inventory`의 내용을 문자열에 추가하세요.

# --hints--

`" In your inventory you have: "` 문자열은 변경하지 마세요.

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*\+=\s*('|")\sIn your inventory you have:\s\1/)
```

문자열에 `+`의 내용을 추가하려면 연결 연산자 `inventory`를 사용하세요.
