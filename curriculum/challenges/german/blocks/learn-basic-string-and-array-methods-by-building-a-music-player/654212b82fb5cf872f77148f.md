...
Erstellen Sie innerhalb des `button`-Elements zwei weitere `span`-Elemente.

Das erste `span`-Element sollte eine `class` von `playlist-song-artist` haben. Fügen Sie zwischen den `span`-Tags `${song.artist}` hinzu.

Das zweite `span`-Element sollte eine `class` von `playlist-song-duration` haben. Fügen Sie zwischen den `span`-Tags `${song.duration}` hinzu.

# --hints--

Sie sollten das bestehende `button`-Element und dessen Inhalt nicht verändern.
...
Sie sollten ein `span`-Element mit der Klasse `playlist-song-artist` erstellen.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

Ihr `span`-Element mit der Klasse `playlist-song-artist` sollte `song.artist` als Inhalt interpolieren.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

Sie sollten ein `span`-Element mit der Klasse `playlist-song-duration` erstellen.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

Ihr `span`-Element mit der Klasse `playlist-song-duration` sollte `song.duration` als Inhalt interpolieren.
