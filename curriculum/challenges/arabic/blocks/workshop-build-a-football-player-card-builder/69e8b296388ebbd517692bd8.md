...
أضف خاصية `value` بقيمة `player.overallRating` إلى الـ `input` الخاص بالتقييم العام. بعد ذلك، أضف خاصية `onChange` تُعيّن إلى دالة سهمية تأخذ `e` كمعلمة وتستدعي `setPlayer` مع نسخ من `player` وتعيين `overallRating` إلى `Number(e.target.value)`.

بما أن `overallRating` رقم، استخدم `Number()` لتحويل القيمة النصية القادمة من المُدخَل.

# --hints--

يجب أن يحتوي الـ `input` الخاص بالتقييم العام على خاصية `value` تساوي التقييم العام الحالي للاعب.
...
يجب أن يحتوي الـ `input` الخاص بالتقييم العام على خاصية `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

يجب أن تستدعي خاصية `onChange` في الـ `input` الخاص بالتقييم العام الدالة `setPlayer` مع نسخ من `player` وتعيين `overallRating` إلى `Number(e.target.value)`.
