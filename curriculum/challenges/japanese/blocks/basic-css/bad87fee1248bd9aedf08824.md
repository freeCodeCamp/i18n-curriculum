...
時々、要素の各辺に異なる`margin`を設定したい場合があります。

CSSでは、`margin`、`margin-top`、`margin-right`、`margin-bottom`プロパティを使って、要素の4つの個別の辺すべての`margin-left`を制御できます。

# --instructions--

青いボックスの上と左の辺に`margin`の`40px`を、下と右の辺には`20px`だけを設定してください。

# --hints--

`blue-box`クラスは、要素の上辺に`40px`の`margin`を設定する必要があります。
...
`blue-box`クラスは、要素の右辺に`20px`の`margin`を設定する必要があります。
...
`blue-box`クラスは、要素の下辺に`20px`の`margin`を設定する必要があります。
...
`blue-box`クラスは、要素の左辺に`40px`の`margin`を設定する必要があります。
