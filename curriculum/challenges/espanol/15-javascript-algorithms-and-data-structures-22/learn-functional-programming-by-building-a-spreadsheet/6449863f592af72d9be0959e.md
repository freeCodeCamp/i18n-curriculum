...
Después de que tu `rangeRegex` encuentre el `:`, necesita buscar el mismo patrón de letra y número que antes.

Copia tus dos grupos de captura existentes y pégalos después del colon.

# --hints--

Debes agregar un tercer grupo de captura a tu `rangeRegex`, después del colon.
...
Tu tercer grupo de captura debe usar una clase de caracteres.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[.*\]\)/);
```

Tu tercer grupo de captura debe coincidir con los caracteres de la `A` a la `J`.
...
Debes agregar un cuarto grupo de captura a tu `rangeRegex`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(.*\)/);
```

Tu cuarto grupo de captura debe coincidir con uno o dos dígitos.
