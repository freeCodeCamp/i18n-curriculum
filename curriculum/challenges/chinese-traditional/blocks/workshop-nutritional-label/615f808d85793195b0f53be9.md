...
在最後一個 `.divider` 之後創建另一個 `p` 元素，併爲其指定文本 `Total Sugars 12g`。 爲該 `p` 元素分配縮進和移除底部邊框所需的 `class` 值。 然後在新的 `p` 元素下方創建另一個 `.divider`。

# --hints--

你應該在 `.daily-value` 元素末尾創建一個新的 `p` 和 `div` 元素。
...
你的新的 `div` 應該將 `class` 屬性設置爲 `divider`。
