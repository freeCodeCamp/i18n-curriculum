...
Sasa baada ya kuwa na orodha ya nyimbo zinazoonyeshwa kwenye skrini, itakuwa vizuri kuzipanga kwa mpangilio wa alfabeti kwa kichwa cha wimbo.

Anza kwa kuunda kitendakazi cha mshale kinachoitwa `sortSongs`. 

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `sortSongs`.

```js
assert.isFunction(sortSongs);
```

Kitendakazi chako cha `sortSongs` kinapaswa kutumia sintaksia ya kitendakazi cha mshale.
