...
Tu clase `ShoppingCart` necesita la capacidad de agregar elementos. Crea un método `addItem` vacío, que reciba dos parámetros: `id` y `products`. Crear un método podría verse así:
...
El primer parámetro, `id`, es el `id` del producto que el usuario ha añadido a su carrito. El segundo parámetro, `products`, es un arreglo de instancias `Dessert`. Al usar un parámetro en lugar de referenciar directamente tu arreglo `products` existente, este método será más flexible si quisieras agregar listas adicionales de productos en el futuro.

# --hints--

Tu clase `ShoppingCart` debe tener un método `addItem`.
...
Tu método `addItem` debe recibir dos parámetros: `id` y `products`.
...
Tu método `addItem` debe estar vacío.
