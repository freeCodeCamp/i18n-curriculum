...
열리는 `a` 태그에 `target` 속성을 `"_blank"`로 설정하세요. 그런 다음 `href` 속성을 `${forumTopicUrl}${slug}/${id}`로 설정하세요.

이 변경으로 freeCodeCamp 포럼 리더보드 프로젝트가 이제 완료되었습니다!

# --hints--

열리는 앵커 태그에 `target` 속성을 `"_blank"`로 설정해야 합니다.
...
열리는 앵커 태그에 `href` 속성을 `${forumTopicUrl}${slug}/${id}`로 설정해야 합니다.
