...
Ogni voce avrà un input di testo per il nome dell'elemento e un input numerico per le calorie. Per ottenere il conteggio del numero di voci, è possibile valutare gli input di testo.

Passa la stringa `input[type="text"]` al metodo `querySelectorAll()`. Remember that if you use single quotes for your string, you must also use double quotes within it (or vice-versa).
