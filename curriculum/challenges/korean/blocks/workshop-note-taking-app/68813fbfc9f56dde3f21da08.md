...
사용자가 노트를 편집한 후 탭 키를 누르거나 카드 외부를 클릭하면, 노트가 성공적으로 저장되었다는 상태 메시지가 표시되어야 합니다.

먼저, `div` 요소를 만들고 `id` 속성을 `"status"`로 설정하세요.

# --hints--

두 번째 `div` 요소가 있어야 합니다.

```js
assert.lengthOf(document.querySelectorAll("div"), 2);
```

두 번째 `div` 요소에는 `id` 속성이 있어야 합니다.

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

두 번째 `div` 요소에는 `id` 속성이 `"status"`로 설정되어야 합니다.
