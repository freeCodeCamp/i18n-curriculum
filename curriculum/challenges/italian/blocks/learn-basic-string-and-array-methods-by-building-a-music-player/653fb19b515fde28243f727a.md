...
Per mostrare le canzoni nell'interfaccia utente (UI), devi creare una funzione.

Usa la sintassi della funzione freccia per creare una funzione `renderSongs` che prende `array` come parametro.

# --hints--

Dovresti usare `const` per creare una funzione vuota chiamata `renderSongs`.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs` dovrebbe essere una funzione.

```js
assert.isFunction(renderSongs)
```

La tua funzione `renderSongs` dovrebbe usare la sintassi della funzione freccia.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

La tua funzione `renderSongs` dovrebbe prendere `array` come parametro.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

La tua funzione `renderSongs` dovrebbe essere vuota.
