...
title: 'Problema 73: contare le frazioni in un intervallo'
challengeType: 1
forumTopicId: 302186
...
Considera la frazione, $\frac{n}{d}$, dove `n` e `d` sono numeri interi positivi. Se `n` &lt; `d` e il massimo comune divisore, ${HCF}(n, d) = 1$, si chiama frazione propria ridotta.

Se elenchiamo l'insieme delle frazioni proprie ridotte per `d` ≤ 8 in ordine crescente di grandezza, otteniamo:
...
Si può vedere che ci sono `3` frazioni tra $\frac{1}{3}$ e $\frac{1}{2}$.

Quante frazioni si trovano tra $\frac{1}{3}$ e $\frac{1}{2}$ nell'insieme ordinato delle frazioni proprie ridotte per `d` ≤ `limit`?
