...
要對數組中的元素進行排序，可以使用名爲 `.sort()` 的內置方法。 因此，你可以將在 `inputValues` 數組上調用 `.sort()` 的結果賦值給 `sortedValues` 變量，從而更新這個變量。

# --hints--

你應該更新 `sortedValues` 變量，將在 `inputValues` 數組上調用 `.sort()` 方法的結果賦值給它。
