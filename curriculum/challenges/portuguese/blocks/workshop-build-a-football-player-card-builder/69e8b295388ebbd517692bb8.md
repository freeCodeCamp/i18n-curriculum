...
Abaixo do elemento `div` com o `className` de `card-tier-badge`, adicione um `div` com `className` de `card-club` e o texto `Santos FC`.

# --hints--

Você deve adicionar um `div` com o `className` de `card-club` abaixo do `.card-tier-badge`.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

O elemento `.card-club` deve ter o texto `Santos FC`.
