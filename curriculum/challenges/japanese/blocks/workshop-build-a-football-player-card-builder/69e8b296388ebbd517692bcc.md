...
`PlayerCard` コンポーネントを `main` の `className` の `main` 要素でラップしてください。

`main` 要素の中に `layout` の `className` の `div` を追加してください。`layout` の中に、`form-panel` の `className` の `div` 要素と `preview-panel` のそれぞれ別の 2 つの `div` 要素を追加してください。

# --hints--

あなたの `FootballPlayerCard` は `div.page` の中に `main` の `className` を持つ `main` 要素をレンダリングする必要があります。
...
あなたの `main.main` は `layout` の `className` を持つ `div` を含む必要があります。
...
あなたの `layout` は `form-panel` の `className` を持つ `div` を含む必要があります。
...
あなたの `layout` は `preview-panel` の `className` を持つ `div` を含む必要があります。
