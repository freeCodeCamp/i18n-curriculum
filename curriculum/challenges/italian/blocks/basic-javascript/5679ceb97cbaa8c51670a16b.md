...
Potresti ricordare da <a href="/learn/javascript-algorithms-and-data-structures/basic-javascript/comparison-with-the-equality-operator" target="_blank" rel="noopener noreferrer nofollow">Confronto con l'operatore di uguaglianza</a> che tutti gli operatori di confronto restituiscono un valore booleano `true` o `false`.

A volte le persone usano un'istruzione `if/else` per fare un confronto, così:
