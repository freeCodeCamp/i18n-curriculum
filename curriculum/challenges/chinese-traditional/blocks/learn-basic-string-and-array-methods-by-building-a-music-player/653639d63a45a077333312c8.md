...
在播放歌曲之前，你需要確保它從頭開始。這可以透過在 `audio` 物件上使用 `currentTime` 屬性來達成。 

為你的程式碼添加一個 `if` 述語，以檢查 `userData?.currentSong` 是否為假值 *或* `userData?.currentSong.id` 是否嚴格不相等於 `song.id`。此條件將檢查是否沒有正在播放的當前歌曲，或當前歌曲是否與即將播放的歌曲不同。

在 `if` 區塊內，將 `audio` 物件的 `currentTime` 屬性設為 `0`。

# --hints--

你應該創建一個帶有條件 `userData?.currentSong === null || userData?.currentSong.id !== song.id` 的 `if` 述語。別忘了包含選用串鏈。
...
你應該在你的 `if` 區塊內將 `audio.currentTime` 設為 `0`。
