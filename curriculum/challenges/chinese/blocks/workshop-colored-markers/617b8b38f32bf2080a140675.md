---
id: 617b8b38f32bf2080a140675
title: 第 17 步
challengeType: 0
dashedName: step-17
---

# --description--

正如你在之前的课程中学到的，函数是一段可以接受输入并执行特定操作的代码。CSS 中的 `rgb` 函数接受红色、绿色和蓝色的值，或称为<dfn>参数</dfn>，并生成一种颜色：

```css
rgb(red, green, blue);
```

每个红色、绿色和蓝色的值都是从 `0` 到 `255` 的数字。`0` 表示该颜色为 0%，即黑色。`255` 表示该颜色为 100%。

在 `.one` 的 CSS 规则中，修改 `rgb` 函数中的 `red`的值。 对于 `rgb` 函数，将红色的值设置为 `255`，将绿色的值设置为 `0`，将蓝色的值设置为 `0`。

# --hints--

你的 `.one` CSS 规则不应该使用 `red` 关键字来设置 `background-color`。

```js
assert.notStrictEqual(new __helpers.CSSHelp(document).getStyle('.one')?.backgroundColor, 'red');
```

你的 `.one` CSS 规则应该有一个 `background-color` 属性，设置为 `rgb(255, 0, 0)`。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.one')?.backgroundColor, 'rgb(255, 0, 0)');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Colored Markers</title>
    <link rel="stylesheet" href="styles.css">
  </head>
  <body>
    <h1>CSS Color Markers</h1>
    <div class="container">
      <div class="marker one">
      </div>
      <div class="marker two">
      </div>
      <div class="marker three">
      </div>
    </div>
  </body>
</html>
```

```css
h1 {
  text-align: center;
}

.container {
  background-color: rgb(0, 0, 0);
}

.marker {
  width: 200px;
  height: 25px;
  margin: 10px auto;
}

.one {
--fcc-editable-region--
  background-color: red;
--fcc-editable-region--
}

.two {
  background-color: green;
}

.three {
  background-color: blue;
}

```
