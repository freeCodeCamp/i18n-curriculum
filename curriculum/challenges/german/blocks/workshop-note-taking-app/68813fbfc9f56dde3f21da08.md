...
Wenn ein Benutzer eine Notiz bearbeitet und entweder mit der Tabulatortaste oder durch Klicken außerhalb der Karte wechselt, sollte eine Statusmeldung angezeigt werden, die den Benutzenden mitteilt, dass die Notiz erfolgreich gespeichert wurde.

Erstellen Sie zunächst ein `div`-Element mit einem `id`-Attribut, das auf `"status"` gesetzt ist.

# --hints--

Sie sollten ein zweites `div`-Element haben.

```js
assert.lengthOf(document.querySelectorAll("div"), 2);
```

Ihr zweites `div`-Element sollte ein `id`-Attribut besitzen.

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

Ihr zweites `div`-Element sollte ein `id`-Attribut haben, das auf `"status"` gesetzt ist.
