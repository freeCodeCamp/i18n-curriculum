...
曲リストが変わるので、曲を再レンダリングし、ハイライト表示し、再生ボタンのアクセシブルテキストを設定する必要があります。

`renderSongs` 関数を呼び出し、`userData?.songs` 配列を引数として渡してください。これで変更されたプレイリストが表示されます。

その後、もしあれば現在の曲をハイライトするために `highlightCurrentSong` 関数を呼び出し、再生ボタンのアクセシブルテキストを更新するために `setPlayButtonAccessibleText` 関数を呼び出してください。

# --hints--

既存の `deleteSong` 関数とその内容は変更しないでください。
...
`renderSongs` を使って `userData?.songs` 関数を呼び出してください。
...
`highlightCurrentSong` 関数を呼び出してください。
...
`setPlayButtonAccessibleText` 関数を呼び出してください。
