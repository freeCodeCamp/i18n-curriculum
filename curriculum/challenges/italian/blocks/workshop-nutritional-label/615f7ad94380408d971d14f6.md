...
Il pseudo-selettore `:not` può essere usato per selezionare tutti gli elementi che non corrispondono alla regola CSS data.
...
Modifica il tuo selettore `.daily-value p` per escludere gli elementi `.no-divider`.
...
Non dovresti modificare le proprietà nel selettore `.daily-value p:not(.no-divider)`.
