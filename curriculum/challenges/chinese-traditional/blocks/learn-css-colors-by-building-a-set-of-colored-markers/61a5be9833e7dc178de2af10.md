...
這樣看起來好多了，但綠色筆的底邊需要更深一些，以增加立體感。

在同一個 `linear-gradient` 函數中，添加一個十六進制顏色代碼，其值 `11` 表示紅色，`6C` 表示綠色，`31` 表示藍色，作爲第三個顏色參數。

# --hints--

你的 `.green` CSS 規則應該有一個 `background` 屬性，設置爲 `linear-gradient(180deg, #55680D, #71F53E, #116C31)`。
