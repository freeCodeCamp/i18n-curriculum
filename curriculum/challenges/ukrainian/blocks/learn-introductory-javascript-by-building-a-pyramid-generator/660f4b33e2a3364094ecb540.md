...
Знову додайте результат виклику `padRow` з вашими змінними `i` і `count` до вашого масиву `rows`.

Відкрийте консоль, щоб побачити перевернуту піраміду.

# --hints--

Ваш цикл `for` має викликати `rows.push()`.

```js
assert.match(__helpers.removeJSComments(code), /for\s*\(\s*let\s+i\s*=\s*count\s*;\s*i\s*>\s*0\s*;\s*i\s*=\s*i\s*-\s*1\s*\)\s*\{\s*rows\.push\(/);
```

Ви маєте викликати `padRow()` у вашому виклику `.push()`.
...
Ви маєте передати `i` як перший аргумент у ваш виклик `padRow()`.
...
Ви маєте передати `count` як другий аргумент у ваш виклик `padRow()`.
