...
Para que la función de localStorage finalmente funcione, cambia la inicialización del estado de `defaultPlayer` a `loadPlayer`. Esto asegura que el estado comience con cualquier dato guardado disponible.

Ahora prueba la función de localStorage. Todo lo que ingreses debería mantenerse después de que la vista previa se actualice o recargues la página.

También puedes cambiar el enlace de la imagen `src` a `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` para ver el cambio de imagen en tiempo real. Solo asegúrate de cambiar el nombre del jugador a Maradona.

Con eso, ¡tu proyecto de creador de tarjetas de futbolistas está completo!

# --hints--

Tu `useState` debería inicializarse con `loadPlayer` en lugar de `defaultPlayer`.
