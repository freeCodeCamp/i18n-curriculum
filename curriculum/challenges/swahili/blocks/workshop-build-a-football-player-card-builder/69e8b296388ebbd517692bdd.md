...
PAS `input` yako inapaswa kuwa na `value` sawa na takwimu ya `pas` ya mchezaji.
...
PAS `input` yako inapaswa kuwa na mshughulikiaji wa `onChange` anayefanya wito wa `setPlayer` kwa kueneza `player` na `pas` iliyowekwa kuwa `Number(e.target.value)`.
