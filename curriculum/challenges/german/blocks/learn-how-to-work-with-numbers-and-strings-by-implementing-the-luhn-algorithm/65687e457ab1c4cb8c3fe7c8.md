...
Passen Sie den `verify_card_number`-Aufruf so an, dass, wenn er `True` zurückgibt, `'VALID!'` in der Konsole ausgegeben wird. Andernfalls geben Sie `'INVALID!'` aus.

# --hints--

Sie sollten `if verify_card_number(translated_card_number):` innerhalb der `main`-Funktion haben.
...
Sie sollten `print('VALID!')` innerhalb der `if`-Anweisung haben.
...
Sie sollten `print('INVALID!')` innerhalb der `else`-Klausel haben.
