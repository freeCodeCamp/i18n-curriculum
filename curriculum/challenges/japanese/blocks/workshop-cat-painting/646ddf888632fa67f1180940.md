...
左目を `position` プロパティに `absolute`、`top` に `54px`、そして `left` に `39px` を設定して位置を調整してください。

# --hints--

`.cat-left-eye` セレクターには `position` プロパティを `absolute` に設定してください。

```js
assert.equal( new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.position, 'absolute')
```

`.cat-left-eye` セレクターには `top` プロパティを `54px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.top, '54px')
```

`.cat-left-eye` セレクターには `left` プロパティを `39px` に設定してください。
