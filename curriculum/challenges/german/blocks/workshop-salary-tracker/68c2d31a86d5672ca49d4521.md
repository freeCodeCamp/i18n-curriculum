...
Jetzt ist es Zeit, der `__init__`-Methode eine Validierung hinzuzufügen. Erstellen Sie am Anfang der Methode eine `if`-Anweisung, die überprüft, ob entweder `name` oder `level` keine Instanzen von `str` sind.

Innerhalb der `if`-Anweisung lösen Sie eine `TypeError` mit der Nachricht `'name' and 'level' attribute must be of type 'str'.` aus.

# --hints--

Sie sollten eine `if`-Anweisung in Ihrer `__init__`-Methode haben.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_function("__init__").find_ifs()[0]`)) })
```

Ihre `if`-Anweisung sollte prüfen, ob entweder `name` oder `level` keine Instanzen von `str` sind.
...
Ihre `if`-Anweisung sollte eine `TypeError` mit der Nachricht `'name' and 'level' attribute must be of type 'str'.` auslösen.
