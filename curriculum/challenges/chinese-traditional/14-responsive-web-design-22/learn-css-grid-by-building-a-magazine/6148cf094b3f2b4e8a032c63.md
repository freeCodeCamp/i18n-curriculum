...
創建一個 `.first-paragraph::first-letter` 選擇器，並將 `font-size` 屬性設置爲 `6rem`。 同時給它一個 `color` 屬性，設置爲 `orangered`，以使其突出顯示。
...
你的 `.first-paragraph::first-letter` 選擇器應該有一個 `font-size` 屬性，設置爲 `6rem`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

你的 `.first-paragraph::first-letter` 選擇器應該有一個 `color` 屬性，設置爲 `orangered`。
