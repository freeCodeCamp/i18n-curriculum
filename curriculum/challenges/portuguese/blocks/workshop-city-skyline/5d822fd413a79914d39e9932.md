...
Você deve dar a `.fb5` uma propriedade `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

Você deve dar a `background` um `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Você deve dar ao `repeating-linear-gradient` uma cor inicial de `--building-color2` de `0%` a `5%`.
...
Você deve dar ao `repeating-linear-gradient` uma segunda cor de `transparent` de `5%` a `10%`.
