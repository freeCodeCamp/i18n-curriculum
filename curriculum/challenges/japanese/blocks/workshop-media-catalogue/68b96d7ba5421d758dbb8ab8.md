...
別の`except`節を作成して`MediaError as e`をキャッチしてください。`except`ブロック内で、f文字列を使って`Media Error:`を出力し、その後にスペースとエラーメッセージを続けてください。

# --hints--

`except`節で`MediaError as e`をキャッチしている必要があります。
...
新しい`Media Error:`ブロック内で、f文字列を使って`except`を出力し、その後にスペースとエラーメッセージを続けてください。
