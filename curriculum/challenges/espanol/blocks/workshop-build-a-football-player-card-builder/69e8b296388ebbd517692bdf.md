...
Agrega los atributos `value` y `onChange` a las entradas DEF y PHY.

Para DEF, asigna `value` a `player.def` y `onChange` a una función flecha que llama a `setPlayer` con un spread de `player` y `def` asignado a `Number(e.target.value)`.

Para PHY, asigna `value` a `player.phy` y `onChange` a una función flecha que llama a `setPlayer` con un spread de `player` y `phy` asignado a `Number(e.target.value)`.

Con este paso, todas las entradas están conectadas al estado y la vista previa en vivo se actualizará para cada campo.

# --hints--

Tu `input` de DEF debe tener un `value` igual a la estadística `def` del jugador.
...
Tu `input` de DEF debe tener un manejador `onChange` que llame a `setPlayer` con el spread de `player` y `def` asignado a `Number(e.target.value)`.
...
Tu `input` de PHY debe tener un `value` igual a la estadística `phy` del jugador.
...
Tu `input` de PHY debe tener un manejador `onChange` que llame a `setPlayer` con el spread de `player` y `phy` asignado a `Number(e.target.value)`.
