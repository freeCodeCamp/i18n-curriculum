...
داخل طريقة `calculate_best_price`، أنشئ قائمة باسم `prices` تحتوي على `product.price`. هذا يضمن أنه إذا لم تُطبق أي خصومات، يدفع العميل السعر الكامل.

# --hints--

داخل `calculate_best_price`، يجب أن تنشئ قائمة باسم `prices` تحتوي على `product.price`.
