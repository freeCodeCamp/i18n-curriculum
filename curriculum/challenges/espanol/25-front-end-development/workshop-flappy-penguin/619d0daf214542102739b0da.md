...
Seleccione los elementos `.eye-lid`, y déles un `width` de `150%`, un `height` de `100%`, y un `background-color` de `--penguin-face`.

# --hints--

Debes usar el selector `.eye-lid`.

```js
assert.match(code, /\.eye-lid\s*\{/);
```

Debe dar a `.eye-lid` un `width` de `--fcc-expected--`, se encuentra `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

Debe dar a `.eye-lid` un `height` de `--fcc-expected--`, se encuentra `--fcc-actual--`.
