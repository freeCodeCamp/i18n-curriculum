...
Come fatto per il precedente `multipleCard`, prendi di nuovo l'elemento `multipleCard` e imposta il suo `innerHTML` iterando attraverso `DrawingType`. Usa `Object.values()` e chiama su di esso il metodo `map`, passando `type` come argomento.

# --hints--

Dovresti assegnare `multipleCard.innerHTML` chiamando `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

Dovresti passare `DrawingType` al metodo `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

Dovresti chiamare il metodo `map` su `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Il tuo metodo `map` dovrebbe avere un parametro chiamato `type`.
