...
ادفع كائن `astronaut` إلى مصفوفة `crew` وسجل في وحدة التحكم الرسالة `Added ${astronaut.name} as ${astronaut.role}`. يمكنك استخدام إما قوالب حرفية أو ربط السلاسل النصية لرسالة السجل.

# --hints--

يجب أن تدفع `astronaut` إلى مصفوفة `crew`.
...
يجب أن تسجل `name` رائد الفضاء بالتنسيق التالي، `Added {astronaut.name} as {astronaut.role}`.
