...
要對陣列中的每個元素執行動作，請使用用於遍歷陣列的 `method`。

使用 `forEach()` 方法，並傳入一個空的回呼，該回呼以 `num` 和 `i` 作為參數。

# --hints--

你應該使用 `.forEach()` 方法來迭代 `array` 參數。

```js
assert.match(code, /array\.forEach\(/);
```

你的 `.forEach()` 方法應該使用箭頭語法來接收回呼函式（函數）。

```js
assert.match(code, /array\.forEach\s*\(\s*\(.*\)\s*=>/);
```

你的回呼函式應該接受兩個參數，`num` 和 `i`。
