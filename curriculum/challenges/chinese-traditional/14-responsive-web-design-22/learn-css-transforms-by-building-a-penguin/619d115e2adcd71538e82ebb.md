...
應該設置 `.beak` 的 `height` 爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.height, '10%');
```

應該將 `.beak` 的 `background-color` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.backgroundColor, 'orange');
```

應該將 `.beak` 的 `border-radius` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
