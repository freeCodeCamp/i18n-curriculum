...
أخيرًا، أنشئ محدِّدًا جديدًا بنسبة `75%` بين محدِّدات `50%` و `100%`. امنح هذا المحدِّد الجديد خاصية `background-color` بقيمة `yellow`.

بهذا، تصبح حركتك أكثر سلاسة ويكتمل دولاب فيريس الخاص بك.

# --hints--

يجب أن تنشئ محدِّدًا جديدًا بنسبة `75%` داخل قاعدة `@keyframes cabins` الخاصة بك.
...
يجب أن يكون محدِّد `75%` بين محدِّدات `50%` و `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText, '75%');
```

يجب أن يحتوي محدِّد `75%` على خاصية `background-color` بقيمة `yellow`.
