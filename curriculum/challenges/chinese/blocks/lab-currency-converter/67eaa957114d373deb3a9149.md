...
1. 你的 `CurrencyConverter` 组件应渲染一个 `input` 元素以接受要转换的金额。
2. 你的 `input` 元素应接受数字。
3. 你的 `CurrencyConverter` 组件应渲染两个 `select` 元素，用于选择要转换的货币 **从** 和 **到**。
4. 你的 `select` 元素应包含 **至少** `USD`、`EUR`、`GBP` 和 `JPY` 的选项。你可以使用任何汇率，只要货币之间不存在一对一的映射。以下是一些好的和不好的映射示例：
...
5. 你的 `CurrencyConverter` 组件应缓存 **from** 货币的转换金额计算，以便 **to** `select` 选项的更改不会重新计算转换金额。
6. 你的 `CurrencyConverter` 组件应渲染一个元素，显示格式为 `XX.XX CCC` 的转换金额，其中 `XX.XX` 是转换后的金额，`CCC` 是货币代码。
7. 转换后的金额应四舍五入到两位小数。

# --hints--

你应该导出一个 `CurrencyConverter` 组件。
...
你应该有一个 `input[type="number"]` 元素来接受要转换的金额。
...
`select` 元素应至少包含 `USD`、`EUR`、`GBP` 和 `JPY` 的选项。
...
更改第一个 `select` 元素的值应导致转换后的金额重新计算。
...
更改第二个 `select` 元素的值不应导致转换金额被重新计算。
...
转换后的金额应以 `XX.XX CCC` 形式显示，其中 `XX.XX` 是四舍五入到两位小数的转换金额，`CCC` 是货币代码。
