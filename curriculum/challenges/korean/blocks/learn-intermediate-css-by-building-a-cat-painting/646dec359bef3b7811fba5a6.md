...
오른쪽 안쪽 눈을 `position`의 `absolute`와 `top`의 `8px`, 그리고 `left`의 `18px`로 위치시키세요. 또한, `60%`의 테두리 반경을 주고 `-5deg`만큼 회전시키세요.

# --hints--

`.cat-right-inner-eye` 선택자에 `position`으로 설정된 `absolute` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.position === 'absolute')
```

`.cat-right-inner-eye` 선택자에 `top`으로 설정된 `8px` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.top === '8px')
```

`.cat-right-inner-eye` 선택자에 `left`으로 설정된 `18px` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.left === '18px')
```

`.cat-right-inner-eye` 선택자에 `border-radius`으로 설정된 `60%` 속성이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.borderRadius === '60%')
```

`.cat-right-inner-eye` 선택자에 `transform`으로 설정된 `-5deg` 속성이 있어야 합니다.
