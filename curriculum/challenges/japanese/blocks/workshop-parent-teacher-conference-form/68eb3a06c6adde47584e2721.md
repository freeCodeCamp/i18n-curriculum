...
クラス`submit-btn`の要素を選択してください。`cursor`を`pointer`に設定して（クリック可能であることを示すため）、`background-color`を`royalblue`、`color`を`whitesmoke`、`border`を`none`、`border-radius`を`6px`、`padding`を`12px 20px`に設定してください。

# --hints--

クラス`submit-btn`の要素をターゲットにしてください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

`cursor`プロパティを`pointer`に設定してください。
...
`background-color`プロパティを`royalblue`に設定してください。
...
`color`プロパティを`whitesmoke`に設定してください。
...
`border-radius`プロパティを`6px`に設定してください。
...
`padding`プロパティを`12px 20px`に設定してください。
