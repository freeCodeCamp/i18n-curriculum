...
Ihr `.text`-Element ist kein CSS Grid, aber Sie können innerhalb eines Elements Spalten erstellen, ohne Grid zu verwenden, indem Sie die `column-width`-Eigenschaft nutzen.

Geben Sie Ihrem `.text`-Selektor eine `column-width`-Eigenschaft mit dem Wert `25rem`.

# --hints--

Ihr `.text`-Selektor sollte eine `column-width`-Eigenschaft mit dem Wert `25rem` haben.
