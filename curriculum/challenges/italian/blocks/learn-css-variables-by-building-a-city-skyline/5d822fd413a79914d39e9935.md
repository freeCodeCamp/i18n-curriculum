...
Aggiungi un altro gradiente ripetuto a questo edificio; fallo uguale a quello che hai appena aggiunto, ma senza aggiungere la direzione `90deg` e usa il colore della tua finestra invece dei due colori `transparent`.

# --hints--

Dovresti dare a `.fb6` una seconda `repeating-linear-gradient` nella proprietà `background`.
...
Dovresti dare al secondo `repeating-linear-gradient` un primo colore `--building-color3` da `0%` a `10%`.
...
Dovresti dare al secondo `repeating-linear-gradient` un secondo colore `--window-color3` da `10%` a `30%`.
