...
Wenn die Differenz innerhalb des angegebenen `tolerance` liegt, weisen Sie den Wert von `root` `mid` zu und verlassen Sie die Schleife.

# --hints--

Sie sollten die vorhandene `pass`-Anweisung entfernen.
...
Sie sollten den Wert von `mid` `root` zuweisen und die Schleife verlassen.
