...
Mais uma coisa. If you keep clicking the `Load More Authors` button until there's no more data to load and the text changes to `"No more data to load"`, the cursor value is still `pointer`. Por que não mudar o valor do cursor para `not-allowed` em vez disso?
