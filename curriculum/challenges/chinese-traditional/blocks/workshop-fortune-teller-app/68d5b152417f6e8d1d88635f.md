...
還有更多屬性要添加到 `Card` 介面。先將 `value_int` 設為 `number` 型別。其他所有屬性都應該是 `string` 型別：`suit`、`type`、`img`、`meaning_up`、`meaning_rev` 和 `desc`。

# --hints--

你應該有一個屬性 `value_int`，其型別為 `number`。
...
你應該有一個屬性 `meaning_up`，其型別為 `string`。
...
你應該有一個屬性 `meaning_rev`，其型別為 `string`。
