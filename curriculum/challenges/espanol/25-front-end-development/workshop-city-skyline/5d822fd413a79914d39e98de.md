...
Cree una nueva variable debajo de su variable `--building-color1`. Nombre su nueva variable `--building-color2` y asígnale un valor de `#66cc99`. Luego establézcala como el `background-color` de `.bb2`.

# --hints--

Debe definir una nueva variable de propiedad llamada `--building-color2`.

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color2'));
```

Debe asignar a `--building-color2` un valor de `#66cc99` dentro de la clase `.bb1`.
...
Debe establecer el `background-color` usando la variable `--building-color2`.
