...
次に、`classList` プロパティと `add()` メソッドを使って、`"playing"` 要素に `playButton` クラスを追加してください。これは CSS ファイル内の `"playing"` クラスを探して、`playButton` 要素に追加します。

最後に曲を再生するには、`play()` 変数の `audio` メソッドを使ってください。`play()` は mp3 ファイルを再生するためのウェブオーディオ API のメソッドです。

# --hints--

`classList` プロパティと `add` メソッドを使って、`"playing"` に `playButton` クラスを追加してください。

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

`play()` メソッドを `audio` 変数に使ってください。
