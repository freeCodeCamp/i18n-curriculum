...
Dovresti ottimizzare il tuo codice. Sposta le proprietà e i valori `position` e `top` da `.foreground-buildings` a `.background-buildings`. Poi seleziona sia `.background-buildings` che `.foreground-buildings` lì, applicando così quegli stili a entrambi gli elementi. Puoi usare una virgola (`,`) per separare i selettori in questo modo: `selector1, selector2`.
