...
Le opzioni del menu a discesa sono attualmente vuote. Per aggiungere contenuto, inserisci il testo seguente in ciascun elemento `option` successivo:
...
Dovresti dare al quarto elemento `option` il testo `freeCodeCamp Forum`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.textContent, 'freeCodeCamp Forum');
```

Dovresti dare al quinto elemento `option` il testo `Other`.
