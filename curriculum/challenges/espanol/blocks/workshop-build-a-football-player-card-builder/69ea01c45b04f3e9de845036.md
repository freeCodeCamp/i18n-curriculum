...
El problema ahora es que una vez que la vista previa se actualiza o recargas la página, todo lo que agregas para la tarjeta del jugador desaparece.

Guardar todas las entradas en localstorage evitará esto. Para comenzar con eso, crea una constante `STORAGE_KEY` con la cadena `football_player_card` como su valor.

# --hints--

Debes declarar una variable `STORAGE_KEY`.
...
Tu `STORAGE_KEY` debe tener la cadena `football_player_card` como su valor.
