...
Створіть селектор `img` і надайте йому властивість `width` зі значенням `100%` та властивість `object-fit` зі значенням `cover`.

Властивість `object-fit` повідомляє браузеру, як розташувати елемент в своєму контейнері. В цьому разі `cover` налаштує зображення на заповнення контейнера, обрізаючи за потреби, щоб уникнути зміни співвідношення сторін.
...
Ваш селектор `img` повинен мати властивість `width` зі значенням `100%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('img')?.width === '100%');
```

Ваш селектор `img` повинен мати властивість `object-fit` зі значенням `cover`.
