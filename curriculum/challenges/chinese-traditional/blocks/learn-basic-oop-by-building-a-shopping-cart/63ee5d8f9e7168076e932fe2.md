...
在你的 `h2` 元素之後，創建兩個 `p` 元素。 爲第一個元素指定一個 `dessert-price` 的 `class`，並將文本設置爲美元符號 `"$"` 後跟 `price` 變量的值。 爲第二個元素指定一個 `product-category` 的 `class`，以及文本 `"Category: "`，後跟 `category` 變量的值。
...
你的第一個 `p` 元素應該包含美元符號 `"$"` 後跟 `price` 變量的值。
...
你的第二個 `p` 元素應該包含 `"Category: "` 後跟 `category` 變量的值。
