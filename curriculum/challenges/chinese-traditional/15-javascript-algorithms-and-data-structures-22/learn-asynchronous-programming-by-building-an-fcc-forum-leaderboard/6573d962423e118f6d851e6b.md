...
接下來，使用 `getElementById()` 方法訪問 `#posts-container` 元素。 將其賦值給一個名爲 `postsContainer` 的新常量。

# --hints--

你應該使用 `const` 來聲明 `postsContainer` 變量。

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

你應該使用 `document.getElementById()` 來獲取 `#posts-container` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

你應該將 `#posts-container` 元素賦值給 `postsContainer` 變量。
