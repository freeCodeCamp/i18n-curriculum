...
Adicione um terceiro objeto com as seguintes propriedades e valores:
...
Seu array `allSongs` deve ter um objeto como seu terceiro valor.

```js
assert.isObject(allSongs[2]);
```

O terceiro objeto no seu array `allSongs` deve ter uma propriedade `id` definida com o número `2`.

```js
assert.equal(allSongs[2].id, 2);
```

O terceiro objeto no seu array `allSongs` deve ter uma propriedade `title` definida com a string `"Still Learning"`.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

O terceiro objeto no seu array `allSongs` deve ter uma propriedade `artist` definida com a string `"Quincy Larson"`.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

O terceiro objeto no seu array `allSongs` deve ter uma propriedade `duration` definida como a string `"3:51"`.

```js
assert.equal(allSongs[2].duration, "3:51");
```

O terceiro objeto no seu array `allSongs` deve ter uma propriedade `src` definida para a string `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`.
