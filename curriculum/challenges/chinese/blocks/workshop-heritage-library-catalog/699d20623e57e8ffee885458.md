...
year 字段以字串形式传入，但你需要它作为数字以便后续计算。使用 `parseInt()` 来转换它。

year 也可能为空，因此你需要一个备用值。三元操作符 `condition ? valueIfTrue : valueIfFalse` 根据条件返回两个值中的一个。

将 return 语句中的 `year` 属性更新为：
