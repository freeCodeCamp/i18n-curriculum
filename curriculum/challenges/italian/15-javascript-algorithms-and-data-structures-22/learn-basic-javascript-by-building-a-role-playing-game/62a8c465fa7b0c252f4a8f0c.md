...
Ora che stai aggiornando le variabili `gold` e `health`, hai bisogno di mostrare i nuovi valori sullo schermo di gioco. You have retrieved the `healthText` and `goldText` elements in a prior step.

Dopo le righe di assegnazione, assegna la proprietà `innerText` di `goldText` in modo che sia la variabile `gold`. Usa lo stesso modello per aggiornare `healthText` con la variabile `health`.
