...
Dentro del array `allSongs`, crea un objeto con las siguientes propiedades y valores:
...
Tu array `allSongs` debería tener un valor.

```js
assert.equal(allSongs.length, 1);
```

Tu array `allSongs` debería tener un objeto como su primer valor.

```js
assert.isObject(allSongs[0]);
```

Tu array `allSongs` debería tener un objeto con una propiedad `id` configurada al número `0`.

```js
assert.strictEqual(allSongs[0].id, 0);
```

Tu array `allSongs` debería tener un objeto con una propiedad `title` configurada a la cadena `\"Scratching The Surface\"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Tu array `allSongs` debería tener un objeto con una propiedad `artist` configurada a la cadena `\"Quincy Larson\"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Tu array `allSongs` debería tener un objeto con una propiedad `duration` configurada a la cadena `\"4:25\"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Tu array `allSongs` debería tener un objeto con una propiedad `src` configurada a la cadena `\"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3\"`.
