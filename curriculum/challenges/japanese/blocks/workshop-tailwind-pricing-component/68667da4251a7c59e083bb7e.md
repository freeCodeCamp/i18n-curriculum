...
各リスト項目内の `span` 要素に、マーカーを緑色にしてテキストから少し離すために `text-green-500` と `mr-2` のクラスを代入してください。

また、最初の料金プランの `aria-hidden` 要素に行ったように、`span` の属性も代入してください。

# --hints--

`span` 要素には `text-green-500` のクラスを付けてください。
...
`span` 要素には `aria-hidden` の属性を付けてください。
