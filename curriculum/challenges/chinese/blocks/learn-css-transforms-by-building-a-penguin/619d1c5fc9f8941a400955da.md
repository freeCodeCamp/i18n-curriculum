...
在某些浏览器中， *爱心* emoji 表情可能与上一步略有不同。 这是因为字符的某些属性被 `font-weight` 的 `bold` 样式覆盖。

要修复这个问题，需要定位心形表情符号的 `div`， 并将其 `font-weight` 设置为其原始值。

# --hints--

你应该使用 `.shirt div` 选择器来定位有心形表情符号的 `div` 元素。

```js
assert.match(code, /\.shirt div\s*\{/);
```

你应该将 `.shirt div` 的 `font-weight` 设置为 `initial` 或 `normal`。
