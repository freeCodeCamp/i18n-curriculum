...
Crea un elemento `ul` all'interno dell'elemento `nav`. All'interno di quell'elemento `ul`, crea tre elementi di lista.

# --hints--

Dovresti creare un elemento `ul` all'interno del tuo elemento `nav`.

```js
assert.exists(document.querySelector('nav ul'));
```

Dovresti creare tre elementi `li` all'interno del tuo elemento `ul`.
