...
Більшість браузерів додають свої власні CSS-властивості та значення за замовчуванням для різних елементів. Якщо придивитися уважно, можна помітити, що файл `input` менший за інші текстові елементи `input`. За замовчуванням `padding` з `1px 2px` задається `input` елементам, у які можна вводити текст.

Використовуючи інший селектор атрибутів, стилізуйте `input` з відступом `type` рівним `file`, щоб він був таким самим, як у інших `input` елементів.
...
Ви повинні задати `input[type="file"]` відступ `padding` рівний `1px 2px`.
