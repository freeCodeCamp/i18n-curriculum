...
Unda kipengele kingine cha `p`, kipa maandishi ya `Calcium 260mg 20%`. Panga `20%` upande wa kulia. Chini yake, unda kipengele cha `p` chenye maandishi ya `Iron 8mg 45%`, ukiwapanga `45%` upande wa kulia.

# --hints--

Unapaswa kuunda vipengele viwili vipya vya `p` mwishoni mwa kipengele chako cha `.daily-value`.
...
Kipengele chako cha kwanza kipya cha `p` kinapaswa kuwa na maandishi ya `Calcium 260mg 20%`.
...
Kipengele chako cha kwanza kipya cha `p` kinapaswa kuwa na kipengele cha `span`.
...
Kipengele chako cha kwanza cha `span` hakipaswi kuwa herufi nzito.
...
Kipengele chako cha kwanza cha `span` kinapaswa kuzunguka maandishi ya `20%`.
...
Kipengele chako cha pili kipya cha `p` kinapaswa kuwa na maandishi ya `Iron 8mg 45%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText?.match(/Iron 8mg[\s|\n]+45%/));
```

Kipengele chako cha pili kipya cha `p` kinapaswa kuwa na kipengele cha `span`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.firstElementChild?.localName === 'span');
```

Kipengele chako cha pili cha `span` hakipaswi kuwa herufi nzito.
