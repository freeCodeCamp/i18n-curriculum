...
既然你可以評估數學表達式，就需要考慮操作行為的順序。宣告一個帶有 `str` 參數的 `highPrecedence` 函式（程式）。

# --hints--

你應該宣告一個 `highPrecedence` 變數。

```js
assert.match(code, /(?:var|let|const)\s+highPrecedence/);
```

你應該使用 `const` 來宣告你的 `highPrecedence` 變數。

```js
assert.match(code, /const\s+highPrecedence/);
```

你的 `highPrecedence` 變數應該是一個函式（程式）。

```js
assert.isFunction(highPrecedence);
```

你的 `highPrecedence` 函式（程式）應該使用箭頭語法。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

你的 `highPrecedence` 函式（程式）應該有一個 `str` 參數。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>/);
```

你的 `highPrecedence` 函式（程式）應該是空的。
