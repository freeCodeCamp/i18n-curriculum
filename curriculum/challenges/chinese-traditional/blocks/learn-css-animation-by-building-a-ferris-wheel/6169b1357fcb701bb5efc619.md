...
在你的 `0%` 和 `50%` 選擇器之間創建一個新的 `25%` 選擇器。為這個新的選擇器設定 `background-color` 屬性為 `yellow`。

# --hints--

你應該在你的 `@keyframes cabins` 規則中創建一個新的 `25%` 選擇器。
