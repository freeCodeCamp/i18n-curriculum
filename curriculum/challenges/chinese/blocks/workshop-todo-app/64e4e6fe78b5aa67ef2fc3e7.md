...
接下来，使用 `getElementById()` 方法访问 `discard-btn`、`tasks-container` 和 `title-input` 元素。 将它们分别保存在名为 `discardBtn`、`tasksContainer` 和 `titleInput` 的变量中。

# --hints--

你应该使用 `getElementById()` 来访问 `discard-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)discard\-btn\1\s*\)\s*;?/)
```

你应该将 `discard-btn` 元素分配给变量 `discardBtn`。 不要忘记使用 `const` 来声明变量。

```js
assert.match(code, /const\s+discardBtn\s*=\s*document\.getElementById\(\s*('|"|`)discard\-btn\1\s*\)\s*;?/)
```

你应该使用 `getElementById()` 来访问 `tasks-container` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)tasks\-container\1\s*\)\s*;?/)
```

你应该将 `tasks-container` 元素分配给变量 `tasksContainer`。 不要忘记使用 `const` 来声明变量。

```js
assert.match(code, /const\s+tasksContainer\s*=\s*document\.getElementById\(\s*('|"|`)tasks\-container\1\s*\)\s*;?/)
```

你应该使用 `getElementById()` 来访问 `title-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)title\-input\1\s*\)\s*;?/)
```

你应该将 `title-input` 元素分配给变量 `titleInput`。 不要忘记使用 `const` 来声明变量。
