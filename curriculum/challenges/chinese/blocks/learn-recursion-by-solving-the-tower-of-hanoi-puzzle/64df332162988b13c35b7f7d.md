...
现在你需要对 `elif` 语句执行相同的操作：将 `print()` 和 `make_allowed_move()` 调用放入 `if` 语句中，以便在 `n` 为奇数时执行。

另外，将 `# make_allowed_move(source, auxiliary)` 注释转换为代码。

# --hints--

你应该将 `print()` 和 `make_allowed_move()` 调用嵌套在 `if` 语句中，以便在 `n` 为奇数时执行。
