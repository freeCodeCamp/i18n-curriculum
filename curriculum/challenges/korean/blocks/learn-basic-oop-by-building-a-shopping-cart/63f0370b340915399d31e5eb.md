...
이 메서드에서 HTML도 업데이트할 거예요. `textContent`의 `cartSubTotal`를 `subTotal`의 값을 소수점 둘째 자리로 고정한 값으로 설정하세요. 템플릿 리터럴 구문을 사용해 값 앞에 달러 기호를 추가하세요.

# --hints--

`textContent` 요소의 `cartSubTotal` 속성에 접근해야 합니다.

```js
assert.match(cart.calculateTotal.toString(), /cartSubTotal\.textContent/);
```

`.toFixed()` 변수에서 `subTotal` 메서드를 호출하고 `2`를 인수로 전달해야 합니다.

```js
assert.match(cart.calculateTotal.toString(), /subTotal\.toFixed\(\s*2\s*\)/);
```

템플릿 리터럴 구문을 사용해 `.toFixed()` 호출 앞에 달러 기호를 추가해야 합니다.
...
`textContent` 요소의 `cartSubTotal`를 템플릿 문자열로 설정해야 합니다.
