...
通过在 `.foreground-buildings` 元素的顶部和底部添加两个空 `div` 元素，以及在 `.fb2` 和 `.fb3` 之间再添加一个，来再次使建筑物紧凑。

# --hints--

你应该添加两个 `div` 元素作为 `.foreground-buildings` 的最初的子元素。
...
你应该添加两个 `div` 元素作为 `.foreground-buildings` 的最后的子元素。
