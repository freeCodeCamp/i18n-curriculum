...
如果你想為你的社交圖示添加更多，但保持它們在同一列，你需要更新 `grid-template-columns` 來創建額外的行。作為替代方案，你可以使用 `grid-auto-flow` 屬性。

此屬性的第一個值接受 `row` 或 `column`，第二個值為選用的 `dense`。`grid-auto-flow` 使用自動配置演算法來調整網格佈局。將其設定為 `column` 將告訴演算法根據需要創建新的行。`dense` 值允許演算法回溯並用較小的項目填補網格中的空洞，這可能導致項目出現順序錯亂。

為你的 `.social-icons` 選擇器，將 `grid-auto-flow` 屬性設為 `column`。

# --hints--

你的 `.social-icons` 選擇器應該有一個 `grid-auto-flow` 屬性設定為 `column`。
