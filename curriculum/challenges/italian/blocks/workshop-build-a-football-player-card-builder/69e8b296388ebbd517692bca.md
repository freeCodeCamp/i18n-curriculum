...
`FootballPlayerCard` attualmente renderizza la card con un `defaultPlayer` fisso. Per rendere l'app interattiva, devi gestire i dati del giocatore nello stato di React in modo che gli aggiornamenti possano attivare nuovi render.

All'interno del componente `FootballPlayerCard`, crea una variabile di stato chiamata `player` con un setter `setPlayer` che conterrà i dati correnti del giocatore. Inizializza questo stato con `defaultPlayer` e specifica che lo stato usa il tipo generico `PlayerData` così TypeScript sa quale forma deve avere l'oggetto giocatore.

Dopodiché, aggiorna il return per passare la variabile di stato `player` come prop a `PlayerCard`.

# --hints--

Dovresti usare la destrutturazione di array per impostare una variabile di stato `player` e un setter `setPlayer`.

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

Il tuo `player` e `setPlayer` dovrebbero usare il hook `useState`.
...
Il tuo `useState` dovrebbe essere inizializzato con `defaultPlayer` e avere `PlayerData` come tipo generico.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

Dovresti avere `player` come valore della prop `player` passata a `PlayerCard`.
