...
قم بتفليف كل قيمة من القيم المحددة بالفعل لخاصية `grid-template-columns` في دالة `minmax` ، باستخدام كل قيمة كـ argument ثانية. يجب أن تكون الـ argument الأولى `2rem` و `min-content` و `2rem` على التوالي.

# --hints--

يجب أن يكون لمنتقي `main` الخاص بك خاصية `grid-template-columns` بقيمة `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
