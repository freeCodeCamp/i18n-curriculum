...
Fügen Sie jedem `span` zwei `className`-Elemente mit der `stat-value` von `stat-label` bzw. `stat-row` hinzu.

# --hints--

Jedes `.stat-row` im zweiten `.stat-col` sollte ein `span` mit der Klasse `stat-value` und ein `span` mit der Klasse `stat-label` enthalten.
