...
الآن بعد أن عرفت كيف تجد الأغنية التالية للتشغيل، أنشئ دالة باسم `playNextSong`. داخل دالة `playNextSong`، أنشئ جملة شرطية `if` للتحقق مما إذا كانت قيمة `currentSong` في `userData` تساوي بدقة `null`. هذا سيتحقق مما إذا لم تكن هناك أغنية حالية تُشغل في كائن `userData`.

إذا تحقق الشرط، نفذ استدعاء للدالة `playSong` مع `id` أول أغنية في مصفوفة `userData.songs` كمعلمة.

ثم أضف جملة `return` في نهاية كتلة `if`.

# --hints--

يجب أن يكون لديك دالة باسم `playNextSong`.

```js
assert.isFunction(playNextSong);
```

عندما تكون قيمة `currentSong` في `userData` تساوي بدقة `null`، يجب أن تستدعي دالة `playNextSong` الدالة `playSong` مع `id` أول أغنية في مصفوفة `userData.songs` كمعلمة.
...
يجب أن تحتوي نهاية كتلة `if` على جملة `return`.
