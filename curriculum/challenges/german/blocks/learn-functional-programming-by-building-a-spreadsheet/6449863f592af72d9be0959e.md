...
Nachdem Ihr `rangeRegex` das `:` gefunden hat, muss es nach dem gleichen Buchstaben- und Zahlenmuster suchen wie zuvor.

Kopieren Sie Ihre zwei vorhandenen Capture-Gruppen und fügen Sie sie nach dem Doppelpunkt ein.

# --hints--

Sie sollten Ihrer `rangeRegex` eine dritte Capture-Gruppe hinzufügen, nach dem Doppelpunkt.
...
Ihre dritte Capture-Gruppe sollte eine Zeichenklasse verwenden.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[.*\]\)/);
```

Ihre dritte Capture-Gruppe sollte die Zeichen `A` bis `J` erfassen.
...
Sie sollten Ihrer `rangeRegex` eine vierte Capture-Gruppe hinzufügen.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(.*\)/);
```

Ihre vierte Capture-Gruppe sollte eine oder zwei Ziffern erfassen.
