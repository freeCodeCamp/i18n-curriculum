...
코드가 꽤 반복적이라는 것을 눈치채셨을 거예요! 비슷한 여러 요소를 처리하는 더 좋은 방법이 있습니다.

각 버튼을 개별적으로 선택하는 대신, `querySelectorAll(".emoji-btn")`를 사용해 `emoji-btn` 클래스를 가진 모든 버튼을 한 번에 선택하세요. 결과를 `btns`라는 변수에 저장하세요.
...
모든 이모지 버튼을 선택하려면 `querySelectorAll`를 사용하세요.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

결과를 `btns`라는 변수에 저장해야 합니다.
...
`btns` 변수에는 4개의 이모지 버튼이 모두 포함되어야 합니다.
