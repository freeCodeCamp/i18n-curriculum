...
使用 `+=` 運算符將 `" Your <weapon> breaks."` 添加至 `text.innerText` 末尾，並在 `Your` 前面留一個空格。 使用 `inventory.pop()` 將 `<weapon>` 替換爲 `inventory` 數組中的最後一項，這將刪除數組中的最後一項並將其返回，以便它出現在你的字符串中。

# --hints--

你應該使用複合賦值運算符來修改 `text.innerText`。
