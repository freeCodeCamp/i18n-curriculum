...
`else` 블록을 추가하세요. `else` 블록 안에서 현재 재생 중인 노래의 `playSong`를 인수로 하여 `id` 함수를 호출하세요.

이렇게 하면 재생 버튼을 클릭할 때 현재 재생 중인 노래가 계속 재생됩니다.

# --hints--

`playButton`를 클릭하고 `userData.currentSong`가 `null`가 아닐 때, 현재 재생 중인 노래의 `playSong`를 인수로 하여 `id` 함수를 호출해야 합니다.
...
`playSong`를 클릭하고 `id`가 `playButton`일 때도, 재생 목록의 첫 번째 노래의 `userData.currentSong`를 인수로 하여 `null` 함수를 호출해야 합니다.
