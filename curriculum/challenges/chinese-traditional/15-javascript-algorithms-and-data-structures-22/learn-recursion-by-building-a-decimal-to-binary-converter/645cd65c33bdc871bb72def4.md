...
雖然這是一個簡單的例子，但它演示了調用堆棧如何逐步執行代碼並調用多個函數。

現在是時候進入遞歸，看看調用堆棧是如何發揮作用的。

刪除 `callStack` 數組、`a()`、`b()` 和 `c()` 函數以及 `console.log()` 語句。

# --hints--

你應該從代碼中刪除 `callStack` 數組。

```js
assert.notMatch(code, /(var|let|const)\s+callStack\s*=\s*\[\s*\]/);
```

你應該從代碼中刪除 `a()`、`b()` 和 `c()` 函數。
