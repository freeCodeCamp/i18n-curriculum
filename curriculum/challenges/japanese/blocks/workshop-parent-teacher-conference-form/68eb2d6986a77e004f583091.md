...
次はラジオボタンをカスタマイズする時間です。デフォルトでは、ラジオボタンはブラウザのネイティブな見た目を持っています。これを取り除いて、カスタムの見た目を作成したいです。

クラス`contact-method-radio-btn`の要素を選択してください。`appearance`を`none`に設定してください。

# --hints--

クラス`contact-method-radio-btn`の要素をターゲットにしてください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

`appearance`プロパティを`none`に設定してください。
