...
获取数据后，将响应保存到一个变量中并解析为 JSON。给解析后的数据赋予类型 `Deck`，然后将其赋值给 `cards` 属性。在 `catch` 块内，确保使用消息 `"Error fetching cards:", error` 记录误差。

# --hints--

你应该在 `data` 内声明一个名为 `fetchCardsData` 的变量。
...
你应该将 `data.cards` 保存到 `this.cards` 中。

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

你的 `catch` 块应该使用 `console.error()` 记录带有消息 `"Error fetching cards:", error` 的误差。
