...
`.fb4`의 위치를 현재 위치에서 `position`의 `relative`와 `left`의 `10%`를 추가하여 이동하세요. `.fb5`에도 같은 작업을 하되 `right` 대신 `left`를 사용하세요. 이렇게 하면 건물 사이의 남은 흰 공간을 가릴 수 있습니다.

# --hints--

`.fb4`에 `position`의 `relative`를 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.position, 'relative');
```

`.fb4`에 `left`의 `10%`를 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb4')?.left, '10%');
```

`.fb5`에 `position`의 `relative`를 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.fb5')?.position, 'relative');
```

`.fb5`에 `right`의 `10%`를 지정하세요.
