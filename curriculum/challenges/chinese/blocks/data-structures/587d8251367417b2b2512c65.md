...
在我们继续讨论另一个数据结构之前，让我们先了解链表的最后几点练习。
...
就像我们的 `remove(element)` 方法一样，<a href="/learn/coding-interview-prep/data-structures/remove-elements-from-a-linked-list" target="_blank" rel="noopener noreferrer nofollow">我们在之前的课程中讲过</a>，当我们在 `removeAt(index)` 方法中移除节点时，需要小心不要使列表的其余部分成为孤立。我们通过确保引用被移除节点的节点引用下一个节点，来保持节点的连续性。

# --instructions--

编写一个 `removeAt(index)` 方法，该方法删除并返回给定 `index` 处的节点。 如果给定的 `index` 为负数，或大于等于链表的 `length`，该方法应返回 `null`。
