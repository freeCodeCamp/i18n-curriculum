...
Aggiungi un `constructor` all'interno della classe. Nel costruttore, devi usare gli elementi creati in precedenza per selezionare i corrispondenti elementi DOM usando la funzione `getElement`.

Per ora, gestisci `singleCardBtn` e `singleCard`. Per `singleCardBtn`, passa `#btn-single-card`, e per `singleCard`, passa `.single_card`.

# --hints--

La classe `Game` dovrebbe avere un `constructor`.
...
Il tuo costruttore dovrebbe assegnare `this.elements`.
...
`singleCardBtn` dovrebbe essere impostato usando `getElement` con `'#btn-single-card'`.
...
`singleCard` dovrebbe essere impostato usando `getElement` con `'.single_card'`.
