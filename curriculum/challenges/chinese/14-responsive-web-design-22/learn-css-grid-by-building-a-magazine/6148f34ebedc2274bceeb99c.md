...
设置一个对应 `max-width` 为 `720px` 的 `only screen` 的 `@media` 查询。 在里面，创建一个 `.image-wrapper` 选择器，并给它一个值为 `1fr` 的 `grid-template-columns` 属性。

这将在较小的屏幕上将三个图像折叠成一列。

# --hints--

你应该有一个新的 `@media` 查询，对应 `only screen and (max-width: 720px)`。
...
你的新 `.image-wrapper` 选择器应该具有值为 `1fr` 的 `grid-template-columns` 属性。
