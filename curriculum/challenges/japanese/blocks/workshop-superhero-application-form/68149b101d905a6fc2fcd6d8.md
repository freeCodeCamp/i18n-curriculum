...
分割代入された `value` と `checked` の下に、チェックされたパワーのリストを更新してください。

配列の引数を取る `setPowers` 関数を呼び出してください。`checked` が true かどうかを三項演算子で確認します。true なら、既存の `powers` 配列を新しい配列に展開し、`value` を追加してください。true でなければ、`value` を使って `powers` から `powers.filter(p => p !== value)` を除外してください。

# --hints--

`checked` が true かどうかを三項演算子で確認する必要があります。true なら、既存の `powers` と `value` を配列に展開してください。そうでなければ、`value` から `powers` を除外してください。
