...
داخل دالة `playSong`، نفذ استدعاء دالة `highlightCurrentSong`.

بعد ذلك، جرّب أزرار التحكم لترى كيف تعمل دالة `highlightCurrentSong`.

# --hints--

يجب أن تنفذ استدعاء دالة `highlightCurrentSong` داخل دالة `playSong`.
