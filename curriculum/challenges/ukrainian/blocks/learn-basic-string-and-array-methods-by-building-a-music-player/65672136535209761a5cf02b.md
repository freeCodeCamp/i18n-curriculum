...
Щоб відтворювати пісню щоразу, коли користувач натискає на неї, додайте атрибут `onclick` до першого елемента button. Усередині `onclick` викличте функцію `playSong` з `song.id`.

Не забудьте, що тут потрібно інтерполювати за допомогою знака долара.

# --hints--

Ви повинні додати атрибут `onclick` зі значенням `playSong(${song.id})` до елемента `button`.
