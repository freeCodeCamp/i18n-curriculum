...
1. Crie um `div` para cada ponto de dados no array

2. Dê a cada `div` uma altura dinâmica, usando uma função de retorno de chamada no método `style()` que define a altura igual ao valor dos dados
