...
Створіть останній запит `@media` для `only screen` з `max-width` зі значенням `420px`. Всередині створіть селектор `.hero-title` з властивістю `font-size` зі значенням `4.5rem`.

Вітаємо! Ваш журнал готовий.

# --hints--

Ви повинні мати новий запит `@media` для `only screen and (max-width: 420px)`. Це повинен бути останній запит в списку запитів `@media`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

Новий запит `@media` повинен мати селектор `.hero-title`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

Ваш селектор `.hero-title` повинен мати властивість `font-size` зі значенням `4.5rem`.
