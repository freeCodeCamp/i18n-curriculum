...
Agora que a verificação `if` está no lugar, use `e.target.value` para procurar o bug correspondente em `bugMap` e faça a chamada do seu método `render`. Depois disso, você deve conseguir selecionar os bugs e ver o resultado imediatamente.

Com isso, sua oficina Bug Emoji Picker está completa!

# --hints--

Você deve fazer a chamada do método `render` no bug correspondente de `bugMap`.
