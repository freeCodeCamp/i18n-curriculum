...
Um die aktuelle Zeit des Songs zu speichern, wenn er pausiert wird, setzen Sie die `songCurrentTime` des `userData`-Objekts auf die `currentTime` der `audio`-Variable.

*Hinweis*: Sie sollten für diesen Schritt keine optionale Verkettung verwenden, da `userData.songCurrentTime` zu diesem Zeitpunkt weder `null` noch `undefined` sein wird.

# --hints--

Sie sollten die bestehende `pauseSong`-Funktion und deren Inhalt nicht verändern.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Sie sollten `audio.currentTime` `userData.songCurrentTime` zuweisen.
