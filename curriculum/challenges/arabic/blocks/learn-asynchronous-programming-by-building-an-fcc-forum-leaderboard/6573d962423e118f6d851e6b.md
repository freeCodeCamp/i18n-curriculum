...
بعد ذلك، وصول إلى العنصر `#posts-container` باستخدام الطريقة `getElementById()`. عيّنه إلى ثابت جديد يسمى `postsContainer`.

# --hints--

يجب استخدام `const` لإعلان متغير `postsContainer`.

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

يجب استخدام `document.getElementById()` للحصول على العنصر `#posts-container`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

يجب تعيين العنصر `#posts-container` إلى متغير `postsContainer`.
