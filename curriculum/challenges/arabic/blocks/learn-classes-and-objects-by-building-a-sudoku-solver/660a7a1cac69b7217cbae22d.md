...
بعد جملة `if`، أنشئ حلقة `for` للتكرار على النطاق من `1` إلى `9` شاملة. استخدم `guess` كمتغير للحلقة.

ستمكنك هذه الحلقة من التحقق بشكل منهجي إذا كان أي رقم من `1` إلى `9` مناسبًا لملء الخانة الفارغة.

# --hints--

يجب أن تنشئ حلقة `for` تتكرر على `range(1, 10)`.
...
يجب أن يكون لمتغير الحلقة في `for` اسم `guess`.
