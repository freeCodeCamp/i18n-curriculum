...
Dein `ul`-Element sollte ein öffnendes Tag haben. Opening tags have this syntax: `<elementName>`.

```js
assert.exists(document.querySelector('ul'));
```

Dein `ul`-Element sollte ein schließendes Tag haben. Closing tags have a `/` just after the `<` character.
