...
이제 `div`의 `className`가 `form-group`인 요소를 추가하세요. 그 안에 `label`가 `className`이고 `label` 속성이 `htmlFor`인 `name`을 추가하세요. 그 아래에 `input`가 `id`이고 `name`가 `className`이며 `input` 속성이 `type`인 `text`을 추가하세요.

# --hints--

`div`가 `className`인 `form-group`가 있어야 합니다.
...
`form-group` 안에 `label`가 `className`이고 `label` 속성이 `htmlFor`인 `name`이 있어야 합니다.
...
`form-group` 안에 `input`가 `id`이고 `name`가 `className`이며 `input`이 `type`인 `text`이 있어야 합니다.
