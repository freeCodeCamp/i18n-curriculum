...
`allSongs` 배열에는 두 개의 값이 있어야 합니다.

```js
assert.equal(allSongs.length, 2);
```

`allSongs` 배열의 두 번째 값은 객체여야 합니다.

```js
assert.isObject(allSongs[1]);
```

`allSongs` 배열의 두 번째 객체는 `id` 속성이 숫자 `1`로 설정되어야 합니다.

```js
assert.strictEqual(allSongs[1].id, 1);
```

`allSongs` 배열의 두 번째 객체는 `title` 속성이 문자열 `"Can't Stay Down"`로 설정되어야 합니다.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

`allSongs` 배열의 두 번째 객체는 `artist` 속성이 문자열 `"Quincy Larson"`로 설정되어야 합니다.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

`allSongs` 배열의 두 번째 객체는 `duration` 속성이 문자열 `"4:15"`로 설정되어야 합니다.

```js
assert.equal(allSongs[1].duration, "4:15");
```

`allSongs` 배열의 두 번째 객체는 `src` 속성이 문자열 `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`로 설정되어야 합니다.
