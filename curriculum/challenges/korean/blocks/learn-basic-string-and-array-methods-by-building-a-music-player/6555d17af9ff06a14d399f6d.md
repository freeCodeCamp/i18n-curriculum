...
함수 내부에서 노래 제목과 아티스트를 표시하는 HTML 요소에 대한 참조를 얻으세요.

`#player-song-title` 메서드를 사용해 `#player-song-artist`와 `getElementById()` 요소에 접근하세요. 각각 `playingSong`와 `songArtist` 변수에 할당하세요.

# --hints--

기존 `setPlayerDisplay` 함수와 그 내용을 수정하지 마세요.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

`document.getElementById()`를 사용해 `#player-song-title` 요소를 얻으세요.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

`#player-song-title` 요소를 `playingSong` 변수에 할당하세요. 변수를 선언할 때 `const`를 사용하는 것을 잊지 마세요.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

`document.getElementById()`를 사용해 `#player-song-artist` 요소를 얻으세요.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

`#player-song-artist` 요소를 `songArtist` 변수에 할당하세요. 변수를 선언할 때 `const`를 사용하는 것을 잊지 마세요.
