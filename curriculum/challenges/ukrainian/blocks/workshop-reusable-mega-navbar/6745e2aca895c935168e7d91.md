...
Щоб завершити, встановіть текст `Calendar` для першого елемента посилання, `Chat` для другого та `Email` для третього.

Таким чином, ваш мегакомпонент панелі навігації готовий!

# --hints--

Ви повинні встановити текст `Calendar` для першого елемента посилання.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Ви повинні встановити текст `Chat` для другого елемента посилання.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Ви повинні встановити текст `Email` для третього елемента посилання.
