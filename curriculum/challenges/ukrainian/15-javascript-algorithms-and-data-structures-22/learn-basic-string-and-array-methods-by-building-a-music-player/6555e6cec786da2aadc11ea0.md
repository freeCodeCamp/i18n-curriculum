...
Ви також повинні повторно перемалювати пісні, призупинити поточну пісню, налаштувати відображення програвача та знову встановити доступний текст кнопки відтворення.

Викличте функцію `renderSongs` та передайте `userData?.songs` як аргумент. Також викличте функції `pauseSong`, `setPlayerDisplay` та `setPlayButtonAccessibleText`.

# --hints--

Не змінюйте наявну функцію `shuffle` та її вміст.
