...
لاحظ أن صورة الألبوم في HTML والأغاني في مصفوفة `userData.songs` قد تغيرت. استبدلنا الأغاني الأصلية بأغاني أقصر يمكنك استخدامها لاختبار تطبيقك في الخطوات القادمة.

بعد ذلك، تحتاج إلى التحقق مما إذا كانت هناك أغنية تالية للتشغيل. استخرج فهرس الأغنية الحالية باستدعاء الدالة `getCurrentSongIndex()`، واحفظه في ثابت `currentSongIndex`.

بعد ذلك، أنشئ ثابتًا باسم `nextSongExists` يحتوي على القيمة المنطقية `true` أو `false` حسب وجود الأغنية التالية.

# --hints--

لا تعدل مستمع الحدث `ended` الحالي ومحتواه.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

أنشئ ثابتًا باسم `currentSongIndex` واضبطه على نتيجة استدعاء الدالة `getCurrentSongIndex`.
...
يجب أن تتحقق مما إذا كانت هناك أغنية تالية بمقارنة `userData.songs.length` و`currentSongIndex` وأن تضبط النتيجة في الثابت `nextSongExists`. إذا كان آخر فهرس في مصفوفة الأغاني (`userData.songs.length - 1`) أكبر من currentSongIndex فهذا يعني أن هناك أغنية تالية.
