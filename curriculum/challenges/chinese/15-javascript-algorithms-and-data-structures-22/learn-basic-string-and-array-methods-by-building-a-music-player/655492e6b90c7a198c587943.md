...
创建一个带有条件 `songToHighlight` 的 `if` 语句。 对于语句，在 `songToHighlight` 上使用 `setAttribute` 来传入 `"aria-current"` 和 `"true"` 作为第一个和第二个参数。

# --hints--

你应该创建一个带有条件 `songToHighlight` 的 `if` 语句。

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

你应该在 `if` 语句中的 `songToHighlight` 上使用 `setAttribute()` 方法。

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

你应该将 `"aria-current"` 和 `"true"` 作为 `setAttribute` 方法的第一个和第二个参数传入。
