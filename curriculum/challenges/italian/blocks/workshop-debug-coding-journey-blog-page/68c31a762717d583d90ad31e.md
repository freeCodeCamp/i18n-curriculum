...
Nella pagina del blog, ci sono in totale tre post del blog racchiusi all'interno di elementi generici `div`. Ma sarebbe meglio usare elementi semantici per racchiudere ogni post.

Cambia ogni elemento `div` che racchiude ogni post del blog in un elemento `article`.

# --hints--

Il tuo primo elemento `div` per il primo post dovrebbe essere cambiato in un elemento `article`.
...
Il tuo secondo elemento `div` per il secondo post dovrebbe essere cambiato in un elemento `article`.
...
Il tuo terzo elemento `div` per il terzo post dovrebbe essere cambiato in un elemento `article`.
