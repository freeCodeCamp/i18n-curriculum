...
أضف محدِّدًا جديدًا يغيّر `background-color` لعنصر `.card` إلى `khaki` عند تحويم المؤشر فوقه.

# --hints--

يجب أن يكون لديك محدِّد `.card:hover`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card:hover"));
```

يجب أن يحتوي محدِّد `.card:hover` على `background-color` مضبوط على `khaki`.
