...
title: Crear una app de cuestionario con tarjetas didácticas
challengeType: 25
...
En este laboratorio, crearás una app que muestra y almacena datos de tarjetas didácticas que se pueden recuperar después.
...
1) Debes tener un elemento HTML con un id de `flashcard`.  
2) Debes tener una interfaz llamada `FlashCard`.  
3) La interfaz `FlashCard` debe contener las siguientes propiedades `questionText` y `questionAnswer`, ambas del tipo `string`.  
4) Debes tener una colección de elementos `FlashCard` llamada `currentCards`.  
5) Cuando se haga clic en el elemento `#flashcard`, la tarjeta debe tener la clase `flipped`.  
6) Debes tener un elemento con un id de `delete-btn`.  
7) Cuando se haga clic en el elemento `#delete-btn`, debe eliminar la tarjeta actual y mostrar los datos de la tarjeta anterior.  
8) Debes crear un formulario de entrada con un id de `entry-form` para poder agregar más tarjetas didácticas a la colección `currentCards` al enviarlo.  
9) Los dos elementos dentro del formulario deben tener un id de `front-text` y `back-text` respectivamente.  
10) Debes crear y llamar a un `InvalidUserInputError` cuando el texto de la pregunta o la respuesta de la pregunta estén vacíos en el formulario de entrada.

# --hints--

Debes tener un elemento HTML con un id de `flashcard`.
...
Debes tener una interfaz `FlashCard`.
...
La interfaz `FlashCard` debe tener una propiedad `questionText` de tipo `string`.
...
La interfaz `FlashCard` debe tener una propiedad `questionAnswer` de tipo `string`.
...
Debes tener una colección de elementos `FlashCard` llamada `currentCards` con el tipo `FlashCard[]`.
...
Cuando se haga clic por primera vez en el elemento `#flashcard`, el elemento debe recibir la clase `flipped`.  
...
Debes tener un elemento con un id de `delete-btn`. 
...
Cuando se haga clic en `delete-btn`, se debe eliminar un elemento de tarjeta didáctica de la colección `currentCards`.
...
Cuando se haga clic en `delete-btn`, se deben mostrar los datos de la tarjeta didáctica anterior. 
...
Debes crear un formulario de entrada con un id de `entry-form`. 
...
Debes tener dos elementos `textarea` con ids `front-text` y `back-text` respectivamente dentro del formulario.
...
Se debe lanzar un `InvalidUserInputError` cuando el texto de la pregunta o la respuesta de la pregunta estén vacíos en el formulario de entrada.
