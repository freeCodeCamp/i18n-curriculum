...
`Logs` 是系统保存的用于跟踪事件和操作的记录数据。它们对于查找进程中发生的情况或诊断问题非常有帮助。例如：

`The IT team checked the system logs to find out why the server crashed.` - 这里，`logs` 用于跟踪和记录系统活动，帮助团队了解导致服务器故障的原因。
