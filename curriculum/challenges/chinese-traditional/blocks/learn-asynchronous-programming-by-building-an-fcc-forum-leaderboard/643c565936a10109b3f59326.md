...
當你建置你的 `showLatestPosts()` 函式（程式）時，你需要呼叫它來查看你的變更。

在你的 `try` 區段結尾呼叫 `showLatestPosts()` 函式，並將 `data` 作為引數傳入。

# --hints--

你應該在你的 `try` 區塊結尾呼叫 `showLatestPosts()`。

```js
assert.match(code, /\s*showLatestPosts\(.*\)\s*;?\s*/);
```

你應該將 `data` 作為引數傳遞給 `showLatestsPosts()`。
