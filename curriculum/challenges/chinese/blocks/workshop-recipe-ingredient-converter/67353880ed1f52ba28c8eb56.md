...
是时候看看你的柯里函数的实际效果了。以下是你可能调用我们示例函数的方式：

```js
const dinner = tikkaMasala("curry powder")("tofu")("onion");
```

声明一个 `gramsResult` 变量，并将使用你的柯里的函数将 `2` 单位的度量 `cup` 转换为 `gram` 的结果赋值给它。

将该值记录到控制台日志。

# --hints--

你应该有一个 `gramsResult` 变量。

```js
assert.isDefined(gramsResult);
```

你的 `gramsResult` 变量应包含调用 `convertQuantity("cup")("gram")(2)` 的结果。

```js
assert.strictEqual(gramsResult, convertQuantity("cup")("gram")(2));
```

你应该将 `gramsResult` 记录到控制台日志。
