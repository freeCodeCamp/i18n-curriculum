...
La foto del giocatore viene caricata da un URL hardcoded. Sostituisci il valore dell'attributo `src` `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` con `player.imageUrl`.

Sostituisci anche il testo `alt` hardcoded con un valore dinamico usando un template literal con `player.name` e il testo `headshot`.

Nota che quando usi un'espressione JavaScript come valore di un attributo in TSX, devi usare le parentesi graffe `{}` invece delle virgolette. Per esempio `src={player.imageUrl}`, non `src="{player.imageUrl}"`.

# --hints--

Dovresti sostituire l'URL dell'immagine hardcoded con `player.imageUrl` come attributo `src`.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

Dovresti sostituire il testo `alt` hardcoded con un template literal usando `player.name` e il testo `headshot`.
