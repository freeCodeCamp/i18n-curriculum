...
Ndani ya wito wa kurudisha `useEffect`, ongeza sharti la `if` ili kukagua kama `query.trim() === ''`. Hii inakagua kama `query` ni mfuatano wa herufi tupu au ina nafasi tu. Ikiwa ni hivyo, fanya wito wa `setResults([])` na rudisha mapema.

# --hints--

Unapaswa kukagua kama `query.trim() === ''`.
...
Ikiwa `query.trim() === ''` ni kweli, fanya wito wa `setResults()` ukiwa na safu ya data tupu, kisha tumia neno kuu la `return` baada ya hapo.
