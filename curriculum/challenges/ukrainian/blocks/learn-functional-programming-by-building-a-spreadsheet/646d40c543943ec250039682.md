...
Тепер, коли у вас є регулярний вираз для пошуку множення або ділення, ви можете оцінити цей вираз.

Оголосіть змінну `str2` і призначте їй результат виклику `infixEval` з аргументами `str` та `regex`.

# --hints--

Ви повинні оголосити змінну `str2`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*(?:const|let|var)\s+str2/);
```

Ви повинні використати `const` для оголошення вашої змінної `str2`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2/);
```

Ви повинні призначити `str2` результат виклику вашої функції `infixEval`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(/);
```

Ви повинні передати `str` як перший аргумент у виклик `infixEval`.
...
Ви повинні передати `regex` як другий аргумент у виклик `infixEval`.
