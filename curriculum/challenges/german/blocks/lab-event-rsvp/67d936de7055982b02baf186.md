...
title: Ein Event-RSVP erstellen
challengeType: 25
...
1. Sie sollen ein Formular mit Feldern für Name, E-Mail, Anzahl der Teilnehmer, Ernährungspräferenzen und einer Option erstellen, um anzugeben, ob Sie zusätzliche Gäste mitbringen.

2. Sie sollen ein Texteingabefeld haben, in das Sie Ihren Namen eingeben (Pflichtfeld).

3. Sie sollen ein E-Mail-Eingabefeld haben, in das Sie Ihre E-Mail-Adresse eingeben (Pflichtfeld). Das Formular soll das Format validieren, um sicherzustellen, dass es sich um eine gültige E-Mail handelt.

4. Sie sollen ein Zahlen-Eingabefeld haben, in das Sie die Anzahl der Teilnehmer im Formular eingeben (Pflichtfeld), und die Zahl darf nicht kleiner als eins sein.

5. Sie sollen ein Texteingabefeld haben, in das Sie Ihre Ernährungspräferenzen eingeben, und diese Information soll optional sein.

6. Sie sollen ein Kontrollkästchen aktivieren oder deaktivieren können, um anzugeben, ob Sie zusätzliche Gäste mitbringen.

7. Sie sollen eine Schaltfläche haben, die das Formular absendet, und das Formular soll beim Absenden die Seite nicht neu laden.

8. Sie sollen nach dem Absenden eine Bestätigungsnachricht unter dem Formular sehen, gefolgt von den angegebenen Details (Name, E-Mail, Anzahl der Teilnehmer, Ernährungspräferenzen und optional zusätzliche Gäste).

Hier ist eine Beispielnachricht nach dem Absenden des Formulars:
...
Sie sollten ein `form`-Element haben, das den gesamten Formularinhalt enthält.
...
Sie sollten zwei `input`-Elemente vom Typ `text` haben.
...
Sie sollten ein `input`-Element vom Typ `email` haben.
...
Sie sollten ein `input`-Element vom Typ `number` haben.
...
Sie sollten ein `input`-Element vom Typ `checkbox` haben.
...
Ein `input[type="text"]`-Element sollte erforderlich sein.
...
Das `input[type="email"]`-Element sollte erforderlich sein.
...
Das `input[type="number"]`-Element sollte erforderlich sein.
...
Das `input[type="number"]`-Element darf keine Werte kleiner als 1 akzeptieren.
...
Das Ändern des Werts der `input[type="text"]`-Elemente sollte den Komponentenstatus aktualisieren.
...
Das Ändern des Werts der `input[type="email"]`-Elemente sollte den Komponentenstatus aktualisieren.
...
Das Ändern des Werts der `input[type="number"]`-Elemente sollte den Komponentenstatus aktualisieren.
...
Das Ändern des Werts der `input[type="checkbox"]`-Elemente sollte den Komponentenstatus aktualisieren.
...
Das Absenden des Formulars sollte nicht zum Neuladen der Seite führen.
...
Nach dem Absenden sollte ein Element auf der Seite den Status des Namens `input` anzeigen.
...
Nach dem Absenden sollte ein Element auf der Seite den Status der E-Mail `input` anzeigen.
...
Nach dem Absenden sollte ein Element auf der Seite den Status der Anzahl der Teilnehmer `input` anzeigen.
...
Nach dem Absenden sollte ein Element auf der Seite den Status der Ernährungspräferenzen `input` anzeigen.
...
Nach dem Absenden sollte ein Element auf der Seite den Status der zusätzlichen Gäste `input` anzeigen.
