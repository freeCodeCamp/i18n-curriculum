...
窓は一番右の紫色の建物の上に重なっています。建物をフレックスボックスの親要素にして、`flex-wrap` プロパティを使って窓を横並びにし、収まらない場合は新しい行に押し下げてください。

# --hints--

`.fb4b` に `display` を `flex` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4b")?.display, "flex");
```

`.fb4b` に `flex-wrap` を `wrap` に設定してください。
