...
次にスタイルを適用するのは価格のテキストで、`span`も含みます。価格の`p`要素に`text-4xl`、`font-bold`、`text-gray-900`のクラスを付けてください。

# --hints--

`p`要素には`text-4xl`クラスを付けてください。
...
`p`要素には`text-gray-900`クラスを付けてください。
