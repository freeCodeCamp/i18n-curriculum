...
你应该在你的 `info` 按钮元素内添加一个 `<i class="fas fa-info-circle"></i>`。
...
你应该在你的 `delete` 按钮元素内添加一个 `<i class="fas fa-trash"></i>`。
...
你的每个 `i` 元素都应该有一个闭合标签，并且 `<i class="fas fa-thumbs-up"></i>` 位于你的 `like` 按钮元素中。
