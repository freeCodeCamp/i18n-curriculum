...
1. フォームには、ユーザーのフルネームを入力するためのタイプ `text` の `input` 要素と、ID `name` が含まれていること。
1. ユーザーのメールアドレスを入力するためのタイプ `email` の `input` 要素と、ID `email` の別の要素があること。
1. フォームには、職種を選択できる ID `position` の `select` 要素が含まれていること。
1. クラス `radio-group` の `fieldset` 要素があること。
1. `.radio-group` の中に、タイプ `radio` の `input` 要素のセットがあり、利用可能なオプション（例：フルタイム、パートタイム）を選択できること。グループ `name` は `availability` であること。
1. メッセージを入力するための ID `message` の `textarea` 要素があること。
1. すべての `input`、`select`、および `textarea` 要素に `label` 要素が関連付けられていること。
1. フォームを提出するためのタイプ `submit` の `button` 要素があること。
1. `input` と `textarea` 要素に `:focus` 疑似クラスを追加し、フォーカス時にボーダーの色を変え、デフォルトのアウトラインを削除すること。
...
1. ラジオボタンが選択されたときに関連する `label` のテキスト色を変えるために `:checked` 疑似クラスをラジオボタンに使うこと。
...
フォームには、ユーザーのフルネームを入力するためのタイプ `text` の `input` 要素と、ID `name` が含まれていること。

```js
assert.exists(document.querySelector("div.container > form input#name[type='text']"));
```

ユーザーのメールアドレスを入力するためのタイプ `email` の `input` 要素と、ID が `email` の別の要素があること。

```js
assert.exists(document.querySelector("div.container > form input#email[type='email']"));
```

フォームには、ID `position` の `select` 要素があり、いくつかの `option` 要素が含まれていること。
...
`.radio-group` の中に、利用可能なオプションを選択するためのタイプ `radio` の `input` 要素のグループがあること。グループ `name` は `availability` であること。
...
メッセージを入力するための ID `message` の `textarea` 要素があること。

```js
assert.isNotEmpty(document.querySelectorAll("div.container > form textarea#message"));
```

すべての `input`、`select`、および `textarea` 要素に `label` 要素が関連付けられていること。
...
ラジオボタンが選択されたときに関連する `label` のテキスト色を変えるために `:checked` 疑似クラスを使うこと。
