...
O elemento `section` deve ter uma tag de abertura. Opening tags have this syntax: `<elementName>`.
...
O elemento `h2` deve ter uma tag de abertura. Opening tags have this syntax: `<elementName>`.

```js
assert(document.querySelectorAll('h2').length === 2);
```

O elemento `h2` deve ter uma tag de fechamento. Closing tags have a `/` just after the `<` character.
