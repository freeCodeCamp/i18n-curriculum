...
Aggiungi un gestore `onerror` all'elemento immagine per gestire i fallimenti di caricamento. Al primo fallimento, contrassegnalo usando `dataset.failed` e sostituisci la sorgente con l'immagine predefinita locale. Se l'immagine fallisce di nuovo, rimuovi la classe hidden e nascondi il segnaposto di caricamento usando `previousElementSibling`.

# --hints--

Il tuo elemento `img` dovrebbe avere un gestore `onerror`.
...
Il tuo gestore `onerror` dovrebbe contenere un'istruzione `if` che verifica se `dataset.failed` è falsy.
...
`dataset.failed` dovrebbe essere impostato su `'1'` all'interno del blocco `if`. Non dimenticare la parola chiave `this` qui.
...
Il tuo blocco `if` dovrebbe aggiornare `this.src` con l'immagine predefinita locale. Non dimenticare la parola chiave `this` qui.
...
Dovresti avere un blocco `else` che rimuove la classe `hidden`. Non dimenticare la parola chiave `this` qui.
...
Il tuo blocco `else` dovrebbe impostare `previousElementSibling.style.display` su `'none'`. Non dimenticare la parola chiave `this` qui.
