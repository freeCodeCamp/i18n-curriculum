...
Kitendakazi chako sasa kimekamilika. Kijaribu tena kwa kuandika `genParentheses(2)` badala ya `genParentheses(1)`.

# --hints--

Unapaswa kuandika `genParentheses(2)`.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*genParentheses\s*\(\s*2\s*\)\s*\)/);
```

Haupasi kuwa na `console.log(genParentheses(1))` katika msimbo wako.
