...
هذه جملة وسمية، تُستخدم لتأكيد أو طلب الموافقة على العبارة.

# --explanation--

الجملة الرئيسية هي `Their current projects would stop` (جملة مثبتة).

الجملة الوسمية هي `wouldn't they?` (الصيغة المنفية من `would` مع الضمير `they`).
