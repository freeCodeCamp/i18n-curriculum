...
每个 `region` 角色都需要一个标签，以帮助屏幕阅读器用户理解该区域的用途。 添加标签的一种方法是，在区域内添加一个标题元素，然后用 `aria-labelledby` 属性引用它。
