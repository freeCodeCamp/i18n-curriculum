...
你可以將函數 <dfn>reference</dfn> 作爲回調參數傳遞。 函數引用是不帶括號的函數名稱。 舉個例子：
...
這裏的 `.map()` 方法將調用 `myFunc` 函數，並傳遞 `.map()` 回調所採用的相同參數。 第一個參數是當前迭代的數組的值，因此 `newArray` 將是 `[value: 1, value: 2, value: 3]`。

將對 `elemValue` 函數的引用作爲對 `.map()` 方法的回調傳遞。
...
你應該將對 `elemValue` 的引用作爲回調傳遞給你的 `.map()` 方法。
