...
الخاصية `object-fit` تخبر المتصفح كيفية وضع العنصر داخل الحاوية (container). في هذه الحالة ، سيقوم `cover` بتعيين الصورة لملء الحاوية، مع الاقتصاص حسب الحاجة لتجنب تغيير نسبة العرض إلى الارتفاع.

# --hints--

يجب أن يكون لديك منتقي `img`.

```js
assert(new __helpers.CSSHelp(document).getStyle('img'));
```

يجب أن يكون لمنتقي `img` الخاص بك الخاصية `width` بقيمة `100%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('img')?.width === '100%');
```

يجب أن يحتوي منتقي `img` على خاصية `object-fit` بقيمة `cover`.
