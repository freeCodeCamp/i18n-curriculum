...
`deleteTask` 함수에서 했던 것처럼, 편집할 작업의 인덱스를 찾아야 합니다.

`dataArrIndex` 변수를 만드세요. 그 값으로 `findIndex()`에서 `taskData` 메서드를 사용하세요. 콜백 함수의 매개변수로 `item`를 전달하고, `id`의 `item`가 `id`의 `parentElement`의 `buttonEl`와 같은지 확인하세요.

# --hints--

`editTask` 함수는 변경하지 마세요.

```js
assert.match(code, /const\s+editTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*/)
```

`const`을 사용해 `dataArrIndex` 변수를 선언하고 `taskData.findIndex()`으로 설정하세요.

```js
assert.match(code, /const\s+editTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(/)
```

`item` 화살표 함수 콜백의 매개변수로 `findIndex()`를 전달하세요. 중괄호는 사용하지 마세요.

```js
assert.match(code, /const\s+editTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)/)
```

화살표 함수 콜백에서 `item.id === buttonEl.parentElement.id`인지 확인하세요.
