...
새로 만든 커링된 함수를 테스트해야 합니다. `servingsResult` 변수를 선언하고, 기본 수량 4에서 6인분으로 조정한 결과를 할당하세요.

그 변수를 콘솔에 출력하세요.

# --hints--

`servingsResult` 변수를 선언해야 합니다.

```js
assert.isDefined(servingsResult);
```

`servingsResult` 변수에는 `adjustQuantity(4)(6)`를 호출한 결과가 있어야 합니다.

```js
assert.strictEqual(servingsResult, 24);
```

`servingsResult` 값을 콘솔에 출력해야 합니다.
