...
Neste projeto você aprenderá métodos básicos de string e array construindo um aplicativo de player de música.Você poderá reproduzir, pausar, pular e embaralhar músicas.

O HTML e o CSS deste projeto foram fornecidos para você, para que possa se concentrar no JavaScript.

Comece acessando os elementos `#playlist-songs`, `#play` e `#pause` com o método `getElementById()`.Atribua-os às variáveis `playlistSongs`, `playButton` e `pauseButton`, respectivamente.


# --hints--

Você deve ter uma variável chamada `playlistSongs`. 

```js
assert.isDefined(playlistSongs);
```

Você deve usar `document.getElementById()` para obter o elemento `#playlist-songs`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

Você deve atribuir o elemento `#playlist-songs` à variável `playlistSongs`. 

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

Você deve ter uma variável chamada `playButton`. 

```js
assert.isDefined(playButton);
```

Você deve usar `document.getElementById()` para obter o elemento `#play`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

Você deve atribuir o elemento `#play` à variável `playButton`. 

```js
assert.equal(playButton, document.getElementById('play'));
```

Você deve ter uma variável chamada `pauseButton`. 

```js
assert.isDefined(pauseButton);
```

Você deve usar `document.getElementById()` para obter o elemento `#pause`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

Você deve atribuir o elemento `#pause` à variável `pauseButton`. 
