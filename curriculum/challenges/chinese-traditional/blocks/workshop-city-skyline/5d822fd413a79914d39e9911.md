...
看起來不錯！ 構建前景建築！ 通過在其中嵌套三個新的 `div` 元素，將 `.fb1` 建築物變成三個部分。 按順序給它們 `fb1a`、`fb1b` 和 `fb1c` 類。
