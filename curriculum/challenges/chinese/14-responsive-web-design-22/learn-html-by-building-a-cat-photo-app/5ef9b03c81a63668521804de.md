...
Both radio buttons should have a `name` attribute. 请确保在开始标签的名称后面要有一个空格，且所有的属性名称前面也要有一个空格。
...
Both radio buttons should have a `type` attribute. 请检查在开始标签的名称后面要有一个空格，且/或所有的属性名称前面也要有一个空格。
...
Both radio buttons should have a `name` attribute with the value `indoor-outdoor`. 你可能落下了这个值或者有拼写错误。 所有属性的值都应该放在引号中。
