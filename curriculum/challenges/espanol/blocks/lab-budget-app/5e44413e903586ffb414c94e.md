...
1. La clase `Category` debe tener los siguientes métodos:
...
1. Cuando se imprima un objeto `Category`, debe:  
   - Mostrar una línea de título de 30 caracteres con el nombre de la categoría centrado entre caracteres `*`.  
   - Listar cada entrada de `ledger` con hasta 23 caracteres de su descripción alineados a la izquierda y el monto alineado a la derecha (dos decimales, máximo 7 caracteres).  
   - Mostrar una línea final `Total: [balance]`, donde `[balance]` debe ser reemplazado por el total de la categoría.
   ...
   Y aquí un ejemplo de la salida:
...
1. Debes tener una función fuera de la clase `Category` llamada `create_spend_chart(categories)` que recibe una lista de categorías y devuelve una cadena con un gráfico de barras. Para construir el gráfico:
   ...
   Esta función será probada con hasta cuatro categorías.

   Asegúrate de que el espaciado coincida exactamente con el ejemplo de salida:
