...
在你的新註解之後，撰寫一個 `for` 迴圈來迭代 `constraints` 列表。使用 `constraint` 和 `pattern` 作為迴圈變數。

# --hints--

你應該撰寫一個 `for` 迴圈來迭代 `constraints` 列表。
...
你的 `for` 迴圈應該使用 `constraint` 和 `pattern` 作為迴圈變數來迭代 `constraints` 列表。
