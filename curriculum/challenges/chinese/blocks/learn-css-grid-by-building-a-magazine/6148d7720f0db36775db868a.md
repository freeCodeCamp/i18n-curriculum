...
你将需要有一个文本列和一个图像列。 将 `.text-with-images` 选择器的 `grid-template-columns` 属性设置为 `1fr 2fr`。 并将 `column-gap` 属性设置为 `3rem`，以提供更大的列间距。

# --hints--

你的 `.text-with-images` 选择器应该有一个 `grid-template-columns` 属性，设置为 `1fr 2fr`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

你的 `.text-with-images` 选择器应该有一个 `column-gap` 属性，设置为 `3rem`。
