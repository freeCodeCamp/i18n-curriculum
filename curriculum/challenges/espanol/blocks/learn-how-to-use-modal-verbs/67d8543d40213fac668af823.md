---
id: 67d8543d40213fac668af823
title: Tarea 160
challengeType: 19
dashedName: task-160
lang: en-US
---

<!-- READING -->

# --instructions--

Lee el texto y responde la pregunta a continuación.

# --questions--

## --text--

Jake decidió enviar un correo electrónico al equipo de seguridad para informarles sobre una brecha de seguridad reciente y solicitar su ayuda en la investigación.

`Subject: URGENT. Security Breach Investigation in Progress`

`Hi team,`

`We detected unusual activity on our internal network last night, indicating a potential security breach. While reviewing the logs, we found a high volume of traffic from a specific IP address, which suggests a targeted attack. At this point, we are unsure how they got past our firewall, but we are investigating whether an open port or another vulnerability was exploited.`

`Our immediate steps are:`

`1. Isolating affected systems to prevent further access.`

`2. Running a full security scan to assess the extent of the breach.`

`3. Reviewing access logs to determine which files or systems were compromised.`

`Please prioritize this investigation and report any findings as soon as possible. If you notice anything unusual in the logs or have any insights, let me know immediately.`

`Thanks,`

`Jake`

¿Cuáles son los tres pasos inmediatos que el equipo está tomando para manejar la brecha?

## --answers--

Cambiar todas las contraseñas de los empleados, actualizar los firewalls y reiniciar los servidores.

### --feedback--

Estas son buenas medidas de seguridad, pero el correo electrónico de Jake enumera diferentes acciones inmediatas.

---

Apagar toda la red, llamar a las autoridades y restaurar las copias de seguridad.

### --feedback--

Jake se está enfocando en contener e investigar la brecha, no en apagar todo.

---

Bloqueando todas las direcciones IP externas, cifrando todos los archivos y limitando el acceso del usuario.

### --feedback--

El correo electrónico de Jake enumera diferentes acciones inmediatas.

---

Aislar los sistemas afectados, ejecutar un escaneo de seguridad completo y revisar los registros de acceso.

## --video-solution--

4

# --explanation--

Jake describe tres pasos inmediatos:

`Isolating affected systems to prevent further access.`

`Running a full security scan to assess the extent of the breach.`

`Reviewing access logs to determine which files or systems were compromised.`

Concéntrate en cómo estos pasos ayudan a contener el ataque e identificar vulnerabilidades.
