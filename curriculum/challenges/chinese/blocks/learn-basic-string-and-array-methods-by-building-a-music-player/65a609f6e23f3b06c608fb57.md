...
`sort()` 方法接受定义排序顺序的比较回调函数。 

在此示例中，第一个条件 `(a.name < b.name)` 用于查看第一个水果的名称是否小于第二个水果的名称。如果是，则第一个水果会排在第二个水果之前。

字符串按字典顺序进行比较，这意味着它们逐个字符进行比较。 例如，`"Apples"` 小于 `"Bananas"`，因为按字母表顺序，`"A"` 位于 `"B"` 之前。

此示例返回数字的原因是因为 `sort()` 方法期望返回一个数字。 如果返回负数，则第一项将排在第二项之前。 
...
在回调函数内部，添加 `if` 语句来检查 `a.title` 是否小于 `b.title`。 如果是，则返回 `-1`。 
...
你的 `if` 语句应检查 `a.title` 是否小于 `b.title`。

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
``` 

如果 `a.title` 小于 `b.title`，则 `if` 语句应返回 `-1`。
