...
Тепер настав час кастомізувати радіокнопки. За замовчуванням радіокнопки мають нативний вигляд браузера. Ви хочете прибрати це, щоб створити кастомний вигляд.

Виберіть елемент із класом `contact-method-radio-btn`. Встановіть `appearance` у `none`.

# --hints--

Ви маєте націлитися на елемент із класом `contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Встановіть властивість `appearance` у `none`.
