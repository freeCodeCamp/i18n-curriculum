...
Weise deinem neuen Selektor eine `font-size`-Eigenschaft von `0.7em` zu, damit er wie eine Unterüberschrift aussieht.

# --hints--

Du solltest einen `h1 .flex span:first-of-type`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1 .flex span:first-of-type'));
```

Dein `h1 .flex span:first-of-type`-Selektor sollte eine `font-size`-Eigenschaft von `0.7em` haben.
