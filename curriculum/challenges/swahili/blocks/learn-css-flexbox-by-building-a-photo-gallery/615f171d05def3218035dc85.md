...
Ondoa ukingo kutoka kwenye kipengele chako cha body, weka `font-family` kuwa `sans-serif`, na umpe `background-color` yenye thamani ya `#f5f6f7`.
...
Kichaguzi chako cha `body` kinapaswa kuwa na sifa ya `margin` iliyowekwa thamani 0.
...
Kichaguzi chako cha `body` kinapaswa kuwa na sifa ya `font-family` iliyowekwa thamani ya `sans-serif`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily === 'sans-serif');
```

Kichaguzi chako cha `body` kinapaswa kuwa na sifa ya `background-color` iliyowekwa thamani ya `#f5f6f7`.
