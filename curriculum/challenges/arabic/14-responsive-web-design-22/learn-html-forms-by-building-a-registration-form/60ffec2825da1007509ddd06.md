...
باستخدام منتقي سمة (attribute selector) آخر، قم بتصميم `input` من `type` بقيمة `file` ليكون بنفس قيمة الـ padding كعناصر `input` الأخرى.

# --hints--

يجب عليك استخدام منتقي السمة (attribute selector) لتصميم (style) عنصر `input`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="file"]'));
```

يجب عليك إعطاء `input[type="file"]` الـ `padding` بقيمة `1px 2px`.
