...
Prima di fidarti dei dati analizzati, inizia con la validazione dei campi obbligatori.

Crea una funzione chiamata `validateEntry` che prende `entry` come parametro. Dichiara `let isValid = true`.

Aggiungi un controllo di validazione per `title`: imposta `isValid = false` se `title` manca, è falso o è uguale a `"Unknown"`.

Restituisci `isValid`. Aggiungerai i controlli per gli altri campi nei passaggi successivi.

# --hints--

`validateEntry` dovrebbe restituire un booleano.

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })` dovrebbe restituire `true`.

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})` dovrebbe restituire `false`.

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })` dovrebbe restituire `false`.

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })` dovrebbe restituire `false`.
