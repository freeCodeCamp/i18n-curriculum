...
Um das Lied schließlich abzuspielen, verwenden Sie die `play()`-Methode auf der `audio`-Variablen. `play()` ist eine Methode der Web-Audio-API zum Abspielen einer mp3-Datei.

# --hints--

Sie sollten die `classList`-Eigenschaft und die `add`-Methode verwenden, um die Klasse `"playing"` zu `playButton` hinzuzufügen.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

Sie sollten die `play()`-Methode auf der `audio`-Variablen verwenden.
