...
El control de misión requiere un método para copiar astronautas elegibles para EVA de una tripulación. Crea una función vacía llamada `getEVAReadyCrew` que acepte un parámetro `crew`.

# --hints--

Debes crear una función llamada `getEVAReadyCrew`.

```js
assert.isFunction(getEVAReadyCrew);
```

Tu función `getEVAReadyCrew` debe tener un parámetro `crew`.
