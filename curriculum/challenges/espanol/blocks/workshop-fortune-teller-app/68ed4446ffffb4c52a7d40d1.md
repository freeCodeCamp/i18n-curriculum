...
Dentro del constructor, agrega llamadas a `fetchCardsData()` y `initializeEventListeners()` usando la palabra clave `this`, para que se ejecuten cuando se inicialice la clase.

El método `fetchCardsData` se encargará de cargar los datos de las cartas del tarot desde una API, y `initializeEventListeners` configurará los manejadores de clic para los botones. Crearás ambos métodos en los próximos pasos.

# --hints--

Debes llamar al método `fetchCardsData()` dentro de `constructor`.
...
Debes llamar al método `initializeEventListeners()` dentro del constructor.
