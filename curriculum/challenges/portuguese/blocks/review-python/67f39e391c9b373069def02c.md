...
- **Declaração de variáveis**: Para declarar uma variável, comece com o nome da variável, seguido pelo operador de atribuição (`=`) e, depois, pelo valor. Isto pode ser um número, string, boolean, etc. Aqui estão alguns exemplos:
...
A posição inicial representa o índice onde a extração deve começar. A posição de parada é onde o slice deve terminar.  This position is non-inclusive. A posição do passo representa o intervalo para incrementar no fatiamento. Aqui estão alguns exemplos:
...
- **`find()`**: Este método de string retorna o índice da primeira ocorrência de uma substring. Se nenhuma for encontrada, então `-1` é retornado:
...
- **`maketrans()`**: Este método de string é usado para criar uma tabela de mapeamentos de caractere 1 para 1 para tradução. Ele é frequentemente usado com o método `translate()` que aplica essa tabela a uma string e retorna o resultado traduzido.
...
- **`float()` Function**: Você pode usar esta função para converter um inteiro em um float.
...
- **`int()` Function**: Você pode usar esta função para converter um float em um inteiro.
...
- Se o item especificado no método `count()` não estiver presente na tupla, então o valor de retorno será `0`:
...
- Se nenhum argumento for passado para o `count()` método, então o Python retornará um `TypeError`.
...
- **Operador de Diferença (`-`)**: O operador de diferença `-` retorna um novo conjunto com os elementos do primeiro conjunto que não estão no outro conjunto.
...
- **IndexError**: Você receberá um `IndexError` se acessar um índice que não existe em uma lista ou outras sequências como tuple e string. Por exemplo, em uma string `Hello world`, o índice do último caractere é `10`. Se você acessar um caractere dessa forma, `greet = "hello world"; print(greet[11])`, você receberá um erro com a mensagem `IndexError: string index out of range`.
...
- **operações aritméticas como adicionar, subtração, multiplicação, divisão e outras**. `__add__()` é chamada para adicionar, `__sub__()` para subtração, `__mul__()` para multiplicação e `__truediv__()` para divisão.
...
- **Classes**: O modelo para criar objetos. Cada objeto criado a partir de uma classe possui atributos que definem dados e métodos que determinam os comportamentos do objeto.
...
- **Função `super()`**: Uma função que permite você **chamar** um método de uma classe pai, quando uma classe tem uma implementação diferente desse método, ou estende o método, sem duplicar código.
