---
id: 699b64096301fd75e3efeb1b
title: ステップ 7
challengeType: 1
dashedName: step-7
---

# --description--

もう一度コンソールを開くと、異なるエラーメッセージが表示されます。

```md
Property 'bio' is missing in type '{ username: string; age: number; isLoggedIn: false; }' but required in type '{ username: string; age: number; isLoggedIn: boolean; bio: string; }'.
```

前のレッスンで覚えているように、プロパティ名の横に`?`記号を使うことで、プロパティをオプショナルとしてマークできます。

```ts
const musician: {
  name: string;
  instrument: string;
  isActive: boolean;
  numberOfAlbums?: number;
} = {
  name: "Ludwig van Beethoven",
  instrument: "Piano",
  isActive: false
};
```

この例では、`numberOfAlbums`がオプショナルとしてマークされているため、この場合TypeScriptはエラーをスローしません。

`bio`プロパティをオプショナルとしてマークするように更新してください。

# --hints--

`bio`プロパティはオプショナルとしてマークされている必要があります。

```js
const explorer = await __helpers.Explorer(code);
const { profile } = explorer.variables;
const prop = { name: "bio", type: "string", isOptional: true };
assert.isTrue(profile.annotation.hasTypeProps(prop));
```

# --seed--

## --seed-contents--

```ts
const profile: {
  username: string;
  age: number;
  isLoggedIn: boolean;
--fcc-editable-region--
  bio: string
--fcc-editable-region--
} = {
  username: "codeLearner",
  age: 25,
  isLoggedIn: false
};

console.log(profile);
```
