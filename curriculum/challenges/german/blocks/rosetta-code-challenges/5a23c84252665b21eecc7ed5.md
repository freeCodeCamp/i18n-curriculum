...
Rittertour-Problem: Sie haben ein leeres `width` \* `height` Schachbrett, auf dem sich nur ein Springer auf einem Feld befindet. Der Springer muss eine Folge von erlaubten Zügen ausführen, sodass er jedes Feld des Schachbretts genau einmal besucht. Beachten Sie, dass es *nicht* erforderlich ist, dass die Tour „geschlossen“ ist; das heißt, der Springer muss nicht in einem Zug wieder an seine Startposition zurückkehren.

# --instructions--

Schreiben Sie eine Funktion, die `width` und `height` als Parameter erhält und die Anzahl der Startpositionen zurückgibt, von denen aus es möglich ist, die oben beschriebene Aufgabe zu erfüllen.
