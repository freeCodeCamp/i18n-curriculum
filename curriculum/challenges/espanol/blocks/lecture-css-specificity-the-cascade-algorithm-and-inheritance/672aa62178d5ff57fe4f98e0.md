...
En este ejemplo, el primer párrafo será rojo porque un selector `id` tiene una especificidad mayor que las clases y los selectores de tipo. Los elementos `.example-para` tendrán un color verde mientras que los párrafos restantes tendrán un color azul. 
...
Aquí tienes un ejemplo de cómo establecer el color de todos los elementos a rojo usando el selector `*`. Sin embargo, no verás ningún elemento rojo porque hay selectores de ID y de tipo que anulan esos estilos, lo que resalta la baja especificidad del selector `*`. 
