...
Adicione outro `label` com o texto `Personal Recommendation`. O atributo `for` deve estar definido como `"recommendation"`.

Abaixo do elemento `label`, adicione outro `input` de caixa de seleção com os atributos `id`, `name` e `value` definidos como `"recommendation"`.
