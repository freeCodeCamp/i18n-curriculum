...
要移除耳朵所有尖銳的邊緣，請將右下和左下邊框半徑各設為 `40%`，左上邊框半徑設為 `90px`，右上邊框半徑設為 `10px`。

# --hints--

你的 `.cat-left-inner-ear` 選擇器應該有一個 `border-bottom-right-radius` 屬性設定為 `40%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderBottomRightRadius === '40%')
```

你的 `.cat-left-inner-ear` 選擇器應該有一個 `border-bottom-left-radius` 屬性設定為 `40%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderBottomLeftRadius === '40%')
```

你的 `.cat-left-inner-ear` 選擇器應該有一個 `border-top-left-radius` 屬性設定為 `90px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderTopLeftRadius === '90px')
```

你的 `.cat-left-inner-ear` 選擇器應該有一個 `border-top-right-radius` 屬性設定為 `10px`。
