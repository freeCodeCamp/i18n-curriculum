...
Als Nächstes möchtest du das `#menu` horizontal zentrieren. Dies kannst du tun, indem du `margin-left`- und `margin-right`-Eigenschaften auf `auto` einstellst. Think of the margin as an invisible space around an element. Mit diesen beiden Margin-Eigenschaften zentrierst du das `#menu`-Element innerhalb des `body`-Elements.
