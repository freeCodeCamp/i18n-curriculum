...
`h1` セレクターを作成し、`font-family` プロパティを `Anton` に設定し、フォールバックとして `sans-serif` を指定してください。
...
`h1` セレクターには、`font-family` プロパティが `Anton` に設定され、フォールバックとして `sans-serif` が指定されているはずです。
