...
أسفل العنصر `.double-indent` الخاص بك، أضف عنصر `p` جديدًا يحتوي على النص `Includes 10g Added Sugars 20%`. يجب أن يكون عنصر `p` الجديد مزدوج المسافة البادئة أيضًا، وألا يحتوي على حدود سفلية. استخدم عنصر `span` لجعل `20%` عريضًا ومحاذيًا إلى اليمين.

ثم أنشئ فاصلًا آخر بعد ذلك العنصر `p`.

# --hints--

يجب أن تنشئ عنصرين جديدين `p` و `div` في نهاية العنصر `.daily-value` الخاص بك.
...
يجب أن يحتوي عنصر `p` الجديد على النص `Includes 10g Added Sugars 20%`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Includes 10g Added Sugars[\s|\n]+20%/));
```

يجب أن يحتوي عنصر `p` الجديد على الخاصية `class` مضبوطة على `double-indent no-divider`.
...
يجب أن يحتوي عنصر `span` على الخاصية `class` مضبوطة على `bold`.
...
يجب أن يحتوي عنصر `div` الجديد على الخاصية `class` مضبوطة على `divider`.
