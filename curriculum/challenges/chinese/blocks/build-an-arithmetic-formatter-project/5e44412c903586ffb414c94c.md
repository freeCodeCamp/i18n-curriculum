...
完成`arithmetic_arranger`函数，该函数接收作为算术问题的字符串列表，并返回垂直并排排列的问题。 该函数应该接受可选的第二个参数。 当第二个参数设置为 `True` 时，应显示答案。
...
- 将返回错误的情况：
    - 如果向函数提供的**问题太多**。限制是**五个**，任何更多的都会返回：
      `'Error: Too many problems.'`
    - 函数将接受的适当运算符是**加法**和**减法**。乘法和除法将返回错误。本要点中未提及的其他运算符不需要进行测试。返回的错误为：
...
`arithmetic_arranger(["44 + 815", "909 - 2", "45 + 43", "123 + 49", "888 + 40", "653 + 87"])`应返回`'Error: Too many problems.'`。
...
`arithmetic_arranger(["3 / 855", "3801 - 2", "45 + 43", "123 + 49"])`应返回`"Error: Operator must be '+' or '-'."`。
...
`arithmetic_arranger(["24 + 85215", "3801 - 2", "45 + 43", "123 + 49"])`应返回`'Error: Numbers cannot be more than four digits.'`。
...
`arithmetic_arranger(["98 + 3g5", "3801 - 2", "45 + 43", "123 + 49"])`应返回`'Error: Numbers must only contain digits.'`。
