...
`input` と `textarea` の要素を背景テーマに馴染ませるために、それらの `background-color` を `#0a0a23` に設定してください。それから、`1px`、`solid` のボーダーを `#0a0a23` の色で付けてください。

# --hints--

カンマ区切りの要素セレクターを使って `input` と `textarea` の要素を選択してください。
...
`input` と `textarea` の要素に `background-color` を `#0a0a23` で付けてください。
...
`input` と `textarea` の要素に `1px`、`solid` のボーダーを `#0a0a23` の色で付けてください。
