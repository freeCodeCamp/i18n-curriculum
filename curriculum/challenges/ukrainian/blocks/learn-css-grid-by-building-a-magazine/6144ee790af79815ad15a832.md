...
Тепер ви готові почати створювати макет сітки. CSS Grid пропонує двовимірний макет на основі сітки, що дозволяє центрувати елементи по горизонталі та вертикалі, при цьому зберігаючи контроль для таких дій, як накладання елементів.

Почніть зі створення селектора `main` і надайте йому властивість `display` зі значенням `grid`.
