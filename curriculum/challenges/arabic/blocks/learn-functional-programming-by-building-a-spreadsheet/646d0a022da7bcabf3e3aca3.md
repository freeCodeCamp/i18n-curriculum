...
مفهوم إرجاع دالة داخل دالة أخرى يُسمى <dfn>التطبيق الجزئي</dfn>. تتيح لك هذه الطريقة إنشاء متغير يحمل دالة ليتم استدعاؤها لاحقًا، مع مرجع إلى معلمات استدعاء الدالة الخارجية.
...
`innerOne` ستكون هي دالتك `inner`، حيث يتم تعيين `num` إلى `1`، و`final` سيكون له قيمة الخلية التي تحمل `id` يساوي `"A1"`. هذا ممكن لأن الدوال لديها وصول إلى جميع المتغيرات التي تم إعلانها عند إنشائها. يُسمى هذا بـ <dfn>إغلاق (closure)</dfn>.

ستحصل على مزيد من التدريب على هذا. أعلن دالة باسم `addCharacters` تأخذ معلمة `character1`.

# --hints--

يجب أن تعلن متغيرًا باسم `addCharacters`.
...
يجب أن تستخدم `const` لإعلان متغير `addCharacters`.
...
يجب أن يكون متغير `addCharacters` دالة سهمية.
...
يجب ألا تستخدم دالة `addCharacters` إرجاعًا ضمنيًا.
...
يجب أن تحتوي دالة `addCharacters` على معلمة `character1`.
