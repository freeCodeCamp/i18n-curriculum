...
L'oggetto `spreadsheetFunctions` dovrebbe avere una proprietà `random`.

```js
assert.property(spreadsheetFunctions, "random");
```

La proprietà `random` dovrebbe essere una funzione.

```js
assert.isFunction(spreadsheetFunctions.random);
```

La funzione `random` dovrebbe restituire un numero casuale tra i primi due numeri nell'array.
...
La funzione `random` dovrebbe restituire un numero intero.
