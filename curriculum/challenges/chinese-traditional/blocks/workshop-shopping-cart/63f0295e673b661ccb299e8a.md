...
你需要一種方法來訪問購物車中的商品總數。 最好的方法是向 `ShoppingCart` 類添加另一種方法，而不是直接訪問 `items` 數組。

向 `ShoppingCart` 類添加 `getCounts` 方法。 它應該返回 `items` 數組中的項目數。

# --hints--

你的 `ShoppingCart` 類應該有一個 `getCounts` 方法。
...
你的 `getCounts` 方法應該返回 `items` 數組中的項目數。
