...
首先建立一個名為 `playerScoreSpanElement` 的變數，並將其指定為 id 為 `player-score` 的元素。

然後創建一個名為 `computerScoreSpanElement` 的變數，並將其指定為具有 id `computer-score` 的元素的值。

最後，創建一個名為 `roundResultsMsg` 的變數，並將其指定為具有 id `results-msg` 的元素的值。

# --hints--

你應該有一個名為 `playerScoreSpanElement` 的變數。

```js
assert.isNotNull(playerScoreSpanElement);
```

你應該將 id 為 `player-score` 的元素的值指定給 `playerScoreSpanElement`。

```js
assert.equal(playerScoreSpanElement, document.getElementById("player-score"));
```

你應該有一個名為 `computerScoreSpanElement` 的變數。

```js
assert.isNotNull(computerScoreSpanElement);
```

你應該將 id 為 `computer-score` 的元素值指定給 `computerScoreSpanElement`。

```js
assert.equal(computerScoreSpanElement, document.getElementById("computer-score"));
```

你應該有一個名為 `roundResultsMsg` 的變數。

```js
assert.isNotNull(roundResultsMsg);
```

你應該將 id 為 `results-msg` 的元素的值指定給 `roundResultsMsg`。
