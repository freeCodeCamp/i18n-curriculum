...
`lunchCalories` 変数を宣言し、`getCaloriesFromInputs` を使って `lunchNumberInputs` を呼び出した結果を代入してください。

# --hints--

あなたの `calculateCalories` 関数は `lunchCalories` 変数を持っているはずです。

```js
assert.match(calculateCalories.toString(), /lunchCalories\s*=/);
```

`getCaloriesFromInputs` を引数にして `lunchNumberInputs` を呼び出してください。

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*lunchNumberInputs\s*\)/);
```

`getCaloriesFromInputs` の結果を `lunchCalories` に代入してください。
