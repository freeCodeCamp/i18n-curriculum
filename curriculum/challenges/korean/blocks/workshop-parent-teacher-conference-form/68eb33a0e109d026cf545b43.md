...
내부 원을 위치시키고 처음에는 보이지 않게 만들어야 합니다.

여전히 `.contact-method-radio-btn::before`를 대상으로 하여 `transform`을 `translate(3px, 3px) scale(0)`로 설정하세요. `translate` 함수는 원을 약간 이동시키고, `scale(0)`는 크기를 0으로 줄여 기본적으로 숨깁니다.

또한, 나타날 때 부드러운 애니메이션을 만들기 위해 `transition`에 `all 0.3s ease-in`를 추가하세요. 이는 스타일 변경이 0.3초 동안 천천히 시작해 점점 빨라지면서 애니메이션된다는 뜻입니다.

# --hints--

계속해서 `.contact-method-radio-btn::before` 가상 요소를 대상으로 해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

`transform` 속성을 `translate(3px, 3px) scale(0)`로 설정하세요.
...
`transition` 속성을 `all 0.3s ease-in`로 설정하세요.
