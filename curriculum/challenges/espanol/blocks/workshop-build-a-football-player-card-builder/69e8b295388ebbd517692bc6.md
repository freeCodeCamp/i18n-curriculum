...
La foto del jugador se carga desde una URL codificada. Reemplaza el valor del atributo `src` `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` por `player.imageUrl`.

También reemplaza el texto codificado `alt` por un valor dinámico usando una plantilla literal con `player.name` y el texto `headshot`.

Ten en cuenta que cuando usas una expresión de JavaScript como valor de un atributo en TSX, usas llaves `{}` en lugar de comillas. Por ejemplo `src={player.imageUrl}`, no `src="{player.imageUrl}"`.

# --hints--

Debes reemplazar la URL de la imagen codificada por `player.imageUrl` como el atributo `src`.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

Debes reemplazar el texto codificado `alt` por una plantilla literal usando `player.name` y el texto `headshot`.
