...
Ви повинні надати `.blush` властивість `top` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.top, '65%');
```

Ви повинні надати `.blush` властивість `border-radius` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.
