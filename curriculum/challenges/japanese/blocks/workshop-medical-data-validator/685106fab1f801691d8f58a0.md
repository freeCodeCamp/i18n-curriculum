...
`gender` 辞書にもう1つのキー `constraints` を追加してください。前のステップで書いた式の形式に従い、`gender` が文字列であることを確認します。次に、`and` 演算子を使って、小文字の `gender` が `('male', 'female')` に含まれているかどうかをチェックしてください。

# --hints--

あなたの `constraints` 辞書にはキー `gender` があるはずです。
...
あなたの `gender` 辞書の `constraints` キーは `isinstance(gender, str) and gender.lower() in ('male', 'female')` の値を持つべきです。
