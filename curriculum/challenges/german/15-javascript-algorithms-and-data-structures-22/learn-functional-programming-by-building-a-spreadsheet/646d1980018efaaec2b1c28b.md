...
Verwende die gleiche Syntax wie deine `addCharacters`-Funktion, um deine `elemValue`-Funktion zu aktualisieren. Sie sollte nicht mehr `inner` deklarieren, sondern die Funktion implizit zurückgeben.

# --hints--

Deine `elemValue`-Funktion sollte eine implizite Rückgabe verwenden.
...
Deine `elemValue`-Funktion sollte implizit eine Arrow-Funktion mit einem `character`-Parameter zurückgeben.
...
Deine innere Arrow-Funktion sollte eine implizite Rückgabe verwenden.
...
Deine innere Arrow-Funktion sollte da Ergebnis des Aufrufs von `idToText()` mit `character + num` als Argument zurückgeben.
