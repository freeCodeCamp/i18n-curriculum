...
يجب عليك استخدام منتقي `.penguin-head`.

```js
assert.match(code, /\.penguin-head\s*\{/);
```

يجب عليك إعطاء `.penguin-head` خاصية `width` بقيمة `50%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.width, '50%');
```

يجب عليك إعطاء `.penguin-head` خاصية `height` بقيمة `45%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.height, '45%');
```

يجب عليك إعطاء `.penguin-head` خاصية `background` من `linear-gradient(45deg, gray, rgb(239, 240, 228))`.
