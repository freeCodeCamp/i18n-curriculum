...
这里发生了许多事情。 首先，这个例子使用反引号（`` ` ``），而不是引号（`'` 或 `"`）来包裹字符串。 其次，注意无论在代码还是输出中，字符串都是多行的。 这样就省去了在字符串中插入 `\n`。 上面使用的 `${variable}` 语法是一个占位符。 基本上，你将不再需要使用 `+` 运算符来拼接字符串。 要向字符串中添加变量，你只需把变量放入模板字符串中，并用 `${` 和 `}` 将其包裹起来。 同样，你也可以在字符串字面量中包含其他表达式，例如 `${a + b}`。 这种创建字符串的新方式让你能更灵活地创建健壮的字符串。
