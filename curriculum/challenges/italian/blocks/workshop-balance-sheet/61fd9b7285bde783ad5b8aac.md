...
All'interno del `tbody`, aggiungi un `tr` con l'attributo `class` impostato su `total`. Al suo interno, aggiungi un `th` con il testo `Total Net Worth`, e avvolgi `Net Worth` in un `span` con l'attributo `class` impostato su `sr-only`.

Poi aggiungi tre elementi `td`, dando al terzo un attributo `class` impostato su `current`, e assegnando a ciascuno il seguente testo: `$-171`, `$136`, `$334`.

# --hints--

Il tuo elemento `tbody` dovrebbe avere un elemento `tr`.
...
Il tuo elemento `tr` dovrebbe avere l'attributo `class` impostato su `total`.
...
Il tuo `tr` dovrebbe avere un elemento `th`.
...
Il tuo elemento `th` dovrebbe avere il testo `Total Net Worth`.
...
Dovresti avvolgere il testo `Net Worth` in un elemento `span`.
...
Il tuo primo elemento `td` dovrebbe avere il testo `$-171`.
...
Il tuo secondo elemento `td` dovrebbe avere il testo `$136`.
...
Il tuo terzo elemento `td` dovrebbe avere il testo `$334`.
