...
Então, no elemento de `input` do checkbox, adicione um atributo `checked` e defina-o como `isChecked`. Embora não seja estritamente necessário definir o atributo `checked`, é uma boa prática garantir que cada checkbox reflita o estado atual do array `selectedItems`.

# --hints--

Você deve adicionar um atributo `checked` ao elemento `input` do checkbox.
...
Você deve definir o atributo `checked` para `isChecked`.
