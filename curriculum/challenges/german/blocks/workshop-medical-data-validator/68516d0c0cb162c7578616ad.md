...
Fügen Sie dem `last_visit_id`-Dictionary einen letzten Schlüssel `constraints` hinzu. Verwenden Sie für dessen Wert `isinstance`, um zu überprüfen, ob `last_visit_id` ein String ist.

# --hints--

Ihr `constraints`-Dictionary sollte einen Schlüssel `last_visit_id` enthalten.
...
Der `last_visit_id`-Schlüssel Ihres `constraints`-Dictionaries sollte den Wert von `isinstance(last_visit_id, str)` haben.
