...
1. `lang`가 `en`로 설정된 `html` 요소가 있어야 합니다.
...
1. 재료 섹션을 위한 텍스트 `Ingredients`가 있는 `h2` 요소가 하나 있어야 합니다.
1. 첫 번째 `h2` 요소 아래에 재료를 나열하는 최소 네 개의 항목(`li` 요소)을 포함한 순서 없는 목록(`ul` 요소)이 있어야 합니다.
1. 지침 섹션을 위한 텍스트 `Instructions`가 있는 두 번째 `h2` 요소가 있어야 합니다.
1. 두 번째 `h2` 아래에 레시피 단계를 순서대로 나열하는 최소 네 개의 항목이 있는 순서 있는 목록(`ol` 요소)이 있어야 합니다.
1. 유효한 이미지가 설정된 `src` 속성을 가진 `img` 요소가 하나 있어야 하며, 원한다면 `https://cdn.freecodecamp.org/curriculum/labs/recipe.jpg`를 사용할 수 있고, 이미지를 설명하는 `alt` 속성도 있어야 합니다.
...
`lang`가 `en`로 설정된 `html` 요소가 있어야 합니다.
