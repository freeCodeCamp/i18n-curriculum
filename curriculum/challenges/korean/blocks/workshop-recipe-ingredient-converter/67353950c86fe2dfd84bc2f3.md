...
계산할 인분 수에 맞게 조정해야 합니다. 또 다른 커링된 함수를 사용할 좋은 기회입니다.

`adjustForServings` 함수를 선언하세요. 이 함수는 `baseQuantity` 인수를 받습니다. 그리고 `newServings` 인수를 받는 두 번째 함수를 반환해야 합니다.

내부 함수는 `baseQuantity`과 `newServings`를 곱한 결과를 반환해야 합니다.

# --hints--

`adjustForServings` 함수를 선언해야 합니다.

```js
assert.isFunction(adjustForServings);
```

`adjustForServings` 함수는 `baseQuantity` 인수를 받아야 합니다.

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

`adjustForServings` 함수는 함수를 반환해야 합니다.

```js
assert.isFunction(adjustForServings?.());
```

내부 함수는 `newServings` 매개변수를 가져야 합니다.

```js
assert.match(adjustForServings?.()?.toString(), /\(newServings\)/);
```

내부 함수는 `baseQuantity`과 `newServings`을 곱한 결과를 반환해야 합니다.
