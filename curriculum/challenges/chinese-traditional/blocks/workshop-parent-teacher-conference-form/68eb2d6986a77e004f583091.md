...
現在，是時候自訂圓鈕了。預設情況下，圓鈕具有原生的瀏覽器外觀。你想要移除這個外觀，以便你可以創建訂製的樣式。

選取具有類別 `contact-method-radio-btn` 的元素。將 `appearance` 設為 `none`。

# --hints--

你應該標的具有類別 `contact-method-radio-btn` 的元素。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

將 `appearance` 屬性設為 `none`。
