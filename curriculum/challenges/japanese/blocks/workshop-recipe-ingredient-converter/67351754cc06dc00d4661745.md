...
このワークショップでは、これまでのレッスンで学んだ関数型プログラミングの概念を使って、材料変換アプリを作成します。

HTMLとCSSはすでに用意されています。

まず、空の`conversionTable`オブジェクトを宣言してください。

# --hints--

`conversionTable`変数を宣言してください。

```js
assert.isDefined(conversionTable);
```

`conversionTable`変数はオブジェクトにしてください。

```js
assert.isObject(conversionTable);
```

`conversionTable`オブジェクトは空にしてください。
