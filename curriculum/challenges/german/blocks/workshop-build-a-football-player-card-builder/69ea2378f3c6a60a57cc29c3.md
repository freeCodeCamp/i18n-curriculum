...
Durchlaufen Sie nun das `POSITIONS`-Array mit einer Pfeilfunktion, die jeden Positions-String als `pos` erhält und ein `option`-Element zurückgibt. Die Option sollte einen `key` von `pos` und den Wert `pos` haben. Außerdem soll `pos` als Text jedes generierten `option` angezeigt werden.

Sie können jeden beliebigen Namen als Parameter für Ihre map-Funktion verwenden, es muss nicht `pos` sein.

# --hints--

Ihr Positions-`select` sollte `option`-Elemente enthalten, die aus dem `POSITIONS`-Array gemappt wurden.
...
Der `value` jeder Option sollte mit dem angezeigten Text übereinstimmen und eine gültige Position sein.
