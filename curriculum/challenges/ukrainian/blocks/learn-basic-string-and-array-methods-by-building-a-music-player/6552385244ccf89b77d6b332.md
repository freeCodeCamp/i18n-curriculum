...
Вам потрібно працювати з відтворенням наступної та попередньої пісні. Для цього вам знадобляться функції `playNextSong` і `playPreviousSong`.

Використайте `const` та стрілковий синтаксис, щоб створити порожню функцію `playNextSong`.

# --hints--

Ви повинні використати `const`, щоб створити порожню функцію з ім’ям `playNextSong`.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` має бути функцією.

```js
assert.isFunction(playNextSong)
```

Ваша функція `playNextSong` має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Ваша функція `playNextSong` має бути порожньою.
