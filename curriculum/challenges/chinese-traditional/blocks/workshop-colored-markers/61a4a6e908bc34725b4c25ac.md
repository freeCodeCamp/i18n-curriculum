...
接著，將紅色的 color-stop 設為 `0%`，綠色的 color-stop 設為 `50%`，藍色的 color-stop 設為 `100%`。

# --hints--

你的 `.red` CSS 規則應該有一個 `background` 屬性，設定為 `linear-gradient(180deg, rgb(255, 0, 0) 0%, rgb(0, 255, 0) 50%, rgb(0, 0, 255) 100%)`。
