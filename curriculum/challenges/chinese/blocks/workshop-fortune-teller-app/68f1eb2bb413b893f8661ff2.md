...
此时，你应该能够在点击卡片后看到每张卡片显示的运势。剩下要做的就是让“新阅读”按钮生效。

在 `newReading` 方法内，调用 `showElements()` 和 `hideElements()`。

# --hints--

你应该在 `showElements` 方法内调用 `newReading` 函数。

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

你应该在 `hideElements` 方法内调用 `newReading` 函数。
