...
미션 컨트롤에서는 승무원 요약을 기록하는 함수를 하나 더 필요로 합니다. `printCrewSummary`라는 이름의 빈 함수를 만들고 `crew` 매개변수를 받으세요.

# --hints--

`printCrewSummary`라는 이름의 함수를 만들어야 합니다.

```js
assert.isFunction(printCrewSummary);
```

`printCrewSummary` 함수는 `crew` 매개변수를 가져야 합니다.
