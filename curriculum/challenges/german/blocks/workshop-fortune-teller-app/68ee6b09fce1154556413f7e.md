...
Fügen Sie innerhalb der `initializeEventListeners`-Methode einen `click`-Ereignis-Listener an `singleCardBtn` an. Rufen Sie innerhalb des Event-Handlers die `singleCardSelected`-Methode auf. Denken Sie daran, `singleCardBtn` aus `elements` zu holen.
