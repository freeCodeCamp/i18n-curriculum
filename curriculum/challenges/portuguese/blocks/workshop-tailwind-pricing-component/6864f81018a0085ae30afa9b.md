...
A marca de seleção em cada `span` no início de um item da lista é decorativa.Adicione `aria-hidden` a esses elementos `span` para ocultá-los dos leitores de tela.

Além disso, atribua as classes `text-green-700` e `mr-2` a cada um dos elementos `span`.

# --hints--

Seus dois elementos `span` devem ter o atributo `aria-hidden`.
...
Seus dois elementos `span` devem ter a classe `text-green-700`.
...
Seus dois elementos `span` devem ter a classe `mr-2`.
