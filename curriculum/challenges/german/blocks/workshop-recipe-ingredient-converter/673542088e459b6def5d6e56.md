...
Sie benötigen eine Funktion, die tatsächlich das DOM aktualisiert. Deklarieren Sie eine `updateResultsList`-Funktion.

Zu Beginn sollte die Funktion das HTML des `resultList`-Elements löschen.

# --hints--

Sie sollten eine `updateResultsList`-Funktion definieren.

```js
assert.isFunction(updateResultsList);
```

Ihre `updateResultsList`-Funktion sollte den `resultList.innerHTML` auf einen leeren String setzen.
