...
Erstellen Sie innerhalb des `allSongs`-Arrays ein Objekt mit den folgenden Eigenschaften und Werten:
...
Ihr `allSongs`-Array sollte einen Wert enthalten.

```js
assert.equal(allSongs.length, 1);
```

Ihr `allSongs`-Array sollte als ersten Wert ein Objekt enthalten.

```js
assert.isObject(allSongs[0]);
```

Ihr `allSongs`-Array sollte ein Objekt mit einer `id`-Eigenschaft enthalten, die auf die Zahl `0` gesetzt ist.

```js
assert.strictEqual(allSongs[0].id, 0);
```

Ihr `allSongs`-Array sollte ein Objekt mit einer `title`-Eigenschaft enthalten, die auf den String `"Scratching The Surface"` gesetzt ist.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Ihr `allSongs`-Array sollte ein Objekt mit einer `artist`-Eigenschaft enthalten, die auf den String `"Quincy Larson"` gesetzt ist.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Ihr `allSongs`-Array sollte ein Objekt mit einer `duration`-Eigenschaft enthalten, die auf den String `"4:25"` gesetzt ist.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Ihr `allSongs`-Array sollte ein Objekt mit einer `src`-Eigenschaft enthalten, die auf den String `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"` gesetzt ist.
