...
Come accennato in precedenza, quando l'utente modifica la nota e poi clicca all'esterno della nota, dovrebbe comparire un messaggio che indica che la nota è stata salvata.

Per farlo, imposta `statusEl.textContent` sulla stringa `"Note saved successfully!"`.

Ora, quando lo proverai, dovresti vedere quel messaggio comparire sullo schermo.

# --hints--

Quando l'utente modifica una nota, il messaggio di stato dovrebbe essere aggiornato per indicare che la nota è stata salvata.
