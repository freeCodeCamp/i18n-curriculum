...
تسترجع الطريقة <dfn>find()</dfn> أول عنصر داخل مصفوفة يفي بالشروط المحددة في دالة الاستدعاء الراجعة المقدمة. إذا لم يلبِ أي عنصر الشرط، تُرجع الطريقة القيمة `undefined`.

في المثال أدناه، تُستخدم الطريقة `find()` للعثور على أول رقم أكبر من 25:
...
استخدم `const` لإنشاء متغير باسم `song` وعيّن له نتيجة استدعاء الطريقة `find()` على مصفوفة `userData?.songs`. استخدم `song` كمعلمة في دالة الاستدعاء الراجعة لـ `find()` وتحقق مما إذا كان `song.id` يساوي `id` بدقة.

سيؤدي هذا إلى التكرار خلال مصفوفة `userData?.songs`، بحثًا عن أغنية تطابق الـ `id` الممرر إلى دالة `playSong`.

# --hints--

يجب أن تستخدم الطريقة `find` على `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

يجب أن تحتوي الطريقة `find` على `song` كمعلمة في دالة الاستدعاء الراجعة السهمية الخاصة بها.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

يجب أن تستخدم الطريقة `find` المساواة الصارمة للتحقق مما إذا كان `song.id` يساوي `id`.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

يجب تعيين نتيجة الطريقة `find` إلى ثابت باسم `song`.
