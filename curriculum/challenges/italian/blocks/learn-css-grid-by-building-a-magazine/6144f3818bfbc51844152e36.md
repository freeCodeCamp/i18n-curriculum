...
Usa la funzione `minmax` per rendere le tue colonne responsivi su qualsiasi dispositivo. La funzione `minmax` prende due argomenti, il primo essendo il valore minimo e il secondo il massimo. Questi valori possono essere una lunghezza, una percentuale, `fr` o anche una parola chiave come `max-content`.

Avvolgi ciascuno dei valori già definiti della proprietà `grid-template-columns` in una funzione `minmax`, usando ogni valore come secondo argomento. Il primo argomento dovrebbe essere rispettivamente `2rem`, `min-content` e `2rem`.

# --hints--

Il tuo selettore `main` dovrebbe avere una proprietà `grid-template-columns` impostata su `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
