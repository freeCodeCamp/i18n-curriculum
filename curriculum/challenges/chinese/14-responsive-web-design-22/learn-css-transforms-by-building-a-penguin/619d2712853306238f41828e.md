...
`.penguin-body::before` 的 `background-color` 值应该为 `var(--penguin-skin)`, but found `--fcc-actual--`。
...
`.arm` 的 `background` 值应该为 `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`。
