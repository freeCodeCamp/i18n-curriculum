...
現在，在 `renderCard` 函式的模板字串中，創建一個 `div` 元素。在 `div` 裡，添加一個輸出 `h2` 的 `drawingType` 元素，以及一個具有類別 `figure` 和屬性 `card_container ${isReversed ? "reversed-card" : ""}` 的 `data-id="${shortName}"` 元素。

# --hints--

你應該在 `div` 函式的模板字串中有一個 `renderCard` 元素。
...
你應該在 `h2` 元素內有一個插入 `drawingType` 的 `div` 元素。
...
你的 `figure` 元素應該有類別 `card_container`，並使用三元運算子有條件地添加 `reversed-card`。
...
你的 `figure` 元素應該有一個屬性 `data-id`，其值設為 `${shortName}`。
