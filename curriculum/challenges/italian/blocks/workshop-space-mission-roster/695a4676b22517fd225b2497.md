...
Mission Control richiede un'altra funzione per registrare un riepilogo del tuo equipaggio. Crea una funzione vuota chiamata `printCrewSummary` che accetta un parametro `crew`.

# --hints--

Dovresti creare una funzione chiamata `printCrewSummary`.

```js
assert.isFunction(printCrewSummary);
```

La tua funzione `printCrewSummary` dovrebbe avere un parametro `crew`.
