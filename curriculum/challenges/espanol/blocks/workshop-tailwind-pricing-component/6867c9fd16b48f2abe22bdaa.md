...
El siguiente elemento a estilizar es el texto del precio, incluyendo el `span` dentro de él. Dale al elemento `p` del precio las clases `text-4xl`, `font-bold` y `text-gray-900`.

# --hints--

Tu elemento `p` debe tener la clase `text-4xl`.
...
Tu elemento `p` debe tener la clase `font-bold`.
...
Tu elemento `p` debe tener la clase `text-gray-900`.
