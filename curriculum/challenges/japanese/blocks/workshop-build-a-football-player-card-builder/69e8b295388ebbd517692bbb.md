...
`card-image-wrap` 要素の後に、`div` の `className` を持つ `card-name-strip` を追加してください。その中に、`span` の `className` とテキスト `card-name` を持つ `PELE` を追加してください。

# --hints--

`div` の `className` を持つ `card-name-strip` を追加してください。

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

`span` の中に、`className` の `card-name` を持つ `.card-name-strip` を追加してください。

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

`.card-name` スパンにはテキスト `PELE` を設定してください。
