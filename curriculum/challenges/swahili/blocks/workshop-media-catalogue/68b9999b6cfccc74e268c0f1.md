...
Kabla ya mzunguko wako wa `for`, tengeneza tamko la `if` ambalo linaongeza mfuatano wa herufi `=== MOVIES ===` (unaomalizika na tabia ya newline) kwenye thamani ya sasa ya `result` wakati orodha ya `movies` si tupu.

# --hints--

Unapaswa kuwa na tamko la `if` ndani ya njia yako ya `__str__` ambalo linakagua kama orodha ya `movies` si tupu.
...
Unapaswa kuwa na `result += '=== MOVIES ===\n'` ndani ya tamko lako la `if`.
