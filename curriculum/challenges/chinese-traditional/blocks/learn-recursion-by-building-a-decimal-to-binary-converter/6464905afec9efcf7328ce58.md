...
接著，使用 `.forEach()` 方法來迴圈瀏覽 `animationData` 陣列。對於 `.forEach()` 方法的回呼函式，將 `obj` 作為參數傳入，但目前先將回呼函式的內容留空。

# --hints--

你應該使用 `.forEach()` 方法來迴圈瀏覽 `animationData` 陣列。

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\(/);
```

你應該將 `obj` 作為參數傳入 `.forEach()` 方法的回呼函式中。
