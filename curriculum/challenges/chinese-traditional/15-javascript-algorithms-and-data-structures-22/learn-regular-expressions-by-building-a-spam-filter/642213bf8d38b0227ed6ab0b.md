...
你的下一個正則表達式將查找類似 `free money` 的字符串。 首先聲明一個 `freeRegex` 變量併爲其分配一個與字符串 `free money` 匹配的正則表達式。 請記住不區分大小寫。

# --hints--

你應該使用 `const` 聲明一個 `freeRegex` 變量。

```js
assert.match(code, /const\s+freeRegex\s*=/);
```

你的 `freeRegex` 變量應該是一個正則表達式。
