...
각 포럼 주제에는 Python이나 JavaScript 같은 카테고리가 포함되어 있습니다. 다음 몇 단계에서는 모든 포럼 카테고리와 `classNames` 스타일링을 담는 카테고리 객체를 만들 것입니다.

먼저 `allCategories`이라는 새 상수를 만들고 빈 객체 값을 할당하세요.

# --hints--

`allCategories`라는 상수가 있어야 합니다.

```js
assert.match(code, /const\s+allCategories\s*=/)
```

`allCategories` 변수에 빈 객체를 할당해야 합니다.
