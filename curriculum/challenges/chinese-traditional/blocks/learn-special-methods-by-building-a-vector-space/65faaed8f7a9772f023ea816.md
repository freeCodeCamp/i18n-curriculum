...
向量可以乘以一個*純量*，即一個乘以每個單一組件的數字。純量乘法的結果是一個與原始向量方向相同但大小不同的向量。

透過檢查 `other` 是否為 `int` 或 `float` 來實作純量乘法。如果是，傳回目前類別的新實體，其起始向量的每個組件皆乘以 `other`。這將是純量乘法所產生的向量。

確保這些方法能夠用於計算任意維度向量的純量乘法。

# --hints--

你的方法應該只在 `other` 的型別為 `int` 或 `float` 時，傳回目前類別的新實體。
