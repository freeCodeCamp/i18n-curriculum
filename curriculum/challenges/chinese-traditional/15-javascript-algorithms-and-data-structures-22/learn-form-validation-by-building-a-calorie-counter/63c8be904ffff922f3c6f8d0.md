...
還記得你之前編寫過一個函數來清理用戶的輸入嗎？ 你需要在這裏使用該功能。

將你的 `currVal` 聲明更新爲使用 `item.value` 調用 `cleanInputString` 的結果。

# --hints--

你應該在 `getCaloriesFromInputs` 函數中調用 `cleanInputString` 函數。

```js
assert.match(getCaloriesFromInputs.toString(), /cleanInputString\(/);
```

你應該將 `item.value` 作爲 `cleanInputString` 函數的參數傳遞。

```js
assert.match(getCaloriesFromInputs.toString(), /cleanInputString\(\s*item\.value\s*\)/);
```

你應該將 `cleanInputString` 調用的結果分配給 `currVal` 變量。
