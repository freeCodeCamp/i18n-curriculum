...
Agora, é hora de criar a função que será responsável por exibir os autores na página. 

Comece criando uma função chamada `displayAuthors` que recebe um parâmetro `authors`. Sua função deve então percorrer o array `authors` e adicionar um cartão para cada autor ao elemento `authorContainer`. 

O cartão deve ter a seguinte estrutura:
...
- Você pode usar qualquer tipo de loop para iterar sobre o array `authors`. Ex. `for`, `forEach`, `map`, etc.
- Você deve usar a propriedade `innerHTML` para adicionar o card ao `authorContainer` dentro do loop. Você também precisará usar a sintaxe de template literal para adicionar a marcação.
...
Quando a função `displayAuthors` é chamada com um array de autores, ela deve adicionar um cartão para cada autor ao elemento `#authorContainer`.
