...
Створіть ще один елемент `p` під вашим елементом `.first-paragraph` і додайте йому такий текст:
...
Ви маєте створити другий елемент `p` всередині вашого елемента `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 2)
```

Ваш другий елемент `p` повинен містити наданий текст.
