...
year 欄位以字串形式傳入，但你需要將它轉成數字以便後續計算。使用 `parseInt()` 來轉換它。

year 也可能是空白，因此你需要一個備用值。三元運算子 `condition ? valueIfTrue : valueIfFalse` 根據條件傳回兩個值中的其中一個。

將 return 述語中的 `year` 屬性更新為：
