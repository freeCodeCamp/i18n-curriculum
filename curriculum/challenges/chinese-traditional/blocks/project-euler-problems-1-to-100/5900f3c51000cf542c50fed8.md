...
可以表示為質數平方、質數立方和質數四次方和的最小數字是 `28`。事實上，低於五十的數字中，正好有四個可以用這種方式表示：
...
在 `n` 以下有多少數字可以表示為質數平方、質數立方和質數四次方的和？

# --hints--

`primePowerTriples(50)` 應該傳回一個數字。

```js
assert(typeof primePowerTriples(50) === 'number');
```

`primePowerTriples(50)` 應該傳回 `4`。

```js
assert.strictEqual(primePowerTriples(50), 4);
```

`primePowerTriples(10035)` 應該傳回 `684`。

```js
assert.strictEqual(primePowerTriples(10035), 684);
```

`primePowerTriples(500000)` 應該傳回 `18899`。

```js
assert.strictEqual(primePowerTriples(500000), 18899);
```

`primePowerTriples(5000000)` 應該傳回 `138932`。

```js
assert.strictEqual(primePowerTriples(5000000), 138932);
```

`primePowerTriples(50000000)` 應該傳回 `1097343`。
