...
在先前的專案中，你學會了如何使用 `shift()` 方法從陣列中移除第一個元素，如下所示：
...
使用 `shift()` 方法從 `inventory` 陣列中取出第一個元素，並將它指定到你的 `currentWeapon` 變數。

# --hints--

你的 `sellWeapon` 函式應該使用 `shift()` 方法。

```js
assert.match(sellWeapon.toString(), /shift\(\s*\)/);
```

你應該在 `inventory` 陣列上使用 `shift()` 方法。

```js
assert.match(sellWeapon.toString(), /inventory\.shift\(\s*\)/);
```

你應該將 `inventory.shift()` 的值指定給你的 `currentWeapon` 變數。
