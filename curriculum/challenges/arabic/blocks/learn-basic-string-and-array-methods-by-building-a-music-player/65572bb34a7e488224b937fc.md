...
تحتاج إلى إضافة `resetText` إلى عنصر `resetButton` كطفل، وأيضًا إضافة `resetButton` إلى عنصر `playlistSongs` كطفل. لهذا، هناك طريقة `appendChild()` لاستخدامها.

تسمح لك `appendChild()` بإضافة عقدة أو عنصر كطفل لعنصر آخر. في المثال أدناه، سيتم إرفاق النص "Click me" بالزر:
...
استخدم `appendChild()` لإرفاق `resetText` بعنصر `resetButton`، و`resetButton` بعنصر `playlistSongs`.

# --hints--

يجب أن تستخدم طريقة `appendChild()` على `resetButton`.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

يجب أن تمرر `resetText` كقيمة لأول استدعاء `appendChild()` لديك.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

يجب أن تستخدم طريقة `appendChild()` على `playlistSongs`.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

يجب أن تمرر `resetButton` كقيمة لاستدعاء `appendChild()` الثاني.
