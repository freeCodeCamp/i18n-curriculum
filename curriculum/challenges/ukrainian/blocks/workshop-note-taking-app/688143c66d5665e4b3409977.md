...
Здається, все працює, крім однієї невеликої проблеми. Якщо ви редагуєте нотатку, а потім залишаєте її, ви повинні побачити повідомлення `"Note saved successfully!"`. Але якщо ви спробуєте знову редагувати нотатку, повідомлення все ще відображається.

Бажана поведінка — щоб повідомлення зникало, коли користувач ставить фокус на елемент. Для цього можна використати подію `focus`.
...
Приєднайте `addEventListener` до змінної `noteEl`. Першим аргументом має бути подія `"focus"`, а другим — функція зворотного виклику. Усередині цієї функції зворотного виклику встановіть `statusEl.textContent` у порожній рядок. Це скине живу область, коли нотатка отримує фокус.

І з цією останньою зміною ваше практичне заняття завершене!

# --hints--

Коли користувач ставить фокус на нотатку, ви повинні встановити `statusEl.textContent` у порожній рядок.
