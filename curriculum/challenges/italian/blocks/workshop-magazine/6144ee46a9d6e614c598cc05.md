...
Crea un selettore `a` e dagli una proprietà `text-decoration` impostata su `none` e una proprietà `color` impostata su `linen`.

# --hints--

Dovresti avere un selettore `a`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('a'));
```

Il tuo selettore `a` dovrebbe avere una proprietà `text-decoration` impostata su `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration, 'none');
```

Il tuo selettore `a` dovrebbe avere una proprietà `color` impostata su `linen`.
