...
Uma imagem padrão para o card e a URL para fazer uma requisição à API foram fornecidas, você só precisa de algumas variáveis para armazená-las.

Crie uma variável `CDN_URL` e defina-a como `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. Também crie uma variável `defaultImg` com o valor `new URL("default.svg", CDN_URL + "/")` e uma variável `LOCAL_DEFAULT_IMG` com o valor `defaultImg`.

# --hints--

Você deve criar uma variável chamada `CDN_URL`.
...
Sua variável `CDN_URL` deve ser definida como `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"`.
...
Você deve criar uma variável chamada `defaultImg`.
...
Sua variável `defaultImg` deve ser definida como `new URL("default.svg", CDN_URL + "/")`.
...
Você deve criar uma variável chamada `LOCAL_DEFAULT_IMG`.
...
Sua variável `LOCAL_DEFAULT_IMG` deve ser definida como `defaultImg`.
