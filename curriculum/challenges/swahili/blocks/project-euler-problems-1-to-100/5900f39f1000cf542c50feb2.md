...
title: 'Zoezi la 51: Mbadala wa nambari msingi'
challengeType: 1
forumTopicId: 302162
...
Kwa kubadilisha nambari ya kwanza ya nambari ya tarakimu 2 \*3, inajitokeza kuwa sita kati ya thamani tisa zinazowezekana: 13, 23, 43, 53, 73, na 83, zote ni namba msingi.

Kwa kubadilisha tarakimu ya tatu na ya nne ya 56\*\*3 kwa nambari ileile, nambari hii ya tarakimu 5 ni mfano wa kwanza kuwa na namba msingi saba kati ya namba kumi zilizotengenezwa, ikizalisha familia: 56003, 56113, 56333, 56443, 56663, 56773, na 56993. Hivyo basi 56003, ikiwa ni mwanachama wa kwanza wa familia hii, ni namba msingi ndogo zaidi yenye sifa hii.

Tafuta namba msingi ndogo zaidi ambayo, kwa kubadilisha sehemu ya nambari (si lazima tarakimu zilizo karibu) kwa nambari ileile, ni sehemu ya familia ya namba msingi `n`.

# --hints--

`primeDigitReplacements(6)` inapaswa kurudisha nambari.

```js
assert(typeof primeDigitReplacements(6) === 'number');
```

`primeDigitReplacements(6)` inapaswa kurudisha `13`.

```js
assert.strictEqual(primeDigitReplacements(6), 13);
```

`primeDigitReplacements(7)` inapaswa kurudisha `56003`.

```js
assert.strictEqual(primeDigitReplacements(7), 56003);
```

`primeDigitReplacements(8)` inapaswa kurudisha `121313`.
