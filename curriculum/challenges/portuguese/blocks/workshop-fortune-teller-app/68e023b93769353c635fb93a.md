...
Agora, dentro do template literal da função `renderCard`, crie um elemento `div`. Dentro do `div`, adicione um elemento `h2` que exiba `drawingType`, e um elemento `figure` com a classe `card_container ${isReversed ? "reversed-card" : ""}` e o atributo `data-id="${shortName}"`.

# --hints--

Você deve ter um elemento `div` no template literal da função `renderCard`.
...
Você deve ter um elemento `h2` que interpole `drawingType` dentro do elemento `div`.
...
Você deve ter um elemento `figure` dentro do elemento `div`.
...
Seu elemento `figure` deve ter a classe `card_container` e usar um ternário para adicionar condicionalmente `reversed-card`.
...
Seu elemento `figure` deve ter um atributo `data-id` definido como `${shortName}`.
