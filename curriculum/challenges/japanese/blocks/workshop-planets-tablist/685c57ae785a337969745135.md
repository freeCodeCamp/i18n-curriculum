...
最後のステップでは、関連するタブがクリックされたときにパネルを表示させる必要があります。これは、`hidden`の`panel`プロパティを`false`に設定することで実現できます。

どのタブでもクリックすると、それに対応するパネルの内容が表示されるはずです。

この最後の変更で、ワークショップは完了です！

# --hints--

タブがクリックされたとき、パネルの`hidden`プロパティを`false`に設定してください。
