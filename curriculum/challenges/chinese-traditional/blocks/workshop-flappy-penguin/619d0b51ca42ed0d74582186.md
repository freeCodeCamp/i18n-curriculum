...
選擇 `.eye` 元素，並將其 `width` 設置爲 `15%`、`height` 設置爲 `17%`，`background-color` 設置爲 `black`。
...
你應該將 `.eye` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.width, '15%');
```

你應該將 `.eye` 的 `height` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.height, '17%');
```

你應該將 `.eye` 的 `background-color` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
