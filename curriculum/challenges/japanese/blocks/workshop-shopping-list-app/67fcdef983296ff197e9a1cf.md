...
`filteredItems` という名前の変数を作成し、`items` 配列に対して `filter()` メソッドでフィルターした結果を代入してください。`filter()` メソッドには、`item` を引数に取る無名関数を渡してください。無名関数の中では、今はとりあえず `item` を返してください。

# --hints--

`filteredItems` という名前の変数を作成してください。

```js
assert.match(code, /(const|let|var)\s+filteredItems\s*=/);
```

`filter()` 配列に対して `items` メソッドを使ってください。

```js
assert.match(code, /items\.filter\s*\(/);
```

`filter()` メソッドには、`item` を引数に取り `item` を返す無名関数を渡してください。
