...
Erstellen Sie nach dem Konstruktor eine abstrakte `render`-Methode, die einen Rückgabetyp von `void` hat. Sie verwenden diese in Unterklassen, um das angezeigte Emoji zu aktualisieren.

# --hints--

Ihre `Bug`-Klasse sollte eine abstrakte `render`-Methode haben.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

Ihre `render`-Methode sollte einen Rückgabetyp von `void` haben.
