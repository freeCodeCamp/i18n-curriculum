...
마지막으로, 노래를 다시 렌더링하고, 재생 버튼의 접근 가능한 텍스트를 업데이트하며, 재생 목록에서 재설정 버튼을 제거하세요. 또한 DOM에서 `resetButton`를 제거해야 합니다.

노래를 알파벳 순서로 다시 렌더링하려면 `renderSongs()`를 인수로 하여 `sortSongs()` 함수를 호출하세요.

재생 버튼의 접근 가능한 텍스트를 업데이트하려면 `setPlayButtonAccessibleText()` 함수를 호출하세요.

`remove()` 변수에서 `resetButton` 메서드를 호출하여 재생 목록에서 재설정 버튼을 제거하세요.

**참고**: 이제 모든 노래를 제거해 보고 어떤 일이 일어나는지 확인할 수 있습니다.

# --hints--

기존 이벤트 리스너와 그 내용을 수정하지 마세요.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

`renderSongs`를 인수로 하여 `sortSongs()` 함수를 호출해야 합니다.
...
DOM에서 `remove()`를 제거하려면 `resetButton` 메서드를 사용하세요.
