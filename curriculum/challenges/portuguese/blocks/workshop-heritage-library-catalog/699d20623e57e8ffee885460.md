...
Antes de confiar em dados analisados, comece validando os campos obrigatórios.

Crie uma função chamada `validateEntry` que receba `entry` como parâmetro. Declare `let isValid = true`.

Adicione uma verificação de validação para `title`: defina `isValid = false` se `title` estiver ausente, for falso ou igual a `"Unknown"`.

Retorne `isValid`. Você adicionará verificações para os outros campos nos próximos passos.

# --hints--

`validateEntry` deve retornar um booleano.

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })` deve retornar `true`.

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})` deve retornar `false`.

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })` deve retornar `false`.

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })` deve retornar `false`.
