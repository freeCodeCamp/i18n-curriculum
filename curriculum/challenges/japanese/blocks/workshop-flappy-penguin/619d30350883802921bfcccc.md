...
これから、CSSアニメーションを使ってペンギンを手を振っているように動かします。

新しい`@keyframes`を`wave`という名前で定義してください。

# --hints--

新しい`@keyframes`ルールを定義する必要があります。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getCSSRules('keyframes'));
```

`@keyframes`ルールに`--fcc-expected--`という名前を付ける必要がありますが、`--fcc-actual--`が見つかりました。
