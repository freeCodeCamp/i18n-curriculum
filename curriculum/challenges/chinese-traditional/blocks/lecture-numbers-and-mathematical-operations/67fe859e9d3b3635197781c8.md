...
有時候，你可能會注意到涉及浮點數的操作結果有比預期更多的小數位數。 例如，`0.1 + 0.2` 的和是 `0.30000000000000004`，而不是 `0.3`。

這是因為數字是以二進位表單儲存，而某些分數無法被精確地以二進位表述。因此，它們被儲存為有限的近似值，就像分數 `1/3` 無法用有限位數的十進位數字表述，並且在其無限位數（`0.33333...`）之後被截斷一樣。
