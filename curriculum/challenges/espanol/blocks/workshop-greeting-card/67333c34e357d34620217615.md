...
Cree un selector para los elementos `section` y asígneles:

- una propiedad `margin` establecida en `20px auto`,
- un `max-width` establecido en `600px`.
- una propiedad `background-color` establecida en `whitesmoke`
- una propiedad `padding` establecida en `20px`
- una propiedad `border-radius` establecida en `10px`

# --hints--

Debe tener un selector `section`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle("section"));
```

El selector `section` debe tener una propiedad `margin` establecida en `20px auto`.
...
El selector `section` debe tener una propiedad `max-width` establecida en `600px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("max-width"), "600px");
```

El selector `section` debe tener una propiedad `background-color` establecida en `whitesmoke`.
...
El selector `section` debe tener una propiedad `padding` establecida en `20px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("padding"), "20px");
```

El selector `section` debe tener una propiedad `border-radius` establecida en `10px`.
