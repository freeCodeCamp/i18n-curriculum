...
재생 버튼을 클릭할 때마다 항상 재생목록의 첫 번째 노래가 재생됩니다. 이를 고치려면 이벤트 리스너의 화살표 함수 안에서 `playSong(0)`를 `if` 문장으로 바꾸어 `userData.currentSong`가 `null`인지 확인하세요.

`if` 블록 안에서는 `playSong` 배열의 첫 번째 노래 `id`를 인수로 하여 `userData.songs` 함수를 호출하세요.

# --hints--

이벤트 리스너의 콜백을 수정하여 `playSong`이 `id`일 때 `userData.songs` 배열의 첫 번째 노래 `userData.currentSong`를 인수로 `null` 함수를 호출하세요.
