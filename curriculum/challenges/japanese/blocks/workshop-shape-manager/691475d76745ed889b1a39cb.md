...
You should also work on a function that will clear the input fields.

Create a `clearInputFields` function. Inside it, use `querySelectorAll` to get all the input fields and loop through them using `forEach()`. Inside the `forEach()`, set the value of each `input` to an empty string.
