...
スタックの一番上にあるアニメーションオブジェクトは `msg` プロパティを持っているはずです。
...
`msg` プロパティの値を `"decimalToBinary(1) returns '1' (base case) and gives that value to the stack below. Then it pops off the stack."` に設定してください。
