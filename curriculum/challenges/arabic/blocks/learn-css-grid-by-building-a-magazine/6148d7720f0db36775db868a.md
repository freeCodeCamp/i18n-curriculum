...
ستحتاج إلى عمود للنص وعمود للصور. أعطِ المحدِّد `.text-with-images` خاصية `grid-template-columns` بقيمة `1fr 2fr`. كما عيّن خاصية `column-gap` إلى `3rem` لتوفير مزيد من الفراغ بين الأعمدة.

# --hints--

يجب أن يحتوي المحدِّد `.text-with-images` على خاصية `grid-template-columns` بقيمة `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

يجب أن يحتوي المحدِّد `.text-with-images` على خاصية `column-gap` بقيمة `3rem`.
