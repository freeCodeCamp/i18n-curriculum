...
أضف كتلة `else` إلى جملة `if`. داخل كتلة `else`، استدعِ الدالة `getCurrentSongIndex()` وعيّن نتيجتها إلى ثابت باسم `currentSongIndex`.

# --hints--

لا تعدل جملة `if` الموجودة.
...
يجب إضافة `else` إلى جملة `if` الموجودة.
...
عيّن الثابت `currentSongIndex` إلى نتيجة استدعاء `getCurrentSongIndex()` داخل كتلة `else`.
