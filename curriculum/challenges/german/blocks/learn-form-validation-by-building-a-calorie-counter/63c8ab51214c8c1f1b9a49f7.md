...
Deklariere in deiner neuen Funktion eine `calories`-Variable und weise ihr den Wert `0` zu. Verwende `let` um sie zu deklarieren, da du sie später neu zuweisen wirst.

# --hints--

Du solltest eine `calories`-Variable deklarieren.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

Deiner `calories`-Variablen sollte der Wert `0` zugewiesen werden.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Du solltest eine `calories`-Variable mit `let` deklarieren.
