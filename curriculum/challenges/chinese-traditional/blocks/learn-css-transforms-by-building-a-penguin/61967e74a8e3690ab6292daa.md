...
針對 `body` 元素，將其 `background` 設置爲順時針 45 度的線性漸變，從 `rgb(118, 201, 255)` 開始到 `rgb(247, 255, 222)` 結束。
...
你應該在 `body` 選擇器內設置 `background` 屬性。

```js
assert.isTrue(new __helpers.CSSHelp(document).isPropertyUsed('background'));
```

你應該將 `background` 設置爲 `linear-gradient(45deg, rgb(118, 201, 255), rgb(247, 255, 222))`。
