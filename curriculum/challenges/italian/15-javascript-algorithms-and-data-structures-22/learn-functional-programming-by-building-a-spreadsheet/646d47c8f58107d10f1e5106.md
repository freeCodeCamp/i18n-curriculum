...
Il primo argomento della chiamata di `evalFormula` deve essere il contenuto della cella (che hai memorizzato in `value`). Tuttavia, il contenuto inizia con un carattere `=` per attivare la funzione, quindi è necessario passare la sotto-stringa di `value` a partire dall'indice `1`.

# --hints--

Dovresti passare `value` come primo argomento alla chiamata `evalFormula()`.
...
Dovresti chiamare il metodo `.slice()` sull'argomento `value`.
...
Dovresti passare il numero `1` come argomento alla chiamata `.slice()`.
