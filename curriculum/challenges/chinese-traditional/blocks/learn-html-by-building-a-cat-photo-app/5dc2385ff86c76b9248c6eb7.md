...
HTML5 有一些元素能夠標識不同的內容區域。 這些元素能讓你的 HTML 易於閱讀，並有助於搜索引擎優化（SEO）和無障礙。
...
在 `h1` 元素之前添加 `<main>` 開始標籤，在 `p` 元素之後添加 `</main>` 結束標籤，標識此頁面的主要部分。
...
你的 `main` 元素的開始標籤應該在 `body` 元素的下面。 你把它們的順序寫錯了。
...
你的 `main` 元素的開始標籤應該在 `h1` 元素的上面。 你把它們的順序寫錯了。
...
你的 `main` 元素的結束標籤應該在 `p` 元素的下面。 你把它們的順序寫錯了。
