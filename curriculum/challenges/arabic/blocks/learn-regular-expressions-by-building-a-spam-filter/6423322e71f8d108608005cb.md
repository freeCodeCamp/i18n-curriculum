...
حدّث الآن الحرف `o` ليطابق `o` و`0` (الرقم).

# --hints--

يجب أن يستخدم `freeRegex` صنفًا من الأحرف لمطابقة `o` و`0`.

```js
assert.match(freeRegex.source, /\[(o0|0o)\]/);
```

يجب أن يطابق `freeRegex` عبارة `free money`.

```js
assert.match('free money', freeRegex);
```

يجب أن يطابق `freeRegex` عبارة `fr33 m0n3y`.
