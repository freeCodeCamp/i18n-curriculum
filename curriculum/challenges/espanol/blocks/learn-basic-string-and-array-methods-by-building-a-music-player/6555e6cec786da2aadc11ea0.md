...
También debes volver a renderizar las canciones, pausar la canción que se está reproduciendo actualmente, configurar la visualización del reproductor y establecer el texto accesible del botón de reproducción nuevamente.

Llama a la función `renderSongs` y pasa `userData?.songs` como un argumento. Además, llama a las funciones `pauseSong`, `setPlayerDisplay` y `setPlayButtonAccessibleText`.

# --hints--

No debes modificar la función `shuffle` existente ni su contenido.
