...
如果 `fetch` 调用发生错误， `catch` 块将会处理它。 

在 `catch` 块中，添加 `console.log` 以记录 `err` 参数。

此外，既然你已经了解了 `fetch` 调用返回的内容，现在可以从你的 `try` 块中删除 `console.log(data);`。

# --hints--

你应该使用 `console.log` 来记录 `err` 变量。

```js
assert.match(code, /\s*console\.log\(\s*err\s*\)/);
```

你的 `console.log` 应该放在 `catch` 块里面。

```js
assert.match(code, /\s*try\s*{[^}]*\s*}\s*catch\s*\(\s*err\s*\)\s*{[^}]*\s*console\.log\(\s*err\s*\)[^}]*}/);
```

你应该从 `try` 块中移除 `console.log(data);`。
