...
`oninput`에서 `shapeTypeSelect` 이벤트를 사용하고 `handleShapeSelect`로 설정하세요. 이렇게 하면 도형 선택이 바뀔 때마다 함수가 실행됩니다.

그다음 각 도형의 입력 필드를 볼 수 있어야 합니다. 하지만 계산은 아직 작동하지 않으며, 이것을 다음에 처리할 것입니다.

# --hints--

`shapeTypeSelect.oninput`을 `handleShapeSelect`로 설정하세요.
