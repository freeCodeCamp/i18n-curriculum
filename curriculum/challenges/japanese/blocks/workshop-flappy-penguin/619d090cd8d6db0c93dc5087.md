...
`.chin` 要素の下に、`div` の `class` を持つ `eye` 要素を2つ追加してください。また、最初の `.eye` 要素には `class` の `left` を、2番目の `.eye` 要素には `class` の `right` を付けてください。

# --hints--

`div` の中に `.penguin-head` 要素を2つ追加する必要があります。期待される `.penguin-head > div` 要素は --fcc-expected-- ですが、実際には --fcc-actual-- でした。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 5);
```

最初の新しい `div` に `class` の `eye` を付けてください。

```js
assert.exists(document.querySelector('.penguin-head > div.eye'));
```

2番目の新しい `div` に `class` の `eye` を付けてください。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div.eye'), 2);
```

最初の新しい `div` に `class` の `left` を付けてください。

```js
assert.exists(document.querySelector('.penguin-head > div.eye.left'));
```

2番目の新しい `div` に `class` の `right` を付けてください。

```js
assert.exists(document.querySelector('.penguin-head > div.eye.right'));
```

`div.eye.left` の後に `div.chin` を配置してください。

```js
assert.exists(document.querySelector('.chin + .eye.left'));
```

`div.eye.right` の後に `div.eye.left` を配置してください。
