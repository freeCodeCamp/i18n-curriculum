...
forループの中で、`low`から`high`までの区間の中点を計算してください。この値を変数`mid`に代入してください。

また、中点（`mid`）の二乗を計算し、変数`square_mid`にストアしてください。
...
`mid`ループの本体内で変数`(low + high) / 2`を宣言し、`for`を代入してください。
...
forループの本体内で変数`square_mid`を宣言し、`mid**2`を代入してください。
