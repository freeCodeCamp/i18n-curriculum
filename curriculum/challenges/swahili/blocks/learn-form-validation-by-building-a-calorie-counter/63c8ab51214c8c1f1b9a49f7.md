...
Katika kitendakazi chako kipya, tangaza kigezo cha `calories` na kipa thamani `0`. Tumia `let` kutangaza, kwa sababu utakuwa unarekebisha thamani yake baadaye.

# --hints--

Unapaswa kutangaza kigezo cha `calories`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

Kigezo chako cha `calories` kinapaswa kupewa thamani `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Unapaswa kutangaza kigezo chako cha `calories` kwa `let`.
