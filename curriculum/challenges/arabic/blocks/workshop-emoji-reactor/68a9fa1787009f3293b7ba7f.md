...
ربما لاحظت أن شفرتك أصبحت متكررة جدًا! هناك طريقة أفضل للتعامل مع عدة عناصر متشابهة.

بدلًا من اختيار كل زر على حدة، استخدم `querySelectorAll(".emoji-btn")` لاختيار كل الأزرار التي تحمل الصنف `emoji-btn` دفعة واحدة. خزّن النتيجة في متغير اسمه `btns`.
...
يجب أن تستخدم `querySelectorAll` لاختيار كل أزرار الإيموجي.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

يجب أن تخزن النتيجة في متغير اسمه `btns`.
...
يجب أن يحتوي المتغير `btns` على كل أزرار الإيموجي الأربعة.
