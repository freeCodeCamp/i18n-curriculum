...
**Nota:** Um valor negativo de `tabindex` (tipicamente -1) indica que um elemento é focável, mas não é acessível pelo teclado.Este método é geralmente usado para trazer o foco para o conteúdo programaticamente (como quando um `div` usado para uma janela pop-up é ativado) e está além do escopo desses desafios.
