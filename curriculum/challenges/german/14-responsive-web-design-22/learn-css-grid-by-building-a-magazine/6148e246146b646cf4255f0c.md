...
Du solltest einen `.list-title, .list-subtitle`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'))
```

Dein `.list-title, .list-subtitle`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `#00beef` haben.
