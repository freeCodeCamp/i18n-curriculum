...
Вам потрібно отримати `id` пісні, яка зараз відтворюється. Для цього ви можете використати `userData?.currentSong?.id`.

Використайте `getElementById()`, щоб отримати `id` пісні, яка зараз відтворюється, а потім за допомогою шаблонних літералів додайте префікс `song-`. Призначте це константі `songToHighlight`.

# --hints--

Ви не повинні змінювати існуючу функцію `highlightCurrentSong` та її вміст.
...
Ви повинні використати `document.getElementById()` і передати `` `song-${userData?.currentSong?.id}` ``.

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Ви повинні призначити свій `getElementById()` константі `songToHighlight`.
