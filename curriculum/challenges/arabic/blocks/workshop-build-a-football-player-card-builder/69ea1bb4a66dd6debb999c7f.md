...
الآن ستتعامل مع اختيار موقع اللاعب. عليك إعادة هيكلة المُدخَل إلى قائمة اختيار، ثم تمرير مصفوفة من مواقع كرة القدم الصالحة كقيم لخيارات القائمة.

لتبدأ، أنشئ مصفوفة `POSITIONS` تحتوي على القيم النصية `GK`، `CB`، `LB`، `RB`، `CDM`، `CM`، `CAM`، `LW`، `RW`، `ST`، و `CF`. أضف `as const` مباشرة بعد الأقواس المربعة المغلقة ليُضيّق TypeScript النوع من مصفوفة عامة `string[]` إلى صف مرتب للقراءة فقط يحتوي على تلك القيم الحرفية بالضبط.

هذا يعني أن TypeScript يعرف بدقة ما هي السلاسل النصية الصالحة، بدلاً من معرفته فقط أنها مصفوفة من السلاسل النصية.

# --hints--

يجب أن تعلن عن ثابت `POSITIONS`.
...
يجب أن تحتوي مصفوفة `POSITIONS` على جميع سلاسل مواقع كرة القدم المطلوبة، واستخدم `as const` لتضييق نوعها إلى سلاسل نصية حرفية للقراءة فقط.
