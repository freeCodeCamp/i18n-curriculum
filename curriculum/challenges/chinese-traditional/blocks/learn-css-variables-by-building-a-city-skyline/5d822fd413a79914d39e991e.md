...
將 `.fb2-window` 元素的 `width` 設置爲 `22%`，`height` 設置爲 `100%`，`background-color` 設置爲變量 `--window-color3`。

# --hints--

你應該給 `.fb2-window` 元素設置一個值爲 `22%` 的 `width`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

你應該給 `.fb2-window` 元素設置一個值爲 `100%` 的 `height`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

你應該給 `.fb2-window` 元素設置一個值爲 `--window-color3` 的 `background-color`。
