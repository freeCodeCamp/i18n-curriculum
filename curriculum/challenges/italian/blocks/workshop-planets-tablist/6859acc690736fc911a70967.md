...
Al momento, tutti e tre i fatti sono visibili contemporaneamente. Ma dovrebbe essere visibile solo un fatto alla volta.

Per ottenere questo, devi impostare l'attributo `hidden` sul secondo e sul terzo elemento `tabpanel`.

# --hints--

Il tuo secondo elemento `tabpanel` dovrebbe avere l'attributo `hidden`.

```js
assert.isTrue(document.querySelectorAll('div[role="tabpanel"]')[1].hasAttribute("hidden"));
```

Il tuo terzo elemento `tabpanel` dovrebbe avere l'attributo `hidden`.
