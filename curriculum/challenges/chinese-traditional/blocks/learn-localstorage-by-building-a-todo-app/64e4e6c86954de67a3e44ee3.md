...
你需要使用 `getElementById()` 方法存取更多元素。這次你需要 `close-task-form-btn`、`add-or-update-task-btn` 和 `cancel-btn` 元素。將它們存檔在變數 `closeTaskFormBtn`、`addOrUpdateTaskBtn` 和 `cancelBtn`。

# --hints--

你應該使用 `getElementById()` 來存取 `close-task-form-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)close\-task\-form\-btn\1\s*\)\s*;?/)
```

你應該將 `close-task-form-btn` 元素指定給變數 `closeTaskFormBtn`。別忘了使用 `const` 來宣告該變數。

```js
assert.match(code, /const\s+closeTaskFormBtn\s*=\s*document\.getElementById\(\s*('|"|`)close\-task\-form\-btn\1\s*\)\s*;?/)
```

你應該使用 `getElementById()` 來存取 `add-or-update-task-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)add\-or\-update\-task\-btn\1\s*\)\s*;?/)
```

你應該將 `add-or-update-task-btn` 元素指定給變數 `addOrUpdateTaskBtn`。別忘了使用 `const` 來宣告變數。

```js
assert.match(code, /const\s+addOrUpdateTaskBtn\s*=\s*document\.getElementById\(\s*('|"|`)add\-or\-update\-task\-btn\1\s*\)\s*;?/)
```

你應該使用 `getElementById()` 來存取 `cancel-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)cancel\-btn\1\s*\)\s*;?/)
```

你應該將 `cancel-btn` 元素指定給變數 `cancelBtn`。別忘了使用 `const` 來宣告該變數。
