...
Erstellen Sie einen `html`-Selektor und geben Sie ihm eine `font-size`-Eigenschaft mit dem Wert `62.5%`. Dadurch wird die Standard-Schriftgröße Ihrer Webseite auf 10px gesetzt (der Browser-Standard ist 16px).

Das erleichtert Ihnen später die Arbeit mit `rem`-Einheiten, da `2rem` dann 20px entsprechen würde.

# --hints--

Sie sollten einen `html`-Selektor erstellen.

```js
assert(new __helpers.CSSHelp(document).getStyle('html'));
```

Ihr `html`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `62.5%` haben.
