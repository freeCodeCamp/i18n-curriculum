...
你需要給接下來的兩個部分添加相同的漸變。 不要像之前那樣做，而是創建一個名爲 `bb1-window` 的新類選擇器，並從 `.bb1a` 中將 `height` 和 `background` 移動到新的類選擇器。

# --hints--

你應該創建一個名爲 `bb1-window` 的新類選擇器。
