...
أضف الآن مفتاح `diagnosis` إلى قاموس `constraints`. لقيمته، اكتب تعبيرًا يتحقق من أن `diagnosis` إما كائن من نوع `str` أو يساوي `None`.

# --hints--

يجب أن يحتوي قاموس `constraints` على مفتاح `diagnosis`.
...
يجب أن يكون لمفتاح `diagnosis` في قاموس `constraints` القيمة `isinstance(diagnosis, str) or diagnosis is None`.
