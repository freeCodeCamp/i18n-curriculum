...
Ora, aggiungi un `div` con la classe `img-loader`. Dopo, crea un elemento `img` e assegna un URL condizionale per l'immagine usando ``${img ? `${CDN_URL}/${img}` : LOCAL_DEFAULT_IMG}``.

# --hints--

Dovresti avere un elemento `div` dentro il tuo elemento `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

Il tuo `div` dovrebbe avere una classe `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

Dovresti avere un elemento `img` dentro il tuo elemento `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

L'elemento `img` dovrebbe usare `CDN_URL` e `img` per il suo `src` quando `img` è fornito, e ricadere su `LOCAL_DEFAULT_IMG` quando non lo è.
...
Il tuo elemento `img` dovrebbe avere le classi `card-img` e `hidden`.
