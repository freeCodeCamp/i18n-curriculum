...
創建一個 `.hero-title` 選擇器併爲其指定一個設置爲 `center` 的 `text-align` 屬性、一個設置爲 `orangered` 的 `color` 屬性以及一個設置爲 `8rem` 的 `font-size` 屬性。

# --hints--

你應該有一個 `.hero-title` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

你的 `.hero-title` 選擇器應該有一個 `text-align` 屬性，設置爲 `center`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign === 'center');
```

你的 `.hero-title` 選擇器應該有一個 `color` 屬性，設置爲 `orangered`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color === 'orangered');
```

你的 `.hero-title` 選擇器應該有一個 `font-size` 屬性，設置爲 `8rem`。
