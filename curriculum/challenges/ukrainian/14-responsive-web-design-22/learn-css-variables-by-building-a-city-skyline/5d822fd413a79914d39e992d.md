...
Ви повинні надати `.fb4-window` властивість `width` зі значенням `30%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

Ви повинні надати `.fb4-window` властивість `height` зі значенням `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

Ви повинні надати `.fb4-window` властивість `border-radius` зі значенням `50%`.
