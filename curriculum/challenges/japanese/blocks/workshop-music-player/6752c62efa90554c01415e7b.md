...
`setPlayerDisplay` という名前の関数を作成してください。関数内で、現在の曲のタイトルが定義されていれば `textContent` の `playingSong` に設定し、そうでなければ空文字列に設定してください。

その下で、現在の曲のアーティストが定義されていれば `textContent` の `songArtist` に設定し、そうでなければ空文字列に設定してください。


# --hints--

`setPlayerDisplay` という名前の関数があるはずです。

```js
assert.isFunction(setPlayerDisplay);
```

`setPlayerDisplay` 関数は、現在の曲のタイトルが定義されていれば `textContent` の `playingSong` に設定し、そうでなければ空文字列に設定する必要があります。
...
`setPlayerDisplay` 関数は、現在の曲のアーティストが定義されていれば `textContent` の `songArtist` に設定し、そうでなければ空文字列に設定する必要があります。
