...
Тепер потрібно додати атрибут `aria-current` до пісні, яка зараз відтворюється.

Перевірте, чи існує `songToHighlight`. Якщо так, використайте метод `setAttribute` для `songToHighlight` і передайте `"aria-current"` та `"true"` як перший і другий аргументи.

# --hints--

`highlightCurrentSong` має додати атрибут `aria-current` до пісні, яка зараз відтворюється, і видалити атрибут з пісні, яка відтворювалася раніше.
