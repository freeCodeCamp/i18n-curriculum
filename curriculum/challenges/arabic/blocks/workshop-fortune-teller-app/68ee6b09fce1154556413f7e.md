...
داخل طريقة `initializeEventListeners`، أرفق مستمع حدث `click` إلى `singleCardBtn`. ضمن معالِج الحدث، نفذ استدعاء الطريقة `singleCardSelected`. تذكّر الحصول على `singleCardBtn` من `elements`.

ستنشئ هذه الطريقة في الخطوات القادمة.

# --hints--

يجب أن تُرفق حدث `click` إلى عنصر `singleCardBtn`. لا تنسَ أنك تحتاج إلى كلمة `this` هنا.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

داخل استدعاء راجع الحدث، يجب أن تنفذ استدعاء لطريقة `singleCardSelected`.
