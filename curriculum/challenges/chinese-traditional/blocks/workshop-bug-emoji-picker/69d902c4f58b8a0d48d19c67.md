...
現在為 `bugMap` 填入兩個項目。新增 `"bee"` 作為鍵，對應到一個新的 `Bee` 實體，並新增 `"spider"` 作為鍵，對應到另一個新的 `Spider` 實體。兩者都傳入 `bugEmojiElement` 作為引數。

# --hints--

你的 `bugMap` 應該有一個 `"bee"` 項目，設定為 `new Bee(bugEmojiElement)`。

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

你的 `bugMap` 應該有一個 `"spider"` 項目，設定為 `new Spider(bugEmojiElement)`。
