...
في الدروس السابقة، تعلمت كيف تتعامل مع الطريقة `filter()` التي تُستخدم لإرجاع مصفوفة جديدة من النتائج المصفاة.
...
أنشئ دالة `getBooksByAuthor` بمعلمتين - مصفوفة تحتوي على كائنات كتب وسلسلة نصية تحتوي على اسم المؤلف.

يجب أن تُرجع الدالة مصفوفة تحتوي على الكتب الخاصة بمؤلف معين.

# --hints--

يجب أن تنشئ دالة `getBooksByAuthor`.

```js
assert.isFunction(getBooksByAuthor);
```

يجب أن تحتوي دالة `getBooksByAuthor` على معلمتين.

```js
assert.lengthOf(getBooksByAuthor, 2);
```

يجب أن تُرجع دالة `getBooksByAuthor` مصفوفة.

```js
assert.isArray(getBooksByAuthor(library, ''));
```

يجب أن تُرجع دالة `getBooksByAuthor` العدد الصحيح من الكتب لأي من المؤلفين.
