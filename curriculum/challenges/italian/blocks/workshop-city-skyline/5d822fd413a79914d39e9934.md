...
Finalmente! Sei arrivato all’ultimo edificio! Aggiungi un gradiente ripetuto con una direzione `90deg`. Usa il colore dell’edificio da `0%` a `10%` e `transparent` da `10%` a `30%`.

# --hints--

Dovresti aggiungere un `repeating-linear-gradient` a `.fb6` nella proprietà `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

Dovresti impostare una direzione `repeating-linear-gradient` per `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Dovresti impostare un primo colore `repeating-linear-gradient` per `--building-color3` da `0%` a `10%`.
...
Dovresti impostare un secondo colore `repeating-linear-gradient` per `transparent` da `10%` a `30%`.
