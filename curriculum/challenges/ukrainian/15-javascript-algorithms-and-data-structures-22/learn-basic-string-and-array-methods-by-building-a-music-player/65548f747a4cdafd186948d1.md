...
Вам потрібно отримати `id` пісні, яка зараз відтворюється. Для цього можна використати `userData?.currentSong?.id`.

Використайте `getElementById()`, щоб отримати `id` пісні, яка зараз відтворюється, а потім використайте шаблонні літерали, щоб додати до неї префікс `song-`. Призначте його до константи `songToHighlight`.

# --hints--

Не змінюйте наявну функцію `highlightCurrentSong` та її вміст.
...
Використайте `document.getElementById()` та передайте `` `song-${userData?.currentSong?.id}` ``.

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Призначте `getElementById()` до константи `songToHighlight`.
