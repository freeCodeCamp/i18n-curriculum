...
现在对 `multipleCardsBtn` 和 `newReadingBtn` 做同样的操作，给 `multipleCardsBtn` 添加一个点击事件监听器，调用 `multipleCardSelected`，再给 `newReadingBtn` 添加另一个点击事件监听器，调用 `newReading`。

你将在接下来的步骤中创建这两个方法。

# --hints--

你应该给 `click` 元素添加一个 `multipleCardsBtn` 事件。

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

在 `multipleCardsBtn` 回调事件中，你应该调用 `multipleCardSelected` 方法。

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

你应该给 `click` 元素添加一个 `newReadingBtn` 事件。

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

在 `newReadingBtn` 回调事件中，你应该调用 `newReading` 方法。
