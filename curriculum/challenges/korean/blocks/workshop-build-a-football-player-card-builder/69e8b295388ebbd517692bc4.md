...
티어 배지가 여전히 하드코딩된 텍스트 `GOLD`를 보여줍니다. 올바른 티어가 표시되도록 동적 표현식으로 바꾸세요.

`getPlayerTier()`를 인수로 하여 `player.overallRating`을 호출해 티어 문자열을 얻은 다음, 결과에 `.toUpperCase()`을 호출해 대문자로 표시하세요. 전체 표현식을 중괄호로 감싸는 것도 잊지 마세요.

# --hints--

`getPlayerTier(player.overallRating).toUpperCase()` 안에 `.card-tier-badge`를 사용해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

`.card-tier-badge` 요소는 평점이 98인 플레이어에 대해 `ELITE`을 표시해야 합니다.
