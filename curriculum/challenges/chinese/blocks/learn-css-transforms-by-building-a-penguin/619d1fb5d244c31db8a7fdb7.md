...
选择 `.foot` 元素，并将其 `width` 设置为 `15%`、`height` 设置为 `30%`、`background-color` 设置为 `orange`。
...
你应该将 `.foot` 的 `width` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.width, '15%');
```

你应该将 `.foot` 的 `height` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.height, '30%');
```

你应该将 `.foot` 的 `background-color` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
