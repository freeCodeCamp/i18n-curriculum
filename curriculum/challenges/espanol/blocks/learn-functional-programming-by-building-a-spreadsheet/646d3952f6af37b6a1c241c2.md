...
Ahora que tu función `.map()` está recibiendo la referencia a la función `num => charRange(...).map(...)` devuelta por las llamadas currificadas a `addCharacters`, iterará correctamente sobre los elementos y pasará cada elemento como `n` a esa función.

Notarás que no estás usando tu `match` parámetro. En JavaScript, es una convención común prefijar un parámetro no utilizado con un guion bajo `_`. También podrías dejar el parámetro vacío así: `(, char1)`  pero a menudo es más claro nombrar el parámetro para la legibilidad futura.

Prefija tu `match` parámetro con un guion bajo.

# --hints--

Deberías prefijar tu `match` parámetro con un guion bajo.
