...
你在 `handleGateAtTick` 函式中的最後任務是控制當前時脈的任何上限溢位。

如果存在任何上限溢位（`result.overflow > 0`），你應該：

- 記錄以下訊息 `"Overflow of " + result.overflow + " attendees. Rerouting..."`。
- 透過呼叫你的 `rerouteOverflow()` 函式，並帶入 `gates`、`gate`、`tickIndex` 和 `result.overflow`，重新導向該上限溢位。

# --hints--

如果存在任何上限溢位（`result.overflow > 0`），你應該先記錄訊息 `"Overflow of " + result.overflow + " attendees. Rerouting..."`。
...
如果存在任何上限溢位（`result.overflow > 0`），你應該透過呼叫 `rerouteOverflow()`，並帶入 `gates`、`gate`、`tickIndex` 和 `result.overflow`，重新導向該上限溢位。
