...
クラスセレクターを使って、`.cat-whisker-left-top` 要素を `position` の `absolute`、`top` の `120px`、および `left` の `52px` で配置してください。

# --hints--

`.cat-whisker-left-top` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top'))
```

`.cat-whisker-left-top` セレクターには `position` に設定された `absolute` プロパティがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.position === 'absolute')
```

`.cat-whisker-left-top` セレクターには `top` に設定された `120px` プロパティがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.top === '120px')
```

`.cat-whisker-left-top` セレクターには `left` に設定された `52px` プロパティがあるはずです。
