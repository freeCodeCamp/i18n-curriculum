...
Per archiviare il tempo corrente della canzone quando è in pausa, imposta la `songCurrentTime` dell'oggetto `userData` sul valore di `currentTime` della variabile `audio`.

*Nota*: non dovresti usare il concatenamento opzionale in questo passaggio perché `userData.songCurrentTime` non sarà `null` o `undefined` a questo punto.
...
Dovresti assegnare `audio.currentTime` a `userData.songCurrentTime`.
