...
Настав час перевірити повну систему знижок! Додайте інструкцію `if`, яка перевіряє, чи `__name__ == '__main__'`, і перемістіть `product = Product('Wireless Mouse', 50.0)` всередину неї.
