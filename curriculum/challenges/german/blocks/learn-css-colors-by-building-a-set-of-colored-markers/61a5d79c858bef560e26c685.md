...
Und als drittes Farbargument übergeben Sie die `hsl`-Funktion mit den Werten `240` für Farbton, `56%` für Sättigung und `42%` für Helligkeit.

# --hints--

Ihre `.blue`-CSS-Regel sollte eine `background`-Eigenschaft mit dem Wert `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))` haben.
