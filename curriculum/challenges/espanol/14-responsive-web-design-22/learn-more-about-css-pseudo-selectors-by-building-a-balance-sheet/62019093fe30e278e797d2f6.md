...
Tu selector `tr[class="total"]` debe tener una propiedad `border-bottom` establecida en `4px double #0a0a23`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"]')?.getPropertyValue('border-bottom') === '4px double rgb(10, 10, 35)');
```

Tu selector `tr[class="total"]` debe tener una propiedad `font-weight` establecida en `bold`.
