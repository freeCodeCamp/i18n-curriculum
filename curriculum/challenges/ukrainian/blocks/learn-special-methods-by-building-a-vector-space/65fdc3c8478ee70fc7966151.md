...
Так само, як ви робили раніше для методу `__eq__`, створіть інструкцію `if`, яка перевіряє, чи `self` і `other` не належать до одного класу, і повертає `NotImplemented`.
