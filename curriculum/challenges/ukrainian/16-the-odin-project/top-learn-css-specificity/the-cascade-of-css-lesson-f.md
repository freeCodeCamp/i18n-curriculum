---
id: 6489cf6782cf2e4f86f03ae7
title: Каскадність CSS. Урок №6
challengeType: 15
dashedName: the-cascade-of-css-lesson-f
---

# --description--

```css
/* rule 1 */
.class.second-class {
  font-size: 12px;
}

/* rule 2 */
.class .second-class {
  font-size: 24px;
}
```

Обидва правила мають однакову специфічність. Правило №1 використовує об’єднуючий селектор (немає пробілу), а правило №2 використовує нащадка (є пробіл). Однак обидва правила мають два класи, а символ комбінатора не додає специфічності.

```css
/* rule 1 */
.class.second-class {
  font-size: 12px;
}
/* rule 2 */
.class > .second-class {
  font-size: 24px;
}
```

Цей приклад показує те саме. Хоча правило №2 використовує дочірній комбінатор (`>`), значення специфічності не змінюється. Обидва правила мають два класи, тому специфічність однакова.

*Примітка:* не все впливає на специфічність

Порівнюючи селектори, ви можете наткнутись на спеціальні символи універсального селектора (`*`), а також комбінаторів (`+`, `~`, `>` й пробіл). Такі символи не додають жодної специфічності.

# --questions--

## --text--

Based on the given CSS code, which font size would be applied to an element that has both the `class` and `second-class` classes?

## --answers--

12px

---

36px

---

24px

---

48px

## --video-solution--

1
