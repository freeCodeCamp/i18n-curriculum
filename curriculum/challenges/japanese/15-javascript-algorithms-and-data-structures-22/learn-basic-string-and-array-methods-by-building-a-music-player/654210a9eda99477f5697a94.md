...
`button` 要素を作成し、クラス `playlist-song-info` を設定してください。 `button` 要素の中に `span` 要素を追加してクラス `playlist-song-title` を設定し、`span` 要素のテキストとして `song.title` を文字列補間で挿入してください。

# --hints--

クラス `playlist-song-info` を持つ `button` 要素を作成する必要があります。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

`button` 要素の中に `span` 要素を作成する必要があります。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

`span` 要素にはクラス `playlist-song-title` が必要です。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

`span` 要素のテキストコンテンツとして `song.title` を文字列補間で挿入する必要があります。
