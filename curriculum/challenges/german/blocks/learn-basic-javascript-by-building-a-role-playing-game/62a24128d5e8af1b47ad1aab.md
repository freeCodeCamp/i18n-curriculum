...
Verwenden Sie eine Selektorliste (`selector1, selector2`), um sowohl Ihren `#controls`- als auch Ihren `#stats`-Elementen einen `border` von `1px solid #0a0a23`, eine `#0a0a23`-Textfarbe und einen `5px` von `padding` zu geben.

# --hints--

Sie sollten eine `#controls, #stats`-Selektorliste haben.
...
Ihr `#controls, #stats`-Selektor sollte einen `border` von `1px solid #0a0a23` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#controls, #stats')?.border, '1px solid rgb(10, 10, 35)');
```

Ihr `#controls, #stats`-Selektor sollte einen `color` von `#0a0a23` haben.
...
Ihr `#controls, #stats`-Selektor sollte `5px` von Innenabstand haben.
