...
أنشئ محدِّد `img` وأعطه خاصية `width` بقيمة `100%`، وخاصية `object-fit` بقيمة `cover`.

تُخبر خاصية `object-fit` المتصفح بكيفية وضع العنصر داخل الحاوية. في هذه الحالة، ستجعل القيمة `cover` الصورة تملأ الحاوية، مع اقتصاصها حسب الحاجة لتجنب تغيير نسبة العرض إلى الارتفاع.
...
يجب أن يحتوي محدِّد `img` على خاصية `width` بقيمة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('img')?.width, '100%');
```

يجب أن يحتوي محدِّد `img` على خاصية `object-fit` بقيمة `cover`.
