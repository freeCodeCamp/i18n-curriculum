...
在函數內部，獲取負責顯示歌曲標題和藝術家的 HTML 元素的引用。

使用 `getElementById()` 方法訪問 `#player-song-title` 和 `#player-song-artist` 元素。 將它們分別分配給變量 `playingSong` 和 `songArtist`。

# --hints--

你不應修改現有的 `setPlayerDisplay` 函數及其內容。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

你應該使用 `document.getElementById()` 來獲取 `#player-song-title` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

你應該將 `#player-song-title` 元素分配給變量 `playingSong`。 不要忘記使用 `const` 來聲明變量。

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

你應該使用 `document.getElementById()` 來獲取 `#player-song-artist` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

你應該將 `#player-song-artist` 元素分配給變量 `songArtist`。 不要忘記使用 `const` 來聲明變量。
