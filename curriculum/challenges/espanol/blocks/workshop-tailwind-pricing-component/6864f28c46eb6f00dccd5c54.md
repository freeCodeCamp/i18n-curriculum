...
Moviendo al `h2` dentro del **Listener plan**, dale la clase de utilidad `text-lg` para hacerlo más grande, `font-semibold` para hacerlo más negrita, y `text-indigo-600` para darle un hermoso color índigo.

# --hints--

Tu elemento `h2` debe tener la clase `text-lg`.
...
Tu elemento `h2` debe tener la clase `font-semibold`.
...
Tu elemento `h2` debe tener la clase `text-indigo-600`.
