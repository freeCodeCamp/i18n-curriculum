...
你需要一個函式來驗證輸入的數字，並確保它們不是負數。

為此，創建一個 `handleInput` 函式，並帶有一個 `e` 參數，該參數的型別為 `Event`。在函式內，創建一個 `value` 變數，並將其設為標的值。確保它被轉換為數字，並使用 `HTMLInputElement` 關鍵字轉型為 `as` 型別。

# --hints--

你應該創建一個 `handleInput` 函式。

```js
assert.isFunction(handleInput)
```

你的 `handleInput` 函式應該有一個 `e` 參數，型別為 `Event`。
