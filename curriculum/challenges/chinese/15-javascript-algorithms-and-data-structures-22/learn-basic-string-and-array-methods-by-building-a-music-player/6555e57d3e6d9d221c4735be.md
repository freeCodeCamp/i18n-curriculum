...
当按下随机播放按钮时，你需要将 `currentSong` 设置为无，并将 `songCurrentTime` 设置为 `0`。

将 `userData.currentSong` 设置为 `null`，将 `userData.songCurrentTime` 设置为 `0`。

*注意*：你不应该在此步骤中使用可选链接，因为你明确将 `currentSong` 和 `songCurrentTime` 属性分别设置为 `null` 和 `0`。

# --hints--

你应该将 `userData.currentSong` 设置为 `null`。

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

你应该将 `userData.songCurrentTime` 设置为 `0`。
