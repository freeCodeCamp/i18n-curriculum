...
في هذه الخطوة، أضف مستمع حدث `submit` إلى `recipeForm` بحيث عند نقر زر `Convert`، تُستدعى الدالة `updateResultsList()`.

أضف مستمع حدث `submit` إلى `recipeForm`. عند إرسال النموذج، استدعِ `event.preventDefault()` لإيقاف السلوك الافتراضي، ثم استدعِ `updateResultsList()`.

سيسمح لك هذا بتشغيل الدالة عبر واجهة المستخدم بملء النموذج والضغط على `Convert`، وهو مفيد لتصحيح الأخطاء أثناء إضافة منطق التحويل في الخطوة التالية.

# --hints--

اجعل نقر زر `Convert` يشغّل `updateResultsList()` بإضافة مستمع حدث `"submit"` إلى `recipeForm` يستدعي `event.preventDefault()` لإيقاف السلوك الافتراضي للنموذج.
