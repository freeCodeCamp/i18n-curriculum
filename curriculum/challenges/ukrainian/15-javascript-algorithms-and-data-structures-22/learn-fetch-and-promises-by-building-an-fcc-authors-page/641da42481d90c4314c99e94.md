...
Метод `fetch()` повертає `Promise` (обіцянку), яка є об’єктом-заповнювачем, яка або буде виконаною, якщо запит буде успішним, або відмовленою, якщо запит буде невдалим.

Якщо `Promise` виконано, вона перетворюється на об’єкт `Response`, та можна використати метод `.then()` для доступу до `Response`.

Ось як ви можете зв’язати `.then()` до методу `fetch()`:
...
З’єднайте метод `.then()` з викликом `fetch`. Всередині методу `.then()` додайте функцію зворотного виклику з параметром `res`, а потім зареєструйте `res` на консолі, щоб ви могли бачити об’єкт `Response`. Відкрийте консоль браузера та розгорніть об’єкт `Response`, щоб побачити, що він містить.

Знову ж таки, поки що не закінчуйте код крапкою з комою.
...
Зареєструйте параметр `res` на консолі, щоб ви могли бачити об’єкт `Response`.
...
Ви не повинні мати крапки з комою після `.then()`.
