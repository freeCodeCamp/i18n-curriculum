...
Додайте блок `else` для обробки поточного часу відтворення пісні. Це дозволяє відновити поточну пісню з місця, де її було призупинено.

В межах блоку `else` встановіть властивість `currentTime` об’єкта `audio` на значення, яке зберігається в `userData?.songCurrentTime`.
...
Ви повинні додати блок `else` до наявного блоку `if`.
...
Встановіть `audio.currentTime` на `userData?.songCurrentTime` в межах блоку `else` інструкції `if`.
