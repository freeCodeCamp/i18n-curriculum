...
Debes usar un selector de clase para seleccionar la clase `sleeve`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.sleeve'));
```

La regla CSS `.sleeve` debe tener la propiedad `width` establecida a `110px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.sleeve')?.width === '110px');
```

La regla CSS `.sleeve` debe tener la propiedad `height` establecida a `25px`.
