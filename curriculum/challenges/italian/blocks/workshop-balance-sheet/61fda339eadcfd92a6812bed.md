...
Il selettore `span[class~="sr-only"]` selezionerà qualsiasi elemento `span` il cui `class` _include_ `sr-only`. Crea quel selettore e dagli una proprietà `border` impostata su `0`.
