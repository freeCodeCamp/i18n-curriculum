...
你的 `span` 元素应该有一个开始标签，而不是一个结束标签。

```js
assert(document.querySelector('form > input') && !code.match(/<\/input\>/g));
```

You should only have added one `input` element for your radio button. 删除任何多余的。

```js
assert(document.querySelectorAll('form > input'), 2);
```

Your new `input` element should be above the existing `input` with `type` attribute set to `text`. 你把它们的顺序写错了。
...
Your new `input` element should have only one `type` attribute. 删除任何多余的。
...
Your new `input` element should have a `type` attribute with the value `radio`. 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
...
The text `Indoor` should be located directly to the right of your `radio` button. 你要么遗漏了文本，要么拼写有误。
