...
`firsttwo` 매개변수를 받고 `nums` 배열의 처음 두 요소를 순서대로 반환하는 `nums` 속성을 추가하세요. 그런 다음 `lasttwo` 배열의 마지막 두 요소를 순서대로 반환하는 `nums` 속성을 추가하세요.

# --hints--

`spreadsheetFunctions` 객체에 `firsttwo` 속성이 있어야 합니다.

```js
assert.property(spreadsheetFunctions, "firsttwo");
```

`firsttwo` 속성은 함수여야 합니다.

```js
assert.isFunction(spreadsheetFunctions.firsttwo);
```

`firsttwo` 함수는 배열의 처음 두 숫자를 반환해야 합니다.

```js
assert.deepEqual(spreadsheetFunctions.firsttwo([1, 2, 3, 4, 5, 6]), [1, 2]);
```

`spreadsheetFunctions` 객체에 `lasttwo` 속성이 있어야 합니다.

```js
assert.property(spreadsheetFunctions, "lasttwo");
```

`lasttwo` 속성은 함수여야 합니다.

```js
assert.isFunction(spreadsheetFunctions.lasttwo);
```

`lasttwo` 함수는 배열의 마지막 두 숫자를 반환해야 합니다.
