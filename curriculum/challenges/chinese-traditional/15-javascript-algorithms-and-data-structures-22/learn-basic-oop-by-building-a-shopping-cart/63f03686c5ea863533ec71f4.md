...
聲明一個變量 `tax`，併爲其分配調用新 `.calculateTaxes()` 方法的值，同時傳遞 `subTotal` 作爲參數。

# --hints--

使用 `const` 聲明一個名爲 `tax` 的變量。
...
將調用新的 `.calculateTaxes()` 方法的值分配給 `tax` 變量，並將 `subTotal` 作爲參數傳遞。 請記住使用 `this` 關鍵字。
