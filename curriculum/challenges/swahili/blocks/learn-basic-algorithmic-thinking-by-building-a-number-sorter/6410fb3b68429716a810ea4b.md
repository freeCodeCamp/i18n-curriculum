...
Mpangilio wa selection sort unategemea kufuatilia kielezo cha thamani ndogo kabisa katika safu ya data. Tangaza kigezo `minIndex` na uweke thamani yake kuwa `i` - hii inahakikisha kwamba ikiwa thamani yako ya sasa ndogo kabisa, itabadilishwa na yenyewe na haitahamishwa. Utahitaji uwezo wa kuweka thamani mpya ya `minIndex` unaporudia kupitia safu ya data.

Kisha, andika mzunguko mwingine wa `for`, ukitumia `j` kama mzunguko. Mzunguko huu unahitaji kuanza katika kielezo kinachofuata `i` na kurudia kupitia sehemu iliyobaki ya safu ya data.

# --hints--

Unapaswa kutumia `let` kutangaza `minIndex`.
...
Unapaswa kuweka `minIndex` kuwa `i`.
...
Unapaswa kuwa na mzunguko wa `for` ulio pangwa ndani.
...
Mzunguko wako wa `for` unapaswa kuanzisha `j` kwa `i + 1`.
...
Mzunguko wako wa `for` unapaswa kurudia kupitia sehemu iliyobaki ya safu ya data.
...
Mzunguko wako wa `for` unapaswa kuongeza `j`.
