...
在上一个项目中，你学习了如何使用 `shift()` 方法从数组中删除第一个元素，如下所示：
...
使用 `shift()` 方法从 `inventory` 数组中获取第一个元素并将其分配给 `currentWeapon` 变量。

# --hints--

你的 `sellWeapon` 函数应该使用 `shift()` 方法。

```js
assert.match(sellWeapon.toString(), /shift\(\s*\)/);
```

你应该在 `inventory` 数组上使用 `shift()` 方法。

```js
assert.match(sellWeapon.toString(), /inventory\.shift\(\s*\)/);
```

你应该将 `inventory.shift()` 的值分配给你的 `currentWeapon` 变量。
