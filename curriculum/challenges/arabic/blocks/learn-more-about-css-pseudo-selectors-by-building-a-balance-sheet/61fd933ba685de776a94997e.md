...
داخل أول `tr`، أضف عنصر `th` بالنص `Loans The outstanding balance on our startup loan.`. لف هذا النص، عدا `Loans `، داخل عنصر `span` مع تعيين `class` إلى `description`.

أضف ثلاثة عناصر `td` أسفله، وأعطها النصوص التالية بالترتيب: `$500`، `$250`، و`$0`. عيّن للعنصر الثالث `td` الخاصية `class` بقيمة `current`.
