...
La tarjeta de precios premium debe ser la que más destaque en la página. Además, es la única tarjeta que tendrá una insignia `Most popular`. Para hacer que esta tarjeta y su insignia destaquen, puedes usar las clases de utilidad de posicionamiento relativo y escalado de Tailwind.

Aquí tienes cómo hacer un elemento más grande con escalado en Tailwind CSS:
...
Te encargarás del posicionamiento en los próximos pasos. Por ahora, asigna la clase `scale-105` al `div` contenedor para hacerlo un poco más grande que las otras tarjetas de precios.

# --hints--

Tu elemento `div` contenedor para el plan premium debe tener la clase `scale-105`.
