...
Ora che il setter `salary` è stato creato, aggiorna `__init__` per chiamarlo invece di assegnare direttamente a `self._salary`. Cambia `self._salary = Employee._base_salaries[level]` in `self.salary = Employee._base_salaries[level]`.

# --hints--

Il tuo metodo `__init__` dovrebbe usare `self.salary = Employee._base_salaries[level]` invece di `self._salary = Employee._base_salaries[level]`.
