...
Erstellen Sie eine Funktion namens `setPlayerDisplay`. Innerhalb der Funktion setzen Sie die `textContent` von `playingSong` auf den Titel des aktuellen Songs, falls definiert. Andernfalls setzen Sie sie auf einen leeren String.

Darunter setzen Sie die `textContent` von `songArtist` auf den Künstler des aktuellen Songs, falls definiert. Andernfalls setzen Sie sie auf einen leeren String.


# --hints--

Sie sollten eine Funktion namens `setPlayerDisplay` haben.

```js
assert.isFunction(setPlayerDisplay);
```

Ihre `setPlayerDisplay`-Funktion sollte die `textContent` von `playingSong` auf den Titel des aktuellen Songs setzen, falls definiert, oder auf einen leeren String.
...
Ihre `setPlayerDisplay`-Funktion sollte die `textContent` von `songArtist` auf den Künstler des aktuellen Songs setzen, falls definiert, oder auf einen leeren String.
