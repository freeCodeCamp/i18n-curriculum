...
Da Benutzer Songs in der Wiedergabeliste mischen und löschen können, müssen Sie eine Kopie des `allSongs`-Arrays erstellen, ohne das Original zu verändern. Hier kommt der `spread`-Operator ins Spiel.

Der Spread-Operator (`...`) ermöglicht es Ihnen, alle Elemente eines Arrays in ein anderes zu kopieren. Er kann auch verwendet werden, um mehrere Arrays zu einem einzigen zusammenzufügen. Im folgenden Beispiel wurden sowohl `arr1` als auch `arr2` in `combinedArr` gespreadet:
...
Erstellen Sie im `userData`-Objekt eine `songs`-Eigenschaft. Für den Wert spreaden Sie `allSongs` in ein Array. 

# --hints--

Ihr `userData`-Objekt sollte einen Schlüssel namens `songs` haben.

```js
assert.property(userData, "songs");
```

Ihre `songs`-Eigenschaft sollte den Wert `[...allSongs]` haben.
