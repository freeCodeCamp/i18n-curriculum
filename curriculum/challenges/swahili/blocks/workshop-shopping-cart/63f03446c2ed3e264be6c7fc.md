...
Anza kwa kubadilisha thamani ya `isCartShowing`. Kumbuka kwamba unaweza kutumia kielekezi cha mantiki cha not `!` kubadilisha thamani ya thamani ya kweli au siyo kweli. Weka thamani iliyobadilishwa kwa `isCartShowing`.

# --hints--

Unapaswa kubadilisha thamani ya `isCartShowing`.

```js
assert.match(code, /!isCartShowing/);
```

Unapaswa kuweka thamani iliyobadilishwa ya `isCartShowing` kwa `isCartShowing`.
