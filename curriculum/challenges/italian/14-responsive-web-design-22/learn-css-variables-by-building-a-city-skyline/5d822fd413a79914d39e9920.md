...
Dovresti dare a `.fb2a` una proprietà `border-bottom` con il valore `10vh solid var(--building-color3)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderBottom.trim(), "10vh solid var(--building-color3)");
```

Dovresti dare a `.fb2a` una proprietà `border-left` con il valore `1vw solid transparent`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderLeft, "1vw solid transparent");
```

Dovresti dare a `.fb2a` una proprietà `border-right` con il valore `1vw solid transparent`.
