...
Modifica la tua chiamata `console.log` per registrare il valore selezionato dal menu a discesa.

Puoi consultare il valore corrente di un elemento `select` usando la sua proprietà `.value`. Per esempio:
...
Dovresti registrare il valore selezionato dal menu a discesa quando viene attivato un evento `change` su `selectContainer`.
