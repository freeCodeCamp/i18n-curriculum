...
CSV（カンマ区切り値）は、スプレッドシートやデータのインポートに使われるフォーマットです。各行は1つのエントリーを表し、フィールドはカンマで区切られています。

まず、テスト用だった`console.log(exportToJSON(...))`の呼び出しを削除してください。次に、`exportToCSV`をパラメータに取る`catalog`という名前の関数を作成してください。その中で、`const header = "Title,Author,Year,Location"`と`const rows = []`を宣言してください。

# --hints--

`exportToJSON`のconsole.log呼び出しは削除してください。

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

`exportToCSV`というパラメータを持つ`catalog`という名前の関数を作成してください。
...
`header`という名前で値が`"Title,Author,Year,Location"`の変数を宣言してください。

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

空の配列に設定された`rows`という名前の変数を宣言してください。
