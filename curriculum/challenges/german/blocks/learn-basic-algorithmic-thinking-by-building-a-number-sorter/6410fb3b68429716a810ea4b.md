...
Ein Selection Sort basiert darauf, den Index des kleinsten Werts im Array zu verfolgen. Deklarieren Sie eine Variable `minIndex` und setzen Sie sie auf `i` – so wird sichergestellt, dass, wenn Ihr aktueller Wert der kleinste ist, er mit sich selbst getauscht wird und nicht verschoben wird. Sie müssen in der Lage sein, den Wert von `minIndex` neu zuzuweisen, während Sie das Array durchlaufen.

Schreiben Sie dann eine weitere `for`-Schleife, die `j` als Iterator verwendet. Diese Schleife muss am Index nach `i` starten und den Rest des Arrays durchlaufen.

# --hints--

Sie sollten `let` verwenden, um `minIndex` zu deklarieren.
...
Sie sollten `minIndex` auf `i` setzen.
...
Sie sollten eine verschachtelte `for`-Schleife haben.
...
Ihre `for`-Schleife sollte `j` mit `i + 1` initialisieren.
...
Ihre `for`-Schleife sollte den Rest des Arrays durchlaufen.
...
Ihre `for`-Schleife sollte `j` inkrementieren.
