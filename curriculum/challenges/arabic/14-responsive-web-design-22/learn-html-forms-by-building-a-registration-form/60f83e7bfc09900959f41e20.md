...
أضف تحققًا مخصصًا (custom validation) إلى عنصر `input` لكلمة المرور، عن طريق إضافة سمة `minlength` بقيمة `8`. يؤدي القيام بذلك إلى منع إرسال المدخلات التي تقل عن 8 رموز.

# --hints--

يجب أن تعطي عنصر `input` لكلمة المرور السمة `minlength`.

```js
assert.notEqual(document.querySelector('input[type="password"]')?.minLength, -1);
```

يجب أن تعطي السمة `minlength` قيمة `8`.
