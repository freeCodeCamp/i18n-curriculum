...
向你的 `buyWeapon` 函數添加 `else` 語句。 在該語句中，將 `text.innerText` 設置爲等於 `"You do not have enough gold to buy a weapon."`。

# --hints--

你應該向 `buyWeapon` 函數添加 `else` 語句。

```js
assert.match(buyWeapon.toString(), /else/);
```

你的 `else` 語句應位於 `if` 語句之後。
...
你應該將 `text.innerText` 設置爲 `"You do not have enough gold to buy a weapon."`。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You do not have enough gold to buy a weapon.\1/);
```

你的 `else` 語句應將 `text.innerText` 設置爲 `"You do not have enough gold to buy a weapon."`。
