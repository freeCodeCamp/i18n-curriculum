...
Коли ваша умова `if` істинна, потрібно поміняти місцями два елементи, «спливаючи» більший елемент до кінця масиву.

Для цього оголосіть змінну `temp` і призначте їй значення `array[j]`. Потім призначте `array[j]` значення `array[j + 1]`. Нарешті, призначте `array[j + 1]` значення `temp`.

# --hints--

У межах вашої інструкції `if` слід оголосити змінну `temp`.
...
Ви повинні призначити `temp` значення `array[j]`.
...
Ви повинні призначити `array[j]` значення `array[j + 1]`.
...
Ви повинні призначити `array[j + 1]` значення `temp`.
