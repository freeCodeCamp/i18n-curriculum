...
`Sort` ボタンはクリックすると正しく動作しているように見えますが、これは配列内のすべての値が一桁の数字であるためで、より複雑な値ではソートが期待通りに動作しない場合があります。

`value` と `option` である `selected` 要素のテキストを `1` から `10` に変更し、もう一度 `Sort` ボタンをクリックしてください。

# --hints--

`value` である `option` 要素の `selected` を `10` に更新する必要があります。
...
`option` である `selected` 要素のテキストを `10` に更新してください。
