...
يجب أن يكون لديك قائمة من منتقيات `.one, .two`.
...
يجب عليك تعيين خاصية `filter` إلى `blur(1px)`.
...
يجب أن يكون لعنصرك `.one` خاصية `filter` بقيمة `blur(1px)`.
...
يجب أن يكون لعنصرك `.two` خاصية `filter` بقيمة `blur(1px)`.
