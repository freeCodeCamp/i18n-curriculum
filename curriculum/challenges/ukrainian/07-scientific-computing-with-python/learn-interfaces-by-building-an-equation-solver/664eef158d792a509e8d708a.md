...
З функції `__str__` поверніть виклик `sub()`, передаючи рядок `'1'`, порожній рядок та наявний виклик `equation_string.strip('+')` як аргументи. Це замінить кожен `1` порожнім рядком. Результат ще не удосконалено, і ви продовжите працювати над шаблоном регулярних виразів у наступних кроках.
