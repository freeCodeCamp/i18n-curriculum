...
Crie um seletor `.quote`.Dê uma propriedade `color` definida como `#00beef`, uma propriedade `font-size` definida como `2.4rem` e uma propriedade `text-align` definida como `center`.

# --hints--

Você deve ter um seletor `.quote`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote'));
```

O seletor `.quote` deve ter a propriedade `color` definida como `#00beef`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.color, 'rgb(0, 190, 239)');
```

O seletor `.quote` deve ter a propriedade `font-size` definida como `2.4rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize, '2.4rem');
```

O seletor `.quote` deve ter a propriedade `text-align` definida como `center`.
