...
**Listener plan** 内の `h2` に移動し、それを大きくするためにユーティリティクラス `text-lg`、太字にするために `font-semibold`、美しいインディゴ色にするために `text-indigo-600` を付けてください。

# --hints--

`h2` 要素にはクラス `text-lg` を付けてください。
...
`h2` 要素にはクラス `font-semibold` を付けてください。
...
`h2` 要素にはクラス `text-indigo-600` を付けてください。
