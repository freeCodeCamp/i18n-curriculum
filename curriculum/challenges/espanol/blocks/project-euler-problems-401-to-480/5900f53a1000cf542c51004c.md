...
De manera notable, `f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3.1415926… ≈ π.
...
`almostPi` debe ser una función.

```js
assert.isFunction(almostPi);
```

`almostPi` debe devolver un número.

```js
assert.strictEqual(typeof almostPi(10), 'number');
```

`almostPi(29)` debe devolver `1208`.

```js
assert.strictEqual(almostPi(29), 1208);
```

`almostPi(50)` debe devolver `4152`.

```js
assert.strictEqual(almostPi(50), 4152);
```

`almostPi(200)` debe devolver `64658`.
