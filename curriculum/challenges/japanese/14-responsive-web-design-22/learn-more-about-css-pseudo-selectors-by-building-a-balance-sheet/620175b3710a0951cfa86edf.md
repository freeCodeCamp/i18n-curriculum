...
`#years` の `margin` を `0 -2px` に、`padding` を `0.5rem calc(1.25rem + 2px) 0.5rem 0` に設定してください。

# --hints--

`#years` セレクターの `margin` プロパティを `0 -2px` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('margin') === '0px -2px');
```

`#years` セレクターの `padding` プロパティを `0.5rem calc(1.25rem + 2px) 0.5rem 0` に設定する必要があります。
