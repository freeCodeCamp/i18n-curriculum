...
曲が一時停止されたときの現在の再生時間を保存するには、`songCurrentTime`変数の`userData`を`currentTime`オブジェクトの`audio`に代入してください。

*注意*: このステップではオプショナルチェーンを使わないでください。なぜなら、この時点で`userData.songCurrentTime`は`null`や`undefined`ではないからです。
...
`audio.currentTime`に`userData.songCurrentTime`を代入してください。
