...
`label` のテキストをフォーム要素の上に表示するとより見やすくなります。

`input`、`textarea`、`select` のすべての要素を選択し、それらが親要素の幅いっぱいに広がるようにしてください。

また、選択した要素の上部に `10px` の `margin` を追加し、他のマージンは `0` に設定してください。

# --hints--

`input`、`textarea`、`select` の要素を選択するには、カンマ区切りの要素セレクターを使うべきです。
...
`margin-top` プロパティを `10px` に設定してください。
...
`margin-bottom` プロパティを `0` に設定してください。
...
`margin-left` プロパティを `0` に設定してください。
...
`margin-right` プロパティを `0` に設定してください。
