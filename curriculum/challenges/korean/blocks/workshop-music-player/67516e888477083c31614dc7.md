...
자세히 보면, 현재 재생 중인 노래가 재생 목록에서 강조 표시되지 않아 어떤 노래가 재생 중인지 알 수 없습니다. 재생 중인 노래를 강조 표시하는 함수를 만들어 이 문제를 해결할 수 있습니다.

`highlightCurrentSong`라는 이름의 함수를 만드세요. 함수 내부에서 `.playlist-song`를 가진 `aria-current="true"` 요소가 있는지 확인하세요. 있다면 `removeAttribute` 메서드를 사용해 그 요소에서 `aria-current` 속성을 제거하세요.

# --hints--

`highlightCurrentSong`라는 이름의 함수가 있어야 합니다.

```js
assert.isFunction(highlightCurrentSong)
```

`highlightCurrentSong` 함수는 `removeAttribute`를 가진 `aria-current` 요소가 존재하면 `.playlist-song` 메서드를 사용해 그 요소에서 `aria-current="true"` 속성을 제거해야 합니다.
