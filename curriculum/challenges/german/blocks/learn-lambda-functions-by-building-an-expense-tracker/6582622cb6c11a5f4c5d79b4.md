...
Noch innerhalb der `elif`-Klausel übergeben Sie den `expenses_from_category`-Iterator an einen `print_expenses`-Aufruf.

# --hints--

Sie sollten `print_expenses()` aufrufen und `expenses_from_category` als Argument in Ihrer `elif`-Klausel übergeben.
...
Ihr `print_expenses()`-Aufruf sollte nach der `expenses_from_category`-Variablen erfolgen.
