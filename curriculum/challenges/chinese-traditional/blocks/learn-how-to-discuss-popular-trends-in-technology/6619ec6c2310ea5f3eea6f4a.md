...
當你將 `it is` 與形容詞（例如 `hard`、`easy`、`simple` 等）結合時，接在這個組合後的動詞必須使用 `to + infinitive` 形式。範例：`It is easy to study for English tests.`（在形容詞 `easy` 之後，你必須在 `study` 前加上 `to`）。在疑問句中，將 `it` 和 `is` 顛倒。範例：`Is it simple to learn to play an instrument?`
