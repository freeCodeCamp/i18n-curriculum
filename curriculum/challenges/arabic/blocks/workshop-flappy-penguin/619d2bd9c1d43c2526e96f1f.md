...
داخل المحدِّد `.arm.left`، غيّر نقطة الأصل للدالة `transform` لتكون الزاوية العلوية اليسرى لعنصر الأصل.

# --hints--

يجب استخدام الخاصية `transform-origin` للقيام بذلك.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyleAny(['.arm.left', '.left.arm'])?.transformOrigin);
```

يجب أن تعطي `.arm.left` خاصية `transform-origin` بقيمة `0% 0%` أو `top left`.
