...
Ora è il momento di impostare la scheda cliccata come selezionata.

Usa il metodo `setAttribute` su `tab` per impostare l'attributo `aria-selected` su `"true"`. 

# --hints--

Quando si clicca su `tab`, l'attributo `aria-selected` della scheda cliccata dovrebbe essere impostato su `"true"`.
