...
Es wird sechs Kontrollkästchen geben, daher ist es am besten, auch ein Array für sie zu erstellen und dieses zu durchlaufen, um sie als Kontrollkästchen anzuzeigen. Dafür wurde Ihnen ein `powersOptions`-Array bereitgestellt.

Verwenden Sie eine Pfeilfunktion, um das `powersOptions`-Array mit dem Parameter `power` zu durchlaufen. Erstellen Sie innerhalb des Durchlaufs ein `label`-Element mit einem `key` von `power` und einem Text von `Hello`.

Danach sollten Sie sechs `Hello`-Texte sehen. Die Elemente des Arrays füllen Sie im nächsten Schritt aus.

# --hints--

Sie sollten das `powersOptions` durchlaufen.

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\(/)
```

Sie sollten `power` als Parameter Ihres Durchlaufs verwenden.

```js
assert.match(code, /{\s*powersOptions\s*\.\s*map\s*\(\s*(power|\(\s*power\s*\))\s*=>/)
```

Sie sollten ein `label`-Element mit einem `key` von `power` erstellen.

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\([\s\S]*<\s*label\s+key\s*=\s*\{\s*power\s*\}\s*>[\s\S]*<\s*\/\s*label\s*>/)
```

Ihr `label`-Element sollte den Text `Hello` haben.
