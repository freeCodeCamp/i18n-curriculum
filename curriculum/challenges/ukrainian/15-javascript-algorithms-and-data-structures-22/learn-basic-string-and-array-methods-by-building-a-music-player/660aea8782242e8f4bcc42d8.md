...
Якщо функція має лише один параметр, ви можете опустити дужки навколо списку параметрів таким чином:
...
Створіть нову стрілкову функцію під назвою `printMessage`, яка має один параметр під назвою `org`. Усередині тіла цієї функції додайте консольну інструкцію. У цю консольну інструкцію додайте шаблонний літерал `${org} is awesome!`.

Під функцією `printMessage` викличте функцію та передайте рядок `"freeCodeCamp"` як аргумент.
...
Використайте синтаксис стрілкових функцій.

```js
assert.match(code, /(let|const)\s+printMessage\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Функція `printMessage` повинна мати параметр під назвою `org`.
...
Функція `printMessage` повинна мати консольну інструкцію.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

Консольна інструкція має містити шаблонний літерал `${org} is awesome!`.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

Викличте функцію `printMessage` та передайте рядок `"freeCodeCamp"`.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

Виклик функції має бути нижче визначення функції `printMessage`.
