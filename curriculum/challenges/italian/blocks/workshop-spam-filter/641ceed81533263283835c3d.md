...
Invece di utilizzare il metodo `.match()`, puoi utilizzare il metodo `.test()` di un'espressione regolare per verificare se una stringa corrisponde al pattern. A differenza di `.match()`, `.test()` restituisce un valore booleano che indica se la stringa corrisponde o meno al pattern.

Update your `isSpam()` function to use the `.test()` method on `helpRegex` to test if `msg` is a match.

Quindi, prova a inserire alcuni messaggi sulla pagina e vedere il risultato.
