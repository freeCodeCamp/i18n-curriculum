...
在 `href` 屬性之後，將 `class` 屬性設定為名為 `linkClass` 的常數。 

在 `class` 屬性之後，將 `target` 屬性設為 `"_blank"`。 

最後，將 `linkText` 常數放置在錨點標籤之間，以顯示聯結中的文字。

# --hints--

在 `href` 屬性之後，你應該有一個 `class` 屬性。
...
你應該將 `class` 屬性的值設定為名為 `linkClass` 的常數。
