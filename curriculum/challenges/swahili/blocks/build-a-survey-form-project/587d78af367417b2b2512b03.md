...
**Lengo:** Jenga programu inayofanya kazi kama <a href="https://survey-form.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://survey-form.freecodecamp.rocks</a>. **Usikopi mradi huu wa maonyesho**.
...
1. Ndani ya kipengele cha fomu, unahitajika kuingiza jina lako katika sehemu ya ingizo ya `input` yenye `id` ya `name` na `type` ya `text`
1. Ndani ya kipengele cha fomu, unahitajika kuingiza barua pepe yako katika sehemu ya ingizo ya `input` yenye `id` ya `email`
1. Ukichukua barua pepe isiyo na muundo sahihi, utaona kosa la uthibitisho la HTML5
1. Ndani ya fomu, unaweza kuingiza nambari katika sehemu ya ingizo ya `input` yenye `id` ya `number`
1. Sehemu ya ingizo ya nambari haipaswi kuruhusu kuingiza vitu visivyo nambari, iwe kwa kukuzuia kuandika au kwa kuonyesha kosa la uthibitisho la HTML5 (kulingana na kivinjari chako).
1. Ukichukua nambari zisizo ndani ya anuwai ya sehemu ya ingizo ya nambari, zilizoainishwa na sifa za `min` na `max`, utaona kosa la uthibitisho la HTML5
1. Kwa sehemu za ingizo za jina, barua pepe, na nambari, unaweza kuona vipengele vinavyolingana vya `label` katika fomu, vinavyoelezea madhumuni ya kila sehemu kwa vitambulisho vifuatavyo: `id="name-label"`, `id="email-label"`, na `id="number-label"`
1. Kwa sehemu za ingizo za jina, barua pepe, na nambari, unaweza kuona maandishi ya kiwakilishi yanayotoa maelezo au maelekezo kwa kila sehemu
...
1. Ndani ya kipengele cha fomu, unaweza kuchagua sehemu kadhaa kutoka kwa mfululizo wa visanduku vya kuchagua, kila kikiwa na sifa ya `value`
1. Ndani ya kipengele cha fomu, unawasilishwa na sehemu ya maoni ya ziada ya `textarea`
1. Ndani ya kipengele cha fomu, unawasilishwa na kitufe chenye `id` ya `submit` ili kuwasilisha ingizo zote

Kamilisha hadithi za watumizi na pita vipimo vyote hapa chini ili kumaliza mradi huu. Toa mtindo wako binafsi. Furahia kuandika msimbo!

**Kumbuka:** Hakikisha unaongeza `<link rel="stylesheet" href="styles.css">` katika HTML yako kuunganisha karatasi yako ya mitindo na kutumia CSS yako
