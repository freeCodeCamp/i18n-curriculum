...
تُغيّر خصائص الرابط عندما يتم فعليًا زيارة الرابط باستخدام <dfn>محدِّد زائف</dfn> يشبه `a:visited { propertyName: propertyValue; }`.

غيّر لون رابط تذييل `Visit our website` إلى اللون `grey` عندما يزور المستخدم الرابط.

# --hints--

يجب أن تستخدم المحدِّد الزائف `a:visited`.
...
يجب أن تضبط خاصية `color` على `grey`.
...
يجب أن يكون للمحدد الزائف `a:visited` خاصية `color` بقيمة `grey`.
