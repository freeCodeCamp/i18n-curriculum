...
이 워크숍에서는 이전 수업에서 배운 함수형 프로그래밍 개념을 적용해 재료 변환기 앱을 만듭니다.

HTML과 CSS는 이미 제공되어 있습니다.

빈 `conversionTable` 객체를 선언하는 것부터 시작하세요.

# --hints--

`conversionTable` 변수를 선언해야 합니다.

```js
assert.isDefined(conversionTable);
```

`conversionTable` 변수는 객체여야 합니다.

```js
assert.isObject(conversionTable);
```

`conversionTable` 객체는 비어 있어야 합니다.
