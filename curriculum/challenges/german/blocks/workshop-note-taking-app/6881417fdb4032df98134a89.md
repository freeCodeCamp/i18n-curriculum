...
Die verbleibenden Schritte für den Workshop beinhalten das Hinzufügen des Codes, um die Nachricht „Notizen gespeichert“ dynamisch anzuzeigen und die aktuellen Notizen in der Konsole zu protokollieren.

Beginnen Sie damit, eine Variable namens `noteEl` zu erstellen und ihr das Ergebnis der Abfrage des Dokuments nach dem Element mit dem `id` von `note` zuzuweisen.

Erstellen Sie dann eine weitere Variable namens `statusEl` und weisen Sie ihr das Ergebnis der Abfrage des Dokuments nach dem Element mit dem `id` von `status` zu.

# --hints--

Sie sollten eine `noteEl`-Variable haben und ihr das Ergebnis der Abfrage des Dokuments nach dem Element mit dem `id` von `note` zuweisen.

```js
assert.equal(noteEl, document.getElementById("note"));
``` 

Sie sollten eine `statusEl`-Variable haben und ihr das Ergebnis der Abfrage des Dokuments nach dem Element mit dem `id` von `status` zuweisen.
