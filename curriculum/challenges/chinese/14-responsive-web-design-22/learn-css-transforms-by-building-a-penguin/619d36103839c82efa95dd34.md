...
应该设置 `.penguin` 的 `transition-duration` 值为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionDuration, '1s');
```

应该设置 `.penguin` 的 `transition-timing-function` 为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionTimingFunction, 'ease-in-out');
```

应该设置 `.penguin` 的 `transition-delay` 为 `--fcc-expected--`，而不是 `--fcc-actual--`。
