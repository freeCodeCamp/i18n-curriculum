...
1. Unapaswa kuona kipengele cha `a` chenye `id="tribute-link"` sambamba, ambacho kinaunganishwa na tovuti ya nje, ambacho kina maelezo ya ziada kuhusu mada ya ukurasa wa kushukuru. HINT: You must give your element an attribute of `target` and set it to `_blank` in order for your link to open in a new tab.
