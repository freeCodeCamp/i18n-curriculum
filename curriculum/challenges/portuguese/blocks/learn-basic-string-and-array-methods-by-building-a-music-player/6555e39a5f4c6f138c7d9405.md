...
Em etapas anteriores, você aprendeu como trabalhar com o método `sort()` para ordenar as músicas em ordem alfabética. Outro caso de uso para a função callback é randomizar um array. 

Uma maneira de randomizar um array de itens seria subtrair `0.5` de `Math.random()`, que produz valores aleatórios que são positivos ou negativos. Isso faz com que o resultado da comparação seja uma mistura de valores positivos e negativos, levando a uma ordenação aleatória dos elementos.
...
Use o método `sort()` no array `userData?.songs`. Passe um callback para o método e retorne o resultado de `Math.random() - 0.5`.


# --hints--

Você não deve modificar a função `shuffle` existente.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

Você deve usar o método `sort()` em `userData?.songs`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

Seu método `sort()` deve ter uma função callback que use a sintaxe de arrow.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

O callback do seu método `sort()` deve retornar `Math.random() - 0.5`.
