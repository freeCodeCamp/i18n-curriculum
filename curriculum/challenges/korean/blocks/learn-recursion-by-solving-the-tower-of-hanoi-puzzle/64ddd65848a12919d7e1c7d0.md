...
`move()` 함수에서 `make_allowed_move()` 함수로 코드를 옮길 시간입니다. 첫 번째 `if` 문 안에 중첩된 코드(첫 번째 `print()` 호출은 제외)를 새 함수로 옮기세요. 들여쓰기에 특히 주의하세요.
`pass` 키워드를 제거하는 것도 잊지 마세요.

# --hints--

첫 번째 `if` 문 안에 중첩된 코드(첫 번째 `print()` 호출은 제외)를 새 함수로 옮겨야 합니다.
