...
`addCharacters(char1)` також повертає функцію, яка повертає ще одну функцію. Зробіть ще один виклик функції, щоб отримати доступ до цього внутрішнього посилання на функцію для зворотного виклику `.map()`. JavaScript дозволяє негайно викликати повернуті функції:

```js
myFunc(1)("hi");
```

Негайно викличте функцію, повернуту з виклику `addCharacters(char1)`, та передайте `char2` як аргумент.

# --hints--

Створіть ланцюжок з викликом функції до виклику `addCharacters(char1)`.
...
Передайте `char2` як аргумент до виклику функції у ланцюжку.
