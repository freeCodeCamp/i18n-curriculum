...
استدعِ الطريقة `.map()` على استدعائك لـ `rangeFromString()`، مع تمرير مرجع إلى `addCharacters` كدالة استدعاء راجع.

# --hints--

يجب أن تستدعي الطريقة `.map()` على استدعائك لـ `rangeFromString()`.
...
يجب أن تمرر مرجعًا إلى `addCharacters` كدالة استدعاء راجع لطريقتك `.map()`.
