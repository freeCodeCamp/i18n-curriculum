...
Dado que los usuarios podrán mezclar y eliminar canciones de la lista de reproducción, necesitarás crear una copia del arreglo `allSongs` sin modificar el original. Aquí es donde el operador `spread` resulta útil.

El operador de propagación (`...`) te permite copiar todos los elementos de un arreglo a otro. También puede usarse para concatenar múltiples arreglos en uno. En el ejemplo siguiente, tanto `arr1` como `arr2` se han propagado en `combinedArr`:
...
Dentro del objeto `userData` crea una propiedad `songs`. Para el valor, propaga `allSongs` en un arreglo.

# --hints--

Tu objeto `userData` debe tener una clave llamada `songs`.

```js
assert.property(userData, "songs");
```

Tu propiedad `songs` debe tener un valor de `[...allSongs]`.
