...
У вашому об’єкті `userData` створіть властивість `songs` і встановіть її значення у масив `allSongs`.

Потім, щоб обробляти інформацію про поточну пісню та відстежувати час її відтворення, створіть властивості `currentSong` і `songCurrentTime`. Встановіть їхні значення відповідно у `null` і `0`.

# --hints--

Ваш об’єкт `userData` має містити ключ `songs`, встановлений у масив `allSongs`.

```js
assert.propertyVal(userData, "songs", allSongs);
```

Ваш об’єкт `userData` має містити ключ `currentSong`, встановлений у `null`.

```js
assert.propertyVal(userData, "currentSong", null);
```

Ваш об’єкт `userData` має містити ключ `songCurrentTime`, встановлений у `0`.
