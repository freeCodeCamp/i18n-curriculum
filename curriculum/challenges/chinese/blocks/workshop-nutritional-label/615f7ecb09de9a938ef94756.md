...
在最后一个 `p` 元素下方，添加另一个带有文本 `Total Carbohydrate 37g 13%` 的 `p` 元素。 像之前一样，使用 `span` 元素使文本 `Total Carbohydrate` 和 `13%` 加粗。 然后，将营养成分和数量放在一个 `span` 中，以便对齐。
...
你的新的 `p` 元素应包含文本 `Total Carbohydrate 37g 13%`。
...
`Total Carbohydrate` 周围的 `span` 元素应该将 `class` 属性设置为 `bold`。
...
`13%` 周围的 `span` 元素应该将 `class` 属性设置为 `bold`。
