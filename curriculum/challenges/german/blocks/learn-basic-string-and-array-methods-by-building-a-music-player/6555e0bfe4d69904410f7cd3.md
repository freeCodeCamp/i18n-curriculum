...
Verwenden Sie `const` und Pfeilsyntax, um eine leere Funktion namens `shuffle` zu erstellen.

Diese Funktion ist dafür verantwortlich, die Songs in der Wiedergabeliste zu mischen und nach dem Mischen die notwendigen Aktualisierungen der Zustandsverwaltung durchzuführen.

# --hints--

Sie sollten `const` verwenden, um eine leere Funktion namens `shuffle` zu erstellen.

```js
assert.match(code, /const\s+shuffle\s*=\s*/)
```

`shuffle` sollte eine Funktion sein.

```js
assert.isFunction(shuffle)
```

Ihre `shuffle`-Funktion sollte eine Pfeilsyntax verwenden und keinen Parameter entgegennehmen.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

Ihre `shuffle`-Funktion sollte leer sein.
