...
為你的 `previousButton` 元素添加一個 `"click"` 事件監聽器，然後將 `playPreviousSong` 作為第二個引數傳入。

# --hints--

你應該在你的 `previousButton` 變數上呼叫 `addEventListener()` 方法。
...
你應該將 `playPreviousSong` 作為你的 `addEventListener` 方法的第二個引數傳入。
