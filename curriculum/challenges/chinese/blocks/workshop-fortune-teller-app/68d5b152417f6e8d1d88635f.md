...
还有更多属性需要添加到 `Card` 接口。首先将 `value_int` 设置为 `number` 类型。所有其他属性都应为 `string` 类型：`suit`、`type`、`img`、`meaning_up`、`meaning_rev` 和 `desc`。

# --hints--

你应该有一个属性 `value_int`，类型为 `number`。
...
你应该有一个属性 `meaning_up`，类型为 `string`。
...
你应该有一个属性 `meaning_rev`，类型为 `string`。
