...
在 `if` 语句之外，将 `number` 添加到 `sum_of_even_digits`。 另外，删除 `print` 调用。

# --hints--

`for` 循环中应该有 `sum_of_even_digits += number`。
...
`if` 语句中不应包含 `sum_of_even_digits += number`。
