...
Тепер, коли ви створили кнопку, потрібно призначити до неї текст. Для цього вам потрібно використати метод DOM `createTextNode()`.

Метод `createTextNode()` використовують для створення текстового вузла. Щоб використати його, ви викликаєте його та передаєте текст як рядок:
...
Використайте метод `createTextNode()`, щоб створити текст `"Reset Playlist"`, а потім призначте його до константи `resetText`.
...
Ви повинні використати `document.createTextNode()`, щоб створити текст `"Reset Playlist"`.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

Призначте створений текстовий вузол до константи `resetText`.
