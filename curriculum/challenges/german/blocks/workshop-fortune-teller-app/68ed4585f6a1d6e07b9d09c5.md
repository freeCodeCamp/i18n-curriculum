...
Deklarieren Sie innerhalb der `fetchCardsData` Methode eine Variable namens `response` und weisen Sie ihr den Wert zu, indem Sie die `fetch` Funktion mit der folgenden URL aufrufen: `` `${CDN_URL}/card_data.json` ``.

Fügen Sie das Schlüsselwort `await` vor dem `fetch` Aufruf hinzu und umschließen Sie die gesamte Logik mit einem `try...catch` Block.

# --hints--

Sie sollten eine `response` Variable innerhalb der `fetchCardsData` Methode deklarieren.
...
Ihre `response` Variable sollte durch den Aufruf von `await fetch` mit der URL `` `${CDN_URL}/card_data.json` `` zugewiesen werden.
...
Ihr `fetch` Aufruf sollte in einem `try...catch` Block umschlossen sein, mit `error` als `catch` Parameter.
