...
È buona pratica permettere agli utenti di aggiungere note o preoccupazioni aggiuntive.

Sotto il tuo terzo elemento `fieldset`, aggiungi un nuovo elemento `fieldset`. All’interno, aggiungi un elemento `legend` con il testo `Additional Notes`.


# --hints--

Dovresti avere un totale di 4 elementi `fieldset` in questa pagina.
...
Dovresti aggiungere un elemento `fieldset` sotto il terzo elemento `fieldset`.
...
Il nuovo elemento `fieldset` dovrebbe contenere un elemento `legend`.
...
L’elemento `legend` dovrebbe avere il testo `Additional Notes`.
