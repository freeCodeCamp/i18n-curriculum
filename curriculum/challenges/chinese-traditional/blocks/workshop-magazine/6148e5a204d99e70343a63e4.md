...
`gap` 屬性是同時設置 `column-gap` 和 `row-gap` 值的簡寫方式。 如果給定一個值，它會將 `column-gap` 和 `row-gap` 都設置爲該值。 如果給定兩個值，它會將 `row-gap` 設置爲第一個值，將 `column-gap` 設置爲第二個值。

給 `.image-wrapper` 選擇器一個 `gap` 屬性，設置爲 `2rem`。

# --hints--

你的 `.image-wrapper` 元素應該有一個 `gap` 屬性，設置爲 `2rem`。
