...
イベントリスナーに戻って、`result` 要素のテキストを更新します。 You can use a ternary operator to achieve this task.
...
`if` 文の後で、`isSpam()` に実引数として `messageInput.value` を渡して呼び出した結果が真値 (truthy) かどうかを、三項演算子を使ってチェックしてください。 If true, set the `textContent` property on the `result` element to `"Oh no! This looks like a spam message."`. Otherwise, set it to `"This message does not seem to contain any spam."`
