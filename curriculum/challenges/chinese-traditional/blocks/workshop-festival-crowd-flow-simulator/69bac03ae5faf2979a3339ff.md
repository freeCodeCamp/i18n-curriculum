...
既然你知道要重新導向到哪個閘門，就可以移動上限溢位的參加者。

取得 `overflowAmount` 並將它添加到下一個閘門在相同 `tickIndex` 的佇列中。

具體來說，你必須：

- 使用 `nextGateIndex` 存取下一個閘門。
- 存取它的 `queue`。
- 將 `overflowAmount` 添加到位置 `tickIndex` 的值中。

# --hints--

你的 `rerouteOverflow` 函式應該將 `overflowAmount` 添加到下一個閘門 `tickIndex` 中 `queue` 位置的值。
