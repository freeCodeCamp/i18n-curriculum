...
`.card` 要素にもう少しスタイルを追加してください：`border-radius` の `10px` と、値が `box-shadow` の `0 4px 8px gray` を追加してください。

# --hints--

`.card` セレクターには、値が `border-radius` のプロパティ `10px` を設定してください。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("border-radius"), "10px");
```

`.card` セレクターには、値が `box-shadow` のプロパティ `0 4px 8px gray` を設定してください。
