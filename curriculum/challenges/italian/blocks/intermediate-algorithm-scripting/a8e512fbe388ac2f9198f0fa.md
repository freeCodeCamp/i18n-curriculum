...
Crea una funzione che esamini un array di oggetti (primo argomento) e restituisca un array di tutti gli oggetti che hanno coppie nome e valore corrispondenti (secondo argomento). Ogni coppia nome e valore dell'oggetto sorgente deve essere presente nell'oggetto della collezione per poter essere inclusa nell'array restituito.

Per esempio, se il primo argomento è `[{ first: "Romeo", last: "Montague" }, { first: "Mercutio", last: null }, { first: "Tybalt", last: "Capulet" }]` e il secondo argomento è `{ last: "Capulet" }`, allora devi restituire il terzo oggetto dell'array (il primo argomento), perché contiene il nome e il suo valore, che sono stati passati come secondo argomento.
