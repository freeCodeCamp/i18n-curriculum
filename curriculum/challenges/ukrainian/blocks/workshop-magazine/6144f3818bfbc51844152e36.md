...
Використайте функцію `minmax`, щоб зробити ваші колонки адаптивними на будь-якому пристрої. Функція `minmax` приймає два аргументи: перший — мінімальне значення, другий — максимальне. Ці значення можуть бути довжиною, відсотком, `fr` або навіть ключовим словом, як `max-content`.

Обгорніть кожне з уже визначених значень властивості `grid-template-columns` у функцію `minmax`, використовуючи кожне значення як другий аргумент. Перший аргумент має бути `2rem`, `min-content` та `2rem` відповідно.

# --hints--

Ваш селектор `main` має мати властивість `grid-template-columns`, встановлену в `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`.
