...
`.eye.left` 内に `div` 要素を 1 個追加する必要がありますが、`--fcc-actual--` 個が見つかりました。

```js
assert.equal(document.querySelectorAll('.eye.left > div')?.length ?? 0, 1);
```

`.eye.right` 内に `div` 要素を 1 個追加する必要がありますが、`--fcc-actual--` 個が見つかりました。

```js
assert.equal(document.querySelectorAll('.eye.right > div')?.length ?? 0, 1);
```

1 つ目の新しい `div` に `eye-lid` という `class` を設定する必要があります。

```js
assert.exists(document.querySelector('.eye.left > div.eye-lid'));
```

2 つ目の新しい `div` に `eye-lid` という `class` を設定する必要があります。
