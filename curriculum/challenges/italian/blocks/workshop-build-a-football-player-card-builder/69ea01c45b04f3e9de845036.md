...
Il problema attuale è che una volta che l'anteprima si aggiorna o ricarichi la pagina, tutto ciò che aggiungi per la scheda del giocatore scompare.

Salvare tutti gli input nel localstorage impedirà questo. Per iniziare, crea una costante `STORAGE_KEY` con la stringa `football_player_card` come suo valore.

# --hints--

Dovresti dichiarare una variabile `STORAGE_KEY`.
...
La tua `STORAGE_KEY` dovrebbe avere la stringa `football_player_card` come valore.
