...
创建一个 `.daily-value p` 选择器，针对 `daily-value` 部分中的所有 `p` 元素。 给这个新的选择器添加一个 `border-bottom`，设置为 `1px solid #888989`。

# --hints--

你应该有一个新的 `.daily-value p` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

你的 `.daily-value p` 选择器应该有一个 `border-bottom` 属性，设置为 `1px solid #888989`。
