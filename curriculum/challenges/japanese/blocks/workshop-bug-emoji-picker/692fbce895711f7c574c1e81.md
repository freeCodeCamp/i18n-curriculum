...
`isSelect` 関数の中で、`element` が `HTMLSelectElement` のインスタンスかどうかを示すブール値を返してください。このチェックには `instanceof` 演算子を使います。

# --hints--

`isSelect` 関数は `element instanceof HTMLSelectElement` を返す必要があります。
