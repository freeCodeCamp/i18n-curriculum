...
Crie uma instância de `DiscountEngine` e armazene-a na variável `engine`. Depois disso, calcule o melhor preço usando o método `calculate_best_price` e armazene-o em uma variável chamada `best_price`.

# --hints--

Você deve criar uma instância de `DiscountEngine` com a lista `strategies`.
...
Você deve chamar `engine.calculate_best_price(product, user_tier)` e armazenar o resultado em `best_price`.
