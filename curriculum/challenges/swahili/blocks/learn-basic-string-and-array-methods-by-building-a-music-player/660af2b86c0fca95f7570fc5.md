...
Ikiwa kitendakazi cha mshale kinarejesha usemi wa kihesabu rahisi, unaweza kuondoa neno kuu la `return` na mabano ya mviringo ya `{}`. Hii huitwa kurejesha kwa njia isiyo wazi.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

Ikiwa kitendakazi chako cha mshale kina mistari mingi ya msimbo ndani ya mwili wa kitendakazi, basi unahitaji kutumia neno kuu la `return` na mabano ya mviringo ya `{}`.
...
Badilisha, au sahihisha, kitendakazi chako cha `addTwoNumbers` ili kuondoa neno kuu la `return` na mabano ya mviringo ya `{}`. Kitendakazi chako cha `addTwoNumbers` kinapaswa badala yake kutumia kurejesha kwa njia isiyo wazi.

Fungua konsoli ili kuhakikisha bado unapata matokeo sahihi.

# --hints--

Kitendakazi chako cha `addTwoNumbers` kilichosahihishwa kinapaswa kutumia kurejesha kwa njia isiyo wazi.
