...
Nelle lezioni precedenti, hai imparato a usare il metodo `filter()` che serve a restituire un nuovo array con i risultati filtrati.
...
Crea una funzione `getBooksByAuthor` con due parametri: un array di oggetti libro e una stringa con l'autore.

La funzione deve restituire un array che contiene i libri di un autore specifico.

# --hints--

Dovresti creare una funzione `getBooksByAuthor`.

```js
assert.isFunction(getBooksByAuthor);
```

La tua funzione `getBooksByAuthor` dovrebbe avere due parametri.

```js
assert.lengthOf(getBooksByAuthor, 2);
```

La tua funzione `getBooksByAuthor` dovrebbe restituire un array.

```js
assert.isArray(getBooksByAuthor(library, ''));
```

La tua funzione `getBooksByAuthor` dovrebbe restituire il numero corretto di libri per ciascuno degli autori.
