...
Assicurati che il link si apra in una nuova scheda aggiungendo un attributo `target` con il valore `_blank`.
...
Il nuovo elemento `p` dovrebbe essere annidato nell'elemento `footer`. Dovresti avere esattamente un elemento `p`.
...
Il nuovo elemento `a` dovrebbe avere l'attributo `target` impostato su `_blank`.
