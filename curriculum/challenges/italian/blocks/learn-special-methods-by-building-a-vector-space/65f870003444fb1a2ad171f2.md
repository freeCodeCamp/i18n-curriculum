...
Modifica le due chiamate `print` per stampare la norma dei tuoi vettori e verifica che il metodo `norm()` funzioni correttamente.

# --hints--

Dovresti stampare `v1.norm()`.

```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(v1.norm())")`)) })
```

Dovresti stampare `v2.norm()`.
