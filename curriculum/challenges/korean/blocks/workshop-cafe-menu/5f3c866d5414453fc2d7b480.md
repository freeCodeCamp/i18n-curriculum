...
기존 coffee/price 쌍 아래에, 각 요소 안에 두 개의 중첩된 `article` 요소가 있는 `p` 요소를 사용하여 다음 커피와 가격을 추가하세요.
...
이전과 마찬가지로, 첫 번째 `p` 요소의 텍스트에는 커피 맛을, 두 번째 `p` 요소의 텍스트에는 가격을 포함해야 합니다.

# --hints--

`article` 요소가 다섯 개 있어야 합니다.

```js
assert.lengthOf(document.querySelectorAll('article'),5);
```

각 `article` 요소는 두 개의 `p` 요소를 포함해야 합니다.
...
첫 번째 `article` 요소는 `p` 요소를 포함하며, 텍스트는 `French Vanilla` 와 `3.00` 이어야 합니다.
...
두 번째 `article` 요소는 `p` 요소를 포함하며, 텍스트는 `Caramel Macchiato` 와 `3.75` 이어야 합니다.
...
세 번째 `article` 요소는 `p` 요소를 포함하며, 텍스트는 `Pumpkin Spice` 와 `3.50` 이어야 합니다.
...
네 번째 `article` 요소는 `p` 요소를 포함하며, 텍스트는 `Hazelnut` 와 `4.00` 이어야 합니다.
...
다섯 번째 `article` 요소는 `p` 요소를 포함하며, 텍스트는 `Mocha` 와 `4.50` 이어야 합니다.
