...
现在添加一个 `div`，它的 `className` 是 `card-stats`。在其中，添加一个 `div`，它的 `className` 是 `stat-col`。在那个里面，添加三个 `div` 元素，每个都有 `className` 是 `stat-row`。

# --hints--

你应该在 `div` 后添加一个 `className`，它的 `card-stats` 是 `.card-name-strip`。

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

你应该在 `div` 内添加一个 `className`，它的 `stat-col` 是 `.card-stats`。

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

你应该在 `div` 内添加三个 `className` 元素，每个都有 `stat-row` 是 `.stat-col`。
