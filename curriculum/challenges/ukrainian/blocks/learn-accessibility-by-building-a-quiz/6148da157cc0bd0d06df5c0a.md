...
Щоб вирівняти блоки введення один з одним, створіть новий набір правил, який націлюється на всі `input` та `label` елементи всередині `.info` елемента, і встановіть властивість `display` у значення `inline-block`.

Також вирівняйте текст елемента `label` по правому краю.
...
Ви повинні задати елементам `input` та `label` властивість `display` зі значенням `inline-block`.
...
Ви повинні задати елементу `label` властивість `text-align` зі значенням `right`.
