...
Toa darasa la vitu la `bb4-window` `width` ya `18%`, `height` ya `90%`, na ongeza variable yako ya `--window-color4` kama `background-color`.

# --hints--

Unapaswa kumpa `.bb4-window` `width` ya `18%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

Unapaswa kumpa `.bb4-window` `height` ya `90%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

Unapaswa kumpa `.bb4-window` `background-color` ya `--window-color4`.
