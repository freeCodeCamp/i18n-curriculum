...
같은 패턴을 따라 `number`, `#dinner`, `#snacks` 요소에서 `#exercise` 입력을 쿼리하세요. 이전 두 개의 명명 규칙을 따라 변수에 할당하세요.

# --hints--

`dinnerNumberInputs` 변수를 선언하세요.

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

`document.querySelectorAll()`를 사용해 `#dinner input[type='number']`를 호출하세요.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` 호출 결과를 `dinnerNumberInputs`에 할당하세요.
