...
在你空的 `.reduce()` 回呼中，宣告一個變數 `difference` 並將其設定為 `el` 減去 `mean` 的值。然後宣告一個 `squared` 變數，並將其設定為 `difference` 的 2 次方的值。最後，傳回 `acc` 加上 `squared` 的值。

# --hints--

你的 `reduce` 回呼應該有一個 `difference` 變數。

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

你的 `difference` 變數應該設定為 `el` 減去 `mean` 的值。
...
你的 `squared` 變數應該設定為 `difference` 的 2 次方值。
...
你的 `reduce` 回呼應該傳回 `acc` 加上 `squared` 的值。
