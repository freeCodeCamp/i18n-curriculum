...
如果沒有適當的引號，引用就不能算是真正的引用。 你可以使用 CSS 僞選擇器添加它們。

創建一個 `.quote::before` 選擇器，將 `content` 屬性設置爲 `"`，並在其後添加一個空格。

另外，創建一個 `.quote::after` 選擇器，將 `content` 屬性設置爲 `"`，並在其前面添加一個空格。

# --hints--

你應該有一個 `.quote::before` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

你的 `.quote::before` 選擇器應該有一個 `content` 屬性，設置爲 `'" '`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content?.match(/\"\\"\s\"/));
```

你應該有一個 `.quote::after` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

你的 `.quote::after` 選擇器應該有一個 `content` 屬性，設置爲 `' "'`。
