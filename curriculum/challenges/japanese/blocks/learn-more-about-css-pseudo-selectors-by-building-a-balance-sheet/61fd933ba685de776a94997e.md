...
その下に `td` 要素を3つ追加し、それぞれに順番に `$500`、`$250`、`$0` のテキストを設定してください。3番目の `td` 要素には `class` が `current` に設定されている必要があります。
