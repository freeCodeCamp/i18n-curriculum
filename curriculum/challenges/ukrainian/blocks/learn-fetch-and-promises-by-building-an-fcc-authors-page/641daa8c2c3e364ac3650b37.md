...
Далі, між ім’ям і зображенням кожного автора та рештою деталей на картці майже немає відокремлення. Роздільник надасть карткам авторів чітку візуальну ієрархію.

Додайте елемент `div` над біографією автора і надайте йому `class` `"purple-divider"`.
...
Ви маєте створити елемент `div` перед вашим елементом `p`.

```js
await __helpers.retryingTest(() => document.querySelector('p')?.previousElementSibling?.tagName === 'DIV', "'div' element not found");
```

Ваш елемент `div` має мати множину `class`, встановлену в `"purple-divider"`.
