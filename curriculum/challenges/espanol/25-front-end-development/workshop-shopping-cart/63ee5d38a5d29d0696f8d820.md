...
En tu literal de plantilla, crea un elemento `div` con una clase de `dessert-card`. En ese `div`, crea un elemento `h2` y asígnale el texto de la variable `name`.
...
Tu elemento `div` debe tener una clase de `dessert-card`.
...
Tu elemento `h2` debe tener el texto de la variable `name`.

```js
assert.equal(document.querySelectorAll('h2')[0]?.textContent, 'Vanilla Cupcakes (6 Pack)');
```

Tu elemento `h2` debe estar dentro del elemento `div`.
