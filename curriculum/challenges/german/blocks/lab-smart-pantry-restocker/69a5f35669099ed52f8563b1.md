...
2. Sie sollten eine `planRestock(pantry, shipment)`-Funktion implementieren, die die aktuelle Vorratskammer mit der eingehenden Lieferung vergleicht und ein Array von Aktionen in der Form `{ type: "restock" | "discard" | "donate", item }` zurückgibt.

3. Sie sollten eine `groupByZone(actions)`-Funktion implementieren, die die Aktionen basierend auf der `zone`-Eigenschaft jedes Artikels in Lagerzonen gruppiert.
