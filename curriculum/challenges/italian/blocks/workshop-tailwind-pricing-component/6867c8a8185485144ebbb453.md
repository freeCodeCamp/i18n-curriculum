...
Questo sistema un po' di spaziatura di base per la scheda dell'elemento, incluso il pulsante CTA. Per il resto del contenuto interno, devi disporre tutto in una griglia puntando all'interno di `div`.

Assegna le classi `grid`, `grid-rows-[auto_auto_auto_1fr]` e `gap-y-2` all'interno di `div`.

# --hints--

Il tuo elemento interno `div` per il piano famiglia dovrebbe avere la classe `grid`.
...
Il tuo elemento interno `div` per il piano famiglia dovrebbe avere la classe `grid-rows-[auto_auto_auto_1fr]`.
...
Il tuo elemento interno `div` per il piano famiglia dovrebbe avere la classe `gap-y-2`.
