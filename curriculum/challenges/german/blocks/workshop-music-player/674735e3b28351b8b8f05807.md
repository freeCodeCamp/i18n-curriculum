...
Die `find()`-Methode ruft das erste Element in einem Array ab, das die in der übergebenen Callback-Funktion angegebenen Bedingungen erfüllt. Wenn kein Element die Bedingung erfüllt, gibt die Methode `undefined` zurück.
...
Verwenden Sie in Ihrer `playSong`-Funktion die `find()`-Methode für das `userData.songs`-Array, um nach einem Lied zu suchen, dessen `id` strikt gleich dem in die `id`-Funktion übergebenen `playSong` ist. Weisen Sie den Aufruf von `find` einer Variablen namens `song` zu.

# --hints--

Ihre `playSong`-Funktion sollte die `find`-Methode für `userData.songs` aufrufen.
...
Ihr Aufruf von `find` sollte ein Lied zurückgeben, dessen `id` strikt gleich dem in die `id`-Funktion übergebenen `playSong` ist.
...
Sie sollten Ihren Aufruf von `find` einer Variablen namens `song` zuweisen.
