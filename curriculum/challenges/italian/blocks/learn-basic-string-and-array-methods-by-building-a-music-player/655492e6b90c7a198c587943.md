...
Ora devi aggiungere l'attributo alla canzone attualmente in riproduzione.

Crea un'istruzione `if` con la condizione `songToHighlight`. Per l'istruzione, usa `setAttribute` su `songToHighlight` per passare `"aria-current"` e `"true"` come primo e secondo argomento.

# --hints--

Dovresti creare un'istruzione `if` con la condizione `songToHighlight`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

Dovresti usare il metodo `setAttribute()` su `songToHighlight` all'interno della tua istruzione `if`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

Dovresti passare `"aria-current"` e `"true"` come primo e secondo argomento del tuo metodo `setAttribute`.
