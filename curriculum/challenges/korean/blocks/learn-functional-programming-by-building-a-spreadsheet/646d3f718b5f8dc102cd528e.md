...
수학 표현식을 평가할 수 있게 되었으니, 연산 순서를 고려해야 합니다. `highPrecedence` 함수를 선언하고 `str` 매개변수를 받으세요.

# --hints--

`highPrecedence` 변수를 선언해야 합니다.

```js
assert.match(code, /(?:var|let|const)\s+highPrecedence/);
```

`const`를 사용해 `highPrecedence` 변수를 선언하세요.

```js
assert.match(code, /const\s+highPrecedence/);
```

`highPrecedence` 변수는 함수여야 합니다.

```js
assert.isFunction(highPrecedence);
```

`highPrecedence` 함수는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`highPrecedence` 함수는 `str` 매개변수를 가져야 합니다.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>/);
```

`highPrecedence` 함수는 비어 있어야 합니다.
