...
創建一個 `button` 元素，`className` 為 `submit-btn`，`type` 為 `submit`。給按鈕設定文字為 `Join the League`。

在你完成那個動作後，嘗試提交表單並查看會發生什麼。
...
你的按鈕元素應該有一個 `className` 屬性，並設為 `submit-btn`。

```js
assert.exists(document.querySelector("button.submit-btn"));
```

你的 `button` 元素應該有 `Join the League` 作為其文字。
