...
クラスが `button` で、`playlist-song-delete` 属性が `aria-label` に `Delete` を補間した値に設定された別の `song.title` 要素を作成してください。削除アイコンの内容には、以下の SVG を貼り付けてください。
...
最初の属性と値として、クラスが `button` の `playlist-song-delete` 要素を作成する必要があります。

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

button 要素には、2番目の属性と値として `aria-label` 属性に `Delete ${song.title}` の値を設定してください。

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

提供された `svg` を `button` 要素の内容として貼り付けてください。
