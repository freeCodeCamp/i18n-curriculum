...
Per l'ultima parte del workshop, sono necessarie alcune modifiche alla sezione contatti in fondo.

La prima modifica consiste nell'inserire la sezione contatti all'interno di un elemento `footer`. Questo aiuterà i lettori di schermo a identificare questa sezione come il piè di pagina della pagina.

# --hints--

La tua pagina dovrebbe avere un elemento `footer`.
...
Il tuo elemento `footer` dovrebbe contenere un elemento `h3` con il testo `Contact Me`.
...
Il tuo elemento `footer` dovrebbe contenere un elemento `p` con il testo `Email me at camperbot@blog.io`.
