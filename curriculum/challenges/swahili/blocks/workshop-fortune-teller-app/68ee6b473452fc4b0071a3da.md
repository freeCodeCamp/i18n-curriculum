...
Sasa fanya vivyo hivyo kwa `multipleCardsBtn` na `newReadingBtn` kwa kuambatanisha msikilizaji wa tukio la bofya kwa `multipleCardsBtn` unaofanya wito wa `multipleCardSelected`, na mwingine kwa `newReadingBtn` unaofanya wito wa `newReading`.

Utatengeneza njia hizo mbili katika hatua zinazofuata.

# --hints--

Unapaswa kuambatanisha tukio la `click` kwa kipengele cha `multipleCardsBtn`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Ndani ya wito wa kurudisha wa tukio la `multipleCardsBtn`, unapaswa kuita njia ya `multipleCardSelected`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

Unapaswa kuambatanisha tukio la `click` kwa kipengele cha `newReadingBtn`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Ndani ya wito wa kurudisha wa tukio la `newReadingBtn`, unapaswa kuita njia ya `newReading`.
