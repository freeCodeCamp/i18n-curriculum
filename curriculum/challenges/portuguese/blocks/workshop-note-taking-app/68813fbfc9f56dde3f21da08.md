...
Quando um usuário edita uma nota e pressiona tab ou clica fora do cartão, uma mensagem de status deve ser exibida notificando os usuários que a nota foi salva com sucesso. 

Para começar, crie um elemento `div` com um atributo `id` definido como `"status"`. 

# --hints--

Você deve ter um segundo elemento `div`.

```js
assert.lengthOf(document.querySelectorAll("div"), 2);
```

Seu segundo elemento `div` deve ter um atributo `id`.

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

Seu segundo elemento `div` deve ter um atributo `id` definido como `"status"`.
