...
Im Moment ist das `songsHTML` ein Array. Wenn Sie versuchen würden, dieses so anzuzeigen, würden die Songs durch Kommas getrennt dargestellt. Das ist nicht das gewünschte Ergebnis, da Sie die Songs als Liste anzeigen möchten. Um das zu beheben, müssen Sie das Array mit der <dfn>join()</dfn>-Methode zu einem einzigen String zusammenfügen.

Die `join()`-Methode wird verwendet, um alle Elemente eines Arrays zu einem einzigen String zu verketten. Sie nimmt einen optionalen Parameter namens `separator` entgegen, der verwendet wird, um jedes Element des Arrays zu trennen. Zum Beispiel:
...
Verketten Sie die `join()`-Methode an Ihre `map()`-Methode und übergeben Sie einen leeren String als Trennzeichen.

Um mehrere Methoden zu verketten, können Sie die `join()`-Methode auf das Ergebnis der `map()`-Methode aufrufen. Zum Beispiel:
...
Sie sollten `join("")` zum bestehenden Code hinzufügen.
