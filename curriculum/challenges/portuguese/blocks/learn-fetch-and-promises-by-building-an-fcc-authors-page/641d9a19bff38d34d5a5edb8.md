...
Todo o HTML e CSS para este projeto foram fornecidos para você. Você pode dar uma olhada nos dois arquivos para se familiarizar com eles.
...
Você deve atribuir o elemento `#author-container` à variável `authorContainer`. Não esqueça de usar `const` para declarar a variável.
...
Você deve atribuir o elemento `#load-more-btn` à variável `loadMoreBtn`. Não esqueça de usar `const` para declarar a variável.
