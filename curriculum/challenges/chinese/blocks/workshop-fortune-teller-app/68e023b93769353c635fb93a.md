...
现在，在 `renderCard` 函数的模板字面中，创建一个 `div` 元素。在 `div` 内，添加一个输出 `h2` 的 `drawingType` 元素，以及一个带有类 `figure` 和属性 `card_container ${isReversed ? "reversed-card" : ""}` 的 `data-id="${shortName}"` 元素。

# --hints--

你应该在 `div` 函数的模板字面中有一个 `renderCard` 元素。
...
你应该在 `h2` 元素内有一个插值 `drawingType` 的 `div` 元素。
...
你的 `figure` 元素应该有类 `card_container`，并使用三元表达式有条件地添加 `reversed-card`。
...
你的 `figure` 元素应该有一个属性 `data-id`，其值设置为 `${shortName}`。
