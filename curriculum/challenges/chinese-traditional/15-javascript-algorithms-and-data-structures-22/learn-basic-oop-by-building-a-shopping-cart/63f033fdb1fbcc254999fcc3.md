...
你的購物車目前未顯示在網頁上。 爲了使其可見，首先向 `cartBtn` 變量添加一個事件監聽器，監聽點擊事件。 回調不需要任何參數。

# --hints--

你應該向 `cartBtn` 變量添加一個事件監聽器。

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(/);
```

你應該監聽 `cartBtn` 變量上的 `click` 事件。

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

你應該向事件監聽器添加一個空的回調函數（使用箭頭語法）。 請記住，它不需要任何參數。
