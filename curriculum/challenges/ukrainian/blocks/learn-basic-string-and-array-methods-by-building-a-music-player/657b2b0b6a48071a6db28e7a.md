...
Метод `map()` використовується для ітерування по масиву та повернення нового масиву. Це корисно, коли ви хочете створити новий масив на основі значень існуючого масиву. Наприклад:
...
Зверніть увагу, що метод `map()` приймає функцію як аргумент. Це називається <dfn>функція зворотного виклику</dfn> — функція, яка передається іншій функції як аргумент. У наведеному вище прикладі функція зворотного виклику — це `(number) => number * 2`, і вона запускається для кожного елемента масиву `numbers`. Потім метод `map()` повертає новий масив із результатами.

Передайте функцію зворотного виклику методу `map()`. Функція зворотного виклику повинна приймати `song` як параметр, використовувати синтаксис стрілкової функції та мати порожнє тіло.

# --hints--

Функція зворотного виклику вашого методу `map()` має використовувати синтаксис стрілкової функції.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

Ваш метод `map()` повинен мати параметр `song` у своїй функції зворотного виклику.
