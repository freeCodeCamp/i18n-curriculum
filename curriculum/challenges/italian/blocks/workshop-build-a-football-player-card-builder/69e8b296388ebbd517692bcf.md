...
Procedi aggiungendo un `div` di `className` di `form-row`. Al suo interno, aggiungi due elementi `div` con il `className` di `form-group` ciascuno.

Il primo elemento `div` dovrebbe avere un `label` di `className` di `label`, un attributo `htmlFor` di `position`, un testo `Position` e un `input` con il `id` di `position`, un `className` di `input` e un `type` di `text`.

# --hints--

La tua sezione Player Info dovrebbe contenere un `div` con il `className` di `form-row`.
...
Il tuo `div.form-row` dovrebbe contenere due elementi `div` ciascuno con il `className` di `form-group`.
...
Il tuo primo `form-group` nella riga dovrebbe avere un `label` con il `className` di `label` e un `htmlFor` di `position`.
...
Il tuo `label` di posizione dovrebbe avere il testo `Position`.
...
Il tuo primo `form-group` nella riga dovrebbe avere un `input` con il `className` di `input`, un `id` di `position` e `type` di `text`.
