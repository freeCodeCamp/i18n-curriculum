...
قم بتسكين عنصر `textarea` واحد داخل العنصر الثاني `div.answer`، وحدد عدد الصفوف والأعمدة التي يحتويها.

# --hints--

يجب أن تسكن عنصر `textarea` واحد داخل العنصر الثاني `div.answer`.

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

يجب أن تعطي عنصر `textarea` خاصية `rows` مع رقم.
...
يجب أن تعطي عنصر `textarea` خاصية `cols` مع رقم.
