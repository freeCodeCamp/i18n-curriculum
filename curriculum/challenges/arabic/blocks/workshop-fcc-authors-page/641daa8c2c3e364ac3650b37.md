...
بعد ذلك، لا يوجد فصل كبير بين اسم وصورة كل مؤلف وبقية التفاصيل على البطاقة. سيُعطي الفاصل للبطاقات الخاصة بالمؤلفين ترتيبًا بصريًا واضحًا.

أضف عنصر `div` فوق السيرة الذاتية للمؤلف وأعطه خاصية `class` بقيمة `"purple-divider"`.
...
يجب أن تنشئ عنصر `div` قبل عنصر `p` الخاص بك.

```js
await __helpers.retryingTest(() => document.querySelector('p')?.previousElementSibling?.tagName === 'DIV', "'div' element not found");
```

يجب أن يحتوي عنصر `div` الخاص بك على خاصية `class` مضبوطة على `"purple-divider"`.
