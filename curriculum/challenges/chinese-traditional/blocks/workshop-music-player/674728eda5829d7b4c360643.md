...
你的 `allSongs` 陣列的第三個值應該是一個物件。

```js
assert.isObject(allSongs[2]);
```

你的 `allSongs` 陣列中的第三個物件應該有一個 `id` 屬性，設定為數字 `2`。

```js
assert.equal(allSongs[2].id, 2);
```

你 `allSongs` 陣列中的第三個物件應該有一個 `title` 屬性，該屬性設定為字串 `"Still Learning"`。

```js
assert.equal(allSongs[2].title, "Still Learning");
```

你的 `allSongs` 陣列中的第三個物件應該有一個 `artist` 屬性，並設定為字串 `"Quincy Larson"`。

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

你 `allSongs` 陣列中的第三個物件應該有一個 `duration` 屬性，並設定為字串 `"3:51"`。

```js
assert.equal(allSongs[2].duration, "3:51");
```

你 `allSongs` 陣列中的第三個物件應該有一個 `src` 屬性，並設定為字串 `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`。
