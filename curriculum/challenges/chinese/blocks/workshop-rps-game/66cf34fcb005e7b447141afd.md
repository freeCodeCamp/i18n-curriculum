...
创建一个带有名为 `userOption` 参数的 `showResults` 函数。

在你的 `showResults` 函数内，`roundResultsMsg` 应该使用本轮的结果进行更新。

然后，`playerScoreSpanElement` 和 `computerScoreSpanElement` 也应更新，以显示玩家和计算机的最新分数。

请记住，这里的顺序很重要。你需要先更新 `roundResultsMsg`，然后更新 `playerScoreSpanElement`，最后更新 `computerScoreSpanElement`，因为 `roundResultsMsg` 将用于确定分数。

# --hints--

你应该有一个名为 `showResults` 的函数。

```js
assert.isFunction(showResults);
```

你的 `showResults` 函数应有一个名为 `userOption` 的参数。

```js
assert.match(code, /showResults\s*(?:=\s*)?(?:\(\s*)?userOption(?:\s*\))?\s*/);
```

你的 `showResults` 函数应该使用回合结果更新 `roundResultsMsg`。
...
你的 `showResults` 函数应该更新 `computerScoreSpanElement` 以显示更新后的电脑得分。
...
你的 `showResults` 函数应该更新 `playerScoreSpanElement` 以显示玩家的最新得分。
