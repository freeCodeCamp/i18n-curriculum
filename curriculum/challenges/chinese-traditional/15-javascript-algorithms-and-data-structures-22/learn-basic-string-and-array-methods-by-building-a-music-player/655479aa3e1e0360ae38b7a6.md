...
接下來，你需要檢索播放列表中的下一首歌曲。 爲此，你需要獲取當前歌曲的索引，然後向其中添加 `1`。

創建一個名爲 `nextSong` 的常量，並將 `userData?.songs[currentSongIndex + 1]` 分配給它。

最後，調用 `playSong` 函數並傳入 `nextSong.id` 作爲參數。

# --hints--

你不應修改現有的 `if` 語句、其 `else` 塊和內容。
...
你應該將 `userData?.songs[currentSongIndex + 1]` 分配給 `nextSong` 常量。
...
你應該使用 `nextSong.id` 調用 `playSong` 函數。
