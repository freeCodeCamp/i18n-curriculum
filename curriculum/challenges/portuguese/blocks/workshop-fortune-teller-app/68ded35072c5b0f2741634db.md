...
Você precisa de outra função que mostre elementos no DOM. Semelhante à função anterior, crie uma função chamada `showElements`. Desta vez use `classList.remove("hidden")`.

# --hints--

Você deve ter uma função chamada `showElements`.
...
Sua função `showElements` deve ter um parâmetro chamado `...elements` com o tipo `HTMLElement[]`.
...
Sua função `showElements` deve ter um método `forEach` anexado ao parâmetro `elements`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Seu método `forEach` deve receber um parâmetro chamado `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Você deve remover a classe `hidden` usando o método `classList.remove` dentro do método `forEach`.
