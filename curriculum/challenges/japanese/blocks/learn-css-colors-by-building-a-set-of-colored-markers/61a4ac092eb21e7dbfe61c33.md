...
現在純粋な緑色になっている `linear-gradient` 関数の2番目の色の引数を変更してください。

`rgb` 関数を更新して、赤の値が `245`、緑の値が `62`、青の値が `113` になるようにしてください。

# --hints--

`.red` の CSS ルールには `background` プロパティが `linear-gradient(180deg, rgb(122, 74, 14) 0%, rgb(245, 62, 113) 50%, rgb(0, 0, 255) 100%)` に設定されている必要があります。
