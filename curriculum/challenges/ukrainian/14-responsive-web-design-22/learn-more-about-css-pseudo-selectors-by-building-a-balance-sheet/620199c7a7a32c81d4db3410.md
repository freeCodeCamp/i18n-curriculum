...
Надайте елементам `tr.total` ефект наведення, який змінює фон на `#99c9ff`.

# --hints--

Ви повинні мати селектор `tr.total:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total:hover'));
```

Ваш селектор `tr.total:hover` повинен мати властивість `background-color` зі значенням `#99c9ff`.
