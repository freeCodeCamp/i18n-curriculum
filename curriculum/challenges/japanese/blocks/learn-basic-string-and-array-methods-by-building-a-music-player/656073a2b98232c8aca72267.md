...
現在の曲の情報を管理し、再生時間を追跡するために、`currentSong` と `songCurrentTime` のプロパティを作成してください。それぞれの値は `null` と `0` に設定してください。

# --hints--

既存の `userData` オブジェクトとその内容は変更しないでください。

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

`userData` オブジェクトには `currentSong` キーがあり、その値は `null` に設定されている必要があります。

```js
assert.propertyVal(userData, "currentSong", null)
```

`userData` オブジェクトには `songCurrentTime` キーがあり、その値は `0` に設定されている必要があります。
