...
Si juegas un poco con el reproductor, notarás algunos errores. Primero, cuando reproduces una canción, la pausas y luego presionas el botón anterior o siguiente, la canción no comenzará desde el principio.

Para comenzar a solucionar eso, agrega un segundo parámetro llamado `start` a tu función `playSong`. Dale el valor predeterminado de `true`.

# --hints--

Tu función `playSong` debe tener un segundo parámetro llamado `start` con el valor predeterminado de `true`.
