...
Dichiara una funzione `toNumberList` che prende un parametro `args` e restituisce implicitamente il risultato di dividere `args` usando le virgole. Poi concatena un metodo `map` al tuo metodo `split` e passa `parseFloat` come argomento al metodo `map`.
...
Dovresti usare `const` per dichiarare la tua variabile `toNumberList`.
...
La tua variabile `toNumberList` dovrebbe essere una funzione freccia.
...
La tua funzione `toNumberList` dovrebbe avere un parametro `args`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

La tua funzione `toNumberList` dovrebbe usare un ritorno implicito.
...
La tua funzione `toNumberList` dovrebbe restituire il risultato della chiamata al metodo `.split()` di `args`.
...
Dovresti dividere `args` sul carattere `","`.
...
Dovresti passare un riferimento a `parseFloat` come callback a `.map()`.
