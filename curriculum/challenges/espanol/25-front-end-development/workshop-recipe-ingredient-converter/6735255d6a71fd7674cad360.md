...
Finalmente, da a tu `conversionTable` un objeto `teaspoon` con una propiedad `cup` establecida en `1/48`, una propiedad `gram` establecida en `5`, y una propiedad `ounce` establecida en `0.167`.

# --hints--

Tu `conversionTable` debería tener una propiedad `teaspoon`.

```js
assert.property(conversionTable, "teaspoon");
```

Tu propiedad `teaspoon` debería ser un objeto.

```js
assert.isObject(conversionTable.teaspoon);
```

Tu objeto `teaspoon` debería tener una propiedad `cup`.

```js
assert.property(conversionTable.teaspoon, "cup");
```

Tu propiedad `cup` debería estar establecida en `1/48`.

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

Tu objeto `teaspoon` debería tener una propiedad `gram`.

```js
assert.property(conversionTable.teaspoon, "gram");
```

Tu propiedad `gram` debería estar establecida en `5`.

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

Tu objeto `teaspoon` debería tener una propiedad `ounce`.

```js
assert.property(conversionTable.teaspoon, "ounce");
```

Tu propiedad `ounce` debería estar establecida en `0.167`.
