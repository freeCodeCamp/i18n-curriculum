...
Para el bloque `catch`, agrega un parámetro `error`, luego use `console.error` para registrar cualquier error que ocurra mientras se obtiene o procesa la información.

# --hints--

El bloque `catch` debe tener un parámetro `error`.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

Debe registrar cualquier error en el bloque `catch` usando `console.error`.
