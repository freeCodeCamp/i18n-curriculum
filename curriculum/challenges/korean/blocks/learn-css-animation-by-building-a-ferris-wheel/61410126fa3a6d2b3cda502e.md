...
오두막 애니메이션이 더 자연스러운 흔들림처럼 보이게 하려면 `ease-in-out` 타이밍 함수를 사용할 수 있습니다. 이 설정은 애니메이션이 시작과 끝에서 느리게 움직이고, 사이클 중간에서는 더 빠르게 움직이도록 만듭니다.

`linear` 선택자에서 `ease-in-out`을 `.cabin`로 바꾸세요.

# --hints--

`.cabin` 선택자에 `animation` 속성이 `cabins 10s ease-in-out infinite`로 설정되어 있어야 합니다.
