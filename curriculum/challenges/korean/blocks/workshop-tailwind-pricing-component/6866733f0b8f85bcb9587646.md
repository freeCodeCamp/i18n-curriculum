...
프리미엄 가격 카드가 페이지에서 가장 눈에 띄어야 합니다. 또한, `Most popular` 배지가 있는 유일한 카드입니다. 이 카드와 배지를 돋보이게 하려면 Tailwind의 상대 위치 지정과 크기 조절 유틸리티 클래스를 사용할 수 있습니다.

Tailwind CSS에서 크기 조절로 요소를 더 크게 만드는 방법은 다음과 같습니다.
...
위치는 다음 단계에서 다룰 예정입니다. 지금은 래퍼 `scale-105`에 클래스를 `div` 할당하여 다른 가격 카드보다 약간 더 크게 만드세요.

# --hints--

프리미엄 플랜의 래퍼 `div` 요소에 `scale-105` 클래스를 할당하세요.
