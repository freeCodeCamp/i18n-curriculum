...
Ili kumaliza, weka maandishi ya `Calendar` kwa tagi ya nanga ya kwanza, `Chat` kwa ya pili, na `Email` kwa ya tatu.

Kwa hivyo, sehemu yako kubwa ya navbar imekamilika!

# --hints--

Unapaswa kuweka maandishi ya `Calendar` kwa kipengele cha nanga cha kwanza.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Unapaswa kuweka maandishi ya `Chat` kwa kipengele cha nanga cha pili.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Unapaswa kuweka maandishi ya `Email` kwa kipengele cha nanga cha tatu.
