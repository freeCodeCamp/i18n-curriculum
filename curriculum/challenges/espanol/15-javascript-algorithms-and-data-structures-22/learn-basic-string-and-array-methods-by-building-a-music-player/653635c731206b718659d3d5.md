...
Dentro de la función `playSong`, establece la propiedad `audio.src` igual a `song.src`. Esto indica al elemento de audio dónde encontrar los datos de audio para la canción seleccionada.
...
No debes modificar la función existente `playSong` y su contenido.
...
Debes establecer `audio.src` a `song.src`.
...
Debes establecer `audio.title` a `song.title`.
