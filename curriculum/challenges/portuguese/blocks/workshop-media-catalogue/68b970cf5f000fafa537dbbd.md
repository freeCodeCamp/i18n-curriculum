...
Como você está armazenando o objeto que causa a exceção no objeto de exceção, pode fazer algum uso dele.

Após sua chamada existente `print`, adicione `print(f'Unable to add {e.obj}: {type(e.obj)}')`.

Esse tipo de mensagem pode ser extremamente útil quando você está tentando depurar seu código.

# --hints--

Você deve ter `print(f'Unable to add {e.obj}: {type(e.obj)}')` após sua chamada existente `print(f'Media Error: {e}')`.
