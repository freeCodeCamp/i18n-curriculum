...
`showLatestPosts()` 関数を作成していく中で、変更を確認するために呼び出す必要があります。

`showLatestPosts()` ブロックの最後で `try` 関数を呼び出し、引数として `data` を渡してください。

# --hints--

`showLatestPosts()` ブロックの最後で `try` を呼び出す必要があります。

```js
assert.match(code, /\s*showLatestPosts\(.*\)\s*;?\s*/);
```

`data` に引数として `showLatestsPosts()` を渡してください。
