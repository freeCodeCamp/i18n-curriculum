...
將 `.cabin` 規則的 `animation` 屬性設置爲 `cabins 10s linear infinite`。 這將按順序設置 `animation-name`、`animation-duration`、`animation-timing-function` 和 `animation-iteration-count` 屬性。

# --hints--

你的 `.cabin` 選擇器應該有一個 `animation` 屬性值爲 `cabins 10s linear infinite`。
