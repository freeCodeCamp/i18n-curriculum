...
Dato l'array `arr`, itera su di esso e rimuovi ogni elemento a partire dal primo elemento (l'indice 0) finché la funzione `func` non restituisce `true` quando le viene passato l'elemento iterato.

Poi restituisci il resto dell'array una volta soddisfatta la condizione, altrimenti `arr` dovrebbe essere restituito come un array vuoto.
