...
أنشئ قاعدة `.image-1, .image-3` وأعطها خاصية `grid-column` بقيمة `1 / -1`. هذا سيسمح للصورة الأولى والثالثة بأن تمتد على العرض الكامل للشبكة.

# --hints--

يجب أن يكون لديك محدِّد `.image-1, .image-3`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

يجب أن يحتوي محدِّد `.image-1, .image-3` على خاصية `grid-column` بقيمة `1 / -1`.
