...
Erstellen Sie eine weitere `except`-Klausel, die `MediaError as e` abfängt. Verwenden Sie innerhalb des `except`-Blocks einen f-String, um `Media Error:` gefolgt von einem Leerzeichen und der Fehlermeldung auszugeben.

# --hints--

Sie sollten eine `except`-Klausel haben, die `MediaError as e` abfängt.
...
Sie sollten innerhalb Ihres neuen `Media Error:`-Blocks einen f-String verwenden, um `except` gefolgt von einem Leerzeichen und der Fehlermeldung auszugeben.
