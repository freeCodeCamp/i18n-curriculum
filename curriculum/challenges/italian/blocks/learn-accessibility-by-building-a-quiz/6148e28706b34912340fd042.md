...
Dovresti dare a `address` una `text-align` di `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Dovresti dare a `address` una `padding-top` di almeno `1px`.
...
Dovresti dare a `address` una `padding-right` di almeno `1px`.
...
Dovresti dare a `address` una `padding-bottom` di almeno `1px`.
...
Dovresti dare a `address` una `padding-left` di almeno `1px`.
