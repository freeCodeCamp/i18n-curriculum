...
요소를 비교해야 하므로 중첩된 `for` 반복문을 사용해야 합니다. 이 반복문은 배열의 마지막 요소를 제외한 모든 요소를 순차적으로 반복해야 합니다. 내부 반복문의 반복 변수로 `j`을 사용하세요.
...
내부 `for` 반복문은 배열의 마지막 요소를 제외한 모든 요소를 순차적으로 반복해야 합니다.
...
내부 `for` 반복문은 매번 `j`을 `1`만큼 증가시켜야 합니다.

```js
assert.match(code, /const\s+bubbleSort\s*=\s*(\(\s*array\s*\)|array)\s*=>\s*{\s*for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*array\.length\s*;\s*i\s*\+\+\s*\)\s*{\s*for\s*\(\s*let\s+j\s*=\s*0\s*;\s*j\s*<\s*array\.length\s*-\s*1\s*;\s*j\s*\+\+\s*\)/);
```

내부 `for` 반복문은 비워 두어야 합니다.
