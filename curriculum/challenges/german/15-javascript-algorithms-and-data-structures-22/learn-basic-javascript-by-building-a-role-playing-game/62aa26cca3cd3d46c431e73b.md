...
Du solltest `Math.floor(Math.random() * 11)` verwenden, um die Zufallszahl zu erzeugen.

```js
assert.match(pick.toString(), /Math\.floor\(\s*Math\.random\(\s*\)\s*\*\s*11\s*\)/);
```

Du solltest das Ergebnis von `Math.floor(Math.random() * 11)` am Ende des `numbers` Arrays `push`en.
