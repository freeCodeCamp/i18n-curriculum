...
可以實作 `__eq__` 方法來指定當比較運算子（`==`）用於比較一個物件與其他東西時應該發生什麼。

在 `R2Vector` 類別中，創建一個 `__eq__` 方法並給它兩個參數：`self` 和 `other`。

# --hints--

你應該在 `R2Vector` 類別中定義一個 `__eq__` 方法。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__eq__")`)) })
```

你的 `__eq__` 方法應該接受兩個參數：`self` 和 `other`。
