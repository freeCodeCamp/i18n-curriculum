...
Wenn weder die `if`- noch die `elif`-Bedingung erfüllt sind, wäre der Wert von `mid` größer als `square_target`. Erstellen Sie in diesem Fall eine `else`-Klausel und weisen Sie den Wert von `mid` `high` zu.
...
Sie sollten den Wert von `mid` in der `high`-Klausel `else` zuweisen.
