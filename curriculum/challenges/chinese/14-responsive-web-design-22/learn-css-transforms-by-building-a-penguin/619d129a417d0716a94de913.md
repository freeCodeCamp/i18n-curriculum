...
应该使用 `.beak.bottom` 选择器。

```js
assert.match(code, /\.beak\.bottom\s*\{/);
```

应该将 `.beak.bottom` 的 `width` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.width, '16%');
```

应该将 `.beak.bottom` 的 `top` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.top, '65%');
```

应该将 `.beak.bottom` 的 `left` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
