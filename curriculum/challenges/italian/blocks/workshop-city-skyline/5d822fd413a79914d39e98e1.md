...
Non ha funzionato, perché le variabili che hai dichiarato in `.bb1` non si propagano agli elementi fratelli `.bb2` e `.bb3`. È così che funziona il CSS. Per questo motivo, le variabili vengono spesso dichiarate nel selettore `:root`. Questo è il selettore di livello più alto in CSS; mettere le tue variabili lì le renderà utilizzabili ovunque. Aggiungi il selettore `:root` in cima al tuo foglio di stile e sposta tutte le dichiarazioni delle variabili lì.
...
Dovresti definire `--building-color1` con un valore di `#aa80ff` nel selettore `:root`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color1')?.trim(), '#aa80ff');
```

Dovresti definire `--building-color2` con un valore di `#66cc99` nel selettore `:root`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color2')?.trim(), '#66cc99');
```

Dovresti definire `--building-color3` con un valore di `#cc6699` nel selettore `:root`.
