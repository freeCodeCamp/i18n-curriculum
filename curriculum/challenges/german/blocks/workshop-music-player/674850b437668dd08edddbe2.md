...
In den vorherigen Schritten haben Sie die Funktionalität zum Abspielen eines Songs aufgebaut. Jetzt müssen Sie die Funktionalität für die Wiedergabetaste hinzufügen.

Verwenden Sie die `addEventListener()`-Methode an `playButton` und übergeben Sie ein `"click"`-Ereignis als erstes Argument. Verwenden Sie vorerst ein Callback, das `playSong(0)` als zweites Argument aufruft.

Testen Sie es anschließend, indem Sie auf die Wiedergabetaste klicken.


# --hints--

Sie sollten einen Ereignis-Listener für das `click`-Ereignis an `playButton` hinzufügen und eine Callback-Funktion übergeben, die `playSong(0)` als zweites Argument an die `addEventListener`-Methode aufruft.

```js
assert.match(
  code,
  /playButton\.addEventListener\(\s*["']click["']\s*,[\s\S]*playSong\(\s*0\s*\)/
);
```
