...
在 `HTMLString` 末尾创建另一个 `label` 元素（在新行上）。 这个 `label` 应该包含文本 `Entry # Calories`，使用模板文字语法将 `#` 替换为 `entryNumber` 的值，并将 `for` 属性设置为 `X-#-calories`，其中 `X` 是 `entryDropdown` 的 `value`，而 `#` 是 `entryNumber` 的值。

# --hints--

你的 `HTMLString` 中应该有两个 `label` 元素。
...
你的新 `label` 元素应将 `for` 属性设置为 `${entryDropdown.value}-${entryNumber}-calories`。
...
你的新 `label` 元素应该包含文本 `Entry ${entryNumber} Calories`。
