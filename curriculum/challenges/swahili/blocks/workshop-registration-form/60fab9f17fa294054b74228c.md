...
Tunaendelea na `fieldset` ya mwisho. Je, ungetaka mtumizi apakie picha ya wasifu?

Naam, aina ya `input` `file` inaruhusu hilo tu. Ongeza `label` yenye maandishi `Upload a profile picture: `, na panga ndani `input` inayokubali kupakia faili.
...
Unapaswa kupanga ndani kipengele cha `input` ndani ya kipengele cha `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label > input'));
```

Unapaswa kumpa kipengele cha `input` `type` ya `file`.
