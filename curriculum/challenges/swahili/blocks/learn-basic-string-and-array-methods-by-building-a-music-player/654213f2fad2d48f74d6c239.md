...
Tengeneza kipengele kingine cha `button` chenye darasa la vitu la `playlist-song-delete` na sifa ya kipengele `aria-label` iliyowekwa kuwa `Delete` ikichanganywa na `song.title`. Kwa maudhui ya ikoni ya futa, bandika SVG ifuatayo:
...
Unapaswa kutengeneza kipengele cha `button` chenye darasa la vitu la `playlist-song-delete` kama sifa ya kipengele yake ya kwanza na thamani yake.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

Kipengele chako cha kitufe kinapaswa kuwa na sifa ya kipengele `aria-label` yenye thamani ya `Delete ${song.title}` kama sifa ya kipengele ya pili na thamani yake.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

Unapaswa kubandika `svg` iliyotolewa kama maudhui ya kipengele chako cha `button`.
