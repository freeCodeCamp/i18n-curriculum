...
將每個已定義的 `grid-template-columns` 屬性值放在 `minmax` 函數中，使用每個值作爲第二個參數。 第一個參數應分別爲 `2rem`、`min-content` 和 `2rem`。

# --hints--

你的 `main` 選擇器應該有一個 `grid-template-columns` 屬性設置爲 `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`。
