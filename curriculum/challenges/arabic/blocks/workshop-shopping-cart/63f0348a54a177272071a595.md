...
الآن عيّن `textContent` للمتغير `showHideCartSpan` نتيجة تعبير ثلاثي يتحقق مما إذا كانت قيمة `isCartShowing` صحيحة. إذا كانت كذلك، عيّن `textContent` إلى `"Hide"`، وإلا عيّنه إلى `"Show"`.

# --hints--

يجب استخدام معامل التعيين على خاصية `textContent` للمتغير `showHideCartSpan`.
...
يجب تعيين `textContent` للمتغير `showHideCartSpan` إلى `"Hide"` إذا كانت قيمة `isCartShowing` صحيحة.

```js
assert.match(code, /showHideCartSpan\s*\.\s*textContent\s*=\s*isCartShowing\s*\?\s*('|"|`)Hide\1\s*:\s*/)
```

يجب تعيين `textContent` للمتغير `showHideCartSpan` إلى `"Show"` إذا كانت قيمة `isCartShowing` خاطئة.
