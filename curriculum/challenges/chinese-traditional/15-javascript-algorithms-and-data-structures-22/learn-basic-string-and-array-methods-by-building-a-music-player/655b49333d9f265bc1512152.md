...
請注意，HTML 中的專輯封面和 `userData.songs` 數組中的歌曲已經發生了變化。 我們將原始歌曲替換爲較短的歌曲，你可以在接下來的步驟中使用這些歌曲來測試你的應用程序。

接下來，你需要檢查是否有下一首歌曲要播放。 通過調用 `getCurrentSongIndex()` 函數檢索當前歌曲索引，並將其保存在 `currentSongIndex` 常量中。

之後，創建一個 `nextSongExists` 常量，其中包含布爾值 `true` 或 `false`，具體取決於下一首歌曲是否存在。

# --hints--

你不應修改現有的 `ended` 事件監聽器及其內容。

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

你應該創建一個 `currentSongIndex` 常量並將其設置爲 `getCurrentSongIndex` 函數的調用。
...
你應該比較 `userData.songs.length` 和 `currentSongIndex` 來檢查是否存在下一首歌曲，並將其設置爲 `nextSongExists` 常量。 如果歌曲數組的最後一個索引（`userData.songs.length - 1`）大於 currentSongIndex，則表示有下一首歌曲。
