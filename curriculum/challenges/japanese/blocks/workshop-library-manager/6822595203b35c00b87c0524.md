...
本のタイトルの配列は良いですが、各本の`title`と`author`を次のように表示できると便利です。
...
`map()`メソッドの使い方を更新して、代わりにこの形式の文字列の配列を返すようにしてください：`TITLE by AUTHOR`。

この結果を得るには、テンプレートリテラルか`+`演算子を使った文字列連結のどちらかを使う必要があります。
...
`getBookInformation`関数は、各本の`title`と`author`を含む文字列の配列を返す必要があります。
