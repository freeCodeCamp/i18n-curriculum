...
Sasa, katika faili la JavaScript, tengeneza mabadiliko wa `happyBtn`.

Tumia `querySelector` kuhifadhi rejea ya kipengele cha `#happy-btn` katika mabadiliko wa `happyBtn`.

# --hints--

Unapaswa kuwa na mabadiliko unaoitwa `happyBtn`.

```js
assert.exists(happyBtn);
```

Mabadiliko ya `happyBtn` yanapaswa kuhifadhi rejea ya kipengele cha `#happy-btn`.
