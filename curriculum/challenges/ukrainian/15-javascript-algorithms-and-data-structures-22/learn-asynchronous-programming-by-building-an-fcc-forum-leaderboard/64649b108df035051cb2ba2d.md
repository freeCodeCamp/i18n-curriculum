...
Для початкового теґу `a` встановіть атрибут `target` на `"_blank"`. Потім встановіть атрибут `href` до `${forumTopicUrl}${slug}/${id}`.

І з цими змінами проєкт таблиці лідерів форуму freeCodeCamp тепер завершено!

# --hints--

Початковий тег прив’язки повинен мати атрибут `target` зі значенням `"_blank"`.
...
Початковий тег прив’язки повинен мати атрибут `href` зі значенням `${forumTopicUrl}${slug}/${id}`.
