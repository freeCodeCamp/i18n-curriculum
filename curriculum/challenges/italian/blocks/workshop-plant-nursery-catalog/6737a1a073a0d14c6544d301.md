...
Dichiara una variabile chiamata `plantsSet` e assegnale una chiamata alla funzione `displayPlantsSet`.

Poi stampa `plantsSet` nella console. Dovrai aprire la console del browser per vedere il tuo insieme.


# --hints--

Dovresti avere una variabile chiamata `plantsSet`.

```js
assert.exists(plantsSet);
```

Dovresti chiamare `displayPlantsSet` e assegnarla a `plantsSet`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

Dovresti stampare `plantsSet` nella console.
