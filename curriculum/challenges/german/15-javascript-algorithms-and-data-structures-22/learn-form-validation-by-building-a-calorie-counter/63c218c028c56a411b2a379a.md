...
Um deinen neuen HTML-Inhalt für den `targetInputContainer` zu sehen, musst du die <dfn>innerHTML</dfn>-Eigenschaft verwenden.

The `innerHTML` property sets or returns the HTML content inside an element.

Hier ist ein `form`-Element mit einem `label` und `input`-Element verschachtelt innen.
...
Deine `addEntry`-Funktion sollte auf die `innerHTML`-Eigenschaft von `targetInputContainer` zugreifen.

```js
assert.match(addEntry.toString(), /targetInputContainer\.innerHTML/);
```

Du solltest den `+=`-Operator für `targetInputContainer.innerHTML` verwenden.

```js
assert.match(addEntry.toString(), /targetInputContainer\.innerHTML\s*\+=/);
```

Du solltest deinen `HTMLString` zur `innerHTML` des `targetInputContainer`-Elements hinzufügen.
