...
Successivamente, crea un tipo `Position` con il valore `typeof POSITIONS[number]`. Questo espande l'array `POSITIONS` in un insieme delle sue singole stringhe letterali, facendolo apparire come `"GK" | "CB" | "LB" | ...` e così via.

# --hints--

Dovresti creare un alias di tipo chiamato `Position`.
...
Il tuo tipo `Position` dovrebbe essere impostato su `typeof POSITIONS[number]`.
