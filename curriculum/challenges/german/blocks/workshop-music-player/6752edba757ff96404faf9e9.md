...
Erstellen Sie eine Funktion namens `setPlayButtonAccessibleText`. Innerhalb der Funktion setzen Sie das `aria-label`-Attribut der Wiedergabetaste auf `Play` gefolgt von einem Leerzeichen und dem Titel des aktuellen Songs. Wenn `userData.currentSong` `null` ist, setzen Sie es auf `Play`.

# --hints--

Sie sollten eine Funktion namens `setPlayButtonAccessibleText` haben.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

Ihre `setPlayButtonAccessibleText`-Funktion sollte das `aria-label`-Attribut der Wiedergabetaste auf `Play` gefolgt von einem Leerzeichen und dem Titel des aktuellen Songs setzen.
...
Wenn `userData.currentSong` `null` ist, sollte `setPlayButtonAccessibleText` das `aria-label`-Attribut der Wiedergabetaste auf `Play` setzen.
