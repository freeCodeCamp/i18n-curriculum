...
أخيرًا، أنشئ جملة `if` أخيرة للتحقق مما إذا كانت قيمة `duration` ليست أكبر من `0` وارفع `ValueError` مع الرسالة `Duration must be positive`.

# --hints--

يجب رفع `ValueError` مع الرسالة `Duration must be positive` عندما لا يكون `duration` عددًا موجبًا.
...
لا يجب رفع `ValueError` عندما يكون `duration` رقمًا صالحًا.
