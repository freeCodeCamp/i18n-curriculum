...
داخل بند `else`، قم بتعيين السلسلة النصية `"general"` إلى `selectedCategory.className`.

أسفل ذلك، قم بتعيين السلسلة النصية `"General"` إلى `selectedCategory.category`.

أخيرًا، قم بتعيين الرقم `1` إلى `selectedCategory.id`.

# --hints--

يجب أن تعيّن السلسلة النصية `"general"` إلى `selectedCategory.className`.

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

يجب أن تعيّن السلسلة النصية `"General"` إلى `selectedCategory.category`.

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

يجب أن تعيّن الرقم `1` إلى `selectedCategory.id`.
