...
Якщо ви хочете додати більше іконок соціальних мереж, але залишити їх в одному рядку, потрібно оновити `grid-template-columns`, щоб створити додаткові колонки. Як альтернативу, можна використати властивість `grid-auto-flow`.

Ця властивість приймає або `row`, або `column` як перше значення, з необов’язковим другим значенням `dense`. `grid-auto-flow` використовує алгоритм автоматичного розміщення для налаштування макету сітки. Встановлення його в `column` повідомить алгоритм створювати нові колонки для вмісту за потреби. Значення `dense` дозволяє алгоритму повертатися назад і заповнювати порожнини в сітці меншими елементами, що може призвести до появи елементів у невідповідному порядку.
