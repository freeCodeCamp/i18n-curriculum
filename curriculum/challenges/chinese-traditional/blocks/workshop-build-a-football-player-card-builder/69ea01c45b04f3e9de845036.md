...
目前的問題是，一旦預覽重新整理或你重新載入頁面，所有你為玩家卡片添加的內容都會消失。

將所有輸入儲存在區域的 localstorage 中可以防止這種情況。為了開始這個過程，創建一個 `STORAGE_KEY` 常數，其值為字串 `football_player_card`。

# --hints--

你應該宣告一個 `STORAGE_KEY` 變數。
...
你的 `STORAGE_KEY` 應該有字串 `football_player_card` 作為其值。
