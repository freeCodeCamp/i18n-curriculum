...
En el pasado, había más tipos de medios, como `handheld` y `tv`, pero la mayoría de estos han sido desaprobados en favor de usar funcionalidades para apuntar con mayor precisión a los dispositivos.
...
Las media queries también pueden combinar múltiples condiciones usando operadores lógicos. El operador `and` se usa para combinar múltiples funcionalidades de medios, mientras que `not` y `only` pueden usarse para negar o aislar media queries. Aquí tienes un ejemplo que combina múltiples funcionalidades:
...
En una media query, ¿cuál es el propósito de usar el operador `and`?
