...
你應該做的第一件事是檢查 `items` 數組是否爲空。 如果是，則向用戶顯示 `alert` 和文本 `Your shopping cart is already empty`，然後從函數返回。

請記住 `0` 是一個假值，因此你可以使用 `!` 運算符來檢查數組是否爲空。

顯示 alert 後，從函數返回以停止執行。

# --hints--

你應該創建一個 `if` 語句，使用否定運算符檢查 `items` 數組是否爲空。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

你的 `if` 語句應該向用戶顯示 `alert` 和文本 `Your shopping cart is already empty`。
