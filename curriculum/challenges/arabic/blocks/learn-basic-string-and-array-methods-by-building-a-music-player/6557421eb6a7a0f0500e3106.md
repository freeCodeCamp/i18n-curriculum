...
داخل عنصر الزر في دالة `renderSongs`، أضف خاصية `onclick`. للقيمة، استدعي الدالة `deleteSong` وادمج `song.id`.

# --hints--

يجب إضافة خاصية `onclick` إلى زر الحذف وتمرير القيمة `"deleteSong(${song.id})"`.
