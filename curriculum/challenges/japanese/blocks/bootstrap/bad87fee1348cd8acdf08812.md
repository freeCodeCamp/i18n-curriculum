...
ブートストラップには`button`要素用の独自のスタイルがあり、通常のHTMLのものよりずっと見栄えが良くなっています。

大きな子猫の写真の下に新しい`button`要素を作成してください。`btn`と`btn-default`のクラスを付け、テキストは`Like`にしてください。

# --hints--

テキストが`button`の新しい`Like`要素を作成してください。
...
新しいボタンには`btn`と`btn-default`の2つのクラスを付けてください。
