...
你可以通过将 `submit` 事件监听器重构为两个独立的函数来增强代码的可读性和可维护性。 第一个函数可用于将输入值添加到 `taskData`，而第二个函数可负责将任务添加到 DOM。

使用箭头语法创建 `addOrUpdateTask` 函数。 然后将 `dataArrIndex` 变量、`taskObj` 对象和 `if` 语句移动到 `addOrUpdateTask` 函数中。

# --hints--

你应该使用 `const` 和箭头语法来创建 `addOrUpdateTask` 函数。

```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

你应该将 `dataArrIndex` 变量移入 `addOrUpdateTask` 函数中。
...
你应该将 `taskObj` 对象移到 `addOrUpdateTask` 函数中。
...
你应该将带有条件 `dataArrIndex === -1` 的 `if` 语句移到 `addOrUpdateTask` 函数中。
