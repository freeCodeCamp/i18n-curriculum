...
حان الوقت لإنشاء المسميات والمُدخَلات. في النموذج، أنشئ `label` يحتوي على النص `Hero Name` وعنصر `input` من النوع `text` بداخله.

اربط `input` الخاص بـ `Hero Name` بمتغير الحالة `heroName` عبر إعطائه خاصية `value` بقيمة `heroName`.
...
يجب أن يحتوي عنصر `label` الخاص بك على النص `Hero Name`.
...
يجب أن تنشئ عنصر `input` من النوع `text` داخل عنصر `label` الخاص بك.
...
يجب أن تكون قيمة عنصر `input` الخاص بك مضبوطة على متغير الحالة `heroName`.
