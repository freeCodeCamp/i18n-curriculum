...
三項演算子を使って、`.hasOwnProperty()` の呼び出しを条件にしてください。オブジェクトにそのプロパティがあれば、`apply` と `fn` を引数にして `args` を呼び出した結果を返してください。そうでなければ、`match` を返してください。

# --hints--

コールバック関数は三項構文を使うべきです。
...
三項の条件が真の場合、コールバック関数は `apply()` の呼び出し結果を返すべきです。
