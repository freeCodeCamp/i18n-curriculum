...
Ongeza sifa ya kipengele `value` ya `player.imageUrl` kwa URL ya Picha `input`. Kisha weka sifa ya kipengele `onChange` kuwa kitendakazi cha mshale kinachopokea `e` kama kigezo chake na kufanya wito wa `setPlayer` kwa kueneza `player` na `imageUrl` kuwekwa kuwa `e.target.value`.

# --hints--

URL yako ya Picha `input` inapaswa kuwa na `value` sawa na URL ya picha ya mchezaji.
...
URL yako ya Picha `input` inapaswa kuwa na mshughulikiaji wa `onChange` anayefanya wito wa `setPlayer` kwa kueneza `player` na `imageUrl` kuwekwa kuwa `e.target.value`.
