...
Метод `.extend()` дозволяє додати елементи з ітерованого об’єкта в кінець списку:
...
Створіть умову `else` і використайте метод `.extend()` для додавання поточного шляху вузла до шляху сусіднього вузла.

# --hints--

Ви повинні створити умову `else` після вкладеної інструкції `if`.
...
У вашій умові `paths[node].extend(paths[current])` має бути `else`.
