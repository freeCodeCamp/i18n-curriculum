...
Il metodo `.trim()` rimuove gli spazi bianchi da entrambi i lati di una stringa. Per esempio, `" hello ".trim()` restituisce `"hello"`.

All'interno del corpo del loop, inserisci `parts[i].trim()` in `trimmedParts`. Poi aggiorna l'istruzione `return` esistente per restituire `trimmedParts` invece di `parts`.

Dopo questo passaggio, l'output registrato dovrebbe mostrare valori puliti senza spazi extra.

# --hints--

Dovresti inserire `parts[i].trim()` in `trimmedParts` all'interno del loop.

```js
assert.match(parseCard.toString(), /trimmedParts\s*\.\s*push\s*\(\s*parts\s*\[\s*\w+\s*\]\s*\.\s*trim\s*\(\s*\)\s*\)/);
```

`parseCard` dovrebbe restituire `trimmedParts`.
...
`parseCard` dovrebbe restituire un array in cui ogni elemento non ha spazi bianchi iniziali o finali.

```js
assert.isArray(parseCard("  A  |  B  |  C  |  D  "));
```

Quando viene chiamato con `"  A  |  B  |  C  |  D  "`, il primo elemento dell'array restituito dovrebbe essere `"A"`.
