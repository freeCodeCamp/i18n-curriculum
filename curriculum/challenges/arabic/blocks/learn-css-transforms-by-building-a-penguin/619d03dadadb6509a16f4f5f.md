...
استهدف العنصر `.chin`، وأعطه خاصية `width` بقيمة `90%`، و`height` بقيمة `70%`، و`background-color` بقيمة `white`.

# --hints--

يجب أن تستخدم المحدِّد `.chin`.

```js
assert.match(code, /\.chin\s*\{/);
```

يجب أن تعطي `.chin` خاصية `width`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.width);
```

يجب أن تعطي `.chin` خاصية `width` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.width, '90%');
```

يجب أن تعطي `.chin` خاصية `height`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.height);
```

يجب أن تعطي `.chin` خاصية `height` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.height, '70%');
```

يجب أن تعطي `.chin` خاصية `background-color`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.backgroundColor);
```

يجب أن تعطي `.chin` خاصية `background-color` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
