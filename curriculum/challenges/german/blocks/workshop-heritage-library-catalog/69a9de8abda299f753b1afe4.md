...
Fügen Sie eine `for`-Schleife zum Durchlaufen von `catalog` hinzu. Deklarieren Sie innerhalb der Schleife `const entry = catalog[i]`. Erstellen Sie einen String für die Zeile mit den vier Feldern, die durch Kommas getrennt sind, und fügen Sie ihn in `rows` ein.

String-Felder benötigen im Ausgabeformat wörtliche Anführungszeichen (z. B. `"King, Stephen"`), damit Kommas innerhalb der Werte das CSV-Format nicht unterbrechen. Da `year` eine Zahl ist, benötigt es keine Anführungszeichen.
...
Jede Zeile sollte die Felder des Eintrags durch Kommas getrennt enthalten.

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

String-Felder sollten in Anführungszeichen eingeschlossen sein.
