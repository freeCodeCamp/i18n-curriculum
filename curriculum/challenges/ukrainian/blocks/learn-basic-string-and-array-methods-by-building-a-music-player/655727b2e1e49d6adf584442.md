...
Якщо плейлист порожній, потрібно створити елемент `resetButton` і текст для нього. Ця кнопка з’явиться лише тоді, коли плейлист порожній.

`createElement()` — це DOM-метод, який ви можете використати для динамічного створення елемента за допомогою JavaScript. Щоб використати `createElement()`, викличте його, передавши назву тегу у вигляді рядка:
...
Також ви можете призначити його змінній:

```js
const divElement = document.createElement('div')
```

Усередині вашої інструкції `if` оголосіть константу `resetButton`, а потім використайте `createElement()` для створення `"button"`.


# --hints--

Вам не слід змінювати існуючу інструкцію `if` та її вміст.

```js
assert.match(code, /if\s*\(\s*userData\?\.songs\.length\s*===\s*0\s*\)\s*\{\s*.*\s*\}/)
```

Використайте метод `createElement` для створення `"button"`.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

Призначте щойно створений елемент `button` константі `resetButton`.
