...
你需要一个函数来验证输入的数字，并确保它们不是负数。

为此，创建一个带有 `handleInput` 参数且参数类型为 `e` 的 `Event` 函数。在函数内部，创建一个 `value` 变量，其值设置为目标值。确保它被转换为数字，并使用 `HTMLInputElement` 关键字强制转换为 `as` 类型。

# --hints--

你应该创建一个 `handleInput` 函数。

```js
assert.isFunction(handleInput)
```

你的 `handleInput` 函数应该有一个类型为 `e` 的 `Event` 参数。
