...
それぞれのカテゴリーには、[freeCodeCamp フォーラム](https://forum.freecodecamp.org/)上の対応するカテゴリーにリンクする URL があります。

const の変数 `url` を作成し、テンプレートリテラルを割り当ててください。 そのテンプレートリテラルには `${forumCategoryUrl}${selectedCategory.className}/${id}` という値を入れてください。
...
変数 `url` にはテンプレートリテラルを割り当ててください。

```js
assert.match(code, /const\s+url\s*=\s*`[\s\S]*`/);
```

テンプレートリテラルの中には `${forumCategoryUrl}${selectedCategory.className}/${id}` の値が必要です。
