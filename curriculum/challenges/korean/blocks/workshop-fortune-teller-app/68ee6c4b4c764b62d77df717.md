...
경우에 따라 카드가 항상 올바른 위치에 있지 않으므로 카드를 뒤집어야 할 때가 있습니다.

`isReversed` 메서드 안에 `singleCardSelected`라는 변수를 선언하세요. `Math.random()` 메서드를 사용해 무작위 불리언 값을 생성하세요. 값이 범위를 벗어나지 않도록 `0.5`과 비교하세요.
