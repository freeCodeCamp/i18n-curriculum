...
Zungusha kilele, kwa kumpa kipengele cha bandia pembe za chini mduara wa `100%`, ukiwaacha pembe za juu zikiwa `0%`.

# --hints--

Unapaswa kutumia sifa ya `border-radius` kuzungusha kilele.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

Unapaswa kumpa `.penguin-body::before` `border-radius` ya `0% 0% 100% 100%`.
