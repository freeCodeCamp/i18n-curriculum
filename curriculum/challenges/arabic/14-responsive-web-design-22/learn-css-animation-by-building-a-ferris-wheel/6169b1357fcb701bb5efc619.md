...
يجب عليك إنشاء منتقي `25%` جديد في قاعدة `@keyframes cabins` الخاصة بك.
...
يجب أن يكون منتقي `25%` بين منتقيات `0%` و `50%` الخاصة بك.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '25%');
```

يجب أن يحتوي منتقي `25%` على خاصية `background-color` تم تعيينها إلى `yellow`.
