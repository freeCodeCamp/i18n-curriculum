...
Crear otro elemento `div`. Dentro de él, anidar un `button` para `submit` el formulario. Este botón debe tener el texto `Calculate Remaining Calories`.

Luego agrega un `button` con el `id` asignado a `clear` para borrar el formulario (no olvides darle un atributo `type` que impida enviarlo). Este botón necesita el texto `Clear`.
