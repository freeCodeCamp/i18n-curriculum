...
Щоб побачити результати даних, виведіть у консоль змінну `data` всередині вашого блоку `try`.

Після визначення `fetchData` викличте функцію та відкрийте консоль, щоб побачити результати.

# --hints--

Ви повинні використати `console.log`, щоб вивести у консоль змінну `data`.

```js
assert.match(code, /\s*console\.log\(\s*data\s*\)/);
```

Ваш `console.log` має бути всередині блоку `try`.

```js
assert.match(code, /\s*try\s*{[\s\S]*\s*console\.log\(\s*data\s*\)[\s\S]*}\s*catch\s*\(\s*err\s*\)\s*{\s*}/);
```

Ви повинні викликати функцію `fetchData()` після того, як визначена функція `fetchData()`.
