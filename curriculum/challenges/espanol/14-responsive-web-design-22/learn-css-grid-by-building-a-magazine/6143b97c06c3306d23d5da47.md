...
Within your `article` element, create an `h3` element with the `class` set to `list-title` and the text of `A Brief History`. Debajo de eso, crea un elemento `p` con el texto `Of the Curriculum`. Luego crea un elemento `ul` con la clase `lists`.
...
Su nuevo elemento `h3` debe tener `class` establecido en `list-title`.
...
Su nuevo `ul`, el elemento debe tener el `class` a `lists`.
