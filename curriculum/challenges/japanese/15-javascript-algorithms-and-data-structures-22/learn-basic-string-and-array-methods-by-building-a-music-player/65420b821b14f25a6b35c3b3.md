...
ページ上に曲が表示されるとき、それぞれの曲のタイトル、アーティスト、再生時間、および削除ボタンを表示するようにします。

ここからいくつかのステップでは、<dfn>map()</dfn> メソッドを使用してそれぞれの曲に対して新しい HTML を追加する方法を学びます。 このメソッドについては次のステップで詳しく説明します。

では `const` を使用して `songsHTML` という名前の変数を宣言し、`array.map()` を代入するところから始めましょう。

# --hints--

`songsHTML` という `const` 変数が必要です。

```js
assert.match(code, /const\s+songsHTML/);
```

`songsHTML` に `array.map()` を代入してください。
