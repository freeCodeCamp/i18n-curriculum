...
Necesitas obtener el `id` de la canción que se está reproduciendo actualmente. Para esto, puedes utilizar `userData?.currentSong?.id`.

Usa `getElementById()` para obtener el `id` de la canción que se está reproduciendo actualmente, luego utiliza literales de plantilla para anteponerle con `song-`. Asígnalo a la constante `songToHighlight`.

# --hints--

No debes modificar la función existente `highlightCurrentSong` ni su contenido.
...
Debes utilizar `document.getElementById()` y pasar ``song-${userData?.currentSong?.id}``.

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Debes asignar tu `getElementById()` a una constante `songToHighlight`.
