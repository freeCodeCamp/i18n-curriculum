...
最后，允许你的正则表达式匹配整个单词（就像你用 `freeRegex` 做的那样）。
...
你的 `stockRegex` 应该匹配 `it's stock alert time`。

```js
assert.match("it's stock alert time", stockRegex);
```

你的 `stockRegex` 应该匹配 `stock alert time`。

```js
assert.match('stock alert time', stockRegex);
```

你的 `stockRegex` 应该匹配 `it's stock alert`。

```js
assert.match("it's stock alert", stockRegex);
```

你的 `stockRegex` 应该匹配 `stock alert`。

```js
assert.match('stock alert', stockRegex);
```

你的 `stockRegex` 不应该匹配 `hands-stock alert time`。

```js
assert.notMatch('hands-stock alert', stockRegex);
```

你的 `stockRegex` 不应该匹配 `stock alert-management`。
