...
Ora che hai creato il pulsante, devi assegnargli un testo. Per farlo, devi usare il metodo `createTextNode()` del DOM.

Il metodo `createTextNode()` serve per creare un nodo di testo. Per usarlo, lo chiami passando il testo come stringa:
...
Usa il metodo `createTextNode()` per creare un testo `"Reset Playlist"`, quindi assegnalo a una costante `resetText`.
...
Dovresti usare `document.createTextNode()` per creare un testo `"Reset Playlist"`.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

Dovresti assegnare il nodo di testo creato a una costante `resetText`.
