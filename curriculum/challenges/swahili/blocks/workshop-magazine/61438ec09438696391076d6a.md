...
Chini ya kipengele chako cha `.heading`, tengeneza kipengele kipya cha `section` chenye `class` imewekwa kuwa `text`. Ndani yake, tengeneza kipengele cha `p` chenye `class` imewekwa kuwa `first-paragraph` na maandishi yafuatayo:
...
Kipengele chako kipya cha `section` kinapaswa kuja baada ya kipengele chako cha `.heading`.

```js
assert.equal(document.querySelectorAll('section')?.[1]?.previousElementSibling?.className, 'heading');
```

Kipengele chako kipya cha `section` kinapaswa kuwa na `class` imewekwa kuwa `text`.

```js
assert.equal(document.querySelectorAll('section')?.[1]?.className, 'text');
```

Unapaswa kutengeneza kipengele kipya cha `p` ndani ya kipengele chako cha `.text`.

```js
assert.lengthOf(document.querySelector('.text')?.querySelectorAll('p'), 1);
```

Kipengele chako kipya cha `p` kinapaswa kuwa na `class` imewekwa kuwa `first-paragraph`.
