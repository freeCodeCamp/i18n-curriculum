...
要查看資料結果，請在你的 `try` 區塊內將 `data` 變數記錄到主控台。 

在你的 `fetchData` 定義下方，呼叫該函式並打開主控台以查看結果。

# --hints--

你應該使用 `console.log` 將 `data` 變數記錄到主控台。

```js
assert.match(code, /\s*console\.log\(\s*data\s*\)/);
```

你的 `console.log` 應該在 `try` 區塊內。

```js
assert.match(code, /\s*try\s*{[\s\S]*\s*console\.log\(\s*data\s*\)[\s\S]*}\s*catch\s*\(\s*err\s*\)\s*{\s*}/);
```

你應該在定義 `fetchData()` 函式（程式）後呼叫 `fetchData()` 函式（程式）。
