...
现在你知道了 `linear-gradient` 函数和色标的基本原理，你可以用它们来使马克笔看起来更加真实。

在 `linear-gradient` 函数中，将 `gradientDirection` 设置为 `180deg`。

# --hints--

你的 `.red` CSS 规则应该有一个 `background` 属性，设置为 `linear-gradient(180deg, rgb(255, 0, 0) 75%, rgb(0, 255, 0), rgb(0, 0, 255))`。
