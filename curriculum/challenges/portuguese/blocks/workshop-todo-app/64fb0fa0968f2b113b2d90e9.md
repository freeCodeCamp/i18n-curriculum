...
Como você fez na função `deleteTask`, você precisa encontrar o índice da tarefa a ser editada.

Crie uma variável `dataArrIndex`. Para obter seu valor, utilize o método `findIndex()` em `taskData`. Passe `item` como parâmetro para sua função de callback e verifique se o `id` de `item` é igual ao `id` do `parentElement` de `buttonEl`.

# --hints--

Você não deve alterar a função `editTask`.
...
Você deve passar `item` como parâmetro do callback da função arrow `findIndex()`. Não use chaves.

```js
assert.match(code, /const\s+editTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)/)
```

Sua função de callback arrow deve verificar se `item.id === buttonEl.parentElement.id`.
