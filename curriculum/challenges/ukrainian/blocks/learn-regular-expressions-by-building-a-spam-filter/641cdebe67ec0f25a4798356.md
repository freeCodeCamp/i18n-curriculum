...
Ваша функція зворотного виклику має містити інструкцію `if`.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

Ваша інструкція `if` має перевіряти, чи є `value` у `messageInput` порожнім рядком.
...
Ваша інструкція `if` має відобразити користувачу сповіщення з повідомленням `"Please enter a message."`.
...
Ваша інструкція `if` має припинити виконання функції.
