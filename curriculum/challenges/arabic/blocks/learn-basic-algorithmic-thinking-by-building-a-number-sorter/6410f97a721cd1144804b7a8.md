...
مثل فرز الفقاعات، يحتاج فرز الاختيار إلى التكرار خلال المصفوفة. أعلن عن حلقة `for` للقيام بذلك.
...
يجب أن تهيئ حلقة `for` متغيرًا `i` بقيمة `0`.

```js
assert.match(code, /const\s+selectionSort\s*=\s*(\(\s*array\s*\)|array)\s*=>\s*{\s*for\s*\(\s*let\s+i\s*=\s*0\s*;\s*/);
```

يجب أن تحتوي حلقة `for` على شرط يتحقق مما إذا كانت قيمة `i` أقل من طول المصفوفة.
...
يجب أن تحتوي حلقة `for` على جسم فارغ.
