...
Ahora comenzarás el proceso para cambiar el input de posición a un select.

Cambia el input a un elemento `select` con el id `position`, un `className` de `input` y un `value` de `player.position`.

# --hints--

Debes cambiar el `input` de posición a un elemento `select` con un `id` de `position` y `className` de `input`.
...
Tu `select` de posición debe tener un `value` de `player.position`.
