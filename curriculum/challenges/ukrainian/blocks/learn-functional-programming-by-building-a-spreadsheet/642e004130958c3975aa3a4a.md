...
Наразі ваша функція `range` повертає масив з правильною довжиною, але всі значення в ньому дорівнюють значенню `start`. Щоб це виправити, додайте метод `.map()` до вашого методу `.fill()`.

Передайте методу `.map()` функцію зворотного виклику, яка приймає `element` і `index` як параметри та повертає суму цих параметрів.

# --hints--

Вам слід використати метод `.map()`.

```js
assert.match(code, /\.map\(/);
```

Вам слід додати метод `.map()` до вашого методу `.fill()`.

```js
assert.match(code, /const\s+range\s*=\s*\(\s*start\s*,\s*end\s*\)\s*=>\s*Array\(\s*end\s*-\s*start\s*\+\s*1\s*\)\.fill\(\s*start\s*\)\.map\(/);
```

Вам слід передати функцію зворотного виклику методу `.map()` за допомогою стрілкової функції.

```js
assert.match(code, /const\s+range\s*=\s*\(\s*start\s*,\s*end\s*\)\s*=>\s*Array\(\s*end\s*-\s*start\s*\+\s*1\s*\)\.fill\(\s*start\s*\)\.map\(\s*\(.*\)\s*=>/);
```

Ваша функція зворотного виклику `.map()` має приймати `element` як перший параметр.

```js
assert.match(code, /const\s+range\s*=\s*\(\s*start\s*,\s*end\s*\)\s*=>\s*Array\(\s*end\s*-\s*start\s*\+\s*1\s*\)\.fill\(\s*start\s*\)\.map\(\s*\(\s*element/);
```

Ваша функція зворотного виклику `.map()` має приймати `index` як другий параметр.
...
Ваша функція зворотного виклику `.map()` має використовувати неявне повернення.
...
Ваша функція зворотного виклику `.map()` має неявно повертати суму `element` і `index`.
