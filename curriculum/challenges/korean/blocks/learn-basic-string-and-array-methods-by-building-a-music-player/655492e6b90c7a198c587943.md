...
현재 재생 중인 노래에 속성을 다시 추가해야 합니다.

조건 `if`을 가진 `songToHighlight` 문을 만드세요. 이 문에서 `setAttribute`에 대해 `songToHighlight`를 사용하여 `"aria-current"`와 `"true"`를 첫 번째와 두 번째 인수로 전달하세요.

# --hints--

조건 `if`을 가진 `songToHighlight` 문을 만들어야 합니다.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

`setAttribute()` 문 안에서 `songToHighlight`에 `if` 메서드를 사용해야 합니다.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

`"aria-current"` 메서드의 첫 번째와 두 번째 인수로 `"true"`과 `setAttribute`를 전달해야 합니다.
