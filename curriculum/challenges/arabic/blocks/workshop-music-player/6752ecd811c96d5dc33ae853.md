...
لضمان تحديث عرض المشغل كلما بدأ تشغيل أغنية جديدة، نفذ استدعاء الدالة `setPlayerDisplay()` داخل الدالة `playSong()`.

الآن يجب أن ترى عنوان الأغنية والفنان يظهران في العرض.

# --hints--

يجب أن تنفذ استدعاء `setPlayerDisplay` داخل دالتك `playSong`.
