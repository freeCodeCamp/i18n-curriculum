...
在 `is_valid` 方法中，透過呼叫 `valid_in_row()` 方法並以 `row` 和 `num` 作為引數，檢查該數字是否可在指定的列中插入，並將結果指定給變數 `valid_in_row`。記得使用 `self` 來取用目前實體的方法。

# --hints--

你應該宣告一個變數 `valid_in_row` 並指定、設值為 `self.valid_in_row(row, num)`。
