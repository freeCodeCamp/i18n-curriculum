...
У попередніх кроках ви навчилися працювати з методом `sort()` для сортування пісень в алфавітному порядку. Іншим випадком використання функції зворотного виклику є рандомізація масиву.

Одним зі способів рандомізації масиву елементів було б відняти `0.5` від `Math.random()`, що дає випадкові значення, які є додатними або від’ємними. Це робить результат порівняння сумішшю позитивних та негативних значень, що призводить до випадкового впорядкування елементів.
...
Використайте метод `sort()` на масиві `userData?.songs`. Передайте методу зворотний виклик та поверніть результат `Math.random() - 0.5`.


# --hints--

Не змінюйте наявну функцію `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

Використайте метод `sort()` на `userData?.songs`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

Метод `sort()` повинен мати функцію зворотного виклику яка використовує стрілковий синтаксис.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

Зворотний виклик методу `sort()` має повернути `Math.random() - 0.5`.
