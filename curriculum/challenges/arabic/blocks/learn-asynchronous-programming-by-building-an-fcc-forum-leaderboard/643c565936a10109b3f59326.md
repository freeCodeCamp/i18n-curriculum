...
أثناء بناء دالة `showLatestPosts()`، ستحتاج إلى استدعائها لرؤية تغييراتك.

استدعِ دالة `showLatestPosts()` في نهاية كتلة `try` ومرر `data` كمعلمة.

# --hints--

يجب استدعاء `showLatestPosts()` في نهاية كتلة `try`.

```js
assert.match(code, /\s*showLatestPosts\(.*\)\s*;?\s*/);
```

يجب أن تمرر `data` كمعلمة إلى الدالة `showLatestsPosts()`.
