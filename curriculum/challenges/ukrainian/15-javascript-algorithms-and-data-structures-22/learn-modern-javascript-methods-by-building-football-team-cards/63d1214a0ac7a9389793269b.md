...
Всередині цього масиву `players` створіть новий об’єкт із такими властивостями:
...
_ПРИМІТКА_: номери команд упорядковано за алфавітом прізвища. Це відрізняється від звичайної нумерації, де номери відповідають тим, що на футболках гравців.

# --hints--

Масив `myFavoriteFootballTeam.players` повинен мати значення об’єкта.

```js
assert.isObject(myFavoriteFootballTeam.players[0])
```

Новий об’єкт повинен мати властивість `name`.
...
Новий об’єкт повинен мати властивість `position`.
...
Новий об’єкт повинен мати властивість `number`.

```js
assert.property(myFavoriteFootballTeam.players[0], 'number');
```

Властивість `number` повинна бути числом.
...
Новий об’єкт повинен мати властивість `isCaptain`.

```js
assert.property(myFavoriteFootballTeam.players[0], 'isCaptain');
```

Ваша властивість `isCaptain` повинна бути булевої.
...
Новий об’єкт повинен мати властивість `nickname`.
