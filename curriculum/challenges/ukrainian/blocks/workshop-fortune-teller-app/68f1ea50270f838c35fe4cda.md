...
Тепер зробіть те саме для `subTitle`, призначивши його `meaning_up`, а для `title` призначте `foundCard.name`. Після цього викличте метод `showElements` і передайте `fortuneDescription` з елементів.

# --hints--

Ви повинні призначити `this.elements.subTitle.textContent` значенню `foundCard.meaning_up`.
...
Ви повинні призначити `this.elements.title.textContent` значенню `foundCard.name`.
...
Ви повинні викликати функцію `showElements` всередині `showFortune`.
...
Ви повинні передати `this.elements.fortuneDescription` як аргумент у `showElements`.
