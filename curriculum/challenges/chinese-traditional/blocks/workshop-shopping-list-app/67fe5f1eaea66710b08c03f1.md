...
現在你將根據一些邏輯來劃掉列表中選定的任何項目。

首先，由於此 `filteredItems.map()` 調用中的所有內容都被隱式返回，因此請用花括號將其中的所有代碼括起來，並顯式返回 `li` 元素。

例如，你可以使用呈現名稱列表的簡單組件執行此操作：
...
你應該將 `filteredItems.map()` 調用內的代碼用花括號括起來。
