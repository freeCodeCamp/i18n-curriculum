...
Функція Мебіуса, позначена як $μ(n)$, визначається так:

- $μ(n) = (-1)^{ω(n)}$, якщо $n$ є безквадратним (де $ω(n)$ — кількість різних простих множників числа $n$)
- $μ(n) = 0$, якщо $n$ не є безквадратним.

Нехай $P(a, b)$ — кількість цілих чисел $n$ в інтервалі $[a, b]$, для яких $μ(n) = 1$.

Нехай $N(a, b)$ — кількість цілих чисел $n$ в інтервалі $[a, b]$, для яких $μ(n) = -1$.

Наприклад, $P(2, 10) = 2$ та $N(2, 10) = 4$.

Нехай $C(n)$ — кількість пар цілих чисел $(a, b)$ таких, що:
...
`mobiusFunctionAndIntervals()` має повертати `198775297232878`.
