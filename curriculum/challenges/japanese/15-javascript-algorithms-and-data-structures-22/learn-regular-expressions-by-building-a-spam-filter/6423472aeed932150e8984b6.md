...
同じ構文を使用して、`c` について `c`、`{`、`[`、`(` とマッチするように変更してください。

# --hints--

`stockRegex` では文字クラスを使用して、`c`、`{`、`[`、および `(` にマッチさせてください。
...
`stockRegex` が `570(k 4l3r7` にマッチするようにしてください。

```js
assert.match('570(k 4l3r7', stockRegex);
```

`stockRegex` が `sto{k alert` にマッチするようにしてください。
