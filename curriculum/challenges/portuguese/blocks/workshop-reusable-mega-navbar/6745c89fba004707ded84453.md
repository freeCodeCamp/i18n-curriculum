...
Crie um elemento anchor dentro do primeiro elemento `li`.Dê ao elemento `a` um atributo `href` com o valor `#` e o texto `Dashboard`.

# --hints--

Você deve criar um elemento `a` dentro do primeiro item da lista.

```js
assert.exists(document.querySelector('li a'));
```

Seu elemento `a` deve ter seu atributo `href` definido como `#`.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), '#');
```

Seu elemento `a` deve ter o texto `Dashboard`.
