...
3番目の`section`要素の中に、テキストが`h2`の`Cat Form`要素を追加してください。

# --hints--

3番目の`section`要素が見つかりません。誤って削除したか、開始タグまたは終了タグが欠けている可能性があります。
...
`h2`要素には開始タグと終了タグが必要です。どちらか一方または両方が欠けているかもしれません。
...
`h2`要素は1つだけ追加してください。余分なものは削除してください。

```js
assert(document.querySelectorAll('h2').length === 3);
```

新しい`h2`要素は3番目の`section`要素の終了タグのすぐ上に配置してください。
