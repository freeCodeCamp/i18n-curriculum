---
id: 657b138d33db9e50f116b5f0
title: Tarefa 15
challengeType: 19
dashedName: task-15
---

<!-- (Audio) Anna: Great! Can you tell me about your qualifications? Were you educated in computer science? -->

# --description--

Ouça o áudio e responda à pergunta.

# --questions--

## --text--

Como Anna pergunta sobre a formação educacional do candidato em ciência da computação?

## --answers--

`Are you educated in computer science?`

### --feedback--

`Are you` está no tempo presente, mas a pergunta da Anna é sobre a educação passada do candidato.

---

`Do you study computer science?`

### --feedback--

`Do you study` está no tempo presente.A pergunta é sobre educação passada.

---

`Were you educated in computer science?`

---

`Have you studied computer science?`

### --feedback--

`Have you studied` é uma forma diferente, perguntando se eles já estudaram isso em algum momento. `Were you educated` pergunta especificamente sobre educação formal no passado.

## --video-solution--

3

# --scene--

```json
{
  "setup": {
    "background": "interview-room3.png",
    "characters": [
      {
        "character": "Anna",
        "position": {"x":50,"y":15,"z":1.2},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "3.1-1.mp3",
      "startTime": 1,
      "startTimestamp": 34.10,
      "finishTimestamp": 38.20
    }
  },
  "commands": [
    {
      "character": "Anna",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Anna",
      "startTime": 1,
      "finishTime": 5.10,
      "dialogue": {
        "text": "Great. Can you tell me about your qualifications? Were you educated in computer science?",
        "align": "center"
      }
    },
    {
      "character": "Anna",
      "opacity": 0,
      "startTime": 5.60
    }
  ]
}
```
