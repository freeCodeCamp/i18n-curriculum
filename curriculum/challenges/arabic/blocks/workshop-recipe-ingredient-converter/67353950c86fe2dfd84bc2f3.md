...
ستحتاج إلى تعديل عدد الحصص التي يتم حسابها. هذه فرصة رائعة لاستخدام دالة من نوع currying أخرى.

أعلن دالة `adjustForServings` التي تقبل معلمة `baseQuantity`. يجب أن تُرجع دالة ثانية تقبل معلمة `newServings`.

يجب أن تُرجع الدالة الداخلية نتيجة الضرب بين `baseQuantity` و `newServings`.

# --hints--

يجب أن تعلن دالة `adjustForServings`.

```js
assert.isFunction(adjustForServings);
```

يجب أن تقبل دالة `adjustForServings` معلمة `baseQuantity`.

```js
assert.match(adjustForServings?.toString(), /\(?baseQuantity\)?/);
```

يجب أن تُرجع دالة `adjustForServings` دالة أخرى.

```js
assert.isFunction(adjustForServings?.());
```

يجب أن تحتوي الدالة الداخلية على معلمة `newServings`.
