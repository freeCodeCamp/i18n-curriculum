...
Sasa baada ya kuwa na orodha ya nyimbo zinazoonyeshwa kwenye skrini, itakuwa vizuri kuzipanga kwa mpangilio wa alfabeti kulingana na kichwa cha habari. Unaweza kusahihisha safu ya data ya `allSongs` kwa mkono, lakini JavaScript ina njia ya safu ya data unayoweza kutumia inayoitwa <dfn>sort()</dfn>.

Njia ya `sort()` hubadilisha vipengele vya safu ya data kuwa mfuatano wa herufi na kuzipanga mahali pake kulingana na thamani zao katika usimbaji wa `UTF-16`.
...
Ndani ya kitendakazi chako cha `sortSongs`, ongeza njia ya `sort()` kwa `userData?.songs`. 

# --hints--

Unapaswa kuongeza njia ya `sort()` kwa `userData?.songs`.
