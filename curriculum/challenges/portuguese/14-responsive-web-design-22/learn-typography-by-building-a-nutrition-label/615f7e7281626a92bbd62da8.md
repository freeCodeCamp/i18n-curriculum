...
Após o último elemento `p`, crie outro elemento `p` e dê a ele o texto `Sodium 160mg 7%`. Put `Sodium` and `7%` each in their own `span` with a class of a `bold` like you did with the others.
