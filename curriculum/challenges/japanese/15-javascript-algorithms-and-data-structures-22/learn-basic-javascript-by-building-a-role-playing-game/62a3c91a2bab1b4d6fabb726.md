...
次は `goCave` 関数を作成し、`"Going to cave."` をコンソールに表示してください。

# --hints--

`function` キーワードを使用して `goCave` を宣言してください。

```js
assert.match(code, /function\s+goCave/);
```

`goCave` は関数でなければなりません。

```js
assert.isFunction(goCave);
```

コード内に `console.log("Going to cave.");` という行が必要です。

```js
assert.match(code, /console\.log\(\s*('|")Going to cave\.\1\s*\)/);
```

`console.log("Going to cave.");` という行は `goCave` 関数内に置く必要があります。
