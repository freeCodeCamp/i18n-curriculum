...
두 번째 표로 넘어갈 시간입니다. 먼저 `caption` 요소를 `Liabilities`로 설정하세요. 그런 다음 `thead`와 `tbody`를 추가하세요.

# --hints--

두 번째 `table` 요소에는 `caption` 요소가 있어야 합니다.
...
`caption` 요소에는 `Liabilities` 텍스트가 있어야 합니다.
...
두 번째 `table` 요소에는 `thead` 요소가 있어야 합니다.
...
두 번째 `table` 요소에는 `tbody` 요소가 있어야 합니다.
