...
Per mostrare finalmente i calcoli, itera sull'oggetto `propertyInputs` usando un loop `for...of` con `Object.entries()`. Puoi ignorare la chiave usando una virgola prima di `input` nella dichiarazione del loop.

# --hints--

Dovresti creare un loop `for...of` che destruttura ogni elemento da `Object.entries(propertyInputs)` saltando la chiave e mantenendo solo `input`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Il tuo loop `for...of` dovrebbe iterare su `Object.entries(propertyInputs)`.
