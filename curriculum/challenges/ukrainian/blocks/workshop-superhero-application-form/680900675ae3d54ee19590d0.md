...
Щоб виправити помилку, створіть стрілкову функцію `handlePowersChange` з параметром `e`.

Всередині функції деструктуруйте `value` і `checked` з `e.target`, щоб отримати значення прапорця та інформацію про те, чи він встановлений.

# --hints--

Ви маєте створити стрілкову функцію `handlePowersChange` з параметром `e`.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

Ви маєте деструктурувати `value` і `checked` з `e.target`.
