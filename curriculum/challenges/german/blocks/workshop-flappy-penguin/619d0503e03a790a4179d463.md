...
Positionieren Sie das `.chin`-Element so, dass es `25%` vom oberen Rand und `5%` vom linken Rand seines übergeordneten Elements entfernt ist. Geben Sie dann den oberen Ecken einen Radius von `70%` und den unteren Ecken einen Radius von `100%`.

# --hints--

Sie sollten `.chin` eine `top`-Eigenschaft geben.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Sie sollten `.chin` eine `top` von `--fcc-expected--` geben, aber `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Sie sollten `.chin` eine `left`-Eigenschaft geben.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Sie sollten `.chin` eine `left` von `--fcc-expected--` geben, aber `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Sie sollten `.chin` eine `border-radius` von `70% 70% 100% 100%` geben.
