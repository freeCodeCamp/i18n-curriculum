...
콜백 함수에는 네 개의 매개변수가 필요합니다. `match`, `arg1`, `operator`, 그리고 `arg2`입니다.

`match` 매개변수는 사용하지 않을 것이므로 접두사를 붙여 주세요.
