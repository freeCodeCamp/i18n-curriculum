...
title: 'Problema 66: equazione diofantea'
challengeType: 1
forumTopicId: 302178
...
Considera le equazioni diofantee quadratiche della forma:

<div style='text-align: center;'>x<sup>2</sup> – Dy<sup>2</sup> = 1</div>

Per esempio, quando D=13, la soluzione minima in x è 649<sup>2</sup> – 13×180<sup>2</sup> = 1.

Si può assumere che non esistano soluzioni in numeri interi positivi quando D è un quadrato.

Trovando le soluzioni minime in x per D = {2, 3, 5, 6, 7}, otteniamo quanto segue:
...
Quindi, considerando le soluzioni minime in `x` per D ≤ 7, il più grande `x` si ottiene quando D=5.

Trova il valore di D ≤ `n` nelle soluzioni minime di `x` per cui si ottiene il valore più grande di `x`.
