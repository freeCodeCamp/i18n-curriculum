...
而字幕，则是为不懂你所讲语言的人准备的。 这不仅有助于失聪或听力困难的人，也有助于在嘈杂或安静的环境中观看视频的人。

要为视频或音频内容添加说明或字幕，可以使用 `track` 或 `video` 元素中的 `audio` 元素：
...
在 `caption` 或 `video` 元素内使用 `audio` 标签。
...
在 `track` 或 `video` 元素内使用 `audio` 标签。

---

在 `subtitle` 或 `video` 元素内使用 `audio` 标签。
