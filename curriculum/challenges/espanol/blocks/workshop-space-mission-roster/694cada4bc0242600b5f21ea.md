...
El resto de tu tripulación ha sido creado en un arreglo llamado `remainingCrew` con los siguientes datos: 
...
Recorre el arreglo `remainingCrew` y añade cada astronauta a `squad` usando la función `addCrewMember()`.

# --hints--

Debes usar un ciclo `for` para llamar a `addCrewMember()` con cada astronauta en `remainingCrew`.
