...
次の行で、加算代入演算子 `+=` を使用して、`text` の値に文字列 `" You attack it with your <weapon>."` を追加してください。`<weapon>` の部分はプレイヤーの現在の武器に置き換えてください。 また、テキスト全体を適切に表示するため、この行のテキストはスペースから始まることを忘れないでください。

# --hints--

ドット記法を使用して、新しい行で `text` の `innerText` プロパティにアクセスしてください。
