...
Como você se lembra das lições anteriores, você pode usar o método `map` para iterar pelos diferentes objetos e renderizar os componentes dinamicamente assim:
...
Use o método `map()` para renderizar um único `Card` a partir do array. Passe os `name`, `title` e `bio` como props para o componente `Card`.

# --hints--

Você deve usar o método `map()` para iterar pelo array `profiles`.
