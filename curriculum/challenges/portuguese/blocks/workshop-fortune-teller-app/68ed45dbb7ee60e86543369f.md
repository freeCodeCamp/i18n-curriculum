...
Após buscar os dados, armazene a resposta em uma variável e analise-a para JSON. Dê ao dado analisado o tipo `Deck` e então atribua-o à propriedade `cards`. Dentro do bloco `catch`, certifique-se de registrar o erro com esta mensagem: `"Error fetching cards:", error`.

# --hints--

Você deve declarar uma variável chamada `data` dentro de `fetchCardsData`.
...
Sua variável `data` deve ter o tipo `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

Sua variável `data` deve ser atribuída analisando a resposta como JSON.
...
Você deve armazenar `data.cards` em `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Seu bloco `catch` deve usar `console.error()` para registrar o erro com a mensagem `"Error fetching cards:", error`.
