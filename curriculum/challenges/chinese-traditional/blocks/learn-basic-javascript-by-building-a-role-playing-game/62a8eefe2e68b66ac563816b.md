...
在你的 `defeatMonster` 函式中，將 `gold` 設為 `gold` 加上怪物的層次乘以 `6.7`。記得你可以使用 `monsters[fighting].level` 取得怪物的層次。

這裡是一個將 `num` 設為 `num` 加上 `5` 乘以 `8` 的範例：`num += 5 * 8`。使用 `Math.floor()` 將結果向下取整。
...
你應該使用 `Math.floor()` 來對怪物的層次乘以 `6.7` 的結果進行捨去取整。
...
你應該將 `Math.floor(monsters[fighting].level * 6.7)` 的結果添加到 `gold`。
