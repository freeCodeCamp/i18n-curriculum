...
작은 버그가 있습니다. `arg1`와 `arg2`은 숫자가 아니라 문자열입니다. `infixToFunction['+']("1", "2")`는 수학적으로 올바르지 않은 `12`를 반환합니다.

각 `infixToFunction[operator]` 인수를 `parseFloat()` 호출로 감싸세요.

# --hints--

`arg1`을 `parseFloat()` 호출로 감싸야 합니다.
