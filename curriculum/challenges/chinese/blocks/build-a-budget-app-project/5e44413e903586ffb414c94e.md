...
- `transfer` 方法接受金额和另一个预算类别作为参数。 该方法应添加一个提款，金额和描述为“转账到[目的地预算类别]”。描述 然后，该方法应将存款添加到其他预算类别中，金额和描述为“从 [源预算类别] 转账”。 如果没有足够的资金，则不应向任何账簿添加任何内容。 如果发生传输，此方法应返回`True`，否则返回`False`。
- 一个接受金额作为参数的`check_funds`方法。 如果金额大于预算类别的余额，则返回`False`，否则返回`True`。 此方法应同时用于`withdraw`方法和`transfer`方法。

打印预算对象时，应显示：

- 30个字符的标题行，其中类别名称居中于一行`*`字符中。
