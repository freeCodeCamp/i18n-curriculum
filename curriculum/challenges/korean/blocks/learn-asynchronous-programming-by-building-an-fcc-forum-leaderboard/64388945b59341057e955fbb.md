...
포럼 리더보드를 데이터로 채우려면 API에서 데이터를 요청해야 합니다. 이것은 <dfn>비동기</dfn> 작업으로, 메인 프로그램 흐름과 독립적으로 작업이 실행된다는 뜻입니다.

`async` 키워드를 사용해 프로미스를 반환하는 비동기 함수를 만들 수 있습니다.
...
`async` 키워드를 사용해 `fetchData`라는 비동기 화살표 함수를 만드세요.

# --hints--

`async` 키워드를 사용해 `fetchData`라는 비동기 화살표 함수를 만들어야 합니다.

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*[^}]*\s*}/);
```

`fetchData`가 정의되어야 합니다.

```js
assert.isDefined(fetchData);
```

`fetchData`는 함수여야 합니다.
