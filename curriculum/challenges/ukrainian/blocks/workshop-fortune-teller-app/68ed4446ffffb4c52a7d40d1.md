...
У конструкторі додайте виклики `fetchCardsData()` і `initializeEventListeners()` за допомогою ключового слова `this`, щоб вони запускалися під час ініціалізації класу.

Метод `fetchCardsData` відповідатиме за завантаження даних таро з API, а `initializeEventListeners` налаштує обробники натискань кнопок. Ви створите обидва методи на наступних кроках.

# --hints--

Ви повинні викликати метод `fetchCardsData()` всередині `constructor`.
...
Ви повинні викликати метод `initializeEventListeners()` всередині конструктора.
