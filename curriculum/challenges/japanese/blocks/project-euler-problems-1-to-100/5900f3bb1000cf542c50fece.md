...
オンラインバンキングでよく使われるセキュリティ方法の一つに、パスコードの中からランダムに3文字をユーザーに尋ねるものがあります。例えば、パスコードが `531278` の場合、2文字目、3文字目、5文字目を尋ねることがあり、その場合の期待される回答は `317` となります。

配列 `keylog1`、`keylog2`、および `keylog3` には、50回の成功したログイン試行が含まれています。

3文字は常に順番に尋ねられることがわかっているので、この配列を分析して、長さが不明な最短の秘密のパスコードを特定してください。
