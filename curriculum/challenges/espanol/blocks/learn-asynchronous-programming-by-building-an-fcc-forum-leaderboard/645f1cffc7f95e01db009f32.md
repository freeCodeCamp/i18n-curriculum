...
El siguiente paso es hacer un bucle a través del arreglo `posters` para obtener todos sus avatares. 

Comience agregando una palabra clave `return` seguida de `posters.map()`. Para la función callback, agregue un parámetro llamado `poster`.

# --hints--

Debes devolver `posters.map()`.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

Tu función de callback debería tener un parámetro llamado `poster`.
