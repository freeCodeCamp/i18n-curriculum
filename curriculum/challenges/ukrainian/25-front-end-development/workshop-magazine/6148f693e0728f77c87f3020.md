...
Нове правило `@media` повинне мати селектор `.hero-title`, селектор `.hero-subtitle, .author, .quote, .list-title`, селектор `.social-icons` та селектор `.text`. These selectors should be in this order.
