...
In diesem Projekt lernen Sie grundlegende String- und Array-Methoden, indem Sie eine Musikplayer-App erstellen. Sie können Lieder abspielen, pausieren, überspringen und mischen.

Das HTML und CSS dieses Projekts wurden für Sie bereitgestellt, sodass Sie sich auf das JavaScript konzentrieren können.

Beginnen Sie damit, auf die Elemente `#playlist-songs`, `#play` und `#pause` mit der `getElementById()`-Methode zuzugreifen. Weisen Sie sie den Variablen `playlistSongs`, `playButton` und `pauseButton` zu.


# --hints--

Sie sollten eine Variable namens `playlistSongs` haben. 

```js
assert.isDefined(playlistSongs);
```

Sie sollten `document.getElementById()` verwenden, um das `#playlist-songs`-Element zu erhalten.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

Sie sollten das `#playlist-songs`-Element der Variable `playlistSongs` zuweisen. 

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

Sie sollten eine Variable namens `playButton` haben. 

```js
assert.isDefined(playButton);
```

Sie sollten `document.getElementById()` verwenden, um das `#play`-Element zu erhalten.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

Sie sollten das `#play`-Element der Variable `playButton` zuweisen. 

```js
assert.equal(playButton, document.getElementById('play'));
```

Sie sollten eine Variable namens `pauseButton` haben. 

```js
assert.isDefined(pauseButton);
```

Sie sollten `document.getElementById()` verwenden, um das `#pause`-Element zu erhalten.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

Sie sollten das `#pause`-Element der Variable `pauseButton` zuweisen. 
