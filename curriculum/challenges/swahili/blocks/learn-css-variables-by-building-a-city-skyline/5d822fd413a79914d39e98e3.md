...
Unda sifa ya kigezo mpya iitwayo `--building-color4` na uipe thamani ya `#538cc6`. Hakikisha iko katika kichaguzi cha `:root` wakati huu. Kisha itumie kujaza jengo la mwisho.

# --hints--

Unapaswa kufafanua sifa ya kigezo mpya iitwayo `--building-color4`.

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color4'));
```

Unapaswa kumpa `--building-color4` thamani ya `#538cc6` katika kichaguzi cha `:root`.
