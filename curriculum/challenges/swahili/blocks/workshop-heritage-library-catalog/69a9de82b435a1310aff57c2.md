...
Baadhi ya vitabu vina `"Unknown"` kama mwaka wao. Tutashughulikia kesi hiyo ya kipekee kwanza.

Ndani ya mzunguko, angalia kama `book.year` ni sawa na mfuatano wa herufi `"Unknown"`. Kisha, angalia kama `grouped["Unknown"]` bado haipo na uanze kama safu ya data tupu. Sogeza `book` ndani ya `grouped["Unknown"]`.

Mwishowe, tumia `continue` kuruka sehemu nyingine za mwili wa mzunguko. Kauli ya `continue` inaruka moja kwa moja kwa mzunguko ujao.

# --hints--

Unapaswa kuangalia kama `book.year` ni sawa na `"Unknown"`.

```js
assert.match(groupByDecade.toString(), /book\.year\s*===?\s*("|')Unknown\1/);
```

Unapaswa kuangalia kama `grouped["Unknown"]` bado haipo na uanze kama safu ya data tupu.
...
Unapaswa kusogeza `book` ndani ya `grouped["Unknown"]`.

```js
assert.match(groupByDecade.toString(), /grouped\s*\[\s*("|')Unknown\1\s*\]\s*\.push\s*\(\s*book\s*\)/);
```

Unapaswa kutumia `continue` kuruka kwa mzunguko ujao baada ya kushughulikia mwaka usiojulikana.
