...
按照與 `cartSubTotal` 相同的模式，更新 `cartTaxes` 以顯示 `tax` 值，並更新 `cartTotal` 以顯示 `total` 屬性。

# --hints--

你應該訪問 `cartTaxes` 元素的 `textContent` 屬性。

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

你應該在 `tax` 變量上調用 `.toFixed()` 方法，並將 `2` 作爲參數傳遞。

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

你應該使用模板字面量語法在 `.toFixed()` 調用之前添加美元符號。
...
你應該將 `cartTaxes` 元素的 `textContent` 設置爲 `tax.toFixed` 模板字符串。
...
你應該訪問 `cartTotal` 元素的 `textContent` 屬性。

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

你應該在 `total` 變量上調用 `.toFixed()` 方法，並將 `2` 作爲參數傳遞。 請記住使用 `this` 關鍵字。
...
你應該將 `cartTotal` 元素的 `textContent` 設置爲你的 `total.toFixed` 模板字符串。
