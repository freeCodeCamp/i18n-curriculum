...
通常の関数と同様に、アロー関数も値を戻すことができます。

以下は、2つの数字を乗算した結果を戻すアロー関数の例です：
...
`addTwoNumbers` という新しい変数を作成し、アロー関数を代入してください。このアロー関数は `num1` と `num2` という2つのパラメータを受け取ります。関数の本体内で、`num1` と `num2` を足した式を戻してください。

`addTwoNumbers` 関数の下にコンソール文を追加してください。そのコンソール文の中で `addTwoNumbers` 関数を呼び出し、引数として数字の `3` と `4` を渡してください。

コンソールを開いて出力を確認しましょう。

# --hints--

`addTwoNumbers` という関数があるはずです。

```js
assert.isFunction(addTwoNumbers);
```

`addTwoNumbers` 関数はアロー関数の構文を使うべきです。

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

`addTwoNumbers` 関数は `num1` と `num2` という2つのパラメータを持つべきです。

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

`addTwoNumbers` 関数は `num1` と `num2` を足した式を戻すべきです。

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

`console.log(addTwoNumbers(3, 4));` 文があるはずです。
