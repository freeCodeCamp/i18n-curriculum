...
Para estilizar o botão de envio, você pode usar um seletor de *atributo*, que seleciona um elemento com base no valor do atributo fornecido. Aqui está um exemplo:
...
Você deve usar um seletor de atributo `input[type="submit"]` para estilizar o botão de enviar.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="submit"]'));
```

Você deve dar ao botão de enviar o `display` de `block`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.display, 'block');
```

Você deve dar ao botão de enviar o `width` de `60%`.
