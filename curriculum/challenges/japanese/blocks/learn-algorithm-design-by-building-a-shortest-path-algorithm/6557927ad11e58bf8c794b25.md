...
`for` ループを作成して `targets_to_print` を繰り返し、次の f-string を出力してください: `f'\n{start}-{node} distance: {distances[node]}\nPath: {" -> ".join(paths[node])}'`。

# --hints--

`for` を繰り返すために `targets_to_print` ループを作成してください。ループ変数として `node` を使います。
...
新しい `for` ループ内で指定された文字列を出力してください。
