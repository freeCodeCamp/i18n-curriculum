...
أضف معلمة `error` إلى كتلة `catch`، ثم استخدم `console.error` لتسجيل أي خطأ يحدث أثناء جلب البيانات أو معالجتها.

# --hints--

يجب أن تحتوي كتلة `catch` على معلمة `error`.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

يجب تسجيل أي خطأ في كتلة `catch` باستخدام `console.error`.
