...
Тепер ви обробите вибір позиції гравця. Потрібно переробити `input` на `select`, а потім спроєктувати масив дійсних футбольних позицій як значення опцій `select`.

Для початку створіть масив `POSITIONS` зі рядковими значеннями `GK`, `CB`, `LB`, `RB`, `CDM`, `CM`, `CAM`, `LW`, `RW`, `ST` та `CF`. Додайте `as const` одразу після закриваючих квадратних дужок, щоб TypeScript звузив тип із загального `string[]` до readonly кортежа з цих точних літеральних значень.

Це означає, що TypeScript точно знає, які рядки є дійсними, а не просто те, що це якийсь масив рядків.

# --hints--

Ви повинні оголосити константу `POSITIONS`.
...
Ваш масив `POSITIONS` має містити всі потрібні рядкові значення футбольних позицій, використайте `as const` для звуження типу до readonly рядкових літералів.
