...
استخدم صيغة السهم لإنشاء دالة `updateTaskContainer`. ثم انقل `taskData.forEach()` ومحتواها من مستمع حدث `submit` الخاص بـ `taskForm` إلى الدالة التي أنشأتها حديثًا.

# --hints--

يجب أن تستخدم `const` وصيغة السهم لإنشاء دالة `updateTaskContainer`.

```js
assert.match(code, /const\s+updateTaskContainer\s*=\s*\(\s*\)\s*=>\s*\{/)
```

يجب أن تنقل `taskData.forEach()` ومحتواها إلى دالة `updateTaskContainer()`.
