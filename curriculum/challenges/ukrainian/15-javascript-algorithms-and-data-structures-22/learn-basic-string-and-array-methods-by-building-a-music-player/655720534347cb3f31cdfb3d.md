...
Перш ніж видалити пісню, потрібно перевірити, чи пісня зараз відтворюється. Якщо так, вам потрібно призупинити пісню та відтворити наступну пісню зі списку відтворення.

Використайте інструкцію `if`, щоб перевірити, чи `userData?.currentSong?.id` дорівнює `id` пісні, яку ви хочете видалити.

# --hints--

Створіть порожню інструкцію `if` з умовою `userData?.currentSong?.id === id`.
