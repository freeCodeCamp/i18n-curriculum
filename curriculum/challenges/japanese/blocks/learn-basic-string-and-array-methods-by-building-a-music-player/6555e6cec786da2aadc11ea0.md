...
曲を再度レンダリングし、現在再生中の曲を一時停止し、プレーヤーの表示を設定し、再生ボタンのアクセシブルテキストを再設定する必要があります。

`renderSongs` 関数を呼び出し、`userData?.songs` を引数として渡してください。また、`pauseSong`、`setPlayerDisplay`、`setPlayButtonAccessibleText` 関数も呼び出してください。

# --hints--

既存の `shuffle` 関数とその内容は変更しないでください。
...
`renderSongs` 関数を `userData?.songs` で呼び出してください。
