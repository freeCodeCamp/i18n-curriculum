...
A veces necesitarás probar más de una cosa a la vez. El operador <dfn>logical and</dfn> (`&&`) devuelve `true` si y solo si los <dfn>operands</dfn> a la izquierda y derecha de él son verdaderos.
...
Reemplaza las dos sentencias if con una sola sentencia, usando el operador `&&`, que devolverá la cadena `Yes` si `val` es menor o igual a `50` y mayor o igual a `25`. De lo contrario, devolverá la cadena `No`.
