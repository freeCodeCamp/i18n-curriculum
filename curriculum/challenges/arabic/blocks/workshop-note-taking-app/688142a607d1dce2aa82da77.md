...
سيكون من الجيد رؤية التغييرات المعدلة في وحدة التحكم.

أضف `console.log(currentContent);`.

لاختبار تغييراتك، جرّب تعديل الملاحظة ثم النقر خارج الملاحظة. يجب أن ترى الآن الملاحظة المعدلة في وحدة التحكم.

# --hints--

يجب أن تضيف `console.log(currentContent);` داخل مستمع الحدث، بعد تعيين `newContent` إلى `currentContent`.
