...
卡片標題已完成。現在你需要添加玩家照片區塊。

在 `div` 的 `card-header` 結尾後，且仍在 `div` 的 `className` `card` 內，添加一個帶有 `div` `className` 的 `card-image-wrap`。目前先留空，下一步你會添加影像。

# --hints--

你應該添加一個帶有 `div` `className` 的 `card-image-wrap`。
