...
`.beak` 요소를 `class`의 `bottom`로 선택하고, `width`보다 `4%`만큼 작고, 위쪽에서 `.beak.top`만큼 더 떨어져 있으며, 부모 요소의 왼쪽에서 `5%`보다 `2%`만큼 더 떨어진 `.beak.top`를 지정하세요.

# --hints--

`.beak.bottom` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak']));
```

`.beak.bottom`에 `width`의 `--fcc-expected--`를 지정해야 하지만 `--fcc-actual--`이(가) 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak'])?.width, '16%');
```

`.beak.bottom`에 `top`의 `--fcc-expected--`를 지정해야 하지만 `--fcc-actual--`이(가) 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak'])?.top, '65%');
```

`.beak.bottom`에 `left`의 `--fcc-expected--`를 지정해야 하지만 `--fcc-actual--`이(가) 발견되었습니다.
