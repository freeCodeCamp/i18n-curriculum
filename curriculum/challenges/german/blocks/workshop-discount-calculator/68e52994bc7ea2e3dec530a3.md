...
Erstellen Sie innerhalb der `calculate_best_price`-Methode eine Liste namens `prices`, die `product.price` enthält. So stellen Sie sicher, dass der Kunde den vollen Preis zahlt, falls keine Rabatte gelten.

# --hints--

Innerhalb von `calculate_best_price` sollten Sie eine Liste namens `prices` erstellen, die `product.price` enthält.
