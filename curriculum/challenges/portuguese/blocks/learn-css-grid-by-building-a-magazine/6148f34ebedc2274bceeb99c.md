...
Agora que o layout de revista está terminado, você precisa torná-lo responsivo.
...
Você deve ter uma nova `@media` query para `only screen and (max-width: 720px)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

A nova `@media` query deverá ter um seletor `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

O novo seletor `.image-wrapper` deve ter a propriedade `grid-template-columns` de `1fr`.
