...
Now that you know how to find the next song to play, create a function named `playNextSong`. 関数 `playNextSong` の中で、`userData` の `currentSong` が `null` と厳密に等しいかをチェックする `if` 文を作成してください。 これにより、`userData` オブジェクトに現在再生中の曲がないことを確認します。
