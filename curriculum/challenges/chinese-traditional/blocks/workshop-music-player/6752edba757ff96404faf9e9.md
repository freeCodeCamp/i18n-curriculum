...
創建一個名為 `setPlayButtonAccessibleText` 的函式（程式）。在該函式中，將播放按鈕的 `aria-label` 屬性設為 `Play`，後面接一個空格和當前歌曲的標題。如果 `userData.currentSong` 是 `null`，則將其設為 `Play`。

# --hints--

你應該有一個名為 `setPlayButtonAccessibleText` 的函式（程式）。

```js
assert.isFunction(setPlayButtonAccessibleText);
```

你的 `setPlayButtonAccessibleText` 函式（程式）應該將播放按鈕的 `aria-label` 屬性設定為 `Play`，後面接一個空格和目前歌曲的標題。
...
```
 ...
 當 `userData.currentSong` 為 `null` 時，`setPlayButtonAccessibleText` 應該將播放按鈕的 `aria-label` 屬性設為 `Play`。
