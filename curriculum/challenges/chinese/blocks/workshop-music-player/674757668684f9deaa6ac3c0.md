...
仍然在 `playSong` 函数内，将 `audio.src` 属性设置为 `song.src`。这告诉音频元素去哪里查找所选歌曲的音频数据。

另外，将 `audio.title` 属性设置为等于 `song.title`。 这告诉音频元素要显示什么作为歌曲的标题。

# --hints--

你应该在你的 `playSong` 函数内将 `audio.src` 属性设置为与 `song.src` 相等。
...
你应该在你的 `playSong` 函数内将 `audio.title` 属性设置为与 `song.title` 相等。
