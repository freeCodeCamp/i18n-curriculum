...
これで UI に表示したい物は揃いました。 次は、`Load More Authors` ボタンをクリックしたときに、追加の著者データが読み込まれるようにします。 ボタンに `click` イベントを追加して、`startingIndex` と `endingIndex` を少しずつ増やしていくことでこれを実現します。

では、アロー関数構文を使って `fetchMoreAuthors` 関数を作成してください。 関数内にはまだ何も入れないでください。 また、この関数には複数の式を入れるため、波括弧を使用してください。
