...
أنشئ عنصر `button` آخر مع الصنف `playlist-song-delete` وخاصية `aria-label` مضبوطة على `Delete` مع تضمين `song.title`. للصق محتوى أيقونة الحذف، ألصق الـ SVG التالي:
...
يجب أن تنشئ عنصر `button` مع الصنف `playlist-song-delete` كأول خاصية وقيمة له.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

يجب أن يحتوي عنصر الزر على خاصية `aria-label` بقيمة `Delete ${song.title}` كثاني خاصية وقيمة له.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

يجب أن تلصق الـ `svg` المقدم كمحتوى لعنصر `button` الخاص بك.
