...
`.image-wrapper` 要素内で、最初の `img` 要素に `src` の `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`、`alt` の `image of the quote machine project`、`class` の `image-1`、`loading` に設定された `lazy` 属性、`width` の `600` 属性、`height` の `400` 属性を付けてください。

# --hints--

最初の `img` 要素には `src` に設定された `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png` 属性を付けてください。
