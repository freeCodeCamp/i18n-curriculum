...
Wenn die Pfeilfunktion einen einfachen Ausdruck zurückgibt, können Sie das Schlüsselwort `return` und die geschweiften Klammern `{}` weglassen. Das nennt man einen impliziten Rückgabewert.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

Wenn Ihre Pfeilfunktion mehrere Codezeilen im Funktionskörper hat, müssen Sie das Schlüsselwort `return` und die geschweiften Klammern `{}` verwenden.
...
Refaktorieren oder aktualisieren Sie Ihre `addTwoNumbers`-Funktion, um das Schlüsselwort `return` und die geschweiften Klammern `{}` zu entfernen. Ihre `addTwoNumbers`-Funktion sollte stattdessen einen impliziten Rückgabewert verwenden.

Öffnen Sie die Konsole, um sicherzustellen, dass Sie weiterhin die korrekte Ausgabe erhalten.

# --hints--

Ihre refaktorierte `addTwoNumbers`-Funktion sollte einen impliziten Rückgabewert verwenden.
