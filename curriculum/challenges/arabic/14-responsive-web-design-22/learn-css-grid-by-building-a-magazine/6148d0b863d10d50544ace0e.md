...
يجب أن يكون لمنتقي `.first-paragraph::first-letter` الخاص بك خاصية `float` بقيمة `left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float === 'left');
```

يجب أن يكون لمنتقي `.first-paragraph::first-letter` الخاص بك خاصية `margin-right` بقيمة `1rem`.
