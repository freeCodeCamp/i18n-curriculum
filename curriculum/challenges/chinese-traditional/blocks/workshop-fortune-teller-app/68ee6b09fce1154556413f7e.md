...
在 `initializeEventListeners` 方法中，為 `singleCardBtn` 附加一個 `click` 事件監聽器。在事件處理常式中，呼叫 `singleCardSelected` 方法。記得從 `elements` 取得 `singleCardBtn`。

你將在接下來的步驟中創建此方法。

# --hints--

你應該為 `click` 元素附加 `singleCardBtn` 事件。別忘了這裡需要使用 `this` 關鍵字。

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

在事件回呼中，你應該呼叫 `singleCardSelected` 方法。
