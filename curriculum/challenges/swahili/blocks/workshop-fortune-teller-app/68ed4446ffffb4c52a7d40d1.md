...
Ndani ya kijenzi, ongeza wito kwa `fetchCardsData()` na `initializeEventListeners()` kwa kutumia neno kuu la `this`, ili ziendeshwe wakati darasa la vitu linapowekwa.

Njia ya `fetchCardsData` itashughulikia upakiaji wa data za kadi za tarot kutoka API, na `initializeEventListeners` itapanga mshughulikiaji wa bofya za vitufe. Utaunda njia zote mbili katika hatua zinazofuata.

# --hints--

Unapaswa kuita njia ya `fetchCardsData()` ndani ya `constructor`.
...
Unapaswa kuita njia ya `initializeEventListeners()` ndani ya kijenzi.
