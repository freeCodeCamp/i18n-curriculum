...
Ancora una cosa. If you keep clicking the `Load More Authors` button until there's no more data to load and the text changes to `"No more data to load"`, the cursor value is still `pointer`. Perché invece non modificare il valore del cursore in `not-allowed`?
