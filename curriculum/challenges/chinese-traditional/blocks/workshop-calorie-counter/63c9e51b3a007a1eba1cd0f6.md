...
現在宣告一個 `remainingCalories` 變數，並將其值設為從 `budgetCalories` 減去 `consumedCalories` 再加上 `exerciseCalories`。

# --hints--

你的 `calculateCalories` 函式（程式）應該有一個 `remainingCalories` 變數。

```js
assert.match(calculateCalories.toString(), /remainingCalories\s*=/);
```

你應該找到 `budgetCalories - consumedCalories + exerciseCalories` 的值。
...
你應該將 `budgetCalories - consumedCalories + exerciseCalories` 的值指定給 `remainingCalories`。
