...
지금은 세 가지 사실이 모두 동시에 보입니다. 하지만 한 번에 하나의 사실만 보여야 합니다.

이를 위해 두 번째와 세 번째 `hidden` 요소에 `tabpanel` 속성을 설정해야 합니다.

# --hints--

두 번째 `tabpanel` 요소에는 `hidden` 속성이 있어야 합니다.

```js
assert.isTrue(document.querySelectorAll('div[role="tabpanel"]')[1].hasAttribute("hidden"));
```

세 번째 `tabpanel` 요소에는 `hidden` 속성이 있어야 합니다.
