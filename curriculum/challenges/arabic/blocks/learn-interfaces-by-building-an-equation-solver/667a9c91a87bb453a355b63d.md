...
لا تتردد في تغيير معاملات `lin_eq` لترى كيف يتغير الناتج.

بعد ذلك، احذف استدعاء `print(solver(lin_eq))`، واطبع نتيجة استدعاء `solver()` مع `quadr_eq` كمعلمة.

# --hints--

يجب ألا يحتوي شفرتك على `print(solver(lin_eq))`.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

يجب أن تطبع `solver(quadr_eq)`.
