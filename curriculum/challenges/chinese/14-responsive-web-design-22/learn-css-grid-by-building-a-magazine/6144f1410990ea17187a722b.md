...
在这种情况下，你的父元素是 `main` 元素。 通过添加值为 `1fr 94rem 1fr` 的 `grid-template-columns` 属性，将内容设置为具有三列布局。 这将创建三列，其中中间列的宽度为 `94rem`，第一列和最后一列都是网格容器中剩余空间的一半。

# --hints--

你的 `main` 部分应该有一个 `grid-template-columns` 属性设置为 `1fr 94rem 1fr`。
