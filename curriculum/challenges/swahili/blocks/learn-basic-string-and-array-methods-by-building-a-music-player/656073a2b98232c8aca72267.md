...
Ili kushughulikia taarifa za wimbo wa sasa na kufuatilia muda wake wa kucheza, tengeneza vigezo vya `currentSong` na `songCurrentTime`. Weka thamani zao kuwa `null` na `0` mtawalia.

# --hints--

Haupaswi kubadilisha kitu cha `userData` kilichopo na yaliyomo ndani yake.

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

Kituo chako cha `userData` kinapaswa kuwa na ufunguo wa `currentSong` uliowekwa kuwa `null`.

```js
assert.propertyVal(userData, "currentSong", null)
```

Kituo chako cha `userData` kinapaswa kuwa na ufunguo wa `songCurrentTime` uliowekwa kuwa `0`.
