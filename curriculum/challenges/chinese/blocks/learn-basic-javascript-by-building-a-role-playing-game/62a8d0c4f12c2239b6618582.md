...
向你的 `buyWeapon` 函数添加 `else` 语句。 在该语句中，将 `text.innerText` 设置为等于 `"You do not have enough gold to buy a weapon."`。

# --hints--

你应该向 `buyWeapon` 函数添加 `else` 语句。

```js
assert.match(buyWeapon.toString(), /else/);
```

你的 `else` 语句应位于 `if` 语句之后。
...
你应该将 `text.innerText` 设置为 `"You do not have enough gold to buy a weapon."`。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You do not have enough gold to buy a weapon.\1/);
```

你的 `else` 语句应将 `text.innerText` 设置为 `"You do not have enough gold to buy a weapon."`。
