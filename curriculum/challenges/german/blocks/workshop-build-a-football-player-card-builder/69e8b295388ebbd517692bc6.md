...
Das Spielerfoto wird von einer fest codierten URL geladen. Ersetzen Sie den Wert des `src`-Attributs `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` durch `player.imageUrl`.

Ersetzen Sie außerdem den fest codierten `alt`-Text durch einen dynamischen Wert mithilfe eines Template-Literals mit `player.name` und dem Text `headshot`.

Beachten Sie, dass Sie bei der Verwendung eines JavaScript-Ausdrucks als Attributwert in TSX geschweifte Klammern `{}` anstelle von Anführungszeichen verwenden. Zum Beispiel `src={player.imageUrl}`, nicht `src="{player.imageUrl}"`.

# --hints--

Sie sollten die fest codierte Bild-URL durch `player.imageUrl` als `src`-Attribut ersetzen.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

Sie sollten den fest codierten `alt`-Text durch ein Template-Literal ersetzen, das `player.name` und den Text `headshot` verwendet.
