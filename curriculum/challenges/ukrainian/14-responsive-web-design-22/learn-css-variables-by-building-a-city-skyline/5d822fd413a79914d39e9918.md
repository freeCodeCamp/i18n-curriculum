...
Ви використаєте ще деякі трюки з кордонами для верхньої секції. Додайте `border-bottom` зі значенням `7vh solid var(--building-color4)` до `.fb1a`. Внизу з’явиться кордон висотою `7vh`. Але оскільки елемент має нульовий розмір, він відображається лише як лінія шириною 2px від кордону 1px, який є на всіх елементах.
