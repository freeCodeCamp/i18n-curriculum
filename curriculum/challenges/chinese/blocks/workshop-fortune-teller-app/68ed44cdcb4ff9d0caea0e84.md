...
既然你在构造函数中调用了 `fetchCardsData` 和 `initializeEventListeners`，现在你需要在类中定义它们。将两者都创建为 `private` 方法。确保 `fetchCardsData` 是异步的，因为它将处理 API 调用。

# --hints--

你应该有一个名为 `private` 的 `fetchCardsData` 方法。
...
你应该有一个名为 `private` 的 `initializeEventListeners` 方法。
