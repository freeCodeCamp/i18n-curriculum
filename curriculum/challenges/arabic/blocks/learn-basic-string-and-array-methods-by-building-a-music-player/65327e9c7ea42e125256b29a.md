...
أضفنا بقية الأغاني إلى مصفوفة `allSongs` لك.

بعد ذلك، ستتعلم عن <dfn>واجهة برمجة تطبيقات الصوت في الويب</dfn> وكيف تستخدمها لتشغيل الأغاني. جميع المتصفحات الحديثة تدعم واجهة برمجة تطبيقات الصوت في الويب، التي تتيح لك توليد ومعالجة الصوت في تطبيقات الويب.

استخدم `const` لإنشاء متغير باسم `audio` وعيّنه بقيمة `new Audio()`. هذا سينشئ عنصر `audio` جديد من HTML5.

# --hints--

يجب أن تستخدم الكلمة المفتاحية `new` لإنشاء نسخة من كائن `Audio`.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

يجب أن تعيّن كائن `Audio` إلى ثابت باسم `audio`.
