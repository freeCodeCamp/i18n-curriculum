...
Sasa ni wakati wa kutumia kile ulichojifunza kuhusu vitendakazi vya mshale katika mradi wako wa mchezaji wa muziki.

Anza kwa kuondoa vitendakazi vya `printGreeting`, `printMessage`, na `addTwoNumbers`. Pia, ondoa kauli zote za konsoli na miito ya vitendakazi.

# --hints--

Haupaswi kuwa na kitendakazi cha `printGreeting`.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

Haupaswi kuwa na kitendakazi cha `printMessage`.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

Haupaswi kuwa na kitendakazi cha `addTwoNumbers`.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

Haupaswi kuwa na wito wa kitendakazi cha `printGreeting();`.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

Haupaswi kuwa na wito wa kitendakazi cha `printMessage('freeCodeCamp');`.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

Haupaswi kuwa na kauli ya konsoli ya `console.log(addTwoNumbers(3,4));`.
