---
id: 657b7acdc06454fbb0152538
title: Aufgabe32
challengeType: 22
dashedName: task-32
---

<!-- (audio) Sophie: Electric, for sure! Back to your hobbies, though, do you have any favorite photography themes? -->

# --description--

In Gesprächen kannst du bestimmte Wörter verwenden, um auf ein Thema zurückzukommen, das vorher erwähnt wurde.

# --instructions--

Höre dir das Audio an, um den unteren Satz zu vervollständigen.

# --fillInTheBlank--

## --sentence--

`Electric, for sure! BLANK BLANK your hobbies, though, do you have any favorite photography themes?`

## --blanks--

`Back`

### --feedback--

Achte auf die beiden Wörter, die sie benutzt, um das Gespräch wieder auf das zu lenken, worüber Tom vorhin gesprochen hat.

---

`to`

### --feedback--

Achte auf die beiden Wörter, die sie benutzt, um das Gespräch wieder auf das zu lenken, worüber Tom vorhin gesprochen hat.

# --scene--

```json
{
  "setup": {
    "background": "company2-breakroom.png",
    "characters": [
      {
        "character": "Sophie",
        "position": {"x":50,"y":0,"z":1.4},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "1.3-2.mp3",
      "startTime": 1,
      "startTimestamp": 18.34,
      "finishTimestamp": 22.72
    }
  },
  "commands": [
    {
      "character": "Sophie",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sophie",
      "startTime": 1,
      "finishTime": 3.16,
      "dialogue": {
        "text": "Electric for sure. Aber zurück zu deinen Hobbys.",
        "align": "center"
      }
    },
    {
      "Zeichen": "Sophie",
      "startTime": 3.16,
      "finishTime": 5.38,
      "Dialog": {
        "text": "Haben Sie ein Lieblingsmotiv für Fotografie?",
        "align": "center"
      }
    },
    {
      "Zeichen": "Sophie",
      "Opazität": 0,
      "startTime": 5.88
    }
  ]
}
```
