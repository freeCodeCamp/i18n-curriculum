...
До наступної будівлі! Вкладіть два елементи `div` до `.fb2` і надайте їм класи `fb2a` та `fb2b` (за таким же порядком).

# --hints--

Додайте два елементи `div` до `.fb2`.

```js
assert.equal(document.querySelectorAll("div.fb2 > div")?.length, 2);
```

Надайте першому новому `div` клас `fb2a`.
...
Надайте другому новому `div` клас `fb2b`.
