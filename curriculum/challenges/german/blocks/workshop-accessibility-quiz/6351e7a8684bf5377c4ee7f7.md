...
Obwohl es für `label`-Elemente mit einem verschachtelten `input` nicht zwingend erforderlich ist, ist es dennoch Best Practice, ein `label` explizit mit seinem entsprechenden `input`-Element zu verknüpfen.

Fügen Sie nun jedem Ihrer vier `for` ein `label`-Attribut hinzu, das das `label` mit seinem entsprechenden Radio-`input` verbindet.

# --hints--

Sie sollten dem ersten `label` ein `for`-Attribut geben.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

Sie sollten dem ersten `label` ein `for`-Attribut geben, das mit dem `id` seines `input`-Elements übereinstimmt.
...
Sie sollten dem zweiten `label` ein `for`-Attribut geben.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

Sie sollten dem zweiten `label` ein `for`-Attribut geben, das mit dem `id` seines `input`-Elements übereinstimmt.
...
Sie sollten dem dritten `label` ein `for`-Attribut geben.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

Sie sollten dem dritten `label` ein `for`-Attribut geben, das mit dem `id` seines `input`-Elements übereinstimmt.
...
Sie sollten dem vierten `label` ein `for`-Attribut geben.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

Sie sollten dem vierten `label` ein `for`-Attribut geben, das mit dem `id` seines `input`-Elements übereinstimmt.
