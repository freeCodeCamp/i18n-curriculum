...
أضف خاصية `onChange` مضبوطة على دالة سهمية تأخذ `e` كمعلمة وتستدعي `setPlayer` مع نسخ من `player` و`position` مضبوطة على `e.target.value`، محولة إلى `Position`.

# --hints--

يجب أن يحتوي عنصر `select` الخاص بالموقع على معالِج `onChange` يستدعي `setPlayer` مع نسخ من `player` ويُحدّث `position`.
