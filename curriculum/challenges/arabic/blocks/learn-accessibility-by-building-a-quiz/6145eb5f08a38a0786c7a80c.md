...
إذا نقرت على مُدخَلات الراديو، قد تلاحظ إمكانية اختيار كلا المُدخَلين داخل نفس مجموعة الحقول true/false في الوقت نفسه.

اجمع المُدخَلات ذات الصلة معًا بحيث يمكن اختيار مُدخَل واحد فقط من كل زوج في كل مرة.

# --hints--

يجب أن تعطي أول `input` خاصية `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[0]?.name);
```

يجب أن تعطي ثاني `input` خاصية `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[1]?.name);
```

يجب أن تعطي ثالث `input` خاصية `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[2]?.name);
```

يجب أن تعطي رابع `input` خاصية `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[3]?.name);
```

يجب أن تعطي ثاني `input` خاصية `name` تطابق خاصية `name` لأول `input`.
...
يجب أن تعطي رابع `input` خاصية `name` تطابق خاصية `name` لثالث `input`.
...
يجب أن تعطي خصائص `name` مختلفة لأول `input` ولثالث `input`.
