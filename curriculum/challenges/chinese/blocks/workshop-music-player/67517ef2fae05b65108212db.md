...
你需要获取当前播放歌曲的 `id`。为此，你可以使用 `userData.currentSong?.id`。

使用 `getElementById()` 选择当前播放的歌曲：传入一个模板字面，将 `id` 前缀设为 `song-`。赋值给名为 `songToHighlight` 的变量。

# --hints--

你不应修改现有的 `highlightCurrentSong` 函数及其内容。
...
你应该使用 `document.getElementById()` 并传入 ``song-${userData.currentSong?.id}``。

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

你应该将你的 `getElementById()` 赋值给一个 `songToHighlight` 变量。
