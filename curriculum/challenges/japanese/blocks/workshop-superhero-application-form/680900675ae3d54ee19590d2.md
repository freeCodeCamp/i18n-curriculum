...
`button` 要素を、`className` の `submit-btn` と `type` の `submit` で作成してください。ボタンには `Join the League` というテキストを付けてください。

それができたら、フォームを提出して何が起きるか試してみてください。
...
ボタン要素には `className` 属性を `submit-btn` に設定してください。

```js
assert.exists(document.querySelector("button.submit-btn"));
```

`button` 要素には `Join the League` をテキストとして設定してください。
