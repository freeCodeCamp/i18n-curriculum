...
이제 필요한 DOM 요소를 참조하기 시작하세요.

ID가 `bug-emoji`인 단락 요소를 가져와 `bugEmojiElement`이라는 상수에 저장하세요. 요소가 `!`가 아님을 어서션 연산자 `null`를 사용해 확신하세요.

# --hints--

`bugEmojiElement`라는 상수가 있어야 합니다.
...
`bugEmojiElement`는 `bug-emoji` 타입 인수를 사용해 `document.querySelector`로 ID가 `HTMLParagraphElement`인 요소를 쿼리해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

`bugEmojiElement`는 널이 아님을 확신하는 어서션 연산자(`!`)를 사용해야 합니다.
