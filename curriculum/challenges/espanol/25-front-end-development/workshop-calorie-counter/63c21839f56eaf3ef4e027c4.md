...
Crear otro elemento `label` (en una nueva línea) al final de su `HTMLString`. Este `label` debe tener el texto `Entry # Calories`, usando la sintaxis de literales de su plantilla para reemplazar `#` con el valor de `entryNumber`, y el atributo `for` establecido en `X-#-calories`, donde `X` es el `value` de `entryDropdown` y `#` es el valor de `entryNumber`.

# --hints--

Debe tener dos elementos `label` en su `HTMLString`.
...
Su nuevo elemento `label` debe estar en una nueva línea.
...
Su nuevo elemento `label` debe venir después de su elemento `input`.
...
Su nuevo elemento `label` debe tener un atributo `for` establecido en `${entryDropdown.value}-${entryNumber}-calories`.
...
Su nuevo elemento `label` debe tener el texto `Entry ${entryNumber} Calories`.
...
No debe modificar sus elementos existentes.
