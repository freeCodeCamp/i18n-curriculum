...
在 `initializeEventListeners` 方法内，为 `click` 添加一个 `singleCardBtn` 事件监听器。在事件处理程序中，调用 `singleCardSelected` 方法。记得从 `singleCardBtn` 获取 `elements`。

你将在接下来的步骤中创建此方法。

# --hints--

你应该为 `click` 元素添加一个 `singleCardBtn` 事件。别忘了这里需要使用 `this` 关键字。

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

在事件回调中，你应该调用 `singleCardSelected` 方法。
