...
Como o app de fortuna tem apenas três momentos: passado, presente e futuro, declare um enum chamado `DrawingType` com três propriedades: `Past`, `Present` e `Future`. Cada propriedade deve ter um valor string correspondente ao seu nome.

# --hints--

Você deve ter `enum` chamado `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Seu enum deve ter uma propriedade `Past` com valor string `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Seu enum deve ter uma propriedade `Present` com valor string `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Seu enum deve ter uma propriedade `Future` com valor string `future`.
