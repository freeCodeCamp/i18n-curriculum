...
為你的這個建置添加另一個重複漸層；使其與你剛剛添加的相同，但不要添加 `90deg` 方向，並使用你的視窗顏色取代兩個 `transparent` 顏色。

# --hints--

你應該在 `.fb6` 的 `background` 屬性中給予第二個 `repeating-linear-gradient`。
...
你應該給第二個 `repeating-linear-gradient` 一個從 `0%` 到 `10%` 的第一個顏色 `--building-color3`。
...
你應該給第二個 `repeating-linear-gradient` 在 `10%` 到 `30%` 之間添加第二個顏色 `--window-color3`。
