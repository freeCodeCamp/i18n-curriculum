...
참값 표현식에서 `undefined`를 제거하고 `textContent`의 `currentProductCountSpan`을 `currentProductCount` 다음에 `x`가 오도록 업데이트하세요. 템플릿 리터럴을 사용하세요.

# --hints--

참값 표현식에서 `undefined`를 제거해야 합니다.
...
`textContent`의 `currentProductCountSpan` 속성에 접근해야 합니다.
...
템플릿 리터럴 구문을 사용해 `textContent`을 `${currentProductCount}x`로 업데이트하세요.
