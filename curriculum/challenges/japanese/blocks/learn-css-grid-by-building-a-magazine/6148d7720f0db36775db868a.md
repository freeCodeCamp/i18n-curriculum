...
テキスト用の列と画像用の列が必要です。 `.text-with-images` セレクターの `grid-template-columns` プロパティを `1fr 2fr` に設定してください。 また、列の間の余白を増やすため、`column-gap` プロパティを `3rem` に設定してください。

# --hints--

`.text-with-images` セレクターの `grid-template-columns` プロパティを `1fr 2fr` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

`.text-with-images` セレクターの `column-gap` プロパティを `3rem` に設定する必要があります。
