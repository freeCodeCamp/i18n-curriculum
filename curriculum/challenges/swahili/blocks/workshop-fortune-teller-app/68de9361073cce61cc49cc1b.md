...
Katika somo lililopita, ulijifunza kwamba utambuzi wa aina ni uwezo wa TypeScript wa kubaini na kuweka aina kwa moja kwa moja kwa vigezo, marejeo ya kitendakazi, na usemi wa kihesabu kulingana na matumizi na muktadha wake.

Katika TypeScript, generics hufanya vitendakazi, madarasa ya vitu, na aina ziwe na ufanisi zaidi. Generics huruhusu kuunda vigezo vya aina, hivyo kuondoa haja ya kufafanua aina kila mara kwa uwazi. Hii inaruhusu kitendakazi kimoja kufanya kazi kwa usalama na aina tofauti.

Hapa kuna mfano rahisi wa generics:
...
Vitendakazi vinaweza kutumia vigeni viwili au zaidi. `T` hutumika kwa mkataba, na vigezo vya aina vya ziada kama `S` vinaweza kuongezwa inapohitajika.
...
Tengeneza kitendakazi `getElement` kinachopokea `selector` ya aina ya `string` kama kigezo. Pia kinapaswa kuchukua aina ya kigeni `T` inayopanuka kutoka `HTMLElement`, na kuwa na aina ya kurudisha `T`.

# --hints--

Unapaswa kuwa na kitendakazi cha mshale kinachoitwa `getElement`.
...
Kitendakazi chako cha `getElement` kinapaswa kupokea kigezo kinachoitwa `selector` cha aina ya `string`.
...
Kigezo cha aina ya kigeni `T` kinapaswa kupanuka aina ya `HTMLElement`.
...
Kitendakazi chako cha `getElement` kinapaswa kuwa na aina ya kurudisha `T`.
