...
Dentro do loop, verifique se o autor da entrada atual (convertido para minúsculas) inclui `searchTerm`. O método `.includes()` retorna `true` se uma string contém outra string como substring. Se a condição for verdadeira, adicione a entrada em `results`. Após o loop, retorne `results`.

# --hints--

`findByAuthor(catalog, "king").length` deve ser `6`.

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

`findByAuthor(catalog, "asimov").length` deve ser `2`.

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

`findByAuthor(catalog, "xyz").length` deve ser `0`.
