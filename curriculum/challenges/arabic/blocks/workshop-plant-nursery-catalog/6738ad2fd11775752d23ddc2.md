...
تقبل دالة البناء `Set` أيضًا قابلًا للتكرار كمعلمة وتنشئ مجموعة منه:
...
لإكمال هذه الورشة، عدل دالة `displayPlantsSet` لتُرجع مجموعة تحتوي على أسماء النباتات الشائعة الفريدة في الكتالوج الخاص بك.

لتحقيق ذلك، أنشئ مصفوفة تحتوي على `commonName` لكل كائن نبات مخزن كمفتاح في خريطة `catalog`. مرر المصفوفة إلى دالة البناء `Set` وأرجع المجموعة من الدالة.

# --hints--

يجب أن تُرجع دالة `displayPlantsSet` مجموعة تحتوي على `commonName` لكل نبات في الكتالوج الخاص بك.
