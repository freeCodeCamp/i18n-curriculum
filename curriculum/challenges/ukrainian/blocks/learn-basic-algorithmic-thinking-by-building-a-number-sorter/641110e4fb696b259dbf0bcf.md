...
Для цього алгоритму вам потрібно використати цикл `while`. Цей цикл має мати дві умови:

- По-перше, він не повинен виконуватися за межами початку масиву (до якого є доступ через `j`).
- По-друге, цикл не повинен виконуватися після того, як знайде значення, менше за поточне.

Щоб уникнути безкінечного циклу, зменшуйте `j` всередині циклу.

# --hints--

Вам слід використати цикл `while`.
...
Ваш цикл `while` має мати першу умову, яка перевіряє, що значення `j` є більшим або рівним `0`.
...
Ваш цикл `while` має використовувати оператор AND.
...
Ваш цикл `while` має мати умову, яка перевіряє, що значення `array[j]` є більшим за `currValue`.
...
Ваш цикл `while` має зменшувати `j` всередині циклу.
