...
現在你有了匹配乘法或除法的正則表達式，你可以評估該表達式。

聲明一個 `str2` 變量，並將使用 `str` 和 `regex` 作爲參數調用 `infixEval` 的結果分配給它。
...
你應該將 `regex` 作爲第二個參數傳遞給 `infixEval` 調用。
