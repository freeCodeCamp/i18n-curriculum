...
Ifuatayo, pata kipengele cha `#posts-container` kwa kutumia njia ya `getElementById()`. Weka thamani yake katika thabiti mpya iitwayo `postsContainer`.

# --hints--

Unapaswa kutumia `const` kutangaza kigezo cha `postsContainer`.

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#posts-container`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

Unapaswa kuweka thamani ya kipengele cha `#posts-container` katika kigezo chako cha `postsContainer`.
