...
使用類選擇器，給你的 `.cat-right-inner-eye` 元素一個寬度爲 `10px` 和高度爲 `20px` 的樣式。 另外，將其背景顏色設置爲 `#fff`。

# --hints--

你應該有一個 `.cat-right-inner-eye` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye'))
```

你的 `.cat-right-inner-eye` 選擇器應該設置 `width` 爲 `10px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.width === '10px')
```

你的 `.cat-right-inner-eye` 選擇器應該設置 `height` 爲 `20px` 。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.height === '20px')
```

你的 `.cat-right-inner-eye` 選擇器應該設置 `background-color` 爲 `#fff`。
