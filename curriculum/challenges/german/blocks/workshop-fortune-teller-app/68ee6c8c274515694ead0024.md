...
Das Glück muss zufällig sein, daher wird eine Hilfsfunktion benötigt, um ein zufälliges Glück auszuwählen. Deklarieren Sie eine Variable namens `chosenCard` innerhalb der `singleCardSelected`-Methode. Verwenden Sie die `getRandomItem`-Methode, um eine zufällige Karte zu erhalten, und übergeben Sie `cards` als Argument.

# --hints--

Sie sollten eine Variable namens `chosenCard` innerhalb von `singleCardSelected` deklarieren.
...
Ihre `chosenCard`-Variable sollte durch den Aufruf von `getRandomItem(this.cards)` zugewiesen werden.
...
Sie sollten `this.cards` als Argument an `getRandomItem` übergeben.
