...
У вашій новій функції оголосіть змінну `calories` і призначте їй значення `0`. Використовуйте `let` для оголошення, оскільки пізніше ви будете переназначати цю змінну.

# --hints--

Ви повинні оголосити змінну `calories`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

Вашій змінній `calories` має бути призначене значення `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Ви повинні оголосити змінну `calories` за допомогою `let`.
