...
还有一件事。如果你一直点击 `Load More Authors` 按钮，直到没有更多数据可装载且文本变为 `"No more data to load"`，光标的值仍然是 `pointer`。为什么不把光标值改成 `"not-allowed"` 呢？

访问 `style` 按钮的 `Load More Authors` 属性，并将 `cursor` 设置为 `"not-allowed"`。
...
你应该用点符号访问 `style` 的 `loadMoreBtn` 属性。

```js
assert.match(code, /loadMoreBtn\.style/)
```

你应该用点符号从 `cursor` 属性访问 `style`。
