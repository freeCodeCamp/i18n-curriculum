...
تحتاج إلى التكرار خلال الأزرار في المتغير `addToCartBtns`. مع ذلك، تُرجع الدالة `.getElementsByClassName()` مجموعة لا تحتوي على طريقة `forEach`.

استخدم معامل النشر على المتغير `addToCartBtns` لتحويله إلى مصفوفة. ثم استخدم طريقة `forEach` للتكرار خلال المصفوفة. لا تمرر دالة استدعاء راجع بعد.
...
يجب أن تستخدم طريقة `forEach` على المصفوفة التي أنشأتها.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(/);
```

يجب ألا تمرر دالة استدعاء راجع إلى طريقة `forEach`.
