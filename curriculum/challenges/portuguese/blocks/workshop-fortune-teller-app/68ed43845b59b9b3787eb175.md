...
Adicione um `constructor` dentro da classe. No construtor, você precisa usar os elementos que criou anteriormente para selecionar os elementos DOM correspondentes usando a função `getElement`.

Por enquanto, trate `singleCardBtn` e `singleCard`. Para `singleCardBtn`, passe `#btn-single-card`, e para `singleCard`, passe o `.single_card`.

# --hints--

A classe `Game` deve ter um `constructor`.
...
Seu construtor deve atribuir `this.elements`.
...
`singleCardBtn` deve ser definido usando `getElement` com `'#btn-single-card'`.
...
`singleCard` deve ser definido usando `getElement` com `'.single_card'`.
