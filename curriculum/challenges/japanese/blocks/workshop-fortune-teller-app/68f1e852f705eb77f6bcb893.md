...
`cardId` という名前の変数を宣言し、`data-id` メソッドを使って `cardElement` から `getAttribute()` 属性の値を代入してください。

# --hints--

`cardId` の中で `showFortune` という名前の変数を宣言してください。
...
`cardId` 変数には `cardElement.getAttribute("data-id")` を呼び出して代入してください。
...
`getAttribute` に対して `cardElement` メソッドを使う必要があります。

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

`'data-id'` に `getAttribute` を渡してください。
