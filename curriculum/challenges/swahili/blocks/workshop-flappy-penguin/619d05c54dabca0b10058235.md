...
Hadi sasa, vipengele vya `.face` na `.chin` vina `background-color` sawa.

Tengeneza sifa ya CSS maalum iitwayo `--penguin-face`, na uiweke kuwa `white`.
...
Unapaswa kumpa `:root` sifa ya `--penguin-face`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--penguin-face'));
```

Unapaswa kumpa `:root` `--penguin-face` ya `--fcc-expected--`, lakini umeipata `--fcc-actual--`.
