...
當按下 shuffle 按鈕時，你想將 `currentSong` 設為空，並將 `songCurrentTime` 設為 `0`。

將 `userData.currentSong` 設為 `null`，並將 `userData.songCurrentTime` 設為 `0`。

*注意*：你不應該在此步驟使用選用串鏈，因為你是明確地將 `currentSong` 和 `songCurrentTime` 屬性分別設定為 `null` 和 `0`。

# --hints--

你應該將 `userData.currentSong` 設定為 `null`。

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

你應該將 `userData.songCurrentTime` 設定為 `0`。
