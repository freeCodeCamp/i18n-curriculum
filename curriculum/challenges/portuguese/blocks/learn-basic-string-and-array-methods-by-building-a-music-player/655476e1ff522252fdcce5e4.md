...
Adicione um bloco `else` à declaração `if`.Dentro do bloco `else`, chame a função `getCurrentSongIndex()` e atribua-a a uma constante chamada `currentSongIndex`.

# --hints--

Você não deve modificar a declaração `if` existente.
...
Você deve adicionar um `else` à declaração `if` existente.
...
Você deve definir a constante `currentSongIndex` para `getCurrentSongIndex()` dentro do bloco `else`.
