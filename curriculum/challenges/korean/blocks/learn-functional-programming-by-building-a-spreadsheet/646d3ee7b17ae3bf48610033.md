...
`infixToFunction[operator]`는 함수를 반환합니다. 그 함수를 직접 호출하면서 `arg1`과 `arg2`를 인수로 전달하세요.

# --hints--

콜백 함수는 `infixToFunction[operator]` 호출 결과를 반환해야 합니다.
...
`arg1`를 `infixToFunction[operator]` 호출의 첫 번째 인수로 전달해야 합니다.
...
`arg2`를 `infixToFunction[operator]` 호출의 두 번째 인수로 전달해야 합니다.
