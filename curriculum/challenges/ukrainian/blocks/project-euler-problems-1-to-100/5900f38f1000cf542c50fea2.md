...
Число 197 називають круговим простим числом, тому що всі оберти його цифр: 197, 971 і 719, самі по собі є простими числами.

Існує тринадцять таких простих чисел менше за 100: 2, 3, 5, 7, 11, 13, 17, 31, 37, 71, 73, 79 і 97.

Скільки кругових простих чисел існує менше за `n`, якщо 100 ≤ `n` ≤ 1000000?

**Примітка:**  

Окремий оберт кругового простого числа може перевищувати `n`.

# --hints--

`circularPrimes(100)` має повертати число.

```js
assert(typeof circularPrimes(100) === 'number');
```

`circularPrimes(100)` має повертати 13.

```js
assert(circularPrimes(100) == 13);
```

`circularPrimes(100000)` має повертати 43.

```js
assert(circularPrimes(100000) == 43);
```

`circularPrimes(250000)` має повертати 45.

```js
assert(circularPrimes(250000) == 45);
```

`circularPrimes(500000)` має повертати 49.

```js
assert(circularPrimes(500000) == 49);
```

`circularPrimes(750000)` має повертати 49.

```js
assert(circularPrimes(750000) == 49);
```

`circularPrimes(1000000)` має повертати 55.
