...
要排序陣列的元素，你可以使用內建的 `.sort()` 方法。因此，你可以透過將 `inputValues` 陣列呼叫 `.sort()` 的結果指定給 `sortedValues` 變數來更新它。

# --hints--

你應該將你的 `sortedValues` 變數更新為對你的 `inputValues` 陣列呼叫 `.sort()` 方法的結果。
