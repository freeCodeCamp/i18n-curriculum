...
Per ottenere il risultato di "bubble up", devi verificare se l'elemento corrente è più grande del prossimo elemento. Puoi farlo consultando il `array` a `j` e `j+1`.

Crea una condizione `if` che controlli se l'elemento corrente è più grande del prossimo elemento.

# --hints--

Non dovresti rimuovere la tua istruzione `console.log`.
...
Dovresti usare un'istruzione `if` nel tuo loop annidato.
...
La tua istruzione `if` dovrebbe controllare se il `array[j]` è più grande del `array[j+1]`.
