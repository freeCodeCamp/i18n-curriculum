...
同じ構文を使用して、`#lunch` 要素内の `number` タイプの入力欄を取得して `lunchNumberInputs` に代入してください。

# --hints--

変数 `lunchNumberInputs` を宣言してください。

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=/);
```

`document.querySelectorAll()` に `#lunch input[type='number']` を渡して呼び出してください。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` 呼び出しの結果を `lunchNumberInputs` に代入してください。
