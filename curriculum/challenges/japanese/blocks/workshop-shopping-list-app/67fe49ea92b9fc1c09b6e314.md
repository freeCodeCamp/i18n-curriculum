...
次に、`includes()` メソッドを使って、`item` の小文字バージョンが `query` の小文字バージョンを部分文字列として含んでいるかどうかを確認してください。

例えば、文字列 `toLowerCase()` が `includes()` を含んでいるかどうかを確認するために、`freeCodeCamp` と `code` メソッドを使う方法は次の通りです。
...
小文字に変換した `includes()` に `item` メソッドを連結し、小文字に変換した `query` を含んでいるかどうかを確認してください。`query` も小文字に変換することを忘れないでください。

# --hints--

小文字に変換した `includes()` に `item` メソッドを連結する必要があります。
...
小文字に変換した `item` 文字列が、小文字に変換した `query` を部分文字列として含んでいるかどうかを確認してください。
