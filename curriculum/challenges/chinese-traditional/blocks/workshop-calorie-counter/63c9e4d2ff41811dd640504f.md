...
是時候開始準備你的計算了。首先宣告一個 `consumedCalories` 變數，並將它指定為 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的總和（請注意測試時順序很重要）。務必在你的 `if` 述語之後執行此操作。

# --hints--

你的函式（程式）應該有一個 `consumedCalories` 變數。

```js
assert.match(calculateCalories.toString(), /consumedCalories\s*=/);
```

你的 `consumedCalories` 變數應該放在你的 `if` 述語之後。

```js
assert.isAbove(calculateCalories.toString().indexOf('consumedCalories'), calculateCalories.toString().indexOf('if'));
```

你應該依序計算 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的總和。

```js
assert.match(calculateCalories.toString(), /breakfastCalories\s*\+\s*lunchCalories\s*\+\s*dinnerCalories\s*\+\s*snacksCalories\s*;?\s*$/m);
```

你應該將 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的總和指定給 `consumedCalories`。
