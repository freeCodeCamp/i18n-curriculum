...
새로운 `.indent` 선택자를 만들고 `margin-left` 속성을 `1em`로 설정하세요.

# --hints--

새로운 `.indent` 선택자가 있어야 합니다.

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('.indent'));
```

`.indent` 선택자는 `margin-left` 속성을 `1em`로 설정해야 합니다.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.indent')?.marginLeft, '1em');
```

`.indent` 선택자는 다른 여백 속성이 아니라 `margin-left` 속성만 설정해야 합니다.
