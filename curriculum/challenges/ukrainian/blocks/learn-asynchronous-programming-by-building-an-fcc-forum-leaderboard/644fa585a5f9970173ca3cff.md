...
Створіть константу з назвою `linkClass` і призначте їй шаблонний рядок. Всередині цього шаблонного рядка додайте значення `category ${selectedCategory.className}`.

Ці імена класів будуть використані для застосування стилів до елемента посилання.

# --hints--

У вас має бути константа з назвою `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

Ви повинні призначити шаблонний рядок змінній `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

Усередині шаблонного рядка має бути значення `category ${selectedCategory.className}`.
