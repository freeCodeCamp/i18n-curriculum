...
يجب عليك إنشاء عناصر `img` ثلاثة جديدة داخل عنصر `aside` الخاص بك.

```js
assert(document.querySelectorAll('aside img')?.length === 3);
```

يجب عليك إنشاء عنصر `blockquote` داخل عنصر `aside` الخاص بك.

```js
assert.exists(document.querySelector('aside blockquote'));
```

يجب أن يحتوي عنصر `blockquote` الخاص بك على `class` تم تعيينه إلى `image-quote`.

```js
assert(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

يجب أن تكون عناصرك الجديدة بالترتيب الصحيح.
