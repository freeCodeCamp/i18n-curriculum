...
يجب أن يكون نص الوصف أكبر قليلًا. اختر العنصر الذي يحمل الصنف `description` واضبط خاصية `font-size` إلى `1.2rem`.

# --hints--

يجب أن تستهدف العنصر الذي يحمل الصنف `description`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.description'));
```

اضبط خاصية `font-size` إلى `1.2rem`.
