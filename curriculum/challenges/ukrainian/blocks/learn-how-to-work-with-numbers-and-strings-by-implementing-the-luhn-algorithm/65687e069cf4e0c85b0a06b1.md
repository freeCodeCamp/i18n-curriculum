...
Поза межами інструкції `if` додайте `number` до `sum_of_even_digits`. Також видаліть виклик `print`.

# --hints--

У вас має бути `sum_of_even_digits += number` всередині циклу `for`.
...
У вас не має бути `print(number)` всередині циклу `for`.
...
У вас не має бути `sum_of_even_digits += number` всередині інструкції `if`.
