...
You should only add one opening `fieldset` tag. 请删除任何多余的。
...
You should only add one closing `fieldset` tag. 请删除任何多余的。
...
Your new `fieldset` element should be below the existing `fieldset` element. 你把它们的顺序写错了。
