...
In questo momento la sezione `footer` ha un'intestazione `h3`. Ma dovrebbe essere un'intestazione `h2` per mantenere una gerarchia corretta delle intestazioni.

Cambia il `h3` in un `h2`.

# --hints--

Il tuo elemento `footer` dovrebbe contenere un elemento `h2` invece di un elemento `h3`.
