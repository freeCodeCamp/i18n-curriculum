...
Tengeneza kichaguzi cha `.quote`. Mpe sifa ya `color` iliyowekwa kuwa `#00beef`, sifa ya `font-size` iliyowekwa kuwa `2.4rem`, na sifa ya `text-align` iliyowekwa kuwa `center`.
...
Kichaguzi chako cha `.quote` kinapaswa kuwa na sifa ya `color` iliyowekwa kuwa `#00beef`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.color, 'rgb(0, 190, 239)');
```

Kichaguzi chako cha `.quote` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `2.4rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize, '2.4rem');
```

Kichaguzi chako cha `.quote` kinapaswa kuwa na sifa ya `text-align` iliyowekwa kuwa `center`.
