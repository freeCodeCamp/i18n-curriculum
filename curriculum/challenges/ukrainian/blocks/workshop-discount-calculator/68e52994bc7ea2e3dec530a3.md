...
Усередині методу `calculate_best_price` створіть список з назвою `prices`, який містить `product.price`. Це гарантує, що якщо знижки не застосовуються, клієнт сплатить повну ціну.

# --hints--

Усередині `calculate_best_price` потрібно створити список з назвою `prices`, який містить `product.price`.
