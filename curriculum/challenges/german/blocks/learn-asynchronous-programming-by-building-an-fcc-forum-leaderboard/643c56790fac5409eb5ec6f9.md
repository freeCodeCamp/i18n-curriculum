...
Gehen Sie zurück zu Ihrer `showLatestPosts()`-Funktion und verwenden Sie Destrukturierung, um die `topic_list`- und `users`-Eigenschaften aus dem `data`-Objekt zu erhalten.

# --hints--

Sie sollten `const` verwenden, um `topic_list` und `users` aus dem `data`-Objekt zu destrukturieren.
