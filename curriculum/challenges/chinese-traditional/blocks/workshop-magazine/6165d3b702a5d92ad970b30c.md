---
id: 6165d3b702a5d92ad970b30c
title: 步驟4
challengeType: 0
dashedName: step-4
---

# --description--

在 `img` 元素之後，添加一個 `h1` 元素，其中 `class` 設置爲 `hero-title`，文本設置爲 `OUR NEW CURRICULUM`，然後添加一個 `p` 元素，其中 `class` 設置爲 `hero-subtitle`，文本設置爲 `Our efforts to restructure our curriculum with a more project-based focus`。

# --hints--

你應該創建一個 `h1` 元素。

```js
assert.exists(document.querySelector('h1'));
```

你的 `h1` 元素應該在你的 `img` 元素之後。

```js
assert.equal(document.querySelector('h1')?.previousElementSibling?.localName, 'img');
```

你的 `h1` 元素應將 `class` 設置爲 `hero-title`。

```js
assert.equal(document.querySelector('h1')?.className, 'hero-title');
```

你的 `h1` 元素的文本應該爲 `OUR NEW CURRICULUM`。

```js
assert.equal(document.querySelector('h1')?.textContent, 'OUR NEW CURRICULUM');
```

你應該創建一個新的 `p` 元素。

```js
assert.exists(document.querySelector('p'));
```

你的 `p` 元素應該在 `h1` 元素之後。

```js
assert.equal(document.querySelector('p')?.previousElementSibling?.localName, 'h1');
```

你的 `p` 元素應將 `class` 設置爲 `hero-subtitle`。

```js
assert.equal(document.querySelector('p')?.className, 'hero-subtitle');
```

你的 `p` 元素的文本應該爲 `Our efforts to restructure our curriculum with a more project-based focus`。

```js
assert.equal(document.querySelector('p')?.textContent?.trim()?.replace(/\s{2,}/, ' '), 'Our efforts to restructure our curriculum with a more project-based focus');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Magazine</title>
    <link
      href="https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap"
      rel="stylesheet"
    />
    <link
      rel="stylesheet"
      href="https://use.fontawesome.com/releases/v5.8.2/css/all.css"
    />
    <link rel="stylesheet" href="styles.css" />
  </head>
  <body>
--fcc-editable-region--
    <main>
      <section class="heading">
        <header class="hero">
          <img
            src="https://cdn.freecodecamp.org/platform/universal/fcc_meta_1920X1080-indigo.png"
            alt="freecodecamp logo"
            loading="lazy"
            class="hero-img"
          />
        </header>
      </section>
    </main>
--fcc-editable-region--
  </body>
</html>
```

```css

```
