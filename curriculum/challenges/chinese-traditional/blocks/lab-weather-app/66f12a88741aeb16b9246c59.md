...
在這個實驗中，你將建置一個提供不同城市天氣資訊的應用程式。

你將使用天氣 API。輸出資料具有以下格式：
...
1. 你應該有一個 `button` 元素，其 `id` 為 `get-weather-btn`。

1. 你應該有一個 `select` 元素，裡面巢狀七個 `option` 元素。第一個選項的文字和 `value` 屬性應該是空字串。其餘的選項文字和數值應如下（數值為小寫）：
...
1. 如果沒有選擇城市，按下按鈕應該不會有任何動作。

1. 如果選擇了城市，顯示天氣的元素應該出現：

- 你應該有一個 id 為 `weather-icon` 的 `img` 元素來顯示天氣圖示。

- 你應該有一個 id 為 `main-temperature` 的元素來顯示主要溫度。

- 你應該有一個 id 為 `feels-like` 的元素來顯示體感溫度。

- 你應該有一個 id 為 `humidity` 的元素來顯示空氣中的濕度量。

- 你應該有一個 id 為 `wind` 的元素來顯示風速。

- 你應該有一個 id 為 `wind-gust` 的元素來顯示陣風。

- 你應該有一個 id 為 `weather-main` 的元素來顯示主要的天氣型別。

- 你應該有一個 id 為 `location` 的元素來顯示當前位置。

1. 你應該有一個名為 `getWeather` 的非同步函式（程式），它會從 `https://weather-proxy.freecodecamp.rocks/api/city/<CITY>` API 取得天氣資訊並傳回。請注意，此 API 使用公制系統回傳資料，也就是風速以 m/s 為單位，溫度以攝氏度為單位。

1. `getWeather` 非同步函式（程式）應該接受一個城市作為它的引數。

1. 你應該控制在 `getWeather` 函式（程式）中發生的任何錯誤，並將它們記錄到主控台。

1. 你應該有一個非同步的 `showWeather` 函式（程式），它接受一個城市作為參數。

1. `showWeather` 函式（程式）應該呼叫 `getWeather` 函式（程式），以從下拉選單中取得所選城市的天氣資料。

1. 如果 `getWeather` 函式（程式）有錯誤，應用程式只應顯示一個警示，內容為 `Something went wrong, please try again later`。

1. 如果來自 `getWeather` 的資料可用，`showWeather` 函式應該在對應的元素中顯示天氣資料。如果 API 的某個值是 `undefined`，你應該在對應的元素中寫入 `N/A`。

1. `getWeather` 函式和 `showWeather` 函式都不應該使用 `const` 宣告，因為測試需要重新指定它們。

**注意：** `getWeather` 函式和 `showWeather` 函式都不應該使用 `const` 宣告，因為測試需要重新指定它們。此外，測試會花一些時間完成，只要你在主控台看到 `// running tests`，表示測試正在執行中。
...
你應該有一個 `button` 元素，其 `id` 為 `get-weather-btn`。
...
在 `select` 元素內，第一個子元素應該是具有空 `value` 屬性的 `option` 元素。
...
在 `select` 元素內應該有 6 個 `option` 元素，每個元素對應以下城市之一：Paris、London、Tokyo、Los Angeles、Chicago、New York。
...
你應該有一個 `showWeather` 函式（程式）。

```js
assert.isFunction(showWeather);
```

你應該有一個 `getWeather` 函式（程式）。

```js
assert.isFunction(getWeather);
```

`getWeather` 函式（程式）應該接受一個城市作為唯一的引數，並傳回來自 Weather API 的 JSON。
...
`getWeather` 函式和 `showWeather` 函式都不應該使用 `const` 宣告。
...
`showWeather` 函式（程式）應該呼叫 `getWeather` 函式（程式）以取得天氣資料。
...
`getWeather` 傳回 `undefined` 時，`showWeather` 函式應該處理該情況。
...
當選擇 New York 時，`showWeather` 函式（程式）應該在相應的 HTML 元素中顯示來自 API 的資料。如果 API 的值是 `undefined`，你應該顯示 `N/A`。
...
當選擇 Chicago 時，`showWeather` 函式（程式）應該在相應的 HTML 元素中顯示來自 API 的資料。如果 API 的值是 `undefined`，你應該顯示 `N/A`。
...
當選擇 London 時，`showWeather` 函式（程式）應該在相應的 HTML 元素中顯示來自 API 的資料。如果 API 的值是 `undefined`，你應該顯示 `N/A`。
...
當選擇 Tokyo 時，`showWeather` 函式（程式）應該在相應的 HTML 元素中顯示來自 API 的資料。如果 API 的值是 `undefined`，你應該顯示 `N/A`。
...
當選擇 Los Angeles 時，`showWeather` 函式（程式）應該在相應的 HTML 元素中顯示來自 API 的資料。如果 API 的值是 `undefined`，你應該顯示 `N/A`。
...
如果有錯誤，你的 `getWeather` 函式（程式）應該將錯誤記錄到主控台。
...
當選擇 Paris 時，應用程式應顯示帶有 `Something went wrong, please try again later` 的警示。
