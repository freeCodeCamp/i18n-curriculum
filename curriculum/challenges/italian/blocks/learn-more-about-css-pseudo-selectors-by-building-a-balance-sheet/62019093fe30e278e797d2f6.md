...
Il selettore `tr[class="total"]` dovrebbe avere una proprietà `border-bottom` con il valore `4px double #0a0a23`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"]')?.getPropertyValue('border-bottom') === '4px double rgb(10, 10, 35)');
```

Il selettore `tr[class="total"]` dovrebbe avere una proprietà `font-weight` con il valore `bold`.
