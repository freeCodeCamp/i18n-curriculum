...
Ваш клас `ShoppingCart` повинен мати можливість додавати елементи. Створіть порожній метод `addItem`, який приймає два параметри: `id` і `products`. Створення методу може виглядати так:
...
Перший параметр, `id`, — це `id` продукту, який користувач додав до свого кошика. Другий параметр, `products`, — це масив екземплярів `Dessert`. Використовуючи параметр замість прямого посилання на існуючий масив `products`, цей метод буде гнучкішим, якщо ви захочете додати додаткові списки продуктів у майбутньому.

# --hints--

Ваш клас `ShoppingCart` повинен мати метод `addItem`.
...
Ваш метод `addItem` має приймати два параметри: `id` і `products`.
...
Ваш метод `addItem` має бути порожнім.
