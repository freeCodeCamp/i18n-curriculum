...
これから position の入力を select に変更する作業を始めます。

position の入力を、id が `select`、`position` が `className`、`input` が `value` の `player.position` 要素に変更してください。

# --hints--

position の `input` を、`select` の `id` と `position` の `className` を持つ `input` 要素に変更してください。
...
position の `select` は `value` の `player.position` を持つべきです。
