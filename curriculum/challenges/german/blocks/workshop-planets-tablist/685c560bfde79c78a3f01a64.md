...
Da `associatedPanel` ein `id` ist, können Sie damit das Dokument abfragen und das entsprechende Panel-Element finden.

Erstellen Sie eine neue Variable namens `panel` und weisen Sie ihr den Wert von `document.getElementById(associatedPanel)` zu.

# --hints--

Sie sollten eine Variable namens `panel` haben, die das Panel-Element enthält, das mit dem angeklickten Tab verknüpft ist.
