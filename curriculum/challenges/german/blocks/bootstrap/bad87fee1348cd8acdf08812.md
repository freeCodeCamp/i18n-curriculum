...
title: Erstellen Sie einen Bootstrap-Button
challengeType: 0
forumTopicId: 16811
...
Bootstrap hat eigene Styles für `button`-Elemente, die viel besser aussehen als die einfachen HTML-Elemente.

Erstellen Sie unter Ihrem großen Katzenbild ein neues `button`-Element. Geben Sie ihm die Klassen `btn` und `btn-default` sowie den Text `Like`.

# --hints--

Sie sollten ein neues `button`-Element mit dem Text `Like` erstellen.
...
Ihr neuer Button sollte zwei Klassen haben: `btn` und `btn-default`.
