...
將 `.chin` 元素定位，使其距離父元素頂部 `25%`，距離左側 `5%`。然後，將頂部角的半徑設為 `70%`，底部角的半徑設為 `100%`。
