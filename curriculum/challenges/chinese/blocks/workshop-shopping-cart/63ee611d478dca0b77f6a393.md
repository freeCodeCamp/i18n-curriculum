...
你在之前的课程中学到，`this` 关键字指的是当前正在构造的实例，正如你在 `Dessert` 构造函数中看到的那样。在这里，你将把同样的思路应用到 `ShoppingCart` 类中。
...
在你的构造函数中，使用 `this` 关键字将 `items` 属性设置为空数组。同时，将 `total` 属性设置为 `0`，将 `taxRate` 属性设置为 `8.25`。

# --hints--

你应该使用 `this` 关键字将你的类的 `items` 属性设置为空数组。
...
你应该使用 `this` 关键字将你的类的 `total` 属性设置为 `0`。
...
你应该使用 `this` 关键字将你的类的 `taxRate` 属性设置为 `8.25`。
