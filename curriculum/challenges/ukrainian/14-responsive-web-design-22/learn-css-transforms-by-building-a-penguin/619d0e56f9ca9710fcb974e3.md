...
Ви повинні надати `.eye-lid` властивість `top` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.top, '25%');
```

Ви повинні надати `.eye-lid` властивість `left` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.left, '-23%');
```

Ви повинні надати `.eye-lid` властивість `border-radius` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.
