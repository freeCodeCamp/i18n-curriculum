...
`salary` セッターが作成されたので、`__init__` を更新して、`self._salary` に直接代入するのではなくそれを呼び出すようにしてください。`self._salary = Employee._base_salaries[level]` を `self.salary = Employee._base_salaries[level]` に変更してください。

# --hints--

`__init__` メソッドは `self.salary = Employee._base_salaries[level]` の代わりに `self._salary = Employee._base_salaries[level]` を使うべきです。
