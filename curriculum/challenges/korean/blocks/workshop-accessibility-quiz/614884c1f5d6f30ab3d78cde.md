...
`.formrow` 요소에 위쪽 여백과 좌우 패딩을 주세요. 나머지 패딩 값은 `0`이어야 합니다.

그다음, 모든 `input` 요소의 폰트 크기를 키우세요.

# --hints--

`.formrow` 요소를 선택하려면 `.formrow` 선택자를 사용하세요.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.formrow'));
```

`.formrow`에 최소 `margin-top`의 `1px`을 주세요.
...
`input` 요소를 선택하려면 `input` 선택자를 사용하세요.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input'));
```

`input`에 `font-size`보다 큰 `13px`을 주세요.
