...
Le riviste spesso usano il testo giustificato nei loro contenuti stampati per strutturare il layout e controllare il flusso del contenuto. Anche se questo funziona nella forma stampata, il testo giustificato sui siti web può rappresentare un problema di accessibilità, ad esempio creando difficoltà per chi ha dislessia.

Per far sembrare il tuo progetto una rivista stampata, dai al selettore `.text` una proprietà `text-align` impostata su `justify`.

# --hints--

Il tuo selettore `.text` dovrebbe avere una proprietà `text-align` impostata su `justify`.
