...
마지막으로, 사용자 아바타 이미지를 반환해야 합니다.

`return`를 추가한 다음 템플릿 리터럴을 작성하세요. 템플릿 리터럴 안에 `img` 요소를 넣으세요.

`img` 태그 안에는 `src` 속성을 `${userAvatarUrl}` 값으로 추가하세요. `alt` 속성에는 `${user.name}` 값을 넣으세요.
...
`src` 속성의 값을 `userAvatarUrl`라는 상수로 설정해야 합니다.
...
`alt` 속성의 값을 `user.name`라는 상수로 설정해야 합니다.
