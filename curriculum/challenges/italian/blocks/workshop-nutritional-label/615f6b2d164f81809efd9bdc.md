...
Dopo il tuo ultimo elemento `.divider`, crea un elemento `p` e dagli il testo `Total Fat 8g 10%`. Poi, avvolgi il testo `Total Fat` in un elemento `span`, il testo `10%` in un altro, e assegna a entrambi la classe `bold`.
