...
Your emphasis `em` element should have an opening tag. Le etichette di apertura hanno questa sintassi: `<nomeElemento>`.

```js
assert(document.querySelector('em'));
```

Your emphasis `em` element should have a closing tag. I tag di chiusura hanno un carattere`/` subito dopo il carattere `<`.
...
Your emphasis `em` element should surround the text `love`. Hai omesso il testo o hai un refuso.
