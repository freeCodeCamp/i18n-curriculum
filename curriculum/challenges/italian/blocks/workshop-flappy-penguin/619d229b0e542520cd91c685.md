...
Per far sembrare i piedi del pinguino più da pinguino, ruota il piede sinistro di `80deg` e quello destro di `-80deg`.

# --hints--

Dovresti dare a `.foot.left` una `transform` di `rotate(80deg)`.
...
Dovresti dare a `.foot.right` una `transform` di `rotate(-80deg)`.
