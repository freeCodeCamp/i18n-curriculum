...
在 `.penguin-body` 中，在 `.foot` 元素前面添加兩個 `div` 元素，每個的 `class` 都是 `arm`。 爲第一個 `.arm` 添加一個 `left` `class`，爲第二個 `.arm` 添加一個 `right` `class`。
