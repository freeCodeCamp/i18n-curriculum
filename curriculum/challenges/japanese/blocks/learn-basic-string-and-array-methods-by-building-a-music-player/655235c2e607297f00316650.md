...
次に、`classList` プロパティと `add()` メソッドを使って、`playButton` 要素に `"playing"` クラスを追加してください。これは CSS ファイル内の `"playing"` クラスを探して、`playButton` 要素に追加します。

最後に曲を再生するには、`audio` 変数の `play()` メソッドを使ってください。`play()` は mp3 ファイルを再生するためのウェブオーディオ API のメソッドです。
