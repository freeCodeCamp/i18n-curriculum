...
正しい要素を指定できたので、先ほど指定した遅延の後にそのテキストを更新できます。

対象の要素の`textContent`プロパティを、`msg`メソッドのチェーンで現在のオブジェクトの`.getElementById()`プロパティに設定してください。

# --hints--

`.textContent`メソッドに`.getElementById()`をチェーンする必要があります。
...
対象の要素の`textContent`プロパティを、現在のオブジェクトの`msg`プロパティに設定してください。
