...
フォーカスされたときに`.card-links a`要素を対象とする新しいセレクターを追加してください。`outline`プロパティを`2px solid yellow`に設定してください。

# --hints--

`.card-links a:focus`セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:focus"));
```

`.card-links a:focus`セレクターには`outline`プロパティが`2px solid yellow`に設定されている必要があります。
