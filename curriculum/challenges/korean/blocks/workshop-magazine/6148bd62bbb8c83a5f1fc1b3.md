...
`.hero-title` 선택자를 만들고 `text-align` 속성을 `center`로, `color` 속성을 `orangered`로, `font-size` 속성을 `8rem`로 설정하세요.

# --hints--

`.hero-title` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

`.hero-title` 선택자에 `text-align` 속성을 `center`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign, 'center');
```

`.hero-title` 선택자에 `color` 속성을 `orangered`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color, 'orangered');
```

`.hero-title` 선택자에 `font-size` 속성을 `8rem`로 설정하세요.
