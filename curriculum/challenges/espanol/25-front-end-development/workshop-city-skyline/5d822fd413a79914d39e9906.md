...
El siguiente edificio tendrá tres secciones. Anide tres elementos `div` dentro de `.bb4`. Asígneles las clases de `bb4a`, `bb4b` y `bb4c` en ese orden.
...
Debe darle al primer nuevo `div` una clase de `bb4a`.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

Debe darle al segundo nuevo `div` una clase de `bb4b`.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

Debe darle al tercer nuevo `div` una clase de `bb4c`.
