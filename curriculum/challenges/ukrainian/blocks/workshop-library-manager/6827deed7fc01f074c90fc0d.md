...
Було б добре протестувати вашу функцію `getBooksByAuthor` з іншим автором.

Почніть із виведення повідомлення `"\nList of books by James Clear:\n"` у консоль.

Нижче цього `console.log()` додайте ще один `console.log()`. Усередині цього `console.log()` викличте функцію `getBooksByAuthor()` з аргументами `library` та `"James Clear"`.

Тепер у консолі ви повинні побачити всі книги цього конкретного автора.

# --hints--

Ви повинні вивести повідомлення `"\nList of books by James Clear:\n"` у консоль.

```js
assert.match(code, /console\.log\(("|')\\nList\s+of\s+books\s+by\s+James\s+Clear:\\n\1\);?/);
```

У вас має бути `console.log(getBooksByAuthor(library, "James Clear"))`.
