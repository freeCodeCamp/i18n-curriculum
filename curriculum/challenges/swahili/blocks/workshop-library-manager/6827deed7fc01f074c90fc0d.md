...
Itakuwa vizuri kujaribu kitendakazi chako cha `getBooksByAuthor` na mwandishi mwingine.

Anza kwa kuandika ujumbe `"\nList of books by James Clear:\n"` kwenye konsoli.

Chini ya `console.log()`, ongeza `console.log()` nyingine. Ndani ya `console.log()` hiyo, fanya wito wa kitendakazi cha `getBooksByAuthor()` kwa kutumia `library` na `"James Clear"` kama hoja.

Sasa, unapaswa kuona vitabu vyote vya mwandishi huyo maalum kwenye konsoli.

# --hints--

Unapaswa kuandika ujumbe `"\nList of books by James Clear:\n"` kwenye konsoli.

```js
assert.match(code, /console\.log\(("|')\\nList\s+of\s+books\s+by\s+James\s+Clear:\\n\1\);?/);
```

Unapaswa kuwa na `console.log(getBooksByAuthor(library, "James Clear"))`.
