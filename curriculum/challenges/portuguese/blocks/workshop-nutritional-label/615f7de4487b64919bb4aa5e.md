...
Após o último `.divider`, crie um elemento `p` com o texto `Cholesterol 0mg 0%`.Então, envolva o texto `Cholesterol` em um elemento `span`, `0%` em outro, e dê a cada um deles uma classe `bold`.

Por fim, aninhe o elemento `span` contendo o texto `Cholesterol`, juntamente com o texto `0mg`, em um elemento `span` adicional para o alinhamento.
...
O novo elemento `p` deve ter exatamente três elementos `span`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelectorAll('span')?.length === 3);
```

O texto `Cholesterol` deve estar aninhado em um `span`.
...
O elemento `span` que contém o texto `Cholesterol` deve ter seu atributo `class` definido como `bold`.
...
O texto `0%` deve estar aninhado em um `span`.
...
O elemento `span` que contém o texto `0%` deve ter seu atributo `class` definido como `bold`.
...
O elemento `span` que contém o texto `Cholesterol` e o texto `0mg` deve ser envolvido junto em outro elemento `span`.
