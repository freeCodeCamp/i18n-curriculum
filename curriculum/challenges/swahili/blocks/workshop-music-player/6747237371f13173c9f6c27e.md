...
Pata upatikanaji wa vipengele vya `#next` na `#previous` kutoka kwenye faili la HTML.

Weka thamani zao katika vigezo vinavyoitwa `nextButton` na `previousButton`, kwa mpangilio huo.
...
Unapaswa kuweka thamani ya kipengele cha `#next` katika kigezo `nextButton`. 
...
Unapaswa kuweka thamani ya kipengele cha `#previous` katika kigezo `previousButton`. 
