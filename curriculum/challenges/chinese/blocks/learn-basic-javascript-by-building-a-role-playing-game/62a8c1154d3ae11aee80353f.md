...
按照与按钮文本相同的模式，将三个按钮的 `onclick` 赋值更新为 `"button functions"` 数组的第一个、第二个和第三个值。

# --hints--

你应该访问 `location` 参数的 `"button functions"` 属性的第一个元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

你应该将 `button1.onclick` 属性设置为 `location` 参数的 `"button functions"` 属性的第一个元素。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

你应该访问 `location` 参数的 `"button functions"` 属性的第二个元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

你应该将 `button2.onclick` 属性设置为 `location` 参数的 `"button functions"` 属性的第二个元素。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

你应该访问 `location` 参数的 `"button functions"` 属性的第三个元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*2\s*\]/);
```

你应该将 `button3.onclick` 属性设置为 `location` 参数的 `"button functions"` 属性的第三个元素。
