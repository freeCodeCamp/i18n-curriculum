...
Menyu yako ya kuchagua inahitaji chaguzi kwa kila kipengele cha chakula na mazoezi cha `fieldset` ulichotengeneza katika hatua zilizopita. Tumia kipengele cha `option` kuunda chaguo jipya kwa kila `fieldset`. Sifa ya `value` ya kila chaguo inapaswa kuwa `id` ya `fieldset`, na maandishi ya kila chaguo yanapaswa kuwa maandishi ya `legend`.

Weka chaguo la `Breakfast` kama chaguo la `selected`.

# --hints--

Unapaswa kuunda vipengele vitano vya `option` ndani ya kipengele chako cha `select`.
