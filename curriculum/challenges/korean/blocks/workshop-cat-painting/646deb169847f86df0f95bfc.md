...
클래스 선택자를 사용해 `.cat-right-inner-eye` 요소에 너비 `10px`와 높이 `20px`를 지정하세요. 또한, 배경색을 `#fff`로 설정하세요.

# --hints--

`.cat-right-inner-eye` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye'))
```

`.cat-right-inner-eye` 선택자에 `width`가 `10px`로 설정되어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.width, '10px')
```

`.cat-right-inner-eye` 선택자에 `height`가 `20px`로 설정되어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.height, '20px')
```

`.cat-right-inner-eye` 선택자에 `background-color`가 `#fff`로 설정되어야 합니다.
