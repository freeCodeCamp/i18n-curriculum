...
title: Costruisci un progetto calcolatore di tempo
challengeType: 23
...
Scrivi una funzione chiamata `add_time` che prende due parametri obbligatori e uno opzionale:

- un orario di inizio nel formato a 12 ore (terminante con AM o PM)
- una durata che indica il numero di ore e minuti
- (opzionale) un giorno della settimana di partenza, senza distinzione tra maiuscole e minuscole

La funzione deve aggiungere la durata all'orario di inizio e restituire il risultato.

Se il risultato sarà il giorno successivo, deve mostrare `(next day)` dopo l'orario. Se il risultato sarà più di un giorno dopo, deve mostrare `(n days later)` dopo l'orario, dove "n" è il numero di giorni dopo.

Se alla funzione viene passato il parametro opzionale del giorno della settimana di partenza, allora l'output deve mostrare il giorno della settimana del risultato. Il giorno della settimana nell'output deve apparire dopo l'orario e prima del numero di giorni dopo.

Di seguito ci sono alcuni esempi di diversi casi che la funzione deve gestire. Presta molta attenzione agli spazi e alla punteggiatura dei risultati.
...
Non importare nessuna libreria Python. Presumi che gli orari di inizio siano orari validi. I minuti nella durata saranno un numero intero inferiore a 60, ma le ore possono essere qualsiasi numero intero.

Nota: apri la console del browser con F12 per vedere un output più dettagliato dei test.

# --hints--
Chiamare `add_time('3:30 PM', '2:12')` dovrebbe restituire `'5:42 PM'`.
...
Chiamare `add_time('11:55 AM', '3:12')` dovrebbe restituire `'3:07 PM'`.
...
L'orario previsto dovrebbe terminare con `'(next day)'` quando è il giorno successivo.
...
Il periodo previsto dovrebbe cambiare da `AM` a `PM` a `12:00`.
...
Chiamare `add_time('2:59 AM', '24:00')` dovrebbe restituire `'2:59 AM (next day)'`.
...
Chiamare `add_time('11:59 PM', '24:05')` dovrebbe restituire `'12:04 AM (2 days later)'`.
...
Chiamare `add_time('8:16 PM', '466:02')` dovrebbe restituire `'6:18 AM (20 days later)'`.
...
Si prevede che aggiungere `0:00` restituisca l'orario iniziale.
...
Chiamare `add_time('3:30 PM', '2:12', 'Monday')` dovrebbe restituire `'5:42 PM, Monday'`.
...
Chiamare `add_time('2:59 AM', '24:00', 'saturDay')` dovrebbe restituire `'2:59 AM, Sunday (next day)'`.
...
Chiamare `add_time('11:59 PM', '24:05', 'Wednesday')` dovrebbe restituire `'12:04 AM, Friday (2 days later)'`.
...
Chiamare `add_time('8:16 PM', '466:02', 'tuesday') ` dovrebbe restituire `'6:18 AM, Monday (20 days later)'`.
