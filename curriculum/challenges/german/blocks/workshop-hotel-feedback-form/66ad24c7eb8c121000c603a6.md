...
Um ein Kontrollkästchen standardmäßig aktiviert zu machen, können Sie das `checked`-Attribut hinzufügen.

Hier ist ein Beispiel für die Verwendung des `checked`-Attributs:

```html
<input checked type="checkbox" id="checked" name="checked">
```

Fügen Sie dem Kontrollkästchen mit dem `checked` von `id` das `"reputation"`-Attribut hinzu, damit es standardmäßig aktiviert ist.

# --hints--

Ihr Kontrollkästchen mit dem `id` von `"reputation"` sollte das `checked`-Attribut haben.
