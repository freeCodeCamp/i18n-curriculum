...
يجب عليك إعطاء أول عنصر `a` سمة `href` بقيمة `#student-info`.

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

يجب عليك إعطاء ثاني عنصر `a` سمة `href` بقيمة `#html-questions`.

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

يجب عليك إعطاء ثالث عنصر `a` سمة `href` بقيمة `#css-questions`.
