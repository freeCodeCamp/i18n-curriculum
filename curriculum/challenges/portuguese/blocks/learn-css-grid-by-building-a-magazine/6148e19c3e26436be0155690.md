...
Crie uma regra `.lists li` tendo como alvo os itens da lista dentro do elemento `.lists`.Dê uma propriedade `margin-bottom` definida como `1.5rem`.

# --hints--

Você deve ter um seletor `.lists li`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

O seletor `.lists li` deve ter a propriedade `margin-bottom` definida como `1.5rem`.
