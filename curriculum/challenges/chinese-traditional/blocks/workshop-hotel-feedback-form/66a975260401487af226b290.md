...
要使 `option` 默認爲選中狀態，你可以向希望被選中的 `option` 元素添加 `selected` 屬性。

下面是一個使用 `selected` 屬性的示例：

```html
<option selected value="amazing">Amazing</option>
```

在你的 `select` 元素中，給值爲 `"excellent"` 的 `option` 元素添加 `selected` 屬性。

# --hints--

值爲 `"excellent"` 的 `option` 元素應具有 `selected` 屬性。
