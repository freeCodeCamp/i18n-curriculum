...
حان الوقت الآن لإضافة وظيفة إعادة التعيين إلى `resetButton`. سيعيد هذا الأغاني في قائمة التشغيل عند النقر عليه.

أضف مستمع حدث نقر إلى المتغير `resetButton`. مرر دالة استدعاء راجع باستخدام صيغة السهم واتركها فارغة في الوقت الحالي.

# --hints--

يجب أن تستخدم الطريقة `addEventListener()` على `resetButton`.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

يجب أن يستمع مستمع الحدث الخاص بـ `resetButton` إلى حدث `"click"`.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

يجب أن تستخدم دالة الاستدعاء الراجع لمستمع الحدث صيغة السهم وأن تحتوي على زوج فارغ من الأقواس المتعرجة.
