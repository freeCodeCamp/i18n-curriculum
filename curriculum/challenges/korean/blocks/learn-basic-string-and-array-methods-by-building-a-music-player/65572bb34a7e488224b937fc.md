...
`resetText`를 `resetButton` 요소의 자식으로 추가하고, `resetButton`도 `playlistSongs` 요소의 자식으로 추가해야 합니다. 이를 위해 `appendChild()` 메서드를 사용하세요.

`appendChild()`는 노드나 요소를 다른 요소의 자식으로 추가할 수 있게 해줍니다. 아래 예시에서 "Click me"라는 텍스트가 버튼에 붙게 됩니다:
...
`appendChild()`를 사용해 `resetText`를 `resetButton` 요소에 붙이고, `resetButton`를 `playlistSongs` 요소에 붙이세요.

# --hints--

`appendChild()`에서 `resetButton` 메서드를 사용하세요.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

첫 번째 `resetText` 값으로 `appendChild()`를 전달해야 합니다.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

`appendChild()`에서 `playlistSongs` 메서드를 사용하세요.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

두 번째 `resetButton` 값으로 `appendChild()`를 전달해야 합니다.
