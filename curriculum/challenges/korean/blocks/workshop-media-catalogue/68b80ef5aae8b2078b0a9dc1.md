...
보시다시피, `series1`는 부모의 `Movie` 메서드를 상속받아 `__str__` 객체와 같은 형식으로 표시됩니다. 다음 단계에서는 `__str__` 메서드를 재정의하여 사용자 지정 문자열 표현을 제공할 것입니다.

지금은 코드에서 `print(series1)` 줄을 삭제하세요.

# --hints--

코드에 `print(series1)`가 없어야 합니다
