...
Um zwischen den verschiedenen Arten von User Stories zu unterscheiden, müssen Sie das Genre der ausgewählten Story an die `displayStory`-Funktion übergeben.

Fügen Sie der `genre`-Funktion einen Parameter namens `displayStory` hinzu.

# --hints--

Sie sollten der `genre`-Funktion den Parameter `displayStory` hinzufügen.
