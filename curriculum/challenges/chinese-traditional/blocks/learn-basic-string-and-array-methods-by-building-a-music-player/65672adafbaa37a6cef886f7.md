...
現在你已經創建了按鈕，你需要為它指定文字。為此，你需要使用 DOM 的 `createTextNode()` 方法。

`createTextNode()` 方法用來創建文字節點。要使用它，你呼叫它並傳入文字作為字串：
...
使用 `createTextNode()` 方法創建一個 `"Reset Playlist"` 文字，然後將它指定給一個 `resetText` 常數。
...
你應該使用 `document.createTextNode()` 來創建一個 `"Reset Playlist"` 文字。

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

你應該將你創建的文字節點指定給一個 `resetText` 常數。
