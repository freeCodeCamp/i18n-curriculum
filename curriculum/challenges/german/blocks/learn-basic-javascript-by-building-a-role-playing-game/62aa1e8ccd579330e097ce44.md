...
Verwende das `return` Schlüsselwort, um den Wert von `hit` am Ende der Funktion zurückzugeben.

# --hints--

Du solltest das `return` Schlüsselwort verwenden, um den Wert von `hit` zurückzugeben.

```js
assert.match(getMonsterAttackValue, /return\s+hit/);
```

Deine Funktion sollte den erwarteten Wert zurückgeben.
