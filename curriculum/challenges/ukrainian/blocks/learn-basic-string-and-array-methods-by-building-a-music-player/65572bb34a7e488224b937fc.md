...
Вам потрібно додати `resetText` до елемента `resetButton` як дочірній елемент, а також `resetButton` до елемента `playlistSongs` як дочірній елемент. Для цього є метод `appendChild()`, який слід використати.

`appendChild()` дозволяє додати вузол або елемент як дочірній елемент іншого елемента. У прикладі нижче текст "Click me" буде приєднано до кнопки:
...
Використайте `appendChild()`, щоб приєднати `resetText` до елемента `resetButton`, а `resetButton` — до елемента `playlistSongs`.

# --hints--

Ви повинні використати метод `appendChild()` на `resetButton`.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

Ви повинні передати `resetText` як значення вашого першого `appendChild()`.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

Ви повинні використати метод `appendChild()` на `playlistSongs`.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

Ви повинні передати `resetButton` як значення вашого другого `appendChild()`.
