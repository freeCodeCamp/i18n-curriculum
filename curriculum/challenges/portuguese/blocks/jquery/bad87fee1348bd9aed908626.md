...
Embora seja possível adicionar múltiplas classes em uma única chamada `.addClass()`, vamos adicioná-las ao mesmo elemento de *três maneiras separadas*.
...
**Nota:** Você deve estar direcionando apenas um elemento e adicionando apenas uma classe por vez. Ao todo, seus três seletores individuais acabarão adicionando as três classes `shake`, `animated` e `btn-primary` para `#target1`.
