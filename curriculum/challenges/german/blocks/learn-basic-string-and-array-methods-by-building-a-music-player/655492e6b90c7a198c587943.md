...
Jetzt müssen Sie das Attribut zum aktuell abgespielten Song hinzufügen.

Erstellen Sie eine `if`-Anweisung mit der Bedingung `songToHighlight`. Verwenden Sie für die Anweisung `setAttribute` auf `songToHighlight`, um `"aria-current"` und `"true"` als erstes und zweites Argument zu übergeben.

# --hints--

Sie sollten eine `if`-Anweisung mit der Bedingung `songToHighlight` erstellen.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

Sie sollten die `setAttribute()`-Methode auf `songToHighlight` innerhalb Ihrer `if`-Anweisung verwenden.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

Sie sollten `"aria-current"` und `"true"` als erstes und zweites Argument Ihrer `setAttribute`-Methode übergeben.
