...
1. Unapaswa kuwa na kipengele cha `div` chenye `id` ya `drum-machine` inayojumuisha vipengele vyote vingine.

2. Ndani ya kipengele cha `#drum-machine` unapaswa kuwa na kipengele kingine cha `div` chenye `id` ya `pad-bank`.

3. Ndani ya kipengele cha `#drum-machine` unapaswa kuwa na kipengele cha `p` chenye `id` ya `display`.

4. Ndani ya kipengele chako cha `#pad-bank` unapaswa kuwa na vipengele tisa vya pad ya ngoma vinavyoweza kubofywa kila kikiwa na darasa la `drum-pad`, sifa ya kipengele ya `id` ya kipekee inayofafanua kipande cha sauti ambacho pad ya ngoma itapangwa kuanzisha, na maandishi ya ndani yanayolingana na moja ya funguo zifuatazo kwenye kibodi: `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`. Vipad vya ngoma VINAPASWA kuwa katika mpangilio huu.

5. Kila `.drum-pad` inapaswa kuwa na kipengele cha `audio` chenye darasa la `clip`, sifa ya kipengele ya `src` inayorejelea kipande cha sauti, na `id` inayolingana na maandishi ya ndani ya kipengele cha `.drum-pad` cha mzazi wake (mfano `id="Q"`, `id="W"`, `id="E"` nk).

6. Unapobofya kipengele cha `.drum-pad`, kipande cha sauti kilicho ndani ya kipengele chake cha sauti kinapaswa kuanzishwa.

7. Unapobonyeza funguo inayohusiana na kila `.drum-pad`, kipande cha sauti kilicho ndani ya kipengele chake cha sauti kinapaswa kuanzishwa (mfano, kubonyeza funguo ya `Q` kunapaswa kuanzisha pad ya ngoma inayojumuisha mfuatano wa herufi `Q`, kubonyeza funguo ya `W` kunapaswa kuanzisha pad ya ngoma inayojumuisha mfuatano wa herufi `W`, nk).

8. Kipengele cha `.drum-pad` kinapoanzishwa, unapaswa kuonyesha mfuatano wa herufi unaoelezea kipande cha sauti kinachohusiana kama maandishi ya ndani ya kipengele cha `#display` (kila mfuatano lazima uwe wa kipekee).

Baadhi ya sampuli za sauti unazoweza kutumia kwa mashine yako ya ngoma zinaweza kupatikana kwenye `https://cdn.freecodecamp.org/curriculum/drum/<fileName>`, ambapo `<fileName>` ni kama ifuatavyo:

| jina la ngoma | `fileName` |
| --- | --- |
...
Unapaswa kuwa na kipengele cha `div` chenye `id` ya `drum-machine` inayojumuisha vipengele vyote vingine.

```js
assert.isNotNull(document.querySelector('div#drum-machine'));
```

Ndani ya kipengele cha `#drum-machine` unapaswa kuwa na kipengele kingine cha `div` chenye `id` ya `pad-bank`.

```js
assert.isNotNull(document.querySelector('#drum-machine div#pad-bank'));
```

Ndani ya kipengele cha `#drum-machine` unapaswa kuwa na kipengele cha `p` chenye `id` ya `display`.

```js
assert.isNotNull(document.querySelector('#drum-machine p#display'));
```

Ndani ya kipengele chako cha `#pad-bank` unapaswa kuwa na vipengele tisa vya pad ya ngoma vinavyoweza kubofywa kila kikiwa na darasa la `drum-pad`.

```js
assert.lengthOf(document.querySelectorAll('#pad-bank .drum-pad'), 9);
```

Kila `.drum-pad` inapaswa kuwa kipengele cha `button`.
...
Kila `.drum-pad` inapaswa kuwa na moja ya herufi zifuatazo kama `innerText`, kwa mpangilio: `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`.
...
Kila `.drum-pad` inapaswa kuwa na kipengele cha `audio` chenye darasa la `clip`, sifa ya kipengele ya `src` inayorejelea kipande cha sauti, na `id` inayolingana na maandishi ya ndani ya kipengele cha mzazi wake cha `.drum-pad` (mfano `id="Q"`, `id="W"`, `id="E"` nk).
...
Unapobofya kipengele cha `.drum-pad`, kipande cha sauti kilicho ndani ya kipengele chake cha sauti kinapaswa kuanzishwa.
...
Unapobonyeza moja ya funguo Q, W, E, A, S, D, Z, X, C kwenye kibodi yako, kipengele kinacholingana cha `audio` kinapaswa kucheza sauti inayolingana.
...
Kipengele cha `.drum-pad` kinapoanzishwa, unapaswa kuonyesha mfuatano wa herufi unaoelezea kipande cha sauti kinachohusiana kama maandishi ya ndani ya kipengele cha `#display` (kila mfuatano lazima uwe wa kipekee).
