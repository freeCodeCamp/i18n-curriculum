...
Agora que você criou uma classe `Dessert`, crie uma classe `ShoppingCart`.
...
Você deve usar a palavra-chave `class` para declarar uma classe `ShoppingCart`.

```js
assert.match(code, /class\s+ShoppingCart\s*/);
```

Sua classe `ShoppingCart` deve estar vazia.
