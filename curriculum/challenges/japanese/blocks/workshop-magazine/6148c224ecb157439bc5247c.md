...
CSS Grid のデフォルト設定は Flexbox と違い、必要に応じて追加の行を作成します。アイコンを1行に並べるために、`.social-icons` セレクターに `grid-template-columns` プロパティを `repeat(5, 1fr)` に設定してください。

# --hints--

`.social-icons` セレクターには `grid-template-columns` プロパティを `repeat(5, 1fr)` に設定してください。
