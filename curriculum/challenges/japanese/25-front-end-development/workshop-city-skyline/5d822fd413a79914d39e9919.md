...
左右の境界線のサイズを大きくすると、下端の境界線は左右の境界線の幅を足し合わせた幅にまで広がります。 `.fb1a` の `border-left` プロパティと `border-right` プロパティの値として `2vw solid transparent` を追加してください。 左右の境界線は表示されませんが、下端の境界線の幅が `4vw` になります。
