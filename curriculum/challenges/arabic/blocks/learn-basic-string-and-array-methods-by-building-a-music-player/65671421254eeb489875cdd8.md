...
داخل دالة الاستدعاء الراجع، استخدم الطريقة `removeAttribute()` لإزالة الخاصية `"aria-current"`. هذا سيزيل الخاصية من كل الأغاني.

# --hints--

لا تعدل على `forEach()` الحالية ومحتواها.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

استخدم الطريقة `removeAttribute()` على `songEl` ومرر الخاصية `"aria-current"`.
