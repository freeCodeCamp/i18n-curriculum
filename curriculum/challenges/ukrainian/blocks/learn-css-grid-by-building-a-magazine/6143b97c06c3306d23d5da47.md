...
У межах вашого елемента `article` створіть елемент `h3` з множиною `class`, встановленою на `list-title`, і текстом `A Brief History`. Нижче створіть елемент `p` з текстом `Of the Curriculum`. Потім створіть елемент `ul` з класом `lists`.

# --hints--

Ви повинні створити елемент `h3` всередині вашого елемента `article`.

```js
assert.exists(document.querySelector('article h3'));
```

Ви повинні створити елемент `p` всередині вашого елемента `article`.

```js
assert.exists(document.querySelector('article p'));
```

Ви повинні створити елемент `ul` всередині вашого елемента `article`.

```js
assert.exists(document.querySelector('article ul'));
```

Ваші елементи всередині елемента `article` мають бути в правильному порядку.
...
Ваш новий елемент `h3` має мати множину `class`, встановлену на `list-title`.

```js
assert(document.querySelector('article h3')?.className === 'list-title');
```

Ваш новий елемент `h3` має містити текст `A Brief History`.

```js
assert(document.querySelector('article h3')?.innerText === 'A Brief History');
```

Ваш новий елемент `p` має містити текст `Of the Curriculum`.

```js
assert(document.querySelector('article p')?.innerText === 'Of the Curriculum');
```

Ваш новий елемент `ul` має мати множину `class`, встановлену на `lists`.
