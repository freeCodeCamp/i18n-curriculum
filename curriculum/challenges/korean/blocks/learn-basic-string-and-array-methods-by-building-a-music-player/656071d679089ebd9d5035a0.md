...
사용자가 재생목록에서 노래를 섞거나 삭제할 수 있으므로, 원본을 변경하지 않고 `allSongs` 배열의 복사본을 만들어야 합니다. 이때 `spread` 연산자가 유용합니다.

전개 연산자(`...`)는 한 배열의 모든 요소를 다른 배열로 복사할 수 있게 해줍니다. 또한 여러 배열을 하나로 연결하는 데도 사용할 수 있습니다. 아래 예시에서 `arr1`과 `arr2`가 모두 `combinedArr`에 전개되었습니다:
...
`userData` 객체 안에 `songs` 속성을 만드세요. 값으로는 `allSongs`을 배열로 전개하세요.

# --hints--

`userData` 객체에는 `songs`이라는 키가 있어야 합니다.

```js
assert.property(userData, "songs");
```

`songs` 속성의 값은 `[...allSongs]`여야 합니다.
