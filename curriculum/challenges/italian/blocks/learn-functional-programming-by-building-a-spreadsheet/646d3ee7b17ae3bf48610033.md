...
`infixToFunction[operator]` restituisce una funzione. Chiama direttamente quella funzione, passando `arg1` e `arg2` come argomenti.

# --hints--

La tua funzione di callback dovrebbe restituire il risultato della chiamata a `infixToFunction[operator]`.
...
Dovresti passare `arg1` come primo argomento alla tua chiamata `infixToFunction[operator]`.
...
Dovresti passare `arg2` come secondo argomento alla tua chiamata `infixToFunction[operator]`.
