...
`.ground` 要素はページレイアウトのスタッキングコンテキストで3番目になるので、`z-index` を `3` に、`position` を `absolute` に設定してください。

# --hints--

`.ground` に `z-index` の `3` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.ground')?.zIndex, '3');
```

`.ground` に `position` の `absolute` を設定してください。
