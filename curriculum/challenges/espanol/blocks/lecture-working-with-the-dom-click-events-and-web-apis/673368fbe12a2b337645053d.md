...
Todos los objetos `type` tendrán la propiedad `Event`. Esta propiedad revela el tipo de evento que activó la carga útil, como `"keydown"` o `"click"`. Estos valores corresponderán a los mismos valores que puede pasar a `addEventListener()`, donde puede capturar y utilizar el objeto `Event`.
