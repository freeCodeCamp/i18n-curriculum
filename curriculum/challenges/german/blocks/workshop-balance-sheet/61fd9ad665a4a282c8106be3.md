...
Erstellen Sie innerhalb Ihres `thead`-Elements ein `tr`-Element. Fügen Sie darin ein `td`-Element und drei `th`-Elemente hinzu. Innerhalb jedes der `th`-Elemente fügen Sie ein `span`-Element mit dem `class`-Attribut, das auf `sr-only` gesetzt ist, und dem folgenden Text in der Reihenfolge hinzu: `2019`, `2020` und `2021`.
...
Jedes Ihrer neuen `span`-Elemente sollte das `class`-Attribut mit dem Wert `sr-only` haben.
