...
`.logo` セレクターを作成してロゴをスタイリングし始めましょう。 `width` の値を `200px` に、`position` の値を `absolute` に、`top` の値を `23px` に設定してください。
