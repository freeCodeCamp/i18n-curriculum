...
Тепер заповніть `bugMap` двома записами. Додайте `"bee"` як ключ, спроєктований на новий екземпляр `Bee`, і `"spider"` як ключ, спроєктований на новий екземпляр `Spider`. Передайте `bugEmojiElement` як аргумент обом.

# --hints--

Ваш `bugMap` має містити запис `"bee"`, встановлений у `new Bee(bugEmojiElement)`.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

Ваш `bugMap` має містити запис `"spider"`, встановлений у `new Spider(bugEmojiElement)`.
