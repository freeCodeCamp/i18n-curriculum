...
Перший крок у розборі картки каталогу — розділити чотири поля. Метод `.split()` розбиває рядок на масив підрядків при кожній появі вказаного роздільника.
...
Всередині `parseCard` викличте `.split("|")` на `rawString` і поверніть результат. Після цього кроку в консолі має з’явитися масив із чотирьох рядків.

**Примітка:** Ви можете помітити зайві пробіли навколо значень. Ви очистите їх на наступному кроці.

# --hints--

Ви маєте викликати `.split("|")` на `rawString` у вашій функції `parseCard`.

```js
assert.match(parseCard.toString(), /rawString\s*\.\s*split\s*\(\s*('|"|`)\|\1\s*\)\s*;?/);
```

`parseCard` має повертати масив.

```js
assert.isArray(parseCard("A | B | C | D"));
```

При виклику з `"A | B | C | D"` `parseCard` має повертати масив із чотирьох елементів.
