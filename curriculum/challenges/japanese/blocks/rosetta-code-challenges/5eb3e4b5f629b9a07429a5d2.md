...
**SHA-1** または **SHA1** は一方向ハッシュ関数で、160ビットのメッセージダイジェストを計算します。

SHA-1 はセキュリティプロトコルでよく使われます。たとえば、多くの HTTPS ウェブサイトは接続を保護するために RSA と SHA-1 を使用しています。

BitTorrent はダウンロードの検証に SHA-1 を使います。

Git や Mercurial はコミットを識別するために SHA-1 ダイジェストを使用します。

アメリカ政府の標準である <a href="https://rosettacode.org/wiki/SHA-1/FIPS-180-1" target="_blank" rel="noopener noreferrer nofollow">FIPS 180-1</a> が SHA-1 を定義しています。

# --instructions--

与えられた文字列の SHA-1 メッセージダイジェストを返す関数を書いてください。
