...
这解决了项卡的一些基本样式间距，包括 CTA 按钮。对于其余的内部内容，你需要通过定位内部的 `div` 将所有内容排列在一个网格中。

为内部的 `div` 赋值类 `grid`、`grid-rows-[auto_auto_auto_1fr]` 和 `gap-y-2`。

# --hints--

你的家庭计划的内部 `div` 元素应具有 `grid` 类。
...
你的家庭计划的内部 `div` 元素应具有类 `grid-rows-[auto_auto_auto_1fr]`。
