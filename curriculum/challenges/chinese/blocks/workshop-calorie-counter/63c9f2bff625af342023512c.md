...
To complete this workshop, add an event listener to the `clearButton` button. 当单击按钮时，它应该调用 `clearForm` 函数。

恭喜！ 你的项目完成了。

# --hints--

你应该向 `clearButton` 按钮添加一个事件监听器。

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(/);
```

你应该将 `"click"` 作为第一个参数传递给 `.addEventListener()`。

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

你应该将 `clearForm` 作为第二个参数传递给 `.addEventListener()`。
