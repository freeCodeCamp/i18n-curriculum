...
`b()` が実行されたので、呼び出しスタックからそれを取り除いてください。その後、`a()` へのモック呼び出しを次のように更新してください：`'a(): returns "freeCodeCamp " + "is awesome!"'`。
...
`callStack` の文字列は `'a(): returns "freeCodeCamp " + "is awesome!"'` であるべきです。
