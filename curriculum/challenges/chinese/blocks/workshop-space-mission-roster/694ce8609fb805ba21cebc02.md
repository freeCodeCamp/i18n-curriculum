...
任务控件需要一个方法来交换名单中两个成员的位置。创建一个名为 `swapCrewMembers` 的空函数，带有三个参数：

- `crew`：一个宇航员对象的数组

- `fromIndex`：第一个要交换的宇航员的索引

- `toIndex`：第二个宇航员交换的索引

# --hints--

你应该创建一个名为 `swapCrewMembers` 的函数。

```js
assert.isFunction(swapCrewMembers);
```

你的 `swapCrewMembers` 函数应具有参数 `crew`、`fromIndex` 和 `toIndex`。
