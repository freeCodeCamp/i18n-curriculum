...
Перевірте, чи є предикат (другий аргумент) <dfn>істинним</dfn> для всіх елементів колекції (перший аргумент).

Іншими словами, вам дається масив колекції об’єктів. Предикат `pre` буде властивістю об’єкта, і вам потрібно повернути `true`, якщо його значення `truthy`. Інакше поверніть `false`.

У JavaScript, `truthy` значення — це значення, які при оцінюванні в булевому контексті перетворюються на `true`.

Пам’ятайте, що ви можете отримати доступ до властивостей об’єкта як через точкову нотацію, так і через `[]` нотацію.
