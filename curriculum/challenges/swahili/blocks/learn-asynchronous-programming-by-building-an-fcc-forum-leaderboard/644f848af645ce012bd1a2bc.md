...
Unda thabiti iitwayo `linkText` na weka thamani yake kuwa `selectedCategory.category`. Hii itaonyesha jina la kategoria katika kipengele cha nanga.

# --hints--

Unapaswa kuwa na thabiti iitwayo `linkText`.

```js
assert(code.match(/const\s+linkText\s*=/));
```

Unapaswa kuweka thamani ya `selectedCategory.category` kwa mabadiliko yako ya `linkText`.
