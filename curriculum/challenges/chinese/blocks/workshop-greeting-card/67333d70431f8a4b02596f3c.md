...
另一个可以用于 `transform` 属性的值是 `skewX`，该函数水平倾斜元素。
...
为你的 `section` 元素添加一个在悬停时生效的选择器。将 `transform` 属性设置为 `skewX(10deg)`。 

# --hints--

你应该有一个 `section:hover` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle("section:hover"));
```

`section:hover` 选择器应具有设置为 `skewX(10deg)` 的 `transform` 属性。
