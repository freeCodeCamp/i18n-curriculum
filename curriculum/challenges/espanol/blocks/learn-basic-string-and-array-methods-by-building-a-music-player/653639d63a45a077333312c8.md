...
Antes de reproducir la canción, debes asegurarte de que comience desde el principio. Esto se puede lograr utilizando la propiedad `currentTime` en el objeto `audio`. 

Agrega una declaración `if` para verificar si `userData?.currentSong` es falso *O* si `userData?.currentSong.id` no es estrictamente igual a `song.id`. Esta condición comprobará si no se está reproduciendo ninguna canción actual o si la canción actual es diferente de la que está a punto de reproducirse.

Dentro del bloque `if`, establezca la propiedad `currentTime` del objeto `audio` a `0`.

# --hints--

Deberías crear una declaración `if` con la condición `userData?.currentSong === null || userData?.currentSong.id !== song.id`. No olvides incluir el encadenamiento opcional.
...
Deberías establecer `audio.currentTime` en `0` dentro de tu bloque `if`.
