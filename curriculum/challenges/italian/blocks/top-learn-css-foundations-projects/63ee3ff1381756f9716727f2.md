...
title: Esercizio sulle basi di CSS D
challengeType: 25
...
Con questo esercizio, ti abbiamo fornito un file HTML completo, quindi dovrai modificare solo il file CSS. Per questo esercizio, è più importante capire come funziona il concatenamento di selettori diversi che come aggiungere effettivamente gli attributi.

1. Dovresti vedere un `width` di `300px` sulle classi `avatar` e `proportioned`.
1. Dovresti assegnargli un'altezza in modo che mantenga le sue proporzioni quadrate originali (non impostare un valore fisso in pixel per l'altezza!).
1. Dovresti dare agli elementi con entrambe le classi `avatar` e `distorted` un `width` di `200px`.
1. Dovresti assegnargli un `height` doppio rispetto alla sua larghezza.

# --hints--

Dovresti avere un `width` di `300px` sulle classi `avatar` e `proportioned`.
...
Dovresti avere un `width` di `200px` sulle classi `avatar` e `distorted`.
...
Dovresti avere un `height` doppio rispetto alla larghezza sulle classi `avatar` e `distorted`.
