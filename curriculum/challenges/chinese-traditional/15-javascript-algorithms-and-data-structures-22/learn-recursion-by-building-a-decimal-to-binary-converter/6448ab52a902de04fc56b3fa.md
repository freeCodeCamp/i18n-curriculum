...
現在你將開始構建函數來實際執行十進制到二進制的轉換。

創建一個名爲 `decimalToBinary` 的函數，並以 `input` 作爲參數。 函數主體暫時留空。

# --hints--

`decimalToBinary` 應該是一個函數。

```js
assert.isFunction(decimalToBinary);
```

你應該將 `input` 作爲參數傳遞給 `decimalToBinary` 函數。

```js
assert.match(String(decimalToBinary), /function\s+decimalToBinary\s*\(\s*input\s*\)/);
```

`decimalToBinary` 函數的主體應該爲空。
