...
Fanya mazoezi ya kuchagua kitenzi sahihi kwa idara moja.
...
`Verifican` ni fomu ya `ellos/ellas`, lakini `el departamento` ni ya wingi mmoja.
...
`Verificas` ni fomu ya `tú`, si ya `el departamento`.
...
`El departamento de control de calidad verifica el software.` - Idara ya udhibiti wa ubora inakagua programu.

Mhusika `el departamento` ni wa wingi mmoja, kwa hivyo inahitaji fomu ya kitenzi ya wingi mmoja `verifica`. 
