...
Зверніть увагу, як число `10` розміщено на початку масиву. Це пов’язано з тим, що поведінка за замовчуванням `.sort()` — це перетворити числові значення на рядки та сортувати їх за алфавітом. І `"10"` стоїть перед `"2"` за алфавітом.

Щоб виправити це, можна передати функцію зворотного виклику до методу `.sort()`. Функція зворотного виклику має два параметри — використайте `a` та `b`. Параметри `a` та `b` представляють числові значення в масиві, який буде відсортовано.
...
Функція зворотного виклику має приймати два параметри: `a` та `b`.
