...
現在你需要在具有 `targetId` 的元素中定位 `.input-container` 元素。 聲明一個新的 `targetInputContainer` 變量，併爲其分配 `document.querySelector()` 的值。 使用連接符將 `targetId` 和 `'.input-container'` 用空格分隔，並將該字符串傳遞給 `querySelector()`。

# --hints--

你的 `addEntry` 函數應該有一個 `targetInputContainer` 變量。

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=/);
```

你的 `targetInputContainer` 變量應設置爲 `document.querySelector()`。
...
你應該將 `' .input-container'` 連接到 `targetId`。 請記住在 `.input-container` 的開頭包含空格。
