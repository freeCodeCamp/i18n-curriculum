...
Prima di iniziare a gestire la riproduzione della canzone successiva e precedente, devi ottenere l'indice di ogni canzone nella proprietà `songs` di `userData`. Per farlo puoi usare il metodo array `indexOf()`, che restituisce il primo indice in cui un dato elemento può essere trovato nell'array, oppure `-1` se l'elemento non è presente.
...
Crea una funzione chiamata `getCurrentSongIndex` che restituisca l'indice in cui `userData.currentSong` può essere trovato in `userData.songs` e `-1` quando la canzone corrente è `null`.

# --hints--

Dovresti avere una funzione chiamata `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

La tua funzione `getCurrentSongIndex` dovrebbe restituire l'indice in cui `userData.currentSong` può essere trovato in `userData.songs`.
...
La tua funzione `getCurrentSongIndex` dovrebbe restituire `-1` quando `userData.currentSong` è `null`.
