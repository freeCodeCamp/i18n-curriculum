...
El objeto `Event` es una carga útil que se activa cuando un usuario interactúa con su página web de alguna manera. Estas interacciones pueden ser cualquier cosa desde hacer clic en un botón o enfocar un input hasta sacudir su dispositivo móvil.
...
Todos los objetos `Evento` tendrán la propiedad `tipo`. Esta propiedad revela el tipo de evento que activó la carga útil, como `"keydown"` o `"click"`. Estos valores corresponderán a los mismos valores que puede pasar a `addEventListener()`, donde puede capturar y utilizar el objeto `Event`.
...
Si quieres manejar las presentaciones de formularios tú mismo, por ejemplo, podrías llamar a `preventDefault()` para evitar que el navegador intente enviar los datos del formulario como una solicitud `POST`. Aprenderás más sobre las solicitudes `POST` en lecciones futuras.

También es probable que te encuentres con el método `stopPropagation()`. Este método evita que el evento se propague a los elementos padre.  You'll learn more about what this means in a future lesson.
