...
Тепер ви можете стилізувати макет своєї сітки. CSS Grid схожий на гнучкий контейнер тим, що має спеціальну властивість як для батьківського, так і для дочірніх елементів.

У цьому випадку вашим батьківським елементом є елемент `main`. Встановіть вміст так, щоб він мав трьохколонковий макет, додавши властивість `grid-template-columns` зі значенням `1fr 94rem 1fr`. Це створить три колонки, де середня колонка буде `94rem` завширшки, а перша й остання колонки — по 1 дробу від залишкового простору в контейнері сітки.

# --hints--

Ваш розділ `main` має мати властивість `grid-template-columns` зі значенням `1fr 94rem 1fr`.
