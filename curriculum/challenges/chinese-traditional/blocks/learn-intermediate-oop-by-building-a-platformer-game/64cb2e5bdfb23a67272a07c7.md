...
現在你需要為 `CheckPoint` 類別創建一個 `draw` 方法。 

在 `draw` 方法內，為 `ctx` 物件的 `fillStyle` 屬性指定十六進位顏色 `"#f1be32"`。 

在 `fillStyle` 屬性下，對 `ctx` 物件使用 `fillRect` 方法，並將 `x`、`y`、`width` 和 `height` 屬性作為引數傳入。

# --hints--

你的 `CheckPoint` 類別應該有一個 `draw` 方法。
...
你的 `draw` 方法應該為 `ctx` 物件上的 `fillStyle` 屬性指定一個值。
...
你應該將十六進位顏色 `"#f1be32"` 指定給 `ctx` 物件上的 `fillStyle` 屬性。
...
你的 `draw` 方法應該在 `ctx` 物件上喚起 `fillRect` 方法。
...
當呼叫 `ctx.fillRect` 時，你應該將 `position.x`、`position.y`、`width` 和 `height` 屬性作為引數傳入。別忘了 `this` 關鍵字。
