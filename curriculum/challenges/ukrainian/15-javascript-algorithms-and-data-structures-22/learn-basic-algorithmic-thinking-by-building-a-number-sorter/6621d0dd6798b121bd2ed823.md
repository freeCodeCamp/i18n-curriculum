...
Оновіть зворотний виклик `.map()`, щоб викликати функцію `Number()`. Передайте `dropdown.value` до виклику цієї функції.

Відкрийте вкладку «Консоль», щоб побачити, що `inputValues` тепер є масивом чисел.

# --hints--

Викличте функцію `Number()` у зворотному виклику `.map()`.
...
Передайте `dropdown.value` у виклик `Number()`.
