...
Usa l'evento `oninput` su `shapeTypeSelect` e impostalo su `handleShapeSelect`. Questo farà eseguire la funzione ogni volta che la selezione della forma cambia.

Dopo aver fatto questo, dovresti riuscire a vedere i campi di input di ogni forma. Ma i calcoli non funzioneranno ancora, e questo è ciò che affronterai nel passaggio successivo.

# --hints--

Dovresti impostare `shapeTypeSelect.oninput` su `handleShapeSelect`.
