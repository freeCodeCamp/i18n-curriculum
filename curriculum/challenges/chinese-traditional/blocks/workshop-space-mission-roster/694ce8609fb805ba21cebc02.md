...
任務控制元件需要一個方法來交換名冊中兩個成員的位置。創建一個名為 `swapCrewMembers` 的空函式，並帶有三個參數：

- `crew`：一個太空人物件的陣列

- `fromIndex`：第一個要交換的太空人索引

- `toIndex`：第二個要交換的太空人索引

# --hints--

你應該創建一個名為 `swapCrewMembers` 的函式。

```js
assert.isFunction(swapCrewMembers);
```

你的 `swapCrewMembers` 函式應該有參數 `crew`、`fromIndex` 和 `toIndex`。
