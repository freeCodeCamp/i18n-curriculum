...
É hora de ver o validador detectar um problema real. `catalog[0]` é "From a Buick 8" de Stephen King, que tem todos os quatro campos preenchidos. `catalog[16]` é "Unknown Title", que tem o campo do autor em branco.

Registre `validateEntry(catalog[0])` e `validateEntry(catalog[16])`. O primeiro deve retornar `true`; o segundo deve retornar `false`.

# --hints--

Você deve registrar `validateEntry(catalog[0])` no console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

Você deve registrar `validateEntry(catalog[16])` no console.
