...
使用 `catalogue` 的 `add` 方法将 `series1` 追加到 `items` 列表。

然后，创建另一个变量 `series2`。为它赋值另一个 `TVSeries` 实例，并将 `series2` 追加到目录中。

# --hints--

你应该使用 `catalogue` 的 `add` 方法传入 `series1`。
...
你应该创建一个名为 `series2` 的变量，并赋值为一个 `TVSeries` 的实例。
...
你应该使用 `catalogue` 的 `add` 方法传入 `series2`。
