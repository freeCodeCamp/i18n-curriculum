...
같은 구문을 사용하여 `c`을 `c`, `{`, `[`, 그리고 `(`와 일치하도록 업데이트하세요.

# --hints--

`stockRegex`는 문자 클래스를 사용하여 `c`, `{`, `[`, 그리고 `(`와 일치해야 합니다.
...
`stockRegex`은 `570(k 4l3r7`와 일치해야 합니다.

```js
assert.match('570(k 4l3r7', stockRegex);
```

`stockRegex`는 `sto{k alert`와 일치해야 합니다.

```js
assert.match('sto{k alert', stockRegex);
```

`stockRegex`은 `sto[k alert`와 일치해야 합니다.
