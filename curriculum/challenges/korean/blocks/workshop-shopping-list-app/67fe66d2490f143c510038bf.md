...
그다음, 체크박스 `input` 요소에 `checked` 속성을 추가하고 `isChecked`로 설정하세요. `checked` 속성을 꼭 설정할 필요는 없지만, 각 체크박스가 현재 `selectedItems` 배열 상태를 반영하도록 하는 것이 좋은 습관입니다.

# --hints--

체크박스 `checked` 요소에 `input` 속성을 추가해야 합니다.
...
`checked` 속성을 `isChecked`로 설정해야 합니다.
