...
Establece el color de fondo de `footer` a `#2a2a40`, y usa _Flexbox_ para centrar horizontalmente el texto.

# --hints--

Debes usar el selector del elemento `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

Deberías dar a `footer` un `background-color` de `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

Deberías dar a `footer` un `display` de `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Deberías dar a `footer` un `justify-content` de `center`.
