...
最后，在 `hideElements` 内传入参数 `singleCard`、`multipleCard`、`fortuneContainer` 和 `fortuneDescription`。记得从元素中获取它们。

这样，你的算命应用就完成了！

# --hints--

你应该将 `singleCard` 作为第一个参数传给 `hideElements`。
...
你应该将 `multipleCard` 作为参数传给 `hideElements`。
...
你应该将 `fortuneContainer` 作为参数传给 `hideElements`。
...
你应该将 `fortuneDescription` 作为参数传给 `hideElements`。
