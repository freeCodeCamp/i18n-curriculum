...
Crea otro elemento `p` después de tu último `.divider`, y dale el texto `Total Sugars 12g`. Asigna a ese elemento `p` los valores de `class` necesarios para sangrarlo y eliminar el borde inferior. Luego crea otro `.divider` debajo de tu nuevo elemento `p`.

# --hints--

Debes crear un nuevo elemento `p` y `div` al final de tu elemento `.daily-value`.
...
Tu nuevo elemento `p` debería tener el texto `Total Sugars 12g`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Total Sugars[\s|\n]+12g/));
```

Su nuevo elemento `p` debe tener el atributo `class` configurado a `indent no-divider`.
...
Tu nuevo `div` debe tener el atributo `class` establecido como `divider`.
