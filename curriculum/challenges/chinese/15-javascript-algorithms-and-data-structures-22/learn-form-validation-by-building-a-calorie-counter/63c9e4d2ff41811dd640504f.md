...
现在是时候开始准备计算了。 首先声明一个 `consumedCalories` 变量，并为其分配 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的总和（请注意，顺序对测试很重要）。 请确保在 `if` 语句之后执行此操作。

# --hints--

你的函数应该有一个 `consumedCalories` 变量。

```js
assert.match(calculateCalories.toString(), /consumedCalories\s*=/);
```

你的 `consumedCalories` 变量应位于 `if` 语句之后。

```js
assert.isAbove(calculateCalories.toString().indexOf('consumedCalories'), calculateCalories.toString().indexOf('if'));
```

你应该按顺序计算 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的总和。

```js
assert.match(calculateCalories.toString(), /breakfastCalories\s*\+\s*lunchCalories\s*\+\s*dinnerCalories\s*\+\s*snacksCalories\s*;?\s*$/m);
```

你应该将 `breakfastCalories`、`lunchCalories`、`dinnerCalories` 和 `snacksCalories` 的总和分配给 `consumedCalories`。
