...
在開發過程中，查看 `fieldset` 預設邊框是很有用的。然而，它們會讓內容看起來過於分離。

移除 `border`，並且只為每個 `fieldset` 的上下添加 `2rem` 的內距。務必移除左右的 `padding`。

# --hints--

你可以使用 `none` 或 `0` 的值來移除 `border`。

```js
assert.match(new __helpers.CSSHelp(document).getStyle('fieldset')?.border, /(none)|(0px)|(medium)/);
```

你應該為每個 `fieldset` 的上下方添加 `2rem` 的 `padding`。
