...
`playlist-song-artist` 클래스를 가진 `span` 요소를 만들어야 합니다.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

`playlist-song-artist` 클래스를 가진 `span` 요소는 `song.artist`를 내용으로 포함해야 합니다.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

`playlist-song-duration` 클래스를 가진 `span` 요소를 만들어야 합니다.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

`playlist-song-duration` 클래스를 가진 `span` 요소는 `song.duration`를 내용으로 포함해야 합니다.
