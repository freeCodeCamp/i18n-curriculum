...
بما أنك تخزن الكائن الذي تسبب في الاستثناء داخل كائن الاستثناء، يمكنك الاستفادة منه بعض الشيء.

بعد استدعاء `print` الحالي، أضف `print(f'Unable to add {e.obj}: {type(e.obj)}')`.

هذا النوع من الرسائل يمكن أن يكون مفيدًا جدًا عندما تحاول معالجة الأخطاء / تصحيح الأخطاء في شفرتك.

# --hints--

يجب أن يكون لديك `print(f'Unable to add {e.obj}: {type(e.obj)}')` بعد استدعاء `print(f'Media Error: {e}')` الحالي.
