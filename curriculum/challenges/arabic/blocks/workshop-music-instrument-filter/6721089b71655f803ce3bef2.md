...
أضف الآن أربعة خيارات إلى القائمة المنسدلة، وهي `All` و`Woodwinds` و`Brass` و`Percussion` بهذا الترتيب. لكل خيار، استخدم الخيار بأحرف صغيرة لتحديد خاصية `id` وخاصية `value`.

# --hints--

يجب أن تحتوي على أربعة عناصر `option` داخل عنصر `select` الخاص بك.

```js
assert.lengthOf(document.querySelectorAll("select>option"), 4);
```

يجب أن يحتوي أول عنصر `option` على النص `All`.

```js
assert.equal(document.querySelectorAll("select>option")[0].innerText,"All");
```

يجب أن يحتوي خيار `All` على خاصية `id` وخاصية `value` بقيمة `all`.
...
يجب أن يحتوي ثاني عنصر `option` على النص `Woodwinds`.

```js
assert.equal(document.querySelectorAll("select>option")[1].innerText,"Woodwinds");
```

يجب أن يحتوي خيار `Woodwinds` على خاصية `id` وخاصية `value` بقيمة `woodwinds`.
...
يجب أن يحتوي ثالث عنصر `option` على النص `Brass`.

```js
assert.equal(document.querySelectorAll("select>option")[2].innerText,"Brass");
```

يجب أن يحتوي خيار `Brass` على خاصية `id` وخاصية `value` بقيمة `brass`.
...
يجب أن يحتوي رابع عنصر `option` على النص `Percussion`.

```js
assert.equal(document.querySelectorAll("select>option")[3].innerText,"Percussion");
```

يجب أن يحتوي خيار `Percussion` على خاصية `id` وخاصية `value` بقيمة `percussion`.
