...
Agrega un detector de eventos `"click"` al elemento `previousButton`, luego pasa `playPreviousSong` como el segundo argumento.

# --hints--

Debes llamar al método `addEventListener()` sobre tu variable `previousButton`.
...
Debes pasar `playPreviousSong` como el segundo argumento de tu método `addEventListener`.
