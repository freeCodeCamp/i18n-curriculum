...
点击 `Submit Form` 按钮而不提供电子邮件地址，你将看到弹出消息，提示你填写该字段。
...
使用电子邮件输入的另一个优点是电子邮件输入具有一些基本验证，以确保电子邮件地址格式正确。例如，如果你输入随机单词并点击提交，浏览器将显示一个警告，提示缺少 `@` 符号。 

在电子邮件字段中输入 `abc` 并点击提交按钮。因为这不是有效的电子邮件地址，你应该看到弹出消息。
...
电子邮件 `input` 的其他验证形式是使用 `minlength` 和 `maxlength` 属性。以下是使用额外验证的示例。

在 `field` 中输入 `b@m` 并点击提交按钮。你会看到弹出一条 `message`，因为它未达到最小要求长度。
