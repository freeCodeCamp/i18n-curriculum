...
استبدل كل حالات `parseInt(numberInput.value)` بـ `inputInt`.

# --hints--

يجب استبدال `isNaN(parseInt(numberInput.value))` بـ `isNaN(inputInt)` و`parseInt(numberInput.value) < 0` بـ `inputInt < 0`.
...
يجب استبدال `parseInt(numberInput.value) === 5` بـ `inputInt === 5`.
...
يجب استبدال `decimalToBinary(parseInt(numberInput.value))` بـ `decimalToBinary(inputInt)`.
...
يجب أن تحتوي دالة `checkUserInput()` على حالة واحدة فقط من `parseInt(numberInput.value)`.
