...
이제 함수가 제대로 작동하는지 테스트할 시간입니다.

`console.log()` 함수 아래에 `getBooksByAuthor()`를 추가하세요.

`console.log()` 안에서 `getBooksByAuthor()` 함수를 `library`와 `"Arvid Kahl"` 인수로 호출하세요.

이제 콘솔에서 해당 작가의 모든 책이 보여야 합니다.

# --hints--

`console.log(getBooksByAuthor(library, "Arvid Kahl"))`가 있어야 합니다.
