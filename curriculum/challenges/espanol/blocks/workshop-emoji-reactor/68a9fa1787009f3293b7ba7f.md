...
Probablemente hayas notado que tu código se está volviendo bastante repetitivo. Hay una mejor manera de manejar múltiples elementos similares.

En lugar de seleccionar cada botón individualmente, usa `querySelectorAll(".emoji-btn")` para seleccionar todos los botones con la clase `emoji-btn` de una vez. Guarda el resultado en una variable llamada `btns`.
...
Debes usar `querySelectorAll` para seleccionar todos los botones de emoji.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

Debes almacenar el resultado en una variable llamada `btns`.
...
La variable `btns` debe contener los 4 botones de emoji.
