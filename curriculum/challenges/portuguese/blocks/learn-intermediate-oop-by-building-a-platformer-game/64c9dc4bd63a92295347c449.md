...
Em seguida, adicione uma declaração `else if` para verificar se a tecla esquerda foi pressionada e se `isCheckpointCollisionDetectionActive` é verdadeira.

Dentro dessa condição, adicione um loop `forEach` para iterar pelo array `platforms`.

Dentro do loop, use o operador de atribuição de adição para adicionar 5 à posição `x` da plataforma.

# --hints--

Você deve ter uma condição que verifica se a tecla esquerda foi pressionada e se `isCheckpointCollisionDetectionActive` é true.
...
Você deve adicionar um loop `forEach` que itere pelo array `platforms`.

```js
assert.match(code, /else\s+if\s*\(.*\)\s*{\s*platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

Você deve usar o operador de atribuição de adição para adicionar 5 à posição `x` da plataforma.
