...
في الوقت الحالي، في كل مرة يحرر المستخدم ملاحظة وينقر خارج الملاحظة، يتم تحديث المتغير `currentContent`. لكن ماذا عن الحالات التي يركز فيها المستخدم على الملاحظة ويقرر المغادرة دون إجراء أي تعديلات؟

في هذه الحالة، يمكنك إضافة شرط للتعامل مع هذا السيناريو.

فوق السطر `currentContent = newContent;`، أضف جملة `if` للتحقق مما إذا كان `currentContent` يساوي `newContent`. إذا كان كذلك، ارجع دون تنفيذ المزيد.

# --hints--

يجب أن تضيف جملة `if` للتحقق مما إذا كان `currentContent` يساوي `newContent`. إذا كان كذلك، ارجع.
