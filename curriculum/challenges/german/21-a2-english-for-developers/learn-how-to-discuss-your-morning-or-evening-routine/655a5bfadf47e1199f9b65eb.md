...
Past tense verbs are used to talk about actions that happened in the past. Some verbs change their form in the past tense, like `say` to `said`. Hier ein Beispiel:
...
Another example of an irregular verb is `tell`, which becomes `told` in the past tense. Zum Beispiel:
