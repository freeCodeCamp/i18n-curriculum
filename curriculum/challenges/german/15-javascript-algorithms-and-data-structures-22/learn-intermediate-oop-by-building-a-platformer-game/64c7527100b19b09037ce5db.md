...
Rufe darunter die `ctx.fillRect`-Methode auf und gib die `x`- und `y`-Koordinaten zusammen mit den `width`- und `height`-Eigenschaften ein. Denke daran, `this` vor jeder Eigenschaft einzufügen.
...
Du solltest die `ctx.fillRect`-Methode mit den richtigen Argumenten übergeben.
