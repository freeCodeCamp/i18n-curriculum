...
Щоб відсортувати пісні в алфавітному порядку за назвою, потрібно буде передати функцію зворотного виклику порівняння в метод `sort()`.
...
У наступних кількох кроках ви дізнаєтесь, що робить кожна з цих інструкцій `if` у цій функції зворотного виклику. Але наразі додайте порожню функцію зворотного виклику до методу `sort()` і використайте `a` та `b` для імен параметрів.

# --hints--

Ви повинні мати порожню функцію зворотного виклику `()=>{}` у методі `sort()` з `a` та `b` для імен параметрів.
