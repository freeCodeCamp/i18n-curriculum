...
在你的 `for` 迴圈中，第一個 `if` 述語之後，創建一個 `if` 述語，當目前字典的鍵集合與 `key_set` 不同時執行。這是為了確保字典中沒有遺失或無效的鍵。

在新的 `if` 述語中，列印 `Invalid format: <dictionary> at position <index> has missing and/or invalid keys.`（其中 `<dictionary>` 和 `<index>` 應該被當前迭代的字典和索引取代）並將 `is_invalid` 設為 `True`。

# --hints--

你應該在你的 `for` 迴圈內有第二個 `if` 述語。
...
你的新 `if` 述語應該在 `set(dictionary.keys())` 與 `key_set` 不同時執行。
...
你應該在你的新 `if` 述語中列印 `Invalid format: <dictionary> at position <index> has missing and/or invalid keys.`（其中 `<dictionary>` 和 `<index>` 應該被替換為當前迭代的字典和索引）。
...
你應該在你的新 `if` 述語內將 `is_invalid` 設為 `True`。
