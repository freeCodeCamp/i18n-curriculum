...
Para la nueva regla `@media`, establece el `width` de `#piano` a `675px` y el `width` de `.keys` a `633px`.

¡Con eso, tu piano está completo!

# --hints--

Tu segunda regla `@media` debe tener un selector `#piano`.
...
Tu nuevo selector `#piano` debe tener un `width` de `675px`.
...
Tu segunda regla `@media` debe tener un selector `.keys`.
...
Tu nuevo selector `.keys` debe tener un `width` de `633px`.
