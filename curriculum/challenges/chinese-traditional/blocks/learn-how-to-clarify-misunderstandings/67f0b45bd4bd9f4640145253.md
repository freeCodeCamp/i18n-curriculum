...
這個兩字片語表示仔細或詳細地檢查某物，通常是為了找出問題或了解其運作方式。
...
這個字的複數形式指的是用來找出系統或程式問題的測試或工具。

# --explanation--

`Dig into the code` 是指仔細查看程式碼以找出問題原因。例如：

`We'll need to dig into the code to find the bug.` – 這表示仔細閱讀並檢查程式碼是否有問題。

`Run diagnostics` 是指使用工具或指令來測試系統並找出任何問題。例如：

`Let's run diagnostics to see what's causing the crash.` – 這表示使用某些軟體來檢查系統錯誤。
