...
Fügen Sie dem `transition`-Selektor eine `.card-links a`-Eigenschaft hinzu und geben Sie ihr den Wert `background-color 0.3s ease`.

# --hints--

Der `.card-links a`-Selektor sollte eine `transition`-Eigenschaft mit dem Wert `background-color 0.3s ease` haben.
