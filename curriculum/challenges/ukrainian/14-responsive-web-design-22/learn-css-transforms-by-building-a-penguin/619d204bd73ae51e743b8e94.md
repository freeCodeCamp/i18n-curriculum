...
Ви повинні надати `.foot` властивість `top` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.top, '85%');
```

Ви повинні надати `.foot` властивість `border-radius` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.
