...
أنشئ دالة باسم `setPlayButtonAccessibleText`. داخل الدالة، عيّن الخاصية `aria-label` لزر التشغيل إلى `Play` متبوعة بمسافة وعنوان الأغنية الحالية. إذا كانت `userData.currentSong` تساوي `null`، عيّنها إلى `Play`.

# --hints--

يجب أن يكون لديك دالة باسم `setPlayButtonAccessibleText`.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

يجب أن تعيّن دالة `setPlayButtonAccessibleText` الخاصية `aria-label` لزر التشغيل إلى `Play` متبوعة بمسافة وعنوان الأغنية الحالية.
...
عندما تكون `userData.currentSong` تساوي `null`، يجب أن تعيّن دالة `setPlayButtonAccessibleText` الخاصية `aria-label` لزر التشغيل إلى `Play`.
