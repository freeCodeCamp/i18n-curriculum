...
Erstelle einen `.hero-title`-Selektor und gib ihm eine `text-align`-Eigenschaft mit dem Wert `center`, eine `color`-Eigenschaft mit dem Wert `orangered` und eine `font-size`-Eigenschaft mit dem Wert `8rem`.
...
Dein `.hero-title`-Selektor sollte eine `text-align`-Eigenschaft mit dem Wert `center` enthalten.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign === 'center');
```

Dein `.hero-title`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `orangered` enthalten.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color === 'orangered');
```

Dein `.hero-title`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `8rem` enthalten.
