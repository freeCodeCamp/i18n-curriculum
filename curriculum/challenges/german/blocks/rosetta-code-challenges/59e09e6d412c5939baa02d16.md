...
title: Einen Markov-Algorithmus ausführen
challengeType: 1
forumTopicId: 302260
...
Markov-Konzepte werden im maschinellen Lernen verwendet, weil sie einen einfachen Ansatz zur Datenmanipulation bieten. Mit einer festgelegten Anzahl von `rules` können Sie gegebene `data` manipulieren, um ein gewünschtes `output` zu erzeugen.

Wir haben im Hintergrund hinzugefügt:

Das `rules` in Form eines verschachtelten Arrays und das `data` ebenfalls in Form eines Arrays. Und das gewünschte `outputs`.
...
Verwenden Sie den Markov-Algorithmus, um das `data` mithilfe des für Sie bereitgestellten `outputs` in das gewünschte `rules` zu ändern.

# --hints--

`markov` sollte eine Funktion sein.
