...
Прив'яжіть перший елемент `label` до елементу `select` та надайте елементу `select` атрибут `name`.
...
Елементу `select` надайте `id`, що відповідає атрибуту `for` елемента `label`.
