...
聲明一個函數 `elemValue`，它接受一個 `num` 參數。 該函數應該爲空。

# --hints--

你應該在 `rangeFromString()` 函數之後聲明一個 `elemValue` 變量。
