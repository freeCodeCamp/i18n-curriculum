...
All'interno dell'elemento `preview-panel`, aggiungi un elemento `p` di `className` di `preview-label` e il testo `Live Preview`, un altro elemento `p` di `className` di `preview-hint` e il testo `Updates as you type`, e un `div` con la prop `className` impostata sul letterale template `preview-box tier-${getPlayerTier(player.overallRating)}`.

All'interno dell'ultimo `div`, sposta il componente `PlayerCard` e la sua prop.

# --hints--

Il tuo elemento `div.preview-panel` dovrebbe contenere un `p` con la prop `className` di `preview-label` e il testo `Live Preview`.
...
Il tuo `div.preview-panel` dovrebbe contenere un `p` con la prop `className` di `preview-hint` e il testo `Updates as you type`.
...
Il tuo `div.preview-panel` dovrebbe contenere un `div` con la prop `className` di `preview-box` e un livello dinamico `className` con prefisso `tier-`.
...
Il tuo `PlayerCard` dovrebbe essere renderizzato all'interno dell'elemento `div` con la prop `className` di `preview-box`.
