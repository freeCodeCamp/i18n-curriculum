...
Tangaza kigezo cha `cart`, na umweke thamani ya mfano mpya wa `ShoppingCart`. Angalia matumizi ya neno kuu la `new` wakati wa kutengeneza mfano huo.

# --hints--

Unapaswa kutumia `const` kutangaza kigezo cha `cart`.

```js
assert.match(code, /const\s+cart\s*=/);
```

Unapaswa kutumia neno kuu la `new` kutengeneza mfano mpya wa kitu cha `ShoppingCart`.

```js
assert.match(code, /new\s+ShoppingCart\s*\(\s*\)/);
```

Unapaswa kuweka thamani ya mfano mpya wa `ShoppingCart` katika kigezo cha `cart`.
