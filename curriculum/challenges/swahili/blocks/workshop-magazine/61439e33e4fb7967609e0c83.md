...
Baada ya vipengele vitatu vya `p` ndani ya kipengele chako cha `.text`, tengeneza kipengele cha `blockquote`. Ndani yake, ongeza kipengele cha `hr`, kipengele cha `p` chenye `class` iliyowekwa kuwa `quote`, na kipengele cha pili cha `hr`.

Mpe kipengele cha `.quote` maandishi ya `The entire curriculum should be a series of projects`.

# --hints--

Unapaswa kutengeneza kipengele kipya cha `blockquote` ndani ya kipengele chako cha `.text`.

```js
assert.exists(document.querySelector('.text blockquote'));
```

Kipengele chako cha `blockquote` kinapaswa kuja baada ya vipengele vyako vitatu vya `p`.
...
Watoto wa `blockquote` wanapaswa kuwa katika mpangilio sahihi.
...
Kipengele chako kipya cha `p` kinapaswa kuwa na `class` iliyowekwa kuwa `quote`.

```js
assert.equal(document.querySelector('.text blockquote p')?.className, 'quote');
```

Kipengele chako kipya cha `p` kinapaswa kuwa na maandishi ya `The entire curriculum should be a series of projects`.
