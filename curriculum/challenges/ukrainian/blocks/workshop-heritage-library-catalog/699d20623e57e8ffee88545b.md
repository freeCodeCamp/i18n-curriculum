...
Давайте перевіримо, чи працює пошук. У каталозі є шість книг Стівена Кінга.

Викличте `findByAuthor(catalog, "king")` і призначте результат змінній з ім’ям `kingBooks`. Виведіть у консоль `kingBooks.length`, щоб підтвердити кількість.

Потім використайте `for` цикл, щоб вивести кожен результат. Використайте шаблонний рядок, щоб включити назву та рік, наприклад `` `${kingBooks[i].title} (${kingBooks[i].year})` ``.

# --hints--

`kingBooks` має мати довжину `6`.

```js
assert.strictEqual(kingBooks.length, 6);
```

Ви маєте вивести `kingBooks.length` у консоль.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*kingBooks\.length\s*\)/);
```

Використайте `for` цикл, щоб вивести назву та рік кожної книги.
