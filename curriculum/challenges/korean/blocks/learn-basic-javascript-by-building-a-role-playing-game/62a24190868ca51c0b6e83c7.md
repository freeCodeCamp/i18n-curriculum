...
지금은 `#monsterStats` 요소를 `display` 속성으로 숨기세요. 다른 스타일은 변경하지 마세요.
...
`#monsterStats` 선택자에는 `display` 속성이 `none` 이어야 합니다.
...
`#monsterStats` 선택자에는 `border` 이 `1px solid #0a0a23` 이어야 합니다.
...
`#monsterStats` 선택자에는 `5px` 패딩이 있어야 합니다.
...
`#monsterStats` 선택자에는 `color` 이 `#ffffff` 이어야 합니다.
...
`#monsterStats` 선택자에는 `background-color` 이 `#c70d0d` 이어야 합니다.
