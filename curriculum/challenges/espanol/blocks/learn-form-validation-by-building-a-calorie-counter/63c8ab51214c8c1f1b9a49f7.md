...
En tu nueva función, declara una variable `calories` y asígnale el valor `0`. Usa `let` para declararla, ya que la reasignarás más adelante.

# --hints--

Deberías declarar una variable `calories`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

Tu variable `calories` debería tener asignado el valor `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Deberías declarar tu variable `calories` con `let`.
