...
إذا ضغطت على زر السابق عندما لا تُشغل أي أغنية، ستُشغل الأغنية الأولى. لإصلاح هذا، أضف جملة `if` في بداية دالة `playPreviousSong`. تحقق مما إذا كانت الأغنية الحالية `null` وأوقف تنفيذ الدالة باستخدام جملة `return`.

# --hints--

يجب أن تضيف جملة `if` تتحقق مما إذا كانت `userData.currentSong === null` وتوقف تنفيذ الدالة باستخدام جملة `return` داخل دالة `playPreviousSong`.
