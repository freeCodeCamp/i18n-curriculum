...
最後，在媒體查詢的 `:root` 選擇器中，重新定義所有四個 `--window-color` 變數以使用 `#777`。完成後，調整視窗大小並觀察它從白天變成夜晚。

變數主要用於顏色，而你在這裡就是這樣使用它們的。但它們可以被賦予任何值並用於任何屬性。你的專案看起來很棒！

# --hints--

你應該為你的 `--window-color1` 添加值為 `#777`。
