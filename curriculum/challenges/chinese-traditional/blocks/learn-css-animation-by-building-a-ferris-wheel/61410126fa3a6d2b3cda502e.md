...
為了讓你的 cabin 動畫看起來更像自然擺動的動作，你可以使用 `ease-in-out` 計時函式（函式（程式）、函數（數學））。此設定會讓動畫在開始和結束時速度較慢，但在週期中間移動得較快。

在 `.cabin` 選擇器中將 `linear` 取代為 `ease-in-out`。

# --hints--

你的 `.cabin` 選擇器應該有一個 `animation` 屬性設定為 `cabins 10s ease-in-out infinite`。
