...
ルディック数は素数に関連しており、素数を生成するためにエラトステネスの篩が使われるのとよく似た篩によって生成されます。

最初のルディック数は1です。

次のルディック数を生成するには、2から始まる増加する整数の配列を作成します。
...
<ul>
  <li>結果の配列の最初の要素を次のルディック数 <span style='color:blue;font-weight:bold'>2</span> とします。</li>
  <li>配列から<strong>2<sup>番目</sup></strong>のインデックスの要素をすべて削除します（最初の要素も含む）。</li>
...
<ul>
  <li>（いくつかのループを展開すると…）</li>
  <li>結果の配列の最初の要素を次のルディック数 <span style='color:blue;font-weight:bold'>3</span> とします。</li>
  <li>配列から<strong>3<sup>番目</sup></strong>のインデックスの要素をすべて削除します（最初の要素も含む）。</li>
...
<ul>
  <li>結果の配列の最初の要素を次のルディック数 <span style='color:blue;font-weight:bold'>5</span> とします。</li>
  <li>配列から<strong>5<sup>番目</sup></strong>のインデックスの要素をすべて削除します（最初の要素も含む）。</li>
...
<ul>
  <li>結果の配列の最初の要素を次のルディック数 <span style='color:blue;font-weight:bold'>7</span> とします。</li>
  <li>配列から<strong>7<sup>番目</sup></strong>のインデックスの要素をすべて削除します（最初の要素も含む）。</li>
...
<ul>
  <li><big><b> ... </b></big></li>
  <li>現在の配列の最初の要素を次のルディック数 <span style='color:blue;font-weight:bold'>L</span> とします。</li>
  <li>配列から<strong>L<sup>番目</sup></strong>のインデックスの要素をすべて削除します（最初の要素も含む）。</li>
  <li><big><b> ... </b></big></li>
</ul>

# --instructions--

与えられた数以下のすべてのルディック数を返す関数を書いてください。
