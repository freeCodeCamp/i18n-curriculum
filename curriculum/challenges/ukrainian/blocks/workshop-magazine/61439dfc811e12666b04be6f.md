...
Додайте третій `p` елемент у кінець вашого `.text` елемента та надайте йому такий текст:
...
Ви маєте створити третій `p` елемент у вашому `.text` елементі.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 3);
```

Ваш третій `p` елемент повинен містити наданий текст.
