...
باستثناء زر الدعوة إلى اتخاذ إجراء `Go Premium`، بقية المحتوى موجود داخل `div`. هذا يجعل شبكة CSS خيارًا جيدًا لترتيب كل شيء.

أضف الأصناف `grid`، و`grid-rows-[auto_auto_auto_1fr]`، و`gap-y-2` إلى الـ `div` الذي يضم عناصر `h2` و`p` و`ul`.

# --hints--

يجب أن يحتوي عنصر `div` الذي يضم عناصر `h2` و`p` و`ul` على الصنف `grid`.
...
يجب أن يحتوي عنصر `div` الذي يضم عناصر `h2` و`p` و`ul` على الصنف `grid-rows-[auto_auto_auto_1fr]`.
...
يجب أن يحتوي عنصر `div` الذي يضم عناصر `h2` و`p` و`ul` على الصنف `gap-y-2`.
