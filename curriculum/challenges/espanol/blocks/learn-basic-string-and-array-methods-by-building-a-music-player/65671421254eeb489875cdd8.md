...
Dentro de la función de callback, utiliza el método `removeAttribute()` para eliminar el atributo `"aria-current"`. Esto eliminará el atributo para cada una de las canciones.

# --hints--

No debes modificar el `forEach()` existente y su contenido.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

Debes usar el método `removeAttribute()` en `songEl` y pasar `"aria-current"`.
