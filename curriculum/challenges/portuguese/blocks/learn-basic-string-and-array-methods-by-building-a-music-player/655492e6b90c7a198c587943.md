...
Agora você precisa adicionar o atributo de volta à música que está tocando atualmente.

Crie uma declaração `if` com a condição `songToHighlight`.Para a declaração, use `setAttribute` em `songToHighlight` para passar `"aria-current"` e `"true"` como o primeiro e segundo argumentos.

# --hints--

Você deve criar uma declaração `if` com a condição `songToHighlight`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

Você deve usar o método `setAttribute()` em `songToHighlight` dentro do seu comando `if`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

Você deve passar `"aria-current"` e `"true"` como o primeiro e segundo argumento do seu método `setAttribute`.
