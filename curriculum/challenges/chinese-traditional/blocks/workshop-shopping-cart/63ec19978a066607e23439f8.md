...
現在你需要獲取兩個按鈕。 繼續該模式，獲取 `#cart-btn` 和 `#clear-cart-btn` 元素。 將它們分別存儲在名爲 `cartBtn` 和 `clearCartBtn`的變量中。

# --hints--

你應該使用 `document.getElementById()` 來獲取 `#cart-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)cart-btn\1\s*\)/);
```

你應該將 `#cart-btn` 元素分配給一個名爲 `cartBtn` 的變量。 記住使用 `const` 來聲明變量。

```js
assert.match(code, /const\s+cartBtn\s*=\s*document\.getElementById\(\s*('|"|`)cart-btn\1\s*\)/);
```

你應該使用 `document.getElementById()` 來獲取 `#clear-cart-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)clear-cart-btn\1\s*\)/);
```

你應該將 `#clear-cart-btn` 元素分配給一個名爲 `clearCartBtn` 的變量。 記住使用 `const` 來聲明變量。
