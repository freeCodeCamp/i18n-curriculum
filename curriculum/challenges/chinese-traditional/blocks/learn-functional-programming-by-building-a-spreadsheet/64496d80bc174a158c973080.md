...
使用你的 `isEven` 函式檢查 `length` 是否為偶數。如果是，傳回位於 `middle` 索引的數字與其後一個數字的平均值。如果是奇數，傳回位於 `middle` 索引的數字－你需要將 `middle` 值向上取整。
...
如果 `length` 是偶數，你應該傳回位於 `middle` 索引的數字和其後一個數字的平均值。 
...
如果 `length` 是奇數，你應該傳回 `middle` 索引處的值。 
