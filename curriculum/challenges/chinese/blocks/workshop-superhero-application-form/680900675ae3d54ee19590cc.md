...
`powerSourceOptions` 的值现在显示为 `select` 元素的选项。

现在，将 `powerSource` 用作 `select` 元素的值。然后，使用箭头函数设置一个 `onChange` 处理器，该函数以 `e` 作为参数并隐式返回 `setPowerSource(e.target.value)`。

# --hints--

你应该使用 `{powerSource}` 作为 `select` 元素的值。

```js
assert.match(code, /<\s*select\s+[\s\S]*value\s*=\s*\{\s*powerSource\s*\}/)
```

你应该使用带有隐式返回的箭头函数通过 `onChange` 设置 `select` 元素的值，以使用 `setPowerSource` 更新 `powerSource` 状态。
