...
現在你將使用 `localStorage` 為 `useEffect` 添加儲存邏輯。

在 React 中，存取或儲存 `localStorage` 被視為副作用，因此你應該使用 `useEffect` 來處理這個邏輯。

開始時，從 React 解構 `useEffect`，就像之前對 `useState` 所做的那樣。

# --hints--

你應該從 `useEffect` 解構 `React`。
