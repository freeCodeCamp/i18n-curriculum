...
將另一個對象添加到你的 `locations` 數組中。 將 `name` 設置爲 `"easter egg"`，將 `"button text"` 設置爲包含字符串 `"2"`、`"8"` 和 `"Go to town square?"` 的數組，將 `"button functions"` 設置爲包含變量 `pickTwo`、`pickEight` 和 `goTown` 的數組，並將 `text` 設置爲 `"You find a secret game. Pick a number above. Ten numbers will be randomly chosen between 0 and 10. If the number you choose matches one of the random numbers, you win!"`。

# --hints--

你的 `locations` 數組應該有 8 個值。

```js
assert.lengthOf(locations, 8);
```

你的第八個 `locations` 值應該是一個對象。

```js
assert.isObject(locations[7]);
```

你的第八個 `locations` 值應該具有一個 `name` 屬性，其值爲 `"easter egg"`。

```js
assert.equal(locations[7].name, "easter egg");
```

你的第八個 `locations` 值應該有一個 `"button text"` 數組，其中包含字符串 `"2"`、`"8"` 和 `"Go to town square?"`。

```js
assert.deepEqual(locations[7]["button text"], ["2", "8", "Go to town square?"]);
```

你的第八個 `locations` 值應該有一個 `"button functions"` 數組，其中包含變量 `pickTwo`、`pickEight` 和 `goTown`。

```js
assert.deepEqual(locations[7]["button functions"], [pickTwo, pickEight, goTown]);
```

你的第八個 `locations` 值應具有一個 `text` 屬性，其值爲 `"You find a secret game. Pick a number above. Ten numbers will be randomly chosen between 0 and 10. If the number you choose matches one of the random numbers, you win!"`。
