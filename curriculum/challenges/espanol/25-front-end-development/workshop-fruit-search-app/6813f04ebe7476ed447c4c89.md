...
A continuación, llama a `.json()` sobre la respuesta y almacena el resultado en una variable llamada `data`. Como todavía estás dentro de una función `async`, debes usar `await` antes de `response.json()`.

# --hints--

Debes llamar a `json()` sobre `response` y almacenarlo en una variable `data`.
