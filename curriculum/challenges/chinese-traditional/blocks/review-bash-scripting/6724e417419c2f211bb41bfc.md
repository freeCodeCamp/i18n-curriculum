...
- **離開狀態**：每個命令都有一個離開狀態，用來顯示成功或失敗。
  - 使用 `$?` 存取最後一個命令的離開狀態
  - 離開狀態 `0` 表示成功（true／無錯誤）
  - 任何非零離開狀態表示失敗（false／發生錯誤）
  - 常見錯誤代碼：`127`（找不到命令）、`1`（一般錯誤）
...
- **權限格式**：輸出會顯示像 `-rw-r--r--` 這樣的權限，其中：
  - 第一個字元（`-`）：檔案型別（- 代表一般檔案，d 代表目錄）
  - 接下來 9 個字元：分別代表擁有者、群組和其他人的權限
  - `r` = 讀取，`w` = 寫入，`x` = 執行
