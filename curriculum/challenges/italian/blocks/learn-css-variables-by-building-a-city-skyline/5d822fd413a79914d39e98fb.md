...
Le transizioni a gradiente spesso cambiano gradualmente da un colore all'altro. Quando serve un cambiamento più netto, la transizione può essere fatta con un'interruzione brusca come questa:
...
`.bb2b` dovrebbe avere una transizione `linear-gradient` da `--building-color2` a `0%` a `6%`, e `--window-color2` a `6%` a `9%`.
