...
Successivamente, crea un costruttore per la classe `Bug` che prenda un singolo parametro chiamato `emojiElement` di tipo `HTMLParagraphElement`. All'interno del costruttore, assegna il parametro `emojiElement` alla proprietà `emojiElement` della classe.

# --hints--

La tua classe `Bug` dovrebbe avere un costruttore.
...
Il tuo costruttore `Bug` dovrebbe avere un parametro chiamato `emojiElement` di tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Il tuo costruttore `Bug` dovrebbe assegnare `emojiElement` a `this.emojiElement`.
