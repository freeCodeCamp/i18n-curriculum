...
Use el método `classList` y `remove()` para quitar la clase `playing` del `playButton`, ya que la canción se pausará en este punto.

Para finalmente pausar la canción, use el método `pause()` en la variable `audio`. `pause()` es un método de la API de Audio de la Web para pausar archivos musicales.

# --hints--

No debe modificar la función existente `pauseSong` y su contenido.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

Debe utilizar la propiedad `classList` y el método `remove()` para quitar la clase `"playing"` del `playButton`.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

Debe usar el método `pause()` en su variable `audio`.
