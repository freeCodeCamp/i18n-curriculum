...
继续学习 `meta` 元素，`viewport` 定义告诉浏览器如何呈现页面。 包括改善移动设备上的视觉可访问性，并改进 *SEO*（搜索引擎优化）。

添加一个 `viewport` 定义，`content` 属性需详细说明页面的 `width` 和 `initial-scale`。
...
你应将 `meta` 的 `name` 属性设置为 `viewport`。

```js
assert.equal(document.querySelectorAll('head > meta[name="viewport"]')?.length, 1);
```

你应将 `meta` 的 `content` 属性设置为 `width=device-width, initial-scale=1`。
