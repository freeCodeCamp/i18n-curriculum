...
Bevor Sie damit beginnen, den nächsten und vorherigen Song abzuspielen, müssen Sie den Index jedes Songs in der `songs`-Eigenschaft von `userData` ermitteln. Dazu können Sie die `indexOf()`-Array-Methode verwenden, die den ersten Index zurückgibt, an dem ein bestimmtes Element im Array gefunden werden kann, oder `-1`, wenn das Element nicht vorhanden ist.
...
Erstellen Sie eine Funktion namens `getCurrentSongIndex`, die den Index zurückgibt, an dem `userData.currentSong` in `userData.songs` gefunden werden kann, und `-1`, wenn der aktuelle Song `null` ist.

# --hints--

Sie sollten eine Funktion namens `getCurrentSongIndex` haben.

```js
assert.isFunction(getCurrentSongIndex);
```

Ihre `getCurrentSongIndex`-Funktion sollte den Index zurückgeben, an dem `userData.currentSong` in `userData.songs` gefunden werden kann.
...
Ihre `getCurrentSongIndex`-Funktion sollte `-1` zurückgeben, wenn `userData.currentSong` `null` ist.
