...
إذا كانت قائمة التشغيل فارغة، تحتاج إلى إنشاء عنصر `resetButton` ونص له. سيظهر هذا الزر فقط إذا كانت قائمة التشغيل فارغة.

`createElement()` هي طريقة في DOM يمكنك استخدامها لإنشاء عنصر ديناميكيًا باستخدام JavaScript. لاستخدام `createElement()`، تستدعيها ثم تمرر اسم الوسم كسلسلة نصية:
...
يمكنك أيضًا تعيينها إلى متغير:

```js
const divElement = document.createElement('div')
```

داخل جملة `if` الخاصة بك، أعلن عن ثابت `resetButton`، ثم استخدم `createElement()` لإنشاء `"button"`.


# --hints--

لا يجب تعديل جملة `if` الحالية ومحتواها.

```js
assert.match(code, /if\s*\(\s*userData\?\.songs\.length\s*===\s*0\s*\)\s*\{\s*.*\s*\}/)
```

يجب استخدام طريقة `createElement` لإنشاء `"button"`.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

يجب تعيين عنصر `button` الذي أنشأته حديثًا إلى ثابت `resetButton`.
