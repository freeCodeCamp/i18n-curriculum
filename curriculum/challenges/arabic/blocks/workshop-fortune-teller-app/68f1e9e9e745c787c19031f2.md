...
داخل جملة `if`، احصل على عنصر `cardTitle` واضبط خاصية `textContent` له على `foundCard.name`. افعل الشيء نفسه لعنصر `description` بضبط خاصية `textContent` له على `foundCard.desc`.

# --hints--

يجب ضبط خاصية `textContent` لعنصر `cardTitle` على `foundCard.name`. لا تنسَ استخدام الكلمة المفتاحية `this`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

يجب ضبط خاصية `textContent` لعنصر `description` على `foundCard.desc`. لا تنسَ استخدام الكلمة المفتاحية `this`.
