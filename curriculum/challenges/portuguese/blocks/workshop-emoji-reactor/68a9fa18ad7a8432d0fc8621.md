...
Nas lições anteriores você aprendeu que `querySelectorAll` retorna um `NodeList`, que é uma estrutura semelhante a um array, e um dos métodos que um `NodeList` possui é o `forEach()`, e ele funciona da mesma forma que o `forEach` que existe em arrays.

Itere sobre `btns` com um `forEach()` que adiciona o mesmo event listener a cada botão.

Use uma função callback que receba um parâmetro `btn` e, dentro da callback, adicione um listener de evento de clique que chame `updateCount(btn)`.

Com isso você terminou o workshop.

# --hints--

Você deve usar `btns.forEach()` para iterar sobre os botões.

```js
assert.match(code, /btns\.forEach\s*\(/);
```

Todos os botões devem ter event listeners que chamem `updateCount` quando clicados.
