...
按照與按鈕文本相同的模式，將三個按鈕的 `onclick` 賦值更新爲 `"button functions"` 數組的第一個、第二個和第三個值。

# --hints--

你應該訪問 `location` 參數的 `"button functions"` 屬性的第一個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

你應該將 `button1.onclick` 屬性設置爲 `location` 參數的 `"button functions"` 屬性的第一個元素。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

你應該訪問 `location` 參數的 `"button functions"` 屬性的第二個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

你應該將 `button2.onclick` 屬性設置爲 `location` 參數的 `"button functions"` 屬性的第二個元素。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

你應該訪問 `location` 參數的 `"button functions"` 屬性的第三個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*2\s*\]/);
```

你應該將 `button3.onclick` 屬性設置爲 `location` 參數的 `"button functions"` 屬性的第三個元素。
