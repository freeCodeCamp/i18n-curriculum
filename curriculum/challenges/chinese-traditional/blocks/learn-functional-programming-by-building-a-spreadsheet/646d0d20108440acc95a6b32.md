...
在你的 `elemValue` 函式（程式）中，你明確地宣告了一個名為 `inner` 的函式（程式）並傳回它。然而，因為你使用箭頭語法，你可以隱式地傳回一個函式（程式）。例如：

```js
const curry = soup => veggies => {};
```

`curry` 是一個接受 `soup` 參數並傳回一個接受 `veggies` 參數的函式。使用此語法，更新你的 `addCharacters` 函式，使其傳回一個接受 `character2` 參數的空函式。

# --hints--

你的 `addCharacters` 函式應該使用隱喻的傳回。
...
你的 `addCharacters` 函式應該傳回一個箭頭函式，該函式具有一個 `character2` 參數。
