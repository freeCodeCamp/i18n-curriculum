...
È il momento di definire lo stile dell'ultima sezione della rivista: le immagini.

Le immagini sono racchiuse in un elemento `aside` che usa la classe `image-wrapper`, quindi crea un selettore `.image-wrapper`. Imposta la proprietà `display` su `grid`.

# --hints--

Dovresti avere un selettore `.image-wrapper`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

Il tuo selettore `.image-wrapper` dovrebbe avere una proprietà `display` impostata su `grid`.
