...
Add your `--building-color1` variable as value of the `background-color` property of `.fb4b`. 然後，從 `.fb4` 中刪除 `background-color`。

# --hints--

你應該從 `.fb4` 中刪除 `background-color`。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb4")?.backgroundColor);
```

你應該給 `.fb4b` 一個 `background-color` 爲 `--building-color1`。
