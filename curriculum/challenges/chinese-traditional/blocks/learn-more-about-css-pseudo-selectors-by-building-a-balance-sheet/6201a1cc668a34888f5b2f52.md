...
選擇 `class` 屬性爲 `data` 的 `tr` 元素。 給他們一個 `linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)` 的背景圖片。
...
你的 `tr.data` 選擇器應該將 `background-image` 屬性設置爲 `linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)`。
