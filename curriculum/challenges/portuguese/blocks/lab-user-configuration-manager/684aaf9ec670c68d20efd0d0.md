...
`add_setting({'theme': 'light'}, ('THEME': 'dark'))` deve retornar a mensagem de erro `Configuração 'theme' já existe!`Não é possível adicionar uma nova configuração com este nome.`.
...
`add_setting({'theme': 'light'}, ('volume', 'high')` deve adicionar um novo par chave-valor e retornar a mensagem de sucesso `Configuração 'volume' adicionada com valor 'high' com sucesso!`.
