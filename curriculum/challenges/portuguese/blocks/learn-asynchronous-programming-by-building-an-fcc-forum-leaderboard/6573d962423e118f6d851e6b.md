...
Em seguida, acesse o elemento `#posts-container` usando o método `getElementById()`. Atribua-o a uma nova constante chamada `postsContainer`.

# --hints--

Você deve usar `const` para declarar uma variável `postsContainer`.

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

Você deve usar `document.getElementById()` para obter o elemento `#posts-container`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

Você deve atribuir o elemento `#posts-container` à sua variável `postsContainer`.
