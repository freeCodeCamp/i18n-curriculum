...
`gender` 사전에 `constraints`라는 또 다른 키를 추가하세요. 이전 단계에서 작성한 표현식 형식을 따라 `gender`가 문자열인지 확인하세요. 그런 다음 `and` 연산자를 사용해 소문자 `gender`가 `('male', 'female')`에 있는지 검사하세요.

# --hints--

`constraints` 사전에 `gender` 키가 있어야 합니다.
...
`gender` 사전의 `constraints` 키는 `isinstance(gender, str) and gender.lower() in ('male', 'female')` 값을 가져야 합니다.
