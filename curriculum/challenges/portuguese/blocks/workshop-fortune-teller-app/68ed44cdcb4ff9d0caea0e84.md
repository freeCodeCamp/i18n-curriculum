...
Como você está fazendo chamadas `fetchCardsData` e `initializeEventListeners` dentro do construtor, agora precisa defini-las dentro da classe. Crie ambas como métodos `private`. Certifique-se de que `fetchCardsData` seja assíncrono, pois ele vai lidar com chamadas de API.

# --hints--

Você deve ter um método `private` chamado `fetchCardsData`.
...
Seu método `fetchCardsData` deve ser `async`.
...
Você deve ter um método `private` chamado `initializeEventListeners`.
