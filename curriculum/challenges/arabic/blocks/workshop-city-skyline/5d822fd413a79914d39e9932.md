...
لننتقل إلى المبنى التالي! إنه المبنى الأخضر في المقدمة. أضف له `repeating-linear-gradient` بلون المبنى الخاص بك من `0%` إلى `5%`، و`transparent` من `5%` إلى `10%`.

# --hints--

يجب أن تعطي `.fb5` خاصية `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

يجب أن تعطي `background` تدرجًا خطيًا متكررًا `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

يجب أن تعطي `repeating-linear-gradient` اللون الأول `--building-color2` من `0%` إلى `5%`.
...
يجب أن تعطي `repeating-linear-gradient` اللون الثاني `transparent` من `5%` إلى `10%`.
