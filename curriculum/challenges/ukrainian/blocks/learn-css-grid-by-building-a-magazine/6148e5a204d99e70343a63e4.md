...
Властивість `gap` — це скорочений спосіб одночасно задати значення `column-gap` і `row-gap`. Якщо вказано одне значення, воно встановлює `column-gap` і `row-gap` обидва на це значення. Якщо вказано два значення, воно встановлює `row-gap` на перше значення, а `column-gap` — на друге.

Задайте селектору `.image-wrapper` властивість `gap` зі значенням `2rem`.

# --hints--

Ваш елемент `.image-wrapper` має мати властивість `gap`, встановлену в `2rem`.
