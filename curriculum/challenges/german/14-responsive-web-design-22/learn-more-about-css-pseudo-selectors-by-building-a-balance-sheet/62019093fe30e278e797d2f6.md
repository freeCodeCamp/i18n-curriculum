...
Du solltest einen `tr[class="total"]`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"]'));
```

Dein `tr[class="total"]`-Selektor sollte eine `border-bottom`-Eigenschaft auf `4px double #0a0a23` gesetzt haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"]')?.getPropertyValue('border-bottom') === '4px double rgb(10, 10, 35)');
```

Dein `tr[class="total"]`-Selektor sollte eine `font-weight`-Eigenschaft auf `bold` gesetzt haben.
