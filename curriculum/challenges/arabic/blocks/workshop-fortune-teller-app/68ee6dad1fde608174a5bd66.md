...
داخل الطريقة `singleCardSelected`، استخرج عنصر `singleCard` وحدث خاصية `innerHTML` الخاصة به باستدعاء الدالة `renderCard`. مرر `"Click the card and reveal the fortune"` كالمعلمة الأولى، و`isReversed` كالمعلمة الثانية.

# --hints--

يجب تحديث `this.elements.singleCard.innerHTML` باستدعاء الدالة `renderCard`.
...
يجب أن يحتوي استدعاء `renderCard` على `"Click the card and reveal the fortune"` كالمعلمة الأولى.
...
يجب تمرير `isReversed` كالمعلمة الثانية إلى `renderCard`.
