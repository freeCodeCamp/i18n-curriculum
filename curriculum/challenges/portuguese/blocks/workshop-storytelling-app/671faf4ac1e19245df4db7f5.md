...
Dentro da função `displayStory`, verifique se o objeto `storyObj` possui uma propriedade que corresponda ao parâmetro `genre` e defina o conteúdo de texto do parágrafo `#result` para a história do gênero selecionado.

Além disso, defina a cor da borda do `storyContainer` para a propriedade `borderColor` do gênero selecionado.

Teste isso chamando a função `displayStory` com o gênero `scary`. A cor da borda deve ser `#ee4b2b` e a história deve ser `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin.  They enter the cabin and awaken something malevolent that had been dormant for centuries."`.

# --hints--

Você deve chamar a função `displayStory` com o gênero `"scary"`.

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

Quando você chama `displayStory("scary")`, a cor da borda deve ser `#ee4b2b`.
...
Quando você chama `displayStory("scary")`, a história exibida deve ser `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."`.
