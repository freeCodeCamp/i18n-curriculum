...
回想一下，每次循环运行时，`input` 都是 `input` 的前一个值除以 `2` 后的商，向下舍入。 最终，`input` 小于 `1`，循环停止运行。

你只需一步即可完成此操作。

在 `while` 循环内，将 `input` 设置为等于 `input` 除以 `2` 的商。 另外，请记住使用 `Math.floor()` 将商向下舍入。

# --hints--

你应该在 `while` 循环内设置 `input` 的值。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=/);
```

你应该使用 `Math.floor()` 将商向下舍入。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=\s*Math\.floor\(/);
```

你应该在 `while` 循环内将 `input` 设置为等于 `input` 除以 `2` 的商（向下舍入）。
