...
`.gallery` セレクターに `padding` プロパティを `20px 10px` に設定して、コンテナの周りにスペースを作ってください。

次に、`max-width` を `1400px` に設定し、`margin` を `0 auto` に設定して中央に配置してください。

# --hints--

`.gallery` セレクターには、`padding` プロパティを `20px 10px` に設定してください。
...
`.gallery` セレクターには、`max-width` プロパティを `1400px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.gallery')?.maxWidth === '1400px');
```

`.gallery` 要素は、`margin` を使い、`0 auto` を値として中央に配置してください。
