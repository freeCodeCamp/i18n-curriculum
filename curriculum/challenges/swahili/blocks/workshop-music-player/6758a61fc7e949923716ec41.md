...
Sasa, tumia njia ya `querySelectorAll` kulenga vipengele vya `.playlist-song` na vihifadhi katika kigezo kinachoitwa `songs`.

Kisha, fanya wito wa njia ya `forEach` kwenye `songs`. Pitia kitendakazi cha mwitiko kinachotumia `song` kama kigezo cha njia hiyo. Acha mabano ya mviringo yakiwa tupu kwa sasa.

# --hints--

Unapaswa kuwa na kigezo kinachoitwa `songs`.

```js
assert.isDefined(songs);
```

Unapaswa kuweka thamani mkusanyiko wa vipengele vyote vya `.playlist-song` katika kigezo chako cha `songs`.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

Unapaswa kufanya wito wa njia ya `forEach` kwenye `songs` na kupitisha kitendakazi cha mwitiko kinachotumia `song` kama kigezo cha njia hiyo.
