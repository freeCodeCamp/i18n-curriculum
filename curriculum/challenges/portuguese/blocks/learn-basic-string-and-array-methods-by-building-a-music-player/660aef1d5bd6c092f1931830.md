...
Assim como funções regulares, funções arrow podem retornar valores. 

Aqui está um exemplo de uma arrow function retornando o resultado da multiplicação de dois números:
...
Crie uma nova variável chamada `addTwoNumbers` e atribua a ela uma função arrow. Esta função arrow deve receber dois parâmetros chamados `num1` e `num2`. Dentro do corpo da função, retorne a expressão de soma de `num1` e `num2`.

Abaixo da função `addTwoNumbers`, adicione uma instrução de console. Dentro dessa instrução do console, chame a função `addTwoNumbers` e passe os números `3` e `4` como argumentos.
...
Sua função `addTwoNumbers` deve usar a sintaxe de função arrow.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Sua função `addTwoNumbers` deve ter dois parâmetros chamados `num1` e `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

Sua função `addTwoNumbers` deve retornar a expressão de soma de `num1` e `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

Você deve ter uma instrução `console.log(addTwoNumbers(3, 4));`.
