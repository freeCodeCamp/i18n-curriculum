...
Поверніть число всіх перестановок заданого рядка, у яких немає повторюваних підрядкових літер. Припускайте, що всі символи у заданому рядку унікальні.

Наприклад, `aab` має повернути 2, тому що у нього є 6 загальних перестановок (`aab`, `aab`, `aba`, `aba`, `baa`, `baa`), але лише 2 з них (`aba` і `aba`) не мають однакової літери (у цьому випадку `a`), що повторюється.
