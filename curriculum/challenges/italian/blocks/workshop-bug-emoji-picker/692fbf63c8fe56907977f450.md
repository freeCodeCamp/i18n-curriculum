...
Successivamente, definisci un oggetto chiamato `bugMap` di tipo `Record<string, Bug<string>>` e assegnagli per ora un oggetto vuoto.

# --hints--

Dovresti avere una costante chiamata `bugMap`.
...
Il tuo `bugMap` dovrebbe avere un'annotazione di tipo `Record<string, Bug<string>>`.
...
Il tuo `bugMap` dovrebbe essere inizializzato a un oggetto vuoto.
