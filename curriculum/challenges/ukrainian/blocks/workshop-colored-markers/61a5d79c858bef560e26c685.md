...
І як третій аргумент, вкажіть функцію `hsl` зі значеннями `240` для відтінку, `56%` для насиченості та `42%` для освітленості.

# --hints--

Ваше CSS-правило `.blue` повинне мати властивість `background` зі значенням `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))`.
