...
ジェネリック型パラメータを `Spider` に設定して、`Bug` クラスを拡張するサブクラス `string` を作成してください。

# --hints--

`Spider` という名前のクラスがあるはずです。
...
`Spider` クラスは `Bug` を拡張している必要があります。
...
`Spider` クラスは型引数に `Bug` を指定して `string` を拡張している必要があります。
