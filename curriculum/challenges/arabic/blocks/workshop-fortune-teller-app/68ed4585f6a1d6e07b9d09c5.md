...
داخل طريقة `fetchCardsData`، أعلن عن متغير باسم `response` وقم بتعيينه باستدعاء دالة `fetch` مع عنوان URL التالي: `` `${CDN_URL}/card_data.json` ``.

أضف الكلمة المفتاحية `await` قبل استدعاء `fetch`، وضمّن كل المنطق داخل كتلة `try...catch`.

# --hints--

يجب إعلان متغير `response` داخل طريقة `fetchCardsData`.
...
يجب تعيين متغير `response` الخاص بك عن طريق استدعاء `await fetch` مع عنوان URL `` `${CDN_URL}/card_data.json` ``.
...
يجب أن يكون استدعاء `fetch` الخاص بك ضمن كتلة `try...catch`، مع `error` كمعلمة للـ `catch`.
