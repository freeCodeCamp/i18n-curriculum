...
title: Einen Währungsrechner bauen
challengeType: 25
...
1. Ihre `CurrencyConverter`-Komponente sollte ein `input`-Element rendern, um den umzuwandelnden Betrag einzugeben.
2. Ihr `input`-Element sollte Zahlen akzeptieren.
3. Ihre `CurrencyConverter`-Komponente sollte zwei `select`-Elemente rendern, um die Währung auszuwählen, von der und zu der konvertiert wird.
4. Ihr `select`-Element sollte Optionen für **mindestens** `USD`, `EUR`, `GBP` und `JPY` enthalten. Sie können jeden beliebigen Wechselkurs verwenden, solange keine Eins-zu-eins-Zuordnung zwischen den Währungen besteht. Hier sind einige Beispiele für gute und schlechte Zuordnungen:
...
5. Ihre `CurrencyConverter`-Komponente sollte die Berechnung der umgerechneten Beträge für die **von**-Währung memoizieren, sodass eine Änderung der **zu**-`select`-Option die umgerechneten Beträge nicht neu berechnet.
6. Ihre `CurrencyConverter`-Komponente sollte ein Element rendern, das den umgerechneten Betrag im Format `XX.XX CCC` anzeigt, wobei `XX.XX` der umgerechnete Betrag und `CCC` der Währungscode ist.
7. Der umgerechnete Betrag sollte auf zwei Dezimalstellen gerundet sein.

# --hints--

Sie sollten eine `CurrencyConverter`-Komponente exportieren.
...
Sie sollten ein `input[type="number"]`-Element haben, um den umzuwandelnden Betrag einzugeben.
...
Sie sollten zwei `select`-Elemente haben.
...
Die `select`-Elemente sollten Optionen für mindestens `USD`, `EUR`, `GBP` und `JPY` enthalten.
...
Eine Änderung des Werts des ersten `select`-Elements sollte die umgerechneten Beträge neu berechnen.
...
Eine Änderung des Werts des zweiten `select`-Elements sollte die umgerechneten Beträge nicht neu berechnen.
...
Eine Änderung des Werts des ersten `select`-Elements sollte den neuen umgerechneten Betrag anzeigen.
...
Eine Änderung des Werts des zweiten `select`-Elements sollte den neuen umgerechneten Betrag und die Währung anzeigen.
...
Der umgerechnete Betrag sollte im Format `XX.XX CCC` angezeigt werden, wobei `XX.XX` der auf zwei Dezimalstellen gerundete umgerechnete Betrag und `CCC` der Währungscode ist.
...
Der umgerechnete Betrag sollte sich vom Eingabebetrag unterscheiden.
