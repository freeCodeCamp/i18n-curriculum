...
ボタン要素をパラメータとして受け取る `updateCount` という関数を作成してください。

この関数には、ボタンのカウントを更新するためのすべてのロジックを含めます：カウント要素の検索、現在のカウントの解析、最大値かどうかの確認、インクリメント、表示の更新です。

そして、イベントリスナーのコールバックをインラインのロジックの代わりに `updateCount(happyBtn)` を呼び出すように更新してください。

# --hints--

`updateCount` という名前の関数を作成してください。

```js
assert.isFunction(updateCount);
```

`updateCount` 関数はボタンのパラメータを受け取る必要があります。

```js
assert.equal(updateCount.length, 1);
```

`updateCount` 関数はどのボタン要素でも正しく動作するようにしてください。
...
イベントリスナーはボタンを引数として `updateCount` を呼び出す必要があります。
