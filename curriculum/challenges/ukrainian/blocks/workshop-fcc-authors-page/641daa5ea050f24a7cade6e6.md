...
Якщо ця умова виконується, вимкніть кнопку, встановивши її властивість `disabled` у значення `true`. Також встановіть `textContent` кнопки в `"No more data to load"`.
...
Ви повинні встановити властивість `disabled` у `loadMoreBtn` в значення `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Ви повинні встановити `textContent` у `loadMoreBtn` в значення `"No more data to load"`.
