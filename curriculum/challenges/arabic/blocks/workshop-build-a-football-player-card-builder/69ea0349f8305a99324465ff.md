...
في كتلة `try`، عرّف متغيرًا باسم `saved`. للقيمة، استخدم الطريقة `getItem()` لاسترجاع `STORAGE_KEY`. أسفل هذا المتغير مباشرة، أنشئ جملة `if` بشرط `saved`. اترك كتلة `if` فارغة في الوقت الحالي.

# --hints--

يجب أن تستدعي دالة `loadPlayer` الخاص بك `localStorage.getItem` مع `STORAGE_KEY` داخل كتلة `try`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

يجب تخزين نتيجة `localStorage.getItem` في متغير يسمى `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

يجب أن تحتوي كتلة `try` على جملة `if` بشرط `saved`.
