...
Erstellen Sie eine letzte `@media` Abfrage für `only screen` mit einer `max-width` von `420px`. Erstellen Sie darin einen `.hero-title` Selektor mit einer `font-size` Eigenschaft, die auf `4.5rem` gesetzt ist.

Herzlichen Glückwunsch! Ihr Magazin ist nun fertiggestellt.

# --hints--

Sie sollten eine neue `@media` Abfrage für `only screen and (max-width: 420px)` haben. Dies sollte die letzte Abfrage in der `@media` Abfrageliste sein.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

Ihre neue `@media` Abfrage sollte einen `.hero-title` Selektor enthalten.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

Ihr `.hero-title` Selektor sollte eine `font-size` Eigenschaft haben, die auf `4.5rem` gesetzt ist.
