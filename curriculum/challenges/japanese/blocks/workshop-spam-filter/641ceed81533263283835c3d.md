...
`.match()` メソッドの代わりに、正規表現の `.test()` メソッドを使って文字列がパターンに一致するかどうかをテストできます。`.match()` と違い、`.test()` は文字列がパターンに一致するかどうかを示すブール値を返します。

`isSpam()` 関数を更新して、`.test()` の `helpRegex` メソッドを使い、`msg` が一致するかどうかをテストしてください。

その後、ページでいくつかメッセージを入力して結果を確認してください。

# --hints--

`isSpam()` 関数は `.test()` メソッドを使うべきです。
...
`isSpam()` 関数は `.test()` の `helpRegex` メソッドを `msg` を引数にして呼び出すべきです。
...
`isSpam()` 関数は `helpRegex.test(msg)` の結果を返すべきです。
