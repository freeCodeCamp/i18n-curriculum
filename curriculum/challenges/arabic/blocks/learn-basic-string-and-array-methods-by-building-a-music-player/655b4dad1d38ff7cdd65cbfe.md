...
بعد إعداد كل شيء، استدعِ الدوال `pauseSong()`، و`setPlayerDisplay()`، و`highlightCurrentSong()`، و`setPlayButtonAccessibleText()` لتحديث المشغل بشكل صحيح.

تهانينا على إكمال مشغل الموسيقى الخاص بك! بعد أن انتهينا من اختبار واستخدام الأغاني القصيرة، استبدلناها بالمقاطع الأصلية التي اختارها Quincy خصيصًا لتستمتع بها.

# --hints--

يجب استدعاء الدالة `pauseSong`.
...
يجب استدعاء الدالة `setPlayerDisplay`.
...
يجب استدعاء الدالة `highlightCurrentSong`.
...
يجب استدعاء الدالة `setPlayButtonAccessibleText`.
