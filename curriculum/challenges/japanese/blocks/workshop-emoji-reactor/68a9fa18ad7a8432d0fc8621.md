...
前のレッスンで、`querySelectorAll` は配列のような構造である `NodeList` を返し、`NodeList` が持つメソッドの一つに `forEach()` があり、これは配列にある `forEach` と同じように動作することを学びました。

`btns` を `forEach()` で繰り返し処理し、各ボタンに同じイベントリスナーを追加してください。

パラメータに `btn` を取るコールバック関数を使い、その中でクリックイベントリスナーを追加し、`updateCount(btn)` を呼び出してください。

これでワークショップは完了です。

# --hints--

ボタンを繰り返すには `btns.forEach()` を使うべきです。

```js
assert.match(code, /btns\.forEach\s*\(/);
```

すべてのボタンにはクリックされたときに `updateCount` を呼び出すイベントリスナーが必要です。
