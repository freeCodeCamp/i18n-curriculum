...
Agora, crie uma função chamada `getPreviousSong`.Faça com que retorne a música que pode ser encontrada em `getCurrentSongIndex()` menos um em `userData.songs`, ou `undefined` se a música atual for a primeira na playlist.

# --hints--

Você deve ter uma função chamada `getPreviousSong`.

```js
assert.isFunction(getPreviousSong);
```

Sua função `getPreviousSong` deve chamar `getCurrentSongIndex`.
...
Sua função `getPreviousSong` deve retornar a música anterior à música atual.
...
Sua função `getPreviousSong` deve retornar `undefined` quando a música atual for a primeira da playlist.
