...
Le ombre per i tuoi marker verde e blu avranno la stessa posizione, sfocatura e diffusione. L'unica differenza sarà il colore.
...
La tua regola CSS `.green` dovrebbe avere una proprietà abbreviata `box-shadow` con il valore `0 0 20px 0 green`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.green')?.boxShadow, 'green 0px 0px 20px 0px');
```

La tua regola CSS `.blue` dovrebbe avere una proprietà abbreviata `box-shadow` con il valore `0 0 20px 0 blue`.
