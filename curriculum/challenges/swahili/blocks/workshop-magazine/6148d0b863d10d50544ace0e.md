...
Maandishi mengine yamehamishwa kutoka mahali pake. Yahamishie mahali kwa kumpa kichaguzi cha `.first-paragraph::first-letter` sifa ya `float` iliyowekwa kuwa `left` na sifa ya `margin-right` iliyowekwa kuwa `1rem`.

# --hints--

Kichaguzi chako cha `.first-paragraph::first-letter` kinapaswa kuwa na sifa ya `float` iliyowekwa kuwa `left`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

Kichaguzi chako cha `.first-paragraph::first-letter` kinapaswa kuwa na sifa ya `margin-right` iliyowekwa kuwa `1rem`.
