...
L'altro testo è stato spostato fuori posto. Riportalo in posizione assegnando al selettore `.first-paragraph::first-letter` una proprietà `float` impostata su `left` e una proprietà `margin-right` impostata su `1rem`.

# --hints--

Il tuo selettore `.first-paragraph::first-letter` dovrebbe avere una proprietà `float` impostata su `left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float === 'left');
```

Il tuo selettore `.first-paragraph::first-letter` dovrebbe avere una proprietà `margin-right` impostata su `1rem`.
