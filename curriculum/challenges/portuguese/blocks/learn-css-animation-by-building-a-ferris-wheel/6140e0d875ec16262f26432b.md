...
A propriedade `animation-iteration-count` define quantas vezes sua animação deve repetir.Isso pode ser definido como um número, ou como `infinite` para repetir indefinidamente a animação.Sua roda-gigante nunca deve parar, então defina o seletor de `.wheel` para ter um `animation-iteration-count` definido como `infinite`.
