...
أول `fieldset` سيحتوي على حقول الاسم والبريد الإلكتروني وكلمة المرور. ابدأ بإضافة أربعة عناصر `label` إلى أول `fieldset`.
...
يجب أن تكون عناصر `fieldset` جميعها أبناء مباشرين لعنصر الـ `form`.

```js
assert.equal(document.querySelectorAll('form > fieldset')?.length, 3);
```

يجب أن تكون العناصر الأربعة `label` جميعها أبناء مباشرين لأول `fieldset`. تأكد من إغلاق عناصر `label`.
