...
现在，是时候自定义单选按钮了。默认情况下，单选按钮具有本地的浏览器外观。你需要移除它，以便你可以创建自定义外观。

选择类为 `contact-method-radio-btn` 的元素。将 `appearance` 设置为 `none`。

# --hints--

你应该定位具有类 `contact-method-radio-btn` 的元素。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

将 `appearance` 属性设置为 `none`。
