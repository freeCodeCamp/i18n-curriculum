...
Restituisci il numero totale di permutazioni della stringa fornita che non contengono lettere consecutive ripetute. Assumi che tutti i caratteri nella stringa fornita siano unici.

Ad esempio, `aab` dovrebbe restituire 2 perché ha 6 permutazioni totali (`aab`, `aab`, `aba`, `aba`, `baa`, `baa`), ma solo 2 di queste (`aba` e `aba`) non hanno la stessa lettera (in questo caso `a`) ripetuta.
