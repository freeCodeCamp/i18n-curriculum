...
La tua rivista avrà tre sezioni principali. Hai già impostato il layout generale nella regola `main`, ma puoi regolare la posizione nelle regole figlio.

Un'opzione è la proprietà `grid-column`, che è una scorciatoia per `grid-column-start` e `grid-column-end`. La proprietà `grid-column` indica all'elemento griglia da quale linea della griglia iniziare e a quale terminare.

Crea una regola `.heading` e imposta la proprietà `grid-column` su `2 / 3`. Questo dirà all'elemento `.heading` di iniziare alla linea della griglia 2 e terminare alla linea della griglia 3.

# --hints--

Dovresti avere un selettore `.heading`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.heading'));
```

Il tuo selettore `.heading` dovrebbe avere una proprietà `grid-column` impostata su `2 / 3`.
