...
Devi sapere per quanti segni di graduazione deve girare la simulazione. Ricorda che:

- Ogni gate ha un array `queue` che mostra quanti partecipanti arrivano a ogni segno di graduazione.  
- Poiché tutti i gate hanno lo stesso numero di segni di graduazione, puoi semplicemente usare la lunghezza della coda del primo gate.  

Crea una variabile chiamata `maxTicks` e assegnale il valore di `gates[0].queue.length`.

Questo valore servirà a controllare il loop della simulazione, assicurando che ogni segno di graduazione venga processato per tutti i gate. 

# --hints--

Devi creare una variabile chiamata `maxTicks` e assegnarle il valore di `gates[0].queue.length`.
