...
- **remove():** Usado para eliminar un elemento de la lista. El método `remove()` solo eliminará la primera ocurrencia de un elemento en la lista:
...
- **Bucle `for`**: Se usa para iterar sobre una secuencia (como una lista, tupla o cadena) y ejecutar un bloque de código para cada elemento en esa secuencia. Aquí tienes un ejemplo de cómo usar un bucle `for` para recorrer una lista e imprimir cada lenguaje en la consola:
