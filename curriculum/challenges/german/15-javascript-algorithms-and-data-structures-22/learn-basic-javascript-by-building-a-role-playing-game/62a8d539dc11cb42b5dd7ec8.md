...
Verwende die `shift()`-Methode, um das erste Element aus dem `inventory`-Array zu nehmen und es deiner `currentWeapon`-Variablen zuzuweisen.

# --hints--

Deine `sellWeapon`-Funktion sollte die `shift()`-Methode verwenden.

```js
assert.match(sellWeapon.toString(), /shift\(\s*\)/);
```

Du solltest die `shift()`-Methode für das `inventory`-Array verwenden.

```js
assert.match(sellWeapon.toString(), /inventory\.shift\(\s*\)/);
```

Du solltest deiner `currentWeapon`-Variable den Wert von `inventory.shift()` zuweisen.
