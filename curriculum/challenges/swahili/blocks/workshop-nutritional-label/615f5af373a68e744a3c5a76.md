...
Tengeneza kichaguzi cha `.calories-info span`, weka sifa yake ya `font-size` kuwa `2.4em` na `font-weight` kuwa `700`.

# --hints--

Unapaswa kutengeneza kichaguzi cha `.calories-info span`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span'));
```

Kichaguzi chako cha `.calories-info span` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `2.4em`.
