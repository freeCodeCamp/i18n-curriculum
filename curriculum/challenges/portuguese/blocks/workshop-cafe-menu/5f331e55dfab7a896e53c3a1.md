---
id: 5f331e55dfab7a896e53c3a1
title: Passo 3
challengeType: 0
dashedName: step-3
---

# --description--

Dentro do elemento `head`, aninhe um elemento `meta` com um atributo chamado `charset` definido com o valor `utf-8`.Isso informa ao navegador como codificar caracteres para a página.

# --hints--

Você deve criar uma tag `meta`.

```js
assert.match(code, /<meta.*?\/?>/is);
```

O elemento `meta` é um elemento vazio, não deve ter uma tag de fechamento `</meta>`.

```js
assert.notMatch(code, /<\/meta>/i);
```

A tag `meta` deve ter um atributo `charset`.

```js
assert.match(code, /<meta\s+charset\s*/i);
```

Seu atributo `charset` deve ter um valor de `utf-8`.

```js
assert.match(code, /charset\s*=\s*('|")utf-8\1/i);
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
--fcc-editable-region--
  <head>
    <title>Cafe Menu</title>
  </head>
--fcc-editable-region--
</html>
```
