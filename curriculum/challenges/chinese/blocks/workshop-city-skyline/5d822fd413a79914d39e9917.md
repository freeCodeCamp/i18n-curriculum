...
在已有的 `repeating-linear-gradient` 下面给 `.fb1c` 再添加一个； 从 `0%` 到 `10%` 使用 `--building-color4`，从 `10%` 到 `90%` 使用 `--window-color4`。 这将在你之前添加的渐变后面。

# --hints--

你不应更改第一个 `repeating-linear-gradient`。
...
你应该在第一个 `repeating-linear-gradient` 后面加一个逗号（`,`）。
...
你应该添加一个 `repeating-linear-gradient`，从 `0%` 到 `10%` 处为第一个颜色 `--building-color4`。
...
你应该在从 `10%` 到 `90%` 处添加第二种颜色 `--window-color4`。
