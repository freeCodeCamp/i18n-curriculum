...
두 3차원 벡터 \\( \mathbf{a} \\)와 \\( \mathbf{b} \\) 사이의 벡터곱은 다음과 같이 계산할 수 있습니다:
...
위 공식을 구현하여 두 3차원 벡터 사이의 벡터곱을 계산하고 `cross()` 메서드에서 결과 벡터를 반환하세요.

# --hints--

`cross()` 메서드는 벡터곱 계산 결과로 새로운 `R3Vector` 인스턴스를 반환해야 합니다.
