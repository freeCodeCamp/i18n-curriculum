...
Перед вашим циклом `for` створіть інструкцію `if`, яка додає рядок `=== MOVIES ===` (що закінчується символом нового рядка) до поточного значення `result`, коли список `movies` не порожній.

# --hints--

У вашому методі `if` має бути інструкція `__str__`, яка перевіряє, чи список `movies` не порожній.
...
У вашій інструкції `result += '=== MOVIES ===\n'` має бути `if`.
