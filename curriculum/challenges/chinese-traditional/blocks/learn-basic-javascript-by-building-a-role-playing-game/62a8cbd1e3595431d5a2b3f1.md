...
你應該告訴玩家他們買了什麼武器。在你剛寫的兩行之間，使用 `let` 初始化一個名為 `newWeapon` 的新變數。將它設定為相等於 `weapons`。

# --hints--

你的 `buyWeapon` 函式應該有一個名為 `newWeapon` 的變數。

```js
assert.match(buyWeapon.toString(), /newWeapon/);
```

你應該使用 `let` 來宣告 `newWeapon`。

```js
assert.match(code, /let\s+newWeapon/);
```

`newWeapon` 應該初始化為具有 `weapons` 的值。別忘了你的分號。

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

在你修改 `text` 之前，應該先宣告 `newWeapon`。
