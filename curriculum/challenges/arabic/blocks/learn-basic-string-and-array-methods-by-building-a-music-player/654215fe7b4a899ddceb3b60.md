...
الآن `songsHTML` هي مصفوفة. إذا حاولت عرضها كما هي، سترى الأغاني مفصولة بفواصل. هذا ليس النتيجة المطلوبة لأنك تريد عرض الأغاني كقائمة. لإصلاح ذلك، تحتاج إلى دمج المصفوفة في سلسلة نصية واحدة باستخدام الطريقة <dfn>join()</dfn>.

تُستخدم الطريقة `join()` لربط كل عناصر المصفوفة في سلسلة نصية واحدة. تأخذ معلمة اختيارية تسمى `separator` تُستخدم لفصل كل عنصر من عناصر المصفوفة. على سبيل المثال:
...
اربط الطريقة `join()` بطريقة `map()` الخاصة بك ومرر سلسلة فارغة كفاصل.

لربط عدة طرق معًا، يمكنك استدعاء الطريقة `join()` على نتيجة طريقة `map()`. على سبيل المثال:
...
يجب أن تضيف `join("")` إلى الشفرة الموجودة.
