...
由於你從玩家身上減去了生命值，你需要檢查玩家的 `health` 是否小於或等於 `0`。 如果是的話，調用 `lose` 函數。

# --hints--

你的 `pick` 函數應該在 `else` 代碼塊中嵌套一個 `if` 語句。
...
你的 `if` 語句應該檢查玩家的 `health` 是否小於或等於 `0`。

```js
assert.match(pick.toString(), /if\s*\(\s*health\s*<=\s*0\s*\)/);
```

你在 `else` 代碼塊中的 `if` 語句應該調用 `lose` 函數。
