...
**Ziel:** Erstellen Sie eine App, die funktional ähnlich ist wie <a href="https://technical-documentation-page.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://technical-documentation-page.freecodecamp.rocks</a>. **Kopieren Sie dieses Demo-Projekt nicht**.
...
1. Jedes Element mit der Klasse `nav-link` sollte Text enthalten, der dem `header`-Text innerhalb jedes `section` entspricht (z. B. wenn Sie einen Abschnitt/Kopfzeile „Hello world“ haben, sollte Ihre Navigationsleiste ein Element mit dem Text „Hello world“ enthalten)
1. Wenn Sie auf ein Element in der Navigationsleiste klicken, sollte die Seite zum entsprechenden Abschnitt des `#main-doc`-Elements navigieren (z. B. wenn Sie auf ein `.nav-link`-Element klicken, das den Text „Hello world“ enthält, navigiert die Seite zu einem `section`-Element mit dieser ID und enthält die entsprechende Kopfzeile)
1. Auf Geräten mit normaler Größe (Laptops, Desktops) sollte das Element mit `id="navbar"` auf der linken Seite des Bildschirms angezeigt werden und für den Benutzer immer sichtbar sein
...
**Hinweis:** Fügen Sie unbedingt `<link rel="stylesheet" href="styles.css">` in Ihrem HTML hinzu, um Ihr Stylesheet zu verlinken und Ihr CSS anzuwenden
...
Jedes `.nav-link` sollte Text enthalten, der dem `header`-Text seines zugehörigen `section` entspricht (z. B. wenn Sie einen Abschnitt/Kopfzeile „Hello world“ haben, sollte Ihr `#navbar` ein `.nav-link` mit dem Text „Hello world“ enthalten).
...
Jedes `.nav-link` sollte ein `href`-Attribut haben, das auf sein entsprechendes `.main-section` verweist (z. B. wenn Sie auf ein `.nav-link`-Element klicken, das den Text „Hello world“ enthält, navigiert die Seite zu einem `section`-Element mit dieser ID).
...
Ihr Projekt zur technischen Dokumentation sollte mindestens eine Media Query verwenden.
