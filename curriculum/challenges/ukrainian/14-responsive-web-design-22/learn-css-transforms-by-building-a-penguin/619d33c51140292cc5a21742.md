...
Ви повинні надати `.arm.left` властивість `animation-name` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationName, 'wave');
```

Ви повинні надати `.arm.left` властивість `animation-duration` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationDuration, '3s');
```

Ви повинні надати `.arm.left` властивість `animation-iteration-count` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationIterationCount, 'infinite');
```

Ви повинні надати `.arm.left` властивість `animation-timing-function` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.
