...
将 `.chin` 元素放置在其父元素顶部 `25%` 和左侧 `5%` 的位置。 然后，将顶角的半径设置为 `70%` ，将底角的半径设置为 `100%`。
