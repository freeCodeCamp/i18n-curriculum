...
`conversionTable` に `ounce` プロパティを追加し、そのプロパティに `cup` プロパティが `0.125` に設定され、`gram` プロパティが `28.35` に設定され、`teaspoon` プロパティが `6` に設定されたオブジェクトを代入してください。

# --hints--

`conversionTable` オブジェクトは `ounce` プロパティを持つ必要があります。

```js
assert.property(conversionTable, "ounce");
```

`ounce` プロパティはオブジェクトである必要があります。

```js
assert.isObject(conversionTable.ounce);
```

`ounce` オブジェクトは `cup` プロパティを持つ必要があります。

```js
assert.property(conversionTable.ounce, "cup");
```

`cup` プロパティは `0.125` に設定してください。

```js
assert.strictEqual(conversionTable.ounce?.cup, 0.125);
```

`ounce` オブジェクトは `gram` プロパティを持つ必要があります。

```js
assert.property(conversionTable.ounce, "gram");
```

`gram` プロパティは `28.35` に設定してください。

```js
assert.strictEqual(conversionTable.ounce?.gram, 28.35);
```

`ounce` オブジェクトは `teaspoon` プロパティを持つ必要があります。

```js
assert.property(conversionTable.ounce, "teaspoon");
```

`teaspoon` プロパティは `6` に設定してください。
