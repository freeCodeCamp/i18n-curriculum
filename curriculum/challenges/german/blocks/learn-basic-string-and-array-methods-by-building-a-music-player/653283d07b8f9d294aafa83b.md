...
Es ist Zeit, die Funktionalität zum Abspielen der angezeigten Songs zu implementieren.

Definieren Sie eine `playSong`-Funktion mit `const`. Die Funktion soll einen `id`-Parameter erhalten, der die eindeutige Kennung des Songs darstellt, den Sie abspielen möchten.

# --hints--

Sie sollten `const` verwenden, um eine leere Funktion mit dem Namen `playSong` zu erstellen.

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

`playSong` sollte eine Funktion sein.

```js
assert.isFunction(playSong)
```

Ihre `playSong`-Funktion sollte eine Pfeilfunktion sein.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Ihre `playSong`-Funktion sollte einen `id`-Parameter erhalten.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

Ihre `playSong`-Funktion sollte leer sein.
