...
配列がある要素を含んでいるかを確認するための `.includes()` メソッドは、`true` または `false` を返します。

こちらが `.includes()` 構文の使用例です。
...
`for` ループの後に、配列 `numbers` に `guess` が含まれているかを確認するための `if` 文を追加しましょう。 `.includes()` メソッドを使って、配列が `guess` を含んでいるかチェックすることができます。

# --hints--

`pick` 関数に `if` 文を持たせてください。
