...
接下来，你需要在播放器显示屏上显示当前歌曲标题和艺术家。 使用 `const` 和箭头语法创建一个空的 `setPlayerDisplay` 函数。

# --hints--

你应该使用 `const` 创建一个名为 `setPlayerDisplay` 的空函数。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` 应该是一个函数。

```js
assert.isFunction(setPlayerDisplay)
```

你的 `setPlayerDisplay` 函数应该使用箭头语法。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `setPlayerDisplay` 函数应该是空的。
