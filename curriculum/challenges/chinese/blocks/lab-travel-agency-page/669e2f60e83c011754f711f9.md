...
1. 三个锚元素应该分别包含一个带有适当描述的 `img` 属性和设置为有效的图像的 `alt` 属性的 `src` 元素。 如果愿意，可以使用 `https://cdn.freecodecamp.org/curriculum/labs/colosseo.jpg`、`https://cdn.freecodecamp.org/curriculum/labs/alps.jpg` 和 `https://cdn.freecodecamp.org/curriculum/labs/sea.jpg`。
