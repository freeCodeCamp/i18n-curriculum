...
Your emphasis `em` element should have an opening tag. Opening tags تكتب هكذا: `<elementName>`.

```js
assert.exists(document.querySelector('em'));
```

Your emphasis `em` element should have a closing tag. Closing tags يجب ان تتبع بـ `/` مباشرة بعد رمز `<`.
...
Your emphasis `em` element should surround the text `love`. إما أنك حذفت النص أو لديك خطأ إملائي.
