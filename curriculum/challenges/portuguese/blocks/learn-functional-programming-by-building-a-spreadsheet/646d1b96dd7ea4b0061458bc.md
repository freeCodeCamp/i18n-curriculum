...
Você pode passar uma <dfn>referência</dfn> de função como um parâmetro da função de callback.Uma referência de função é um nome de função sem os parênteses.Por exemplo:
...
O método `.map()`, aqui, chamará a função `myFunc`, passando os mesmos argumentos que uma função de callback de `.map()` receberia.O primeiro argumento é o valor do array na iteração atual, então `newArray` seria `[value: 1, value: 2, value: 3]`.

Passe uma referência à função `elemValue` como a função de callback do método `.map()`.

# --hints--

Você não deve chamar a função `elemValue`.
...
Você deve passar uma referência à função `elemValue` como a função de callback de `.map()`.
