...
Елемент `hr` можна використовувати для відтворення дільника між розділами різного вмісту.
...
Спочатку додайте елемент `hr` між елементом `p` із класом `established` та першим елементом `section`.
...
Елемент `hr` є порожнім елементом, він не повинен мати кінцевий тег `</hr>`.

```js
assert.notMatch(code, /<\/hr>/i);
```

Не змінюйте наявний елемент `p` з класом `established`.
...
Елемент `hr` має бути між елементами `p` та `section`.
