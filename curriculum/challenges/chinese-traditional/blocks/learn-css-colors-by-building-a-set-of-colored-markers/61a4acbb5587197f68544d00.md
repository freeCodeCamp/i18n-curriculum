...
最後，修改 `linear-gradient` 函式中的第三個顏色引數，目前是純藍色。

更新 `rgb` 函式（程式），使紅色的值為 `162`，綠色的值為 `27`，藍色的值為 `27`。

# --hints--

你的 `.red` CSS 規則應該有一個 `background` 屬性，設定為 `linear-gradient(180deg, rgb(122, 74, 14) 0%, rgb(245, 62, 113) 50%, rgb(162, 27, 27) 100%)`。
