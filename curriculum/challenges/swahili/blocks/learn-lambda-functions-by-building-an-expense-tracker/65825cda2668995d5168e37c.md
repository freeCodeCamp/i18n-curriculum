...
Tengeneza kifungu kingine cha `elif` kinachokagua kama `choice == '4'`. Ndani ya `elif` mpya, tengeneza kigezo `category` na weka thamani ya `input('Enter category to filter: ')` ili kuchuja aina ya matumizi.

# --hints--

Unapaswa kutengeneza kifungu cha `elif` kinachokagua kama `choice == '4'`.
...
Unapaswa kutangaza kigezo kinachoitwa `category` katika kifungu chako kipya cha `elif`.
...
Unapaswa kuweka thamani ya `input('Enter category to filter: ')` kwa kigezo chako cha `category`.
