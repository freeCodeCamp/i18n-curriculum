...
Passiamo all’edificio successivo! Crea una nuova variabile di proprietà chiamata `--window-color3` in `:root` e assegnale un valore di `#d98cb3`. Questo sarà il colore secondario per gli edifici rosa.
...
Dovresti assegnare a `--window-color3` un valore di `#d98cb3`.
