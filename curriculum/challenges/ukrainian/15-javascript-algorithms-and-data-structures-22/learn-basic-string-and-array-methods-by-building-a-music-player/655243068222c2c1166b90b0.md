...
В межах функції `playNextSong` створіть інструкцію `if`, щоб перевірити, чи `currentSong` з `userData` суворо дорівнює `null`. Це перевірить, чи не відтворюється поточна пісня в об’єкті `userData`.

Якщо умова вірна, викличте функцію `playSong` з `id` першої пісні в масиві `userData?.songs` як аргумент.

# --hints--

Не змінюйте наявну функцію `playNextSong` та її вміст.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

Ви повинні створити інструкцію `if` з умовою `userData?.currentSong === null`.
...
Викличте функцію `playSong` з `userData?.songs[0].id` всередині інструкції `if`.
