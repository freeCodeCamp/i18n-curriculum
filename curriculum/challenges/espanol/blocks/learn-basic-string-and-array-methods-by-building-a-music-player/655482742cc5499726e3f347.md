...
Dentro de la función `playPreviousSong`, agrega una sentencia `if` con una condición de `userData?.currentSong === null`. Esto verificará si actualmente no hay ninguna canción reproduciéndose. Si no hay ninguna, sal de la función usando un `return`.

Dentro del bloque `else`, crea una constante llamada `currentSongIndex` y asígnale `getCurrentSongIndex()`.

# --hints--

No debes alterar la función existente `playPreviousSong` y su contenido.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Deberías tener una declaración `if` con la condición `userData?.currentSong === null` dentro de tu función `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

Deberías tener `return` dentro del bloque de tu declaración `if`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

Tu declaración `if` debe tener un bloque `else`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

Debes llamar a `getCurrentSongIndex` y asignarlo a `currentSongIndex` dentro del bloque `else` de tu declaración `if`.
