...
Вкладіть один елемент `textarea` в межах другого елементу `div.answer` та вкажіть кількість рядків та стовпців.

# --hints--

Вкладіть один елемент `textarea` в межах другого елемента `div.answer`.

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

Надайте елементу `textarea` атрибут `rows` з числом.
...
Надайте елементу `textarea` атрибут `cols` з числом.
