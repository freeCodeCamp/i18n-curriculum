...
你添加的图像没有像上面的 `Coffee` 标题那样水平居中。`img` 元素“类似”内联元素。

要使图像的行为像标题一样的块级元素，请创建一个 `img` 类型选择器。 将 `display` 属性设置为 `block`，并使用 `margin-left` 和 `margin-right` 属性将其水平居中。
...
你的 `img` 元素的 `margin-left` 和 `margin-right` 的值应为 `auto`。
