...
Primero, crea una copia superficial del arreglo de entrada `crew` usando `slice()` y asígnala a una variable llamada `sorted`.

# --hints--

Debes usar el método `slice()` para copiar el arreglo de entrada `crew` en una nueva variable llamada `sorted`.
