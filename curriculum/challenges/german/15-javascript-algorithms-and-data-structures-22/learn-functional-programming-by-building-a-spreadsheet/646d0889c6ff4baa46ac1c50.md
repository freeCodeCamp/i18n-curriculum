...
Deklariere in deiner `elemValue`-Funktion eine Funktion namens `inner`, die einen `character`-Parameter annimmt.

Gib dann die `inner`-Funktion zurück.

# --hints--

Du solltest eine `inner`-Variable deklarieren.
...
Du solltest `const` verwenden, um deine `inner`-Variable zu deklarieren.
...
Deine `inner`-Variable sollte eine Arrow-Funktion sein.
...
Deine `inner`-Funktion sollte `character` als einzigen Parameter haben.
...
Deine `inner`-Funktion sollte leer sein.
...
Du solltest explizit deine `inner`-Funktion zurückgeben.
