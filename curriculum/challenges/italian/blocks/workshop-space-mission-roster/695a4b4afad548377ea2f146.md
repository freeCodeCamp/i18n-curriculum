...
Usa un loop `for` per registrare il `name` di ogni astronauta nell'array `sorted`.

# --hints--

Dovresti usare un loop `for` per iterare su l'array `sorted` e registrare il `name` di ogni astronauta.
