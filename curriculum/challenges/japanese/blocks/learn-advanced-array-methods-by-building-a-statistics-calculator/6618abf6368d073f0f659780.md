...
奇数リストと偶数リストの中央値の見つけ方がよりよく理解できたので、前のステップで書いたテストコードはすべて削除してください。


# --hints--

コードに`const testArr1 = [1, 2, 3, 4, 5];`が含まれていないことを確認してください。

```js
assert.notMatch(code, /const testArr1 = \[1, 2, 3, 4, 5\];/);
```

コードに`const testArr2 = [1, 2, 3, 4, 5, 6];`が含まれていないことを確認してください。

```js
assert.notMatch(code, /const testArr2 = \[1, 2, 3, 4, 5, 6\];/);
```

コードに`const isEven = testArr2.length % 2 === 0;`が含まれていないことを確認してください。

```js
assert.notMatch(code, /const isEven = testArr2.length % 2 === 0;/);
```

コードに`console.log(isEven);`が含まれていないことを確認してください。

```js
assert.notMatch(code, /console\.log\(isEven\);/);
```

コードに`const oddListMedian = testArr1[Math.floor(testArr1.length / 2)];`が含まれていないことを確認してください。

```js
assert.notMatch(code, /const oddListMedian = testArr1\[Math\.floor\(testArr1\.length \/ 2\)\];/);
```

コードに`console.log(oddListMedian);`が含まれていないことを確認してください。

```js
assert.notMatch(code, /console\.log\(oddListMedian\);/);
```

コードに`const evenListMedian = getMean([testArr2[testArr2.length / 2 - 1], testArr2[testArr2.length / 2]]);`が含まれていないことを確認してください。

```js
assert.notMatch(code, /const evenListMedian = getMean\(\[testArr2\[testArr2\.length \/ 2 - 1\], testArr2\[testArr2\.length \/ 2\]\]\);/);
```

コードに`console.log(evenListMedian);`が含まれていないことを確認してください。
