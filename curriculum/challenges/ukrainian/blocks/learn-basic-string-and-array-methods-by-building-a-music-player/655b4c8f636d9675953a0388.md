...
Якщо у плейлисті немає наступної пісні, використайте блок `else`, щоб скинути ключ `currentSong` об’єкта `userData` до `null`, а його властивість `songCurrentTime` до `0`.
...
Ваша інструкція `if` має містити блок `else`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Ви повинні використати точкову нотацію, щоб встановити властивість `currentSong` об’єкта `userData` у значення `null` у блоці `else` вашої інструкції `if`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

Ви повинні використати точкову нотацію, щоб встановити властивість `songCurrentTime` об’єкта `userData` у значення `0` у блоці `else` вашої інструкції `if`.
