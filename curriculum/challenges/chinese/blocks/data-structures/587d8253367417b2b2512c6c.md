...
在本练习中，我们将对两组数据执行联合。 我们将在我们的 `Set` 数据结构上创建一个名为 `union` 的方法。 此方法应将另一个 `Set` 作为参数，并返回两个集合的 `union` 集，不包括任何重复值。
...
包含值 `["a", "b", "c"]` 的 `Set` 与包含值 `["c", "d"]` 的 `Set` 的并集，应返回一个包含值 `["a", "b", "c", "d"]` 的新 `Set`。
