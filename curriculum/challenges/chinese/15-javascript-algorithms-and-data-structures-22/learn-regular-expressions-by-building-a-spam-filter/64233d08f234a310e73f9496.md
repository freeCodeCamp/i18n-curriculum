...
你的下一个正则表达式将匹配类似 `stock alert` 的字符串。 声明一个 `stockRegex` 变量，并为其赋值一个能够匹配字符串 `stock alert` 的正则表达式。 记得不区分大小写。

也将其添加到你的 `denyList` 数组中。

# --hints--

你应该使用 `const` 声明你的 `stockRegex` 变量。

```js
assert.match(code, /const\s+stockRegex\s*=/);
```

你的 `stockRegex` 变量应当赋值为一个正则表达式。

```js
assert.instanceOf(stockRegex, RegExp);
```

你的 `stockRegex` 应匹配 `stock alert`。

```js
assert.match('stock alert', stockRegex);
```

你的 `stockRegex` 应该是不区分大小写的。
