...
A questo punto, gli elementi sono troppo vicini tra loro. Aggiungi `gap-8` per creare un po' di spazio tra di essi e usa `mt-16` per spingerli un po' verso il basso rispetto alla parte superiore.

# --hints--

Il tuo elemento `div` dovrebbe avere la classe `gap-8`.
...
Il tuo elemento `div` dovrebbe avere la classe `mt-16`.
