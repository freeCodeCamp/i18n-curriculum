...
أضفت صنف `center` إلى بعض العناصر في HTML. الآن، يجب تعريف هذا الصنف لمركزة تلك العناصر.

استهدف صنف `center` ومركز العناصر التي تحمله باستخدام خاصية `text-align` مضبوطة على `center`.

# --hints--

يجب استهداف العناصر التي تحمل الصنف `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.center'));
```

يجب ضبط خاصية `text-align` على القيمة `center`.
