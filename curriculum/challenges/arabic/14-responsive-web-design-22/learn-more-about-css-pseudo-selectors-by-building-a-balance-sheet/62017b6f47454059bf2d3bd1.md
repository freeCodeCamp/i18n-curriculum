...
يتم استخدام `:not()` وهو عنصر زائف (pseudo-selector) لاستهداف جميع العناصر التي لا تتطابق مع المنتقي - في هذه الحالة أي من عناصرك `span` لا يحتوي على فئة (class) تسمى `sr-only`. هذا يضمن عدم الكتابة فوق القواعد السابقة الخاصة بمنتقي `span[class~="sr-only"]`.

# --hints--

يجب أن يكون لديك منتقي `span:not(.sr-only)`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)'));
```

يجب ان يكون لدى منتقى`span:not(.sr-only)` الـ `font-weight` بقيمة `normal`.
