...
All'interno del metodo `singleCardSelected`, recupera l'elemento `singleCard` e aggiorna il suo `innerHTML` chiamando la funzione `renderCard`. Passa `"Click the card and reveal the fortune"` come primo argomento e `isReversed` come secondo argomento.

# --hints--

Dovresti aggiornare `this.elements.singleCard.innerHTML` chiamando la funzione `renderCard`.
...
La tua chiamata a `renderCard` dovrebbe avere `"Click the card and reveal the fortune"` come primo argomento.
...
Dovresti passare `isReversed` come secondo argomento a `renderCard`.
