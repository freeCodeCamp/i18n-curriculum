...
Rimuovi il margine dal tuo elemento body, imposta il `font-family` su `sans-serif` e assegna a esso una `background-color` con valore `#f5f6f7`.
...
Il tuo selettore `body` dovrebbe avere una proprietà `margin` impostata a 0 come valore.
...
Il tuo selettore `body` dovrebbe avere una proprietà `font-family` impostata a `sans-serif` come valore.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily, 'sans-serif');
```

Il tuo selettore `body` dovrebbe avere una proprietà `background-color` impostata a `#f5f6f7` come valore.
