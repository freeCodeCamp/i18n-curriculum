...
Wenn Sie ein wenig mit Ihrem Player spielen, werden Sie einige Bugs bemerken. Erstens, wenn Sie einen Song abspielen, ihn pausieren und dann entweder die Zurück- oder die Weiter-Taste drücken, startet der Song nicht von Anfang an.

Um das zu beheben, fügen Sie Ihrer `start`-Funktion einen zweiten Parameter namens `playSong` hinzu. Geben Sie ihm den Standardwert `true`.

# --hints--

Ihre `playSong`-Funktion sollte einen zweiten Parameter namens `start` mit dem Standardwert `true` haben.
