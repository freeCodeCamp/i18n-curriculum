...
Verwenden Sie innerhalb der Callback-Funktion die `removeAttribute()`-Methode, um das `"aria-current"`-Attribut zu entfernen. Dadurch wird das Attribut für jeden der Songs entfernt.

# --hints--

Sie sollten das vorhandene `forEach()` und dessen Inhalt nicht verändern.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

Sie sollten die `removeAttribute()`-Methode auf `songEl` anwenden und `"aria-current"` übergeben.
