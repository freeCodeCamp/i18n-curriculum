...
Badilisha `row_str` comprehension ili kutoa mfuatano wa herufi tu wakati kipengee si sifuri, na herufi ya nyota vinginevyo.

# --hints--

Orodha ya comprehension iliyopewa thamani katika `row_str` inapaswa kuita `str()` kwa kila kipengee `i` katika `row` ikiwa `i` ni kweli, na inapaswa kutathmini kuwa `'*'` vinginevyo.
