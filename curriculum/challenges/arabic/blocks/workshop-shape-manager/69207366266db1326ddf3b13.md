...
أضف حدث `DOMContentLoaded` إلى المستند الجذري ومرّر `initializeApp` كدالة استدعاء راجع للتنفيذ. هذا يضمن أن تتم كل عمليات التهيئة فقط بعد تحميل DOM بالكامل.

# --hints--

يجب إضافة حدث `DOMContentLoaded` إلى `document` الجذري.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

يجب أن تشير إلى `initializeApp` كالدالة التي تُنفذ في مستمع الحدث.
