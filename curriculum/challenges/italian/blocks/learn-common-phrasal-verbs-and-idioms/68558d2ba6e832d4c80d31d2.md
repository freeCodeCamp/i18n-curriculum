---
id: 68558d2ba6e832d4c80d31d2
title: Attività 63
challengeType: 19
dashedName: task-63
lang: en-US
---

<!-- (Audio) Linda: First off, the password creation screen. We need to make the instructions clearer and more prominent. Users often overlook them. -->

<!-- SPEAKING -->

# --instructions--

Ascolta l'audio e rispondi alla domanda qui sotto.

# --questions--

## --text--

Come potrebbe Jake dire a Linda che pensa che la sua idea sia buona?

## --answers--

`I like that.`

### --audio-id--

EN7bfbebe8

---

`Are you sure?`

### --audio-id--

EN7f651271

### --feedback--

Questo suona dubbioso, non di supporto.

## --video-solution--

1

# --explanation--

`I like that` è un modo informale e amichevole per mostrare accordo o approvazione dell'idea di qualcuno. Per esempio:

- **Anna:** `Let's move the meeting to 10 a.m.` – Questa persona sta suggerendo un cambio di programma.

- **Diane:** `I like that.` – Questo significa che chi parla supporta o è d'accordo con il suggerimento.

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Linda",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_24-2.mp3",
      "startTime": 1,
      "startTimestamp": 17.4,
      "finishTimestamp": 24.16
    }
  },
  "commands": [
    {
      "character": "Linda",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Linda",
      "startTime": 1,
      "finishTime": 3.24,
      "dialogue": {
        "text": "First off, the password creation screen.",
        "align": "center"
      }
    },
    {
      "character": "Linda",
      "startTime": 3.44,
      "finishTime": 6.22,
      "dialogue": {
        "text": "We need to make the instructions clearer and more prominent.",
        "align": "center"
      }
    },
    {
      "character": "Linda",
      "startTime": 6.38,
      "finishTime": 7.76,
      "dialogue": {
        "text": "Users often overlook them.",
        "align": "center"
      }
    },
    {
      "character": "Linda",
      "opacity": 0,
      "startTime": 8.26
    }
  ]
}
```
