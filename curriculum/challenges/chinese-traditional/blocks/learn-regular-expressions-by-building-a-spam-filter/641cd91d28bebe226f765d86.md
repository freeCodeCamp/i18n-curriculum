...
為你的 `checkMessageButton` 附加一個事件監聽器，監聽 `click` 事件。給它一個空的回呼函式。

# --hints--

你應該在你的 `checkMessageButton` 元素上呼叫 `.addEventListener()`。

```js
assert.match(code, /checkMessageButton\.addEventListener\(/);
```

你的 `.addEventListener()` 方法應該有一個 `"click"` 事件類型。

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,/);
```

你的 `.addEventListener()` 方法應該有一個空的回呼函式。
