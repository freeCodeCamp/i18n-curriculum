...
Agora adicione um `div` com o `className` de `card-stats`. Dentro dele, adicione um `div` com o `className` de `stat-col`. Dentro disso, adicione três elementos `div`, cada um com o `className` de `stat-row`.

# --hints--

Você deve adicionar um `div` com o `className` de `card-stats` depois de `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

Você deve adicionar um `div` com o `className` de `stat-col` dentro de `.card-stats`.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

Você deve adicionar três elementos `div`, cada um com o `className` de `stat-row` dentro de `.stat-col`.
