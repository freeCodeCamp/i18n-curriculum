...
残りの `allCategories` オブジェクトはこちらで完成させておきました。

ここからの数ステップでは、`allCategories` オブジェクトからカテゴリー名を取得する関数を作成します。

まず、`forumCategory` という名前のアロー関数を作成して、仮引数として `id` を取るようにしてください。

# --hints--

`forumCategory` という名前の空の関数を作成しましょう。

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

`forumCategory` 関数には、`id` という名前の仮引数が必要です。
