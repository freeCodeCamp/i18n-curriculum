...
أضف مفتاحًا جديدًا للرقم `417` بقيمة كائن فارغ.

داخل ذلك الكائن، أضف خاصية بمفتاح `category` وقيمة سلسلة نصية هي `"freeCodeCamp Support"`.

أسفل تلك الخاصية، أضف مفتاحًا آخر يسمى `className` بقيمة سلسلة نصية هي `"support"`.

# --hints--

يجب أن يحتوي كائن `allCategories` على خاصية `417`.

```js
assert.property(allCategories, '417');
```

يجب أن تكون قيمة `417` كائنًا.

```js
assert.isObject(allCategories['417'])
```

يجب أن يحتوي كائن `417` على خاصية `category`.

```js
assert.property(allCategories['417'], 'category');
```

يجب تعيين خاصية `category` إلى السلسلة النصية `"freeCodeCamp Support"`.

```js
assert.equal(allCategories["417"].category, 'freeCodeCamp Support');
```

يجب أن يحتوي كائن `417` على خاصية `className`.

```js
assert.property(allCategories['417'], 'className');
```

يجب تعيين خاصية `className` إلى السلسلة النصية `"support"`.
