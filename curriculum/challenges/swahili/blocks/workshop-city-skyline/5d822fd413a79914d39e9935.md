...
Ongeza mwinuko wa rangi unaojirudia kwenye jengo hili; uufanye kuwa sawa na ule uliouongeza hivi karibuni, isipokuwa usiongeze mwelekeo wa `90deg` na tumia rangi ya dirisha lako badala ya rangi mbili za `transparent`.

# --hints--

Unapaswa kumpa `.fb6` `repeating-linear-gradient` ya pili katika sifa ya `background`.
...
Unapaswa kumpa `repeating-linear-gradient` ya pili rangi ya kwanza ya `--building-color3` kutoka `0%` hadi `10%`.
...
Unapaswa kumpa `repeating-linear-gradient` ya pili rangi ya pili ya `--window-color3` kutoka `10%` hadi `30%`.
