...
在你清除 `multipleCard` 后， 在 `singleCardSelected` 方法内，调用 `showElements` 函数，并将 `singleCard` 作为第一个参数，`fortuneContainer` 作为第二个参数传入。

# --hints--

你应该调用 `showElements` 函数。

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

你应该将 `elements.singleCard` 作为第一个参数传入。

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

你应该将 `elements.fortuneContainer` 作为第二个参数传入。
