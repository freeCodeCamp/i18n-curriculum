...
داخل حلقة `for` المسكنة، أضف استدعاء `console.log()` للتحقق من قيم `array` و`array[j]` و`array[minIndex]` في كل تكرار. يمكنك النقر على زر `Sort` لترى كيف تتنقل خوارزميتك عبر المصفوفة.

ثم اكتب جملة `if` تتحقق مما إذا كانت القيمة في `array[j]` أصغر من القيمة في `array[minIndex]`. إذا كانت كذلك، عيّن `minIndex` إلى `j`.

# --hints--

يجب أن يكون لديك استدعاء `console.log()` داخل حلقة `for` المسكنة.
...
يجب أن تمرر `array` كالمعلمة الأولى إلى `console.log()`.
...
يجب أن تمرر `array[j]` كالمعلمة الثانية إلى `console.log()`.
...
يجب أن تمرر `array[minIndex]` كالمعلمة الثالثة إلى `console.log()`.
...
يجب أن يكون لديك جملة `if` بعد استدعاء `console.log()`.
...
يجب أن تحتوي جملة `if` على شرط يتحقق مما إذا كانت `array[j]` أقل من `array[minIndex]`.
...
يجب أن تعيّن جملة `if` قيمة `minIndex` إلى `j`.
