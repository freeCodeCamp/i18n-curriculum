...
داخل عناصر `div.question-block`، قم بتسكين عنصر `label` واحد، وأضف سؤالًا متعلقًا بـ `CSS` إلى نص الـ `label`.

# --hints--

يجب أن تسكن عنصر `label` واحد داخل أول عنصر `div.question-block`.

```js
assert.exists(document.querySelectorAll('.formrow > .question-block')?.[0]?.querySelector('label'));
```

يجب أن تسكن عنصر `label` واحد داخل ثاني عنصر `div.question-block`.

```js
assert.exists(document.querySelectorAll('.formrow > .question-block')?.[1]?.querySelector('label'));
```

يجب أن تعطي عنصر `label` الأول محتوى نصيًا.

```js
assert.isAtLeast(document.querySelectorAll('.formrow > .question-block')?.[0]?.querySelector('label')?.textContent?.length, 1);
```

يجب أن تعطي عنصر `label` الثاني محتوى نصيًا.
