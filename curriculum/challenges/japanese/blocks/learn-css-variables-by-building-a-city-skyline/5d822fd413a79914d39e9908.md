...
`background-color` から `.bb4` のプロパティと値を削除し、3つの新しいセクション `.bb4a`、`.bb4b`、`.bb4c` に追加して、セクションだけが埋まるようにしてください。

# --hints--

`background-color` から `.bb4` を削除してください。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb4")?.backgroundColor);
```

`.bb4a` に `background-color` の `--building-color4` を与えてください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4a")?.backgroundColor.trim(), "var(--building-color4)");
```

`.bb4b` に `background-color` の `--building-color4` を与えてください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4b")?.backgroundColor.trim(), "var(--building-color4)");
```

`.bb4c` に `background-color` の `--building-color4` を与えてください。
