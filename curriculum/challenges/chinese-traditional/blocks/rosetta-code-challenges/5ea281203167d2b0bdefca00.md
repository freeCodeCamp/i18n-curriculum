...
Ludic 數與質數相關，因為它們是透過一種篩法產生，這種篩法類似於用來產生質數的 Eratosthenes 篩法。

第一個 ludic 數是 1。

要產生後續的 ludic 數字，請創建一個從 2 開始遞增整數的陣列。
...
撰寫一個函式（程式），傳回所有小於或相等於給定數字的 ludic 數字。
...
`ludic(5)` 應該傳回 `[1, 2, 3, 5]`。

```js
assert.deepEqual(ludic(5), [1, 2, 3, 5]);
```

`ludic(20)` 應該傳回 `[1, 2, 3, 5, 7, 11, 13, 17]`。

```js
assert.deepEqual(ludic(20), [1, 2, 3, 5, 7, 11, 13, 17]);
```

`ludic(26)` 應該傳回 `[1, 2, 3, 5, 7, 11, 13, 17, 23, 25]`。
