...
같은 선택자 구문을 사용하여 `th`이 `class`인 테이블 행 내의 `total` 요소를 선택하세요. 텍스트를 왼쪽 정렬하고, 패딩을 `0.5rem 0 0.25rem 0.5rem`로 지정하세요.

# --hints--

`tr[class="total"] th` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"] th'));
```

`tr[class="total"] th` 선택자에 `text-align` 속성을 `left`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"] th')?.getPropertyValue('text-align') === 'left');
```

`tr[class="total"] th` 선택자에 `padding` 속성을 `0.5rem 0 0.25rem 0.5rem`로 설정해야 합니다.
