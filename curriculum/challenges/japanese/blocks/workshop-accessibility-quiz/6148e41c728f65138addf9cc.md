...
最後に、特定の種類の動きに基づくアニメーションは、一部のユーザーに不快感を与える場合があります。特に、<dfn>前庭</dfn>障害を持つ人は、特定の動きのトリガーに敏感です。

`@media` アットルールには、ユーザーのアニメーション設定に基づいて CSS を設定するための `prefers-reduced-motion` というメディア機能があります。これは次のいずれかの値を取ることができます：
...
この機能については今後のレッスンで詳しく学びます。今は、`scroll-behavior: smooth` を設定するスタイルルールを、メディア機能 `@media` に値 `prefers-reduced-motion` を設定した `no-preference` アットルールで囲んでください。

よくできました。アクセシビリティクイズの練習プロジェクトを完了しました。

# --hints--

`@media (prefers-reduced-motion: no-preference)` ルールが1つあるはずです。

```js
assert.lengthOf(new __helpers.CSSHelp(document).getRuleListsWithinMedia('(prefers-reduced-motion: no-preference)'), 1);
```

既存の `*` ルールを `@media` ルールで囲む必要があります。
