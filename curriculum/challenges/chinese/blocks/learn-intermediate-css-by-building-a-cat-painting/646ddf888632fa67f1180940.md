...
将左眼移动到合适的位置，设置 `position` 为 `absolute`、`top` 为 `54px`，`left` 为 `39px`。

# --hints--

你的 `.cat-left-eye` 选择器应设置 `position` 属性为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.position === 'absolute')
```

你的 `.cat-left-eye` 选择器应设置 `top` 属性为 `54px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.top === '54px')
```

你的 `.cat-left-eye` 选择器应设置 `left` 属性为 `39px`。
