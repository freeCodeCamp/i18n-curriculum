...
الآن بعد أن انتهى تصميم المجلة، عليك جعله متجاوبًا.

ابدأ بـ `@media` استعلام لـ `only screen` مع `max-width` بقيمة `720px`. بداخله، أنشئ محدِّد `.image-wrapper` وأعطه خاصية `grid-template-columns` بقيمة `1fr`.

هذا سيُحوّل الصور الثلاث إلى عمود واحد على الشاشات الأصغر.

# --hints--

يجب أن يكون لديك قاعدة `@media` جديدة لـ `only screen and (max-width: 720px)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

يجب أن تحتوي قاعدة `@media` الجديدة على محدِّد `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

يجب أن يحتوي محدِّد `.image-wrapper` الجديد على خاصية `grid-template-columns` بقيمة `1fr`.
