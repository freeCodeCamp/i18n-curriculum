...
Dentro de tu sentencia `if`, crea una variable llamada `category` para almacenar la categoría del gasto. Asígnale una llamada a `input()` y usa `'Enter category: '` como argumento.

# --hints--

Debes declarar una variable llamada `category` en tu sentencia `if`.
...
Debes asignar `input('Enter category: ')` a tu variable `category`.
