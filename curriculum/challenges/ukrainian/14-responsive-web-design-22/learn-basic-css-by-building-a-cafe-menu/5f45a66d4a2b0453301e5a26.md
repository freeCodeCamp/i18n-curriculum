...
Пересуваючись до елемента `footer`, задайте кожному тексту значення `14px` для розміру шрифту.

# --hints--

Ви повинні мати селектор `footer`.
...
Ваш селектор `footer` повинен бути під коментарем.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Ви повинні встановити властивість `font-size` на `14px`.
...
Ваш елемент `footer` повинен мати `font-size` зі значенням `14px`.
