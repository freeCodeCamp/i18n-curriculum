...
أعلن عن متغير باسم `cardElement` واستخدم الصنف `card_container` للحصول على العنصر. استخدم المحدِّد `closest` للحصول عليه من `target`.

# --hints--

يجب إعلان متغير باسم `cardElement` داخل `showFortune`.
...
يجب أن يستخدم `cardElement` الخاص بك `target?.closest` للعثور على `.card_container`.
...
يجب تمرير `.card_container` كمعلمة إلى `closest`.
