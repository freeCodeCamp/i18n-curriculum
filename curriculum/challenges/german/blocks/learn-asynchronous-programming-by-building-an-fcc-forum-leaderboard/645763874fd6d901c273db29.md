...
Fügen Sie im ersten `td`-Element einen eingebetteten Ausdruck `${}` hinzu. Rufen Sie innerhalb dieses Ausdrucks die `forumCategory`-Funktion mit dem Argument `category_id` auf.

Jetzt sollten Sie unter jedem Beitragsthema eine Kategorie angezeigt sehen.

# --hints--

Sie sollten die `forumCategory`-Funktion mit dem Argument `category_id` aufrufen.
