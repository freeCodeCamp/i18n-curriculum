...
給偽元素設定 `width` 為其父元素的一半，`height` 為 `45%`，以及 `background-color` 為 `gray`。

# --hints--

你應該給 `.penguin-body::before` 一個 `width` 為 `50%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.width, '50%');
```

你應該給 `.penguin-body::before` 一個 `height` 為 `45%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.height, '45%');
```

你應該給 `.penguin-body::before` 一個 `background-color` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。
