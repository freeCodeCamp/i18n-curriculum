...
O posicionamento `sticky` é um híbrido dos posicionamentos `relative` e `fixed`. Ele permite que um elemento "**se grude**" a uma posição específica dentro do elemento que o contém ou da viewport, com base na posição de rolagem.

Altere o valor da propriedade `position` de `.cat-head` para `sticky`, defina `top` como `0` e remova `left` e seu valor.

**Note**: To see how `sticky` works, you have to place a couple of texts before and after your `.cat-head` element. Se você rolar para baixo depois disso, verá que `.cat-head` fica preso na parte superior e permanece lá.
