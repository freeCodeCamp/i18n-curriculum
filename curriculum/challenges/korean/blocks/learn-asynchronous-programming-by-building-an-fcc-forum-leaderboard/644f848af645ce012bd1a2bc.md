...
`linkText`라는 상수를 만들고 `selectedCategory.category` 값을 할당하세요. 이렇게 하면 앵커 요소에 카테고리 이름이 표시됩니다.

# --hints--

`linkText`라는 상수가 있어야 합니다.

```js
assert(code.match(/const\s+linkText\s*=/));
```

`selectedCategory.category`을 `linkText` 변수에 할당해야 합니다.
