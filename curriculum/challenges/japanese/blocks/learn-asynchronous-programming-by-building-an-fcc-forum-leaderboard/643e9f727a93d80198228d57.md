...
トピックのタイトルを表示するには、最初の`p`要素の中に`td`要素を追加してください。

段落タグの間に、`title`変数を含む埋め込み式を追加します。次に、開く段落タグの中に`"post-title"`というクラスを追加してください。

# --hints--

最初の`p`要素の中に`td`要素があるはずです。

```js
assert(/return\s*`\s*<tr>\s*<td>\s*\S*\s*<p.*>\s*.*\s*<\/p>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>\s*\S*\s*<\/td>\s*<td>.*<\/td>\s*<td>.*<\/td>\s*<\/tr>\s*`/.test(code));
```

開く`"post-title"`タグの中に`p`というクラスがあるはずです。
