...
Mission Control requiere una función más para registrar un resumen de tu tripulación. Crea una función vacía llamada `printCrewSummary` que acepte un parámetro `crew`.

# --hints--

Debes crear una función llamada `printCrewSummary`.

```js
assert.isFunction(printCrewSummary);
```

Tu función `printCrewSummary` debe tener un parámetro `crew`.
