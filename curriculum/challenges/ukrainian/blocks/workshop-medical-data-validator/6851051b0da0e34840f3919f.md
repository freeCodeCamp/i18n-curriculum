...
`age` має бути не просто цілим числом, а додатним цілим числом, більшим або рівним `18`.

Використовуючи оператор `and`, додайте другий вираз до значення ключа `age`, щоб це перевірити.

# --hints--

Ключ `age` у вашому словнику `constraints` має мати значення `isinstance(age, int) and age >= 18`.
