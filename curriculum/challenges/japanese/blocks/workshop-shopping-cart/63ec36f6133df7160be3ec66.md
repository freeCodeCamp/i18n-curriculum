...
分割代入を使って、配列やオブジェクトから複数の値を1つの文で取り出せることを覚えておいてください。

コールバック関数の最初のパラメータとして、渡された`name`インスタンスから`id`、`price`、`category`、`Dessert`プロパティを分割代入してください。

# --hints--

`name`、`id`、`price`、`category`のパラメータを分割代入で宣言してください。このテストでは順序が重要です。

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

分割代入はコールバック関数の最初のパラメータである必要があります。
