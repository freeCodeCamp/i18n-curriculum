...
El pseudo-selector `::first-letter` permite seleccionar la primera letra en el contenido de texto de un elemento.

Crea un selector `.first-paragraph::first-letter` y establece la propiedad `font-size` a `6rem`. También dale una propiedad `color` establecida en `orangered` para que destaque.

# --hints--

Debes tener un selector `.first-paragraph::first-letter`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

Tu selector `.first-paragraph::first-letter` debe tener una propiedad `font-size` establecida en `6rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize, '6rem');
```

Tu selector `.first-paragraph::first-letter` debe tener una propiedad `color` establecida en `orangered`.
