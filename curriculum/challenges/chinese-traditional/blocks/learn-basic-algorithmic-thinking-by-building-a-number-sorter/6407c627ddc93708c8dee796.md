...
創建一個名爲 `outputValueNode` 的變量，並將其值設置爲調用 `document.getElementById()` 方法的結果。 使用模板字面量語法將 `` `output-value-${i}` `` 字符串傳遞給 `.getElementById()`。

# --hints--

使用 `const` 在 `.forEach()` 回調中聲明一個 `outputValueNode` 變量。

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=/)
```

`outputValueNode` 應該被賦值爲調用 `document.getElementById()` 的結果。

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=\s*document\.getElementById\s*\(/)
```

使用模板字面量語法將 `output-value-${i}` 字符串傳遞給 `.getElementById()`。
