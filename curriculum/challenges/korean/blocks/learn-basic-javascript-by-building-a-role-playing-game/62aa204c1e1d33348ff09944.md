...
`attack` 함수에서 `monsterHealth` 변수를 업데이트하는 코드 줄을 찾아 `if` 함수를 호출하는 조건과 함께 `isMonsterHit` 블록 안에 넣으세요.
...
`if` 문은 조건에서 `isMonsterHit` 를 호출해야 합니다.

```js
assert.match(attack.toString(), /if\s*\(\s*isMonsterHit\s*\(\s*\)/)
```

`monsterHealth -= weapons[currentWeaponIndex].power + Math.floor(Math.random() * xp) + 1` 을 `if` 블록 안에 넣으세요.
