...
最後に、クラスの外で書いたコードを修正し、速度、高さ、角度の希望値を受け取り、射出物の詳細、座標のテーブル、軌道のグラフを順にターミナルに出力する小さなユーティリティ関数`projectile_helper`にまとめてください。

`projectile_helper`を一度、任意の値で呼び出してください。

# --hints--

グローバルスコープに`projectile_helper`を作成してください。
...
指定された文字列を順番に出力してください。他の値は出力しないでください。
...
`projectile_helper`関数を少なくとも一度は呼び出してください。
...
グローバルスコープに変数やprintの呼び出しがあってはいけません。
