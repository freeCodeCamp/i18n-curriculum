...
もう一度、カードをランダムかつ裏向きにする必要があります。`isReversed` と `card` という名前の変数を宣言してください。最初の変数には `Math.random` メソッドを呼び出して `< 0.5` と比較します。2つ目の変数には `getRandomItem` を呼び出し、`this.cards` を渡してください。

# --hints--

`isReversed` という名前の変数を宣言してください。
...
`isReversed` 変数は、カードが裏向きかどうかをランダムに決めるために `Math.random()` を使うべきです。
...
`card` 変数は `getRandomItem` を引数にして `this.cards` メソッドを呼び出すことで代入されるべきです。
