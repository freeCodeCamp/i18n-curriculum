...
أسفل عنصر `h1`، أضف عنصر `p` بصنف `description`.

داخل عنصر `p`، اكتب النص `Click on the buttons below to rate your emotions.`

# --hints--

يجب إنشاء عنصر `p` جديد أسفل عنصر `h1`.

```js
assert.exists(document.querySelector("h1 + p"));
```

يجب أن يحتوي عنصر `p` الجديد على صنف `description`.

```js
assert.equal(document.querySelector("h1 + p")?.className, "description");
```

يجب أن يحتوي عنصر `p` الجديد على النص `Click on the buttons below to rate your emotions.`.
