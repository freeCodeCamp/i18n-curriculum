...
داخل قسم الاتصال، سترغب في عرض معلومات الاتصال الخاصة بمؤلف المدونة. ستستخدم عنصر <dfn>address</dfn> لهذا الغرض.

يُستخدم عنصر `address` لتمثيل معلومات الاتصال الخاصة بشخص أو منظمة.

إليك مثالًا يستخدم عنصر `address` لعنوان فعلي. يُستخدم عنصر `br` هنا لإنشاء فاصل سطر بين النص.
...
يجب أن يحتوي شفرتك على وسم افتتاحي `address`.

```js
assert.match(code, /<address>/i);
```

يجب أن يحتوي شفرتك على وسم إغلاق `address`.

```js
assert.match(code, /<\/address>/i);
```

يجب أن يكون عنصر `address` داخل عنصر `section` الخاص بك.
