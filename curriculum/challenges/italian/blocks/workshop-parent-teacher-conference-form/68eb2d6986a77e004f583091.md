...
Ora è il momento di personalizzare i pulsanti di opzione. Per impostazione predefinita, i pulsanti di opzione hanno un aspetto nativo del browser. Vuoi rimuoverlo per poter creare un aspetto personalizzato.

Seleziona l'elemento con la classe `contact-method-radio-btn`. Imposta `appearance` su `none`.

# --hints--

Dovresti selezionare l'elemento con la classe `contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Imposta la proprietà `appearance` su `none`.
