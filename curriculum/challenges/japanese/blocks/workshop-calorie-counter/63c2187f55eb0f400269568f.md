...
最後に、2つ目の`label`の後の新しい行に、もう1つ`input`要素を作成してください。この要素には、`type`属性を`number`に、負のカロリーが追加されないように`min`属性を`0`に、`placeholder`属性を`Calories`に、そして2つ目の`label`要素の`for`属性と一致する`id`属性を設定してください。
