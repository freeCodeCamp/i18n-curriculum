...
Em espanhol, você **não usa** um artigo (`un` ou `una`) antes de uma profissão.
...
Em espanhol, despedidas nem sempre precisam ser combinadas, mas é comum vê-las usadas juntas. Por exemplo:
...
Os acentos em espanhol mostram onde colocar a ênfase e às vezes mudam o significado de uma palavra. Estes são alguns exemplos desta lição:
...
`Ñ` é uma letra separada no alfabeto espanhol. Sua versão minúscula é `ñ`. Por exemplo: 
