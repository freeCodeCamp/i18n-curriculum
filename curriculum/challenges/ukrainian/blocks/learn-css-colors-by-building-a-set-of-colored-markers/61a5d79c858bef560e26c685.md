...
І як третій колірний аргумент передайте функцію `hsl` зі значеннями `240` для відтінку, `56%` для насиченості та `42%` для світлості.

# --hints--

Ваше CSS-правило `.blue` має містити властивість `background`, встановлену в `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))`.
