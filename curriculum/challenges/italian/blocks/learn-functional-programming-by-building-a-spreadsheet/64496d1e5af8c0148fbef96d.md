...
Ora dichiara una variabile `length` e assegnale la lunghezza del tuo array ordinato, e una variabile `middle` che abbia il valore della lunghezza divisa per `2`, meno `1`.

# --hints--

Dovresti dichiarare una variabile `length` dopo la tua variabile `sorted`.
...
Dovresti usare `const` per dichiarare la tua variabile `length`.
...
Dovresti assegnare la lunghezza dell'array `sorted` alla tua variabile `length`.
...
Dovresti dichiarare una variabile `middle` dopo la tua variabile `length`.
...
Dovresti usare `const` per dichiarare la tua variabile `middle`.
...
Dovresti assegnare a `middle` il valore della divisione della tua variabile `length` per `2`.
...
Dovresti sottrarre `1` dal tuo calcolo `length / 2`.
