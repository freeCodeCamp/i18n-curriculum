...
在 `card-rating` div 下方，添加一個 `div`，其 `className` 為 `card-position`，並包含文字 `ST`，代表前鋒。

# --hints--

你應該在 `div` div 下方添加一個 `className`，其 `card-position` 為 `.card-rating`。

```js
assert.exists(document.querySelector('.card-rating + .card-position'));
```

`.card-position` 元素應該包含文字 `ST`。
