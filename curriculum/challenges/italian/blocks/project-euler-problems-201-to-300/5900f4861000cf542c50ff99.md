...
title: 'Problema 282: la funzione di Ackermann'
challengeType: 1
forumTopicId: 301933
...
Per numeri interi non negativi $m$, $n$, la funzione di Ackermann $A(m, n)$ è definita come segue:

$$A(m, n) =
\begin{cases}
n + 1                 & \text{se $m = 0$}             \\\\
A(m - 1, 1)           & \text{se $m > 0$ e $n = 0$} \\\\
A(m - 1, A(m, n - 1)) & \text{se $m > 0$ e $n > 0$}
\end{cases}$$

Per esempio $A(1, 0) = 2$, $A(2, 2) = 7$ e $A(3, 4) = 125$.

Trova $\displaystyle\sum_{n = 0}^6 A(n, n)$ e fornisci la tua risposta modulo ${14}^8$.
