...
`div`の`className`を持つ`card-tier-badge`要素の下に、`div`の`className`を持つ`card-club`とテキスト`Santos FC`を追加してください。

# --hints--

`div`の下に`className`の`card-club`を持つ`.card-tier-badge`を追加してください。

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

`.card-club`要素にはテキスト`Santos FC`が含まれている必要があります。
