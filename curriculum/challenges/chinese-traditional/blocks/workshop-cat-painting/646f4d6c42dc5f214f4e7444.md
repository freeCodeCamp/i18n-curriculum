...
現在你將開始將右側鬍鬚移動到適當位置。使用類別選擇器來標的 `.cat-whisker-right-top` 元素，並給它 `position` 為 `absolute`、`top` 為 `120px`，以及 `left` 為 `109px`。

# --hints--

你應該有一個 `.cat-whisker-right-top` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top'));
```

你的 `.cat-whisker-right-top` 選擇器應該有一個 `position` 屬性設定為 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.position, 'absolute');
```

你的 `.cat-whisker-right-top` 選擇器應該有一個設定為 `120px` 的 `top` 屬性。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.top, '120px');
```

你的 `.cat-whisker-right-top` 選擇器應該有一個設定為 `109px` 的 `left` 屬性。
