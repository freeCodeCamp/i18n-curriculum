...
对于你的 `timeAgo` 函数，你需要计算当前时间与一个主题上最后一次活动的时间之间的差值。 这样就可以显示自某个主题有活动以来过去了多长时间。

完成符合以下要求的 `timeAgo` 函数：

- 如果已过去的分钟数少于 `60` 分钟，则返回字符串 `"xm ago"`。 `x` 将表示分钟数。
- 如果已过去的小时数少于 `24`，则返回字符串 `"xh ago"`。 `x` 将表示小时数。
- 否则，返回字符串 `"xd ago"`。 `x` 将表示天数。
...
如果已过去的分钟数为 `30` 分钟，你应该返回字符串 `"30m ago"`。
...
如果已过去的小时数为 `1` 小时，你应该返回字符串 `"1h ago"`。
