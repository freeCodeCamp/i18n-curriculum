...
給 `.key.black--key::after` 添加一個 `position` 屬性並設為 `absolute`，以及一個 `left` 屬性並設為 `-18px`。

# --hints--

你的 `.key.black--key::after` 選擇器應該有一個 `position` 屬性設為 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.key.black--key::after')?.position === 'absolute');
```

你的 `.key.black--key::after` 選擇器應該有一個 `left` 屬性設定為 `-18px`。
