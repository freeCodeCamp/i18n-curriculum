...
預設情況下，`label` 元素是行內的。若要讓它們垂直堆疊（你的圓鈕標籤除外），請使用 `:not()` 偽類別。

`:not()` 偽類別是否定一個選擇。在這裡，它選擇所有沒有 `contact-method` 類別的 `label` 元素。

選取 `label:not(.contact-method)`，然後將 `display` 設為 `block`，`margin` 設為 `10px 0`。

# --hints--

你應該有一個 `label:not(.contact-method)` 選擇器。
