...
为 `"midfielder"` 添加一个新的 `case`，按照与上一步相同的模式检查 `player.position` 是否等于 `"midfielder"`。

# --hints--

你应该在 `switch` 语句中为 `"midfielder"` 添加一个新的 `case`。

```js
assert.match(code, /\s*case\s*('|"|`)\s*midfielder\s*\1\s*:\s*/)
```

在你的 `"midfielder"` `case` 中，你应该调用 `setPlayerCards` 函数。
...
你的 `filter()` 回调应该隐式返回检查 `player.position` 是否为 `"midfielder"` 的结果。
