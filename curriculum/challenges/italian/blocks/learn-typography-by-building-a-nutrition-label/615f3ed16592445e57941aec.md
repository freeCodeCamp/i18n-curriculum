...
Le righe possono aiutare a separare e raggruppare contenuti importanti, soprattutto quando lo spazio è limitato.

Crea un elemento `div` sotto il tuo elemento `h1` e dagli un attributo `class` impostato su `divider`.
...
Il tuo elemento `.divider` dovrebbe essere all'interno del tuo elemento `.label`.
