---
id: 6553a44b1801991847d8cc69
title: 步骤 47
challengeType: 20
dashedName: step-47
---

# --description--

接下来，修改你的 `print()` 调用以打印 `'encrypted text:', encrypted_text` 并将其放在 `for` 循环之外，以便加密字符串被打印一次。

# --hints--

你应该在 for 循环后打印 `'encrypted text:', encrypted_text`

```js
({ test: () => assert.match(code, /^print\s*\(\s*("|')encrypted\stext:\1\s*,\s*encrypted_text\s*\)/m) })
```

你的代码中不应包含 `print('char:', char, 'encrypted text:', encrypted_text)` 。

```js
({ test: () => {
    const commentless_code = __helpers.python.removeComments(code);
    assert.notMatch(commentless_code, /print\s*\(\s*("|')char:\1\s*,\s*char\s*,\s*("|')encrypted\stext:\2\s*,\s*encrypted_text\s*\)/)
  }
})
```

# --seed--

## --seed-contents--

```py
--fcc-editable-region--
text = 'Hello Zaira'
shift = 3
alphabet = 'abcdefghijklmnopqrstuvwxyz'
encrypted_text = ''

for char in text.lower():
    if char == ' ':
        encrypted_text += char
    else:
        index = alphabet.find(char)
        new_index = (index + shift) % len(alphabet)
        encrypted_text += alphabet[new_index]
    print('char:', char, 'encrypted text:', encrypted_text)
--fcc-editable-region--
```
