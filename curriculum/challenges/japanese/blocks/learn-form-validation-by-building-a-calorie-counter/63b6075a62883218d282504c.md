...
あなたの `input` 要素にはいくつか追加の属性が必要です。数値のみを許可するために `type` を `number` に設定し、正の数のみを許可するために `min` 属性を `0` に設定し、さらに `placeholder` を `Daily calorie budget` に設定してください。

最後に、`input` 要素を `required` としてマークしてください。
...
あなたの `input` 要素には `min` 属性を `0` に設定してください。

```js
assert.equal(document.querySelector('form input').min, '0');
```

あなたの `input` 要素には `placeholder` 属性を `Daily calorie budget` に設定してください。

```js
assert.equal(document.querySelector('form input').placeholder, 'Daily calorie budget');
```

あなたの `input` 要素には `required` 属性を設定してください。
