...
各 `stat-row` には、それぞれ `span` と `className` の `stat-value` を持つ 2 つの `stat-label` 要素が含まれている必要があります。

`spans` 要素の中の `div` 要素には、次のものが含まれている必要があります。

- `99` と `DRI`
- `41` と `DEF`
- `75` と `PHY`

`DRI` はドリブル、`DEF` はディフェンス、`PHY` はフィジカルを表します。

# --hints--

2 番目の `.stat-row` の最初の `.stat-col` に、値を `99`、ラベルを `DRI` に設定してください。
...
2 番目の `.stat-row` の 2 番目の `.stat-col` に、値を `41`、ラベルを `DEF` に設定してください。
...
2 番目の `.stat-row` の 3 番目の `.stat-col` に、値を `75`、ラベルを `PHY` に設定してください。
