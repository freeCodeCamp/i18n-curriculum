...
현재 노래의 인덱스를 얻으려면 <dfn>indexOf()</dfn> 메서드를 사용할 수 있습니다. `indexOf()` 배열 메서드는 주어진 요소를 배열에서 처음 찾을 수 있는 인덱스를 반환하며, 요소가 없으면 `-1`을 반환합니다.
...
`getCurrentSongIndex` 함수 안에서 `userData?.songs.indexOf()`을 반환하세요. `indexOf()` 인수에는 `userData?.currentSong`를 설정하세요.

# --hints--

`userData?.songs.indexOf()` 함수 안에서 `getCurrentSongIndex`을 반환해야 합니다.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

`userData?.currentSong` 메서드에 `indexOf()`을 전달해야 합니다.
