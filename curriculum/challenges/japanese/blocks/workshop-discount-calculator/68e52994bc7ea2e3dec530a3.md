...
`calculate_best_price` メソッド内に、`prices` を含む `product.price` という名前のリストを作成してください。これにより、割引が適用されない場合は顧客が定価を支払うことが保証されます。

# --hints--

`calculate_best_price` 内に、`prices` を含む `product.price` という名前のリストを作成する必要があります。
