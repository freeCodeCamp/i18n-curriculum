...
接下来是最后的 `fieldset`。 如果你想允许用户上传个人资料图片，应该怎么做呢？

好吧，`input` 类型 `file` 就允许这样做。为你的 `label` 添加文本 `Upload a profile picture: `，并嵌套一个接受文件上传的 `input`。

# --hints--

你应该添加一个带有文本 `Upload a profile picture: ` 的 `label`。

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label')?.innerText, /Upload a profile picture:/i);
```

你应该在 `label` 元素内嵌套一个 `input` 元素。

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label > input'));
```

你应该给 `input` 元素一个 `file` `type`。
