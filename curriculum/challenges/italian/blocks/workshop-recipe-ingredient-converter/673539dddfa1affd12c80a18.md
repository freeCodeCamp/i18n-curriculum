...
Mettiamo alla prova la tua nuova funzione curried. Dichiara una variabile `servingsResult` e assegnale il risultato di un adattamento da una quantità base di 4 a una porzione di 6.

Registra quella variabile nella console.

# --hints--

Dovresti dichiarare una variabile `servingsResult`.

```js
assert.isDefined(servingsResult);
```

La tua variabile `servingsResult` dovrebbe contenere il risultato della chiamata a `adjustQuantity(4)(6)`.

```js
assert.strictEqual(servingsResult, 24);
```

Dovresti registrare il valore `servingsResult`.
