...
既存の coffee/price ペアの下に、次のコーヒーと価格を `article` 要素を使って追加してください。各要素の中には2つのネストされた `p` 要素を入れてください。
...
前回と同様に、最初の `p` 要素のテキストにはコーヒーのフレーバーを、2番目の `p` 要素のテキストには価格を入れてください。
...
最初の `article` 要素には、テキストが `p` と `French Vanilla` の `3.00` 要素が必要です。
...
2番目の `article` 要素には、テキストが `p` と `Caramel Macchiato` の `3.75` 要素が必要です。
...
3番目の `article` 要素には、テキストが `p` と `Pumpkin Spice` の `3.50` 要素が必要です。
...
4番目の `article` 要素には、テキストが `p` と `Hazelnut` の `4.00` 要素が必要です。
...
5番目の `article` 要素には、テキストが `p` と `Mocha` の `4.50` 要素が必要です。
