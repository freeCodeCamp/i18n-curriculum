...
يجب أن يكون لديك منتقي `.hero-title`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

يجب أن يكون لمنتقي `.hero-title` الخاص بك خاصية `text-align` بقيمة `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign === 'center');
```

يجب أن يكون لمنتقي `.hero-title` الخاص بك خاصية `color` بقيمة `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color === 'orangered');
```

يجب أن يكون لمنتقي `.hero-title` الخاص بك خاصية `font-size` بقيمة `8rem`.
