...
Thamani za mwisho zilizowekwa moja kwa moja ni takwimu sita za mchezaji. Badilisha seti tatu za kwanza za nambari zilizowekwa moja kwa moja na sifa zinazolingana za `player`:
...
Unapaswa kubadilisha thamani ya `97` kwa `PAC` na `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

Unapaswa kubadilisha thamani ya `98` kwa `SHO` na `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

Unapaswa kubadilisha thamani ya `83` kwa `PAS` na `player.pas`.
