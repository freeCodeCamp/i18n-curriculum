...
當 `target` 為空時，必須將碟從 `source` 移動到 `target`。

在 `forward` 的宣告之後，添加一個 `if` 述語來檢查 `rods[target]` 是否為空。如果是，將 `forward` 改為 `True`。

# --hints--

你應該有一個 `if` 述語來檢查 `rods[target]` 是否為空，並將 `True` 指定給 `forward`。
