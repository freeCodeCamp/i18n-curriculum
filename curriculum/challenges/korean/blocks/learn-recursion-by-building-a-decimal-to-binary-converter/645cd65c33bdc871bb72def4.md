...
간단한 예제이지만, 호출 스택이 코드를 어떻게 단계별로 실행하며 여러 함수를 호출하는지 보여줍니다.

이제 재귀로 넘어가 호출 스택이 어떻게 작동하는지 살펴보세요.

`callStack` 배열, `a()`, `b()`, `c()` 함수들과 `console.log()` 문장을 제거하세요.

# --hints--

코드에서 `callStack` 배열을 제거해야 합니다.

```js
assert.notMatch(code, /(var|let|const)\s+callStack\s*=\s*\[\s*\]/);
```

코드에서 `a()`, `b()`, `c()` 함수를 제거해야 합니다.
