...
表單的下一部分將負責詢問用戶是否曾入住過該酒店。

首先添加一個 `fieldset` 元素。

在 `fieldset` 元素內部，添加一個 `legend` 元素，其文本爲 `Was this your first time at our hotel?`。
...
你的 `legend` 元素的文本應爲 `Was this your first time at our hotel?`。
