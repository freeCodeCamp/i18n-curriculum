...
Crea una función llamada `setPlayButtonAccessibleText`. Dentro de la función, establece el atributo `aria-label` del botón de reproducción a `Play` seguido de un espacio y el título de la canción actual. Si `userData.currentSong` es `null`, establécelo en `Play`.

# --hints--

Debe tener una función llamada `setPlayButtonAccessibleText`.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

Tu función `setPlayButtonAccessibleText` debe establecer el atributo `aria-label` del botón de reproducción a `Play` seguido de un espacio y el título de la canción actual.
...
```
 ...
 Cuando `userData.currentSong` es `null`, `setPlayButtonAccessibleText` debe establecer el atributo `aria-label` del botón de reproducción a `Play`.
