...
你應該做的第一件事是檢查 `items` 陣列是否為空。如果是，則顯示一個帶有文字 `Your shopping cart is already empty` 的 `alert` 給使用者，然後從函式中傳回。

請記住 `0` 是一個假值，所以你可以使用 `!` 運算子來檢查陣列是否為空。

在顯示警示後，從函式中傳回以停止執行。

# --hints--

你應該創建一個 `if` 述語，使用否定運算子來檢查 `items` 陣列是否為空。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

你的 `if` 述語應該顯示一個帶有文字 `Your shopping cart is already empty` 的 `alert` 給使用者。
