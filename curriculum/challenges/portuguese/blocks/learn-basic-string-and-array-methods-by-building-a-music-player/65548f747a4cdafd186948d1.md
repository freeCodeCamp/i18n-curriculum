...
Use `getElementById()` para obter o `id` da música que está tocando no momento e depois use template literals para prefixá-lo com `song-`. Atribua-o à constante `songToHighlight`.
...
Você deve atribuir seu `getElementById()` a uma constante `songToHighlight`.
