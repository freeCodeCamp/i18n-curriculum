...
Il prossimo elemento da definire nello stile è il testo del prezzo, incluso il `span` al suo interno. Dai all'elemento `p` del prezzo le classi `text-4xl`, `font-bold` e `text-gray-900`.

# --hints--

Il tuo elemento `p` dovrebbe avere la classe `text-4xl`.
...
Il tuo elemento `p` dovrebbe avere la classe `font-bold`.
...
Il tuo elemento `p` dovrebbe avere la classe `text-gray-900`.
