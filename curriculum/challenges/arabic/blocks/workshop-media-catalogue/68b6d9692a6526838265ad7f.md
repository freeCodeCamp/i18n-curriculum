...
سيتم إنشاء كائن `TVSeries` مع خصائص إضافية مقارنةً بصنف الأصل `Movie`. لذلك، أنشئ طريقة `__init__` بالمعلمات التالية: `self`، `title`، `year`، `director`، `duration`، `seasons`، و`total_episodes`.

داخل طريقة `__init__`، قم بتعيين `seasons` و`total_episodes` إلى خصائص بنفس الاسم.

# --hints--

يجب أن يحتوي صنف `TVSeries` على طريقة `__init__`.
...
يجب أن تحتوي طريقة `__init__` على المعلمات التالية: `self`، `title`، `year`، `director`، `duration`، `seasons`، و`total_episodes`.
...
يجب تعيين `seasons` إلى `self.seasons` داخل طريقة `__init__` الخاصة بك.
...
يجب تعيين `total_episodes` إلى `self.total_episodes` داخل طريقة `__init__` الخاصة بك.
