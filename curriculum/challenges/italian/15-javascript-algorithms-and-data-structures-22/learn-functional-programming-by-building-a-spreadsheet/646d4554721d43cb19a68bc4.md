...
La funzione `apply` dovrebbe utilizzare un return implicito.
...
La funzione `apply` dovrebbe accedere all'oggetto `spreadsheetFunctions`.
...
La funzione `apply` dovrebbe accedere alla proprietà dell'oggetto `spreadsheetFunctions` che corrisponde al valore di `fn`.
...
La funzione `apply` dovrebbe chiamare il metodo `.toLowerCase()` su `fn` accedendo alla proprietà.
