...
チェックボックスは6つあるので、それらのために配列を作成し、mapで回してチェックボックスとして表示するのが最適です。そのために、`powersOptions` 配列が用意されています。

アロー関数を使って、`powersOptions` 配列を`power` というパラメータでmapしてください。mapの中で、`label` を`key` に持つ`power` 要素と、`Hello` のテキストを作成します。

その後、6つの`Hello` テキストが表示されるはずです。配列の要素は次のステップで埋めていきます。

# --hints--

`powersOptions` をmapで回してください。

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\(/)
```

mapのパラメータには`power` を使ってください。

```js
assert.match(code, /{\s*powersOptions\s*\.\s*map\s*\(\s*(power|\(\s*power\s*\))\s*=>/)
```

`label` を持つ`key` の`power` 要素を作成してください。

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\([\s\S]*<\s*label\s+key\s*=\s*\{\s*power\s*\}\s*>[\s\S]*<\s*\/\s*label\s*>/)
```

`label` 要素には`Hello` のテキストを入れてください。
