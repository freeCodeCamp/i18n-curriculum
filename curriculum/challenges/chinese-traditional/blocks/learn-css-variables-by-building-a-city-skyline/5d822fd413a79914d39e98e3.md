...
創建另一個名為 `--building-color4` 的變數並賦值為 `#538cc6`。確保這次它位於 `:root` 選擇器中。然後使用它來填充最後一棟建築。

# --hints--

你應該定義一個名為 `--building-color4` 的新屬性變數。

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color4'));
```

你應該在 `:root` 選擇器中為 `--building-color4` 指定 `#538cc6` 的值。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color4').trim(), '#538cc6');
```

你應該在 `.bb4` 選擇器中添加 `background-color: var(--building-color4)`。
