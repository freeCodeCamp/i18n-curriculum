...
Ora devi creare una funzione per nascondere gli elementi dal DOM. Dichiara una funzione `hideElements` che prende `...elements` come parametro con il tipo `HTMLElement[]`.

Questa funzione, con una classe CSS `hidden`, sarà usata in tutta l'app per alternare la visibilità degli elementi dell'interfaccia utente.

# --hints--

Dovresti aver dichiarato una funzione chiamata `hideElements`.
...
La tua funzione `hideElements` dovrebbe avere un parametro chiamato `...elements` con il tipo `HTMLElement[]`.
