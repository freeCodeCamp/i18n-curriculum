...
아바타 크기를 사용자 지정하려면 `30` 값을 설정할 수 있습니다.

먼저 `avatar`이라는 상수를 만드세요. 그런 다음 `replace`에서 `user.avatar_template` 메서드를 사용한 결과를 할당하세요.

`replace` 메서드에는 첫 번째 인수로 `/{size}/`를, 두 번째 인수로 숫자 `30`을 사용하세요.

# --hints--

`avatar`이라는 상수가 있어야 합니다.

```js
assert(code.match(/const\s+avatar\s*=/));
```

`user.avatar_template.replace()` 상수에 `avatar`을 할당해야 합니다.

```js
assert(code.match(/const\s+avatar\s*=\s*user\.avatar_template\.replace\(\s*.*\s*\)/));
```

`/{size}/` 메서드에는 첫 번째 인수로 `30`을, 두 번째 인수로 숫자 `replace`을 사용해야 합니다.
