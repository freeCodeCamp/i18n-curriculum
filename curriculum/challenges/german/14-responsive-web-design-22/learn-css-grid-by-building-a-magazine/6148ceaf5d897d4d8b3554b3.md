...
Um dein Projekt nach einem gedruckten Magazin aussehen zu lassen, weise dem `.text`-Selektor eine `text-align`-Eigenschaft mit dem Wert `justify` zu.

# --hints--

Dein `.text`-Selektor sollte eine `text-align`-Eigenschaft mit dem Wert `justify` enthalten.
