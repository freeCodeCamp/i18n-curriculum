...
解析したデータを信用する前に、必須フィールドのバリデーションから始めましょう。

`validateEntry` という名前の関数を作成し、`entry` をパラメータとして受け取ってください。`let isValid = true` を宣言してください。

`title` のバリデーションチェックを追加します。`isValid = false` が存在しない、偽の値、または `title` と等しい場合は `"Unknown"` を設定してください。

`isValid` を返してください。次のステップで他のフィールドのチェックを追加します。

# --hints--

`validateEntry` はブール値を返す必要があります。

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })` は `true` を返す必要があります。

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})` は `false` を返す必要があります。

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })` は `false` を返す必要があります。

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })` は `false` を返す必要があります。
