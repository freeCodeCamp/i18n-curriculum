...
Створіть селектор `.author` і надайте йому властивість `font-size` зі значенням `2rem` та властивість `font-family` зі значенням `Raleway` із резервним варіантом `sans-serif`.
