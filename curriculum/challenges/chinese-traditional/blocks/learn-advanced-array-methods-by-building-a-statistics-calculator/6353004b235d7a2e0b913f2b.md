...
要計算根指數，例如 $\sqrt[n]{x}$，你可以使用倒數指數 $x^{1/n}$。JavaScript 有內建的 `Math.pow()` 函式（程式），可用來計算指數。

以下是 `Math.pow()` 函式的基本語法：
...
宣告一個 `standardDeviation` 變數，並使用 `Math.pow()` 函式（程式）將其指定為 $variance^{1/2}$ 的值。

# --hints--

你應該有一個 `standardDeviation` 變數。

```js
assert.match(getStandardDeviation.toString(), /standardDeviation\s*=/);
```

你的 `standardDeviation` 變數應該使用 `Math.pow()` 函式（程式）。

```js
assert.match(getStandardDeviation.toString(), /standardDeviation\s*=\s*Math\.pow\(/);
```

你的 `Math.pow()` 函式（程式）應該以 `variance` 為底數，並以 `1/2` 為指數。
