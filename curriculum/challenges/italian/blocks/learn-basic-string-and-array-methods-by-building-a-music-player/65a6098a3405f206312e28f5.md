...
Per ordinare le canzoni in ordine alfabetico per titolo, dovrai passare una funzione di callback di confronto al tuo metodo `sort()`.

Ecco un esempio di ordinamento di una lista di frutti per nome.
...
Nei prossimi passaggi, imparerai cosa fa ciascuna di quelle istruzioni `if` all'interno di quella funzione di callback. Ma per ora, aggiungi una funzione di callback vuota al tuo metodo `sort()` e usa `a` e `b` come nomi dei parametri.

# --hints--

Dovresti avere una funzione di callback vuota `()=>{}` all'interno del tuo metodo `sort()` con `a` e `b` come nomi dei parametri.
