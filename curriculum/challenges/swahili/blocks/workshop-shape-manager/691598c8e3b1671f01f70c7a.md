...
Ndani ya kitendakazi cha `handleShapeSelect`, fanya wito wa njia ya `.preventDefault()` kwenye `e`, na fanya wito wa kitendakazi cha `clearInputFields()` kufuta sehemu za ingizo.

# --hints--

Unapaswa kufanya wito wa `.preventDefault()` kwenye `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

Unapaswa kufanya wito wa kitendakazi cha `clearInputFields`.
