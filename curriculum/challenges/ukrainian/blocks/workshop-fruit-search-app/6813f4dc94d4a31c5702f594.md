...
Тепер, коли у вас є дані з API, оновіть стан `results`, викликавши `setResults` і передавши масив, який містить лише властивість `name` з кожного фрукта в масиві `data`. Для цього використайте метод `map()`.

# --hints--

Ви повинні оновити `setResults` за допомогою `data.map(fruit => fruit.name)`.
