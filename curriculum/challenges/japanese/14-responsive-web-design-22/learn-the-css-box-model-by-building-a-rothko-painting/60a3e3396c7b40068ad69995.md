...
`.one` セレクターに `transform` プロパティを使用して、反時計回りに 0.6 度回転 (`rotate`) させてください。

# --hints--

`transform` プロパティを `rotate(-0.6deg)` に設定する必要があります。
...
`.one` 要素の `transform` の値が `rotate(-0.6deg)` である必要があります。
