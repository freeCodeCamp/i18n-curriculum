...
`checkMessageButton`にイベントリスナーを追加し、`click`イベントを監視してください。空のコールバック関数を渡してください。

# --hints--

`.addEventListener()`要素に対して`checkMessageButton`を呼び出す必要があります。

```js
assert.match(code, /checkMessageButton\.addEventListener\(/);
```

`.addEventListener()`メソッドには`"click"`イベントタイプを指定してください。

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,/);
```

`.addEventListener()`メソッドには空のコールバック関数を指定してください。
