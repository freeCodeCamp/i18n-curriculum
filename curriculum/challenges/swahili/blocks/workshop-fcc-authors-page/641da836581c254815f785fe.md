...
Jambo linalofuata utalionesha ni maelezo ya wasifu kuhusu mwandishi. Unaweza kufanya hivi kwa kutumia sifa ya kipengele `bio`.

Ongeza kipengele cha aya chenye sifa ya kipengele `class` ya `"bio"`, kisha weka `bio` ndani ya kipengele cha aya.
...
Kipengele chako cha `p` kinapaswa kuwa na darasa la vitu `"bio"`

```js
assert.include(document.querySelector('p')?.className, 'bio');
```

Unapaswa kuweka `bio` ndani ya kipengele chako cha `p`.
