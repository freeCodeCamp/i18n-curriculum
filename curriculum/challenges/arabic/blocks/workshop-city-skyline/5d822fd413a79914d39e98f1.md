...
تريد إضافة نفس التدرج اللوني إلى القسمين التاليين. بدلًا من ذلك، أنشئ محدد صنف جديد باسم `bb1-window`، وانقل خاصيتي `height` و`background` وقيمهما من `.bb1a` إلى محدد الصنف الجديد.

# --hints--

يجب أن تنشئ محدد صنف جديد باسم `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

يجب أن تنقل خاصية `height` وقيمتها من `.bb1a` إلى `.bb1-window`.
...
يجب أن تنقل خاصية `background` وقيمتها من `.bb1a` إلى `.bb1-window`.
...
لا يجب أن تنقل خاصية `background-color` من `.bb1a`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

لا يجب أن تنقل خاصية `width` من `.bb1a`.
