...
У цьому практичному занятті ви попрактикуєтеся у роботі з атрибутами `aria-live` і `aria-label`, створюючи доступний застосунок для нотаток.

Більшість HTML і весь CSS вже надані. Перші кілька кроків полягатимуть у додаванні решти розмітки.

Якщо подивитися на попередній перегляд, там є текст, який каже натиснути на картку, щоб редагувати її. Але якщо ви спробуєте це зробити, нічого не вийде. Щоб зробити цей елемент `div` редагованим, можна використати атрибут `contenteditable` ось так:

```html
<div contenteditable="true"></div>
```

У відкриваючому тегу `div` додайте атрибут `contenteditable` і встановіть його значення `"true"`. Тепер ви зможете натиснути на елемент і редагувати текст.

# --hints--

Ваш відкриваючий тег `div` має містити атрибут `contenteditable`.

```js
assert.isTrue(document.getElementById("note").hasAttribute("contenteditable"));
```

Ваш елемент `div` має мати атрибут `contenteditable`, встановлений у `"true"`.
