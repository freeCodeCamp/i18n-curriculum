...
Sotto il tuo ultimo elemento `p`, crea un altro elemento `p` con il testo `Sodium 160mg 7%`. Metti `Sodium` e `7%` ciascuno nel proprio `span` con una classe di un `bold` come hai fatto con gli altri.

Poi, aggiungi un ulteriore elemento `span` intorno a `Sodium 160mg` per l’allineamento di nuovo.
...
Il `span` intorno a `Sodium` dovrebbe avere l’attributo `class` impostato su `bold`.
...
Un elemento `span` dovrebbe avvolgere il testo `7%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.lastElementChild?.innerText === '7%');
```

L’elemento `span` intorno a `7%` dovrebbe avere l’attributo `class` impostato su `bold`.
