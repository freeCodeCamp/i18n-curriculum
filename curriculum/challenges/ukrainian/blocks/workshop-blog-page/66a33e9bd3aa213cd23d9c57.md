...
Для цього кроку вам потрібно додати номер телефону та адресу е-пошти автора блогу.

Всередині елемента `address` додайте елемент абзац із текстом `Phone: 555-555-5555`.

Під цим елементом абзацу додайте ще один елемент абзац із текстом `Email: fake@email.com`.

# --hints--

Усередині вашого елемента `p` має бути два елементи `address`.
...
Перший елемент `p` має містити текст `Phone: 555-555-5555`.
...
Другий елемент `p` має містити текст `Email: fake@email.com`.
