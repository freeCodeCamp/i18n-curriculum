...
Dentro del primer elemento `span`, añade el texto `Question`.

En el segundo elemento `span`, añade el texto `Question`.

# --hints--

Debes dar al primer elemento `span` el texto de `Question`.

```js
assert.equal(document.querySelector('h3 > span')?.textContent, 'Question');
```

Debes dar al segundo elemento `span` el texto de `Question`.
