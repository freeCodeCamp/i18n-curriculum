...
코를 `position`의 `absolute` 속성, `top`의 `108px` 그리고 `left`의 `85px`로 위치시키세요.

# --hints--

`.cat-nose` 선택자에 `position` 속성을 `absolute`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.position === 'absolute')
```

`.cat-nose` 선택자에 `top` 속성을 `108px`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.top === '108px')
```

`.cat-nose` 선택자에 `left` 속성을 `85px`로 설정하세요.
