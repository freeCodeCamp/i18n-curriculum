...
接著，當使用者在輸入框中輸入時，你的應用程式會從 API 取得水果名稱。

首先，為你的元件添加一個 `useEffect` 掛鉤，使其在 `query` 狀態變更時執行。請確保將箭頭函式作為第一個引數傳遞給 `useEffect`，並將 `query` 包含在相依性陣列中。
...
你應該將 `query` 包含在你的 `useEffect` 掛鉤的相依性陣列中。
