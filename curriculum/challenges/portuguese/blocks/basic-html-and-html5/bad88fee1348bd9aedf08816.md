...
Elementos `a` (*âncora*) também podem ser usados para criar links internos para pular para diferentes seções dentro de uma página da web."
...
Quando os usuários clicarem no link `Contacts`, eles serão direcionados para a seção da página da web com o elemento de título **Contacts**.
...
Em seguida, adicione um atributo `id` com o valor `footer` ao elemento `<footer>` na parte inferior da página.
