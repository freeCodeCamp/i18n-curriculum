...
당신의 `addCharacters(char1)`도 또 다른 함수를 반환하는 함수를 반환하고 있습니다. `.map()` 콜백에 가장 안쪽 함수 참조에 접근하려면 또 다른 함수 호출을 해야 합니다. JavaScript는 반환된 함수를 즉시 호출할 수 있게 허용합니다:

```js
myFunc(1)("hi");
```

`addCharacters(char1)` 호출에서 반환된 함수를 즉시 호출하고, `char2`을 인수로 전달하세요.

# --hints--

`addCharacters(char1)` 호출에 함수 호출을 연결해야 합니다.
...
연결된 함수 호출에 `char2`를 인수로 전달해야 합니다.
