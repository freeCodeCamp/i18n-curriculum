...
title: "Завдання 323: визначник настрою пісні"
challengeType: 28
...
За заданим рядком жанру та числом BPM пісні визначте настрій, використовуючи таку таблицю:

| Настрій | Жанр | Діапазон BPM |
|------|-------|-----------|
...
`getMood("rock", 111)` має повернути `"happy"`.

```js
assert.equal(getMood("rock", 111), "happy");
```

`getMood("electronic", 74)` має повернути `"focus"`.

```js
assert.equal(getMood("electronic", 74), "focus");
```

`getMood("classical", 180)` має повернути `"happy"`.

```js
assert.equal(getMood("classical", 180), "happy");
```

`getMood("rock", 155)` має повернути `"hype"`.

```js
assert.equal(getMood("rock", 155), "hype");
```

`getMood("electronic", 90)` має повернути `"happy"`.

```js
assert.equal(getMood("electronic", 90), "happy");
```

`getMood("classical", 67)` має повернути `"focus"`.

```js
assert.equal(getMood("classical", 67), "focus");
```

`getMood("pop", 100)` має повернути `"happy"`.

```js
assert.equal(getMood("pop", 100), "happy");
```

`getMood("electronic", 135)` має повернути `"hype"`.
