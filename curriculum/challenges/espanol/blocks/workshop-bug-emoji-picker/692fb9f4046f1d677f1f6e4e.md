...
Crea una función llamada `isSelect` que reciba un solo parámetro `element` de tipo `EventTarget | null`. Esta función verificará si el elemento seleccionado es un elemento HTML `<select>`.

# --hints--

Debes tener una función llamada `isSelect`.
...
Tu función `isSelect` debe tener un parámetro llamado `element` de tipo `EventTarget | null`.
...
Tu función `isSelect` debe tener un tipo de retorno `element is HTMLSelectElement`.
