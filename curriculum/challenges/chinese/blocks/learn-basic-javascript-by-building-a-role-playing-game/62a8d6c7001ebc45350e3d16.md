...
现在使用 `+=` 运算符将字符串 `" In your inventory you have: "` 和 `inventory` 的内容添加到 `text.innerText` 中。 确保 `" In your inventory you have: "` 字符串的开头和结尾包含空格。

# --hints--

你应该添加另一行 `text.innerText`。
...
你应该将字符串 `" In your inventory you have: "` 添加到第二行 `text.innerText`。 间距很重要。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*\+=\s*('|") In your inventory you have: \1/);
```

你应该使用连接运算符将 `inventory` 的值添加到第二个 `text.innerText` 行的末尾。
