...
表全体をセクション要素で囲んであるので、そのセクション要素にスタイルを設定することで表の周りに境界線を作ることができます。 `section` セレクターを作成し、レスポンシブデザインにするために `max-width` プロパティを `40rem` に設定してください。 中央揃えにするため `margin` プロパティを `0 auto` に設定し、`border` プロパティは `2px solid #d0d0d5` に設定してください。
