...
이제 `playNextSong` 함수를 테스트할 시간입니다.

`"click"` 요소에 `nextButton` 이벤트 리스너를 추가한 다음, 이벤트 리스너의 두 번째 인수로 `playNextSong`을 전달하세요. 이 함수가 이벤트 리스너가 실행할 함수입니다.

먼저 재생 버튼을 클릭한 다음 다음 버튼을 클릭하여 앱을 테스트하세요. 모든 것이 예상대로 작동하는 것을 확인할 수 있을 것입니다.

# --hints--

`addEventListener()` 변수에서 `nextButton` 메서드를 호출해야 합니다.
...
`playNextSong` 메서드의 두 번째 인수로 `addEventListener`을 전달해야 합니다.
