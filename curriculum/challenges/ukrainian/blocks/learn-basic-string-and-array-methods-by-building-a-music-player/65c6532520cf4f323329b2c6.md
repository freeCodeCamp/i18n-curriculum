...
Наразі порядок пісень не змінився. Це тому, що оновлення, які ви зробили за допомогою методу `sort`, не відбудуться, доки не буде виконано виклик функції `sortSongs`.

Змініть свою функцію `renderSongs`, щоб вона викликала функцію `sortSongs`.

Тепер ви повинні побачити пісні в алфавітному порядку.

# --hints--

У вас має бути `renderSongs(sortSongs())`.
