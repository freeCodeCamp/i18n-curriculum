...
在 `singleCardSelected` 方法中，获取 `singleCard` 元素并通过调用 `innerHTML` 函数更新其 `renderCard`。将 `"Click the card and reveal the fortune"` 作为第一个参数传入，`isReversed` 作为第二个参数传入。

# --hints--

你应该通过调用 `this.elements.singleCard.innerHTML` 函数来更新 `renderCard`。
...
你的 `renderCard` 调用应以 `"Click the card and reveal the fortune"` 作为第一个参数。
...
你应该将 `isReversed` 作为第二个参数传递给 `renderCard`。
