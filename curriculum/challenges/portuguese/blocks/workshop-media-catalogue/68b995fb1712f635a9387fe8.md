...
Crie outro método chamado `get_tv_series` e faça com que ele retorne uma lista com todas as instâncias de `TVSeries` que você encontrar em `self.items`.

# --hints--

Sua classe `MediaCatalogue` deve ter um método chamado `get_tv_series`.
...
Seu método `get_tv_series` deve retornar uma lista contendo todos os itens em `self.items` que são instâncias da classe `TVSeries`.
