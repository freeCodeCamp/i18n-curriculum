...
عيّن لون خلفية `footer` إلى `#2a2a40`، واستخدم _Flexbox_ لمركزة النص أفقيًا.

# --hints--

يجب استخدام محدِّد عنصر `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

يجب أن تعطي `footer` لون خلفية `background-color` بقيمة `#2a2a40`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

يجب أن تعطي `footer` خاصية `display` بقيمة `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

يجب أن تعطي `footer` خاصية `justify-content` بقيمة `center`.
