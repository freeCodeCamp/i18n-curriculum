...
Agora faça a chamada da função `showElements` e passe `multipleCard`, `fortuneContainer` e `text` como props.

# --hints--

Você deve chamar a função `showElements` dentro do método `multipleCardSelected`.
...
Você deve passar `multipleCard` como primeiro argumento para a função `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

Você deve passar `fortuneContainer` como argumento para a função `showElements`.
...
Você deve passar `text` como argumento para a função `showElements`.
