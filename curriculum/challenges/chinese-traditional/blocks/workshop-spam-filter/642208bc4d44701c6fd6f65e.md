...
美元值可能超過一個字元。為了符合這點，可以使用 `+` 量詞－這會匹配一個或多個連續出現的字元。<dfn>例如</dfn>，正規表達式 `/a+/` 匹配一個或多個連續的 `a` 字元。

更新你的正規表達式以符合一個或多個連續的數字。
...
你的 `dollarRegex` 應該在你的 `[0-9]` 字元類別上使用 `+` 量詞。

```js
assert.match(dollarRegex.source, /\[0-9\]\+/);
```

你的 `dollarRegex` 應該仍然能匹配 `"100 dollars"`。
...
你的 `dollarRegex` 應該仍然能匹配 `"3 dollars"`。
