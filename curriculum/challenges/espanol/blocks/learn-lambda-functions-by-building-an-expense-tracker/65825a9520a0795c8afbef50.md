...
Después de la llamada a `print()`, llama a la función `print_expenses` para mostrar todos los gastos que se han agregado hasta ahora. Pasa la lista `expenses` como argumento.

# --hints--

Debes llamar a la función `print_expenses()` dentro de tu cláusula `elif`.
...
Debes pasar la lista `expenses` como argumento a tu llamada `print_expenses()`.
