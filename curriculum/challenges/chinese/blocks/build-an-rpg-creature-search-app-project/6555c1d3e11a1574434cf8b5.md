...
在这个项目中，你将构建一个应用程序，通过名称或 ID 查找 RPG 游戏中的生物，并将结果显示给用户。为了获取生物的 `data` 和 `images`，你将使用 freeCodeCamp 的 <a href="https://rpg-creature-api.freecodecamp.rocks/" target="_blank" rel="noopener noreferrer nofollow">RPG Creature API</a>。

 **注意：** 前 13 个步骤必须在 `index.html` 文件中完成。 

**目标：** 构建一个功能上类似于 <a href="https://rpg-creature-search-app.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://rpg-creature-search-app.freecodecamp.rocks</a> 的应用程序。

**用户故事：**

1. 你应该有一个 `id` 为 `"search-input"` 的 `input` 元素，并且是必填项。
1. 你应该有一个 `id` 为 `"search-button"` 的 `button` 元素。
1. 你应该有一个 `id` 为 `"creature-name"` 的元素。
1. 你应该有一个 `id` 为 `"creature-id"` 的元素。
1. 你应该有一个 `id` 为 `"weight"` 的元素。
1. 你应该有一个 `id` 为 `"height"` 的元素。
1. 你应该有一个 `id` 为 `"types"` 的元素。
1. 你应该有一个 `id` 为 `"hp"` 的元素。
1. 你应该有一个 `id` 为 `"attack"` 的元素。
1. 你应该有一个 `id` 为 `"defense"` 的元素。
1. 你应该有一个 `id` 为 `"special-attack"` 的元素。
1. 有一个 `id` 为 `"special-defense"` 的元素。
1. 你应该有一个 `id` 为 `"speed"` 的元素。
1. 当 `#search-input` 元素包含值 `Red` 且点击 `#search-button` 元素时，应弹出带有文本 `"Creature not found"` 的警告。
1. 当 `#search-input` 元素包含值 `Pyrolynx` 且点击 `#search-button` 元素时，`#creature-name`、`#creature-id`、`#weight`、`#height`、`#hp`、`#attack`、`#defense`、`#special-attack`、`#special-defense` 和 `#speed` 元素中的值应分别为 `PYROLYNX`、`#1` 或 `1`、`Weight: 42` 或 `42`、`Height: 32` 或 `32`、`65`、`80`、`50`、`90`、`55` 和 `100`。
1. 当 `#search-input` 元素包含值 `Pyrolynx` 且点击 `#search-button` 元素时，应在 `#types` 元素内添加一个包含文本 `FIRE` 的单个元素。每次查找之间应清除 `#types` 元素内容。
1. 当 `#search-input` 元素包含值 `2` 且点击 `#search-button` 元素时，`#creature-name`、`#creature-id`、`#weight`、`#height`、`#hp`、`#attack`、`#defense`、`#special-attack`、`#special-defense` 和 `#speed` 元素中的值应分别为 `AQUOROC`、`#2` 或 `2`、`Weight: 220` 或 `220`、`Height: 53` 或 `53`、`85`、`90`、`120`、`60`、`70` 和 `40`。
1. 当 `#search-input` 元素包含值 `2` 且点击 `#search-button` 元素时，应在 `#types` 元素内分别添加包含文本值 `WATER` 和 `ROCK` 的两个元素。每次查找时应清除 `#types` 元素内容。
1. 当 `#search-input` 元素包含无效的生物名称，且点击 `#search-button` 元素时，应弹出带有文本 `"Creature not found"` 的警告。
1. 当 `#search-input` 元素包含有效的生物 ID 且点击 `#search-button` 元素时，UI 应填充正确的数据。
...
你应该有一个 `id` 为 `"search-input"` 的 `input` 元素. `input` 元素应该是必填的。
...
当 `#search-input` 元素包含值 `Red` 且点击 `#search-button` 元素时，应弹出带有文本 `"Creature not found"` 的警告。
...
当 `#search-input` 元素包含值 `Pyrolynx` 且点击 `#search-button` 元素时，`#creature-name`、`#creature-id`、`#weight`、`#height`、`#hp`、`#attack`、`#defense`、`#special-attack`、`#special-defense` 和 `#speed` 元素中的值应分别为 `PYROLYNX`、`#1` 或 `1`、`Weight: 42` 或 `42`、`Height: 32` 或 `32`、`65`、`80`、`50`、`90`、`55` 和 `100`。
...
当 `#search-input` 元素包含值 `Pyrolynx` 且点击 `#search-button` 元素时，应在 `#types` 元素内添加一个包含文本 `FIRE` 的单个元素。每次查找之间应清除 `#types` 元素内容。
...
当 `#search-input` 元素包含值 `2` 且点击 `#search-button` 元素时，`#creature-name`、`#creature-id`、`#weight`、`#height`、`#hp`、`#attack`、`#defense`、`#special-attack`、`#special-defense` 和 `#speed` 元素中的值应分别为 `AQUOROC`、`#2` 或 `2`、`Weight: 220` 或 `220`、`Height: 53` 或 `53`、`85`、`90`、`120`、`60`、`70` 和 `40`。
...
当 `#search-input` 元素包含值 `2` 且点击 `#search-button` 元素时，应在 `#types` 元素内分别添加包含文本值 `WATER` 和 `ROCK` 的两个元素。每次查找时应清除 `#types` 元素内容。
...
当 `#search-input` 元素包含无效的生物名称且点击 `#search-button` 元素时，应弹出带有文本 `"Creature not found"` 的警告。
...
当 `#search-input` 元素包含有效的生物 ID 且点击 `#search-button` 元素时，UI 应填充正确的数据。
...
当点击搜索按钮时，应用应将获取请求发送到生物名称或ID的正确端点。
