...
Chini ya `value` na `checked` zilizovunjwa, sasisha orodha ya nguvu zilizochaguliwa.

Fanya wito wa kitendakazi cha `setPowers`, ambacho kinachukua hoja ya safu ya data. Tumia opereta ya ternary kuangalia kama `checked` ni kweli. Ikiwa ni kweli, sambaza safu ya data ya `powers` iliyopo ndani ya safu mpya na ongeza `value` ndani yake. Ikiwa si kweli, chuja `value` kutoka `powers` kwa kutumia `powers.filter(p => p !== value)`.

# --hints--

Unapaswa kutumia opereta ya ternary kuangalia kama `checked` ni kweli. Ikiwa ni kweli, sambaza `powers` na `value` zilizopo ndani ya safu ya data. Ikiwa si kweli, chuja `value` kutoka `powers`.
