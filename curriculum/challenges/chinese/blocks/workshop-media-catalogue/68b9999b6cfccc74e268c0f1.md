...
在你的 `for` 循环之前，创建一个 `if` 语句，当 `movies` 列表不为空时，将字串 `=== MOVIES ===`（以换行符结尾）添加到 `result` 的当前值中。

# --hints--

你应该在你的 `__str__` 方法中有一个 `if` 语句，用来查看 `movies` 列表是否不为空。
