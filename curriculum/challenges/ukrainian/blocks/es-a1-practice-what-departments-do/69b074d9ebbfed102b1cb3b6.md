---
id: 69b074d9ebbfed102b1cb3b6
title: Завдання 9
challengeType: 19
dashedName: task-9
lang: es
---

<!-- (Audio) El departamento de ingeniería desarrolla el software. El departamento de datos y analítica analiza la información. El departamento de producto y gestión de proyectos organiza los proyectos. El departamento de diseño y experiencia de usuario diseña los productos. El departamento de marketing promueve la empresa. El departamento de control de calidad verifica el software. El departamento de recursos humanos apoya a los empleados. El departamento de aprendizaje y desarrollo capacita al equipo. -->

# --description--

Практикуйте визначення, чим займається кожен відділ у компанії.

# --instructions--

Прослухайте аудіо та дайте відповідь на запитання нижче.

# --questions--

## --text--

Який відділ просуває компанію?

## --answers--

`El departamento de ingeniería.`

### --feedback--

Цей відділ створює або розробляє програмне забезпечення, а не просуває компанію.

---

`El departamento de marketing.`

---

`El departamento de recursos humanos.`

### --feedback--

Цей відділ підтримує працівників, а не просуває компанію.

---

`El departamento de control de calidad.`

### --feedback--

Цей відділ перевіряє або контролює програмне забезпечення, а не просуває компанію.

## --video-solution--

2

# --explanation--

Правильна відповідь — `el departamento de marketing`, тому що Себастьян каже `el departamento de marketing promueve la empresa`, що означає "відділ маркетингу просуває компанію".

# --scene--

```json
{
  "setup": {
    "background": "company1-reception.png",
    "characters": [
      {
        "character": "Sebastián",
        "position": {
          "x": 50,
          "y": 25,
          "z": 1.5
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "ES_A1_what_each_department_does.mp3",
      "startTime": 1,
      "startTimestamp": 0.29,
      "finishTimestamp": 48.26
    }
  },
  "commands": [
    {
      "character": "Sebastián",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sebastián",
      "startTime": 1,
      "finishTime": 5.47,
      "dialogue": {
        "text": "El departamento de ingeniería desarrolla el software.",
        "align": "center"
      }
    },
    {
      "character": "Sebastián",
      "startTime": 6.63,
      "finishTime": 11.84,
      "dialogue": {
        "text": "El departamento de datos y analítica analiza la información.",
        "align": "center"
      }
    },
    {
      "character": "Sebastián",
      "startTime": 13.2,
      "finishTime": 18.96,
      "dialogue": {
        "text": "El departamento de producto y gestión de proyectos organiza los proyectos.",
        "align": "center"
      }
    },
    {
      "character": "Sebastián",
      "startTime": 20.45,
      "finishTime": 25.94,
      "dialogue": {
        "text": "El departamento de diseño y experiencia de usuario diseña los productos.",
        "align": "center"
      }
    },
    {
      "character": "Sebastián",
      "startTime": 27.14,
      "finishTime": 31.06,
      "dialogue": {
        "text": "El departamento de marketing promueve la empresa.",
        "align": "center"
      }
    },
    {
      "character": "Sebastián",
      "startTime": 32.34,
      "finishTime": 37.15,
      "dialogue": {
        "text": "El departamento de control de calidad verifica el software.",
        "align": "center"
      }
    },
    {
      "character": "Sebastián",
      "startTime": 38.31,
      "finishTime": 43.19,
      "dialogue": {
        "text": "El departamento de recursos humanos apoya a los empleados.",
        "align": "center"
      }
    },
    {
      "character": "Sebastián",
      "startTime": 43.94,
      "finishTime": 48.97,
      "dialogue": {
        "text": "El departamento de aprendizaje y desarrollo capacita al equipo.",
        "align": "center"
      }
    },
    {
      "character": "Sebastián",
      "opacity": 0,
      "startTime": 49.47
    }
  ]
}
```
