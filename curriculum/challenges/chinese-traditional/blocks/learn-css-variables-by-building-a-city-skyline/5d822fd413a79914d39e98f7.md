...
從 `.bb1d` 漸層中移除 `orange` 並將 `80%` 改為 `50%`。這將使 `--building-color1` 在上半部變為實心，然後過渡到下半部的 `--window-color1`。

# --hints--

你應該從 `linear-gradient` 中移除 `orange`。

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'orange');
```

你應該將現在第一個 `linear-gradient` 顏色改為在 `50%` 過渡。
