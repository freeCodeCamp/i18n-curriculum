...
`div.form-group` yako inapaswa kuwa na `label` yenye sifa ya kipengele `htmlFor` ya `imageUrl`, `className` ya `label`, na maandishi ya `Image URL`.
...
URL ya Picha yako ya `form-group` inapaswa kuwa na `input` yenye `id` ya `imageUrl`, `className` ya `input`, na `type` ya `text`.
