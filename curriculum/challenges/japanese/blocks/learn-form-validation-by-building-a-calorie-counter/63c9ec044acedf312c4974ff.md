...
ループが完了したら、`budgetNumberInput`をクリアする必要があります。`value`の`budgetNumberInput`プロパティを空の文字列に設定してください。

# --hints--

`clearForm`関数は、`value`要素の`budgetNumberInput`プロパティにアクセスする必要があります。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value/);
```

`clearForm`関数は、`value`要素の`budgetNumberInput`プロパティを空の文字列に設定する必要があります。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value\s*=\s*('|"|`)\1/);
```

`budgetNumberInput`ループの後に`for`を修正してください。
