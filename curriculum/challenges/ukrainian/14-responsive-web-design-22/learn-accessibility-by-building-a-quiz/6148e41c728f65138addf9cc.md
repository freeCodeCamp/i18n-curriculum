...
Певні типи анімації на основі руху можуть викликати дискомфорт у деяких користувачів. Зокрема, люди з <dfn>вестибулярними</dfn> порушеннями чутливі до певних фізичних рухів.

В-правило `@media` має медіавластивість `prefers-reduced-motion` для встановлення CSS на основі вподобань користувача. Воно може приймати одне з таких значень:
...
Обгорніть правило стилю, яке встановлює `scroll-behavior: smooth` в межах директиви `@media`, за допомогою `prefers-reduced-motion` зі значенням `no-preference`.

Хороша робота. Ви завершили створення вікторини «Доступність».

# --hints--

Ви повинні мати одне правило `@media (prefers-reduced-motion: no-preference)`.
