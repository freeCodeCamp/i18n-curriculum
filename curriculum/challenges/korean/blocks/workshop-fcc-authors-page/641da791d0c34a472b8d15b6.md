...
이제 `authorDataArr`는 아까 콘솔에 기록한 `data`와 같습니다. 이를 확인하려면 `authorDataArr`를 콘솔에 기록하세요.

`console.log()` 문장 안에 텍스트 `"Author Data Array:"`를 첫 번째 인수로, `authorDataArr`를 두 번째 인수로 추가하세요. 텍스트와 `authorDataArr`는 쉼표로 구분하세요.
...
텍스트 `"Author Data Array:"`가 포함된 콘솔 로그가 있어야 합니다.

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1/)
```

`"Author Data Array:"` 텍스트와 `authorDataArr`를 쉼표로 구분해야 합니다.

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1\s*,/)
```

`authorDataArr`는 콘솔 로그 문장의 두 번째 인수여야 합니다.
