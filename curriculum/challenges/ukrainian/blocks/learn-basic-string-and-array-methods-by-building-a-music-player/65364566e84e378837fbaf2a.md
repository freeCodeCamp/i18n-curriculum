...
У попередніх кроках ви реалізували функціонал для відтворення пісні. Тепер потрібно додати функціонал до кнопки відтворення, щоб вона запускала поточну пісню при натисканні.

Використайте метод `addEventListener()` і передайте подію `"click"` як перший аргумент, а порожню функцію зворотного виклику у вигляді стрілкової функції як другий аргумент, наприклад `() => {}`.

# --hints--

Ви повинні викликати метод `addEventListener()` на вашій змінній `playButton`.
...
Використайте стрілковий синтаксис, щоб передати порожню функцію зворотного виклику у ваш слухач подій. Не забудьте також про порожні фігурні дужки.
