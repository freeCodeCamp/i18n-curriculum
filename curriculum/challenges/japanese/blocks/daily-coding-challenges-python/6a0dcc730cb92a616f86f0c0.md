...
`is_valid_schema({"username": "alice", "posts": 10, "verified": False})` は `True` を返すべきです。
...
`is_valid_schema({"username": "carol", "posts": 15, "verified": True, "followers": 25})` は `True` を返すべきです。
...
`is_valid_schema({"username": "frank", "posts": "21", "verified": True})` は `False` を返すべきです。
...
`is_valid_schema({"username": "sam", "posts": 17, "verified": "false"})` は `False` を返すべきです。
...
`is_valid_schema({"username": "bill", "verified": True})` は `False` を返すべきです。
...
`is_valid_schema({"username": "fred", "verified": True})` は `False` を返すべきです。
...
`is_valid_schema({"username": 5, "posts": 10, "verified": True})` は `False` を返すべきです。
