...
Escreva uma função que retorne as `n` primeiras palavras de Fibonacci. O número `n` será fornecido como um parâmetro para a função. A função deve retornar um array de objetos. Os objetos devem estar no formato: `{ N: 1, Length: 1, Entropy: 0, Word: '1' }`. `Entropy` é calculado para a string `Word` e arredondado para 8 dígitos decimais de precisão. Observe que os índices dessa sequência começam em `1`.
