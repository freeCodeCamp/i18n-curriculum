...
`.first-paragraph::first-letter` セレクターの `font-size` プロパティを `6rem` に設定してください。 また、`color` プロパティを `orangered` に設定して目立たせてください。

# --hints--

`.first-paragraph::first-letter` セレクターが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

`.first-paragraph::first-letter` セレクターの `font-size` プロパティを `6rem` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

`.first-paragraph::first-letter` セレクターの `color` プロパティを `orangered` に設定する必要があります。
