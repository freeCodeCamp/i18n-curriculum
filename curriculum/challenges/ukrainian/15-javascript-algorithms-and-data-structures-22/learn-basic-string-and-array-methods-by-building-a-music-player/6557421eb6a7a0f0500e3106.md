...
В межах елемента кнопки у функції `renderSongs` додайте атрибут `onclick`. Для значення, викличте функцію `deleteSong` та інтерполюйте `song.id`.

# --hints--

Додайте атрибут `onclick` до кнопки видалення та передайте `"deleteSong(${song.id})"`.
