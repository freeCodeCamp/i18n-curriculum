...
입력된 숫자가 음수가 아닌지 확인하는 함수를 만들어야 합니다.

이를 위해 `handleInput` 함수에 `e` 매개변수를 `Event` 타입으로 만드세요. 그 안에 `value` 변수를 생성하고 target 값으로 설정하세요. 숫자로 변환하고 `HTMLInputElement` 키워드를 사용해 `as` 타입으로 형변환하는 것을 잊지 마세요.

# --hints--

`handleInput` 함수를 만들어야 합니다.

```js
assert.isFunction(handleInput)
```

`handleInput` 함수는 `e` 매개변수를 `Event` 타입으로 가져야 합니다.
