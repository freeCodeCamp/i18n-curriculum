...
title: 'Zoezi la 73: Kuhesabu sehemu katika safu'
challengeType: 1
forumTopicId: 302186
...
Fikiria sehemu, $\frac{n}{d}$, ambapo `n` na `d` ni nambari kamili chanya. Ikiwa `n` &lt; `d` na kigezo cha juu kabisa cha pamoja, ${HCF}(n, d) = 1$, inaitwa sehemu sahihi iliyorahisishwa.

Ikiwa tutaorodhesha seti ya sehemu sahihi iliyorahisishwa kwa `d` ≤ 8 kwa mpangilio wa ukubwa kuongezeka, tunapata:
...
Inaonekana kwamba kuna sehemu `3` kati ya $\frac{1}{3}$ na $\frac{1}{2}$.

Ni sehemu ngapi ziko kati ya $\frac{1}{3}$ na $\frac{1}{2}$ katika seti iliyopangwa ya sehemu sahihi iliyorahisishwa kwa `d` ≤ `limit`?

# --hints--

`countingFractionsInARange(8)` inapaswa kurudisha nambari.

```js
assert(typeof countingFractionsInARange(8) === 'number');
```

`countingFractionsInARange(8)` inapaswa kurudisha `3`.

```js
assert.strictEqual(countingFractionsInARange(8), 3);
```

`countingFractionsInARange(1000)` inapaswa kurudisha `50695`.
...
`countingFractionsInARange(12000)` inapaswa kurudisha `7295372`.
