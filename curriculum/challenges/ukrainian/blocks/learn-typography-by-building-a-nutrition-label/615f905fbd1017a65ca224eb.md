...
Додайте середній роздільник після вашого елемента `.daily-value`. Нижче цього нового роздільника створіть елемент `p` з атрибутом `class`, встановленим у `note`.

Додайте елементу `p` наступний текст:
...
Ви повинні створити новий елемент `div` після вашого елемента `.daily-value`.

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

Ваш новий елемент `div` має мати атрибут `class`, встановлений у `divider medium`.
...
Ви повинні створити елемент `p` після вашого нового елемента `div`.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'p');
```

Ваш новий елемент `p` має мати атрибут `class`, встановлений у `note`.
