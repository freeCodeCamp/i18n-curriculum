...
Weka thamani ya sifa ya `conversionTable` kwa `cup` yako, na ipe thamani ya kitu chenye sifa ya `gram` iliyowekwa kuwa `240`, sifa ya `ounce` iliyowekwa kuwa `8.0`, na sifa ya `teaspoon` iliyowekwa kuwa `48`.

# --hints--

`conversionTable` yako inapaswa kuwa na sifa ya `cup`.

```js
assert.property(conversionTable, "cup");
```

Sifa yako ya `cup` inapaswa kuwa kitu.

```js
assert.isObject(conversionTable.cup);
```

Kitu chako cha `cup` kinapaswa kuwa na sifa ya `gram`.

```js
assert.property(conversionTable.cup, "gram");
```

Sifa yako ya `gram` inapaswa kuwekwa kuwa `240`.

```js
assert.strictEqual(conversionTable.cup?.gram, 240);
```

Kitu chako cha `cup` kinapaswa kuwa na sifa ya `ounce`.

```js
assert.property(conversionTable.cup, "ounce");
```

Sifa yako ya `ounce` inapaswa kuwekwa kuwa `8.0`.

```js
assert.strictEqual(conversionTable.cup?.ounce, 8.0);
```

Kitu chako cha `cup` kinapaswa kuwa na sifa ya `teaspoon`.

```js
assert.property(conversionTable.cup, "teaspoon");
```

Sifa yako ya `teaspoon` inapaswa kuwekwa kuwa `48`.
