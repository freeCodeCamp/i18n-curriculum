...
在事件監聽器的箭頭函數中，添加 `if` 來檢查 `userData?.currentSong` 是否是 `null`。

在 `if` 塊內，使用 `userData?.songs` 數組中第一首歌曲的 `id` 調用 `playSong()` 函數。 這將確保播放列表中的第一首歌曲首先播放。
...
你應該在 `if` 塊內使用 `userData?.songs[0].id` 調用 `playSong` 函數。
