...
Настав час почати працювати з іншими кнопками. Решта елементів `button` вже додані для вас у HTML-файлі.

Для кожної нової кнопки (`#confused-btn`, `#sad-btn` і `#loving-btn`) виберіть її за допомогою `querySelector` з її ідентифікатором, а потім додайте слухача події click, який викликає `updateCount` з цією кнопкою.

Використовуйте імена змінних `confusedBtn`, `sadBtn`, `lovingBtn`.

# --hints--

Ви повинні створити змінну з ім’ям `confusedBtn`.

```js
assert.exists(confusedBtn);
```

Ви повинні зберегти елемент `#confused-btn` у змінній `confusedBtn`.

```js
assert.equal(confusedBtn, document.querySelector('#confused-btn'));
```

Ви повинні створити змінну з ім’ям `sadBtn`.

```js
assert.exists(sadBtn);
```

Ви повинні зберегти елемент `#sad-btn` у змінній `sadBtn`.

```js
assert.equal(sadBtn, document.querySelector('#sad-btn'));
```

Ви повинні створити змінну з ім’ям `lovingBtn`.

```js
assert.exists(lovingBtn);
```

Ви повинні зберегти елемент `#loving-btn` у змінній `lovingBtn`.

```js
assert.equal(lovingBtn, document.querySelector('#loving-btn'));
```

Елемент `#confused-btn` має мати слухача події click, який викликає `updateCount`.
...
Елемент `#sad-btn` має мати слухача події click, який викликає `updateCount`.
...
Елемент `#loving-btn` має мати слухача події click, який викликає `updateCount`.
