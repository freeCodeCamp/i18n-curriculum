...
Create a new variable called `--window-color4` in `:root` and give it a value of `#8cb3d9`. 這將是最後一個背景建築的間色。

# --hints--

你應該定義一個新的屬性變量 `--window-color4`。

```js
assert(new __helpers.CSSHelp(document).isPropertyUsed("--window-color4"));
```

你應該將 `--window-color4` 的值設置爲 `#8cb3d9`。
