...
احذف الكلمة المفتاحية `pass`. في كتلة شفرة حلقة `while`، عيّن `left_part[left_array_index]` إلى `array[sorted_index]`.

# --hints--

يجب أن تحتوي جسم الحلقة على `array[sorted_index] = left_part[left_array_index]`. لا تنس حذف الكلمة المفتاحية `pass` من جسم الحلقة.
