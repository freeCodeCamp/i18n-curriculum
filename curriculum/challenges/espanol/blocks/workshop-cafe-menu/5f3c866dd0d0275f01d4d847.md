...
Bueno, eso no funcionó. Estilizar los elementos `p` como `inline-block` y colocarlos en líneas separadas crea un espacio adicional a la derecha del primer elemento `p`, causando que el segundo pase a la siguiente línea.

Una forma de solucionar eso es hacer que el ancho de cada elemento `p` sea ligeramente menor que `50%`. Entonces, cambia el valor de `width` a `49%` para cada clase para ver qué sucede.
