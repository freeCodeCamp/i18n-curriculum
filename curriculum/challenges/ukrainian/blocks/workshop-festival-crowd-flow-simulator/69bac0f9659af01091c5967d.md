...
Корисно мати функцію для відображення підсумку пропускної здатності під час симуляції. Створіть порожню функцію з назвою `printSummary` з параметром `summary`.

# --hints--

Ви повинні створити функцію з назвою `printSummary`.

```js
assert.isFunction(printSummary);
```

Ваша функція `printSummary` має мати параметр `summary`.
