...
title: Eine Flashcard-Quiz-App erstellen
challengeType: 25
...
In diesem Labor erstellen Sie eine App, die Flashcard-Daten anzeigt und speichert, die später abgerufen werden können.

**Ziel:** Erfüllen Sie die untenstehenden User Stories und bestehen Sie alle Tests, um das Labor abzuschließen.

**User Stories:**

1) Sie sollten ein HTML-Element mit der ID `flashcard` haben.  
2) Sie sollten eine Schnittstelle namens `FlashCard` haben.  
3) Die `FlashCard`-Schnittstelle sollte die folgenden Eigenschaften `questionText` und `questionAnswer` enthalten, beide vom Typ `string`.  
4) Sie sollten eine Sammlung von `FlashCard`-Elementen namens `currentCards` haben.  
5) Wenn auf das `#flashcard`-Element geklickt wird, sollte die Karte die Klasse `flipped` erhalten.  
6) Sie sollten ein Element mit der ID `delete-btn` haben.  
7) Wenn auf das `#delete-btn`-Element geklickt wird, sollte die aktuelle Karte entfernt und die vorherigen Kartendaten angezeigt werden.  
8) Sie sollten ein Eingabeformular mit der ID `entry-form` erstellen, um beim Absenden weitere Flashcards zur `currentCards`-Sammlung hinzuzufügen.  
9) Die beiden Elemente im Formular sollten jeweils die IDs `front-text` und `back-text` haben.  
10) Sie sollten eine `InvalidUserInputError` aufrufen, wenn entweder der Fragetext oder die Antwort im Eingabeformular leer ist.

# --hints--

Sie sollten ein HTML-Element mit der ID `flashcard` haben.
...
Sie sollten eine `FlashCard`-Schnittstelle haben.
...
Die `FlashCard`-Schnittstelle sollte eine `questionText`-Eigenschaft vom Typ `string` haben.
...
Die `FlashCard`-Schnittstelle sollte eine `questionAnswer`-Eigenschaft vom Typ `string` haben.
...
Sie sollten eine Sammlung von `FlashCard`-Elementen namens `currentCards` mit dem Typ `FlashCard[]` haben.
...
Wenn das `#flashcard`-Element zum ersten Mal angeklickt wird, sollte das Element die Klasse `flipped` erhalten.
...
Sie sollten ein Element mit der ID `delete-btn` haben.
...
Wenn auf das `delete-btn` geklickt wird, sollte ein Flashcard-Element aus der `currentCards`-Sammlung entfernt werden.
...
Wenn auf das `delete-btn` geklickt wird, sollten die vorherigen Flashcard-Daten angezeigt werden.
...
Sie sollten ein Eingabeformular mit der ID `entry-form` erstellen.
...
Sie sollten zwei `textarea`-Elemente mit den IDs `front-text` und `back-text` jeweils im Formular haben.
...
Eine `InvalidUserInputError` sollte geworfen werden, wenn entweder der Fragetext oder die Antwort im Eingabeformular leer ist.
