...
У класі `Spider` реалізуйте метод `render`, префіксований ключовим словом `override`, щоб оновити властивість `innerText` об’єкта `emojiElement` значенням властивості `emoji`.

# --hints--

Ваш клас `Spider` має мати метод `render`.
...
Ваш метод `Spider` `render` має використовувати ключове слово `override`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?override\s+render/);
```

Ваш метод `Spider` `render` має встановити `this.emojiElement.innerText` у `this.emoji`.
