...
Se clicchi sugli input radio, potresti notare che entrambi gli input all'interno dello stesso fieldset true/false possono essere selezionati contemporaneamente.

Raggruppa gli input rilevanti in modo che possa essere selezionato solo un input per volta da ogni coppia.
...
Dovresti dare al quarto `input` un attributo `name`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label > input')?.[3]?.name);
```

Dovresti dare al secondo `input` un attributo `name` che corrisponda all'attributo `name` del primo `input`.
...
Dovresti dare al quarto `input` un attributo `name` che corrisponda all'attributo `name` del terzo `input`.
...
Dovresti dare attributi `name` diversi al primo `input` e al terzo `input`.
