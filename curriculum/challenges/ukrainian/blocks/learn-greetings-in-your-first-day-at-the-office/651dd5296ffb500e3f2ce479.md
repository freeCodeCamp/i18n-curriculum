...
Contractions are commonly used to make speech sound more natural and fluent. `You're` є скороченням `you are`. This contraction is a combination of the pronoun `you` and the verb `are`, which is part of the verb `to be`.

Дієслово `to be` є неправильним. When conjugated with `you`, it becomes `are`. For example, `You are an English learner`.
