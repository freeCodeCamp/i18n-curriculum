...
仅当用户确认提示时，你才需要清空购物车。 创建一个 `if` 语句来检查用户是否确认提示。

在 `if` 语句中，将 `items` 属性设置回空数组，然后将 `total` 属性设置为 `0`。

# --hints--

你应该创建一个 `if` 语句来检查用户是否确认了提示。 请记住，你可以直接检查 `isCartCleared` 的真实性。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

你的 `if` 语句应将 `items` 属性设置回空数组。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

你的 `if` 语句应将 `total` 属性设置为 `0`。
