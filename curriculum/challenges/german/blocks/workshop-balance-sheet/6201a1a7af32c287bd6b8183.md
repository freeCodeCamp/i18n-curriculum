...
Wählen Sie Ihre `td`-Elemente mit dem `class`-Wert `current` aus, und machen Sie die Schriftart kursiv.

# --hints--

Sie sollten einen `td.current`-Selektor haben.
...
Ihr `td.current`-Selektor sollte die `font-style`-Eigenschaft auf `italic` gesetzt haben.
