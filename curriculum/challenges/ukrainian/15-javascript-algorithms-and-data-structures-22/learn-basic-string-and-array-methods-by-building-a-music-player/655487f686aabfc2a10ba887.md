...
Перш ніж почати працювати над відтворенням наступної та попередньої пісні, вам потрібно отримати індекс кожної пісні у властивості `songs` з `userData`.

Почніть зі створення стрілкової функції під назвою `getCurrentSongIndex`.

# --hints--

Ви повинні мати функцію `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Функція `getCurrentSongIndex` повинна використовувати стрілковий синтаксис.
