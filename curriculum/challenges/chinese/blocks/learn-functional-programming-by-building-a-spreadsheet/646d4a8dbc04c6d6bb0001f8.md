...
添加一个 `range` 属性，生成从 `nums` 中的第一个数字到 `nums` 中的第二个数字的范围。 请记住，你有一个可以在此处重复使用的 `range` 函数。

# --hints--

你的 `spreadsheetFunctions` 对象应该具有 `range` 属性。
...
你的 `range` 函数应该返回从参数中的第一个数字到参数中的第二个数字的连续数字数组。
