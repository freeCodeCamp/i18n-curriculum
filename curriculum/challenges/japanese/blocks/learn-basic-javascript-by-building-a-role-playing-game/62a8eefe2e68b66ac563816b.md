...
あなたの`defeatMonster`関数内で、`gold`に`gold`とモンスターのレベルに`6.7`を掛けた値の合計を代入してください。モンスターのレベルは`monsters[fighting].level`を使って取得できます。

`num`に`num`と`5` * `8`を足した値を設定する例はこちらです：`num += 5 * 8`。結果を切り捨てるには`Math.floor()`を使ってください。

# --hints--

`gold`を変更するには複合代入を使うことをお勧めします。

```js
assert.match(defeatMonster.toString(), /gold\s*\+=/);
```

モンスターのレベルに`Math.floor()`を掛けた結果を丸めるには`6.7`を使ってください。
...
`Math.floor(monsters[fighting].level * 6.7)`の結果を`gold`に加算してください。
