...
最后，使用字符串插值将段落元素的文本设置为 `decimalToBinary(${currVal})`，其中 `currVal` 是当前对象的 `inputVal` 属性。 此后，通过在数字输入中输入数字 `5` 并单击 `Convert` 按钮来测试你的代码。

# --hints--

你应该使用字符串插值将段落元素的文本设置为 `decimalToBinary(${obj.inputVal})`。
