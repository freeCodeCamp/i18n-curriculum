...
Se han añadido el resto de las tarjetas del catálogo. Observa las tres entradas cerca del final con campos faltantes; las manejarás en un paso posterior.

`parseCard` tomará una cadena de catálogo sin procesar y devolverá un objeto estructurado con cuatro propiedades: `title`, `author`, `year` y `location`. Por ahora, crea una función `parseCard` con un parámetro llamado `rawString`.

# --hints--

Deberías tener una función llamada `parseCard`.

```js
assert.isFunction(parseCard);
```

`parseCard` debe ser una función con un parámetro llamado `rawString`.
