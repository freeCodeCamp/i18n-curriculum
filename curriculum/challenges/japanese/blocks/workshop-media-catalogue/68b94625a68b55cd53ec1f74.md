...
ドックストリングは `__doc__` 属性を通じてアクセスでき、これはデフォルトでは `None` に設定されています。

`movie1.__doc__` と `series1.__doc__` を出力して確認してください。`__doc__` 属性はどの子クラスにも継承されないことに注意してください。

# --hints--

`movie1.__doc__` を出力してください。
