...
Додайте два елементи `span` з відповідними `className` `stat-value` та `stat-label` до кожного `div.stat-row`.

# --hints--

Кожен `.stat-row` має містити `span` з `className` `stat-value` та `span` з `className` `stat-label`.
