...
El proceso de renderizado consta de tres etapas: activación, renderizado y compromiso. Veamos esto con más detalle.
...
Para el componente `Counter`, la etapa de renderizado es el momento en que React ejecuta las funciones nuevamente con el nuevo valor de `count`. React recalcula cómo debería verse la parte `<h1>{count}</h1>` del componente basándose en el valor actualizado de `count`, pero no verás ningún cambio en la pantalla hasta la siguiente etapa: commit.
