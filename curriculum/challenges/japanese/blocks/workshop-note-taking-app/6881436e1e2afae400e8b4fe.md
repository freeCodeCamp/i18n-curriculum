...
現在、ユーザーがノートを編集してノートの外をクリックすると、`currentContent` 変数が更新されます。しかし、ユーザーがノートにフォーカスして編集せずに離れる場合はどうでしょうか？

この場合、そのユースケースを処理する条件を追加できます。

`currentContent = newContent;` の行の上に、`if` が `currentContent` と等しいかどうかをチェックする `newContent` 文を追加してください。もしそうなら、return してください。

# --hints--

`if` が `currentContent` と等しいかどうかをチェックする `newContent` 文を追加してください。もしそうなら、return してください。
