...
現在再生中の曲と`playButton`要素の見た目を更新する必要があります。

`song`オブジェクトの`currentSong`プロパティに`userData`を代入してください。

*注意*: このステップではオプショナルチェーン演算子`?.`を使わないでください。なぜならこの時点で`userData.currentSong`は`null`でも`undefined`でもないからです。

# --hints--

`song`を`userData.currentSong`に代入してください。
