...
Cree otro elemento `p` debajo de su gran divisor. Dé al elemento `p` el texto `Vitamin D 2mcg 10%`.

El elemento `p` contiene solo texto, puede envolver el porcentaje en un elemento `span` para que se considere una entidad separada del resto del texto y se mueva a la derecha.
...
Su elemento `span` no debe estar en negrita.
