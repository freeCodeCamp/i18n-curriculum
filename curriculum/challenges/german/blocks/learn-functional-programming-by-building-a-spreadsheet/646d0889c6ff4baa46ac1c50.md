...
Deklarieren Sie in Ihrer `elemValue`-Funktion eine Funktion namens `inner`, die einen `character`-Parameter entgegennimmt.

Geben Sie dann Ihre `inner`-Funktion zurück.

# --hints--

Sie sollten eine `inner`-Variable deklarieren.
...
Sie sollten `const` verwenden, um Ihre `inner`-Variable zu deklarieren.
...
Ihre `inner`-Variable sollte eine Pfeilfunktion sein.
...
Ihre `inner`-Funktion sollte `character` als einzigen Parameter haben.
...
Ihre `inner`-Funktion sollte leer sein.
...
Sie sollten Ihre `inner`-Funktion explizit zurückgeben.
