...
在上面的迴圈中，`callable()` 是一個內建函式，當傳入的物件可以像函式或方法一樣被呼叫時，會回傳 `True`，否則回傳 `False`。透過檢查 `not callable(getattr(person, attr))`，迴圈會跳過方法，只列印像 `name` 和 `age` 這樣的資料屬性。
...
錯誤輸出發生是因為 `inventory_id` 類別及其實體中缺少 `Product`。
