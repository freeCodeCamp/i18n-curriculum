...
Das Häkchen in jedem `span` am Anfang eines Listenelements ist dekorativ. Fügen Sie diesen `aria-hidden`-Elementen `span` hinzu, um sie für Screenreader auszublenden.

Weisen Sie außerdem jedem der `text-green-700`-Elemente die Klassen `mr-2` und `span` zu.

# --hints--

Ihre beiden `span`-Elemente sollten das `aria-hidden`-Attribut haben.
...
Ihre beiden `span`-Elemente sollten die Klasse `text-green-700` haben.
...
Ihre beiden `span`-Elemente sollten die Klasse `mr-2` haben.
