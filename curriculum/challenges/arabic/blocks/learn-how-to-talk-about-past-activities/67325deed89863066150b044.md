---
id: 67325deed89863066150b044
title: المهمة 113
challengeType: 22
dashedName: task-113
lang: en-US
---

<!-- (audio) James: It's interesting to think about it. I used to just write code without thinking much about the end user. -->

# --instructions--

استمع إلى التسجيل الصوتي وأكمل الجملة أدناه.

# --fillInTheBlank--

## --sentence--

`It's interesting to think about it. I BLANK just write code without BLANK about the end user.`

## --blanks--

`used to`

### --feedback--

هذا الفراغ يتكون من كلمتين. الأولى فعل، والثانية حرف جر. معًا، تدلان على عادة أو فعل في الماضي لم يعد يحدث الآن.

---

`thinking much`

### --feedback--

هذا الفراغ يتكون من كلمتين. الأولى فعل ينتهي بـ `-ing`، والثانية حال. هذه العبارة تعني إعطاء الانتباه أو التفكير في شيء ما.

# --explanation--

`Used to` تشير إلى عادة أو فعل كان منتظمًا في الماضي ولم يعد مستمرًا في الحاضر.

`Thinking much` تعني إعطاء القليل من الانتباه أو التفكير.

# --scene--

```json
{
  "setup": {
    "background": "interview-room3.png",
    "characters": [
      {
        "character": "James",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_3-3.mp3",
      "startTime": 1,
      "startTimestamp": 29.28,
      "finishTimestamp": 34.44
    }
  },
  "commands": [
    {
      "character": "James",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "James",
      "startTime": 1,
      "finishTime": 2.84,
      "dialogue": {
        "text": "It's interesting to think about it.",
        "align": "center"
      }
    },
    {
      "character": "James",
      "startTime": 2.92,
      "finishTime": 6.16,
      "dialogue": {
        "text": "I used to just write code without thinking much about the end user,",
        "align": "center"
      }
    },
    {
      "character": "James",
      "opacity": 0,
      "startTime": 6.66
    }
  ]
}
```
