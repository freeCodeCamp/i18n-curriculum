...
今度は、`for` ループを作成して、3x3 の正方形内の行だけを反復してください。`range()` 関数を使って `row_start` から始まるシーケンスを生成し、ループ変数として `row_no` を使います。

# --hints--

`for` ループを作成して、`range(row_start, row_start + 3)` を反復してください。
...
`for` ループは `row_no` をループ変数として使うべきです。
