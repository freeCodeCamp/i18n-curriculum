...
Depois do elemento `img`, adicione um elemento `h1` com a `class` definida como `hero-title` e o texto definido como `OUR NEW CURRICULUM`, seguido de um elemento `p` com a `class` definida como `hero-subtitle` e o texto definido como `Our efforts to restructure our curriculum with a more project-based focus`.
