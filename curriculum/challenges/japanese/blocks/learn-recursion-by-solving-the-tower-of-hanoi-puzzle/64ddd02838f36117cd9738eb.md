...
`else` ブロックの外側に、`display our progress` とコメントを追加し、リストの内容を出力してすべてが正しく動作しているか確認してください。

# --hints--

提供されたテキストを使って、`else` ブロックの外側にコメントを追加してください。

```js
({ test: () => assert.match(code, /rods\s*\[\s*source\s*\]\s*\.append\s*\(\s*rods\s*\[\s*target\s*\]\s*\.pop\s*\(\s*\)\s*\)\s+#\s*display\sour\sprogress/) })
```

コメントの後に、`rods` ブロックの外側で `else` オブジェクトを出力してください。
