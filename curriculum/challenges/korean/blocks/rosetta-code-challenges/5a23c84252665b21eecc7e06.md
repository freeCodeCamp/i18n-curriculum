...
주어진 직사각형은 *m* × *n* 개의 정사각형으로 이루어져 있습니다. 만약 *m*과 *n*이 모두 홀수가 아니라면, 정사각형의 모서리를 따라 경로를 잘라 직사각형을 두 개의 연결된 조각으로 나눌 수 있으며, 이 두 조각은 한 조각을 180° 회전시킨 후에 같은 모양이 됩니다. 2 × 2와 4 × 3 직사각형에 대한 모든 가능한 경로가 아래에 표시되어 있습니다.
...
<div style="width: 100%; text-align: center;">
  <svg xmlns="https://www.w3.org/2000/svg" xmlns:xlink="https://www.w3.org/1999/xlink" width="520" height="170" aria-hidden="true" alt="Diagram showing the possible paths for 2 by 2 and 4 by 3 rectangles">
    <style>
.diagram-g { 채우기: 없음; 선: #ccc }
      .diagram-s, .diagram-s2 { 채우기: #bff; 선: 검정; 채우기-불투명도: .4 }
      .diagram-s2 { 채우기: #fbf }
      .diagram-d { 선: 검정; 채우기: 없음}
    </style>
    <defs>
      <g id="diagram-m">
        <g id="diagram-h4">
...
*m* × *n* 직사각형을 자르는 서로 다른 방법의 수를 계산하는 함수를 작성하세요.

# --hints--

`cutRectangle`는 함수여야 합니다.

```js
assert(typeof cutRectangle == 'function');
```

`cutRectangle(2, 2)`은 숫자를 반환해야 합니다.

```js
assert(typeof cutRectangle(2, 2) == 'number');
```

`cutRectangle(2, 2)`는 `2`을 반환해야 합니다.

```js
assert.equal(cutRectangle(2, 2), 2);
```

`cutRectangle(4, 3)`는 `9`를 반환해야 합니다.

```js
assert.equal(cutRectangle(4, 3), 9);
```

`cutRectangle(4, 4)`는 `22`을 반환해야 합니다.

```js
assert.equal(cutRectangle(4, 4), 22);
```

`cutRectangle(8, 3)`는 `53`를 반환해야 합니다.

```js
assert.equal(cutRectangle(8, 3), 53);
```

`cutRectangle(7, 4)`는 `151`을 반환해야 합니다.
