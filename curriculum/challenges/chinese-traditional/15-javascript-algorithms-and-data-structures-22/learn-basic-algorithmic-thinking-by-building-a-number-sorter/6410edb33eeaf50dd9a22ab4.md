...
當你的 `if` 條件爲真時，你需要交換這兩個元素，將較大的元素 "冒泡" 到數組的末尾。

爲此，聲明一個 `temp` 變量併爲其賦值 `array[j]` 的值。 然後將 `array[j]` 的值賦給 `array[j + 1]`。 最後，將 `array[j + 1]` 的值賦給 `temp`。

# --hints--

在你的 `if` 語句中，你應該聲明一個 `temp` 變量。
