...
يمكنك جعل الشفرة أكثر قابلية لإعادة الاستخدام.

أنشئ دالة باسم `updateCount` تأخذ عنصر زر كمعلمة.

يجب أن تحتوي هذه الدالة على كل المنطق الخاص بتحديث عداد الزر: إيجاد عنصر العداد، تحليل العدد الحالي، التحقق مما إذا كان عند الحد الأقصى، الزيادة، وتحديث العرض.

ثم حدّث استدعاء راجع مستمع الحدث ليُستدعى `updateCount(happyBtn)` بدلاً من وجود المنطق مضمّنًا.

# --hints--

يجب أن تنشئ دالة باسم `updateCount`.

```js
assert.isFunction(updateCount);
```

يجب أن تأخذ دالة `updateCount` معلمة زر.

```js
assert.equal(updateCount.length, 1);
```

يجب أن تعمل دالة `updateCount` بشكل صحيح مع أي عنصر زر.
...
يجب أن يستدعي مستمع الحدث دالة `updateCount` مع الزر كمعلمة.
