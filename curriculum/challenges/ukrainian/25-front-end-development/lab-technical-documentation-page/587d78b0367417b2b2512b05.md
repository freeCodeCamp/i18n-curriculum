...
1. Крім того, навігаційна панель повинна містити елементи зв'язку (`a`) класу `nav-link`. На кожен елемент класу `main-section` має бути одне посилання.
...
`#navbar` повинен мати один елемент `header` всередині.
...
Всі елементи `.nav-link` повинні бути елементами посилання (`a`).
...
Елемент `header` в `#navbar` повинен йти перед елементами посилання (`a`), також в `#navbar`.
...
`#navbar` завжди повинен знаходитись у верхній частині вюпорту.
...
Проєкт «Технічна документація» повинен використовувати принаймні один медіазапит.
