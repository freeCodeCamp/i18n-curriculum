...
Para resolver esto, puedes proporcionar contexto sobre lo que se necesita añadiendo un elemento `legend` con el texto `Account type (required)` antes de los elementos `label` dentro del segundo `fieldset`. Luego añade el atributo `checked` a la entrada `Personal` para asegurar que el formulario se envíe con los datos requeridos.
