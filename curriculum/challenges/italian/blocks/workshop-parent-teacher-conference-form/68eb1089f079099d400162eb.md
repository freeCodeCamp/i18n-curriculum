...
Hai aggiunto una classe `center` ad alcuni elementi nell'HTML. Ora, dovresti definire quella classe per centrare quegli elementi.

Seleziona la classe `center` e centra gli elementi che la possiedono impostando la proprietà `text-align` su `center`.

# --hints--

Dovresti selezionare gli elementi con la classe `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.center'));
```

Dovresti impostare la proprietà `text-align` su `center`.
