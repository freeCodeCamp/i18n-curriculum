...
معطاة سلسلة أعداد مفصولة بفواصل، أرجع مصفوفة الأعداد مرتبة من الأصغر إلى الأكبر.

# --hints--

`sortNumbers("3,1,2")` يجب أن تُرجع `[1, 2, 3]`.

```js
assert.deepEqual(sortNumbers("3,1,2"), [1, 2, 3]);
```

`sortNumbers("5,3,8,1,9,2")` يجب أن تُرجع `[1, 2, 3, 5, 8, 9]`.

```js
assert.deepEqual(sortNumbers("5,3,8,1,9,2"), [1, 2, 3, 5, 8, 9]);
```

`sortNumbers("12,61,49,80,19,50,77,38")` يجب أن تُرجع `[12, 19, 38, 49, 50, 61, 77, 80]`.

```js
assert.deepEqual(sortNumbers("12,61,49,80,19,50,77,38"), [12, 19, 38, 49, 50, 61, 77, 80]);
```

`sortNumbers("0,6,-19,44,-2,7,0")` يجب أن تُرجع `[-19, -2, 0, 0, 6, 7, 44]`.
