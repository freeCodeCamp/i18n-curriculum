...
El último conjunto de elementos que debes seleccionar son los elementos de resultado. Usa tu función `getElement` para obtener el elemento con los ids `result-text` y `result-card`. Ambos deben usar `HTMLElement` como tipo.

Recuerda que ya declaraste las variables `resultText` y `resultCard` para ellos, así que solo necesitas asignarles un valor.

# --hints--

Debes asignar un valor a la variable `resultText`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` debe seleccionar el elemento `result-text`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

Debes usar `HTMLElement` como tipo del `getElement` que usas para seleccionar el elemento `result-text`.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

Debes asignar un valor a la variable `resultCard`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` debe seleccionar el elemento `result-card`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

Debes usar `HTMLElement` como tipo del `getElement` que usas para seleccionar el elemento `result-card`.
