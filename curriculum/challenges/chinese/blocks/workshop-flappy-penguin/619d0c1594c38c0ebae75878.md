...
选择类为 `left` 的 `.eye` 元素，将其定位在父元素左侧 `25%` 的位置。 然后，选择类为 `right` 的 `.eye` 元素，将其定位在父元素右侧 `25%` 的位置。
