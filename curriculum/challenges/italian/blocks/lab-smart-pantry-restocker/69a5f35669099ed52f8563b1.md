...
2. Devi implementare una funzione `planRestock(pantry, shipment)` che confronta la dispensa attuale con la spedizione in arrivo e restituisce un array di azioni nella forma `{ type: "restock" | "discard" | "donate", item }`.

3. Devi implementare una funzione `groupByZone(actions)` che raggruppa le azioni in zone di stoccaggio basate sulla proprietà `zone` di ogni elemento.
