...
title: 'Завдання 12: трикутне число з великою кількістю дільників'
challengeType: 1
forumTopicId: 301746
...
Послідовність трикутних чисел утворюється додаванням натуральних чисел. Тож сьоме трикутне число буде 1 + 2 + 3 + 4 + 5 + 6 + 7 = 28. Перші десять членів послідовності:

<div style='text-align: center;'>1, 3, 6, 10, 15, 21, 28, 36, 45, 55, ...</div>

Перелічимо дільники перших семи трикутних чисел:
...
Ми бачимо, що 28 — перше трикутне число, яке має більше п’яти дільників.

Яке значення має перше трикутне число, що має більше ніж `n` дільників?

# --hints--

`divisibleTriangleNumber(5)` має повертати число.

```js
assert.isNumber(divisibleTriangleNumber(5));
```

`divisibleTriangleNumber(5)` має повертати 28.

```js
assert.strictEqual(divisibleTriangleNumber(5), 28);
```

`divisibleTriangleNumber(23)` має повертати 630.

```js
assert.strictEqual(divisibleTriangleNumber(23), 630);
```

`divisibleTriangleNumber(167)` має повертати 1385280.

```js
assert.strictEqual(divisibleTriangleNumber(167), 1385280);
```

`divisibleTriangleNumber(374)` має повертати 17907120.

```js
assert.strictEqual(divisibleTriangleNumber(374), 17907120);
```

`divisibleTriangleNumber(500)` має повертати 76576500.
