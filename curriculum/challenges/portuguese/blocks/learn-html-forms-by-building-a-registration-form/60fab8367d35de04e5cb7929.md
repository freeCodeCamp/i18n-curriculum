...
Adicione o texto `I accept the terms and conditions` ogo depois do elemento `input` no `label` recém-adicionado. Então link o texto `terms and conditions` para o destino a seguir:
...
Você deve dar ao elemento `a` um `href` com o valor de `https://www.freecodecamp.org/news/terms-of-service/`.
...
Há espaços em branco a mais antes ou depois do texto dentro do elemento de âncora. Os únicos espaços no texto âncora deveriam estar entre as palavras `terms`, `and` `conditions`.
