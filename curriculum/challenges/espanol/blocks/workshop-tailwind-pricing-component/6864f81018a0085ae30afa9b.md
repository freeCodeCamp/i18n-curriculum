...
La marca de verificación en cada `span` al inicio de un elemento de lista es decorativa. Agrega `aria-hidden` a estos elementos `span` para ocultarlos de los lectores de pantalla.

Además, asigna las clases `text-green-700` y `mr-2` a cada uno de los elementos `span`.

# --hints--

Tus dos elementos `span` deben tener el atributo `aria-hidden`.
...
Tus dos elementos `span` deben tener la clase `text-green-700`.
...
Tus dos elementos `span` deben tener la clase `mr-2`.
