...
다음으로, `width`에서 `height`와 `.bb2a`을 제거하고 `border-left`와 `border-right`를 `5vw` 대신 `1vw`를 사용하도록 변경하세요. 그러면 요소의 크기가 0이 되고 테두리가 가운데에서 만나게 됩니다.

# --hints--

`width`에서 `.bb2a`을 제거해야 합니다.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.width);
```

`height`에서 `.bb2a`을 제거해야 합니다.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.height);
```

`border-left`를 `5vw`를 사용하도록 변경해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid rgb(153, 153, 153)");
```

`border-right`를 `5vw`를 사용하도록 변경해야 합니다.
