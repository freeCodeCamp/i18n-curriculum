...
Crea una regla `.image-1, .image-3` y asígnale una propiedad `grid-column` establecida en `1 / -1`. Esto permitirá que la primera y la tercera imagen abarquen todo el ancho de la cuadrícula.

# --hints--

Deberías tener un selector `.image-1, .image-3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

Tu selector `.image-1, .image-3` debe tener una propiedad `grid-column` establecida en `1 / -1`.
