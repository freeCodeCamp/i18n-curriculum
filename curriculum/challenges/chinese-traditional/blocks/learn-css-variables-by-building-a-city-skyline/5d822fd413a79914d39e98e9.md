...
開始優化你的代碼，首先將 `position` 和 `top` 屬性及其值從 `.foreground-buildings` 中移動到 `.background-buildings` 中，然後修改現有的 `.background-buildings` 選擇器，使其​​同時作用於 `.foreground-buildings`，從而實現樣式複用。 你可以使用逗號（`,`）分隔選擇器，例如：`selector1, selector2`。

# --hints--

你不應刪除 `.foreground-buildings` 聲明。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.foreground-buildings'));
```

你應該從 `.foreground-buildings` 中刪除 `position` 屬性。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.position);
```

你應該從 `.foreground-buildings` 中刪除 `top` 屬性。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.top);
```

你應該將值爲 `absolute` 的 `position` 屬性添加到 `.background-buildings, .foreground-buildings`。
...
你應該將值爲 `0` 的 `top` 屬性添加到 `.background-buildings, .foreground-buildings`。
...
你應該使用逗號在同一樣式聲明中同時使用 `.foreground-buildings` 和 `.background-buildings` 選擇器。
...
你不應該有一個單獨的 `.background-builds` 選擇器。
