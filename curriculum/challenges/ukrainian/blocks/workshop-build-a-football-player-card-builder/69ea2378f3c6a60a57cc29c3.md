...
Тепер спроєктуйте масив `POSITIONS` за допомогою стрілкової функції, яка приймає кожен рядок позиції як `pos` і повертає елемент `option`. Опція має мати `key` зі значенням `pos`, а також значення `pos`. Також відобразіть `pos` як текст кожного згенерованого `option`.

Ви можете використовувати будь-яке ім’я параметра для вашого map, це не обов’язково має бути `pos`.

# --hints--

Ваш масив позицій `select` повинен містити елементи `option`, спроєктовані з масиву `POSITIONS`.
...
Кожен `value` опції має відповідати відображеному тексту і бути дійсною позицією.
