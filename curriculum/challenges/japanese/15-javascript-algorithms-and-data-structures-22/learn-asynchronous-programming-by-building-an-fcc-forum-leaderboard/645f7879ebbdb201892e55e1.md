...
次に、`userAvatarUrl` を組み立てます。

まず、`userAvatarUrl` と言う名前の const 変数を作成してください。 Then use a ternary operator to check if `avatar` starts with `"/user_avatar/"`.

当てはまる場合、`concat` メソッドを使用して `avatar` を `avatarUrl` に連結してください。 それ以外の場合は、`avatar` をそのまま返してください。

これにより、相対 URL の場合でも絶対 URL の場合でも、アバター URL が正しく組み立てられます。

# --hints--

You should have a constant named `userAvatarUrl`.
