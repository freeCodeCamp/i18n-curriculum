...
Dovresti dare a `.penguin-body::before` una proprietà `width` del `50%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.width, '50%');
```

Dovresti dare a `.penguin-body::before` una proprietà `height` del `45%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.height, '45%');
```

Dovresti dare a `.penguin-body::before` un `background-color` di `--fcc-expected--`, invece che `--fcc-actual--`.
