...
Dovresti aggiungere un elemento `div` all'interno di `div.shirt`. Dovrebbero esserci `--fcc-expected--` elementi `div` invece di `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.shirt > div')?.length, 1);
```

Dovresti dare al nuovo `div` un contenuto di `💜` invece di `--fcc-actual--`.
