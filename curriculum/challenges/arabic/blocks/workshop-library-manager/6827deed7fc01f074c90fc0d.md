...
سيكون من الجيد اختبار دالة `getBooksByAuthor` مع مؤلف آخر.

ابدأ بتسجيل الرسالة `"\nList of books by James Clear:\n"` في وحدة التحكم.

أسفل ذلك `console.log()`، أضف `console.log()` آخر. داخل هذا `console.log()`، نفذ استدعاء الدالة `getBooksByAuthor()` مع `library` و`"James Clear"` كمعلمات.

الآن، يجب أن ترى جميع الكتب لذلك المؤلف المحدد في وحدة التحكم.
...
يجب أن يكون لديك `console.log(getBooksByAuthor(library, "James Clear"))`.
