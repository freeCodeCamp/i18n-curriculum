...
まず、単一の`showLatestPosts`パラメータを受け取る`data`というアロー関数を作成してください。

# --hints--

`showLatestPosts`というアロー関数があるはずです。

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

`showLatestPosts`関数は`data`パラメータを持つべきです。

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

`showLatestPosts`が定義されているはずです。

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts`は関数であるべきです。
