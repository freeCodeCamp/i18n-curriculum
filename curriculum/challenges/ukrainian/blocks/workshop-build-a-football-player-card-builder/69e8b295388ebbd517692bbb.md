...
Після елемента `card-image-wrap` додайте `div` з `className` `card-name-strip`. Всередині нього додайте `span` з `className` `card-name` та текстом `PELE`.

# --hints--

Ви повинні додати `div` з `className` `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

Ви повинні додати `span` з `className` `card-name` всередині `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

Спан `.card-name` має містити текст `PELE`.
