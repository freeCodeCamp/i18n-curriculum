...
接下來，你將需要使用 `document.querySelector` 來獲取 `.start-screen` 和 `.checkpoint-screen` 元素。

將它們分別存儲在名爲 `startScreen` 和 `checkpointScreen` 的 `const` 變量中。
...
你應該獲取具有 `start-screen` 的 `class` 的元素。 不要忘記在 `document.querySelector` 方法中包含 `.`。

```js
assert.match(code, /document\.querySelector\(\s*('|"|`)\.start-screen\1\s*\)/);
```

你應該使用 `const` 聲明名爲 `startScreen` 的變量，並將 `.start-screen` 元素分配給它。

```js
assert.match(code, /const\s+startScreen\s*=\s*document\.querySelector\(\s*('|"|`)\.start-screen\1\s*\)/);
```

你應該使用 `document.querySelector()` 方法來獲取 `.checkpoint-screen` 元素。

```js
assert.match(code, /document\.querySelector\(\s*('|"|`)\.checkpoint-screen\1\s*\)/);
```

你應該使用 `const` 聲明名爲 `checkpointScreen` 的變量，並將 `.checkpoint-screen` 元素分配給它。
