...
Ndani ya tamko lako la `if`, tengeneza kigezo kinachoitwa `category` kuhifadhi aina ya gharama. Weka thamani yake kwa wito wa `input()` na tumia `'Enter category: '` kama hoja.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `category` katika tamko lako la `if`.
...
Unapaswa kuweka thamani ya `input('Enter category: ')` kwa kigezo chako cha `category`.
