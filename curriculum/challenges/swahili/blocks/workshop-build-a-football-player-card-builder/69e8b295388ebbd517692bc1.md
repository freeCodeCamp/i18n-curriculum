...
Sasa interface ya `PlayerData` imefafanuliwa, sasisha sehemu ya `PlayerCard` ili ikubali vigezo `player` vilivyo na aina ya `PlayerData` ili viendane na interface ya `PlayerData`.

Kumbuka unaweza kuvunja vigezo vinavyopitishwa hivi:
...
Unapaswa kusasisha `PlayerCard` ili ikubali vigezo `player` vilivyo na aina ya `PlayerData`.
