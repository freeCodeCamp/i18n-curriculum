...
爲 `"defender"` 添加一個新的 `case`，按照與上一步相同的模式檢查 `player.position` 是否等於 `"defender"`。

# --hints--

你應該在 `switch` 語句內爲 `"defender"` 添加一個新的 `case`。

```js
assert.match(code, /\s*case\s*('|"|`)\s*defender\s*\1\s*:\s*/)
```

在你的 `"defender"` `case` 中，你應該調用 `setPlayerCards` 函數。
...
你的 `filter()` 回調應該隱式返回檢查 `player.position` 是否爲 `"defender"` 的結果。
