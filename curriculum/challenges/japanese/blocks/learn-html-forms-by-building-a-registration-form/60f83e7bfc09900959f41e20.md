...
特定の`type`属性値には組み込みのフォームバリデーションが付いています。例えば、`type="email"`は値が有効なメールアドレスである必要があります。

パスワード`input`要素にカスタムバリデーションを追加してください。`minlength`属性を値`8`で追加することで、8文字未満の入力が提出されるのを防げます。

# --hints--

パスワード`input`要素に`minlength`属性を付ける必要があります。

```js
assert.notEqual(document.querySelector('input[type="password"]')?.minLength, -1);
```

`minlength`属性に`8`の値を付ける必要があります。
