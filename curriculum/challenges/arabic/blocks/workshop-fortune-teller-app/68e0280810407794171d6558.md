...
أضف الآن عنصر `div` مع الصنف `img-loader`. بعد ذلك، أنشئ عنصر `img` وامنحه عنوان URL مشروط للصورة باستخدام ``${img ? `${CDN_URL}/${img}` : LOCAL_DEFAULT_IMG}``.

# --hints--

يجب أن يكون لديك عنصر `div` داخل عنصر `figure` الخاص بك.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

يجب أن يحتوي عنصر `div` الخاص بك على الصنف `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

يجب أن يكون لديك عنصر `img` داخل عنصر `figure` الخاص بك.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

يجب أن يستخدم عنصر `img` الخاص به `CDN_URL` و `img` في خاصية `src` عندما يكون `img` متوفرًا، ويعود إلى `LOCAL_DEFAULT_IMG` عندما لا يكون كذلك.
...
يجب أن يحتوي عنصر `img` الخاص بك على الأصناف `card-img` و `hidden`.
