...
أضف المعلمتين المتبقيتين `chosenCard.name_short` و `chosenCard.img`. ثم خارج دالة `renderCard`، أعد تعيين الخاصية `innerHTML` لعنصر `multipleCard` إلى سلسلة فارغة.

# --hints--

يجب أن تمرر `chosenCard.name_short` كمعلمة إلى `renderCard`.
...
يجب أن تمرر `chosenCard.img` كآخر معلمة إلى `renderCard`.
...
يجب أن تعيد تعيين `this.elements.multipleCard.innerHTML` إلى سلسلة فارغة.
