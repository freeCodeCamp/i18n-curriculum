...
실제로 DOM을 업데이트하는 함수가 필요합니다. `updateResultsList` 함수를 선언하세요.

먼저, 함수는 `resultList` 요소의 HTML을 지워야 합니다.

# --hints--

`updateResultsList` 함수를 정의해야 합니다.

```js
assert.isFunction(updateResultsList);
```

`updateResultsList` 함수는 `resultList.innerHTML`를 빈 문자열로 설정해야 합니다.
