...
Тепер використайте метод `querySelectorAll` для цільового вибору елементів `.playlist-song` і збережіть їх у змінній з ім’ям `songs`.

Потім викличте метод `forEach` на `songs`. Передайте функцію зворотного виклику, яка використовує `song` як параметр методу. Поки що залиште фігурні дужки порожніми.

# --hints--

У вас має бути змінна з ім’ям `songs`.

```js
assert.isDefined(songs);
```

Ви маєте призначити колекцію всіх елементів `.playlist-song` вашій змінній `songs`.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

Ви маєте викликати метод `forEach` на `songs` і передати функцію зворотного виклику, яка використовує `song` як параметр методу.
