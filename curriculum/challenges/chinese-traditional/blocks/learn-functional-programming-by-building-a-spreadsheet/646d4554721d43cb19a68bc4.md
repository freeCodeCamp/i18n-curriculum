...
`fn` 參數將會被傳入一個函式名稱，例如 `"SUM"`。更新 `apply`，使用 `fn` 變數作為物件存取的鍵，從你的 `spreadsheetFunctions` 物件中隱式傳回該函式。

請記住 `fn` 可能不包含小寫字串，所以你需要將它轉換為小寫字串。
...
你的 `apply` 函式（程式）應該存取 `spreadsheetFunctions` 物件。
...
你的 `apply` 函式（程式）應該存取與 `fn` 值相符的 `spreadsheetFunctions` 物件的屬性。
...
你的 `apply` 函式應該在屬性存取中對 `fn` 呼叫 `.toLowerCase()` 方法。
