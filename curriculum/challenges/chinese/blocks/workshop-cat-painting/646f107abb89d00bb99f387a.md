...
使用后代选择器来定位 `.cat-whiskers-left` 元素内部的三个 `div` 元素。 为它添加 `40px` 的 `width`、`1px` 的 `height` 以及 `#000` 的 `background-color`。

# --hints--

你应该有一个 `.cat-whiskers-left div` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div'));
```

你的 `.cat-whiskers-left div` 选择器应将 `width` 设置为 `40px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.width, '40px');
```

你的 `.cat-whiskers-left div` 选择器应将 `height` 设置为 `1px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.height, '1px');
```

你的 `.cat-whiskers-left div` 选择器应将 `background-color` 设置为 `#000`。
