...
Ora aggiorna `FootballPlayerCard` per passare `defaultPlayer` a `PlayerCard` invece dell'oggetto in linea.

# --hints--

Dovresti avere `defaultPlayer` come valore della prop `player` passata a `PlayerCard`.
