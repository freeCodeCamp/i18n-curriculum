...
請注意，HTML 中的專輯封面和 `userData.songs` 陣列中的歌曲已經更改。我們已將原始歌曲替換為較短的歌曲，供你在接下來的步驟中測試你的應用程式使用。

接著，你需要檢查是否有下一首歌曲可播放。透過呼叫 `getCurrentSongIndex()` 函式，取得目前歌曲索引，並將其存檔在 `currentSongIndex` 常數中。

接著，創建一個 `nextSongExists` 常數，其包含布林值 `true` 或 `false`，取決於下一首歌曲是否存在。

# --hints--

你不應該修改現有的 `ended` 事件監聽器及其內容。

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

你應該創建一個 `currentSongIndex` 常數並將其設定為呼叫 `getCurrentSongIndex` 函式（程式）的結果。
...
你應該比較 `userData.songs.length` 和 `currentSongIndex` 來檢查是否存在下一首歌曲，並將結果設定為一個 `nextSongExists` 常數。如果歌曲陣列的最後索引（`userData.songs.length - 1`）大於 `currentSongIndex`，表示存在下一首歌曲。
