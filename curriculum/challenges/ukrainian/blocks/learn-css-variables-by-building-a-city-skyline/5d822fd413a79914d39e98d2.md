...
Вкладіть `div` з класом `bb1` у контейнер фонового будинку. Відкрийте свій `styles.css` файл і надайте `.bb1` `width` зі значенням `10%` та `height` зі значенням `70%`. "bb" означає "фонова будівля", це буде ваша перша будівля.

# --hints--

Ви повинні створити новий `div` елемент.
...
Ви повинні надати елементу `.bb1` `width` зі значенням `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1')?.width, '10%');
```

Ви повинні надати елементу `.bb1` `height` зі значенням `70%`.
