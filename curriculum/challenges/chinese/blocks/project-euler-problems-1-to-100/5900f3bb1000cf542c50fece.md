...
一种常见的在线银行安全性方法是要求用户提供密码中的三个随机字符。例如，如果密码是 `531278`，他们可能会要求第 2、第 3 和第 5 个字符；预期的回复是：`317`。

数组 `keylog1`、`keylog2` 和 `keylog3` 包含五十次成功的登录尝试。

鉴于总是按顺序询问三个字符，分析该文件以确定未知长度的最短可能秘密密码。

# --hints--

`passcodeDerivation(keylog1)` 应该返回一个数字。
...
`passcodeDerivation(keylog1)` 应该返回 `531278`。
...
`passcodeDerivation(keylog2)` 应该返回 `1230567`。
...
`passcodeDerivation(keylog3)` 应该返回 `73162890`。
