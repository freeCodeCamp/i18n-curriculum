...
`playSong` 関数の中で、`audio.src` プロパティを `song.src` と等しく設定してください。これにより、オーディオ要素が選択された曲のオーディオデータの場所を認識します。

また、`audio.title` プロパティを `song.title` と等しく設定してください。これにより、オーディオ要素に曲のタイトルとして表示される内容が指定されます。

# --hints--

既存の `playSong` 関数とその内容は変更しないでください。
...
`audio.src` を `song.src` に設定してください。
...
`audio.title` を `song.title` に設定してください。
