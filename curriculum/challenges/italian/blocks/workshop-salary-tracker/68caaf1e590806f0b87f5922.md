...
All'inizio del tuo setter, crea un'istruzione `if` che solleva un `TypeError` con il messaggio `'salary' must be a number.` quando `new_salary` non è né un intero né un float.

# --hints--

Dovresti avere un'istruzione `if` nel tuo setter `salary`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[0]`)) })
```

Dovresti sollevare un `TypeError` con il messaggio `'salary' must be a number.` quando `new_salary` non è né un intero né un float.
...
Non dovresti sollevare alcuna eccezione quando `new_salary` è un intero o un float.
