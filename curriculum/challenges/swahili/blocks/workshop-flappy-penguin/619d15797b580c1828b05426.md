...
Unapaswa kuongeza kipengele cha `div` ndani ya `div.shirt`. Inatarajiwa kipengele cha `div`, lakini kilipatikana --fcc-actual--.

```js
assert.lengthOf(document.querySelectorAll('.shirt > div'), 1);
```

Unapaswa kumpa `div` mpya maudhui ya `💜`, lakini kilipatikana `--fcc-actual--`.
