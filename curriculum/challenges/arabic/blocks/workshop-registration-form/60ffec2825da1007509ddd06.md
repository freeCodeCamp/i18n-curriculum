...
تُدرج معظم المتصفحات خصائص وقيم CSS افتراضية خاصة بها لعناصر مختلفة. إذا نظرت عن كثب، قد تلاحظ أن مُدخَل `input` الخاص بالملف أصغر من عناصر النص الأخرى من نوع `input`. افتراضيًا، يُعطى مُدخَل `input` الذي يمكنك الكتابة فيه حشو `padding` بقيمة `1px 2px`.

باستخدام محدِّد خاصية آخر، نسق الـ `input` الذي له خاصية `type` بقيمة `file` ليحصل على نفس الحشو مثل باقي عناصر الـ `input`.

# --hints--

يجب أن تستخدم محدد خاصية لتنسيق عنصر `input`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="file"]'));
```

يجب أن تعطي `input[type="file"]` حشو `padding` بقيمة `1px 2px`.
