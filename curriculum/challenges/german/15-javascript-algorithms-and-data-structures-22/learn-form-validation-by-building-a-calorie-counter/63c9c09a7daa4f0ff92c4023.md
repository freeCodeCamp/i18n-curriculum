...
Verwende dieselbe Syntax für die Abfrage deiner `number`-Eingaben im `#lunch`-Element und weise sie `lunchNumberInputs` zu.

# --hints--

Du solltest eine `lunchNumberInputs`-Variable deklarieren.
...
Du solltest `lunchNumberInputs` das Ergebnis deines `document.querySelectorAll()`-Aufrufs zuweisen.
