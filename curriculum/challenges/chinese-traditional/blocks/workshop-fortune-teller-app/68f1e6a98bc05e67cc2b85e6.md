...
宣告一個名為 `cardElement` 的變數，並使用 `card_container` 類別來取得該元素。使用 `closest` 選擇器從 `target` 中取得它。

# --hints--

你應該在 `cardElement` 內宣告一個名為 `showFortune` 的變數。
...
你的 `cardElement` 應該使用 `target?.closest` 來尋找 `.card_container`。
...
你應該將 `.card_container` 作為引數傳入 `closest`。
