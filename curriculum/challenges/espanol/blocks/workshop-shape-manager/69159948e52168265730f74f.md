...
Necesitas una forma de acceder de manera segura al elemento select y protegerte contra valores `null`. Para eso, crea una constante `val` con el valor `e.currentTarget` y tipéala como `HTMLSelectElement` usando la palabra clave `as`.

Debajo de la constante, usa una sentencia `if` para verificar si `val` es falso negándolo. En el bloque `if`, retorna `target value not found`.

# --hints--

Deberías tener una constante llamada `val`.

```js
assert.match(code, /const\s+val/)
```

Tu constante `val` debería tener un valor `e.currentTarget` tipado como `HTMLSelectElement` usando la palabra clave `as`.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

Deberías tener una sentencia `if` que verifique si `val` es null.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

Tu sentencia `if` debería retornar `target value not found`.
