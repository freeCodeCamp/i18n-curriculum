...
Com um navegador headless, antes de executar os testes, você precisa **visitar** a página que será testada.
...
E use o hook `suiteSetup` para direcionar o `browser` para a rota `/` com o seguinte código. **Nota**: `done` é passado como um callback para `browser.visit`, você não deve invocá-lo.
