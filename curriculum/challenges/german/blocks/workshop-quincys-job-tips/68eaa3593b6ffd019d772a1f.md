...
Fügen Sie unter dem Blockzitat-Element ein Absatz-Element mit dem Text `&mdash;Quincy Larson, How to Learn to Code and Get a Developer Job [Full Book]` hinzu.

`&mdash;` ist eine HTML-Entität, die einen Gedankenstrich `—` darstellt.

# --hints--

Sie sollten ein `p`-Element unter dem `blockquote`-Element haben.

```js
assert.exists(document.querySelector('section:first-of-type > blockquote + p'));
```

Ihr `p`-Element sollte den Text `&mdash;Quincy Larson, How to Learn to Code and Get a Developer Job [Full Book]` enthalten.
