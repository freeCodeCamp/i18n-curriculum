...
Наступний крок — циклічний перегляд масиву `posters`, щоб отримати всі їхні аватари.

Для початку додайте ключове слово `return`, а потім `posters.map()`. Додайте параметр під назвою `poster` для функції зворотного виклику.

# --hints--

Поверніть `posters.map()`.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

Функція зворотного виклику повинна мати параметр під назвою `poster`.
