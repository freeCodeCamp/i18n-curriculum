...
In den vorherigen Lektionen haben Sie gelernt, dass `querySelectorAll` ein `NodeList` zurückgibt, das eine array-ähnliche Struktur ist, und eine der Methoden, die ein `NodeList` hat, ist `forEach()`, die genauso funktioniert wie das `forEach`, das bei Arrays existiert.

Durchlaufen Sie `btns` mit einer `forEach()`, die jedem Button denselben Ereignis-Listener hinzufügt.

Verwenden Sie eine Callback-Funktion, die einen `btn`-Parameter annimmt, und fügen Sie innerhalb des Callbacks einen Klick-Ereignis-Listener hinzu, der `updateCount(btn)` aufruft.

Damit haben Sie den Workshop abgeschlossen.

# --hints--

Sie sollten `btns.forEach()` verwenden, um die Buttons zu durchlaufen.

```js
assert.match(code, /btns\.forEach\s*\(/);
```

Alle Buttons sollten Ereignis-Listener haben, die `updateCount` aufrufen, wenn sie angeklickt werden.
