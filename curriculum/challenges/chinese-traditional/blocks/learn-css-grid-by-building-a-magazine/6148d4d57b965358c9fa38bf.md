...
現在是時候為你的第三個 `section` 設定樣式。請注意，它的 `class` 屬性具有 `text` 和 `text-with-images` 值，這表示它已經繼承了你 `.text` 規則的樣式。

創建 `.text-with-images` 選擇器並將 `display` 屬性設為 `grid`。

# --hints--

你應該有一個 `.text-with-images` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

你的 `.text-with-images` 選擇器應該有一個設定為 `grid` 的 `display` 屬性。
