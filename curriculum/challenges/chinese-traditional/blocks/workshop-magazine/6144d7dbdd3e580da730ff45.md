...
創建一個 `body` 選擇器。將 `font-family` 屬性設定為 `Baskervville`，並以 `serif` 作為備用字型。將 `color` 屬性設定為 `linen`，並將 `background-color` 屬性設定為 `rgb(20, 30, 40)`。
...
你的 `body` 選擇器應該設定 `font-family` 屬性為 `Baskervville`，並以 `serif` 作為備用字型。
...
你的 `body` 選擇器應該有設定 `color` 屬性為 `linen`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.color, 'linen');
```

你的 `body` 選擇器應該有一個 `background-color` 屬性設定為 `rgb(20, 30, 40)`。
