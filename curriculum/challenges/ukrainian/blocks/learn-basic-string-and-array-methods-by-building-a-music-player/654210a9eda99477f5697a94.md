...
Створіть елемент `button` з класом `playlist-song-info`. Всередині `button` додайте елемент `span` з класом `playlist-song-title`, а потім вставте `song.title` як текст.

# --hints--

Ви повинні створити `button` з класом `playlist-song-info`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

Ви повинні створити елемент `span` всередині вашого елемента `button`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

Ваш елемент `span` має мати клас `playlist-song-title`.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

Ви повинні вставити `song.title` як вміст вашого елемента `span`.
