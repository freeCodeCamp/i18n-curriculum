...
Njia ya `sort()` inakubali kitendakazi cha mwitiko cha kulinganisha kinachobainisha mpangilio wa kupanga.

Katika mfano huu, sharti la kwanza `(a.name < b.name)` linakagua kama jina la tunda la kwanza ni dogo kuliko jina la tunda la pili. Ikiwa ni hivyo, tunda la kwanza linapangwa kabla ya tunda la pili.

Mfuatano wa herufi hulinganishwa kwa mpangilio wa alfabeti, ambayo inamaanisha hulinganishwa herufi kwa herufi. Kwa mfano, `"Apples"` ni dogo kuliko `"Bananas"` kwa sababu `"A"` huja kabla ya `"B"` katika alfabeti.

Sababu ya mfano huu kurudisha nambari ni kwa sababu njia ya `sort()` inatarajia kurudisha nambari. Ikiwa utarudisha nambari hasi, kipengee cha kwanza kinapangwa kabla ya kipengee cha pili.
...
Ndani ya kitendakazi chako cha mwitiko, ongeza tamko la `if` ili ukague kama `a.title` ni dogo kuliko `b.title`. Ikiwa ni hivyo, rudisha `-1`.

# --hints--

Unapaswa kuwa na tamko la `if` ndani ya kitendakazi chako cha mwitiko.

```js
assert.match(code, /if\s*\(/);
```

Tamko lako la `if` linapaswa kukagua kama `a.title` ni dogo kuliko `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

Tamko lako la `if` linapaswa kurudisha `-1` ikiwa `a.title` ni dogo kuliko `b.title`.
