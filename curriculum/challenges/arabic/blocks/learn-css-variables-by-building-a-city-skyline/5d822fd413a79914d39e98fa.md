...
أنشئ متغيرًا جديدًا في `:root` باسم `window-color2` بقيمة `#8cd9b3`. سيُستخدم هذا كلون ثانوي لهذا المبنى.

# --hints--

يجب إنشاء متغير خاصية جديد باسم `window-color2` داخل `:root`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--window-color2'));
```

يجب إعطاء `window-color2` القيمة `#8cd9b3`.
