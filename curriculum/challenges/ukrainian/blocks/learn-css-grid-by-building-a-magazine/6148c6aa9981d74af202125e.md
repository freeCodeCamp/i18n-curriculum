...
Надайте селектору `.text` множину властивостей: `font-size` зі значенням `1.8rem` та `letter-spacing` зі значенням `0.6px`.

# --hints--

Ваш селектор `.text` має мати властивість `font-size` зі значенням `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize === '1.8rem');
```

Ваш селектор `.text` має мати властивість `letter-spacing` зі значенням `0.6px`.
