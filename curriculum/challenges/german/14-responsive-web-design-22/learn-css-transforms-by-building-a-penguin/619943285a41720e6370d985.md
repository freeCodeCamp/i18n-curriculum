...
Du solltest `.penguin-body` eine `top`-Eigenschaft zuweisen.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top);
```

Du solltest `.penguin-body` eine `top`-Eigenschaft von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top, '40%');
```

Du solltest `.penguin-body` eine `left`-Eigenschaft zuweisen.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.left);
```

Du solltest `.penguin-body` eine `left`-Eigenschaft von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.
