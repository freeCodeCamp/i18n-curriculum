...
`switch` 文では、case の条件がいずれも評価対象の値と一致しなかった場合、`default` のケースが実行されます。 これは他のあらゆるケースの受け皿として機能します。 例えば、次のように記述します。
...
では、`default` の場合、`currentDateParagraph` のテキストコンテンツが `formattedDate` の値に設定されるようにしてください。

これで、日付フォーマッターの完成です！ 現在の日付を 3 種類の異なる形式でフォーマットできるようになりました。
