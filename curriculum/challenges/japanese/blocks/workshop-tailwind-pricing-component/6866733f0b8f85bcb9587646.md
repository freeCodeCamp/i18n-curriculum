...
プレミアムプランの料金カードは、ページ上で最も目立つようにしてください。また、`Most popular` バッジが付く唯一のカードでもあります。このカードとバッジを目立たせるために、Tailwind の相対位置指定とスケーリングのユーティリティクラスを使うことができます。

Tailwind CSS で要素をスケーリングして大きくする方法は次の通りです。
...
位置指定は今後のステップで行います。今は、ラッパー `scale-105` にクラス `div` を代入して、他の料金カードより少し大きくしてください。

# --hints--

プレミアムプランのラッパー `div` 要素にはクラス `scale-105` を付けてください。
