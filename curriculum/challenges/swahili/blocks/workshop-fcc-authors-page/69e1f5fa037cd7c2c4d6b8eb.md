...
Mwishowe, unahitaji kushughulikia njia ya `catch`. Kwanza, panga ndani tamko la `fetch`, tamko la `json`, na wito wa displayAuthors ndani ya blokki ya `try` ndani ya `initialFetch`. Kisha, badilisha njia ya `catch` iliyopangwa mnyororo kuwa blokki ya catch kama inavyoonyeshwa kwenye mfano hapa chini.
...
Ikiwa umefanya hivi kwa usahihi, blokki ya `try` itakuwa na mistari mitatu ya msimbo ndani yake na blokki ya `catch` itakuwa na mstari mmoja tu wa msimbo. Pia, `SyntaxError` itatatuliwa na programu inapaswa kufanya kazi kawaida.

Jipongeze! Umejifunza jinsi ya kufanya msimbo wako isiyo ya wakati mmoja usomeke zaidi kwa kutumia vionyesho vya `async` na `await`! Sasa ukurasa wako wa waandishi umekamilika.
...
Unapaswa kuunda blokki ya `try/catch`

```js
assert.match(code, /try\s*\{[\s\S]*?\}\s*catch\s*\([^)]*\)\s*\{/);
```

`catch` haipaswi tena kuwa njia iliyopangwa mnyororo

```js
assert.notMatch(code, /\.catch/);
```

Blokki ya `try` inapaswa kuwa na tamko tatu zinazopata data, kuunda `json` na kisha kuionyesha.
...
Ukurasa wa Waandishi wa fCC unapaswa kuonyesha kwa usahihi
