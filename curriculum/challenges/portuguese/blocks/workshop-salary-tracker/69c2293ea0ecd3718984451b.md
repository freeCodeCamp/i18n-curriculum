...
Agora que o setter `salary` foi criado, atualize `__init__` para chamá-lo em vez de atribuir diretamente a `self._salary`. Altere `self._salary = Employee._base_salaries[level]` para `self.salary = Employee._base_salaries[level]`.

# --hints--

Seu método `__init__` deve usar `self.salary = Employee._base_salaries[level]` em vez de `self._salary = Employee._base_salaries[level]`.
