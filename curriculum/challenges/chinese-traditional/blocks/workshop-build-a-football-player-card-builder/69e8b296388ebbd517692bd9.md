...
為 Club `input` 添加一個 `value` 屬性，值為 `player.club`。接著，添加一個 `onChange` 屬性，設定為一個箭頭函式，該函式以 `e` 作為參數，並呼叫 `setPlayer`，傳入展開的 `player` 和 `club`，其中 `club` 設為 `e.target.value`。

# --hints--

你的 Club `input` 應該有一個 `value`，其值與玩家的 club 相等。
...
你的 Club `input` 應該有一個 `onChange` 處理常式，該處理常式呼叫 `setPlayer`，並傳入展開的 `player` 和 `club`，其中 `club` 設為 `e.target.value`。
