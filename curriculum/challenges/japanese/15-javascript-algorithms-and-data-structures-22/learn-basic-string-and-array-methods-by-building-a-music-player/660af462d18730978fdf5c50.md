...
ここで学んだアロー関数についての知識を、音楽プレーヤープロジェクトに応用しましょう。

最初に、関数 `printGreeting`、`printMessage`、`addTwoNumbers` を削除してください。 関連するコンソール文と関数呼び出しもすべて削除してください。

# --hints--

`printGreeting` 関数が無いようにしてください。

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

`printMessage` 関数が無いようにしてください。

```js
assert.notMatch(code, /const printMessage = org => {/);
```

`addTwoNumbers` 関数が無いようにしてください。

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

`printGreeting();` 関数呼び出しが無いようにしてください。

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

`printMessage('freeCodeCamp');` 関数呼び出しが無いようにしてください。

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

コンソール文 `console.log(addTwoNumbers(3,4));` が無いようにしてください。
