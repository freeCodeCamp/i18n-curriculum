...
لقد أدى سجل `byDecade` الغرض منه. احذف `console.log(byDecade)` من شفرتك.

بعد ذلك، لجعل مدخلات الكتالوج أسهل في الفحص، أنشئ دالة باسم `renderEntry` تأخذ `entry` كمعلمة. اترك جسم الدالة فارغًا في الوقت الحالي. أسفل الدالة، سجّل `renderEntry(catalog[0])`.

# --hints--

يجب أن تكون `renderEntry` دالة.

```js
assert.isFunction(renderEntry);
```

يجب أن تكون `renderEntry` دالة تحتوي على معلمة `entry`.
