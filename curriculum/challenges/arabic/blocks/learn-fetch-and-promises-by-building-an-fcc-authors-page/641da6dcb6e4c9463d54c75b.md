...
داخل سلسلة القالب، أنشئ عنصر `div` مع تعيين `id` إلى `index` من طريقة المصفوفة `.forEach()`. تذكر استخدام الاستيفاء النصي للقيام بذلك.

أضف أيضًا `class` بقيمة `"user-card"` إلى عنصر الـ `div`.
...
يجب إنشاء عنصر `div` مع الصنف `"user-card"`.
...
يجب أن يحتوي عنصر `div` على `id` بقيمة `index`. تذكر استخدام الاستيفاء النصي لتعيين الـ `id`.
