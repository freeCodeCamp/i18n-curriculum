...
ルートドキュメントに `DOMContentLoaded` イベントを追加し、実行するコールバック関数として `initializeApp` を渡してください。これにより、DOMが完全に読み込まれた後にのみ初期化が行われるようになります。

# --hints--

ルート `DOMContentLoaded` に `document` イベントを追加してください。

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

イベントリスナーで実行する関数として `initializeApp` を参照してください。
