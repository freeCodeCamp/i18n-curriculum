...
Erstellen Sie innerhalb Ihres `aside`-Elements zwei `img`-Elemente, ein `blockquote`-Element und ein drittes `img`-Element. Geben Sie dem `blockquote`-Element ein `class`, das auf `image-quote` gesetzt ist.

# --hints--

Sie sollten drei `img`-Elemente innerhalb Ihres `aside`-Elements erstellen.

```js
assert.lengthOf(document.querySelectorAll('aside img'), 3);
```

Sie sollten ein `blockquote`-Element innerhalb Ihres `aside`-Elements erstellen.

```js
assert.exists(document.querySelector('aside blockquote'));
```

Ihr `blockquote`-Element sollte ein `class` haben, das auf `image-quote` gesetzt ist.

```js
assert.isTrue(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

Ihre neuen Elemente sollten in der richtigen Reihenfolge sein.
