...
你的音樂播放器應該追蹤歌曲、目前正在播放的歌曲，以及目前歌曲的時間。為此，你需要創建一個物件來儲存這些資訊。

首先使用 `let` 關鍵字宣告一個名為 `userData` 的新變數，並將它指定為一個空物件。

# --hints--

你應該使用 `let` 關鍵字來創建 `userData` 變數。

```js
assert.match(code, /let\s+userData\s*=/)
```

你應該將一個空物件指定給你的 `userData` 變數。
