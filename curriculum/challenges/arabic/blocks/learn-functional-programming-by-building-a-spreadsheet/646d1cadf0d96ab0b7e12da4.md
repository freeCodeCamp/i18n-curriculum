...
بما أن `elemValue` تُعيد دالة، فإن دالة `addCharacters` تُعيد في النهاية مصفوفة من مراجع الدوال. تريد أن تُشغّل طريقة `.map()` الدالة الداخلية في دالة `elemValue`، وهذا يعني أنك تحتاج إلى استدعاء `elemValue` بدلًا من مجرد الإشارة إليها. مرر `num` كمعلمة إلى دالة `elemValue` الخاصة بك.

<!-- TODO: Explain further? -->

# --hints--

يجب استدعاء `elemValue()` داخل طريقة `.map()` الخاصة بك.
...
يجب تمرير `num` إلى استدعاء `elemValue()`.
