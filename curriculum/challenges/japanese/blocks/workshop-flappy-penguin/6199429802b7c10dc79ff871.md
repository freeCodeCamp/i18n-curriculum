...
`.penguin-head` 要素を親要素の上から `10%`、左から `25%` の位置に配置してください。

# --hints--

`.penguin-head` に `top` プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.top);
```

`.penguin-head` に `top` プロパティを `--fcc-expected--` の値で設定する必要がありますが、現在は `--fcc-actual--` になっています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.top, '10%');
```

`.penguin-head` に `left` プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-head')?.left);
```

`.penguin-head` に `left` プロパティを `--fcc-expected--` の値で設定する必要がありますが、現在は `--fcc-actual--` になっています。
