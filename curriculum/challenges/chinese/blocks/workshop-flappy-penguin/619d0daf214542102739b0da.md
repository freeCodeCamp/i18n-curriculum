...
选择 `.eye-lid` 元素，并将其 `width` 设置为 `150%`、`height` 设置为 `100%`，`background-color` 设置为 `--penguin-face`。
...
你应该将 `.eye-lid` 的 `width` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

你应该将 `.eye-lid` 的 `height` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.height, '100%');
```

你应该将 `.eye-lid` 的 `background-color` 设置为 `var(--penguin-face)`。
