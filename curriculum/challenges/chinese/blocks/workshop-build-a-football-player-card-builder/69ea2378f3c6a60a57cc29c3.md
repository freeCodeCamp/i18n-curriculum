...
现在使用箭头函数遍历 `POSITIONS` 数组，该函数接收每个位置字串作为 `pos` 并返回一个 `option` 元素。该选项应具有 `key` 的 `pos`，其值也应为 `pos`。同时，将 `pos` 显示为每个生成的 `option` 的文本。

你可以随意使用任何作为 map 的参数，不必非得是 `pos`。

# --hints--

你的位置 `select` 应包含从 `option` 数组映射而来的 `POSITIONS` 元素。
...
每个选项的 `value` 应与其显示的文本匹配，并且是有效的位置。
