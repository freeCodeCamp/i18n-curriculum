...
Adicione um atributo `value` com valor `player.imageUrl` ao URL da imagem `input`. Depois defina o atributo `onChange` como uma arrow function que recebe `e` como parâmetro e faz uma chamada a `setPlayer` com um spread de `player` e `imageUrl` definido como `e.target.value`.

# --hints--

O URL da imagem `input` deve ter um `value` igual ao URL da imagem do jogador.
...
O URL da imagem `input` deve ter um manipulador `onChange` que chama `setPlayer` com um spread de `player` e `imageUrl` definido como `e.target.value`.
