...
Задача туру лицаря: у вас є порожня шахівниця `width` \* `height`, на якій стоїть лише один лицар на певній клітинці. Лицар повинен виконати послідовність легальних ходів так, щоб відвідати кожну клітинку шахівниці рівно один раз. Зверніть увагу, що *не* обов’язково, щоб тур був "замкненим"; тобто лицар не повинен закінчувати хід у клітинці, що знаходиться в один хід від початкової позиції.

# --instructions--

Напишіть функцію, яка приймає `width` і `height` як параметри та повертає число ініціальних позицій, з яких можливо виконати описане вище завдання.
...
`knightTour(6, 6)` має повертати число.

```js
assert(typeof knightTour(6, 6) == 'number');
```

`knightTour(6, 6)` має повертати `36`.

```js
assert.equal(knightTour(6, 6), 36);
```

`knightTour(5, 6)` має повертати `30`.

```js
assert.equal(knightTour(5, 6), 30);
```

`knightTour(4, 6)` має повертати `12`.

```js
assert.equal(knightTour(4, 6), 12);
```

`knightTour(7, 3)` має повертати `10`.

```js
assert.equal(knightTour(7, 3), 10);
```

`knightTour(8, 6)` має повертати `48`.
