...
1. `#pattern`, `#test-string`, `#test-btn`, `#result` 요소에 접근해서 각각 `regexPattern`, `stringToTest`, `testButton`, `testResult` 변수에 저장하세요.
1. `#i`와 `#g` 플래그 체크박스에 접근해서 각각 `caseInsensitiveFlag`와 `globalFlag` 변수에 저장하세요.
1. 체크된 플래그 체크박스에서 플래그를 가져와 문자열로 반환하는 `getFlags`라는 이름의 함수를 만드세요. 체크된 것이 없으면 빈 문자열을 반환해야 합니다.
1. `testButton` 버튼을 클릭할 때, 정규식 패턴이 테스트 문자열과 일치하면 일치한 텍스트를 강조 표시하세요. 강조 표시하려면 `stringToTest`의 `innerHTML`을 업데이트해서 각 일치 항목을 `highlight` 클래스를 가진 `span` 요소로 감싼 동일한 텍스트로 교체하세요. 해당 CSS는 이미 추가되어 있습니다.
1. 일치하는 항목이 있으면, 일치한 텍스트를 `#result` 안에 표시하세요. 여러 개가 있을 경우 각 일치 텍스트는 쉼표와 공백으로 구분하세요.
1. 일치하는 항목이 없으면, `#result` 안에 `no match`라는 텍스트를 표시하세요.
