...
El `textarea` parece demasiado pequeño. Para darle un tamaño inicial, puedes agregar los atributos `rows` y `cols`.

Agrega un tamaño inicial de `3` filas y `30` columnas.

# --hints--

Debes darle al `textarea` un atributo `rows` con valor `3`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

Debes darle al `textarea` un atributo `cols` con valor `30`.
