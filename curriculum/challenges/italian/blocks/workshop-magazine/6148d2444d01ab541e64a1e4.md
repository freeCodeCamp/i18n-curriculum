...
Crea un selettore `.quote`. Dagli una proprietà `color` impostata su `#00beef`, una proprietà `font-size` impostata su `2.4rem` e una proprietà `text-align` impostata su `center`.

# --hints--

Dovresti avere un selettore `.quote`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote'));
```

Il tuo selettore `.quote` dovrebbe avere una proprietà `color` impostata su `#00beef`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.color, 'rgb(0, 190, 239)');
```

Il tuo selettore `.quote` dovrebbe avere una proprietà `font-size` impostata su `2.4rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize, '2.4rem');
```

Il tuo selettore `.quote` dovrebbe avere una proprietà `text-align` impostata su `center`.
