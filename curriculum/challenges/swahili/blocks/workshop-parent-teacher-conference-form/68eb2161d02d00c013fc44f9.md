...
Ili kufanya maandishi ya `legend` yaonekane zaidi, unahitaji kuyapanga mtindo.

Chagua kipengele cha `fieldset legend`. Weka sifa yake ya `font-size` kuwa `1.3rem` na sifa yake ya `font-weight` kuwa `600`.

`font-weight` hudhibiti unene wa herufi nzito.

Thamani za kawaida za `font-weight` ni:

- `normal` → Uzito wa kawaida (chaguo-msingi)
- `bold` → Herufi nzito
- `lighter` → Nyepesi kuliko kipengele cha mzazi
- `bolder` → Nzito zaidi kuliko kipengele cha mzazi
- Thamani za nambari kama:
  - `100` (nyembamba)
  - `400` (kawaida)
  - `700` (nzito)
  - `900` (nzito sana)
...
Unapaswa kuwa na kichaguzi cha `fieldset legend`.

```js
assert(new __helpers.CSSHelp(document).getStyle('fieldset legend'));
```

Weka sifa ya `font-size` kuwa `1.3rem`.
...
Weka sifa ya `font-weight` kuwa `600`.
