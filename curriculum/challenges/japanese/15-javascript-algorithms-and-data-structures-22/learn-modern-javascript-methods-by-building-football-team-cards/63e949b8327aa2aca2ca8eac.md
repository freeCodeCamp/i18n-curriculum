...
次は、選手カードの情報をページに追加するために使う新しい配列を作成します。

`setPlayerCards` 関数の中で、まずは `arr` について、引数として空のコールバック関数を取る `map` メソッドを呼び出します。 そして、加算代入演算子 `+=` を使って、新しい配列を `playerCards.innerHTML` に代入してください。

`innerHTML` プロパティは、`playerCards` 要素内の HTML マークアップを取得したり、設定したりすることを思い出してください。 ここでは設定に使います。
