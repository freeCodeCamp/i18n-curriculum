...
Призначте отриманий рядок до змінної `nicknameIntroduction`.

Then, log the value of `nicknameIntroduction` to the console.

# --hints--

Ви повинні мати змінну під назвою `nicknameIntroduction`.

```js
assert.isNotNull(nicknameIntroduction);
```

Змінна `nicknameIntroduction` має бути рядком.

```js
assert.isString(nicknameIntroduction);
```

You should use string concatenation to join the string `"My nickname is "` with the `bot` variable followed by a period (`.`). Пам’ятайте про пробіли.
