...
- **Maria:** `Not really. It was too slow.` - This person did not like the movie much.
...
`Once` means the same as `as soon as` or `after`, indicating that everything ran smoothly after the database was fixed. For example:

`Once the updates were installed, the system worked perfectly.` - This means that the system worked after the updates were installed.
