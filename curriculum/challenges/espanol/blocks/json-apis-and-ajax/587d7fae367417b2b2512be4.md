...
Another interesting "key-value pair" in the first object is `"codeNames":["Juggernaut","Mrs.  Wallace","ButterCup"]`. Aquí `codeNames` es la clave y su valor es un arreglo de tres cadenas. Es posible tener arreglos de objetos así como una clave con un arreglo como un valor.
...
Para el gato con el `id` de 2, imprime en la consola el segundo valor en el arreglo `codeNames`. Debe usar el corchete y la notación de puntos en el objeto (que se guarda en la variable `json`) para acceder al valor.
