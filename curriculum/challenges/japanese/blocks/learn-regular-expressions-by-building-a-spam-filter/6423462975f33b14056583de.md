...
文字クラスは2文字以上を指定できます。`a` の文字を、`a`、`@`、`4` にマッチする文字クラスに置き換えてください。

# --hints--

`stockRegex` は文字クラスを使って、`a`、`@`、`4` にマッチするようにしてください。

```js
assert.match(stockRegex.source, /\[(a@4|a4@|4@a|4a@|@a4|@4a)]/);
```

`stockRegex` は `stock alert` にマッチするようにしてください。

```js
assert.match('stock alert', stockRegex);
```

`stockRegex` は `stock @lert` にマッチするようにしてください。

```js
assert.match('stock @lert', stockRegex);
```

`stockRegex` は `stock 4lert` にマッチするようにしてください。

```js
assert.match('stock 4lert', stockRegex);
```

`stockRegex` は `570ck 4l3r7` にマッチするようにしてください。
