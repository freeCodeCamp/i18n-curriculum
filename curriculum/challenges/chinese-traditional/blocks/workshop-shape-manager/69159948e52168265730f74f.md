...
你需要一種安全存取 select 元素並防範 `null` 值的方法。為此，使用 `val` 關鍵字創建一個值為 `e.currentTarget` 且型別為 `HTMLSelectElement` 的 `as` 常數。

在該常數下方，使用 `if` 述語檢查 `val` 是否為 falsy（透過取反判斷）。在 `if` 區塊中，傳回 `target value not found`。
