...
Dentro del elemento `preview-panel`, agrega un elemento `p` de `className` de `preview-label` y texto `Live Preview`, otro elemento `p` de `className` de `preview-hint` y el texto `Updates as you type`, y un `div` con el `className` establecido en el literal de plantilla `preview-box tier-${getPlayerTier(player.overallRating)}`.

Dentro del último `div`, mueve el componente `PlayerCard` y su prop.

# --hints--

Tu elemento `div.preview-panel` debe contener un `p` con el `className` de `preview-label` y el texto `Live Preview`.
...
Tu `div.preview-panel` debe contener un `p` con el `className` de `preview-hint` y el texto `Updates as you type`.
...
Tu `div.preview-panel` debe contener un `div` con el `className` de `preview-box` y un `className` de nivel dinámico con prefijo `tier-`.
...
Tu `PlayerCard` debe renderizarse dentro del elemento `div` con `className` de `preview-box`.
