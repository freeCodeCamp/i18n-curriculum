...
<dfn>替代序列</dfn> `|` 可用於匹配 `|` 左側的文本或右側的文本。 例如，正則表達式 `/yes|no/` 將匹配 `yes` 或 `no`。

更新你的 `helpRegex` 以匹配 `please help` 或 `assist me`。

# --hints--

你的 `helpRegex` 應該使用 `|` 替代序列。

```js
assert.include(helpRegex.toString(), "|");
```

你的 `helpRegex` 應該與字符串 `please help` 匹配。
...
你的 `helpRegex` 應該與字符串 `assist me` 匹配。
