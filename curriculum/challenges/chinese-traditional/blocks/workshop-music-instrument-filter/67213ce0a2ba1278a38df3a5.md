...
從事件監聽器中移除你的 `console.log`，並將 `productsContainer` 的內部 HTML 設定為以所選類別選項呼叫 `instrumentCards` 的結果。

然後，從你的下拉選單中選擇不同的選項，並在預覽視窗中查看結果。
...
當下拉選單選項改變時，你應該將 `productsContainer` 的內部 HTML 設定為以所選選項作為引數呼叫 `instrumentCards` 函式（程式）的結果。
...
你應該從你的事件監聽器的回呼中移除 `console.log(instrumentCards(selectContainer.value))`。
