...
Katika mafundisho ya awali, ulijifunza jinsi ya kutumia njia ya `filter()` ambayo hutumika kurudisha safu mpya ya data yenye matokeo yaliyosafishwa.
...
Tengeneza kitendakazi cha `getBooksByAuthor` chenye vigezo viwili - safu ya data yenye vitu vya vitabu na mfuatano wa herufi wenye mwandishi.

Kitendakazi lazima kirudishe safu ya data inayojumuisha vitabu vya mwandishi fulani.

# --hints--

Unapaswa kutengeneza kitendakazi cha `getBooksByAuthor`.

```js
assert.isFunction(getBooksByAuthor);
```

Kitendakazi chako cha `getBooksByAuthor` kinapaswa kuwa na vigezo viwili.

```js
assert.lengthOf(getBooksByAuthor, 2);
```

Kitendakazi chako cha `getBooksByAuthor` kinapaswa kurudisha safu ya data.

```js
assert.isArray(getBooksByAuthor(library, ''));
```

Kitendakazi chako cha `getBooksByAuthor` kinapaswa kurudisha idadi sahihi ya vitabu kwa yeyote kati ya waandishi.
