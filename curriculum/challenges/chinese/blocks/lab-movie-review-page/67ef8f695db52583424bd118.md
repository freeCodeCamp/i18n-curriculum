...
7. 你应该有一个 `ul` 元素。
8. 在 `ul` 元素内部，应该包含多个 `li` 元素，每个都应包含一个 `strong` 元素表示演员姓名，之后是对应的角色名，并且以 `as` 连接。 （例如，`James Holloway as Ethan Carter`）。
...
在第二个 `p` 元素中，你应该有一个 `strong` 元素，文本为 `Movie Rating:`。
...
每个 `li` 元素应包含一个 `strong` 元素表示演员姓名，并在其后用 `as` 连接角色名。 （例如，`James Holloway as Ethan Carter`）。
