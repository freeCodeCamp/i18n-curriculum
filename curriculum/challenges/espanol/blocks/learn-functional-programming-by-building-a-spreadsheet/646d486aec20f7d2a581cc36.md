...
Desafortunadamente, esa propiedad `children` está devolviendo una colección de elementos, que es similar a un array pero no es un array. Envuelva su segundo argumento en `Array.from()` para convertirlo en un array.

# --hints--

Debe envolver su `document.getElementById('container').children` en `Array.from()`.
