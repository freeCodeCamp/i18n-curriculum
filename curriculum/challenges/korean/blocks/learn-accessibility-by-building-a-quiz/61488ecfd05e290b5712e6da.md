...
첫 번째 섹션을 더 인라인처럼 보이게 하려면 `input` 요소 내의 `.info` 요소만 선택하고, 그들의 `width`를 `50%`로 설정한 다음 텍스트를 왼쪽 정렬하세요.

# --hints--

`.info input` 또는 `.info > input` 선택자 중 하나를 사용하세요.
...
`input` 요소에 `text-align`를 `left`로 지정하세요.
