...
El método `map()` crea un nuevo arreglo aplicando la función `(arrayValue) => { return arrayValue * 2; }` a cada elemento del arreglo original. Esto es particularmente útil cuando quieres transformar los elementos de un arreglo sin cambiar el arreglo original.
