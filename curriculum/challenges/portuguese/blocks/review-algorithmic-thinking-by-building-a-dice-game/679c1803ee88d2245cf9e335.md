...
Por fim, você deve organizar seu código com um pouco de refatoração!Você pode notar que seu código ativou o último botão de opção ("None of the Above") várias vezes dentro das suas funções `getHighestDuplicates`, `detectFullHouse` e `checkForStraights`.Refatore seu código para que, em vez de habilitar o último botão de rádio no final de cada função, isso seja feito apenas uma vez dentro de `rollDiceBtn`.

# --hints--

Sua função `getHighestDuplicates` não deve habilitar o botão de rádio final, nem definir o texto do `span` final, independentemente de os duplicados serem rolados.
...
Sua função `detectFullHouse` não deve ativar o botão de rádio final, nem definir o texto do `span` final, independentemente de um full house ter sido obtido.
...
Sua função `checkForStraights` não deve habilitar o botão de rádio final, nem definir o texto do `span` final, independentemente de uma sequência ter sido obtida.
...
Você deve ativar o botão de rádio final, definir o valor para `0` e atualizar o texto exibido para `, score = 0` quando seu `rollDiceBtn` for clicado.
