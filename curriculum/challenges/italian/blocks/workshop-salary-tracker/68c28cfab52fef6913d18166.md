...
Ora rimuovi le ultime due chiamate `print`.

# --hints--

Non dovresti avere `print(charlie_brown.name)` nel tuo codice.

```js
({ test: () => assert(runPython(`not _Node(_code).has_stmt("print(charlie_brown.name)")`)) })
```

Non dovresti avere `print(charlie_brown.level)` nel tuo codice.
