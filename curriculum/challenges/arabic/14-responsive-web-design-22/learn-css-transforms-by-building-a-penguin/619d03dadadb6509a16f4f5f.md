...
يجب عليك استخدام منتقي `.chin`.

```js
assert.match(code, /\.chin\s*\{/);
```

يجب عليك إعطاء عنصر `.chin` خاصية `width`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.width);
```

يجب أن يحتوي عنصر `.chin` على خاصية `width` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.width, '90%');
```

يجب عليك إعطاء عنصر `.chin` خاصية `height`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.height);
```

يجب أن يحتوي عنصر `.chin` على خاصية `height` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.height, '70%');
```

يجب عليك إعطاء عنصر `.chin` خاصية `background-color`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.backgroundColor);
```

يجب أن يحتوي عنصر `.chin` على خاصية `background-color` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
