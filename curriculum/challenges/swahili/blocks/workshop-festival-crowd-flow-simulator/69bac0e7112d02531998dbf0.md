...
Zoezi la mwisho katika kitendakazi chako cha `handleGateAtTick` ni kushughulikia kuzidi kwa sasa kwa tick.

Kama kuna kuzidi (`result.overflow > 0`), unapaswa:

- Andika ujumbe ufuatao `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
- Badilisha njia ya kuzidi kwa kufanya wito wa kitendakazi chako cha `rerouteOverflow()` kwa `gates`, `gate`, `tickIndex`, na `result.overflow`.

# --hints--

Kama kuna kuzidi (`result.overflow > 0`), unapaswa kwanza kuandika ujumbe `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
...
Kama kuna kuzidi (`result.overflow > 0`), unapaswa kubadilisha njia ya kuzidi kwa kufanya wito wa `rerouteOverflow()` kwa `gates`, `gate`, `tickIndex`, na `result.overflow`.
