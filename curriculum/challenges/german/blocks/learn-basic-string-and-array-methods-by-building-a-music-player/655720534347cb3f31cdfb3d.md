...
Bevor Sie einen Song löschen, müssen Sie prüfen, ob der Song gerade abgespielt wird. Wenn ja, müssen Sie den Song pausieren und den nächsten Song in der Wiedergabeliste abspielen.

Verwenden Sie eine `if`-Anweisung, um zu prüfen, ob der `userData?.currentSong?.id` gleich dem `id` des Songs ist, den Sie löschen möchten.

# --hints--

Sie sollten eine leere `if`-Anweisung mit der Bedingung `userData?.currentSong?.id === id` erstellen.
