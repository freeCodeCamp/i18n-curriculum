...
Erstellen Sie eine `.image-1, .image-3`-Regel und geben Sie ihr eine `grid-column`-Eigenschaft mit dem Wert `1 / -1`. Dadurch können das erste und das dritte Bild die volle Breite des Grids einnehmen.

# --hints--

Sie sollten einen `.image-1, .image-3`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

Ihr `.image-1, .image-3`-Selektor sollte eine `grid-column`-Eigenschaft mit dem Wert `1 / -1` haben.
