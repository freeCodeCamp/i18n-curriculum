...
Si la función flecha devuelve una expresión simple, puedes omitir la palabra clave `return` y las llaves `{}`. Esto se llama un retorno implícito.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

Si tu función flecha tiene múltiples líneas de código en el cuerpo de la función, entonces necesitas usar la palabra clave `return` y las llaves `{}`.
...
Refactoriza, o actualiza, tu función `addTwoNumbers` para eliminar la palabra clave `return` y las llaves `{}`. Tu función `addTwoNumbers` debería en su lugar utilizar un retorno implícito.

Abre la consola para asegurarte de que sigues obteniendo la salida correcta.

# --hints--

Tu función `addTwoNumbers` refactorizada debería utilizar un retorno implícito.
