...
El reproductor de música debe realizar el seguimiento de las canciones, la canción actual que está sonando y el tiempo de la canción actual. Para hacer esto, necesitarás crear un objeto para almacenar esta información.

Comienza usando la palabra clave `let` para declarar una nueva variable llamada `userData` y asígnale un objeto vacío.

# --hints--

Deberías usar la palabra clave `let` para crear la variable `userData`.

```js
assert.match(code, /let\s+userData\s*=/)
```

Deberías asignar un objeto vacío a tu variable `userData`.
