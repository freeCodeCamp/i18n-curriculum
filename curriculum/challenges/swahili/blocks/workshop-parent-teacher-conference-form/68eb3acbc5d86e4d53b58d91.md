...
Weka `font-size` kuwa `1.1rem`, `display` kuwa `block`, na `margin` kuwa `auto` ili kuweka kitufe katikati.

# --hints--

Unapaswa kuendelea kulenga darasa la vitu la `.submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Weka sifa ya `font-size` kuwa `1.1rem`.
