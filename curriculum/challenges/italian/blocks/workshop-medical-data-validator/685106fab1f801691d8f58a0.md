...
Aggiungi un'altra chiave `gender` al dizionario `constraints`. Seguendo il formato dell'espressione che hai scritto nei passaggi precedenti, verifica che `gender` sia una stringa. Poi, usa l'operatore `and` per controllare che `gender` in minuscolo sia in `('male', 'female')`.

# --hints--

Il tuo dizionario `constraints` dovrebbe avere una chiave `gender`.
...
La chiave `gender` del tuo dizionario `constraints` dovrebbe avere il valore di `isinstance(gender, str) and gender.lower() in ('male', 'female')`.
