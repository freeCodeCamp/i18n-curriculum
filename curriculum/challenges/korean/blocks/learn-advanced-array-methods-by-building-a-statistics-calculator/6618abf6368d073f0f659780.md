...
홀수와 짝수 숫자 목록에서 중앙값을 찾는 방법을 더 잘 이해했으니, 이전 단계에서 작성한 모든 테스트 코드를 제거하세요.


# --hints--

코드에 `const testArr1 = [1, 2, 3, 4, 5];`가 없어야 합니다.

```js
assert.notMatch(code, /const testArr1 = \[1, 2, 3, 4, 5\];/);
```

코드에 `const testArr2 = [1, 2, 3, 4, 5, 6];`가 없어야 합니다.

```js
assert.notMatch(code, /const testArr2 = \[1, 2, 3, 4, 5, 6\];/);
```

코드에 `const isEven = testArr2.length % 2 === 0;`가 없어야 합니다.

```js
assert.notMatch(code, /const isEven = testArr2.length % 2 === 0;/);
```

코드에 `console.log(isEven);`가 없어야 합니다.

```js
assert.notMatch(code, /console\.log\(isEven\);/);
```

코드에 `const oddListMedian = testArr1[Math.floor(testArr1.length / 2)];`가 없어야 합니다.

```js
assert.notMatch(code, /const oddListMedian = testArr1\[Math\.floor\(testArr1\.length \/ 2\)\];/);
```

코드에 `console.log(oddListMedian);`가 없어야 합니다.

```js
assert.notMatch(code, /console\.log\(oddListMedian\);/);
```

코드에 `const evenListMedian = getMean([testArr2[testArr2.length / 2 - 1], testArr2[testArr2.length / 2]]);`가 없어야 합니다.

```js
assert.notMatch(code, /const evenListMedian = getMean\(\[testArr2\[testArr2\.length \/ 2 - 1\], testArr2\[testArr2\.length \/ 2\]\]\);/);
```

코드에 `console.log(evenListMedian);`가 없어야 합니다.
