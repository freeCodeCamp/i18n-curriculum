...
Il metodo `.extend()` ti permette di aggiungere elementi da un iterabile alla fine di una lista:
...
Crea una clausola `else` e usa il metodo `.extend()` per aggiungere il percorso del nodo corrente al percorso del nodo vicino.

# --hints--

Dovresti creare una clausola `else` dopo la tua istruzione `if` annidata.
...
Dovresti avere `paths[node].extend(paths[current])` nella tua clausola `else`.
