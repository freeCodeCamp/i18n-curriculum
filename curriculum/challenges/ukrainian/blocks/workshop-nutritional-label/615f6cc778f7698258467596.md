...
Під вашим елементом з текстом `Total Fat` створіть новий елемент `p` з текстом `Saturated Fat 1g 5%`. Обгорніть `5%` у `span` з атрибутом `class`, встановленим у `bold`. У цьому випадку цього достатньо, щоб вирівняти відсоток по `5%`.

# --hints--

Ви повинні створити новий елемент `p` під вашим елементом з текстом `Total Fat`.
