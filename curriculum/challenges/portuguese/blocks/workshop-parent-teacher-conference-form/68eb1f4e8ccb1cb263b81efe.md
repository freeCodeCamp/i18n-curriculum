...
O texto da descrição precisa ser um pouco maior. Selecione o elemento com a classe `description` e defina seu `font-size` para `1.2rem`.

# --hints--

Você deve direcionar o elemento com a classe `description`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.description'));
```

Defina a propriedade `font-size` para `1.2rem`.
