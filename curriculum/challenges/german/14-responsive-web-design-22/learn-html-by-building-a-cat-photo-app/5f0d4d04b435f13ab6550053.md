...
Füge ein `legend`-Element mit dem Text `What's your cat's personality?` in das zweite `fieldset`-Element hinzu.
...
Dein `legend`-Element sollte ein öffnendes Tag haben. Öffnende Tags haben folgende Syntax: `<elementName>`.
...
Dein `legend`-Element sollte ein schließendes Tag haben. Schließende Tags haben ein `/` direkt nach dem `<`-Zeichen.
