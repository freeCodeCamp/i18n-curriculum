...
الآن ستُنشئ دالة لملء واجهة المستخدم ببيانات المؤلفين. ستستدعي هذه الدالة داخل الطريقة الثانية `.then()`.

أنشئ دالة سهمية فارغة باسم `displayAuthors` تأخذ `authors` كمعلمة.
...
يجب أن تستخدم `const` لإنشاء دالة فارغة باسم `displayAuthors`.
...
يجب أن تستخدم دالة `displayAuthors` صيغة الدالة السهمية.
...
يجب أن تكون دالة `displayAuthors` فارغة.
