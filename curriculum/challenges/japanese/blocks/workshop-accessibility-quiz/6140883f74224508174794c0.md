...
`header` を親コンテナの幅いっぱいに広げ、`height` を `50px` に設定し、`background-color` を `#1b1b32` に設定してください。その後、`display` を _Flexbox_ を使うように設定してください。

# --hints--

`header` 要素セレクターを使うべきです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('header'));
```

`header` に `width` を `100%` に設定するべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.width, '100%');
```

`header` に `height` を `50px` に設定するべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.height, '50px');
```

`header` に `background-color` を `#1b1b32` に設定するべきです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.backgroundColor, 'rgb(27, 27, 50)');
```

`header` に `display` を `flex` に設定するべきです。
