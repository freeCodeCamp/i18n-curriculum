...
Другий аргумент методу `.replace()` не обов’язково має бути рядком. Замість того ви можете передати функцію зворотного виклику, щоб реалізувати складнішу логіку обробки знайденого рядка.

Функція зворотного виклику приймає кілька параметрів. Перший - знайдений рядок. Передайте порожню функцію зворотного виклику до виклику `.replace()` та надайте до неї параметр `match`.

# --hints--

Передайте стрілкову функцію як другий аргумент до методу `.replace()`.
...
Стрілкова функція має приймати параметр `match`.
