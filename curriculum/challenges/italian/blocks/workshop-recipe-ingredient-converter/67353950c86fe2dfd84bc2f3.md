...
Dovrai adattarti al numero di porzioni da calcolare. Questa è un’ottima occasione per usare un’altra funzione curry.

Dichiara una funzione `adjustForServings` che accetta un argomento `baseQuantity`. Deve restituire una seconda funzione che accetta un argomento `newServings`.

La funzione interna deve restituire il risultato della moltiplicazione tra `baseQuantity` e `newServings`.

# --hints--

Dovresti dichiarare una funzione `adjustForServings`.

```js
assert.isFunction(adjustForServings);
```

La tua funzione `adjustForServings` dovrebbe accettare un argomento `baseQuantity`.

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

La tua funzione `adjustForServings` dovrebbe restituire una funzione.

```js
assert.isFunction(adjustForServings?.());
```

La tua funzione interna dovrebbe avere un parametro `newServings`.

```js
assert.match(adjustForServings?.()?.toString(), /\(newServings\)/);
```

La tua funzione interna dovrebbe restituire il risultato della moltiplicazione tra `baseQuantity` e `newServings`.
