...
Kwa sasa, kila wakati mtumizi anapobadilisha kumbukumbu na kubofya nje ya kumbukumbu hiyo, kigezo cha `currentContent` kinasasishwa. Lakini vipi katika hali ambapo mtumizi anakuwa makini na kumbukumbu na anaamua kuondoka bila kufanya mabadiliko yoyote?

Vizuri, katika hali hii, unaweza kuongeza sharti la kushughulikia kesi hiyo.

Juu ya mstari wako wa `currentContent = newContent;`, ongeza tamko la `if` ili kukagua kama `currentContent` ni sawa na `newContent`. Ikiwa ni hivyo, rudisha.

# --hints--

Unapaswa kuongeza tamko la `if` ili kukagua kama `currentContent` ni sawa na `newContent`. Ikiwa ni hivyo, rudisha.
