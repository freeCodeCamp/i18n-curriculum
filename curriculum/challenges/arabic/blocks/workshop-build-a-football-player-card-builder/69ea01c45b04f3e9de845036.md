...
المشكلة الآن هي أنه بمجرد أن يتم تحديث المعاينة أو تعيد تحميل الصفحة، كل ما تضيفه لبطاقة اللاعب يختفي.

حفظ كل المُدخَلات في `localStorage` سيوقف هذا. للبدء بذلك، أنشئ ثابتًا باسم `STORAGE_KEY` بقيمة سلسلة نصية هي `football_player_card`.

# --hints--

يجب إعلان متغير `STORAGE_KEY`.
...
يجب أن يحتوي `STORAGE_KEY` على السلسلة النصية `football_player_card` كقيمته.
