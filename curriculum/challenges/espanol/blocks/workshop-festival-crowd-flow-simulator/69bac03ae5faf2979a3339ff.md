...
Ahora que sabes a qué puerta redirigir, puedes mover a los asistentes excedentes.

Toma el `overflowAmount` y agrégalo a la fila de la siguiente puerta en la misma `tickIndex`.

Específicamente, debes:

- Acceder a la siguiente puerta usando `nextGateIndex`.
- Acceder a su `queue`.
- Agregar el `overflowAmount` al valor en la posición `tickIndex`.

# --hints--

Tu función `rerouteOverflow` debe agregar `overflowAmount` al valor en `tickIndex` dentro del `queue` de la siguiente puerta.
