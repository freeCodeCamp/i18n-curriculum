...
では、`fetch` の呼び出しに `.then()` メソッドを繋げてください。 `.then()` メソッドの括弧内には、仮引数 `res` を持つ コールバック関数を追加して、`Response` オブジェクトを確認するためその `res` をコンソールに出力してください。 Open your browser console and expand the `Response` object to see what it contains.
