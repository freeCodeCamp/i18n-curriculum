...
Wenn Sie auf einen Song in Ihrer Wiedergabeliste klicken, möchten Sie diesen Song abspielen. Dafür müssen Sie einen Ereignis-Listener für das `click`-Ereignis an jedem `button`-Element hinzufügen, das ein Kind eines `song`-Elements ist.

Vervollständigen Sie den Callback Ihres `forEach`, um dies zu erreichen. Rufen Sie innerhalb des `addEventListener`-Callbacks `playSong(n)` auf, wobei `n` die Song-ID ist, die Sie aus dem `id`-Attribut von `song` erhalten können.

Beachten Sie, dass jedes `song` ein `id`-Attribut mit `song-n` hat: Sie müssen den `n`-Teil extrahieren und in eine Zahl umwandeln, bevor Sie ihn an `playSong()` übergeben.


# --hints--

Sie sollten einen Ereignis-Listener für das `click`-Ereignis an jedem `button`-Element hinzufügen, das ein Kind eines `.playlist-song`-Elements ist, und `playSong` mit der `id` des entsprechenden Songs aufrufen.
