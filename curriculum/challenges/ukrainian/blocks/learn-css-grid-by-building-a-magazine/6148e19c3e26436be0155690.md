...
Створіть правило `.lists li`, щоб націлити елементи списку всередині вашого елемента `.lists`. Надайте йому множину властивостей `margin-bottom`, встановлену в `1.5rem`.

# --hints--

У вас має бути селектор `.lists li`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

Ваш селектор `.lists li` повинен мати властивість `margin-bottom`, встановлену в `1.5rem`.
