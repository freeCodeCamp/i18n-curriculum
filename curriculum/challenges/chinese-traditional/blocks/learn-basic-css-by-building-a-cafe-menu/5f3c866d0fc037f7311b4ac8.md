...
這比較接近了，但價格沒有保持在右邊。這是因為 `inline-block` 元素只佔據其內容的寬度。

為你的 `flavor` 和 `price` 類別選擇器添加一個 `width` 屬性，且每個的值為 `50%`，以將它們分散開來。

# --hints--

你應該在你的 `.flavor` 選擇器中將 `width` 屬性設為 `50%`。
...
你應該在你的 `.price` 選擇器中將 `width` 屬性設為 `50%`。
