...
Geben Sie Ihrer `MediaError`-Klasse eine `__init__`-Methode mit den folgenden Parametern: `self`, `message` und `obj`.

Rufen Sie darin `super().__init__(message)` auf, um sicherzustellen, dass die Mechanismen zum Speichern und Anzeigen der Fehlermeldung, die bereits in der `Exception`-Klasse vorhanden ist, richtig funktionieren.

Weisen Sie danach `obj` `self.obj` zu.

# --hints--

Ihre `MediaError`-Klasse sollte eine `__init__`-Methode mit den Parametern `self`, `message` und `obj` haben.
...
Sie sollten `super().__init__(message)` innerhalb Ihrer `__init__`-Methode aufrufen.
...
Sie sollten `obj` innerhalb Ihrer `self.obj`-Methode `__init__` zuweisen.
