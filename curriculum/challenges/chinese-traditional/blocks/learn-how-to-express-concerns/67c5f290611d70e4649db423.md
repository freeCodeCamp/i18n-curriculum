...
`To draft` means to create an initial version of something, such as a document, plan, or design. 例如：

`She drafted an email to send to the client before finalizing it.` - This means she wrote an early version of the email.

A `schedule` refers to a plan that outlines when tasks or events will take place. 例如：
