...
У вашому `thead` створіть елемент `tr`. Усередині нього додайте `td` та три елементи `th`. У кожному з елементів `th` додайте елемент `span` з атрибутом `class`, встановленим у `sr-only`, та наступним текстом у порядку: `2019`, `2020` і `2021`.
...
Ваш елемент `tr` має містити елемент `td` як перший дочірній елемент.
