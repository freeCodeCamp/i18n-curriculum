...
이제 `draw` 클래스에 `CheckPoint` 메서드를 만들어야 합니다.

`draw` 메서드 안에서 `fillStyle` 객체의 `ctx` 속성에 16진수 색상 `"#f1be32"`를 할당하세요.

`fillStyle` 속성 아래에서 `fillRect` 객체의 `ctx` 메서드를 호출하고 `x`, `y`, `width`, `height` 속성을 인수로 전달하세요.

# --hints--

`CheckPoint` 클래스에 `draw` 메서드가 있어야 합니다.
...
`draw` 메서드는 `fillStyle` 객체의 `ctx` 속성에 값을 할당해야 합니다.
...
16진수 색상 `"#f1be32"`를 `fillStyle` 객체의 `ctx` 속성에 할당해야 합니다.
...
`draw` 메서드는 `fillRect` 객체의 `ctx` 메서드를 호출해야 합니다.
...
`ctx.fillRect`를 호출할 때 `position.x`, `position.y`, `width`, `height` 속성을 인수로 전달해야 합니다. `this` 키워드를 잊지 마세요.
