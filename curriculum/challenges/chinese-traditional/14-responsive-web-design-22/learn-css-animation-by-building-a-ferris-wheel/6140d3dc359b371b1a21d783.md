...
Time to position the cabins around the wheel. Select the first `.cabin` element. 設置 `right` 屬性爲 `-8.5%`、`top` 屬性爲 `50%`。

# --hints--

你應該有一個 `.cabin:nth-of-type(1)` 選擇器。
...
你的 `.cabin:nth-of-type(1)` 選擇器應該有一個 `right` 屬性值爲 `-8.5%`。
...
你的 `.cabin:nth-of-type(1)` 選擇器應該有一個 `top` 屬性值爲 `50%`。
