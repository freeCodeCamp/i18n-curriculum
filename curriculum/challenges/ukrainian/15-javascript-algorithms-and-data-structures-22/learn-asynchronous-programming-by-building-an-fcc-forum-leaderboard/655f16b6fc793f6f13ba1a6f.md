...
У вікні попереднього перегляду ви повинні побачити стовпець із комами. Щоб виправити це, приєднайте метод `join` до методу `map`. Для роздільника введіть порожній рядок.
...
Передайте порожній рядок для роздільника.
