...
بعد ذلك، تحتاج إلى استرجاع الأغنية التالية في قائمة التشغيل. لهذا، يجب أن تحصل على فهرس الأغنية الحالية ثم تضيف `1` إليه.

أنشئ ثابتًا باسم `nextSong` وقم بتعيين `userData?.songs[currentSongIndex + 1]` له.

أخيرًا، استدعِ الدالة `playSong` ومرر `nextSong.id` كمعلمة.

# --hints--

لا تعدل جملة `if` الموجودة، ولا كتلة `else` أو محتواها.
...
يجب تعيين `userData?.songs[currentSongIndex + 1]` إلى ثابت `nextSong`.
...
يجب استدعاء الدالة `playSong` مع `nextSong.id`.
