...
Nella tua nuova funzione, dichiara una variabile `calories` e assegnale il valore `0`. Usa `let` per dichiararla, dato che la riassegnerai più avanti.
...
La tua variabile `calories` dovrebbe avere assegnato il valore `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Dovresti dichiarare la tua variabile `calories` con `let`.
