...
Por fim, você precisa retornar o valor de `mean`.

# --hints--

A função `getMean` deve usar a palavra-chave `return`.
...
A função `getMean` deve retornar o valor de `mean`.
...
Sua declaração `return` deve estar após a declaração da variável `mean`.
