...
أنشئ الآن حلقة `for` تقوم بتكرار `self.strategies`. لكل `strategy`، تحقق مما إذا كانت قابلة للتطبيق باستخدام الطريقة `is_applicable`. إذا كانت كذلك، احسب السعر بعد الخصم باستخدام `apply_discount` وخزنه في متغير اسمه `discounted`. ثم أضف السعر بعد الخصم إلى قائمة `prices`.

# --hints--

يجب أن تنشئ حلقة `for` تقوم بتكرار `self.strategies`.
...
داخل الحلقة، يجب أن تكون جملة `if` الخاصة بك هي `if strategy.is_applicable(product, user_tier):`.
...
إذا كانت الاستراتيجية قابلة للتطبيق، يجب أن تستدعي `apply_discount` وتضيف النتيجة إلى `prices`.
