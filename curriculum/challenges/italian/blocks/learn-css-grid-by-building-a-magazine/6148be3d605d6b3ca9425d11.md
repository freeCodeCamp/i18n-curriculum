...
Anche il sottotitolo deve essere stilizzato. Crea un selettore `.hero-subtitle` e imposta su di esso una proprietà `font-size` con valore `2.4rem`, una proprietà `color` con valore `orangered` e una proprietà `text-align` con valore `center`.

# --hints--

Dovresti avere un selettore `.hero-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

Il tuo selettore `.hero-subtitle` dovrebbe avere una proprietà `font-size` impostata su `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

Il tuo selettore `.hero-subtitle` dovrebbe avere una proprietà `color` impostata su `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

Il tuo selettore `.hero-subtitle` dovrebbe avere una proprietà `text-align` impostata su `center`.
