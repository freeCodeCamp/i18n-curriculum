...
يجب عليك إضافة `repeating-linear-gradient` إلى `.fb6` في خاصية `background`.
...
يجب عليك إضافة `repeating-linear-gradient` يحمل لون أولي `--building-color3` من `0%` إلى `10%`.
...
يجب أن تعطي `repeating-linear-gradient` لون ثان `transparent` من `10%` إلى `30%`.
