...
Huenda umeona kwamba `EVAChunks` ni safu ya data yenye safu ndogo ndani yake, au safu ya data ya vipimo viwili (2D). Mzunguko wa `for` ulio pangwa ndani unaweza kutumika kurekodi data kutoka kwa safu ya data ya vipimo viwili:
...
Katika mfano hapo juu, `rootArray` ni safu kuu, `rootArray[i]` ni safu ndogo ndani ya safu kuu, na `rootArray[i][j]` ni kitu kimoja ndani ya safu ndogo hiyo na `.property` hupata thamani ndani ya kitu hicho.

Tumia mzunguko wa `for` uliopangwa ndani rudia kupitia `EVAChunks` na rekodi `name` ya kila mwananga angani katika kila kipande. Hakikisha:

- unatumia `EVAChunks` kama safu yako kuu
- rekodi `Chunk ${i+1}:` kati ya vipande

# --hints--

Unapaswa kutumia mzunguko wa `for` uliopangwa ndani rudia kupitia `EVAChunks` na rekodi `name` ya kila mwananga angani katika kila kipande. Rekodi `Chunk ${i+1}:` kati ya vipande.
