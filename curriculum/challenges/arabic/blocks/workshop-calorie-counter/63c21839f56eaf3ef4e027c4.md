...
أنشئ عنصر `label` آخر (في سطر جديد) في نهاية `HTMLString`. يجب أن يحتوي هذا `label` على النص `Entry # Calories`، مستخدمًا صيغة القالب النصي لاستبدال `#` بقيمة `entryNumber`، وأن تكون خاصية `for` مساوية لـ `X-#-calories`، حيث أن `X` هي `value` لـ `entryDropdown` و`#` هي قيمة `entryNumber`.

# --hints--

يجب أن يكون لديك عنصران `label` في `HTMLString` الخاص بك.
...
يجب أن يكون عنصر `label` الجديد في سطر جديد.
...
يجب أن يأتي عنصر `label` الجديد بعد عنصر `input` الخاص بك.
...
يجب أن يحتوي عنصر `label` الجديد على خاصية `for` مضبوطة على `${entryDropdown.value}-${entryNumber}-calories`.
...
يجب أن يحتوي عنصر `label` الجديد على النص `Entry ${entryNumber} Calories`.
...
لا يجب تعديل العناصر الموجودة لديك.
