...
現在你可以建置 `attack` 函式（程式）。首先，更新 `text` 訊息為 `"The <monster name> attacks."`，將 `<monster name>` 替換成怪物的名稱。記得你可以使用串接運算子來達成這點。

# --hints--

你應該使用點記法來存取 `text` 的 `innerText` 屬性。

```js
assert.match(attack.toString(), /text\.innerText/);
```

你應該將字串 `"The "` 指定給 `text` 的 `innerText` 屬性。

```js
assert.match(attack.toString(), /text\.innerText\s*=\s*('|")The \1/);
```

你應該使用串接運算子將 `monsters[fighting].name` 的值添加到字串 `"The "`。

```js
assert.match(attack.toString(), /text\.innerText\s*=\s*('|")The \1\s*\+\s*monsters\s*\[\s*fighting\s*\]\s*\.name/);
```

你應該使用串接運算子將字串 `" attacks."` 添加到 `monsters[fighting].name` 字串。
