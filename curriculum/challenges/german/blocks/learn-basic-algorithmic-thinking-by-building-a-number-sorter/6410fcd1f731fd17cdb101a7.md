...
Fügen Sie innerhalb Ihrer verschachtelten `for`-Schleife einen `console.log()`-Aufruf hinzu, um bei jeder Iteration die Werte von `array`, `array[j]` und `array[minIndex]` zu überprüfen. Sie können auf die `Sort`-Schaltfläche klicken, um zu sehen, wie Ihr Algorithmus das Array durchläuft.

Schreiben Sie dann eine `if`-Anweisung, die überprüft, ob der Wert bei `array[j]` kleiner ist als der Wert bei `array[minIndex]`. Falls ja, setzen Sie `minIndex` auf `j`.

# --hints--

Sie sollten einen `console.log()`-Aufruf innerhalb Ihrer verschachtelten `for`-Schleife haben.
...
Sie sollten `array` als erstes Argument an `console.log()` übergeben.
...
Sie sollten `array[j]` als zweites Argument an `console.log()` übergeben.
...
Sie sollten `array[minIndex]` als drittes Argument an `console.log()` übergeben.
...
Sie sollten eine `if`-Anweisung nach Ihrem `console.log()`-Aufruf haben.
...
Ihre `if`-Anweisung sollte eine Bedingung enthalten, die überprüft, ob `array[j]` kleiner ist als `array[minIndex]`.
...
Ihre `if`-Anweisung sollte `minIndex` auf `j` setzen.
