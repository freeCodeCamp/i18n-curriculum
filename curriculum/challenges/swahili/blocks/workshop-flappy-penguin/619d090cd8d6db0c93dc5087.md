...
Chini ya kipengele cha `.chin`, ongeza vipengele viwili vya `div` kila kikiwa na `class` ya `eye`. Pia, mpe kipengele cha kwanza cha `.eye` `class` ya `left`, na kipengele cha pili cha `.eye` `class` ya `right`.
...
Unapaswa kumpa `div` mpya ya kwanza `class` ya `eye`.

```js
assert.exists(document.querySelector('.penguin-head > div.eye'));
```

Unapaswa kumpa `div` mpya ya pili `class` ya `eye`.
