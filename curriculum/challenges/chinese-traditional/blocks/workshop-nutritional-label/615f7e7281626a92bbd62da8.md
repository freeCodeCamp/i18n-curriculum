...
在最後一個 `p` 元素下方，使用文本 `Sodium 160mg 7%` 創建另一個 `p` 元素。 將 `Sodium` 和 `7%` 分別放在各自的 `span` 中，並像處理其他內容一樣，給它們設置 `bold` 類。

然後，在 `Sodium 160mg` 外面添加一個 `span` 元素，以便對齊。
...
`Sodium` 周圍的 `span` 應該將 `class` 屬性設置爲 `bold`。
...
`7%` 周圍的 `span` 元素應將 `class` 屬性設置爲 `bold`。
