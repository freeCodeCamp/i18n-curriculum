...
سيتم تمرير اسم دالة إلى المعلمة `fn`، مثل `"SUM"`. حدّث `apply` ليُرجع ضمنيًا الدالة من كائن `spreadsheetFunctions` باستخدام المتغير `fn` كمفتاح للوصول إلى الكائن.

تذكر أن `fn` قد لا يحتوي على سلسلة نصية بأحرف صغيرة، لذا عليك تحويله إلى سلسلة نصية بأحرف صغيرة.

# --hints--

يجب أن تستخدم دالة `apply` إرجاعًا ضمنيًا.
...
يجب أن تصل دالة `apply` إلى كائن `spreadsheetFunctions`.
...
يجب أن تصل دالة `apply` إلى الخاصية في كائن `spreadsheetFunctions` التي تطابق قيمة `fn`.
...
يجب أن تستدعي دالة `apply` الطريقة `.toLowerCase()` على `fn` عند الوصول إلى الخاصية.
