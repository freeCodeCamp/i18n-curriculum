...
更改 `.penguin` 元素變換過程中的 `transition` 行爲， 設置動畫持續時間爲 `1s`，緩動函數爲 `ease-in-out`，延遲爲 `0ms`。

# --hints--

你應該將 `.penguin` 的 `transition-duration` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionDuration, '1s');
```

你應該將 `.penguin` 的 `transition-timing-function` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionTimingFunction, 'ease-in-out');
```

你應該將 `.penguin` 的 `transition-delay` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
