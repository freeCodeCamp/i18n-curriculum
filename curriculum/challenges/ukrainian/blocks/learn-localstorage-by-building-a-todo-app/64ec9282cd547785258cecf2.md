...
Використовуючи стрілковий синтаксис, завершіть функцію зворотного виклику `forEach`. Усередині тіла функції зворотного виклику використайте операцію додавання з присвоєнням, щоб призначити `innerHTML` об’єкта `tasksContainer` порожні лапки.

# --hints--

Не слід змінювати існуючий `taskData.forEach()` та значення, які ви деструктурували.
...
Використайте стрілковий синтаксис і приєднайте `innerHTML` до `tasksContainer`.

```js
assert.match(code, /taskData\.forEach\(\s*\(\s*\{.*\}\s*\)\s*=>\s*(\s*\{?\(\s*tasksContainer\.innerHTML\s*\)\s*\}?)?/)
```

Використайте операцію додавання з присвоєнням, щоб призначити `innerHTML` об’єкта `tasksContainer` порожні лапки.
