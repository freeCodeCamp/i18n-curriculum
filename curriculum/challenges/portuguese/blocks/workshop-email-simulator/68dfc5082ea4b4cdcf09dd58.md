...
Depois de obter o índice real, crie uma instrução if para verificar se o `actual_index` é menor que 0 ou maior ou igual ao comprimento da lista `self.emails`. Se for, imprima a mensagem `Invalid email number.\n` e use `return` para sair do método.
...
Se o índice for inválido, você deve imprimir a mensagem `Invalid email number.\n` e usar `return` para sair do método.
