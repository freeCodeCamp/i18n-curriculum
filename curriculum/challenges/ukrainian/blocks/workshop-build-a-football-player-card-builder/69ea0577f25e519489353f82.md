...
У блоці `catch` виведіть у консоль рядок `Failed to load player data, using defaults:`, а потім параметр `error`.

Потім, поза межами `catch`, але все ще в межах функції `loadPlayer`, поверніть `defaultPlayer`.

Усе це завантажить дані гравця, заповнить їх у інтерфейсі користувача та завантажить об’єкт `defaultPlayer`, якщо процес не вдасться.

# --hints--

Ваш блок `catch` має вивести `'Failed to load player data, using defaults:', error` у консоль.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

Ваша функція `loadPlayer` має повертати `defaultPlayer` як запасний варіант.
