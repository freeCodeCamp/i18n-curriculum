...
`checkMessageButton`에 이벤트 리스너를 붙여 `click` 이벤트를 감지하세요. 빈 콜백 함수를 지정하세요.

# --hints--

`.addEventListener()` 요소에서 `checkMessageButton`를 호출해야 합니다.

```js
assert.match(code, /checkMessageButton\.addEventListener\(/);
```

`.addEventListener()` 메서드는 `"click"` 이벤트 타입을 가져야 합니다.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,/);
```

`.addEventListener()` 메서드는 빈 콜백 함수를 가져야 합니다.
