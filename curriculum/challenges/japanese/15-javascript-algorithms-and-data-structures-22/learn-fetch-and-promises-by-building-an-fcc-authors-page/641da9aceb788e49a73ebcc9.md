...
ステップ 1 で、`Load More Authors` ボタンを取得して `loadMoreBtn` に割り当てたことを思い出してください。

Use `addEventListener` to add a `"click"` event listener to `loadMoreBtn`. また、`fetchMoreAuthors` 関数への参照を渡し、ボタンがクリックされる度に実行されるようにしましょう。

これが完了すると、ボタンをクリックする度に追加で 8 人の著者データが表示されるようになります。
