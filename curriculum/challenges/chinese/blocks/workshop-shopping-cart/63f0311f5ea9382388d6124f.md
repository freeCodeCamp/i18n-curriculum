...
`.toFixed()` 返回字符串的问题是你希望能够使用税率进行计算。 为了解决这个问题，你可以将 `.toFixed()` 调用（包括计算）传递给 `parseFloat()` 函数。 这会将固定字符串转换回数字，保留现有的小数位。

将你的 `.toFixed()` 调用传递给 `parseFloat()`。

# --hints--

你应该将整个计算（不包括 `return` 语句）传递给 `parseFloat()`。
