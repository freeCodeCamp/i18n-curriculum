...
Масиви мають метод `.some()`. Як й метод `.filter()`, `.some()` приймає функцію зворотного виклику, яка має приймати елемент масиву як аргумент. Метод `.some()` поверне `true`, якщо функція зворотного виклику поверне `true` принаймні для одного елемента в масиві.

Ось приклад виклику методу `.some()`, щоб перевірити, чи є будь-який елемент у масиві великою літерою.
...
Використайте метод `.some()`, щоб перевірити, чи тестування `msg` на будь-яких з регулярних виразів `denyList` повертає `true`.

Use `regex` as the parameter for the callback function, for clarity.

# --hints--

Функція `isSpam` має неявно повернути результат `denyList.some()`.
