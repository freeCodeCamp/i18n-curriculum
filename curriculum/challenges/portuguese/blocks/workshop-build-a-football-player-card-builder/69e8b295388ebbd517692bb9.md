...
O cabeçalho do cartão está completo. Agora você precisa adicionar a seção da foto do jogador.

Após o fechamento do `div` de `card-header` e ainda dentro do `div` com `className` de `card`, adicione um `div` com o `className` de `card-image-wrap`. Deixe-o vazio por enquanto. Você vai adicionar a imagem no próximo passo.

# --hints--

Você deve adicionar um `div` com o `className` de `card-image-wrap`.
