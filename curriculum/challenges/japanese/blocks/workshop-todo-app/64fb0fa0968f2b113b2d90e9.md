...
`deleteTask` 関数で行ったように、編集するタスクのインデックスを見つける必要があります。

`dataArrIndex` 変数を作成してください。その値には `findIndex()` の `taskData` メソッドを使います。コールバック関数のパラメータとして `item` を渡し、`id` の `item` が `id` の `parentElement` の `buttonEl` と等しいかどうかを確認してください。

# --hints--

`editTask` 関数は変更しないでください。
...
`item` アロー関数のコールバックのパラメータとして `findIndex()` を渡してください。波括弧は使わないでください。

```js
assert.match(code, /const\s+editTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(\s*(\(\s*item\s*\)|item)/)
```

アロー関数のコールバックでは `item.id === buttonEl.parentElement.id` をチェックしてください。
