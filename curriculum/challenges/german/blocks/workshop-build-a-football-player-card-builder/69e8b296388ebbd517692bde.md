...
Fügen Sie dem DRI-Input ein `value`-Attribut mit dem Wert `player.dri` hinzu. Dann fügen Sie ein `onChange`-Attribut hinzu, das auf eine Pfeilfunktion gesetzt ist, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `dri` auf `Number(e.target.value)` gesetzt ist.

# --hints--

Ihr DRI-`input` sollte ein `value` haben, das dem `dri`-Wert des Spielers entspricht.
...
Ihr DRI-`input` sollte einen `onChange`-Handler haben, der `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `dri` auf `Number(e.target.value)` gesetzt ist.
