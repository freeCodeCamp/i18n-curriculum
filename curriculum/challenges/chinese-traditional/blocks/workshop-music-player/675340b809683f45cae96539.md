...
如果在沒有播放任何歌曲時按下上一首按鈕，將會播放第一首歌曲。為了修正這個問題，請在 `playPreviousSong` 函式的開頭添加一個 `if` 述語。檢查目前的歌曲是否為 `null`，並使用 `return` 述語停止函式執行。

# --hints--

你應該為你的 `playPreviousSong` 函式添加一個 `if` 述語，檢查 `userData.currentSong === null`，並在其中使用 `return` 述語停止函式執行。
