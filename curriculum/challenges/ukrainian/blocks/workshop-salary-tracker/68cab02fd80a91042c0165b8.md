...
Після вашої існуючої інструкції `if` створіть ще одну для випадку, коли нова зарплата менша за базову зарплату для поточного рівня. Використовуйте `hasattr(self, '_level')`, щоб уникнути `AttributeError` під час ініціалізації.

Всередині інструкції `if` викличте виключення `ValueError` з повідомленням `Salary must be higher than minimum salary $`, після якого вкажіть базову зарплату для поточного рівня та крапку.

# --hints--

У вашому сеттері `if` має бути друга інструкція `salary`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[1]`)) })
```

Ваша інструкція `if` має використовувати `hasattr(self, '_level')`, щоб перевірити наявність `_level` перед порівнянням.
...
Коли нова зарплата менша за базову зарплату для поточного рівня, слід викликати виключення `ValueError` з повідомленням `Salary must be higher than minimum salary $`, після якого вказати базову зарплату для поточного рівня та крапку.
...
Не слід викликати жодного виключення, коли `new_salary` більша за поточну зарплату.
