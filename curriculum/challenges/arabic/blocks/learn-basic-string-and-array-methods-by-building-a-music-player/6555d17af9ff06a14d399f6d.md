...
داخل الدالة، احصل على مراجع لعناصر HTML المسؤولة عن عرض عنوان الأغنية والفنان.

احصل على عناصر `#player-song-title` و `#player-song-artist` باستخدام طريقة `getElementById()`. عيّنها إلى المتغيرين `playingSong` و `songArtist` على التوالي.

# --hints--

لا تعدل الدالة `setPlayerDisplay` الحالية ومحتواها.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

استخدم `document.getElementById()` للحصول على عنصر `#player-song-title`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

عيّن عنصر `#player-song-title` إلى المتغير `playingSong`. لا تنسَ استخدام `const` لإعلان المتغير.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

استخدم `document.getElementById()` للحصول على عنصر `#player-song-artist`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

عيّن عنصر `#player-song-artist` إلى المتغير `songArtist`. لا تنسَ استخدام `const` لإعلان المتغير.
