...
`:root`で変数を作成すると、その変数の値はページ全体に設定されます。

その後、特定のセレクター内で変数を再度設定することで、これらの変数を上書きできます。

# --instructions--

`--penguin-belly`クラス内で`white`の値を`penguin`に変更してください。

# --hints--

`penguin`クラスは`--penguin-belly`変数を`white`に再代入する必要があります。

```js
assert.match(__helpers.removeCssComments(code), /\.penguin\s*?{[\s\S]*--penguin-belly\s*?:\s*?white\s*?;[\s\S]*}/gi);
```

`penguin`クラスには`background-color`プロパティを含めないでください。
