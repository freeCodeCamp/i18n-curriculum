...
現在再生中の曲の`id`を取得する必要があります。そのために`userData.currentSong?.id`を使えます。

`getElementById()`を使って現在再生中の曲を選択してください：テンプレートリテラルを渡して`id`に`song-`を接頭辞として付けます。`songToHighlight`という名前の変数に代入してください。

# --hints--

既存の`highlightCurrentSong`関数とその内容は変更しないでください。
...
`document.getElementById()`を使い、`` `song-${userData.currentSong?.id}` ``を渡してください。

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

`getElementById()`を`songToHighlight`変数に代入してください。
