...
تريد أن تجلس حاوية `.foreground-buildings` مباشرة فوق عنصر `.background-buildings`. امنحها `width` و `height` بقيمة `100%`، واضبط `position` على `absolute`، و `top` على `0`. هذا سيجعلها بنفس حجم الجسم وينقل بدايتها إلى الزاوية العلوية اليسرى.

# --hints--

يجب أن تستخدم محدِّد `.foreground-buildings`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.foreground-buildings'));
```

يجب أن تعطي عنصر `.foreground-buildings` `width` بقيمة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.width, '100%');
```

يجب أن تعطي عنصر `.foreground-buildings` `height` بقيمة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.height, '100%');
```

يجب أن تعطي عنصر `.foreground-buildings` `position` بقيمة `absolute`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.position, 'absolute');
```

يجب أن تعطي عنصر `.foreground-buildings` `top` بقيمة `0`.
