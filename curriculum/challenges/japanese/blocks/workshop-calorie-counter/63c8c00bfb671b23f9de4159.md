...
入力が有効であることも確認する必要があります。`invalidInputMatch` 変数を宣言し、`isInvalidInput` 関数を `currVal` を引数として呼び出した結果を代入してください。

# --hints--

`invalidInputMatch` 変数を宣言してください。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=/);
```

`isInvalidInput` 変数に `invalidInputMatch` を呼び出した結果を代入してください。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(/);
```

`currVal` に `isInvalidInput` を引数として渡してください。
