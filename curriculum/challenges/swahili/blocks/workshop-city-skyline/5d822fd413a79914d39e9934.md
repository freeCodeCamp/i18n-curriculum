...
Mwishowe! Umefika kwenye jengo la mwisho! Ongeza mwelekeo unaojirudia kwa `90deg`. Tumia rangi ya jengo kutoka `0%` hadi `10%` na `transparent` kutoka `10%` hadi `30%`.
...
Unapaswa kumpa `repeating-linear-gradient` mwelekeo wa `90deg`.
...
Unapaswa kumpa `repeating-linear-gradient` rangi ya pili ya `transparent` kutoka `10%` hadi `30%`.
