...
宣告一個 `apply` 函式（程式），該函式接受 `fn` 和 `args` 參數。
...
你的 `apply` 函式（程式）應該將 `args` 作為第二個參數。
