...
Weisen Sie den `span`-Elementen innerhalb jedes Listenelements die Klassen `text-green-500` und `mr-2` zu, damit die Marker grün werden und etwas vom Text abgerückt sind.

Weisen Sie ihnen außerdem das `aria-hidden`-Attribut zu, so wie Sie es bei den `span`-Elementen des ersten Preismodells gemacht haben.

# --hints--

Ihre `span`-Elemente sollten die Klasse `text-green-500` haben.
...
Ihre `span`-Elemente sollten die Klasse `mr-2` haben.
...
Ihre `span`-Elemente sollten das `aria-hidden`-Attribut haben.
