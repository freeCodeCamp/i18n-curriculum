...
`if` 문장 뒤에, `isSpam()`을 인수로 하여 `messageInput.value`를 호출한 결과의 진릿값을 삼항 연산자로 확인하세요. 참이면 `result` 요소의 `textContent` 속성을 `"Oh no! This looks like a spam message."`으로 설정하세요. 그렇지 않으면 `"This message does not seem to contain any spam."`으로 설정하세요.

그다음 `messageInput` 요소의 `value` 속성을 빈 문자열로 설정하세요.

# --hints--

`result` 요소의 `textContent` 속성을 설정하려면 대입 연산자를 사용하세요.
...
삼항의 참 표현식은 `result` 요소의 `textContent` 속성을 `"Oh no! This looks like a spam message."`으로 설정해야 합니다.
...
삼항의 거짓 표현식은 `result` 요소의 `textContent` 속성을 `"This message does not seem to contain any spam."`으로 설정해야 합니다.
...
삼항 연산자 뒤에 `messageInput` 요소의 `value` 속성을 빈 문자열로 설정하세요.
