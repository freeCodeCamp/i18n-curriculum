...
Durante lo sviluppo, è utile vedere il bordo predefinito intorno agli elementi `fieldset`. Tuttavia, potrebbe non essere lo stile che vuoi.
...
Dovresti dare a `.question` una proprietà `border` con il valore `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

Dovresti dare a `.question` una proprietà `padding-bottom` di `0`.
