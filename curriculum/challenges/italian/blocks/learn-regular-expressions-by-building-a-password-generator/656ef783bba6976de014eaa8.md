...
Metti la dichiarazione della variabile `password` e il successivo loop `for` all'interno di un loop `while`. Usa `True` come condizione per il nuovo loop.

# --hints--

Dovresti creare un loop `while True` che racchiuda la tua dichiarazione `password` esistente e il loop `for`.
