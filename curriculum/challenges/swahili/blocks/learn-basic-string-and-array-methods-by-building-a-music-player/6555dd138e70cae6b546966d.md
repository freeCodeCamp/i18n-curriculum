...
Unahitaji kupata wimbo unaochezwa kwa sasa au wimbo wa kwanza katika orodha ya nyimbo. Ili kufanya hivyo, tengeneza thabiti ya `song` na tumia mtaalamu wa OR (`||`) kuiweka kuwa wimbo wa sasa wa `userData`, **au** wimbo wa kwanza katika safu ya data ya `userData?.songs`.

Usisahau kutumia uunganishaji wa mfululizo.

# --hints--

Haupaswi kubadilisha `setPlayButtonAccessibleText` iliyopo na yaliyomo ndani yake.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Unapaswa kupata upatikanaji wa `userData?.currentSong` au `userData?.songs[0]`.

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

Unapaswa kuweka thamani ya `userData?.currentSong || userData?.songs[0]` katika thabiti ya `song`.
