...
Isso move o texto do badge, mas não exatamente para onde você quer. A mágica está nas classes utilitárias `right-[num]`, `left-[num]`, `top-[num]` e `bottom-[num]`.
...
Você também pode indicar valores negativos prefixando-os com um hífen:
...
Adicione as classes `-top-3` e `right-3` ao `Most popular` badge `div`.

# --hints--

Seu elemento `div` com o texto `Most popular` deve ter a classe `-top-3`.
...
Seu elemento `div` com o texto `Most popular` deve ter a classe `right-3`.
