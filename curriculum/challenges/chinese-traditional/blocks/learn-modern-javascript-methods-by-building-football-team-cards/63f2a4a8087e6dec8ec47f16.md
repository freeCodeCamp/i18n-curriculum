...
接著，為 `"forward"` 添加一個 `case` 語句。 

在該 `case` 中，使用引數為 `players.filter()` 的方式呼叫 `setPlayerCards` 函式（程式）。

在 `filter()` 方法中，為你的回呼函式添加一個 `player` 參數，該參數將檢查 `player.position` 是否相等於 `"forward"`。

最後，在 `setPlayerCards` 函式（程式）呼叫下方添加一個 `break` 述語。 

# --hints--

你應該在你的 `switch` 述語中為 `"forward"` 添加一個新的 `case`。

```js
assert.match(code, /\s*case\s*('|"|`)\s*forward\s*\1\s*:\s*/)
```

在你的 `"forward"` `case` 中，你應該呼叫 `setPlayerCards` 函式（程式）。
...
你的 `filter()` 回呼應該隱喻地傳回檢查 `player.position` 是否為 `"forward"` 的結果。
