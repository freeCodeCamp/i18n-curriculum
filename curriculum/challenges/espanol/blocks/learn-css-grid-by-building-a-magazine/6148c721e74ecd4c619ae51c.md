...
Su elemento `.text` no es un CSS Grid, pero puede crear columnas dentro de un elemento sin usar Grid mediante la propiedad `column-width`.

Dé a su selector `.text` una propiedad `column-width` establecida en `25rem`.
