...
`PlayerCard` コンポーネントは動作しますが、ハードコードされた値を使っています。柔軟にするために、TypeScriptのインターフェースを使ってプレイヤーデータの形状を記述する必要があります。

`PlayerCard` 関数の前に、`PlayerData` という名前のインターフェースを作成してください。このインターフェースには、`name` が `string` 型、`overallRating` が `number` 型、`position` が `string` 型、`club` が `string` 型であることを含めてください。

# --hints--

`PlayerData` という名前のインターフェースを作成してください。
...
`PlayerData` インターフェースには `name` が `string` 型である必要があります。
...
`PlayerData` インターフェースには `overallRating` が `number` 型である必要があります。
...
`PlayerData` インターフェースには `position` が `string` 型である必要があります。
...
`playerData` インターフェースには `club` が `string` 型である必要があります。
