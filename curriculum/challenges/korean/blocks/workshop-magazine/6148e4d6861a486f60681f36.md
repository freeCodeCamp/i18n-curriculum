...
이미지는 2열 3행 레이아웃 안에 있어야 합니다.

`.image-wrapper` 선택자에 `grid-template-columns` 속성을 `2fr 1fr`로, `grid-template-rows` 속성을 `repeat(3, min-content)`로 설정하세요. 이렇게 하면 컨테이너를 기준으로 고정 너비를 유지하는 열과 콘텐츠에 따라 높이가 조절되는 행을 가진 그리드를 만들 수 있습니다.

# --hints--

`.image-wrapper` 선택자에 `grid-template-columns` 속성을 `2fr 1fr`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

`.image-wrapper` 선택자에 `grid-template-rows` 속성을 `repeat(3, min-content)`로 설정하세요.
