...
Se la funzione freccia restituisce un'espressione semplice, puoi omettere la parola chiave `return` e le parentesi graffe `{}`. Questo si chiama ritorno implicito.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

Se la tua funzione freccia ha più righe di codice nel corpo della funzione, allora devi usare la parola chiave `return` e le parentesi graffe `{}`.
...
Rifattorizza, o aggiorna, la tua funzione `addTwoNumbers` per rimuovere la parola chiave `return` e le parentesi graffe `{}`. La tua funzione `addTwoNumbers` dovrebbe invece usare un ritorno implicito.

Apri la console per assicurarti di ottenere ancora l'output corretto.

# --hints--

La tua funzione `addTwoNumbers` rifattorizzata dovrebbe usare un ritorno implicito.
