...
Визначте `pauseSong` функцію, яка не приймає параметрів. Усередині нової функції, щоб зберегти поточний час пісні під час паузи, встановіть `songCurrentTime` об’єкта `userData` у значення `currentTime` змінної `audio`.


# --hints--

У вас має бути `pauseSong` функція.

```js
assert.isFunction(pauseSong);
```

Ваша `pauseSong` функція має встановити `songCurrentTime` об’єкта `userData` у значення `currentTime` змінної `audio`.
