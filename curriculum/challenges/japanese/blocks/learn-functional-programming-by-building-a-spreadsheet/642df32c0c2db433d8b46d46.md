...
最後に、`.appendChild()` メソッドを使って、`label` 要素を `container` 要素に追加してください。

# --hints--

`.appendChild()` 要素の `container` メソッドにアクセスしてください。

```js
assert.match(code, /container\.appendChild\(/);
```

`label` 要素を `.appendChild()` メソッドに渡してください。

```js
assert.match(code, /container\.appendChild\(\s*label\s*\)/);
```

属性を設定した後に `label` を追加してください。
