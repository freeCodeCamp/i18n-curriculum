...
Für den zweiten `fieldset` sollen die `input` und `label` Texte in derselben Zeile erscheinen.

Beginnen Sie damit, den `input`-Elementen im zweiten `fieldset` die Klasse `inline` zu geben.

# --hints--

Sie sollten dem ersten `input` die Klasse `inline` zuweisen.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

Sie sollten dem zweiten `input` die Klasse `inline` zuweisen.
