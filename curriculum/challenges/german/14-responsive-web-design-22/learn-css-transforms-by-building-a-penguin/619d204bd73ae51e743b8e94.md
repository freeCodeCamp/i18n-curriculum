...
Du solltest `.foot` ein `top` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.top, '85%');
```

Du solltest `.foot` einen `border-radius` von `--fcc-expected--` zuweisen, aber es wurde `--fcc-actual--` gefunden.
