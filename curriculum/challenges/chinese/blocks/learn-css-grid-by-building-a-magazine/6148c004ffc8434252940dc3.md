...
创建一个 `.social-icons` 选择器。 将其 `display` 属性设置为 `grid`，并将 `font-size` 属性设置为 `3rem`。

# --hints--

你应该有一个 `.social-icons` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

你的 `.social-icons` 选择器应该有一个 `display` 属性，设置为 `grid`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display === 'grid');
```

你的 `.social-icons` 选择器应该有一个 `font-size` 属性，设置为 `3rem`。
