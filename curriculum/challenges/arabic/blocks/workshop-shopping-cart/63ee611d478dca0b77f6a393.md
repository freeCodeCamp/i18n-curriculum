...
تعلمت في محاضرة سابقة أن الكلمة المفتاحية `this` تشير إلى النسخة الحالية التي تُبنى، كما رأيت في دالة البناء `Dessert`. هنا، ستطبق نفس الفكرة على صنف `ShoppingCart`.
...
في دالة البناء، استخدم الكلمة المفتاحية `this` لتعيين الخاصية `items` إلى مصفوفة فارغة. أيضًا، عيّن الخاصية `total` إلى `0`، والخاصية `taxRate` إلى `8.25`.
