...
你的 `calculateTaxes` 方法应返回 `taxRate` 的值（除以 100，将其转换为百分比）乘以 `amount` 参数。

为了清楚起见，将 `taxRate / 100` 计算结果括在括号中。

# --hints--

你应该在 `calculateTaxes` 方法中将 `taxRate` 除以 100。 请记住使用 `this` 关键字。
...
你应该将 `this.taxRate / 100` 计算结果括在括号中。
...
你应该在 `calculateTaxes` 方法中将 `amount` 参数乘以 `taxRate`，然后除以 100。
