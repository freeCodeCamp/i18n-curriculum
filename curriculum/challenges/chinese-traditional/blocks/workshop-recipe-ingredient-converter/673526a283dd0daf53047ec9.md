...
現在你需要創建你的第一個柯里化函式（程式）。請記住，柯里化函式（程式）是傳回另一個函式（程式）的函式（程式），如下所示：
...
首先宣告一個 `convertQuantity` 函式（程式）。它應該只有一個 `fromUnit` 參數，且應該是空的。

# --hints--

你應該宣告一個 `convertQuantity` 函式（程式）。

```js
assert.isFunction(convertQuantity)
```

你的 `convertQuantity` 函式（程式）應該有一個 `fromUnit` 參數。

```js
assert.match(convertQuantity?.toString(), /\(fromUnit\)/);
```

你的 `convertQuantity` 函式（程式）應該是空的。
