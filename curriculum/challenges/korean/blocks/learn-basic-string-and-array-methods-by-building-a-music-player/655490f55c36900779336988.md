...
`playlistSongElements`를 `forEach` 메서드로 반복하세요.

`forEach` 메서드는 배열을 반복하며 배열의 각 요소에 함수를 수행하는 데 사용됩니다. 예를 들어, 숫자 배열이 있고 각 숫자를 콘솔에 출력하고 싶다고 가정해 보세요.
...
`forEach`에 `playlistSongElements` 메서드를 사용하세요. 매개변수로 `songEl`를 전달하고 화살표 구문을 사용해 빈 콜백을 추가하세요.

# --hints--

`forEach()`에 `playlistSongElements`를 체인으로 연결해야 합니다.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

`forEach()` 메서드는 `songEl` 매개변수를 가져야 하며 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

`forEach`은 빈 중괄호 쌍을 가져야 합니다.
