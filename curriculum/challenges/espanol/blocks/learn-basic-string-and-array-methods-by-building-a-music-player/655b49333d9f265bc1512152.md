...
Observa que la carátula del álbum en el HTML y las canciones en el arreglo `userData.songs` han cambiado. Hemos intercambiado las canciones originales por otras más cortas que puedes usar para probar tu aplicación en los próximos pasos.

A continuación, necesitas verificar si hay una siguiente canción para reproducir. Recupera el índice de la canción actual llamando a la función `getCurrentSongIndex()` y guárdalo en una constante `currentSongIndex`.

Después de eso, crea una constante `nextSongExists` que contenga el valor booleano `true` o `false` dependiendo de si la siguiente canción existe.

# --hints--

No debes modificar el listener de eventos `ended` ni su contenido.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Debes crear una constante `currentSongIndex` y establecerla al llamar a la función `getCurrentSongIndex()`.
...
Debes verificar si existe una siguiente canción comparando `userData.songs.length` y `currentSongIndex` y establecerlo en una constante `nextSongExists`. Si el último índice del arreglo de canciones (`userData.songs.length - 1`) es mayor que el currentSongIndex, significa que existe una siguiente canción.
