...
將 `.shirt` 元素設為標的，並將其 `font-size` 設為 `25px`、`font-family` 設為 `Helvetica`（備用字型為 `sans-serif`），以及 `font-weight` 設為 `bold`。
...
你應該給 `.shirt` 一個 `font-size` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontSize, '25px');
```

你應該給 `.shirt` 一個 `font-family` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontFamily, 'Helvetica, sans-serif');
```

你應該給 `.shirt` 一個 `font-weight` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。
