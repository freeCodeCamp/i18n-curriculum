...
Crie outro elemento `p` após o último `.divider` e dê a ele o texto `Total Sugars 12g`. Atribua nesse elemento `p` os valores de `class` necessários para recuá-lo e remover a borda inferior. Em seguida, crie outro `.divider` abaixo do novo elemento `p`.
...
O novo elemento `p` deve ter o texto `Total Sugars 12g`.
