...
Você deve criar uma cópia superficial do `crew` array usando o método `slice()`. Nomeie o novo array `updatedCrew`.

Lembre-se de que chamar `slice()` sem argumentos retorna uma cópia de todo o array:
...
Você deve usar o método `slice()` para copiar o array de entrada `crew` e armazenar o resultado em uma nova variável chamada `updatedCrew`.
