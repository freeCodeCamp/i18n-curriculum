...
La salida se está volviendo bastante difícil de leer, así que deberías reemplazar `print(graph.create_trajectory())` con un ciclo que imprima cada fila por separado. 

# --hints--

Elimina la llamada a `print(graph.create_trajectory())`.
...
Debes tener un ciclo que itere sobre `graph.create_trajectory()`.
...
Debes imprimir cada fila en `graph.create_trajectory()`.
