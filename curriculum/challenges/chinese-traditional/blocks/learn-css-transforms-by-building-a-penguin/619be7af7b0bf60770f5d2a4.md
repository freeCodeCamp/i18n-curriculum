...
將僞元素放置在其父元素頂部 `10%` 和左側 `25%` 的位置。

# --hints--

你應該將 `.penguin-body::before` 的 `top` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

你應該將 `.penguin-body::before` 的 `left` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
