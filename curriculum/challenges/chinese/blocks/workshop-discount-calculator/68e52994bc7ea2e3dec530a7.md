...
要完成工作坊，为你的代码添加一个最终的打印语句，使用 f-string 以两位小数的形式显示最佳价格，格式为：`Best price for [product.name] for [user_tier] user: $[best_price]`，其中 `[product.name]`、`[user_tier]` 和 `[best_price]` 应替换为实际的变量值。

一个示例输出可能是：`Best price for Wireless Buds for Premium user: $93.00`。请注意价格的小数点后两位。

至此，折扣计算器工作坊完成！

# --hints--

你的输出应匹配格式：`Best price for {product.name} for {user_tier} user: ${best_price:.2f}`。
