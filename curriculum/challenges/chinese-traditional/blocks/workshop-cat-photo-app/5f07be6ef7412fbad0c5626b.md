...
`section` 元素用於在文檔中定義各部分，如章節、頁眉、頁腳或文檔的任何其他部分。 它是一個對 SEO 和無障礙有幫助的語義化元素。
...
將你的 `h2` 元素、兩個 `p` 元素和錨元素（`a`）嵌套在一個 `section` 元素中。
...
整個 `section` 元素應該在 `main` 元素的開始和結束標籤之間。

```js
assert.equal(document.querySelector('section')?.parentNode.nodeName, 'MAIN');
```

現有的 `h2` 元素、`p` 元素和錨元素（`a`）應該在 `section` 元素的開始和結束標籤之間。
