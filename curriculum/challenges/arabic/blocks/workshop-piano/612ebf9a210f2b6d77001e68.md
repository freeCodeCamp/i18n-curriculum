...
أضف الآن محدِّد `.logo` إلى استعلام `@media`، واضبط خاصية `width` على `150px`.

# --hints--

يجب أن يحتوي قاعدة `@media` على محدِّد `.logo`.
...
يجب أن يكون لمحدِّد `.logo` الجديد خاصية `width` بقيمة `150px`.
