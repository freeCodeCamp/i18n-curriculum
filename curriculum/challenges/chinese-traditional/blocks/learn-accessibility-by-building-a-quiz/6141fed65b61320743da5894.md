...
為了提升頁面的存取性，可以使用 `role` 屬性來向輔助技術指出頁面上元素的用途。`role` 屬性是 *Web Accessibility Initiative*（WAI）的一部分，並接受預設值。

為你的每個 `section` 元素添加 `region` 角色。

# --hints--

你應該給第一個 `section` 元素指定 `region` 角色。

```js
assert.equal(document.querySelectorAll('section')?.[0]?.getAttribute('role'), 'region');
```

你應該為第二個 `section` 元素指定 `region` 角色。

```js
assert.equal(document.querySelectorAll('section')?.[1]?.getAttribute('role'), 'region');
```

你應該為第三個 `section` 元素指定 `region` 角色。
