...
استخدم `profile-picture` و`age` و`referrer` و`bio` كقيم لخاصيات `id` المقابلة.

# --hints--

يجب أن يحتوي أول عنصر `input` على `id` بقيمة `profile-picture`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[0]?.matches('#profile-picture'))
```

يجب أن يحتوي ثاني عنصر `input` على `id` بقيمة `age`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) input')?.[1]?.matches('#age'))
```

يجب أن يحتوي عنصر `select` على `id` بقيمة `referrer`.

```js
assert.isTrue(document.querySelector('fieldset:nth-of-type(3) select')?.matches('#referrer'))
```

يجب أن يحتوي عنصر `textarea` على `id` بقيمة `bio`.
