...
將 `.key.black--key::after` 的 `position` 屬性設置爲 `absolute`，並將 `left` 屬性設置爲 `-18px`。

# --hints--

`.key.black--key::after` 選擇器應該有一個 `position` 屬性設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.key.black--key::after')?.position === 'absolute');
```

`.key.black--key::after` 選擇器應該有一個 `left` 屬性設置爲 `-18px`。
