...
تكرار العناصر في `playlistSongElements` باستخدام طريقة `forEach`.

تُستخدم طريقة `forEach` للتكرار عبر مصفوفة وتنفيذ دالة على كل عنصر من عناصر المصفوفة. على سبيل المثال، افترض أن لديك مصفوفة من الأرقام وتريد تسجيل كل رقم في وحدة التحكم.
...
استخدم طريقة `forEach` على `playlistSongElements`. مرر `songEl` كمعلمة واستخدم صيغة السهم لإضافة استدعاء راجع فارغ.

# --hints--

يجب ربط `forEach()` بـ `playlistSongElements`.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

يجب أن تحتوي طريقة `forEach()` على معلمة `songEl` وتستخدم صيغة السهم.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

يجب أن تحتوي `forEach` على زوج فارغ من الأقواس المتعرجة.
