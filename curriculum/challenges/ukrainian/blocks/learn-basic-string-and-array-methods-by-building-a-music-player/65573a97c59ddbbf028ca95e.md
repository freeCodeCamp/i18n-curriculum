...
Нарешті, ви маєте знову відтворити пісні, оновити доступний текст кнопки відтворення та видалити кнопку скидання зі списку відтворення. Також потрібно видалити `resetButton` з DOM.

Викличте функцію `renderSongs()` з аргументом `sortSongs()`, щоб знову відтворити пісні в алфавітному порядку.

Викличте функцію `setPlayButtonAccessibleText()`, щоб оновити доступний текст кнопки відтворення.

Видаліть кнопку скидання зі списку відтворення, викликавши метод `remove()` на змінній `resetButton`.

**Примітка**: Тепер ви можете спробувати видалити всі пісні, щоб побачити, що станеться.

# --hints--

Ви не повинні змінювати існуючий слухач подій та його вміст.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Ви маєте викликати функцію `renderSongs` з `sortSongs()`.
...
Ви маєте викликати функцію `setPlayButtonAccessibleText`.
...
Ви маєте використати метод `remove()`, щоб видалити `resetButton` з DOM.
