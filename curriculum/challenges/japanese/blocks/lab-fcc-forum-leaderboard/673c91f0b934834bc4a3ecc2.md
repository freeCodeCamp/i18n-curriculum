...
`avatar_template` の値は相対パスであることに注意してください：`/` で始まり、ピクセルサイズに置き換えるべき`{size}` プレースホルダーを含みます。`avatarUrl` 定数は、パスが相対の場合に先頭に付けるベースURLを保持しています。

**目的：** 以下のユーザーストーリーを満たし、すべてのテストに合格してラボを完了してください。

**ユーザーストーリー：**

1. ISO 8601形式のタイムスタンプを引数に取る`timeAgo`という名前の関数を作成してください。
1. `timeAgo` 関数は、引数として渡された時間と現在の時間の差を計算し、以下を返す必要があります：
   - 経過分数が`xm ago`未満の場合は`x`（`60`は分を表します）
   - 経過時間が`xh ago`未満の場合は`x`（`24`は時間を表します）
   - それ以外の場合は`xd ago`（`x`は日を表します）
1. 投稿の閲覧数を引数に取る`viewCount`という名前の関数を作成してください。
1. 引数として渡された閲覧数が`1000`以上の場合、`viewCount` 関数は閲覧数を`1000`で割り、小数点以下切り捨てし、`k`の文字を付けた文字列を返す必要があります。そうでなければ閲覧数の値を返してください。
1. 選択されたカテゴリのidを引数に取る`forumCategory`という名前の関数を作成してください。
1. `forumCategory` 関数は、選択されたカテゴリidが`allCategories`オブジェクトのプロパティであることを確認し、以下を含むアンカー要素の文字列を返す必要があります：
   ...
   - `category`に続く選択されたカテゴリの`className`プロパティのクラス
   - `href`は`<className>/<id>`を`forumCategoryUrl`に付加して形成されます（注：`forumCategoryUrl`はすでに`/`で終わっているため、追加の`/`区切りは不要です）。ここで`<className>`は選択されたカテゴリの`className`プロパティ、`id`は`forumCategory`に渡された引数です。
1. `allCategories`オブジェクトに選択されたカテゴリidがプロパティとして存在しない場合、`category`は`General`として示され、`className`は`general`として示されるべきです。
1. 投稿者とユーザーを表す2つの配列を引数に取る`avatars`という名前の関数を作成してください。
1. `avatars` 関数は、`img`配列の各`user_id`に対して1つずつ、`posters`要素を結合した文字列を返す必要があります。`img` URLは`user_id`配列の`posters`プロパティを参照し、`id`配列の一致する`users`プロパティを見つけて取得します。
1. `avatars` 関数は、`avatar_template`の`users`と比較して`id`が一致する`user_id`配列内のユーザーオブジェクトの`posters`プロパティにアクセスし、URL文字列内の`{size}`プレースホルダーを`30`に置き換えて各アバターのサイズを設定します。
1. 各画像要素には投稿者の`name`プロパティの値をaltテキストとして設定してください。
1. 各`img`要素には、マッチしたユーザーの`src`を設定した`avatar_template`属性を持たせてください。`avatar_template`が`/`で始まる場合は、`avatarUrl`を直接先頭に付けてください。
1. 単一のパラメータを取る`showLatestPosts`という名前の関数を作成してください。
1. `showLatestPosts` は、引数として渡されたオブジェクトから`users`と`topic_list`プロパティを抽出します。また、`topics`に含まれる`topic_list`配列のオブジェクトの以下のプロパティを処理します：
   - `id`: 投稿のid
   - `title`: 投稿のタイトル
   - `views`: 投稿の閲覧数
...
1. `showLatestPosts` は、`#posts-container`の各アイテムに対して1つずつ`tr`要素を結合した文字列を`topics`のinner HTMLに設定します。
1. 各`tr`要素には5つの`td`要素が含まれます：
   - `td`には2つのアンカー要素が含まれます。1つは`post-title`クラス、`href`の`<forumTopicUrl><slug>/<id>`、`<title>`のアンカーテキストを持ち、もう1つは`forumCategory`を`category_id`で呼び出して得られたものです。
   - `td`には`avatar-container`クラスの`div`要素が含まれ、`avatars`関数を`posters`と`users`の引数で呼び出して返された画像が含まれます。
   - `td`には投稿への返信数が含まれます。_ヒント:_ `posts_count - 1`を使用してください。
   - `td`には投稿の閲覧数が含まれます。
   - `td`には最後のアクティビティから経過した時間が含まれます。
1. `fetchData`という名前の非同期関数を作成してください。
1. `fetchData` 関数は`forumLatest`からデータをリクエストし、レスポンスをJSONとして解析したものを`showLatestPosts`に渡して呼び出します。
1. データ取得時にエラーが発生した場合、`fetchData` 関数はエラーをコンソールに記録します。これには特に`console.log`を使用してください。

# --hints--

単一の引数を取る`timeAgo`という名前の関数を作成してください。
...
引数として渡された時間と現在の時間の差が`50`分の場合、`timeAgo`は`50m ago`を返すべきです。
...
引数として渡された時間と現在の時間の差が`60`分の場合、`timeAgo`は`1h ago`を返すべきです。
...
引数として渡された時間と現在の時間の差が`115`分の場合、`timeAgo`は`1h ago`を返すべきです。
...
引数として渡された時間と現在の時間の差が`15`時間の場合、`timeAgo`は`15h ago`を返すべきです。
...
引数として渡された時間と現在の時間の差が`24`時間の場合、`timeAgo`は`1d ago`を返すべきです。
...
引数として渡された時間と現在の時間の差が`46`時間の場合、`timeAgo`は`1d ago`を返すべきです。
...
引数として渡された時間と現在の時間の差が`3`日間の場合、`timeAgo`は`3d ago`を返すべきです。
...
単一の引数を取る`viewCount`という名前の関数を作成してください。
...
`viewCount(597)`は`597`を返すべきです。

```js
assert.strictEqual(597, viewCount(597));
```

`viewCount(1000)`は`1k`を返すべきです。

```js
assert.equal('1k', viewCount(1000));
```

`viewCount(2730)`は`2k`を返すべきです。

```js
assert.equal('2k', viewCount(2730));
```

単一の引数を取る`forumCategory`という名前の関数を作成してください。
...
`forumCategory(299)`は`Career Advice`のテキストを含むアンカー要素の文字列を返すべきです。
...
`forumCategory(299)`は`href="https://forum.freecodecamp.org/c/career/299"`を含むアンカー要素の文字列を返すべきです。
...
`forumCategory(299)`は`class="category career"`を含むアンカー要素の文字列を返すべきです。
...
`forumCategory(200)`は`General`のテキストを含むアンカー要素の文字列を返すべきです。
...
`forumCategory(200)`は`href="https://forum.freecodecamp.org/c/general/200"`を含むアンカー要素の文字列を返すべきです。
...
`forumCategory(200)`は`class="category general"`を含むアンカー要素の文字列を返すべきです。
...
2つの引数を取る`avatars`という名前の関数を作成してください。
...
`avatars` 関数は、ユーザー配列内の投稿者ごとに1つずつ`img`要素を結合した文字列を返すべきです。
...
`avatars` 関数が返す文字列内の各`img`要素は、投稿者の`alt`プロパティの値を持つ`name`テキストを持つべきです。
...
`avatars` 関数は、`avatar_template`プロパティにアクセスし、`{size}`を`30`に置き換えて各アバターのサイズを設定します。
...
各`img`要素は、マッチしたユーザーの`src`を設定した`avatar_template`属性を持つべきです。`avatar_template`が`/`で始まる場合は、`avatarUrl`を直接先頭に付けてください。
...
単一のパラメータを取る`showLatestPosts`という名前の関数を作成してください。
...
`fetchData`という名前の関数を作成してください。

```js
assert.isFunction(fetchData);
```

`fetchData` 関数は`https://cdn.freecodecamp.org/curriculum/forum-latest/latest.json`からデータをリクエストするべきです。
...
`fetchData` 関数は`showLatestPosts`を呼び出すべきです。
...
エラーが発生した場合、`fetchData` 関数は`console.log`を使用してエラーをコンソールに記録するべきです。
...
`showLatestPosts` は、`#posts-container`の各アイテムに対して1つずつ`tr`要素を結合した文字列を`topics`のinner HTMLに設定します。
...
`tr` が返す文字列の各`showLatestPosts`要素は5つの`td`要素を含みます。
...
`td` が返す文字列の各テーブル行の最初の`showLatestPosts`要素は、2つのアンカー要素を含みます。1つ目は`post-title`クラス、`href`の`<forumTopicUrl><slug>/<id>`、`<title>`のアンカーテキストを持ち、2つ目は`forumCategory`を`category_id`で呼び出して得られたものです。
...
`td` が返す文字列の各テーブル行の2番目の`showLatestPosts`要素は、`avatars`関数を`posters`と`users`の引数で呼び出して返された画像を、`avatar-container`クラスの`div`要素内にネストして含みます。
...
`td` が返す文字列の各テーブル行の3番目の`showLatestPosts`要素は投稿への返信数を含みます。_ヒント:_ `posts_count - 1`を使用してください。
...
`td` が返す文字列の各テーブル行の4番目の`showLatestPosts`要素は投稿の閲覧数を含みます。
...
`td` が返す文字列の各テーブル行の5番目の`showLatestPosts`要素は、`timeAgo`関数を使って生成された最後のアクティビティから経過した時間を含みます。
