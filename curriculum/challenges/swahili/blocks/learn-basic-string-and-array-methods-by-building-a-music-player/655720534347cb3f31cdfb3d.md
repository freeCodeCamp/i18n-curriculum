...
Kabla ya kufuta wimbo, unahitaji kuangalia kama wimbo unachezwa kwa sasa. Ikiwa unachezwa, unahitaji kusitisha wimbo na kucheza wimbo unaofuata kwenye orodha ya nyimbo.

Tumia usemi wa `if` kuangalia kama `userData?.currentSong?.id` ni sawa na `id` wa wimbo unayotaka kufuta.

# --hints--

Unapaswa kuunda usemi tupu wa `if` wenye sharti la `userData?.currentSong?.id === id`.
