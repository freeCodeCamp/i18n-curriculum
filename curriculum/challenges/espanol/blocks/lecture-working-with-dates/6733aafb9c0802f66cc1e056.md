...
En esta lección, repasaremos los conceptos básicos del objeto `Date` y algunos de los métodos más comunes que usarás al trabajar con fechas y horas en JavaScript.
...
Por ahora, está usando el formato de hora militar, así que `14:30` es `2:30 PM`. `GMT-0700` es el desplazamiento de la zona horaria, y `Pacific Daylight Time` es el nombre de la zona horaria.
...
Para obtener la fecha y hora actual, puedes usar el método `Date.now()`, que devuelve el número de milisegundos desde el `January 1, 1970, 00:00:00 UTC`. Esto se conoce como tiempo épico de Unix.
...
El mes está basado en cero, por lo tanto `January` es `0`, `February` es `1`, y así sucesivamente. Si el mes es inválido, devolverá `NaN`.
...
Si el año es inválido, devolverá `NaN`.
