...
Para redefinir a playlist ao seu estado original, espalhe `allSongs` em um array e atribua-o a `userData.songs`.

*Nota*: Você não deve usar encadeamento opcional para o `userData.songs` porque a música não será `null` ou `undefined` neste ponto.
...
Você deve atribuir `[...allSongs]` a `userData.songs`.
