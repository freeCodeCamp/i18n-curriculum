...
`Bookstores` は本を販売する店で、`theaters` はライブパフォーマンスや映画を観る場所です。

トムが読書やパフォーマンスを見る場所について尋ねるとき、単語の最後に`bookstores`を付けて`theaters`と`-s`の複数形を使っています。これが英語でほとんどの名詞を複数形にする方法です。
