...
Um sicherzustellen, dass die Anzeige des Spielers aktualisiert wird, sobald ein neues Lied beginnt zu spielen, rufen Sie die `setPlayerDisplay()` Funktion innerhalb der `playSong()` Funktion auf.

Jetzt sollten Sie den Liedtitel und den Künstler in der Anzeige sehen.

# --hints--

Sie sollten die `setPlayerDisplay` Funktion innerhalb Ihrer `playSong` Funktion aufrufen.
