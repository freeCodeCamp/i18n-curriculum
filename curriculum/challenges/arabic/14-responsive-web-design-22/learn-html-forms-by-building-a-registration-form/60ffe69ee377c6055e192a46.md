...
يحدد أعلاه عناصر `input` التي تحتوي على السمة `name` بقيمة `password`.

الآن، استخدم منتقي السمة (attribute selector) لتصميم (style) زر submit مع `display` بقيمة `block` و `width` بقيمة `60%`.

# --hints--

يجب عليك استخدام منتقي السمة `input[type="submit"]` لتصميم (style) زر submit.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="submit"]'));
```

يجب أن تعطي زر submit الـ `display` بقيمة `block`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.display, 'block');
```

يجب أن تعطي زر submit الـ `width` بقيمة `60%`.
