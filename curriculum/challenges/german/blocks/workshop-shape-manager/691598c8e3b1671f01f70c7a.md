...
Rufen Sie innerhalb der `handleShapeSelect`-Funktion die `.preventDefault()`-Methode auf `e` auf und rufen Sie die `clearInputFields()`-Funktion auf, um die Eingabefelder zu löschen.

# --hints--

Sie sollten `.preventDefault()` auf `e` aufrufen.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

Sie sollten die `clearInputFields`-Funktion aufrufen.
