...
もっと多くのソーシャルアイコンを追加しても同じ行に表示したい場合、通常は `grid-template-columns` を更新して、列を追加する必要があります。 あるいは別のやり方として、`grid-auto-flow` プロパティを使うことができます。

このプロパティは、1 つ目の値として `row` か `column` のどちらかを受け取ります。2 つ目の値はオプションで、`dense` を指定できます。 `grid-auto-flow` は、自動配置アルゴリズムを使用してグリッドレイアウトを調整します。 これを `column` に設定すると、必要に応じて新しい列を作成してコンテンツを収めるようアルゴリズムに指示します。 `dense` 値を指定すると、グリッドをさかのぼって空いている箇所を小さなアイテムで埋めることができますが、その結果、アイテムの順序が保たれなくなる可能性があります。

この例の `.social-icons` セレクターでは、`grid-auto-flow` プロパティを `column` に設定してください。

# --hints--

`.social-icons` セレクターの `grid-auto-flow` プロパティを `column` に設定する必要があります。
