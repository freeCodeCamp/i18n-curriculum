...
騎士巡迴問題：你有一個空的 `width` \* `height` 棋盤，只有一個騎士位於某個格子上。騎士必須執行一連串合法的移動，使騎士恰好造訪棋盤上的每個格子一次。請注意，巡迴不必是「封閉」的；也就是說，騎士不需要在一個移動範圍內回到起始位置。

# --instructions--

撰寫一個函式，該函式以 `width` 和 `height` 作為參數，並傳回可從中達成上述任務的初始位置數量。

# --hints--

`knightTour` 應該是一個函式（程式）。

```js
assert(typeof knightTour == 'function');
```

`knightTour(6, 6)` 應該傳回一個數字。

```js
assert(typeof knightTour(6, 6) == 'number');
```

`knightTour(6, 6)` 應該傳回 `36`。

```js
assert.equal(knightTour(6, 6), 36);
```

`knightTour(5, 6)` 應該傳回 `30`。

```js
assert.equal(knightTour(5, 6), 30);
```

`knightTour(4, 6)` 應該傳回 `12`。

```js
assert.equal(knightTour(4, 6), 12);
```

`knightTour(7, 3)` 應該傳回 `10`。

```js
assert.equal(knightTour(7, 3), 10);
```

`knightTour(8, 6)` 應該傳回 `48`。
