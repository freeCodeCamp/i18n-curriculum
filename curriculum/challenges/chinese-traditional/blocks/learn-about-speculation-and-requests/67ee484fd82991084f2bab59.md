...
A `code submission` is the set of code files that a programmer submits for review. 例如：
...
A `loop structure` executes a block of code repeatedly until a certain condition is satisfied, saving time by automating repetitive tasks, such as `for` loops or `while` loops. 例如：
