...
Використовуючи той самий синтаксис, оновіть `c`, щоб збігатися з `c`, `{`, `[` та `(`.

# --hints--

`stockRegex` повинен використати символьний клас, щоб збігатися з `c`, `{`, `[`, та `(`.
...
`stockRegex` має збігатися з `570(k 4l3r7`.

```js
assert.match('570(k 4l3r7', stockRegex);
```

`stockRegex` має збігатися з `sto{k alert`.
