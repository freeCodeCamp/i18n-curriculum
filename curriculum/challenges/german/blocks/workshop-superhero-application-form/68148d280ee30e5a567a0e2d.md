...
Entfernen Sie den `Hello`-Text innerhalb des `label`, und erstellen Sie dann ein `input` vom Typ `checkbox` sowie ein `value` vom `power`.

Zeigen Sie schließlich die Werte aus dem Array neben jedem Kontrollkästchen an, indem Sie ein `span`-Element direkt vor dem schließenden `label` mit einem Textinhalt von `{power}` erstellen.

# --hints--

Sie sollten den `Hello`-Text aus dem `label`-Element entfernen.

```js
assert.notMatch(code, /hello/i)
```

Sie sollten ein `input` erstellen, dessen `type` auf `checkbox` gesetzt ist, und ein `value`, das auf `power` gesetzt ist, als erstes Kind des Labels.

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

Sie sollten ein `span`-Element mit `{power}` als Textinhalt als letztes Kind des Labels erstellen.
