...
Na kama hoja ya tatu ya rangi, pita kitendakazi cha `hsl` na thamani za `240` kwa hue, `56%` kwa saturation, na `42%` kwa uangavu.

# --hints--

Kanuni yako ya `.blue` ya CSS inapaswa kuwa na sifa ya `background` iliyowekwa kuwa `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))`.
