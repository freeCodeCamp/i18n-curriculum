...
Cambiar la posición de un elemento a relative no lo quita del flujo normal - otros elementos al rededor de este se siguen comportando como si dicho elemento estuviese en su posición original.

**Nota:** el posicionamiento te da mucha flexibilidad y poder sobre el diseño visual de una página. Es bueno recordar que, sin importar la posición de los elementos, el código HTML subyacente debe estar organizado y tener sentido cuando se lee de arriba hacia abajo. Así es como los usuarios con discapacidades visuales (que dependen de dispositivos de asistencia como lectores de pantalla) acceden a tu contenido.

# --instructions--

Cambia la propiedad `position` de `h2` a `relative`, y usa CSS offset para moverlo 15 pixeles lejos de la parte superior `top` donde se ubica dentro del flujo normal. Observa que no hay impacto en las posiciones de los elementos h1 y p que están al rededor.
...
Tu código debe usar CSS offset para posicionar relativamente el elemento `h2` 15px lejos de la parte superior `top` donde se ubica normalmente.
