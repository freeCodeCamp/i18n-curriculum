...
Під елементом `h1` додайте елемент `p` з класом `description`.

Всередині елемента `p` напишіть текст `Click on the buttons below to rate your emotions.`

# --hints--

Ви маєте створити новий елемент `p` під елементом `h1`.

```js
assert.exists(document.querySelector("h1 + p"));
```

Новий елемент `p` повинен мати клас `description`.

```js
assert.equal(document.querySelector("h1 + p")?.className, "description");
```

Новий елемент `p` повинен містити текст `Click on the buttons below to rate your emotions.`.
