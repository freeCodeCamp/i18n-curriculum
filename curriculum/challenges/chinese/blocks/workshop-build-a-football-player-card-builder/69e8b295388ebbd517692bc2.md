...
在 JSX 和 TSX 中，你通过括弧状的花括弧传递 JavaScript 表达式作为属性值。模板字面值使用反引号和 `${}` 进行插值。例如：

```tsx
<div className={`prefix-${someVariable}`}>
```

卡片包装器 `className` 当前被硬编码为 `card-wrapper tier-gold`。通过使用调用 `gold` 并传入 `getPlayerTier` 的模板字面值，将 `player.overallRating` 替换为动态值。这样就能根据玩家的整体评分引用 CSS 中正确的等级类。

# --hints--

你应该使用带有 `getPlayerTier(player.overallRating)` 的模板字面值，作为 `className` 的 `.card-wrapper`。

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

当玩家评分为 98 时，`.card-wrapper` 元素应具有 `tier-elite` 类。
