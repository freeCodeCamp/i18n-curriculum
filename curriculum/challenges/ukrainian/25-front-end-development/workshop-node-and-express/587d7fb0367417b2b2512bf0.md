...
If you don’t know what middleware is... don’t worry, this will be discussed in detail later. Загалом, проміжне ПЗ — це функції, які перехоплюють обробники маршруту, додаючи деяку інформацію. Проміжне ПЗ підключається за допомогою методу `app.use(path, middlewareFunction)`. Перший аргумент `path` є необов’язковим. Якщо його не передати, то проміжне ПЗ буде виконуватися для всіх запитів.
