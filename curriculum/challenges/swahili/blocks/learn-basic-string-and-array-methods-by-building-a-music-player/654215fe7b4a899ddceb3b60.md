...
Kwa sasa `songsHTML` ni safu ya data. Ikiwa unajaribu kuionyesha kama ilivyo, utaona nyimbo zikitenganishwa kwa koma. Hii si matokeo unayotaka kwa sababu unataka kuonyesha nyimbo hizo kama orodha. Ili kurekebisha hili, utahitaji kuunganisha safu ya data kuwa mfuatano wa herufi mmoja kwa kutumia njia ya <dfn>join()</dfn>.

Njia ya `join()` hutumika kuunganisha vipengele vyote vya safu ya data kuwa mfuatano wa herufi mmoja. Inachukua kigezo cha hiari kinachoitwa `separator` ambacho hutumika kutenganisha kila kipengele cha safu ya data. Kwa mfano:
...
Unganisha njia ya `join()` na njia yako ya `map()` na upitie mfuatano wa herufi tupu kama kitengezaji.

Ili kuunganisha njia nyingi pamoja, unaweza kuita njia ya `join()` kwenye matokeo ya njia ya `map()`. Kwa mfano:
...
Unapaswa kuongeza `join("")` kwenye msimbo uliopo.
