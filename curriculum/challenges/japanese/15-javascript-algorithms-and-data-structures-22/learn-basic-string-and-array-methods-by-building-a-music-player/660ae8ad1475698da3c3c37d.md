...
名前付きアロー関数式を呼び出すためには、関数をその名前で参照することができます。

```js
exampleArrowFunction();
```

`printGreeting` 関数の定義の下で、関数を呼び出してください。 コンソールを開いて結果を確認してください。

# --hints--

`printGreeting` 関数を呼び出してください。

```js
assert.match(code, /printGreeting\(\);?/);
```

`printGreeting` 関数の呼び出しは、`printGreeting` 関数の定義より後に記述してください。
