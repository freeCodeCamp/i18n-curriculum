...
在 `.image-wrapper` 元素內，為你的第一個 `img` 元素設定 `src` 為 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`、`alt` 為 `image of the quote machine project`、`class` 為 `image-1`、`loading` 屬性設為 `lazy`、`width` 屬性為 600，以及 `height` 屬性為 400。

# --hints--

你的第一個 `img` 元素應該有一個 `src` 屬性設定為 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`。
