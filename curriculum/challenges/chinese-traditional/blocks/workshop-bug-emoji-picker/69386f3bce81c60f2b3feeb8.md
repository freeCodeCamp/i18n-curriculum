...
從 DOM 中取得 ID 為 `select` 的 `species` 元素，並將它存入名為 `selectElement` 的常數。使用 `!` 非空斷言運算子來斷言該元素不是 `null`

# --hints--

你應該有一個名為 `selectElement` 的常數。
...
你的 `selectElement` 應該使用帶有 `species` 型別引數的 `document.querySelector` 來查詢 ID 為 `HTMLSelectElement` 的元素。

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

你的 `selectElement` 應該使用非空斷言運算子（`!`）。
