...
フォーラムのトピックにはそれぞれ Python や JavaScript のようなカテゴリーの情報が含まれています。 ここからの数ステップで、フォーラムの全カテゴリーとスタイル設定のための `classNames` を格納する、カテゴリーオブジェクトを作成します。

まず、`allCategories` という名前の新しい const 変数 を作成し、その値に空のオブジェクトを代入してください。

# --hints--

`allCategories` という定数が必要です。

```js
assert.match(code, /const\s+allCategories\s*=/)
```

変数 `allCategories` に空のオブジェクトを割り当ててください。
