...
노래의 현재 재생 시간을 처리하는 `else` 블록을 추가하세요. 이렇게 하면 일시 중지한 지점부터 현재 노래를 다시 재생할 수 있습니다.

`else` 블록 내에서 `currentTime` 객체의 `audio` 속성을 `userData.songCurrentTime`에 저장된 값으로 설정하세요.

# --hints--

`playSong` 함수는 현재 노래가 재생 중일 때 `currentTime` 객체의 `audio` 속성을 `userData.songCurrentTime`에 저장된 값으로 설정해야 합니다.
