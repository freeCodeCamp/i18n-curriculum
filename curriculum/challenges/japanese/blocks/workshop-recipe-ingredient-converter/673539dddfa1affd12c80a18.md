...
新しいカリー化された関数をテストする必要があります。`servingsResult` 変数を宣言し、基準量4から6人分に調整した結果を代入してください。

その変数をコンソールに出力してください。

# --hints--

`servingsResult` 変数を宣言してください。

```js
assert.isDefined(servingsResult);
```

`servingsResult` 変数には `adjustQuantity(4)(6)` を呼び出した結果を代入してください。

```js
assert.strictEqual(servingsResult, 24);
```

`servingsResult` の値をコンソールに出力してください。
