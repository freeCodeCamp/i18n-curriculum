...
`#years` 要素内のテキストにスタイルを適用するために、`#years span[class]` セレクターを作成してください。`span[class]` 構文は、属性の値に関係なく、`span` 属性が設定されている任意の `class` 要素を対象にします。

新しいセレクターに `bold` フォント、幅 `4.5rem`、および右揃えのテキストを指定してください。

# --hints--

`#years span[class]` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]'));
```

`#years span[class]` セレクターには `font-weight` プロパティが `bold` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]')?.getPropertyValue('font-weight') === 'bold');
```

`#years span[class]` セレクターには `width` プロパティが `4.5rem` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('#years span[class]')?.getPropertyValue('width') === '4.5rem');
```

`#years span[class]` セレクターには `text-align` プロパティが `right` に設定されているはずです。
