...
將 `section` 元素的寬度設為其父容器的 `80%`。然後，使用邊距將 `section` 元素置中，並在底部邊距添加 `10px`。

此外，請確保 `section` 元素的寬度不超過 `600px`。
