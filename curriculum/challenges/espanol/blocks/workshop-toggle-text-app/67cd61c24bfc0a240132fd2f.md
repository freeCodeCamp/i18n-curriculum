...
Sería bueno mostrar dinámicamente las palabras `Show Message` o `Hide Message` según el estado de `isVisible`.

Anteriormente, revisaste cómo trabajar con renderizado condicional en línea. Vas a aplicar una lógica similar al texto del botón.

Aquí tienes un ejemplo de cómo usar el operador ternario para mostrar condicionalmente las palabras `Hide` y `Show`:

```jsx
<button>{isMenuVisible? "Hide" : "Show"} Menu</button>
```

Para el último paso del taller, utiliza el operador ternario para mostrar condicionalmente las palabras `Hide` o `Show` en el botón de mensaje. Coloca la condición antes del texto `Message` para que se muestre `Show Message` o `Hide Message`.

¡Y con ese último cambio, tu taller está completo! Prueba tu botón para verlo en acción.

# --hints--

Cuando `isVisible` es `false`, el texto del botón debería decir `Show Message`.
...
Cuando `isVisible` es `true`, el texto del botón debería decir `Hide Message`.
