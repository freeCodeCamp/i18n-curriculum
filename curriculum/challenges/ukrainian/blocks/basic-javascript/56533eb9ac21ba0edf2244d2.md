...
The inequality operator (`!=`) is the opposite of the equality operator. Нерівність означає не рівне. Оператор нерівності повертає `false`, коли оператор рівності поверне `true` та *навпаки*. Like the equality operator, the inequality operator will convert data types of values while comparing.
