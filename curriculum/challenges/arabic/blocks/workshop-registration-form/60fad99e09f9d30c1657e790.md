...
وسّط عنصر الـ `form` بإعطائه هامشًا (`margin`) بقيمة `0 auto`. ثم، ثبّت حجمه ليكون بعرض أقصى `500px`، وعرض أدنى `300px`. ضمن هذا النطاق، اجعله يمتلك عرضًا (`width`) بقيمة `60vw`.

# --hints--

يجب استخدام محدِّد `form` لتنسيق عنصر الـ `form`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('form'));
```

يجب إعطاء الـ `form` هامشًا (`margin`) بقيمة `0 auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.margin, '0px auto');
```

يجب إعطاء الـ `form` عرضًا أقصى (`max-width`) بقيمة `500px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.maxWidth, '500px');
```

يجب إعطاء الـ `form` عرضًا أدنى (`min-width`) بقيمة `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('form')?.minWidth, '300px');
```

يجب إعطاء الـ `form` عرضًا (`width`) بقيمة `60vw`.
