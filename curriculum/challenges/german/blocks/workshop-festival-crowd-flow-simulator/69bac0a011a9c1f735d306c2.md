...
Verarbeiten Sie das Eingangs-Gate für den aktuellen Tick, indem Sie `processGateFlow()` mit `gate` und `tickIndex` aufrufen und das Ergebnis einer Variablen namens `result` zuweisen.

# --hints--

Ihre `handleGateAtTick`-Funktion sollte die `processGateFlow()`-Methode aufrufen.
...
Sie sollten `processGateFlow()` mit `gate` und `tickIndex` aufrufen und das Ergebnis einer Variablen namens `result` zuweisen.
