...
Тепер, коли на екрані відображається список пісень, було б добре відсортувати їх в алфавітному порядку за назвою.

Почніть зі створення стрілкової функції під назвою `sortSongs`.

# --hints--

Ви повинні мати функцію під назвою `sortSongs`.

```js
assert.isFunction(sortSongs);
```

Функція `sortSongs` має використовувати синтаксис стрілкових функцій.
