...
Далі вам потрібно відобразити назву поточної пісні та виконавця на дисплеї плеєра. Використайте `const` та стрілковий синтаксис, щоб створити порожню функцію `setPlayerDisplay`.

# --hints--

Використайте `const`, щоб створити порожню функцію під назвою `setPlayerDisplay`.
...
Функція `setPlayerDisplay` повинна використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

Функція `setPlayerDisplay` має бути порожньою.
