...
Тепер створіть функцію з назвою `getNextSong`. Вона має повертати пісню, яка знаходиться на `getCurrentSongIndex()` плюс один у `userData.songs`, або `undefined`, якщо поточна пісня — остання у плейлисті.

# --hints--

У вас має бути функція з назвою `getNextSong`.

```js
assert.isFunction(getNextSong);
```

Ваша функція `getNextSong` має викликати `getCurrentSongIndex`.
...
Ваша функція `getNextSong` має повертати пісню, що йде після поточної.
...
Ваша функція `getNextSong` має повертати `undefined`, коли поточна пісня — остання у плейлисті.
