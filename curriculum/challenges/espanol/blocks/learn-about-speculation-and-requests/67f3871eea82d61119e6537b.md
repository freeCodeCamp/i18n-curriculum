...
`Weak passwords` are passwords that are easy to guess or crack. Por ejemplo:

`123456 and qwerty are weak passwords.` - A strong password should be long and contain letters, numbers, and symbols.

`Shared credentials` are login details (username and password) that are used by multiple people. Por ejemplo:
