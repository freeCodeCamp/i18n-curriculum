...
Ahora, es momento de agregar la funcionalidad de reinicio al `resetButton`. Esto traerá de vuelta las canciones en la lista de reproducción cuando se haga clic.

Agrega un escucha de evento de clic a la variable `resetButton`. Pasa un callback usando la sintaxis de flecha y déjalo vacío por ahora.

# --hints--

Debes usar el método `addEventListener()` en `resetButton`.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

El escucha de eventos de tu `resetButton` debe escuchar un evento de `"click"`.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

La función de callback de tu escucha de eventos debe usar sintaxis de flecha y tener un par de llaves vacías.
