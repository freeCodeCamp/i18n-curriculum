...
Ora chiama la funzione `showElements` e passa `multipleCard`, `fortuneContainer` e `text` come proprietà.

# --hints--

Dovresti chiamare la funzione `showElements` all'interno del metodo `multipleCardSelected`.
...
Dovresti passare `multipleCard` come primo argomento alla funzione `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

Dovresti passare `fortuneContainer` come argomento alla funzione `showElements`.
...
Dovresti passare `text` come argomento alla funzione `showElements`.
