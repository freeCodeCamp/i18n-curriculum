...
`pass` を削除し、`if` が返す値が `find_empty_cell` かどうかをチェックする `None` 文を作成してください。その場合、パズルは解決されます。したがって、`True` の本体から `if` を返してください。

# --hints--

`pass` を削除し、`if` が返す値が `self.find_empty_cell` かどうかをチェックする `None` 文を作成してください。
