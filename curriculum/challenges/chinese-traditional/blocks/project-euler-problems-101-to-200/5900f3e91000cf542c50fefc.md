...
回文數字 595 很有趣，因為它可以寫成連續平方數的和：$6^2 + 7^2 + 8^2 + 9^2 + 10^2 + 11^2 + 12^2$。

在一千以下，有且只有十一個回文數可以寫成連續平方和，而這些回文數的總和是 4164。請注意，$1 = 0^2 + 1^2$ 未被包含，因為此問題關注的是正整數的平方。

找出所有小於 `limit` 且同時為回文數且可寫成連續平方和的數字總和。

# --hints--
`palindromicSums(100000000)` 應該傳回 `2906969179`。
...
`palindromicSums(100)` 應該傳回 `137`。

```js
assert.strictEqual(palindromicSums(100), 137);
```

`palindromicSums(1000)` 應該傳回 `4164`。
