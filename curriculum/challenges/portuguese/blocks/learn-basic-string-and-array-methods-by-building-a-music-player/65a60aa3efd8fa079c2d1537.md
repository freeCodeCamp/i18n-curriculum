...
A segunda condição neste exemplo verifica se `a.name > b.name`. Se for o caso, a função retorna `1`, que ordena a primeira fruta depois da segunda fruta.
...
Dentro da sua função de callback, adicione outra declaração `if` para verificar se `a.title` é maior que `b.title`. Se for o caso, retorne o número `1`. 

# --hints--

Você deve ter uma segunda declaração `if` dentro da sua função callback.
...
Sua declaração `if` deve verificar se `a.title` é maior que `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
``` 

Sua declaração `if` deve retornar o número `1` se `a.title` for maior que `b.title`.
