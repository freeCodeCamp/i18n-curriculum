...
在类中添加一个 `constructor`。在构造函数中，你需要使用之前创建的元素，通过 `getElement` 函数选择对应的 DOM 元素。

目前，处理 `singleCardBtn` 和 `singleCard`。对于 `singleCardBtn`，传入 `#btn-single-card`，对于 `singleCard`，传入 `.single_card`。

# --hints--

`Game` 类应该有一个 `constructor`。
...
`singleCardBtn` 应该使用 `getElement` 和 `'#btn-single-card'` 赋值。
...
`singleCard` 应该使用 `getElement` 和 `'.single_card'` 赋值。
