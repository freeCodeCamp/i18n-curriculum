...
在 `.penguin` 元素上面，添加一個 `div`，其 `class` 爲 `left-mountain`。

# --hints--

你應該在 `body` 中添加一個新的 `div`。 應該是 `--fcc-expected--` `div` 元素，而不是 `--fcc-actual--`。

```js
assert.equal(document.querySelectorAll('body > div')?.length, 3);
```

你應該將 `div` 的 `class` 設置爲 `left-mountain`。

```js
assert.include(document.querySelector('body > div:not(.ground, .penguin)')?.className, 'left-mountain');
```

你應該將 `.left-mountain` 放在 `.penguin` 前面。
