...
アクセシビリティツリーは、スクリーンリーダーなどの支援技術がウェブページ上のコンテンツを解釈し、操作するために使う構造です。
...
- **`display: none;`**: `display: none;` を使うと、スクリーンリーダーや他の支援技術がこのコンテンツにアクセスできなくなります。これはアクセシビリティツリーに含まれないためです。したがって、視覚的な表示とアクセシビリティの両方から完全にコンテンツを除去したい場合にのみ、この方法を使うことが重要です。
- **`visibility: hidden;`**: このプロパティと値はコンテンツを視覚的に隠しますが、文書の流れには残るため、ページ上のスペースは占有し続けます。これらの要素はアクセシビリティツリーからも除外されるため、スクリーンリーダーによる読み上げもされなくなります。
- **`.sr-only` CSSクラス**: これはコンテンツを視覚的に隠しつつ、スクリーンリーダーにはアクセス可能なままにする一般的な手法です。
...
上記の例では、ユーザーがデバイスで動きの削減を設定していない場合にスムーズスクロールが有効になります。
...
プレースホルダーテキストの使用はアクセシビリティにあまり適していません。多くの場合、ユーザーはプレースホルダーテキストを実際の入力値と混同し、すでに入力に値があると思い込んでしまいます。
