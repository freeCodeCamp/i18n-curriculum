...
Agora que o método `__init__` está completo, crie uma variável chamada `series1` dentro do bloco `try`. Atribua a ela uma instância de `TVSeries` passando argumentos de sua escolha. Em seguida, imprima `series1` no console.

# --hints--

Você deve atribuir uma instância de `TVSeries` a uma variável chamada `series1` dentro do bloco `try`.
...
Você deve imprimir `series1` dentro do bloco `try`.
