...
이제 JavaScript 파일에서 `happyBtn` 변수를 만드세요.

`querySelector`를 사용해 `#happy-btn` 변수에 있는 `happyBtn` 요소를 참조하도록 저장하세요.

# --hints--

`happyBtn`라는 변수가 있어야 합니다.

```js
assert.exists(happyBtn);
```

`happyBtn` 변수는 `#happy-btn` 요소를 참조해야 합니다.
