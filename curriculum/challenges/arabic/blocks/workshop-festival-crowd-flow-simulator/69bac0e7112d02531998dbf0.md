...
المهمة النهائية في دالتك `handleGateAtTick` هي التعامل مع أي تجاوز سعة للـ tick الحالي.

إذا وُجد أي تجاوز سعة (`result.overflow > 0`)، يجب عليك:

- سجّل الرسالة التالية `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
- أعد توجيه التجاوز باستدعاء دالتك `rerouteOverflow()` مع `gates`، `gate`، `tickIndex`، و `result.overflow`.

# --hints--

إذا وُجد أي تجاوز سعة (`result.overflow > 0`)، يجب أولًا تسجيل الرسالة `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
...
إذا وُجد أي تجاوز سعة (`result.overflow > 0`)، يجب إعادة توجيه التجاوز باستدعاء `rerouteOverflow()` مع `gates`، `gate`، `tickIndex`، و `result.overflow`.
