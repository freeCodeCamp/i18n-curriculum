...
使用 `const` 關鍵字和箭頭函式語法定義一個 `pauseSong` 函式（程式）。該函式不應帶有任何參數。

# --hints--

你應該使用 `const` 來創建一個名為 `pauseSong` 的空函式（程式）。

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` 應該是一個函式（程式）。

```js
assert.isFunction(pauseSong)
```

你的 `pauseSong` 函式（程式）應該使用箭頭語法。

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `pauseSong` 函式（程式）應該是空的。
