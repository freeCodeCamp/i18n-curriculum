...
En este paso, adjunta un listener de evento `submit` a `recipeForm` para que cuando se haga clic en el botón `Convert`, se llame a la función `updateResultsList()`.

Adjunta un listener de evento `submit` a `recipeForm`. Cuando se envíe el formulario, llama a `event.preventDefault()` para detener el comportamiento predeterminado, y luego llama a `updateResultsList()`.

Esto te permitirá activar la función a través de la interfaz de usuario llenando el formulario y presionando `Convert`, útil para depurar mientras agregas la lógica de conversión en el siguiente paso.

# --hints--

Haz que al hacer clic en el botón `Convert` se ejecute `updateResultsList()` agregando un listener de evento `"submit"` a `recipeForm` que llame a `event.preventDefault()` para detener el comportamiento predeterminado del formulario.
