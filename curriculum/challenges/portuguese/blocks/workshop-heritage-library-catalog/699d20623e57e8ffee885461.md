...
O mesmo padrão se aplica aos outros três campos. Adicione verificações equivalentes para `author`, `year` e `location`.

Note que `isValid` acumula ao longo das quatro verificações: se algum campo falhar, ele se torna `false` e permanece `false` mesmo que as verificações restantes passem. Isso significa que a função sempre inspeciona todos os campos em vez de parar no primeiro problema.

# --hints--

`validateEntry(catalog[0])` deve retornar `true`.

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` deve retornar `false`.

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` deve retornar `false`.

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` deve retornar `false` porque a localização é `"Unknown"`.
