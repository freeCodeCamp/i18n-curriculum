...
正如你在前一步中所做的那样，使用后代选择器来选中你的 `.cat-whiskers-right` 元素中的三个 `div` 元素。 为它添加 `40px` 的 `width`、`1px` 的 `height` 以及 `#000` 的 `background-color`。
...
你的 `.cat-whiskers-right div` 选择器应该将 `background-color` 设置为 `#000`。
