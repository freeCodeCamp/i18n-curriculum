...
Implementieren Sie die `is_applicable`-Methode für `PremiumUserDiscount`. Diese Methode soll prüfen, ob der Benutzer ein Premium-Mitglied ist.

Geben Sie `True` zurück, wenn `user_tier.lower()` gleich `'premium'` ist, andernfalls geben Sie `False` zurück. Die Verwendung von `.lower()` stellt sicher, dass die Prüfung nicht zwischen Groß- und Kleinschreibung unterscheidet.

# --hints--

Ihre `PremiumUserDiscount`-Klasse sollte eine `is_applicable`-Methode haben.
...
Ihre `is_applicable`-Methode sollte `True` zurückgeben, wenn `user_tier.lower()` gleich `'premium'` ist.
...
Ihre `is_applicable`-Methode sollte `False` zurückgeben, wenn `user_tier` nicht `'premium'` ist.
