...
Per chiamare una funzione freccia con nome, puoi fare riferimento alla funzione tramite il suo nome.

```js
exampleArrowFunction();
```

Sotto la definizione della tua funzione `printGreeting`, chiama la funzione. Apri la console per vedere l'output.

# --hints--

Dovresti chiamare la tua funzione `printGreeting`.

```js
assert.match(code, /printGreeting\(\);?/);
```

La chiamata alla tua funzione `printGreeting` dovrebbe essere sotto la definizione della funzione `printGreeting`.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

La chiamata alla tua funzione `printGreeting` dovrebbe mostrare `Hello there!` nella console.
