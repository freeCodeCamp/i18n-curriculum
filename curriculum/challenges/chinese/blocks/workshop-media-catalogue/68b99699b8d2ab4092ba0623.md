...
在你的 `__str__` 方法中的 `if` 语句之后，创建两个名为 `movies` 和 `series` 的变量，并分别赋值为调用 `get_movies` 方法和调用 `get_tv_series` 方法。

# --hints--

你应该在你的 `__str__` 方法中有一个名为 `movies` 的变量。
...
你将 `self.get_movies()` 赋值给你的 `movies` 变量。
...
你应该在你的 `__str__` 方法中有一个名为 `series` 的变量。
...
你将 `self.get_tv_series()` 赋值给你的 `series` 变量。
