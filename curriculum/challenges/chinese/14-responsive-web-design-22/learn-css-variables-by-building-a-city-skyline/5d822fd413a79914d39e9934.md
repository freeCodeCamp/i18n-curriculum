...
你应该在 `background` 属性中将 `repeating-linear-gradient` 添加到 `.fb6` 中。
...
你应该为 `repeating-linear-gradient` 设置从 `0%` 到 `10%` 是第一种颜色 `--building-color3`。
...
你应该为 `repeating-linear-gradient` 设置从 `10%` 到 `30%` 是第二种颜色 `transparent`。
