...
Dentro del `map()`, agregue una declaración `return` con comillas invertidas donde interpolará todos los elementos responsables de mostrar los detalles de la canción.

Dentro de las comillas invertidas, cree un elemento `li` con un atributo `id` de `song-${song.id}` y un atributo `class` de `playlist-song`.

# --hints--

Debes agregar una declaración `return` con un par vacío de comillas invertidas.

```js
assert.match(code, /return\s*`/)
```

Debes crear una etiqueta `li` dentro de las comillas invertidas.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

Tu etiqueta `li` de apertura debe tener un atributo `id` establecido como `song-${song.id}`.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

Tu etiqueta `li` de apertura debe tener un atributo `class` establecido como `playlist-song`.
