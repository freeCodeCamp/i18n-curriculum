...
要显示总支出，请创建一个 `elif` 语句来检查是否为 `choice == '3'`。

如果为真，则表示用户想要查看总支出。 因此调用 `print()` 并将字符串 `'\nTotal Expenses: '` 作为第一个参数传递，将 `total_expenses(expenses)` 作为第二个参数传递。

# --hints--

你应该创建一个 `elif` 子句来检查是否 `choice == '3'`。
