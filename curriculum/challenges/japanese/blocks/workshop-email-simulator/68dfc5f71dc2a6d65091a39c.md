...
メールのインデックスが有効な場合、`actual_index` のメールにアクセスし、その `display_full_email` メソッドを呼び出してメールの内容を表示してください。

# --hints--

`actual_index` リストの `self.emails` にあるメールにアクセスし、その `display_full_email` メソッドを呼び出してください。
