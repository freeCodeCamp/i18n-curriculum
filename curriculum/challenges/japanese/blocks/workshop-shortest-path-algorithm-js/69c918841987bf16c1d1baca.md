...
隣接ノードへのより短い経路を見つけた場合、その経路を記録したパスも更新する必要があります。

JavaScriptでは、スプレッド演算子を使って既存の配列と新しい要素を組み合わせた新しい配列を作成できます。

```js
const newPath = [...existingArray, newElement];
```

ネストされた`if`ブロック内で、`paths[nodeNo]`を現在のノードへのパスに`nodeNo`を末尾に追加したものに、次のようなスプレッド構文で更新してください：`[...paths[current], nodeNo]`。

# --hints--

ネストされた`paths[nodeNo]`ブロック内の`if`を更新する必要があります。
...
`paths[nodeNo]`は`[...paths[current], nodeNo]`に設定してください。
