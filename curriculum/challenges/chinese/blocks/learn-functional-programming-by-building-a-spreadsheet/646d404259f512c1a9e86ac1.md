...
# --description--
在 `highPrecedence` 函数中，使用 `const` 声明一个变量并为其分配一个正则表达式，该正则表达式检查传递给 `str` 参数的字符串是否与数字后跟 `*` 或 `/` 运算符后跟另一个数字的模式匹配。

你的函数应该返回一个布尔值。 请记住，你可以为此使用 `test()` 方法。

# --hints--

你应该在 `highPrecedence` 函数中为正则表达式声明一个变量。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*(?:const|let|var)\s+\w+/);
```

你应该使用 `const` 来声明你的正则表达式变量。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+\w+/);
```

你的正则表达式变量应该包含一个正则表达式。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+\w+\s*=\s*\//);
```

你的 `highPrecedence` 函数应该返回一个布尔值。

```js
assert.isBoolean(highPrecedence("12*2"));
```

你的 `highPrecedence` 函数应该正确检查字符串是否与数字后跟 `*` 或 `/` 运算符后跟另一个数字的模式匹配。
