...
`textarea`가 너무 작게 보입니다. 초기 크기를 지정하려면 `rows`와 `cols` 속성을 추가하세요.

초기 크기를 `3` 행과 `30` 열로 지정하세요.

# --hints--

`textarea`에 값이 `rows`인 `3` 속성을 지정해야 합니다.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

`textarea`에 값이 `cols`인 `30` 속성을 지정해야 합니다.
