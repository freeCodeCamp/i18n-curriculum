...
Panga ndani kipengele cha `select` (pamoja na vipengele vyake vya `option`) ndani ya kipengele cha `label` chenye maandishi ya `How did you hear about us?`. Maandishi yanapaswa kuja kabla ya kipengele cha `select`.

# --hints--

Unapaswa kupanga ndani kipengele cha `select` tu ndani ya kipengele cha `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(3) > select'));
```

Unapaswa kumpa kipengele cha `label` maandishi ya `How did you hear about us?`.
