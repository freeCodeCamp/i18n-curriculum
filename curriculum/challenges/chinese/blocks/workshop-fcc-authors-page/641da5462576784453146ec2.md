...
现在你有了所需的数据，可以使用它来填充 UI。 

但是你获取的 `data` 总共有 26 个作者。你应该一次显示 8 个作者，而不是一次显示全部 26 个作者。

首先使用 `let` 关键字创建两个名为 `startingIndex` 和 `endingIndex` 的变量，并分别赋值为数字 `0` 和 `8`。你需要在这里使用 `let`，因为你稍后会重新赋值这些值。

然后，创建一个 `authorDataArr` 变量并赋值为空数组。 
...
你的 `startingIndex` 变量应该是一个数字。

```js
assert.isNumber(startingIndex);
```

你的 `startingIndex` 变量应设置为 `0`。
...
你应该有一个名为 `authorDataArr` 的变量。

```js
assert.isDefined(authorDataArr);
```

你的 `authorDataArr` 变量应是一个空数组。
