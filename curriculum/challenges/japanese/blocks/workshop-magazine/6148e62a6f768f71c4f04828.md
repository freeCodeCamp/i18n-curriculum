...
`place-items` プロパティは、`align-items` と `justify-items` の値を同時に設定するために使えます。`place-items` プロパティは1つまたは2つの値を取ります。1つの値が指定された場合、それは `align-items` と `justify-items` の両方のプロパティに使われます。2つの値が指定された場合、最初の値は `align-items` プロパティに、2つ目の値は `justify-items` プロパティに使われます。

`.image-wrapper` セレクターに `place-items` プロパティを `center` に設定してください。
