...
Die `.remove()`-Methode entfernt aus einer Liste das erste passende Element, das als Argument übergeben wird:
...
Beenden Sie die `while`-Schleife, indem Sie den aktuellen Knoten aus der `unvisited`-Liste entfernen. Achten Sie auf die Einrückung.

# --hints--

Sie sollten die `.remove()`-Methode verwenden, um den aktuellen Knoten aus `unvisited` nach Ihrer `for`-Schleife zu entfernen.
