...
今度は `product` をカートの `items` 配列に追加する必要があります。`this` キーワードを使うのを忘れないでください。

# --hints--

`push` 配列の `items` メソッドを呼び出してください。
...
`this` 配列にアクセスするには `items` キーワードを使う必要があることを覚えておいてください。
...
`push` 配列に `product` 変数を `items` する必要があります。
