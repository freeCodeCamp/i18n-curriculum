...
ضع العنصر `.penguin-body` بمسافة `40%` من الأعلى، و`23.5%` من اليسار بالنسبة لعنصره الأصل.

# --hints--

يجب أن تعطي `.penguin-body` خاصية `top`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top);
```

يجب أن تعطي `.penguin-body` خاصية `top` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.top, '40%');
```

يجب أن تعطي `.penguin-body` خاصية `left`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.left);
```

يجب أن تعطي `.penguin-body` خاصية `left` بقيمة `--fcc-expected--`، لكن وُجدت القيمة `--fcc-actual--`.
