...
Per resettare la playlist al suo stato originale, espandi `allSongs` in un array e assegnalo a `userData.songs`.

*Nota*: non dovresti usare il concatenamento opzionale per `userData.songs` perché a questo punto la canzone non sarà `null` né `undefined`.
...
Dovresti assegnare `[...allSongs]` a `userData.songs`.
