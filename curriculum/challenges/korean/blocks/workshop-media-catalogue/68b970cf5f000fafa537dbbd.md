...
예외 객체에 예외를 발생시킨 객체를 저장하고 있으므로, 이를 어느 정도 활용할 수 있습니다.

기존 `print` 호출 뒤에 `print(f'Unable to add {e.obj}: {type(e.obj)}')`을 추가하세요.

이런 종류의 메시지는 코드를 디버그할 때 매우 유용할 수 있습니다.

# --hints--

기존 `print(f'Unable to add {e.obj}: {type(e.obj)}')` 호출 뒤에 `print(f'Media Error: {e}')`가 있어야 합니다.
