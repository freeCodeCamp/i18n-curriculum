...
Seleziona l'elemento `.penguin-body` e assegna un `width` di `53%` e un `height` di `45%`. Poi, imposta il `background` su un gradiente lineare a `45deg`, `rgb(134, 133, 133)` da `0%`, `rgb(234, 231, 231)` da `25%` e `white` da `67%`.
...
Dovresti assegnare a `.penguin-body` un `width` di `--fcc-expected--`, ma è stato trovato `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

Dovresti assegnare a `.penguin-body` un `height` di `--fcc-expected--`, ma è stato trovato `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

Dovresti assegnare a `.penguin-body` un `background` di `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)`.
