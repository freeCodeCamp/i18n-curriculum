...
Створіть ще один елемент `button` з класом `playlist-song-delete` та атрибутом `aria-label` зі значенням `Delete` інтерпольованим з `song.title`. Щоб отримати вміст іконки видалення, вставте такий SVG:
...
Створіть елемент `button` з класом `playlist-song-delete` як його перший атрибут та значення.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

Ваш елемент кнопка повинен мати атрибут `aria-label` зі значенням `Delete ${song.title}` як другий атрибут та значення.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

Вставте наданий `svg` як вміст елемента `button`.
