...
在 `Desserts` 標題下方為最後一張影像添加使用 URL `https://cdn.freecodecamp.org/curriculum/css-cafe/pie.jpg`。將該影像的 `alt` 值設為 `pie icon`。
...
你的新 `img` 元素應該有一個 `src`，其值為 `https://cdn.freecodecamp.org/curriculum/css-cafe/pie.jpg`。
...
你的新 `img` 元素應該有一個 `alt` 為 `pie icon`。
