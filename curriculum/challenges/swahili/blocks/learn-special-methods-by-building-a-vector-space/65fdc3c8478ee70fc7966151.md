...
Kwa njia ile ile uliyofanya awali kwa njia ya `__eq__`, tengeneza usemi wa `if` unaokagua kama `self` na `other` haviko katika darasa la vitu moja na urudishe `NotImplemented`.

# --hints--

Unapaswa kutengeneza usemi wa `if` unaokagua kama `self` na `other` haviko katika darasa la vitu moja.
...
Unapaswa kurudisha `NotImplemented` kutoka mwilini mwa `if`.
