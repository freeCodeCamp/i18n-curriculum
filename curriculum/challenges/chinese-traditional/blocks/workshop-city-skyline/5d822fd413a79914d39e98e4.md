...
背景建築開始看起來相當不錯。在 `.background-buildings` 元素下方創建一個新的 `div`，並為它添加一個 `foreground-buildings` 類別。這將是另一個用於更多建築的容器。
...
新的 `div` 應該位於 `div.background-buildings` 元素之後。

```js
assert.exists(document.querySelector('div.background-buildings + div'));
```

你的新 `div` 應該有一個 `foreground-buildings` 的類別。
