...
`showResults` 関数を完成させてください。プレイヤーとコンピューターの更新されたスコアを表示するために、`playerScoreSpanElement` と `computerScoreSpanElement` を更新する必要があります。

また、ラウンドの結果を表示するために `roundResultsMsg` も更新してください。

**ヒント**

- 要素のテキスト内容を更新するために `innerText` プロパティの使い方を学んだことを思い出してください。
- ラウンドの結果を取得するには `getRoundResults` 関数を使うことができます。

# --hints--

あなたの `showResults` 関数はラウンドの結果を `roundResultsMsg` に更新する必要があります。
...
あなたの `showResults` 関数はコンピューターの更新されたスコアを `computerScoreSpanElement` に表示するように更新してください。
...
あなたの `showResults` 関数はプレイヤーの更新されたスコアを `playerScoreSpanElement` に表示するように更新してください。
