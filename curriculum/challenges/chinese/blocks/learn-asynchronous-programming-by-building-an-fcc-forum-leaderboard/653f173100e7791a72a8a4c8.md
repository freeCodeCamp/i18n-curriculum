...
在上一个项目中，你使用了 `.catch()` 方法来处理错误。 在这里，你将使用 `try...catch` 语句。

`try` 代码块旨在处理潜在的错误，如果发生错误，`catch` 代码块内的代码将被执行。
