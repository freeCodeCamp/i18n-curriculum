...
Unahitaji kupata `id` ya wimbo unaochezwa kwa sasa. Kwa hili, unaweza kutumia `userData?.currentSong?.id`.

Tumia `getElementById()` kupata `id` ya wimbo unaochezwa kwa sasa, kisha tumia herufi za kiolezo kuiongeza kiambishi awali `song-`. Weka thamani yake katika thabiti `songToHighlight`.

# --hints--

Haupaswi kubadilisha kitendakazi cha `highlightCurrentSong` kilichopo na yaliyomo ndani yake.
...
Unapaswa kutumia `document.getElementById()` na kupitisha `` `song-${userData?.currentSong?.id}` ``.

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Unapaswa kuweka thamani ya `getElementById()` katika thabiti `songToHighlight`.
