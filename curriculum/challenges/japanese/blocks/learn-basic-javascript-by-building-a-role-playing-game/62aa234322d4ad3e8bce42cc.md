...
プレイヤーの唯一の武器が壊れてしまうのは避けたいです。論理積（AND）演算子は、2つの条件が両方とも真かどうかをチェックします。

<dfn>論理積（AND）</dfn>演算子 `&&` を使って、`if` 文に2つ目の条件を追加してください。プレイヤーの武器は、`inventory.length` が (`!==`) 1と等しくない場合にのみ壊れるべきです。

2つの条件を持つ `if` 文の例を示します。
...
論理積（AND）演算子を使って、`inventory.length` が `1` と等しくないかどうかをチェックしてください。
