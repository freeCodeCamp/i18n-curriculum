...
在 `animate` 函式（程式）中，你需要將每個平台繪製到畫布上。

為你的程式碼添加一個 `forEach` 迴圈，迭代 `platforms` 陣列。 

在回呼函式（程式）內，為你的 `platform` 添加一個參數，並在函式（程式）主體中對每個 `platform` 呼叫 `draw` 方法。

# --hints--

你應該有一個 `forEach` 迴圈來迭代 `platforms` 陣列。

```js
assert.match(code, /platforms\.forEach\(\s*(\(.*\)|[^\s()]+)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

你應該為回呼函式（程式）添加一個 `platform` 參數。

```js
assert.match(code, /platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

你應該在每個 `platform` 上呼叫 `draw` 方法。
