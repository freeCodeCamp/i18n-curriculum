...
التنسيق الافتراضي لعنصر `address` هو تعيين خاصية `font-style` إلى `italic`. أضف محدِّدًا لعنصر `address` واضبط خاصية `font-style` على `normal`.

# --hints--

يجب أن يكون لديك محدِّد لعنصر `address`.

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('address'));
```

يجب أن يضبط محدِّد `address` خاصية `font-style`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('address').getPropVal('font-style'));
```

يجب تعيين خاصية `font-style` إلى `normal`.
