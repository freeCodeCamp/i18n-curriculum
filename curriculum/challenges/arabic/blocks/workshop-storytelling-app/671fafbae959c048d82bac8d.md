...
عدّل مستمع الحدث الخاص بـ `scaryStoryBtn` لاستدعاء الدالة `displayStory` مع المعلمة `"scary"`. أضف أيضًا مستمع حدث `click` إلى زري `funnyStoryBtn` و `adventureStoryBtn` يستدعيان الدالة `displayStory` مع المعلمات `"funny"` و `"adventure"` على التوالي.

بهذا تكون قد أكملت ورشة العمل بنجاح.

# --hints--

عند نقر `scaryStoryBtn`، يجب أن يعرض فقرة `#result` القصة المخيفة `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` مع لون حدود `#ee4b2b`.
...
عند نقر `funnyStoryBtn`، يجب أن تعرض فقرة `#result` القصة المضحكة مع لون حدود `#f1be32`.
...
عند نقر `adventureStoryBtn`، يجب أن تعرض فقرة `#result` قصة المغامرة مع لون حدود `#acd157`.
