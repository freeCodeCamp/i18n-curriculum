...
`::first-letter` 疑似セレクターを使うと、要素のテキストコンテンツの最初の文字を指定できます。

`.first-paragraph::first-letter` セレクターを作成し、`font-size` プロパティを `6rem` に設定してください。また、目立たせるために `color` プロパティも `orangered` に設定してください。

# --hints--

`.first-paragraph::first-letter` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

`.first-paragraph::first-letter` セレクターには `font-size` プロパティが `6rem` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

`.first-paragraph::first-letter` セレクターには `color` プロパティが `orangered` に設定されているはずです。
