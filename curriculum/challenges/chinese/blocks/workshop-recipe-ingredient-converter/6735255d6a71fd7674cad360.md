...
最后，为你的 `conversionTable` 添加一个 `teaspoon` 对象，该对象的 `cup` 属性设置为 `1/48`，`gram` 属性设置为 `5`，`ounce` 属性设置为 `0.167`。

# --hints--

你的 `conversionTable` 应该有一个 `teaspoon` 属性。

```js
assert.property(conversionTable, "teaspoon");
```

你的 `teaspoon` 属性应该是一个对象。

```js
assert.isObject(conversionTable.teaspoon);
```

你的 `teaspoon` 对象应有一个 `cup` 属性。
...
你的 `teaspoon` 对象应具有一个 `ounce` 属性。

```js
assert.property(conversionTable.teaspoon, "ounce");
```

你的 `ounce` 属性应设置为 `0.167`。
