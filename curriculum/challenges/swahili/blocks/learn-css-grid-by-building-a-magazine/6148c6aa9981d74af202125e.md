...
Toa kichaguzi cha `.text` sifa ya `font-size` iliyowekwa kuwa `1.8rem` na sifa ya `letter-spacing` iliyowekwa kuwa `0.6px`.

# --hints--

Kichaguzi chako cha `.text` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize === '1.8rem');
```

Kichaguzi chako cha `.text` kinapaswa kuwa na sifa ya `letter-spacing` iliyowekwa kuwa `0.6px`.
