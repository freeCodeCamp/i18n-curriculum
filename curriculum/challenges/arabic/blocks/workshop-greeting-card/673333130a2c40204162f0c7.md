...
عنصر `.message` يحتاج إلى بعض التنسيق. امنحه:

- خاصية `font-size` بقيمة `1.2em`،
- خاصية `margin-bottom` بقيمة `20px`.

# --hints--

يجب أن يكون لديك محدِّد `.message`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".message"));
```

يجب أن يحتوي محدِّد `.message` على خاصية `font-size` مضبوطة على `1.2em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("font-size"), "1.2em");
```

يجب أن يحتوي محدِّد `.message` على خاصية `margin-bottom` مضبوطة على `20px`.
