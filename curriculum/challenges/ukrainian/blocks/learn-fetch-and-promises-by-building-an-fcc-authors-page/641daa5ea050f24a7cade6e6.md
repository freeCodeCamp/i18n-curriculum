...
Якщо ця умова виконується, вимкніть кнопку, встановивши її властивість `disabled` у значення `true`. Також встановіть `textContent` кнопки у `"No more data to load"`.
...
Ви повинні встановити властивість `disabled` у `loadMoreBtn` зі значенням `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Ви повинні встановити `textContent` у `loadMoreBtn` зі значенням `"No more data to load"`.
