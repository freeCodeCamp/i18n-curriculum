...
Ahora el `applyFunction` necesita devolver un resultado. Devuelve el resultado de llamar al método `.replace()` en `str2`. Pasa tu expresión regular `functionCall` y un callback vacío.

# --hints--

La función `applyFunction` debe devolver el resultado de llamar al método `.replace()` en `str2`.
...
Debes pasar `functionCall` como el primer argumento de tu llamada a `.replace()`.
...
Debes pasar una función flecha vacía como segundo argumento de tu llamada a `.replace()`.
