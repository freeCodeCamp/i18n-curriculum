...
前のプロジェクトでは、エラー処理に`.catch()`メソッドを使いました。ここでは代わりに`try...catch`文を使います。

`try`ブロックは潜在的なエラーを処理するためのもので、エラーが発生した場合は`catch`ブロック内のコードが実行されます。
...
`fetchData`関数の中に`try...catch`文を追加してください。`catch`ブロックには`err`という名前のエラーパラメータが必要です。

# --hints--

`try`関数の中に`fetchData`ブロックがあるはずです。

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}/);
```

`catch`ブロックの後に`try`ブロックがあるはずです。

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}\s*catch\s*\(.*\)\s*{[^}]*\s*}/);
```

`catch`ブロックには`err`パラメータが必要です。
