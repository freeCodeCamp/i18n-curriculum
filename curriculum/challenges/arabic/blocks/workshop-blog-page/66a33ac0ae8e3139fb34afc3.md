...
الآن بعد أن أنهيت إضافة جميع منشورات المدونة، جرّب النقر على رابط `Posts` وسترى أن الصفحة تنتقل إلى قسم `Posts`.

آخر مكوِّن تضيفه إلى صفحة مدونتك سيكون قسم الاتصال.

أسفل عنصر `main`، أضف عنصر `footer`.

# --hints--

يجب أن يحتوي عنصر `footer` على وسم البداية.

```js
assert.match(code, /<footer\>/);
```

يجب أن يحتوي عنصر `footer` على وسم الإغلاق.

```js
assert.match(code, /<\/footer\>/);
```

يجب أن يكون عنصر `footer` داخل عنصر `body`.
