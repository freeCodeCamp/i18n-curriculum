...
`forEach` 配列メソッドを使って elements 引数を繰り返すことで関数を完成させてください。コールバック関数の中で、`.add()` の `classList` メソッドを使って各要素に hidden クラスを追加してください。

# --hints--

`forEach` パラメータに `elements` メソッドが付いているはずです。
...
forEach メソッドは `el` という名前のパラメータを取るはずです。
...
forEach メソッドの中で、`classList.add` メソッドを使って hidden クラスを追加してください。
