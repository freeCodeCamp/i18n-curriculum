...
f-strings는 `.nf`이 표시할 10진수 숫자 개수인 `n` 형식 지정자를 사용해 숫자 데이터에 특정 정밀도를 설정할 수 있습니다.

`result_list` 안에 있는 f-string의 중괄호 안에 `3` 바로 뒤에 `:+` 10진수 숫자를 표시하는 데 필요한 형식 지정자를 작성하세요.

# --hints--

`result_list` 블록 안에 있는 `case 1` 문자열을 `f'x = {results[0]:+.3f}'`으로 수정하세요.
...
`result_list` 블록 안에 있는 `case 2` 문자열을 수정해 결과가 `3` 10진수 숫자로 표시되도록 하세요.
