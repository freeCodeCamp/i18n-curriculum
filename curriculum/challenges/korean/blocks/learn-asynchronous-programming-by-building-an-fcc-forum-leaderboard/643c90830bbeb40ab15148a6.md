...
이제 `postsContainer` 안에 데이터를 채우기 시작하세요.

먼저 `map()` 배열에서 `topics` 메서드를 호출하세요. 콜백 함수로는 `item`를 매개변수로 받는 빈 화살표 함수를 사용하세요.

그다음 `map()` 메서드의 결과를 `postsContainer.innerHTML`에 할당하세요.

# --hints--

`map()` 배열에서 `topics` 메서드를 사용해야 합니다.

```js
assert.match(code, /topics\.map\(.*/);
```

`map()` 메서드에 `item`를 매개변수 이름으로 하는 콜백 함수를 추가해야 합니다.

```js
assert.match(code, /topics\.map\(\s*(\(\s*item\s*\)|item)\s*=>/g);
```

콜백 함수의 본문은 비워 두어야 합니다.

```js
assert.match(code, /topics\.map\(\s*(\(\s*item\s*\)|item)\s*=>\s*{\s*}/g);
```

`map()` 메서드의 결과를 `postsContainer.innerHTML`에 할당해야 합니다.
