...
選取 `.back-mountain` 元素，並將其 `width` 和 `height` 設定為 `300px`。然後，將 `background` 設定為從 `rgb(203, 241, 228)` 開始到 `rgb(47, 170, 255)` 結束的線性漸層。

# --hints--

你應該使用 `.back-mountain` 選擇器。

```js
assert.match(code, /\.back-mountain\s*\{/);
```

你應該給 `.back-mountain` 一個 `width` 為 `300px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

你應該給 `.back-mountain` 一個 `height` 為 `300px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

你應該給 `.back-mountain` 一個 `background` 為 `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`。
