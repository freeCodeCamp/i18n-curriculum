...
`select` 요소(`option` 요소들과 함께)를 `label` 텍스트가 있는 `How did you hear about us?` 요소 안에 중첩하세요. 텍스트는 `select` 요소 앞에 와야 합니다.

# --hints--

`select` 요소만 `label` 요소 안에 중첩하세요.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(3) > select'));
```

`label` 요소에 `How did you hear about us?` 텍스트를 넣으세요.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(3)')?.innerText?.trim(), 'How did you hear about us?');
```

텍스트는 `select` 요소 앞에 위치해야 합니다.
...
`option` 요소 안에 `select` 요소가 5개 있어야 합니다.
