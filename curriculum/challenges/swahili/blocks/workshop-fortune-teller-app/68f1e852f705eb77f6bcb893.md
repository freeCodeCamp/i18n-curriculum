...
Tangaza kigezo kinachoitwa `cardId`, na kisha weka thamani ya sifa ya kipengele `data-id` kutoka `cardElement` kwa kutumia njia `getAttribute()`.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `cardId` ndani ya `showFortune`.
...
Kigezo chako cha `cardId` kinapaswa kupewa thamani kwa kufanya wito wa `cardElement.getAttribute("data-id")`.
...
Unapaswa kutumia njia `getAttribute` kwenye `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

Unapaswa kupitisha `'data-id'` kwa `getAttribute`.
