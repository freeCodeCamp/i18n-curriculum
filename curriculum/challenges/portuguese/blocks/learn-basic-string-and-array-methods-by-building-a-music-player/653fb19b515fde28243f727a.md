...
Para exibir as músicas na UI (User Interface), você precisará criar uma função.

Use a sintaxe de função arrow para criar uma função `renderSongs` que recebe `array` como seu parâmetro.

# --hints--

Você deve usar `const` para criar uma função vazia chamada `renderSongs`.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs` deve ser uma função.

```js
assert.isFunction(renderSongs)
```

Sua função `renderSongs` deve usar a sintaxe de função arrow.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

Sua função `renderSongs` deve receber `array` como parâmetro.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

Sua função `renderSongs` deve estar vazia.
