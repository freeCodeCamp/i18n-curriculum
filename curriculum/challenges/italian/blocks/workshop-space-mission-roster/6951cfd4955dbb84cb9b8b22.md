...
Avevi precedentemente aggiunto una chiamata `console.log()` alla tua funzione `addCrewMember` per validarne il comportamento, e ora quella chiamata non è più necessaria. Per evitare che il terminale si riempia nei passaggi successivi, procedi a eliminare la chiamata `console.log()` attuale.

# --hints--

Elimina la chiamata `console.log()` dalla tua funzione `addCrewMember`. Dovresti cancellare l'intera riga.
