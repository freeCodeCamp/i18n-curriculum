...
أنشئ `label` آخر يحتوي على النص `Real Name` وضمنه `input` من نوع `password`. عيّن للـ `input` قيمة `value` تساوي `realName`.

ثم استخدم حدث `onChange` لتحديث `realName` بنفس الطريقة التي حدثت بها `heroName`.

# --hints--

يجب أن تنشئ عنصر `label`.
...
يجب أن يحتوي عنصر `label` على النص `Real Name`.
...
يجب أن تنشئ `input` من النوع `password` داخل عنصر `label` الخاص بك.
...
يجب أن تكون قيمة عنصر `input` مضبوطة على متغير الحالة `realName`.

```js
assert.match(code, /value\s*=\s*\{\s*realName\s*\}/)
```

يجب أن تستخدم حدث `onChange` لضبط قيمة متغير الحالة `realName` حتى تتمكن من الكتابة في حقل الـ `input`.
