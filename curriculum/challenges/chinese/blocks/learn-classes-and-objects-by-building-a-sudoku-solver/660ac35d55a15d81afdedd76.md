...
在 `for` 循环内，声明一个变量 `row_str` 并为其分配一个列表推导，该列表推导遍历 `row` 并将 `row` 中的每个项目 `i` 转换为字符串。 为此使用 `str()` 函数。

# --hints--

你应该声明一个名为 `row_str` 的变量。
...
你应该将遍历 `row` 的列表推导分配给 `row_str` 变量。
...
分配给 `row_str` 变量的列表推导应该使用 `i` 作为迭代变量。
...
分配给 `row_str` 变量的列表推导应该对 `row` 中的每个项目 `i` 调用 `str()`。
