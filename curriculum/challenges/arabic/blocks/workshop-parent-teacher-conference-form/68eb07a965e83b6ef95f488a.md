...
استمر مع الحاوية الرئيسية، وأعطِ صنف `container` خاصية `width` بقيمة `80%` و`max-width` بقيمة `600px`. أضف أيضًا `border-radius` بقيمة `10px` واضبط `margin` على `20px auto` لتمركزها أفقيًا.

# --hints--

يجب أن تستهدف صنف `container`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.container'));
```

اضبط خاصية `width` على `80%`.
...
اضبط خاصية `max-width` على `600px`.
...
اضبط خاصية `border-radius` على `10px`.
...
اضبط خاصية `margin` على `20px auto`.
