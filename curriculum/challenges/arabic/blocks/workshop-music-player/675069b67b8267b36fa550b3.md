...
أضف مستمع حدث `"click"` إلى عنصر `previousButton`، ثم مرر `playPreviousSong` كالمعلمة الثانية.

# --hints--

يجب استدعاء الطريقة `addEventListener()` على متغير `previousButton` الخاص بك.
...
يجب تمرير `playPreviousSong` كالمعلمة الثانية لطريقة `addEventListener`.
