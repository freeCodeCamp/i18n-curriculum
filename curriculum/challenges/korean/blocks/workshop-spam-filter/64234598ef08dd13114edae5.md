...
다음으로 `s`와 `t` 문자를 각각 `5`와 `7`에 맞게 업데이트하세요.

# --hints--

`stockRegex`는 문자 `s`와 숫자 `5`에 맞도록 문자 클래스를 사용해야 합니다.

```js
assert.match(stockRegex.source, /\[(s5|5s)\]/);
```

`stockRegex`는 문자 `t`와 숫자 `7`에 맞도록 문자 클래스를 사용해야 합니다.
...
`stockRegex`는 `570ck al3r7`에 맞아야 합니다.
