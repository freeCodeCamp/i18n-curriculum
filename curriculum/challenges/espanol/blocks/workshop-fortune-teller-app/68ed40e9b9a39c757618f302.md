...
Ahora necesitas una clase principal que ayude a renderizar la interfaz de usuario dinámicamente en el DOM.

Crea una clase llamada `Game`. Dentro de ella, agrega una propiedad llamada `cards` de tipo `Card[]`, y por ahora, inicialízala con un arreglo vacío.

# --hints--

Debes tener una clase llamada `Game`.
...
Debes tener una propiedad `cards` dentro de la clase `Game`.
...
Tu propiedad `cards` debe tener un tipo `Card[]`.
...
Tu propiedad cards debe estar asignada a un `array` vacío.
