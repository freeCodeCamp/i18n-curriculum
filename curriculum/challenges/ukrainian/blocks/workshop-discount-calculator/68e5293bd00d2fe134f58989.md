...
Тепер реалізуйте метод `apply_discount` для `FixedAmountDiscount`. Цей метод має відняти фіксовану суму від ціни продукту і повернути нову ціну.

# --hints--

Ваш клас `FixedAmountDiscount` має містити метод `apply_discount`.
...
Ваш метод `apply_discount` має повертати `product.price - self.amount`.
