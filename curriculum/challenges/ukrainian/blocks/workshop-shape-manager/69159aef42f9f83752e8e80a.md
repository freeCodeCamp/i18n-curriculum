...
Тепер викличте функцію `chooseShape()` з `val.value`, щоб показати правильні поля введення, а потім викличте `updateResult()`, щоб перерахувати та відобразити область.

# --hints--

Ви повинні викликати функцію `chooseShape` з `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

Ви повинні викликати функцію `updateResult()`.
