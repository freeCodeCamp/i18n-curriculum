...
將 `.cat-head` 元素的 `position` 屬性設置爲 `absolute`。 將 `right`、`left`、`top`、`bottom` 屬性值設置爲 `0`，然後將其 `margin` 屬性的所有方向都設置爲 `auto`。 這是使用 CSS 定位將元素垂直和水平居中的一種方法。

# --hints--

你的 `.cat-head` 選擇器應將 `position` 屬性設置爲 `absolue`。 確保你添加了分號。
