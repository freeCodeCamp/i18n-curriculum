...
استخدم حلقة `for` لتسجيل `name` لكل رائد فضاء في مصفوفة `updatedCrew`. بعد الحلقة، أرجع مصفوفة `updatedCrew` لإكمال دالة `swapCrewMembers` الخاصة بك.

# --hints--

يجب أن تستخدم حلقة `for` للتكرار خلال مصفوفة `updatedCrew` وتسجيل `name` لكل رائد فضاء.
...
يجب أن ترجع مصفوفة `updatedCrew` بعد تسجيل أسماء رواد الفضاء.
