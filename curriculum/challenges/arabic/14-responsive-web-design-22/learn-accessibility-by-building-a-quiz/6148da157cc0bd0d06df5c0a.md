...
لمواءمة صناديق الإدخال مع بعضها البعض، أنشئ مجموعة قواعد جديدة تستهدف جميع عناصر `input` و `label` داخل `.info` وتعيين خاصية `display` إلى `inline-block`.

أيضًا، بمحاذي (align) نص عنصر `label` إلى اليمين.

# --hints--

يجب عليك استخدام منتقي `.info > label, .info > input` أو `.info label, .info input`.
...
يجب عليك إعطاء عناصر `input` و `label` خاصية `display` بقيمة `inline-block`.
...
يجب عليك إعطاء `label` خاصية `text-align` بقيمة `right`.
