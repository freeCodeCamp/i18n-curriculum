...
`.list-title, .list-subtitle` セレクターを作成し、`color` プロパティを `#00beef` に設定してください。

# --hints--

`.list-title, .list-subtitle` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'));
```

`.list-title, .list-subtitle` セレクターには `color` プロパティが `#00beef` に設定されているはずです。
