...
Rufen Sie innerhalb der `playSong`-Funktion die `highlightCurrentSong`-Funktion auf.

Probieren Sie danach die Steuerungstasten aus, um zu sehen, wie die `highlightCurrentSong`-Funktion funktioniert.

# --hints--

Sie sollten `highlightCurrentSong` innerhalb der `playSong`-Funktion aufrufen.
