...
要完成实验，你必须使用递归构建结果，并且不得使用循环（`for`、`while`）、`Array.from()` 或数组迭代方法（`forEach`、`map`、`filter`、`reduce`）。每次调用都应生成自己的结果数组（不要使用全局的变量来保存倒计时）。
...
5. 你必须使用递归（该函数应调用自身），且不得使用任何形式的循环（`for`、`while`、`Array.from()` 或诸如 `forEach`、`map`、`filter`、`reduce` 之类的高阶方法）。
...
你的代码不应依赖任何类型的循环（`for`、`while`、`Array.from()` 或更高阶的函数，如 `forEach`、`map`、`filter` 和 `reduce`）。
