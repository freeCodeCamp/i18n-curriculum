...
Para el último paso, necesitas hacer que el panel sea visible cuando se haga clic en su pestaña asociada. Puedes hacer esto configurando la propiedad `hidden` del `panel` a `false`.

Haz clic en cualquiera de las pestañas, y deberías ver el contenido del panel correspondiente mostrado.

¡Y con ese último cambio, tu taller está completo!

# --hints--

Cuando se hace clic en una pestaña, debes establecer la propiedad `hidden` del panel en `false`.
