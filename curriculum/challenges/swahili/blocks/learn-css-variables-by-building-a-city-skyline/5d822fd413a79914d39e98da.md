...
Jengo lako la kwanza linaonekana vizuri sasa. Panga ndani vipengele vitatu vipya vya `div` katika kontena la `.background-buildings` na uwape madarasa ya `bb2`, `bb3`, na `bb4` kwa mpangilio huo. Hivi vitakuwa majengo matatu zaidi kwa mandhari ya nyuma.

# --hints--

Unapaswa kuunda `div` yenye darasa la `bb2`.

```js
assert.exists(document.querySelector('div.bb2'));
```

Unapaswa kuunda `div` yenye darasa la `bb3`.

```js
assert.exists(document.querySelector('div.bb3'));
```

Unapaswa kuunda `div` yenye darasa la `bb4`.

```js
assert.exists(document.querySelector('div.bb4'));
```

Unapaswa kuunda vipengele vipya 3 vya `div`.
...
Unapaswa kuweka vipengele kwa mpangilio sahihi.
