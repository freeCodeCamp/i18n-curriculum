...
الآن، تحتاج إلى إضافة الخاصية `aria-current` إلى الأغنية التي تُشغل حاليًا.

تحقق مما إذا كان `songToHighlight` موجودًا. إذا كان كذلك، استخدم طريقة `setAttribute` على `songToHighlight` ومرر الخاصيتين `"aria-current"` و `"true"` كالمعلمة الأولى والثانية.

# --hints--

يجب أن تضيف `highlightCurrentSong` الخاصية `aria-current` إلى الأغنية التي تُشغل حاليًا وتزيل الخاصية من الأغنية التي تم تشغيلها سابقًا.
