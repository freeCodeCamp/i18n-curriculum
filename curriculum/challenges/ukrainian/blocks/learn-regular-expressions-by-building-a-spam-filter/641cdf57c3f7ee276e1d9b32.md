...
Повернувшись до свого слухача подій, потрібно оновити текст властивості елемента `result`. Для цього можна використати тернарний оператор.

Ось приклад присвоєння результату тернарного оператора властивості текстового вмісту елемента:

```js
el.textContent = condition ? "Use this text if the condition is true" : "Use this text if the condition is false";
```

Після інструкції `if` використайте тернарний оператор, щоб перевірити істинність виклику `isSpam()` з аргументом `messageInput.value`. Якщо це правда, призначте властивість `textContent` елемента `result` значенням `"Oh no! This looks like a spam message."`. Інакше призначте їй `"This message does not seem to contain any spam."`.

Потім призначте властивість `messageInput` елемента `value` порожнім рядком.

# --hints--

Ви повинні використати оператор присвоєння, щоб призначити властивість `textContent` елемента `result`.
...
Ви повинні викликати функцію `isSpam()` після оператора присвоєння `=` і перед тернарним оператором `?`.
...
Використайте синтаксис тернарного оператора, щоб перевірити істинність `isSpam(messageInput.value)`.
...
Істинний вираз вашого тернарного оператора має призначити властивість `textContent` елемента `result` значенням `"Oh no! This looks like a spam message."`.
...
Хибний вираз вашого тернарного оператора має призначити властивість `textContent` елемента `result` значенням `"This message does not seem to contain any spam."`.
...
Після тернарного оператора призначте властивість `value` елемента `messageInput` порожнім рядком.
