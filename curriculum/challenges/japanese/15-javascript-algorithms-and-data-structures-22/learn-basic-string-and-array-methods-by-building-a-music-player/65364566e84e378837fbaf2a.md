...
ここまでのステップで、曲を再生する機能を実装しました。 次はその機能を再生ボタンに追加して、クリックされたときに現在の曲を再生するようにします。

`addEventListener()` メソッドを使用して、第一引数に `"click"` イベントを渡し、第二引数にアロー構文の空のコールバック関数を渡してください。(例: `() => {}`)

# --hints--

変数 `playButton` の `addEventListener()` メソッドを呼び出してください。
...
アロー構文を使用して、空のコールバックをイベントリスナーに渡してください。 また、空の波括弧のペアが必要であることを忘れないでください。
