...
O atributo `src` do seu elemento `img` deve estar definido como `https://cdn.freecodecamp.org/curriculum/cat-photo-app/relaxing-cat.jpg`. Você ou omitiu a URL ou cometeu um erro de digitação. A diferenciação entre maiúsculas e minúsculas na URL é importante.
...
Seu elemento `img` deve ser fechado com `>` ou `/>`.
