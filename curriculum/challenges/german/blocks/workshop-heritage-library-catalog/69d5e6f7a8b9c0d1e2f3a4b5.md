...
Füllen Sie jetzt `renderEntry` aus. Deklarieren Sie eine Fallback-Konstante für jedes Feld: `const title = entry.title || "Unknown"`, und wenden Sie dasselbe Muster für `author`, `year` und `location` an.

Geben Sie ein mehrzeiliges Template-Literal zurück, das den Eintrag als Bibliothekskarte formatiert. Verwenden Sie `"-".repeat(25)` für den oberen und unteren Rahmen und fügen Sie beschriftete Zeilen für `Title`, `Author`, `Year` und `Location` ein. Zum Beispiel würde eine beschriftete Zeile für den Titel so aussehen: `Title: ${title}`.

# --hints--

`renderEntry` sollte einen String zurückgeben.

```js
assert.isString(renderEntry(catalog[0]));
```

Wenn `catalog[0]` übergeben wird, sollte der zurückgegebene String `"From a Buick 8"` enthalten.

```js
assert.include(renderEntry(catalog[0]), "From a Buick 8");
```

Wenn `catalog[0]` übergeben wird, sollte der zurückgegebene String `"King, Stephen"` enthalten.

```js
assert.include(renderEntry(catalog[0]), "King, Stephen");
```

Wenn `catalog[0]` übergeben wird, sollte der zurückgegebene String das Jahr `2002` enthalten.

```js
assert.include(renderEntry(catalog[0]), "2002");
```

Wenn `catalog[0]` übergeben wird, sollte der zurückgegebene String `"Shelf K7"` enthalten.

```js
assert.include(renderEntry(catalog[0]), "Shelf K7");
```

Der zurückgegebene String sollte einen oberen und unteren Rahmen mit `"-".repeat(25)` enthalten.
