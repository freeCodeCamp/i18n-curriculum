...
마지막으로 `hideElements` 안에서 `singleCard`, `multipleCard`, `fortuneContainer`, `fortuneDescription` 인수를 전달하세요. 요소에서 값을 가져오는 것을 잊지 마세요.

이로써 운세 앱이 완성됩니다!

# --hints--

`singleCard`를 `hideElements`의 첫 번째 인수로 전달해야 합니다.
...
`multipleCard`를 `hideElements`의 인수로 전달해야 합니다.
...
`fortuneContainer`를 `hideElements`의 인수로 전달해야 합니다.
...
`fortuneDescription`를 `hideElements`의 인수로 전달해야 합니다.
