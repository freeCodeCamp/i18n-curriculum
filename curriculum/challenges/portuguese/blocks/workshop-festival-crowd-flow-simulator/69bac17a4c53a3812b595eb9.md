...
Antes de iniciar a simulação, você precisa de um objeto para acompanhar quantos participantes cada portão processa durante todos os ticks. Faça uma chamada a `initializeThroughput()` com `gates` e atribua o resultado a uma variável chamada `throughputSummary`.

# --hints--

Você deve fazer uma chamada a `initializeThroughput()` com `gates` e atribuir o resultado a uma variável chamada `throughputSummary`.
...
Sua função `simulateFestival` deve fazer uma chamada à função `initializeThroughput()` com `gates`.
