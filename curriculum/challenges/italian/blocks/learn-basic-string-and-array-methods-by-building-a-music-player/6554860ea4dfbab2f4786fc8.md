...
Se guardi bene, vedrai che la canzone in riproduzione non è evidenziata nella playlist, quindi non sai davvero quale canzone sta suonando. Puoi risolvere questo creando una funzione per evidenziare qualsiasi canzone che viene riprodotta.

Usando una sintassi a freccia, crea una funzione `highlightCurrentSong`. All’interno della funzione, usa `querySelectorAll` per ottenere l’elemento `.playlist-song` e assegnalo a una costante `playlistSongElements`.

# --hints--

Dovresti usare `const` per creare una funzione `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` dovrebbe essere una funzione.

```js
assert.isFunction(highlightCurrentSong)
```

La tua funzione `highlightCurrentSong` dovrebbe usare una sintassi a freccia.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

Dovresti creare una costante `playlistSongElements` all’interno della tua funzione `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

Dovresti usare il metodo `querySelectorAll()` per selezionare l’elemento `.playlist-song` e assegnarlo alla costante `playlistSongElements`.
