...
أنشئ قاعدة `.lists li` لاستهداف عناصر القائمة داخل عنصر `.lists`. امنحها خاصية `margin-bottom` بقيمة `1.5rem`.

# --hints--

يجب أن يكون لديك محدِّد `.lists li`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

يجب أن يحتوي محدِّد `.lists li` على خاصية `margin-bottom` بقيمة `1.5rem`.
