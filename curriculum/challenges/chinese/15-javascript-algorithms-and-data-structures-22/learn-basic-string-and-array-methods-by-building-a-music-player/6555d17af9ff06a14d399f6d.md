...
在函数内部，获取负责显示歌曲标题和艺术家的 HTML 元素的引用。

使用 `getElementById()` 方法访问 `#player-song-title` 和 `#player-song-artist` 元素。 将它们分别分配给变量 `playingSong` 和 `songArtist`。

# --hints--

你不应修改现有的 `setPlayerDisplay` 函数及其内容。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

你应该使用 `document.getElementById()` 来获取 `#player-song-title` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

你应该将 `#player-song-title` 元素分配给变量 `playingSong`。 不要忘记使用 `const` 来声明变量。

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

你应该使用 `document.getElementById()` 来获取 `#player-song-artist` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

你应该将 `#player-song-artist` 元素分配给变量 `songArtist`。 不要忘记使用 `const` 来声明变量。
