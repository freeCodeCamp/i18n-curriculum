...
Usando la tua nuova classe `flavor` come selettore, imposta il valore della proprietà `text-align` su `left`.
...
Dovresti impostare la proprietà `text-align` su `left`.
...
Il tuo selettore di classe `flavor` dovrebbe impostare la proprietà `text-align` su `left`.
