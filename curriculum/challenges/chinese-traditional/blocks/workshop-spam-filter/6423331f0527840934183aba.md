...
你的正則表達式應該匹配整個單詞，而不是部分單詞。 也就是說，你不想匹配 `hands-free money management`。

爲此，首先檢查模式前後的空格。 你可以使用元字符 `\s` 來執行此操作，它將匹配空格、製表符和換行符。

# --hints--

你的 `freeRegex` 應該使用 `\s` 令牌。

```js
assert.match(freeRegex.source, /\\s/);
```

你的 `freeRegex` 應該在模式的開頭和結尾尋找空格。
