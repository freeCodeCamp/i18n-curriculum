...
Añade un bloque `else` para manejar el tiempo de reproducción actual de la canción. Esto le permite reanudar la canción actual en el punto en el que estaba pausada.

Dentro del bloque `else`, establecer la propiedad `currentTime` del objeto `audio` al valor almacenado en `userData?.songCurrentTime`.

# --hints--

No debes modificar la sentencia existente `if` y su contenido.
...
Debes añadir un bloque `else` al bloque existente `if`.
...
Debes establecer `audio.currentTime` en `userData?.songCurrentTime` dentro del bloque `else` de tu declaración `if`.
