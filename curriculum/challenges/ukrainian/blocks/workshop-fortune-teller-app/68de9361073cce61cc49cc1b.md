...
У попередній лекції ви дізналися, що виведення типів — це здатність TypeScript автоматично визначати та призначати типи змінним, поверненням функцій і виразам на основі їх використання та контексту.

У TypeScript дженерики роблять функції, класи та типи більш гнучкими. Дженерики дозволяють створювати типові змінні, усуваючи необхідність явно визначати типи щоразу. Це дає змогу одній і тій самій функції безпечно працювати з різними типами.

Ось простий приклад дженериків:
...
Функції можуть використовувати два або більше дженеріків. `T` використовується за конвенцією, а додаткові типи, як-от `S`, можна додавати за потреби.
...
Створіть функцію `getElement`, яка приймає параметр `selector` типу `string`. Вона також має приймати дженерік тип `T`, що розширює `HTMLElement`, і повертати тип `T`.

# --hints--

У вас має бути стрілкова функція з назвою `getElement`.
...
Ваша функція `getElement` має приймати параметр з назвою `selector` типу `string`.
...
Дженерік параметр типу `T` має розширювати тип `HTMLElement`.
...
Ваша функція `getElement` має мати тип повернення `T`.
