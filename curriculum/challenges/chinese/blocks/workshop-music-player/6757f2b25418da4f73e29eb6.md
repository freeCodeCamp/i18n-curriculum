...
接下来，你需要在播放器显示屏上显示当前歌曲标题和艺术家。

第一步，访问负责显示歌曲标题和艺术家名称的 `#player-song-title` 和 `#player-song-artist` 元素，并将它们分别保存到变量 `playingSong` 和 `songArtist` 中。

# --hints--

你应该有一个名为 `playingSong` 的变量。

```js
assert.isDefined(playingSong);
```

你的 `playingSong` 变量应具有 `#player-song-title` 元素的值。

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

你应该有一个名为 `songArtist` 的变量。
...
你的 `songArtist` 变量应具有 `#player-song-artist` 元素的值。
