...
A função `pin_extractor` deve ter um parâmetro chamado `poems`.
...
Você deve ter um loop `for` que itere sobre `poems`.
...
O loop deve ter `poem` como variável do loop.
...
Você deve mover o corpo existente da função para o novo `for` loop.
