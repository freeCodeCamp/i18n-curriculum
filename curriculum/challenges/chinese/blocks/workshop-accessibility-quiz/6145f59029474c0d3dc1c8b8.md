...
与其他 `input` 和 `label` 元素一样，将 `textarea` 链接到其对应的 `label` 元素，并赋予其 `name` 属性。

# --hints--

你应该给 `textarea` 元素一个 `id` 属性。
...
你必须为 `textarea` 元素选择一个不同的 `id` ，因为另一个元素正在使用该 `id`。
...
你应该给 `label` 元素一个与 `textarea` 的 `id` 相匹配的 `for` 属性。
