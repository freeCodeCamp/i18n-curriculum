...
이미지 요소에 `onerror` 핸들러를 추가하여 로딩 실패를 처리하세요. 첫 번째 실패 시 `dataset.failed`를 사용해 표시하고 소스를 로컬 기본 이미지로 교체하세요. 이미지가 다시 실패하면 hidden 클래스를 제거하고 로딩 자리 표시자를 `previousElementSibling`로 숨기세요.

# --hints--

`img` 요소에 `onerror` 핸들러가 있어야 합니다.
...
`onerror` 핸들러에는 `if`이 falsy인지 확인하는 `dataset.failed` 문이 포함되어야 합니다.
...
`dataset.failed`은 `'1'` 블록 안에서 `if`로 설정해야 합니다. 여기서 `this` 키워드를 잊지 마세요.
...
`if` 블록은 `this.src`을 로컬 기본 이미지로 업데이트해야 합니다. 여기서 `this` 키워드를 잊지 마세요.
...
`hidden` 클래스를 제거하는 `else` 블록이 있어야 합니다. 여기서 `this` 키워드를 잊지 마세요.
...
`else` 블록은 `previousElementSibling.style.display`를 `'none'`로 설정해야 합니다. 여기서 `this` 키워드를 잊지 마세요.
