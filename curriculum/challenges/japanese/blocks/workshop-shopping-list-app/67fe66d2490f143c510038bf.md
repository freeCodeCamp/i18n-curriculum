...
次に、チェックボックス `input` 要素に `checked` 属性を追加し、`isChecked` に設定してください。`checked` 属性を設定することは必須ではありませんが、各チェックボックスが現在の `selectedItems` 配列の状態を反映していることを確認するために設定することをお勧めします。

# --hints--

チェックボックス `checked` 要素に `input` 属性を追加してください。
...
`checked` 属性を `isChecked` に設定してください。
