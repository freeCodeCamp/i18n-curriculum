...
Para a última parte do workshop, são necessárias algumas mudanças na seção de contato na parte inferior.

A primeira mudança seria envolver a seção de contato dentro de um elemento `footer`. Isso ajudará leitores de tela a identificar essa seção como o rodapé da página.

# --hints--

Sua página deve ter um elemento `footer`.
...
Seu elemento `footer` deve conter um elemento `h3` com o texto `Contact Me`.
...
Seu elemento `footer` deve conter um elemento `p` com o texto `Email me at camperbot@blog.io`.
