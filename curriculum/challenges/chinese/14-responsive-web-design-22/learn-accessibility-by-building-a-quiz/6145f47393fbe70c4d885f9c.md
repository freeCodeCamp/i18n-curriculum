...
在第二个 `div.answer` 元素中嵌套一个 `textarea` 元素，并设置它的行数和列数。

然后，给 `textarea` 添加描述示例答案的占位符文本。

# --hints--

你应该在第二个 `div.answer` 元素中嵌套一个 `textarea` 元素。

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

你应该给 `textarea` 一个带有数字的 `rows` 属性。
...
您应该给 `textarea` 添加一个 `placeholder` 属性，其中包含描述示例答案的文本。
