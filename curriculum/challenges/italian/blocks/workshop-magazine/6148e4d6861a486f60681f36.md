...
Le immagini devono essere disposte in una griglia di due colonne e tre righe.

Imposta la proprietà `.image-wrapper` del selettore `grid-template-columns` su `2fr 1fr` e la proprietà `grid-template-rows` su `repeat(3, min-content)`. Questo farà sì che le righe della griglia si adattino in altezza in base al contenuto, mentre le colonne manterranno una larghezza fissa basata sul contenitore.
