...
Toa vipengele vya `.fb4-window` `width` ya `30%`, `height` ya `10%`, na `border-radius` ya `50%`. Hii itaunda madirisha ya mduara kwa jengo hili.

# --hints--

Unapaswa kumpa `.fb4-window` `width` ya `30%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

Unapaswa kumpa `.fb4-window` `height` ya `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

Unapaswa kumpa `.fb4-window` `border-radius` ya `50%`.
