...
`associatedPanel` は `id` なので、ドキュメントをクエリして対応するパネル要素を見つけることができます。

`panel` という新しい変数を作成し、`document.getElementById(associatedPanel)` の値を代入してください。

# --hints--

クリックされたタブに関連付けられたパネル要素を保持する `panel` という変数があるはずです。
