...
對於你的 `timeAgo` 函數，你需要計算當前時間與一個主題上最後一次活動的時間之間的差值。 這樣就可以顯示自某個主題有活動以來過去了多長時間。

完成符合以下要求的 `timeAgo` 函數：

- 如果已過去的分鐘數少於 `60` 分鐘，則返回字符串 `"xm ago"`。 `x` 將表示分鐘數。
- 如果已過去的小時數少於 `24`，則返回字符串 `"xh ago"`。 `x` 將表示小時數。
- 否則，返回字符串 `"xd ago"`。 `x` 將表示天數。
...
如果已過去的分鐘數爲 `30` 分鐘，你應該返回字符串 `"30m ago"`。
...
如果已過去的小時數爲 `1` 小時，你應該返回字符串 `"1h ago"`。
