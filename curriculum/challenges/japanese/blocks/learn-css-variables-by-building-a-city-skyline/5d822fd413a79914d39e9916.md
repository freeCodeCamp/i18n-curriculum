...
下のスペースは気にしないでください。建物の上部の要素に高さを追加すると、すべて下に移動します。

`repeating-linear-gradient` を `.fb1c` に `90deg` の角度で追加し、`--building-color4` から `0%` までの `10%` と、`transparent` から `10%` までの `15%` を追加してください。

# --hints--

`.fb1c` に `background` の `repeating-linear-gradient` を設定してください。
...
最初の色は `--building-color4` から `0%` までの `10%` を使うべきです。
...
2番目の色は `transparent` から `10%` までの `15%` を使うべきです。
