...
Baada ya kigawanyo chako cha mwisho, tengeneza kipengele kingine cha `p` chenye maandishi ya `Protein 3g`. Tumia madarasa yanayohitajika kuondoa mpaka wa chini, na `span` ili kufanya `Protein` kuwa herufi nzito. Kisha fungua maandishi ya `Protein 3g` ikiwa ni pamoja na kipengele kipya cha `span`, ndani ya kipengele kipya cha `span`.

Baada ya kipengele hiki, tengeneza kigawanyo kikubwa.
...
Kipengele chako kipya cha `p` kinapaswa kuwa na maandishi ya `Protein 3g`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Protein[\s|\n]+3g/));
```

Kipengele chako kipya cha `p` kinapaswa kuwa na sifa ya kipengele ya `class` iliyowekwa kuwa `no-divider`.
...
Unapaswa kuwa na kipengele cha `span` kinachozunguka maandishi ya `Protein 3g`.
...
`div` yako mpya inapaswa kuwa na sifa ya kipengele ya `class` iliyowekwa kuwa `divider large`.
