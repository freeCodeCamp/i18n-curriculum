...
右の内耳に取り組む時間です。クラスセレクターを使って、`.cat-right-inner-ear` 要素に左右のボーダーを `20px solid transparent` に設定してください。また、下のボーダーを `40px solid #3b3b4f` に設定してください。

# --hints--

`.cat-right-inner-ear` セレクターがあるはずです。

```js 
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear'))
```

`.cat-right-inner-ear` セレクターには `border-left` プロパティを `20px solid transparent` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderLeft, '20px solid transparent')
```

`.cat-right-inner-ear` セレクターには `border-right` プロパティを `20px solid transparent` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderRight, '20px solid transparent')
```

`.cat-right-inner-ear` セレクターには `border-bottom` プロパティを `40px solid #3b3b4f` に設定してください。
