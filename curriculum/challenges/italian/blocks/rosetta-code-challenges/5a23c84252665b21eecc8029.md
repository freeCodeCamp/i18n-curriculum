...
Implementa funzioni per criptare e decriptare un messaggio usando il metodo della **scacchiera incrociata**.

Ogni funzione riceverà due argomenti, una stringa `message` e un array `alphabet`. L'array `alphabet` conterrà 3 stringhe che rappresentano le 3 righe della scacchiera incrociata.

L'output della funzione `straddle()` dovrebbe essere una serie di cifre decimali. I numeri devono essere criptati inserendo il carattere di escape prima di ogni cifra, poi includendo la cifra <em>non criptata</em>.

Questo processo deve essere invertito per la decriptazione con la funzione `unstraddle()`.
