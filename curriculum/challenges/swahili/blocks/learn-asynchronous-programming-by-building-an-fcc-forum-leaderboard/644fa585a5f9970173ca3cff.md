...
Unda thabiti iitwayo `linkClass` na weka thamani yake kuwa usemi wa template. Ndani ya usemi huo wa template, ongeza thamani ya `category ${selectedCategory.className}`.

Majina haya ya madarasa ya vitu yatatumika kuweka mitindo kwa kipengele cha nanga.

# --hints--

Unapaswa kuwa na thabiti iitwayo `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

Unapaswa kuweka usemi wa template kwa mabadiliko yako ya `linkClass`.

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

Unapaswa kuwa na thamani ya `category ${selectedCategory.className}` ndani ya usemi wa template.
