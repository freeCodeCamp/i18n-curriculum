---
id: 656877c202f44995ed952608
title: Passaggio 2
challengeType: 20
dashedName: step-2
---

# --description--

Sostituisci l'istruzione `pass` con una variabile chiamata `card_number` e un valore di `'4111-1111-4555-1142'`.

# --hints--

Dovresti avere `card_number = '4111-1111-4555-1142'` all'interno della funzione `main`.

```js
({
    test: () => {
        const transformedCode = code.replace(/\r/g, "");
        const main = __helpers.python.getDef("\n" + transformedCode, "main");
        const { function_body } = main;

        assert.match(function_body, / +card_number\s*=\s*('|")4111-1111-4555-1142\1/);
    }
})
```

Non dovresti avere un `pass` nel tuo codice.

```js
({
    test: () => {
        const transformedCode = code.replace(/\r/g, "");
        const no_comments = __helpers.python.removeComments(transformedCode);
        assert.notInclude(no_comments, "pass");
    }
})
```

# --seed--

## --seed-contents--

```py
--fcc-editable-region--
def main():
    pass

--fcc-editable-region--
```
