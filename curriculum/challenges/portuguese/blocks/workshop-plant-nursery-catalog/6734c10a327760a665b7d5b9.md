...
Quando o número especificado de vasos for maior que os vasos disponíveis, faça sua função retornar `Not enough <pot-size> size pots for <item-name>. Only <pot-number> left.`. Substitua `<pot-size>` pelo tamanho do vaso, `<pot-number>` pelo número restante de vasos desse tamanho e `<item-name>` pelo nome científico da planta seguido de um espaço e o nome do cultivar entre aspas simples.
