...
Esegui una ricerca e sostituzione nella frase usando gli argomenti forniti e restituisci la nuova frase.

Il primo argomento è la frase su cui eseguire la ricerca e sostituzione.

Il secondo argomento è la parola che sostituirai (prima).

Il terzo argomento è con cosa sostituirai il secondo argomento (dopo).

**Nota:** Conserva il maiuscolo o minuscolo del primo carattere della parola originale quando la sostituisci. Per esempio, se devi sostituire la parola `Book` con la parola `dog`, dovrebbe essere sostituita come `Dog`
...
`myReplace("This has a spellngi error", "spellngi", "spelling")` dovrebbe restituire la stringa `This has a spelling error`.
