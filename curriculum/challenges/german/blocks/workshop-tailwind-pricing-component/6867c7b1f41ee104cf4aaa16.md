...
Jetzt verwenden Sie `grid`, um die Elemente innerhalb des Wrappers `div` anzuordnen.

Fügen Sie die Klassen `grid`, `grid-rows-[1fr_auto]` und `gap-6` dem Wrapper `div` hinzu.

# --hints--

Ihr Wrapper-Element `div` für den Familientarif sollte die Klasse `grid` haben.
...
Ihr Wrapper-Element `div` für den Familientarif sollte die Klasse `grid-rows-[1fr_auto]` haben.
...
Ihr Wrapper-Element `div` für den Familientarif sollte die Klasse `gap-6` haben.
