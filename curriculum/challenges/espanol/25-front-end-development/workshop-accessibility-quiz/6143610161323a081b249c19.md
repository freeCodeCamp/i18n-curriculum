...
Completando el contenido del cuestionario, debajo de `#student-info`, agrega tres elementos `div` con una `class` de `info`.

Luego, dentro de cada `div` anida un elemento `label` y un elemento `input`.

# --hints--

Deberías anidar tres elementos `div` debajo del elemento `h2#student-info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.length, 3);
```

Deberías dar al primer `div` una `class` de `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.className, 'info');
```

Deberías dar al segundo `div` una `class` de `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.className, 'info');
```

Deberías dar al tercer `div` una `class` de `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.className, 'info');
```

Deberías anidar un elemento `label` dentro del primer `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.querySelectorAll('label')?.length, 1);
```

Deberías anidar un elemento `input` dentro del primer `div`, después del `label`.
...
Deberías anidar un elemento `label` dentro del segundo `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.querySelectorAll('label')?.length, 1);
```

Deberías anidar un elemento `input` dentro del segundo `div`, después del `label`.
...
Deberías anidar un elemento `label` dentro del tercer `div`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.querySelectorAll('label')?.length, 1);
```

Deberías anidar un elemento `input` dentro del tercer `div`, después del `label`.
