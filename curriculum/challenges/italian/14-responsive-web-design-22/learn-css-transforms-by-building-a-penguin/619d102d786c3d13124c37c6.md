...
Dovresti dare a `.blush` una proprietà `top` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.top, '65%');
```

Dovresti dare a `.blush` una proprietà `border-radius` di `--fcc-expected--` invece di `--fcc-actual--`.
