...
Finalmente, agrega una cláusula `else` que imprima el movimiento permitido y llame a `make_allowed_move`. Intenta averiguar los argumentos correctos.
...
Debes tener `print(f'Move {i + 1} allowed between {source} and {target}')` dentro del bloque `else`.
...
Debes tener `make_allowed_move(source, target)` dentro del bloque `else`.
