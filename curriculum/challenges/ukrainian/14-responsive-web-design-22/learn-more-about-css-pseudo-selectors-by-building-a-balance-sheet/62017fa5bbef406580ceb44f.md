...
Ви додали ключове слово `!important`, тому з селектора `span` можна видалити `:not(.sr-only)`.

# --hints--

Ви не повинні мати селектор `span:not(.sr-only)`.

```js
assert(!(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)')));
```

Ви повинні мати селектор `span`.
