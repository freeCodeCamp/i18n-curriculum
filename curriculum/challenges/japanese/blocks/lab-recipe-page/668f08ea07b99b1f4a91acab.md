...
1. レシピ名を含む `title` 要素と、`charset` 属性が `UTF-8` に設定された `meta` 要素を含む `head` 要素が必要です。
1. `body` 要素が必要です。
1. レシピ名を持つ `h1` 要素は1つだけにしてください。
1. `h1` の下にレシピを紹介する `p` 要素が必要です。
1. 材料のセクション用にテキストが `Ingredients` の `h2` 要素が1つ必要です。
1. 最初の `h2` 要素の下に、材料をリストアップする4つ以上のリスト項目（`li` 要素）を持つ番号なしリスト（`ul` 要素）が必要です。
1. 手順のセクション用にテキストが `Instructions` の2つ目の `h2` 要素が必要です。
1. 2つ目の `h2` の下に、レシピのステップを順番にリストアップする4つ以上のリスト項目を持つ番号付きリスト（`ol` 要素）が必要です。
1. 有効な画像を指定した `src` 属性と、画像を説明する `alt` 属性を持つ `img` 要素が1つ必要です。必要なら `https://cdn.freecodecamp.org/curriculum/labs/recipe.jpg` を使ってもかまいません。
...
あなたの `html` 要素には終了タグが必要です。終了タグは `<` の直後に `/` が付きます。
...
あなたの `body` 要素には終了タグが必要です。終了タグは `<` の直後に `/` が付きます。

```js
assert.match(code, /<\/\s*body\s*>/i);
```

レシピ名を持つ `h1` 要素は1つだけにしてください。
