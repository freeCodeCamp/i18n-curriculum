...
Всередині масиву `allSongs` створіть об’єкт із такими властивостями та значеннями:
...
Ваш масив `allSongs` має містити одне значення.

```js
assert.equal(allSongs.length, 1);
```

Ваш масив `allSongs` має містити об’єкт як перше значення.

```js
assert.isObject(allSongs[0]);
```

Ваш масив `allSongs` має містити об’єкт із властивістю `id`, значення якої дорівнює числу `0`.

```js
assert.equal(allSongs[0].id, 0);
```

Ваш масив `allSongs` має містити об’єкт із властивістю `title`, значення якої дорівнює рядку `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

Ваш масив `allSongs` має містити об’єкт із властивістю `artist`, значення якої дорівнює рядку `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

Ваш масив `allSongs` має містити об’єкт із властивістю `duration`, значення якої дорівнює рядку `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

Ваш масив `allSongs` має містити об’єкт із властивістю `src`, значення якої дорівнює рядку `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
