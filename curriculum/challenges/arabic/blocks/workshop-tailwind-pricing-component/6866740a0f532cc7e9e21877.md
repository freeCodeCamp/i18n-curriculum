...
انتقل الآن إلى شارة `Most popular`. يجب وضعها في الزاوية العلوية اليمنى من البطاقة، لذا يمكنك استخدام بعض فئات المساعدة الخاصة بالتموضع في Tailwind لتحقيق ذلك.

إليك كيف تستخدم تموضع CSS في Tailwind:
...
عيّن الصنف `relative` على عنصر `div` الحاوي و`absolute` على عنصر `div` الذي يحتوي على النص `Most popular`.

# --hints--

يجب أن يحتوي عنصر `div` الحاوي لخطة البريميوم على الصنف `relative`.
...
يجب أن يحتوي عنصر `div` الذي يحتوي على النص `Most popular` على الصنف `absolute`.
