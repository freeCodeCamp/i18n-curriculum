...
最後に、`cartContainer` 変数の `style` オブジェクトの `display` プロパティを、`isCartShowing` が true かどうかをチェックする別の三項に更新してください。もし true なら、`display` プロパティを `"block"` に設定し、そうでなければ `"none"` に設定します。

これでカートが表示され、アイテムを追加できるはずです。

# --hints--

`cartContainer` 変数の `style` プロパティの `display` プロパティにアクセスしてください。

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display/)
```

`cartContainer` 変数の `style` プロパティの `display` プロパティに代入演算子を使ってください。
...
`cartContainer` 変数の `style` プロパティの `display` プロパティを、`"block"` が true の場合は `isCartShowing` に設定してください。

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*isCartShowing\s*\?\s*('|"|`)block\1\s*:\s*/)
```

`cartContainer` 変数の `style` プロパティの `display` プロパティを、`"none"` が false の場合は `isCartShowing` に設定してください。
