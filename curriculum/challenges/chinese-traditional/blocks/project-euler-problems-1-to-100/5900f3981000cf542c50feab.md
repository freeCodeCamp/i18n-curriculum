...
五邊形數由公式 P<sub>n</sub>=`n`(3`n`−1)/2 產生。前十個五邊形數為：

1、5、12、22、35、51、70、92、117、145、...

可以看出 P<sub>4</sub> + P<sub>7</sub> = 22 + 70 = 92 = P<sub>8</sub>。然而，它們的差，70 − 22 = 48，並不是五邊形數。

尋找五邊形數 P<sub>j</sub> 和 P<sub>k</sub> 的配對，使得它們的和與差皆為五邊形數，且 D = |P<sub>k</sub> − P<sub>j</sub>| 最小；D 的值為多少？

# --hints--

`pentagonNumbers()` 應該傳回一個數字。

```js
assert(typeof pentagonNumbers() === 'number');
```

`pentagonNumbers()` 應該傳回 5482660。
