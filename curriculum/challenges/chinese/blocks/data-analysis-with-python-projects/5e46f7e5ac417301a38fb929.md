...
你将使用我们的 Ona 启动代码 <a href="https://app.ona.com/?autostart=true#https://github.com/freeCodeCamp/boilerplate-demographic-data-analyzer/" target="_blank" rel="noopener noreferrer nofollow">来完成这个项目</a>。
...
- 这个数据集中每个种族的人数是多少？这应该是一个 Pandas 串行，种族名称作为索引标签。（`race` 列）
- 男性的平均年龄是多少？
- 拥有学士学位的人的百分比是多少？
...
- 哪个国家拥有收入超过 50K 的人比例最高，该比例是多少？
- 识别在印度收入超过 50K 的人中最受欢迎的职业。

使用文件 `demographic_data_analyzer.py` 中的入门代码。更新代码，使所有设置为 `None` 的变量设置为适当的计算或代码。将所有小数四舍五入到最近的十分位。

## 开发

在 `demographic_data_analyzer.py` 中编写你的代码。开发时，你可以使用 `main.py` 来测试你的代码。
...
Dua, D. 和 Graff, C.（2019）。<a href="http://archive.ics.uci.edu/ml" target="_blank" rel="noopener noreferrer nofollow">UCI 机器学习库</a>。加利福尼亚州尔湾：加利福尼亚大学信息与计算机科学学院。
