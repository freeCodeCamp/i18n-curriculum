...
Якщо натиснути кнопку попередньої пісні, коли жодна пісня не відтворюється, почне грати перша пісня. Щоб це виправити, додайте інструкцію `if` на початку функції `playPreviousSong`. Перевірте, чи поточна пісня є `null`, і зупиніть виконання функції інструкцією `return`.

# --hints--

Ви повинні додати інструкцію `if`, яка перевіряє, чи `userData.currentSong === null`, і зупиняє виконання функції інструкцією `return` всередині вашої функції `playPreviousSong`.
