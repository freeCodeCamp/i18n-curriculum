...
Crie uma variável do CSS personalizada chamada `--penguin-skin` e defina como `gray`.Em seguida, substitua todos os valores relevantes de propriedade por ela.

# --hints--

Você deve dar a `:root` uma propriedade `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

Você deve dar ao `--penguin-skin` um valor `gray`, mas foi encontrado `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

Você deve dar a `.penguin-head` um `background` de `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
Você deve dar ao `.penguin-body::before` uma `background-color` de `var(--penguin-skin)`, mas foi encontrado `--fcc-actual--`.
...
Você deve dar a `.arm` um `background` de `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
