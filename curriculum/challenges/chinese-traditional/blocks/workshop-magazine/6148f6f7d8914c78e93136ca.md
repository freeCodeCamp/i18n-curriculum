...
爲 `only screen` 創建一個最終 `@media` 查詢，其中 `max-width` 爲 `420px`。 在其中，創建一個 `.hero-title` 選擇器，並將 `font-size` 屬性設置爲 `4.5rem`。

恭喜！ 你的雜誌現在已經完成了。

# --hints--

你應該有一個新的 `@media` 查詢，對應 `only screen and (max-width: 420px)`。 這應該是 `@media` 查詢列表中的最後一個查詢。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

你的新 `@media` 查詢應該有一個 `.hero-title` 選擇器。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

你的 `.hero-title` 選擇器應該有一個 `font-size` 屬性，設置爲 `4.5rem`。
