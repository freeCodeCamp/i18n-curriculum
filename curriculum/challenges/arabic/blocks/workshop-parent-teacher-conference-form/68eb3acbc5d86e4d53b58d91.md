...
عيّن `font-size` إلى `1.1rem`، و`display` إلى `block`، و`margin` إلى `auto` لمركزة الزر.

# --hints--

يجب أن تستمر في استهداف صنف `.submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

عيّن خاصية `font-size` إلى `1.1rem`.
