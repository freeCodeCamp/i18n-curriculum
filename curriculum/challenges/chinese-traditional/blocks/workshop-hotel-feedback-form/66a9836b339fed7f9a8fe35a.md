...
對於你的 `textarea` 元素，添加一個 `id` 和 `name` 屬性，值均爲 `"comments"`。

`cols` 屬性值設置爲 `30`，`rows` 屬性值設置爲 `10`。

# --hints--

你的 `textarea` 元素應將 `id` 屬性設置爲 `"comments"`。

```js
assert.exists(document.querySelector('textarea#comments'));
```

你的 `textarea` 元素應將 `name` 屬性設置爲 `"comments"`。

```js
assert.exists(document.querySelector('textarea[name="comments"]'));
```

你的 `textarea` 元素應將 `cols` 屬性設置爲 `"30"`。

```js
assert.exists(document.querySelector('textarea[cols="30"]'));
```

你的 `textarea` 元素應將 `rows` 屬性設置爲 `"10"`。
