...
Aggiungi un attributo `onChange` all'input per catturare ciò che l'utente digita. Impostalo su una funzione freccia che prende `e` come argomento. All'interno della funzione, aggiorna lo stato `query` passando `e.target.value` alla funzione setter, che contiene il valore corrente dell'input. Ecco un esempio:
...
Dovresti assicurarti che il gestore `onChange` sia definito.
...
Il tuo gestore `onChange` dovrebbe avere un parametro `e`.
...
Il tuo gestore `onChange` dovrebbe chiamare `setQuery()` con `e.target.value`.
