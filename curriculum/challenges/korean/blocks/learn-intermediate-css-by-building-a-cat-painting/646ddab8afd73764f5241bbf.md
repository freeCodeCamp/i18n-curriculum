...
왼쪽 내부 귀에서 했던 것처럼, 오른쪽 내부 귀의 날카로운 모서리를 없애려면 아래쪽 오른쪽과 아래쪽 왼쪽 테두리 반경을 `40%`로, 위쪽 왼쪽 테두리 반경을 `90px`로, 위쪽 오른쪽 테두리 반경을 `10px`로 설정하세요.

# --hints--

`.cat-right-inner-ear` 선택자에 `border-bottom-right-radius` 속성을 `40%`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderBottomRightRadius === '40%')
```

`.cat-right-inner-ear` 선택자에 `border-bottom-left-radius` 속성을 `40%`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderBottomLeftRadius === '40%')
```

`.cat-right-inner-ear` 선택자에 `border-top-left-radius` 속성을 `90px`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderTopLeftRadius === '90px')
```

`.cat-right-inner-ear` 선택자에 `border-top-right-radius` 속성을 `10px`로 설정해야 합니다.
