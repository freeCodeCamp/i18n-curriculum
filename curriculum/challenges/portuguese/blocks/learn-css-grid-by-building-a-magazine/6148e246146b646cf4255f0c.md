...
Crie um seletor `.list-title, .list-subtitle` e defina a propriedade `color` como `#00beef`.

# --hints--

Você deve ter um seletor `.list-title, .list-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'))
```

O seletor `.list-title, .list-subtitle` deve ter a propriedade `color` definida como `#00beef`.
