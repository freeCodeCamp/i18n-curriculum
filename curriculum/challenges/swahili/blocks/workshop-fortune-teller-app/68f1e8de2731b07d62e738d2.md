...
Tangaza kigezo kingine kinachoitwa `foundCard`, na kitoe kwa matokeo ya kuita njia ya `find()` kwenye `cards`. Pitia `card` kama kigezo cha wito wa kurudisha, na linganisha `card.name_short` na `cardId` ili kupata kadi inayolingana kwa usawa mkali.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `foundCard` ndani ya `showFortune`.
...
Kigezo chako cha `foundCard` kinapaswa kupewa thamani kwa kuita `find` kwenye `this.cards` na wito wa kurudisha sahihi.
...
Wito wa kurudisha wa `find` unapaswa kuwa na kigezo kinachoitwa `card`.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

Ndani ya wito wa kurudisha, `card.name_short` inapaswa kulinganishwa na `cardId`.
