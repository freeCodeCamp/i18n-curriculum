...
Далі додайте `useEffect` хук, який запускається щоразу, коли змінюється стан `player`. Всередині нього використайте стрілкову функцію та додайте `try..catch` з `error` як параметром блоку `catch`.

# --hints--

Ви повинні викликати `useEffect` всередині `FootballPlayerCard`.
...
Ваш `useEffect` має мати `[player]` як масив залежностей.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

Ваш callback `useEffect` має містити блок `try...catch` з `error` як параметром блоку `catch`.
