...
Infine, dai al tuo `conversionTable` un oggetto `teaspoon` con una proprietà `cup` impostata su `1/48`, una proprietà `gram` impostata su `5` e una proprietà `ounce` impostata su `0.167`.

# --hints--

Il tuo `conversionTable` dovrebbe avere una proprietà `teaspoon`.

```js
assert.property(conversionTable, "teaspoon");
```

La proprietà `teaspoon` dovrebbe essere un oggetto.

```js
assert.isObject(conversionTable.teaspoon);
```

Il tuo oggetto `teaspoon` dovrebbe avere una proprietà `cup`.

```js
assert.property(conversionTable.teaspoon, "cup");
```

La tua proprietà `cup` dovrebbe essere impostata su `1/48`.

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

Il tuo oggetto `teaspoon` dovrebbe avere una proprietà `gram`.

```js
assert.property(conversionTable.teaspoon, "gram");
```

La tua proprietà `gram` dovrebbe essere impostata su `5`.

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

Il tuo oggetto `teaspoon` dovrebbe avere una proprietà `ounce`.

```js
assert.property(conversionTable.teaspoon, "ounce");
```

La tua proprietà `ounce` dovrebbe essere impostata su `0.167`.
