...
Bado ndani ya kizuizi cha `else`, tengeneza mzunguko wa `while` unaothibitisha kuwa `current_node.next` si `None`.

Ndani ya mzunguko wa `while`, weka thamani ya `current_node.next` kwa `current_node`.

# --hints--

Unapaswa kutengeneza mzunguko wa `while` unaothibitisha kama `current_node.next` si `None`.
