...
Dovrai tenere una lista delle unità supportate dalla tua app.

Dichiara un array `units` e assegnagli i valori stringa `"cup"`, `"gram"`, `"ounce"` e `"teaspoon"`.

# --hints--

Dovresti dichiarare una variabile `units`.

```js
assert.isDefined(units);
```

La tua variabile `units` dovrebbe essere un array.

```js
assert.isArray(units);
```

Il tuo array `units` dovrebbe avere quattro elementi.

```js
assert.lengthOf(units, 4);
```

Il tuo array `units` dovrebbe includere la stringa `"cup"`.

```js
assert.include(units, "cup");
```

Il tuo array `units` dovrebbe includere la stringa `"gram"`.

```js
assert.include(units, "gram");
```

Il tuo array `units` dovrebbe includere la stringa `"ounce"`.

```js
assert.include(units, "ounce");
```

Il tuo array `units` dovrebbe includere la stringa `"teaspoon"`.
