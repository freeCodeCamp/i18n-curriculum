...
`allSongs` 배열 안에 다음 속성과 값을 가진 객체를 만드세요:
...
`allSongs` 배열에는 값이 하나 있어야 합니다.

```js
assert.equal(allSongs.length, 1);
```

`allSongs` 배열의 첫 번째 값은 객체여야 합니다.

```js
assert.isObject(allSongs[0]);
```

`allSongs` 배열에는 `id` 속성이 숫자 `0`로 설정된 객체가 있어야 합니다.

```js
assert.equal(allSongs[0].id, 0);
```

`allSongs` 배열에는 `title` 속성이 문자열 `"Scratching The Surface"`로 설정된 객체가 있어야 합니다.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

`allSongs` 배열에는 `artist` 속성이 문자열 `"Quincy Larson"`로 설정된 객체가 있어야 합니다.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

`allSongs` 배열에는 `duration` 속성이 문자열 `"4:25"`로 설정된 객체가 있어야 합니다.

```js
assert.equal(allSongs[0].duration, "4:25");
```

`allSongs` 배열에는 `src` 속성이 문자열 `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`로 설정된 객체가 있어야 합니다.
