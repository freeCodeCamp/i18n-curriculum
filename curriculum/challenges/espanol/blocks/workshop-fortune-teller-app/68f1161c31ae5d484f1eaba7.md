...
Ahora agrega otro método llamado `showFortune`. Como parámetro, pasa `e` y asígnale el tipo `Event`. También crea un método `newReading` y déjalo vacío por ahora.

# --hints--

Debes declarar un método llamado `showFortune`.
...
Tu método `showFortune` debe tener un parámetro `e` con el tipo `Event`.
...
Debes declarar un método llamado `newReading`.
