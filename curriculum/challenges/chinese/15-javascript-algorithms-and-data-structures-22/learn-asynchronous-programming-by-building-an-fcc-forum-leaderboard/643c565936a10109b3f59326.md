...
当你构建 `showLatestPosts()` 函数时，你需要调用它来查看你的更改。

在 `try` 块的末尾调用 `showLatestPosts()` 函数，并传入 `data` 作为参数。

# --hints--

你应该在 `try` 代码块的末尾调用 `showLatestPosts()`。

```js
assert.match(code, /\s*showLatestPosts\(.*\)\s*;?\s*/);
```

你应该将 `data` 作为参数传递给 `showLatestsPosts()`。
