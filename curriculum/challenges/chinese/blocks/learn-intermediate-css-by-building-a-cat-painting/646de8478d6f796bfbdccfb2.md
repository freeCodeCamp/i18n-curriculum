...
为了使右眼看起来像一只眼睛，给它设置一个 `60%` 的边框半径。 同时，使用 `transform` 属性，将其旋转 `-25` 度。

# --hints--

你的 `.cat-right-eye` 选择器应将 `border-radius` 属性为 `60%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.borderRadius === '60%')
```

你的 `.cat-right-eye` 选择器应将 `transform` 属性为 `rotate(-25deg)`。
