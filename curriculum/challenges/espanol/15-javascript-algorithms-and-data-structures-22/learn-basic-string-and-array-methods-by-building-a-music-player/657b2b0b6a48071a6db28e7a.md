...
El método `map()` se utiliza para iterar a través de un array y devolver un nuevo array. Es útil cuando deseas crear un nuevo array basado en los valores de un array existente. Por ejemplo:
...
Observe que el método `map()` toma una función como argumento. Esto se llama una <dfn>función de devolución de llamada</dfn>, que es una función que se pasa a otra función como argumento. En el ejemplo anterior, la función de devolución de llamada es `(number) => number * 2`, y se ejecuta en cada elemento del array `numbers`. El método `map()` luego devuelve un nuevo array con los resultados.

Pasa una función de devolución de llamada al método `map()`. La función de devolución de llamada debe tomar `song` como parámetro, usar la sintaxis de función flecha y tener un cuerpo vacío.

# --hints--

La función de devolución de llamada de tu método `map()` debe usar la sintaxis de función flecha.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

Tu método `map()` debe tener un parámetro `song` en su función de devolución de llamada.
