...
Setzen Sie die Hintergrundfarbe von `footer` auf `#2a2a40` und verwenden Sie _Flexbox_, um den Text horizontal zu zentrieren.

# --hints--

Sie sollten den `footer`-Elementselektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

Sie sollten `footer` eine `background-color` von `#2a2a40` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

Sie sollten `footer` eine `display` von `flex` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Sie sollten `footer` eine `justify-content` von `center` geben.
