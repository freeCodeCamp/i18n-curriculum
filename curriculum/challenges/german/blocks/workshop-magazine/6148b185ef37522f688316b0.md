...
Jetzt können Sie den Inhalt des `.heading`-Elements mit CSS Grid gestalten.

Die CSS-`repeat()`-Funktion wird verwendet, um einen Wert zu wiederholen, anstatt ihn manuell auszuschreiben, und ist hilfreich für Grid-Layouts. Zum Beispiel würde das Setzen der `grid-template-columns`-Eigenschaft auf `repeat(20, 200px)` 20 Spalten erzeugen, die jeweils `200px` breit sind.

Geben Sie Ihrem `.heading`-Element eine `grid-template-columns`-Eigenschaft mit dem Wert `repeat(2, 1fr)`, um zwei Spalten gleicher Breite zu erstellen.

# --hints--

Ihr `.heading`-Selektor sollte eine `grid-template-columns`-Eigenschaft mit dem Wert `repeat(2, 1fr)` haben.
