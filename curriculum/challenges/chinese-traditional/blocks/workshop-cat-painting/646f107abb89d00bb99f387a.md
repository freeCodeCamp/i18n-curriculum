...
使用子孫選擇器來標的你 `.cat-whiskers-left` 元素內的三個 `div` 元素。為它設定 `width` 為 `40px`、`height` 為 `1px`，以及 `background-color` 為 `#000`。

# --hints--

你應該有一個 `.cat-whiskers-left div` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div'));
```

你的 `.cat-whiskers-left div` 選擇器應該將 `width` 設定為 `40px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.width, '40px');
```

你的 `.cat-whiskers-left div` 選擇器應該將 `height` 設定為 `1px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.height, '1px');
```

你的 `.cat-whiskers-left div` 選擇器應該設定 `background-color` 為 `#000`。
