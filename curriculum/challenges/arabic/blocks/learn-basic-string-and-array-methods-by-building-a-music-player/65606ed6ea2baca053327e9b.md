...
أضف كائنًا ثالثًا بالخصائص والقيم التالية:
...
يجب أن تحتوي مصفوفة `allSongs` على كائن كقيمة ثالثة.

```js
assert.isObject(allSongs[2]);
```

يجب أن يحتوي الكائن الثالث في مصفوفة `allSongs` على خاصية `id` بقيمة الرقم `2`.

```js
assert.equal(allSongs[2].id, 2);
```

يجب أن يحتوي الكائن الثالث في مصفوفة `allSongs` على خاصية `title` بقيمة السلسلة النصية `"Still Learning"`.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

يجب أن يحتوي الكائن الثالث في مصفوفة `allSongs` على خاصية `artist` بقيمة السلسلة النصية `"Quincy Larson"`.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

يجب أن يحتوي الكائن الثالث في مصفوفة `allSongs` على خاصية `duration` بقيمة السلسلة النصية `"3:51"`.

```js
assert.equal(allSongs[2].duration, "3:51");
```

يجب أن يحتوي الكائن الثالث في مصفوفة `allSongs` على خاصية `src` بقيمة السلسلة النصية `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`.
