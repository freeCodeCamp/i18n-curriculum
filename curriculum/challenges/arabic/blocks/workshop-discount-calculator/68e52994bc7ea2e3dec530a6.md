...
حان الوقت لاختبار نظام الخصم الكامل! أضف جملة `if` تتحقق مما إذا كان `__name__ == '__main__'` وانقل `product = Product('Wireless Mouse', 50.0)` داخلها.

بعد ذلك، داخل كتلة `if`، أنشئ متغيرًا باسم `user_tier` وضعه مساويًا للسلسلة النصية `Premium`. ثم أنشئ متغيرًا باسم `strategies` وضعه مساويًا لقائمة من الاستراتيجيات التي تحتوي على `PercentageDiscount(10)` و`FixedAmountDiscount(5)` و`PremiumUserDiscount()`.

# --hints--

يجب أن يكون لديك جملة `if` تتحقق مما إذا كان `__name__ == '__main__'`.
...
يجب أن تنقل `product = Product('Wireless Mouse', 50.0)` داخل كتلة `if`.
...
يجب أن تضبط `user_tier` على السلسلة النصية `Premium` داخل كتلة `if`.
...
يجب أن تنشئ قائمة باسم `strategies` تحتوي على `PercentageDiscount(10)` و`FixedAmountDiscount(5)` و`PremiumUserDiscount()`.
