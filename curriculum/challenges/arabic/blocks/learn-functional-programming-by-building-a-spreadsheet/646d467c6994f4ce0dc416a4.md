...
استخدم المعامل الثلاثي لتحويل استدعاء `.hasOwnProperty()` إلى شرط. إذا كان الكائن يحتوي على الخاصية، أعد نتيجة استدعاء `apply` مع `fn` و `args` كمعلمات. وإلا، أعد `match`.

# --hints--

يجب أن تستخدم دالة الاستدعاء الراجع صيغة نحوية ثلاثية.
...
إذا كان شرط الثلاثي صحيحًا، يجب أن تعيد دالة الاستدعاء الراجع نتيجة استدعاء `apply()`.
...
إذا كان الثلاثي خاطئًا، يجب أن تعيد `match`.
