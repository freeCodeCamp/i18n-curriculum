...
Unapobofya wimbo wa orodha yako ya nyimbo unataka kuupiga wimbo huo. Kwa hiyo utahitaji kuongeza msikilizaji wa tukio la `click` kwenye kila kipengele cha `button` ambacho ni mtoto wa kipengele cha `song`.

Kamilisha wito wa kurudisha wa `forEach` kufanya hivyo. Ndani ya wito wa kurudisha wa `addEventListener`, fanya wito wa `playSong(n)`, ambapo `n` ni kitambulisho cha wimbo ambacho unaweza kupata kutoka kwenye sifa ya kipengele ya `id` ya `song`.

Kumbuka kwamba kila `song` ina sifa ya kipengele ya `id` ya `song-n`: utahitaji kupata sehemu ya `n` na kuibadilisha kuwa nambari kabla ya kuipatia `playSong()`.


# --hints--

Unapaswa kuongeza msikilizaji wa tukio la `click` kwenye kila kipengele cha `button` ambacho ni mtoto wa kipengele cha `.playlist-song` na fanya wito wa `playSong` kwa `id` ya wimbo husika.
