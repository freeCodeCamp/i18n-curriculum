...
Tu selector `h1` debe tener una propiedad `max-width` establecida en `37.25rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1')?.getPropertyValue('max-width') === '37.25rem');
```

Tu selector `h1` debe tener una propiedad `margin` establecida en `0 auto`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1')?.getPropertyValue('margin') === '0px auto');
```

Tu selector `h1` debe tener una propiedad `padding` establecida en `1.5rem 1.25rem`.
