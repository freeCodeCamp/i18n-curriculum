...
Il passaggio successivo è iterare sull'array `posters` per ottenere tutti i loro avatar.

Inizia aggiungendo una parola chiave `return` seguita da `posters.map()`. Per la funzione di callback, aggiungi un parametro chiamato `poster`.

# --hints--

Dovresti restituire `posters.map()`.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

La tua funzione di callback dovrebbe avere un parametro chiamato `poster`.
