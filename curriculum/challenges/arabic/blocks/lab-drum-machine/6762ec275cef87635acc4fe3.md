...
1. يجب أن تحتوي على عنصر `div` يحمل `id` باسم `drum-machine` يحتوي على جميع العناصر الأخرى.

2. داخل عنصر `#drum-machine` يجب أن يكون لديك عنصر `div` آخر يحمل `id` باسم `pad-bank`.

3. داخل عنصر `#drum-machine` يجب أن يكون لديك عنصر `p` يحمل `id` باسم `display`.

4. داخل عنصر `#pad-bank` يجب أن يكون لديك تسعة عناصر طبلة قابلة للنقر، كل منها يحمل صنف `drum-pad`، و`id` فريد يصف مقطع الصوت الذي ستُفعّل به طبلة الطبل، ونص داخلي يتوافق مع أحد المفاتيح التالية على لوحة المفاتيح: `Q`، `W`، `E`، `A`، `S`، `D`، `Z`، `X`، `C`. يجب أن تكون طبول الطبل بهذا الترتيب بالضبط.

5. يجب أن يحتوي كل عنصر `.drum-pad` على عنصر `audio` يحمل صنف `clip`، ويمتلك خاصية `src` تشير إلى مقطع صوتي، و`id` يتطابق مع النص الداخلي لعنصر `.drum-pad` الأب (مثلًا `id="Q"`، `id="W"`، `id="E"`، وهكذا).

6. عند النقر على عنصر `.drum-pad`، يجب تشغيل مقطع الصوت الموجود في عنصر الصوت الفرعي الخاص به.

7. عند الضغط على مفتاح التفعيل المرتبط بكل `.drum-pad`، يجب تشغيل مقطع الصوت الموجود في عنصر الصوت الفرعي الخاص به (مثلاً الضغط على مفتاح `Q` يجب أن يُفعّل لوحة الطبول التي تحتوي على السلسلة النصية `Q`، والضغط على مفتاح `W` يجب أن يُفعّل لوحة الطبول التي تحتوي على السلسلة النصية `W`، وهكذا).

8. عند تفعيل `.drum-pad`، يجب عرض سلسلة نصية تصف مقطع الصوت المرتبط كنص داخلي لعنصر `#display` (يجب أن تكون كل سلسلة فريدة).

يمكنك استخدام بعض عينات الصوت لآلة الطبول الخاصة بك من العنوان `https://cdn.freecodecamp.org/curriculum/drum/<fileName>`، حيث `<fileName>` كما يلي:
...
يجب أن تحتوي على عنصر `div` يحمل `id` باسم `drum-machine` يحتوي على جميع العناصر الأخرى.

```js
assert.isNotNull(document.querySelector('div#drum-machine'));
```

داخل عنصر `#drum-machine` يجب أن يكون لديك عنصر `div` آخر يحمل `id` باسم `pad-bank`.

```js
assert.isNotNull(document.querySelector('#drum-machine div#pad-bank'));
```

داخل العنصر `#drum-machine` يجب أن يكون لديك عنصر `p` يحمل `id` بقيمة `display`.

```js
assert.isNotNull(document.querySelector('#drum-machine p#display'));
```

داخل عنصر `#pad-bank` يجب أن يكون لديك تسعة عناصر لوحة طبول قابلة للنقر، كل منها بصنف `drum-pad`.

```js
assert.lengthOf(document.querySelectorAll('#pad-bank .drum-pad'), 9);
```

يجب أن يكون كل `.drum-pad` عنصر `button`.
...
يجب أن يحتوي كل عنصر من عناصر `.drum-pad` على واحدة من الحروف التالية كـ `innerText`، بالترتيب: `Q`، `W`، `E`، `A`، `S`، `D`، `Z`، `X`، `C`.
...
يجب أن يحتوي كل `.drum-pad` على عنصر `audio` يحمل صنف `clip`، ويمتلك خاصية `src` تشير إلى مقطع صوتي، و`id` يتطابق مع النص الداخلي لعنصر `.drum-pad` الأب (مثلًا `id="Q"`، `id="W"`، `id="E"`، وهكذا).
...
عند النقر على عنصر `.drum-pad`، يجب تشغيل مقطع الصوت الموجود في عنصر الصوت الفرعي الخاص به.
...
عند الضغط على أحد المفاتيح Q، W، E، A، S، D، Z، X، C على لوحة المفاتيح، يجب أن يشغل عنصر `audio` المقابل الصوت المناسب.
...
عند تفعيل `.drum-pad`، يجب عرض سلسلة نصية تصف مقطع الصوت المرتبط كنص داخلي لعنصر `#display` (يجب أن تكون كل سلسلة فريدة).
