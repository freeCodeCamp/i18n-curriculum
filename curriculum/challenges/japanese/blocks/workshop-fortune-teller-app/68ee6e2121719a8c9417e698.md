...
`multipleCard` をクリアした直後に、`singleCardSelected` メソッド内で `showElements` 関数を呼び出し、`singleCard` を最初の引数として、`fortuneContainer` を2番目の引数として渡してください。

# --hints--

`showElements` 関数を呼び出す必要があります。

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

`elements.singleCard` を最初の引数として渡す必要があります。

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

`elements.fortuneContainer` を2番目の引数として渡す必要があります。
