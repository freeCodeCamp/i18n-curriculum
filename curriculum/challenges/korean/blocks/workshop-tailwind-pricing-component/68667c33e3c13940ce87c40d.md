...
지속 시간이 있는 `span`에도 스타일을 적용해야 하므로 `text-base`, `font-medium`, `text-fuchsia-300` 클래스를 추가하세요.

# --hints--

`span` 요소에 `text-base` 클래스가 있어야 합니다.
...
`span` 요소에 `font-medium` 클래스가 있어야 합니다.
...
`span` 요소에 `text-fuchsia-300` 클래스가 있어야 합니다.
