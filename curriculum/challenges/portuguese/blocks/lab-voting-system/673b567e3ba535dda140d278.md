...
`addOption("Egypt")` deve retornar `Option "Egypt" added to the poll.`
...
Adicionar uma opção vazia deve retornar `"Option cannot be empty."`
...
Quando `Nigeria` não estiver nas opções de votação, `vote("Nigeria", "traveler2")` deve retornar `Option "Nigeria" does not exist.`
