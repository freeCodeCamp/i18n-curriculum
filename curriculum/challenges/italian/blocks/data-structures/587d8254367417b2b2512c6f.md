...
In questo esercizio, eseguiremo un test di sottoinsieme su 2 insiemi di dati. Creeremo un metodo sulla nostra struttura dati `Set` chiamato `isSubsetOf`. Questo confronterà il primo insieme con il secondo e, se il primo insieme è completamente contenuto nel secondo, restituirà `true`.

Per esempio, se `setA = ['a','b']` e `setB = ['a','b','c','d']`, allora `setA` è un sottoinsieme di `setB`, quindi `setA.isSubsetOf(setB)` dovrebbe restituire `true`.
