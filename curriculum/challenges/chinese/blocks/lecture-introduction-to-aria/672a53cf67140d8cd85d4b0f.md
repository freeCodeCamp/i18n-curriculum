...
ARIA 角色定义了网站或网络应用中元素的用途。下面是将 `div` 元素的角色设置为 `button` 的示例。

```html
<div role="button">Click Me</div>
```

这样做是向辅助技术表明该元素是一个按钮。然而，角色本身不提供任何功能性。仅仅给这个 `div` 设置 `role` 为 `button` 并不会让它表现得像按钮。要让它看起来并行为像按钮，你需要使用 CSS 和 JavaScript 来实现预期效果。

这是一个使用超文本标记语言、CSS 和 JavaScript 创建自定义 `button` 元素的示例。

**注意**：不用担心尝试理解 CSS 和 JavaScript 代码。你将在未来的模块中学习这些语言。
...
只有在 HTML 无法满足需求时才使用 WAI-ARIA，别忘了用屏幕阅读器等辅助技术测试，或者让有残疾的人测试你的作品。同时，确保你的 WAI-ARIA 状态和属性能实时随着内容更新。避免过度使用 ARIA，因为这通常会让人感到困惑。
