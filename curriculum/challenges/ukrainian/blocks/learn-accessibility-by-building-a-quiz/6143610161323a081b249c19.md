...
Заповніть вміст тесту, нижче `#student-info`, додайте три `div` елементи з `class` рівним `info`.

Потім у кожен `div` вкладіть по одному `label` елементу та одному `input` елементу.

# --hints--

Ви повинні вкласти три `div` елементи під `h2#student-info` елементом.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.length, 3);
```

Першому `div` слід надати `class` зі значенням `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.className, 'info');
```

Другому `div` слід надати `class` зі значенням `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.className, 'info');
```

Третьому `div` слід надати `class` зі значенням `info`.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.className, 'info');
```

В першому `label` слід вкласти один `div` елемент.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.querySelectorAll('label')?.length, 1);
```

В першому `input`, після `div`, слід вкласти один `label` елемент.
...
В другому `label` слід вкласти один `div` елемент.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.querySelectorAll('label')?.length, 1);
```

В другому `input`, після `div`, слід вкласти один `label` елемент.
...
В третьому `label` слід вкласти один `div` елемент.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[2]?.querySelectorAll('label')?.length, 1);
```

В третьому `input`, після `div`, слід вкласти один `label` елемент.
