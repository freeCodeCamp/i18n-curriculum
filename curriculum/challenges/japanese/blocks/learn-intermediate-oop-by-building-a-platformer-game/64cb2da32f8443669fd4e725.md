...
次のステップは、`width` と `height` を `CheckPoint` クラスに追加することです。

`width` と `height` はそれぞれ `proportionalSize(40)` と `proportionalSize(70)` にする必要があります。

# --hints--

`width` クラスの中に `CheckPoint` プロパティがあるはずです。
...
`height` クラスの中に `CheckPoint` プロパティがあるはずです。
...
`width` プロパティに `proportionalSize(40)` を代入する必要があります。
...
`height` プロパティに `proportionalSize(70)` を代入する必要があります。
