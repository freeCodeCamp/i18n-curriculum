...
選擇 `class` 爲 `left` 的 `.foot` 元素，將其放置在父元素左側 `25%` 的位置。 然後，選擇 `class` 爲 `right` 的 `.foot` 元素，將其放置在父元素右側 `25%` 的位置。
...
你應該將 `.foot.left` 的 `left` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
...
你應該將 `.foot.right` 的 `right` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
