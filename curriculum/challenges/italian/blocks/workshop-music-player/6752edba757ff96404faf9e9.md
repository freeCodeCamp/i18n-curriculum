...
Crea una funzione chiamata `setPlayButtonAccessibleText`. All'interno della funzione, imposta l'attributo `aria-label` del pulsante di riproduzione su `Play` seguito da uno spazio e dal titolo della canzone attuale. Se `userData.currentSong` è `null`, impostalo su `Play`.

# --hints--

Dovresti avere una funzione chiamata `setPlayButtonAccessibleText`.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

La tua funzione `setPlayButtonAccessibleText` dovrebbe impostare l'attributo `aria-label` del pulsante di riproduzione su `Play` seguito da uno spazio e dal titolo della canzone attuale.
...
```
 ...
 Quando `userData.currentSong` è `null`, `setPlayButtonAccessibleText` dovrebbe impostare l'attributo `aria-label` del pulsante di riproduzione su `Play`.
