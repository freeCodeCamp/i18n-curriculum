...
En pasos anteriores, aprendiste a trabajar con el método `sort()` para ordenar las canciones en orden alfabético. Otro caso de uso para la función de callback es aleatorizar un array. 

Una forma de aleatorizar un array de elementos sería restar `0.5` de `Math.random()`, lo que produce valores aleatorios que pueden ser positivos o negativos. Esto hace que el resultado de la comparación sea una mezcla de valores positivos y negativos, llevando a un ordenamiento aleatorio de elementos.
...
Utiliza el método `sort()` en el array `userData?.songs`. Pasa un callback al método, y devuelve el resultado de `Math.random() - 0.5`.


# --hints--

No debes modificar la función existente `shuffle`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

Debes usar el método `sort()` en `userData?.songs`.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

Tu método `sort()` debe tener una función de callback que use la sintaxis de flecha.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

El callback de tu método `sort()` debe devolver `Math.random() - 0.5`.
