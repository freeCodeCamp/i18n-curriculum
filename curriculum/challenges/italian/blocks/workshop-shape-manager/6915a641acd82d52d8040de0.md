...
Tornando alla funzione `initializeApp`, qui metterai in uso i tipi in linea e la funzione `getElement()` che hai creato prima, ottenendo gli elementi DOM necessari per l'app.

All'interno della funzione `initializeApp`, riassegna `shapeTypeSelect` chiamando `getElement()` con `"shape-type"`, poi usa la parola chiave `as` per convertire il risultato in `HTMLSelectElement`.

# --hints--

Dovresti riassegnare `shapeTypeSelect` con il valore appropriato.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

Dovresti chiamare la funzione `getElement` con `shape-type` e assegnarla a `shapeTypeSelect`.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

Dovresti chiamare `getElement` con `HTMLSelectElement` come tipo di `shapeTypeSelect` usando la parola chiave `as`.
