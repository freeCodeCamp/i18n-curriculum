...
Sie benötigen eine Möglichkeit, auf die Gesamtanzahl der Artikel im Warenkorb zuzugreifen. Der beste Weg dafür ist, eine weitere Methode zu Ihrer `ShoppingCart`-Klasse hinzuzufügen, anstatt direkt auf das `items`-Array zuzugreifen.

Fügen Sie der `ShoppingCart`-Klasse eine `getCounts`-Methode hinzu. Sie soll die Anzahl der Artikel im `items`-Array zurückgeben.

# --hints--

Ihre `ShoppingCart`-Klasse sollte eine `getCounts`-Methode haben.
...
Ihre `getCounts`-Methode sollte die Anzahl der Artikel im `items`-Array zurückgeben.
