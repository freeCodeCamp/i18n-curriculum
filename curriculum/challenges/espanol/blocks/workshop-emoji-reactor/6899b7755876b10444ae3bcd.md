...
Ahora es momento de enlazar el archivo JavaScript.

Agrega un elemento `script` justo antes de la etiqueta de cierre del elemento `body` con el atributo y valor correctos para enlazar el archivo `script.js`.

# --hints--

Debes tener un elemento `script`.

```js
assert.match(code, /<script[^>]*>\s*<\/script>/);
```

El elemento `script` debe tener un `src` de `./script.js`.
