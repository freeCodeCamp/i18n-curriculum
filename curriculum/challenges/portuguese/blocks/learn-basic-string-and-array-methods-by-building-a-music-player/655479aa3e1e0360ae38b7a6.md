...
Em seguida, você precisará recuperar a próxima música na playlist. Para isso, você precisará obter o índice da música atual e então adicionar `1` a ele.

Crie uma constante chamada `nextSong` e atribua `userData?.songs[currentSongIndex + 1]` a ela.

Por fim, chame a função `playSong` e passe `nextSong.id` como argumento.

# --hints--

Você não deve modificar a declaração `if` existente, seu bloco `else` e o conteúdo.
...
Você deve atribuir `userData?.songs[currentSongIndex + 1]` a uma constante `nextSong`.
...
Você deve chamar a função `playSong` com `nextSong.id`.
