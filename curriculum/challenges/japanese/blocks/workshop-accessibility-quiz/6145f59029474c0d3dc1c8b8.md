...
他の`input`および`label`要素と同様に、`textarea`を対応する`label`要素にリンクし、`name`属性を付けてください。

# --hints--

`textarea`要素に`id`属性を付ける必要があります。

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id);
```

`id`要素の`textarea`にはスペースを含めることはできません。

```js
assert.match(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id, /^\S+$/);
```

`id`要素には別の要素がその`textarea`を使用しているため、異なる`id`を選ぶ必要があります。
...
`label`要素の`for`に一致する`id`属性を`textarea`要素に付けるべきです。
