...
包含剩余的参数 `chosenCard.name_short` 和 `chosenCard.img`。然后在 `renderCard` 函数外，将 `multipleCard` 元素的 `innerHTML` 重置为空字串。

# --hints--

你应该将 `chosenCard.name_short` 作为参数传递给 `renderCard`。
...
你应该将 `chosenCard.img` 作为最后一个参数传递给 `renderCard`。
...
你应该将 `this.elements.multipleCard.innerHTML` 重置为空字串。
