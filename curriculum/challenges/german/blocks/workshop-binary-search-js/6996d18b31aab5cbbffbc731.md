...
Anstatt also nur `pathToTarget` in der `if`-Anweisung zurückzugeben, fügen Sie einen Template-Literal mit der Nachricht `Value found at index ${mid}` hinzu.

Damit ist Ihr Workshop zum binären Suchalgorithmus abgeschlossen!

# --hints--

Sie sollten die `pathToTarget, \`Value found at index ${mid}\`` inside the `if`-Anweisung zurückgeben.
