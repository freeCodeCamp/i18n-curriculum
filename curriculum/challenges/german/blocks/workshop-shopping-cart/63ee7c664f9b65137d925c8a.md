...
Ihre `ShoppingCart`-Klasse benötigt die Fähigkeit, Elemente hinzuzufügen. Erstellen Sie eine leere `addItem`-Methode, die zwei Parameter entgegennimmt: `id` und `products`. Das Erstellen einer Methode könnte so aussehen:
...
Der erste Parameter, `id`, ist der `id` des Produkts, das der Benutzer zu seinem Warenkorb hinzugefügt hat. Der zweite Parameter, `products`, ist ein Array von `Dessert`-Instanzen. Indem Sie einen Parameter verwenden, anstatt direkt auf Ihr bestehendes `products`-Array zuzugreifen, wird diese Methode flexibler, falls Sie in Zukunft weitere Produktlisten hinzufügen möchten.

# --hints--

Ihre `ShoppingCart`-Klasse sollte eine `addItem`-Methode haben.
...
Ihre `addItem`-Methode sollte zwei Parameter entgegennehmen: `id` und `products`.
...
Ihre `addItem`-Methode sollte leer sein.
