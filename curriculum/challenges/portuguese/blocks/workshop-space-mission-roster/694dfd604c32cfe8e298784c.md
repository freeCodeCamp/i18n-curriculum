...
O controle da missão solicitou que você troque as posições de dois astronautas no seu `squad` array, especificamente os membros nos índices `2` e `5`. Use sua `swapCrewMembers()` função para realizar a troca e armazene o resultado em uma nova variável chamada `updatedSquad`.

# --hints--

Você deve usar `swapCrewMembers()` para trocar os membros do `squad` nos índices `2` e `5` e armazenar o array resultante em uma nova variável chamada `updatedSquad`.
