...
حان الوقت للرسم البياني. أنشئ طريقة `create_trajectory` واستبدل آخر استدعاء `print` في أسفل شفرتك بـ `print(graph.create_trajectory())`.

كخطوة أولى في هذه الطريقة الجديدة، أنشئ نسخة محلية من الإحداثيات بحيث تكون كل القيم مقربة إلى أعداد صحيحة. احفظ هذا الإصدار الجديد من الإحداثيات في متغير اسمه `rounded_coords`، وأرجع هذا المتغير.

# --hints--

يجب أن تعرف طريقة باسم `create_trajectory`.
...
يجب أن تستبدل `print(graph.create_coordinates_table())` بـ `print(graph.create_trajectory())`.
...
يجب أن تُرجع الدالة `rounded_coords`.
...
يجب أن يحتوي المتغير `rounded_coords` على الإحداثيات المقربة.
