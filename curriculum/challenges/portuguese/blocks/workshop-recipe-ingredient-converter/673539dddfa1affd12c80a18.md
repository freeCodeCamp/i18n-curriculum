...
Você precisará testar sua nova função curried. Declare uma variável `servingsResult` e atribua a ela o resultado do ajuste de uma quantidade base de 4 para uma porção de 6.

Registre essa variável no console.

# --hints--

Você deve declarar uma variável `servingsResult`.

```js
assert.isDefined(servingsResult);
```

Sua variável `servingsResult` deve conter o resultado da chamada `adjustQuantity(4)(6)`.

```js
assert.strictEqual(servingsResult, 24);
```

Você deve registrar o valor de `servingsResult`.
