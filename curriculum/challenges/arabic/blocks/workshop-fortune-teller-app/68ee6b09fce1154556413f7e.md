...
داخل طريقة `initializeEventListeners`، أرفق مستمع حدث `click` إلى `singleCardBtn`. ضمن معالِج الحدث، نفذ استدعاء لطريقة `singleCardSelected`. تذكر أن تحصل على `singleCardBtn` من `elements`.
