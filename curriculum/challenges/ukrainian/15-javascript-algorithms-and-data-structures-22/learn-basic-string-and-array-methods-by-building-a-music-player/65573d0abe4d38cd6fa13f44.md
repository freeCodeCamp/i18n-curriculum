...
Уся основна функціональність вже доступна. Єдина проблема зараз полягає в тому, що наступна пісня не відтворюється автоматично, коли закінчується поточна пісня.

Щоб виправити це, ви можете налаштувати слухач подій, який виявить, коли закінчується поточна пісня. Слухач подій `"ended"` підходить для цього. Він запускається, коли відтворення медіафайлу досягає кінця.

Додайте слухач подій до елемента `audio`, який слухає по подію `"ended"`. Передайте зворотний виклик використовуючи стрілковий синтаксис з порожніми фігурними дужками.

# --hints--

Під’єднайте метод `addEventListener()` до змінній `audio`.

```js
assert.match(code, /audio\.addEventListener\(/)
```

Слухач подій, який ви використовували на змінній `audio`, має слухати по подію `"ended"`.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1/)
```

Використайте стрілковий синтаксис, щоб передати порожній зворотний виклик до слухача подій `"ended"`.
