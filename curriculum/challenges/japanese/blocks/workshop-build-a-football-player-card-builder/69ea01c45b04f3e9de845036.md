...
現在の問題は、プレビューが更新されたりページをリロードしたりすると、プレイヤーカードに追加した内容がすべて消えてしまうことです。

すべての入力を `localStorage` に保存すれば、これを防げます。まずは、文字列 `STORAGE_KEY` を値に持つ `football_player_card` 定数を作成してください。

# --hints--

`STORAGE_KEY` 変数を宣言する必要があります。
...
`STORAGE_KEY` は文字列 `football_player_card` を値に持つべきです。
