...
最后，添加一个 `else` 子句来打印允许的移动并调用 `make_allowed_move`。 尝试找出正确的参数。
...
你应该在 `else` 块内有 `print(f'Move {i + 1} allowed between {source} and {target}')`。
...
你应该在 `else` 块内有 `make_allowed_move(source, target)`。
