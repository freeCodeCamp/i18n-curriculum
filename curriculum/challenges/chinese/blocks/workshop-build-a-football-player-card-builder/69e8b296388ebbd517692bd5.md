...
在 DRI `form-group` 之后，添加两个 `div` 元素，每个元素内部在 `stats-grid` 中包含 `form-group` 的 `className`，一个用于 DEF，另一个用于 PHY。

对于 DEF，添加一个带有 `def` 类型的 `label`，文本为 `DEF`，然后添加一个具有 `def` 类型和 `number` 属性的输入框。

对于 PHY，添加一个带有 `phy` 类型的标签，文本为 `PHY`，然后添加一个具有 `phy` 类型和 `number` 属性的输入框。

# --hints--

你应该添加另一组两个 `div` 元素，每个元素内部在 `div` 的 `className` 中包含 `form-group` 的 `className`，这些都在 `stats-grid` 中。
...
你的第五个 `div` 元素，类型为 `className`，位于 `form-group` 中，应包含一个带有 `htmlFor` 属性为 `def` 的 `label`，一个类型为 `label` 的 `className`，以及文本 `DEF`。
...
你的第五个 `div` 元素，类型为 `className`，位于 `form-group` 中，应包含一个带有 `className` 类型的 `input`，一个 `id` 为 `def`，以及一个 `type` 为 `number`。
...
你的第六个 `div` 元素，类型为 `className`，位于 `form-group` 中，应包含一个带有 `htmlFor` 属性为 `phy` 的 `label`，一个类型为 `label` 的 `className`，以及文本 `PHY`。
...
你的第四个 `div` 元素，类型为 `className`，位于 `form-group` 中，应包含一个带有 `className` 类型的 `input`，一个 `id` 为 `phy`，以及一个 `type` 为 `number`。
