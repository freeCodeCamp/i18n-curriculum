...
现在你需要创建你的第一个柯里的函数。记住，柯里的函数是返回另一个函数的函数，示例如下：
...
首先声明一个 `convertQuantity` 函数。它应有一个单一的 `fromUnit` 参数，并且应为空。

# --hints--

你应该声明一个 `convertQuantity` 函数。

```js
assert.isFunction(convertQuantity)
```

你的 `convertQuantity` 函数应有一个 `fromUnit` 参数。

```js
assert.match(convertQuantity?.toString(), /\(fromUnit\)/);
```

你的 `convertQuantity` 函数应为空。
