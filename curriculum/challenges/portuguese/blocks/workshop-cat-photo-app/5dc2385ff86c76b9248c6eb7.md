...
O elemento `main` deve ter uma tag de abertura. Opening tags have this syntax: `<elementName>`.

```js
assert.exists(document.querySelector('main'));
```

O elemento `main` deve ter uma tag de fechamento. Closing tags have a `/` just after the `<` character.
