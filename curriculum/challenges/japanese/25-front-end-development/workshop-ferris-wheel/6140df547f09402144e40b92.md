...
`@keyframes` ルールを CSS セレクターにリンクするには、`animation-name` プロパティを使用します。 このプロパティの値と `@keyframes` ルールの名前を一致させる必要があります。 `.wheel` セレクターの `animation-name` プロパティを `wheel` に設定してください。
