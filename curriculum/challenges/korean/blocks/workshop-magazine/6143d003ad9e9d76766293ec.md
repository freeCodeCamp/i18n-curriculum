...
`.image-quote` 요소 안에 `hr` 요소, `p` 요소, 그리고 두 번째 `hr` 요소를 중첩하세요. `p` 요소에 `class`를 `quote`로 설정하고 텍스트는 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`로 지정하세요.

# --hints--

`hr` 요소에 `.image-quote` 요소 두 개를 추가하세요.

```js
assert.lengthOf(document.querySelectorAll('.image-quote hr'), 2);
```

`p` 요소에 `.image-quote` 요소를 추가하세요.

```js
assert.lengthOf(document.querySelectorAll('.image-quote p'), 1);
```

`.image-quote` 자식 요소들이 올바른 순서에 있어야 합니다.
...
새로운 `p` 요소에 `class`를 `quote`로 설정하세요.

```js
assert.isTrue(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

새로운 `p` 요소에 텍스트 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`를 넣으세요.
