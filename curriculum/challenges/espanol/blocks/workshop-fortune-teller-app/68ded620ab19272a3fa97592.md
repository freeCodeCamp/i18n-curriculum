...
Como el proyecto ahora trabaja con elementos del DOM HTML, necesitas una forma de mostrar la fortuna en el DOM.

Declara una función llamada `renderCard` que tome cuatro argumentos: `drawingType`, `isReversed`, `shortName` y `img`, y que retorne un literal de plantilla.

Los parámetros `drawingType`, `shortName` y `img` deben ser del tipo `string`, mientras que `isReversed` debe ser del tipo `boolean`. La función debe tener un tipo de retorno `string`.

# --hints--

Debes tener una función llamada `renderCard`.
...
Tu función `renderCard` debe tener cuatro parámetros.
...
Tu primer parámetro debe ser `drawingType` del tipo `string`.
...
Tu segundo parámetro debe ser `isReversed` del tipo `boolean`.
...
Tu tercer parámetro debe ser `shortName` del tipo `string`.
...
Tu cuarto parámetro debe ser `img` del tipo `string`.
...
Tu función `renderCard` debe tener un tipo de retorno `string`.
...
Tu función `renderCard` debe devolver un template literal.
