...
Nel tuo `sortInputArray` devi ottenere i valori dai tuoi elementi `select`. Siccome hanno tutti la classe `values-dropdown`, puoi interrogarli tutti insieme.

Usa `document.getElementsByClassName()` per ottenere tutti gli elementi con questa classe passando l'argomento `"values-dropdown"`. Assegna questo a una variabile `inputValues` con `const`.
...
Dovresti assegnare il risultato di `document.getElementsByClassName()` a una variabile chiamata `inputValues`.
