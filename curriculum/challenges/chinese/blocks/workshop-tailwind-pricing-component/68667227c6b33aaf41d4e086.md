...
要定位卡片中的项，你需要再次使用网格类。为包装器 `div` 赋值工具类 `grid`、`grid-rows-[1fr_auto]` 和 `gap-6`。

另外，使用类 `rounded-xl` 去除犀利的边缘。

# --hints--

你的高级计划的包装器 `div` 元素应具有 `grid` 类。
...
你的高级计划的包装器 `div` 元素应具有类 `grid-rows-[1fr_auto]`。
...
你的高级计划的包装器 `div` 元素应具有类 `rounded-xl`。
