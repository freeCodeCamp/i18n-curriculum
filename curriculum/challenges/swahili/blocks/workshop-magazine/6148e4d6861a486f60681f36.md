...
Picha zinapaswa kuwa ndani ya mpangilio wa safu mbili na mistari mitatu.

Toa kichaguzi cha `.image-wrapper` sifa ya `grid-template-columns` iliyowekwa kuwa `2fr 1fr` na sifa ya `grid-template-rows` iliyowekwa kuwa `repeat(3, min-content)`. Hii itatoa mistari ya gridi yetu inayobadilika urefu kulingana na maudhui, lakini safu zitabaki na upana thabiti kulingana na kontena.

# --hints--

Kichaguzi chako cha `.image-wrapper` kinapaswa kuwa na sifa ya `grid-template-columns` iliyowekwa kuwa `2fr 1fr`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

Kichaguzi chako cha `.image-wrapper` kinapaswa kuwa na sifa ya `grid-template-rows` iliyowekwa kuwa `repeat(3, min-content)`.
