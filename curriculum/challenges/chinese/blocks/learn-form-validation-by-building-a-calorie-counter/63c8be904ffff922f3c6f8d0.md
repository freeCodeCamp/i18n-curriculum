...
还记得你之前编写过一个函数来清理用户的输入吗？ 你需要在这里使用该功能。

将你的 `currVal` 声明更新为使用 `item.value` 调用 `cleanInputString` 的结果。

# --hints--

你应该在 `getCaloriesFromInputs` 函数中调用 `cleanInputString` 函数。

```js
assert.match(getCaloriesFromInputs.toString(), /cleanInputString\(/);
```

你应该将 `item.value` 作为 `cleanInputString` 函数的参数传递。

```js
assert.match(getCaloriesFromInputs.toString(), /cleanInputString\(\s*item\.value\s*\)/);
```

你应该将 `cleanInputString` 调用的结果分配给 `currVal` 变量。
