...
You should add a new `label` element after the checkbox element. تأكد من أن لديه علامة فتح وإغلاق.
...
Your `label` element should have a `for` attribute with the value `loving`.  توقعت عنصر بقيمة `--fcc-expected--`, لكن وجدت `--fcc-actual--`.
