...
Non preoccuparti dello spazio in basso, tutto verrà spostato più in basso più tardi quando aggiungerai un po’ di altezza all’elemento in cima all’edificio.

Aggiungi un `repeating-linear-gradient` a `.fb1c` con un angolo `90deg`, il tuo `--building-color4` da `0%` a `10%` e `transparent` da `10%` a `15%`.

# --hints--

Dovresti dare a `.fb1c` un `background` con un `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.background, "repeating-linear-gradient");
```

Dovresti usare una direzione `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb1c")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Dovresti usare un primo colore `--building-color4` da `0%` a `10%`.
...
Dovresti usare un secondo colore `transparent` da `10%` a `15%`.
