...
1. Ви повинні мати елемент `div` з `id` зі значенням `legend`. It should contain a `span` element with the text `Availability` and a `div` element with the `id` of `legend-gradient`.
...
Таблиця повинна мати не менше `5` рядків.

```js
assert.isAtLeast(document.querySelectorAll('tr')?.length, 5);
```

Таблиця повинна мати не менше `3` стовпців.
