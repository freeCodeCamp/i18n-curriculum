...
Dai al tuo elemento `#monsterStats` lo stesso `border` e `padding` del tuo elemento `#stats`. Imposta il suo colore su `#ffffff` e dagli uno sfondo `#c70d0d`.
...
Il tuo selettore `#monsterStats` dovrebbe avere un `border` di `1px solid #0a0a23`.
...
Il tuo selettore `#monsterStats` dovrebbe avere un `color` di `#ffffff`.
...
Il tuo selettore `#monsterStats` dovrebbe avere un `background-color` di `#c70d0d`.
