...
امنح عنصر `#monsterStats` نفس الـ `border` و`padding` مثل عنصر `#stats`. اضبط لونه على `#ffffff` وأعطه خلفية بلون `#c70d0d`.
...
يجب أن يحتوي محدِّد `#monsterStats` على `border` بقيمة `1px solid #0a0a23`.
...
يجب أن يحتوي محدِّد `#monsterStats` على `5px` من الحشو.
...
يجب أن يحتوي محدِّد `#monsterStats` على `color` بقيمة `#ffffff`.
...
يجب أن يحتوي محدِّد `#monsterStats` على `background-color` بقيمة `#c70d0d`.
