...
このプロジェクトでは、RPGゲームのクリーチャーを名前またはIDで検索し、結果をユーザーに表示するアプリを作成します。クリーチャーのデータと画像を取得するために、freeCodeCampの<a href="https://rpg-creature-api.freecodecamp.rocks/" target="_blank" rel="noopener noreferrer nofollow">RPG Creature API</a>を使用します。

**注意:** 最初の13ステップは`index.html`ファイル内で完了する必要があります。

**目的:** <a href="https://rpg-creature-search-app.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://rpg-creature-search-app.freecodecamp.rocks</a>と機能的に似たアプリを作成してください。

**ユーザーストーリー:**

1. `input`要素に`id`が`"search-input"`であり、必須であるものが必要です。
1. `button`要素に`id`が`"search-button"`であるものが必要です。
1. `id`が`"creature-name"`である要素が必要です。
1. `id`が`"creature-id"`である要素が必要です。
1. `id`が`"weight"`である要素が必要です。
1. `id`が`"height"`である要素が必要です。
1. `id`が`"types"`である要素が必要です。
1. `id`が`"hp"`である要素が必要です。
1. `id`が`"attack"`である要素が必要です。
1. `id`が`"defense"`である要素が必要です。
1. `id`が`"special-attack"`である要素が必要です。
1. `id`が`"special-defense"`である要素が必要です。
1. `id`が`"speed"`である要素が必要です。
1. `#search-input`要素に`Red`の値が含まれ、`#search-button`要素がクリックされると、テキスト`"Creature not found"`のアラートが表示されます。
1. `#search-input`要素に`Pyrolynx`の値が含まれ、`#search-button`要素がクリックされると、`#creature-name`、`#creature-id`、`#weight`、`#height`、`#hp`、`#attack`、`#defense`、`#special-attack`、`#special-defense`、`#speed`要素の値がそれぞれ`PYROLYNX`、`#1`または`1`、`Weight: 42`または`42`、`Height: 32`または`32`、`65`、`80`、`50`、`90`、`55`、`100`になります。
1. `#search-input`要素に`Pyrolynx`の値が含まれ、`#search-button`要素がクリックされると、`#types`要素内にテキスト`FIRE`を含む単一の要素が追加されます。検索間で`#types`要素の内容はクリアされます。
1. `#search-input`要素に`2`の値が含まれ、`#search-button`要素がクリックされると、`#creature-name`、`#creature-id`、`#weight`、`#height`、`#hp`、`#attack`、`#defense`、`#special-attack`、`#special-defense`、`#speed`要素の値がそれぞれ`AQUOROC`、`#2`または`2`、`Weight: 220`または`220`、`Height: 53`または`53`、`85`、`90`、`120`、`60`、`70`、`40`になります。
1. `#search-input`要素に`2`の値が含まれ、`#search-button`要素がクリックされると、`#types`要素内にテキスト`WATER`と`ROCK`を含む2つの要素が追加されます。検索間で`#types`要素の内容はクリアされます。
1. `#search-input`要素に無効なクリーチャー名が含まれ、`#search-button`要素がクリックされると、テキスト`"Creature not found"`のアラートが表示されます。
1. `#search-input`要素に有効なクリーチャーIDが含まれ、`#search-button`要素がクリックされると、UIに正しいデータが入力されます。

ユーザーストーリーを満たし、以下のすべてのテストに合格してこのプロジェクトを完成させてください。自分のスタイルで取り組んでください。コーディングを楽しんでください！

**注意:** テスト実行時にわずかな遅延があります。テストが完了するまで数秒待ってください。完了前にページを更新しないでください。

# --hints--

`input`要素に`id`が`"search-input"`であるものが必要です。`input`は必須としてマークしてください。
...
`button`要素に`id`が`"search-button"`であるものが必要です。
...
`id`が`"special-attack"`である要素が必要です。
...
`id`が`"special-defense"`である要素が必要です。
...
`#search-input`要素に`Red`の値が含まれ、`#search-button`要素がクリックされると、テキスト`"Creature not found"`のアラートが表示されます。
...
`#search-input`要素に`Pyrolynx`の値が含まれ、`#search-button`要素がクリックされると、`#creature-name`、`#creature-id`、`#weight`、`#height`、`#hp`、`#attack`、`#defense`、`#special-attack`、`#special-defense`、`#speed`要素の値がそれぞれ`PYROLYNX`、`#1`または`1`、`Weight: 42`または`42`、`Height: 32`または`32`、`65`、`80`、`50`、`90`、`55`、`100`になります。
...
`#search-input`要素に`Pyrolynx`の値が含まれ、`#search-button`要素がクリックされると、`#types`要素内にテキスト`FIRE`を含む単一の要素が追加されます。検索間で`#types`要素の内容はクリアされます。
...
`#search-input`要素に`2`の値が含まれ、`#search-button`要素がクリックされると、`#creature-name`、`#creature-id`、`#weight`、`#height`、`#hp`、`#attack`、`#defense`、`#special-attack`、`#special-defense`、`#speed`要素の値がそれぞれ`AQUOROC`、`#2`または`2`、`Weight: 220`または`220`、`Height: 53`または`53`、`85`、`90`、`120`、`60`、`70`、`40`になります。
...
`#search-input`要素に`2`の値が含まれ、`#search-button`要素がクリックされると、`#types`要素内にテキスト`WATER`と`ROCK`を含む2つの要素が追加されます。検索間で`#types`要素の内容はクリアされます。
...
`#search-input`要素に無効なクリーチャー名が含まれ、`#search-button`要素がクリックされると、テキスト`"Creature not found"`のアラートが表示されます。
...
`#search-input`要素に有効なクリーチャーIDが含まれ、`#search-button`要素がクリックされると、UIに正しいデータが入力されます。
...
検索ボタンがクリックされると、アプリはクリーチャー名またはIDに対応する正しいエンドポイントにfetchリクエストを送信します。
