...
Фінальні жорстко закодовані значення — це шість характеристик гравця. Замініть перші три множини жорстко закодованих чисел відповідною властивістю `player`:
...
Ви маєте замінити значення `97` для `PAC` на `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

Ви маєте замінити значення `98` для `SHO` на `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

Ви маєте замінити значення `83` для `PAS` на `player.pas`.
