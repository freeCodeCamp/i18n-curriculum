...
또 다른 `div` 요소를 만드세요. 그 안에 `button`를 중첩하여 `submit` 폼을 만드세요. 이 버튼에는 `Calculate Remaining Calories`라는 텍스트가 있어야 합니다.

그다음 `button`를 추가하고 `id`를 `clear`로 설정하여 폼을 지우세요(폼 제출을 막는 `type` 속성을 꼭 추가하세요). 이 버튼에는 `Clear`라는 텍스트가 필요합니다.
