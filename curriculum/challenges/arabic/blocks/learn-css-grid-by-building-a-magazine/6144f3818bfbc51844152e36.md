...
غلف كل قيمة من القيم التي عرّفتها مسبقًا في خاصية `grid-template-columns` بدالة `minmax`، مستخدمًا كل قيمة كالمعلمة الثانية. يجب أن تكون المعلمة الأولى `2rem`، `min-content`، و`2rem` على التوالي.
