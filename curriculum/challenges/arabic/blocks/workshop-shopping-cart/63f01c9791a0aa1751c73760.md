...
لا تزال هناك وظائف إضافية يحتاج إليها صنف `ShoppingCart` الخاص بك، لكن أولًا يجب أن تكون قادرًا على اختبار الشفرة التي كتبتها حتى الآن. ستحتاج إلى <dfn>إنشاء نسخة</dfn> جديدة من كائن `ShoppingCart` وتعيينه إلى متغير. إليك مثالًا على إنشاء نسخة من صنف `Computer` من الأمثلة السابقة:

```js
const myComputer = new Computer();
```

أعلن عن متغير `cart`، وعيّن له كائنًا جديدًا من `ShoppingCart`. لاحظ استخدام الكلمة المفتاحية `new` عند إنشاء النسخة.

# --hints--

يجب أن تستخدم `const` لإعلان متغير `cart`.

```js
assert.match(code, /const\s+cart\s*=/);
```

يجب أن تستخدم الكلمة المفتاحية `new` لإنشاء نسخة جديدة من كائن `ShoppingCart`.

```js
assert.match(code, /new\s+ShoppingCart\s*\(\s*\)/);
```

يجب أن تعيّن كائن `ShoppingCart` الجديد إلى المتغير `cart`.
