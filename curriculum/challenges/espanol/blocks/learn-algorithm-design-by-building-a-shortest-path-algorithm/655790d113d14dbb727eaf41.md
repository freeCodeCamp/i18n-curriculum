...
Dentro de tu condicional, anida otra estructura `if` que se active cuando el último elemento de `paths[node]` sea igual a `node`. Usa `pass` para llenar el cuerpo de la estructura `if`.
