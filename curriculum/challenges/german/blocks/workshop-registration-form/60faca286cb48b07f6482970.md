...
Das Absenden des Formulars mit einer ausgewählten Option würde keinen sinnvollen Wert an den Server senden. Deshalb benötigt jedes `option`-Element ein `value`-Attribut. Ohne dieses Attribut wird der Textinhalt des `option` an den Server übermittelt.

Geben Sie dem ersten `option` ein `value` mit dem Wert `""`, und den folgenden `option`-Elementen `value`-Attribute von `1` bis `4`.

# --hints--

Sie sollten dem ersten `option` ein `value` mit dem Wert `""` geben.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

Sie sollten dem zweiten `option` ein `value` mit dem Wert `1` geben.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

Sie sollten dem dritten `option` ein `value` mit dem Wert `2` geben.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

Sie sollten dem vierten `option` ein `value` mit dem Wert `3` geben.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

Sie sollten dem fünften `option` ein `value` mit dem Wert `4` geben.
