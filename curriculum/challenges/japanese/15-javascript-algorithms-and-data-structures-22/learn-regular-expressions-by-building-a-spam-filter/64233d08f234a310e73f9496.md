...
次の正規表現では、`stock alert` という文字列を検出します。 変数 `stockRegex` を宣言し、`stock alert` という文字列にマッチする正規表現を代入してください。 また、大文字小文字を区別せずマッチするようにしてください。

そして、配列 `denyList` にも追加してください。

# --hints--

変数 `stockRegex` の宣言には `const` を使ってください。

```js
assert.match(code, /const\s+stockRegex\s*=/);
```

変数 `stockRegex` には正規表現を代入してください。

```js
assert.instanceOf(stockRegex, RegExp);
```

正規表現 `stockRegex` を `stock alert` にマッチするようにしてください。
