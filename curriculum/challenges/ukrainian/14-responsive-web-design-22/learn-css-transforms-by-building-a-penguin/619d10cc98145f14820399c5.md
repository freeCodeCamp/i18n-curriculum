...
Під елементом `.blush.right` додайте два елементи `div`, які мають `class` зі значенням `beak`. Також надайте першому елементу `.beak` властивість `class` зі значенням `top`, а другому елементу `.beak` — властивість `class` зі значенням `bottom`.
...
Першому новому `div` надайте `class` зі значенням `beak`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

Другому новому `div` надайте `class` зі значенням `beak`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div.beak')?.length, 2);
```

Першому новому `div` надайте `class` зі значенням `top`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

Другому новому `div` надайте `class` зі значенням `bottom`.
