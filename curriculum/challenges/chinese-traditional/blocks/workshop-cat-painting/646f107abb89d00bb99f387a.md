...
使用後代選擇器來定位 `.cat-whiskers-left` 元素內部的三個 `div` 元素。 爲它添加 `40px` 的 `width`、`1px` 的 `height` 以及 `#000` 的 `background-color`。
