...
title: Assegna a ogni elemento un id unico
challengeType: 0
forumTopicId: 18191
...
Vogliamo anche poter usare jQuery per selezionare ogni pulsante tramite il suo id unico.

Assegna a ciascuno dei tuoi pulsanti un id unico, iniziando con `target1` e terminando con `target6`.

Assicurati che `target1` a `target3` siano in `#left-well`, e `target4` a `target6` siano in `#right-well`.
