...
أكمل الـ setter بطباعة `'{name}' promoted to '{new_level}'.` داخل طريقتك. استبدل `{name}` و`{new_level}` باسم الموظف والمستوى الجديد، على التوالي.

# --hints--

يجب أن تطبع `'{name}' promoted to '{new_level}'.` حيث يجب استبدال `{name}` و`{new_level}` باسم الموظف والمستوى الجديد، على التوالي.
