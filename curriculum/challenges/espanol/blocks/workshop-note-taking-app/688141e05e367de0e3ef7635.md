...
En este momento, `currentContent` está inicializado con el valor de una cadena vacía. Pero el resultado deseado sería que `currentContent` contenga el valor del texto de la nota actual.

En una lección anterior, aprendiste cómo trabajar con el evento `DOMContentLoaded` así:
...
Cuando todo en el documento HTML se haya cargado y analizado, podrás acceder a `noteEl` de forma segura.

Comienza adjuntando un método `addEventListener` al objeto `window`. El primer argumento para el método `addEventListener` debe ser el evento `"DOMContentLoaded"`. El segundo argumento debe ser una función flecha. Dentro del cuerpo de la función flecha, asigna el valor de `noteEl.textContent` a `currentContent`.

# --hints--

Debes adjuntar un método `addEventListener` al objeto `window`.

```js
assert.match(code, /window\.addEventListener/);
```

Tu listener de eventos debe escuchar el evento `"DOMContentLoaded"`.

```js
assert.match(code, /window\.addEventListener\(['"]DOMContentLoaded['"]\,/);
```

Dentro del cuerpo de la función flecha, asigna el valor de `noteEl.textContent` a `currentContent`.
