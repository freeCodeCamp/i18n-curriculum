...
اكتب قاعدة جديدة باستخدام محدِّد الصنف `.frame`.

استخدم إعلان الاختصار `border` لإعطاء عنصر `.frame` حدودًا صلبة سوداء بعرض `50px`.

# --hints--

يجب أن يحتوي شفرتك على محدِّد `.frame`.
...
يجب تعيين خاصية `border` إلى `50px solid black`.
...
يجب أن يحتوي عنصر `.frame` على خاصية `border` بقيمة `50px solid black`.
