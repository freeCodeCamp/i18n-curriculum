...
你应该使用 `console.log()` 来打印 `inputValues` 的结果。 在 `sortInputArray` 函数中编写此代码。

要查看记录的 `inputValues` 数组，请点击排序按钮并打开控制台。 你应该会看到一个字符串数组如下：

```js
[ "8", "2", "4", "1", "3" ]
```

在继续之前，请确保你注意到在控制台中打印结果的数据类型。
...
你应该将 `inputValues` 传递给你的 `console.log()` 调用。
