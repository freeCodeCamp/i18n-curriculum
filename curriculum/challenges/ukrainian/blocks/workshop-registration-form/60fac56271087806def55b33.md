...
Вкладіть елемент `select` (з елементами `option`) в елемент `label` з текстом `How did you hear about us?`. Текст повинен йти перед елементом `select`.
...
В межах елемента `select` має бути п’ять елементів `option`.
