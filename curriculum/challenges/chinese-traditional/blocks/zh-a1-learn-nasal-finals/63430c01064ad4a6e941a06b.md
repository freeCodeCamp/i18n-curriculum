...
`uen` 是一個平滑且連續的音。要發出這個音，先從圓唇的 `u` 開始，然後快速經過輕柔的 `e` 音，最後以前鼻音 `n` 結束，舌尖輕觸上前牙後方。

在拼寫中，`uen` 通常寫作 `un`。

當 `uen` 與字首結合時，你可以組成像是 `lun`、`dun`、`tun`、`gun`、`kun`、`hun` 和 `chun` 這樣的音節。

讓我們練習發音 `uen` 和音節，然後以不同聲調使用 `hun` 作為範例。
