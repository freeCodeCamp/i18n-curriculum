...
Em seguida, você começará a trabalhar com o `JavaScript`.Para isso, comece vinculando o arquivo `script.js` ao seu HTML.

Crie um elemento `script` com seu atributo `src` apontando para o arquivo `script.js`.Por exemplo: 
...
Você deve criar um elemento `script`.

```js
assert.lengthOf(document.querySelectorAll('script'), 1);
```

Seu elemento `script` deve ter um atributo `src` definido como `script.js`.
