...
如果你尝试输入消息 `free money`，你会注意到它与表达式不匹配！ 这是因为 `\s` 不匹配文本的开头或结尾。

要匹配文本的开头，可以使用 `^` 锚点。 这断言你的模式匹配从整个字符串的开头开始。

将你的第一个 `\s` 字符替换为一个非捕获组，该组匹配 `\s` 或 `^`。

# --hints--

你的 `freeRegex` 应该使用一个非捕获组。

```js
assert.match(freeRegex.source, /\(\?:/);
```

你的 `freeRegex` 应该使用非捕获组匹配 `\s` 或 `^`。
