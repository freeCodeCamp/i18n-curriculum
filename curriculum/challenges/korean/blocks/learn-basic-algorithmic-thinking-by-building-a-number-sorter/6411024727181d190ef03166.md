...
중첩된 `for` 반복문 이후에 가장 작은 값을 찾았습니다. 현재 값과 교환해야 합니다.

버블 정렬에서 했던 것처럼 `temp` 변수를 사용해 `array[i]` 위치의 값을 추출한 다음 `array[i]`와 `array[minIndex]` 위치의 값을 교환하세요.

# --hints--

중첩된 `for` 반복문 이후에 `temp` 변수를 선언하세요.
...
`array[minIndex]`를 `array[i]`에 할당해야 합니다.
...
`temp`를 `array[minIndex]`에 할당해야 합니다.
