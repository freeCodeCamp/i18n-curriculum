...
Щоб отримати попередню пісню, відніміть `1` від `currentSongIndex` у `userData?.songs` і призначте це константі `previousSong`. Після цього викличте функцію `playSong` і передайте `previousSong.id` як аргумент.
...
Ви повинні призначити `userData?.songs[currentSongIndex - 1]` константі `previousSong`.
...
Ви повинні викликати функцію `playSong` з `previousSong.id`.
