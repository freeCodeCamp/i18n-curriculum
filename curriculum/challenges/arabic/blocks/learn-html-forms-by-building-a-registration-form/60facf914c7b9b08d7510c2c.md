...
يبدو أن `textarea` صغير جدًا. لإعطائه حجمًا ابتدائيًا، يمكنك إضافة خاصيتي `rows` و `cols`.

أضف حجمًا ابتدائيًا مكونًا من `3` صفوف و `30` أعمدة.

# --hints--

يجب أن تعطي `textarea` خاصية `rows` بقيمة `3`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

يجب أن تعطي `textarea` خاصية `cols` بقيمة `30`.
