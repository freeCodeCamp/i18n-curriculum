...
给 `.image-wrapper` 选择器一个设置为 `2fr 1fr` 的 `grid-template-columns` 属性和一个设置为 `repeat(3, min-content)` 的 `grid-template-rows` 属性。 这将使我们的网格行可以根据内容调整高度，但列可以根据容器保持固定的宽度。

# --hints--

你的 `.image-wrapper` 选择器应该有一个 `grid-template-columns` 属性，设置为 `2fr 1fr`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

你的 `.image-wrapper` 选择器应该有一个 `grid-template-rows` 属性，设置为 `repeat(3, min-content)`。
