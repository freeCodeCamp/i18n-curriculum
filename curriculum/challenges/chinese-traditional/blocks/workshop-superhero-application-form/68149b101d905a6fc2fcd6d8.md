...
Call the `setPowers` function, which takes an array argument. Use a ternary operator to check if `checked` is true. If it is, spread the existing `powers` array into a new array and add the `value` to it. 如果不是，則使用 `powers.filter(p => p !== value)` 從 `powers` 中過濾掉 `value`。

# --hints--

你應該使用三元運算符來檢查 `checked` 是否爲真。 如果是，則將現有的 `powers` 和 `value` 分散到數組中。 如果不是，則從 `powers` 中過濾掉 `value`。
