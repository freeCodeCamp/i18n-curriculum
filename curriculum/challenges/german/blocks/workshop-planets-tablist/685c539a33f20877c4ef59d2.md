...
Jetzt ist es Zeit, daran zu arbeiten, das richtige Panel anzuzeigen, wenn auf einen Tab geklickt wird. Jedes der Panels hat ein `id`, das mit dem `aria-controls`-Attribut des entsprechenden Tabs übereinstimmt. Zum Beispiel hat das erste Panel ein `id` mit dem Wert `panel-earth`, der mit dem `aria-controls`-Attributwert des ersten Tabs übereinstimmt, der `panel-earth` ist.

Erstellen Sie eine Variable namens `associatedPanel` und weisen Sie ihr den Wert `tab.getAttribute("aria-controls")` zu. Dadurch erhalten Sie das `id` des Panels, das dem angeklickten Tab entspricht.

# --hints--

Sie sollten eine Variable namens `associatedPanel` haben, die das `id` des Panels enthält, das mit dem angeklickten Tab verknüpft ist.
