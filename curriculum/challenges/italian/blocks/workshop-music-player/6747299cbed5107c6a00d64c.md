...
Il resto delle canzoni è stato aggiunto all'array `allSongs` per te.

Nelle lezioni precedenti, hai imparato a conoscere la <dfn>Web Audio API</dfn> e come usarla per riprodurre canzoni. Tutti i browser moderni supportano la Web Audio API, che ti permette di generare e processare audio nelle applicazioni web.

Crea una variabile chiamata `audio` e assegnale il valore `new Audio()`. Questo creerà un nuovo elemento `audio` HTML5.

# --hints--

Dovresti avere una variabile chiamata `audio`.

```js
assert.isDefined(audio);
```

Dovresti assegnare una nuova istanza dell'oggetto `Audio` alla variabile chiamata `audio`.
