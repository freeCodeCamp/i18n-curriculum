...
Sfortunatamente, quella proprietà `children` restituisce una raccolta di elementi, che è simile a un array ma non è un array. Avvolgi il tuo secondo argomento in `Array.from()` per convertirlo in un array.

# --hints--

Dovresti avvolgere il tuo `document.getElementById('container').children` in `Array.from()`.
