...
`.face.right` 요소 아래에 `div`인 `class`를 가진 `chin` 요소를 추가하세요.

# --hints--

`div` 안에 `.penguin-head` 요소를 하나 추가해야 합니다. 예상된 `--fcc-expected--` `div` 요소가 발견된 `--fcc-actual--`입니다.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 3);
```

`div`에 `class`인 `chin`을 지정해야 합니다.
