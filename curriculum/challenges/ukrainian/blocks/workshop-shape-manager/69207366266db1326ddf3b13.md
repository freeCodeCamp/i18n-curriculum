...
Додайте подію `DOMContentLoaded` до кореневого документа та передайте `initializeApp` як функцію зворотного виклику для запуску. Це гарантує, що вся ініціалізація відбуватиметься лише після повного завантаження DOM.

# --hints--

Ви повинні додати подію `DOMContentLoaded` до кореневого `document`.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

Ви повинні посилатися на `initializeApp` як функцію для запуску у вашому слухачі подій.
