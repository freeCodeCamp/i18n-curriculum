...
Mzunguko umekamilika. Rudisha `grouped` kutoka kwenye kitendakazi.

Kisha, chini ya kitendakazi, fanya wito wa `groupByDecade(catalog)` na weka thamani ya matokeo katika kigezo kinachoitwa `byDecade`. Andika `byDecade` ili ukague matokeo yaliyokusanywa, na utapata kitu chenye funguo kama `"1950s"`, `"1960s"`, `"1970s"`, na kadhalika, kila moja ikielekeza kwenye safu ya data ya vitabu kutoka muongo huo.

# --hints--

`groupByDecade` inapaswa kurudisha kitu.

```js
assert.isObject(groupByDecade(catalog));
```

`byDecade["1980s"].length` inapaswa kuwa `5`.

```js
assert.strictEqual(groupByDecade(catalog)["1980s"].length, 5);
```

`byDecade["Unknown"].length` inapaswa kuwa `1`.

```js
assert.strictEqual(groupByDecade(catalog)["Unknown"].length, 1);
```

Unapaswa kuweka thamani ya matokeo ya `groupByDecade(catalog)` katika kigezo kinachoitwa `byDecade`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+byDecade\s*=\s*groupByDecade\s*\(\s*catalog\s*\)/);
```

Unapaswa kuandika `byDecade` kwenye konsoli.
