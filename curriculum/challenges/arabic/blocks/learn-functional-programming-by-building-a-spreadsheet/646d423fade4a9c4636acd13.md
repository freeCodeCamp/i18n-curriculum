...
أولًا، عليك التعامل مع العوامل ذات الأولوية الأعلى. أعلن عن متغير `noHigh`، وقم بتعيين نتيجة استدعاء `highPrecedence()` مع تمرير `str` كمعلمة.

# --hints--

يجب أن تعلن عن متغير `noHigh`.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*(?:var|let|const)\s+noHigh\s*=/);
```

يجب أن تستخدم `const` لإعلان متغير `noHigh` الخاص بك.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=/);
```

يجب أن تعيّن إلى `noHigh` نتيجة استدعاء `highPrecedence()`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(/);
```

يجب أن تمرر `str` كمعلمة لاستدعاء `highPrecedence()`.
