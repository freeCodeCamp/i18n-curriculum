...
In JavaScript, values can either be <dfn>truthy</dfn> or <dfn>falsy</dfn>. ある値がブール値に変換されたときに `true` と評価される場合、その値は真値です。 ブール値に変換されたときに `false` と評価される場合、その値は偽値です。 `null` は偽値の一つです。
