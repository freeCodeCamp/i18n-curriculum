...
你的正則表達式應該匹配整個字，而不是部分字。也就是說，你不想匹配 `hands-free money management`。

為此，請先檢查你的樣式前後是否有空白。你可以使用元字元 `\s` 來做到這點，該字元會匹配空白、製表符和換行。

# --hints--

你的 `freeRegex` 應該使用 `\s` 語彙單元。

```js
assert.match(freeRegex.source, /\\s/);
```

你的 `freeRegex` 應該尋找樣式開頭和結尾的空格。
