...
Debes tener un selector `.message`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".message"));
```

El selector `.message` debe tener una propiedad `font-size` establecida en `1.2em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("font-size"), "1.2em");
```

El selector `.message` debe tener una propiedad `color` establecida en `gray`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("color"), "gray");
```

El selector `.message` debe tener una propiedad `margin-bottom` establecida en `20px`.
