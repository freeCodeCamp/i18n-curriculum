...
Alle Kernfunktionen sind jetzt vorhanden. Das einzige Problem ist, dass das nächste Lied nicht automatisch abgespielt wird, wenn das aktuell laufende Lied endet.

Um das zu beheben, können Sie einen Ereignis-Listener einrichten, der erkennt, wann das aktuell laufende Lied endet. Der `"ended"` Ereignis-Listener ist dafür geeignet. Er wird ausgelöst, wenn die Wiedergabe eines Mediums zu Ende ist.

Fügen Sie dem `audio` Element einen Ereignis-Listener hinzu, der auf das `"ended"` Ereignis hört. Übergeben Sie einen Callback mit Pfeilfunktion-Syntax und leeren geschweiften Klammern.
...
Der Ereignis-Listener, den Sie an Ihre `audio` Variable angehängt haben, sollte auf ein `"ended"` Ereignis hören.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1/)
```

Sie sollten Pfeilfunktion-Syntax verwenden, um Ihrem `"ended"` Ereignis-Listener einen leeren Callback zu übergeben.
