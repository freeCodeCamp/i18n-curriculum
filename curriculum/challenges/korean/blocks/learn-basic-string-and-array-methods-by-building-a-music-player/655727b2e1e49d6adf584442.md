...
재생목록이 비어 있으면 `resetButton` 요소와 그에 대한 텍스트를 만들어야 합니다. 이 버튼은 재생목록이 비어 있을 때만 나타납니다.

`createElement()`은 JavaScript를 사용해 동적으로 요소를 만들 때 사용할 수 있는 DOM 메서드입니다. `createElement()`를 사용하려면 호출한 다음 태그 이름을 문자열로 전달하세요:
...
`if` 문장 안에서 `resetButton` 상수를 선언한 다음 `createElement()`를 사용해 `"button"`를 만드세요.
...
`createElement` 메서드를 사용해 `"button"`를 만들어야 합니다.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

새로 만든 `button` 요소를 `resetButton` 상수에 할당하세요.
