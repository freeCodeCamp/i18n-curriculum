...
Dovresti dare a `.fb4a` una proprietà `border-top` con il valore `5vh solid transparent`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4a")?.borderTop.trim(), "5vh solid transparent")
```

Dovresti dare a `.fb4a` una proprietà `border-left` con il valore `8vw solid var(--building-color1)`.
