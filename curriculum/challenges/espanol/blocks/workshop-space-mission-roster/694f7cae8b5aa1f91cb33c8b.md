...
Como antes, debes validar tu entrada. Específicamente, si `size` es menor que `1`, debes registrar `"Chunk size must be >= 1"` en la consola y luego llamar a `return` para salir de la función.

# --hints--

Si el `size` de entrada es inválido, debes registrar el siguiente mensaje de error en la consola, `"Chunk size must be >= 1"`.
...
Después de registrar el mensaje de error, debes llamar inmediatamente a `return` para salir de la función temprano.
...
Tu función `chunkCrew` debe pasar la validación sin errores cuando `size` sea mayor o igual a 1.
