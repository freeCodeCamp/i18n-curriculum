...
純量乘法運作正常。現在，修改 `v5` 的指派，使其為點積 `v1 * v2`。同時，更新 `print` 呼叫。

在此之前，請隨意嘗試將 `v1` 乘以無效型別的物件，以查看列印在主控台上的錯誤訊息。


# --hints--

你應該更新 `v5` 的指派，將其指定為純量積 `v1 * v2`。

```js
({ test: () => assert(runPython(`_Node(_code).find_variable("v5").is_equivalent("v5 = v1 * v2")`)) })
```

你應該更新你的 `print` 呼叫以列印 f-string `f'v1 * v2 = {v5}'`。 
