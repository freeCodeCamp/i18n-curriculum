...
لإصلاح الخطأ، أنشئ دالة سهمية `handlePowersChange` بمعلمة `e`.

داخل الدالة، فكك `value` و `checked` من `e.target` للحصول على قيمة مربع الاختيار وما إذا كان محددًا أم لا.

# --hints--

يجب أن تنشئ دالة `handlePowersChange` بمعلمة `e`.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

يجب أن تفكك `value` و `checked` من `e.target`.
