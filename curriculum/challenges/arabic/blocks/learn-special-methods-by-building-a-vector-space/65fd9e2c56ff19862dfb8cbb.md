...
تُستدعى الطريقة `__lt__` ضمنيًا عند استخدام عامل `<` لمقارنة كائن بشيء آخر.

أضف طريقة `__lt__` فارغة وعرّف لها معلمتين: `self` و `other`.

# --hints--

يجب تعريف طريقة `__lt__` داخل صنف `R2Vector`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__lt__")`)) })
```

يجب أن تحتوي طريقة `__lt__` على معلمتين، `self` و `other`.
