...
Necesitas agregar el `resetText` al elemento `resetButton` como hijo, y también el `resetButton` al elemento `playlistSongs` como hijo. Para esto, hay que usar el método `appendChild()`.

`appendChild()` te permite agregar un nodo o un elemento como hijo de otro elemento. En el ejemplo abajo, el texto "Clic aquí" se adjuntaría al botón:
...
Usa `appendChild()` para adjuntar `resetText` al elemento `resetButton`, y `resetButton` al elemento `playlistSongs`.

# --hints--

Deberías usar el método `appendChild()` en `resetButton`.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

Deberías pasar `resetText` como el valor de tu primer `appendChild()`.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

Deberías usar el método `appendChild()` en `playlistSongs`.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

Deberías pasar `resetButton` como el valor de tu segundo `appendChild()`.
