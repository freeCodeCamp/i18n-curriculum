...
이제 라디오 버튼을 사용자 지정할 차례입니다. 기본적으로 라디오 버튼은 브라우저 고유의 모양을 가지고 있습니다. 이 모양을 제거해서 사용자 지정 스타일을 만들 수 있게 하세요.

클래스가 `contact-method-radio-btn`인 요소를 선택하세요. `appearance`을 `none`로 설정하세요.

# --hints--

클래스가 `contact-method-radio-btn`인 요소를 선택해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

`appearance` 속성을 `none`로 설정하세요.
