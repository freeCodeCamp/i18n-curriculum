...
Ifuatayo, tengeneza kijenzi kwa darasa la `Bug` ambalo linapokea kigezo kimoja kinachoitwa `emojiElement` cha aina ya `HTMLParagraphElement`. Ndani ya kijenzi, weka thamani ya kigezo cha `emojiElement` kwenye sifa ya `emojiElement` ya darasa hilo.

# --hints--

Darasa lako la `Bug` linapaswa kuwa na kijenzi.
...
Kijenzi chako cha `Bug` kinapaswa kuwa na kigezo kinachoitwa `emojiElement` cha aina ya `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Kijenzi chako cha `Bug` kinapaswa kuweka thamani ya `emojiElement` kwenye `this.emojiElement`.
