...
Llame al método `.map()` en su llamada a `rangeFromString()`, pasando una referencia a `addCharacters` como la función de callback.

# --hints--

Debería llamar al método `.map()` en su llamada a `rangeFromString()`.
...
Debería pasar una referencia a `addCharacters` como el callback a su método `.map()`.
