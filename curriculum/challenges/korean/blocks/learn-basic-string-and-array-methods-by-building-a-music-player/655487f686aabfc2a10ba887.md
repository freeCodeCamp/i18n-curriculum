...
다음 곡과 이전 곡을 재생하는 작업을 시작하기 전에 `songs`의 `userData` 속성에서 각 곡의 인덱스를 가져와야 합니다.

`getCurrentSongIndex`라는 화살표 함수를 만드는 것부터 시작하세요.

# --hints--

`getCurrentSongIndex` 함수가 있어야 합니다.

```js
assert.isFunction(getCurrentSongIndex);
```

`getCurrentSongIndex` 함수는 화살표 구문을 사용해야 합니다.
