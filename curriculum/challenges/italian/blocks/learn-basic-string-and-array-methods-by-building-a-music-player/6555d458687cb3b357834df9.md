...
Consulta le proprietà `userData?.currentSong?.title` e `userData?.currentSong?.artist` e assegnale rispettivamente a due variabili `currentTitle` e `currentArtist`.

# --hints--

Dovresti consultare il `title` di `currentSong` dall'oggetto `userData`. Non dimenticare di usare il concatenamento opzionale.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

Dovresti assegnare il `title` del `currentSong` a una costante `currentTitle`.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

Dovresti consultare il `artist` di `currentSong` dall'oggetto `userData`. Non dimenticare di usare il concatenamento opzionale.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

Dovresti assegnare il `artist` del `currentSong` a una costante `currentArtist`.
