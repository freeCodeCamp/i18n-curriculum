...
最初の引数を`setTimeout`の`async`アロー関数に更新してください。これにより、遅延処理の中で`await`を使うことができます。その関数の中で、データ取得時の潜在的なエラーを処理するために`try...catch`ブロックを作成してください。

# --hints--

`async`の最初の引数として`setTimeout`アロー関数があるはずです。

```js
assert.match(code, /setTimeout\s*\(\s*async\s*\(\s*\)\s*=>\s*{/);
```

`async`関数の中で`try...catch`ブロックを作成してください。
