...
Erstellen Sie ein `button`-Element mit dem `className` von `submit-btn` und einem `type` von `submit`. Geben Sie dem Button den Text `Join the League`.

Versuchen Sie danach, das Formular abzusenden, und beobachten Sie, was passiert.
...
Ihr Button-Element sollte ein `className`-Attribut mit dem Wert `submit-btn` haben.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

Ihr `button`-Element sollte `Join the League` als Text enthalten.
