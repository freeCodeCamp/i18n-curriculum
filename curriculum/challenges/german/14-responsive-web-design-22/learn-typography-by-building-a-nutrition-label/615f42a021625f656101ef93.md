...
Bette den Text `2/3 cup (55g)` in ein `span`-Element ein.

# --hints--

Du solltest ein neues `span`-Element erstellen.

```js
assert(document.querySelector('span'));
```

Dein `span`-Element sollte den Text `2/3 cup (55g)` enthalten.

```js
assert(document.querySelector('span')?.textContent?.trim() === '2/3 cup (55g)');
```

Dein `p`-Element sollte immer noch den Text `Serving size 2/3 cup (55g)` enthalten.
