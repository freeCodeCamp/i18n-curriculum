...
Agora combine todas as linhas em uma string CSV que você pode exportar ou compartilhar.

Após o primeiro loop, declare `let csv = header`. Adicione outro loop `for` para anexar cada linha a `csv` com uma nova linha: `csv = csv + "\n" + rows[i]`. Retorne `csv`. Então, abaixo da função, registre `exportToCSV(catalog)` para visualizar a saída formatada.

# --hints--

`exportToCSV` deve retornar uma string.

```js
assert.isString(exportToCSV(catalog));
```

A primeira linha da string retornada deve ser `"Title,Author,Year,Location"`.

```js
assert.strictEqual(exportToCSV(catalog).split("\n")[0], "Title,Author,Year,Location");
```

A saída deve conter `"King, Stephen"` (entre aspas para lidar com a vírgula).

```js
assert.include(exportToCSV(catalog), '"King, Stephen"');
```

Você deve registrar `exportToCSV(catalog)` no console.
