...
Seleziona l'elemento `.back-mountain` e imposta i suoi `width` e `height` su `300px`. Poi, imposta il `background` su un gradiente lineare che inizia da `rgb(203, 241, 228)` e termina a `rgb(47, 170, 255)`.
...
Dovresti dare a `.back-mountain` un `width` di `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

Dovresti dare a `.back-mountain` un `height` di `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

Dovresti dare a `.back-mountain` un `background` di `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`.
