...
`fn` 參數將傳遞一個函數的名稱，例如 `"SUM"`。 更新 `apply` 以使用 `fn` 變量作爲對象訪問的鍵從 `spreadsheetFunctions` 對象隱式返回函數。

請記住，`fn` 可能不包含小寫字符串，因此你需要將其轉換爲小寫字符串。
...
你的 `apply` 函數應該訪問 `spreadsheetFunctions` 對象。
...
你的 `apply` 函數應該訪問與 `fn` 值匹配的 `spreadsheetFunctions` 對象的屬性。
...
你的 `apply` 函數應該在屬性訪問中的 `fn` 上調用 `.toLowerCase()` 方法。
