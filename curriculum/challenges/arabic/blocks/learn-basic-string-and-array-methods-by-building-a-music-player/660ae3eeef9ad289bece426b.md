...
في المشاريع السابقة، استخدمت الدوال العادية. لكن في بقية المشاريع، ستعمل مع <dfn>الدوال السهمية</dfn>. الخطوات التالية ستركز على تعليمك أساسيات الدوال السهمية.

الدالة السهمية هي تعبير دالة مجهولة وطريقة مختصرة لكتابة الدوال. المجهولة تعني أن الدالة لا تحمل اسمًا. الدوال السهمية دائمًا مجهولة.
...
بـ تعيين تعريف الدالة السهمية إلى متغير، تربطها بمعرف.
...
أنشئ دالة سهمية جديدة وعيّنها إلى المتغير `printGreeting`. داخل جسم الدالة، استخدم الطريقة `console.log()` لطباعة السلسلة النصية `Hello there!`.

# --hints--

يجب أن يكون لديك متغير اسمه `printGreeting` وتعيّن له دالة سهمية.

```js
assert.isFunction(printGreeting);
```

يجب أن تنشئ دالة سهمية وتعيّنها إلى المتغير `printGreeting`.

```js
assert.match(code, /(let|const)\s+printGreeting\s*=\s*\(.*\)\s*=>\s*{\s*[\s\S]*}\s*;?/)
```

يجب أن تقوم دالة `printGreeting` بطباعة `Hello there!` في وحدة التحكم.
