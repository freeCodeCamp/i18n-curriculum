...
CSSを始めるには、`*`を使ってすべての要素をターゲットにし、`::before`と`::after`の疑似セレクターも含めてCSSルールを正規化してください。

`padding`と`margin`のプロパティを両方とも`0`に設定し、`box-sizing`のプロパティを`border-box`に設定してください。
...
`*, ::before, ::after`セレクターには`padding`プロパティが`0`に設定されているはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.padding, '0px');
```

`*, ::before, ::after`セレクターには`margin`プロパティが`0`に設定されているはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.margin, '0px');
```

`*, ::before, ::after`セレクターには`box-sizing`プロパティが`border-box`に設定されているはずです。
