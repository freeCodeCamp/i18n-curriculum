...
Der Text der Listenelemente sollte in Anker-Tags eingeschlossen werden.

# --hints--

Du solltest ein `ul`-Element innerhalb der `nav` verschachteln.

```js
assert.equal(document.querySelectorAll('nav > ul')?.length, 1);
```

Du solltest drei `li`-Elemente im `ul`-Element verschachteln.

```js
assert.equal(document.querySelectorAll('nav > ul > li')?.length, 3);
```

Du solltest ein `a`-Element in jedes `li`-Element verschachteln.

```js
assert.equal(document.querySelectorAll('nav > ul > li > a')?.length, 3);
```

Du solltest dem ersten `a`-Element den Text `INFO` geben.

```js
assert.equal(document.querySelectorAll('nav > ul > li > a')?.[0]?.textContent, 'INFO');
```

Du solltest dem zweiten `a`-Element den Text `HTML` geben.

```js
assert.equal(document.querySelectorAll('nav > ul > li > a')?.[1]?.textContent, 'HTML');
```

Du solltest dem dritten `a`-Element den Text `CSS` geben.
