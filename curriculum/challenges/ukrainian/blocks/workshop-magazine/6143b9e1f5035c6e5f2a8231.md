...
Усередині вашого елемента `ul` створіть шість елементів `li`. Додайте елемент `h4` з множиною `class`, встановленою на `list-subtitle`, та елемент `p` до кожного з ваших елементів `li`.

Потім надайте елементам `h4` і `p` такий текстовий вміст, у порядку, причому кожен `h4` використовує те, що ліворуч від двокрапки, а кожен `p` — те, що праворуч:
...
Ваш елемент `ul` має містити шість елементів `li`.

```js
assert.lengthOf(document.querySelectorAll('.lists li'), 6);
```

Кожен із нових елементів `li` повинен містити елементи `h4` і `p`.
...
Ваш перший `h4` має містити текст `V1 - 2014`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

Ваш перший `p` має містити текст `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
Ваш другий `h4` має містити текст `V2 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[1]?.innerText, 'V2 - 2015');
```

Ваш другий `p` має містити текст `We added interactive algorithm challenges.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[1]?.innerText, 'We added interactive algorithm challenges.');
```

Ваш третій `h4` має містити текст `V3 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[2]?.innerText, 'V3 - 2015');
```

Ваш третій `p` має містити текст `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`
...
Ваш четвертий `h4` має містити текст `V4 - 2016`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[3]?.innerText, 'V4 - 2016');
```

Ваш четвертий `p` має містити текст `We expanded the curriculum to 3 certifications, including Front-End, Back-End, and Data Visualization. They each had 10 required projects, but only the Front-End section had its own challenges. For the other certs, we were still using external resources like Node School.`
...
Ваш п’ятий `h4` має містити текст `V5 - 2017`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[4]?.innerText, 'V5 - 2017');
```

Ваш п’ятий `p` має містити текст `We added the back-end and data visualization challenges.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[4]?.innerText, 'We added the back-end and data visualization challenges.');
```

Ваш шостий `h4` має містити текст `V6 - 2018`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[5]?.innerText, 'V6 - 2018');
```

Ваш шостий `p` має містити текст `We launched 6 new certifications to replace our old ones. This was the biggest curriculum improvement to date.`
...
Ваші шість елементів `h4` мають кожен мати клас `list-subtitle`.
