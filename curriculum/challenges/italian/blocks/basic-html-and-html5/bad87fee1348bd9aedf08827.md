...
title: Crea una lista non ordinata con punti elenco
challengeType: 0
...
HTML ha un elemento speciale per creare <dfn>liste non ordinate</dfn>, cioè liste con punti elenco.

Le liste non ordinate iniziano con un elemento `<ul>` di apertura, seguito da un numero qualsiasi di elementi `<li>`. Infine, le liste non ordinate si chiudono con un `</ul>`.
...
creerebbe una lista con punti elenco di `milk` e `cheese`.

# --instructions--

Rimuovi gli ultimi due elementi `p` e crea una lista non ordinata di tre cose che i gatti amano in fondo alla pagina.
...
I tuoi elementi `li` dovrebbero avere tag di chiusura.
