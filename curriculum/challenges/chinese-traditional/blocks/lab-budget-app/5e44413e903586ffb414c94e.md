...
1. `Category` 類別應該有以下方法：
   ...
   - 一個接受金額和選用描述的 `deposit` 方法。如果沒有提供描述，應該預設為空字串。該方法應該以 `ledger` 的形式，將一個物件附加到 `{'amount': amount, 'description': description}` 列表中。
   - 一個接受金額和選用描述（預設為空字串）的 `withdraw` 方法。該方法應該將傳入的金額以負數形式儲存在 `ledger` 中，並且如果提款成功則傳回 `True`，否則傳回 `False`。
   - 一個根據 `get_balance` 傳回目前類別餘額的 `ledger` 方法。
   - 一個接受金額和另一個 `transfer` 實體的 `Category` 方法，會以描述 `Transfer to [Destination]` 提款該金額，並以描述 `Transfer from [Source]` 將其存入另一個分類，其中 `[Destination]` 和 `[Source]` 應該被目的地和來源分類的名稱取代。當轉帳成功時，該方法應傳回 `True`，否則傳回 `False`。
   - 一個接受金額並在超過餘額時傳回 `check_funds`，否則傳回 `False` 的 `True` 方法。此方法必須被 `withdraw` 和 `transfer` 兩個方法使用。

1. 當列印一個 `Category` 物件時，應該：  
...
1. 你應該有一個在 `Category` 類別外的函式，名為 `create_spend_chart(categories)`，該函式接受一個類別列表並回傳一個長條圖字串。建置圖表的方法：

   - 以標題 `Percentage spent by category` 開頭。
   - 百分比只從提款計算，不包含存款。百分比應該是每個類別花費金額佔所有類別總花費的百分比（向下取整至最接近的 10）。
