...
Avrai notato che `EVAChunks` è essenzialmente un array con array al suo interno, cioè un array bidimensionale (2D). Un loop `for` annidato può essere usato per registrare dati da un array 2D:
...
Nell'esempio sopra, `rootArray` è l'array principale, `rootArray[i]` è un sotto-array all'interno dell'array principale, `rootArray[i][j]` è un oggetto dentro quel sotto-array e `.property` consulta un valore in quell'oggetto.

Usa un loop `for` annidato per iterare su `EVAChunks` e registrare il `name` di ogni astronauta in ogni blocco. Assicurati di:

- usare `EVAChunks` come array radice
- registrare `Chunk ${i+1}:` tra i blocchi

# --hints--

Dovresti usare un loop `for` annidato per iterare su `EVAChunks` e registrare il `name` di ogni astronauta in ogni blocco. Registra `Chunk ${i+1}:` tra i blocchi.
