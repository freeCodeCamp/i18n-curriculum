---
id: 65e9972a13329b53a5aff71f
title: Завдання 43
challengeType: 19
dashedName: task-43
lang: en-US
---

<!-- (Audio) Sophie: That sounds straightforward. Anything else I should keep in mind? -->

# --description--

Прослухайте аудіо та дайте відповідь на запитання.

# --questions--

## --text--

Що Софі просить у своїй інструкції?

## --answers--

Sophie is confirming that she understands everything.

### --feedback--

Софі розуміє інструкції, але запитує, чи є ще щось, що потрібно врахувати, а не просто підтверджує розуміння.

---

Sophie is asking if there are more instructions or details to consider.

---

Sophie is requesting a change in the instructions.

### --feedback--

Вона не просить змінити інструкції, а радше дізнатися, чи є додаткова інформація, яку їй потрібно знати.

---

Sophie is inquiring about a different topic.

### --feedback--

Питання Софі безпосередньо стосується поточної теми повідомлень комітів і використання Git, а не іншої теми.

## --video-solution--

2

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Sophie",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "7.1-3.mp3",
      "startTime": 1,
      "startTimestamp": 20.14,
      "finishTimestamp": 22.92
    }
  },
  "commands": [
    {
      "character": "Sophie",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sophie",
      "startTime": 1,
      "finishTime": 3.78,
      "dialogue": {
        "text": "That sounds straightforward. Anything else I should keep in mind?",
        "align": "center"
      }
    },
    {
      "character": "Sophie",
      "opacity": 0,
      "startTime": 4.28
    }
  ]
}
```
