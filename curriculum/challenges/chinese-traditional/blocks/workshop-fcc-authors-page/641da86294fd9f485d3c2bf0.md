...
接下來，添加指向 freeCodeCamp News 上作者頁面的鏈接。

Add an anchor element with a `class` attribute of `"author-link"`, interpolate `url` as the value for the `href` attribute, and set `target` to `"_blank"`. 對於錨元素的文本，插入 `author`，後跟文本 `"'s author page"`。 例如，`"Quincy Larson's author page"`。
