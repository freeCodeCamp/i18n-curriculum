...
向 `if` 語句添加 `else` 塊。 在 `else` 塊內，調用 `getCurrentSongIndex()` 函數並將其分配給名爲 `currentSongIndex` 的常量。
...
你應該在 `else` 塊內將 `currentSongIndex` 常量設置爲 `getCurrentSongIndex()`。
