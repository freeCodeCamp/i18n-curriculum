...
最后一个 `fieldset` 元素的边框看起来有点不合适。 

在之前的课程中，你学习了如何使用 `last-of-type` CSS 伪类，如下所示： 
...
创建一个定位最后一个 `fieldset` 元素的新选择器，并将其 `border-bottom` 设置为 `none`。

# --hints--

你可以使用 `:last-of-type` 伪类仅选择特定类型的最后一个元素。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type'));
```

你的 `fieldset:last-of-type` 应设置 `border-bottom` 为 `none`。
