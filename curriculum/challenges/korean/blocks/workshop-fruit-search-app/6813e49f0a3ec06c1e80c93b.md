...
첫 번째 인수를 `setTimeout`에 `async` 화살표 함수로 업데이트하세요. 이렇게 하면 지연된 로직 안에서 `await`를 사용할 수 있습니다. 그 함수 안에 데이터를 가져올 때 발생할 수 있는 오류를 처리하기 위해 `try...catch` 블록을 만드세요.

# --hints--

`async`의 첫 번째 인수로 `setTimeout` 화살표 함수가 있어야 합니다.

```js
assert.match(code, /setTimeout\s*\(\s*async\s*\(\s*\)\s*=>\s*{/);
```

`async` 함수 안에 `try...catch` 블록을 만들어야 합니다.
