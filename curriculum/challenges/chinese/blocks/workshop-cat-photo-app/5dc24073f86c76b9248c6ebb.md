...
下面是一个 `img` 元素的 `src` 属性指向 freeCodeCamp 标志的例子：
...
你的 `img` 元素应该有一个 `src` 属性。 你要么省略了属性，要么有拼写错误。 确保元素名称和属性名称之间有一个空格。

```js
assert.exists(document.querySelector('img')?.src);
```

你的 `img` 元素的 `src` 属性应设置为 '`https://cdn.freecodecamp.org/curriculum/cat-photo-app/relaxing-cat.jpg`'。 你要么省略了 URL，要么有拼写错误。 URL 的大小写很重要。
