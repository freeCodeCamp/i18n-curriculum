...
各フォーラム投稿には、そのトピックの会話に参加しているすべてのユーザーを表すユーザーアバター画像のリストが含まれます。

まず、`avatars`という名前のアロー関数を作成し、`posters`と`users`という2つのパラメータを持たせてください。

# --hints--

`avatars`という空のアロー関数があるはずです。

```js
assert.match(code, /const\s+avatars\s*=\s*\([\s\S]*\)\s*=>\s*{\s*}/);
```

`posters`という関数に、`users`と`avatars`という名前のパラメータがあるはずです。
