...
Lempel-Ziv-Welch（LZW）アルゴリズムは、ロスレスデータ圧縮を提供します。

# --instructions--

2つのパラメータを受け取る関数を書いてください。最初のパラメータはブール値で、`true`は圧縮を示し、`false`は解凍を示します。2番目のパラメータは処理対象の文字列か配列のいずれかです。圧縮する文字列の場合は数値の配列を返してください。数値の配列を解凍する場合は文字列を返してください。
