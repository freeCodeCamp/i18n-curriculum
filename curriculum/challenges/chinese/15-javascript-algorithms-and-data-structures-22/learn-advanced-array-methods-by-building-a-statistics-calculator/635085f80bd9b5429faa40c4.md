...
其实你可以把这个逻辑整理一下。 使用箭头函数的隐式返回，你可以直接返回 `.reduce()` 方法除以数组长度的值，而无需分配任何变量。

如上文所述，更新你的 `getMean` 函数。
...
你应该使用隐式返回语法直接返回 `reduce` 的结果除以数组长度。
