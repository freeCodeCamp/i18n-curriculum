...
Um den ersten Abschnitt inline-ähnlicher aussehen zu lassen, richten Sie sich nur an die `input`-Elemente innerhalb der `.info`-Elemente, setzen Sie deren `width` auf `50%` und richten Sie deren Text linksbündig aus.

# --hints--

Sie sollten entweder den `.info input`- oder den `.info > input`-Selektor verwenden.
...
Sie sollten den `input`-Elementen eine `width` von `50%` geben.
...
Sie sollten den `input`-Elementen eine `text-align` von `left` geben.
