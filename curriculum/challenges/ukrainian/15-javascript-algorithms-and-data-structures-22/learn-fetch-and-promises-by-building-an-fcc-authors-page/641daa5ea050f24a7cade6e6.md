...
Якщо ця умова виконана, вимкніть кнопку, встановивши її властивість `disabled` на `true`. Також встановіть `textContent` кнопки на `"No more data to load"`.
...
Встановіть властивість `disabled` для `loadMoreBtn` на `true`.

```js
assert.match(code, /loadMoreBtn\.disabled\s*=\s*true\s*;?/)
```

Встановіть `textContent` для `loadMoreBtn` на `"No more data to load"`.
