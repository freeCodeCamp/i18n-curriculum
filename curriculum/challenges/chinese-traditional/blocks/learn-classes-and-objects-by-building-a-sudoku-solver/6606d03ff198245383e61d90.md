...
現在，透過建立一個 `for` 迴圈，只對 3x3 方格內的列進行迭代。使用 `range()` 函式產生從 `row_start` 開始的序列，並使用 `row_no` 作為迴圈變數。

# --hints--

你應該創建一個 `for` 迴圈，迭代 `range(row_start, row_start + 3)`。
