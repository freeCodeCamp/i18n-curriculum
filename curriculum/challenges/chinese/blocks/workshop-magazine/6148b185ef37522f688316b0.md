...
现在你可以使用 CSS Grid 为 `.heading` 元素的内容设置样式。

CSS `repeat()` 函数用于重复一个值，而不是手动写出它，这对网格布局很有帮助。 例如，将 `grid-template-columns` 属性设置为 `repeat(20, 200px)` 将创建 20 列，每列 `200px` 宽。

给你的 `.heading` 元素一个 `grid-template-columns` 属性，设置为 `repeat(2, 1fr)`，以创建两个等宽的列。

# --hints--

你的 `.heading` 选择器应该有一个 `grid-template-columns` 属性，设置为 `repeat(2, 1fr)`。
