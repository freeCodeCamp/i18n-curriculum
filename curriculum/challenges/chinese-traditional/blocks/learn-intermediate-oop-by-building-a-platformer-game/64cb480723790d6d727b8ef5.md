...
在你的 `else if` 述語中，為你的 `checkpoints` 陣列添加一個 `forEach` 迴圈來迭代。將 `checkpoint` 用作回呼函式的參數名稱。

在 `forEach` 回呼中，使用加法指定運算子將 5 加到 checkpoints 的 `x` 位置。
...
你應該在 `forEach` 回呼函式內使用 `checkpoint` 參數。
...
你應該使用加法指定運算子，將 5 添加到 `checkpoint` 的 `x` 位置。
