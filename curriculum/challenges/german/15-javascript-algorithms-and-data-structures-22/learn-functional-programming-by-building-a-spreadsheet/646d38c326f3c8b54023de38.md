...
`addCharacters` gibt eine Funktion zurück, also möchtest du sie aufrufen. Übergib `char1` als das Argument.

# --hints--

Du solltest deine `addCharacters()`-Funktion in deiner `.map()`-Methode aufrufen.
...
Du solltest deinem `addCharacters()`-Aufruf `char1` als Argument übergeben.
