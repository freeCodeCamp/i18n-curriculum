...
داخل مصفوفة `allSongs`، أنشئ كائنًا بالخصائص والقيم التالية:
...
يجب أن تحتوي مصفوفة `allSongs` على قيمة واحدة.

```js
assert.equal(allSongs.length, 1);
```

يجب أن تحتوي مصفوفة `allSongs` على كائن كأول قيمة لها.

```js
assert.isObject(allSongs[0]);
```

يجب أن يحتوي الكائن في مصفوفة `allSongs` على خاصية `id` بقيمة الرقم `0`.

```js
assert.strictEqual(allSongs[0].id, 0);
```

يجب أن يحتوي الكائن في مصفوفة `allSongs` على خاصية `title` بقيمة السلسلة النصية `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

يجب أن يحتوي الكائن في مصفوفة `allSongs` على خاصية `artist` بقيمة السلسلة النصية `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

يجب أن يحتوي الكائن في مصفوفة `allSongs` على خاصية `duration` بقيمة السلسلة النصية `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

يجب أن يحتوي الكائن في مصفوفة `allSongs` على خاصية `src` بقيمة السلسلة النصية `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
