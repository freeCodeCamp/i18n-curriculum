...
画像のタグを作成し、`class` `"user-img"`を付けてください。文字列の補間を使って、先ほど分割代入した`src`を`image`属性に設定してください。`alt`属性には`author`に続けてテキスト`"avatar"`を設定してください。`author`変数と単語`"avatar"`の間には必ずスペースを入れてください。例えば、`"Quincy Larson avatar"`のようにします。
