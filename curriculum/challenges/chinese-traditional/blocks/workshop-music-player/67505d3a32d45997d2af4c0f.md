...
創建一個名為 `playPreviousSong` 的函式（程式）。在你的新函式（程式）內，使用由 `getPreviousSong` 函式（程式）傳回的值來尋找要播放的歌曲。當 `getPreviousSong()` 是 `undefined` 時，播放你的播放列表中的第一首歌曲。否則，播放目前正在播放的歌曲之前的歌曲。

# --hints--

你應該有一個名為 `playPreviousSong` 的函式（程式）。

```js
assert.isFunction(playPreviousSong);
```

`playPreviousSong` 函式（程式）應該播放播放列表中目前播放歌曲的前一首歌曲。 
...
當 `getPreviousSong()` 是 `undefined` 時，`playPreviousSong` 函式應該播放播放列表中的第一首歌曲。 
