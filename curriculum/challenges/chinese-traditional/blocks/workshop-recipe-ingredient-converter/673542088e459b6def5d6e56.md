...
你需要一個實際更新 DOM 的函式。宣告一個 `updateResultsList` 函式。

首先，該函式應該清除 `resultList` 元素中的 HTML。

# --hints--

你應該定義一個 `updateResultsList` 函式（程式）。

```js
assert.isFunction(updateResultsList);
```

你的 `updateResultsList` 函式應該將 `resultList.innerHTML` 設為空字串。
