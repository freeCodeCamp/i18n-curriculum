...
`:root`에 `window-color2` 값을 가진 `#8cd9b3`이라는 새 변수를 만드세요. 이 변수는 이 건물의 보조 색상으로 사용됩니다.

# --hints--

`window-color2` 안에 `:root`라는 새 속성 변수를 만들어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--window-color2'));
```

`window-color2`에 `#8cd9b3` 값을 할당해야 합니다.
