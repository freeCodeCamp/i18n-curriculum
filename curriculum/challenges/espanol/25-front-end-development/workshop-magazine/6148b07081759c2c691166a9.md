...
Crea un selector `.text` y dale una propiedad `grid-column` establecida en `2 / 3`.

# --hints--

Debes tener un selector `.text`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.text'));
```

Tu selector `.text` debe tener una propiedad `grid-column` establecida en `2 / 3`.
