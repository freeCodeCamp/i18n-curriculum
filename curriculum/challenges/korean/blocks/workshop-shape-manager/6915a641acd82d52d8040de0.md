...
`initializeApp` 함수로 돌아가서, 여기서 앞서 만든 인라인 타입과 `getElement()` 함수를 사용하여 앱에 필요한 DOM 요소를 가져오세요.

`initializeApp` 함수 안에서 `shapeTypeSelect`를 `getElement()`와 함께 호출하여 `"shape-type"`에 다시 할당한 다음, `as` 키워드를 사용해 결과를 `HTMLSelectElement`로 형변환하세요.

# --hints--

적절한 값으로 `shapeTypeSelect`를 다시 할당해야 합니다.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

`getElement` 함수를 `shape-type`와 함께 호출하고 `shapeTypeSelect`에 할당해야 합니다.

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

`getElement`를 `HTMLSelectElement` 타입으로 `shapeTypeSelect`에 `as` 키워드를 사용해 호출해야 합니다.
