...
بعد ذلك، اجعل التطبيق يحصل على أسماء الفواكه من API عندما يكتب المستخدم في المُدخَل.

أولًا، أضف خطاف `useEffect` ليعمل في كل مرة يتغير فيها حالة `query`. تأكد من تمرير دالة سهمية كالمعلمة الأولى إلى `useEffect`، وضمّن `query` في مصفوفة التبعيات.

# --hints--

يجب أن تستخدم خطاف `useEffect`.
...
يجب أن تضمّن `query` في مصفوفة التبعيات لخطاف `useEffect`.
