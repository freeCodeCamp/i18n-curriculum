...
Sie sollten dem zweiten `label` den Text `False` geben.
...
Sie sollten dem dritten `label` den Text `True` oder `False` geben.

```js
assert.include(['True', 'False'], document.querySelectorAll('ul.answers-list > li > label')?.[2]?.textContent?.trim());
```

Sie sollten dem vierten `label` den Text `True` geben.
...
Sie sollten dem vierten `label` den Text `False` geben.
...
Sie sollten dem ersten `input` ein `value` geben, das zum `label`-Textinhalt passt.
...
Sie sollten dem zweiten `input` ein `value` geben, das zum `label`-Textinhalt passt.
...
Sie sollten dem dritten `input` ein `value` geben, das zum `label`-Textinhalt passt.
...
Sie sollten dem vierten `input` ein `value` geben, das zum `label`-Textinhalt passt.
