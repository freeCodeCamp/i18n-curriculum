...
Створіть селектор `hr` та надайте йому властивість `margin` зі значенням `1.5rem 0`.

# --hints--

Ви повинні мати селектор `hr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('hr'));
```

Ваш селектор `hr` повинен мати властивість `margin` зі значенням `1.5rem 0`.
