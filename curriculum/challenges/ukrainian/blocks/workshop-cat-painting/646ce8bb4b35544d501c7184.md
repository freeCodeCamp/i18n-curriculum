...
Всередині вашого елемента `.cat-ears` створіть два елементи `div` з відповідними класами `cat-left-ear` та `cat-right-ear`.

# --hints--

Ви не повинні змінювати існуючий елемент `div` з класом `cat-ears`.

```js
assert.lengthOf(document.querySelectorAll('div.cat-ears'), 1);
```

Ви маєте створити два елементи `div` всередині вашого елемента `.cat-ears`.
