...
재생목록에서 노래를 삭제하는 기능을 구현할 시간입니다. 이 기능은 재생목록에서 노래를 제거하고, 노래가 삭제될 때 관련된 다른 작업을 처리하며, `Reset Playlist` 버튼을 생성합니다.

`const`과 화살표 구문을 사용해 빈 `deleteSong` 함수를 만들고 `id`을 매개변수로 전달하세요.

# --hints--

빈 함수를 만들 때 `const`를 사용해 `deleteSong`라는 이름으로 만드세요.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong`는 함수여야 합니다.

```js
assert.isFunction(deleteSong)
```

`deleteSong` 함수는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

`deleteSong` 함수는 `id` 매개변수를 받아야 합니다.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

`deleteSong` 함수는 비어 있어야 합니다.
