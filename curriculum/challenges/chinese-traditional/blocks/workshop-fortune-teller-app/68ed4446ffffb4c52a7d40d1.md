...
在建構式內，使用 `fetchCardsData()` 關鍵字呼叫 `initializeEventListeners()` 和 `this`，讓它們在類別初始化時執行。

`fetchCardsData` 方法將負責從 API 載入塔羅牌資料，而 `initializeEventListeners` 則會設定按鈕點擊的處理常式。你會在接下來的步驟中創建這兩個方法。

# --hints--

你應該在 `fetchCardsData()` 內呼叫 `constructor` 方法。
...
你應該在建構式內呼叫 `initializeEventListeners()` 方法。
