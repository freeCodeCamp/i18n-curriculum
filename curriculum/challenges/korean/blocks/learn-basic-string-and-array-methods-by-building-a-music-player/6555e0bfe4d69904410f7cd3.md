...
`const`와 화살표 구문을 사용해 `shuffle`이라는 빈 함수를 만드세요.

이 함수는 재생 목록의 노래를 섞고 섞은 후 필요한 상태 관리를 업데이트하는 역할을 합니다.

# --hints--

빈 함수 `const`을 만들 때 `shuffle`를 사용하세요.
...
`shuffle` 함수는 화살표 구문을 사용하고 매개변수를 받지 않아야 합니다.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*/)
```

`shuffle` 함수는 비어 있어야 합니다.
