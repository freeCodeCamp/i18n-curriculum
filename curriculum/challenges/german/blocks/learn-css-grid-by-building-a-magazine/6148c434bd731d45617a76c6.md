...
Wenn Sie weitere Social-Icons hinzufügen möchten, diese aber in derselben Reihe bleiben sollen, müssten Sie `grid-template-columns` aktualisieren, um zusätzliche Spalten zu erstellen. Alternativ können Sie die `grid-auto-flow`-Eigenschaft verwenden.

Diese Eigenschaft nimmt entweder `row` oder `column` als ersten Wert an, mit einem optionalen zweiten Wert `dense`. `grid-auto-flow` verwendet einen Auto-Platzierungsalgorithmus, um das Grid-Layout anzupassen. Wenn Sie ihn auf `column` setzen, weist das den Algorithmus an, bei Bedarf neue Spalten für Inhalte zu erstellen. Der Wert `dense` erlaubt dem Algorithmus, zurückzugehen und Lücken im Grid mit kleineren Elementen zu füllen, was dazu führen kann, dass Elemente außerhalb der Reihenfolge erscheinen.

Für Ihren `.social-icons`-Selektor setzen Sie die `grid-auto-flow`-Eigenschaft auf `column`.

# --hints--

Ihr `.social-icons`-Selektor sollte eine `grid-auto-flow`-Eigenschaft mit dem Wert `column` haben.
