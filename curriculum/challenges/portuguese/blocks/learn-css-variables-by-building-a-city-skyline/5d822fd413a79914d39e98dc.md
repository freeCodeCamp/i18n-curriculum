...
Você deve adicionar um `display` de `flex` à classe `background-buildings`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.background-buildings')?.display, 'flex');
```

Você deve adicionar um `align-items` de `flex-end` à classe `background-buildings`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.background-buildings')?.alignItems, 'flex-end');
```

Você deve adicionar um `justify-content` de `space-evenly` à classe `background-buildings`.
