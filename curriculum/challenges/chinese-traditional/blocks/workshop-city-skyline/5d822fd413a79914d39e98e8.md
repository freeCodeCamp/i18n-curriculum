...
為你的 `.foreground-buildings` 添加與 `.background-buildings` 相同的 `display`、`align-items` 和 `justify-content` 屬性及其值。這將再次使用 Flexbox 將建築物均勻地分佈在其容器的底部。

# --hints--

你應該為 `.foreground-buildings` 指定 `display` 屬性為 `flex`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.display, 'flex');
```

你應該為 `.foreground-buildings` 添加 `align-items` 屬性，並設為 `flex-end`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.alignItems, 'flex-end');
```

你應該為 `.foreground-buildings` 給予 `justify-content` 屬性，值為 `space-evenly`。
