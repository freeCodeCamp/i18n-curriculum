...
Тепер змініть другий аргумент кольору у функції `linear-gradient`, який наразі є чисто зеленим.

Оновіть функцію `rgb` так, щоб значення для червоного було `245`, значення зеленого — `62`, а значення синього — `113`.

# --hints--

Ваш CSS-правило `.red` має містити властивість `background`, встановлену в `linear-gradient(180deg, rgb(122, 74, 14) 0%, rgb(245, 62, 113) 50%, rgb(0, 0, 255) 100%)`.
