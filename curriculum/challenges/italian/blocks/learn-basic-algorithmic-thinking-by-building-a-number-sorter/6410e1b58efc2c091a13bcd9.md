...
Per eseguire il debug, aggiungi una chiamata `console.log()` nel tuo loop interno. Passale gli argomenti `array`, `array[j]` e `array[j+1]`.

# --hints--

Dovresti usare una chiamata `console.log()` nel tuo loop interno.
...
Dovresti passare `array[j+1]` come terzo argomento a `console.log()`.
