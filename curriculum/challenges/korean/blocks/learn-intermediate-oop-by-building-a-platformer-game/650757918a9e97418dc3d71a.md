...
마지막으로 해야 할 일은 `else if` 문장을 추가하는 것입니다.

조건문은 플레이어의 `x` 위치가 체크포인트의 `x` 위치보다 크거나 같고, 체크포인트의 `x` 위치에 `40`를 더한 값보다 작거나 같은지 확인해야 합니다.

`else if` 문장 본문 안에서 `showCheckpointScreen` 함수를 호출하고 문자열 `"You reached a checkpoint!"`을 인수로 전달해야 합니다.

축하합니다! 플랫폼 게임 프로젝트를 완료했습니다!

# --hints--

플레이어의 `else if` 위치가 체크포인트의 `x` 위치보다 크거나 같고, 체크포인트의 `x` 위치에 `x`를 더한 값보다 작거나 같은지 확인하는 `40` 절을 추가하세요.
...
`showCheckpointScreen` 함수를 호출하고 "You reached a checkpoint!" 문자열을 인수로 전달하세요.
