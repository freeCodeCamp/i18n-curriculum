...
عند عرض الأغاني على الصفحة، يجب أن تظهر العنوان، والفنان، ومدة كل أغنية، وزر الحذف.

في الخطوات التالية، ستتعلم كيفية إضافة HTML جديد لكل أغنية باستخدام طريقة <dfn>map()</dfn>. ستُشرح هذه الطريقة بمزيد من التفصيل في الخطوة التالية.

ابدأ باستخدام `const` لإعلان متغير باسم `songsHTML` وتعيين `array.map()` له.

# --hints--

يجب أن يكون لديك متغير `const` يسمى `songsHTML`.

```js
assert.match(code, /const\s+songsHTML/);
```

يجب تعيين `array.map()` إلى متغير `songsHTML`.
