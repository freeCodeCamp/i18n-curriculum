...
在你的第一個 `p` 元素中，添加一個 `span` 元素。為該 `span` 添加一個 `class` 為 `product-count`，以及一個 `id` 為 `product-count-for-id${id}`。然後，在 `span` 之後，為你的 `p` 元素添加 `name` 變數的文字。

# --hints--

你的第一個 `p` 元素應該有一個 `span` 元素。
...
你的 `span` 元素應該有一個 `class` 為 `product-count`。
...
你的 `span` 元素應該有一個 `id`，其值為 `product-count-for-id${id}`。
...
你的第一個 `p` 元素應該有 `name` 變數的文字。這應該在 span 外面。
