...
其他的文本被移位了。 通过将 `.first-paragraph::first-letter` 选择器的 `float` 属性设置为 `left`，`margin-right` 属性设置为 `1rem`，将其移回来。

# --hints--

你的 `.first-paragraph::first-letter` 选择器应该有一个 `float` 属性，设置为 `left`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

你的 `.first-paragraph::first-letter` 选择器应该有一个 `margin-right` 属性，设置为 `1rem`。
