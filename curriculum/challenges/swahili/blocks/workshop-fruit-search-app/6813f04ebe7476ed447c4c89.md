...
Ifuatayo, fanya wito `.json()` kwenye jibu na hifadhi matokeo katika kigezo kinachoitwa `data`. Kwa kuwa bado uko ndani ya kitendakazi cha `async`, unapaswa kutumia `await` kabla ya `response.json()`.

# --hints--

Unapaswa kufanya wito `json()` kwenye `response` na kuihifadhi katika kigezo cha `data`.
