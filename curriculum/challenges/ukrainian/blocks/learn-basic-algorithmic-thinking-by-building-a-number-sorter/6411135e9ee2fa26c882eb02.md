...
На кожній ітерації вашого циклу `while` він знаходить елемент, який більший за ваше поточне значення. Вам потрібно перемістити цей елемент вправо, щоб звільнити місце для вашого поточного значення.

Зробіть це, призначивши значення за індексом `j` наступному індексу.

# --hints--

Перед тим як зменшити `j`, призначте значення за `j` індексу `j + 1`.
