...
`parseCatalog` atualmente retorna um array vazio. Para preenchê-lo, adicione um loop `for` dentro da função que itere sobre `rawCards`. Dentro do loop, faça uma chamada `parseCard` em cada elemento e insira o resultado em `catalog`.

# --hints--

`parseCatalog(rawCatalogCards).length` deve ser `19`.

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

`parseCatalog` deve usar um loop `for` para iterar sobre `rawCards`.

```js
assert.match(parseCatalog.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*rawCards\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Você deve fazer uma chamada `parseCard` dentro do loop.
