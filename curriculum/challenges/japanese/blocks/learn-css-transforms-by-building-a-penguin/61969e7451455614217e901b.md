...
山をもっと山らしく見せるために、`skew` の transform 関数を使うことができます。この関数は2つの引数を取ります。1つ目はx軸をせん断する角度、2つ目はy軸をせん断する角度です。

`transform` プロパティを使って、x軸に`0deg`、y軸に`44deg` の角度で山を傾けてください。

# --hints--

`.left-mountain` に `transform` プロパティを与える必要があります。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.transform);
```

`skew` に `transform` 関数を使うべきです。

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.transform, 'skew');
```

`.left-mountain` に `transform` の `skew(0deg, 44deg)` を与えるべきです。
