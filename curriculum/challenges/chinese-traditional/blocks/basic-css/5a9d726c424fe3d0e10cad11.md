...
要創建 CSS 變數，你只需要給它一個名稱，名稱前面加上兩個連字號，並像這樣指定一個值：

```css
--penguin-skin: gray;
```

這將創建一個名為 `--penguin-skin` 的變數並指定其值為 `gray`。現在你可以在 CSS 的其他地方使用該變數，將其他屬性的值更改為灰色。

# --instructions--

在 `penguin` 類別中，創建一個名為 `--penguin-skin` 的變數並賦值為 `gray`。
