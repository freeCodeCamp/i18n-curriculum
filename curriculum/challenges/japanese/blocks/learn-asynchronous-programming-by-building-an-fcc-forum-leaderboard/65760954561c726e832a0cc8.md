...
`allCategories` オブジェクトの中に、キーが数値の `299` で値が空のオブジェクトの新しいキーを追加してください。

そのオブジェクトの中に、キーが `category` で値が文字列の `"Career Advice"` のプロパティを追加してください。そのプロパティの下に、キーが `className` で値が文字列の `"career"` の別のキーを追加してください。

# --hints--

あなたの `allCategories` オブジェクトは `299` プロパティを持つべきです。

```js
assert.property(allCategories, '299');
```

あなたの `299` の値はオブジェクトであるべきです。

```js
assert.isObject(allCategories['299'])
```

あなたの `299` オブジェクトは `category` プロパティを持つべきです。

```js
assert.property(allCategories['299'], 'category');
```

あなたの `category` プロパティは文字列の `"Career Advice"` に設定されるべきです。

```js
assert.equal(allCategories["299"].category, 'Career Advice');
```

あなたの `299` オブジェクトは `className` プロパティを持つべきです。

```js
assert.property(allCategories['299'], 'className');
```

あなたの `className` プロパティは文字列の `"career"` に設定されるべきです。
