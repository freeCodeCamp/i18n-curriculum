...
Далі використайте властивість `classList` та метод `add()`, щоб додати клас `"playing"` до елемента `playButton`. Це шукатиме клас `"playing"` у CSS-файлі та додасть його до елемента `playButton`.

Щоб нарешті відтворити пісню, використайте метод `play()` для змінної `audio`. `play()` — це метод з web audio API для відтворення mp3-файлу.

# --hints--

Ви повинні використати властивість `classList` та метод `add`, щоб додати клас `"playing"` до `playButton`.

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

Ви повинні використати метод `play()` для змінної `audio`.
