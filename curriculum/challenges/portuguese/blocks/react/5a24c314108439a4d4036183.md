...
Você também pode escrever JavaScript diretamente nos seus métodos `render`, antes da instrução `return`, ***sem*** inseri-lo dentro de chaves.Isto é devido a ainda não estar dentro do código JSX.Quando você quiser usar uma variável mais tarde no código JSX *dentro* da declaração `return`, você coloca o nome da variável dentro de chaves.
