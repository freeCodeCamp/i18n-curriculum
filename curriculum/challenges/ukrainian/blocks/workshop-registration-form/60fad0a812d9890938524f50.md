...
Щоб дати учасникам уявлення про те, що вказати у своєму біо, використовується атрибут `placeholder`. `placeholder` приймає текстове значення, яке відображається, доки користувач не почне вводити текст.

Дайте `textarea` атрибут `placeholder` зі значенням `I like coding on the beach...`.

# --hints--

Ви повинні дати `textarea` атрибут `placeholder`.

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

Ви повинні дати `placeholder` значення `I like coding on the beach...`.
