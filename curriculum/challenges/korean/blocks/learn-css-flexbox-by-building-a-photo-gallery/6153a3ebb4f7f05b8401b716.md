...
`alt` 이미지 속성은 이미지 내용을 설명해야 합니다. 화면 읽기 프로그램은 이미지 대신 대체 텍스트를 읽어 줍니다. 이미지가 로드되지 않으면 이 텍스트가 이미지 대신 표시됩니다.

프로젝트를 완료하려면, 고양이 이미지 9개 모두에 `alt` 속성을 추가해 설명하세요. 각 속성 값은 최소 5자 이상이어야 합니다.


# --hints--

고양이 이미지 9개 각각의 `img` 요소에 `alt` 속성을 넣어 이미지 내용을 설명하세요. 각 설명은 최소 `5` 글자 이상이어야 합니다.
