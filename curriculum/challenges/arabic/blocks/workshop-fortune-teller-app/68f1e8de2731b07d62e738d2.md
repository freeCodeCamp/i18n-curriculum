...
أعلن عن متغير آخر باسم `foundCard`، وقم بتعيينه إلى نتيجة استدعاء الطريقة `find()` على `cards`. مرر `card` كمعلمة استدعاء راجع، وقارن بين `card.name_short` و `cardId` للعثور على البطاقة المطابقة باستخدام المساواة الصارمة.

# --hints--

يجب أن تعلن عن متغير باسم `foundCard` داخل `showFortune`.
...
يجب تعيين `foundCard` الخاص بك عن طريق استدعاء `find` على `this.cards` مع استدعاء راجع صحيح.
...
يجب أن يحتوي استدعاء راجع `find` على معلمة باسم `card`.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

داخل الاستدعاء الراجع، يجب مقارنة `card.name_short` مع `cardId`.
