...
最後，你應該透過一些重構來整理你的程式碼！你可能會注意到你的程式碼在 `getHighestDuplicates`、`detectFullHouse`` 和 `checkForStraights` 函式中多次啟用了最後一個圓鈕（「None of the Above」）。請重構你的程式碼，使得啟用最後一個圓鈕不再在每個函式結尾執行，而只在 `rollDiceBtn` 中執行一次。

# --hints--

無論是否出現重複，你的 `getHighestDuplicates` 函式都不應該啟用最後一個圓鈕，或設定最後一個 `span` 文字。
...
你的 `detectFullHouse` 函式不應該啟用最後的圓鈕，或設定最後的 `span` 文字，無論是否擲出滿堂彩。
...
你的 `checkForStraights` 函式不應該啟用最後一個圓鈕，或設定最後一個 `span` 文字，無論是否擲出順子。
...
當你的 `rollDiceBtn` 被點擊時，你應該啟用最後一個圓鈕，將值設為 `0`，並將顯示的文字更新為 `, score = 0`。
