...
Le impostazioni predefinite per CSS Grid creeranno righe aggiuntive secondo necessità, a differenza di Flexbox. Imposta la proprietà `grid-template-columns` del selettore `.social-icons` su `repeat(5, 1fr)` per disporre le icone in un'unica riga.
