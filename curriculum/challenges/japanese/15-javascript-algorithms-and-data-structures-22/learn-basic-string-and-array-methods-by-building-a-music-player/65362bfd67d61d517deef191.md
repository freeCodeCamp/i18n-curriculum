...
<dfn>find()</dfn> メソッドは、配列内の要素のうち、引数として与えられたコールバック関数内に定義されている条件を満たす最初の要素を取得します。 条件を満たす要素が無い場合は `undefined` を返します。

たとえば、`find()` メソッドを使って「25 より大きい」という条件を満たす最初の数値を見つける例がこちらです。
...
では `const` を使って `song` という名前の変数を作成し、`userData?.songs` 配列に対して `find()` メソッドを呼び出した結果を割り当ててください。 `find()` メソッドのコールバック関数の仮引数として `song` を使用し、`song.id` が `id` と厳密に等しいかチェックしてください。

これで `userData?.songs` 配列を反復処理して、`playSong` 関数に渡された `id` に対応する曲を探すことができます。

# --hints--

`userData?.songs` に `find` メソッドを使ってください。

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

`find` メソッドのコールバックのアロー関数は、仮引数として `song` を持つようにしてください。

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

`find` メソッドは、`song.id` が `id` と等しいかのチェックに厳密等価を使用する必要があります。

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

`find` メソッドの結果を定数 `song` に割り当ててください。
