...
fCC Authors ページが完全に機能するようになったので、Promise チェーンのメソッド `async` の代わりに `await` と `then` を使って、可読性を向上させるためにリファクタリングしましょう。

`await` 演算子を使って `Promise` を返す関数の完了を待つには、その関数呼び出しを `async` 関数でラップする必要があることを思い出してください（モジュールのメインボディ内でない場合）。

`fetch` の呼び出しは非同期関数でもモジュールのメインボディでも定義されていないため、`await` を使う前にそれを修正する必要があります。

`fetch` 文とチェーンされた then と catch メソッドのシーケンス全体を、引数を取らない新しい非同期アロー関数 `initialFetch` でラップしてください。

`async` 関数宣言の例は以下の通りです：
...
`initialFetch` は関数である必要があります。

```js
assert.isFunction(initialFetch)
```

`const` を使って `initialFetch` 関数を作成してください。

```js
assert.match(code, /const\s+initialFetch\s*=\s*/)
```

`initialFetch` は非同期関数である必要があります。
...
`initialFetch` 関数はパラメータを取らないでください。

```js
assert.match(code, /const\s+initialFetch\s*=\s*.*\s*\(\s*\)\s*/)
```

`initialFetch` 関数はアロー構文を使ってください。

```js
assert.match(code, /const\s+initialFetch\s*=\s*.*\s*\(.*\)\s*=>\s*/)
```

`initialFetch` 関数は空であってはいけません。
...
`initialFetch` 関数には `fetch` 文とチェーンされたメソッドを含めてください。
