...
Geben Sie der `sky` Klasse eine `radial-gradient`. Verwenden Sie `#ffcf33` von `0%` bis `20%`, `#ffff66` bei `21%` und `#bbeeff` bei `100%`. Dadurch wird ein kreisförmiger Farbverlauf zum Hintergrund hinzugefügt, der Ihre Sonne darstellt.

# --hints--

Sie sollten `.sky` eine `radial-gradient` in der `background` Eigenschaft geben.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".sky")?.background, "radial-gradient");
```

Sie sollten der `radial-gradient` eine erste Farbe von `#ffcf33` geben.

```js
assert.match(new __helpers.CSSHelp(document).getStyle(".sky")?.background, /radial-gradient\(rgb\(255, 207, 51\)( 0%)?, rgb\(255, 207, 51\) 20%/);
```

Sie sollten der `radial-gradient` eine zweite Farbe von `#ffff66` bei `21%` geben.
...
Sie sollten der `radial-gradient` eine dritte Farbe von `#bbeeff` bei `100%` geben.
