...
Створіть ще один `@media` запит для `only screen` з `max-width` `600px`. Всередині створіть правило `.text-with-images` і надайте йому властивість `grid-template-columns` зі значенням `1fr`.

Це звузить вашу нижню текстову область до одного стовпця на менших екранах.

# --hints--

Ви повинні створити новий `@media` запит для `only screen and (max-width: 600px)`. Він має бути розташований нижче вашого попереднього `@media` запиту.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

Ваш новий `@media` запит має мати селектор `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

Ваш новий селектор `.text-with-images` має мати властивість `grid-template-columns` зі значенням `1fr`.
