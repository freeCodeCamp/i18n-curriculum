...
Ni wakati wa kuanza kutekeleza kitendakazi cha kucheza nyimbo zilizoonyeshwa.

Fafanua kitendakazi cha `playSong` ukitumia `const`. Kitendakazi hicho kinapaswa kuchukua kigezo cha `id` ambacho kitawakilisha kitambulisho cha kipekee cha wimbo unayotaka kuucheza.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi tupu kinachoitwa `playSong`.

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

`playSong` inapaswa kuwa kitendakazi.

```js
assert.isFunction(playSong)
```

Kitendakazi chako cha `playSong` kinapaswa kutumia sintaksia ya mshale.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Kitendakazi chako cha `playSong` kinapaswa kuchukua kigezo cha `id`.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

Kitendakazi chako cha `playSong` kinapaswa kuwa tupu.
