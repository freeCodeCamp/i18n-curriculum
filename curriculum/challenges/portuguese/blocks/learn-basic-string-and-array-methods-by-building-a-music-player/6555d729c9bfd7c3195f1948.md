...
`textContent` define o texto de um nó e permite que você defina ou recupere o conteúdo de texto de um elemento HTML.
...
Você pode usar um operador ternário para definir condicionalmente o valor do conteúdo de texto. Aqui está um exemplo de atribuição do resultado de um operador ternário a uma variável:

```js
const example = condition ? "I'm true" : "I'm false";
```

Use um operador ternário para verificar se `currentTitle` avalia para um valor truthy. Se isso acontecer, defina `playingSong.textContent` como `currentTitle`. Caso contrário, defina-o como uma string vazia.

Então abaixo disso, use um operador `ternary` para verificar se `currentArtist` é truthy. Se for o caso, defina `songArtist.textContent` como `currentArtist`. Caso contrário, defina como string vazia. 

# --hints--

Você deve encadear a propriedade `textContent` a `playingSong`.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

Você deve usar o operador `ternary` para definir a propriedade `textContent` de `playingSong` para `currentTitle` ou `""`.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

Você deve encadear a propriedade `textContent` a `songArtist`.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

Você deve usar o operador `ternary` para definir a propriedade `textContent` de `songArtist` para `currentArtist` ou `""`.
