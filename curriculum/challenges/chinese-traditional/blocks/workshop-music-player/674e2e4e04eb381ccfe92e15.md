...
為你的 `pauseButton` 元素添加一個 `"click"` 事件監聽器，然後將 `pauseSong` 作為事件監聽器的第二個引數傳入。這是事件監聽器將要執行的函式。

先點擊播放按鈕，然後點擊暫停按鈕來測試你的應用程式。你應該會看到一切運作如預期。

# --hints--

你應該在你的 `pauseButton` 變數上呼叫 `addEventListener()` 方法。
...
你應該將 `pauseSong` 作為你的 `addEventListener` 方法的第二個引數傳入。
