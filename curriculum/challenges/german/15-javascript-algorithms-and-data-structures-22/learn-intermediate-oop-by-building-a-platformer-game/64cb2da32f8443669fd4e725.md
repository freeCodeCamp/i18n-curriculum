...
Der nächste Schritt besteht darin, die `width` und `height` zur `CheckPoint`-Klasse hinzuzufügen.

The `width` and `height` should be `proportionalSize(40)` and `proportionalSize(70)` respectively.

# --hints--

Du solltest eine `width`-Eigenschaft in der `CheckPoint`-Klasse haben.
...
Du solltest eine `height`-Eigenschaft in der `CheckPoint`-Klasse haben.
...
Du solltest die `width`-Eigenschaft `proportionalSize(40)` zuweisen.
...
Du solltest die `height`-Eigenschaft `proportionalSize(70)` zuweisen.
