...
`header` を整理して、_Flexbox_ を使って子要素の間にスペースを作り、縦方向に中央揃えにしてください。

# --hints--

`header` に `justify-content` の `space-between` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

`header` に `align-items` の `center` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

`header` に `top` の `0` を設定してください。
