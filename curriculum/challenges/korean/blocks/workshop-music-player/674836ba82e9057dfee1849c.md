...
다음으로, `classList` 속성과 `add()` 메서드를 사용해 `playing` 클래스를 `playButton` 요소에 추가하세요. 이렇게 하면 CSS 파일에서 `playing` 클래스를 찾아 `playButton` 요소에 추가합니다.

그다음 `userData.currentSong`을 `song`로 설정하고, 마지막으로 노래를 재생하려면 `play()` 변수에서 `audio` 메서드를 호출하세요. `play()`은 mp3 파일을 재생하기 위한 웹 오디오 API의 메서드입니다.

# --hints--

`playing` 함수 내에서 `playButton` 요소에 `playSong` 클래스를 추가해야 합니다.
...
`userData.currentSong` 함수 내에서 `song`을 `playSong`로 설정해야 합니다.
...
`play()` 함수 내에서 `audio` 변수에 `playSong` 메서드를 호출해야 합니다.
