...
Усі основні функції тепер на місці. Єдина проблема — наступна пісня не починає автоматично відтворюватися, коли закінчується поточна.

Щоб це виправити, можна додати слухача подій, який визначатиме, коли поточна пісня закінчиться. Подія `"ended"` підходить для цього. Вона спрацьовує, коли відтворення медіаконтенту доходить до кінця.

Додайте слухача подій до елемента `audio`, який слухатиме подію `"ended"`. Передайте функцію зворотного виклику у вигляді стрілкової функції з порожніми фігурними дужками.

# --hints--

Ви маєте викликати метод `addEventListener()` у вашій змінній `audio`.

```js
assert.match(code, /audio\.addEventListener\(/)
```

Слухач подій, який ви використали у змінній `audio`, має слухати подію `"ended"`.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1/)
```

Використайте стрілкову функцію, щоб передати порожню функцію зворотного виклику у ваш слухач подій `"ended"`.
