...
Deklarieren Sie eine `rangeFromString` Pfeilfunktion, die zwei Parameter, `num1` und `num2`, entgegennimmt. Die Funktion soll implizit das Ergebnis des Aufrufs von `range` mit `num1` und `num2` als Argumente zurückgeben.

Um sicherzugehen, parsen Sie `num1` und `num2` in ganze Zahlen, während Sie sie an `range` übergeben.

# --hints--

Sie sollten eine `rangeFromString` Variable nach Ihrer `rangeRegex` deklarieren.
...
Sie sollten `const` verwenden, um Ihre `rangeFromString` Variable zu deklarieren.
...
Ihre `rangeFromString` Variable sollte eine Pfeilfunktion sein.
...
Ihre `rangeFromString` Funktion sollte `num1` als ersten Parameter haben.
...
Ihre `rangeFromString` Funktion sollte `num2` als zweiten Parameter haben.
...
Ihre `rangeFromString` Funktion sollte eine implizite Rückgabe verwenden.
...
Ihre `rangeFromString` Funktion sollte das Ergebnis des Aufrufs Ihrer `range` Funktion zurückgeben.
...
Sie sollten `parseInt` mit `num1` als Argument aufrufen und das Ergebnis an den `range` Aufruf übergeben.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

Sie sollten `parseInt` mit `num2` als Argument aufrufen und das Ergebnis an den `range` Aufruf übergeben.
