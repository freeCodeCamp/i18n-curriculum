...
在 `checkUserInput` 函數中，刪除 `console.log()` 語句。 然後，調用 `decimalToBinary` 函數並將 `numberInput` 的 `value` 作爲參數傳入。 另外，請確保使用 `parseInt()` 函數將輸入轉換爲數字。

# --hints--

你應該從 `checkUserInput` 函數中刪除 `console.log()` 語句。

```js
assert.notMatch(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*console\.log\((\s|.)*\)/);
```

你應該在 `checkUserInput` 函數主體內的 `if` 語句之後調用 `decimalToBinary` 函數。

```js
assert.match(String(checkUserInput), /(?:window\.|globalThis\.)?alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*decimalToBinary\(/);
```

你應該使用 `parseInt()` 將 `numberInput` 的 `value` 轉換爲數字，並將其作爲參數傳遞給 `decimalToBinary` 函數。
