...
邏輯操作符可以用來構建更復雜的媒體查詢。 `and` 邏輯操作符用於查詢兩個媒體條件。

例如，針對顯示寬度在 500px 和 1000px 之間的媒體查詢就是這樣：
...
添加另一個 `@media` 規則，條件是瀏覽器窗口大於 `769px` 但小於 `1199px`。
...
你的 `@media` 查詢應該設置 `min-width` 爲 `769px`，設置 `max-width` 爲 `1199px`。
