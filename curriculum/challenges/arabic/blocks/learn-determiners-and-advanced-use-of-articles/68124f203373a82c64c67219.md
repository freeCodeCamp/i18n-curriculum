---
id: 68124f203373a82c64c67219
title: المهمة 84
challengeType: 22
dashedName: task-84
lang: en-US
---

<!-- (Audio) Bob: It's important that no part of the project gets delayed. -->

# --instructions--

استمع إلى الصوت وأكمل الجملة أدناه.

# --fillInTheBlank--

## --sentence--

`It's important that BLANK part of BLANK project gets delayed.`

## --blanks--

`no`

### --feedback--

هذه الأداة تحدد معنى "لا يوجد أي"؛ تُظهر نفيًا تامًا.

---

`the`

### --feedback--

تُستخدم هذه الأداة لأنهم يتحدثون عن مشروع محدد ومعروف.

# --explanation--

تستخدم عبارة `No part` كلمة `no` لتعني عدم وجود أي جزء على الإطلاق - نفي تام.

تستخدم عبارة `The project` كلمة `the` لأنها تشير إلى المشروع المحدد الذي يناقشه بوب وأليس بالفعل.

# --scene--

```json
{
  "setup": {
    "background": "company1-boardroom.png",
    "characters": [
      {
        "character": "Bob",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_20-2.mp3",
      "startTime": 1,
      "startTimestamp": 47.24,
      "finishTimestamp": 50.5
    }
  },
  "commands": [
    {
      "character": "Bob",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Bob",
      "startTime": 1,
      "finishTime": 4.26,
      "dialogue": {
        "text": "It's important that no part of the project gets delayed.",
        "align": "center"
      }
    },
    {
      "character": "Bob",
      "opacity": 0,
      "startTime": 4.76
    }
  ]
}
```
