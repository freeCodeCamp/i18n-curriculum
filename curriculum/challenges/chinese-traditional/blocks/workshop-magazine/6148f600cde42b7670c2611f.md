...
為 `only screen` 且 `max-width` 為 `600px` 創建另一個 `@media` 查詢。在其中，創建 `.text-with-images` 規則並為其設定 `grid-template-columns` 屬性為 `1fr`。

這將在較小的螢幕上將你的底部文字區域摺疊成單一行。

# --hints--

你應該為 `only screen and (max-width: 600px)` 創建一個新的 `@media` 查詢。這應該放在你之前的 `@media` 查詢之下。
...
你的新 `.text-with-images` 選擇器應該有一個 `grid-template-columns` 屬性，值為 `1fr`。
