...
Die Missionskontrolle benötigt eine Methode, um EVA-berechtigte Astronauten aus einer Crew zu kopieren. Erstellen Sie eine leere Funktion namens `getEVAReadyCrew`, die einen `crew`-Parameter akzeptiert.

# --hints--

Sie sollten eine Funktion namens `getEVAReadyCrew` erstellen.

```js
assert.isFunction(getEVAReadyCrew);
```

Ihre `getEVAReadyCrew`-Funktion sollte einen `crew`-Parameter haben.
