...
Ili miguu ya pengwini ionekane zaidi kuwa ya pengwini, pinda mguu wa kushoto kwa `80deg`, na wa kulia kwa `-80deg`.

# --hints--

Unapaswa kumpa `.foot.left` `transform` ya `rotate(80deg)`.
...
Unapaswa kumpa `.foot.right` `transform` ya `rotate(-80deg)`.
