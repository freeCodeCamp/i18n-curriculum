...
두 개의 `make_allowed_move()` 절 안에서 `elif` 함수를 다시 호출하고, 올바른 인수를 전달하세요.

# --hints--

첫 번째 `make_allowed_move(source, auxiliary)` 절 안에서 `elif`를 호출해야 합니다.
...
두 번째 `make_allowed_move(auxiliary, target)` 절 안에서 `elif`를 호출해야 합니다.
