...
在你的 `tbody` 元素中，添加四个 `tr` 元素。 将前三个的 `class` 属性设置为 `data`，第四个的 `class` 属性设置为 `total`。

# --hints--

你的 `tbody` 元素应该有四个 `tr` 元素。
...
你的前三个 `tr` 元素应将 `class` 属性设置为 `data`。
...
你的第四个 `tr` 元素应将 `class` 属性设置为 `total`。
