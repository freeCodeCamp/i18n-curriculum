...
Todavía dentro del tercer elemento `li`, crea un elemento `ul` para el menú emergente. Asígnale un `className` de `sub-menu` y un `aria-label` de `Apps`.

# --hints--

Debes crear un elemento `ul` dentro de tu tercer elemento `li`.

```js
assert.exists(document.querySelector('li ul'));
```

Debes establecer el atributo `className` de tu elemento `ul` a `sub-menu`.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

Debes establecer el atributo `aria-label` de tu elemento `ul` a `Apps`.
