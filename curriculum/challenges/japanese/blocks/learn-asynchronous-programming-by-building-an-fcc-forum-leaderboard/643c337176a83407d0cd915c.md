...
データの結果を確認するには、`data` ブロック内で `try` 変数をコンソールにログしてください。

`fetchData` の定義の下で関数を呼び出し、コンソールを開いて結果を確認しましょう。

# --hints--

`console.log` 変数をコンソールにログするには `data` を使うべきです。

```js
assert.match(code, /\s*console\.log\(\s*data\s*\)/);
```

`console.log` は `try` ブロック内にあるべきです。

```js
assert.match(code, /\s*try\s*{[\s\S]*\s*console\.log\(\s*data\s*\)[\s\S]*}\s*catch\s*\(\s*err\s*\)\s*{\s*}/);
```

`fetchData()` 関数が定義された後に `fetchData()` 関数を呼び出すべきです。
