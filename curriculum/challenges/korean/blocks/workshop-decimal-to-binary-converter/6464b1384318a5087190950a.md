...
이제 올바른 요소를 선택했으니, 앞서 지정한 지연 후에 해당 요소의 텍스트를 업데이트할 수 있습니다.

`textContent` 메서드에서 체이닝을 사용해 선택한 요소의 `msg` 속성을 현재 객체의 `.getElementById()` 속성과 같게 설정하세요.

# --hints--

`.textContent` 메서드에 `.getElementById()`을 체이닝해야 합니다.
...
선택한 요소의 `textContent` 속성을 현재 객체의 `msg` 속성과 같게 설정해야 합니다.
