...
最後に、正規表現が完全な単語にのみマッチするようにしてください。(`freeRegex` でも同じことをしました。)

# --hints--

正規表現 `stockRegex` には、非キャプチャグループを使ってください。

```js
assert.match(stockRegex.source, /\(\?:/);
```

`stockRegex` では非キャプチャグループを使用して、`\s` または `^` にマッチさせてください。

```js
assert.match(stockRegex.source, /\(\?:(\^\|\\s|\\s\|\^)\)/);
```

`stockRegex` に、2 つ目の非キャプチャグループを使ってください。
...
`stockRegex` が `it's stock alert time` にマッチするようにしてください。

```js
assert.match("it's stock alert time", stockRegex);
```

`stockRegex` が `stock alert time` にマッチするようにしてください。

```js
assert.match('stock alert time', stockRegex);
```

`stockRegex` が `it's stock alert` にマッチするようにしてください。
...
`stockRegex` が `hands-stock alert time` とマッチしないようにしてください。

```js
assert.notMatch('hands-stock alert', stockRegex);
```

`stockRegex` が `stock alert-management` とマッチしないようにしてください。
