...
Тепер настав час створити функцію, яка відповідатиме за відображення авторів на сторінці.

Почніть зі створення функції з назвою `displayAuthors`, яка прийматиме параметр `authors`. Ваша функція має ітерувати над масивом `authors` і додавати картку для кожного автора до елемента `authorContainer`.

Картка повинна мати таку структуру:
...
- Ви можете використовувати будь-який тип циклу, щоб ітерувати над масивом `authors`. Наприклад, `for`, `forEach`, `map` тощо.
- Ви повинні використовувати властивість `innerHTML`, щоб додати картку до `authorContainer` всередині циклу. Також вам знадобиться синтаксис шаблонних рядків для додавання розмітки.
...
Ваша функція `displayAuthors` має приймати параметр `authors`.
...
Коли функція `displayAuthors` викликається з масивом авторів, вона має додати картку для кожного автора до елемента `#authorContainer`.
