...
Du solltest eine Funktion mit dem Namen `b` erstellen.

```js
assert.isFunction(b);
```

Du solltest eine Funktion mit dem Namen `c` erstellen.

```js
assert.isFunction(c);
```

Deine `b`-Funktion sollte `"is " + c()` zurückgeben.

```js
assert.match(String(b), /return\s*('|"|`)is\s*\1\s*\+\s*c\s*\(\s*\)\s*;?/);
```

Deine `c`-Funktion sollte `"awesome!"` zurückgeben.
