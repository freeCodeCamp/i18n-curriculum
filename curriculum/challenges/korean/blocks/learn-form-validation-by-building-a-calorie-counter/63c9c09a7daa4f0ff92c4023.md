...
같은 구문을 사용하여 `number` 요소에서 `#lunch` 입력을 쿼리하고 `lunchNumberInputs`에 할당하세요.

# --hints--

`lunchNumberInputs` 변수를 선언해야 합니다.

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=/);
```

`document.querySelectorAll()`를 사용하여 `#lunch input[type='number']`를 호출해야 합니다.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` 호출 결과를 `lunchNumberInputs`에 할당해야 합니다.
