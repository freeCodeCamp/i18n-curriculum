...
Speichern Sie innerhalb des `try`-Blocks die aktuellen Spieldaten in `localStorage`, indem Sie `localStorage.setItem` mit dem Speicher-Schlüssel und dem als String formatierten Spielerobjekt aufrufen.

Und im `catch`-Block protokollieren Sie den String `Failed to save player data:` gefolgt vom Parameter `error` in der Konsole.

# --hints--

Ihr `useEffect`-try-Block sollte `localStorage.setItem` mit `STORAGE_KEY` und dem als String formatierten `player`-Objekt aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

Ihr `useEffect`-catch-Block sollte `'Failed to save player data:'` und `error` in der Konsole protokollieren.
