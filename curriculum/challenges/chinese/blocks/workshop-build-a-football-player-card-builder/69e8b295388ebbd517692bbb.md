...
在 `card-image-wrap` 元素后，添加一个 `div`，其 `className` 为 `card-name-strip`。在其中，添加一个 `span`，其 `className` 为 `card-name`，并包含文本 `PELE`。

# --hints--

你应该添加一个 `div`，其 `className` 为 `card-name-strip`。

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

你应该在 `span` 内添加一个 `className`，其 `card-name` 为 `.card-name-strip`。

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

`.card-name` 元素应包含文本 `PELE`。
