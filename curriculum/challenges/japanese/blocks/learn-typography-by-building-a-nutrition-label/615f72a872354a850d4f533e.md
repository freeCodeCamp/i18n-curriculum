...
`.daily-value p` セクション内のすべての `p` 要素を対象にする `daily-value` セレクターを作成してください。この新しいセレクターに `border-bottom` を `1px solid #888989` に設定してください。

# --hints--

新しい `.daily-value p` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

`.daily-value p` セレクターには `border-bottom` プロパティが `1px solid #888989` に設定されているはずです。
