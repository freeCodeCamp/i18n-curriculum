...
在最后一个 `.divider` 之后，创建另一个 `p` 元素，其中包含文本 `Trans Fat 0g`。 通过将单词 `Trans` 包装在 `i` 元素中，将其变为斜体。 将新的 `p` 元素的 `class` 属性设置为 `indent no-divider`。 将 `Trans Fat 0g` 放在一个 `span` 元素中，以对齐它。
