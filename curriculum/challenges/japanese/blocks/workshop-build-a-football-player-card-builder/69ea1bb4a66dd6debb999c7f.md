...
これからプレイヤーのポジション選択を扱います。入力をセレクトにリファクタリングし、有効なサッカーのポジションの配列をマップして、セレクトのオプションの値にしてください。

まず、`POSITIONS` 配列を作成し、`GK`、`CB`、`LB`、`RB`、`CDM`、`CM`、`CAM`、`LW`、`RW`、`ST`、`CF` の文字列の値を入れてください。閉じる大括弧のすぐ後に `as const` を追加して、TypeScript が一般的な `string[]` からこれらの正確なリテラル値の読み取り専用タプルに型を絞り込むようにします。

これにより、TypeScript はどの文字列が有効かを正確に把握でき、単に文字列の配列であることだけを知るのではなくなります。

# --hints--

`POSITIONS` 定数を宣言してください。
...
`POSITIONS` 配列には必要なすべてのサッカーのポジションの文字列を含め、`as const` を使って型を読み取り専用の文字列リテラルに絞り込んでください。
