...
ستبدأ الآن عملية تحويل مُدخَل الموقع إلى عنصر اختيار.

غيّر المُدخَل إلى عنصر `select` له المعرف `position`، و`className` بقيمة `input`، و`value` بقيمة `player.position`.

# --hints--

يجب تغيير عنصر `input` الخاص بالموضع إلى عنصر `select` مع `id` يساوي `position` و`className` يساوي `input`.
...
يجب أن يحتوي عنصر `select` الخاص بالموقع على `value` بقيمة `player.position`.
