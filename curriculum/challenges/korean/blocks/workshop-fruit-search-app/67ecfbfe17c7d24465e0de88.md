...
`form` 요소 안에서, 레이블 다음에 `id`를 `search-input`으로, `type`를 `search`으로 하는 입력을 추가하세요.

# --hints--

`input` 요소 안에 `form`를 `id`으로 하는 `search-input`가 있어야 합니다.
...
`input`에는 `type` 값을 가진 `search`이 있어야 합니다.
