...
O seletor `.hero-title` deve ter a `font-size` definida como `6rem`.
...
O seletor `.social-icons` deve ter a `font-size` definida como `2rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize, '2rem');
```

O seletor `.text` deve ter a `font-size` definida como `1.6rem`.
