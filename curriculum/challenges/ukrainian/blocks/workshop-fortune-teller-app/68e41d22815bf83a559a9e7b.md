...
Тепер використайте подію `onload`, щоб видалити клас `hidden` під час завантаження зображення та сховати заповнювач завантаження, використовуючи його `previousElementSibling`.

# --hints--

Ваш елемент `img` має мати обробник `onload`.
...
Ваш обробник `onload` має використовувати `classList.remove` для видалення класу `hidden`. Не забудьте, що тут потрібне ключове слово `this`.
...
Ваш обробник `onload` має встановити `previousElementSibling.style.display` у `'none'`. Не забудьте, що для цього потрібне ключове слово `this`.
