...
これまで、`.face` と `.chin` 要素は同じ `background-color` を持っています。

`--penguin-face` というカスタム CSS プロパティを作成し、`white` に設定してください。
...
`:root` に `--penguin-face` プロパティを与えるべきです。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--penguin-face'));
```

`:root` に `--penguin-face` を `--fcc-expected--` に設定するべきですが、`--fcc-actual--` が見つかりました。
