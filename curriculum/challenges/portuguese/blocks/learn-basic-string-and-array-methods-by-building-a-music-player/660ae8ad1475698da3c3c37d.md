...
Para chamar uma expressão de função arrow nomeada, você pode referenciar a função pelo seu nome. 

```js
exampleArrowFunction();
```

Abaixo da definição da sua função `printGreeting`, chame a função. Abra o console para ver a saída.

# --hints--

Você deve chamar sua função `printGreeting`.

```js
assert.match(code, /printGreeting\(\);?/);
```

A chamada da sua função `printGreeting` deve estar abaixo da definição da função `printGreeting`.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

A chamada da sua função `printGreeting` deve exibir `Hello there!` no console. 
