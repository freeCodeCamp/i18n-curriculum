...
마지막으로, 특정 유형의 움직임 기반 애니메이션은 일부 사용자에게 불편함을 줄 수 있습니다. 특히, <dfn>전정기관</dfn> 장애가 있는 사람들은 특정 움직임 자극에 민감합니다.

`@media` @규칙에는 사용자의 애니메이션 선호도에 따라 CSS를 설정하는 `prefers-reduced-motion`라는 미디어 기능이 있습니다. 이 기능은 다음 값 중 하나를 가질 수 있습니다:
...
이 기능에 대해서는 앞으로 있을 수업에서 더 배우게 됩니다. 지금은 `scroll-behavior: smooth`를 설정하는 스타일 규칙을 `@media` @규칙 안에 감싸고, 미디어 기능 `prefers-reduced-motion`에 `no-preference` 값을 설정하세요.

잘했습니다. 접근성 퀴즈 연습 프로젝트를 완료했습니다.

# --hints--

`@media (prefers-reduced-motion: no-preference)` 규칙이 하나 있어야 합니다.

```js
assert.lengthOf(new __helpers.CSSHelp(document).getRuleListsWithinMedia('(prefers-reduced-motion: no-preference)'), 1);
```

기존 `*` 규칙을 `@media` 규칙 안에 감싸야 합니다.
