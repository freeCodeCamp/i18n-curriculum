...
Um método é uma função que pertence a um objeto ou classe específicos. Você vai aprender mais sobre classes, objetos, funções e métodos mais adiante. Por enquanto, saiba que os métodos precisam ser chamados no objeto ao qual pertencem, escrevendo o objeto seguido de um ponto e da chamada do método. Python oferece vários métodos embutidos que você pode usar para manipular strings. Eles incluem, mas não se limitam a, os seguintes:
