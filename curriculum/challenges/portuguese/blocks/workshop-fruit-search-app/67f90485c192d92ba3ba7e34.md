...
Dentro do callback `useEffect`, adicione uma condição `if` para verificar se `query.trim() === ''`.Isso verifica se `query` é uma string vazia ou contém apenas espaços.Se for o caso, chame `setResults([])` e retorne antecipadamente.

# --hints--

Você deve verificar se `query.trim() === ''`.
...
Se `query.trim() === ''` for verdadeiro, chame `setResults()` com um array vazio e depois use a palavra-chave `return`.
