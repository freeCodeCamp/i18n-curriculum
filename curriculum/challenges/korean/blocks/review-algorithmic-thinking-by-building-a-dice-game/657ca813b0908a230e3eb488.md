...
게임의 각 라운드에서 사용자는 최대 세 번까지 주사위를 굴릴 수 있습니다. 사용자가 `rollDiceBtn`를 클릭했지만 이미 세 번 굴렸다면, 브라우저는 점수를 선택해야 한다는 것을 알리기 위해 `alert()`를 표시해야 합니다. 그렇지 않으면 현재처럼 주사위를 굴리고 `rolls` 변수를 증가시켜야 합니다.

# --hints--

이벤트 리스너가 주사위를 굴리도록 하세요.
...
주사위를 세 번 굴렸다면 브라우저가 경고창을 표시해야 합니다.
