...
你可以使用 `else if` 使 `else` 語句具有條件。 這是一個示例：
...
在 `if` 語句末尾，添加 `else if` 語句來檢查 `monsterHealth` 是否小於或等於 `0`。 在你的 `else if` 中，調用 `defeatMonster` 函數。
...
你的 `else if` 語句應檢查 `monsterHealth` 是否小於或等於 `0`。

```js
assert.match(attack.toString(), /else\s+if\s*\(\s*monsterHealth\s*<=\s*0\s*\)/);
```

你的 `else if` 語句應該調用 `defeatMonster` 函數。
