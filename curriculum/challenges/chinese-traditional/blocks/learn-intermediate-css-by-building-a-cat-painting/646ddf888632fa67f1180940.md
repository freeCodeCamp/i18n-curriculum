...
將左眼移動到合適的位置，設置 `position` 爲 `absolute`、`top` 爲 `54px`，`left` 爲 `39px`。

# --hints--

你的 `.cat-left-eye` 選擇器應設置 `position` 屬性爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.position === 'absolute')
```

你的 `.cat-left-eye` 選擇器應設置 `top` 屬性爲 `54px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-eye')?.top === '54px')
```

你的 `.cat-left-eye` 選擇器應設置 `left` 屬性爲 `39px`。
