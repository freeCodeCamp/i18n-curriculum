...
你的 `ShoppingCart` 类需要添加项的功能。创建一个空的 `addItem` 方法，该方法接受两个参数：`id` 和 `products`。创建方法可能看起来像这样：
...
第一个参数 `id` 是用户添加到购物车中的产品的 `id`。第二个参数 `products` 是一个 `Dessert` 实例的数组。通过使用参数而不是直接引用你现有的 `products` 数组，如果你以后想添加额外的产品列表，这个方法会更加灵活。

# --hints--

你的 `ShoppingCart` 类应该有一个 `addItem` 方法。
...
你的 `addItem` 方法应该接受两个参数：`id` 和 `products`。
