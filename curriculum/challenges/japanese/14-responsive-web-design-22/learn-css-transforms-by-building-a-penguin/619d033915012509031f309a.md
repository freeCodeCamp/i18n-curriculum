...
`.penguin-head` 内に `div` 要素を 1 つ加える必要があります。 `--fcc-expected--` 個の `div` 要素があるはずですが、`--fcc-actual--` 個となっています。

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 3);
```

追加した `div` の `class` を `chin` に設定する必要があります。

```js
assert.exists(document.querySelector('.penguin-head > div.chin'));
```

`div` 要素を `.face.right` 要素の下に置く必要があります。
