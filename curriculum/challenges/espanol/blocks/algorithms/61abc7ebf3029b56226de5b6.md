...
1. Encuentra el `value` medio de un arreglo ordenado. Si `value == target` devuelve `true` (El valor ha sido encontrado y la búsqueda está completa).
1. Si el `value < target` medio, busca en la mitad derecha del arreglo en la siguiente comparación.
1. Si el valor medio `value > target`, busca en la mitad izquierda del arreglo en la siguiente comparación.
1. Si después de buscar en todo el array el valor no está presente, devuelve `false` (El array ha sido buscado y el valor no está en el array).
