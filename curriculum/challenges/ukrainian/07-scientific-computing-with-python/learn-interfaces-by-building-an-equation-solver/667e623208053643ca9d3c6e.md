...
Тепер замініть цикл `for` та інструкцію `if`, які ви додали на попередньому кроці, інструкцією `if`, яка використовує вбудовану функцію `any()`.
...
Передайте вираз генератора як аргумент виклику `any()`.
...
Вираз генератора, переданий у `any()`, має ітерувати через `args`.
...
Інструкція `if` повинна перевіряти, чи будь-який з аргументів у `args` не є екземпляром `int` або `float`.
...
Використайте наданий рядок, щоб підняти `TypeError` в нової інструкції `if`.
