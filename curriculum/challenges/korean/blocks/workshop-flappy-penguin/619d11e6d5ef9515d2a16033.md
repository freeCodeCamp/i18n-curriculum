...
`.beak` 요소를 `class`의 `top`으로 지정하고, `width`의 `20%`을 주며, 부모 요소의 위에서 `60%`만큼, 왼쪽에서 `40%`만큼 위치시키세요.

# --hints--

`.beak.top` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.beak.top', '.top.beak']));
```

`.beak.top`에 `width`의 `--fcc-expected--`을 주어야 하지만 `--fcc-actual--`이 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.top', '.top.beak'])?.width, '20%');
```

`.beak.top`에 `top`의 `--fcc-expected--`을 주어야 하지만 `--fcc-actual--`이 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.top', '.top.beak'])?.top, '60%');
```

`.beak.top`에 `left`의 `--fcc-expected--`을 주어야 하지만 `--fcc-actual--`이 발견되었습니다.
