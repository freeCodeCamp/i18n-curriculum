...
Encadeie outro `.then()` ao método `.then()` existente. Desta vez, passe `data` como o parâmetro para a função de callback. For the callback, use curly braces because you will have more than one expression. Dentro da função de callback, registre `data` no console usando console.log() para ver a aparência dos dados.
