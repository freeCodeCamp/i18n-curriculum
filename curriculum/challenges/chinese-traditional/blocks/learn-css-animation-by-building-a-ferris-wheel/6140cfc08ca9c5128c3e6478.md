...
將 `.line` 選擇器的 `position` 屬性設置爲 `absolute`、`left` 屬性設置爲 `50%`、`top` 屬性設置爲 `50%`。

# --hints--

你的 `.line` 選擇器應該有一個 `position` 屬性，設置爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.line')?.position === 'absolute');
```

你的 `.line` 選擇器應該有一個 `left` 屬性，設置爲 `50%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.line')?.left === '50%');
```

你的 `.line` 選擇器應該有一個 `top` 屬性，設置爲 `50%`。
