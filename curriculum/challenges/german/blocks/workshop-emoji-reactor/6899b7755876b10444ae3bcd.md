...
Jetzt ist es Zeit, die JavaScript-Datei zu verlinken.

Fügen Sie ein `script`-Element direkt vor dem schließenden Tag des `body`-Elements mit dem richtigen Attribut und Wert hinzu, um die `script.js`-Datei zu verlinken.
...
Das `script`-Element sollte ein `src` von `./script.js` haben.
