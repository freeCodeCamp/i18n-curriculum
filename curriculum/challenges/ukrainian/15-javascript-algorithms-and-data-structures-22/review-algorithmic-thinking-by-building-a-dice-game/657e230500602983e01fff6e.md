...
Для останньої частини гри потрібно буде створити алгоритм, який перевіряє наявність черги. A small straight is when four of the dice have consecutive values in any order (Ex. in a roll of `41423`, we have `1234`) resulting in a score of `30` points. A large straight is when all five dice have consecutive values in any order (Ex. in a roll of `35124`, we have `12345`) resulting in a score of `40` points.

Оголосіть функцію `checkForStraights`, яка приймає масив чисел. Якщо користувач отримує велику чергу, оновіть п’ятий перемикач на рахунок `40`. Якщо користувач отримує малу чергу, оновіть четвертий перемикач на рахунок `30`. Якщо користувач не отримав чергу, оновіть останній перемикач, щоб відобразити `0`.

Викличте функцію `checkForStraights`, коли клацнуто `rollDiceBtn`, щоб завершити гру в кості!

# --hints--

Змінна `checkForStraights` має бути функцією.

```js
assert.isFunction(checkForStraights);
```

Якщо випадає мала черга, функція `checkForStraights` має ввімкнути четвертий перемикач, встановити значення на `30` та оновити відображуваний текст на `, score = 30`.
...
Якщо випадає велика черга, функція `checkForStraights` має ввімкнути п’ятий перемикач, встановити значення на `40` та оновити відображуваний текст на `, score = 40`.
...
Якщо випадає велика черга, функція `checkForStraights` також має ввімкнути четвертий перемикач, встановити значення на `30` та оновити відображуваний текст на `, score = 30`.
