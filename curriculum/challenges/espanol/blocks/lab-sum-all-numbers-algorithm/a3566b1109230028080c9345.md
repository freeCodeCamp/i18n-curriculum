...
1. `sumAll([n, m])` debería devolver la suma de `n` y `m` más la suma de todos los números entre ellos. El número más bajo no siempre será el primero. Por ejemplo, `sumAll([4,1])` debería devolver `10` porque la suma de todos los números entre `1` y `4` (ambos inclusive) es `10`.
