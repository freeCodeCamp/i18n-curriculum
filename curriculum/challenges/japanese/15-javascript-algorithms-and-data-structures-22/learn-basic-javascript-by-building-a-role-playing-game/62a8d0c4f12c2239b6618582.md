...
`buyWeapon` 関数に `else` 文を追加しましょう。 その中では、`text.innerText` を `"You do not have enough gold to buy a weapon."` に設定してください。

# --hints--

`buyWeapon` 関数に `else` 文を追加してください。

```js
assert.match(buyWeapon.toString(), /else/);
```

`else` 文は `if` 文の後に来る必要があります。
...
`text.innerText` を `"You do not have enough gold to buy a weapon."` に設定してください。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You do not have enough gold to buy a weapon.\1/);
```

`else` ステートメントは `text.innerText` を `"You do not have enough gold to buy a weapon."` に設定する必要があります。
