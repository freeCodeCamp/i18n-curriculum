...
Додайте `constructor` всередину класу. У конструкторі потрібно використати елементи, створені раніше, щоб вибрати відповідні DOM-елементи за допомогою функції `getElement`.

Поки що обробіть `singleCardBtn` і `singleCard`. Для `singleCardBtn` передайте `#btn-single-card`, а для `singleCard` передайте `.single_card`.

# --hints--

Клас `Game` має мати `constructor`.
...
Ваш конструктор має призначити `this.elements`.
...
`singleCardBtn` має бути призначений за допомогою `getElement` з `'#btn-single-card'`.
...
`singleCard` має бути призначений за допомогою `getElement` з `'.single_card'`.
