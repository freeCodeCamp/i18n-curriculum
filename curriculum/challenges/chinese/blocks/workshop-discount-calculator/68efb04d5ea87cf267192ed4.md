...
创建一个 `DiscountEngine` 实例并将其保存到变量 `engine` 中。之后，使用 `calculate_best_price` 方法计算最佳价格，并将其保存到名为 `best_price` 的变量中。

# --hints--

你应该创建一个带有 `strategies` 列表的 `DiscountEngine` 实例。
...
你应该调用 `engine.calculate_best_price(product, user_tier)` 并将结果保存到 `best_price`。
