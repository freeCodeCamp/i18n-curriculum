...
接下来，为 `constraints` 字典添加一个 `medications` 密钥。对于它的值，使用 `isinstance` 来查看 `medications` 是否是一个列表。

# --hints--

你的 `constraints` 字典应包含一个 `medications` 密钥。
...
你的 `constraints` 字典中的 `medications` 密钥应具有 `isinstance(medications, list)` 的值。
