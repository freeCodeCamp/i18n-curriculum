...
這確實有效：它會匹配四個數字字元。但與其多次寫出相同的類別，不如給它一個量詞。
...
請注意，你不能只用這個語法來設定最大值：你必須總是設定最小值。但如果你將最小值設為 `1`，就能有效達到相同的效果。
...
不過其實有一個針對單一選用字元的特殊簡寫量詞，就是問號（`?`）。讓我們用問號來取代原本的量詞：
...
不過我們的樣式又變長了。幸好還有另一個簡寫，表示「匹配前一個字元零次或多次」，就是星號（`*`）符號。讓我們用它來取代量詞，並測試看看：
...
現在我們成功匹配任何以零個或多個字母開頭，接著是四到六個數字的識別符號。但結果發現這會讓系統當機：我們其實必須至少要求一個字母。

我們可以再次使用最小值為一且無最大值的量詞，或者使用另一個特殊語法，就是加號（`+`）符號：
