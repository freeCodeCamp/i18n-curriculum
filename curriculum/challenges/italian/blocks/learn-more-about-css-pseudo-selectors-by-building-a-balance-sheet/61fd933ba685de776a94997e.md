...
All'interno del primo `tr`, aggiungi un elemento `th` con il testo `Loans The outstanding balance on our startup loan.`. Avvolgi quel testo, tranne `Loans `, all'interno di un elemento `span` con l'attributo `class` impostato su `description`.

Aggiungi tre elementi `td` sotto di esso, e assegna loro il seguente testo, in ordine: `$500`, `$250` e `$0`. Dai al terzo elemento `td` un attributo `class` impostato su `current`.
...
Il tuo elemento `th` dovrebbe avere il testo `Loans The outstanding balance on our startup loan.`.
...
Dovresti avvolgere il testo `The outstanding balance on our startup loan.` in un elemento `span`.
