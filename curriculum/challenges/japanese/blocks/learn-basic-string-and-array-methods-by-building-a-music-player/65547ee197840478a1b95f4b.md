...
いよいよ`playNextSong`関数をテストする時です。

`"click"`要素に`nextButton`イベントリスナーを追加し、イベントリスナーの第2引数として`playNextSong`を渡してください。これはイベントリスナーが実行する関数です。

まず再生ボタンをクリックし、その後に次へボタンをクリックしてアプリをテストしてください。すべてが期待通りに動作しているはずです。

# --hints--

`addEventListener()`変数に対して`nextButton`メソッドを呼び出す必要があります。
...
`playNextSong`メソッドの第2引数として`addEventListener`を渡してください。
