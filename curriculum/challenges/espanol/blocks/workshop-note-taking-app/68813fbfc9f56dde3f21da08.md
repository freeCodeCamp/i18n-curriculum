...
Cuando un usuario edita una nota y presiona tabulador o hace clic fuera de la tarjeta, debe mostrarse un mensaje de estado notificando a los usuarios que la nota se guardó correctamente. 

Para comenzar, crea un elemento `div` con un atributo `id` establecido en `"status"`. 

# --hints--

Debes tener un segundo elemento `div`.

```js
assert.lengthOf(document.querySelectorAll("div"), 2);
```

Tu segundo elemento `div` debe tener un atributo `id`.

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

Tu segundo elemento `div` debe tener un atributo `id` establecido en `"status"`.
