...
Ndani ya darasa la `Spider`, tekeleza njia ya `render`, ikiwa na kiambishi awali cha `override`, ili kusahihisha sifa ya `innerText` ya `emojiElement` kwa thamani ya sifa ya `emoji`.

# --hints--

Darasa lako la `Spider` linapaswa kuwa na njia ya `render`.
...
Njia yako ya `Spider` `render` inapaswa kutumia kiambishi awali cha `override`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?override\s+render/);
```

Njia yako ya `Spider` `render` inapaswa kuweka `this.emojiElement.innerText` kuwa `this.emoji`.
