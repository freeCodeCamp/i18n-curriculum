...
Mission Control benötigt noch eine Funktion, um eine Zusammenfassung Ihrer Crew zu protokollieren. Erstellen Sie eine leere Funktion namens `printCrewSummary`, die einen `crew`-Parameter akzeptiert.

# --hints--

Sie sollten eine Funktion namens `printCrewSummary` erstellen.

```js
assert.isFunction(printCrewSummary);
```

Ihre `printCrewSummary`-Funktion sollte einen `crew`-Parameter haben.
