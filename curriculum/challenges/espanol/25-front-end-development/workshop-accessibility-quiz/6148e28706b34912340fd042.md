...
Centrar horizontalmente todo el texto dentro del elemento `address`, y añadir algo de relleno.

# --hints--

Debes usar el selector del elemento `address`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

Debes dar al elemento `address` un `text-align` de `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Debes dar al elemento `address` un `padding-top` de al menos `1px`.
...
Debes dar al elemento `address` un `padding-right` de al menos `1px`.
...
Debes dar al elemento `address` un `padding-bottom` de al menos `1px`.
...
Debes dar al elemento `address` un `padding-left` de al menos `1px`.
