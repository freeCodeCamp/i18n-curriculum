...
`Coffee` という見出しの下に、`https://cdn.freecodecamp.org/curriculum/css-cafe/coffee.jpg` という URL を使用して画像を追加しましょう。 画像には `alt` 属性値 `coffee icon` を加えてください。
...
`img` 要素は空要素なので、終了タグ `</img>` は必要ありません。
