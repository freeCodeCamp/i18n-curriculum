...
前回の`.divider`の後に、テキストが`p`の別の`Trans Fat 0g`要素を作成してください。`Trans`という単語は`i`要素で囲んで斜体にしてください。新しい`p`要素に`class`属性を設定し、その値を`indent no-divider`にしてください。`Trans Fat 0g`は整列のために`span`要素で囲んでください。

# --hints--

`p`要素の最後に新しい`.daily-value`要素を作成する必要があります。

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'p');
```

新しい`p`要素にはテキスト`Trans Fat 0g`が含まれている必要があります。

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText === 'Trans Fat 0g');
```

新しい`p`要素には`class`属性が`indent no-divider`に設定されている必要があります。
