...
`find_empty_cell` повертає `(0, 0)`, який є позицією першої порожньої клітинки на дошці судоку.

Перетворіть перший `0` всередині `puzzle` на `1`. Ви побачите у вихідних даних, що буде знайдена наступна порожня клітинка.

# --hints--

Ви повинні змінити перший елемент першого списку в `puzzle` на `1`.
