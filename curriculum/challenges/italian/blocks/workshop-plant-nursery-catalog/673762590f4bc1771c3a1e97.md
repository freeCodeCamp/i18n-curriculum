...
Prima di andare avanti, rimuovi le chiamate `console.log` dal tuo codice.

# --hints--

Non dovresti avere `console.log(removePlant(ballerina))` nel tuo codice.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*removePlant\s*\(\s*ballerina\s*\)\s*\)/);
```

Non dovresti avere `console.log(sellPlants(ballerina, "small", 10));` nel tuo codice.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

Non dovresti avere `console.log(catalog)` nel tuo codice.
