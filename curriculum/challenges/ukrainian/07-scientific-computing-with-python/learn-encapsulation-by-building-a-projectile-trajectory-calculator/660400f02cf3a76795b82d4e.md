...
Використайте наступну формулу для обчислення переміщення снаряда: \\[ d = \frac{v \cdot \cos\theta \cdot \left(v\cdot\sin\theta + \sqrt{v^2\sin^2\theta + 2 \cdot g \cdot h}\right)}{g} \\]
...
Оголосіть функцію під назвою `__calculate_displacement` за допомогою `def __calculate_displacement(self):`.
