...
페이지에 노래가 표시될 때 각 노래의 제목, 아티스트, 재생 시간과 삭제 버튼이 보여야 합니다.

다음 몇 단계에서는 <dfn>map()</dfn> 메서드를 사용해 각 노래에 대한 새로운 HTML을 추가하는 방법을 배우게 됩니다. 이 메서드는 다음 단계에서 더 자세히 설명합니다.

`const`를 사용해 `songsHTML`이라는 변수를 선언하고 `array.map()`를 할당하는 것부터 시작하세요.

# --hints--

`const` 변수 `songsHTML`가 있어야 합니다.

```js
assert.match(code, /const\s+songsHTML/);
```

`array.map()`를 `songsHTML`에 할당해야 합니다.
