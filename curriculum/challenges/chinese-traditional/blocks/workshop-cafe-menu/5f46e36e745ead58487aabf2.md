...
現在頂部間距看起來不錯。選單底部位址下方的空間比選單頂部和 `h1` 元素的空間稍大一點。

若要減少位址 `p` 元素下方的預設外距空間，請創建名為 `address` 的類別選擇器，並將 `margin-bottom` 屬性的值設為 `5px`。

# --hints--

你應該為你的 `.address` 選擇器添加。
...
你的 `.address` 選擇器應該將 `margin-bottom` 屬性設為 `5px`。
