...
사용자 정의 라디오 버튼에 크기와 모양을 지정하세요.

여전히 `.contact-method-radio-btn`를 대상으로 하여 `width`을 `20px`로, `height`을 `20px`로, `border-radius`를 `50%`으로, `border`을 `2px solid gray`로 설정하세요.

# --hints--

계속해서 `.contact-method-radio-btn` 클래스를 대상으로 해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

`width` 속성을 `20px`로 설정하세요.
...
`border` 속성을 `2px solid gray`로 설정하세요.
