...
Primeiro, crie uma classe `Hat` em `main.py`. A classe deve ter um número variável de argumentos que especificam o número de bolas de cada cor que estão no chapéu. Por exemplo, um objeto da classe poderia ser criado de qualquer uma dessas maneiras:
...
Em seguida, crie uma função `experiment` em `main.py` (não dentro da classe `Hat`). Esta função deve aceitar os seguintes argumentos:

- `hat`: Um objeto de chapéu contendo bolas que devem ser copiadas dentro da função.
- `expected_balls`: Um objeto que indica o grupo exato de bolas a serem retiradas do chapéu para o experimento. Por exemplo, para determinar a probabilidade de tirar 2 bolas azuis e 1 bola vermelha do chapéu, defina `expected_balls` como `{'blue':2, 'red':1}`.
...
*Dica: Considere usar os módulos que já estão importados no topo.* Não inicialize a semente aleatória dentro do arquivo.*

Nota: abra o console do navegador com F12 para ver uma saída mais detalhada dos testes.

# --hints--

A criação do objeto `hat` deve adicionar o conteúdo correto.
...
O método `draw` na classe `hat` deve reduzir o número de itens em contents.
...
O método `draw` deve se comportar corretamente quando o número de bolas a serem extraídas for maior do que o número de bolas no chapéu.
...
O método `experiment` deve retornar uma probabilidade diferente.
