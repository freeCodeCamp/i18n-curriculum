...
حان الوقت الآن لإنشاء الدالة المسؤولة عن عرض المؤلفين على الصفحة.

ابدأ بإنشاء دالة تسمى `displayAuthors` تأخذ معلمة `authors`. يجب أن تقوم دالتك بعد ذلك بالتكرار على مصفوفة `authors` وإضافة بطاقة لكل مؤلف إلى عنصر `authorContainer`.

يجب أن تكون البطاقة بالهيكل التالي:
...
- يمكنك استخدام أي نوع من الحلقات للتكرار على مصفوفة `authors`، مثل `for`، `forEach`، `map`، وغيرها.
- يجب استخدام الخاصية `innerHTML` لإضافة البطاقة إلى `authorContainer` داخل الحلقة. ستحتاج أيضًا إلى استخدام صيغة القوالب النصية لإضافة الترميز.
...
يجب أن تكون `displayAuthors` دالة.

```js
assert.isFunction(displayAuthors)
```

يجب أن تأخذ دالة `displayAuthors` معلمة `authors`.
...
عند استدعاء دالة `displayAuthors` مع مصفوفة المؤلفين، يجب أن تضيف بطاقة لكل مؤلف إلى عنصر `#authorContainer`.
