...
在本次工作坊中，你将通过创建一个显示不同错误表情符号的错误物种选择器，学习 TypeScript 抽象类和泛型。

项目的超文本标记语言和 CSS 已经为你准备好，这样你就可以专注于 TypeScript。你可以查看这些文件，了解稍后将要选择的元素。

首先，你需要为所有错误创建一个抽象基类。抽象类不能被直接实例化，必须由其他类继承。

创建一个名为 `Bug` 的抽象类，并带有一个泛型类型参数 `T`。
