...
title: 'Problema 56: Suma poderosa de dígitos'
challengeType: 1
forumTopicId: 302167
...
Un googol ($10^{100}$) es un número enorme: un uno seguido de cien ceros; $100^{100}$ es casi inimaginablemente grande: un uno seguido de doscientos ceros. A pesar de su tamaño, la suma de los dígitos en cada número es solo 1.

Considerando números naturales de la forma, $a^b$, donde `a`, `b` &lt; `n`, ¿cuál es la suma digital máxima?

# --hints--

`powerfulDigitSum(3)` debería devolver un número.

```js
assert(typeof powerfulDigitSum(3) === 'number');
```

`powerfulDigitSum(3)` debería devolver `4`.

```js
assert.strictEqual(powerfulDigitSum(3), 4);
```

`powerfulDigitSum(10)` debería devolver `45`.

```js
assert.strictEqual(powerfulDigitSum(10), 45);
```

`powerfulDigitSum(50)` debería devolver `406`.

```js
assert.strictEqual(powerfulDigitSum(50), 406);
```

`powerfulDigitSum(75)` debería devolver `684`.

```js
assert.strictEqual(powerfulDigitSum(75), 684);
```

`powerfulDigitSum(100)` debería devolver `972`.
