...
Primero, añade un elemento `hr` entre el elemento `p` con clase `established` y el primer elemento `section`.

Note que el elemento `hr` es un elemento vacío.

# --hints--

Debes añadir un elemento `hr`.

```js
assert.match(code, /<hr\s?\/?>/i);
```

El elemento `hr` es un elemento vacío, no debe tener una etiqueta de cierre `</hr>`.
