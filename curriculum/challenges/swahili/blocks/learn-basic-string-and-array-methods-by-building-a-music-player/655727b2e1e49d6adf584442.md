...
Kama orodha ya nyimbo ni tupu, unahitaji kuunda kipengee cha `resetButton` na maandishi kwa ajili yake. Kitufe hiki kitaonekana tu ikiwa orodha ya nyimbo ni tupu.

`createElement()` ni njia ya DOM unayoweza kutumia kuunda kipengee kwa nguvu kwa kutumia JavaScript. Ili kutumia `createElement()`, unaitisha, kisha upitishe jina la lebo kama mfuatano wa herufi:
...
Pia unaweza kuweka thamani yake kwenye kigezo:

```js
const divElement = document.createElement('div')
```

Ndani ya tamko lako la `if`, tangaza thabiti ya `resetButton`, kisha tumia `createElement()` kuunda `"button"`.
...
Unapaswa kutumia njia ya `createElement` kuunda `"button"`.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

Unapaswa kuweka kipengee chako kipya cha `button` kwenye thabiti ya `resetButton`.
