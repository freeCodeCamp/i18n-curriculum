...
루프가 완료된 후에는 `budgetNumberInput`를 지워야 합니다. `value`의 `budgetNumberInput` 속성을 빈 문자열로 설정하세요.

# --hints--

`clearForm` 함수는 `value` 요소의 `budgetNumberInput` 속성에 접근해야 합니다.

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value/);
```

`clearForm` 함수는 `value` 요소의 `budgetNumberInput` 속성을 빈 문자열로 설정해야 합니다.

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value\s*=\s*('|"|`)\1/);
```

`budgetNumberInput`는 `for` 루프가 끝난 후에 수정해야 합니다.
