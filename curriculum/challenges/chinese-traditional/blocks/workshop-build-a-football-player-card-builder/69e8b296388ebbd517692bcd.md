...
現在你將開始建置表單。在 `div` 裡面，`className` 的 `form-panel` 中，新增一個 `div` 來包裹「Player Info」區塊。在那個新的 `div` 裡面，新增一個 `p`，其 `className` 為 `form-section-title`，並帶有文字 `Player Info`。

你會在接下來的步驟繼續為這個區塊添加欄位。

# --hints--

你的 `div.form-panel` 應該包含一個 `div` 來包裹 Player Info 區塊。
...
`div` 在 `div.form-panel` 裡面，應該包含一個 `p`，其 `className` 為 `form-section-title`，並帶有文字 `Player Info`。
