...
現在該看圖表了。 創建一個方法 `create_trajectory`，並將代碼底部的最後一個 `print` 調用替換爲 `print(graph.create_trajectory())`。

作爲該新方法的第一步，對座標進行本地複製，但所有值都四捨五入爲整數。 將這個新版本的座標保存在名爲 `rounded_coords` 的變量中，並返回該變量。

# --hints--

你應該定義一個名爲 `create_trajectory` 的方法。
...
你應該將 `print(graph.create_coordinates_table())` 替換爲 `print(graph.create_trajectory())`。
