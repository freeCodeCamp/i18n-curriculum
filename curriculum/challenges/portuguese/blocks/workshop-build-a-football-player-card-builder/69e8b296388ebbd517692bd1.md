...
Adicione outro `div` com o `className` de `form-group`. Dentro dele, adicione um elemento `label` com o `className` de `label`, um `htmlFor` de `imageUrl` e o texto `Image URL`. Depois adicione um elemento `input` com um `id` de `imageUrl`, um `className` de `input` e um `type` de `text`.
