...
Necesitas obtener el `id` de la canción que se está reproduciendo actualmente. Para esto, puedes usar `userData.currentSong?.id`.

Usa `getElementById()` para seleccionar la canción que se está reproduciendo actualmente: pasa una plantilla literal para prefijar el `id` con `song-`. Asígnalo a una variable llamada `songToHighlight`.

# --hints--

No deberías modificar la función existente `highlightCurrentSong` y su contenido.
...
Deberías usar `document.getElementById()` y pasar `` `song-${userData.currentSong?.id}` ``.

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Debes asignar tu `getElementById()` a una variable `songToHighlight`.
