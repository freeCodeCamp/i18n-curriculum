...
أضف الآن محدِّد `100%` إلى قاعدة `@keyframes wheel`. داخلها، عيّن الخاصية `transform` إلى `rotate(360deg)`. بهذا، ستُكمل الحركة دورانًا كاملاً.

# --hints--

يجب أن تحتوي قاعدة `@keyframes wheel` على محدِّد `100%`.
...
يجب أن يحتوي محدِّد `100%` على خاصية `transform` مضبوطة إلى `rotate(360deg)`.
