...
現在，你需要更新當前怪物的名稱和健康狀況的文本。

首先將 `monsters[fighting].name` 分配給 `monsterName` 的 `innerText` 屬性。 然後，將 `monsterHealth` 分配給 `monsterHealthText` 的 `innerText` 屬性。

# --hints--

你應該使用點符號來訪問 `monsterName` 的 `innerText` 屬性。

```js
assert.match(goFight.toString(), /monsterName\.innerText/);
```

你應該將 `monsters[fighting].name` 分配給 `monsterName.innerText`。

```js
assert.match(goFight.toString(), /monsterName\.innerText\s*=\s*monsters\s*\[\s*fighting\s*\]\s*\.name/);
```

你應該使用點符號來訪問 `monsterHealthText` 的 `innerText` 屬性。

```js
assert.match(goFight.toString(), /monsterHealthText\.innerText/);
```

你應該將 `monsterHealth` 分配給 `monsterHealthText.innerText`。
