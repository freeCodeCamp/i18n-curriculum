...
事情似乎運作正常，除了有一個小問題。如果你編輯一則筆記然後離開該筆記，你應該會看到 `"Note saved successfully!"` 訊息。但如果你嘗試再次編輯該筆記，該訊息仍然顯示。 

期望的行為是在使用者聚焦於該元素時，訊息消失。你可以使用 `focus` 事件來達成此目的。 
...
為你的 `noteEl` 變數附加一個 `addEventListener`。第一個引數應該是 `"focus"` 事件，第二個引數應該是一個回呼函式。在該回呼函式內，將 `statusEl.textContent` 設為空字串。這將在聚焦於筆記時重設即時區域。

隨著最後一次變更，你的工作坊已完成！

# --hints--

當使用者聚焦於該註解時，你應該將 `statusEl.textContent` 設為空字串。
