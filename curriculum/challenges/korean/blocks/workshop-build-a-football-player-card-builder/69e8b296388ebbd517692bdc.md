...
SHO `value`에 `player.sho` 속성을 `input`로 추가하세요. 그런 다음 `onChange` 속성을 매개변수로 `e`를 받고 `setPlayer`를 펼친 뒤 `player`를 `sho`로 설정하여 `Number(e.target.value)`를 호출하는 화살표 함수로 설정하세요.

# --hints--

SHO `input`는 현재 플레이어의 `value` 스탯과 같은 `sho`을 가져야 합니다.
...
SHO `input`는 `onChange`로 설정된 `setPlayer`와 `player`를 펼쳐서 `sho`를 호출하는 `Number(e.target.value)` 핸들러를 가져야 합니다.
