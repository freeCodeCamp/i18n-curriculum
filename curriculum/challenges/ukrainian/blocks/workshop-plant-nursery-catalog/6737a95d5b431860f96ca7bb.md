...
Тепер спробуйте додати об’єкт `ballerina` до вашої множини `catalogSet` всередині функції вдруге. Ви побачите, що нічого не станеться, бо цей об’єкт уже збережений у множині, а дублікати елементів у множинах не допускаються.

Якщо хочете, можете перевірити кількість елементів у множині, використовуючи властивість `size`, так само, як ви робили це раніше для вашої мапи.

# --hints--

Ви повинні `add` об’єкт `ballerina` вдруге до множини всередині функції `displayPlantsSet`.
