...
No seu arquivo JavaScript, selecione o elemento `.story-container` e armazene-o em uma variável chamada `storyContainer`.

# --hints--

Você deve ter uma variável `storyContainer`.

```js
assert.isDefined(storyContainer);
```

Você deve selecionar o elemento `.story-container` usando o método `querySelector` e armazená-lo na variável `storyContainer`.
