...
El primer argumento para su llamada `evalFormula` debe ser el contenido de la celda (que almacenaste en `value`). Sin embargo, el contenido comienza con un carácter `=` para activar la función, así que necesitas pasar la subcadena de `value` comenzando en el índice `1`.

# --hints--

Debe pasar `value` como primer argumento a su llamada `evalFormula()`.
...
Debe llamar al método `.slice()` en el argumento `value`.
...
Debe pasar el número `1` como argumento a su llamada `.slice()`.
