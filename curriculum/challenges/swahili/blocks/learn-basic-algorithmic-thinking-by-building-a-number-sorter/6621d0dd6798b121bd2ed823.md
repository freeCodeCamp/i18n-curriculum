...
Sahihisha wito wako wa kurudisha wa `.map()` ili uite kitendakazi cha `Number()`. Pitia `dropdown.value` kwa wito huo wa kitendakazi.

Fungua kichupo cha Konsoli kuona kuwa `inputValues` sasa ni safu ya data ya nambari.

# --hints--

Unapaswa kuita kitendakazi cha `Number()` katika wito wako wa kurudisha wa `.map()`.
...
Unapaswa kupitisha `dropdown.value` kwa wito wako wa `Number()`.
