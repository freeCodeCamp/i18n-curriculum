...
現在你需要一個主要類別，幫助在 DOM 中動態呈現使用者介面。

創建一個名為 `Game` 的類別。在裡面，新增一個名為 `cards` 的屬性，型別為 `Card[]`，目前將它初始化為空的陣列。
