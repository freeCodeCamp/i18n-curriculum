...
Crea un selector `hr`, y dale una propiedad `margin` establecida en `1.5rem 0`.

# --hints--

Debes tener un selector `hr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('hr'));
```

Tu selector `hr` debe tener una propiedad `margin` establecida en `1.5rem 0`.
