...
`.fb2-window` 要素に `width` を `22%`、`height` を `100%`、そして `background-color` をあなたの `--window-color3` 変数で設定してください。

# --hints--

`.fb2-window` 要素に `width` を `22%` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

`.fb2-window` 要素に `height` を `100%` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

`.fb2-window` 要素に `background-color` を `--window-color3` に設定する必要があります。
