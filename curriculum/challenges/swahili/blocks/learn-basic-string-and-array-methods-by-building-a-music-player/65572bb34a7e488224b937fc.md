...
Unahitaji kuongeza `resetText` kama mtoto wa kipengele cha `resetButton`, na pia `resetButton` kama mtoto wa kipengele cha `playlistSongs`. Kwa hili, kuna njia ya `appendChild()` ya kutumia.

`appendChild()` inakuwezesha kuongeza nodi au kipengele kama mtoto wa kipengele kingine. Katika mfano uliopo hapa chini, maandishi "Click me" yangebandikwa kwenye kitufe:
...
Tumia `appendChild()` kuambatanisha `resetText` kwenye kipengele cha `resetButton`, na `resetButton` kwenye kipengele cha `playlistSongs`.

# --hints--

Unapaswa kutumia njia ya `appendChild()` kwenye `resetButton`.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

Unapaswa kuingiza `resetText` kama thamani ya `appendChild()` yako ya kwanza.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

Unapaswa kutumia njia ya `appendChild()` kwenye `playlistSongs`.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

Unapaswa kuingiza `resetButton` kama thamani ya `appendChild()` yako ya pili.
