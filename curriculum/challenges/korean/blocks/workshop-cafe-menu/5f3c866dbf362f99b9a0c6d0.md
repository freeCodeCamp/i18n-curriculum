---
id: 5f3c866dbf362f99b9a0c6d0
title: Step 33
challengeType: 0
dashedName: step-33
---

# --description--

`p` 요소들이 `article` 요소 안에 `item`라는 클래스 속성을 가진 상태로 중첩되어 있습니다. `p`라는 클래스를 가진 요소 안 어디에나 중첩된 모든 `item` 요소를 다음과 같이 스타일링할 수 있습니다:

```css
.item p { }
```

위 선택자를 사용해 `display` 요소가 인라인 요소처럼 더 동작하도록 `inline-block` 속성을 `p` 값으로 추가하세요.

# --hints--

`.item p` 선택자를 사용하세요.

```js
const itemPStyle = new __helpers.CSSHelp(document).getStyle('.item p');
assert.exists(itemPStyle);
```

`display` 속성을 `inline-block`으로 설정하세요.

```js
const hasDisplay = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style.display === 'inline-block');
assert.isTrue(hasDisplay);
```

`.item p` 선택자가 `display` 속성을 `inline-block`으로 설정해야 합니다.

```js
const itemPDisplay = new __helpers.CSSHelp(document).getStyle('.item p')?.getPropertyValue('display');
assert.equal(itemPDisplay, 'inline-block');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Cafe Menu</title>
    <link href="styles.css" rel="stylesheet"/>
  </head>
  <body>
    <div class="menu">
      <main>
        <h1>CAMPER CAFE</h1>
        <p>Est. 2020</p>
        <section>
          <h2>Coffee</h2>
          <article class="item">
            <p class="flavor">French Vanilla</p>
            <p class="price">3.00</p>
          </article>
          <article>
            <p>Caramel Macchiato</p>
            <p>3.75</p>
          </article>
          <article>
            <p>Pumpkin Spice</p>
            <p>3.50</p>
          </article>
          <article>
            <p>Hazelnut</p>
            <p>4.00</p>
          </article>
          <article>
            <p>Mocha</p>
            <p>4.50</p>
          </article>
        </section>
      </main>
    </div>
  </body>
</html>
```

```css
body {
  background-image: url(https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg);
}

h1, h2, p {
  text-align: center;
}

.menu {
  width: 80%;
  background-color: burlywood;
  margin-left: auto;
  margin-right: auto;
}

--fcc-editable-region--

--fcc-editable-region--

.flavor {
  text-align: left;
}

.price {
  text-align: right;
}
```
