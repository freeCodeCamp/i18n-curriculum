...
Je, umeona kwamba unaweza kuwasilisha fomu bila kujaza sehemu za ingizo?

Ili kuzuia hilo, ongeza sifa ya kipengele ya `disabled` ili kuzima kitufe cha kuwasilisha ikiwa `heroName`, `realName`, au `powerSource` ni za uongo, au ikiwa urefu wa `powers` ni `0`.

Sasa, kitufe kitazimwa ikiwa kitu hakijajazwa. Jaza fomu na taarifa zako za shujaa na wasilisha ili kujiunga na ligi.

Kwa hivyo, fomu yako ya maombi ya shujaa imekamilika.

# --hints--

Haupaswi kubadilisha yaliyopo kwenye kitufe.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2/)
```

Kipengele chako cha `button` kinapaswa kuwa na sifa ya `disabled`.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

Unapaswa kuzima `button` ikiwa yoyote kati ya `heroName`, `realName`, na `powerSource` ni uongo, au ikiwa urefu wa `powers` ni `0`.
