...
Para su tercera tabla, agregue un `caption` con el texto `Net Worth` y configure un encabezado de tabla y un cuerpo de tabla.

# --hints--

El tercer elemento `table` debería tener un elemento `caption`.

```js
assert(document.querySelectorAll('table')?.[2]?.children?.[0]?.localName === 'caption');
```

El elemento `caption` debería tener el texto `Net Worth`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('caption')?.textContent === 'Net Worth');
```

El tercer elemento `table` debería tener un elemento `thead`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('thead'));
```

El tercer elemento `table` debería tener un elemento `tbody`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('tbody'));
```

El elemento `thead` debería estar inmediatamente debajo del elemento `caption`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('caption')?.nextElementSibling?.localName === 'thead');
```

El elemento `tbody` debería estar inmediatamente debajo del elemento `thead`.
