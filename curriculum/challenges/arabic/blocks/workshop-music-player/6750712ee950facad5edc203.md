...
إذا جربت تشغيل الموسيقى قليلاً على مشغلك، ستلاحظ بعض الأخطاء البرمجية. أولاً، عندما تشغل أغنية، توقفها مؤقتًا ثم تضغط على زر السابق أو التالي، لن تبدأ الأغنية من البداية.

لبدء إصلاح ذلك، أضف معلمة ثانية باسم `start` إلى دالة `playSong` الخاصة بك. اجعل قيمتها الافتراضية `true`.

# --hints--

يجب أن تحتوي دالة `playSong` على معلمة ثانية باسم `start` بقيمة افتراضية `true`.
