...
Als Nächstes müssen Sie eine Funktion erstellen, um Elemente im DOM auszublenden. Deklarieren Sie eine `hideElements`-Funktion, die `...elements` als Parameter mit dem Typ `HTMLElement[]` entgegennimmt.

Diese Funktion mit einer CSS-`hidden`-Klasse wird in der gesamten App verwendet, um die Sichtbarkeit von UI-Elementen umzuschalten.

# --hints--

Sie sollten eine Funktion mit dem Namen `hideElements` deklariert haben.
...
Ihre `hideElements`-Funktion sollte einen Parameter namens `...elements` mit dem Typ `HTMLElement[]` haben.
