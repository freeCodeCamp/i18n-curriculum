...
你的 `span` 元素應該有一個開始標籤，而不是一個結束標籤。

```js
assert(document.querySelector('form > input') && !code.match(/<\/input\>/g));
```

You should only have added one `input` element for your radio button. 刪除任何多餘的。

```js
assert(document.querySelectorAll('form > input'), 2);
```

Your new `input` element should be above the existing `input` with `type` attribute set to `text`. 你把它們的順序寫錯了。
...
Your new `input` element should have only one `type` attribute. 刪除任何多餘的。
...
Your new `input` element should have a `type` attribute with the value `radio`. 你要麼遺漏了屬性值，要麼拼寫有誤。 記住，屬性值應該放在引號中。
...
The text `Indoor` should be located directly to the right of your `radio` button. 你要麼遺漏了文本，要麼拼寫有誤。
