...
Sotto l'elemento `h3`, aggiungi una lista ordinata con questi tre elementi:

`flea treatment` `thunder` `other cats`

# --hints--

L'elemento `ol` dovrebbe avere un tag di apertura. Opening tags have this syntax: `<elementName>`.

```js
assert(document.querySelector('ol'));
```

L'elemento `ol` dovrebbe avere un tag di chiusura. Closing tags have a `/` just after the `<` character.
