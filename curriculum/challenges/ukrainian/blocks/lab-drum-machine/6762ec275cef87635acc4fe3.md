...
1. У вас має бути `div` елемент з `id` `drum-machine`, який містить усі інші елементи.

2. Всередині `#drum-machine` елемента має бути ще один `div` з `id` `pad-bank`.

3. Всередині `#drum-machine` елемента має бути `p` елемент з `id` `display`.

4. Усередині вашого `#pad-bank` елемента має бути дев’ять активних елементів драм-педів, кожен із класом `drum-pad`, унікальним `id`, який описує аудіокліп, який цей драм-пед буде запускати, і внутрішнім текстом, що відповідає одному з наступних клавіш на клавіатурі: `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`. Драм-педи МАЮТЬ бути в цьому порядку.

5. Кожен `.drum-pad` має містити `audio` елемент із класом `clip`, `src` атрибутом, що вказує на аудіокліп, і `id`, що відповідає внутрішньому тексту його батьківського `.drum-pad` елемента (наприклад, `id="Q"`, `id="W"`, `id="E"` тощо).

6. Коли ви натискаєте на `.drum-pad` елемент, має запускатися аудіокліп, що міститься в його дочірньому аудіоелементі.

7. Коли ви натискаєте клавішу, пов’язану з кожним `.drum-pad`, має запускатися аудіокліп, що міститься в його дочірньому аудіоелементі (наприклад, натискання клавіші `Q` має запускати драм-пед, який містить рядок `Q`, натискання клавіші `W` має запускати драм-пед, який містить рядок `W` тощо).

8. Коли запускається `.drum-pad`, ви маєте відобразити рядок, що описує пов’язаний аудіокліп, як внутрішній текст `#display` елемента (кожен рядок має бути унікальним).

Деякі аудіозразки, які ви можете використати для своєї драм-машини, можна знайти за адресою `https://cdn.freecodecamp.org/curriculum/drum/<fileName>`, де `<fileName>` має такий вигляд:

| назва драму | `fileName` |
| --- | --- |
...
У вас має бути `div` елемент з `id` `drum-machine`, який містить усі інші елементи.

```js
assert.isNotNull(document.querySelector('div#drum-machine'));
```

Всередині `#drum-machine` елемента має бути ще один `div` з `id` `pad-bank`.

```js
assert.isNotNull(document.querySelector('#drum-machine div#pad-bank'));
```

Всередині `#drum-machine` елемента має бути `p` елемент з `id` `display`.

```js
assert.isNotNull(document.querySelector('#drum-machine p#display'));
```

Усередині вашого `#pad-bank` елемента має бути дев’ять активних елементів драм-педів, кожен із класом `drum-pad`.

```js
assert.lengthOf(document.querySelectorAll('#pad-bank .drum-pad'), 9);
```

Кожен `.drum-pad` має бути `button` елементом.
...
Кожен `.drum-pad` має мати один із наступних літер як `innerText`, у такому порядку: `Q`, `W`, `E`, `A`, `S`, `D`, `Z`, `X`, `C`.
...
Кожен `.drum-pad` має містити `audio` елемент із класом `clip`, `src` атрибутом, що вказує на аудіокліп, і `id`, що відповідає внутрішньому тексту його батьківського `.drum-pad` елемента (наприклад, `id="Q"`, `id="W"`, `id="E"` тощо).
...
Коли ви натискаєте на `.drum-pad` елемент, має запускатися аудіокліп, що міститься в його дочірньому аудіоелементі.
...
Коли ви натискаєте одну з клавіш Q, W, E, A, S, D, Z, X, C на клавіатурі, відповідний `audio` елемент має відтворити відповідний звук.
...
Коли запускається `.drum-pad`, ви маєте відобразити рядок, що описує пов’язаний аудіокліп, як внутрішній текст `#display` елемента (кожен рядок має бути унікальним).
