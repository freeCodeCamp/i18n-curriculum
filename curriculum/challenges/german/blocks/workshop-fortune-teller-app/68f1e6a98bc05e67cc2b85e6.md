...
Deklarieren Sie eine Variable namens `cardElement` und verwenden Sie die `card_container`-Klasse, um das Element zu erhalten. Nutzen Sie den `closest`-Selektor, um es aus dem `target` zu holen.

# --hints--

Sie sollten eine Variable namens `cardElement` innerhalb von `showFortune` deklarieren.
...
Ihre `cardElement` sollte `target?.closest` verwenden, um das `.card_container` zu finden.
...
Sie sollten `.card_container` als Argument an `closest` übergeben.
