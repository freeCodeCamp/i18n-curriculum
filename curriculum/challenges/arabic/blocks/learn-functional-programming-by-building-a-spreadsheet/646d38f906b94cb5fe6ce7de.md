...
دالتك `addCharacters(char1)` تُعيد أيضًا دالة، وهذه الدالة تُعيد دالة أخرى. تحتاج إلى إجراء استدعاء دالة آخر للوصول إلى المرجع الداخلي للدالة لاستخدامه كـ `.map()` استدعاء راجع. جافاسكريبت تسمح لك باستدعاء الدوال المُعادة مباشرةً:

```js
myFunc(1)("hi");
```

استدعِ فورًا الدالة التي تُعاد من استدعاء `addCharacters(char1)`، ومرر `char2` كمعلمة.

# --hints--

يجب أن تربط استدعاء دالة لاستدعاء `addCharacters(char1)` الخاص بك.
...
يجب أن تمرر `char2` كمعلمة لاستدعاء الدالة المرتبط.
