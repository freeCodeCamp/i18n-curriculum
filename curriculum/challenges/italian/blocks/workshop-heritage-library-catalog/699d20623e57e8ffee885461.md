...
Lo stesso pattern si applica agli altri tre campi. Aggiungi controlli equivalenti per `author`, `year` e `location`.

Nota che `isValid` si accumula attraverso tutti e quattro i controlli: se un campo fallisce, diventa `false` e rimane `false` anche se i controlli rimanenti passano. Questo significa che la funzione controlla sempre ogni campo invece di fermarsi al primo problema.

# --hints--

`validateEntry(catalog[0])` dovrebbe restituire `true`.

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` dovrebbe restituire `false`.

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` dovrebbe restituire `false`.

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` dovrebbe restituire `false` perché la posizione è `"Unknown"`.
