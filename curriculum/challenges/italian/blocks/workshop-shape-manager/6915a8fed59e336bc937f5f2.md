...
L'ultimo insieme di elementi da selezionare sono gli elementi risultato. Usa la tua funzione `getElement` per ottenere l'elemento con gli id `result-text` e `result-card`. Entrambi dovrebbero usare `HTMLElement` come tipo.

Ricorda che hai già dichiarato le variabili `resultText` e `resultCard` per loro, quindi devi solo assegnarle.

# --hints--

Dovresti assegnare un valore alla variabile `resultText`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` dovrebbe selezionare l'elemento `result-text`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

Dovresti usare `HTMLElement` come tipo del `getElement` che stai usando per selezionare l'elemento `result-text`.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

Dovresti assegnare un valore alla variabile `resultCard`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` dovrebbe selezionare l'elemento `result-card`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

Dovresti usare `HTMLElement` come tipo del `getElement` che stai usando per selezionare l'elemento `result-card`.
