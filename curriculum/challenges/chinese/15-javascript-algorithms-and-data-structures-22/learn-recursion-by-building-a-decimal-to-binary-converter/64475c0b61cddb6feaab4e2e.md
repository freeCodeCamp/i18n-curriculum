...
现在，如果用户没有输入数字，或者在尝试将数字转换为二进制之前数字无效，你可以提醒用户。

在 `if` 语句的主体中，使用 `alert()` 方法显示文本 `"Please provide a decimal number greater than or equal to 0"`。

请注意，`alert()` 是浏览器中 `window` 对象上的方法，因此你可以使用 `window.alert()` 或 `alert()`。
