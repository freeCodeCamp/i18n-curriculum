...
يمثل كل `tab` أحد الأزرار في قائمة التبويبات. يجب أن يتمكن المستخدم من النقر على تبويب لعرض اللوحة المقابلة وإخفاء البقية.

في الوحدات السابقة، تعلمت كيف تُرفق مستمع حدث `"click"` لكل زر في `NodeList` بهذه الطريقة:
...
استخدم طريقة `addEventListener` لإرفاق مستمع حدث `"click"` لكل زر `tab`. يجب أن تكون المعلمة الثانية لطريقة `addEventListener` دالة سهمية فارغة `() => {}` في الوقت الحالي.

# --hints--

يجب أن يكون لديك `tab.addEventListener('click', () => {})` داخل حلقة `forEach`.
