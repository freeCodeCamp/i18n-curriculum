...
입력 필드에 텍스트가 있을 때만 `Cancel`와 `Discard` 버튼을 사용자에게 표시하세요.

먼저 `confirmCloseDialog.showModal();` 줄을 지우세요.

그다음 `closeTaskFormBtn` 이벤트 리스너 안에서 `titleInput`, `dateInput` **또는** `descriptionInput` 필드에 값이 있는지 확인하는 로직을 작성하세요.

이 입력 필드 중 하나라도 값이 있으면 `showModal()`에서 `confirmCloseDialog` 메서드를 사용하세요. 그렇지 않고 변경 사항이 없으면 입력 필드를 지우고 폼 모달을 숨기기 위해 `reset()` 함수를 호출하세요.

# --hints--

입력 필드 중 하나라도 값이 있으면 확인 대화상자가 표시되어야 합니다. 그렇지 않으면 `reset` 함수가 호출되어야 합니다.
