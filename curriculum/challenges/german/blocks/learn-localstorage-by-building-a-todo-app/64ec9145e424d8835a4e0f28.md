...
Da Sie die Aufgabe nun im `taskData`-Array gespeichert haben, sollten Sie die Aufgabe auf der Seite anzeigen, indem Sie sie durchlaufen.

Verwenden Sie `forEach()` auf `taskData`, und destrukturieren Sie dann `id`, `title`, `date`, `description` als Parameter. Geben Sie noch nichts zurück.

# --hints--

Sie sollten `forEach()` auf `taskData` verwenden.

```js
assert.match(code, /taskData\.forEach\(\s*/)
```

Sie sollten `{id, title, date, description}` als destrukturierte Parameter der `forEach()`-Callback-Funktion verwenden.
