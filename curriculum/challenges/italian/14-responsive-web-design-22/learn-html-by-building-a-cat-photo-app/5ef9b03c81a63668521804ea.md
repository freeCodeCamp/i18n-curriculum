...
Your `head` element should have an opening tag. I tag di apertura hanno la seguente sintassi: `<nomeElemento>`.

```js
assert(code.match(/\<head\>/));
```

Your `head` element should have a closing tag. I tag di chiusura hanno un carattere `/` subito dopo il carattere `<`.
