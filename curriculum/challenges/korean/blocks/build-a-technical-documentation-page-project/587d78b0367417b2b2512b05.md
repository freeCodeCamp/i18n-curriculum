...
1. `main-section` 클래스를 가진 각 `section` 요소에는 `id`가 있어야 하며, 이 id는 그 안에 포함된 각 `header` 텍스트와 일치해야 합니다. 공백은 밑줄(_)로 교체되어야 합니다(예: "JavaScript and Java"라는 헤더가 있는 섹션은 해당하는 `id="JavaScript_and_Java"`를 가져야 합니다).
