...
마지막 문제를 해결할 시간입니다. 항목의 이름이나 설명에 따옴표 같은 특수 문자가 포함된 할 일 작업이 있으면 애플리케이션이 오류를 일으킵니다. 겉보기와 달리 올바른 항목이 사라진 것처럼 보입니다.

이를 고치려면 문자열을 입력으로 받아 모든 특수 문자를 제거하는 `removeSpecialChars`라는 함수를 만들어야 합니다.

# --hints--

`removeSpecialChars` 함수를 정의하세요.

```js
assert.isFunction(removeSpecialChars);
```

`removeSpecialChars` 함수는 공백을 제거하지 않아야 합니다.
