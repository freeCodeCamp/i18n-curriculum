...
創建一個 `random` 屬性。 此屬性應使用數組中的前兩個數字來生成隨機整數。 這個數字的範圍從兩個數字中的第一個數字（包含）開始，到這兩個數字之和之前結束。 使用 `Math.floor()` 和 `Math.random()` 方法進行計算。

# --hints--

你的 `spreadsheetFunctions` 對象應該具有 `random` 屬性。
...
你的 `random` 函數應該返回數組中前兩個數字之間的一個隨機數。
