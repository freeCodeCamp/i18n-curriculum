...
Ora è il momento di creare la funzione che si occuperà di mostrare gli autori sulla pagina.

Inizia creando una funzione chiamata `displayAuthors` che prende un parametro `authors`. La tua funzione deve poi iterare sull'array `authors` e aggiungere una scheda per ogni autore all'elemento `authorContainer`.

La scheda deve avere la seguente struttura:
...
- Puoi usare qualsiasi tipo di loop per iterare sull'array `authors`. Es. `for`, `forEach`, `map`, ecc.
- Dovresti usare la proprietà `innerHTML` per aggiungere la scheda a `authorContainer` all'interno del loop. Ti servirà anche la sintassi dei template literal per aggiungere il markup.
...
La tua funzione `displayAuthors` dovrebbe prendere un parametro `authors`.
...
Quando la funzione `displayAuthors` viene chiamata con un array di autori, dovrebbe aggiungere una scheda per ogni autore all'elemento `#authorContainer`.
