...
Your `input` element does not have a `type` attribute with the value `text`. 检查开始标签的名称后面是否有空格和/或所有属性名称之前是否有空格。

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('type'));
```

Your `input` element should have a `type` attribute with the value `text`. 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
