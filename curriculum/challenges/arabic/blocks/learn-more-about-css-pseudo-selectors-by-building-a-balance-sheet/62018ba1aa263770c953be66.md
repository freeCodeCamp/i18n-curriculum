...
أعطِ التسمية التوضيحية موقعًا مطلقًا، وحركها بمقدار `-2.25rem` من الأعلى و`0.5rem` من اليسار.

# --hints--

يجب أن يحتوي محدِّد `table caption` على خاصية `position` مضبوطة على `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('position') === 'absolute');
```

يجب أن يحتوي محدِّد `table caption` على خاصية `top` مضبوطة على `-2.25rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('top') === '-2.25rem');
```

يجب أن يحتوي محدِّد `table caption` على خاصية `left` مضبوطة على `0.5rem`.
