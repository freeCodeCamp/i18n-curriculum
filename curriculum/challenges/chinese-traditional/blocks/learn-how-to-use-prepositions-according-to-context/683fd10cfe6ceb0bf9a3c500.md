...
`Middle of` means the center of a place. 例如：

`The desk is in the middle of the room.` – This shows it is in the center.

`Reachable` means something or someone can be easily contacted or accessed. 例如：

`The manager is always reachable by email.` – This means you can contact them easily.

`Behind` shows position at the back of something. 例如：
