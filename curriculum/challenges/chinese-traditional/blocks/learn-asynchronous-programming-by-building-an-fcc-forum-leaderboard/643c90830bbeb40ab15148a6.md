...
現在是時候開始在 `postsContainer` 中填充資料。

首先在你的 `topics` 陣列上呼叫 `map()` 方法。對於回呼函式，使用一個以 `item` 為參數的空箭頭函式。

然後將 `map()` 方法的結果指定給 `postsContainer.innerHTML`。

# --hints--

你應該在 `topics` 陣列上使用 `map()` 方法。

```js
assert.match(code, /topics\.map\(.*/);
```

你應該為 `map()` 方法添加一個回呼函式，其參數名稱為 `item`。
...
你應該將 `map()` 方法的結果指定給 `postsContainer.innerHTML`。
