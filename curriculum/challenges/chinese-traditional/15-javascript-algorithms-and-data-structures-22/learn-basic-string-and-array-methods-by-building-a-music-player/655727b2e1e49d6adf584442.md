...
如果播放列表爲空，你需要創建一個 `resetButton` 元素併爲其創建文本。 只有在播放列表爲空的情況下，該按鈕纔會顯示。

`createElement()` 是一種 DOM 方法，可用於使用 JavaScript 動態創建元素。 要使用 `createElement()`，你需要調用它，然後將標籤名稱作爲字符串傳入：
...
在你的 `if` 語句中，聲明一個 `resetButton` 常量，然後使用 `createElement()` 創建一個 `"button"`。
...
你應該使用 `createElement` 方法創建一個 `"button"`。

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

你應該把新創建的 `button` 元素分配給 `resetButton` 常量。
