...
Utiliza la misma sintaxis que tu función `addCharacters` para actualizar tu función `elemValue`. Ya no debería declarar `inner`, pero debería devolver la función implícitamente.

# --hints--

Tu función `elemValue` debe usar un retorno implícito.
...
Tu función `elemValue` debería devolver implícitamente una función flecha con un parámetro `character`.
...
Tu función flecha interna debería usar un retorno implícito.
...
Tu función flecha interna debería devolver el resultado de llamar a `idToText()` con `character + num` como argumento.
