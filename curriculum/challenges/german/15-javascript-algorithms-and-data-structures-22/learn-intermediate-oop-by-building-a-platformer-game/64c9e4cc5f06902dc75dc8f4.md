...
Start by calling the `forEach` method on the `platforms` array. Übergib für die Callback-Funktion `platform` als Parameter.
...
Deine Callback-Funktion sollte einen `platform`-Parameter verwenden.
