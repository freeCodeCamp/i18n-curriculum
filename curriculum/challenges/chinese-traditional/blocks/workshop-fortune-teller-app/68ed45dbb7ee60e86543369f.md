...
取得資料後，將回應存入變數並解析成 JSON。將解析後的資料指定型別為 `Deck`，然後指派給 `cards` 屬性。在 `catch` 區段內，確保使用這則訊息 `"Error fetching cards:", error` 來記錄錯誤。

# --hints--

你應該在 `data` 內宣告一個名為 `fetchCardsData` 的變數。
...
你應該將 `data.cards` 存入 `this.cards`。

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

你的 `catch` 區段應該使用 `console.error()` 來用訊息 `"Error fetching cards:", error` 記錄錯誤。
