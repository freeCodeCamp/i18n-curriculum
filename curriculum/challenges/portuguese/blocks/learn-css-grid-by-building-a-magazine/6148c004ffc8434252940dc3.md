...
Crie o seletor `.social-icons`. Dê a ele a propriedade `display` definida com `grid` e a propriedade `font-size` definida como `3rem`.

# --hints--

Você deve ter um seletor `.social-icons`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

O seletor `.social-icons` deve ter a propriedade `display` definida como `grid`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display === 'grid');
```

O seletor `.social-icons` deve ter a propriedade `font-size` definida como `3rem`.
