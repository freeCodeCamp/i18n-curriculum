...
リスト内包表記は、イテラブルからアイテムを除外するための`if`節も受け入れます。
...
無効なキーだけを含むリストを返したいので、`if`節を内包表記に追加し、`key`が偽の場合にのみ各`value`がリストに追加されるようにしてください。

# --hints--

あなたの`find_invalid_records`関数は、`key`が偽の場合に`key, value`の各`constraints.items()`に対して`value`を評価するリスト内包表記を返す必要があります。
...
あなたの`find_invalid_records`関数は、`constraints.items()`を繰り返すリスト内包表記を返す必要があります。
...
あなたの`find_invalid_records`関数は、`key, value`を使って`constraints.items()`を繰り返すリスト内包表記を返す必要があります。
...
あなたの`find_invalid_records`関数は、`key`の各`key, value`に対して`constraints.items()`を評価するリスト内包表記を返す必要があります。
