...
Хоча це не обов’язково для `label` елементів із вкладеним `input`, все ж найкращою практикою є явне зв’язування `label` з відповідним `input` елементом.

Тепер додайте атрибут `for` до кожного з ваших чотирьох `label`, який зв’язує `label` з відповідним радіо `input`.

# --hints--

Ви повинні додати атрибут `label` першому `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[0]?.htmlFor);
```

Ви повинні додати атрибут `label` першому `for`, який співпадає з `id` його `input` елемента.
...
Ви повинні додати атрибут `label` другому `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[1]?.htmlFor);
```

Ви повинні додати атрибут `label` другому `for`, який співпадає з `id` його `input` елемента.
...
Ви повинні додати атрибут `label` третьому `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[2]?.htmlFor);
```

Ви повинні додати атрибут `label` третьому `for`, який співпадає з `id` його `input` елемента.
...
Ви повинні додати атрибут `label` четвертому `for`.

```js
assert.notEmpty(document.querySelectorAll('ul.answers-list > li > label')?.[3]?.htmlFor);
```

Ви повинні додати атрибут `label` четвертому `for`, який співпадає з `id` його `input` елемента.
