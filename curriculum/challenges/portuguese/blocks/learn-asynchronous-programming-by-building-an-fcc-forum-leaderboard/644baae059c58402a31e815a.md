...
Se o `id` não estiver no objeto `allCategories`, você precisará exibir a categoria `General`.

Abaixo da sua declaração `if`, adicione uma cláusula `else`. 

# --hints--

Você deve ter uma cláusula `else` abaixo da sua declaração `if`.
