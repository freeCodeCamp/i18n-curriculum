...
シミュレーションを始める前に、各ゲートがすべての目盛りで処理した参加者数を追跡するオブジェクトが必要です。`initializeThroughput()`を`gates`で呼び出し、その結果を`throughputSummary`という名前の変数に代入してください。

# --hints--

`initializeThroughput()`を`gates`で呼び出し、その結果を`throughputSummary`という名前の変数に代入してください。
...
あなたの`simulateFestival`関数は`initializeThroughput()`関数を`gates`で呼び出す必要があります。
