...
Журнал `byDecade` виконав своє завдання. Видаліть `console.log(byDecade)` зі свого коду.

Потім, щоб записи каталогу було легше переглядати, створіть функцію з назвою `renderEntry`, яка прийматиме `entry` як параметр. Тіло функції поки що залиште порожнім. Нижче за функцією виведіть у журнал `renderEntry(catalog[0])`.

# --hints--

`renderEntry` має бути функцією.

```js
assert.isFunction(renderEntry);
```

`renderEntry` має бути функцією з параметром `entry`.
...
Виклик `console.log(byDecade)` слід видалити.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*byDecade\s*\)/);
```

Ви маєте вивести у журнал `renderEntry(catalog[0])`.
