...
Використайте метод `filter()`, щоб видалити об’єкт пісні, який відповідає параметру `id` із масиву `userData?.songs`.

Метод `filter` зберігає лише ті елементи масиву, які задовольняють передану йому функцію зворотного виклику:
...
Використайте метод `filter()` на `userData?.songs`. Передайте `song` як параметр стрілкової функції зворотного виклику та використайте неявне повернення, щоб перевірити, чи `song.id` суворо не дорівнює `id`. Призначте все це `userData.songs`.

*Примітка*: ви не повинні використати необов’язковий ланцюжок, коли ви призначаєте результат `userData?.songs.filter` до `userData.songs`, тому що масив `allSongs` не буде `undefined` або `null` на цьому етапі.

# --hints--

Не змінюйте наявну функцію `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Використайте метод `filter()` на `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

Функція зворотного виклику методу `filter()` має використати стрілковий синтаксис та мати параметр `song`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

Метод фільтрації має неявно повернути `song.id !== id`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

Призначте метод `filter()`, який ви використовували на `userData?.songs`, до `userData.songs`.
