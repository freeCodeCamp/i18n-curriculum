...
Bevor Sie geparste Daten vertrauen, beginnen Sie mit der Validierung der Pflichtfelder.

Erstellen Sie eine Funktion namens `validateEntry`, die `entry` als Parameter erhält. Deklarieren Sie `let isValid = true`.

Fügen Sie eine Validierungsprüfung für `title` hinzu: Setzen Sie `isValid = false`, wenn `title` fehlt, falsch ist oder `"Unknown"` entspricht.

Geben Sie `isValid` zurück. In den nächsten Schritten fügen Sie Prüfungen für die anderen Felder hinzu.

# --hints--

`validateEntry` sollte einen Boolean zurückgeben.

```js
assert.isBoolean(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "T", author: "A", year: 2000, location: "L" })` sollte `true` zurückgeben.

```js
assert.isTrue(validateEntry({ title: "T", author: "A", year: 2000, location: "L" }));
```

`validateEntry({})` sollte `false` zurückgeben.

```js
assert.isFalse(validateEntry({}));
```

`validateEntry({ title: "", author: "A", year: 2000, location: "L" })` sollte `false` zurückgeben.

```js
assert.isFalse(validateEntry({ title: "", author: "A", year: 2000, location: "L" }));
```

`validateEntry({ title: "Unknown", author: "A", year: 2000, location: "L" })` sollte `false` zurückgeben.
