...
يمكنك الآن ربط `str(len(words[line_index]))` بـ `secret_code`. احذف استدعاء `print` واستخدم التعيين المعزز `+=` للربط.

# --hints--

داخل الحلقة يجب أن يكون لديك `secret_code += str(len(words[line_index]))`.
