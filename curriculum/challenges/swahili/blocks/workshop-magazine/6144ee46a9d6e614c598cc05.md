...
Unda kichaguzi cha `a`, na kipa sifa ya `text-decoration` iwe `none` na sifa ya `color` iwe `linen`.
...
Kichaguzi chako cha `a` kinapaswa kuwa na sifa ya `text-decoration` iwe `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration, 'none');
```

Kichaguzi chako cha `a` kinapaswa kuwa na sifa ya `color` iwe `linen`.
