...
`select` 요소에서 값을 가져와야 합니다. 이 값들은 현재 문자열이며 숫자로 변환할 것입니다.

배열을 순회하려면 `map` 함수를 사용하세요. `map`에 콜백 함수를 전달하는데, 이 함수는 `dropdown` 매개변수를 받고 `dropdown.value`를 반환합니다.

# --hints--

배열에서 `map` 메서드를 바로 연결해서 사용하세요.

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.getElementsByClassName\(\s*('|"|`)values-dropdown\1\s*\)\s*\]\s*\.map\(/);
```

`map` 콜백 함수는 화살표 함수 구문을 사용해야 합니다.

```js
assert.match(code, /inputValues\s*=\s*\[\s*\.\.\.document\.getElementsByClassName\(\s*('|"|`)values-dropdown\1\s*\)\s*\]\s*\.map\s*\(\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`map` 콜백 함수는 `dropdown` 매개변수를 받아야 합니다.
...
`map` 콜백 함수는 `dropdown.value`을 암묵적으로 반환해야 합니다.
