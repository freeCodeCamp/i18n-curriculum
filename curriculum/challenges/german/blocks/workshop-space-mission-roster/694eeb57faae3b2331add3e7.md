...
Rufen Sie innerhalb der `getEVAReadyCrew`-Funktion `sortByPriorityDescending()` mit dem lokalen `eligible`-Array auf, damit es sortiert wird, bevor es zurückgegeben wird.

# --hints--

Sie sollten `sortByPriorityDescending()` mit dem lokalen `eligible`-Array aufrufen.
