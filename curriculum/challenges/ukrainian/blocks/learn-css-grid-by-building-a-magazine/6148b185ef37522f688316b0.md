...
Тепер ви можете стилізувати вміст елемента `.heading` за допомогою CSS сітки.

CSS функція `repeat()` використовується для повторення значення замість того, щоб писати його вручну, і є корисною для макетів сітки. Наприклад, встановлення властивості `grid-template-columns` у значення `repeat(20, 200px)` створить 20 стовпців, кожен шириною `200px`.

Задайте вашому елементу `.heading` властивість `grid-template-columns` зі значенням `repeat(2, 1fr)`, щоб створити два стовпці однакової ширини.

# --hints--

Ваш селектор `.heading` повинен мати властивість `grid-template-columns` зі значенням `repeat(2, 1fr)`.
