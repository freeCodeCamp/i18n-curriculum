...
Nun bist du bereit, das Rasterlayout zu erstellen. „CSS Grid“ bietet ein zweidimensionales Gitterlayout an, das es dir erlaubt, Elemente horizontal und vertikal zu zentrieren und dabei gleichzeitig die Kontrolle zu behalten, um Dinge wie Überlappungen durchzuführen.

Beginne mit dem Erstellen eines `main`-Selektors und gib ihm eine `display`-Eigenschaft, die auf `grid` eingestellt ist.

# --hints--

Du solltest einen `main`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('main'));
```

Dein `main`-Selektor sollte eine `display`-Eigenschaft mit `grid` als Wert haben.
