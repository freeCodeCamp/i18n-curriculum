...
Так ближче, але ціна не залишилась в правій частині. Це тому що елементи `inline-block` займають лише ширину свого вмісту. To spread them out, add a `width` property to the `flavor` and `price` class selectors that have a value of `50%` each.
