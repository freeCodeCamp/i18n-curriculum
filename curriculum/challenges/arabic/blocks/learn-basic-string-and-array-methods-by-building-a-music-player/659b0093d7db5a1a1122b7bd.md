...
للحصول على الفهرس للأغنية الحالية، يمكنك استخدام الطريقة <dfn>indexOf()</dfn>. تُرجع طريقة المصفوفة `indexOf()` أول فهرس يمكن العثور فيه على عنصر معين في المصفوفة، أو `-1` إذا لم يكن العنصر موجودًا.
...
داخل دالتك `getCurrentSongIndex`، أرجع `userData?.songs.indexOf()`. بالنسبة لمُعلمة `indexOf()`، اجعلها `userData?.currentSong`.

# --hints--

يجب أن تُرجع `userData?.songs.indexOf()` داخل دالتك `getCurrentSongIndex`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

يجب أن تمرر `userData?.currentSong` إلى طريقة `indexOf()`.
