...
你為你的腳本添加了 `console.log()` 呼叫來驗證你的 `EVAReadySquad` 陣列，現在該呼叫已不再需要。為了防止終端機在後續步驟中變得混亂，請將目前用來將 `EVAReadySquad` 太空人記錄到主控台的 `for` 迴圈移除。

# --hints--

移除將 `EVAReadySquad` 太空人記錄到主控台的 `for` 迴圈。你應該刪除整個迴圈區塊。
