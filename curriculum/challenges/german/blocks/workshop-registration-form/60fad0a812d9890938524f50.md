...
Um den Lernenden eine Vorstellung davon zu geben, was sie in ihre Bio schreiben können, wird das `placeholder`-Attribut verwendet. Das `placeholder` akzeptiert einen Textwert, der angezeigt wird, bis der Benutzer zu tippen beginnt.

Geben Sie dem `textarea` ein `placeholder` von `I like coding on the beach...`.

# --hints--

Sie sollten dem `textarea` ein `placeholder`-Attribut geben.

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

Sie sollten dem `placeholder` einen Wert von `I like coding on the beach...` geben.
