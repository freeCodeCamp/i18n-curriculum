...
Crea un'ultima query `@media` per `only screen` con un `max-width` di `420px`. All’interno, crea un selettore `.hero-title` con una proprietà `font-size` impostata su `4.5rem`.

Congratulazioni! La tua rivista è ora completa.

# --hints--

Dovresti avere una nuova query `@media` per `only screen and (max-width: 420px)`. Questa dovrebbe essere l’ultima query nella lista di query `@media`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

La tua nuova query `@media` dovrebbe avere un selettore `.hero-title`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText === '.hero-title');
```

Il tuo selettore `.hero-title` dovrebbe avere una proprietà `font-size` impostata su `4.5rem`.
