...
أعطِ المحدِّد `.text` خاصية `font-size` بقيمة `1.8rem` وخاصية `letter-spacing` بقيمة `0.6px`.

# --hints--

يجب أن يحتوي المحدِّد `.text` على خاصية `font-size` بقيمة `1.8rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize, '1.8rem');
```

يجب أن يحتوي المحدِّد `.text` على خاصية `letter-spacing` بقيمة `0.6px`.
