...
Font Awesome é uma biblioteca conveniente de ícones.  These icons can be web fonts or vector graphics. Estes ícones são tratados assim como as fontes. Você pode especificar seu tamanho utilizando pixels, e eles assumirão o tamanho de fonte de seus elementos pais no HTML.

# --instructions--

Utilize Font Awesome para adicionar um ícone `info-circle` ao botão de informação e um ícone `trash` para o botão de deleção.

**Nota:** Você pode usar elementos `i` ou `span` para completar este desafio.
...
Você deve adicionar um `<i class="fas fa-trash"></i>` dentro do elemento do seu botão `delete`.
...
Cada um dos seus elementos `i` deve ter uma tag de fechamento e `<i class="fas fa-thumbs-up"></i>` está no seu elemento `like` do botão.
