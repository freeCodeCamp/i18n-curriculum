...
`.face` 클래스가 있는 `left` 요소를 선택하고, 부모의 왼쪽에 `5%` 위치시키세요.

# --hints--

`.face.left` 선택자를 사용하세요.

```js
assert.match(code, /\.face\.left\s*\{/);
```

`.face.left`에 `left` 속성을 지정하세요.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face.left')?.left);
```

`.face.left`에 `left`을 `--fcc-expected--`로 지정해야 하지만 `--fcc-actual--`가 발견되었습니다.
