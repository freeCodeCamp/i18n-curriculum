...
Dentro del `tbody`, agregue un `tr` con una `class` establecida en `total`. En eso, agrega un `th` con el texto `Total Net Worth`, y anida `Net Worth` en un `span` con la `class` establecida en `sr-only`.
