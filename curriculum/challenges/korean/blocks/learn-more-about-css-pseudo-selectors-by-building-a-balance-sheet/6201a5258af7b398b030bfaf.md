...
현재 달러 금액이 정렬되지 않았습니다. `td` 요소 안에 있는 `tr.data` 요소를 선택하는 선택자를 만드세요. 텍스트를 수직으로 위쪽에 맞추고, 수평으로 오른쪽에 맞추며, 패딩을 `0.3rem 0.25rem 0`로 설정하세요.

# --hints--

`tr.data td` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td'));
```

`tr.data td` 선택자에 `vertical-align` 속성을 `top`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td')?.getPropertyValue('vertical-align') === 'top');
```

`tr.data td` 선택자에 `text-align` 속성을 `right`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td')?.getPropertyValue('text-align') === 'right');
```

`tr.data td` 선택자에 `padding` 속성을 `0.3rem 0.25rem 0`로 설정해야 합니다.
