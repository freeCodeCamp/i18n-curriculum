...
你對 `evalFormula` 的第一次呼叫引數需要是儲存於 `value` 的儲存格內容。然而，內容以 `=` 字元開頭以觸發函式（程式），所以你需要傳遞從索引 `1` 開始的 `value` 子字串。

# --hints--

你應該將 `value` 作為第一個引數傳遞給你的 `evalFormula()` 呼叫。
