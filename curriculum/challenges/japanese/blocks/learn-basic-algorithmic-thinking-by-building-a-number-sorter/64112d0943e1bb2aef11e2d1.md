...
配列の要素をソートするには、`.sort()`という組み込みメソッドを使えます。したがって、`sortedValues`配列に対して`.sort()`を呼び出した結果を代入して、`inputValues`変数を更新できます。

# --hints--

`sortedValues`配列に対して`.sort()`メソッドを呼び出した結果で、`inputValues`変数を更新してください。

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(/);
```

`.sort()`メソッドには何も渡さないでください。

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*\)/);
```

コード内で`selectionSort`を呼び出さないでください。
