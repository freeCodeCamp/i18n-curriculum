...
入力が有効であることも確認する必要があります。 `invalidInputMatch` という変数を宣言し、`currVal` を引数として渡して `isInvalidInput` 関数を呼び出した結果を、その変数に割り当ててください。

# --hints--

変数 `invalidInputMatch` を宣言してください。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=/);
```

`isInvalidInput` の呼び出し結果を、変数 `invalidInputMatch` に代入してください。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(/);
```

`isInvalidInput` に引数として `currVal` を渡してください。
