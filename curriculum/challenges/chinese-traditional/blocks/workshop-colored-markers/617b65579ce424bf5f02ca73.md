...
正如你在講座中學到的，有兩種主要的顏色模型：用於電子設備的 <dfn>加色</dfn> RGB（紅、綠、藍）模型，以及用於打印的 <dfn>減色</dfn> CMYK（青、品紅、黃，黑）模型。

在這個項目中，將使用 RGB 模型。 這意味着顏色從黑色開始，隨着紅色、綠色和藍色的不同層次而變化。 一個簡單的方法就是使用 CSS `rgb` 函數。

創建一個定位 `container` 類的新 CSS 規則，並使用 `rgb(0, 0, 0)` 將其 `background-color` 設置爲黑色。

# --hints--

你應該使用一個類選擇器來定位類 `container`。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.container'));
```

你的 `.container` CSS 規則應該有一個 `background-color` 屬性，設置爲 `rgb(0, 0, 0)`。
