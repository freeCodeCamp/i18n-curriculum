...
이 `@keyframes` @규칙은 애니메이션된 요소에 어떤 영향을 주나요?
...
다음 CSS `@keyframes` @규칙에서 `50%`는 무엇을 나타내나요?
...
너무 많은 CSS 애니메이션은 검색 엔진 결과에서 순위가 낮아지거나 아예 나타나지 않을 수 있습니다.
...
`prefers-reduced-motion` 미디어 쿼리 내에서 전환을 비활성화하는 선언은 무엇인가요?
