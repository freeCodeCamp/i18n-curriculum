...
이제 `groupByDecade`를 만들어서 카탈로그를 10년 단위로 정리하세요. 각 키는 `"1970s"` 같은 문자열이고, 각 값은 해당 기간의 책들로 이루어진 배열입니다.

`groupByDecade`를 매개변수로 받는 `catalog`라는 함수를 만드세요. 내부에 `const grouped = {}`를 선언하세요. `for`를 순회하는 `catalog` 반복문을 추가하고, 반복문 안에서 `const book = catalog[i]`를 선언하세요.

# --hints--

`groupByDecade` 매개변수를 가진 `catalog`라는 함수가 있어야 합니다.
