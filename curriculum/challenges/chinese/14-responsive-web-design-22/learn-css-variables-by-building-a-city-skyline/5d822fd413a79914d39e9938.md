...
给 `sky` 类一个 `radial-gradient`。 Use `#ffcf33` from `0%` to `20%`, `#ffff66` at `21%`, and `#bbeeff` at `100%`. 这将为背景添加圆形渐变，也就是你的太阳。

# --hints--

你应该在 `background` 属性中为 `.sky` 提供一个 `radial-gradient`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".sky")?.background, "radial-gradient");
```

你应该给 `radial-gradient` 第一种颜色 `#ffcf33`。

```js
assert.match(new __helpers.CSSHelp(document).getStyle(".sky")?.background, /radial-gradient\(rgb\(255, 207, 51\)( 0%)?, rgb\(255, 207, 51\) 20%/);
```

你应该在 `21%` 处为 `radial-gradient` 提供第二种颜色 `#ffff66`。
...
你应该在 `100%` 处为 `radial-gradient` 提供第三种颜色 `#bbeeff`。
