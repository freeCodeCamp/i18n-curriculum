...
كلمة `Everything` تُستخدم للإشارة إلى جميع الأشياء في مجموعة معينة. عندما يقول توم `everything is perfect`، فهو يقصد أن كل جوانب مكان عمله الجديد مثالية.
...
هذه الكلمة تشير إلى كل شيء دون استثناء.
...
هذه صيغة من فعل `to be`، وتُستخدم مع `he` أو `she` أو `it`.
