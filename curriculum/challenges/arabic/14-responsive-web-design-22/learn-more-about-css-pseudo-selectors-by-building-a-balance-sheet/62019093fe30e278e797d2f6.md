...
يجب أن يكون لديك منتقي `tr[class="total"]`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"]'));
```

يجب يحتوي المنتقي `tr[class="total"]` الخاص بك على خاصية `border-bottom` بقيمة `4px double #0a0a23`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"]')?.getPropertyValue('border-bottom') === '4px double rgb(10, 10, 35)');
```

يجب يحتوي منتقي `tr[class="total"]` الخاص بك على خاصية `font-weight` بقيمة `bold`.
