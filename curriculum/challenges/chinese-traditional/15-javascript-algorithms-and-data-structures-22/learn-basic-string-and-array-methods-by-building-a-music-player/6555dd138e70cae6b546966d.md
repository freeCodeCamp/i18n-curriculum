...
你需要獲取當前播放的歌曲或播放列表中的第一首歌曲。 爲此，創建一個 `song` 常量並使用 OR 運算符 (`||`) 將其設置爲 `userData` 的當前歌曲 **或** `userData?.songs` 數組中的第一首歌曲。

不要忘記使用可選鏈接。

# --hints--

你不應修改現有的 `setPlayButtonAccessibleText` 及其內容。

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

你應該訪問 `userData?.currentSong` 或 `userData?.songs[0]`。

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

你應該將 `userData?.currentSong || userData?.songs[0]` 分配給 `song` 常量。
