...
Es hora de implementar una funcionalidad de eliminación para la lista de reproducción. Esto gestionaría la eliminación de una canción de la lista de reproducción, manejaría otras acciones relacionadas cuando se elimina una canción, y crearía un botón de `Reset Playlist`.

Usa `const` y la sintaxis de flecha para crear una función vacía `deleteSong` y pasa `id` como un parámetro.

# --hints--

Debes usar `const` para crear una función vacía llamada `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` debería ser una función.

```js
assert.isFunction(deleteSong)
```

Tu función `deleteSong` debería usar una sintaxis de flecha.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Tu función `deleteSong` debería aceptar un parámetro `id`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

Tu función `deleteSong` debería estar vacía.
