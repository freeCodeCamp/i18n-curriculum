...
开始优化你的代码，首先将 `position`
和 `top` 属性及其值从 `.foreground-buildings` 中移动到 `.background-buildings` 中，然后修改现有的 `.background-buildings` 选择器，使其​​同时作用于 `.foreground-buildings`，从而实现样式复用。 你可以使用逗号（`,`）分隔选择器，例如：`selector1, selector2`。
...
你不应该有一个单独的 `.background-buildings` 选择器。
