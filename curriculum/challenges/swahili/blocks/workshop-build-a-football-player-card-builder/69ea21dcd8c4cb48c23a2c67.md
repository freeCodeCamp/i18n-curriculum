...
Add an `onChange` attribute set to an arrow function that takes `e` as its parameter and calls `setPlayer` with a spread of `player` and `position` set to `e.target.value`, cast as `Position`.
