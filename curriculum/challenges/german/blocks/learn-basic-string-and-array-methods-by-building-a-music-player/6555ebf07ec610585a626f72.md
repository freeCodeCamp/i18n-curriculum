...
Verwenden Sie die `filter()`-Methode, um das Song-Objekt zu entfernen, das mit dem `id`-Parameter aus dem `userData?.songs`-Array übereinstimmt.

Die `filter`-Methode behält nur die Elemente eines Arrays bei, die die übergebene Callback-Funktion erfüllen:
...
Verwenden Sie die `filter()`-Methode auf `userData?.songs`. Übergeben Sie `song` als Parameter der Pfeilfunktion-Callback und verwenden Sie eine implizite Rückgabe, um zu prüfen, ob `song.id` strikt ungleich `id` ist. Weisen Sie das alles `userData.songs` zu.

*Hinweis*: Sie sollten keine optionale Verkettung verwenden, wenn Sie das Ergebnis von `userData?.songs.filter` `userData.songs` zuweisen, da das `allSongs`-Array zu diesem Zeitpunkt weder `undefined` noch `null` sein wird.

# --hints--

Sie sollten die bestehende `deleteSong`-Funktion nicht ändern.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Sie sollten die `filter()`-Methode auf `userData?.songs` verwenden.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

Die Callback-Funktion Ihrer `filter()`-Methode sollte Pfeilsyntax verwenden und einen `song`-Parameter haben.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

Ihre Filter-Methode sollte implizit `song.id !== id` zurückgeben.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

Sie sollten die `filter()`-Methode, die Sie auf `userData?.songs` verwendet haben, `userData.songs` zuweisen.
