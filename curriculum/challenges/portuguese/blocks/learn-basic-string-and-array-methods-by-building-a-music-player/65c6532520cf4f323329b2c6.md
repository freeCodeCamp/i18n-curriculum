...
No momento a ordem das músicas não mudou.Isso acontece porque as atualizações que você fez usando o método `sort` não ocorrerão até que a função `sortSongs` seja chamada. 

Altere sua função `renderSongs` para chamar a função `sortSongs`.

Agora você deve ver as músicas em ordem alfabética.

# --hints--

Você deve ter `renderSongs(sortSongs())`.
