...
Оголосіть змінну з іменем `plantsSet` і призначте їй виклик функції `displayPlantsSet`.

Потім виведіть `plantsSet` у консоль. Вам потрібно буде відкрити консоль браузера, щоб побачити вашу множину.


# --hints--

У вас має бути змінна з іменем `plantsSet`.

```js
assert.exists(plantsSet);
```

Ви маєте викликати `displayPlantsSet` і призначити це `plantsSet`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

Виведіть `plantsSet` у консоль.
