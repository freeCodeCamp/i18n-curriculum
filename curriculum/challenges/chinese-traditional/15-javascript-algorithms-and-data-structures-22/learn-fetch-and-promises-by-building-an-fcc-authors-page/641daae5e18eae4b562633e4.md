...
最後，如果出現錯誤並且作者數據無法加載怎麼辦？ 然後我們需要在 UI 中顯示錯誤。 這正是 `.catch()` 方法的用途——處理錯誤。

在 `.catch()` 內部，刪除 `console.error()`，並將 `authorContainer` 的 `innerHTML` 設置爲 `p` 元素，其中包含 `class`、`"error-msg"` 和文本 `"There was an error loading the authors"`。
...
你應該刪除 `console.error` 及其文本。

```js
assert.notMatch(code, /console\.error\(\s*`There\s+was\s+an\s+error:\s+\$\{err\}`\s*\)\s*;?/)
```

你應該訪問 `authorContainer` 的 `innerHTML` 並將其設置爲 `p` 元素。 不要忘記用模板文字包圍 `p` 元素。
...
你的 `p` 元素應具有 `"error-msg"` 類。

```js
assert.match(code, /(`|"|')\s*<p\s+class\s*=\s*("|')error-msg\2>/)
```

你的 `p` 元素應該包含文本 `"There was an error loading the authors"`。
