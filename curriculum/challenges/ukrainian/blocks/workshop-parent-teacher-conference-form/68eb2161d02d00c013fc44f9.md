...
Щоб зробити текст `legend` більш помітним, потрібно його стилізувати.

Виберіть елемент `fieldset legend`. Встановіть його `font-size` у `1.3rem` та його `font-weight` у `600`.

`font-weight` керує жирністю тексту.

Загальні значення `font-weight` такі:

- `normal` → звичайна вага (за замовчуванням)
- `bold` → жирний текст
- `lighter` → світліший за батьківський елемент
- `bolder` → жирніший за батьківський елемент
...
Ви маєте мати селектор `fieldset legend`.

```js
assert(new __helpers.CSSHelp(document).getStyle('fieldset legend'));
```

Встановіть властивість `font-size` у `1.3rem`.
...
Встановіть властивість `font-weight` у `600`.
