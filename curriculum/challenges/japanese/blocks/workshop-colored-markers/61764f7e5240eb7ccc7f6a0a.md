...
次に、クラス `one` を対象にする新しい CSS ルールを作成し、その `background-color` プロパティを `red` に設定してください。

# --hints--

クラス `one` を対象にするには、クラスセレクターを使う必要があります。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.one'));
```

あなたの `.one` CSS ルールには、`background-color` プロパティが `red` に設定されている必要があります。
