...
Aggiungi due elementi `span` con il `className` di `stat-value` e `stat-label` rispettivamente a ogni `stat-row`.

# --hints--

Ogni `.stat-row` nel secondo `.stat-col` dovrebbe contenere un `span` con classe `stat-value` e un `span` con classe `stat-label`.
