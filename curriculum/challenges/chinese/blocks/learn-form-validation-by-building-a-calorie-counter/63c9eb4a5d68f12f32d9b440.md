...
请记住，`document.querySelectorAll` 返回 `NodeList`，它类似于数组，但不是数组。 但是，`Array` 对象有一个 `.from()` 方法，它接受类似数组的对象并返回一个数组。 当你想要访问更强大的数组方法时这很有用，你将在未来的项目中了解这些方法。

以下示例采用 `li` 元素的 `NodeList` 并将其转换为 `li` 元素数组：
...
将你的 `inputContainers` 查询选择器包装在 `Array.from()` 中。 在与声明相同的行上执行此操作。

# --hints--

你应该将 `querySelectorAll` 包装在 `Array.from()` 中。

```js
assert.match(clearForm, /Array\.from\(\s*document\.querySelectorAll\(\s*('|")\.input-container*\1\s*\)\s*\)/);
```

你应该将 `Array.from()` 的结果分配给 `inputContainers` 变量。
