...
Якщо `messageInput` порожній, покажіть користувачу сповіщення з повідомленням `"Please enter a message."`.

Потім вийдіть із виконання функції.

# --hints--

Функція зворотнього виклику повинна мати інструкцію `if`.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

Інструкція `if` має перевірити, чи `value` елемента `messageInput` є порожнім рядком.
...
Інструкція `if` має показати користувачу сповіщення з повідомленням `"Please enter a message."`.
...
Інструкція `if` має завершити виконання функції.
