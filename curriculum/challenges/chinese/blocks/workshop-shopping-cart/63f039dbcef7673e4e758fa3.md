...
浏览器有一个内置的 `confirm()` 函数，它向用户显示确认提示符。`confirm()` 接受一个字串，该字串是显示给用户的消息。如果用户确认，它返回 `true`，如果用户取消，它返回 `false`。

声明一个变量 `isCartCleared`，并为其分配使用字符串 `"Are you sure you want to clear all items from your shopping cart?"` 调用 `confirm()` 的值。

# --hints--

你应该使用 `const` 来声明 `isCartCleared` 变量。
...
你应该将字符串 `Are you sure you want to clear all items from your shopping cart?` 传递给 `confirm()` 函数。

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(\s*('|"|`)Are you sure you want to clear all items from your shopping cart\?\1\s*\)/);
```

你应该将 `confirm()` 函数的值分配给 `isCartCleared` 变量。
