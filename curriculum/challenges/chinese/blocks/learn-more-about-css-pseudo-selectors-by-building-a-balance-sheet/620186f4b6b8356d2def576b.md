...
接下来，你需要设置 `caption` 元素的样式，使其看起来更像标题。 创建一个 `table caption` 选择器。 将文本设置为具有 `#356eaf` 的颜色、`1.3em` 的大小和正常的粗细。

# --hints--

你应该有一个 `table caption` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption'));
```

你的 `table caption` 选择器应该有一个 `color` 属性，设置为 `#356eaf`。

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('color') === 'rgb(53, 110, 175)');
```

你的 `table caption` 选择器应该有一个 `font-size` 属性，设置为 `1.3em`。

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('font-size') === '1.3em');
```

你的 `table caption` 选择器应该有一个 `font-weight` 属性，设置为 `normal`。
