...
Gradientübergänge ändern sich oft allmählich von einer Farbe zur anderen. Wenn ein abruptes Umschalten erforderlich ist, kann der Übergang mit einem harten Stopp wie diesem erfolgen:
...
Fügen Sie ein `linear-gradient` zu `.bb2b` hinzu, das `--building-color2` von `0%` bis `6%` und `--window-color2` von `6%` bis `9%` verwendet.

# --hints--

Sie sollten `.bb2b` eine `background` Eigenschaft geben.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

Sie sollten ein `linear-gradient` auf dem `background` verwenden.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

Sie sollten `--building-color2` von `0%` bis `6%` verwenden.
...
Sie sollten `--window-color2` von `6%` bis `9%` verwenden.
...
`.bb2b` sollte eine `linear-gradient` haben, die von `--building-color2` bei `0%` bis `6%` und von `--window-color2` bei `6%` bis `9%` übergeht.
