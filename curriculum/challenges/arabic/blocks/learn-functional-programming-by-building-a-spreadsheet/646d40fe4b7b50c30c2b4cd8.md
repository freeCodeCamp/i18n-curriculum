...
دالتك `infixEval` ستُقيّم فقط أول عملية ضرب أو قسمة، لأن `regex` ليست شاملة. هذا يعني أنك ستحتاج إلى استخدام نهج تكراري لتقييم السلسلة النصية كاملة.

إذا لم تجد `infixEval` أي تطابقات، فستُعيد قيمة `str` كما هي. باستخدام تعبير ثلاثي، تحقق مما إذا كانت `str2` تساوي `str`. إذا كانت كذلك، أعد `str`، وإلا أعد نتيجة استدعاء `highPrecedence()` على `str2`.

# --hints--

يجب أن تستخدم دالتك `highPrecedence` الكلمة المفتاحية `return`.
...
يجب أن تستخدم الكلمة المفتاحية `return` مع شرط للتحقق مما إذا كانت `str` تساوي `str2`.
...
يجب أن تستخدم صيغة ثلاثية مع جملة `return` الخاصة بك.
...
إذا كان شرط الثلاثي صحيحًا، يجب أن تعيد `str`.
...
إذا كان شرط الثلاثي خاطئًا، يجب أن تعيد نتيجة استدعاء `highPrecedence()`.
...
يجب أن تمرر `str2` إلى استدعاء `highPrecedence()`.
