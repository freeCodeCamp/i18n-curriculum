...
你需要另一个函数来显示 DOM 中的元素。类似于上一个函数，创建一个名为 `showElements` 的函数。这次使用 `classList.remove("hidden")`。

# --hints--

你应该有一个名为 `showElements` 的函数。
...
你的 `showElements` 函数应该有一个名为 `...elements` 的参数，类型为 `HTMLElement[]`。
