...
Welche Aussage zeigt anhand des Dialogs die korrekte Verwendung von `going to` basierend auf gegenwärtigen Beweisen?
...
Dieser Satz verwendet `will` für eine wahrscheinliche zukünftige Handlung, die jedoch nicht den Eindruck einer Entscheidung basierend auf gegenwärtigen Beweisen vermittelt.
...
Hier wird `will` für eine zukünftige Handlung verwendet, die auf einer aktuellen Situation basiert, aber `going to` wäre passender, da die Entscheidung offenbar auf der gegenwärtigen Expertise des Teams beruht.
...
Dieser Satz verwendet `will` für eine spontane Entscheidung (`calling the designer`), was angemessen ist, aber er veranschaulicht nicht die Verwendung von `going to` basierend auf gegenwärtigen Beweisen.
