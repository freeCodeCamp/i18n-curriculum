...
每个列表项开头的 `span` 中的勾选标记是装饰性的。为这些 `span` 元素添加 `aria-hidden`，以将它们从屏幕阅读器中隐藏。

另外，为每个 `span` 元素赋值类 `text-green-700` 和 `mr-2`。

# --hints--

你的两个 `span` 元素应具有 `aria-hidden` 属性。
...
你的两个 `span` 元素应具有类 `text-green-700`。
