...
Você precisará manter uma lista das unidades que seu app suporta.

Declare um array `units` e atribua a ele os valores string `"cup"`, `"gram"`, `"ounce"` e `"teaspoon"`.

# --hints--

Você deve declarar uma variável `units`.

```js
assert.isDefined(units);
```

Sua variável `units` deve ser um array.

```js
assert.isArray(units);
```

Seu array `units` deve ter quatro elementos.

```js
assert.lengthOf(units, 4);
```

Seu array `units` deve incluir a string `"cup"`.

```js
assert.include(units, "cup");
```

Seu array `units` deve incluir a string `"gram"`.

```js
assert.include(units, "gram");
```

Seu array `units` deve incluir a string `"ounce"`.

```js
assert.include(units, "ounce");
```

Seu array `units` deve incluir a string `"teaspoon"`.
