...
في الخطوة الأخيرة من ورشة عمل نموذج تقييم الفندق، تحتاج إلى إضافة زر إرسال إلى النموذج.

تذكر أنك تعلمت كيفية التعامل مع أزرار الإرسال في الدروس السابقة.

أضف عنصر `button` مع خاصية `type` مضبوطة على `"submit"` ومحتوى نصي مضبوط على `Submit`.

وبذلك، يصبح نموذج تقييم الفندق الخاص بك مكتملًا!

# --hints--

يجب أن يكون لديك عنصر `button` داخل النموذج.

```js
assert.exists(document.querySelector('form button'));
```

يجب ضبط خاصية `type` للزر على `"submit"`.

```js
assert.exists(document.querySelector('button[type="submit"]'));
```

يجب أن يحتوي الزر على النص `Submit`.
