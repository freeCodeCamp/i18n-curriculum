...
`solve` メソッドを実装する時が来ました。2次方程式の判別式が負の場合、その方程式は実数解（根）を持ちません。

`solve` メソッド内で、判別式が負かどうかを確認する`if` 文を作成してください。もし負であれば、空のリストを返してください。

# --hints--

`if` が `self.delta` より小さいかどうかを確認する`0` 文を作成する必要があります。
