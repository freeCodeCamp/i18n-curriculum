...
Fügen Sie unter Ihrem letzten `p`-Element ein weiteres `p`-Element mit dem Text `Total Carbohydrate 37g 13%` hinzu. Verwenden Sie wie zuvor `span`-Elemente, um den Text `Total Carbohydrate` und `13%` fett zu formatieren. Wickeln Sie dann das Nährstoff- und Mengenangabe erneut in ein `span` für die Ausrichtung ein.
...
Ihr neues `p`-Element sollte den Text `Total Carbohydrate 37g 13%` enthalten.
...
Ein `span`-Element sollte den Text `Total Carbohydrate 37g` umschließen.
...
Ihr zweites `span`-Element sollte den Text `13%` umschließen.
