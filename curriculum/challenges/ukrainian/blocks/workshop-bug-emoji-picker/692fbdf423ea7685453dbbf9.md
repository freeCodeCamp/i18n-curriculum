...
Тепер ви почнете посилатися на потрібні елементи DOM.

Отримайте елемент абзацу з ідентифікатором `bug-emoji` і збережіть його в константі з ім’ям `bugEmojiElement`. Використайте оператор твердження non-null `!`, щоб підтвердити, що елемент не є `null`.

# --hints--

У вас має бути константа з ім’ям `bugEmojiElement`.
...
Ваш `bugEmojiElement` має отримувати елемент з ідентифікатором `bug-emoji` за допомогою `document.querySelector` з типом аргументу `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

Ваш `bugEmojiElement` має використовувати оператор твердження non-null (`!`).
