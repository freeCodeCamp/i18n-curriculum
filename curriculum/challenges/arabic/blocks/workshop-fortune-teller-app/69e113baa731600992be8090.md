...
تم توفير صورة افتراضية للبطاقة ورابط URL لإجراء طلب API، كل ما تحتاجه هو بعض المتغيرات لحفظها.

أنشئ متغيرًا باسم `CDN_URL` واضبطه على `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. وأنشئ أيضًا متغيرًا باسم `defaultImg` بقيمة `new URL("default.svg", CDN_URL + "/")`، ومتغيرًا باسم `LOCAL_DEFAULT_IMG` بقيمة `defaultImg`.

# --hints--

يجب أن تنشئ متغيرًا باسم `CDN_URL`.
...
يجب أن يكون متغير `CDN_URL` مضبوطًا على `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"`.
...
يجب أن تنشئ متغيرًا باسم `defaultImg`.
...
يجب أن يكون متغير `defaultImg` مضبوطًا على `new URL("default.svg", CDN_URL + "/")`.
...
يجب أن تنشئ متغيرًا باسم `LOCAL_DEFAULT_IMG`.
...
يجب أن يكون متغير `LOCAL_DEFAULT_IMG` مضبوطًا على `defaultImg`.
