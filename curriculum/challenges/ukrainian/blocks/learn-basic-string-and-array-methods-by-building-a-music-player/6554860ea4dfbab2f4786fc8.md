...
Якщо ви уважно перевірите, ви побачите, що пісня, яка зараз відтворюється, не виділена в списку відтворення, тому ви насправді не знаєте, яка пісня відтворюється. Це можна виправити, створивши функцію для виділення будь-якої пісні, яка відтворюється.

Використовуючи стрілковий синтаксис, створіть функцію `highlightCurrentSong`. В межах функції, використайте `querySelectorAll`, щоб отримати елемент `.playlist-song` та призначте до константи `playlistSongElements`.

# --hints--

Використайте `const`, щоб створити функцію `highlightCurrentSong`.
...
Функція `highlightCurrentSong` має використати стрілковий синтаксис.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

Створіть константу `playlistSongElements` у функції `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

Використайте метод `querySelectorAll()`, щоб вибрати елемент `.playlist-song` та призначте його до константи `playlistSongElements`.
