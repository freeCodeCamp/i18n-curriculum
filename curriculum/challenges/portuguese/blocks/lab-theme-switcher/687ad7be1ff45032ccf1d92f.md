...
7. Você deve ter um array `themes` com pelo menos dois objetos que contenham cada um uma propriedade `name` e `message`. O `name` representará um tema diferente e a `message` será exibida quando o tema for alterado.  You are free to come up with `name` and `message` values of your choice but the `name` values should match the text content of your `li` elements.
