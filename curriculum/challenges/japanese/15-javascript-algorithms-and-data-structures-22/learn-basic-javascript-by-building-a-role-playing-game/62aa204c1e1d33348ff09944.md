...
attack 関数の中で変数 `monsterHealth` を更新しているコード行を見つけ、それを `if` ブロックの中に入れてください。if 文の条件では `isMonsterHit` 関数を呼び出してください。
...
`if` 文の条件内で `isMonsterHit` 関数を呼び出してください。

```js
assert.match(attack.toString(), /if\s*\(\s*isMonsterHit\s*\(\s*\)/)
```

`monsterHealth -= weapons[currentWeaponIndex].power + Math.floor(Math.random() * xp) + 1` の部分を `if` ブロックに入れてください。
