...
Target the `.foot` element with a `class` of `left`, and position it `25%` left of its parent. Luego, apunte al elemento `.foot` con una `class` de `right`, y colóquelo `25%` a la derecha de su padre.
