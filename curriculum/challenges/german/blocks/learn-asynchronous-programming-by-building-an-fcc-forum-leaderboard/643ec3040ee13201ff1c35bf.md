...
Lassen Sie das zweite `td`-Element leer, da Sie später Inhalt hinzufügen werden.

Fügen Sie im dritten `td`-Element den folgenden eingebetteten Ausdruck hinzu: `${posts_count - 1}`.

Dieser zeigt die Anzahl der Antworten auf das Thema an.

# --hints--

Sie sollten `${posts_count - 1}` im dritten `td`-Element haben.
