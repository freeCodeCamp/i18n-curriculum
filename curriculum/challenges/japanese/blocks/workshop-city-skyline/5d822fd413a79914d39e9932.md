...
次の建物に進みましょう！手前の緑色の建物です。`repeating-linear-gradient` から `0%` までのあなたの建物の色で `5%` に色を付け、`transparent` から `5%` まで `10%` に色を付けてください。

# --hints--

`.fb5` に `background` プロパティを付ける必要があります。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

`background` に `repeating-linear-gradient` を付ける必要があります。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

`repeating-linear-gradient` に `--building-color2` の最初の色を `0%` から `5%` まで付ける必要があります。
...
`repeating-linear-gradient` に `transparent` の2番目の色を `5%` から `10%` まで付ける必要があります。
