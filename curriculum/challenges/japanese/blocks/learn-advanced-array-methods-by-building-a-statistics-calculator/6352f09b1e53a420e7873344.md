...
`getMode()` 関数を `calculate` ロジックに追加し、該当する HTML 要素を更新してください。

# --hints--

`calculate` 関数には、値が `mode` の `getMode(numbers)` 変数が必要です。

```js
assert.match(calculate.toString(), /mode\s*=\s*getMode\(\s*numbers\s*\)\s*;?/);
```

`calculate` 関数は DOM から `#mode` 要素をクエリし、`textContent` を `mode` に設定してください。
