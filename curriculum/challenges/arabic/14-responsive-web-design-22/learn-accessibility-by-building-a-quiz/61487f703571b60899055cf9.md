...
عيّن عرض عناصر `section` إلى `80%` من الحاوية الأساسي (parent container). ثم استخدم margins لتركيز عناصر `section`، بإضافة `10px` إلى bottom margin.

أيضا، تأكد من أن عناصر `section` لا يمكن أن تكون أكبر من width بقيمة `600px`.

# --hints--

يجب عليك استخدام منتقي `section`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('section'));
```

يجب عليك إعطاء `section` خاصية `width` بقيمة `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.width, '80%');
```

يجب عليك إعطاء `section` خاصية `margin-top` بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginTop, '0px');
```

يجب عليك إعطاء `section` خاصية `margin-right` بقيمة `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginRight, 'auto');
```

يجب عليك إعطاء `section` خاصية `margin-bottom` بقيمة `10px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginBottom, '10px');
```

يجب عليك إعطاء `section` خاصية `margin-left` بقيمة `auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('section')?.marginLeft, 'auto');
```

يجب عليك إعطاء `section` خاصية `max-width` بقيمة `600px`.
