...
La insignia de nivel todavía muestra el texto codificado `GOLD`. Necesitas reemplazarlo con una expresión dinámica para que se muestre el nivel correcto.

Llama a `getPlayerTier()` con `player.overallRating` para obtener la cadena del nivel, luego llama a `.toUpperCase()` sobre el resultado para mostrarlo en mayúsculas. No olvides envolver toda la expresión entre llaves.

# --hints--

Debes usar `getPlayerTier(player.overallRating).toUpperCase()` dentro de `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

El elemento `.card-tier-badge` debería mostrar `ELITE` para un jugador con una puntuación de 98.
