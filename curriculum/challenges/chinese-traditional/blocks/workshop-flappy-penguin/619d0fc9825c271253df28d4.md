...
選擇 `.blush` 元素，並將其 `width` 設置爲 `15%`、`height` 設置爲 `10%`，`background-color` 設置爲 `pink`。
...
你應該將 `.blush` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.width, '15%');
```

你應該將 `.blush` 的 `height` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.height, '10%');
```

你應該將 `.blush` 的 `background-color` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
