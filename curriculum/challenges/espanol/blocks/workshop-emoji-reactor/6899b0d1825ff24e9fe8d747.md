...
Debajo del elemento `h1`, agrega un elemento `p` con la clase `description`.

Dentro del elemento `p` escribe el texto `Click on the buttons below to rate your emotions.`

# --hints--

Debes crear un nuevo elemento `p` debajo del elemento `h1`.

```js
assert.exists(document.querySelector("h1 + p"));
```

El nuevo elemento `p` debe tener una clase `description`.

```js
assert.equal(document.querySelector("h1 + p")?.className, "description");
```

El nuevo elemento `p` debe tener el texto `Click on the buttons below to rate your emotions.`.
