...
`except` 절을 하나 더 만들어 `MediaError as e`을 잡으세요. `except` 블록 안에서 f-string을 사용해 `Media Error:`를 출력한 다음 공백과 오류 메시지를 출력하세요.

# --hints--

`except` 절이 있어 `MediaError as e`을 잡아야 합니다.
...
새로운 `Media Error:` 블록 안에서 f-string을 사용해 `except`를 출력한 다음 공백과 오류 메시지를 출력하세요.
