...
`location` 매개변수의 `"button text"` 속성에서 두 번째 요소에 접근해야 합니다.

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*1\s*\]/);
```

`button2.innerText` 속성을 `location` 매개변수의 `"button text"` 속성에서 두 번째 요소로 설정해야 합니다.

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*location\s*\[\s*('|")button text\1\s*\]\s*\[\s*1\s*\]/);
```

`location` 매개변수의 `"button text"` 속성에서 세 번째 요소에 접근해야 합니다.

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*2\s*\]/);
```

`button3.innerText` 속성을 `location` 매개변수의 `"button text"` 속성에서 세 번째 요소로 설정해야 합니다.
