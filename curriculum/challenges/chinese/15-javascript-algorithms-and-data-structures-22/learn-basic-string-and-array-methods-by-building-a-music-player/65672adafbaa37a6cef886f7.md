...
现在你已经创建了按钮，你需要为其分配一个文本。 为此，你需要使用 DOM 的 `createTextNode()` 方法。

`createTextNode()` 方法用于创建文本节点。 要使用它，你可以调用它并将文本作为字符串传入：
...
使用 `createTextNode()` 方法创建 `"Reset Playlist"` 文本，然后将其分配给 `resetText` 常量。
...
你应该使用 `document.createTextNode()` 来创建 `"Reset Playlist"` 文本。

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

你应该将创建的文本节点分配给 `resetText` 常量。
