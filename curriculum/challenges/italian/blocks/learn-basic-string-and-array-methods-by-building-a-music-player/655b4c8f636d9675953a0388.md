...
Se non c’è una canzone successiva nella playlist, usa il blocco `else` per resettare la chiave `currentSong` di `userData` a `null`, e la sua proprietà `songCurrentTime` a `0`.
...
Dovresti usare la notazione a punto per impostare la proprietà `currentSong` di `userData` a `null` nel blocco `else` della tua istruzione `if`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

Dovresti usare la notazione a punto per impostare la proprietà `songCurrentTime` di `userData` a `0` nel blocco `else` della tua istruzione `if`.
