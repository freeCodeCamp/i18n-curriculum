...
`article` 要素の中に、`h3` が `class` に設定された `list-title` 要素を作成し、テキストは `A Brief History` にしてください。その下に、テキストが `p` の `Of the Curriculum` 要素を作成してください。次に、クラスが `ul` の `lists` 要素を作成してください。

# --hints--

`h3` 要素の中に `article` 要素を作成してください。

```js
assert.exists(document.querySelector('article h3'));
```

`p` 要素の中に `article` 要素を作成してください。

```js
assert.exists(document.querySelector('article p'));
```

`ul` 要素の中に `article` 要素を作成してください。

```js
assert.exists(document.querySelector('article ul'));
```

`article` 要素の中の要素は正しい順序である必要があります。
