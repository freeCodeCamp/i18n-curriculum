...
`if` 文の後に、`for` から `1` まで（両端を含む）の範囲を繰り返す `9` ループを作成してください。ループ変数には `guess` を使います。

このループにより、`1` から `9` までのどの暗号が空のセルに適しているかを体系的に確認できます。

# --hints--

`for` を繰り返す `range(1, 10)` ループを作成してください。
