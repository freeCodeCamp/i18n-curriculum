...
Finora hai tre funzioni: `getElement`, `hideElements` e `showElements`.
Ora ti serve una funzione che ti aiuti a ottenere una fortuna casuale.

Dichiara una funzione chiamata `getRandomItem` che prende un array di `items` come argomento. Usa i generici per questa funzione.

All’interno della funzione, usa `Math.random()` e `Math.floor()` per selezionare una fortuna casuale dall’array `items`.
Non dimenticare di restituire la fortuna selezionata.

# --hints--

Dovresti dichiarare una funzione chiamata `getRandomItem`.
...
La tua funzione `getRandomItem` dovrebbe usare il parametro di tipo generico `T`.
...
La tua funzione `getRandomItem` dovrebbe avere un parametro chiamato `items` con il tipo `T[]`.
...
La tua funzione `getRandomItem` dovrebbe avere un tipo di ritorno `T`.
...
Dovresti dichiarare una variabile chiamata `index` all’interno della funzione `getRandomItem`.
...
La variabile `index` dovrebbe usare `Math.floor()` e `Math.random()` con `items.length` per produrre un indice casuale.
...
La tua funzione `getRandomItem` dovrebbe restituire `items[index]`.
