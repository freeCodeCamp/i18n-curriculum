...
現在是測試 `playNextSong` 函式（程式）的時候了。 

為你的 `nextButton` 元素添加一個 `"click"` 事件監聽器，然後將 `playNextSong` 作為你的事件監聽器的第二個引數傳入。這是事件監聽器將會執行的函式。

先點擊播放按鈕，接著點擊下一步按鈕來測試你的應用程式。你應該會看到一切運作如預期。

# --hints--

你應該在你的 `nextButton` 變數上呼叫 `addEventListener()` 方法。
...
你應該將 `playNextSong` 作為你的 `addEventListener` 方法的第二個引數傳入。
