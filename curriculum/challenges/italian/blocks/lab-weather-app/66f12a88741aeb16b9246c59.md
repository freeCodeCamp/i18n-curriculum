...
In questo laboratorio costruirai un'app che fornisce informazioni meteo di diverse città.

Userai un'API meteo. I dati di output hanno il seguente formato:
...
1. Dovresti avere un elemento `button` con un `id` di `get-weather-btn`.

1. Dovresti avere un elemento `select` con sette elementi `option` annidati al suo interno. La prima opzione dovrebbe avere una stringa vuota come testo e l'attributo `value`. Le restanti dovrebbero avere per testo e valore (con il valore in minuscolo) i seguenti:
...
1. Se non è selezionata nessuna città, premere il pulsante non dovrebbe fare nulla.

1. Se è selezionata una città, dovrebbero apparire elementi per mostrare il meteo:

- Dovresti avere un elemento `img` con id `weather-icon` per mostrare l'icona del meteo.

- Dovresti avere un elemento con id `main-temperature` per mostrare la temperatura principale.

- Dovresti avere un elemento con id `feels-like` per mostrare la temperatura percepita.

- Dovresti avere un elemento con id `humidity` per mostrare l'umidità dell'aria.

- Dovresti avere un elemento con id `wind` per mostrare la velocità del vento.

- Dovresti avere un elemento con id `wind-gust` per mostrare le raffiche di vento.

- Dovresti avere un elemento con id `weather-main` per mostrare il tipo principale di meteo.

- Dovresti avere un elemento con id `location` per mostrare la posizione attuale.

1. Dovresti avere una funzione asincrona chiamata `getWeather` che recupera le informazioni meteo dall'API `https://weather-proxy.freecodecamp.rocks/api/city/<CITY>` e le restituisce. Nota che questa API restituisce dati usando il sistema metrico, cioè m/s per la velocità del vento e gradi Celsius per la temperatura.

1. La funzione asincrona `getWeather` dovrebbe accettare una città come argomento.

1. Dovresti gestire eventuali errori che si verificano all'interno della funzione `getWeather` e registrarli nella console.

1. Dovresti avere una funzione asincrona `showWeather` che accetta una città come parametro.

1. La funzione `showWeather` dovrebbe chiamare la funzione `getWeather` per recuperare i dati meteo per la città selezionata dal menu a tendina.

1. Se la funzione `getWeather` ha avuto un errore, l'app dovrebbe mostrare solo un alert con il messaggio `Something went wrong, please try again later`.

1. Se i dati da `getWeather` sono utilizzabili, la funzione `showWeather` dovrebbe mostrare i dati meteo negli elementi corrispondenti. Se un certo valore dall'API è `undefined`, dovresti scrivere `N/A` nell'elemento corrispondente.

1. Né la funzione `getWeather` né la funzione `showWeather` dovrebbero essere dichiarate usando `const`, poiché i test devono poterle riassegnare.

**Nota:** Né la funzione `getWeather` né la funzione `showWeather` dovrebbero essere dichiarate usando `const`, poiché i test devono poterle riassegnare. Inoltre, i test richiederanno tempo per completarsi, quindi finché vedi `// running tests` nella console, stanno ancora venendo eseguiti.
...
Dovresti avere un elemento `button` con un `id` di `get-weather-btn`.

```js
assert.exists(document.querySelector('button#get-weather-btn'));
```

Dovresti avere un elemento `select`.

```js
assert.exists(document.querySelector('select'));
```

All'interno dell'elemento `select` il primo figlio dovrebbe essere un elemento `option` con un attributo `value` vuoto.
...
All'interno dell'elemento `select` dovrebbero esserci 6 elementi `option`, uno per ciascuna delle seguenti città: Parigi, Londra, Tokyo, Los Angeles, Chicago, New York.
...
Dovresti avere una funzione `showWeather`.

```js
assert.isFunction(showWeather);
```

Dovresti avere una funzione `getWeather`.

```js
assert.isFunction(getWeather);
```

La funzione `getWeather` dovrebbe accettare una città come unico argomento e restituire il JSON dall'API meteo.
...
Né la funzione `getWeather` né la funzione `showWeather` dovrebbero essere dichiarate usando `const`.
...
La funzione `showWeather` dovrebbe chiamare la funzione `getWeather` per ottenere i dati meteo.
...
La funzione `showWeather` dovrebbe gestire il caso in cui `getWeather` restituisce `undefined`.
...
Quando è selezionata New York la funzione `showWeather` dovrebbe mostrare i dati dall'API negli elementi HTML rispettivi. Se il valore dall'API è `undefined`, dovresti mostrare `N/A`.
...
Quando è selezionata Chicago la funzione `showWeather` dovrebbe mostrare i dati dall'API negli elementi HTML rispettivi. Se il valore dall'API è `undefined`, dovresti mostrare `N/A`.
...
Quando è selezionata Londra la funzione `showWeather` dovrebbe mostrare i dati dall'API negli elementi HTML rispettivi. Se il valore dall'API è `undefined`, dovresti mostrare `N/A`.
...
Quando è selezionata Tokyo la funzione `showWeather` dovrebbe mostrare i dati dall'API negli elementi HTML rispettivi. Se il valore dall'API è `undefined`, dovresti mostrare `N/A`.
...
Quando è selezionata Los Angeles la funzione `showWeather` dovrebbe mostrare i dati dall'API negli elementi HTML rispettivi. Se il valore dall'API è `undefined`, dovresti mostrare `N/A`.
...
Se c'è un errore, la tua funzione `getWeather` dovrebbe registrare l'errore nella console.
...
Quando è selezionata Parigi l'app dovrebbe mostrare un alert con `Something went wrong, please try again later`.
