...
Два останні семантичні елементи HTML цього проєкту — `footer` та `address`. Елемент `footer` є контейнером для збору вмісту, пов’язаного зі сторінкою, а елемент `address` є контейнером для контактної інформації автора сторінки.
