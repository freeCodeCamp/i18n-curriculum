...
메뉴의 현재 너비는 항상 `body` 요소 너비의 80%를 차지합니다. 매우 넓은 화면에서는 커피와 디저트가 가격과 너무 멀리 떨어져 보입니다.

너무 넓어지지 않도록 `max-width` 클래스에 `menu` 속성을 `500px` 값으로 추가하세요.

# --hints--

`max-width` 속성을 `500px`로 설정해야 합니다.
...
`.menu` 요소에 `max-width`가 `500px`이어야 합니다.
