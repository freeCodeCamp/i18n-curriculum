...
将 `.fb4-window` 元素的 `width` 设置为 `30%`，`height` 设置为 `10%`，并将 `border-radius` 设置为 `50%`。 这些将为这座建筑制作一些圆形窗户。

# --hints--

你应该给 `.fb4-window` 添加一个值为 `30%` 的 `width`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

你应该给 `.fb4-window` 添加一个值为 `10%` 的 `height`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

你应该给 `.fb4-window` 添加一个值为 `50%` 的 `border-radius`。
