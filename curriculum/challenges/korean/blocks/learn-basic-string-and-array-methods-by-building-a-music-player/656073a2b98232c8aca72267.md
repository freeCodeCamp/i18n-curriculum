...
현재 노래 정보를 관리하고 재생 시간을 추적하려면 `currentSong`와 `songCurrentTime` 프로퍼티를 만드세요. 각각의 값을 `null`와 `0`으로 설정하세요.

# --hints--

기존 `userData` 객체와 그 내용을 수정하지 마세요.

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

`userData` 객체에 `currentSong` 키가 `null`으로 설정되어 있어야 합니다.

```js
assert.propertyVal(userData, "currentSong", null)
```

`userData` 객체에 `songCurrentTime` 키가 `0`으로 설정되어 있어야 합니다.
