...
將右眼移動到位置，使用 `position` 屬性為 `absolute`，`top` 為 54px，`left` 為 134px。

# --hints--

你的 `.cat-right-eye` 選擇器應該有一個 `position` 屬性設定為 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.position === 'absolute')
```

你的 `.cat-right-eye` 選擇器應該有一個設定為 `54px` 的 `top` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.top === '54px')
```

你的 `.cat-right-eye` 選擇器應該有一個設定為 `134px` 的 `left` 屬性。
