...
在带有 `Total Fat` 文本的元素下方，使用文本 `Saturated Fat 1g 5%` 创建一个新的 `p` 元素。 将 `5%` 包装在 `span` 中，并将 `class` 属性设置为 `bold`。 在这种情况下，将百分比与 `5%` 对齐即可。

# --hints--

你应该使用 `Total Fat` 文本在元素下方创建一个新的 `p` 元素。
...
你的新的 `p` 元素应包含文本 `Saturated Fat 1g 5%`。
