...
문자 클래스는 두 글자 이상을 포함할 수 있습니다. `a` 문자를 `a`, `@`, 그리고 `4`에 일치하는 문자 클래스로 바꾸세요.

# --hints--

`stockRegex`는 `a`, `@`, 그리고 `4`에 일치하도록 문자 클래스를 사용해야 합니다.

```js
assert.match(stockRegex.source, /\[(a@4|a4@|4@a|4a@|@a4|@4a)]/);
```

`stockRegex`는 `stock alert`에 일치해야 합니다.

```js
assert.match('stock alert', stockRegex);
```

`stockRegex`는 `stock @lert`에 일치해야 합니다.

```js
assert.match('stock @lert', stockRegex);
```

`stockRegex`는 `stock 4lert`에 일치해야 합니다.

```js
assert.match('stock 4lert', stockRegex);
```

`stockRegex`는 `570ck 4l3r7`에 일치해야 합니다.
