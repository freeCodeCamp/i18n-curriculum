...
1. 你应该有一个 `head` 元素，其中包含一个 `meta` 空元素，其 `charset` 设置为 `utf-8`，`title` 的文本为 `Travel Agency Page`。
1. 在你的 `head` 元素中，你应该有一个 `meta` 标签，包含你网站用于 SEO 的简短描述。
1. 你应该有一个 `h1` 元素来显示你的旅行目的地。
1. 在 `h1` 元素下面，你应该有一个介绍旅行机会的段落。
1. 你应该有一个 `h2` 元素，文本为 `Packages` 。
1. 你应该有一个`p`元素，简要介绍各种旅游方式。
1. 你应该有一个包含两个列表项的无序列表元素。 这两个列表项应该分别具有文本 `Group Travels` 和 `Private Tours`。 每个列表项的文本都应该被锚元素包围。
1. 你应该有一个 `h2` 元素，文本为 `Top Itineraries` 。
1. 你应该至少有三个 `figure` 元素，每个元素包含一个锚元素和一个 `figcaption` 元素。
1. 三个锚元素应该分别包含一个带有适当描述的 `alt` 属性和设置为有效的图像的 `src` 属性的 `img` 元素。 如果愿意，可以使用 `https://cdn.freecodecamp.org/curriculum/labs/colosseo.jpg`、`https://cdn.freecodecamp.org/curriculum/labs/alps.jpg` 和 `https://cdn.freecodecamp.org/curriculum/labs/sea.jpg`。
1. 所有五个锚元素都应该有一个值为 `https://www.freecodecamp.org/learn` 的 `href` 属性和一个值为 `_blank` 的 `target` 属性。
...
你的 `head` 元素中应该有一两个个 `meta` 元素。

```js
assert.match(code, /<\s*head\s*>[\s\S]*(<\s*meta[\s\S]*>[\s\S]*){2,}<\/\s*head\s*>/i);
```

一个 `meta` 元素应该有一个值为 `description` 的 `name` 属性和一个非空的 `content` 属性。
...
一个 `meta` 元素应该将它的 `charset` 属性设置为 `UTF-8`。
...
Each `a` element should have an `href` attribute with the value of `https://www.freecodecamp.org/learn`. 不要忘记列表项中的链接。
...
每个 `a` 元素应该有一个 `target` 属性，其值为 `_blank`。 不要忘记列表项中的链接。
