...
클래스 선택자를 사용해 `.cat-nose` 요소에 좌우 테두리를 각각 `15px solid transparent`로 지정하세요. 또한 아래쪽 테두리는 `20px solid #442c2c`로 지정하세요.

# --hints--

`.cat-nose` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose'))
```

`.cat-nose` 선택자에 `border-left` 속성을 `15px solid transparent`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderLeft === '15px solid transparent')
```

`.cat-nose` 선택자에 `border-right` 속성을 `15px solid transparent`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderRight === '15px solid transparent')
```

`.cat-nose` 선택자에 `border-bottom` 속성을 `20px solid #442c2c`로 설정해야 합니다.
