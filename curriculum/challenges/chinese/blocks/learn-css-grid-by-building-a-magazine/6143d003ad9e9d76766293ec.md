...
在 `.image-quote` 元素中，嵌套一个 `hr` 元素、一个 `p` 元素和第二个 `hr` 元素。 给 `p` 元素一个值为 `quote` 的 `class` 和文本 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`。

# --hints--

你应该在 `.image-quote` 元素中添加两个 `hr` 元素。

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

你应该在 `.image-quote` 元素中添加一个 `p` 元素。

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

你的 `.image-quote` 子元素应按正确的顺序排列。
...
你的新 `p` 元素应将 `class` 设置为 `quote`。

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

你的新 `p` 元素应包含文本 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`。
