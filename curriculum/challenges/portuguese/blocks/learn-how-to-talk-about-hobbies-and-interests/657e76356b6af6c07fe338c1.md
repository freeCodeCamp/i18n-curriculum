...
No Presente Perfeito, `for` e `since` são usados para discutir duração de tempo e pontos de início.  
`For` indica há quanto tempo algo está acontecendo. `Since` indica quando algo começou.
...
Esta preposição expressa a duração de uma ação ou situação neste contexto.
