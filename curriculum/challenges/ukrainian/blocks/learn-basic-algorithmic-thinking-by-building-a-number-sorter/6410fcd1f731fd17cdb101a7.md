...
Всередині вашого вкладеного `for` циклу додайте виклик `console.log()`, щоб перевірити значення `array`, `array[j]` і `array[minIndex]` на кожній ітерації. Ви можете натиснути на кнопку `Sort`, щоб побачити, як ваш алгоритм проходить масив.

Потім напишіть інструкцію `if`, яка перевіряє, чи значення в `array[j]` менше за значення в `array[minIndex]`. Якщо так, встановіть `minIndex` у `j`.

# --hints--

У вашому вкладеному `console.log()` циклі має бути виклик `for`.
...
Ви маєте передати `array` як перший аргумент у `console.log()`.
...
Ви маєте передати `array[j]` як другий аргумент у `console.log()`.
...
Ви маєте передати `array[minIndex]` як третій аргумент у `console.log()`.
...
Після виклику `if` має бути інструкція `console.log()`.
...
У вашій інструкції `if` має бути умова, яка перевіряє, чи `array[j]` менше за `array[minIndex]`.
...
Ваша інструкція `if` має встановлювати `minIndex` у `j`.
