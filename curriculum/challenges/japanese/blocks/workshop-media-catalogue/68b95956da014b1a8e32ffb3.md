...
前のモジュールで、組み込みクラスの`Exception`を継承して独自のカスタム例外を作成できることを学びました。
...
`MediaError`という名前のクラスを作成してください。`Exception`クラスを継承させ、`Custom exception for media-related errors.`というドキュメンテーション文字列を追加してください。

# --hints--

`MediaError`という名前のクラスを作成してください。
...
`MediaError`クラスは`Exception`を継承している必要があります。
...
`Custom exception for media-related errors.`クラスに`MediaError`というドキュメンテーション文字列を追加してください。
