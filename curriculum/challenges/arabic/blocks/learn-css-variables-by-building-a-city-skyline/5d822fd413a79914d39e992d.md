...
امنح عناصر `.fb4-window` `width` بقيمة `30%`، و`height` بقيمة `10%`، و`border-radius` بقيمة `50%`. ستُنتج هذه بعض النوافذ الدائرية لهذا المبنى.

# --hints--

يجب أن تعطي `.fb4-window` `width` بقيمة `30%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

يجب أن تعطي `.fb4-window` `height` بقيمة `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

يجب أن تعطي `.fb4-window` `border-radius` بقيمة `50%`.
