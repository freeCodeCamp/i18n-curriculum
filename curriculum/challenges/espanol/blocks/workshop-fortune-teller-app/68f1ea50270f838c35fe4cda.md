...
Ahora haz lo mismo con `subTitle` asignándolo a `meaning_up`, y con `title` asígnalo a `foundCard.name`. Después, llama al método `showElements` y pasa `fortuneDescription` de los elementos.

# --hints--

Debes asignar `this.elements.subTitle.textContent` a `foundCard.meaning_up`.
...
Debes asignar `this.elements.title.textContent` a `foundCard.name`.
...
Debes llamar a la función `showElements` dentro de `showFortune`.
...
Debes pasar `this.elements.fortuneDescription` como argumento a `showElements`.
