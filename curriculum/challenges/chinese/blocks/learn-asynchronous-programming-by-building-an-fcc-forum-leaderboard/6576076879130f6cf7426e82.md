...
每个论坛主题都包括一个类别，比如 Python 或 JavaScript。 在接下来的几个步骤中，你将构建一个类别对象，该对象包含所有论坛类别和用于样式的 `classNames`。 

首先创建一个名为 `allCategories` 的新常量，并将其赋值为空对象。 

# --hints--

你应该有一个名为 `allCategories` 的常量。

```js
assert.match(code, /const\s+allCategories\s*=/)
```

你应该给 `allCategories` 变量赋一个空对象。
