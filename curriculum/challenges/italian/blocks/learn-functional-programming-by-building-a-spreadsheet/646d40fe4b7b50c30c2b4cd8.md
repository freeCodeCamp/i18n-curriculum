...
La tua funzione `infixEval` valuterà solo la prima operazione di moltiplicazione o divisione, perché `regex` non è globale. Questo significa che dovrai usare un approccio ricorsivo per valutare l'intera stringa.

Se `infixEval` non trova corrispondenze, restituirà il valore `str` così com'è. Usando un'espressione ternaria, verifica se `str2` è uguale a `str`. Se lo è, restituisci `str`, altrimenti restituisci il risultato della chiamata a `highPrecedence()` su `str2`.

# --hints--

La tua funzione `highPrecedence` dovrebbe usare la parola chiave `return`.
...
Dovresti usare la parola chiave `return` con una condizione per verificare se `str` è uguale a `str2`.
...
Dovresti usare la sintassi ternaria con la tua istruzione `return`.
...
Se la condizione ternaria è falsa, dovresti restituire il risultato della chiamata a `highPrecedence()`.
...
Dovresti passare `str2` alla tua chiamata `highPrecedence()`.
