...
<dfn>find()</dfn> 方法检索数组中满足提供的回调函数中指定的条件的第一个元素。 如果没有元素满足条件，该方法返回 `undefined`。

在下面的例子中，`find()` 方法用于查找第一个大于 25 的数字：
...
使用 `const` 创建一个名为 `song` 的变量，并将其分配给 `userData?.songs` 数组上的 `find()` 方法调用的结果。 使用 `song` 作为 `find()` 回调的参数，并检查 `song.id` 是否严格等于 `id`。

这将遍历 `userData?.songs` 数组，搜索与传递到 `playSong` 函数的 `id` 相对应的歌曲。

# --hints--

你应该在 `userData?.songs` 上使用 `find` 方法。

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

你的 `find` 方法应该将 `song` 作为其箭头函数回调的参数。

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

你的 `find` 方法应使用严格相等性来检查 `song.id` 是否等于 `id`。

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

你的 `find` 方法应该被分配给 `song` 常量。
