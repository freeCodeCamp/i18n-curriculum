...
當 `create_character` 被呼叫且第一個引數不是空字串時，不應該傳回 `The character should have a name`。
...
當 `create_character` 被呼叫且第一個引數不包含空格時，不應該傳回 `The character name should not contain spaces`。
