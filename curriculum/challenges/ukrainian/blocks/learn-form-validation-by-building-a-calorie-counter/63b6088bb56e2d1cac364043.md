...
Ваш третій `fieldset` елемент повинен містити `legend` елемент.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]?.querySelector('legend'));
```

Ваш новий `legend` елемент повинен містити текст `Dinner`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.querySelector('legend')?.innerText, "Dinner");
```

Ваш третій `fieldset` елемент повинен містити `div` елемент.

```js
assert.exists(document.querySelectorAll('form fieldset')[2]?.querySelector('div'));
```

Ваш новий `div` елемент повинен мати множину `class`, встановлену в `input-container`.
