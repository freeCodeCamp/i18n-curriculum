...
The negative form of the verb `to be` in the `Simple Past` tense is `was not` or `wasn't` for singular subjects, and `were not` or `weren't` for plural subjects. It is used to describe something that did not happen or was not true. Por ejemplo:
