...
Agora você vai começar o processo para trocar a entrada de posição por um select.

Altere a entrada para um elemento `select` com o id `position`, um `className` de `input` e um `value` de `player.position`.

# --hints--

Você deve mudar o `input` de posição para um elemento `select` com um `id` de `position` e `className` de `input`.
...
O seu `select` de posição deve ter um `value` de `player.position`.
