...
Fügen Sie in Ihrem vierten `tr`-Element ein `th`-Element mit dem Text `Total Liabilities` hinzu. Wickeln Sie den Text `Liabilities` in ein `span`-Element mit dem `class`-Attribut, das auf `sr-only` gesetzt ist.

Fügen Sie anschließend drei `td`-Elemente mit dem folgenden Text (in dieser Reihenfolge) hinzu: `$750`, `$600`, `$475`. Geben Sie dem dritten `td`-Element ein `class`-Attribut, das auf `current` gesetzt ist.
...
Ihr `th`-Element sollte den Text `Total Liabilities` haben.
...
Sie sollten den Text `Liabilities` in ein `span`-Element einwickeln.
