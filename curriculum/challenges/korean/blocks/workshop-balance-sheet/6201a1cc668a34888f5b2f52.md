...
`tr`가 `class`로 설정된 `data` 요소를 선택하세요. 이 요소들에 `linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)` 배경 이미지를 지정하세요.

# --hints--

`tr.data` 선택자가 있어야 합니다.
...
`tr.data` 선택자에 `background-image` 속성을 `linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)`로 설정하세요.
