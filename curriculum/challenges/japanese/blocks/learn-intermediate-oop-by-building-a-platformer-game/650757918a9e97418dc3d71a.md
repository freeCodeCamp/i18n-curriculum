...
最後に追加する必要があるのは `else if` 文です。

条件は、プレイヤーの `x` 位置がチェックポイントの `x` 位置以上であり、かつチェックポイントの `x` 位置に `40` を加えた値以下であるかどうかを確認してください。

`else if` 文の本体内で、`showCheckpointScreen` 関数を呼び出し、文字列 `"You reached a checkpoint!"` を引数として渡す必要があります。

おめでとうございます！プラットフォーマーゲームのプロジェクトを完成させました！

# --hints--

プレイヤーの `else if` 位置がチェックポイントの `x` 位置以上であり、かつチェックポイントの `x` 位置に `x` を加えた値以下であるかどうかを確認するために、`40` 節を追加してください。
...
`showCheckpointScreen` 関数を呼び出し、"You reached a checkpoint!" を引数として渡してください。
