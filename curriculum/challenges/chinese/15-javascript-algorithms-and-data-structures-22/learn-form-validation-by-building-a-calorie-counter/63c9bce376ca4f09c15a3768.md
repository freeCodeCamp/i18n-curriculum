...
请记住，`return` 结束函数的执行。 在 `if` 块之后，你需要处理输入有效时的逻辑。 因为你的 `if` 语句返回一个值，所以你不需要 `else` 语句。

使用加法赋值运算符将 `currVal` 添加到你的 `calories` 总数中。 你需要使用 `Number` 构造函数将 `currVal` 转换为数字。

`Number` 构造函数是一个将值转换为数字的函数。 如果该值无法转换，则返回 `NaN`，代表“非数字”。
