...
在新行上，使用加法賦值運算符 (`+=`)，將字符串 `" You attack it with your <weapon>."` 添加到 `text` 值，並用玩家當前的武器替換 `<weapon>`。 此外，請記住此行文本以空格開頭，以便能夠正確顯示。

# --hints--

你應該使用點符號在新行上訪問 `text` 的 `innerText` 屬性。
...
你應該使用帶有 `text.innerText` 的複合賦值。

```js
assert.match(attack.toString(), /text\.innerText\s*\+=/);
```

你應該將字符串 `" You attack it with your "` 添加到 `text.innerText` 值。 請記住間距很重要。

```js
assert.match(attack.toString(), /text\.innerText\s*\+=\s*('|") You attack it with your \1/);
```

你應該使用連接運算符將當前武器添加到字符串中。 你可以使用 `weapons[currentWeaponIndex].name` 獲取當前武器。
