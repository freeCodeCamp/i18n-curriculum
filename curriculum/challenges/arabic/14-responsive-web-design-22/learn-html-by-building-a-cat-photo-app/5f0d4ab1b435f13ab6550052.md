...
الآن، ستضيف بعض عناصر `input` الجديدة للـ form ، لذلك أضف عنصر `fieldset` آخر تحت عنصر `fieldset` مباشرة.
...
يجب عليك إضافة opening tag واحد فقط للـ `fieldset`. الرجاء إزالة أي زيادات.

```js
assert(document.querySelectorAll('fieldset').length === 2);
```

يجب أن يحتوي العنصر `fieldset` الجديد على closing tag. Closing tags لها `/` مباشرة بعد رمز `<`.

```js
assert(code.match(/<\/fieldset>/g).length >= 2);
```

يجب عليك إضافة closing tag واحد فقط للـ `fieldset`. الرجاء إزالة أي زيادات.

```js
assert(code.match(/<\/fieldset>/g).length === 2);
```

عنصر `fieldset` الثاني يجب أن لا يكون مدمج في عنصر `fieldset` الأول.
...
يجب أن تكون عناصر `fieldset` فوق عنصر fieldset للنص. هم بترتيب خاطئ.
...
يجب أن يكون عنصر `fieldset` الجديد الخاص بك تحت عنصر `fieldset` الموجود. لقد وضعتهم بترتيب خاطئ.
