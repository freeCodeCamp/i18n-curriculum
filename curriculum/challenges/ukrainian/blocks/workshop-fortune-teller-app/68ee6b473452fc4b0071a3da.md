...
Тепер зробіть те саме для `multipleCardsBtn` і `newReadingBtn`, приєднавши слухача події click до `multipleCardsBtn`, який викликає `multipleCardSelected`, і ще одного до `newReadingBtn`, який викликає `newReading`.

Ви створите ці два методи на наступних кроках.

# --hints--

Ви повинні приєднати подію `click` до елемента `multipleCardsBtn`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

У функції зворотного виклику події `multipleCardsBtn` ви повинні викликати метод `multipleCardSelected`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

Ви повинні приєднати подію `click` до елемента `newReadingBtn`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

У функції зворотного виклику події `newReadingBtn` ви повинні викликати метод `newReading`.
