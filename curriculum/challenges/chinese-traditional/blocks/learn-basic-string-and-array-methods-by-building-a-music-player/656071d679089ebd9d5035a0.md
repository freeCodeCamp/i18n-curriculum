...
由於使用者將能夠隨機播放和刪除播放列表中的歌曲，你需要創建一份 `allSongs` 陣列的複本，而不會改變原始陣列。這時 `spread` 運算子就非常有用。

展開運算子（`...`）允許你將一個陣列中的所有元素複製到另一個陣列中。它也可以用來將多個陣列串接成一個。在下面的範例中，`arr1` 和 `arr2` 都已經被展開到 `combinedArr` 中：
...
在 `userData` 物件內創建一個 `songs` 屬性。對於該值，將 `allSongs` 展開成一個陣列。

# --hints--

你的 `userData` 物件應該有一個名為 `songs` 的鍵。

```js
assert.property(userData, "songs");
```

你的 `songs` 屬性應該有一個值為 `[...allSongs]`。
