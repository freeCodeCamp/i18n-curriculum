...
Para obtener la canción anterior, resta `1` del `currentSongIndex` de `userData?.songs` y asígnalo a la constante `previousSong`. Después de eso, llama a la función `playSong` pasando `previousSong.id` como argumento.
...
Debes asignar `userData?.songs[currentSongIndex - 1]` a una constante llamada `previousSong`.
...
Debes llamar a la función `playSong` con `previousSong.id`.
