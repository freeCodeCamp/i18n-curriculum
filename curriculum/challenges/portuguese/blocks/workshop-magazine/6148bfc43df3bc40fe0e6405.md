...
Crie o seletor `.publish-date` e dê a ele a propriedade `color` definida com `rgba(255, 255, 255, 0.5)`.

# --hints--

Você deve ter um seletor `.publish-date`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

O seletor `.publish-date` deve ter a propriedade `color` definida como `rgba(255, 255, 255, 0.5)`.
