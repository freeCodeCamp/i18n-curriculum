...
اعطي `textarea` ال `placeholder` بقيمة `I like coding on the beach...`.

# --hints--

يجب عليك إعطاء `textarea` سمة `placeholder`.

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

يجب أن تعطي `placeholder` القيمة `I like coding on the beach...`.
