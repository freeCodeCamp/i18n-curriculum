...
`print()` の呼び出しの後、これまでに追加されたすべての支出を表示するために `print_expenses` 関数を呼び出してください。`expenses` リストを引数として渡してください。

# --hints--

`print_expenses()` 関数は `elif` 節の中で呼び出す必要があります。
...
`expenses` リストを `print_expenses()` 呼び出しの引数として渡す必要があります。
