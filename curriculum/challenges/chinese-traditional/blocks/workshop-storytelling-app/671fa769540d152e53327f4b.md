...
選擇 `#scary-btn`、`#funny-btn` 和 `#adventure-btn` 按鈕，並將它們存儲在名爲 `scaryStoryBtn`、`funnyStoryBtn` 和 `adventureStoryBtn` 的變量中。

# --hints--

你應該使用 `getElementById` 方法選擇 `#scary-btn` 按鈕並將其存儲在 `scaryStoryBtn` 變量中。
...
你應該使用 `getElementById` 方法選擇 `#funny-btn` 按鈕並將其存儲在 `funnyStoryBtn` 變量中。
...
你應該使用 `getElementById` 方法選擇 `#adventure-btn` 按鈕並將其存儲在 `adventureStoryBtn` 變量中。
