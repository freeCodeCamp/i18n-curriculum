...
現在你已經創建了 `resetButton`，你需要爲其分配 `id` 和 `aria-label` 屬性。 JavaScript 提供了你需要爲此使用的 `id` 和 `ariaLabel` 屬性。

例如，`element.id` 將設置 `id` 屬性，而 `element.ariaLabel` 將設置 `aria-label` 屬性。 它們都以字符串形式接受它們的值。

將 `resetButton` 元素的 `id` 屬性設置爲 `"reset"`，並將其 `aria-label` 屬性設置爲 `"Reset playlist"`。
...
你應該使用 `resetButton.id` 爲 `resetButton` 創建一個名爲 `"reset"` 的 `id` 屬性。

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

你應該使用 `resetButton.ariaLabel` 爲 `resetButton` 創建一個名爲 `"Reset playlist"` 的 `"aria-label"` 屬性。
