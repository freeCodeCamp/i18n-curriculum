...
在循环体内，将 `pass` 替换为 `if` 语句，该语句检查该数字是否是当前单元格的有效选择。

使用 `is_valid` 调用构建 `if` 条件，传递 `next_empty` 和 `guess` 作为参数。

# --hints--

你应该删除 `pass` 并创建一个 `if` 语句来检查 `self.is_valid(next_empty, guess)` 是否为 `True` 。
