...
使用类选择器将 `.cat-whisker-left-bottom` 移动到位，将其 `position` 设置为 `absolute`，`top` 设置为 `134px`，`left` 设置为 `52px`。

# --hints--

你应该有一个 `.cat-whisker-left-bottom` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom'))
```

你的 `.cat-whisker-left-bottom` 选择器应将 `position` 属性设置为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.position === 'absolute')
```

你的 `.cat-whisker-left-bottom` 选择器应该将 `top` 属性设置为 `134px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.top === '134px')
```

你的 `.cat-whisker-left-bottom` 选择器应该将 `left` 属性设置为 `52px`。
