...
للسيطرة الإضافية على تخطيط المحتوى، يمكنك وضع شبكة CSS داخل شبكة CSS أخرى.

عيّن خاصية `display` لمحدِّد `.heading` إلى `grid`.

# --hints--

يجب أن يحتوي محدِّد `.heading` على خاصية `display` مضبوطة على `grid`.
