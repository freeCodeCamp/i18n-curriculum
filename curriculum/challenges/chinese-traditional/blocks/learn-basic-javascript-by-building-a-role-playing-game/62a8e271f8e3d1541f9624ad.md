...
現在，你需要更新目前怪物的名稱和生命值的文字。

首先將 `monsters[fighting].name` 指定到 `monsterName` 的 `innerText` 屬性。然後，將 `monsterHealth` 指定到 `monsterHealthText` 的 `innerText` 屬性。

# --hints--

你應該使用點記法來存取 `monsterName` 的 `innerText` 屬性。

```js
assert.match(goFight.toString(), /monsterName\.innerText/);
```

你應該將 `monsters[fighting].name` 指定給 `monsterName.innerText`。

```js
assert.match(goFight.toString(), /monsterName\.innerText\s*=\s*monsters\s*\[\s*fighting\s*\]\s*\.name/);
```

你應該使用點記法來存取 `monsterHealthText` 的 `innerText` 屬性。

```js
assert.match(goFight.toString(), /monsterHealthText\.innerText/);
```

你應該將 `monsterHealth` 指定給 `monsterHealthText.innerText`。
