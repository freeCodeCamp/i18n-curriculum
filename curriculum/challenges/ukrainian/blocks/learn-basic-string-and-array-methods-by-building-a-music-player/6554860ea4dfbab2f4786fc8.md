...
Якщо придивитися уважно, ви побачите, що пісня, яка зараз відтворюється, не виділена у плейлисті, тож ви насправді не знаєте, яка пісня грає. Ви можете виправити це, створивши функцію, яка виділятиме будь-яку пісню, що відтворюється.

Використовуючи стрілковий синтаксис, створіть `highlightCurrentSong` функцію. Усередині функції використайте `querySelectorAll`, щоб отримати `.playlist-song` елемент і призначити його константі `playlistSongElements`.

# --hints--

Ви повинні використовувати `const`, щоб створити `highlightCurrentSong` функцію.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` має бути функцією.

```js
assert.isFunction(highlightCurrentSong)
```

Ваша `highlightCurrentSong` функція має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

Ви повинні створити константу `playlistSongElements` всередині вашої `highlightCurrentSong` функції.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

Ви повинні використати метод `querySelectorAll()`, щоб вибрати `.playlist-song` елемент і призначити його константі `playlistSongElements`.
