...
In früheren Schritten haben Sie gelernt, wie Sie mit der `sort()`-Methode die Songs alphabetisch sortieren. Ein weiterer Anwendungsfall für die Callback-Funktion ist das Zufallsmischen eines Arrays.

Eine Möglichkeit, ein Array von Elementen zu mischen, besteht darin, `0.5` von `Math.random()` zu subtrahieren, was zufällige Werte erzeugt, die entweder positiv oder negativ sind. Dadurch wird das Vergleichsergebnis zu einer Mischung aus positiven und negativen Werten, was zu einer zufälligen Anordnung der Elemente führt.
...
Verwenden Sie die `sort()`-Methode für das `userData?.songs`-Array. Übergeben Sie der Methode eine Callback-Funktion und geben Sie das Ergebnis von `Math.random() - 0.5` zurück.


# --hints--

Sie sollten die vorhandene `shuffle`-Funktion nicht ändern.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

Sie sollten die `sort()`-Methode für `userData?.songs` verwenden.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

Ihre `sort()`-Methode sollte eine Callback-Funktion mit Pfeil-Syntax enthalten.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

Der Callback Ihrer `sort()`-Methode sollte `Math.random() - 0.5` zurückgeben.
