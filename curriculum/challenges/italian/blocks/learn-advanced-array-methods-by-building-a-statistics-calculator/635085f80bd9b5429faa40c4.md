...
Puoi semplificare un po’ questa logica. Usando il ritorno implicito di una funzione freccia, puoi restituire direttamente il valore del metodo `.reduce()` diviso per la lunghezza dell’array, senza dover assegnare variabili.

Aggiorna la tua funzione `getMean` come descritto sopra.
...
Non dovresti modificare la logica all’interno del metodo `reduce`.
...
Dovresti usare la sintassi del ritorno implicito per restituire direttamente il risultato di `reduce` diviso per la lunghezza dell’array.
