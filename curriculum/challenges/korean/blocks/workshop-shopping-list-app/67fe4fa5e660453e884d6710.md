...
`toggleItem`라는 이름의 함수를 만들고 `item`을 인수로 받으세요.

함수 안에서는 일단 `setSelectedItems()` 함수를 호출하기만 하세요. 나중에 선택된 항목을 토글하는 로직을 구현할 예정입니다.

# --hints--

`toggleItem`을 인수로 받는 `item`라는 함수가 있어야 합니다.
...
`setSelectedItems()` 함수 안에서 `toggleItem()` 함수를 호출해야 합니다.
