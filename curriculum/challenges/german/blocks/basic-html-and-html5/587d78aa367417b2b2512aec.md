...
title: Definieren Sie den Head und Body eines HTML-Dokuments
challengeType: 0
...
Sie können in Ihrem HTML-Dokument eine weitere Organisationsebene innerhalb der `html`-Tags mit den `head`- und `body`-Elementen hinzufügen. Alle Auszeichnungen mit Informationen über Ihre Seite würden in den `head`-Tag eingefügt. Dann würden alle Auszeichnungen mit dem Inhalt der Seite (was für einen Benutzer angezeigt wird) in den `body`-Tag eingefügt.

Metadaten-Elemente wie `link`, `meta`, `title` und `style` befinden sich typischerweise innerhalb des `head`-Elements.
...
Bearbeiten Sie die Auszeichnung so, dass es einen `head` und einen `body` gibt. Das `head`-Element sollte nur den `title` enthalten, und das `body`-Element sollte nur den `h1` und `p` enthalten.

# --hints--

Es sollte nur ein `head`-Element auf der Seite geben.
...
Es sollte nur ein `body`-Element auf der Seite geben.
...
Das `body`-Element sollte sowohl die `h1`- als auch die `p`-Elemente umschließen.
