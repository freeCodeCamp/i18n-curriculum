...
Der nächste Schritt ist, das `posters`-Array zu durchlaufen, um alle ihre Avatare zu erhalten.

Beginnen Sie damit, ein `return`-Schlüsselwort gefolgt von `posters.map()` hinzuzufügen. Für die Callback-Funktion fügen Sie einen Parameter namens `poster` hinzu.

# --hints--

Sie sollten `posters.map()` zurückgeben.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

Ihre Callback-Funktion sollte einen Parameter namens `poster` haben.
