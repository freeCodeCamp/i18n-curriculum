...
داخل عنصر `button`، أنشئ عنصر `span` يحتوي على `😊`

أعطِ عنصر `span` خاصية `role` بقيمة `img` وخاصية `aria-hidden` بقيمة `true`.

ثم أنشئ عنصر `span` ثانيًا مع `class` بقيمة `count` يحتوي على النص `0/10`.

# --hints--

داخل عنصر `button` يجب أن يكون هناك عنصران `span`.

```js
assert.lengthOf(document.querySelectorAll("button > span"), 2);
```

يجب أن يحتوي عنصر `span` الأول على نص `😊`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.textContent.trim(), "😊");
```

يجب أن يحتوي عنصر `span` الأول على خاصية `role` بقيمة `img`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("role"), "img");
```

يجب أن يحتوي عنصر `span` الأول على خاصية `aria-hidden` بقيمة `true`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("aria-hidden"), "true");
```

يجب أن يحتوي عنصر `span` الثاني على خاصية `class` بقيمة `count`.

```js
assert.equal(document.querySelectorAll("button > span")[1]?.className, "count");
```

يجب أن يحتوي عنصر `span` الثاني على نص `0/10`.
