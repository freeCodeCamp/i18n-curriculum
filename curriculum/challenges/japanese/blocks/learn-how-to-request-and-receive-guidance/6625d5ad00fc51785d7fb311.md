---
id: 6625d5ad00fc51785d7fb311
title: タスク 40
challengeType: 19
dashedName: task-40
lang: en-US
---

<!-- (Audio) Sarah: Let's walk through the steps together. First, download the installer from the official website. It'll guide you through the setup process. If you have any issues during the installation, don't hesitate to reach out. -->

# --description--

サラはトムを助けるために一連の行動を提案しています。

# --questions--

## --text--

ソフトウェアのセットアップのためにサラがトムに提案した一連の行動は何ですか？

## --answers--

インストーラーをダウンロードし、セットアップ手順に従い、必要なら助けを求める。

---

公式ウェブサイトを訪れてインストールガイドを読む。

### --feedback--

ウェブサイトを訪れることは暗示されていますが、サラは特にインストーラーのダウンロードとセットアップ手順に従うことを述べています。

---

公式ウェブサイトを無視してガイドなしでインストールを試みる。

### --feedback--

サラは公式インストーラーを使い、その指示に従うことを勧めており、無視することは勧めていません。

---

助けを待つだけでインストールを試みない。

### --feedback--

サラの提案はトムが積極的に行動することであり、ただ助けを待つだけではありません。

## --video-solution--

1

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Sarah",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "9.2-2.mp3",
      "startTime": 1,
      "startTimestamp": 20.18,
      "finishTimestamp": 31.7
    }
  },
  "commands": [
    {
      "character": "Sarah",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sarah",
      "startTime": 1,
      "finishTime": 2.6,
      "dialogue": {
        "text": "Let's walk through the steps together.",
        "align": "center"
      }
    },
    {
      "character": "Sarah",
      "startTime": 3.42,
      "finishTime": 6.1,
      "dialogue": {
        "text": "First, download the installer from the official website.",
        "align": "center"
      }
    },
    {
      "character": "Sarah",
      "startTime": 6.1,
      "finishTime": 8.46,
      "dialogue": {
        "text": "It'll guide you through the setup process.",
        "align": "center"
      }
    },
    {
      "character": "Sarah",
      "startTime": 8.88,
      "finishTime": 12.52,
      "dialogue": {
        "text": "If you have any issues during the installation, don't hesitate to reach out.",
        "align": "center"
      }
    },
    {
      "character": "Sarah",
      "opacity": 0,
      "startTime": 13.02
    }
  ]
}
```
