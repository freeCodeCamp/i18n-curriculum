...
聲明一個 `breakfastNumberInputs` 變量，並賦予其使用選擇器 `#breakfast input[type='number']` 調用 `document.querySelectorAll()` 的值。 這將返回 `#breakfast` 元素中的任何 `number` 輸入。

# --hints--

你應該聲明一個 `breakfastNumberInputs` 變量。

```js
assert.match(calculateCalories.toString(), /breakfastNumberInputs\s*=/);
```

你應該使用 `#breakfast input[type='number']` 調用 `document.querySelectorAll()`。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#breakfast input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你應該將 `document.querySelectorAll()` 調用的結果分配給 `breakfastNumberInputs`。
