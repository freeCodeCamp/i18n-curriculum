...
要開始你的 CSS，請透過標的所有元素的 `*`，包括 `::before` 和 `::after` 偽元素，來標準化 CSS 規則。

將 `padding` 和 `margin` 屬性都設為 `0`，並將 `box-sizing` 屬性設為 `border-box`。
