...
在你最後一個 `.divider` 之後，創建一個新的 `p` 元素，文字為 `Cholesterol 0mg 0%`。然後，將文字 `Cholesterol` 包裹在一個 `span` 元素中，`0%` 包裹在另一個 `span` 元素中，並為它們各自指定一個 `bold` 類別。

最後，將包含文字 `Cholesterol` 的 `span` 元素與文字 `0mg` 一起巢狀在另一個 `span` 元素中以便對齊。

# --hints--

你應該在你的 `.daily-value` 元素末端創建一個新的 `p` 元素。

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'p');
```

你的新 `p` 元素應該有文字 `Cholesterol 0mg 0%`。
...
包含文字 `Cholesterol` 的 `span` 元素應將其 `class` 屬性設為 `bold`。
...
包含文字 `0%` 的 `span` 元素應將其 `class` 屬性設為 `bold`。
...
包含文字 `Cholesterol` 和文字 `0mg` 的 `span` 元素應該一起包裹在另一個 `span` 元素中。
