...
Diese dreiteilige Phrase bezeichnet eine Gruppe von Personen, die an einem Sicherheitsereignis oder -problem arbeiten.

# --explanation--

`Incident response team` ist eine spezialisierte Gruppe, die Sicherheitsvorfälle bearbeitet. Zum Beispiel:

- `The incident response team handled the security breach.` – Das Team kümmerte sich um das Problem, als das System gehackt wurde.

- `We called the incident response team when we noticed suspicious activity.` – Sie kontaktierten das Team, als sie etwas Ungewöhnliches bemerkten.
