...
`re` 模块的 `sub` 函数允许你基于正则表达式替换字符串中的文本。
...
它接收三个参数：要匹配的正则表达式，替换内容和要执行替换的字符串。

从 `__str__` 方法返回 `sub()` 调用，接收字符串 `'1'` ，一个空字符串和已经存在的 `equation_string.strip('+')` 调用作为参数。 这将用空字符串替换每一个 `1`。 结果尚未完善，你将在接下来的步骤中继续处理正则表达式。

# --hints--

你应该从 `__str__` 方法中返回 `re.sub()` 调用。 将字符串 `'1'` ，空字符串和现有的 `equation_string.strip('+')` 调用作为参数传递给 `re.sub()` 函数。
