...
1. Debes tener un `button` con un `id` de `close-btn` dentro de tu elemento `.lightbox`. Puedes usar `&times;` como su texto si quieres.
...
1. Cuando tu elemento  `.lightbox` esté visible y hagas clic en el  `#close-btn` o el elemento  `.lightbox`, el  `.lightbox` del elemento  `display` se debe ajustar nuevamente a  `none`.
...
Dentro de tu elemento  `.lightbox`, debes tener un elemento  `button` con el  `id` configurado en  `close-btn`.
...
Cuando haces clic en uno de tus elementos  `.gallery-item`, el atributo  `src` del elemento  `#lightbox-image` debe configurarse en una versión de tamaño completo de la imagen clicada eliminando  `-thumbnail` del atributo  `src` de la imagen.
...
Cuando tu elemento `.lightbox` esté visible y hagas clic en el botón `#close-btn`, el `.lightbox` del elemento `display` se debe ajustar nuevamente a `none`.
...
Cuando tu elemento `.lightbox` esté visible y hagas clic en el elemento `.lightbox`, el `.lightbox` del elemento `display` se debe ajustar nuevamente a `none`.
