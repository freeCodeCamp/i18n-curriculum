...
أنشئ حلقة `for` للتكرار على `targets_to_print` واطبع سلسلة f التالية: `f'\n{start}-{node} distance: {distances[node]}\nPath: {" -> ".join(paths[node])}'`.

# --hints--

يجب أن تنشئ حلقة `for` للتكرار على `targets_to_print`. استخدم `node` كمتغير للحلقة.
...
يجب أن تطبع السلسلة المقدمة داخل حلقة `for` الجديدة.
