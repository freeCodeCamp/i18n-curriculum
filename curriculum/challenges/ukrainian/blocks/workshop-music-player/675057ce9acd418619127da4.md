...
Тепер створіть функцію з назвою `getPreviousSong`. Вона має повертати пісню, яка знаходиться на позиції `getCurrentSongIndex()` мінус один у `userData.songs`, або `undefined`, якщо поточна пісня є першою у плейлисті.

# --hints--

У вас має бути функція з назвою `getPreviousSong`.

```js
assert.isFunction(getPreviousSong);
```

Ваша функція `getPreviousSong` має викликати `getCurrentSongIndex`.
...
Ваша функція `getPreviousSong` має повертати пісню перед поточною.
...
Ваша функція `getPreviousSong` має повертати `undefined`, коли поточна пісня є першою у плейлисті.
