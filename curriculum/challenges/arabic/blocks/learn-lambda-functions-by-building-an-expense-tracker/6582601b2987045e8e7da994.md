...
بعد استدعاء `print()`، تحتاج إلى تصفية النفقات وطباعة القائمة المصفاة. أعلن متغيرًا باسم `expenses_from_category` وقم بتعيين استدعاء `filter_expenses_by_category` له مع تمرير `expenses` و`category` كمعلمات.

# --hints--

يجب إعلان متغير باسم `expenses_from_category` في بند `elif` الخاص بك.
...
يجب استدعاء `filter_expenses_by_category()` مع تمرير `expenses` و`category` كمعلمات وتعيينه إلى متغير `expenses_from_category` الخاص بك.
...
يجب أن يأتي متغير `expenses_from_category` بعد استدعاء `print()`.
