...
Propriedades de objeto consistem em pares de chave/valor. Você pode usar nomes de propriedades abreviados ao declarar um objeto literal. Ao usar a sintaxe do nome da propriedade abreviada, o nome da variável se torna a chave da propriedade e seu valor o valor da propriedade.

O exemplo a seguir declara um objeto `user` com as propriedades `userId`, `firstName` e `loggedIn`.
...
Para manter o controle de todas as funções da planilha, declare um objeto `spreadsheetFunctions`. Usando a sintaxe de notação abreviada, defina `sum`, `average` e `median` como propriedades no objeto `spreadsheetFunctions`.
