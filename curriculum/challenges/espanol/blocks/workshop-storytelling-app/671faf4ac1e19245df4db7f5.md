...
Dentro de la función `displayStory`, verifica si el objeto `storyObj` tiene una propiedad que coincide con el parámetro `genre` y establece el contenido del texto del párrafo `#result` con la historia del género seleccionado.

Además, ajusta el color del borde de `storyContainer` al color de la propiedad `borderColor` del género seleccionado.

Prueba esto llamando a la función `displayStory` con el género `scary`. El color del borde debe ser <code>#ee4b2b` y la historia debe ser `"En el bosque oscuro, un grupo de amigos se encontró con una cabaña vieja y abandonada. "</code>. Entraron a la cabaña y despertaron algo malévolo que había estado dormido durante siglos.

# --hints--

Debes llamar a la función `displayStory` con el género `"scary"`.

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

Cuando llames a `displayStory("scary")`, el color del borde debe ser `#ee4b2b`.
...
Cuando llames a <code>displayStory("scary")`, la historia mostrada debe ser `"En el bosque oscuro, un grupo de amigos se encontró con una cabaña vieja y abandonada. "</code>. Entraron a la cabaña y despertaron algo malévolo que había estado dormido durante siglos.
