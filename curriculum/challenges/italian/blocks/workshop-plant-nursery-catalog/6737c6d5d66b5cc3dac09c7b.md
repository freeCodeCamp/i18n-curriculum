...
Mappe e insiemi condividono alcuni metodi che hanno lo stesso nome e scopo. Per esempio puoi usare il metodo `has` su un insieme per verificare se contiene un elemento specifico.

Usa il metodo `has` per controllare se l’oggetto `ballerina` è contenuto nel tuo `plantsSet` e registra il risultato nella console.

# --hints--

Dovresti registrare `plantsSet.has(ballerina)` nella console.
