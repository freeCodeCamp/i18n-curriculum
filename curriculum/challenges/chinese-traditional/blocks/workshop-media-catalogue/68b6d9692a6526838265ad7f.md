...
`TVSeries` 物件將會相較於其父類別 `Movie` 具現化時帶有額外的屬性。因此，請創建一個帶有以下參數的 `__init__` 方法：`self`、`title`、`year`、`director`、`duration`、`seasons` 和 `total_episodes`。

在 `__init__` 方法中，將 `seasons` 和 `total_episodes` 指定到同名的屬性。

# --hints--

你的 `TVSeries` 類別應該有一個 `__init__` 方法。
...
你的 `__init__` 方法應該有以下參數：`self`、`title`、`year`、`director`、`duration`、`seasons` 和 `total_episodes`。
...
你應該在你的 `__init__` 方法中將 `seasons` 指定給 `self.seasons`。
...
你應該在你的 `__init__` 方法中將 `total_episodes` 指定到 `self.total_episodes`。
