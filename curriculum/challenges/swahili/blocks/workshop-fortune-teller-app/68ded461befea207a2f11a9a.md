...
Hadi sasa, una vitendakazi vitatu: `getElement`, `hideElements`, na `showElements`.
Sasa unahitaji kitendakazi kinachokusaidia kupata bahati ya nasibu.

Tangaza kitendakazi kinachoitwa `getRandomItem` kinachopokea safu ya data ya `items` kama hoja. Tumia generics kwa kitendakazi hiki.

Ndani ya kitendakazi, tumia `Math.random()` na `Math.floor()` kuchagua bahati ya nasibu kutoka kwenye safu ya data ya `items`.
Usisahau kurudisha bahati iliyochaguliwa.

# --hints--

Unapaswa kutangaza kitendakazi kinachoitwa `getRandomItem`.
...
Kitendakazi chako cha `getRandomItem` kinapaswa kutumia kigezo cha aina ya generic `T`.
...
Kitendakazi chako cha `getRandomItem` kinapaswa kuwa na kigezo kinachoitwa `items` chenye aina ya `T[]`.
...
Kitendakazi chako cha `getRandomItem` kinapaswa kuwa na aina ya kurudisha `T`.
...
Unapaswa kutangaza kigezo kinachoitwa `index` ndani ya kitendakazi cha `getRandomItem`.
...
Kigezo cha `index` kinapaswa kutumia `Math.floor()` na `Math.random()` pamoja na `items.length` kutengeneza kielezo cha bahati.
...
Kitendakazi chako cha `getRandomItem` kinapaswa kurudisha `items[index]`.
