...
Weka maandishi yote ndani ya kipengele cha `address` katikati kwa usawa wa wima, na ongeza nafasi ya ndani.

# --hints--

Unapaswa kutumia kichaguzi cha kipengele cha `address`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

Unapaswa kumpa `address` `text-align` ya `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Unapaswa kumpa `address` `padding-top` ya angalau `1px`.
...
Unapaswa kumpa `address` `padding-right` ya angalau `1px`.
...
Unapaswa kumpa `address` `padding-left` ya angalau `1px`.
