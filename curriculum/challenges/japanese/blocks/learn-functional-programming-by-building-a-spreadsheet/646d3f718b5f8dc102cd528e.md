...
数学の式を評価できるようになったので、演算の順序を考慮する必要があります。`highPrecedence` 関数を宣言し、`str` パラメータを受け取ってください。

# --hints--

`highPrecedence` 変数を宣言してください。

```js
assert.match(code, /(?:var|let|const)\s+highPrecedence/);
```

`const` を使って `highPrecedence` 変数を宣言してください。

```js
assert.match(code, /const\s+highPrecedence/);
```

`highPrecedence` 変数は関数である必要があります。

```js
assert.isFunction(highPrecedence);
```

`highPrecedence` 関数はアロー構文を使ってください。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`highPrecedence` 関数は `str` パラメータを持つべきです。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>/);
```

`highPrecedence` 関数は空であるべきです。
