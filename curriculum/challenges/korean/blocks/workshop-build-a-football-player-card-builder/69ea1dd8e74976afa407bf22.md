...
다음으로, 값이 `Position`인 `typeof POSITIONS[number]` 타입을 만드세요. 이 타입은 `POSITIONS` 배열을 개별 문자열 리터럴의 합집합으로 풀어내어, 사실상 `"GK" | "CB" | "LB" | ...`처럼 보이게 만듭니다.

# --hints--

`Position`라는 타입 별칭을 만들어야 합니다.
...
`Position` 타입은 `typeof POSITIONS[number]`로 설정해야 합니다.
