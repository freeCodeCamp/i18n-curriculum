...
"버블 업" 결과를 얻으려면 현재 요소가 다음 요소보다 큰지 확인해야 합니다. `array`와 `j`에서 `j+1`에 접근하여 이 작업을 할 수 있습니다.

현재 요소가 다음 요소보다 큰지 확인하는 `if` 조건을 만드세요.

# --hints--

`console.log` 문장은 제거하지 마세요.
...
`if` 문장은 `array[j]`가 `array[j+1]`보다 큰지 확인해야 합니다.
