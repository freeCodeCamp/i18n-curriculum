...
اعطي المنتقي الجديد خاصية `font-size` من `0.7em` لجعله يبدو مثل sub-heading.

# --hints--

يجب أن يكون لديك منتقي `h1 .flex span:first-of-type`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1 .flex span:first-of-type'));
```

يجب ان يكون لدى منتقي `h1 .flex span:first-of-type` خاصية `font-size` بقيمة `0.7em`.
