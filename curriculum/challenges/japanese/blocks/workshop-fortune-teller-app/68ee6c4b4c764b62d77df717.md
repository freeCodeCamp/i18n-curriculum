...
カードは常に正しい位置にあるとは限らないため、場合によってはカードを裏返す必要があります。

`isReversed` メソッド内で `singleCardSelected` という変数を宣言してください。`Math.random()` メソッドを使ってランダムなブール値を生成します。値が範囲内に収まるように、`0.5` と比較してください。
...
`isReversed` 変数は、カードが裏返しになるかどうかをランダムに決めるために `Math.random()` を使うべきです。
