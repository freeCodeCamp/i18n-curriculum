...
Finalmente, passe `false` como o segundo argumento para a chamada da função `playSong(userData.currentSong.id)`.Então, teste se tudo funciona como esperado tocando uma música, pausando-a e então pressionando o botão anterior ou o botão próximo.

# --hints--

Você deve passar `false` como o segundo argumento para a chamada da função `playSong(userData.currentSong.id)`.
