...
أعلن عن متغير `functionCall`، وقم بتعيين هذا التعبير النمطي له: `/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

هذا التعبير يبحث عن استدعاءات الدوال مثل `sum(1, 4)`.

# --hints--

يجب إعلان متغير `functionCall`.
...
يجب استخدام `const` لإعلان متغير `functionCall`.
...
يجب تعيين التعبير النمطي المقدم إلى `functionCall`.
