...
1. Você deve ter uma função `getBookmarks` que retorna o array `bookmarks` armazenado no armazenamento local. If the `bookmarks` key has not been set yet, or it doesn't contain a valid array with bookmarks, the `getBookmarks` function should return an empty array.
