...
Declara una variable llamada `cardElement` y usa la clase `card_container` para obtener el elemento. Usa el selector `closest` para obtenerlo desde el `target`.

# --hints--

Debes declarar una variable llamada `cardElement` dentro de `showFortune`.
...
Tu `cardElement` debe usar `target?.closest` para encontrar el `.card_container`.
...
Debes pasar `.card_container` como argumento a `closest`.
