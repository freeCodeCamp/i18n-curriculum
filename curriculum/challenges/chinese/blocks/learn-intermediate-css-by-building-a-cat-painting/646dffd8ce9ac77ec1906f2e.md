...
在你的 `.cat-mouth` 元素内，创建一个带有类 `cat-mouth-line-left` 的 `div` 元素和另一个带有类 `cat-mouth-line-right` 的 `div` 元素。

# --hints--

你不应该更改现有的类名为 `cat-mouth` 的 `div` 元素。

```js
assert(document.querySelectorAll('div.cat-mouth').length === 1);
```

你应该在 `.cat-mouth` 元素内有两个 `div` 元素。

```js
assert(document.querySelectorAll('.cat-mouth div').length === 2);
```

在 `.cat-mouth` 元素内的第一个 `div` 元素应具有类 `cat-mouth-line-left`。

```js
assert(document.querySelectorAll('.cat-mouth div')[0]?.classList.contains('cat-mouth-line-left'));
```

在 `.cat-mouth` 元素内的第二个 `div` 元素应该有类 `cat-mouth-line-right`。
