...
从现有的咖啡/价格对开始，使用 `article` 元素添加下列的咖啡和价格，每个元素内部有两个嵌套的 `p` 元素：
...
和之前一样，第一个 `p` 元素的文本应该包含咖啡风味，第二个 `p` 元素的文本应该包含价格。
...
你的第一个 `article` 元素应该有包含文本 `French Vanilla` 和 `3.00`的 `p` 元素。
...
你的第二个 `article` 元素应该有包含文本 `Caramel Macchiato` 和 `3.75`的 `p` 元素。
...
你的第三个 `article` 元素应该有包含文本 `Pumpkin Spice` 和 `3.50`的 `p` 元素。
...
你的第四个 `article` 元素应该有包含文本 `Hazelnut` 和 `4.00`的 `p` 元素。
...
你的第五个 `article` 元素应该有包含文本 `Mocha` 和 `4.50`的 `p` 元素。
