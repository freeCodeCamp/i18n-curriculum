...
Actualmente, el carrito no es visible en la página web. Para hacerlo visible, empieza añadiendo un escucha de eventos a la variable `cartBtn`, escuchando el evento de clic. La función de llamada no necesita ningún parámetro.

# --hints--

Debe añadir un escucha de eventos a la variable `cartBtn`.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(/);
```

Debes escuchar un evento de `click` en la variable `cartBtn`.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

Debe añadir una función de llamada vacía (usando sintaxis de flecha) al escucha de eventos. Recuerde que no necesita ningún parámetro.
