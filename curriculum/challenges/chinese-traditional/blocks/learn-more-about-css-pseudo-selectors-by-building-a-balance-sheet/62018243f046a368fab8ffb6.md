...
創建一個 `table` 選擇器來定位你的表格。 將 `border-collapse` 屬性設置爲 `collapse`，這將允許單元格邊框摺疊成單個邊框，而不是每個單元格周圍都有邊框。 同時將 `border` 屬性設置爲 `0` 以隱藏邊框本身。
