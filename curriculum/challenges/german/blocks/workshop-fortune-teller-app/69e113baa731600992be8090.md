...
Erstellen Sie eine `CDN_URL`-Variable und setzen Sie sie auf `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. Erstellen Sie außerdem eine `defaultImg`-Variable mit dem Wert `new URL("default.svg", import.meta.url)` und eine `LOCAL_DEFAULT_IMG`-Variable mit dem Wert `defaultImg`.
...
Ihre `defaultImg`-Variable sollte auf `new URL("default.svg", import.meta.url)` gesetzt werden.
