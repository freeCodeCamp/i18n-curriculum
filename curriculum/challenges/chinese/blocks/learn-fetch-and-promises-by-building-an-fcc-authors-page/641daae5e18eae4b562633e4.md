...
最后，如果出现错误并且作者数据加载失败怎么办？然后我们需要在 UI 中显示错误。这正是 `.catch()` 方法的用途——处理错误。

在 `.catch()` 内部，删除 `console.error()`，并将 `innerHTML` 的 `authorContainer` 设置为 `p` 元素，其中包含 `class`、`"error-msg"` 和文本 `"There was an error loading the authors"`。
