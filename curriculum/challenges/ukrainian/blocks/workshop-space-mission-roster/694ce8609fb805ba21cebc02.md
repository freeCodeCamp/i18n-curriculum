...
Центр управління потребує методу для обміну позиціями двох членів екіпажу у списку. Створіть порожню функцію з назвою `swapCrewMembers` з трьома параметрами:

- `crew`: масив об’єктів астронавтів

- `fromIndex`: індекс першого астронавта для обміну

- `toIndex`: індекс другого астронавта для обміну

# --hints--

Ви повинні створити функцію з назвою `swapCrewMembers`.

```js
assert.isFunction(swapCrewMembers);
```

Ваша функція `swapCrewMembers` має мати параметри `crew`, `fromIndex` і `toIndex`.
