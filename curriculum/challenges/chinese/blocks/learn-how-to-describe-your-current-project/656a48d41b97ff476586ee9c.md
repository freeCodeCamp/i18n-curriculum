...
In this context, to be `out` means to be available or published. 例如：

`The new book is out next week.` - This means the book will be available then.

`Looking forward to` + a verb that ends in `-ing` is a common way to express excitement about a future event. 例如：
