...
Proprio come le funzioni normali, anche le funzioni freccia possono restituire valori.

Ecco un esempio di una funzione freccia che restituisce il risultato della moltiplicazione di due numeri:
...
Crea una nuova variabile chiamata `addTwoNumbers` e assegnale una funzione freccia. Questa funzione freccia deve prendere due parametri chiamati `num1` e `num2`. All'interno del corpo della funzione, restituisci l'espressione che somma `num1` e `num2`.

Sotto la funzione `addTwoNumbers`, aggiungi un'istruzione console. All'interno di questa istruzione console, chiama la funzione `addTwoNumbers` e passa i numeri `3` e `4` come argomenti.

Apri la console per vedere l'output.
...
La tua funzione `addTwoNumbers` dovrebbe usare la sintassi della funzione freccia.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

La tua funzione `addTwoNumbers` dovrebbe avere due parametri chiamati `num1` e `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

La tua funzione `addTwoNumbers` dovrebbe restituire l'espressione che somma `num1` e `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

Dovresti avere un'istruzione `console.log(addTwoNumbers(3, 4));`.
