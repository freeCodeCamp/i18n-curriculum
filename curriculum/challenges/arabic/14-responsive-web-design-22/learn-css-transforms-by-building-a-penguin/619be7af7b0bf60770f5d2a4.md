...
بداخل عنصر الزائف `.penguin-body::before` يجب أن تكون خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

بداخل عنصر الزائف `.penguin-body::before` يجب أن تكون خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
