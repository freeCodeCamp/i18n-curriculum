...
حان الوقت لتنسيق زر الإرسال.

اختر العنصر الذي يحمل الصنف `submit-btn`. عيّن `cursor` إلى `pointer` (للدلالة على أنه قابل للنقر)، و`background-color` إلى `royalblue`، و`color` إلى `whitesmoke`، و`border` إلى `none`، و`border-radius` إلى `6px`، و`padding` إلى `12px 20px`.

# --hints--

يجب أن تستهدف العنصر الذي يحمل الصنف `submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

عيّن الخاصية `cursor` إلى `pointer`.
...
عيّن الخاصية `background-color` إلى `royalblue`.
...
عيّن الخاصية `color` إلى `whitesmoke`.
...
عيّن الخاصية `border-radius` إلى `6px`.
...
عيّن الخاصية `padding` إلى `12px 20px`.
