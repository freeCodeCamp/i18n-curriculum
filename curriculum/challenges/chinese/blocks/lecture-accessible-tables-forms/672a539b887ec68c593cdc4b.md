...
`scope` 属性有四种可能的值。 你最常用的两个是用于列的 `col` 和用于行的 `row`。 在下面的代码中，你可以看到我们为列和行表头添加了 `scope` 属性。 三个列表头 (`Name`, `Age`, and `Type`) 具有列值为 `scope` 的 `col`。

两个行表头（`Nora` 和 `Gino`）具有值为 `scope` 的 `row`。
...
在此表格中，包含 `Nora` 年龄（`5`）的单元格将有一个列头部（`Age`）和两个行头部（`Dogs` 和 `Nora`）。包含 `Gino` 年龄（`2`）的单元格也将有一个列头部（`Age`）和两个行头部（`Dogs` 和 `Gino`）。
