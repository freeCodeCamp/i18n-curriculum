...
子孫セレクターを使って、クラス`div`を持つ`div`の中にある2つの`cat-mouth`要素を選択してください。幅を`30px`、高さを`50px`、ボーダーを`2px solid #000`に設定してください。

# --hints--

`.cat-mouth div`セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth div'))
```

`.cat-mouth div`セレクターには`width`プロパティを`30px`に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth div')?.width === '30px')
```

`.cat-mouth div`セレクターには`height`プロパティを`50px`に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth div')?.height === '50px')
```

`.cat-mouth div`セレクターには`border`プロパティを`2px solid #000`に設定してください。
