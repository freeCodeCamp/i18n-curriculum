...
Aggiungi un attributo `value` di `player.pas` al PAS `input`. Poi aggiungi un attributo `onChange` impostato su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `pas` impostato su `Number(e.target.value)`.

# --hints--

Il tuo PAS `input` dovrebbe avere un `value` uguale alla statistica `pas` del giocatore.
...
Il tuo PAS `input` dovrebbe avere un gestore `onChange` che chiama `setPlayer` con uno spread di `player` e `pas` impostato su `Number(e.target.value)`.
