...
حان الوقت الآن لتطبيق ما تعلمته عن الدوال السهمية في مشروع مشغل الموسيقى الخاص بك.

ابدأ بحذف الدوال `printGreeting` و `printMessage` و `addTwoNumbers`. واحذف أيضًا جميع عبارات وحدة التحكم واستدعاءات الدوال.

# --hints--

يجب ألا يكون لديك دالة `printGreeting`.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

يجب ألا يكون لديك دالة `printMessage`.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

يجب ألا يكون لديك دالة `addTwoNumbers`.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

يجب ألا يكون لديك استدعاء دالة `printGreeting();`.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

يجب ألا يكون لديك استدعاء دالة `printMessage('freeCodeCamp');`.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

يجب ألا يكون لديك عبارة وحدة تحكم `console.log(addTwoNumbers(3,4));`.
