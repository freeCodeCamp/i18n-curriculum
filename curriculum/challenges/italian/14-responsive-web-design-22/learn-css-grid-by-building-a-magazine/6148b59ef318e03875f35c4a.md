...
La proprietà `object-fit` dice al browser come posizionare l'elemento all'interno del suo contenitore. In questo caso, `cover` imposterà l'immagine per riempire il contenitore, se necessario ritagliando per evitare di cambiare il rapporto d'aspetto.
...
Il selettore `img` dovrebbe avere una proprietà `width` impostata al `100%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('img')?.width === '100%');
```

Il selettore `img` dovrebbe avere una proprietà `object-fit` impostata su `cover`.
