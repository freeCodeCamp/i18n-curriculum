...
選擇 `.arm` 元素，將它們的 `width` 設置爲 `30%`，`height` 設置爲 `60%`，`background` 設置爲線性漸變，順時針方向 `90deg`，以 `gray` 處開始，以 `rgb(209, 210, 199)` 結束。
...
你應該將 `.arm` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

你應該將 `.arm` 的 `height` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

你應該將 `.arm` 的 `background` 設置爲 `linear-gradient(90deg, gray, rgb(209, 210, 199))`。
