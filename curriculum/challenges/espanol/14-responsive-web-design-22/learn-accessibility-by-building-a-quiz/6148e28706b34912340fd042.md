...
Debes dar al `address` un `text-align` de `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Debes dar al `address` un `padding-top` de al menos `1px`.
...
Debes dar al `address` un `padding-right` de al menos `1px`.
...
Debes dar al `address` un `padding-bottom` de al menos `1px`.
...
Debes dar al `address` un `padding-left` de al menos `1px`.
