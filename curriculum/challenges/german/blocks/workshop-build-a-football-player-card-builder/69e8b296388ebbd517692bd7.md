...
Sie beginnen jetzt den Prozess, die Positions-Eingabe in ein Auswahlfeld zu ändern.

Ändern Sie die Eingabe in ein `select`-Element mit der ID `position`, einem `className` von `input` und einem `value` von `player.position`.

# --hints--

Sie sollten die Positions-`input` in ein `select`-Element mit einem `id` von `position` und `className` von `input` ändern.
...
Ihr Positions-`select` sollte ein `value` von `player.position` haben.
