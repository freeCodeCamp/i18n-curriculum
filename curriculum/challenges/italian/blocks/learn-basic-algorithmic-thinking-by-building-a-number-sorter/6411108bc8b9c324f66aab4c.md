...
Dichiara una variabile `currValue` e assegnale il valore `array[i]`. Poi, dichiara una variabile `j` e assegnale `i - 1`. La tua variabile `j` dovrebbe poter essere riassegnata.

# --hints--

Dovresti dichiarare una variabile `currValue` con `const`.
...
Dovresti assegnare a `currValue` il valore `array[i]`.
...
Dovresti dichiarare una variabile `j` con `let`.
...
Dovresti assegnare a `j` il valore di `i - 1`.
