...
メニュー項目と価格にフォーカスすると、各行の間にかなり大きな隙間があります。

クラス名が`p`の要素にネストされたすべての`item`要素をターゲットにする既存のセレクターを使い、それらの上下のマージンを`5px`に設定してください。

# --hints--

`margin-top`プロパティを`5px`に設定してください。
...
`margin-bottom`プロパティを`5px`に設定してください。
...
`p`要素にネストされた`.item`要素は、`margin-top`の`5px`を持つべきです。
...
`p`要素にネストされた`.item`要素は、`margin-bottom`の`5px`を持つべきです。
...
`p`要素にネストされた`.item`要素には、`margin-left`が設定されていないはずです。
...
`p`要素にネストされた`.item`要素には、`margin-right`が設定されていないはずです。
