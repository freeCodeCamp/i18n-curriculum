...
Du solltest dem Spieler mitteilen, welche Waffe er gekauft hat. Benutze zwischen den zwei Zeilen, die du eben geschrieben hast, `let`, um eine neue Variable namens `newWeapon` zu initialisieren. Setze dies gleich `weapons`.
...
Du solltest `let` verwenden, um `newWeapon` zu deklarieren.

```js
assert.match(code, /let\s+newWeapon/);
```

`newWeapon` sollte so initialisiert werden, dass es den Wert von `weapons` hat. Don't forget your semicolon.

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

`newWeapon` sollte deklariert werden, bevor du `text` änderst.
