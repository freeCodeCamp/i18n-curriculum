...
Час побачити вашу карировану функцію в дії. Ось як ви можете викликати наш приклад функції:

```js
const dinner = tikkaMasala("curry powder")("tofu")("onion");
```

Оголосіть змінну `gramsResult` і призначте їй результат використання вашої карированої функції для конвертації `2` одиниць виміру `cup` у `gram`.

Виведіть це значення в консоль.

# --hints--

У вас має бути змінна `gramsResult`.

```js
assert.isDefined(gramsResult);
```

Ваша змінна `gramsResult` має містити результат виклику `convertQuantity("cup")("gram")(2)`.

```js
assert.strictEqual(gramsResult, convertQuantity("cup")("gram")(2));
```

Ви маєте вивести `gramsResult` у консоль.
