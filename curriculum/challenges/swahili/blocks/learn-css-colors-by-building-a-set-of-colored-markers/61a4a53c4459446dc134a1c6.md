...
Sasa unajua misingi ya jinsi kitendakazi cha `linear-gradient` na vituo vya rangi vinavyofanya kazi, unaweza kuvitumia kufanya alama zionekane halisi zaidi.

Katika kitendakazi cha `linear-gradient`, weka `gradientDirection` kuwa `180deg`.

# --hints--

Kanuni yako ya `.red` CSS inapaswa kuwa na sifa ya `background` iliyowekwa kuwa `linear-gradient(180deg, rgb(255, 0, 0) 75%, rgb(0, 255, 0), rgb(0, 0, 255))`.
