...
Verwenden Sie `const` und Pfeilsyntax, um eine leere `playPreviousSong`-Funktion zu erstellen.

# --hints--

Sie sollten `const` verwenden, um eine leere Funktion mit dem Namen `playPreviousSong` zu erstellen.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` sollte eine Funktion sein.

```js
assert.isFunction(playPreviousSong)
```

Ihre `playPreviousSong`-Funktion sollte eine Pfeilsyntax verwenden.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Ihre `playPreviousSong`-Funktion sollte leer sein.
