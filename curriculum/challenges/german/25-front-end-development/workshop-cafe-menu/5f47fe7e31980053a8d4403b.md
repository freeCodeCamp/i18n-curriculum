...
Es gibt eine einfachere Möglichkeit - füge einfach einen negativen Wert für den oberen Rand zu den `img`-Elementen hinzu, um ihre aktuelle Position weiter oben auszurichten. Negative Werte werden mit einem `-` vor dem Wert erzeugt. To complete this workshop, go ahead and use a negative top margin of `25px` in the `img` type selector.
