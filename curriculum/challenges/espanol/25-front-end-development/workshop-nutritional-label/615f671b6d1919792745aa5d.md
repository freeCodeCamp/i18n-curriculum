...
Cree un nuevo elemento `div` debajo de su elemento `.medium`. Asígnele un atributo `class` configurado en `daily-value small-text`. Dentro de este nuevo elemento `div`, agregue un elemento `p` con el texto `% Daily Value *`, y establezca el atributo `class` en `bold right`.
...
Su nuevo elemento `div` debe tener un atributo `class` configurado en `daily-value small-text`.
...
Su nuevo elemento `p` debe tener un atributo `class` configurado en `bold right`.
