...
Erstellen Sie einen `.publish-date`-Selektor und geben Sie ihm eine `color`-Eigenschaft mit dem Wert `rgba(255, 255, 255, 0.5)`.

# --hints--

Sie sollten einen `.publish-date`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

Ihr `.publish-date`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `rgba(255, 255, 255, 0.5)` haben.
