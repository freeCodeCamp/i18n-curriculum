...
Pata kipengele cha `select` chenye kitambulisho `species` kutoka kwenye DOM na uhifadhi katika thabiti iitwayo `selectElement`. Tumia kiendeshaji cha uthibitisho wa non-null `!` kuthibitisha kuwa kipengele si `null`

# --hints--

Unapaswa kuwa na thabiti iitwayo `selectElement`.
...
`selectElement` yako inapaswa kuomba kipengele chenye kitambulisho `species` kwa kutumia `document.querySelector` na hoja ya aina ya `HTMLSelectElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

`selectElement` yako inapaswa kutumia kiendeshaji cha uthibitisho wa non-null (`!`).
