...
メニューの上端から `CAMPER CAFE` というテキストまでのスペースと、メニューの下端から住所までのスペースが異なっています。 これはブラウザが `h1` 要素にデフォルトの上部マージンの値を設定していることが原因です。

`h1` 要素の上部マージンの値を `0` に変更して上部マージンをすべて削除してください。

# --hints--

`margin-top` プロパティを `0` に設定する必要があります。
...
`h1` 要素には値が `0` の `margin-top` が必要です。
