...
숫자와 `dollars` 텍스트 사이에 자리값을 잡으세요.

숫자와 `|` 사이에 `hundred`, `thousand`, `million`, 또는 `billion`가 올 수 있도록 `dollars` 토큰을 사용하세요.
...
`dollarRegex`는 `|` 토큰을 사용해 `hundred`, `thousand`, `million`, 또는 `billion`와 일치해야 합니다.
