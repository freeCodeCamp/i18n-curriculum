...
Los números lúdicos están relacionados con los números primos ya que se generan mediante un tamiz muy parecido al Sieve of Eratosthenes que se usa para generar números primos.

El primer número lúdico es 1.

Para generar números lúdicos sucesivos, crea un arreglo de enteros crecientes comenzando desde 2.
...
Escribe una función que devuelva todos los números lúdicos menores o iguales al número dado.

# --hints--

`ludic` debería ser una función.

```js
assert(typeof ludic === 'function', '<code>ludic</code> should be a function.');
```

`ludic(2)` debería devolver un array.

```js
assert(Array.isArray(ludic(2)));
```

`ludic(2)` debería devolver `[1, 2]`.

```js
assert.deepEqual(ludic(2), [1, 2]);
```

`ludic(3)` debería devolver `[1, 2, 3]`.

```js
assert.deepEqual(ludic(3), [1, 2, 3]);
```

`ludic(5)` debería devolver `[1, 2, 3, 5]`.

```js
assert.deepEqual(ludic(5), [1, 2, 3, 5]);
```

`ludic(20)` debería devolver `[1, 2, 3, 5, 7, 11, 13, 17]`.

```js
assert.deepEqual(ludic(20), [1, 2, 3, 5, 7, 11, 13, 17]);
```

`ludic(26)` debería devolver `[1, 2, 3, 5, 7, 11, 13, 17, 23, 25]`.
