...
在 `getElement` 函数内部，使用带有泛型的 `querySelector` 并将结果保存到一个变量中。添加一个 `if` 语句来查看该元素是否不存在，如果不存在，则抛出带有消息 `Element not found: ${error message here}` 的误差。如果元素存在，则返回它。

# --hints--

你应该在 `el` 函数内部声明一个名为 `getElement` 的变量。
...
你的 `el` 变量应该赋值为 `document.querySelector<T>(selector)`。
