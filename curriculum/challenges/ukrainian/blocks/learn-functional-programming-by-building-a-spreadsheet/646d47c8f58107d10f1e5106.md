...
Першим аргументом виклику `evalFormula` має бути вміст клітинки (який ви зберегли у `value`). Однак для запуску функції вміст починається з символу `=`, тому потрібно передати підрядок з `value`, починаючи з індексу `1`.

# --hints--

Передайте `value` як перший аргумент до виклику `evalFormula()`.
...
Викличте метод `.slice()` на аргументі `value`.
...
Передайте число `1` як аргумент до виклику `.slice()`.
