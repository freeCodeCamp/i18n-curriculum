...
Die `SHA-2`-Familie ist eine stärkere Alternative zu `SHA-1`. Der Hauptunterschied zwischen ihnen ist die Länge des Hashs. Das bedeutet, `SHA-1` liefert einen kürzeren Code mit weniger Möglichkeiten für einzigartige Kombinationen. `SHA-2` oder `SHA-256` erzeugt einen längeren und damit komplexeren Hash mit mehr Möglichkeiten.

# --instructions--

Recherchieren Sie Implementierungsdetails und schreiben Sie eine Funktion, die einen String als Parameter nimmt und einen Hash mit `SHA-256` zurückgibt.
