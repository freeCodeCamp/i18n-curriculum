...
接著，使用 `classList` 屬性和 `add()` 方法，為你的 `playButton` 元素添加 `"playing"` 類別。這將會在 CSS 檔案中尋找 `"playing"` 類別並將其添加到 `playButton` 元素。

要最終播放歌曲，請對 `audio` 變數使用 `play()` 方法。`play()` 是用於播放 mp3 檔案的網頁音訊 API 中的方法。

# --hints--

你應該使用 `classList` 屬性和 `add` 方法，為你的 `playButton` 添加 `"playing"` 類別。

```js
assert.match(code, /playButton\.classList\.add\(\s*("|')playing\1\s*\)\s*;?/)
```

你應該在 `audio` 變數上使用 `play()` 方法。
