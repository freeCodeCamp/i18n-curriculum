...
Kipengele cha kwanza cha `input` chenye `type` ya `submit` kinawekwa moja kwa moja kuwasilisha kipengele cha `form` cha mzazi kilicho karibu zaidi.
...
Unapaswa kumpa kipengele cha `input` sifa ya `type` yenye thamani ya `submit`.

```js
assert.exists(document.querySelector('fieldset + input[type="submit"]'));
```

Unapaswa kumpa kipengele cha `input` sifa ya `value` yenye thamani ya `Submit`.
