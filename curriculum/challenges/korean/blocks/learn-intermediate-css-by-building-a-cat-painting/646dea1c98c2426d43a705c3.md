...
왼쪽 안쪽 눈을 `position` 속성에 `absolute` 프로프, `top`의 `8px`, 그리고 `left`의 `2px`로 위치시키세요. 또한, `60%`의 테두리 반경을 주고 `10`도 회전시키세요.

# --hints--

`.cat-left-inner-eye` 선택자에 `position` 속성을 `absolute`으로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.position === 'absolute')
```

`.cat-left-inner-eye` 선택자에 `top` 속성을 `8px`으로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.top === '8px')
```

`.cat-left-inner-eye` 선택자에 `left` 속성을 `2px`으로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.left === '2px')
```

`.cat-left-inner-eye` 선택자에 `border-radius` 속성을 `60%`으로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.borderRadius === '60%')
```

`.cat-left-inner-eye` 선택자에 `transform` 속성을 `10deg`으로 설정하세요.
