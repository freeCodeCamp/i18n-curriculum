...
Crea un selector `.social-icons`. Dale una propiedad `display` establecida en `grid` y una propiedad `font-size` establecida en `3rem`.
...
Tu selector `.social-icons` debe tener una propiedad `display` establecida en `grid`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

Tu selector `.social-icons` debe tener una propiedad `font-size` establecida en `3rem`.
