...
Imposta la proprietà `grid-template-columns` del selettore `.image-wrapper` su `2fr 1fr` e la proprietà `grid-template-rows` su `repeat(3, min-content)`. Questo farà sì che le righe della griglia si adattino in altezza in base al contenuto, mentre le colonne manterranno una larghezza fissa basata sul contenitore.
