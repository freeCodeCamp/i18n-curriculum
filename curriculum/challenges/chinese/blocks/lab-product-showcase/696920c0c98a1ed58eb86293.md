...
1. 你应该定义一个名为 `Book` 的接口，继承自 `Item`。它的 `type` 属性应设置为字面值 `"book"`。它还应有类型为 `title` 和 `author` 的 `string` 属性。

1. 你应该定义一个名为 `Electronics` 的接口，继承自 `Item`。它的 `type` 属性应设置为字面值 `"electronics"`。它还应有类型为 `item` 和 `model` 的 `string` 属性，以及一个可选的类型为 `warranty` 的 `number` 属性。

1. 你应该定义一个名为 `Clothing` 的接口，继承自 `Item`。它的 `type` 属性应设置为字面值 `"clothing"`。它还应有类型为 `item` 和 `brand` 的 `string` 属性，以及一个可选的 `size` 属性，该属性只能是 `"S"`、`"M"` 或 `"L"`。
