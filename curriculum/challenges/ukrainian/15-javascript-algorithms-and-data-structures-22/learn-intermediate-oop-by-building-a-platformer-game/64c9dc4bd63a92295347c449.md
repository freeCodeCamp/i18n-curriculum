...
Потім, додайте інструкцію `else if`, щоб перевірити, чи була натиснута ліва клавіша та, чи значення `isCheckpointCollisionDetectionActive` є істинним.

У цю умову додайте цикл `forEach` для ітерування над масивом `platforms`.

В межах циклу використайте оператор присвоєння додавання, щоб додати 5 до позиції `x` платформи.

# --hints--

Ви повинні мати умову, яка перевіряє, чи була натиснута ліва клавіша та, чи значення `isCheckpointCollisionDetectionActive` є істинним.
...
Додайте цикл `forEach`, який ітерує над масивом `platforms`.

```js
assert.match(code, /else\s+if\s*\(.*\)\s*{\s*platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

Використайте оператор присвоєння додавання, щоб додати 5 до позиції `x` платформи.
