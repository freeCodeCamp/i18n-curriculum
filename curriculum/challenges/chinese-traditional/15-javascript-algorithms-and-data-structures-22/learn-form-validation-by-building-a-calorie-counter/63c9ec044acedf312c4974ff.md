...
循環完成後，你需要清除 `budgetNumberInput`。 將 `budgetNumberInput` 的 `value` 屬性設置爲空字符串。

# --hints--

你的 `clearForm` 函數應該訪問 `budgetNumberInput` 元素的 `value` 屬性。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value/);
```

你的 `clearForm` 函數應該將 `budgetNumberInput` 元素的 `value` 屬性設置爲空字符串。

```js
assert.match(clearForm.toString(), /budgetNumberInput\.value\s*=\s*('|"|`)\1/);
```

你應該在 `for` 循環之後修改 `budgetNumberInput`。
