...
Ви повинні надати `.eye` властивість `top` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.top, '45%');
```

Ви повинні надати `.eye` властивість `border-radius` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.
