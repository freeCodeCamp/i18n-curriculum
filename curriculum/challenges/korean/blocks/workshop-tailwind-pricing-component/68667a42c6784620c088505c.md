...
`Go Premium` CTA를 제외하고 나머지 콘텐츠는 `div` 안에 있습니다. 그래서 CSS grid를 사용해 모두 배치하는 것이 좋은 선택입니다.

`grid`, `grid-rows-[auto_auto_auto_1fr]`, `gap-y-2` 요소를 감싸는 `div`에 `h2`, `p`, `ul` 클래스를 추가하세요.

# --hints--

`div`, `h2`, `p` 요소를 감싸는 `ul` 요소에 `grid` 클래스를 추가해야 합니다.
...
`div`, `h2`, `p` 요소를 감싸는 `ul` 요소에 `grid-rows-[auto_auto_auto_1fr]` 클래스를 추가해야 합니다.
...
`div`, `h2`, `p` 요소를 감싸는 `ul` 요소에 `gap-y-2` 클래스를 추가해야 합니다.
