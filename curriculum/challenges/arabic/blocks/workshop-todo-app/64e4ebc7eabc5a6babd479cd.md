...
إذا نقر المستخدم على زر `Discard`، تريد إغلاق النافذة المنبثقة التي تعرض زري `Cancel` و`Discard`، ثم إخفاء نافذة النموذج المنبثقة.

أضف مستمع حدث نقر إلى `discardBtn`، ثم استخدم الطريقة `close()` على المتغير `confirmCloseDialog`. أيضًا، استخدم `classList` لتبديل الصنف `hidden` على `taskForm` حتى تُغلق نافذة النموذج المنبثقة أيضًا.
