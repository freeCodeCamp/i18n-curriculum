...
O primeiro passo para analisar um cartão de catálogo é separar os quatro campos. O método `.split()` divide uma string em um array de substrings a cada ocorrência do separador que você fornecer.
...
Dentro de `parseCard`, chame `.split("|")` em `rawString` e retorne o resultado. Após esse passo, o console deve mostrar um array com quatro strings.

**Observação:** Você pode notar espaços em branco extras ao redor dos valores. Você vai limpar isso no próximo passo.

# --hints--

Você deve chamar `.split("|")` em `rawString` dentro da sua função `parseCard`.

```js
assert.match(parseCard.toString(), /rawString\s*\.\s*split\s*\(\s*('|"|`)\|\1\s*\)\s*;?/);
```

`parseCard` deve retornar um array.

```js
assert.isArray(parseCard("A | B | C | D"));
```

Quando chamado com `"A | B | C | D"`, `parseCard` deve retornar um array com quatro elementos.
