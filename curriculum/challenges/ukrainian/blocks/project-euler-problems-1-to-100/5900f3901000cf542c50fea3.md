...
title: 'Завдання 36: подвійні паліндроми'
challengeType: 1
forumTopicId: 302020
...
Десяткове число 585 = 1001001001<sub>2</sub> (двійкове) є паліндромним у обох системах числення.

Знайдіть суму всіх чисел, менших за `n`, де 1000 ≤ `n` ≤ 1000000, які є паліндромними у десятковій та двійковій системах числення.

(Зверніть увагу, що паліндромне число в будь-якій системі не може містити провідних нулів.)

# --hints--

`doubleBasePalindromes(1000)` має повертати число.

```js
assert(typeof doubleBasePalindromes(1000) === 'number');
```

`doubleBasePalindromes(1000)` має повертати 1772.

```js
assert(doubleBasePalindromes(1000) == 1772);
```

`doubleBasePalindromes(50000)` має повертати 105795.

```js
assert(doubleBasePalindromes(50000) == 105795);
```

`doubleBasePalindromes(500000)` має повертати 286602.

```js
assert(doubleBasePalindromes(500000) == 286602);
```

`doubleBasePalindromes(1000000)` має повертати 872187.
