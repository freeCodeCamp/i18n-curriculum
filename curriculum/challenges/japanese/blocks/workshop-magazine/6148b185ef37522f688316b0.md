...
これで `.heading` 要素の内容を CSS Grid でスタイル設定できます。

CSS の `repeat()` 関数は値を手動で繰り返す代わりに使い、グリッドレイアウトで役立ちます。たとえば、`grid-template-columns` プロパティを `repeat(20, 200px)` に設定すると、幅が `200px` の列が20個作成されます。

`.heading` 要素に `grid-template-columns` プロパティを `repeat(2, 1fr)` に設定して、同じ幅の2列を作成してください。

# --hints--

`.heading` セレクターに `grid-template-columns` プロパティを `repeat(2, 1fr)` に設定してください。
