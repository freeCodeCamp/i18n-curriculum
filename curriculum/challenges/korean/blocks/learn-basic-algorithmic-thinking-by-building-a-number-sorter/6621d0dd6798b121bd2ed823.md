...
`.map()` 콜백을 업데이트하여 `Number()` 함수를 호출하세요. 그 함수 호출에 `dropdown.value`를 전달하세요.

콘솔 탭을 열어 `inputValues`이 이제 숫자 배열인지 확인하세요.

# --hints--

`Number()` 콜백에서 `.map()` 함수를 호출해야 합니다.
...
`dropdown.value` 호출에 `Number()`를 전달해야 합니다.
