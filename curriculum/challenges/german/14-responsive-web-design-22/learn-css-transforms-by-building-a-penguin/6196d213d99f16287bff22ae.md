...
Du solltest ein neues `div`-Element zum `body` hinzufügen. Es sind `--fcc-expected--` `div`-Elemente erwartet worden, aber es wurden `--fcc-actual--` gefunden.

```js
assert.equal(document.querySelectorAll('body > div')?.length, 5);
```

Du solltest dem neuen `div`-Element eine `class` mit dem Wert `sun` zuweisen.

```js
assert.include(document.querySelector('div:not(.back-mountain, .left-mountain, .penguin, .ground)')?.className, 'sun');
```

Du solltest das neue `div`-Element direkt hinter das `.back-mountain`-Element platzieren.
