...
設置一個對應 `max-width` 爲 `720px` 的 `only screen` 的 `@media` 查詢。 在裏面，創建一個 `.image-wrapper` 選擇器，並給它一個值爲 `1fr` 的 `grid-template-columns` 屬性。

這將在較小的屏幕上將三個圖像摺疊成一列。

# --hints--

你應該有一個新的 `@media` 查詢，對應 `only screen and (max-width: 720px)`。
...
你的新 `.image-wrapper` 選擇器應該具有值爲 `1fr` 的 `grid-template-columns` 屬性。
