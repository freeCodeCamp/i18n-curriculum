...
`.fb2a` に `width` の `100%` を、`.fb2b` に `width` の `100%` と `height` の `75%` を与えてください。

# --hints--

`.fb2a` に `width` の `100%` を与える必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.width, "100%");
```

`.fb2b` に `width` の `100%` を与える必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2b")?.width, "100%");
```

`.fb2b` に `height` の `75%` を与える必要があります。
