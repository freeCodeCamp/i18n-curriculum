...
<dfn>Модальний</dfn> — це елемент, який запобігає будь-якій взаємодії з елементами поза ним, доки модальний не буде відхилено.
...
Викличте метод `addEventListener()` на змінній `closeTaskFormBtn`.
...
Функція зворотного виклику слухача подій має викликати метод `showModal()` на елементі `confirmCloseDialog`.
