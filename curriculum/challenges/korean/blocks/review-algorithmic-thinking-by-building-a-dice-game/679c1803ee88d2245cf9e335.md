...
마지막으로, 약간의 리팩토링으로 코드를 정리하세요! `getHighestDuplicates`, `detectFullHouse`, `checkForStraights` 함수 내에서 마지막 라디오 버튼("None of the Above")을 여러 번 활성화한 것을 알 수 있을 겁니다. 각 함수 끝에서 마지막 라디오 버튼을 활성화하는 대신, `rollDiceBtn` 안에서 한 번만 활성화하도록 코드를 리팩토링하세요.

# --hints--

`getHighestDuplicates` 함수는 중복이 나와도 마지막 라디오 버튼을 활성화하거나 마지막 `span` 텍스트를 설정하지 않아야 합니다.
...
`detectFullHouse` 함수는 풀 하우스가 나와도 마지막 라디오 버튼을 활성화하거나 마지막 `span` 텍스트를 설정하지 않아야 합니다.
...
`checkForStraights` 함수는 스트레이트가 나와도 마지막 라디오 버튼을 활성화하거나 마지막 `span` 텍스트를 설정하지 않아야 합니다.
...
`0`가 클릭되면 마지막 라디오 버튼을 활성화하고, 값을 `, score = 0`으로 설정하며, 표시된 텍스트를 `rollDiceBtn`로 업데이트해야 합니다.
