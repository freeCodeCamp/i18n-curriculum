...
Ora aggiungerai la logica per salvare su `localStorage` usando `useEffect`.

Consultare o salvare dati in `localStorage` è considerato un effetto collaterale in React, quindi dovresti usare `useEffect` per questa logica.

Inizia destrutturando `useEffect` da React, proprio come è stato fatto con `useState`.

# --hints--

Dovresti destrutturare `useEffect` da `React`.
