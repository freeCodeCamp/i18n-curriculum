...
接下来，你要在水平方向上将 `#menu` 居中。 You can do this by setting its `margin-left` and `margin-right` properties to `auto`. 可以把页边距看作是元素周围不可见的空间。 Using these two margin properties, center the `#menu` element within the `body` element.
