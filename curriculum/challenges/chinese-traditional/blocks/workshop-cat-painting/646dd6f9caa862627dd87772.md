...
將內耳移動到位，其 `position` 屬性設置爲 `absolute`，`top` 爲 `22px`，`left` 爲 `-20px`。

# --hints--

你的 `.cat-left-inner-ear` 選擇器應該有一個 `position` 屬性，設置爲 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.position, 'absolute')
```

你的 `.cat-left-inner-ear` 選擇器應該具有一個 `top` 屬性，設置爲 `22px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.top, '22px')
```

你的 `.cat-left-inner-ear` 選擇器應該有一個 `left` 屬性，設置爲 `-20px`。
