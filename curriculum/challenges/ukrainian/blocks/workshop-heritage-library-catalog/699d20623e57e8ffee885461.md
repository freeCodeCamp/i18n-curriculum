...
Такий самий шаблон застосовується до інших трьох полів. Додайте еквівалентні перевірки для `author`, `year` і `location`.

Зверніть увагу, що `isValid` накопичується у всіх чотирьох перевірках: якщо будь-яке поле не проходить, воно стає `false` і залишається `false`, навіть якщо інші перевірки проходять. Це означає, що функція завжди перевіряє кожне поле, а не зупиняється на першій проблемі.

# --hints--

`validateEntry(catalog[0])` має повертати `true`.

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` має повертати `false`.

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` має повертати `false`.

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` має повертати `false`, тому що розташування — `"Unknown"`.
