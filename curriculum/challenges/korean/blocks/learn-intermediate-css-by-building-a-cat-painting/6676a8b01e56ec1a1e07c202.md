...
이제 두꺼운 테두리가 있는 흰색 사각형이 생긴 것을 확인하세요. 테두리는 대각선 모서리를 가지고 있어 삼각형을 만드는 데 사용할 수 있습니다.
...
`.cat-right-ear` 선택자에 `height` 속성이 `0`로 설정되어 있어야 합니다. 세미콜론을 잊지 마세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.height == '0px')
```

`.cat-right-ear` 선택자에 `width` 속성이 `0`로 설정되어 있어야 합니다. 세미콜론을 잊지 마세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.width === '0px')
```

`.cat-right-ear` 선택자에 `border-left` 속성이 `35px solid transparent`로 설정되어 있어야 합니다. 세미콜론을 잊지 마세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderLeft === '35px solid transparent')
```

`.cat-right-ear` 선택자에 `border-right` 속성이 `35px solid transparent`로 설정되어 있어야 합니다. 세미콜론을 잊지 마세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderRight === '35px solid transparent')
```

`.cat-right-ear` 선택자에 `border-top` 속성이 `35px solid transparent`로 설정되어 있어야 합니다. 세미콜론을 잊지 마세요.
