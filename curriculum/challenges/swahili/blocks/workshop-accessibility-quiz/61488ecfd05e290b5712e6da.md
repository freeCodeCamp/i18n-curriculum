...
Ili kufanya sehemu ya kwanza ionekane zaidi ndani ya mstari, lenga tu vipengele vya `input` ndani ya vipengele vya `.info`, na weka `width` yao kuwa `50%`, na panga maandishi yao upande wa kushoto.

# --hints--

Unapaswa kutumia kichaguzi cha `.info input` au `.info > input`.
...
Unapaswa kuwapa vipengele vya `input` `width` ya `50%`.
...
Unapaswa kuwapa vipengele vya `input` `text-align` ya `left`.
