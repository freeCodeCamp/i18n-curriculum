...
Тепер, коли макет журналу завершено, потрібно зробити його адаптивним.

Почніть із `@media` запиту для `only screen` з `max-width` рівним `720px`. Всередині створіть селектор `.image-wrapper` і надайте йому властивість `grid-template-columns` зі значенням `1fr`.

Це згорне три зображення в один стовпець на менших екранах.

# --hints--

У вас має бути нове правило `@media` для `only screen and (max-width: 720px)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

Ваше нове правило `@media` має містити селектор `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

Ваш новий селектор `.image-wrapper` має мати властивість `grid-template-columns` зі значенням `1fr`.
