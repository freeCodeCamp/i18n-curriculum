...
是時候為雜誌的最後一節 - 影像 - 進行樣式設計。

影像使用帶有 `image-wrapper` 類別的 `aside` 元素包裝，因此請創建 `.image-wrapper` 選擇器。將 `display` 屬性設為 `grid`。

# --hints--

你應該有一個 `.image-wrapper` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

你的 `.image-wrapper` 選擇器應該有一個設定為 `grid` 的 `display` 屬性。
