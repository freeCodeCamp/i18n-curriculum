...
الـ `input` التالي هو مربع اختيار لتحديد كل قوى الأبطال الخارقين التي تنطبق على المستخدم.

أنشئ عنصر `label` مع `className` بقيمة `section` و `column`، والنص `List your powers (select all that apply):`.

# --hints--

يجب أن تنشئ عنصر `label` مع `className` بقيمة `section` و `column`.
...
يجب أن يحتوي عنصر `label` الخاص بك على النص `List your powers (select all that apply):`.
