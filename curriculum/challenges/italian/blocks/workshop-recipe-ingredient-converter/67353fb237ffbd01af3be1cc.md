...
Finalmente puoi iniziare a collegare la tua logica all'interfaccia utente. Dovrai fare una query del DOM per ottenere alcuni elementi tramite il loro id. Prendi gli elementi con gli id `ingredient`, `quantity`, `unit`, `servings`, `recipe-form` e `result-list` e assegnali rispettivamente alle variabili `ingredientName`, `ingredientQuantity`, `unitToConvert`, `numberOfServings`, `recipeForm` e `resultList`.

# --hints--

Dovresti definire una variabile `ingredientName`.

```js
assert.isDefined(ingredientName);
```

La tua variabile `ingredientName` dovrebbe essere l'elemento restituito da `.getElementById()` con l'id `ingredient`.

```js
assert.strictEqual(ingredientName, document.getElementById('ingredient'));
```

Dovresti definire una variabile `ingredientQuantity`.

```js
assert.isDefined(ingredientQuantity);
```

La tua variabile `ingredientQuantity` dovrebbe essere l'elemento restituito da `.getElementById()` con l'id `quantity`.

```js
assert.strictEqual(ingredientQuantity, document.getElementById('quantity'));
```

Dovresti definire una variabile `unitToConvert`.

```js
assert.isDefined(unitToConvert);
```

La tua variabile `unitToConvert` dovrebbe essere l'elemento restituito da `.getElementById()` con l'id `unit`.

```js
assert.strictEqual(unitToConvert, document.getElementById('unit'));
```

Dovresti definire una variabile `numberOfServings`.

```js
assert.isDefined(numberOfServings);
```

La tua variabile `numberOfServings` dovrebbe essere l'elemento restituito da `.getElementById()` con l'id `servings`.

```js
assert.strictEqual(numberOfServings, document.getElementById('servings'));
```

Dovresti definire una variabile `recipeForm`.

```js
assert.isDefined(recipeForm);
```

La tua variabile `recipeForm` dovrebbe essere l'elemento restituito da `.getElementById()` con l'id `recipe-form`.

```js
assert.strictEqual(recipeForm, document.getElementById("recipe-form"));
```

Dovresti definire una variabile `resultList`.

```js
assert.isDefined(resultList);
```

La tua variabile `resultList` dovrebbe essere l'elemento restituito da `.getElementById()` con l'id `result-list`.
