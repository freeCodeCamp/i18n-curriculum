...
现在你有产品列表了，你可以使用 JavaScript 将它们插入 HTML。 通过这种方法，如果你决定添加更多产品，HTML 将自动更新。

首先调用 `products` 数组的 `.forEach` 方法。 使用箭头函数语法创建一个空的回调函数。

# --hints--

你应该调用 `products` 数组的 `.forEach` 方法。

```js
assert.match(code, /products\.forEach\(/);
```

你应该使用箭头函数语法来创建空的回调函数。

```js
assert.match(code, /\(\s*\)\s*=>\s*\{\s*\}/)
```

你应该将空的回调函数传入 `.forEach` 方法中。
