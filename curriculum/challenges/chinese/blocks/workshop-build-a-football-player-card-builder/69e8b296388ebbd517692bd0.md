...
在 `form-row` 之后，添加一个 `div`，其 `className` 为 `form-group`。在其中，添加一个 `label` 元素，其 `className` 为 `label`，`htmlFor` 为 `club`，以及文本 `Club`。然后添加一个输入元素，其 `id` 为 `club`，`className` 为 `input`，以及 `type` 为 `text`。
...
你的 `div.form-group` 应该有一个 `label`，带有 `htmlFor` 属性，属性值为 `club`，`className` 为 `label`，以及文本 `Club`。
...
你的 Club `form-group` 应该包含一个 `input`，其 `id` 为 `club`，`className` 为 `input`，以及 `type` 为 `text`。
