...
title: Tweet埋め込みでalign-itemsプロパティを使う
challengeType: 0
...
前回のチャレンジでは`align-items`プロパティを紹介し、例を示しました。このプロパティは、いくつかのツイート埋め込み要素に適用して、その中のフレックスアイテムを整列させることができます。

# --instructions--

ヘッダーの`align-items`要素にCSSプロパティ`.follow-btn`を追加してください。値は`center`に設定してください。
...
`.follow-btn`要素には、`align-items`プロパティを`center`の値で設定してください。
