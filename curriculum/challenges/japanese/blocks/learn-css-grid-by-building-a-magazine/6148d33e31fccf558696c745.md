...
引用文のテキストをもっと目立たせるため、`.quote` セレクターの `font-family` プロパティを `Raleway` に設定し、フォールバックとして `sans-serif` を指定してください。

# --hints--

`.quote` セレクターの `font-family` プロパティには `Raleway` と、フォールバックとして `sans-serif` を設定する必要があります。
