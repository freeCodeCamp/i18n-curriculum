...
أنشئ محدِّد `a`، وأعطه خاصية `text-decoration` بقيمة `none` وخاصية `color` بقيمة `linen`.

# --hints--

يجب أن يكون لديك محدِّد `a`.

```js
assert(new __helpers.CSSHelp(document).getStyle('a'));
```

يجب أن يحتوي محدِّد `a` على خاصية `text-decoration` بقيمة `none`.

```js
assert(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration === 'none');
```

يجب أن يحتوي محدِّد `a` على خاصية `color` بقيمة `linen`.
