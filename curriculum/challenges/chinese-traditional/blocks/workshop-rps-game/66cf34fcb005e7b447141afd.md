...
創建一個名為 `showResults` 的函式（程式），並帶有一個名為 `userOption` 的參數。

在你的 `showResults` 函式中，`roundResultsMsg` 應該要更新為該回合的結果。

然後，`playerScoreSpanElement` 和 `computerScoreSpanElement` 也應該更新，以顯示玩家和電腦的更新分數。

請記住，這裡的順序很重要。你需要先更新 `roundResultsMsg`，然後是 `playerScoreSpanElement`，最後是 `computerScoreSpanElement`，因為 `roundResultsMsg` 將用來決定分數。

# --hints--

你應該有一個名為 `showResults` 的函式（程式）。

```js
assert.isFunction(showResults);
```

你的 `showResults` 函式應該有一個名為 `userOption` 的參數。

```js
assert.match(code, /showResults\s*(?:=\s*)?(?:\(\s*)?userOption(?:\s*\))?\s*/);
```

你的 `showResults` 函式（程式）應該使用該回合的結果更新 `roundResultsMsg`。
...
你的 `showResults` 函式（程式）應該更新 `computerScoreSpanElement` 以顯示電腦的更新分數。
...
你的 `showResults` 函式應該更新 `playerScoreSpanElement` 以顯示玩家的更新分數。
