...
小さい画面では、ナビゲーションバーの番号なしリストが画面の右側からはみ出しています。

_Flexbox_ を使って `ul` の内容を折り返すように修正してください。その後、テキストを正しく揃えるために次の CSS プロパティを設定してください。
