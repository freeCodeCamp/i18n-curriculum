...
次は、`o` という文字を `o` および (数字の) `0` にマッチするように変更してください。

# --hints--

正規表現 `freeRegex` では、文字クラスを使用して `o` および `0` にマッチさせてください。

```js
assert.match(freeRegex.source, /\[(o0|0o)\]/);
```

正規表現 `freeRegex` は `free money` とマッチする必要があります。

```js
assert.match('free money', freeRegex);
```

正規表現 `freeRegex` は `fr33 m0n3y` とマッチする必要があります。
