...
في بعض الحالات، يجب عكس البطاقات، إذ إنها ليست دائمًا في الموضع الصحيح.

أعلن عن متغير اسمه `isReversed` داخل الطريقة `singleCardSelected`. استخدم الطريقة `Math.random()` لتوليد قيمة منطقية عشوائية. لضمان بقاء القيمة ضمن الحدود، قارنها بـ `0.5`.
