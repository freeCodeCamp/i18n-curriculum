...
title: تطبيق خاصية flex-direction لإنشاء صفوف في تضمين التغريدة
challengeType: 0
...
يحتوي `header` و`footer` في مثال تضمين التغريدة على عناصر فرعية يمكن ترتيبها كصفوف باستخدام خاصية `flex-direction`. هذا يُخبر CSS بمحاذاة العناصر الفرعية أفقيًا.

# --instructions--

أضف خاصية CSS `flex-direction` إلى كل من `header` و`footer` واضبط القيمة على `row`.

# --hints--

يجب عرض `.follow-btn` على الصفحة. تأكد من تعطيل أي إضافات مثل مانعات الإعلانات.
...
يجب أن يحتوي `header` على خاصية `flex-direction` بقيمة `row`.

```js
assert.match(code, /header\s*?{[^}]*?flex-direction:\s*?row;/g);
```

يجب أن يحتوي `footer` على خاصية `flex-direction` مضبوطة على القيمة `row`.
