...
由于大量冗余计算，此实现具有指数时间复杂度。计算 `climbStairsRecursive(5)` 时，发生了以下情况：

- `climbStairsRecursive(5)` 调用 `climbStairsRecursive(4)` 和 `climbStairsRecursive(3)`
    
- `climbStairsRecursive(4)` 调用 `climbStairsRecursive(3)` 和 `climbStairsRecursive(2)`
    
- 现在 `climbStairsRecursive(3)` 被计算了 **两次**
    
- `climbStairsRecursive(3)` 调用 `climbStairsRecursive(2)` 和 `climbStairsRecursive(1)`
    
- `climbStairsRecursive(2)` 总共被计算了 **3 次**
    

仅对于 `n=5`，我们进行了 9 次函数调用，而实际上只需要 5 次唯一计算。随着 `n` 的增长，这种冗余呈指数级爆炸——`climbStairsRecursive(30)` 需要超过几百万次函数调用！时间复杂度变为 `O(2^n)`，使其对于较大 `n` 值来说既低效又不切实际。
...
记忆化非常高效，因为从 `1` 到 `n` 的每个唯一值只计算一次。当我们再次需要 `climbStairsMemo(3)` 时，不是重新计算它（这会触发更多递归调用），而是直接在我们的备忘录字典中以 `O(1)` 时间查找。

让我们通过自顶向下的方法追踪 `climbStairsMemo(5)` 的执行，看看记忆化如何消除冗余工作：
...
- **真正的影响**：`climbStairsMemo(30)` 从数百万次函数调用减少到大约 30 次唯一计算！
...
让我们看看自底向上的方法如何系统地构建方案。以下是 `climbStairsTabulation(5)` 的迭代构造：
