...
Agrega la etiqueta `<!DOCTYPE html>` y un elemento `html` con un atributo `lang` de `en`.

# --hints--

Debes tener la declaración `<!DOCTYPE html>`.

```js
assert(code.match(/<!DOCTYPE html>/i));
```

Debes tener una apertura `<html lang="en">` con el idioma configurado en inglés.
