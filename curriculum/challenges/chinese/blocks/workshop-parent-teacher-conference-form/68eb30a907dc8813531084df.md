...
默认情况下，像单选按钮这样的表单元素可能不会与它们的标签完全对齐，尤其是当标签文本较高或具有不同字体设置时。

要使单选按钮与其标签垂直对齐，你可以使用 `vertical-align` 属性。该属性控制内联或内联块元素与周围文本的垂直排列方式。

现在，将 `vertical-align` 设置为 `bottom` 用于单选按钮。你可以尝试其他值，比如 `middle` 或 `top`，以查看它们如何影响对齐，但 `bottom` 通常适用于标签旁边的单选按钮。

# --hints--

你应该继续定位 `.contact-method-radio-btn` 类。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

将 `vertical-align` 属性设置为 `bottom`。
