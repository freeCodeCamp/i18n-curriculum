...
Por fim, atualize a propriedade `display` do objeto `style` da variável `cartContainer` para que tenha outro ternário que verifica se `isCartShowing` é verdadeiro. Se for, defina a propriedade `display` como `"block"`, caso contrário defina-a como `"none"`.
...
Você deve definir a propriedade `display` da propriedade `style` da variável `cartContainer` para `"block"` se `isCartShowing` for verdadeiro.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*isCartShowing\s*\?\s*('|"|`)block\1\s*:\s*/)
```

Você deve definir a propriedade `display` da propriedade `style` da variável `cartContainer` para `"none"` se `isCartShowing` for falso.
