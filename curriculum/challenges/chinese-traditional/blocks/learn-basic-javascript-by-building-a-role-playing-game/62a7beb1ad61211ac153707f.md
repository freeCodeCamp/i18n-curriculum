...
現在，添加一行程式碼將 `button2` 的文字更新為 `"Buy weapon (30 gold)"`，並將 `button3` 的文字更新為 `"Go to town square"`。

# --hints--

你應該使用點記法來存取 `button2` 的 `innerText` 屬性。

```js
assert.match(code, /button2\.innerText/);
```

你不應該使用 `let` 或 `const` 來存取 `button2` 的 `innerText` 屬性。

```js
assert.notMatch(code, /(let|const)\s+button2\.innerText/);
```

你應該將 `button2` 的 `innerText` 屬性更新為 `"Buy weapon (30 gold)"`。
...
你應該使用點記法來存取 `button3` 的 `innerText` 屬性。

```js
assert.match(code, /button3\.innerText/);
```

你不應該使用 `let` 或 `const` 來存取 `button3` 的 `innerText` 屬性。

```js
assert.notMatch(code, /(let|const)\s+button3\.innerText/);
```

你應該將 `button3` 的 `innerText` 屬性更新為 `"Go to town square"`。
