...
كما ترى، أصبح المجموع الآن فارغًا تمامًا. احذف استدعائي `clear` و `has` من شفرتك.

# --hints--

يجب ألا يحتوي شفرتك على `plantsSet.clear()`.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

يجب ألا يحتوي شفرتك على `plantsSet.has(ballerina)`.
