...
Nun ist es an der Zeit, deine dritte `section` zu gestalten. Beachte, dass es die `text` und `text-with-images`-Werte für die `class` enthält, was bedeutet, dass es bereits die Stile deiner `.text`-Regel erbt.
