...
Por ejemplo, un `Car` es un tipo de `Vehicle`, por lo que podrías definir una clase `Vehicle` con las propiedades y métodos más generales comunes a todos los tipos de vehículos, y luego definir una clase `Car` que hereda las propiedades y métodos de la clase `Vehicle`.

Esta relación "es un" es característica de la herencia. En este ejemplo, `Car` es una forma más especializada de `Vehicle`.
...
En nuestro ejemplo anterior, `Vehicle` sería la clase padre de la clase `Car` mientras que la clase `Car` sería la clase hija de `Vehicle`.
...
En este ejemplo, en el que solo tenemos las primeras líneas de las definiciones de clase, puedes ver que la clase `Car` extiende la clase `Vehicle`, por lo que `Car` hereda de `Vehicle`.
...
Digamos que la clase `Vehicle` tiene dos propiedades: `brand` y `year`. Puedes ver esto en el código actualizado.

`Car` tiene un método llamado `honk`. Sin embargo, observa que la clase `Car` no tiene su propio constructor. Veamos si realmente hereda las propiedades.
...
Para definir una instancia de `Car`, necesitarás pasar dos argumentos: `brand` y `year`. En este caso, la marca es `freeCodeCamp Motors` y el año es `2019`.
...
De hecho, la salida es correcta, por lo que estas propiedades y método están definidos en la instancia de `Car` y te has ahorrado mucha repetición de código al heredar estas propiedades desde la clase `Vehicle`.
...
Esta vez, agregaremos una propiedad `numDoors` a la subclase `Car`.

Puedes ver que ahora esta clase tiene un constructor en el código actualizado. Dentro del constructor, hay una llamada a `super()`, pasando los argumentos `brand` y `year`.

`super()` invoca el constructor de la superclase, por lo que al usar `super(brand, year)` estarás definiendo esencialmente las propiedades de la superclase en la subclase. Esta es otra terminología que puedes usar: superclase es la clase padre y subclase es la clase hija.

Después, la línea `this.numDoors = numDoors` define una nueva propiedad que pertenece exclusivamente a la clase `Car`, el número de puertas (`numDoors`).

Esta es una forma de extender la clase `Vehicle`, agregando una propiedad que es exclusiva para instancias de `Car`.

Eso es un ejemplo de extensibilidad, una de las ventajas fundamentales de la herencia.

Ahora, si creas una instancia de la clase `Car`, verás que tiene las tres propiedades: `brand`, `year` y `numDoors`. También tendrás que pasar los argumentos necesarios al crear la instancia.
