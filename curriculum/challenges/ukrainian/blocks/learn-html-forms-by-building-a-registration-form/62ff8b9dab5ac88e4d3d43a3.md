...
Використовуйте `first-name`, `last-name`, `email` та `new-password` як значення для відповідних атрибутів `id`.

# --hints--

Перший елемент `input` повинен мати `id` `first-name`.

```js
assert(document.querySelectorAll('input')?.[0]?.matches('#first-name'))
```

Другий елемент `input` повинен мати `id` `last-name`.

```js
assert(document.querySelectorAll('input')?.[1]?.matches('#last-name'))
```

Третій елемент `input` повинен мати `id` ідентифікатор `email`.

```js
assert(document.querySelectorAll('input')?.[2]?.matches('#email'))
```

Четвертий елемент `input` повинен мати `id` `new-password`.
