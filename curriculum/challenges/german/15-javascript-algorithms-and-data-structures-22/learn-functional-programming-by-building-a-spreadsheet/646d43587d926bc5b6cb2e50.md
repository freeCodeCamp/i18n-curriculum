...
Deklariere eine `str2`-Variable und weise ihr das Ergebnis des Aufrufs von `infixEval()` mit `noHigh` und `infix` als Argumente zu.
...
Du solltest `str2` das Ergebnis des Aufrufs von `infixEval()` zuweisen.
...
Du solltest `noHigh` als erstes Argument an `infixEval()` übergeben.
...
Du solltest `infix` als zweites Argument an `infixEval()` übergeben.
