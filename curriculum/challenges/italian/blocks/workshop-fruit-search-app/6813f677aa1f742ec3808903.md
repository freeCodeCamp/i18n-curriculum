...
Per il blocco `catch`, aggiungi un parametro `error`, poi usa `console.error` per registrare qualsiasi errore che si verifichi durante il recupero o l'elaborazione dei dati.

# --hints--

Il tuo blocco `catch` dovrebbe avere un parametro `error`.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

Dovresti registrare qualsiasi errore nel blocco `catch` usando `console.error`.
