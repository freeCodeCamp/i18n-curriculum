...
`.fb2a` に `border-bottom` の `10vh solid var(--building-color3)` と、`border-left` の `border-right` および `1vw solid transparent` を追加してください。今回はボーダーのトリックで台形の形を作ります。

# --hints--

`.fb2a` に `border-bottom` の `10vh solid var(--building-color3)` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderBottom.trim(), "10vh solid var(--building-color3)");
```

`.fb2a` に `border-left` の `1vw solid transparent` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderLeft, "1vw solid transparent");
```

`.fb2a` に `border-right` の `1vw solid transparent` を設定してください。
