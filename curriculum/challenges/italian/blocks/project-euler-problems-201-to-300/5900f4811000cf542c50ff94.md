...
Una sequenza di Collatz modificata di numeri interi si ottiene da un valore iniziale $a_1$ nel modo seguente:

$a_{n + 1} = \frac{a_n}{3}$ se $a_n$ è divisibile per 3. Indicheremo questo come un grande passo verso il basso, "D".

$a_{n + 1} = \frac{4a_n + 2}{3}$ se dividendo $a_n$ per 3 si ottiene un resto di 1. Indicheremo questo come un passo verso l’alto, "U".

$a_{n + 1} = \frac{2a_n - 1}{3}$ se dividendo $a_n$ per 3 si ottiene un resto di 2. Indicheremo questo come un piccolo passo verso il basso, "d".

La sequenza termina quando qualche $a_n = 1$.

Dato un qualsiasi numero intero, possiamo elencare la sequenza dei passi. Per esempio, se $a_1 = 231$, allora la sequenza $\\{a_n\\} = \\{231, 77, 51, 17, 11, 7, 10, 14, 9, 3, 1\\}$ corrisponde ai passi "DdDddUUdDD".
