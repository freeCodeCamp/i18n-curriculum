...
Кожен `tab` представляє одну з кнопок у списку вкладок. Користувач повинен мати змогу натиснути на вкладку, щоб показати відповідну панель і приховати інші.

У попередніх модулях ви навчились приєднувати слухача подій `"click"` до кожної кнопки у `NodeList` ось так:
...
Використайте метод `addEventListener`, щоб приєднати слухача подій `"click"` до кожної кнопки `tab`. Другим аргументом методу `addEventListener` має бути порожня стрілкова функція `() => {}` поки що.

# --hints--

У вас має бути `tab.addEventListener('click', () => {})` всередині циклу `forEach`.
