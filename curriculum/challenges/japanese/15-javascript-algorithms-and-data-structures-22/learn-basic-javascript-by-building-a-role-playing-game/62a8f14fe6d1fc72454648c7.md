...
モンスターが倒されたら、そのモンスターのステータスボックスは表示されないようにしましょう。

`update` 関数の最初の行で、`monsterStats.style.display` を使用して `display` の値を `none` に変更してください。

# --hints--

`update` 関数内で `monsterStats.style.display` を使用してください。

```js
assert.match(update.toString(), /monsterStats\.style\.display/);
```

`monsterStats.style.display` を `none` に設定してください。

```js
assert.match(update.toString(), /monsterStats\.style\.display\s*=\s*('|")none\1/);
```

コードは `update` 関数の最初の行に追加してください。
