...
計算する分量の数に合わせて調整する必要があります。これは別のカリー化された関数を使う絶好の機会です。

`adjustForServings` 関数を宣言してください。この関数は `baseQuantity` 引数を受け取ります。次に、`newServings` 引数を受け取る2番目の関数を返す必要があります。

内側の関数は `baseQuantity` と `newServings` を乗算した結果を返すべきです。

# --hints--

`adjustForServings` 関数を宣言してください。

```js
assert.isFunction(adjustForServings);
```

`adjustForServings` 関数は `baseQuantity` 引数を受け取るべきです。

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

`adjustForServings` 関数は関数を返すべきです。

```js
assert.isFunction(adjustForServings?.());
```

内側の関数は `newServings` パラメータを持つべきです。
