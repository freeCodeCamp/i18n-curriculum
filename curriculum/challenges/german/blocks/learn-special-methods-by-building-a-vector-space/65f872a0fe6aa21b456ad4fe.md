...
Geben Sie schließlich den `tuple()`-Aufruf als Argument an die `str()`-Funktion weiter.

# --hints--

Sie sollten den `tuple()`-Aufruf, den Sie gerade aus der `__str__`-Methode zurückgeben, an die `str()`-Funktion übergeben.
