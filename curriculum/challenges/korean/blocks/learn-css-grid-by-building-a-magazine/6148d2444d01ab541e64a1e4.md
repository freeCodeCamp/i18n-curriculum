...
`.quote` 선택자를 만드세요. `color` 속성을 `#00beef`로, `font-size` 속성을 `2.4rem`로, `text-align` 속성을 `center`로 설정하세요.
