...
`.calories-info` 要素の中に `div` 要素を作成してください。その `div` 要素に `class` 属性を `left-container` に設定してください。新しく作成した `div` 要素の中に、テキスト `h2` を持つ `Amount per serving` 要素を作成してください。`h2` 要素に `class` 属性を `bold small-text` に設定してください。

# --hints--

`div` 要素の中に新しい `.calories-info` 要素を作成してください。

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.localName === 'div');
```

新しい `div` 要素には `class` 属性を `left-container` に設定してください。

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.classList.contains('left-container'));
```

`h2` 要素の中に新しい `.left-container` 要素を作成してください。

```js
assert(document.querySelector('.left-container')?.children?.[0]?.localName === 'h2');
```

新しい `h2` 要素には `class` 属性を `bold small-text` に設定してください。
...
新しい `h2` 要素にはテキスト `Amount per serving` を設定してください。
