...
Greifen Sie auf die `#next` und `#previous` Elemente aus der HTML-Datei zu.

Weisen Sie sie jeweils Variablen mit den Namen `nextButton` und `previousButton` zu.
...
Sie sollten das `#next` Element der Variable `nextButton` zuweisen.
...
Sie sollten das `#previous` Element der Variable `previousButton` zuweisen.
