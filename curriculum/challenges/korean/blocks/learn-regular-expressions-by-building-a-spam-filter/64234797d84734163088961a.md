...
마지막으로, 정규식이 전체 단어와 일치하도록 허용하세요 (`freeRegex`처럼 하셨던 것처럼).

# --hints--

`stockRegex`은 캡처하지 않는 그룹을 사용해야 합니다.

```js
assert.match(stockRegex.source, /\(\?:/);
```

`stockRegex`는 `\s` 또는 `^`와 일치하도록 캡처하지 않는 그룹을 사용해야 합니다.

```js
assert.match(stockRegex.source, /\(\?:(\^\|\\s|\\s\|\^)\)/);
```

`stockRegex`는 두 번째 캡처하지 않는 그룹을 사용해야 합니다.
...
`stockRegex`는 `it's stock alert time`과 일치해야 합니다.

```js
assert.match("it's stock alert time", stockRegex);
```

`stockRegex`는 `stock alert time`와 일치해야 합니다.

```js
assert.match('stock alert time', stockRegex);
```

`stockRegex`는 `it's stock alert`와 일치해야 합니다.
...
`stockRegex`는 `hands-stock alert time`와 일치하지 않아야 합니다.

```js
assert.notMatch('hands-stock alert', stockRegex);
```

`stockRegex`는 `stock alert-management`와 일치하지 않아야 합니다.
