...
このラボでは、さまざまな都市の天気情報を提供するアプリを作成します。

天気APIを使用します。出力データは以下の形式です：
...
1. `button` 要素を持ち、その `id` が `get-weather-btn` である必要があります。

1. `select` 要素を持ち、その中に7つの `option` 要素がネストされている必要があります。最初のオプションはテキストが空文字で `value` 属性を持つ必要があります。残りはテキストと値が以下の通りで、値は小文字である必要があります：
...
1. 都市が選択されていない場合、ボタンを押しても何も起こらない必要があります。

1. 都市が選択されている場合、天気を表示する要素が現れる必要があります：

- 天気アイコンを表示するために、id が `img` の `weather-icon` 要素を持つ必要があります。

- 主な気温を表示するために、id が `main-temperature` の要素を持つ必要があります。

- 体感温度を表示するために、id が `feels-like` の要素を持つ必要があります。

- 湿度を表示するために、id が `humidity` の要素を持つ必要があります。

- 風速を表示するために、id が `wind` の要素を持つ必要があります。

- 突風を表示するために、id が `wind-gust` の要素を持つ必要があります。

- 主な天気の種類を表示するために、id が `weather-main` の要素を持つ必要があります。

- 現在の場所を表示するために、id が `location` の要素を持つ必要があります。

1. `getWeather` という名前の非同期関数を持ち、`https://weather-proxy.freecodecamp.rocks/api/city/<CITY>` API から天気情報を取得して返す必要があります。このAPIはメートル法を使用しており、風速は m/s、気温は摂氏で返されます。

1. `getWeather` 非同期関数は都市を引数として受け取る必要があります。

1. `getWeather` 関数内で発生したエラーを処理し、コンソールにログを出力する必要があります。

1. `showWeather` という非同期関数を持ち、都市をパラメータとして受け取る必要があります。

1. `showWeather` 関数は、ドロップダウンから選択された都市の天気データを取得するために `getWeather` 関数を呼び出す必要があります。

1. `getWeather` 関数でエラーが発生した場合、アプリは `Something went wrong, please try again later` というアラートのみを表示する必要があります。

1. `getWeather` からのデータが使用可能な場合、`showWeather` 関数は対応する要素に天気データを表示する必要があります。APIからの特定の値が `undefined` の場合、対応する要素には `N/A` と書く必要があります。

1. テストで再代入が必要なため、`getWeather` 関数も `showWeather` 関数も `const` を使って宣言してはいけません。

**注意:** テストで再代入が必要なため、`getWeather` 関数も `showWeather` 関数も `const` を使って宣言してはいけません。また、テストの完了には時間がかかるため、コンソールに `// running tests` と表示されている間はテストが実行中です。
...
`button` 要素を持ち、その `id` が `get-weather-btn` である必要があります。
...
`select` 要素の中で、最初の子要素は空の `value` 属性を持つ `option` 要素である必要があります。
...
`select` 要素の中に、パリ、ロンドン、東京、ロサンゼルス、シカゴ、ニューヨークの6つの `option` 要素がある必要があります。
...
`showWeather` 関数を持つ必要があります。

```js
assert.isFunction(showWeather);
```

`getWeather` 関数を持つ必要があります。

```js
assert.isFunction(getWeather);
```

`getWeather` 関数は都市を唯一の引数として受け取り、天気APIからのJSONを返す必要があります。
...
`getWeather` 関数も `showWeather` 関数も `const` を使って宣言してはいけません。
...
`showWeather` 関数は `getWeather` 関数を呼び出して天気データを取得する必要があります。
...
`showWeather` 関数は `getWeather` が `undefined` を返した場合の処理を管理する必要があります。
...
ニューヨークが選択された場合、`showWeather` 関数はAPIからのデータを対応するHTML要素に表示する必要があります。APIの値が `undefined` の場合、`N/A` を表示する必要があります。
...
シカゴが選択された場合、`showWeather` 関数はAPIからのデータを対応するHTML要素に表示する必要があります。APIの値が `undefined` の場合、`N/A` を表示する必要があります。
...
ロンドンが選択された場合、`showWeather` 関数はAPIからのデータを対応するHTML要素に表示する必要があります。APIの値が `undefined` の場合、`N/A` を表示する必要があります。
...
東京が選択された場合、`showWeather` 関数はAPIからのデータを対応するHTML要素に表示する必要があります。APIの値が `undefined` の場合、`N/A` を表示する必要があります。
...
ロサンゼルスが選択された場合、`showWeather` 関数はAPIからのデータを対応するHTML要素に表示する必要があります。APIの値が `undefined` の場合、`N/A` を表示する必要があります。
...
エラーがある場合、`getWeather` 関数はエラーをコンソールにログ出力する必要があります。
...
パリが選択された場合、アプリは `Something went wrong, please try again later` というアラートを表示する必要があります。
