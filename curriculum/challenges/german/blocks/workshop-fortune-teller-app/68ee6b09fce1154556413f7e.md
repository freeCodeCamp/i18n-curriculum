...
Fügen Sie innerhalb der `initializeEventListeners`-Methode einen `click`-Ereignis-Listener an `singleCardBtn` an. Rufen Sie innerhalb des Ereignis-Handlers die `singleCardSelected`-Methode auf. Denken Sie daran, das `singleCardBtn` von `elements` zu holen.

Diese Methode werden Sie in den kommenden Schritten erstellen.

# --hints--

Sie sollten ein `click`-Ereignis an das `singleCardBtn`-Element anhängen. Vergessen Sie nicht, dass Sie hier das Schlüsselwort `this` benötigen.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Innerhalb des Callback-Ereignisses sollten Sie die `singleCardSelected`-Methode aufrufen.
