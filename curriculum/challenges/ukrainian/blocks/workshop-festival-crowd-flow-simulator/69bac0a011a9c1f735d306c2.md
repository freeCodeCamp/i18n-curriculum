...
Обробіть вхідні дані воріт для поточного такту, викликавши `processGateFlow()` з `gate` та `tickIndex` і призначивши результат змінній з ім’ям `result`.

# --hints--

Ваша функція `handleGateAtTick` має викликати метод `processGateFlow()`.
...
Ви повинні викликати `processGateFlow()` з `gate` та `tickIndex` і призначити результат змінній з ім’ям `result`.
