...
現在，為 `p` 元素內的 `span` 指定 `text-base`、`font-medium` 和 `text-gray-500` 類別。

# --hints--

你 `p` 元素內的 `span` 應該具有 `text-base` 類別。
...
你 `p` 元素內的 `span` 應該有 `font-medium` 類別。
...
你 `p` 元素內的 `span` 應該具有 `text-gray-500` 類別。
