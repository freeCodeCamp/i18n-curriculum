...
在 `.penguin-body` 元素中，`.foot` 元素前，添加兩個 `div` 元素，元素的 `class` 都爲 `arm`。 給第一個 `.arm` 添加 `left` `class`，給第二個 `.arm` 添加 `right` `class`。
