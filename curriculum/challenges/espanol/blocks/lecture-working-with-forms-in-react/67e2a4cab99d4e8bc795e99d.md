...
title: "¿Cuál es el uso del hook useActionState y cómo funciona?"
challengeType: 19
...
React 19 llegó con dos nuevas características notables llamadas server components y server actions. 
...
En el navegador, verías tu botón de formulario cambiar de `Greet` a `Greeting...` mientras la acción está `isPending` - y el saludo mostraría `Good morning, {name}`, `Good afternoon, {name}`, o `Good evening, {name}`, dependiendo de la hora del día en que el formulario fue enviado.
