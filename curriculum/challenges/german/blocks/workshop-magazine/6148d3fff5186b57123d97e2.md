...
Ein Zitat ist ohne die richtigen Anführungszeichen kein richtiges Zitat. Sie können diese mit CSS-Pseudoselektoren hinzufügen.

Erstellen Sie einen `.quote::before`-Selektor und setzen Sie die `content`-Eigenschaft auf `"` mit einem nachfolgenden Leerzeichen.

Erstellen Sie außerdem einen `.quote::after`-Selektor und setzen Sie die `content`-Eigenschaft auf `"` mit einem vorangestellten Leerzeichen.

# --hints--

Sie sollten einen `.quote::before`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

Ihr `.quote::before`-Selektor sollte eine `content`-Eigenschaft mit dem Wert `'" '` haben.

```js
assert.match(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content, /\"\\"\s\"/);
```

Sie sollten einen `.quote::after`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

Ihr `.quote::after`-Selektor sollte eine `content`-Eigenschaft mit dem Wert `' "'` haben.
