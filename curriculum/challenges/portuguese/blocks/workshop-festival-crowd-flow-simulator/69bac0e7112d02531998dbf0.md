...
A tarefa final na sua função `handleGateAtTick` é lidar com qualquer overflow para o tick atual.

Se existir algum overflow (`result.overflow > 0`), você deve:

- Registrar a seguinte mensagem `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
- Redirecionar o overflow chamando sua função `rerouteOverflow()` com `gates`, `gate`, `tickIndex` e `result.overflow`.

# --hints--

Se existir algum overflow (`result.overflow > 0`), você deve primeiro registrar a mensagem `"Overflow of " + result.overflow + " attendees. Rerouting..."`.
...
Se existir algum overflow (`result.overflow > 0`), você deve redirecionar o overflow chamando `rerouteOverflow()` com `gates`, `gate`, `tickIndex` e `result.overflow`.
