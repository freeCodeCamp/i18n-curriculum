...
`showFortune` という名前のメソッドをもう1つ追加してください。パラメータとして `e` を渡し、その型は `Event` にしてください。また、`newReading` というメソッドも作成し、今は空のままにしてください。

# --hints--

`showFortune` という名前のメソッドを宣言する必要があります。
...
`showFortune` メソッドには `e` という名前のパラメータがあり、その型は `Event` である必要があります。
...
`newReading` という名前のメソッドを宣言する必要があります。
