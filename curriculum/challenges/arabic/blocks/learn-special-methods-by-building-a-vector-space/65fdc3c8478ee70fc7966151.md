...
بنفس الطريقة التي فعلتها سابقًا مع الطريقة `__eq__`، أنشئ جملة `if` تتحقق مما إذا كان `self` و `other` لا ينتميان إلى نفس الصنف وأرجع `NotImplemented`.
...
يجب أن تُرجع `NotImplemented` من جسم جملة `if`.
