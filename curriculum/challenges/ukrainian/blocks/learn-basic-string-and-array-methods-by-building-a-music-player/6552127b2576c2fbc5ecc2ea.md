...
Додайте блок `else`. Усередині блоку `else` викличте функцію `playSong` з ідентифікатором `id` пісні, що зараз відтворюється, як аргумент.

Це гарантує, що пісня, яка зараз відтворюється, продовжить грати, коли натиснути на кнопку відтворення.
...
Ви повинні додати блок `else` до вашої інструкції `if`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{\s*playSong\(\s*userData\?\.songs\s*\[\s*0\s*\]\s*\.id\s*\)\s*;?\s*\}\s*else\s*\{/)
```

Ви повинні викликати функцію `playSong` з `userData?.currentSong.id` у блоці `else` вашої інструкції `if`.
