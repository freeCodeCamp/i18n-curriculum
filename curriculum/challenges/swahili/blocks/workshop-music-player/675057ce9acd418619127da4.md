...
Sasa, tengeneza kitendakazi kinachoitwa `getPreviousSong`. Kifanye kirudishe wimbo unaopatikana `getCurrentSongIndex()` ukipunguzwa moja katika `userData.songs`, au `undefined` ikiwa wimbo wa sasa ni wa kwanza katika orodha ya nyimbo.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `getPreviousSong`.

```js
assert.isFunction(getPreviousSong);
```

Kitendakazi chako cha `getPreviousSong` kinapaswa kuita `getCurrentSongIndex`.

```js
assert.match(getPreviousSong.toString(), /getCurrentSongIndex\s*\(\s*\)/);
```

Kitendakazi chako cha `getPreviousSong` kinapaswa kurudisha wimbo uliotangulia wimbo wa sasa.
...
Kitendakazi chako cha `getPreviousSong` kinapaswa kurudisha `undefined` wakati wimbo wa sasa ni wa kwanza katika orodha ya nyimbo.
