...
Em `tests/1_unit-tests.js`, no teste de número `#2`, no grupo de testes `Basic Assertions`, modifique cada `assert` para `assert.isDefined()` ou para `assert.isUndefined()`, de maneira que cada teste passe (seja avaliado como `true`). Não altere os argumentos passados para as assertivas.
