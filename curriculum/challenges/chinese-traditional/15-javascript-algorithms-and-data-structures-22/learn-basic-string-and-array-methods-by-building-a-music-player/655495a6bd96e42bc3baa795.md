...
接下來，你需要在播放器顯示屏上顯示當前歌曲標題和藝術家。 使用 `const` 和箭頭語法創建一個空的 `setPlayerDisplay` 函數。

# --hints--

你應該使用 `const` 創建一個名爲 `setPlayerDisplay` 的空函數。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` 應該是一個函數。

```js
assert.isFunction(setPlayerDisplay)
```

你的 `setPlayerDisplay` 函數應該使用箭頭語法。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `setPlayerDisplay` 函數應該是空的。
