...
Хоча в попередньому уроці ви додали `placeholder` до першого елемента `input`, це не найкраще застосування для доступності; користувачі занадто часто плутають текст заповнювача з дійсним вхідним значенням — вони думають, що в введенні вже є значення.
