...
Пам’ятайте, що властивість `grid-column` визначає, в яких стовпчиках починається та закінчується елемент. Бувають випадки, коли ви невпевнені, скільки стовпчиків матиме сітка, але потрібно, щоб елемент закінчувався в останньому стовпчику. Для цього ви можете використати `-1` до останнього стовпчика.

Створіть селектор `.hero` та надайте йому властивість `grid-column` зі значенням `1 / -1`. Це скаже елементу охопити всю ширину сітки.

# --hints--

Ви повинні мати селектор `.hero`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero'));
```

Ваш селектор `.hero` повинен мати властивість `grid-column` зі значенням `1 / -1`.
