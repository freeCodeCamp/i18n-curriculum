...
Successivamente, chiama `.json()` sulla risposta e archivia il risultato in una variabile chiamata `data`. Poiché sei ancora all'interno di una funzione `async`, dovresti usare `await` prima di `response.json()`.

# --hints--

Dovresti chiamare `json()` su `response` e archiviarlo in una variabile `data`.
