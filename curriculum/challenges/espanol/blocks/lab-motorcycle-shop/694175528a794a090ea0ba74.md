...
title: Laboratorio tienda de motocicletas
challengeType: 25
...
Para este laboratorio, se te ha proporcionado todo el HTML y CSS. Vas a usar TypeScript para completar las funcionalidades de una página de tienda de motocicletas.
...
1. Debes crear un `type` llamado `Category` que consista en los siguientes valores posibles: `'Sport'`, `'Cruiser'`, `'Touring'`, `'Dirt'`, `'Adventure'`, `'Naked'`, `'Electric'`.
2. Debes crear un `interface` llamado `Motorcycle` que tenga las siguientes propiedades: un `id` de tipo `string`, un `name` de tipo `string`, un `manufacturer` de tipo `string`, un `category` de tipo `Category`, un `price` de tipo `number`, un `image_url` de tipo `string`, una propiedad `created_at` de tipo `Date`, un `description` de tipo `string` y un year de tipo `number`.
3. Debes crear una función llamada `fetchMotorcycles` que retorne un `Promise` que resuelva a un arreglo de objetos `Motorcycle`.
4. Debes cargar datos `Motorcycle` desde el endpoint `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
5. Debes crear una función llamada `renderMotorcycleCard` que tome un solo objeto `Motorcycle` como parámetro y retorne una cadena que contenga el HTML con una cadena HTML formateada para la tarjeta de motocicleta.
   1. La función `renderMotorCycleCard` debe tomar un parámetro llamado `motorcycle` de tipo `Motorcycle`.
   2. La función `renderMotorCycleCard` debe retornar un tipo `string`.
   3. Cada componente de tarjeta de motocicleta debe tener un elemento `img` que tenga una imagen válida y una clase `motorcycle-card-image-container`.
   4. Cada componente de tarjeta de motocicleta debe tener un elemento con clase `motorcycle-card-year-badge` que liste el año en texto plano.
   5. Cada componente de tarjeta de motocicleta debe tener un elemento con clase `motorcycle-card-title` que liste el nombre de la motocicleta en texto plano.
   6. Cada componente de tarjeta de motocicleta debe tener un elemento con clase `motorcycle-card-manufacturer` que liste el nombre del fabricante de la motocicleta en texto plano.
   7. Cada componente de tarjeta de motocicleta debe tener un elemento con clase `motorcycle-card-category` que liste la categoría de la motocicleta en texto plano.
   8. Cada componente de tarjeta de motocicleta debe tener un elemento con clase `motorcycle-card-description` que liste la descripción de la motocicleta en texto plano.
   9. Cada componente de tarjeta de motocicleta debe tener un elemento con clase `motorcycle-card-price` que liste el precio de la motocicleta en texto plano.
   10. Cada componente de tarjeta de motocicleta debe tener un elemento con clase `motorcycle-card-engine` que liste la potencia de cada motor.
6. Debes tener una clase llamada `MotorcycleGalleryApp`.
   1. La clase debe tener un arreglo `private` llamado `allMotorcycles` que esté tipado estáticamente como un arreglo de objetos `Motorcycle`.
   2. Esa clase debe tener una función llamada `renderMotorcycles` que renderice los componentes equivalentes de tarjeta de motocicleta de la variable `allMotorcycles` dentro del elemento con id `motorcycle-grid`.
7. El número de elementos `Motorcycle` que se muestran debe listarse dentro de un elemento con id `results-number`.
8. El filtro debe actualizarse en un evento `input`.

# --hints--

Debes tener un `type` llamado `Category`.
...
Tu tipo `Category` debe establecerse como una unión de los valores correctos.
...
Debes tener un `interface` llamado `Motorcycle`.
...
Debes tener todas las propiedades especificadas en la interfaz `Motorcycle`.
...
Debes tener una función llamada `fetchMotorcycles`.

```js
assert.isFunction(fetchMotorcycles);
```

Debes usar el endpoint `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
...
Debes retornar una promesa de 25 objetos `Motorcycle` desde `fetchMotorcycles`.
...
Debes tener una función llamada `renderMotorcycleCard`.

```js
assert.isFunction(renderMotorcycleCard);
```

La función `renderMotorcycle` debe aceptar un parámetro tipado estáticamente `Motorcycle`.
...
La función `renderMotorcycleCard` debe retornar explícitamente un tipo `string`.
...
La función `renderMotorcycleCard` debe retornar un `string`.
...
Debes tener una clase llamada `MotorcycleGalleryApp`.
...
El `MotorcycleGalleryApp` debe tener un arreglo llamado `allMotorcycles`.
...
La propiedad `allMotorcycles` debe ser `private`.
...
La propiedad `allMotorcycles` debe estar tipada estáticamente como `Motorcycle[]`.
...
El `MotorcycleGalleryApp` debe tener una función llamada `renderMotorcycles`.
...
La función `renderMotorcycles` debe renderizar 25 elementos con una clase `motorcycle-card-image-container`.
...
La función `renderMotorcycles` debe renderizar 25 elementos con una clase `motorcycle-card-year-badge`.
...
La función `renderMotorcycles` debe renderizar 25 elementos con una clase `motorcycle-card-title`.
...
La función `renderMotorcycles` debe renderizar 25 elementos con una clase `motorcycle-card-manufacturer`.
...
La función `renderMotorcycles` debe renderizar 25 elementos con una clase `motorcycle-card-category`.
...
La función `renderMotorcycles` debe renderizar 25 elementos con una clase `motorcycle-card-description`.
...
La función `renderMotorcycles` debe renderizar 25 elementos con una clase `motorcycle-card-price`.
...
La función `renderMotorcycles` debe renderizar 25 elementos con una clase `motorcycle-card-engine`.
