...
Para ajudar a acompanhar como os participantes navegam pelo sistema, registre a seguinte mensagem no console: `overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`. Você pode usar concatenação de string ou template literals.

# --hints--

Sua função `rerouteOverflow` deve registrar a seguinte mensagem no console `overflowAmount + " attendees rerouted to " + gates[nextGateIndex].id`.
