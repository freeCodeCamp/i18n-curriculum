...
使用類別選擇器，為你的 `.cat-nose` 元素的左右邊框各設定 `15px solid transparent`。同時，為它設定一個底部邊框為 `20px solid #442c2c`。
...
你的 `.cat-nose` 選擇器應該有一個設定為 `15px solid transparent` 的 `border-left` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderLeft === '15px solid transparent')
```

你的 `.cat-nose` 選擇器應該有一個設定為 `15px solid transparent` 的 `border-right` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderRight === '15px solid transparent')
```

你的 `.cat-nose` 選擇器應該有一個設定為 `20px solid #442c2c` 的 `border-bottom` 屬性。
