...
背景建筑开始看起来很不错了。 在 `.background-buildings` 元素下方创建一个新的 `div`，并为其添加一个 `foreground-buildings` 类。 这将是另一个容纳更多建筑的容器。
...
新的 `div` 应该在 `div.background-buildings` 元素之后。

```js
assert.exists(document.querySelector('div.background-buildings + div'));
```

你的新 `div` 应该有一个 `foreground-buildings` 类。
