...
داخل العنصر الأول `div.answer`، قم بتسكين عنصر `select` واحد مطلوب يحتوي على ثلاثة عناصر `option`.

امنح العنصر الأول `option` خاصية `value` بقيمة `""`، والنص `Select an option`. امنح العنصر الثاني `option` خاصية `value` بقيمة `yes`، والنص `Yes`. امنح العنصر الثالث `option` خاصية `value` بقيمة `no`، والنص `No`.

# --hints--

يجب أن تسكن عنصر `select` واحد داخل العنصر الأول `div.answer`.

```js
assert.exists(document.querySelector('div.answer > select'));
```

يجب أن تسكن ثلاثة عناصر `option` داخل عنصر `select`.

```js
assert.equal(document.querySelectorAll('div.answer > select > option')?.length, 3);
```

يجب أن تمنح العنصر الأول `option` خاصية `value` بقيمة `""`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(1)')?.value, '');
```

يجب أن تمنح العنصر الأول `option` محتوى نصي `Select an option`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(1)')?.textContent.trim(), 'Select an option');
```

يجب أن تمنح العنصر الثاني `option` خاصية `value` بقيمة `yes`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(2)')?.value, 'yes');
```

يجب أن تمنح العنصر الثاني `option` محتوى نصي `Yes`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(2)')?.textContent.trim(), 'Yes');
```

يجب أن تمنح العنصر الثالث `option` خاصية `value` بقيمة `no`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(3)')?.value, 'no');
```

يجب أن تمنح العنصر الثالث `option` محتوى نصي `No`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(3)')?.textContent.trim(), 'No');
```

يجب أن تمنح عنصر `select` خاصية `required`.
