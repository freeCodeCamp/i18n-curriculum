...
算术序列1487,4817,8147，其中每个项增加3330，在两个方面是不寻常的：（i）三个项中的每一个都是素数，并且，（ii）每个4位数字是彼此的排列。

没有由三个1位，2位或3位数的素数组成的算术序列，表现出这种性质，但还有另外一个4位数的增加序列。

你通过连接这个序列中的三个术语来形成12位数字？

# --hints--

`primePermutations()` 应该返回一个数字。

```js
assert(typeof primePermutations() === 'number');
```

`primePermutations()` 应该返回 296962999629。
