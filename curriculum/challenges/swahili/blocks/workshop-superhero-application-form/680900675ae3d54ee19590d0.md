...
Ili kurekebisha kosa, tengeneza kitendakazi cha mshale cha `handlePowersChange` chenye kigezo cha `e`.

Ndani ya kitendakazi, tengeneza vipande `value` na `checked` kutoka `e.target` ili kupata thamani ya kisanduku cha kuchagua na kama kimechaguliwa au la.

# --hints--

Unapaswa kutengeneza kitendakazi cha `handlePowersChange` chenye kigezo cha `e`.

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

Unapaswa kutenganisha `value` na `checked` kutoka `e.target`.
