...
Dentro da sua declaração `console.log()`, adicione o texto `"Author Data Array:"` como o primeiro argumento e `authorDataArr` como o segundo argumento.Use a vírgula para separar o texto de `authorDataArr`.
...
Você deve ter um console log com o texto `"Author Data Array:"`.

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1/)
```

Você deve usar vírgula para separar seu texto `"Author Data Array:"` e `authorDataArr`.

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1\s*,/)
```

`authorDataArr` deve ser o segundo argumento da sua instrução de console log.
