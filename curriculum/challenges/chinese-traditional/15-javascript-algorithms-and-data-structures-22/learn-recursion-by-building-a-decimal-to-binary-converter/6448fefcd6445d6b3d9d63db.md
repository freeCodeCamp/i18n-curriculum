...
每當事件觸發事件監聽器時，就會自動創建一個事件對象。 你並不總是需要使用這個對象，就像上面的 `click` 處理程序一樣，但它對於訪問有關觸發事件的信息很有用。

首先，將 `e` 作爲參數傳遞給回調函數。 請記住，`e` 是事件對象的通用參數名稱。 接下來，將 `e` 記錄到回調函數主體中的控制檯。
