...
Hadi sasa, mionekano yote ya mviringo uliyoitengeneza imekuwa ikianzia juu hadi chini, hiyo ndiyo mwelekeo wa kawaida. Unaweza kubainisha mwelekeo mwingine kwa kuuweka kabla ya rangi zako kama ifuatavyo:
...
Jaza `.bb3` na `repeating-linear-gradient`. Tumia `90deg` kwa mwelekeo, `building-color3` kwa rangi mbili za kwanza, na `window-color3` kwa `15%` kwa rangi ya tatu.

Unapokosa kubainisha umbali kwa rangi, itatumia thamani inayofaa. Katika kesi hii, rangi mbili za kwanza zitakuwa `0%` na `7.5%` kwa sababu inaanzia `0%`, na `7.5%` ni nusu ya `15%`, hivyo huna haja ya kuzibainisha.

# --hints--

Unapaswa kumpa `.bb3` `background` kwa kutumia `repeating-linear-gradient`.
...
Unapaswa kutumia `--window-color3` kwa `15%` kwa rangi ya tatu.
