...
你的 `calculateTaxes` 方法應返回 `taxRate` 的值（除以 100，將其轉換爲百分比）乘以 `amount` 參數。

爲了清楚起見，將 `taxRate / 100` 計算結果括在括號中。

# --hints--

你應該在 `calculateTaxes` 方法中將 `taxRate` 除以 100。 請記住使用 `this` 關鍵字。
...
你應該將 `this.taxRate / 100` 計算結果括在括號中。
...
你應該在 `calculateTaxes` 方法中將 `amount` 參數乘以 `taxRate`，然後除以 100。
