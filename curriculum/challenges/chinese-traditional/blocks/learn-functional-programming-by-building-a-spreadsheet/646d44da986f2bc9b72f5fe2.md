...
聲明一個 `toNumberList` 函數，該函數接受 `args` 參數並隱式返回用逗號拆分 `args` 的結果。 然後將 `map` 方法鏈接到 `split` 方法，並將 `parseFloat` 作爲參數傳遞給 `map` 方法。

# --hints--

你應該聲明一個 `toNumberList` 變量。
...
你應該使用 `const` 來聲明你的 `toNumberList` 變量。
...
你的 `toNumberList` 函數應該有一個 `args` 參數。

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(\s*str\s*\)\s*;?\s*const\s+infix\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\+-|-\+)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(\s*noHigh\s*,\s*infix\s*\)\s*;?\s*const\s+functionCall\s*=\s*\/\(\[a-z0-9\]\*\)\\\(\(\[0-9\., \]\*\)\\\)\(\?!\.\*\\\(\)\/i\s*;?\s*const\s+toNumberList\s*=\s*(\(\s*args\s*\)|args)\s*=>/);
```

你的 `toNumberList` 函數應該使用隱式返回。
...
你的 `toNumberList` 函數應該返回調用 `args` 的 `.split()` 方法的結果。
...
你應該將對 `parseFloat` 的引用作爲對 `.map()` 的回調傳遞。
