...
Construa um aplicativo JavaScript full stack que seja funcionalmente semelhante a este: <a href="https://anonymous-message-board.freecodecamp.rocks/" target="_blank" rel="noopener noreferrer nofollow">https://anonymous-message-board.freecodecamp.rocks/</a>.

Trabalhar nesse projeto vai fazer com que você escreva o código usando um dos seguintes métodos:

-   Clone <a href="https://github.com/freeCodeCamp/boilerplate-project-messageboard/" target="_blank" rel="noopener noreferrer nofollow">este repositório do GitHub</a> e complete seu projeto localmente.
...
Você pode enviar uma solicitação POST para `/api/threads/{board}` com dados de formulário, incluindo `text` e `delete_password`.O registro de banco de dados salvo terá pelo menos os campos `_id`, `text`, `created_on` (data e hora), `bumped_on` (data e hora, começa igual a `created_on`), `reported` (booleano), `delete_password` e `replies` (array).
...
Você pode enviar uma requisição POST para `/api/replies/{board}` com dados de formulário incluindo `text`, `delete_password` e `thread_id`.Isto atualizará a data de `bumped_on` para a data do comentário.No array `replies` da thread, um objeto será salvo com pelo menos as propriedades `_id`, `text`, `created_on`, `delete_password` e `reported`.
...
Você pode enviar uma requisição DELETE para `/api/threads/{board}` e passar o `thread_id` e o `delete_password` para deletar a thread.O conteúdo retornado será a string `incorrect password` ou `success`.
...
Você pode enviar uma requisição DELETE para `/api/replies/{board}` e passar o `thread_id`, `reply_id` e `delete_password`.O conteúdo retornado será a string `incorrect password` ou `success`.Em caso de sucesso, o texto de `reply_id` será alterado para `[deleted]`.
...
Você pode enviar uma requisição PUT para `/api/replies/{board}` e passar o `thread_id` e o `reply_id`.O conteúdo retornado será a string `reported`.O valor `reported` de `reply_id` será alterado para `true`.
