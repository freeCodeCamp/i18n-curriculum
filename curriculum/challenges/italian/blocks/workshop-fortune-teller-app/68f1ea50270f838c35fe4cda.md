...
Ora fai lo stesso per `subTitle` assegnandolo a `meaning_up`, e per `title` assegnalo a `foundCard.name`. Dopo chiama il metodo `showElements` e passa `fortuneDescription` dagli elementi.

# --hints--

Dovresti impostare `this.elements.subTitle.textContent` su `foundCard.meaning_up`.
...
Dovresti impostare `this.elements.title.textContent` su `foundCard.name`.
...
Dovresti chiamare la funzione `showElements` dentro `showFortune`.
...
Dovresti passare `this.elements.fortuneDescription` come argomento a `showElements`.
