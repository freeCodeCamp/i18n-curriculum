...
创建另一个名为 `get_tv_series` 的方法，并使其返回一个包含你在 `self.items` 中找到的所有 `TVSeries` 实例的列表。

# --hints--

你的 `MediaCatalogue` 类应有一个名为 `get_tv_series` 的方法。
...
你的 `get_tv_series` 方法应返回一个列表，包含 `self.items` 中所有属于 `TVSeries` 类的实例项。
