...
Після вашого останнього `.divider` створіть новий елемент `p` з текстом `Cholesterol 0mg 0%`. Потім обгорніть текст `Cholesterol` у елемент `span`, `0%` — в інший, і надайте кожному з них клас `bold`.

Нарешті, вкладiть елемент `span`, що містить текст `Cholesterol`, разом із текстом `0mg` у додатковий елемент `span` для вирівнювання.

# --hints--

Ви повинні створити новий елемент `p` в кінці вашого елемента `.daily-value`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'p');
```

Ваш новий елемент `p` має містити текст `Cholesterol 0mg 0%`.
...
Елемент `span`, що містить текст `Cholesterol`, має мати атрибут `class`, встановлений у `bold`.
...
Текст `0%` має бути вкладений у `span`.
...
Елемент `span`, що містить текст `0%`, має мати атрибут `class`, встановлений у `bold`.
...
Елемент `span`, що містить текст `Cholesterol` і текст `0mg`, має бути обгорнутий разом в іншому елементі `span`.
