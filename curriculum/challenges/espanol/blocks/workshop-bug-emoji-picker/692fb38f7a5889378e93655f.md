...
A continuación, crea una subclase llamada `Bee` que extienda la clase `Bug` con el parámetro de tipo genérico establecido en `string`.

# --hints--

Deberías tener una clase llamada `Bee`.
...
Tu clase `Bee` debería extender `Bug`.
...
Tu clase `Bee` debería extender `Bug` con un argumento de tipo `string`.
