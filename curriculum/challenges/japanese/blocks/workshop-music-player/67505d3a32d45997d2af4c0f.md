...
`playPreviousSong` という名前の関数を作成してください。新しい関数の中で、`getPreviousSong` 関数が返す値を使って再生する曲を見つけてください。`getPreviousSong()` が `undefined` の場合は、プレイリストの最初の曲を再生してください。そうでなければ、現在再生中の曲の前の曲を再生してください。

# --hints--

`playPreviousSong` という名前の関数があるはずです。

```js
assert.isFunction(playPreviousSong);
```

`playPreviousSong` 関数は、プレイリストで現在再生中の曲の前の曲を再生する必要があります。
...
`getPreviousSong()` が `undefined` の場合、`playPreviousSong` 関数はプレイリストの最初の曲を再生する必要があります。
