...
Dovresti definire `--building-color1` con un valore `#aa80ff` nel selettore `:root`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color1')?.trim(), '#aa80ff');
```

Dovresti definire `--building-color2` con un valore `#66cc99` nel selettore `:root`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color2')?.trim(), '#66cc99');
```

Dovresti definire `--building-color3` con un valore `#cc6699` nel selettore `:root`.
