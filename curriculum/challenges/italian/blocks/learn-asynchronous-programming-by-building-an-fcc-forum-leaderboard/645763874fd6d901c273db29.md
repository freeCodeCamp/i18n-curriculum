...
All'interno del primo elemento `td`, aggiungi un'espressione incorporata `${}`. All'interno di quell'espressione, chiama la funzione `forumCategory` con l'argomento `category_id`. 

Ora dovresti vedere una categoria visualizzata sotto ogni argomento del post. 

# --hints--

Dovresti chiamare la funzione `forumCategory` con l'argomento `category_id`
