...
いよいよ関数を使う時です。4つの引数 `processIngredient`、`baseQuantity`、`baseUnit`、`newUnit` を受け取る新しい `newServings` 関数を宣言してください。

この関数は、基本の分量を新しい分量に合わせて調整し、その調整した分量を基本単位から新しい単位に変換し、変換後の分量を小数点以下2桁の固定小数点文字列として返す必要があります。

もし行き詰まったら、ログに記録した関数の呼び出し例を参照してください。両方の例の値を変更して試してみることもできます。

# --hints--

`processIngredient` 関数を宣言してください。

```js
assert.isFunction(processIngredient);
```

`processIngredient` 関数の最初のパラメータは `baseQuantity` であるべきです。

```js
assert.match(processIngredient?.toString(), /\(baseQuantity/);
```

`processIngredient` 関数の2番目のパラメータは `baseUnit` であるべきです。

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit/);
```

`processIngredient` 関数の3番目のパラメータは `newUnit` であるべきです。

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit/);
```

`processIngredient` 関数の4番目のパラメータは `newServings` であるべきです。

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit,\s*newServings\)/);
```

`processIngredient` 関数は文字列を返すべきです。

```js
assert.isString(processIngredient(10, "gram", "cup", 5));
```

`processIngredient` 関数は正しい値を返すべきです。
