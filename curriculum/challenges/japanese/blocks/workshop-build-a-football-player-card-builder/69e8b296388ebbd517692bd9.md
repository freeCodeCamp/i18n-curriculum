...
Club `value` に `player.club` の `input` 属性を追加してください。その後、`onChange` をパラメータに取り、`e` をスプレッドし、`setPlayer` を `player` に設定して `club` を呼び出すアロー関数に設定された `e.target.value` 属性を追加してください。

# --hints--

あなたの Club `input` はプレイヤーのクラブと等しい `value` を持つべきです。
...
あなたの Club `input` は `onChange` に設定された `setPlayer` とスプレッドされた `player` を使って `club` を呼び出す `e.target.value` ハンドラを持つべきです。
