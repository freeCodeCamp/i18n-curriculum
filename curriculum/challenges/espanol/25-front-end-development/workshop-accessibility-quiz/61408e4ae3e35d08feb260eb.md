...
Cambia el color de la fuente del `h1` a `#f1be32`, y establece el tamaño de la fuente a `min(5vw, 1.2em)`.

# --hints--

Debes usar el selector de elemento `h1`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1'));
```

Debes darle al `h1` un `color` de `#f1be32`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.color, 'rgb(241, 190, 50)');
```

Debes darle al `h1` un `font-size` de `min(5vw, 1.2em)`.
