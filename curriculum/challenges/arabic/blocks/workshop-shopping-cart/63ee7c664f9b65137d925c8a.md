...
يحتاج صنف `ShoppingCart` إلى القدرة على إضافة عناصر. أنشئ طريقة `addItem` فارغة، تأخذ معلمتين: `id` و `products`. قد يبدو إنشاء طريقة مثل هذا:
...
المعلمة الأولى، `id`، هي `id` المنتج الذي أضافه المستخدم إلى سلة التسوق. المعلمة الثانية، `products`، هي مصفوفة من مثيلات `Dessert`. باستخدام معلمة بدلاً من الإشارة مباشرة إلى مصفوفة `products` الموجودة، ستكون هذه الطريقة أكثر مرونة إذا أردت إضافة قوائم منتجات إضافية في المستقبل.
