...
Para fazer com que fiquem na mesma linha, você precisa aplicar algum tipo de estilo aos elementos `p`, de modo que eles comecem a se comportar mais como elementos <dfn>inline</dfn>. Para fazer isso, comece adicionando um atributo `class` com o valor `item` para o primeiro elemento `article` sob o título `Coffee`.
