...
لمنع أن يكون حاصل ضرب رقم واحد أكبر من `9`، ضمن حلقة الأرقام الزوجية، أضف جملة `if` تتحقق مما إذا كان `number` أكبر من أو يساوي `10`. إذا كان كذلك، اطبع `number`.

# --hints--

يجب أن تحتوي الحلقة `for` على `if number >= 10:`.
...
يجب أن تحتوي جملة `if` على `print(number)`.
