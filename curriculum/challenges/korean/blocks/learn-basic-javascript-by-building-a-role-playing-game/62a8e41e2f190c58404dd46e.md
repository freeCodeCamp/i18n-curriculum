...
새 줄에서 덧셈 대입 연산자(`+=`)를 사용해 `" You attack it with your <weapon>."` 값에 문자열 `text`을 더하세요. 이때 `<weapon>`는 플레이어의 현재 무기로 바꾸세요. 또한 이 텍스트 줄은 공백으로 시작하므로 제대로 표시됩니다.

# --hints--

새 줄에서 점 표기법을 사용해 `innerText`의 `text` 속성에 접근하세요.
