...
正しい入力フィールドを表示するために `chooseShape()` 関数を `val.value` で呼び出し、その後 `updateResult()` を呼び出してエリアを再計算し表示してください。

# --hints--

`chooseShape` 関数を `val.value` で呼び出す必要があります。

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

`updateResult()` 関数を呼び出す必要があります。
