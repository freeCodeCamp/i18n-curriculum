...
`.replace()` 方法的第二個引數不必是字串。你可以改為傳入回呼函式，以對匹配的字串執行更複雜的邏輯。

回呼函式帶有幾個參數。第一個是匹配的字串。為你的 `.replace()` 呼叫傳入一個空的回呼函式，並給它一個 `match` 參數。

# --hints--

你應該將箭頭函式作為第二個引數傳遞給你的 `.replace()` 方法。
...
你的箭頭函式（arrow function）應該帶有一個 `match` 參數。
