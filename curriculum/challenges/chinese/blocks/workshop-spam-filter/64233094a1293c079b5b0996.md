...
垃圾邮件通常使用数字而不是字母来绕过过滤器。 你的正则表达式应该能捕捉到这些。

将正则表达式中的 `e` 字符替换为与 `e` 和 `3` 匹配的字符类。

# --hints--

你的 `freeRegex` 应该使用字符类。

```js
assert.match(freeRegex.source, /\[.*\]/);
```

你的 `freeRegex` 应该使用字符类来匹配 `e` 和 `3`。

```js
assert.match(freeRegex.source, /\[(e3|3e)\]/);
```

你的 `freeRegex` 应该使用三个字符类来匹配 `e` 和 `3`。
...
你的 `freeRegex` 应该与 `fr33 mon3y` 匹配。
