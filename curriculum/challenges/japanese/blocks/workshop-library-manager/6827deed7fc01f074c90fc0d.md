...
別の著者で`getBooksByAuthor`関数をテストすると良いでしょう。

まず、メッセージ`"\nList of books by James Clear:\n"`をコンソールにログ出力してください。

その下に`console.log()`を追加し、さらに`console.log()`を追加してください。その`console.log()`の中で、`getBooksByAuthor()`関数を`library`と`"James Clear"`の引数で呼び出してください。

これで、その特定の著者のすべての本がコンソールに表示されるはずです。
...
`console.log(getBooksByAuthor(library, "James Clear"))`が必要です。
