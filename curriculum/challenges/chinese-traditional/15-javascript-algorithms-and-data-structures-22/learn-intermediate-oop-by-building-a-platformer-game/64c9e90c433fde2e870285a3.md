...
在回調函數內部，創建一個名爲 `collisionDetectionRules` 的新 `const` 變量併爲其分配一個空數組。

在該數組內，添加一個布爾表達式，檢查玩家的 `y` 位置加上玩家的身高是否小於或等於平臺的 `y` 位置。

# --hints--

你應該有一個名爲 `collisionDetectionRules` 的 `const` 變量，該變量被分配一個空數組。

```js
assert.match(code, /const\s+collisionDetectionRules\s*=\s*\[\s*(?:[^\]]*\s*)*\s*\]\s*;?/);
```

你應該有一個布爾表達式來檢查玩家的 `y` 位置加上玩家的身高是否小於或等於平臺的 `y` 位置。
