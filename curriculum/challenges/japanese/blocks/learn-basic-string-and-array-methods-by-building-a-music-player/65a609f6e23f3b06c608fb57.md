...
`sort()` メソッドは、ソート順を定義する比較用のコールバック関数を受け取ります。

この例では、最初の条件 `(a.name < b.name)` が、最初の果物の名前が2番目の果物の名前よりも小さいかどうかをチェックしています。もしそうなら、最初の果物が2番目の果物より前にソートされます。

文字列は辞書式順序で比較されます。つまり、文字ごとに比較されます。例えば、`"Apples"` は `"Bananas"` より小さいです。なぜなら `"A"` がアルファベット順で `"B"` より前に来るからです。

この例で数値が返される理由は、`sort()` メソッドが数値の返却を期待しているからです。負の数を返すと、最初のアイテムが2番目のアイテムより前にソートされます。
...
コールバック関数の中で、`if` 文を追加して `a.title` が `b.title` より小さいかどうかをチェックしてください。もしそうなら、`-1` を返してください。

# --hints--

コールバック関数の中に `if` 文があるはずです。

```js
assert.match(code, /if\s*\(/);
```

`if` 文は `a.title` が `b.title` より小さいかどうかをチェックしてください。

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

`if` 文は `-1` が `a.title` より小さい場合に `b.title` を返すべきです。
