...
Agora que sua função `.map()` está recebendo a referência da função retornada `num => charRange(...).map(...)` das chamadas curried de `addCharacters`, ela irá iterar corretamente sobre os elementos e passar cada elemento como `n` para essa função.
