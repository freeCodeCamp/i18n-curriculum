...
표 제목의 마지막 부분은 연도입니다. `#years` 선택자를 만들고, 플렉스박스를 활성화하세요. 플렉스 방향의 끝으로 내용을 정렬하고, 요소를 스티키로 만드세요. `top: 0`을 사용해 컨테이너 상단에 고정하세요.
