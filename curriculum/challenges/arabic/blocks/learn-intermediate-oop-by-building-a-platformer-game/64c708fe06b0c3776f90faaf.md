...
أضف بند `else` يُعيّن الرقم `0` إلى سرعة اللاعب في المحور `x`.

# --hints--

يجب أن تضيف جملة `else` إلى دالة `animate` الخاصة بك.

```js
assert.match(animate.toString(), /\}\s*else\s*\{/);
```

يجب أن تأتي جملة `else` بعد جملة `else if` الخاصة بك.
...
يجب أن تتضمن بند `else` يُعيّن الرقم `0` إلى سرعة اللاعب في المحور `x`.
