...
افتراضيًا، قد لا تجلس عناصر النموذج مثل أزرار الاختيار بمحاذاة مثالية مع مسمياتها، خاصة إذا كان نص المسمى أطول أو له إعدادات خط مختلفة.

لمحاذاة أزرار الاختيار عموديًا مع مسمياتها، يمكنك استخدام خاصية `vertical-align`. تتحكم هذه الخاصية في كيفية محاذاة العناصر المضمَّنة أو الكتلية المضمَّنة عموديًا مع النص المحيط.

الآن، عيّن `vertical-align` إلى `bottom` لأزرار الاختيار. يمكنك تجربة قيم أخرى مثل `middle` أو `top` لترى كيف تؤثر على المحاذاة، لكن `bottom` عادةً ما تعمل جيدًا لأزرار الاختيار بجانب المسميات.

# --hints--

يجب أن تستمر في استهداف صنف `.contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

عيّن خاصية `vertical-align` إلى `bottom`.
