...
أنشئ متغير CSS مخصص باسم `--penguin-skin`، واضبطه على `gray`. ثم استبدل جميع قيم الخاصية ذات الصلة به.

# --hints--

يجب أن تعطي `:root` خاصية `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

يجب أن تعطي `--penguin-skin` قيمة `gray`، لكن وُجد `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

يجب أن تعطي `.penguin-head` خاصية `background` بقيمة `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
يجب أن تعطي `.penguin-body::before` خاصية `background-color` بقيمة `var(--penguin-skin)`، لكن وُجد `--fcc-actual--`.
...
يجب أن تعطي `.arm` خاصية `background` بقيمة `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
