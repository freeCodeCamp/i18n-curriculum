...
コンストラクタの後に、戻り値の型が `render` である抽象 `void` メソッドを作成してください。これはサブクラスで表示される絵文字を更新するために使います。

# --hints--

あなたの `Bug` クラスには抽象 `render` メソッドが必要です。

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

あなたの `render` メソッドは戻り値の型が `void` である必要があります。
