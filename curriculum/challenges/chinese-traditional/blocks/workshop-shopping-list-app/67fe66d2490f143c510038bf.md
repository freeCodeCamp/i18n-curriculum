...
然後，在核取方塊的 `input` 元素中，為你的 `checked` 屬性添加並設定為 `isChecked`。雖然不一定非得設定 `checked` 屬性，但這是確保每個核取方塊反映 `selectedItems` 陣列當前狀態的良好做法。

# --hints--

你應該為你的核取方塊 `input` 元素添加 `checked` 屬性。
