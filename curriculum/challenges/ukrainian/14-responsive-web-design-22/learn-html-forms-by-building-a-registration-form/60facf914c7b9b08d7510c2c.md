...
Додайте початковий розмір із `3` рядків та `30` стовпців.

# --hints--

Ви повинні надати `textarea` атрибут `rows` зі значенням `3`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

Ви повинні надати `textarea` атрибут `cols` зі значенням `30`.
