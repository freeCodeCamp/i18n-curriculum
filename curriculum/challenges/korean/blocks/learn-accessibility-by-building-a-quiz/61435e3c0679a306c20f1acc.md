...
페이지 내에서 탐색할 수 있도록 각 앵커 요소에 `href`가 `id` 요소들의 `h2`에 대응하도록 지정하세요.

# --hints--

첫 번째 `a` 요소에 `href`를 `#student-info`로 지정하세요.

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

두 번째 `a` 요소에 `href`를 `#html-questions`로 지정하세요.

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

세 번째 `a` 요소에 `href`를 `#css-questions`로 지정하세요.
