...
Dentro do elemento `button`, crie um elemento `span` que contenha `😊`

Dê ao elemento `span` um atributo `role` com o valor `img` e um atributo `aria-hidden` com o valor `true`.

Então, crie um segundo elemento `span` com uma `class` de `count` contendo o texto `0/10`.

# --hints--

Dentro do elemento `button` devem haver dois elementos `span`.

```js
assert.lengthOf(document.querySelectorAll("button > span"), 2);
```

O primeiro elemento `span` deve ter um conteúdo de texto `😊`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.textContent.trim(), "😊");
```

O primeiro elemento `span` deve ter um atributo `role` com valor `img`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("role"), "img");
```

O primeiro elemento `span` deve ter um atributo `aria-hidden` com valor `true`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("aria-hidden"), "true");
```

O segundo elemento `span` deve ter um atributo `class` com valor `count`.

```js
assert.equal(document.querySelectorAll("button > span")[1]?.className, "count");
```

O segundo elemento `span` deve ter um conteúdo de texto `0/10`.
