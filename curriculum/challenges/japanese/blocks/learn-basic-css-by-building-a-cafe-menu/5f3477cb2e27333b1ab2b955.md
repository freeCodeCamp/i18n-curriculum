...
今度は `styles.css` ファイルをリンクして、スタイルを再び適用させる必要があります。`head` 要素の中に `link` 要素を追加してください。`rel` 属性に `"stylesheet"` の値を、`href` 属性に `"styles.css"` の値を与えてください。
...
`link` 要素は空要素なので、終了タグ `</link>` を付けてはいけません。

```js
assert.notMatch(code, /<\/link>/i);
```

既存の `head` 要素は変更しないでください。終了タグを削除していないか確認してください。
