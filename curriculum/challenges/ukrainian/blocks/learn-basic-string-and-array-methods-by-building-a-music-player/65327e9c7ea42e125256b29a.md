...
Ми додали залишок пісень до масиву `allSongs` для вас.

Далі ви дізнаєтеся про <dfn>Web Audio API</dfn> і як його використовувати для відтворення пісень. Усі сучасні браузери підтримують Web Audio API, який дозволяє генерувати та обробляти аудіо у веб-застосунках.

Використайте `const`, щоб створити змінну з іменем `audio` і призначити їй значення `new Audio()`. Це створить новий HTML5 елемент `audio`.

# --hints--

Ви повинні використати ключове слово `new`, щоб створити екземпляр об’єкта `Audio`.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

Ви повинні призначити об’єкт `Audio` константі з іменем `audio`.
