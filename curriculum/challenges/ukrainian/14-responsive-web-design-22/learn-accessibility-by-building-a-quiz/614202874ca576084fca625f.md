...
Then, within each `section` element, nest one `h2` element with an `id` matching the corresponding `aria-labelledby` attribute. Надайте кожному `h2` пригожий текстовий вміст.
...
Надайте першому елементу `h2` пригожий текстовий вміст. _Hint: I would have chosen `Student Info`_

```js
assert.isAtLeast(document.querySelectorAll('h2')?.[0]?.textContent?.length, 1);
```

Надайте другому елементу `h2` пригожий текстовий вміст. _Hint: I would have chosen `HTML`_

```js
assert.isAtLeast(document.querySelectorAll('h2')?.[1]?.textContent?.length, 1);
```

Надайте третьому елементу `h2` пригожий текстовий вміст. _Hint: I would have chosen `CSS`_
