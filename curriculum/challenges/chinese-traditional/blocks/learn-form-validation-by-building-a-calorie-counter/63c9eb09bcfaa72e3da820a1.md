...
你需要獲取所有輸入容器。 聲明一個 `inputContainers` 變量，並將其賦值爲查詢文檔中所有屬於 `input-container` 類的元素的值。

# --hints--

你應該在 `clearForm` 函數中聲明一個 `inputContainers` 變量。

```js
assert.match(clearForm.toString(), /inputContainers\s*=/);
```

你應該使用 `querySelectorAll` 方法來獲取所有屬於 `input-container` 類的元素。

```js
assert.match(clearForm.toString(), /document\.querySelectorAll\(\s*('|")\.input-container\1\s*\)/);
```

你應該將 `querySelectorAll` 方法的值分配給 `inputContainers` 變量。
