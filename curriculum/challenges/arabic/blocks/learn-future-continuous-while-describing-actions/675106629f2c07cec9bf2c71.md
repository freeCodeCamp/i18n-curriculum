...
أي جملة تستخدم زمن `Future Continuous` بشكل صحيح لوصف ما ستفعله جيسيكا؟
...
هذا زمن `Simple Future`، وليس زمن `Future Continuous`.
...
هذا زمن `Present Continuous`، وليس زمن `Future Continuous`.
...
هذا زمن `Simple Present`، وليس زمن `Future Continuous`.
...
يُستخدم زمن `Future Continuous` لوصف فعل مستمر سيحدث في المستقبل. تركيبه هو `will be` + الفعل المنتهي بـ `-ing`.
