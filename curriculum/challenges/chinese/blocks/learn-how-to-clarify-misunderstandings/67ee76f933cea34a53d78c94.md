...
`Consistent` means something that happens regularly without changes. 例如：
...
`Happens enough` means that something occurs often enough to be noticeable, but not all the time. 例如：
...
`Annoying` refers to something that bothers or frustrates you. 例如：
