...
Declare uma variável chamada `plantsSet` e atribua a ela uma chamada para a função `displayPlantsSet`.

Então registre `plantsSet` no console.Você precisará abrir o console do navegador para ver seu set.


# --hints--

Você deve ter uma variável chamada `plantsSet`.

```js
assert.exists(plantsSet);
```

Você deve chamar `displayPlantsSet` e atribuí-lo a `plantsSet`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

Você deve registrar `plantsSet` no console.
