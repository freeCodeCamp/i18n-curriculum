...
На кожній ітерації циклу `while` він знаходить елемент, більший за поточне значення. Потрібно перемістити цей елемент вправо, щоб створити місце для поточного значення.

Зробіть це, присвоївши значення в індексі `j` наступному індексу.

# --hints--

Перш ніж зменшувати `j`, присвойте значення за індексом `j` індексу `j + 1`.
