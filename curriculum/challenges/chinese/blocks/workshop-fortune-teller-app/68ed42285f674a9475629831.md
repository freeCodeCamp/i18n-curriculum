...
在类内部，添加一个私有 `elements` 属性，类型为对象。在其中，添加属性 `singleCardBtn`、`singleCard` 和 `multipleCardsBtn`，它们的类型均为 `HTMLElement`。

# --hints--

你应该在 `private` 类中声明一个名为 `elements` 的 `Game` 属性。
...
你的 `elements` 私有属性应包含一个类型为 `singleCardBtn` 的 `HTMLElement` 属性。
...
你的 `elements` 私有属性应包含一个类型为 `multipleCardsBtn` 的 `HTMLElement` 属性。
...
你的 `elements` 私有属性应包含一个类型为 `singleCard` 的 `HTMLElement` 属性。
