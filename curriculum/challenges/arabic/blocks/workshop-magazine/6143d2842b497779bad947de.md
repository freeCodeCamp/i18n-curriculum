...
لبدء CSS الخاص بك، قم بتطبيع قواعد CSS من خلال استهداف كل العناصر باستخدام `*`، بما في ذلك محددات الحالة الزائفة `::before` و `::after`.

اضبط خاصيتي `padding` و `margin` كلاهما على `0` واضبط خاصية `box-sizing` على `border-box`.
...
يجب أن يحتوي محدد `*, ::before, ::after` على خاصية `padding` مضبوطة على `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.padding, '0px');
```

يجب أن يحتوي محدد `*, ::before, ::after` على خاصية `margin` مضبوطة على `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.margin, '0px');
```

يجب أن يحتوي محدد `*, ::before, ::after` على خاصية `box-sizing` مضبوطة على `border-box`.
