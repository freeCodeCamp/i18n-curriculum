...
你可以使用 `else if` 使 `else` 语句具有条件。 这是一个示例：
...
在 `if` 语句末尾，添加 `else if` 语句来检查 `monsterHealth` 是否小于或等于 `0`。 在你的 `else if` 中，调用 `defeatMonster` 函数。
...
你的 `else if` 语句应检查 `monsterHealth` 是否小于或等于 `0`。

```js
assert.match(attack.toString(), /else\s+if\s*\(\s*monsterHealth\s*<=\s*0\s*\)/);
```

你的 `else if` 语句应该调用 `defeatMonster` 函数。
