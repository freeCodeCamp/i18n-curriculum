...
Aquí tienes un ejemplo de la especificación oficial de HTML, usando el elemento `i` para mostrar una frase idiomática en francés.
...
El atributo `lang` dentro de la etiqueta de apertura `<i>` se usa para especificar el idioma del contenido. En este caso, el idioma sería francés. El elemento `i` no indica si el texto es importante o no, solo muestra que es de alguna manera diferente del texto que lo rodea.
