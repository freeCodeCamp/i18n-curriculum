...
Puoi passare un <dfn>riferimento</dfn> a una funzione come parametro callback. Un riferimento a una funzione è il nome della funzione senza le parentesi tonde. Per esempio:
...
Il metodo `.map()` qui chiamerà la funzione `myFunc`, passando gli stessi argomenti che una callback `.map()` prende. Il primo argomento è il valore dell'array all'iterazione corrente, quindi `newArray` sarebbe `[value: 1, value: 2, value: 3]`.

Passa un riferimento alla tua funzione `elemValue` come callback al metodo `.map()`.

# --hints--

Non dovresti chiamare la tua funzione `elemValue`.
...
Dovresti passare un riferimento a `elemValue` come callback al metodo `.map()`.
