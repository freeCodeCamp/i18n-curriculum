...
Тепер настав час застосувати те, що ви дізналися про стрілкові функції, до проєкту музичного програвача.

Почніть із видалення функцій `printGreeting`, `printMessage` та `addTwoNumbers`. Також видаліть усі консольні інструкції та виклики функцій.

# --hints--

Ви не повинні мати функцію `printGreeting`.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

Ви не повинні мати функцію `printMessage`.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

Ви не повинні мати функцію `addTwoNumbers`.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

Ви не повинні мати виклик функції `printGreeting();`.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

Ви не повинні мати виклик функції `printMessage('freeCodeCamp');`.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

Код не повинен мати консольну інструкцію `console.log(addTwoNumbers(3,4));`.
