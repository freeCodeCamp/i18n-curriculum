...
你已将段落元素的 `id` 属性设置为 `obj.inputVal` 属性。

现在，使用 `.getElementById()` 方法选择具有该属性值的元素，同样使用 `obj.inputVal` 属性。你应该将 `obj.inputVal` 值直接传递给你的 `getElementById` 调用。

之后，将选定元素的 `textContent` 属性设置为等于当前对象的 `msg` 属性，以便在你之前指定的延迟之后更新其文本。

# --hints--

你应该使用 `.getElementById()` 方法来定位 `id` 属性与当前对象的 `inputVal` 值匹配的元素。
...
你应该将 `.textContent` 链接到 `.getElementById()` 方法。
...
你应该将目标元素的 `textContent` 属性设置为等于当前对象的 `msg` 属性。
