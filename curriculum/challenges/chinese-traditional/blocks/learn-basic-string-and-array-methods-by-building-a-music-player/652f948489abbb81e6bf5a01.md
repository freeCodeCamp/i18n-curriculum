...
在這個專案中，你將透過建置一個音樂播放器應用程式來學習基本的字串和陣列方法。你將能夠播放、暫停、跳過和隨機播放歌曲。

這個專案的 HTML 和 CSS 已經為你提供，所以你可以專注於 JavaScript。

首先使用 `getElementById()` 方法存取 `#playlist-songs`、`#play` 和 `#pause` 元素。分別將它們指定給變數 `playlistSongs`、`playButton` 和 `pauseButton`。


# --hints--

你應該有一個名為 `playlistSongs` 的變數。 

```js
assert.isDefined(playlistSongs);
```

你應該使用 `document.getElementById()` 來取得 `#playlist-songs` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

你應該將 `#playlist-songs` 元素指定到變數 `playlistSongs`。 

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

你應該有一個名為 `playButton` 的變數。 

```js
assert.isDefined(playButton);
```

你應該使用 `document.getElementById()` 來取得 `#play` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

你應該將 `#play` 元素指定到變數 `playButton`。 

```js
assert.equal(playButton, document.getElementById('play'));
```

你應該有一個名為 `pauseButton` 的變數。 

```js
assert.isDefined(pauseButton);
```

你應該使用 `document.getElementById()` 來取得 `#pause` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

你應該將 `#pause` 元素指定到變數 `pauseButton`。 
