...
這個遊戲的目標是將四個數字排列成一種方式，使得計算結果為 24。

# --instructions--

實作一個函式，該函式以一個包含四個數字的字串作為引數，每個數字介於 1 到 9（含）且允許重複，並傳回一個運算式，該運算式的計算結果為 24。如果不存在此類解法，則傳回 "no solution exists"。
...
`solve24("4878")` 應該傳回 `(7-8/8)*4`、`4*(7-8/8)`，或類似的有效字串

```js
assert(isValidSolution_(solve24(testCases_[0])));
```

`solve24("1234")` 應該傳回 `1*2*3*4` 或類似的有效字串

```js
assert(isValidSolution_(solve24(testCases_[1])));
```

`solve24("6789")` 應該傳回 `(6*8)/(9-7)`、`(8*6)/(9-7)` 或類似的有效字串

```js
assert(isValidSolution_(solve24(testCases_[2])));
```

`solve24("1127")` 應該傳回 `(1+7)*(1+2)` 或類似的有效字串
