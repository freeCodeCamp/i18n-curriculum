...
Нарешті, оновіть вашу внутрішню функцію так, щоб вона повертала третю функцію. Ця третя функція також має приймати один аргумент з іменем `quantity` і мати порожнє тіло.

# --hints--

Ваша друга функція має повертати ще одну функцію.

```js
assert.isFunction(convertQuantity()());
```

Ваша найвнутрішніша функція має мати параметр `quantity`.

```js
assert.match(convertQuantity()()?.toString(), /\(quantity\)/);
```

Ваша найвнутрішніша функція має бути порожньою.
