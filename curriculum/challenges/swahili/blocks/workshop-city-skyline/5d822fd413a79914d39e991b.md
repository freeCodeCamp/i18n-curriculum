...
Mpe `.fb2a` `width` ya `100%` na `.fb2b` `width` ya `100%` na `height` ya `75%`.

# --hints--

Unapaswa kumpa `.fb2a` `width` ya `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.width, "100%");
```

Unapaswa kumpa `.fb2b` `width` ya `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2b")?.width, "100%");
```

Unapaswa kumpa `.fb2b` `height` ya `75%`.
