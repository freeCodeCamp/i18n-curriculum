...
CSV (valores separados por comas) es un formato usado para hojas de cálculo e importación de datos. Cada línea representa una entrada con campos separados por comas.

Comienza eliminando la llamada `console.log(exportToJSON(...))`, ya que solo se usaba para pruebas. Luego crea una función llamada `exportToCSV` que reciba `catalog` como parámetro. Dentro, declara `const header = "Title,Author,Year,Location"` y `const rows = []`.

# --hints--

Debes eliminar la llamada `exportToJSON` console.log.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

Debes tener una función llamada `exportToCSV` con un parámetro `catalog`.
...
Debes declarar una variable llamada `header` con valor `"Title,Author,Year,Location"`.

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

Debes declarar una variable llamada `rows` asignada a un arreglo vacío.
