...
Por fim, você precisará retornar a imagem para o avatar do usuário.

Comece adicionando um `return` seguido por um conjunto de template literals. Dentro dos template literals, adicione um elemento `img`. 

Dentro da tag `img`, adicione um atributo `src` com o valor de `${userAvatarUrl}`. Para o atributo `alt`, adicione um valor de `${user.name}`.
...
Você deve retornar um template literal.
...
Você deve ter um elemento `img` dentro do template literal.
...
Você deve ter um atributo `src` dentro da tag `img`.
...
Você deve definir o valor do atributo `src` para a constante chamada `userAvatarUrl`.
...
Você deve ter um atributo `alt` após o atributo `src`.
...
Você deve definir o valor do atributo `alt` para a constante chamada `user.name`.
