...
Da Sie nun wissen, zu welchem Gate Sie die Überlaufteilnehmenden umleiten müssen, können Sie diese verschieben.

Nehmen Sie das `overflowAmount` und fügen Sie es in die Warteschlange des nächsten Gates an derselben `tickIndex` ein.

Konkret müssen Sie:

- Auf das nächste Gate mit `nextGateIndex` zugreifen.
- Auf dessen `queue` zugreifen.
- Das `overflowAmount` zum Wert an der Position `tickIndex` hinzufügen.

# --hints--

Ihre `rerouteOverflow`-Funktion sollte `overflowAmount` zum Wert bei `tickIndex` im `queue` des nächsten Gates hinzufügen.
