...
يمكنك تحسين قابلية قراءة الشفرة وصيانتها بإعادة هيكلة مستمع حدث `submit` إلى دالتين منفصلتين. يمكن استخدام الدالة الأولى لإضافة قيم المُدخَل إلى `taskData`، بينما تكون الدالة الثانية مسؤولة عن إضافة المهام إلى DOM.

استخدم صيغة السهم لإنشاء دالة `addOrUpdateTask`. ثم انقل المتغير `dataArrIndex`، والكائن `taskObj`، وعبارة `if` إلى داخل دالة `addOrUpdateTask`.

# --hints--

يجب أن تستخدم `const` وصيغة السهم لإنشاء دالة `addOrUpdateTask`.

```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

يجب أن تنقل المتغير `dataArrIndex` إلى داخل دالة `addOrUpdateTask`.
...
يجب أن تنقل الكائن `taskObj` إلى داخل دالة `addOrUpdateTask`.
...
يجب أن تنقل عبارة `if` التي تحتوي على الشرط `dataArrIndex === -1` إلى داخل دالة `addOrUpdateTask`.
