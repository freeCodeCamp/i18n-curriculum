...
오른쪽 귀를 `position` 속성을 `absolute`로, `top`를 `-26px`로, 그리고 `left`를 `163px`로 설정하여 위치시키세요.

# --hints--

`.cat-right-ear` 선택자에 `position` 속성을 `absolute`로 설정해야 합니다. 세미콜론을 잊지 마세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.position, 'absolute')
```

`.cat-right-ear` 선택자에 `top` 속성을 `-26px`로 설정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.top, '-26px')
```

`.cat-right-ear` 선택자에 `left` 속성을 `163px`로 설정해야 합니다.
