...
既然你已經有了應用程式的基本結構，現在是時候為你的應用程式添加一些項目並讓搜尋功能運作。

創建一個陣列並將它指定給名為 `items` 的變數。在陣列中，為你的陣列添加字串 `Apples`、`Bananas`、`Strawberries`、`Blueberries`、`Mangoes`、`Pineapple`、`Lettuce`、`Broccoli`、`Paper Towels` 和 `Dish Soap`。這將會是應用程式中要顯示的項目列表。
...
你的 `items` 陣列應該包含字串 `Apples`、`Bananas`、`Strawberries`、`Blueberries`、`Mangoes`、`Pineapple`、`Lettuce`、`Broccoli`、`Paper Towels` 和 `Dish Soap`。
