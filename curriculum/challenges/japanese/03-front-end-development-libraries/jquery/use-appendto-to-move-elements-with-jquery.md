...
title: jQuery で appendTo を使用して要素を移動する
challengeType: 6
forumTopicId: 18340
...
たとえば、右のウェルから左のウェルに `target4` を移動するには、次のようにします。

```js
$("#target4").appendTo("#left-well");
```

`left-well` から `right-well` に `target2` 要素を移動してください。

# --hints--

`target2` 要素を `left-well` の中に入れません。

```js
assert($('#left-well').children('#target2').length === 0);
```

`target2` 要素を `right-well` の中に入れます。
