...
就像你的 `locations` 数组一样，你的 `weapons` 数组将保存对象。 向 `weapons` 数组添加四个对象，每个对象具有两个属性：`name` 和 `power`。 第一个应该将 `name` 设置为 `"stick"`，将 `power` 设置为 `5`。 第二个应该是 `"dagger"` 和 `30`。 第三个，`"claw hammer"` 和 `50`。 第四个，`"sword"` 和 `100`。
...
你的第一个 `weapons` 对象应该将 `name` 设置为 `"stick"`，并将 `power` 设置为 `5`。
...
你的第二个 `weapons` 对象应将 `name` 设置为 `"dagger"`，并将 `power` 设置为 `30`。
...
你的第三个 `weapons` 对象应将 `name` 设置为 `"claw hammer"`，并将 `power` 设置为 `50`。
...
你的第四个 `weapons` 对象应该将 `name` 设置为 `"sword"`，并将 `power` 设置为 `100`。
