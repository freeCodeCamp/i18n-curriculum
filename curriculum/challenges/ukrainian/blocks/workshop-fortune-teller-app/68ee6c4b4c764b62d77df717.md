...
Іноді карти потрібно перевертати, оскільки вони не завжди знаходяться у правильній позиції.

Оголосіть змінну з назвою `isReversed` всередині методу `singleCardSelected`. Використайте метод `Math.random()` для генерації випадкового булевого значення. Щоб переконатися, що значення не виходить за межі, порівняйте його з `0.5`.

# --hints--

Ви повинні оголосити змінну з назвою `isReversed`.
