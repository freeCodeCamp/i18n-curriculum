...
在 `card-stats` div 裡，第一個 `stat-col` 之後，你需要添加兩個子元素。

首先，一個自閉合的 `div`，其 `className` 為 `stat-divider`，用來在兩個統計行之間呈現一條細的垂直線；接著是第二個 `div`，其 `className` 為 `stat-col`，包含三個 `div` 元素，每個都有 `className` 為 `stat-row`。

# --hints--

你應該在第一個 `div` 之後添加一個自閉合的 `className`，其 `stat-divider` 為 `.stat-col`。

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

你應該在 `div` 之後添加第二個 `className`，其 `stat-col` 為 `.stat-divider`。

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

第二個 `.stat-col` 應該包含三個 `div` 元素，每個都有 `className` 為 `stat-row`。
