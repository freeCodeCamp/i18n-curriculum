...
Тепер час додати посилання на файл JavaScript.

Додайте елемент `script` безпосередньо перед кінцевим тегом елемента `body` з правильним атрибутом і значенням, щоб підключити файл `script.js`.
