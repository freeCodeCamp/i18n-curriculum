...
Maps と Sets は、名前と目的が同じいくつかのメソッドを共有しています。たとえば、セットに特定の要素が含まれているかどうかを確認するために `has` メソッドを使うことができます。

`has` メソッドを使って、`ballerina` オブジェクトが `plantsSet` に含まれているかどうかを確認し、その結果をコンソールに出力してください。

# --hints--

`plantsSet.has(ballerina)` をコンソールに出力してください。
