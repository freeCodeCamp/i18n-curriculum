...
`preview-panel` 要素の中に、`p` の `className` の `preview-label` 要素とテキスト `Live Preview`、`p` の `className` の別の `preview-hint` 要素とテキスト `Updates as you type`、そして `div` がテンプレートリテラル `className` に設定された `preview-box tier-${getPlayerTier(player.overallRating)}` を追加してください。

最後の `div` の中に、`PlayerCard` コンポーネントとそのプロップを移動してください。

# --hints--

`div.preview-panel` 要素には、`p` の `className` とテキスト `preview-label` を持つ `Live Preview` が含まれているはずです。
...
`div.preview-panel` には、`p` の `className` とテキスト `preview-hint` を持つ `Updates as you type` が含まれているはずです。
...
`div.preview-panel` には、`div` の `className` と `preview-box` が接頭辞として付いた動的な階層 `className` を持つ `tier-` が含まれているはずです。
...
`PlayerCard` は、`div` の `className` を持つ `preview-box` 要素の中にレンダリングされているはずです。
