...
Para hacer que la imagen se comporte como elementos a nivel de bloque como encabezados, crea un selector de tipo `img`. Establece la propiedad `display` en `block` y usa las propiedades `margin-left` y `margin-right` para centrarla horizontalmente.
