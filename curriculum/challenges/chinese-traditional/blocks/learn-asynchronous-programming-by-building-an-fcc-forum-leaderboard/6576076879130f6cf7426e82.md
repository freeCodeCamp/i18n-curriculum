...
每個論壇主題都包含像 Python 或 JavaScript 這樣的分類。在接下來的幾個步驟中，你將建置一個分類物件，該物件包含所有論壇分類和用於樣式的 `classNames`。 

首先建立一個名為 `allCategories` 的新常數，並將其指定為空物件的值。 

# --hints--

你應該有一個名為 `allCategories` 的常數。

```js
assert.match(code, /const\s+allCategories\s*=/)
```

你應該將一個空的物件指定給 `allCategories` 變數。
