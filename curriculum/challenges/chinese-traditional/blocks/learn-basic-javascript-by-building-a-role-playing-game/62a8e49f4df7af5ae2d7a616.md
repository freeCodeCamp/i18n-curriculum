...
將 `monsterHealth` 設為 `monsterHealth` 減去玩家當前武器的威力。 

請記住，你可以使用 `weapons[currentWeaponIndex].power` 來存取玩家當前武器的威力。

# --hints--

你應該使用複合指派來修改 `monsterHealth` 變數。

```js
assert.match(attack.toString(), /monsterHealth\s*-=/);
```

你應該使用括號表示法和 `currentWeaponIndex` 來存取 `weapons`。

```js
assert.lengthOf(attack.toString().match(/weapons\s*\[\s*currentWeaponIndex\s*\]/g), 2);
```

你應該使用點記法來存取 `weapons[currentWeaponIndex]` 的 `power` 屬性。
