...
Перш ніж почати працювати з відтворенням наступної та попередньої пісні, потрібно отримати індекс кожної пісні у властивості `songs` об’єкта `userData`. Для цього можна використати метод масиву `indexOf()`, який повертає перший індекс, за яким заданий елемент можна знайти в масиві, або `-1`, якщо елемент відсутній.
...
Створіть функцію з назвою `getCurrentSongIndex`, яка повертає індекс, за яким `userData.currentSong` можна знайти в `userData.songs` і `-1`, коли поточна пісня — `null`.

# --hints--

У вас має бути функція з назвою `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Ваша функція `getCurrentSongIndex` має повертати індекс, за яким `userData.currentSong` можна знайти в `userData.songs`.
...
Ваша функція `getCurrentSongIndex` має повертати `-1`, коли `userData.currentSong` дорівнює `null`.
