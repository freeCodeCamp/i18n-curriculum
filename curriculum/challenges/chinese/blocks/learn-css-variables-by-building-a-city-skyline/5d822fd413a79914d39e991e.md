...
将 `.fb2-window` 元素的 `width` 设置为 `22%`，`height` 设置为 `100%`，`background-color` 设置为变量 `--window-color3`。

# --hints--

你应该给 `.fb2-window` 元素设置一个值为 `22%` 的 `width`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

你应该给 `.fb2-window` 元素设置一个值为 `100%` 的 `height`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

你应该给 `.fb2-window` 元素设置一个值为 `--window-color3` 的 `background-color`。
