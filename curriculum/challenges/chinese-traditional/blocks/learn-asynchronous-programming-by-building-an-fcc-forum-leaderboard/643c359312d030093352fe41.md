...
首先建立一個名為 `showLatestPosts` 的箭頭函式（arrow function），它接受一個單一的 `data` 參數。

# --hints--

你應該有一個名為 `showLatestPosts` 的箭頭函式（程式）。

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

你的 `showLatestPosts` 函式應該有一個 `data` 參數。

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

應該定義 `showLatestPosts`。

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts` 應該是一個函式（程式）。
