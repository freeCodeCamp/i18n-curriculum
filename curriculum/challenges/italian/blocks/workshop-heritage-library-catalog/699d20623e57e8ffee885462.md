...
È ora di vedere il validatore individuare un problema reale. `catalog[0]` è "From a Buick 8" di Stephen King, che ha tutti e quattro i campi compilati. `catalog[16]` è "Unknown Title", che ha il campo autore vuoto.

Registra in console `validateEntry(catalog[0])` e `validateEntry(catalog[16])`. Il primo dovrebbe restituire `true`; il secondo dovrebbe restituire `false`.

# --hints--

Dovresti registrare `validateEntry(catalog[0])` nella console.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

Dovresti registrare `validateEntry(catalog[16])` nella console.
