...
Verwenden Sie die Pfeilsyntax, um eine `updateTaskContainer`-Funktion zu erstellen. Verschieben Sie dann die `taskData.forEach()` und deren Inhalt aus dem `taskForm`-`submit`-Ereignis-Listener in die neu erstellte Funktion.

# --hints--

Sie sollten `const` und die Pfeilsyntax verwenden, um eine `updateTaskContainer`-Funktion zu erstellen.

```js
assert.match(code, /const\s+updateTaskContainer\s*=\s*\(\s*\)\s*=>\s*\{/)
```

Sie sollten `taskData.forEach()` und dessen Inhalt in die `updateTaskContainer()`-Funktion verschieben.
