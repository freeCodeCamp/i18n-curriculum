...
Видаліть масив `myTaskArr` і весь код для `localStorage`, оскільки вони більше не потрібні.

# --hints--

Ви маєте видалити `myTaskArr` і весь код, пов’язаний із `localStorage`, який ви щойно вивчили.
...
Ви маєте видалити всі залишкові референси на `myTaskArr` у будь-якому місці коду.

```js
assert.notMatch(code, /myTaskArr/);
```

Ви маєте видалити будь-які референси на `localStorage.getItem` для елемента `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Ви маєте видалити всі залишкові референси на `getTaskArrObj` у будь-якому місці коду.
