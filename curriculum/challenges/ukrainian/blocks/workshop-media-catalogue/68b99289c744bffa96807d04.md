...
Наразі фільми та TV серіали відображаються разом, коли ви виводите каталог. У наступних кроках ви подбаєте про фільтрацію елементів у дві категорії для окремого відображення.

Всередині класу `MediaCatalogue` створіть метод з назвою `get_movies` і зробіть так, щоб він повертав список, що містить лише екземпляри батьківського класу `Movie`, які можна знайти в `self.items`.

Використовуйте будь-який підхід, який вам подобається, але пам’ятайте, що потрібно застосовувати функцію `type`, оскільки `isinstance` не зможе розрізнити батьківські та дочірні класи.

# --hints--

Ваш клас `MediaCatalogue` має містити метод з назвою `get_movies`.
...
Ваш метод `get_movies` має повертати список, що містить усі елементи в `self.items`, які є екземплярами класу `Movie`.
