...
次のいくつかのステップで、そのコールバック関数内の各 `if` 文が何をしているのかを学びます。ですが今は、空のコールバック関数を `sort()` メソッドに追加し、パラメータ名に `a` と `b` を使ってください。

# --hints--

`()=>{}` メソッドの中に、パラメータ名として `sort()` と `a` を使った空のコールバック関数 `b` があるはずです。
