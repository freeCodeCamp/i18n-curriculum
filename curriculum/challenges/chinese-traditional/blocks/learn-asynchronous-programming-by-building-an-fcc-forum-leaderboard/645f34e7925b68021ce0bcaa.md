...
下一步是從 `users` 陣列中找到正確的使用者。

首先建立一個名為 `user` 的常數，並指定其為 `users.find()`。`find` 方法應該有一個帶有 `user` 參數的回呼函式（函式）。 

在 `find` 方法的回呼函式中，隱式傳回檢查 `user.id` 是否嚴格相等於 `poster.user_id` 的結果。
