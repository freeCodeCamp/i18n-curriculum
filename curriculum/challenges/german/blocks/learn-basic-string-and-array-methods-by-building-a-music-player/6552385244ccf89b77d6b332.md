...
Sie müssen daran arbeiten, den nächsten und den vorherigen Song abzuspielen. Dafür benötigen Sie eine `playNextSong`- und eine `playPreviousSong`-Funktion.

Verwenden Sie `const` und die Pfeilsyntax, um eine leere `playNextSong`-Funktion zu erstellen.

# --hints--

Sie sollten `const` verwenden, um eine leere Funktion mit dem Namen `playNextSong` zu erstellen.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` sollte eine Funktion sein.

```js
assert.isFunction(playNextSong)
```

Ihre `playNextSong`-Funktion sollte die Pfeilsyntax verwenden.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Ihre `playNextSong`-Funktion sollte leer sein.
