...
Unda `div` mpya chini ya kipengele chako cha `.large` na kipa sifa ya kipengele ya `class` iliyowekwa kuwa `calories-info`.
...
`div` mpya unayounda inapaswa kuwa na sifa ya kipengele ya `class` iliyowekwa kuwa `calories-info`.

```js
assert(document.querySelector('.label')?.lastElementChild?.classList?.contains('calories-info'));
```

`div` mpya unayounda inapaswa kuja baada ya kipengele cha `.large`.
