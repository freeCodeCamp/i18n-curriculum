...
All'interno del tuo elemento `aside`, crea due elementi `img`, un elemento `blockquote` e un terzo elemento `img`. Dai all'elemento `blockquote` un insieme `class` impostato su `image-quote`.
...
Dovresti creare un elemento `blockquote` all'interno del tuo elemento `aside`.

```js
assert.exists(document.querySelector('aside blockquote'));
```

Il tuo elemento `blockquote` dovrebbe avere un insieme `class` impostato su `image-quote`.

```js
assert(document.querySelector('aside blockquote')?.classList?.contains('image-quote'));
```

I tuoi nuovi elementi dovrebbero essere nell'ordine corretto.
