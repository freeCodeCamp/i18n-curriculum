...
La función `setPlayButtonAccessibleText` establecerá el atributo `aria-label` a la cadena actual, o a la primera cadena de la lista de reproducción. Y si la lista de reproducción está vacía, establece el `aria-label` en `"Play"`.

Utiliza el método `setAttribute` en el elemento `playButton` para establecer un atributo llamado `"aria-label"`. Usando un ternario, establece el valor del atributo en `Play ${song.title}` o `"Play"` si `song?.title` no está disponible.

No olvides que necesitas interpolación de cadenas aquí, por lo que debes usar acentos graves.

# --hints--

No debes modificar la función `setPlayButtonAccessibleText` existente y su contenido.
...
Debes usar el método `setAttribute()` en `playButton`.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` debe ser el primer valor de tu método `setAttribute()`.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

Tu método `setAttribute` debe tener ``song?.title ? `Play ${song.title}` : "Play"`` como el segundo argumento.
