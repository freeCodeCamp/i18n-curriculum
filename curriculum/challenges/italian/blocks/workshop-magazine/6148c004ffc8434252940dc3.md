...
Crea un selettore `.social-icons`. Dagli una proprietà `display` impostata su `grid` e una proprietà `font-size` impostata su `3rem`.

# --hints--

Dovresti avere un selettore `.social-icons`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

Il tuo selettore `.social-icons` dovrebbe avere una proprietà `display` impostata su `grid`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

Il tuo selettore `.social-icons` dovrebbe avere una proprietà `font-size` impostata su `3rem`.
