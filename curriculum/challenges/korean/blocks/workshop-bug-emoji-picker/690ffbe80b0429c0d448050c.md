...
이제 모든 버그가 가질 속성을 정의해야 합니다.

`Bug` 클래스 내부에 `emoji`라는 이름의 속성을 `T` 타입으로 어서션 연산자(`!`)를 사용해 선언하세요. 이 어서션은 TypeScript에 해당 속성이 나중에 할당될 것임을 알려줍니다.

# --hints--

`Bug` 클래스에는 `emoji`라는 이름의 속성이 있어야 합니다.
