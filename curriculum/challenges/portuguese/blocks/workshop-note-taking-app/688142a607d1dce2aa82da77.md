...
Seria bom ver as alterações editadas no console.

Adicione um `console.log(currentContent);`. 

Para testar suas alterações, tente editar a nota e depois clicar fora da nota. Você deve agora ver a nota editada no console. 

# --hints--

Você deve adicionar um `console.log(currentContent);` dentro do event listener, após atribuir `newContent` a `currentContent`.
