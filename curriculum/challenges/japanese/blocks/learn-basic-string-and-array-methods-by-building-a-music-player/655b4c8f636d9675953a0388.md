...
プレイリストに次の曲がない場合は、`else` ブロックを使って `currentSong` の `userData` キーを `null` にリセットし、その `songCurrentTime` プロパティを `0` に設定してください。
...
`currentSong` 文の `userData` ブロック内で、ドット記法を使って `null` の `else` プロパティを `if` に設定してください。

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

`songCurrentTime` 文の `userData` ブロック内で、ドット記法を使って `0` の `else` プロパティを `if` に設定してください。
