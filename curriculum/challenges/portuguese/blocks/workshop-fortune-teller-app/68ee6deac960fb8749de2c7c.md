...
Inclua os argumentos restantes `chosenCard.name_short` e `chosenCard.img`. Depois, fora da função `renderCard`, redefina o `multipleCard` do elemento `innerHTML` para uma string vazia.

# --hints--

Você deve passar `chosenCard.name_short` como argumento para `renderCard`.
...
Você deve passar `chosenCard.img` como último argumento para `renderCard`.
...
Você deve redefinir `this.elements.multipleCard.innerHTML` para uma string vazia.
