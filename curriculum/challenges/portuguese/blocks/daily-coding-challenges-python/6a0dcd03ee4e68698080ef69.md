...
Dado um array 2D onde cada array interno contém (nessa ordem) um nome de ideia, uma estimativa otimista, uma estimativa realista e uma estimativa pessimista (em dias), retorne um array com os nomes das ideias ordenados pelo tempo esperado para conclusão, do menor para o maior.
...
`analyze_ideas([["Add logging", 2, 5, 15], ["SEO optimization", 4, 8, 20], ["Fix bug", 1, 3, 5]])` deve retornar `["Fix bug", "Add logging", "SEO optimization"]`.
...
`analyze_ideas([["Dark mode", 1, 3, 8], ["Real-time collaboration feature", 6, 12, 20], ["Add tooltip", 1, 2, 4]])` deve retornar `["Add tooltip", "Dark mode", "Real-time collaboration feature"]`.
...
`analyze_ideas([["Update user profile page", 3, 7, 14], ["Add pagination", 2, 5, 10], ["Add tags", 2, 3, 6], ["Fix login bug", 1, 4, 8]])` deve retornar `["Add tags", "Fix login bug", "Add pagination", "Update user profile page"]`.
...
`analyze_ideas([["Migrate database", 14, 25, 40], ["Add chat assistant", 8, 15, 24], ["Redesign onboarding flow", 3, 7, 13], ["Add language support", 6, 11, 18]])` deve retornar `["Redesign onboarding flow", "Add language support", "Add chat assistant", "Migrate database"]`.
