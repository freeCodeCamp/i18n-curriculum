...
`label` 要素に、`for` 属性を `X-#-name` の値で付けてください。ここで `X` は `value` 要素の `entryDropdown` であり、`#` は `entryNumber` の値です。HTMLの属性は二重引用符で囲む必要があることを忘れないでください。

# --hints--

`label` 要素には `for` 属性が `${entryDropdown.value}-${entryNumber}-name` に設定されている必要があります。

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>/)
```

`label` 要素は以前と同じテキストである必要があります。
