...
Ora l'algoritmo di posizionamento automatico entrerà in funzione quando aggiungi un nuovo elemento icona. Tuttavia, l'algoritmo imposta per default la larghezza della nuova colonna a `auto`, che non corrisponderà alle tue colonne attuali.

Puoi sovrascrivere questo comportamento con la proprietà `grid-auto-columns`. Imposta al selettore `.social-icons` la proprietà `grid-auto-columns` con valore `1fr`.

# --hints--

Il tuo selettore `.social-icons` dovrebbe avere la proprietà `grid-auto-columns` impostata su `1fr`.
