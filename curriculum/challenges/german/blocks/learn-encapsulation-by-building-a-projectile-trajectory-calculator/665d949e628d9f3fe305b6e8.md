...
Die Ausgabe wird zunehmend schwer lesbar, daher sollten Sie `print(graph.create_trajectory())` durch eine Schleife ersetzen, die jede Zeile einzeln ausgibt.

# --hints--

Entfernen Sie den `print(graph.create_trajectory())`-Aufruf.
...
Sie sollten eine Schleife haben, die über `graph.create_trajectory()` iteriert.
...
Sie sollten jede Zeile in `graph.create_trajectory()` ausgeben.
