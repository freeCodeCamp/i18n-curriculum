...
잡지 레이아웃이 완성되었으니, 이제 반응형으로 만드세요.

`@media`에 `only screen` 값의 `max-width`를 가진 `720px` 쿼리부터 시작하세요. 그 안에 `.image-wrapper` 선택자를 만들고 `grid-template-columns` 값을 가진 `1fr` 속성을 주세요.

이렇게 하면 작은 화면에서 세 개의 이미지가 한 열로 축소됩니다.

# --hints--

`@media`에 대한 새로운 `only screen and (max-width: 720px)` 규칙이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

새로운 `@media` 규칙에는 `.image-wrapper` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

새로운 `.image-wrapper` 선택자에는 `grid-template-columns` 값을 가진 `1fr` 속성이 있어야 합니다.
