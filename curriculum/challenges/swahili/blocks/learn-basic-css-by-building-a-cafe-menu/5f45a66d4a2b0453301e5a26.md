...
Sogelea chini hadi kipengele cha `footer`, fanya maandishi yote yawe na thamani ya `14px` kwa ukubwa wa fonti.

# --hints--

Unapaswa kuwa na kichaguzi cha `footer`.
...
Kichaguzi chako cha `footer` kinapaswa kuwa chini ya maoni yako.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Unapaswa kuweka sifa ya `font-size` kuwa `14px`.
...
Kipengele chako cha `footer` kinapaswa kuwa na `font-size` ya `14px`.
