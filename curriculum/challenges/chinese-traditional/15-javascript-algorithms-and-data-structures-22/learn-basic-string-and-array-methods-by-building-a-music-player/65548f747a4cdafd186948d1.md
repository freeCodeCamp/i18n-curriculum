...
你需要獲取當前正在播放的歌曲的 `id`。 爲此，你可以使用 `userData?.currentSong?.id`。

使用 `getElementById()` 獲取當前播放的歌曲的 `id`，然後使用模板文字在其前面加上 `song-`。 將其分配給常量 `songToHighlight`。

# --hints--

你不應修改現有的 `highlightCurrentSong` 函數及其內容。
...
你應該將 `getElementById()` 分配給 `songToHighlight` 常量。
