...
Infine, i pulsanti possono essere distanziati meglio tra loro e rispetto alla parte superiore. Per farlo, devi prendere di mira l'elemento `div` che li racchiude entrambi. Quindi, aggiungi le classi `flex`, `gap-2` e `mt-6` a quel `div`.

Con questo, il tuo workshop sul componente CTA è completo!

# --hints--

L'elemento `div` che racchiude gli elementi àncora dovrebbe avere la classe di utilità `flex`.
...
L'elemento `div` che racchiude gli elementi àncora dovrebbe avere la classe di utilità `gap-2`.
...
L'elemento `div` che racchiude gli elementi àncora dovrebbe avere la classe di utilità `mt-6`.
