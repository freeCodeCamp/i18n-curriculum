...
開発中は、`fieldset` のデフォルトのボーダーを見るのが便利です。しかし、それらはコンテンツが離れすぎて見える原因になります。

`border` を削除し、各 `2rem` の上下にだけ `fieldset` のパディングを追加してください。左右の `padding` は必ず削除してください。

# --hints--

`none` を削除するには、`0` または `border` の値を使えます。

```js
assert.match(new __helpers.CSSHelp(document).getStyle('fieldset')?.border, /(none)|(0px)|(medium)/);
```

各 `padding` の上下に `2rem` の `fieldset` を追加してください。
...
各 `padding` の左右から `fieldset` を削除してください。
