...
fortune 앱에는 과거, 현재, 미래 세 가지 순간만 있으므로 `DrawingType`라는 이름의 enum을 선언하고 `Past`, `Present`, `Future` 세 가지 속성을 만드세요. 각 속성은 자신의 이름과 일치하는 문자열 값을 가져야 합니다.

# --hints--

`enum`라는 이름의 `DrawingType`가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

enum에는 문자열 값이 `Past`인 `past` 속성이 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

enum에는 문자열 값이 `Present`인 `present` 속성이 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

enum에는 문자열 값이 `Future`인 `future` 속성이 있어야 합니다.
