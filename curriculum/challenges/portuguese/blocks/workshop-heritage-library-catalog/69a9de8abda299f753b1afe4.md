...
Adicione um loop `for` sobre `catalog`. Dentro do loop, declare `const entry = catalog[i]`. Construa uma string de linha com os quatro campos separados por vírgulas e adicione-a em `rows`.

Campos string precisam de caracteres literais de aspas duplas na saída (por exemplo, `"King, Stephen"`), para que vírgulas dentro dos valores não quebrem o formato CSV. Como `year` é um número, ele não precisa delas.
...
Cada linha deve ter os campos da entrada separados por vírgulas.

```js
assert.match(exportToCSV.toString(), /rows\.push/);
```

Campos string devem estar entre aspas duplas.
