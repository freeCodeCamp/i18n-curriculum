...
Замініть усі екземпляри `parseInt(numberInput.value)` на `inputInt`.

# --hints--

Замініть `isNaN(parseInt(numberInput.value))` на `isNaN(inputInt)` та `parseInt(numberInput.value) < 0` на `inputInt < 0`.
...
Замініть `parseInt(numberInput.value) === 5` на `inputInt === 5`.
...
Замініть `decimalToBinary(parseInt(numberInput.value))` на `decimalToBinary(inputInt)`.
...
Функція `checkUserInput()` має містити лише один екземпляр `parseInt(numberInput.value)`.
