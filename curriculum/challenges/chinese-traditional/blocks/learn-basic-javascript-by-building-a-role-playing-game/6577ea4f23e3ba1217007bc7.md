...
最後，你將需要為你的按鈕添加一些樣式。首先將 `cursor` 屬性設為 `pointer`。然後將文字 `color` 設為 `#0a0a23`，並將 `background-color` 設為 `#feac32`。

然後將 `background-image` 屬性設定為 `linear-gradient(#fecc4c, #ffac33)`。最後，將 `border` 設定為 `3px solid #feac32`。
...
你的 `button` 選擇器應該有設定 `cursor` 屬性為 `pointer`。
...
你的 `button` 選擇器應該設定 `color` 屬性為 `#0a0a23`。
...
你的 `button` 選擇器應該設定 `background-color` 屬性為 `#feac32`。
...
你的 `button` 選擇器應該設定 `background-image` 屬性為 `linear-gradient(#fecc4c, #ffac33)`。
...
你的 `button` 選擇器應該設定 `3px solid #feac32` 的 `border` 屬性。
