...
1. `CurrencyConverter` コンポーネントは、変換する金額を受け取るための `input` 要素をレンダリングする必要があります。
2. `input` 要素は数字を受け付ける必要があります。
3. `CurrencyConverter` コンポーネントは、変換元と変換先の通貨を選択するための2つの `select` 要素をレンダリングする必要があります。
4. `select` 要素には、少なくとも `USD`、`EUR`、`GBP`、`JPY` のオプションが含まれている必要があります。通貨間で1対1の対応関係がない限り、任意の為替レートを使用できます。以下は良いマッピングと悪いマッピングの例です:
...
5. `CurrencyConverter` コンポーネントは、変換元通貨の変換金額の計算をメモ化し、変換先の `select` オプションの変更によって変換金額が再計算されないようにする必要があります。
6. `CurrencyConverter` コンポーネントは、`XX.XX CCC` の形式で変換後の金額を表示する要素をレンダリングする必要があります。ここで `XX.XX` は変換後の金額、`CCC` は通貨コードです。
7. 変換後の金額は小数点以下2桁に丸める必要があります。

# --hints--

`CurrencyConverter` コンポーネントをエクスポートしてください。
...
変換する金額を受け取るための `input[type="number"]` 要素が1つ必要です。
...
`select` 要素には、少なくとも `USD`、`EUR`、`GBP`、`JPY` のオプションが必要です。
...
最初の `select` 要素の値を変更すると、変換金額が再計算される必要があります。
...
2番目の `select` 要素の値を変更しても、変換金額は再計算されません。
...
最初の `select` 要素の値を変更すると、新しい変換金額が表示される必要があります。
...
2番目の `select` 要素の値を変更すると、新しい変換金額と通貨が表示される必要があります。
...
変換後の金額は `XX.XX CCC` の形式で表示される必要があります。ここで `XX.XX` は小数点以下2桁に丸められた変換金額、`CCC` は通貨コードです。
