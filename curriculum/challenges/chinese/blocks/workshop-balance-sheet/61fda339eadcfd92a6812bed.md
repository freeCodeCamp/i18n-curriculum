...
在设计更多样式之前，你应该使用 `sr-only` 类。 你可以使用 CSS 使具有此类的元素在可视页面中完全隐藏，但仍可由屏幕阅读器朗读。

你将要编写的 CSS 是一组通用属性，用于确保元素在视觉上完全隐藏。

`span[class~="sr-only"]` 选择器将选择任何 `class` _包括_ `sr-only` 的 `span` 元素。 创建该选择器，并将其 `border` 属性设置为 `0`。
...
你的 `span[class~="sr-only"]` 选择器应该有一个 `border` 属性，设置为 `0`。
