...
Finalmente, dentro de `hideElements`, pasa los argumentos `singleCard`, `multipleCard`, `fortuneContainer` y `fortuneDescription`. Recuerda obtenerlos de los elementos.

¡Con eso, tu aplicación de adivinación está completa!

# --hints--

Debes pasar `singleCard` como primer argumento a `hideElements`.
...
Debes pasar `multipleCard` como argumento a `hideElements`.
...
Debes pasar `fortuneContainer` como argumento a `hideElements`.
...
Debes pasar `fortuneDescription` como argumento a `hideElements`.
