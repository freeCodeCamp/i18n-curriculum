...
在 `try` 块内，使用 `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` URL 调用 `fetch` 函数，并将结果保存到名为 `response` 的变量中。由于你在一个 `async` 函数内，应在 `fetch` 前使用 `await`。

# --hints--

你应该使用 `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` 调用 `fetch` 并将结果保存到 `response` 变量中。
