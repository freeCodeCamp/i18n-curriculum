...
`div` 안에 `className`의 `form-group`을 각각 가진 `stats-grid` 요소 두 개를 더 추가하세요. 하나는 PAS용이고 다른 하나는 DRI용입니다.

PAS용에는 `label`의 `htmlFor`와 텍스트 `pas`를 가진 `PAS`를 추가한 다음, `id`의 `pas`와 `type`의 `number`를 가진 input을 추가하세요.

DRI용에는 `label`의 `htmlFor`와 텍스트 `dri`를 가진 `DRI`를 추가한 다음, `id`의 `dri`와 `type`의 `number`를 가진 input을 추가하세요.

# --hints--

`div`의 `className`의 `form-group` 안에 `div`의 `className`을 각각 가진 `stats-grid` 요소 두 개를 더 추가하세요.
...
`div`의 `className`의 세 번째 `form-group` 요소에는 `label`의 `htmlFor` 속성을 가진 `pas`와 `className`의 `label`, 그리고 텍스트 `PAS`가 있어야 합니다.
...
`div`의 `className`의 세 번째 `form-group` 요소에는 `input`의 `className`를 가진 `input`, `id`의 `pas`, 그리고 `type`의 `number`가 있어야 합니다.
...
`div`의 `className`의 네 번째 `form-group` 요소에는 `label`의 `htmlFor` 속성을 가진 `dri`와 `className`의 `label`, 그리고 텍스트 `DRI`가 있어야 합니다.
...
`div`의 `className`의 네 번째 `form-group` 요소에는 `input`의 `className`를 가진 `input`, `id`의 `dri`, 그리고 `type`의 `number`가 있어야 합니다.
