...
`addCharacters` 함수와 같은 구문을 사용하여 `elemValue` 함수를 업데이트하세요. 더 이상 `inner`를 선언하지 않고, 함수를 암묵적으로 반환해야 합니다.

# --hints--

`elemValue` 함수는 암묵적 반환을 사용해야 합니다.
...
`elemValue` 함수는 `character` 매개변수를 가진 화살표 함수를 암묵적으로 반환해야 합니다.
...
내부 화살표 함수는 `idToText()`를 인수로 하여 `character + num`를 호출한 결과를 반환해야 합니다.
