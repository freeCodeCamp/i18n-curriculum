...
أعطِ `.fb2a` عرضًا (`width`) بقيمة `100%` و`.fb2b` عرضًا (`width`) بقيمة `100%` وارتفاعًا (`height`) بقيمة `75%`.

# --hints--

يجب أن تعطي `.fb2a` عرضًا (`width`) بقيمة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.width, "100%");
```

يجب أن تعطي `.fb2b` عرضًا (`width`) بقيمة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2b")?.width, "100%");
```

يجب أن تعطي `.fb2b` ارتفاعًا (`height`) بقيمة `75%`.
