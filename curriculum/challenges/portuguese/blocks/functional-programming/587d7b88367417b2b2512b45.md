...
Complete o código para a função `squareList` usando qualquer combinação de `map()`, `filter()` e `reduce()`. A função deve retornar um novo array contendo os quadrados de *apenas* os inteiros positivos (números decimais não são inteiros) quando um array de números reais for passado para ela. Um exemplo de array de números reais é `[-3, 4.8, 5, 3, -3.2]`.

**Nota:** Sua função não deve usar nenhum tipo de laços `for` ou `while` nem a função `forEach()`.

# --hints--

`squareList` deve ser uma `function`.
