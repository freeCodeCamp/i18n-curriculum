...
La scheda del prezzo premium dovrebbe essere quella che risalta di più sulla pagina. Inoltre, è l’unica scheda che avrà un badge `Most popular`. Per far risaltare questa scheda e il suo badge, puoi usare le classi di posizionamento relativo e di scala di Tailwind.

Ecco come ingrandire un elemento con la scala in Tailwind CSS:
...
Ti occuperai del posizionamento nei passaggi successivi. Per ora, assegna la classe `scale-105` al contenitore `div` per renderlo un po’ più grande rispetto alle altre schede dei prezzi.

# --hints--

Il tuo elemento contenitore `div` per il piano premium dovrebbe avere la classe `scale-105`.
