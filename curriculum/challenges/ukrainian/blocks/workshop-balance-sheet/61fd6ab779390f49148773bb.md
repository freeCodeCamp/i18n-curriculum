...
Під вашим елементом `h1` створіть елемент `div`. Додайте йому атрибут `id` зі значенням `years`. Ви хочете, щоб цей конкретний елемент був прихований від читачів екрана, тому додайте атрибут `aria-hidden` зі значенням `true`.
