...
Значок рівня досі показує жорстко закодований текст `GOLD`. Потрібно замінити його на динамічний вираз, щоб відображався правильний рівень.

Викличте `getPlayerTier()` з `player.overallRating`, щоб отримати рядок рівня, а потім викличте `.toUpperCase()` на результаті, щоб відобразити його у верхньому регістрі. Не забудьте обгорнути весь вираз у фігурні дужки.

# --hints--

Ви повинні використати `getPlayerTier(player.overallRating).toUpperCase()` всередині `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

Елемент `.card-tier-badge` має показувати `ELITE` для гравця з рейтингом 98.
