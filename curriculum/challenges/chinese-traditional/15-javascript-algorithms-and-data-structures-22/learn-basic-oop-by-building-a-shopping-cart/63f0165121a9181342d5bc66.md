...
對於你的真值表達式，刪除 `undefined`，你需要將 `currentProductCountSpan` 的 `textContent` 更新爲 `currentProductCount`，後跟 `x`。 使用模板字面量來執行此操作。

# --hints--

你應該從真值表達式中刪除 `undefined`。
...
你應該訪問 `currentProductCountSpan` 的 `textContent` 屬性。
...
你應該使用模板字面量語法將 `textContent` 更新爲 `${currentProductCount}x`。
