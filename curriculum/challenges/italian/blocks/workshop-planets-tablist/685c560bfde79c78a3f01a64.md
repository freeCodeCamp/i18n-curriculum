...
Dato che `associatedPanel` è un `id`, questo ti permetterà di fare una query nel documento e trovare l'elemento pannello corrispondente.

Crea una nuova variabile chiamata `panel` e assegnale il valore di `document.getElementById(associatedPanel)`.

# --hints--

Dovresti avere una variabile chiamata `panel` che contiene l'elemento pannello associato alla scheda cliccata.
