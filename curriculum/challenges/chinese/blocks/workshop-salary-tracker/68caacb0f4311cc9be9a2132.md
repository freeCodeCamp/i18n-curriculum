...
现在你将专注于编写 `salary` 的 setter。在 `salary` 的 getter 之后，为 `salary` 属性创建一个简单的 setter，该 setter 将 `self._salary` 设置为作为参数传递给该方法的值。之后，打印 `Salary updated to $`，后跟新的薪水和句号。

你将在接下来的几个步骤中负责验证新建的薪资。

# --hints--

你的 `Employee` 类应有一个用 `@salary.setter` 装饰的 `salary` 方法。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].has_decorators("salary.setter")`)) })
```

你的 `salary` 设置器应将 `self._salary` 设置为作为参数传递给它的值。
...
你的 `salary` 设置器应打印 `Salary updated to $`，后跟新的工资和句号。
