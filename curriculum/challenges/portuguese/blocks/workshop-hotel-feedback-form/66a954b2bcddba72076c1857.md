...
Abaixo do seu elemento `label`, adicione um botão `radio` com o `id` definido como `"no-option"`, o atributo `name` definido como `"hotel-stay"` e um atributo `value` definido como `"no"`. 
...
Seu elemento de entrada deve ter um atributo `value` definido como `"no"`.
