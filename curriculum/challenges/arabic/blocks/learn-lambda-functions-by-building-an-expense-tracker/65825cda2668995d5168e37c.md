...
أنشئ بند `elif` آخر يتحقق مما إذا كانت `choice == '4'`. داخل بند `elif` الجديد، أنشئ متغيرًا باسم `category` وقم بتعيين `input('Enter category to filter: ')` له لتصفية فئة المصروفات.

# --hints--

يجب أن تنشئ بند `elif` يتحقق مما إذا كانت `choice == '4'`.
...
يجب إعلان متغير باسم `category` في بند `elif` الجديد الخاص بك.
...
يجب تعيين `input('Enter category to filter: ')` إلى متغير `category` الخاص بك.
