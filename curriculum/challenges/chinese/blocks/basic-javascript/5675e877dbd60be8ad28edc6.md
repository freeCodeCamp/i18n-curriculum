...
记住数组的索引是从零开始的，这意味着数组的最后一个下标是 `length - 1`。 我们这个循环的条件是 `i < arr.length`，当 `i` 的值等于 `length` 时循环就会停止。 在这个例子中，最后一次迭代是 `i === 4`，也就是说，当 `i` 等于 `arr.length - 1` 时，向控制台输出 `6`。 然后 `i` 增加到 `5`，循环终止，因为 `i < arr.length` 为 `false`。
