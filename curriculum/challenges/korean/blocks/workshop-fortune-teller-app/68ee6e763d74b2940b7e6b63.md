...
점쟁이 앱은 여러 카드를 렌더링하는 것을 지원하므로, 여러 카드를 렌더링하는 데 도움이 되는 메서드를 만들어야 합니다. `multipleCardSelected`라는 메서드를 선언하세요.

메서드 안에서 `hideElements` 함수를 호출하고 `singleCard`, `singleCardBtn`, `multipleCardsBtn`, `headerTitle` 프로퍼티를 전달하세요. 이 프로퍼티들은 `this.elements`에서 접근해야 한다는 점을 잊지 마세요.

# --hints--

`multipleCardSelected` 메서드를 선언해야 합니다.
...
`hideElements` 메서드 안에서 `multipleCardSelected` 함수를 호출해야 합니다.
...
`singleCard` 함수에 첫 번째 인수로 `hideElements`을 전달해야 합니다.
...
`singleCardBtn` 함수에 `hideElements`를 인수로 전달해야 합니다.
...
`multipleCardsBtn` 함수에 `hideElements`를 인수로 전달해야 합니다.
...
`headerTitle` 함수에 `hideElements`를 인수로 전달해야 합니다.
