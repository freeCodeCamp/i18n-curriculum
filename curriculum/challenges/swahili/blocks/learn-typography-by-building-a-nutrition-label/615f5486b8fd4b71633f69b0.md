...
Chini ya kipengele chako cha `.small-text`, tengeneza kipengele kipya cha `p` chenye maandishi ya `Calories`. Pia chini ya kipengele cha `.left-container`, tengeneza kipengele kipya cha `span` chenye maandishi ya `230`.
...
Kipengele chako cha `p` kinapaswa kuwa na maandishi ya `Calories`.

```js
assert(document.querySelector('.small-text')?.nextElementSibling?.innerText === 'Calories');
```

Kipengele chako cha `span` kinapaswa kuwa na maandishi ya `230`.
