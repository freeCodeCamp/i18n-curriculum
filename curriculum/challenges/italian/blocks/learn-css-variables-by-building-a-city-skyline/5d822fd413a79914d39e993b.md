...
Copia e incolla la tua intera classe `sky` insieme a tutte le sue proprietà e valori dentro la media query. Creerai un altro schema di colori per lo skyline che lo trasforma dal giorno alla notte.

Nota: dovrai scorrere oltre la regione modificabile per copiare la classe.

# --hints--

Non dovresti eliminare la dichiarazione `.sky` esistente.
...
Dovresti copiare la dichiarazione `.sky` esistente dentro la media query.
