...
`if` 文の後に、`isSpam()` を引数として `messageInput.value` を呼び出した結果の真偽値を三項演算子でチェックしてください。真の場合は、`result` 要素の `textContent` プロパティに `"Oh no! This looks like a spam message."` を代入します。そうでなければ `"This message does not seem to contain any spam."` を代入してください。

その後、`messageInput` 要素の `value` プロパティを空文字に設定してください。

# --hints--

`result` 要素の `textContent` プロパティを設定するには代入演算子を使う必要があります。
...
三項演算子の真の場合の式は、`result` 要素の `textContent` プロパティに `"Oh no! This looks like a spam message."` を代入してください。
...
三項演算子の偽の場合の式は、`result` 要素の `textContent` プロパティに `"This message does not seem to contain any spam."` を代入してください。
...
三項演算子の後に、`messageInput` 要素の `value` プロパティを空文字に設定してください。
