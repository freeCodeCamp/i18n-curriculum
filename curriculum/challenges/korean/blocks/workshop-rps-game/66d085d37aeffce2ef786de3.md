...
플레이어나 컴퓨터가 게임에서 이겼다면, 게임을 재설정하고 다시 플레이할 수 있는 옵션이 있어야 합니다.

`resetGameBtn` 버튼에 이벤트 리스너를 추가하세요. 이벤트 리스너는 `"click"` 이벤트와 `resetGame` 함수에 대한 참조를 받아야 합니다.
...
`resetGameBtn` 버튼에 이벤트 리스너를 추가해야 합니다. 이벤트 리스너는 `"click"` 이벤트와 `resetGame` 함수에 대한 참조를 받아야 합니다.
