...
將標的 `.beak` 元素中 `class` 為 `bottom` 的部分設定為比 `.beak.top` 的 `width` 小 `4%`、比 `.beak.top` 離頂部遠 `5%`，且比 .beak.top 離其父元素左側遠 `2%`。

# --hints--

你應該有一個 `.beak.bottom` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak']));
```

你應該給 `.beak.bottom` 一個 `width` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak'])?.width, '16%');
```

你應該給 `.beak.bottom` 一個 `top` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.beak.bottom', '.bottom.beak'])?.top, '65%');
```

你應該給 `.beak.bottom` 一個 `left` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。
