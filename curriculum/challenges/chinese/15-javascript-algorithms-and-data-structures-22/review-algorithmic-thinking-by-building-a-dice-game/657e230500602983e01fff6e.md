...
声明一个 `checkForStraights` 函数，接受一个数组作为参数。 如果用户得到大顺子，请用 `40` 的分数更新第五个单选按钮。 如果用户得到小顺子，请用 `30` 的分数更新第四个单选按钮。 Regardless, it should always update the last radio button to display a score of 0, with the correct attributes.
