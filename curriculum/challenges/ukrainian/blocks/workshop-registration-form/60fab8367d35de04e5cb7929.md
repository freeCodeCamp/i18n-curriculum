...
Додайте елемент посилання з текстом `Read our terms and conditions` перед новододаним `label`. Встановіть `href` на:

```md
https://www.freecodecamp.org/news/terms-of-service/
```

Потім додайте текст `I accept the terms and conditions` одразу після вхідних даних у мітці.

# --hints--

У вас має бути елемент `a` безпосередньо перед `label` з текстом `Read our terms and conditions`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.innerText.trim().replaceAll(/\s+/g, ' '), /Read our terms and conditions\.?/i);
```

Елемент `a` має мати `href` `https://www.freecodecamp.org/news/terms-of-service/`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.href, /https:\/\/www\.freecodecamp\.org\/news\/terms-of-service\/?/);
```

Усередині `input` має бути елемент `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) + a + label > input'));
```

Всередині `label`, одразу після елемента `input`, має бути текст `I accept the terms and conditions`.
