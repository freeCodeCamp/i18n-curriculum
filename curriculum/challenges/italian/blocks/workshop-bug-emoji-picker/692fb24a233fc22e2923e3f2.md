...
Dopo il costruttore, crea un metodo astratto `render` che abbia un tipo di ritorno `void`. Lo userai nelle sottoclassi per aggiornare l'emoji mostrata.

# --hints--

La tua classe `Bug` dovrebbe avere un metodo astratto `render`.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

Il tuo metodo `render` dovrebbe avere un tipo di ritorno `void`.
