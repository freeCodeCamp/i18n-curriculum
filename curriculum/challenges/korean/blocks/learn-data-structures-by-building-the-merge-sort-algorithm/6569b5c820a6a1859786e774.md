...
`while` 반복문 안에서 `right_part[right_array_index]`에 `array[sorted_index]`을 할당하세요.

# --hints--

반복문 본문에 `array[sorted_index] = right_part[right_array_index]`이 있어야 합니다. 반복문 본문에서 `pass` 키워드를 제거하는 것도 잊지 마세요.
...
`pass` 반복문 안에는 `while` 키워드가 없어야 합니다.
