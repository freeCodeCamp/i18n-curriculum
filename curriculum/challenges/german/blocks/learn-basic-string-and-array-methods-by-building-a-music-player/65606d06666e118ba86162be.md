...
Fügen Sie ein zweites Objekt mit den folgenden Schlüsseln und Werten hinzu:
...
Ihr `allSongs` Array sollte zwei Werte enthalten.

```js
assert.equal(allSongs.length, 2);
```

Ihr `allSongs` Array sollte als zweiten Wert ein Objekt enthalten.

```js
assert.isObject(allSongs[1]);
```

Das zweite Objekt in Ihrem `allSongs` Array sollte eine `id` Eigenschaft mit dem Wert der Zahl `1` haben.

```js
assert.equal(allSongs[1].id, 1);
```

Das zweite Objekt in Ihrem `allSongs` Array sollte eine `title` Eigenschaft mit dem Wert des Strings `"Can't Stay Down"` haben.

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

Das zweite Objekt in Ihrem `allSongs` Array sollte eine `artist` Eigenschaft mit dem Wert des Strings `"Quincy Larson"` haben.

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

Das zweite Objekt in Ihrem `allSongs` Array sollte eine `duration` Eigenschaft mit dem Wert des Strings `"4:15"` haben.

```js
assert.equal(allSongs[1].duration, "4:15");
```

Das zweite Objekt in Ihrem `allSongs` Array sollte eine `src` Eigenschaft mit dem Wert des Strings `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"` haben.
