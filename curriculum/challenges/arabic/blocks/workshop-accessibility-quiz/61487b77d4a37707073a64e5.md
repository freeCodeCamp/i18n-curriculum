...
عندما يكون عرض الشاشة صغيرًا، لا يلتف `h1` حول محتوى النص كما يجب. قم بمحاذاة النص لعنصر `h1` في الوسط.

بعد ذلك، أعطِ عنصر `main` حشوًا بحيث يمكن رؤية عنوان قسم `Student Info` بالكامل.

# --hints--

يجب أن تعطي `h1` خاصية `text-align` بقيمة `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

يجب أن تضيف محدِّد `main` لاستهداف عنصر `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

يجب أن تعطي `main` حشوًا علويًا (`padding-top`) لا يقل عن `25px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

يجب أن تغير فقط قيمة `padding-top`.
