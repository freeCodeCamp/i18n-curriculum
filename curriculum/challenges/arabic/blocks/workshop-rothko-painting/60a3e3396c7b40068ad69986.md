...
أنشئ `div` جديدًا بقيمة `class` تساوي `three` مباشرة تحت العنصر `.two`. سيكون هذا المستطيل الثالث الخاص بك.

# --hints--

يجب ألا تتغير العناصر `.one` و `.two` الموجودة لديك.
...
يجب أن يكون `div` الجديد مسكنًا داخل عنصر `.canvas` الخاص بك.

```js
assert(document.querySelector('.canvas').children[2].tagName === 'DIV');
```

يجب أن يأتي `div` الجديد بعد العنصر `.two`.
