...
在回调函数内部，创建一个名为 `collisionDetectionRules` 的新 `const` 变量并为其分配一个空数组。 

在该数组内，添加一个布尔表达式，检查玩家的 `y` 位置加上玩家的身高是否小于或等于平台的 `y` 位置。

# --hints--

你应该有一个名为 `collisionDetectionRules` 的 `const` 变量，该变量被分配一个空数组。

```js
assert.match(code, /const\s+collisionDetectionRules\s*=\s*\[\s*(?:[^\]]*\s*)*\s*\]\s*;?/);
```

你应该有一个布尔表达式来检查玩家的 `y` 位置加上玩家的身高是否小于或等于平台的 `y` 位置。
