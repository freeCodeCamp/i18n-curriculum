...
استخدم طريقة `add` في `catalogue` لإضافة `series1` إلى قائمة `items`.

بعد ذلك، أنشئ متغيرًا آخر باسم `series2`. عيّن له نسخة أخرى من `TVSeries` وأضف `series2` إلى الكتالوج أيضًا.

# --hints--

يجب أن تستخدم طريقة `add` في `catalogue` مع تمرير `series1`.
...
يجب أن تنشئ متغيرًا باسم `series2` وتعيّن له نسخة من `TVSeries`.
...
يجب أن تستخدم طريقة `add` في `catalogue` مع تمرير `series2`.
