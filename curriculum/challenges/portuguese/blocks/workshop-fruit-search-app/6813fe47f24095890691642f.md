...
Em seguida, você precisará de uma função para quando o formulário for enviado. Defina uma função `handleSubmit` que aceita um objeto de evento, `e`, e chama `e.preventDefault()`.

# --hints--

Você deve definir uma função `handleSubmit` com um parâmetro `e`.
...
Você deve chamar `e.preventDefault()` dentro da função `handleSubmit()`.
