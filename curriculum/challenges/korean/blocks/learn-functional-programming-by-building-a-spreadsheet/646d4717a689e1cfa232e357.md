...
`highPrecedence()` 함수에서 했던 것처럼, `evalFormula()` 함수도 모든 것을 평가하고 교체했는지 확인해야 합니다.

삼항 연산자를 사용해 `functionExpanded`가 원래 문자열 `x`와 같은지 확인하세요. 같으면 `functionExpanded`를 반환하고, 그렇지 않으면 `evalFormula()`와 `functionExpanded`를 인수로 하여 `cells`를 다시 호출한 결과를 반환하세요.

# --hints--

`evalFormula` 함수는 `return` 키워드를 사용해야 합니다.
