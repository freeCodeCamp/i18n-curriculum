...
在你的 `else` 語句中，將字串 `"general"` 指定給 `selectedCategory.className`。

在那之下，將字串 `"General"` 指定給 `selectedCategory.category`。

最後，將數字 `1` 指定到 `selectedCategory.id`。

# --hints--

你應該將字串 `"general"` 指定給 `selectedCategory.className`。

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

你應該將字串 `"General"` 指定到 `selectedCategory.category`。

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

你應該將數字 `1` 指定給 `selectedCategory.id`。
