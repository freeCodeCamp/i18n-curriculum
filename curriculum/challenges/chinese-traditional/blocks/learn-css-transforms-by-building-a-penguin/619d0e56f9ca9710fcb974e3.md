...
將 `.eye-lid` 元素定位於其父元素頂部的 `25%` 和左側的 `-23%`。然後，為所有角落設定半徑為 `50%`。

# --hints--

你應該給 `.eye-lid` 一個 `top` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.top, '25%');
```

你應該給 `.eye-lid` 一個 `left` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.left, '-23%');
```

你應該給 `.eye-lid` 一個 `--fcc-expected--` 的 `border-radius`，但發現是 `--fcc-actual--`。
