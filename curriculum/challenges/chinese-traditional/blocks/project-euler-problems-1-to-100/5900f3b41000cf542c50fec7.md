...
在 `d` ≤ `limit` 的條件下，約分真分數集合中會包含多少個元素？

# --hints--

`countingFractions(8)` 應該傳回一個數字。

```js
assert(typeof countingFractions(8) === 'number');
```

`countingFractions(8)` 應該傳回 `21`。

```js
assert.strictEqual(countingFractions(8), 21);
```

`countingFractions(20000)` 應該傳回 `121590395`。

```js
assert.strictEqual(countingFractions(20000), 121590395);
```

`countingFractions(500000)` 應該傳回 `75991039675`。

```js
assert.strictEqual(countingFractions(500000), 75991039675);
```

`countingFractions(1000000)` 應該傳回 `303963552391`。
