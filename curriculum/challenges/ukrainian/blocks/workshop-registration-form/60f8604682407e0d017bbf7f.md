...
У кожному відповідному елементі `label` і безпосередньо після елемента `input` додайте пробіл і додайте наступний текст:
