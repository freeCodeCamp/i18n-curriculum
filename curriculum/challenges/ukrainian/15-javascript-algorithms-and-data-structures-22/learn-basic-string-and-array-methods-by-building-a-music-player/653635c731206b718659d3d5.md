...
В межах функції `playSong` встановіть властивість `audio.src` рівною `song.src`. Це вказує аудіоелементу, де знайти аудіодані для вибраної пісні.

Також встановіть властивість `audio.title` рівною `song.title`. Це вказує аудіоелементу, що відображати як назву пісні.

# --hints--

Не змінюйте наявну функцію `playSong` та її вміст.
...
Встановіть `audio.src` на `song.src`.
...
Встановіть `audio.title` на `song.title`.
