...
获取你的 `#budget` 元素并将其分配给 `budgetNumberInput`，以及你的 `#entry-dropdown` 元素并将其分配给 `entryDropdown`。

# --hints--

你应该声明一个名为 `budgetNumberInput` 的变量。

```js
assert.isDefined(budgetNumberInput);
```

你应该使用 `document.getElementById()` 来获取 `#budget` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")budget\1\s*\)/g);
```

你应该将 `#budget` 元素存储在名为 `budgetNumberInput` 的变量中。

```js
assert.equal(budgetNumberInput, document.getElementById('budget'));
```

你应该声明一个名为 `entryDropdown` 的变量。

```js
assert.isDefined(entryDropdown);
```

你应该使用 `document.getElementById()` 来获取 `#entry-dropdown` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")entry-dropdown\1\s*\)/g);
```

你应该将 `#entry-dropdown` 元素存储在名为 `entryDropdown` 的变量中。
