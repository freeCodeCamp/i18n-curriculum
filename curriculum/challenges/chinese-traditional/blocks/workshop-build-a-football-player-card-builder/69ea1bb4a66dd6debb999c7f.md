...
現在你將控制代碼球員位置的選擇。你必須將輸入改寫成下拉選單，然後將有效足球位置的陣列映射成下拉選單選項的值。

首先，創建一個 `POSITIONS` 陣列，裡面包含 `GK`、`CB`、`LB`、`RB`、`CDM`、`CM`、`CAM`、`LW`、`RW`、`ST` 和 `CF` 這些字串值。在閉合的中括號後面緊接著加上 `as const`，讓 TypeScript 將型別從一般的 `string[]` 縮小為這些確切字面值的唯讀元組。

這表示 TypeScript 能精確知道哪些字串是有效的，而不只是知道它是某個字串陣列。

# --hints--

你應該宣告一個 `POSITIONS` 常數。
...
你的 `POSITIONS` 陣列應該包含所有必須的足球位置字串，並使用 `as const` 將其型別縮小為唯讀字串字面值。
