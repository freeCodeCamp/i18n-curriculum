...
现在你有了捕获组，你可以将整个模式标记为可选匹配。 `?` 量词匹配前一个字符或组的零个或一个出现。 例如，正则表达式 `/colou?r/` 同时匹配 `color` 和 `colour`，因为 `u` 是可选的。
...
你的 `(hundred|thousand|million|billion)` 捕获组应该是可选的。

```js
assert.match(dollarRegex.source, /\(hundred\|thousand\|million\|billion\)\?/);
```

你的 `dollarRegex` 应该仍然与 `1 million dollars` 匹配。
