...
创建一个 `.hero-title` 选择器并为其指定一个设置为 `center` 的 `text-align` 属性、一个设置为 `orangered` 的 `color` 属性以及一个设置为 `8rem` 的 `font-size` 属性。

# --hints--

你应该有一个 `.hero-title` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

你的 `.hero-title` 选择器应该有一个 `text-align` 属性，设置为 `center`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign === 'center');
```

你的 `.hero-title` 选择器应该有一个 `color` 属性，设置为 `orangered`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color === 'orangered');
```

你的 `.hero-title` 选择器应该有一个 `font-size` 属性，设置为 `8rem`。
