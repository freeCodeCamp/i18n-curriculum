...
Para completar el laboratorio, debes construir el resultado usando recursión y no debes usar bucles (`for`, `while`), `Array.from()`, ni métodos de iteración de arreglos (`forEach`, `map`, `filter`, `reduce`). Cada llamada debe producir su propio arreglo de resultados (no uses variables globales para almacenar la cuenta regresiva).
...
5. Debes usar recursión (la función debe llamarse a sí misma) y no debes usar bucles de ningún tipo (`for`, `while`, `Array.from()`, o métodos de orden superior como `forEach`, `map`, `filter`, `reduce`).
...
Tu código no debe depender de ningún tipo de bucles (`for`, `while`, `Array.from()` o funciones de orden superior como `forEach`, `map`, `filter` y `reduce`).
