...
雑誌はレイアウトを整え、内容の流れを制御するために、印刷物で両端揃えのテキストをよく使います。印刷物では効果的ですが、ウェブ上の両端揃えテキストはアクセシビリティの問題になる場合があり、例えばディスレクシアの方にとっては読みづらくなることがあります。

プロジェクトを印刷された雑誌のように見せるために、`.text`セレクターに`text-align`プロパティを`justify`に設定してください。

# --hints--

`.text`セレクターには`text-align`プロパティを`justify`に設定してください。
