...
`#header-img` يجب أن يكون تابعا لـ `#header`.
...
`#header-img` يجب أن يحتوي على سمة `src`.
...
قيمة `src` لـ `#header-img` يجب أن تكون عنوان URL صالح (يبدأ بـ `http`).
...
`#nav-bar` يجب أن يكون تابعا لـ `#header`.
...
`#email` الخاص بك يجب أن يكون تابعا لـ `#form`.
...
يجب أن يكون للـ `#email` سمة `placeholder` مع placeholder text.
...
`#email` الخاص بك يجب أن يستخدم التحقق من صحة HTML5 عن طريق تعيين `type` إلى `email`.
...
`#submit` الخاص بك يجب أن يكون تابعا لـ `#form`.
...
يجب أن تحتوي ال `#form` على سمة `action` بقيمة `https://www.freecodecamp.com/email-submit`.
...
`#email` الخاص بك يجب أن يحتوي على سمة `name` بقيمة `email`.
...
عنصر `#nav-bar` الخاص بك يجب أن يكون دائما في الجزء العلوي من الـ viewport.
