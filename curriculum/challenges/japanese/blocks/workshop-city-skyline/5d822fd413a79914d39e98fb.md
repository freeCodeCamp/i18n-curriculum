...
グラデーションのトランジションは、多くの場合、ある色から別の色へ徐々に変化します。より急激な変化が必要な場合は、このようにハードストップでトランジションを作ることができます。
...
`linear-gradient`に`.bb2b`を追加し、`--building-color2`の`0%`から`6%`までと、`--window-color2`の`6%`から`9%`までを使ってください。

# --hints--

`.bb2b`に`background`プロパティを設定してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

`linear-gradient`に`background`を使ってください。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

`--building-color2`から`0%`まで`6%`を使ってください。
...
`--window-color2`から`6%`まで`9%`を使ってください。
...
`.bb2b`には、`linear-gradient`の`--building-color2`から`0%`まで、そして`6%`の`--window-color2`から`6%`までトランジションする`9%`が必要です。
