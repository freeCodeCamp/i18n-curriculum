...
在 `forEach` 回調中， 將右側賦值語句 `totalCountPerProduct[dessert.id] || 0` 放在括號 `()` 中來確保正確計算，然後將值加 1。
...
你應該使用方括號來訪問與 `dessert.id` 相對應的 `totalCountPerProduct` 屬性。
...
你應該使用賦值運算符來更新與 `dessert.id` 相對應的 `totalCountPerProduct` 屬性的值。
...
你應該在加 1 之前將 `totalCountPerProduct[dessert.id] || 0` 放到括號 `()` 中。
