...
聲明一個 `functionCall` 變量，併爲其分配以下正則表達式：`/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

該表達式將查找類似 `sum(1, 4)` 的函數調用。

# --hints--

你應該聲明一個 `functionCall` 變量。
...
你應該使用 `const` 來聲明你的 `functionCall` 變量。
