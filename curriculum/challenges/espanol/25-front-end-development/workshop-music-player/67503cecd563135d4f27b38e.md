...
Ahora es momento de probar la función `playNextSong`.

Añade un oyente de evento `"click"` al elemento `nextButton`, luego pasa `playNextSong` como el segundo argumento de tu oyente de evento. Esta es la función que el oyente de evento ejecutará.

Prueba tu aplicación primero haciendo clic en el botón de reproducir seguido por el botón siguiente. Deberías ver que todo está funcionando como se espera.

# --hints--

Deberías llamar al método `addEventListener()` en tu variable `nextButton`.

```js
assert.match(__helpers.removeJSComments(code), /nextButton\.addEventListener\(/)
```

Tu oyente de evento debería escuchar un evento `"click"`.

```js
assert.match(__helpers.removeJSComments(code), /nextButton\.addEventListener\(\s*('|"|`)click\1/)
```

Deberías pasar `playNextSong` como el segundo argumento de tu método `addEventListener`.
