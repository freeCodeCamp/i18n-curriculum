...
每個類別都將有一個 URL，指向 [freeCodeCamp 論壇](https://forum.freecodecamp.org/)上的類別。

創建一個名爲 `url` 的常量，併爲其分配一個模板字面量。 在該模板字面量中，放置 `${forumCategoryUrl}${selectedCategory.className}/${id}` 的值。
...
你應該在模板字面量中使用 `${forumCategoryUrl}${selectedCategory.className}/${id}` 的值。
