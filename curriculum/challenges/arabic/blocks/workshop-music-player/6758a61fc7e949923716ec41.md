...
الآن، استخدم الطريقة `querySelectorAll` لاستهداف عناصر `.playlist-song` وخزنها في متغير اسمه `songs`.

بعد ذلك، استدعِ الطريقة `forEach` على `songs`. مرر دالة استدعاء راجع تستخدم `song` كمعلمة للطريقة. اترك الأقواس المتعرجة فارغة في الوقت الحالي.

# --hints--

يجب أن يكون لديك متغير اسمه `songs`.

```js
assert.isDefined(songs);
```

يجب أن تعيّن مجموعة كل عناصر `.playlist-song` إلى متغير `songs` الخاص بك.

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

يجب أن تستدعي الطريقة `forEach` على `songs` وتمرر دالة استدعاء راجع تستخدم `song` كمعلمة للطريقة.
