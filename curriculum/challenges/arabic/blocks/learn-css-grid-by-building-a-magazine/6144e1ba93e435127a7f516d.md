...
أنشئ محدِّد `h2, h3, h4, h5, h6`. أعطه خاصية `font-family` مضبوطة على `Raleway` مع نظام بديل `sans-serif`.

# --hints--

يجب أن يكون لديك محدِّد `h2, h3, h4, h5, h6`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h2, h3, h4, h5, h6'));
```

يجب أن يحتوي محدِّد `h2, h3, h4, h5, h6` على خاصية `font-family` مضبوطة على `Raleway` مع نظام بديل `sans-serif`.
