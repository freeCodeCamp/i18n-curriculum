...
All'interno del metodo `calculate_best_price`, crea una lista chiamata `prices` che contenga `product.price`. Questo garantisce che se non si applicano sconti, il cliente pagherà il prezzo pieno.

# --hints--

All'interno di `calculate_best_price`, dovresti creare una lista chiamata `prices` che contenga `product.price`.
