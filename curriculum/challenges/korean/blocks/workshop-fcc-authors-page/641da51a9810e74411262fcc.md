...
두 번째 `.then()` 메서드에 `data` 매개변수를 추가해야 합니다.
...
`.catch()` 메서드는 `console.error()`를 사용하여 텍스트 `` `There was an error: ${err}` ``와 함께 `err`를 콘솔에 기록해야 합니다. 백틱 사용을 잊지 마세요.
