...
ユーザーがノートを編集し、タブキーを押すかカードの外をクリックすると、ノートが正常に保存されたことを知らせるステータスメッセージが表示されるようにしてください。

まず、`div` 要素を作成し、`id` 属性を `"status"` に設定してください。
...
2つ目の `div` 要素には `id` 属性が必要です。

```js
assert.isTrue(document.querySelectorAll("div")[1].hasAttribute("id"));
```

2つ目の `div` 要素には `id` 属性を `"status"` に設定してください。
