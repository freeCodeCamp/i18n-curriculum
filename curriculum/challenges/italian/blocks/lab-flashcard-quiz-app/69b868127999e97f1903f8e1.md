...
title: "Crea un'app quiz con flashcard"
challengeType: 25
...
In questo laboratorio, creerai un'app che mostra e archivia dati di flashcard che possono essere recuperati in seguito.

**Obiettivo:** Completa le storie utente qui sotto e fai passare tutti i test per completare il laboratorio.

**Storie utente:**

1) Devi avere un elemento HTML con un id di `flashcard`.  
2) Devi avere un'interfaccia chiamata `FlashCard`.  
3) L'interfaccia `FlashCard` deve contenere le seguenti proprietà `questionText` e `questionAnswer` entrambe di tipo `string`.  
4) Devi avere una collezione di elementi `FlashCard` chiamata `currentCards`.  
5) Quando si clicca sull'elemento `#flashcard`, la scheda deve avere la classe `flipped`.  
6) Devi avere un elemento con un id di `delete-btn`.  
7) Quando si clicca sull'elemento `#delete-btn`, deve rimuovere la scheda corrente e mostrare i dati della scheda precedente.  
8) Devi creare un modulo di inserimento con un id di `entry-form` per poter aggiungere altre flashcard alla collezione `currentCards` al momento dell'invio.  
9) I due elementi all'interno del modulo devono avere rispettivamente un id di `front-text` e `back-text`.  
10) Devi creare e chiamare un `InvalidUserInputError` quando il testo della domanda o la risposta alla domanda sono vuoti nel modulo di inserimento.

# --hints--

Devi avere un elemento HTML con un id di `flashcard`.
...
Devi avere un'interfaccia `FlashCard`.
...
L'interfaccia `FlashCard` deve avere una proprietà `questionText` di tipo `string`.
...
L'interfaccia `FlashCard` deve avere una proprietà `questionAnswer` di tipo `string`.
...
Devi avere una collezione di elementi `FlashCard` chiamata `currentCards` con il tipo `FlashCard[]`.
...
Quando si clicca per la prima volta sull'elemento `#flashcard`, l'elemento deve ricevere la classe `flipped`.
...
Devi avere un elemento con un id di `delete-btn`.
...
Quando si clicca su `delete-btn`, un elemento flashcard deve essere rimosso dalla collezione `currentCards`.
...
Quando si clicca su `delete-btn`, devono essere mostrati i dati della flashcard precedente.
...
Devi creare un modulo di inserimento con un id di `entry-form`.
...
Devi avere due elementi `textarea` con id `front-text` e `back-text` rispettivamente all'interno del modulo.
...
Deve essere lanciato un `InvalidUserInputError` quando il testo della domanda o la risposta alla domanda sono vuoti nel modulo di inserimento.
