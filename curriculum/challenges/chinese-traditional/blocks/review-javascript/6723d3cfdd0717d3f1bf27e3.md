...
- **數值**：數值代表整數和浮點數。 整數的例子包括 7、19 和 90。
- **浮點數**：浮點數是帶有小數點的數值。 例子包括 3.14、0.5 和 0.0001。
- **字符串**：字符串是用引號括起來的一串字符或文本。 `"I like coding"` 和 `'JavaScript is fun'` 都是字符串的例子。
- **布爾**：布爾代表兩種可能值之一：`true` 或 `false`。 你可以使用布爾來表示條件，例如 `isLoggedIn = true`。
...
- **符號**：符號數據類型是唯一且不可變的值，可用作對象屬性的標識符。
...
- **大整數**：當數字對於 `Number` 數據類型來說太大時，你可以使用大整數數據類型來表示任意長度的整數。
...
- 除了 `let`，你也可以使用 `const` 來聲明變量。 但是，`const` 變量不能重新賦值。
...
- 使用 `const` 聲明變量在聲明常量時使用，常量在整個代碼中不得更改，如 `PI` 或 `MAX_SIZE`。

## 變量命名準則

- 變量名應具有描述性和意義。
- 變量名應爲小駝峯，如 `cityName`、`isLoggedIn` 和 `veryBigNumber`。
- 變量名不應以數字開頭。 必須以字母、`_` 或 `$` 開頭。
- 變量名不得包含空格或特殊字符，`_` 和 `$` 除外。
- 變量名不應是保留關鍵字。
- 變量名區分大小寫。 `age` 和 `Age` 是不同的變量。

## JavaScript 中的字符串和字符串不可變性

- 字符串是用引號括起來的一串字符。 它們可以使用單引號和雙引號創建。
...
- 在 JavaScript 中，字符串是不可變的。 這意味着一旦創建了字符串，就不能更改字符串中的字符。 不過，你仍可以將字符串重新賦值。
...
- 連接是將多個字符串連接起來或將字符串與包含文本的變量結合起來的過程。 `+` 操作符是最簡單、最常用的字符串連接方法之一。
...
- 如果需要添加或追加到現有字符串，則可使用 `+=` 操作符。 當你想在一個字符串的基礎上逐漸添加更多文字時，這很有幫助。
...
- 另一種連接字符串的方式是使用 `concat()` 方法。 這種方法是將兩個或多個字符串連接在一起。
...
- `console.log()` 方法用於將消息記錄到控制檯。 它是調試和測試代碼的有用工具。
...
- 分號主要用於標記語句的結束。 這有助於 JavaScript 引擎理解單個指令之間的分離，這對正確執行至關重要。
...
- 分號有助於防止代碼執行中的歧義，並確保語句正確終止。

## JavaScript 中的註釋

- 任何註釋掉的代碼行都會被 JavaScript 引擎忽略。 註釋用於解釋代碼、做備註或暫時禁用代碼。
- 單行註釋使用 `//` 創建。
...
## 作爲動態類型語言的 JavaScript

- JavaScript 是一種動態類型語言，這意味着在聲明變量時不必指定其數據類型。 JavaScript 引擎會根據分配給變量的值自動確定數據類型。
...
- `typeof` 操作符用於檢查變量的數據類型。 它返回一個字符串，表示變量的類型。
...
- **camelCasing**: By convention, JavaScript developers will use camel casing for naming variables and functions. Camel casing is where the first word is all lowercase and the following words start with a capital letter. 例如： `isLoading`.
...
- One of the biggest advantages of `async/await` is error handling via `try/catch` blocks. 這是一個示例：
