...
你应该有一个 `.first-paragraph::first-letter` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

你的 `.first-paragraph::first-letter` 选择器应该有一个 `font-size` 属性，设置为 `6rem`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

你的 `.first-paragraph::first-letter` 选择器应该有一个 `color` 属性，设置为 `orangered`。
