...
2. 你的 `button` 元素應該有以下屬性：
   - 設定為 `id` 的 `"theme-switcher-button"` 屬性。
   - 設定為 `aria-haspopup` 的 `"true"` 屬性。
   - 設定為 `aria-expanded` 的 `"false"` 屬性。
   - 設定為 `aria-controls` 的 `"theme-dropdown"` 屬性。
3. 你應該有一個帶有以下屬性的 `ul` 元素：
   - 一個 `id` 屬性設定為 `"theme-dropdown"`。
   - 設定為 `role` 的 `"menu"` 屬性。
   - 設定為 `aria-labelledby` 的 `"theme-switcher-button"` 屬性。
   - 一個 `hidden` 屬性。
4. 你的 `ul` 元素應該至少有兩個 `li` 項目，其 `role` 屬性設定為 `"menuitem"`，並且包含你選擇的表述不同主題的文字。
5. 你的每個 `li` 元素都應該有一個以 `id` 開頭並以你為該 `theme-` 元素文字設定的主題結尾的 `li` 屬性。例如，如果你的其中一個主題是 `Light`，那麼你的 `id` 應該是 `theme-light`。
6. 你應該有一個元素，其 `aria-live` 屬性設為 `"polite"`。
7. 你應該有一個包含至少兩個物件的 `themes` 陣列，每個物件都包含 `name` 和 `message` 屬性。`name` 將表述不同的主題，而 `message` 將在主題切換時顯示。你可以自由設計 `name` 和 `message` 的值，但 `name` 的值應該與你的 `li` 元素的文字內容相符。
8. 當使用者點擊 `#theme-switcher-button` 時，`#theme-dropdown` 元素應該顯示可用的主題。
9. 當 `#theme-dropdown` 元素顯示時，`aria-expanded` 元素的 `"true"` 屬性應設為 `button`。
10. 當使用者在 `#theme-switcher-button` 元素顯示時點擊 `#theme-dropdown`，應該關閉選單，並添加 `hidden` 屬性，且將 `aria-expanded` 屬性設為 `"false"`。
11. 當使用者點擊 `#theme-switcher-button` 並選擇主題時，應將對應的 `theme-<name>` 類別添加到 `body` 元素，並在 `themes` 元素中顯示你在 `aria-live="polite"` 陣列中設定的相關主題訊息。
12. 當使用者點擊 `#theme-switcher-button` 並選擇與目前不同的主題時，`body` 的主題類別和狀態訊息應更新以符合新的選擇。
...
你的 `button` 元素應該有一個設定為 `aria-haspopup` 的 `"true"` 屬性。 
...
你的每個 `li` 元素都應該有一個以 `id` 開頭並以該元素對應主題結尾的 `theme-` 屬性。確保你的 `id` 值全部為小寫。
...
你的每個 `name` 屬性值都應該與你其中一個 `li` 元素的文字內容相符。確保這些值全部為小寫。
...
當使用者點擊 `#theme-switcher-button` 以顯示主題選項時，應該從 `hidden` 元素中移除 `#theme-dropdown` 屬性。
...
當 `#theme-dropdown` 元素顯示時，`aria-expanded` 元素的 `"true"` 屬性應設為 `button`。
...
當使用者在 `#theme-switcher-button` 元素顯示時點擊 `#theme-dropdown`，下拉選單應該被設回 `hidden`。
...
當使用者在 `#theme-switcher-button` 元素顯示時點擊 `#theme-dropdown`，`aria-expanded` 屬性應該設為 `"false"`。
...
當使用者點擊 `#theme-switcher-button` 並選擇主題時，應該在 `message` 元素中顯示來自 `theme` 陣列且與所選 `themes` 相關的 `aria-live="polite"`。
...
當使用者點擊 `#theme-switcher-button` 並選擇兩個不同的主題時，`body` 類別和狀態訊息應更新以符合每個所選主題。
