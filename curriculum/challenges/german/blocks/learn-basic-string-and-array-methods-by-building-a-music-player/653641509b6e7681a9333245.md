...
Sie müssen den aktuell gespielten Song sowie das Aussehen des `playButton`-Elements aktualisieren.

Weisen Sie `song` der `currentSong`-Eigenschaft des `userData`-Objekts zu.

*Hinweis*: Sie sollten den optionalen Verkettungsoperator `?.` in diesem Schritt nicht verwenden, da `userData.currentSong` zu diesem Zeitpunkt weder `null` noch `undefined` sein wird.

# --hints--

Sie sollten `song` `userData.currentSong` zuweisen.
