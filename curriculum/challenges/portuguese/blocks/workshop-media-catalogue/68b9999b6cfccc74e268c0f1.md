...
Antes do seu loop `for`, crie uma declaração `if` que adicione a string `=== MOVIES ===` (terminando com um caractere de nova linha) ao valor atual de `result` quando a lista `movies` não estiver vazia.

# --hints--

Você deve ter uma declaração `if` no seu método `__str__` que verifica se a lista `movies` não está vazia.
...
Você deve ter `result += '=== MOVIES ===\n'` dentro do seu `if` statement.
