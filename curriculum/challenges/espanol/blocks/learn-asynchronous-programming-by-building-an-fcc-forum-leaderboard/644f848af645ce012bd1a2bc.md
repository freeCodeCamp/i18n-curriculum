...
Crea una constante llamada `linkText` y asigna el valor de `selectedCategory.category`. Esto mostrará el nombre de la categoría en el elemento anclaje.

# --hints--

Debes tener una constante llamada `linkText`.

```js
assert(code.match(/const\s+linkText\s*=/));
```

Debes asignar `selectedCategory.category` a tu variable `linkText`.
