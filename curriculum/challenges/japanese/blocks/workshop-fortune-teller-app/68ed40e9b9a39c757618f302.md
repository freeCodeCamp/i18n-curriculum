...
今度は、DOM内でUIを動的にレンダリングするのに役立つメインのクラスが必要です。

`Game` という名前のクラスを作成してください。その中に、`cards` 型の `Card[]` というプロパティを追加し、今は空の配列で初期化してください。

# --hints--

`Game` という名前のクラスがあるはずです。
...
`cards` クラスの中に `Game` プロパティがあるはずです。
...
cards プロパティは空の `array` に設定されているはずです。
