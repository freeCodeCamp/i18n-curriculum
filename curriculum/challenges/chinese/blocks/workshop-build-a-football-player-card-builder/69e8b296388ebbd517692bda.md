...
为图像 URL `value` 添加一个 `player.imageUrl` 属性，属性值为 `input`。然后将 `onChange` 属性设置为一个箭头函数，该函数以 `e` 作为参数，并调用 `setPlayer`，传入 `player` 的展开和 `imageUrl` 设置为 `e.target.value`。

# --hints--

你的图像 URL `input` 应该有一个 `value`，其值等于玩家的图像 URL。
...
你的图像 URL `input` 应该有一个 `onChange` 处理程序，该处理程序调用 `setPlayer`，传入 `player` 的展开和 `imageUrl` 设置为 `e.target.value`。
