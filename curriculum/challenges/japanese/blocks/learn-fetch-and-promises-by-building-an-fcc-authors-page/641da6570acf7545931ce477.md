...
データの分割代入を使って、ページのHTMLを作成し始めましょう。これは複合代入演算子（`+=`）と`innerHTML`プロパティの組み合わせで行えます。

コールバック関数の中で、複合代入演算子を使って`innerHTML`の`authorContainer`に空のテンプレートリテラルを追加してください。
...
`innerHTML`の`authorContainer`にアクセスしてください。

```js
assert.match(code, /authorContainer\.innerHTML\s*/)
```

複合代入を使って、`innerHTML`の`authorContainer`に空のテンプレートリテラルを追加してください。
