...
Finalmente, agora que você completou o setter `salary`, você pode chamá-lo dentro do setter `level`.

Modifique a linha `self._salary = Employee._base_salaries[new_level]` para que ela chame o setter `salary` em vez de modificar diretamente o atributo `_salary`.

Com isso, o workshop do salary tracker está completo.

# --hints--

Você deve definir `self.salary` como `Employee._base_salaries[new_level]` dentro do setter `level`.
