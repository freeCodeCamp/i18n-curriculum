...
宣告一個 `functionCall` 變數，並將這個正規表達式指定給它：`/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

此表達式將尋找像 `sum(1, 4)` 這樣的函式呼叫。

# --hints--

你應該宣告一個 `functionCall` 變數。
...
你應該使用 `const` 來宣告你的 `functionCall` 變數。
