...
Das erste Argument für Ihren `evalFormula`-Aufruf muss der Inhalt der Zelle sein (den Sie in `value` gespeichert haben). Da der Inhalt jedoch mit einem `=`-Zeichen beginnt, um die Funktion auszulösen, müssen Sie den Teilstring von `value` übergeben, der am Index `1` beginnt.

# --hints--

Sie sollten `value` als erstes Argument an Ihren `evalFormula()`-Aufruf übergeben.
...
Sie sollten die `.slice()`-Methode am Argument `value` aufrufen.
...
Sie sollten die Zahl `1` als Argument an Ihren `.slice()`-Aufruf übergeben.
