...
Ahora necesitas trabajar en pausar la canción que se está reproduciendo actualmente.

Define una función `pauseSong` que no acepte parámetros. Dentro de tu nueva función, para almacenar el tiempo actual de la canción cuando se pausa, establece el `songCurrentTime` del objeto `userData` al `currentTime` de la variable `audio`.


# --hints--

Debes tener una función `pauseSong`.

```js
assert.isFunction(pauseSong);
```

Tu función `pauseSong` debe establecer el `songCurrentTime` del objeto `userData` al `currentTime` de la variable `audio`.
