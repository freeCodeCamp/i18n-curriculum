...
Тепер ви почнете створювати форму. Всередині `div` з `className` `form-panel` додайте `div`, щоб обгорнути розділ "Player Info". Всередині цього нового `div` додайте `p` з `className` `form-section-title` і текстом `Player Info`.

Ви продовжите додавати поля до цього розділу в наступних кроках.

# --hints--

Ваш `div.form-panel` має містити `div`, щоб обгорнути розділ Player Info.
...
`div` всередині `div.form-panel` має містити `p` з `className` `form-section-title` і текстом `Player Info`.
