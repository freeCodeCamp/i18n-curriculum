...
Тепер додайте `div` з класом `img-loader`. Після цього створіть `img` елемент і задайте йому умовне посилання на зображення, використовуючи ``${img ? `${CDN_URL}/${img}` : LOCAL_DEFAULT_IMG}``.
