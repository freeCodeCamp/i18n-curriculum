...
De vuelta en tu controlador de eventos, debes actualizar el texto del elemento `result`. Puedes usar un operador ternario para lograr esta tarea.

Aquí hay un ejemplo de cómo asignar el resultado de un operador ternario al contenido de texto de un elemento:

```js
el.textContent = condition ? "Use this text if the condition is true" : "Use this text if the condition is false";
```

Después de la instrucción `if`, utiliza un operador ternario para verificar la veracidad de llamar a `isSpam()` con `messageInput.value` como argumento.  If true, set the `textContent` property on the `result` element to `"Oh no! This looks like a spam message."`. De lo contrario, establécelo en `"This message does not seem to contain any spam."`

Luego, establece la propiedad `value` del elemento `messageInput` en una cadena vacía.

# --hints--

Debes usar el operador de asignación para establecer la propiedad `textContent` del elemento `result`.
...
Debes llamar a la función `isSpam()` después del operador de asignación `=` y antes del operador ternario `?`.
...
Debes usar la sintaxis ternaria para verificar la veracidad de `isSpam(messageInput.value)`.
...
La expresión truthy de tu ternario debe establecer la propiedad `textContent` del elemento `result` a `"Oh no! This looks like a spam message."`.
...
La expresión falsy de tu ternario debe establecer la propiedad `textContent` del elemento `result` en `"This message does not seem to contain any spam."`.
...
Después de tu ternario, establece la propiedad `value` en el elemento `messageInput` en una cadena vacía.
