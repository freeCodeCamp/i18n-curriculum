...
يجب أن يكون لديك منتقي `.author`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author'));
```

يجب أن يكون لمنتقي `.author` الخاص بك خاصية `font-size` بقيمة `2rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

منتقي `.author` يجب أن يكون لديه خاصية `font-family` بقيمة `Raleway`، مع fallback بقيمة `sans-serif`.
