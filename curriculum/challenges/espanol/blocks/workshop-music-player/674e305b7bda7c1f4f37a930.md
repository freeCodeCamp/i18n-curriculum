...
Antes de empezar a trabajar en reproducir la siguiente y la anterior canción, necesitas obtener el índice de cada canción en la propiedad `songs` de `userData`. Para hacer eso, puede usar el método de array `indexOf()`, que devuelve el primer índice en el que un elemento dado puede ser encontrado en el array, o `-1` si el elemento no está presente.
...
Cree una función llamada `getCurrentSongIndex` que devuelva el índice en el que `userData.currentSong` puede ser encontrado en `userData.songs` y `-1` cuando la canción actual es `null`.

# --hints--

Debe tener una función llamada `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

Su función `getCurrentSongIndex` debe devolver el índice en el que `userData.currentSong` puede ser encontrado en `userData.songs`.
...
Su función `getCurrentSongIndex` debe devolver `-1` cuando `userData.currentSong` es `null`.
