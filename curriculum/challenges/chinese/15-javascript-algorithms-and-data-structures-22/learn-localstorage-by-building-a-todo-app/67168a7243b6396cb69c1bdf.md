...
在 `title` 上调用 `removeSpecialChars`，并在 `taskObj` 中调用 `description` 属性。 对于 `id` 属性，仅在属性值的 `titleInput.value` 部分调用它。
