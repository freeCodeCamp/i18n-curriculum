...
Endelea na jengo linalofuata! Ni lile la kijani lililo mbele. Mpe `repeating-linear-gradient` rangi yako ya jengo kutoka `0%` hadi `5%`, na `transparent` kutoka `5%` hadi `10%`.

# --hints--

Unapaswa kumpa `.fb5` sifa ya `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

Unapaswa kumpa `background` sifa ya `repeating-linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

Unapaswa kumpa `repeating-linear-gradient` rangi ya kwanza ya `--building-color2` kutoka `0%` hadi `5%`.
...
Unapaswa kumpa `repeating-linear-gradient` rangi ya pili ya `transparent` kutoka `5%` hadi `10%`.
