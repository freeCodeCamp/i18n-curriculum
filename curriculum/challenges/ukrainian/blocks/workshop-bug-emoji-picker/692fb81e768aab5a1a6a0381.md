...
У класі `Spider` створіть конструктор, який приймає один параметр `emojiElement` типу `HTMLParagraphElement`.

У конструкторі викличте функцію `super` з параметром `emojiElement`, щоб викликати конструктор батьківського класу `Bug`. Потім встановіть властивість `emoji` у рядкове значення `"🕷️"`.

# --hints--

Ваш клас `Spider` має мати конструктор.
...
Ваш конструктор `Spider` має мати параметр з іменем `emojiElement` типу `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Ваш конструктор `Spider` має викликати `super` з параметром `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?super\s*\(\s*emojiElement\s*\)/);
```

Ваш конструктор `Spider` має встановити `this.emoji` у `"🕷️"`.
