...
تمت إضافة الأدوات المتبقية لك في مصفوفة `instrumentsArr`.

الآن، أعلن عن متغيرين `selectContainer` و `productsContainer` وقم بتعيين قائمة HTML المنسدلة وعنصر الصنف `products-container` لهما على التوالي.

# --hints--

يجب أن يكون لديك متغير `selectContainer`.

```js
assert.exists(selectContainer);
```

يجب أن يحتوي متغير `selectContainer` على قيمة عنصر `select` الخاص بك.

```js
assert.strictEqual(selectContainer, document.querySelector("select"));
```

يجب أن يكون لديك متغير `productsContainer`.

```js
assert.exists(productsContainer);
```

يجب أن يحتوي متغير `productsContainer` على قيمة عنصر `.products-container` الخاص بك.
