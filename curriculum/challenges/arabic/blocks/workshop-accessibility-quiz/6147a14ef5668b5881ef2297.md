...
لجعل أزرار التنقل تبدو أكثر كأزرار نموذجية، أزل الخط السفلي من وسم الرابط للنص العالي.

بعد ذلك، أنشئ محدِّدًا جديدًا يستهدف عناصر قائمة التنقل بحيث عند تحويم المؤشر فوقها، يتبدل لون الخلفية ولون النص، ويصبح المؤشر على شكل مؤشر يد.

# --hints--

يجب استخدام محدِّد `li > a` الموجود لضبط خاصية `text-decoration` إلى `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

يجب استخدام إما محدِّد `nav > ul > li:hover` أو `nav li:hover` لتنسيق العناصر عند التحويم.
...
يجب أن تُعطى عناصر `li` التي تحوم فوقها خاصية `background-color` بقيمة `#dfdfe2`.
...
يجب أن تُعطى عناصر `li` التي تحوم فوقها خاصية `color` بالقيمة `#1b1b32`.
...
يجب إعطاء عناصر `li` عند التحويم مؤشر `cursor` من نوع `pointer`.
