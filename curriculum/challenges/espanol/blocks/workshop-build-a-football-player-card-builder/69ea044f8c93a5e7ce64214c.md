...
En el bloque de la declaración `if`, devuelve un objeto que expanda tanto `defaultPlayer` como `JSON.parse(saved)` por separado. Asegúrate de que estén separados por una coma, ya que son dos objetos distintos que se combinan en uno solo.

# --hints--

Tu bloque `if (saved)` debe devolver un objeto que expanda tanto `defaultPlayer` como los datos guardados analizados.
