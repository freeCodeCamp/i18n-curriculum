...
Sasa unaweza kuanza kuunganisha mantiki yako na kiolesura cha mtumiaji. Utahitaji kufanya hoja ya utafutaji kwenye DOM kupata vipengele kadhaa kwa kutumia ID. Pata vipengele vyenye ID za `ingredient`, `quantity`, `unit`, `servings`, `recipe-form`, na `result-list`, na weka thamani zao katika vigezo `ingredientName`, `ingredientQuantity`, `unitToConvert`, `numberOfServings`, `recipeForm`, na `resultList`, kwa mpangilio huo huo.

# --hints--

Unapaswa kufafanua kigezo cha `ingredientName`.

```js
assert.isDefined(ingredientName);
```

Kigezo chako cha `ingredientName` kinapaswa kuwa kipengele kilichorejeshwa kutoka `.getElementById()` chenye ID ya `ingredient`.

```js
assert.strictEqual(ingredientName, document.getElementById('ingredient'));
```

Unapaswa kufafanua kigezo cha `ingredientQuantity`.

```js
assert.isDefined(ingredientQuantity);
```

Kigezo chako cha `ingredientQuantity` kinapaswa kuwa kipengele kilichorejeshwa kutoka `.getElementById()` chenye ID ya `quantity`.

```js
assert.strictEqual(ingredientQuantity, document.getElementById('quantity'));
```

Unapaswa kufafanua kigezo cha `unitToConvert`.

```js
assert.isDefined(unitToConvert);
```

Kigezo chako cha `unitToConvert` kinapaswa kuwa kipengele kilichorejeshwa kutoka `.getElementById()` chenye ID ya `unit`.

```js
assert.strictEqual(unitToConvert, document.getElementById('unit'));
```

Unapaswa kufafanua kigezo cha `numberOfServings`.

```js
assert.isDefined(numberOfServings);
```

Kigezo chako cha `numberOfServings` kinapaswa kuwa kipengele kilichorejeshwa kutoka `.getElementById()` chenye ID ya `servings`.

```js
assert.strictEqual(numberOfServings, document.getElementById('servings'));
```

Unapaswa kufafanua kigezo cha `recipeForm`.

```js
assert.isDefined(recipeForm);
```

Kigezo chako cha `recipeForm` kinapaswa kuwa kipengele kilichorejeshwa kutoka `.getElementById()` chenye ID ya `recipe-form`.

```js
assert.strictEqual(recipeForm, document.getElementById("recipe-form"));
```

Unapaswa kufafanua kigezo cha `resultList`.

```js
assert.isDefined(resultList);
```

Kigezo chako cha `resultList` kinapaswa kuwa kipengele kilichorejeshwa kutoka `.getElementById()` chenye ID ya `result-list`.
