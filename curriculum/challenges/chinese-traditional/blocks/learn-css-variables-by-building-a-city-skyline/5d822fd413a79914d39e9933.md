...
在你剛剛添加的 `repeating-linear-gradient` 下方再添加另一個。給它一個 `90deg` 方向，從 `0%` 到 `12%` 使用你的建造顏色，從 `12%` 到 `44%` 使用視窗顏色。這將會產生一堆矩形視窗。

# --hints--

你應該在 `background` 屬性的 `.fb5` 中給予第二個 `repeating-linear-gradient`。
...
你應該給第二個 `repeating-linear-gradient` 一個從 `0%` 到 `12%` 的第一個顏色 `--building-color2`。
...
你應該給第二個 `repeating-linear-gradient` 在 `12%` 到 `44%` 之間添加第二個顏色 `--window-color2`。
