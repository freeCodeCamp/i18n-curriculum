...
Il secondo elemento `img` dovrebbe avere un attributo `src` con il valore `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`.
...
Il secondo elemento `img` dovrebbe avere l'attributo `alt` impostato su `image of a calculator project`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('alt') === 'image of a calculator project');
```

Il secondo elemento `img` dovrebbe avere un attributo `loading` impostato su `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('loading') === 'lazy');
```

Il secondo elemento `img` dovrebbe avere l'attributo `class` impostato su `image-2`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.classList?.contains('image-2'));
```

Il secondo elemento `img` dovrebbe avere un attributo `width` impostato a `400`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('width') === '400');
```

Il secondo elemento `img` dovrebbe avere un attributo `height` impostato a `400`.
