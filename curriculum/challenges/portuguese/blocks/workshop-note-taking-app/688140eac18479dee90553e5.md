...
Como você está usando outro elemento `div`, será necessário tornar isso mais acessível para leitores de tela. 

Em uma lição anterior, você aprendeu sobre o atributo `aria-live` que é usado para criar uma região ao vivo em uma página. Isso permitirá que usuários de leitores de tela sejam automaticamente notificados quando o conteúdo da região ao vivo mudar, sem a necessidade de focar manualmente ou interagir com ela.

No seu elemento `#status`, adicione um atributo `aria-live` e defina seu valor como `"polite"`. 

O valor `polite` indicará aos leitores de tela para aguardarem até que qualquer anúncio atual seja concluído ou até que o usuário pare de digitar antes de anunciar a atualização.

# --hints--

Seu elemento `#status` deve ter o atributo `aria-live`.

```js
assert.isTrue(document.getElementById("status").hasAttribute("aria-live"));
```

Seu elemento `#status` deve ter o atributo `aria-live` definido como `"polite"`.
