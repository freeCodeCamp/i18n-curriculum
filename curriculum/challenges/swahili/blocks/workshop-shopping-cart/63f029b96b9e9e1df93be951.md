...
Sasa unaweza kusahihisha jumla ya vitu kwenye ukurasa wa mtandao. Weka thamani ya njia ya `cart` ya kitu chako cha `.getCounts()` kwenye sifa ya `textContent` ya kigezo cha `totalNumberOfItems`.

# --hints--

Unapaswa kupata upatikanaji wa sifa ya `textContent` ya kigezo cha `totalNumberOfItems`.

```js
assert.match(code, /totalNumberOfItems\s*\.\s*textContent/)
```

Unapaswa kuweka thamani ya njia ya `cart` ya kitu chako cha `.getCounts()` kwenye sifa ya `textContent` ya kigezo cha `totalNumberOfItems`.
