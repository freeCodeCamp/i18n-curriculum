...
Per questo algoritmo, dovrai usare un loop `while`. Questo loop richiede due condizioni:

- In primo luogo, non dovrebbe essere eseguito oltre l'inizio dell'array (accessibile con `j`).
- In secondo luogo, il loop non dovrebbe funzionare dopo aver trovato un valore inferiore al valore corrente.

Per evitare un loop infinito, decrementa `j` all'interno del loop.
...
La prima condizione del loop `while` dovrebbe verificare che il valore di `j` sia maggiore o uguale a `0`.
...
Il loop`while` dovrebbe utilizzare un operatore AND.
...
Il loop `while` dovrebbe avere una condizione che verifica che il valore di `array[j]` sia maggiore di `currValue`.
...
Il loop `while` dovrebbe decrementare `j` all'interno del loop.
