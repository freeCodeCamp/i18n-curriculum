...
関数の最後で、計算したデータを返して関数の外で使えるようにしてください。

`distances` から他のすべてのノードへの最短 `start_node` のリストと、各ノードに至る `paths` のリストの両方を返してください。

# --hints--

関数の最後に `return` 文があるはずです。
...
`distances` と `paths` の両方を返す必要があります。
