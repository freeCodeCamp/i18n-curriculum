...
Aggiungi un divisore medio dopo il tuo elemento `.daily-value`. Sotto quel nuovo divisore, crea un elemento `p` con l'attributo `class` impostato su `note`.
...
Il tuo nuovo `div` dovrebbe avere l'attributo `class` impostato su `divider medium`.
...
Il tuo nuovo elemento `p` dovrebbe avere l'attributo `class` impostato su `note`.
