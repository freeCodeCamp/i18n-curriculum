...
O parâmetro `fn` receberá o nome de uma função, como `"SUM"`. Atualize `apply` para retornar implicitamente a função do seu objeto `spreadsheetFunctions` usando a variável `fn` como a chave para o acesso ao objeto.

Lembre-se de que `fn` pode não conter uma string em minúsculas, então você precisará convertê-la para uma string em minúsculas.

# --hints--

A função `apply` deve usar um retorno implícito.
...
A função `apply` deve acessar o objeto `spreadsheetFunctions`.
...
A função `apply` deve acessar a propriedade do objeto `spreadsheetFunctions` que corresponda ao valor de `fn`.
...
A função `apply` deve chamar o método `.toLowerCase()` em `fn` no acesso à propriedade.
