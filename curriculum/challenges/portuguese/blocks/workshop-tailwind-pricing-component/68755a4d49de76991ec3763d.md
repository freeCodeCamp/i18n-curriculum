...
Agora, atribua ao `span` dentro do elemento `p` as classes `text-base`, `font-medium` e `text-gray-500`.

# --hints--

O `span` dentro do seu elemento `p` deve ter a classe `text-base`.
...
O `span` dentro do seu elemento `p` deve ter a classe `font-medium`.
...
O `span` dentro do seu elemento `p` deve ter a classe `text-gray-500`.
