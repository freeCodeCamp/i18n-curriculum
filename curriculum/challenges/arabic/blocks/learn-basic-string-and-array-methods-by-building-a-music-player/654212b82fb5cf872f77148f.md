...
داخل عنصر `button`، أنشئ عنصرين إضافيين من نوع `span`.

يجب أن يحتوي أول عنصر `span` على `class` باسم `playlist-song-artist`. أضف بين وسمَي `span` القيمة `${song.artist}`.

يجب أن يحتوي ثاني عنصر `span` على `class` باسم `playlist-song-duration`. أضف بين وسمَي `span` القيمة `${song.duration}`.

# --hints--

لا تعدل عنصر `button` الحالي أو محتواه.
...
أنشئ عنصر `span` مع الصنف `playlist-song-artist`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

يجب أن يحتوي عنصر `span` مع الصنف `playlist-song-artist` على قيمة `song.artist` كمحتوى له.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

أنشئ عنصر `span` مع الصنف `playlist-song-duration`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

يجب أن يحتوي عنصر `span` مع الصنف `playlist-song-duration` على قيمة `song.duration` كمحتوى له.
