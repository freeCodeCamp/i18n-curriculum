...
Usa la propiedad `margin` para centrar horizontalmente el elemento `.penguin`, y establece el `margin-top` en `75px`.

# --hints--

Debe dar a `.penguin` un `margin` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.
...
Debe dar a `.penguin` un `margin-top` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.
