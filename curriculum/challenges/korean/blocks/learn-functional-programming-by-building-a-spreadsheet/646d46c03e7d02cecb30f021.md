...
이제 함수 파서를 `evalFormula` 논리에 적용하기 시작할 수 있습니다. `functionExpanded` 변수를 선언하고, `applyFunction` 문자열을 인수로 하여 `cellExpanded`를 호출한 결과를 할당하세요.

# --hints--

`functionExpanded` 변수를 선언해야 합니다.
...
`cellExpanded`를 `applyFunction` 호출에 전달하세요.
