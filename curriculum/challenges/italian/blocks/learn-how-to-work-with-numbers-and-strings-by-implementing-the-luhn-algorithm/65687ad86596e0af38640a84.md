...
Usa un loop `for` per iterare su ogni cifra nella lista `odd_digits`. Sposta la tua chiamata `print` dal passaggio precedente all'interno del loop `for` e modificala per stampare ogni cifra.

# --hints--

Dovresti usare un loop `for` su `odd_digits`.
...
Dovresti avere `--fcc-expected--` all'interno del loop `for`.
