...
Коли натискається кнопка перемішування, потрібно встановити `currentSong` у порожнє значення, а `songCurrentTime` — у `0`.

Встановіть `userData.currentSong` у `null`, а `userData.songCurrentTime` — у `0`.

*Примітка*: не слід використовувати опціональний ланцюжок для цього кроку, оскільки ви явно встановлюєте властивості `currentSong` і `songCurrentTime` відповідно у `null` і `0`.

# --hints--

Ви повинні встановити `userData.currentSong` у `null`.

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

Ви повинні встановити `userData.songCurrentTime` у `0`.
