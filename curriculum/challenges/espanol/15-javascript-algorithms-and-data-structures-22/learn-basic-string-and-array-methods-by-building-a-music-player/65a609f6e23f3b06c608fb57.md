...
El método `sort()` acepta una función de devolución de llamada de comparación que define el orden de clasificación.

En este ejemplo, la primera condición `(a.name < b.name)` verifica si el nombre de la primera fruta es menor que el nombre de la segunda fruta. Si es así, la primera fruta se ordena antes que la segunda fruta.

Las cadenas se comparan lexicográficamente, lo que significa que se comparan carácter por carácter. Por ejemplo, `"Apples"` es menor que `"Bananas"` porque `"A"` viene antes que `"B"` en el alfabeto.

La razón por la cual este ejemplo está devolviendo números es porque el método `sort()` espera que se devuelva un número. Si devuelves un número negativo, el primer elemento se ordena antes que el segundo elemento.
...
Dentro de tu función de devolución de llamada, agrega una instrucción `if` para verificar si `a.title` es menor que `b.title`. Si es así, devuelve `-1`.

# --hints--

Deberías tener una instrucción `if` dentro de tu función de devolución de llamada.

```js
assert.match(code, /if\s*\(/);
```

Tu instrucción `if` debe verificar si `a.title` es menor que `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

Tu instrucción `if` debe devolver `-1` si `a.title` es menor que `b.title`.
