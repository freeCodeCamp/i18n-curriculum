...
`.sr-only` のテキストがまだ見えています。スクリーンリーダーだけが読み取れるようにテキストを視覚的に隠す一般的なパターンがあります。

このパターンは、以下のCSSプロパティを設定することです：
