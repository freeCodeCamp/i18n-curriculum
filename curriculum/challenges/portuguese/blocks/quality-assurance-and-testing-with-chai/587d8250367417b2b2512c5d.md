...
Quando a requisição é concluída com sucesso, o código cliente adiciona um `<div>` contendo as informações da resposta ao DOM.
...
Primeiro, o método `fill` do objeto `browser` preenche o campo `surname` do formulário com o valor `'Polo'`. `fill` retorna uma promise, então `then` é encadeado a partir dela.
...
4.  Há `1` elemento `<span id='dates'></span>`.
