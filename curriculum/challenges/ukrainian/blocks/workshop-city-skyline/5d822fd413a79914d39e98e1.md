...
Це не спрацювало, бо змінні, які ви оголосили в `.bb1`, не наслідуються братськими елементами `.bb2` і `.bb3`. Саме так працює CSS. Через це змінні часто оголошують у селекторі `:root`. Це найвищий рівень селектора в CSS; розміщення змінних там зробить їх доступними скрізь. Додайте селектор `:root` на початок вашого стилю і перемістіть туди всі оголошення змінних.

# --hints--

Ви повинні оголосити селектор `:root` на початку стилю.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root'));
```

Ви повинні визначити `--building-color1` зі значенням `#aa80ff` у селекторі `:root`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color1')?.trim(), '#aa80ff');
```

Ви повинні визначити `--building-color2` зі значенням `#66cc99` у селекторі `:root`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color2')?.trim(), '#66cc99');
```

Ви повинні визначити `--building-color3` зі значенням `#cc6699` у селекторі `:root`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color3')?.trim(), '#cc6699');
```

Ви повинні видалити змінні властивостей із `.bb1`.
