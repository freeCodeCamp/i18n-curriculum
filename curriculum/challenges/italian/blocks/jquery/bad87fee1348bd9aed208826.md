...
title: Consultare i figli di un elemento usando jQuery
challengeType: 6
forumTopicId: 18320
...
Quando gli elementi HTML sono posizionati un livello sotto un altro, si chiamano <dfn>figli</dfn> di quell'elemento. Per esempio, gli elementi button in questa sfida con il testo `#target1`, `#target2` e `#target3` sono tutti figli dell'elemento `<div class="well" id="left-well">`.

jQuery ha una funzione chiamata `children()` che ti permette di consultare i figli di qualunque elemento tu abbia selezionato.
...
Tutti i figli di `#right-well` dovrebbero avere il testo arancione.
