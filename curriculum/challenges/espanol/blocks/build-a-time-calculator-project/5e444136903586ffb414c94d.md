...
title: Construye un Proyecto Calculadora de Tiempo
challengeType: 23
...
Escribe una función nombrada `add_time` que tome dos parámetros requeridos y un parámetro opcional:

- una hora de comienzo con el formato de 12 horas (terminando en AM o PM)
- un tiempo de duración indicando el número de horas y minutos
- (opcional) día de la semana de comienzo, en mayúsculas o minúsculas

El función debe agregar la duración a la hora de inicio y devolver el resultado.
...
# --hints--
Llamar a `add_time('3:30 PM', '2:12')` debería devolver `'5:42 PM'`.
...
Llamar a `add_time('11:55 AM', '3:12')` debería devolver `'3:07 PM'`.
...
Tiempo esperado para terminar con `'(next day)'` cuando es el día siguiente.
...
Se esperaba que el período cambiara de `AM` a `PM` a las `12:00`.
...
Llamar a `add_time('2:59 AM', '24:00')` debería devolver `'2:59 AM (next day)'`.
...
Llamar a `add_time('11:59 PM', '24:05')` debería devolver `'12:04 AM (2 days later)'`.
...
Llamar a `add_time('8:16 PM', '466:02')` debería devolver `'6:18 AM (20 days later)'`.
...
Se esperaba agregar `0:00` para devolver el tiempo inicial.
...
Llamar a `add_time('3:30 PM', '2:12', 'Monday')` debería devolver `'5:42 PM, Monday'`.
...
Llamar a `add_time('2:59 AM', '24:00', 'saturDay')` debería devolver `'2:59 AM, Sunday (next day)'`.
...
Llamar a `add_time('11:59 PM', '24:05', 'Wednesday')` debería devolver `'12:04 AM, Friday (2 days later)'`.
...
Llamar a `add_time('8:16 PM', '466:02', 'tuesday')` debería devolver `'6:18 AM, Monday (20 days later)'`.
