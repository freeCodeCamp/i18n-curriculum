...
Una forma de encontrar elementos específicos en tu HTML es usar el método `querySelector()`. El método `querySelector()` toma un selector CSS como argumento y devuelve el primer elemento que coincida con ese selector.  For example, to find the `<h1>` element in your HTML, you would write:
