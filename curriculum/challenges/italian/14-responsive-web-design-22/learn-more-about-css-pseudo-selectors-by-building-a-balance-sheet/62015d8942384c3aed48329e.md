...
Il selettore `section` dovrebbe avere una proprietà `max-width` con il valore `40rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('section')?.getPropertyValue('max-width') === '40rem');
```

Il selettore `section`dovrebbe avere una proprietà `margin` con il valore `0 auto`.

```js
assert(new __helpers.CSSHelp(document).getStyle('section')?.getPropertyValue('margin') === '0px auto');
```

Il selettore `section` dovrebbe avere una proprietà `border` con il valore `2px solid #d0d0d5`.
