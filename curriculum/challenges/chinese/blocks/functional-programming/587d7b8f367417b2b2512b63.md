...
回调函数接收三个参数。 第一个参数是当前正在处理的数组项。 第二个参数是这个元素的索引，第三个参数是在其上调用 `filter` 方法的数组。
...
`watchList` 是包含一些电影信息的对象。 结合 `filter` 和 `map` 返回一个 `watchList` 只包含 `title` 和 `rating` 属性的新数组。 新数组只包含 `imdbRating` 值大于或等于 8.0 的对象。 请注意，`rating` 值在对象中保存为字符串，你可能需要将它转换成数字来执行运算。
