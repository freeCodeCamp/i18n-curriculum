...
雑誌の最後のセクション、画像のスタイルを設定する時間です。

画像は`aside`要素で囲まれており、`image-wrapper`クラスが付いているので、`.image-wrapper`セレクターを作成してください。`display`プロパティを`grid`に設定します。

# --hints--

`.image-wrapper`セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

`.image-wrapper`セレクターには`display`プロパティを`grid`に設定してください。
