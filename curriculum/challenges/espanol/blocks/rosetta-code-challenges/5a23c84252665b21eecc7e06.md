...
Un rectángulo dado está hecho de cuadrados *m* × *n*. Si *m* y *n* no son ambos impares, entonces es posible cortar un camino a través del rectángulo a lo largo de los bordes de los cuadrados de modo que el rectángulo se divida en dos piezas conectadas con la misma forma (después de rotar una de las piezas 180°). Todos esos caminos para rectángulos 2 × 2 y 4 × 3 se muestran a continuación.
...
Escribe una función que calcule el número de formas diferentes de cortar un rectángulo *m* × *n*.

# --hints--

`cutRectangle` debería ser una función.

```js
assert(typeof cutRectangle == 'function');
```

`cutRectangle(2, 2)` debería devolver un número.

```js
assert(typeof cutRectangle(2, 2) == 'number');
```

`cutRectangle(2, 2)` debería devolver `2`.

```js
assert.equal(cutRectangle(2, 2), 2);
```

`cutRectangle(4, 3)` debería devolver `9`.

```js
assert.equal(cutRectangle(4, 3), 9);
```

`cutRectangle(4, 4)` debería devolver `22`.

```js
assert.equal(cutRectangle(4, 4), 22);
```

`cutRectangle(8, 3)` debería devolver `53`.

```js
assert.equal(cutRectangle(8, 3), 53);
```

`cutRectangle(7, 4)` debería devolver `151`.
