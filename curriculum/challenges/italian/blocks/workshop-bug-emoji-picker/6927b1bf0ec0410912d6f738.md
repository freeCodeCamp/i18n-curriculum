...
Sempre all'interno della classe `Bug`, dichiara una proprietà chiamata `emojiElement` con il tipo `HTMLParagraphElement`. Questa proprietà conterrà un riferimento all'elemento paragrafo HTML dove verrà mostrata l'emoji del bug.

# --hints--

La tua classe `Bug` dovrebbe avere una proprietà chiamata `emojiElement`.
...
La tua proprietà `emojiElement` dovrebbe avere il tipo `HTMLParagraphElement`.
...
La tua proprietà `emojiElement` dovrebbe usare l'operatore di asserzione di assegnazione definita (`!`).
