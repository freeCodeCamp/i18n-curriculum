...
通过在你的方法内打印 `'{name}' promoted to '{new_level}'.` 来完成 setter。分别将 `{name}` 和 `{new_level}` 替换为员工的姓名和新级别。

# --hints--

你应该打印 `'{name}' promoted to '{new_level}'.`，其中 `{name}` 和 `{new_level}` 应分别替换为员工的姓名和新级别。
