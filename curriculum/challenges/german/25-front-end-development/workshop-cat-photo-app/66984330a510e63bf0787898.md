...
Your `ul` element should have an opening tag. Öffnende Tags haben diese Syntax: `<elementName>`.

```js
assert.exists(document.querySelector('ul'));
```

Your `ul` element should have a closing tag. Abschließende Tags enthalten ein `/` direkt nach dem `<`-Zeichen.
