...
Uma promessa possui três estados: pendente (`pending`), cumprida (`fulfilled`) e rejeitada (`rejected`).A promessa que você criou no desafio anterior está presa no estado `pending` para sempre porque você não adicionou uma forma de concluir a promessa.Os parâmetros `resolve` e `reject` fornecidos ao argumento da promise são usados para isso. `resolve` é usado quando você quer que sua promise seja bem-sucedida, e `reject` é usado quando você quer que ela falhe.Ambos são métodos que recebem apenas um argumento, como no exemplo abaixo.
