...
Il costruttore `Array()` ha un metodo `.fill()` che può essere usato per riempire un array con un valore. Puoi usarlo per riempire il tuo array con il valore `start`.

Collega il metodo `.fill()` al tuo costruttore `Array()` e passagli il valore `start`.
...
Dovresti chiamare il metodo `.fill()` sul tuo costruttore `Array()`.
