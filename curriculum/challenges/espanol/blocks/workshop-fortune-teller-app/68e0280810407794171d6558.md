...
Ahora, agrega un `div` con la clase `img-loader`. Después, crea un elemento `img` y asígnale una URL condicional para la imagen usando ``${img ? `${CDN_URL}/${img}` : LOCAL_DEFAULT_IMG}``.

# --hints--

Deberías tener un elemento `div` dentro de tu elemento `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

Tu `div` debe tener la clase `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

Deberías tener un elemento `img` dentro de tu elemento `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

El elemento `img` debe usar `CDN_URL` y `img` para su `src` cuando se proporcione `img`, y usar `LOCAL_DEFAULT_IMG` como alternativa cuando no se proporcione.
...
Tu elemento `img` debe tener las clases `card-img` y `hidden`.
