...
`messageInput`가 비어 있으면, `"Please enter a message."` 메시지로 사용자에게 경고를 표시하세요.

그런 다음, 함수 실행을 종료하세요.

# --hints--

콜백 함수에는 `if` 문장이 있어야 합니다.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

`if` 문장은 `value`의 `messageInput`가 빈 문자열인지 확인해야 합니다.
...
`if` 문장은 `"Please enter a message."` 메시지로 사용자에게 경고를 표시해야 합니다.
