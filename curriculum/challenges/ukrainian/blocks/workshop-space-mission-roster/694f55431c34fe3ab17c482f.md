...
Центр управління місією запросив нову функцію для розбиття екіпажу на частини змінного розміру. Створіть порожню функцію з назвою `chunkCrew`, яка приймає два параметри, `crew` і `size`.

# --hints--

Ви повинні створити функцію з назвою `chunkCrew`.

```js
assert.isFunction(chunkCrew);
```

Ваша функція `chunkCrew` має мати параметри `crew` і `size`.
