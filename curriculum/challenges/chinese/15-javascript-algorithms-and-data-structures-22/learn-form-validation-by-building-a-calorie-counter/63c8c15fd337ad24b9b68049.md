...
在 JavaScript 中，值可以是 <dfn>truthy</dfn> 或 <dfn>falsy</dfn>。 如果将某个值转换为布尔值时结果为 `true`，则该值是真值。 如果将某个值转换为布尔值时结果为 `false`，则该值即为假值。 `null` 是假值的一个例子。
...
添加 `if` 语句来检查 `invalidInputMatch` 是否为真。

# --hints--

你应该向 `getCaloriesFromInputs` 函数添加 `if` 语句。
...
你应该在 `if` 条件中检查 `invalidInputMatch` 的真实性。
