...
`button` 요소를 `className`의 `submit-btn`와 `type`의 `submit`로 만드세요. 버튼에 `Join the League`라는 텍스트를 넣으세요.

그다음, 폼을 제출해 보고 어떤 일이 일어나는지 확인하세요.
...
버튼 요소에 `className` 속성을 `submit-btn`로 설정해야 합니다.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

`button` 요소에 `Join the League`을 텍스트로 넣어야 합니다.
