...
1. 你应该有一个带有 `div` 类的 `container` 元素。
1. 在 `div` 元素内，你应该有一个 `form` 元素。
1. 该表单应包含一个 `input` 元素，其类型为 `text`，且 id 为 `name`，用于输入用户的全名。
1. 你应该有另一个类型为 `input` 且 id 为 `email` 的 `email` 元素，用于输入用户的电子邮件地址。
1. 该表单应包含一个 id 为 `select` 的 `position` 元素，允许用户选择职位。
1. 你应该有一个带有 `fieldset` 类的 `radio-group` 元素。
1. 在 `.radio-group` 内，你应该有一组类型为 `input` 的 `radio` 元素和相关标签，用于选择可用性选项（例如，全职、兼职）。该组的 `name` 应为 `availability`。
1. 你应该有一个 id 为 `textarea` 的 `message` 元素用于输入消息。
1. 你应该将每个 `input`、`select` 和 `textarea` 元素与一个 `label` 元素关联。
1. 你应该有一个 `button` 元素，其类型为 `submit`，用于提交表单。
1. 为 `:focus` 和 `input` 元素添加 `textarea` 伪类，以在聚焦时更改它们的边框颜色并移除默认轮廓。
...
1. 在 `:checked` 上使用 `.radio-group input[type="radio"]` 伪类，当单选按钮被选中时添加边框颜色、背景颜色和盒子阴影。
1. 在单选按钮上使用 `:checked` 伪类，当选项被选中时更改关联的 `label` 的文本颜色。
1. 为 `:first-of-type` 元素添加 `input` 伪类，以不同方式样式化第一个输入字段。（例如，圆角）。

**注意：** 确保在 HTML 中链接样式表以应用 CSS。

# --hints--

你应该有一个带有 `div` 类的 `container` 元素。
...
你应该有一个带有类 `fieldset` 的 `section` 或 `.radio-group` 元素。

```js
assert.lengthOf(document.querySelectorAll('form fieldset.radio-group, form section.radio-group'), 1)
```

在 `.radio-group` 内，你应该有一组类型为 `input` 的 `radio` 元素，用于选择可用性选项。该组的 `name` 应为 `availability`。
...
你应该有一个 id 为 `textarea` 的 `message` 元素用于输入消息。

```js
assert.isNotEmpty(document.querySelectorAll("div.container > form textarea#message"));
```

你应该将每个 `input`、`select` 和 `textarea` 元素与一个 `label` 元素关联。
...
你应该在 `:checked` 上使用 `.radio-group input[type="radio"]` 伪类，以便在单选按钮被选中时添加边框颜色。
...
你应该在 `:checked` 上使用 `.radio-group input[type="radio"]` 伪类，以便在选中单选按钮时添加背景颜色。
...
你应该在 `:checked` 上使用 `.radio-group input[type="radio"]` 伪类，以便在选中单选按钮时添加盒子阴影。
...
为 `:first-of-type` 元素添加 `input` 伪类，以不同方式样式化第一个输入字段。
