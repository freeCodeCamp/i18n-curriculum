---
id: 69bac027df64d4433c708d44
title: Hatua 12
challengeType: 1
dashedName: step-12
---

# --description--

Unaporudisha upatikanaji wa kuzidi, unapaswa kutuma washiriki kwenye lango lifuatalo katika safu ya data ya `gates`. Ili kufanya hivyo, unahitaji kupata kielezo cha lango lifuatalo. Kawaida, unaweza kuongeza 1 kwenye kielezo cha sasa. Hata hivyo, ikiwa lango la sasa ndilo la mwisho katika safu ya data, unahitaji kurudia tena kuanzia lango la kwanza.

Unaweza kutumia operator wa modulo (`%`) kushughulikia hili:

```js
const nextIndex = (currentIndex + 1) % array.length;
```

Hii inafanya kazi kwa sababu wakati `currentIndex + 1` inalingana na `array.length`, matokeo huwa 0.

Kwa kutumia mbinu hii, tengeneza kigezo kinachoitwa `nextGateIndex` kinachohifadhi kielezo cha lango lifuatalo katika safu ya data ya `gates`.

# --hints--

Unapaswa kutumia mbinu ya modulo kupata na kuweka thamani ya kielezo kinachofuata kutoka `currentIndex` katika safu ya data ya `gates` kwenye kigezo kinachoitwa `nextGateIndex`. Fuata muundo wa mstari mmoja kama katika mfano.

```js
const rawFuncStr = __helpers.removeJSComments(rerouteOverflow.toString());
assert.match(
  rawFuncStr,
  /(var|const|let)\s+nextGateIndex/,
  "You must have at least one space between the declaration keyword (let/const) and 'nextGateIndex'."
);

const cleaned = __helpers.removeWhiteSpace(rawFuncStr);
const solution = /(var|const|let)nextGateIndex=\((currentIndex\+1|1\+currentIndex)\)%gates\.length;?}$/;

assert(
  solution.test(cleaned),
  "Inside your `rerouteOverflow` function, use the modulo technique to create a variable named `nextGateIndex` that stores the index of the next gate."
);
```

# --seed--

## --seed-contents--

```js
const morningGates = [
  { id: "North", capacity: 5, queue: [3, 6, 2, 4] },
  { id: "East", capacity: 3, queue: [2, 4, 3, 5] },
  { id: "South", capacity: 4, queue: [1, 2, 3, 1] },
  { id: "West", capacity: 2, queue: [4, 1, 2, 3] },
];

const nightGates = [
  { id: "North", capacity: 4, queue: [6, 2, 5, 1] },
  { id: "East", capacity: 2, queue: [3, 3, 4, 2] },
  { id: "South", capacity: 5, queue: [2, 1, 2, 3] },
  { id: "West", capacity: 3, queue: [5, 2, 1, 4] },
];

function initializeThroughput(gates) {
  const summary = {};
  for (const gate of gates) {
    summary[gate.id] = 0;
  };
  return summary;
}

function processGateFlow(gate, tickIndex) {
  let currentTickQueue = gate.queue[tickIndex];
  let processed = 0;
  while (currentTickQueue > 0 && processed < gate.capacity) {
    currentTickQueue--;
    processed++;
  }
  return {
    processed: processed,
    overflow: currentTickQueue
  };
}

function rerouteOverflow(gates, currentGate, tickIndex, overflowAmount) {
  const currentIndex = gates.indexOf(currentGate);
--fcc-editable-region--

--fcc-editable-region--
}
```
