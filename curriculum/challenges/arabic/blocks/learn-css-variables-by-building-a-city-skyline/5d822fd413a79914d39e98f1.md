...
تريد إضافة نفس التدرج اللوني إلى القسمين التاليين. بدلًا من ذلك، أنشئ محدِّد صنف جديد باسم `bb1-window`، وانقل خاصيتي `height` و`background` والقيم الخاصة بهما من `.bb1a` إلى محدِّد الصنف الجديد.

# --hints--

يجب أن تنشئ محدِّد صنف جديد باسم `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

يجب أن تنقل خاصية وقيمة `height` من `.bb1a` إلى `.bb1-window`.
...
يجب أن تنقل خاصية وقيمة `background` من `.bb1a` إلى `.bb1-window`.
