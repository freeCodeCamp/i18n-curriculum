...
使用类选择器来定位 `.cat-whisker-left-middle` 元素。 通过将 `position` 属性设置为 `absolute`、`top` 设置为 `127px` 以及 `left` 设置为 `52px`，把它移动到位。

# --hints--

你应该有一个 `.cat-whisker-left-middle` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-middle'))
```

你的 `.cat-whisker-left-middle` 选择器应设置 `position` 属性为 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-middle')?.position === 'absolute')
```

你的 `.cat-whisker-left-middle` 选择器应设置 `top` 属性为 `127px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-middle')?.top === '127px')
```

你的 `.cat-whisker-left-middle` 选择器应设置 `left` 属性为 `52px`。
