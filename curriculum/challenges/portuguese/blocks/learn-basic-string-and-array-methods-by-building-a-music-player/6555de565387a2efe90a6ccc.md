...
A função `setPlayButtonAccessibleText` definirá o atributo `aria-label` para a música atual ou para a primeira música na playlist. E se a playlist estiver vazia, ela define o `aria-label` como `"Play"`.

Use o método `setAttribute` no elemento `playButton` para definir um atributo chamado `"aria-label"`. Usando um ternário, defina o valor do atributo para `Play ${song.title}` ou `"Play"` se `song?.title` não estiver disponível.

Não se esqueça de que precisa de interpolação de strings aqui. Por isso, você precisa usar backticks.

# --hints--

Você não deve modificar a função `setPlayButtonAccessibleText` existente nem seu conteúdo.
...
Você deve usar o método `setAttribute()` em `playButton`.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` deve ser o primeiro valor do seu método `setAttribute()`.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

Seu método `setAttribute` deve ter `song?.title ? `Play ${song.title}` : "Play"` como o segundo argumento.
