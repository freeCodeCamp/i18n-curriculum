...
Você deve adicionar um `repeating-linear-gradient` a `.fb6` na propriedade `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

Você deve dar ao `repeating-linear-gradient` uma direção de `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

Você deve dar ao `repeating-linear-gradient` uma cor de inicial de `--building-color3` de `0%` a `10%`.
...
Você deve dar ao `repeating-linear-gradient` uma segunda cor de `transparent` de `10%` a `30%`.
