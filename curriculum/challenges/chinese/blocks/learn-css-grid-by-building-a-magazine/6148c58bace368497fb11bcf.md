...
与 Flexbox 类似，使用 CSS Grid 你可以通过 `align-items` 和 `justify-items` 对网格项的内容进行对齐。`align-items` 会沿列轴对齐子元素，`justify-items` 会沿行轴对齐子元素。

给 `.social-icons` 选择器一个 `align-items` 属性，设置为 `center`。

# --hints--

你的 `.social-icons` 选择器应该有一个 `align-items` 属性，设置为 `center`。
