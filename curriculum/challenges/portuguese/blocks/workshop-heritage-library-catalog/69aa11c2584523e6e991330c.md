...
O método `.trim()` remove espaços em branco das duas extremidades de uma string. Por exemplo, `" hello ".trim()` retorna `"hello"`.

Dentro do corpo do loop, insira `parts[i].trim()` em `trimmedParts`. Depois, atualize a declaração `return` existente para retornar `trimmedParts` em vez de `parts`.

Após este passo, a saída registrada deve mostrar valores limpos sem espaços extras.

# --hints--

Você deve inserir `parts[i].trim()` em `trimmedParts` dentro do loop.

```js
assert.match(parseCard.toString(), /trimmedParts\s*\.\s*push\s*\(\s*parts\s*\[\s*\w+\s*\]\s*\.\s*trim\s*\(\s*\)\s*\)/);
```

`parseCard` deve retornar `trimmedParts`.
...
`parseCard` deve retornar um array onde cada elemento não tenha espaços em branco no início ou no fim.

```js
assert.isArray(parseCard("  A  |  B  |  C  |  D  "));
```

Quando chamado com `"  A  |  B  |  C  |  D  "`, o primeiro elemento do array retornado deve ser `"A"`.
