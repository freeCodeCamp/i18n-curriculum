...
你需要調整正在計算的份數。這是使用另一個柯里化函式的絕佳機會。

宣告一個 `adjustForServings` 函式（程式），它接受一個 `baseQuantity` 引數。它應該傳回第二個接受 `newServings` 引數的函式（程式）。

內部函式（程式）應該傳回將 `baseQuantity` 和 `newServings` 相乘的結果。

# --hints--

你應該宣告一個 `adjustForServings` 函式（程式）。

```js
assert.isFunction(adjustForServings);
```

你的 `adjustForServings` 函式應該接受一個 `baseQuantity` 引數。

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

你的 `adjustForServings` 函式應該傳回一個函式。

```js
assert.isFunction(adjustForServings?.());
```

你的內部函式（程式）應該有一個 `newServings` 參數。

```js
assert.match(adjustForServings?.()?.toString(), /\(newServings\)/);
```

你的內部函式應該傳回 `baseQuantity` 和 `newServings` 相乘的結果。
