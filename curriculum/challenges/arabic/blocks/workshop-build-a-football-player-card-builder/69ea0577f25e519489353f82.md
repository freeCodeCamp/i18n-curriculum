...
في كتلة `catch`، سجّل السلسلة النصية `Failed to load player data, using defaults:` متبوعة بمعلمة `error` في وحدة التحكم.

ثم خارج كتلة `catch` ولكن لا تزال داخل دالة `loadPlayer`، أرجع `defaultPlayer`.

كل هذه الخطوات ستُحمّل بيانات اللاعب، وتملأها في واجهة المستخدم، وتُحمّل كائن `defaultPlayer` إذا فشل العملية.

# --hints--

يجب أن تسجل كتلة `catch` الخاصة بك `'Failed to load player data, using defaults:', error` في وحدة التحكم.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

يجب أن تُرجع دالة `loadPlayer` قيمة `defaultPlayer` كنظام بديل.
