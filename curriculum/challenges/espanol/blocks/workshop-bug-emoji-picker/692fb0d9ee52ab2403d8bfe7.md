...
A continuación, crea un constructor para la clase `Bug` que reciba un único parámetro llamado `emojiElement` de tipo `HTMLParagraphElement`. Dentro del constructor, asigna el parámetro `emojiElement` a la propiedad `emojiElement` de la clase.

# --hints--

Tu clase `Bug` debe tener un constructor.
...
Tu constructor `Bug` debe tener un parámetro llamado `emojiElement` de tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Tu constructor `Bug` debe asignar `emojiElement` a `this.emojiElement`.
