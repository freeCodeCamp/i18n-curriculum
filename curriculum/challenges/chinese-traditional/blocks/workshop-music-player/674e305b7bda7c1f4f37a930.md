...
在你開始實作播放下一首和上一首歌曲之前，你需要取得 `userData` 的 `songs` 屬性中每首歌曲的索引。為此，你可以使用 `indexOf()` 陣列方法，該方法會傳回陣列中第一個可以找到指定元素的索引，如果元素不存在則傳回 `-1`。
...
創建一個名為 `getCurrentSongIndex` 的函式（程式），該函式（程式）傳回 `userData.currentSong` 在 `userData.songs` 中的位置索引，當目前歌曲為 `null` 時傳回 `-1`。

# --hints--

你應該有一個名為 `getCurrentSongIndex` 的函式（程式）。

```js
assert.isFunction(getCurrentSongIndex);
```

你的 `getCurrentSongIndex` 函式應該傳回 `userData.currentSong` 在 `userData.songs` 中可以找到的索引。
...
當 `userData.currentSong` 是 `null` 時，你的 `getCurrentSongIndex` 函式應該傳回 `-1`。
