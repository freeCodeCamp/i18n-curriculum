...
يجب أن يكون لديك منتقي `h1`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1'));
```

يجب أن يحتوي منتقيك `h1` على خاصية `max-width` بقيمة `37.25rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1')?.getPropertyValue('max-width') === '37.25rem');
```

يجب أن يحتوي منتقيك `h1` على خاصية `margin` بقيمة `0 auto`.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1')?.getPropertyValue('margin') === '0px auto');
```

يجب أن يحتوي منتقيك `h1` على خاصية `padding` بقيمة `1.5rem 1.25rem`.
