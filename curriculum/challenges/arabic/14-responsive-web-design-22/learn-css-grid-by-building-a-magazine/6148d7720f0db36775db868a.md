...
يجب أن يحتوي منتقي `.text-with-images` على خاصية `grid-template-columns` بقيمة `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

يجب أن يكون لمنتقي `.text-with-images` الخاص بك الخاصية `column-gap` بقيمة `3rem`.
