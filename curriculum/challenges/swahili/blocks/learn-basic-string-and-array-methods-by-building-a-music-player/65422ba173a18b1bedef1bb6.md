...
Ndani ya kitendakazi cha mshale cha msikilizaji wa tukio, ongeza `if` ili kukagua kama `userData?.currentSong` ni `null`.

Ndani ya blokki ya `if`, fanya wito wa kitendakazi cha `playSong()` kwa `id` ya wimbo wa kwanza katika safu ya data ya `userData?.songs`. Hii itahakikisha wimbo wa kwanza katika orodha ya nyimbo unachezwa kwanza.

# --hints--

Unapaswa kuunda tamko la `if` lenye sharti la `userData?.currentSong === null`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{/);
```

Unapaswa kufanya wito wa kitendakazi cha `playSong` kwa `userData?.songs[0].id` ndani ya blokki yako ya `if`.
