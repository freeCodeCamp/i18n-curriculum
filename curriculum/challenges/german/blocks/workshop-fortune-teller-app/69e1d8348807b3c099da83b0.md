...
Sie sollten Ihren bisherigen Fortschritt testen. Erstellen Sie eine Instanz der `Game`-Klasse und weisen Sie sie einer `game`-Variablen zu.

An diesem Punkt werden Sie sehen, dass Sie die Karten sehen können, wenn Sie auf die Schaltflächen Single Card Reading und Three Card Spread klicken.

# --hints--

Sie sollten eine Variable mit dem Namen `game` deklarieren.
...
Ihre `game`-Variable sollte mit `new Game()` initialisiert werden.
