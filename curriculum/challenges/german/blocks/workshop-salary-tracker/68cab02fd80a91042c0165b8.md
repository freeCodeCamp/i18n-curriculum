...
Nach Ihrer bestehenden `if`-Anweisung erstellen Sie eine weitere für den Fall, dass das neue Gehalt niedriger ist als das Grundgehalt für die aktuelle Stufe. Verwenden Sie `hasattr(self, '_level')`, um eine `AttributeError` während der Initialisierung zu vermeiden.

Innerhalb der `if`-Anweisung lösen Sie eine `ValueError` mit der Nachricht `Salary must be higher than minimum salary $` aus, gefolgt vom Grundgehalt für die aktuelle Stufe und einem Punkt.

# --hints--

Sie sollten eine zweite `if`-Anweisung in Ihrem `salary`-Setter haben.
...
Wenn das neue Gehalt niedriger ist als das Grundgehalt für die aktuelle Stufe, sollten Sie eine `ValueError` mit der Nachricht `Salary must be higher than minimum salary $` auslösen, gefolgt vom Grundgehalt für die aktuelle Stufe und einem Punkt.
...
Sie sollten keine Ausnahme auslösen, wenn `new_salary` höher ist als das aktuelle Gehalt.
