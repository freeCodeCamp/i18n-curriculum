...
Los docstrings pueden ser accedidos a través del atributo `__doc__`, que está configurado en `None` por defecto.

Verifícalo imprimiendo `movie1.__doc__` y `series1.__doc__`. Ten en cuenta que el atributo `__doc__` no es heredado por ninguna clase hija.

# --hints--

Debes imprimir `movie1.__doc__`.
...
Debes imprimir `series1.__doc__`.
