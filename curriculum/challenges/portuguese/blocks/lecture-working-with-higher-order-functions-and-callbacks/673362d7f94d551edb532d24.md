...
Isso acontece porque o método `sort` converte os elementos em strings e depois compara suas sequências de valores de unidades de código UTF-16.
...
## Lidando com valores `undefined` e espaços vazios

Também é importante entender como o método `sort` lida com valores `undefined` e espaços vazios em um array.

Quando o método `sort` encontra um valor `undefined`, ele não passa esse valor para a função de comparação. Em vez disso, os valores `undefined` são automaticamente movidos para o final do array.

Aqui está um exemplo que mostra como o `sort` lida com um array que contém um valor `undefined`:
...
Note que o `undefined` é colocado no final do array ordenado, depois de todos os outros elementos.

Da mesma forma, quando o método `sort` encontra um espaço vazio em um array esparso, ele preserva esse espaço vazio e o move para o final do array. Se houver valores `undefined` e espaços vazios, os valores `undefined` virão primeiro, seguidos pelos espaços vazios no final.

Aqui está um exemplo que demonstra esse comportamento:
...
Neste exemplo, as strings `"apple"` e `"banana"` são ordenadas alfabeticamente primeiro. Depois, o valor `undefined` é colocado após elas. Por fim, o espaço vazio é preservado no final do array.
