...
Una citazione non è davvero una citazione senza le virgolette corrette. Puoi aggiungerle con i pseudo selettori CSS.

Crea un selettore `.quote::before` e imposta la proprietà `content` su `"` con uno spazio dopo.

Inoltre, crea un selettore `.quote::after` e imposta la proprietà `content` su `"` con uno spazio prima.

# --hints--

Dovresti avere un selettore `.quote::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

Il tuo selettore `.quote::before` dovrebbe avere una proprietà `content` impostata su `'" '`.
...
Il tuo selettore `.quote::after` dovrebbe avere una proprietà `content` impostata su `' "'`.
