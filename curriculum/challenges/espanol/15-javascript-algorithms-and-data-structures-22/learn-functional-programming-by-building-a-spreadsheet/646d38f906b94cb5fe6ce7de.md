...
Tu `addCharacters(char1)` también está devolviendo una función, que devuelve otra función. Necesitas hacer otra llamada de función para acceder a esa referencia de función interna para la callback de `.map()`. JavaScript te permite invocar inmediatamente funciones retornadas:

```js
myFunc(1)("hi");
```

Inmediatamente invoca la función retornada de tu llamada a `addCharacters(char1)`, y pasa `char2` como argumento.

# --hints--

Deberías encadenar una llamada de función a tu llamada a `addCharacters(char1)`.
...
Deberías pasar `char2` como argumento a tu llamada de función encadenada.
