...
أخيرًا، تحتاج إلى إرجاع قيمة `mean`.

# --hints--

يجب أن تستخدم دالة `getMean` الكلمة المفتاحية `return`.
...
يجب أن تُرجع دالة `getMean` قيمة `mean`.
...
يجب أن يكون بيان `return` بعد إعلان المتغير `mean`.
