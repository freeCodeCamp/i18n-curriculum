...
Проблема зараз у тому, що після оновлення попереднього перегляду або перезавантаження сторінки все, що ви додали для картки гравця, зникає.

Збереження всіх введених даних у `localStorage` зупинить це. Щоб почати, створіть константу `STORAGE_KEY` з рядком `football_player_card` як значенням.

# --hints--

Ви повинні оголосити змінну `STORAGE_KEY`.
...
Ваша `STORAGE_KEY` має мати рядок `football_player_card` як значення.
