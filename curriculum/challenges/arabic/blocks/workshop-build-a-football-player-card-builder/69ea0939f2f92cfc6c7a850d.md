...
بعد ذلك، أضف خطاف `useEffect` يعمل كلما تغيرت حالة `player`. بداخله، استخدم دالة سهمية وأضف كتلة `try..catch` مع `error` كمعلمة لكتلة الـ `catch`.

# --hints--

يجب استدعاء `useEffect` داخل `FootballPlayerCard`.
...
يجب أن تحتوي تبعيات `useEffect` على `[player]` كمصفوفة تبعيات.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

يجب أن يحتوي استدعاء راجع `useEffect` على كتلة `try...catch` مع `error` كمعلمة لكتلة الـ `catch`.
