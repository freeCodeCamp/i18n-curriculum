...
Нарешті, вам потрібно опрацювати метод `catch`. Спочатку вкладіть інструкцію `fetch`, інструкцію `json` та виклик displayAuthors у блок `try` всередині `initialFetch`. Потім змініть ланцюжковий метод `catch` на блок catch, як показано в прикладі нижче.
...
Якщо ви зробили це правильно, блок `try` міститиме три рядки коду, а блок `catch` — лише один рядок коду. Також `SyntaxError` буде оброблено, і застосунок має працювати нормально.

Похваліть себе! Ви навчилися робити свій асинхронний код більш читабельним, використовуючи оператори `async` та `await`! Тепер ваша сторінка авторів завершена.
...
Ви маєте створити блок `try/catch`

```js
assert.match(code, /try\s*\{[\s\S]*?\}\s*catch\s*\([^)]*\)\s*\{/);
```

`catch` більше не має бути ланцюжковим методом

```js
assert.notMatch(code, /\.catch/);
```

Блок `try` має містити три інструкції, які отримують дані, створюють `json` і потім відображають їх.
...
Сторінка авторів fCC має відтворюватися правильно
