...
Njia ya `find()` inapata kipengele cha kwanza ndani ya safu ya data kinachokidhi masharti yaliyotajwa katika kitendakazi cha mwitiko kilichotolewa. Ikiwa hakuna kipengele kinachokidhi sharti, njia hiyo inarudisha `undefined`.

Katika mfano hapa chini, njia ya `find()` inatumiwa kutafuta nambari ya kwanza kubwa zaidi ya 25:
...
Ndani ya kitendakazi chako cha `playSong`, tumia njia ya `find()` kwenye safu ya data ya `userData.songs` kutafuta wimbo ambao una `id` sawa kabisa na `id` iliyopitishwa kwenye kitendakazi cha `playSong`. Weka wito wa `find` katika kigezo kinachoitwa `song`.

# --hints--

Kitendakazi chako cha `playSong` kinapaswa kuita njia ya `find` kwenye `userData.songs`.
...
Wito wako wa `find` unapaswa kurudisha wimbo ambao `id` yake ni sawa kabisa na `id` iliyopitishwa kwenye kitendakazi cha `playSong`.
...
Unapaswa kuweka wito wako wa `find` katika kigezo kinachoitwa `song`.
