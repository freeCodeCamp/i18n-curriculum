...
在本次研讨会中，你将创建一个 Emoji Reactor 应用。已经为你提供了超文本标记语言公式化和所有的 CSS。

首先创建一个 `main` 元素。

在 `main` 元素内创建一个 `h1` 元素，`class` 为 `title`，`text` 为 `How are you feeling today?`。
...
`h1` 元素应包含 `How are you feeling today?` 文本。
