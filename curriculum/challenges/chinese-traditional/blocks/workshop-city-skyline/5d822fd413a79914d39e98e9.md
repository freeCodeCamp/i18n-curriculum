...
你應該優化你的代碼。 將 `position` 和 `top` 屬性和值從 `.foreground-buildings` 移動到 `.background-buildings`。 然後在此處同時選擇 `.background-buildings` 和 `.foreground-buildings`，有效地將這些樣式應用於這兩個元素。 你可以使用逗號（`,`）分隔選擇器，例如：`selector1, selector2`。
