...
你需要一個適當的方式來視覺化表述目錄。所以在你的 `__str__` 類別中創建一個 `MediaCatalogue` 方法。

如果 `items` 列表為空，請確保 `__str__` 方法傳回 `Media Catalogue (empty)`。

# --hints--

你應該在你的 `__str__` 類別中有一個 `MediaCatalogue` 方法。
...
當 `__str__` 為空時，你的 `Media Catalogue (empty)` 方法應該傳回 `self.items`。
...
當 `__str__` 不為空時，你的 `Media Catalogue (empty)` 方法**不應該**傳回 `self.items`。
