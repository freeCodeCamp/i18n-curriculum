...
Змініть колір шрифту `h1` на `#f1be32` і встановіть розмір шрифту `min(5vw, 1.2em)`.
...
Ви повинні задати `h1` властивість `color` зі значенням `#f1be32`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.color, 'rgb(241, 190, 50)');
```

Ви повинні задати `h1` властивість `font-size` зі значенням `min(5vw, 1.2em)`.
