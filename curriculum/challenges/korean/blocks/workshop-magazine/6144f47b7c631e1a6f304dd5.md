...
잡지에는 세 개의 주요 섹션이 있습니다. 이미 `main` 규칙에서 전체 레이아웃을 설정했지만, 자식 규칙에서 배치를 조정할 수 있습니다.

한 가지 방법은 `grid-column` 속성으로, `grid-column-start`와 `grid-column-end`의 축약형입니다. `grid-column` 속성은 그리드 항목이 어느 그리드 선에서 시작하고 끝날지 알려줍니다.

`.heading` 규칙을 만들고 `grid-column` 속성을 `2 / 3`로 설정하세요. 그러면 `.heading` 요소가 그리드 선 2에서 시작해 그리드 선 3에서 끝나도록 지정합니다.

# --hints--

`.heading` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.heading'));
```

`.heading` 선택자에는 `grid-column` 속성이 `2 / 3`로 설정되어야 합니다.
