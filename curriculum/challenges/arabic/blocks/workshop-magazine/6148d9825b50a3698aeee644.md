...
أنشئ محدِّد `.lists` واضبط خاصية `list-style-type` على `none`. هذا سيزيل نقاط التعداد من عناصر القائمة.

# --hints--

يجب أن يكون لديك محدِّد `.lists`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.lists'));
```

يجب أن يحتوي محدِّد `.lists` على خاصية `list-style-type` مضبوطة على `none`.
