...
Функція `showAnimation()` має містити виклик до функції `setTimeout`.

```js
assert.match(code, /setTimeout\(/);
```

Передайте функцію зворотного виклику як перший аргумент до функції `setTimeout`.
...
Передайте `1000` як другий аргумент до функції `setTimeout`.
