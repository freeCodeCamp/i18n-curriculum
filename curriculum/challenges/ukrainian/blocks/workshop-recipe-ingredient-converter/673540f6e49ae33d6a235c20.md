...
Вам потрібно зберегти список одиниць, які підтримує ваш застосунок.

Оголосіть масив `units` і додайте до нього рядкові значення `"cup"`, `"gram"`, `"ounce"` та `"teaspoon"`.

# --hints--

Ви повинні оголосити змінну `units`.

```js
assert.isDefined(units);
```

Ваша змінна `units` має бути масивом.

```js
assert.isArray(units);
```

Ваш масив `units` повинен містити чотири елементи.

```js
assert.lengthOf(units, 4);
```

Ваш масив `units` має включати рядок `"cup"`.

```js
assert.include(units, "cup");
```

Ваш масив `units` має включати рядок `"gram"`.

```js
assert.include(units, "gram");
```

Ваш масив `units` має включати рядок `"ounce"`.

```js
assert.include(units, "ounce");
```

Ваш масив `units` має включати рядок `"teaspoon"`.
