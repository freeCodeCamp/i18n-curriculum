...
在西班牙語中，數字 80 稱為 `ochenta`。

這些數字是由使用十位數字的字 `ochenta`（80），接著是連接詞 `y`（"和"），然後是個位數字（1-9）所組成。
...
數字 `ochenta y uno` (81) 在此範圍中是獨特的，因為它在陽性名詞前會變成 `ochenta y un`。例如：

`Ochenta y cuatro años` - 八十四年。
