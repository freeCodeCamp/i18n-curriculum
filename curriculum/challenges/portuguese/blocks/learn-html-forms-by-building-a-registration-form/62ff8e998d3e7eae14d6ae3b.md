...
Adicione um elemento `label`.Dentro do elemento `label` recém-criado, adicione um elemento `input` e defina o atributo `type` como `checkbox`.Torne este elemento `input` obrigatório (`required`), pois os usuários não devem se inscrever sem concordar com os termos e condições.

Adicione os atributos `id` e `for` com o valor `terms-and-conditions` aos elementos para auxiliar na acessibilidade.
