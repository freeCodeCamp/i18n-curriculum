...
通過在 `.back-mountain` 元素後創建一個新的 `div` 元素，並賦予其 `sun` 類，爲背景添加一個太陽以完成背景。

# --hints--

你應該在 `body` 中添加一個新的 `div` 元素。 應該是 --fcc-expected-- `div` 元素，而不是 --fcc-actual--。

```js
assert.lengthOf(document.querySelectorAll('body > div'), 5);
```

你應該將新的 `div` 元素的 `class` 設置爲 `sun`。

```js
assert.include(document.querySelector('div:not(.back-mountain, .left-mountain, .penguin, .ground)')?.className, 'sun');
```

你應該將新的 `div` 元素放在 `.back-mountain` 元素之後。
