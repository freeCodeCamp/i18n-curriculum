...
`userData?.currentSong?.title`와 `userData?.currentSong?.artist` 프로퍼티에 접근하여 각각 `currentTitle`와 `currentArtist` 변수에 할당하세요.

# --hints--

`title` 객체에서 `currentSong`의 `userData`에 접근해야 합니다. 선택적 체이닝을 사용하는 것을 잊지 마세요.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

`title`의 `currentSong`을 `currentTitle` 상수에 할당해야 합니다.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

`artist` 객체에서 `currentSong`의 `userData`에 접근해야 합니다. 선택적 체이닝을 사용하는 것을 잊지 마세요.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

`artist`의 `currentSong`을 `currentArtist` 상수에 할당해야 합니다.
