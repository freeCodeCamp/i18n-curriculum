...
현재 플레이어가 점프할 때 캔버스 높이를 넘어갈 수 있습니다.

이를 고치려면 플레이어가 캔버스 높이 아래로 떨어지지 않도록 멈추는 조건을 추가해야 합니다.

플레이어의 `if` 위치, 높이, 그리고 `y` 속도의 합이 캔버스 높이보다 작거나 같은지 확인하는 빈 `y` 문을 만드세요.

# --hints--

`update()` 메서드에는 `if` 문이 있어야 합니다.
...
`if` 문에 대한 조건은 플레이어의 `y` 위치, 높이, 그리고 `y` 속도의 합이 캔버스 높이보다 작거나 같은지 확인해야 합니다.
