...
`.note` セレクターを作成し、フォントのサイズを `0.6rem` に設定してください。また、上下のマージンを `5px` に設定し、左右のマージンはなくしてください。

# --hints--

新しい `.note` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.note'));
```

`.note` セレクターには `font-size` プロパティが `0.6rem` に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.note')?.fontSize === '0.6rem');
```

`.note` セレクターには `margin` プロパティが `5px 0` に設定されているはずです。
