...
在先前的步驟中，你已經建立了播放歌曲的功能。現在你需要為播放按鈕添加功能，讓它在被點擊時播放目前的歌曲。

使用 `addEventListener()` 方法，並將 `"click"` 事件作為第一個引數傳入，將使用箭頭語法的空回呼函式作為第二個引數，例如 `() => {}`。

# --hints--

你應該在你的 `playButton` 變數上呼叫 `addEventListener()` 方法。
...
你應該使用箭頭語法將空的回呼傳入你的事件監聽器。別忘了你也需要一對空的大括號。
