...
最後，你應該再次呈現歌曲，更新播放按鈕的可存取文字，並從播放列表中移除重設按鈕。你也需要從 DOM 中移除 `resetButton`。

使用 `sortSongs()` 作為引數呼叫 `renderSongs()` 函式（程式），以便依字母順序重新呈現歌曲。

呼叫 `setPlayButtonAccessibleText()` 函式（程式）以更新播放按鈕的可存取文字。

透過對 `resetButton` 變數呼叫 `remove()` 方法，從播放列表中移除重設按鈕。

**注意**：現在你可以嘗試移除所有歌曲以查看會發生什麼。
...
你應該使用 `sortSongs()` 來呼叫 `renderSongs` 函式（程式）。
...
你應該使用 `remove()` 方法將 `resetButton` 從 DOM 中移除。
