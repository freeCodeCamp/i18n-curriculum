...
Об’єкт `Math` у JavaScript містить статичні властивості та методи для математичних констант та функцій. Одною із них є `Math.random()`, яка генерує випадкове число від `0` (включно) до `1` (виключно). Іншим є `Math.floor()`, яке заокруглює задане число до найближчого цілого числа.
...
Дотримуючись цього шаблону, використайте оператор додавання (`+`), щоб додати випадкове число між `1` та значенням `xp` до `monsterHealth -= weapons[currentWeaponIndex].power`.
...
Додайте до значення `weapons[currentWeaponIndex].power`.
...
Додайте результат `Math.floor(Math.random() * xp) + 1` до результату `weapons[currentWeaponIndex].power`.
