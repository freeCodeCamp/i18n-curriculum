...
Як ви бачите, `series1` відображається у тому ж форматі, що й об’єкт `Movie`, тому що він наслідує метод `__str__` свого батьківського елемента. Наступного кроку ви замістите метод `__str__`, щоб надати кастомне представлення рядка.

Поки що видаліть лінію `print(series1)` зі свого коду.

# --hints--

У вашому коді не має бути `print(series1)`
