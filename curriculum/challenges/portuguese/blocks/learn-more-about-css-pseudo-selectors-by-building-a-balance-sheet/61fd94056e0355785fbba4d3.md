...
Within the second `tr`, add a `th` element with the text `Expenses Annual anticipated expenses, such as payroll.`. Coloque esse texto, com exceção da palavra `Expenses`, envolto em um elemento `span` com uma `class` definida como `description`.
