...
`noteEl` 変数に `addEventListener` メソッドを付けてください。最初の引数は `"blur"` イベントで、2番目の引数はアロー関数にしてください。そのアロー関数の本体内で、`const` を使って `newContent` という新しい変数を作成し、`noteEl.innerHTML` の値を代入してください。

`innerHTML` はここでメモのスペースやフォーマットを保持するために使われています。

# --hints--

`noteEl` 変数に `addEventListener` メソッドを付ける必要があります。
