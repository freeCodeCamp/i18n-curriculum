...
既然你已經更了解如何找出奇數和偶數數字列表的中位數，你可以移除之前步驟中所有的測試程式碼。


# --hints--

你不應該在你的程式碼中有 `const testArr1 = [1, 2, 3, 4, 5];`。

```js
assert.notMatch(code, /const testArr1 = \[1, 2, 3, 4, 5\];/);
```

你的程式碼中不應該有 `const testArr2 = [1, 2, 3, 4, 5, 6];`。

```js
assert.notMatch(code, /const testArr2 = \[1, 2, 3, 4, 5, 6\];/);
```

你不應該在你的程式碼中有 `const isEven = testArr2.length % 2 === 0;`。

```js
assert.notMatch(code, /const isEven = testArr2.length % 2 === 0;/);
```

你的程式碼中不應該有 `console.log(isEven);`。

```js
assert.notMatch(code, /console\.log\(isEven\);/);
```

你的程式碼中不應該有 `const oddListMedian = testArr1[Math.floor(testArr1.length / 2)];`。

```js
assert.notMatch(code, /const oddListMedian = testArr1\[Math\.floor\(testArr1\.length \/ 2\)\];/);
```

你的程式碼中不應該有 `console.log(oddListMedian);`。

```js
assert.notMatch(code, /console\.log\(oddListMedian\);/);
```

你的程式碼中不應該有 `const evenListMedian = getMean([testArr2[testArr2.length / 2 - 1], testArr2[testArr2.length / 2]]);`。

```js
assert.notMatch(code, /const evenListMedian = getMean\(\[testArr2\[testArr2\.length \/ 2 - 1\], testArr2\[testArr2\.length \/ 2\]\]\);/);
```

你的程式碼中不應該有 `console.log(evenListMedian);`。
