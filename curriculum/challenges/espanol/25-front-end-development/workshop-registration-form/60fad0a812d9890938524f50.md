...
Para dar a los Campers una idea de qué poner en su biografía, se usa el atributo `placeholder`. El `placeholder` acepta un valor de texto, que se muestra hasta que el usuario comienza a escribir.

Dale al `textarea` un `placeholder` de `I like coding on the beach...`.

# --hints--

Debe dar al `textarea` un atributo `placeholder`.

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

Debe dar al `placeholder` un valor de `I like coding on the beach...`.
