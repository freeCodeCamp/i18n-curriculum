...
Construa um aplicativo JavaScript full stack que seja funcionalmente semelhante a este: <a href="https://sudoku-solver.freecodecamp.rocks/" target="_blank" rel="noopener noreferrer nofollow">https://sudoku-solver.freecodecamp.rocks/</a>. Trabalhar neste projeto envolverá você escrever seu código usando um dos seguintes métodos:

-   Clone <a href="https://github.com/freecodecamp/boilerplate-project-sudoku-solver" target="_blank" rel="noopener noreferrer nofollow">este repositório do GitHub</a> e complete seu projeto localmente.
...
- Toda a lógica do quebra-cabeça pode ir em `/controllers/sudoku-solver.js`
  ...
  - As funções `check` devem validar o estado *atual* do tabuleiro.
  - A função `solve` deve lidar com a resolução de qualquer string de quebra-cabeça válida fornecida, não apenas com as entradas de teste e soluções. Espera-se que você escreva a lógica para resolver isso.
...
Você pode `POST` `/api/solve` com dados de formulário contendo `puzzle` que será uma string contendo uma combinação de números (1-9) e pontos `.` para representar espaços vazios. O objeto retornado conterá uma propriedade `solution` com o quebra-cabeça resolvido.
...
O valor retornado do `POST` para `/api/check` será um objeto contendo uma propriedade `valid`, que é `true` se o número puder ser colocado na coordenada fornecida e `false` se o número não puder. Se for false, o objeto retornado também conterá uma propriedade `conflict`, que é um array contendo as strings `"row"`, `"column"` e/ou `"region"` dependendo do que torna a colocação inválida.
...
Todos os 12 testes de unidade foram concluídos e deram aprovação.
