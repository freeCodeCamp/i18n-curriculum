...
<dfn>Fetch API</dfn> 是一個內置的 JavaScript 接口，用於向服務器發出網絡請求。 它有一個 `fetch()` 方法，你可以使用它來發出 `GET`、`POST`、`PUT` 或 `PATCH` 請求。 在這個項目中，你將向一個 URL 發出 `GET` 請求，以獲取一個包含 freeCodeCamp News 作者信息的 JSON 文件。
