...
배경을 완성하려면 `div` 요소 바로 뒤에 새로운 `.back-mountain` 요소를 만들고, `sun` 클래스를 부여하여 태양을 추가하세요.

# --hints--

새로운 `div` 요소를 `body`에 추가해야 합니다. `--fcc-expected--` `div` 요소가 예상되었으나 `--fcc-actual--`가 발견되었습니다.

```js
assert.equal(document.querySelectorAll('body > div')?.length, 5);
```

새로운 `div` 요소에 `class`를 `sun`로 지정해야 합니다.
