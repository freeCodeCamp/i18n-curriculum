...
為你的 `.bb1d` 添加 `linear-gradient`，第一個顏色為 `orange`，第二個為 `--building-color1`，第三個為 `--window-color1`。記得將漸層用於 `background` 屬性。
...
你應該使用 `--window-color1` 作為 `linear-gradient` 中的第三個顏色。
