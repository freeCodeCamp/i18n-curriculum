...
使用引數 `players.filter()` 呼叫 `setPlayerCards` 函式。

在 `filter` 方法中，為你的回呼函式添加一個名為 `player` 的參數，並隱式傳回 `player.nickname` 不是 `null`。 

# --hints--

在你的 `nickname` 情況下，你應該呼叫 `setPlayerCards` 函式（程式）。
...
你的 `filter()` 回呼應該隱式地傳回檢查 `player.nickname` 是否不為 `null` 的結果。
