...
첫 번째 `p` 요소 안에 `span` 요소를 추가하세요. `span`에 `product-count` 클래스를 주고 `id`에 `product-count-for-id${id}`를 지정하세요. 그런 다음 span 뒤에 `p` 요소에 `name` 변수의 텍스트를 넣으세요.

# --hints--

첫 번째 `p` 요소에는 `span` 요소가 있어야 합니다.
...
`span` 요소에는 `class`가 `product-count`여야 합니다.
...
`span` 요소에는 `id`가 `product-count-for-id${id}`여야 합니다.
...
첫 번째 `p` 요소에는 `name` 변수의 텍스트가 있어야 합니다. 이 텍스트는 span 밖에 있어야 합니다.
