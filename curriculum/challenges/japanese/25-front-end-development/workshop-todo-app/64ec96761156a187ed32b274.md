...
Create two `button` elements with the `type` attribute set to `button` and the `class` attribute set to `btn`. 最初のボタンのテキストを `Edit` に、2 番目のボタンのテキストを `Delete` に設定してください。
