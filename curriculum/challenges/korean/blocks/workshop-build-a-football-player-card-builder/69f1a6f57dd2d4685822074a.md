...
각 `span`에 `className`와 `stat-value`의 `stat-label`을 가진 두 개의 `div.stat-row` 요소를 추가하세요.

# --hints--

각 `.stat-row`에는 `span`의 `className`을 가진 `stat-value`와 `span`의 `className`을 가진 `stat-label`가 포함되어야 합니다.
