...
يجب عليك إضافة عنصرين `hr` إلى عنصر `.image-quote` الخاص بك.

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

يجب عليك إضافة عنصرين `p` إلى عنصر `.image-quote` الخاص بك.

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

يجب أن يكون العناصر الأطفال في `.image-quote` بالترتيب الصحيح.
...
عنصر `p` الجديد الخاص بك يجب أن يحتوي على النص `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
