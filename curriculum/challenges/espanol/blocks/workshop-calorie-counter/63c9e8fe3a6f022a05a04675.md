...
Usando la misma sintaxis de interpolación, añade un segundo elemento `p` con el texto `consumedCalories Calories Consumed` y un tercero con el texto `exerciseCalories Calories Burned`. Recuerda reemplazar tus variables `consumedCalories` y `exerciseCalories` por los valores apropiados.
...
Tu segundo elemento `p` debe tener el texto `${consumedCalories} Calories Consumed`.
...
Tu tercer elemento `p` debe tener el texto `${exerciseCalories} Calories Burned`.
