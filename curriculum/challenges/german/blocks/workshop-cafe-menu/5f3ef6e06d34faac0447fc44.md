...
Setzen Sie den `Est. 2020` kursiv, indem Sie einen `established`-Klassen-Selektor erstellen und ihm eine `font-style`-Eigenschaft mit dem Wert `italic` geben.

# --hints--

Sie sollten einen `.established`-Selektor haben.
...
Sie sollten die `font-style`-Eigenschaft auf `italic` setzen.
...
Ihr `.established`-Selektor sollte die `font-style`-Eigenschaft auf `italic` setzen.
