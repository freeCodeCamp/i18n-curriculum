...
1. No seu seletor `:root`, você deve definir seis variáveis CSS nomeadas `--color#`, onde `#` é um número de `0` a `5`, e atribuir a cada uma um valor de cor. Use essas variáveis para definir a cor de fundo dos elementos correspondentes `.available-#`.
1. No seu seletor `:root`, você deve definir variáveis CSS chamadas `--solid-border` e `--dashed-border`. Use essas variáveis para estilizar as bordas inferiores das suas células de dados, alternando entre bordas sólidas e tracejadas dependendo da linha. Dê às linhas a classe `sharp` ou `half` para estilizar elas.
1. Você deve ter um elemento `div` com o `id` `legend`. Ele deve conter um elemento `span` com o texto `Disponibilidade` e um elemento `div` com o `id` `legend-gradient`.
1. Você deve aplicar ao elemento `#legend-gradient` um gradiente linear que transite entre todas as cores de `--color0` a `--color5`. Cada valor de cor deve ter dois pontos de parada (expressos em porcentagens) para que a transição entre as cores seja uma linha dura.
