...
أنشئ بند `elif` للحالة `n == 1`. داخل بند `elif`، أنشئ سلسلة نصية f تحتوي على المعامل متبوعًا مباشرة بـ `x` بأحرف صغيرة وأضفها إلى قائمة `terms`.

# --hints--

يجب أن تنشئ بند `elif` للتحقق مما إذا كانت قيمة `n` تساوي `1`.
...
يجب أن تضيف `f'{coefficient}x'` إلى قائمة `terms` داخل بند `elif` الجديد.
