...
두 번째 `fieldset`에서 `input`과 `label` 텍스트가 같은 줄에 나타나도록 하세요.

먼저, 두 번째 `input` 안의 `fieldset` 요소에 `inline` 클래스를 부여하세요.

# --hints--

첫 번째 `input`에 `inline` 클래스를 부여해야 합니다.

```js
assert(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

두 번째 `input`에 `inline` 클래스를 부여해야 합니다.
