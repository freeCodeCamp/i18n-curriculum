...
Loggen Sie im `catch`-Block den String `Failed to load player data, using defaults:` gefolgt vom `error`-Parameter in die Konsole.

Geben Sie dann außerhalb des `catch`, aber noch innerhalb der `loadPlayer`-Funktion `defaultPlayer` zurück.

All dies würde die Spieldaten laden, sie in der Benutzeroberfläche anzeigen und das `defaultPlayer`-Objekt laden, falls der Vorgang fehlschlägt.

# --hints--

Ihr `catch`-Block sollte `'Failed to load player data, using defaults:', error` in der Konsole ausgeben.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

Ihre `loadPlayer`-Funktion sollte `defaultPlayer` als Fallback-Wert zurückgeben.
