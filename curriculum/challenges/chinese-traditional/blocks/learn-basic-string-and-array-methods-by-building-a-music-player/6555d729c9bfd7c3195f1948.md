...
`textContent` 設定節點的文字，並允許你設定或取得 HTML 元素的文字內容。
...
你可以使用三元運算子有條件地設值文字內容。以下是將三元運算子結果指定給變數的範例：

```js
const example = condition ? "I'm true" : "I'm false";
```

使用三元運算子來檢查 `currentTitle` 是否評估為真值。如果是，將 `playingSong.textContent` 設定為 `currentTitle`。否則，將其設定為空字串。

然後在下面，使用 `ternary` 運算子來檢查 `currentArtist` 是否為真值。如果是，將 `songArtist.textContent` 設為 `currentArtist`。否則，將它設為空字串。 

# --hints--

你應該將 `textContent` 屬性串鏈到 `playingSong`。

```js
assert.match(code, /playingSong\.textContent\s*/)
```

你應該使用 `ternary` 運算子來將 `playingSong` 的 `textContent` 屬性設定為 `currentTitle` 或 `""`。

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

你應該將 `textContent` 屬性串鏈到 `songArtist`。

```js
assert.match(code, /songArtist\.textContent\s*/)
```

你應該使用 `ternary` 運算子將 `songArtist` 的 `textContent` 屬性設為 `currentArtist` 或 `""`。
