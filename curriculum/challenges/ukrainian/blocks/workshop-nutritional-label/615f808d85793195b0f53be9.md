...
Створіть ще один елемент `p` після останнього `.divider` та надайте йому текст `Total Sugars 12g`. Призначте цьому елементу `p` значення `class`, які необхідні для того, щоб зсунути його та видалити нижній кордон. Потім створіть ще один `.divider` під новим елементом `p`.
...
Ваш новий елемент `p` повинен мати текст `Total Sugars 12g`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Total Sugars[\s|\n]+12g/));
```

Новий елемент `p` повинен мати атрибут `class` зі значенням `indent no-divider`.
