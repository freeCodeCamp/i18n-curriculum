...
링크가 실제로 방문된 경우에 링크의 속성을 변경하려면 `a:visited { propertyName: propertyValue; }`처럼 생긴 <dfn>의사선택자</dfn>를 사용하세요.

사용자가 방문한 경우 푸터 `Visit our website` 링크의 색상을 `grey`로 변경하세요.

# --hints--

`a:visited` 의사선택자를 사용하세요.
...
`a:visited`에 `color`가 `grey`인 것이 있어야 합니다.
