...
Спочатку ви знайдете позицію конкретних воріт у масиві `gates`.

Використайте метод `indexOf()` на `gates`, щоб знайти елемент `currentGate` і призначити результат змінній з ім’ям `currentIndex`.

Ось нагадування, як це можна зробити:
...
Ви маєте використати `indexOf()` на масиві `gates`, щоб отримати індекс `currentGate` і призначити його змінній з ім’ям `currentIndex`. Це слід зробити в одному рядку.
