...
將右眼移動到位，其 `position` 屬性爲 `absolute`，`top` 爲 `54px`，`left` 爲 `134px`。

# --hints--

你的 `.cat-right-eye` 選擇器應該設置一個 `position` 屬性爲 `absolute`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.position === 'absolute')
```

你的 `.cat-right-eye` 選擇器應該設置一個 `top` 屬性爲 `54px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.top === '54px')
```

你的 `.cat-right-eye` 選擇器應該設置一個 `left` 屬性爲 `134px`。
