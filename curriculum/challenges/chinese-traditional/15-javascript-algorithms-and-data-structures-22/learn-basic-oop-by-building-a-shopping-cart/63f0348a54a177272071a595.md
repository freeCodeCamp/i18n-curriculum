...
現在將三元表達式的結果分配給 `showHideCartSpan` 變量的 `textContent`，該表達式檢查 `isCartShowing` 是否爲真。 如果是，則將 `textContent` 設置爲 `"Hide"`，否則將其設置爲 `"Show"`。

# --hints--

你應該對 `showHideCartSpan` 變量的 `textContent` 屬性使用賦值運算符。
...
如果 `isCartShowing` 爲真，則應將 `showHideCartSpan` 變量的 `textContent` 設置爲 `"Hide"`。

```js
assert.match(code, /showHideCartSpan\s*\.\s*textContent\s*=\s*isCartShowing\s*\?\s*('|"|`)Hide\1\s*:\s*/)
```

如果 `isCartShowing` 爲假，則應將 `showHideCartSpan` 變量的 `textContent` 設置爲 `"Show"`。
