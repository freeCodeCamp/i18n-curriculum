...
다음으로, `useEffect` 상태가 변경될 때마다 실행되는 `player` 훅을 추가하세요. 그 안에서 화살표 함수를 사용하고 `try..catch`를 `error` 블록의 매개변수로 하는 `catch`를 추가하세요.

# --hints--

`useEffect` 안에서 `FootballPlayerCard`를 호출해야 합니다.
...
`useEffect`에는 `[player]`를 의존성 배열로 포함해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

`useEffect` 콜백 안에는 `try...catch` 블록의 매개변수로 `error`를 사용하는 `catch` 블록이 있어야 합니다.
