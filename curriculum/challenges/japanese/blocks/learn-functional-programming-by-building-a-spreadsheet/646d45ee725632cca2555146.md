...
今、あなたの `applyFunction` は結果を返す必要があります。`.replace()` の `str2` メソッドを呼び出した結果を返してください。`functionCall` の正規表現と空のコールバックを渡してください。

# --hints--

あなたの `applyFunction` 関数は、`.replace()` の `str2` メソッドを呼び出した結果を返すべきです。
...
`functionCall` の呼び出しに、`.replace()` を最初の引数として渡すべきです。
...
`.replace()` の呼び出しに、空のアロー関数を2番目の引数として渡すべきです。
