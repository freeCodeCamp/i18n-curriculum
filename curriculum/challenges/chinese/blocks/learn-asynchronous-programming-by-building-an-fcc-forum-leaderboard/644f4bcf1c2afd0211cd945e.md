...
每个类别都会有一个 URL 指向 [freeCodeCamp 论坛](https://forum.freecodecamp.org/) 上的该类别。

创建一个名为 `url` 的常量，并为其分配一个模板字面量。 在该模板字面量中，放置 `${forumCategoryUrl}${selectedCategory.className}/${id}` 的值。
...
你应该在模板字面量中使用 `${forumCategoryUrl}${selectedCategory.className}/${id}` 的值。 
