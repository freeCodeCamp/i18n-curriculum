...
`address` 요소의 기본 스타일은 `font-style`이 `italic`로 설정되어 있습니다. `address` 요소에 대한 선택자를 추가하고 `font-style`를 `normal`로 설정하세요.

# --hints--

`address` 요소에 대한 선택자가 있어야 합니다.

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('address'));
```

`address` 선택자는 `font-style` 속성을 설정해야 합니다.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('address').getPropVal('font-style'));
```

`font-style` 속성은 `normal`로 설정되어야 합니다.
