...
在 `card-stats` div 内，紧跟第一个 `stat-col` 后，你需要添加两个子元素。

首先，添加一个自闭合的 `div`，其 `className` 为 `stat-divider`，用于在两个统计列之间渲染一条细竖线；其次，添加第二个 `div`，其 `className` 为 `stat-col`，包含三个 `div` 元素，每个元素的 `className` 为 `stat-row`。

# --hints--

你应该在第一个 `div` 后添加一个自闭合的 `className`，其 `stat-divider` 为 `.stat-col`。

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

你应该在 `div` 后添加第二个 `className`，其 `stat-col` 为 `.stat-divider`。

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

第二个 `.stat-col` 应包含三个 `div` 元素，每个元素的 `className` 为 `stat-row`。
