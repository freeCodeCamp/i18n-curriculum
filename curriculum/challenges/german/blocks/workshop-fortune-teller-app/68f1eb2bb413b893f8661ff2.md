...
An diesem Punkt sollten Sie in der Lage sein, das Schicksal zu sehen, das jede Karte anzeigt, sobald Sie auf eine Karte klicken. Das Einzige, was noch zu tun bleibt, ist, die Schaltfläche für eine neue Legung funktionsfähig zu machen.

Rufen Sie innerhalb der `newReading`-Methode `showElements()` und `hideElements()` auf.

# --hints--

Sie sollten die `showElements`-Funktion innerhalb der `newReading`-Methode aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /newReading\s*\(\s*\)\s*\{[\s\S]*?showElements\s*\(/);
```

Sie sollten die `hideElements`-Funktion innerhalb der `newReading`-Methode aufrufen.
