...
添加另一个布尔表达式，检查玩家的 `position.y` 是否大于或等于检查点的 `position.y`。

在该语句下方，添加另一个布尔表达式，检查玩家的 `position.y` 加上玩家的 `height` 是否小于或等于检查点的 `position.y` 加上检查点的 `height`。

在该语句下方，添加 `isCheckpointCollisionDetectionActive` 变量。

# --hints--

你应该有一个布尔表达式来检查玩家的 `position.y` 是否大于或等于检查点的 `position.y`
...
你应该有一个布尔表达式，检查玩家的 `position.y` 加上玩家的身高是否小于或等于检查点的 `position.y` 加上检查点的高度。
...
你应该将 `isCheckpointCollisionDetectionActive` 添加为 `checkpointDetectionRules` 数组的最后一项。
