...
Jetzt müssen Sie daran arbeiten, das aktuell abgespielte Lied zu pausieren.

Definieren Sie eine `pauseSong`-Funktion, die keine Parameter annimmt. Innerhalb Ihrer neuen Funktion setzen Sie, um die aktuelle Zeit des Lieds beim Pausieren zu speichern, die `songCurrentTime` des `userData`-Objekts auf die `currentTime` der `audio`-Variable.


# --hints--

Sie sollten eine `pauseSong`-Funktion haben.

```js
assert.isFunction(pauseSong);
```

Ihre `pauseSong`-Funktion sollte die `songCurrentTime` des `userData`-Objekts auf die `currentTime` der `audio`-Variable setzen.
