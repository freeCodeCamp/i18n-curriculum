...
Cree un nuevo elemento `div` debajo de su elemento `.medium`. Asígnele un atributo `class` establecido en `daily-value small-text`. Dentro de este nuevo `div`, agregue un elemento `p` con el texto `% Daily Value *`, y configure el atributo `class` a `bold right`.
