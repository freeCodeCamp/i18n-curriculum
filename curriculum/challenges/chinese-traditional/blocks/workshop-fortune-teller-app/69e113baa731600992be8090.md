...
創建一個 `CDN_URL` 變數並將其設為 `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`。同時創建一個 `defaultImg` 變數，值為 `new URL("default.svg", import.meta.url)`，以及一個 `LOCAL_DEFAULT_IMG` 變數，值為 `defaultImg`。
...
你的 `defaultImg` 變數應該設為 `new URL("default.svg", import.meta.url)`。
