...
Use notação de colchetes para acessar um objeto dentro do array `weapons` e para atribuí-lo à variável `newWeapon`. Place the variable `currentWeaponIndex` within the brackets.

Quando você usa uma variável na notação de colchete, você está acessando a propriedade ou índice pelo *valor* dessa variável.
