...
Poi, aggiungi un ulteriore elemento `span` attorno a `Sodium 160mg` per l’allineamento di nuovo.
...
Il `span` attorno a `Sodium` dovrebbe avere l’attributo `class` impostato su `bold`.
