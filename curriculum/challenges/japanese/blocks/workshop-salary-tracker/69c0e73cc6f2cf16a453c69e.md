...
`level` セッターが無効なレベルのバリデーションを処理するようになったので、`if level not in Employee._base_salaries` から `__init__` チェックを削除してください。これは次のステップでセッターが処理します。

# --hints--

`__init__` メソッドには `level not in Employee._base_salaries` チェックを含めないでください。
