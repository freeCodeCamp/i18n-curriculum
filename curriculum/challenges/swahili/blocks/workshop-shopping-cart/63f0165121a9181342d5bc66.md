...
Kwa usemi wako wa kihesabu wa kweli, ukiondoa `undefined`, unahitaji kusahihisha `textContent` ya `currentProductCountSpan` ili iwe `currentProductCount` ikifuatiwa na `x`. Tumia template literal kufanya hivyo.

# --hints--

Unapaswa kuondoa `undefined` kutoka kwenye usemi wako wa kihesabu wa kweli.
...
Unapaswa kupata sifa ya `textContent` ya `currentProductCountSpan`.
...
Unapaswa kutumia sintaksia ya template literal kusahihisha `textContent` ili iwe `${currentProductCount}x`.
