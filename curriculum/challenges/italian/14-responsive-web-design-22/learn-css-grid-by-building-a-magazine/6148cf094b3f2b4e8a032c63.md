...
Crea un selettore `.first-paragraph::first-letter` e imposta la proprietà `font-size` su `6rem`. Dagli anche una proprietà `color` impostata su `orangered` per farlo risaltare.
...
Il selettore `.first-paragraph::first-letter` dovrebbe avere una proprietà `font-size` impostata a `6rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

Il selettore `.first-paragraph::first-letter` dovrebbe avere una proprietà `color` impostata su `orangered`.
