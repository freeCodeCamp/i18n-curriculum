...
Nella tua funzione `inner`, restituisci il risultato della chiamata a `idToText` con `character + num` come argomento.

# --hints--

La tua funzione `inner` dovrebbe usare un return esplicito.
...
La tua funzione `inner` dovrebbe restituire il risultato della chiamata alla tua funzione `idToText`.
...
Dovresti passare `character + num` come argomento alla tua funzione `idToText`.
