...
Створіть елемент `tr` в `thead`. Всередину додайте `td` та три елементи `th`. До кожного елемента `th` додайте елемент `span` з `class` зі значенням `sr-only` і таким текстом: `2019`, `2020` та `2021`.
