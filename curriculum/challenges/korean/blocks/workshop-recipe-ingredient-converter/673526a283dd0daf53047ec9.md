...
이제 첫 번째 커링된 함수를 만들어야 합니다. 커링된 함수는 다른 함수를 반환하는 함수라는 점을 기억하세요. 예를 들어 다음과 같습니다:
...
먼저 `convertQuantity` 함수를 선언하세요. 이 함수는 단일 `fromUnit` 매개변수를 가져야 하며, 함수 본문은 비워 두세요.

# --hints--

`convertQuantity` 함수를 선언해야 합니다.

```js
assert.isFunction(convertQuantity)
```

`convertQuantity` 함수는 `fromUnit` 매개변수를 가져야 합니다.

```js
assert.match(convertQuantity?.toString(), /\(fromUnit\)/);
```

`convertQuantity` 함수는 비워 두어야 합니다.
