...
Hatua inayofuata ni kuzungusha kupitia safu ya data ya `posters` ili kupata picha zao zote za wasifu. 

Anza kwa kuongeza neno kuu la `return` linalofuatiwa na `posters.map()`. Kwa kitendakazi cha mwitiko, ongeza kigezo kinachoitwa `poster`.

# --hints--

Unapaswa kurudisha `posters.map()`.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

Kitendakazi chako cha mwitiko kinapaswa kuwa na kigezo kinachoitwa `poster`.
