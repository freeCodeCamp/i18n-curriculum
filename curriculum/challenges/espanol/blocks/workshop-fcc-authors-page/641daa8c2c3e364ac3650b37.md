...
A continuación, no hay mucha separación entre el nombre y la imagen de cada autor, y el resto de los detalles en la tarjeta. Un divisor proporcionará a las tarjetas de los autores una jerarquía visual clara.

Agrega un elemento `div` encima de la biografía del autor y dale un atributo `class` de `"purple-divider"`.
...
Debes crear un elemento `div` antes de tu elemento `p`.
...
Tu elemento `div` debe tener la `class` configurada a `"purple-divider"`.
