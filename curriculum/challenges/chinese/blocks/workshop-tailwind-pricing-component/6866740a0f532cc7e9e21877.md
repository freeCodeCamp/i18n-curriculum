...
让我们继续讲解 `Most popular` 徽章。你需要将它放置在卡片的右上角，因此你可以使用 Tailwind 的一些定位实用类来实现。

以下是你如何在 Tailwind 中使用 CSS 定位的方法：
...
为你的包装器 `div` 赋值类 `relative`，并为带有文本 `Most popular` 的 `div` 赋值类 `absolute`。

# --hints--

你的高级计划的包装器 `div` 元素应具有类 `relative`。
...
你的 `div` 元素中带有文本 `Most popular` 应该具有类 `absolute`。
