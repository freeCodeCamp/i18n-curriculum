...
لديك خطأ برمجي بسيط. `arg1` و `arg2` هما سلسلتان نصيتان، وليستا أرقامًا. `infixToFunction['+']("1", "2")` سترجع `12`، وهذا غير صحيح رياضيًا.

غلف كل معلمة من معلمات `infixToFunction[operator]` باستدعاء `parseFloat()`.

# --hints--

يجب أن تغلف `arg1` باستدعاء `parseFloat()`.
...
يجب أن تغلف `arg2` باستدعاء `parseFloat()`.
