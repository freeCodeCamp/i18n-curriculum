...
`fieldset` 要素にスタイルを適用して、はっきりしたボーダーをつけましょう。

`fieldset` 要素を選択してください。`border` を `1px solid gray` に、`border-radius` を `5px` に、`margin` を `20px 0` に、`padding` を `20px` に設定してください。

# --hints--

`fieldset` 要素をターゲットにしてください。
...
`margin` プロパティを `20px 0` に設定してください。
