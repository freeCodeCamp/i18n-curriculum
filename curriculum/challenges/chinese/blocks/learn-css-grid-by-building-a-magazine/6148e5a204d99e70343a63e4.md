...
`gap` 属性是同时设置 `column-gap` 和 `row-gap` 值的简写方式。 如果给定一个值，它会将 `column-gap` 和 `row-gap` 都设置为该值。 如果给定两个值，它会将 `row-gap` 设置为第一个值，将 `column-gap` 设置为第二个值。

给 `.image-wrapper` 选择器一个 `gap` 属性，设置为 `2rem`。

# --hints--

你的 `.image-wrapper` 元素应该有一个 `gap` 属性，设置为 `2rem`。
