...
`initializeEventListeners` メソッド内で、`singleCardBtn` に `click` イベントリスナーを追加してください。イベントハンドラ内で、`singleCardSelected` メソッドを呼び出してください。`singleCardBtn` は `elements` から取得することを忘れないでください。
