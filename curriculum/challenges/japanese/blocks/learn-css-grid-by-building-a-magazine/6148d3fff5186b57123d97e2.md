...
引用文は、適切な引用符を付けなければ引用文とは言えません。 引用符は CSS 擬似セレクターを使用して追加できます。

`.quote::before` セレクターを作成し、`content` プロパティを `"` に設定し、その直後にスペース文字を入れてください。

また、`.quote::after` セレクターを作成し、`content` プロパティを `"` に設定し、その直前にスペース文字を入れてください。

# --hints--

`.quote::before` セレクターが必要です。
