...
Nos módulos anteriores, você aprendeu como iterar por um `NodeList` assim:
...
Use o método `forEach` para iterar pelas `tabs`. Para a função de callback, use uma função arrow que recebe um único parâmetro, `tab`. 

# --hints--

Você deve ter um `tabs.forEach(tab => {})` no seu código.
