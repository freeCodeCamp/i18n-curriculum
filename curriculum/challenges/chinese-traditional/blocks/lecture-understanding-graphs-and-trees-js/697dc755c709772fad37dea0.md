...
JavaScript 沒有內建的堆積模組，但你可以使用陣列實作一個最小堆積。

這裡是一個基本的 JavaScript `min-heap` 實作。
...
要使用這個堆積，你可以創建一個空的堆積。這將是堆積的底層資料結構：

```js
const myHeap = new MinHeap();
```

要為堆積添加元素，你會呼叫 `push()`。這將自動將元素添加到應該在的位置，以維護堆積屬性：

```js
myHeap.push(9);
```

要取得具有最高優先權（在此情況下為最小值）的元素，你會呼叫 `pop()`：

```js
myHeap.pop();
```

`pushPop()` 將兩個操作行為合併成一次呼叫。

這比起分別依序呼叫它們更高效，尤其是在堆積很大時，因為它只執行一次重新排序的操作行為：

```js
myHeap.pushPop(15);
```

如果你已經有一個陣列並且想將它轉換成堆積，你可以呼叫 `heapify()`：
...
你可以透過儲存具有此結構的陣列來做到這一點：`[priority, element]`。

在 JavaScript 中，陣列不會自動逐元素比較以排序，所以你需要將一個先比較優先權的比較函式傳遞給堆積。

請注意，在此情況下，較低的數值將表述較高的優先權。這表示優先權為 1 的條款將比優先權為 3 的條款具有**較高**的優先權：
...
如果你需要以插入的順序移除具有相同優先權的元素，你可以考慮將唯一計數器作為你的項目的第二個元素來打破平手，像這樣 `[priority, counter, element]`。

例如，你可以先依照優先權比較，然後再依照計數器比較：
...
堆積的空間複雜度是線性的，O(n)，其中 `n` 是它所包含的元素數量。它只需要儲存元素和陣列物件本身的一點額外開銷。
