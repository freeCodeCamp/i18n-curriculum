...
أسفل المتغيرين المفككين `value` و `checked`، حدّث قائمة القوى المختارة.

استدعِ الدالة `setPowers` التي تأخذ معلمة مصفوفة. استخدم معاملًا ثلاثيًا للتحقق مما إذا كانت `checked` صحيحة. إذا كانت كذلك، وزّع مصفوفة `powers` الحالية في مصفوفة جديدة وأضف إليها `value`. وإذا لم تكن صحيحة، استبعد `value` من `powers` باستخدام `powers.filter(p => p !== value)`.

# --hints--

يجب أن تستخدم معاملًا ثلاثيًا للتحقق مما إذا كانت `checked` صحيحة. إذا كانت كذلك، وزّع `powers` الحالية و `value` في مصفوفة. وإذا لم تكن، استبعد `value` من `powers`.
