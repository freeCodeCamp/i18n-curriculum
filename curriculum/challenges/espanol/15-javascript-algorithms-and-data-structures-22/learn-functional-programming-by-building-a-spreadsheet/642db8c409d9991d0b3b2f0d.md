...
Start by setting the `onload` property of `window` to an arrow function with no parameters. En la función, declara una variable `container` y asignale el valor de obtener el elemento por el `id` de `"container"`.
