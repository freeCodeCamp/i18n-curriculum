...
Тепер потрібно створити внутрішнє коло, яке з’явиться, коли радіокнопка буде вибрана. Для цього ви використаєте псевдоелемент `::before`.

Псевдоелемент, як `::before`, дозволяє вставити додатковий вміст перед самим елементом. Це часто використовується для декоративних цілей.

Звертаючись до псевдоелемента `.contact-method-radio-btn::before`, встановіть властивість `display` зі значенням `block`, властивість `content` зі значенням `" "`, властивість `width` зі значенням `10px`, властивість `height` зі значенням `10px` та властивість `border-radius` зі значенням `50%`.

# --hints--

Ви повинні мати селектор `.contact-method-radio-btn::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

Встановіть властивість `display` у значення `block`.
...
Встановіть властивість `content` у значення `" "`.
...
Встановіть властивість `width` у значення `10px`.
...
Встановіть властивість `height` у значення `10px`.
...
Встановіть властивість `border-radius` у значення `50%`.
