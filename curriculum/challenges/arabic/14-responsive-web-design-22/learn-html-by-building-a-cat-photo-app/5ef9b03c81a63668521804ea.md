...
Your `head` element should have an opening tag. Opening tags تكتب هكذا: `<elementName>`.

```js
assert(code.match(/\<head\>/));
```

Your `head` element should have a closing tag. يجب أن تأتي علامات الغلق بـ `/` بعد رمز `<` مباشرة.
