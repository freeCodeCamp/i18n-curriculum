...
為你的 `MediaError` 類別添加一個帶有以下參數的 `__init__` 方法：`self`、`message` 和 `obj`。

在其中，呼叫 `super().__init__(message)` 以確保已存在於 `Exception` 類別中的儲存和顯示錯誤訊息的機制能正常運作。

之後，將 `obj` 指定給 `self.obj`。

# --hints--

你的 `MediaError` 類別應該有一個帶有 `self`、`message` 和 `obj` 參數的 `__init__` 方法。
...
你應該在你的 `__init__` 方法中呼叫 `super().__init__(message)`。
...
你應該在你的 `__init__` 方法中將 `obj` 指定給 `self.obj`。
