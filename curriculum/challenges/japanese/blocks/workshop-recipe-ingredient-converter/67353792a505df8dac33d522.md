...
最も内側のカリー化された関数が値を変換するロジックを持ちます。

まず、`fromUnit` の引数を使って `conversionTable` オブジェクトにアクセスし、次に `toUnit` の引数を使ってネストされたオブジェクトにアクセスしてください。

その変換値に `quantity` の値を乗算した結果を返してください。

# --hints--

関数は `convertQuantity("gram")("cup")(1)` の正しい変換結果を返す必要があります。

```js
assert.strictEqual(convertQuantity("gram")("cup")(1), 1/240)
```

関数は `convertQuantity("teaspoon")("ounce")(6)` の正しい変換結果を返す必要があります。
