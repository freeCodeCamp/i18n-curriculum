...
在 `else if` 语句中，添加 `forEach` 循环来遍历 `checkpoints` 数组。 使用 `checkpoint` 作为回调函数的参数名。

在 `forEach` 回调中，使用加法赋值运算符在 checkpoints 数组的 `x` 位置加 5。
...
你应该在 `forEach` 回调函数中使用 `checkpoint` 参数。
...
你应该使用加法赋值运算符给 `checkpoint` 的 `x` 位置添加 5。
