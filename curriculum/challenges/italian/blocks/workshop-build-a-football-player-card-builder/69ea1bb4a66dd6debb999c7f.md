...
Ora gestirai la selezione della posizione del giocatore. Devi rifattorizzare l'input in un select, quindi mappare un array di posizioni calcistiche valide come valore delle opzioni del select.

Per cominciare, crea un array `POSITIONS` con i valori stringa `GK`, `CB`, `LB`, `RB`, `CDM`, `CM`, `CAM`, `LW`, `RW`, `ST` e `CF`. Aggiungi `as const` subito dopo la parentesi quadra di chiusura in modo che TypeScript restringa il tipo da un generico `string[]` a una tupla readonly di quei precisi valori letterali.

Questo significa che TypeScript sa esattamente quali stringhe sono valide, invece di sapere solo che si tratta di un array di stringhe.

# --hints--

Dovresti dichiarare una costante `POSITIONS`.
...
Il tuo array `POSITIONS` dovrebbe contenere tutte le stringhe richieste per le posizioni calcistiche; usa `as const` per restringere il suo tipo a stringhe letterali readonly.
