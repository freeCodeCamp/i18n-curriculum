...
ボタンを作成したので、次はテキストを代入する必要があります。これを行うには、DOMの`createTextNode()`メソッドを使います。

`createTextNode()`メソッドはテキストノードを作成するために使います。使い方は、呼び出して文字列としてテキストを渡します。
...
`createTextNode()`メソッドを使って`"Reset Playlist"`テキストを作成し、それを`resetText`定数に代入してください。
...
`document.createTextNode()`を使って`"Reset Playlist"`テキストを作成してください。

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

作成したテキストノードは`resetText`定数に代入してください。
