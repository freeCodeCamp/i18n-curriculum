...
使用 `forEach` 方法循環遍歷 `playlistSongElements`。

`forEach` 方法用於循環遍歷數組並對數組的每個元素執行函數。 例如，假設你有一個數字數組，並且你想要將每個數字記錄到控制檯。
...
對 `playlistSongElements` 使用 `forEach` 方法。 傳入 `songEl` 作爲參數並使用箭頭語法添加一個空的回調。

# --hints--

你應該將 `forEach()` 鏈接到 `playlistSongElements`。
