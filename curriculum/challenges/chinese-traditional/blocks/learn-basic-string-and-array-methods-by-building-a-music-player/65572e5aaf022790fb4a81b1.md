...
現在，是時候為 `resetButton` 添加重置功能。點擊時，這將恢復播放列表中的歌曲。

為你的 `resetButton` 變數添加一個點擊事件監聽器。使用箭頭語法傳入一個回呼，並暫時保持其為空。

# --hints--

你應該在 `resetButton` 上使用 `addEventListener()` 方法。

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

你的 `resetButton` 監聽器應該監聽 `"click"` 事件。

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

你的事件監聽器的回呼函式（程式）應該使用箭頭語法並且有一對空的大括號。
