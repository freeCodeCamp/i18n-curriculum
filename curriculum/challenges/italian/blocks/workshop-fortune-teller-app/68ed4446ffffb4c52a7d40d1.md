...
All'interno del costruttore, aggiungi le chiamate a `fetchCardsData()` e `initializeEventListeners()` usando la parola chiave `this`, in modo che vengano eseguite quando la classe viene inizializzata.

Il metodo `fetchCardsData` gestirà il caricamento dei dati delle carte dei tarocchi da un'API, e `initializeEventListeners` imposterà i gestori dei clic dei pulsanti. Creerai entrambi i metodi nei passaggi successivi.

# --hints--

Dovresti chiamare il metodo `fetchCardsData()` all'interno di `constructor`.
...
Dovresti chiamare il metodo `initializeEventListeners()` all'interno del costruttore.
