...
Entfernen Sie das `myTaskArr`-Array und den gesamten Code für `localStorage`, da Sie diese nicht mehr benötigen.

# --hints--

Sie sollten `myTaskArr` und den gesamten Code, der mit `localStorage` zusammenhängt, den Sie gerade gelernt haben, entfernen.
...
Sie sollten alle verbleibenden Referenzen auf `myTaskArr` im Code entfernen.

```js
assert.notMatch(code, /myTaskArr/);
```

Sie sollten jede Referenz auf `localStorage.getItem` für das Element `"data"` entfernen.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Sie sollten alle verbleibenden Referenzen auf `getTaskArrObj` im Code entfernen.
