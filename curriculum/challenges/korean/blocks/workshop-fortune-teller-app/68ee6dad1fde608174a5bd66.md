...
`singleCardSelected` 메서드 내에서 `singleCard` 요소를 가져와 `innerHTML` 함수를 호출해 `renderCard`를 업데이트하세요. `"Click the card and reveal the fortune"`를 첫 번째 인수로, `isReversed`를 두 번째 인수로 전달하세요.

# --hints--

`this.elements.singleCard.innerHTML` 함수를 호출해 `renderCard`를 업데이트해야 합니다.
...
`renderCard` 호출 시 `"Click the card and reveal the fortune"`를 첫 번째 인수로 사용하세요.
...
`isReversed`에 두 번째 인수로 `renderCard`를 전달해야 합니다.
