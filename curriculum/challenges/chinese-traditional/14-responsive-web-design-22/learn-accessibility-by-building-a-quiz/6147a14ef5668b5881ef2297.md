...
要使導航按鈕看起來更像典型按鈕，從錨標籤中移除下劃線。

然後，創建一個針對導航列表元素的新選擇器，以便當光標懸停在它們上方時， 背景顏色和文字顏色被切換，光標變成指針。

# --hints--

你應該使用現有的 `li > a` 選擇器將 `text-decoration` 設置爲 `none`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

你應該使用 `nav > ul > li:hover` 或 `nav li:hover` 選擇器來設置懸停元素的樣式。
...
你應該爲懸停的 `li` 元素設置 `background-color` 值爲 `#dfdfe2`。
...
你應該爲懸停的 `li` 元素設置 `color` 值爲 `#1b1b32`。
...
你應該爲懸停的 `li` 元素設置 `cursor` 值爲 `pointer`。
