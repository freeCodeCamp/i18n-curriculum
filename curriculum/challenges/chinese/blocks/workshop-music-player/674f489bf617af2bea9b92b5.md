...
现在，创建一个名为 `getNextSong` 的函数。让它返回在 `userData.songs` 中位于 `getCurrentSongIndex()` 加 1 位置的歌曲，如果当前歌曲是播放列表中的最后一首，则返回 `undefined`。

# --hints--

你应该有一个名为 `getNextSong` 的函数。

```js
assert.isFunction(getNextSong);
```

你的 `getNextSong` 函数应调用 `getCurrentSongIndex`。
...
你的 `getNextSong` 函数应返回当前歌曲的下一首歌曲。
...
当当前歌曲是播放列表中的最后一首时，你的 `getNextSong` 函数应返回 `undefined`。
