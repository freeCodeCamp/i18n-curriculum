...
`if` 문장 밖에서 `number`을 `sum_of_even_digits`에 추가하세요. 또한 `print` 호출을 제거하세요.

# --hints--

`sum_of_even_digits += number` 반복문 안에 `for`가 있어야 합니다.
...
`print(number)` 반복문 안에 `for`가 있으면 안 됩니다.
...
`sum_of_even_digits += number` 문장 안에 `if`가 있으면 안 됩니다.
