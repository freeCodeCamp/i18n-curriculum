...
Далі вам потрібно отримати наступну пісню у плейлисті. Для цього потрібно взяти індекс поточної пісні, а потім додати до нього `1`.

Створіть константу з назвою `nextSong` і призначте їй значення `userData?.songs[currentSongIndex + 1]`.

Нарешті, викличте функцію `playSong` і передайте їй `nextSong.id` як аргумент.

# --hints--

Вам не слід змінювати існуючу інструкцію `if`, її блок `else` та вміст.
...
Ви повинні призначити `userData?.songs[currentSongIndex + 1]` константі `nextSong`.
...
Ви повинні викликати функцію `playSong` з `nextSong.id`.
