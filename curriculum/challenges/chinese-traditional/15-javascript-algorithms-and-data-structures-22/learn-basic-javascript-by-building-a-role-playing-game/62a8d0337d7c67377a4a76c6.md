...
到目前爲止，每次更新 `text.innerText` 時，舊文本都會被刪除。 這次，使用 `+=` 運算符將文本添加到 `text.innerText` 的末尾。

添加字符串 `" In your inventory you have: "` - 包括開頭和結尾的空格。

# --hints--

你應該添加一個帶有 `text.innerText` 的新行。
...
你應該使用 `+=` 運算符將其添加到 `text.innerText`。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*\+=\s*/)
```

你應該將字符串 `" In your inventory you have: "` 添加到 `text.innerText` 的末尾。 注意空格！
