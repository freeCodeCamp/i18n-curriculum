...
- 使用反映清晰组织结构的层次化标题。例如，页面标题应为 `h1`，主要部分应使用 `h2` 标题引入，子部分使用 `h3`，依此类推，直到 `h6`。
- 不要跳过标题层级，比如从 `h1` 跳到 `h3`，或从 `h2` 跳到 `h4`。
- 使用清晰且描述性的文本来总结每个标题后面的内容。
- 不要单独使用标题；标题后必须跟随一些内容。
- 必要时使用合适的标题，而不是仅仅通过格式化文本让它看起来像标题。
- 每个页面应只有一个 `h1` 元素，代表主要主题或标题。
