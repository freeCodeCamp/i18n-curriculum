...
Your `figcaption` element should have a closing tag. 終了タグは `<` の直後に `/` があります。
...
The `figcaption` element nested in the `figure` element should be below the `img` element. 順番に誤りがあるようです。
