...
يجب عليك استخدام منتقي `.foot`.

```js
assert.match(code, /\.foot\s*\{/);
```

يجب أن يحتوي عنصر `.foot` على خاصية `width` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.width, '15%');
```

يجب أن يحتوي عنصر `.foot` على خاصية `height` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.height, '30%');
```

يجب أن يحتوي عنصر `.foot` على خاصية `background-color` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
