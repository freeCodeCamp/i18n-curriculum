...
Weisen Sie nun die `textContent` der `showHideCartSpan`-Variablen das Ergebnis eines ternären Ausdrucks zu, der überprüft, ob `isCartShowing` wahr ist. Wenn ja, setzen Sie die `textContent` auf `"Hide"`, andernfalls setzen Sie sie auf `"Show"`.

# --hints--

Sie sollten den Zuweisungsoperator auf die `textContent`-Eigenschaft der `showHideCartSpan`-Variablen verwenden.
...
Sie sollten die `textContent` der `showHideCartSpan`-Variablen auf `"Hide"` setzen, wenn `isCartShowing` wahr ist.

```js
assert.match(code, /showHideCartSpan\s*\.\s*textContent\s*=\s*isCartShowing\s*\?\s*('|"|`)Hide\1\s*:\s*/)
```

Sie sollten die `textContent` der `showHideCartSpan`-Variablen auf `"Show"` setzen, wenn `isCartShowing` falsch ist.
