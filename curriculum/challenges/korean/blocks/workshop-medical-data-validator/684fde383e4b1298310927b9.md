...
수량자는 정규 표현식에서 문자가 몇 번 반복될 수 있는지 지정하는 데 사용됩니다. 예를 들어, `+` 문자는 이전 문자가 한 번 이상 반복되는 것과 일치합니다:
...
따라서 하나 이상의 숫자와 일치하도록 정규식 패턴에 `+` 수량자를 추가하세요.

# --hints--

현재 정규식 패턴 `+`에 `p\d` 수량자를 추가해야 합니다.
