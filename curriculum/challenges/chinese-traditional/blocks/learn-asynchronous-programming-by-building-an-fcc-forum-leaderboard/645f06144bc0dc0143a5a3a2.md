...
每則論壇貼文都會包含一個用戶頭像影像的列表，該列表表述所有參與該主題對話的使用者。

首先建立一個名為 `avatars` 的箭頭函式（程式），並帶有兩個名為 `posters` 和 `users` 的參數。

# --hints--

你應該有一個名為 `avatars` 的空箭頭函式。

```js
assert.match(code, /const\s+avatars\s*=\s*\([\s\S]*\)\s*=>\s*{\s*}/);
```

你應該在名為 `avatars` 的函式（程式）中擁有名為 `posters` 和 `users` 的參數。
