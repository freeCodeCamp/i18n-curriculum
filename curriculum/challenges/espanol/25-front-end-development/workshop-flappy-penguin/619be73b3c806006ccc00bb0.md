...
Dé al pseudo-elemento un `width` la mitad del de su elemento padre, un `height` de `45%` y un `background-color` de `gray`.

# --hints--

Debe dar a `.penguin-body::before` un `width` de `50%`.
...
Debe dar a `.penguin-body::before` un `background-color` de `--fcc-expected--`, pero se encontró `--fcc-actual--`.
