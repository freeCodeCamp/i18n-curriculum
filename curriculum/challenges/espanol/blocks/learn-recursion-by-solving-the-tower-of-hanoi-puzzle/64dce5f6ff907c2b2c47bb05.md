...
Cuando `target` está vacío, el disco debe moverse necesariamente de `source` a `target`.

Después de la declaración de `forward`, agrega una estructura `if` para verificar si `rods[target]` está vacío. Si lo está, cambia `forward` a `True`.

# --hints--

Debes tener una estructura `if` para verificar si `rods[target]` está vacío, y asignar `True` a `forward`.
