...
接著，你需要在播放器顯示中顯示目前的歌曲標題和藝術家。

第一步，存取負責顯示歌曲標題和歌手的 `#player-song-title` 和 `#player-song-artist` 元素，並分別將它們存入變數 `playingSong` 和 `songArtist`。

# --hints--

你應該有一個名為 `playingSong` 的變數。

```js
assert.isDefined(playingSong);
```

你的 `playingSong` 變數應該具有 `#player-song-title` 元素的值。

```js
assert.equal(playingSong, document.getElementById("player-song-title"));
```

你應該有一個名為 `songArtist` 的變數。
...
你的 `songArtist` 變數應該具有 `#player-song-artist` 元素的值。
