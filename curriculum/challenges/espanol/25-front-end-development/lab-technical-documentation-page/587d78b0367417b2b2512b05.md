...
1. Dentro del elemento `#main-doc`, se pueden ver varios elementos `section`, cada uno con la clase `main-section`. There should be a minimum of five.
...
1. Cada elemento `section` con la clase `main-section` debería tener también un `id` que corresponda al texto de cada `header` contenido dentro de él. Any spaces should be replaced with underscores (e.g. The section that contains the header "JavaScript and Java" should have a corresponding `id="JavaScript_and_Java"`).
...
1. Además, la barra de navegación debe contener elementos de enlace (`a`) con la clase `nav-link`. There should be one for every element with the class `main-section`.
