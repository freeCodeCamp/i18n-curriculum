...
يجب عليك إضافة ثلاث عناصر `div` داخل `.bb4`.

```js
assert.equal(document.querySelector("div.bb4")?.children?.length, 3);
```

يجب عليك إعطاء أول `div` جديد فئة `bb4a`.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

يجب عليك إعطاء ثاني `div` جديد فئة `bb4b`.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

يجب أن تعطي الفئة الثالثة `div` الجديدة `bb4c`.

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

يجب عليك وضع عناصر `div` الجديدة في هذا الترتيب `.bb4a + .bb4b + .bb4c`.
