...
So wird sichergestellt, dass der aktuell abgespielte Song weiterläuft, wenn der Play-Button geklickt wird.
...
Sie sollten Ihrer `else`-Anweisung einen `if`-Block hinzufügen.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{\s*playSong\(\s*userData\?\.songs\s*\[\s*0\s*\]\s*\.id\s*\)\s*;?\s*\}\s*else\s*\{/)
```

Sie sollten die `playSong`-Funktion mit `userData?.currentSong.id` im `else`-Block Ihrer `if`-Anweisung aufrufen.
