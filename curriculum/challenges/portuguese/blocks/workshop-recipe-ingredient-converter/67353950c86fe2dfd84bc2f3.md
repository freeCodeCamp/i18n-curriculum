...
Você precisará ajustar para o número de porções que está sendo calculado. Esta é uma ótima oportunidade para usar outra função curried.

Declare uma função `adjustForServings`, que aceita um argumento `baseQuantity`. Deve retornar uma segunda função que aceita um argumento `newServings`.

A função interna deve retornar o resultado da multiplicação de `baseQuantity` e `newServings`.

# --hints--

Você deve declarar uma função `adjustForServings`.

```js
assert.isFunction(adjustForServings);
```

Sua função `adjustForServings` deve aceitar um argumento `baseQuantity`.

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

Sua função `adjustForServings` deve retornar uma função.

```js
assert.isFunction(adjustForServings?.());
```

Sua função interna deve ter um parâmetro `newServings`.

```js
assert.match(adjustForServings?.()?.toString(), /\(newServings\)/);
```

Sua função interna deve retornar o resultado da multiplicação de `baseQuantity` e `newServings`.
