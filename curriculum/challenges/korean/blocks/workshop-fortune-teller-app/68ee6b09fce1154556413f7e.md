...
`initializeEventListeners` 메서드 안에서 `click`에 `singleCardBtn` 이벤트 핸들러를 붙이세요. 이벤트 핸들러 안에서는 `singleCardSelected` 메서드를 호출하세요. `singleCardBtn`에서 `elements`를 가져오는 것을 잊지 마세요.

이 메서드는 다음 단계에서 만들게 됩니다.

# --hints--

`click` 요소에 `singleCardBtn` 이벤트를 붙여야 합니다. 여기서 `this` 키워드가 필요하다는 점을 잊지 마세요.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

이벤트 콜백 안에서는 `singleCardSelected` 메서드를 호출해야 합니다.
