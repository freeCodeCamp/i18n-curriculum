...
`addCharacters(char1)` yako pia inarudisha kitendakazi, ambacho kinarudisha kitendakazi kingine. Unahitaji kufanya wito mwingine wa kitendakazi ili kupata rejea ya kitendakazi cha ndani kabisa kwa ajili ya wito wa kurudisha wa `.map()`. JavaScript inakuwezesha kuitisha mara moja vitendakazi vinavyorudishwa:

```js
myFunc(1)("hi");
```

Taitishe mara moja kitendakazi kinachorudishwa kutoka kwa wito wako wa `addCharacters(char1)`, na pita `char2` kama hoja.

# --hints--

Unapaswa kuunganisha wito wa kitendakazi kwa wito wako wa `addCharacters(char1)`.
...
Unapaswa kupitisha `char2` kama hoja kwa wito wako wa kitendakazi uliounganishwa.
