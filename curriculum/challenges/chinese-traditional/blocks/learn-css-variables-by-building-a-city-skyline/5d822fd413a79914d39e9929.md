...
只剩下三個建築物了。在 `.fb4` 元素內巢狀兩個新的 `div` 元素，並依序為它們添加 `fb4a` 和 `fb4b` 類別。記得你有點顛倒了 `.fb4` 和 `.fb5` 的位置，所以你現在正在處理的是最右邊的紫色建築物。

# --hints--

你應該在 `.fb4` 中添加兩個 `div` 元素。

```js
assert.equal(document.querySelectorAll("div.fb4 > div")?.length, 2);
```

你應該給第一個新的 `div` 一個 `fb4a` 類別。

```js
assert.exists(document.querySelector("div.fb4a"));
```

你應該給第二個新的 `div` 一個 `fb4b` 的類別。
