...
這是加總結果為小於一百的質數中，連續質數加總的最長序列。

在一千以下，和為質數且包含 21 項的最長連續質數和為 953。

在一百萬以下，哪一個質數可以寫成最多連續質數的總和？

# --hints--

`consecutivePrimeSum(1000)` 應該傳回一個數字。

```js
assert(typeof consecutivePrimeSum(1000) === 'number');
```

`consecutivePrimeSum(1000)` 應該傳回 953。

```js
assert.strictEqual(consecutivePrimeSum(1000), 953);
```

`consecutivePrimeSum(1000000)` 應該傳回 997651。
