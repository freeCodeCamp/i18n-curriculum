---
id: 6811e1569ecaf80dbe0fc768
title: タスク 52
challengeType: 22
dashedName: task-52
lang: en-US
---

<!-- (Audio) Jake: Will do, Maria. I'll make sure everything is carefully tested so we can choose a solution that balances performance, cost, security, and compliance. -->

# --instructions--

音声を聞いて、以下の文を完成させてください。

# --fillInTheBlank--

## --sentence--

`Will do, Maria. I'll make sure everything is carefully tested so we can choose BLANK that balances performance, cost, security, and BLANK.`

## --blanks--

`a solution`

### --feedback--

この単語は問題やチャレンジに対する答えを指します。必要に応じて冠詞を使うのを忘れないでください。

---

`compliance`

### --feedback--

この単語はルールや法律を守ることを指します。必要に応じて冠詞を使うのを忘れないでください。

# --explanation--

`A solution` は問題やチャレンジに対する答えです。ここでは、たくさんの中の一つの新しい単数の選択肢を示すために `a` が使われています。別の例：

`She found a solution that works for both teams.` - 彼女は両チームにとってうまくいく解決策を見つけました。

`Compliance` はルールや法的要件を守ることを意味する不可算名詞です。一般的に話すときは冠詞は使いません。例えば：

`Compliance is important when dealing with sensitive information.` - 機密情報を扱う際にはルールを守ることが重要です。

# --scene--

```json
{
  "setup": {
    "background": "company2-boardroom.png",
    "characters": [
      {
        "character": "Jake",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_20-1.mp3",
      "startTime": 1,
      "startTimestamp": 91.12,
      "finishTimestamp": 100.76
    }
  },
  "commands": [
    {
      "character": "Jake",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Jake",
      "startTime": 1,
      "finishTime": 4.72,
      "dialogue": {
        "text": "Will do, Maria. I'll make sure everything is carefully tested",
        "align": "center"
      }
    },
    {
      "character": "Jake",
      "startTime": 4.72,
      "finishTime": 10.74,
      "dialogue": {
        "text": "so we can choose a solution that balances performance, cost, security, and compliance.",
        "align": "center"
      }
    },
    {
      "character": "Jake",
      "opacity": 0,
      "startTime": 11.54
    }
  ]
}
```
