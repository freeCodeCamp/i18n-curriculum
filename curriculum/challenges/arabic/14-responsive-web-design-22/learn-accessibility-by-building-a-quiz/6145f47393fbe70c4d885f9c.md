...
ادمج عنصر `textarea` واحد ضمن عنصر `div.answer` الثاني، وقم بتعيين عدد الصفوف والأعمدة التي يحتوي عليها.

ثم أعطِ `textarea` نص نائب (placeholder) الذي يصف مثالًا للإجابة.

# --hints--

يجب أن تدمج عنصر `textarea` واحد ضمن عنصر `div.answer` الثاني.

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

يجب أن تعطي `textarea` سمة `rows` مع عدد.
...
يجب أن تعطي `textarea` سمة `cols` مع عدد.
...
يجب أن تعطي `textarea` سمة `placeholder` مع نص يصف مثال لإجابة.
