...
Todavía dentro de la cláusula `elif`, pasa el iterador `expenses_from_category` a una llamada `print_expenses`.

# --hints--

Debes llamar a `print_expenses()` pasando `expenses_from_category` como argumento en tu cláusula `elif`.
...
Tu llamada a `print_expenses()` debe venir después de la variable `expenses_from_category`.
