...
디버깅을 위해 내부 반복문에 `console.log()` 호출을 추가하세요. `array`, `array[j]`, `array[j+1]` 인수를 전달하세요.

# --hints--

내부 반복문에 `console.log()` 호출을 사용하세요.
...
`array`에 첫 번째 인수로 `console.log()`를 전달하세요.
...
`array[j]`에 두 번째 인수로 `console.log()`를 전달하세요.
...
`array[j+1]`에 세 번째 인수로 `console.log()`를 전달하세요.
