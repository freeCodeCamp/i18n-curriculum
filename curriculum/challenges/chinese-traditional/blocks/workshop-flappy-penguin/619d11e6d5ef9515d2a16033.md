...
將標的 `.beak` 中具有 `class` 為 `top` 的元素設定 `width` 為 `20%`，並將其從父元素的頂部定位在 `60%`，從左側定位在 `40%`。
...
你應該給 `.beak.top` 一個 `width` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.width, '20%');
```

你應該給 `.beak.top` 一個 `top` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.top')?.top, '60%');
```

你應該給 `.beak.top` 一個 `left` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。
