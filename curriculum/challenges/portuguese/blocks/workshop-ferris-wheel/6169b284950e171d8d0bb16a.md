...
Finalmente, crie um novo seletor `75%` entre seus seletores `50%` e `100%`. Dê a esse novo seletor uma propriedade `background-color` definida como `yellow`.

Dessa forma, sua animação fica muito mais suave e a roda-gigante está completa.

# --hints--

Você deve criar um seletor de `75%` em sua regra `@keyframes cabins`.
...
O elemento `75%` deve estar entre os seletores de `50%` e `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText, '75%');
```

O seletor de `75%` deve ter a propriedade `background-color` definida como `yellow`.
