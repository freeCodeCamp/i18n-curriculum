...
Sasa kwa kuwa una usemi wa kawaida wa kulinganisha mno au kugawanya, unaweza kutathmini usemi huo wa kihesabu.

Tangaza kigezo cha `str2` na weka thamani yake matokeo ya wito wa `infixEval` ukiwa na `str` na `regex` kama hoja.

# --hints--

Unapaswa kutangaza kigezo cha `str2`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*(?:const|let|var)\s+str2/);
```

Unapaswa kutumia `const` kutangaza kigezo chako cha `str2`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2/);
```

Unapaswa kuweka thamani ya `str2` matokeo ya wito wa kitendakazi chako cha `infixEval`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(/);
```

Unapaswa kupitisha `str` kama hoja ya kwanza kwa wito wako wa `infixEval`.
...
Unapaswa kupitisha `regex` kama hoja ya pili kwa wito wako wa `infixEval`.
