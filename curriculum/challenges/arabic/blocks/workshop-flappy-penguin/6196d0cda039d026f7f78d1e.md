...
غيّر مستوى التكديس للعنصر `.back-mountain` بحيث يكون مباشرة خلف العنصر `.left-mountain`.
...
يجب تعيين خاصية `z-index` إلى `1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.zIndex, '1');
```

لا يجب تغيير خاصية `z-index` للعنصر `.left-mountain`.
