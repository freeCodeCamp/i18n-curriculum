...
Ваш `addCharacters(char1)` також повертає функцію, яка повертає ще одну функцію. Вам потрібно зробити ще один виклик функції, щоб отримати доступ до найвнутрішнішого референсу функції для `.map()` функції зворотного виклику. JavaScript дозволяє одразу викликати повернені функції:

```js
myFunc(1)("hi");
```

Одразу викличте функцію, що повертається з вашого виклику `addCharacters(char1)`, і передайте `char2` як аргумент.

# --hints--

Ви повинні додати виклик функції до вашого виклику `addCharacters(char1)`.
...
Ви повинні передати `char2` як аргумент до вашого доданого виклику функції.
