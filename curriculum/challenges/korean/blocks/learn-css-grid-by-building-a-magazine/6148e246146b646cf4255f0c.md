...
`.list-title, .list-subtitle` 선택자를 만들고 `color` 속성을 `#00beef`로 설정하세요.

# --hints--

`.list-title, .list-subtitle` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.list-title, .list-subtitle'))
```

`.list-title, .list-subtitle` 선택자에 `color` 속성이 `#00beef`로 설정되어야 합니다.
