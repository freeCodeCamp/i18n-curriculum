...
配置 `helmet.hsts()` 以在未來 90 天內使用 HTTPS。 傳遞配置對象 `{maxAge: timeInSeconds, force: true}`。 你可以創建一個變量 `ninetyDaysInSeconds = 90*24*60*60;` 來用於 `timeInSeconds`。 Gitpod already has hsts enabled. 要覆蓋它的設置，你需要在配置對象中把 “force” 字段設置爲 true。 We will intercept and restore the Gitpod header, after inspecting it for testing.
