...
最後に必要なメソッドは `__ge__` で、これは `>=` を使って2つのオブジェクトを比較するときに呼び出されます。

`__ge__` と `self` をパラメータに持つ `other` メソッドを定義し、`self < other` の逆の値を返すようにしてください。

# --hints--

`__ge__` クラス内に `R2Vector` メソッドを定義する必要があります。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__ge__")`)) })
```

`__ge__` メソッドは2つのパラメータ、`self` と `other` を持つべきです。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__ge__").has_args("self, other")`)) })
```

`__ge__` メソッドは `self < other` の逆の値を返すべきです。
