...
`Category` クラスを完成させてください。このクラスは、*food*、*clothing*、*entertainment* のような異なる予算カテゴリに基づいてオブジェクトを生成できる必要があります。オブジェクトが作成されるとき、カテゴリ名が渡されます。クラスには `ledger` というリストのインスタンス変数が必要です。また、以下のメソッドを含む必要があります。

- 金額と説明を受け取る `deposit` メソッド。説明が与えられなければ空文字列がデフォルトになります。このメソッドは `{'amount': amount, 'description': description}` の形で元帳リストにオブジェクトを追加します。
- `withdraw` メソッドに似た `deposit` メソッドですが、渡された金額は元帳に負の数として保存されます。資金が不足している場合は元帳に何も追加されません。このメソッドは引き出しが行われた場合 `True` を返し、そうでなければ `False` を返します。
- これまでの入金と引き出しに基づいて予算カテゴリの現在の残高を返す `get_balance` メソッド。
- 金額と別の予算カテゴリを引数に取る `transfer` メソッド。このメソッドは「Transfer to [移動先の予算カテゴリ]」という説明で引き出しを追加し、その後「Transfer from [移動元の予算カテゴリ]」という説明で他の予算カテゴリに入金を追加します。資金が不足している場合はどちらの元帳にも何も追加されません。このメソッドは移動が行われた場合 `True` を返し、そうでなければ `False` を返します。
- 金額を引数に取る `check_funds` メソッド。金額が予算カテゴリの残高より大きければ `False` を返し、そうでなければ `True` を返します。このメソッドは `withdraw` メソッドと `transfer` メソッドの両方で使われます。

予算オブジェクトを出力すると、以下のように表示される必要があります。

- カテゴリ名が `*` 文字で中央揃えされた30文字のタイトル行。
- 元帳の項目リスト。各行は説明と金額を表示します。説明は最初の23文字まで表示し、その後に金額を表示します。金額は右揃えで小数点以下2桁、最大7文字まで表示します。
- カテゴリ合計を表示する行。
...
`Category` クラスのほかに、`create_spend_chart` という関数（クラス外）を作成してください。この関数はカテゴリのリストを引数に取り、棒グラフの文字列を返します。

チャートは関数に渡された各カテゴリで使われた割合を示します。割合は入金ではなく引き出しのみで計算し、各カテゴリの支出額が全カテゴリの合計支出に占める割合です。チャートの左側には0から100までのラベルがあり、棒グラフの「バー」は 'o' 文字で作られます。各バーの高さは10の位で切り捨てられます。バーの下の横線は最後のバーより2文字分長くなります。各カテゴリ名はバーの下に縦書きで表示されます。タイトルは「Percentage spent by category」と表示されます。

この関数は最大4つのカテゴリでテストされます。

以下の出力例をよく見て、スペースの位置が例と完全に一致するようにしてください。
...
注意：テストの詳細な出力を見るには、F12でブラウザのコンソールを開いてください。

# --hints--

`deposit` メソッドは元帳のインスタンス変数に特定のオブジェクトを作成する必要があります。
...
`withdraw` メソッドは引き出しが行われた場合 `True` を返します。
...
`food.deposit(900, 'deposit')` と `food.withdraw(45.67, 'milk, cereal, eggs, bacon, bread')` を呼び出すと `854.33` の残高が返されます。
...
カテゴリオブジェクトで `transfer` メソッドを呼び出すと、そのカテゴリの元帳に特定の項目が作成されます。
...
`transfer` メソッドは移動が行われた場合 `True` を返します。
...
カテゴリオブジェクトで `transfer` を呼び出すと、そのカテゴリの残高が減少します。
...
`transfer` メソッドは引数として渡されたカテゴリオブジェクトの残高を増加させます。
...
`transfer` メソッドは引数として渡されたカテゴリオブジェクトの元帳に特定の項目を作成します。
...
`check_funds` メソッドは渡された金額がカテゴリの残高より大きい場合 `False` を返します。
...
`check_funds` メソッドは渡された金額がカテゴリの残高より大きくない場合 `True` を返します。
...
`withdraw` メソッドは引き出しが行われなかった場合 `False` を返します。
...
`transfer` メソッドは移動が行われなかった場合 `False` を返します。
...
`Category` インスタンスを出力すると、オブジェクトの異なる文字列表現が得られます。
...
`create_spend_chart` チャートの上部タイトルは `Percentage spent by category` と表示されます。
...
`create_spend_chart` チャートは左側に正しい割合を表示します。
...
`create_spend_chart` チャートの各バーの高さは10の位で切り捨てられます。
...
`create_spend_chart` チャートの各行は同じ長さです。異なるカテゴリのバーは2スペースで区切られ、最後のバーの後にも2スペースあります。
...
`create_spend_chart` はバーの下に正しい横線を表示します。各カテゴリに3つの `-` 文字を使い、合計で最後のバーより2文字分長くなります。
...
`create_spend_chart` チャートの最後に改行文字はありません。
...
`create_spend_chart` チャートは各カテゴリ名をバーの下に縦書きで表示します。各行は同じ長さで、各カテゴリは2スペースで区切られ、最後のカテゴリの後にも2スペースあります。
...
`create_spend_chart` は異なるチャート表現を出力します。すべてのスペースが正確であることを確認してください。詳細はF12でブラウザのコンソールを開いてください。
