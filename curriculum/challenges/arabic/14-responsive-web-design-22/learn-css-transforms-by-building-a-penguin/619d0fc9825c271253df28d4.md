...
يجب عليك استخدام منتقي `.blush`.

```js
assert.match(code, /\.blush\s*\{/);
```

يجب أن يحتوي عنصر `.blush` على خاصية `width` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.width, '15%');
```

يجب أن يحتوي عنصر `.blush` على خاصية `height` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.height, '10%');
```

يجب أن يحتوي عنصر `.blush` على خاصية `background-color` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
