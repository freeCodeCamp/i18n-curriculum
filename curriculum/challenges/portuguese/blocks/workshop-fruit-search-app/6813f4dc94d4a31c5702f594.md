...
Agora que você tem os dados da API, atualize o estado `results` chamando `setResults` e passando um array que contém apenas a propriedade `name` de cada fruta no array `data`. Use o método `map()` para isso.

# --hints--

Você deve atualizar `setResults` com `data.map(fruit => fruit.name)`.
