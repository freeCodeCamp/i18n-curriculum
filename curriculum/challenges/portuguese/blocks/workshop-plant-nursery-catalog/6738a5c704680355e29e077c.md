...
Como você já aprendeu, o método `delete` é usado para remover uma entrada de um mapa.Conjuntos têm um método `delete` que você pode usar para remover um elemento deles também

Às vezes você pode precisar remover todos os elementos de um map ou de um set.O método `clear` faz exatamente esse trabalho.

Chame o método `clear` em `plantsSet` para remover todos os elementos dele.

# --hints--

Você deve chamar o método `clear` em `plantsSet`.
