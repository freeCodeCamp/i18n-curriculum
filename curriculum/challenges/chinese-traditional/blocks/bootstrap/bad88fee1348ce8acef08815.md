...
title: 使用 Bootstrap Grid 將元素並排放置
challengeType: 0
forumTopicId: 18371
...
Bootstrap 使用響應式 12 行網格系統，讓你輕鬆將元素放入列中並指定每個元素的相對寬度。大多數 Bootstrap 的類別都可以套用到 `div` 元素。

Bootstrap 有不同的行寬屬性，會根據使用者的螢幕寬度來使用。 例如，手機的螢幕較窄，筆記型電腦的螢幕較寬。

例如 Bootstrap 的 `col-md-*` 類別。這裡，`md` 代表中等，而 `*` 是一個數字，用來指定元素應該佔用多少行寬度。在這個例子中，指定了元素在中等大小螢幕（例如筆記型電腦）上的行寬。

在我們正在建置的 Cat Photo App 中，我們會使用 `col-xs-*`，其中 `xs` 代表超小（像是超小型手機螢幕），而 `*` 是指定元素寬度的行數。

將 `Like`、`Info` 和 `Delete` 按鈕並排放置，方法是將這三個按鈕全部巢狀在一個 `<div class="row">` 元素中，然後將它們各自放在 `<div class="col-xs-4">` 元素中。

`row` 類別套用到一個 `div`，而按鈕本身可以巢狀在其中。

# --hints--

你的按鈕都應該巢狀在具有 `row` 類別的同一個 `div` 元素內。
...
你的每個 Bootstrap 按鈕都應該巢狀在具有類別 `col-xs-4` 的獨立 `div` 元素中。
