...
Метод `.remove()` видаляє з списку перший відповідний елемент, який передається як аргумент:
...
Завершіть цикл `while`, видаливши поточний вузол зі списку `unvisited`. Зверніть увагу на відступи.

# --hints--

Ви повинні використати метод `.remove()` для видалення поточного вузла з `unvisited` після вашого циклу `for`.
