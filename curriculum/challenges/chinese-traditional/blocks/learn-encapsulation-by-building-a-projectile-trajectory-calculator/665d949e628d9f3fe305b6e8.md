...
輸出變得相當難以閱讀，所以你應該將 `print(graph.create_trajectory())` 替換為一個分別列印每一列的迴圈。 

# --hints--

移除 `print(graph.create_trajectory())` 呼叫。
...
你應該有一個迴圈來迭代 `graph.create_trajectory()`。
...
你應該列印 `graph.create_trajectory()` 中的每一列。
