...
Você havia previamente adicionado uma chamada `console.log()` à sua função `swapCrewMembers` para validar seu comportamento, e agora a chamada não é mais necessária. Para evitar que o terminal fique poluído nos próximos passos, vá em frente e remova o `for` loop atual que registra no console.

# --hints--

Remova o `for` loop da `swapCrewMembers` função. Você deve excluir o bloco inteiro do loop.
