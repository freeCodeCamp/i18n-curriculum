...
Ahora, llama a la función `chooseShape()` con `val.value` para mostrar los campos de entrada correctos, luego llama a `updateResult()` para recalcular y mostrar el área.

# --hints--

Debes llamar a la función `chooseShape` con `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

Debes llamar a la función `updateResult()`.
