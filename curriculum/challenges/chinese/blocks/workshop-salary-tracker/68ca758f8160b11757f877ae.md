...
最后，创建第三个 `if` 语句，当新等级的基本工资低于当前等级的基本工资时，引起带有消息 `Cannot change to lower level.` 的 `ValueError`。

# --hints--

你应该在你的 `level` setter 内有第三个 `if` 语句。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("level")[1].find_ifs()[2]`)) })
```

当 `new_level` 低于 `self.level` 时，你应该引起一个带有消息 `Cannot change to lower level.` 的 `ValueError`。
...
当 `new_level` 高于 `self.level` 时，你不应引起任何异常。
