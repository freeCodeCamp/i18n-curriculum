...
Ndani ya darasa la `Bee`, tengeneza kijenzi kinachopokea kigezo kimoja `emojiElement` cha aina `HTMLParagraphElement`.

Ndani ya kijenzi, fanya wito wa kitendakazi `super` ukiwa na kigezo `emojiElement` ili kuitisha kijenzi cha darasa la mzazi `Bug`. Kisha, weka sifa ya `emoji` kuwa thamani ya mfuatano wa herufi `"🐝"`.

# --hints--

Darasa lako la `Bee` linapaswa kuwa na kijenzi.
...
Kijenzi chako cha `Bee` kinapaswa kuwa na kigezo kinachoitwa `emojiElement` cha aina `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Bee[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Kijenzi chako cha `Bee` kinapaswa kuita `super` ukiwa na kigezo `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /super\s*\(\s*emojiElement\s*\)/);
```

Kijenzi chako cha `Bee` kinapaswa kuweka `this.emoji` kuwa `"🐝"`.
