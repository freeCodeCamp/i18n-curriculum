...
Si `userData.currentSong` no es `null`, necesitas encontrar la siguiente canción para reproducir. Recuerda que puedes utilizar `getNextSong` para eso.

Completa tu función `playNextSong` de manera que reproduzca la siguiente canción en la lista. Si la canción actual es la última, establece `userData.currentSong` a `null`, `userData.songCurrentTime` a cero y llama a la función `pauseSong`.

# --hints--

Cuando el `currentSong` de `userData` es verdadero y no es la última canción de la lista, la función `playNextSong` debería reproducir la canción siguiente a la actual. 
...
Cuando el `currentSong` de `userData` es la última canción de la lista, la función `playNextSong` debería establecer `userData.currentSong` en `null`.
...
Cuando el `currentSong` de `userData` es la última canción de la lista, la función `playNextSong` debería establecer `userData.songCurrentTime` a `0`.
...
Cuando el `currentSong` de `userData` es la última canción de la lista, la función `playNextSong` debería llamar a la función `pauseSong`.
...
Cuando el `currentSong` de `userData` es igual a `null`, tu función `playNextSong` debería aún llamar a la función `playSong` con el `id` de la primera canción en el arreglo `userData.songs` como argumento.
