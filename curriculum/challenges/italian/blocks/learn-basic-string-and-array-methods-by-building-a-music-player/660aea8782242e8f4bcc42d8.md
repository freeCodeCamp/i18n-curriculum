...
Proprio come le funzioni normali, le funzioni freccia possono accettare più parametri.

Ecco un esempio di funzione freccia nominata con un parametro:
...
Se la funzione ha solo un parametro, puoi omettere le parentesi tonde intorno alla lista dei parametri così:
...
Crea una nuova funzione freccia nominata chiamata `printMessage` che abbia un parametro chiamato `org`. All’interno del corpo di quella funzione, aggiungi un’istruzione console. All’interno di quell’istruzione console, aggiungi il letterale template `${org} is awesome!`.

Sotto la tua funzione `printMessage`, chiama la funzione e passa come argomento la stringa `"freeCodeCamp"`.

Apri la console per vedere il risultato.

# --hints--

Dovresti avere una funzione `printMessage`.
...
La tua funzione `printMessage` dovrebbe avere un parametro chiamato `org`.
...
La tua funzione `printMessage` dovrebbe avere un’istruzione console.

```js
assert.match(printMessage.toString(),/console\.log\(/);
```

La tua istruzione console dovrebbe includere il letterale template `${org} is awesome!`.

```js
assert.match(code, /console\.log\(\s*`[^`]*\${org} is awesome![^`]*`\s*\)/);
```

Dovresti chiamare la funzione `printMessage` e passare come argomento la stringa `"freeCodeCamp"`.

```js
assert.match(code,/printMessage\(\s*["']freeCodeCamp["']\s*\)/);
```

La chiamata alla tua funzione dovrebbe essere sotto la definizione della funzione `printMessage`.
...
La chiamata alla funzione `printMessage` dovrebbe produrre in output `freeCodeCamp is awesome!` nella console.
