...
现在你需要准备更新显示用户新增的产品。 声明一个 `currentProductCount` 变量，并将其值分配为与 `product` 的 `id` 匹配的 `totalCountPerProduct` 对象的属性。
...
你应该将 `totalCountPerProduct[product.id]` 的值赋给 `currentProductCount`。
