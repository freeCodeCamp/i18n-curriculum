...
Die `map()`-Methode wird verwendet, um ein Array durchzulaufen und ein neues Array zurückzugeben. Sie ist hilfreich, wenn Sie ein neues Array basierend auf den Werten eines bestehenden Arrays erstellen möchten. Zum Beispiel:
...
Beachten Sie, dass die `map()`-Methode eine Funktion als Argument annimmt. Dies wird <dfn>Callback-Funktion</dfn> genannt, eine Funktion, die als Argument an eine andere Funktion übergeben wird. Im obigen Beispiel ist die Callback-Funktion `(number) => number * 2`, und sie wird für jedes Element im `numbers`-Array ausgeführt. Die `map()`-Methode gibt dann ein neues Array mit den Ergebnissen zurück.

Übergeben Sie eine Callback-Funktion an die `map()`-Methode. Die Callback-Funktion sollte `song` als Parameter verwenden, die Pfeilfunktions-Syntax nutzen und einen leeren Körper haben.

# --hints--

Die Callback-Funktion Ihrer `map()`-Methode sollte die Pfeilfunktions-Syntax verwenden.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

Ihre `map()`-Methode sollte einen `song`-Parameter in ihrer Callback-Funktion haben.
