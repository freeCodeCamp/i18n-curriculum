...
Todas las funcionalidades principales ahora están en su lugar. El único problema ahora es que la próxima canción no se reproduce automáticamente cuando la canción que está sonando termina.

Para solucionar eso, puedes configurar un oyente de eventos que detectará cuando la canción que está sonando termine. El oyente de eventos `"ended"` es apropiado para esto. Se dispara cuando la reproducción de un medio llega al final.

Agrega un oyente de eventos al elemento `audio` que escucha el evento `"ended"`. Pasa una función de devolución de llamada usando sintaxis de flecha con llaves vacías.

# --hints--

Deberías encadenar el método `addEventListener()` a tu variable `audio`.
...
Deberías usar sintaxis de flecha para pasar una función de devolución de llamada vacía a tu oyente de eventos `"ended"`.
