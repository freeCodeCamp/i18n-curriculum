...
创建另一个名为 `cabins` 的 `@keyframes` 规则。 使用与 `@keyframes wheel` 相同的属性，复制 `0%` 和 `100%` 规则，但将 `100%` 选择器的 `transform` 属性设置为 `rotate(-360deg)`。
...
你的 `0%` 选择器应该有一个 `transform` 属性，设置为 `rotate(0deg)`。
