...
`div`の`className`を持つ`form-group`を追加してください。その中に、`label`の`className`の`label`と、`htmlFor`の`name`属性を追加してください。その下に、`input`に設定された`id`を持つ`name`、`className`の`input`、および`type`の`text`属性を追加してください。

# --hints--

`div`の`className`を持つ`form-group`があるはずです。
...
`form-group`の中に、`label`の`className`と`label`の`htmlFor`属性を持つ`name`が含まれているはずです。
...
`form-group`の中に、`input`の`id`、`name`の`className`、および`input`の`type`を持つ`text`が含まれているはずです。
