...
現在聲明一個 `lunchCalories` 變量，並賦予它使用 `lunchNumberInputs` 調用 `getCaloriesFromInputs` 的值。

# --hints--

你的 `calculateCalories` 函數應該有一個 `lunchCalories` 變量。

```js
assert.match(calculateCalories.toString(), /lunchCalories\s*=/);
```

你應該使用 `lunchNumberInputs` 作爲參數調用 `getCaloriesFromInputs`。

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*lunchNumberInputs\s*\)/);
```

你應該將 `getCaloriesFromInputs` 的結果分配給 `lunchCalories`。
