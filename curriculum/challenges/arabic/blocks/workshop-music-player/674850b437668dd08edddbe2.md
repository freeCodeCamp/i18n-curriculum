...
في الخطوات السابقة، بنيت وظيفة تشغيل أغنية. الآن عليك إضافة الوظيفة إلى زر التشغيل.

استخدم طريقة `addEventListener()` على `playButton` ومرر حدث `"click"` كالمعلمة الأولى. استخدم الآن استدعاء راجع يستدعي `playSong(0)` كالمعلمة الثانية.

بعدها، اختبر ذلك بالنقر على زر التشغيل.


# --hints--

يجب أن تضيف مستمع حدث لحدث `click` على `playButton` وتمرر دالة استدعاء راجع تستدعي `playSong(0)` كالمعلمة الثانية لطريقة `addEventListener`.
