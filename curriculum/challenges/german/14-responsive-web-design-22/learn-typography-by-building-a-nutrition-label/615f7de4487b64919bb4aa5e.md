...
Erstelle nach deinem letzten `.divider` ein neues `p`-Element mit dem Text `Cholesterol 0mg 0%`. Then, wrap the text `Cholesterol` in a `span` element, `0%` in another, and give each of them a class of `bold`.
