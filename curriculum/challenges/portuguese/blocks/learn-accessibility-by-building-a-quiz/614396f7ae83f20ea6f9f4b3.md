...
Use o padrão acima para definir a regra do CSS `.sr-only`.
...
Você deve dar ao `.sr-only` uma `position` `absolute`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sr-only')?.position, 'absolute');
```

Você deve dar ao `.sr-only` uma `width` de `1px`.
...
Você deve dar ao `.sr-only` um `clip-path` de `inset(50%)`.
