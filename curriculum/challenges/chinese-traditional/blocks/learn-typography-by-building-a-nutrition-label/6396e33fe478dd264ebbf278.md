...
創建一個新的 `.calories-info` 選擇器，並將其 `display` 屬性設置爲 `flex`。 再給它一個 `justify-content` 屬性，設置爲 `space-between`，以及一個 `align-items` 屬性，設置爲 `flex-end`。
...
你的 `.calories-info` 選擇器應該將 `justify-content` 屬性設置爲 `space-between`。
