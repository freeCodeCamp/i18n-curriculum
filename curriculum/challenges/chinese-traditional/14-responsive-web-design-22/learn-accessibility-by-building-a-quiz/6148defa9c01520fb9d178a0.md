...
雖然 `ul`/`li` 元素貼心地爲列表項提供了項目符號，但是你的單選按鈕不需要它們。 你可以使用 `list-style` 屬性來控制項目符號的樣式。 例如，你可以使用以下代碼將項目符號變成圓圈：
...
通過將 `.answers-list` 項的樣式設置爲 `none` 來移除默認樣式，並移除無序列表的內邊距。
