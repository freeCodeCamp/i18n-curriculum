...
Ahora, es momento de personalizar los botones de opción. Por defecto, los botones de opción tienen una apariencia nativa del navegador. Quieres eliminar esto para poder crear un aspecto personalizado.

Selecciona el elemento con la clase `contact-method-radio-btn`. Establece `appearance` en `none`.

# --hints--

Debes apuntar al elemento con la clase `contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Establece la propiedad `appearance` en `none`.
