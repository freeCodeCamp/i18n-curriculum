...
方程式が2次の場合のために、もう1つの`case`を追加してください。`analyze`の`QuadraticEquation`メソッドが返すのと同じ形式の辞書を使います。

次に、`details_list`に`concavity = <concavity>`と`<min_max> = (<x>, <y>)`の形式の2つの文字列を含むリストを代入してください。`<x>`と`<y>`は`3`の10進数の桁数で表示するようにフォーマットしてください。

最後に、`match`/`case`ブロックの後で、`details_list`を繰り返し処理し、各アイテムを`output_string`の現在の値に追加してください。各文字列アイテムは改行文字で終わるようにしてください。ここで追加のフォーマットオプションは使わないでください。

# --hints--

`match`文の主語の値は変更しないでください。

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[1].find_match_subject().is_equivalent("details")`)) })
```

既存の`case`ブロックは変更しないでください。
