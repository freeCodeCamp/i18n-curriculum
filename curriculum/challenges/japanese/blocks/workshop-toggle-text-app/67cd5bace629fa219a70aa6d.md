...
いよいよ `handleToggleVisibility` 関数を使う時です。

これまでのレッスンでは、React の Synthetic Event System と `onClick` や `onSubmit` のような属性の扱い方を学びました。

`onClick` 属性に関数の参照を次のように渡せることを覚えておいてください。
...
既存の `button` 要素を更新して、`onClick` 属性を追加し、`handleToggleVisibility` 関数の参照を渡してください。

これでボタンをクリックすると、段落テキストの可視性が隠れる状態と表示される状態で切り替わるのがページ上で見られます。

# --hints--

`toggle-button` がクリックされると、メッセージ `"I love freeCodeCamp!"` が画面に表示されるはずです。
