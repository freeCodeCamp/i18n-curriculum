...
為了提供一種離開程式的方法，使用另一個 `elif` 語句來檢查 `choice` 是否相等於字串 `'5'`。

在新的 `elif` 語句中，列印字串 `'Exiting the program.'` 以顯示程式正在終止其執行。

# --hints--

你應該創建一個 `elif` 語句來檢查 `choice == '5'`。
...
你應該在你的新 `elif` 語句中列印 `'Exiting the program.'`。
