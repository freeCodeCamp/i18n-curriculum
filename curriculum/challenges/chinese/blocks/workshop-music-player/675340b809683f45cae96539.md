...
如果在没有播放歌曲时按下上一首按钮，第一首歌曲将播放。为了解决这个问题，在 `playPreviousSong` 函数的开头添加一个 `if` 语句。查看当前歌曲是否为 `null`，并使用 `return` 语句停止函数执行。

# --hints--

你应该为你的 `playPreviousSong` 函数添加一个 `if` 语句，查看 `userData.currentSong === null`，并使用 `return` 语句停止函数执行。
