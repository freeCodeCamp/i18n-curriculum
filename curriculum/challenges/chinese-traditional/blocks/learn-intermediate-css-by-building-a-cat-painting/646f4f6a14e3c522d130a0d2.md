...
使用類別選擇器來標的 `.cat-whisker-right-middle` 元素，然後將右中鬚以 `position` 為 `absolute`、`top` 為 `127px` 和 `left` 為 `109px` 移動到位置。

# --hints--

你應該有一個 `.cat-whisker-right-middle` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle'))
```

你的 `.cat-whisker-right-middle` 選擇器應該有一個設定為 `absolute` 的 `position` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.position === 'absolute')
```

你的 `.cat-whisker-right-middle` 選擇器應該有一個 `top` 屬性設定為 `127px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.top === '127px')
```

你的 `.cat-whisker-right-middle` 選擇器應該有一個設定為 `109px` 的 `left` 屬性。
