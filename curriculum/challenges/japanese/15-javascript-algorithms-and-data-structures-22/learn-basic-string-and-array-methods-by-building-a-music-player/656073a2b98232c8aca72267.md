...
現在の曲の情報と再生時間を扱うために、`currentSong` と `songCurrentTime` というプロパティを作成してください。 それぞれ、値は `null` と `0` に設定してください。

# --hints--

`userData` オブジェクトの既存プロパティのコンテンツは変更しないでください。

```js
assert.match(code, /let\s+userData\s*=\s*\{\s*songs:\s*\[\s*\.\.\.allSongs\s*\]\s*,?\s*/);
```

`userData` オブジェクトは、`currentSong` キーが `null` に設定されている必要があります。

```js
assert.propertyVal(userData, "currentSong", null)
```

`userData` オブジェクトは、`songCurrentTime` キーが `0` に設定されている必要があります。
