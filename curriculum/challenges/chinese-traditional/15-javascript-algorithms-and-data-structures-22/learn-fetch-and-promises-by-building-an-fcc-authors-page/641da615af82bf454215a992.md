...
將一個空的回調函數傳遞給 `.forEach()` 方法。 對於回調的第一個參數，解構 `author`、`image`、`url` 和 `bio` 項。

對於第二個參數，傳入 `index`。 這將代表每個作者的位置，並且對於以後的分頁很有用。

# --hints--

你應該將 `author`、`image`、`url` 和 `bio` 解構爲回調函數的第一個參數。 不要忘記將它們放在花括號內。
...
你應該將 `index` 解構爲回調函數的第二個參數。 它必須位於其他內容所用的花括號外面。
