...
Sasa sahihisha kichaguzi chako cha `h1` kuwa `header h1` ili kulenga hasa kipengele chako cha `h1` ndani ya `header` mpya.
...
Haupasi kuwa na kichaguzi cha `h1`.

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('h1'));
```

Haupasi kubadilisha vigezo vyovyote katika kichaguzi.
