...
接著，使用 `getElementById()` 方法存取 `discard-btn`、`tasks-container` 和 `title-input` 元素。分別將它們存檔在名為 `discardBtn`、`tasksContainer` 和 `titleInput` 的變數中。

# --hints--

你應該使用 `getElementById()` 來存取 `discard-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)discard\-btn\1\s*\)\s*;?/)
```

你應該將 `discard-btn` 元素指定到變數 `discardBtn`。別忘了使用 `const` 來宣告變數。

```js
assert.match(code, /const\s+discardBtn\s*=\s*document\.getElementById\(\s*('|"|`)discard\-btn\1\s*\)\s*;?/)
```

你應該使用 `getElementById()` 來存取 `tasks-container` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)tasks\-container\1\s*\)\s*;?/)
```

你應該將 `tasks-container` 元素指定到變數 `tasksContainer`。別忘了使用 `const` 來宣告變數。

```js
assert.match(code, /const\s+tasksContainer\s*=\s*document\.getElementById\(\s*('|"|`)tasks\-container\1\s*\)\s*;?/)
```

你應該使用 `getElementById()` 來存取 `title-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)title\-input\1\s*\)\s*;?/)
```

你應該將 `title-input` 元素指定給變數 `titleInput`。別忘了使用 `const` 來宣告變數。
