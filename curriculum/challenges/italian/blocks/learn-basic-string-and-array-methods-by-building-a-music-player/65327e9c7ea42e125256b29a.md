...
Abbiamo aggiunto il resto delle canzoni all'array `allSongs` per te.

Ora imparerai cos’è la <dfn>Web Audio API</dfn> e come usarla per riprodurre canzoni. Tutti i browser moderni supportano la Web Audio API, che ti permette di generare e processare audio nelle applicazioni web.

Usa `const` per creare una variabile chiamata `audio` e assegnale il valore `new Audio()`. Questo creerà un nuovo elemento HTML5 `audio`.

# --hints--

Dovresti usare la parola chiave `new` per creare un’istanza dell’oggetto `Audio`.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

Dovresti assegnare l’oggetto `Audio` a una costante chiamata `audio`.
