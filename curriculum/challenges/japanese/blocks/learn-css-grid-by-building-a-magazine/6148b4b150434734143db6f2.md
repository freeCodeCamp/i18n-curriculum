...
`grid-column` プロパティは、要素がどの列で始まりどの列で終わるかを指定します。 しかし、グリッドに列がいくつあるかわからないものの、最後の列で要素を終了させたいという場合があります。 その場合は、`-1` で終了列を指定できます。

`.hero` セレクターを作成し、`grid-column` プロパティを `1 / -1` に設定してください。 これで、要素がグリッドの幅全体に広がるように指示します。
...
`.hero` セレクターの `grid-column` プロパティを `1 / -1` に設定する必要があります。
