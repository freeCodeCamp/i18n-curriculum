...
L'elemento `span` che ha la durata ha bisogno anche di un po' di stile, quindi aggiungi le classi `text-base`, `font-medium` e `text-fuchsia-300` a esso.
...
Il tuo elemento `span` dovrebbe avere la classe `text-fuchsia-300`.
