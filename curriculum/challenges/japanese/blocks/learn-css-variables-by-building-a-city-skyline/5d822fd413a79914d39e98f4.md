...
`linear-gradient`に`.bb1d`を、最初の色に`orange`、2番目に`--building-color1`、3番目に`--window-color1`を使って追加してください。グラデーションは`background`プロパティに使うことを忘れないでください。

# --hints--

`background`に`.bb1d`を使うべきです。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background);
```

`background`プロパティに`linear-gradient`を指定するべきです。

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'linear-gradient');
```

`orange`の最初の色に`linear-gradient`を使うべきです。

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.getPropVal('background', true), 'linear-gradient(orange');
```

`--building-color1`の2番目の色に`linear-gradient`を使うべきです。
...
`--window-color1`の3番目の色に`linear-gradient`を使うべきです。
