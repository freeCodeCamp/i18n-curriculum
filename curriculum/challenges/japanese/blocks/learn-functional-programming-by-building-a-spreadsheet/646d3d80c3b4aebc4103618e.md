...
同じパターンに従って、乗算 `*` と除算 `/` のためのプロパティを適切な関数で追加してください。

# --hints--

あなたの `infixToFunction` オブジェクトには `*` プロパティが必要です。

```js
assert.property(infixToFunction, '*');
```

あなたの `*` プロパティは関数である必要があります。

```js
assert.isFunction(infixToFunction['*']);
```

あなたの `*` 関数はアロー構文を使うべきです。

```js
assert.match(code, /('|"|`)\*\1\s*:\s*\(/);
```

あなたの `*` 関数は最初のパラメータとして `x` を持つべきです。

```js
assert.match(code, /('|"|`)\*\1\s*:\s*\(\s*x/);
```

あなたの `*` 関数は2番目のパラメータとして `y` を持つべきです。

```js
assert.match(code, /('|"|`)\*\1\s*:\s*\(\s*x\s*,\s*y\s*\)/);
```

あなたの `*` 関数は暗黙の戻り値を使うべきです。

```js
assert.notMatch(code, /('|"|`)\*\1\s*:\s*\(\s*x\s*,\s*y\s*\)\s*\{/);
```

あなたの `*` 関数は `x` と `y` を乗算した結果を返すべきです。
