...
デバッグのために、内側のループに`console.log()`の呼び出しを追加してください。引数として`array`、`array[j]`、`array[j+1]`を渡してください。

# --hints--

内側のループで`console.log()`の呼び出しを使うべきです。
...
`array[j+1]`の3番目の引数として`console.log()`を渡すべきです。
