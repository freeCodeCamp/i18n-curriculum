...
Після свого елемента `h2` створіть два елементи `p`. Надайте першому `class` зі значенням `dessert-price` та встановіть текст на знак долара `"$"` за яким слідує значення змінної `price`. Дайте другому `class` типу `product-category`, та текст `"Category: "`, за яким слідує значення змінної `category`.

# --hints--

Ви повинні створити два елементи `p`.
...
Ваші елементи `p` повинен йти після вашого елемента `h2`.
...
Ваш перший елемент `p` повинен мати `class` типу `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Перший елемент `p` повинен містити знак долара `"$"`, за яким слідує значення змінної `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Ваш другий елемент `p` повинен мати `class` типу `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Другий елемент `p` повинен містити текст `"Category: "`, за яким слідує значення змінної `category`.
