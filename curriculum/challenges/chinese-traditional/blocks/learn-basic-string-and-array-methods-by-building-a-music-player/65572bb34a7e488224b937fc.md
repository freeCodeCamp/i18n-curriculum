...
你需要將 `resetText` 作為子元素添加到 `resetButton` 元素，並且也將 `resetButton` 作為子元素添加到 `playlistSongs` 元素。為此，有一個 `appendChild()` 方法可供使用。

`appendChild()` 讓你將節點或元素添加為另一個元素的子元素。以下範例中，文字 "Click me" 將會附加到按鈕上：
...
使用 `appendChild()` 將 `resetText` 附加到 `resetButton` 元素，並將 `resetButton` 附加到 `playlistSongs` 元素。

# --hints--

你應該在 `resetButton` 上使用 `appendChild()` 方法。

```js
assert.match(code, /resetButton\.appendChild\(/)
```

你應該將 `resetText` 作為你第一個 `appendChild()` 的值傳入。

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

你應該在 `playlistSongs` 上使用 `appendChild()` 方法。

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

你應該將 `resetButton` 傳入作為你第二個 `appendChild()` 的值。
