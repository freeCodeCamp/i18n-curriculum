...
`showFortune` 메서드 안에 `target`이라는 변수를 선언하고 `e.target`을 할당하세요. 그리고 빈 `if` 문을 하나 더 만드세요.

# --hints--

`target` 안에 `showFortune`라는 변수를 선언해야 합니다.
...
`target` 변수에 `e.target`가 할당되어야 합니다.
...
`if` 변수 바로 다음에 빈 `target` 문이 있어야 합니다.
