...
Você deve chamar a função `showAnimation()` dentro do corpo do seu segundo `if`.

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

Após chamar a função `showAnimation()`, você deve usar a palavra-chave `return` para sair da função `checkUserInput()` antecipadamente.
