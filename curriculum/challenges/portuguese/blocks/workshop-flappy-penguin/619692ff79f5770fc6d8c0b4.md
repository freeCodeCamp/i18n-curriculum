...
Acima do elemento `.ground`, adicione uma `div` com a `class` `penguin`.Essa `div` terá o Flappy Penguin.

# --hints--

Você deve adicionar um novo `div` dentro do `body`.

```js
assert.lengthOf(document.querySelectorAll('body > div'), 2);
```

Você deve dar à `div` uma `class` `penguin`.

```js
assert.include(document.querySelector('body > div:not(.ground)')?.className, 'penguin');
```

Você deve colocar `div.penguin` antes de `div.ground`.
