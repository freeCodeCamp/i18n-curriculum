...
Fügen Sie Ihrem `instrumentsArr` zwei weitere Objekte hinzu, um die verbleibenden Holzblasinstrumente Klarinette und Oboe darzustellen.

# --hints--

Sie sollten `{ category: "woodwinds", instrument: "Clarinet", price: 200 }` innerhalb Ihres `instrumentsArr` haben.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

Sie sollten `{ category: "woodwinds", instrument: "Oboe", price: 4000 }` innerhalb Ihres `instrumentsArr` haben.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

Sie sollten weiterhin `{ category: "woodwinds", instrument: "Flute", price: 500 }` innerhalb Ihres `instrumentsArr` haben.
