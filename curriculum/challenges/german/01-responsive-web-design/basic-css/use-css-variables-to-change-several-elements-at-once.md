...
title: Verwende CSS-Variablen, um mehrere Elemente auf einmal zu ändern
challengeType: 0
...
Folge den Anweisungen unten, um zu sehen, wie das Ändern von nur drei Werten das Styling vieler Elemente verändern kann.

# --instructions--

Ändere in der `penguin`-Klasse den Wert `black` auf `gray`, den Wert `gray` auf `white`, und `yellow` auf `orange`.
...
Die `penguin`-Klasse sollte die Variable `--penguin-belly` deklarieren und ihr `white` zuweisen.

```js
assert.match(code, /.penguin\s*?{[\s\S]*--penguin-belly\s*?:\s*?white\s*?;[\s\S]*}/gi);
```

Die `penguin`-Klasse sollte die Variable `--penguin-beak` deklarieren und ihr `orange` zuweisen.
