...
이 건물에 또 다른 반복 그라디언트를 추가하세요; 방금 추가한 것과 같게 만드되, `90deg` 방향은 추가하지 말고 두 `transparent` 색상 대신 창문 색상을 사용하세요.

# --hints--

`.fb6`에 두 번째 `repeating-linear-gradient`를 `background` 속성으로 지정하세요.
...
두 번째 `repeating-linear-gradient`에 `--building-color3`에서 `0%`까지 `10%` 색상을 첫 번째 색상으로 지정하세요.
...
두 번째 `repeating-linear-gradient`에 `--window-color3`에서 `10%`까지 `30%` 색상을 두 번째 색상으로 지정하세요.
