...
У цьому проєкті ви вивчите базові методи для рядків і масивів, створюючи застосунок музичного плеєра. Ви зможете відтворювати, ставити на паузу, пропускати та перемішувати пісні.

HTML і CSS цього проєкту вже надані, тож ви можете зосередитися на JavaScript.

Почніть із доступу до елементів `#playlist-songs`, `#play` і `#pause` за допомогою методу `getElementById()`. Призначте їх відповідно змінним `playlistSongs`, `playButton` і `pauseButton`.


# --hints--

У вас має бути змінна з назвою `playlistSongs`. 

```js
assert.isDefined(playlistSongs);
```

Використайте `document.getElementById()`, щоб отримати елемент `#playlist-songs`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

Призначте елемент `#playlist-songs` змінній `playlistSongs`. 

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

У вас має бути змінна з назвою `playButton`. 

```js
assert.isDefined(playButton);
```

Використайте `document.getElementById()`, щоб отримати елемент `#play`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

Призначте елемент `#play` змінній `playButton`. 

```js
assert.equal(playButton, document.getElementById('play'));
```

У вас має бути змінна з назвою `pauseButton`. 

```js
assert.isDefined(pauseButton);
```

Використайте `document.getElementById()`, щоб отримати елемент `#pause`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

Призначте елемент `#pause` змінній `pauseButton`. 
