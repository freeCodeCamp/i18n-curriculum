...
你需要取得目前播放歌曲的 `id`。為此，你可以使用 `userData.currentSong?.id`。

使用 `getElementById()` 選取目前播放的歌曲：傳入一個模板字串，將 `id` 前置式為 `song-`。將它指定給名為 `songToHighlight` 的變數。
...
你應該將你的 `getElementById()` 指定到一個 `songToHighlight` 變數。
