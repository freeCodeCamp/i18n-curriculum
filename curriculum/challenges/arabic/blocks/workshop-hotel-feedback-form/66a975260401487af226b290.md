...
لجعل `option` محددًا افتراضيًا، يمكنك إضافة الخاصية `selected` إلى عنصر `option` الذي تريد تحديده.

إليك مثالًا يستخدم الخاصية `selected`:

```html
<option selected value="amazing">Amazing</option>
```

داخل عنصر `select` الخاص بك، أضف الخاصية `selected` إلى عنصر `option` الذي قيمته `"excellent"`.

# --hints--

يجب أن يحتوي عنصر `option` الذي قيمته `"excellent"` على الخاصية `selected`.
