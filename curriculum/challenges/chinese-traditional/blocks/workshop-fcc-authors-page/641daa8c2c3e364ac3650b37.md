...
接著，每個作者的名稱和 `image` 以及卡片上的其他細節之間沒有太多區隔。分隔線會為作者卡片提供清晰的視覺階層體系。

在作者簡介上方添加一個 `div` 元素，並為它添加 `class` 屬性，值為 `"purple-divider"`。
