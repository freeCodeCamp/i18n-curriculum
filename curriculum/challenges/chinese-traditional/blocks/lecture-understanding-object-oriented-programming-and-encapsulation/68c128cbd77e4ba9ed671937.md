...
在 Python 中，裝飾器是一個函式（程式），用來修改其他函式（程式）或類別的功能，而不改變它們原本的 `code`。

雖然可以創建訂製的裝飾器，但這超出本課程的範圍。只要知道，為了創建一個屬性，你需要定義一個方法並在其上方放置 `@property` 裝飾器。這會將該方法轉換成屬性，因此它可以像屬性一樣被存取，同時內部會呼叫被裝飾的方法。
