...
Die letzten fest codierten Werte sind die sechs Spielerwerte. Ersetzen Sie die ersten drei fest codierten Zahlen durch die entsprechende `player` Eigenschaft:
...
Sie sollten den Wert `97` für `PAC` durch `player.pac` ersetzen.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

Sie sollten den Wert `98` für `SHO` durch `player.sho` ersetzen.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

Sie sollten den Wert `83` für `PAS` durch `player.pas` ersetzen.
