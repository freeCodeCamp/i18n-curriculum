...
ブラウザには組み込みの`alert()`関数があり、これを使ってユーザーにポップアップメッセージを表示できます。表示するメッセージは`alert()`関数の引数として渡します。

テンプレートリテラルを使い、`if`ブロック内で`alert()`関数を呼び出して、ユーザーに`"Invalid Input: "`を伝え、その後に`invalidInputMatch`配列の最初の値を表示してください。

# --hints--

`alert()`ブロック内で`if`関数を呼び出す必要があります。

```js
assert.match(getCaloriesFromInputs.toString(), /if\s*\(\s*invalidInputMatch\s*\)\s*\{\s*(?:window\.|globalThis\.)?alert\(/);
```

テンプレートリテラルを使って`"Invalid Input: "`メッセージを`alert()`関数に渡してください。
...
テンプレートリテラルを使い、`invalidInputMatch`テキストの後に`"Invalid Input: "`配列の最初の要素を表示してください。
