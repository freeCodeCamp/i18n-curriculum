...
Incluye los argumentos restantes `chosenCard.name_short` y `chosenCard.img`. Luego, fuera de la función `renderCard`, restablece el `multipleCard` del elemento `innerHTML` a una cadena vacía.

# --hints--

Debes pasar `chosenCard.name_short` como argumento a `renderCard`.
...
Debes pasar `chosenCard.img` como último argumento a `renderCard`.
...
Debes restablecer `this.elements.multipleCard.innerHTML` a una cadena vacía.
