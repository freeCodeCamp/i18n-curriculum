...
Primero, necesitas obtener el número de asistentes que llegan durante el tick actual. Accede al valor en el arreglo `queue` de la compuerta de entrada en el índice `tickIndex`, y asígnalo a una variable llamada `currentTickQueue`.

# --hints--

Debes asignar el elemento en `gate.queue[tickIndex]` a una variable llamada `currentTickQueue`.
