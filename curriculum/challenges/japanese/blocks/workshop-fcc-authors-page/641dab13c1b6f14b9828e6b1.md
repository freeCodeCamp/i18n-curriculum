...
もう一つだけ。`Load More Authors` ボタンをクリックし続けて読み込むデータがなくなり、テキストが `"No more data to load"` に変わっても、カーソルの値はまだ `pointer` のままです。なぜカーソルの値を `not-allowed` に変えないのでしょうか？
...
`cursor` のプロパティから `style` にドット記法でアクセスしてください。
