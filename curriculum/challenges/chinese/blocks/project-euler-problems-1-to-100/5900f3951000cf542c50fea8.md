...
我们称一个 `n` 位数是全数字的，如果它恰好使用了从 1 到 `n` 的所有数字各一次。例如，2143 是一个 4 位全数字的数，同时它也是素数。

存在的最大的 `n` 位全数字（pandigital）素数是多少？

# --hints--

`pandigitalPrime(4)` 应该返回一个数字。

```js
assert(typeof pandigitalPrime(4) === 'number');
```

`pandigitalPrime(4)` 应该返回 4231。

```js
assert(pandigitalPrime(4) == 4231);
```

`pandigitalPrime(7)` 应该返回 7652413。
