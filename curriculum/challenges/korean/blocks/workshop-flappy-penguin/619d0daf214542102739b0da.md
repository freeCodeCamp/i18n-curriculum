...
`.eye-lid` 요소를 선택하고, `width`의 `150%`, `height`의 `100%`, 그리고 `background-color`의 `--penguin-face`를 지정하세요.
...
`.eye-lid`에 `width`의 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾을 수 있습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

`.eye-lid`에 `height`의 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾을 수 있습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.height, '100%');
```

`.eye-lid`에 `background-color`의 `var(--penguin-face)`를 지정하세요.
