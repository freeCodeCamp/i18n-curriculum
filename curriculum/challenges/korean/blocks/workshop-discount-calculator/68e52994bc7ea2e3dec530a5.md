...
마지막으로, `prices` 함수를 사용해 `min()` 리스트에서 최솟값을 반환하세요. 이렇게 하면 모든 적용 가능한 할인을 고려한 후 가장 좋은(가장 낮은) 가격을 얻을 수 있습니다.

# --hints--

`calculate_best_price` 메서드는 `prices` 함수를 사용해 `min` 리스트에서 최솟값을 반환해야 합니다.
