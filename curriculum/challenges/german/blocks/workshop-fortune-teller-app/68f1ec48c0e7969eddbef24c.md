...
Schließlich übergeben Sie innerhalb von `hideElements` die Argumente `singleCard`, `multipleCard`, `fortuneContainer` und `fortuneDescription`. Denken Sie daran, sie aus den Elementen zu holen.

Damit ist Ihre Wahrsage-App fertig!

# --hints--

Sie sollten `singleCard` als erstes Argument an `hideElements` übergeben.
...
Sie sollten `multipleCard` als Argument an `hideElements` übergeben.
...
Sie sollten `fortuneContainer` als Argument an `hideElements` übergeben.
...
Sie sollten `fortuneDescription` als Argument an `hideElements` übergeben.
