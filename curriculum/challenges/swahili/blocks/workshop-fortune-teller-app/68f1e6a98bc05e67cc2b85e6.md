...
Tangaza kigezo kinachoitwa `cardElement` na tumia darasa la vitu la `card_container` kupata kipengele. Tumia kichaguzi cha `closest` kukipata kutoka `target`.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `cardElement` ndani ya `showFortune`.
...
Kigezo chako cha `cardElement` kinapaswa kutumia `target?.closest` kupata `.card_container`.
...
Unapaswa kupitisha `.card_container` kama hoja kwa `closest`.
