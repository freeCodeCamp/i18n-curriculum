...
최종 계산 결과를 보여주려면 `propertyInputs`와 함께 `for...of` 반복문을 사용해 `Object.entries()` 객체를 순회하세요. 반복문 선언에서 `input` 앞에 쉼표를 사용하면 키는 무시할 수 있습니다.

# --hints--

키를 건너뛰고 `for...of`만 유지하면서 `Object.entries(propertyInputs)`의 각 항목을 구조 분해하는 `input` 반복문을 만드세요.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

`for...of` 반복문은 `Object.entries(propertyInputs)`를 순회해야 합니다.
