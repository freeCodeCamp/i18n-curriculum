...
Adicionamos o restante das músicas ao array `allSongs` para você.

Em seguida, você aprenderá sobre a <dfn>Web Audio API</dfn> e como usá-la para reproduzir músicas.Todos os navegadores modernos suportam a Web Audio API, que permite gerar e processar áudio em aplicações web.

Use `const` para criar uma variável chamada `audio` e definir seu valor como `new Audio()`.Isso criará um novo elemento HTML5 `audio`.

# --hints--

Você deve usar a palavra-chave `new` para criar uma instância do objeto `Audio`.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

Você deve atribuir o objeto `Audio` a uma constante chamada `audio`.
