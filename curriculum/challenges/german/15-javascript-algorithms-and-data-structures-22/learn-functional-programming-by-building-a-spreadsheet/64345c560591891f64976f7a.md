...
Nun, da deine Hilfsfunktionen vollständig sind, solltest du in deinem `onload`-Event-Handler eine `letters`-Variable deklarieren. Assign it the result of calling `charRange()` with the letters `"A"` and `"J"` as arguments.

# --hints--

Du solltest eine `letters`-Variable deklarieren.

```js
assert.match(code, /(?:let|const|var)\s+letters/);
```

Du solltest `const` verwenden, um deine `letters`-Variable zu deklarieren.

```js
assert.match(code, /const\s+letters/);
```

Du solltest deiner `letters`-Variablen einen `charRange()`-Aufruf zuweisen.
