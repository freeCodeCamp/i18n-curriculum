...
Deklarieren Sie eine `apply`-Funktion, die einen `fn`- und einen `args`-Parameter entgegennimmt.

# --hints--

Sie sollten eine `apply`-Variable deklarieren.
...
Sie sollten `const` verwenden, um Ihre `apply`-Variable zu deklarieren.
...
Ihre `apply`-Variable sollte eine Pfeilfunktion zugewiesen bekommen.
...
Ihre `apply`-Funktion sollte `fn` als ersten Parameter haben.
...
Ihre `apply`-Funktion sollte `args` als zweiten Parameter haben.
...
Ihre `apply`-Funktion sollte leer sein.
