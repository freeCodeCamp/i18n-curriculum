...
이 워크숍에서는 점술 앱을 만들어 TypeScript 작업을 계속 연습합니다.

`Card`라는 이름의 인터페이스를 선언하는 것부터 시작하세요. `Card` 인터페이스 안에 다음 속성들을 추가하세요: `name`, `name_short`, `value`. `name`와 `name_short` 속성은 `string` 타입으로 설정해야 하며, `value`은 `string | number`의 유니언 타입으로 설정해야 합니다.

# --hints--

`Card`라는 이름의 인터페이스를 선언해야 합니다.
...
`Card` 인터페이스는 `name` 속성을 `string` 타입으로 가져야 합니다.
...
`Card` 인터페이스는 `name_short` 속성을 `string` 타입으로 가져야 합니다.
...
`Card` 인터페이스는 `value` 속성을 `string`와 `number`의 유니언 타입으로 가져야 합니다.
