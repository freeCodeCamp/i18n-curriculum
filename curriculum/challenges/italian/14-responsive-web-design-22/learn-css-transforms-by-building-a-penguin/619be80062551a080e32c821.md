...
Dovresti utilizzare la proprietà `border-radius` per arrotondare la cresta.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

Dovresti dare a `.penguin-body::before` una proprietà `border-radius` di `0% 0% 100% 100%`.
