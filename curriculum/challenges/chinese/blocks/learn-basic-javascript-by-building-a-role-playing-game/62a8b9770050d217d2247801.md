...
`location["button text"]` 是一个包含三个元素的数组。 将 `button1.innerText` 赋值更改为 `location["button text"][0]`，它代表数组的第一个元素。

# --hints--

你应该访问 `location` 参数的 `"button text"` 属性的第一个元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*0\s*\]/);
```

你应该将 `button1.innerText` 属性设置为 `location` 参数的 `"button text"` 属性的第一个元素。
