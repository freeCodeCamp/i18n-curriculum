...
Wie die anderen Methoden benötigt auch `.reduce()` einen Callback. Dieser Callback benötigt jedoch mindestens zwei Parameter. Der erste ist der <dfn>Akkumulator</dfn> und der zweite ist das aktuelle Element im Array. Der Rückgabewert für den Callback wird bei der nächsten Iteration zum Wert des Akkumulators.
...
Deine `reduce`-Methode sollte die Summe von `acc` und `el` zurückgeben.
