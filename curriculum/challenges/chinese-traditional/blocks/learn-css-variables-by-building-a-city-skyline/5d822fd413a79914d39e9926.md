...
給 `.fb3-window` 元素設定 `width` 為 `25%`、`height` 為 `80%`，並使用你的 `--window-color1` 變數作為 `background-color` 的值。

# --hints--

你應該給 `.fb3-window` 一個 `width` 為 `25%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

你應該給 `.fb3-window` 一個 `height` 為 `80%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

你應該為 `.fb3-window` 設定 `background-color` 為 `--window-color1`。
