...
ここまでの進捗をテストしましょう。`Game` クラスのインスタンスを作成し、`game` 変数に代入してください。

この時点で、Single Card Reading と Three Card Spread ボタンをクリックするとカードが表示されるのがわかります。

# --hints--

`game` という名前の変数を宣言してください。
...
`game` 変数は `new Game()` で初期化する必要があります。
