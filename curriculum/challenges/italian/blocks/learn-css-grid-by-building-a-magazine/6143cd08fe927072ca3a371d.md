...
All’interno del tuo elemento `.image-wrapper`, assegna al secondo elemento `img` un attributo `src` con valore `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`, un attributo `alt` con valore `image of a calculator project`, un attributo `loading` impostato su `lazy`, un attributo `class` impostato su `image-2`, un attributo `width` impostato su `400` e un attributo `height` impostato su `400`.

# --hints--

Il tuo secondo elemento `img` dovrebbe avere un attributo `src` impostato su `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`.
...
Il tuo secondo elemento `img` dovrebbe avere un attributo `alt` impostato su `image of a calculator project`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('alt') === 'image of a calculator project');
```

Il tuo secondo elemento `img` dovrebbe avere un attributo `loading` impostato su `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('loading') === 'lazy');
```

Il tuo secondo elemento `img` dovrebbe avere un attributo `class` impostato su `image-2`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.classList?.contains('image-2'));
```

Il tuo secondo elemento `img` dovrebbe avere un attributo `width` impostato su `400`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[1]?.getAttribute('width') === '400');
```

Il tuo secondo elemento `img` dovrebbe avere un attributo `height` impostato su `400`.
