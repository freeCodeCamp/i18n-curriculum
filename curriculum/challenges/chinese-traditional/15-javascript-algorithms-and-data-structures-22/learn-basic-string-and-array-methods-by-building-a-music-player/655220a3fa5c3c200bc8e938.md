...
使用 `classList` 和 `remove()` 方法從 `playButton` 中刪除 `playing` 類，因爲歌曲將在此時暫停。

要最終暫停歌曲，請對 `audio` 變量使用 `pause()` 方法。 `pause()` 是 Web Audio API 用於暫停音樂文件的方法。

# --hints--

你不應修改現有的 `pauseSong` 函數及其內容。

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

你應該使用 `classList` 屬性和 `remove()` 方法從 `playButton` 中刪除類 `"playing"`。

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

你應該在 `audio` 變量上使用 `pause()` 方法。
