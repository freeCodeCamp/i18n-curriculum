...
Dentro de ese elemento, crea un elemento `img` con el `src` establecido en `https://cdn.freecodecamp.org/platform/universal/fcc_meta_1920X1080-indigo.png`, el `alt` establecido en `freecodecamp logo` y la `class` establecida en `hero-img`.
...
Tu elemento `img` debe tener la `class` establecida en `hero-img`.
