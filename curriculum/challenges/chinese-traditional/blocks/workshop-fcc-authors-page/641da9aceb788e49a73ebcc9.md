...
請記得在步驟 1 中你選擇了 `Load More Authors` 按鈕並將它指定給 `loadMoreBtn`。

使用 `addEventListener` 為 `loadMoreBtn` 添加一個 `"click"` 事件監聽器。並且，傳入 `fetchMoreAuthors` 函式的引用，以便在按鈕被點擊時執行。

之後，當你點擊按鈕時，你應該會看到多 8 位作者。
...
你應該在你的 `loadMoreBtn` 變數上呼叫 `addEventListener()` 方法。
...
你的事件監聽器應該以 `fetchMoreAuthors` 作為要執行的函式。
