...
У кожному відповідному елементі `label` одразу після елемента `input` додайте пробіл і додайте наступний текст:
...
Ви повинні додати одному пробілу на початку тексту першого `label`.

```js
assert.equal(document.querySelector('fieldset:nth-child(2) > label')?.innerText?.[0], ' ');
```

Ви повинні додати одному пробілу на початку тексту другого `label`.
