...
سيكون من الجيد عرض الكلمات `Show Message` أو `Hide Message` بشكل ديناميكي بناءً على حالة `isVisible`.

سبق أن راجعت كيفية العمل مع العرض الشرطي المضمَّن. ستطبق منطقًا مشابهًا على نص الزر.

إليك مثالًا على استخدام المعامل الثلاثي لعرض الكلمات `Hide` و `Show` بشكل شرطي:

```jsx
<button>{isMenuVisible ? "Hide" : "Show"} Menu</button>
```

في الخطوة الأخيرة من ورشة العمل، استخدم المعامل الثلاثي لعرض الكلمات `Hide` أو `Show` بشكل شرطي لزر الرسالة. ضع الشرط قبل نص `Message` ليعرض `Show Message` أو `Hide Message`.

وبهذا التغيير الأخير، تكتمل ورشة العمل! اختبر زرّك لترى النتيجة في التطبيق.

# --hints--

عندما تكون قيمة `isVisible` هي `false`، يجب أن يقول نص الزر `Show Message`.
...
عندما تكون قيمة `isVisible` هي `true`، يجب أن يقول نص الزر `Hide Message`.
