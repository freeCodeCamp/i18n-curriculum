...
بما أن عنصر `div` يُستخدم للملاحظة، فلن يفهم قارئ الشاشة غرضه. يمكنك إصلاح ذلك بإضافة خاصية `aria-label`. هذا سيساعد في إمكانية الوصول ويوفر معنى لهذا العنصر `div`.

في وسم `div` الافتتاحي، أضف خاصية `aria-label` وضع قيمتها على `"Note editor"`.

# --hints--

يجب أن يحتوي وسم `div` الافتتاحي على خاصية `aria-label`.

```js
assert.isTrue(document.getElementById("note").hasAttribute("aria-label"));
```

يجب أن يحتوي وسم `div` الافتتاحي على خاصية `aria-label` بقيمة `"Note editor"`.
