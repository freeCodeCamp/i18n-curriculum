...
Aggiungi un `linear-gradient` a `.bb2b` con `--building-color2` dallo `0%` al `6%` e `--window-color2` dal `6%` al `9%`.
...
Dovresti utilizzare un `linear-gradient` per la proprietà `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background, "linear-gradient");
```

Dovresti usare `--building-color2` dallo `0%` al `6%`.
...
Dovresti utilizzare `--window-color2` dal `6%` al `9%`.
...
`.bb2b` dovrebbe avere un `linear-gradient` con `--building-color2` allo `0%` al `6%` e `--window-color2` dal `6%` al `9%`.
