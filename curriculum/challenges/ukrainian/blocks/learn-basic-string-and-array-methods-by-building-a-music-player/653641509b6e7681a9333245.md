...
Вам потрібно оновити поточну пісню, що відтворюється, а також зовнішній вигляд елемента `playButton`.

Призначте `song` властивості `currentSong` об’єкта `userData`.

*Примітка*: На цьому кроці не слід використовувати оператор опціонального ланцюжка `?.`, оскільки `userData.currentSong` наразі не буде `null` або `undefined`.

# --hints--

Ви повинні призначити `song` `userData.currentSong`.
