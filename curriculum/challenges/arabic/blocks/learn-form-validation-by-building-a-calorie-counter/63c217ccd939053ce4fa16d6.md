...
بعد عنصر `label` الخاص بك، وعلى سطر جديد في سلسلة القالب، أنشئ عنصر `input`. أعطه خاصية `type` بقيمة `text`، وخاصية `placeholder` بقيمة `Name`، وخاصية `id` تطابق خاصية `for` في عنصر `label` الخاص بك.

# --hints--

لا يجب تعديل عنصر `label` الخاص بك.

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>Entry\s\$\{entryNumber\}\sName<\/label>/);
```

يجب إضافة عنصر `input` على سطر جديد.

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>Entry\s\$\{entryNumber\}\sName<\/label>\n\s*<input/);
```

يجب أن يحتوي عنصر `input` على خاصية `type` بقيمة `text`.
...
يجب أن يحتوي عنصر `input` على خاصية `placeholder` بقيمة `Name`.
...
يجب أن يحتوي عنصر `input` على خاصية `id` بقيمة `${entryDropdown.value}-${entryNumber}-name`.
...
يجب أن يحتوي عنصر `input` على علامة الإغلاق `>`.
