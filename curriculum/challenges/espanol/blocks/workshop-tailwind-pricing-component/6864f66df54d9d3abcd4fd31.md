...
Para el `span` dentro del elemento `p` que tiene el texto `/month`, dale las clases `text-base`, `font-medium` y `text-gray-500`. Esto lo distinguirá del precio en sí al hacer que el texto sea más claro.

# --hints--

Tu elemento `span` debe tener la clase `text-base`.
...
Tu elemento `span` debe tener la clase `font-medium`.
...
Tu elemento `span` debe tener la clase `text-gray-500`.
