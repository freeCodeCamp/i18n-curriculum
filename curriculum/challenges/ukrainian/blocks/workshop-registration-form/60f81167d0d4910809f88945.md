...
Перший елемент `input` з `type` зі значенням `submit` автоматично встановлюється для надсилання свого найближчого батьківського елемента `form`.
...
Надайте елементу `input` атрибут `type` зі значенням `submit`.

```js
assert.exists(document.querySelector('fieldset + input[type="submit"]'));
```

Надайте елементу `input` атрибут `value` зі значенням `Submit`.
