...
在 `.penguin-body` 元素中，添加两个 `div` 元素，每个元素的 `class` 为 `foot`。 给第一个 `.foot` 一个 `class` 为 `left`，给第二个 `.foot` 一个 `class` 为 `right`。
...
你应该给第一个新的 `div` 一个 `class` 为 `foot`。

```js
assert.exists(document.querySelector('.penguin-body > div.foot'));
```

你应该给第二个新的 `div` 一个 `class` 为 `foot`。
