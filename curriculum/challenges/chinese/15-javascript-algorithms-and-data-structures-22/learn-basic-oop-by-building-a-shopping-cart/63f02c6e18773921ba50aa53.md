...
鉴于计算机存储和处理数字的方式，涉及小数的计算可能会出现一些奇怪的行为。 例如，`0.1 + 0.2` 不等于 `0.3`。 这是因为计算机将十进制数存储为二进制分数，而某些二进制分数不能精确地表示为十进制分数。

我们想清理你计算的数字结果。 将你的计算括在括号中（不要包含 `return` 语句！）并在其上调用 `.toFixed()` 方法。 将数字 `2` 作为参数传递给 `.toFixed()` 方法。 这会将数字四舍五入到小数点后两位并返回一个字符串。
