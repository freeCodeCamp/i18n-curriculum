...
Усередині вашої функції `forumCategory` створіть нову змінну `let` з іменем `selectedCategory` і призначте їй порожній об’єкт. Він буде використовуватися для збереження назви категорії та імені класу для кожної категорії.

# --hints--

Ви повинні використати `let` для оголошення змінної з іменем `selectedCategory`.

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

Ви повинні призначити порожній об’єкт змінній `selectedCategory`.
