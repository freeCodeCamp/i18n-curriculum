...
Agora que você tem a lista de músicas exibida na tela, seria bom ordená-las em ordem alfabética pelo título.

Comece criando uma função arrow chamada `sortSongs`. 

# --hints--

Você deve ter uma função chamada `sortSongs`.

```js
assert.isFunction(sortSongs);
```

Sua função `sortSongs` deve usar a sintaxe de função arrow.
