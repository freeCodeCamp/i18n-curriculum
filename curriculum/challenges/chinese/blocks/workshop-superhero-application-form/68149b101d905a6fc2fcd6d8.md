...
在解构的 `value` 和 `checked` 下面，更新已选中能力的列表。

调用 `setPowers` 函数，该函数接受一个数组参数。使用三元操作符查看 `checked` 是否为真。如果为真，将现有的 `powers` 数组展开到一个新数组中并添加 `value`。如果不为真，则使用 `powers.filter(p => p !== value)` 从 `powers` 中过滤掉 `value`。
