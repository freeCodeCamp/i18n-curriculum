...
现在，创建一个名为 `getPreviousSong` 的函数。让它返回在 `userData.songs` 中位于 `getCurrentSongIndex()` 减一位置的歌曲，如果当前歌曲是播放列表中的第一首，则返回 `undefined`。

# --hints--

你应该有一个名为 `getPreviousSong` 的函数。

```js
assert.isFunction(getPreviousSong);
```

你的 `getPreviousSong` 函数应调用 `getCurrentSongIndex`。
...
你的 `getPreviousSong` 函数应返回当前歌曲之前的歌曲。
...
当当前歌曲是播放列表中的第一首时，你的 `getPreviousSong` 函数应返回 `undefined`。
