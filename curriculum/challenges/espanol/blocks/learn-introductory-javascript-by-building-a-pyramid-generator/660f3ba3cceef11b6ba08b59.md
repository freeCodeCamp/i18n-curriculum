...
Tu bucle necesita ser actualizado para ejecutarse cuando `i` también sea `8`. Mirando tu lógica, esto significa que tu bucle debería ejecutarse cuando `i` es <dfn>menor o igual que</dfn> `count`.  You can use the less than or equal to operator `<=` for this.
