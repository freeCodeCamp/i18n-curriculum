...
では、データを画面上に表示していきましょう。

まず、`showLatestPosts` と言う名前のアロー関数を作成して、仮引数 `data` を取るようにしてください。

# --hints--

`showLatestPosts` というアロー関数が必要です。

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

`showLatestPosts` 関数に仮引数 `data` を持たせてください。

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

`showLatestPosts` を定義してください。

```js
assert.isDefined(showLatestPosts);
```

`showLatestPosts` は関数でなければなりません。
