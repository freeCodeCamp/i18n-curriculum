...
`h2` 요소 다음에 `p` 요소 두 개를 만드세요. 첫 번째 요소에는 `class`의 `dessert-price`를 주고, 텍스트는 달러 기호 `"$"` 다음에 `price` 변수의 값을 넣으세요. 두 번째 요소에는 `class`의 `product-category`를 주고, 텍스트는 `"Category: "` 다음에 `category` 변수의 값을 넣으세요.
