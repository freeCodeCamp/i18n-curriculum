...
新しい`.left-container p`セレクターを作成し、上下のマージンを`-5px`に、左右のマージンを`-2px`に設定してください。また、`font-size`を`2em`に、`font-weight`を`700`に設定してください。

# --hints--

新しい`.left-container p`セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

新しい`.left-container p`セレクターには、`margin`プロパティが`-5px -2px`に設定されているはずです。
...
新しい`.left-container p`セレクターには、`font-size`プロパティが`2em`に設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

新しい`.left-container p`セレクターには、`font-weight`プロパティが`700`に設定されているはずです。
