...
在刚刚添加的渐变下面添加另一个 `repeating-linear-gradient`。 给它一个 `90deg` 的方向，从 `0%` 到 `12%` 使用其建筑颜色，从 `12%` 到`44%` 使用窗户颜色。 这将形成一些矩形窗户。

# --hints--

你应该在 `background` 属性中为 `.fb5` 添加第二个 `repeating-linear-gradient`。
...
你应该给第二个 `repeating-linear-gradient` 一个 `90deg` 的方向。
...
你应该将从 `0%` 到 `12%` 的 `--building-color2` 作为第二个 `repeating-linear-gradient` 的第一个颜色。
...
你应该将从 `12%` 到 `44%` 的 `--window-color2` 作为第二个 `repeating-linear-gradient` 的第二个颜色。
