...
`addCharacters` inarudisha kitendakazi, kwa hivyo utataka kuitumia wito. Pitia `char1` kama hoja.

# --hints--

Unapaswa kuita kitendakazi chako cha `addCharacters()` katika njia yako ya `.map()`.
...
Unapaswa kupitisha `char1` kama hoja kwa wito wako wa `addCharacters()`.
