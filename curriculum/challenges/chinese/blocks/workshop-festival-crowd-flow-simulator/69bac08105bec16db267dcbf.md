...
接下来，将以下消息记录到控制台：`gate.queue[tickIndex] + " attendees arriving."`。

# --hints--

你的 `handleGateAtTick` 函数应该将以下消息记录到控制台：`gate.queue[tickIndex] + " attendees arriving."`。
...
你的 `handleGateAtTick` 函数应该根据给定 tickIndex 时门口的队列记录正确的参与人数。
