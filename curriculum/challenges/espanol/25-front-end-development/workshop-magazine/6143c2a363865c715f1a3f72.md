...
Dentro del elemento `.image-wrapper`, dale a tu primer elemento `img` un `src` de `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, un `alt` de `imagen del proyecto de la máquina de citas`, una `class` de `image-1`, un atributo `loading` establecido a `lazy`, un atributo `width` de `600`, y un atributo `height` de `400`.

# --hints--

Tu primer elemento `img` debe tener un atributo `src` establecido a `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`.
...
Tu primer elemento `img` debe tener un atributo `alt` establecido a `imagen del proyecto de la máquina de citas`.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('alt'), 'image of the quote machine project');
```

Tu primer elemento `img` debe tener un atributo `class` establecido a `image-1`.

```js
assert.isTrue(document.querySelectorAll('.image-wrapper img')?.[0]?.classList?.contains('image-1'));
```

Tu primer elemento `img` debe tener un atributo `loading` establecido a `lazy`.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('loading'), 'lazy');
```

Tu primer elemento `img` debe tener un atributo `width` establecido a `600`.

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('width'), '600');
```

Tu primer elemento `img` debe tener un atributo `height` establecido a `400`.
