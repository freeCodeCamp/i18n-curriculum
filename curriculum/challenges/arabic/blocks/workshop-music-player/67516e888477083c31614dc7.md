...
إذا تحققت عن قرب، سترى أن الأغنية التي تُشغل حاليًا غير مميزة في قائمة التشغيل، لذلك لا تعرف حقًا أي أغنية تُشغل. يمكنك إصلاح ذلك بإنشاء دالة لتحديد أي أغنية يتم تشغيلها.

أنشئ دالة باسم `highlightCurrentSong`. داخل الدالة، تحقق من وجود عنصر `.playlist-song` يحمل الخاصية `aria-current="true"`. إذا وُجد، استخدم الطريقة `removeAttribute` لإزالة الخاصية `aria-current` منه.

# --hints--

يجب أن يكون لديك دالة باسم `highlightCurrentSong`.

```js
assert.isFunction(highlightCurrentSong)
```

يجب أن تستخدم دالة `highlightCurrentSong` الطريقة `removeAttribute` لإزالة الخاصية `aria-current` من عنصر `.playlist-song` الذي يحمل الخاصية `aria-current="true"` إذا كان هذا العنصر موجودًا.
