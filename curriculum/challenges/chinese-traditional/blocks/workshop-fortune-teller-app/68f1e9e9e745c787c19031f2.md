...
在 `if` 述語中，取得 `cardTitle` 元素並將其 `textContent` 設為 `foundCard.name`。對 `description` 元素執行相同操作，將其 `textContent` 設為 `foundCard.desc`。

# --hints--

你應該將 `cardTitle` 的 `textContent` 設為 `foundCard.name`。別忘了使用 `this` 關鍵字。

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

你應該將 `description` 的 `textContent` 設為 `foundCard.desc`。別忘了使用 `this` 關鍵字。
