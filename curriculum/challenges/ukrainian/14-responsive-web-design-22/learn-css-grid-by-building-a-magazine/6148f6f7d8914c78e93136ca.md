...
Create one final `@media` query for `only screen` with a `max-width` of `420px`. Всередині створіть селектор `.hero-title` з властивістю `font-size` зі значенням `4.5rem`.
...
Ваш новий запит `@media` повинен мати селектор `.hero-title`.
