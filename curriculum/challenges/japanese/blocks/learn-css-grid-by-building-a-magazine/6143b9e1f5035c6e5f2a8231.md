...
`ul` 要素の中に、6つの `li` 要素を作成してください。各 `h4` 要素には、`class` が `list-subtitle` に設定された `p` 要素と `li` 要素を追加してください。

次に、`h4` と `p` 要素に、以下のテキスト内容を順番に設定してください。各 `h4` にはコロンの左側の内容を、各 `p` には右側の内容を使います。
...
`ul` 要素には6つの `li` 要素が必要です。

```js
assert(document.querySelectorAll('.lists li')?.length === 6);
```

新しく作成する各 `li` 要素には、`h4` と `p` 要素が含まれている必要があります。
...
最初の `h4` にはテキスト `V1 - 2014` を設定してください。

```js
assert(document.querySelectorAll('.lists li h4')?.[0]?.innerText === 'V1 - 2014');
```

最初の `p` にはテキスト `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.` を設定してください。
...
2番目の `h4` にはテキスト `V2 - 2015` を設定してください。

```js
assert(document.querySelectorAll('.lists li h4')?.[1]?.innerText === 'V2 - 2015');
```

2番目の `p` にはテキスト `We added interactive algorithm challenges.` を設定してください。

```js
assert(document.querySelectorAll('.lists li p')?.[1]?.innerText === 'We added interactive algorithm challenges.');
```

3番目の `h4` にはテキスト `V3 - 2015` を設定してください。

```js
assert(document.querySelectorAll('.lists li h4')?.[2]?.innerText === 'V3 - 2015');
```

3番目の `p` にはテキスト `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).` を設定してください。
...
4番目の `h4` にはテキスト `V4 - 2016` を設定してください。

```js
assert(document.querySelectorAll('.lists li h4')?.[3]?.innerText === 'V4 - 2016');
```

4番目の `p` にはテキスト `We expanded the curriculum to 3 certifications, including Front-End, Back-End, and Data Visualization. They each had 10 required projects, but only the Front-End section had its own challenges. For the other certs, we were still using external resources like Node School.` を設定してください。
...
5番目の `h4` にはテキスト `V5 - 2017` を設定してください。

```js
assert(document.querySelectorAll('.lists li h4')?.[4]?.innerText === 'V5 - 2017');
```

5番目の `p` にはテキスト `We added the back-end and data visualization challenges.` を設定してください。

```js
assert(document.querySelectorAll('.lists li p')?.[4]?.innerText === 'We added the back-end and data visualization challenges.');
```

6番目の `h4` にはテキスト `V6 - 2018` を設定してください。

```js
assert(document.querySelectorAll('.lists li h4')?.[5]?.innerText === 'V6 - 2018');
```

6番目の `p` にはテキスト `We launched 6 new certifications to replace our old ones. This was the biggest curriculum improvement to date.` を設定してください。
...
6つの `h4` 要素には、それぞれクラス `list-subtitle` を付けてください。
