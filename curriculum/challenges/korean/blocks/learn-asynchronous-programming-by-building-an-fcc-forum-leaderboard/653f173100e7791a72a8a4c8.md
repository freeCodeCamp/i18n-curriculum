...
지난 프로젝트에서는 오류를 처리하기 위해 `.catch()` 메서드를 사용했습니다. 여기서는 대신 `try...catch` 문을 사용하세요.

`try` 블록은 잠재적인 오류를 처리하도록 설계되었으며, 오류가 발생할 경우 `catch` 블록 안의 코드가 실행됩니다.
...
`fetchData` 함수 안에 `try...catch` 문을 추가하세요. `catch` 블록에는 `err`라는 이름의 오류 매개변수가 있어야 합니다.

# --hints--

`try` 함수 안에 `fetchData` 블록이 있어야 합니다.

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}/);
```

`catch` 블록 다음에 `try` 블록이 있어야 합니다.

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}\s*catch\s*\(.*\)\s*{[^}]*\s*}/);
```

`catch` 블록에는 `err` 매개변수가 있어야 합니다.
