...
Ili kuita usemi wa kitendakazi cha mshale chenye jina, unaweza kurejea kitendakazi kwa jina lake.

```js
exampleArrowFunction();
```

Chini ya ufafanuzi wako wa kitendakazi `printGreeting`, ita kitendakazi hicho. Fungua konsoli ili kuona matokeo.

# --hints--

Unapaswa kuita kitendakazi chako cha `printGreeting`.

```js
assert.match(code, /printGreeting\(\);?/);
```

Wito wako wa kitendakazi cha `printGreeting` unapaswa kuwa chini ya ufafanuzi wa kitendakazi cha `printGreeting`.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

Wito wako wa kitendakazi cha `printGreeting` unapaswa kutoa `Hello there!` kwenye konsoli.
