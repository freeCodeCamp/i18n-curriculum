...
Sasa, tengeneza kitendakazi kinachoitwa `getNextSong`. Kifanye kirudishe wimbo unaopatikana kwenye `getCurrentSongIndex()` kuongeza moja katika `userData.songs`, au `undefined` ikiwa wimbo wa sasa ni wa mwisho katika orodha ya nyimbo.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `getNextSong`.

```js
assert.isFunction(getNextSong);
```

Kitendakazi chako cha `getNextSong` kinapaswa kuita `getCurrentSongIndex`.

```js
assert.match(getNextSong.toString(), /getCurrentSongIndex\s*\(\s*\)/);
```

Kitendakazi chako cha `getNextSong` kinapaswa kurudisha wimbo unaofuata wimbo wa sasa.
...
Kitendakazi chako cha `getNextSong` kinapaswa kurudisha `undefined` wakati wimbo wa sasa ni wa mwisho katika orodha ya nyimbo.
