...
Zuerst finden Sie die Position eines bestimmten Gates innerhalb des `gates`-Arrays.

Verwenden Sie die `indexOf()`-Methode auf `gates`, um das `currentGate`-Element zu finden, und weisen Sie das Ergebnis einer Variablen namens `currentIndex` zu.

Hier eine Auffrischung, wie das gemacht werden kann:
...
Sie sollten `indexOf()` auf dem `gates`-Array verwenden, um auf den Index von `currentGate` zuzugreifen und ihn einer Variablen namens `currentIndex` zuzuweisen. Dies sollte in einer Zeile erfolgen.
