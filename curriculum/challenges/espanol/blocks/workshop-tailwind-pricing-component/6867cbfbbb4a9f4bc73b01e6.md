...
Los marcadores de los elementos de la lista necesitan un poco de espacio. Entonces, da a cada `span` en los elementos de la lista las clases `text-green-700` y `mr-2`.

Además, agrega el atributo `aria-hidden` a cada uno de los elementos `span`.

# --hints--

Tus elementos `span` deben tener la clase `text-green-700`.
