...
In JSX und TSX übergeben Sie JavaScript-Ausdrücke als Attributwerte mithilfe von geschweiften Klammern. Template-Literale verwenden Ticks und `${}` für die Interpolation. Zum Beispiel:

```tsx
<div className={`prefix-${someVariable}`}>
```

Der Karten-Wrapper `className` ist derzeit fest als `card-wrapper tier-gold` codiert. Ersetzen Sie `gold` durch einen dynamischen Wert, indem Sie ein Template-Literal verwenden, das `getPlayerTier` mit `player.overallRating` aufruft. Dadurch wird die korrekte Tier-Klasse aus dem CSS referenziert, basierend auf der Gesamtbewertung des Spielers.

# --hints--

Sie sollten ein Template-Literal mit `getPlayerTier(player.overallRating)` für das `className` von `.card-wrapper` verwenden.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

Das `.card-wrapper`-Element sollte die `tier-elite`-Klasse haben, wenn die Bewertung des Spielers 98 beträgt.
