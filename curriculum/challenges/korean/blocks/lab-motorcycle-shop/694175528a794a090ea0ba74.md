...
이 실습에서는 모든 HTML과 CSS가 제공되었습니다. TypeScript를 사용해 오토바이 가게 페이지의 기능을 완성하세요.

**목표:** 아래 사용자 스토리를 충족하고 모든 테스트를 통과해 실습을 완료하세요.

**사용자 스토리:**

1. 다음 가능한 값들로 구성된 `type`라는 `Category`을 생성해야 합니다: `'Sport'`, `'Cruiser'`, `'Touring'`, `'Dirt'`, `'Adventure'`, `'Naked'`, `'Electric'`.
2. 다음 속성을 가진 `interface`라는 `Motorcycle`을 생성해야 합니다: `id` 타입 `string`, `name` 타입 `string`, `manufacturer` 타입 `string`, `category` 타입 `Category`, `price` 타입 `number`, `image_url` 타입 `string`, `created_at` 속성 타입 `Date`, `description` 타입 `string`, 그리고 year 타입 `number`.
3. `fetchMotorcycles`라는 이름의 함수를 생성해야 하며, `Promise` 객체 배열을 반환하는 `Motorcycle`를 반환해야 합니다.
4. `Motorcycle` 엔드포인트에서 `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json` 데이터를 불러와야 합니다.
5. `renderMotorcycleCard`라는 이름의 함수를 생성해야 하며, 단일 `Motorcycle` 객체를 매개변수로 받아 오토바이 카드의 형식화된 HTML 문자열을 포함하는 문자열을 반환해야 합니다.
   1. `renderMotorCycleCard` 함수는 `motorcycle` 타입의 `Motorcycle`이라는 매개변수를 받아야 합니다.
   2. `renderMotorCycleCard` 함수는 `string` 타입을 반환해야 합니다.
   3. 각 오토바이 카드 컴포넌트는 유효한 이미지와 `img` 클래스를 가진 `motorcycle-card-image-container` 요소를 가져야 합니다.
   4. 각 오토바이 카드 컴포넌트는 연도를 일반 텍스트로 나열하는 `motorcycle-card-year-badge` 클래스를 가진 요소를 가져야 합니다.
   5. 각 오토바이 카드 컴포넌트는 오토바이 이름을 일반 텍스트로 나열하는 `motorcycle-card-title` 클래스를 가진 요소를 가져야 합니다.
   6. 각 오토바이 카드 컴포넌트는 오토바이 제조사 이름을 일반 텍스트로 나열하는 `motorcycle-card-manufacturer` 클래스를 가진 요소를 가져야 합니다.
   7. 각 오토바이 카드 컴포넌트는 오토바이 카테고리를 일반 텍스트로 나열하는 `motorcycle-card-category` 클래스를 가진 요소를 가져야 합니다.
   8. 각 오토바이 카드 컴포넌트는 오토바이 설명을 일반 텍스트로 나열하는 `motorcycle-card-description` 클래스를 가진 요소를 가져야 합니다.
   9. 각 오토바이 카드 컴포넌트는 오토바이 가격을 일반 텍스트로 나열하는 `motorcycle-card-price` 클래스를 가진 요소를 가져야 합니다.
   10. 각 오토바이 카드 컴포넌트는 각 엔진의 마력을 나열하는 `motorcycle-card-engine` 클래스를 가진 요소를 가져야 합니다.
6. `MotorcycleGalleryApp`라는 클래스를 가져야 합니다.
   1. 이 클래스는 `private` 객체 배열 타입으로 정적 타입이 지정된 `allMotorcycles`라는 `Motorcycle` 배열을 가져야 합니다.
   2. 이 클래스는 `renderMotorcycles` 변수에 해당하는 오토바이 카드 컴포넌트를 `allMotorcycles` 아이디를 가진 요소 안에 렌더링하는 `motorcycle-grid`라는 함수를 가져야 합니다.
7. 표시되는 `Motorcycle` 요소의 수는 `results-number` 아이디를 가진 요소 안에 나열되어야 합니다.
8. 필터는 `input` 이벤트에서 업데이트되어야 합니다.

# --hints--

`type`라는 `Category`가 있어야 합니다.
...
`Category` 타입은 올바른 값들의 유니언으로 설정해야 합니다.
...
`interface`라는 `Motorcycle`가 있어야 합니다.
...
`Motorcycle` 인터페이스에 지정된 모든 속성이 있어야 합니다.
...
`fetchMotorcycles`라는 이름의 함수가 있어야 합니다.

```js
assert.isFunction(fetchMotorcycles);
```

`https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json` 엔드포인트를 사용해야 합니다.
...
`Motorcycle`에서 25개의 `fetchMotorcycles` 객체를 반환하는 프로미스를 반환해야 합니다.
...
`renderMotorcycleCard`라는 이름의 함수가 있어야 합니다.

```js
assert.isFunction(renderMotorcycleCard);
```

`renderMotorcycle` 함수는 `Motorcycle`의 정적 타입 매개변수를 받아야 합니다.
...
`renderMotorcycleCard` 함수는 명시적으로 `string` 타입을 반환해야 합니다.
...
`renderMotorcycleCard` 함수는 `string`를 반환해야 합니다.
...
`MotorcycleGalleryApp`라는 이름의 클래스가 있어야 합니다.
...
`MotorcycleGalleryApp`는 `allMotorcycles`라는 배열을 가져야 합니다.
...
`allMotorcycles` 속성은 `private`이어야 합니다.
...
`allMotorcycles` 속성은 `Motorcycle[]`로 정적 타입이 지정되어야 합니다.
...
`MotorcycleGalleryApp`는 `renderMotorcycles`라는 함수를 가져야 합니다.
...
`renderMotorcycles` 함수는 `motorcycle-card-image-container` 클래스를 가진 25개의 요소를 렌더링해야 합니다.
...
`renderMotorcycles` 함수는 `motorcycle-card-year-badge` 클래스를 가진 25개의 요소를 렌더링해야 합니다.
...
`renderMotorcycles` 함수는 `motorcycle-card-title` 클래스를 가진 25개의 요소를 렌더링해야 합니다.
...
`renderMotorcycles` 함수는 `motorcycle-card-manufacturer` 클래스를 가진 25개의 요소를 렌더링해야 합니다.
...
`renderMotorcycles` 함수는 `motorcycle-card-category` 클래스를 가진 25개의 요소를 렌더링해야 합니다.
...
`renderMotorcycles` 함수는 `motorcycle-card-description` 클래스를 가진 25개의 요소를 렌더링해야 합니다.
...
`renderMotorcycles` 함수는 `motorcycle-card-price` 클래스를 가진 25개의 요소를 렌더링해야 합니다.
...
`renderMotorcycles` 함수는 `motorcycle-card-engine` 클래스를 가진 25개의 요소를 렌더링해야 합니다.
