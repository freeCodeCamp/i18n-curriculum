...
Ora, crea le variabili di stato e i setter per `powerSource` e `powers`. `powerSource` dovrebbe avere un valore iniziale di stringa vuota, e `powers` dovrebbe avere un valore iniziale di un array vuoto.

# --hints--

Dovresti usare la destrutturazione di array per impostare una variabile di stato `powerSource` e un setter `setPowerSource`.

```js
assert.match(code, /(const|let|var)\s+\[\s*powerSource\s*,\s*setPowerSource\s*\]/);
```

Il tuo `powerSource` e `setPowerSource` dovrebbero usare il hook `useState`.
...
Il tuo hook `useState` per `powerSource` dovrebbe avere un valore iniziale di stringa vuota.
...
Dovresti usare la destrutturazione di array per impostare una variabile di stato `powers` e un setter `setPowers`.

```js
assert.match(code, /(const|let|var)\s+\[\s*powers\s*,\s*setPowers\s*\]/);
```

Il tuo `powers` e `setPowers` dovrebbero usare il hook `useState`.
...
Il tuo hook `useState` per `powers` dovrebbe avere un valore iniziale di array vuoto.
