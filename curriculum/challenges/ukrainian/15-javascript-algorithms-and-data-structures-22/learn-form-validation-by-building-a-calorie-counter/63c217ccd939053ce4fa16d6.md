...
Після елемента `label` та у новому рядку у шаблонному рядку створіть елемент `input`. Надайте йому атрибут `type` зі значенням `text`, атрибут `placeholder` зі значенням `Name` та атрибут `id`, який відповідає атрибуту `for` елемента `label`.

# --hints--

Не змінюйте наявний елемент `label`.

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>Entry\s\$\{entryNumber\}\sName<\/label>/);
```

Ви повинні додати елемент `input` у новому рядку.
