...
用戶做出選擇後，應該能夠保留該分數並點擊 `keepScoreBtn` 進入下一輪。

當該按鈕被點擊時，你應該找到被選中的單選項並獲取其值和值屬性。 如果用戶已選擇一個選項，調用你的函數來更新分數，重置單選項，並將該值和值添加到分數歷史中。

如果用戶未選擇選項，則顯示一個警告提醒他們這樣做。

# --hints--

當你的 `keepScoreBtn` 被點擊時，分數應在 `totalScoreElement` 文本中更新。
...
當你的 `keepScoreBtn` 被點擊時，分數歷史應被更新。
...
當你的 `keepScoreBtn` 被點擊時，未選擇任何選項時，你應向用戶顯示一個警告。
