...
Dentro del elemento de botón en la función `renderSongs`, añade un atributo `onclick`. Para el valor, llama a la función `deleteSong` e interpola `song.id`.

# --hints--

Deberías añadir un atributo `onclick` al botón de eliminar y pasar `"deleteSong(${song.id})"`.
