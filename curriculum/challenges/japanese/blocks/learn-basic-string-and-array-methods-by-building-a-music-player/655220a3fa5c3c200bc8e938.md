...
曲が一時停止されるので、`classList`から`remove()`クラスを`playing`と`playButton`メソッドを使って削除してください。

曲を最終的に一時停止するには、`pause()`変数に対して`audio`メソッドを使います。`pause()`は音楽ファイルを一時停止するためのWeb Audio APIのメソッドです。

# --hints--

既存の`pauseSong`関数とその内容は変更しないでください。

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

`classList`から`remove()`クラスを削除するには、`"playing"`プロパティと`playButton`メソッドを使うべきです。

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

`pause()`変数に対して`audio`メソッドを使うべきです。
