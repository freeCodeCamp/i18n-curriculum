...
当用户点击表单控制或用键盘标签键选择表单时， 这意味着它处于 `focused` 状态。 当输入处于 `focused` 状态时，大多数浏览器都会在输入周围显示蓝色高亮边框。 但你可以选择在 CSS 中添加其他样式。

点击预览窗口中空白区域的任意部分，然后按下 `tab` 密钥以查看焦点状态。要查看预览，你需要启用交互式编辑器。
...
启用交互式编辑器并尝试点击电子邮件 input，你会注意到它将不再获得焦点。
...
启用交互式编辑器并尝试在预览窗口中编辑当前的 `example@email.com` 值，你会注意到这是不可能的。
