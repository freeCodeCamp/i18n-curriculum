...
يجب أن يحتوي عنصر `.eye-lid` على خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.top, '25%');
```

يجب أن يحتوي عنصر `.eye-lid` على خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.left, '-23%');
```

يجب أن يحتوي عنصر `.eye-lid` على خاصية `border-radius` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
