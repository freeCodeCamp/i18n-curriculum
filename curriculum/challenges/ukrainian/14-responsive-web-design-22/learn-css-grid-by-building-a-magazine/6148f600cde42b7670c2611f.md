...
Створіть ще один запит `@media` для `only screen` з `max-width` зі значенням `600px`. Всередині створіть правило `.text-with-images` і надайте йому властивість `grid-template-columns` зі значенням `1fr`.

Це згорне нижню область тексту в один стовпчик на менших екранах.

# --hints--

Ви повинні створити новий запит `@media` для `only screen and (max-width: 600px)`. Він повинен бути нижче вашого попереднього запиту `@media`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

Ваш новий запит `@media` повинен мати селектор `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

Ваш новий селектор `.text-with-images` повинен мати властивість `grid-template-columns` зі значенням `1fr`.
