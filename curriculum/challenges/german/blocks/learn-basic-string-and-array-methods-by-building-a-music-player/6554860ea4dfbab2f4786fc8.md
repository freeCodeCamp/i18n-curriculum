...
Wenn Sie genau hinschauen, sehen Sie, dass der aktuell abgespielte Song in der Wiedergabeliste nicht hervorgehoben ist, sodass Sie nicht wirklich wissen, welcher Song gerade läuft. Sie können das beheben, indem Sie eine Funktion erstellen, die jeden Song hervorhebt, der gerade abgespielt wird.

Erstellen Sie mit einer Pfeilsyntax eine `highlightCurrentSong`-Funktion. Verwenden Sie innerhalb der Funktion `querySelectorAll`, um das `.playlist-song`-Element zu erhalten, und weisen Sie es einer `playlistSongElements`-Konstanten zu.

# --hints--

Sie sollten `const` verwenden, um eine `highlightCurrentSong`-Funktion zu erstellen.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` sollte eine Funktion sein.

```js
assert.isFunction(highlightCurrentSong)
```

Ihre `highlightCurrentSong`-Funktion sollte eine Pfeilsyntax verwenden.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

Sie sollten eine `playlistSongElements`-Konstante innerhalb Ihrer `highlightCurrentSong`-Funktion erstellen.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

Sie sollten die `querySelectorAll()`-Methode verwenden, um das `.playlist-song`-Element auszuwählen und es der `playlistSongElements`-Konstanten zuzuweisen.
