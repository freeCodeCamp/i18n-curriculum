...
أخيرًا، ستحتاج إلى إضافة بعض الأنماط لأزرارك. ابدأ بتعيين خاصية `cursor` إلى `pointer`. ثم عيّن لون النص `color` إلى `#0a0a23` ولون الخلفية `background-color` إلى `#feac32`.

بعدها عيّن خاصية `background-image` إلى `linear-gradient(#fecc4c, #ffac33)`. وأخيرًا، عيّن الحدود `border` إلى `3px solid #feac32`.

# --hints--

يجب أن يكون لديك محدِّد `button`.
...
يجب أن يحتوي محدِّد `button` على خاصية `cursor` مضبوطة على `pointer`.
...
يجب أن يحتوي محدِّد `button` على خاصية `color` مضبوطة على `#0a0a23`.
...
يجب أن يحتوي محدِّد `button` على خاصية `background-color` مضبوطة على `#feac32`.
...
يجب أن يحتوي محدِّد `button` على خاصية `background-image` مضبوطة على `linear-gradient(#fecc4c, #ffac33)`.
...
يجب أن يحتوي محدِّد `button` على خاصية `border` مضبوطة على `3px solid #feac32`.
