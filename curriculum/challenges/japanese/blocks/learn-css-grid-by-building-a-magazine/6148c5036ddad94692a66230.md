...
新しいアイコン要素を追加すると、自動配置アルゴリズムが動作します。ただし、アルゴリズムは新しい列の幅をデフォルトで`auto`に設定するため、現在の列幅とは一致しません。

`grid-auto-columns`プロパティでこれをオーバーライドできます。`.social-icons`セレクターに`grid-auto-columns`プロパティを設定し、値を`1fr`にしてください。

# --hints--

`.social-icons`セレクターに`grid-auto-columns`プロパティを設定し、値を`1fr`にしてください。
