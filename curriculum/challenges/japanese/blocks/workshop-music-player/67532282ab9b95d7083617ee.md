...
`playNextSong` 関数の中で、プレイリストが終了したときにプレイヤーを正しく更新するために、`setPlayerDisplay`、`highlightCurrentSong`、および `setPlayButtonAccessibleText` 関数を呼び出してください。

# --hints--

`setPlayerDisplay` 関数の `else` 文の中で `playNextSong` を呼び出す必要があります。
...
`highlightCurrentSong` 関数の `else` 文の中で `playNextSong` を呼び出す必要があります。
...
`setPlayButtonAccessibleText` 関数の `else` 文の中で `playNextSong` を呼び出す必要があります。
