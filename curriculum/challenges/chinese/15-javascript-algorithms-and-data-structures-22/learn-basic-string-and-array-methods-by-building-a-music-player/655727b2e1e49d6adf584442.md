...
如果播放列表为空，你需要创建一个 `resetButton` 元素并为其创建文本。 只有在播放列表为空的情况下，该按钮才会显示。

`createElement()` 是一种 DOM 方法，可用于使用 JavaScript 动态创建元素。 要使用 `createElement()`，你需要调用它，然后将标签名称作为字符串传入：
...
在你的 `if` 语句中，声明一个 `resetButton` 常量，然后使用 `createElement()` 创建一个 `"button"`。
...
你应该使用 `createElement` 方法创建一个 `"button"`。

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

你应该把新创建的 `button` 元素分配给 `resetButton` 常量。
