...
`useMemo()` は2つの引数を取ります：メモ化したい値を返す関数と、依存関係の配列です。メモ化された値は、依存関係の1つ以上が変わったときにのみ再計算されます。

`useMemo()` の基本的な構文は次の通りです：
...
`filteredItems` を `useMemo()` に等しく設定し、波括弧付きの無名関数を渡してください。波括弧の中で、`return` 文を使って既存のフィルタリングロジックを明示的に返してください。また、依存関係の配列には `query` を唯一の依存関係として追加してください。これにより、`query` が変わったときだけフィルタリング処理が再実行されます。

# --hints--

`filteredItems` を `useMemo()` フックの呼び出しに等しく設定してください。
...
`return` 文を使って既存のフィルタリングロジックを明示的に返してください。
...
依存関係の配列には `query` を唯一の依存関係として追加してください。
