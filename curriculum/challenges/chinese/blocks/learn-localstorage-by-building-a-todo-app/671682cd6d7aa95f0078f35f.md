...
现在该处理最后一个问题了。 如果待办任务的名称或描述中包含特殊字符（例如引号），则应用程序会中断。 尽管看起来并非如此，但正确的项目似乎丢失了。  

为了解决这个问题，我们需要创建一个名为 `removeSpecialChars` 的函数，该函数以字符串作为输入并删除所有特殊字符。 

# --hints--

你应该定义一个 `removeSpecialChars` 函数。

```js
assert.isFunction(removeSpecialChars);
```

你的 `removeSpecialChars` 不应删除空格。
