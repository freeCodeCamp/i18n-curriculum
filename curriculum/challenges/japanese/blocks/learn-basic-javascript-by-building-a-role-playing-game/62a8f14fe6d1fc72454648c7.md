...
モンスターが倒された後、そのモンスターのステータスボックスは表示されなくなります。

`update` 関数の最初の行で、`monsterStats.style.display` を使って `display` の値を `none` に変更してください。

# --hints--

`monsterStats.style.display` 関数内で `update` を使うべきです。
