...
Come ricordi dalle lezioni precedenti, puoi usare il metodo `map` per iterare su diversi oggetti e renderizzare i componenti dinamicamente in questo modo:
...
Usa il metodo `map()` per renderizzare un singolo `Card` dall'array. Passa `name`, `title` e `bio` come props al componente `Card`.

# --hints--

Dovresti usare il metodo `map()` per iterare su `profiles` array.

```js
assert.match(code, /profiles\.map\s*\(/);
```

Dovresti passare le props `name`, `title` e `bio` dall'oggetto `profile`.
