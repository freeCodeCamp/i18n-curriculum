...
Deine neue Checkbox sollte ein `id`-Attribut mit dem Wert `lazy` und ein `name`-Attribut mit dem Wert `personality` enthalten. Überprüfe, ob ein Leerzeichen nach dem Namen des öffnenden Tags und/oder Leerzeichen vor allen Attributnamen vorhanden sind.
