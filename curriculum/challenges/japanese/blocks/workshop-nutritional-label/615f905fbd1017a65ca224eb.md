...
`.daily-value` 要素の後にミディアム区切りを追加してください。その新しい区切りの下に、`p` 属性が `class` に設定された `note` 要素を作成してください。
...
`div` 要素の後に新しい `.daily-value` を作成する必要があります。

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

新しい `div` は `class` が `divider medium` に設定されている必要があります。
...
新しい `p` 要素の後に `div` 要素を作成してください。

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'p');
```

新しい `p` 要素は `class` が `note` に設定されている必要があります。
