...
قبل تشغيل الأغنية، تأكد من أنها تبدأ من البداية. يمكن تحقيق ذلك باستخدام خاصية `currentTime` على كائن `audio`.

أضف جملة `if` للتحقق مما إذا كانت `userData?.currentSong` غير صحيحة *أو* إذا كانت `userData?.currentSong.id` لا تساوي تمامًا `song.id`. هذا الشرط يتحقق مما إذا لم تكن هناك أغنية حالية تُشغل أو إذا كانت الأغنية الحالية مختلفة عن التي ستُشغل.

داخل كتلة `if`، عيّن خاصية `currentTime` لكائن `audio` إلى `0`.

# --hints--

يجب أن تنشئ جملة `if` بالشرط `userData?.currentSong === null || userData?.currentSong.id !== song.id`. لا تنسَ تضمين التسلسل الاختياري.
...
يجب أن تعيّن `audio.currentTime` إلى `0` داخل كتلة `if` الخاصة بك.
