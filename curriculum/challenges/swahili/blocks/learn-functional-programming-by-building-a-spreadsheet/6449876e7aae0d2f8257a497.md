...
Tangaza kitendakazi cha mshale `rangeFromString` kinachopokea vigezo viwili, `num1` na `num2`. Kitendakazi kinapaswa kurudisha kwa njia isiyo wazi matokeo ya wito wa `range` kwa `num1` na `num2` kama hoja.

Ili kuwa salama, tafsiri `num1` na `num2` kuwa nambari kamili unazowapasa kuzipeleka ndani ya `range`.
...
Unapaswa kutumia `const` kutangaza tofauti `rangeFromString` yako.
...
Tofauti `rangeFromString` yako inapaswa kuwa kitendakazi cha mshale.
...
Kitendakazi `rangeFromString` chako kinapaswa kuwa na `num1` kama kigezo cha kwanza.
...
Kitendakazi `rangeFromString` chako kinapaswa kuwa na `num2` kama kigezo cha pili.
...
Kitendakazi `rangeFromString` chako kinapaswa kutumia kurudisha kwa njia isiyo wazi.
...
Kitendakazi `rangeFromString` chako kinapaswa kurudisha matokeo ya wito wa kitendakazi `range` chako.
...
Unapaswa kuita `parseInt` kwa `num1` kama hoja na kupitisha matokeo kwa wito wa `range`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

Unapaswa kuita `parseInt` kwa `num2` kama hoja na kupitisha matokeo kwa wito wa `range`.
