...
最後のステップでは、`"Well, it was nice to talk to you. Have a nice day!"` のボットのメッセージをコンソールにログ出力します。

これで、あなたのあいさつボットは完成です！

# --hints--

コードには `console` 文があるはずです。

```js
assert.lengthOf(__helpers.removeJSComments(code).match(/console\.log(.*)/g), 9);
```

`console` 文は文字列 `"Well, it was nice to talk to you. Have a nice day!"` をログ出力する必要があります。
