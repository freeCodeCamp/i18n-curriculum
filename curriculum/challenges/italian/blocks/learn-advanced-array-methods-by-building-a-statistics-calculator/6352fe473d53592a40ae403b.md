...
All'interno della tua callback `.reduce()` vuota, dichiara una variabile `difference` e impostala al valore di `el` meno `mean`. Poi dichiara una variabile `squared` e impostala al valore di `difference` elevato alla potenza di `2`. Infine, restituisci il valore di `acc` più `squared`.

# --hints--

La tua callback `reduce` dovrebbe avere una variabile `difference`.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

La tua variabile `difference` dovrebbe essere impostata al valore di `el` meno `mean`.
...
La tua callback `reduce` dovrebbe avere una variabile `squared`.
...
La tua variabile `squared` dovrebbe essere impostata al valore di `difference` elevato alla potenza di 2.
...
La tua callback `reduce` dovrebbe restituire il valore di `acc` più `squared`.
...
Non dovresti rimuovere il valore iniziale di `0` dal tuo metodo `.reduce()`.
