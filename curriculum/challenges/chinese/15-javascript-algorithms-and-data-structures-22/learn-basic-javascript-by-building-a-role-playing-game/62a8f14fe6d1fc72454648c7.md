...
在 `update` 函数的第一行，使用 `monsterStats.style.display` 将 `display` 的值更改为 `none`。

# --hints--

你应该在 `update` 函数中使用 `monsterStats.style.display`。

```js
assert.match(update.toString(), /monsterStats\.style\.display/);
```

你应该将 `monsterStats.style.display` 设置为 `none`。

```js
assert.match(update.toString(), /monsterStats\.style\.display\s*=\s*('|")none\1/);
```

你应该在 `update` 函数的第一行添加你的代码。
