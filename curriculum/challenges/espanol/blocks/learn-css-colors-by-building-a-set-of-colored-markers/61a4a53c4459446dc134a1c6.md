...
Ahora que conoces los fundamentos de cómo funcionan la función `linear-gradient` y los puntos de color, puedes utilizarlos para hacer que los marcadores se vean más realistas.

En la función `linear-gradient`, establece `gradientDirection` a `180deg`.

# --hints--

La regla CSS `.red` debe tener una propiedad `background` configurada a `linear-gradient(180deg, rgb(255, 0, 0) 75%, rgb(0, 255, 0), rgb(0, 0, 255))`.
