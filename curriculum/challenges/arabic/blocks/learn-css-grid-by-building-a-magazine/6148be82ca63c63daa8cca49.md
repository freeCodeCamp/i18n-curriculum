...
أنشئ محدِّد `.author` وأعطه خاصية `font-size` بقيمة `2rem` وخاصية `font-family` بقيمة `Raleway` مع النظام البديل `sans-serif`.

# --hints--

يجب أن يكون لديك محدِّد `.author`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author'));
```

يجب أن يحتوي محدِّد `.author` على خاصية `font-size` بقيمة `2rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize === '2rem');
```

يجب أن يحتوي محدِّد `.author` على خاصية `font-family` بقيمة `Raleway` مع النظام البديل `sans-serif`.
