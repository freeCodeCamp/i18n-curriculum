...
Füge ein weiteres Objekt zu deinem `locations` Array hinzu. Set `name` to `"easter egg"`, set `"button text"` to an array with the strings `"2"`, `"8"`, and `"Go to town square?"`, set `"button functions"` to an array with the variables `pickTwo`, `pickEight`, and `goTown`, and `text` to `"You find a secret game. Wähle oben eine Zahl aus. Es werden zufällig zehn Zahlen zwischen 0 und 10 ausgewählt. If the number you choose matches one of the random numbers, you win!"`.
...
Your eighth `locations` value should have a `text` property with the value `"You find a secret game. Wähle oben eine Zahl aus. Es werden zufällig zehn Zahlen zwischen 0 und 10 ausgewählt. If the number you choose matches one of the random numbers, you win!"`.
