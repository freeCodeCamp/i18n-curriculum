...
每個條目都會有一個用於條目名稱的文字輸入，以及一個用於卡路里的數字輸入。若要取得條目數量，可以透過文字輸入進行查詢。

將字串 `input[type="text"]` 傳遞給 `querySelectorAll()` 方法。請記得，如果你對字串使用單引號，字串內也必須使用雙引號（反之亦然）。

這將傳回表單中所有文字輸入的 `NodeList`。你接著可以存取該 `NodeList` 的 `length` 屬性以取得項目數量。請在同一行執行此操作。

# --hints--

你應該將字串 `input[type="text"]` 傳遞給 `querySelectorAll()` 方法。
...
你應該存取 `querySelectorAll()` 的 `length` 屬性。

```js
assert.match(addEntry.toString(), /\.querySelectorAll\(.*\)\.(?=(length))(\1$|\1\s*;)/)
```

你的 `entryNumber` 變數應該是 `querySelectorAll` 的 `length`。
