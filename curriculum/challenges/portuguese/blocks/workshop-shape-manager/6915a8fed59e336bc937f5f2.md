...
O último conjunto de elementos a selecionar são os elementos de resultado. Use sua função `getElement` para obter o elemento com os ids `result-text` e `result-card`. Ambos devem usar `HTMLElement` como tipo.

Lembre-se que você já declarou as variáveis `resultText` e `resultCard` para eles, então só precisa atribuí-las.

# --hints--

Você deve atribuir um valor à variável `resultText`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` deve selecionar o elemento `result-text`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

Você deve usar `HTMLElement` como tipo do `getElement` que está usando para selecionar o elemento `result-text`.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

Você deve atribuir um valor à variável `resultCard`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` deve selecionar o elemento `result-card`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

Você deve usar `HTMLElement` como tipo do `getElement` que está usando para selecionar o elemento `result-card`.
