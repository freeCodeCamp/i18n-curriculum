...
球員照片是從硬編碼的 URL 載入。將 `src` 屬性值 `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` 替換成 `player.imageUrl`。

同時將硬編碼的 `alt` 文字替換成使用 `player.name` 和文字 `headshot` 的模板字串動態值。

請注意，當你在 TSX 中使用 JavaScript 表達式作為屬性值時，要使用大括號 `{}`，而不是引號。例如 `src={player.imageUrl}`，而非 `src="{player.imageUrl}"`。

# --hints--

你應該將硬編碼的圖片 URL 替換成 `player.imageUrl` 作為 `src` 屬性。

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

你應該將硬編碼的 `alt` 文字替換成使用 `player.name` 和文字 `headshot` 的模板字串。
