...
最後に、ボタン同士および上部からの間隔をもっと広くできます。そのためには、両方のボタンを包んでいる`div`要素を対象にする必要があります。そこで、その`flex`に`gap-2`、`mt-6`、`div`のクラスを追加してください。

これでCTAコンポーネントのワークショップは完成です！

# --hints--

アンカー要素を包んでいる`div`要素にはユーティリティクラス`flex`を付けるべきです。
...
アンカー要素を包んでいる`div`要素にはユーティリティクラス`gap-2`を付けるべきです。
...
アンカー要素を包んでいる`div`要素にはユーティリティクラス`mt-6`を付けるべきです。
