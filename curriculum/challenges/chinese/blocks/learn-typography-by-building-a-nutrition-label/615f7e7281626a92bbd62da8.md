...
在最后一个 `p` 元素下方，使用文本 `Sodium 160mg 7%` 创建另一个 `p` 元素。 将 `Sodium` 和 `7%` 分别放在各自的 `span` 中，并像处理其他内容一样，给它们设置 `bold` 类。

然后，在 `Sodium 160mg` 外面添加一个 `span` 元素，以便对齐。
...
`Sodium` 周围的 `span` 应该将 `class` 属性设置为 `bold`。
...
`7%` 周围的 `span` 元素应将 `class` 属性设置为 `bold`。
