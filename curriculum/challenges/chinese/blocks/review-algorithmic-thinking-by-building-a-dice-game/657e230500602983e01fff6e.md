...
在游戏的最后部分，你需要创建一个算法来查看是否存在顺子。小顺子是指四个骰子有连续的数值，顺序可以是任意的（例如，在一次掷骰结果为 `41423` 时，我们有 `1234`），得分为 `30` 分。大顺子是指五个骰子都有连续的数值，顺序可以是任意的（例如，在一次掷骰结果为 `35124` 时，我们有 `12345`），得分为 `40` 分。
