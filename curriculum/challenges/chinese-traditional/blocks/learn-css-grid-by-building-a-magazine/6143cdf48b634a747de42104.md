...
在你的 `.image-wrapper` 元素中，為你的第三個 `img` 元素設定 `src` 為 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`、`alt` 為 `four people working on code`、`loading` 屬性為 `lazy`、`class` 設為 `image-3`、`width` 屬性設為 `600`，以及 `height` 屬性設為 `400`。

# --hints--

你的第三個 `img` 元素的 `src` 應該設定為 `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`。
