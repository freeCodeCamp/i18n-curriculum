...
`checkUserInput` 関数内で、`console.log()` 文を削除してください。それから、`decimalToBinary` 関数を呼び出し、`value` の `numberInput` を引数として渡してください。また、入力を数値に変換するために `parseInt()` 関数を使うことを忘れないでください。

# --hints--

`console.log()` 関数から `checkUserInput` 文を削除してください。

```js
assert.notMatch(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*console\.log\((\s|.)*\)/);
```

`decimalToBinary` 関数の本体内で `if` 文の後に `checkUserInput` 関数を呼び出してください。

```js
assert.match(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*decimalToBinary\(/);
```

`parseInt()` の `value` を数値に変換するために `numberInput` を使い、それを引数として `decimalToBinary` 関数に渡してください。
