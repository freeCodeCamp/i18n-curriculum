...
Tengeneza kitendakazi kinachoitwa `isSelect` kinachopokea kigezo kimoja `element` cha aina `EventTarget | null`. Kitendakazi hiki kitakagua kama kipengele kilichochaguliwa ni kipengele cha HTML cha `<select>`.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `isSelect`.
...
Kitendakazi chako cha `isSelect` kinapaswa kuwa na kigezo kinachoitwa `element` cha aina `EventTarget | null`.
...
Kitendakazi chako cha `isSelect` kinapaswa kuwa na aina ya kurudisha `element is HTMLSelectElement`.
