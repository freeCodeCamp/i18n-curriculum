...
現在，你將使用 `grid` 來排列外覆 `div` 內的項目。

為你的 `div` 外覆添加類別 `grid`、`grid-rows-[1fr_auto]` 和 `gap-6`。

# --hints--

你的家庭方案的外覆 `div` 元素應該有 `grid` 類別。
...
你的家庭方案的外覆 `div` 元素應該具有類別 `grid-rows-[1fr_auto]`。
