...
Виберіть лише елементи `.inline` і надайте їм `width` зі значенням `unset`. Це скасує попереднє правило, яке встановлювало для всіх елементів `input` значення `width: 100%`.

# --hints--

Ви повинні використати селектор `.inline`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.inline'));
```

Ви повинні надати елементам `.inline` `width` зі значенням `unset`.
