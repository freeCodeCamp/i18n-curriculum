...
Your `head` element should have an opening tag. Öffnende Tags haben die folgende Syntax: `<elementName>`.

```js
assert(code.match(/\<head\>/));
```

Your `head` element should have a closing tag. Abschließende Tags haben ein `/` genau nach dem `<`-Zeichen.
