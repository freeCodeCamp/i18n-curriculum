...
將帶有愛心表情符號的 `div` 從其父元素的頂部定位在 `22.5px`，從左側定位在 `12px`。

# --hints--

你應該給 `.shirt div` 一個 `top` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt div')?.top, '22.5px');
```

你應該給 `.shirt div` 一個 `left` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。
