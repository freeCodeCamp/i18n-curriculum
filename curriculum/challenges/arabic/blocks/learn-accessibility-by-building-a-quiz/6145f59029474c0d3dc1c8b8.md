...
كما في عناصر `input` و `label` الأخرى، اربط `textarea` بعنصر `label` المقابل له، وأعطه خاصية `name`.

# --hints--

يجب أن تعطي عنصر `textarea` خاصية `id`.

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id);
```

لا يمكنك تضمين مسافات في خاصية `id` لعنصر `textarea`.

```js
assert.match(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id, /^\S+$/);
```

يجب أن تختار `id` مختلفًا لعنصر `textarea` لأن عنصرًا آخر يستخدم نفس الـ `id`.
...
يجب أن تعطي عنصر `label` خاصية `for` تطابق `id` الخاص بعنصر `textarea`.
...
يجب أن تعطي عنصر `textarea` خاصية `name`.

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.name);
```

يجب أن تستخدم على الأقل حرفًا واحدًا غير مسافة في خاصية `name` لعنصر `textarea`.
