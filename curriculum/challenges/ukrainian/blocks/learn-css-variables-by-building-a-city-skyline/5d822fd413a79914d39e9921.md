...
Для наступної будівлі вкладіть чотири елементи `div` всередину `.fb3` з класами `fb3a`, `fb3b`, знову `fb3a` і знову `fb3b` у такому порядку. Ця будівля матиме чотири розділи, і верхні два будуть майже такими ж, як і нижні два.

# --hints--

Ви повинні додати чотири елементи `div` всередину `.fb3`.

```js
assert.equal(document.querySelectorAll("div.fb3 > div")?.length, 4);
```

Першому новому `div` слід додати клас `fb3a`.

```js
assert.equal(document.querySelector("div.fb3").firstElementChild, document.querySelector("div.fb3a"));
```

Другому новому `div` слід додати клас `fb3b`.

```js
assert.equal(document.querySelector("div.fb3 :nth-child(2)"), document.querySelector("div.fb3b"));
```

Третьому новому `div` слід додати клас `fb3a`.

```js
assert.equal(document.querySelector("div.fb3 :nth-child(3)"), document.querySelector("div.fb3b + div.fb3a"));
```

Четвертому новому `div` слід додати клас `fb3b`.
