...
`remove()` を使ってタスクをDOMから削除し、`taskData` を使って `splice()` 配列からも削除する必要があります。

`splice()` は、指定したインデックスで要素を削除、置換、または追加し、削除した要素を返す配列メソッドです。最大で3つの引数を取ることができ、最初の引数は開始する必須のインデックス、2番目は削除するアイテムの数、3番目はオプションの置換要素です。以下は例です。
...
`remove()` メソッドを使って、DOMから `parentElement` の `buttonEl` を削除してください。その後、`splice()` を使って `taskData` 配列からタスクを削除します。`dataArrIndex` の引数として `1` と `splice()` を渡してください。

`dataArrIndex` は開始するインデックスで、`1` は削除するアイテムの数です。

# --hints--

`dataArrIndex` 文は変更しないでください。
