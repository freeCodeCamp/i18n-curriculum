...
Debes eliminar los argumentos `gradientDirection` de las funciones `linear-gradient` en las reglas CSS `.red` y `.green`.

```js
assert(!code.match(/linear-gradient\s*\(\s*180deg/gi));
```

La regla CSS `.red` debe tener la propiedad `background` establecida a `linear-gradient(rgb(122, 74, 14), rgb(245, 62, 113), rgb(162, 27, 27))`.
...
La regla CSS `.green` debe tener la propiedad `background` establecida a `linear-gradient(#55680D, #71F53E, #116C31)`.
