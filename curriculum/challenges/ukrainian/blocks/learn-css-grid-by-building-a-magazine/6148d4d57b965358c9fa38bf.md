...
Тепер настав час стилізувати ваш третій `section`. Зверніть увагу, що він має значення `text` і `text-with-images` для атрибута `class`, що означає, що він уже успадковує стилі з вашого правила `.text`.

Створіть селектор `.text-with-images` і встановіть властивість `display` у значення `grid`.

# --hints--

У вас має бути селектор `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

Ваш селектор `.text-with-images` повинен мати властивість `display`, встановлену у значення `grid`.
