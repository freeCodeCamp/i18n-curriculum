...
Zum Schluss können die Buttons besser voneinander und vom oberen Rand getrennt werden. Dazu müssen Sie das `div`-Element ansprechen, das beide umschließt. Fügen Sie also die Klassen `flex`, `gap-2` und `mt-6` zu diesem `div` hinzu.

Damit ist Ihr Workshop zur CTA-Komponente abgeschlossen!

# --hints--

Das `div`-Element, das die Ankerelemente umschließt, sollte die Utility-Klasse `flex` haben.
...
Das `div`-Element, das die Ankerelemente umschließt, sollte die Utility-Klasse `gap-2` haben.
...
Das `div`-Element, das die Ankerelemente umschließt, sollte die Utility-Klasse `mt-6` haben.
