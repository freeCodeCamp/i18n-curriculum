...
`.fb2a`에 `border-bottom`의 `10vh solid var(--building-color3)`과 `border-left`의 `border-right` 및 `1vw solid transparent`를 추가하세요. 이번에는 테두리 트릭으로 사다리꼴 모양을 만들 것입니다.

# --hints--

`.fb2a`에 `border-bottom`의 `10vh solid var(--building-color3)`를 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderBottom.trim(), "10vh solid var(--building-color3)");
```

`.fb2a`에 `border-left`의 `1vw solid transparent`를 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderLeft, "1vw solid transparent");
```

`.fb2a`에 `border-right`의 `1vw solid transparent`를 지정하세요.
