...
이제 `getNextSong`라는 함수를 만드세요. 이 함수는 `getCurrentSongIndex()`에서 `userData.songs`보다 하나 더 큰 위치에 있는 노래를 반환하거나, 현재 노래가 재생 목록의 마지막 노래라면 `undefined`을 반환하게 하세요.

# --hints--

`getNextSong`라는 함수가 있어야 합니다.

```js
assert.isFunction(getNextSong);
```

`getNextSong` 함수는 `getCurrentSongIndex`를 호출해야 합니다.
...
`getNextSong` 함수는 현재 노래 다음에 있는 노래를 반환해야 합니다.
...
`getNextSong` 함수는 현재 노래가 재생 목록의 마지막 노래일 때 `undefined`를 반환해야 합니다.
