...
`parseInt(numberInput.value)` のすべてのインスタンスを `inputInt` に置き換えてください。

# --hints--

`isNaN(parseInt(numberInput.value))` を `isNaN(inputInt)` に、`parseInt(numberInput.value) < 0` を `inputInt < 0` に置き換える必要があります。
...
`parseInt(numberInput.value) === 5` を `inputInt === 5` に置き換えてください。
...
`decimalToBinary(parseInt(numberInput.value))` を `decimalToBinary(inputInt)` に置き換えてください。
...
`checkUserInput()` 関数には `parseInt(numberInput.value)` のインスタンスが1つだけ含まれている必要があります。
