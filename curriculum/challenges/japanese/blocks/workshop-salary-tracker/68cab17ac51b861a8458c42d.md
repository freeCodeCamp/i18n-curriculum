...
ついに、`salary` セッターを完成させたので、`level` セッター内でそれを呼び出すことができます。

`self._salary = Employee._base_salaries[new_level]` の行を修正して、`salary` 属性を直接変更するのではなく、`_salary` セッターを呼び出すようにしてください。

これで給与トラッカーのワークショップは完了です。

# --hints--

`self.salary` セッター内で `Employee._base_salaries[new_level]` を `level` に設定する必要があります。
