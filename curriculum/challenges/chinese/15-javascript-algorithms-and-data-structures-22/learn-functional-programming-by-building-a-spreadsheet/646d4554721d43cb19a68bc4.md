...
`fn` 参数将传递一个函数的名称，例如 `"SUM"`。 更新 `apply` 以使用 `fn` 变量作为对象访问的键从 `spreadsheetFunctions` 对象隐式返回函数。

请记住，`fn` 可能不包含小写字符串，因此你需要将其转换为小写字符串。
...
你的 `apply` 函数应该访问 `spreadsheetFunctions` 对象。
...
你的 `apply` 函数应该访问与 `fn` 值匹配的 `spreadsheetFunctions` 对象的属性。
...
你的 `apply` 函数应该在属性访问中的 `fn` 上调用 `.toLowerCase()` 方法。
