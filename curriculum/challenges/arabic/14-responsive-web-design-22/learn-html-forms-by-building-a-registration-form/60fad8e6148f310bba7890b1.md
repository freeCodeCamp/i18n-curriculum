...
يجب عليك استخدام منتقي عناصر مفصول بفواصل لتصميم (style) العناصر `h1` و `p`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

يجب عليك استخدام `margin` من `1em auto` لتوسيط العناصر `h1` و `p`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

يجب عليك استخدام `text-align` من `center` لتوسيط نص `h1` و `p`.
