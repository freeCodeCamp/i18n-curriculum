...
Kama unabonyeza kitufe cha nyuma wakati hakuna wimbo unaocheza, wimbo wa kwanza utaanza kucheza. Ili kurekebisha hili, ongeza tamko la `if` mwanzoni mwa kitendakazi cha `playPreviousSong`. Angalia kama wimbo wa sasa ni `null` na simamisha utekelezaji wa kitendakazi kwa tamko la `return`.

# --hints--

Unapaswa kuongeza tamko la `if` linalokagua kama `userData.currentSong === null` na kusimamisha utekelezaji wa kitendakazi kwa tamko la `return` ndani ya kitendakazi chako cha `playPreviousSong`.
