...
- **`display: none;`**: `display: none;`을 사용하면 화면 읽기 프로그램과 다른 보조 기술이 이 콘텐츠에 접근할 수 없는데, 이는 접근성 트리에 포함되지 않기 때문입니다. 따라서 이 방법은 시각적 표현과 접근성 모두에서 콘텐츠를 완전히 제거하려는 경우에만 사용하는 것이 중요합니다.
- **`visibility: hidden;`**: 이 속성과 값은 콘텐츠를 시각적으로 숨기지만 문서 흐름에는 남겨두어 페이지에서 공간을 차지하게 합니다. 이 요소들은 접근성 트리에서 제거되어 화면 읽기 프로그램이 더 이상 읽지 않습니다.
- **`.sr-only` CSS 클래스**: 화면에는 보이지 않게 하면서도 화면 읽기 프로그램이 접근할 수 있도록 하는 일반적인 기법입니다.
...
위 예제에서는 사용자가 기기에서 동작 감소 선호도를 설정하지 않은 경우 부드러운 스크롤이 활성화됩니다.
...
자리 표시자 텍스트는 접근성에 좋지 않습니다. 사용자들이 자리 표시자 텍스트를 실제 입력값과 혼동하여 이미 입력값이 있다고 생각하는 경우가 너무 많습니다.
