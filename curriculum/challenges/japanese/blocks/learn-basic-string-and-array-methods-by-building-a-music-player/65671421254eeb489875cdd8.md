...
コールバック関数内で、`removeAttribute()` メソッドを使って `"aria-current"` 属性を削除してください。これにより、各曲の属性が削除されます。

# --hints--

既存の `forEach()` とその内容は変更しないでください。

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*\{\s*/)
```

`removeAttribute()` に対して `songEl` メソッドを使い、`"aria-current"` を渡してください。
