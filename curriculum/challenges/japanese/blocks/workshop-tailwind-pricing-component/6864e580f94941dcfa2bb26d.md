...
次は、プランのカード内のアイテムの配置を調整しましょう。

そのために、コンテナをグリッド表示に設定し、Tailwindのクラスで大括弧を使ってカスタムの行サイズを指定してください。

分数単位を使ってカスタムのグリッド行を設定する方法は次の通りです。
...
プランの外側の`div`にユーティリティクラス`grid`と`grid-rows-[1fr_auto]`を付けてください。

# --hints--

`div`要素にはクラス`grid`を付けてください。
...
`div`要素にはクラス`grid-rows-[1fr_auto]`を付けてください。
