...
Ongeza sifa ya kipengele ya `value` yenye thamani ya `player.dri` kwenye ingizo la DRI. Kisha ongeza sifa ya kipengele ya `onChange` iliyowekwa kuwa kitendakazi cha mshale kinachopokea `e` kama kigezo chake na kufanya wito wa `setPlayer` kwa kueneza `player` na `dri` kuwekwa kuwa `Number(e.target.value)`.

# --hints--

`input` ya DRI yako inapaswa kuwa na `value` sawa na takwimu ya `dri` ya mchezaji.
...
`input` ya DRI yako inapaswa kuwa na mshughulikiaji wa `onChange` anayefanya wito wa `setPlayer` kwa kueneza `player` na `dri` kuwekwa kuwa `Number(e.target.value)`.
