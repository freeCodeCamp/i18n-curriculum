...
仍然在外层 `if` 语句的体内，在 `if seat_type == 'Premium':` 和 `else:` 行之间添加一个 `elif` 子句，并查看 `seat_type` 是否相等于 `Gold`。在 `elif` 子句的体内，将 `service_charges` 的值更新为 `3`。

在嵌套的 `if...elif...else` 语句下，使用 `print()` 调用打印一条消息，显示 `Service charges:`，后跟更新后的 `service_charges` 值。然后，在终端查看输出。
...
你应该在你的 `elif` 体内将整数值 `3` 赋值给 `service_charges` 变量。
...
你应该在你的 `if...elif...else` 语句下方有 `print('Service charges:', service_charges)`。
