...
`clip-path` プロパティは `clip` プロパティの形状を指定します。 `clip-path` プロパティを `inset(50%)` の値に設定し、clip-path を要素内部の長方形に整形してください。

# --hints--

`span[class~="sr-only"]` セレクターの `clip` プロパティを `rect(1px, 1px, 1px, 1px)` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('clip') === 'rect(1px, 1px, 1px, 1px)');
```

`span[class~="sr-only"]` セレクターの `clip-path` プロパティを `inset(50%)` に設定する必要があります。
