...
Додайте слухач подій `"click"` до елемента `nextButton`, а потім передайте `playNextSong` як другий аргумент слухача подій. Це функція, яку запускатиме слухач подій.

Перевірте застосунок, натиснувши спочатку кнопку відтворення, а потім кнопку «наступна». Ви повинні побачити, що все працює, як очікувалося.

# --hints--

Викличте метод `addEventListener()` на змінній `nextButton`.
...
Передайте `playNextSong` як другий аргумент методу `addEventListener`.
