...
雜誌經常在其列印內容中使用對齊文字來結構其版面並控制內容的流程。雖然這在列印表單中有效，但在網站上使用對齊文字可能會造成存取性問題，例如對於有閱讀障礙的人來說會帶來挑戰。

為你的專案中的 `.text` 選擇器添加 `text-align` 屬性並設為 `justify`，即可讓你的專案看起來像列印的雜誌。

# --hints--

你的 `.text` 選擇器應該有一個設定為 `justify` 的 `text-align` 屬性。
