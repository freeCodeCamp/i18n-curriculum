...
使用与 `addCharacters` 函数相同的语法来更新 `elemValue` 函数。 它不应再声明 `inner`，而应隐式返回该函数。

# --hints--

你的 `elemValue` 函数应该使用隐式返回。
...
你的 `elemValue` 函数应该隐式返回一个带有 `character` 参数的箭头函数。
...
你的内部箭头函数应返回以 `character + num` 作为参数调用 `idToText()` 的结果。
