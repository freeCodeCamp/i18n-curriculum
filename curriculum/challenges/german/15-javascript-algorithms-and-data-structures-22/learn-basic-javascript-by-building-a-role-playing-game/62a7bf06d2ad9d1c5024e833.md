...
Du wirst auch die Funktionen aktualisieren müssen, die beim erneuten Anklicken der Schaltflächen ausgeführt werden.

Aktualisiere die `onclick`-Eigenschaft für jeden Button in deiner `goStore()`-Funktion, um bzw. `buyHealth`, `buyWeapon` oder `goTown` auszuführen.
...
Du solltest nicht `let` oder `const` verwenden, um auf die `onclick`-Eigenschaft von `button1` zuzugreifen.

```js
assert.notMatch(code, /(let|const)\s+button1\.onclick/);
```

Du solltest die `onclick`-Eigenschaft von `button1` auf `buyHealth` setzen.
...
Du solltest nicht `let` oder `const` verwenden, um auf die `onclick`-Eigenschaft von `button2` zuzugreifen.

```js
assert.notMatch(code, /(let|const)\s+button2\.onclick/);
```

Du solltest die `onclick`-Eigenschaft von `button2` auf `buyWeapon` setzen.

```js
assert.match(code, /button2\.onclick\s*=\s*buyWeapon/);
```

Du solltest die `onclick`-Eigenschaft von `button2` in deiner `goStore`-Funktion einstellen.
...
Du solltest nicht `let` oder `const` verwenden, um auf die `onclick`-Eigenschaft von `button3` zuzugreifen.

```js
assert.notMatch(code, /(let|const)\s+button3\.onclick/);
```

Du solltest die `onclick`-Eigenschaft von `button3` auf `goTown` setzen.

```js
assert.match(code, /button3\.onclick\s*=\s*goTown/);
```

Du solltest die `onclick`-Eigenschaft von `button3` in deiner `goStore`-Funktion einstellen.
