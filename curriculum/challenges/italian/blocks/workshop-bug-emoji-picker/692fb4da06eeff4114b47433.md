...
All'interno della classe `Bee`, crea un costruttore che prende un singolo parametro `emojiElement` di tipo `HTMLParagraphElement`. 

Nel costruttore, chiama la funzione `super` con il parametro `emojiElement` per invocare il costruttore della classe genitore `Bug`. Poi, imposta la proprietà `emoji` al valore stringa `"🐝"`.

# --hints--

La tua classe `Bee` dovrebbe avere un costruttore.
...
Il tuo costruttore `Bee` dovrebbe avere un parametro chiamato `emojiElement` di tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Bee[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Il tuo costruttore `Bee` dovrebbe chiamare `super` con il parametro `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /super\s*\(\s*emojiElement\s*\)/);
```

Il tuo costruttore `Bee` dovrebbe impostare `this.emoji` su `"🐝"`.
