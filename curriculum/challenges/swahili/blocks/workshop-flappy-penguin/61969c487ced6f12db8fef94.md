...
Lenga kipengele cha `.left-mountain`, na weka `width` na `height` kuwa `300px`. Kisha, weka `background` kuwa mwelekeo wa mstari unaoanza kwa `rgb(203, 241, 228)` na kumalizika kwa `rgb(80, 183, 255)`.
...
Unapaswa kumpa `.left-mountain` `width` ya `300px`. Inatarajiwa `--fcc-actual--` iwe `--fcc-expected--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.width, '300px');
```

Unapaswa kumpa `.left-mountain` `height` ya `300px`. Inatarajiwa `--fcc-actual--` iwe `--fcc-expected--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.height, '300px');
```

Unapaswa kumpa `.left-mountain` `background` ya `linear-gradient(rgb(203, 241, 228), rgb(80, 183, 255))`.
