...
你需要重新算繪歌曲、醒目標示它，並設定播放按鈕的存取文字，因為歌曲列表將會改變。

呼叫 `renderSongs` 函式，並將 `userData?.songs` 陣列作為引數傳入，這會呈現修改後的播放列表。

之後，若有目前的歌曲，也呼叫 `highlightCurrentSong` 函式（程式）來凸顯標示該歌曲，並呼叫 `setPlayButtonAccessibleText` 函式（程式）來更新播放按鈕的可存取文字。

# --hints--

你不應該修改現有的 `deleteSong` 函式（程式）及其內容。
...
你應該使用 `userData?.songs` 來呼叫 `renderSongs` 函式。
