...
Dentro de la función, obtén referencias a los elementos HTML responsables de mostrar el título de la canción y el artista.

Accede a los elementos `#player-song-title` y `#player-song-artist` con el método `getElementById()`. Asígnalos a las variables `playingSong` y `songArtist` respectivamente.

# --hints--

No debes modificar la función existente `setPlayerDisplay` y su contenido.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Debes usar `document.getElementById()` para obtener el elemento `#player-song-title`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Debes asignar el elemento `#player-song-title` a la variable `playingSong`. No te olvides de usar `const` para declarar la variable.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Debes usar `document.getElementById()` para obtener el elemento `#player-song-artist`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Debes asignar el elemento `#player-song-artist` a la variable `songArtist`. No te olvides de usar `const` para declarar la variable.
