...
Ongeza kichaguzi kipya kinachobadilisha `background-color` ya kipengele cha `.card` kuwa `khaki` wakati kinapopigwa kwa kipanya.

# --hints--

Unapaswa kuwa na kichaguzi cha `.card:hover`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card:hover"));
```

Kichaguzi cha `.card:hover` kinapaswa kuwa na `background-color` iliyowekwa kuwa `khaki`.
