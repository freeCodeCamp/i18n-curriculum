...
- メッセージ `"\nTick " + (tickIndex + 1)` をログに出力してください。
- `gates` 配列の各ゲートをループし、次の引数で `handleGateAtTick()` を呼び出してください：
  - `gates`：完全な gates 入力配列。
  - `gate`：現在のゲートオブジェクト。
  - `tickIndex`：現在のティック。
  - `throughputSummary`：ゲートごとに処理された合計参加者を追跡するローカルオブジェクト。
- 次のティックに進むために `tickIndex` を 1 増やしてください。

# --hints--

次のメッセージ `"\nTick " + (tickIndex + 1)` をコンソールにログ出力してください。
...
コンソールにログ出力した後、`gates` 配列の各ゲートをループし、`handleGateAtTick()`、`gates`、`gate`、`tickIndex` を引数にして `throughputSummary` を呼び出してください。
...
`gates` 配列のループの後、`tickIndex` を 1 増やしてください。
