...
在 `TVSeries` 类中，创建一个 `__str__` 方法，该方法返回一个字串，格式如下：`title (year) - S seasons, N episodes, duration min avg, director`，其中 `title`、`year`、`S`、`N`、`duration` 和 `director` 应分别替换为 `title`、`year`、`seasons`、`total_episodes`、`duration` 和 `director` 属性。

# --hints--

你的 `TVSeries` 类应有一个 `__str__` 方法。
...
你的 `__str__` 方法应返回格式为 `title (year) - S seasons, N episodes, duration min avg, director` 的字串。
