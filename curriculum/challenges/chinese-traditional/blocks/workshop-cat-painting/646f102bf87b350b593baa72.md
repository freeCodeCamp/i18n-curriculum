...
在 `.cat-whiskers-right` 元素內，創建 3 個具有類別 `cat-whisker-right-top`、`cat-whisker-right-middle` 和 `cat-whisker-right-bottom` 的 `div` 元素。

# --hints--

你不應該更改具有類別 `cat-whiskers-right` 的現有 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers-right'), 1);
```

你應該在你的 `.cat-whiskers-right` 元素內創建三個 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers-right div'), 3);
```

`.cat-whiskers-right` 元素內的第一個 `div` 元素應該具有類別 `cat-whisker-right-top`。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-right div')[0].classList.contains('cat-whisker-right-top'));
```

`.cat-whiskers-right` 元素內的第二個 `div` 元素應該有 `cat-whisker-right-middle` 類別。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-right div')[1].classList.contains('cat-whisker-right-middle'));
```

`.cat-whiskers-right` 元素內的第三個 `div` 元素應該有 `cat-whisker-right-bottom` 類別。
