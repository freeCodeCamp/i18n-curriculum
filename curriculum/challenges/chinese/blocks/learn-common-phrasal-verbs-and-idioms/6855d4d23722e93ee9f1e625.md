---
id: 6855d4d23722e93ee9f1e625
title: 任务 69
challengeType: 22
dashedName: task-69
lang: en-US
---

<!-- (Audio) Jake: I'll take note of that. I also want to address the security questions for password recovery. -->

# --instructions--

请听音频，并完成下面的填空。

# --fillInTheBlank--

## --sentence--

`I'll take note of that. I also want to address the BLANK questions for password BLANK.`

## --blanks--

`security`

### --feedback--

这意味着受到威胁或伤害的保护。

---

`recovery`

### --feedback--

这个词指的是重新获得对你账户的访问的进程。

# --explanation--

`Security questions` 是用于验证用户同一性的问题，通常在登录或恢复账户时使用。例如：

`One of the security questions asked for my mother's maiden name.` – 这意味着在验证用户同一性时，会以用户母亲婚前的名字作为一种形式进行询问。

`Password recovery` 是在密码遗忘或丢失时重新获得账户访问的进程。例如：

`The app sent me a reset link for password recovery.` – 这意味着用户获得了帮助来重置他们忘记的密码。

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Linda",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_24-2.mp3",
      "startTime": 1,
      "startTimestamp": 33.52,
      "finishTimestamp": 37.92
    }
  },
  "commands": [
    {
      "character": "Linda",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Linda",
      "startTime": 1,
      "finishTime": 3.28,
      "dialogue": {
        "text": "I'll take note of that. I also want to address",
        "align": "center"
      }
    },
    {
      "character": "Linda",
      "startTime": 3.28,
      "finishTime": 5.4,
      "dialogue": {
        "text": "the security questions for password recovery.",
        "align": "center"
      }
    },
    {
      "character": "Linda",
      "opacity": 0,
      "startTime": 5.9
    }
  ]
}
```
