...
بعد جملة `if` في طريقة `__str__`، أنشئ متغيرين باسم `movies` و `series` وقم بتعيين استدعاء طريقة `get_movies` إلى الأول واستدعاء طريقة `get_tv_series` إلى الثاني على التوالي.

# --hints--

يجب أن يكون لديك متغير باسم `movies` في طريقة `__str__`.
...
قم بتعيين `self.get_movies()` إلى متغير `movies` الخاص بك.
...
يجب أن يكون لديك متغير باسم `series` في طريقة `__str__`.
...
قم بتعيين `self.get_tv_series()` إلى متغير `series` الخاص بك.
