...
`updateResult` 函式會從下拉選單讀取所選的形狀型別，收集對應的輸入值，創建一個 `shape` 物件，將它傳遞給 `calculateArea()` 函式來計算面積，並在 `resultText` 元素中顯示計算結果。

在函式內，檢查所選的形狀並建置正確的物件以傳入 `calculateArea` 函式。

如果形狀是 `"circle"`，則呼叫 `calculateArea` 函式，並傳入包含型別和半徑輸入數值的物件。

# --hints--

你的 `updateResult` 函式應該在形狀為圓形時正確更新結果文字。
