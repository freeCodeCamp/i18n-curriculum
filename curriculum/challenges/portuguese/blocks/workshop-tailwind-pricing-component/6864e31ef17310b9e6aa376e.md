...
Os itens da grade estão muito próximos das bordas esquerda e direita da página, então você deve trabalhar no espaçamento.

Atribua a classe `max-w-6xl` ao `div` para definir uma largura fixa de `72rem` e evitar que os itens se estiquem demais pela página.

Como isso vai empurrar todos para a esquerda, adicione `mx-auto` ao `div` para centralizar os itens horizontalmente.

# --hints--

Seu elemento `div` deve ter a classe `max-w-6xl`.
...
Seu elemento `div` deve ter a classe `mx-auto`.
