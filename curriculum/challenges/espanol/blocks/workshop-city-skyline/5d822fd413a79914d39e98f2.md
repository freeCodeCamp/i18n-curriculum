...
Agregue la nueva clase `bb1-window` a los elementos `.bb1a`, `.bb1b` y `.bb1c`. Esto aplicará el degradado a ellos.

# --hints--

No debes eliminar la clase `bb1a`.

```js
assert.exists(document.querySelector('div.bb1a'));
```

Debes añadir la clase `bb1-window` al elemento `.bb1a`.

```js
assert.exists(document.querySelector('div.bb1a.bb1-window'));
```

No debes eliminar la clase `bb1b`.

```js
assert.exists(document.querySelector('div.bb1b'));
```

Debes añadir la clase `bb1-window` al elemento `.bb1b`.

```js
assert.exists(document.querySelector('div.bb1b.bb1-window'));
```

No debes eliminar la clase `bb1c`.

```js
assert.exists(document.querySelector('div.bb1c'));
```

Debes añadir la clase `bb1-window` al elemento `.bb1c`.

```js
assert.exists(document.querySelector('div.bb1c.bb1-window'));
```

No debes cambiar el elemento `.bb1d`.
