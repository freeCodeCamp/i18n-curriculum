...
Die Funktion soll die berechneten Daten zurückgeben, damit sie von Aufrufern programmatisch verwendet werden können.

Fügen Sie nach der `for...of`-Schleife eine `return`-Anweisung hinzu, die ein Array zurückgibt, das `distances` und `paths` in dieser Reihenfolge enthält.

# --hints--

Sie sollten `[distances, paths]` nach der `for...of`-Schleife zurückgeben.
