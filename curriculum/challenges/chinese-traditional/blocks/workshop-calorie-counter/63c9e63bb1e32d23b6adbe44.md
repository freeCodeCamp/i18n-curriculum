...
當你需要將字串轉成小寫時，你可以使用 <dfn>toLowerCase()</dfn> 方法。此方法會傳回轉換成小寫的呼叫字串值。
...
你的 `output.innerHTML` 字串需要一個 `span` 元素。創建該元素，並為它設定一個 `class` 屬性，其值為 `surplusOrDeficit` 變數。你的 `surplusOrDeficit` 變數應該使用 `toLowerCase()` 方法轉換成小寫。

不要給你的 `span` 任何文字。

# --hints--

你應該在你的模板字面量中創建一個 `span` 元素。
...
你的 `span` 應該將 `class` 屬性設為 `surplusOrDeficit.toLowerCase()`。
