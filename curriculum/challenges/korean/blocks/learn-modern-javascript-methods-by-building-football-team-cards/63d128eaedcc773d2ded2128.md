...
`myFavoriteFootballTeam.players` 배열에는 두 개의 값이 있어야 합니다.
...
`myFavoriteFootballTeam.players` 배열의 두 번째 값은 객체여야 합니다.
...
`name` 속성은 `"Sergio Batista"`로 설정해야 합니다.
...
`position` 속성은 `"midfielder"`로 설정해야 합니다.
