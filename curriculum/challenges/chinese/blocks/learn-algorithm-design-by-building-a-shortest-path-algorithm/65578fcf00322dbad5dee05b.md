...
创建一个`if`语句，检查邻居节点（已处理元组中的第二项）的距离加上`current`的距离是否小于邻居节点（处理元组中第一项）的当前已知距离。 

使用`pass`关键字临时填充 `if`中的内容。

# --hints--

你应该有一个`if`语句来检查`distance + distances[current]`是否小于`distances[node]`。
