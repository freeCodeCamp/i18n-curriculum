...
Вам знадобиться стовпчик для тексту та стовпчик для зображень. Надайте селектору `.text-with-images` властивість `grid-template-columns` зі значенням `1fr 2fr`. Також встановіть властивість `column-gap` на `3rem`, щоб забезпечити більше простору між стовпчиками.

# --hints--

Ваш селектор `.text-with-images` повинен мати властивість `grid-template-columns` зі значенням `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

Ваш селектор `.text-with-images` повинен мати властивість `column-gap` зі значенням `3rem`.
