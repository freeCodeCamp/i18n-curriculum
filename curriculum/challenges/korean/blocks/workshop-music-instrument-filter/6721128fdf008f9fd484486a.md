...
나머지 목관악기인 클라리넷과 오보에를 나타내도록 `instrumentsArr`에 객체 두 개를 더 추가하세요.

# --hints--

`{ category: "woodwinds", instrument: "Clarinet", price: 200 }` 안에 `instrumentsArr`이 있어야 합니다.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

`{ category: "woodwinds", instrument: "Oboe", price: 4000 }` 안에 `instrumentsArr`이 있어야 합니다.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

여전히 `{ category: "woodwinds", instrument: "Flute", price: 500 }` 안에 `instrumentsArr`이 있어야 합니다.
