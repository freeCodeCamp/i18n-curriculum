...
使用 `h1 .flex` 選擇器定位你的 flex 容器。 給它一個 `display` 屬性設，置爲 `flex`，啓用 flexbox 佈局。 然後將 `flex-direction` 屬性設置爲 `column-reverse`——這將從下到上顯示嵌套元素。 最後，將 `gap` 屬性設置爲 `1rem`，在元素之間創建一些空間。
