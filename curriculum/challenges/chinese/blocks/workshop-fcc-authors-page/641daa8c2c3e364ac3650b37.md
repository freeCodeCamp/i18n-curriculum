...
其次，每个作者的姓名和图像与卡片上的其他详细信息之间没有太多的分隔。 分隔符将为作者卡片提供清晰的视觉层次。

在作者简介上方添加一个 `div` 元素，并为其添加一个 `class` 属性，值为 `"purple-divider"`。
