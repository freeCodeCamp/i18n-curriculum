...
title: "Aufgabe 306: HTML-Inhaltsextraktor"
challengeType: 28
...
Gegeben ist ein String mit HTML. Geben Sie den reinen Textinhalt zurück, wobei alle Tags entfernt sind.

# --hints--

`extractContent('<p>hello world</p>')` sollte `"hello world"` zurückgeben.

```js
assert.equal(extractContent('<p>hello world</p>'), "hello world");
```

`extractContent('<p>hello <span>world</span></p>')` sollte `"hello world"` zurückgeben.

```js
assert.equal(extractContent('<p>hello <span>world</span></p>'), "hello world");
```

`extractContent('<a href="example.com">Click me</a>')` sollte `"Click me"` zurückgeben.

```js
assert.equal(extractContent('<a href="example.com">Click me</a>'), "Click me");
```

`extractContent('<p><button onClick="learnToCode()">Learn</button> to <code>code<code> <br/>for <strong>free</strong> <br/>on <a href="https://freecodecamp.org/" target="_blank"><span class="highlight">freecodecamp</span>.org</a>')` sollte `"Learn to code for free on freecodecamp.org"` zurückgeben.
