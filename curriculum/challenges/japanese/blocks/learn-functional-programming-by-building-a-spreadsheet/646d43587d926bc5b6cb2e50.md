...
`str2` 変数を宣言し、`infixEval()` と `noHigh` を引数として `infix` を呼び出した結果を代入してください。
...
`str2` に `infixEval()` を呼び出した結果を代入してください。
...
`noHigh` の最初の引数に `infixEval()` を渡してください。
...
`infix` の2番目の引数に `infixEval()` を渡してください。
