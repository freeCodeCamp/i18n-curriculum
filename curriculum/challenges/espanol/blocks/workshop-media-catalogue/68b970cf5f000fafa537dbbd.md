...
Dado que estás almacenando el objeto que causa la excepción en el objeto de excepción, puedes hacer algún uso de él.

Después de tu llamada existente a `print`, agrega `print(f'Unable to add {e.obj}: {type(e.obj)}')`.

Este tipo de mensajes puede ser extremadamente útil cuando estás tratando de depurar tu código.

# --hints--

Debes tener `print(f'Unable to add {e.obj}: {type(e.obj)}')` después de tu llamada existente `print(f'Media Error: {e}')`.
