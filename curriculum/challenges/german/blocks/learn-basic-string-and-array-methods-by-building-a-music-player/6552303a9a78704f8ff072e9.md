...
Jetzt ist es Zeit, die Pause-Taste zu testen.

Fügen Sie dem `"click"`-Element einen `pauseButton`-Ereignislistener hinzu und übergeben Sie `pauseSong` als zweiten Parameter des Ereignislisteners. Dies ist die Funktion, die der Ereignislistener ausführen wird.

Testen Sie Ihre App, indem Sie zuerst auf die Wiedergabetaste und dann auf die Pause-Taste klicken. Sie sollten sehen, dass alles wie erwartet funktioniert.

# --hints--

Sie sollten die `addEventListener()`-Methode auf Ihrer `pauseButton`-Variablen aufrufen.
...
Sie sollten `pauseSong` als zweiten Parameter Ihrer `addEventListener`-Methode übergeben.
