...
要设置提交按钮的样式，你可以使用*属性*选择器，它根据给定的属性值选择元素。 这是一个示例：

```css
input[name="password"]
```

以上内容选择 `name` 属性值为 `password` 的 `input` 元素。

现在，使用属性选择器设置提交按钮的样式，将 `display` 设置为 `block`，将 `width` 设置为 `60%`。
