...
`case` の長さが `results` の場合に備えて、もう1つの `1` を追加してください。この場合、`result_list` に `x = <root>` という形式の文字列を含むリストを代入してください。ここで `<root>` は方程式の解です。解の正負の符号が両方表示されるように文字列をフォーマットしてください。
...
既存の `case` ブロックの後に、`1` パターンの新しい `case` を作成してください。
...
新しい `f'x = {results[0]:+}'` 本体の中で、`result_list` を含むリストを `case` に代入してください。
