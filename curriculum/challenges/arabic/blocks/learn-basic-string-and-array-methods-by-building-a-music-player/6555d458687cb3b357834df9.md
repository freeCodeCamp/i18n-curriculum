...
ادخل إلى خاصيتي `userData?.currentSong?.title` و`userData?.currentSong?.artist` وقم بتعيينهما إلى متغيري `currentTitle` و`currentArtist` على التوالي.

# --hints--

يجب أن تدخل إلى `title` الخاص بـ `currentSong` من كائن `userData`. لا تنسَ استخدام التسلسل الاختياري.

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

يجب أن تعيّن `title` الخاص بـ `currentSong` إلى ثابت `currentTitle`.

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

يجب أن تدخل إلى `artist` الخاص بـ `currentSong` من كائن `userData`. لا تنسَ استخدام التسلسل الاختياري.

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

يجب أن تعيّن `artist` الخاص بـ `currentSong` إلى ثابت `currentArtist`.
