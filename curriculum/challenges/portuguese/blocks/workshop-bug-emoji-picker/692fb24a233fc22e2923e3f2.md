...
Após o construtor, crie um método `render` abstrato que tenha um tipo de retorno `void`. Você vai usá-lo nas subclasses para atualizar o emoji exibido.

# --hints--

Sua classe `Bug` deve ter um método `render` abstrato.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

Seu método `render` deve ter um tipo de retorno `void`.
