...
同じ補間構文を使って、テキスト `consumedCalories Calories Consumed` を持つ 2 つ目の `p` 要素と、テキスト `exerciseCalories Calories Burned` を持つ 3 つ目の `p` 要素を追加してください。 忘れずに、`consumedCalories` と `exerciseCalories` の部分を適切な値に変更してください。

# --hints--

テンプレートリテラルに 2 つ目の `p` 要素を追加する必要があります。

```js
assert.isAtLeast(getTemplateContents(code)?.match(/<p>[^<]*<\/p>/g)?.length, 2);
```

2 つ目の `p` 要素は新しい行に記述してください。
...
2 つ目の `p` 要素はテキスト `${consumedCalories} Calories Consumed` を持つようにしてください。
