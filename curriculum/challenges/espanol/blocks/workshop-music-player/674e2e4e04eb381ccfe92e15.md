...
Agrega un oyente de eventos `"click"` al elemento `pauseButton`, luego pasa `pauseSong` como segundo argumento del oyente de eventos. Esta es la función que ejecutará el oyente de eventos.

Prueba tu aplicación primero haciendo clic en el botón de reproducción seguido del botón de pausa. Debes ver que todo funciona como se espera.

# --hints--

Debes llamar al método `addEventListener()` en tu variable `pauseButton`.
...
Debes pasar `pauseSong` como segundo argumento de tu método `addEventListener`.
