...
`TVSeries` クラス内に、次の形式の文字列を返す `__str__` メソッドを作成してください：`title (year) - S seasons, N episodes, duration min avg, director`。ここで `title`、`year`、`S`、`N`、`duration`、および `director` は、それぞれ `title`、`year`、`seasons`、`total_episodes`、`duration`、`director` 属性で置き換えられます。

# --hints--

あなたの `TVSeries` クラスには `__str__` メソッドが必要です。
...
あなたの `__str__` メソッドは `title (year) - S seasons, N episodes, duration min avg, director` 形式の文字列を返す必要があります。
