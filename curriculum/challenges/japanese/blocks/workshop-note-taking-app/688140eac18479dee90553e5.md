...
別の `div` 要素を使用しているので、スクリーンリーダーにとってよりアクセスしやすくする必要があります。

前のレッスンで、ページ上にライブリージョンを作成するために使われる `aria-live` 属性について学びました。これにより、スクリーンリーダーユーザーはライブリージョンの内容が変わったときに、自動的に通知を受け取ることができ、手動でフォーカスを移動したり操作したりする必要がなくなります。

`#status` 要素に `aria-live` 属性を追加し、その値を `"polite"` に設定してください。

`polite` の値は、スクリーンリーダーに現在のアナウンスが終わるか、ユーザーが入力をストップするまで待ってから更新を通知するよう指示します。

# --hints--

`#status` 要素には `aria-live` 属性が必要です。

```js
assert.isTrue(document.getElementById("status").hasAttribute("aria-live"));
```

`#status` 要素には `aria-live` 属性を `"polite"` に設定してください。
