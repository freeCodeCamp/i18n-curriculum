...
Até agora, nada muda quando você seleciona um bug diferente.

Adicione um listener de evento `change` a `selectElement`. No callback, use `isSelect` para verificar se `e.target` é um `HTMLSelectElement`. Você ainda não precisa fazer nada dentro do bloco `if`.

# --hints--

Seu `selectElement` deve ter um listener de evento `change` com um callback que recebe um parâmetro event.
...
O callback do seu listener de evento deve ter uma declaração `if` com a condição `isSelect(e.target)`.
