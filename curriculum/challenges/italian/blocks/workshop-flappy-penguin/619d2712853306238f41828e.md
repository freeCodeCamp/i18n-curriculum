...
Crea una variabile CSS personalizzata chiamata `--penguin-skin` e impostala su `gray`. Poi, sostituisci tutti i valori delle proprietà rilevanti con essa.

# --hints--

Dovresti dare a `:root` una proprietà `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

Dovresti dare a `--penguin-skin` un valore di `gray`, ma è stato trovato `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

Dovresti dare a `.penguin-head` una `background` di `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
Dovresti dare a `.penguin-body::before` una `background-color` di `var(--penguin-skin)`, ma è stato trovato `--fcc-actual--`.
...
Dovresti dare a `.arm` una `background` di `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
