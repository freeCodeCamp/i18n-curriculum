...
**Nota:** Se você tiver a fonte `Lobster` instalada no seu computador, não verá a degradação porque seu navegador consegue encontrar a fonte.
...
Você deve comentar sua chamada para o Google da fonte `Lobster` colocando `<!--` na frente dela.

```js
assert.match(code, /<!--[^fc]/gi);
```

Você deve fechar seu comentário adicionando `-->`.
