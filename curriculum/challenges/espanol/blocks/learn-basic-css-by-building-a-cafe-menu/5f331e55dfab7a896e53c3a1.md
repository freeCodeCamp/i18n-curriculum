...
Dentro del elemento `head`, anida un elemento `meta` con un atributo `charset` con el valor `utf-8` para decirle al navegador cómo se codificarán los caracteres para la página.
...
El elemento `meta` es un elemento vacío, no debe tener una etiqueta de cierre `</meta>`.
