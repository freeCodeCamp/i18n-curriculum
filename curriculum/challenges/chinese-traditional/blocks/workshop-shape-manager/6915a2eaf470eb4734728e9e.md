...
現在，創建一個 `if` 述語，檢查 `value` 是否小於 `0`。如果為真，表示輸入的值是負數。接著，使用 alert 向使用者顯示訊息 `Negative values are not allowed.`，然後呼叫 `clearInputFields()` 函式清除輸入欄位。

在 `if` 述語下方，呼叫 `updateResult` 函式重新計算面積。

# --hints--

你應該在 `if` 函式內創建一個條件為 `value < 0` 的 `handleInput` 述語。

```js
assert.match(handleInput.toString(), /if\s*\(\s*value\s*<\s*0\s*\)\s*\{\s*/)
```

你應該在 `Negative values are not allowed.` 述語內使用 alert 顯示 `if`。

```js
assert.match(handleInput.toString(), /if\s*\(\s*value\s*<\s*0\s*\)\s*\{\s*(window.)?alert\(["']Negative\s+values\s+are\s+not\s+allowed\.["']\);?\s*/)
```

你應該在 `clearInputFields` 述語內呼叫 `if` 函式。
...
你應該在 `updateResult` 述語下方呼叫 `if` 函式。
