...
Зверніть увагу, що обкладинка альбому в HTML і пісні в масиві `userData.songs` змінилися. Ми замінили оригінальні пісні на коротші, які ви зможете використати для тестування свого застосунку в наступних кроках.

Далі потрібно перевірити, чи є наступна пісня для відтворення. Отримайте індекс поточної пісні, викликавши функцію `getCurrentSongIndex()`, і збережіть його у константі `currentSongIndex`.

Потім створіть константу `nextSongExists`, яка міститиме булеве значення `true` або `false` залежно від того, чи існує наступна пісня.

# --hints--

Ви не повинні змінювати існуючий обробник події `ended` та його вміст.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Ви повинні створити константу `currentSongIndex` і присвоїти їй виклик функції `getCurrentSongIndex`.
...
Ви повинні перевірити, чи існує наступна пісня, порівнюючи `userData.songs.length` і `currentSongIndex`, і зберегти це у константі `nextSongExists`. Якщо останній індекс масиву пісень (`userData.songs.length - 1`) більший за currentSongIndex, це означає, що наступна пісня є.
