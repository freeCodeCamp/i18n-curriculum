...
接下来，告诉表单如何提交。 向表单元素添加一个值为 `post` 的 `method` 属性和一个值为 `https://superhero-application-form.freecodecamp.org` 的 `action` 属性。

现在，提交该 `form` 将把它发送到该 URL。你将在下一步执行此操作。

# --hints--

你的 `form` 元素应将 `method` 属性设置为 `post`。
...
你的 `form` 元素应将 `action` 属性设置为 `https://superhero-application-form.freecodecamp.org`。
