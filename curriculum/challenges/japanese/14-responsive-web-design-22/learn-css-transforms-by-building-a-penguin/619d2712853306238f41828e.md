...
`:root` には `--penguin-skin` プロパティが必要です。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

`--penguin-skin` には `gray` という値を設定する必要がありますが、`--fcc-actual--` が設定されています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

`.penguin-head` の `background` を `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))` に設定する必要があります。
...
`.penguin-body::before` の `background-color` を `var(--penguin-skin)` に設定する必要がありますが、`--fcc-actual--` が得られました。
...
`.arm` の `background` を `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))` に設定する必要があります。
