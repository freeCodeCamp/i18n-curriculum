...
Оскільки ви перемістили порівняння в умову `while`, можна видалити усю інструкцію `if`.

# --hints--

Інструкція `if` більше не потрібна.

```js
assert.notMatch(__helpers.removeJSComments(code), /if\s*\(\s*done\s*===\s*count\s*\)/);
```

Ви більше не повинні встановлювати `continueLoop` на `false`.
