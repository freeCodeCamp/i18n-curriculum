...
In deiner `elemValue`-Funktion hast du explizit eine Funktion mit dem Namen `inner` deklariert und diese zurückgegeben. Da du jedoch die Arrow-Syntax verwendest, kannst du eine Funktion implizit zurückgeben. Hier ein Beispiel:

```js
const curry = soup => veggies => {};
```

`curry` ist eine Funktion, die einen `soup`-Parameter verwendet und eine Funktion zurückgibt, die einen `veggies`-Parameter verwendet. Aktualisiere deine `addCharacters`-Funktion mit dieser Syntax, um eine leere Funktion zurückzugeben, die einen `character2`-Parameter verwendet.

# --hints--

Deine `addCharacters`-Funktion sollte eine implizite Rückgabe verwenden.
...
Deine `addCharacters`-Funktion sollte eine Arrow-Funktion zurückgeben, die einen `character2`-Parameter enthält.
...
Deine innere Arrow-Funktion sollte leer sein.
