...
Jisikie huru kubadilisha viwango vya `lin_eq` ili kuona jinsi matokeo yanavyobadilika.

Kisha, futa wito wako wa `print(solver(lin_eq))`, na chapisha matokeo ya kuita `solver()` ukiwa na `quadr_eq` kama hoja.

# --hints--

Haupaswi kuwa na `print(solver(lin_eq))` katika msimbo wako.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

Unapaswa kuchapisha `solver(quadr_eq)`.
