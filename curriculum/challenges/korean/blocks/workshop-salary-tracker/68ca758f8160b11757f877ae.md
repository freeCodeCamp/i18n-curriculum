...
`if` 문장은 새 레벨의 기본 급여가 더 낮은지 확인하기 위해 `Employee._base_salaries[new_level]`와 `Employee._base_salaries[self.level]`를 비교해야 합니다.
...
`new_level`이 `self.level`보다 높을 때는 예외를 발생시키면 안 됩니다.
