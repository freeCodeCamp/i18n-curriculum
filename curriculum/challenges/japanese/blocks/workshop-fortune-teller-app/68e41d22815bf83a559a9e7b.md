...
今度は、画像が読み込まれたときに `onload` イベントを使って `hidden` クラスを削除し、読み込み中のプレースホルダーを `previousElementSibling` を使って非表示にしてください。

# --hints--

`img` 要素には `onload` ハンドラを設定してください。
...
`onload` ハンドラでは `classList.remove` を使って `hidden` クラスを削除してください。ここで `this` キーワードが必要なことを忘れないでください。
...
`onload` ハンドラでは `previousElementSibling.style.display` を `'none'` に設定してください。これを行うには `this` キーワードが必要です。
