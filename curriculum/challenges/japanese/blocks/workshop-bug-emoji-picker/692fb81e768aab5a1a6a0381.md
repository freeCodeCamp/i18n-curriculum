...
`Spider` クラスの中に、`emojiElement` 型の単一のパラメータ `HTMLParagraphElement` を受け取るコンストラクタを作成してください。

コンストラクタの中で、親クラス `super` のコンストラクタを呼び出すために、`emojiElement` パラメータを使って `Bug` 関数を呼び出してください。その後、`emoji` プロパティに文字列値 `"🕷️"` を設定してください。

# --hints--

`Spider` クラスにはコンストラクタが必要です。
