---
id: 662f150f6e708633720c8c52
title: Tarea 63
challengeType: 19
dashedName: task-63
lang: en-US
---

<!-- (Audio) Tom: Hey, Sarah, I was reading about object-oriented programming on the internet. It's a new concept for me. -->

# --description--

Por favor, responda a la pregunta a continuación.

# --questions--

## --text--

¿Qué pregunta podría querer hacerle Tom a Sarah basándose en su sentencia?

## --answers--

`Can you explain what the internet is?`

### --feedback--

La declaración de Tom indica que entiende cómo usar internet ya que estaba leyendo sobre ello; su enfoque está en `object-oriented programming`.

---

`Can you help me understand object-oriented programming?`

---

`Do you have any books on object-oriented programming?`

### --feedback--

Aunque es posible que Tom quiera recursos, su sentencia sugiere que está buscando una explicación directa en lugar de material de lectura adicional.

---

`Why is object-oriented programming considered old?`

### --feedback--

Tom mencionó que `object-oriented programming` es un concepto nuevo para él, no que sea viejo o anticuado.

## --video-solution--

2

# --scene--

```json
{
  "setup": {
    "background": "company2-breakroom.png",
    "characters": [
      {
        "character": "Tom",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "5.1-3.mp3",
      "startTime": 1,
      "startTimestamp": 0,
      "finishTimestamp": 6.28
    }
  },
  "commands": [
    {
      "character": "Tom",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Tom",
      "startTime": 1,
      "finishTime": 7.28,
      "dialogue": {
        "text": "Hey, Sarah, I was reading about object-oriented programming on the internet. It's a new concept for me.",
        "align": "center"
      }
    },
    {
      "character": "Tom",
      "opacity": 0,
      "startTime": 7.78
    }
  ]
}
```
