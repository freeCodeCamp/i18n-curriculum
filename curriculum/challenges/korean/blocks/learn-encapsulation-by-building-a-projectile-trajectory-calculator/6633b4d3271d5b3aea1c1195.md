...
이제 그래프를 만들 차례입니다. `create_trajectory`라는 메서드를 만들고 코드 맨 아래에 있는 마지막 `print` 호출을 `print(graph.create_trajectory())`로 바꾸세요.

이 새 메서드의 첫 번째 단계로, 좌표들의 로컬 복사본을 만드는데 모든 값이 정수로 반올림되도록 하세요. 이 새 좌표 버전을 `rounded_coords`라는 변수에 저장하고 이 변수를 반환하세요.

# --hints--

`create_trajectory`라는 메서드를 정의해야 합니다.
...
`print(graph.create_coordinates_table())`를 `print(graph.create_trajectory())`로 바꿔야 합니다.
...
`rounded_coords` 변수에는 반올림된 좌표가 들어 있어야 합니다.
