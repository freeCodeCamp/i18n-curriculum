...
Siguiendo el patrón, selecciona los siguientes elementos `.cabin` y aplica las reglas específicas a ellos:

- El segundo `.cabin` debe tener la propiedad `right` establecida en `17%` y la propiedad `top` establecida en `93.5%`.
- El tercer `.cabin` debe tener la propiedad `right` establecida en `67%` y la propiedad `top` establecida en `93.5%`.
