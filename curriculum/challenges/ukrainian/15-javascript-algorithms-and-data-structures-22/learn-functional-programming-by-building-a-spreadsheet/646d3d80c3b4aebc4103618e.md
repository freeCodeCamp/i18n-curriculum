...
Дотримуючись того самого шаблону, додайте властивість множення `*` та ділення `/` за допомогою відповідних функцій.
...
Функція `*` має повернути результат множення `x` на `y`.
...
Функція `/` має повернути результат ділення `x` на `y`.
