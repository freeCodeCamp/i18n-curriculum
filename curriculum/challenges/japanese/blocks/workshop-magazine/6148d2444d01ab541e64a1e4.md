...
`.quote` セレクターを作成してください。`color` プロパティを `#00beef` に、`font-size` プロパティを `2.4rem` に、`text-align` プロパティを `center` に設定してください。
