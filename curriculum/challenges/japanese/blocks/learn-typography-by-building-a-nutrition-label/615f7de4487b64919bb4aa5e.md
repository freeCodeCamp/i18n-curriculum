...
最後の `.divider` の後に、テキスト `Cholesterol 0mg 0%` を持つ新しい `p` 要素を作成してください。次に、テキスト `Cholesterol` を `span` 要素で、`0%` を別の `span` 要素で囲み、それぞれに `bold` クラスを付けてください。

最後に、テキスト `Cholesterol` を含む `span` 要素とテキスト `0mg` を、整列のためにさらに `span` 要素にネストしてください。
...
新しい `p` 要素にはテキスト `Cholesterol 0mg 0%` を設定してください。
...
`Cholesterol` の `span` には `class` 属性を `bold` に設定してください。
...
`0%` の `span` には `class` 属性を `bold` に設定してください。
...
`Cholesterol` の `span` と `0mg` のテキストは `span` で囲んでください。
