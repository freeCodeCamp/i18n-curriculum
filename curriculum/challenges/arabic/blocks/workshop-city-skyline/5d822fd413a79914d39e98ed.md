...
حرّك موضع `.fb4` بالنسبة لمكانه الحالي بإضافة `position` بقيمة `relative` و`left` بقيمة `10%` إليه. افعل الشيء نفسه مع `.fb5` لكن استخدم `right` بدلًا من `left`. هذا سيغطي الفراغ الأبيض المتبقي بين المباني.
...
يجب أن تعطي `.fb4` خاصية `left` بقيمة `10%`.
...
يجب أن تعطي `.fb5` خاصية `right` بقيمة `10%`.
