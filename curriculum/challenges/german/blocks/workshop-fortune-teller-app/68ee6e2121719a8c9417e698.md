...
Direkt nachdem Sie `multipleCard` gelöscht haben, rufen Sie innerhalb der `singleCardSelected`-Methode die `showElements`-Funktion auf und übergeben `singleCard` als erstes Argument und `fortuneContainer` als zweites Argument.

# --hints--

Sie sollten die `showElements`-Funktion aufrufen.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

Sie sollten `elements.singleCard` als erstes Argument übergeben.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

Sie sollten `elements.fortuneContainer` als zweites Argument übergeben.
