...
Wie zuvor sollten Sie Ihre Eingabe validieren. Insbesondere, wenn `size` kleiner als `1` ist, sollten Sie `"Chunk size must be >= 1"` in der Konsole ausgeben und dann `return` aufrufen, um die Funktion zu beenden.

# --hints--

Wenn die Eingabe `size` ungültig ist, sollten Sie die folgende Fehlermeldung in der Konsole ausgeben, `"Chunk size must be >= 1"`.
...
Nachdem Sie die Fehlermeldung ausgegeben haben, sollten Sie sofort `return` aufrufen, um die Funktion frühzeitig zu beenden.
...
Ihre `chunkCrew`-Funktion sollte die Validierung ohne Fehler bestehen, wenn `size` größer oder gleich 1 ist.
