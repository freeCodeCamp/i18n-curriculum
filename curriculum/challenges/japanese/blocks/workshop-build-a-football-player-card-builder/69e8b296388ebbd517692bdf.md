...
DEF と PHY の両方の入力に `value` と `onChange` 属性を追加してください。

DEF には `value` を `player.def` に、`onChange` を `setPlayer` を呼び出し、`player` をスプレッドし、`def` を `Number(e.target.value)` に設定するアロー関数にしてください。

PHY には `value` を `player.phy` に、`onChange` を `setPlayer` を呼び出し、`player` をスプレッドし、`phy` を `Number(e.target.value)` に設定するアロー関数にしてください。

このステップで、すべての入力が状態に接続され、ライブプレビューが各フィールドごとに更新されるようになります！

# --hints--

DEF の `input` はプレイヤーの `value` ステータスと等しい `def` を持つ必要があります。
...
DEF の `input` は `onChange` を呼び出し、`setPlayer` をスプレッドし、`player` を `def` に設定する `Number(e.target.value)` ハンドラを持つ必要があります。
...
PHY の `input` はプレイヤーの `value` ステータスと等しい `phy` を持つ必要があります。
...
PHY の `input` は `onChange` を呼び出し、`setPlayer` をスプレッドし、`player` を `phy` に設定する `Number(e.target.value)` ハンドラを持つ必要があります。
