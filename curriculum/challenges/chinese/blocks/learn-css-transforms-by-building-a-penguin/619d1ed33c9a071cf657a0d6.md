...
在 `.penguin-body` 元素中，添加两个 `div` 元素，每个的 `class` 都是 `foot`。 为第一个 `.foot` 添加一个 `left` `class`，为第二个 `.foot` 添加一个 `right` `class`。
...
你应该将 `.foot.right` 放在 `.foot.left` 之后。
