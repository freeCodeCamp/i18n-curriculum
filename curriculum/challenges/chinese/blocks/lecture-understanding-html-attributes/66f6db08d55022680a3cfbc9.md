...
虽然许多代码格式化工具如 Prettier 会选择在空元素中包含 `/`，但 HTML 规格声明 `/` 的存在“并不表示开始标签是自闭合的，而是多余的且没有任何效果”。

在实际开发中，你会看到这两种形式，因此熟悉它们都很重要。

如果你想显示一张图像，需要在你的图像元素中包含几个属性。属性是用于调整 HTML 元素行为的特殊值。
...
HTML 表现组成墙壁的块、混凝土和铁。它是使建筑坚固的基础。CSS 表现室内和外部设计，使建筑看起来美观。JavaScript 表现电力和供水系统，确保水和电的持续供应。
