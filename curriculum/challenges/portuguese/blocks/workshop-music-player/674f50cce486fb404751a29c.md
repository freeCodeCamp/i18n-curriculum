...
Agora que você sabe como encontrar a próxima música para tocar, crie uma função chamada `playNextSong`. Dentro da função `playNextSong`, crie uma declaração `if` para verificar se a `currentSong` de `userData` é estritamente igual a `null`. Isso verificará se não há nenhuma música tocando atualmente no objeto `userData`.

Se a condição for verdadeira, chame a função `playSong` com o `id` da primeira música no array `userData.songs` como argumento.

Então adicione uma declaração `return` ao final do bloco `if`.

# --hints--

Você deve ter uma função chamada `playNextSong`.

```js
assert.isFunction(playNextSong);
```

Quando o `currentSong` de `userData` for estritamente igual a `null`, sua função `playNextSong` deve chamar a função `playSong` com o `id` da primeira música no array `userData.songs` como argumento.
...
Você deve ter uma declaração `return` ao final do bloco `if`.
