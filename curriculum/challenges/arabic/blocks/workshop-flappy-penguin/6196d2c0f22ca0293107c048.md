...
أعطِ عنصر `.sun` قيمة `width` و `height` مقدارها `200px`، و`background-color` بلون `yellow`.

# --hints--

يجب أن تستخدم المحدِّد `.sun`.

```js
assert.match(code, /\.sun\s*\{/);
```

يجب أن تعطي `.sun` قيمة `width` مقدارها `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.width, '200px');
```

يجب أن تعطي `.sun` قيمة `height` مقدارها `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.height, '200px');
```

يجب أن تعطي `.sun` خاصية `background-color` بقيمة `yellow`.
