...
يجب عليك إضافة عنصرين `div` إلى `.penguin`. متوقع عدد `--fcc-expected--` من عناصر `.penguin > div`، لكن وجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin > div')?.length, 2);
```

يجب عليك إعطاء أول عنصر `div` الـ `class` بقيمة `penguin-head`.

```js
assert.include(document.querySelector('.penguin > div:nth-of-type(1)')?.className, 'penguin-head');
```

يجب عليك إعطاء ثاني عنصر `div` الـ `class` بقيمة `penguin-body`.
