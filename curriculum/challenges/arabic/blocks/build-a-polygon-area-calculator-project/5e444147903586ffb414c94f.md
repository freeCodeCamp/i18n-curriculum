...
title: بناء مشروع حاسبة مساحة مضلع
challengeType: 23
...
في هذا المشروع ستستخدم البرمجة كائنية التوجه لإنشاء صنف `Rectangle` وصنف `Square`. يجب أن يكون صنف `Square` فرعًا من صنف `Rectangle`، ويرث طرقه وخصائصه.

## صنف Rectangle

عند إنشاء كائن من صنف Rectangle، يجب تهيئته بخاصيتي `width` و `height`. يجب أن يحتوي الصنف أيضًا على الطرق التالية:

- `set_width`
- `set_height`
- `get_area`: يُرجع المساحة (`width * height`)
- `get_perimeter`: يُرجع المحيط (`2 * width + 2 * height`)
- `get_diagonal`: يُرجع القطر (`(width ** 2 + height ** 2) ** .5`)
...
- `get_amount_inside`: يأخذ شكلًا آخر (مربع أو مستطيل) كمعلمة. يُرجع عدد المرات التي يمكن أن يتسع فيها الشكل الممرر داخل الشكل الحالي (بدون تدوير). على سبيل المثال، مستطيل بعرض 4 وارتفاع 8 يمكن أن يحتوي على مربعين بأضلاع 4.

بالإضافة إلى ذلك، إذا تم تمثيل نسخة من `Rectangle` كسلسلة نصية، يجب أن تبدو هكذا: `'Rectangle(width=5, height=10)'`.

## صنف Square

يجب أن يكون صنف `Square` فرعًا من صنف `Rectangle`. عند إنشاء كائن من `Square`، يتم تمرير طول ضلع واحد. يجب أن يخزن دالة `__init__` طول الضلع في خاصيتي `width` و `height` من صنف `Rectangle`.

يجب أن يستطيع صنف `Square` الوصول إلى طرق صنف `Rectangle`، لكنه يجب أن يحتوي أيضًا على طريقة `set_side`. إذا تم تمثيل نسخة من `Square` كسلسلة نصية، يجب أن تبدو هكذا: `'Square(side=9)'`.

بالإضافة إلى ذلك، يجب أن تقوم طريقتا `set_width` و `set_height` في صنف `Square` بضبط كل من العرض والارتفاع.
...
يجب أن يكون صنف `Square` صنفًا مميزًا مختلفًا عن صنف `Rectangle`.
...
يجب أن يكون كائن المربع نسخة من صنف `Square` وصنف `Rectangle` معًا.
...
يجب أن يكون التمثيل النصي لـ `Rectangle(3, 6)` هو `'Rectangle(width=3, height=6)'`.
...
يجب أن يكون التمثيل النصي لـ `Square(5)` هو `'Square(side=5)'`.
...
يجب أن يكون لنسخة من صنف `Rectangle` تمثيل نصي مختلف بعد ضبط قيم جديدة.
...
يجب أن يكون لنسخة من صنف `Square` تمثيل نصي مختلف بعد ضبط قيم جديدة باستخدام `.set_side()`.
...
يجب أن يكون لنسخة من صنف `Square` تمثيل نصي مختلف بعد ضبط قيم جديدة باستخدام `.set_width()` أو `set_height()`.
...
يجب أن تُرجع طريقة `.get_picture()` تمثيلًا نصيًا مختلفًا لنسخة من صنف `Rectangle`.
...
يجب أن تُرجع طريقة `.get_picture()` تمثيلًا نصيًا مختلفًا لنسخة من صنف `Square`.
...
يجب أن تُرجع طريقة `.get_picture()` السلسلة `'Too big for picture.'` إذا كانت خاصيتي `width` أو `height` أكبر من `50`.
