...
title: Ein Projekt zum Erstellen eines Zeitrechners
challengeType: 23
...
Schreiben Sie eine Funktion namens `add_time`, die zwei erforderliche Parameter und einen optionalen Parameter entgegennimmt:

- eine Startzeit im 12-Stunden-Format (endet mit AM oder PM)
- eine Dauerzeit, die die Anzahl der Stunden und Minuten angibt
- (optional) ein Starttag der Woche, ohne Berücksichtigung der Groß- und Kleinschreibung

Die Funktion soll die Dauerzeit zur Startzeit addieren und das Ergebnis zurückgeben.

Wenn das Ergebnis am nächsten Tag liegt, soll `(next day)` nach der Zeit angezeigt werden. Wenn das Ergebnis mehr als einen Tag später liegt, soll `(n days later)` nach der Zeit angezeigt werden, wobei „n“ die Anzahl der Tage später ist.

Wenn der Funktion der optionale Starttag der Woche übergeben wird, soll die Ausgabe den Wochentag des Ergebnisses anzeigen. Der Wochentag in der Ausgabe soll nach der Zeit und vor der Angabe der Anzahl der Tage später erscheinen.

Nachfolgend finden Sie einige Beispiele für verschiedene Fälle, die die Funktion behandeln soll. Achten Sie genau auf die Abstände und die Zeichensetzung der Ergebnisse.
...
Importieren Sie keine Python-Bibliotheken. Gehen Sie davon aus, dass die Startzeiten gültige Zeiten sind. Die Minuten in der Dauerzeit sind eine ganze Zahl kleiner als 60, aber die Stunden können jede ganze Zahl sein.

Hinweis: Öffnen Sie die Browser-Konsole mit F12, um eine ausführlichere Ausgabe der Tests zu sehen.

# --hints--
Ein Aufruf von `add_time('3:30 PM', '2:12')` sollte `'5:42 PM'` zurückgeben.
...
Ein Aufruf von `add_time('11:55 AM', '3:12')` sollte `'3:07 PM'` zurückgeben.
...
Die erwartete Zeit endet mit `'(next day)'`, wenn es der nächste Tag ist.
...
Die erwartete Tageszeit ändert sich von `AM` zu `PM` um `12:00`.
...
Ein Aufruf von `add_time('2:59 AM', '24:00')` sollte `'2:59 AM (next day)'` zurückgeben.
...
Ein Aufruf von `add_time('11:59 PM', '24:05')` sollte `'12:04 AM (2 days later)'` zurückgeben.
...
Ein Aufruf von `add_time('8:16 PM', '466:02')` sollte `'6:18 AM (20 days later)'` zurückgeben.
...
Es wird erwartet, dass das Hinzufügen von `0:00` die Anfangszeit zurückgibt.
...
Ein Aufruf von `add_time('3:30 PM', '2:12', 'Monday')` sollte `'5:42 PM, Monday'` zurückgeben.
...
Ein Aufruf von `add_time('2:59 AM', '24:00', 'saturDay')` sollte `'2:59 AM, Sunday (next day)'` zurückgeben.
...
Ein Aufruf von `add_time('11:59 PM', '24:05', 'Wednesday')` sollte `'12:04 AM, Friday (2 days later)'` zurückgeben.
...
Ein Aufruf von `add_time('8:16 PM', '466:02', 'tuesday') ` sollte `'6:18 AM, Monday (20 days later)'` zurückgeben.
