...
大多数猫的耳朵和耳朵内部的颜色不同。 你也可以这样做。 这就是为什么你之前为左右内耳都定义了一个 `div` 元素。

使用类选择器，为你的 `.cat-left-inner-ear` 元素添加左右各 `20px solid transparent` 的边框。 还要给它一个底部边框，样式为 `40px solid #3b3b4f`。

# --hints--

你应该有一个 `.cat-left-inner-ear` 选择器。

```js 
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear'))
```

你的 `.cat-left-inner-ear` 选择器应该有一个 `border-left` 属性，设置为 `20px solid transparent`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderLeft, '20px solid transparent')
```

你的 `.cat-left-inner-ear` 选择器应该有一个 `border-right` 属性，设置为 `20px solid transparent`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderRight, '20px solid transparent')
```

你的 `.cat-left-inner-ear` 选择器应该有一个 `border-bottom` 属性，设置为 `40px solid #3b3b4f`。
