...
Tumia njia za `classList` na `remove()` kuondoa darasa la vitu la `playing` kutoka kwa `playButton`, kwa sababu wimbo utaachwa kwa muda katika hatua hii.

Ili kuacha wimbo kabisa, tumia njia ya `pause()` kwenye kigezo cha `audio`. `pause()` ni njia ya Web Audio API kwa ajili ya kuacha faili za muziki kwa muda.

# --hints--

Haupaswi kubadilisha kitendakazi cha `pauseSong` kilichopo na yaliyomo ndani yake.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

Unapaswa kutumia sifa ya `classList` na njia ya `remove()` kuondoa darasa la vitu la `"playing"` kutoka kwa `playButton`.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

Unapaswa kutumia njia ya `pause()` kwenye kigezo chako cha `audio`.
