...
Sasa utaunda setter kwa sifa ya kipengele `_level`. Unda njia iitwayo `level` yenye vigezo `self` na `new_level`. Pamba njia hiyo kwa `@level.setter`.

Ndani ya njia, weka `self._level` kuwa `new_level`.

# --hints--

Darasa lako la `Employee` linapaswa kuwa na njia ya `level` yenye vigezo `self` na `new_level`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("level")[1].has_args("self, new_level")`)) })
```

Njia yako ya `level` inapaswa kupambwa na `@level.setter`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("level")[1].has_decorators("level.setter")`)) })
```

Njia yako ya `level` inapaswa kuweka `self._level` kuwa `new_level`.
