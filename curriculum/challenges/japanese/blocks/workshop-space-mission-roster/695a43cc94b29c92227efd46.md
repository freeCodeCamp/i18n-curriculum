...
スクリプトに`console.log()`の呼び出しを追加して`EVAChunks`配列を検証しましたが、その呼び出しはもう必要ありません。次のステップのためにターミナルをすっきりさせるため、`for loop`からのデータをログに記録している`EVAChunks`ブロックを削除してください。

# --hints--

`for loop`からのデータをログに記録している`EVAChunks`ブロックを削除してください。両方のループを含むブロック全体を削除してください。
