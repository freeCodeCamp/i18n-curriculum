---
id: 59f40b17e79dbf1ab720ed7a
title: Abteilungsnummern
challengeType: 1
forumTopicId: 302249
dashedName: department-numbers
---

# --description--

Es gibt eine sehr gut organisierte Stadt, die beschlossen hat, jeder ihrer Abteilungen eine Nummer zuzuweisen:

<ul>
  <li>Polizeiabteilung</li>
  <li>Reinigungsabteilung</li>
  <li>Feuerwehrabteilung</li>
</ul>

Jede Abteilung kann eine Nummer zwischen 1 und 7 (einschließlich) haben.

Die drei Abteilungsnummern müssen einzigartig sein (unterschiedlich voneinander) und zusammen die Zahl 12 ergeben.

Der Polizeichef mag keine ungeraden Zahlen und möchte für seine Abteilung eine gerade Zahl haben.

# --instructions--

Schreiben Sie ein Programm, das alle gültigen Kombinationen als Array ausgibt.

```js
[2, 3, 7] [2, 4, 6] [2, 6, 4]
[2, 7, 3] [4, 1, 7] [4, 2, 6]
[4, 3, 5] [4, 5, 3] [4, 6, 2]
[4, 7, 1] [6, 1, 5] [6, 2, 4]
[6, 4, 2] [6, 5, 1]
```

# --before-each--

```js
const nums = [1, 2, 3, 4, 5, 6, 7];
const total = 12;
const len = 14;
const result = [
  [2, 3, 7],
  [2, 4, 6],
  [2, 6, 4],
  [2, 7, 3],
  [4, 1, 7],
  [4, 2, 6],
  [4, 3, 5],
  [4, 5, 3],
  [4, 6, 2],
  [4, 7, 1],
  [6, 1, 5],
  [6, 2, 4],
  [6, 4, 2],
  [6, 5, 1]
];
```

# --hints--

`combinations` sollte eine Funktion sein.

```js
assert(typeof combinations === 'function');
```

`combinations([1, 2, 3], 6)` sollte ein Array zurückgeben.

```js
assert(Array.isArray(combinations([1, 2, 3], 6)));
```

`combinations([1, 2, 3, 4, 5, 6, 7], 12)` sollte ein Array der Länge 14 zurückgeben.

```js
assert(combinations(nums, total).length === len);
```

`combinations([1, 2, 3, 4, 5, 6, 7], 12)` sollte alle gültigen Kombinationen zurückgeben.

```js
assert.deepEqual(combinations(nums, total), result);
```

# --seed--

## --seed-contents--

```js
function combinations(possibleNumbers, total) {

  return true;
}
```

# --solutions--

```js
function combinations(possibleNumbers, total) {
  let firstNumber;
  let secondNumber;
  let thirdNumber;
  const allCombinations = [];

  for (let i = 0; i < possibleNumbers.length; i += 1) {
    firstNumber = possibleNumbers[i];

    if (firstNumber % 2 === 0) {
      for (let j = 0; j < possibleNumbers.length; j += 1) {
        secondNumber = possibleNumbers[j];

        if (j !== i && firstNumber + secondNumber <= total) {
          thirdNumber = total - firstNumber - secondNumber;

          if (thirdNumber !== firstNumber && thirdNumber !== secondNumber && possibleNumbers.includes(thirdNumber)) {
            allCombinations.push([firstNumber, secondNumber, thirdNumber]);
          }
        }
      }
    }
  }
  return allCombinations;
}
```
