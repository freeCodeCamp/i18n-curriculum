...
現在是處理最後一個問題的時候了。如果待辦任務的名稱或條款描述中包含像引號這樣的特殊字元，應用程式就會當掉。雖然看起來不是如此，但正確的條款會顯示為遺失。  

為了解決這個問題，我們需要創建一個名為 `removeSpecialChars` 的函式（程式），該函式接受一個字串作為輸入並移除所有特殊字元。 

# --hints--

你應該定義一個 `removeSpecialChars` 函式（程式）。

```js
assert.isFunction(removeSpecialChars);
```

你的 `removeSpecialChars` 不應該移除空格。
