...
Recuerda que la propiedad `grid-column` determina en qué columnas comienza y termina un elemento. Puede haber momentos en los que no estés seguro de cuántas columnas tendrá tu cuadrícula, pero deseas que un elemento se detenga en la última columna. Para hacer esto, puedes usar `-1` para la columna final.

Crea un selector `.hero` y asígnale una propiedad `grid-column` establecida en `1 / -1`. Esto indicará al elemento que abarque todo el ancho de la cuadrícula.

# --hints--

Debes tener un selector `.hero`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero'));
```

Tu selector `.hero` debe tener una propiedad `grid-column` establecida en `1 / -1`.
