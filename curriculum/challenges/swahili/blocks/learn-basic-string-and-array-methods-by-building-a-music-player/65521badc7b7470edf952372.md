...
Fafanua kitendakazi cha `pauseSong` kwa kutumia neno kuu la `const` na sintaksia ya kitendakazi cha mshale. Kitendakazi hicho hakipaswi kuchukua vigezo vyovyote.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi tupu kinachoitwa `pauseSong`.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` inapaswa kuwa kitendakazi.

```js
assert.isFunction(pauseSong)
```

Kitendakazi chako cha `pauseSong` kinapaswa kutumia sintaksia ya mshale.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Kitendakazi chako cha `pauseSong` kinapaswa kuwa tupu.
