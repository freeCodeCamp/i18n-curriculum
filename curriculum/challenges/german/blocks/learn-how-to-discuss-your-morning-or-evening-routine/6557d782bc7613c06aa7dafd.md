...
Das Wort `never` ist ein Adverb mit der Bedeutung `not at any time` oder `not ever`. Es wird oft verwendet um über Dinge zu sprechen, die jemand nicht tut oder getan hat. Hier ein Beispiel:
...
The word `thought` is the past tense of `think`. `Thought of that` means to have considered or come up with an idea in the past. Hier ein Beispiel:
