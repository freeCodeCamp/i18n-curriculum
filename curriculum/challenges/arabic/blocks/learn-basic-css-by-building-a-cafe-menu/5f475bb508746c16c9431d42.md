...
الصورة التي أضفتها ليست في الوسط أفقيًا مثل عنوان `Coffee` أعلاها. عناصر `img` "تشبه" العناصر المضمَّنة.

لجعل الصورة تتصرف مثل عناصر مستوى الكتلة مثل العناوين، أنشئ محدِّد نوع `img`. اضبط خاصية `display` على `block`، واستخدم خاصيتي `margin-left` و`margin-right` لمركزة الصورة أفقيًا.

# --hints--

يجب أن تستخدم محدِّد `img`.
...
يجب أن تضبط خاصية `display` على `block`.
...
يجب أن تضبط خاصية `margin-left` على `auto`.
...
يجب أن تضبط خاصية `margin-right` على `auto`.
...
يجب أن يكون لعنصر `img` خاصيتي `margin-left` و`margin-right` بقيمة `auto`.
