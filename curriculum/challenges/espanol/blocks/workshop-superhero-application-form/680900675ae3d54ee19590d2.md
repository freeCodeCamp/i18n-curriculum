...
Cree un elemento `button` con el `className` de `submit-btn` y un `type` de `submit`. Dé al botón el texto `Join the League`.

Después de hacer eso, intente enviar el formulario y vea qué sucede.
...
Su elemento botón debe tener un atributo `className` configurado como `submit-btn`.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

Su elemento `button` debe tener `Join the League` como su texto.
