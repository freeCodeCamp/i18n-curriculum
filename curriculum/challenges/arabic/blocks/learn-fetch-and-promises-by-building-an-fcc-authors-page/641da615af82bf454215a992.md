...
مرر دالة استدعاء راجع فارغة إلى الطريقة `.forEach()`. بالنسبة للمعلمة الأولى من دالة الاستدعاء الراجع، فكك العناصر `author` و`image` و`url` و`bio`.

بالنسبة للمعلمة الثانية، مرر `index`. هذا سيمثل موقع كل مؤلف، وسيكون مفيدًا للتقسيم إلى صفحات لاحقًا.
...
يجب أن تفكك `author` و`image` و`url` و`bio` كالمعلمة الأولى لدالة الاستدعاء الراجع. لا تنسَ وضعها داخل الأقواس المتعرجة.
...
يجب أن تفكك `index` كالمعلمة الثانية لدالة الاستدعاء الراجع. يجب أن تكون خارج الأقواس المتعرجة التي وضعت فيها العناصر الأخرى.

```js
assert.match(code, /authors\.forEach\(\s*\(\s*\{.*\}\s*,\s*index\s*\)/)
```

يجب أن تستخدم دالة الاستدعاء الراجع صيغة السهم.
