...
O método <dfn>find()</dfn> recupera o primeiro elemento dentro de um array que atende às condições especificadas na função de callback fornecida. Se nenhum elemento satisfizer a condição, o método retorna `undefined`.
...
Use `const` para criar uma variável chamada `song` e atribuir a ela o resultado da chamada do método `find()` no array `userData?.songs`. Use `song` como parâmetro do callback `find()` e verifique se `song.id` é estritamente igual a `id`.

Isso irá iterar pelo array `userData?.songs`, procurando por uma música que corresponda ao `id` passado para a função `playSong`.

# --hints--

Você deve usar o método `find` em `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

Seu método `find` deve ter `song` como parâmetro da sua função callback de seta.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

Seu método `find` deve usar igualdade estrita para verificar se `song.id` é igual a `id`.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

Seu método `find` deve ser atribuído a uma constante `song`.
