...
Deklariere eine `apply`-Funktion, die einen `fn`- und `args`-Parameter verwendet.

# --hints--

Du solltest eine `apply`-Variable deklarieren.
...
Du solltest `const`-verwenden, um deine `apply`-Variable zu deklarieren.
...
Deiner `apply`-Variablen sollte eine Arrow-Funktion zugewiesen sein.
...
Deine `apply`-Funktion sollte `fn` als ersten Parameter haben.
...
Deine `apply`-Funktion sollte `args` als zweiten Parameter haben.
...
Deine `apply`-Funktion sollte leer sein.
