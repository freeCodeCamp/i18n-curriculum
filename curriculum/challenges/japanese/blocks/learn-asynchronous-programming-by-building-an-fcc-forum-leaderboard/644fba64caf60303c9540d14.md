...
`href` 属性に続けて、`class` 属性を const 変数の `linkClass` に設定してください。

After the `class` attribute, set the `target` attribute to `"_blank"`.

最後に、リンクテキストを表示するために、const 変数 `linkText` をアンカータグの間に入れてください。

# --hints--

`href` 属性の後に、`class` 属性を追加してください。
...
`class` 属性の後に、`target` 属性を追加してください。
