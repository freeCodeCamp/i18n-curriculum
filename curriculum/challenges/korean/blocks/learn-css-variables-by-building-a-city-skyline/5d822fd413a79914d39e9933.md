...
방금 추가한 `repeating-linear-gradient` 아래에 또 다른 repeating-linear-gradient를 추가하세요. `90deg` 방향을 주고, `0%`에서 `12%`까지는 건물 색상, `12%`에서 `44%`까지는 창문 색상을 사용하세요. 이렇게 하면 여러 개의 직사각형 창문이 만들어집니다.
