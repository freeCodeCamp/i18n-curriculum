...
現在是時候呼叫 `displayAuthors` 函式了。但你仍然不想一次將所有作者填入頁面。相反地，你可以使用 `startingIndex` 和 `endingIndex` 變數擷取部分作者。做到這點的最佳方法是使用 `.slice()` 陣列方法。

首先，移除顯示 `authorDataArr` 的主控台述語。然後，使用 `authorDataArr` 陣列和 `.slice()` 呼叫 `displayAuthors` 函式，起始點使用 `startingIndex` 變數，結束點使用 `endingIndex` 變數。
...
你應該將 `data` 指定到 `authorDataArr` 變數。

```js
assert.match(code, /authorDataArr\s*=\s*data\s*;?/)
```

你應該移除顯示 `authorDataArr` 的主控台記錄述語。
