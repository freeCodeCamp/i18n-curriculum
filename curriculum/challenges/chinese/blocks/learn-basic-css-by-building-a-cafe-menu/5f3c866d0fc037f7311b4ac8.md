...
更接近了，但是价格没有保持在右边。 这是由于 `inline-block` 元素只会占用其内容的宽度。

为了将它们展开，请给 `flavor` 和 `price` 类选择器添加 `width` 属性，且每个属性的值均为 `50%`。

# --hints--

你应该在你的 `.flavor` 选择器中将 `width` 属性设置为 `50%`。
...
你应该在你的 `.price` 选择器中将 `width` 属性设置为 `50%`。
