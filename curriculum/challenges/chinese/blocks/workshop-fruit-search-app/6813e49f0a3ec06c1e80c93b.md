...
将第一个参数更新为 `setTimeout` 的 `async` 箭头函数。这允许你在延迟逻辑中使用 `await`。在该函数内部，创建一个 `try...catch` 块来处理获取数据时的任何潜在误差。

# --hints--

你应该将一个 `async` 箭头函数作为传递给 `setTimeout` 的第一个参数。

```js
assert.match(code, /setTimeout\s*\(\s*async\s*\(\s*\)\s*=>\s*{/);
```

在 `async` 函数内部，你应该创建一个 `try...catch` 块。
