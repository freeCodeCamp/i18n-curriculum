...
Всередині елемента `button` створіть елемент `span`, який містить `😊`.

Додайте елементу `span` атрибут `role` зі значенням `img` та атрибут `aria-hidden` зі значенням `true`.

Потім створіть другий елемент `span` з `class` `count`, який містить текст `0/10`.

# --hints--

Всередині елемента `button` має бути два елементи `span`.

```js
assert.lengthOf(document.querySelectorAll("button > span"), 2);
```

Перший елемент `span` повинен мати текстовий вміст `😊`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.textContent.trim(), "😊");
```

Перший елемент `span` повинен мати атрибут `role` зі значенням `img`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("role"), "img");
```

Перший елемент `span` повинен мати атрибут `aria-hidden` зі значенням `true`.

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("aria-hidden"), "true");
```

Другий елемент `span` повинен мати атрибут `class` зі значенням `count`.

```js
assert.equal(document.querySelectorAll("button > span")[1]?.className, "count");
```

Другий елемент `span` повинен мати текстовий вміст `0/10`.
