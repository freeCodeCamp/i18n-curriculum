...
この建物にもう一つ繰り返しのグラデーションを追加してください。先ほど追加したものと同じにしますが、`90deg` の方向は追加せず、2つの `transparent` の色の代わりにウィンドウの色を使ってください。

# --hints--

`.fb6` に2つ目の `repeating-linear-gradient` を `background` プロパティで指定してください。
...
2つ目の `repeating-linear-gradient` に、`--building-color3` から `0%` まで `10%` の1つ目の色を指定してください。
...
2つ目の `repeating-linear-gradient` に、`--window-color3` から `10%` まで `30%` の2つ目の色を指定してください。
