...
`.cat-whiskers-right` 요소 안에 클래스가 각각 `div`, `cat-whisker-right-top`, `cat-whisker-right-middle`인 `cat-whisker-right-bottom` 요소 3개를 만드세요.

# --hints--

기존에 있는 클래스 `div`인 `cat-whiskers-right` 요소는 변경하지 마세요.

```js
assert(document.querySelectorAll('div.cat-whiskers-right').length === 1)
```

`div` 요소 안에 `.cat-whiskers-right` 요소 3개를 만드세요.

```js
assert(document.querySelectorAll('.cat-whiskers-right div').length === 3)
```

`div` 요소 안에 있는 첫 번째 `.cat-whiskers-right` 요소는 클래스 `cat-whisker-right-top`을 가져야 합니다.

```js
assert(document.querySelectorAll('.cat-whiskers-right div')[0].classList.contains('cat-whisker-right-top'))
```

`div` 요소 안에 있는 두 번째 `.cat-whiskers-right` 요소는 클래스 `cat-whisker-right-middle`을 가져야 합니다.

```js
assert(document.querySelectorAll('.cat-whiskers-right div')[1].classList.contains('cat-whisker-right-middle'))
```

`div` 요소 안에 있는 세 번째 `.cat-whiskers-right` 요소는 클래스 `cat-whisker-right-bottom`을 가져야 합니다.
