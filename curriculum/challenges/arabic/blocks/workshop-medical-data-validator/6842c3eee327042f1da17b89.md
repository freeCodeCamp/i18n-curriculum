...
أنشئ عبارة `if`. استخدم عامل `not` لنفي `is_sequence` كشرط لها. داخل عبارة `if`، اطبع `Invalid format: expected a list or tuple.` وأرجع `False`.

# --hints--

يجب أن تنشئ عبارة `if` داخل دالة `validate` الخاصة بك.
...
يجب أن يكون شرط عبارة `if` هو `not is_sequence`.
...
يجب أن تطبع `Invalid format: expected a list or tuple.` داخل عبارة `if` الخاصة بك.
...
يجب أن ترجع `False` في نهاية عبارة `if`.
