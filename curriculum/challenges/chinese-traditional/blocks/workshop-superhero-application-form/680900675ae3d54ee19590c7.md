...
在其中創建另一個帶有文字 `Real Name` 的 `label`，並包含一個型別為 `password` 的 `input`。將 `input` 的 `value` 設為 `realName`。 
...
你的 `label` 元素應該以 `Real Name` 作為其文字。
...
你應該在你的 `label` 元素內創建一個型別為 `password` 的 `input`。
...
你的 `input` 元素應該將其值設為 `realName` 狀態變數。

```js
assert.match(code, /value\s*=\s*\{\s*realName\s*\}/)
```

你應該使用 `onChange` 事件來設定 `realName` 狀態變數的值，這樣你才能在 `input` 欄位中輸入。
