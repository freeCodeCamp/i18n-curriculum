...
마지막으로, `p` 요소들 뒤에 `button` 요소를 만드세요. `id`를 `id` 변수 값으로 설정하고, `class`는 `btn add-to-cart-btn`로 하며, 텍스트에는 `"Add to cart"`를 사용하세요.
...
`button` 요소는 `id`을 `id` 변수 값으로 설정해야 합니다.

```js
assert.equal(document.querySelector('.dessert-card button')?.id, '1');
```

`button` 요소는 `class`를 `btn add-to-cart-btn`로 설정해야 합니다.
...
`button` 요소는 텍스트 `"Add to cart"`를 가져야 합니다.
