...
由於電腦儲存和處理數字的方式，涉及小數的計算可能會導致一些奇怪的行為。例如，`0.1 + 0.2` 並不相等於 `0.3`。這是因為電腦將小數儲存為二進位分數，而某些二進位分數無法精確表述為小數分數。

我們想要清理你計算的數字結果。將你的計算包裹在小括弧中（不要包含 `return` 述語！），並對它呼叫 `.toFixed()` 方法。將數字 `2` 作為引數傳遞給 `.toFixed()` 方法。這將把數字四捨五入到小數點後兩位並傳回字串。
