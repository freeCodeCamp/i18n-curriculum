...
Dovresti dare a `.eye` un valore `top` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.top, '45%');
```

Dovresti dare a `.eye` un valore `border-radius` di `--fcc-expected--` invece di `--fcc-actual--`.
