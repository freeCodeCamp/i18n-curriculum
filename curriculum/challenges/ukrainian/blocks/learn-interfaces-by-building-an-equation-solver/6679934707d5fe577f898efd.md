...
Додайте ще один випадок, коли довжина `results` дорівнює `2`. Цього разу призначте `result_list` список, що містить два рядки у форматі `x1 = <root1>` та `x2 = <root2>`. Знову зробіть так, щоб рішення відображало як додатні, так і від’ємні знаки.

# --hints--

Ви не повинні змінювати значення subject у вашій інструкції `match`.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("len(results)")`)) })
```

Ви не повинні змінювати існуючі блоки `case`.
...
Ви повинні створити новий `case` із шаблоном `2` після існуючого блоку `case`.
...
Ви повинні призначити списку, що містить два рядки у форматі `x1 = <root1>` та `x2 = <root2>`, змінній `result_list` всередині тіла вашого нового `case`. Відображайте як додатні, так і від’ємні знаки для результатів.
