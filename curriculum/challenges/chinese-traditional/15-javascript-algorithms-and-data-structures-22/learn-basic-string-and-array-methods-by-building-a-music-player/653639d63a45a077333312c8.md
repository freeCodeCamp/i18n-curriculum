...
在播放歌曲之前，你需要確保它從頭開始播放。 這可以通過使用 `audio` 對象上的 `currentTime` 屬性來實現。

添加 `if` 語句來檢查 `userData?.currentSong` 是否爲假 *或者* `userData?.currentSong.id` 嚴格不等於 `song.id`。 此條件將檢查當前是否沒有播放歌曲或當前歌曲是否與即將播放的歌曲不同。

在 `if` 塊內，將 `audio` 對象的 `currentTime` 屬性設置爲 `0`。

# --hints--

你應該創建一個帶有條件 `userData?.currentSong === null || userData?.currentSong.id !== song.id` 的 `if` 語句。 不要忘記包含可選鏈接。
...
你應該在 `if` 塊內將 `audio.currentTime` 設置爲 `0`。
