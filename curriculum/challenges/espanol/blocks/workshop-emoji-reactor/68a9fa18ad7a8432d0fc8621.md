...
En lecciones anteriores aprendiste que `querySelectorAll` devuelve un `NodeList`, que es una estructura similar a un arreglo, y uno de los métodos que tiene un `NodeList` es `forEach()`, y funciona de la misma manera que el `forEach` que existe en los arreglos.

Itera sobre `btns` con un `forEach()` que añade el mismo event listener a cada botón.

Usa una función callback que tome un parámetro `btn`, y dentro del callback, agrega un listener de evento click que llame a `updateCount(btn)`.

Con esto has terminado el taller.

# --hints--

Debes usar `btns.forEach()` para iterar sobre los botones.

```js
assert.match(code, /btns\.forEach\s*\(/);
```

Todos los botones deben tener listeners de eventos que llamen a `updateCount` cuando se haga clic.
