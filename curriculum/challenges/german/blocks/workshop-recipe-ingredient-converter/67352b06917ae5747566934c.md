...
Aktualisieren Sie schließlich Ihre innere Funktion so, dass sie eine dritte Funktion zurückgibt. Diese dritte Funktion soll ebenfalls ein einzelnes Argument namens `quantity` akzeptieren und einen leeren Körper haben.

# --hints--

Ihre zweite Funktion sollte noch eine weitere Funktion zurückgeben.

```js
assert.isFunction(convertQuantity()());
```

Ihre innerste Funktion sollte einen `quantity`-Parameter haben.

```js
assert.match(convertQuantity()()?.toString(), /\(?quantity\)?/);
```

Ihre innerste Funktion sollte leer sein.
