...
Picha uliyoongeza haijawekwa katikati mlalo kama kichwa cha `Coffee` ikilicho juu yake. Vipengele vya `img` ni "kama" vipengee vya ndani.

Ili kuifanya picha kuwa kama vipengele vya kichwa (ambavyo ni block-level), tengeneza kichaguzi cha aina ya `img` na utumie thamani ya `block` kwa sifa ya `display` na utumie thamani zinazotumika za `margin-left` na `margin-right` ili kuiweka katikati kwa mlalo.
