...
Одразу після того, як ви очистите `multipleCard`, всередині методу `singleCardSelected` викличте функцію `showElements` і передайте `singleCard` як перший аргумент, а `fortuneContainer` — як другий.

# --hints--

Ви повинні викликати функцію `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

Ви повинні передати `elements.singleCard` як перший аргумент.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

Ви повинні передати `elements.fortuneContainer` як другий аргумент.
