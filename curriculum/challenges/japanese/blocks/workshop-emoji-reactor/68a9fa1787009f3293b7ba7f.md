...
コードがかなり繰り返しになっていることに気づいたかもしれません！似たような複数の要素を扱うもっと良い方法があります。

それぞれのボタンを個別に選択する代わりに、`querySelectorAll(".emoji-btn")` を使ってクラス `emoji-btn` を持つすべてのボタンを一度に選択してください。結果は `btns` という変数に保存します。
...
すべての絵文字ボタンを選択するには `querySelectorAll` を使うべきです。

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

結果は `btns` という変数に保存してください。
...
`btns` 変数には4つの絵文字ボタンすべてが含まれている必要があります。
