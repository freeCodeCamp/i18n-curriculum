...
接著，創建一個名為 `forumCategoryUrl` 的 `const` 變數，並將字串 `"https://forum.freecodecamp.org/c/"` 指定給它。 

在那下面，創建另一個名為 `avatarUrl` 的 `const` 變數，並將字串 `"https://sea1.discourse-cdn.com/freecodecamp"` 指定給它。

# --hints--

你應該使用 `const` 宣告一個 `forumCategoryUrl` 變數。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=/);
```

你應該將 `"https://forum.freecodecamp.org/c/"` 指定到你的 `forumCategoryUrl` 變數。

```js
assert.match(code, /const\s+forumCategoryUrl\s*=\s*('|")https:\/\/forum\.freecodecamp\.org\/c\/\1\s*/);
```

你應該使用 `const` 宣告一個 `avatarUrl` 變數。

```js
assert.match(code, /const\s+avatarUrl\s*=/);
```

你應該將 `"https://sea1.discourse-cdn.com/freecodecamp"` 指定給你的 `avatarUrl` 變數。
