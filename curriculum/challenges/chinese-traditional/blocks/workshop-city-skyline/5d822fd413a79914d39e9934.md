...
到最後一步啦！ 你要構建最後一棟建築了！ 爲其添加方向爲 `90deg` 的重複漸變。 從 `0%` 到 `10%` 使用其建築物顏色，從 `10%` 到 `30%` 使用 `transparent`。

# --hints--

你應該在 `background` 屬性中將 `repeating-linear-gradient` 添加到 `.fb6`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

你應該給 `repeating-linear-gradient` 一個 `90deg` 的方向。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

你應該將從 `0%` 到 `10%` 的 `--building-color3` 作爲 `repeating-linear-gradient` 的第一種顏色。
...
你應該將從 `10%` 到 `30%` 的 `transparent` 作爲 `repeating-linear-gradient` 的第二種顏色。
