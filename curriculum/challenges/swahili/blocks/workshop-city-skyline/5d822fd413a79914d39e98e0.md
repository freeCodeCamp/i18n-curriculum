...
Tengeneza sifa mpya ya kigezo chini ya zile zilizopo ikaitwa `--building-color3` na uipe thamani ya `#cc6699`. Kisha itumie kama `background-color` ya darasa la vitu la `.bb3` na uipe thamani ya njia mbadala ya `pink`.

# --hints--

Unapaswa kufafanua sifa mpya ya kigezo iitwayo `--building-color3`.

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color3'));
```

Unapaswa kumpa `--building-color3` thamani ya `#cc6699`.
...
Unapaswa kuweka `background-color` ukitumia sifa ya kigezo ya `--building-color3` na njia mbadala ya `pink`.
