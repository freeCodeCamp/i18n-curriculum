...
為根文件添加 `DOMContentLoaded` 事件，並傳入 `initializeApp` 作為要執行的回呼函式（程式）。這可確保所有初始化僅在 DOM 完全載入後才進行。

# --hints--

你應該為根 `DOMContentLoaded` 添加 `document` 事件。

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

你應該將 `initializeApp` 參考為事件監聽器中要執行的函式（程式）。
