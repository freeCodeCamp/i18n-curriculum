...
Der Rest Ihrer Crew wurde in einem Array namens `remainingCrew` mit den folgenden Daten erstellt:

| id | Name | Rolle | isEVAEligible | Priorität |
| -- | ---- | ---- | ------------- | -------- |
...
Durchlaufen Sie das `remainingCrew`-Array und fügen Sie jeden Astronauten mit der `squad`-Funktion zu `addCrewMember()` hinzu.

# --hints--

Sie sollten eine `for`-Schleife verwenden, um `addCrewMember()` mit jedem Astronauten in `remainingCrew` aufzurufen.
