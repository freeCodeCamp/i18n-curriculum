...
如果你嘗試玩這個遊戲，你會看到你可以玩無限回合。但規則規定第一個獲得一到三分的人獲勝。你想檢查是否有贏家，並顯示一則訊息。

在你的 `showResults` 函式（程式）中，如果玩家已經達到三分，將 `winnerMsgElement` 更新為 `"Player has won the game!"`。如果電腦已經達到三分，將 `winnerMsgElement` 更新為 `"Computer has won the game!"`。

如果有贏家，將 `resetGameBtn` 按鈕的 `display` 設為 `block` 以顯示，並將 `optionsContainer` 的 `display` 設為 `none` 以隱藏。

現在，嘗試玩遊戲，並查看當玩家達到三分時是否顯示勝利者訊息。
...
如果玩家或電腦已經達到三分，你應該隱藏 `optionsContainer`。
