...
Erstelle eine eigene CSS-Variable namens `--penguin-skin` und setze sie auf `gray`. Ersetze dann alle relevanten Eigenschaftswerte damit.

# --hints--

Du solltest `:root` eine `--penguin-skin`-Eigenschaft geben.
...
Du solltest `.penguin-head` einen `background` von `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))` geben.
...
Du solltest `.arm` einen `background` von `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))` geben.
