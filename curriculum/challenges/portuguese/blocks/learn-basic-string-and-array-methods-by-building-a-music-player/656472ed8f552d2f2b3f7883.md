...
Você deve adicionar um bloco `else` ao bloco `if` existente.
...
Você deve definir `audio.currentTime` para `userData?.songCurrentTime` dentro do bloco `else` da sua declaração `if`.
