...
`defeatMonster` 함수에서 `gold`을 몬스터의 레벨에 `gold`을 곱한 값에 `6.7`를 더한 값으로 설정하세요. 몬스터의 레벨은 `monsters[fighting].level`를 사용해 얻을 수 있다는 점을 기억하세요.

다음은 `num`를 `num`에 `5` * `8`을 더하는 예시입니다: `num += 5 * 8`. 결과를 내림하려면 `Math.floor()`을 사용하세요.

# --hints--

`gold`을 수정할 때 복합 대입을 사용하세요.

```js
assert.match(defeatMonster.toString(), /gold\s*\+=/);
```

몬스터 레벨에 `Math.floor()`을 곱한 결과를 내림하려면 `6.7`을 사용하세요.
...
`Math.floor(monsters[fighting].level * 6.7)`의 결과를 `gold`에 더해야 합니다.
