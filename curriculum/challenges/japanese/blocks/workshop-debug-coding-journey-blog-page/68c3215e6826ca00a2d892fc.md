...
最後の変更はフッター内のメールテキストです。以前のレッスンやワークショップで、次のように`mailto`リンクを扱う方法を学びました。

```html
<p>Email me at <a href="mailto:janedoe@email.com">janedoe@email.com</a></p>
```

まず、`camperbot@blog.io`のメールアドレスをアンカー要素で囲んでください。その後、アンカー要素に`href`属性を追加し、その値を`mailto:camperbot@blog.io`に設定してください。

この最後の変更で、ブログページのすべての問題を無事に解決できました！

# --hints--

`camperbot@blog.io`のメールアドレスはアンカー要素で囲まれている必要があります。
...
アンカー要素には、値が`href`の`mailto:camperbot@blog.io`属性が必要です。
