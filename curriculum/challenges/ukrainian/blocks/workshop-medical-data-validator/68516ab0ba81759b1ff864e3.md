...
Функція `all` повертає `True`, якщо всі елементи з ітерованого об’єкта, переданого їй, є істинними, інакше повертає `False`:
...
Передайте список `[isinstance(i, str) for i in medications]` у функцію `all`, щоб переконатися, що кожен елемент у ньому є рядком.

# --hints--

Ви маєте змінити значення свого ключа `medications`, передавши `[isinstance(i, str) for i in medications]` у функцію `all`.
