...
給定一組正整數，撰寫一個函式（程式）來排序這些整數，使數字串接後形成最大的整數，並傳回此整數。

# --hints--

`maxCombine` 應該是一個函式（程式）。

```js
assert(typeof maxCombine == 'function');
```

`maxCombine([1, 3, 3, 4, 55])` 應該傳回一個數字。

```js
assert(typeof maxCombine([1, 3, 3, 4, 55]) == 'number');
```

`maxCombine([1, 3, 3, 4, 55])` 應該傳回 `554331`。

```js
assert.equal(maxCombine([1, 3, 3, 4, 55]), 554331);
```

`maxCombine([71, 45, 23, 4, 5])` 應該傳回 `71545423`。

```js
assert.equal(maxCombine([71, 45, 23, 4, 5]), 71545423);
```

`maxCombine([14, 43, 53, 114, 55])` 應該傳回 `55534314114`。

```js
assert.equal(maxCombine([14, 43, 53, 114, 55]), 55534314114);
```

`maxCombine([1, 34, 3, 98, 9, 76, 45, 4])` 應該傳回 `998764543431`。

```js
assert.equal(maxCombine([1, 34, 3, 98, 9, 76, 45, 4]), 998764543431);
```

`maxCombine([54, 546, 548, 60])` 應該傳回 `6054854654`。
