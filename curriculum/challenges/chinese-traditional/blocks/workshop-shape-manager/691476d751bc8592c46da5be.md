...
你需要一個方法來回應使用者在下拉選單中更改形狀選擇的動作。為此，創建一個帶有 `handleShapeSelect` 參數且參數型別為 `e` 的 `Event` 函式（程式）。

此函式將控制代碼下拉選單的變更，方法是清除所有輸入、阻止預設行為、驗證選擇、根據是否選擇了形狀來顯示或隱藏結果卡、顯示所選形狀的適當輸入欄位，並重新計算面積。

目前先將函式留空。

# --hints--

你應該創建一個 `handleShapeSelect` 函式。

```js
assert.isFunction(handleShapeSelect)
```

你的 `handleShapeSelect` 函式應該有一個 `e` 參數，型別為 `Event`。
