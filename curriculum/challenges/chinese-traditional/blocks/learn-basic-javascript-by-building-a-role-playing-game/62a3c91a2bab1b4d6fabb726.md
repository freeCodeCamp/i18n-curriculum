...
現在創建一個 `goCave` 函式（程式），將 `"Going to cave."` 列印到主控台。

# --hints--

你應該使用 `function` 關鍵字來宣告 `goCave`。
...
你的程式碼中應該有一行 `console.log("Going to cave.");`。

```js
assert.match(code, /console\.log\(\s*('|")Going to cave\.\1\s*\)/);
```

你的 `console.log("Going to cave.");` 行應該放在你的 `goCave` 函式（程式）內。
