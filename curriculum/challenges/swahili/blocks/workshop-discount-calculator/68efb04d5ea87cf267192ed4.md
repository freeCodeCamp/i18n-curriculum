...
Unda mfano wa `DiscountEngine` na uhifadhi katika kigezo `engine`. Baada ya hapo, hesabu bei bora kwa kutumia njia ya `calculate_best_price` na uhifadhi katika kigezo kinachoitwa `best_price`.

# --hints--

Unapaswa kuunda mfano wa `DiscountEngine` kwa kutumia orodha ya `strategies`.
...
Unapaswa kuita `engine.calculate_best_price(product, user_tier)` na kuhifadhi matokeo katika `best_price`.
