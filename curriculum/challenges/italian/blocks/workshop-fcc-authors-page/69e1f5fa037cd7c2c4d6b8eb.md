...
Infine, devi gestire il metodo `catch`. Per prima cosa, annida l'istruzione `fetch`, l'istruzione `json` e la chiamata a displayAuthors in un blocco `try` all'interno di `initialFetch`. Poi, modifica il metodo `catch` concatenato in modo che diventi un blocco catch, come mostrato nell'esempio qui sotto.
...
Se hai fatto tutto correttamente, il blocco `try` conterrà tre righe di codice e il blocco `catch` conterrà solo una riga di codice. Inoltre, `SyntaxError` sarà risolto e l'app dovrebbe funzionare normalmente.

Complimenti! Hai imparato come rendere il tuo codice asincrono più leggibile usando gli operatori `async` e `await`! Ora la tua pagina degli autori è completa.
...
Dovresti creare un blocco `try/catch`

```js
assert.match(code, /try\s*\{[\s\S]*?\}\s*catch\s*\([^)]*\)\s*\{/);
```

`catch` non dovrebbe più essere un metodo concatenato

```js
assert.notMatch(code, /\.catch/);
```

Il blocco `try` dovrebbe contenere le tre istruzioni che recuperano i dati, creano il `json` e poi lo mostrano.
...
La pagina fCC Authors dovrebbe renderizzare correttamente
