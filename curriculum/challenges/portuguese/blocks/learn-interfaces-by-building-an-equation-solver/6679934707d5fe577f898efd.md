...
Adicione outro caso para quando o comprimento de `results` for `2`. Desta vez, atribua a `result_list` uma lista contendo duas strings com o formato `x1 = <root1>` e `x2 = <root2>`. Novamente, faça a solução exibir os sinais positivo e negativo.
...
Você não deve modificar seus blocos `case` existentes.
...
Você deve criar um novo `case` com o padrão `2` após o bloco `case` existente.
...
Você deve atribuir uma lista contendo duas strings com o formato `x1 = <root1>` e `x2 = <root2>` para `result_list` dentro do seu novo corpo `case`. Exiba os sinais positivos e negativos para os resultados.
