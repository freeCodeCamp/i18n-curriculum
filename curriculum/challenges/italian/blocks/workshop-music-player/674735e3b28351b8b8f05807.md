...
Il metodo `find()` recupera il primo elemento all'interno di un array che soddisfa le condizioni specificate nella funzione di callback fornita. Se nessun elemento soddisfa la condizione, il metodo restituisce `undefined`.
...
All'interno della tua funzione `playSong`, usa il metodo `find()` sull'array `userData.songs` per cercare una canzone che abbia un `id` strettamente uguale al `id` passato alla funzione `playSong`. Assegna la chiamata `find` a una variabile chiamata `song`.

# --hints--

La tua funzione `playSong` dovrebbe chiamare il metodo `find` su `userData.songs`.
...
La tua chiamata `find` dovrebbe restituire una canzone il cui `id` è strettamente uguale al `id` passato alla funzione `playSong`.
...
Dovresti assegnare la tua chiamata `find` a una variabile chiamata `song`.
