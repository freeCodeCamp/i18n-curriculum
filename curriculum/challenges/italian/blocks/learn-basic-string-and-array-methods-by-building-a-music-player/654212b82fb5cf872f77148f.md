...
All'interno dell'elemento `button`, crea altri due elementi `span`. 

Il primo elemento `span` dovrebbe avere un `class` di `playlist-song-artist`. Tra i tag `span`, aggiungi `${song.artist}`. 

Il secondo elemento `span` dovrebbe avere un `class` di `playlist-song-duration`. Tra i tag `span`, aggiungi `${song.duration}`. 

# --hints--

Non dovresti modificare l'elemento `button` esistente e il suo contenuto.
...
Dovresti creare un elemento `span` con la classe `playlist-song-artist`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

Il tuo elemento `span` con la classe `playlist-song-artist` dovrebbe interpolare `song.artist` come contenuto.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

Dovresti creare un elemento `span` con la classe `playlist-song-duration`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

Il tuo elemento `span` con la classe `playlist-song-duration` dovrebbe interpolare `song.duration` come contenuto.
