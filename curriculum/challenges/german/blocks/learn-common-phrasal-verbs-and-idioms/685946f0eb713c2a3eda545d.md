---
id: 685946f0eb713c2a3eda545d
title: Aufgabe 115
challengeType: 22
dashedName: task-115
lang: en-US
---

<!-- (Audio) Bob: Hi Sophie, I was thinking we could schedule it for the last week of this month. -->

# --instructions--

Hören Sie sich die Audioaufnahme an und vervollständigen Sie den folgenden Satz.

# --fillInTheBlank--

## --sentence--

`Hi Sophie, I was thinking we could BLANK it for the last week of this month.`

## --blanks--

`schedule`

### --feedback--

Das bedeutet, etwas für ein bestimmtes Datum oder eine bestimmte Uhrzeit zu planen.

# --explanation--

`To schedule` bedeutet, eine bestimmte Zeit und ein bestimmtes Datum für das Stattfinden eines Ereignisses festzulegen. Zum Beispiel:

`Let's schedule the meeting for 10 a.m. on Tuesday.` – Das bedeutet, dass das Treffen so geplant wird, dass es zu dieser Zeit stattfindet.

# --scene--

```json
{
  "setup": {
    "background": "company2-boardroom.png",
    "characters": [
      {
        "character": "Bob",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_24-3.mp3",
      "startTime": 1,
      "startTimestamp": 6.72,
      "finishTimestamp": 10.42
    }
  },
  "commands": [
    {
      "character": "Bob",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Bob",
      "startTime": 1,
      "finishTime": 3.22,
      "dialogue": {
        "text": "Hi Sophie, I was thinking we could schedule it",
        "align": "center"
      }
    },
    {
      "character": "Bob",
      "startTime": 3.22,
      "finishTime": 4.7,
      "dialogue": {
        "text": "for the last week of this month.",
        "align": "center"
      }
    },
    {
      "character": "Bob",
      "opacity": 0,
      "startTime": 5.2
    }
  ]
}
```
