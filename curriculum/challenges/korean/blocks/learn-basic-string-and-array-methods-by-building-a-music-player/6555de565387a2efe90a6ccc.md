...
`setPlayButtonAccessibleText` 함수는 `aria-label` 속성을 현재 노래로 설정하거나, 재생 목록의 첫 번째 노래로 설정합니다. 그리고 재생 목록이 비어 있으면 `aria-label`를 `"Play"`으로 설정합니다.

`setAttribute` 요소에서 `playButton` 메서드를 사용해 `"aria-label"`라는 이름의 속성을 설정하세요. 삼항 연산자를 사용해 `Play ${song.title}`가 없으면 속성 값을 `"Play"` 또는 `song?.title`로 설정하세요.

여기서는 문자열 보간이 필요하니 백틱을 사용해야 한다는 점을 잊지 마세요.

# --hints--

기존 `setPlayButtonAccessibleText` 함수와 그 내용을 수정하지 마세요.
...
`setAttribute()`에서 `playButton` 메서드를 사용해야 합니다.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` 메서드의 첫 번째 값이 `setAttribute()`이어야 합니다.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

`setAttribute` 메서드의 두 번째 인수는 ``song?.title ? `Play ${song.title}` : "Play"``이어야 합니다.
