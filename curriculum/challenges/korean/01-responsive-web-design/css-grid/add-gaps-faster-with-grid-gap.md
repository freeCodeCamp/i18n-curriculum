...
`grid-gap`은 이전 과제에서 살펴본 `grid-row-gap`과 `grid-column-gap`의 축약 속성이며 사용하기 더 용이합니다. `grid-gap`에 하나의 값만 지정하면, 모든 행과 열 사이에 동일한 간격이 생성됩니다. 그러나 값이 두 개라면 첫번째 값은 행, 두번째 값은 열에 적용될 것입니다.
