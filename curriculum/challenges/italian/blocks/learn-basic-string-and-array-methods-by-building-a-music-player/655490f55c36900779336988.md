...
Itera sull'array `playlistSongElements` con un metodo `forEach`.

Il metodo `forEach` serve per iterare su un array ed eseguire una funzione su ogni elemento dell'array. Per esempio, supponi di avere un array di numeri e di voler stampare ogni numero nella console.
...
Usa il metodo `forEach` su `playlistSongElements`. Passa `songEl` come parametro e usa la sintassi arrow per aggiungere una callback vuota.

# --hints--

Dovresti concatenare `forEach()` a `playlistSongElements`.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

Il tuo metodo `forEach()` dovrebbe avere un parametro `songEl` e usare la sintassi arrow.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

Il tuo `forEach` dovrebbe avere una coppia vuota di parentesi graffe.
