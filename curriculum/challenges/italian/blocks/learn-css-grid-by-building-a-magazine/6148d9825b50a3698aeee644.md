...
Crea un selettore `.lists` e imposta la proprietà `list-style-type` su `none`. Questo eliminerà i punti elenco dagli elementi della lista.

# --hints--

Dovresti avere un selettore `.lists`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists'));
```

Il tuo selettore `.lists` dovrebbe avere una proprietà `list-style-type` impostata su `none`.
