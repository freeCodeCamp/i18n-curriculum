...
メニューは見た目が良いですが、コーヒー豆の背景画像以外は主にテキストだけです。

`Coffee` の見出しの下に、URL `https://cdn.freecodecamp.org/curriculum/css-cafe/coffee.jpg` を使って画像を追加してください。画像には `alt` 属性で `coffee icon` の値を付けてください。
...
`img` 要素は空要素なので、終了タグ `</img>` を付けてはいけません。

```js
assert.notMatch(code, /<\/img>/i);
```

`img` 要素には `src` 属性があり、その値は `"https://cdn.freecodecamp.org/curriculum/css-cafe/coffee.jpg"` であるべきです。
...
`img` 要素には `alt` 属性があり、その値は `"coffee icon"` であるべきです。
