...
첫 번째 `li` 요소 안에 앵커 요소를 만드세요. `a` 요소에 `href` 속성을 `#` 값으로 주고, 텍스트는 `Dashboard`로 하세요.

# --hints--

첫 번째 목록 항목 안에 `a` 요소를 만들어야 합니다.

```js
assert.exists(document.querySelector('li a'));
```

`a` 요소에 `href` 속성을 `#` 값으로 설정해야 합니다.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), '#');
```

`a` 요소에 텍스트 `Dashboard`을 넣어야 합니다.
