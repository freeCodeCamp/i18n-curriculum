...
Du solltest eine `current`-Variable innerhalb deiner `while`-Schleife erstellen.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("shortest_path").find_whiles()[0].has_variable("current")`)) })
```

Du solltest deiner `current`-Variablen `min(unvisited)` zuweisen. Remember to delete `pass`.
