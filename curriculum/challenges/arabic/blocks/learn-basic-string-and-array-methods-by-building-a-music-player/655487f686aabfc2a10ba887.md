...
قبل أن تبدأ العمل على تشغيل الأغنية التالية والسابقة، تحتاج إلى الحصول على فهرس كل أغنية في خاصية `songs` داخل `userData`.

ابدأ بإنشاء دالة سهمية تسمى `getCurrentSongIndex`.

# --hints--

يجب أن يكون لديك دالة `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

يجب أن تستخدم دالتك `getCurrentSongIndex` صيغة نحوية سهمية.
