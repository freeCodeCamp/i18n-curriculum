...
`catch` 블록에서 문자열 `Failed to load player data, using defaults:`과 `error` 매개변수를 차례로 콘솔에 기록하세요.

그다음 `catch` 바깥이지만 여전히 `loadPlayer` 함수 내에서 `defaultPlayer`를 반환하세요.

이 모든 과정은 플레이어 데이터를 불러와 UI에 채우고, 과정이 실패하면 `defaultPlayer` 객체를 불러옵니다.

# --hints--

`catch` 블록은 `'Failed to load player data, using defaults:', error`을 콘솔에 기록해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

`loadPlayer` 함수는 대체 수단 값으로 `defaultPlayer`을 반환해야 합니다.
