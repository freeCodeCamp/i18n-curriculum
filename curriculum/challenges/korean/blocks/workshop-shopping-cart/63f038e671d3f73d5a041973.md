...
가장 먼저 해야 할 일은 `items` 배열이 비어 있는지 확인하는 것입니다. 비어 있다면, `alert`라는 텍스트가 포함된 `Your shopping cart is already empty`을 사용자에게 표시한 다음 함수에서 반환하세요.

`0`는 거짓 같은 값이므로 `!` 연산자를 사용해 배열이 비어 있는지 확인할 수 있다는 점을 기억하세요.

경고를 표시한 후에는 실행을 멈추기 위해 함수에서 반환하세요.

# --hints--

부정 연산자를 사용해 `if` 배열이 비어 있는지 확인하는 `items` 문을 만드세요.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

`if` 문은 `alert`라는 텍스트가 포함된 `Your shopping cart is already empty`을 사용자에게 표시해야 합니다.
