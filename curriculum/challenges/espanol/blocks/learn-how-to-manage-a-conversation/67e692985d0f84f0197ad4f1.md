...
`To address` means to take action to deal with a problem or situation. Por ejemplo:

`We need to address the login error before users report it.` – This means action needs to be taken to fix the issue.

`Quickly` expresses how fast something is done. `Quick`, on the other hand, simply describes something as being fast. Por ejemplo:
