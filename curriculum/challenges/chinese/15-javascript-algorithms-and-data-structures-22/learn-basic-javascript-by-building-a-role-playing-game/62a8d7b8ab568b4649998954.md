...
当 `inventory` 长度不超过一时，使用 `else` 语句运行。 将 `text.innerText` 设置为 `"Don't sell your only weapon!"`。

# --hints--

你的 `sellWeapon` 函数应该有一个 `else` 语句。

```js
assert.match(sellWeapon.toString(), /else/);
```

你应该将 `text.innerText` 设置为 `"Don't sell your only weapon!"`。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")Don't sell your only weapon!\1/);
```

你的新 `text.innerText` 行应该位于你的 `else` 语句中。
