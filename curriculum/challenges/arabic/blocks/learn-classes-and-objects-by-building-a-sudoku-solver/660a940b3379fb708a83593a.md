...
أنشئ عبارة `if` تتحقق مما إذا كانت طريقة `solver()` المستدعاة من كائن `gameboard` تُرجع `True`.

ثم أضف استدعاء `print()` داخل جسم الـ `if` يمرر سلسلة نصية بصيغة f-string التالية: `f'Solved puzzle:\n{gameboard}'`.


# --hints--

يجب أن تنشئ عبارة `if` تتحقق مما إذا كانت طريقة `solver()` المستدعاة من كائن `gameboard` تُرجع `True`.
...
يجب أن تطبع `f'Solved puzzle:\n{gameboard}'` داخل جسم الـ `if` الجديد.
