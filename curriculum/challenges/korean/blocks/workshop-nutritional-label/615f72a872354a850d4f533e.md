...
`.daily-value p` 섹션에 있는 모든 `p` 요소를 선택하도록 `daily-value` 선택자를 만드세요. 이 새 선택자에 `border-bottom` 속성을 `1px solid #888989`로 설정하세요.

# --hints--

새로운 `.daily-value p` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

`.daily-value p` 선택자에 `border-bottom` 속성이 `1px solid #888989`로 설정되어 있어야 합니다.
