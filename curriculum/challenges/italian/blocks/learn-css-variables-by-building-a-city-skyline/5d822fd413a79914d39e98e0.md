...
Crea una nuova variabile sotto le altre chiamata `--building-color3` e assegnale un valore di `#cc6699`. Poi usala come `background-color` della classe `.bb3` e assegna un valore fallback di `pink`.
...
Dovresti assegnare a `--building-color3` un valore di `#cc6699`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb1')?.getPropertyValue('--building-color3')?.trim(), '#cc6699');
```

Dovresti impostare il `background-color` di `.bb3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb3')?.backgroundColor);
```

Dovresti impostare la `background-color` usando la variabile `--building-color3` con un fallback di `pink`.
