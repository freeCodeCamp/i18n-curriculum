...
يجب أن تكون الصور ضمن تخطيط من عمودين وثلاثة صفوف.

أعط محدِّد `.image-wrapper` خاصية `grid-template-columns` بقيمة `2fr 1fr` وخاصية `grid-template-rows` بقيمة `repeat(3, min-content)`. هذا سيجعل صفوف الشبكة تتغير في الارتفاع حسب المحتوى، بينما تبقى الأعمدة بعرض ثابت بناءً على الحاوية.
