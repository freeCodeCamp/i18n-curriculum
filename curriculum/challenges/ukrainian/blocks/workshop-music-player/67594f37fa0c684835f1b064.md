...
Коли ви натискаєте на пісню у своєму плейлисті, ви хочете відтворити цю пісню. Для цього потрібно додати слухача події `click` на кожен елемент `button`, який є дочірнім елементом `song`.

Завершіть функцію зворотного виклику вашого `forEach`, щоб це зробити. Усередині функції зворотного виклику `addEventListener` викличте `playSong(n)`, де `n` — це ідентифікатор пісні, який ви можете отримати з атрибута `id` елемента `song`.

Зверніть увагу, що кожен `song` має атрибут `id` зі значенням `song-n`: вам потрібно отримати частину `n` і перетворити її на число перед тим, як передати у `playSong()`.


# --hints--

Ви повинні додати слухача події `click` на кожен елемент `button`, який є дочірнім елементом `.playlist-song`, і викликати `playSong` з `id` відповідної пісні.
