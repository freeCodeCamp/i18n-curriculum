...
`.inline` 요소와 `label` 텍스트 사이에 오른쪽 `margin`를 `0.5em`만큼 주어 약간의 공간을 만드세요. 또한, 나머지 모든 여백은 `0`로 설정하세요.

# --hints--

`.inline` 요소에 `margin-top`를 `0`만큼 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginTop, '0px');
```

`.inline` 요소에 `margin-right`를 `0.5em`만큼 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginRight, '0.5em');
```

`.inline` 요소에 `margin-bottom`를 `0`만큼 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginBottom, '0px');
```

`.inline` 요소에 `margin-left`를 `0`만큼 주어야 합니다.
