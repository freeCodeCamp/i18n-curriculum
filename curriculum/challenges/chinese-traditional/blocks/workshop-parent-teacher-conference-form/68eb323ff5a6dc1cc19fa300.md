...
現在，你應該創建當圓鈕被選取時會出現的內圈。你將使用 `::before` 偽元素來達成這個效果。

像 `::before` 這樣的偽元素讓你在實際元素之前插入額外的內容。這通常用於裝飾目的。

針對 `.contact-method-radio-btn::before` 偽元素，設定 `display` 為 `block`、`content` 為 `" "`、`width` 為 `10px`、`height` 為 `10px`，以及 `border-radius` 為 `50%`。

# --hints--

你應該有一個 `.contact-method-radio-btn::before` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn::before'));
```

將 `display` 屬性設定為 `block`。
