...
Fügen Sie eine mittelgroße Trennlinie nach Ihrem `.daily-value`-Element hinzu. Erstellen Sie unterhalb dieser neuen Trennlinie ein `p`-Element mit dem `class`-Attribut, das auf `note` gesetzt ist.

Geben Sie dem `p`-Element folgenden Text:
...
Sie sollten ein neues `div` nach Ihrem `.daily-value`-Element erstellen.

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

Ihr neues `div` sollte das `class` auf `divider medium` gesetzt haben.
...
Sie sollten ein `p`-Element nach Ihrem neuen `div`-Element erstellen.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'p');
```

Ihr neues `p`-Element sollte das `class` auf `note` gesetzt haben.
