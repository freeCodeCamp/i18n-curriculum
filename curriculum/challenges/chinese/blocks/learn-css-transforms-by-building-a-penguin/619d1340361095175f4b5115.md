...
企鹅的身体看起来有点单调。 在 `.penguin-body` 前面添加一个 `class` 为 `shirt` 的 `div` 元素，来装扮一下它。

# --hints--

你应该在 `.penguin` 中添加一个 `div` 元素。 应该是 `--fcc-expected--` `div` 元素，而不是 `--fcc-actual--`。

```js
assert.equal(document.querySelectorAll('.penguin > div')?.length, 3);
```

你应该给新的 `div` 一个 `shirt` `class`。

```js
assert.exists(document.querySelector('.penguin > div.shirt'));
```

你应该将新的 `div` 放在 `.penguin-body` 前面。
