...
حان الوقت للتحقق من أن البحث يعمل. لدى ستيفن كينغ ستة كتب في الكتالوج.

استدعِ `findByAuthor(catalog, "king")` وعيّن النتيجة إلى متغير باسم `kingBooks`. سجّل `kingBooks.length` لتأكيد العدد.

ثم استخدم حلقة `for` لتسجيل كل نتيجة. استخدم قالب نصي لتضمين العنوان والسنة، مثل `` `${kingBooks[i].title} (${kingBooks[i].year})` ``.

# --hints--

يجب أن يكون طول `kingBooks` هو `6`.

```js
assert.strictEqual(kingBooks.length, 6);
```

يجب أن تسجل `kingBooks.length` في وحدة التحكم.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*kingBooks\.length\s*\)/);
```

يجب أن تستخدم حلقة `for` لتسجيل عنوان كل كتاب وسنته.
