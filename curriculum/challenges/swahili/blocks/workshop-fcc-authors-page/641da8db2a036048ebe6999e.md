...
Ndani ya kitendakazi cha `fetchMoreAuthors`, weka vigezo `startingIndex` na `endingIndex` kuwa `+= 8` kila moja.
...
Unapaswa kuweka kigezo `startingIndex` kuwa `+=8`.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Unapaswa kuweka kigezo `endingIndex` kuwa `+=8`.
