...
Du solltest einen `.social-icons`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

Dein `.social-icons`-Selektor sollte eine `display`-Eigenschaft mit dem Wert `grid` haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display === 'grid');
```

Dein `.social-icons`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `3rem` haben.
