...
Створіть селектор `.quote`. Надайте йому множину властивостей `color`, встановлену в `#00beef`, властивість `font-size`, встановлену в `2.4rem`, і властивість `text-align`, встановлену в `center`.

# --hints--

У вас має бути селектор `.quote`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote'));
```

Ваш селектор `.quote` повинен мати властивість `color`, встановлену в `#00beef`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.color === 'rgb(0, 190, 239)');
```

Ваш селектор `.quote` повинен мати властивість `font-size`, встановлену в `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote')?.fontSize === '2.4rem');
```

Ваш селектор `.quote` повинен мати властивість `text-align`, встановлену в `center`.
