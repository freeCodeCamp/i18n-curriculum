...
Você deve usar concatenação de strings com o operador `+` para juntar a string `"My name is "` com a variável `bot` seguida por um ponto (`.`). Fique atento aos espaços. Assign this concatenated string to the `botIntroduction` variable.
