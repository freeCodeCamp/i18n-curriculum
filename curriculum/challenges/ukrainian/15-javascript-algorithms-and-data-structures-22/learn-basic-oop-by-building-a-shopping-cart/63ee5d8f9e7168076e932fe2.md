...
Після свого елемента `h2` створіть два елементи `p`. Give the first a `class` of `dessert-price`, and set the text to a dollar sign `"$"` followed by the value of the `price` variable. Дайте другому `class` типу `product-category`, та текст `"Category: "`, за яким слідує значення змінної `category`.
