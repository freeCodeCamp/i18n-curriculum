...
Enviar el formulario con una opción seleccionada no enviaría un valor útil al servidor. Debido a eso, cada elemento `option` necesita un atributo `value`. Sin este atributo, el contenido de texto del `option` se enviará al servidor.

Da al primer `option` un `value` de `""`, y a los elementos `option` subsiguientes, atributos `value` del `1` al `4`.

# --hints--

Debes dar al primer `option` un `value` de `""`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

Debes dar al segundo `option` un `value` de `1`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

Debes dar al tercer `option` un `value` de `2`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

Debes dar al cuarto `option` un `value` de `3`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

Debes dar al quinto `option` un `value` de `4`.
