...
أخيرًا! وصلت إلى المبنى الأخير! أضف تدرجًا متكررًا له باتجاه `90deg`. استخدم لون المبنى من `0%` إلى `10%` و`transparent` من `10%` إلى `30%`.

# --hints--

يجب أن تضيف `repeating-linear-gradient` إلى `.fb6` في خاصية `background`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

يجب أن تعطي `repeating-linear-gradient` اتجاهًا بقيمة `90deg`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

يجب أن تعطي `repeating-linear-gradient` اللون الأول `--building-color3` من `0%` إلى `10%`.
...
يجب أن تعطي `repeating-linear-gradient` اللون الثاني `transparent` من `10%` إلى `30%`.
