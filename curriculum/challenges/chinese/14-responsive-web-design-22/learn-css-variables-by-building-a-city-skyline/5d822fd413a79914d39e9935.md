...
你应该在 `background` 属性中为 `.fb6` 提供第二个 `repeating-linear-gradient`。
...
你应该为第二个 `repeating-linear-gradient` 设置从 `0%` 到 `10%` 是第一种颜色 `--building-color3`。
...
你应该为第二个 `repeating-linear-gradient` 设置从 `10%` 到 `30%` 是第二种颜色 `--window-color3`。
