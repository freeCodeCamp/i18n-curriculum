...
Use o método `filter()` para remover o objeto da música que corresponde ao parâmetro `id` do array `userData?.songs`.

O método `filter` mantém apenas os elementos de um array que satisfazem a função callback passada para ele:
...
Use o método `filter()` em `userData?.songs`.Passe `song` como parâmetro da função arrow callback e use retorno implícito para verificar se `song.id` é estritamente diferente de `id`.Atribua tudo isso a `userData.songs`.

*Nota*: Você não deve usar encadeamento opcional quando atribuir o resultado de `userData?.songs.filter` para `userData.songs` porque o array `allSongs` não será `undefined` ou `null` naquele momento.

# --hints--

Você não deve modificar a função `deleteSong` existente.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Você deve usar o método `filter()` em `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

A função de callback do seu método `filter()` deve usar a sintaxe de arrow e ter um parâmetro `song`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

Seu método de filtro deve retornar implicitamente `song.id !== id`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

Você deve atribuir o método `filter()` que você usou em `userData?.songs` para `userData.songs`.
