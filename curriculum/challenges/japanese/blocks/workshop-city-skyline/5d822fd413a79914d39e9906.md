...
次の建物は3つのセクションを持ちます。`div`の中に3つの`.bb4`要素をネストしてください。順番に`bb4a`、`bb4b`、`bb4c`のクラスを付けてください。

# --hints--

`div`の中に3つの`.bb4`要素を追加してください。

```js
assert.equal(document.querySelector("div.bb4")?.children?.length, 3);
```

最初の新しい`div`に`bb4a`のクラスを付けてください。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

2番目の新しい`div`に`bb4b`のクラスを付けてください。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

3番目の新しい`div`に`bb4c`のクラスを付けてください。

```js
assert.exists(document.querySelector("div.bb4 > div.bb4a"));
```

新しい`div`要素は`.bb4a + .bb4b + .bb4c`の順番で配置してください。
