...
Você havia previamente adicionado uma chamada `console.log()` à sua função `addCrewMember` para validar seu comportamento, e agora a chamada não é mais necessária. Para evitar que o terminal fique desorganizado nos próximos passos, vá em frente e remova a chamada atual `console.log()`.

# --hints--

Remova a chamada `console.log()` da sua função `addCrewMember`. Você deve excluir a linha inteira.
