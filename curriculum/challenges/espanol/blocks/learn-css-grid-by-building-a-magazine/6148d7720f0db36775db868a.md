...
Necesitarás una columna para texto y una columna para imágenes. Asigna al selector `.text-with-images` una propiedad `grid-template-columns` establecida en `1fr 2fr`. También establece la propiedad `column-gap` en `3rem` para proporcionar más espacio entre las columnas.

# --hints--

Tu selector `.text-with-images` debe tener una propiedad `grid-template-columns` establecida en `1fr 2fr`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

Tu selector `.text-with-images` debe tener una propiedad `column-gap` establecida en `3rem`.
