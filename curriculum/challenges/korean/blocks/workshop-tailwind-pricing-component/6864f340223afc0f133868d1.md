...
`p`와 `span` 사이의 가격 텍스트로 이동하여, 가격이 더 잘 보이도록 `p` 요소에 `text-4xl`, `font-bold`, `text-gray-900` 클래스를 추가하세요.

# --hints--

`p` 요소에 `text-4xl` 클래스가 있어야 합니다.
...
`p` 요소에 `text-gray-900` 클래스가 있어야 합니다.
