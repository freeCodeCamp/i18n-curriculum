...
Tu selector `.wheel` debe tener la propiedad `animation-iteration-count` establecida en `infinite`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.wheel')?.animationIterationCount === 'infinite');
```

Tu selector `.wheel` debe tener la propiedad `animation-timing-function` establecida en `linear`.
