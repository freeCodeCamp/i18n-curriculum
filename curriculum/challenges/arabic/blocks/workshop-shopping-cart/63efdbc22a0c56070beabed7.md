...
أنت على المسار الصحيح! مع ذلك، دعنا نأخذ لحظة لمعالجة مشكلة شائعة عند العمل مع الكائنات في JavaScript.

عندما تحاول الوصول إلى خاصية في كائن غير موجودة، تُرجع JavaScript القيمة `undefined`. وإذا حاولت بعد ذلك إجراء العمليات الحسابية على `undefined`، فقد يؤدي ذلك إلى نتائج غير متوقعة، مثل `NaN`.
...
الآن، دعنا نطبق هذا المفهوم على كائن `totalCountPerProduct` في استدعاء `forEach`. تأكد من تهيئة كل خاصية `dessert.id` بشكل صحيح.

هيئ `totalCountPerProduct[dessert.id]` بقيمة افتراضية `0` باستخدام العامل `||`.

# --hints--

يجب استخدام ترميز النقطة للوصول إلى الخاصية `id` في `dessert`.
...
يجب استخدام ترميز الأقواس للوصول إلى الخاصية في `totalCountPerProduct` التي تتوافق مع `dessert.id`.
