...
2番目の`fieldset`では、`input`と`label`のテキストを同じ行に表示したいです。

まず、2番目の`input`内の`fieldset`要素に`inline`というクラスを付けてください。

# --hints--

最初の`input`には`inline`というクラスを付けてください。

```js
assert(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

2番目の`input`には`inline`というクラスを付けてください。
