...
الاقتباس ليس اقتباسًا حقيقيًا بدون علامات اقتباس مناسبة. يمكنك إضافتها باستخدام محددات CSS الزائفة.

أنشئ محدد `.quote::before` واضبط خاصية `content` على `"` مع وجود مسافة بعدها.

أيضًا، أنشئ محدد `.quote::after` واضبط خاصية `content` على `"` مع وجود مسافة قبلها.

# --hints--

يجب أن يكون لديك محدد `.quote::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

يجب أن يحتوي محدد `.quote::before` على خاصية `content` مضبوطة على `'" '`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content?.match(/\"\\"\s\"/));
```

يجب أن يكون لديك محدد `.quote::after`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

يجب أن يحتوي محدد `.quote::after` على خاصية `content` مضبوطة على `' "'`.
