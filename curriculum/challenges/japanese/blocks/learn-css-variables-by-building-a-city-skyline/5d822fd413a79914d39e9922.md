...
`.fb3a` 要素に `width` の `80%` と `height` の `15%` を設定してください。次に `.fb3b` 要素に `width` の `100%` と `height` の `35%` を設定してください。

# --hints--

`.fb3a` に `width` の `80%` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

`.fb3a` に `height` の `15%` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

`.fb3b` に `width` の `100%` を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3b")?.width, "100%");
```

`.fb3b` に `height` の `35%` を設定してください。
