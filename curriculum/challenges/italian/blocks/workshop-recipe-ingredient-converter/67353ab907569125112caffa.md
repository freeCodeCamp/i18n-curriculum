...
Ora è il momento di usare le tue funzioni. Dichiara una nuova funzione `processIngredient` che accetta quattro argomenti: `baseQuantity`, `baseUnit`, `newUnit` e `newServings`.

La tua funzione dovrebbe adattare le porzioni per la quantità base alle nuove porzioni, convertire quella quantità adattata dall'unità base alla nuova unità e restituire la quantità convertita come stringa fissa con due cifre decimali.

Se ti blocchi, fai riferimento alle chiamate di funzione di esempio che hai registrato. Puoi modificare i valori in entrambi quegli esempi per esplorare.

# --hints--

Dovresti dichiarare una funzione `processIngredient`.

```js
assert.isFunction(processIngredient);
```

La tua funzione `processIngredient` dovrebbe avere `baseQuantity` come primo parametro.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity/);
```

La tua funzione `processIngredient` dovrebbe avere `baseUnit` come secondo parametro.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit/);
```

La tua funzione `processIngredient` dovrebbe avere `newUnit` come terzo parametro.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit/);
```

La tua funzione `processIngredient` dovrebbe avere `newServings` come quarto parametro.

```js
assert.match(processIngredient?.toString(), /\(baseQuantity,\s*baseUnit,\s*newUnit,\s*newServings\)/);
```

La tua funzione `processIngredient` dovrebbe restituire una stringa.

```js
assert.isString(processIngredient(10, "gram", "cup", 5));
```

La tua funzione `processIngredient` dovrebbe restituire il valore corretto.
