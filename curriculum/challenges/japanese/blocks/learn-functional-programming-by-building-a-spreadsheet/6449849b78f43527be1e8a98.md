...
数式の中でセル範囲をマッチさせる必要があります。セル範囲は `A1:B12` や `A3:A25` のように見えます。これらのパターンをマッチさせるには正規表現を使えます。

まず `rangeRegex` 変数を宣言し、スプレッドシートの列の範囲である `A` から `J` までにマッチする正規表現を代入してください。キャプチャグループと文字クラスを使ってこれを実現します。

# --hints--

`rangeRegex` 関数の後に `idToText` 変数を宣言してください。
...
`const` 変数を宣言するには `rangeRegex` を使うべきです。
...
`rangeRegex` はキャプチャグループを使うべきです。

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(.*\)\/\s*;?/);
```

`rangeRegex` はキャプチャグループ内で文字クラスを使うべきです。

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[.*\]\)\/\s*;?/);
```

`rangeRegex` は `A` から `J` までにマッチする文字クラスを使うべきです。
