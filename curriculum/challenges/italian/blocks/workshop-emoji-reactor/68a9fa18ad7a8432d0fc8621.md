...
Nelle lezioni precedenti hai imparato che `querySelectorAll` restituisce un `NodeList`, che è una struttura simile a un array, e uno dei metodi che un `NodeList` possiede è `forEach()`, che funziona allo stesso modo del `forEach` presente negli array.

Itera su `btns` con un `forEach()` che aggiunge lo stesso listener di evento a ogni pulsante.

Usa una funzione di callback che prende un parametro `btn` e, all’interno della callback, aggiungi un listener per l’evento click che chiama `updateCount(btn)`.

Con questo hai completato il workshop.

# --hints--

Dovresti usare `btns.forEach()` per iterare sui pulsanti.

```js
assert.match(code, /btns\.forEach\s*\(/);
```

Tutti i pulsanti dovrebbero avere listener di evento che chiamano `updateCount` quando vengono cliccati.
