...
No início do seu setter, crie uma declaração `if` que levante um `TypeError` com a mensagem `'salary' must be a number.` quando `new_salary` não for um inteiro ou um float.

# --hints--

Você deve ter uma declaração `if` no seu setter de `salary`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[0]`)) })
```

Você deve lançar um `TypeError` com a mensagem `'salary' must be a number.` quando `new_salary` não for um inteiro ou um float.
...
Você não deve gerar nenhuma exceção quando `new_salary` for um inteiro ou um float.
