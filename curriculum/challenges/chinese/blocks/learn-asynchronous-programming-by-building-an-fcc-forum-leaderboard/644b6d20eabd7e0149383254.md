...
在你的 `forumCategory` 函数中，创建一个名为 `selectedCategory` 的新 `let` 变量，并为其赋值一个空对象。 这将用于存储每个类别的类别名称和类名。

# --hints--

你应该使用 `let` 声明一个名为 `selectedCategory` 的变量。 

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

你应该为 `selectedCategory` 变量分配一个空对象。
