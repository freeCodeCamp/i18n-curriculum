...
これでアニメーションが滑らかになり、観覧車が完成しました。

# --hints--

新しく `75%` セレクターを `@keyframes cabins` ルール内に作成する必要があります。
...
`75%` セレクターは `50%` と `100%` セレクターの間に置く必要があります。

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText === '75%');
```

`75%` セレクターの `background-color` プロパティを `yellow` に設定する必要があります。
