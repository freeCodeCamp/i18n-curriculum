...
Kipengele cha `.message` kinahitaji mtindo fulani. Mpe:

- `font-size` ya `1.2em`,
- `margin-bottom` ya `20px`.

# --hints--

Unapaswa kuwa na kichaguzi cha `.message`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".message"));
```

Kichaguzi cha `.message` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `1.2em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("font-size"), "1.2em");
```

Kichaguzi cha `.message` kinapaswa kuwa na sifa ya `margin-bottom` iliyowekwa kuwa `20px`.
