...
删除 `pass` 并创建一个 `if` 语句来判断 `find_empty_cell` 的返回值是否为 `None` 。 这样的话，谜题就解开了。 因此，从 `if` 主体返回 `True` 。

# --hints--

你应该删除 `pass` 并创建一个 `if` 语句来判断 `self.find_empty_cell` 的返回值是否为 `None`。
