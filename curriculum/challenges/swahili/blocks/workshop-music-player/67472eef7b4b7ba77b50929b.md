...
Katika kitu chako cha `userData`, tengeneza sifa ya `songs` na weka thamani yake kuwa safu ya data ya `allSongs`.

Kisha, ili kushughulikia taarifa za wimbo wa sasa na kufuatilia muda wake wa kucheza, tengeneza sifa za `currentSong` na `songCurrentTime`. Weka thamani zao kuwa `null` na `0`, mtawalia.

# --hints--

Kitu chako cha `userData` kinapaswa kuwa na ufunguo wa `songs` uliowekwa kwa safu ya data ya `allSongs`.

```js
assert.propertyVal(userData, "songs", allSongs);
```

Kitu chako cha `userData` kinapaswa kuwa na ufunguo wa `currentSong` uliowekwa kwa `null`.

```js
assert.propertyVal(userData, "currentSong", null);
```

Kitu chako cha `userData` kinapaswa kuwa na ufunguo wa `songCurrentTime` uliowekwa kwa `0`.
