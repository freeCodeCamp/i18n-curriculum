...
في البرمجة، يُقصد بـ `null` تمثيل غياب القيمة. في هذه الحالة، إذا أدخل المستخدم مُدخَلًا غير صالح، تريد تنبيهه ثم إرجاع `null` للدلالة على فشل الدالة.

لا تزال داخل كتلة `if`، عيّن `isError` إلى `true` وأرجع `null`.

# --hints--

بعد `alert`، يجب أن تعيّن `isError` إلى `true`.
...
بعد تعديل `isError`، يجب أن تُرجع القيمة `null` باستخدام `return`.
