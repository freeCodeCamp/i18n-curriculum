...
使用相同的模式，为第三个 `.line`、第四个 `.line`、第五个 `.line` 和第六个 `.line` 分别创建单独的选择器。
...
你的 `.line:nth-of-type(3)` 选择器应该有一个 `transform` 属性，设置为 `rotate(120deg)`。
...
你的 `.line:nth-of-type(4)` 选择器应该有一个 `transform` 属性，设置为 `rotate(180deg)`。
...
你的 `.line:nth-of-type(5)` 选择器应该有一个 `transform` 属性，设置为 `rotate(240deg)`。
...
你的 `.line:nth-of-type(6)` 选择器应该有一个 `transform` 属性，设置为 `rotate(300deg)`。
