...
Mfumo ule ule unatumika kwa sehemu nyingine tatu. Ongeza ukaguzi sawa kwa `author`, `year`, na `location`.

Kumbuka kuwa `isValid` hukusanya matokeo kutoka kwa ukaguzi wote wanne: ikiwa sehemu yoyote itashindwa, inakuwa `false` na hubaki `false` hata kama ukaguzi mwingine unafaulu. Hii ina maana kitendakazi kila mara hukagua kila sehemu badala ya kusitisha ukaguzi kwa tatizo la kwanza.

# --hints--

`validateEntry(catalog[0])` inapaswa kurudisha `true`.

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])` inapaswa kurudisha `false`.

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])` inapaswa kurudisha `false`.

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])` inapaswa kurudisha `false` kwa sababu eneo ni `"Unknown"`.
