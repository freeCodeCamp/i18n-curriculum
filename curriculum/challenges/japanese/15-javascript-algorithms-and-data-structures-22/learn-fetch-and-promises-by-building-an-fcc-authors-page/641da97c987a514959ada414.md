...
では、以前やったのと同じように、著者データの一部を渡して `displayAuthors` 関数を呼び出してください。

関数の呼び出しを追記した後に `Load More Authors` ボタンをクリックしても、この時点では動作しません。 まだ、ボタンに `click` イベントリスナーを追加する必要があるためです。 それは次のステップで対応しましょう。
