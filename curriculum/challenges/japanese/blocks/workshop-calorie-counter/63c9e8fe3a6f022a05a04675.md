...
同じ補間構文を使って、テキストが`consumedCalories Calories Consumed`の2つ目の`p`要素と、テキストが`exerciseCalories Calories Burned`の3つ目の`p`要素を追加してください。`consumedCalories`と`exerciseCalories`の変数は適切な値に置き換えるのを忘れないでください。

# --hints--

テンプレートリテラルに2つ目の`p`要素を追加してください。
...
2つ目の`p`要素のテキストは`${consumedCalories} Calories Consumed`にしてください。
...
3つ目の`p`要素のテキストは`${exerciseCalories} Calories Burned`にしてください。
