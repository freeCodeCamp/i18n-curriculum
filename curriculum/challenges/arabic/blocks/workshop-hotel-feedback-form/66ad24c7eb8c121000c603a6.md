...
لجعل مربع اختيار مُدخَل محددًا افتراضيًا، يمكنك إضافة الخاصية `checked`.

إليك مثالًا على استخدام الخاصية `checked`:

```html
<input checked type="checkbox" id="checked" name="checked">
```

أضف الخاصية `checked` إلى مربع اختيار المُدخَل الذي يحمل `id` بقيمة `"reputation"` لجعله محددًا افتراضيًا.

# --hints--

يجب أن يحتوي مربع اختيار المُدخَل الذي يحمل `id` بقيمة `"reputation"` على الخاصية `checked`.
