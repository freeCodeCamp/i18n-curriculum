...
Cambia il valore della chiave `position` con il `Position` derivato.

# --hints--

Il campo `PlayerData` della tua interfaccia `position` dovrebbe usare il tipo `Position` invece di `string`.
