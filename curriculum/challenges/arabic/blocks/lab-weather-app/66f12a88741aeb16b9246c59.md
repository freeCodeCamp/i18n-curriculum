...
في هذا المختبر، ستبني تطبيقًا يوفر معلومات الطقس من مدن مختلفة.

ستستخدم واجهة برمجة تطبيقات الطقس. البيانات الناتجة لها التنسيق التالي:
...
1. يجب أن يكون لديك عنصر `button` مع `id` يساوي `get-weather-btn`.

1. يجب أن يكون لديك عنصر `select` يحتوي على سبعة عناصر `option` متداخلة بداخله. يجب أن يكون الخيار الأول نصه وسمة `value` فارغين. يجب أن تحتوي الباقي على النص والقيم التالية (مع كون القيمة بأحرف صغيرة):
...
1. إذا لم يتم اختيار مدينة، فلا يجب أن يفعل الضغط على الزر أي شيء.

1. إذا تم اختيار مدينة، يجب أن تظهر عناصر لعرض الطقس:

- يجب أن يكون لديك عنصر `img` بالمعرف `weather-icon` لعرض أيقونة الطقس.

- يجب أن يكون لديك عنصر بالمعرف `main-temperature` لعرض درجة الحرارة الرئيسية.

- يجب أن يكون لديك عنصر بالمعرف `feels-like` لعرض درجة الحرارة المحسوسة.

- يجب أن يكون لديك عنصر بالمعرف `humidity` لعرض نسبة الرطوبة في الهواء.

- يجب أن يكون لديك عنصر بالمعرف `wind` لعرض سرعة الرياح.

- يجب أن يكون لديك عنصر بالمعرف `wind-gust` لعرض هبات الرياح.

- يجب أن يكون لديك عنصر بالمعرف `weather-main` لعرض نوع الطقس الرئيسي.

- يجب أن يكون لديك عنصر بالمعرف `location` لعرض الموقع الحالي.

1. يجب أن يكون لديك دالة غير متزامنة باسم `getWeather` تستدعي معلومات الطقس من واجهة برمجة التطبيقات `https://weather-proxy.freecodecamp.rocks/api/city/<CITY>` وتُرجعها. لاحظ أن هذه الواجهة تعيد البيانات باستخدام النظام المتري، أي م/ثانية لسرعة الرياح، ودرجة مئوية للحرارة.

1. يجب أن تقبل الدالة غير المتزامنة `getWeather` مدينة كمعلمة.

1. يجب أن تتعامل مع أي أخطاء تحدث داخل دالة `getWeather` وتسجلها في وحدة التحكم.

1. يجب أن يكون لديك دالة غير متزامنة `showWeather` تقبل مدينة كمعلمة.

1. يجب أن تستدعي دالة `showWeather` دالة `getWeather` لاسترجاع بيانات الطقس للمدينة المختارة من القائمة المنسدلة.

1. إذا حدث خطأ في دالة `getWeather`، يجب أن يعرض التطبيق تنبيهًا يقول `Something went wrong, please try again later`.

1. إذا كانت بيانات `getWeather` صالحة، يجب أن تعرض دالة `showWeather` بيانات الطقس في العناصر المناسبة. إذا كانت قيمة معينة من الواجهة `undefined`، يجب كتابة `N/A` في العنصر المقابل.

1. لا يجب إعلان دالتي `getWeather` و `showWeather` باستخدام `const`، لأن الاختبارات تحتاج إلى إعادة تعيينهما.

**ملاحظة:** لا يجب إعلان دالتي `getWeather` و `showWeather` باستخدام `const`، لأن الاختبارات تحتاج إلى إعادة تعيينهما. أيضًا، ستستغرق الاختبارات وقتًا لإكمالها، لذا طالما ترى `// running tests` في وحدة التحكم، فهذا يعني أنها تُنفذ.
...
يجب أن يكون لديك عنصر `button` مع `id` يساوي `get-weather-btn`.

```js
assert.exists(document.querySelector('button#get-weather-btn'));
```

يجب أن يكون لديك عنصر `select`.

```js
assert.exists(document.querySelector('select'));
```

داخل عنصر `select` يجب أن يكون الطفل الأول عنصر `option` بسمة `value` فارغة.
...
داخل عنصر `select` يجب أن يكون هناك 6 عناصر `option`، واحدة لكل من المدن التالية: باريس، لندن، طوكيو، لوس أنجلوس، شيكاغو، نيويورك.
...
يجب أن يكون لديك دالة `showWeather`.

```js
assert.isFunction(showWeather);
```

يجب أن يكون لديك دالة `getWeather`.

```js
assert.isFunction(getWeather);
```

يجب أن تقبل دالة `getWeather` مدينة كمعلمتها الوحيدة وتُرجع JSON من واجهة برمجة تطبيقات الطقس.
...
لا يجب إعلان دالتي `getWeather` و `showWeather` باستخدام `const`.
...
يجب أن تستدعي دالة `showWeather` دالة `getWeather` للحصول على بيانات الطقس.
...
يجب أن تتعامل دالة `showWeather` مع الحالة التي تُرجع فيها `getWeather` القيمة `undefined`.
...
عند اختيار نيويورك، يجب أن تعرض دالة `showWeather` البيانات من الواجهة في عناصر HTML المناسبة. إذا كانت القيمة من الواجهة `undefined`، يجب عرض `N/A`.
...
عند اختيار شيكاغو، يجب أن تعرض دالة `showWeather` البيانات من الواجهة في عناصر HTML المناسبة. إذا كانت القيمة من الواجهة `undefined`، يجب عرض `N/A`.
...
عند اختيار لندن، يجب أن تعرض دالة `showWeather` البيانات من الواجهة في عناصر HTML المناسبة. إذا كانت القيمة من الواجهة `undefined`، يجب عرض `N/A`.
...
عند اختيار طوكيو، يجب أن تعرض دالة `showWeather` البيانات من الواجهة في عناصر HTML المناسبة. إذا كانت القيمة من الواجهة `undefined`، يجب عرض `N/A`.
...
عند اختيار لوس أنجلوس، يجب أن تعرض دالة `showWeather` البيانات من الواجهة في عناصر HTML المناسبة. إذا كانت القيمة من الواجهة `undefined`، يجب عرض `N/A`.
...
إذا حدث خطأ، يجب أن تسجل دالة `getWeather` الخطأ في وحدة التحكم.
...
عند اختيار باريس، يجب أن يعرض التطبيق تنبيهًا يقول `Something went wrong, please try again later`.
