...
添加一個 `firsttwo` 屬性，該屬性接收一個 `nums` 參數，並按順序返回 `nums` 數組的前兩個元素。 然後添加一個 `lasttwo` 屬性，它按順序返回 `nums` 數組的最後兩個元素。

# --hints--

你的 `spreadsheetFunctions` 對象應該具有 `firsttwo` 屬性。

```js
assert.property(spreadsheetFunctions, "firsttwo");
```

你的 `firsttwo` 屬性應該是一個函數。

```js
assert.isFunction(spreadsheetFunctions.firsttwo);
```

你的 `firsttwo` 函數應該返回數組的前兩個數字。

```js
assert.deepEqual(spreadsheetFunctions.firsttwo([1, 2, 3, 4, 5, 6]), [1, 2]);
```

你的 `spreadsheetFunctions` 對象應該具有 `lasttwo` 屬性。

```js
assert.property(spreadsheetFunctions, "lasttwo");
```

你的 `lasttwo` 屬性應該是一個函數。

```js
assert.isFunction(spreadsheetFunctions.lasttwo);
```

你的 `lasttwo` 函數應該返回數組的最後兩個數字。
