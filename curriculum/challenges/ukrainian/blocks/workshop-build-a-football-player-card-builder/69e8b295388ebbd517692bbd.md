...
Всередині `card-stats` div, після першого `stat-col`, потрібно додати ще два дочірні елементи.

Спочатку самозакриваючийся `div` з `className` `stat-divider`, щоб відтворити тонку вертикальну лінію між двома колонками статистики, а потім другий `div` з `className` `stat-col`, який містить ще три `div` елементи, кожен з `className` `stat-row`.

# --hints--

Ви повинні додати самозакриваючийся `div` з `className` `stat-divider` після першого `.stat-col`.

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

Ви повинні додати другий `div` з `className` `stat-col` після `.stat-divider`.

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

Другий `.stat-col` має містити три `div` елементи, кожен з `className` `stat-row`.
