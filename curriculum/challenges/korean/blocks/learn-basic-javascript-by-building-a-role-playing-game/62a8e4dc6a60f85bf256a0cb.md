...
JavaScript의 `Math` 객체는 수학 상수와 함수에 대한 정적 프로퍼티와 메서드를 포함합니다. 그중 하나가 `Math.random()`로, `0`(포함)부터 `1`(미포함)까지의 무작위 숫자를 생성합니다. 또 다른 하나는 `Math.floor()`로, 주어진 숫자를 가장 가까운 정수로 내림합니다.

이것들을 사용하면 범위 내에서 무작위 숫자를 생성할 수 있습니다. 예를 들어, 이것은 `1`와 `5` 사이의 무작위 숫자를 생성합니다: `Math.floor(Math.random() * 5) + 1;`.

이 패턴을 따라, 덧셈 연산자(`+`)를 사용해 `1`와 `xp` 값 사이의 무작위 숫자를 `monsterHealth -= weapons[currentWeaponIndex].power`에 더하세요.

# --hints--

이미 작성한 `monsterHealth` 줄을 사용하세요.

```js
assert.lengthOf(attack.toString().match(/monsterHealth/g), 1);
```

`weapons[currentWeaponIndex].power` 값에 더해야 합니다.
