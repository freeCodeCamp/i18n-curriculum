...
Quando `inverted` è falso, vuoi costruire una piramide standard. Usa `.push()` come hai fatto nei passaggi precedenti per ottenerlo.

# --hints--

Dovresti chiamare il metodo `.push()` di `rows` nel tuo blocco `else`.
...
Dovresti passare una chiamata `padRow()` come argomento per il tuo metodo `.push()`.
