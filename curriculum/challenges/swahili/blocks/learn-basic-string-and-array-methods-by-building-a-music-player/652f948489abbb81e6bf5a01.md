...
Katika mradi huu utajifunza njia za msingi za mfuatano wa herufi na safu ya data kwa kujenga programu ya kuendesha muziki. Utaweza kucheza, kusitisha, kuruka, na kuchanganya nyimbo.

HTML na CSS za mradi huu zimeandaliwa tayari kwa ajili yako, hivyo unaweza kuwa makini na JavaScript.

Anza kwa kupata upatikanaji wa vipengele `#playlist-songs`, `#play`, na `#pause` kwa njia ya `getElementById()`. Weka thamani zao katika vigezo `playlistSongs`, `playButton` na `pauseButton` kwa mpangilio huo huo.


# --hints--

Unapaswa kuwa na kigezo kinachoitwa `playlistSongs`. 

```js
assert.isDefined(playlistSongs);
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#playlist-songs`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

Unapaswa kuweka kipengele cha `#playlist-songs` katika kigezo `playlistSongs`. 

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

Unapaswa kuwa na kigezo kinachoitwa `playButton`. 

```js
assert.isDefined(playButton);
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#play`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

Unapaswa kuweka kipengele cha `#play` katika kigezo `playButton`. 

```js
assert.equal(playButton, document.getElementById('play'));
```

Unapaswa kuwa na kigezo kinachoitwa `pauseButton`. 

```js
assert.isDefined(pauseButton);
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#pause`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

Unapaswa kuweka kipengele cha `#pause` katika kigezo `pauseButton`. 
