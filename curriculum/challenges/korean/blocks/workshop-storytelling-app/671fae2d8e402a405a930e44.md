...
`click`에 `scaryStoryBtn` 이벤트에 대한 이벤트 리스너를 추가하고 `displayStory` 함수를 전달하세요. 그런 다음 버튼을 클릭하고 콘솔에서 출력 결과를 확인하세요.

# --hints--

`click`에 `scaryStoryBtn` 이벤트에 대한 이벤트 리스너를 추가하고 `displayStory` 함수를 전달해야 합니다.
