...
Після конструктора створіть абстрактний метод `render` з типом повернення `void`. Ви будете використовувати його в підкласах, щоб оновити відображуваний емодзі.

# --hints--

Ваш клас `Bug` має містити абстрактний метод `render`.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

Ваш метод `render` має мати тип повернення `void`.
