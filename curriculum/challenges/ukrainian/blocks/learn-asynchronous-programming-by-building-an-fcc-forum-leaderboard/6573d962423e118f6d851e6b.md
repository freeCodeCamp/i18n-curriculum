...
Далі отримайте доступ до елемента `#posts-container` за допомогою методу `getElementById()`. Призначте його новій константі з назвою `postsContainer`.

# --hints--

Ви повинні використати `const` для оголошення змінної `postsContainer`.

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

Ви повинні використати `document.getElementById()` для отримання елемента `#posts-container`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

Ви повинні призначити елемент `#posts-container` вашій змінній `postsContainer`.
