...
现在在 `showElements` 内，将 `singleCardBtn`、`multipleCardsBtn` 和 `headerTitle` 作为参数传入。记住你是从 `elements` 获取它们，并用 `this` 关键字访问它们。

# --hints--

你应该将 `singleCardBtn` 作为第一个参数传给 `showElements` 函数。
...
你应该将 `multipleCardsBtn` 作为第二个参数传给 `showElements` 函数。
...
你应该将 `headerTitle` 作为参数传给 `showElements` 函数。
