...
Você não deve alterar o primeiro `repeating-linear-gradient`.
...
Você deve ter uma vírgula (`,`) após o primeiro `repeating-linear-gradient`.
...
Você deve dar um `repeating-linear-gradient` uma cor de inicial de `--building-color4` de `0%` a `10%`.
...
Você deve usar a segunda cor de `--window-color4` de `10%` a `90%`.
