...
Você deve dar a `.fb6` um segundo `repeating-linear-gradient` na propriedade `background`.
...
Você deve dar ao segundo `repeating-linear-gradient` uma cor de inicial de `--building-color3` de `0%` a `10%`.
...
Você deve dar ao segundo `repeating-linear-gradient` uma segunda cor de `--window-color3` de `10%` a `30%`.
