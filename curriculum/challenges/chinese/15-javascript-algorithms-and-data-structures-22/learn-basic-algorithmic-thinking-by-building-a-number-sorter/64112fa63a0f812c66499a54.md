...
`Sort` 按钮点击时可能看起来工作正常，但这只是因为数组中的所有值都是个位数，而排序在处理更复杂值时可能不如预期。

将 `selected` 的 `option` 元素的 `value` 和文本从 `1` 更改为 `10`，然后再次点击 `Sort` 按钮。

# --hints--

你应该将 `selected` 的 `option` 元素的 `value` 更新为 `10`。
...
你应该将 `selected` 的 `option` 元素的文本更新为 `10`。
