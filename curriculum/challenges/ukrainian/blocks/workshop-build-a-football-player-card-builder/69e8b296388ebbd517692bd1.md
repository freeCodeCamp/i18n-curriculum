...
Додайте ще один `div` з `className` `form-group`. Всередині нього додайте елемент `label` з `className` `label`, `htmlFor` `imageUrl` і текстом `Image URL`. Потім додайте елемент `input` з `id` `imageUrl`, `className` `input` і `type` `text`.
