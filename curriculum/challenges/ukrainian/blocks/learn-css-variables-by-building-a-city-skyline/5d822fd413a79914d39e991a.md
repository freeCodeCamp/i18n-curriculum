...
Переходимо до наступної будівлі! Вкладіть два елементи `div` всередину `.fb2` і надайте їм класи `fb2a` та `fb2b` відповідно.

# --hints--

Ви повинні додати два елементи `div` всередину `.fb2`.

```js
assert.equal(document.querySelectorAll("div.fb2 > div")?.length, 2);
```

Першому новому елементу `div` слід додати клас `fb2a`.
...
Другому новому елементу `div` слід додати клас `fb2b`.
