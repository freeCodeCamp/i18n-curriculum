...
`.cat-whiskers` 要素の中に、クラス `div` と `cat-whiskers-left` を持つ `cat-whiskers-right` 要素を2つ作成してください。

# --hints--

既存のクラス `div` を持つ `cat-whiskers` 要素は変更しないでください。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers'), 1);
```

`div` 要素の中に `.cat-whiskers` 要素が2つあるはずです。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers div'), 2);
```

`div` 要素の中の最初の `.cat-whiskers` 要素はクラス `cat-whiskers-left` を持つべきです。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers div')[0].classList.contains('cat-whiskers-left'));
```

`div` 要素の中の2番目の `.cat-whiskers` 要素はクラス `cat-whiskers-right` を持つべきです。
