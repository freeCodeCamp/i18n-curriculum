...
`gap` 속성은 `column-gap`과 `row-gap`의 값을 동시에 설정하는 축약형입니다. 하나의 값이 주어지면 `column-gap`과 `row-gap` 모두 그 값으로 설정합니다. 두 개의 값이 주어지면 `row-gap`는 첫 번째 값으로, `column-gap`는 두 번째 값으로 설정합니다.

`.image-wrapper` 선택자에 `gap` 속성을 `2rem`로 설정하세요.

# --hints--

`.image-wrapper` 요소에 `gap` 속성을 `2rem`로 설정해야 합니다.
