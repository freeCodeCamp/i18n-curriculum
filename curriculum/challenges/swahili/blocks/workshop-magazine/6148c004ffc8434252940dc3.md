...
Unda kichaguzi cha `.social-icons`. Mpe sifa ya `display` iwe `grid`, na sifa ya `font-size` iwe `3rem`.
...
Kichaguzi chako cha `.social-icons` kinapaswa kuwa na sifa ya `display` iliyowekwa kuwa `grid`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

Kichaguzi chako cha `.social-icons` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `3rem`.
