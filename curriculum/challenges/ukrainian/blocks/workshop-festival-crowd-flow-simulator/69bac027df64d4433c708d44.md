...
Коли ви перенаправляєте переповнення, слід відправляти відвідувачів до наступних воріт у масиві `gates`. Для цього потрібно знайти індекс наступних воріт. Зазвичай можна додати 1 до поточного індексу. Однак якщо поточні ворота — останні в масиві, потрібно повернутися до перших воріт.

Ви можете використати оператор за модулем (`%`), щоб це зробити:

```js
const nextIndex = (currentIndex + 1) % array.length;
```

Це працює, бо коли `currentIndex + 1` дорівнює `array.length`, результат стає 0.

Використовуючи цей підхід, створіть змінну з ім’ям `nextGateIndex`, яка зберігатиме індекс наступних воріт у масиві `gates`.

# --hints--

Ви повинні використати техніку за модулем, щоб отримати доступ і призначити наступний індекс з `currentIndex` у масиві `gates` змінній з ім’ям `nextGateIndex`. Дотримуйтеся того самого однорядкового шаблону, що й у прикладі.
