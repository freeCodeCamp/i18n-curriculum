...
送信ボタンをより目立たせるために、`background-color`に`#3b3b4f`を、`border-color`に`white`を設定してください。

# --hints--

送信ボタンに`background-color`として`#3b3b4f`を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.backgroundColor, 'rgb(59, 59, 79)');
```

送信ボタンに`border-color`として`white`を設定してください。
