...
為你的 `if` 述語添加一個 `else` 區段。在 `else` 區段內，呼叫 `getCurrentSongIndex()` 函式並將其指定給名為 `currentSongIndex` 的常數。
...
你應該在 `else` 區塊內將 `currentSongIndex` 常數設定為 `getCurrentSongIndex()`。
