...
Для вашої третьої таблиці додайте `caption` з текстом `Net Worth` та налаштуйте заголовок таблиці і тіло таблиці.

# --hints--

Ваш третій `table` елемент повинен містити `caption` елемент.

```js
assert(document.querySelectorAll('table')?.[2]?.children?.[0]?.localName === 'caption');
```

Ваш `caption` елемент повинен містити текст `Net Worth`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('caption')?.textContent === 'Net Worth');
```

Ваш третій `table` елемент повинен містити `thead` елемент.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('thead'));
```

Ваш третій `table` елемент повинен містити `tbody` елемент.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('tbody'));
```

Ваш `thead` елемент повинен бути безпосередньо під вашим `caption` елементом.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('caption')?.nextElementSibling?.localName === 'thead');
```

Ваш `tbody` елемент повинен бути безпосередньо під вашим `thead` елементом.
