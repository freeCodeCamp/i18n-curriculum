...
이 건물의 창문에 보조 색상을 채우세요. 또한 창문에 약간의 공간을 주기 위해 `margin`에 `10%`을 추가하세요.

# --hints--

`.fb4-window`에 `background-color`의 `--window-color1`을 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.backgroundColor.trim(), "var(--window-color1)");
```

`.fb4-window`에 `margin`의 `10%`을 주어야 합니다.
