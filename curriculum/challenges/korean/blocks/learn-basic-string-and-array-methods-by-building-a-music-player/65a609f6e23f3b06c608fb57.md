...
`sort()` 메서드는 정렬 순서를 정의하는 비교 콜백 함수를 받습니다.

이 예제에서 첫 번째 조건 `(a.name < b.name)`은 첫 번째 과일의 이름이 두 번째 과일의 이름보다 작은지 확인합니다. 그렇다면 첫 번째 과일이 두 번째 과일보다 먼저 정렬됩니다.

문자열은 사전식으로 비교되며, 이는 문자 하나하나를 차례로 비교한다는 뜻입니다. 예를 들어, `"Apples"`는 `"Bananas"`보다 작습니다. 왜냐하면 `"A"`가 알파벳 순서에서 `"B"`보다 앞서기 때문입니다.

이 예제가 숫자를 반환하는 이유는 `sort()` 메서드가 숫자 반환을 기대하기 때문입니다. 음수를 반환하면 첫 번째 항목이 두 번째 항목보다 먼저 정렬됩니다.
...
콜백 함수 안에 `if` 문을 추가하여 `a.title`이 `b.title`보다 작은지 확인하세요. 그렇다면 `-1`을 반환하세요.

# --hints--

콜백 함수 안에 `if` 문이 있어야 합니다.

```js
assert.match(code, /if\s*\(/);
```

`if` 문은 `a.title`이 `b.title`보다 작은지 확인해야 합니다.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

`if` 문은 `-1`이 `a.title`보다 작을 때 `b.title`을 반환해야 합니다.
