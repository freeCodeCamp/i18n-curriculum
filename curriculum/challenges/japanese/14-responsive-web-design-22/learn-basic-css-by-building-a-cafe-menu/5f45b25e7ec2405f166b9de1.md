...
訪問されたリンクのプロパティを変更するには、`a:visited { propertyName: propertyValue; }` というような<dfn>疑似セレクター</dfn>を使用します。

フッターの `Visit our website` というリンクをユーザーが訪問したことがある場合には、その色が `grey` に変わるようにしてください。

# --hints--

`a:visited` という疑似セレクターを使用する必要があります。
...
`color` プロパティを `grey` に設定する必要があります。
...
`a:visited` の `color` は `grey` である必要があります。
