...
Após o último elemento `p`, crie outro elemento `p` e dê a ele o texto `Dietary Fiber 4g`. Dê ao elemento `p` a `class` necessária para indentá-lo e remover a borda de divisão. Em seguida, crie um divisor abaixo do elemento `p`.
...
O novo elemento `p` deve ter o texto `Dietary Fiber 4g`.
