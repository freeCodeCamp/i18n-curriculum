...
Число десять можна записати як суму простих чисел рівно п’ятьма різними способами:
...
Яке перше число можна записати як суму простих чисел більш ніж `n` способами?

# --hints--

`primeSummations(5)` має повертати число.

```js
assert(typeof primeSummations(5) === 'number');
```

`primeSummations(5)` має повертати `11`.

```js
assert.strictEqual(primeSummations(5), 11);
```

`primeSummations(100)` має повертати `31`.

```js
assert.strictEqual(primeSummations(100), 31);
```

`primeSummations(1000)` має повертати `53`.

```js
assert.strictEqual(primeSummations(1000), 53);
```

`primeSummations(5000)` має повертати `71`.
