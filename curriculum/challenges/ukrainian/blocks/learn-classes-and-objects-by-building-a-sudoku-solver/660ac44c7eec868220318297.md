...
Змініть `row_str` comprehension так, щоб він повертав рядок лише тоді, коли елемент не дорівнює нулю, а в іншому випадку — символ зірочки.

# --hints--

Список comprehension, призначений змінній `row_str`, має викликати `str()` для кожного елемента `i` у `row`, якщо `i` істинне, інакше він має оцінюватися як `'*'`.
