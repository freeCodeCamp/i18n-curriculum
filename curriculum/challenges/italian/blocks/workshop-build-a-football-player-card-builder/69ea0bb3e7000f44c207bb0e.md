...
Per far funzionare finalmente la funzionalità localStorage, cambia l'inizializzazione dello stato da `defaultPlayer` a `loadPlayer`. Questo garantisce che lo stato inizi con eventuali dati salvati disponibili.

Ora prova la funzionalità localStorage. Tutto ciò che inserisci dovrebbe rimanere anche dopo che l'anteprima si aggiorna o ricarichi la pagina.

Puoi anche cambiare il collegamento dell'immagine `src` in `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` per vedere l'immagine cambiare in tempo reale. Assicurati solo di cambiare il nome del giocatore in Maradona.

Con questo, il tuo progetto per creare la scheda del calciatore è completo!

# --hints--

Il tuo `useState` dovrebbe essere inizializzato con `loadPlayer` invece di `defaultPlayer`.
