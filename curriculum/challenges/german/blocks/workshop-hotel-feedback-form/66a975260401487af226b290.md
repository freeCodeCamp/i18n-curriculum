...
Um ein `option` standardmäßig auszuwählen, können Sie das `selected`-Attribut zu dem `option`-Element hinzufügen, das ausgewählt sein soll.

Hier ist ein Beispiel mit dem `selected`-Attribut:

```html
<option selected value="amazing">Amazing</option>
```

Fügen Sie innerhalb Ihres `select`-Elements das `selected`-Attribut zum `option`-Element mit dem Wert `"excellent"` hinzu.

# --hints--

Ihr `option`-Element mit dem Wert `"excellent"` sollte das `selected`-Attribut haben.
