...
这个两个词的短语意思是邀请或为团队或项目添加新成员。第一个词以 `-ing` 结尾。

# --explanation--

`Bring in` 是一个短语动词，意思是邀请、添加或雇佣某人加入团队、项目或公司。例如：

- `He brought in a security expert to check the system.` - 他请了一位专家加入并检查系统的安全性。

- `We might need to bring in outside help if the issue continues.` - 如果问题没有停止，他们可能需要外部支持。
