...
Aggiungi altri due oggetti al tuo `instrumentsArr` per rappresentare gli strumenti a fiato rimasti, clarinetto e oboe.

# --hints--

Dovresti avere `{ category: "woodwinds", instrument: "Clarinet", price: 200 }` dentro il tuo `instrumentsArr`.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

Dovresti avere `{ category: "woodwinds", instrument: "Oboe", price: 4000 }` dentro il tuo `instrumentsArr`.

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

Dovresti ancora avere `{ category: "woodwinds", instrument: "Flute", price: 500 }` dentro il tuo `instrumentsArr`.
