...
`allCategories` 對象的其餘部分已經爲你寫好了。

在接下來的幾個步驟中，你將創建一個函數，用於從 `allCategories` 對象中檢索類別名稱。

首先創建一個名爲 `forumCategory` 的箭頭函數，並將 `id` 作爲參數名。

# --hints--

你需要創建一個名爲 `forumCategory` 的空函數。

```js
assert.match(code, /\s*const\s+forumCategory\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*{\s*}\s*/);
```

你應該在 `forumCategory` 函數中有一個名爲 `id` 的參數。
