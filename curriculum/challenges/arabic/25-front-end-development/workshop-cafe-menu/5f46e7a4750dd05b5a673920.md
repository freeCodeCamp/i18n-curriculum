...
يجب عليك تطبيق السمة `class="address"`.

```js
assert(code.match(/class=('|")address\1/i));
```

عنصر `.address` الخاص بك يجب أن يكون عنصر `p` الخاص بك.

```js
assert.equal(document.querySelector('.address')?.tagName, 'P');
```

يجب أن يحتوي عنصر `.address` الخاص بك على النص `123 Free Code Camp Drive`.
