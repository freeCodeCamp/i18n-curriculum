...
在 `forEach` 回调中，将右侧赋值 `totalCountPerProduct[dessert.id] || 0` 括在括号 `()` 中以确保正确评估，然后将值加一。
...
你应该使用赋值运算符来更新与 `dessert.id` 对应的 `totalCountPerProduct` 属性的值。
...
在将 `totalCountPerProduct[dessert.id] || 0` 加一之前，你应该将其括在括号 `()` 中。
