...
title: Rappresentazione dei numeri di Zeckendorf
challengeType: 1
forumTopicId: 302346
...
Proprio come i numeri possono essere rappresentati in una notazione posizionale come somme di multipli delle potenze di dieci (decimale) o due (binario); tutti i numeri interi positivi possono essere rappresentati come la somma di uno o zero volte i membri distinti della serie di Fibonacci. Ricorda che i primi sei numeri distinti di Fibonacci sono: `1, 2, 3, 5, 8, 13`.

Il numero decimale undici può essere scritto come `0*13 + 1*8 + 0*5 + 1*3 + 0*2 + 0*1` o `010100` in notazione posizionale dove le colonne rappresentano la moltiplicazione per un particolare membro della sequenza. Gli zeri iniziali vengono eliminati così che 11 decimale diventa `10100`. Tuttavia 10100 non è l’unico modo per ottenere 11 dai numeri di Fibonacci, anche `0*13 + 1*8 + 0*5 + 0*3 + 1*2 + 1*1` o 010011 rappresenterebbero l’11 decimale. Per un vero numero di Zeckendorf c’è la restrizione aggiuntiva che *non si possono usare due numeri di Fibonacci consecutivi* il che porta alla prima soluzione unica.

# --instructions--

Scrivi una funzione che generi e restituisca la rappresentazione del numero di Zeckendorf di `n`.
