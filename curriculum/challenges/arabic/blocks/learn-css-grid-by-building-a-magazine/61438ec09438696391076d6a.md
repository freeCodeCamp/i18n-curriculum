...
أسفل عنصر `.heading` الخاص بك، أنشئ عنصر `section` جديد مع تعيين `class` إلى `text`. بداخله، أنشئ عنصر `p` مع تعيين `class` إلى `first-paragraph` والنص التالي:
...
يجب أن يأتي عنصر `section` الجديد بعد عنصر `.heading` الخاص بك.

```js
assert(document.querySelectorAll('section')?.[1]?.previousElementSibling?.className === 'heading');
```

يجب أن يحتوي عنصر `section` الجديد على `class` مضبوط إلى `text`.

```js
assert(document.querySelectorAll('section')?.[1]?.className === 'text');
```

يجب أن تنشئ عنصر `p` جديد داخل عنصر `.text` الخاص بك.

```js
assert(document.querySelector('.text')?.querySelectorAll('p')?.length === 1);
```

يجب أن يحتوي عنصر `p` الجديد على `class` مضبوط إلى `first-paragraph`.
