...
أضف كتلة `else`. داخل كتلة `else`، نفذ استدعاء الدالة `playSong` مع `id` الأغنية التي تُشغل حاليًا كمعلمة.

هذا يضمن استمرار تشغيل الأغنية الحالية عند النقر على زر التشغيل.
...
أضف كتلة `else` إلى جملة `if` الخاصة بك.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{\s*playSong\(\s*userData\?\.songs\s*\[\s*0\s*\]\s*\.id\s*\)\s*;?\s*\}\s*else\s*\{/)
```

نفذ استدعاء الدالة `playSong` مع `userData?.currentSong.id` داخل كتلة `else` في جملة `if` الخاصة بك.
