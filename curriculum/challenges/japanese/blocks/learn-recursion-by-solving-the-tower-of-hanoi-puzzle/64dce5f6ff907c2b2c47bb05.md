...
`target` が空の場合、ディスクは必ず `source` から `target` に移動される必要があります。

`forward` の宣言の後に、`if` が空かどうかを確認する `rods[target]` 文を追加してください。空であれば、`forward` を `True` に変更してください。

# --hints--

`if` が空かどうかを確認する `rods[target]` 文があり、`True` を `forward` に代入しているはずです。
