...
باستخدام منتقي النمط (style selector) نفسه في الخطوة السابقة، اجعل حجم الخط (font size) للعناصر والأسعار أكبر باستخدام قيمة `18px`.

# --hints--

يجب عليك تعيين خاصية `font-size` إلى `18px`.
...
عناصر `p` الخاصة بك المتداخلة في `.item` يجب أن تحتوي على `font-size` من `18px`.
