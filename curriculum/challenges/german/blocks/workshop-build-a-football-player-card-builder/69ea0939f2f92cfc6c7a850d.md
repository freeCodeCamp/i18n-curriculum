...
Fügen Sie als Nächstes einen `useEffect`-Hook hinzu, der immer dann ausgeführt wird, wenn sich der `player`-Status ändert. Verwenden Sie darin eine Pfeilfunktion und fügen Sie einen `try..catch` mit `error` als Parameter des `catch`-Blocks hinzu.

# --hints--

Sie sollten `useEffect` innerhalb von `FootballPlayerCard` aufrufen.
...
Ihr `useEffect` sollte `[player]` als Abhängigkeits-Array haben.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

Ihr `useEffect`-Callback sollte einen `try...catch`-Block mit `error` als Parameter des `catch`-Blocks enthalten.
