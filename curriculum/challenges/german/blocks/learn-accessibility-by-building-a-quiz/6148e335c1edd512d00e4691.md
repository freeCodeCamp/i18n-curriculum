...
Wenn Sie auf die Navigationslinks klicken, sollte der Viewport zum entsprechenden Abschnitt springen. Dieses Springen kann jedoch für einige Nutzer verwirrend sein.

Wählen Sie alle Elemente aus und setzen Sie das `scroll-behavior` auf `smooth`.
...
Sie sollten `*` eine `scroll-behavior` von `smooth` geben.
