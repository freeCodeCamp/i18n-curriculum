...
変数を作成したら、その名前を参照して他の CSS プロパティに値を代入できます。

```css
background: var(--penguin-skin);
```

これは、対象の要素の背景をグレーに変更します。なぜなら、それが `--penguin-skin` 変数の値だからです。変数名が完全に一致しないとスタイルは適用されないことに注意してください。

# --instructions--

`--penguin-skin` 変数を `background` プロパティに `penguin-top`、`penguin-bottom`、`right-hand`、`left-hand` クラスに適用してください。

# --hints--

`--penguin-skin` 変数は `background` プロパティに `penguin-top` クラスで適用する必要があります。
...
`--penguin-skin` 変数は `background` プロパティに `penguin-bottom` クラスで適用する必要があります。
...
`--penguin-skin` 変数は `background` プロパティに `right-hand` クラスで適用する必要があります。
...
`--penguin-skin` 変数は `background` プロパティに `left-hand` クラスで適用する必要があります。
