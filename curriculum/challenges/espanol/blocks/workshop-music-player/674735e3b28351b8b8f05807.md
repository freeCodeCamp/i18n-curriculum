...
El método `find()` recupera el primer elemento dentro de un arreglo que cumple con las condiciones especificadas en la función de callback proporcionada. Si ningún elemento satisface la condición, el método devuelve `undefined`.
...
Dentro de su función `playSong`, use el método `find()` en el arreglo `userData.songs` para buscar una canción que tenga un `id` estrictamente igual al `id` pasado a la función `playSong`. Asigne la llamada a `find` a una variable llamada `song`.

# --hints--

Su función `playSong` debe llamar al método `find` en `userData.songs`.
...
Su llamada a `find` debe devolver una canción cuyo `id` sea estrictamente igual al `id` pasado a la función `playSong`.
...
Debería asignar su llamada a `find` a una variable llamada `song`.
