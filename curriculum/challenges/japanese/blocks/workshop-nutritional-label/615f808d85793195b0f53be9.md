...
最後の `p` の後にもう一つ `.divider` 要素を作成し、テキストに `Total Sugars 12g` を設定してください。その `p` 要素にインデントを付け、下のボーダーを削除するために必要な `class` の値を代入してください。次に、新しい `.divider` 要素の下にもう一つ `p` を作成してください。
...
新しい `p` 要素のテキストは `Total Sugars 12g` にしてください。
