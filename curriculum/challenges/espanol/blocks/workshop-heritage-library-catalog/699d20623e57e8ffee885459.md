...
`parseCard` ha sido probado y funciona correctamente. La variable `cardResult` ya no es necesaria.

Comienza eliminando la línea `const cardResult` y la línea `console.log(cardResult)`. Luego crea una función llamada `parseCatalog` que reciba `rawCards` como parámetro. Por ahora, declara un arreglo vacío llamado `catalog` dentro de la función y devuélvelo.

Debajo de la función, llama a `parseCatalog(rawCatalogCards)`, asigna el resultado a una variable llamada `catalog` y muestra `catalog.length` en la consola.

# --hints--

`parseCatalog` debe ser una función que devuelva un arreglo.

```js
assert.isArray(parseCatalog(rawCatalogCards));
```

`catalog` debe estar definida y asignada a `parseCatalog(rawCatalogCards)`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+catalog\s*=\s*parseCatalog\s*\(/);
```

Debes eliminar la variable `cardResult` y la llamada relacionada `console.log()`.

```js
assert.notMatch(__helpers.removeJSComments(code), /\bcardResult\b/);
```

Debes mostrar `catalog.length` en la consola.
