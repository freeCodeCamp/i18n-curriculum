...
Якщо ви хочете додати більше іконок соціальних мереж, але залишити їх в одному рядку, вам потрібно оновити `grid-template-columns`, щоб створити додаткові колонки. Як альтернативу, ви можете використати властивість `grid-auto-flow`.

Ця властивість приймає або `row`, або `column` як перше значення, з необов’язковим другим значенням `dense`. `grid-auto-flow` використовує алгоритм автоматичного розміщення для налаштування макета сітки. Встановлення його в `column` повідомить алгоритму створювати нові колонки для вмісту за потреби. Значення `dense` дозволяє алгоритму відкотитися назад і заповнити порожнини в сітці меншими елементами, що може призвести до появи елементів у невідповідному порядку.

Для вашого селектора `.social-icons` встановіть властивість `grid-auto-flow` у значення `column`.

# --hints--

Ваш селектор `.social-icons` має мати властивість `grid-auto-flow`, встановлену в `column`.
