...
أضف حشوًا إلى الأعلى واليسار لعناصر `.info`، واضبط القيم الأخرى على `0`.

# --hints--

يجب أن تستخدم المحدِّد `.info`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.info'));
```

يجب أن تعطي `.info` حشوًا علويًا (`padding-top`) لا يقل عن `1px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('.info')?.paddingTop?.replace(/\D+/, '')), 1);
```

يجب أن تعطي `.info` حشوًا يمينيًا (`padding-right`) بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingRight, '0px');
```

يجب أن تعطي `.info` حشوًا سفليًا (`padding-bottom`) بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingBottom, '0px');
```

يجب أن تعطي `.info` حشوًا يساريًا (`padding-left`) لا يقل عن `1px`.
