...
Il metodo `map()` serve per iterare su un array e restituire un nuovo array. È utile quando vuoi creare un nuovo array basato sui valori di un array esistente. Per esempio:
...
Nota che il metodo `map()` prende una funzione come argomento. Questa si chiama <dfn>funzione di callback</dfn>, cioè una funzione passata a un'altra funzione come argomento. Nell'esempio sopra, la funzione di callback è `(number) => number * 2` ed è eseguita su ogni elemento dell'array `numbers`. Il metodo `map()` restituisce quindi un nuovo array con i risultati.

Passa una funzione di callback al metodo `map()`. La funzione di callback dovrebbe prendere `song` come parametro, usare la sintassi della funzione freccia e avere un corpo vuoto.

# --hints--

La funzione di callback del tuo metodo `map()` dovrebbe usare la sintassi della funzione freccia.

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

Il tuo metodo `map()` dovrebbe avere un parametro `song` nella sua funzione di callback.
