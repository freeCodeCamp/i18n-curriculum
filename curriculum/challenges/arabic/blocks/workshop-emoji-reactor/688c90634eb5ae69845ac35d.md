...
في هذه الورشة ستنشئ تطبيق Emoji Reactor. تم توفير قالب جاهز لـ HTML وكل CSS لك.

ابدأ بإنشاء عنصر `main`.

داخل عنصر `main` أنشئ عنصر `h1` مع `class` اسمه `title` ونصه `How are you feeling today?`.
...
يجب أن يكون لديك عنصر `h1` كابن لعنصر `main`.

```js
assert.exists(document.querySelector("main > h1"));
```

يجب أن يحتوي عنصر `h1` على صنف `title`.

```js
assert.equal(document.querySelector("main > h1")?.className, "title");
```

يجب أن يحتوي عنصر `h1` على النص `How are you feeling today?`.
