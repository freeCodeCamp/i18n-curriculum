...
下一步是在玩家旁邊顯示字 `(Captain)`，如果他們被列為該隊的隊長。 

在 `${name}` 表達式之前，為你的程式碼添加一個新的嵌入式表達式。在該表達式內，使用三元運算子檢查 `isCaptain` 是否為 true。如果是，傳回 `"(Captain)"`，否則傳回空字串。 

# --hints--

你應該使用三元運算子來檢查 `isCaptain` 是否為 true，並傳回 `"(Captain)"` 或傳回空字串。

```js
assert.match(code, /\s*<h2\s*>\s*\$\{isCaptain(\s*===\s*true)?\s*\?\s*('|"|`)\(Captain\)\2\s*:\s*('|"|`)\3\}\s*\${\s*name\s*}\s*<\/h2>\s*/)
```

你應該在 `"(Captain)"` 和 name 之間加一個空格。
