...
داخل حلقة `for`، أعلن متغيرًا باسم `row_str` وقم بتعيين تعبير توليدي للقائمة يكرر عبر `row` ويحوّل كل عنصر `i` في `row` إلى سلسلة نصية. استخدم الدالة `str()` لذلك.

# --hints--

يجب إعلان متغير `row_str`.
...
يجب تعيين تعبير توليدي للقائمة يكرر عبر `row` إلى المتغير `row_str`.
...
يجب أن يستخدم التعبير التوليدي للقائمة المعين إلى المتغير `row_str` المتغير `i` كمتغير تكرار.
...
يجب أن يستدعي التعبير التوليدي للقائمة المعين إلى المتغير `row_str` الدالة `str()` على كل عنصر `i` في `row`.
