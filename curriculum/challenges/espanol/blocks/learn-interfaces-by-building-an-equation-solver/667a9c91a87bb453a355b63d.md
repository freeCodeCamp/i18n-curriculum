...
Siéntete libre de cambiar los coeficientes de tu `lin_eq` para ver cómo cambia la salida.

Luego, elimina tu llamada `print(solver(lin_eq))` e imprime el resultado de llamar a `solver()` con `quadr_eq` como argumento.

# --hints--

No deberías tener `print(solver(lin_eq))` en tu código.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

Debes imprimir `solver(quadr_eq)`.
