...
Eine Sache noch. If you keep clicking the `Load More Authors` button until there's no more data to load and the text changes to `"No more data to load"`, the cursor value is still `pointer`. Why not change the cursor value to `not-allowed` instead?

Access the `style` property of the `Load More Authors` button and set `cursor` to `"not-allowed"`.

Damit ist die Autorenseite fertig!
