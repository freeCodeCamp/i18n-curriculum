...
你需要使用 `remove()` 从 DOM 中删除任务，并使用 `splice()` 从 `taskData` 数组中删除任务。

`splice()` 是一个数组方法，它通过删除、替换或添加指定索引处的元素来修改数组，同时返回被删除的元素。 它最多可以接受三个参数：第一个是必须的起始索引，第二个是需要删除的项目数，第三个是可选的替换元素。 这是一个示例：
...
使用 `remove()` 方法从 DOM 中删除 `buttonEl` 的 `parentElement`。 然后使用 `splice()` 从 `taskData` 数组中删除该任务。 将 `dataArrIndex` 和 `1` 作为 `splice()` 的参数传入。

`dataArrIndex` 是开始的索引，`1` 是要删除的项目数。

# --hints--

你不应更改 `dataArrIndex` 语句。
