...
Dé a los elementos `.fb2-window` un `ancho` de `22%`, `alto` de `100%`, y un `color de fondo` de su variable `--window-color3`.

# --hints--

Debe dar a los elementos `.fb2-window` un `ancho` de `22%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

Debe dar a los elementos `.fb2-window` un `alto` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

Debe dar a los elementos `.fb2-window` un `color de fondo` de `--window-color3`.
