...
使用 URL `https://cdn.freecodecamp.org/curriculum/css-cafe/pie.jpg` 在 `Desserts` 标题下添加最后一张图片。 给图像添加 `alt` 值 `pie icon`。
...
你的新 `img` 元素应具有 `src` 值为 `https://cdn.freecodecamp.org/curriculum/css-cafe/pie.jpg`。
...
你的新 `img` 元素应该具有 `alt` 值为 `pie icon`。
