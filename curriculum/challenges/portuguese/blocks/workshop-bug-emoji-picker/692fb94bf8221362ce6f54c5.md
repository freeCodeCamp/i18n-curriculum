...
Dentro da classe `Spider`, implemente o método `render`, prefixado pela palavra-chave `override`, para atualizar a propriedade `innerText` do `emojiElement` com o valor da propriedade `emoji`.

# --hints--

Sua classe `Spider` deve ter um método `render`.
...
Seu método `Spider` `render` deve usar a palavra-chave `override`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?override\s+render/);
```

Seu método `Spider` `render` deve definir `this.emojiElement.innerText` como `this.emoji`.
