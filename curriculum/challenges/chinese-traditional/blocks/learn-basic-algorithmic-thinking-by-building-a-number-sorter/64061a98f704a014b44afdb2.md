...
在這個專案中，你將會建置一個數字排序器。已經為你提供了 HTML 和 CSS。歡迎自由探索它們。

當你準備好時，宣告一個 `sortButton` 變數並將 `.getElementById()` 的引數 `"sort"` 指定給它。

# --hints--

你應該使用 `const` 宣告一個 `sortButton` 變數。

```js
assert.match(code, /const\s+sortButton\s*=/);
```

你應該使用引數 `"sort"` 呼叫 `document.getElementById()`。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)sort\1\s*\)/);
```

你應該將 `document.getElementById()` 的值指定給 `sortButton`。
