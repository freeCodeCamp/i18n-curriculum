...
これでグリッドのレイアウトにスタイルを適用できます。CSS GridはFlexboxと似ていて、親要素と子要素の両方に特別なプロパティがあります。

この場合、親要素は`main`要素です。`grid-template-columns`プロパティに`1fr 94rem 1fr`の値を設定して、3列のレイアウトにしてください。これにより、中央の列が`94rem`の幅になり、最初と最後の列はグリッドコンテナの残りのスペースのそれぞれ1分数ずつになります。

# --hints--

`main`セクションには`grid-template-columns`プロパティを`1fr 94rem 1fr`に設定してください。
