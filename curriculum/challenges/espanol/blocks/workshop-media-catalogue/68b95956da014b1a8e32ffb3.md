...
En un módulo anterior, aprendiste que puedes crear tus propias excepciones personalizadas heredando de la clase incorporada `Exception`:
...
Crea una clase llamada `MediaError`. Haz que herede de la clase `Exception` y agrégale un docstring que diga `Custom exception for media-related errors.`.

# --hints--

Debes crear una clase llamada `MediaError`.
...
Tu clase `MediaError` debería heredar de `Exception`.
...
Debes agregar el docstring `Custom exception for media-related errors.` a tu clase `MediaError`.
