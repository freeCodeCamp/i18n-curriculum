...
メソッド内で `'{name}' promoted to '{new_level}'.` を出力することでセッターを完成させてください。`{name}` と `{new_level}` はそれぞれ従業員の名前と新しいレベルに置き換えてください。

# --hints--

`'{name}' promoted to '{new_level}'.` を出力し、`{name}` と `{new_level}` はそれぞれ従業員の名前と新しいレベルに置き換える必要があります。
