...
title: ¿Qué es el método `filter` y cómo funciona?
challengeType: 19
...
En este ejemplo, el método `filter` aplica una función callback a cada elemento del arreglo `numbers`. La callback verifica si cada número es par usando el operador de resto (`%`).
