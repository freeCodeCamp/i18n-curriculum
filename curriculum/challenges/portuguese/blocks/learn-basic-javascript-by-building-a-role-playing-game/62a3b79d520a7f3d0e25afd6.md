...
Um método para encontrar elementos específicos no HTML é usando o método `querySelector()`. O método `querySelector()` recebe um seletor de CSS como argumento e retorna o primeiro elemento que corresponde ao seletor. Por exemplo, para encontrar o elemento `<h1>` no seu HTML, você escreveria:
