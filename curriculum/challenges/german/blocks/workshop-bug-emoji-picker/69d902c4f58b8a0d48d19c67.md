...
Füllen Sie jetzt `bugMap` mit zwei Einträgen. Fügen Sie `"bee"` als Schlüssel hinzu, der auf eine neue `Bee`-Instanz verweist, und `"spider"` als Schlüssel, der auf eine neue `Spider`-Instanz verweist. Übergeben Sie `bugEmojiElement` als Argument an beide.

# --hints--

Ihr `bugMap` sollte einen `"bee"`-Eintrag haben, der auf `new Bee(bugEmojiElement)` gesetzt ist.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

Ihr `bugMap` sollte einen `"spider"`-Eintrag haben, der auf `new Spider(bugEmojiElement)` gesetzt ist.
