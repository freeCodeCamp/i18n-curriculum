...
You should not have an `addTwoNumbers` function.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

You should not have a `printGreeting();` function call.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

You should not have a `printMessage('freeCodeCamp');` function call.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

You should not have a `console.log(addTwoNumbers(3,4));` console statement.
