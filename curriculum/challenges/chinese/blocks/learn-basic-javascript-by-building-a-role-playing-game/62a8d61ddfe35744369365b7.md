...
在 `currentWeapon` 之后，使用连接运算符将 `text.innerText` 设置为字符串 `"You sold a "`，然后是 `currentWeapon`，然后是字符串 `"."`。

# --hints--

你应该将赋值运算符与 `text.innerText` 一起使用。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=/);
```

你应该将 `"You sold a "` 添加到 `text.innerText`。 间距很重要。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

你应该将 `currentWeapon` 的值添加到 `"You sold a "` 字符串中。 使用连接运算符在同一行执行此操作。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a\s\1\s*\+\s*currentWeapon/);
```

你应该将字符串 `"."` 添加到 `currentWeapon` 的值中。 使用连接运算符在同一行执行此操作。

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1\s*\+\s*currentWeapon\s+\+\s+('|")\.\2/);
```

你的 `text.innerText` 应该更新为正确的字符串。
