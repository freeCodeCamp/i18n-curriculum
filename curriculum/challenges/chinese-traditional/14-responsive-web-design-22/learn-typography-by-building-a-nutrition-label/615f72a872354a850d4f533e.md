...
創建一個 `.daily-value p` 選擇器以定位 `daily-value` 部分中的所有 `p` 元素。 給這個新的選擇器一個 `border-bottom` 設置爲 `1px solid #888989`。

# --hints--

應該有一個新的 `.daily-value p` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

`.daily-value p` 選擇器應該有一個 `border-bottom` 屬性設置爲 `1px solid #888989`。
