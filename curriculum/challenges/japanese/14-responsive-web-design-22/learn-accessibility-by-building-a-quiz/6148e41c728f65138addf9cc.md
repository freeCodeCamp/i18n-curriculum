...
最後に、ある種のモーションベースのアニメーションは、一部のユーザーに不快感を与える可能性があります。 特に<dfn>前庭</dfn>障害のある人は、特定の動きのきっかけに対して敏感な場合があります。

`@media` アットルールには、ユーザーの好みに基づいて CSS を設定する `prefers-reduced-motion` というメディア特性があります。 これは次のいずれかの値を取ることができます:
...
`scroll-behavior: smooth` を設定するスタイルルールを、`@media` アットルールで囲み、メディア特性 `prefers-reduced-motion` の値として `no-preference` を設定してください。

よくできました！ アクセシビリティクイズの練習プロジェクトが完成しました。

# --hints--

`@media (prefers-reduced-motion: no-preference)` ルールが 1 つ必要です。
