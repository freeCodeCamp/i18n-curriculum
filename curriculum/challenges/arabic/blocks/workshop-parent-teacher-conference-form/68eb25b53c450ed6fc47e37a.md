...
افتراضيًا، عناصر `label` مضمَّنة. لجعلها تتكدس عموديًا (باستثناء مسميات أزرار الاختيار)، استخدم فئة مزيفة `:not()`.

فئة مزيفة `:not()` تنفي التحديد. هنا، تختار كل عناصر `label` التي لا تملك الصنف `contact-method`.

اختر `label:not(.contact-method)`، ثم عيّن خاصية `display` إلى `block` و`margin` إلى `10px 0`.

# --hints--

يجب أن يكون لديك محدِّد `label:not(.contact-method)`.

```js
assert(new __helpers.CSSHelp(document).getStyle('label:not(.contact-method)'));
```

عيّن خاصية `display` إلى `block`.
