...
你在之前的課程中學到，`this` 關鍵字是指正在被建構的當前實體，就像你在 `Dessert` 建構式中看到的那樣。這裡，你將把相同的概念應用到 `ShoppingCart` 類別。
...
在你的建構式中，使用 `this` 關鍵字將 `items` 屬性設為空陣列。同時，將 `total` 屬性設為 `0`，並將 `taxRate` 屬性設為 `8.25`。
...
你應該使用 `this` 關鍵字將你的類別的 `taxRate` 屬性設為 `8.25`。
