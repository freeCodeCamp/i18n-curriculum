...
現在是時候將你所學到的關於箭頭函數的知識應用到你的音樂播放器項目中了。

首先刪除 `printGreeting`、`printMessage` 和 `addTwoNumbers` 函數。 另外，刪除所有控制檯語句和函數調用。

# --hints--

你不應該有 `printGreeting` 函數。

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

你不應該有 `printMessage` 函數。

```js
assert.notMatch(code, /const printMessage = org => {/);
```

你不應該有 `addTwoNumbers` 函數。

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

你不應該有 `printGreeting();` 函數調用。

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

你不應該有 `printMessage('freeCodeCamp');` 函數調用。

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

你不應該有 `console.log(addTwoNumbers(3,4));` 控制檯語句。
