...
Debe haber un nuevo elemento de anclaje en el primer elemento `p`.

```js
assert.exists(document.querySelector('p')?.querySelector('a'));
```

El texto del ancla debe ser `cute cats`.

```js
assert.strictEqual(document.querySelector('a')?.innerText.trim(), "cute cats");
```

El atributo `href` del nuevo elemento de anclaje debe ser `https://cdn.freecodecamp.org/curriculum/cat-photo-app/running-cats.jpg`.

```js
assert.strictEqual(document.querySelector('a')?.href, "https://cdn.freecodecamp.org/curriculum/cat-photo-app/running-cats.jpg");
```

Debe haber solo una instancia de la frase `cute cats` en tu código.  

```js
assert.strictEqual(code.indexOf("cute cats"),code.lastIndexOf("cute cats")); 
```

El texto del elemento `p` todavía debe ser `Everyone loves cute cats online!`.
