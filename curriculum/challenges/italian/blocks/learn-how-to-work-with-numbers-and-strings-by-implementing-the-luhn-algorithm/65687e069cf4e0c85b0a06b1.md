...
All'esterno dell'istruzione `if`, aggiungi `number` a `sum_of_even_digits`. Inoltre, rimuovi la chiamata `print`.

# --hints--

Dovresti avere `sum_of_even_digits += number` all'interno del loop `for`.
...
Non dovresti avere `print(number)` all'interno del loop `for`.
...
Non dovresti avere `sum_of_even_digits += number` all'interno dell'istruzione `if`.
