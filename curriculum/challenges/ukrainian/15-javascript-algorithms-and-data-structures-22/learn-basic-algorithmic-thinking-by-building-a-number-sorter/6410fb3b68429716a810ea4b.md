...
Сортування вибором ґрунтується на відстеженні індексу найменшого значення в масиві. Оголосіть змінну `minIndex` та встановіть її на `i` — це гарантує, що якщо поточне значення є найменшим, воно буде замінено самим собою та не буде переміщено. Вам потрібно буде мати можливість переприпризначити значення `minIndex` під час ітерації масиву.

Потім напишіть ще один цикл `for` використовуючи `j` як ітератор. Цей цикл має починатися з індексу після `i` та ітерувати над рештою масиву.

# --hints--

Використайте `let`, щоб оголосити `minIndex`.
...
Ви повинні мати вкладений цикл `for`.
...
Цикл `for` має ініціалізувати `j` значенням `i + 1`.
...
Цикл `for` має ітерувати над рештою масиву.
