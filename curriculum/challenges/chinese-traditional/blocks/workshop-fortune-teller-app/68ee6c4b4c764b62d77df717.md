...
在某些情況下，卡牌需要翻轉，因為它們並不總是處於正確的位置。

在 `isReversed` 方法內宣告一個名為 `singleCardSelected` 的變數。使用 `Math.random()` 方法來產生一個隨機的布林值。為了確保該值保持在範圍內，將它與 `0.5` 進行比較。
...
你的 `isReversed` 變數應該使用 `Math.random()` 來隨機決定卡牌是否翻轉。
