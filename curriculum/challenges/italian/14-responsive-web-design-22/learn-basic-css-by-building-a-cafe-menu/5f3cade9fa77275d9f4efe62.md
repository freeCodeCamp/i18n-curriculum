...
Potresti continuare a provare diverse percentuali per le larghezze. Invece, usa il tasto back space della tastiera per spostare l'elemento `p` con la classe `price` affianco all'elemento `p` con la classe `flavor` in modo che siano sulla stessa riga nell'editor. Make sure there is no space between the two elements.
