...
ボタンを中央に配置するために、`font-size`を`1.1rem`に、`display`を`block`に、`margin`を`auto`に設定してください。

# --hints--

引き続き`.submit-btn`クラスを対象にしてください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

`font-size`プロパティを`1.1rem`に設定してください。
