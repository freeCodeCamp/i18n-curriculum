...
要呼叫命名的箭頭函式（程式）表達式，你可以透過其名稱來取用該函式（程式）。 

```js
exampleArrowFunction();
```

在你的 `printGreeting` 函式（程式）定義下方，呼叫該函式（程式）。打開主控台以查看輸出。

# --hints--

你應該呼叫你的 `printGreeting` 函式（程式）。

```js
assert.match(code, /printGreeting\(\);?/);
```

你的 `printGreeting` 函式呼叫應該位於 `printGreeting` 函式定義之下。

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

你的 `printGreeting` 函式呼叫應該會在主控台輸出 `Hello there!`。 
