...
Потім надайте йому `max-width` зі значенням `1400px` та додайте `margin` зі значенням `0 auto`, щоб зцентрувати його.

# --hints--

Селектор `.gallery` повинен мати властивість `padding` зі значенням `20px 10px`.
...
Селектор `.gallery` повинен мати властивість `max-width` зі значенням `1400px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.gallery')?.maxWidth === '1400px');
```

Ваш елемент `.gallery` повинен бути зцентрованим з допомогою `margin` зі значенням `0 auto`.
