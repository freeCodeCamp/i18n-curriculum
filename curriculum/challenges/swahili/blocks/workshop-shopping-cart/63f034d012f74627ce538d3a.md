...
Mwishowe, sahihisha sifa ya `display` ya kitu cha `style` cha mabadiliko ya `cartContainer` kwa kikokotoo kingine cha masharti cha pande tatu kinachokagua kama `isCartShowing` ni kweli. Ikiwa ni kweli, weka sifa ya `display` kuwa `"block"`, vinginevyo weka kuwa `"none"`.

Sasa unapaswa kuweza kuona gari lako na kuongeza vitu ndani yake.

# --hints--

Unapaswa kupata upatikanaji wa sifa ya `display` ya sifa ya `style` ya mabadiliko ya `cartContainer`.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display/)
```

Unapaswa kutumia opereta ya mgawo kwenye sifa ya `display` ya sifa ya `style` ya mabadiliko ya `cartContainer`.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*/)
```

Unapaswa kutumia sintaksia ya kikokotoo cha masharti cha pande tatu kukagua kama `isCartShowing` ni kweli.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*isCartShowing\s*\?\s*/)
```

Unapaswa kuweka sifa ya `display` ya sifa ya `style` ya mabadiliko ya `cartContainer` kuwa `"block"` ikiwa `isCartShowing` ni kweli.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*isCartShowing\s*\?\s*('|"|`)block\1\s*:\s*/)
```

Unapaswa kuweka sifa ya `display` ya sifa ya `style` ya mabadiliko ya `cartContainer` kuwa `"none"` ikiwa `isCartShowing` ni uongo.
