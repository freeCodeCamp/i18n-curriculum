...
Der automatische Platzierungs-Algorithmus wird aktiviert, wenn du ein neues Symbol-Element hinzufügst. Der Algorithmus wird jedoch standardmäßig die neue Spaltenbreite auf `auto` einstellen, was nicht mit den aktuellen Spalten übereinstimmt.

Du kannst dies mit der `grid-auto-columns`-Eigenschaft überschreiben. Gib dem `.social-icons`-Selektor eine `grid-auto-columns`-Eigenschaft mit dem Wert `1fr`.

# --hints--

Dein `.social-icons` sollte eine `grid-auto-columns`-Eigenschaft mit dem Wert `1fr` haben.
