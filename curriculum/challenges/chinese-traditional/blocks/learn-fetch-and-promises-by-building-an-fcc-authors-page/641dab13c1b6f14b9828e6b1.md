...
還有一件事。如果你持續點擊 `Load More Authors` 按鈕直到沒有更多資料可載入且文字變成 `"No more data to load"`，游標值仍然是 `pointer`。為什麼不將游標值改成 `not-allowed` 呢？

存取 `Load More Authors` 按鈕的 `style` 屬性，並將 `cursor` 設定為 `"not-allowed"`。
...
你應該使用點記法存取 `loadMoreBtn` 的 `style` 屬性。

```js
assert.match(code, /loadMoreBtn\.style/)
```

你應該使用點記法從 `style` 屬性存取 `cursor`。
