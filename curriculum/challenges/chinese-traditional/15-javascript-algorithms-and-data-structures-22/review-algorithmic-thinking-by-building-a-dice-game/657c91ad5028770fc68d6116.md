...
當用戶點擊 `Roll the dice` 按鈕時，應該生成五個隨機骰子數字並顯示在屏幕上。

構建邏輯，使得單擊 `rollDiceBtn` 會生成 1 到 6 之間的五個隨機數，將 `diceValuesArr` 設置爲僅包含這五個數字，並在 `listOfAllDice` 元素中按順序顯示這些數字。

# --hints--

當你的 `rollDiceBtn` 被點擊時，你的 `diceValuesArr` 數組應該包含五個元素。
...
當第二次單擊 `rollDiceBtn` 時，`diceValuesArr` 數組應該只包含五個新元素。
...
每個 `.die` 元素都應包含來自 `diceValuesArr` 數組的相應值。
