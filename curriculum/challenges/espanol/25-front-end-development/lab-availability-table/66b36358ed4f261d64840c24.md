...
Completa las instrucciones que se indican a continuación para pasar todas las pruebas para aprobar el laboratorio.
...
1. Debe tener un elemento `div` con un atributo `id` `legend`. Este debe contener un elemento `span` con el texto `Availability` y un elemento `div` con el atributo `id` de `legend-gradient`.
1. Debes darle al elemento `#legend-gradient` un gradiente lineal que transiciona entre todos los colores desde `--color0` hasta `--color5`. Cada valor de color debe tener dos paradas de color (expresadas como porcentajes) para hacer la transición entre los colores una línea dura.
...
Debes tener un elemento `div` con un atributo `id` `legend`.
...
Debes usar dos paradas de color (expresadas en porcentaje) para hacer la transición de un color al siguiente color una línea dura para tu elemento `#legend-gradient`. Recuerda usar tus variables `--color#`.
