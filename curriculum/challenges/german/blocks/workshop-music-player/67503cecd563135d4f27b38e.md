...
Jetzt ist es Zeit, die `playNextSong`-Funktion zu testen.

Fügen Sie dem `"click"`-Element einen `nextButton`-Ereignislistener hinzu und übergeben Sie `playNextSong` als zweites Argument Ihres Ereignislisteners. Dies ist die Funktion, die der Ereignislistener ausführen wird.

Testen Sie Ihre App, indem Sie zuerst auf die Wiedergabetaste und anschließend auf die Weiter-Taste klicken. Sie sollten sehen, dass alles wie erwartet funktioniert.

# --hints--

Sie sollten die `addEventListener()`-Methode auf Ihrer `nextButton`-Variable aufrufen.
...
Sie sollten `playNextSong` als zweites Argument Ihrer `addEventListener`-Methode übergeben.
