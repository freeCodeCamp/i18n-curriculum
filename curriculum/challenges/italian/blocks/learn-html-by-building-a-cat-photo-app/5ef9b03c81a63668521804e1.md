...
L'elemento `fieldset` serve a raggruppare insieme input e etichette correlate in un modulo web. Gli elementi `fieldset` sono <dfn>elementi a livello di blocco</dfn>, il che significa che appaiono su una nuova riga.
...
Il tuo elemento `fieldset` dovrebbe avere un tag di apertura. I tag di apertura hanno la seguente sintassi: `<elementName>`.

```js
assert(document.querySelector('fieldset'));
```

Il tuo elemento `fieldset` dovrebbe avere un tag di chiusura. I tag di chiusura hanno un `/` subito dopo il carattere `<`.

```js
assert(code.match(/<\/fieldset\>/));
```

Sia i pulsanti di opzione sia le etichette associate dovrebbero essere tra i tag di apertura e chiusura dell'elemento `fieldset`.
