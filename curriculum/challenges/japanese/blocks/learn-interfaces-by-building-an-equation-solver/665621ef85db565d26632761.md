...
新しい関数内で、`if`が`equation`クラスのインスタンスでないかどうかをチェックする`Equation`文を作成し、`TypeError`という文字列を使ってカスタムメッセージを提供しながら`'Argument must be an Equation object'`を発生させてください。

# --hints--

`if`関数内で、`equation`が`Equation`クラスのインスタンスでないかどうかをチェックする`solver`文を作成してください。
...
新しい`TypeError`文内で、指定された文字列を使って`if`を発生させてください。
