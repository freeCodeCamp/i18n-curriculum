...
1. يجب أن تُعرّف وتصدر مكوِّن `ColorPicker`.
1. يجب أن تستخدم الخطاف `useState`.
1. يجب أن يكون لديك عنصر `#color-picker-container` بخلفية بيضاء.
1. يجب أن يكون لديك عنصر `#color-input` وهو مُدخَل من نوع `color`.
1. يجب أن يكون `#color-input` تابعًا لـ `#color-picker-container`.
1. عند تغيير `#color-input`، يجب أن يتغير خلفية `#color-picker-container` إلى القيمة الجديدة.

# --hints--

يجب أن تُعرّف وتصدر مكوِّن `ColorPicker`.
...
يجب أن تستخدم قيمة الإرجاع من الخطاف `useState`.

```js
assert.match(code, /=\s*(?:React\.)?useState\(/);
```

يجب أن تستخدم التفكيك لتعيين قيمة الإرجاع من `useState` إلى متغيرين.

```js
assert.match(code, /\[.*,.*\]\s*=\s*(?:React\.)?useState\(/);
```

يجب أن تمرر `#ffffff` كقيمة ابتدائية لاستدعاء `useState`.
...
يجب أن تعرض عنصرًا له `id` باسم `color-picker-container`.

```js
assert.exists(document.getElementById("color-picker-container"));
```

يجب أن يكون لـ `#color-picker-container` خلفية بيضاء في البداية.
...
يجب أن تعرض عنصر `input` له `id` باسم `color-input`.
...
يجب أن يكون `#color-input` تابعًا لـ `#color-picker-container`.
...
يجب أن يكون `#color-input` مُدخَلًا من نوع `color`.
...
يجب أن يحتوي `#color-input` على خاصية `onChange`.
...
يجب أن يكون لـ `#color-input` قيمة ابتدائية `#ffffff`.
...
يجب أن تُدار القيمة الابتدائية لـ `#color-input` بواسطة الخطاف `useState`.
...
عند تغيير قيمة `#color-input`، يجب أن يغير `#color-picker-container` لون خلفيته ليعكس تلك القيمة.
