...
现在，调用 `map()`，将你的 `lambda` 函数作为第一个参数传递，并将 `expenses` 列表作为第二个参数传递。

# --hints--

你应该在 `total_expenses` 函数内调用 `map()` 函数。

```js
({ test: () => assert(runPython(`len(_Node(_code).find_function("total_expenses").find_calls("map")) == 1`)) })
```

你应该将 `lambda` 函数作为第一个参数传递给 `map()` 调用。
...
你应该将 `expenses` 列表作为第二个参数传递给 `map()` 调用。
