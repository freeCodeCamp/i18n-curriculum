...
أضف عنصرين `span` مع `className` بقيمتي `stat-value` و `stat-label` على التوالي إلى كل `div.stat-row`.

# --hints--

يجب أن يحتوي كل `.stat-row` على عنصر `span` مع `className` بقيمة `stat-value` وعنصر `span` مع `className` بقيمة `stat-label`.
