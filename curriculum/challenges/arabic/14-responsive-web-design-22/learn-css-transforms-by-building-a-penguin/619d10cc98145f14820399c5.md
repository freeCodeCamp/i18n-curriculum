...
يجب عليك إضافة عنصرين `div` بداخل `.penguin-head`. متوقع عدد `--fcc-expected--` من عناصر `div`، لكن وجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 9);
```

يجب عليك إعطاء أول عنصر `div` سمة `class` بقيمة `beak`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

يجب عليك إعطاء ثاني عنصر `div` سمة `class` بقيمة `beak`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div.beak')?.length, 2);
```

يجب عليك إعطاء أول عنصر `div` سمة `class` بقيمة `top`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

يجب عليك إعطاء ثاني عنصر `div` سمة `class` بقيمة `bottom`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.bottom'));
```

يجب عليك وضع `div.beak.top` بعد `div.blush.right`.

```js
assert.exists(document.querySelector('.blush.right + .beak.top'));
```

يجب عليك وضع `div.beak.bottom` بعد `div.beak.top`.
