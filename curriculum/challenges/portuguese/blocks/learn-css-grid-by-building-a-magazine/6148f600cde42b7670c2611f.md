...
Crie outra `@media` query para `only screen` com `max-width` de `600px`.Dentro, crie um seletor `.text-with-images` e dê a ele uma propriedade `grid-template-columns` de `1fr`.

Isto recolherá a área de texto mais abaixo em uma única coluna em telas menores.

# --hints--

Você deve criar uma `@media` query para `only screen and (max-width: 600px)`.Ela ficará abaixo da `@media` query anterior.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText === 'only screen and (max-width: 600px)');
```

A nova `@media` query deverá ter um seletor `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText === '.text-with-images');
```

O novo seletor `.text-with-images` deve ter a propriedade `grid-template-columns` com o valor de `1fr`.
