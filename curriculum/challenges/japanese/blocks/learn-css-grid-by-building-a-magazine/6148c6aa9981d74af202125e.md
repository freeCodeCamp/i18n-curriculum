...
`.text` セレクターに `font-size` プロパティを `1.8rem` に設定し、`letter-spacing` プロパティを `0.6px` に設定してください。

# --hints--

`.text` セレクターには `font-size` プロパティを `1.8rem` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize === '1.8rem');
```

`.text` セレクターには `letter-spacing` プロパティを `0.6px` に設定してください。
