...
在这个例子中，第一个段落将是红色，因为 ID 选择器的特异性高于类选择器和类型选择器。`.example-para` 元素的颜色将是绿色，而其余的段落元素颜色将是蓝色。
...
这里是一个使用 `*` 选择器将所有元素颜色设置为红色的例子。不过你不会看到任何红色元素，因为有 ID 和类型选择器覆写了这些样式，这突显了 `*` 选择器的低特异性。
:::interactive_editor
...
内部 CSS 定义在 HTML 文档的 `head` 部分的 `style` 元素内。它的优先级低于内联样式。内部和外部 CSS 具有相同的优先级水平——优先级由所使用的选择器决定，而不是 CSS 定义的位置。当两个规则具有相同的优先级时，文档中后出现的规则优先。因此，如果外部样式表在内部 `style` 块之后被链接，外部规则优先。如果它在之前被链接，内部规则优先。
...
在此示例中，文本将是蓝色，除非应用了内联样式或更具体的选择器。

外部 CSS 通过 `head` 部分的 `link` 元素链接，并写在单独的 `.css` 文件 中。像内部 CSS 一样，外部样式的特异性由所使用的选择器决定。外部 CSS 为较大项目提供了最佳的可维护性。
...
在此示例中，文本颜色在 `styles.css` 文件中定义，除非被更具体的选择器或内联样式覆盖，否则将应用该颜色。
