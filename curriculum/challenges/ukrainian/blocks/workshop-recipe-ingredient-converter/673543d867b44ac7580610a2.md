...
На цьому кроці ви завершите функцію `updateResultsList`.

Тепер ітеруйте по масиву `units`. Для кожного значення перевірте, чи воно не дорівнює поточній одиниці (`unitToConvert.value`).

Якщо умова істинна, перетворіть існуючу одиницю на поточну одиницю та додайте новий елемент списку до `resultList` у форматі:

`ingredient: quantity unit`

Наприклад, якщо вхідні дані `flour`, `5` і `gram`, результат має бути таким:
...
При натисканні на кнопку ваша функція `updateResultsList` має правильно оновити DOM.
