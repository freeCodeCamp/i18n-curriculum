...
算命应用支持渲染多张卡片，因此你需要一个可以帮助渲染多张卡片的方法。声明一个名为 `multipleCardSelected` 的方法。

在该方法内部，调用 `hideElements` 函数，并传入属性 `singleCard`、`singleCardBtn`、`multipleCardsBtn` 和 `headerTitle`。别忘了你必须从 `this.elements` 访问它们。

# --hints--

你应该声明一个 `multipleCardSelected` 方法。
...
你应该在 `hideElements` 方法内部调用 `multipleCardSelected` 函数。
...
你应该将 `singleCard` 作为第一个参数传递给 `hideElements` 函数。
...
你应该将 `singleCardBtn` 作为参数传递给 `hideElements` 函数。
...
你应该将 `multipleCardsBtn` 作为参数传递给 `hideElements` 函数。
...
你应该将 `headerTitle` 作为参数传递给 `hideElements` 函数。
