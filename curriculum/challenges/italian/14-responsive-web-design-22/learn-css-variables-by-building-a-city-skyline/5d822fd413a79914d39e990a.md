...
Dovresti spostare `display` con il valore `flex` in `.building-wrap`.
...
Dovresti spostare `flex-direction` con il valore `column` in `.building-wrap`.
...
Dovresti spostare `align-items` con il valore `center` in `.building-wrap`.
