...
Dentro de tu `thead`, crea un elemento `tr`. Dentro de eso, agrega un elemento `td` y tres elementos `th`. Dentro de cada uno de los elementos `th`, añade un elemento `span` con la `class` establecida en `sr-only` y el siguiente texto, en el siguiente orden: `2019`, `2020`y `2021`.
...
Cada uno de tus nuevos elementos `span` debe tener el atributo `class` establecido en `sr-only`.
