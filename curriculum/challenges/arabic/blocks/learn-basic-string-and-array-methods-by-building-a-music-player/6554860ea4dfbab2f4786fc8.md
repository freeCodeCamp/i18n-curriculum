...
إذا تحققت عن قرب، سترى أن الأغنية التي تُشغل حاليًا غير مميزة في قائمة التشغيل، لذلك لا تعرف حقًا أي أغنية تُشغل. يمكنك إصلاح ذلك بإنشاء دالة لتحديد أي أغنية تُشغل.

باستخدام صيغة السهم، أنشئ دالة `highlightCurrentSong`. داخل الدالة، استخدم `querySelectorAll` للحصول على عنصر `.playlist-song` وعيّنه في ثابت `playlistSongElements`.

# --hints--

يجب استخدام `const` لإنشاء دالة `highlightCurrentSong`.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

يجب أن تكون `highlightCurrentSong` دالة.

```js
assert.isFunction(highlightCurrentSong)
```

يجب أن تستخدم دالة `highlightCurrentSong` صيغة السهم.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

يجب إنشاء ثابت `playlistSongElements` داخل دالة `highlightCurrentSong` الخاصة بك.

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

يجب استخدام طريقة `querySelectorAll()` لاختيار عنصر `.playlist-song` وتعيينه إلى الثابت `playlistSongElements`.
