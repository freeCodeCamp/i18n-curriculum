...
`if` 文を作成してください。その条件には `not` 演算子を使って `is_sequence` を否定してください。`if` 文の中で `Invalid format: expected a list or tuple.` を出力し、`False` を返してください。

# --hints--

`if` 関数の中に `validate` 文を作成してください。
...
`if` 文の条件は `not is_sequence` にしてください。
...
`Invalid format: expected a list or tuple.` 文の中で `if` を出力してください。
