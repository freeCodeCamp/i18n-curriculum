...
問題番号は十分に説明的ではありません。これは視覚障害のあるユーザーにとって特にそうです。この問題を回避する一つの方法は、要素に目に見えるテキストを追加せずに、スクリーンリーダーだけが読み取れるテキストを追加することです。

各`span`要素の番号の前に、`class`の`sr-only`を持つ`h3`要素をネストしてください。

# --hints--

最初の`span`要素の中に`h3`要素を追加してください。

```js
assert.exists(document.querySelector('h3 > span'));
```

最初の`span`要素に`class`の`sr-only`を付与してください。

```js
assert.equal(document.querySelector('span')?.className, 'sr-only');
```

2番目の`span`要素の中に`h3`要素を追加してください。

```js
assert.exists(document.querySelectorAll('h3 > span')[1]);
```

2番目の`span`要素に`class`の`sr-only`を付与してください。

```js
assert.equal(document.querySelectorAll('span')[1]?.className, 'sr-only');
```

最初の`span`要素の中の数字1の前に`h3`要素を追加してください。

```js
assert.equal(document.querySelector('span')?.nextSibling?.textContent.trim(), '1');
```

2番目の`span`要素の中の数字2の前に`h3`要素を追加してください。
