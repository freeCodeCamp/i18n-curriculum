...
在 `.eye.right` 元素下方，添加兩個 `div` 元素，每個元素的 `class` 為 `blush`。此外，為第一個 `.blush` 元素添加 `class` 為 `left`，為第二個 `.blush` 元素添加 `class` 為 `right`。
...
你應該給第一個新的 `div` 一個 `class` 為 `blush`。

```js
assert.exists(document.querySelector('.penguin-head > div.blush'));
```

你應該給第二個新的 `div` 一個 `class` 為 `blush`。
