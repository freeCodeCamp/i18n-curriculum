...
داخل الدالة `getElement`، استخدم `querySelector` مع الأنواع العامة وخزن النتيجة في متغير. أضف جملة `if` للتحقق مما إذا كان العنصر غير موجود، وإذا كان كذلك، قم برمي خطأ بالرسالة `Element not found: ${error message here}`. إذا كان العنصر موجودًا، أعده.

# --hints--

يجب إعلان متغير باسم `el` داخل الدالة `getElement`.
...
يجب تعيين المتغير `el` بالقيمة `document.querySelector<T>(selector)`.
...
يجب أن تحتوي على جملة if الشرطية تتحقق مما إذا لم يُعثر على `el`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

يجب رمي خطأ جديد برسالة `Element not found: ${selector}` داخل جملة `if` الخاصة بك.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

يجب إعادة المتغير `el` داخل دالة getElement.
