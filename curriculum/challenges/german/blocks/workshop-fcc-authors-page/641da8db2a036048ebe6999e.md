...
Setzen Sie innerhalb der `fetchMoreAuthors`-Funktion die Variablen `startingIndex` und `endingIndex` jeweils auf `+= 8`.
...
Sie sollten die Variable `startingIndex` auf `+=8` setzen.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Sie sollten die Variable `endingIndex` auf `+=8` setzen.
