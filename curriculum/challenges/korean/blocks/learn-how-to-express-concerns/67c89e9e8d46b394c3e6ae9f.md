---
id: 67c89e9e8d46b394c3e6ae9f
title: 작업 87
challengeType: 22
dashedName: task-87
lang: en-US
---

<!-- (Audio) James: Sometimes we get updates too late and it affects our work. -->

# --instructions--

오디오를 듣고 아래 문장을 완성하세요.

# --fillInTheBlank--

## --sentence--

`Sometimes we get updates too late and it BLANK our work.`

## --blanks--

`affects`

### --feedback--

이것은 무언가에 영향을 미치거나 변화를 일으킨다는 뜻입니다.

# --explanation--

`To affect`는 무언가에 영향을 미치거나 변화를 일으킨다는 뜻입니다. 예를 들어:

`The weather can affect our travel plans.` - 이것은 날씨가 우리가 여행하는 방식에 영향을 미치거나 변화를 줄 수 있다는 뜻입니다.  

# --scene--

```json
{
  "setup": {
    "background": "company2-boardroom.png",
    "characters": [
      {
        "character": "James",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.2
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_12-2.mp3",
      "startTime": 1,
      "startTimestamp": 51.48,
      "finishTimestamp": 54.42
    }
  },
  "commands": [
    {
      "character": "James",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "James",
      "startTime": 1,
      "finishTime": 3.94,
      "dialogue": {
        "text": "Sometimes we get updates too late and it affects our work.",
        "align": "center"
      }
    },
    {
      "character": "James",
      "opacity": 0,
      "startTime": 4.44
    }
  ]
}
```
