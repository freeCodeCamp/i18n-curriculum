...
這些圓鈕尚未被指定值。因此，透過表單發出的 POST 請求將不會記錄選擇了哪個選項。

為了使請求完全強固，請為兩個圓鈕分別添加 `value` 屬性，值為 `personal` 和 `business`。 

# --hints--

你應該為第一個單選輸入給予 `value` 屬性，且值為 `personal`。

```js
assert.equal(document.querySelector('fieldset:nth-child(2) label:nth-child(1) input[type="radio"]')?.value, 'personal');
```

你應該為第二個單選輸入給予 `value` 屬性，且其值為 `business`。
