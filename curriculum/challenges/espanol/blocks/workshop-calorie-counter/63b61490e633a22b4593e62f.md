...
Siguiendo el mismo patrón, asigna tu `#add-entry` elemento a `addEntryButton`, tu `#clear` elemento a `clearButton`, y tu `#output` elemento a `output`.

# --hints--

Debes declarar una variable llamada `addEntryButton`.

```js
assert.isDefined(addEntryButton);
```

Debes usar `document.getElementById()` para obtener el `#add-entry` elemento.

```js
assert.match(code, /document\.getElementById\(\s*('|")add-entry\1\s*\)/g);
```

Debes asignar el `#add-entry` elemento a `addEntryButton`.

```js
assert.deepEqual(addEntryButton, document.getElementById('add-entry'));
```

Debes declarar una variable llamada `clearButton`.

```js
assert.isDefined(clearButton);
```

Debes usar `document.getElementById()` para obtener el `#clear` elemento.

```js
assert.match(code, /document\.getElementById\(\s*('|")clear\1\s*\)/g);
```

Debes asignar el `#clear` elemento a `clearButton`.

```js
assert.deepEqual(clearButton, document.getElementById('clear'));
```

Debes declarar una variable llamada `output`.
...
Debes usar `document.getElementById()` para obtener el `#output` elemento.

```js
assert.match(code, /document\.getElementById\(\s*('|")output\1\s*\)/g);
```

Debes asignar el `#output` elemento a `output`.
