...
Тепер додайте `div` з класом `img-loader`. Після цього створіть елемент `img` і надайте йому умовне посилання на зображення, використовуючи `{img ? ${CDN_URL}/${img} : LOCAL_DEFAULT_IMG}`.

# --hints--

Усередині вашого елемента `div` має бути елемент `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

Ваш `div` має мати клас `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

Усередині вашого елемента `img` має бути елемент `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

Елемент `img` має використовувати `CDN_URL` і `img` для свого `src`, коли надається `img`, і повертатися до `LOCAL_DEFAULT_IMG`, коли його немає.
...
Ваш елемент `img` має мати класи `card-img` і `hidden`.
