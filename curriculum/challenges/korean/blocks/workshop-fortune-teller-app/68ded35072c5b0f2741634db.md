...
DOM에 있는 요소를 보여 주는 또 다른 함수를 만들어야 합니다. 이전 함수와 비슷하게 `showElements`라는 이름의 함수를 만드세요. 이번에는 `classList.remove("hidden")`을 사용하세요.

# --hints--

`showElements`라는 이름의 함수가 있어야 합니다.
...
`showElements` 함수는 `...elements`라는 이름의 매개변수를 가지고, 그 타입은 `HTMLElement[]`여야 합니다.
...
`showElements` 함수에는 `forEach` 매개변수에 연결된 `elements` 메서드가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

`forEach` 메서드는 `el`이라는 매개변수를 받아야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

`hidden` 메서드 안에서 `classList.remove` 메서드를 사용해 `forEach` 클래스를 제거해야 합니다.
