...
Kama hakuna wimbo unaofuata katika orodha ya nyimbo, tumia kipande cha `else` kurekebisha ufunguo wa `currentSong` wa `userData` kuwa `null`, na sifa yake ya `songCurrentTime` kuwa `0`.
...
Tamko lako la `if` linapaswa kuwa na kipande cha `else`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Unapaswa kutumia uandishi wa nukta kuweka sifa ya `currentSong` ya `userData` kuwa `null` katika kipande cha `else` cha tamko lako la `if`.

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

Unapaswa kutumia uandishi wa nukta kuweka sifa ya `songCurrentTime` ya `userData` kuwa `0` katika kipande cha `else` cha tamko lako la `if`.
