...
أعلن دالة `apply` التي تأخذ معلمتين `fn` و `args`.

# --hints--

يجب إعلان متغير `apply`.
...
يجب استخدام `const` لإعلان متغير `apply`.
...
يجب تعيين متغير `apply` إلى دالة سهمية.
...
يجب أن تحتوي دالة `apply` على `fn` كمعلم أول.
...
يجب أن تحتوي دالة `apply` على `args` كمعلم ثاني.
