...
Якщо `value` `numberInput` дорівнює `5`, викличте функцію `showAnimation()`, а потім достроково `return`.

# --hints--

Ви повинні викликати функцію `showAnimation()` у тілі другої інструкції `if`.

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

Після виклику функції `showAnimation()` слід використати ключове слово `return`, щоб достроково вийти з функції `checkUserInput()`.
