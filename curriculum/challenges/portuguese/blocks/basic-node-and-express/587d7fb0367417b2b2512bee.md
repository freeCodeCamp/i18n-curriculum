...
Use o método `app.get()` para servir a string "Hello Express" para solicitações de GET que correspondam ao caminho `/` (root). Certifique-se de que seu código funciona olhando os logs.

**Nota:** Todo o código para estas lições deve ser adicionado entre as poucas linhas de código com as quais começamos para você.
