...
لاستدعاء تعبير دالة سهمية مسماة، يمكنك الإشارة إلى الدالة باسمها.

```js
exampleArrowFunction();
```

أسفل تعريف دالة `printGreeting`، استدعِ الدالة. افتح وحدة التحكم لرؤية الناتج.

# --hints--

يجب أن تستدعي دالتك `printGreeting`.

```js
assert.match(code, /printGreeting\(\);?/);
```

يجب أن يكون استدعاء دالة `printGreeting` أسفل تعريف الدالة `printGreeting`.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

يجب أن يُظهر استدعاء دالة `printGreeting` النص `Hello there!` في وحدة التحكم.
