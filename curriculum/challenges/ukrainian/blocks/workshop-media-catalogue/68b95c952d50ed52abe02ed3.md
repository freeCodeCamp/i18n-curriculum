...
Додайте до свого класу `MediaError` метод `__init__` з такими параметрами: `self`, `message` і `obj`.

Всередині нього викличте `super().__init__(message)`, щоб переконатися, що механізми збереження та відображення повідомлення про помилку, які вже є в класі `Exception`, можуть працювати правильно.

Після цього призначте `obj` `self.obj`.

# --hints--

Ваш клас `MediaError` має мати метод `__init__` з параметрами `self`, `message` і `obj`.
...
Ви маєте викликати `super().__init__(message)` всередині свого методу `__init__`.
...
Ви маєте призначити `obj` `self.obj` всередині свого методу `__init__`.
