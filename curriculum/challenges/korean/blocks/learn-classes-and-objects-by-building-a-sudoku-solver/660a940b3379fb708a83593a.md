...
`if` 문을 만들어 `solver()` 객체에서 `gameboard` 메서드 호출이 `True`를 반환하는지 확인하세요.

그런 다음 `print()` 본문 안에 `if` 호출을 추가하고 다음 f-문자열을 전달하세요: `f'Solved puzzle:\n{gameboard}'`.


# --hints--

`if` 객체에서 `solver()` 메서드 호출이 `gameboard`를 반환하는지 확인하는 `True` 문을 만들어야 합니다.
