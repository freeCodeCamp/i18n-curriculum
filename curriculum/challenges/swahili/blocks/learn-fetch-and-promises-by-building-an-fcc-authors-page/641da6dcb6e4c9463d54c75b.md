...
Ndani ya mfuatano wa herufi wa template, tengeneza kipengele cha `div` chenye `id` iliyowekwa kuwa `index` kutoka kwa njia ya safu ya data ya `.forEach()`. Kumbuka kutumia uingizaji wa thamani kufanya hivi.

Pia, ongeza `class` ya `"user-card"` kwa `div`.
...
Kipengele chako cha `div` kinapaswa kuwa na `id` ya `index`. Kumbuka kutumia uingizaji wa thamani kuweka `id`.
...
Kipengele chako cha `div` kinapaswa kuwa tupu.
