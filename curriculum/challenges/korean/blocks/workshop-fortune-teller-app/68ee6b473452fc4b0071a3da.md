...
이제 `multipleCardsBtn`에 `newReadingBtn`를 호출하는 클릭 이벤트 리스너를, `multipleCardsBtn`에 `multipleCardSelected`를 호출하는 또 다른 클릭 이벤트 리스너를 각각 붙여 `newReadingBtn`과 `newReading`에도 동일하게 하세요.

두 메서드는 다음 단계에서 만들게 됩니다.

# --hints--

`click` 요소에 `multipleCardsBtn` 이벤트를 붙여야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

`multipleCardsBtn` 콜백 이벤트 안에서 `multipleCardSelected` 메서드를 호출해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

`click` 요소에 `newReadingBtn` 이벤트를 붙여야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

`newReadingBtn` 콜백 이벤트 안에서 `newReading` 메서드를 호출해야 합니다.
