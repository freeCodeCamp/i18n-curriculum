...
Ora prova ad aggiungere l'oggetto `ballerina` al tuo `catalogSet` all'interno della tua funzione per la seconda volta. Vedrai che non succede nulla, perché l'oggetto è già archiviato nel tuo insieme e gli elementi duplicati non sono permessi negli insiemi.

Se vuoi, puoi controllare il numero di elementi nel tuo insieme usando la proprietà `size`, nello stesso modo in cui l'hai fatto prima per la tua mappa.

# --hints--

Dovresti `add` l'oggetto `ballerina` una seconda volta al tuo insieme all'interno della funzione `displayPlantsSet`.
