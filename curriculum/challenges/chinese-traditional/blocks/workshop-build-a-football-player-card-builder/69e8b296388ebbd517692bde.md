...
為 DRI 輸入新增一個 `value` 屬性，值為 `player.dri`。接著新增一個 `onChange` 屬性，設定為一個箭頭函式，該函式以 `e` 作為參數，並呼叫 `setPlayer`，傳入展開的 `player` 和 `dri`，其中 dri 設為 `Number(e.target.value)`。

# --hints--

你的 DRI `input` 應該有一個 `value`，其值與玩家的 `dri` 狀態相等。
...
你的 DRI `input` 應該有一個 `onChange` 處理常式，該處理常式呼叫 `setPlayer`，傳入展開的 `player` 和 `dri`，其中 dri 設為 `Number(e.target.value)`。
