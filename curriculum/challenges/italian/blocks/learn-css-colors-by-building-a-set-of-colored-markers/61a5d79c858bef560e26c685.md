...
E come terzo argomento colore, passa la funzione `hsl` con i valori `240` per la tonalità, `56%` per la saturazione e `42%` per la luminosità.

# --hints--

La tua regola CSS `.blue` dovrebbe avere una proprietà `background` impostata su `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))`.
