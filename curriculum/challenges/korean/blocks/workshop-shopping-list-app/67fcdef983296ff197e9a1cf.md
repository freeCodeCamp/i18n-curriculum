...
`filteredItems`라는 변수를 만들고 `items` 메서드로 `filter()` 배열을 필터링한 결과를 할당하세요. `filter()` 메서드에 익명 함수를 전달하는데, 이 함수는 `item`를 인수로 받습니다. 익명 함수 안에서는 일단 `item`를 반환하세요.

# --hints--

`filteredItems`라는 변수를 만들어야 합니다.

```js
assert.match(code, /(const|let|var)\s+filteredItems\s*=/);
```

`filter()` 배열에 `items` 메서드를 사용해야 합니다.

```js
assert.match(code, /items\.filter\s*\(/);
```

`filter()` 메서드는 `item`를 인수로 받고 `item`를 반환하는 익명 함수를 인수로 받아야 합니다.
