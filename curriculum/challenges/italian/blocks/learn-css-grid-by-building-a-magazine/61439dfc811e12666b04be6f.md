...
Aggiungi un terzo elemento `p` alla fine del tuo elemento `.text` e assegnagli il seguente testo:
...
Dovresti creare un terzo elemento `p` nel tuo elemento `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 3);
```

Il tuo terzo elemento `p` dovrebbe contenere il testo fornito.
