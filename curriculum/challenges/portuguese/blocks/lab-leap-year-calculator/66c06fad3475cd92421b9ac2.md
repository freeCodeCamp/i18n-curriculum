...
5. Se o ano for bissexto, retorne `[year] is a leap year.`. Caso contrário, retorne `[year] is not a leap year.`. Você vai substituir `[year]` pelo parâmetro definido na função `isLeapYear`.
6. Você deve chamar a função `isLeapYear` com `year` como argumento e atribuir o resultado a uma variável chamada `result`.
7. Você deve exibir a variável `result` no console usando `console.log()`.
...
A função `isLeapYear` deve ter um parâmetro.
