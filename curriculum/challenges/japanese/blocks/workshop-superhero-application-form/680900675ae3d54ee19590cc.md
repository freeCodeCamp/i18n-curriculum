...
`powerSourceOptions` の値が `select` 要素の選択肢として表示されるようになりました。

次に、`powerSource` 要素の値として `select` を使ってください。そして、`onChange` をパラメータに取り、`e` を暗黙的に返すアロー関数を使って `setPowerSource(e.target.value)` ハンドラを設定してください。

# --hints--

`{powerSource}` 要素の値として `select` を使うべきです。

```js
assert.match(code, /<\s*select\s+[\s\S]*value\s*=\s*\{\s*powerSource\s*\}/)
```

アロー関数を使って `select` で `onChange` 要素の値を設定し、`powerSource` で `setPowerSource` の状態を更新するようにしてください。
