...
Метод `sort()` приймає функцію зворотного виклику для порівняння, яка визначає порядок сортування.

У цьому прикладі перша умова `(a.name < b.name)` перевіряє, чи є назва першого фрукта меншою за назву другого фрукта. Якщо так, перший фрукт сортується перед другим.

Рядки порівнюються лексикографічно, тобто символ за символом. Наприклад, `"Apples"` менший за `"Bananas"`, бо `"A"` йде перед `"B"` в алфавіті.

Причина, чому цей приклад повертає числа, полягає в тому, що метод `sort()` очікує, що буде повернене число. Якщо ви повернете від’ємне число, перший елемент буде відсортований перед другим.
...
Усередині вашої функції зворотного виклику додайте інструкцію `if`, щоб перевірити, чи `a.title` менший за `b.title`. Якщо так, поверніть `-1`.

# --hints--

У вашій функції зворотного виклику має бути інструкція `if`.

```js
assert.match(code, /if\s*\(/);
```

Ваша інструкція `if` має перевіряти, чи `a.title` менший за `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

Ваша інструкція `if` має повертати `-1`, якщо `a.title` менший за `b.title`.
