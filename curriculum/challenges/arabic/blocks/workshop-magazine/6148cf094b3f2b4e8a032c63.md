...
يتيح لك المحدِّد الزائف `::first-letter` استهداف الحرف الأول في محتوى النص لعنصر ما.

أنشئ محدِّد `.first-paragraph::first-letter` واضبط خاصية `font-size` على `6rem`. وأضف له أيضًا خاصية `color` بقيمة `orangered` ليبرز.
