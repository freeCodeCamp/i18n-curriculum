...
Завершіть функцію, використавши метод масиву `forEach` для ітерування над параметром elements. Усередині функції зворотного виклику додайте клас hidden до кожного елемента, використовуючи метод `.add()` об’єкта `classList`.

# --hints--

У вас має бути метод `forEach`, приєднаний до параметра `elements`.
...
Ваш метод forEach має приймати параметр функції зворотного виклику.
...
Всередині методу forEach слід додати клас hidden, використовуючи метод `classList.add`.
