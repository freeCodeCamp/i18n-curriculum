...
下一步是為你的 `CheckPoint` 類別添加 `width` 和 `height`。 

`width` 和 `height` 應分別為 `proportionalSize(40)` 和 `proportionalSize(70)`。

# --hints--

你應該在 `CheckPoint` 類別中有一個 `width` 屬性。
...
你應該在 `CheckPoint` 類別中有一個 `height` 屬性。
...
你應該將 `width` 屬性指定為 `proportionalSize(40)`。
...
你應該將 `height` 屬性指定為 `proportionalSize(70)`。
