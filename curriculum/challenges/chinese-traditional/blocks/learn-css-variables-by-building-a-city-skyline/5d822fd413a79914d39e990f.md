...
窗戶在紫色建築後面的左側堆疊在一起。 在 `.building-wrap` 下面添加一個名爲 `window-wrap` 的新類。 使 `.window-wrap` 成爲一個 flexbox 容器，使用 `align-items` 和 `justify-content` 屬性將它的子元素垂直居中，並分別在它們的父元素中均勻排布。

# --hints--

你應該創建一個 `.window-wrap` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".window-wrap"));
```

你應該給 `.window-wrap` 添加一個值爲 `flex` 的 `display`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".window-wrap")?.display, "flex");
```

你應該給 `.window-wrap` 添加一個值爲 `center` 的 `align-items`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".window-wrap")?.alignItems, "center");
```

你應該給 `.window-wrap` 添加一個值爲 `space-evenly` 的 `justify-content`。
