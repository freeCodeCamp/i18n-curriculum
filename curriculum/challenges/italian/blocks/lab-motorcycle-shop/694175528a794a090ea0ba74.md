...
title: Laboratorio negozio di motociclette
challengeType: 25
...
Per questo laboratorio, ti sono stati forniti tutto l'HTML e il CSS. Userai TypeScript per completare le funzionalità di una pagina di un negozio di motociclette.

**Obiettivo:** Completa le storie utente qui sotto e fai superare tutti i test per completare il laboratorio.

**Storie utente:**

1. Dovresti creare un `type` chiamato `Category` che consiste nei seguenti valori possibili: `'Sport'`, `'Cruiser'`, `'Touring'`, `'Dirt'`, `'Adventure'`, `'Naked'`, `'Electric'`.
2. Dovresti creare un `interface` chiamato `Motorcycle` che ha le seguenti proprietà: un `id` di tipo `string`, un `name` di tipo `string`, un `manufacturer` di tipo `string`, un `category` di tipo `Category`, un `price` di tipo `number`, un `image_url` di tipo `string`, una proprietà `created_at` di tipo `Date`, un `description` di tipo `string` e un year di tipo `number`
3. Dovresti creare una funzione chiamata `fetchMotorcycles` che restituisce una `Promise` che risolve in un array di oggetti `Motorcycle`.
4. Dovresti caricare i dati `Motorcycle` dall'endpoint `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
5. Dovresti creare una funzione chiamata `renderMotorcycleCard` che prende un singolo oggetto `Motorcycle` come parametro e restituisce una stringa contenente l'HTML formattato per la scheda della motocicletta.
   1. La funzione `renderMotorCycleCard` dovrebbe prendere un parametro chiamato `motorcycle` di tipo `Motorcycle`.
   2. La funzione `renderMotorCycleCard` dovrebbe restituire un tipo `string`.
   3. Ogni componente scheda motocicletta dovrebbe avere un elemento `img` che contiene un'immagine valida e una classe `motorcycle-card-image-container`.
   4. Ogni componente scheda motocicletta dovrebbe avere un elemento con classe `motorcycle-card-year-badge` che elenca l'anno in testo semplice.
   5. Ogni componente scheda motocicletta dovrebbe avere un elemento con classe `motorcycle-card-title` che elenca il nome della motocicletta in testo semplice.
   6. Ogni componente scheda motocicletta dovrebbe avere un elemento con classe `motorcycle-card-manufacturer` che elenca il nome del produttore della motocicletta in testo semplice.
   7. Ogni componente scheda motocicletta dovrebbe avere un elemento con classe `motorcycle-card-category` che elenca la categoria della motocicletta in testo semplice.
   8. Ogni componente scheda motocicletta dovrebbe avere un elemento con classe `motorcycle-card-description` che elenca la descrizione della motocicletta in testo semplice.
   9. Ogni componente scheda motocicletta dovrebbe avere un elemento con classe `motorcycle-card-price` che elenca il prezzo della motocicletta in testo semplice.
   10. Ogni componente scheda motocicletta dovrebbe avere un elemento con classe `motorcycle-card-engine` che elenca la potenza di ogni motore.
6. Dovresti avere una classe chiamata `MotorcycleGalleryApp`.
   1. La classe dovrebbe avere un array `private` chiamato `allMotorcycles` che è tipizzato staticamente come un array di oggetti `Motorcycle`.
   2. Questa classe dovrebbe avere una funzione chiamata `renderMotorcycles` che renderizza i componenti scheda motocicletta equivalenti alla variabile `allMotorcycles` all'interno dell'elemento con id `motorcycle-grid`.
7. Il numero di elementi `Motorcycle` visualizzati dovrebbe essere elencato all'interno di un elemento con id `results-number`.
8. Il filtro dovrebbe essere aggiornato su un evento `input`.

# --hints--

Dovresti avere un `type` chiamato `Category`.
...
Il tuo tipo `Category` dovrebbe essere impostato come unione dei valori corretti.
...
Dovresti avere un `interface` chiamato `Motorcycle`.
...
Dovresti avere tutte le proprietà specificate nell'interfaccia `Motorcycle`.
...
Dovresti avere una funzione chiamata `fetchMotorcycles`.

```js
assert.isFunction(fetchMotorcycles);
```

Dovresti usare l'endpoint `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
...
Dovresti restituire una promise di 25 oggetti `Motorcycle` da `fetchMotorcycles`.
...
Dovresti avere una funzione chiamata `renderMotorcycleCard`.

```js
assert.isFunction(renderMotorcycleCard);
```

La funzione `renderMotorcycle` dovrebbe accettare un parametro tipizzato staticamente `Motorcycle`.
...
La funzione `renderMotorcycleCard` dovrebbe restituire esplicitamente un tipo `string`.
...
La funzione `renderMotorcycleCard` dovrebbe restituire un `string`.
...
Dovresti avere una classe chiamata `MotorcycleGalleryApp`.
...
`MotorcycleGalleryApp` dovrebbe avere un array chiamato `allMotorcycles`.
...
La proprietà `allMotorcycles` dovrebbe essere `private`.
...
La proprietà `allMotorcycles` dovrebbe essere tipizzata staticamente come `Motorcycle[]`.
...
`MotorcycleGalleryApp` dovrebbe avere una funzione chiamata `renderMotorcycles`.
...
La funzione `renderMotorcycles` dovrebbe renderizzare 25 elementi con classe `motorcycle-card-image-container`.
...
La funzione `renderMotorcycles` dovrebbe renderizzare 25 elementi con classe `motorcycle-card-year-badge`.
...
La funzione `renderMotorcycles` dovrebbe renderizzare 25 elementi con classe `motorcycle-card-title`.
...
La funzione `renderMotorcycles` dovrebbe renderizzare 25 elementi con classe `motorcycle-card-manufacturer`.
...
La funzione `renderMotorcycles` dovrebbe renderizzare 25 elementi con classe `motorcycle-card-category`.
...
La funzione `renderMotorcycles` dovrebbe renderizzare 25 elementi con classe `motorcycle-card-description`.
...
La funzione `renderMotorcycles` dovrebbe renderizzare 25 elementi con classe `motorcycle-card-price`.
...
La funzione `renderMotorcycles` dovrebbe renderizzare 25 elementi con classe `motorcycle-card-engine`.
