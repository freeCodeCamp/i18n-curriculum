---
id: 5f3c866d5414453fc2d7b480
title: الخطوة 31
challengeType: 0
dashedName: step-31
---

# --description--

بدءًا من أسفل زوج coffee/price الحالي، أضف coffee و prices التالية باستخدام عناصر `article` مع عنصرين `p` متداخلين داخل كل منهما. كما كان الحال من قبل، يجب أن يحتوي نص العنصر `p` الأول على coffee flavor ويجب أن يحتوي نص العنصر `p` الثاني على price.

```bash
Caramel Macchiato 3.75
Pumpkin Spice 3.50
Hazelnut 4.00
Mocha 4.50
```

# --hints--

يجب أن يكون لديك خمسة عناصر `article`.

```js
assert.lengthOf(document.querySelectorAll('article'),5);
```

كل عنصر `article` يجب أن يحتوي على عنصرين `p`.

```js
const articles = document.querySelectorAll('article');
assert.lengthOf(articles?.[0]?.children, 2);
assert.lengthOf(articles?.[1]?.children, 2);
assert.lengthOf(articles?.[2]?.children, 2);
assert.lengthOf(articles?.[3]?.children, 2);
assert.lengthOf(articles?.[4]?.children, 2);
```

يجب أن يحتوي عنصر `article` الأول على عناصر `p` مع النص `French Vanilla` و `3.00`.

```js
const children = document.querySelector('article')?.children;
assert.match(children?.[0]?.innerText, /French Vanilla/i);
assert.match(children?.[1]?.innerText, /3\.00/i);
```

يجب أن يحتوي عنصر `article` الثاني على عناصر `p` مع النص `Caramel Macchiato` و `3.75`.

```js
const children = document.querySelectorAll('article')?.[1]?.children;
assert.match(children?.[0]?.innerText, /Caramel Macchiato/i);
assert.match(children?.[1]?.innerText, /3\.75/i);
```

يجب أن يحتوي عنصر `article` الثالث على عناصر `p` مع النص `Pumpkin Spice` و `3.50`.

```js
const children = document.querySelectorAll('article')?.[2]?.children;
assert.match(children?.[0]?.innerText, /Pumpkin Spice/i);
assert.match(children?.[1]?.innerText, /3\.50/i);
```

يجب أن يحتوي عنصر `article` الرابع على عناصر `p` مع النص `Hazelnut` و `4.00`.

```js
const children = document.querySelectorAll('article')?.[3]?.children;
assert.match(children?.[0]?.innerText, /Hazelnut/i);
assert.match(children?.[1]?.innerText, /4\.00/i);
```

يجب أن يحتوي عنصر `article` الخامس على عناصر `p` مع النص `Mocha` و `4.50`.

```js
const children = document.querySelectorAll('article')?.[4]?.children;
assert.match(children?.[0]?.innerText, /Mocha/i);
assert.match(children?.[1]?.innerText, /4\.50/i);
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Cafe Menu</title>
    <link href="styles.css" rel="stylesheet"/>
  </head>
  <body>
    <div class="menu">
      <main>
        <h1>CAMPER CAFE</h1>
        <p>Est. 2020</p>
        <section>
          <h2>Coffee</h2>
--fcc-editable-region--
          <article>
            <p>French Vanilla</p>
            <p>3.00</p>
          </article>
--fcc-editable-region--
        </section>
      </main>
    </div>
  </body>
</html>
```

```css
body {
  background-image: url(https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg);
}

h1, h2, p {
  text-align: center;
}

.menu {
  width: 80%;
  background-color: burlywood;
  margin-left: auto;
  margin-right: auto;
}
```
