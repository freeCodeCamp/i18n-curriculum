...
点击预览窗口的空白处。然后使用 `Tab` 键查看焦点在不同元素之间移动。要与示例交互，你需要启用交互式编辑器。
...
请注意，触发`accesskey`的具体组合键可能因浏览器和操作系统而异。 通常在 Windows 上是 `ALT + Specified Key`，在 Mac 上是 `CTRL + Option + Specified Key`。
...
这是一个为 `button` 元素设置焦点状态样式的示例。点击预览窗口的空白处，然后按下 `Tab` 键以聚焦该按钮。要与示例交互，你需要启用交互式编辑器。
