...
`place-items` 屬性可用來同時設定 `align-items` 和 `justify-items` 的值。`place-items` 屬性接受一個或兩個值。如果只提供一個值，該值會同時用於 `align-items` 和 `justify-items` 屬性。如果提供兩個值，第一個值會用於 `align-items` 屬性，第二個值會用於 `justify-items` 屬性。

為 `.image-wrapper` 選擇器添加一個設定為 `center` 的 `place-items` 屬性。

# --hints--

你的 `.image-wrapper` 選擇器應該有一個 `place-items` 屬性設定為 `center`。
