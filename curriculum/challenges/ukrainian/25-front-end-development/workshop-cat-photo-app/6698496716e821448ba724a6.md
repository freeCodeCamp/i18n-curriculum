...
Елемент `strong` повинен мати початковий тег. Opening tags have this syntax: `<elementName>`.

```js
assert.exists(document.querySelector('strong'));
```

Елемент `strong` повинен мати кінцевий тег. Closing tags have a `/` just after the `<` character.
