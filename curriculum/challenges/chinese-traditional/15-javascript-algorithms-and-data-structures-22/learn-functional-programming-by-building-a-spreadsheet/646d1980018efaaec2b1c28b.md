...
使用與 `addCharacters` 函數相同的語法來更新 `elemValue` 函數。 它不應再聲明 `inner`，而應隱式返回該函數。

# --hints--

你的 `elemValue` 函數應該使用隱式返回。
...
你的 `elemValue` 函數應該隱式返回一個帶有 `character` 參數的箭頭函數。
...
你的內部箭頭函數應返回以 `character + num` 作爲參數調用 `idToText()` 的結果。
