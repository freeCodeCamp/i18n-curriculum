...
现在，当你添加新的图标元素时，自动放置算法将启动。 但是，该算法默认新列宽为 `auto`，这与你当前的列不匹配。

你可以使用 `grid-auto-columns` 属性覆盖它。 给 `.social-icons` 选择器一个 `grid-auto-columns` 属性，设置为 `1fr`。

# --hints--

你的 `.social-icons` 选择器应该有一个 `grid-auto-columns` 属性，设置为 `1fr`。
