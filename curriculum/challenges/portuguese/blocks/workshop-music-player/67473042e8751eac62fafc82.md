...
Defina uma função `playSong` que recebe um único parâmetro que representará o identificador único da música que você deseja tocar. 

# --hints--

Você deve ter uma função vazia chamada `playSong`.

```js
assert.isFunction(playSong);
```

Sua função `playSong` deve receber um único parâmetro.
