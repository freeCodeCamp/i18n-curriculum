...
ضمن عنصر `div.answer` الأول، ادمج عنصر واحد `select` يكون required مع ثلاثة عناصر `option`.

اعطي العنصر `option` الأول `value` بقيمة `""` والنص `Select an option`. اعطي العنصر `option` الثاني `value` بقيمة `yes` والنص `Yes`. اعطي العنصر `option` الثالث `value` بقيمة `no` والنص `No`.

# --hints--

يجب عليك دمج عنصر `select` واحد داخل أول عنصر `div.answer`.

```js
assert.exists(document.querySelector('div.answer > select'));
```

يجب عليك تضمين ثلاثة عناصر `option` ضمن عنصر `select`.

```js
assert.equal(document.querySelectorAll('div.answer > select > option')?.length, 3);
```

يجب عليك إعطاء أول عنصر `option` سمة `value` بقيمة `""`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(1)')?.value, '');
```

يجب أن تعطي عنصر `option` الأول المحتوى النصي `Select an option`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(1)')?.textContent, 'Select an option');
```

يجب عليك إعطاء ثاني عنصر `option` سمة `value` بقيمة `yes`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(2)')?.value, 'yes');
```

يجب أن تعطي عنصر `option` الثاني المحتوى النصي `Yes`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(2)')?.textContent, 'Yes');
```

يجب عليك إعطاء ثالث عنصر `option` سمة `value` بقيمة `no`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(3)')?.value, 'no');
```

يجب أن تعطي عنصر `option` الثالث المحتوى النصي `No`.

```js
assert.equal(document.querySelector('div.answer > select > option:nth-child(3)')?.textContent, 'No');
```

يجب أن تعطي `select` سمة `required`.
