...
O construtor `Set` também aceita um iterável como argumento e cria um conjunto a partir dele:
...
Para completar este workshop, modifique seu `displayPlantsSet` para retornar um conjunto dos nomes comuns únicos das plantas no seu catálogo.

Para conseguir isso, crie um array contendo o `commonName` de cada objeto planta armazenado como uma chave do mapa `catalog`. Passe o array para o construtor `Set` e retorne o set da função.

# --hints--

Sua função `displayPlantsSet` deve retornar um conjunto contendo o `commonName` de cada planta no seu catálogo.
