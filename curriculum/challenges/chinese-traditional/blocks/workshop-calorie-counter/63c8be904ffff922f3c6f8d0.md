...
記得你之前寫過一個函式（程式）來清理使用者的輸入嗎？你需要在這裡使用那個函式（程式）。

將你的 `currVal` 宣告更新為呼叫 `cleanInputString` 並以 `item.value` 作為結果。

# --hints--

你應該在 `getCaloriesFromInputs` 函式（程式）內呼叫 `cleanInputString` 函式（程式）。

```js
assert.match(getCaloriesFromInputs.toString(), /cleanInputString\(/);
```

你應該將 `item.value` 作為 `cleanInputString` 函式的引數傳遞。

```js
assert.match(getCaloriesFromInputs.toString(), /cleanInputString\(\s*item\.value\s*\)/);
```

你應該將 `cleanInputString` 呼叫的結果指定給你的 `currVal` 變數。
