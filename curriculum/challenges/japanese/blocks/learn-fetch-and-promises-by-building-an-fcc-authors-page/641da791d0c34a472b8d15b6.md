...
今、`authorDataArr` は先ほどコンソールにログした `data` と同じです。これを確認するために `authorDataArr` をコンソールにログしてください。

`console.log()` 文の中で、テキスト `"Author Data Array:"` を最初の引数として、`authorDataArr` を2番目の引数として追加してください。テキストと `authorDataArr` はカンマで区切ってください。
...
`data` を `authorDataArr` 変数に代入する必要があります

```js
assert.match(code, /authorDataArr\s*=\s*data\s*;?/)
```

テキスト `"Author Data Array:"` を含むコンソールログが必要です。

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1/)
```

`"Author Data Array:"` のテキストと `authorDataArr` をカンマで区切ってください。

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1\s*,/)
```

`authorDataArr` はコンソールログ文の2番目の引数である必要があります。
