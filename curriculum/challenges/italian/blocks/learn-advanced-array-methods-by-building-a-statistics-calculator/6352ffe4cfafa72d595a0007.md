...
Il tuo calcolo finale è la <dfn>deviazione standard</dfn>, che è la radice quadrata della varianza.
...
Dovresti avere una funzione `getStandardDeviation`.

```js
assert.isFunction(getStandardDeviation);
```

La tua funzione `getStandardDeviation` dovrebbe avere un parametro `array`.

```js
assert.match(getStandardDeviation.toString(), /array/);
```

La tua funzione `getStandardDeviation` dovrebbe avere una variabile `variance`.

```js
assert.match(getStandardDeviation.toString(), /variance\s*=/);
```

La tua variabile `variance` dovrebbe avere il valore di `getVariance(array)`.
