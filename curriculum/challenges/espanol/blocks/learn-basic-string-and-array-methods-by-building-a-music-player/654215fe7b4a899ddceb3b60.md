...
Ahora mismo, el `songsHTML` es un array. Si intentaras mostrar esto tal como está, verías las canciones separadas por comas. Este no es el resultado deseado porque deseas mostrar las canciones como una lista. Para solucionar esto, necesitarás unir el array en una sola cadena utilizando el método <dfn>join()</dfn>.

El método `join()` se utiliza para concatenar todos los elementos de un array en una sola cadena. Toma un parámetro opcional llamado `separator` que se utiliza para separar cada elemento del array. Por ejemplo:
...
Encadena el método `join()` a tu método `map()` y pasa una cadena vacía como separador.

Para encadenar varios métodos, puedes llamar al método `join()` sobre el resultado del método `map()`. Por ejemplo:
...
Debes añadir `join("")` al código existente.
