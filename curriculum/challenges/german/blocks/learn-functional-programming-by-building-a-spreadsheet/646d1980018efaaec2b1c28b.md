...
Verwenden Sie dieselbe Syntax wie bei Ihrer `addCharacters`-Funktion, um Ihre `elemValue`-Funktion zu aktualisieren. Sie sollte `inner` nicht mehr deklarieren, sondern die Funktion implizit zurückgeben.

# --hints--

Ihre `elemValue`-Funktion sollte eine implizite Rückgabe verwenden.
...
Ihre `elemValue`-Funktion sollte implizit eine Pfeilfunktion mit einem `character`-Parameter zurückgeben.
...
Ihre innere Pfeilfunktion sollte eine implizite Rückgabe verwenden.
...
Ihre innere Pfeilfunktion sollte das Ergebnis des Aufrufs von `idToText()` mit `character + num` als Argument zurückgeben.
