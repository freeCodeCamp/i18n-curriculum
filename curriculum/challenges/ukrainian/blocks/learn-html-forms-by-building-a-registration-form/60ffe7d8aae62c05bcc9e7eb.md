...
З `display` зі значенням `block` кнопка відправки розташована вплотну до лівого краю свого батьківського елемента.

Використайте ту саму техніку, що й для центрування `form`, щоб поцентрувати кнопку відправки.

# --hints--

Ви повинні задати кнопці відправки `margin` зі значенням `0 auto`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.margin, '0px auto');
```

Ви не повинні задавати кнопці відправки `min-width` або `max-width`.
