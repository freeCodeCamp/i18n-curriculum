...
`setPlayButtonAccessibleText` 函數將 `aria-label` 屬性設置爲當前歌曲，或者播放列表中的第一首歌曲。 如果播放列表爲空，則將 `aria-label` 設置爲 `"Play"`。

在 `playButton` 元素上使用 `setAttribute` 方法設置名爲 `"aria-label"` 的屬性。 使用三元組，將屬性值設置爲 `Play ${song.title}` 或 `"Play"`（如果 `song?.title` 不可用）。

不要忘記這裏需要字符串插值，所以你需要使用反引號。

# --hints--

你不應修改現有的 `setPlayButtonAccessibleText` 函數及其內容。
...
`"aria-label"` 應該是 `setAttribute()` 方法的第一個值。

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

你的 `setAttribute` 方法應該有 ``song?.title ? `Play ${song.title}` : "Play"`` 作爲第二個參數。
