...
`MediaCatalogue` クラスに戻り、`A catalogue that can store different types of media items.` と記述したドキュメンテーション文字列を追加してください。

# --hints--

`A catalogue that can store different types of media items.` クラスに `MediaCatalogue` のドキュメンテーション文字列を追加してください。
