...
最後の `fieldset` 要素のボーダーが少し違和感があります。特定のタイプの最後の要素を選択するには、`last-of-type` CSS 疑似クラスを使うことができます。例えば、次のようにします。

```css
p:last-of-type { }
```

これで最後の `p` 要素が選択されます。最後の `fieldset` 要素をターゲットにする新しいセレクターを作成し、その `border-bottom` を `none` に設定してください。

# --hints--

特定のタイプの最後の要素だけを選択するには、`:last-of-type` 疑似クラスを使えます。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type'));
```

あなたの `fieldset:last-of-type` は `border-bottom` を `none` に設定する必要があります。
