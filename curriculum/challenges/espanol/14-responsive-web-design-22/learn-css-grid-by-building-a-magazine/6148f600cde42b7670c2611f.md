...
Create another `@media` query for `only screen` with a `max-width` of `600px`. Dentro, cree una regla `.text-with-images` y asígnele una propiedad `grid-template-columns` de `1fr`.
