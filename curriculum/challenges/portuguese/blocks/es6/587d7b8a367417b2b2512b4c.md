...
Use uma atribuição por desestruturação com a sintaxe rest para emular o comportamento de `Array.prototype.slice()`. `removeFirstTwo()` deve retornar um subarray do array original `list` com os dois primeiros elementos omitidos.
