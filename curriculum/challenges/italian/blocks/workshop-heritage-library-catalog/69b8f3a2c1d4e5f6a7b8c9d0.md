...
`parseCatalog` attualmente restituisce un array vuoto. Per riempirlo, aggiungi un loop `for` all'interno della funzione che itera su `rawCards`. All'interno del loop, chiama `parseCard` su ogni elemento e inserisci il risultato in `catalog`.

# --hints--

`parseCatalog(rawCatalogCards).length` dovrebbe essere `19`.

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

`parseCatalog` dovrebbe usare un loop `for` per iterare su `rawCards`.

```js
assert.match(parseCatalog.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*rawCards\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Dovresti chiamare `parseCard` all'interno del loop.
