...
Para el paso final, registrarás el mensaje del bot de `"Bueno, fue agradable hablar contigo. ¡Que tengas un buen día!"` en la consola.

¡Y con eso, tu bot de saludo está completo!

# --hints--

Debes tener una sentencia `console` en tu código.

```js
assert.lengthOf(__helpers.removeJSComments(code).match(/console\.log(.*)/g), 9);
```

Tu sentencia `console` debe registrar la cadena `"Bueno, fue agradable hablar contigo. ¡Que tengas un buen día!"`.
