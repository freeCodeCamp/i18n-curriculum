...
`.toFixed()`이 문자열을 반환하는 문제는 세율로 계산을 할 수 있어야 한다는 점입니다. 이를 고치려면 `.toFixed()` 호출(계산 포함)을 `parseFloat()` 함수에 전달하세요. 이렇게 하면 고정된 문자열이 다시 숫자로 변환되어 기존 소수 자릿수가 유지됩니다.

`.toFixed()` 호출을 `parseFloat()`에 전달하세요.

# --hints--

전체 계산식(`return` 문장은 제외)을 `parseFloat()`에 전달해야 합니다.
