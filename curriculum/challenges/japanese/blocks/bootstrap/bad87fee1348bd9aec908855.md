...
jQueryで各ボタンをその一意のidで指定できるようにしたいです。

各ボタンに一意のidを付けてください。idは`target1`で始まり、`target6`で終わるようにしてください。

`target1`から`target3`までは`#left-well`の中にあり、`target4`から`target6`までは`#right-well`の中にあることを確認してください。

# --hints--

1つの`button`要素にはid `target1`を付ける必要があります。
...
1つの`button`要素にはid `target2`を付ける必要があります。
...
1つの`button`要素にはid `target3`を付ける必要があります。
...
1つの`button`要素にはid `target4`を付ける必要があります。
...
1つの`button`要素にはid `target5`を付ける必要があります。
...
1つの`button`要素にはid `target6`を付ける必要があります。
