...
Dein drittes `table`-Element sollte ein `caption`-Element enthalten.

```js
assert(document.querySelectorAll('table')?.[2]?.children?.[0]?.localName === 'caption');
```

Dein `caption`-Element sollte den Text `Net Worth` enthalten.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('caption')?.textContent === 'Net Worth');
```

Dein drittes `table`-Element sollte ein `thead`-Element enthalten.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('thead'));
```

Dein drittes `table`-Element sollte ein `tbody`-Element enthalten.
