...
أضف بعض اللون إلى `#years`. اجعل النص بلون `#fff` والخلفية بلون `#0a0a23`.

# --hints--

يجب أن تحتوي `#years` على خاصية `color` مضبوطة على `#fff`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('color') === 'rgb(255, 255, 255)');
```

يجب أن تحتوي `#years` على خاصية `background-color` مضبوطة على `#0a0a23`.
