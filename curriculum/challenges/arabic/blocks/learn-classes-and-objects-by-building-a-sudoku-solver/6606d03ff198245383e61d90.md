...
الآن، كرر فقط على الصفوف داخل المربع 3x3 بإنشاء حلقة `for`. استخدم الدالة `range()` لتوليد متتالية تبدأ من `row_start`، واستخدم `row_no` كمتغير للحلقة.

# --hints--

يجب أن تنشئ حلقة `for` تكرر على `range(row_start, row_start + 3)`.
...
يجب أن تستخدم حلقة `for` المتغير `row_no` كمتغير للحلقة.
