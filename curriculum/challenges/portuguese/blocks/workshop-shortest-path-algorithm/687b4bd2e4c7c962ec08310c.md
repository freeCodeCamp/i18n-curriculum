...
Agora você vai demonstrar que sua função funciona. No final do seu programa, chame sua função `shortest_path` com a `adj_matrix` e um `start_node` de `0` e um `target_node` de `5` como argumentos.

Com isso, o algoritmo de caminho mais curto está completo!

# --hints--

Você deve chamar a função `shortest_path` com três argumentos: `adj_matrix`, `0` e `5`.
