...
أنشئ محدِّد `html` وأعطه خاصية `font-size` بقيمة `62.5%`. هذا سيضبط حجم الخط الافتراضي لصفحة الويب إلى 10px (القيمة الافتراضية للمتصفح هي 16px).

هذا سيجعل من الأسهل عليك العمل بوحدات `rem` لاحقًا، إذ سيكون `2rem` مساويًا لـ 20px.

# --hints--

يجب أن تنشئ محدِّد `html`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('html'));
```

يجب أن يحتوي محدِّد `html` على خاصية `font-size` بقيمة `62.5%`.
