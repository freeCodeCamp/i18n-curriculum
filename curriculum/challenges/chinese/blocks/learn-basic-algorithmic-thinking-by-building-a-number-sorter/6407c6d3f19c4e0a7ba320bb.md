...
在你的 `sortInputArray()` 函数中，调用你的 `updateUI()` 函数，并将 `inputValues` 作为参数传递。

现在你应该可以点击 `Sort` 按钮，并在 `Output` 部分看到输入的数组。

# --hints--

你应该在 `sortInputArray()` 函数中调用 `updateUI()`。

```js
assert.match(sortInputArray.toString(), /updateUI\(/);
```

你应该将 `inputValues` 作为参数传递给 `updateUI()`。
