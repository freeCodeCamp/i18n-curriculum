...
你应该在 `background` 属性中为 `.fb5` 提供第二个 `repeating-linear-gradient`。
...
你应该为第二个 `repeating-linear-gradient` 设置从 `0%` 到 `12%` 是第一个颜色 `--building-color2`。
...
你应该为第二个 `repeating-linear-gradient` 设置从 `12%` 到 `44%` 是第二种颜色 `--window-color2`。
