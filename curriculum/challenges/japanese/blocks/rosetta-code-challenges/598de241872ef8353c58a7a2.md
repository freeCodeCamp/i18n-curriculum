---
id: 598de241872ef8353c58a7a2
title: 二項係数を計算する
challengeType: 1
forumTopicId: 302259
dashedName: evaluate-binomial-coefficients
---

# --description--

n と k の値が与えられたときの二項係数を計算する関数を書いてください。

推奨される公式は次の通りです：

$\\binom{n}{k} = \\frac{n!}{(n-k)!k!} = \\frac{n(n-1)(n-2)\\ldots(n-k+1)}{k(k-1)(k-2)\\ldots 1}$

# --hints--

`binom` は関数である必要があります。

```js
assert(typeof binom === 'function');
```

`binom(5,3)` は 10 を返す必要があります。

```js
assert.equal(binom(5, 3), 10);
```

`binom(7,2)` は 21 を返す必要があります。

```js
assert.equal(binom(7, 2), 21);
```

`binom(10,4)` は 210 を返す必要があります。

```js
assert.equal(binom(10, 4), 210);
```

`binom(6,1)` は 6 を返す必要があります。

```js
assert.equal(binom(6, 1), 6);
```

`binom(12,8)` は 495 を返す必要があります。

```js
assert.equal(binom(12, 8), 495);
```

# --seed--

## --seed-contents--

```js
function binom(n, k) {

}
```

# --solutions--

```js
function binom(n, k) {
  let coeff = 1;
  for (let i = n - k + 1; i <= n; i++) coeff *= i;
  for (let i = 1; i <= k; i++) coeff /= i;
  return coeff;
}
```
