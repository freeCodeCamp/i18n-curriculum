...
1. Deberías tener un elemento `select` con siete elementos `option` anidados en él. La primera opción debería tener una cadena vacía como su texto y atributo `value`. El resto debería tener lo siguiente para su texto y valores (con el valor en minúsculas):
...
1. Si se selecciona una ciudad, deberían aparecer elementos para mostrar el clima:
...
1. Si la función `getWeather` tuvo un error, la aplicación solo debería mostrar una alerta que diga `Something went wrong, please try again later`.
...
1. Ni la función `getWeather` ni la función `showWeather` deben ser declaradas usando `const`, ya que las pruebas necesitan reasignarlas.

**Nota:** Ni la función `getWeather` ni la función `showWeather` deben ser declaradas usando `const`, ya que las pruebas necesitan reasignarlas. Además, las pruebas tardarán en completarse, así que mientras veas `// running tests` en la consola, se están ejecutando.
...
Ni la función `getWeather` ni la función `showWeather` deben ser declaradas usando `const`.
...
Cuando se selecciona París, la aplicación debería mostrar una alerta con `Something went wrong, please try again later`.
