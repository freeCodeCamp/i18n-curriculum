...
1. 该集合是循环的，因为每个数字的最后两位数字是下一个数字的前两位数字（包括与第一个数字相关的最后一个数字）。
2. 每种多边形类型：三角形（`$P_3(127) = 8128$`）、正方形（`$P_4(91) = 8281$`）和五边形（`$P_5(44) = 2882$`），在集合中由不同的数字表现。
3. 这是具有此属性的唯一一组4位数字。

查找所有有序集合中 `n` 个循环 4 位数的数字之和，其中每个 $P_3$ 到 $P_{n + 2}$ 多边形类型由集合中的不同数字表现。

# --hints--

`cyclicalFigurateNums(3)` 应该返回一个数字。

```js
assert(typeof cyclicalFigurateNums(3) === 'number');
```

`cyclicalFigurateNums(3)` 应该返回 `19291`。

```js
assert.strictEqual(cyclicalFigurateNums(3), 19291);
```

`cyclicalFigurateNums(4)` 应该返回 `28684`。

```js
assert.strictEqual(cyclicalFigurateNums(4), 28684);
```

`cyclicalFigurateNums(5)` 应该返回 `76255`。

```js
assert.strictEqual(cyclicalFigurateNums(5), 76255);
```

`cyclicalFigurateNums(6)` 应该返回 `28684`。
