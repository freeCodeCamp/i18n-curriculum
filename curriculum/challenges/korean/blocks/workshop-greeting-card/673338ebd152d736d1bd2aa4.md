...
호버할 때 `.card-links a` 요소를 대상으로 하는 가상 클래스 선택자를 만드세요.

`background-color`은 `orangered`로 바뀌어야 합니다.

# --hints--

`.card-links a:hover` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:hover"));
```

`.card-links a:hover` 선택자에 `background-color` 속성이 `orangered`로 설정되어야 합니다.
