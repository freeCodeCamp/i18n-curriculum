...
以前のプロジェクトでは、`fetch()` と共に `.then()` メソッドを使って、プロミスが解決された後のロジックを実行しました。 ここでは、`await` キーワードを使って、`fetch()` メソッドの非同期的な性質を扱います。

`await` キーワードは、プロミスが解決されて結果が返るのを待ちます。
...
では `try` ブロックの中に、`res` と言う名前の const 変数を作成して、`await fetch()` を代入してください。 `fetch` の呼び出しには `forumLatest` 変数を渡してください。

# --hints--

`try` ブロック内に `res` という定数が必要です。

```js
assert.match(code, /\s*const\s+res/)
```

変数 `res` に `await fetch()` を割り当ててください。

```js
assert.match(code, /\s*const\s+res\s*=\s*await\s+fetch\(\s*(.*)\s*\)/);
```

定数 `forumLatest` を `fetch` 呼び出しに渡してください。
