...
卡片头部已经完成。现在你需要添加玩家照片部分。

在 `div` 的闭合 `card-header` 之后，且仍在带有 `div` 的 `className` 的 `card` 内，添加一个带有 `div` 的 `className` `card-image-wrap`。暂时保持为空。你将在下一步添加图像。

# --hints--

你应该添加一个带有 `div` 的 `className` `card-image-wrap`。
