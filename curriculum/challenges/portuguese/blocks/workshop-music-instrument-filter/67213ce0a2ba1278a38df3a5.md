...
Remova seu `console.log` do event listener e defina o HTML interno de `productsContainer` para o resultado da chamada de `instrumentCards` com a opção de categoria selecionada.

Então, selecione diferentes opções do seu dropdown e veja o resultado na janela de visualização.
...
Quando a opção do menu dropdown for alterada você deve definir o HTML interno de `productsContainer` para o resultado da função `instrumentCards` chamada com a opção selecionada como argumento.
...
Você deve remover `console.log(instrumentCards(selectContainer.value))` do callback do seu event listener.
