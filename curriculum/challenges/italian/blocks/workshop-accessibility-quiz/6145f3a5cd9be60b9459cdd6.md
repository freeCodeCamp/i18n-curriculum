...
Collega il primo elemento `label` all'elemento `select` e assegna all'elemento `select` un attributo `name`.

# --hints--

Dovresti assegnare all'elemento `label` un attributo `for`.

```js
assert.notEmpty(document.querySelector('.question-block > label')?.htmlFor);
```

Dovresti assegnare all'elemento `select` un attributo `id`.

```js
assert.notEmpty(document.querySelector('.answer > select')?.id);
```

Dovresti assegnare all'elemento `select` un attributo `id` corrispondente all'attributo `for` dell'elemento `label`.

```js
assert.equal(document.querySelector('.answer > select')?.id, document.querySelector('.question-block > label')?.htmlFor);
```

Dovresti assegnare all'elemento `select` un attributo `name`.
