...
كما فعلت مع `multipleCard` السابقة، احصل على عنصر `multipleCard` مرة أخرى وقم بتعيين `innerHTML` الخاص به بالتكرار عبر `DrawingType`. استخدم `Object.values()` واستدعِ طريقة `map` عليه، مع تمرير `type` كمعلمة.

# --hints--

يجب تعيين `multipleCard.innerHTML` باستدعاء `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

يجب تمرير `DrawingType` إلى طريقة `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

يجب استدعاء طريقة `map` على `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

يجب أن تحتوي طريقة `map` على معلمة باسم `type`.
