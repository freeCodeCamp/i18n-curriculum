...
حان الوقت للبدء في العمل على الأزرار الأخرى. تم إضافة عناصر `button` المتبقية لك في ملف HTML.

لكل زر جديد (`#confused-btn`، `#sad-btn` و `#loving-btn`)، حدده باستخدام `querySelector` مع معرفه، ثم أضف مستمع حدث نقر يستدعي `updateCount` مع ذلك الزر.

استخدم أسماء المتغيرات `confusedBtn`، `sadBtn`، `lovingBtn`.

# --hints--

يجب أن تنشئ متغيرًا باسم `confusedBtn`.

```js
assert.exists(confusedBtn);
```

يجب أن تخزن عنصر `#confused-btn` في المتغير `confusedBtn`.

```js
assert.equal(confusedBtn, document.querySelector('#confused-btn'));
```

يجب أن تنشئ متغيرًا باسم `sadBtn`.

```js
assert.exists(sadBtn);
```

يجب أن تخزن عنصر `#sad-btn` في المتغير `sadBtn`.

```js
assert.equal(sadBtn, document.querySelector('#sad-btn'));
```

يجب أن تنشئ متغيرًا باسم `lovingBtn`.

```js
assert.exists(lovingBtn);
```

يجب أن تخزن عنصر `#loving-btn` في المتغير `lovingBtn`.

```js
assert.equal(lovingBtn, document.querySelector('#loving-btn'));
```

يجب أن يحتوي عنصر `#confused-btn` على مستمع حدث نقر يستدعي `updateCount`.
...
يجب أن يحتوي عنصر `#sad-btn` على مستمع حدث نقر يستدعي `updateCount`.
...
يجب أن يحتوي عنصر `#loving-btn` على مستمع حدث نقر يستدعي `updateCount`.
