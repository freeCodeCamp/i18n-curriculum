...
Devi ottenere il `id` della canzone attualmente in riproduzione. Per questo, puoi usare `userData.currentSong?.id`.

Usa `getElementById()` per selezionare la canzone attualmente in riproduzione: passa un letterale template per aggiungere il prefisso `id` al `song-`. Assegna il risultato a una variabile chiamata `songToHighlight`.

# --hints--

Non dovresti modificare la funzione `highlightCurrentSong` esistente né il suo contenuto.
...
Dovresti usare `document.getElementById()` e passare `` `song-${userData.currentSong?.id}` ``.

```js
assert.match(__helpers.removeJSComments(code), /document\.getElementById\(\s*`song-\$\{userData\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Dovresti assegnare il tuo `getElementById()` a una variabile `songToHighlight`.
