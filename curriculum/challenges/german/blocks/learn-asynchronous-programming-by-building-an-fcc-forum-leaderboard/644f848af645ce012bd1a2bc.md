...
Erstellen Sie eine Konstante namens `linkText` und weisen Sie ihr den Wert von `selectedCategory.category` zu. Dadurch wird der Name der Kategorie im Ankerelement angezeigt.

# --hints--

Sie sollten eine Konstante namens `linkText` haben.

```js
assert(code.match(/const\s+linkText\s*=/));
```

Sie sollten `selectedCategory.category` Ihrer Variablen `linkText` zuweisen.
