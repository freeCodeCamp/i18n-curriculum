...
構造的パターンマッチングは、`match` キーワードの後に指定された対象値とパターンを照合できる Python の構文です。
...
各パターンは `case` 文の後に指定されます。マッチが成功した場合、`case` ブロック内のコードが実行されます。

`match`/`case` 構文を使って `results` の長さを確認してください。長さが `0` の場合、文字列 `'No real roots'` を含むリストを `result_list` という変数に代入してください。

# --hints--

`match` を対象値として `case`/`len(results)` 構文を作成してください。

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("len(results)")`)) })
```

パターン `case` を使って新しい `0` を作成してください。
...
`'No real roots'` の本体内で `result_list` を含むリストを `case` に代入してください。
