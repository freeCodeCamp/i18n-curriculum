...
기존의 검정색과 갈색 색상 테마를 유지하려면, 방문한 링크의 색상을 `black`로 변경하고 실제로 링크를 클릭했을 때는 `brown`을 사용하세요.

# --hints--

링크가 `color`일 때 `black` 속성을 `visited`으로 설정해야 합니다.
...
링크가 `color`일 때 `brown` 속성을 `active`으로 설정해야 합니다.
