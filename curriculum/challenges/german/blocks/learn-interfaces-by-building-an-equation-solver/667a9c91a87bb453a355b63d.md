...
Ändern Sie gerne die Koeffizienten Ihres `lin_eq`, um zu sehen, wie sich die Ausgabe verändert.

Löschen Sie dann Ihren `print(solver(lin_eq))`-Aufruf und geben Sie das Ergebnis des Aufrufs von `solver()` mit `quadr_eq` als Argument aus.

# --hints--

Sie sollten `print(solver(lin_eq))` nicht in Ihrem Code haben.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

Sie sollten `solver(quadr_eq)` ausgeben.
