...
現在是時候將你所學到的關於箭頭函式的知識應用到你的音樂播放器專案中。 

首先移除 `printGreeting`、`printMessage` 和 `addTwoNumbers` 函式。也移除所有主控台述語和函式呼叫。 

# --hints--

你不應該有 `printGreeting` 函式（程式）。

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

你不應該有 `printMessage` 函式（程式）。

```js
assert.notMatch(code, /const printMessage = org => {/);
```

你不應該有 `addTwoNumbers` 函式（程式）。

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

你不應該有 `printGreeting();` 函式（程式）呼叫。

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

你不應該有 `printMessage('freeCodeCamp');` 函式呼叫。

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

你不應該有 `console.log(addTwoNumbers(3,4));` 主控台述語。
