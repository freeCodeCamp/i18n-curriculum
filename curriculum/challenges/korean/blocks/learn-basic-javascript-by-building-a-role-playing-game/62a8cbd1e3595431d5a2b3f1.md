...
플레이어가 어떤 무기를 샀는지 알려야 합니다. 방금 작성한 두 줄 사이에 `let`를 사용해 `newWeapon`라는 새 변수를 초기화하세요. 이 변수는 `weapons`와 같도록 설정하세요.

# --hints--

`buyWeapon` 함수에는 `newWeapon`라는 변수가 있어야 합니다.

```js
assert.match(buyWeapon.toString(), /newWeapon/);
```

`let`를 사용해 `newWeapon`를 선언해야 합니다.

```js
assert.match(code, /let\s+newWeapon/);
```

`newWeapon`는 `weapons` 값을 갖도록 초기화해야 합니다. 세미콜론을 잊지 마세요.

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

`newWeapon`는 `text`를 수정하기 전에 선언되어야 합니다.
