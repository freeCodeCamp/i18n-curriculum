...
В межах елемента `button` створіть ще два елементи `span`.

Перший елемент `span` повинен мати `class` типу `playlist-song-artist`. Між тегами `span` додайте `${song.artist}`.

Другий елемент `span` повинен мати `class` типу `playlist-song-duration`. Між тегами `span` додайте `${song.duration}`.

# --hints--

Не змінюйте наявний елемент `button` та його вміст.
...
Створіть елемент `span` з класом `playlist-song-artist`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

Елемент `span` із класом `playlist-song-artist` має інтерполювати `song.artist` як свій вміст.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

Створіть елемент `span` з класом `playlist-song-duration`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

Елемент `span` із класом `playlist-song-duration` має інтерполювати `song.duration` як свій вміст.
