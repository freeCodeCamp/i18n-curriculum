...
排版是排列文字以有效传达信息的艺术与技术。 它涉及文本格式设置和字体选择。 在本课中，你将学习如何运用排版的最佳实践，以提升可读性、层次结构和无障碍性。
...
你可以利用字体大小，为标题、副标题、段落和其他元素创建视觉层次。
...
例如，网页上的主标题应使用更大的字体，然后是较小字体的副标题。 这将为层次结构中的每个元素赋予特定的字体大小，帮助用户通过视觉层次浏览内容。

你应该最多使用两到三种字体来创造视觉一致性。 使用过多字体会使文本更难阅读，并削弱你的品牌识别度。 这还可能因增加网站加载时间而影响用户体验。
...
一旦选定了这两种或三种字体，就要在不同页面和元素中保持一致。 这些字体应与你的网页应用、产品或服务的整体设计保持一致。
...
你还应提供调整字体大小的选项，并尽可能避免使用复杂字体。 作为一般规则，应尽可能使用简单字体。 如果你的内容易于阅读，用户就更可能与之互动。
...
运用排版的最后一条建议是，确保你的字体在不同屏幕尺寸和设备上正确显示。
