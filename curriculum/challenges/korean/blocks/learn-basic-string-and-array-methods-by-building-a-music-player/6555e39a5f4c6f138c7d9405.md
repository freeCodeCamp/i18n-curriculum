...
이전 단계에서 `sort()` 메서드를 사용해 노래를 알파벳 순으로 정렬하는 방법을 배웠습니다. 콜백 함수의 또 다른 활용 사례는 배열을 무작위로 섞는 것입니다.

배열의 항목을 무작위로 섞는 한 가지 방법은 `0.5`에서 `Math.random()`을 빼는 것으로, 이 결과는 양수 또는 음수인 무작위 값을 만듭니다. 이렇게 하면 비교 결과가 양수와 음수의 혼합이 되어 요소들이 무작위로 정렬됩니다.
...
`sort()` 배열에 `userData?.songs` 메서드를 사용하세요. 메서드에 콜백을 전달하고 `Math.random() - 0.5`의 결과를 반환하세요.


# --hints--

기존 `shuffle` 함수는 수정하지 마세요.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

`sort()`에 `userData?.songs` 메서드를 사용하세요.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

`sort()` 메서드에는 화살표 구문을 사용하는 콜백 함수가 있어야 합니다.

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

`sort()` 메서드의 콜백은 `Math.random() - 0.5`을 반환해야 합니다.
