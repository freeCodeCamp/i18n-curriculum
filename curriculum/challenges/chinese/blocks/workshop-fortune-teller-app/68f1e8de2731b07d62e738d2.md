...
声明另一个名为 `foundCard` 的变量，并将其赋值为在 `find()` 上调用 `cards` 方法的结果。传入 `card` 作为回调参数，并使用严格相等性比较 `card.name_short` 和 `cardId` 来查找匹配的卡片。

# --hints--

你应该在 `foundCard` 内声明一个名为 `showFortune` 的变量。
...
你的 `foundCard` 应该通过在 `find` 上调用 `this.cards` 并传入正确的回调来赋值。
