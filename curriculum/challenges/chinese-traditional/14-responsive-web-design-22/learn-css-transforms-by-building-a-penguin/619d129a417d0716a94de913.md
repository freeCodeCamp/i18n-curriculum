...
應該使用 `.beak.bottom` 選擇器。

```js
assert.match(code, /\.beak\.bottom\s*\{/);
```

應該將 `.beak.bottom` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.width, '16%');
```

應該將 `.beak.bottom` 的 `top` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.top, '65%');
```

應該將 `.beak.bottom` 的 `left` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
