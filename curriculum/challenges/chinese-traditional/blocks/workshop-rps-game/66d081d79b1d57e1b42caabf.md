...
在工作坊的最後部分，你將專注於建置贏家訊息和重設按鈕功能。

首先建立一個名為 `winnerMsgElement` 的變數，用來儲存 id 為 `winner-msg` 的元素。 

然後，創建一個名為 `optionsContainer` 的變數，用來儲存具有 `options-container` 類別的元素。 

最後，創建一個名為 `resetGameBtn` 的變數，用來儲存 id 為 `reset-game-btn` 的元素。 

# --hints--

你應該有一個 `winnerMsgElement` 變數。

```js
assert.isNotNull(winnerMsgElement);
```

你的 `winnerMsgElement` 變數應該儲存 id 為 `winner-msg` 的元素。

```js
assert.equal(winnerMsgElement, document.getElementById("winner-msg"));
```

你應該有一個 `optionsContainer` 變數。

```js
assert.isNotNull(optionsContainer);
```

你的 `optionsContainer` 變數應該儲存具有 `options-container` 類別的元素。

```js
assert.equal(optionsContainer, document.querySelector(".options-container"));
```

你應該有一個 `resetGameBtn` 變數。

```js
assert.isNotNull(resetGameBtn);
```

你的 `resetGameBtn` 變數應該儲存 id 為 `reset-game-btn` 的元素。
