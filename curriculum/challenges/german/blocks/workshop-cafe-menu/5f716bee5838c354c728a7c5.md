...
Fügen Sie unter dem Dessert, das Sie gerade hinzugefügt haben, die restlichen Desserts und Preise mit drei weiteren `article`-Elementen hinzu, von denen jedes zwei verschachtelte `p`-Elemente enthält. Jedes Element sollte den korrekten Dessert- und Preistext haben, und alle sollten die richtigen Klassen besitzen.
...
Sie sollten vier `article`-Elemente in Ihrem zweiten `section` haben.
...
Sie sollten vier `.dessert`-Elemente haben.

```js
assert.lengthOf(document.querySelectorAll('.dessert'), 4);
```

Sie sollten vier neue `.price`-Elemente haben.
...
Ihr `section`-Element sollte acht `p`-Elemente enthalten.
...
Ihre `.dessert`-Elemente sollten die Texte `Donut`, `Cherry Pie`, `Cheesecake` und `Cinnamon Roll` enthalten.
...
Ihre neuen `.price`-Elemente sollten die Texte `1.50`, `2.75`, `3.00` und `2.50` enthalten.
...
Zwischen Ihren `p`-Elementen sollten keine Leerzeichen sein.
