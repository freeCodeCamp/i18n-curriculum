...
Aggiungi un blocco `else` all’istruzione `if`. All’interno del blocco `else`, chiama la funzione `getCurrentSongIndex()` e assegnala a una costante chiamata `currentSongIndex`.
...
Dovresti aggiungere un `else` all’istruzione `if` esistente.
...
Dovresti assegnare la costante `currentSongIndex` a `getCurrentSongIndex()` all’interno del blocco `else`.
