...
Como sugerido pelo título, você está criando um formulário. Então, após o elemento `p` , insira um elemento `form` com um atributo `action` direcionando para `https://register-demo.freecodecamp.org`.
...
Seu elemento `form` deve ter uma tag de fechamento `</form>`.
