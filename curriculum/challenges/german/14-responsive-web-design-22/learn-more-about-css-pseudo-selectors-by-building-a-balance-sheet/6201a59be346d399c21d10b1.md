...
Damit ist deine Bilanz fertig!

# --hints--

Du solltest einen `tr.data td:last-of-type`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td:last-of-type'));
```

Dein `tr.data td:last-of-type`-Selektor sollte eine `padding-right`-Eigenschaft von `0.5rem` enthalten.
