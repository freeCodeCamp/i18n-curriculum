...
Sasa unaweza kuunda kitendakazi kwa ajili ya kuiga sherehe. Unda kitendakazi tupu kinachoitwa `simulateFestival` chenye vigezo `gates` na `timeBlock`.

# --hints--

Unapaswa kuunda kitendakazi kinachoitwa `simulateFestival`.

```js
assert.isFunction(simulateFestival);
```

Kitendakazi chako cha `simulateFestival` kinapaswa kuwa na vigezo `gates` na `timeBlock`.
