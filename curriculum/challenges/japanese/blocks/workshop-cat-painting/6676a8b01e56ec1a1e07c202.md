...
白い四角形に太いボーダーがついているのがわかります。ボーダーは斜めのエッジになっていて、三角形を作るのに使えます。これを使って猫の耳を作ります。
...
`.cat-right-ear` セレクターには `height` プロパティを `0` に設定してください。セミコロンを忘れないでください。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.height, '0px')
```

`.cat-right-ear` セレクターには `width` プロパティを `0` に設定してください。セミコロンを忘れないでください。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.width, '0px')
```

`.cat-right-ear` セレクターには `border-left` プロパティを `35px solid transparent` に設定してください。セミコロンを忘れないでください。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderLeft, '35px solid transparent')
```

`.cat-right-ear` セレクターには `border-right` プロパティを `35px solid transparent` に設定してください。セミコロンを忘れないでください。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderRight, '35px solid transparent')
```

`.cat-right-ear` セレクターには `border-top` プロパティを `35px solid transparent` に設定してください。セミコロンを忘れないでください。
