...
Далі потрібно перевірити, чи є плейлист порожнім. Якщо так, слід скинути об’єкт `userData` до його початкового стану.

Використайте інструкцію `if`, щоб перевірити, чи має `userData?.songs` довжину `0`.

# --hints--

Вам слід створити порожню інструкцію `if` з умовою `userData?.songs.length === 0`.
