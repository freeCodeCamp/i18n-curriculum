...
既然我們正在使用 Bootstrap，我們可以將標題元素置中，使其看起來更好。你只需要為你的 `h2` 元素添加類別 `text-center`。

請記住，你可以透過空格分隔多個類別，將它們添加到同一個元素，如下所示：
