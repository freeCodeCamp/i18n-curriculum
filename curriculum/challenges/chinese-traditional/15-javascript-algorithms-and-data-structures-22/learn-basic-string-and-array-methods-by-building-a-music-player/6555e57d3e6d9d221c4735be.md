...
當按下隨機播放按鈕時，你需要將 `currentSong` 設置爲無，並將 `songCurrentTime` 設置爲 `0`。

將 `userData.currentSong` 設置爲 `null`，將 `userData.songCurrentTime` 設置爲 `0`。

*注意*：你不應該在此步驟中使用可選鏈接，因爲你明確將 `currentSong` 和 `songCurrentTime` 屬性分別設置爲 `null` 和 `0`。

# --hints--

你應該將 `userData.currentSong` 設置爲 `null`。

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

你應該將 `userData.songCurrentTime` 設置爲 `0`。
