...
コールバック内で、2つの関数呼び出しを行う必要があります。まず `createLabel()` を呼び出し、`number` を引数として渡してください。スプレッドシートにいくつかの数字が表示されるはずです。

次に、`.forEach()` 配列の `letters` メソッドを呼び出してください。`letter` パラメータを受け取る空のコールバック関数を渡してください。

# --hints--

`createLabel()` 関数を呼び出す必要があります。

```js
assert.match(code, /range\(\s*1\s*,\s*99\s*\)\.forEach\(\s*(\(\s*number\s*\)|number)\s*=>\s*\{\s*createLabel\(/);
```

`number` 呼び出しに `createLabel()` を渡す必要があります。

```js
assert.match(code, /range\(\s*1\s*,\s*99\s*\)\.forEach\(\s*(\(\s*number\s*\)|number)\s*=>\s*\{\s*createLabel\(/)
```

`.forEach()` 配列の `letters` メソッドを呼び出す必要があります。

```js
assert.lengthOf(code.match(/letters\.forEach\(/g), 2)
```

`.forEach()` メソッドにアロー構文のコールバック関数を渡す必要があります。

```js
assert.match(code, /letters\.forEach\(\s*(\([^)]*\)|[^\s()]+)\s*=>\s*\{/)
```

コールバック関数は `letter` パラメータを持つ必要があります。

```js
assert.match(code, /letters\.forEach\(\s*(\(\s*letter\s*\)|letter)\s*=>\s*\{/)
```

コールバック関数は空である必要があります。

```js
assert.match(code, /letters\.forEach\(\s*(\(\s*letter\s*\)|letter)\s*=>\s*\{\s*\}/)
```

`letters.forEach()` コールバック関数は `range(1, 99).forEach(number => {}` コールバック関数の中にネストされている必要があります。
