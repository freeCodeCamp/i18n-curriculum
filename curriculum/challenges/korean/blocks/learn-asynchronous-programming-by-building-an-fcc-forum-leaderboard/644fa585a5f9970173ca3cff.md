...
`linkClass`라는 상수를 만들고 템플릿 리터럴을 할당하세요. 그 템플릿 리터럴 안에 `category ${selectedCategory.className}`의 값을 추가하세요.

이 클래스 이름들은 앵커 요소에 스타일을 적용하는 데 사용됩니다.

# --hints--

`linkClass`라는 상수가 있어야 합니다.

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

`linkClass` 변수에 템플릿 리터럴을 할당해야 합니다.

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

템플릿 리터럴 안에 `category ${selectedCategory.className}`의 값이 있어야 합니다.
