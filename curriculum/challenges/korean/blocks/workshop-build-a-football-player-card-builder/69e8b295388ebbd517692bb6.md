...
`card-rating` div 아래에 `div`의 `className`를 가진 `card-position`을 추가하고, 텍스트는 `ST`로 하세요. 이는 스트라이커를 나타냅니다.

# --hints--

`div` div 아래에 `className`의 `card-position`를 가진 `.card-rating`를 추가해야 합니다.

```js
assert.exists(document.querySelector('.card-rating + .card-position'));
```

`.card-position` 요소는 텍스트 `ST`을 가져야 합니다.
