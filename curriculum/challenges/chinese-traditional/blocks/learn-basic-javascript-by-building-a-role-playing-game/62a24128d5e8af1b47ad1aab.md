...
使用選擇器列表（`selector1, selector2`）為你的 `#controls` 和 `#stats` 元素同時設定 `1px solid #0a0a23` 的 `border`、`#0a0a23` 的文字顏色，以及 `5px` 的 `padding`。

# --hints--

你應該有一個 `#controls, #stats` 選擇器列表。
...
你的 `#controls, #stats` 選擇器應該有 `5px` 的內距。
