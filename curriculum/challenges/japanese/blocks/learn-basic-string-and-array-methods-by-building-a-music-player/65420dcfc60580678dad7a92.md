...
`map()` 内に `return` ステートメントと一組のバッククォートを追加してください。バッククォートと文字列補間を使って、曲の詳細を表示するのに必要なすべての要素を挿入していきます。

バッククォートの中に `li` 要素を作成して、`id` 属性を `song-${song.id}` に、`class` 属性を `playlist-song` に設定してください。

# --hints--

`return` ステートメントと空のバッククォートのペアを追加してください。

```js
assert.match(code, /return\s*`/)
```

バッククォートの中に `li` タグを作成してください。

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

`li` の開始タグで、`id` 属性を `song-${song.id}` に設定してください。

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

`li` の開始タグで、`class` 属性を `playlist-song` に設定してください。
