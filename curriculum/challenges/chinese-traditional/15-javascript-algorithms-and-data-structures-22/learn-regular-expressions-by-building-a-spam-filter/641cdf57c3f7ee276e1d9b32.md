...
在 `if` 語句之後，使用三元運算符檢查以 `messageInput.value` 作爲參數調用 `isSpam()` 的真假性。 If true, set the `textContent` property on the `result` element to `"Oh no! This looks like a spam message."`. Otherwise, set it to `"This message does not seem to contain any spam."`
