...
`Sort` 버튼은 클릭했을 때 제대로 작동하는 것처럼 보일 수 있지만, 이는 배열의 모든 값이 한 자리 숫자이기 때문이며, 더 복잡한 값에서는 정렬이 예상대로 작동하지 않을 수 있습니다.

`value`와 `option`인 `selected` 요소의 텍스트를 `1`에서 `10`로 변경하고 `Sort` 버튼을 다시 클릭하세요.

# --hints--

`value`인 `option` 요소의 `selected`를 `10`으로 업데이트해야 합니다.
...
`option`인 `selected` 요소의 텍스트를 `10`으로 업데이트해야 합니다.
