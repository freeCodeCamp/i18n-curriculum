...
안타깝게도, 그 `children` 속성은 배열과 비슷하지만 배열은 아닌 요소들의 컬렉션을 반환합니다. 두 번째 인수를 `Array.from()`로 감싸서 배열로 변환하세요.

# --hints--

`document.getElementById('container').children`를 `Array.from()`로 감싸야 합니다.
