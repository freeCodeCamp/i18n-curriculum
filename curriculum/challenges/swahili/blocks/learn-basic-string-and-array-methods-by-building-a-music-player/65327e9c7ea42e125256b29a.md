...
Tumeongeza nyimbo zilizobaki kwenye safu ya data ya `allSongs` kwa ajili yako.

Ifuatayo, utajifunza kuhusu <dfn>Web Audio API</dfn> na jinsi ya kuitumia kucheza nyimbo. Vivinjari vyote vya kisasa vinaunga mkono Web Audio API, ambayo inakuwezesha kuzalisha na kusindika sauti katika programu za mtandao.

Tumia `const` kuunda kigezo kinachoitwa `audio` na uweke thamani yake kuwa `new Audio()`. Hii itaunda kipengele kipya cha HTML5 cha `audio`.

# --hints--

Unapaswa kutumia neno kuu la `new` kuunda mfano wa kitu cha `Audio`.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

Unapaswa kuweka kitu cha `Audio` katika thabiti iitwayo `audio`.
