...
Ora che il controllo `if` è in posizione, usa `e.target.value` per cercare il bug corrispondente in `bugMap` e chiamane il metodo `render`. Dopo di che, dovresti poter selezionare i bug e vedere subito il risultato.

Con questo, il tuo workshop Bug Emoji Picker è completo!

# --hints--

Dovresti chiamare il metodo `render` sul bug corrispondente da `bugMap`.
