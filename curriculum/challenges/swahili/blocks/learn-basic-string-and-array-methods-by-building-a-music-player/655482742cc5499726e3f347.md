...
Ndani ya kitendakazi cha `playPreviousSong`, ongeza tamko la `if` lenye sharti la `userData?.currentSong === null`. Hii itakagua kama hakuna wimbo unaochezwa kwa sasa. Ikiwa hakuna, toka kwenye kitendakazi kwa kutumia `return`.

Ndani ya blokki ya `else`, tengeneza thabiti iitwayo `currentSongIndex` na weka thamani `getCurrentSongIndex()`.

# --hints--

Haupaswi kubadilisha kitendakazi cha `playPreviousSong` kilichopo na yaliyomo ndani yake.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Unapaswa kuwa na tamko la `if` lenye sharti la `userData?.currentSong === null` ndani ya kitendakazi chako cha `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

Unapaswa kuwa na `return` ndani ya blokki ya tamko lako la `if`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

Tamko lako la `if` linapaswa kuwa na blokki ya `else`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

Unapaswa kuita `getCurrentSongIndex` na kuwekea thamani `currentSongIndex` ndani ya blokki ya `else` ya tamko lako la `if`.
