...
Verschachtel ein `legend`-Element sowie ein `ul`-Element mit zwei Optionen innerhalb jeden `fieldset`.

# --hints--

Du solltest ein `legend`-Element innerhalb des ersten `fieldset`-Elements verschachteln.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(1) > fieldset > legend')?.length, 1);
```

Du solltest ein `ul`-Element innerhalb des ersten `fieldset`-Elements verschachteln.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(1) > fieldset > ul')?.length, 1);
```

Du solltest zwei `li`-Elemente innerhalb des ersten `ul`-Elements verschachteln.

```js
assert.equal(document.querySelectorAll('fieldset > ul')?.[0]?.querySelectorAll('li')?.length, 2);
```

Du solltest ein `legend`-Element innerhalb des zweiten `fieldset`-Elements verschachteln.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(2) > fieldset > legend')?.length, 1);
```

Du solltest ein `ul`-Element innerhalb des zweiten `fieldset`-Elements verschachteln.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(2) > fieldset > ul')?.length, 1);
```

Du solltest zwei `li`-Elemente innerhalb des zweiten `ul`-Elements verschachteln.
