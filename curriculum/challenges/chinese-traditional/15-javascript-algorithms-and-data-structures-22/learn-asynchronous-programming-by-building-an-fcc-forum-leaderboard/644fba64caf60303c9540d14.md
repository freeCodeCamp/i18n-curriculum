...
在 `href` 屬性後，將 `class` 屬性設置爲名爲 `linkClass` 的常量。

在 `class` 屬性後，將 `target` 屬性設置爲 `"_blank"`。

最後，將 `linkText` 常量放在錨標籤之間，以顯示鏈接中的文本。

# --hints--

你應該在 `href` 屬性後有一個 `class` 屬性。
...
你應該將 `class` 屬性的值設置爲名爲 `linkClass` 的常量。
