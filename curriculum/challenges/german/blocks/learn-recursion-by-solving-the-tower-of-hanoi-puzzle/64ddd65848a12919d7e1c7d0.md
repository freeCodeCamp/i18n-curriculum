...
Es ist Zeit, etwas Code von der `move()`-Funktion in die `make_allowed_move()`-Funktion zu verschieben. Verschieben Sie den Code, der innerhalb der ersten `if`-Anweisung verschachtelt ist (außer dem ersten `print()`-Aufruf), in Ihre neue Funktion. Achten Sie genau auf die Einrückung.  
Vergessen Sie nicht, das Schlüsselwort `pass` zu entfernen.

# --hints--

Sie sollten den Code, der innerhalb der ersten `if`-Anweisung verschachtelt ist (außer dem ersten `print()`-Aufruf), in Ihre neue Funktion verschieben.
