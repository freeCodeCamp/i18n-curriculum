...
Usando esa misma sintaxis, consulta tus entradas de `number` en el elemento `#lunch` y asígnalas a `lunchNumberInputs`.

# --hints--

Debes declarar una variable `lunchNumberInputs`.

```js
assert.match(calculateCalories.toString(), /lunchNumberInputs\s*=/);
```

Debes llamar `document.querySelectorAll()` con `#lunch input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#lunch input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Debes asignar el resultado de tu llamada a `document.querySelectorAll()` a `lunchNumberInputs`.
