...
Verschachteln Sie sechs `div`-Elemente innerhalb von `.foreground-buildings` und geben Sie ihnen nacheinander die Klassen `fb1` bis `fb6`. „fb“ steht für „foreground building“. Diese werden sechs weitere Gebäude für den Vordergrund sein.

# --hints--

Sie sollten ein neues `div` mit der Klasse `fb1` erstellen.

```js
assert.exists(document.querySelector('div.fb1'));
```

Sie sollten ein neues `div` mit der Klasse `fb2` erstellen.

```js
assert.exists(document.querySelector('div.fb2'));
```

Sie sollten ein neues `div` mit der Klasse `fb3` erstellen.

```js
assert.exists(document.querySelector('div.fb3'));
```

Sie sollten ein neues `div` mit der Klasse `fb4` erstellen.

```js
assert.exists(document.querySelector('div.fb4'));
```

Sie sollten ein neues `div` mit der Klasse `fb5` erstellen.

```js
assert.exists(document.querySelector('div.fb5'));
```

Sie sollten ein neues `div` mit der Klasse `fb6` erstellen.

```js
assert.exists(document.querySelector('div.fb6'));
```

Sie sollten diese neuen `div`-Elemente innerhalb des `.foreground-buildings`-Elements platzieren.
