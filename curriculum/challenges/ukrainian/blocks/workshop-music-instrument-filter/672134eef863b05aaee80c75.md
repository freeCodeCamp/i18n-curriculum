...
Щоб реалізувати функціонал фільтрації, вам потрібна функція. Оголосіть порожню функцію з іменем `instrumentCards`, яка приймає один параметр.

# --hints--

У вас має бути функція з іменем `instrumentCards`.

```js
assert.isFunction(instrumentCards);
```

Ваша функція `instrumentCards` повинна приймати один параметр.
