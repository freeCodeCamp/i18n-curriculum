...
你应该告诉玩家他们购买了什么武器。 在你刚刚写的两行之间，使用 `let` 初始化一个名为 `newWeapon` 的新变量。 将其设置为等于 `weapons`。

# --hints--

你的 `buyWeapon` 函数应该有一个名为 `newWeapon` 的变量。

```js
assert.match(buyWeapon.toString(), /newWeapon/);
```

你应该使用 `let` 来声明 `newWeapon`。

```js
assert.match(code, /let\s+newWeapon/);
```

`newWeapon` 应初始化为具有 `weapons` 的值。 不要忘记分号。

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

在修改 `text` 之前，应先声明 `newWeapon`。
