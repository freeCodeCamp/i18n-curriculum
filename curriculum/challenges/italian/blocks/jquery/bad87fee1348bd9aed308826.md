...
title: Consultare il genitore di un elemento usando jQuery
challengeType: 6
forumTopicId: 18321
...
Ogni elemento HTML ha un elemento `parent` da cui `inherits` proprietà.

Per esempio, l'elemento `h3` nel tuo `jQuery Playground` ha come genitore l'elemento `<div class="container-fluid">`, che a sua volta ha come genitore l'elemento `body`.

jQuery ha una funzione chiamata `parent()` che ti permette di consultare il genitore di qualunque elemento tu abbia selezionato.

Ecco un esempio di come useresti la funzione `parent()` se volessi dare al genitore dell'elemento `left-well` un colore di sfondo blu:
