...
Під вашим елементом `.heading` створіть новий елемент `section` з множиною `class`, встановленою на `text`. Всередині нього створіть елемент `p` з множиною `class`, встановленою на `first-paragraph`, і наступним текстом:
...
Ваш новий елемент `section` має йти після вашого елемента `.heading`.

```js
assert(document.querySelectorAll('section')?.[1]?.previousElementSibling?.className === 'heading');
```

Ваш новий елемент `section` має мати множину `class`, встановлену на `text`.

```js
assert(document.querySelectorAll('section')?.[1]?.className === 'text');
```

Ви повинні створити новий елемент `p` всередині вашого елемента `.text`.

```js
assert(document.querySelector('.text')?.querySelectorAll('p')?.length === 1);
```

Ваш новий елемент `p` має мати множину `class`, встановлену на `first-paragraph`.

```js
assert(document.querySelector('.text p')?.className === 'first-paragraph');
```

Ваш новий елемент `p` має містити наданий текст.
