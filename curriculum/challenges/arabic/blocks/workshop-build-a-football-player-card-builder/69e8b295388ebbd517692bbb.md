...
بعد عنصر `card-image-wrap`، أضف عنصر `div` مع `className` باسم `card-name-strip`. بداخله، أضف عنصر `span` مع `className` باسم `card-name` والنص `PELE`.

# --hints--

يجب أن تضيف عنصر `div` مع `className` باسم `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

يجب أن تضيف عنصر `span` مع `className` باسم `card-name` داخل `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

يجب أن يحتوي عنصر `.card-name` على النص `PELE`.
