...
لجعل نص الاقتباس يبرز أكثر، أعطِ المحدِّد `.quote` خاصية `font-family` مضبوطة على `Raleway` مع النظام البديل `sans-serif`.

# --hints--

يجب أن يحتوي المحدِّد `.quote` على خاصية `font-family` مضبوطة على `Raleway` مع النظام البديل `sans-serif`.
