...
其余的歌曲已为你添加到 `allSongs` 数组中。

在之前的课程中，你学习了 <dfn>Web Audio API</dfn> 以及如何使用它来播放歌曲。所有现代浏览器都支持 Web Audio API，它让你能够在网页应用中生成和处理音频。

创建一个名为 `audio` 的变量，并将其设置为 `new Audio()`。这将创建一个新的 HTML5 `audio` 元素。
...
你应该将一个 `Audio` 对象的新建实例赋值给名为 `audio` 的变量。
