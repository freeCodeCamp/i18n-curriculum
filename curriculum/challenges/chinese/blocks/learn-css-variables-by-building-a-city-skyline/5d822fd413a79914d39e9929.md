...
只剩下三座建筑了。 在 `.fb4` 元素中嵌套两个新的 `div` 元素，并按顺序为它们添加 `fb4a` 和 `fb4b` 类。 请记住，你之前颠倒了 `.fb4` 和 `.fb5` 的位置，所以它是你现在正在处理的最右边的紫色建筑。

# --hints--

你应该在 `.fb4` 中添加两个 `div` 元素。

```js
assert.equal(document.querySelectorAll("div.fb4 > div")?.length, 2);
```

你应该给第一个新的 `div` 一个 `fb4a` 类。

```js
assert.exists(document.querySelector("div.fb4a"));
```

你应该给第二个新的 `div` 一个 `fb4b` 类。
