...
Badilisha msikilizaji wa tukio la `scaryStoryBtn` ili ufanye wito wa kitendakazi cha `displayStory` kwa hoja `"scary"`. Pia, ongeza msikilizaji wa tukio la `click` kwa vifungo vya `funnyStoryBtn` na `adventureStoryBtn` ambao hufanya wito wa kitendakazi cha `displayStory` kwa hoja za `"funny"` na `"adventure"` mtawalia.

Kwa hili, umefanikiwa kumaliza warsha hii.

# --hints--

Unapobofya `scaryStoryBtn`, aya ya `#result` inapaswa kuonyesha hadithi ya kutisha `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` ikiwa na rangi ya mpaka ya `#ee4b2b`.
...
Unapobofya `funnyStoryBtn`, aya ya `#result` inapaswa kuonyesha hadithi ya kuchekesha ikiwa na rangi ya mpaka ya `#f1be32`.
...
Unapobofya `adventureStoryBtn`, aya ya `#result` inapaswa kuonyesha hadithi ya msisimko ikiwa na rangi ya mpaka ya `#acd157`.
