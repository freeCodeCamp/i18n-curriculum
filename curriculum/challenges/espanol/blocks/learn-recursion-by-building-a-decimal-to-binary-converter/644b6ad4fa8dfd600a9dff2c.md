...
Para dividir números en JavaScript, usa el operador de división (`/`). Por ejemplo:
...
Debes crear una variable llamada `quotient` dentro de tu bucle `while`.

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*(var|let|const)\s+quotient\s*=/);
```

Debes asignar a `quotient` el valor de `input` dividido por `2`.
