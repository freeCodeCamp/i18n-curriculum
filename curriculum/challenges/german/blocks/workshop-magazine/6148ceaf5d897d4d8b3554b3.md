...
Magazine verwenden oft im Druck Blocksatz, um ihr Layout zu strukturieren und den Fluss ihres Inhalts zu steuern. Während dies im Druck gut funktioniert, kann Blocksatz auf Websites ein Barrierefreiheitsproblem darstellen, zum Beispiel für Menschen mit Legasthenie.

Damit Ihr Projekt wie ein gedrucktes Magazin aussieht, geben Sie dem `.text`-Selektor eine `text-align`-Eigenschaft mit dem Wert `justify`.

# --hints--

Ihr `.text`-Selektor sollte eine `text-align`-Eigenschaft mit dem Wert `justify` haben.
