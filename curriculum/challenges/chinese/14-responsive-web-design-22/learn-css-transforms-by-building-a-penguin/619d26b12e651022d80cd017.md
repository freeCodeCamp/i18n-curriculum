...
`.arm` 的 `width` 应该为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

`.arm` 的 `height` 应该为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

`.arm` 的 `background` 应该为 `linear-gradient(90deg, gray, rgb(209, 210, 199))`。
