...
これを修正するため、`left` という `class` を 1 つ目の `.face` 要素に、`right` という `class` を 2 つ目の `.face` 要素に追加してください。

# --hints--

`left` という `class` を 1 つ目の `.face` 要素に設定する必要があります。

```js
assert.include(document.querySelector('.face:nth-of-type(1)').className, 'left');
```

`right` という `class` を 2 つ目の `.face` 要素に設定する必要があります。
