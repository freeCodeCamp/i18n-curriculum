...
Debajo de tu elemento `.double-indent`, agrega un nuevo elemento `p` con el texto `Includes 10g Added Sugars 20%`. Tu nuevo elemento `p` también debe estar doblemente indentado y no tener borde inferior. Usa un `span` para hacer el `20%` en negrita y alineado a la derecha.
...
Tu nuevo elemento `p` debe tener el texto `Includes 10g Added Sugars 20%`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Includes 10g Added Sugars[\s|\n]+20%/));
```

Tu nuevo elemento `p` debe tener el atributo `class` establecido como `double-indent no-divider`.
