...
最後の `.divider` の後に、テキスト `Cholesterol 0mg 0%` を持つ新しい `p` 要素を作成してください。次に、テキスト `Cholesterol` を `span` 要素で、テキスト `0%` を別の要素で囲み、それぞれに `bold` クラスを付けてください。

最後に、テキスト `Cholesterol` を含む `span` 要素とテキスト `0mg` を、整列のためにさらに別の `span` 要素にネストしてください。
...
新しい `p` 要素にはテキスト `Cholesterol 0mg 0%` を入れてください。
...
テキスト `Cholesterol` を含む `span` 要素の `class` 属性は `bold` に設定してください。
...
テキスト `0%` を含む `span` 要素の `class` 属性は `bold` に設定してください。
...
テキスト `Cholesterol` とテキスト `0mg` を含む `span` 要素は、別の `span` 要素で一緒に囲んでください。
