...
Настав час реалізувати функціонал видалення для плейлиста. Це дозволить керувати видаленням пісні з плейлиста, обробляти інші пов’язані дії при видаленні пісні та створити кнопку `Reset Playlist`.

Використайте `const` та стрілковий синтаксис, щоб створити порожню функцію `deleteSong` і передати `id` як параметр.

# --hints--

Ви повинні використати `const` для створення порожньої функції з ім’ям `deleteSong`.
...
Ваша функція `deleteSong` має використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Ваша функція `deleteSong` має приймати параметр `id`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

Ваша функція `deleteSong` має бути порожньою.
