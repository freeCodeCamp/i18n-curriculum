...
これから、メールシミュレーターを実演するメイン関数を作成します。

`main` 関数を作成し、その中で `User` クラスを使って、2人のユーザー `Tory` と `Ramy` を作成し、それぞれ `tory` と `ramy` という変数に代入してください。

# --hints--

`main` という名前の関数を定義してください。
...
`Tory` という名前のユーザーを作成し、`tory` という変数に代入してください。
...
`Ramy` という名前のユーザーを作成し、`ramy` という変数に代入してください。
