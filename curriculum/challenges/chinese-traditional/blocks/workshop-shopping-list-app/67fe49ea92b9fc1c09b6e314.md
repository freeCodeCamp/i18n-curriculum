...
接著，使用 `includes()` 方法檢查 `item` 的小寫版本是否包含 `query` 的小寫版本作為子字串。

例如，這裡示範如何使用 `toLowerCase()` 和 `includes()` 方法來檢查字串 `freeCodeCamp` 是否包含 `code`：
...
將 `includes()` 方法串鏈到小寫的 `item`，並檢查它是否包含小寫的 `query`。記得也要將 `query` 轉成小寫。

# --hints--

你應該將 `includes()` 方法串鏈到小寫的 `item`。
...
你應該檢查小寫的 `item` 字串是否包含小寫的 `query` 作為子字串。
