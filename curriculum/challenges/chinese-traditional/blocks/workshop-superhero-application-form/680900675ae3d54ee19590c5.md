...
是時候創建標籤和輸入欄位了。在表單中，創建一個帶有文字 `Hero Name` 的 `label`，並在其中放入一個型別為 `text` 的 `input` 元素。

將 `Hero Name` 的 `input` 連接到 `heroName` 狀態變數，方法是為它添加一個 `value` 屬性並設為 `heroName`。
...
你的 `label` 元素應該有 `Hero Name` 作為其文字。
...
你應該在你的 `label` 元素內創建一個型別為 `text` 的 `input` 元素。
...
你的 `input` 元素應該將其值設為 `heroName` 狀態變數。
