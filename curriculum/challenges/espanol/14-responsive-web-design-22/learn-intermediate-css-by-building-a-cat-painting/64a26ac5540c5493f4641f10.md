...
Tu selector `.cat-mouth div` debe tener una propiedad `border-color` establecida en `black transparent transparent transparent` transparente.

```js
assert.match(code, /border-color:\s*black\s*(transparent)\s*\1\s*\1\s*;?/)
```

Tu selector `.cat-mouth div` debe tener una propiedad `border-radius` establecida a `190%/190px 150px 0 0`.
