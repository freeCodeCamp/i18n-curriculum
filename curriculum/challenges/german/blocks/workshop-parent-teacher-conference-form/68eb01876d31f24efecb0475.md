...
Da die HTML-Struktur nun fertig ist, sollten Sie mit dem CSS-Styling weitermachen.

Öffnen Sie Ihre `styles.css`-Datei. Legen Sie zuerst eine dunkle Hintergrundfarbe für das `body` und eine helle Farbe für den Text fest.

Wählen Sie das `body`-Element aus und setzen Sie seine `background-color` auf `midnightblue` und `color` auf `whitesmoke`.

# --hints--

Sie sollten das `body`-Element ansprechen.

```js
assert(new __helpers.CSSHelp(document).getStyle('body'));
```

Setzen Sie die `background-color`-Eigenschaft auf `midnightblue`.
...
Setzen Sie die `color`-Eigenschaft auf `whitesmoke`.
