...
`.toFixed()` 返回字符串的問題是你希望能夠使用稅率進行計算。 爲了解決這個問題，你可以將 `.toFixed()` 調用（包括計算）傳遞給 `parseFloat()` 函數。 這會將固定字符串轉換回數字，保留現有的小數位。

將你的 `.toFixed()` 調用傳遞給 `parseFloat()`。

# --hints--

你應該將整個計算（不包括 `return` 語句）傳遞給 `parseFloat()`。
