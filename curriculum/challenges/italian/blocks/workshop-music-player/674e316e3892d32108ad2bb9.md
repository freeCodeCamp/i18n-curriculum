...
Ogni volta che clicchi il pulsante play, verrà sempre riprodotta la prima canzone della tua playlist. Per risolvere questo problema, all’interno della funzione freccia del listener dell’evento, sostituisci `playSong(0)` con un’istruzione `if` che verifica se `userData.currentSong` è `null`.

All’interno del blocco `if`, chiama la funzione `playSong` con l’`id` della prima canzone nell’array `userData.songs`.

# --hints--

Dovresti modificare la callback del listener dell’evento per chiamare la funzione `playSong` con l’`id` della prima canzone nell’array `userData.songs` quando `userData.currentSong` è `null`.
