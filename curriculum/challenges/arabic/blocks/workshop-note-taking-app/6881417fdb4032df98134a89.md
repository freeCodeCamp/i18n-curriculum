...
الخطوات المتبقية في ورشة العمل ستتضمن إضافة الشفرة لعرض رسالة حفظ الملاحظات بشكل ديناميكي وتسجيل الملاحظات الحالية في وحدة التحكم.

ابدأ بإنشاء متغير يسمى `noteEl` وتعيين نتيجة استعلام الوثيقة عن العنصر الذي يحمل `id` بقيمة `note`.

ثم أنشئ متغيرًا آخر يسمى `statusEl` وقم بتعيين نتيجة استعلام الوثيقة عن العنصر الذي يحمل `id` بقيمة `status`.

# --hints--

يجب أن يكون لديك متغير `noteEl` وتعيين نتيجة استعلام الوثيقة عن العنصر الذي يحمل `id` بقيمة `note`.

```js
assert.equal(noteEl, document.getElementById("note"));
``` 

يجب أن يكون لديك متغير `statusEl` وتعيين نتيجة استعلام الوثيقة عن العنصر الذي يحمل `id` بقيمة `status`.
