...
Crea un altro elemento `p` sotto il tuo elemento `.first-paragraph` e assegnagli il seguente testo:
...
Dovresti creare un secondo elemento `p` all'interno del tuo elemento `.text`.

```js
assert(document.querySelectorAll('.text p')?.length === 2)
```

Il tuo secondo elemento `p` dovrebbe contenere il testo fornito.
