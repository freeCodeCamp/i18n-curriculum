...
1. Você deve ter um elemento `main` como o único filho do elemento `body`.
...
1. Você deve ter três elementos `section` abaixo do seu primeiro parágrafo.
1. Cada seção deve conter um elemento `h2`, um parágrafo e um elemento `iframe`, nesta ordem.
...
Cada elemento `section` deve começar com um elemento `h2` que serve como o título dessa seção.
