...
`infixToFunction[operator]` retorna uma função.Chama essa função diretamente, passando `arg1` e `arg2` como argumentos.

# --hints--

A função de callback deve retornar o resultado da chamada de `infixToFunction[operator]`.
...
Você deve passar `arg1` como o primeiro argumento para a chamada de `infixToFunction[operator]`.
...
Você deve passar `arg2` como o segundo argumento para a chamada de `infixToFunction[operator]`.
