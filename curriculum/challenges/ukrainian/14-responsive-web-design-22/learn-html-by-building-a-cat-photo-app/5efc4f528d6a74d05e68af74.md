...
You should add a new `label` element after the checkbox element. Переконайтеся, що він має початковий та кінцевий теґи.
...
Your `label` element should have a `for` attribute with the value `loving`.  Очікувався `--fcc-expected--`, але знайдено `--fcc-actual--`.
