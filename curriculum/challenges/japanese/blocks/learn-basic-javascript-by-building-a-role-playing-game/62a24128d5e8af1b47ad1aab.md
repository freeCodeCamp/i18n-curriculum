...
セレクターリスト (`selector1, selector2`) を使って、`#controls` と `#stats` の両方の要素に `border` の `1px solid #0a0a23`、`#0a0a23` のテキストカラー、そして `5px` の `padding` を指定してください。

# --hints--

`#controls, #stats` のセレクターリストがあるはずです。
...
`#controls, #stats` のセレクターには `border` の `1px solid #0a0a23` を指定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('#controls, #stats')?.border, '1px solid rgb(10, 10, 35)');
```

`#controls, #stats` のセレクターには `color` の `#0a0a23` を指定してください。
...
`#controls, #stats` のセレクターにはパディングの `5px` を指定してください。
