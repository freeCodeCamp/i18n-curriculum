...
Ndani ya njia ya `initializeEventListeners`, unganisha mshughulikiaji wa tukio la `click` kwa `singleCardBtn`. Ndani ya mshughulikiaji wa tukio, fanya wito wa njia ya `singleCardSelected`. Kumbuka kupata `singleCardBtn` kutoka kwa `elements`.

Utaunda njia hii katika hatua zinazofuata.

# --hints--

Unapaswa kuunganisha tukio la `click` kwenye kipengele cha `singleCardBtn`. Usisahau unahitaji neno kuu la `this` hapa.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Ndani ya wito wa kurudisha wa tukio, unapaswa kufanya wito wa njia ya `singleCardSelected`.
