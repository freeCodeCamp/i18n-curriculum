...
<dfn>CSS Variables</dfn> 是一種強大的方式，只需更改一個值即可同時更改許多 CSS 樣式屬性。

請遵循以下指示，了解只改變三個數值如何改變許多元素的樣式。

# --instructions--

在 `penguin` 類別中，將 `black` 值改為 `gray`，將 `gray` 值改為 `white`，並將 `yellow` 值改為 `orange`。

# --hints--

`penguin` 類別應該宣告 `--penguin-skin` 變數並將其指定為 `gray`。

```js
assert.match(__helpers.removeHtmlComments(code), /.penguin\s*?{[\s\S]*--penguin-skin\s*?:\s*?gray\s*?;[\s\S]*}/gi);
```

`penguin` 類別應該宣告 `--penguin-belly` 變數並將其指定為 `white`。

```js
assert.match(code, /.penguin\s*?{[\s\S]*--penguin-belly\s*?:\s*?white\s*?;[\s\S]*}/gi);
```

`penguin` 類別應該宣告 `--penguin-beak` 變數並將它指定為 `orange`。
