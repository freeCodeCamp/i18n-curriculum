...
Dentro de la función `playPreviousSong`, agrega una declaración `if` con una condición de `userData?.currentSong === null`.  Esto verificará si actualmente no hay ninguna canción sonando. Si no hay ninguna, sal de la función usando un `return`.
...
Tu declaración `if` debería tener un bloque `else`.
