...
Your `ul` element should have an opening tag. Le etichette di apertura hanno questa sintassi: `<nomeElemento>`.

```js
assert(document.querySelector('ul'));
```

Your `ul` element should have a closing tag. I tag di chiusura hanno un carattere`/` subito dopo il carattere `<`.
