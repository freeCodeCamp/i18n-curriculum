...
Agora você pode criar uma função para simular o festival. Crie uma função vazia chamada `simulateFestival` com os parâmetros `gates` e `timeBlock`.

# --hints--

Você deve criar uma função chamada `simulateFestival`.

```js
assert.isFunction(simulateFestival);
```

Sua função `simulateFestival` deve ter os parâmetros `gates` e `timeBlock`.
