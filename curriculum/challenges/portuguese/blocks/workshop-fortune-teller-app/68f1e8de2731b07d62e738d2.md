...
Declare outra variável chamada `foundCard` e atribua o resultado da chamada do método `find()` em `cards`. Passe `card` como parâmetro callback e compare `card.name_short` com `cardId` para encontrar a carta correspondente usando igualdade estrita.

# --hints--

Você deve declarar uma variável chamada `foundCard` dentro de `showFortune`.
...
Sua `foundCard` deve ser atribuída chamando `find` em `this.cards` com o callback correto.
...
O callback `find` deve ter um parâmetro chamado `card`.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

Dentro do callback, `card.name_short` deve ser comparado com `cardId`.
