...
选择 `.shirt` 元素，并将其 `font-size` 设置为 `25px`、`font-family` 设置为 `Helvetica`， 备用值为 `sans-serif`，并将 `font-weight` 设置为 `bold`。
...
你应该将 `.shirt` 的 `font-size` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontSize, '25px');
```

你应该将 `.shirt` 的 `font-family` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontFamily, 'Helvetica, sans-serif');
```

你应该将 `.shirt` 的 `font-weight` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
