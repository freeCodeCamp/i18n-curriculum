...
داخل عنصر `ul` الخاص بك، أنشئ ستة عناصر `li`. أضف عنصر `h4` مع تعيين `class` إلى `list-subtitle` وعنصر `p` لكل من عناصر `li` الخاصة بك.

ثم أعطِ عناصر `h4` و `p` محتوى النص التالي، بالترتيب، بحيث يستخدم كل `h4` ما على الجانب الأيسر من النقطتين، ويستخدم كل `p` ما على الجانب الأيمن:
...
يجب أن يحتوي عنصر `ul` الخاص بك على ستة عناصر `li`.

```js
assert(document.querySelectorAll('.lists li')?.length === 6);
```

يجب أن يحتوي كل عنصر جديد من عناصر `li` على عنصر `h4` وعنصر `p`.
...
يجب أن يحتوي أول عنصر `h4` على النص `V1 - 2014`.

```js
assert(document.querySelectorAll('.lists li h4')?.[0]?.innerText === 'V1 - 2014');
```

يجب أن يحتوي أول عنصر `p` على النص `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
يجب أن يحتوي ثاني عنصر `h4` على النص `V2 - 2015`.

```js
assert(document.querySelectorAll('.lists li h4')?.[1]?.innerText === 'V2 - 2015');
```

يجب أن يحتوي ثاني عنصر `p` على النص `We added interactive algorithm challenges.`

```js
assert(document.querySelectorAll('.lists li p')?.[1]?.innerText === 'We added interactive algorithm challenges.');
```

يجب أن يحتوي ثالث عنصر `h4` على النص `V3 - 2015`.

```js
assert(document.querySelectorAll('.lists li h4')?.[2]?.innerText === 'V3 - 2015');
```

يجب أن يحتوي ثالث عنصر `p` على النص `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`
...
يجب أن يحتوي رابع عنصر `h4` على النص `V4 - 2016`.

```js
assert(document.querySelectorAll('.lists li h4')?.[3]?.innerText === 'V4 - 2016');
```

يجب أن يحتوي رابع عنصر `p` على النص `We expanded the curriculum to 3 certifications, including Front-End, Back-End, and Data Visualization. They each had 10 required projects, but only the Front-End section had its own challenges. For the other certs, we were still using external resources like Node School.`
...
يجب أن يحتوي خامس عنصر `h4` على النص `V5 - 2017`.

```js
assert(document.querySelectorAll('.lists li h4')?.[4]?.innerText === 'V5 - 2017');
```

يجب أن يحتوي خامس عنصر `p` على النص `We added the back-end and data visualization challenges.`

```js
assert(document.querySelectorAll('.lists li p')?.[4]?.innerText === 'We added the back-end and data visualization challenges.');
```

يجب أن يحتوي سادس عنصر `h4` على النص `V6 - 2018`.

```js
assert(document.querySelectorAll('.lists li h4')?.[5]?.innerText === 'V6 - 2018');
```

يجب أن يحتوي سادس عنصر `p` على النص `We launched 6 new certifications to replace our old ones. This was the biggest curriculum improvement to date.`
...
يجب أن يحمل كل من عناصر `h4` الستة الصنف `list-subtitle`.
