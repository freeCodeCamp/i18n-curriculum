...
Vuoi confrontare due vettori solo quando appartengono alla stessa classe. Per questo crea un'istruzione `if` che verifichi se `self` e `other` non appartengono alla stessa classe e in tal caso restituisca `NotImplemented`.
# --hints--
...
Dovresti restituire `NotImplemented` dal corpo `if`.
