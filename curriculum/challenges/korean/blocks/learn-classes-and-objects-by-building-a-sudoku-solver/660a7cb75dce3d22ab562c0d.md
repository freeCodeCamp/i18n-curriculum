...
루프 본문 안에서 `pass`을 현재 셀에 유효한 선택인지 확인하는 `if` 문으로 바꾸세요.

`if` 호출을 사용해 `is_valid` 조건을 만들고, `next_empty`와 `guess`를 인수로 전달하세요.

# --hints--

`pass`을 삭제하고 `if`이 `self.is_valid(next_empty, guess)`인지 확인하는 `True` 문을 만드세요.
