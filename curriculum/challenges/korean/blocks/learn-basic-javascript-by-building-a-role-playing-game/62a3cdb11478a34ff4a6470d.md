...
이제 `fightDragon` 함수를 만들어 `"Fighting dragon."`을 콘솔에 출력하세요.

# --hints--

`function`를 선언하려면 `fightDragon` 키워드를 사용하세요.

```js
assert.match(code, /function\s+fightDragon/);
```

`fightDragon`는 함수여야 합니다.

```js
assert.isFunction(fightDragon);
```

코드에 `console.log("Fighting dragon.");` 줄이 있어야 합니다.

```js
assert.match(code, /console\.log\(\s*('|")Fighting dragon\.\1\s*\)/);
```

`console.log("Fighting dragon.");` 줄은 `fightDragon` 함수 안에 있어야 합니다.
