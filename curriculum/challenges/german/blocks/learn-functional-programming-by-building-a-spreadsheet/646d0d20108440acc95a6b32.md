...
In Ihrer `elemValue`-Funktion haben Sie explizit eine Funktion namens `inner` deklariert und zurückgegeben. Da Sie jedoch die Pfeilfunktion-Syntax verwenden, können Sie eine Funktion implizit zurückgeben. Zum Beispiel:

```js
const curry = soup => veggies => {};
```

`curry` ist eine Funktion, die einen `soup`-Parameter entgegennimmt und eine Funktion zurückgibt, die einen `veggies`-Parameter entgegennimmt. Verwenden Sie diese Syntax, um Ihre `addCharacters`-Funktion so zu aktualisieren, dass sie eine leere Funktion zurückgibt, die einen `character2`-Parameter entgegennimmt.

# --hints--

Ihre `addCharacters`-Funktion sollte eine implizite Rückgabe verwenden.
...
Ihre `addCharacters`-Funktion sollte eine Pfeilfunktion zurückgeben, die einen `character2`-Parameter hat.
...
Ihre innere Pfeilfunktion sollte leer sein.
