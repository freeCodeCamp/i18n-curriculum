...
你已將段落元素的 `id` 屬性設為 `obj.inputVal` 屬性。

現在，使用 `.getElementById()` 方法來選取具有該屬性值的元素，同樣使用 `obj.inputVal` 屬性。你應該將 `obj.inputVal` 值直接傳遞給你的 `getElementById` 呼叫。

接著，將所選元素的 `textContent` 屬性設定為目前物件的 `msg` 屬性，以便在你先前指定的延遲後更新其文字。

# --hints--

你應該使用 `.getElementById()` 方法來標的 `id` 屬性與目前物件的 `inputVal` 值相符的元素。
...
你應該將 `.textContent` 串鏈到 `.getElementById()` 方法。
...
你應該將標的元素的 `textContent` 屬性設為當前物件的 `msg` 屬性相等。
