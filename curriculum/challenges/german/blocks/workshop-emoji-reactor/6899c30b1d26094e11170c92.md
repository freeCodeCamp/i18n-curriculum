...
Jetzt müssen Sie einen Ereignislistener am `happyBtn`-Element hinzufügen.

Schreiben Sie den Ereignislistener und sein Callback so, dass beim Klicken auf die Schaltfläche `Button clicked!` in der Konsole ausgegeben wird.

# --hints--

Wenn auf das `happyBtn` geklickt wird, soll `Button clicked!` in der Konsole ausgegeben werden.
