...
Dopo l'elemento `h2`, crea due elementi `p`. Give the first a `class` of `dessert-price`, and set the text to a dollar sign `"$"` followed by the value of the `price` variable. Give the second a `class` of `product-category`, and the text `"Category: "` followed by the value of the `category` variable.
...
Gli elementi `p` dovrebbero trovarsi dopo l'elemento `h2`.
...
Il primo elemento `p` dovrebbe avere una `class` con il valore `dessert-price`.
...
Il secondo elemento `p` dovrebbe avere una `class` con il valore `product-category`.
