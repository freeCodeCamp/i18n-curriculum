...
你为你的脚本添加了一个 `console.log()` 调用来验证你的 `EVAReadySquad` 数组，现在该调用不再需要。为了防止终端在后续步骤中变得杂乱，请继续删除当前将 `EVAReadySquad` 宇航员记录到控制台的 `for` 循环。

# --hints--

删除将 `EVAReadySquad` 宇航员记录到控制台的 `for` 循环。你应该删除整个循环块。
