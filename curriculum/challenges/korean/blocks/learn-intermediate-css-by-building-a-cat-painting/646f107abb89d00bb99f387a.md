...
자손 선택자를 사용해 `div` 요소 안에 있는 세 개의 `.cat-whiskers-left` 요소를 선택하세요. `width` 값을 가진 `40px`, `height` 값을 가진 `1px`, 그리고 `background-color` 값을 가진 `#000`를 지정하세요.

# --hints--

`.cat-whiskers-left div` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div'))
```

`.cat-whiskers-left div` 선택자에는 `width`로 설정된 `40px`이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.width === '40px')
```

`.cat-whiskers-left div` 선택자에는 `height`로 설정된 `1px`이 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-left div')?.height === '1px')
```

`.cat-whiskers-left div` 선택자에는 `background-color`로 설정된 `#000`이 있어야 합니다.
