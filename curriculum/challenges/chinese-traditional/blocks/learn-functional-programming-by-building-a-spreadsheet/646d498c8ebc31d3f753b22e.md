...
為你的物件添加一個 `firsttwo` 屬性，該屬性接受一個 `nums` 參數並傳回 `nums` 陣列中前兩個元素的順序。然後添加一個 `lasttwo` 屬性，該屬性傳回 `nums` 陣列中最後兩個元素的順序。

# --hints--

你的 `spreadsheetFunctions` 物件應該有一個 `firsttwo` 屬性。

```js
assert.property(spreadsheetFunctions, "firsttwo");
```

你的 `firsttwo` 屬性應該是一個函式（程式）。

```js
assert.isFunction(spreadsheetFunctions.firsttwo);
```

你的 `firsttwo` 函式應該傳回陣列的前兩個數字。

```js
assert.deepEqual(spreadsheetFunctions.firsttwo([1, 2, 3, 4, 5, 6]), [1, 2]);
```

你的 `spreadsheetFunctions` 物件應該有一個 `lasttwo` 屬性。

```js
assert.property(spreadsheetFunctions, "lasttwo");
```

你的 `lasttwo` 屬性應該是一個函式（程式）。

```js
assert.isFunction(spreadsheetFunctions.lasttwo);
```

你的 `lasttwo` 函式應該傳回陣列的最後兩個數字。
