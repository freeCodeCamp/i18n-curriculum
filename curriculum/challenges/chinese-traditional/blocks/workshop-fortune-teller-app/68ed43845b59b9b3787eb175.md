...
在類別內添加一個 `constructor`。在建構式中，你需要使用先前創建的元素，透過 `getElement` 函式選取對應的 DOM 元素。

目前要控制 `singleCardBtn` 和 `singleCard`。對於 `singleCardBtn`，傳入 `#btn-single-card`，而對於 `singleCard`，傳入 `.single_card`。

# --hints--

`Game` 類別應該有一個 `constructor`。
...
`singleCardBtn` 應該使用 `getElement` 搭配 `'#btn-single-card'` 來設值。
...
`singleCard` 應該使用 `getElement` 搭配 `'.single_card'` 來設值。
