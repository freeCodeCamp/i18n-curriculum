...
Per dare ai Camper un'idea di cosa inserire nella loro bio, si usa l'attributo `placeholder`. `placeholder` accetta un valore di testo, che viene mostrato finché l'utente non inizia a digitare.

Dai a `textarea` un `placeholder` di `I like coding on the beach...`.

# --hints--

Dovresti dare a `textarea` un attributo `placeholder`.

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

Dovresti dare a `placeholder` un valore `I like coding on the beach...`.
