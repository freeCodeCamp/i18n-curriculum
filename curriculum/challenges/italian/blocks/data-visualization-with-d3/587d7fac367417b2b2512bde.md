...
title: Usa una scala predefinita per posizionare gli elementi
challengeType: 6
forumTopicId: 301494
...
Con le scale impostate, è il momento di mappare di nuovo il grafico a dispersione. Le scale sono come funzioni di elaborazione che trasformano i dati grezzi `x` e `y` in valori che si adattano e si renderizzano correttamente sull’SVG. Mantengono i dati all’interno dell’area di tracciamento dello schermo.

Imposti i valori degli attributi di coordinate per una forma SVG con la funzione di scala. Questo include gli attributi `x` e `y` per gli elementi `rect` o `text`, oppure `cx` e `cy` per `circles`. Ecco un esempio:

```js
shape.attr('x', d => xScale(d[0]));
```

Le scale impostano gli attributi di coordinate della forma per posizionare i punti dati sull’SVG. Non devi applicare le scale quando mostri il valore reale dei dati, per esempio, nel metodo `text()` per un tooltip o un’etichetta.

# --instructions--

Usa `xScale` e `yScale` per posizionare sia le forme `circle` sia `text` sull’SVG. Per `circles`, applica le scale per impostare gli attributi `cx` e `cy`. Dai loro anche un raggio di `5` unità.

Per gli elementi `text`, applica le scale per impostare gli attributi `x` e `y`. Le etichette dovrebbero essere spostate a destra dei punti. Per farlo, aggiungi `10` unità al valore dei dati `x` prima di passarli a `xScale`.
...
Il secondo elemento `circle` dovrebbe avere un valore `cx` di circa `159` e un valore `cy` di circa `181` dopo aver applicato le scale. Dovrebbe anche avere un valore `r` di `5`.
...
Il terzo elemento `circle` dovrebbe avere un valore `cx` di circa `340` e un valore `cy` di circa `329` dopo aver applicato le scale. Dovrebbe anche avere un valore `r` di `5`.
...
Il quarto elemento `circle` dovrebbe avere un valore `cx` di circa `131` e un valore `cy` di circa `60` dopo aver applicato le scale. Dovrebbe anche avere un valore `r` di `5`.
...
Il sesto elemento `circle` dovrebbe avere un valore `cx` di circa `271` e un valore `cy` di circa `306` dopo aver applicato le scale. Dovrebbe anche avere un valore `r` di `5`.
...
Il settimo elemento `circle` dovrebbe avere un valore `cx` di circa `361` e un valore `cy` di circa `351` dopo aver applicato le scale. Dovrebbe anche avere un valore `r` di `5`.
...
L’ottavo elemento `circle` dovrebbe avere un valore `cx` di circa `261` e un valore `cy` di circa `132` dopo aver applicato le scale. Dovrebbe anche avere un valore `r` di `5`.
...
Il nono elemento `circle` dovrebbe avere un valore `cx` di circa `131` e un valore `cy` di circa `144` dopo aver applicato le scale. Dovrebbe anche avere un valore `r` di `5`.
