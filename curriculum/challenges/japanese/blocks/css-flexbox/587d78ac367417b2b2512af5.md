...
title: ツイート埋め込みで flex-direction プロパティを使ってカラムを作成する
challengeType: 0
...
ツイート埋め込みの `header` と `footer` は以前 `flex-direction` プロパティを row の値で使いました。同様に、`.profile-name` 要素の中のアイテムはカラムとして積み重ねるとよく機能します。

# --instructions--

ヘッダーの `flex-direction` 要素に CSS プロパティ `.profile-name` を追加し、値を `column` に設定してください。
...
`.profile-name` 要素には `flex-direction` プロパティを `column` に設定する必要があります。
