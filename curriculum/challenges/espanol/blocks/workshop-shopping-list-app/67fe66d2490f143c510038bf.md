...
Luego, en el elemento de tipo `input` para la casilla de verificación, agrega un atributo `checked` y asígnalo a `isChecked`. Aunque no es estrictamente necesario establecer el atributo `checked`, es una buena práctica asegurarse de que cada casilla de verificación refleje el estado actual del arreglo `selectedItems`.

# --hints--

Debes agregar un atributo `checked` al elemento de entrada de la casilla de verificación.
...
Debes establecer el atributo `checked` en `isChecked`.
