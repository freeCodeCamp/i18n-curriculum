...
Unter Verwendung der gleichen Interpolationssyntax, fügst du ein zweites `p`-Element mit dem Text `consumedCalories Calories Consumed` und ein drittes mit dem Text `exerciseCalories Calories Burned` hinzu. Denke daran, deine `consumedCalories`- und `exerciseCalories`-Variablen durch die entsprechenden Werte zu ersetzen.

# --hints--

Du solltest ein zweites `p`-Element in deinem Template-Literal erstellen.

```js
assert.isAtLeast(getTemplateContents(code)?.match(/<p>[^<]*<\/p>/g)?.length, 2);
```

Dein zweites `p`-Element sollte in einer neuen Zeile stehen.
...
Dein zweites `p`-Element sollte den Text `${consumedCalories} Calories Consumed` haben.
