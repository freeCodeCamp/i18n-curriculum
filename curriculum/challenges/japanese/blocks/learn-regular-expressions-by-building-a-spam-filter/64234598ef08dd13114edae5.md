...
次に文字 `s` と `t` も、それぞれ `5` と `7` にもマッチするように更新してください。

# --hints--

`stockRegex` では文字クラスを使用して、文字 `s` および数字 `5` にマッチさせてください。

```js
assert.match(stockRegex.source, /\[(s5|5s)\]/);
```

`stockRegex` では文字クラスを使用して、文字 `t` および数字 `7` にマッチさせてください。
...
`stockRegex` が `570ck al3r7` にマッチするようにしてください。
