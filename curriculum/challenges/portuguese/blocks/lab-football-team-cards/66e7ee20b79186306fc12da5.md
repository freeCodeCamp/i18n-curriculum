...
1. Você deve exibir os dados dos jogadores na página dentro do elemento `#player-cards`, cada jogador deve ser exibido em um `div` com a classe `player-card`, e aninhado nele, um `h2` contendo o nome do jogador e `(Captain)` no caso do jogador ser capitão, e um `p` contendo `Position:` e a posição do jogador. Aqui está um exemplo de como o HTML deve ser:
