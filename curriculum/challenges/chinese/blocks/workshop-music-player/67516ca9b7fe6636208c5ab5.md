...
最后，将 `false` 作为第二个参数传递给 `playSong(userData.currentSong.id)` 函数调用。然后，通过播放一首歌曲、暂停它，再按上一首按钮或下一首按钮，测试一切是否按预期工作。

# --hints--

你应该将 `false` 作为第二个参数传递给 `playSong(userData.currentSong.id)` 函数调用。
