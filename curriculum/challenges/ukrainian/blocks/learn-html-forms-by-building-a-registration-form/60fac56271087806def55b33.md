...
Вкладіть елемент `select` (разом із його елементами `option`) всередину елемента `label` з текстом `How did you hear about us?`. Текст має йти перед елементом `select`.

# --hints--

Ви повинні вкласти лише елемент `select` всередину елемента `label`.
...
У вас має бути 5 елементів `option` всередині елемента `select`.
