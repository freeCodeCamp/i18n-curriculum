...
title: "Usa la proprietà align-items nell'incorporamento di tweet"
challengeType: 0
...
L'ultima sfida ha introdotto la proprietà `align-items` e ha fornito un esempio. Questa proprietà può essere applicata a pochi elementi di incorporamento di tweet per allineare gli elementi flex al loro interno.

# --instructions--

Aggiungi la proprietà CSS `align-items` all'elemento `.follow-btn` dell'intestazione. Imposta il valore su `center`.
...
L'elemento `.follow-btn` dovrebbe avere la proprietà `align-items` impostata con un valore di `center`.
