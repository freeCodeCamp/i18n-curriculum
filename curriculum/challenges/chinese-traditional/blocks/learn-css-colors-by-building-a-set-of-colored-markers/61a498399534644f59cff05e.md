...
如你所見，`linear-gradient` 函式（程式）產生了平滑的紅綠漸層。雖然 `linear-gradient` 函式（程式）需要至少兩個顏色引數才能運作，但它可以接受多個顏色引數。

使用 `rgb` 函式（程式）將純藍色作為第三個顏色引數添加到 `linear-gradient` 函式（程式）。

# --hints--

你的 `.red` CSS 規則應該有一個 `background` 屬性，設定為 `linear-gradient(90deg, rgb(255, 0, 0), rgb(0, 255, 0), rgb(0, 0, 255))`。
