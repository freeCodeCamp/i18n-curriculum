...
Щоб почати працювати з даними, вам потрібно буде використати ще один метод `.then()`.

Приєднайте ще один `.then()` до існуючого методу `.then()`. Цього разу передайте `data` як параметр для функції зворотного виклику. Для функції зворотного виклику використовуйте фігурні дужки, бо у вас буде більше одного виразу. Усередині функції зворотного виклику виведіть у консоль `data`, щоб побачити, як це виглядає.
...
Ваш перший `.then()` не повинен мати крапку з комою.
...
Ваш другий метод `.then()` має виводити у консоль `data`.

```js
assert.match(code, /fetch\(\s*('|"|`)https:\/\/cdn\.freecodecamp\.org\/curriculum\/news\-author\-page\/authors\.json\1\s*\)\s*\.then\(\s*(\(\s*res\s*\)|res)\s*=>\s*res\.json\(\s*\)\s*\)\s*\.then\(\s*(\(\s*data\s*\)|data)\s*=>\s*\{\s*\n?\s*?console\.log\(\s*data\s*\)\s*;?\s*\}\s*\)/)
```

Після другого `.then()` не повинно бути крапки з комою.
