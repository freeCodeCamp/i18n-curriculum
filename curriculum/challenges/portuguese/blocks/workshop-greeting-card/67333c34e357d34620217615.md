...
Crie um seletor para os elementos `section` e dê a eles:

- uma propriedade `margin` definida como `20px auto`,
- uma `max-width` definida para `600px`.
- uma propriedade `background-color` definida como `whitesmoke`
- uma propriedade `padding` definida como `20px`
- uma propriedade `border-radius` definida como `10px`
...
O seletor `section` deve ter uma propriedade `margin` definida como `20px auto`.
...
O seletor `section` deve ter uma propriedade `max-width` definida como `600px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("max-width"), "600px");
```

O seletor `section` deve ter uma propriedade `background-color` definida como `whitesmoke`.
...
O seletor `section` deve ter uma propriedade `padding` definida como `20px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle("section").getPropertyValue("padding"), "20px");
```

O seletor `section` deve ter uma propriedade `border-radius` definida como `10px`.
