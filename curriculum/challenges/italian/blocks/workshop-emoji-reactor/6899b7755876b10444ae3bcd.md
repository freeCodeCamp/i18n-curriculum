...
Ora è il momento di collegare il file JavaScript.

Aggiungi un elemento `script` subito prima del tag di chiusura dell'elemento `body` con l'attributo e il valore corretti per collegare il file `script.js`.
...
L'elemento `script` dovrebbe avere un `src` di `./script.js`.
