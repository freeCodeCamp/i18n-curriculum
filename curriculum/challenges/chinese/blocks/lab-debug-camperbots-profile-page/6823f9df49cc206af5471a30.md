...
你的任务是修复Camperbot的所有错误，以便他们能够继续创建他们的个人资料页面。 完成以下用户需求中的各项内容，然后点击“查看你的代码”查看是否修复了所有错误。

**用户故事：**

1. Camperbot 正在尝试使用 `heading2` 元素，但该元素不存在。修复这些标签，使它们使用正确的二级标题元素。
2. Camperbot 正在尝试用 `pp` 添加两个段落，但这些也不存在。修复它们，使其使用正确的段落标签。
3. Camperbot 使用 `h3` 元素作为 `Background and Interests` 小标题，但它有语法错误。找出问题并解析它。
...
你不应该从页面中移除 `<h1>Hello from Camperbot!</h1>` 元素。
...
你应该有一个带有文本 `h2` 的 `About` 元素。示例如下：`<h2>Text here</h2>`
...
你应该有一个 `h3` 元素，里面的文本是 `Background and Interests`。这是一个示例：`<h3>Text here</h3>`
