...
メニューのコンテンツと両端の間にスペースを作るには、さまざまな`padding`プロパティを使えます。

`menu`クラスに、同じ値`padding-left`の`padding-right`と`20px`を与えてください。

# --hints--

`padding-left`プロパティを`20px`に設定してください。
...
`padding-right`プロパティを`20px`に設定してください。
...
`.menu`要素は`padding-left`の`20px`を持つべきです。
...
`.menu`要素は`padding-right`の`20px`を持つべきです。
