...
Innerhalb des vierten `td`-Elements aktualisieren Sie den aktuellen Wert, sodass stattdessen die `viewCount`-Funktion mit der `views`-Variablen als Argument aufgerufen wird.

# --hints--

Sie sollten die `viewCount`-Funktion aufrufen und die `views`-Variable als Argument übergeben.
