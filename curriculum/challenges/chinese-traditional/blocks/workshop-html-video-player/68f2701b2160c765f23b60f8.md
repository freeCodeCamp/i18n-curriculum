---
id: 68f2701b2160c765f23b60f8
title: 步驟 12
challengeType: 0
dashedName: step-12
---

# --description--

另一個常見的 MIME 型別是 `video/ogg` MIME 型別。

Ogg 是一種數位多媒體容器格式，旨在提供高效的數位多媒體串流和操作。它由 Xiph.Org Foundation 維護，且是免費且開放的，不受軟體專利限制。其名稱源自電腦遊戲 Netrek 中的術語「ogging」。

在你的第二個 `source` 元素下方，添加第三個 `source` 元素，並為它設定 `src` 屬性，值為 `https://cdn.freecodecamp.org/curriculum/labs/mapmethod.ogg`，以及 `type` 屬性，值為 `video/ogg`。

# --hints--

你應該有第三個 `source` 元素。

```js
assert.exists(document.querySelector('video > source:nth-of-type(3)'));
```

你的第三個 `source` 元素應該有一個 `src` 屬性。

```js
const source = document.querySelector('source:nth-of-type(3)');
assert.isTrue(source?.hasAttribute('src'));
```

你的第三個 `source` 元素應該有一個 `src` 屬性，其值為 `https://cdn.freecodecamp.org/curriculum/labs/mapmethod.ogg`。

```js
const source = document.querySelector('source:nth-of-type(3)');
assert.strictEqual(source?.getAttribute('src'), 'https://cdn.freecodecamp.org/curriculum/labs/mapmethod.ogg');
```

你的第三個 `source` 元素應該有一個 `type` 屬性。

```js
const source = document.querySelector('source:nth-of-type(3)');
assert.isTrue(source?.hasAttribute('type'));
```

你的第三個 `source` 元素應該有一個 `type` 屬性，其值為 `video/ogg`。

```js
const source = document.querySelector('source:nth-of-type(3)');
assert.strictEqual(source?.getAttribute('type'), 'video/ogg');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Working with the HTML Video Element</title>
</head>
<body>
  <h1>Working with the HTML Video Element</h1>
  <video
    width="640"
    loop
    controls
    muted
    poster="https://cdn.freecodecamp.org/curriculum/labs/past-event2.jpg"
  >
    <source
      src="https://cdn.freecodecamp.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4"
      type="video/mp4"
    >
    <source
      src="https://cdn.freecodecamp.org/curriculum/labs/mapmethod.webm"
      type="video/webm"
    >
--fcc-editable-region--
    
--fcc-editable-region--
  </video>
</body>
</html>
```
