...
在第一个 `div.answer` 元素中，嵌套一个必需的 `select` 元素和三个 `option` 元素。

给第一个 `option` 元素一个 `value` 为 `""`，以及文本 `Select an option`。 给第二个 `option` 元素一个 `value` 为 `yes`，以及文本 `Yes`。 给第三个 `option` 元素一个 `value` 为 `no`，以及文本 `No`。
