...
In einer vorherigen Vorlesung haben Sie gelernt, dass Typinferenz die Fähigkeit von TypeScript ist, automatisch Typen für Variablen, Funktionsrückgaben und Ausdrücke basierend auf ihrer Verwendung und ihrem Kontext zu bestimmen und zuzuweisen.

In TypeScript machen Generics Funktionen, Klassen und Typen flexibler. Generics erlauben die Erstellung von Typvariablen, wodurch es nicht mehr nötig ist, Typen jedes Mal explizit zu definieren. So kann dieselbe Funktion sicher mit verschiedenen Typen arbeiten.

Hier ist ein einfaches Beispiel für Generics:
...
Funktionen können zwei oder mehr Generics verwenden. `T` wird als Konvention verwendet, und bei Bedarf können zusätzliche Typvariablen wie `S` hinzugefügt werden.
...
Erstellen Sie eine Funktion `getElement`, die `selector` vom Typ `string` als Parameter entgegennimmt. Sie soll außerdem einen generischen Typ `T` verwenden, der von `HTMLElement` abgeleitet ist, und einen Rückgabetyp `T` haben.

# --hints--

Sie sollten eine Pfeilfunktion namens `getElement` haben.
...
Ihre Funktion `getElement` sollte einen Parameter namens `selector` vom Typ `string` haben.
...
Der generische Typparameter `T` sollte vom Typ `HTMLElement` abgeleitet sein.
...
Ihre Funktion `getElement` sollte einen Rückgabetyp von `T` haben.
