...
La tua funzione di callback dovrebbe contenere un'istruzione `if`.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

La tua istruzione `if` dovrebbe verificare se il `value` di `messageInput` è una stringa vuota.
...
La tua istruzione `if` dovrebbe mostrare un alert all'utente con il messaggio `"Please enter a message."`.
...
La tua istruzione `if` dovrebbe terminare l'esecuzione della funzione.
