...
现在添加一个 `div`，其 `className` 为 `form-group`。在其中，添加一个 `label`，其 `className` 为 `label`，并且有一个 `htmlFor` 属性为 `name`。在其下添加一个 `input`，其 `id` 设置为 `name`，有一个 `className` 为 `input`，以及一个 `type` 属性为 `text`。

# --hints--

你应该有一个 `div`，其 `className` 为 `form-group`。
...
你的 `form-group` 应该包含一个 `label`，其 `className` 为 `label`，并且有一个 `htmlFor` 属性为 `name`。
...
你的 `form-group` 应该包含一个 `input`，其有一个 `id` 为 `name`，`className` 为 `input`，以及一个 `type` 为 `text`。
