...
Füge unter deinem `blockquote`-Element ein weiteres `p`-Element mit dem folgenden Text hinzu:
...
Du solltest ein viertes `p`-Element zu deinem `.text`-Element hinzufügen.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 5);
```

Dein neues `p`-Element sollte nach deinem `blockquote`-Element stehen.
