...
Вам потрібно попрацювати над відтворенням наступної та попередньої пісні. Для цього вам знадобляться функції `playNextSong` та `playPreviousSong`.

Використайте `const` та стрілковий синтаксис, щоб створити порожню функцію `playNextSong`.

# --hints--

Використайте `const`, щоб створити порожню функцію під назвою `playNextSong`.
...
Функція `playNextSong` повинна використовувати стрілковий синтаксис.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Функція `playNextSong` має бути порожньою.
