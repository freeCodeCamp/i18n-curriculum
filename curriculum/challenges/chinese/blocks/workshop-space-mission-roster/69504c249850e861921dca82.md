...
你可能已经注意到 `EVAChunks` 本质上是一个内部包含数组的数组，或者说是一个二维（2D）数组。可以使用嵌套的 `for` 循环来记录二维数组中的数据：
...
在上面的示例中，`rootArray` 是主数组，`rootArray[i]` 是主数组内的子数组，`rootArray[i][j]` 是该子数组内的一个对象，`.property` 访问该对象中的一个值。

使用嵌套的 `for` 循环迭代 `EVAChunks` 并记录每个块中每位宇航员的 `name`。确保：

- 使用 `EVAChunks` 作为你的根数组
- 记录 `Chunk ${i+1}:` 之间的块

# --hints--

你应该使用嵌套的 `for` 循环来迭代 `EVAChunks` 并记录每个块中每位宇航员的 `name`。在块之间记录 `Chunk ${i+1}:`。
