...
Wenn die Shuffle-Taste gedrückt wird, sollen Sie `currentSong` auf nichts setzen und `songCurrentTime` auf `0`.

Setzen Sie `userData.currentSong` auf `null` und `userData.songCurrentTime` auf `0`.

*Hinweis*: Sie sollten für diesen Schritt keine optionale Verkettung verwenden, da Sie die Eigenschaften `currentSong` und `songCurrentTime` explizit auf `null` bzw. `0` setzen.
