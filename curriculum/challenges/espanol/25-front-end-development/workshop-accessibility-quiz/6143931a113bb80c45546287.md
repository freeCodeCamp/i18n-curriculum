...
Dentro del segundo elemento `section`, añade dos elementos `div` con una clase de `question-block`.

Luego, dentro de cada elemento `div.question-block`, añade un elemento `h3` con texto de números incrementales, comenzando en `1`, y un elemento `fieldset` con una clase de `question`.

# --hints--

Deberías anidar dos elementos `div` dentro del segundo elemento `section`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.length, 2);
```

Deberías darle al primer elemento nuevo `div` una clase de `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[0]?.className, 'question-block');
```

Deberías darle al segundo elemento nuevo `div` una clase de `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[1]?.className, 'question-block');
```

Deberías anidar un elemento `h3` dentro de cada elemento `div.question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')?.length, 2);
```

Deberías darle al primer elemento `h3` un texto de `1`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')?.[0]?.textContent, '1');
```

Deberías darle al segundo elemento `h3` un texto de `2`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > h3')?.[1]?.textContent, '2');
```

Deberías anidar un elemento `fieldset` dentro de cada elemento `div.question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > fieldset')?.length, 2);
```

Deberías colocar el primer elemento `fieldset` después del primer elemento `h3`.

```js
assert.exists(document.querySelector('section:nth-of-type(2) > div.question-block > h3 + fieldset'));
```

Deberías colocar el segundo elemento `fieldset` después del segundo elemento `h3`.

```js
assert.exists(document.querySelector('section:nth-of-type(2) > div.question-block:nth-of-type(2) > h3 + fieldset'));
```

Deberías darle al primer elemento `fieldset` una clase de `question`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > fieldset')?.[0]?.className, 'question');
```

Deberías darle al segundo elemento `fieldset` una clase de `question`.
