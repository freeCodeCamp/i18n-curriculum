...
現在、3つの事実がすべて同時に表示されています。しかし、表示されるのは1つの事実だけにする必要があります。

これを実現するには、2番目と3番目の`hidden`要素に`tabpanel`属性を設定してください。

# --hints--

2番目の`tabpanel`要素には`hidden`属性を設定してください。

```js
assert.isTrue(document.querySelectorAll('div[role="tabpanel"]')[1].hasAttribute("hidden"));
```

3番目の`tabpanel`要素には`hidden`属性を設定してください。
