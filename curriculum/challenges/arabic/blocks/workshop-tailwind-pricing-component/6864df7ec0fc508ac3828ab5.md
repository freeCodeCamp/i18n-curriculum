...
من CSS العادي، تعلمت أن تعيين عرض `grid` لعنصر لا يفعل شيئًا حتى تحدد صفوف وأعمدة الشبكة. هذا هو نفس العمل في Tailwind.

في Tailwind، لتعيين صف شبكة وعمود شبكة، تستخدم فئات الأدوات الخاصة بهما كما يلي:
...
عيّن للفئة `div` المغلفة فئات الأدوات `grid-cols-1` و `md:grid-cols-3`.

فئة `grid-cols-1` ترتب العناصر الثلاثة في عمود واحد، مكدسة عموديًا. على الشاشات المتوسطة والأكبر، تقوم `md:grid-cols-3` بتبديل التخطيط إلى ثلاثة أعمدة، مع عرض العناصر جنبًا إلى جنب.

# --hints--

يجب أن يحتوي عنصر `div` الخاص بك على الفئة `grid-cols-1`.
...
يجب أن يحتوي عنصر `div` الخاص بك على الفئة `md:grid-cols-3`.
