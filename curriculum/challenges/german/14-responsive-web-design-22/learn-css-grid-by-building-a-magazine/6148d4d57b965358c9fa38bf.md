...
Nun ist es an der Zeit, deine dritte `section` zu gestalten. Note that it has the `text` and `text-with-images` values for the `class` attribute, which means it is already inheriting the styles from your `.text` rule.

Erstelle einen `.text-with-images`-Selektor und setze die `display`-Eigenschaft auf `grid`.

# --hints--

Du solltest einen `.text-with-images`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

Dein `.text-with-images`-Selektor sollte die `display`-Eigenschaft auf `grid` gesetzt haben.
