...
العنصر التالي لتنسيقه هو نص السعر، بما في ذلك `span` الموجود فيه. أضف إلى عنصر `p` الخاص بالسعر الأصناف `text-4xl` و`font-bold` و`text-gray-900`.

# --hints--

يجب أن يحتوي عنصر `p` على الصنف `text-4xl`.
...
يجب أن يحتوي عنصر `p` على الصنف `font-bold`.
...
يجب أن يحتوي عنصر `p` على الصنف `text-gray-900`.
