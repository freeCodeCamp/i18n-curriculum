...
Adicione os atributos `value` e `onChange` tanto nas entradas DEF quanto PHY.

Para DEF, defina `value` como `player.def` e `onChange` como uma arrow function que chama `setPlayer` com um spread de `player` e `def` definido como `Number(e.target.value)`.

Para PHY, defina `value` como `player.phy` e `onChange` como uma arrow function que chama `setPlayer` com um spread de `player` e `phy` definido como `Number(e.target.value)`.

Com este passo, todas as entradas agora estão conectadas ao estado e a pré-visualização ao vivo será atualizada para cada campo!

# --hints--

Seu `input` DEF deve ter um `value` igual ao status `def` do jogador.
...
Seu `input` DEF deve ter um manipulador `onChange` que chama `setPlayer` com o spread de `player` e `def` definido como `Number(e.target.value)`.
...
Seu `input` PHY deve ter um `value` igual ao status `phy` do jogador.
...
Seu `input` PHY deve ter um manipulador `onChange` que chama `setPlayer` com o spread de `player` e `phy` definido como `Number(e.target.value)`.
