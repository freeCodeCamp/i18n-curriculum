...
Da der `salary` Setter jetzt erstellt ist, aktualisieren Sie `__init__`, damit er diesen aufruft, anstatt direkt `self._salary` zuzuweisen. Ändern Sie `self._salary = Employee._base_salaries[level]` zu `self.salary = Employee._base_salaries[level]`.

# --hints--

Ihre `__init__` Methode sollte `self.salary = Employee._base_salaries[level]` anstelle von `self._salary = Employee._base_salaries[level]` verwenden.
