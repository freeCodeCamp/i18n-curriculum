...
你還應該重新渲染歌曲，暫停當前播放的歌曲，設置播放器顯示，並再次設置播放按鈕可訪問文本。

調用 `renderSongs` 函數並傳入 `userData?.songs` 作爲參數。 另外，調用 `pauseSong`、`setPlayerDisplay` 和 `setPlayButtonAccessibleText` 函數。

# --hints--

你不應修改現有的 `shuffle` 函數及其內容。
