...
En tu objeto `userData`, crea una propiedad `songs` y establece su valor en el array `allSongs`.

Luego, para manejar la información de la canción actual y rastrear su tiempo de reproducción, crea las propiedades `currentSong` y `songCurrentTime`. Establece los valores en `null` y `0`, respectivamente.

# --hints--

Tu objeto `userData` debería tener una clave `songs` establecida en el array `allSongs`.

```js
assert.propertyVal(userData, "songs", allSongs);
```

Tu objeto `userData` debería tener una clave `currentSong` establecida en `null`.

```js
assert.propertyVal(userData, "currentSong", null);
```

Tu objeto `userData` debería tener una clave `songCurrentTime` establecida en `0`.
