...
キーのスタイルを設定する時間です。`#piano`ルールの下に、`class`セレクターで`.keys`要素を指定してください。新しいルールに`background-color`プロパティとして`#040404`、`width`プロパティとして`949px`、`height`プロパティとして`180px`を設定してください。
