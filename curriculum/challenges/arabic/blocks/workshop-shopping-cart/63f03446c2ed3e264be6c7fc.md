...
ابدأ بعكس قيمة `isCartShowing`. تذكر أنه يمكنك استخدام عامل النفي المنطقي `!` لعكس قيمة قيمة منطقية. قم بتعيين القيمة المعكوسة إلى `isCartShowing`.

# --hints--

يجب أن تعكس قيمة `isCartShowing`.

```js
assert.match(code, /!isCartShowing/);
```

يجب أن تعيّن القيمة المعكوسة لـ `isCartShowing` إلى `isCartShowing`.
