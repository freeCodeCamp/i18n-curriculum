...
받은편지함이 비어 있지 않고 이메일 인덱스가 유효할 때, `del` 키워드를 사용해 주어진 인덱스의 이메일을 삭제하고 `Email deleted.\n` 확인 메시지를 출력하세요.

# --hints--

`del` 목록에서 `actual_index` 위치의 이메일을 삭제하려면 `self.emails` 키워드를 사용하세요.
