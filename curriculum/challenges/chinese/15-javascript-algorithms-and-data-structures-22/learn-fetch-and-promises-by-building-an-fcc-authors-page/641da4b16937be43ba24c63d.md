...
将另一个 `.then()` 链接到现有的 `.then()` 方法。 这次，传入 `data` 作为回调函数的参数。 For the callback, use curly braces because you will have more than one expression. 在回调函数中，将 `data` 记录到控制台以查看它是什么样子。
