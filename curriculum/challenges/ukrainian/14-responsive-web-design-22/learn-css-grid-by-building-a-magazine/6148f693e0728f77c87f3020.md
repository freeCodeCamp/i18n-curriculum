...
Ви повинні мати новий запит `@media` для `only screen` з `max-width` зі значенням `550px`. Він повинен йти після двох попередніх.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

Ваше нове правило `@media` повинне мати селектор `.hero-title`, селектор `.hero-subtitle, .author, .quote, .list-title` , селектор `.social-icons` та селектор `.text`. Ці селектори повинні бути в такому порядку.
...
Ваш селектор `.hero-title` повинен мати `font-size` зі значенням `6rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[0]?.style?.fontSize === '6rem');
```

Ваш селектор `.hero-subtitle, .author, .quote, .list-title` повинен мати `font-size` зі значенням `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[1]?.style?.fontSize === '1.8rem');
```

Ваш селектор `.social-icons` повинен мати `font-size` зі значенням `2rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize === '2rem');
```

Ваш селектор `.text` повинен мати `font-size` зі значенням `1.6rem`.
