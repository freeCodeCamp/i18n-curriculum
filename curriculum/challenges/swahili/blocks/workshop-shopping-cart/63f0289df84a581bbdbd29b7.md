...
Katika wito wa kurudisha wa msikilizaji wako wa tukio, fanya wito wa njia ya `.addItem()` ya kitu chako cha `cart`, na pita `event.target.id`. Kumbuka kwamba `id` hapa itakuwa mfuatano wa herufi, kwa hivyo unahitaji kuibadilisha kuwa nambari.

Pita safu yako ya data ya `products` kama hoja ya pili.

# --hints--

Wito wa kurudisha wa msikilizaji wako wa tukio unapaswa kufanya wito wa njia ya `.addItem()` ya kitu chako cha `cart`.

```js
assert.match(code, /cart\.addItem\(/);
```

Njia yako ya `.addItem()` inapaswa kuitwa na `event.target.id` kama hoja ya kwanza. Kumbuka kubadilisha `id` kuwa nambari kwa kutumia `Number()`.

```js
assert.match(code, /cart\.addItem\(\s*Number\(\s*event\.target\.id\s*\)\s*/);
```

Njia yako ya `.addItem()` inapaswa kuitwa na safu ya data ya `products` kama hoja ya pili.
