...
カリー化された関数が実際に動く様子を見てみましょう。例として、次のように関数を呼び出すことができます。

```js
const dinner = tikkaMasala("curry powder")("tofu")("onion");
```

`gramsResult` という変数を宣言し、カリー化された関数を使って `2` の単位で表された `cup` の値を `gram` に変換した結果を代入してください。

その値をコンソールに出力してください。

# --hints--

`gramsResult` という変数を用意してください。

```js
assert.isDefined(gramsResult);
```

`gramsResult` という変数には `convertQuantity("cup")("gram")(2)` を呼び出した結果が入っているはずです。

```js
assert.strictEqual(gramsResult, convertQuantity("cup")("gram")(2));
```

`gramsResult` をコンソールに出力してください。
