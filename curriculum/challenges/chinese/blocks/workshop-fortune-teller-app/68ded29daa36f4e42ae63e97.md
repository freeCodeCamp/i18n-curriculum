...
通过使用 `forEach` 数组方法迭代 elements 参数，完成该函数。在回调函数内部，使用 `.add()` 的 `classList` 方法为每个元素添加 hidden 类。

# --hints--

你应该在 `forEach` 参数上调用 `elements` 方法。
...
在 forEach 方法内部，你应该使用 `classList.add` 方法添加 hidden 类。
