...
Тепер, коли у вас на екрані відображається список пісень, було б добре відсортувати їх в алфавітному порядку за назвою. Ви могли б вручну оновити масив `allSongs`, але в JavaScript є метод масиву, який ви можете використати, він називається <dfn>sort()</dfn>.

Метод `sort()` перетворює елементи масиву на рядки і сортує їх на місці, базуючись на їхніх значеннях у кодуванні `UTF-16`.
...
Усередині вашої функції `sortSongs` додайте метод `sort()` до `userData?.songs`. 

# --hints--

Ви повинні додати метод `sort()` до `userData?.songs`.
