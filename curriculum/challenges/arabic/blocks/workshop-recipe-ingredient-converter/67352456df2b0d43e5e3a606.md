...
أضف خاصية `gram` إلى `conversionTable`، وعيّن لها كائنًا يحتوي على خاصية `cup` بقيمة `1/240`، وخاصية `ounce` بقيمة `0.0353`، وخاصية `teaspoon` بقيمة `0.2`.

# --hints--

يجب أن يحتوي `conversionTable` على خاصية `gram`.

```js
assert.property(conversionTable, "gram");
```

يجب أن تكون خاصية `gram` كائنًا.

```js
assert.isObject(conversionTable.gram);
```

يجب أن يحتوي كائن `gram` على خاصية `cup`.

```js
assert.property(conversionTable.gram, "cup");
```

يجب تعيين خاصية `cup` إلى القيمة `1/240`.

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

يجب أن يحتوي كائن `gram` على خاصية `ounce`.

```js
assert.property(conversionTable.gram, "ounce");
```

يجب تعيين خاصية `ounce` إلى القيمة `0.0353`.

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

يجب أن يحتوي كائن `gram` على خاصية `teaspoon`.

```js
assert.property(conversionTable.gram, "teaspoon");
```

يجب تعيين خاصية `teaspoon` إلى القيمة `0.2`.
