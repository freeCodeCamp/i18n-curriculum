...
Você adicionou uma chamada `console.log()` ao seu script para validar seu array `EVAReadySquad`, e agora a chamada não é mais necessária. Para evitar que o terminal fique desorganizado nos próximos passos, vá em frente e remova o `for` loop atual que registra os astronautas `EVAReadySquad` no console.

# --hints--

Remova o `for` loop que registra os astronautas `EVAReadySquad` no console. Você deve excluir todo o bloco do loop.
