...
Altere o nível de pilha do elemento `.penguin` para que ele apareça em frente do elemento `.ground` e dê a ele uma `position` de `relative`.
...
Você deve dar ao elemento `.penguin` uma `z-index` de `4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex, '4');
```

Você deve dar à classe `.penguin` uma propriedade `position`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin')?.position);
```

Você deve dar a `.penguin` uma `position` de `relative`.
