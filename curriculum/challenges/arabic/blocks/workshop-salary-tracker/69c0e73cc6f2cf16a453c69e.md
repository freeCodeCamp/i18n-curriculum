...
بما أن المُعيّن `level` يتولى التحقق من المستويات غير الصالحة، احذف شرط `if level not in Employee._base_salaries` من دالة `__init__`. ستتولى المُعيّن هذه المهمة في الخطوة التالية.

# --hints--

يجب ألا تحتوي دالة `__init__` على شرط `level not in Employee._base_salaries`.
