...
从 `.bb1d` 渐变中移除 `orange` ，并将 `80%` 更改为 `50%` 。 这将使上半部分为 `--building-color1`，然后下半部分过渡到 `--window-color1`。

# --hints--

你应该从 `linear-gradient` 中删除 `orange`。

```js
assert.notInclude(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background, 'orange');
```

你应该将现在的第一个 `linear-gradient` 颜色更改为在 `50%` 处过渡。
