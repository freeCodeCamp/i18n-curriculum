...
다음으로, `Bug` 클래스에 `emojiElement` 타입의 `HTMLParagraphElement`이라는 단일 매개변수를 받는 생성자를 만드세요. 생성자 안에서 `emojiElement` 매개변수를 클래스의 `emojiElement` 속성에 할당하세요.

# --hints--

`Bug` 클래스에는 생성자가 있어야 합니다.
...
`Bug` 생성자에는 `emojiElement` 타입의 `HTMLParagraphElement`이라는 매개변수가 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

`Bug` 생성자는 `emojiElement`을 `this.emojiElement`에 할당해야 합니다.
