...
The `Math` object in JavaScript contains static properties and methods for mathematical constants and functions. One of those is `Math.random()`, which generates a random number from `0` (inclusive) to `1` (exclusive). Eine andere ist `Math.floor()`, die eine gegebene Zahl auf die nächste ganze Zahl abrundet.

Mit diesen kannst du eine Zufallszahl innerhalb eines Bereichs erzeugen. Dies erzeugt zum Beispiel eine Zufallszahl zwischen `1` und `5`: `Math.floor(Math.random() * 5) + 1;`.
...
Du solltest `Math.floor()` verwenden.

```js
assert.match(attack.toString(), /Math\.floor/);
```

Du solltest `Math.random()` verwenden.

```js
assert.match(attack.toString(), /Math\.random/);
```

Du solltest `Math.random()` mit dem Wert `xp` multiplizieren.

```js
assert.match(attack.toString(), /Math\.random\(\s*\)\s*\*\s*xp/);
```

Du solltest `Math.floor()` verwenden, um das Ergebnis von `Math.random() * xp` abzurunden.

```js
assert.match(attack.toString(), /Math\.floor\(\s*Math\.random\(\s*\)\s*\*\s*xp\s*.*\)/);
```

Du solltest `1` zum Ergebnis von `Math.floor()` hinzufügen.
