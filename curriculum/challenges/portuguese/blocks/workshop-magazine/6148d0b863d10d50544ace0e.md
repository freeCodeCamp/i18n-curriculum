...
O outro texto foi deslocado para fora de lugar. Mova-o para a posição dando ao seletor `.first-paragraph::first-letter` uma propriedade `float` definida como `left` e uma propriedade `margin-right` definida como `1rem`.

# --hints--

O seletor `.first-paragraph::first-letter` deve ter a propriedade `float` definida como `left`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

O seletor `.first-paragraph::first-letter` deve ter a propriedade `margin-right` definida como `1rem`.
