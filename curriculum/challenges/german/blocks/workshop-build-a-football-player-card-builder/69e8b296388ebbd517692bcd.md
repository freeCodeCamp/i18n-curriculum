...
Jetzt beginnen Sie mit dem Aufbau des Formulars. Fügen Sie innerhalb des `div` mit dem `className` von `form-panel` einen `div` hinzu, um den Abschnitt „Player Info“ einzuschließen. Fügen Sie innerhalb dieses neuen `div` einen `p` mit dem `className` von `form-section-title` und dem Text `Player Info` hinzu.

Sie werden in den nächsten Schritten weitere Felder zu diesem Abschnitt hinzufügen.

# --hints--

Ihr `div.form-panel` sollte einen `div` enthalten, der den Abschnitt Player Info umschließt.
...
Der `div` innerhalb von `div.form-panel` sollte einen `p` mit dem `className` von `form-section-title` und dem Text `Player Info` enthalten.
