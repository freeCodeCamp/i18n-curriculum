...
정규식은 단어의 일부가 아닌 전체 단어와 일치해야 합니다. 즉, `hands-free money management`와 일치하지 않아야 합니다.

이를 위해 먼저 패턴 앞뒤에 공백이 있는지 확인하세요. 공백, 탭, 줄 바꿈과 일치하는 메타 문자 `\s`를 사용하면 됩니다.

# --hints--

`freeRegex`는 `\s` 토큰을 사용해야 합니다.

```js
assert.match(freeRegex.source, /\\s/);
```

`freeRegex`는 패턴의 시작과 끝에서 공백을 찾아야 합니다.
