...
このメソッド内のHTMLも更新します。`textContent`の`cartSubTotal`を`subTotal`の値に固定小数点2桁で設定してください。テンプレートリテラル構文を使って、値の先頭にドル記号を追加してください。

# --hints--

`textContent`要素の`cartSubTotal`プロパティにアクセスしてください。

```js
assert.match(cart.calculateTotal.toString(), /cartSubTotal\.textContent/);
```

`.toFixed()`変数の`subTotal`メソッドを呼び出し、`2`を引数として渡してください。
...
`textContent`要素の`cartSubTotal`をテンプレート文字列に設定してください。
