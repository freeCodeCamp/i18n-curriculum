...
Spostandoti sull'elemento lista, aggiungi le classi `mt-6`, `space-y-2`, `text-sm` e `text-fuchsia-100` a `ul` per distanziare gli elementi della lista e renderli più gradevoli alla vista.

# --hints--

Il tuo elemento `ul` dovrebbe avere la classe `mt-6`.
...
Il tuo elemento `ul` dovrebbe avere la classe `space-y-2`.
...
Il tuo elemento `ul` dovrebbe avere la classe `text-sm`.
...
Il tuo elemento `ul` dovrebbe avere la classe `text-fuchsia-100`.
