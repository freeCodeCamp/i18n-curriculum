...
現在你的 `.map()` 函式正在接收從柯里化的 `addCharacters` 呼叫傳回的 `num => charRange(...).map(...)` 函式參考，它將正確地迭代元素並將每個元素作為 `n` 傳遞給該函式。

你會注意到你沒有使用你的 `match` 參數。在 JavaScript 中，習慣上會用底線 `_` 作為未使用參數的前置式。你也可以像這樣將參數留空：`(, char1)`，但通常為了未來的可讀性，命名參數會比較清楚。

為你的 `match` 參數添加底線前置式。

# --hints--

你應該在你的 `match` 參數前置式加上底線。
