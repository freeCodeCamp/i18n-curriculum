...
Sebbene non sia richiesto per gli elementi `label` con un `input` annidato, è comunque consigliabile collegare esplicitamente un elemento `label` con l'elemento `input` corrispondente.

Adesso aggiungi un attributo `for` a ognuno dei quattro `label` che collega il `label` al corrispondente `input` radio.
