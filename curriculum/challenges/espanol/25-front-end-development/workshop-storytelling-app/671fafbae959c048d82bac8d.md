...
Modifica el oyente de eventos `scaryStoryBtn` para llamar a la función `displayStory` con el argumento `"scary"`. Además, agrega un oyente de eventos `click` a los botones `funnyStoryBtn` y `adventureStoryBtn` que llaman a la función `displayStory` con los argumentos `"funny"` y `"adventure"` respectivamente.

Con esto, has completado exitosamente el taller.

# --hints--

Cuando haces clic en el `scaryStoryBtn`, el párrafo `#result` debe mostrar la historia aterradora `"En los bosques oscuros, un grupo de amigos tropezó con una cabina vieja y abandonada. Ellos entran en la cabaña y despiertan algo malévolo que había estado inactivo durante siglos."` con un color de borde de `#ee4b2b`.
...
Cuando haces clic en el `funnyStoryBtn`, el párrafo `#result` debe mostrar la historia divertida con un color de borde de `#f1be32`.
...
Cuando haces clic en el `adventureStoryBtn`, el párrafo `#result` debe mostrar la historia de aventura con un color de borde de `#acd157`.
