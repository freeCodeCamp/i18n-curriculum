...
`pauseButton` の要素に `"click"` イベントリスナーを追加し、第二引数として `pauseSong` を渡してください。 これは、イベントリスナーが実行する関数です。

まず再生ボタン、次に一時停止ボタンをクリックして、アプリをテストしましょう。 すべて期待通りに動作していることが確認できるはずです。

# --hints--

変数 `pauseButton` の `addEventListener()` メソッドを呼び出してください。
...
`addEventListener` メソッドの第二引数として `pauseSong` を渡してください。
