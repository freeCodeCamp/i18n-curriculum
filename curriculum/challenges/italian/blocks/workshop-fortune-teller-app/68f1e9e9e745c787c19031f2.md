...
All'interno dell'istruzione `if`, prendi l'elemento `cardTitle` e imposta il suo `textContent` su `foundCard.name`. Fai lo stesso per l'elemento `description` impostando il suo `textContent` su `foundCard.desc`.

# --hints--

Dovresti impostare il `cardTitle` `textContent` su `foundCard.name`. Non dimenticare di usare la parola chiave `this`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

Dovresti impostare il `description` `textContent` su `foundCard.desc`. Non dimenticare di usare la parola chiave `this`.
