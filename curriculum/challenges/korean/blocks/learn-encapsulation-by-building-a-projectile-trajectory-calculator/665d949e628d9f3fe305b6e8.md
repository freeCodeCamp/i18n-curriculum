...
출력이 점점 읽기 어려워지고 있으니, `print(graph.create_trajectory())`을 각 행을 따로 출력하는 반복문으로 바꾸세요. 

# --hints--

`print(graph.create_trajectory())` 호출을 제거하세요.
...
`graph.create_trajectory()`를 반복하는 반복문이 있어야 합니다.
...
`graph.create_trajectory()`의 각 행을 출력해야 합니다.
