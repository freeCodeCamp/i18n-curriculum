...
在 DRI `form-group` 之後，添加兩個 `div` 元素，每個元素內部都有 `className`，其 `className` 為 `form-group`，並且位於 `stats-grid` 中，一個用於 DEF，另一個用於 PHY。

對於 DEF，添加一個 `label`，其 `htmlFor` 為 `def`，文字為 `DEF`，接著添加一個輸入欄位，該輸入欄位的 `id` 為 `def`，且 `type` 為 `number`。

對於 PHY，添加一個標籤，其 `htmlFor` 為 `phy`，文字為 `PHY`，接著添加一個輸入欄位，該輸入欄位的 `id` 為 `phy`，且 `type` 為 `number`。

# --hints--

你應該添加另一組兩個 `div` 元素，每個元素內部都有 `className`，其 `className` 為 `form-group`，並且位於 `div` 的 `className` 中，該 `className` 位於 `stats-grid`。
...
你的第五個 `div` 元素，該元素的 `className` 位於 `form-group` 中，應該有一個 `label`，其 `htmlFor` 屬性為 `def`，`className` 為 `label`，且文字為 `DEF`。
...
你的第五個 `div` 元素，該元素的 `className` 位於 `form-group` 中，應該有一個 `input`，其 `className` 為 `input`，`id` 為 `def`，且 `type` 為 `number`。
...
你的第六個 `div` 元素，該元素的 `className` 位於 `form-group` 中，應該有一個 `label`，其 `htmlFor` 屬性為 `phy`，`className` 為 `label`，且文字為 `PHY`。
...
你的第四個 `div` 元素，該元素的 `className` 位於 `form-group` 中，應該有一個 `input`，其 `className` 為 `input`，`id` 為 `phy`，且 `type` 為 `number`。
