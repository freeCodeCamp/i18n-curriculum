...
Dovresti aggiungere una proprietà `display` con il valore `flex` alla classe `background-buildings`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.background-buildings')?.display, 'flex');
```

Dovresti aggiungere una proprietà `align-items` con il valore `flex-end` alla classe `background-buildings`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.background-buildings')?.alignItems, 'flex-end');
```

Dovresti aggiungere una proprietà `justify-content` con il valore `space-evenly` alla classe `background-buildings`.
