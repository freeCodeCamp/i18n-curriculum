...
حان الوقت لنقل بعض الشفرة من الدالة `move()` إلى الدالة `make_allowed_move()`. انقل الشفرة المسكنة داخل أول جملة `if` (باستثناء أول استدعاء لـ `print()`) إلى دالتك الجديدة. انتبه جيدًا للمسافة البادئة.
لا تنسَ إزالة الكلمة المفتاحية `pass`.

# --hints--

يجب أن تنقل الشفرة المسكنة داخل أول جملة `if` (باستثناء أول استدعاء لـ `print()`) إلى دالتك الجديدة.
