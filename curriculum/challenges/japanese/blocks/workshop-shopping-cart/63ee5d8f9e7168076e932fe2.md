...
`h2` 要素の後に、2つの `p` 要素を作成してください。最初の要素には `class` の `dessert-price` を付け、テキストはドル記号 `"$"` に続けて `price` 変数の値を設定してください。2つ目の要素には `class` の `product-category` を付け、テキストは `"Category: "` に続けて `category` 変数の値を設定してください。

# --hints--

2つの `p` 要素を作成する必要があります。
