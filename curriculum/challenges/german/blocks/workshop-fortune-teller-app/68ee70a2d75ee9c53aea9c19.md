...
Innerhalb von `map` geben Sie die `renderCard`-Funktion zurück, indem Sie `type`, `isReversed`, `name_short` und `img` als Argumente übergeben, und fügen Sie die `join`-Methode auf `map` mit einem leeren String hinzu.

# --hints--

Sie sollten die `renderCard`-Funktion zurückgeben.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

Sie sollten `type` als erstes Argument in der `renderCard`-Funktion übergeben.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

Sie sollten `isReversed` als zweites Argument in der `renderCard`-Funktion übergeben.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?isReversed\s*/);
```

Sie sollten `card.name_short` als Argument in der `renderCard`-Funktion übergeben.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*/);
```

Sie sollten `card.img` als Argument in der `renderCard`-Funktion übergeben.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

Sie sollten `join("")` an die `map`-Methode anhängen.
