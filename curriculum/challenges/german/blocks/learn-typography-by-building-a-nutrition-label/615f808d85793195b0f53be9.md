...
Erstellen Sie ein weiteres `p`-Element nach Ihrem letzten `.divider` und geben Sie ihm den Text `Total Sugars 12g`. Weisen Sie diesem `p`-Element die `class`-Werte zu, die notwendig sind, um es einzurücken und den unteren Rahmen zu entfernen. Erstellen Sie dann ein weiteres `.divider` unter Ihrem neuen `p`-Element.
...
Ihr neues `p`-Element sollte den Text `Total Sugars 12g` haben.
