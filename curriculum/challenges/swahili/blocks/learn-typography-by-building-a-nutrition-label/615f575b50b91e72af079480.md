...
Tengeneza kichaguzi kipya cha `.left-container p` ukisahihisha ukingo wa juu na chini kuwa `-5px`, na ukingo wa kushoto na kulia kuwa `-2px`. Pia weka `font-size` kuwa `2em` na `font-weight` kuwa `700`.
...
Kichaguzi chako kipya cha `.left-container p` kinapaswa kuwa na sifa ya `margin` iliyowekwa kuwa `-5px -2px`.
...
Kichaguzi chako kipya cha `.left-container p` kinapaswa kuwa na sifa ya `font-size` iliyowekwa kuwa `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

Kichaguzi chako kipya cha `.left-container p` kinapaswa kuwa na sifa ya `font-weight` iliyowekwa kuwa `700`.
