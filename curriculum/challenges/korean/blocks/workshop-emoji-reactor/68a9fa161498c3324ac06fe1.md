...
다른 버튼 작업을 시작할 시간입니다. 나머지 `button` 요소들이 HTML 파일에 추가되어 있습니다.

새 버튼들(`#confused-btn`, `#sad-btn`, `#loving-btn`) 각각을 ID로 `querySelector`를 사용해 선택한 다음, 해당 버튼을 인자로 `updateCount`를 호출하는 클릭 이벤트 리스너를 추가하세요.

변수 이름은 `confusedBtn`, `sadBtn`, `lovingBtn`을 사용하세요.

# --hints--

`confusedBtn`라는 변수를 만들어야 합니다.

```js
assert.exists(confusedBtn);
```

`#confused-btn` 요소를 `confusedBtn` 변수에 저장해야 합니다.

```js
assert.equal(confusedBtn, document.querySelector('#confused-btn'));
```

`sadBtn`라는 변수를 만들어야 합니다.

```js
assert.exists(sadBtn);
```

`#sad-btn` 요소를 `sadBtn` 변수에 저장해야 합니다.

```js
assert.equal(sadBtn, document.querySelector('#sad-btn'));
```

`lovingBtn`라는 변수를 만들어야 합니다.

```js
assert.exists(lovingBtn);
```

`#loving-btn` 요소를 `lovingBtn` 변수에 저장해야 합니다.

```js
assert.equal(lovingBtn, document.querySelector('#loving-btn'));
```

`#confused-btn` 요소에 `updateCount`를 호출하는 클릭 이벤트 리스너가 있어야 합니다.
...
`#sad-btn` 요소에 `updateCount`를 호출하는 클릭 이벤트 리스너가 있어야 합니다.
...
`#loving-btn` 요소에 `updateCount`를 호출하는 클릭 이벤트 리스너가 있어야 합니다.
