...
HTML 문서에서 `html` 태그 안에 `head`와 `body` 요소를 사용해 또 다른 조직 단계를 추가할 수 있습니다. 페이지에 관한 정보가 담긴 마크업은 `head` 태그 안에 들어갑니다. 그리고 사용자에게 표시되는 페이지 내용이 담긴 마크업은 `body` 태그 안에 들어갑니다.

`link`, `meta`, `title`, `style` 같은 메타데이터 요소는 보통 `head` 요소 안에 들어갑니다.
...
마크업을 수정해 `head`와 `body`가 있도록 하세요. `head` 요소는 `title`만 포함해야 하며, `body` 요소는 `h1`와 `p`만 포함해야 합니다.

# --hints--

페이지에는 `head` 요소가 하나만 있어야 합니다.
...
`body` 요소는 `h1`와 `p` 요소 둘 다를 감싸야 합니다.
