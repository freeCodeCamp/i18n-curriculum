...
Ili kupanga masanduku ya ingizo kwa mstari mmoja, tengeneza kanuni mpya inayolenga vipengele vyote vya `input` na `label` ndani ya kipengele cha `.info` na weka sifa ya `display` kuwa `inline-block`.

Pia, panga maandishi ya kipengele cha `label` upande wa kulia.

# --hints--

Unapaswa kutumia kichaguzi cha `.info > label, .info > input` au `.info label, .info input`.
...
Unapaswa kumpa vipengele vya `input` na `label` sifa ya `display` yenye thamani ya `inline-block`.
...
Unapaswa kumpa kipengele cha `label` sifa ya `text-align` yenye thamani ya `right`.
