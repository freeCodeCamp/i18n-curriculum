...
Infine, restituisci `secret_codes`, poi togli il commento dalla chiamata della funzione e aggiorna l'argomento a `[poem, poem2, poem3]`. Ora la tua funzione funziona con più poesie contemporaneamente e può estrarre più spille!

La funzione `pin_extractor` è completata.

# --hints--

La funzione dovrebbe restituire `secret_codes` alla fine.
...
Dovresti avere la chiamata della funzione `print(pin_extractor([poem, poem2, poem3]))`.
...
`pin_extractor([poem, poem2, poem3])` dovrebbe restituire `['5202', '3346', '50000']`.
...
La funzione dovrebbe funzionare correttamente con altre liste di poesie.
