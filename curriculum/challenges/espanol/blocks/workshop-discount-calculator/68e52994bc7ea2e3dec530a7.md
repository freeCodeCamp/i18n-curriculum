...
Para completar el taller, agrega una última instrucción de impresión para mostrar el mejor precio con dos decimales usando un f-string y este formato: `Best price for [product.name] for [user_tier] user: $[best_price]` donde `[product.name]`, `[user_tier]` y `[best_price]` deben ser reemplazados con los valores reales de las variables.

Un ejemplo de salida podría ser: `Best price for Wireless Buds for Premium user: $93.00`. Observa los dos decimales en el precio.

Con eso, ¡el taller del calculador de descuentos está completo!

# --hints--

Tu salida debe coincidir con el formato: `Best price for {product.name} for {user_tier} user: ${best_price:.2f}`.
