...
`"click"` 要素に `pauseButton` イベントリスナーを追加し、イベントリスナーの2番目の引数として `pauseSong` を渡してください。これはイベントリスナーが実行する関数です。

まず再生ボタンをクリックし、その後一時停止ボタンをクリックしてアプリをテストしてください。すべてが期待通りに動作しているはずです。

# --hints--

`addEventListener()` 変数に対して `pauseButton` メソッドを呼び出す必要があります。
...
`pauseSong` メソッドの2番目の引数として `addEventListener` を渡してください。
