...
계산 준비를 시작할 시간입니다. `consumedCalories` 변수를 선언하고 `breakfastCalories`, `lunchCalories`, `dinnerCalories`, `snacksCalories`의 합을 할당하세요(테스트에서는 순서가 중요합니다). 이 작업은 `if` 문장 다음에 하세요.

# --hints--

함수에 `consumedCalories` 변수가 있어야 합니다.

```js
assert.match(calculateCalories.toString(), /consumedCalories\s*=/);
```

`consumedCalories` 변수는 `if` 문장 다음에 와야 합니다.

```js
assert.isAbove(calculateCalories.toString().indexOf('consumedCalories'), calculateCalories.toString().indexOf('if'));
```

`breakfastCalories`, `lunchCalories`, `dinnerCalories`, `snacksCalories`의 합을 순서대로 계산해야 합니다.

```js
assert.match(calculateCalories.toString(), /breakfastCalories\s*\+\s*lunchCalories\s*\+\s*dinnerCalories\s*\+\s*snacksCalories\s*;?\s*$/m);
```

`breakfastCalories`, `lunchCalories`, `dinnerCalories`, `snacksCalories`의 합을 `consumedCalories`에 할당해야 합니다.
