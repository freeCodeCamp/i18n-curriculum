...
ユーザーがクリックしたときにいつでも曲を再生するには、最初のボタン要素に`onclick`属性を追加してください。`onclick`の中で、`playSong`を使って`song.id`関数を呼び出してください。

ここではドル記号での補間が必要なことを忘れないでください。

# --hints--

`onclick`要素に`playSong(${song.id})`の値を持つ`button`属性を追加してください。
