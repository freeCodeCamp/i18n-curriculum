...
现在你将处理球员位置的选择。你需要将输入重构为一个选择框，然后遍历一个包含有效足球位置的数组，将其作为选择框选项的值。

首先，创建一个 `POSITIONS` 数组，包含字符串值 `GK`、`CB`、`LB`、`RB`、`CDM`、`CM`、`CAM`、`LW`、`RW`、`ST` 和 `CF`。在方括弧闭合后紧接着添加 `as const`，这样 TypeScript 会将类型从通用的 `string[]` 缩小为这些确切字面值的只读元组。

这意味着 TypeScript 能准确知道哪些字符串是有效的，而不仅仅是知道它是某个字符串数组。

# --hints--

你应该声明一个 `POSITIONS` 常量。
...
你的 `POSITIONS` 数组应包含所有必需的足球位置字符串，使用 `as const` 将其类型缩小为只读字面字符串。
