...
1. Cada elemento `section` con la clase `main-section` también debe tener un `id` que corresponda con el texto de cada `header` contenido dentro de él. Cualquier espacio debe ser reemplazado por guiones bajos (por ejemplo, la sección que contiene el encabezado "JavaScript and Java" debe tener un `id="JavaScript_and_Java"` correspondiente)
