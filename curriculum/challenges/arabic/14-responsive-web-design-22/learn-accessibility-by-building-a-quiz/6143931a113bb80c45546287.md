...
يجب أن يكون لديك عنصرين `div` داخل عنصر `section` الثاني.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.length, 2);
```

يجب عليك إعطاء أول عنصر `div` جديد class بقيمة `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[0]?.className, 'question-block');
```

يجب أن تعطي العنصر `div` الجديد الثاني class بقيمة `question-block`.
...
يجب عليك إعطاء ثاني عنصر `fieldset` سمة class بقيمة `question`.
