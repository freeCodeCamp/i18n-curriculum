...
`parseCatalog` 目前傳回一個空陣列。要填滿它，請在函式內添加一個 `for` 迴圈，對 `rawCards` 進行迭代。在迴圈內，對每個元素呼叫 `parseCard`，並將結果推入 `catalog`。

# --hints--

`parseCatalog(rawCatalogCards).length` 應該是 `19`。

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

`parseCatalog` 應該使用 `for` 迴圈來迭代 `rawCards`。
