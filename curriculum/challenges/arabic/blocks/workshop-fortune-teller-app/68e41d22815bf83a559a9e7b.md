...
الآن، استخدم حدث `onload` لإزالة صنف `hidden` عند تحميل الصورة وإخفاء عنصر نائب التحميل باستخدام `previousElementSibling` الخاص به.

# --hints--

يجب أن يحتوي عنصر `img` على معالِج لحدث `onload`.
...
يجب أن يستخدم معالِج `onload` الخاص بك `classList.remove` لإزالة صنف `hidden`. لا تنسَ أنك تحتاج إلى كلمة `this` هنا.
...
يجب أن يضبط معالِج `onload` الخاص بك الخاصية `previousElementSibling.style.display` إلى `'none'`. لا تنسَ أنك تحتاج إلى كلمة `this` للقيام بذلك.
