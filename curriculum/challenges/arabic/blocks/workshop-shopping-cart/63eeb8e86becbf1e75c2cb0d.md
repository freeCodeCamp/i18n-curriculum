...
تحتاج الآن إلى العدد الإجمالي لكل منتج لدى المستخدم في السلة. أعلن عن متغير `totalCountPerProduct`، وقم بتعيين كائن فارغ له.

# --hints--

يجب أن تعلن عن متغير `totalCountPerProduct` داخل دالة `addItem` الخاصة بك.
...
يجب أن تستخدم `const` لإعلان `totalCountPerProduct`.
...
يجب أن تعيّن كائنًا فارغًا إلى `totalCountPerProduct`.
