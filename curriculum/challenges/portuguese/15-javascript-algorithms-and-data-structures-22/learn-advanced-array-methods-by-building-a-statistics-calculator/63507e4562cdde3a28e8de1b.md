...
No entanto, você não pode verificar a igualdade aqui, porque `NaN` não é igual a si mesmo. Instead, you can use the `isNaN()` method, which returns `true` if the argument is `NaN`.
