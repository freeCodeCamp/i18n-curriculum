...
داخل صنف `Spider`، أنشئ دالة بناء تأخذ معلمة واحدة `emojiElement` من نوع `HTMLParagraphElement`.

داخل دالة البناء، استدعِ الدالة `super` مع المعلمة `emojiElement` لاستدعاء دالة البناء الخاصة بالصنف الأب `Bug`. ثم عيّن الخاصية `emoji` إلى القيمة السلسلة النصية `"🕷️"`.

# --hints--

يجب أن يحتوي صنف `Spider` على دالة بناء.
...
يجب أن تحتوي دالة البناء في `Spider` على معلمة باسم `emojiElement` من نوع `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

يجب أن تستدعي دالة البناء في `Spider` الدالة `super` مع المعلمة `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?super\s*\(\s*emojiElement\s*\)/);
```

يجب أن تُعيّن دالة البناء في `Spider` الخاص بك الخاصية `this.emoji` إلى القيمة `"🕷️"`.
