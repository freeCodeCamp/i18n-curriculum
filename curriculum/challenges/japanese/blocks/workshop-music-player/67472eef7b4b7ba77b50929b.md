...
あなたの `userData` オブジェクトに `songs` プロパティを作成し、その値を `allSongs` 配列に設定してください。

次に、現在の曲の情報を管理し再生時間を追跡するために、`currentSong` と `songCurrentTime` プロパティを作成します。それぞれの値を `null` と `0` に設定してください。

# --hints--

あなたの `userData` オブジェクトには、`songs` キーが `allSongs` 配列に設定されている必要があります。

```js
assert.propertyVal(userData, "songs", allSongs);
```

あなたの `userData` オブジェクトには、`currentSong` キーが `null` に設定されている必要があります。

```js
assert.propertyVal(userData, "currentSong", null);
```

あなたの `userData` オブジェクトには、`songCurrentTime` キーが `0` に設定されている必要があります。
