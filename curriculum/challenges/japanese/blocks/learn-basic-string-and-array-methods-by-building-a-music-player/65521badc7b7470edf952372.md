...
現在再生中の曲を一時停止する処理に取り組みましょう。

`pauseSong` キーワードとアロー関数の構文を使って、パラメータを取らない `const` 関数を定義してください。

# --hints--

空の関数を作成するには `const` を使い、名前は `pauseSong` にしてください。

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong` は関数である必要があります。

```js
assert.isFunction(pauseSong)
```

`pauseSong` 関数はアロー構文を使うべきです。

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

`pauseSong` 関数は空であるべきです。
