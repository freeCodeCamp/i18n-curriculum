...
프리미엄 플랜의 `text-lg` 요소에 `font-semibold`, `text-fuchsia-200`, 그리고 `h2` 클래스를 추가하세요.

# --hints--

`h2` 요소에 `text-lg` 클래스가 있어야 합니다.
...
`h2` 요소에 `font-semibold` 클래스가 있어야 합니다.
...
`h2` 요소에 `text-fuchsia-200` 클래스가 있어야 합니다.
