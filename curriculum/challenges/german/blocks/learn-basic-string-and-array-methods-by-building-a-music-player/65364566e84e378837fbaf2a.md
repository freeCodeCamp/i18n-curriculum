...
In den vorherigen Schritten haben Sie die Funktionalität zum Abspielen eines Songs erstellt. Jetzt müssen Sie die Funktionalität für die Wiedergabetaste hinzufügen, damit der aktuelle Song abgespielt wird, wenn darauf geklickt wird.

Verwenden Sie die `addEventListener()`-Methode und übergeben Sie ein `"click"`-Ereignis als erstes Argument sowie eine leere Callback-Funktion mit Pfeilsyntax als zweites Argument, z. B. `() => {}`.

# --hints--

Sie sollten die `addEventListener()`-Methode auf Ihrer `playButton`-Variablen aufrufen.

```js
assert.match(code, /playButton\.addEventListener\(/)
```

Ihr Event Listener sollte auf ein `"click"`-Ereignis hören.

```js
assert.match(code, /playButton\.addEventListener\(\s*('|"|`)click\1/)
```

Sie sollten Pfeilsyntax verwenden, um eine leere Callback-Funktion an Ihren Event Listener zu übergeben. Vergessen Sie nicht, dass Sie auch ein leeres Paar geschweifter Klammern benötigen.
