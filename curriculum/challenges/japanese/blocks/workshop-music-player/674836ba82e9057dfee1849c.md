...
次に、`classList` プロパティと `add()` メソッドを使って、`playButton` 要素に `playing` クラスを追加してください。これにより、CSS ファイル内の `playing` クラスが探され、`playButton` 要素に追加されます。

その後、`userData.currentSong` を `song` に設定し、最後に曲を再生するために `audio` 変数に対して `play()` メソッドを使ってください。`play()` は mp3 ファイルを再生するためのウェブオーディオ API のメソッドです。

# --hints--

`playSong` 関数内で `playButton` 要素に `playing` クラスを追加してください。
...
`playSong` 関数内で `audio` 変数に対して `play()` メソッドを呼び出してください。
