...
創建一個 `if` 語句，檢查從 `gameboard` 對象調用 `solver()` 方法是否返回 `True`。

然後，在 `if` 主體內添加一個 `print()` 調用，傳遞以下 f 字符串：`f'Solved puzzle:\n{gameboard}'`。


# --hints--

你應該創建一個 `if` 語句來檢查從 `gameboard` 對象調用 `solver()` 方法是否返回 `True`。
...
你應該在新的 `if` 正文中打印 `f'Solved puzzle:\n{gameboard}'` 。
