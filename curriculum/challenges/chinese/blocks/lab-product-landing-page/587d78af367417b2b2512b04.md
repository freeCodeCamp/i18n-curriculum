...
1. 你的产品落地页应该有一个 `id="header"` 的 `header` 元素。
2. 在 `header` 元素中，你应该有一个 `id="header-img"` 的图像（建议使用 logo）。
3. 在 `#header` 元素中，你应该有一个 `id="nav-bar"` 的 `nav` 元素。
4. 在 `nav` 元素中，你应该至少包含三个可点击元素，每个的类都是 `nav-link`。
5. 当你单击 `nav` 元素中的 `.nav-link` 按钮时，你将跳转到落地页的相应部分。
6. 你应该有一个 `id="video"` 的嵌入式产品视频。
7. 你的落地页面应该有一个 `id="form"` 的 `form` 元素。
8. 在表单中，应该有一个 `id="email"` 的 `input` 字段，用于输入电子邮件地址。
9. `#email` 输入字段应有占位符文本，以便让用户知道该字段的用途。
10. `#email` 输入字段应使用 HTML5 验证，以确认输入的文本是电子邮件地址。
11. 在表单中，应该有一个 `id="submit"` 的提交 `input`。
12. 点击 `#submit` 元素时，电子邮件应该被提交到一个静态页面（使用此模拟 URL：`https://www.freecodecamp.org/email-submit`）。
13. 导航栏应该始终位于视口的顶部。
14. 你的产品落地页应该至少要有一个媒体查询。
15. 你的产品落地页应该至少使用一次 CSS flexbox。
...
你的 `#header-img` 的 `src` 的值应该是一个有效的 URL（以 `http` 开头）。
...
你的 `#form` 应该有一个 `action` 属性，值为 `https://www.freecodecamp.org/email-submit`。
