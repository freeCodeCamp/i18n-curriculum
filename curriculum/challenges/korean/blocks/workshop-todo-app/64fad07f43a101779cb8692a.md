...
코드 가독성과 유지보수성을 높이려면 `submit` 이벤트 리스너를 두 개의 별도 함수로 리팩터링하세요. 첫 번째 함수는 입력 값을 `taskData`에 추가하는 데 사용하고, 두 번째 함수는 작업을 DOM에 추가하는 역할을 맡으세요.

화살표 구문을 사용해 `addOrUpdateTask` 함수를 만드세요. 그런 다음 `dataArrIndex` 변수, `taskObj` 객체, `if` 문장을 `addOrUpdateTask` 함수로 옮기세요.

# --hints--

화살표 구문과 `const`를 사용해 `addOrUpdateTask` 함수를 만드세요.

```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

`dataArrIndex` 변수를 `addOrUpdateTask` 함수로 옮기세요.
...
`taskObj` 객체를 `addOrUpdateTask` 함수로 옮기세요.
...
조건 `if`가 포함된 `dataArrIndex === -1` 문장을 `addOrUpdateTask` 함수로 옮기세요.
