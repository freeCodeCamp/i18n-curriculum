...
为整体 `value` 添加一个 `player.overallRating` 属性，属性值为 `input`。之后，添加一个 `onChange` 属性，属性值为一个箭头函数，该函数以 `e` 作为参数，并调用 `setPlayer`，传入 `player` 的展开和 `overallRating` 设置为 `Number(e.target.value)`。

由于 `overallRating` 是数字，使用 `Number()` 将输入中的字串值转换为数字。

# --hints--

你的整体 `input` 应该有一个 `value`，其值等于当前玩家的整体评分。
...
你的整体 `input` 应该有一个 `onChange` 属性。

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

你的整体 `onChange` 的 `input` 应该调用 `setPlayer`，传入 `player` 的展开和 `overallRating` 设置为 `Number(e.target.value)`。
