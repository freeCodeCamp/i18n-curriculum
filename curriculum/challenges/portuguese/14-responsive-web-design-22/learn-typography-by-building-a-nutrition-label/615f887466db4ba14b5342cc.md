...
Após o último divisor, crie outro elemento `p` e dê a ele o texto `Protein 3g`. Use as classes necessárias para remover a borda inferior e um `span` para deixar `Protein` em negrito. Em seguida, envolva o texto `Protein 3g`, incluindo o novo elemento `span`, em um novo elemento `span`.

Depois desse elemento, crie um divisor grande.
...
A nova `div` deve ter o atributo `class` definido como `divider large`.
