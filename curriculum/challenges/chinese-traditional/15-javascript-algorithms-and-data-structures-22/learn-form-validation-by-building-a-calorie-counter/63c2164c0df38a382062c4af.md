...
每個條目都會有一個文本輸入作爲條目名稱，還有一個數字輸入作爲卡路里。 要獲取條目數量，你可以通過文本輸入進行查詢。 請注意，你不能通過數字輸入進行查詢，因爲你有一個用於用戶卡路里預算的額外數字輸入。

將字符串 `input[type="text"]` 傳遞給 `querySelectorAll()` 方法。 請記住，你需要對字符串使用單引號，以便可以在其中使用雙引號。
