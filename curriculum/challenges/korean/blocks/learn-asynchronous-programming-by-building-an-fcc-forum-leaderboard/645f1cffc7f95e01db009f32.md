...
다음 단계는 `posters` 배열을 반복하여 모든 아바타를 가져오는 것입니다.

먼저 `return` 키워드 다음에 `posters.map()`를 추가하세요. 콜백 함수에는 `poster`라는 매개변수를 추가하세요.

# --hints--

`posters.map()`를 반환해야 합니다.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

콜백 함수에는 `poster`라는 매개변수가 있어야 합니다.
