...
要计算根指数，如 $\sqrt[n]{x}$，可以使用反指数 $x^{1/n}$。 JavaScript 内置的 `Math.pow()` 函数可用于计算指数。
...
声明一个 `standardDeviation` 变量，并使用 `Math.pow()` 函数为其赋值 $variance^{1/2}$。

# --hints--

你应该有一个名为 `standardDeviation` 的变量。

```js
assert.match(getStandardDeviation.toString(), /standardDeviation\s*=/);
```

你的 `standardDeviation` 变量应该使用 `Math.pow()` 函数。

```js
assert.match(getStandardDeviation.toString(), /standardDeviation\s*=\s*Math\.pow\(/);
```

你的 `Math.pow()` 函数的基数应该为 `variance`，指数应该为 `1/2`。
