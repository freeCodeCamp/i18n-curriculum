...
Um mit Ihrem CSS zu beginnen, normalisieren Sie die CSS-Regeln, indem Sie alle Elemente mit `*` ansprechen, einschließlich der Pseudo-Selektoren `::before` und `::after`.

Setzen Sie die Eigenschaften `padding` und `margin` jeweils auf `0` und setzen Sie die Eigenschaft `box-sizing` auf `border-box`.
...
Ihr `*, ::before, ::after`-Selektor sollte eine Eigenschaft `padding` mit dem Wert `0` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.padding, '0px');
```

Ihr `*, ::before, ::after`-Selektor sollte eine Eigenschaft `margin` mit dem Wert `0` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.margin, '0px');
```

Ihr `*, ::before, ::after`-Selektor sollte eine Eigenschaft `box-sizing` mit dem Wert `border-box` haben.
