...
Du solltest einen `h1 .flex span:last-of-type`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('h1 .flex span:last-of-type'));
```

Dein `h1 .flex span:last-of-type`-Selektor sollte eine `font-size`-Eigenschaft auf `1.2em` gesetzt haben.
