...
Ora è il momento di aggiungere la funzionalità di reset a `resetButton`. Questo riporterà le canzoni nella playlist quando viene cliccato.

Aggiungi un listener per l'evento click alla variabile `resetButton`. Passa una callback usando la sintassi arrow e lasciala vuota per ora.

# --hints--

Dovresti usare il metodo `addEventListener()` su `resetButton`.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

Il tuo listener per l'evento `resetButton` dovrebbe ascoltare un evento `"click"`.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

La funzione di callback del tuo listener per l'evento dovrebbe usare la sintassi arrow e avere una coppia vuota di parentesi graffe.
