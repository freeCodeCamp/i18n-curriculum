...
Sarebbe bello vedere le modifiche apportate nella console.

Aggiungi un `console.log(currentContent);`.

Per testare le modifiche, prova a modificare la nota e poi a fare clic fuori dalla nota. Ora dovresti vedere la nota modificata nella console.

# --hints--

Dovresti aggiungere un `console.log(currentContent);` all'interno del listener dell'evento, dopo aver assegnato `newContent` a `currentContent`.
