...
Continuando con il contenitore principale, assegna alla classe `container` una proprietà `width` con valore `80%` e una proprietà `max-width` con valore `600px`. Inoltre, aggiungi una proprietà `border-radius` con valore `10px` e imposta `margin` su `20px auto` per centrarlo orizzontalmente.

# --hints--

Dovresti puntare alla classe `container`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.container'));
```

Imposta la proprietà `width` su `80%`.
...
Imposta la proprietà `max-width` su `600px`.
...
Imposta la proprietà `border-radius` su `10px`.
...
Imposta la proprietà `margin` su `20px auto`.
