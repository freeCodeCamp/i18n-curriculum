...
次に、`userAvatarUrl`を作成します。

まず、`userAvatarUrl`という定数を作成してください。次に、三項演算子を使って`avatar`が`"/user_avatar/"`で始まるかどうかを確認します。

もしそうなら、`concat`メソッドを使って`avatar`を`avatarUrl`に連結してください。そうでなければ`avatar`を返します。

これにより、アバターのURLが相対URLでも絶対URLでも正しく形成されます。

# --hints--

`userAvatarUrl`という名前の定数があるはずです。

```js
assert(code.match(/const\s+userAvatarUrl\s*=/));
```

`ternary`は、`avatar`が`"/user_avatar/"`で始まるかどうかをチェックし、`avatarUrl.concat(avatar)`か`avatar`を返すようにしてください。
