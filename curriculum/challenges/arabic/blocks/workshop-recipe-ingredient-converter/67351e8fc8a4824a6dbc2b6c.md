...
أضف إلى `conversionTable` خاصية `cup`، وعيّن لها كائنًا يحتوي على خاصية `gram` بقيمة `240`، وخاصية `ounce` بقيمة `8.0`، وخاصية `teaspoon` بقيمة `48`.

# --hints--

يجب أن يحتوي `conversionTable` على خاصية `cup`.

```js
assert.property(conversionTable, "cup");
```

يجب أن تكون خاصية `cup` كائنًا.

```js
assert.isObject(conversionTable.cup);
```

يجب أن يحتوي كائن `cup` على خاصية `gram`.

```js
assert.property(conversionTable.cup, "gram");
```

يجب أن تكون خاصية `gram` بقيمة `240`.

```js
assert.strictEqual(conversionTable.cup?.gram, 240);
```

يجب أن يحتوي كائن `cup` على خاصية `ounce`.

```js
assert.property(conversionTable.cup, "ounce");
```

يجب أن تكون خاصية `ounce` بقيمة `8.0`.

```js
assert.strictEqual(conversionTable.cup?.ounce, 8.0);
```

يجب أن يحتوي كائن `cup` على خاصية `teaspoon`.

```js
assert.property(conversionTable.cup, "teaspoon");
```

يجب أن تكون خاصية `teaspoon` بقيمة `48`.
