...
Додайте два елементи `span` з `className` `stat-value` та `stat-label` відповідно до кожного `stat-row`.

# --hints--

Кожен `.stat-row` у другому `.stat-col` має містити `span` з класом `stat-value` та `span` з класом `stat-label`.
