...
1. 你应该使用一个属性选择器来选中类名包含单词 `one` 的 `span` 元素的第一个子元素，并将其 `background-image` 属性设置为 `linear-gradient`。
1. 你应该使用一个属性选择器来选中类名包含单词 `two` 的 `span` 元素的前两个子元素，并将它们的 `background-image` 属性设置为 `linear-gradient`。
1. 你应该使用一个属性选择器来选中类名包含单词 `three` 的 `span` 元素内的三个 `span` 元素，并将它们的 `background-image` 属性设置为 `linear-gradient`。
