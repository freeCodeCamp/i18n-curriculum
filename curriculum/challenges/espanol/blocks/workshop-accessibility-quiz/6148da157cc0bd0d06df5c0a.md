...
Para alinear las cajas de entrada entre sí, crea una nueva estructura de reglas que apunte a todos los elementos `input` y `label` dentro de un elemento `.info` y establece la propiedad `display` en `inline-block`.

Además, alinea el texto del elemento `label` a la derecha.

# --hints--

Debes usar un selector `.info > label, .info > input` o `.info label, .info input`.
