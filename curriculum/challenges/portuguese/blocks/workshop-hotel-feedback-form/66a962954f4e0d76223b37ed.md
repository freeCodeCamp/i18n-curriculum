...
Em seguida, adicione outro `input` do tipo checkbox com os atributos `id` e `value` definidos como `"location"` e o atributo `name` definido como `"choice"`.

Para o elemento `label`, o texto de `Location` e o atributo `for` devem ser definidos como `"location"`.

Abaixo daquele elemento `label`, adicione outro `input` do tipo checkbox com os atributos `id` e `value` definidos como `"reputation"` e o atributo `name` definido como `"choice"`.
...
Sua terceira caixa de seleção `input` deve ter um atributo `name` definido como `"choice"`.
...
Sua quarta caixa de seleção `input` deve ter um atributo `name` definido como `"choice"`.
