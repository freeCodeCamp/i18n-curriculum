...
Обгорніть елементи `h1` та `p` в елемент `div`. Надайте цьому елементу `div` атрибут `class` зі значенням `label`.
...
Ваші елементи `h1` та `p` повинні бути в новому елементі `.label`.
