...
最後に、`conversionTable` に `teaspoon` オブジェクトを渡してください。そのオブジェクトには `cup` プロパティが `1/48` に、`gram` プロパティが `5` に、`ounce` プロパティが `0.167` に設定されています。

# --hints--

`conversionTable` には `teaspoon` プロパティが必要です。

```js
assert.property(conversionTable, "teaspoon");
```

`teaspoon` プロパティはオブジェクトである必要があります。

```js
assert.isObject(conversionTable.teaspoon);
```

`teaspoon` オブジェクトには `cup` プロパティが必要です。

```js
assert.property(conversionTable.teaspoon, "cup");
```

`cup` プロパティは `1/48` に設定してください。

```js
assert.strictEqual(conversionTable.teaspoon?.cup, 1/48);
```

`teaspoon` オブジェクトには `gram` プロパティが必要です。

```js
assert.property(conversionTable.teaspoon, "gram");
```

`gram` プロパティは `5` に設定してください。

```js
assert.strictEqual(conversionTable.teaspoon?.gram, 5);
```

`teaspoon` オブジェクトには `ounce` プロパティが必要です。

```js
assert.property(conversionTable.teaspoon, "ounce");
```

`ounce` プロパティは `0.167` に設定してください。
