...
كما فعلت مع دالتك `highPrecedence()`، يجب على دالة `evalFormula()` التأكد من أنها قيمت واستبدلت كل شيء.

استخدم معامل ثلاثي للتحقق مما إذا كانت `functionExpanded` تساوي السلسلة النصية الأصلية `x`. إذا كانت كذلك، أعد `functionExpanded`، وإلا أعد نتيجة استدعاء `evalFormula()` مرة أخرى مع `functionExpanded` و `cells` كمعلمات.

# --hints--

يجب أن تستخدم دالة `evalFormula` الكلمة المفتاحية `return`.
