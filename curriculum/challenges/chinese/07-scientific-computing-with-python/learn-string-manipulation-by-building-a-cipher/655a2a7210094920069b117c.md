...
Comparison operators allow you to compare two objects based on their values. You can use a comparison operator by placing it between the objects you want to compare. 它们返回一个 *布尔值* — 即 `True` 或 `False` — 取决于表达式的真实性。
...
At the beginning of your loop body, print the result of comparing `char` with a space (`' '`). 为此使用相等运算符 `==`。
