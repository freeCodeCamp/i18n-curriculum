...
定位 `.penguin-body` 元素，並將其 `width` 設置爲 `53%`，`height` 設置爲 `45%`。 然後，將 `background` 設置爲漸變軸爲 `45deg` 的線性漸變，起始點 `0%` 爲 `rgb(134, 133, 133)`，過渡到中間 `25%` 位置的 `rgb(234, 231, 231)`，再過渡到 `67%` 位置的 `white`。

# --hints--

你應該使用 `.penguin-body` 選擇器。

```js
assert.match(code, /\.penguin-body\s*\{/);
```

你應該將 `.penguin-body` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

你應該將 `.penguin-body` 的 `height` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

你應該將 `.penguin-body` 的 `background` 設置爲 `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)`。
