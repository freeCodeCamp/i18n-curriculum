...
El segundo argumento del método `.replace()` no tiene que ser una cadena. En su lugar, puedes pasar una función de callback para ejecutar lógica más compleja en la cadena coincidente.

La función de callback toma algunos parámetros. El primero es la cadena coincidente. Pasa una función de callback vacía a tu llamada `.replace()` y dale un parámetro `match`.

# --hints--

Debes pasar una función de flecha como segundo argumento de tu método `.replace()`.
...
Tu función de flecha debe tomar un parámetro `match`.
