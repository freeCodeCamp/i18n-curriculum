...
عربة التسوق غير مرئية حاليًا على صفحة الويب. لجعلها مرئية، ابدأ بإضافة مستمع حدث إلى المتغير `cartBtn`، يستمع إلى حدث النقر. دالة الاستدعاء الراجع لا تحتاج إلى أي معلمات.

# --hints--

يجب إضافة مستمع حدث إلى المتغير `cartBtn`.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(/);
```

يجب الاستماع إلى حدث `click` على المتغير `cartBtn`.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

يجب إضافة دالة استدعاء راجع فارغة (باستخدام صيغة السهم) إلى مستمع الحدث. تذكر أنها لا تحتاج إلى أي معلمات.
