...
ユーザーにドロップダウンメニューから選択してもらいたい場合、`select` と `option` 要素を使うことができます。

ここでは、異なる都市のドロップダウンを作成するために `select` と `option` 要素を使った例を示します。
...
まず、`select` 要素を追加し、`name` と `id` 属性を `"service"` に設定してください。

# --hints--

あなたの `fieldset` には `select` 要素が必要です。

```js
assert.exists(document.querySelector('fieldset:nth-of-type(4) select'));
```

あなたの `select` 要素には `name` 属性を `"service"` に設定してください。

```js
assert.exists(document.querySelector('select[name="service"]'));
```

あなたの `select` 要素には `id` 属性を `"service"` に設定してください。
