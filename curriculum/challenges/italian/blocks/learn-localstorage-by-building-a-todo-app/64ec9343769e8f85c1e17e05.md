...
Crea un elemento `div` con la classe `task`. Usa le template string per impostare l'attributo `id` del `div` sul `id` che hai destrutturato dai dati dell'attività.
...
Il tuo elemento `div` dovrebbe avere il `id` `${id}`.
