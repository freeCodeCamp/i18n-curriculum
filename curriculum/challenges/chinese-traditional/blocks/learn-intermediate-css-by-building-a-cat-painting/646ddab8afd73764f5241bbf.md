...
如同你對左側內耳所做的，透過設定右下和左下邊框半徑為 `40%`、左上邊框半徑為 `90px`，以及右上邊框半徑為 `10px`，來移除右側內耳的銳利邊緣。

# --hints--

你的 `.cat-right-inner-ear` 選擇器應該有一個 `border-bottom-right-radius` 屬性設定為 `40%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderBottomRightRadius === '40%')
```

你的 `.cat-right-inner-ear` 選擇器應該有一個 `border-bottom-left-radius` 屬性設定為 `40%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderBottomLeftRadius === '40%')
```

你的 `.cat-right-inner-ear` 選擇器應該有一個 `border-top-left-radius` 屬性設為 `90px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderTopLeftRadius === '90px')
```

你的 `.cat-right-inner-ear` 選擇器應該有一個 `border-top-right-radius` 屬性設定為 `10px`。
