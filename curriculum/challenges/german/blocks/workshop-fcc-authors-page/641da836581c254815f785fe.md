...
Als Nächstes zeigen Sie biografische Details über den Autor an. Das können Sie tun, indem Sie die `bio`-Eigenschaft verwenden.

Fügen Sie ein Absatz-Element mit einem `class`-Attribut von `"bio"` hinzu, und interpolieren Sie dann `bio` innerhalb des Absatz-Elements.
...
Sie sollten `bio` in Ihrem `p`-Element interpolieren.
