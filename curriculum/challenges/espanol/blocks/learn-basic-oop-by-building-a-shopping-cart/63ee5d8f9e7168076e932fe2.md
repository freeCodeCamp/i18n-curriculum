...
Después de tu elemento `h2`, crea dos elementos `p`. Dale al primero un `class` de `dessert-price`, y establece el texto con un signo de dólar `"$"` seguido del valor de la variable `price`. Dale al segundo un `class` de `product-category`, y el texto `"Category: "` seguido del valor de la variable `category`.
