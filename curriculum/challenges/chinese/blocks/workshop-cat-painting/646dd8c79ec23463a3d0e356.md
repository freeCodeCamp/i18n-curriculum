...
是时候开始处理右内耳了。 使用类选择器，为你的 `.cat-right-inner-ear` 元素设置左边和右边各 `20px solid transparent` 的边框。 另外，为其添加 `40px solid #3b3b4f` 的底部边框。

# --hints--

你应该有一个 `.cat-right-inner-ear` 选择器。

```js 
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear'))
```

你的 `.cat-right-inner-ear` 选择器应该有一个 `border-left` 属性，设置为 `20px solid transparent`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderLeft, '20px solid transparent')
```

你的 `.cat-right-inner-ear` 选择器应该有一个 `border-right` 属性，设置为 `20px solid transparent`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderRight, '20px solid transparent')
```

你的 `.cat-right-inner-ear` 选择器应该有一个 `border-bottom` 属性，设置为 `40px solid #3b3b4f`。
