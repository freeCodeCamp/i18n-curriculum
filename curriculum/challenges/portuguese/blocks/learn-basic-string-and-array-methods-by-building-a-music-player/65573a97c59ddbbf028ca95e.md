...
Finalmente, você deve renderizar as músicas novamente, atualizar o texto acessível do botão de reprodução e remover o botão de reset da playlist. Você também precisa remover o `resetButton` do DOM.

Chame a função `renderSongs()` com `sortSongs()` como argumento para renderizar as músicas novamente em ordem alfabética.

Chame a função `setPlayButtonAccessibleText()` para atualizar o texto acessível do botão de reprodução.

Remova o botão de reset da playlist chamando o método `remove()` na variável `resetButton`.

**Nota**: Agora você pode tentar remover todas as músicas para ver o que acontece.

# --hints--

Você não deve modificar o event listener existente e seu conteúdo.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Você deve chamar a função `renderSongs` com `sortSongs()`.
...
Você deve usar o método `remove()` para remover o `resetButton` do DOM.
