...
现在你有一些类似于建筑物的东西。 可以准备创建第一个变量了。  In previous lessons you learned that variable declarations begin with two dashes (`-`) and are given a name and a value like this: `--variable-name: value;`. 在 `bb1` 类中，创建一个名为 `--building-color1` 的变量，并为其赋值 `#999`。

# --hints--

你应该创建一个名为 `--building-color1` 的新变量。
