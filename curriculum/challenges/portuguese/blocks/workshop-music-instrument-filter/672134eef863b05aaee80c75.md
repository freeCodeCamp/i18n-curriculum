...
Para implementar a funcionalidade de filtro, você precisará de uma função. Declare uma função vazia chamada `instrumentCards` que recebe um único parâmetro.

# --hints--

Você deve ter uma função chamada `instrumentCards`.

```js
assert.isFunction(instrumentCards);
```

Sua função `instrumentCards` deve receber um único parâmetro.
