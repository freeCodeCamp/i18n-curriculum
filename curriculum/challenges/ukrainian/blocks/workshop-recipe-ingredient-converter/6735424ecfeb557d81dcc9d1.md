...
На цьому кроці приєднайте слухача події `submit` до `recipeForm`, щоб коли натиснути на кнопку `Convert`, викликалася функція `updateResultsList()`.

Приєднайте слухача події `submit` до `recipeForm`. Коли форму відправляють, викличте `event.preventDefault()`, щоб зупинити поведінку за замовчуванням, а потім викличте `updateResultsList()`.

Це дозволить вам запускати функцію через інтерфейс, заповнюючи форму і натискаючи `Convert`, що корисно для налагодження під час додавання логіки конвертації на наступному кроці.

# --hints--

Зробіть так, щоб натискання на кнопку `Convert` запускало `updateResultsList()`, додавши слухача події `"submit"` до `recipeForm`, який викликає `event.preventDefault()`, щоб зупинити поведінку форми за замовчуванням.
