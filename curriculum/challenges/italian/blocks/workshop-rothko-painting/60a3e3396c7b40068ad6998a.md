...
Centra l'elemento `.three` sul canvas impostando la sua `margin` su `auto`.

# --hints--

Dovresti impostare la proprietà `margin` su `auto`.
...
Il tuo elemento `.three` dovrebbe avere un valore `margin` di `auto`.
