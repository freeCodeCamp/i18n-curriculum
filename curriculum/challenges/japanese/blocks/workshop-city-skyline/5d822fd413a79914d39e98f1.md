...
次の2つのセクションに同じグラデーションを追加したいです。そうする代わりに、`bb1-window`という新しいクラスセレクターを作成し、`height`から`background`と`.bb1a`のプロパティと値を新しいクラスセレクターに移動してください。

# --hints--

`bb1-window`という新しいクラスセレクターを作成してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

`height`から`.bb1a`に`.bb1-window`のプロパティと値を移動してください。
...
`background`から`.bb1a`に`.bb1-window`のプロパティと値を移動してください。
