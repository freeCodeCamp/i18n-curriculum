...
`Brute force attacks` are when hackers try multiple passwords until they find the right one. Por ejemplo:

`The system detected a brute force attack on the login page.` - The attack tried many password guesses to break in.

`SQL injection attempts` happen when hackers try to insert malicious SQL code to access or modify a database. Por ejemplo:
