...
次に、`#posts-container` メソッドを使って `getElementById()` 要素にアクセスしてください。新しい定数 `postsContainer` に代入してください。

# --hints--

`const` を使って `postsContainer` 変数を宣言してください。

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

`document.getElementById()` を使って `#posts-container` 要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

`#posts-container` 要素を `postsContainer` 変数に代入してください。
