...
생성자 안에서 `fetchCardsData`와 `initializeEventListeners`을 호출하고 있으니, 이제 클래스 안에 이들을 정의해야 합니다. 두 메서드를 `private` 메서드로 만드세요. API 호출을 처리할 것이므로 `fetchCardsData`가 비동기여야 한다는 점을 꼭 확인하세요.

# --hints--

`private` 메서드 이름이 `fetchCardsData`이어야 합니다.
...
`fetchCardsData` 메서드는 `async`여야 합니다.
...
`private` 메서드 이름이 `initializeEventListeners`이어야 합니다.
