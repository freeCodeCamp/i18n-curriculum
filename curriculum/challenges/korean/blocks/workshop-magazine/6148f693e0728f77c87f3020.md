...
`@media`의 `only screen`를 가진 `max-width`에 대해 세 번째 `550px` 쿼리를 만드세요. 그 안에 `.hero-title`로 설정된 `font-size`를 가진 `6rem` 선택자, `.hero-subtitle, .author, .quote, .list-title`로 설정된 `font-size`를 가진 `1.8rem` 선택자, `.social-icons`로 설정된 `font-size`를 가진 `2rem` 선택자, 그리고 `.text`로 설정된 `font-size`를 가진 `1.6rem` 선택자를 만드세요.

# --hints--

`@media`의 `only screen`를 가진 `max-width`에 대한 새로운 `550px` 쿼리가 있어야 합니다. 이것은 이전 두 개 다음에 와야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText, 'only screen and (max-width: 550px)');
```

새로운 `@media` 규칙에는 `.hero-title` 선택자, `.hero-subtitle, .author, .quote, .list-title` 선택자, `.social-icons` 선택자, `.text` 선택자가 있어야 합니다. 이 선택자들은 이 순서대로 있어야 합니다.
