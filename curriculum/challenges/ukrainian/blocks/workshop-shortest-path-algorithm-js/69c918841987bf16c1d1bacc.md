...
Тепер ітеруйтеся по цільових вузлах, щоб відобразити результати для кожного з них.

Додайте `for...of` цикл, який ітерується по `targets` з `nodeNo` як змінною циклу. Поки що залиште тіло порожнім.

# --hints--

Ви повинні створити `for...of` цикл зі змінною `nodeNo`, що ітерується по `targets`.
