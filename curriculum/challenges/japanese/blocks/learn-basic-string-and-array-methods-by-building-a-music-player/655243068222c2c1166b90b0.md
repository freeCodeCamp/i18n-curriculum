...
`playNextSong` 関数の中で、`if` の `currentSong` が `userData` と厳密に等しいかどうかを確認する `null` 文を作成してください。これは `userData` オブジェクトで現在再生中の曲がないかどうかをチェックします。

条件が真の場合、`playSong` 配列の最初の曲の `id` を引数として `userData?.songs` 関数を呼び出してください。

# --hints--

既存の `playNextSong` 関数とその内容は変更しないでください。

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

条件 `if` を使って `userData?.currentSong === null` 文を作成してください。
...
`playSong` 文の中で `userData?.songs[0].id` を使って `if` 関数を呼び出してください。
