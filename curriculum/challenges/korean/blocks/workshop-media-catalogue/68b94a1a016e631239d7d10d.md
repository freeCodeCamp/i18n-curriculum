...
`MediaCatalogue` 클래스에 돌아가서 `A catalogue that can store different types of media items.`라고 말하는 도큐스트링을 추가하세요.

# --hints--

`A catalogue that can store different types of media items.` 클래스에 `MediaCatalogue` 도큐스트링을 추가해야 합니다.
