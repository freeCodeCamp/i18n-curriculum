...
Im Moment sind alle drei Fakten gleichzeitig sichtbar. Aber es sollte jeweils nur ein Fakt sichtbar sein.

Um das zu erreichen, müssen Sie das `hidden`-Attribut bei den zweiten und dritten `tabpanel`-Elementen setzen.

# --hints--

Ihr zweites `tabpanel`-Element sollte das `hidden`-Attribut haben.

```js
assert.isTrue(document.querySelectorAll('div[role="tabpanel"]')[1].hasAttribute("hidden"));
```

Ihr drittes `tabpanel`-Element sollte das `hidden`-Attribut haben.
