...
1. Dentro do seu elemento `.newspaper`, você deve ter pelo menos mais sete elementos: um para o nome do jornal que possui uma classe `name`, um para a data do artigo com uma classe `date`, um para a manchete com uma classe `headline`, um para a submanchete com uma classe `sub-headline`, um para o autor com uma classe `author`, e dois parágrafos, cada um com uma classe `text`. Todos esses elementos devem ser preenchidos com as informações do seu artigo.
