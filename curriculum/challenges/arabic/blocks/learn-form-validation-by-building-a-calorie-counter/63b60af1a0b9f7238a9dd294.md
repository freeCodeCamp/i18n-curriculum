...
تحتاج قائمة الاختيار إلى خيارات لكل من عناصر `fieldset` الخاصة بالطعام والتمارين التي أنشأتها في الخطوات السابقة. استخدم عنصر `option` لإنشاء خيار جديد لكل `fieldset`. يجب أن تكون الخاصية `value` لكل خيار هي `id` الخاص بـ `fieldset`، ونص كل خيار يجب أن يكون نص `legend`.

اجعل خيار `Breakfast` هو الخيار `selected`.

# --hints--

يجب أن تنشئ خمسة عناصر `option` داخل عنصر `select` الخاص بك.

```js
assert.equal(document.querySelectorAll('.controls select option')?.length, 5);
```

يجب أن يحتوي خيارك الأول `option` على النص `Breakfast`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[0]?.textContent?.trim(), 'Breakfast');
```

يجب أن تحتوي الخاصية `value` في خيارك الأول `option` على القيمة `breakfast`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[0]?.value, 'breakfast');
```

يجب أن يحتوي خيارك الثاني `option` على النص `Lunch`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[1]?.textContent?.trim(), 'Lunch');
```

يجب أن تحتوي الخاصية `value` في خيارك الثاني `option` على القيمة `lunch`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[1]?.value, 'lunch');
```

يجب أن يحتوي خيارك الثالث `option` على النص `Dinner`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[2]?.textContent?.trim(), 'Dinner');
```

يجب أن تحتوي الخاصية `value` في خيارك الثالث `option` على القيمة `dinner`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[2]?.value, 'dinner');
```

يجب أن يحتوي خيارك الرابع `option` على النص `Snacks`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[3]?.textContent?.trim(), 'Snacks');
```

يجب أن تحتوي الخاصية `value` في خيارك الرابع `option` على القيمة `snacks`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[3]?.value, 'snacks');
```

يجب أن يحتوي خيارك الخامس `option` على النص `Exercise`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[4]?.textContent?.trim(), 'Exercise');
```

يجب أن تحتوي الخاصية `value` في خيارك الخامس `option` على القيمة `exercise`.

```js
assert.equal(document.querySelectorAll('.controls select option')?.[4]?.value, 'exercise');
```

يجب أن يكون خيارك الأول `option` محددًا افتراضيًا.
