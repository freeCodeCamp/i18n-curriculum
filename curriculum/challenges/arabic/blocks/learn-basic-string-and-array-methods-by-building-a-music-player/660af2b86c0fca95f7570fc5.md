...
إذا كانت الدالة السهمية تُرجع تعبيرًا بسيطًا، يمكنك حذف كلمة `return` والأقواس المتعرجة `{}`. يُسمى هذا الإرجاع الضمني.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

إذا كانت الدالة السهمية تحتوي على عدة أسطر من الشفرة في جسم الدالة، فعليك استخدام كلمة `return` والأقواس المتعرجة `{}`.
...
أعد هيكلة أو حدّث دالة `addTwoNumbers` لإزالة كلمة `return` والأقواس المتعرجة `{}`. يجب أن تستخدم دالة `addTwoNumbers` إرجاعًا ضمنيًا بدلاً من ذلك.

افتح وحدة التحكم لتتأكد من أنك لا تزال تحصل على الناتج الصحيح.

# --hints--

يجب أن تستخدم دالة `addTwoNumbers` المعاد هيكلتها إرجاعًا ضمنيًا.
