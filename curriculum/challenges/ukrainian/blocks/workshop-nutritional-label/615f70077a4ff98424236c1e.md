...
Створіть новий селектор `.indent` і надайте йому множину властивостей `margin-left`, встановлену в `1em`.

# --hints--

У вас має бути новий селектор `.indent`.

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('.indent'));
```

Ваш селектор `.indent` повинен мати множину властивостей `margin-left`, встановлену в `1em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.indent')?.marginLeft, '1em');
```

Ваш селектор `.indent` має встановлювати лише властивість `margin-left`, а не інші властивості поля.
