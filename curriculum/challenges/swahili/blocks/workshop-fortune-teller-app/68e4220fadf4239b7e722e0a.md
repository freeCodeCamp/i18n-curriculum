...
Kwa kuwa programu ya fortune ina nyakati tatu tu: Zamani, Sasa, na Baadaye, tangaza enum iitwayo `DrawingType` yenye sifa tatu: `Past`, `Present`, na `Future`. Kila sifa inapaswa kuwa na thamani ya mfuatano wa herufi inayolingana na jina lake.

# --hints--

Unapaswa kuwa na `enum` iitwayo `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Enum yako inapaswa kuwa na sifa ya `Past` yenye thamani ya mfuatano wa herufi `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Enum yako inapaswa kuwa na sifa ya `Present` yenye thamani ya mfuatano wa herufi `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Enum yako inapaswa kuwa na sifa ya `Future` yenye thamani ya mfuatano wa herufi `future`.
