...
表單現在已完成。為了讓預覽卡在你輸入時即時更新，你需要將輸入欄位連結到 `player` 狀態。

從 Name `input` 開始，為它添加一個 `value` 屬性，值為 `player.name`。接著添加一個 `onChange` 屬性，設定為一個箭頭函式，該函式以 `e` 為參數，並呼叫 `setPlayer`，參數為展開 `player` 並將 `name` 設為 `e.target.value`。別忘了將展開運算和 `name` 的值設定包在一個物件中。

# --hints--

你的 name 輸入欄位 `value` 應該綁定到 player 狀態的 `name` 欄位。
...
你的 name 輸入欄位應該有一個 `onChange` 處理常式。

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

`onChange` 處理常式應該呼叫 `setPlayer`，參數是帶有展開 `setPlayer` 並將 `player` 設為 `name` 的物件的 `e.target.value`。
