...
为了使山更有层次，添加另一座山，在 `.left-mountain` 后面创建一个新的 `div`，设置这个 `div` 的 `class` 为 `back-mountain`。

# --hints--

你应该在 `body` 中添加一个新的 `div`。

```js
assert.lengthOf(document.querySelectorAll('body > div'), 4);
```

你应该将 `div` 的 `class` 设置为 `back-mountain`。

```js
assert.include(document.querySelector('div:not(.left-mountain, .ground, .penguin)')?.className, 'back-mountain');
```

你应该将 `.back-mountain` 放在 `.left-mountain` 后面。
