...
title: 'Changamoto 77: Jumla za namba msingi'
challengeType: 1
forumTopicId: 302190
...
Inawezekana kuandika nambari kumi kama jumla ya namba msingi kwa njia tano tofauti kabisa:
...
Ni thamani gani ya kwanza ambayo inaweza kuandikwa kama jumla ya namba msingi kwa njia zaidi ya `n`? 

# --hints--

`primeSummations(5)` inapaswa kurudisha nambari.

```js
assert(typeof primeSummations(5) === 'number');
```

`primeSummations(5)` inapaswa kurudisha `11`.

```js
assert.strictEqual(primeSummations(5), 11);
```

`primeSummations(100)` inapaswa kurudisha `31`.

```js
assert.strictEqual(primeSummations(100), 31);
```

`primeSummations(1000)` inapaswa kurudisha `53`.

```js
assert.strictEqual(primeSummations(1000), 53);
```

`primeSummations(5000)` inapaswa kurudisha `71`.
