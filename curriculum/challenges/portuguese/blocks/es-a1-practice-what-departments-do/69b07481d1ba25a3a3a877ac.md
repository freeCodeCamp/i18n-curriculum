...
Pratique escolher a forma correta do verbo para um departamento no singular.
...
`Verifican` é a forma para `ellos/ellas`, mas `el departamento` está no singular.
...
`Verificar` está no infinitivo, então não completa a frase corretamente aqui.
...
`El departamento de control de calidad verifica el software.` - O departamento de controle de qualidade verifica o software.

O sujeito `el departamento` está no singular, então precisa da forma verbal singular `verifica`. 
