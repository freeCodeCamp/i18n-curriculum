...
새 노래가 재생될 때마다 플레이어의 표시가 업데이트되도록 하려면 `setPlayerDisplay()` 함수 안에서 `playSong()` 함수를 호출하세요.

이제 표시창에 노래 제목과 아티스트가 나타나는 것을 볼 수 있을 것입니다.

# --hints--

`setPlayerDisplay` 함수 안에서 `playSong` 함수를 호출해야 합니다.
