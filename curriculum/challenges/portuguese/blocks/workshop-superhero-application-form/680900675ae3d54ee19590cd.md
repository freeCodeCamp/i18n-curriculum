...
O próximo `input` é uma caixa de seleção para marcar todos os poderes de super-herói que se aplicam ao usuário.

Crie um elemento `label` com um `className` de `section` e `column` e o texto `List your powers (select all that apply):`.
...
Seu elemento `label` deve conter o texto `List your powers (select all that apply):`.
