...
次に、`case` 節を `"forward"` に追加してください。

その `case` の中で、`setPlayerCards` 関数を `players.filter()` の引数で呼び出してください。

`filter()` メソッドの中で、`player` のパラメータを持つコールバック関数を追加し、`player.position` が `"forward"` と等しいかどうかをチェックしてください。

最後に、`break` 関数呼び出しの下に `setPlayerCards` 文を追加してください。

# --hints--

`case` 文の中に `"forward"` 用の新しい `switch` を追加する必要があります。

```js
assert.match(code, /\s*case\s*('|"|`)\s*forward\s*\1\s*:\s*/)
```

`"forward"` `case` の中で、`setPlayerCards` 関数を呼び出すべきです。

```js
assert.match(code, /\s*case\s*('|"|`)\s*forward\s*\1\s*:\s*setPlayerCards\(/)
```

`players.filter()` 呼び出しに `setPlayerCards` を渡すべきです。

```js
assert.match(code, /\s*case\s*('|"|`)\s*forward\s*\1\s*:\s*setPlayerCards\(\s*players\.filter\(/)
```

`players.filter()` 呼び出しにアローコールバック関数を渡すべきです。

```js
assert.match(code, /\s*case\s*('|"|`)\s*forward\s*\1\s*:\s*setPlayerCards\(\s*players\.filter\(\s*(\(.*\)|[^\s()]+)\s*=>/);
```

`filter()` コールバックは `player` パラメータを受け取るべきです。

```js
assert.match(code, /\s*case\s*('|"|`)\s*forward\s*\1\s*:\s*setPlayerCards\(\s*players\.filter\(\s*(\(\s*player\s*\)|player)\s*=>/)
```

`filter()` コールバックは `player.position` が `"forward"` であるかどうかをチェックした結果を暗黙的に返すべきです。
