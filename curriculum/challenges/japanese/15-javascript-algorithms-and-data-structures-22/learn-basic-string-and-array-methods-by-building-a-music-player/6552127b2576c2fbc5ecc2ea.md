...
`else` ブロックを追加してください。 `else` ブロックの中では、`playSong` 関数の引数に現在再生中の曲の `id` を渡して呼び出してください。

これで、再生ボタンがクリックされたときに現在再生中の曲があれば、続きから再生されます。
