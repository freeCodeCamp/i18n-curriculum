...
最後に、最初のアンカー要素には`Calendar`のテキストを、2番目には`Chat`のテキストを、3番目には`Email`のテキストを設定してください。

これで、あなたのメガナビゲーションバーコンポーネントが完成です！

# --hints--

最初のアンカー要素には`Calendar`のテキストを設定してください。

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

2番目のアンカー要素には`Chat`のテキストを設定してください。

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

3番目のアンカー要素には`Email`のテキストを設定してください。
