...
Ifuatayo, badilisha `#999` mbili za `.bb2a` kuwa `transparent`. Hii itafanya mipaka ya kushoto na kulia isionekane.

# --hints--

Unapaswa kubadilisha `border-left` ili itumie `transparent`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid transparent");
```

Unapaswa kubadilisha `border-right` ili itumie `transparent`.
