...
Використайте `const` та стрілковий синтаксис, щоб створити функцію `updateTaskContainer`.

```js
assert.match(code, /const\s+updateTaskContainer\s*=\s*\(\s*\)\s*=>\s*\{/)
```

Перемістіть `taskData.forEach()` та його вміст до функції `updateTaskContainer()`.
