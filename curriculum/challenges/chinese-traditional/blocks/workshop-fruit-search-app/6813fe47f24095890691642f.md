...
接下來，你需要一個在表單提交時使用的函式（程式）。定義一個接受 `event` 物件 `e` 並呼叫 `e.preventDefault()` 的 `handleSubmit` 函式（程式）。

# --hints--

你應該定義一個帶有 `e` 參數的 `handleSubmit` 函式（程式）。
...
你應該在 `handleSubmit()` 函式（程式）內呼叫 `e.preventDefault()`。
