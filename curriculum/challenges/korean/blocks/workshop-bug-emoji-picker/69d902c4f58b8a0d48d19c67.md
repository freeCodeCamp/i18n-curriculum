...
이제 `bugMap`에 두 개의 항목을 채우세요. `"bee"`을 키로 하여 새로운 `Bee` 인스턴스에 매핑하고, `"spider"`을 키로 하여 새로운 `Spider` 인스턴스에 매핑하세요. 두 인스턴스 모두에 `bugEmojiElement`를 인수로 전달하세요.

# --hints--

`bugMap`에는 `"bee"` 항목이 `new Bee(bugEmojiElement)`로 설정되어 있어야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

`bugMap`에는 `"spider"` 항목이 `new Spider(bugEmojiElement)`로 설정되어 있어야 합니다.
