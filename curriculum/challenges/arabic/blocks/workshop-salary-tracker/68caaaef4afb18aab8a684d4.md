...
حان الوقت لاختبار الـ setter الجديد. جرّب تعيين قيم غير صالحة مثل سلسلة نصية عشوائية أو المستوى الحالي (`trainee`) إلى `charlie_brown.level` وشاهد رسائل الخطأ في وحدة التحكم.

بعد الانتهاء، احذف الأسطر التي ترفع الأخطاء وقم بتعيين `charlie_brown.level` إلى السلسلة النصية `junior`.

# --hints--

يجب تعيين `charlie_brown.level` إلى السلسلة النصية `junior`.
