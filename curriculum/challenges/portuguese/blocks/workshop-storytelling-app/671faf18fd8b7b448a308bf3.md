...
Para diferenciar entre os diferentes tipos de histórias, você precisará passar o gênero da história selecionada para a função `displayStory`.

Adicione um parâmetro chamado `genre` à função `displayStory`.

# --hints--

Você deve adicionar o parâmetro `genre` à função `displayStory`.
