...
Ahora necesitarás crear tu primera función curry. Recuerda que una función curry es una función que devuelve otra función, de esta manera:
...
Comienza declarando una función `convertQuantity`. Debería tener un único parámetro `fromUnit`, y debería estar vacía.

# --hints--

Deberías declarar una función `convertQuantity`.

```js
assert.isFunction(convertQuantity)
```

Tu función `convertQuantity` debería tener un parámetro `fromUnit`.

```js
assert.match(convertQuantity?.toString(), /\(fromUnit\)/);
```

Tu función `convertQuantity` debería estar vacía.
