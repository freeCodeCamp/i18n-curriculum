...
アロー関数が単純な式を返す場合、`return` キーワードと波括弧 `{}` を省略できます。これを暗黙の return と呼びます。

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

アロー関数の関数本体に複数行のコードがある場合は、`return` キーワードと波括弧 `{}` を使う必要があります。
...
`addTwoNumbers` 関数をリファクタリング（更新）して、`return` キーワードと波括弧 `{}` を取り除いてください。`addTwoNumbers` 関数は代わりに暗黙の return を使うべきです。

コンソールを開いて、正しい出力が得られていることを確認してください。

# --hints--

リファクタリングした `addTwoNumbers` 関数は暗黙の return を使うべきです。
