...
Zentrieren Sie den gesamten Text innerhalb des `address`-Elements horizontal und fügen Sie etwas Innenabstand hinzu.

# --hints--

Sie sollten den Selektor des `address`-Elements verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

Sie sollten `address` einen `text-align` von `center` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Sie sollten `address` einen `padding-top` von mindestens `1px` geben.
...
Sie sollten `address` einen `padding-right` von mindestens `1px` geben.
...
Sie sollten `address` einen `padding-bottom` von mindestens `1px` geben.
...
Sie sollten `address` einen `padding-left` von mindestens `1px` geben.
