...
Sie haben wahrscheinlich bemerkt, dass Ihr Code ziemlich repetitiv wird! Es gibt eine bessere Methode, um mit mehreren ähnlichen Elementen umzugehen.

Anstatt jeden Button einzeln auszuwählen, verwenden Sie `querySelectorAll(".emoji-btn")`, um alle Buttons mit der Klasse `emoji-btn` auf einmal auszuwählen. Speichern Sie das Ergebnis in einer Variablen namens `btns`.
...
Sie sollten `querySelectorAll` verwenden, um alle Emoji-Buttons auszuwählen.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

Sie sollten das Ergebnis in einer Variablen namens `btns` speichern.
...
Die Variable `btns` sollte alle 4 Emoji-Buttons enthalten.
