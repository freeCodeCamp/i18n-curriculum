...
`textarea` の `placeholder` を `I like coding on the beach...` に設定してください。

# --hints--

`textarea` に `placeholder` 属性を追加する必要があります。

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

`placeholder` の値を `I like coding on the beach...` に設定する必要があります。
