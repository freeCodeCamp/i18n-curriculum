...
До цього моменту нічого не змінюється, коли ви вибираєте іншу помилку.

Додайте слухача події `change` до `selectElement`. У функції зворотного виклику використайте `isSelect`, щоб перевірити, чи є `e.target` `HTMLSelectElement`. Поки що не потрібно нічого робити всередині блоку `if`.

# --hints--

Ваш `selectElement` має мати слухача події `change` з функцією зворотного виклику, яка приймає параметр події.
...
У функції зворотного виклику слухача події має бути інструкція `if` з умовою `isSelect(e.target)`.
