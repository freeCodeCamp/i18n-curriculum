...
標的 `.eye-lid` 元素，並給予它們 `width` 為 `150%`、`height` 為 `100%`，以及 `background-color` 為 `--penguin-face`。
...
你應該給 `.eye-lid` 一個 `width` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

你應該給 `.eye-lid` 一個 `height` 為 `--fcc-expected--`，但實際為 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.height, '100%');
```

你應該給 `.eye-lid` 一個 `background-color` 為 `var(--penguin-face)`。
