...
使用類選擇器，爲 `.cat-right-ear` 元素的左右邊框分別設置 `35px solid transparent`。 另外，將底部邊框設置爲 `70px solid #5e5e5e`。
