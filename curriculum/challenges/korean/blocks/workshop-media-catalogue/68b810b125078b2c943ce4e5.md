...
`TVSeries` 클래스 내에 `__str__` 메서드를 만들어 다음 형식의 문자열을 반환하세요: `title (year) - S seasons, N episodes, duration min avg, director`. 여기서 `title`, `year`, `S`, `N`, `duration`, 그리고 `director`는 각각 `title`, `year`, `seasons`, `total_episodes`, `duration` 및 `director` 속성으로 교체되어야 합니다.

# --hints--

`TVSeries` 클래스에 `__str__` 메서드가 있어야 합니다.
...
`__str__` 메서드는 `title (year) - S seasons, N episodes, duration min avg, director` 형식의 문자열을 반환해야 합니다.
