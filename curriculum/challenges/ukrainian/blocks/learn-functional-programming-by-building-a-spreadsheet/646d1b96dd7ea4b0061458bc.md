...
Ви можете передати референс функції як параметр функції зворотного виклику. Референс функції — це ім’я функції без круглих дужок. Наприклад:
...
Метод `.map()` тут викличе функцію `myFunc`, передаючи ті самі аргументи, які приймає функція зворотного виклику `.map()`. Перший аргумент — це значення масиву на поточній ітерації, тому `newArray` буде `[value: 1, value: 2, value: 3]`.

Передайте референс вашої функції `elemValue` як функцію зворотного виклику до методу `.map()`.

# --hints--

Ви не повинні викликати вашу функцію `elemValue`.
...
Ви повинні передати референс `elemValue` як функцію зворотного виклику до методу `.map()`.
