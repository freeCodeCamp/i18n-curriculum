...
이제 이메일 시뮬레이터를 보여 주는 메인 함수를 만드세요.

`main` 함수를 만들고 그 안에서 `User` 클래스를 사용해 두 명의 사용자 `Tory`와 `Ramy`를 각각 `tory`와 `ramy` 변수에 할당하세요.

# --hints--

`main`라는 이름의 함수를 정의하세요.
...
`Tory`라는 사용자를 만들고 `tory`라는 변수에 할당하세요.
...
`Ramy`라는 사용자를 만들고 `ramy`라는 변수에 할당하세요.
