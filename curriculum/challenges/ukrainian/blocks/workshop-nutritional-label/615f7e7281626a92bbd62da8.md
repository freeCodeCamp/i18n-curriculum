...
Під вашим останнім елементом `p` створіть ще один елемент `p` з текстом `Sodium 160mg 7%`. Помістіть `Sodium` і `7%` кожен у свій власний елемент `span` з класом `bold`, як ви робили з іншими.

Потім додайте додатковий елемент `span` навколо `Sodium 160mg` для вирівнювання знову.
...
Ваш новий елемент `p` має містити текст `Sodium 160mg 7%`.
...
Елемент `span` навколо `Sodium` має мати атрибут `class`, встановлений у `bold`.
...
Елемент `span` має обгорнути текст `7%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.lastElementChild?.innerText === '7%');
```

Елемент `span` навколо `7%` має мати атрибут `class`, встановлений у `bold`.
