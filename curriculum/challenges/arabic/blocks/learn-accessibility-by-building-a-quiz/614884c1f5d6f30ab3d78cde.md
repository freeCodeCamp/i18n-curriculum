...
يجب أن تعطي `.formrow` `margin-top` لا يقل عن `1px`.
...
يجب أن تعطي `.formrow` `padding-top` بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingTop, '0px');
```

يجب أن تعطي `.formrow` `padding-right` لا يقل عن `1px`.
...
يجب أن تعطي `.formrow` `padding-bottom` بقيمة `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingBottom, '0px');
```

يجب أن تعطي `.formrow` `padding-left` لا يقل عن `1px`.
...
يجب أن تعطي `input` `font-size` أكبر من `13px`.
