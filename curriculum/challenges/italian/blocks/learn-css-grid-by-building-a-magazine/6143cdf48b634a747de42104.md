...
All'interno del tuo elemento `.image-wrapper`, assegna al terzo elemento `img` un attributo `src` con valore `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`, un attributo `alt` con valore `four people working on code`, un attributo `loading` con valore `lazy`, un insieme `class` impostato su `image-3`, un attributo `width` impostato su `600` e un attributo `height` impostato su `400`.

# --hints--

Il tuo terzo elemento `img` dovrebbe avere un attributo `src` impostato su `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`.
...
Il tuo terzo elemento `img` dovrebbe avere un attributo `alt` impostato su `four people working on code`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('alt') === 'four people working on code');
```

Il tuo terzo elemento `img` dovrebbe avere un attributo `loading` impostato su `lazy`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('loading') === 'lazy');
```

Il tuo terzo elemento `img` dovrebbe avere un insieme `class` impostato su `image-3`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.classList?.contains('image-3'));
```

Il tuo terzo elemento `img` dovrebbe avere un attributo `width` impostato su `600`.

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('width') === '600');
```

Il tuo terzo elemento `img` dovrebbe avere un attributo `height` impostato su `400`.
