...
現在你可以整合你的一些程式碼。首先將 `goTown` 函式（程式）內的程式碼複製並貼到你的 `update` 函式（程式）中。然後，移除 `goTown` 和 `goStore` 函式（程式）內的所有程式碼。

# --hints--

你的 `update` 函式（程式）應該將 `button1.innerText` 設定為 `"Go to store"`。

```js
assert.match(update.toString(), /button1\.innerText\s*=\s*('|")Go to store\1/);
```

你的 `update` 函式應該將 `button2.innerText` 設定為 `"Go to cave"`。

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*('|")Go to cave\1/);
```

你的 `update` 函式（程式）應該將 `button3.innerText` 設定為 `"Fight dragon"`。

```js
assert.match(update.toString(), /button3\.innerText\s*=\s*('|")Fight dragon\1/);
```

你的 `update` 函式（程式）應該將 `button1.onclick` 設定為 `goStore`。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*goStore/);
```

你的 `update` 函式（程式）應該將 `button2.onclick` 設定為 `goCave`。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*goCave/);
```

你的 `update` 函式（程式）應該將 `button3.onclick` 設定為 `fightDragon`。

```js
assert.match(update.toString(), /button3\.onclick\s*=\s*fightDragon/);
```

你的 `update` 函式（程式）應該將 `text.innerText` 設定為 `"You are in the town square. You see a sign that says \"Store\"."`。

```js
assert.match(update.toString(), /text\.innerText\s*=\s*"You are in the town square. You see a sign that says \\"Store\\"\."/);
```

你的 `goTown` 函式（程式）應該是空的。

```js
assert.match(goTown.toString(), /function goTown\(\s*\) \{\}/);
```

你的 `goStore` 函式（程式）應該是空的。
