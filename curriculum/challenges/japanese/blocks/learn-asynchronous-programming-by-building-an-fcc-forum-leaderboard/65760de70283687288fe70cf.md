...
数値 `417` の新しいキーを空のオブジェクトの値で追加してください。

そのオブジェクトの中に、キーが `category` で文字列の値が `"freeCodeCamp Support"` のプロパティを追加してください。

そのプロパティの下に、キーが `className` で文字列の値が `"support"` の別のキーを追加してください。

# --hints--

あなたの `allCategories` オブジェクトは `417` プロパティを持つべきです。

```js
assert.property(allCategories, '417');
```

あなたの `417` の値はオブジェクトであるべきです。

```js
assert.isObject(allCategories['417'])
```

あなたの `417` オブジェクトは `category` プロパティを持つべきです。

```js
assert.property(allCategories['417'], 'category');
```

あなたの `category` プロパティは文字列 `"freeCodeCamp Support"` に設定されるべきです。

```js
assert.equal(allCategories["417"].category, 'freeCodeCamp Support');
```

あなたの `417` オブジェクトは `className` プロパティを持つべきです。

```js
assert.property(allCategories['417'], 'className');
```

あなたの `className` プロパティは文字列 `"support"` に設定されるべきです。
