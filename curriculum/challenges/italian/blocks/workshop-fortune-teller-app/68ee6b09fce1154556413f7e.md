...
All'interno del metodo `initializeEventListeners`, allega un gestore di evento `click` a `singleCardBtn`. Nel gestore dell'evento, chiama il metodo `singleCardSelected`. Ricorda di ottenere il `singleCardBtn` da `elements`.
