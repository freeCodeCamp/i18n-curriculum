...
A sequência Thue-Morse $\{T_n\}$ é uma sequência binária que satisfaz:

- $T_0 = 0$
- $T_{2n} = T_n$
- $T_{2n + 1} = 1 - T_n$

Os primeiros termos de $\{T_n\}$ são dados da seguinte forma: $01101001\color{red}{10010}1101001011001101001\ldots$.

Definimos $\{A_n\}$ como a sequência ordenada de inteiros tal que a expressão binária de cada elemento aparece como uma subsequência em $\{T_n\}$.Por exemplo, o número decimal 18 é expresso como 10010 em binário. 10010 aparece em $\{T_n\}$ ($T_8$ a $T_{12}$), então 18 é um elemento de $\{A_n\}$.O número decimal 14 é expresso como 1110 em binário. 1110 nunca aparece em $\{T_n\}$, então 14 não é um elemento de $\{A_n\}$.
