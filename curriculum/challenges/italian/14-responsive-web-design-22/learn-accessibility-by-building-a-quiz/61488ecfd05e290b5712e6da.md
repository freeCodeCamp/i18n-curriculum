...
Per far sembrare la prima sezione più in linea, seleziona gli elementi `input` dentro gli elementi `.info` e imposta la loro proprietà `width` al `50%` e allinea a sinistra il loro testo.

# --hints--

Dovresti usare il selettore `.info input` o `.info > input`.
...
Dovresti dare agli elementi `input` una proprietà `width` del `50%`.
...
Dovresti dare agli elementi `input` una proprietà `text-align` con il valore `left`.
