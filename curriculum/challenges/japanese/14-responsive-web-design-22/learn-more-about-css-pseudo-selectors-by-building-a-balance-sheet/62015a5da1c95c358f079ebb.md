...
新しく追加したセレクターの `font-size` プロパティを `0.7em` に設定して、サブ見出しらしい見た目にしましょう。

# --hints--

`h1 .flex span:first-of-type` セレクターが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('h1 .flex span:first-of-type'));
```

`h1 .flex span:first-of-type` セレクターの `font-size` プロパティを `0.7em` に設定する必要があります。
