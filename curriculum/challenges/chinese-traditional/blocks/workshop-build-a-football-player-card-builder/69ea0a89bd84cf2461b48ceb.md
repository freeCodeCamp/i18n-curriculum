...
在 `try` 區塊內，透過呼叫 `localStorage` 並使用儲存鍵與字串化的玩家物件，將目前的玩家資料存檔到 `localStorage.setItem`。

而在 `catch` 區塊內，將字串 `Failed to save player data:` 和 `error` 參數依序輸出到主控台。

# --hints--

你的 `useEffect` try 區塊應該呼叫 `localStorage.setItem`，並帶入 `STORAGE_KEY` 以及字串化的 `player` 物件。

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

你的 `useEffect` catch 區塊應該將 `'Failed to save player data:'` 和 `error` 輸出到主控台。
