...
声明一个函数 `elemValue`，它接受一个 `num` 参数。 该函数应该为空。

# --hints--

你应该在 `rangeFromString()` 函数之后声明一个 `elemValue` 变量。
