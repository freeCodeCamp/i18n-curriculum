...
对于 `a` 开始标签，将 `target` 属性设置为 `"_blank"`。 然后，将 `href` 属性设置为 `${forumTopicUrl}${slug}/${id}`。

有了这些更改，你的 freeCodeCamp 论坛排行榜项目现在已经完成了！

# --hints--

你的开始锚标签应该将 `target` 属性设置为 `"_blank"`。
...
你的开始锚标签应该设置一个 `href` 属性，其值为 `${forumTopicUrl}${slug}/${id}`。
