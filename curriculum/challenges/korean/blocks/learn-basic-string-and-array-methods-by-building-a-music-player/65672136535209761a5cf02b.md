...
사용자가 노래를 클릭할 때마다 재생하려면 첫 번째 버튼 요소에 `onclick` 속성을 추가하세요. `onclick` 안에서 `playSong` 함수를 `song.id`와 함께 호출하세요.

여기서는 달러 기호로 보간해야 한다는 점을 잊지 마세요.

# --hints--

`onclick` 요소에 값이 `playSong(${song.id})`인 `button` 속성을 추가해야 합니다.
