...
コードを整理しましょう。まず、`goTown` 関数の中のコードをコピーして、`update` 関数に貼り付けてください。それから、`goTown` と `goStore` 関数の中のコードはすべて削除してください。

# --hints--

`update` 関数は `button1.innerText` を `"Go to store"` に設定する必要があります。

```js
assert.match(update.toString(), /button1\.innerText\s*=\s*('|")Go to store\1/);
```

`update` 関数は `button2.innerText` を `"Go to cave"` に設定する必要があります。

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*('|")Go to cave\1/);
```

`update` 関数は `button3.innerText` を `"Fight dragon"` に設定する必要があります。

```js
assert.match(update.toString(), /button3\.innerText\s*=\s*('|")Fight dragon\1/);
```

`update` 関数は `button1.onclick` を `goStore` に設定する必要があります。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*goStore/);
```

`update` 関数は `button2.onclick` を `goCave` に設定する必要があります。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*goCave/);
```

`update` 関数は `button3.onclick` を `fightDragon` に設定する必要があります。

```js
assert.match(update.toString(), /button3\.onclick\s*=\s*fightDragon/);
```

`update` 関数は `text.innerText` を `"You are in the town square. You see a sign that says \"Store\"."` に設定する必要があります。
