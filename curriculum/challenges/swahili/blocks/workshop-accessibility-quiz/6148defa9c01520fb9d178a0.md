...
Wakati vipengele vya `ul`/`li` ni vizuri kwa kutoa alama za orodha kwa vitu vya orodha, vitufe vyako vya radio havihitaji hivyo. Unaweza kudhibiti muonekano wa alama hizo kwa kutumia sifa ya `list-style`. Kwa mfano, unaweza kugeuza alama zako kuwa mizunguko kwa yafuatayo:
...
Ondoa mtindo wa kawaida kwa vitu vya `.answers-list` kwa kuweka mtindo wake kuwa `none`, na pia ondoa nafasi ya ndani ya orodha isiyopangwa.
...
Unapaswa kumpa `.answers-list` vigezo vya `list-style` vya `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.answers-list')?.listStyle, 'none');
```

Unapaswa kumpa `.answers-list` vigezo vya `padding` vya `0`.
