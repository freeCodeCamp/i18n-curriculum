...
Ora crea un metodo chiamato `calculate_best_price` nella tua classe `DiscountEngine`. Questo metodo deve prendere `self`, `product` e `user_tier` come parametri e restituire un `float`.

Per ora, usa solo `pass` nel corpo del metodo. Implementerai la logica nei passaggi successivi.

# --hints--

La tua classe `DiscountEngine` dovrebbe avere un metodo chiamato `calculate_best_price`.
...
Il tuo metodo `calculate_best_price` dovrebbe avere i parametri `self`, `product: Product` e `user_tier: str`.
...
Il tuo metodo `calculate_best_price` dovrebbe avere un tipo di ritorno `-> float`.
