...
表单现在已经完成。为了让预览卡在你输入时更新，你需要将输入连接到 `player` 状态。

从名称 `input` 开始，为它添加一个 `value` 属性，属性值为 `player.name`。然后添加一个 `onChange` 属性，设置为一个箭头函数，该函数以 `e` 作为参数，并调用 `setPlayer`，传入一个展开 `player` 的对象，同时将 `name` 设置为 `e.target.value`。别忘了将展开操作和 `name` 的值设置放在一个对象中。

# --hints--

你的名称输入 `value` 应该绑定到 player 状态的 `name` 字段。
...
你的名称输入应该有一个 `onChange` 处理器。

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

`onChange` 处理器应该调用 `setPlayer`，传入一个对象，该对象展开 `setPlayer` 并将 `player` 设置为 `name`，其中 `e.target.value`。
