...
Il prossimo `input` è una casella di spunta per segnare tutti i superpoteri che si applicano all'utente.

Crea un elemento `label` con un `className` di `section` e `column`, e il testo `List your powers (select all that apply):`.
...
Il tuo elemento `label` dovrebbe avere il testo `List your powers (select all that apply):`.
