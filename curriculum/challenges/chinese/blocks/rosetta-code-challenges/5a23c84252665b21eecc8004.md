...
The <a href="https://rosettacode.org/wiki/Sorting_algorithms/Cocktail_sort" target="_blank" rel="noopener noreferrer nofollow">cocktail sort</a> is an improvement on the Bubble Sort. 给定一组数字，鸡尾酒排序将从头到尾遍历数组，将最大的数字移动到末尾。 然后，它将向后遍历数组，并将最小的数字移动到开头。 它重复这两个步骤，将下一个最大/最小的数字移动到其在数组中的正确位置，直到排序完成。
