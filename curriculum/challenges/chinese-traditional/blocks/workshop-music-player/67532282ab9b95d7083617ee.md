...
在 `playNextSong` 函式（程式）內，呼叫 `setPlayerDisplay`、`highlightCurrentSong` 和 `setPlayButtonAccessibleText` 函式（程式），以在播放清單結束時正確更新播放器。

# --hints--

你應該在你的 `playNextSong` 函式（程式）的 `else` 述語中呼叫 `setPlayerDisplay`。
...
你應該在你的 `playNextSong` 函式（程式）的 `else` 述語中呼叫 `highlightCurrentSong`。
...
你應該在你的 `playNextSong` 函式（程式）的 `else` 述語中呼叫 `setPlayButtonAccessibleText`。
