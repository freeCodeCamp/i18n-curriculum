...
你不应该更改已有的带有类 `cat-left-eye` 的 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-left-eye'), 1);
```

你不应该更改已有的带有类 `cat-right-eye` 的 `div` 元素。
