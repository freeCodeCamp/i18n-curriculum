...
`button` 요소 아래에 `id`의 `message`를 가진 단락 요소를 추가하고 텍스트는 `I love freeCodeCamp!`로 하세요.
...
단락 요소는 `id`의 `message`를 가져야 합니다.

```js
assert.exists(document.getElementById("message"))
```

단락 요소는 `I love freeCodeCamp!` 텍스트를 가져야 합니다.
