...
Rimuovi la proprietà `background-color` e il suo valore da `.fb3`, e aggiungili a `.fb3a` e `.fb3b`.

# --hints--

Dovresti rimuovere `background-color` da `.fb3`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

Dovresti dare a `.fb3a` una `background-color` di `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

Dovresti dare a `.fb3b` una `background-color` di `--building-color1`.
