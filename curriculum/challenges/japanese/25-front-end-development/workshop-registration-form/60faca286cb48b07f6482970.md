...
このまま選択肢を選択してフォームを送信しても、サーバーには有用な値が送信されません。 Because of that, each `option` element needs a `value` attribute. Without this attribute, the text content of the `option` will be submitted to the server.
