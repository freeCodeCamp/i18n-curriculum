...
右目の位置を調整するために、`position` プロパティを `absolute` に、`top` を `54px` に、`left` を `134px` に設定してください。

# --hints--

`.cat-right-eye` セレクターの `position` プロパティを `absolute` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.position, 'absolute')
```

`.cat-right-eye` セレクターの `top` プロパティを `54px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.top, '54px')
```

`.cat-right-eye` セレクターの `left` プロパティを `134px` に設定してください。
