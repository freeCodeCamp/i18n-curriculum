...
Sasa, ni wakati wa kubinafsisha vitufe vya radio. Kwa chaguo-msingi, vitufe vya radio vina muonekano wa kivinjari asilia. Unataka kuondoa hili ili uweze kuunda muonekano maalum.

Chagua kipengele chenye darasa la `contact-method-radio-btn`. Weka `appearance` kuwa `none`.

# --hints--

Unapaswa kulenga kipengele chenye darasa la `contact-method-radio-btn`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn'));
```

Weka sifa ya `appearance` kuwa `none`.
