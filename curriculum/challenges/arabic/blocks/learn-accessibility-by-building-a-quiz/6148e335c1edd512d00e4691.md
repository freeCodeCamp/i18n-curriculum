...
يجب أن يؤدي النقر على روابط التنقل إلى قفز إطار العرض إلى القسم المناسب. مع ذلك، قد يكون هذا القفز مربكًا لبعض المستخدمين.

اختر كل العناصر، واضبط الخاصية `scroll-behavior` على القيمة `smooth`.

# --hints--

يجب أن تستخدم المحدد `*`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('*'));
```

يجب أن تعطي المحدد `*` الخاصية `scroll-behavior` بالقيمة `smooth`.
