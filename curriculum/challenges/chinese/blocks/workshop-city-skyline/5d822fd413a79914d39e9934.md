...
到最后一步啦！ 你要构建最后一栋建筑了！ 为其添加方向为 `90deg` 的重复渐变。 从 `0%` 到 `10%` 使用其建筑物颜色，从 `10%` 到 `30%` 使用 `transparent`。

# --hints--

你应该在 `background` 属性中将 `repeating-linear-gradient` 添加到 `.fb6`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

你应该给 `repeating-linear-gradient` 一个 `90deg` 的方向。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

你应该将从 `0%` 到 `10%` 的 `--building-color3` 作为 `repeating-linear-gradient` 的第一种颜色。
...
你应该将从 `10%` 到 `30%` 的 `transparent` 作为 `repeating-linear-gradient` 的第二种颜色。
