...
今度は、すべてのバグが持つプロパティを定義する必要があります。

`Bug` クラスの中で、`emoji` という名前のプロパティを `T` 型で、確定代入アサーション演算子（`!`）を使って宣言してください。これは TypeScript に対して、そのプロパティは後で代入されることを伝えます。

# --hints--

あなたの `Bug` クラスには `emoji` という名前のプロパティが必要です。
...
あなたの `emoji` プロパティは `T` 型である必要があります。
...
あなたの `emoji` プロパティは確定代入アサーション演算子（`!`）を使う必要があります。
