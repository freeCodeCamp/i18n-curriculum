...
通过使用 `scary` 类型调用 `displayStory` 函数来测试这一点。 边框颜色应为 `#ee4b2b`，故事应为 `“在黑暗的树林里，一群朋友偶然发现了一间废弃的旧小屋。 他们进入小屋并唤醒了沉睡了几个世纪的邪恶力量。`。
...
当你调用 `displayStory("scary")` 时，显示的故事应该是 `“在黑暗的树林里，一群朋友偶然发现了一间废弃的旧小屋。 他们进入小屋并唤醒了沉睡了几个世纪的邪恶力量。`。
