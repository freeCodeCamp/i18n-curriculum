...
Crea un selettore `.publish-date` e assegna a esso una proprietà `color` con valore `rgba(255, 255, 255, 0.5)`.

# --hints--

Dovresti avere un selettore `.publish-date`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

Il tuo selettore `.publish-date` dovrebbe avere una proprietà `color` impostata su `rgba(255, 255, 255, 0.5)`.
