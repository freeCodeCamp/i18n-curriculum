...
Agregaste una clase `center` a algunos elementos en el HTML. Ahora, debes definir esa clase para centrar esos elementos.

Apunta a la clase `center` y centra los elementos que la tengan con una propiedad `text-align` establecida en `center`.

# --hints--

Debes apuntar a los elementos con la clase `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.center'));
```

Debes establecer la propiedad `text-align` en `center`.
