...
마찬가지로, 사용자가 이메일 주소를 클릭해서 기본 이메일 클라이언트에서 이메일을 보낼 수 있어야 합니다.

이메일 주소를 클릭 가능하게 만드는 방법은 다음과 같습니다:

```html
<a href="mailto:contact@company.com">contact@company.com</a>
```

마지막 단계로, 텍스트 `fake@email.com`를 앵커 요소로 감싸고 `mailto:`를 사용해 클릭 가능한 이메일 주소로 만드세요.

이 변경을 완료하면 블로그 페이지가 완성됩니다.

# --hints--

텍스트 `fake@email.com`를 `a` 요소로 감싸야 합니다.
...
새로운 `href` 요소의 `a` 속성은 `mailto:fake@email.com`로 설정해야 합니다.
