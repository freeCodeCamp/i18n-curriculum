...
`parseCard` foi testado e funciona corretamente. A variável `cardResult` não é mais necessária.

Comece removendo a linha `const cardResult` e a linha `console.log(cardResult)`. Depois crie uma função chamada `parseCatalog` que receba `rawCards` como parâmetro. Por enquanto, declare um array vazio chamado `catalog` dentro da função e retorne-o.

Abaixo da função, faça a chamada `parseCatalog(rawCatalogCards)`, atribua o resultado a uma variável chamada `catalog` e registre `catalog.length` no console.

# --hints--

`parseCatalog` deve ser uma função que retorna um array.

```js
assert.isArray(parseCatalog(rawCatalogCards));
```

`catalog` deve ser declarada e atribuída a `parseCatalog(rawCatalogCards)`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+catalog\s*=\s*parseCatalog\s*\(/);
```

Você deve excluir a variável `cardResult` e a chamada relacionada `console.log()`.
