...
ユーザーがドロップダウンから `Year, Month, Day` のオプションを選ぶと、選んだ物が日付の形式に反映されるようにします。

そのためには、`switch` 文の中に、式 `expr` との一致をチェックする `case` 節を追加します。case 節の後には、一致した場合に実行するコードを書きます。 下記は、`expr` が文字列 `"case123"` と一致するかチェックする `case` 節の例です:
...
では、値が `"yyyy-mm-dd"` の場合に一致する `case` 節を追加してください。 その `case` では、`currentDateParagraph` のテキストコンテンツを `formattedDate` の値に設定してください。
