...
إذا كان `value` الخاص بـ `numberInput` يساوي `5`، فقم باستدعاء الدالة `showAnimation()`، ثم استخدم `return` للخروج مبكرًا.

# --hints--

يجب استدعاء الدالة `showAnimation()` داخل جسم جملة `if` الثانية.

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

بعد استدعاء الدالة `showAnimation()`، يجب استخدام الكلمة المفتاحية `return` للخروج مبكرًا من الدالة `checkUserInput()`.
