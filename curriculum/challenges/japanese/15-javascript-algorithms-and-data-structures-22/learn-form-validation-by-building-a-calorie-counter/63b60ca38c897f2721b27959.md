...
フォームには結果を表示する場所が必要です。 空の `div` 要素を追加して `id` を `output` に設定し、`class` の値を `output` と `hide` にしてください。

# --hints--

`form` の後に `div` 要素を追加してください。

```js
assert.exists(document.querySelector('.container > div'));
```

追加した `div` 要素の `id` を `output` に設定してください。

```js
assert.equal(document.querySelector('.container > div')?.id, 'output');
```

追加した `div` 要素の `class` の値を `output` と `hide` にしてください。
