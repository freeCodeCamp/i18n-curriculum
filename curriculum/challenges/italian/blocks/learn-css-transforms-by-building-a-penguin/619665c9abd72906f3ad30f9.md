...
Costruirai un felice Flappy Penguin, esplorando ulteriormente le trasformazioni CSS e le animazioni.

Inizia con il tuo codice boilerplate HTML di base. Includi la dichiarazione `DOCTYPE`, l'elemento `html` con la lingua impostata su inglese, i tag `meta` appropriati, un `head`, `body` e un elemento `title`. Inoltre, collega il tuo foglio di stile alla pagina.
...
Il tuo elemento `html` dovrebbe avere un tag di apertura con un attributo `lang` impostato su `en`.
