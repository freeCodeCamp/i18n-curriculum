...
숫자 `10`가 배열의 맨 앞에 위치하는 것을 확인하세요. 이는 `.sort()`의 기본 동작이 숫자 값을 문자열로 변환하고 알파벳 순으로 정렬하기 때문입니다. 그리고 `"10"`가 `"2"`보다 알파벳 순으로 앞서기 때문입니다.

이 문제를 해결하려면 `.sort()` 메서드에 콜백 함수를 전달할 수 있습니다. 콜백 함수는 두 개의 매개변수를 가지며, 여기서는 `a`와 `b`를 사용하세요. `a`와 `b` 매개변수는 정렬할 배열 내 숫자 값을 나타냅니다.

함수는 지금은 비워 두세요.

# --hints--

`.sort()` 메서드에 콜백 함수를 전달해야 합니다. 화살표 구문을 사용하세요.

```js
assert.match(code, /const\s+sortedValues\s*=\s*inputValues\s*\.\s*sort\s*\(\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

콜백 함수는 두 개의 매개변수 `a`과 `b`를 가져야 합니다.
