...
1. 你應該有一個 `html` 元素，且 `lang` 設為 `en`。
1. 你應該有一個 `head` 元素，裡面包含一個帶有你食譜名稱的 `title` 元素，以及一個 `meta` 元素，其 `charset` 屬性設為 `UTF-8`。
1. 你應該有一個 `body` 元素。
1. 你應該只有一個帶有你食譜名稱的 `h1` 元素。
1. 你應該有一個 `p` 元素，位於 `h1` 下面，用來介紹這個食譜。
1. 你應該有一個 `h2` 元素，文字為 `Ingredients`，用於食材區塊。
1. 你應該在第一個 `h2` 元素下面有一個無序列表（`ul` 元素），裡面至少有四個列出食材的列表項目（`li` 元素）。
1. 你應該有第二個 `h2` 元素，文字為 `Instructions`，用於步驟說明區塊。
1. 你應該在第二個 `h2` 元素下面有一個有序列表（`ol` 元素），裡面至少有四個列出食譜步驟的列表項目，且依序排列。
1. 你應該有一個 `img` 元素，其 `src` 屬性設為有效的圖片，且有一個描述圖片的 `alt` 屬性。如果你願意，可以使用 `https://cdn.freecodecamp.org/curriculum/labs/recipe.jpg`。
...
你的 `html` 元素應該有一個閉合標籤。閉合標籤在 `<` 字元後緊接著有一個 `/`。
...
你的 `title` 元素應該有一個結束標籤。結束標籤在 `<` 字元後面緊接著有一個 `/`。
...
你的 `body` 元素應該有一個閉合標籤。閉合標籤在 `<` 字元後緊接著有一個 `/`。

```js
assert.match(code, /<\/\s*body\s*>/i);
```

你應該只有一個帶有你食譜名稱的 `h1` 元素。
