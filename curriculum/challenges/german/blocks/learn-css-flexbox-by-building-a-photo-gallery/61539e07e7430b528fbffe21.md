...
Geben Sie Ihrem `.gallery`-Selektor eine `padding`-Eigenschaft mit dem Wert `20px 10px`, um etwas Abstand um den Container zu schaffen.

Geben Sie ihm dann eine `max-width` von `1400px` und fügen Sie eine `margin` von `0 auto` hinzu, um ihn zu zentrieren.

# --hints--

Ihr `.gallery`-Selektor sollte eine `padding`-Eigenschaft mit dem Wert `20px 10px` haben.
...
Ihr `.gallery`-Selektor sollte eine `max-width`-Eigenschaft mit dem Wert `1400px` haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.gallery')?.maxWidth === '1400px');
```

Ihr `.gallery`-Element sollte mit einer `margin` mit dem Wert `0 auto` zentriert werden.
