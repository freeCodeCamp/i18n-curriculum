...
當處理屬性名稱和值相同的物件時，你可以使用簡寫屬性名稱語法。若屬性值與屬性名稱相同，這種語法允許你省略屬性值。
...
在建構式內，為 `this.position` 添加並指定一個包含 `x` 和 `y` 座標的物件。請確保使用簡寫屬性語法。
...
`this.position` 屬性應該是一個包含 `x` 和 `y` 座標的物件。請確保使用簡寫屬性語法。
