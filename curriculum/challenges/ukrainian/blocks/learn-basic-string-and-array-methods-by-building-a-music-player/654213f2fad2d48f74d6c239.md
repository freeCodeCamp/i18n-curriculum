...
Створіть ще один елемент `button` з класом `playlist-song-delete` та атрибутом `aria-label`, встановленим у `Delete`, інтерпольованим з `song.title`. Для вмісту іконки видалення вставте наступний SVG:
...
Ви маєте створити елемент `button` з класом `playlist-song-delete` як перший атрибут і значення.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

Ваш елемент кнопки повинен мати атрибут `aria-label` зі значенням `Delete ${song.title}` як другий атрибут і значення.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

Ви маєте вставити наданий `svg` як вміст вашого елемента `button`.
