...
Так само, як і звичайні функції, стрілкові функції можуть повертати значення.

Ось приклад стрілкової функції, яка повертає результат множення двох чисел:
...
Створіть нову змінну з назвою `addTwoNumbers` і призначте їй стрілкову функцію. Ця стрілкова функція має приймати два параметри з іменами `num1` і `num2`. Усередині тіла функції поверніть вираз додавання `num1` і `num2`.

Під функцією `addTwoNumbers` додайте інструкцію console. Усередині цієї інструкції викличте функцію `addTwoNumbers` і передайте їй числа `3` і `4` як аргументи.

Відкрийте консоль, щоб побачити вихідні дані.

# --hints--

У вас має бути функція з назвою `addTwoNumbers`.

```js
assert.isFunction(addTwoNumbers);
```

Ваша функція `addTwoNumbers` має використовувати синтаксис стрілкової функції.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Ваша функція `addTwoNumbers` має мати два параметри з іменами `num1` і `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

Ваша функція `addTwoNumbers` має повертати вираз додавання `num1` і `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

У вас має бути інструкція `console.log(addTwoNumbers(3, 4));`.
