...
Para un control adicional sobre el diseño de tu contenido, puedes tener un CSS Grid dentro de un CSS Grid.

Establecer la propiedad `display` de tu selector `.heading` en `grid`.

# --hints--

Tu selector `.heading` debe tener una propiedad `display` establecida en `grid`.
