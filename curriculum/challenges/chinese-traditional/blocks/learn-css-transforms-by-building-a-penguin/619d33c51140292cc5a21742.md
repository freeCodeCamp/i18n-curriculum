...
在左臂上應用 `wave` 動畫。 讓動畫持續 `3s`，無限循環，並具有線性緩動函數。

# --hints--

你應該將 `.arm.left` 的 `animation-name` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationName, 'wave');
```

你應該將 `.arm.left` 的 `animation-duration` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationDuration, '3s');
```

你應該將 `.arm.left` 的 `animation-iteration-count` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm.left')?.animationIterationCount, 'infinite');
```

你應該將 `.arm.left` 的 `animation-timing-function` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
