...
노래의 현재 재생 시간을 처리하는 `else` 블록을 추가하세요. 이렇게 하면 노래가 일시 중지된 지점부터 다시 재생할 수 있습니다.
...
`audio.currentTime` 문장의 `userData?.songCurrentTime` 블록 안에서 `else`을 `if`로 설정하세요.
