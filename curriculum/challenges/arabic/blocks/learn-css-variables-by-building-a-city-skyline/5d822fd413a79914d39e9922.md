...
امنح العنصر `.fb3a` عرضًا (`width`) بنسبة `80%` وارتفاعًا (`height`) بنسبة `15%`. ثم امنح العنصر `.fb3b` عرضًا (`width`) بنسبة `100%` وارتفاعًا (`height`) بنسبة `35%`.

# --hints--

يجب أن تمنح `.fb3a` عرضًا (`width`) بنسبة `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

يجب أن تمنح `.fb3a` ارتفاعًا (`height`) بنسبة `15%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

يجب أن تمنح `.fb3b` عرضًا (`width`) بنسبة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3b")?.width, "100%");
```

يجب أن تمنح `.fb3b` ارتفاعًا (`height`) بنسبة `35%`.
