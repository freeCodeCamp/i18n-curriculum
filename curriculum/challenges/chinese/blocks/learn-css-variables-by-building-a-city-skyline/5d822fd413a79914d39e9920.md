...
对于 `.fb2a`，添加一个值为 `10vh solid var(--building-color3)` 的 `border-bottom`，以及值为 `1vw solid transparent` 的 `border-left` 和 `border-right`。 这一次边框技巧将创建一个梯形形状。

# --hints--

你应该给 `.fb2a` 一个值为 `10vh solid var(--building-color3)` 的 `border-bottom`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderBottom.trim(), "10vh solid var(--building-color3)");
```

你应该给 `.fb2a` 一个值为 `1vw solid transparent` 的 `border-left`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderLeft, "1vw solid transparent");
```

你应该给 `.fb2a` 一个值为 `1vw solid transparent` 的 `border-right`。
