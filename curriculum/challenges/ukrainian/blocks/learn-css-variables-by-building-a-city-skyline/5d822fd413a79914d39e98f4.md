...
Градієнти можуть містити стільки кольорів, скільки ви хочете, ось так:
...
Додайте `linear-gradient` до `.bb1d` з `orange` як першим кольором, `--building-color1` як другим і `--window-color1` як третім. Не забудьте застосувати градієнт до властивості `background`.

# --hints--

Ви повинні використати `background` на `.bb1d`.
...
Ви повинні використати `orange` як перший колір у `linear-gradient`.

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1d')?.getPropVal('background', true), 'linear-gradient(orange');
```

Ви повинні використати `--building-color1` як другий колір у `linear-gradient`.
...
Ви повинні використати `--window-color1` як третій колір у `linear-gradient`.
