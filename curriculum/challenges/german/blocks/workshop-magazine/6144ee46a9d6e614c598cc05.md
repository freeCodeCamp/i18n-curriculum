...
Erstellen Sie einen `a`-Selektor und geben Sie ihm eine `text-decoration`-Eigenschaft mit dem Wert `none` sowie eine `color`-Eigenschaft mit dem Wert `linen`.

# --hints--

Sie sollten einen `a`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('a'));
```

Ihr `a`-Selektor sollte eine `text-decoration`-Eigenschaft mit dem Wert `none` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration, 'none');
```

Ihr `a`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `linen` haben.
