...
title: 'Problema 418: triple di fattorizzazione'
challengeType: 1
forumTopicId: 302087
...
Sia $n$ un numero intero positivo. Una tripla di interi ($a$, $b$, $c$) si chiama tripla di fattorizzazione di $n$ se:

- $1 ≤ a ≤ b ≤ c$
- $a \times b \times c = n$.

Definisci $f(n)$ come $a + b + c$ per la tripla di fattorizzazione ($a$, $b$, $c$) di $n$ che minimizza $\frac{c}{a}$. Si può dimostrare che questa tripla è unica.
