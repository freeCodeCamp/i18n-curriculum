...
أنشئ عنصر `button` مع `className` بقيمة `submit-btn` و`type` بقيمة `submit`. اجعل نص الزر هو `Join the League`.

بعد ذلك، جرّب إرسال النموذج وانظر ماذا يحدث.
...
يجب أن يحتوي عنصر الزر على خاصية `className` مضبوطة على `submit-btn`.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

يجب أن يكون نص عنصر `button` هو `Join the League`.
