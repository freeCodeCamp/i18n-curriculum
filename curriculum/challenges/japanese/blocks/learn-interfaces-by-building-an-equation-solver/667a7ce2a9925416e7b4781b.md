...
構造的パターンマッチングでは、対象が特定の構造を持っているかどうかを検証できます。それに加えて、パターン内の名前を対象の要素にバインドします。例えば：
...
`match`/`case` の構造を `results` ではなく `len(results)` にマッチするように変更してください。次に、各 `case` を適切な要素数のリストを使うように変更してください。リストが1つの要素を含む場合は `x` を使い、リストが2つの要素を含む場合は `x1` と `x2` を使ってください。

最後に、f文字列をそれぞれの `case` で使われている変数名を使うように変更してください。

# --hints--

`match` 文を `results` を対象値として使うように変更してください。

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("results")`)) })
```

最初の `case` をパターン `[]` を使うように変更してください。

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_cases()[0].find_case_pattern().is_equivalent("[]")`)) })
```

最初の `case` の本体は変更しないでください。
...
2番目の `case` をパターン `[x]` を使うように変更してください。
...
`result_list` 内の f文字列を `x` の代わりに `result[0]` を使うように変更してください。
...
3番目の `case` を `x1` と `x2` を含むリストをパターンとして使うように変更してください。
...
`result_list` 内の f文字列をパターンからバインドされた変数を使うように変更してください。
