...
Dichiara una `rangeFromString` funzione freccia che prende due parametri, `num1` e `num2`. La funzione dovrebbe restituire implicitamente il risultato della chiamata a `range` con `num1` e `num2` come argomenti.

Per sicurezza, converti `num1` e `num2` in numeri interi mentre li passi a `range`.

# --hints--

Dovresti dichiarare una variabile `rangeFromString` dopo la tua `rangeRegex`.
...
Dovresti usare `const` per dichiarare la tua variabile `rangeFromString`.
...
La tua variabile `rangeFromString` dovrebbe essere una funzione freccia.
...
La tua funzione `rangeFromString` dovrebbe avere `num1` come primo parametro.
...
La tua funzione `rangeFromString` dovrebbe avere `num2` come secondo parametro.
...
La tua funzione `rangeFromString` dovrebbe usare un ritorno implicito.
...
La tua funzione `rangeFromString` dovrebbe restituire il risultato della chiamata alla tua funzione `range`.
...
Dovresti chiamare `parseInt` con `num1` come argomento e passare il risultato alla chiamata `range`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

Dovresti chiamare `parseInt` con `num2` come argomento e passare il risultato alla chiamata `range`.
