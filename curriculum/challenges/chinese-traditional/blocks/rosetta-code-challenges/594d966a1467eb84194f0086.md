...
計算整數集合 $1$ 到 $10$（含）三種<a href="https://en.wikipedia.org/wiki/Pythagorean_means" target="_blank" rel="noopener noreferrer nofollow">畢氏平均數</a>。

證明對這組正整數有 $A(x_1,\\ldots,x_n) \\geq G(x_1,\\ldots,x_n) \\geq H(x_1,\\ldots,x_n)$。
...
`pythagoreanMeans` 應該是一個函式（程式）。

```js
assert(typeof pythagoreanMeans === 'function');
```

`pythagoreanMeans([1, 2, ..., 10])` 應該與上述相同輸出相等。
