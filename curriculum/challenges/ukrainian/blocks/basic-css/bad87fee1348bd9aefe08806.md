...
title: стилізуйте кілька елементів за допомогою CSS-класу
challengeType: 0
...
Класи дозволяють використовувати однакові CSS-стилі для кількох HTML-елементів. Ви можете побачити це, застосувавши свій `red-text` клас до першого `p` елемента.

# --hints--

Ваш `h2` елемент має бути червоним.
...
Ваш `h2` елемент має мати клас `red-text`.

```js
assert.isTrue(document.querySelector('h2').classList.contains('red-text'));
```

Ваш перший `p` елемент має бути червоним.
...
Ваші другий і третій `p` елементи не повинні бути червоними.
...
Ваш перший `p` елемент має мати клас `red-text`.
