...
對於 `.key.black--key::after`，將 `width` 設定為 `32px`，並將 `height` 設定為 `100px`。

# --hints--

你的 `.key.black--key::after` 應該有一個 `width` 屬性設定為 `32px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.key.black--key::after')?.width === '32px');
```

你的 `.key.black--key::after` 應該有一個 `height` 屬性設定為 `100px`。
