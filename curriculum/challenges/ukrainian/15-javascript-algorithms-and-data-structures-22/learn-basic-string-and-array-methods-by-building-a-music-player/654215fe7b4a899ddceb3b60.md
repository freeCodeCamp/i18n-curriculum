...
Прямо зараз `songsHTML` є масивом. Якщо ви спробуєте показати його як є, ви побачите пісні, розділені комами. Це не бажаний результат, оскільки ви хочете показати пісні у вигляді списку. Щоб виправити це, потрібно буде об’єднати масив в один рядок використавши метод <dfn>join()</dfn>.

Метод `join()` використовується для об’єднання всіх елементів масиву в один рядок. Він приймає додатковий параметр під назвою `separator`, який використовується для розділення кожного елемента масиву. Наприклад:
...
Зв’яжіть метод `join()` до методу `map()` і передайте порожній рядок для роздільника.

Щоб об’єднати кілька методів, ви можете викликати метод `join()` на результаті методу `map()`. Наприклад:
