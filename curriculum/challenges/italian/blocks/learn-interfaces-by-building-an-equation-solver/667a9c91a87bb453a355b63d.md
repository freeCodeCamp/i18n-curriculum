...
Sentiti libero di modificare i coefficienti del tuo `lin_eq` per vedere come cambia l'output.

Poi, elimina la chiamata a `print(solver(lin_eq))` e stampa il risultato della chiamata a `solver()` con `quadr_eq` come argomento.

# --hints--

Non dovresti avere `print(solver(lin_eq))` nel tuo codice.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

Dovresti stampare `solver(quadr_eq)`.
