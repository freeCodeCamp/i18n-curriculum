...
Вам також слід створити функцію, яка очищатиме поля введення.

Створіть `clearInputFields` функцію. Усередині неї використайте `querySelectorAll`, щоб отримати всі поля введення, і пройдіться по них циклом `forEach()`. Усередині `forEach()` встановіть значення кожного `input` у порожній рядок.

# --hints--

Вам слід створити `clearInputFields` функцію.

```js
assert.isFunction(clearInputFields)
```

Ваша `clearInputFields` функція має встановлювати кожне поле введення у порожній рядок.
