...
حدّث استدعاء الراجع في `.map()` لاستدعاء الدالة `Number()`. مرّر `dropdown.value` إلى هذا الاستدعاء.

افتح تبويب وحدة التحكم لترى أن `inputValues` أصبحت مصفوفة من الأرقام الآن.

# --hints--

يجب استدعاء الدالة `Number()` في استدعاء الراجع الخاص بـ `.map()`.
...
يجب تمرير `dropdown.value` إلى استدعاء `Number()`.
