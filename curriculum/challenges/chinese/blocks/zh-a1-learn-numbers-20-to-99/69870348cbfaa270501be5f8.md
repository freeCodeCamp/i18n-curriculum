...
现在你知道 11 到 19 的数字是由 `十 (shí)`（10）后面加上个位数字（1–9）组成的。相同的逻辑也适用于 21 到 29 的数字。

在中文里，数字 20 被称为 `二十 (èr shí)`。

要组成 21 到 29 的数字，只需取 `二十 (èr shí)`（20）并直接在后面添加个位数字（1–9）。
