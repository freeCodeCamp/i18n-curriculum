...
目前，每当用户编辑笔记并点击笔记外部时，`currentContent` 变量都会被更新。但如果用户聚焦于笔记后决定离开且没有进行任何编辑，该怎么办？

好吧，在这种情况下，你可以添加一个条件来处理那个使用分支。 

在你的 `currentContent = newContent;` 语句上方，添加一个 `if` 语句来查看 `currentContent` 是否相等于 `newContent`。如果是，则返回。

# --hints--

你应该添加一个 `if` 语句来查看 `currentContent` 是否与 `newContent` 相等。如果是，则返回。
