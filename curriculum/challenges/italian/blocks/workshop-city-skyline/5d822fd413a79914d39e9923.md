...
Dovresti dare a `.fb3a` una proprietà `background-color` con il valore `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

Dovresti dare a `.fb3b` una proprietà `background-color` con il valore `--building-color1`.
