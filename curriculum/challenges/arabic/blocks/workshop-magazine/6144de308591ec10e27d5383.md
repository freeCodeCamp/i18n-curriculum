...
أنشئ محدِّد `h1`، واضبط خاصية `font-family` على `Anton` مع النظام البديل `sans-serif`.
...
يجب أن يحتوي محدِّد `h1` على خاصية `font-family` مضبوطة على `Anton` مع النظام البديل `sans-serif`.
