...
Da mesma forma que você fez antes para o método `__eq__`, crie uma declaração `if` que verifique se `self` e `other` não pertencem à mesma classe e retorne `NotImplemented`.
