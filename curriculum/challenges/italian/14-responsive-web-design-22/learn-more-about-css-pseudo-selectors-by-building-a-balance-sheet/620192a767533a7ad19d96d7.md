...
Il selettore `tr.total td` dovrebbe avere una proprietà `text-align` con il valore `right`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td')?.getPropertyValue('text-align') === 'right');
```

Il selettore `tr.total td` dovrebbe avere una proprietà `padding` con il valore `0 0.25rem`.
