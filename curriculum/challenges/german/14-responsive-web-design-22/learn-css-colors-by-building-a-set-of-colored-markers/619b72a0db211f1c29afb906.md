...
Dein Code sollte keinen `.one`-Klassenselektor haben.

```js
assert(!new __helpers.CSSHelp(document).getStyle('.one'));
```

Du solltest einen Klassen-Selektor verwenden, um die Klasse `red` auszuwählen.

```js
assert(new __helpers.CSSHelp(document).getStyle('.red'));
```

Deine `.red`-CSS-Regel sollte eine `background-color`-Eigenschaft auf `rgb(0, 0, 0)` gesetzt haben.
