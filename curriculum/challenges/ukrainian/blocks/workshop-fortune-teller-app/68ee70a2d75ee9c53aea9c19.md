...
Всередині `map` поверніть функцію `renderCard`, передавши `type`, `isReversed`, `name_short` і `img` як аргументи, а також додайте метод `join` до `map` з порожнім рядком.

# --hints--

Ви повинні повернути функцію `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

Ви повинні передати `type` як перший аргумент у функцію `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

Ви повинні передати `isReversed` як другий аргумент у функцію `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?isReversed\s*/);
```

Ви повинні передати `card.name_short` як аргумент у функцію `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.name_short\s*/);
```

Ви повинні передати `card.img` як аргумент у функцію `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

Ви повинні приєднати `join("")` до методу `map`.
