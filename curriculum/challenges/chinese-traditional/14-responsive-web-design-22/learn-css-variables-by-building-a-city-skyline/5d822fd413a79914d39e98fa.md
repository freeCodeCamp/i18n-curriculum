...
Create a new variable in `:root` named `window-color2` with a value of `#8cd9b3`. 這將被用作該建築物的間色。

# --hints--

你應該在 `:root` 中創建一個名爲 `window-color2` 的新屬性變量。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--window-color2'));
```

你應該給 `window-color2` 一個值爲 `#8cd9b3`。
