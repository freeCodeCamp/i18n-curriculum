...
在你的第四個 `tr` 元素中，為你的 `th` 元素添加文字 `Total Liabilities`。將文字 `Liabilities` 包裹在一個 `span` 元素中，並將其 `class` 屬性設為 `sr-only`。

接著，添加三個帶有以下文字（依序為）：`$750`、`$600`、`$475` 的 `td` 元素。將第三個 `td` 元素的 `class` 屬性設為 `current`。
...
你的 `th` 元素應該有文字 `Total Liabilities`。
...
你應該將文字 `Liabilities` 包裹在 `span` 元素中。
