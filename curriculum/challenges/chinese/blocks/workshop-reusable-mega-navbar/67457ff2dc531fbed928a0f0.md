...
现在是使用 JSX 构建 `Navbar` 组件 的时候了。

你在之前的课程中学到，JSX 是 JavaScript 的一种语法扩展，它允许你在 JavaScript 中编写类似超文本标记语言的代码，从而更容易创建和管理 React 组件。
...
在 `Navbar` 组件内，返回一个具有 `className` 为 `navbar` 的 `nav` 元素。

这里使用 `className` 而不是 `class` 的原因是因为 `class` 是 JavaScript 中的保留关键字。所以，React 使用 `className` 作为为元素应用类的方式。
...
你的 `nav` 元素应具有值为 `navbar` 的 `className` 属性。
