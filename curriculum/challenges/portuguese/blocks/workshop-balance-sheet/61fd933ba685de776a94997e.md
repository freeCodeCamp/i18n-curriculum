...
Dentro do primeiro `tr`, adicione um elemento `th` com o texto `Loans The outstanding balance on our startup loan.`. Envolva esse texto, exceto `Loans `, dentro de um elemento `span` com a `class` definida como `description`.
...
Seu elemento `th` deve conter o texto `Loans The outstanding balance on our startup loan.`
