...
`bugMap` に2つのエントリーを追加してください。`"bee"` をキーとして新しい `Bee` インスタンスにマッピングし、`"spider"` をキーとして新しい `Spider` インスタンスにマッピングしてください。両方の引数として `bugEmojiElement` を渡してください。

# --hints--

`bugMap` は `"bee"` エントリーを `new Bee(bugEmojiElement)` に設定する必要があります。

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

`bugMap` は `"spider"` エントリーを `new Spider(bugEmojiElement)` に設定する必要があります。
