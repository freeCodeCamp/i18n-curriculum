...
IDが`bug-emoji`の段落要素を取得し、`bugEmojiElement`という名前の定数に格納してください。要素が`!`でないことを`null`の非nullアサーション演算子で保証してください。

# --hints--

`bugEmojiElement`という名前の定数があるはずです。
...
`bugEmojiElement`は、型引数に`bug-emoji`を使って`document.querySelector`でIDが`HTMLParagraphElement`の要素をクエリすべきです。

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

`bugEmojiElement`は非nullアサーション演算子（`!`）を使うべきです。
