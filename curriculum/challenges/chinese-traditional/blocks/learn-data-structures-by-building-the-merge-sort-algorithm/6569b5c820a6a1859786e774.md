...
在 `while` 迴圈中，將 `right_part[right_array_index]` 指定到 `array[sorted_index]`。

# --hints--

你應該在迴圈主體中有 `array[sorted_index] = right_part[right_array_index]`。不要忘記移除迴圈主體中的 `pass` 關鍵字。
