...
Ahora tienes que vincular el archivo `styles.css` para que los estilos se apliquen de nuevo. Dentro del elemento `head`, añade un elemento `link`. Dale un atributo `rel` con el valor de `"stylesheet"` y un atributo `href` con el valor de `"styles.css"`.
...
El elemento `link` es un elemento vacío, no debe tener una etiqueta de cierre `</link>`.
