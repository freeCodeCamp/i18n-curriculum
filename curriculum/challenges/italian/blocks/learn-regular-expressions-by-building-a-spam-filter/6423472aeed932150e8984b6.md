...
Usando la stessa sintassi, aggiorna `c` per farla corrispondere a `c`, `{`, `[` e `(`.

# --hints--

Il tuo `stockRegex` dovrebbe usare una classe di caratteri per far corrispondere `c`, `{`, `[` e `(`.
...
Il tuo `stockRegex` dovrebbe corrispondere a `570(k 4l3r7`.

```js
assert.match('570(k 4l3r7', stockRegex);
```

Il tuo `stockRegex` dovrebbe corrispondere a `sto{k alert`.
