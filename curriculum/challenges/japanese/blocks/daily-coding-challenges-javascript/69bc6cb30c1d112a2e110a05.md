...
整数の配列が与えられたとき、次のルールに従って新しい配列を返してください。

1. 整数を昇順にソートする
2. その後、インデックスが3の倍数であるすべての値を、その直前の値と交換する

# --hints--

`sortAndSwap([3, 1, 2, 4, 6, 5])` は `[1, 2, 4, 3, 5, 6]` を返すべきです。

```js
assert.deepEqual(sortAndSwap([3, 1, 2, 4, 6, 5]), [1, 2, 4, 3, 5, 6]);
```

`sortAndSwap([9, 7, 5, 3, 1, 2, 4, 6, 8])` は `[1, 2, 4, 3, 5, 7, 6, 8, 9]` を返すべきです。

```js
assert.deepEqual(sortAndSwap([9, 7, 5, 3, 1, 2, 4, 6, 8]), [1, 2, 4, 3, 5, 7, 6, 8, 9]);
```

`sortAndSwap([1, 2, 3, 4, 5, 6, 7, 8, 9])` は `[1, 2, 4, 3, 5, 7, 6, 8, 9]` を返すべきです。

```js
assert.deepEqual(sortAndSwap([1, 2, 3, 4, 5, 6, 7, 8, 9]), [1, 2, 4, 3, 5, 7, 6, 8, 9]);
```

`sortAndSwap([12, 5, 8, 1, 3, 10, 2, 7, 6, 4, 9, 11])` は `[1, 2, 4, 3, 5, 7, 6, 8, 10, 9, 11, 12]` を返すべきです。

```js
assert.deepEqual(sortAndSwap([12, 5, 8, 1, 3, 10, 2, 7, 6, 4, 9, 11]), [1, 2, 4, 3, 5, 7, 6, 8, 10, 9, 11, 12]);
```

`sortAndSwap([100, -50, 0, 75, -25, 50, -75, 25])` は `[-75, -50, 0, -25, 25, 75, 50, 100]` を返すべきです。

```js
assert.deepEqual(sortAndSwap([100, -50, 0, 75, -25, 50, -75, 25]), [-75, -50, 0, -25, 25, 75, 50, 100]);
```

`sortAndSwap([5, 9, 13, 77, 88, 313, -10, -65, 0, 8, 99, 101, -4, 2])` は `[-65, -10, 0, -4, 2, 8, 5, 9, 77, 13, 88, 101, 99, 313]` を返すべきです。
