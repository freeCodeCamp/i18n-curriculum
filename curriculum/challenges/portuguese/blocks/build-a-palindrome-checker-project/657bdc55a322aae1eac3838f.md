...
**Objetivo:** Construir um aplicativo que seja funcionalmente semelhante ao <a href="https://palindrome-checker.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://palindrome-checker.freecodecamp.rocks</a>.
...
Você deve ter um elemento `div`, `span` ou `p` com um `id` igual a `"result"`.
...
Quando você clicar no elemento `#check-btn` sem digitar um valor no elemento `#text-input`, um alerta deverá aparecer com o texto `Please input a value`.
...
Quando o elemento `#text-input` possuir o texto `My age is 0, 0 si ega ym.` e o elemento `#check-btn` for clicado, o elemento `#result` deverá conter o texto `My age is 0, 0 si ega ym. is a palindrome`.
