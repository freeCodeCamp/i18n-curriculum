...
Rufen Sie nun die `chooseShape()`-Funktion mit `val.value` auf, um die richtigen Eingabefelder anzuzeigen, und rufen Sie dann `updateResult()` auf, um den Bereich neu zu berechnen und anzuzeigen.

# --hints--

Sie sollten die `chooseShape`-Funktion mit `val.value` aufrufen.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

Sie sollten die `updateResult()`-Funktion aufrufen.
