...
Dentro do seu `if` statement, crie uma variável chamada `category` para armazenar a categoria de despesa.Atribua a ela uma chamada para `input()` e use `'Enter category: '` como argumento.

# --hints--

Você deve declarar uma variável chamada `category` na sua declaração `if`.
...
Você deve atribuir `input('Enter category: ')` à sua variável `category`.
