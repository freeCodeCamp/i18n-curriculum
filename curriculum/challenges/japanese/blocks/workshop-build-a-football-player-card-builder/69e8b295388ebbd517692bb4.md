...
`card` の中に、`div` の `className` を持つ `card-header` を追加してください。このセクションにはプレイヤーの評価、ポジション、ティアバッジ、クラブ名が入ります。

# --hints--

`div` の `className` を持つ `card-header` の中に、`div` の `className` を持つ `.card` を追加してください。
