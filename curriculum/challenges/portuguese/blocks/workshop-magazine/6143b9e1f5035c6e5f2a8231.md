...
- `V4 - 2016`: `Expandimos o currículo para 3 certificações, incluindo Frontend, Backend e Data Visualization. Cada uma tinha 10 projetos obrigatórios, mas apenas a seção Frontend tinha seus próprios desafios. Para as outras certificações, ainda estávamos usando recursos externos como Node School.`
- `V5 - 2017`: `Adicionamos os desafios de backend e visualização de dados.`
...
Seu quarto `p` deve conter o texto `We expanded the curriculum to 3 certifications, including Frontend, Backend, and Data Visualization. They each had 10 required projects, but only the Frontend section had its own challenges. For the other certs, we were still using external resources like Node School.`
...
Seu quinto `p` deve conter o texto `We added the backend and data visualization challenges.`
