...
Utahitaji kitendakazi kinachosasisha DOM kwa kweli. Tangaza kitendakazi cha `updateResultsList`.

Kuanza, kitendakazi kinapaswa kufuta HTML kutoka kwenye kipengele cha `resultList`.

# --hints--

Unapaswa kutangaza kitendakazi cha `updateResultsList`.

```js
assert.isFunction(updateResultsList);
```

Kitendakazi chako cha `updateResultsList` kinapaswa kuweka `resultList.innerHTML` kuwa mfuatano wa herufi tupu.
