...
Erstellen Sie einen `img`-Selektor und geben Sie ihm eine `width`-Eigenschaft mit dem Wert `100%` sowie eine `object-fit`-Eigenschaft mit dem Wert `cover`.

Die `object-fit`-Eigenschaft gibt dem Browser an, wie das Element innerhalb seines Containers positioniert wird. In diesem Fall sorgt `cover` dafür, dass das Bild den Container ausfüllt und bei Bedarf zugeschnitten wird, um das Seitenverhältnis nicht zu verändern.
...
Ihr `img`-Selektor sollte eine `object-fit`-Eigenschaft mit dem Wert `cover` haben.
