...
スパムメッセージはフィルターを回避するために文字の代わりに数字を使うことがよくあります。あなたの正規表現はこれらを検出する必要があります。

正規表現の`e`の文字を、`e`と`3`にマッチする文字クラスに置き換えてください。

# --hints--

`freeRegex`は文字クラスを使うべきです。

```js
assert.match(freeRegex.source, /\[.*\]/);
```

`freeRegex`は`e`と`3`にマッチする文字クラスを使うべきです。

```js
assert.match(freeRegex.source, /\[(e3|3e)\]/);
```

`freeRegex`は`e`と`3`にマッチする3つの文字クラスを使うべきです。
...
`freeRegex`は`fr33 mon3y`にマッチするべきです。
