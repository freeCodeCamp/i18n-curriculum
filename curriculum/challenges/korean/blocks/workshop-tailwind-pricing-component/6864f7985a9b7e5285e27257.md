...
다음으로, 유틸리티 클래스 `mt-6`, `space-y-2`, `text-sm`, 그리고 `text-gray-700`를 `ul`에 할당하세요.

# --hints--

`ul` 요소에 `mt-6` 클래스를 할당해야 합니다.
...
`ul` 요소에 `space-y-2` 클래스를 할당해야 합니다.
...
`ul` 요소에 `text-gray-700` 클래스를 할당해야 합니다.
