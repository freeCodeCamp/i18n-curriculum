...
Ora cambiamo il colore di una parte del nostro testo.

Possiamo farlo modificando la `style` del tuo elemento `h2`.
...
Nota che è buona pratica terminare le dichiarazioni di `style` in linea con un `;` .

# --instructions--

Modifica lo stile del tuo elemento `h2` in modo che il colore del testo sia rosso.
...
Il tuo elemento `h2` dovrebbe avere la proprietà color impostata su `red`.
