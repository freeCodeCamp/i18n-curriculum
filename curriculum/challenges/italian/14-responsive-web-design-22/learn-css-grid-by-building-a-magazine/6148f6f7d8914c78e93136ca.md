...
Complimenti! Adesso la tua rivista è completa.

# --hints--

Dovresti avere una nuova `@media` query per `only screen and (max-width: 420px)`. Questa dovrebbe essere l'ultima query nell'elenco di `@media` query.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

La nuova `@media` query dovrebbe avere un selettore `.hero-title`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText === '.hero-title');
```

Il selettore `.hero-title` dovrebbe avere una proprietà `font-size` impostata su `4.5rem`.
