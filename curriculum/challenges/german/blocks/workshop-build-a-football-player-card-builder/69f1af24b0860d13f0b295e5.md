...
Ersetzen Sie nun die verbleibenden fest codierten Zahlen durch die folgenden:
...
Sie sollten den fest codierten Wert `99` durch `DRI` mit `player.dri` ersetzen.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

Sie sollten den fest codierten Wert `41` durch `DEF` mit `player.def` ersetzen.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

Sie sollten den fest codierten Wert `99` durch `PHY` mit `player.phy` ersetzen.
