...
你需要获取当前播放的歌曲或播放列表中的第一首歌曲。 为此，创建一个 `song` 常量并使用 OR 运算符 (`||`) 将其设置为 `userData` 的当前歌曲 **或** `userData?.songs` 数组中的第一首歌曲。

不要忘记使用可选链接。

# --hints--

你不应修改现有的 `setPlayButtonAccessibleText` 及其内容。

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

你应该访问 `userData?.currentSong` 或 `userData?.songs[0]`。

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

你应该将 `userData?.currentSong || userData?.songs[0]` 分配给 `song` 常量。
