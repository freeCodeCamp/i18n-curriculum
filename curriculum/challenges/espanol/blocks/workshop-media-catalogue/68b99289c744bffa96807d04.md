...
Actualmente, tanto las películas como las series de televisión se muestran juntas cuando imprimes el catálogo. En los siguientes pasos, te encargarás de filtrar los elementos en dos categorías para mostrarlos por separado.

Dentro de la clase `MediaCatalogue`, crea un método llamado `get_movies` y haz que retorne una lista que contenga solo instancias de la clase padre `Movie` que se puedan encontrar en `self.items`.

Usa el enfoque que prefieras para hacerlo, pero recuerda usar la función `type` ya que `isinstance` no podrá discriminar entre clases padre e hijo.

# --hints--

Tu clase `MediaCatalogue` debería tener un método llamado `get_movies`.
...
Tu método `get_movies` debería devolver una lista que contenga todos los elementos en `self.items` que sean instancias de la clase `Movie`.
