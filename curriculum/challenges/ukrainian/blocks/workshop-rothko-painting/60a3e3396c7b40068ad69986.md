...
Створіть новий `div` зі значенням `class` рівним `three` безпосередньо під елементом `.two`. Це буде ваш третій прямокутник.

# --hints--

Ваші існуючі елементи `.one` і `.two` не повинні змінюватися.
...
Новий `div` має бути вкладений у ваш елемент `.canvas`.

```js
assert(document.querySelector('.canvas').children[2].tagName === 'DIV');
```

Новий `div` має йти після вашого елемента `.two`.

```js
assert(document.querySelector('.two').nextElementSibling.tagName === 'DIV');
```

Новий елемент `div` повинен мати `class` зі значенням `three`.
