...
`make_allowed_move()` hupokea `rod1` na `rod2` kama vigezo. Unahitaji marekebisho kidogo hapa. Badilisha kila tukio la `source` kuwa `rod1`.

# --hints--

Unapaswa kubadilisha kila tukio la `source` ndani ya kitendakazi cha `make_allowed_move()` kuwa `rod1`.
