...
`.fb4-window` 요소에 `width`의 `30%`, `height`의 `10%`, 그리고 `border-radius`의 `50%`를 주세요. 이렇게 하면 이 건물에 원형 창문이 만들어집니다.

# --hints--

`.fb4-window`에 `width`의 `30%`를 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

`.fb4-window`에 `height`의 `10%`를 주어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

`.fb4-window`에 `border-radius`의 `50%`를 주어야 합니다.
