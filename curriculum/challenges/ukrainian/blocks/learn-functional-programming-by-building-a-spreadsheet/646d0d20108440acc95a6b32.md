...
У вашій функції `elemValue` ви явно оголосили функцію з назвою `inner` і повернули її. Однак, оскільки ви використовуєте стрілковий синтаксис, ви можете неявно повертати функцію. Наприклад:

```js
const curry = soup => veggies => {};
```

`curry` — це функція, яка приймає параметр `soup` і повертає функцію, яка приймає параметр `veggies`. Використовуючи цей синтаксис, оновіть вашу функцію `addCharacters`, щоб вона повертала порожню функцію, яка приймає параметр `character2`.

# --hints--

Ваша функція `addCharacters` має використовувати неявне повернення.
...
Ваша функція `addCharacters` має повертати стрілкову функцію, яка має параметр `character2`.
...
Ваша внутрішня стрілкова функція має бути порожньою.
