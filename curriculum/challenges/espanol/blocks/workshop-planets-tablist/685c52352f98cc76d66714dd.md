...
Ahora es momento de establecer que la pestaña clickeada esté seleccionada.

Usa el método `setAttribute` en el `tab` para establecer el atributo `aria-selected` a `"true"`.

# --hints--

Cuando se hace clic en una `tab`, el atributo `aria-selected` de la pestaña clickeada debe establecerse en `"true"`.
