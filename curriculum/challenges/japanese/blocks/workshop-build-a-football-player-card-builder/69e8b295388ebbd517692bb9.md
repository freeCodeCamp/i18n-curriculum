...
カードのヘッダーは完成しました。次にプレイヤーの写真セクションを追加する必要があります。

`div` の閉じる `card-header` の後で、`div` の `className` を持つ `card` の中に、`div` の `className` を持つ `card-image-wrap` を追加してください。今は空のままにしておきます。画像は次のステップで追加します。

# --hints--

`div` の `className` を持つ `card-image-wrap` を追加してください。
