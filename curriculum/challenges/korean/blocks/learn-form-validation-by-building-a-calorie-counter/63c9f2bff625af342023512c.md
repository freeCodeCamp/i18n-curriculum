...
이 프로젝트를 완료하려면 `clearButton` 버튼에 이벤트 리스너를 추가하세요. 버튼을 클릭하면 `clearForm` 함수를 호출해야 합니다.

축하합니다! 프로젝트가 완료되었습니다.

# --hints--

`clearButton` 버튼에 이벤트 리스너를 추가해야 합니다.

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(/);
```

`"click"`에 첫 번째 인수로 `.addEventListener()`를 전달해야 합니다.

```js
assert.match(code, /clearButton\s*\.addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

`clearForm`에 두 번째 인수로 `.addEventListener()`를 전달해야 합니다.
