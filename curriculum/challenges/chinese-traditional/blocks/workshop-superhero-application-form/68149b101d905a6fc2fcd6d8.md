...
在解構的 `value` 和 `checked` 下面，更新已勾選能力的列表。

呼叫 `setPowers` 函式（程式），該函式接受一個陣列引數。使用三元運算子檢查 `checked` 是否為 true。如果是，將現有的 `powers` 陣列展開到一個新陣列中，並將 `value` 添加到其中。如果不是，則使用 `powers.filter(p => p !== value)` 從 `powers` 中過濾掉 `value`。

# --hints--

你應該使用三元運算子來檢查 `checked` 是否為 true。如果是，將現有的 `powers` 和 `value` 展開到一個陣列中。如果不是，則從 `powers` 中過濾掉 `value`。
