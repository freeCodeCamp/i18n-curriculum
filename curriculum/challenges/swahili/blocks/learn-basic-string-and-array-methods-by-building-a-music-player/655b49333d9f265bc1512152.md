...
Tambua kwamba picha ya albamu katika HTML na nyimbo katika safu ya data ya `userData.songs` zimebadilika. Tumebadilisha nyimbo za awali kwa nyimbo fupi ambazo unaweza kutumia kujaribu programu yako katika hatua zinazofuata.

Ifuatayo, unahitaji kuangalia kama kuna wimbo unaofuata wa kuchezwa. Pata kielezo cha wimbo wa sasa kwa kufanya wito wa kitendakazi cha `getCurrentSongIndex()`, na uhifadhi katika thabiti ya `currentSongIndex`.

Baadaye, tengeneza thabiti ya `nextSongExists` inayoshikilia thamani ya kweli au siyo kweli ya `true` au `false` kulingana na kama wimbo unaofuata upo.

# --hints--

Haupaswi kubadilisha msikilizaji wa tukio wa `ended` uliopo na yaliyomo ndani yake.

```js
assert.match(code, /audio\.addEventListener\(\s*('|")ended\1\s*,\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Unapaswa kuunda thabiti ya `currentSongIndex` na kuiweka sawa na wito wa kitendakazi cha `getCurrentSongIndex`.
...
Unapaswa kuangalia kama kuna wimbo unaofuata kwa kulinganisha `userData.songs.length` na `currentSongIndex` na kuiweka katika thabiti ya `nextSongExists`. Ikiwa kielezo cha mwisho cha safu ya data ya nyimbo (`userData.songs.length - 1`) ni kikubwa kuliko currentSongIndex, hiyo ina maana kuna wimbo unaofuata.
