...
스팸 메시지는 필터를 우회하기 위해 종종 문자가 아닌 숫자를 사용합니다. 정규 표현식이 이를 잡아내야 합니다.

정규 표현식에서 `e` 문자를 `e`과 `3`에 매치되는 문자 클래스로 바꾸세요.

# --hints--

`freeRegex`는 문자 클래스를 사용해야 합니다.

```js
assert.match(freeRegex.source, /\[.*\]/);
```

`freeRegex`는 `e`와 `3`에 매치되는 문자 클래스를 사용해야 합니다.

```js
assert.match(freeRegex.source, /\[(e3|3e)\]/);
```

`freeRegex`는 `e`과 `3`에 매치되는 세 개의 문자 클래스를 사용해야 합니다.

```js
assert.lengthOf(freeRegex.source.match(/\[(e3|3e)\]/g), 3);
```

`freeRegex`는 `free money`에 매치되어야 합니다.

```js
assert.equal("free money".match(freeRegex)?.[0], "free money");
```

`freeRegex`는 `fr33 mon3y`에 매치되어야 합니다.
