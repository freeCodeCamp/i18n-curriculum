...
يجب تنسيق العنوان الفرعي أيضًا. أنشئ محدِّد `.hero-subtitle` وأعطه خاصية `font-size` بقيمة `2.4rem`، وخاصية `color` بقيمة `orangered`، وخاصية `text-align` بقيمة `center`.

# --hints--

يجب أن يكون لديك محدِّد `.hero-subtitle`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

يجب أن يحتوي محدِّد `.hero-subtitle` على خاصية `font-size` بقيمة `2.4rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

يجب أن يحتوي محدِّد `.hero-subtitle` على خاصية `color` بقيمة `orangered`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

يجب أن يحتوي محدِّد `.hero-subtitle` على خاصية `text-align` بقيمة `center`.
