...
Para resolver esto, puedes proporcionar contexto de lo que se necesita agregando un elemento `legend` con el texto `Account type (required)` antes de los elementos `label` dentro del segundo `fieldset`. Luego agrega el atributo `checked` a la entrada `Personal` para asegurar que el formulario se envíe con los datos requeridos.
...
Debes agregar el texto `Account type (required)` al elemento `legend`.
