...
Verkette die `.then()`-Methode mit deinem `fetch`-Aufruf. Füge innerhalb der `.then()`-Methode eine Callback-Funktion mit `res` als Parameter hinzu und protokolliere dann `res` auf der Konsole, damit du das `Response`-Objekt sehen kannst. Open your browser console and expand the `Response` object to see what it contains.
