...
Si miras el HTML, verás que el segundo y tercer panel están ocultos. Esto se hace usando el atributo `hidden`. El primer panel es visible por defecto. Cuando un usuario hace clic en una pestaña, todos los paneles deben estar ocultos.

Usa el método `forEach` para recorrer los `panels` y establecer el atributo `hidden` en cada panel a `true`. 

# --hints--

Debes recorrer los `panels` y establecer el atributo `hidden` de cada panel en `true`.
