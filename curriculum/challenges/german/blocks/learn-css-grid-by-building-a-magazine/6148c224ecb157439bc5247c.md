...
Die Standardeinstellungen für CSS Grid erzeugen bei Bedarf zusätzliche Zeilen, im Gegensatz zu Flexbox. Geben Sie dem `.social-icons`-Selektor eine `grid-template-columns`-Eigenschaft mit dem Wert `repeat(5, 1fr)`, um die Symbole in einer einzigen Reihe anzuordnen.

# --hints--

Ihr `.social-icons`-Selektor sollte eine `grid-template-columns`-Eigenschaft mit dem Wert `repeat(5, 1fr)` haben.
