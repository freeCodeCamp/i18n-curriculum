...
Per gli elementi `span` all'interno di ogni elemento della lista, assegna loro le classi `text-green-500` e `mr-2` per rendere i marker verdi e distanziarli un po' dal testo.

Inoltre, assegna loro l'attributo `aria-hidden` come hai fatto per gli elementi `span` del primo piano tariffario.

# --hints--

I tuoi elementi `span` dovrebbero avere la classe `text-green-500`.
...
I tuoi elementi `span` dovrebbero avere la classe `mr-2`.
...
I tuoi elementi `span` dovrebbero avere l'attributo `aria-hidden`.
