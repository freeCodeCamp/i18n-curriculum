...
生成自引用序列有几种方法。一种非常常见的方法（<a href="https://rosettacode.org/wiki/Look-and-say_sequence" target="_blank" rel="noopener noreferrer nofollow">Look-and-say sequence</a>）是从一个正整数开始，然后通过连接相邻相同数字的枚举组来生成下一个项：
...
根据种子值，以这种方式生成的串行总是收敛到一个稳定值或一个短周期模式。（对于我们的目的，收敛意味着一个元素匹配之前见过的元素。）所示的序列，种子值为 0，在 11 次迭代后收敛到稳定值 1433223110。收敛最快的种子值是 22。它在第一个元素后变得稳定。（下一个元素是 22，之前已经见过。）
