...
يمكنك الآن بناء دالة لمحاكاة المهرجان. أنشئ دالة فارغة باسم `simulateFestival` بمعلمتين `gates` و `timeBlock`.

# --hints--

يجب أن تنشئ دالة باسم `simulateFestival`.

```js
assert.isFunction(simulateFestival);
```

يجب أن تحتوي دالتك `simulateFestival` على المعلمتين `gates` و `timeBlock`.
