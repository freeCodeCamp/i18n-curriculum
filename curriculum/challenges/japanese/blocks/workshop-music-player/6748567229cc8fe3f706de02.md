...
現在再生中の曲を一時停止する処理を作成してください。

パラメータを取らない`pauseSong`関数を定義してください。新しい関数内で、曲を一時停止したときの現在の再生時間を保存するために、`songCurrentTime`変数の`userData`を`currentTime`オブジェクトの`audio`に設定してください。


# --hints--

`pauseSong`関数を作成しているはずです。

```js
assert.isFunction(pauseSong);
```

`pauseSong`関数は、`songCurrentTime`変数の`userData`を`currentTime`オブジェクトの`audio`に設定する必要があります。
