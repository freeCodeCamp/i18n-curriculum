...
Dovresti dare all'elemento `.penguin` una proprietà `z-index` con il valore `4`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.zIndex, '4');
```

Dovresti dare all'elemento `.penguin` una proprietà `position`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.penguin')?.position);
```

Dovresti dare all'elemento `.penguin` una proprietà `position` con il valore `relative`.
