---
id: 5900f3931000cf542c50fea5
title: 'Завдання 38: пандигітальні кратні'
challengeType: 1
forumTopicId: 302042
dashedName: problem-38-pandigital-multiples
---

# --description--

Візьміть число 192 і помножте його на кожне з чисел 1, 2 та 3:

$$\begin{align}
  192 × 1 = 192\\\\
  192 × 2 = 384\\\\
  192 × 3 = 576\\\\
\end{align}$$

Об’єднавши кожен добуток у рядок, отримаємо пандигітальне число від 1 до 9 — 192384576. Назвемо 192384576 конкатенованим добутком числа 192 та (1, 2, 3).

Те саме можна зробити, починаючи з 9 і помноживши на 1, 2, 3, 4 та 5, отримуючи пандигітальне число 918273645, яке є конкатенованим добутком 9 та (1, 2, 3, 4, 5).

Яке найбільше пандигітальне число від 1 до `k` `k`-цифрове число, яке можна утворити як конкатенований добуток цілого числа з (1, 2, ..., `n`), де `n` > 1?

# --hints--

`pandigitalMultiples(8)` має повертати число.

```js
assert(typeof pandigitalMultiples(8) === 'number');
```

`pandigitalMultiples(8)` має повертати `78156234`.

```js
assert.strictEqual(pandigitalMultiples(8), 78156234);
```

`pandigitalMultiples(9)` має повертати `932718654`.

```js
assert.strictEqual(pandigitalMultiples(9), 932718654);
```

# --seed--

## --seed-contents--

```js
function pandigitalMultiples(k) {

  return true;
}

pandigitalMultiples(8);
```

# --solutions--

```js
function pandigitalMultiples(k) {
  function getKDigitConcatenatedProduct(num, k) {
    // returns false if concatenated product is not k digits
    let concatenatedProduct = num.toString();
    for (let i = 2; concatenatedProduct.length < k; i++) {
      concatenatedProduct += num * i;
    }
    return concatenatedProduct.length === k ? concatenatedProduct : false;
  }

  function is1toKPandigital(num, k) {
    const numStr = num.toString();

    // check if length is not k
    if (numStr.length !== k) {
      return false;
    }

    // check if pandigital
    for (let i = k; i > 0; i--) {
      if (numStr.indexOf(i.toString()) === -1) {
        return false;
      }
    }
    return true;
  }

  let largestNum = 0;
  for (let i = 10 ** Math.floor(k / 2) + 1; i >= 1; i--) {
    const concatenatedProduct = getKDigitConcatenatedProduct(i, k);
    if (is1toKPandigital(concatenatedProduct, k)) {
      const number = parseInt(concatenatedProduct, 10);
      if (number > largestNum) {
        largestNum = number;
      }
    }
  }
  return largestNum;
}
```
