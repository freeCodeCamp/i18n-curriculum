...
El texto de la descripción necesita ser un poco más grande. Selecciona el elemento con la clase `description` y establece su `font-size` en `1.2rem`.

# --hints--

Debes apuntar al elemento con la clase `description`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.description'));
```

Establece la propiedad `font-size` en `1.2rem`.
