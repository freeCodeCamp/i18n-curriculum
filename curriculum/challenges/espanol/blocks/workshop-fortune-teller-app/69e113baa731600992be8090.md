...
Crea una variable `CDN_URL` y asígnale el valor `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. También crea una variable `defaultImg` con el valor `new URL("default.svg", import.meta.url)`, y una variable `LOCAL_DEFAULT_IMG` con el valor `defaultImg`.
...
Tu variable `defaultImg` debe asignarse a `new URL("default.svg", import.meta.url)`.
