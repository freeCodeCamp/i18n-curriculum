...
Debes tener un selector `footer`.
...
Tu selector `footer` debe estar debajo de tu comentario.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

La propiedad `font-size` debe tener un valor dé `14px`.
...
Tú `footer` debe tener un tamaño de fuente (`font-size`) dé `14px`.
