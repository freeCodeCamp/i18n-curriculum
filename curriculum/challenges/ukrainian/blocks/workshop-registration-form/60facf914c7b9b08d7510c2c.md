...
`textarea` здається занадто маленьким. Щоб задати йому ініціальний розмір, ви можете додати атрибути `rows` та `cols`.

Додайте ініціальний розмір у `3` рядки та `30` стовпці.

# --hints--

Ви повинні додати `textarea` атрибут `rows` зі значенням `3`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.rows, 3);
```

Ви повинні додати `textarea` атрибут `cols` зі значенням `30`.
