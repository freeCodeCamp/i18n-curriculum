...
Solo quieres limpiar el carrito si el usuario confirma el mensaje. Crea una declaración `if` que verifique si el usuario confirmó el mensaje.

En la declaración `if`, establece la propiedad `items` de nuevo como un array vacío, luego establece la propiedad `total` a `0`.

# --hints--

Debes crear una declaración `if` que verifique si el usuario confirmó el mensaje. Recuerda que puedes verificar directamente la veracidad de `isCartCleared`.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

La declaración `if` debe establecer la propiedad `items` de nuevo como un array vacío.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

La declaración `if` debe establecer la propiedad `total` a `0`.
