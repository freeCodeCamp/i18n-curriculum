...
Ваш селектор `.first-paragraph::first-letter` повинен мати властивість `float` зі значенням `left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float === 'left');
```

Ваш селектор `.first-paragraph::first-letter` повинен мати властивість `margin-right` зі значенням `1rem`.
