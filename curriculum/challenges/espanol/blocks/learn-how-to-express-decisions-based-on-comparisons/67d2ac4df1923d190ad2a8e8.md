...
This word is often used at the beginning of a sentence to introduce a response or explanation. La primer letra se escribe en mayúscula.
...
`Depends on` means that something is influenced by another factor. Por ejemplo:
...
`Looking for` means searching for something specific. Por ejemplo:
