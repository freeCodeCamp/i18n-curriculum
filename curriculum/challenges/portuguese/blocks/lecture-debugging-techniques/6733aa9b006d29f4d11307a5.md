...
Um `RangeError` acontece quando seu código tenta usar um valor que está fora do intervalo que o JavaScript pode manipular. Aqui está um exemplo de atribuição de um valor inválido ao comprimento de um array:
...
Como o `length` de um array precisa ser um inteiro não negativo, defini-lo como `-1` gera um `RangeError`.
...
Qual erro ocorre quando você atribui um valor inválido, como `-1`, ao `length` de um array?
