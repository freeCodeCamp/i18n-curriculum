...
現在創建一個 `fightDragon` 函數，將 `"Fighting dragon."` 打印到控制檯。

# --hints--

你應該使用 `function` 關鍵字來聲明 `fightDragon`。
...
你的代碼中應該有一個 `console.log("Fighting dragon.");` 行。

```js
assert.match(code, /console\.log\(\s*('|")Fighting dragon\.\1\s*\)/);
```

你的 `console.log("Fighting dragon.");` 行應該位於你的 `fightDragon` 函數內。
