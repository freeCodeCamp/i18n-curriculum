...
プレイヤーから体力を引いたので、プレイヤーの`health`が`0`以下かどうかを確認する必要があります。もしそうなら、`lose`関数を呼び出してください。

# --hints--

あなたの`pick`関数には、`if`の本体にネストされた`else`文があるはずです。
...
あなたの`if`文は、プレイヤーの`health`が`0`以下かどうかをチェックする必要があります。

```js
assert.match(pick.toString(), /if\s*\(\s*health\s*<=\s*0\s*\)/);
```

`if`の本体にある`else`文は、`lose`関数を呼び出すべきです。
