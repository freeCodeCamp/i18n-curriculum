...
Вам потрібна ще одна функція, яка показує елементи в DOM. Подібно до останньої функції, створіть функцію з іменем `showElements`. Цього разу використайте `classList.remove("hidden")`.

# --hints--

У вас має бути функція з іменем `showElements`.
...
Ваша функція `showElements` має мати параметр з іменем `...elements` типу `HTMLElement[]`.
...
У вашій функції `showElements` має бути приєднаний метод `forEach` до параметра `elements`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Ваш метод `forEach` має приймати параметр з іменем `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Ви маєте видалити клас `hidden` за допомогою методу `classList.remove` всередині методу `forEach`.
