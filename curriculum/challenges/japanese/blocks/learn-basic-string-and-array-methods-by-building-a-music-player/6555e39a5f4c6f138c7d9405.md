...
前のステップでは、`sort()` メソッドを使って曲をアルファベット順に並べ替える方法を学びました。コールバック関数のもう一つの使い方は、配列をランダムに並べ替えることです。

配列の要素をランダムに並べ替える一つの方法は、`0.5` から `Math.random()` を引くことで、正または負のランダムな値を生成することです。これにより比較結果が正負の値の混合となり、要素がランダムに並ぶことになります。
...
`sort()` 配列に対して `userData?.songs` メソッドを使ってください。メソッドにコールバックを渡し、`Math.random() - 0.5` の結果を返してください。


# --hints--

既存の `shuffle` 関数は変更しないでください。

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*[\s\S]*?\}\s*;?/);
```

`sort()` に対して `userData?.songs` メソッドを使う必要があります。

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(/);
```

`sort()` メソッドのコールバック関数はアロー構文を使ってください。

```js
assert.match(code, /const\s+shuffle\s*=\s*\(\s*\)\s*=>\s*{\s*userData\?\.songs\.sort\(\s*\(\s*\)\s*=>\s*/);
```

`sort()` メソッドのコールバックは `Math.random() - 0.5` を返す必要があります。
