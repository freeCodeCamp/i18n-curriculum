...
接下來，將 `RenderInput` 添加到 `MyApp` 中的 render 方法中，然後創建一個名爲 `input` 的 prop，並將 `state` 中的 `inputValue` 傳遞給它。 完成後，可以在 `GetInput` 組件中的 `input` 字段中鍵入內容，然後該組件通過 props 調用其父組件中的處理函數方法。 這將更新處於父組件 `state` 中的 input，該 input 將作爲 props 傳遞給兩個子組件。 觀察數據如何在組件之間流動，以及單一數據源如何保持父組件`state`。 誠然，這個示例有點刻意，但是應該能用來說明數據和回調是如何在 React 組件之間傳遞的。
