...
`button` 要素の中に、さらに2つの `span` 要素を作成してください。

最初の `span` 要素は、`class` の `playlist-song-artist` を持つべきです。`span` タグの間に `${song.artist}` を追加してください。

2つ目の `span` 要素は、`class` の `playlist-song-duration` を持つべきです。`span` タグの間に `${song.duration}` を追加してください。

# --hints--

既存の `button` 要素とその内容は変更しないでください。
...
クラス `span` を持つ `playlist-song-artist` 要素を作成してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

クラス `span` を持つ `playlist-song-artist` 要素は、内容として `song.artist` を挿入してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

クラス `span` を持つ `playlist-song-duration` 要素を作成してください。

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

クラス `span` を持つ `playlist-song-duration` 要素は、内容として `song.duration` を挿入してください。
