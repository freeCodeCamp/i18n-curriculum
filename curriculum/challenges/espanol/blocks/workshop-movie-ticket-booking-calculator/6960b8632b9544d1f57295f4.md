...
Usa el operador `and` para construir una expresión que verifique si `age` es mayor o igual a `18` y `show_time` no es `Evening`. Usa el operador `or` para combinarlo con la condición existente del último `if`. No crees nuevas variables. 

# --hints--

La última condición de tu sentencia `if` debería ser `age >= 21 or age >= 18 and show_time != 'Evening'`.
