...
Відправлення форми з вибраним варіантом не надішле корисного значення на сервер. Через це кожен `option` елемент потребує атрибуту `value`. Без цього атрибуту текстовий вміст `option` буде відправлено на сервер.

Дайте першому `option` атрибут `value` зі значенням `""`, а наступним `option` елементам — атрибути `value` зі значеннями від `1` до `4`.

# --hints--

Ви повинні дати першому `option` атрибут `value` зі значенням `""`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

Ви повинні дати другому `option` атрибут `value` зі значенням `1`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

Ви повинні дати третьому `option` атрибут `value` зі значенням `2`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

Ви повинні дати четвертому `option` атрибут `value` зі значенням `3`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

Ви повинні дати п’ятому `option` атрибут `value` зі значенням `4`.
