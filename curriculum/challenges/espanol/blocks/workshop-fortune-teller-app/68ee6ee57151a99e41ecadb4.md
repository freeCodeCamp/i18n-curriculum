...
Ahora llama a la función `showElements` y pasa `multipleCard`, `fortuneContainer` y `text` como propiedades.

# --hints--

Debes llamar a la función `showElements` dentro del método `multipleCardSelected`.
...
Debes pasar `multipleCard` como primer argumento a la función `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

Debes pasar `fortuneContainer` como argumento a la función `showElements`.
...
Debes pasar `text` como argumento a la función `showElements`.
