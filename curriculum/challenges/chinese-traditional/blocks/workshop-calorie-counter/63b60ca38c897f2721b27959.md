...
你的表單需要一個地方來顯示結果。為你的表單添加一個空的 `div` 元素，並給它一個 `id` 為 `output`，以及 `class` 值為 `output` 和 `hide`。

# --hints--

你應該在你的 `form` 後面添加一個 `div` 元素。

```js
assert.exists(document.querySelector('.container > div'));
```

你的新 `div` 元素應該設定一個 `id` 為 `output`。

```js
assert.equal(document.querySelector('.container > div')?.id, 'output');
```

你的新 `div` 元素應該具有 `output` 和 `hide` 的 `class` 值。
