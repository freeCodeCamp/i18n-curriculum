...
Porqué `elemValue` devuelve una función, su función `addCharacters` finalmente devuelve un array de referencias de funciones. Desea que el método `.map()` ejecute la función interna de su función `elemValue`, lo que significa que necesita llamar a `elemValue` en lugar de referenciarla. Pase `num` como argumento a su función `elemValue`.

<!-- TODO: Explain further? -->

# --hints--

Debe llamar a `elemValue()` en su método `.map()`.
...
Debe pasar `num` a su llamada `elemValue()`.
