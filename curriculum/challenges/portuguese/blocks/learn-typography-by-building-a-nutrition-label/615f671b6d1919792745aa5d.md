...
Crie um elemento `div` abaixo do elemento `.medium`.Dê a ele um atributo `class` definido como `daily-value small-text`.Dentro dessa nova `div`, adicione um elemento `p` com o texto `% Daily Value *` e defina o atributo `class` como `bold right`.

# --hints--

Você deve criar um elemento `div` após o elemento `.medium`.
...
A nova `div` deve ter o atributo `class` definido como `daily-value small-text`.
