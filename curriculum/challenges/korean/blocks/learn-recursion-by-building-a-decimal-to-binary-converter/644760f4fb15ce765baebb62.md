...
숫자 입력이 비어 있거나 값이 숫자가 아니면 사용자에게 알린 후, `return` 키워드를 사용해 이 함수에서 조기에 빠져나올 수 있습니다. 이렇게 하면 이 함수 내의 이후 코드가 실행되지 않습니다.

`return` 다음에 `alert()` 키워드를 추가하세요.

# --hints--

`return` 다음에 `alert()` 키워드를 사용해야 합니다.
