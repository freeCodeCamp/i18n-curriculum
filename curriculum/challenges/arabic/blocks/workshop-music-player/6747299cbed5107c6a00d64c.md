...
تمت إضافة بقية الأغاني إلى مصفوفة `allSongs` من أجلك.

في الدروس السابقة، تعلّمت عن <dfn>واجهة برمجة تطبيقات الصوت في الويب</dfn> وكيفية استخدامها لتشغيل الأغاني. جميع المتصفحات الحديثة تدعم هذه الواجهة، التي تتيح لك توليد ومعالجة الصوت في تطبيقات الويب.

عرّف متغيرًا باسم `audio` وعيّنه بقيمة `new Audio()`. هذا سينشئ عنصر `audio` جديد من HTML5.

# --hints--

يجب أن يكون لديك متغير باسم `audio`.

```js
assert.isDefined(audio);
```

يجب تعيين نسخة جديدة من كائن `Audio` إلى المتغير المسمى `audio`.
