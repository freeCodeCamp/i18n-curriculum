...
Si la lista de reproducción está vacía, necesita crear un `resetButton` elemento y un texto para ella. Este botón solo aparecerá si la lista de reproducción está vacía.

`createElement()` es un método DOM que puede usar para crear dinámicamente un elemento utilizando JavaScript. Para usar `createElement()`, llámalo y luego pase el nombre de la etiqueta como una cadena:
...
Dentro de tu instrucción `if`, declara una constante `resetButton`, luego use `createElement()` para crear un `"button"`.


# --hints--

No debes modificar la declaración `if` existente y su contenido.

```js
assert.match(code, /if\s*\(\s*userData\?\.songs\.length\s*===\s*0\s*\)\s*\{\s*.*\s*\}/)
```

Debe usar el método `createElement` para crear un `"button"`.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

Debe asignar su elemento `button` recién creado a una constante `resetButton`.
