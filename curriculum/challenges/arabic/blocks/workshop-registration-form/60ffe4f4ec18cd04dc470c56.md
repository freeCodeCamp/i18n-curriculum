...
انقل خاصية وقيمة `min-height` بحيث تحصل الأنواع الثلاثة من العناصر على نفس قيمة `min-height`، ويظل عنصر `select` بخلفية بيضاء.

# --hints--

يجب أن تنقل خاصية وقيمة `min-height` إلى محدِّد `input, textarea, select`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight);
```

يجب أن تعطي محدِّد `input, textarea, select` قيمة `min-height` تساوي `2em`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight, '2em');
```

يجب أن تزيل إعلان `min-height` من محدِّد `input, textarea`.
