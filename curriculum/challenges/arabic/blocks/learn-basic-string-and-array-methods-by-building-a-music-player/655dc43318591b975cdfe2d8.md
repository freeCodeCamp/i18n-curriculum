...
يجب أن تحتوي مصفوفة `allSongs` على كائن بخاصية `id` قيمتها العدد `0`.

```js
assert.equal(allSongs[0].id, 0);
```

يجب أن تحتوي مصفوفة `allSongs` على كائن بخاصية `title` قيمتها السلسلة النصية `"Scratching The Surface"`.

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

يجب أن تحتوي مصفوفة `allSongs` على كائن بخاصية `artist` قيمتها السلسلة النصية `"Quincy Larson"`.

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

يجب أن تحتوي مصفوفة `allSongs` على كائن بخاصية `duration` قيمتها السلسلة النصية `"4:25"`.

```js
assert.equal(allSongs[0].duration, "4:25");
```

يجب أن تحتوي مصفوفة `allSongs` على كائن بخاصية `src` قيمتها السلسلة النصية `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`.
