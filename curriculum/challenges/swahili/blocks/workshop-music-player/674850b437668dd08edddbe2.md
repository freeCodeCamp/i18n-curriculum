...
Katika hatua zilizopita uliyajenga utendaji wa kuimba wimbo. Sasa unahitaji kuongeza utendaji kwa kitufe cha kucheza.

Tumia njia ya `addEventListener()` kwenye `playButton` na pita tukio la `"click"` kama hoja ya kwanza. Kwa sasa, tumia wito wa kurudisha unaoitisha `playSong(0)` kama hoja ya pili.

Kisha, jaribu kwa kubofya kitufe cha kucheza.


# --hints--

Unapaswa kuongeza msikilizaji wa tukio la `click` kwa `playButton` na pita kitendakazi cha mwitiko kinachoita `playSong(0)` kama hoja ya pili kwa njia ya `addEventListener`.
