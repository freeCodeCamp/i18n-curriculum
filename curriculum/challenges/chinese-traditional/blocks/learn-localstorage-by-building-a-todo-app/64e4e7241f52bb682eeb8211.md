...
你需要從 HTML 檔案中取得的最後一組元素是 `date-input` 和 `description-input` 元素。分別將它們存入變數 `dateInput` 和 `descriptionInput`。

# --hints--

你應該使用 `getElementById()` 來存取 `date-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)date\-input\1\s*\)\s*;?/)
```

你應該將 `date-input` 元素指定給變數 `dateInput`。別忘了使用 `const` 來宣告該變數。

```js
assert.match(code, /const\s+dateInput\s*=\s*document\.getElementById\(\s*('|"|`)date\-input\1\s*\)\s*;?/)
```

你應該使用 `getElementById()` 來存取 `description-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)description\-input\1\s*\)\s*;?/)
```

你應該將 `description-input` 元素指定給變數 `descriptionInput`。別忘了使用 `const` 來宣告變數。
