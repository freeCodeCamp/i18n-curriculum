...
現在將 `PlayerCard` 組件包裹在 `main` 元素中，該元素屬於 `className` 的 `main`。

在 `main` 元素內，添加 `div`，它是 `className` 的 `layout`。在 `layout` 中，分別添加兩個獨立的 `div` 元素，分別屬於 `className` 的 `form-panel` 和 `preview-panel`。

# --hints--

你的 `FootballPlayerCard` 應該算繪一個 `main` 元素，該元素內有 `className`，它是 `main` 的 `div.page`。
...
你的 `main.main` 應該包含一個 `div`，該元素有 `className`，它是 `layout`。
...
你的 `layout` 應該包含一個 `div`，該元素有 `className`，它是 `form-panel`。
...
你的 `layout` 應該包含一個 `div`，該元素有 `className`，它是 `preview-panel`。
