...
画面幅が狭いとき、`h1` はテキストコンテンツを正しく折り返しません。`h1` 要素のテキストを中央揃えにしてください。

次に、`main` セクションヘッダーが完全に見えるように、`Student Info` にパディングを付けてください。

# --hints--

`h1` に `text-align` の `center` を付ける必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

`main` 要素を対象にするために `main` セレクターを追加してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

`main` に少なくとも `padding-top` の `25px` を付ける必要があります。
