...
現在是時候驗證搜尋功能是否正常。Stephen King 在目錄中有六本書。

呼叫 `findByAuthor(catalog, "king")` 並將結果指定給名為 `kingBooks` 的變數。將 `kingBooks.length` 記錄到主控台以確認數量。

接著使用 `for` 迴圈來記錄每個結果。使用模板字串包含書名和年份，格式如 `` `${kingBooks[i].title} (${kingBooks[i].year})` ``。
