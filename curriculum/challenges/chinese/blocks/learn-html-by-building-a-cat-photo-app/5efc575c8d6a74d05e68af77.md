...
你的新复选框应该有一个值为 `energetic` 的 `id` 属性和一个值为 `personality` 的 `name` 属性。 确保在开始标签的名称后面有一个空格，且所有的属性名称前面也要有空格。
...
新的 `label` 应该有一个 `for` 属性，其值与新复选框的 `id` 属性值相同。 你要么遗漏了属性值，要么拼写有误。
