...
Lassen Sie die `h3`-Elemente mit den folgenden CSS-Eigenschaften eine höhere Priorität erhalten:
...
Sie sollten den `h3`-Elementselektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3'));
```

Sie sollten dem `h3`-Element eine `margin-top` von `5px` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Sie sollten dem `h3`-Element eine `padding-left` von `15px` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Sie sollten dem `h3`-Element eine `font-size` von `1.375ren` geben.
