...
È ora di iniziare a implementare la funzionalità per riprodurre le canzoni visualizzate.

Definisci una funzione `playSong` usando `const`. La funzione dovrebbe prendere un parametro `id` che rappresenterà l'identificatore univoco della canzone che vuoi riprodurre.

# --hints--

Dovresti usare `const` per creare una funzione vuota chiamata `playSong`.

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

`playSong` dovrebbe essere una funzione.

```js
assert.isFunction(playSong)
```

La tua funzione `playSong` dovrebbe usare la sintassi a freccia.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

La tua funzione `playSong` dovrebbe prendere un parametro `id`.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

La tua funzione `playSong` dovrebbe essere vuota.
