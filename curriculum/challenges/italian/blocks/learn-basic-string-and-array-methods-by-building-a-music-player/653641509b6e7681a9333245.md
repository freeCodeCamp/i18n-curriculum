...
Devi aggiornare la canzone attualmente in riproduzione e anche l'aspetto dell'elemento `playButton`.

Assegna `song` alla proprietà `currentSong` sull'oggetto `userData`.

*Nota*: in questo passaggio non dovresti usare l'operatore di concatenamento opzionale `?.` perché `userData.currentSong` non sarà `null` né `undefined` a questo punto.

# --hints--

Dovresti assegnare `song` a `userData.currentSong`.
