...
Du solltest drei `div`-Elemente unterhalb des `h2#student-info`-Elements verschachteln.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.length, 3);
```

Du solltest dem ersten `div` eine `class` des Werts `info` geben.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[0]?.className, 'info');
```

Du solltest dem zweiten `div` eine `class` des Werts `info` geben.

```js
assert.equal(document.querySelectorAll('h2#student-info ~ div')?.[1]?.className, 'info');
```

Du solltest dem dritten `div` eine `class` des Werts `info` geben.
...
Du solltest ein `input`-Element innerhalb des ersten `div` einfügen, direkt nach dem `label`.
...
Du solltest ein `input`-Element innerhalb des zweiten `div` nach dem `label` einfügen.
...
Du solltest ein `input`-Element innerhalb des dritten `div` nach `label` einfügen.
