...
O atributo `lang` dentro da tag de abertura `<i>` é usado para especificar o idioma do conteúdo. Neste caso, o idioma seria francês. O elemento `i` não indica se o texto é importante ou não, ele apenas mostra que é de alguma forma diferente do texto ao redor.
