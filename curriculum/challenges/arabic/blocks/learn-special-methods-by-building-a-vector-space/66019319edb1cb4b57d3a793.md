...
الطريقة الأخيرة التي تحتاجها هي `__ge__`، التي تُستدعى عند استخدام `>=` لمقارنة كائنين.

عرّف طريقة `__ge__` بمعلمتين `self` و `other` واجعلها تُرجع العكس لـ `self < other`.

# --hints--

يجب أن تعرف طريقة `__ge__` داخل صنف `R2Vector`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").has_function("__ge__")`)) })
```

يجب أن تحتوي طريقة `__ge__` على معلمتين، `self` و `other`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("R2Vector").find_function("__ge__").has_args("self, other")`)) })
```

يجب أن تُرجع طريقة `__ge__` العكس لـ `self < other`.
