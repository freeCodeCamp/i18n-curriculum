...
A propriedade `gap` é uma forma abreviada para definir o valor de `column-gap` e `row-gap` ao mesmo tempo.Se receber um valor, ele define `column-gap` e `row-gap` para esse valor.Se receber dois valores, ele define `row-gap` para o primeiro valor e `column-gap` para o segundo.

Dê ao seletor `.image-wrapper` uma propriedade `gap` definida como `2rem`.

# --hints--

O elemento `.image-wrapper` deve ter a propriedade `gap` definida como `2rem`.
