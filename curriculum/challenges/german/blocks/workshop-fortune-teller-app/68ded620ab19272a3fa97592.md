...
Da das Projekt jetzt mit HTML-DOM-Elementen arbeitet, benötigen Sie eine Möglichkeit, das Fortune im DOM zu rendern.

Deklarieren Sie eine Funktion namens `renderCard`, die vier Argumente entgegennimmt: `drawingType`, `isReversed`, `shortName` und `img`, und ein Template Literal zurückgibt.

Die Parameter `drawingType`, `shortName` und `img` müssen vom Typ `string` sein, während `isReversed` vom Typ `boolean` sein muss. Die Funktion sollte einen Rückgabetyp von `string` haben.

# --hints--

Sie sollten eine Funktion namens `renderCard` haben.
...
Ihre `renderCard`-Funktion sollte vier Parameter haben.
...
Ihr erster Parameter sollte `drawingType` vom Typ `string` sein.
...
Ihr zweiter Parameter sollte `isReversed` vom Typ `boolean` sein.
...
Ihr dritter Parameter sollte `shortName` vom Typ `string` sein.
...
Ihr vierter Parameter sollte `img` vom Typ `string` sein.
...
Ihre `renderCard`-Funktion sollte einen Rückgabetyp von `string` haben.
...
Ihre `renderCard`-Funktion sollte ein Template Literal zurückgeben.
