...
テーブルの見出しの最後の部分は年数です。`#years` セレクターを作成し、フレックスボックスを有効にしてください。コンテンツをフレックス方向の終わりに寄せ、要素をstickyにしてください。`top: 0` を使ってコンテナの上部に固定してください。
