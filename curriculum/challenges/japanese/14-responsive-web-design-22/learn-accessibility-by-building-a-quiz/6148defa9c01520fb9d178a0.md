...
`ul` もしくは `li` 要素を使うと、箇条書きのための記号を表示させることができますが、ここで作成するラジオボタンには必要ありません。 `list-style` プロパティを使うと、箇条書きの見た目を制御できます。 例えば、下記のようにすると記号を円 (中空円) に変更できます。
...
リスト項目 `.answers-list` のスタイルを `none` に設定してデフォルトスタイルを除去し、さらに順序なしリストのパディングを除去してください。
...
`.answers-list` の `list-style` を `none` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.answers-list')?.listStyle, 'none');
```

`.answers-list` の `padding` を `0` に設定する必要があります。
