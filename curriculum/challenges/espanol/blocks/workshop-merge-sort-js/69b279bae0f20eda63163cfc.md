...
Lo último que hay que hacer para el ciclo `while` es avanzar al siguiente índice en el arreglo ordenado.

Suma `1` al valor de `sortedIndex`.

Ten en cuenta que el incremento debe hacerse después de asignar el elemento de la parte izquierda al arreglo ordenado.

# --hints--

Debes incrementar el valor actual de `sortedIndex` en uno dentro del ciclo `while`.
