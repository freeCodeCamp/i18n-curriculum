...
Sehemu ya mwisho ya swali hili itakuwa na menyu kunjuzi, na kisanduku cha maandishi.

Anza kwa kuweka `div` yenye `class` ya `formrow`, na kuweka vipengele vinne vya `div` ndani yake, ukibadilisha sifa za `class` kua `question-block` na `answer`.
...
Unapaswa kuipa `div` ya kwanza `class` ya `formrow`.
