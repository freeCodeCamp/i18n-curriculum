...
接下來，你需要使用 `document.querySelector` 來取得 `.start-screen` 和 `.checkpoint-screen` 元素。 

將它們分別儲存在名為 `startScreen` 和 `checkpointScreen` 的 `const` 變數中。 
...
你應該取得具有 `class` 為 `start-screen` 的元素。別忘了在 `document.querySelector` 方法中包含 `.`。

```js
assert.match(code, /document\.querySelector\(\s*('|"|`)\.start-screen\1\s*\)/);
```

你應該使用 `const` 來宣告名為 `startScreen` 的變數，並將 `.start-screen` 元素指定給它。 

```js
assert.match(code, /const\s+startScreen\s*=\s*document\.querySelector\(\s*('|"|`)\.start-screen\1\s*\)/);
```

你應該使用 `document.querySelector()` 方法來取得 `.checkpoint-screen` 元素。

```js
assert.match(code, /document\.querySelector\(\s*('|"|`)\.checkpoint-screen\1\s*\)/);
```

你應該使用 `const` 來宣告名為 `checkpointScreen` 的變數，並將 `.checkpoint-screen` 元素指定給它。 
