...
你應該在 `background` 屬性中爲 `.fb6` 提供第二個 `repeating-linear-gradient`。
...
你應該爲第二個 `repeating-linear-gradient` 設置從 `0%` 到 `10%` 是第一種顏色 `--building-color3`。
...
你應該爲第二個 `repeating-linear-gradient` 設置從 `10%` 到 `30%` 是第二種顏色 `--window-color3`。
