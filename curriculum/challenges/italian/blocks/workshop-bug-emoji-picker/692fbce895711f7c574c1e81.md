...
Ora, all'interno della funzione `isSelect`, restituisci un booleano che indichi se `element` è un'istanza di `HTMLSelectElement`. Usa l'operatore `instanceof` per effettuare questo controllo.

# --hints--

La tua funzione `isSelect` dovrebbe restituire `element instanceof HTMLSelectElement`.
