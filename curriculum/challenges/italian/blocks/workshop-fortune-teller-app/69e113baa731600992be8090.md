...
Crea una variabile `CDN_URL` e impostala su `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. Crea anche una variabile `defaultImg` con il valore `new URL("default.svg", import.meta.url)` e una variabile `LOCAL_DEFAULT_IMG` con il valore `defaultImg`.
...
La tua variabile `defaultImg` dovrebbe essere impostata su `new URL("default.svg", import.meta.url)`.
