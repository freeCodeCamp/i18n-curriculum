...
最後に、曲をもう一度レンダリングして、再生ボタンのアクセシブルテキストを更新し、リセットボタンをプレイリストから削除してください。さらに、DOMから`resetButton`も削除する必要があります。

曲をアルファベット順にもう一度レンダリングするために、`renderSongs()`を引数として`sortSongs()`関数を呼び出してください。

再生ボタンのアクセシブルテキストを更新するために、`setPlayButtonAccessibleText()`関数を呼び出してください。

`remove()`変数の`resetButton`メソッドを呼び出して、リセットボタンをプレイリストから削除してください。

**注意**：すべての曲を削除して何が起こるか試してみることもできます。

# --hints--

既存のイベントリスナーとその内容は変更しないでください。

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

`renderSongs`を使って`sortSongs()`関数を呼び出す必要があります。
...
`remove()`をDOMから削除するには、`resetButton`メソッドを使う必要があります。
