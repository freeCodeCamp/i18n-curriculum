...
為 `.fb1c` 添加一個 `repeating-linear-gradient`，放在現有的下面；使用你的 `--building-color4` 從 `0%` 到 `10%`，以及 `--window-color4` 從 `10%` 到 `90%`。這將填充在你上次添加的漸層後面。

# --hints--

你不應該更改第一個 `repeating-linear-gradient`。
...
你應該在第一個 `repeating-linear-gradient` 後面加上逗號（`,`）。
...
你應該為你的 `repeating-linear-gradient` 添加第一個顏色為 `--building-color4`，範圍從 `0%` 到 `10%`。
...
你應該在 `10%` 到 `90%` 之間使用第二種 `--window-color4` 顏色。
