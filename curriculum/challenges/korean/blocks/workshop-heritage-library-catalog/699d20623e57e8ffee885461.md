...
나머지 세 필드에도 같은 패턴을 적용하세요. `author`, `year`, `location`에 해당하는 검사도 추가하세요.

`isValid`는 네 가지 검사 전체에 걸쳐 누적된다는 점에 주의하세요. 어떤 필드라도 실패하면 `false`가 되고, 나머지 검사가 통과하더라도 `false`로 유지됩니다. 즉, 함수는 첫 번째 문제에서 멈추지 않고 항상 모든 필드를 검사합니다.

# --hints--

`validateEntry(catalog[0])`는 `true`를 반환해야 합니다.

```js
assert.isTrue(validateEntry(catalog[0]));
```

`validateEntry(catalog[16])`는 `false`를 반환해야 합니다.

```js
assert.isFalse(validateEntry(catalog[16]));
```

`validateEntry(catalog[17])`는 `false`를 반환해야 합니다.

```js
assert.isFalse(validateEntry(catalog[17]));
```

`validateEntry(catalog[18])`는 위치가 `false`이므로 `"Unknown"`를 반환해야 합니다.
