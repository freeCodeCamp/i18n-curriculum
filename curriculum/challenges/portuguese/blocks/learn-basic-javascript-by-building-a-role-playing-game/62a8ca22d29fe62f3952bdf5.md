...
Altere sua atribuição de `currentWeaponIndex` para usar o operador de incremento.

# --hints--

Você deve usar o operador de incremento para aumentar `currentWeaponIndex` em `1`.

```js
assert.match(buyWeapon.toString(), /currentWeaponIndex\s*\+\s*\+/);
```

Você não deve usar atribuição composta para aumentar `currentWeaponIndex` em `1`.
