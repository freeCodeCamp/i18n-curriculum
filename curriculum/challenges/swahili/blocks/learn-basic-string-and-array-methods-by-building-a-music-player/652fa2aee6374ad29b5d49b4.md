...
Pata vipengele vya `#next`, `#previous` na `#shuffle` kutoka kwenye faili ya HTML.

Weka thamani zao katika vigezo vinavyoitwa `nextButton`, `previousButton`, na `shuffleButton`, kwa mpangilio huo.

# --hints--

Unapaswa kuwa na kigezo kinachoitwa `nextButton`.

```js
assert.isDefined(nextButton);
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#next`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

Unapaswa kuweka kipengele cha `#next` katika kigezo `nextButton`. 

```js
assert.equal(nextButton, document.getElementById('next'));
```

Unapaswa kuwa na kigezo kinachoitwa `previousButton`.

```js
assert.isDefined(previousButton);
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#previous`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

Unapaswa kuweka kipengele cha `#previous` katika kigezo `previousButton`. 

```js
assert.equal(previousButton, document.getElementById('previous'));
```

Unapaswa kuwa na kigezo kinachoitwa `shuffleButton`.

```js
assert.isDefined(shuffleButton);
```

Unapaswa kutumia `document.getElementById()` kupata kipengele cha `#shuffle`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

Unapaswa kuweka kipengele cha `#shuffle` katika kigezo `shuffleButton`. 
