...
接下來，使用 `getElementById()` 方法訪問 `discard-btn`、`tasks-container` 和 `title-input` 元素。 將它們分別保存在名爲 `discardBtn`、`tasksContainer` 和 `titleInput` 的變量中。

# --hints--

你應該使用 `getElementById()` 來訪問 `discard-btn` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)discard\-btn\1\s*\)\s*;?/)
```

你應該將 `discard-btn` 元素分配給變量 `discardBtn`。 不要忘記使用 `const` 來聲明變量。

```js
assert.match(code, /const\s+discardBtn\s*=\s*document\.getElementById\(\s*('|"|`)discard\-btn\1\s*\)\s*;?/)
```

你應該使用 `getElementById()` 來訪問 `tasks-container` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)tasks\-container\1\s*\)\s*;?/)
```

你應該將 `tasks-container` 元素分配給變量 `tasksContainer`。 不要忘記使用 `const` 來聲明變量。

```js
assert.match(code, /const\s+tasksContainer\s*=\s*document\.getElementById\(\s*('|"|`)tasks\-container\1\s*\)\s*;?/)
```

你應該使用 `getElementById()` 來訪問 `title-input` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)title\-input\1\s*\)\s*;?/)
```

你應該將 `title-input` 元素分配給變量 `titleInput`。 不要忘記使用 `const` 來聲明變量。
