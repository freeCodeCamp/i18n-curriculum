...
このロジックは実際に少しすっきりさせることができます。アロー関数の暗黙の戻り値を使えば、変数を代入することなく、`.reduce()` メソッドの値を配列の長さで割った結果を直接返せます。

上記のように`getMean` 関数を更新してください。
...
`reduce` の結果を配列の長さで割ったものを、暗黙の戻り値構文を使って直接返してください。
