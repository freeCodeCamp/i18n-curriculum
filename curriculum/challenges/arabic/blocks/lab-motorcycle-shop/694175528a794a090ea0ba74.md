...
في هذا المختبر، تم تزويدك بكل شفرة HTML وCSS. ستستخدم TypeScript لإكمال وظائف صفحة متجر الدراجات النارية.

**الهدف:** حقق قصص المستخدم أدناه واجعل جميع الاختبارات تنجح لإكمال المختبر.

**قصص المستخدم:**

1. يجب أن تنشئ `type` باسم `Category` يحتوي على القيم التالية الممكنة: `'Sport'`، `'Cruiser'`، `'Touring'`، `'Dirt'`، `'Adventure'`، `'Naked'`، `'Electric'`.
2. يجب أن تنشئ `interface` باسم `Motorcycle` يحتوي على الخصائص التالية: `id` من نوع `string`، `name` من نوع `string`، `manufacturer` من نوع `string`، `category` من نوع `Category`، `price` من نوع `number`، `image_url` من نوع `string`، خاصية `created_at` من نوع `Date`، `description` من نوع `string`، وسنة من نوع `number`
3. يجب أن تنشئ دالة باسم `fetchMotorcycles` تُرجع `Promise` تحل إلى مصفوفة من كائنات `Motorcycle`.
4. يجب أن تحمل بيانات `Motorcycle` من نقطة النهاية `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
5. يجب أن تنشئ دالة باسم `renderMotorcycleCard` تأخذ كائن `Motorcycle` واحد كمعلمة وتُرجع سلسلة نصية تحتوي على HTML منسق لبطاقة الدراجة النارية.
   1. يجب أن تأخذ دالة `renderMotorCycleCard` معلمة باسم `motorcycle` من نوع `Motorcycle`.
   2. يجب أن تُرجع دالة `renderMotorCycleCard` نوع `string`.
   3. يجب أن يحتوي كل مكوِّن بطاقة دراجة نارية على عنصر `img` به صورة صالحة وفئة `motorcycle-card-image-container`.
   4. يجب أن يحتوي كل مكوِّن بطاقة دراجة نارية على عنصر بفئة `motorcycle-card-year-badge` يعرض السنة كنص عادي.
   5. يجب أن يحتوي كل مكوِّن بطاقة دراجة نارية على عنصر بفئة `motorcycle-card-title` يعرض اسم الدراجة كنص عادي.
   6. يجب أن يحتوي كل مكوِّن بطاقة دراجة نارية على عنصر بفئة `motorcycle-card-manufacturer` يعرض اسم الشركة المصنعة للدراجة كنص عادي.
   7. يجب أن يحتوي كل مكوِّن بطاقة دراجة نارية على عنصر بفئة `motorcycle-card-category` يعرض فئة الدراجة كنص عادي.
   8. يجب أن يحتوي كل مكوِّن بطاقة دراجة نارية على عنصر بفئة `motorcycle-card-description` يعرض وصف الدراجة كنص عادي.
   9. يجب أن يحتوي كل مكوِّن بطاقة دراجة نارية على عنصر بفئة `motorcycle-card-price` يعرض سعر الدراجة كنص عادي.
   10. يجب أن يحتوي كل مكوِّن بطاقة دراجة نارية على عنصر بفئة `motorcycle-card-engine` يعرض قوة حصان كل محرك.
6. يجب أن يكون لديك صنف باسم `MotorcycleGalleryApp`.
   1. يجب أن يحتوي الصنف على مصفوفة `private` باسم `allMotorcycles` مع نوع ثابت لمصفوفة من كائنات `Motorcycle`.
   2. يجب أن يحتوي الصنف على دالة باسم `renderMotorcycles` تعرض مكوِّنات بطاقات الدراجات النارية المكافئة لمتغير `allMotorcycles` داخل العنصر الذي له معرف `motorcycle-grid`.
7. يجب أن يُعرض عدد عناصر `Motorcycle` المعروضة داخل عنصر له معرف `results-number`.
8. يجب تحديث الفلتر عند حدث `input`.

# --hints--

يجب أن يكون لديك `type` باسم `Category`.
...
يجب أن يكون نوع `Category` اتحادًا للقيم الصحيحة.
...
يجب أن يكون لديك `interface` باسم `Motorcycle`.
...
يجب أن تحتوي واجهة `Motorcycle` على كل الخصائص المحددة.
...
يجب أن يكون لديك دالة باسم `fetchMotorcycles`.

```js
assert.isFunction(fetchMotorcycles);
```

يجب أن تستخدم نقطة النهاية `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
...
يجب أن تُرجع وعدًا يحتوي على 25 كائنًا من `Motorcycle` من دالة `fetchMotorcycles`.
...
يجب أن يكون لديك دالة باسم `renderMotorcycleCard`.

```js
assert.isFunction(renderMotorcycleCard);
```

يجب أن تقبل دالة `renderMotorcycle` معلمة ذات نوع ثابت من `Motorcycle`.
...
يجب أن تُرجع دالة `renderMotorcycleCard` نوعًا صريحًا من `string`.
...
يجب أن تُرجع دالة `renderMotorcycleCard` سلسلة نصية `string`.
...
يجب أن يكون لديك صنف باسم `MotorcycleGalleryApp`.
...
يجب أن يحتوي `MotorcycleGalleryApp` على مصفوفة باسم `allMotorcycles`.
...
يجب أن تكون خاصية `allMotorcycles` خاصة `private`.
...
يجب أن تكون خاصية `allMotorcycles` ذات نوع ثابت `Motorcycle[]`.
...
يجب أن يحتوي `MotorcycleGalleryApp` على دالة باسم `renderMotorcycles`.
...
يجب أن تعرض دالة `renderMotorcycles` 25 عنصرًا بفئة `motorcycle-card-image-container`.
...
يجب أن تعرض دالة `renderMotorcycles` 25 عنصرًا بفئة `motorcycle-card-year-badge`.
...
يجب أن تعرض دالة `renderMotorcycles` 25 عنصرًا بفئة `motorcycle-card-title`.
...
يجب أن تعرض دالة `renderMotorcycles` 25 عنصرًا بفئة `motorcycle-card-manufacturer`.
...
يجب أن تعرض دالة `renderMotorcycles` 25 عنصرًا بفئة `motorcycle-card-category`.
...
يجب أن تعرض دالة `renderMotorcycles` 25 عنصرًا بفئة `motorcycle-card-description`.
...
يجب أن تعرض دالة `renderMotorcycles` 25 عنصرًا بفئة `motorcycle-card-price`.
...
يجب أن تعرض دالة `renderMotorcycles` 25 عنصرًا بفئة `motorcycle-card-engine`.
