...
Jetzt sollten Sie an der Platzierung der Elemente innerhalb der Karte für den Plan arbeiten.

Dazu setzen Sie den Container auf die Anzeige als Grid und geben eine benutzerdefinierte Zeilengröße mit eckigen Klammern in Ihrer Tailwind-Klasse an.

So können Sie eine benutzerdefinierte Grid-Zeile mit Bruchteilen festlegen:
...
Geben Sie dem äußeren `div` für den Plan die Utility-Klassen `grid` und `grid-rows-[1fr_auto]`.

# --hints--

Ihr `div`-Element sollte die Klasse `grid` haben.
...
Ihr `div`-Element sollte die Klasse `grid-rows-[1fr_auto]` haben.
