...
All'interno del metodo `fetchCardsData`, dichiara una variabile chiamata `response` e assegnala chiamando la funzione `fetch` con il seguente URL: `` `${CDN_URL}/card_data.json` ``.

Aggiungi la parola chiave `await` prima della chiamata `fetch` e racchiudi tutta la logica all'interno di un blocco `try...catch`.

# --hints--

Dovresti dichiarare una variabile `response` all'interno del metodo `fetchCardsData`.
...
La tua variabile `response` dovrebbe essere assegnata chiamando `await fetch` con l'URL `` `${CDN_URL}/card_data.json` ``.
...
La tua chiamata `fetch` dovrebbe essere racchiusa all'interno di un blocco `try...catch`, con `error` come parametro `catch`.
