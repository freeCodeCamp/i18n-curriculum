...
In alcuni casi, le carte devono essere girate, perché non sono sempre nella posizione corretta.

Dichiara una variabile chiamata `isReversed` all'interno del metodo `singleCardSelected`. Usa il metodo `Math.random()` per generare un valore booleano casuale. Per assicurarti che il valore rimanga entro i limiti, confrontalo con `0.5`.
