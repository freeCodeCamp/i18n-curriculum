...
Нарешті, використайте метод `.appendChild()`, щоб додати свій елемент `label` до елемента `container`.

# --hints--

Ви повинні отримати доступ до методу `.appendChild()` елемента `container`.

```js
assert.match(code, /container\.appendChild\(/);
```

Ви повинні передати свій елемент `label` методу `.appendChild()`.

```js
assert.match(code, /container\.appendChild\(\s*label\s*\)/);
```

Ви повинні додати `label` після встановлення атрибутів.
