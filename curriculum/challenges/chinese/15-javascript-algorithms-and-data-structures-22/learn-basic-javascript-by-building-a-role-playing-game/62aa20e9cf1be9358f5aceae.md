...
在 `attack()` 函数内的第一个 `if` 语句中添加 `else` 语句。 在 `else` 语句中，使用 `+=` 运算符将文本 `" You miss."` 添加到 `text.innerText` 的末尾。

# --hints--

你应该在 `if (isMonsterHit())` 块之后添加一个 `else` 块。
...
你应该将文本 `" You miss."` 添加到 `text.innerText` 的末尾。 记住使用复合赋值，并确保单词 `You` 之前有一个空格。
