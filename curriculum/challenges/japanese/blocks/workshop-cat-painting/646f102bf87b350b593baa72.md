...
`.cat-whiskers-right` 要素の中に、クラスが `div`、`cat-whisker-right-top`、`cat-whisker-right-middle` の `cat-whisker-right-bottom` 要素を3つ作成してください。

# --hints--

既存のクラス `div` を持つ `cat-whiskers-right` 要素は変更しないでください。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers-right'), 1);
```

`div` 要素の中に3つの `.cat-whiskers-right` 要素を作成してください。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers-right div'), 3);
```

`div` 要素の中の最初の `.cat-whiskers-right` 要素にはクラス `cat-whisker-right-top` を付けてください。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-right div')[0].classList.contains('cat-whisker-right-top'));
```

`div` 要素の中の2番目の `.cat-whiskers-right` 要素にはクラス `cat-whisker-right-middle` を付けてください。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-right div')[1].classList.contains('cat-whisker-right-middle'));
```

`div` 要素の中の3番目の `.cat-whiskers-right` 要素にはクラス `cat-whisker-right-bottom` を付けてください。
