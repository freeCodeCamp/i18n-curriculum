...
Rufen Sie innerhalb der `playNextSong`-Funktion die `setPlayerDisplay`-, `highlightCurrentSong`- und `setPlayButtonAccessibleText`-Funktionen auf, um den Spieler korrekt zu aktualisieren, wenn die Wiedergabeliste endet.

# --hints--

Sie sollten `setPlayerDisplay` innerhalb der `else`-Anweisung Ihrer `playNextSong`-Funktion aufrufen.
...
Sie sollten `highlightCurrentSong` innerhalb der `else`-Anweisung Ihrer `playNextSong`-Funktion aufrufen.
...
Sie sollten `setPlayButtonAccessibleText` innerhalb der `else`-Anweisung Ihrer `playNextSong`-Funktion aufrufen.
