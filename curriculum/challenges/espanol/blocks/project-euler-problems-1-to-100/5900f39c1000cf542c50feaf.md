...
title: 'Problema 48: Potencias propias'
challengeType: 1
forumTopicId: 302157
...
Encuentra los últimos diez dígitos de la serie, 1<sup>1</sup> + 2<sup>2</sup> + 3<sup>3</sup> + ... + 1000<sup>1000</sup>.

# --hints--

`selfPowers(10, 3)` debe devolver un número.

```js
assert(typeof selfPowers(10, 3) === 'number');
```

`selfPowers(10, 3)` debe devolver 317.

```js
assert.strictEqual(selfPowers(10, 3), 317);
```

`selfPowers(150, 6)` debe devolver 29045.

```js
assert.strictEqual(selfPowers(150, 6), 29045);
```

`selfPowers(673, 7)` debe devolver 2473989.

```js
assert.strictEqual(selfPowers(673, 7), 2473989);
```

`selfPowers(1000, 10)` debe devolver 9110846700.
