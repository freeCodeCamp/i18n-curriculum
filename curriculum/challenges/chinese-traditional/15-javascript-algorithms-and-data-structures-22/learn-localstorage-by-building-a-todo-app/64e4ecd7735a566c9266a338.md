...
現在你已經完成了打開和關閉窗口的操作，接下來就是從輸入字段中獲取值，將它們保存到 `taskData` 數組中，並將它們顯示在頁面上。

首先，向你的 `taskForm` 元素添加一個 `submit` 事件監聽器，並將 `e` 作爲箭頭函數的參數傳入。 在花括號內，使用 `preventDefault()` 方法來阻止瀏覽器在提交表單後刷新頁面。

# --hints--

你應該在 `taskForm` 變量上調用 `addEventListener()` 方法。

```js
assert.match(code, /taskForm\.addEventListener\(/)
```

你的事件監聽器應該監聽 `submit` 事件。

```js
assert.match(code, /taskForm\.addEventListener\(\s*('|"|`)submit\1/)
```

你應該使用箭頭語法將事件監聽器設置爲一對空的花括號，並以 `e` 作爲參數。

```js
assert.match(code, /taskForm\.addEventListener\(\s*('|"|`)submit\1\s*,\s*(\(\s*e\s*\)|e)\s*=>\s*\{/)
```

你應該使用 `e.preventDefault()` 方法來阻止瀏覽器重新加載頁面。
