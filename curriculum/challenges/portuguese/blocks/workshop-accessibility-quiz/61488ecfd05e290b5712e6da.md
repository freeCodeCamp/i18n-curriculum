...
Para dar uma aparência em linha à primeira seção, selecione apenas os elementos `input` dentro dos elementos `.info`, defina sua `width` como `50%` e alinhe seu texto à esquerda.
...
Você deve dar aos elementos `input` uma `width` de `50%`.
...
Você deve dar aos elementos `input` um `text-align` de `left`.
