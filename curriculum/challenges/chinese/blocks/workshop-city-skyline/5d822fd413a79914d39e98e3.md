...
创建另一个名为 `--building-color4` 的变量，并为其赋值 `#538cc6`。 这次确保它在 `:root` 选择器中。 然后用它来填充最后一个建筑物。

# --hints--

你应该定义一个名为 `--building-color4` 的新属性变量。

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--building-color4'));
```

你应该在 `:root` 选择器中将 `--building-color4` 的值设置为 `#538cc6`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--building-color4').trim(), '#538cc6');
```

你应该在 `.bb4` 选择器中添加 `background-color: var(--building-color4)`。
