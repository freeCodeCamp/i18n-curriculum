...
Você tem um pequeno erro. `arg1` e `arg2` são strings, não números. `infixToFunction['+']("1", "2")` retornaria `12`, o que não é matematicamente correto.

Envolva cada um dos argumentos de `infixToFunction[operator]` em uma chamada de `parseFloat()`.

# --hints--

Você deve envolver `arg1` em uma chamada de `parseFloat()`.
...
Você deve envolver `arg2` em uma chamada de `parseFloat()`.
