...
現在你需要準備更新顯示用戶新增的產品。 聲明一個 `currentProductCount` 變量，並將其值分配爲與 `product` 的 `id` 匹配的 `totalCountPerProduct` 對象的屬性。

# --hints--

你應該在 `addItem` 函數中聲明一個 `currentProductCount` 變量。
...
你應該使用 `const` 來聲明 `currentProductCount`。
...
你應該將 `totalCountPerProduct[product.id]` 的值賦給 `currentProductCount`。
