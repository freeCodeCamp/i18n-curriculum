...
Um das Ergebnis des „Bubble-up“ zu erreichen, müssen Sie prüfen, ob das aktuelle Element größer ist als das nächste Element. Sie können dies tun, indem Sie auf das `array` bei `j` und `j+1` zugreifen.

Erstellen Sie eine `if`-Bedingung, die überprüft, ob das aktuelle Element größer ist als das nächste Element.

# --hints--

Sie sollten Ihre `console.log`-Anweisung nicht entfernen.
...
Sie sollten eine `if`-Anweisung in Ihrer verschachtelten Schleife verwenden.
...
Ihre `if`-Anweisung sollte prüfen, ob das `array[j]` größer ist als das `array[j+1]`.
