...
이제 새 아이콘 요소를 추가하면 자동 배치 알고리즘이 작동합니다. 하지만 알고리즘은 새 열 너비를 기본값인 `auto`로 설정하는데, 이는 현재 열과 일치하지 않습니다.

`grid-auto-columns` 속성으로 이를 재정의할 수 있습니다. `.social-icons` 선택자에 `grid-auto-columns` 속성을 `1fr`로 설정하세요.

# --hints--

`.social-icons` 선택자에 `grid-auto-columns` 속성을 `1fr`로 설정해야 합니다.
