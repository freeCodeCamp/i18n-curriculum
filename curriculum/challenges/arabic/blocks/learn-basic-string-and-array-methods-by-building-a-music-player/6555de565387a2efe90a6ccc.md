...
دالة `setPlayButtonAccessibleText` ستضبط خاصية `aria-label` على الأغنية الحالية أو على أول أغنية في قائمة التشغيل. وإذا كانت قائمة التشغيل فارغة، ستضبط خاصية `aria-label` على `"Play"`.

استخدم الطريقة `setAttribute` على عنصر `playButton` لتعيين خاصية باسم `"aria-label"`. باستخدام تعبير ثلاثي، عيّن قيمة الخاصية إلى `Play ${song.title}` أو `"Play"` إذا لم تتوفر قيمة `song?.title`.

لا تنسَ أنك تحتاج إلى استيفاء السلسلة النصية هنا، لذا يجب استخدام علامات دقيقة (backticks).

# --hints--

لا تعدل الدالة `setPlayButtonAccessibleText` الحالية أو محتواها.
...
يجب استخدام الطريقة `setAttribute()` على `playButton`.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

يجب أن تكون القيمة الأولى لطريقة `setAttribute()` هي `"aria-label"`.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

يجب أن تحتوي طريقة `setAttribute` على ``song?.title ? `Play ${song.title}` : "Play"`` كالمعلمة الثانية.
