...
在具有 `div` 的 `className` 的 `card-tier-badge` 元素下方，添加一个具有 `div` 的 `className` 的 `card-club`，并包含文本 `Santos FC`。

# --hints--

你应该在 `div` 下方添加一个具有 `className` 的 `card-club` 的 `.card-tier-badge`。

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

`.card-club` 元素应包含文本 `Santos FC`。
