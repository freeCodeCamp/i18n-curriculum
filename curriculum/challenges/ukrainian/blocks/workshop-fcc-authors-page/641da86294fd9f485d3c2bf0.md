...
Add an anchor element with a `class` attribute of `"author-link"`, interpolate `url` as the value for the `href` attribute, and set `target` to `"_blank"`. Для тексту елемента прив’язки вставте `author`, а потім текст `"'s author page"`. Наприклад, `"Quincy Larson's author page"`.
