...
如果使用者擲出三個相同的數字，和兩個另一個數字，這稱為滿堂彩。宣告一個接受單一引數的 `detectFullHouse` 函式。當呼叫時，該函式將會接收 `diceValuesArr` 陣列。

你的 `detectFullHouse` 函式應該檢查使用者是否擲出三個相同數字和兩個另一個數字。如果是，應該更新第三個圓鈕以顯示分數 `25`，並帶有正確的屬性。不論如何，應該始終更新最後一個圓鈕以顯示分數 0，並帶有正確的屬性。

當擲骰子時，別忘了呼叫你的新函式（程式）。

# --hints--

你應該有一個 `detectFullHouse` 函式（程式）。

```js
assert.isFunction(detectFullHouse);
```

你的 `detectFullHouse` 函式（程式）應該有一個參數。

```js
assert.match(code, /(?:var|let|const)\s+detectFullHouse\s*=\s*(\(\s*\w+\s*\)|\s*\w+\s*)\s*=>\s*|function\s+detectFullHouse\s*\(\s*\w+\s*\)/);
```

當擲出滿堂彩時，你的 `detectFullHouse` 函式應該啟用第三個圓鈕，將其值設為 `25`，並將第三個 `span` 設定為顯示文字 `, score = 25`。
...
當沒有擲出滿堂彩時，你的 `detectFullHouse` 函式應該啟用最後一個圓鈕，將其值設為 `0`，並將最後一個 `span` 設為顯示文字 `, score = 0`。
...
當你的 `rollDiceBtn` 被點擊時，你應該呼叫你的 `detectFullHouse` 函式（程式）。
