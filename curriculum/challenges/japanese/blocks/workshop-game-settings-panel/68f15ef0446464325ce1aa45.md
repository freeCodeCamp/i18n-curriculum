...
最後に、`label`要素に`cursor`プロパティを`pointer`に設定してください。これにより、ユーザーが`label`にカーソルを合わせたときに手のアイコンに変わり、クリック可能であることを示してユーザー体験が向上します。
...
`label`セレクターに`display`プロパティを追加してください。
...
`label`セレクターに`margin`プロパティを追加してください。
...
`label`セレクターに`cursor`プロパティを追加してください。
