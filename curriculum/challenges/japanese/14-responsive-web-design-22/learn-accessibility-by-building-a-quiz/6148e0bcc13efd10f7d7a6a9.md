...
`footer` の背景色を `#2a2a40` に設定し、さらに_フレックスボックス_を使用して水平方向の中央にテキストを配置してください。

# --hints--

`footer` 要素セレクターを使用する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

`footer` の `background-color` を `#2a2a40` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.backgroundColor, 'rgb(42, 42, 64)');
```

`footer` の `display` を `flex` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

`footer` の `justify-content` を `center` に設定する必要があります。
