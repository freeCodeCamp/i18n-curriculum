...
Para el `input` y `label` finales dentro de este `fieldset`, agregarás un `input` de casilla de verificación con los atributos `id` y `value` establecidos en `"price"`, y un atributo `name` establecido en `"choice"`.
...
Tu quinto `input` de casilla de verificación debe tener un atributo `name` establecido en `"choice"`.
