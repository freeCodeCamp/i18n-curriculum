...
`.toFixed()` 傳回字串的問題在於你想要能夠對稅率進行計算。為了解決這個問題，你可以將 `.toFixed()` 呼叫（包含計算）傳入 `parseFloat()` 函式（程式）。這將把固定的字串轉換回數字，並保留現有的小數位數。

將你的 `.toFixed()` 呼叫傳遞給 `parseFloat()`。

# --hints--

你應該將整個計算（不包含 `return` 述語）傳遞給 `parseFloat()`。
