...
A continuación, agrega otro `input` de tipo checkbox con los atributos `id` y `value` establecidos en `"location"`, y el atributo `name` establecido en `"choice"`.

Para el elemento `label`, el texto de `Location` y el atributo `for` deben establecerse en `"location"`.

Debajo de ese elemento `label`, agrega otro `input` de tipo checkbox con los atributos `id` y `value` establecidos en `"reputation"`, y el atributo `name` establecido en `"choice"`.
...
Tu tercer `input` de casilla de verificación debe tener un atributo `name` establecido en `"choice"`.
...
Tu cuarto `input` de casilla de verificación debe tener un atributo `name` establecido en `"choice"`.
