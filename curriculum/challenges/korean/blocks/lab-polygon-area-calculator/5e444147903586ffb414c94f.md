...
이 프로젝트에서는 객체지향 프로그래밍을 사용해 `Rectangle` 클래스와 `Square` 클래스를 만드세요. `Square` 클래스는 `Rectangle`의 서브클래스여야 하며, 그 메서드와 속성을 상속받아야 합니다.

**목표:** 아래 사용자 스토리를 충족하고 모든 테스트를 통과해 실습을 완료하세요.

**사용자 스토리:**

1. `Rectangle` 클래스를 만들어야 합니다.

2. `Rectangle` 객체가 생성될 때 `width`와 `height` 속성으로 초기화되어야 합니다. 클래스에는 다음 메서드가 포함되어야 합니다:

    - `set_width`: 직사각형의 너비를 설정합니다.
    - `set_height`: 직사각형의 높이를 설정합니다.
    - `get_area`: 넓이(\\( \text{width} \times \text{height} \\))를 반환합니다.
    - `get_perimeter`: 둘레 \\( 2(\text{width} + \text{height}) \\)를 반환합니다.
    - `get_diagonal`: 대각선 길이 \\( \sqrt{\text{width}^2 + \text{height}^2} \\)를 반환합니다.
    - `get_picture`: `*` 문자로 이루어진 선들로 도형을 나타내는 문자열을 반환합니다. 선의 개수는 높이와 같고, 각 선의 `*` 개수는 너비와 같아야 합니다. 각 선 끝에는 개행 문자(`\n`)가 있어야 합니다. 너비나 높이가 `50`보다 크면 `Too big for picture.` 문자열을 반환해야 합니다.
    - `get_amount_inside`: 다른 도형(정사각형 또는 직사각형)을 인수로 받습니다. 회전 없이 전달된 도형이 이 도형 안에 몇 번 들어갈 수 있는지 반환합니다. 예를 들어, 너비가 `4`이고 높이가 `8`인 직사각형은 한 변이 `4`인 정사각형 두 개가 들어갈 수 있습니다.

3. `Rectangle` 인스턴스가 문자열로 표현되면 `Rectangle(width=5, height=10)`처럼 보여야 합니다.

4. `Square` 클래스를 만들고 `Rectangle`를 서브클래스로 해야 합니다.
5. `Square` 객체가 생성될 때 한 변의 길이로 초기화되어야 합니다. `__init__` 메서드는 `width` 클래스의 `height`와 `Rectangle` 속성 모두에 변 길이를 저장해야 합니다.
6. `Square` 클래스에는 다음 메서드가 포함되어야 합니다:

    - `set_width`: `set_width` 클래스의 `Rectangle` 메서드를 재정의합니다. 너비와 높이를 변 길이로 설정해야 합니다.
    - `set_height`: `set_height` 클래스의 `Rectangle` 메서드를 재정의합니다. 너비와 높이를 변 길이로 설정해야 합니다.
    - `set_side`: 정사각형의 높이와 너비를 변 길이와 같게 설정합니다.

7. `Square` 클래스는 `Rectangle` 클래스의 메서드에 접근할 수 있어야 합니다.
8. `Square` 인스턴스가 문자열로 표현되면 `Square(side=9)`처럼 보여야 합니다.
...
`Rectangle(3, 6)`의 문자열 표현은 `Rectangle(width=3, height=6)`이어야 합니다.
...
`Square(5)`의 문자열 표현은 `Square(side=5)`이어야 합니다.
...
`Square` 클래스 인스턴스는 `.set_side()`을 사용해 새 값을 설정한 후 다른 문자열 표현을 가져야 합니다.
...
`Square` 클래스 인스턴스는 `.set_width()` 또는 `set_height()`을 사용해 새 값을 설정한 후 다른 문자열 표현을 가져야 합니다.
...
`.get_picture()` 메서드는 `Too big for picture.` 또는 `width` 속성이 `height`보다 크면 `50` 문자열을 반환해야 합니다.
