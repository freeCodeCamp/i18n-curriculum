...
ابدأ بـ query للـ `@media` لـ `only screen` مع `max-width` من `720px`. في داخلها، قم بإنشاء منتقي `.image-wrapper` وإعطائه خاصية `grid-template-columns` بقيمة `1fr`.

سيؤدي هذا إلى طي الصور الثلاث في عمود واحد على شاشات أصغر.

# --hints--

يجب أن يكون لديك `@media` جديد لـ `only screen and (max-width: 720px)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText === 'only screen and (max-width: 720px)');
```

يجب أن تحتوي قاعدة `@media` الجديدة على منتقي `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText === '.image-wrapper');
```

يجب أن يحتوي منتقي `.image-wrapper` الجديد على خاصية `grid-template-columns` بقيمة `1fr`.
