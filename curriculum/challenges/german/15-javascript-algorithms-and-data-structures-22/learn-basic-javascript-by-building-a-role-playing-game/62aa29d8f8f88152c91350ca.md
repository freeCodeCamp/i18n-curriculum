...
Füge nun eine `else`-Anweisung hinzu. Inside, add `"Wrong! You lose 10 health!"` to the end of `text.innerText`. Subtrahiere `10` von `health` und aktualisiere `healthText.innerText`.
...
Dein `else` Block sollte die Komponentenzuordnung verwenden, um `10` von `health` abzuziehen.

```js
assert.match(pick.toString(), /health\s*-=\s*10/);
```

Dein `else` Block sollte die `healthText.innerText` aktualisieren, um den neuen Wert von `health` wiederzugeben.
