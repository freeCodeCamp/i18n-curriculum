...
다음으로, `age`가 정수인지 확인하세요. 그래서 `age` 사전에 `constraints`라는 키를 하나 더 추가하세요. 그 값으로는 `isinstance`와 `age`를 인수로 전달하여 `int`를 호출하세요.

# --hints--

`constraints` 사전에 `age` 키가 있어야 합니다.
...
`age` 사전의 `constraints` 키는 `isinstance(age, int)` 값을 가져야 합니다.
