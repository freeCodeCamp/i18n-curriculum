...
Your `section` element should have an opening tag. I tag di apertura hanno la seguente sintassi: `<nomeElemento>`.

```js
assert(document.querySelector('section'));
```

Your `section` element should have a closing tag. I tag di chiusura hanno un carattere `/` subito dopo il carattere `<`.
