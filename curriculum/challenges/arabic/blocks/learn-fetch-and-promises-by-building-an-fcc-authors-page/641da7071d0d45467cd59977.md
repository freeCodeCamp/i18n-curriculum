...
الآن عليك عرض بعض المعلومات عن المؤلف. أولًا، اعرض اسم المؤلف.

أنشئ وسم `h2` مع `class` باسم `"author-name"`. ثم أدرج `author` داخل وسم `h2`. هذا هو اسم المؤلف.
...
يجب أن يحتوي عنصر `h2` على صنف مضبوط إلى `"author-name"`
...
يجب أن تدرج `author` داخل وسم `h2` لعرض أسماء المؤلفين
