...
Rudi kwenye kitendakazi chako cha `showLatestPosts()`, tumia ufumbuzi wa muundo kupata vigezo vya `topic_list` na `users` kutoka kwenye kitu cha `data`.

# --hints--

Unapaswa kutumia `const` kufumbua `topic_list` na `users` kutoka kwenye kitu cha `data`.
