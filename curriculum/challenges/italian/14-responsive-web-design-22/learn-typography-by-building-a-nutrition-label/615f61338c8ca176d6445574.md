...
Dovresti creare un nuovo `div` all'interno dell'elemento `.label`.

```js
assert(document.querySelectorAll('.label > div')?.length === 3)
```

Il nuovo elemento `div` dovrebbe avere l'attributo `class` con il valore `divider medium`. Questo div dovrebbe essere l'ultimo elemento nell'elemento `.label`.
