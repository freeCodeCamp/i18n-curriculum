...
現在對 `subTitle` 做相同的操作，將它指定給 `meaning_up`，然後將 `title` 指定給 `foundCard.name`。接著，呼叫 `showElements` 方法並從元素中傳入 `fortuneDescription`。

# --hints--

你應該將 `this.elements.subTitle.textContent` 設值為 `foundCard.meaning_up`。
...
你應該將 `this.elements.title.textContent` 設值為 `foundCard.name`。
...
你應該在 `showElements` 中呼叫 `showFortune` 函式。
...
你應該將 `this.elements.fortuneDescription` 作為引數傳入 `showElements`。
