...
Всередині `footer` додайте елемент `p`. Потім вкладіть елемент прив'язки (`a`) в `p`, що посилає на `https://www.freecodecamp.org` та має текст `Visit our website`.
...
Ваш новий елемент `p` повинен бути вкладеним в межах елемента `footer`. Ви повинні мати тільки один елемент `p`.
...
Ваш новий елемент `a` повинен бути вкладеним в межах нового елемента `p`. Ви повинні мати тільки один елемент `a`.
