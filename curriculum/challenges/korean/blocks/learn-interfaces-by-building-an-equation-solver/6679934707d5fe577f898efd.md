...
`results`의 길이가 `2`일 때를 위한 또 다른 경우를 추가하세요. 이번에는 `result_list`에 `x1 = <root1>`과 `x2 = <root2>` 형식의 두 문자열을 포함하는 리스트를 할당하세요. 다시 한 번, 결과에 양수와 음수 기호를 모두 표시하세요.

# --hints--

`match` 문장의 주어 값을 수정하지 마세요.
...
기존 `case` 블록 뒤에 `2` 패턴을 가진 새로운 `case`을 만드세요.
...
새로운 `x1 = <root1>` 본문 안에서 `x2 = <root2>`에 `result_list`과 `case` 형식의 두 문자열을 포함하는 리스트를 할당하세요. 결과에 양수와 음수 기호를 모두 표시하세요.
