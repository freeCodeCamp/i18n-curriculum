...
Greifen Sie auf die Elemente `#next`, `#previous` und `#shuffle` aus der HTML-Datei zu.

Weisen Sie sie den Variablen `nextButton`, `previousButton` und `shuffleButton` zu.

# --hints--

Sie sollten eine Variable namens `nextButton` haben.

```js
assert.isDefined(nextButton);
```

Sie sollten `document.getElementById()` verwenden, um auf das Element `#next` zuzugreifen.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

Sie sollten das Element `#next` der Variable `nextButton` zuweisen.

```js
assert.equal(nextButton, document.getElementById('next'));
```

Sie sollten eine Variable namens `previousButton` haben.

```js
assert.isDefined(previousButton);
```

Sie sollten `document.getElementById()` verwenden, um auf das Element `#previous` zuzugreifen.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

Sie sollten das Element `#previous` der Variable `previousButton` zuweisen.

```js
assert.equal(previousButton, document.getElementById('previous'));
```

Sie sollten eine Variable namens `shuffleButton` haben.

```js
assert.isDefined(shuffleButton);
```

Sie sollten `document.getElementById()` verwenden, um auf das Element `#shuffle` zuzugreifen.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

Sie sollten das Element `#shuffle` der Variable `shuffleButton` zuweisen.
