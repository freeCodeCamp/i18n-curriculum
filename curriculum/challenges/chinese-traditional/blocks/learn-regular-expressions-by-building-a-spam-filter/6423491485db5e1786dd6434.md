...
你的最終正規表達式將尋找像 `dear friend` 這樣的字串。宣告一個 `dearRegex` 並指定一個能匹配字串 `dear friend` 的正規表達式。記得要設為不區分大小寫，並將它添加到你的 `denyList` 陣列中。

# --hints--

你應該使用 `const` 來宣告你的 `dearRegex` 變數。

```js
assert.match(code, /const\s+dearRegex\s*=/);
```

你的 `dearRegex` 變數應該被指定一個正規表達式。
