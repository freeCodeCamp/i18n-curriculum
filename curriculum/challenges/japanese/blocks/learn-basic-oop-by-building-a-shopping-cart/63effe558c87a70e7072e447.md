...
`addItem` メソッドの動作は、商品がすでにカートにあるかどうかで変わる必要があります。現在の商品がすでにカートにあるかどうかをチェックする三項演算子を作成してください。構文エラーを避けるために、真の場合と偽の場合の両方で `undefined` を使ってください。

# --hints--

`currentProductCount` が `1` より大きいかどうかをチェックしてください。
...
`currentProductCount > 1` の条件で三項演算子を使うべきです。
