...
Todos los números reales forman el conjunto incontable R. Entre sus subconjuntos, relativamente simples son los conjuntos convex, cada una de ellas expresada como un rango entre dos números reales *a* y *b* donde *a* a *b*. En realidad hay cuatro casos por el significado de "entre", dependiendo del límite abierto o cerrado:
...
Escribe una función que tome 2 objetos, una cadena y un array como parámetros. Los objetos representan el conjunto y tienen atributos: `low`, `high` y `rangeType`.
...
La cadena representa la operación a realizar en los conjuntos. Puede ser: `"union"`, `"intersect"` y `"subtract"` (diferencia).
...
`realSet({"low":0, "high":3, "rangeType":3}, {"low":0, "high":1, "rangeType":1}, "subtract", [0, 1, 2])` debe devolver `[true, true, true]`.
...
`realSet({"low":0, "high":3, "rangeType":3}, {"low":0, "high":1, "rangeType":0}, "subtract", [0, 1, 2])` debe devolver `[false, false, true]`.
...
`realSet({"low":0, "high":33, "rangeType":1}, {"low":30, "high":31, "rangeType":0}, "intersect", [30, 31, 32])` debería devolver `[true, true, false]`.
