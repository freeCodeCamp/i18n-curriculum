...
Jetzt beginnen Sie damit, die Methoden zu erstellen.

Erstellen Sie eine Methode namens `singleCardSelected`. Rufen Sie darin die Funktion `hideElements` auf und übergeben Sie die folgenden Props: `singleCardBtn`, `multipleCardsBtn`, `multipleCard`, `text` und `headerTitle`.

Denken Sie daran, dass Sie auf alle mit `this.elements` zugreifen müssen.

# --hints--

Sie sollten eine Methode namens `singleCardSelected` definieren.
...
Innerhalb von `singleCardSelected` sollten Sie die Funktion `hideElements` aufrufen.
...
Sie sollten `singleCardBtn` an die Funktion `hideElements` übergeben.
...
Sie sollten `multipleCardsBtn` an die Funktion `hideElements` übergeben.
...
Sie sollten `multipleCard` an die Funktion `hideElements` übergeben.
...
Sie sollten `text` an die Funktion `hideElements` übergeben.
...
Sie sollten `headerTitle` an die Funktion `hideElements` übergeben.
