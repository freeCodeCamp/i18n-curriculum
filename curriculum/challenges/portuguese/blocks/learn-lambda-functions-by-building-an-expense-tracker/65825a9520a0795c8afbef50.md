...
Após a chamada de `print()`, chame a função `print_expenses` para exibir todas as despesas que foram adicionadas até agora. Passe a lista `expenses` como argumento.

# --hints--

Você deve chamar a função `print_expenses()` dentro da sua cláusula `elif`.
...
Você deve passar a lista `expenses` como argumento para sua chamada `print_expenses()`.
