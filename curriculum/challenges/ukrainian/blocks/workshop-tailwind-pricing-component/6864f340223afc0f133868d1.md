...
Перейдіть до тексту ціни всередині `p` та `span`, додайте елементу `p` класи `text-4xl`, `font-bold` і `text-gray-900`, щоб ціна була більш помітною.

# --hints--

Ваш елемент `p` має мати клас `text-4xl`.
...
Ваш елемент `p` має мати клас `font-bold`.
...
Ваш елемент `p` має мати клас `text-gray-900`.
