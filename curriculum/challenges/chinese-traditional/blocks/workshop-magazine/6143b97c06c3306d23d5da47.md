...
在你的 `article` 元素內，創建一個 `h3` 元素，並將 `class` 設定為 `list-title`，文字為 `A Brief History`。在其下方，創建一個 `p` 元素，文字為 `Of the Curriculum`。然後創建一個 `ul` 元素，並將 `class` 設定為 `lists`。

# --hints--

你應該在你的 `article` 元素內創建一個 `h3` 元素。

```js
assert.exists(document.querySelector('article h3'));
```

你應該在你的 `article` 元素中創建一個 `p` 元素。

```js
assert.exists(document.querySelector('article p'));
```

你應該在你的 `article` 元素中創建一個 `ul` 元素。

```js
assert.exists(document.querySelector('article ul'));
```

你在 `article` 元素中的元素應該保持正確的順序。
