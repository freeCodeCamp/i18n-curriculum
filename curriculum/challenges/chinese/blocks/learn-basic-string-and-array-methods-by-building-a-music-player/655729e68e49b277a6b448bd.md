...
现在你已经创建了 `resetButton`，你需要为其分配 `id` 和 `aria-label` 属性。 JavaScript 提供了你需要为此使用的 `id` 和 `ariaLabel` 属性。

例如，`element.id` 将设置 `id` 属性，而 `element.ariaLabel` 将设置 `aria-label` 属性。 它们都以字符串形式接受它们的值。

将 `resetButton` 元素的 `id` 属性设置为 `"reset"`，并将其 `aria-label` 属性设置为 `"Reset playlist"`。
...
你应该使用 `resetButton.id` 为 `resetButton` 创建一个名为 `"reset"` 的 `id` 属性。

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

你应该使用 `resetButton.ariaLabel` 为 `resetButton` 创建一个名为 `"Reset playlist"` 的 `"aria-label"` 属性。
