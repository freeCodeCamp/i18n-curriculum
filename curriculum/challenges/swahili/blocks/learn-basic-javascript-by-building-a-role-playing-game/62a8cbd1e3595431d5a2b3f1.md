...
Unapaswa kumwambia mchezaji silaha aliyonunua. Kati ya mistari miwili uliyoiandika, tumia `let` kutangaza kigezo kipya kinachoitwa `newWeapon`. Kigezo hiki kilinganishwe na `weapons`.

# --hints--

Kitendakazi chako cha `buyWeapon` kinapaswa kuwa na kigezo kinachoitwa `newWeapon`.

```js
assert.match(buyWeapon.toString(), /newWeapon/);
```

Unapaswa kutumia `let` kutangaza `newWeapon`.

```js
assert.match(code, /let\s+newWeapon/);
```

`newWeapon` inapaswa kuanzishwa na thamani ya `weapons`. Usisahau alama ya nukta.

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

`newWeapon` inapaswa kutangazwa kabla ya kubadilisha `text`.
