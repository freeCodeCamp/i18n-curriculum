...
最後，在你的第二個 `label` 之後的新行中，創建另一個 `input` 元素。為這個元素添加一個 `type` 屬性並設為 `number`，一個 `min` 屬性並設為 `0`（以確保無法添加負數的卡路里），一個 `placeholder` 屬性並設為 `Calories`，以及一個與你第二個 `label` 元素的 `for` 屬性相符的 `id` 屬性。

# --hints--

你應該在你的 `HTMLString` 中有兩個 `input` 元素。
...
你的新 `input` 元素應該放在你的第二個 `label` 元素之後。
...
你的新 `input` 元素應該有一個 `type` 屬性設定為 `number`。
...
你的 `input` 元素應該有一個設定為 `Calories` 的 `placeholder` 屬性。
...
你的 `input` 元素應該有一個 `id` 屬性，設定為 `${entryDropdown.value}-${entryNumber}-calories`。
