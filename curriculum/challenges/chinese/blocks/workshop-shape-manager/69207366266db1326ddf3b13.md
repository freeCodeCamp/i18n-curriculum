...
为根文档添加 `DOMContentLoaded` 事件，并传入 `initializeApp` 作为要运行的回调函数。这确保所有初始化仅在 DOM 完全装载后进行。

# --hints--

你应该为根 `DOMContentLoaded` 添加 `document` 事件。

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

你应该将 `initializeApp` 作为要在事件监听器中运行的函数引用。
