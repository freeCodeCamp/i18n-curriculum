...
最後，在 `.bb2a` 的 `border-bottom` 屬性中，將 `1vw` 改為 `5vh`，並將 `#000` 顏色改為你的 `--building-color2` 變數。這樣就完成了，現在看起來很棒！在整個專案中，隨時可以將你一開始為所有元素添加的 `border` 屬性註解掉或移除，以查看移除後建築物的樣子。

# --hints--

你應該將 `border-bottom` 改為使用 `5vh`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderBottom, "5vh");
```

你應該將 `border-bottom` 改為使用 `--building-color2`。
