...
Infine, all'interno di `hideElements`, passa gli argomenti `singleCard`, `multipleCard`, `fortuneContainer` e `fortuneDescription`. Ricorda di prenderli dagli elementi.

Con questo, la tua app di lettura della fortuna è completa!

# --hints--

Dovresti passare `singleCard` come primo argomento a `hideElements`.
...
Dovresti passare `multipleCard` come argomento a `hideElements`.
...
Dovresti passare `fortuneContainer` come argomento a `hideElements`.
...
Dovresti passare `fortuneDescription` come argomento a `hideElements`.
