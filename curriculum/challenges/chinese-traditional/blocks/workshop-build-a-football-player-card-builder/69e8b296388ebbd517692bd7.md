...
你現在將開始將 position 輸入改為選擇框的處理程序。

將輸入改為一個 `select` 元素，並設定 id 為 `position`、`className` 為 `input`，以及 `value` 為 `player.position`。

# --hints--

你應該將 position `input` 改為一個 `select` 元素，並且有 `id` 為 `position` 以及 `className` 為 `input`。
...
你的 position `select` 應該有一個 `value` 為 `player.position`。
