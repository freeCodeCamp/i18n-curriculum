...
Fassen wir zum Abschluss zusammen. Loggen Sie `catalog.length` und `Object.keys(byDecade).length`, um die Gesamtzahl der Bücher und die Jahrzehntgruppen zu sehen.

Deklarieren Sie `let oldestYear = Infinity` und `let newestYear = 0`. Verwenden Sie eine `for`-Schleife über `catalog`: Für jeden Eintrag, bei dem das Jahr nicht `"Unknown"` ist, aktualisieren Sie `oldestYear`, wenn das Jahr kleiner ist, und `newestYear`, wenn es größer ist. Loggen Sie beide Werte.

Sie haben den Heritage Library Catalog Workshop abgeschlossen!
