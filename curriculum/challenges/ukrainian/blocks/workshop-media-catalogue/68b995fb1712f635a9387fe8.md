...
Створіть ще один метод з назвою `get_tv_series` і зробіть так, щоб він повертав список усіх екземплярів `TVSeries`, які ви знайдете в `self.items`.

# --hints--

Ваш клас `MediaCatalogue` має містити метод з назвою `get_tv_series`.
...
Ваш метод `get_tv_series` має повертати список, що містить усі елементи в `self.items`, які є екземплярами класу `TVSeries`.
