...
Тепер, коли ваша функція `.map()` отримує повернутий референс функції `num => charRange(...).map(...)` від карованих викликів `addCharacters`, вона правильно ітеруватиме над елементами та передаватиме кожен елемент як `n` цій функції.

Ви помітите, що не використовуєте свій параметр `match`. У JavaScript загальною конвенцією є додавати префікс підкреслення `_` до невикористаного параметра. Також можна залишити параметр порожнім, наприклад: `(, char1)`, але часто зрозуміліше назвати параметр для кращої читабельності в майбутньому.

Додайте префікс підкреслення до свого параметра `match`.

# --hints--

Ви повинні додати префікс підкреслення до свого параметра `match`.
