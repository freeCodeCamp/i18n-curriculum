...
创建一个 `random` 属性。 此属性应使用数组中的前两个数字来生成随机整数。 这个数字的范围从两个数字中的第一个数字（包含）开始，到这两个数字之和之前结束。 使用 `Math.floor()` 和 `Math.random()` 方法进行计算。

# --hints--

你的 `spreadsheetFunctions` 对象应该具有 `random` 属性。
...
你的 `random` 函数应该返回数组中前两个数字之间的一个随机数。
