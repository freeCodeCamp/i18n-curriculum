...
ある種のモーションベースのアニメーションは、一部のユーザーに不快感を与える可能性があります。 特に、<dfn>前庭</dfn>疾患の人々は、特定の動きのきっかけに対して感受性があります。
...
`scroll-behavior: smooth` を設定するスタイルルールを `@media` アットルール内でラップし、メディア特性 `prefers-reduced-motion` の値として `no-preference` を設定します。
