...
The `p` elements are nested in an `article` element with the class attribute of `item`. Ви можете стилізувати всі елементи `p`, вкладені в будь-якому елементі класу з назвою `item` ось так:
