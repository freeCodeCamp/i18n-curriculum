...
Dovresti dare a `.fb5` un secondo `repeating-linear-gradient` nella proprietà `background`.
...
Dovresti dare al secondo `repeating-linear-gradient` una direzione di `90deg`.
...
Dovresti dare al secondo `repeating-linear-gradient` un primo colore di `--building-color2` dallo `0%` al `12%`.
...
Dovresti dare al secondo `repeating-linear-gradient` un secondo colore di `--window-color2` dal `12%` al `44%`.
