...
Mwishowe, unapaswa onyesha tena nyimbo, sasisha maandishi ya kitufe cha kucheza yanayopatikana, na ondoa kitufe cha kuweka upya kutoka kwenye orodha ya nyimbo. Pia unahitaji kuondoa `resetButton` kutoka kwenye DOM.

Fanya wito wa kitendakazi cha `renderSongs()` ukiwa na `sortSongs()` kama hoja ili onyesha tena nyimbo kwa mpangilio wa alfabeti.

Fanya wito wa kitendakazi cha `setPlayButtonAccessibleText()` kusasisha maandishi yanayopatikana ya kitufe cha kucheza.

Ondoa kitufe cha kuweka upya kutoka kwenye orodha ya nyimbo kwa kufanya wito wa njia ya `remove()` kwenye kigezo cha `resetButton`.

**Kumbuka**: Sasa unaweza jaribu kuondoa nyimbo zote kuona kinachotokea.

# --hints--

Haupasi kubadilisha msikilizaji wa tukio uliopo na yaliyomo yake.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Unapaswa kufanya wito wa kitendakazi cha `renderSongs` ukiwa na `sortSongs()`.
...
Unapaswa kutumia njia ya `remove()` kuondoa `resetButton` kutoka kwenye DOM.
