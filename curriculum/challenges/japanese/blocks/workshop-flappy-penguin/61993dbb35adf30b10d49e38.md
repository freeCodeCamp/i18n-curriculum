...
`.penguin-body` 要素をターゲットにして、`width` の `53%` と `height` の `45%` を設定してください。それから、`background` を `45deg` での線形グラデーションに設定し、`rgb(134, 133, 133)` からの `0%`、`rgb(234, 231, 231)` からの `25%`、`white` からの `67%` を指定してください。

# --hints--

`.penguin-body` セレクターを使うべきです。

```js
assert.match(code, /\.penguin-body\s*\{/);
```

`.penguin-body` に `width` の `--fcc-expected--` を与えるべきですが、`--fcc-actual--` が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

`.penguin-body` に `height` の `--fcc-expected--` を与えるべきですが、`--fcc-actual--` が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

`.penguin-body` に `background` の `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)` を与えるべきです。
