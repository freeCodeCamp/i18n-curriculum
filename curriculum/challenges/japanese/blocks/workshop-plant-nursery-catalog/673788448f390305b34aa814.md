...
`Set` オブジェクトは、あらゆる型の一意の値を保存できます。`Set` コンストラクタを使って空のセットを作成できます。

```js
const mySet = new Set();
``` 

`displayPlantsSet` という名前の関数を作成し、新しい `Set` オブジェクトを返すようにしてください。

# --hints--

`displayPlantsSet` という名前の関数を作成してください。

```js
assert.isFunction(displayPlantsSet);
```

`displayPlantsSet` 関数は空の `Set` オブジェクトを返す必要があります。
