...
`else`ブロック内で、`currentTime`オブジェクトの`audio`プロパティに`userData?.songCurrentTime`に保存されている値を設定してください。
...
既存の`else`ブロックに`if`ブロックを追加してください。
...
`audio.currentTime`文の`userData?.songCurrentTime`ブロック内で`else`に`if`を設定してください。
