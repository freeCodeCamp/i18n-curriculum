...
あなたの`addCharacters(char1)`は関数を返しており、その関数もさらに別の関数を返しています。`.map()`コールバックのために最も内側の関数の参照にアクセスするには、もう一度関数を呼び出す必要があります。JavaScriptでは、返された関数をすぐに実行することができます。

```js
myFunc(1)("hi");
```

`addCharacters(char1)`の呼び出しから返された関数をすぐに実行し、`char2`を引数として渡してください。

# --hints--

`addCharacters(char1)`の呼び出しに関数呼び出しを連結する必要があります。
...
連結した関数呼び出しに`char2`を引数として渡してください。
