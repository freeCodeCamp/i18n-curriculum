...
لعرض الأغاني في واجهة المستخدم (UI)، تحتاج إلى إنشاء دالة.

استخدم صيغة الدالة السهمية لإنشاء دالة `renderSongs` التي تأخذ `array` كمعلم لها.

# --hints--

يجب استخدام `const` لإنشاء دالة فارغة باسم `renderSongs`.

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

يجب أن تكون `renderSongs` دالة.

```js
assert.isFunction(renderSongs)
```

يجب أن تستخدم دالتك `renderSongs` صيغة الدالة السهمية.

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

يجب أن تأخذ دالتك `renderSongs` المعلمة `array`.

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

يجب أن تكون دالتك `renderSongs` فارغة.
