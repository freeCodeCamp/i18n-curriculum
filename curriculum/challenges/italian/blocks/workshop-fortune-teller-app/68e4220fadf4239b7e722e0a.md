...
Poiché l'app fortune ha solo tre momenti: passato, presente e futuro, dichiara un enum chiamato `DrawingType` con tre proprietà: `Past`, `Present` e `Future`. Ogni proprietà deve avere un valore stringa corrispondente al suo nome.

# --hints--

Dovresti avere `enum` chiamato `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Il tuo enum dovrebbe avere una proprietà `Past` con un valore stringa di `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Il tuo enum dovrebbe avere una proprietà `Present` con un valore stringa di `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Il tuo enum dovrebbe avere una proprietà `Future` con un valore stringa di `future`.
