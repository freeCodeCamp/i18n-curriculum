...
Da Ihre `.map()`-Funktion nun die zurückgegebene `num => charRange(...).map(...)`-Funktionsreferenz von den curryierten `addCharacters`-Aufrufen erhält, wird sie die Elemente korrekt durchlaufen und jedes Element als `n` an diese Funktion übergeben.

Sie werden feststellen, dass Sie Ihren `match`-Parameter nicht verwenden. In JavaScript ist es eine gängige Konvention, einen ungenutzten Parameter mit einem Unterstrich `_` zu präfixen. Sie könnten den Parameter auch leer lassen, zum Beispiel so: `(, char1)`, aber es ist oft klarer, den Parameter für die zukünftige Lesbarkeit zu benennen.

Präfixen Sie Ihren `match`-Parameter mit einem Unterstrich.

# --hints--

Sie sollten Ihren `match`-Parameter mit einem Unterstrich präfixen.
