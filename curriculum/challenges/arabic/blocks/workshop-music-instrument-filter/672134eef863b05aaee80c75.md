...
لتنفيذ وظيفة الفلتر، تحتاج إلى دالة. أعلن دالة فارغة باسم `instrumentCards` تأخذ معلمة واحدة.

# --hints--

يجب أن يكون لديك دالة باسم `instrumentCards`.

```js
assert.isFunction(instrumentCards);
```

يجب أن تأخذ دالتك `instrumentCards` معلمة واحدة.
