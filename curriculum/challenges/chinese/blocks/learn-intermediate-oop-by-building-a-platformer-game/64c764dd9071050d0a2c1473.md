...
在 `animate` 函数中，你需要将每个平台绘制到画布上。

添加一个 `forEach` 循环，迭代 `platforms` 数组。

在回调函数内部，添加一个 `platform` 参数，并在函数主体中调用每个 `platform` 上的 `draw` 方法。

# --hints--

你应该有一个 `forEach` 循环，用于遍历 `platforms` 数组。

```js
assert.match(code, /platforms\.forEach\(\s*(\(.*\)|[^\s()]+)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

你应该向回调函数添加 `platform` 参数。

```js
assert.match(code, /platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

你应该在每个 `platform` 上调用 `draw` 方法。
