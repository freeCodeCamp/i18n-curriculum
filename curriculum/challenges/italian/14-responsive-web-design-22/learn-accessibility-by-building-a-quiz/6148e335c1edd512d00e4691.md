...
Fare clic sui collegamenti di navigazione dovrebbe spostare la vista della pagina alla sezione pertinente. Tuttavia, questo salto può essere disorientante per alcuni utenti.

Seleziona tutti gli elementi e imposta la proprietà `scroll-behavior` su `smooth`.
...
Dovresti dare al selettore `*` una proprietà `scroll-behavior` con il valore `smooth`.
