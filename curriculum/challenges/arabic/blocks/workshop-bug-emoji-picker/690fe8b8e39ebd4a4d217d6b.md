...
في هذه الورشة، ستتعلم عن الأصناف المجردة في TypeScript والأنواع العامة من خلال بناء محدِّد أنواع الأخطاء البرمجية الذي يعرض رموز تعبيرية مختلفة للأخطاء البرمجية.

تم توفير HTML وCSS للمشروع لتتمكن من التركيز على TypeScript. يمكنك مراجعة الملفات لتتعرف على العناصر التي ستختارها لاحقًا.

لتبدأ، تحتاج إلى صنف أساسي مجرد لجميع الأخطاء البرمجية. لا يمكن إنشاء نسخ مباشرة من الأصناف المجردة، وهي مخصصة لأن تُورَّث من أصناف أخرى.

أنشئ صنفًا مجردًا باسم `Bug` مع معلمة نوع عامة `T`.

# --hints--

يجب أن يكون لديك صنف مجرد باسم `Bug`.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+class\s+Bug/);
```

يجب أن يحتوي صنف `Bug` على معلمة نوع عامة `T`.
