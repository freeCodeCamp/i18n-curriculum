...
title: ブートストラップでフォーム要素をレスポンシブに横並びにする
challengeType: 0
forumTopicId: 18225
required:
  - link: >-
...
フォームの`input`と提出ボタンの`button`を同じ行に並べましょう。これまでと同じ方法で行います：`div`クラスを持つ`row`要素を使い、その中に`div`クラスを持つ他の`col-xs-*`要素をネストします。

フォームのテキスト`input`と提出`button`の両方を、`div`クラスを持つ`row`にネストしてください。フォームのテキスト`input`は`col-xs-7`クラスのdivにネストし、フォームの提出`button`は`div`クラスを持つ`col-xs-5`にネストしてください。

これが今のところCat Photo Appの最後のチャレンジです。Font Awesome、ブートストラップ、レスポンシブデザインの学習を楽しんでいただけたことを願っています！

# --hints--

フォームの提出ボタンとテキスト入力は、`row`クラスのdivにネストしてください。
...
フォームのテキスト入力は、`col-xs-7`クラスのdivにネストしてください。
...
フォームの提出ボタンは、`col-xs-5`クラスのdivにネストしてください。
