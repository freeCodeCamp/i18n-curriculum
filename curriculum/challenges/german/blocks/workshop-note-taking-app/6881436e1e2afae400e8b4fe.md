...
Im Moment wird die Variable `currentContent` jedes Mal aktualisiert, wenn der Benutzer eine Notiz bearbeitet und außerhalb der Notiz klickt. Aber wie sieht es mit Situationen aus, in denen der Benutzer den Fokus auf der Notiz hat und entscheidet, ohne Änderungen zu verlassen?

In diesem Fall können Sie eine Bedingung hinzufügen, um diesen Anwendungsfall zu behandeln.

Fügen Sie oberhalb Ihrer `currentContent = newContent;`-Zeile eine `if`-Anweisung hinzu, um zu prüfen, ob `currentContent` gleich `newContent` ist. Wenn ja, geben Sie zurück.

# --hints--

Sie sollten eine `if`-Anweisung hinzufügen, um zu prüfen, ob `currentContent` gleich `newContent` ist. Wenn ja, geben Sie zurück.
