...
Deine abschließende Berechnung ist die <dfn>standard deviation</dfn>, welche die Quadratwurzel der Varianz ist.
...
Du solltest eine `getStandardDeviation`-Funktion haben.

```js
assert.isFunction(getStandardDeviation);
```

Deine `getStandardDeviation`-Funktion sollte einen `array`-Parameter enthalten.

```js
assert.match(getStandardDeviation.toString(), /array/);
```

Deine `getStandardDeviation`-Funktion sollte eine `variance`-Variable enthalten.
