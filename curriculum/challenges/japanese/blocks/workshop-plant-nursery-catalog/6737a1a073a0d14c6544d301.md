...
`plantsSet` という名前の変数を宣言し、`displayPlantsSet` 関数の呼び出しを代入してください。

その後、`plantsSet` をコンソールに表示してください。表示を見るにはブラウザのコンソールを開く必要があります。


# --hints--

`plantsSet` という名前の変数が必要です。

```js
assert.exists(plantsSet);
```

`displayPlantsSet` を呼び出し、その結果を `plantsSet` に代入してください。

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+plantsSet\s*=\s*displayPlantsSet\s*\(\s*\)/)
```

`plantsSet` をコンソールに表示してください。
