...
現在你的 `decimalToBinary` 函式已完成。隨意試玩它。

但是有一些方法可以改進它。 例如，不需要追蹤輸入和商數。 我們可以整理一下，使函式（程式）更高效。

首先，移除 `decimalToBinary` 函式（程式）主體中的所有內容。然後，使用 `let` 創建一個名為 `binary` 的變數並指定為空字串。

# --hints--

你應該使用 `let` 來創建 `binary` 變數。

```js
assert.match(code, /let\s+binary\s*/);
```

你應該在你的 `decimalToBinary` 函式（程式）內創建一個名為 `binary` 的變數。
...
你應該從你的 `decimalToBinary` 函式（程式）主體中移除其他所有內容。
