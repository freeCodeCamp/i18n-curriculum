...
الآن بعد أن أضفت الكلمة المفتاحية `!important`، يمكنك إزالة `:not(.sr-only)` من محدِّد `span` الخاص بك.

# --hints--

يجب ألا يكون لديك محدِّد `span:not(.sr-only)`.

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)'));
```

يجب أن يكون لديك محدِّد `span`.

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('span'));
```

يجب ألا تغيّر `font-weight`.
