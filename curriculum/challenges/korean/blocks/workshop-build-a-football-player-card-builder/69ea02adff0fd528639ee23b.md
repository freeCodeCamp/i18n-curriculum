...
다음으로, 반환 타입이 `loadPlayer`인 `PlayerData` 함수를 만드세요. 그 안에 `try...catch` 블록의 매개변수로 `error`를 사용하는 `catch` 구문을 추가하세요. 나머지는 모두 비워 두세요.

# --hints--

`loadPlayer`라는 함수를 만들어야 합니다.
...
`loadPlayer` 함수는 `PlayerData` 반환 타입을 가져야 합니다.
...
`loadPlayer` 함수는 `try...catch` 블록을 포함해야 합니다.
