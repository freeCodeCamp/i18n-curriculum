...
`#monsterStats` 要素に、`border` 要素と同じ `padding` と `#stats` を設定してください。色は `#ffffff` にし、`#c70d0d` の背景を付けてください。
...
`#monsterStats` セレクターには `border` の `1px solid #0a0a23` があるはずです。
...
`#monsterStats` セレクターには `5px` のパディングがあります。
...
`#monsterStats` セレクターには `color` の `#ffffff` があります。
...
`#monsterStats` セレクターには `background-color` の `#c70d0d` があります。
