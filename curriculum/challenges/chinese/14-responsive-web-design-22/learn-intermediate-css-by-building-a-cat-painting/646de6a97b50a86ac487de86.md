...
使用类选择器，将 `.cat-right-eye` 元素的宽度设为 `30px`、高度设为 `40px`。 另外，给它一个背景颜色为 `#000`。

# --hints--

你应该有一个 `.cat-right-eye` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye'))
```

你的 `.cat-right-eye` 选择器应将 `width` 设置为 `30px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.width === '30px')
```

你的 `.cat-right-eye` 选择器应将 `height` 设置为 `40px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.height === '40px')
```

你的 `.cat-right-eye` 选择器应将 `background-color` 设置为 `#000`。
