...
나머지 인수 `chosenCard.name_short`와 `chosenCard.img`을 포함하세요. 그런 다음 `renderCard` 함수 밖에서 `multipleCard` 요소의 `innerHTML`를 빈 문자열로 재설정하세요.

# --hints--

`chosenCard.name_short`를 `renderCard`에 인수로 전달해야 합니다.
...
`chosenCard.img`를 `renderCard`의 마지막 인수로 전달해야 합니다.
...
`this.elements.multipleCard.innerHTML`를 빈 문자열로 재설정해야 합니다.
