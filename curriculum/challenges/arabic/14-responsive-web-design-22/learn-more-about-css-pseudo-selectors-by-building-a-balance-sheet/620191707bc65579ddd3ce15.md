...
يجب أن يكون لديك منتقي `tr[class="total"] th`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"] th'));
```

يجب يحتوي منتقي `tr[class="total"] th` الخاص بك على خاصية `text-align` بقيمة `left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr[class="total"] th')?.getPropertyValue('text-align') === 'left');
```

يجب أن يكون لمنتقي `tr[class="total"] th` خاصية `padding` بقيمة `0.5rem 0 0.25rem 0.5rem`.
