...
填写测验内容，在 `#student-info` 下房添加三个 `class` 为 `info` 的 `div` 元素。
...
你应该在第一个 `div` 中的 `label` 后面嵌套一个 `input` 元素。
...
你应该在第二个 `div` 中的 `label` 后面嵌套一个 `input` 元素。
...
你应该在第三个 `div` 中的 `label` 后面嵌套一个 `input` 元素。
