...
Nachdem Sie nun alle Blogbeiträge hinzugefügt haben, versuchen Sie, auf den `Posts`-Link zu klicken. Sie sollten sehen, dass die Seite zum `Posts`-Abschnitt springt.

Die letzte Komponente, die Sie zu Ihrer Blogseite hinzufügen, wird der Kontakt-Abschnitt sein.

Fügen Sie unter dem `main`-Element ein `footer`-Element hinzu.

# --hints--

Ihr `footer`-Element sollte ein öffnendes Tag haben.

```js
assert.match(code, /<footer\>/);
```

Ihr `footer`-Element sollte ein schließendes Tag haben.

```js
assert.match(code, /<\/footer\>/);
```

Ihr `footer`-Element sollte sich innerhalb Ihres `body`-Elements befinden.
