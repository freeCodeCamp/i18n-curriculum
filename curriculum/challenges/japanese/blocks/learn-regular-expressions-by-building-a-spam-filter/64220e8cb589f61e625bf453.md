...
数字と`dollars`テキストの間に、位取りをキャッチしたいです。

`|`トークンを使って、数字と`hundred`の間に`thousand`、`million`、`billion`、または`dollars`を許可してください。

# --hints--

`dollarRegex`は`|`トークンを使うべきです。

```js
assert.match(dollarRegex.source, /\|/);
```

`dollarRegex`は3つの`|`トークンを持つべきです。

```js
assert.lengthOf(dollarRegex.source.match(/\|/g), 3);
```

`dollarRegex`は`|`トークンを使って、`hundred`、`thousand`、`million`、または`billion`にマッチさせてください。
...
`dollarRegex`は`1 hundred dollars`にマッチさせてください。

```js
assert.match('1 hundred dollars', dollarRegex);
```

`dollarRegex`は`1 thousand dollars`にマッチさせてください。

```js
assert.match('1 thousand dollars', dollarRegex);
```

`dollarRegex`は`1 million dollars`にマッチさせてください。

```js
assert.match('1 million dollars', dollarRegex);
```

`dollarRegex`は`1 billion dollars`にマッチさせてください。
