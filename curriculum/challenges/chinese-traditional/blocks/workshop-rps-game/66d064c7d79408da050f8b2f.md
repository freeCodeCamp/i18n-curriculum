...
下一步是創建負責追蹤玩家和電腦分數的變數。

創建一個名為 `playerScore` 的變數，並將其初始化為值 `0`。

然後，創建一個名為 `computerScore` 的變數並將其初始化為值 `0`。

你需要使用 `let` 來宣告這些變數，因為它們的值會在整個遊戲中改變。

# --hints--

你應該有一個名為 `playerScore` 的變數。

```js
assert.isNotNull(playerScore);
```

你應該使用 `let` 來宣告 `playerScore` 變數。

```js
assert.match(code, /let\s+playerScore/);
```

你的 `playerScore` 應該初始化為值 `0`。

```js
assert.strictEqual(playerScore, 0);
```

你應該有一個名為 `computerScore` 的變數。

```js
assert.isNotNull(computerScore);
```

你應該使用 `let` 來宣告 `computerScore` 變數。

```js
assert.match(code, /let\s+computerScore/);
```

你的 `computerScore` 應該初始化為值 `0`。
