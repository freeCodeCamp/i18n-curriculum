...
為你的程式碼添加一個 `console.log(currentContent);`。 

要測試你的更改，請嘗試編輯筆記，然後點擊筆記外的區域。你現在應該會在主控台中看到已編輯的筆記。 

# --hints--

你應該在事件監聽器內，將 `newContent` 指定給 `currentContent` 之後，添加一個 `console.log(currentContent);`。
