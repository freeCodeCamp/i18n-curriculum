...
بالنسبة لعناصر `span` داخل كل عنصر قائمة، عيّن لها الأصناف `text-green-500` و `mr-2` لجعل العلامات خضراء ودفعها قليلاً بعيدًا عن النص.

أيضًا، عيّن لها الخاصية `aria-hidden` كما فعلت مع عناصر `span` في خطة التسعير الأولى.

# --hints--

يجب أن تحتوي عناصر `span` على الصنف `text-green-500`.
...
يجب أن تحتوي عناصر `span` على الصنف `mr-2`.
...
يجب أن تحتوي عناصر `span` على الخاصية `aria-hidden`.
