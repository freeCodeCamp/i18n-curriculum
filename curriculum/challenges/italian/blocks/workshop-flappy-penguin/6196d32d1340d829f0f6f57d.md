...
Dovresti dare all'elemento `.sun` una proprietà `position` con il valore `absolute`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.position, 'absolute');
```

Dovresti dare all'elemento `.sun` una proprietà `border-radius` con il valore `50%`.
