...
Como o projeto agora trabalha com elementos HTML DOM, você precisa de uma forma de renderizar a fortuna no DOM.

Declare uma função chamada `renderCard` que receba quatro argumentos: `drawingType`, `isReversed`, `shortName` e `img`.

Os parâmetros `drawingType`, `shortName` e `img` devem ser do tipo `string`, enquanto `isReversed` deve ser do tipo `boolean`. A função deve ter um tipo de retorno `string`.

# --hints--

Você deve ter uma função chamada `renderCard`.
...
Sua função `renderCard` deve ter quatro parâmetros.
...
Seu primeiro parâmetro deve ser `drawingType` do tipo `string`.
...
Seu segundo parâmetro deve ser `isReversed` do tipo `boolean`.
...
Seu terceiro parâmetro deve ser `shortName` do tipo `string`.
...
Seu quarto parâmetro deve ser `img` do tipo `string`.
...
Sua função `renderCard` deve ter um tipo de retorno `string`.
...
Sua função `renderCard` deve retornar um template literal.
