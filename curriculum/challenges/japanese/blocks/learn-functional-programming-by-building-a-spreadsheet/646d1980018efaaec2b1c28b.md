...
`addCharacters` 関数と同じ構文を使って `elemValue` 関数を更新してください。`inner` を宣言しなくなり、関数を暗黙的に返すようにしてください。

# --hints--

`elemValue` 関数は暗黙の返り値を使うべきです。
...
`elemValue` 関数は `character` パラメータを持つアロー関数を暗黙的に返すべきです。
...
内側のアロー関数は `idToText()` を引数にして `character + num` を呼び出した結果を返すべきです。
