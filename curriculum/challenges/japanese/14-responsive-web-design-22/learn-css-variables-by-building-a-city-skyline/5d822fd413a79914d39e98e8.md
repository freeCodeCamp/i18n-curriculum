...
`.foreground-buildings` の `display` プロパティを `flex` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.display, 'flex');
```

`.foreground-buildings` の `align-items` プロパティを `flex-end` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.alignItems, 'flex-end');
```

`.foreground-buildings` の `justify-content` プロパティを `space-evenly` に設定する必要があります。
