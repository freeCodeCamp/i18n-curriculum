...
Sasa unahitaji kuongeza sifa ya kipengele tena kwenye wimbo unaochezwa kwa sasa.

Tengeneza tamko la `if` lenye sharti la `songToHighlight`. Kwa tamko hilo, tumia `setAttribute` kwenye `songToHighlight` ili kupitisha `"aria-current"` na `"true"` kama hoja ya kwanza na ya pili.

# --hints--

Unapaswa kutengeneza tamko la `if` lenye sharti la `songToHighlight`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

Unapaswa kutumia njia ya `setAttribute()` kwenye `songToHighlight` ndani ya tamko lako la `if`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

Unapaswa kupitisha `"aria-current"` na `"true"` kama hoja ya kwanza na ya pili ya njia yako ya `setAttribute`.
