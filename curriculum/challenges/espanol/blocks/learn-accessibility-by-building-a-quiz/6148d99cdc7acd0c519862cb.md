...
Apunta a todos los elementos `label` dentro de los elementos `.info`, y establece su propiedad `width` en `10%`, y asegúrate de que no ocupen menos de `55px`.

# --hints--

Debes usar el selector `.info label` o `.info > label`.
...
Debes asignar a los elementos `label` un `width` de `10%`.
...
Debes asignar a los elementos `label` un `min-width` de `55px`.
