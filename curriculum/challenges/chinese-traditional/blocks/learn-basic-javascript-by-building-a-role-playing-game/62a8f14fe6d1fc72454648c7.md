...
當怪物被擊敗後，怪物的屬性框應該不再顯示。

在 `update` 函式（程式）的第一行，使用 `monsterStats.style.display` 將 `display` 值更改為 `none`。

# --hints--

你應該在 `update` 函式中使用 `monsterStats.style.display`。

```js
assert.match(update.toString(), /monsterStats\.style\.display/);
```

你應該將 `monsterStats.style.display` 設定為 `none`。

```js
assert.match(update.toString(), /monsterStats\.style\.display\s*=\s*('|")none\1/);
```

你應該將你的程式碼添加到 `update` 函式的第一行。
