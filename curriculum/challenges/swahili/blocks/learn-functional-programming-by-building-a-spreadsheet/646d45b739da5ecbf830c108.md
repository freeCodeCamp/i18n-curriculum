...
Kitendakazi chako cha `apply` kinarejesha kitendakazi cha karatasi ya kazi, lakini hakitumiwi moja kwa moja. Sasisha `apply` kuitisha kitendakazi hicho. Pitia matokeo ya kuitisha `toNumberList` ukiwa na `args` kama hoja.

# --hints--

Kitendakazi chako cha `apply` kinapaswa kuitisha kitendakazi cha `spreadsheetFunctions[fn.toLowerCase()]`.
...
Unapaswa kupitisha wito wa `toNumberList()` kwa wito wako wa `spreadsheetFunctions[fn.toLowerCase()]`.
...
Unapaswa kupitisha `args` kwa wito wako wa `toNumberList()`.
