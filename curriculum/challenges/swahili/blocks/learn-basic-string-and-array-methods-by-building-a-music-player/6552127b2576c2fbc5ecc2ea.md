...
Ongeza kipande cha `else`. Ndani ya kipande cha `else`, fanya wito wa kitendakazi cha `playSong` ukiwa na `id` ya wimbo unaocheza kwa sasa kama hoja. 
...
Unapaswa kuongeza kipande cha `else` kwenye tamko lako la `if`.

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{\s*playSong\(\s*userData\?\.songs\s*\[\s*0\s*\]\s*\.id\s*\)\s*;?\s*\}\s*else\s*\{/)
```

Unapaswa kufanya wito wa kitendakazi cha `playSong` ukiwa na `userData?.currentSong.id` ndani ya kipande cha `else` cha tamko lako la `if`.
