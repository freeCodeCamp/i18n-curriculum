...
تحتاج إلى طريقة للوصول الآمن إلى عنصر select والحماية من قيم `null`. لذلك، أنشئ ثابتًا باسم `val` بقيمة `e.currentTarget` وحدد نوعه `HTMLSelectElement` باستخدام الكلمة المفتاحية `as`.

أسفل الثابت، استخدم جملة `if` للتحقق مما إذا كانت قيمة `val` خاطئة عبر نفيها. في كتلة الـ `if`، أرجع `target value not found`.

# --hints--

يجب أن يكون لديك ثابت يسمى `val`.

```js
assert.match(code, /const\s+val/)
```

يجب أن يحتوي ثابت `val` على قيمة `e.currentTarget` مع تحديد النوع `HTMLSelectElement` باستخدام الكلمة المفتاحية `as`.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

يجب أن يكون لديك جملة `if` تتحقق مما إذا كانت `val` تساوي null.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

يجب أن تُرجع جملة الـ `if` الخاصة بك `target value not found`.
