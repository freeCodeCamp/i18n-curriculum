...
يجب عليك إعطاء `.fb3-window` خاصية `width` بقيمة `25%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

يجب عليك إعطاء `.fb3-window` خاصية `height` بقيمة `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

يجب عليك إعطاء `.fb3-window` خاصية `background-color` بقيمة `--window-color1`.
