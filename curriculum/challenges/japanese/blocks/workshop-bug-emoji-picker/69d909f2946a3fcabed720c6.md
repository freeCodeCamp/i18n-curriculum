...
`if` チェックができたので、`e.target.value` を使って `bugMap` の中から対応するバグを探し、そのバグの `render` メソッドを呼び出してください。その後、バグを選択してすぐに結果を確認できるはずです。

これで Bug Emoji Picker ワークショップは完成です！

# --hints--

対応するバグに対して `render` メソッドを呼び出す必要があります。バグは `bugMap` の中にあります。
