...
当我们尝试无效的输入时，会在控制台看到记录了 `false`。既然我们知道输入无效，就来报告这个无效状态：
...
结果，你会看到浏览器的错误信息“请匹配请求的格式。”

它会立即报告无效状态，而不是等我们提交表单。但它仍然使用默认消息。这是因为 `reportValidity()` 方法只是告诉浏览器输入无效，浏览器仍然决定如何显示无效原因。这时就用到了 `setCustomValidity()` 方法。
...
有几个有用的属性，它们的值都是布尔类型的 `true` 或 `false`。

你可以自己进一步探索的一些有用属性包括 `valueMissing`，当必填输入字段为空时为 `true`，以及 `patternMismatch`，当值不匹配指定的正则表达式模式时为 `true`。
