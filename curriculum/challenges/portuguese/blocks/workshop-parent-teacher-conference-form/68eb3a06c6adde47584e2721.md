...
Hora de estilizar o botão de envio.

Selecione o elemento com a classe `submit-btn`. Defina `cursor` como `pointer` (para indicar que é clicável), `background-color` como `royalblue`, `color` como `whitesmoke`, `border` como `none`, `border-radius` como `6px` e `padding` como `12px 20px`.

# --hints--

Você deve direcionar o elemento com a classe `submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Defina a propriedade `cursor` como `pointer`.
...
Defina a propriedade `background-color` para `royalblue`.
...
Defina a propriedade `border` como `none`.
...
Defina a propriedade `border-radius` para `6px`.
...
Defina a propriedade `padding` para `12px 20px`.
