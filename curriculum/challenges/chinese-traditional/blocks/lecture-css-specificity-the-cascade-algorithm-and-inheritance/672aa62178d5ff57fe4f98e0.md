...
內部 CSS 定義在 HTML `document` 的 `head` 區段中的 `style` 元素內。它的特異性低於行內樣式。內部和外部 CSS 具有相同的特異性層級——特異性由所使用的選擇器決定，而非 CSS 定義的位置。當兩個規則具有相等的特異性時，文件中後出現的規則會勝出。因此，如果外部樣式表在內部 `style` 區塊之後被連結，外部規則具有優先權。如果它在之前被連結，則內部規則勝出。
