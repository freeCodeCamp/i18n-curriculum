...
`.shirt` の `font-size` を `--fcc-expected--` に設定する必要がありますが、`--fcc-actual--` となっています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontSize, '25px');
```

`.shirt` の `font-family` を `--fcc-expected--` に設定する必要がありますが、`--fcc-actual--` となっています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontFamily, 'Helvetica, sans-serif');
```

`.shirt` の `font-weight` を `--fcc-expected--` に設定する必要がありますが、`--fcc-actual--` となっています。
