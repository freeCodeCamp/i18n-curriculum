...
Verwenden Sie die `add`-Methode von `catalogue`, um `series1` an die `items`-Liste anzuhängen.

Erstellen Sie dann eine weitere Variable `series2`. Weisen Sie ihr eine weitere Instanz von `TVSeries` zu und hängen Sie `series2` ebenfalls an den Katalog an.

# --hints--

Sie sollten die `add`-Methode von `catalogue` verwenden und `series1` übergeben.
...
Sie sollten eine Variable namens `series2` erstellen und ihr eine Instanz von `TVSeries` zuweisen.
...
Sie sollten die `add`-Methode von `catalogue` verwenden und `series2` übergeben.
