...
1. Você deve ter uma função `getBookmarks` que retorna o array `bookmarks` armazenado no armazenamento local. Se a chave `bookmarks` ainda não foi definida, ou não contém um array válido com bookmarks, a função `getBookmarks` deve retornar um array vazio.
