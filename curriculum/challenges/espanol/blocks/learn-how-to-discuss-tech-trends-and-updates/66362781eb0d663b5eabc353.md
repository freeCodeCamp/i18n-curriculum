...
`Serverless computing` es un modelo de ejecución de `cloud computing` donde el proveedor de la nube ejecuta el servidor y gestiona dinámicamente la asignación de recursos de la máquina. El precio se basa en la cantidad real de recursos consumidos por una aplicación, en lugar de en unidades de capacidad precompradas. Se llama `serverless` porque los desarrolladores no tienen que manejar servidores, simplemente cargan su código, y el proveedor de la nube se encarga del resto.
