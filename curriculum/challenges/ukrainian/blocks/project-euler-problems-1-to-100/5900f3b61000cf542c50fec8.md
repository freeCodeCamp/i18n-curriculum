...
title: 'Завдання 73: підрахунок дробів у проміжку'
challengeType: 1
forumTopicId: 302186
...
Розглянемо дріб, $\frac{n}{d}$, де `n` і `d` — натуральні числа. Якщо `n` &lt; `d` і найбільший спільний дільник, ${HCF}(n, d) = 1$, такий дріб називається скороченим власним дробом.
...
Можна побачити, що між $\frac{1}{3}$ і $\frac{1}{2}$ знаходиться `3` дробів.

Скільки дробів лежить між $\frac{1}{3}$ і $\frac{1}{2}$ у впорядкованій множині скорочених власних дробів для `d` ≤ `limit`?

# --hints--

`countingFractionsInARange(8)` має повертати число.

```js
assert(typeof countingFractionsInARange(8) === 'number');
```

`countingFractionsInARange(8)` має повертати `3`.

```js
assert.strictEqual(countingFractionsInARange(8), 3);
```

`countingFractionsInARange(1000)` має повертати `50695`.

```js
assert.strictEqual(countingFractionsInARange(1000), 50695);
```

`countingFractionsInARange(6000)` має повертати `1823861`.

```js
assert.strictEqual(countingFractionsInARange(6000), 1823861);
```

`countingFractionsInARange(12000)` має повертати `7295372`.
