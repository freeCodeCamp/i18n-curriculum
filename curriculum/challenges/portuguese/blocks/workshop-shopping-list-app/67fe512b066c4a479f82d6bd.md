...
Dentro do elemento `li`, acima do texto `item`, adicione um elemento `input` com o atributo `type` definido como `checkbox`. Além disso, defina seu atributo `onChange` para uma função anônima que chama `toggleItem()` com `item` como argumento.

Finalmente, envolva o texto `input` e `item` em um elemento `label` para melhor acessibilidade.

# --hints--

Você deve adicionar outro elemento `input` com o atributo `type` definido como `checkbox`.
...
Seu novo `input` deve ter um atributo `onChange` definido para uma função anônima que chama `toggleItem()` com `item` como argumento.
...
Seu novo `input` e o texto do `item` devem estar envolvidos em um elemento `label`.
