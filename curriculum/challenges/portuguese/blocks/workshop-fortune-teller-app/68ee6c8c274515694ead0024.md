...
A sorte deve ser aleatória, então é necessário uma função auxiliar para selecionar uma sorte aleatória. Declare uma variável chamada `chosenCard` dentro do método `singleCardSelected`. Use o método `getRandomItem` para obter uma carta aleatória e passe `cards` como argumento.

# --hints--

Você deve declarar uma variável chamada `chosenCard` dentro de `singleCardSelected`.
...
Sua variável `chosenCard` deve ser atribuída pela chamada de `getRandomItem(this.cards)`.
...
Você deve passar `this.cards` como argumento para `getRandomItem`.
