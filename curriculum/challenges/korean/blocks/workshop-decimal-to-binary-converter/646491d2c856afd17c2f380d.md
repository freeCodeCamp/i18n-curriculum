...
애니메이션 각 프레임의 타이밍이 `addElDelay`에 저장되어 있으므로, 이 값을 `setTimeout()`과 함께 사용해 DOM에 요소를 추가하는 지연 시간을 설정할 수 있습니다.

`.forEach()` 메서드의 콜백 함수 본문 안에 `setTimeout()` 함수를 추가하세요. 첫 번째 인수로 빈 콜백 함수를 전달하고, 두 번째 인수로 `obj.addElDelay`를 전달하세요.

# --hints--

`setTimeout()` 메서드의 콜백 함수 본문 안에 `.forEach()` 함수를 추가해야 합니다.

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\([\s\S]+\{\s*setTimeout\(/);
```

`setTimeout()` 함수의 첫 번째 인수로 빈 콜백 함수를 전달해야 합니다.
...
`setTimeout()` 함수의 콜백 함수 본문은 비워 두어야 합니다.

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\([\s\S]+\{\s*setTimeout\([\s\S]+\{\s*\}/);
```

`obj.addElDelay` 함수의 두 번째 인수로 `setTimeout()`를 전달해야 합니다.
