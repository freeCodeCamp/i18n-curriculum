...
Kitu cha kwanza unachopaswa kufanya ni kukagua kama safu ya data `items` ni tupu. Ikiwa ni tupu, onyesha `alert` kwa mtumizi na maandishi `Your shopping cart is already empty`, kisha rudi kutoka kwenye kitendakazi.

Kumbuka kuwa `0` ni thamani ya uongo, hivyo unaweza kutumia kielezi `!` kukagua kama safu ya data ni tupu.

Baada ya kuonyesha onyo, rudi kutoka kwenye kitendakazi ili kuacha utekelezaji.

# --hints--

Unapaswa kuunda tamko la `if` linalokagua kama safu ya data `items` ni tupu, ukitumia kielezi cha kukanusha.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

Tamko lako la `if` linapaswa kuonyesha `alert` kwa mtumizi na maandishi `Your shopping cart is already empty`.
...
Tamko lako la `if` linapaswa kurudisha kutoka kwenye kitendakazi.
