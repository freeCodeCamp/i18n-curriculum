...
Die <dfn>find()</dfn>-Methode ruft das erste Element in einem Array ab, das die in der bereitgestellten Callback-Funktion angegebenen Bedingungen erfüllt. Wenn kein Element die Bedingung erfüllt, gibt die Methode `undefined` zurück.

Im folgenden Beispiel wird die `find()`-Methode verwendet, um die erste Zahl zu finden, die größer als 25 ist:
...
Verwenden Sie `const`, um eine Variable namens `song` zu erstellen und ihr das Ergebnis des Aufrufs der `find()`-Methode auf dem `userData?.songs`-Array zuzuweisen. Verwenden Sie `song` als Parameter der `find()`-Callback-Funktion und prüfen Sie, ob `song.id` strikt gleich `id` ist.

Dies wird das `userData?.songs`-Array durchlaufen und nach einem Lied suchen, das dem in die `id`-Funktion übergebenen `playSong` entspricht.

# --hints--

Sie sollten die `find`-Methode auf `userData?.songs` verwenden.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

Ihre `find`-Methode sollte `song` als Parameter ihrer Pfeilfunktion-Callback haben.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

Ihre `find`-Methode sollte strikte Gleichheit verwenden, um zu prüfen, ob `song.id` gleich `id` ist.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

Ihre `find`-Methode sollte einer `song`-Konstanten zugewiesen werden.
