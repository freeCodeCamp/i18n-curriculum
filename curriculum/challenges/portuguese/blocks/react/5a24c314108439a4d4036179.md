...
**Nota:** Você também deve chamar `event.preventDefault()` no manipulador de envio, para evitar o comportamento padrão de envio do formulário que atualizará a página web. Por conveniência, o comportamento padrão foi desabilitado aqui para impedir que atualizações redefinam o código do desafio.
