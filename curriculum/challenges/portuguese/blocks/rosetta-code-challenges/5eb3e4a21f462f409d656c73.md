...
Existem várias maneiras de gerar uma sequência autorreferencial.Um muito comum (a <a href="https://rosettacode.org/wiki/Look-and-say_sequence" target="_blank" rel="noopener noreferrer nofollow">sequência Look-and-say</a>) é começar com um número inteiro positivo, depois gerar o próximo termo concatenando grupos enumerados de dígitos adjacentes iguais:
...
Dependendo do valor da seed, séries geradas dessa forma sempre convergem para um valor estável ou para um padrão cíclico curto. (Para nossos propósitos, convergir significa que um elemento corresponde a um elemento visto anteriormente.) A sequência mostrada, com um valor de seed de 0, converge para um valor estável de 1433223110 após 11 iterações.O valor de seed que tem a conversão mais rápida é 22.Ele se estabiliza após o primeiro elemento. (O próximo elemento é 22, que já foi visto antes.)
