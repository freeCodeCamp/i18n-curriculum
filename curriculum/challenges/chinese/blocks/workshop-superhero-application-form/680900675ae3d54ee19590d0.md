...
要修复该错误，请创建一个带有 `e` 参数的 `handlePowersChange` 箭头函数。

在函数内部，从 `e.target` 中解构 `value` 和 `checked` 以获取复选框的值以及它是否被选中。

# --hints--

你应该创建一个带有 `e` 参数的 `handlePowersChange` 函数。

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

你应该从 `e.target` 中解构 `value` 和 `checked`。
