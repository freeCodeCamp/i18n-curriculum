...
`By itself` means something happens automatically, without anyone needing to do it. 例如：

`The layout updates by itself when the window size changes.` – It changes automatically.

`Nicer` is the comparative form of `nice`. It means something is better, more comfortable, or more pleasant than something else. 例如：
