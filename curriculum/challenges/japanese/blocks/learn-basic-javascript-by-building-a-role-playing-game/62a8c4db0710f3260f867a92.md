...
プレイヤーが体力を買うのに十分なゴールドを持っていなかったらどうしますか？プレイヤーが体力を買うのに十分なゴールドを持っているかどうかを確認するために、`if` 文を使う必要があります。

前のプロジェクトでは、このような `if` 文の使い方を学びました。
...
まず、`buyHealth` 関数内のすべてのコードを `if` 文の中に入れてください。`if` 文の条件には、`gold` が `10` 以上かどうかをチェックします。

# --hints--

あなたの `buyHealth` 関数には `if` 文が必要です。

```js
assert.match(buyHealth.toString(), /if/);
```

あなたの `if` 文は `gold` が `10` 以上かどうかをチェックしてください。

```js
assert.match(buyHealth.toString(), /if\s*\(\s*gold\s*>=\s*10\s*\)/);
```

すべての `buyHealth` コードは `if` 文の中に入れてください。
