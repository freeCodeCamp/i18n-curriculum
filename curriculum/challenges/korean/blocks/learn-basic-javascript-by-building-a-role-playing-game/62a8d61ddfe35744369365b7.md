...
`currentWeapon` 후에 연결 연산자를 사용해 `text.innerText`에 문자열 `"You sold a "`를, 그다음 `currentWeapon`를, 그리고 문자열 `"."`를 차례로 할당하세요.

# --hints--

`text.innerText`에 대입 연산자를 사용하세요.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=/);
```

`"You sold a "`에 `text.innerText`를 추가하세요. 띄어쓰기가 중요합니다.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1/);
```

`currentWeapon` 문자열에 `"You sold a "` 값을 추가하세요. 같은 줄에서 연결 연산자를 사용하세요.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a\s\1\s*\+\s*currentWeapon/);
```

`"."` 값에 문자열 `currentWeapon`를 추가하세요. 같은 줄에서 연결 연산자를 사용하세요.

```js
assert.match(sellWeapon.toString(), /text\.innerText\s*=\s*('|")You sold a \1\s*\+\s*currentWeapon\s+\+\s+('|")\.\2/);
```

`text.innerText`가 올바른 문자열로 업데이트되어야 합니다.
