...
Ili kuhifadhi wakati wa sasa wa wimbo wakati umeachwa, weka thamani ya `songCurrentTime` ya kitu cha `userData` kuwa `currentTime` ya mabadiliko ya `audio`.

*Kumbuka*: Haufai kutumia uunganishaji wa mfululizo kwa hatua hii kwa sababu `userData.songCurrentTime` haitakuwa `null` au `undefined` kwa wakati huu.

# --hints--

Haufai kubadilisha kitendakazi cha `pauseSong` kilichopo na yaliyomo ndani yake.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Unapaswa kuweka thamani ya `audio.currentTime` kwa `userData.songCurrentTime`.
