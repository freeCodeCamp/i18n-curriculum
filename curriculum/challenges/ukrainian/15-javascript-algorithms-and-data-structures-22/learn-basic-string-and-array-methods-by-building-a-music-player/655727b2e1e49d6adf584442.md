...
Якщо список відтворення порожній, вам потрібно створити елемент `resetButton` та текст для нього. Ця кнопка відображатиметься, лише тоді, коли список відтворення порожній.

`createElement()` — це метод DOM, який можна використовувати для динамічного створення елемента за допомогою JavaScript. Щоб використати `createElement()`, ви викликаєте його, а потім передаєте назву тегу як рядок:
...
Ви також можете призначити його до змінної:

```js
const divElement = document.createElement('div')
```

У інструкції `if` оголосіть константу `resetButton`, а потім скористайтеся `createElement()`, щоб створити `"button"`.


# --hints--

Ви не повинні змінювати наявну інструкцію `if` та її вміст.

```js
assert.match(code, /if\s*\(\s*userData\?\.songs\.length\s*===\s*0\s*\)\s*\{\s*.*\s*\}/)
```

Використайте метод `createElement`, щоб створити кнопку `"button"`.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

Призначте щойно створений елемент `button` до константи `resetButton`.
