...
Ahora necesitas añadir el atributo de nuevo a la canción que se está reproduciendo actualmente.

Crea una declaración `if` con la condición `songToHighlight`. Para la declaración, usa `setAttribute` en `songToHighlight` para pasar "aria-current" y "true" como el primer y segundo argumento.

# --hints--

Debes crear una declaración `if` con la condición `songToHighlight`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*/)
```

Debes usar el método `setAttribute()` en `songToHighlight` dentro de tu declaración `if`.

```js
assert.match(code, /if\s*\(\s*songToHighlight\s*\)\s*\{?\s*songToHighlight\.setAttribute\(/)
```

Debes pasar "aria-current" y "true" como el primer y segundo argumento de tu método `setAttribute`.
