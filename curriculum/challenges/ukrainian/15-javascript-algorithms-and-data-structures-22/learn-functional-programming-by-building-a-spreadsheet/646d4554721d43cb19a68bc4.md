...
Параметру `fn` буде передано назву функції, як от `"SUM"`. Оновіть `apply`, щоб неявно повернути функцію з об’єкта `spreadsheetFunctions`, використовуючи змінну `fn` як ключ для доступу до об’єкта.

Пам’ятайте, що `fn` може не містити рядок у малому регістрі, тому потрібно буде перетворити її на рядок нижнього регістру.
...
Функція `apply` має викликати метод `.toLowerCase()` на `fn` у доступі до властивості.
