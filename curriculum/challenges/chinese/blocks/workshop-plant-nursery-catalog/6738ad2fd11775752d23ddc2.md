...
`Set` 构造函数也接受一个可迭代对象作为其参数，并从中创建一个集合：
...
要完成此工作坊，修改你的 `displayPlantsSet` 以返回目录中唯一植物通用名的集合。

为实现此目的，创建一个包含作为 `catalog` 映射密钥保存的每个植物对象的 `commonName` 的数组。将该数组传递给 `Set` 构造函数，并从函数中返回该集合。

# --hints--

你的 `displayPlantsSet` 函数应返回一个包含目录中每个植物的 `commonName` 的集合。
