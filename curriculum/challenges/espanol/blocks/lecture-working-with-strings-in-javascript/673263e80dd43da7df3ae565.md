...
Una subcadena es una secuencia de caracteres que aparece dentro de una cadena más grande. Por ejemplo, en la cadena `hello world`, `hello` y `world` son subcadenas.
...
En este contexto, un **argumento** es un valor que le das a una función o método cuando lo llamas, permitiendo que esa función o método realice su tarea usando la información específica que proporcionas. Aprenderás más sobre argumentos en lecciones futuras.
...
En este ejemplo, la palabra `awesome` comienza en el índice `14` en la cadena `JavaScript is awesome!`, por lo que el método `indexOf()` devuelve `14`.
...
Dado que la palabra `fantastic` no aparece en la cadena, el método devuelve `-1`.
