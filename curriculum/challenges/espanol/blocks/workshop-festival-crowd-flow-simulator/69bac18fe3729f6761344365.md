...
Necesitas saber cuántos ticks debe ejecutar la simulación. Recuerda que:

- Cada puerta tiene un arreglo `queue` que muestra cuántos asistentes llegan en cada tick.  
- Como todas las puertas tienen el mismo número de ticks, puedes usar simplemente la longitud de la cola de la primera puerta.  

Crea una variable llamada `maxTicks` y asígnale el valor de `gates[0].queue.length`.

Este valor se usará para controlar el ciclo de la simulación, asegurando que cada tick se procese para todas las puertas. 

# --hints--

Debes crear una variable llamada `maxTicks` y asignarle el valor de `gates[0].queue.length`.
