...
Sasa unaweza kupamba maudhui ya kipengele cha `.heading` kwa kutumia CSS Grid.

Kitendakazi cha CSS `repeat()` kinatumika kurudia thamani, badala ya kuandika kwa mkono, na ni msaada kwa mipangilio ya gridi. Kwa mfano, kuweka sifa ya `grid-template-columns` kuwa `repeat(20, 200px)` kutaunda safu 20 kila moja ikiwa na upana wa `200px`.

Toa sifa ya `.heading` yenye thamani `grid-template-columns` kwa kipengele chako cha `repeat(2, 1fr)` ili kuunda safu mbili zenye upana sawa.

# --hints--

Kichaguzi chako cha `.heading` kinapaswa kuwa na sifa ya `grid-template-columns` yenye thamani `repeat(2, 1fr)`.
