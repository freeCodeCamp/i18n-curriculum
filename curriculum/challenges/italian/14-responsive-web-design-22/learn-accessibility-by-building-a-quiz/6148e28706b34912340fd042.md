...
Centra orizzontalmente tutto il testo all'interno dell'elemento `address` e aggiungi un po' di padding.

# --hints--

Dovresti usare il selettore di elemento `address`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

Dovresti dare ad `address` una proprietà `text-align` con il valore `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

Dovresti dare ad `address` un `padding-top` di almeno `1px`.
...
Dovresti dare ad `address` un `padding-right` di almeno `1px`.
...
Dovresti dare ad `address` un `padding-bottom` di almeno `1px`.
...
Dovresti dare ad `address` un `padding-left` di almeno `1px`.
