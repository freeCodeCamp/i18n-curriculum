...
Ahora, usa el evento `onload` para eliminar la clase `hidden` cuando la imagen cargue y oculta el marcador de carga usando su `previousElementSibling`.

# --hints--

Tu elemento `img` debe tener un manejador `onload`.
...
Tu manejador `onload` debe usar `classList.remove` para eliminar la clase `hidden`. No olvides que aquí necesitas la palabra clave `this`.
...
Tu manejador `onload` debe asignar `previousElementSibling.style.display` a `'none'`. No olvides que para esto necesitas la palabra clave `this`.
