...
في وحدة سابقة، تعلمت أنه يمكنك بناء استثناءات مخصصة خاصة بك عن طريق الوراثة من الصنف المدمج `Exception`:
...
أنشئ صنفًا باسم `MediaError`. اجعله يرث من الصنف `Exception` وأضف إليه سلسلة توثيق تقول `Custom exception for media-related errors.`.

# --hints--

يجب أن تنشئ صنفًا باسم `MediaError`.
...
يجب أن يرث صنف `MediaError` الخاص بك من `Exception`.
...
يجب أن تضيف سلسلة التوثيق `Custom exception for media-related errors.` إلى صنف `MediaError` الخاص بك.
