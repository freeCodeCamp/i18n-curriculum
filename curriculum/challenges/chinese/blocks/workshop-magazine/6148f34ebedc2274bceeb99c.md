...
现在杂志布局已经完成，你需要使其具有响应性。

以针对 `only screen` 的 `@media` 查询开始，其 `max-width` 设置为 `720px`。 在里面，创建一个 `.image-wrapper` 选择器，并给它一个值为 `1fr` 的 `grid-template-columns` 属性。

这将在较小的屏幕上将三个图像排成一列。

# --hints--

你应该有一个新的 `@media` 规则，条件是 `only screen and (max-width: 720px)`。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText, 'only screen and (max-width: 720px)');
```

你的新 `@media` 规则应该有一个 `.image-wrapper` 选择器。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText, '.image-wrapper');
```

你的新 `.image-wrapper` 选择器应该有一个值为 `1fr` 的 `grid-template-columns` 属性。
