...
Maps und Sets teilen einige Methoden, die denselben Namen und Zweck haben. Zum Beispiel können Sie die `has`-Methode bei einem Set verwenden, um zu prüfen, ob es ein bestimmtes Element enthält.

Verwenden Sie die `has`-Methode, um zu überprüfen, ob das `ballerina`-Objekt in Ihrem `plantsSet` enthalten ist, und geben Sie das Ergebnis in der Konsole aus.

# --hints--

Sie sollten `plantsSet.has(ballerina)` in der Konsole ausgeben.
