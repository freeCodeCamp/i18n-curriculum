...
El último cambio será en el texto del correo electrónico dentro del pie de página. En lecciones y talleres anteriores, aprendiste cómo trabajar con el enlace `mailto` así:

```html
<p>Email me at <a href="mailto:janedoe@email.com">janedoe@email.com</a></p>
```

Comienza envolviendo la dirección de correo electrónico `camperbot@blog.io` dentro de un elemento anchor. Luego, agrega el atributo `href` al elemento anchor y asígnale el valor `mailto:camperbot@blog.io`.

Con ese último cambio, ¡has resuelto con éxito todos los problemas en la página del blog!

# --hints--

Tu dirección de correo electrónico `camperbot@blog.io` debe estar dentro de un elemento anchor.
...
Tu elemento anchor debe tener un atributo `href` con un valor de `mailto:camperbot@blog.io`.
