...
Unganisha kipengele cha kwanza cha `label` na kipengele cha `select`, na ukipe kipengele cha `select` sifa ya `name`.
...
Unapaswa kukipa kipengele cha `select` sifa ya `id`.

```js
assert.notEmpty(document.querySelector('.answer > select')?.id);
```

Unapaswa kukipa kipengele cha `select` `id` inayolingana na sifa ya `for` ya kipengele cha `label`.

```js
assert.equal(document.querySelector('.answer > select')?.id, document.querySelector('.question-block > label')?.htmlFor);
```

Unapaswa kukipa kipengele cha `select` sifa ya `name`.
