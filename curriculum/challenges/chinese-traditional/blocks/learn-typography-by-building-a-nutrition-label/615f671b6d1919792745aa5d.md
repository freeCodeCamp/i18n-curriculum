...
在你的 `.medium` 元素下方創建一個新的 `div` 元素。為它的 `class` 屬性設定為 `daily-value small-text`。在這個新的 `div` 中，添加一個 `p` 元素，文字為 `% Daily Value *`，並將 `class` 屬性設定為 `bold right`。

# --hints--

你應該在你的 `.medium` 元素之後創建一個新的 `div` 元素。
...
你的新 `div` 應該有一個 `class` 屬性設為 `daily-value small-text`。
