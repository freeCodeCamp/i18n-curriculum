...
Invoca `getEVAReadyCrew()` con tu lista `updatedSquad` y almacena el resultado en una nueva variable llamada `EVAReadySquad`. Luego, usa un ciclo `for` para registrar el `name` de cada astronauta en el arreglo `EVAReadySquad`.

# --hints--

Debes llamar a `getEVAReadyCrew()` con `updatedSquad` y almacenar el resultado en una nueva variable llamada `EVAReadySquad`.
...
Debes usar un bucle `for` para iterar a través del arreglo `EVAReadySquad` y registrar el `name` de cada astronauta.
