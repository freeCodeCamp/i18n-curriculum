...
创建一个名为 `movePlayer` 的新箭头函数，它具有三个参数，分别称为 `key`，`xVelocity`，`isPressed`。

# --hints--

你应该声明一个 `movePlayer` 函数。

```js
assert.isFunction(movePlayer);
```

你的 `movePlayer` 函数应该采用 `key`、`xVelocity` 和 `isPressed` 的参数。

```js
assert.match(movePlayer.toString(), /\(\s*key\s*,\s*xVelocity\s*,\s*isPressed\s*\)/);
```

你的 `movePlayer` 函数应该是空的。
