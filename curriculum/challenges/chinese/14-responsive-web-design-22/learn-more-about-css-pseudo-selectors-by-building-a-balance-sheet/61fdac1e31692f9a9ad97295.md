...
CSS `clip` 属性用于定义元素的可见部分。 Set the `span[class~="sr-only"]` selector to have a `clip` property of `rect(1px, 1px, 1px, 1px)`.

`clip-path` 属性决定了 `clip` 属性应该采用的形状。 设置 `clip-path` 属性的值为 `inset(50%)`，在元素内剪切形成一个矩形。

# --hints--

`span[class~="sr-only"]` 选择器应该有一个 `clip` 属性设置为 `rect(1px, 1px, 1px, 1px)`。

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('clip') === 'rect(1px, 1px, 1px, 1px)');
```

`span[class~="sr-only"]` 选择器应该有一个 `clip-path` 属性设置为 `inset(50%)`。
