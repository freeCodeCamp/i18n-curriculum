...
創建一個名為 `setPlayerDisplay` 的函式（程式）。在該函式中，如果已定義，將 `playingSong` 的 `textContent` 設為目前歌曲的標題。否則，將其設為空字串。

在那之下，將 `songArtist` 的 `textContent` 設定為目前歌曲的藝術家（如果已定義）。否則，將其設定為空字串。 


# --hints--

你應該有一個名為 `setPlayerDisplay` 的函式（程式）。

```js
assert.isFunction(setPlayerDisplay);
```

你的 `setPlayerDisplay` 函式應該將 `playingSong` 的 `textContent` 設定為目前歌曲的標題（如果已定義），否則設定為空字串。
...
你的 `setPlayerDisplay` 函式應該將 `songArtist` 的 `textContent` 設定為目前歌曲的歌手（如果有定義），否則設定為空字串。
