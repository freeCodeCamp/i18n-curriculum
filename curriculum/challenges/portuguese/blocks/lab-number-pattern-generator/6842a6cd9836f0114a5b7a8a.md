...
`number_pattern` deve retornar `Argument must be an integer value.` quando for passado um valor que não seja um inteiro.
...
`number_pattern` deve retornar `Argument must be an integer greater than 0.` quando receber um inteiro não positivo.
