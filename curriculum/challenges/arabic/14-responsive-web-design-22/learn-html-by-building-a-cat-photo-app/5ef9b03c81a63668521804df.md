...
Your radio button should have an `id` attribute. تحقق من وجود مسافة بعد اسم opening tag و/أو أن هناك مسافات قبل جميع أسماء السمات.

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('id'));
```

Your radio button should have a `type` attribute. تحقق من وجود مسافة بعد اسم opening tag و/أو أن هناك مسافات قبل جميع أسماء السمات.

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('type'));
```

Your radio button should have an `id` attribute with the value `indoor`. إما أنك حذفت القيمة أو لديك خطأ إملائي. تذكر أن تحيط قيمة السمة بعلامات اقتباس.
