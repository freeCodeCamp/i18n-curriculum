...
接下來，從 `.bb2a` 中刪除 `width` 和 `height`，並將 `border-left` 和 `border-right` 更改爲使用 `5vw` 而不是 `1vw`。 該元素現在的大小將是零，並且邊框將在中間聚集在一起。
