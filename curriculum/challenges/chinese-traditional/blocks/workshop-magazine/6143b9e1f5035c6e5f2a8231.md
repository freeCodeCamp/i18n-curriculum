...
在你的 `ul` 元素中，創建六個 `li` 元素。 在每個 `li` 元素中添加一個 `class` 設置爲 `list-subtitle` 的 `h4` 元素和一個 `p` 元素。

然後按順序爲 `h4` 和 `p` 元素添加以下文本內容，每個 `h4` 使用冒號左側的內容，而每個 `p` 元素使用右邊的內容：
...
你的第一個 `h4` 元素應該有文本 `V1 - 2014`。

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

你的第一個 `p` 元素應該有文本 `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
你的第四個 `h4` 元素應該有文本 `V4 - 2016`。

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[3]?.innerText, 'V4 - 2016');
```

你的第四個 `p` 元素應該有文本 `We expanded the curriculum to 3 certifications, including Front End, Back End, and Data Visualization. They each had 10 required projects, but only the Front End section had its own challenges. For the other certs, we were still using external resources like Node School.`
...
你的六個 `h4` 元素都應具有 `list-subtitle` 類。
