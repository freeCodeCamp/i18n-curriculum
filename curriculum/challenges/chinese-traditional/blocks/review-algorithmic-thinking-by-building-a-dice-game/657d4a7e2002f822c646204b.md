...
在使用者做出選擇後，他們應該能夠透過點擊 `keepScoreBtn` 來保留該分數並進入下一回合。

當按鈕被點擊時，你應該找出哪個單選按鈕選項被勾選，並擷取其 value 和 id 屬性。如果使用者已選擇一個選項，呼叫你的函式（程式）來更新分數、重設單選按鈕選項，並將 value 和 id 添加到分數歷史記錄。

如果使用者尚未選擇選項，可顯示提醒通知他們這麼做。

# --hints--

當你的 `keepScoreBtn` 被點擊時，分數應該在 `totalScoreElement` 文字中更新。
...
當你的 `keepScoreBtn` 被點擊時，單選選項應該被重設。
...
當你的 `keepScoreBtn` 被點擊時，分數紀錄應該被更新。
...
當你的 `keepScoreBtn` 被點擊，且沒有選擇任何選項時，你應該向使用者顯示警告。
