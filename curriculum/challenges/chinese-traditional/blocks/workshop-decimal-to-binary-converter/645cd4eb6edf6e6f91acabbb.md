...
既然 `b()` 已經執行完畢，將它從呼叫堆疊中彈出。然後，將你的模擬呼叫 `a()` 更新為以下內容：`'a(): returns "freeCodeCamp " + "is awesome!"'`。

# --hints--

`callStack` 應該有一個元素是字串。
...
`callStack` 中的字串應該是字串 `'a(): returns "freeCodeCamp " + "is awesome!"'`。
