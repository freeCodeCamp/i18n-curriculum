...
將具有 `class` 為 `left` 的 `.foot` 元素標的，並將其定位於父元素的左側 `25%`。然後，將具有 `class` 為 `right` 的 `.foot` 元素標的，並將其定位於父元素的右側 `25%`。

# --hints--

你應該有一個 `.foot.left` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.foot.left', '.left.foot']));
```

你應該給 `.foot.left` 一個 `left` 為 `--fcc-expected--`，但發現是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyleAny(['.foot.left', '.left.foot'])?.left, '25%');
```

你應該有一個 `.foot.right` 選擇器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyleAny(['.foot.right', '.right.foot']));
```

你應該給 `.foot.right` 一個 `right` 為 `--fcc-expected--`，但發現為 `--fcc-actual--`。
