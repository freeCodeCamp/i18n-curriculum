...
Aktualisieren Sie den Wert von `total`, sodass er die Summe der Werte von `subTotal` und `tax` ist.

# --hints--

Sie sollten den Wert von `total` aktualisieren. Denken Sie daran, das Schlüsselwort `this` zu verwenden.

```js
assert.match(cart.calculateTotal.toString(), /this\.total/);
```

Sie sollten den Wert von `total` so setzen, dass er die Summe der Werte von `subTotal` und `tax` ist.
