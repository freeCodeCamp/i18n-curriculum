...
بعد ذلك، أنشئ نوعًا `Position` بقيمة `typeof POSITIONS[number]`. هذا يفكك مصفوفة `POSITIONS` إلى اتحاد من ثوابت السلاسل النصية الفردية، مما يجعلها تبدو مثل  `"GK" | "CB" | "LB" | ...` وهكذا.

# --hints--

يجب أن تنشئ اسمًا مستعارًا للنمط يسمى `Position`.
...
يجب تعيين نوع `Position` إلى `typeof POSITIONS[number]`.
