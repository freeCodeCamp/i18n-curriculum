...
Das Rangabzeichen zeigt noch den fest codierten Text `GOLD`. Sie müssen ihn durch einen dynamischen Ausdruck ersetzen, damit der richtige Rang angezeigt wird.

Rufen Sie `getPlayerTier()` mit `player.overallRating` auf, um den Rang-String zu erhalten, und rufen Sie dann `.toUpperCase()` auf dem Ergebnis auf, um ihn in Großbuchstaben anzuzeigen. Vergessen Sie nicht, den gesamten Ausdruck in geschweifte Klammern zu setzen.

# --hints--

Sie sollten `getPlayerTier(player.overallRating).toUpperCase()` innerhalb von `.card-tier-badge` verwenden.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

Das `.card-tier-badge`-Element sollte `ELITE` für einen Spieler mit einer Bewertung von 98 anzeigen.
