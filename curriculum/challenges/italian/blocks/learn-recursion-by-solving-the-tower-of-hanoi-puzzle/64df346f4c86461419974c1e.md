...
Infine, aggiungi una clausola `else` che stampi la mossa consentita e chiami `make_allowed_move`. Prova a capire quali sono gli argomenti corretti.
...
Dovresti avere `print(f'Move {i + 1} allowed between {source} and {target}')` all'interno del blocco `else`.
...
Dovresti avere `make_allowed_move(source, target)` all'interno del blocco `else`.
