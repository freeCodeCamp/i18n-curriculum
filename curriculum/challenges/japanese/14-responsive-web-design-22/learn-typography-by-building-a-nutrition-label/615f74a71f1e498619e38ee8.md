...
`% Daily Value *` と `Saturated Fat 1g 5%` の要素の下端の境界線が、ラベルの幅全体に広がっていません。 この 2 つの要素の `class` に `no-divider` を追加してください。

# --hints--

`% Daily Value *` のテキストを持つ `p` 要素の `class` 属性に `no-divider` が追加される必要があります。 既存のクラスは削除しないでください。
...
`Saturated Fat 1g 5%` のテキストを持つ `p` 要素の `class` 属性に `no-divider` が追加される必要があります。 既存のクラスは削除しないでください。
