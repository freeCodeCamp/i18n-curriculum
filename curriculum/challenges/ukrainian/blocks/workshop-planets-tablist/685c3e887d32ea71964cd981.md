...
У попередніх модулях ви навчились ітерувати по `NodeList` таким чином:
...
Використайте метод `forEach` для ітерування по `tabs`. Для функції зворотного виклику використайте стрілкову функцію, яка приймає один параметр, `tab`.

# --hints--

У вашому коді має бути `tabs.forEach(tab => {})`.
