...
Em seguida, use a propriedade `classList` e o método `add()` para adicionar a classe `playing` ao elemento `playButton`.Isso irá procurar pela classe `playing` no arquivo CSS e adicioná-la ao elemento `playButton`.

Então, defina `userData.currentSong` como `song` e para finalmente tocar a música, use o método `play()` na variável `audio`. `play()` é um método da API de áudio web para tocar um arquivo mp3.

# --hints--

Você deve adicionar a classe `playing` ao elemento `playButton` dentro da sua função `playSong`.
...
Você deve definir `userData.currentSong` como `song` dentro da sua função `playSong`.
...
Você deve chamar o método `play()` na variável `audio` dentro da sua função `playSong`.
