...
마지막 `fieldset` 요소의 테두리가 약간 어색해 보입니다. `last-of-type` CSS 가상 클래스를 사용해 특정 유형의 마지막 요소를 선택할 수 있습니다. 예를 들어 다음과 같습니다:

```css
p:last-of-type { }
```

이렇게 하면 마지막 `p` 요소가 선택됩니다. 마지막 `fieldset` 요소를 대상으로 하는 새 선택자를 만들고 그 `border-bottom`를 `none`로 설정하세요.

# --hints--

특정 유형의 마지막 요소만 선택하려면 `:last-of-type` 가상 클래스를 사용할 수 있습니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type'));
```

`fieldset:last-of-type`에 `border-bottom`를 `none`로 설정해야 합니다.
