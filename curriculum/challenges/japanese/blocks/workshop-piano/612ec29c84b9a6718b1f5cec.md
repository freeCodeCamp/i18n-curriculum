...
新しい `@media` ルールでは、`width` の `#piano` を `675px` に、`width` の `.keys` を `633px` に設定してください。

これでピアノが完成です！

# --hints--

2つ目の `@media` ルールには `#piano` セレクターが必要です。
...
新しい `#piano` セレクターには `width` の `675px` が必要です。
...
2つ目の `@media` ルールには `.keys` セレクターが必要です。
...
新しい `.keys` セレクターには `width` の `633px` が必要です。
