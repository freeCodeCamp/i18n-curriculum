...
`matrix_list`는 리스트 안에 리스트가 있는 구조이며, 각 요소는 `(x, y)` 좌표를 가집니다. `rounded_coords`에 있는 좌표 리스트를 사용해 `matrix_list`의 해당 좌표에 있는 요소들을 `PROJECTILE` 변수에 있는 기호로 변경하세요. 좌표 그래프는 왼쪽 아래 모서리에 `(0, 0)`가 있다는 점을 기억하세요.
...
메서드는 다양한 인스턴스에 대해 올바른 값을 반환해야 합니다.
