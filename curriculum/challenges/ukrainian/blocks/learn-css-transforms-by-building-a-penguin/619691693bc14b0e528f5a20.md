...
Задайте цільовому `.ground` елементу, щоб його `width` займав повну ширину вікна перегляду. Потім встановіть `height` у `400px`.
...
Ви повинні надати елементу `.ground` `width` зі значенням `100vw`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.ground')?.width, '100vw');
```

Ви повинні надати елементу `.ground` `height` зі значенням `400px`.
