...
Porque `elemValue` retorna uma função, sua função `addCharacters` acaba retornando um array de referências de função.Você quer que o método `.map()` execute a função interna à função `elemValue`, o que significa que você precisa chamar `elemValue` em vez de fazer referência a ela.Passe `num` como argumento para a função `elemValue`.

<!-- TODO: Explain further? -->

# --hints--

Você deve chamar `elemValue()` no método `.map()`.
...
Você deve passar `num` para a chamada de `elemValue()`.
