...
- **Elementos en línea**: Los elementos en línea solo ocupan el ancho que necesitan y no comienzan en una nueva línea. Estos elementos fluyen dentro del contenido, permitiendo que el texto y otros elementos en línea aparezcan junto a ellos. Elementos en línea comunes son `span`, `anchor` y `img`.
...
- **Propiedad `margin`**: Esta propiedad se usa para aplicar espacio afuera del elemento, entre el borde del elemento y los elementos circundantes.
...
- **Especificidad de CSS Interno**: El CSS interno se define dentro de un elemento `style` en la sección `head` del documento HTML. Tiene menor especificidad que los estilos en línea. Tanto el CSS interno como el externo comparten el mismo nivel de especificidad, que está determinado por sus selectores, no por su ubicación.
- **Especificidad de CSS externo**: El CSS externo se vincula mediante un elemento `link` en la sección `head` y se escribe en archivos `.css` separados. Al igual que el CSS interno, su especificidad está determinada por los selectores usados. Cuando dos reglas tienen la misma especificidad, el orden de origen determina el ganador: la regla que aparece más tarde en el documento tiene precedencia. El CSS externo ofrece la mejor mantenibilidad para proyectos grandes.
...
- **Unidad `em`**: Estas unidades son relativas al tamaño de fuente del elemento. Si usas `ems` para la propiedad `font-size`, el tamaño del texto será relativo al tamaño de fuente del elemento padre.
...
- **Pseudoclases estructurales del árbol**: Estas pseudoclases te permiten seleccionar y estilizar elementos según su posición dentro del árbol del documento.
- **Pseudoclase `:root`**: Esta pseudoclase suele ser el elemento raíz `html`. Te ayuda a seleccionar el nivel más alto del documento para aplicar un estilo común a todo el documento.
- **Pseudoclase `:empty`**: Los elementos vacíos, es decir, elementos sin hijos aparte de espacio en blanco, también están incluidos en el árbol del documento. Por eso existe la pseudoclase `:empty` para seleccionar elementos vacíos.
- **Pseudoclase `:nth-child(n)`**: Esta pseudoclase te permite seleccionar elementos según su posición dentro de un padre.
- **Pseudoclase `:nth-last-child(n)`**: Esta pseudoclase permite seleccionar elementos contando desde el final.
...
- **Pseudoclase `:nth-of-type(n)`**: Esta pseudoclase te permite seleccionar un elemento específico dentro de su padre según su posición entre hermanos del mismo tipo.
...
- **Pseudoclase `:has()`**: Esta pseudoclase es conocida a menudo como el selector de `"parent"` porque permite aplicar estilos a elementos que contienen hijos especificados en la lista de selectores.
...
- **Atributo `hidden`**: Este atributo es soportado por la mayoría de los navegadores modernos y oculta contenido tanto visualmente como del árbol de accesibilidad. Se puede activar o desactivar fácilmente con JavaScript.
...
- **Definición**: El selector `attribute` te permite seleccionar elementos HTML basados en sus atributos, como los atributos `href` o `title`.
...
- **Definición**: Esto permite a los desarrolladores aplicar diferentes estilos basados en las características del dispositivo, principalmente el ancho del viewport.
...
- **Tipos de medio `print`**: Está destinado a material paginado y documentos vistos en pantalla en modo de vista previa de impresión.
...
- **Respaldo**: Al usar la propiedad personalizada, puedes proporcionar un valor de respaldo usando la función `var()`, tal como lo harías con propiedades personalizadas estándar.
...
- **Propiedad `animation`**: Esta es la propiedad abreviada usada para aplicar animaciones.
