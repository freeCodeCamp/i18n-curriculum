...
title: Trovare il valore minimo e massimo in un albero binario di ricerca
challengeType: 1
forumTopicId: 301642
...
In questa sfida definirai due metodi, `findMin` e `findMax`. Questi metodi devono restituire il valore minimo e massimo contenuto nell'albero binario di ricerca (non preoccuparti per l'aggiunta di valori all'albero per ora, ne abbiamo aggiunti alcuni in background). Se ti blocchi, rifletti sull'invariante che deve essere vera per gli alberi binari di ricerca: ogni sottoalbero sinistro è minore o uguale al suo genitore e ogni sottoalbero destro è maggiore o uguale al suo genitore. Diciamo anche che il nostro albero può archiviare solo valori interi. Se l'albero è vuoto, entrambi i metodi dovrebbero restituire `null`.
...
L'albero binario di ricerca dovrebbe avere un metodo chiamato `findMax`.
