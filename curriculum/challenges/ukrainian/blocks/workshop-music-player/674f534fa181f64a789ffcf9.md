...
Якщо `userData.currentSong` не є `null`, потрібно знайти наступну пісню для відтворення. Пам’ятайте, що для цього можна використати `getNextSong`.

Завершіть вашу функцію `playNextSong`, щоб вона відтворювала наступну пісню у плейлисті. Якщо поточна пісня — остання, встановіть `userData.currentSong` у `null`, `userData.songCurrentTime` у нуль і викличте функцію `pauseSong`.

# --hints--

Коли `currentSong` `userData` істинний і це не остання пісня плейлиста, функція `playNextSong` має відтворити пісню, що йде після поточної.
...
Коли `currentSong` `userData` — остання пісня плейлиста, функція `playNextSong` має встановити `userData.currentSong` у `null`.
...
Коли `currentSong` `userData` — остання пісня плейлиста, функція `playNextSong` має встановити `userData.songCurrentTime` у `0`.
...
Коли `currentSong` `userData` — остання пісня плейлиста, функція `playNextSong` має викликати функцію `pauseSong`.
...
Коли `currentSong` `userData` дорівнює `null`, ваша функція `playNextSong` все одно має викликати функцію `playSong` з `id` першої пісні у масиві `userData.songs` як аргумент.
