...
Badilisha ukingo wa juu wa vipengele vya `h2` na nafasi ya ndani ya juu ya `60px`.

# --hints--

Unapaswa kumpa `h2` nafasi ya ndani ya `margin-top` ya `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h2')?.marginTop, '0px');
```

Unapaswa kumpa `h2` nafasi ya ndani ya `padding-top` ya `60px`.
