...
Щоб прапорець був встановлений або радіокнопка вибрана за замовчуванням, потрібно додати до неї атрибут `checked`.
...
Не потрібно задавати значення атрибуту `checked`. Замість цього просто додайте слово `checked` до елемента `input`, переконавшись, що між ним і іншими атрибутами є пробіл.

Зробіть так, щоб перша радіокнопка і перший прапорець були вибрані за замовчуванням.

# --hints--

Переконайтеся, що в їх відповідних елементах `fieldset` все ще є дві радіокнопки і три прапорці.
...
Ваша перша радіокнопка з `id`, встановленим у `indoor`, має мати атрибут `checked`.
...
Першому прапорцю бракує атрибута `checked`.
