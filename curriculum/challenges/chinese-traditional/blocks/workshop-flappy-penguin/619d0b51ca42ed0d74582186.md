...
將 `.eye` 元素設為標的，並給予它們 `width` 為 `15%`、`height` 為 `17%` 以及 `background-color` 為 `black`。
...
你應該給 `.eye` 一個 `width` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.width, '15%');
```

你應該給 `.eye` 一個 `height` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye')?.height, '17%');
```

你應該給 `.eye` 一個 `background-color` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。
