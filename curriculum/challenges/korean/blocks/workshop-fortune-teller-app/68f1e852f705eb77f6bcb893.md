...
`cardId`라는 변수를 선언하고 `data-id` 메서드를 사용해 `cardElement`에서 `getAttribute()` 속성 값을 할당하세요.

# --hints--

`cardId` 안에 `showFortune`라는 변수를 선언하세요.
...
`cardId` 변수는 `cardElement.getAttribute("data-id")`를 호출하여 할당해야 합니다.
