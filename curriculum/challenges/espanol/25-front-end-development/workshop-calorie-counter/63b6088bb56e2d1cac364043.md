...
Tu tercer elemento `fieldset` debe venir después de tu segundo elemento `fieldset`.

```js
assert.equal(document.querySelectorAll('form fieldset')[2]?.previousElementSibling?.tagName, "FIELDSET");
```

Tu tercer elemento `fieldset` debe tener un `id` configurado como `dinner`.
...
Tu nuevo elemento `div` debe tener una `class` configurada como `input-container`.
