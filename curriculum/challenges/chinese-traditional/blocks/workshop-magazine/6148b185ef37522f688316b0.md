...
現在你可以使用 CSS Grid 爲 `.heading` 元素的內容設置樣式。

CSS `repeat()` 函數用於重複一個值，而不是手動寫出它，這對網格佈局很有幫助。 例如，將 `grid-template-columns` 屬性設置爲 `repeat(20, 200px)` 將創建 20 列，每列 `200px` 寬。

給你的 `.heading` 元素一個 `grid-template-columns` 屬性，設置爲 `repeat(2, 1fr)`，以創建兩個等寬的列。

# --hints--

你的 `.heading` 選擇器應該有一個 `grid-template-columns` 屬性，設置爲 `repeat(2, 1fr)`。
