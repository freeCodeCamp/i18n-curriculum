...
將你的 `getMode()` 函數添加到你的 `calculate` 邏輯，並更新相應的 HTML 元素。

# --hints--

你的 `calculate` 函數應該有一個 `mode` 變量，其值爲 `getMode(numbers)`。

```js
assert.match(calculate.toString(), /mode\s*=\s*getMode\(\s*numbers\s*\)\s*;?/);
```

你的 `calculate` 函數應該查詢 DOM 獲得 `#mode` 元素，並將 `textContent` 設置爲 `mode`。
