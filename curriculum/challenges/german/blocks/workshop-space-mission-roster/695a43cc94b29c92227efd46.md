...
Sie haben einen `console.log()` Aufruf zu Ihrem Skript hinzugefügt, um Ihr `EVAChunks` Array zu validieren, und dieser Aufruf wird jetzt nicht mehr benötigt. Um das Terminal für die nächsten Schritte sauber zu halten, löschen Sie den `for loop` Block, der Daten von `EVAChunks` protokolliert.

# --hints--

Sie sollten den `for loop` Block löschen, der Daten von `EVAChunks` protokolliert. Löschen Sie den gesamten Block, der beide Schleifen enthält.
