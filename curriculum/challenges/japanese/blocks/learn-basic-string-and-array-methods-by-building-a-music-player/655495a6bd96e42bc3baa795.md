...
次に、プレイヤー表示に現在の曲名とアーティストを表示する必要があります。`const` とアロー構文を使って空の `setPlayerDisplay` 関数を作成してください。

# --hints--

空の関数 `const` を作成するには `setPlayerDisplay` を使うべきです。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` は関数であるべきです。

```js
assert.isFunction(setPlayerDisplay)
```

`setPlayerDisplay` 関数はアロー構文を使うべきです。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

`setPlayerDisplay` 関数は空であるべきです。
