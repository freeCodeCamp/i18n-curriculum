...
Ahora que la estructura HTML está completa, debes pasar al estilo CSS.

Abre tu archivo `styles.css`. Primero, establece un color de fondo oscuro para el `body` y un color claro para el texto.

Selecciona el elemento `body` y establece su `background-color` en `midnightblue` y `color` en `whitesmoke`.

# --hints--

Debes apuntar al elemento `body`.

```js
assert(new __helpers.CSSHelp(document).getStyle('body'));
```

Establece la propiedad `background-color` a `midnightblue`.
