...
Debes crear un cuarto elemento `fieldset` en el `form`.
...
Tu cuarto elemento `div` debe tener una `class` establecida a `input-container`.

```js
assert.equal(document.querySelectorAll('form fieldset')[3]?.querySelector('div')?.className, "input-container");
```

Debes crear un quinto elemento `fieldset` en el `form`.
...
Tu quinto elemento `div` debe tener una `class` establecida a `input-container`.
