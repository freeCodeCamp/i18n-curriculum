...
لجعل حركة الكابينة تبدو أكثر طبيعية مثل التأرجح، يمكنك استخدام دالة التوقيت `ease-in-out`. هذا الإعداد يجعل الحركة تبدأ وتنتهي بوتيرة أبطأ، لكنها تتحرك بسرعة أكبر في منتصف الدورة.

استبدل `linear` بـ `ease-in-out` في محدِّد `.cabin`.

# --hints--

يجب أن يحتوي محدِّد `.cabin` على خاصية `animation` مضبوطة على `cabins 10s ease-in-out infinite`.
