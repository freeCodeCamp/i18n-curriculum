...
`:not` 疑似セレクターは、指定した CSS ルールに一致しないすべての要素を選択するために使えます。
...
上記は、`div` の `id` を持たないすべての `example` 要素を選択します。

`.daily-value p` セレクターを修正して、`.no-divider` 要素を除外してください。

# --hints--

`.daily-value p:not(.no-divider)` セレクターがあるはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('.daily-value p:not(.no-divider)'));
```

`.daily-value p` セレクターはないはずです。

```js
assert(!new __helpers.CSSHelp(document).getStyle('.daily-value p'));
```

`.daily-value p:not(.no-divider)` セレクターのプロパティは変更しないでください。
