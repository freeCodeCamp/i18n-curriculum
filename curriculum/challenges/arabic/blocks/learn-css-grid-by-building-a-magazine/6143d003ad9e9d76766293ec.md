...
داخل عنصر `.image-quote`، قم بتسكين عنصر `hr` وعنصر `p` وعنصر `hr` ثاني. عيّن للعنصر `p` صنفًا `class` بقيمة `quote` والنص `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.

# --hints--

يجب أن تضيف عنصرين `hr` إلى عنصر `.image-quote` الخاص بك.

```js
assert(document.querySelectorAll('.image-quote hr')?.length === 2);
```

يجب أن تضيف عنصر `p` إلى عنصر `.image-quote` الخاص بك.

```js
assert(document.querySelectorAll('.image-quote p')?.length === 1);
```

يجب أن تكون عناصر الأطفال داخل `.image-quote` بالترتيب الصحيح.
...
يجب أن يحتوي عنصر `p` الجديد على `class` مضبوط على `quote`.

```js
assert(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

يجب أن يحتوي عنصر `p` الجديد على النص `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
