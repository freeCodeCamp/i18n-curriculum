...
ユーザーがカロリーを入力する際に食事の種類を選択するドロップダウンメニューと、食事の種類を追加するボタンを表示しましょう。

まず、`div` 要素を作成し、`class` 属性に `controls` という値を割り当ててください。 続いて、この `div` 内に `span` 要素を入れ子にします。

# --hints--

`form` 内に新しい `div` 要素を作成する必要があります。

```js
assert.exists(document.querySelector('form > div'));
```

新しい `div` 要素は `fieldset` 要素の後に置く必要があります。
...
`div` 要素の `class` 属性は `controls` に設定してください。

```js
assert.equal(document.querySelector('form > div')?.className, "controls");
```

`div` 内に `span` 要素を入れ子にする必要があります。
