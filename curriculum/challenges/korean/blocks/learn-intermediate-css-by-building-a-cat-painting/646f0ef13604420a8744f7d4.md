...
`.cat-whiskers` 요소 안에 `div`와 `cat-whiskers-left` 클래스를 가진 두 개의 `cat-whiskers-right` 요소를 만드세요.

# --hints--

기존 `div` 요소와 `cat-whiskers` 클래스는 변경하지 마세요.

```js
assert(document.querySelectorAll('div.cat-whiskers').length === 1)
```

`div` 요소 안에 두 개의 `.cat-whiskers` 요소가 있어야 합니다.

```js
assert(document.querySelectorAll('.cat-whiskers div').length === 2)
```

`div` 요소 안의 첫 번째 `.cat-whiskers` 요소는 `cat-whiskers-left` 클래스를 가져야 합니다.

```js
assert(document.querySelectorAll('.cat-whiskers div')[0].classList.contains('cat-whiskers-left'))
```

`div` 요소 안의 두 번째 `.cat-whiskers` 요소는 `cat-whiskers-right` 클래스를 가져야 합니다.
