...
HTML ha un altro elemento speciale per creare <dfn>liste ordinate</dfn>, o liste numerate.

Le liste ordinate iniziano con un elemento `<ol>` di apertura, seguito da un numero qualsiasi di elementi `<li>`. Infine, le liste ordinate si chiudono con il tag `</ol>`.
...
Crea una lista ordinata delle 3 cose che i gatti odiano di più.
...
Dovresti avere solo un elemento `ul`.
