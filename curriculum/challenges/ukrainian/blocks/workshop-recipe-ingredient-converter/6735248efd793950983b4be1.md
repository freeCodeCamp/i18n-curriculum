...
Додайте вашому `conversionTable` властивість `ounce` і призначте їй об’єкт із властивістю `cup`, встановленою в `0.125`, властивістю `gram`, встановленою в `28.35`, і властивістю `teaspoon`, встановленою в `6`.

# --hints--

Ваш об’єкт `conversionTable` має мати властивість `ounce`.

```js
assert.property(conversionTable, "ounce");
```

Ваша властивість `ounce` має бути об’єктом.

```js
assert.isObject(conversionTable.ounce);
```

Ваш об’єкт `ounce` має мати властивість `cup`.

```js
assert.property(conversionTable.ounce, "cup");
```

Ваша властивість `cup` має бути встановлена в `0.125`.

```js
assert.strictEqual(conversionTable.ounce?.cup, 0.125);
```

Ваш об’єкт `ounce` має мати властивість `gram`.

```js
assert.property(conversionTable.ounce, "gram");
```

Ваша властивість `gram` має бути встановлена в `28.35`.

```js
assert.strictEqual(conversionTable.ounce?.gram, 28.35);
```

Ваш об’єкт `ounce` має мати властивість `teaspoon`.

```js
assert.property(conversionTable.ounce, "teaspoon");
```

Ваша властивість `teaspoon` має бути встановлена в `6`.
