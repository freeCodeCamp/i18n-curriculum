...
根據對話窗，哪一句顯示根據目前證據正確使用 `going to`？
...
這句話使用 `will` 表示可能的未來動作，但缺乏基於當前證據所做決定的感覺。
...
這裡，`will` 用於基於當前情況的未來動作，但 `going to` 會更合適，因為決定似乎是基於團隊目前的專業知識。
...
這句話使用 `will` 來表示一個臨時決定（`calling the designer`），這是適當的，但它沒有說明基於當前證據使用 `going to`。
