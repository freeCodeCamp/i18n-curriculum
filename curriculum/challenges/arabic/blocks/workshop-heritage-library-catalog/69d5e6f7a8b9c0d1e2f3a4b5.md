...
املأ الآن `renderEntry`. أعلن عن ثابت للنظام البديل لكل حقل: `const title = entry.title || "Unknown"`، وطبق نفس النمط على `author` و`year` و`location`.

أرجع سلسلة نصية متعددة الأسطر تُنسق الإدخال كبطاقة مكتبة. استخدم `"-".repeat(25)` لحدود الأعلى والأسفل، وضمن أسطرًا بمسميات لـ `Title` و`Author` و`Year` و`Location`. على سبيل المثال، سيكون السطر الموسوم للعنوان بهذا الشكل `Title: ${title}`.

# --hints--

يجب أن تُرجع `renderEntry` سلسلة نصية.

```js
assert.isString(renderEntry(catalog[0]));
```

عند الاستدعاء مع `catalog[0]`، يجب أن تحتوي السلسلة النصية المرجعة على `"From a Buick 8"`.

```js
assert.include(renderEntry(catalog[0]), "From a Buick 8");
```

عند الاستدعاء مع `catalog[0]`، يجب أن تحتوي السلسلة النصية المرجعة على `"King, Stephen"`.

```js
assert.include(renderEntry(catalog[0]), "King, Stephen");
```

عند الاستدعاء مع `catalog[0]`، يجب أن تحتوي السلسلة النصية المرجعة على السنة `2002`.

```js
assert.include(renderEntry(catalog[0]), "2002");
```

عند الاستدعاء مع `catalog[0]`، يجب أن تحتوي السلسلة النصية المرجعة على `"Shelf K7"`.

```js
assert.include(renderEntry(catalog[0]), "Shelf K7");
```

يجب أن تتضمن السلسلة النصية المرجعة حدودًا علوية وسفلية باستخدام `"-".repeat(25)`.
