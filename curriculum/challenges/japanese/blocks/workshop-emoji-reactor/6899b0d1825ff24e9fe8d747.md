...
`h1` 要素の下に、クラス `p` を持つ `description` 要素を追加してください。

`p` 要素の中にテキスト `Click on the buttons below to rate your emotions.` を書いてください。

# --hints--

`p` 要素の下に新しい `h1` 要素を作成してください。

```js
assert.exists(document.querySelector("h1 + p"));
```

新しい `p` 要素にはクラス `description` を設定してください。

```js
assert.equal(document.querySelector("h1 + p")?.className, "description");
```

新しい `p` 要素にはテキスト `Click on the buttons below to rate your emotions.` を設定してください。
