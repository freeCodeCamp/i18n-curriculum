...
各`fieldset`に適切な`name`属性を付けてください。次に、両方の番号なしリストに`class`として`answers-list`を付けてください。

最後に、`legend`を使って`fieldset`の内容にキャプションを付け、テキスト内容として正誤問題を配置してください。

# --hints--

最初の`fieldset`に適切な`name`属性を付けてください。_ヒント: `html-question-one`を使うと良いです_

```js
assert.notEmpty(document.querySelectorAll('fieldset')?.[0]?.name);
```

2番目の`fieldset`に適切な`name`属性を付けてください。_ヒント: `html-question-two`を使うと良いです_

```js
assert.notEmpty(document.querySelectorAll('fieldset')?.[1]?.name);
```

最初の`ul`要素に`class`として`answers-list`を付けてください。

```js
assert.equal(document.querySelectorAll('fieldset > ul')?.[0]?.className, 'answers-list');
```

2番目の`ul`要素に`class`として`answers-list`を付けてください。

```js
assert.equal(document.querySelectorAll('fieldset > ul')?.[1]?.className, 'answers-list');
```

最初の`legend`要素にテキスト内容を付けてください。

```js
assert.notEmpty(document.querySelectorAll('legend')?.[0]?.textContent);
```

2番目の`legend`要素にテキスト内容を付けてください。

```js
assert.notEmpty(document.querySelectorAll('legend')?.[1]?.textContent);
```

両方の`legend`要素に同じテキスト内容を使わないでください。
