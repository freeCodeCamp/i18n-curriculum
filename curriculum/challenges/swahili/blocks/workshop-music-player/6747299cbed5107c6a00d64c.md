...
Nyimbo zilizobaki zimeongezwa kwenye safu ya data ya `allSongs` kwa ajili yako.

Katika mafundisho yaliyopita, ulijifunza kuhusu <dfn>Web Audio API</dfn> na jinsi ya kuitumia kuimba nyimbo. Vivinjari vyote vya kisasa vinaunga mkono Web Audio API, ambayo inakuwezesha kuzalisha na kusindika sauti katika programu za mtandao.

Tengeneza kigezo kinachoitwa `audio` na uweke thamani yake sawa na `new Audio()`. Hii itaunda kipengele kipya cha HTML5 cha `audio`.

# --hints--

Unapaswa kuwa na kigezo kinachoitwa `audio`.

```js
assert.isDefined(audio);
```

Unapaswa kuweka mfano mpya wa kitu cha `Audio` kwenye kigezo kinachoitwa `audio`.
