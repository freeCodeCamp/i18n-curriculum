...
title: "Cambia una variabile per un'area specifica"
challengeType: 0
...
Quando crei le tue variabili in `:root` imposteranno il valore di quella variabile per l'intera pagina.

Puoi poi sovrascrivere queste variabili impostandole di nuovo all'interno di un selettore specifico.
...
La classe `penguin` dovrebbe riassegnare la variabile `--penguin-belly` a `white`.
