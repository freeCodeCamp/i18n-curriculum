...
Verwenden Sie die `minmax`-Funktion, um Ihre Spalten auf jedem Gerät responsiv zu machen. Die `minmax`-Funktion nimmt zwei Argumente entgegen, wobei das erste das Minimum und das zweite das Maximum ist. Diese Werte können eine Länge, ein Prozentsatz, `fr` oder sogar ein Schlüsselwort wie `max-content` sein.

Fassen Sie jeden Ihrer bereits definierten Werte der `grid-template-columns`-Eigenschaft in einer `minmax`-Funktion zusammen, wobei jeder Wert als zweites Argument verwendet wird. Das erste Argument sollte jeweils `2rem`, `min-content` und `2rem` sein.

# --hints--

Ihr `main`-Selektor sollte eine `grid-template-columns`-Eigenschaft mit dem Wert `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)` haben.
