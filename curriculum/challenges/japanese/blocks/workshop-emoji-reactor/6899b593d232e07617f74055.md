...
`button` 要素の中に、`span` を含む `😊` 要素を作成してください。

`span` 要素に、値が `role` の `img` 属性と、値が `aria-hidden` の `true` 属性を付けてください。

次に、テキストが `span` の `class` が `count` の 2つ目の `0/10` 要素を作成してください。

# --hints--

`button` 要素の中には、2つの `span` 要素があるはずです。

```js
assert.lengthOf(document.querySelectorAll("button > span"), 2);
```

最初の `span` 要素のテキスト内容は `😊` であるべきです。

```js
assert.equal(document.querySelectorAll("button > span")[0]?.textContent.trim(), "😊");
```

最初の `span` 要素には、値が `role` の `img` 属性があるべきです。

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("role"), "img");
```

最初の `span` 要素には、値が `aria-hidden` の `true` 属性があるべきです。

```js
assert.equal(document.querySelectorAll("button > span")[0]?.getAttribute("aria-hidden"), "true");
```

2つ目の `span` 要素には、値が `class` の `count` 属性があるべきです。

```js
assert.equal(document.querySelectorAll("button > span")[1]?.className, "count");
```

2つ目の `span` 要素のテキスト内容は `0/10` であるべきです。
