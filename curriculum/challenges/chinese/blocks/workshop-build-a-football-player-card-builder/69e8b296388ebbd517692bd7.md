...
你现在将开始将 position 输入更改为选择框的过程。

将输入更改为一个 `select` 元素，id 为 `position`，`className` 为 `input`，`value` 为 `player.position`。

# --hints--

你应该将 position `input` 更改为一个 `select` 元素，具有 `id` 为 `position` 和 `className` 为 `input`。
...
你的 position `select` 应该有一个 `value` 为 `player.position`。
