...
Ваш селектор `tr.total td` повинен мати властивість `text-align` зі значенням `right`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td')?.getPropertyValue('text-align') === 'right');
```

Ваш селектор `tr.total td` повинен мати властивість `padding` зі значенням `0 0.25rem`.
