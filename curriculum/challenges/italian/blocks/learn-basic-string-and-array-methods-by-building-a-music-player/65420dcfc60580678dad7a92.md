...
All’interno del `map()`, aggiungi un’istruzione `return` con backtick dove interpolerai tutti gli elementi responsabili della visualizzazione dei dettagli della canzone.

All’interno dei backtick, crea un elemento `li` con un attributo `id` di `song-${song.id}` e un attributo `class` di `playlist-song`.

# --hints--

Dovresti aggiungere un’istruzione `return` con una coppia vuota di backtick.

```js
assert.match(code, /return\s*`/)
```

Dovresti creare un tag `li` all’interno dei backtick.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

Il tuo tag di apertura `li` dovrebbe avere un attributo `id` impostato su `song-${song.id}`.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

Il tuo tag di apertura `li` dovrebbe avere un attributo `class` impostato su `playlist-song`.
