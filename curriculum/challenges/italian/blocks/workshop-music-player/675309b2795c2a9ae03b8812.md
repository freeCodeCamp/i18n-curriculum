...
Nota che la copertina dell'album nell'HTML e le canzoni nell'array `userData.songs` sono cambiate. Abbiamo sostituito le canzoni originali con versioni più brevi che puoi usare per testare la tua app.

Ora tutte le funzionalità principali sono al loro posto. L'unico problema è che la canzone successiva non parte automaticamente quando quella in riproduzione finisce.

Per risolvere questo, puoi impostare un event listener che rilevi quando la canzone in riproduzione termina. L'event listener `"ended"` è adatto a questo scopo. Viene attivato quando la riproduzione di un media raggiunge la fine.

Aggiungi un event listener all'elemento `audio` che ascolti l'evento `"ended"` e passa un riferimento alla funzione `playNextSong`. Così il progetto del lettore musicale sarà completo!

# --hints--

Dovresti concatenare il metodo `addEventListener()` alla tua variabile `audio`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

L'event listener che hai usato sulla tua variabile `audio` dovrebbe ascoltare un evento `"ended"`.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

Dovresti passare un riferimento a `playNextSong` al tuo event listener `"ended"`.
