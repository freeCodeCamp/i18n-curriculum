...
次に、`#posts-container` の要素に `getElementById()` メソッドを使用してアクセスしてください。 それを `postsContainer` と言う名前の新しい const 変数に代入してください。

# --hints--

`const` を使って変数 `postsContainer` を宣言してください。

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

`document.getElementById()` を使って `#posts-container` の要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

`#posts-container` の要素を変数 `postsContainer` に割り当ててください。
