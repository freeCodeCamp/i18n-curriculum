...
Rücken Sie Ihre bestehende `for`-Schleife um eine Ebene ein, damit sie nur ausgeführt wird, wenn der Katalog Filme enthält. Ersetzen Sie dann `self.items` durch `movies`, um nur über Filme zu iterieren.

# --hints--

Sie sollten die bestehende `for`-Schleife um eine Ebene einrücken und `self.items` durch `movies` ersetzen, um nur über Filme zu iterieren.
