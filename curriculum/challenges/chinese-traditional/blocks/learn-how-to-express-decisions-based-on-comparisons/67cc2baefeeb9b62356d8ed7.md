...
`more`、`easier`、`better`、`as flexible`、`nicer`、`as easily` 和 `slightly`。
...
這個字用來形成形容詞的比較級。它位於形容詞之前並顯示較高的程度。  
...
這個片語用於比較中，表示相等的適應能力。第一個字是連接詞，第二個是形容詞。  
...
這個片語用於比較中，表示動作的輕鬆程度相等。第一個字是連接詞，第二個是副詞。  
