...
`footer` 요소 안에 `section` 요소를 추가하고 `id`를 `contact`로 설정하세요.

`section` 요소 안에 `h2` 요소를 추가하고 `Contact` 텍스트를 넣으세요.

# --hints--

`section` 요소 안에 `footer` 요소가 있어야 합니다.
...
`section` 요소에 `id`가 `"contact"`로 설정되어야 합니다.
