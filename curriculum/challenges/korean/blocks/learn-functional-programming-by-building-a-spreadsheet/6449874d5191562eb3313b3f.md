...
마지막으로, `rangeRegex`를 전역(global) 및 대소문자 구분 없이 만드세요.

# --hints--

`rangeRegex`는 대소문자 구분 없이 만들어야 합니다.
...
`rangeRegex`는 전역(global)이어야 합니다.
...
`rangeRegex`는 전역(global) 및 대소문자 구분 없이 모두 적용되어야 합니다.
