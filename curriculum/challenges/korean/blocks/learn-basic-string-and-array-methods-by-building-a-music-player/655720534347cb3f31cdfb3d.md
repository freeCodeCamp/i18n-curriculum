...
노래를 삭제하기 전에, 그 노래가 현재 재생 중인지 확인해야 합니다. 만약 그렇다면, 노래를 일시 정지하고 재생 목록에서 다음 노래를 재생해야 합니다.

삭제하려는 노래의 `if`와 `userData?.currentSong?.id`이 같은지 확인하려면 `id` 문을 사용하세요.

# --hints--

조건 `if`를 가진 빈 `userData?.currentSong?.id === id` 문을 만들어야 합니다.
