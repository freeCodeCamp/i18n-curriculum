...
- **Opções** ou **flags**: modificam o comportamento de um comando e geralmente vêm com prefixo de hífen:
  - **Forma longa (dois hífens)**:  
    - Exemplo: `--help`, `--version`
    - Valores são anexados usando um sinal de igual, por exemplo, `--width=50`.
  - **Forma curta (um hífen)**:  
    - Exemplo: `-a`, `-l`
    - Valores são passados com um espaço, por exemplo, `-w 50`.
    - Várias opções curtas podem ser encadeadas, por exemplo, `ls -alh`.
- **`--help`**: você sempre pode usar um comando com essa flag para entender as opções disponíveis para qualquer comando.
