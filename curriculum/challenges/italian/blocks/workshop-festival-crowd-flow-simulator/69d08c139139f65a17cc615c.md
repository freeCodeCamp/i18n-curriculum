...
All'interno del tuo loop `while`:

- Registra nel log il messaggio `"\nTick " + (tickIndex + 1)`.
- Itera su ogni gate nell'array `gates` e chiama `handleGateAtTick()` con questi argomenti:
  - `gates`: l'intero array di input gates.
  - `gate`: l'oggetto gate corrente.
  - `tickIndex`: il tick corrente.
  - `throughputSummary`: l'oggetto locale che traccia il totale dei partecipanti processati per gate.
- Incrementa `tickIndex` di 1 per passare al tick successivo.

# --hints--

Dovresti registrare nel log il seguente messaggio nella console, `"\nTick " + (tickIndex + 1)`.
...
Dopo aver registrato nella console, dovresti iterare su ogni gate nell'array `gates` e chiamare `handleGateAtTick()` con `gates`, `gate`, `tickIndex` e `throughputSummary`.
...
Dopo il tuo loop sull'array `gates`, dovresti incrementare `tickIndex` di 1.
