...
選取 `.blush` 元素，並為它們設定 `width` 為 `15%`、`height` 為 `10%`，以及 `background-color` 為 `pink`。
...
你應該給 `.blush` 一個 `width` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.width, '15%');
```

你應該給 `.blush` 一個 `height` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.blush')?.height, '10%');
```

你應該給 `.blush` 一個 `background-color`，其值為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。
