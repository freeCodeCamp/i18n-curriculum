...
O código de teste `kingBooks` confirmou que `findByAuthor` funciona corretamente. Antes de avançar para `groupByDecade`, limpe a saída do console removendo-a.

Remova a linha `const kingBooks = ...`, `console.log(kingBooks.length)` e o loop `for` que exibe os títulos e anos dos livros.

# --hints--

A variável `kingBooks`, a declaração `console.log(kingBooks.length)` e o loop `for` devem ser todos removidos.
