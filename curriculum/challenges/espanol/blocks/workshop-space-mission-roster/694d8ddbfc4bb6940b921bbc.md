...
Debes crear una copia superficial del arreglo `crew` usando el método `slice()`. Nombra el nuevo arreglo `updatedCrew`.

Recuerda que llamar a `slice()` sin argumentos devuelve una copia de todo el arreglo:
...
Debes usar el método `slice()` para copiar el arreglo de entrada `crew` y almacenar el resultado en una nueva variable llamada `updatedCrew`.
