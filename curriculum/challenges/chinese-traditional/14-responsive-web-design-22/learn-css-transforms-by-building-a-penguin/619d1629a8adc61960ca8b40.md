...
應該將 `.shirt` 的 `font-size` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontSize, '25px');
```

應該將 `.shirt` 的 `font-family` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.fontFamily, 'Helvetica, sans-serif');
```

應該將 `.shirt` 的 `font-weight` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
