...
現在你可以開始將你的函式解析器套用到你的 `evalFormula` 邏輯中。宣告一個 `functionExpanded` 變數，並將呼叫 `applyFunction` 並傳入你的 `cellExpanded` 字串的結果指定給它。

# --hints--

你應該宣告一個 `functionExpanded` 變數。
...
你應該將 `cellExpanded` 傳遞給你的 `applyFunction` 呼叫。
