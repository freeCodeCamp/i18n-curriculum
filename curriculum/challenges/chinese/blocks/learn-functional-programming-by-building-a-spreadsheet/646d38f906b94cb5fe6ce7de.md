...
你的 `addCharacters(char1)` 也返回一个函数，该函数返回另一个函数。 你需要进行另一个函数调用来访问 `.map()` 回调的最内层函数引用。 JavaScript 允许你立即调用返回的函数：

```js
myFunc(1)("hi");
```

立即调用从 `addCharacters(char1)` 调用返回的函数，并将 `char2` 作为参数传递。

# --hints--

你应该将函数调用链接到 `addCharacters(char1)` 调用。
