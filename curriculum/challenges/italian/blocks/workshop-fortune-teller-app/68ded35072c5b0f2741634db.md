...
Ti serve un'altra funzione che mostri elementi nel DOM. Simile all'ultima funzione, crea una funzione chiamata `showElements`. Questa volta usa `classList.remove("hidden")`.

# --hints--

Dovresti avere una funzione chiamata `showElements`.
...
La tua funzione `showElements` dovrebbe avere un parametro chiamato `...elements` di tipo `HTMLElement[]`.
...
Nella tua funzione `showElements` dovresti avere un metodo `forEach` allegato al parametro `elements`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Il tuo metodo `forEach` dovrebbe prendere un parametro chiamato `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Dovresti rimuovere la classe `hidden` usando il metodo `classList.remove` all'interno del metodo `forEach`.
