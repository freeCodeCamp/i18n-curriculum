...
يجب الآن تنفيذ الحلقة الرئيسية للمحاكاة. تضمن هذه الحلقة معالجة كل بوابة في كل علامة دقيقة في المحاكاة حتى اكتمال جميع العلامات الدقيقة.

أنشئ حلقة `while` فارغة تستمر طالما أن `tickIndex` أقل من `maxTicks`.

# --hints--

يجب إنشاء حلقة `while` فارغة بشرط `(tickIndex < maxTicks)`.
