...
Crea un'altra clausola `except` che intercetti `MediaError as e`. All'interno del blocco `except`, usa una f-string per stampare `Media Error:` seguito da uno spazio e dal messaggio di errore.

# --hints--

Dovresti avere una clausola `except` che intercetti `MediaError as e`.
...
Dovresti usare una f-string per stampare `Media Error:` seguito da uno spazio e dal messaggio di errore all'interno del tuo nuovo blocco `except`.
