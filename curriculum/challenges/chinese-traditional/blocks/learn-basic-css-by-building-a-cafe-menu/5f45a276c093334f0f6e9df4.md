...
專注於選單項目和價格，每行之間有相當大的間距。

使用現有的選擇器，該選擇器標的所有巢狀在類別名為 `item` 的元素中的 `p` 元素，並將它們的上下邊距設為 `5px`。

# --hints--

你應該將 `margin-top` 屬性設為 `5px`。
...
你在 `.item` 項目中的 `p` 元素應該有 `5px` 的 `margin-top`。
...
你在 `.item` 項目中的 `p` 元素應該有 `5px` 的 `margin-bottom`。
...
你在 `.item` 項目中的 `p` 元素不應該設定 `margin-left`。
...
你在 `.item` 項目中巢狀的 `p` 元素不應該設定 `margin-right`。
