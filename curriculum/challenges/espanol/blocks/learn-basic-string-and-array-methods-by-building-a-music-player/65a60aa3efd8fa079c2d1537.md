...
La segunda condición en este ejemplo verifica si `a.name > b.name`. Si es así, la función devuelve `1`, lo que ordena la primera fruta después de la segunda fruta.
...
Dentro de su función de retrollamada, agregue otra declaración `if` para verificar si `a.title` es mayor que `b.title`. Si es así, devuelva el número `1`. 

# --hints--

Debería tener una segunda declaración `if` dentro de su función de retrollamada.
...
Su declaración `if` debería verificar si `a.title` es mayor que `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
``` 

Su declaración `if` debería devolver el número `1` si `a.title` es mayor que `b.title`.
