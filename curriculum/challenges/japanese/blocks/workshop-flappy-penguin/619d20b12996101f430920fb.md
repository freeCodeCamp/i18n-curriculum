...
ペンギンのくちばしと足は同じ`color`を共有しています。

新しいカスタムCSS変数`--penguin-picorna`を作成し、関連するすべてのプロパティ値をそれに置き換えてください。

# --hints--

`:root`に`--penguin-picorna`プロパティを設定してください。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-picorna'));
```

`--penguin-picorna`には`orange`の値を設定する必要がありますが、`--fcc-actual--`が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-picorna', true), 'orange');
```

`.beak`には`background-color`の`var(--penguin-picorna)`を設定する必要がありますが、`--fcc-actual--`が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.getPropVal('background-color', true), 'var(--penguin-picorna)');
```

`.foot`には`background-color`の`var(--penguin-picorna)`を設定する必要がありますが、`--fcc-actual--`が見つかりました。
