...
建筑物间距太大了。 在 `.background-buildings` 元素的顶部添加两个空的 `div` 元素，在底部添加两个， 在 `.bb3` 和 `.bb4` 之间添加一个，使它们挤更紧凑。 这些将作为均匀分布的元素添加到整个容器中，有效地将建筑物移近中心。

# --hints--

你应该在 `.bb1` 元素之前添加两个新的 `div` 元素。
