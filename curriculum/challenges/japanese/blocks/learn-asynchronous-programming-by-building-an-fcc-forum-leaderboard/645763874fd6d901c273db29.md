...
最初の `td` 要素の中に、埋め込み式 `${}` を追加してください。その式の中で、引数 `forumCategory` を使って `category_id` 関数を呼び出してください。

これで、各投稿トピックの下にカテゴリーが表示されるはずです。

# --hints--

引数 `forumCategory` を使って `category_id` 関数を呼び出してください。
