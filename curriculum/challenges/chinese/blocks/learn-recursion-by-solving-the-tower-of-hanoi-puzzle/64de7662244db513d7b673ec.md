...
在两个 `elif` 子句中再次调用 `make_allowed_move()` 函数，并传入正确的参数。

# --hints--

你应该在第一个 `elif` 子句中调用 `make_allowed_move(source, auxiliary)`。
...
你应该在第二个 `elif` 子句中调用 `make_allowed_move(auxiliary, target)`。
