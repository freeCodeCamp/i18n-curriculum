...
이제 `subTitle`에 `meaning_up`을 할당하고, `title`에는 `foundCard.name`을 할당하세요. 그다음 `showElements` 메서드를 호출하고 elements에서 `fortuneDescription`를 전달하세요.

# --hints--

`this.elements.subTitle.textContent`에 `foundCard.meaning_up`을 할당해야 합니다.
...
`this.elements.title.textContent`에 `foundCard.name`을 할당해야 합니다.
...
`showElements` 안에서 `showFortune` 함수를 호출해야 합니다.
...
`this.elements.fortuneDescription`에 인수로 `showElements`를 전달해야 합니다.
