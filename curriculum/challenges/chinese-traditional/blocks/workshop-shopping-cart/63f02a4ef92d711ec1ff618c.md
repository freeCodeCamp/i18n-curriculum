...
當使用者添加一個項目時，你還需要更新購物車的總價格。請在 `ShoppingCart` 類別中創建一個 `calculateTotal` 方法。

在該方法中，宣告一個 `subTotal` 變數，並對 `items` 陣列使用 `reduce` 方法來計算陣列中每個項目的 `price` 屬性的總和。將 `total` 和 `item` 作為你的回呼的參數。

請記得在 `reduce` 方法中設定你的初始值。

# --hints--

你應該在 `ShoppingCart` 類別中創建一個 `calculateTotal` 方法。

```js
assert.isFunction(cart.calculateTotal);
```

你的 `calculateTotal` 方法應該宣告一個使用 `const` 的 `subTotal` 變數。
...
你的 `calculateTotal` 方法應該在 `items` 陣列上使用 `reduce` 方法。
...
你的 `reduce` 回呼應該使用 `total` 和 `item` 作為第一和第二個參數。請記得使用箭頭函式語法。
...
你的 `reduce` 回呼應該傳回 `total` 和 `item.price` 的總和。使用隱式傳回。
