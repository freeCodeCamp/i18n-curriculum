...
Elimina `orange` del degradado `.bb1d` y cambia el `80%` al `50%`. Esto hará que `--building-color1` sea sólido para la mitad superior y luego pasará a `--window-color1` para la mitad inferior.
...
Debes cambiar el ahora primer color `linear-gradient` para hacer una transición al `50%`.
