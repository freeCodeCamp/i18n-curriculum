...
`else` 節の中で、文字列 `"general"` を `selectedCategory.className` に代入してください。

その下で、文字列 `"General"` を `selectedCategory.category` に代入してください。

最後に、数値 `1` を `selectedCategory.id` に代入してください。

# --hints--

文字列 `"general"` を `selectedCategory.className` に代入してください。

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

文字列 `"General"` を `selectedCategory.category` に代入してください。

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

数値 `1` を `selectedCategory.id` に代入してください。
