...
Il metodo `sort()` accetta una funzione di callback di confronto che definisce l'ordine di ordinamento.

In questo esempio, la prima condizione `(a.name < b.name)` verifica se il nome del primo frutto è minore del nome del secondo frutto. Se è così, il primo frutto viene ordinato prima del secondo frutto.

Le stringhe vengono confrontate lessicograficamente, il che significa che vengono confrontate carattere per carattere. Per esempio, `"Apples"` è minore di `"Bananas"` perché `"A"` viene prima di `"B"` nell'alfabeto.

Il motivo per cui questo esempio restituisce numeri è che il metodo `sort()` si aspetta che venga restituito un numero. Se restituisci un numero negativo, il primo elemento viene ordinato prima del secondo elemento.
...
All'interno della tua funzione di callback, aggiungi un'istruzione `if` per verificare se `a.title` è minore di `b.title`. Se è così, restituisci `-1`.

# --hints--

Dovresti avere un'istruzione `if` all'interno della tua funzione di callback.

```js
assert.match(code, /if\s*\(/);
```

La tua istruzione `if` dovrebbe verificare se `a.title` è minore di `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

La tua istruzione `if` dovrebbe restituire `-1` se `a.title` è minore di `b.title`.
