...
Sasa, unahitaji kuongeza sifa ya kipengele `aria-current` kwa wimbo unaochezwa kwa sasa.

Kagua kama `songToHighlight` ipo. Ikiwa ipo, tumia njia ya `setAttribute` kwenye `songToHighlight` na pita `"aria-current"` na `"true"` kama hoja ya kwanza na ya pili.

# --hints--

`highlightCurrentSong` inapaswa kuongeza sifa ya kipengele `aria-current` kwa wimbo unaochezwa kwa sasa na kuondoa sifa hiyo kutoka kwa wimbo uliochezwa awali.
