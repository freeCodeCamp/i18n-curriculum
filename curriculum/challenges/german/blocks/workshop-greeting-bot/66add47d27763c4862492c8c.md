...
Für den letzten Schritt loggen Sie die Nachricht des Bots von `"Well, it was nice to talk to you. Have a nice day!"` in die Konsole.

Damit ist Ihr Begrüßungs-Bot fertig!

# --hints--

Sie sollten eine `console`-Anweisung in Ihrem Code haben.

```js
assert.lengthOf(__helpers.removeJSComments(code).match(/console\.log(.*)/g), 9);
```

Ihre `console`-Anweisung sollte den String `"Well, it was nice to talk to you. Have a nice day!"` in die Konsole schreiben.
