...
Crea un selettore `html` e imposta su di esso una proprietà `font-size` con valore `62.5%`. Questo imposterà la dimensione del carattere tipografico predefinita per la tua pagina web a 10px (il valore predefinito del browser è 16px).

Questo ti faciliterà il lavoro con le unità `rem` in seguito, dato che `2rem` corrisponderebbe a 20px.

# --hints--

Dovresti creare un selettore `html`.

```js
assert(new __helpers.CSSHelp(document).getStyle('html'));
```

Il tuo selettore `html` dovrebbe avere una proprietà `font-size` impostata su `62.5%`.
