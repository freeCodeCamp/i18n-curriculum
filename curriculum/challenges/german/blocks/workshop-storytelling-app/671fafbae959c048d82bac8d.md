...
Ändern Sie den `scaryStoryBtn`-Ereignislistener so, dass er die `displayStory`-Funktion mit dem Argument `"scary"` aufruft. Fügen Sie außerdem einen `click`-Ereignislistener zu den `funnyStoryBtn`- und `adventureStoryBtn`-Schaltflächen hinzu, die die `displayStory`-Funktion mit den Argumenten `"funny"` bzw. `"adventure"` aufrufen.

Damit haben Sie den Workshop erfolgreich abgeschlossen.

# --hints--

Wenn Sie auf den `scaryStoryBtn` klicken, sollte der `#result`-Absatz die gruselige Geschichte `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` mit einer Rahmenfarbe von `#ee4b2b` anzeigen.
...
Wenn Sie auf den `funnyStoryBtn` klicken, sollte der `#result`-Absatz die lustige Geschichte mit einer Rahmenfarbe von `#f1be32` anzeigen.
...
Wenn Sie auf den `adventureStoryBtn` klicken, sollte der `#result`-Absatz die Abenteuergeschichte mit einer Rahmenfarbe von `#acd157` anzeigen.
