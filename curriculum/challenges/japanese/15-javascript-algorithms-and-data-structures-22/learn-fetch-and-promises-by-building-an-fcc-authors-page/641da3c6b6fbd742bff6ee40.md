...
<dfn>フェッチ API</dfn> は、サーバーにネットワークリクエストを行うための、JavaScript の組み込みインターフェイスです。 フェッチ API には `fetch()` メソッドがあり、これを使って `GET`、`POST`、`PUT`、`PATCH` などのリクエストを送信できます。 このプロジェクトでは、freeCodeCamp ニュースの著者に関する情報を含む JSON ファイルを取得するために、`GET` リクエストを送信します。
