...
هذه عملية إعداد البرنامج ليعمل على جهاز. تنتهي بـ `-ing`.

# --explanation--

`Installing` هو فعل وضع برنامج أو تطبيق على جهاز ليُستخدم. على سبيل المثال:

`I'm installing the new update on my laptop.` – هذا يعني أن التحديث يُجهز ليستخدمه النظام.
