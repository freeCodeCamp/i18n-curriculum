...
あなたの猫の写真は現在、角が鋭くなっています。`border-radius`というCSSプロパティを使って、その角を丸くできます。

# --instructions--

`border-radius`をピクセル単位で指定できます。猫の写真に`border-radius`の`10px`を設定してください。

**注意：** このチャレンジには複数の解決方法があります。例えば、`border-radius`を`.thick-green-border`クラスか`.smaller-image`クラスのどちらかに追加してもかまいません。

# --hints--

画像要素には`thick-green-border`クラスを付けてください。

```js
assert.isTrue(document.querySelector('img').classList.contains('thick-green-border'));
```

画像には`10px`のボーダーラジアスを設定してください。
