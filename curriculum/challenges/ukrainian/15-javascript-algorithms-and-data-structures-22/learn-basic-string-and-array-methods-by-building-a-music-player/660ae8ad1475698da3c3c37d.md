...
Щоб викликати іменований вираз стрілкової функції, ви можете посилатися на функцію за її іменем.

```js
exampleArrowFunction();
```

Викличте функцію під визначенням функції `printGreeting`. Відкрийте консоль, щоб побачити вихідні дані.

# --hints--

Викличте функцію `printGreeting`.

```js
assert.match(code, /printGreeting\(\);?/);
```

Виклик функції `printGreeting` має бути нижче визначення функції `printGreeting`.
