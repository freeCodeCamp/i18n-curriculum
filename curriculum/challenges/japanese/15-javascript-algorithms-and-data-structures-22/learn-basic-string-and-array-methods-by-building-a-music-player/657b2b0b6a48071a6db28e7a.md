...
`map()` メソッドは、配列を反復処理して、新しい配列を返すために使われます。 既存の配列の値に基づいて新しい配列を作成したいときに便利です。 例えば、次のように記述します。
...
`map()` メソッドは、引数として関数を取ることに注意してください。 このように引数として別の関数に渡される関数は<dfn>コールバック関数</dfn>と呼ばれます。 上記の例では、`(number) => number * 2` がコールバック関数であり、この関数が `numbers` 配列の各要素に対して実行されます。 そしてその実行結果からなる新しい配列が `map()` メソッドから返されます。

では `map()` メソッドにコールバック関数を渡しましょう。 コールバック関数ではアロー関数構文を使用して、仮引数として `song` を取り、空の関数本体を持つようにしてください。

# --hints--

`map()` メソッドのコールバック関数はアロー関数構文を使用する必要があります。

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

`map()` メソッドのコールバック関数には仮引数 `song` が必要です。
