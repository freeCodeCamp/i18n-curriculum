...
У прикладі, якщо `a.name` дорівнює `b.name`, тоді функція повертає `0`. Це означає, що нічого не змінюється, і порядок `a` та `b` залишається незмінним.
...
Під інструкціями `if` поверніть число `0`, щоб залишити порядок двох елементів незмінним.

# --hints--

Поверніть число `0` під інструкціями `if`.
