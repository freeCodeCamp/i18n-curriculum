...
`f(k, n)` = $e^\frac{k}{n} - 1$ とします。ここで `k` はすべて非負の整数です。

驚くべきことに、`f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3.1415926… ≈ π です。

実際、これは `f(a, 200) + f(b, 200) + f(c, 200) + f(d, 200)` の形での π の最良近似です。

誤差 $\lvert f(a,n) + f(b,n) + f(c,n) + f(d,n) - \Pi\rvert$ を最小化する a、b、c、d に対して、`almostPi(n)` = a<sup>2</sup> + b<sup>2</sup> + c<sup>2</sup> + d<sup>2</sup> とします。

`almostPi(200)` = 6<sup>2</sup> + 75<sup>2</sup> + 89<sup>2</sup> + 226<sup>2</sup> = 64658 が与えられています。
...
`almostPi(29)` は `1208` を返すべきです。

```js
assert.strictEqual(almostPi(29), 1208);
```

`almostPi(50)` は `4152` を返すべきです。

```js
assert.strictEqual(almostPi(50), 4152);
```

`almostPi(200)` は `64658` を返すべきです。
