...
Тепер, коли у вас на екрані відображається список пісень, було б добре відсортувати їх за алфавітом за назвою.

Почніть зі створення стрілкової функції з назвою `sortSongs`. 

# --hints--

У вас має бути функція з назвою `sortSongs`.

```js
assert.isFunction(sortSongs);
```

Ваша функція `sortSongs` має використовувати синтаксис стрілкової функції.
