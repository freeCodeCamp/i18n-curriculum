...
`"click"` 요소에 `shuffleButton` 이벤트 리스너를 추가하세요. 함수가 실행되려면 `shuffle` 함수를 전달하세요.

**참고**: 이 특정 이벤트 리스너 안에 콜백이 필요하지 않습니다. `shuffle` 함수를 호출할 필요도 없고, 식별자만 전달하면 됩니다.

# --hints--

`addEventListener()`에 `shuffleButton` 메서드를 체인으로 연결해야 합니다.
...
`shuffle` 이벤트 리스너의 두 번째 값으로 `"click"`을 전달해야 합니다.
