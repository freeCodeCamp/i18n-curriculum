...
`.chin` 要素を親要素の上から `25%`、左から `5%` の位置に配置してください。それから、上の角に `70%` の半径を、下の角に `100%` の半径を設定してください。

# --hints--

`.chin` に `top` プロパティを設定してください。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

`.chin` に `top` の `--fcc-expected--` を設定する必要がありますが、現在は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

`.chin` に `left` プロパティを設定してください。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

`.chin` に `left` の `--fcc-expected--` を設定する必要がありますが、現在は `--fcc-actual--` です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

`.chin` に `border-radius` の `70% 70% 100% 100%` を設定してください。
