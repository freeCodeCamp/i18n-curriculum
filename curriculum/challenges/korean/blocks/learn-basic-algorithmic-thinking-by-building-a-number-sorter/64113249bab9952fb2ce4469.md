...
`Sort` 버튼을 다시 누르면 `10`이 이제 `Output`의 올바른 위치에 있는 것을 볼 수 있습니다.

이 프로젝트를 완료하려면 `option`을 다시 `value`와 `1` 텍스트로 변경하세요.

# --hints--

`value`인 `option` 요소의 `selected`를 `1`로 업데이트해야 합니다.
...
`option`인 `selected` 요소의 텍스트를 `1`로 업데이트해야 합니다.
