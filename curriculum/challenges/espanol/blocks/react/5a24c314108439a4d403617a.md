...
El componente `MyApp` tiene estado y renderiza un componente `Navbar` como hijo. Pasa la propiedad `name` en su `state` al componente hijo, luego muestra el `name` en la etiqueta `h1` que forma parte del método de renderizado de `Navbar`. `name` debe aparecer después del texto `Hello, my name is:`.
