...
Prossiga adicionando um `div` de `className` de `form-row`. Dentro dele, adicione dois elementos `div` com o `className` de `form-group` cada.

O primeiro elemento `div` deve ter um `label` de `className` de `label`, um atributo `htmlFor` de `position`, um texto `Position` e um `input` com o `id` de `position`, um `className` de `input` e um `type` de `text`.

# --hints--

Sua seção Player Info deve conter um `div` com o `className` de `form-row`.
...
Seu `div.form-row` deve conter dois elementos `div`, cada um com o `className` de `form-group`.
...
Seu primeiro `form-group` na linha deve ter um `label` com o `className` de `label` e um `htmlFor` de `position`.
...
Seu `label` de posição deve ter o texto `Position`.
...
Seu primeiro `form-group` na linha deve ter um `input` com o `className` de `input`, um `id` de `position` e `type` de `text`.
