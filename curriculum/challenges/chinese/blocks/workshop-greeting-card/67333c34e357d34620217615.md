...
为 `section` 元素创建选择器，并赋予它们：

- 设置为 `20px auto` 的 `margin` 属性，
- 设置为 `600px` 的 `max-width`。
- 一个设置为 `whitesmoke` 的 `background-color` 属性
- 一个 `padding` 属性设置为 `20px`
- 设置为 `10px` 的 `border-radius` 属性
...
`section` 选择器应具有设置为 `20px auto` 的 `margin` 属性。
