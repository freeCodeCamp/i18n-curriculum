...
Час перейти до вашої другої таблиці. Почніть із додавання `caption` елемента з множиною `Liabilities`. Потім додайте свої `thead` та `tbody`.

# --hints--

Ваш другий `table` елемент має містити `caption` елемент.

```js
assert(document.querySelectorAll('table')?.[1]?.children?.[0]?.localName === 'caption');
```

Ваш `caption` елемент має містити текст `Liabilities`.

```js
assert(document.querySelectorAll('table')?.[1]?.querySelector('caption')?.textContent === 'Liabilities');
```

Ваш другий `table` елемент має містити `thead` елемент.

```js
assert(document.querySelectorAll('table')?.[1]?.querySelector('thead'));
```

Ваш другий `table` елемент має містити `tbody` елемент.

```js
assert(document.querySelectorAll('table')?.[1]?.querySelector('tbody'));
```

Ваш `thead` елемент має бути безпосередньо під вашим `caption` елементом.

```js
assert(document.querySelectorAll('table')?.[1]?.querySelector('caption')?.nextElementSibling?.localName === 'thead');
```

Ваш `tbody` елемент має бути безпосередньо під вашим `thead` елементом.
