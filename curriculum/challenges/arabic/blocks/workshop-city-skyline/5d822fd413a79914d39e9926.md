...
أعطِ عناصر `.fb3-window` قيمة `width` تساوي `25%`، وقيمة `height` تساوي `80%`، واستخدم متغير `--window-color1` كقيمة لـ `background-color`.

# --hints--

يجب أن تعطي `.fb3-window` قيمة `width` تساوي `25%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

يجب أن تعطي `.fb3-window` قيمة `height` تساوي `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

يجب أن تعطي `.fb3-window` قيمة `background-color` تساوي `--window-color1`.
