...
Es wäre schön, die bearbeiteten Änderungen in der Konsole zu sehen.

Fügen Sie eine `console.log(currentContent);` hinzu.

Um Ihre Änderungen zu testen, bearbeiten Sie die Notiz und klicken Sie dann außerhalb der Notiz. Sie sollten die bearbeitete Notiz jetzt in der Konsole sehen.

# --hints--

Sie sollten eine `console.log(currentContent);` innerhalb des Ereignis-Listeners hinzufügen, nachdem Sie `newContent` `currentContent` zugewiesen haben.
