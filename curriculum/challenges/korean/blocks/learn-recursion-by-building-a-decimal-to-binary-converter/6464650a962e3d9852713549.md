...
`value`의 `numberInput`가 `5`와 같으면 `showAnimation()` 함수를 호출하고, 그다음 `return`를 조기에 실행하세요.

# --hints--

두 번째 `showAnimation()` 문 내부에서 `if` 함수를 호출해야 합니다.

```js
assert.match(String(checkUserInput), /if\s*\(\s*parseInt\(\s*numberInput\s*\.\s*value\s*\)\s*===?\s*5\s*\)\s*\{\s*showAnimation\(\s*\)/);
```

`showAnimation()` 함수를 호출한 후에는 `return` 함수에서 조기에 종료하려면 `checkUserInput()` 키워드를 사용하세요.
