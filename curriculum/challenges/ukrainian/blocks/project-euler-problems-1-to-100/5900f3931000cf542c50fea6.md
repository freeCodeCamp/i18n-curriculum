...
title: 'Завдання 39: цілі прямокутні трикутники'
challengeType: 1
forumTopicId: 302054
...
Якщо `p` — це периметр прямокутного трикутника з цілими сторонами {a,b,c}, то для p = 120 існує рівно три розв’язки.

{20,48,52}, {24,45,51}, {30,40,50}

Для якого значення `p` ≤ `n` кількість розв’язків максимальна?

# --hints--

`intRightTriangles(500)` має повертати число.

```js
assert(typeof intRightTriangles(500) === 'number');
```

`intRightTriangles(500)` має повертати 420.

```js
assert(intRightTriangles(500) == 420);
```

`intRightTriangles(800)` має повертати 720.

```js
assert(intRightTriangles(800) == 720);
```

`intRightTriangles(900)` має повертати 840.

```js
assert(intRightTriangles(900) == 840);
```

`intRightTriangles(1000)` має повертати 840.
