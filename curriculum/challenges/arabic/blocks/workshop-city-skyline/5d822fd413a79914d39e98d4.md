...
أعطِ أجزاء البناء خاصيتي `width` و `height` بالقيم التالية: `70%` و `10%` لـ `.bb1a`، و`80%` و `10%` لـ `.bb1b`، و`90%` و `10%` لـ `.bb1c`، و`100%` و `70%` لـ `.bb1d`. تذكر أن هذه النسب مئوية بالنسبة للحاوية الأصلية، ولاحظ أن الارتفاعات ستجمع لتصل إلى 100% - مما يملأ الحاوية عموديًا.
