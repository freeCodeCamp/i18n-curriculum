...
Practica elegir la forma correcta del verbo para un departamento singular.
...
¿Qué frase usa el verbo correcto?
...
`Verifican` es la forma para `ellos/ellas`, pero `el departamento` es singular.
...
`Verificar` es la forma en infinitivo, por lo que no completa correctamente la frase aquí.
...
`El departamento de control de calidad verifica el software.` - El departamento de control de calidad verifica el software.

El sujeto `el departamento` es singular, por lo que necesita la forma singular del verbo `verifica`. 
