...
У функції `playPreviousSong` додайте інструкцію `if` з умовою `userData?.currentSong === null`.  Це перевірить, чи не відтворюється жодна пісня. Якщо її немає, вийдіть із функції за допомогою `return`.

В межах блоку `else` створіть константу під назвою `currentSongIndex` та призначте до неї `getCurrentSongIndex()`.

# --hints--

Не змінюйте наявну функцію `playPreviousSong` та її вміст.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

Функція `playPreviousSong` повинна містити інструкцію `if` з умовою `userData?.currentSong === null`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

Блок інструкції `if` повинен мати `return`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

Інструкція `if` повинна мати блок `else`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

Викличте `getCurrentSongIndex` та призначте її `currentSongIndex` в межах блоку `else` інструкції `if`.
