...
Il tuo elemento `head` dovrebbe essere vuoto.

```js
assert(code.match(/<head\s*>\s*<\/head\s*>/i));
```

Il tuo elemento `body` dovrebbe essere vuoto.

```js
assert(code.match(/<body\s*>\s*<\/body\s*>/i));
```

Il tuo elemento `body` dovrebbe essere posizionato dopo il tuo elemento `head`.
