...
Fügen Sie innerhalb des `map()` eine `return`-Anweisung mit Backticks hinzu, in der Sie alle Elemente interpolieren, die für die Anzeige der Songdetails verantwortlich sind.

Erstellen Sie innerhalb der Backticks ein `li`-Element mit einem `id`-Attribut von `song-${song.id}` und einem `class`-Attribut von `playlist-song`.

# --hints--

Sie sollten eine `return`-Anweisung mit einem leeren Backtick-Paar hinzufügen.

```js
assert.match(code, /return\s*`/)
```

Sie sollten ein `li`-Tag innerhalb der Backticks erstellen.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

Ihr öffnendes `li`-Tag sollte ein `id`-Attribut mit dem Wert `song-${song.id}` haben.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

Ihr öffnendes `li`-Tag sollte ein `class`-Attribut mit dem Wert `playlist-song` haben.
