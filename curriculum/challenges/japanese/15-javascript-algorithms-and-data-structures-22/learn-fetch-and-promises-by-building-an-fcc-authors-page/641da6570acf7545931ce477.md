...
では、分割代入で取り出したデータを使って、ページに表示する HTML の作成を始めましょう。 これは複合代入演算子 (`+=`) と `innerHTML` プロパティの組み合わせで実現できます。

コールバック関数の中で、`authorContainer` の `innerHTML` に、複合代入演算子を使って空のテンプレートリテラルを追加してください。
