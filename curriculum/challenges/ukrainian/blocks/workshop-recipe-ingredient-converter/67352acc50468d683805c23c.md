...
Тепер, дотримуючись наведеного вище синтаксису, оновіть свою функцію `convertQuantity`, щоб вона повертала другу функцію. Ця внутрішня функція має приймати аргумент `toUnit` і бути порожньою.

# --hints--

Ваша функція `convertQuantity` має повертати функцію.

```js
assert.isFunction(convertQuantity());
```

Повернена функція має мати параметр `toUnit`.

```js
assert.match(convertQuantity()?.toString(), /\(toUnit\)/);
```

Повернена функція має бути порожньою.
