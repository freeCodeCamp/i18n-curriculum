...
ゲームの最後の部分では、ストレートの有無をチェックするアルゴリズムを作成する必要があります。スモールストレートは、4つのサイコロが任意の順序で連続した値を持つ場合です（例：`41423`のロールで`1234`がある場合）、スコアは`30`ポイントになります。ラージストレートは、5つすべてのサイコロが任意の順序で連続した値を持つ場合です（例：`35124`のロールで`12345`がある場合）、スコアは`40`ポイントになります。

数値の配列を受け取る`checkForStraights`関数を宣言してください。ユーザーがラージストレートを出した場合は、5番目のラジオボタンをスコア`40`で更新してください。ユーザーがスモールストレートを出した場合は、4番目のラジオボタンをスコア`30`で更新してください。いずれの場合も、最後のラジオボタンは常にスコア0を表示し、正しい属性で更新する必要があります。

`checkForStraights`がクリックされたときに、`rollDiceBtn`関数を呼び出してサイコロゲームを完成させてください！

# --hints--

あなたの`checkForStraights`変数は関数である必要があります。

```js
assert.isFunction(checkForStraights);
```

スモールストレートが出た場合、`checkForStraights`関数は4番目のラジオボタンを有効にし、値を`30`に設定し、表示されるテキストを`, score = 30`に更新し、5番目のラジオボタンは無効のままにしてください。
...
ラージストレートが出た場合、`checkForStraights`関数は4番目のボタンを有効にし、値を`30`に設定し、表示されるテキストを`, score = 30`に更新します。さらに、5番目のラジオボタンも有効にし、値を`40`に設定し、表示されるテキストを`, score = 40`に更新してください。
...
ストレートが出なかった場合、`checkForStraights`関数は4番目と5番目のラジオボタンを有効にしないでください。
...
ストレートが出なかった場合、`checkForStraights`関数は最後のラジオボタンを有効にし、値を`0`に設定し、表示されるテキストを`, score = 0`に更新してください。
...
`checkForStraights`がクリックされたときに、`rollDiceBtn`関数を呼び出す必要があります。
