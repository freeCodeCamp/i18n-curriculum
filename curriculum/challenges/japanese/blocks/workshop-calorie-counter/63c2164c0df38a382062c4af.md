...
各エントリーには名前用のテキスト入力とカロリー用の数値入力があります。エントリーの数を取得するには、テキスト入力でクエリを実行できます。

文字列`input[type="text"]`を`querySelectorAll()`メソッドに渡してください。文字列には二重引用符を使いたいので、シングルクォートで囲む必要があることを覚えておいてください。

これにより、フォーム内のすべてのテキスト入力の`NodeList`が返されます。次に、同じ行で`length`の`NodeList`プロパティにアクセスしてエントリーの数を取得できます。

# --hints--

文字列`input[type="text"]`を`querySelectorAll()`メソッドに渡してください。
