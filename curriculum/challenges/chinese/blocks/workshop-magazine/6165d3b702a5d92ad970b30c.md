...
在 `img` 元素之后，添加一个 `h1` 元素，其中 `class` 设置为 `hero-title`，文本设置为 `OUR NEW CURRICULUM`，然后添加一个 `p` 元素，其中 `class` 设置为 `hero-subtitle`，文本设置为 `Our efforts to restructure our curriculum with a more project-based focus`。
