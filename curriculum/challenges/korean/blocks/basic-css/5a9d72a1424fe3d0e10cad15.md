...
`:root`에서 변수를 만들면 해당 변수의 값이 페이지 전체에 설정됩니다.

그런 다음 특정 선택자 내에서 변수를 다시 설정하여 덮어쓸 수 있습니다.

# --instructions--

`--penguin-belly` 클래스에서 `white`의 값을 `penguin`로 변경하세요.

# --hints--

`penguin` 클래스는 `--penguin-belly` 변수를 `white`로 다시 할당해야 합니다.

```js
assert.match(__helpers.removeCssComments(code), /\.penguin\s*?{[\s\S]*--penguin-belly\s*?:\s*?white\s*?;[\s\S]*}/gi);
```

`penguin` 클래스에는 `background-color` 속성이 포함되어 있으면 안 됩니다.
