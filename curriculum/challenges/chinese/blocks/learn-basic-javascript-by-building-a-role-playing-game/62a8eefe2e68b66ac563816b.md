...
在你的 `defeatMonster` 函数中，将 `gold` 设置为等于 `gold` 加上怪物等级乘以 `6.7`。 请记住，你可以使用 `monsters[fighting].level` 获取怪物的等级。

下面是将 `num` 设置为 `num` + `5` * `8` 的示例：`num += 5 * 8`。 使用 `Math.floor()` 将结果向下舍入。
...
你应该使用 `Math.floor()` 来将怪物等级乘以 `6.7` 的结果四舍五入。
...
你应该将 `Math.floor(monsters[fighting].level * 6.7)` 的结果添加到 `gold`。
