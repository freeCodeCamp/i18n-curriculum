...
`else` 文に `if` ブロックを追加してください。`else` ブロックの中で、`getCurrentSongIndex()` 関数を呼び出し、`currentSongIndex` という名前の定数に代入してください。
...
`currentSongIndex` ブロックの中で `getCurrentSongIndex()` 定数に `else` を代入してください。
