...
創建另一個帶有類別 `playlist-song-delete` 且 `aria-label` 屬性設為與 `song.title` 插值的 `Delete` 的 `button` 元素。將以下 SVG 貼入刪除圖示的內容中：
