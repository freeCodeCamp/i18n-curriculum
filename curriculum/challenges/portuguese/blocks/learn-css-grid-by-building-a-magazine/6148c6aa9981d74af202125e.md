...
Dê ao seletor `.text` a propriedade `font-size` definida como `1.8rem` e a propriedade `letter-spacing` definida como `0.6px`.

# --hints--

O seletor `.text` deve ter a propriedade `font-size` definida como `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize === '1.8rem');
```

O seletor `.text` deve ter a propriedade `letter-spacing` definida como `0.6px`.
