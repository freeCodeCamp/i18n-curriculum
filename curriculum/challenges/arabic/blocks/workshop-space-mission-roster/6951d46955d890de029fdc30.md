...
كنت قد أضفت سابقًا استدعاء `console.log()` إلى دالة `swapCrewMembers` للتحقق من سلوكها، والآن لم يعد هذا الاستدعاء ضروريًا. لمنع وحدة التحكم من التزاحم في الخطوات القادمة، احذف حلقة `for` الحالية التي تسجل في وحدة التحكم.

# --hints--

احذف حلقة `for` من دالة `swapCrewMembers`. يجب حذف كامل كتلة الحلقة.
