...
Ahora que tienes los datos de la API, actualiza el estado `results` llamando a `setResults` y pasando un arreglo que contenga solo la propiedad `name` de cada fruta en el arreglo `data`. Usa el método `map()` para esto.

# --hints--

Debes actualizar `setResults` con `data.map(fruit => fruit.name)`.
