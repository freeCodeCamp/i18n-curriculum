...
Cuando un usuario edita la nota, quieres hacer un seguimiento de la versión editada. 

Usa `let` para crear una variable llamada `currentContent` y asignarle una cadena vacía.

*NOTA*: En una aplicación real normalmente guardarías tus notas en una base de datos. O incluso podrías guardarlas en el almacenamiento local. Sin embargo, trabajar con bases de datos y almacenamiento local está fuera del alcance de este taller y esos conceptos se enseñarán más adelante. 

# --hints--

Debes usar `let` para crear la variable `currentContent`.

```js
assert.match(code, /let\s+currentContent/);
```

Tu variable `currentContent` debe ser una cadena.

```js
assert.isString(currentContent);
```

Tu variable `currentContent` debe inicializarse con una cadena vacía.
