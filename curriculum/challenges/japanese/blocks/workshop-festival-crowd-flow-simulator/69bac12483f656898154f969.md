...
`for...in` ループを使って、入力の `summary` オブジェクト内のすべてのゲートを繰り返し処理し、処理した合計参加者数をログに記録してください。

各ゲートについて、次の形式でメッセージをログに記録してください：`gate + ": " + summary[gate] + " attendees processed"`。

例として、`summary` オブジェクトが `{ North: 5, East: 3, South: 4, West: 2 }` と定義されている場合、次のようにログに記録する必要があります：
...
`summary` オブジェクト内のすべての要素を繰り返し処理し、各要素ごとに次のメッセージをログに記録してください：`element + ": " + summary[element] + " attendees processed`。
