...
A imagem que você adicionou não está centralizada horizontalmente como o título `Coffee` acima dela. Elementos `img` são "como" elementos inline.

Para fazer a imagem se comportar como elementos de nível de bloco, como títulos, crie um seletor do tipo `img`. Defina a propriedade `display` para `block`, e use `margin-left` e `margin-right` para centralizá-la horizontalmente.

# --hints--

Você deve usar o seletor `img`.
...
O elemento `img` deve ter as propriedades `margin-left` e `margin-right` com o valor `auto`.
