...
Dentro do elemento `preview-panel`, adicione um elemento `p` de `className` de `preview-label` e texto `Live Preview`, outro elemento `p` de `className` de `preview-hint` e o texto `Updates as you type`, e um `div` com o `className` definido para o template literal `preview-box tier-${getPlayerTier(player.overallRating)}`.

Dentro do último `div`, insira o componente `PlayerCard` e sua prop.

# --hints--

Seu elemento `div.preview-panel` deve conter um `p` com o `className` de `preview-label` e o texto `Live Preview`.
...
Seu `div.preview-panel` deve conter um `p` com o `className` de `preview-hint` e o texto `Updates as you type`.
...
Seu `div.preview-panel` deve conter um `div` com o `className` de `preview-box` e um `className` dinâmico de tier prefixado com `tier-`.
...
Seu `PlayerCard` deve ser renderizado dentro do elemento `div` com `className` de `preview-box`.
