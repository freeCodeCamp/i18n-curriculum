...
`location["button text"]` 是一個包含三個元素的數組。 將 `button1.innerText` 賦值更改爲 `location["button text"][0]`，它代表數組的第一個元素。

# --hints--

你應該訪問 `location` 參數的 `"button text"` 屬性的第一個元素。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button text\1\s*\]\s*\[\s*0\s*\]/);
```

你應該將 `button1.innerText` 屬性設置爲 `location` 參數的 `"button text"` 屬性的第一個元素。
