...
现在你可以看到 `toggleItem()` 函数定义首次设置或更改的时间，你还应该记录当函数在各次渲染中保持不变时的情况。

为你刚写的 `if` 语句添加一个 `else` 语句。在 `else` 块内，将字串 `Current toggleItem function` 记录到控制台。

# --hints--

你应该为你在上一步编写的 `if` 语句添加一个 `else` 语句。

```js
assert.match(code, /if\s*\(\s*prevToggleItem\s*!==\s*toggleItem\s*\)\s*{.*}\s*else\s*{/si);
```

你应该在 `else` 语句内将字串 `Current toggleItem function` 记录到控制台。
