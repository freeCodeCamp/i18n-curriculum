...
創建一個名為 `singleCardSelected` 的方法。在其中，呼叫 `hideElements` 函式並傳入以下屬性：`singleCardBtn`、`multipleCardsBtn`、`multipleCard`、`text` 和 `headerTitle`。

記得你必須使用 `this.elements` 來存取它們全部。

# --hints--

你應該定義一個名為 `singleCardSelected` 的方法。
...
在 `singleCardSelected` 裡，你應該呼叫 `hideElements` 函式。
...
你應該將 `singleCardBtn` 傳給 `hideElements` 函式。
...
你應該將 `multipleCardsBtn` 傳給 `hideElements` 函式。
...
你應該將 `multipleCard` 傳給 `hideElements` 函式。
...
你應該將 `headerTitle` 傳給 `hideElements` 函式。
