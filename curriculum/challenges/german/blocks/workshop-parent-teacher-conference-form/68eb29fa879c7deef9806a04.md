...
Um sicherzustellen, dass der in Eingaben und Textareas eingegebene Text sowie deren Platzhalter sichtbar sind, setzen Sie deren Farbe.

Wählen Sie `input, input::placeholder, textarea` aus und setzen Sie `color` auf `whitesmoke`.

# --hints--

Sie sollten `input`, `input::placeholder` und `textarea` ansprechen.
