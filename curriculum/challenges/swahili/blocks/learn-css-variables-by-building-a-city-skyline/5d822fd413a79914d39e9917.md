...
Unaweza kuongeza mchanganyiko wa rangi nyingi kwenye kipengele kwa kuzitenganisha kwa koma (`,`) kama hii:
...
Ongeza `repeating-linear-gradient` kwa `.fb1c` chini ya ile iliyopo; tumia `--building-color4` yako kutoka `0%` hadi `10%` na `--window-color4` kutoka `10%` na `90%`. Hii itajaza nyuma ya mchanganyiko wa rangi uliouongeza mwisho.

# --hints--

Haupaswi kubadilisha `repeating-linear-gradient` ya kwanza.
...
Unapaswa kuongeza `repeating-linear-gradient` yenye rangi ya kwanza `--building-color4` kutoka `0%` hadi `10%`.
...
Unapaswa kutumia rangi ya pili `--window-color4` kutoka `10%` hadi `90%`.
