...
마지막 단계로, `print` 함수에서 `verify_card_number` 호출을 제거하고, `card_number`를 다시 유효한 값으로 변경하세요.

# --hints--

`print` 함수 안에 `verify_card_number` 호출이 없어야 합니다.
...
`card_number`를 다시 유효한 값으로 변경해야 합니다.
