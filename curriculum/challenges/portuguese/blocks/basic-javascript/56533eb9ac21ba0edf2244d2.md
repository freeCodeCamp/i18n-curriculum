...
The inequality operator (`!=`) is the opposite of the equality operator. Desigualdade significa que duas coisas não são iguais. O operador de desigualdade retorna `false` onde o operador de igualdade retornaria `true` e *vice-versa*. Like the equality operator, the inequality operator will convert data types of values while comparing.
