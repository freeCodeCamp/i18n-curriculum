...
Розмістіть елемент `.chin` так, щоб він був `25%` від верху та `5%` від лівого краю свого батьківського елемента. Потім задайте верхнім кутам радіус `70%`, а нижнім кутам — радіус `100%`.

# --hints--

Ви повинні задати `.chin` властивість `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Ви повинні задати `.chin` властивість `top` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Ви повинні задати `.chin` властивість `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Ви повинні задати `.chin` властивість `left` зі значенням `--fcc-expected--`, але знайдено `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Ви повинні задати `.chin` властивість `border-radius` зі значенням `70% 70% 100% 100%`.
