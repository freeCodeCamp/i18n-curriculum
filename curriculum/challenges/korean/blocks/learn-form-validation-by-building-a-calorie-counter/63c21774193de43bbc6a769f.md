...
`label` 요소에 `for` 속성을 `X-#-name` 값으로 지정하세요. 여기서 `X`는 `value` 요소의 `entryDropdown`이고 `#`는 `entryNumber`의 값입니다. HTML 속성은 큰따옴표로 감싸야 한다는 점을 기억하세요.

# --hints--

`label` 요소에 `for` 속성을 `${entryDropdown.value}-${entryNumber}-name` 값으로 지정해야 합니다.

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{entryDropdown\.value\}-\$\{entryNumber\}-name"\s*>/)
```

`label` 요소는 이전과 같은 텍스트를 가져야 합니다.
