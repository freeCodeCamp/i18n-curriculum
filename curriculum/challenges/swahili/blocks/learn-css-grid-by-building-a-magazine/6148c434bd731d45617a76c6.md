...
Ikiwa ungependa kuongeza alama zaidi za kijamii, lakini kuzihifadhi katika safu moja, ungetakiwa kusahihisha `grid-template-columns` ili kuunda safu za ziada. Kama mbadala, unaweza kutumia sifa ya `grid-auto-flow`.

Sifa hii inachukua au `row` au `column` kama thamani ya kwanza, na thamani ya pili ya hiari ya `dense`. `grid-auto-flow` hutumia algoriti ya upangaji wa moja kwa moja kurekebisha mpangilio wa gridi. Kuweka thamani yake kuwa `column` kutamwambia algoriti kuunda safu mpya kwa maudhui inapohitajika. Thamani ya `dense` inaruhusu algoriti kurudi nyuma na kujaza mapengo katika gridi kwa vitu vidogo, jambo ambalo linaweza kusababisha vitu kuonekana nje ya mpangilio.

Kwa kichaguzi chako cha `.social-icons`, weka sifa ya `grid-auto-flow` kuwa `column`.

# --hints--

Kichaguzi chako cha `.social-icons` kinapaswa kuwa na sifa ya `grid-auto-flow` iliyowekwa kuwa `column`.
