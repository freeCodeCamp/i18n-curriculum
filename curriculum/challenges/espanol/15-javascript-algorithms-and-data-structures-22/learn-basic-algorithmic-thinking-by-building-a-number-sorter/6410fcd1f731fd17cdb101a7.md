...
Dentro de tu bucle anidado `for`, añade una llamada a `console.log()` para verificar los valores de `array`, `array[j]` y `array[minIndex]` en cada iteración. Puedes hacer clic en el botón `Sort` para ver cómo tu algoritmo recorre el array.

Luego escribe una declaración `if` que verifique si el valor en `array[j]` es menor que el valor en `array[minIndex]`. Si lo es, establece `minIndex` en `j`.

# --hints--

Deberías tener una llamada a `console.log()` dentro de tu bucle anidado `for`.
...
Deberías pasar `array` como el primer argumento a `console.log()`.
...
Deberías pasar `array[j]` como el segundo argumento a `console.log()`.
...
Deberías pasar `array[minIndex]` como el tercer argumento a `console.log()`.
...
Deberías tener una declaración `if` después de tu llamada a `console.log()`.
...
Deberías tener una condición en tu declaración `if` que verifique si `array[j]` es menor que `array[minIndex]`.
...
Tu declaración `if` debería establecer `minIndex` en `j`.
