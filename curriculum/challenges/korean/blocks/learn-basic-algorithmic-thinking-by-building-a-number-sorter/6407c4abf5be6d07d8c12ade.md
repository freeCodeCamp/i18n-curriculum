...
배열의 각 요소에 작업을 수행하려면 배열을 순회하는 데 적합한 메서드를 사용하세요.

`forEach()` 메서드를 사용하고, `num`과 `i`를 매개변수로 받는 빈 콜백을 전달하세요.

# --hints--

`.forEach()` 매개변수를 순회하려면 `array` 메서드를 사용해야 합니다.

```js
assert.match(code, /array\.forEach\(/);
```

`.forEach()` 메서드는 화살표 구문을 사용하는 콜백 함수를 받아야 합니다.

```js
assert.match(code, /array\.forEach\s*\(\s*\(.*\)\s*=>/);
```

콜백 함수는 `num`과 `i` 두 개의 매개변수를 받아야 합니다.
