...
Os formulários normalmente usam caixas de seleção para perguntas que tenham mais de uma resposta.O elemento `input` com um atributo `type` definido como `checkbox` cria uma caixa de seleção.

Sob o elemento `legend` que você acabou de adicionar, adicione um `input` com atributo do elemento `type` definido como `checkbox` e o texto definido como:
...
O texto ` Loving` deve estar localizado diretamente à direita da sua caixa de seleção.Certifique-se de que haja um espaço entre o elemento e o texto.Você omitiu o texto ou tem um erro de digitação.
