...
Тепер оновіть свій селектор `h1`, щоб він був `header h1` і конкретно націлювався на ваш елемент `h1` всередині вашого нового `header`.

# --hints--

У вас має бути селектор `header h1`.

```js
assert(new __helpers.CSSHelp(document).getStyle('header h1'));
```

У вас не має бути селектора `h1`.

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('h1'));
```

Ви не повинні змінювати жодних пропертів у селекторі.
