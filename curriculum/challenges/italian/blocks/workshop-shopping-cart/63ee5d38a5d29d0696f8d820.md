...
Nel tuo template letterale, crea un elemento `div` con una classe `dessert-card`. In quel `div`, crea un elemento `h2` e assegnagli il testo della variabile `name`.
...
Il tuo elemento `div` dovrebbe avere una classe `dessert-card`.
...
Il tuo elemento `h2` dovrebbe avere il testo della variabile `name`.

```js
assert.equal(document.querySelectorAll('h2')[0]?.textContent, 'Vanilla Cupcakes (6 Pack)');
```

Il tuo elemento `h2` dovrebbe essere all’interno dell’elemento `div`.
