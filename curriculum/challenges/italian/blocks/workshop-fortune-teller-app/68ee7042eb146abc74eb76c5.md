...
Ancora una volta, devi rendere la carta casuale e invertita. Dichiara una variabile chiamata `isReversed` e `card`. Per la prima, chiama il metodo `Math.random` e confrontalo con `< 0.5`. Per la seconda, chiama `getRandomItem` e passa `this.cards`.

# --hints--

Dovresti dichiarare una variabile chiamata `isReversed`.
...
La tua variabile `isReversed` dovrebbe usare `Math.random()` per decidere casualmente se la carta è invertita.
...
Dovresti dichiarare una variabile chiamata `card`.
...
La tua variabile `card` dovrebbe essere assegnata chiamando il metodo `getRandomItem` con `this.cards` come argomento.
