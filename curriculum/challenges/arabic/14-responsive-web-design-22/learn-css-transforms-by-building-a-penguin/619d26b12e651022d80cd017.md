...
يجب عليك استخدام منتقي `.arm`.

```js
assert.match(code, /\.arm\s*\{/);
```

بداخل منتقي `.arm` يجب أن تكون خاصية `width` بقيمة `--fcc-expected--`، ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

بداخل منتقي `.arm` يجب أن تكون خاصية `height` بقيمة `--fcc-expected--`، ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

يجب عليك إعطاء `.arm` الخاصية `background` بقيمة `linear-gradient(90deg, gray, rgb(209, 210, 199))`.
