...
宣告另一個名為 `foundCard` 的變數，並將其指定為對 `find()` 呼叫 `cards` 方法的結果。將 `card` 傳入作為回呼參數，並使用嚴格相等性比較 `card.name_short` 與 `cardId` 以尋找匹配的卡片。

# --hints--

你應該在 `foundCard` 內宣告一個名為 `showFortune` 的變數。
...
你的 `foundCard` 應該透過對 `find` 呼叫 `this.cards` 並傳入正確的回呼來指定。
