...
在 `playSong` 函數內部，將 `audio.src` 屬性設置爲 `song.src`。 這告訴音頻元素在哪裏可以找到所選歌曲的音頻數據。

另外，將 `audio.title` 屬性設置爲等於 `song.title`。 這告訴音頻元素要顯示什麼作爲歌曲的標題。

# --hints--

你不應修改現有的 `playSong` 函數及其內容。
...
你應該將 `audio.title` 設置爲 `song.title`。
