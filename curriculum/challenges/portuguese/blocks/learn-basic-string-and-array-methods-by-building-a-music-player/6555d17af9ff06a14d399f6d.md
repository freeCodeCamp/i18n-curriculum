...
Dentro da função, obtenha referências aos elementos HTML responsáveis por exibir o título da música e o artista.

Acesse os elementos `#player-song-title` e `#player-song-artist` com o método `getElementById()`. Atribua-os às variáveis `playingSong` e `songArtist`, respectivamente.

# --hints--

Você não deve modificar a função `setPlayerDisplay` existente e seu conteúdo.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

Você deve usar `document.getElementById()` para obter o elemento `#player-song-title`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Você deve atribuir o elemento `#player-song-title` à variável `playingSong`. Não se esqueça de usar `const` para declarar a variável.

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

Você deve usar `document.getElementById()` para obter o elemento `#player-song-artist`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

Você deve atribuir o elemento `#player-song-artist` à variável `songArtist`. Não se esqueça de usar `const` para declarar a variável.
