...
لإكمال الخلفية، أضف شمسًا بإنشاء عنصر `div` جديد مباشرة بعد عنصر `.back-mountain`، وأعطه الصنف `sun`.

# --hints--

يجب إضافة عنصر `div` جديد إلى `body`. كان متوقعًا وجود --fcc-expected-- عناصر `div`، لكن وُجد --fcc-actual--.

```js
assert.lengthOf(document.querySelectorAll('body > div'), 5);
```

يجب أن تعطي عنصر الـ `div` الجديد `class` باسم `sun`.
