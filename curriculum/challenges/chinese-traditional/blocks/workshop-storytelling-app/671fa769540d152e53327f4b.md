...
選取 `#scary-btn`、`#funny-btn` 和 `#adventure-btn` 按鈕，並將它們儲存在名為 `scaryStoryBtn`、`funnyStoryBtn` 和 `adventureStoryBtn` 的變數中。

# --hints--

你應該使用 `getElementById` 方法選取 `#scary-btn` 按鈕，並將它儲存在 `scaryStoryBtn` 變數中。
...
你應該使用 `getElementById` 方法選取 `#funny-btn` 按鈕，並將它儲存在 `funnyStoryBtn` 變數中。
...
你應該使用 `getElementById` 方法選取 `#adventure-btn` 按鈕，並將它儲存在 `adventureStoryBtn` 變數中。
