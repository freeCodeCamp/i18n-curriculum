...
يجب عليك استخدام منتقي `.eye-lid`.

```js
assert.match(code, /\.eye-lid\s*\{/);
```

يجب أن يحتوي عنصر `.eye-lid` على خاصية `width` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

يجب أن يحتوي عنصر `.eye-lid` على خاصية `height` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.height, '100%');
```

يجب عليك إعطاء `.eye-lid` خاصية `background-color` بقيمة `var(--penguin-face)`.
