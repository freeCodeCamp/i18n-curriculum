...
你可以在该部分的顶部看到直观的颜色变化。 将此部分的渐变类型从 `linear-gradient` 更改为 `repeating-linear-gradient`。 这将使渐变的四种颜色重复，直到它到达元素的底部；给你一些条纹，让你不必添加一堆元素来创建它们。

# --hints--

你应该将 `.bb2b` 的 `background` 属性从使用 `linear-gradient` 更改为使用 `repeating-linear-gradient`。
