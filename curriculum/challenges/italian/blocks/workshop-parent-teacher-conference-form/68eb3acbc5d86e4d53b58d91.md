...
Imposta `font-size` su `1.1rem`, `display` su `block` e `margin` su `auto` per centrare il pulsante.

# --hints--

Dovresti continuare a puntare alla classe `.submit-btn`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

Imposta la proprietà `font-size` su `1.1rem`.
...
Imposta la proprietà `margin` su `auto`.
