...
2. Deve haver um elemento `p` com o `id` de `char-count`. Ele deve conter inicialmente o texto `Character Count: 0/50`. Este texto placeholder deve ser substituído pela contagem atual de caracteres no elemento `textarea`.
...
7. O usuário não deve ser capaz de inserir mais de `50` caracteres no elemento `textarea`. Quando a contagem de caracteres atingir `50`, qualquer entrada extra deve ser automaticamente cortada e o texto `Character Count: 50/50` deve ser exibido em `red`.
...
O elemento `#char-count` deve conter inicialmente o texto `Character Count: 0/50`.
