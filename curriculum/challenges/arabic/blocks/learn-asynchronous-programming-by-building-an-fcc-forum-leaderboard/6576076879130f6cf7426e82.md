...
كل موضوع في المنتدى يتضمن فئة مثل Python أو JavaScript. في الخطوات التالية، ستبني كائن فئة يحتوي على جميع فئات المنتدى و`classNames` للتنسيق.

ابدأ بإنشاء ثابت جديد يسمى `allCategories` وقم بتعيين قيمة كائن فارغ له.

# --hints--

يجب أن يكون لديك ثابت يسمى `allCategories`.

```js
assert.match(code, /const\s+allCategories\s*=/)
```

يجب تعيين كائن فارغ للمتغير `allCategories`.
