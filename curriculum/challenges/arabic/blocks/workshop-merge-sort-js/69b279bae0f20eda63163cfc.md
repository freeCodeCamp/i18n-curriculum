...
أضف `1` إلى قيمة `sortedIndex`.

لاحظ أن التعيين يجب أن يتم بعد تعيين عنصر الجزء الأيسر إلى المصفوفة المرتبة.

# --hints--

يجب زيادة القيمة الحالية لـ `sortedIndex` بمقدار واحد داخل حلقة `while`.
