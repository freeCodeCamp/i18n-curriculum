...
剩餘的樂器已為你添加到 `instrumentsArr` 陣列中。

現在，宣告兩個變數 `selectContainer` 和 `productsContainer`，並分別將你的 HTML 下拉選單和具有類別 `products-container` 的元素指定給它們。

# --hints--

你應該有一個 `selectContainer` 變數。

```js
assert.exists(selectContainer);
```

你的 `selectContainer` 變數應該具有你的 `select` 元素的值。

```js
assert.strictEqual(selectContainer, document.querySelector("select"));
```

你應該有一個 `productsContainer` 變數。

```js
assert.exists(productsContainer);
```

你的 `productsContainer` 變數應該具有你的 `.products-container` 元素的值。
