...
title: Costruisci una pagina di documentazione tecnica
challengeType: 14
...
**Obiettivo:** Costruisci un'app che sia funzionalmente simile a <a href="https://technical-documentation-page.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://technical-documentation-page.freecodecamp.rocks</a>. **Non copiare questo progetto dimostrativo**.
...
1. All'interno dell'elemento `#main-doc`, puoi vedere diversi elementi `section`, ciascuno con una classe `main-section`. Ce ne devono essere almeno cinque
1. Il primo elemento all'interno di ogni `.main-section` dovrebbe essere un elemento `header`, che contiene un testo che descrive l'argomento di quella sezione.
1. Ogni elemento `section` con la classe `main-section` dovrebbe avere anche un `id` che corrisponde al testo di ogni `header` contenuto al suo interno. Eventuali spazi devono essere sostituiti con underscore (ad esempio la sezione che contiene l'intestazione "JavaScript and Java" dovrebbe avere un corrispondente `id="JavaScript_and_Java"`)
...
1. L'elemento navbar dovrebbe contenere un elemento `header` che contiene un testo che descrive l'argomento della documentazione tecnica
1. Inoltre, la barra di navigazione dovrebbe contenere elementi link (`a`) con la classe `nav-link`. Ce ne deve essere uno per ogni elemento con la classe `main-section`
1. L'elemento `header` nel `#navbar` deve venire prima di qualsiasi elemento link (`a`) nella barra di navigazione
1. Ogni elemento con la classe `nav-link` dovrebbe contenere un testo che corrisponde al testo `header` all'interno di ogni `section` (ad esempio se hai una sezione/intestazione "Hello world", la tua barra di navigazione dovrebbe avere un elemento che contiene il testo "Hello world")
1. Quando clicchi su un elemento della barra di navigazione, la pagina dovrebbe navigare alla sezione corrispondente dell'elemento `#main-doc` (ad esempio se clicchi su un elemento `.nav-link` che contiene il testo "Hello world", la pagina naviga a un elemento `section` con quell'id e contiene l'intestazione corrispondente)
1. Su dispositivi di dimensioni normali (laptop, desktop), l'elemento con `id="navbar"` dovrebbe essere mostrato sul lato sinistro dello schermo e dovrebbe essere sempre visibile all'utente
1. La tua documentazione tecnica dovrebbe usare almeno una media query

Completa le storie degli utenti e supera tutti i test qui sotto per completare questo progetto. Dagli il tuo stile personale. Buona programmazione!
...
Dovresti avere almeno cinque elementi `section` con una classe `main-section`.
...
Tutti i tuoi elementi `.main-section` dovrebbero essere elementi `section`.
...
Dovresti avere almeno cinque elementi `.main-section` che sono discendenti di `#main-doc`.
...
Il primo figlio di ogni `.main-section` dovrebbe essere un elemento `header`.
...
Nessuno dei tuoi elementi `header` dovrebbe essere vuoto.
...
Tutti i tuoi elementi `.main-section` dovrebbero avere un `id`.
...
Ogni `.main-section` dovrebbe avere un `id` che corrisponde al testo del suo primo figlio, con eventuali spazi nel testo del figlio sostituiti da underscore (`_`) per gli id.
...
Dovresti avere almeno 10 elementi `p` (in totale) all'interno dei tuoi elementi `.main-section`.
...
Dovresti avere almeno cinque elementi `code` che sono discendenti di elementi `.main-section`.
...
Dovresti avere almeno cinque elementi `li` che sono discendenti di elementi `.main-section`.
...
Dovresti avere un elemento `nav` con un `id` di `navbar`.
...
Il tuo `#navbar` dovrebbe avere esattamente un elemento `header` al suo interno.
...
Dovresti avere almeno un elemento `a` con una classe `nav-link`.
...
Tutti i tuoi elementi `.nav-link` dovrebbero essere elementi àncora (`a`).
...
Tutti i tuoi elementi `.nav-link` dovrebbero essere nel `#navbar`.
...
Dovresti avere lo stesso numero di elementi `.nav-link` e `.main-section`.
...
L'elemento `header` nel `#navbar` dovrebbe venire prima di qualsiasi elemento link (`a`) anch'esso nel `#navbar`.
...
Ogni `.nav-link` dovrebbe avere un testo che corrisponde al testo `header` del suo `section` correlato (ad esempio se hai una sezione/intestazione "Hello world", il tuo `#navbar` dovrebbe avere un `.nav-link` che ha il testo "Hello world").
...
Ogni `.nav-link` dovrebbe avere un attributo `href` che collega al suo `.main-section` corrispondente (ad esempio se clicchi su un elemento `.nav-link` che contiene il testo "Hello world", la pagina naviga a un elemento `section` con quell'id).
...
Il tuo `#navbar` dovrebbe essere sempre sul bordo sinistro della finestra.
...
Il tuo progetto di documentazione tecnica dovrebbe usare almeno una media query.
