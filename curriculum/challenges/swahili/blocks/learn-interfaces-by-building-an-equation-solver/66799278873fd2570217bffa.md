...
Ongeza `case` nyingine kwa wakati mrefu wa `results` ni `1`. Katika kesi hii, weka thamani ya `result_list` kuwa orodha inayojumuisha mfuatano wa herufi wenye muundo wa `x = <root>`, ambapo `<root>` ni suluhisho la mlinganyo. Panga mfuatano wa herufi ili kuonyesha alama chanya na hasi kwa suluhisho.

# --hints--

Haupaswi kubadilisha thamani ya somo la tamko lako la `match`.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("len(results)")`)) })
```

Haupaswi kubadilisha kipande chako kilichopo cha `case`.
...
Unapaswa kuunda `case` mpya yenye muundo wa `1` baada ya kipande kilichopo cha `case`.
...
Unapaswa kuweka thamani ya orodha inayojumuisha `f'x = {results[0]:+}'` ndani ya mwili mpya wa `result_list` kwa `case`.
