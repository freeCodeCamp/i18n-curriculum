...
You will need to use the `dotenv` package. 它將環境變量從你的 `.env` 文件加載到 `process.env` 中。 `dotenv` 包已經安裝，並且在你項目的 `package.json` 文件中。 在你的 `myApp.js` 文件的頂端，添加 `require('dotenv').config()` 以加載環境變量。
