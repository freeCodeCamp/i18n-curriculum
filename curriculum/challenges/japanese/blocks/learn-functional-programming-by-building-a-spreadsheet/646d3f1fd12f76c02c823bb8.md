...
少しバグがあります。`arg1` と `arg2` は文字列であって、数値ではありません。`infixToFunction['+']("1", "2")` は `12` を返しますが、これは数学的に正しくありません。

`infixToFunction[operator]` の各引数を `parseFloat()` の呼び出しで囲んでください。

# --hints--

`arg1` を `parseFloat()` の呼び出しで囲む必要があります。
...
`arg2` を `parseFloat()` の呼び出しで囲む必要があります。
