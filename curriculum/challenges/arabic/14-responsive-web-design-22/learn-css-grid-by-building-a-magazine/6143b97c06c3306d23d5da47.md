...
يجب عليك إنشاء عنصر `h3` جديد داخل عنصر `article` الخاص بك.

```js
assert.exists(document.querySelector('article h3'));
```

يجب عليك إنشاء عنصر `p` جديد داخل عنصر `article` الخاص بك.

```js
assert.exists(document.querySelector('article p'));
```

يجب عليك إنشاء عنصر `ul` جديد داخل عنصر `article` الخاص بك.

```js
assert.exists(document.querySelector('article ul'));
```

يجب أن تكون عناصرك داخل عنصر `article` بالترتيب الصحيح.
...
عنصر `h3` الجديد الخاص بك يجب أن يحتوي على `class` تم تعيينه إلى `list-title`.
...
يجب أن يحتوي عنصر `ul` الجديد على `class` تم تعيينه إلى `lists`.
