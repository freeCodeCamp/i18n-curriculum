...
Вкладіть шість елементів `div` в `.foreground-buildings` та надайте їм класи від `fb1` до `fb6` (за таким же порядком). «fb» означає «foreground building». Це будуть ще шість будівель для переднього плану.
