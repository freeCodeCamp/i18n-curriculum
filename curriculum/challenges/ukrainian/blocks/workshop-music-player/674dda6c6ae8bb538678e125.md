...
Перед відтворенням пісні потрібно переконатися, що вона починається з початку. Це можна зробити, використовуючи властивість `currentTime` об’єкта `audio`.

Додайте інструкцію `if`, щоб перевірити, чи жодна пісня зараз не відтворюється, перевіривши, що `userData.currentSong` є хибним.
У блоці `if` встановіть властивість `currentTime` об’єкта `audio` у значення `0`.

# --hints--

Ваша функція `playSong` має встановити властивість `currentTime` об’єкта `audio` у значення `0`, коли жодна пісня зараз не відтворюється.
