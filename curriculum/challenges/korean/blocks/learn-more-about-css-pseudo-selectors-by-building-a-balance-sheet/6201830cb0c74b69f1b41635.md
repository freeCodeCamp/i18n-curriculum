...
테이블이 컨테이너를 가득 채우도록 `width` 속성을 `100%`로 설정한 다음, 상대 위치를 지정하고 위쪽 여백을 `3rem`로 주세요.

# --hints--

`table` 선택자에 `width` 속성을 `100%`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('width') === '100%');
```

`table` 선택자에 `position` 속성을 `relative`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('position') === 'relative');
```

`table` 선택자에 `margin-top` 속성을 `3rem`로 설정하세요.
