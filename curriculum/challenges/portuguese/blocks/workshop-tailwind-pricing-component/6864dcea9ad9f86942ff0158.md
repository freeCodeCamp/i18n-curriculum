...
É hora de trabalhar no elemento `div` que envolve os três cartões de preços. Para organizar os cards, você pode usar as classes utilitárias de grid do Tailwind CSS.

Aqui está como atribuir uma exibição `grid` a um elemento usando Tailwind:
...
Atribua uma classe `grid` ao elemento `div` que envolve os cartões de preços.
