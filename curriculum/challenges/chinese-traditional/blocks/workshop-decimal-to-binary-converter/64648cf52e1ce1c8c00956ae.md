...
首先，將 `result` 的 `innerText` 屬性設為 `"Call Stack Animation"`。

# --hints--

你應該將 `result` 的 `innerText` 屬性設為字串 `"Call Stack Animation"`。

```js
assert.match(String(showAnimation), /result\.innerText\s*=\s*('|"|`)Call Stack Animation\1/i);
```

在呼叫 `showAnimation` 函式後，`result` 的 `innerText` 屬性應該是 `"Call Stack Animation"`。
