...
Ingawa programu yako inafanya kazi kama inavyotarajiwa, kawaida hutoi sehemu nyingi za `Card` kwa mkono kama hivi.

Ondoa sehemu tatu za `<Card />` ndani ya kipengele cha `<div className="flex-container">`. Katika hatua chache zijazo, utaunda safu ya data na utatumia kitendakazi cha `map()` kuonyesha sehemu za `Card` ambazo ni njia inayotumika zaidi katika React.

# --hints--

Unapaswa kuondoa sehemu tatu za `<Card />` kutoka kwenye msimbo wako.
