...
Unapaswa pia kufanya kazi kwenye kitendakazi ambacho kitafuta sehemu za ingizo.

Unda kitendakazi cha `clearInputFields`. Ndani yake, tumia `querySelectorAll` kupata sehemu zote za ingizo na zipitie kwa kutumia `forEach()`. Ndani ya `forEach()`, weka thamani ya kila `input` kuwa mfuatano wa herufi tupu.

# --hints--

Unapaswa kuunda kitendakazi cha `clearInputFields`.

```js
assert.isFunction(clearInputFields)
```

Kitendakazi chako cha `clearInputFields` kinapaswa kuweka kila sehemu ya ingizo kuwa mfuatano wa herufi tupu.
