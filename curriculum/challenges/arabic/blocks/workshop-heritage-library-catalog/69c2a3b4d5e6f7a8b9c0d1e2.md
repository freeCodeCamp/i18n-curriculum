...
أكدت شفرة اختبار `kingBooks` أن `findByAuthor` تعمل بشكل صحيح. قبل الانتقال إلى `groupByDecade`، نظف مخرجات وحدة التحكم بحذفها.

احذف السطر `const kingBooks = ...`، و`console.log(kingBooks.length)`، وحلقة `for` التي تعرض عناوين الكتب والسنوات.

# --hints--

يجب حذف متغير `kingBooks`، وعبارة `console.log(kingBooks.length)`، وحلقة `for` كلها.
