...
Логічні оператори можуть бути використані для створення складніших медіазапитів. Логічний оператор `and` використовується для запиту двох медіаумов.
...
Додайте інше правило `@media` для застосування, якщо розмір вікна браузера більший за `769px`, але менший за `1199px`.
...
Ваш запит `@media` повинен мати `min-width` зі значенням `769px` та `max-width` зі значенням `1199px`.
