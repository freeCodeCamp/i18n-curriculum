...
استخدم الرمز `|` للسماح بوجود `hundred` أو `thousand` أو `million` أو `billion` بين أرقامك و`dollars`.
...
يجب أن يستخدم `dollarRegex` الرمز `|` لمطابقة `hundred` أو `thousand` أو `million` أو `billion`.
