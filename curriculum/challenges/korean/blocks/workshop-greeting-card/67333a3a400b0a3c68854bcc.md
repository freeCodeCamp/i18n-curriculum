...
`transition` 선택자에 `.card-links a` 속성을 추가하고 값을 `background-color 0.3s ease`로 지정하세요.

# --hints--

`.card-links a` 선택자에는 `transition` 속성이 `background-color 0.3s ease`로 설정되어 있어야 합니다.
