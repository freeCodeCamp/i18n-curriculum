...
プレイヤーを少し操作すると、いくつかバグがあることに気づくでしょう。まず、曲を再生して一時停止し、その後に前のボタンか次のボタンを押すと、曲が最初から再生されません。

これを修正し始めるために、`start` 関数に `playSong` という名前の2番目のパラメータを追加してください。デフォルト値は `true` にしてください。

# --hints--

`playSong` 関数には、`start` という名前の2番目のパラメータがあり、デフォルト値は `true` である必要があります。
