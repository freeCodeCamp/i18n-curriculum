...
`multipleCard`를 지운 직후, `singleCardSelected` 메서드 안에서 `showElements` 함수를 호출하고 `singleCard`을 첫 번째 인수로, `fortuneContainer`를 두 번째 인수로 전달하세요.

# --hints--

`showElements` 함수를 호출해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

`elements.singleCard`을 첫 번째 인수로 전달해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

`elements.fortuneContainer`을 두 번째 인수로 전달해야 합니다.
