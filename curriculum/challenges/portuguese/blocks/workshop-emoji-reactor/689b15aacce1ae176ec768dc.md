...
Agora, você precisa obter o primeiro número no texto `0/10`, de uma forma que funcione mesmo para `5/10` ou `10/10`.

Analise o conteúdo de texto do elemento `.count` para extrair o número atual.

Converta a contagem de uma string para um número.

Substitua o anterior `console.log(countEl.textContent)` pelo seu novo log.

Registre a contagem atual neste formato: `console.log("Current count:", currCount)`.

Dica: lembre-se que você pode usar o método `split` para dividir uma string em um array de substrings.

# --hints--

Você deve registrar a contagem atual como um número.
...
Você deve registrar a contagem atual como um número de uma forma que funcione para os vários valores possíveis.
