...
Problema del recorrido del caballo: Tienes un tablero de ajedrez vacío de `width` \* `height`, pero con un solo caballo en alguna casilla. El caballo debe realizar una secuencia de movimientos legales que resulten en que el caballo visite cada casilla del tablero exactamente una vez. Ten en cuenta que *no* es un requisito que el recorrido sea "cerrado"; es decir, el caballo no necesita terminar a una sola jugada de su posición inicial.

# --instructions--

Escribe una función que tome `width` y `height` como parámetros y devuelva el número de posiciones iniciales desde donde es posible lograr la tarea mencionada arriba.

# --hints--

`knightTour` debería ser una función.

```js
assert(typeof knightTour == 'function');
```

`knightTour(6, 6)` debería devolver un número.

```js
assert(typeof knightTour(6, 6) == 'number');
```

`knightTour(6, 6)` debería devolver `36`.

```js
assert.equal(knightTour(6, 6), 36);
```

`knightTour(5, 6)` debería devolver `30`.

```js
assert.equal(knightTour(5, 6), 30);
```

`knightTour(4, 6)` debería devolver `12`.

```js
assert.equal(knightTour(4, 6), 12);
```

`knightTour(7, 3)` debería devolver `10`.

```js
assert.equal(knightTour(7, 3), 10);
```

`knightTour(8, 6)` debería devolver `48`.
