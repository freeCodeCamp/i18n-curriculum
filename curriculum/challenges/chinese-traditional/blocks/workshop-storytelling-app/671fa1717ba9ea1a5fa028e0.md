...
創建一個帶有 `story-container` 類別的 `main` 元素。在 `.story-container` 元素內，創建一個帶有 `btn-container` 類別的 `div`。

# --hints--

你應該有一個 `main` 元素，且其類別為 `story-container`。

```js
assert.exists(document.querySelector('main.story-container'));
```

在 `.story-container` 元素內，你應該有一個類別為 `btn-container` 的 `div`。
