...
在天空渐变颜色列表的顶部，你可以在其中放置一个渐变方向；添加 `circle closest-corner at 15% 15%,`。 这会将渐变的起点移动到距离顶部和左侧 `15%` 的位置。 渐变将在 `closest-corner` 处结束，形状为 `circle`。 这些是渐变中内置的一些关键字，用于描述其行为方式。

# --hints--

你应该将 `.sky` 的 `radial-gradient` 方向设为 `circle closest-corner at 15% 15%`。
