...
이제 화살표 함수에 대해 배운 내용을 음악 플레이어 프로젝트에 적용할 시간입니다.

먼저 `printGreeting`, `printMessage`, `addTwoNumbers` 함수를 제거하세요. 그리고 모든 콘솔 문장과 함수 호출도 제거하세요.

# --hints--

`printGreeting` 함수가 없어야 합니다.

```js
assert.notMatch(code, /const printGreeting = \(\) => {/);
```

`printMessage` 함수가 없어야 합니다.

```js
assert.notMatch(code, /const printMessage = org => {/);
```

`addTwoNumbers` 함수가 없어야 합니다.

```js
assert.notMatch(code, /const addTwoNumbers = \(num1, num2\) => num1 \+ num2/);
```

`printGreeting();` 함수 호출이 없어야 합니다.

```js
assert.notMatch(code, /printGreeting\(\);?/);
```

`printMessage('freeCodeCamp');` 함수 호출이 없어야 합니다.

```js
assert.notMatch(code, /printMessage\('freeCodeCamp'\);?/);
```

`console.log(addTwoNumbers(3,4));` 콘솔 문장이 없어야 합니다.
