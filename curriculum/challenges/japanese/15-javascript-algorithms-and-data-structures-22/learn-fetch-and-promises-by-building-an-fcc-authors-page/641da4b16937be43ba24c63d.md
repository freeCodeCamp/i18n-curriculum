...
このデータを扱うには、`.then()` メソッドをもう 1 つ使う必要があります。

メソッドチェーンで、既存の `.then()` メソッドに `.then()` をもう 1 つ繋げてください。 今回は、コールバック関数の仮引数として `data` を渡してください。 For the callback, use curly braces because you will have more than one expression. そしてコールバック関数内で `data` をコンソールに出力して、どのようになっているか確認しましょう。
...
`fetch()` メソッドを使って、`"https://cdn.freecodecamp.org/curriculum/news-author-page/authors.json"` に `GET` リクエストを送信してください。

```js
assert.match(code, /fetch\(\s*('|"|`)https:\/\/cdn\.freecodecamp\.org\/curriculum\/news\-author\-page\/authors\.json\1\s*\)/)
```

最初の `.then()` にはセミコロンを付けないでください。

```js
assert.notMatch(code, /\.then\(\s*(\(\s*res\s*\)|res)\s*=>\s*res\.json\(\s*\)\s*\)\s*;/)
```

既存の `.then()` にもう一つ `.then()` をチェーンしてください。
...
2 つ目の `.then()` メソッドではアロー関数構文を使用してください。
...
2 つ目の `.then()` メソッドで `data` をコンソールにログしてください。

```js
assert.match(code, /fetch\(\s*('|"|`)https:\/\/cdn\.freecodecamp\.org\/curriculum\/news\-author\-page\/authors\.json\1\s*\)\s*\.then\(\s*(\(\s*res\s*\)|res)\s*=>\s*res\.json\(\s*\)\s*\)\s*\.then\(\s*(\(\s*data\s*\)|data)\s*=>\s*\{\s*\n?\s*?console\.log\(\s*data\s*\)\s*;?\s*\}\s*\)/)
```

2 つ目の `.then()` の後にセミコロンを付けないでください。
