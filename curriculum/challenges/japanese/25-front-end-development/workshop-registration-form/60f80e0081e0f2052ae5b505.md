...
Specifying the `type` attribute of an `input` element is important for the browser to know what kind of data it should expect. `type` が指定されていない場合、ブラウザーはデフォルトで `text` を使用します。
