...
Rufen Sie jetzt die `showElements`-Funktion auf und übergeben Sie `multipleCard`, `fortuneContainer` und `text` als Props.

# --hints--

Sie sollten die `showElements`-Funktion innerhalb der `multipleCardSelected`-Methode aufrufen.
...
Sie sollten `multipleCard` als erstes Argument an die `showElements`-Funktion übergeben.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

Sie sollten `fortuneContainer` als Argument an die `showElements`-Funktion übergeben.
...
Sie sollten `text` als Argument an die `showElements`-Funktion übergeben.
