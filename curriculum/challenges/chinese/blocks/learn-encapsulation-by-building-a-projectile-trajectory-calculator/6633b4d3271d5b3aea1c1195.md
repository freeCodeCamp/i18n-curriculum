...
现在该看图表了。 创建一个方法 `create_trajectory`，并将代码底部的最后一个 `print` 调用替换为 `print(graph.create_trajectory())`。

作为该新方法的第一步，对坐标进行本地复制，但所有值都四舍五入为整数。 将这个新版本的坐标保存在名为 `rounded_coords` 的变量中，并返回该变量。

# --hints--

你应该定义一个名为 `create_trajectory` 的方法。
...
你应该将 `print(graph.create_coordinates_table())` 替换为 `print(graph.create_trajectory())`。
