...
Sie müssen den `id` des aktuell abgespielten Songs erhalten. Dafür können Sie `userData?.currentSong?.id` verwenden.

Nutzen Sie `getElementById()`, um den `id` des aktuell abgespielten Songs zu erhalten, und verwenden Sie dann Template-Literale, um ihn mit `song-` zu präfixen. Weisen Sie ihn der Konstanten `songToHighlight` zu.

# --hints--

Sie sollten die vorhandene `highlightCurrentSong`-Funktion und deren Inhalt nicht verändern.
...
Sie sollten `document.getElementById()` verwenden und `` `song-${userData?.currentSong?.id}` `` übergeben.

```js
assert.match(code, /document\.getElementById\(\s*`song-\$\{userData\?\.currentSong\?\.id\}`\s*\)\s*;?\s*\}\s*;?/)
```

Sie sollten Ihre `getElementById()` einer `songToHighlight`-Konstanten zuweisen.
