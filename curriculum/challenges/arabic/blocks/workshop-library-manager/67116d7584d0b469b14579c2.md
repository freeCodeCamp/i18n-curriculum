...
للحصول على قائمة ملخصات الكتب، يمكنك استخدام الخاصية `about` لكل كتاب.

أنشئ دالة `getBookSummaries` بمعلمة واحدة، تستقبل مصفوفة تحتوي على كائنات كتب.

يجب أن تُرجع دالة `getBookSummaries` مصفوفة من سلاسل نصية تمثل الملخص لكل كتاب.

# --hints--

يجب أن تنشئ دالة `getBookSummaries`.

```js
assert.isFunction(getBookSummaries);
```

يجب أن تحتوي دالة `getBookSummaries` على معلمة واحدة.

```js
assert.lengthOf(getBookSummaries, 1);
```

يجب أن تُرجع دالة `getBookSummaries` مصفوفة.

```js
assert.isArray(getBookSummaries(library));
```

يجب أن تُرجع دالة `getBookSummaries` مصفوفة بنفس طول المصفوفة التي تم تمريرها إليها.
...
يجب أن تُرجع دالة `getBookSummaries` مصفوفة تحتوي على الخاصية `about` لكل كتاب من المصفوفة التي تم تمريرها إليها.
