...
メインのシミュレーションループを実装してください。このループは、すべての目盛りが完了するまで、シミュレーション内の各目盛りごとにすべてのゲートが処理されることを保証します。

`while` が `tickIndex` より小さい間、続く空の `maxTicks` ループを作成してください。

# --hints--

条件 `while` を持つ空の `(tickIndex < maxTicks)` ループを作成してください。
