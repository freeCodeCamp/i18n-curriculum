...
Se você não está familiarizado com `let`, confira <a href="/learn/javascript-algorithms-and-data-structures/basic-javascript/explore-differences-between-the-var-and-let-keywords" target="_blank" rel="noopener noreferrer nofollow">este desafio sobre a diferença entre <code>let</code> e <code>var</code></a>.
...
`i` não foi definida porque não foi declarada no escopo global.Ele é declarado apenas dentro da instrução do laço `for`. `printNumTwo()` retornou o valor correto porque três variáveis `i` diferentes com valores únicos (0, 1 e 2) foram criadas pela palavra-chave `let` dentro da instrução do laço.
