...
Agora você precisa trabalhar na pausa da música que está tocando atualmente.

Defina uma função `pauseSong` que não recebe parâmetros. Dentro da sua nova função, para armazenar o tempo atual da música quando ela for pausada, defina o `songCurrentTime` do objeto `userData` para o `currentTime` da variável `audio`.


# --hints--

Você deve ter uma função `pauseSong`.

```js
assert.isFunction(pauseSong);
```

Sua função `pauseSong` deve definir o `songCurrentTime` do objeto `userData` para o `currentTime` da variável `audio`.
