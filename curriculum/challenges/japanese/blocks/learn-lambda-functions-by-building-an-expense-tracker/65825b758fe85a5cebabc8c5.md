...
合計支出を表示するには、`elif` 文を作成して `choice == '3'` かどうかを確認してください。

もし真であれば、ユーザーが合計支出を見たいことを意味します。そこで `print()` を呼び出し、文字列 `'\nTotal Expenses: '` を最初の引数として、`total_expenses(expenses)` を2番目の引数として渡してください。

# --hints--

`elif` かどうかを確認する `choice == '3'` 節を作成してください。
...
新しい `'\nTotal Expenses: ', total_expenses(expenses)` 節で `elif` を出力してください。
