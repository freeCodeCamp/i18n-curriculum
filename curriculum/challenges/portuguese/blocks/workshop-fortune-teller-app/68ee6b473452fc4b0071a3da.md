...
Agora faça o mesmo para `multipleCardsBtn` e `newReadingBtn` anexando um listener de evento de clique ao `multipleCardsBtn` que faça a chamada `multipleCardSelected`, e outro ao `newReadingBtn` que faça a chamada `newReading`.

Você criará os dois métodos nos próximos passos.

# --hints--

Você deve anexar um evento `click` ao elemento `multipleCardsBtn`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Dentro do callback do evento `multipleCardsBtn`, você deve fazer a chamada do método `multipleCardSelected`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

Você deve anexar um evento `click` ao elemento `newReadingBtn`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Dentro do callback do evento `newReadingBtn`, você deve fazer a chamada do método `newReading`.
