...
Erstellen Sie eine leere `while`-Schleife, die so lange fortgesetzt wird, wie noch Teilnehmer warten (`currentTickQueue > 0`) und das Tor noch Kapazität hat (`processed < gate.capacity`).

# --hints--

Die Bedingung Ihrer `while`-Schleife sollte `currentTickQueue > 0` und `processed < gate.capacity` sein.
