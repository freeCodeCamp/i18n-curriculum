...
Bevor Sie das Lied abspielen, müssen Sie sicherstellen, dass es von Anfang an startet. Dies erreichen Sie durch die Verwendung der `currentTime`-Eigenschaft am `audio`-Objekt.

Fügen Sie eine `if`-Anweisung hinzu, um zu überprüfen, ob gerade kein Lied abgespielt wird, indem Sie prüfen, ob `userData.currentSong` falsch ist.
Setzen Sie innerhalb des `if`-Blocks die `currentTime`-Eigenschaft des `audio`-Objekts auf `0`.

# --hints--

Ihre `playSong`-Funktion sollte die `currentTime`-Eigenschaft des `audio`-Objekts auf `0` setzen, wenn gerade kein Lied abgespielt wird.
