...
# --description--
Para a maioria dos verbos, se você quiser formar uma frase negativa no passado, basta colocar `didn't` antes do verbo principal.  
`Didn't` é a forma abreviada de `did not`.

Lembre-se de que o verbo principal permanece em sua forma simples.Apenas o verbo auxiliar `did` é conjugado.Por exemplo: 

**Frase afirmativa no passado**: `I worked yesterday` - O verbo `work` está conjugado no passado.
