...
Після отримання даних збережіть відповідь у змінну та проаналізуйте її у формат JSON. Призначте тип `Deck` проаналізованим даним, а потім присвойте їх властивості `cards`. Усередині блоку `catch` обов’язково залогуй помилку з таким повідомленням: `"Error fetching cards:", error`.

# --hints--

Ви повинні оголосити змінну з ім’ям `data` всередині `fetchCardsData`.
...
Ваша змінна `data` має мати тип `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

Ваша змінна `data` має бути призначена шляхом аналізу відповіді як JSON.
...
Ви повинні зберегти `data.cards` у `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Ваш блок `catch` має використовувати `console.error()` для логування помилки з повідомленням `"Error fetching cards:", error`.
