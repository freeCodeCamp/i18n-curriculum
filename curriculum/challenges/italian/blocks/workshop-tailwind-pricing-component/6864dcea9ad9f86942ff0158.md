...
È il momento di lavorare sull'elemento `div` che racchiude le tre schede dei prezzi. Per disporre le schede, puoi usare le classi di utilità griglia di Tailwind CSS.

Ecco come assegnare a un elemento una proprietà display di `grid` usando Tailwind:
...
Assegna la classe `grid` all'elemento `div` che racchiude le schede dei prezzi.
