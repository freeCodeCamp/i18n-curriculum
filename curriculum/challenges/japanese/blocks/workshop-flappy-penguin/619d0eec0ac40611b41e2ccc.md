...
`.eye.right` 要素の下に、`div` の `class` を持つ `blush` 要素を2つ追加してください。また、最初の `.blush` 要素には `class` の `left` を、2番目の `.blush` 要素には `class` の `right` を付けてください。
...
最初の新しい `div` に `class` の `blush` を付けてください。

```js
assert.exists(document.querySelector('.penguin-head > div.blush'));
```

2番目の新しい `div` に `class` の `blush` を付けてください。
...
`.blush.right` の後に `.blush.left` を配置してください。
