...
像之前的 `multipleCard` 一样，再次获取 `multipleCard` 元素，并通过遍历 `innerHTML` 来设置它的 `DrawingType`。使用 `Object.values()` 并调用它的 `map` 方法，传入 `type` 作为参数。

# --hints--

你应该通过调用 `multipleCard.innerHTML` 来为 `Object.values` 赋值。

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

你应该将 `DrawingType` 传递给 `Object.values` 方法。

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

你应该在 `map` 上调用 `Object.values` 方法。

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

你的 `map` 方法应该有一个名为 `type` 的参数。
