...
Вам потрібно знати, скільки поділок має тривати симуляція. Нагадаємо, що:

- Кожні ворота мають масив `queue`, який показує, скільки відвідувачів приходить на кожній поділці.  
- Оскільки всі ворота мають однакову кількість поділок, ви можете просто використати довжину черги перших воріт.  

Створіть змінну з ім’ям `maxTicks` і призначте їй значення `gates[0].queue.length`.

Це значення буде використано для керування вашим циклом симуляції, щоб переконатися, що кожна поділка оброблена для всіх воріт. 

# --hints--

Ви маєте створити змінну з ім’ям `maxTicks` і призначити їй значення `gates[0].queue.length`.
