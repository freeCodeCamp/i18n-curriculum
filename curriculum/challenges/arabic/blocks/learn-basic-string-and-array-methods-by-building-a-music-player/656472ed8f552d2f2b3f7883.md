...
لا تعدل جملة `if` الحالية ومحتواها.
...
أضف كتلة `else` إلى كتلة `if` الحالية.
...
عيّن `audio.currentTime` إلى `userData?.songCurrentTime` داخل كتلة `else` في جملة `if` الخاصة بك.
