...
ミッションコントロールでは、クルーからEVA対象の宇宙飛行士をコピーするメソッドが必要です。`getEVAReadyCrew` という名前の空の関数を作成し、`crew` パラメータを受け取るようにしてください。

# --hints--

`getEVAReadyCrew` という名前の関数を作成してください。

```js
assert.isFunction(getEVAReadyCrew);
```

`getEVAReadyCrew` 関数は `crew` パラメータを持つ必要があります。
