...
`address` 요소 내 모든 텍스트를 수평으로 가운데 정렬하고, 패딩을 추가하세요.

# --hints--

`address` 요소 선택자를 사용하세요.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

`address`에 `text-align`의 `center`을 지정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

`address`에 최소 `padding-top`의 `1px`을 지정하세요.
...
`address`에 최소 `padding-right`의 `1px`을 지정하세요.
...
`address`에 최소 `padding-bottom`의 `1px`을 지정하세요.
...
`address`에 최소 `padding-left`의 `1px`을 지정하세요.
