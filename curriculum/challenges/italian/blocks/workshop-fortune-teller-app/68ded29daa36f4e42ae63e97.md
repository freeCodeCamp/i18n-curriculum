...
Completa la funzione usando il metodo array `forEach` per iterare su elements. All'interno della funzione di callback, aggiungi la classe hidden a ogni elemento usando il metodo `.add()` di `classList`.

# --hints--

Dovresti avere un metodo `forEach` allegato al parametro `elements`.
...
All'interno del metodo forEach, dovresti aggiungere la classe hidden usando il metodo `classList.add`.
