...
يصبح المخرَج صعب القراءة، لذا يجب استبدال `print(graph.create_trajectory())` بحلقة تطبع كل صف على حدة.

# --hints--

احذف استدعاء `print(graph.create_trajectory())`.
...
يجب أن تحتوي على حلقة تقوم بتكرار `graph.create_trajectory()`.
...
يجب طباعة كل صف في `graph.create_trajectory()`.
