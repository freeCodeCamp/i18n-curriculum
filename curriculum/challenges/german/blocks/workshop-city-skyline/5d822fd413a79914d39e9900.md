...
Ändern Sie als Nächstes die beiden `#999` von `.bb2a` zu `transparent`. Dadurch werden die linken und rechten Rahmen unsichtbar.

# --hints--

Sie sollten das `border-left` so ändern, dass `transparent` verwendet wird.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid transparent");
```

Sie sollten das `border-right` so ändern, dass `transparent` verwendet wird.
