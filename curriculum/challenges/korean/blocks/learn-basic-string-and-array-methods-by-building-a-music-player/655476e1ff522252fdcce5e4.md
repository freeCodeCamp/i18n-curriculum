...
`else` 문장에 `if` 블록을 추가하세요. `else` 블록 안에서 `getCurrentSongIndex()` 함수를 호출하고 `currentSongIndex`라는 이름의 상수에 할당하세요.
...
`currentSongIndex` 블록 안에서 `getCurrentSongIndex()` 상수를 `else`로 설정하세요.
