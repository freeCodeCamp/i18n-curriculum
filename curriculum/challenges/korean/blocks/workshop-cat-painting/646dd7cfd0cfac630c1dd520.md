...
귀의 모든 뾰족한 모서리를 없애려면, 오른쪽 아래와 왼쪽 아래 테두리 반경을 각각 `40%`로 설정하고, 왼쪽 위 테두리 반경은 `90px`로, 오른쪽 위 테두리 반경은 `10px`로 설정하세요.

# --hints--

`.cat-left-inner-ear` 선택자에 `border-bottom-right-radius` 속성을 `40%`로 설정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderBottomRightRadius, '40%')
```

`.cat-left-inner-ear` 선택자에 `border-bottom-left-radius` 속성을 `40%`로 설정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderBottomLeftRadius, '40%')
```

`.cat-left-inner-ear` 선택자에 `border-top-left-radius` 속성을 `90px`로 설정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderTopLeftRadius, '90px')
```

`.cat-left-inner-ear` 선택자에 `border-top-right-radius` 속성을 `10px`로 설정해야 합니다.
