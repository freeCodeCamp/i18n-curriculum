...
Los últimos valores codificados son las seis estadísticas del jugador. Sustituye los primeros tres conjuntos de números codificados por la propiedad `player` correspondiente:
...
Debes reemplazar el valor `97` por `PAC` con `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

Debes reemplazar el valor `98` por `SHO` con `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

Debes reemplazar el valor `83` por `PAS` con `player.pas`.
