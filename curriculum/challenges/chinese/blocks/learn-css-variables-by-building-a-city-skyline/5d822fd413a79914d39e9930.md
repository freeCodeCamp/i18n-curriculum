...
这座建筑的顶部将有另一个三角形。 给顶部一个值为 `5vh solid transparent` 的 `border-top`，以及一个值为 `8vw`、`solid` 的 `border-left`，并使用其建筑颜色变量作为颜色。

# --hints--

你应该给 `.fb4a` 一个值为 `5vh solid transparent` 的 `border-top`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4a")?.borderTop.trim(), "5vh solid transparent")
```

你应该给 `.fb4a` 一个值为 `8vw solid var(--building-color1)` 的 `border-left`。
