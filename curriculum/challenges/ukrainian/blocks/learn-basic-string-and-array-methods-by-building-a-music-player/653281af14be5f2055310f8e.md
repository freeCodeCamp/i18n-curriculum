...
Ваш музичний плеєр має відстежувати пісні, поточну пісню, що відтворюється, та час поточної пісні. Для цього потрібно створити об’єкт, щоб зберігати цю інформацію.

Почніть із використання ключового слова `let` для оголошення нової змінної з назвою `userData` і призначте їй порожній об’єкт.

# --hints--

Ви повинні використати ключове слово `let` для створення змінної `userData`.

```js
assert.match(code, /let\s+userData\s*=/)
```

Ви повинні призначити порожній об’єкт змінній `userData`.
