...
1. `Status` 列中的 `td` 元素应当包含一个 `span` 元素，该元素的 `class` 属性为 `status`，并根据该行的类名包裹文本 `Read`、`To Read` 或 `In Progress`。
...
1. 你应该创建三个属性选择器，选择具有 `read`、`to-read` 和 `in-progress`类的元素，将它们的 `background-image` 属性设置为使用自定义的 `linear-gradient` 的值。
1. 你应该将每个 `span` 元素的 `display` 属性设置为 `inline-block` 。
1. 你应该使用一个属性选择器来选中具有 `status` 类的`span` 元素，这些元素是具有 `to-read` 类的 `tr` 元素的后代，并设置它们的 `border` 和 `background-image` 属性。
1. 你应该使用一个属性选择器来选中具有 `status` 类的`span` 元素，这些元素是具有 `read` 类的 `tr` 元素的后代，并设置它们的 `border` 和 `background-image` 属性。
1. 你应该使用一个属性选择器来选中具有 `status` 类的`span` 元素，这些元素是具有 `in-progress` 类的 `tr` 元素的后代，并设置它们的 `border` 和 `background-image` 属性。
1. 你应该使用一个属性选择器来选择具有 `status` 类的 `span` 元素，以及类值以 `rate` 开头的 `span` 元素，设置它们的 `height`、`width` 和 `padding` 属性。
1. 你应该使用一个属性选择器来选择那些作为 `span` 元素直接子元素的、且其 `class` 属性值以 `rate` 开头的 `span` 元素，设置它们的 `border`、`border-radius`、`margin`、`height`、`width` 和 `background-color` 等属性。
1. 你应该使用一个属性选择器来选中 `class` 值中包含单词 `one` 的 `span` 元素的第一个子元素，并将其 `background-image` 属性设置为使用 `linear-gradient`。
1. 你应该使用一个属性选择器来选中 `class` 值中包含单词 `two` 的 `span` 元素的前两个子元素，并将它们的 `background-image` 属性设置为使用 `linear-gradient`。
1. 你应该使用一个属性选择器来选中 `class` 值中包含单词 `three` 的 `span` 元素内的三个 `span` 元素，并将它们的 `background-image` 属性设置为使用 `linear-gradient`。
...
你应该使用一个属性选择器来选择具有 `read` 类的行，将它们的 `background-image`属性设置为自定义的线性渐变效果。
...
你应该使用一个属性选择器来选择具有 `to-read` 类的行，将它们的 `background-image`属性设置为自定义的线性渐变效果。
...
你应该使用一个属性选择器来选择具有 `in-progress` 类的行，将它们的 `background-image`属性设置为自定义的线性渐变效果。
...
你应该使用一个属性选择器来选择那些位于 `to-read` 类的 `tr` 元素内的、具有 `status` 类的 `span` 元素，设置它们的 `border` 属性。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('tr[class="to-read"] span[class="status"]')?.border);
```

你应该使用一个属性选择器来选择那些位于 `to-read` 类的 `tr` 元素内的、具有 `status` 类的 `span` 元素，设置它们的 `background-image` 属性。
...
你应该使用一个属性选择器来选择那些位于 `read` 类的 `tr` 元素内的、具有 `status` 类的 `span` 元素，设置它们 `border` 属性。
...
你应该使用一个属性选择器来选择那些位于 `in-progress` 类的 `tr` 元素内的、具有 `status` 类的 `span` 元素，设置它们的 `border` 属性。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('tr[class="in-progress"] span[class="status"]')?.border);
```

你应该使用一个属性选择器来选择那些位于 `in-progress` 类的 `tr` 元素内的、具有 `status` 类的 `span` 元素，设置它们的 `background-image` 属性。
...
你应该使用一个属性选择器来选择具有 `status` 类的 `span` 元素，以及类值以 `rate` 开头的 `span` 元素，设置它们的 `height` 属性。
...
你应该使用一个属性选择器来选择具有 `status` 类的 `span` 元素，以及类值以 `rate` 开头的 `span` 元素，设置它们的 `width` 属性。
...
你应该使用一个属性选择器来选择具有 `status` 类的 `span` 元素，以及类值以 `rate` 开头的 `span` 元素，设置它们的 `padding` 属性。
...
你应该使用一个属性选择器，选中 `class` 值包含单词 `one` 的 `span` 元素的第一个子元素，并将该元素的 `background-image` 属性设置为 `linear-gradient`。
...
你应该有一个属性选择器，选中 `class` 值包含单词 `two` 的 `span` 元素的前两个子元素。
...
你应该使用一个属性选择器，选中 `class` 值包含单词 `two` 的 `span` 元素的前两个子元素，并将它们的 `background-image` 属性设置为 `linear-gradient`。
...
你应该有一个属性选择器，选中 `class` 值包含单词 `three` 的 `span` 元素内的 `span` 元素。
...
你应该使用一个属性选择器，选中 `class` 值包含单词 `three` 的 `span` 元素内的 `span` 元素，并将它们的 `background-image` 属性设置为 `linear-gradient`。
