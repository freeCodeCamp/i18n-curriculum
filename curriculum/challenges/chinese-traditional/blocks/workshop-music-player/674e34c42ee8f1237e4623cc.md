...
為你的程式碼添加一個 `else` 區段。在 `else` 區段內，使用目前正在播放的歌曲的 `id` 作為引數，呼叫 `playSong` 函式。 

這可確保當按下播放按鈕時，正在播放的歌曲會繼續播放。

# --hints--

當點擊 `playButton` 且 `userData.currentSong` 不為 `null` 時，你應該使用目前播放歌曲的 `id` 呼叫 `playSong` 函式。
...
當 `playButton` 被點擊且 `userData.currentSong` 為 `null` 時，你仍然應該使用播放列表中第一首歌曲的 `id` 來呼叫 `playSong` 函式（程式）。 
