...
ضع إعلان المتغير `password` وحلقة `for` التالية داخل حلقة `while`. استخدم `True` كشرط للحلقة الجديدة.

# --hints--

يجب أن تنشئ حلقة `while True` تحيط بإعلان `password` الحالي وحلقة `for`.
