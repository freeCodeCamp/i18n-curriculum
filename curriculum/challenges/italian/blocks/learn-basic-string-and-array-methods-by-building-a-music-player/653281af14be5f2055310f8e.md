...
Il tuo lettore musicale dovrebbe tenere traccia delle canzoni, della canzone attualmente in riproduzione e del tempo della canzone corrente. Per farlo, dovrai creare un oggetto per archiviare queste informazioni.

Inizia usando la parola chiave `let` per dichiarare una nuova variabile chiamata `userData` e assegnarle un oggetto vuoto.

# --hints--

Dovresti usare la parola chiave `let` per creare la variabile `userData`.

```js
assert.match(code, /let\s+userData\s*=/)
```

Dovresti assegnare un oggetto vuoto alla tua variabile `userData`.
