...
DRI `form-group`の後に、`stats-grid`の中に`className`が`form-group`の`div`要素を2つ追加してください。1つはDEF用、もう1つはPHY用です。

DEF用には、`htmlFor`が`def`でテキストが`DEF`の`label`を追加し、その後に`id`が`def`で`type`が`number`の入力を追加してください。

PHY用には、`htmlFor`が`phy`でテキストが`PHY`の`label`を追加し、その後に`id`が`phy`で`type`が`number`の入力を追加してください。

# --hints--

`stats-grid`の`className`の`div`の中に、`className`が`form-group`の`div`要素を2セット追加してください。
...
5番目の`className`が`form-group`の`div`要素には、`htmlFor`属性が`def`、`className`が`label`、テキストが`DEF`の`label`が必要です。
...
5番目の`className`が`form-group`の`div`要素には、`className`が`input`、`id`が`def`、`type`が`number`の`input`が必要です。
...
6番目の`className`が`form-group`の`div`要素には、`htmlFor`属性が`phy`、`className`が`label`、テキストが`PHY`の`label`が必要です。
...
6番目の`className`が`form-group`の`div`要素には、`className`が`input`、`id`が`phy`、`type`が`number`の`input`が必要です。
