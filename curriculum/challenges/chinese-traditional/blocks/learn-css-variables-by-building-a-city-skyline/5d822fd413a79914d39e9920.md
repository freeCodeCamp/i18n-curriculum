...
對於 `.fb2a`，添加一個值爲 `10vh solid var(--building-color3)` 的 `border-bottom`，以及值爲 `1vw solid transparent` 的 `border-left` 和 `border-right`。 這一次邊框技巧將創建一個梯形形狀。

# --hints--

你應該給 `.fb2a` 一個值爲 `10vh solid var(--building-color3)` 的 `border-bottom`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderBottom.trim(), "10vh solid var(--building-color3)");
```

你應該給 `.fb2a` 一個值爲 `1vw solid transparent` 的 `border-left`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2a")?.borderLeft, "1vw solid transparent");
```

你應該給 `.fb2a` 一個值爲 `1vw solid transparent` 的 `border-right`。
