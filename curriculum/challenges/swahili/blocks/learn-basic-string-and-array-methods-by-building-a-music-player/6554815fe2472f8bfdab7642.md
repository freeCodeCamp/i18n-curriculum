...
Tumia `const` na sintaksia ya mshale kuunda kitendakazi cha `playPreviousSong` kisicho na maudhui.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi kisicho na maudhui chenye jina `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` inapaswa kuwa kitendakazi.

```js
assert.isFunction(playPreviousSong)
```

Kitendakazi chako cha `playPreviousSong` kinapaswa kutumia sintaksia ya mshale.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Kitendakazi chako cha `playPreviousSong` kinapaswa kuwa kisicho na maudhui.
