...
スプレッドシートは機能していますが、対応できる数式がまだ少ないです。

`even`に`spreadsheetFunctions`プロパティを追加してください。`nums`パラメータを受け取り、`nums`配列をフィルターして偶数のみを含む結果を返すようにします。`isEven`関数の参照を使ってください。

# --hints--

`spreadsheetFunctions`オブジェクトには`even`プロパティが必要です。

```js
assert.property(spreadsheetFunctions, "even");
```

`even`プロパティは関数であるべきです。

```js
assert.isFunction(spreadsheetFunctions.even);
```

`even`関数は`nums`パラメータを受け取るべきです。

```js
assert.match(code, /even\s*:\s*(\(\s*nums\s*\)|nums)\s*=>/)
```

`even`関数は暗黙の戻り値を使うべきです。

```js
assert.notMatch(code, /even\s*:\s*(\(\s*nums\s*\)|nums)\s*=>\s*\{/)
```

`even`関数は`.filter()`の`nums`メソッドを呼び出した結果を返すべきです。

```js
assert.match(code, /even\s*:\s*(\(\s*nums\s*\)|nums)\s*=>\s*nums\s*\.\s*filter/)
```

`isEven()`関数の参照を`.filter()`メソッドのコールバックとして渡すべきです。

```js
assert.match(code, /even\s*:\s*(\(\s*nums\s*\)|nums)\s*=>\s*nums\s*\.\s*filter\s*\(\s*isEven\s*\)/)
```

`even`関数は偶数の配列を返すべきです。
