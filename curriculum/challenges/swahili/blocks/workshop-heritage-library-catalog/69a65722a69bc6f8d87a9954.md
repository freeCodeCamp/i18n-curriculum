...
Tumalize kwa muhtasari. Andika `catalog.length` na `Object.keys(byDecade).length` kuona jumla ya vitabu na makundi ya miongo.

Tangaza `let oldestYear = Infinity` na `let newestYear = 0`. Tumia mzunguko wa `for` juu ya `catalog`: kwa kila kipengee ambacho mwaka si `"Unknown"`, sasisha `oldestYear` ikiwa mwaka ni mdogo zaidi na `newestYear` ikiwa ni mkubwa zaidi. Andika thamani zote mbili.

Umehitimisha warsha ya Katalogi ya Maktaba ya Urithi!

# --hints--

Unapaswa kuandika `catalog.length` kwenye konsoli.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*catalog\.length\s*\)/);
```

Unapaswa kuandika `Object.keys(byDecade).length` kwenye konsoli.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*Object\.keys\s*\(\s*byDecade\s*\)\.length\s*\)/);
```

Unapaswa kutumia mzunguko wa `for` juu ya `catalog` kupata miaka ya zamani zaidi na mpya zaidi.

```js
assert.match(__helpers.removeJSComments(code), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Mwaka wa zamani zaidi katika katalogi unapaswa kuwa `850`.

```js
assert.strictEqual(oldestYear, 850);
```

Mwaka wa mpya zaidi katika katalogi unapaswa kuwa `2011`.

```js
assert.strictEqual(newestYear, 2011);
```

Unapaswa kuandika `oldestYear` kwenye konsoli.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*oldestYear\s*\)/);
```

Unapaswa kuandika `newestYear` kwenye konsoli.
