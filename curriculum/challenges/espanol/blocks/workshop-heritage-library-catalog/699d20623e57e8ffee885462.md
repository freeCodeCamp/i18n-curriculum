...
Es hora de ver cómo el validador detecta un problema real. `catalog[0]` es "From a Buick 8" de Stephen King, que tiene los cuatro campos completos. `catalog[16]` es "Unknown Title", que tiene el campo del autor vacío.

Registra en la consola `validateEntry(catalog[0])` y `validateEntry(catalog[16])`. El primero debería devolver `true`; el segundo debería devolver `false`.

# --hints--

Debes registrar `validateEntry(catalog[0])` en la consola.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

Debes registrar `validateEntry(catalog[16])` en la consola.
