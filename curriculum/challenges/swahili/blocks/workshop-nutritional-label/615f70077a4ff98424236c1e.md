...
Unda kichaguzi kipya cha `.indent` na umpe sifa ya `margin-left` iliyowekwa kuwa `1em`.
...
Kichaguzi chako cha `.indent` kinapaswa kuwa na sifa ya `margin-left` iliyowekwa kuwa `1em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.indent')?.marginLeft, '1em');
```

Kichaguzi chako cha `.indent` kinapaswa kuweka sifa ya `margin-left` tu, si sifa nyingine za ukingo.
