...
JSON（JavaScript Object Notation）は、システム間でデータを共有するための標準フォーマットです。`JSON.stringify` はJavaScriptの値をJSON形式の文字列に変換します。

まず、テスト用だった`validateEntry`の2つのconsole.log呼び出しを削除してください。次に、`exportToJSON`をパラメータとして受け取る`catalog`という名前の関数を作成してください。`JSON.stringify(catalog, null, 2)`を返してください。最初の引数は変換するデータです。2番目の引数（`null`）はカスタムフィルターなしを意味します。3番目の引数（`2`）は2スペースのインデントを追加し、出力を読みやすくします。

関数の下で、最初の2つのエントリーのJSON出力をプレビューするために`exportToJSON(catalog.slice(0, 2))`をログに出力してください。

# --hints--

`validateEntry`のconsole.log呼び出しは削除してください。

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry/);
```

`exportToJSON`は文字列を返す必要があります。

```js
assert.isString(exportToJSON(catalog));
```

`JSON.parse(exportToJSON(catalog))`は有効な配列を生成する必要があります。

```js
assert.isArray(JSON.parse(exportToJSON(catalog)));
```

`JSON.stringify`を2番目と3番目の引数に`null`と`2`を使って呼び出してください。

```js
assert.match(exportToJSON.toString(), /JSON\.stringify\s*\(\s*catalog\s*,\s*null\s*,\s*2\s*\)/);
```

`exportToJSON(catalog.slice(0, 2))`をコンソールにログ出力してください。
