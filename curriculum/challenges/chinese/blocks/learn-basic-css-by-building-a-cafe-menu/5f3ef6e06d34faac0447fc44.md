...
创建名为 `established` 的类选择器，并为其赋予值为 `italic` 的 `font-style` 属性，为 `Est. 2020` 实现斜体效果。

# --hints--

你应该有一个 `.established` 选择器。
...
你的 `.established` 选择器应该将 `font-style` 属性设置为 `italic`。
