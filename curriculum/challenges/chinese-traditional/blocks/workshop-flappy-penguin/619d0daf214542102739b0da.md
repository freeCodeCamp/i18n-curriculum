...
選擇 `.eye-lid` 元素，並將其 `width` 設置爲 `150%`、`height` 設置爲 `100%`，`background-color` 設置爲 `--penguin-face`。
...
你應該將 `.eye-lid` 的 `width` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.width, '150%');
```

你應該將 `.eye-lid` 的 `height` 設置爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.eye-lid')?.height, '100%');
```

你應該將 `.eye-lid` 的 `background-color` 設置爲 `var(--penguin-face)`。
