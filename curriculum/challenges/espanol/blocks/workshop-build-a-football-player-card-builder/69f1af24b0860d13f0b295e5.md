...
Ahora reemplaza los números codificados restantes con lo siguiente:
...
Debes reemplazar el valor codificado `99` por `DRI` con `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

Debes reemplazar el valor codificado `41` por `DEF` con `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

Debes reemplazar el valor codificado `99` por `PHY` con `player.phy`.
