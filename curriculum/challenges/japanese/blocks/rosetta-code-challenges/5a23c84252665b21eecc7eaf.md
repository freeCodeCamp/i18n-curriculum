...
国際銀行口座番号（IBAN）は、国境を越えた銀行口座を識別するために国際的に合意された方法であり、転記ミスのリスクを減らします。

<abbr title="International Bank Account Number">IBAN</abbr>は最大34文字の英数字で構成されています。

<ul>
  <li>最初に2文字の<abbr title="International Organization for Standardization">ISO</abbr> 3166-1 alpha-2国コード</li>
  <li>次に2桁のチェックディジット</li>
  <li>最後に国ごとに異なる基本銀行口座番号（BBAN）</li>
</ul>

チェックディジットは、取引を提出する前に口座番号の整合性を確認するための簡易検査を可能にします。

# --instructions--

IBANの文字列をパラメータとして受け取る関数を書いてください。有効な場合はtrueを返し、そうでなければfalseを返してください。

# --hints--

`isValid` は関数である必要があります。
