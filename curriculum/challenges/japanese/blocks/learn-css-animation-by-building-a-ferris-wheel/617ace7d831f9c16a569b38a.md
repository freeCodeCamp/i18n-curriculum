...
`.wheel` セレクターに `max-height` と `max-width` の両方のプロパティを `500px` に設定してください。

# --hints--

`.wheel` セレクターには `max-height` プロパティを `500px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.wheel')?.maxHeight === '500px');
```

`.wheel` セレクターには `max-width` プロパティを `500px` に設定してください。
