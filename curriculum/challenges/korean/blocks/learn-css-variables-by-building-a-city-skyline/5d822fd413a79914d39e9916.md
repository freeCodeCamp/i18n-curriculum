...
아래 공간은 걱정하지 마세요. 건물 상단 요소에 높이를 추가하면 나중에 모두 아래로 이동됩니다.

`repeating-linear-gradient`에서 `.fb1c`까지 `90deg` 각도로 `--building-color4`를 `0%`에서 `10%`까지, 그리고 `transparent`를 `10%`에서 `15%`까지 추가하세요.

# --hints--

`.fb1c`에 `background`가 있는 `repeating-linear-gradient`를 지정하세요.
...
첫 번째 색상은 `--building-color4`에서 `0%`까지 `10%`를 사용하세요.
...
두 번째 색상은 `transparent`에서 `10%`까지 `15%`를 사용하세요.
