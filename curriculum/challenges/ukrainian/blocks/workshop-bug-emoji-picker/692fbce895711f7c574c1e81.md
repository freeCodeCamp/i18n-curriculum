...
Тепер всередині функції `isSelect` поверніть булеве значення, яке вказує, чи є `element` екземпляром `HTMLSelectElement`. Використайте оператор `instanceof` для виконання цієї перевірки.

# --hints--

Ваша функція `isSelect` має повертати `element instanceof HTMLSelectElement`.
