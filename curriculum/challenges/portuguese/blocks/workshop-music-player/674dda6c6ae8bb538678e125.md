...
Adicione uma declaração `if` para verificar se nenhuma música atual está tocando verificando se `userData.currentSong` é falso. Dentro do bloco `if`, defina a propriedade `currentTime` do objeto `audio` para `0`.

# --hints--

Sua função `playSong` deve definir a propriedade `currentTime` do objeto `audio` para `0` quando nenhuma música estiver tocando.
