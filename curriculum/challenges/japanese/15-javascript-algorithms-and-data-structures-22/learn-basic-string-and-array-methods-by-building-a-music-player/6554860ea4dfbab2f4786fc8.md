...
細かい点ですが、プレイリストで現在再生中の曲がハイライトされていないため、どの曲が再生されているのかわかりません。 これを修正するため、再生中の曲をハイライトする関数を作成しましょう。

矢印構文を使用して、`highlightCurrentSong` 関数を作成してください。 関数内で、`querySelectorAll` を使用して `.playlist-song` の要素を取得し、定数 `playlistSongElements` に代入してください。

# --hints--

`const` を使用して、`highlightCurrentSong` 関数を作成する必要があります。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*/)
```

`highlightCurrentSong` は関数でなければなりません。

```js
assert.isFunction(highlightCurrentSong)
```

あなたの `highlightCurrentSong` 関数は矢印構文を使用する必要があります。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*/)
```

`highlightCurrentSong` 関数内に定数 `playlistSongElements` を作成する必要があります。

```js
assert.match(code, /const\s+highlightCurrentSong\s*=\s*\(\s*\)\s*=>\s*{\s*const\s+playlistSongElements\s*=\s*/)
```

`querySelectorAll()` メソッドを使用して `.playlist-song` の要素を選択し、定数 `playlistSongElements` に代入してください。
