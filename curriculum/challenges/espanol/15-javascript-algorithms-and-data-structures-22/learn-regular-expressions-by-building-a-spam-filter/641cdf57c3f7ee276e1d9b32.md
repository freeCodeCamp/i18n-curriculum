...
Después de la instrucción `if`, utiliza un operador ternario para verificar la veracidad de llamar a `isSpam()` con `messageInput.value` como argumento. Si es verdadero, establece la propiedad `textContent` en el elemento `result` a `"¡Oh no! Esto parece ser un mensaje de spam."`. De lo contrario, establécelo en `"Este mensaje no parece contener spam."`
...
La expresión falsy de tu ternario debe establecer la propiedad `textContent` del elemento `result` en `"Este mensaje no parece contener spam."`.
