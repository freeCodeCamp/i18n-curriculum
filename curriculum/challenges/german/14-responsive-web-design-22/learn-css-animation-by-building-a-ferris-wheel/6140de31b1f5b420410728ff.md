...
Deine `@keyframes wheel`-Regel sollte einen `100%`-Selektor haben.
...
Dein `100%`-Selektor sollte nach deinem `0%`-Selektor stehen.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[0]?.cssRules?.[1]?.keyText === '100%')
```

Dein `100%`-Selektor sollte eine `transform`-Eigenschaft mit dem Wert auf `rotate(360deg)` gesetzt haben.
