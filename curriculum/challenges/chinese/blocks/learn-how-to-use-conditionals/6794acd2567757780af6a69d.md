...
`Pretty` in this context, means `fairly` or `quite`, and it is used to describe the strength or reliability of something. 例如：

`The project is pretty successful.` - This means the project is fairly or quite successful.

`Solid`means `strong`, `reliable`, or `stable`. It is often used to talk about something that is dependable or in good condition. 例如：
