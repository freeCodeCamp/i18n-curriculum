...
أضف الآن `div` مع `className` باسم `card-stats`. بداخله، أضف `div` مع `className` باسم `stat-col`. وداخله، أضف ثلاثة عناصر `div` كل منها يحمل `className` باسم `stat-row`.

# --hints--

يجب إضافة `div` مع `className` باسم `card-stats` بعد `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

يجب إضافة `div` مع `className` باسم `stat-col` داخل `.card-stats`.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

يجب إضافة ثلاثة عناصر `div` كل منها يحمل `className` باسم `stat-row` داخل `.stat-col`.
