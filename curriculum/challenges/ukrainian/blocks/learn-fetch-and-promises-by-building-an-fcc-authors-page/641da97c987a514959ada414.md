...
Тепер викличте функцію `displayAuthors` із частиною даних автора, так само як ви це робили раніше.

Якщо ви натиснете кнопку `Load More Authors` після виклику функції, це не працюватиме. Це пов’язано з тим, що досі потрібно додати до кнопки слухач подій `click`. Ви зробите це далі.
...
Першим аргументом методу `slice()` має бути `startingIndex`.
...
Другим аргументом методу `slice()` має бути `endingIndex`.
