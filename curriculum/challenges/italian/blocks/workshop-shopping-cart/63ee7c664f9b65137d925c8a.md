...
La tua classe `ShoppingCart` deve poter aggiungere elementi. Crea un metodo `addItem` vuoto, che prenda due parametri: `id` e `products`. Creare un metodo potrebbe assomigliare a questo:
...
Il primo parametro, `id`, è il `id` del prodotto che l’utente ha aggiunto al carrello. Il secondo parametro, `products`, è un array di istanze `Dessert`. Usando un parametro invece di fare riferimento diretto al tuo array `products` esistente, questo metodo sarà più flessibile se vorrai aggiungere altre liste di prodotti in futuro.

# --hints--

La tua classe `ShoppingCart` dovrebbe avere un metodo `addItem`.
...
Il tuo metodo `addItem` dovrebbe prendere due parametri: `id` e `products`.
...
Il tuo metodo `addItem` dovrebbe essere vuoto.
