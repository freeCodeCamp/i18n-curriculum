...
Changamoto ya ziara ya Knight: Una ubao wa chess wa `width` \* `height` ambao ni tupu, isipokuwa kwa Knight mmoja kwenye mstatili fulani. Knight lazima ifanye mfululizo wa hatua halali ambazo zitamsababisha Knight kutembelea kila mstatili kwenye ubao wa chess mara moja tu. Kumbuka kwamba si sharti ziara iwe "imefungwa"; yaani, Knight haijalishi kumaliza ndani ya hatua moja kutoka mahali alipoanza.
...
`knightTour` inapaswa kuwa kitendakazi.
...
`knightTour(5, 6)` inapaswa kurudisha `30`.

```js
assert.equal(knightTour(5, 6), 30);
```

`knightTour(4, 6)` inapaswa kurudisha `12`.

```js
assert.equal(knightTour(4, 6), 12);
```

`knightTour(7, 3)` inapaswa kurudisha `10`.
