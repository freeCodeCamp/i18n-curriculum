...
已提供卡片的預設圖片和用於發送 API 請求的 URL，你只需要一些變數來儲存它們。

創建一個 `CDN_URL` 變數並將其設為 `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`。同時創建一個 `defaultImg` 變數，值為 `new URL("default.svg", CDN_URL + "/")`，以及一個 `LOCAL_DEFAULT_IMG` 變數，值為 `defaultImg`。

# --hints--

你應該創建一個名為 `CDN_URL` 的變數。
...
你的 `CDN_URL` 變數應該設為 `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"`。
...
你的 `defaultImg` 變數應該設為 `new URL("default.svg", CDN_URL + "/")`。
...
你應該創建一個名為 `LOCAL_DEFAULT_IMG` 的變數。
...
你的 `LOCAL_DEFAULT_IMG` 變數應該設為 `defaultImg`。
