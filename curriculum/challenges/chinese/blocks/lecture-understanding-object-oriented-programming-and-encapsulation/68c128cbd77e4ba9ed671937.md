...
在 Python 中，装饰器是一个函数，它在不改变其他函数或类的原始 `code` 的情况下修改它们的功能。

虽然可以创建自定义装饰器，但这超出了本课的范围。只需知道，要创建一个属性，你需要定义一个方法并在其上方放置 `@property` 装饰器。这会将该方法转换为属性，因此它可以像访问属性一样访问，同时内部调用被装饰的方法。
