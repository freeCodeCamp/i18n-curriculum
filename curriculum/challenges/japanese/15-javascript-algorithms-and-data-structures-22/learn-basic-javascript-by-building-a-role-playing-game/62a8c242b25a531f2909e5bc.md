...
空の関数を新たに 2 つ作成し、それぞれ `fightSlime` および `fightBeast` と名付けてください。 これらの関数は、この後作成する `cave` オブジェクトで使用します。

# --hints--

`fightSlime` は関数でなければなりません。

```js
assert.isFunction(fightSlime);
```

`fightBeast` は関数でなければなりません。

```js
assert.isFunction(fightBeast);
```

`fightSlime` は空でなければなりません。

```js
assert.match(fightSlime.toString(), /function fightSlime\(\s*\) \{\}/);
```

`fightBeast` は空でなければなりません。
