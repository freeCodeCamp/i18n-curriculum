...
Flexbox와 마찬가지로 CSS Grid에서도 `align-items`와 `justify-items`를 사용해 그리드 항목의 내용을 정렬할 수 있습니다. `align-items`는 자식 요소를 열 축을 따라 정렬하고, `justify-items`는 자식 요소를 행 축을 따라 정렬합니다.

`.social-icons` 선택자에 `align-items` 속성을 `center`로 설정하세요.

# --hints--

`.social-icons` 선택자에 `align-items` 속성을 `center`로 설정해야 합니다.
