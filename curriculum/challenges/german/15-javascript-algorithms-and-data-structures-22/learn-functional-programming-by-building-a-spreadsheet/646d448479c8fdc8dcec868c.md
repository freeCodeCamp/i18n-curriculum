...
Deklariere eine `functionCall`-Variable und weise ihr den folgenden regulären Ausdruck zu: `/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

Dieser Ausdruck sucht nach Funktionsaufrufen wie `sum(1, 4)`.

# --hints--

Du solltest eine `functionCall`-Variable deklarieren.
...
Du solltest `const` verwenden, um deine `functionCall`-Variable zu deklarieren.
...
Du solltest `functionCall` den angegebenen regulären Ausdruck zuweisen.
