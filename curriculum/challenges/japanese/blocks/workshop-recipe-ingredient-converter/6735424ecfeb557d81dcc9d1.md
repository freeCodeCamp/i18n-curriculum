...
このステップでは、`submit`に`recipeForm`イベントリスナーを追加して、`Convert`ボタンがクリックされたときに`updateResultsList()`関数が呼び出されるようにしてください。

`submit`に`recipeForm`イベントリスナーを追加します。フォームが提出されたときに、まず`event.preventDefault()`を呼び出してデフォルトの動作を停止し、その後`updateResultsList()`を呼び出してください。

これにより、フォームに入力して`Convert`を押すことでUIから関数を起動できるようになり、次のステップで変換ロジックを追加しながらデバッグするのに役立ちます。

# --hints--

`Convert`ボタンをクリックしたときに`updateResultsList()`が実行されるように、`"submit"`に`recipeForm`イベントリスナーを追加して、フォームのデフォルト動作を停止するために`event.preventDefault()`を呼び出してください。
