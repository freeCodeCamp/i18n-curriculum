...
Sasa ni wakati wa kutekeleza kitendakazi cha kufuta kwa orodha ya nyimbo. Hii itasimamia kuondoa wimbo kutoka kwenye orodha, kushughulikia vitendo vingine vinavyohusiana wakati wimbo unafutwa, na kuunda kitufe cha `Reset Playlist`.

Tumia `const` na sintaksia ya mshale kuunda kitendakazi tupu cha `deleteSong` na upitie `id` kama kigezo.

# --hints--

Unapaswa kutumia `const` kuunda kitendakazi tupu chenye jina `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` inapaswa kuwa kitendakazi.

```js
assert.isFunction(deleteSong)
```

Kitendakazi chako cha `deleteSong` kinapaswa kutumia sintaksia ya mshale.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Kitendakazi chako cha `deleteSong` kinapaswa kuchukua kigezo cha `id`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

Kitendakazi chako cha `deleteSong` kinapaswa kuwa tupu.
