...
Спочатку потрібно обробити оператори з вищим пріоритетом. Оголосіть змінну `noHigh` і призначте їй результат виклику `highPrecedence()` з `str` як аргументом.

# --hints--

Ви повинні оголосити змінну `noHigh`.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*(?:var|let|const)\s+noHigh\s*=/);
```

Використайте `const` для оголошення змінної `noHigh`.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=/);
```

Ви повинні призначити `noHigh` результат виклику `highPrecedence()`.

```js
assert.match(code, /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*\{\s*const\s+noHigh\s*=\s*highPrecedence\(/);
```

Передайте `str` як аргумент у виклик `highPrecedence()`.
