...
`preview-panel` 요소 안에 `p` 요소를 `className`의 `preview-label`로, 텍스트 `Live Preview`와 함께 추가하세요. 또 다른 `p` 요소를 `className`의 `preview-hint`로, 텍스트 `Updates as you type`와 함께 추가하고, `div` 요소를 `className`가 `preview-box tier-${getPlayerTier(player.overallRating)}` 템플릿 리터럴로 설정된 상태로 추가하세요.

마지막 `div` 안에는 `PlayerCard` 컴포넌트와 그 프로프를 이동하세요.

# --hints--

`div.preview-panel` 요소 안에는 `p`가 `className`의 `preview-label`를 가지고 텍스트 `Live Preview`를 포함해야 합니다.
...
`div.preview-panel` 안에는 `p`가 `className`의 `preview-hint`를 가지고 텍스트 `Updates as you type`를 포함해야 합니다.
...
`div.preview-panel` 안에는 `div`가 `className`의 `preview-box`를 가지고, `className` 접두사가 붙은 동적 티어 `tier-`를 포함해야 합니다.
...
`PlayerCard`는 `div` 요소 안에 `className`가 `preview-box`인 상태로 렌더링되어야 합니다.
