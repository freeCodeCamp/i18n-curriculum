...
`elemValue` 매개변수를 받는 `num` 함수를 선언하세요. 함수는 비워 두세요.

# --hints--

`elemValue` 함수 뒤에 `rangeFromString()` 변수를 선언해야 합니다.
