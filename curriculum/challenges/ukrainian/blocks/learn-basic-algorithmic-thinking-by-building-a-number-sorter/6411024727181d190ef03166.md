...
Після вкладеного циклу `for` ви знайшли найменше значення. Вам потрібно замінити його вашим поточним значенням.

Подібно до сортування бульбашкою, використайте змінну `temp`, щоб отримати значення `array[i]`, а потім поміняйте значення `array[i]` та `array[minIndex]`.

# --hints--

Оголосіть змінну `temp` після вкладеного циклу `for`.
...
Призначте `array[minIndex]` до `array[i]`.
...
Призначте `temp` до `array[minIndex]`.
