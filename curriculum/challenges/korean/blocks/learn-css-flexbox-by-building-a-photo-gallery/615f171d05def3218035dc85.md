...
body 요소의 여백을 제거하고, `font-family`를 `sans-serif`로 설정한 다음, `background-color` 속성에 `#f5f6f7` 값을 지정하세요.
...
`body` 선택자에 `margin` 속성을 값 0으로 설정해야 합니다.
...
`body` 선택자에 `font-family` 속성을 `sans-serif` 값으로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily === 'sans-serif');
```

`body` 선택자에 `background-color` 속성을 `#f5f6f7` 값으로 설정해야 합니다.
