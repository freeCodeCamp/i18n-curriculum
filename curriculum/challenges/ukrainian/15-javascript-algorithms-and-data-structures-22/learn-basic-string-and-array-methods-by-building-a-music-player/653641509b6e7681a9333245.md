...
Вам потрібно оновити поточну пісню, яка відтворюється, а також зовнішній вигляд елемента `playButton`.

Призначте `song` до властивості `currentSong` на об’єкті `userData`.

*Примітка*: ви не повинні використати оператор необов’язкового ланцюжка `?.` на цьому кроці, оскільки `userData.currentSong` не буде `null` або `undefined` на цьому етапі.

# --hints--

Призначте `song` до `userData.currentSong`.
