...
Jetzt ist `authorDataArr` dasselbe wie die `data`, die du vor einer Weile in die Konsole eingegeben hast. Gebe `authorDataArr` in die Konsole ein, um dies zu bestätigen.

Inside your `console.log()` statement, add the text `"Author Data Array:"` as the first argument and `authorDataArr` as the second argument. Verwende ein Komma, um den Text von `authorDataArr` zu trennen.
...
`authorDataArr` sollte der zweite Argument deiner Konsolen-Protokoll-Anweisung sein.
