...
`.formrow` 要素の上部にマージンを入れ、さらに左と右にパディングを入れてください。 他の部分のパディングの値は `0` に設定します。

次に、すべての `input` 要素のフォントサイズを大きくしてください。

# --hints--

`.formrow` セレクターを使用して `.formrow` 要素を選択する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.formrow'));
```

`.formrow` の `margin-top` を最低でも `1px` に設定する必要があります。
...
`.formrow` の `padding-top` を `0` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingTop, '0px');
```

`.formrow` の `padding-right` を最低でも `1px` に設定する必要があります。
...
`.formrow` の `padding-bottom` を `0` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingBottom, '0px');
```

`.formrow` の `padding-left` を最低でも `1px` に設定する必要があります。
...
`input` セレクターを使用して `input` 要素を選択する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input'));
```

`input` の `font-size` を `13px` より大きく設定する必要があります。
