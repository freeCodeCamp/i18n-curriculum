...
Тепер функція `update()` може фактично оцінювати формули. Пам’ятайте, що ви написали умову `if` для перевірки чи функція викликалась.

В межах інструкції `if` встановіть `value` з `element`, щоб вона була результатом функції `evalFormula()`. Поки що не передавайте жодних аргументів.

# --hints--

Оновіть властивість `value` елемента `element` у блоці `if`.
...
Призначте властивості `value` результат виклику функції `evalFormula()`.
...
Не передавайте жодних аргументів до виклику `evalFormula()`.
