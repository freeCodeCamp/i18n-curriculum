...
まず、2つの統計列の間に細い縦線をレンダリングするために、`stat-divider`の`className`を持つ自己終了型の`div`を追加し、次に3つの`div`要素を含む`stat-col`の`className`を持つ2番目の`div`を追加します。各divは`stat-row`の`className`を持っています。
