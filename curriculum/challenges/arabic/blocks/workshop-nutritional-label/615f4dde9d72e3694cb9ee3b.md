...
حدّث الآن محدِّد `h1` ليصبح `header h1` لاستهداف عنصر `h1` الخاص بك داخل الـ `header` الجديد.

# --hints--

يجب أن يكون لديك محدِّد `header h1`.

```js
assert(new __helpers.CSSHelp(document).getStyle('header h1'));
```

يجب ألا يكون لديك محدِّد `h1`.

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('h1'));
```

يجب ألا تغيّر أي خاصيات في المحدِّد.
