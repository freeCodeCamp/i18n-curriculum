---
id: 655be33b7a463a5593c91cb4
title: タスク 22
challengeType: 22
dashedName: task-22
lang: en-US
---

<!-- (Audio) Sarah: Pictures can be great to explain things. We're not using pictures, but it's definitely fun to work with data like this. -->

# --description--

何かが`definite`であると言うとき、それについて確信していることを意味します。

接尾辞`-ly`を付けると、`definite`は副詞の`definitely`になり、何かについて確信している、または自信があることを意味します。

これは話者が真実だと信じていることや、そうである可能性が高いことを強調するためによく使われます。例えば：

- `I can definitely see why this is important.` - これは重要性が明らかであると予想されることを強調しています。

- `She will definitely finish the project on time.` - これは彼女がプロジェクトを完了することに自信を示しています。

# --fillInTheBlank--

## --sentence--

`Pictures can be BLANK to explain things. We're not using pictures, but it's BLANK fun to work with data like this.`

## --blanks--

`great`

### --feedback--

この単語は何かがとても良い、またはとても素敵であることを意味します。

---

`definitely`

### --feedback--

この副詞は信念を強調します。

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Sarah",
        "position": {"x":50,"y":0,"z":1.4},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "2.3-2.mp3",
      "startTime": 1,
      "startTimestamp": 22.12,
      "finishTimestamp": 28.02
    }
  },
  "commands": [
    {
      "character": "Sarah",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sarah",
      "startTime": 1,
      "finishTime": 6.90,
      "dialogue": {
        "text": "Pictures can be great to explain things. We're not using pictures, but it's definitely fun to work with data like this.",
        "align": "center"
      }
    },
    {
      "character": "Sarah",
      "opacity": 0,
      "startTime": 7.40
    }
  ]
}
```
