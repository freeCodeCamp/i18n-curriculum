...
Your radio button should have an `id` attribute. 檢查開始標籤的名稱後面是否有空格和/或所有屬性名稱之前是否有空格。

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('id'));
```

Your radio button should have a `type` attribute. 檢查開始標籤的名稱後面是否有空格和/或所有屬性名稱之前是否有空格。

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('type'));
```

Your radio button should have an `id` attribute with the value `indoor`. 你要麼遺漏了屬性值，要麼拼寫有誤。 記住，屬性值應該放在引號中。
