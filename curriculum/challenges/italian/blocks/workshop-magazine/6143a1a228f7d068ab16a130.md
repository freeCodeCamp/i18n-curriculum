...
Sotto il tuo elemento `blockquote`, aggiungi un altro elemento `p` con il seguente testo:
...
Dovresti aggiungere un quarto elemento `p` al tuo elemento `.text`.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 5);
```

Il tuo nuovo elemento `p` dovrebbe venire dopo il tuo elemento `blockquote`.
