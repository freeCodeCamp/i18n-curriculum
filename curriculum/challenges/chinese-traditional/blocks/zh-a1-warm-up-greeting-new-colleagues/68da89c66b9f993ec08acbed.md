---
id: 68da89c66b9f993ec08acbed
title: Task 9
challengeType: 22
dashedName: task-9
lang: zh-CN
inputType: pinyin-to-hanzi
---

<!-- (Audio) Chen Na: 你們好 (nǐ men hǎo) -->

# --description--

Chen Na wants to greet a group of people. This is a common way to say "Hello, everyone" in Chinese.

Note that for the neutral tone, you should type the number 5.

# --instructions--

Listen to the audio and complete the sentence below.

# --fillInTheBlank--

## --sentence--

`BLANK`

## --blanks--

`你們好 (nǐ men hǎo)`

### --feedback--

This greeting means "Hello, everyone", typed as `ni3 men5 hao3`.

# --explanation--

`你們好 (nǐ men hǎo)` is the plural form of greeting in Chinese. It means "Hello, everyone" or "Hello, you all".

You can use it when greeting more than one person, such as colleagues or friends. For example:

`你們好 (nǐ men hǎo)，歡迎你們學習中文課程 (huān yíng nǐ men xué xí zhōng wén kè chéng)！` - Hello, everyone, welcome to learning the Chinese course.

# --scene--

```json
{
  "setup": {
    "background": "company2-boardroom.png",
    "characters": [
      {
        "character": "Chen Na",
        "position": {
          "x": 50,
          "y": 15,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "ZH_A1_greetings_and_introductions_warm_up.mp3",
      "startTime": 1,
      "startTimestamp": 8.98,
      "finishTimestamp": 9.9
    }
  },
  "commands": [
    {
      "character": "Chen Na",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Chen Na",
      "startTime": 1,
      "finishTime": 1.92,
      "dialogue": {
        "text": "你們好 (nǐ men hǎo)",
        "align": "center"
      }
    },
    {
      "character": "Chen Na",
      "opacity": 0,
      "startTime": 2.42
    }
  ]
}
```
