...
Ahora crea una etiqueta de imagen y dale la `class` `"user-img"`. Utiliza la interpolación de cadenas para establecer el atributo `src` a la `image` que extrajiste anteriormente. Establece el atributo `alt` en `author` seguido del texto `"avatar"`. Asegúrate de que haya un espacio entre la variable `author` y la palabra `"avatar"`, por ejemplo, `"Quincy Larson avatar"`.
