...
أضف مفتاحًا أخيرًا `last_visit_id` إلى قاموس `constraints`. لقيمته، استخدم `isinstance` للتحقق من أن `last_visit_id` هي سلسلة نصية.

# --hints--

يجب أن يحتوي قاموس `constraints` على مفتاح `last_visit_id`.
...
يجب أن يكون للمفتاح `last_visit_id` في قاموس `constraints` القيمة `isinstance(last_visit_id, str)`.
