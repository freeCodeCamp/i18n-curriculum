...
Denke daran, dass die `grid-column`-Eigenschaft bestimmt, in welchen Spalten ein Element startet und endet. Es wird Momente geben, in denen du dir nicht sicher bist, wie viele Spalten dein Raster haben wird, du aber möchtest, dass ein Element an der letzten Spalte endet. Um dies zu tun, kannst du zum Beenden der Spalte `-1` verwenden.

Erstelle einen `.hero`-Selektor und gebe ihm eine `grid-column`-Eigenschaft von `1 / -1`. Dies gibt an das Element weiter, die volle Breite des Rasters zu umfassen.

# --hints--

Du solltest einen `.hero`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero'));
```

Dein `.hero`-Selektor sollte eine `grid-column`-Eigenschaft von `1 / -1` haben.
