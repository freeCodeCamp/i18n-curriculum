...
Adicione um bloco `else`.Dentro do bloco `else`, chame a função `playSong` com o `id` da música que está tocando atualmente como argumento. 

Isso garante que a música que está tocando continuará a tocar quando o botão de play for clicado.

# --hints--

Quando o `playButton` for clicado e `userData.currentSong` não for `null`, você deve chamar a função `playSong` com o `id` da música que está tocando no momento.
...
Você ainda deve chamar a função `playSong` com o `id` da primeira música na sua playlist quando o `playButton` for clicado e `userData.currentSong` for `null`, 
