...
지금 `songsHTML`는 배열입니다. 이 상태로 출력하면 노래들이 쉼표로 구분되어 나타납니다. 원하는 결과는 노래들을 목록으로 표시하는 것이므로 이렇게 하면 안 됩니다. 이를 고치려면 <dfn>join()</dfn> 메서드를 사용해 배열을 하나의 문자열로 합쳐야 합니다.

`join()` 메서드는 배열의 모든 요소를 하나의 문자열로 연결하는 데 사용합니다. 이 메서드는 선택 매개변수인 `separator`를 받는데, 이 매개변수는 배열의 각 요소를 구분하는 데 사용됩니다. 예를 들어:
...
`join()` 메서드를 `map()` 메서드에 연결하고 구분자에는 빈 문자열을 전달하세요.

여러 메서드를 연결하려면 `join()` 메서드의 결과에 `map()` 메서드를 호출하면 됩니다. 예를 들어:
