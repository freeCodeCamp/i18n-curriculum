...
Neste workshop você criará um aplicativo de player de música capaz de reproduzir, pausar e pular músicas de uma playlist.

O HTML e o CSS deste projeto foram fornecidos para você, para que possa se concentrar no JavaScript.

Comece acessando os elementos `#playlist-songs`, `#play` e `#pause` e atribua-os às variáveis `playlistSongs`, `playButton` e `pauseButton`, respectivamente.
