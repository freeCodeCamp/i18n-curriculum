...
`cartSubTotal`과 같은 패턴을 따라 `cartTaxes`을 `tax` 값을 표시하도록 업데이트하고, `cartTotal`은 `total` 속성을 표시하도록 하세요.

# --hints--

`textContent` 요소의 `cartTaxes` 속성에 접근하세요.

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

`.toFixed()` 변수에서 `tax` 메서드를 호출하고 `2`를 인수로 전달하세요.

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

템플릿 리터럴 구문을 사용해 `.toFixed()` 호출 앞에 달러 기호를 추가하세요.
...
`textContent` 요소의 `cartTaxes`을 `tax.toFixed` 템플릿 문자열로 설정하세요.
...
`textContent` 요소의 `cartTotal` 속성에 접근하세요.

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

`.toFixed()` 변수에서 `total` 메서드를 호출하고 `2`를 인수로 전달하세요. `this` 키워드를 사용하는 것을 잊지 마세요.
...
`textContent` 요소의 `cartTotal`을 `total.toFixed` 템플릿 문자열로 설정하세요.
