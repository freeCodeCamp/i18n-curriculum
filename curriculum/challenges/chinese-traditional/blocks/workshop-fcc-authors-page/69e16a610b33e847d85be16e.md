...
此時，應用程式的行為不應該有任何改變。程式碼仍然會從 CDN 取得一個檔案，然後將取得的資料處理成 `JSON`。完成後，它會將該資料儲存到 `authorDataArr` 變數，接著呼叫 `displayAuthors`。

修改對 `fetch` 的呼叫，使其使用 `await` 運算子。將結果存入一個新的 `const` 變數，名稱為 `res`。這個新變數應該在 `initialFetch` 內區域定義。
