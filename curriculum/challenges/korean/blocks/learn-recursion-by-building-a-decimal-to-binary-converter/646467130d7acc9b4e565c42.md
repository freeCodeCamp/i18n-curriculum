...
모든 `parseInt(numberInput.value)`를 `inputInt`로 바꾸세요.

# --hints--

`isNaN(parseInt(numberInput.value))`를 `isNaN(inputInt)`로, `parseInt(numberInput.value) < 0`를 `inputInt < 0`로 바꾸세요.
...
`parseInt(numberInput.value) === 5`를 `inputInt === 5`로 바꾸세요.
...
`decimalToBinary(parseInt(numberInput.value))`를 `decimalToBinary(inputInt)`로 바꾸세요.
...
`checkUserInput()` 함수에는 `parseInt(numberInput.value)`가 한 번만 들어가야 합니다.
