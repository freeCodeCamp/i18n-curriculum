...
使用类选择器，给你的 `.cat-nose` 元素左右各设置一个 `15px solid transparent` 的边框。 还要给它一个底部边框，样式为 `20px solid #442c2c`。
