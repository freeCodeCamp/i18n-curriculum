...
امنح `.card` خاصية `max-width` بقيمة `400px`، و`padding` بقيمة `40px` من جميع الجهات، و`margin` بقيمة `0` للأعلى والأسفل و`auto` لليسار واليمين (استخدم الخاصية المختصرة).

# --hints--

يجب أن تحتوي `.card` على خاصية `max-width` بقيمة `400px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("max-width"), "400px");
```

يجب أن تحتوي `.card` على حشو (`padding`) بمقدار `40px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("padding"), "40px");
```

يجب أن تحتوي `.card` على `margin` بقيمة `0 auto`.
