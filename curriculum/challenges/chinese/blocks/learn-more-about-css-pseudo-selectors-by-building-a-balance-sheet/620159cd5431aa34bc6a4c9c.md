...
使用 `h1 .flex` 选择器定位你的 flex 容器。 给它一个 `display` 属性设，置为 `flex`，启用 flexbox 布局。 然后将 `flex-direction` 属性设置为 `column-reverse`——这将从下到上显示嵌套元素。 最后，将 `gap` 属性设置为 `1rem`，在元素之间创建一些空间。
