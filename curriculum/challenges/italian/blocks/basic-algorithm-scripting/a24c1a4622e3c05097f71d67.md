...
Restituisci l'indice più basso in cui un valore (secondo argomento) dovrebbe essere inserito in un array (primo argomento) una volta che è stato ordinato. Il valore restituito dovrebbe essere un numero.

Per esempio, `getIndexToIns([1,2,3,4], 1.5)` dovrebbe restituire `1` perché è maggiore di `1` (indice 0), ma minore di `2` (indice 1).

Allo stesso modo, `getIndexToIns([20,3,5], 19)` dovrebbe restituire `2` perché una volta ordinato l'array sarà come `[3,5,20]` e `19` è minore di `20` (indice 2) e maggiore di `5` (indice 1).
