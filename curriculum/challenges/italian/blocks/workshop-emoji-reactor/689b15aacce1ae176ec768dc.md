...
Ora devi ottenere il primo numero nel testo `0/10`, in un modo che funzioni anche per `5/10` o `10/10`.

Analizza il contenuto testuale dell'elemento `.count` per estrarre il numero corrente.

Converti il conteggio da stringa a numero.

Sostituisci il precedente `console.log(countEl.textContent)` con il tuo nuovo log.

Registra il conteggio corrente in questo formato: `console.log("Current count:", currCount)`.

Suggerimento: ricorda che puoi usare il metodo `split` per dividere una stringa in un array di sottostringhe.

# --hints--

Dovresti registrare il conteggio corrente come numero.
...
Dovresti registrare il conteggio corrente come numero in un modo che funzioni per i vari valori possibili.
