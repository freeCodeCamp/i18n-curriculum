...
Agrega otro `div` con el `className` de `form-group`. Dentro, añade un elemento `label` con el `className` de `label`, un `htmlFor` de `imageUrl` y el texto `Image URL`. Luego agrega un elemento `input` con un `id` de `imageUrl`, un `className` de `input` y un `type` de `text`.
