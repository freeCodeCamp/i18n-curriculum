...
Corrija esto agregando una `clase` de `left` al primer elemento `.face`, y una `clase` de `right` al segundo elemento `.face`.

# --hints--

Debe agregar una `clase` de `left` al primer elemento `.face`.

```js
assert.include(document.querySelector('.face:nth-of-type(1)').className, 'left');
```

Debe agregar una `clase` de `right` al segundo elemento `.face`.
