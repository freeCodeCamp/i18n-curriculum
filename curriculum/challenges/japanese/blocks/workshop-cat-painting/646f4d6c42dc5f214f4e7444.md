...
右のヒゲを正しい位置に動かす作業をします。クラスセレクターを使って`.cat-whisker-right-top`要素を指定し、`position`の`absolute`、`top`の`120px`、`left`の`109px`を設定してください。
...
`.cat-whisker-right-top`セレクターには`position`に設定された`absolute`プロパティが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.position, 'absolute');
```

`.cat-whisker-right-top`セレクターには`top`に設定された`120px`プロパティが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.top, '120px');
```

`.cat-whisker-right-top`セレクターには`left`に設定された`109px`プロパティが必要です。
