...
`parseCatalog` kwa sasa inarudisha safu ya data tupu. Ili kuijaza, ongeza mzunguko wa `for` ndani ya kitendakazi unaorudia juu ya `rawCards`. Ndani ya mzunguko huo, fanya wito wa `parseCard` kwa kila kipengele na sukuma matokeo ndani ya `catalog`.

# --hints--

`parseCatalog(rawCatalogCards).length` inapaswa kuwa `19`.

```js
assert.strictEqual(parseCatalog(rawCatalogCards).length, 19);
```

`parseCatalog` inapaswa kutumia mzunguko wa `for` kurudia juu ya `rawCards`.

```js
assert.match(parseCatalog.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*rawCards\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Unapaswa kufanya wito wa `parseCard` ndani ya mzunguko.
