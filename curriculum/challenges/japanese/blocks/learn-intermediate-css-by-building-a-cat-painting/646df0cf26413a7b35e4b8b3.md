...
鼻の鋭い角を、左上、右下、左下の各角に`50%`のボーダー半径で丸くしてください。また、180度回転させてください。

# --hints--

`.cat-nose`セレクターには、`border-top-left-radius`プロパティを`50%`に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderTopLeftRadius === '50%')
```

`.cat-nose`セレクターには、`border-bottom-right-radius`プロパティを`50%`に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderBottomRightRadius === '50%')
```

`.cat-nose`セレクターには、`border-bottom-left-radius`プロパティを`50%`に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderBottomLeftRadius === '50%')
```

`.cat-nose`セレクターには、`transform`プロパティを`rotate(180deg)`に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.transform === 'rotate(180deg)')
```

`border-top-right-radius`セレクターに`.cat-nose`プロパティを含めないでください。
