...
Sasa, ukifuata sintaksia iliyo juu, sahihisha kitendakazi chako cha `convertQuantity` kurudisha kitendakazi kingine. Kitendakazi hiki cha ndani kinapaswa kupokea hoja ya `toUnit`, na kinapaswa kuwa tupu.

# --hints--

Kitendakazi chako cha `convertQuantity` kinapaswa kurudisha kitendakazi.

```js
assert.isFunction(convertQuantity());
```

Kitendakazi utakachorudisha kinapaswa kuwa na kigezo cha `toUnit`.

```js
assert.match(convertQuantity()?.toString(), /\(toUnit\)/);
```

Kitendakazi utakachorudisha kinapaswa kuwa tupu.
