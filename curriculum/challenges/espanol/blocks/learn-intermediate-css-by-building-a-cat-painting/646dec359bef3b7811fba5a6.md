...
Mueve el ojo interior derecho a la posición correcta con una `position` de `absolute`, un `top` de `8px`, y un `left` de `18px`. Además, dale un radio de borde de `60%` y gíralo a `-5deg`.
