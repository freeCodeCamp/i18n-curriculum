...
在 `team` 属性下方，添加四个属性键值对。

1. 键名为 `sport`，字符串值为 `"Football"`。
2. 键名为 `year`，数字值为 `1986`。
3. 键名为 `isWorldCupWinner`，布尔值为 `true`。
4. 键名为 `headCoach`，值为空对象。
...
你的 `myFavoriteFootballTeam` 对象应该有一个 `year` 属性。
