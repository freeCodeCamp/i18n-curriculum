...
Para o bloco `catch`, adicione um parâmetro `error` e use `console.error` para registrar qualquer erro que ocorra ao buscar ou processar os dados.

# --hints--

Seu bloco `catch` deve ter um parâmetro `error`.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

Você deve registrar qualquer erro no bloco `catch` usando `console.error`.
