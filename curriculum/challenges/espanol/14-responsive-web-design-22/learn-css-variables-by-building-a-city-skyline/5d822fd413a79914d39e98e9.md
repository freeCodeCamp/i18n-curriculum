...
To begin optimizing your code, first move the `position` and `top` properties and values from `.foreground-buildings` to `.background-buildings`, then change the existing `.background-buildings` selector to also target `.foreground-buildings`, so the styles will be applied to both. Puede usar una coma (`,`) para separar selectores como este: `selector1, selector2`.
