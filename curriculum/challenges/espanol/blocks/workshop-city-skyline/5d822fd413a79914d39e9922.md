...
Asigna al elemento `.fb3a` un `width` de `80%` y un `height` de `15%`. Luego, asigna al elemento `.fb3b` un `width` de `100%` y un `height` de `35%`.

# --hints--

Debes asignar a `.fb3a` un `width` de `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

Debes asignar a `.fb3a` un `height` de `15%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

Debes asignar a `.fb3b` un `width` de `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3b")?.width, "100%");
```

Debes asignar a `.fb3b` un `height` de `35%`.
