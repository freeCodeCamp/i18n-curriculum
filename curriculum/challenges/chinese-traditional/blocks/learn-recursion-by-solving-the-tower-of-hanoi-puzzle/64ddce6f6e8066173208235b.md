...
當 `forward` 是 `False` 時，必須將碟移動到相反方向。為此撰寫一個 `else` 語句。列印移動並相應地更改列表的內容。

# --hints--

你應該有一個 `else` 語句來將碟移動到相反方向。

```js
({ test: () => assert.match(code, /rods\s*\[\s*target\s*\]\s*\.append\s*\(\s*rods\s*\[\s*source\s*\]\s*\.pop\s*\(\s*\)\s*\)\s+else\s*:/) })
```

你應該在你的 `else` 語句中放入 `print(f'Moving disk {rods[target][-1]} from {target} to {source}')`。
...
你應該從 `rods[target]` 中移除最後一個元素。

```js
({ test: () => assert.match(code, /rods\s*\[\s*target\s*\]\s*\.pop\s*\(\s*\)/) })
```

你應該在你的程式碼中有 `rods[source].append(rods[target].pop())`。
