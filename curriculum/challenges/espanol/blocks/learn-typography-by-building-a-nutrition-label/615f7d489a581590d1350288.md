...
Cree otro `.divider` después de su último elemento `p`.

# --hints--

Debe crear un nuevo elemento `div` al final de su elemento `.daily-value`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'div');
```

El nuevo elemento `div` debe tener el atributo `class` establecido en `divider`.
