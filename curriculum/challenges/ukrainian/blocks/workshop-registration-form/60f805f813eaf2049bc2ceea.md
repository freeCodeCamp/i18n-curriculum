...
Вкладіть елемент `input` у кожен `label`. Обов’язково додайте кожен `input` після тексту `label` і поставте пробіл після двокрапки.
...
Ви повинні вкласти елементи `input` у елементи `label`.

```js
assert.equal(document.querySelectorAll('label input')?.length, 4);
```

Ви повинні додати перший `input` після тексту `label` `Enter Your First Name:`, і поставити пробіл після двокрапки.
...
Ви повинні додати другий `input` після тексту `label` `Enter Your Last Name:`, і поставити пробіл після двокрапки.
...
Ви повинні додати третій `input` після тексту `label` `Enter Your Email:`, і поставити пробіл після двокрапки.
...
Ви повинні додати четвертий `input` після тексту `label` `Create a New Password:`, і поставити пробіл після двокрапки.
