...
Tu selector `span[class~="sr-only"]` debe tener una propiedad `width` establecida en `1px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span[class~="sr-only"]')?.getPropertyValue('width') === '1px');
```

Tu elemento `span[class~="sr-only"]` debe tener una propiedad `height` establecida en `1px`.
