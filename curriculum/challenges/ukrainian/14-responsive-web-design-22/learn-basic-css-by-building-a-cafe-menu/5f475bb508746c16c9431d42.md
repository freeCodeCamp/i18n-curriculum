...
Додане зображення не зцентроване горизонтально, як заголовок `Coffee` над ним. Елементи `img` наче вбудовані елементи.

Щоб зображення поводились як заголовкові елементи (на рівні блоків), створіть селектор типу `img`, використайте значення `block` для властивості `display` та використайте застосовані значення `margin-left` та `margin-right`, щоб зцентрувати горизонтально.
...
Ви повинні встановити властивість `display` на `block`.
...
Ваш елемент `img` повинен мати `margin-left` та `margin-right` зі значенням `auto`.
