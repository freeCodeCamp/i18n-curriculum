...
要為提交按鈕設計樣式，你可以使用 *屬性* 選擇器，它會根據給定的屬性值選擇元素。以下是一個範例：

```css
input[name="password"]
```

上述選取具有 `name` 屬性值為 `password` 的 `input` 元素。

現在，使用屬性選擇器將送出按鈕的 `display` 設為 `block`，並將 `width` 設為 60%。

# --hints--

你應該使用 `input[type="submit"]` 的屬性選擇器來設定送出按鈕的樣式。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="submit"]'));
```

你應該給送出按鈕一個 `display` 為 `block`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.display, 'block');
```

你應該給提交按鈕一個 `width` 為 `60%`。
