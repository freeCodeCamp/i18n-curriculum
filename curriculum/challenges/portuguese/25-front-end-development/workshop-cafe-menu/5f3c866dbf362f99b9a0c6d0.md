...
The `p` elements are nested in an `article` element with the class attribute of `item`. Você pode estilizar todos os elementos `p` aninhados em qualquer lugar dos elementos com uma classe chamada `item` como esta:
