...
字体在页面可访问性中扮演着重要角色。 有些字体比其他字体更容易阅读，在低分辨率屏幕上尤其如此。

将 `h1` 和 `h2` 元素的字体更改为 `Verdana`，并使用另一个无衬线系列的网页安全字体作为后备。

此外，给 `h2` 元素添加 `4px solid #dfdfe2` 的 `border-bottom`，以使各部分区分开来。
...
你应该将 `font-family` 属性的第二个值设置为另一个无衬线、安全的网页字体。*提示：我会选择 Tahoma*。
