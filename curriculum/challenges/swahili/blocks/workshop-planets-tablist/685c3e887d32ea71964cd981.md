...
Katika moduli zilizopita, ulijifunza jinsi ya kurudia kupitia `NodeList` kama hii:
...
Tumia njia ya `forEach` kupita kupitia `tabs`. Kwa kitendakazi cha mwitiko, tumia kitendakazi cha mshale kinachochukua kigezo kimoja, `tab`.

# --hints--

Unapaswa kuwa na `tabs.forEach(tab => {})` katika msimbo wako.
