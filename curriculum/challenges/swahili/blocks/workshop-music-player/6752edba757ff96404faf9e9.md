...
Ili kufanya programu iweze kupatikana kwa urahisi zaidi, ni muhimu kwamba kitufe cha kucheza kielezee wimbo unaochezwa sasa au wimbo wa kwanza kwenye orodha ya nyimbo.

Tengeneza kitendakazi kinachoitwa `setPlayButtonAccessibleText`. Ndani ya kitendakazi hicho, weka sifa ya kipengele ya `aria-label` ya kitufe cha kucheza kuwa `Play` ikifuatiwa na nafasi na kichwa cha wimbo unaochezwa sasa. Ikiwa `userData.currentSong` ni `null`, iweke kuwa `Play`.

# --hints--

Unapaswa kuwa na kitendakazi kinachoitwa `setPlayButtonAccessibleText`.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

Kitendakazi chako cha `setPlayButtonAccessibleText` kinapaswa kuweka sifa ya kipengele ya `aria-label` ya kitufe cha kucheza kuwa `Play` ikifuatiwa na nafasi na kichwa cha wimbo unaochezwa sasa.
...
Wakati `userData.currentSong` ni `null`, `setPlayButtonAccessibleText` inapaswa kuweka sifa ya kipengele ya `aria-label` ya kitufe cha kucheza kuwa `Play`.
