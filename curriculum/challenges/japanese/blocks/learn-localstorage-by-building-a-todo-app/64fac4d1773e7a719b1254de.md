...
`hidden` の `taskForm` クラスを切り替える既存のコードを削除し、代わりに `reset` 関数を呼び出してください。これにより、入力フィールドがクリアされ、追加されたタスクをユーザーが見られるようにフォームモーダルも非表示になります。

# --hints--

`hidden` の `taskForm` クラスを切り替えるコードを削除してください。
