...
All'interno della classe `Spider`, crea un costruttore che accetta un singolo parametro `emojiElement` di tipo `HTMLParagraphElement`.

Nel costruttore, chiama la funzione `super` con il parametro `emojiElement` per invocare il costruttore della classe genitore `Bug`. Poi, imposta la proprietà `emoji` al valore stringa `"🕷️"`.

# --hints--

La tua classe `Spider` dovrebbe avere un costruttore.
...
Il tuo costruttore `Spider` dovrebbe avere un parametro chiamato `emojiElement` di tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Il tuo costruttore `Spider` dovrebbe chiamare `super` con il parametro `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?super\s*\(\s*emojiElement\s*\)/);
```

Il tuo costruttore `Spider` dovrebbe impostare `this.emoji` su `"🕷️"`.
