...
点击 `Show Alert` 按钮以在屏幕上看到弹出警报。要与示例交互，你需要启用交互式编辑器。


**注意**：此交互示例使用的是 JavaScript，但你无需担心理解 JavaScript 代码。你将在未来的模块中学习 JavaScript。 
...
`type` 属性的另一个可能值是 `submit`。下面是使用带有 `submit` 类型的 `button` 元素的示例。
...
启用交互式编辑器，通过提供一个假邮箱地址与预览窗口中的邮箱输入进行交互。然后点击重置按钮，查看你的邮箱从字段中消失。 
...
`value` 属性用于显示按钮文本。  So, what is the difference between using the `input` and `button` elements? `input` elements are void elements, which means they cannot have child nodes, such as text, and can only have a start tag. 而 `button` 元素提供了更大的灵活性，因为你可以在其中嵌套文本、图像和图标。
