...
`allCategories` 객체 안에 숫자 `299`에 대한 새 키를 추가하고 값으로 빈 객체를 넣으세요.

그 객체 안에 `category` 키와 `"Career Advice"` 문자열 값을 가진 속성을 추가하세요. 그 속성 아래에 `className`라는 또 다른 키를 추가하고 `"career"` 문자열 값을 넣으세요.

# --hints--

`allCategories` 객체에 `299` 속성이 있어야 합니다.
