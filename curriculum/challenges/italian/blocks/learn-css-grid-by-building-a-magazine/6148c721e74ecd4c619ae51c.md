...
Il tuo elemento `.text` non è una griglia CSS, ma puoi creare colonne all'interno di un elemento senza usare la griglia impostando la proprietà `column-width`.

Dai al tuo selettore `.text` una proprietà `column-width` impostata su `25rem`.

# --hints--

Il tuo selettore `.text` dovrebbe avere una proprietà `column-width` impostata su `25rem`.
