...
HTMLの文字列が与えられたら、すべてのタグを取り除いたプレーンテキストの内容を返してください。

# --hints--

`extractContent('<p>hello world</p>')` は `"hello world"` を返すべきです。

```js
assert.equal(extractContent('<p>hello world</p>'), "hello world");
```

`extractContent('<p>hello <span>world</span></p>')` は `"hello world"` を返すべきです。

```js
assert.equal(extractContent('<p>hello <span>world</span></p>'), "hello world");
```

`extractContent('<a href="example.com">Click me</a>')` は `"Click me"` を返すべきです。

```js
assert.equal(extractContent('<a href="example.com">Click me</a>'), "Click me");
```

`extractContent('<p><button onClick="learnToCode()">Learn</button> to <code>code<code> <br/>for <strong>free</strong> <br/>on <a href="https://freecodecamp.org/" target="_blank"><span class="highlight">freecodecamp</span>.org</a>')` は `"Learn to code for free on freecodecamp.org"` を返すべきです。
