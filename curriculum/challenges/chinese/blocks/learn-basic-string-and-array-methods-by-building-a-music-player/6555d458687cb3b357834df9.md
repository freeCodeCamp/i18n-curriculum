...
访问 `userData?.currentSong?.title` 和 `userData?.currentSong?.artist` 属性并将它们分别分配给 `currentTitle` 和 `currentArtist` 变量。

# --hints--

你应该从 `userData` 对象访问 `currentSong` 的 `title`。 不要忘记使用可选链接。

```js
assert.match(code, /userData\?\.currentSong\?\.title\s*;?/)
```

你应该将 `currentSong` 的 `title` 分配给 `currentTitle` 常量。

```js
assert.match(code, /const\s+currentTitle\s*=\s*userData\?\.currentSong\?\.title\s*;?/)
```

你应该从 `userData` 对象访问 `currentSong` 的 `artist`。 不要忘记使用可选链接。

```js
assert.match(code, /userData\?\.currentSong\?\.artist\s*;?/)
```

你应该将 `currentSong` 的 `artist` 分配给 `currentArtist` 常量。
