...
Verknüpfe das erste `Label` Element mit dem `select` Element und gib dem `select` Element ein `name` Attribut.
...
Du solltest dem `select`-Element eine `id` geben, die dem `for`-Attribut des `label`-Elements entspricht.
