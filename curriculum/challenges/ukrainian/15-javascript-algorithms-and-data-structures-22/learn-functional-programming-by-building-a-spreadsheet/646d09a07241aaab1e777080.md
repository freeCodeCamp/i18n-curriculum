...
У функції `inner` поверніть результат виклику `idToText` з `character + num` як аргументом.

# --hints--

Функція `inner` має використовувати явне повернення.
...
Функція `inner` має повернути результат виклику функції `idToText`.
...
Передайте `character + num` як аргумент функції `idToText`.
