...
Para calcular um expoente de raiz, como $\sqrt[n]{x}$, você pode usar um expoente invertido $x^{1/n}$. JavaScript has a built-in `Math.pow()` function that can be used to calculate exponents.
