...
Finalmente, certos tipos de animações baseadas em propostas podem causar desconforto a alguns utilizadores.Em especial, as pessoas com desordens <dfn>vestibulares</dfn> têm sensibilidade a certos movimentos.

A at-rule `@media` possui uma feature de mídia chamada `prefers-reduced-motion` para definir CSS com base nas preferências de animação do usuário.Ele pode receber um destes valores:
...
Você aprenderá mais sobre esse recurso em uma lição futura.Por enquanto, envolva a regra de estilo que define `scroll-behavior: smooth` dentro de uma regra `@media` com a característica de mídia `prefers-reduced-motion` tendo `no-preference` definido como valor.

Bom trabalho.Você concluiu o projeto prático do Questionário de Acessibilidade.
