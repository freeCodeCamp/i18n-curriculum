...
يجب أن تُرجع طريقة `calculateTaxes` القيمة الناتجة عن `taxRate` (مقسومة على 100 لتحويلها إلى نسبة مئوية) مضروبة في معلمة `amount`.

للتوضيح، ضع حساب `taxRate / 100` داخل أقواس عادية.

# --hints--

يجب أن تقسم `taxRate` على 100 في طريقة `calculateTaxes`. تذكر استخدام الكلمة المفتاحية `this`.
...
يجب أن تضع حساب `this.taxRate / 100` داخل أقواس عادية.
...
يجب أن تضرب معلمة `amount` في `taxRate` مقسومًا على 100 في طريقة `calculateTaxes`.
