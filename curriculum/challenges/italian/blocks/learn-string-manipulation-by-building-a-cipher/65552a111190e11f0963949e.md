...
Elimina la parola chiave `pass` e restituisci `vigenere(message, key)` dalla tua nuova funzione.

# --hints--

La tua funzione `encrypt` dovrebbe restituire `vigenere(message, key)`. Ricorda di eliminare `pass`.
