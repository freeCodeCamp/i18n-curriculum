...
`apply` 関数を宣言し、`fn` と `args` のパラメータを受け取ってください。
...
`apply` 関数の2番目のパラメータは `args` にしてください。
