...
في المشروع السابق، استخدمت طريقة `.catch()` لمعالجة الأخطاء. هنا ستستخدم عبارة `try...catch` بدلاً من ذلك.

تم تصميم كتلة `try` لمعالجة الأخطاء المحتملة، وسيُنفذ الكود داخل كتلة `catch` في حال حدوث خطأ.
...
داخل دالتك `fetchData`، أضف عبارة `try...catch`. يجب أن تحتوي كتلة `catch` على معلمة خطأ باسم `err`.

# --hints--

يجب أن تحتوي دالتك `fetchData` على كتلة `try`.

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}/);
```

يجب أن تحتوي دالتك على كتلة `catch` بعد كتلة `try`.

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}\s*catch\s*\(.*\)\s*{[^}]*\s*}/);
```

يجب أن تحتوي كتلة `catch` على معلمة `err`.
