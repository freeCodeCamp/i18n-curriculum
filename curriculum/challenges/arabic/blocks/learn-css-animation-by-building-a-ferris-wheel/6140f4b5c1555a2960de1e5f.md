...
أنشئ قاعدة `@keyframes` أخرى باسم `cabins`. استخدم نفس الخصائص الموجودة في قاعدة `@keyframes wheel`، وانسخ قواعد `0%` و `100%`، لكن عيّن خاصية `transform` في محدِّد `100%` إلى `rotate(-360deg)`.
...
يجب أن يكون اسم قاعدة `@keyframes` الجديدة هو `cabins`.
...
يجب أن يأتي محدِّد `100%` بعد محدِّد `0%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '100%')
```

يجب أن تحتوي محدِّد `100%` على خاصية `transform` مضبوطة إلى `rotate(-360deg)`.
