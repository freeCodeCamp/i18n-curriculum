...
Bevor Sie damit beginnen, den nächsten und vorherigen Song abzuspielen, müssen Sie den Index jedes Songs in der `songs`-Eigenschaft von `userData` ermitteln.

Beginnen Sie damit, eine Pfeilfunktion namens `getCurrentSongIndex` zu erstellen.

# --hints--

Sie sollten eine `getCurrentSongIndex`-Funktion haben.

```js
assert.isFunction(getCurrentSongIndex);
```

Ihre `getCurrentSongIndex`-Funktion sollte die Pfeil-Syntax verwenden.
