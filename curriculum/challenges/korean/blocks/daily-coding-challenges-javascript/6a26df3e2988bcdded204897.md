---
id: 6a26df3e2988bcdded204897
title: "도전 과제 361: 말로 표현하는 시간"
challengeType: 28
dashedName: challenge-361
---

# --description--

아날로그 시계의 시침과 분침 각도(12시 방향에서 시계 방향으로 도 단위)가 주어지면, 말로 표현한 시간을 반환하세요.

분침 각도를 분으로 변환하세요(360° = 60분). 그런 다음 다음 규칙을 사용하세요:

| 분 | 말로 표현 |
|---------|--------|
| 0 | `"Y o'clock"` |
| 15 | `"quarter past Y"` |
| 1–29 (15 제외) | `"X minutes past Y"` |
| 30 | `"half past Y"` |
| 45 | `"quarter to Z"` |
| 31–59 (45 제외) | `"X minutes to Z"` (`X`는 60 - 분) |

`Y`는 현재 시, `Z`는 다음 시로, 둘 다 시침 각도에서 유도합니다(360° = 12시간).

참고: 시침과 분침 각도가 정확한 숫자에 맞지 않을 수 있으니, 적절히 반올림하세요.

# --hints--

`getSpokenTime(90, 0)`는 `"3 o'clock"`을 반환해야 합니다.

```js
assert.equal(getSpokenTime(90, 0), "3 o'clock");
```

`getSpokenTime(160, 120)`는 `"20 minutes past 5"`를 반환해야 합니다.

```js
assert.equal(getSpokenTime(160, 120), "20 minutes past 5");
```

`getSpokenTime(255, 180)`는 `"half past 8"`를 반환해야 합니다.

```js
assert.equal(getSpokenTime(255, 180), "half past 8");
```

`getSpokenTime(67.5, 92)`는 `"quarter past 2"`를 반환해야 합니다.

```js
assert.equal(getSpokenTime(67.5, 92), "quarter past 2");
```

`getSpokenTime(200, 240)`는 `"20 minutes to 7"`를 반환해야 합니다.

```js
assert.equal(getSpokenTime(200, 240), "20 minutes to 7");
```

`getSpokenTime(322.5, 273)`는 `"quarter to 11"`를 반환해야 합니다.

```js
assert.equal(getSpokenTime(322.5, 273), "quarter to 11");
```

`getSpokenTime(117.5, 335)`는 `"5 minutes to 4"`를 반환해야 합니다.

```js
assert.equal(getSpokenTime(117.5, 335), "5 minutes to 4");
```

# --seed--

## --seed-contents--

```js
function getSpokenTime(hourAngle, minuteAngle) {

  return hourAngle;
}
```

# --solutions--

```js
function getSpokenTime(hourAngle, minuteAngle) {
  const minutes = Math.floor(minuteAngle / 6);
  const hour = Math.floor(hourAngle / 30) || 12;
  const nextHour = (hour % 12) + 1;

  if (minutes === 0) return `${hour} o'clock`;
  if (minutes === 15) return `quarter past ${hour}`;
  if (minutes === 30) return `half past ${hour}`;
  if (minutes === 45) return `quarter to ${nextHour}`;
  if (minutes < 30) return `${minutes} minutes past ${hour}`;
  return `${60 - minutes} minutes to ${nextHour}`;
}
```
