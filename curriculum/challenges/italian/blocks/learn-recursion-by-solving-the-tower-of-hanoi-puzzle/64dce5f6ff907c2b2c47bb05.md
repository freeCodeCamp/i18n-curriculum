...
Quando `target` è vuoto, il disco dovrebbe essere necessariamente spostato da `source` a `target`.

Dopo la dichiarazione di `forward`, aggiungi un'istruzione `if` per verificare se `rods[target]` è vuoto. Se lo è, cambia `forward` in `True`.

# --hints--

Dovresti avere un'istruzione `if` per verificare se `rods[target]` è vuoto, e assegnare `True` a `forward`.
