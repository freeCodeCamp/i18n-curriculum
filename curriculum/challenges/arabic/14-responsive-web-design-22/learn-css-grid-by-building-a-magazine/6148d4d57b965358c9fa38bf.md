...
قم بإنشاء منتقي `.text-with-images` وتعيين خاصية `display` إلى `grid`.

# --hints--

يجب أن يكون لديك منتقي `.text-with-images`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images'));
```

يجب أن يكون لمنتقي `.text-with-images` الخاص بك الخاصية `display` بقيمة `grid`.
