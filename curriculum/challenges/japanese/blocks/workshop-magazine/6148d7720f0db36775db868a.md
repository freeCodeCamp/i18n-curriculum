...
テキスト用の列と画像用の列が必要です。`.text-with-images` セレクターに `grid-template-columns` プロパティを `1fr 2fr` に設定してください。また、列間のスペースを広げるために `column-gap` プロパティを `3rem` に設定してください。

# --hints--

`.text-with-images` セレクターには `grid-template-columns` プロパティを `1fr 2fr` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns, '1fr 2fr');
```

`.text-with-images` セレクターには `column-gap` プロパティを `3rem` に設定してください。
