...
在 `try` 块中，创建一个 `saved` 变量。对于值，使用 `getItem()` 方法来获取 `STORAGE_KEY`。就在该变量下方，创建一个带有条件 `if` 的 `saved` 语句。暂时将 `if` 块留空。

# --hints--

你的 `loadPlayer` 函数应该在 `localStorage.getItem` 块内调用 `STORAGE_KEY` 并传入 `try`。

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

`localStorage.getItem` 的结果应该保存到名为 `saved` 的变量中。

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

`try` 块应包含一个条件为 `if` 的 `saved` 语句。
