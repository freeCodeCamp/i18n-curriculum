...
Um die `input` und `textarea` Elemente mit dem Hintergrundthema zu verschmelzen, setzen Sie deren `background-color` auf `#0a0a23`. Geben Sie ihnen dann einen `1px`, `solid` Rahmen mit der Farbe `#0a0a23`.

# --hints--

Sie sollten einen durch Komma getrennten Elementselektor verwenden, um die `input` und `textarea` Elemente auszuwählen.
...
Sie sollten den `input` und `textarea` Elementen eine `background-color` von `#0a0a23` geben.
...
Sie sollten den `input` und `textarea` Elementen einen `1px`, `solid` Rahmen mit der Farbe `#0a0a23` geben.
