...
Questo ti renderà più facile lavorare con le unità `rem` più tardi, dato che `2rem` sarebbero 20px.
...
Il selettore `html` dovrebbe avere una proprietà `font-size` impostata al `62.5%`.
