...
Você precisa atualizar a música atual que está sendo reproduzida e a aparência do elemento `playButton`.

Atribua `song` à propriedade `currentSong` no objeto `userData`.

*Nota*: Você não deve usar o operador de encadeamento opcional `?.` nesta etapa porque `userData.currentSong` não será `null` ou `undefined` neste ponto.

# --hints--

Você deve atribuir `song` a `userData.currentSong`.
