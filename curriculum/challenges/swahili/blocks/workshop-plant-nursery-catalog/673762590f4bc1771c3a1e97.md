...
Kabla hujaendelea, ondoa wito za `console.log` kutoka kwenye msimbo wako.

# --hints--

Haupaswi kuwa na `console.log(removePlant(ballerina))` kwenye msimbo wako.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*removePlant\s*\(\s*ballerina\s*\)\s*\)/);
```

Haupaswi kuwa na `console.log(sellPlants(ballerina, "small", 10));` kwenye msimbo wako.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

Haupaswi kuwa na `console.log(catalog)` kwenye msimbo wako.
