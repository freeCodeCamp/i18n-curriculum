...
Інший текст змістився з місця. Перемістіть його на потрібне місце, задавши селектору `.first-paragraph::first-letter` множину властивостей: `float` зі значенням `left` та `margin-right` зі значенням `1rem`.

# --hints--

Ваш селектор `.first-paragraph::first-letter` має мати властивість `float` зі значенням `left`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

Ваш селектор `.first-paragraph::first-letter` має мати властивість `margin-right` зі значенням `1rem`.
