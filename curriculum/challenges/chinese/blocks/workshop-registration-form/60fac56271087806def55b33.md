...
将 `select` 元素（连同其 `option` 元素）嵌入一个文本为 `How did you hear about us?` 的 `label` 元素。 文本应位于 `select` 元素之前。
...
`label` 元素的文本应该为 `How did you hear about us?`。
...
你应该在 `select` 元素内有 5 个 `option` 元素。
