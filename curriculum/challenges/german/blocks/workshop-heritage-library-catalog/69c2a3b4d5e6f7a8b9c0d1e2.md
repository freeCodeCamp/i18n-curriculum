...
Der `kingBooks`-Testcode hat bestätigt, dass `findByAuthor` korrekt funktioniert. Bevor Sie mit `groupByDecade` weitermachen, bereinigen Sie die Konsolenausgabe, indem Sie sie entfernen.

Entfernen Sie die `const kingBooks = ...`-Zeile, `console.log(kingBooks.length)` und die `for`-Schleife, die die Buchtitel und Jahre anzeigt.

# --hints--

Die `kingBooks`-Variable, die `console.log(kingBooks.length)`-Anweisung und die `for`-Schleife sollten alle entfernt werden.
