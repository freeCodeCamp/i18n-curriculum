...
이 예제의 두 번째 조건은 `a.name > b.name`인지 확인합니다. 그렇다면 함수는 `1`을 반환하는데, 이는 두 번째 과일 다음에 첫 번째 과일을 정렬합니다.
...
콜백 함수 안에 또 다른 `if` 문을 추가하여 `a.title`이 `b.title`보다 큰지 확인하세요. 그렇다면 숫자 `1`를 반환하세요.

# --hints--

콜백 함수 안에 두 번째 `if` 문이 있어야 합니다.
...
`if` 문은 `a.title`이 `b.title`보다 큰지 확인해야 합니다.

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
```

`if` 문은 `1`가 `a.title`보다 크면 숫자 `b.title`을 반환해야 합니다.
