...
Tu selector `table caption` debe tener la propiedad `position` establecida en `absolute`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('position') === 'absolute');
```

Tu selector `table caption` debe tener la propiedad `top` establecida en `-2.25rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('top') === '-2.25rem');
```

Tu selector `table caption` debe tener la propiedad `left` establecida en `0.5rem`.
