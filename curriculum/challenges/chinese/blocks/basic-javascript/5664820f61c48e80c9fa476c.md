...
In the game of Golf, each hole has a `par`, meaning, the average number of `strokes` a golfer is expected to make in order to sink the ball in the hole to complete the play. 根据你实际 `strokes` 与 `par` 的差距，会有不同的昵称来描述成绩。
