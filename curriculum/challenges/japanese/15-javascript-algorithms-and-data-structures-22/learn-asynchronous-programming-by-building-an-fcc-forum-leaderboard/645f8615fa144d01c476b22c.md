...
最後に、ユーザーのアバター画像を返します。

まずは `return` キーワードと、続けてテンプレートリテラルを追加してください。 テンプレートリテラルの中には `img` 要素を追加してください。

Inside the `img` tag, add a `src` attribute with the value of `${userAvatarUrl}`. また、`alt` 属性と値 `${user.name}` を追加してください。
