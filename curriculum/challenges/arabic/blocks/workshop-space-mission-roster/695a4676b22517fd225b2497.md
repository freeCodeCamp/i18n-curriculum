...
يتطلب مركز التحكم دالة واحدة إضافية لتسجيل ملخص عن طاقمك. أنشئ دالة فارغة باسم `printCrewSummary` تستقبل معلمة `crew`.

# --hints--

يجب أن تنشئ دالة باسم `printCrewSummary`.

```js
assert.isFunction(printCrewSummary);
```

يجب أن تحتوي دالة `printCrewSummary` على معلمة `crew`.
