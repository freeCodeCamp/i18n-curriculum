...
Prima di iniziare a lavorare sulla riproduzione della canzone successiva e precedente, devi ottenere l'indice di ogni canzone nella proprietà `songs` di `userData`.

Inizia creando una funzione freccia chiamata `getCurrentSongIndex`.

# --hints--

Dovresti avere una funzione `getCurrentSongIndex`.

```js
assert.isFunction(getCurrentSongIndex);
```

La tua funzione `getCurrentSongIndex` dovrebbe usare la sintassi della funzione freccia.
