...
Ndani ya `tr` ya kwanza, ongeza kipengele cha `th` chenye maandishi `Loans The outstanding balance on our startup loan.`. Fungia maandishi hayo, isipokuwa `Loans `, ndani ya kipengele cha `span` chenye sifa ya kipengele `class` iliyowekwa kuwa `description`.

Ongeza vipengele vitatu vya `td` chini ya hicho, na uwape maandishi yafuatayo kwa mpangilio: `$500`, `$250`, na `$0`. Kipekee cha tatu cha `td` kitoe sifa ya kipengele `class` iliyowekwa kuwa `current`.
...
Unapaswa kufungia maandishi `The outstanding balance on our startup loan.` ndani ya kipengele cha `span`.
