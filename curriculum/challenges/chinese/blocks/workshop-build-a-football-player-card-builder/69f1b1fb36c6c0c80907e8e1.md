...
在 `preview-panel` 元素内，添加一个 `p` 元素，类型为 `className`，内容为 `preview-label` 和文本 `Live Preview`，另一个 `p` 元素，类型为 `className`，内容为 `preview-hint` 和文本 `Updates as you type`，以及一个 `div`，其 `className` 设置为模板字面 `preview-box tier-${getPlayerTier(player.overallRating)}`。

在最后一个 `div` 内，放入 `PlayerCard` 组件及其属性。

# --hints--

你的 `div.preview-panel` 元素应包含一个 `p`，其 `className` 为 `preview-label`，并带有文本 `Live Preview`。
...
你的 `div.preview-panel` 应包含一个 `p`，其 `className` 为 `preview-hint`，并带有文本 `Updates as you type`。
...
你的 `div.preview-panel` 应包含一个 `div`，其 `className` 为 `preview-box`，并带有一个以 `className` 为前缀的动态层级 `tier-`。
...
你的 `PlayerCard` 应渲染在 `div` 元素内，`className` 为 `preview-box`。
