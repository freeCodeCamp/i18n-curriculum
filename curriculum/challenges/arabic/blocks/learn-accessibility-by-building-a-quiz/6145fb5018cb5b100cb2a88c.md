...
لا يجب أن يحتوي عنصر `address` على موقع جغرافي فعلي. يمكن استخدامه لتوفير رابط إلى الموضوع.

لفّ رابطًا حول النص `freeCodeCamp`، وحدد موقعه إلى `https://freecodecamp.org`.
...
يجب إعطاء عنصر `a` خاصية `href` بقيمة `https://freecodecamp.org`.

```js
assert.equal(document.querySelector('address > a')?.getAttribute('href'), 'https://freecodecamp.org');
```

يجب لف عنصر `a` حول النص الموجود `freeCodeCamp`.
