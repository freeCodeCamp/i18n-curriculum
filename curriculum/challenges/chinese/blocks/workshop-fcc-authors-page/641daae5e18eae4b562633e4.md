...
最后，如果出现错误并且作者数据加载失败怎么办？然后你需要在 UI 中显示错误。这正是 `.catch()` 方法的用途——处理错误。

在 `.catch()` 内，移除 `console.error()` 并将 `innerHTML` 的 `authorContainer` 设置为一个带有 `p` 属性为 `class` 且文本为 `"error-msg"` 的 `"There was an error loading the authors"` 元素。
...
你应该访问 `innerHTML` 的 `authorContainer` 并将其设置为 `p` 元素。不要忘记用模板文字包围 `p` 元素。
