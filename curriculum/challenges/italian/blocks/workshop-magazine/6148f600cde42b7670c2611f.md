...
Crea un'altra query `@media` per `only screen` con una `max-width` di `600px`. All’interno, crea una regola `.text-with-images` e assegna una proprietà `grid-template-columns` con valore `1fr`.

Questo farà sì che la tua area di testo inferiore si riduca a una singola colonna su schermi più piccoli.

# --hints--

Dovresti creare una nuova query `@media` per `only screen and (max-width: 600px)`. Questa dovrebbe essere sotto la tua precedente query `@media`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText, 'only screen and (max-width: 600px)');
```

La tua nuova query `@media` dovrebbe avere un selettore `.text-with-images`.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText, '.text-with-images');
```

Il tuo nuovo selettore `.text-with-images` dovrebbe avere una proprietà `grid-template-columns` con valore `1fr`.
