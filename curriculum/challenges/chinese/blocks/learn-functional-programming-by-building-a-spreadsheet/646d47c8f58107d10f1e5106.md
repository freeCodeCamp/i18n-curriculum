...
`evalFormula` 调用的第一个参数需要是单元格的内容（存储在 `value` 中）。 但是，内容以 `=` 字符开头来触发该函数，因此你需要传递从索引 `1` 开始的 `value` 子字符串。

# --hints--

你应该将 `value` 作为第一个参数传递给 `evalFormula()` 调用。
