...
Ukiona HTML yako, `tab` ya kwanza tayari imechaguliwa kwa chaguo-msingi na ina sifa ya kipengele ya `aria-selected` iliyowekwa kuwa `"true"`. Wakati mtumizi anapobofya kichupo tofauti, vichupo vyote vinapaswa kwanza kuwekwa kuwa `aria-selected="false"`.

Ili kufanikisha hili, unaweza kutumia njia ya `setAttribute` kubadilisha sifa ya kipengele ya `aria-selected` ya kila kichupo.

Hapa kuna mfano wa jinsi ya kutumia njia ya `setAttribute` kwenye orodha ya vipengele vya `div`:
...
Tumia `forEach` kwenye mkusanyiko wa `tabs` kuzunguka kila kichupo na kuweka sifa ya kipengele ya `aria-selected` kuwa `"false"`. Kwa kitendakazi cha mwitiko, unaweza kutumia jina lolote la kigezo unalotaka lakini ni bora usitumie `tab` kwa kuwa tayari inatumika katika mzunguko wa nje wa `forEach`.

# --hints--

Wakati mtumizi anapobofya kichupo, vichupo vyote vinapaswa kuwa na sifa ya kipengele ya `aria-selected` iliyowekwa kuwa `"false"`.
