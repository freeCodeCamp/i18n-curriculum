...
En los pasos anteriores construiste la funcionalidad para reproducir una canción. Ahora necesitas agregar la funcionalidad al botón de reproducción para que reproduzca la canción actual cuando se haga clic en él.

Usa el método `addEventListener()` y pasa un evento `"click"` como primer argumento y una función de devolución de llamada vacía con sintaxis de flecha como segundo argumento, e.g., `() => {}`.

# --hints--

Debes llamar al método `addEventListener()` sobre tu variable `playButton`.
...
Debes usar la sintaxis de flecha para pasar una devolución de llamada vacía en tu escuchador de eventos. No olvides que también necesitas un par de llaves vacío.
