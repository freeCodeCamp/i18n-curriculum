...
Un algoritmo de selección depende de rastrear el índice del valor más pequeño en la lista. Declara una variable `minIndex` y establécelo en `i` - esto asegura que si tu valor actual es el más pequeño, será intercambiado consigo mismo y no se moverá. Necesitarás poder reasignar el valor de `minIndex` mientras iteras a través de la lista.

Entonces, escribe otro bucle `for`, usando `j` como el iterador. Este bucle necesita comenzar en el índice después de `i` e iterar por el resto de la lista.

# --hints--

Debe usar `let` para declarar `minIndex`.
...
Deberías establecer `minIndex` en `i`.
...
Debe tener un bucle `for` anidado.
...
Tu bucle `for` debería inicializar `j` en `i + 1`.
...
Tu bucle `for` debe iterar a través del resto de la lista.
...
Tu bucle `for` debe incrementar `j`.
