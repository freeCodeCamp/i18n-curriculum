...
在 `0%` 和 `100%` 選擇器之間，為你的動畫添加一個 `50%` 選擇器。這將會在動畫週期的中間套用。將 `background-color` 設定為 `purple`。

# --hints--

你應該在你的 `@keyframes cabins` 規則中創建一個新的 `50%` 選擇器。
