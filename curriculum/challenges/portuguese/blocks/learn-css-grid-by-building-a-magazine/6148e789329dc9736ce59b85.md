...
Crie uma regra `.image-1, .image-3` e dê a ela a propriedade `grid-column` definida como `1 / -1`. Isso permitirá que as primeiras e as terceiras imagens tomem a largura total da grade.

# --hints--

Você deve ter um seletor `.image-1, .image-3`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

O seletor `.image-1, .image-3` deve ter a propriedade `grid-column` definida como `1 / -1`.
