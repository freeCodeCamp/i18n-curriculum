...
All'interno dell'istruzione `if`, destruttura `className` e `category` dall'oggetto `allCategories[id]`.

# --hints--

Dovresti destrutturare `className` e `category` dall'oggetto `allCategories[id]`. Le proprietà possono essere in qualsiasi ordine.
