...
**ストラドリングチェッカーボード**方式を使ってメッセージを暗号化および復号化する関数を実装してください。

各関数は2つの引数を取ります。`message`の文字列と`alphabet`の配列です。`alphabet`の配列には、ストラドリングチェッカーボードの3行を表す3つの文字列が含まれます。

`straddle()`関数の出力は一連の10進数の桁である必要があります。数字は、各桁の前にエスケープ文字を挿入し、その後にその桁を<em>暗号化せずに</em>含めることで暗号化してください。

復号化は`unstraddle()`関数でこれを逆にします。

# --hints--

`straddle`は関数である必要があります。
