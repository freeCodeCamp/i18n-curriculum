...
يجب أن تعطي أزرار الاختيار المخصصة حجمًا وشكلًا.

مع الاستمرار في استهداف `.contact-method-radio-btn`، عيّن `width` إلى `20px`، و`height` إلى `20px`، و`border-radius` إلى `50%`، و`border` إلى `2px solid gray`.
...
عيّن الخاصية `border-radius` إلى `50%`.
...
عيّن الخاصية `border` إلى `2px solid gray`.
