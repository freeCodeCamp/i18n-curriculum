...
تحتوي المتصفحات على دالة `confirm()` مدمجة تعرض رسالة تأكيد للمستخدم. تستقبل `confirm()` سلسلة نصية، وهي الرسالة المعروضة للمستخدم. تُرجع `true` إذا أكد المستخدم، و`false` إذا ألغى.

أعلن عن متغير `isCartCleared` وقم بتعيين قيمة استدعاء `confirm()` مع السلسلة النصية `"Are you sure you want to clear all items from your shopping cart?"`.

# --hints--

يجب استخدام `const` لإعلان المتغير `isCartCleared`.
...
يجب استدعاء الدالة `confirm()`.

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(/);
```

يجب تمرير السلسلة النصية `Are you sure you want to clear all items from your shopping cart?` إلى الدالة `confirm()`.

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(\s*('|"|`)Are you sure you want to clear all items from your shopping cart\?\1\s*\)/);
```

يجب تعيين قيمة الدالة `confirm()` إلى المتغير `isCartCleared`.
