...
Geben Sie als Nächstes `new_char` aus und sehen Sie sich die Ausgabe an.

# --hints--

Sie sollten Ihre `new_char`-Variable ausgeben.

```js
assert.match(code, /print\s*\(\s*new_char\s*\)/)
```

Sie sollten Ihre `new_char`-Variable am Ende Ihres Schleifenkörpers ausgeben.
