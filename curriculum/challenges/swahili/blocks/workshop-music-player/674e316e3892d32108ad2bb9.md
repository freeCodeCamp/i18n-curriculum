...
Kila unapobofya kitufe cha kucheza, wimbo wa kwanza katika orodha yako ya nyimbo daima utachezwa. Ili kurekebisha hili, ndani ya kitendakazi cha mshale cha msikilizaji wa tukio, badilisha `playSong(0)` kwa tamko la `if` linalokagua kama `userData.currentSong` ni `null`.

Ndani ya sehemu ya `if`, fanya wito wa kitendakazi cha `playSong` kwa `id` ya wimbo wa kwanza katika safu ya data ya `userData.songs`.

# --hints--

Unapaswa kubadilisha wito wa kurudisha wa msikilizaji wa tukio ili kufanya wito wa kitendakazi cha `playSong` kwa `id` ya wimbo wa kwanza katika safu ya data ya `userData.songs` wakati `userData.currentSong` ni `null`.
