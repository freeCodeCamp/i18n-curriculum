...
`.foot` 요소를 선택하고 `width`의 `15%`, `height`의 `30%`, `background-color`의 `orange`를 지정하세요.
...
`.foot`에 `width`의 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾을 수 있습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.width, '15%');
```

`.foot`에 `height`의 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾을 수 있습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot')?.height, '30%');
```

`.foot`에 `background-color`의 `--fcc-expected--`를 지정하세요. `--fcc-actual--`에서 찾을 수 있습니다.
