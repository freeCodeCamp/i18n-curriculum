...
使用與你的 `addCharacters` 函式相同的語法來更新你的 `elemValue` 函式。它不應再宣告 `inner`，而應隱式地傳回該函式。

# --hints--

你的 `elemValue` 函式應該使用隱式傳回。
...
你的 `elemValue` 函式應該隱喻地傳回一個帶有 `character` 參數的箭頭函式。
...
你的內部箭頭函式（function）應該傳回以 `character + num` 作為引數呼叫 `idToText()` 的結果。
