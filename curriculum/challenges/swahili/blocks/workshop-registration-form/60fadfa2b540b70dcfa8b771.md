...
Anza kwa kumpa vipengele vya `input` katika `fieldset` ya pili darasa la `inline`.

# --hints--

Unapaswa kumpa `input` ya kwanza darasa la `inline`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

Unapaswa kumpa `input` ya pili darasa la `inline`.
