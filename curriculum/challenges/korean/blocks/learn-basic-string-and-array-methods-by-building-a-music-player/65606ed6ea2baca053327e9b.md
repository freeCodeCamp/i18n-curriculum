...
`allSongs` 배열의 세 번째 값은 객체여야 합니다.

```js
assert.isObject(allSongs[2]);
```

`allSongs` 배열의 세 번째 객체는 `id` 속성이 숫자 `2`로 설정되어야 합니다.

```js
assert.equal(allSongs[2].id, 2);
```

`allSongs` 배열의 세 번째 객체는 `title` 속성이 문자열 `"Still Learning"`로 설정되어야 합니다.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

`allSongs` 배열의 세 번째 객체는 `artist` 속성이 문자열 `"Quincy Larson"`로 설정되어야 합니다.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

`allSongs` 배열의 세 번째 객체는 `duration` 속성이 문자열 `"3:51"`로 설정되어야 합니다.

```js
assert.equal(allSongs[2].duration, "3:51");
```

`allSongs` 배열의 세 번째 객체는 `src` 속성이 문자열 `"https://cdn.freecodecamp.org/curriculum/js-music-player/still-learning.mp3"`로 설정되어야 합니다.
