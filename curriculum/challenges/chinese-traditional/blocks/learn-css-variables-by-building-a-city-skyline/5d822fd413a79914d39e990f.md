...
視窗堆疊在區段左側，紫色建築物的後方。在 `.building-wrap` 下方添加一個名為 `window-wrap` 的新類別。將 `.window-wrap` 設為 flexbox 容器，並使用 `align-items` 和 `justify-content` 屬性分別將其子元素垂直置中及在父元素中均勻分布。
