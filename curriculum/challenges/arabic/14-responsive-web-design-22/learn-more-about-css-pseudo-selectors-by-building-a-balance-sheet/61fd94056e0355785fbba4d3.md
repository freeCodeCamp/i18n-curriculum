...
قم بإضافة ثلاثة عناصر `td` أدناه، مع إعطائهم النصوص التالية ، بالترتيب: `$200` و `$300` و `$400`. اعطي عنصر `td` الثالث `class` بقيمة `current`.
...
يجب أن يحتوي عنصر `th` الخاص بك على النص `Expenses Annual anticipated expenses, such as payroll.`.
...
يجب أن يغلف النص `Annual anticipated expenses, such as payroll.` في عنصر `span`.
