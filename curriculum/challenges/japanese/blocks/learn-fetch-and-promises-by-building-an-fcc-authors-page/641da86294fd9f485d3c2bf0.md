...
`class` `"author-link"` のアンカー要素を追加し、`url` 属性の値として `href` を埋め込み、`target` を `"_blank"` に設定してください。アンカー要素のテキストには、`author` に続けてテキスト `"'s author page"` を埋め込んでください。例えば、`"Quincy Larson's author page"` のようになります。
...
アンカー要素にはクラス `"author-link"` を付けてください。

```js
assert.include(document.querySelector('a')?.className, "author-link");
```

アンカーの `url` 属性の値として `href` を埋め込んでください。
...
アンカー要素の中に `author` に続けてテキスト `"'s author page"` を埋め込んでください。
