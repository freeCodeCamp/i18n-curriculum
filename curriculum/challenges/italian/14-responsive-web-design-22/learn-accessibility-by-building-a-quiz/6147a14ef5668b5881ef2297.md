...
Per rendere i pulsanti di navigazione più usuali, rimuovi la sottolineatura dai tag di ancoraggio.

Quindi, crea un nuovo selettore che fa riferimento agli elementi dell'elenco di navigazione, che fa sì che posizionando il cursore su di essi, il colore di sfondo e il colore del testo vengano scambiati e il cursore diventi un puntatore.

# --hints--

Dovresti usare il selettore `li > a` per impostare la proprietà `text-decoration` su `none`.
...
Dovresti dare agli elementi `li` una proprietà `background-color` con il valore `#dfdfe2` al passaggio del mouse.
...
Dovresti dare agli elementi `li` una proprietà `color` con il valore `#1b1b32` al passaggio del mouse.
...
Dovresti dare agli elementi `li` una proprietà `cursor` con il valore `pointer` al passaggio del mouse.
