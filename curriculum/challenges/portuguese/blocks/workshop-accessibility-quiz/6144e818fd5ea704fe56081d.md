...
Você deve dar ao primeiro `fieldset` um atributo `name` adequado. *Dica: eu usaria `html-question-one`*

```js
assert.notEmpty(document.querySelectorAll('fieldset')?.[0]?.name);
```

Você deve dar ao segundo `fieldset` um atributo `name` adequado. *Dica: eu usaria `html-question-two`*
