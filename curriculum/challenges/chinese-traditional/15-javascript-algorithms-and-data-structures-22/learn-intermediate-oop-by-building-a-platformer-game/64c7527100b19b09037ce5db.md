...
在 `draw` 方法內部，將 `"#acd157"` 分配給 `ctx.fillStyle`。

下面，調用 `ctx.fillRect` 方法並傳入 `x` 和 `y` 座標以及 `width` 和 `height` 屬性。 請記住在每個屬性前包含 `this`。

# --hints--

你應該將 `"#acd157"` 分配給 `ctx.fillStyle`。

```js
assert.match(code, /ctx\.fillStyle\s*=\s*('|")#acd157\1\s*;?/);
```

你應該調用 `ctx.fillRect` 方法並傳遞正確的參數。
