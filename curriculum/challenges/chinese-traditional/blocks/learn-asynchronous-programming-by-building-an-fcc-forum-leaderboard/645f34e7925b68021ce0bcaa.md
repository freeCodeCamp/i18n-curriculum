...
下一步是在 `users` 數組中找到正確的用戶。

首先創建一個名爲 `user` 的常量，並將其賦值給 `users.find()`。 `find` 方法應該有一個參數爲 `user` 的回調函數。

在 `find` 方法的回調函數中，隱式返回檢查 `user.id` 是否嚴格等於 `poster.user_id` 的結果。
