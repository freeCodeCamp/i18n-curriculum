...
يجب عليك استخدام منتقي `.beak.bottom`.

```js
assert.match(code, /\.beak\.bottom\s*\{/);
```

يجب أن يحتوي منتقي `.beak.bottom` على خاصية `width` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.width, '16%');
```

يجب أن يحتوي منقي `.beak.bottom` على خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak.bottom')?.top, '65%');
```

يجب أن يحتوي منتقي `.beak.bottom` على خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
