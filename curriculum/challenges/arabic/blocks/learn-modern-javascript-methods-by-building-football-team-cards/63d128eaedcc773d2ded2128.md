...
أسفل ذلك الكائن، أنشئ كائنًا جديدًا بالخصائص التالية:
...
يجب أن تحتوي مصفوفة `myFavoriteFootballTeam.players` على قيمتين.
...
يجب أن تحتوي مصفوفة `myFavoriteFootballTeam.players` على كائن كقيمة ثانية.
...
يجب تعيين خاصية `name` إلى `"Sergio Batista"`.
...
يجب تعيين خاصية `position` إلى `"midfielder"`.
