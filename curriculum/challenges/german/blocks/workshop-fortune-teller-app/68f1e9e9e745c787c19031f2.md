...
Innerhalb der `if`-Anweisung holen Sie das `cardTitle`-Element und setzen dessen `textContent` auf `foundCard.name`. Machen Sie dasselbe für das `description`-Element, indem Sie dessen `textContent` auf `foundCard.desc` setzen.

# --hints--

Sie sollten das `cardTitle`-`textContent` auf `foundCard.name` setzen. Vergessen Sie nicht, das Schlüsselwort `this` zu verwenden.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

Sie sollten das `description`-`textContent` auf `foundCard.desc` setzen. Vergessen Sie nicht, das Schlüsselwort `this` zu verwenden.
