...
在前面的步驟中，你構建了播放歌曲的功能。 現在你需要爲播放按鈕添加功能，以便單擊該按鈕時播放當前歌曲。

使用 `addEventListener()` 方法並傳入 `"click"` 事件作爲第一個參數，並傳入一個帶有箭頭語法的空回調函數作爲第二個參數，例如 `() => {}`。

# --hints--

你應該在 `playButton` 變量上調用 `addEventListener()` 方法。
...
你應該使用箭頭語法將空回調傳遞到你的事件監聽器中。 不要忘記你還需要一對空的花括號。
