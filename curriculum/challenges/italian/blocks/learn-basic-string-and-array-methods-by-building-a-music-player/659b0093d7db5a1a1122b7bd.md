...
Per ottenere l'indice della canzone corrente, puoi usare il metodo <dfn>indexOf()</dfn>. Il metodo `indexOf()` dell'array restituisce il primo indice in cui un dato elemento può essere trovato nell'array, oppure `-1` se l'elemento non è presente.
...
All'interno della tua funzione `getCurrentSongIndex`, restituisci `userData?.songs.indexOf()`. Per l'argomento `indexOf()`, impostalo su `userData?.currentSong`.

# --hints--

Dovresti restituire `userData?.songs.indexOf()` all'interno della tua funzione `getCurrentSongIndex`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

Dovresti passare `userData?.currentSong` al metodo `indexOf()`.
