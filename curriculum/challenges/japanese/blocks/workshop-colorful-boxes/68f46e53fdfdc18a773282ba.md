...
`styles.css` ファイル内に `.box2` セレクターを作成し、`background` プロパティを値 `#f4a261` で追加してください。

また、`order` プロパティを値 `0` で追加してください。
