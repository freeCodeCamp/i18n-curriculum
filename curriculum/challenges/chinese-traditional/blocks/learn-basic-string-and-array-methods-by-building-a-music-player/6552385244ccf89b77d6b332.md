...
你需要處理播放下一首歌曲和上一首歌曲。為此，你將需要一個 `playNextSong` 和 `playPreviousSong` 函式（程式）。

使用 `const` 和箭頭語法來創建一個空的 `playNextSong` 函式（程式）。

# --hints--

你應該使用 `const` 來創建一個名為 `playNextSong` 的空函式（程式）。

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` 應該是一個函式（程式）。

```js
assert.isFunction(playNextSong)
```

你的 `playNextSong` 函式應該使用箭頭語法。

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

你的 `playNextSong` 函式（程式）應該是空的。
