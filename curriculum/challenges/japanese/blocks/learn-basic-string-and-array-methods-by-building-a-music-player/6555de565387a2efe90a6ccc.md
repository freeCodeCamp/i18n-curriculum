...
`setPlayButtonAccessibleText` 関数は `aria-label` 属性を現在の曲、またはプレイリストの最初の曲に設定します。プレイリストが空の場合は、`aria-label` を `"Play"` に設定します。

`setAttribute` 要素の `playButton` メソッドを使って、`"aria-label"` という名前の属性を設定してください。三項演算子を使い、`Play ${song.title}` が利用できない場合は属性の値を `"Play"` または `song?.title` に設定してください。

ここでは文字列補間が必要なので、バックティックを使うことを忘れないでください。

# --hints--

既存の `setPlayButtonAccessibleText` 関数とその内容は変更しないでください。
...
`setAttribute()` に対して `playButton` メソッドを使うべきです。

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` メソッドの最初の値が `setAttribute()` であるべきです。

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

`setAttribute` メソッドの第2引数は ``song?.title ? `Play ${song.title}` : "Play"`` であるべきです。
