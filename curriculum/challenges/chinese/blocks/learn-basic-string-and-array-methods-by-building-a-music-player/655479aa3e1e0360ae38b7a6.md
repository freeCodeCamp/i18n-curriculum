...
接下来，你需要检索播放列表中的下一首歌曲。 为此，你需要获取当前歌曲的索引，然后向其中添加 `1`。

创建一个名为 `nextSong` 的常量，并将 `userData?.songs[currentSongIndex + 1]` 分配给它。

最后，调用 `playSong` 函数并传入 `nextSong.id` 作为参数。

# --hints--

你不应修改现有的 `if` 语句、其 `else` 块和内容。
...
你应该将 `userData?.songs[currentSongIndex + 1]` 分配给 `nextSong` 常量。
...
你应该使用 `nextSong.id` 调用 `playSong` 函数。
