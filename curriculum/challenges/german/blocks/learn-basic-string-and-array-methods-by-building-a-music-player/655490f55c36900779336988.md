...
Durchlaufen Sie das `playlistSongElements` mit einer `forEach`-Methode.

Die `forEach`-Methode wird verwendet, um ein Array zu durchlaufen und eine Funktion auf jedes Element des Arrays anzuwenden. Zum Beispiel nehmen wir an, Sie haben ein Array von Zahlen und möchten jede Zahl in der Konsole ausgeben.
...
Verwenden Sie die `forEach`-Methode auf `playlistSongElements`. Übergeben Sie `songEl` als Parameter und verwenden Sie die Pfeilsyntax, um einen leeren Callback hinzuzufügen.

# --hints--

Sie sollten `forEach()` an `playlistSongElements` anhängen.

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

Ihre `forEach()`-Methode sollte einen `songEl`-Parameter haben und die Pfeilsyntax verwenden.

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

Ihre `forEach` sollte ein leeres Paar geschweifter Klammern haben.
