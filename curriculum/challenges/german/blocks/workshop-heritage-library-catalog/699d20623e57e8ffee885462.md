...
Es ist Zeit, dass der Validator ein echtes Problem erkennt. `catalog[0]` ist „From a Buick 8“ von Stephen King, das alle vier Felder ausgefüllt hat. `catalog[16]` ist „Unknown Title“, bei dem das Autorenfeld leer ist.

Geben Sie `validateEntry(catalog[0])` und `validateEntry(catalog[16])` in der Konsole aus. Das erste sollte `true` zurückgeben; das zweite sollte `false` zurückgeben.

# --hints--

Sie sollten `validateEntry(catalog[0])` in der Konsole ausgeben.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

Sie sollten `validateEntry(catalog[16])` in der Konsole ausgeben.
