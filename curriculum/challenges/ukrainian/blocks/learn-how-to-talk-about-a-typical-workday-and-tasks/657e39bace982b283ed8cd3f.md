...
Фраза `not that much` використовується для позначення невеликої кількості або ступеня, часто у відповідь на запитання про кількість або частоту. For example:
...
З іншого боку, `at all times` означає «завжди», вказуючи на те, що щось відбувається безперервно. For example:
