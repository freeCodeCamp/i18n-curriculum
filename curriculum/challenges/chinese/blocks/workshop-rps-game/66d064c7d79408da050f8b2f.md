...
下一步是创建负责跟踪玩家和计算机分数的变量。

创建一个名为 `playerScore` 的变量，并将其初始化为值 `0`。

然后，创建一个名为 `computerScore` 的变量，并将其初始化为值 `0`。

你需要使用 `let` 来声明这些变量，因为它们的值将在整个游戏过程中变化。

# --hints--

你应该有一个名为 `playerScore` 的变量。

```js
assert.isNotNull(playerScore);
```

你应该使用 `let` 来声明 `playerScore` 变量。

```js
assert.match(code, /let\s+playerScore/);
```

你的 `playerScore` 应该初始化为值 `0`。

```js
assert.strictEqual(playerScore, 0);
```

你应该有一个名为 `computerScore` 的变量。

```js
assert.isNotNull(computerScore);
```

你应该使用 `let` 来声明 `computerScore` 变量。

```js
assert.match(code, /let\s+computerScore/);
```

你的 `computerScore` 应该初始化为值 `0`。
