...
Your `addTwoNumbers` function should use arrow function syntax.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Your `addTwoNumbers` function should have two parameters called `num1` and `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

Your `addTwoNumbers` function should return the expression of adding `num1` and `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

You should have a `console.log(addTwoNumbers(3, 4));` statement.
