...
`sort()` 方法接受一個比較回呼函式，用來定義排序順序。 

在此範例中，第一個條件 `(a.name < b.name)` 用來檢查第一個水果的名稱是否小於第二個水果的名稱。如果是，第一個水果會排在第二個水果之前。

字串是以字元序列進行比較，這表示它們會逐字元比較。 例如，`"Apples"` 小於 `"Bananas"`，因為字母表中 `"A"` 在 `"B"` 之前。

這個範例傳回數字的原因是因為 `sort()` 方法期望傳回一個數字。如果你傳回負數，第一個項目會被排序在第二個項目前面。 
...
在你的回呼函式中，為你的 `if` 述語添加檢查 `a.title` 是否小於 `b.title`。如果是，傳回 `-1`。 

# --hints--

你應該在你的回呼函式中有一個 `if` 述語。

```js
assert.match(code, /if\s*\(/);
```

你的 `if` 述語應該檢查 `a.title` 是否小於 `b.title`。

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
``` 

如果 `a.title` 小於 `b.title`，你的 `if` 述語應該傳回 `-1`。
