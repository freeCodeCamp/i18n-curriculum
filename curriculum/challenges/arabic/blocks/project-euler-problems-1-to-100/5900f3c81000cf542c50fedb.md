...
title: "المشكلة 92: سلاسل مربعات الأرقام"
challengeType: 1
forumTopicId: 302209
...
تُنشأ سلسلة أرقام بإضافة مربع كل رقم في العدد باستمرار لتكوين عدد جديد حتى يظهر العدد مرة أخرى.
...
لذلك، أي سلسلة تصل إلى 1 أو 89 ستعلق في حلقة لا نهائية. والأمر المدهش هو أن كل رقم بداية سينتهي به المطاف عند 1 أو 89.

كم عدد الأرقام الابتدائية الأقل من `limit` التي ستصل إلى 89؟

# --hints--

يجب أن تُرجع الدالة `squareDigitChains(100)` عددًا.

```js
assert(typeof squareDigitChains(100) === 'number');
```

يجب أن تُرجع الدالة `squareDigitChains(100)` القيمة `80`.

```js
assert.strictEqual(squareDigitChains(100), 80);
```

يجب أن تُرجع الدالة `squareDigitChains(1000)` القيمة `857`.

```js
assert.strictEqual(squareDigitChains(1000), 857);
```

يجب أن تُرجع الدالة `squareDigitChains(100000)` القيمة `85623`.

```js
assert.strictEqual(squareDigitChains(100000), 85623);
```

يجب أن تُرجع الدالة `squareDigitChains(10000000)` القيمة `8581146`.
