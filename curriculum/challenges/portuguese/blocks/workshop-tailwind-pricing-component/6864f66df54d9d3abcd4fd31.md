...
Para o `span` dentro do elemento `p` que tem o texto `/month`, dê a ele as classes `text-base`, `font-medium` e `text-gray-500`.Isso o distinguirá do próprio preço tornando o texto mais claro.

# --hints--

Seu elemento `span` deve ter a classe `text-base`.
...
Seu elemento `span` deve ter a classe `font-medium`.
...
Seu elemento `span` deve ter a classe `text-gray-500`.
