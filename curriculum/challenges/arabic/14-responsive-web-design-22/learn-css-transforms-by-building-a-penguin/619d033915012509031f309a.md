...
يجب عليك إضافة عنصر `div` بداخل `.penguin-head`. متوقع عدد `--fcc-expected--` من عناصر `div`، لكن وجد `--fcc-actual--`.

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 3);
```

يجب عليك إعطاء عنصر `div` سمة `class` بقيمة `chin`.

```js
assert.exists(document.querySelector('.penguin-head > div.chin'));
```

يجب عليك وضع عنصر `div` تحت عنصر `.face.right`.
