...
`textContent` はノードのテキストを設定し、HTML要素のテキストコンテンツを設定または取得できます。
...
三項演算子を使って条件付きでテキストコンテンツの値を設定できます。以下は三項演算子の結果を変数に代入する例です。

```js
const example = condition ? "I'm true" : "I'm false";
```

三項演算子を使って`currentTitle`が真値かどうかをチェックしてください。真値なら`playingSong.textContent`に`currentTitle`を設定します。そうでなければ空文字列を設定してください。

その下では、`ternary`演算子を使って`currentArtist`が真値かどうかをチェックしてください。真値なら`songArtist.textContent`に`currentArtist`を設定します。そうでなければ空文字列を設定してください。

# --hints--

`textContent`プロパティを`playingSong`にチェーンしてください。

```js
assert.match(code, /playingSong\.textContent\s*/)
```

`ternary`演算子を使って`textContent`の`playingSong`プロパティを`currentTitle`か`""`に設定してください。

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

`textContent`プロパティを`songArtist`にチェーンしてください。

```js
assert.match(code, /songArtist\.textContent\s*/)
```

`ternary`演算子を使って`textContent`の`songArtist`プロパティを`currentArtist`か`""`に設定してください。
