...
`position` 속성을 `absolute`로, `top`를 `22px`로, `left`를 `-20px`로 설정하여 내부 귀를 위치시키세요.

# --hints--

`.cat-left-inner-ear` 선택자에 `position` 속성을 `absolute`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.position, 'absolute')
```

`.cat-left-inner-ear` 선택자에 `top` 속성을 `22px`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.top, '22px')
```

`.cat-left-inner-ear` 선택자에 `left` 속성을 `-20px`로 설정하세요.
