...
ここで色をつけましょう。`body` に `background-color` を `brown` に設定し、さらに `.card` に `background-color` を `white` に設定してください。

# --hints--

`body` 要素には `background-color` を `brown` に設定する必要があります。
...
`.card` セレクターを使っているはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card"));
```

`.card` 要素には `background-color` を `white` に設定してください。
