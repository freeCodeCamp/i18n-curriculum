...
接下來，添加一個 `else if` 語句來檢查左鍵是否被按下以及 `isCheckpointCollisionDetectionActive` 是否爲真。

在該條件內，添加一個 `forEach` 循環來遍歷 `platforms` 數組。

在循環內部，使用加法賦值運算符將 5 添加到平臺的 `x` 位置。

# --hints--

你應該有一個條件來檢查左鍵是否被按下以及 `isCheckpointCollisionDetectionActive` 是否爲真。
