...
Dovresti dare a `.face` una proprietà `background-color` con il valore `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Dovresti dare a `.chin` una proprietà `background-color` con il valore `var(--penguin-face)`.
