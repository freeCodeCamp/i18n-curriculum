...
Sasa, tumia tukio la `onload` kuondoa darasa la vitu la `hidden` wakati picha inapopakuliwa na ficha kiwakilishi cha upakuaji kwa kutumia `previousElementSibling` yake.

# --hints--

Kipengele chako cha `img` kinapaswa kuwa na mshughulikiaji wa `onload`.
...
Mshughulikiaji wako wa `onload` unapaswa kutumia `classList.remove` kuondoa darasa la vitu la `hidden`. Usisahau unahitaji neno kuu la `this` hapa.
...
Mshughulikiaji wako wa `onload` unapaswa kuweka `previousElementSibling.style.display` kuwa `'none'`. Usisahau unahitaji neno kuu la `this` kufanya hivi.
