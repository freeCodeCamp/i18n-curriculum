...
Ora tutte le funzionalità principali sono a posto. L’unico problema è che la canzone successiva non parte automaticamente quando quella in riproduzione finisce.
...
Aggiungi un event listener all’elemento `audio` che ascolti l’evento `"ended"`. Passa una callback usando la sintassi arrow con parentesi graffe vuote.
...
Dovresti usare la sintassi arrow per passare una callback vuota al tuo event listener `"ended"`.
