...
Quando for relevante, substitua os valores da propriedade pela variável `--penguin-face`.

# --hints--

Você deve dar a `.face` uma `background-color` de `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Você deve dar a `.chin` uma `background-color` de `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Você não deve usar `var(--penguin-face)` no seletor `.penguin-body`.
