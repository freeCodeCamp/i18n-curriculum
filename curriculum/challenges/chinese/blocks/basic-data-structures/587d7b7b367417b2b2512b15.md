...
我们已经定义了一个函数 `filteredArray`，它接受 `arr`（一个嵌套数组）和 `elem` 作为参数，并返回一个新数组。`elem` 表示一个可能存在于 `arr` 中一个或多个嵌套数组内的元素。使用 `for` 循环修改该函数，以返回传入数组的过滤版本，使得 `arr` 中任何包含 `elem` 的嵌套数组都被移除。
