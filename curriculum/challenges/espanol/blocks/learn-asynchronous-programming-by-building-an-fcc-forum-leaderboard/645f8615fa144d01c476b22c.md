...
Por último, tendrá que devolver la imagen para el avatar del usuario.

Comience agregando un `return` seguido por el establecimiento de plantillas literales. Dentro de la plantilla de literales, añade un elemento `img`. 

Dentro de la etiqueta `img`, añade un atributo `src` con el valor de `${userAvatarUrl}`. Para el atributo `alt`, añade un valor de `${user.name}`.
...
Debe devolver una plantilla literal.
...
Debes tener un elemento `img` dentro de la plantilla literal.
...
Debes tener un atributo `src` dentro de la etiqueta `img`.
...
Debe establecer el valor del atributo `src` a la constante llamada `userAvatarUrl`.
...
Debes tener un atributo `alt` después del atributo `src`.
...
Debe establecer el valor del atributo `alt` a la constante llamada `user.name`.
