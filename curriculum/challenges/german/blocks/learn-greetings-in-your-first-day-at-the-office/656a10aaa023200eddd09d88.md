---
id: 656a10aaa023200eddd09d88
title: Aufgabe 26
challengeType: 19
dashedName: task-26
lang: en-US
---

<!--
AUDIO REFERENCE:
Maria: Do you see the desk with a drawing tablet and a computer?
-->

# --description--

Ein `drawing tablet` ist ein Werkzeug, das Menschen mit Computern verwenden, um Dinge zu zeichnen oder zu gestalten.

# --questions--

## --text--

Was ist ein `drawing tablet`?

## --answers--

Ein Buch zum Zeichnen.

### --feedback--

Das ist ein Skizzenbuch, kein Zeichentablett.

---

Ein Werkzeug zum Streichen von Wänden.

### --feedback--

Dafür verwendet man Pinsel oder Farbroller, nicht Zeichentabletts.

---

Ein Werkzeug, das mit Computern zum Zeichnen verwendet wird.

---

Eine Art Stuhl.

### --feedback--

Mit Stühlen zeichnet man nicht. Denken Sie an ein Werkzeug, das mit Computern verwendet wird.

## --video-solution--

3

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Maria",
        "position": {"x":50,"y":0,"z":1.5},
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "1.1-1.mp3",
      "startTime": 1,
      "startTimestamp": 17.75,
      "finishTimestamp": 20.3
    }
  },
  "commands": [
    {
      "character": "Maria",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Maria",
      "startTime": 1,
      "finishTime": 3.4,
      "dialogue": {
        "text": "Do you see the desk with a drawing tablet and a computer?",
        "align": "center"
      }
    },
    {
      "character": "Maria",
      "opacity": 0,
      "startTime": 3.9
    }
  ]
}
```
