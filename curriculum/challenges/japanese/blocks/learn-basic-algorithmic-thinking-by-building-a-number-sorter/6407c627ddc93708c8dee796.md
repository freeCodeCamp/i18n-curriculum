...
`outputValueNode` という名前の変数を作成し、`document.getElementById()` メソッドを呼び出した結果を代入してください。テンプレートリテラル構文を使って、`` `output-value-${i}` `` string to `.getElementById()` を渡してください。

# --hints--

`const` を使って、`outputValueNode` のコールバック内で `.forEach()` 変数を宣言してください。

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=/)
```

`outputValueNode` には `document.getElementById()` を呼び出した値を代入してください。

```js
assert.match(code, /array\.forEach\s*\(\s*\(\s*num\s*,\s*i\s*\)\s*=>\s*\{\s*const\s+outputValueNode\s*=\s*document\.getElementById\s*\(/)
```

テンプレートリテラル構文を使って、`output-value-${i}` 文字列を `.getElementById()` に渡してください。
