...
أضف الآن لمحدِّد `100%` قاعدة `@keyframes wheel`. ضمنها، عيّن خاصية `transform` إلى `rotate(360deg)`. بهذا، ستُكمل الحركة دورانًا كاملاً.
...
يجب أن تحتوي خاصية `transform` في محدِّد `100%` على القيمة `rotate(360deg)`.
