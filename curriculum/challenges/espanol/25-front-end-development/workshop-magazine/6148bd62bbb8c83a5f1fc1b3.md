...
Crea un selector `.hero-title` y dale una propiedad `text-align` establecida en `center`, una propiedad `color` establecida en `orangered` y una propiedad `font-size` establecida en `8rem`.

# --hints--

Debes tener un selector `.hero-title`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-title'));
```

Tu selector `.hero-title` debe tener una propiedad `text-align` establecida en `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.textAlign, 'center');
```

Tu selector `.hero-title` debe tener una propiedad `color` establecida en `orangered`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-title')?.color, 'orangered');
```

Tu selector `.hero-title` debe tener una propiedad `font-size` establecida en `8rem`.
