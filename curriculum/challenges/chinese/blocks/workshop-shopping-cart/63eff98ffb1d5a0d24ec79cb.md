...
你还没有编写代码来生成 HTML，但如果产品已经添加到用户的购物车中，那么就会有一个你需要的匹配元素。

使用 `.getElementById()` 获取匹配元素 - 你将把 `id` 值设置为 `product-count-for-id${product.id}`，因此使用模板文字来查询该值。

将你的查询分配给 `currentProductCountSpan` 变量。

# --hints--

你应该声明一个 `currentProductCountSpan` 变量。
...
你应该使用 `const` 来声明 `currentProductCountSpan`。
...
你应该使用 `document.getElementById()` 来获取匹配元素。
...
你应该将 `document.getElementById()` 的值分配给 `currentProductCountSpan`。
