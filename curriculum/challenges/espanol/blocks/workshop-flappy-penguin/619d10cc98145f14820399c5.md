...
Debajo del elemento `.blush.right`, agregue dos elementos `div` cada uno con una `class` de `beak`. Además, asigna al primer elemento `.beak` una `class` de `top`, y al segundo elemento `.beak` un <code >class</code> de `bottom`.

# --hints--

Debes añadir dos elementos `div` dentro de `.penguin-head`. Se esperaban --fcc-expected-- elementos `div`, pero se encontraron --fcc-actual--.

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 9);
```

Debe dar al primer nuevo `div` una `class` de `beak`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

Debe dar al segundo nuevo `div` una `class` de `beak`.

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div.beak'), 2);
```

Debe dar al primer nuevo `div` una `class` de `top`.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

Debe dar al segundo nuevo `div` una `class` de `bottom`.
