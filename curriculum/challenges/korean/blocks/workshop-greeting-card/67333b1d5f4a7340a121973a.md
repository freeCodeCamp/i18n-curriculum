...
포커스된 `.card-links a` 요소를 대상으로 하는 새 선택자를 추가하세요. `outline` 속성을 `2px solid yellow`로 설정하세요.

# --hints--

`.card-links a:focus` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:focus"));
```

`.card-links a:focus` 선택자는 `outline` 속성을 `2px solid yellow`로 설정해야 합니다.
