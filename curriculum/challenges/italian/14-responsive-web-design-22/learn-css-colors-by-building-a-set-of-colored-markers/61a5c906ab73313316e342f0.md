...
Alleggerisci un altro po' il codice rimuovendo l'argomento `gradientDirection` da entrambe le funzioni `linear-gradient`.

# --hints--

Dovresti rimuovere gli argomenti `gradientDirection` dalle funzioni `linear-gradient` nelle regole CSS `.red` e `.green`.

```js
assert(!code.match(/linear-gradient\s*\(\s*180deg/gi));
```

La regola CSS `.red` dovrebbe avere una proprietà `background` con il valore `linear-gradient(rgb(122, 74, 14), rgb(245, 62, 113), rgb(162, 27, 27))`.
...
La regola CSS `.green` dovrebbe avere una proprietà `background` con il valore `linear-gradient(#55680D, #71F53E, #116C31)`.
