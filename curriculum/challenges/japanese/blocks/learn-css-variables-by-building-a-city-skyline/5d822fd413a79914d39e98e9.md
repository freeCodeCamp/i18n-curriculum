...
コードを最適化していくために、まず `position` と `top` のプロパティとその値を、 `.foreground-buildings` から `.background-buildings` へ移動し、 `.background-buildings` セレクタに加えて、 `.foreground-buildings` も対象とし、両方にスタイルが適用されるようにしてください。 コンマ(`,`) を使用して、`selector1, selector2` のようにセレクターを区切ることができます。
...
個別の `.background-buildings` セレクタを使用しないでください。
