...
創建一個名爲 `--penguin-skin` 的自定義 CSS 變量，並將其設置爲 `gray`。 然後，用它替換所有相關屬性值。

# --hints--

你應該給 `:root` 添加一個 `--penguin-skin` 屬性。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

你應該將 `--penguin-skin` 的值設置爲 `gray`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

你應該將 `.penguin-head` 的 `background` 設置爲 `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`。
...
你應該將 `.penguin-body::before` 的 `background-color` 設置爲 `var(--penguin-skin)`，而不是 `--fcc-actual--`。
...
你應該將 `.arm` 的 `background` 設置爲 `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`。
