...
`section` 요소에 대한 선택자를 만들고 다음을 지정하세요:

- `margin` 속성을 `20px auto`로 설정,
- `max-width`를 `600px`로 설정,
- `background-color` 속성을 `whitesmoke`로 설정,
- `padding` 속성을 `20px`로 설정,
- `border-radius` 속성을 `10px`로 설정

# --hints--

`section` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle("section"));
```

`section` 선택자는 `margin` 속성을 `20px auto`로 설정해야 합니다.
