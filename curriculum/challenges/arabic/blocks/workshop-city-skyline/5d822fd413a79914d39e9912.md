...
امنح `.fb1b` عرضًا (`width`) بنسبة `60%` وارتفاعًا (`height`) بنسبة `10%`، و`.fb1c` عرضًا (`width`) بنسبة `100%` وارتفاعًا (`height`) بنسبة `80%`.

# --hints--

يجب أن تعطي `.fb1b` عرضًا (`width`) بنسبة `60%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.width, "60%");
```

يجب أن تعطي `.fb1b` ارتفاعًا (`height`) بنسبة `10%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1b")?.height, "10%");
```

يجب أن تعطي `.fb1c` عرضًا (`width`) بنسبة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb1c")?.width, "100%");
```

يجب أن تعطي `.fb1c` ارتفاعًا (`height`) بنسبة `80%`.
