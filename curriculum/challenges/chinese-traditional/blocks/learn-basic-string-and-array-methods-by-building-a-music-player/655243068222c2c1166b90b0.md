...
在 `playNextSong` 函式（程式）內，創建一個 `if` 述語來檢查 `userData` 的 `currentSong` 是否嚴格相等於 `null`。這將檢查 `userData` 物件中是否沒有正在播放的歌曲。 

如果條件為真，則以 `userData?.songs` 陣列中第一首歌曲的 `id` 作為引數，呼叫 `playSong` 函式。

# --hints--

你不應該修改現有的 `playNextSong` 函式（程式）及其內容。

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*.*\s*\}\s*;?/)
```

你應該使用條件 `userData?.currentSong === null` 創建一個 `if` 述語。
...
你應該在 `if` 述語內使用 `userData?.songs[0].id` 來呼叫 `playSong` 函式。
