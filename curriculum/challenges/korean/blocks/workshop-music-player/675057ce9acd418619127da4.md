...
이제 `getPreviousSong`라는 함수를 만드세요. 이 함수는 `getCurrentSongIndex()`에서 `userData.songs`보다 하나 적은 인덱스에 있는 노래를 반환하거나, 현재 노래가 재생 목록의 첫 번째 노래일 경우 `undefined`을 반환하게 하세요.

# --hints--

`getPreviousSong`라는 함수가 있어야 합니다.

```js
assert.isFunction(getPreviousSong);
```

`getPreviousSong` 함수는 `getCurrentSongIndex`를 호출해야 합니다.
...
`getPreviousSong` 함수는 현재 노래 이전의 노래를 반환해야 합니다.
...
`getPreviousSong` 함수는 현재 노래가 재생 목록의 첫 번째 노래일 때 `undefined`를 반환해야 합니다.
