...
ペンギンの顔を作り始めましょう。`div`の中に`.penguin-head`要素を2つ追加し、両方に`class`の`face`を設定してください。

# --hints--

`--fcc-expected--`に`div`の`.penguin-head`要素を追加する必要がありますが、`--fcc-actual--`が見つかりました。

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 2);
```

最初の`div`に`class`の`face`を設定する必要がありますが、`--fcc-actual--`が見つかりました。

```js
assert.include(document.querySelector('.penguin-head > div:nth-of-type(1)')?.className, 'face');
```

2番目の`div`に`class`の`face`を設定する必要がありますが、`--fcc-actual--`が見つかりました。
