...
在 map 回调函数中，传入 `platform` 作为参数，并隐式返回创建一个新的 `Platform` 实例，并将 `platform.x` 和 `platform.y` 值作为参数传入。

# --hints--

你的 `map` 回调应隐式返回新的 `Platform` 实例的创建，并将 `platform.x` 和 `platform.y` 值作为参数传入。
