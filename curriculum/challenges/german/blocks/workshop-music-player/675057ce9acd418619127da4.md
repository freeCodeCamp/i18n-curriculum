...
Erstellen Sie nun eine Funktion namens `getPreviousSong`. Sie soll das Lied zurückgeben, das sich an der Position `getCurrentSongIndex()` minus eins in `userData.songs` befindet, oder `undefined`, wenn das aktuelle Lied das erste in der Wiedergabeliste ist.

# --hints--

Sie sollten eine Funktion namens `getPreviousSong` haben.

```js
assert.isFunction(getPreviousSong);
```

Ihre `getPreviousSong`-Funktion sollte `getCurrentSongIndex` aufrufen.

```js
assert.match(getPreviousSong.toString(), /getCurrentSongIndex\s*\(\s*\)/);
```

Ihre `getPreviousSong`-Funktion sollte das Lied vor dem aktuellen Lied zurückgeben.
...
Ihre `getPreviousSong`-Funktion sollte `undefined` zurückgeben, wenn das aktuelle Lied das erste in der Wiedergabeliste ist.
