...
A próxima coisa que você mostrará são os detalhes biográficos sobre o autor. Você pode fazer isso com `bio`, que você desestruturou anteriormente.

Adicione um elemento de parágrafo com a `class` `"bio"`, depois interpole `bio` dentro do elemento de parágrafo.
...
Seu elemento `p` deve ter a classe `"bio"`

```js
assert.include(document.querySelector('p')?.className, 'bio');
```

Você deve interpolar `bio` no elemento `p`.
