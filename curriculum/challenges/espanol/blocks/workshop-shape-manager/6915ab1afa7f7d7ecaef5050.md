...
Para finalmente mostrar los cálculos, recorre el objeto `propertyInputs` usando un bucle `for...of` con `Object.entries()`. Puedes ignorar la clave usando una coma antes de `input` en la declaración del bucle.

# --hints--

Debes crear un bucle `for...of` que desestructure cada entrada de `Object.entries(propertyInputs)` saltando la clave y manteniendo solo `input`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Tu bucle `for...of` debe iterar sobre `Object.entries(propertyInputs)`.
