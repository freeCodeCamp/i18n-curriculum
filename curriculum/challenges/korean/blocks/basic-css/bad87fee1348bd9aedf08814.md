...
현재 고양이 사진은 모서리가 뾰족합니다. CSS 속성인 `border-radius`을 사용해 모서리를 둥글게 만들 수 있습니다.

# --instructions--

픽셀 단위로 `border-radius`을 지정할 수 있습니다. 고양이 사진에 `border-radius`를 `10px`으로 설정하세요.

**참고:** 이 도전 과제는 여러 가지 해결 방법이 가능합니다. 예를 들어, `border-radius`을 `.thick-green-border` 클래스나 `.smaller-image` 클래스 중 어느 쪽에 추가해도 됩니다.

# --hints--

이미지 요소에 `thick-green-border` 클래스를 지정해야 합니다.

```js
assert.isTrue(document.querySelector('img').classList.contains('thick-green-border'));
```

이미지에 `10px`의 테두리 반지름이 있어야 합니다.
