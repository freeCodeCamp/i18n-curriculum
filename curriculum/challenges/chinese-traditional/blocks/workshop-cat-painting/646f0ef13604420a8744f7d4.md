...
在 `.cat-whiskers` 元素內，創建兩個具有類 `cat-whiskers-left` 和 `cat-whiskers-right` 的 `div` 元素。

# --hints--

你不應該更改現有的類爲 `cat-whiskers` 的 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers'), 1);
```

你應該在 `.cat-whiskers` 元素中放置兩個 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers div'), 2);
```

第一個 `div` 元素在 `.cat-whiskers` 元素內，並且具有類 `cat-whiskers-left`。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers div')[0].classList.contains('cat-whiskers-left'));
```

第二個 `div` 元素應位於 `.cat-whiskers` 元素內，並且具有類 `cat-whiskers-right`。
