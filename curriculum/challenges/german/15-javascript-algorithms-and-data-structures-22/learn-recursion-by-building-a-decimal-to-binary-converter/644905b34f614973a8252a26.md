...
Inside your `checkUserInput` function, use an `if` statement to check if the `value` of `numberInput` is equal to an empty string. Lass den Inhalt von der `if` Anweisung im Moment leer.
