...
Your `img` element's `alt` attribute value is set to something other than `A cute orange cat lying on its back`. Stelle sicher, dass der `alt`-Attributwert von Anführungszeichen umgeben ist.
