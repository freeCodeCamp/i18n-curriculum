...
下一步是将 `width` 和 `height` 添加到 `CheckPoint` 类。

`width` 和 `height` 应分别为 `proportionalSize(40)` 和 `proportionalSize(70)`。

# --hints--

你应该在 `CheckPoint` 类中有一个 `width` 属性。
...
你应该在 `CheckPoint` 类中有一个 `height` 属性。
...
你应该将 `width` 属性分配给 `proportionalSize(40)`。
...
你应该将 `height` 属性分配给 `proportionalSize(70)`。
