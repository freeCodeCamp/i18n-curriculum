...
為 PAS `input` 添加一個 `value` 屬性，值為 `player.pas`。接著添加一個 `onChange` 屬性，設定為一個箭頭函式，該函式以 `e` 作為參數，並呼叫 `setPlayer`，傳入展開的 `player` 和 `pas`，其中 `pas` 設為 `Number(e.target.value)`。

# --hints--

你的 PAS `input` 應該有一個 `value`，其值與玩家的 `pas` 狀態相等。
...
你的 PAS `input` 應該有一個 `onChange` 處理常式，該處理常式呼叫 `setPlayer`，並傳入展開的 `player` 和 `pas`，其中 `pas` 設為 `Number(e.target.value)`。
