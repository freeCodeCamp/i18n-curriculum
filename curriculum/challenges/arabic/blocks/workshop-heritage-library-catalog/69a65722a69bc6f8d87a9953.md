...
CSV (القيم المفصولة بفواصل) هو تنسيق يُستخدم لجداول البيانات واستيراد البيانات. كل سطر يُمثل إدخالًا واحدًا مع حقول مفصولة بفواصل.

ابدأ بحذف استدعاء `console.log(exportToJSON(...))` لأنه كان مطلوبًا فقط للاختبار. ثم أنشئ دالة باسم `exportToCSV` تأخذ `catalog` كمعلمة. داخلها، أعلن `const header = "Title,Author,Year,Location"` و `const rows = []`.

# --hints--

يجب حذف استدعاء `exportToJSON` في وحدة التحكم.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\.log\s*\(\s*exportToJSON/);
```

يجب أن يكون لديك دالة باسم `exportToCSV` مع معلمة `catalog`.
...
يجب إعلان متغير باسم `header` بقيمة `"Title,Author,Year,Location"`.

```js
assert.match(exportToCSV.toString(), /Title,Author,Year,Location/);
```

يجب إعلان متغير باسم `rows` وتعيينه إلى مصفوفة فارغة.
