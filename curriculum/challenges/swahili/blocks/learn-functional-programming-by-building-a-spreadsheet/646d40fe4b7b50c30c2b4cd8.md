...
Kitendakazi chako cha `infixEval` kitahesabu tu operesheni ya kwanza ya kuzidisha au kugawanya, kwa sababu `regex` si ya kimataifa. Hii ina maana unahitaji kutumia mbinu ya kurudia ili kuhakiki mfuatano mzima wa herufi.

Ikiwa `infixEval` haitapata mechi yoyote, itarudisha thamani ya `str` kama ilivyo. Kwa kutumia usemi wa kikokotoo cha masharti cha pande tatu, angalia kama `str2` ni sawa na `str`. Ikiwa ni sawa, rudisha `str`, vinginevyo rudisha matokeo ya wito wa `highPrecedence()` kwa `str2`.

# --hints--

Kitendakazi chako cha `highPrecedence` kinapaswa kutumia neno kuu la `return`.
...
Unapaswa kutumia neno kuu la `return` pamoja na sharti la kuangalia kama `str` ni sawa na `str2`.
...
Unapaswa kutumia sintaksia ya kikokotoo cha masharti cha pande tatu na tamko lako la `return`.
...
Ikiwa sharti la kikokotoo cha masharti cha pande tatu ni kweli, unapaswa kurudisha `str`.
...
Ikiwa sharti la kikokotoo cha masharti cha pande tatu ni si kweli, unapaswa kurudisha matokeo ya wito wa `highPrecedence()`.
...
Unapaswa kupitisha `str2` kwa wito wako wa `highPrecedence()`.
