...
Usa la stessa sintassi della tua funzione `addCharacters` per aggiornare la tua funzione `elemValue`. Non dovrebbe più dichiarare `inner`, ma dovrebbe restituire la funzione implicitamente.

# --hints--

La tua funzione `elemValue` dovrebbe usare un ritorno implicito.
...
La tua funzione `elemValue` dovrebbe restituire implicitamente una funzione freccia con un parametro `character`.
...
La tua funzione freccia interna dovrebbe usare un ritorno implicito.
...
La tua funzione freccia interna dovrebbe restituire il risultato della chiamata a `idToText()` con `character + num` come argomento.
