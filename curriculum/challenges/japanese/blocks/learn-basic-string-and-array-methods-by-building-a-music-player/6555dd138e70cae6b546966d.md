...
現在再生中の曲か、プレイリストの最初の曲を取得する必要があります。そのために、`song` 定数を作成し、OR演算子（`||`）を使って `userData` の現在の曲、**または** `userData?.songs` 配列の最初の曲に設定してください。

オプショナルチェーンを使うのを忘れないでください。

# --hints--

既存の `setPlayButtonAccessibleText` とその内容は変更しないでください。

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

`userData?.currentSong` または `userData?.songs[0]` にアクセスしてください。

```js
assert.match(code, /userData\?\.currentSong\s*\|\|\s*userData\?\.songs\s*\[\s*0\s*\]\s*;?/)
```

`userData?.currentSong || userData?.songs[0]` を `song` 定数に代入してください。
