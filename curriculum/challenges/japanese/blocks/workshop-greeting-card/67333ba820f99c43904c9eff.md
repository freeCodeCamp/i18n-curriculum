...
すでに訪問済みの `.card-links a` 要素を対象とする新しいセレクターを作成してください。プロパティ `color` を `crimson` に設定してください。

# --hints--

`.card-links a:visited` セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:visited"));
```

`.card-links a:visited` セレクターにはプロパティ `color` が `crimson` に設定されている必要があります。
