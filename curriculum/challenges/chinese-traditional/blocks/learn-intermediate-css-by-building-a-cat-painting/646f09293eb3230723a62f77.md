...
使用類別選取器存取你的 `.cat-mouth-line-right` 元素，然後將它移動到正確位置，設定 `position` 為 `absolute`，`top` 為 `88px`，以及 `left` 為 `91px`。

# --hints--

你應該有一個 `.cat-mouth-line-right` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-right'))
```

你的 `.cat-mouth-line-right` 選擇器應該有一個設定為 `absolute` 的 `position` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-right')?.position === 'absolute')
```

你的 `.cat-mouth-line-right` 選擇器應該有一個 `top` 屬性設定為 `88px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-right')?.top === '88px')
```

你的 `.cat-mouth-line-right` 選擇器應該有一個 `left` 屬性設定為 `91px`。
