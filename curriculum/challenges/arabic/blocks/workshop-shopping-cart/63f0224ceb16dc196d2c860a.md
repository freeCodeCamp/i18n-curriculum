...
تحتاج إلى الحصول على جميع أزرار `Add to cart` التي أضفتها إلى DOM سابقًا. أعلن عن متغير `addToCartBtns`، وقم بتعيين القيمة الناتجة عن استدعاء طريقة `getElementsByClassName()` على كائن `document`، مع تمرير السلسلة النصية `"add-to-cart-btn"`.
...
يجب استدعاء طريقة `getElementsByClassName()` على كائن `document`.

```js
assert.match(code, /document\s*\.\s*getElementsByClassName\s*\(/);
```

يجب تمرير السلسلة النصية `"add-to-cart-btn"` إلى طريقة `getElementsByClassName()`.

```js
assert.match(code, /document\s*\.\s*getElementsByClassName\s*\(\s*('|"|`)add-to-cart-btn\1\s*\)/);
```

يجب تعيين القيمة التي تُرجعها طريقة `getElementsByClassName()` إلى المتغير `addToCartBtns`.
