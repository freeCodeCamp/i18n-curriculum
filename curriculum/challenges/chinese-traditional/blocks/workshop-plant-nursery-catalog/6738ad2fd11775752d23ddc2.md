...
`Set` 建構式也接受一個可迭代物件作為它的引數，並從中建立一個集合：
...
要完成此工作坊，請修改你的 `displayPlantsSet`，以傳回你目錄中獨特植物俗名的集合。

為了達成此目的，創建一個陣列，該陣列包含作為 `catalog` 映射鍵所儲存的每個植物物件的 `commonName`。將該陣列傳遞給 `Set` 建構式，並從函式中傳回該集合。

# --hints--

你的 `displayPlantsSet` 函式應該傳回一個集合，該集合包含你目錄中每株植物的 `commonName`。
