...
`infixToFunction[operator]` は関数を返します。その関数を直接呼び出し、`arg1` と `arg2` を引数として渡してください。

# --hints--

コールバック関数は `infixToFunction[operator]` を呼び出した結果を返す必要があります。
...
`arg1` を `infixToFunction[operator]` の呼び出しの最初の引数として渡してください。
...
`arg2` を `infixToFunction[operator]` の呼び出しの2番目の引数として渡してください。
