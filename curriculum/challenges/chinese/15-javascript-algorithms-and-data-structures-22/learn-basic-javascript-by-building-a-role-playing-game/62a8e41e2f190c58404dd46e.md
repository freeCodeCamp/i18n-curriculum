...
在新行上，使用加法赋值运算符 (`+=`)，将字符串 `" You attack it with your <weapon>."` 添加到 `text` 值，并用玩家当前的武器替换 `<weapon>`。 此外，请记住此行文本以空格开头，以便能够正确显示。

# --hints--

你应该使用点符号在新行上访问 `text` 的 `innerText` 属性。
...
你应该使用带有 `text.innerText` 的复合赋值。

```js
assert.match(attack.toString(), /text\.innerText\s*\+=/);
```

你应该将字符串 `" You attack it with your "` 添加到 `text.innerText` 值。 请记住间距很重要。

```js
assert.match(attack.toString(), /text\.innerText\s*\+=\s*('|") You attack it with your \1/);
```

你应该使用连接运算符将当前武器添加到字符串中。 你可以使用 `weapons[currentWeaponIndex].name` 获取当前武器。
