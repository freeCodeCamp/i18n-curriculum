...
현재 재생 중인 노래를 일시 정지하는 작업을 해야 합니다.

`pauseSong` 키워드와 화살표 함수 구문을 사용해 `const` 함수를 정의하세요. 이 함수는 매개변수를 받지 않아야 합니다.

# --hints--

빈 함수를 만들려면 `const`를 사용해 `pauseSong`라는 이름의 함수를 만드세요.

```js
assert.match(code, /const\s+pauseSong\s*=\s*/)
```

`pauseSong`는 함수여야 합니다.

```js
assert.isFunction(pauseSong)
```

`pauseSong` 함수는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*/)
```

`pauseSong` 함수는 비어 있어야 합니다.
