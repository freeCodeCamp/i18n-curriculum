...
`__init__` メソッドが完成したので、`series1` ブロック内に `try` という名前の変数を作成してください。任意の引数を渡して `TVSeries` のインスタンスを代入します。その後、`series1` をコンソールに出力してください。

# --hints--

`TVSeries` ブロック内で `series1` のインスタンスを `try` という名前の変数に代入する必要があります。
