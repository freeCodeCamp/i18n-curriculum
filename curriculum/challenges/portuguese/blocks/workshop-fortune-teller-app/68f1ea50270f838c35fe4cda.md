...
Agora faça o mesmo para `subTitle` atribuindo-o a `meaning_up`, e para `title` atribua-o a `foundCard.name`. Depois disso, chame o método `showElements` e passe `fortuneDescription` dos elementos.

# --hints--

Você deve atribuir `this.elements.subTitle.textContent` a `foundCard.meaning_up`.
...
Você deve atribuir `this.elements.title.textContent` a `foundCard.name`.
...
Você deve chamar a função `showElements` dentro de `showFortune`.
...
Você deve passar `this.elements.fortuneDescription` como argumento para `showElements`.
