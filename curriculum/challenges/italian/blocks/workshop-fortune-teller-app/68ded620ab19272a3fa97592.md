...
Dato che il progetto ora funziona con elementi HTML DOM, ti serve un modo per renderizzare la fortuna nel DOM.

Dichiara una funzione chiamata `renderCard` che prende quattro argomenti: `drawingType`, `isReversed`, `shortName` e `img`, e restituisce un template letterale.

I parametri `drawingType`, `shortName` e `img` devono essere di tipo `string`, mentre `isReversed` deve essere di tipo `boolean`. La funzione dovrebbe avere un tipo di ritorno `string`.

# --hints--

Dovresti avere una funzione chiamata `renderCard`.
...
La tua funzione `renderCard` dovrebbe avere quattro parametri.
...
Il tuo primo parametro dovrebbe essere `drawingType` di tipo `string`.
...
Il tuo secondo parametro dovrebbe essere `isReversed` di tipo `boolean`.
...
Il tuo terzo parametro dovrebbe essere `shortName` di tipo `string`.
...
Il tuo quarto parametro dovrebbe essere `img` di tipo `string`.
...
La tua funzione `renderCard` dovrebbe avere un tipo di ritorno `string`.
...
La tua funzione `renderCard` dovrebbe restituire un template literal.
