...
في عنصر `tr` الرابع، أضف عنصر `th` يحتوي على النص `Total Liabilities`. غلف النص `Liabilities` داخل عنصر `span` مع تعيين خاصية `class` إلى `sr-only`.

بعد ذلك، أضف ثلاثة عناصر `td` بالنصوص التالية (بالترتيب): `$750`، `$600`، `$475`. اجعل عنصر `td` الثالث يحمل خاصية `class` بقيمة `current`.
...
يجب أن يحمل عنصر `span` خاصية `class` بقيمة `sr-only`.
