...
Ahora que tienes una expresión regular para hacer coincidir la multiplicación o la división, puedes evaluar esa expresión.

Declara una variable `str2` y asígnale el resultado de llamar a `infixEval` con `str` y `regex` como argumentos.

# --hints--

Debes declarar una variable `str2`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*(?:const|let|var)\s+str2/);
```

Debes usar `const` para declarar tu variable `str2`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2/);
```

Debes asignar a `str2` el resultado de llamar a tu función `infixEval`.

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>\s*{\s*const\s+regex\s*=\s*\/\(\[(?:\\d\.|\.\\d)\]\+\)\(\[(?:\*\\\/|\\\/*)\]\)\(\[(?:\\d\.|\.\\d)\]\+\)\/\s*;?\s*const\s+str2\s*=\s*infixEval\(/);
```

Debes pasar `str` como el primer argumento de tu llamada a `infixEval`.
...
Debes pasar `regex` como el segundo argumento de tu llamada a `infixEval`.
