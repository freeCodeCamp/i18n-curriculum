...
Kama ilivyo awali, unapaswa kuthibitisha ingizo lako. Hasa, ikiwa `size` ni ndogo kuliko `1`, unapaswa kuandika `"Chunk size must be >= 1"` kwenye konsoli kisha ufanye wito wa `return` ili kutoka kwenye kitendakazi.

# --hints--

Ikiwa ingizo la `size` haliko sahihi, unapaswa kuandika ujumbe wa kosa ufuatao kwenye konsoli, `"Chunk size must be >= 1"`.
...
Baada ya kuandika ujumbe wa kosa, unapaswa mara moja kufanya wito wa `return` ili kutoka mapema kwenye kitendakazi.
...
Kitendakazi chako cha `chunkCrew` kinapaswa kupita uthibitisho bila makosa wakati `size` ni kubwa au sawa na 1.
