...
在你最後一個 `p` 元素下方，創建另一個 `p` 元素，內容為 `Sodium 160mg 7%`。將 `Sodium` 和 `7%` 各自放入自己的 `span`，並且像其他一樣給予 `bold` 的類別。

然後，再為 `Sodium 160mg` 添加一個額外的 `span` 元素以重新對齊。
...
`Sodium` 周圍的 `span` 應該將 `class` 屬性設為 `bold`。
...
`7%` 周圍的 `span` 元素應該設定 `class` 屬性為 `bold`。
