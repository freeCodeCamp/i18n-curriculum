...
Dovresti creare un nuovo selettore `25%` nella regola `@keyframes cabins`.
...
Il selettore `25%` dovrebbe trovarsi tra i selettori `0%` e `50%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '25%');
```

Il selettore `25%` dovrebbe avere una proprietà `background-color` impostata su `yellow`.
