...
드디어! 마지막 건물에 도착했습니다! `90deg` 방향으로 반복되는 그라디언트를 추가하세요. `0%`에서 `10%`까지는 건물 색상을, `transparent`에서 `10%`까지는 `30%` 색상을 사용하세요.

# --hints--

`repeating-linear-gradient` 속성에 `.fb6`를 `background`로 추가해야 합니다.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

`repeating-linear-gradient`에 `90deg` 방향을 지정해야 합니다.

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

`repeating-linear-gradient`에 `--building-color3`에서 `0%`까지 `10%`를 첫 번째 색상으로 지정해야 합니다.
...
`repeating-linear-gradient`에 `transparent`에서 `10%`까지 `30%`를 두 번째 색상으로 지정해야 합니다.
