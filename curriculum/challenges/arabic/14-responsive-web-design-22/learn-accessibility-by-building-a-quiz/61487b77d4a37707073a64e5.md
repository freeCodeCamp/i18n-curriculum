...
عندما يكون عرض الشاشة صغيراً، فإن `h1` لا يغلف محتوى نصه كما ينبغي. لتغلفه العنصر كما ينبغي يحاذ (align) كتابة في العنصر `h1` إلى المركز (center).

ثم اعطي `main` خاصية padding بحيث يمكن رؤية section header من `Student Info` بشكل كامل.

# --hints--

يجب عليك إعطاء `h1` خاصية `text-align` بقيمة `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

يجب عليك إضافة منتقي `main` لاستهداف عنصر `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

يجب عليك إعطاء `main` خاصية `padding-top` بقيمة `25px` علي الأقل.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

يجب عليك فقط تغيير قيمة `padding-top`.
