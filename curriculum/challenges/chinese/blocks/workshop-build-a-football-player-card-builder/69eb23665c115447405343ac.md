...
每个 `stat-row` 应该包含两个 `span` 元素，分别具有 `className` 和 `stat-value` 的 `stat-label`。

在 `spans` 元素内的 `div` 元素中，你应该有以下内容：

- `99` 和 `DRI`
- `41` 和 `DEF`
- `75` 和 `PHY`

`DRI` 代表运球，`DEF` 代表防守，`PHY` 代表体能。

# --hints--

你应该将第二个 `.stat-row` 中的第一个 `.stat-col` 设置为值 `99` 和标签 `DRI`。
...
你应该将第二个 `.stat-row` 中的第二个 `.stat-col` 设置为值 `41` 和标签 `DEF`。
...
你应该将第二个 `.stat-row` 中的第三个 `.stat-col` 设置为值 `75` 和标签 `PHY`。
