...
Seleziona tutti gli elementi discendenti dell'elemento `.penguin` e assegna loro un `position` di `absolute`.
...
Dovresti assegnare a `.penguin *` un `position` di `absolute`.
