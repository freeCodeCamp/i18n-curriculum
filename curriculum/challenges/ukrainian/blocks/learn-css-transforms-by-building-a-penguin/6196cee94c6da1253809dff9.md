...
Знайдіть елемент `.back-mountain` і встановіть його `width` та `height` у значення `300px`. Потім встановіть `background` у лінійний градієнт, який починається з `rgb(203, 241, 228)` і закінчується `rgb(47, 170, 255)`.
...
Ви повинні задати `.back-mountain` властивість `width` зі значенням `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

Ви повинні задати `.back-mountain` властивість `height` зі значенням `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

Ви повинні задати `.back-mountain` властивість `background` зі значенням `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`.
