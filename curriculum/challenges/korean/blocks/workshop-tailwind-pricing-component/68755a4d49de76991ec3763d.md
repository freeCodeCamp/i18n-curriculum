...
이제 `span` 요소 안의 `p`에 `text-base`, `font-medium`, `text-gray-500` 클래스를 할당하세요.

# --hints--

`span` 요소 안의 `p`는 `text-base` 클래스를 가져야 합니다.
...
`span` 요소 안의 `p`는 `font-medium` 클래스를 가져야 합니다.
...
`span` 요소 안의 `p`는 `text-gray-500` 클래스를 가져야 합니다.
