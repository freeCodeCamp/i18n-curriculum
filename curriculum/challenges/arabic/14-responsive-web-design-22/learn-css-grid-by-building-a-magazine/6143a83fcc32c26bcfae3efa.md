...
يجب أن يحتوي عنصرك `article` على `class` يسمى `brief-history`.

```js
assert(document.querySelector('.text-with-images article')?.className === 'brief-history');
```

يجب أن يحتوي عنصرك `aside` على `class` يسمى `image-wrapper`.
