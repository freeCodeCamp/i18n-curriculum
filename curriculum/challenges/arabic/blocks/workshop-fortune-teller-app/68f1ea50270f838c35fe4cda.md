...
افعل الشيء نفسه مع `subTitle` بتعيينه إلى `meaning_up`، ومع `title` عيّنها إلى `foundCard.name`. بعد ذلك، استدعِ الطريقة `showElements` ومرر `fortuneDescription` من العناصر.

# --hints--

يجب تعيين `this.elements.subTitle.textContent` إلى `foundCard.meaning_up`.
...
يجب تعيين `this.elements.title.textContent` إلى `foundCard.name`.
...
يجب استدعاء الدالة `showElements` داخل `showFortune`.
...
يجب تمرير `this.elements.fortuneDescription` كمعلمة إلى `showElements`.
