...
Ähnlich wie bei Flexbox können Sie mit CSS Grid den Inhalt von Grid-Elementen mit `align-items` und `justify-items` ausrichten. `align-items` richtet Kindelemente entlang der Spaltenachse aus, und `justify-items` richtet Kindelemente entlang der Zeilenachse aus.

Geben Sie dem `.social-icons`-Selektor eine `align-items`-Eigenschaft mit dem Wert `center`.

# --hints--

Ihr `.social-icons`-Selektor sollte eine `align-items`-Eigenschaft mit dem Wert `center` haben.
