...
现在你需要在具有 `targetId` 的元素中定位 `.input-container` 元素。 声明一个新的 `targetInputContainer` 变量，并为其分配 `document.querySelector()` 的值。 使用连接符将 `targetId` 和 `'.input-container'` 用空格分隔，并将该字符串传递给 `querySelector()`。

# --hints--

你的 `addEntry` 函数应该有一个 `targetInputContainer` 变量。

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=/);
```

你的 `targetInputContainer` 变量应设置为 `document.querySelector()`。

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=\s*document\.querySelector\(/);
```

你应该将 `targetId` 传递给你的 `querySelector()` 方法。

```js
assert.match(addEntry.toString(), /targetInputContainer\s*=\s*document\.querySelector\(\s*targetId\s*/);
```

你应该将 `' .input-container'` 连接到 `targetId`。 请记住在 `.input-container` 的开头包含空格。
