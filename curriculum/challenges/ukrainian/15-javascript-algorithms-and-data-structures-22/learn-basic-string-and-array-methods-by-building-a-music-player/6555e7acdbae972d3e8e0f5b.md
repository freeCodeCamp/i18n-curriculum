...
Додайте слухач події `"click"` до елемента `shuffleButton`. Щоб функція працювала передайте функцію `shuffle`.

**Примітка**: вам не потрібен зворотний виклик у цьому конкретному слухачі подій. Вам також не потрібно викликати функцію `shuffle`, просто передайте її ідентифікатор.

# --hints--

Під’єднайте метод `addEventListener()` до `shuffleButton`.
...
Передайте `shuffle` як друге значення слухача подій `"click"`.
