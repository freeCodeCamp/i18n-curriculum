---
id: 699c8e045ee7cb94ed2322d7
title: "Aufgabe 216: Pi-Tag"
challengeType: 28
dashedName: challenge-216
---

# --description--

Einen frohen Pi (π)-Tag!

Gegeben ist eine ganze Zahl (`n`), wobei `n` zwischen 1 und 1000 (einschließlich) liegt. Geben Sie die `n`te Nachkommastelle von π zurück.

- Stellen Sie sicher, dass Sie eine Zahl und keinen String zurückgeben.

π mit seinen ersten fünf Nachkommastellen ist 3,14159. Wenn zum Beispiel `5` gegeben ist, geben Sie `9` zurück, die fünfte Nachkommastelle.

Sie müssen möglicherweise die ersten 1000 Nachkommastellen von π irgendwo finden.

# --hints--

`getPiDecimal(5)` sollte `9` zurückgeben.

```js
assert.strictEqual(getPiDecimal(5), 9);
```

`getPiDecimal(10)` sollte `5` zurückgeben.

```js
assert.strictEqual(getPiDecimal(10), 5);
```

`getPiDecimal(22)` sollte `6` zurückgeben.

```js
assert.strictEqual(getPiDecimal(22), 6);
```

`getPiDecimal(39)` sollte `7` zurückgeben.

```js
assert.strictEqual(getPiDecimal(39), 7);
```

`getPiDecimal(76)` sollte `2` zurückgeben.

```js
assert.strictEqual(getPiDecimal(76), 2);
```

`getPiDecimal(384)` sollte `4` zurückgeben.

```js
assert.strictEqual(getPiDecimal(384), 4);
```

`getPiDecimal(601)` sollte `0` zurückgeben.

```js
assert.strictEqual(getPiDecimal(601), 0);
```

`getPiDecimal(1000)` sollte `9` zurückgeben.

```js
assert.strictEqual(getPiDecimal(1000), 9);
```

# --seed--

## --seed-contents--

```js
function getPiDecimal(n) {

  return n;
}
```

# --solutions--

```js
function getPiDecimal(n) {
  const pi = ""

  return Number(pi[n]);
}
```
