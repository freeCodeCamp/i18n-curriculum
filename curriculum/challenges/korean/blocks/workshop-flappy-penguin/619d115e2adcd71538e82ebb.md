...
`.beak` 요소를 선택하고, `height`을 `10%`로, `background-color`을 `orange`로 설정하며, 모든 모서리에 `50%`의 반경을 주세요.
...
`.beak`에 `height`을 `--fcc-expected--`로 설정하세요. `--fcc-actual--`에서 찾았습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.height, '10%');
```

`.beak`에 `background-color`을 `--fcc-expected--`로 설정하세요. `--fcc-actual--`에서 찾았습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.backgroundColor, 'orange');
```

`.beak`에 `border-radius`을 `--fcc-expected--`로 설정하세요. `--fcc-actual--`에서 찾았습니다.
