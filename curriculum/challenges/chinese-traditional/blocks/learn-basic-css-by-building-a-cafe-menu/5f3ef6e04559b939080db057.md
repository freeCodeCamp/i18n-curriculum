...
由於選單的所有 `4` 邊具有相同的內部間距，請移除這四個屬性並使用單一 `padding` 屬性，值為 `20px`。

# --hints--

你應該移除 `padding-left` 屬性。

```js
assert(!code.match(/padding-left/i));
```

你應該移除 `padding-right` 屬性。

```js
assert(!code.match(/padding-right/i));
```

你應該移除 `padding-top` 屬性。

```js
assert(!code.match(/padding-top/i));
```

你應該移除 `padding-bottom` 屬性。

```js
assert(!code.match(/padding-bottom/i));
```

你應該將 `padding` 屬性設為 `20px`。
