...
`.message` 요소에 스타일을 지정해야 합니다. 다음을 적용하세요:

- `font-size` 값을 가진 `1.2em`,
- `margin-bottom` 값을 가진 `20px`.

# --hints--

`.message` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".message"));
```

`.message` 선택자에 `font-size` 속성을 `1.2em`로 설정하세요.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("font-size"), "1.2em");
```

`.message` 선택자에 `margin-bottom` 속성을 `20px`로 설정하세요.
