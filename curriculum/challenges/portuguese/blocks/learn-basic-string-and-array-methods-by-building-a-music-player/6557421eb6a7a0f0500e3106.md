...
Dentro do elemento button na função `renderSongs`, adicione um atributo `onclick`.Para o valor, chame a função `deleteSong` e interpolar `song.id`.

# --hints--

Você deve adicionar um atributo `onclick` ao botão de exclusão e passar `"deleteSong(${song.id})"`.
