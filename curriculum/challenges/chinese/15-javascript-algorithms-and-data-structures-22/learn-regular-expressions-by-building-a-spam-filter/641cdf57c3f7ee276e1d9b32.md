...
在 `if` 语句之后，使用三元运算符检查以 `messageInput.value` 作为参数调用 `isSpam()` 的真假性。 If true, set the `textContent` property on the `result` element to `"Oh no! This looks like a spam message."`. Otherwise, set it to `"This message does not seem to contain any spam."`
