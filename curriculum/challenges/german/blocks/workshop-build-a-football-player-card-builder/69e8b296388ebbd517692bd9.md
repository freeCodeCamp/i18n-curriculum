...
Fügen Sie dem Club ein `value`-Attribut mit dem Wert `player.club` hinzu. Danach fügen Sie ein `onChange`-Attribut hinzu, das auf eine Pfeilfunktion gesetzt ist, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `club` auf `e.target.value` gesetzt ist.
