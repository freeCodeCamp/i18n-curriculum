...
이전 `multipleCard`에서 했던 것처럼, `multipleCard` 요소를 다시 가져와 `innerHTML`을 반복하면서 `DrawingType`를 설정하세요. `Object.values()`를 사용하고 `map` 메서드를 호출하며 `type`를 인수로 전달하세요.

# --hints--

`multipleCard.innerHTML`를 `Object.values`를 호출하여 할당하세요.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

`DrawingType`를 `Object.values` 메서드에 전달해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

`map`에서 `Object.values` 메서드를 호출해야 합니다.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

`map` 메서드에는 `type`라는 매개변수가 있어야 합니다.
