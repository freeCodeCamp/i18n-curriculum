...
現在雜誌佈局已經完成，你需要使其具有響應性。

以針對 `only screen` 的 `@media` 查詢開始，其 `max-width` 設置爲 `720px`。 在裏面，創建一個 `.image-wrapper` 選擇器，並給它一個值爲 `1fr` 的 `grid-template-columns` 屬性。

這將在較小的屏幕上將三個圖像排成一列。

# --hints--

你應該有一個新的 `@media` 規則，條件是 `only screen and (max-width: 720px)`。
...
你的新 `.image-wrapper` 選擇器應該有一個值爲 `1fr` 的 `grid-template-columns` 屬性。
