...
現在，對 `multipleCardsBtn` 和 `newReadingBtn` 做同樣的事，為 `multipleCardsBtn` 附加一個點擊事件監聽器，該監聽器會呼叫 `multipleCardSelected`，並為 `newReadingBtn` 附加另一個點擊事件監聽器，該監聽器會呼叫 `newReading`。

你將在接下來的步驟中創建這兩個方法。

# --hints--

你應該為 `click` 元素附加一個 `multipleCardsBtn` 事件。

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

在 `multipleCardsBtn` 回呼事件中，你應該呼叫 `multipleCardSelected` 方法。

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

你應該為 `click` 元素附加一個 `newReadingBtn` 事件。

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

在 `newReadingBtn` 回呼事件中，你應該呼叫 `newReading` 方法。
