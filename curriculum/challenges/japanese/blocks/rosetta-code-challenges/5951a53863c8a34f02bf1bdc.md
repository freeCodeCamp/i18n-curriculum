...
2次元の与えられた点の集合の中から、最も近い2点を見つける関数を提供してください。

単純な解法は $O(n^2)$ のアルゴリズム（*ブルートフォースアルゴリズム*と呼べます）で、擬似コード（インデックスを使ったもの）は以下のようになります。
...
より良いアルゴリズムは再帰的な分割統治法に基づいており、$O(n\log n)$ の計算量です。擬似コードは以下のようになります。
...
入力としては、`Point` オブジェクトの配列で、`x` と `y` メンバーが数値に設定されている引数を想定してください。`distance` と `pair`（最も近い2点のペア）をキーとするオブジェクトを返してください。

例えば、入力配列 `getClosestPair` に対して `points` は以下のように返します。
...
`getClosestPair` は関数である必要があります。

```js
assert(typeof getClosestPair === 'function');
```

`getClosestPair(points1).distance` は `0.0894096443343775` である必要があります。

```js
assert.equal(getClosestPair1_().distance, answer1.distance);
```

`getClosestPair(points1).pair` は `[ { x: 7.46489, y: 4.6268 }, { x: 7.46911, y: 4.71611 } ]` である必要があります。
...
`getClosestPair(points2).distance` は `65.06919393998976` である必要があります。

```js
assert.equal(getClosestPair2_().distance, answer2.distance);
```

`getClosestPair(points2).pair` は `[ { x: 37134, y: 1963 }, { x: 37181, y: 2008 } ]` である必要があります。
...
`getClosestPair(points3).distance` は `6754.625082119658` である必要があります。

```js
assert.equal(getClosestPair3_().distance, answer3.distance);
```

`getClosestPair(points3).pair` は `[ { x: 46817, y: 64975 }, { x: 48953, y: 58567 } ]` である必要があります。
