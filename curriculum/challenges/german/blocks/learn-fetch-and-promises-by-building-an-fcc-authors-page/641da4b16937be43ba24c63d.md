...
Um mit den Daten zu arbeiten, müssen Sie eine weitere `.then()`-Methode verwenden.

Verketten Sie eine weitere `.then()` an die bestehende `.then()`-Methode. Übergeben Sie diesmal `data` als Parameter für die Callback-Funktion. Verwenden Sie für die Callback geschweifte Klammern, da Sie mehr als einen Ausdruck haben werden. Protokollieren Sie innerhalb Ihrer Callback-Funktion `data` in der Konsole, um zu sehen, wie es aussieht.
...
Ihr erster `.then()` sollte kein Semikolon haben.

```js
assert.notMatch(code, /\.then\(\s*(\(\s*res\s*\)|res)\s*=>\s*res\.json\(\s*\)\s*\)\s*;/)
```

Sie sollten eine weitere `.then()` an die bestehende `.then()` verketten.
...
Ihr zweiter `.then()` sollte kein Semikolon dahinter haben.
