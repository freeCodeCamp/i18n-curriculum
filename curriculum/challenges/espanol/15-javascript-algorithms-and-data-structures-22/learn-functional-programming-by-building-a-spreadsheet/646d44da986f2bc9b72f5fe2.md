...
Declare a `toNumberList` function that takes an `args` parameter and implicitly returns the result of splitting the `args` by commas. Luego, encadena un método `map` a tu método `split` y pasa `parseFloat` como el argumento del método `map`.
