...
الآن، يجب أن تنتقل إلى تنسيق العناصر الخاصة بـ **خطة المستمع**.

أضف إلى الـ `div` الخارجي للخطة الأصناف `bg-gray-100`، و`ring-1` لإنشاء إطار سميك حول الـ `div`، و`ring-gray-300` لتعيين لون داكن لذلك الإطار.

# --hints--

يجب أن يحتوي عنصر الـ `div` على الصنف `bg-gray-100`.
...
يجب أن يحتوي عنصر الـ `div` على الصنف `ring-1`.
...
يجب أن يحتوي عنصر الـ `div` على الصنف `ring-gray-300`.
