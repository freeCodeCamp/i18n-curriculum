...
لإنهاء الأمور، عيّن نصًا بـ `Calendar` لوسم الرابط الأول، و`Chat` للثاني، و`Email` للثالث.

بهذا، يصبح مكوِّن شريط التنقل الكبير جاهزًا!

# --hints--

يجب أن تعيّن نصًا بـ `Calendar` لعنصر وسم الرابط الأول.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

يجب أن تعيّن نصًا بـ `Chat` لعنصر وسم الرابط الثاني.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

يجب أن تعيّن نصًا بـ `Email` لعنصر وسم الرابط الثالث.
