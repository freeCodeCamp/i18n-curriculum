...
Erstellen Sie nun innerhalb des Template-Literals der `renderCard`-Funktion ein `div`-Element. Fügen Sie innerhalb des `div` ein `h2`-Element hinzu, das `drawingType` ausgibt, sowie ein `figure`-Element mit der Klasse `card_container ${isReversed ? "reversed-card" : ""}` und dem Attribut `data-id="${shortName}"`.

# --hints--

Sie sollten ein `div`-Element im Template-Literal der `renderCard`-Funktion haben.
...
Sie sollten ein `h2`-Element haben, das `drawingType` innerhalb des `div`-Elements interpoliert.
...
Sie sollten ein `figure`-Element innerhalb des `div`-Elements haben.
...
Ihr `figure`-Element sollte die Klasse `card_container` haben und einen ternären Ausdruck verwenden, um `reversed-card` bedingt hinzuzufügen.
...
Ihr `figure`-Element sollte ein `data-id`-Attribut mit dem Wert `${shortName}` haben.
