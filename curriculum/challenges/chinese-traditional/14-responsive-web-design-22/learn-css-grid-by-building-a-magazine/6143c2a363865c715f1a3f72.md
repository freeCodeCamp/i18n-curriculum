...
你的第一個 `img` 元素應該有一個 `src` 屬性值爲 `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`。
