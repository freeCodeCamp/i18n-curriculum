...
قُم بتدوير القمة، عبر إعطاء زوايا العنصر المزيف السفلية انحناءً بنسبة `100%`، مع ترك الزوايا العلوية عند `0%`.

# --hints--

يجب أن تستخدم خاصية `border-radius` لتدوير القمة.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.borderRadius);
```

يجب أن تعطي `.penguin-body::before` خاصية `border-radius` بقيمة `0% 0% 100% 100%`.
