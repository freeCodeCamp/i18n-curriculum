...
在 `footer` 元素内部，添加一个 `section` 元素，`id` 设置为 `contact`。

在 `section` 元素内部，添加一个 `h2` 元素，其文本为 `Contact`。

# --hints--

你应该在 `footer` 元素中包含一个 `section` 元素。
...
你的 `section` 元素应该有个 `id`，设置为 `"contact"`。
