...
음악 플레이어는 노래 목록, 현재 재생 중인 노래, 그리고 현재 노래의 시간을 추적해야 합니다. 이를 위해 이 정보를 저장할 객체를 만들어야 합니다.

`let` 키워드를 사용해 `userData`이라는 새 변수를 선언하고 빈 객체를 할당하는 것부터 시작하세요.

# --hints--

`let` 키워드를 사용해 `userData` 변수를 만들어야 합니다.

```js
assert.match(code, /let\s+userData\s*=/)
```

`userData` 변수에 빈 객체를 할당해야 합니다.
