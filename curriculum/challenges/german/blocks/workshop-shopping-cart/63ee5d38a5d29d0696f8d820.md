...
Erstellen Sie in Ihrem Template-Literal ein `div`-Element mit einer Klasse von `dessert-card`. Erstellen Sie in diesem `div` ein `h2`-Element und geben Sie ihm den Text der `name`-Variablen.
...
Ihr `div`-Element sollte eine Klasse von `dessert-card` haben.
...
Ihr `h2`-Element sollte den Text der `name`-Variablen haben.

```js
assert.strictEqual(document.querySelectorAll('h2')[0]?.textContent, 'Vanilla Cupcakes (6 Pack)');
```

Ihr `h2`-Element sollte sich innerhalb des `div`-Elements befinden.
