...
接著進入你的第二個表格。首先為它添加一個 `caption` 元素並設定為 `Liabilities`。然後添加你的 `thead` 和 `tbody`。

# --hints--

你的第二個 `table` 元素應該有一個 `caption` 元素。
...
你的 `caption` 元素應該有文字 `Liabilities`。
