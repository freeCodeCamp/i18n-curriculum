...
`playlistSongElements` を `forEach` メソッドでループしてください。

`forEach` メソッドは配列をループして、配列の各要素に対して関数を実行するために使います。たとえば、数字の配列があって、それぞれの数字をコンソールに表示したい場合です。
...
`forEach` に対して `playlistSongElements` メソッドを使ってください。パラメータとして `songEl` を渡し、アロー構文で空のコールバックを追加してください。

# --hints--

`forEach()` に `playlistSongElements` をチェーンする必要があります。

```js
assert.match(code, /playlistSongElements\.forEach\(/)
```

`forEach()` メソッドは `songEl` パラメータを持ち、アロー構文を使うべきです。

```js
assert.match(code, /playlistSongElements\.forEach\(\s*(\(\s*songEl\s*\)|songEl)\s*=>\s*/)
```

`forEach` は空の波括弧を持つべきです。
