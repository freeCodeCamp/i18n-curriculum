...
Füge unter dem `.blush.right`-Element zwei `div`-Elemente mit einer `class` von `beak` hinzu. Gib außerdem dem ersten `.beak` Element eine `class` von `top`, und dem zweiten `.beak` Element eine `class` von `bottom`.
...
Du solltest dem ersten neuen `div` eine `class` von `beak` geben.

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

Du solltest dem zweiten neuen `div` eine `class` von `beak` geben.

```js
assert.equal(document.querySelectorAll('.penguin-head > div.beak')?.length, 2);
```

Du solltest dem ersten neuen `div` eine `class` von `top` geben.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

Du solltest dem zweiten neuen `div` eine `class` von `bottom` geben.

```js
assert.exists(document.querySelector('.penguin-head > div.beak.bottom'));
```

Du solltest `div.beak.top` nach `div.blush.right` platzieren.

```js
assert.exists(document.querySelector('.blush.right + .beak.top'));
```

Du solltest `div.beak.bottom` nach `div.beak.top` platzieren.
