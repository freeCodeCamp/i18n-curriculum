...
تحتاج إلى العمل على تشغيل الأغنية التالية والأغنية السابقة. لهذا، ستحتاج إلى دالتي `playNextSong` و `playPreviousSong`.

استخدم `const` وصيغة السهم لإنشاء دالة `playNextSong` فارغة.

# --hints--

يجب أن تستخدم `const` لإنشاء دالة فارغة باسم `playNextSong`.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

يجب أن تكون `playNextSong` دالة.

```js
assert.isFunction(playNextSong)
```

يجب أن تستخدم دالة `playNextSong` صيغة السهم.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

يجب أن تكون دالة `playNextSong` فارغة.
