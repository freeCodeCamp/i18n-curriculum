...
Ifuatayo, chapisha `new_char` na uone matokeo.

# --hints--

Unapaswa kuchapisha tofauti yako ya `new_char`.

```js
assert.match(code, /print\s*\(\s*new_char\s*\)/)
```

Unapaswa kuchapisha tofauti yako ya `new_char` mwishoni mwa mwili wa mzunguko wako.
