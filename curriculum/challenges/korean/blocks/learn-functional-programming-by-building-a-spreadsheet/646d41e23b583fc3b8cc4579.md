...
이제 함수 구문 분석 로직을 문자열에 적용할 수 있습니다. `applyFunction`라는 함수를 선언하고 `str` 매개변수를 받으세요.

# --hints--

`applyFunction` 변수를 선언해야 합니다.

```js
assert.match(code, /(?:var|let|const)\s+applyFunction\s*=/);
```

`const`를 사용해 `applyFunction` 변수를 선언하세요.

```js
assert.match(code, /const\s+applyFunction\s*=/);
```

`applyFunction` 변수는 함수여야 합니다.

```js
assert.isFunction(applyFunction);
```

`applyFunction` 함수는 화살표 구문을 사용해야 합니다.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`applyFunction` 함수는 `str` 매개변수를 가져야 합니다.

```js
assert.match(code,  /const\s+applyFunction\s*=\s*(\(\s*str\s*\)|str)\s*=>/);
```

`applyFunction`는 비어 있어야 합니다.
