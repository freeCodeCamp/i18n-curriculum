...
Agrega un atributo `onChange` al input para capturar lo que el usuario teclea. Asígnalo a una función flecha que toma `e` como argumento. Dentro de la función, actualiza el estado `query` pasando `e.target.value` a la función asignadora, que contiene el valor actual del input. Aquí tienes un ejemplo:
...
Debes asegurarte de que el manejador `onChange` esté definido.
...
El manejador `onChange` debe tener un parámetro `e`.
...
El manejador `onChange` debe llamar a `setQuery()` con `e.target.value`.
