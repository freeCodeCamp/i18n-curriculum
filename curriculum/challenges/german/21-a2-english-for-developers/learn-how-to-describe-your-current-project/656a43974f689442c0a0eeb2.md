...
`Currently` comes from `current` meaning happening now. Adding `-ly` turns an adjective into an adverb. Hier ein Beispiel:

`She is currently reading a book` (She is reading a book now)

`Trends` are popular styles or ideas that many people are talking about or doing. Das Wort `trend` im Singular, wird zu `trends` im Plural. Zum Beispiel:
