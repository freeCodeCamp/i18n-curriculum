...
Sie können Ihrem Menü mit verschiedenen `padding` Eigenschaften etwas Abstand zwischen dem Inhalt und den Seiten geben.

Geben Sie der `menu` Klasse eine `padding-left` und eine `padding-right` mit demselben Wert `20px`.

# --hints--

Sie sollten die `padding-left` Eigenschaft auf `20px` setzen.
...
Sie sollten die `padding-right` Eigenschaft auf `20px` setzen.
...
Ihr `.menu` Element sollte eine `padding-left` von `20px` haben.
...
Ihr `.menu` Element sollte eine `padding-right` von `20px` haben.
