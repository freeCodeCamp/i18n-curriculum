...
이제 `.map()` 함수가 커링된 `num => charRange(...).map(...)` 호출에서 반환된 `addCharacters` 함수 참조를 받아서, 요소들을 제대로 순회하며 각 요소를 `n`로 그 함수에 전달할 것입니다.

`match` 매개변수를 사용하지 않는다는 것을 알 수 있습니다. JavaScript에서는 사용하지 않는 매개변수에 접두사로 밑줄 `_`을 붙이는 관례가 흔합니다. 매개변수를 비워 둘 수도 있지만 `(, char1)`, 미래 가독성을 위해 매개변수 이름을 명확히 하는 편이 좋습니다.

`match` 매개변수에 접두사로 밑줄을 붙이세요.

# --hints--

`match` 매개변수에 접두사로 밑줄을 붙여야 합니다.
