...
フォームはこれで完成です。入力中にプレビューカードを更新するには、入力を`player`の状態に接続する必要があります。

まずName `input`から始めて、`value`の`player.name`属性を追加してください。次に、`onChange`をパラメータに取り、`e`をスプレッドし、`setPlayer`を`player`に設定したオブジェクトを`name`に呼び出すアロー関数を設定した`e.target.value`属性を追加してください。スプレッドと`name`の値設定はオブジェクト内に入れるのを忘れないでください。

# --hints--

名前の入力`value`はplayer状態の`name`フィールドにバインドされている必要があります。
...
名前の入力には`onChange`ハンドラが必要です。

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

`onChange`ハンドラは、`setPlayer`をスプレッドし、`setPlayer`を`player`に設定したオブジェクトを持つ`name`を`e.target.value`に呼び出す必要があります。
