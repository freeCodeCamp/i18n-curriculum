...
Градієнтні переходи часто поступово змінюються від одного кольору до іншого. Коли потрібна більш різка зміна, перехід можна зробити з різкою зупинкою, як ось так:
...
Додайте `linear-gradient` до `.bb2b`, який використовує `--building-color2` від `0%` до `6%` і `--window-color2` від `6%` до `9%`.

# --hints--

Ви повинні надати `.bb2b` властивість `background`.
...
`.bb2b` має мати `linear-gradient`, що переходить від `--building-color2` на `0%` до `6%`, і `--window-color2` на `6%` до `9%`.
