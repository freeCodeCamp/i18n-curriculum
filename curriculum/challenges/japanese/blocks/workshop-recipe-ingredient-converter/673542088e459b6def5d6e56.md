...
実際にDOMを更新する関数が必要です。`updateResultsList` 関数を宣言してください。

まず、その関数は`resultList` 要素のHTMLをクリアする必要があります。

# --hints--

`updateResultsList` 関数を定義してください。

```js
assert.isFunction(updateResultsList);
```

`updateResultsList` 関数は`resultList.innerHTML` を空の文字列に設定する必要があります。
