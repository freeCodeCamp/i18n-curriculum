...
Deberías crear un nuevo elemento `p` y `div` al final de tu elemento `.daily-value`.
...
Tu nuevo elemento `p` debería tener el atributo `class` establecido como `indent no-divider`.
...
Tu nuevo `div` debería tener el atributo `class` establecido como `divider`.
