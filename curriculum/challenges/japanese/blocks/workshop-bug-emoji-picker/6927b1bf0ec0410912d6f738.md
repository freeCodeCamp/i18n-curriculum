...
`Bug` クラスの中で、`emojiElement` 型の `HTMLParagraphElement` という名前のプロパティを宣言してください。このプロパティは、バグの絵文字を表示する HTML の段落要素への参照を保持します。

# --hints--

`Bug` クラスには `emojiElement` という名前のプロパティが必要です。
...
`emojiElement` プロパティは `HTMLParagraphElement` 型である必要があります。
...
`emojiElement` プロパティは確定代入アサーション演算子（`!`）を使う必要があります。
