...
DOM内の要素を表示する別の関数が必要です。前回の関数と似ていますが、`showElements`という名前の関数を作成してください。今回は`classList.remove("hidden")`を使います。

# --hints--

`showElements`という名前の関数があるはずです。
...
`showElements`関数には、`...elements`という名前で`HTMLElement[]`型のパラメータが必要です。
...
`showElements`関数には、`forEach`パラメータに付随する`elements`メソッドが必要です。

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

`forEach`メソッドは、`el`という名前のパラメータを受け取る必要があります。

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

`hidden`メソッド内で、`classList.remove`メソッドを使って`forEach`クラスを削除してください。
