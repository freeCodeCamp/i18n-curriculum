...
既然你可以评估数学表达式，那么你需要考虑运算顺序。 声明一个采用 `str` 参数的 `highPrecedence` 函数。

# --hints--

你应该声明一个 `highPrecedence` 变量。

```js
assert.match(code, /(?:var|let|const)\s+highPrecedence/);
```

你应该使用 `const` 来声明你的 `highPrecedence` 变量。

```js
assert.match(code, /const\s+highPrecedence/);
```

你的 `highPrecedence` 变量应该是一个函数。

```js
assert.isFunction(highPrecedence);
```

你的 `highPrecedence` 函数应该使用箭头语法。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

你的 `highPrecedence` 函数应该有一个 `str` 参数。

```js
assert.match(code, /const\s+highPrecedence\s*=\s*(\(\s*str\s*\)|str)\s*=>/);
```

你的 `highPrecedence` 函数应该是空的。
