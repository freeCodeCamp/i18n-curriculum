...
在 `allSongs` 數組內，創建一個具有以下屬性和值的對象：
...
你的 `allSongs` 數組應該有一個值。

```js
assert.equal(allSongs.length, 1);
```

你的 `allSongs` 數組應該有一個對象作爲其第一個值。

```js
assert.isObject(allSongs[0]);
```

你的 `allSongs` 數組應該有一個對象，其 `id` 屬性設置爲數字 `0`。

```js
assert.equal(allSongs[0].id, 0);
```

你的 `allSongs` 數組應該有一個對象，其 `title` 屬性設置爲字符串 `"Scratching The Surface"`。

```js
assert.equal(allSongs[0].title, "Scratching The Surface");
```

你的 `allSongs` 數組應該有一個對象，其 `artist` 屬性設置爲字符串 `"Quincy Larson"`。

```js
assert.equal(allSongs[0].artist, "Quincy Larson");
```

你的 `allSongs` 數組應該有一個對象，其 `duration` 屬性設置爲字符串 `"4:25"`。

```js
assert.equal(allSongs[0].duration, "4:25");
```

你的 `allSongs` 數組應該有一個對象，其 `src` 屬性設置爲字符串 `"https://cdn.freecodecamp.org/curriculum/js-music-player/scratching-the-surface.mp3"`。
