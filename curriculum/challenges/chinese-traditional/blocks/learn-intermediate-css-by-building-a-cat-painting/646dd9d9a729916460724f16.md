...
將右側內耳移動到位置，並將 `position` 屬性設為 `absolute`，`top` 設為 `22px`，`left` 設為 `-20px`。

# --hints--

你的 `.cat-right-inner-ear` 選擇器應該有一個設定為 `absolute` 的 `position` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.position === 'absolute')
```

你的 `.cat-right-inner-ear` 選擇器應該有一個 `top` 屬性設定為 `22px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.top === '22px')
```

你的 `.cat-right-inner-ear` 選擇器應該有一個 `left` 屬性設定為 `-20px`。
