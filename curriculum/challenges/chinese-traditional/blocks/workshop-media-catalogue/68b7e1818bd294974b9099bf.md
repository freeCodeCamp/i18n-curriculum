...
在宣告 `result` 變數之後，創建一個使用列舉來迭代目錄中條款的 `for` 迴圈，並將以下格式的字串附加到 `result`：`N. movie`，其中 `N` 是從 `1` 開始的數字索引，`movie` 是該條款的字串表述。

讓字串以換行字元結尾，並在迴圈後傳回 `result`。

# --hints--

你應該在你的 `__str__` 方法中有一個 `for` 迴圈。
...
當 `self.items` 包含 `Movie('Barry Lyndon', 1975, 'Stanley Kubrick', 184)` 和 `Movie('Fahrenheit 451', 1966, 'Francois Truffaut', 112)` 時，你的 `__str__` 方法應該傳回 `Media Catalogue (2 items):\n\n1. Barry Lyndon (1975) - 184 min, Stanley Kubrick\n2. Fahrenheit 451 (1966) - 112 min, Francois Truffaut\n`。
