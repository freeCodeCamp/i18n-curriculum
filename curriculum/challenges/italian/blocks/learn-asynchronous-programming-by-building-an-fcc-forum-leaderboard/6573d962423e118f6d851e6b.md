...
Successivamente, consulta l'elemento `#posts-container` usando il metodo `getElementById()`. Assegnalo a una nuova costante chiamata `postsContainer`.

# --hints--

Dovresti usare `const` per dichiarare una variabile `postsContainer`.

```js
assert.match(code, /const\s+postsContainer\s*=/);
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#posts-container`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)posts-container\1\s*\)/);
```

Dovresti assegnare l'elemento `#posts-container` alla tua variabile `postsContainer`.
