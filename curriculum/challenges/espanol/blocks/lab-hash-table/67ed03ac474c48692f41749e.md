...
4. El método `add` debería:
    ...
    - Usar el valor hash calculado para almacenar el par clave-valor dentro del diccionario `collection`.
    - Si múltiples claves producen el mismo valor hash, sus pares clave-valor deberían almacenarse en un diccionario anidado bajo el mismo valor hash.
...
`HashTable.add('golf', 'sport')` debería añadir el par clave-valor a `collection` en la clave `424`.
...
`HashTable.add('dear', 'friend')` y `HashTable.add('read', 'book')` deberían añadir ambos pares clave-valor a `collection` en el índice `412` como un diccionario anidado.
...
Cuando el par clave-valor `'golf', 'sport'` no existe en `collection`, `HashTable.lookup('golf')` debería devolver `None`.
