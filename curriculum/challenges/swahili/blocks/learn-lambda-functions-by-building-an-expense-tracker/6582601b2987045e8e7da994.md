...
Baada ya wito wako wa `print()`, unahitaji kuchuja matumizi na kuchapisha orodha iliyochujwa. Tangaza kigezo `expenses_from_category` na weka thamani yake kwa wito wa `filter_expenses_by_category` ukipitisha `expenses` na `category` kama hoja.

# --hints--

Unapaswa kutangaza kigezo kinachoitwa `expenses_from_category` katika kifungu chako cha `elif`.
...
Unapaswa kuita `filter_expenses_by_category()` ukipitisha `expenses` na `category` kama hoja na kuwekea thamani kigezoni chako cha `expenses_from_category`.
...
Kigezo chako cha `expenses_from_category` kinapaswa kuja baada ya wito wa `print()`.
