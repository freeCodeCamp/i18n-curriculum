...
現在該處理最後一個問題了。 如果待辦任務的名稱或描述中包含特殊字符（如引號），應用程序就會中斷。 儘管看起來並非如此，但正確的項目似乎丟失了。

爲了解決這個問題，我們需要創建一個名爲 `removeSpecialChars` 的函數，該函數以字符串作爲輸入並刪除所有特殊字符。

# --hints--

你應該定義一個 `removeSpecialChars` 函數。

```js
assert.isFunction(removeSpecialChars);
```

你的 `removeSpecialChars` 不應刪除空格。
