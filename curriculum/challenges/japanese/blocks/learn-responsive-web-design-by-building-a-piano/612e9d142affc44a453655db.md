...
キー自体のスタイルを設定する時間です。`.key`要素に対して`class`セレクターを作成してください。`background-color`セットを`#ffffff`に、`position`プロパティを`relative`に、`width`プロパティを`41px`に、`height`プロパティを`175px`に設定してください。
