...
인용문은 적절한 인용 부호가 없으면 진정한 인용문이 아닙니다. CSS 의사 선택자를 사용해 이 부호를 추가할 수 있습니다.

`.quote::before` 선택자를 만들고 `content` 속성을 `"`로 설정하되, 뒤에 공백이 오도록 하세요.

또한 `.quote::after` 선택자를 만들고 `content` 속성을 `"`로 설정하되, 앞에 공백이 오도록 하세요.

# --hints--

`.quote::before` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

`.quote::before` 선택자에는 `content` 속성이 `'" '`로 설정되어야 합니다.

```js
assert.match(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content, /\"\\"\s\"/);
```

`.quote::after` 선택자가 있어야 합니다.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

`.quote::after` 선택자에는 `content` 속성이 `' "'`로 설정되어야 합니다.
