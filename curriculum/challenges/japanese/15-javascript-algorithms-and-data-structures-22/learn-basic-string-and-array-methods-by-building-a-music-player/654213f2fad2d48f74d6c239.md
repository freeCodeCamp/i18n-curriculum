...
`button` 要素をもう一つ作成し、クラスを `playlist-song-delete` に設定してください。また、`aria-label` 属性を「`Delete` [曲名]」という形式になるように、`song.title` を文字列補間で挿入して設定してください。 削除アイコンが表示されるように、コンテンツとして次の SVG を貼り付けてください。
...
`button` 要素を作成し、1 番目の属性と値として、クラス属性を `playlist-song-delete` に設定してください。

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

ボタン要素の 2 番目の属性と値として、`aria-label` 属性を `Delete ${song.title}` に設定してください。

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

与えられた `svg` を、`button` 要素の中身として貼り付けてください。
