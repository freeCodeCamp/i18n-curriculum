...
2. 你应该实现一个 `planRestock(pantry, shipment)` 函数，该函数比较当前储藏室与即将到货的货物，并以 `{ type: "restock" | "discard" | "donate", item }` 形式返回一个动作数组。

3. 你应该实现一个 `groupByZone(actions)` 函数，根据每个物品的 `zone` 属性将动作分组到存储区域。
