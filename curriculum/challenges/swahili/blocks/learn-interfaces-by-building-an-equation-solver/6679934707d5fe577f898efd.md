...
Ongeza kesi nyingine kwa wakati mrefu wa `results` ni `2`. Mara hii, weka thamani ya `result_list` kuwa orodha yenye mfuatano wa herufi mbili zenye muundo wa `x1 = <root1>` na `x2 = <root2>`. Tena, fanya suluhisho lionyeshe alama chanya na hasi zote mbili.

# --hints--

Haupaswi kubadilisha thamani ya somo la `match` katika tamko lako.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("len(results)")`)) })
```

Haupaswi kubadilisha sehemu zako za `case` zilizopo.
...
Unapaswa kuunda `case` mpya yenye muundo wa `2` baada ya sehemu ya `case` iliyopo.
...
Unapaswa kuweka thamani ya `x1 = <root1>` kuwa orodha yenye mfuatano wa herufi mbili zenye muundo wa `x2 = <root2>` na `result_list` ndani ya mwili mpya wa `case`. Onyesha alama chanya na hasi zote mbili kwa matokeo.
