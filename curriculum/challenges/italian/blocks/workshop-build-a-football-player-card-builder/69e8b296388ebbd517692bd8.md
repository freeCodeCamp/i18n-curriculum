...
Aggiungi un attributo `value` di `player.overallRating` all'Overall `input`. Dopodiché, aggiungi un attributo `onChange` impostato su una funzione freccia che prende `e` come parametro e chiama `setPlayer` con uno spread di `player` e `overallRating` impostato su `Number(e.target.value)`.

Dato che `overallRating` è un numero, usa `Number()` per convertire il valore stringa dall'input.

# --hints--

Il tuo Overall `input` dovrebbe avere un `value` uguale al valore attuale del rating complessivo del giocatore.
...
Il tuo Overall `input` dovrebbe avere un attributo `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Il `onChange` del tuo Overall `input` dovrebbe chiamare `setPlayer` con uno spread di `player` e `overallRating` impostato su `Number(e.target.value)`.
