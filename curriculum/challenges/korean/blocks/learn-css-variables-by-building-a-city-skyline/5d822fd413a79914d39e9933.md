...
방금 추가한 `repeating-linear-gradient` 아래에 또 다른 `repeating-linear-gradient`를 추가하세요. `90deg` 방향을 주고, `0%`에서 `12%`까지는 건물 색상, `12%`에서 `44%`까지는 창문 색상을 사용하세요. 이렇게 하면 여러 개의 직사각형 창문이 만들어집니다.

# --hints--

`.fb5`에 두 번째 `repeating-linear-gradient`를 `background` 속성에 주어야 합니다.
...
두 번째 `repeating-linear-gradient`에 `90deg` 방향을 주어야 합니다.
...
두 번째 `repeating-linear-gradient`에 `--building-color2` 첫 번째 색상을 `0%`에서 `12%`까지 주어야 합니다.
...
두 번째 `repeating-linear-gradient`에 `--window-color2` 두 번째 색상을 `12%`에서 `44%`까지 주어야 합니다.
