...
카드 안의 항목들을 배치하려면 다시 그리드 클래스를 사용해야 합니다. 래퍼 `grid`에 유틸리티 클래스 `grid-rows-[1fr_auto]`, `gap-6`, 그리고 `div`를 할당하세요.

또한, 클래스 `rounded-xl`로 날카로운 모서리를 제거하세요.

# --hints--

프리미엄 플랜용 래퍼 `div` 요소에는 클래스 `grid`가 있어야 합니다.
...
프리미엄 플랜용 래퍼 `div` 요소에는 클래스 `grid-rows-[1fr_auto]`가 있어야 합니다.
...
프리미엄 플랜용 래퍼 `div` 요소에는 클래스 `gap-6`가 있어야 합니다.
...
프리미엄 플랜용 래퍼 `div` 요소에는 클래스 `rounded-xl`가 있어야 합니다.
