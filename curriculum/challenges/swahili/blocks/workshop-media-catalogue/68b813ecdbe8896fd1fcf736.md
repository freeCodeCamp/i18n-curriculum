...
Tumia njia ya `add` ya `catalogue` kuongeza `series1` kwenye orodha ya `items`.

Kisha, tengeneza kigezo kingine `series2`. Weka thamani ya mfano mwingine wa `TVSeries` ndani yake na ongeza `series2` kwenye katalogi pia.

# --hints--

Unapaswa kutumia njia ya `add` ya `catalogue` ukipitisha `series1`.
...
Unapaswa kutengeneza kigezo kinachoitwa `series2` na kuweka thamani ya mfano wa `TVSeries` ndani yake.
...
Unapaswa kutumia njia ya `add` ya `catalogue` ukipitisha `series2`.
