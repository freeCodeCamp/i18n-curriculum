...
En este código, `fetch()` devuelve una promesa y el manejador `.then()` convierte la respuesta resuelta a formato JSON.
...
- Las **promesas** son objetos que representan la finalización o el fallo eventual de una operación asincrónica y su valor resultante. El valor de la promesa es conocido solo cuando la operación asincrónica se completa.
...
- Los scripts `defer` mantienen el orden de ejecución como aparecen en el documento HTML.
...
- El ejemplo a continuación demuestra el método `getCurrentPosition()` de la API, que se usa para obtener la ubicación actual del usuario.
...
En este código, llamamos a `getCurrentPosition` y le pasamos una función, que se ejecutará cuando se obtenga la posición con éxito.

El objeto `position` contiene una variedad de información, pero aquí solo hemos seleccionado `latitude` y `longitude`.

Si hay un problema al obtener la `position`, entonces el error se registrará en la consola.
