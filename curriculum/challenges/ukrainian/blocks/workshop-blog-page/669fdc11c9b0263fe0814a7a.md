...
Текстом першого посилання має бути `About`, а значенням атрибута `href` має бути `"#about"`. The hash symbol in front of `about` represents an `id` name, which will be added later in the project.
