...
بداخل عنصر `.shirt` يجب أن تكون خاصية `top` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.top, '165px');
```

بداخل عنصر `.shirt` يجب أن تكون خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.left, '127.5px');
```

يجب عليك إعطاء `.shirt` الـ `z-index` بقيمة `1`.
