...
title: 모든 버튼에 Font Awesome 아이콘 추가하기
challengeType: 0
forumTopicId: 16637
required:
...
Font Awesome은 편리한 아이콘 라이브러리입니다. 이 아이콘들은 웹 폰트나 벡터 그래픽일 수 있습니다. 이 아이콘들은 폰트처럼 취급됩니다. 픽셀 단위로 크기를 지정할 수 있으며, 부모 HTML 요소의 폰트 크기를 따릅니다.

# --instructions--

Font Awesome을 사용해 정보 버튼에 `info-circle` 아이콘을, 삭제 버튼에 `trash` 아이콘을 추가하세요.

**참고:** 이 도전 과제를 완료하려면 `i` 또는 `span` 요소 중 하나를 사용할 수 있습니다.

# --hints--

`<i class="fas fa-info-circle"></i>` 버튼 요소 안에 `info`를 추가해야 합니다.
...
`<i class="fas fa-trash"></i>` 버튼 요소 안에 `delete`를 추가해야 합니다.
...
각 `i` 요소는 닫는 태그가 있어야 하며, `<i class="fas fa-thumbs-up"></i>`는 `like` 버튼 요소 안에 있어야 합니다.
