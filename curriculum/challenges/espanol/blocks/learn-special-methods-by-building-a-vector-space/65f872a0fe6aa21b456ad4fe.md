...
Finalmente, pasa la llamada `tuple()` como argumento a la función `str()`. 

# --hints--

Debes pasar la llamada `tuple()` que actualmente estás retornando desde el método `__str__` a la función `str()`.
