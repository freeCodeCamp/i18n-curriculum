...
Your new checkbox should have an `id` attribute with the value `lazy` and a `name` attribute with the value `personality`. 检查开始标签的名称后面是否有空格和/或所有属性名称之前是否有空格。
...
Your new checkbox should be after the first one. 你把它们的顺序写错了。
