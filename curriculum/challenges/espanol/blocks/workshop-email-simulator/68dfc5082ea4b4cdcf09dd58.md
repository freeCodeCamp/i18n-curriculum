...
Después de obtener el índice real, crea una sentencia if para verificar si el `actual_index` es menor que 0 o mayor o igual a la longitud de la lista `self.emails`. Si es así, imprime el mensaje `Invalid email number.\n` y usa `return` para salir del método.
...
Si el índice es inválido, debes imprimir el mensaje `Invalid email number.\n` y usar `return` para salir del método.
