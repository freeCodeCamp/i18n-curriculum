...
<dfn>代替シーケンス</dfn> `|` は、`|` の左側のテキストか右側のテキストのどちらかにマッチさせるために使えます。たとえば、正規表現 `/yes|no/` は `yes` または `no` のどちらかにマッチします。

`helpRegex` を更新して、`please help` または `assist me` のどちらかにマッチするようにしてください。

# --hints--

`helpRegex` には `|` の代替シーケンスを使うべきです。

```js
assert.include(helpRegex.toString(), "|");
```

`helpRegex` は文字列 `please help` にマッチするべきです。
...
`helpRegex` は文字列 `assist me` にマッチするべきです。
