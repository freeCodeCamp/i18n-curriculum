...
`getMode()` 함수를 `calculate` 로직에 추가하고, 해당 HTML 요소를 업데이트하세요.

# --hints--

`calculate` 함수에는 값이 `mode`인 `getMode(numbers)` 변수가 있어야 합니다.

```js
assert.match(calculate.toString(), /mode\s*=\s*getMode\(\s*numbers\s*\)\s*;?/);
```

`calculate` 함수는 DOM에서 `#mode` 요소를 쿼리하고 `textContent`을 `mode`로 설정해야 합니다.
