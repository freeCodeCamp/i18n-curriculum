...
让 `header` 占据其父容器的完整宽度，将其 `height` 设置为 `50px`，`background-color` 设置为 `#1b1b32`。 然后，设置 `display` 以使用 *Flexbox*。
