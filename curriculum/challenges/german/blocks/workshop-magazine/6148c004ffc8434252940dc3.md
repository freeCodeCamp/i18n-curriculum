...
Erstellen Sie einen `.social-icons`-Selektor. Geben Sie ihm eine `display`-Eigenschaft mit dem Wert `grid` und eine `font-size`-Eigenschaft mit dem Wert `3rem`.

# --hints--

Sie sollten einen `.social-icons`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.social-icons'));
```

Ihr `.social-icons`-Selektor sollte eine `display`-Eigenschaft mit dem Wert `grid` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

Ihr `.social-icons`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `3rem` haben.
