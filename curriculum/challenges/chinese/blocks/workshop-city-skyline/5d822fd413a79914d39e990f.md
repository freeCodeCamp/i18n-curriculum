...
窗户在紫色建筑后面的左侧堆叠在一起。 在 `.building-wrap` 下面添加一个名为 `window-wrap` 的新类。 使 `.window-wrap` 成为一个 flexbox 容器，使用 `align-items` 和 `justify-content` 属性将它的子元素垂直居中，并分别在它们的父元素中均匀排布。

# --hints--

你应该创建一个 `.window-wrap` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".window-wrap"));
```

你应该给 `.window-wrap` 添加一个值为 `flex` 的 `display`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".window-wrap")?.display, "flex");
```

你应该给 `.window-wrap` 添加一个值为 `center` 的 `align-items`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".window-wrap")?.alignItems, "center");
```

你应该给 `.window-wrap` 添加一个值为 `space-evenly` 的 `justify-content`。
