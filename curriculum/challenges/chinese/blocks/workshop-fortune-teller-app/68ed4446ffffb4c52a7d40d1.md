...
在构造函数内，使用 `fetchCardsData()` 关键字调用 `initializeEventListeners()` 和 `this`，这样它们会在类初始化时运行。

`fetchCardsData` 方法将负责从 API 装载塔罗牌数据，`initializeEventListeners` 将设置按钮点击处理程序。你将在接下来的步骤中创建这两个方法。

# --hints--

你应该在 `fetchCardsData()` 内调用 `constructor` 方法。
...
你应该在构造函数内调用 `initializeEventListeners()` 方法。
