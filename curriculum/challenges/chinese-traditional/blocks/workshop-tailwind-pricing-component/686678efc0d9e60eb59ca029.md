...
徽章可以有更好的樣式。為此，你將使用一些 Tailwind 漸層類別。以下是你如何在 Tailwind 中使用漸層樣式：
...
為你的徽章 `div` 添加類別 `bg-gradient-to-r`、`from-fuchsia-500` 和 `to-indigo-500`。同時，添加 `rounded-full`、`px-3`、`py-1`、`text-xs`、`font-bold` 和 `text-white`。

# --hints--

你的 `div` 元素中帶有 `Most popular` 文字，應該有 `bg-gradient-to-r` 類別。
...
你的 `div` 元素中帶有 `Most popular` 文字，應該有 `from-fuchsia-500` 類別。
...
你的 `div` 元素中帶有 `Most popular` 文字，應該有 `to-indigo-500` 類別。
...
你的 `div` 元素中帶有文字 `Most popular` 應該有類別 `rounded-full`。
...
你的 `div` 元素中帶有文字 `Most popular` 應該有類別 `px-3`。
...
你的 `div` 元素中帶有 `Most popular` 文字，應該有 `py-1` 類別。
...
你的 `div` 元素中帶有文字 `Most popular` 應該有類別 `text-xs`。
...
你的 `div` 元素中帶有文字 `Most popular` 應該有類別 `font-bold`。
...
你的 `div` 元素中帶有文字 `Most popular` 應該有類別 `text-white`。
