...
To get the index for the current song, you can use the <dfn>indexOf()</dfn> method. The `indexOf()` array method returns the first index at which a given element can be found in the array, or `-1` if the element is not present.
