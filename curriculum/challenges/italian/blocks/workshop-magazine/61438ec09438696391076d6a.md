...
Sotto il tuo elemento `.heading`, crea un nuovo elemento `section` con il `class` impostato su `text`. All’interno di questo, crea un elemento `p` con il `class` impostato su `first-paragraph` e il seguente testo:
...
Il tuo nuovo elemento `section` dovrebbe venire dopo il tuo elemento `.heading`.

```js
assert.equal(document.querySelectorAll('section')?.[1]?.previousElementSibling?.className, 'heading');
```

Il tuo nuovo elemento `section` dovrebbe avere il `class` impostato su `text`.

```js
assert.equal(document.querySelectorAll('section')?.[1]?.className, 'text');
```

Dovresti creare un nuovo elemento `p` all’interno del tuo elemento `.text`.

```js
assert.lengthOf(document.querySelector('.text')?.querySelectorAll('p'), 1);
```

Il tuo nuovo elemento `p` dovrebbe avere il `class` impostato su `first-paragraph`.

```js
assert.equal(document.querySelector('.text p')?.className, 'first-paragraph');
```

Il tuo nuovo elemento `p` dovrebbe contenere il testo fornito.
