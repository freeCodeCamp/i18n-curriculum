...
Asigna a tu `conversionTable` una propiedad `ounce`, y asígnale un objeto con una propiedad `cup` establecida en `0.125`, una propiedad `gram` establecida en `28.35`, y una propiedad `teaspoon` establecida en `6`.

# --hints--

Tu objeto `conversionTable` debe tener una propiedad `ounce`.

```js
assert.property(conversionTable, "ounce");
```

Tu propiedad `ounce` debe ser un objeto.

```js
assert.isObject(conversionTable.ounce);
```

Tu objeto `ounce` debe tener una propiedad `cup`.

```js
assert.property(conversionTable.ounce, "cup");
```

Tu propiedad `cup` debe estar establecida en `0.125`.

```js
assert.strictEqual(conversionTable.ounce?.cup, 0.125);
```

Tu objeto `ounce` debe tener una propiedad `gram`.

```js
assert.property(conversionTable.ounce, "gram");
```

Tu propiedad `gram` debe estar establecida en `28.35`.

```js
assert.strictEqual(conversionTable.ounce?.gram, 28.35);
```

Tu objeto `ounce` debe tener una propiedad `teaspoon`.

```js
assert.property(conversionTable.ounce, "teaspoon");
```

Tu propiedad `teaspoon` debe estar establecida en `6`.
