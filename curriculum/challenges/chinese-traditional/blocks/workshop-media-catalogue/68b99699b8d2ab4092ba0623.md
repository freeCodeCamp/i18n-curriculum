...
在你的 `__str__` 方法中的 `if` 述語之後，創建兩個名為 `movies` 和 `series` 的變數，並分別指定為呼叫 `get_movies` 方法和呼叫 `get_tv_series` 方法。

# --hints--

你應該在你的 `__str__` 方法中有一個名為 `movies` 的變數。
...
你將 `self.get_movies()` 指定到你的 `movies` 變數。
...
你應該在你的 `__str__` 方法中有一個名為 `series` 的變數。
...
你將 `self.get_tv_series()` 指定到你的 `series` 變數。
