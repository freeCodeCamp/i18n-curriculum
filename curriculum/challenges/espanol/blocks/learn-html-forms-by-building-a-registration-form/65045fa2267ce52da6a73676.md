...
Utilice `personal-account`, y `business-account` como valores para los respectivos atributos `id`.
...
El primer elemento `label` debe tener un atributo `for` con un valor de `personal-account`.

```js
assert(document.querySelectorAll('fieldset:nth-of-type(2) label')?.[0]?.matches('label[for="personal-account"]'))
```

El segundo elemento `label` debe tener un atributo `for` con un valor de `business-account`.
