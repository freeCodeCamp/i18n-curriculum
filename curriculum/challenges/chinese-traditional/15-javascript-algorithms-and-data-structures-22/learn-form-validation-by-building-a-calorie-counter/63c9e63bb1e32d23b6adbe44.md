...
當你需要將字符串轉換爲小寫時，可以使用 <dfn>toLowerCase()</dfn> 方法。 此方法返回轉換爲小寫的調用字符串值。
...
你的 `output.innerHTML` 字符串將需要一個 `span` 元素。 創建它，並賦予它一個 `class` 屬性，並將其設置爲 `surplusOrDeficit` 變量。 你的 `surplusOrDeficit` 變量應使用 `toLowerCase()` 方法轉換爲小寫。

暫時不要爲 `span` 添加任何文本。

# --hints--

你應該在模板文字中創建一個 `span` 元素。
...
你的 `span` 應將 `class` 屬性設置爲 `surplusOrDeficit.toLowerCase()`。
