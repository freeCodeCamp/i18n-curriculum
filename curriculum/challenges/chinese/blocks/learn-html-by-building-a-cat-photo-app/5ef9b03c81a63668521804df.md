...
Your radio button should have an `id` attribute. 检查开始标签的名称后面是否有空格和/或所有属性名称之前是否有空格。

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('id'));
```

Your radio button should have a `type` attribute. 检查开始标签的名称后面是否有空格和/或所有属性名称之前是否有空格。

```js
assert.isTrue(document.querySelector('input')?.hasAttribute('type'));
```

Your radio button should have an `id` attribute with the value `indoor`. 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
