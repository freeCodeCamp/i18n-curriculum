...
يجب عليك إعطاء `.foreground-buildings` خاصية `display` بقيمة `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.display, 'flex');
```

يجب عليك إعطاء `.foreground-buildings` خاصية `align-items` بقيمة `flex-end`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.alignItems, 'flex-end');
```

يجب عليك إعطاء `.foreground-buildings` خاصية `justify-content` بقيمة `space-evenly`.
