...
Se você verificar o tamanho do `div` no navegador usando o `content-box` e o `border-box` você notará que há uma diferença muito importante.Estes são os dois `div`s em HTML:
...
Nós atribuímos a eles `id`s para selecioná-los individualmente no CSS.Estas são as regras CSS para o `#red-div` e o `#blue-div`:
