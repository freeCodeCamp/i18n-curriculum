...
如果用户点击 `Cancel` 按钮，你需要取消该过程并关闭模式，以便用户可以继续编辑。 HTML `dialog` 元素具有 `close()` 方法，可用于关闭网页上的模态对话框。

```js
dialogElement.close();
```

向 `cancelBtn` 元素添加事件监听器，并为第一个参数传入 `click` 事件，为第二个参数传入回调函数。

对于回调函数，调用 `confirmCloseDialog` 元素上的 `close()` 方法。

# --hints--

你应该在 `cancelBtn` 变量上调用 `addEventListener()` 方法。
...
你的事件监听器的回调函数应该调用 `confirmCloseDialog` 元素上的 `close()` 方法。
