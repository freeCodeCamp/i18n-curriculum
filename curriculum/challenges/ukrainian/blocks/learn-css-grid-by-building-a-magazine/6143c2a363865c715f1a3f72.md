...
У межах елемента `.image-wrapper` надайте першому елементу `img` атрибут `src` зі значенням `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, атрибут `alt` зі значенням `image of the quote machine project`, атрибут `class` зі значенням `image-1`, атрибут `loading` зі значенням `lazy`, атрибут `width` зі значенням `600` та атрибут `height` зі значенням `400`.
