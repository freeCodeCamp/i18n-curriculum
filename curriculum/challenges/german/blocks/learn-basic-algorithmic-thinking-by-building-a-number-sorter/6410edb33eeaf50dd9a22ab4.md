...
Wenn Ihre `if`-Bedingung wahr ist, müssen Sie die beiden Elemente tauschen und das größere Element dabei bis zum Ende des Arrays „hochblasen“.

Deklarieren Sie dazu eine `temp`-Variable und weisen Sie ihr den Wert von `array[j]` zu. Dann weisen Sie `array[j]` den Wert von `array[j + 1]` zu. Schließlich weisen Sie `array[j + 1]` den Wert von `temp` zu.

# --hints--

Innerhalb Ihrer `if`-Anweisung sollten Sie eine `temp`-Variable deklarieren.
...
Sie sollten `temp` den Wert von `array[j]` zuweisen.
...
Sie sollten `array[j]` den Wert von `array[j + 1]` zuweisen.
...
Sie sollten `array[j + 1]` den Wert von `temp` zuweisen.
