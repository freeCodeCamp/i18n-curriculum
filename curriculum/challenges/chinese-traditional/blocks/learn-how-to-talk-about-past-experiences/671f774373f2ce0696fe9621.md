...
`Speaking of which` is used to link a new topic to something that was just mentioned, often as a natural transition in conversation. 例如：
...
In this context, `manage` means to succeed in doing something, especially something that might have been difficult. 例如：
...
`Cybersecurity` refers to the practice of protecting computer systems, networks, and data from digital attacks or unauthorized access. 例如：
