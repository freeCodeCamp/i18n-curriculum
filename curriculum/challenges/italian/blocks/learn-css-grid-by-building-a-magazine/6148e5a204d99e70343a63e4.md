...
La proprietà `gap` è un modo abbreviato per impostare contemporaneamente il valore di `column-gap` e `row-gap`. Se viene fornito un solo valore, imposta sia `column-gap` che `row-gap` su quel valore. Se vengono forniti due valori, imposta `row-gap` sul primo valore e `column-gap` sul secondo.

Dai al selettore `.image-wrapper` una proprietà `gap` impostata su `2rem`.

# --hints--

Il tuo elemento `.image-wrapper` dovrebbe avere una proprietà `gap` impostata su `2rem`.
