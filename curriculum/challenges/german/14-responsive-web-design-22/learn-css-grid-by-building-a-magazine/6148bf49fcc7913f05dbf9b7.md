...
Dies wird nur für das `a`-Element innerhalb des `.author-name` einen Hover-Effekt erzeugen, dass das ursprüngliche freeCodeCamp-Grün im Hintergrund anzeigt.

# --hints--

Du solltest einen `.author-name a:hover`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

Dein `.author-name a:hover`-Selektor sollte eine `background-color`-Eigenschaft mit dem Wert `#306203` haben.
