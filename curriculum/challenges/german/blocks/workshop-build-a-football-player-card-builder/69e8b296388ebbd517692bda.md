...
Fügen Sie dem Image-URL-`value` ein `player.imageUrl`-Attribut mit dem Wert `input` hinzu. Setzen Sie dann das `onChange`-Attribut auf eine Pfeilfunktion, die `e` als Parameter nimmt und `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `imageUrl` auf `e.target.value` gesetzt ist.

# --hints--

Ihre Image-URL `input` sollte ein `value` haben, das der Bild-URL des Spielers entspricht.
...
Ihre Image-URL `input` sollte einen `onChange`-Handler haben, der `setPlayer` mit einer Ausbreitung von `player` aufruft, wobei `imageUrl` auf `e.target.value` gesetzt ist.
