...
Gehe runter zum `footer`-Element und gib dem gesamten Text einen Wert von `14px` für die Schriftgröße.

# --hints--

Du solltest einen `footer`-Selektor haben.
...
Dein `footer`-Selektor sollte unterhalb deines Kommentars stehen.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Du solltest die `font-size`-Eigenschaft auf `14px` setzen.
...
Dein `footer`-Element sollte eine `font-size` von `14px` haben.
