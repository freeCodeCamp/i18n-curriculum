...
El objetivo del juego es organizar cuatro números de tal manera que, al evaluarlos, el resultado sea 24.

# --instructions--

Implementa una función que tome una cadena de cuatro dígitos como argumento, con cada dígito del 1 al 9 (inclusive) permitiendo repeticiones, y que devuelva una expresión aritmética que dé como resultado el número 24. Si no existe tal solución, devuelve "no existe solución".
...
`solve24` debe ser una función.

```js
assert(typeof solve24 === 'function');
```

`solve24("4878")` debe devolver `(7-8/8)*4`, `4*(7-8/8)` o una cadena válida similar

```js
assert(isValidSolution_(solve24(testCases_[0])));
```

`solve24("1234")` debe devolver `1*2*3*4` o una cadena válida similar

```js
assert(isValidSolution_(solve24(testCases_[1])));
```

`solve24("6789")` debe devolver  `(6*8)/(9-7)`, `(8*6)/(9-7)` o una cadena válida similar

```js
assert(isValidSolution_(solve24(testCases_[2])));
```

`solve24("1127")` debe devolver `(1+7)*(1+2)` o una cadena válida similar
