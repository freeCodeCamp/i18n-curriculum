...
向 `previousButton` 元素添加 `"click"` 事件监听器，然后传入 `playPreviousSong` 作为第二个参数。

# --hints--

你应该在 `previousButton` 变量上调用 `addEventListener()` 方法。
...
你应该将 `playPreviousSong` 作为 `addEventListener` 方法的第二个参数传入。
