...
まず、現在のティックで到着する参加者の数を取得する必要があります。入力ゲートの`queue`配列の`tickIndex`インデックスにある値にアクセスし、`currentTickQueue`という名前の変数に代入してください。

# --hints--

`gate.queue[tickIndex]`の要素を`currentTickQueue`という名前の変数に代入してください。
