...
Como estás usando otro elemento `div`, necesitarás hacer esto más accesible para los lectores de pantalla. 

En una lección anterior, aprendiste sobre el atributo `aria-live` que se usa para crear una región activa en una página. Esto permitirá que los usuarios de lectores de pantalla sean notificados automáticamente cuando el contenido de la región activa cambie, sin necesidad de enfocar o interactuar manualmente con ella.

En tu elemento `#status`, agrega un atributo `aria-live` y establece su valor en `"polite"`. 

El valor `polite` indicará a los lectores de pantalla que esperen hasta que cualquier anuncio actual haya terminado o hasta que el usuario deje de escribir antes de anunciar la actualización.

# --hints--

Tu elemento `#status` debe tener el atributo `aria-live`.

```js
assert.isTrue(document.getElementById("status").hasAttribute("aria-live"));
```

Tu elemento `#status` debe tener el atributo `aria-live` configurado en `"polite"`.
