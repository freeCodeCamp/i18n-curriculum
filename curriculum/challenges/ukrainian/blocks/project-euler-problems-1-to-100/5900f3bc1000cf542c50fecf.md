...
title: 'Завдання 80: десятковий розклад квадратного кореня'
challengeType: 1
forumTopicId: 302194
...
Добре відомо, що якщо квадратний корінь натурального числа не є цілим числом, то він ірраціональний. Десятковий розклад таких квадратних коренів є нескінченним і не має жодного повторюваного шаблону.

Квадратний корінь з двох дорівнює `1.41421356237309504880...`, а сума цифр перших ста десяткових знаків дорівнює `475`.

Для перших `n` натуральних чисел знайдіть суму цифрових сум перших ста десяткових знаків для всіх ірраціональних квадратних коренів.

# --hints--

`sqrtDigitalExpansion(2)` має повертати число.

```js
assert(typeof sqrtDigitalExpansion(2) === 'number');
```

`sqrtDigitalExpansion(2)` має повертати `475`.

```js
assert.strictEqual(sqrtDigitalExpansion(2), 475);
```

`sqrtDigitalExpansion(50)` має повертати `19543`.

```js
assert.strictEqual(sqrtDigitalExpansion(50), 19543);
```

`sqrtDigitalExpansion(100)` має повертати `40886`.
