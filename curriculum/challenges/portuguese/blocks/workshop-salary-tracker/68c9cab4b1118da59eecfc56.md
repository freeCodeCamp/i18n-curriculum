...
Após as declarações `if` existentes, crie outra para lançar um `ValueError` quando `new_level` já for o nível selecionado.
...
Sua declaração `if` deve comparar `new_level` com `self.level`.
