...
الآن بعد أن أصبحت دالة `.map()` تستقبل المرجع العائد للدالة `num => charRange(...).map(...)` من استدعاءات `addCharacters` المجعدة، ستكرر العناصر بشكل صحيح وتمرر كل عنصر كـ `n` إلى تلك الدالة.

ستلاحظ أنك لا تستخدم معلمة `match`. في جافاسكريبت، من الاصطلاح الشائع وضع بادئة الشرطة السفلية `_` على المعلمة غير المستخدمة. يمكنك أيضًا ترك المعلمة فارغة هكذا: `(, char1)` لكن غالبًا ما يكون من أوضح تسمية المعلمة لسهولة القراءة المستقبلية.

ضع بادئة الشرطة السفلية على معلمة `match`.

# --hints--

يجب أن تضع بادئة الشرطة السفلية على معلمة `match`.
