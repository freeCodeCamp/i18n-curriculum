...
为了存储歌曲暂停时的当前时间，请将 `userData` 对象的 `songCurrentTime` 设置为 `audio` 变量的 `currentTime`。

*注意*：你不应该在此步骤中使用可选链接，因为此时 `userData.songCurrentTime` 将不会是 `null` 或 `undefined`。

# --hints--

你不应修改现有的 `pauseSong` 函数及其内容。

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

你应该将 `audio.currentTime` 分配给 `userData.songCurrentTime`。
