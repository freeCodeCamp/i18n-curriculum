...
現在你需要準備更新顯示使用者新增的產品。宣告一個 `currentProductCount` 變數，並將它指定為 `totalCountPerProduct` 物件中與 `product` 的 `id` 相符的屬性值。

# --hints--

你應該在你的 `addItem` 函式中宣告一個 `currentProductCount` 變數。
...
你應該使用 `const` 來宣告 `currentProductCount`。
...
你應該將 `totalCountPerProduct[product.id]` 的值指定給 `currentProductCount`。
