...
هذا يعتني ببعض تنسيق التباعد الأساسي لبطاقة العنصر، بما في ذلك زر الدعوة إلى اتخاذ إجراء (CTA). لبقية المحتوى الداخلي، تحتاج إلى ترتيب كل شيء في شبكة من خلال استهداف `div` الداخلي.

عيّن الأصناف `grid`، و`grid-rows-[auto_auto_auto_1fr]`، و`gap-y-2` إلى `div` الداخلي.

# --hints--

يجب أن يحتوي عنصر `div` الداخلي لخطة العائلة على الصنف `grid`.
...
يجب أن يحتوي عنصر `div` الداخلي لخطة العائلة على الصنف `grid-rows-[auto_auto_auto_1fr]`.
...
يجب أن يحتوي عنصر `div` الداخلي لخطة العائلة على الصنف `gap-y-2`.
