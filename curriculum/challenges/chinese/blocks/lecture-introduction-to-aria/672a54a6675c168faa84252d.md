...
你会注意到 `aria-label` 和 `aria-labelledby` 都以 `aria-` 为前缀。那么这意味着什么呢？ARIA 代表 Accessible Rich Internet Applications（无障碍丰富互联网应用）。它是一组以 `aria-` 为前缀的属性，允许开发者向辅助技术传达元素的用途。`aria-label` 属性是交互元素的一个不可见标签。它为元素添加一个文本标签，屏幕阅读器可以读取这个标签。
...
在这种情况下，按钮的文本被用作搜索输入的标签。屏幕阅读器会将输入读作类似 `Search, edit`。如果你后来决定将按钮文本改为 `Find`，输入的标签会自动更新为新文本，因为它引用了按钮文本。

也可以将多个 `id` 值组合成一个 `aria-labelledby` 属性值。下面是它的工作原理：
