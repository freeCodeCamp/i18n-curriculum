...
Де це доречно, замініть значення властивостей на вашу змінну `--penguin-face`.

# --hints--

Ви повинні надати `.face` властивість `background-color` зі значенням `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Ви повинні надати `.chin` властивість `background-color` зі значенням `var(--penguin-face)`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.getPropVal('background-color', true), 'var(--penguin-face)');
```

Ви не повинні використовувати `var(--penguin-face)` у селекторі `.penguin-body`.
