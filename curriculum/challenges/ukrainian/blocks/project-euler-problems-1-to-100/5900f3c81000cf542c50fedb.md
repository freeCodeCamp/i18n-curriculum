...
title: 'Завдання 92: ланцюжки квадратів цифр'
challengeType: 1
forumTopicId: 302209
...
Ланцюжок чисел створюється шляхом безперервного додавання квадрата цифр у числі для формування нового числа, доки це число не з’явиться знову.
...
Отже, будь-який ланцюжок, що доходить до 1 або 89, застрягне у безкінечному циклі. Найдивовижніше те, що КОЖНЕ початкове число врешті-решт призведе до 1 або 89.

Скільки початкових чисел менше `limit` у підсумку призведуть до 89?

# --hints--

`squareDigitChains(100)` має повертати число.

```js
assert(typeof squareDigitChains(100) === 'number');
```

`squareDigitChains(100)` має повертати `80`.

```js
assert.strictEqual(squareDigitChains(100), 80);
```

`squareDigitChains(1000)` має повертати `857`.

```js
assert.strictEqual(squareDigitChains(1000), 857);
```

`squareDigitChains(100000)` має повертати `85623`.

```js
assert.strictEqual(squareDigitChains(100000), 85623);
```

`squareDigitChains(10000000)` має повертати `8581146`.
