...
オイラーのトーシェント関数、${\phi}(n)$（時にファイ関数とも呼ばれます）は、`n` 未満で `n` と互いに素な数の個数を求めるために使われます。例えば、1、2、4、5、7、8 はすべて 9 未満で 9 と互いに素なので、${\phi}(9) = 6$ となります。
...
`n` = 6 が `n` ≤ 10 の範囲で最大の $\displaystyle\frac{n}{{\phi}(n)}$ を生み出すことがわかります。

`n` ≤ `limit` の範囲で $\displaystyle\frac{n}{{\phi(n)}}$ が最大となる値を見つけてください。
