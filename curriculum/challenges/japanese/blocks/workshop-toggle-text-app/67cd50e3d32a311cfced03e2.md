...
それは動作しますが、段落テキストの可視性を切り替えるために `useState` フックの初期値を手動で更新するのはあまり実用的ではありません。

`useState` フックの初期値を `true` から `false` に更新してください。

次のいくつかのステップで、段落テキストの可視性を切り替えるボタンの機能を作成していきます。

# --hints--

`useState` フックの初期値を `false` に戻す必要があります。
