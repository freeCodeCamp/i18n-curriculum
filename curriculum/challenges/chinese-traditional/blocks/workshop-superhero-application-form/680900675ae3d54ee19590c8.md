...
接著，創建一個 `label`，其 `className` 為 `section` 和 `column`，文字為 `How did you get your powers?`。在 `label` 裡面，創建一個 `select` 元素。
...
你的 `label` 元素應該有文字 `How did you get your powers?`。
...
你應該在你的 `label` 元素內創建一個 `select` 元素。
