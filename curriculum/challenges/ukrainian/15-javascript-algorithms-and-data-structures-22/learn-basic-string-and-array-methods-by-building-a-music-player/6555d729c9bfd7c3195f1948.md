...
`textContent` встановлює текст вузла та дозволяє встановити або отримати текстовий вміст елемента HTML.
...
Можна використовувати тернарний оператор, щоб умовно встановити значення текстового вмісту. Ось приклад призначення результату тернарного оператора до змінної:
...
Ви повинні приєднати властивість `textContent` до `playingSong`.

```js
assert.match(code, /playingSong\.textContent\s*/)
```

Ви повинні використати `ternary`(потрійний) оператор, щоб встановити властивість `textContent` з `playingSong` на `currentTitle` або `""`.

```js
assert.match(code, /playingSong\.textContent\s*=\s*currentTitle\s*\?\s*currentTitle\s*:\s*('|")\1\s*;?/)
```

Ви повинні приєднати властивість `textContent` до `songArtist`.

```js
assert.match(code, /songArtist\.textContent\s*/)
```

Ви повинні використати `ternary`(потрійний) оператор, щоб встановити властивість `textContent` з `songArtist` на `currentArtist` або `""`.
