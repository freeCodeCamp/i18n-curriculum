...
バリデーターが実際の問題を検出する様子を見てみましょう。`catalog[0]` はスティーブン・キングの「From a Buick 8」で、4つのフィールドすべてに値が入っています。`catalog[16]` は「Unknown Title」で、著者フィールドが空白です。

`validateEntry(catalog[0])` と `validateEntry(catalog[16])` をコンソールに出力してください。最初は `true` を返すはずで、2つ目は `false` を返すはずです。

# --hints--

`validateEntry(catalog[0])` をコンソールに出力してください。

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

`validateEntry(catalog[16])` をコンソールに出力してください。
