...
Actualiza tu callback de `.map()` para llamar a la función `Number()`. Pasa `dropdown.value` a esa llamada de función.

Abre la pestaña de la Consola para ver que tu `inputValues` es ahora un arreglo de números.

# --hints--

Deberías llamar a la función `Number()` en tu callback de `.map()`.
...
Deberías pasar `dropdown.value` a tu llamada de `Number()`.
