...
あなたの `inner` 関数で、`idToText` を引数として `character + num` を呼び出した結果を返してください。

# --hints--

あなたの `inner` 関数は明示的な return を使うべきです。
...
あなたの `inner` 関数は、`idToText` 関数を呼び出した結果を返すべきです。
...
`character + num` を引数として `idToText` 関数に渡すべきです。
