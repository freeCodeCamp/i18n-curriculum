...
Прості множники числа 13195 — це 5, 7, 13 і 29.

Який найбільший простий множник заданого `number`?

# --hints--

`largestPrimeFactor(2)` має повертати число.

```js
assert.isNumber(largestPrimeFactor(2));
```

`largestPrimeFactor(2)` має повертати 2.

```js
assert.strictEqual(largestPrimeFactor(2), 2);
```

`largestPrimeFactor(3)` має повертати 3.

```js
assert.strictEqual(largestPrimeFactor(3), 3);
```

`largestPrimeFactor(5)` має повертати 5.

```js
assert.strictEqual(largestPrimeFactor(5), 5);
```

`largestPrimeFactor(7)` має повертати 7.

```js
assert.strictEqual(largestPrimeFactor(7), 7);
```

`largestPrimeFactor(8)` має повертати 2.

```js
assert.strictEqual(largestPrimeFactor(8), 2);
```

`largestPrimeFactor(13195)` має повертати 29.

```js
assert.strictEqual(largestPrimeFactor(13195), 29);
```

`largestPrimeFactor(600851475143)` має повертати 6857.
