...
이제 건물과 비슷한 무언가가 생겼습니다. 첫 변수를 만들 준비가 되었습니다. 변수 선언은 두 개의 대시(`-`)로 시작하며, 이름과 값을 이렇게 지정합니다: `--variable-name: value;`. `bb1` 클래스 규칙에서 `--building-color1`라는 이름의 변수를 만들고 `#999` 값을 할당하세요.
...
`--building-color1` 안에 `.bb1` 변수를 정의해야 합니다.
