...
`defeatMonster`関数では、`gold` の値を `gold` + モンスターのレベル × `6.7` に設定してください。 モンスターのレベルは `monsters[fighting].level` で取得できます。

例えば、`num` の値を `num` + `5` * `8` に設定する場合 `num += 5 * 8` となります。 `Math.floor()` を使用して結果を切り下げてください。

# --hints--

複合代入を使用して、`gold` の値を変更してください。

```js
assert.match(defeatMonster.toString(), /gold\s*\+=/);
```

`Math.floor()` を使用して、モンスターのレベル × `6.7` の結果を切り下げてください。
...
`Math.floor(monsters[fighting].level * 6.7)` の結果を `gold` に足してください。
