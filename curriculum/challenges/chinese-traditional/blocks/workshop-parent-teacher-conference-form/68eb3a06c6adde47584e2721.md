...
選取具有類別 `submit-btn` 的元素。將 `cursor` 設為 `pointer`（以顯示它是可點擊的）、`background-color` 設為 `royalblue`、`color` 設為 `whitesmoke`、`border` 設為 `none`、`border-radius` 設為 `6px`，以及 `padding` 設為 `12px 20px`。

# --hints--

你應該標的具有類別 `submit-btn` 的元素。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.submit-btn'));
```

將 `cursor` 屬性設為 `pointer`。
...
將 `background-color` 屬性設為 `royalblue`。
