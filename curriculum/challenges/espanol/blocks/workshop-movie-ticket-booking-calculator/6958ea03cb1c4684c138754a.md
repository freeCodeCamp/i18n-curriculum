...
Al final del cuerpo de tu último `if` statement, calcula el precio final del ticket y guárdalo en una variable llamada `final_price`.

Finalmente, imprime un mensaje que muestre `Final price of ticket:` seguido del valor de `final_price`.

# --hints--

Debes declarar una variable llamada `final_price`.

```js
({ test: () => (runPython(`assert _Node(_code).find_ifs()[4].find_bodies()[0].has_variable("final_price")`)) })
```

Debes asignar el valor `base_price + extra_charges + service_charges - discount` a `final_price`.
...
Debes tener `print('Final price of ticket:', final_price)` debajo de tu variable `final_price`.
