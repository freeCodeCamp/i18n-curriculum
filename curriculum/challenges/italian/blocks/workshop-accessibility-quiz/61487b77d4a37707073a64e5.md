...
Quando la larghezza dello schermo è piccola, il `h1` non va a capo nel suo contenuto testuale come dovrebbe. Allinea il testo per l'elemento `h1` al centro.

Poi, dai al `main` una spaziatura interna tale che l'intestazione della sezione `Student Info` possa essere vista completamente.

# --hints--

Dovresti dare al `h1` un `text-align` di `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

Dovresti aggiungere un selettore `main` per mirare all'elemento `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Dovresti dare al `main` una `padding-top` di almeno `25px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

Dovresti modificare solo il valore `padding-top`.
