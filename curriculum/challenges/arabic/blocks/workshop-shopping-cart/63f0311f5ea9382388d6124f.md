...
المشكلة في أن `.toFixed()` تُرجع سلسلة نصية هي أنك تريد إجراء حسابات على معدل الضريبة. لإصلاح ذلك، يمكنك تمرير استدعاء `.toFixed()` (بما في ذلك الحساب) إلى الدالة `parseFloat()`. هذا سيُحوّل السلسلة النصية المثبتة إلى رقم مرة أخرى، مع الحفاظ على المنازل العشرية الموجودة.
