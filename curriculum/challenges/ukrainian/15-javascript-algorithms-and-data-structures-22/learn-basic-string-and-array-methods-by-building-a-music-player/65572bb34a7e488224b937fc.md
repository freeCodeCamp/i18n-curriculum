...
Потрібно додати `resetText` до елемента `resetButton` як дочірній елемент, а також `resetButton` до елемента `playlistSongs` як дочірній елемент. Для цього існує метод `appendChild()` для використання.

`appendChild()` дозволяє додавати вузол або елемент як дочірній елемент іншого елемента. У наведеному нижче прикладі текст «Click me» буде додано до кнопки:
...
Використайте `appendChild()`, щоб приєднати `resetText` до елемента `resetButton` та `resetButton` до елемента `playlistSongs`.

# --hints--

Використайте метод `appendChild()` на `resetButton`.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

Передайте `resetText` як значення першого `appendChild()`.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

Використайте метод `appendChild()` на `playlistSongs`.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

Передайте `resetButton` як значення другого `appendChild()`.
