...
Для цього алгоритму ви захочете використати цикл `while`. Цей цикл потребує двох умов:

- По-перше, він не повинен вибігати за межі початку масиву (використовується з `j`).
- По-друге, цикл не повинен працювати після того, як він знайде значення, менше поточного значення.

Щоб запобігти нескінченному циклу, зменште `j` у циклі.
...
Цикл `while` повинен мати свою першу умову, яка перевіряє, що значення `j` більше або дорівнює `0`.
...
Цикл `while` має використовувати оператор ОБ’ЄДНУВАННЯ.
...
Цикл `while` повинен мати умову, яка перевіряє, що значення `array[j]` більше або дорівнює `currValue`.
...
Цикл `while` має зменшити `j` в межах циклу.
