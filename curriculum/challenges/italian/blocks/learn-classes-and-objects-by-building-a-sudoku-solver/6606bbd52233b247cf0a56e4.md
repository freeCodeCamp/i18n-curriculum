...
Ora, trasforma di nuovo il `1` che hai modificato prima in un `0` per ripristinare la configurazione originale della scacchiera. Poi, elimina la tua chiamata `print`.

# --hints--

Dovresti trasformare il primo elemento della prima lista di `puzzle` di nuovo in uno zero.
...
Non dovresti avere `print(gameboard.find_empty_cell())` nel tuo codice.
