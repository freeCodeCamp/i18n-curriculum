...
`useEffect` のコールバック内で、`if` の条件を追加して `query.trim() === ''` かどうかを確認してください。これは `query` が空の文字列か、空白のみを含んでいるかをチェックします。そうであれば、`setResults([])` を呼び出して早期に戻ります。

# --hints--

`query.trim() === ''` かどうかを確認してください。
...
`query.trim() === ''` が true の場合、空の配列で `setResults()` を呼び出し、その後に `return` キーワードを使ってください。
