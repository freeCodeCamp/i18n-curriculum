...
네비게이션 버튼이 일반적인 버튼처럼 보이도록 앵커 태그의 밑줄을 제거하세요.

그다음, 네비게이션 목록 요소를 대상으로 하는 새로운 선택자를 만들어서 커서가 위에 있을 때 배경색과 글자색이 바뀌고, 커서 모양이 포인터가 되도록 하세요.

# --hints--

기존 `li > a` 선택자를 사용해 `text-decoration`을 `none`로 설정하세요.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

`nav > ul > li:hover` 또는 `nav li:hover` 선택자 중 하나를 사용해 호버 상태의 요소를 스타일링하세요.
...
호버된 `li` 요소에 `background-color`를 `#dfdfe2`로 지정하세요.
...
호버된 `li` 요소에 `color`를 `#1b1b32`로 지정하세요.
...
호버된 `li` 요소에 `cursor`를 `pointer`로 지정하세요.
