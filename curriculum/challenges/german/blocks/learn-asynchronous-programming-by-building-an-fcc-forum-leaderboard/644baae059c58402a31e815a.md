...
Wenn das `id` nicht im `allCategories`-Objekt enthalten ist, müssen Sie die `General`-Kategorie anzeigen.

Fügen Sie unter Ihrer `if`-Anweisung eine `else`-Klausel hinzu.

# --hints--

Sie sollten eine `else`-Klausel unter Ihrer `if`-Anweisung haben.
