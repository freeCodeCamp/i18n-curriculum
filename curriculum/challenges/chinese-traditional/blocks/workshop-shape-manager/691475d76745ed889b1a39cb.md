...
你也應該著手建立一個函式，用來清除輸入欄位。

創建一個 `clearInputFields` 函式。在裡面，使用 `querySelectorAll` 取得所有輸入欄位，並用 `forEach()` 迴圈遍歷它們。在 `forEach()` 裡，將每個 `input` 的值設為空字串。

# --hints--

你應該創建一個 `clearInputFields` 函式。

```js
assert.isFunction(clearInputFields)
```

你的 `clearInputFields` 函式應該將每個輸入欄位設為空字串。
