...
對於遞迴情況，為你的程式添加一個 `else` 述語，並 `return` 呼叫 `decimalToBinary()` 的結果。將使用 `Math.floor()` 向下取整後的 `input` 除以 2 作為引數傳入。

# --hints--

你應該將你的 `if` 述語轉換成 `if...else` 述語。

```js
assert.match(String(decimalToBinary), /\{\s*return\s+('|"|`)\1\s*;?\s*\}\s*else\s*\{/);
```

在你的 `else` 述語主體中，你應該 `return` 呼叫 `decimalToBinary` 的結果。

```js
assert.match(String(decimalToBinary), /else\s*\{\s*return\s+decimalToBinary\(/);
```

你應該將除以 `2` 並用 `Math.floor()` 無條件捨去的小數部分的 `input` 作為引數傳入 `decimalToBinary`。
