...
Після вашого останнього `.divider` створіть ще один елемент `p` з текстом `Trans Fat 0g`. Виділіть курсивом слово `Trans`, обгорнувши його в елемент `i`. Додайте новому елементу `p` атрибут `class` зі значенням `indent no-divider`. Обгорніть `Trans Fat 0g` в елемент `span` для вирівнювання.
...
Ваш новий елемент `p` має містити текст `Trans Fat 0g`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText === 'Trans Fat 0g');
```

Ваш новий елемент `p` має мати атрибут `class` зі значенням `indent no-divider`.
...
Ваш елемент `span` має обгорнути текст `Trans Fat 0g`.
