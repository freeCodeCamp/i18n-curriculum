...
Target the `.arm` element with a `class` of `left`, and position it `35%` from the top, and `5%` from the left of its parent. Luego, apunte al elemento `.arm` con una `class` de `right`, y colóquelo `0%` desde la parte superior, y `-5%` desde la derecha de su padre.
