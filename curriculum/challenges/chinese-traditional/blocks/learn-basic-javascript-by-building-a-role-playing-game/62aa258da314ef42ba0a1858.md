...
創建兩個名為 `pickTwo` 和 `pickEight` 的新函式（程式）。

在每一個裡面，呼叫 `pick()` 函式（程式），並根據函式（程式）名稱傳入 `2` 或 `8` 作為引數。

# --hints--

你應該使用 `function` 關鍵字來宣告 `pickTwo`。

```js
assert.match(code, /function\s+pickTwo\s*/);
```

你應該使用 `function` 關鍵字來宣告 `pickEight`。

```js
assert.match(code, /function\s+pickEight\s*/);
```

`pickTwo` 應該呼叫 `pick` 函式（程式）。

```js
assert.match(pickTwo.toString(), /pick\(/);
```

`pickTwo` 應該以 `2` 作為引數來呼叫 `pick` 函式。

```js
assert.match(pickTwo.toString(), /pick\(\s*2\s*\)/);
```

`pickEight` 應該呼叫 `pick` 函式（程式）。

```js
assert.match(pickEight.toString(), /pick\(/);
```

`pickEight` 應該以 `8` 作為引數來呼叫 `pick` 函式。
