...
ユーザーがプロンプトを確認した場合にのみカートをクリアしたいです。ユーザーがプロンプトを確認したかどうかをチェックする`if`文を作成してください。

`if`文の中で、`items`プロパティを空の配列に戻し、次に`total`プロパティを`0`に設定してください。

# --hints--

ユーザーがプロンプトを確認したかどうかをチェックする`if`文を作成してください。`isCartCleared`の真偽値を直接チェックできることを覚えておいてください。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

`if`文は`items`プロパティを空の配列に戻す必要があります。

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

`if`文は`total`プロパティを`0`に設定する必要があります。
