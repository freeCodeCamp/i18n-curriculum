...
Se vuoi aggiungere altre icone social ma mantenerle sulla stessa riga, devi aggiornare `grid-template-columns` per creare colonne aggiuntive. In alternativa, puoi usare la proprietà `grid-auto-flow`.

Questa proprietà accetta come primo valore `row` o `column`, con un secondo valore opzionale `dense`. `grid-auto-flow` usa un algoritmo di posizionamento automatico per adattare il layout della griglia. Impostarlo su `column` indica all'algoritmo di creare nuove colonne per il contenuto secondo necessità. Il valore `dense` permette all'algoritmo di tornare indietro e riempire gli spazi vuoti nella griglia con elementi più piccoli, il che può far apparire gli elementi fuori ordine.

Per il tuo selettore `.social-icons`, imposta la proprietà `grid-auto-flow` su `column`.

# --hints--

Il tuo selettore `.social-icons` dovrebbe avere la proprietà `grid-auto-flow` impostata su `column`.
