...
Dentro do `thead`, crie um elemento `tr`. In that, add a `td` and three `th` elements.Dentro de cada um dos elementos `th`, adicione um elemento `span` com a `class` definida como `sr-only` e os seguintes textos, em ordem: `2019`, `2020` e `2021`.
