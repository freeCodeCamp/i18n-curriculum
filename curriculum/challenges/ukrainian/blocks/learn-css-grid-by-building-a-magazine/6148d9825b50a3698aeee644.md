...
Створіть селектор `.lists` і встановіть властивість `list-style-type` у значення `none`. Це прибере маркери списку з елементів списку.

# --hints--

У вас має бути селектор `.lists`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists'));
```

Ваш селектор `.lists` повинен мати властивість `list-style-type`, встановлену в `none`.
