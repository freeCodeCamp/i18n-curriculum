...
接著，為你的程式碼添加一個 `else if` 述語，以檢查是否按下了左鍵且 `isCheckpointCollisionDetectionActive` 為 true。

在該條件內，為你的 `platforms` 陣列添加一個 `forEach` 迴圈來迭代。

在迴圈內，使用加法指定運算子將 5 加到平台的 `x` 位置。

# --hints--

你應該有一個條件來檢查是否按下了左鍵，且 `isCheckpointCollisionDetectionActive` 為 true。
