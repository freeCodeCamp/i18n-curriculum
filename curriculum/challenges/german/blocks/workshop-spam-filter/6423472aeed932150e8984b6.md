...
Verwenden Sie dieselbe Syntax, um `c` so zu aktualisieren, dass sie zu `c`, `{`, `[` und `(` passt.

# --hints--

Ihr `stockRegex` sollte eine Zeichenklasse verwenden, um `c`, `{`, `[` und `(` zu erfassen.
...
Ihr `stockRegex` sollte zu `570(k 4l3r7` passen.

```js
assert.match('570(k 4l3r7', stockRegex);
```

Ihr `stockRegex` sollte zu `sto{k alert` passen.
