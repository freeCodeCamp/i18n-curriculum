...
Los valores RGB pueden variar de 0 a 255.  The alpha value can range from 1, which is fully opaque or a solid color, to 0, which is fully transparent or clear. `rgba()` is great to use in this case, as it allows you to adjust the opacity. Esto significa que no tienes que bloquear completamente el fondo.
