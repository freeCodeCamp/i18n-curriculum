...
你需要再次让卡牌随机且反转。声明两个变量，分别命名为 `isReversed` 和 `card`。对于第一个变量，调用 `Math.random` 方法并与 `< 0.5` 进行比较。对于第二个变量，调用 `getRandomItem` 并传入 `this.cards`。

# --hints--

你应该声明一个名为 `isReversed` 的变量。
...
你的 `isReversed` 变量应该使用 `Math.random()` 来随机决定卡牌是否反转。
...
你的 `card` 变量应该通过调用 `getRandomItem` 方法并传入 `this.cards` 作为参数来赋值。
