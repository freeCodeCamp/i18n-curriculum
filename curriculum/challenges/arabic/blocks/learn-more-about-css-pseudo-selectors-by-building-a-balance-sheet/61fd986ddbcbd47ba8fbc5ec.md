...
داخل العنصر الثالث `tr`، أضف عنصر `th` بالنص `Credit The outstanding balance on our credit card.`. لف ذلك النص، ما عدا `Credit `، داخل عنصر `span` مع تعيين `class` إلى `description`.

أضف ثلاثة عناصر `td` أسفله، وأعطها النصوص التالية بالترتيب: `$50`، `$50`، و`$75`. عيّن للعنصر الثالث `td` الخاصية `class` بقيمة `current`.

# --hints--

يجب أن يحتوي العنصر الثالث `tr` على عنصر `th`.

```js
assert(document.querySelectorAll('table')?.[1]?.querySelector('tbody')?.querySelectorAll('tr')?.[2]?.querySelector('th'));
```

يجب أن يحتوي عنصر `th` على النص `Credit The outstanding balance on our credit card.`.
...
يجب أن تلف النص `The outstanding balance on our credit card.` داخل عنصر `span`.
