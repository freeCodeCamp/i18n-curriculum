...
Ongeza vipengele viwili vya `span` vyenye `className` ya `stat-value` na `stat-label` mtawaliwa kwa kila `div.stat-row`.

# --hints--

Kila `.stat-row` inapaswa kuwa na `span` yenye `className` ya `stat-value` na `span` yenye `className` ya `stat-label`.
