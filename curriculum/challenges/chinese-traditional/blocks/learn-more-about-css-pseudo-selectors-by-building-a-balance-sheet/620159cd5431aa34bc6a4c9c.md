...
以 `h1 .flex` 選擇器標的你的 flex 容器。為它設定 `display` 屬性為 `flex` 以啟用 flexbox 版面配置。然後將 `flex-direction` 屬性設定為 `column-reverse`－這會從下到上顯示巢狀元素。最後，將 `gap` 屬性設定為 `1rem`，以在元素之間產生一些間距。
