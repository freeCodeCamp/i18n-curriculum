...
向 `animationData` 數組添加另一個對象。 你的新對象應將屬性 `inputVal` 和 `addElDelay` 分別設置爲 `1` 和 `2000`。 記住將 `animationData` 數組視爲堆棧，並將新對象添加到堆棧的頂部。

# --hints--

`animationData` 數組的長度應爲 `3`。
...
你應該將 `addElDelay` 屬性的值設置爲數字 `2000`。
