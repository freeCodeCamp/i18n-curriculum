...
你應該最佳化你的程式碼。將 `.foreground-buildings` 的 `position` 和 `top` 屬性及其值移動到 `.background-buildings`。然後在那裡選取 `.background-buildings` 和 `.foreground-buildings`，有效地將那些樣式套用到這兩個元素。你可以使用逗號（`,`）來分隔像這樣的選擇器：`selector1, selector2`。
