...
`tbody` の中に、`tr` が `class` に設定された `total` を追加してください。その中に、テキストが `th` の `Total Net Worth` を追加し、`Net Worth` を `span` が `class` に設定された `sr-only` で囲んでください。

次に、3つの `td` 要素を追加し、3つ目には `class` が `current` に設定されているものを与え、それぞれに次のテキストを設定してください：`$-171`、`$136`、`$334`。

# --hints--

あなたの `tbody` 要素は、1つの `tr` 要素を持つべきです。
...
あなたの `tr` 要素は、`class` が `total` に設定されているべきです。
...
あなたの `th` 要素はテキスト `Total Net Worth` を持つべきです。
...
テキスト `Net Worth` は `span` 要素で囲むべきです。
