...
Pasando al `fieldset` final. ¿Qué pasaría si quisieras permitir que un usuario suba una foto de perfil?

Bueno, el tipo `input` tipo `file` permite exactamente eso. Agrega una `label` con el texto `Subir una foto de perfil:`, y anida un `input` que acepte una carga de archivo.

# --hints--

Debe agregar una `label` con el texto `Subir una foto de perfil:`.
