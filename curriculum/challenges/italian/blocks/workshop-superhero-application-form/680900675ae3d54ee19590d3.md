...
Hai notato che puoi inviare il modulo senza compilare gli input?

Per evitarlo, aggiungi l'attributo `disabled` per disabilitare il pulsante di invio se `heroName`, `realName` o `powerSource` sono falsi, oppure se la lunghezza di `powers` è `0`.

Ora, il pulsante sarà disabilitato se qualcosa non è compilato. Compila il modulo con le informazioni del tuo supereroe e invialo per unirti alla lega.

Con questo, il modulo della tua applicazione per supereroi è completo.

# --hints--

Non dovresti modificare il contenuto esistente del pulsante.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2/)
```

Il tuo elemento `button` dovrebbe avere una proprietà `disabled`.

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

Dovresti disabilitare il `button` se uno qualsiasi di `heroName`, `realName` e `powerSource` è falso, oppure se la lunghezza di `powers` è `0`.
