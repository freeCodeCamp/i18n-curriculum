...
Agora que o setter `name` foi criado, atualize `__init__` para usar `self.name = name` em vez de `self._name = name`. Isso delega a atribuição `name` pelo setter, e a validação será adicionada a ele no próximo passo.
