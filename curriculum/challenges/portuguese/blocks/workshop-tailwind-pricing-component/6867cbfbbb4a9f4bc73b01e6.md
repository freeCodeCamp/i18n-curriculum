...
Os marcadores dos itens da lista precisam estar um pouco mais espaçados.Então, dê a cada `span` nos itens da lista as classes `text-green-700` e `mr-2`.

Além disso, adicione o atributo `aria-hidden` a cada um dos elementos `span`.

# --hints--

Seus elementos `span` devem ter a classe `text-green-700`.
