...
Kama hoja ya rangi ya pili, pita kitendakazi cha `hsl` na thamani za `223` kwa hue, `90%` kwa saturation, na `60%` kwa uangavu.

# --hints--

Kanuni yako ya `.blue` CSS inapaswa kuwa na sifa ya `background` iliyowekwa kuwa `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%))`.
