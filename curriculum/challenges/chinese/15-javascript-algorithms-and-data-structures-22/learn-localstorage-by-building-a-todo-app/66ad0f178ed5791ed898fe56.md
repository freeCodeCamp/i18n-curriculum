...
还剩下两个错误需要修复。 首先，如果你创建的任务标题仅有空格，则即使标题是必需的，也会创建一个空任务。

通过在函数开头添加一个检查，查看是否 `!titleInput.value.trim()`，来修复这一 点。 如果没有标题，显示警报，文本为 `Please provide a title`，之后是 `return`。

# --hints--

你应该在 `addOrUpdateTask` 函数中添加一个带有条件 `!titleInput.value.trim()` 的 `if` 语句。

```js
assert.match(code, /if\s*\(\s*!titleInput\.value\.trim\(\s*\)\s*\)\s*/)
```

你应该在 `if` 块中调用 `alert("Please provide a title")`。
