...
Sua caixa de seleção ainda deve ter um atributo `id` com o valor `loving`. Esperado `--fcc-expected--` mas encontrado `--fcc-actual--`.
...
Seu elemento `label` deve ter um atributo `for` com o valor `loving`. Esperado `--fcc-expected--` e encontrado `--fcc-actual--`.
