...
你需要根据计算的份数进行调整。这是使用另一个柯里的函数的绝佳机会。

声明一个 `adjustForServings` 函数，它接受一个 `baseQuantity` 参数。它应返回第二个函数，该函数接受一个 `newServings` 参数。

内部函数应返回 `baseQuantity` 和 `newServings` 相乘的结果。

# --hints--

你应该声明一个 `adjustForServings` 函数。

```js
assert.isFunction(adjustForServings);
```

你的 `adjustForServings` 函数应接受一个 `baseQuantity` 参数。

```js
assert.match(adjustForServings?.toString(), /\(baseQuantity\)/);
```

你的 `adjustForServings` 函数应返回一个函数。

```js
assert.isFunction(adjustForServings?.());
```

你的内部函数应有一个 `newServings` 参数。

```js
assert.match(adjustForServings?.()?.toString(), /\(newServings\)/);
```

你的内部函数应返回 `baseQuantity` 和 `newServings` 相乘的结果。
