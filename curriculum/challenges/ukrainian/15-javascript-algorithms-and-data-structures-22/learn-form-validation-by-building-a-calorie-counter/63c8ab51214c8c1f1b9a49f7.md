...
У новій функції оголосите змінну `calories` та присвойте до неї значення `0`. Використайте `let`, щоб оголосити її, оскільки ви присвойте її ще раз пізніше.
...
Змінній `calories` потрібно призначити значення `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Оголосіть змінну `calories` за допомогою `let`.
