...
Übergeben Sie eine leere Callback-Funktion an die `.forEach()`-Methode. Destrukturieren Sie für den ersten Parameter des Callbacks die `author`, `image`, `url` und `bio`-Elemente.

Für den zweiten Parameter übergeben Sie `index`. Dieser stellt die Position jedes Autors dar und wird später für die Paginierung nützlich sein.
...
Sie sollten `author`, `image`, `url` und `bio` als ersten Parameter Ihrer Callback-Funktion destrukturieren. Vergessen Sie nicht, sie in geschweifte Klammern zu setzen.
...
Sie sollten `index` als zweiten Parameter Ihrer Callback-Funktion destrukturieren. Dieser muss außerhalb der geschweiften Klammern stehen, in die Sie die anderen gesetzt haben.

```js
assert.match(code, /authors\.forEach\(\s*\(\s*\{.*\}\s*,\s*index\s*\)/)
```

Ihre Callback-Funktion sollte eine Pfeilfunktion sein.
