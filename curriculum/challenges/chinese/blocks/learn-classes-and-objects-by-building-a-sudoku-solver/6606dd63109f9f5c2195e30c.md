...
调用 `valid_in_square()` 方法（以 `row`、`col` 和 `num` 作为参数）检查该数字是否适合插入到指定的 3x3 正方形中，并将结果分配给变量 `valid_in_square`。

# --hints--

你应该声明一个变量 `valid_in_square` 并给它赋值 `self.valid_in_square(row, col, num)` 。
