...
Sie können eine <dfn>Referenz</dfn> auf eine Funktion als Callback-Parameter übergeben. Eine Funktionsreferenz ist der Funktionsname ohne die runden Klammern. Zum Beispiel:
...
Die `.map()`-Methode ruft hier die `myFunc`-Funktion auf und übergibt dieselben Argumente, die ein `.map()`-Callback erwartet. Das erste Argument ist der Wert des Arrays bei der aktuellen Iteration, also wäre `newArray` `[value: 1, value: 2, value: 3]`.

Übergeben Sie eine Referenz auf Ihre `elemValue`-Funktion als Callback an Ihre `.map()`-Methode.

# --hints--

Sie sollten Ihre `elemValue`-Funktion nicht aufrufen.
...
Sie sollten eine Referenz auf `elemValue` als Callback an Ihre `.map()`-Methode übergeben.
