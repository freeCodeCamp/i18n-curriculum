...
选择排序依赖于跟踪数组中最小值的索引。 声明一个变量 `minIndex` 并将其设置为 `i`——这确保了如果当前值是最小的，它将与自身交换而不会移动。 在迭代数组时，你需要能够重新分配 `minIndex` 的值。

然后，写另一个使用 `j` 作为迭代器的 `for` 循环。 该循环需要从 `i` 之后的索引开始，并迭代数组的剩余部分。

# --hints--

你应该使用 `let` 来声明 `minIndex`。
