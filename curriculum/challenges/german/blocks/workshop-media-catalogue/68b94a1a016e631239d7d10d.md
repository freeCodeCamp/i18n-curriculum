...
Zurück zur `MediaCatalogue`-Klasse: Fügen Sie ihr eine Docstring hinzu, die `A catalogue that can store different types of media items.` sagt.

# --hints--

Sie sollten der `A catalogue that can store different types of media items.`-Klasse die Docstring `MediaCatalogue` hinzufügen.
