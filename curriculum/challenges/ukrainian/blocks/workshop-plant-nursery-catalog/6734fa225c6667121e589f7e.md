...
Ваша `sellPlants` функція викликає `TypeError`, тому що намагається прочитати властивість об’єкта, який було видалено з каталогу. Тож вам потрібно врахувати випадок, коли ключ не знайдено у вашій `catalog` мапі. Для цього корисний метод `has`. Він повертає булеве значення, яке вказує, чи існує заданий ключ у мапі.

Змініть вашу `sellPlants` функцію так, щоб вона повертала `Item not found.`, коли каталог не містить рослину, передану у функцію.

# --hints--

Ваша `sellPlants` функція має викликати метод `has` на `catalog`.
...
Ви не повинні безпосередньо змінювати пари ключ-значення, вставлені у вашу `catalog` мапу.
...
`sellPlants(ballerina, "small", 10)` має повертати `Item not found.`

```js
assert.equal(sellPlants(ballerina, "small", 10), "Item not found.")
```

`sellPlants(hidcote, "large", 95)` має повертати `Not enough large size pots for Lavandula angustifolia 'Hidcote'. Only 18 left.`
...
`sellPlants(hidcote, "large", 5)` має повертати `Catalog successfully updated.` і віднімати `5` від властивості `large` об’єкта, пов’язаного з ключем `hidcote` у каталозі.
