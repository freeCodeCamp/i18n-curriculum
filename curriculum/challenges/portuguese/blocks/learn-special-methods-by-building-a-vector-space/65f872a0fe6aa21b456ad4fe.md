...
Finalmente, passe a chamada `tuple()` como argumento para a função `str()`. 

# --hints--

Você deve passar a chamada `tuple()` que você está retornando atualmente do método `__str__` para a função `str()`.
