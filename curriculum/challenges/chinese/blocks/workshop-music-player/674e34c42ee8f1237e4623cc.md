...
当点击 `playButton` 且 `userData.currentSong` 不为 `null` 时，你应该使用当前播放歌曲的 `id` 调用 `playSong` 函数。
...
当 `playButton` 被点击且 `userData.currentSong` 为 `null` 时，你仍然应该使用播放列表中第一首歌曲的 `id` 调用 `playSong` 函数， 
