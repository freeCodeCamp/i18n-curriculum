...
你想讓 `select` 元素保持白色背景，但現在它沒有獲得與 `input` 和 `textarea` 元素相同的 `min-height`。

將 `min-height` 屬性和值移動，使所有三種元素型別具有相同的 `min-height` 值，且 `select` 元素仍然保持白色背景。

# --hints--

你應該將 `min-height` 屬性和值移動到 `input, textarea, select` 選擇器。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight);
```

你應該給 `input, textarea, select` 選擇器設定 `2em` 的 `min-height`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight, '2em');
```

你應該從 `input, textarea` 選擇器中移除 `min-height` 宣告。
