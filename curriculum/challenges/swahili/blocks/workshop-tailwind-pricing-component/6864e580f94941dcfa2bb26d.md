...
Sasa unapaswa kufanya kazi juu ya upangaji wa vitu ndani ya kadi kwa mpango.

Ili kufanya hivyo, weka kontena litumie muonekano wa gridi na bainisha ukubwa maalum wa safu kwa kutumia mabano ya mraba katika darasa lako la Tailwind.

Hivi ndivyo unavyoweza kuweka safu maalum ya gridi kwa kutumia sehemu za kihesabu:
...
Toa `div` ya nje kwa mpango na darasa la huduma `grid` na `grid-rows-[1fr_auto]`.

# --hints--

Kipengele chako cha `div` kinapaswa kuwa na darasa `grid`.
...
Kipengele chako cha `div` kinapaswa kuwa na darasa `grid-rows-[1fr_auto]`.
