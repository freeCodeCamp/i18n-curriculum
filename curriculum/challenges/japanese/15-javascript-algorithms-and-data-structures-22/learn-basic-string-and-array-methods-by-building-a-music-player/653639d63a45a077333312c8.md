...
`if` 文を追加して、「`userData?.currentSong` が falsy *または* `userData?.currentSong.id` が `song.id` と厳密に等しくない」という条件をチェックしてください。 この条件は、「現在再生中の曲がないまたは再生中の曲がこれから再生するものと異なる」という状態かどうかをチェックします。

`if` ブロック内では、`audio` オブジェクトの `currentTime` プロパティを `0` に設定してください。

# --hints--

`userData?.currentSong === null || userData?.currentSong.id !== song.id` という条件の `if` 文を作成してください。 オプショナルチェーンを含めるのを忘れないでください。
...
`if` ブロック内で `audio.currentTime` を `0` に設定してください。
