...
Declara una variable `str2` y asígnale el resultado de la llamada a `infixEval()` con `noHigh` y `infix` como argumentos.
...
Debes asignar a `str2` el resultado de la llamada a `infixEval()`.
...
Debes pasar `noHigh` como primer argumento a `infixEval()`.
...
Debes pasar `infix` como segundo argumento a `infixEval()`.
