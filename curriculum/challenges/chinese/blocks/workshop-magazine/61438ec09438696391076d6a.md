...
在 `.heading` 元素下方，创建一个新的 `section` 元素，并将 `class` 设置为 `text`。 在其中，创建一个 `p` 元素，将 `class` 设置为 `first-paragraph`，并添加以下文本：
...
你的新 `section` 元素应将 `class` 设置为 `text`。
...
你的新 `p` 元素应将 `class` 设置为 `first-paragraph`。
