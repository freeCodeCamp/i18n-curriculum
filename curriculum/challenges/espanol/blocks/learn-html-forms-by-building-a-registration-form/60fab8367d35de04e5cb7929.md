...
Debes usar un elemento `a` para vincular a los términos y condiciones. 
...
Debes dar al elemento `a` un atributo `href` con `https://www.freecodecamp.org/news/terms-of-service/`.
...
Solo debe envolver el elemento `a` alrededor del texto `terms and conditions`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) + label > input + a')?.textContent.trim(), 'terms and conditions');
```

El texto dentro de tu elemento anchor tiene espacio en blanco extra inicial o final. Los únicos espacios en el texto de anclaje deben estar entre las palabras `terms`, `and`, y `conditions`.
