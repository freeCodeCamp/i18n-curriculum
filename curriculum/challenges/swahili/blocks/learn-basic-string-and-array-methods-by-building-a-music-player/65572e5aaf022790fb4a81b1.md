...
Sasa, ni wakati wa kuongeza utendakazi wa kuweka upya kwenye `resetButton`. Hii itarejesha nyimbo kwenye orodha ya kucheza wakati itakapobofyiwa.

Ongeza msikilizaji wa tukio la bofya kwenye mabadiliko ya `resetButton`. Pitia wito wa kurudisha ukitumia sintaksia ya mshale na uiachie tupu kwa sasa.

# --hints--

Unapaswa kutumia njia ya `addEventListener()` kwenye `resetButton`.

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

Msikilizaji wa tukio la `resetButton` unapaswa kusikiliza tukio la `"click"`.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

Kitendakazi cha mwitiko cha msikilizaji wako wa tukio kinapaswa kutumia sintaksia ya mshale na kuwa na mabano ya mviringo tupu.
