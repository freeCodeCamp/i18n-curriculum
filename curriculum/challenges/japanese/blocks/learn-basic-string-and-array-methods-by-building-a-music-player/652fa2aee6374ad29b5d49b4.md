...
HTMLファイルから`#next`、`#previous`、`#shuffle`の要素にアクセスしてください。

それぞれを`nextButton`、`previousButton`、`shuffleButton`という名前の変数に代入してください。

# --hints--

`nextButton`という名前の変数があるはずです。

```js
assert.isDefined(nextButton);
```

`document.getElementById()`を使って`#next`の要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)next\1\s*\)/);
```

`#next`の要素を`nextButton`という変数に代入してください。

```js
assert.equal(nextButton, document.getElementById('next'));
```

`previousButton`という名前の変数があるはずです。

```js
assert.isDefined(previousButton);
```

`document.getElementById()`を使って`#previous`の要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)previous\1\s*\)/);
```

`#previous`の要素を`previousButton`という変数に代入してください。

```js
assert.equal(previousButton, document.getElementById('previous'));
```

`shuffleButton`という名前の変数があるはずです。

```js
assert.isDefined(shuffleButton);
```

`document.getElementById()`を使って`#shuffle`の要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)shuffle\1\s*\)/);
```

`#shuffle`の要素を`shuffleButton`という変数に代入してください。
