...
UI（ユーザーインターフェース）に曲を表示するには、関数を作成する必要があります。

アロー関数の構文を使って、パラメータとして`renderSongs`を受け取る`array`関数を作成してください。

# --hints--

空の関数を作成するには`const`を使い、`renderSongs`という名前にしてください。

```js
assert.match(code, /const\s+renderSongs\s*=\s*/)
```

`renderSongs`は関数である必要があります。

```js
assert.isFunction(renderSongs)
```

`renderSongs`関数はアロー関数の構文を使う必要があります。

```js
assert.match(code, /const\s+renderSongs\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/)
```

`renderSongs`関数はパラメータとして`array`を受け取る必要があります。

```js
assert.match(code, /const\s+renderSongs\s*=\s*\(?\s*array/)
```

`renderSongs`関数は空である必要があります。
