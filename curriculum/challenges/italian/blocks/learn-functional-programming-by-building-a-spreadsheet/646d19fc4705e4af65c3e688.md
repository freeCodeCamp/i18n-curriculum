...
La tua funzione `addCharacters` alla fine restituisce un intervallo di caratteri. Vuoi che restituisca un array di id delle celle. Collega il metodo `.map()` alla tua chiamata `charRange()`. Non passare ancora una funzione di callback.

# --hints--

Dovresti collegare `.map()` alla tua chiamata `charRange()`.
...
Non dovresti passare nulla alla tua chiamata `.map()`.
