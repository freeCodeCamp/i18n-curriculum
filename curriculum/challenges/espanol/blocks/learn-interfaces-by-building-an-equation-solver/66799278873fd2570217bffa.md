...
Agrega otro `case` para cuando la longitud de `results` sea `1`. En este caso, asigna a `result_list` una lista que contenga una cadena con el formato `x = <root>`, donde `<root>` es la solución de la ecuación. Formatea la cadena para que se muestre tanto el signo positivo como el negativo de la solución.
...
Debes crear un nuevo `case` con el patrón `1` después del bloque `case` existente.
...
Debes asignar una lista que contenga `f'x = {results[0]:+}'` a `result_list` dentro de la nueva estructura `case`.
