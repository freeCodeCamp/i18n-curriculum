...
`playPreviousSong`라는 이름의 함수를 만드세요. 새 함수 안에서 `getPreviousSong` 함수가 반환하는 값을 사용해 재생할 노래를 찾으세요. `getPreviousSong()`가 `undefined`일 때는 재생 목록에서 첫 번째 노래를 재생하세요. 그렇지 않으면 현재 재생 중인 노래 바로 이전 노래를 재생하세요.

# --hints--

`playPreviousSong`라는 이름의 함수가 있어야 합니다.

```js
assert.isFunction(playPreviousSong);
```

`playPreviousSong` 함수는 재생 목록에서 현재 재생 중인 노래 바로 이전 노래를 재생해야 합니다.
...
`getPreviousSong()`가 `undefined`일 때 `playPreviousSong` 함수는 재생 목록에서 첫 번째 노래를 재생해야 합니다.
