...
Створіть інструкцію `if`. Для її умови використайте оператор `not` для заперечення `is_sequence`. Усередині інструкції `if` виведіть `Invalid format: expected a list or tuple.` і поверніть `False`.

# --hints--

Ви повинні створити інструкцію `if` всередині вашої функції `validate`.
...
Ваша інструкція `if` має мати `not is_sequence` як умову.
...
Ви повинні вивести `Invalid format: expected a list or tuple.` всередині вашої інструкції `if`.
...
Ви повинні повернути `False` наприкінці вашої інструкції `if`.
