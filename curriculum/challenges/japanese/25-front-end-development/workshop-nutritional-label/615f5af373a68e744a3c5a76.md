...
`.calories-info span` セレクターを作成し、`font-size` を `2.4em` に、`font-weight` を `700` に設定してください。
...
`.calories-info span` セレクターの `font-size` プロパティを `2.4em` に設定する必要があります。

```js
assert(new __helpers.CSSHelp(document).getStyle('.calories-info span')?.fontSize === '2.4em');
```

`.calories-info span` セレクターの `font-weight` プロパティを `700` に設定してください。
