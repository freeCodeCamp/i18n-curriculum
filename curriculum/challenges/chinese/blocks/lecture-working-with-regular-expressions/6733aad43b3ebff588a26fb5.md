...
正则表达式是一种特殊的语法，用于创建“模式”，你可以用它来检查字串、提取文本等。
...
这就引出了我们的第一个方法，`test()` 方法。`test()` 方法存在于 `RegExp` 对象上，这些对象表现一个正则表达式（比如我们刚定义的那个）。
...
`test()` 方法返回一个 `boolean`，表示该字串是否与正则表达式匹配。
...
正则表达式以及所有关联的方法看起来可能很复杂且令人不知所措。但你将在接下来的课程中有机会进一步探索它们。
