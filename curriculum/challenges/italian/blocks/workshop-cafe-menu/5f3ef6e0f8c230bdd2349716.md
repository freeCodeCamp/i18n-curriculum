...
La tipografia degli elementi di intestazione (ad esempio `h1`, `h2`) è impostata dai valori predefiniti dei browser degli utenti.

Aggiungi due nuovi selettori di tipo (`h1` e `h2`). Usa la proprietà `font-size` per entrambi, ma usa il valore `40px` per `h1` e `30px` per `h2`.
...
Il tuo elemento `h1` dovrebbe avere una `font-size` di `40px`.
...
Il tuo elemento `h2` dovrebbe avere una `font-size` di `30px`.
