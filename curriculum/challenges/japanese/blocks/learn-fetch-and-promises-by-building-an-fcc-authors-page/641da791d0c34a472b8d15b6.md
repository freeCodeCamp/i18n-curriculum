...
これで `authorDataArr` が、先ほどコンソールに出力した `data` と同じになりました。 コンソールに `authorDataArr` を出力して確認してみましょう。

Inside your `console.log()` statement, add the text `"Author Data Array:"` as the first argument and `authorDataArr` as the second argument. テキストと `authorDataArr` の間にはコンマを入れてください。
