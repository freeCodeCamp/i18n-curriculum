...
Ora è il momento di definire cosa succede quando il pulsante di opzione è selezionato.

Stai stilizzando solo il cerchio interno quando il pulsante di opzione è selezionato, quindi il tuo selettore dovrebbe essere `.contact-method-radio-btn:checked::before`.

Nella regola, imposta `transform` su `translate(3px, 3px) scale(1)` e `background-color` su `lightgreen`.

Il `transform` sposta leggermente il cerchio interno e lo ingrandisce fino alla dimensione completa con `scale(1)`, e uno sfondo di lightgreen riempie il cerchio di colore per mostrare che l'opzione è selezionata, rendendolo visibile.

# --hints--

Dovresti puntare al pseudo-elemento `.contact-method-radio-btn:checked::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

Imposta la proprietà `transform` su `translate(3px, 3px) scale(1)`.
...
Imposta la proprietà `background-color` su `lightgreen`.
