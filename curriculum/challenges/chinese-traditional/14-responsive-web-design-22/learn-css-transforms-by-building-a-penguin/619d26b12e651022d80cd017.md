...
`.arm` 的 `width` 應該爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

`.arm` 的 `height` 應該爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

`.arm` 的 `background` 應該爲 `linear-gradient(90deg, gray, rgb(209, 210, 199))`。
