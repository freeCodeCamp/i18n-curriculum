...
أصبحت دالتك مكتملة الآن. اختبرها مرة أخرى بتسجيل `genParentheses(2)` بدلًا من `genParentheses(1)`.

# --hints--

يجب أن تسجل `genParentheses(2)`.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*genParentheses\s*\(\s*2\s*\)\s*\)/);
```

يجب ألا تحتوي شفرتك على `console.log(genParentheses(1))`.
