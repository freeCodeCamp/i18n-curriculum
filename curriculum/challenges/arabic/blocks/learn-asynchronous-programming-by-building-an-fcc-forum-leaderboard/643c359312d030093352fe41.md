...
حان الوقت الآن لعرض البيانات على الصفحة.

ابدأ بإنشاء دالة سهمية تسمى `showLatestPosts`، تأخذ معلمة واحدة هي `data`.

# --hints--

يجب أن يكون لديك دالة سهمية تسمى `showLatestPosts`.

```js
assert.match(code, /const\s+showLatestPosts\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>\s*/);
```

يجب أن تحتوي دالتك `showLatestPosts` على معلمة `data`.

```js
assert.match(code, /showLatestPosts\s*=\s*(\(\s*data\s*\)|data)\s*=>/);
```

يجب تعريف `showLatestPosts`.

```js
assert.isDefined(showLatestPosts);
```

يجب أن تكون `showLatestPosts` دالة.
