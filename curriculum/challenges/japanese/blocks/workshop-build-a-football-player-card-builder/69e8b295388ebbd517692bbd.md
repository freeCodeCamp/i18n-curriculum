...
`card-stats` divの中で、最初の`stat-col`の後に、さらに2つの子要素を追加する必要があります。

まず、2つの統計列の間に細い縦線をレンダリングするために、`stat-divider`の`className`を持つ自己終了型の`div`を追加し、次に3つの`div`要素を含む`stat-col`の`className`を持つ2番目の`div`を追加します。各`div`は`stat-row`の`className`を持っています。

# --hints--

最初の`.stat-col`の後に、`stat-divider`の`className`を持つ自己終了型の`div`を追加してください。

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

`.stat-divider`の後に、`stat-col`の`className`を持つ2番目の`div`を追加してください。

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

2番目の`.stat-col`は、`stat-row`の`className`を持つ3つの`div`要素を含む必要があります。
