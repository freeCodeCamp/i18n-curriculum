...
اختر عناصر `td` التي لها قيمة `class` تساوي `current`، واجعل الخط مائلاً.

# --hints--

يجب أن يكون لديك محدِّد `td.current`.
...
يجب أن يحتوي محدِّد `td.current` على خاصية `font-style` مضبوطة على `italic`.
