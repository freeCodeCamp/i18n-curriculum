...
<dfn>替代序列</dfn> `|` 可用來匹配 `|` 左邊或右邊的文字。 例如，正規表達式 `/yes|no/` 會匹配 `yes` 或 `no`。

更新你的 `helpRegex`，使其能匹配 `please help` 或 `assist me`。
...
你的 `helpRegex` 應該匹配字串 `please help`。
