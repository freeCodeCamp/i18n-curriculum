...
Il elemento `table` dovrebbe avere un elemento `caption`.

```js
assert(document.querySelectorAll('table')?.[2]?.children?.[0]?.localName === 'caption');
```

L'elemento `caption` dovrebbe avere il testo `Net Worth`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('caption')?.textContent === 'Net Worth');
```

Il terzo elemento `table` dovrebbe avere un elemento `thead`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('thead'));
```

Il terzo elemento `table` dovrebbe avere un elemento `tbody`.
