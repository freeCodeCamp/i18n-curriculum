...
Um die Elemente in der Karte zu positionieren, benötigen Sie erneut Grid-Klassen. Weisen Sie dem Wrapper `grid` die Utility-Klassen `grid-rows-[1fr_auto]`, `gap-6` und `div` zu.

Entfernen Sie außerdem die scharfen Kanten mit der Klasse `rounded-xl`.

# --hints--

Ihr Wrapper-Element `div` für den Premium-Plan sollte die Klasse `grid` haben.
...
Ihr Wrapper-Element `div` für den Premium-Plan sollte die Klasse `grid-rows-[1fr_auto]` haben.
...
Ihr Wrapper-Element `div` für den Premium-Plan sollte die Klasse `gap-6` haben.
...
Ihr Wrapper-Element `div` für den Premium-Plan sollte die Klasse `rounded-xl` haben.
