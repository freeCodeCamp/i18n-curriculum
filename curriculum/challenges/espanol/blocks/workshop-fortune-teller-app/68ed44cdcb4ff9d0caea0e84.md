...
Como estás llamando a `fetchCardsData` y `initializeEventListeners` dentro del constructor, ahora necesitas definirlos dentro de la clase. Crea ambos como métodos `private`. Asegúrate de que `fetchCardsData` sea asíncrono porque manejará llamadas a la API.

# --hints--

Deberías tener un método `private` llamado `fetchCardsData`.
...
Tu método `fetchCardsData` debería ser `async`.
...
Deberías tener un método `private` llamado `initializeEventListeners`.
