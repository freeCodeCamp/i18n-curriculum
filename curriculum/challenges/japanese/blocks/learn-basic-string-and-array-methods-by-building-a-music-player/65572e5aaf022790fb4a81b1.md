...
次は、`resetButton` にリセット機能を追加する番です。これによりクリックするとプレイリストの曲が元に戻ります。

`resetButton` 変数にクリックイベントリスナーを追加してください。コールバックはアロー構文で渡し、今は空のままにしてください。

# --hints--

`addEventListener()` に `resetButton` メソッドを使う必要があります。

```js
assert.match(code, /resetButton\.addEventListener\(/)
```

`resetButton` のイベントリスナーは `"click"` イベントを監視する必要があります。

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1/)
```

イベントリスナーのコールバック関数はアロー構文で、空の波括弧を持つべきです。
