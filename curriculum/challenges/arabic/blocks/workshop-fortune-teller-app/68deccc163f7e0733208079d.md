...
بعد ذلك، تحتاج إلى إنشاء دالة لإخفاء العناصر من DOM. أعلن دالة `hideElements` التي تأخذ `...elements` كمعلمة من النوع `HTMLElement[]`.

ستُستخدم هذه الدالة، مع صنف CSS `hidden`، في التطبيق بأكمله لتبديل مستوى الظهور لعناصر واجهة المستخدم.

# --hints--

يجب أن تكون قد أعلنت دالة باسم `hideElements`.
...
يجب أن تحتوي دالة `hideElements` على معلمة باسم `...elements` من النوع `HTMLElement[]`.
