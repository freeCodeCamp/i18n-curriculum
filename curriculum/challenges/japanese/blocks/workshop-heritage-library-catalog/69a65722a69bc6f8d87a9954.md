...
まとめをしましょう。`catalog.length` と `Object.keys(byDecade).length` をログに出して、合計の本の数と年代グループを確認してください。

`let oldestYear = Infinity` と `let newestYear = 0` を宣言してください。`for` に対して `catalog` ループを使い、年が `"Unknown"` でない各エントリーについて、年が小さい場合は `oldestYear` を、大きい場合は `newestYear` を更新してください。両方の値をログに出してください。

Heritage Library Catalog ワークショップを完了しました！
