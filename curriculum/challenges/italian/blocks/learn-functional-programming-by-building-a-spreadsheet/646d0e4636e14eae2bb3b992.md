...
Ora aggiorna la tua funzione più interna nella catena `addCharacters` per restituire implicitamente il risultato della chiamata a `charRange()` con `character1` e `character2` come argomenti.

# --hints--

La tua funzione più interna dovrebbe usare un ritorno implicito.
...
La tua funzione più interna dovrebbe restituire il risultato della chiamata a `charRange()`.
...
Dovresti passare `character1` come primo argomento alla chiamata `charRange()`.
...
Dovresti passare `character2` come secondo argomento alla chiamata `charRange()`.
