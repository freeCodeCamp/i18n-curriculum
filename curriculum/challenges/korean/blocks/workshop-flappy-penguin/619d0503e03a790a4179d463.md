...
`.chin` 요소를 부모의 위쪽에서 `25%`만큼, 왼쪽에서 `5%`만큼 위치시키세요. 그런 다음, 위쪽 모서리에는 `70%`의 반경을, 아래쪽 모서리에는 `100%`의 반경을 주세요.

# --hints--

`.chin`에 `top` 속성을 주어야 합니다.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

`.chin`에 `top`를 `--fcc-expected--`로 주어야 하지만 `--fcc-actual--`이 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

`.chin`에 `left` 속성을 주어야 합니다.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

`.chin`에 `left`를 `--fcc-expected--`로 주어야 하지만 `--fcc-actual--`이 발견되었습니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

`.chin`에 `border-radius`를 `70% 70% 100% 100%`로 주어야 합니다.
