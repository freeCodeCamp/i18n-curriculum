...
أعطِ محدِّد `.gallery` خاصية `padding` بقيمة `20px 10px` لإنشاء بعض المساحة حول الحاوية.

ثم، أعطه خاصية `max-width` بقيمة `1400px` وأضف `margin` بقيمة `0 auto` لتمركزه.
...
يجب أن يتمركز عنصر `.gallery` باستخدام `margin` بقيمة `0 auto`.
