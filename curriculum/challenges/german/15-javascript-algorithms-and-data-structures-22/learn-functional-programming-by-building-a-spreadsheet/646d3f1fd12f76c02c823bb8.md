...
Du hast einen kleinen Bug. `arg1` und `arg2` sind Strings und keine Zahlen. `infixToFunction['+']("1", "2")` würde `12` zurückgeben, was mathematisch nicht korrekt ist.

Umschließe jedes deiner `infixToFunction[operator]`-Argumente in einem `parseFloat()`-Aufruf.

# --hints--

Du solltest `arg1` in einem `parseFloat()`-Aufruf umschließen.
...
Du solltest `arg2` in einem `parseFloat()`-Aufruf umschließen.
