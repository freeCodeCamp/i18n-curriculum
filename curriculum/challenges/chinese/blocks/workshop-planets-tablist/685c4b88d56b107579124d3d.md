...
如果你查看超文本标记语言，你会看到第二个和第三个窗格被隐藏了。这是通过使用 `hidden` 属性实现的。第一个窗格默认是可见的。当用户点击一个标签（页）时，所有窗格都应该被隐藏。

使用 `forEach` 方法循环遍历 `panels`，并将每个面板的 `hidden` 属性设置为 `true`。 

# --hints--

你应该循环遍历 `panels` 并将每个窗格的 `hidden` 属性设置为 `true`。
