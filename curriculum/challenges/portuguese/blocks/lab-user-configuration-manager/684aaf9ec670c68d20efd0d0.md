...
`update_setting({'theme': 'light'}, ('theme', 'dark'))` deve atualizar uma chave existente e retornar a mensagem de sucesso `Setting 'theme' updated to 'dark' successfully!`.
...
`update_setting({'theme': 'light'}, ('volume', 'high'))` deve retornar a mensagem de erro `Setting 'volume' does not exist! Cannot update a non-existing setting.` Não é possível atualizar uma configuração inexistente.` quando a chave não existe.
...
`delete_setting({'theme': 'light'}, 'theme')` deve remover a chave existente e retornar a mensagem de sucesso `Setting 'theme' deleted successfully!`.
...
`delete_setting` deve retornar a mensagem de erro `Setting not found!` quando a chave não existir.
...
`view_settings` deve retornar a mensagem `No settings available.` se o dicionário fornecido estiver vazio.
