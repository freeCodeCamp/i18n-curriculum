...
如果申請人的年收入和信用分數低於 `minIncomeForCar` 和 `minCreditScoreForCar`，那麼他們不符合任何貸款資格。所以，傳回字串 `"You don't qualify for any loans."`

# --hints--

如果申請人不符合任何貸款資格，你的 `getLoanMessage` 函式（程式）應該傳回一個字串。

```js
assert.isString(getLoanMessage(25000, 550));
```

如果申請人不符合任何貸款的條件，你的 `getLoanMessage` 函式（程式）應該傳回字串 `"You don't qualify for any loans."`。
