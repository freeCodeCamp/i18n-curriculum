...
Створіть новий селектор `25%` між вашими селекторами `0%` та `50%`. Надайте цьому новому селектору множину властивостей `background-color` зі значенням `yellow`.

# --hints--

Ви повинні створити новий селектор `25%` у вашому правилі `@keyframes cabins`.
...
Ваш селектор `25%` має бути між селекторами `0%` та `50%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '25%');
```

Ваш селектор `25%` має мати властивість `background-color`, встановлену в `yellow`.
