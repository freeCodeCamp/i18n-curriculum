...
Kituo cha mawasiliano kimeomba kitendakazi kipya cha kugawanya kikosi katika vipande vya ukubwa tofauti. Unda kitendakazi tupu kinachoitwa `chunkCrew` kinachokubali vigezo viwili, `crew` na `size`.

# --hints--

Unapaswa kuunda kitendakazi kinachoitwa `chunkCrew`.

```js
assert.isFunction(chunkCrew);
```

Kitendakazi chako cha `chunkCrew` kinapaswa kuwa na vigezo `crew` na `size`.
