...
이제 목록에서 선택된 항목을 취소선으로 표시하는 로직을 작업할 차례입니다.

먼저, 이 `filteredItems.map()` 호출 내부의 모든 내용이 암묵적으로 반환되고 있으니, 그 안의 코드를 중괄호로 감싸고 `li` 요소를 명시적으로 반환하세요.

예를 들어, 이름 목록을 렌더링하는 간단한 컴포넌트에서 이렇게 할 수 있습니다:
...
`filteredItems.map()` 호출 내부의 코드를 중괄호로 감싸야 합니다.
