...
次に、`subTitle` に `meaning_up` を代入し、`title` には `foundCard.name` を代入してください。その後、`showElements` メソッドを呼び出し、elements から `fortuneDescription` を渡してください。

# --hints--

`this.elements.subTitle.textContent` に `foundCard.meaning_up` を代入してください。
...
`this.elements.title.textContent` に `foundCard.name` を代入してください。
...
`showElements` の中で `showFortune` 関数を呼び出してください。
...
`this.elements.fortuneDescription` に引数として `showElements` を渡してください。
