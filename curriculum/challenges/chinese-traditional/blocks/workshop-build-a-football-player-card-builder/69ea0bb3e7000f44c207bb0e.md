...
為了讓 localStorage 功能最終能正常運作，將狀態初始化從 `defaultPlayer` 改為 `loadPlayer`。這樣可以確保狀態從任何可用的已儲存資料開始。

現在測試 localStorage 功能。你輸入的所有內容在預覽重新整理或重新載入頁面後都應該會保留。

你也可以將圖片 `src` 的連結改成 `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png`，以即時看到圖片變化。只要確保你將球員名稱改成 Maradona。

這樣，你的足球球員卡片建置專案就完成了！

# --hints--

你的 `useState` 應該用 `loadPlayer` 來初始化，而不是 `defaultPlayer`。
