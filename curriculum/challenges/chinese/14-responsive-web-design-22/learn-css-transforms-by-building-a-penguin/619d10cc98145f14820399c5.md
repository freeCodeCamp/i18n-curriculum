...
在 `.blush.right` 元素下，添加两个 `div` 元素，每个元素都包含 `beak` `class`。 Also, give the first `.beak` element a `class` of `top`, and the second `.beak` element a `class` of `bottom`.

# --hints--

应该在 `.penguin-head` 中添加两个 `div` 元素。 应该是 `--fcc-expected--` `div` 元素，而不是 `--fcc-actual--`。

```js
assert.equal(document.querySelectorAll('.penguin-head > div')?.length, 9);
```

第一个新的 `div` 应该设置 `class` `beak`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak'));
```

第二个新的 `div` 应该设置 `class` `beak`。

```js
assert.equal(document.querySelectorAll('.penguin-head > div.beak')?.length, 2);
```

第一个新的 `div` 应该设置 `class` `top`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak.top'));
```

第二个新的 `div` 应该设置 `class` `bottom`。

```js
assert.exists(document.querySelector('.penguin-head > div.beak.bottom'));
```

应该将 `div.beak.top` 放在 `div.blush.right` 之后。

```js
assert.exists(document.querySelector('.blush.right + .beak.top'));
```

应该将 `div.beak.bottom` 放在 `div.beak.top` 之后。
