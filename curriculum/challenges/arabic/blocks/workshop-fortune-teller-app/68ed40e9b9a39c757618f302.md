...
تحتاج الآن إلى صنف رئيسي يساعد في عرض واجهة المستخدم ديناميكيًا في DOM.

أنشئ صنفًا باسم `Game`. بداخله، أضف خاصية تسمى `cards` من نوع `Card[]`، وللآن، ابدأها بمصفوفة فارغة.

# --hints--

يجب أن يكون لديك صنف باسم `Game`.
...
يجب أن تحتوي صنف `Game` على خاصية `cards`.
...
يجب أن تكون خاصية `cards` من نوع `Card[]`.
...
يجب تعيين خاصية cards إلى `array` فارغة.
