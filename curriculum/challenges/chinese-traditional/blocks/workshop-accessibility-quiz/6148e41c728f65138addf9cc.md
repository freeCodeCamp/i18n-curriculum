...
最後，某些型別的基於動作的動畫可能會讓部分使用者感到不適。特別是，有 <dfn>前庭</dfn> 疾病的人對某些動作觸發具有敏感性。

`@media` at-rule 有一個名為 `prefers-reduced-motion` 的媒體特性，用來根據使用者的動畫偏好設定 CSS。它可以接受以下其中一個值：
...
你將在未來的課程中進一步了解這個特性。現在，請將設定 `scroll-behavior: smooth` 的樣式規則包裹在一個 `@media` 介面規則中，該介面規則的媒體特性為 `prefers-reduced-motion`，其值設定為 `no-preference`。

做得好。你已完成 Accessibility Quiz 練習專案。

# --hints--

你應該有一個 `@media (prefers-reduced-motion: no-preference)` 規則。

```js
assert.lengthOf(new __helpers.CSSHelp(document).getRuleListsWithinMedia('(prefers-reduced-motion: no-preference)'), 1);
```

你應該將現有的 `*` 規則包裹在 `@media` 規則內。
