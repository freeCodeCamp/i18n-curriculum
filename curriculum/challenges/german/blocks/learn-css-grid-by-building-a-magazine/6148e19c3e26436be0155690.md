...
Erstelle eine `.lists li`-Regel, um die Listenelemente innerhalb deines `.lists`-Elements anzuvisieren. Gib ihr eine `margin-bottom`-Eigenschaft mit dem Wert `1.5rem`.

# --hints--

Du solltest einen `.lists li`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.lists li'));
```

Dein `.lists li`-Selektor sollte eine `margin-bottom`-Eigenschaft mit dem Wert `1.5rem` haben.
