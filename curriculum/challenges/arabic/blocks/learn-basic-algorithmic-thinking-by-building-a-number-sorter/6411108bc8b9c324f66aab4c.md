...
أعلن عن متغير `currValue` وعيّن له القيمة الموجودة في `array[i]`. ثم أعلن عن متغير `j` وعيّن له القيمة `i - 1`. يجب أن يكون متغير `j` قابلًا لإعادة التعيين.

# --hints--

يجب إعلان متغير `currValue` باستخدام `const`.
...
يجب تعيين `currValue` للقيمة الموجودة في `array[i]`.
...
يجب إعلان متغير `j` باستخدام `let`.
