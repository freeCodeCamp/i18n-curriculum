...
노래가 이 시점에서 일시 정지되므로 `classList`에서 `remove()` 클래스를 제거하려면 `playing`와 `playButton` 메서드를 사용하세요.

노래를 최종적으로 일시 정지하려면 `pause()` 변수에 `audio` 메서드를 사용하세요. `pause()`는 음악 파일을 일시 정지하는 Web Audio API의 메서드입니다.

# --hints--

기존 `pauseSong` 함수와 그 내용을 수정하지 마세요.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

`classList`에서 `remove()` 클래스를 제거하려면 `"playing"` 속성과 `playButton` 메서드를 사용하세요.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

`pause()` 변수에 `audio` 메서드를 사용하세요.
