...
プレイヤーの体力を減らしたので、プレイヤーの `health` が `0` 以下かどうかチェックする必要があります。 該当する場合、関数 `lose` を呼び出してください。

# --hints--

Your `pick` 関数は、`else` 本文の中にネストされた `if` 文を持つ必要があります。
...
`if` 文では、プレイヤーの `health` が `0` 以下かどうかをチェックしてください。

```js
assert.match(pick.toString(), /if\s*\(\s*health\s*<=\s*0\s*\)/);
```

`else` 本文内の `if` 文では、関数 `lose` を呼び出す必要があります。
