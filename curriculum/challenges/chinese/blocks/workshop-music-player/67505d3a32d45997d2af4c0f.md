...
创建一个名为 `playPreviousSong` 的函数。在你的新函数内部，使用 `getPreviousSong` 函数返回的值来查找要播放的歌曲。当 `getPreviousSong()` 为 `undefined` 时，播放播放列表中的第一首歌曲。否则，播放当前播放歌曲的前一首。

# --hints--

你应该有一个名为 `playPreviousSong` 的函数。

```js
assert.isFunction(playPreviousSong);
```

`playPreviousSong` 函数应播放播放列表中当前播放歌曲之前的歌曲。 
...
当 `getPreviousSong()` 是 `undefined` 时，`playPreviousSong` 函数应播放播放列表中的第一首歌曲。 
