...
Коли ширина екрану маленька, `h1` не переносить свій текстовий вміст так, як має. Вирівняйте текст для елемента `h1` по центру.

Потім додайте `main` відступ, щоб заголовок розділу `Student Info` був повністю видимим.

# --hints--

Ви повинні додати `h1` зі значенням `text-align` для `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1')?.textAlign, 'center');
```

Ви повинні додати селектор `main`, щоб націлитися на елемент `main`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('main'));
```

Ви повинні додати відступ `main` не менше ніж `padding-top` для `25px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('main')?.paddingTop?.replace(/\D+/, '')), 25);
```

Ви повинні змінити лише значення `padding-top`.
