...
次に、赤のカラーストップを `0%` に、緑のカラーストップを `50%` に、青のカラーストップを `100%` に設定してください。

# --hints--

あなたの `.red` CSSルールには、`background` プロパティが `linear-gradient(180deg, rgb(255, 0, 0) 0%, rgb(0, 255, 0) 50%, rgb(0, 0, 255) 100%)` に設定されている必要があります。
