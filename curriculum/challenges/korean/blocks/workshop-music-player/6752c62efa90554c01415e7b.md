...
`setPlayerDisplay`라는 이름의 함수를 만드세요. 함수 안에서, 현재 노래의 제목이 정의되어 있으면 `textContent`의 `playingSong`을 그 제목으로 설정하세요. 그렇지 않으면 빈 문자열로 설정하세요.

그 아래에, 현재 노래의 아티스트가 정의되어 있으면 `textContent`의 `songArtist`을 그 아티스트로 설정하세요. 그렇지 않으면 빈 문자열로 설정하세요. 


# --hints--

`setPlayerDisplay`라는 이름의 함수가 있어야 합니다.

```js
assert.isFunction(setPlayerDisplay);
```

`setPlayerDisplay` 함수는 현재 노래의 제목이 정의되어 있으면 `textContent`의 `playingSong`을 그 제목으로, 아니면 빈 문자열로 설정해야 합니다.
...
`setPlayerDisplay` 함수는 현재 노래의 아티스트가 정의되어 있으면 `textContent`의 `songArtist`을 그 아티스트로, 아니면 빈 문자열로 설정해야 합니다.
