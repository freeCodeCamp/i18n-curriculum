...
`.map()` 呼び出しで `rangeFromString()` メソッドを呼び出し、コールバック関数として `addCharacters` の参照を渡してください。

# --hints--

`.map()` 呼び出しで `rangeFromString()` メソッドを呼び出す必要があります。
...
`addCharacters` メソッドにコールバックとして `.map()` の参照を渡してください。
