...
Finalmente, use el método `.appendChild()` para añadir el elemento `label` al elemento `container`.

# --hints--

Deberías acceder al método `.appendChild()` del elemento `container`.

```js
assert.match(code, /container\.appendChild\(/);
```

Deberías pasar tu elemento `label` al método `.appendChild()`.

```js
assert.match(code, /container\.appendChild\(\s*label\s*\)/);
```

Deberías agregar `label` después de establecer los atributos.
