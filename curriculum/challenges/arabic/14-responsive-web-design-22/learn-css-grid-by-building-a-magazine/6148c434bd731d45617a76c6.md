...
هذه الخاصية تأخذ إما `row` أو `column` كقيمة أولى، مع قيمة ثانية اختيارية وهي `dense`. `grid-auto-flow` تستخدم خوارزمية وضع تلقائي لضبط الـ grid layout. تعيينها إلى `column` يخبر الخوارزمية لإنشاء أعمدة جديدة للمحتوى حسب الحاجة. القيمة `dense` تسمح للخوارزمية بالتتبع وملء الثقوب في الـ grid بالعناصر الأصغر حجماً. والذي يمكن أن يؤدي إلى ظهور عناصر غير مرغوب فيها.

بالنسبة لمنتقي `.social-icons`، قم بتعيين خاصية `grid-auto-flow` إلى `column`.

# --hints--

يجب أن يكون لمنتقي `.social-icons` الخاص بك الخاصية `grid-auto-flow` بقيمة `column`.
