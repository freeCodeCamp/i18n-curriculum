...
チェックボックスの入力をデフォルトでチェック済みにするには、`checked` 属性を追加できます。

`checked` 属性を使った例を示します。

```html
<input checked type="checkbox" id="checked" name="checked">
```

`checked` の `id` を持つチェックボックスの入力に `"reputation"` 属性を追加して、デフォルトでチェック済みにしてください。

# --hints--

`id` の `"reputation"` を持つチェックボックスの入力には `checked` 属性が必要です。
