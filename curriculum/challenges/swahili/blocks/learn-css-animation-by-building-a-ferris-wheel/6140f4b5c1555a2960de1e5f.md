...
Tengeneza sheria nyingine ya `@keyframes` yenye jina `cabins`. Tumia sifa sawa na za `@keyframes wheel`, ukikopa sheria za `0%` na `100%`, lakini weka sifa ya `transform` ya kichaguzi cha `100%` kuwa `rotate(-360deg)`.
...
Sheria yako mpya ya `@keyframes` inapaswa kuitwa `cabins`.
...
Sheria yako mpya ya `@keyframes` inapaswa kuja baada ya sheria yako ya `@keyframes wheel`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.name === 'cabins');
```

Hupaswi kubadilisha jina la sheria yako ya `@keyframes wheel`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[0]?.name === 'wheel');
```

Sheria yako ya `@keyframes cabins` inapaswa kuwa na kichaguzi cha `0%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[0]?.keyText === '0%');
```

Kichaguzi chako cha `0%` kinapaswa kuwa na sifa ya `transform` iliyowekwa kuwa `rotate(0deg)`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[0]?.style?.transform === 'rotate(0deg)');
```

Sheria yako ya `@keyframes cabins` inapaswa kuwa na kichaguzi cha `100%`.
...
Kichaguzi chako cha `100%` kinapaswa kuja baada ya kichaguzi cha `0%`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText === '100%')
```

Kichaguzi chako cha `100%` kinapaswa kuwa na sifa ya `transform` iliyowekwa kuwa `rotate(-360deg)`.
