...
现在调用 `showElements` 函数，并将 `multipleCard`、`fortuneContainer` 和 `text` 作为属性传入。

# --hints--

你应该在 `showElements` 方法内调用 `multipleCardSelected` 函数。
...
你应该将 `text` 作为参数传递给 `showElements` 函数。
