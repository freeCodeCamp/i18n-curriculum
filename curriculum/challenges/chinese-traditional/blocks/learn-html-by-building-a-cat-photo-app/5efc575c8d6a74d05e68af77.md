...
Your new checkbox should have an `id` attribute with the value `energetic` and a `name` attribute with the value `personality`. 檢查開始標籤的名稱後面是否有空格和/或所有屬性名稱之前是否有空格。
...
Your new checkbox should be after the second one. 你把它們的順序寫錯了。
...
The new `label` should have a `for` attribute with the same value as the `id` attribute of the new checkbox. 你要麼遺漏了屬性值，要麼拼寫有誤。
