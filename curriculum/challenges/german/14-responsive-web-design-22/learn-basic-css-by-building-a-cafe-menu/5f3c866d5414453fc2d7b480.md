---
id: 5f3c866d5414453fc2d7b480
title: Schritt 31
challengeType: 0
dashedName: step-31
---

# --description--

Füge unterhalb des existierenden Paares Kaffee/Preis folgende Kaffee- und Preisangaben mithilfe eines `article`-Elements mit zwei verschachtelten `p`-Elementen hinzu:

```bash
Caramel Macchiato 3.75
Pumpkin Spice 3.50
Hazelnut 4.00
Mocha 4.50
```

Wie zuvor sollte der Text des ersten `p`-Elements die Kaffeesorte und des zweiten `p`-Elements die Preise enthalten.

# --hints--

Du solltest fünf `article`-Elemente haben.

```js
assert.lengthOf(document.querySelectorAll('article'),5);
```

Jedes `article`-Element sollte zwei `p`-Elemente haben.

```js
const articles = document.querySelectorAll('article');
assert.lengthOf(articles?.[0]?.children, 2);
assert.lengthOf(articles?.[1]?.children, 2);
assert.lengthOf(articles?.[2]?.children, 2);
assert.lengthOf(articles?.[3]?.children, 2);
assert.lengthOf(articles?.[4]?.children, 2);
```

Dein erstes `article` Element sollte `p` Elemente mit dem Text `French Vanilla` und `3.00` enthalten.

```js
const children = document.querySelector('article')?.children;
assert.match(children?.[0]?.innerText, /French Vanilla/i);
assert.match(children?.[1]?.innerText, /3\.00/i);
```

Dein zweites `article` Element sollte `p` Elemente mit dem Text `Caramel Macchiato` und `3.75` enthalten.

```js
const children = document.querySelectorAll('article')?.[1]?.children;
assert.match(children?.[0]?.innerText, /Caramel Macchiato/i);
assert.match(children?.[1]?.innerText, /3\.75/i);
```

Dein drittes `article` Element sollte `p` Elemente mit dem Text `Pumpkin Spice` und `3.50` enthalten.

```js
const children = document.querySelectorAll('article')?.[2]?.children;
assert.match(children?.[0]?.innerText, /Pumpkin Spice/i);
assert.match(children?.[1]?.innerText, /3\.50/i);
```

Dein viertes `article` Element sollte `p` Elemente mit dem Text `Hazelnut` und `4.00` enthalten.

```js
const children = document.querySelectorAll('article')?.[3]?.children;
assert.match(children?.[0]?.innerText, /Hazelnut/i);
assert.match(children?.[1]?.innerText, /4\.00/i);
```

Dein fünftes `article` Element sollte `p` Elemente mit dem Text `Mocha` und `4.50` enthalten.

```js
const children = document.querySelectorAll('article')?.[4]?.children;
assert.match(children?.[0]?.innerText, /Mocha/i);
assert.match(children?.[1]?.innerText, /4\.50/i);
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Cafe Menu</title>
    <link href="styles.css" rel="stylesheet"/>
  </head>
  <body>
    <div class="menu">
      <main>
        <h1>CAMPER CAFE</h1>
        <p>Est. 2020</p>
        <section>
          <h2>Coffee</h2>
--fcc-editable-region--
          <article>
            <p>French Vanilla</p>
            <p>3.00</p>
          </article>
--fcc-editable-region--
        </section>
      </main>
    </div>
  </body>
</html>
```

```css
body {
  background-image: url(https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg);
}

h1, h2, p {
  text-align: center;
}

.menu {
  width: 80%;
  background-color: burlywood;
  margin-left: auto;
  margin-right: auto;
}
```
