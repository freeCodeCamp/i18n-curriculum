...
Dentro del `tbody`, agregar un `tr` con la `class` establecida en `total`. En eso, agrega un `th` con el texto `Total Net Worth`, y envuelve `Net Worth` en un `span` con la `class` establecida en `sr-only`.

Luego agrega tres elementos `td`, dando al tercero una `class` establecida en `current` y da a cada uno el siguiente texto: `$-171`, `$136`, `$334`.
...
Tu elemento `tr` debe tener la `class` establecida en `total`.
...
Tu tercer elemento `td` debe tener la `class` establecida en `current`.
