...
Тепер ви можете створити функцію для імітації фестивалю. Створіть порожню функцію з назвою `simulateFestival` з параметрами `gates` та `timeBlock`.

# --hints--

Ви повинні створити функцію з назвою `simulateFestival`.

```js
assert.isFunction(simulateFestival);
```

Ваша функція `simulateFestival` має мати параметри `gates` та `timeBlock`.
