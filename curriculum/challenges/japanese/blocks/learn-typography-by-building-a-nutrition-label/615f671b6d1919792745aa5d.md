...
`div` 要素の下に新しい `.medium` 要素を作成してください。`class` 属性を `daily-value small-text` に設定してください。この新しい `div` の中に、テキストが `p` の `% Daily Value *` 要素を追加し、`class` 属性を `bold right` に設定してください。

# --hints--

`div` 要素の後に新しい `.medium` 要素を作成してください。
...
新しい `div` には `class` 属性を `daily-value small-text` に設定してください。
