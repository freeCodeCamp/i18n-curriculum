...
تم اختبار `parseCard` ويعمل بشكل صحيح. لم يعد هناك حاجة إلى المتغير `cardResult`.

ابدأ بحذف سطر `const cardResult` وسطر `console.log(cardResult)`. ثم أنشئ دالة باسم `parseCatalog` تأخذ `rawCards` كمعلمة. في الوقت الحالي، أعلن عن مصفوفة فارغة باسم `catalog` داخل الدالة وأرجعها.

أسفل الدالة، استدعِ `parseCatalog(rawCatalogCards)`، عيّن النتيجة إلى متغير باسم `catalog`، وسجل `catalog.length`.

# --hints--

يجب أن تكون `parseCatalog` دالة تُرجع مصفوفة.

```js
assert.isArray(parseCatalog(rawCatalogCards));
```

يجب تعريف `catalog` وتعيينها إلى `parseCatalog(rawCatalogCards)`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+catalog\s*=\s*parseCatalog\s*\(/);
```

يجب حذف المتغير `cardResult` واستدعاء `console.log()` المرتبط به.

```js
assert.notMatch(__helpers.removeJSComments(code), /\bcardResult\b/);
```

يجب تسجيل `catalog.length` في وحدة التحكم.
