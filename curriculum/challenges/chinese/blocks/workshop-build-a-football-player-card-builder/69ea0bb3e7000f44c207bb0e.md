...
为了让 localStorage 特色最终生效，将状态初始化从 `defaultPlayer` 改为 `loadPlayer`。这样可以确保状态从任何可用的储存数据开始。

现在测试 localStorage 特色。你输入的所有内容在预览刷新或页面重新加载后都应该保留。

你也可以将图像 `src` 的链接改为 `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png`，以实时查看图像变化。只要确保你把球员名字改成 Maradona。

这样，你的足球球员卡片生成器项目就完成了！

# --hints--

你的 `useState` 应该用 `loadPlayer` 初始化，而不是 `defaultPlayer`。
