...
احذف مصفوفة `myTaskArr` وكل الشفرة الخاصة بـ `localStorage` لأنك لم تعد بحاجة إليها.

# --hints--

يجب حذف `myTaskArr` وكل الشفرة المتعلقة بـ `localStorage` التي تعلمتها للتو.
...
يجب حذف أي مراجع متبقية لـ `myTaskArr` في أي مكان في الشفرة.

```js
assert.notMatch(code, /myTaskArr/);
```

يجب حذف أي مرجع لـ `localStorage.getItem` للعنصر `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

يجب حذف أي مراجع متبقية لـ `getTaskArrObj` في أي مكان في الشفرة.
