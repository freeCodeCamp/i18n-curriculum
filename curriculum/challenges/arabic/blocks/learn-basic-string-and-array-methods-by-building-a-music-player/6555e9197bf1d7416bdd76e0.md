...
حان الوقت لتنفيذ وظيفة حذف لقائمة التشغيل. ستدير هذه الوظيفة إزالة أغنية من قائمة التشغيل، وتعالج الإجراءات الأخرى ذات الصلة عند حذف أغنية، وتُنشئ زرًا لـ `Reset Playlist`.

استخدم `const` وصيغة السهم لإنشاء دالة `deleteSong` فارغة ومرر `id` كمعلمة.

# --hints--

يجب أن تستخدم `const` لإنشاء دالة فارغة باسم `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

يجب أن تكون `deleteSong` دالة.

```js
assert.isFunction(deleteSong)
```

يجب أن تستخدم دالة `deleteSong` صيغة السهم.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

يجب أن تأخذ دالة `deleteSong` معلمة `id`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

يجب أن تكون دالة `deleteSong` فارغة.
