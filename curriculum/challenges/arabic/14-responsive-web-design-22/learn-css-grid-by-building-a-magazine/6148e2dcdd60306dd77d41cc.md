...
يتم تغليف الصور بعنصر `aside` باستخدام class يدعي `image-wrapper`، لذلك قم بإنشاء منتقي `.image-wrapper`. عيّن خاصية `display` إلى `grid`.

# --hints--

يجب أن يكون لديك منتقي `.image-wrapper`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.image-wrapper'));
```

يجب أن يكون لمنتقي `.image-wrapper` الخاص بك خاصية `display` بقيمة `grid`.
