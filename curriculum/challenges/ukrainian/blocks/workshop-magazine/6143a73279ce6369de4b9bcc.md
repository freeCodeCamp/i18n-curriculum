...
Створіть п’ятий `p` елемент в кінці вашого `.text` елемента і додайте до нього такий текст:
...
Ви повинні додати п’ятий `p` елемент.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 6);
```

Ваш новий `p` елемент має містити наданий текст.
