...
`article` 要素の中に、`h3` が `class` に設定された `list-title` 要素を作成し、テキストは `A Brief History` にしてください。その下に、テキストが `p` の `Of the Curriculum` 要素を作成してください。次に、クラスが `lists` の `ul` 要素を作成してください。
