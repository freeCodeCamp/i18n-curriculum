...
Ora è il momento di far passare i partecipanti attraverso il cancello. All'interno del loop `while`:  

- Decrementa `currentTickQueue` di 1 per mostrare che un partecipante è passato. Usa l'operatore di decremento (`--`).
- Incrementa `processed` di 1 per tenere traccia di quanti partecipanti sono stati processati. Usa l'operatore di incremento (`++`).

# --hints--

All'interno del tuo loop `while`, dovresti decrementare `currentTickQueue` usando l'operatore di decremento `--` e incrementare `processed` usando l'operatore di incremento `++`.
