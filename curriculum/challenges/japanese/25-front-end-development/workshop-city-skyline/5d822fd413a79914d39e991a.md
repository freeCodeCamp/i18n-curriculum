...
次のビルに取り掛かりましょう！ `.fb2` の中に 2 つの `div` 要素をネストし、`fb2a` クラスと `fb2b` クラスをこの順序で設定してください。

# --hints--

`.fb2` 内に `div` 要素を 2 つ追加する必要があります。

```js
assert.equal(document.querySelectorAll("div.fb2 > div")?.length, 2);
```

1 つ目の新しい `div` にクラス `fb2a` を設定する必要があります。
...
2 つ目の新しい `div` にクラス `fb2b` を設定する必要があります。
