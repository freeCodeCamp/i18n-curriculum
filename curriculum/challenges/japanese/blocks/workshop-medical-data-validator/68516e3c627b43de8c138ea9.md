...
別の正規表現を使う時が来ました。これまでと同様に、`and` 演算子を使って `constraints['last_visit_id']` の現在の値に式を追加してください。

`and` 演算子の右側には、`fullmatch` モジュールの `re` 関数を使い、`last_visit_id` が文字 `v`（小文字または大文字のいずれか）で始まり、その後に1つ以上の数字が続くことを確認してください。

# --hints--

`last_visit_id` 辞書の `constraints` キーは `isinstance(last_visit_id, str) and re.fullmatch('v\d+', last_visit_id, re.IGNORECASE)` の値である必要があります。
