...
스타일링을 너무 진행하기 전에 `sr-only` 클래스를 사용하세요. 이 클래스를 가진 요소를 CSS로 시각적으로 완전히 숨기면서도 화면 읽기 프로그램에서 읽히도록 만들 수 있습니다.

작성할 CSS는 요소를 시각적으로 완전히 숨기기 위해 자주 사용하는 속성 집합입니다.

`span[class~="sr-only"]` 선택자는 `span`에 `class`가 *포함된* 모든 `sr-only` 요소를 선택합니다. 그 선택자를 만들고 `border` 속성을 `0`으로 설정하세요.
...
`span[class~="sr-only"]` 선택자에 `border` 속성을 `0`으로 설정하세요.
