...
한 자리 수의 곱셈 결과가 `9`보다 커지지 않도록, 짝수 자리 수 반복문 내에 `if`가 `number`보다 크거나 같은지 확인하는 `10` 문장을 추가하세요. 그렇다면 `number`를 출력하세요.

# --hints--

`if number >= 10:` 반복문 안에 `for`가 있어야 합니다.
...
`print(number)` 문장 안에 `if`가 있어야 합니다.
