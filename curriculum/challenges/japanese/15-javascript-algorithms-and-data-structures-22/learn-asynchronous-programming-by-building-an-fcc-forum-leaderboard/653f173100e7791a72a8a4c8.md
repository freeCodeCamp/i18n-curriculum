...
1 つ前のプロジェクトでは、`.catch()` メソッドを使ってエラー処理を行いました。 ここでは、その代わりに `try...catch` 文を使用します。

`try` ブロックは潜在的なエラーを処理するための物で、もしエラーが発生した場合には `catch` ブロック内のコードが実行されます。
...
では `fetchData` 関数の中に、`try...catch` 文を追加してください。 `catch` ブロックには `err` という名前の、エラーを表す仮引数を持たせてください。

# --hints--

`fetchData` 関数に `try` ブロックを含めてください。

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}/);
```

`try` ブロックの後に `catch` ブロックを追加してください。

```js
assert.match(code, /const\s+fetchData\s*=\s*async\s*\(\s*\)\s*=>\s*{\s*try\s*{[^}]*\s*}\s*catch\s*\(.*\)\s*{[^}]*\s*}/);
```

`catch` ブロックにパラメーター `err` を含めてください。
