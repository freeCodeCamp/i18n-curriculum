...
<dfn>대체 수열</dfn> `|`는 `|`의 왼쪽 또는 오른쪽에 있는 텍스트 중 하나와 일치하도록 사용할 수 있습니다. 예를 들어, 정규 표현식 `/yes|no/`는 `yes` 또는 `no`와 일치합니다.

`helpRegex`를 `please help` 또는 `assist me`와 일치하도록 업데이트하세요.
