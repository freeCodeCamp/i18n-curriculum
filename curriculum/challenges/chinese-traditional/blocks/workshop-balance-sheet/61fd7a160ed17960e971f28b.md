...
在你的第四個 `tr` 元素中，添加一個 `th` 元素，文本爲 `Total Assets`。 將文本 `Assets` 包裹在 `span` 元素中，並將 `class` 屬性設置爲 `sr-only`。

接着，添加三個 `td` 元素，並添加文本（按順序）：`$579`、`$736`、`$809`。 給第三個 `td` 元素添加一個 `class` 屬性，設置爲 `current`。
...
你的 `span` 元素應將 `class` 屬性設置爲 `sr-only`。
...
你的第三個 `td` 元素應將 `class` 屬性設置爲 `current`。
