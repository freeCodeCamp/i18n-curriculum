...
現在是時候開始使用你解構後的資料來建置頁面的 HTML 了。你可以結合複合指定運算子（`+=`）和 `innerHTML` 屬性來完成這件事。

在你的回呼函式中，使用複合指定運算子將空的模板字面值附加到 `authorContainer` 的 `innerHTML`。
...
你應該存取 `authorContainer` 的 `innerHTML`。

```js
assert.match(code, /authorContainer\.innerHTML\s*/)
```

你應該使用複合指派，將空的模板字面值附加到 `authorContainer` 的 `innerHTML`。
