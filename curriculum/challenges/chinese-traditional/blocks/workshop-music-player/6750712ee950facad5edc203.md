...
如果你用你的播放器玩一會兒，你會注意到一些錯誤。首先，當你播放一首歌曲，暫停它，然後按上一首按鈕或下一首按鈕時，歌曲不會從頭開始播放。

為你的 `playSong` 函式（程式）添加名為 `start` 的第二個參數。將其預設值設為 `true`。

# --hints--

你的 `playSong` 函式（程式）應該有第二個名為 `start` 的參數，預設值為 `true`。
