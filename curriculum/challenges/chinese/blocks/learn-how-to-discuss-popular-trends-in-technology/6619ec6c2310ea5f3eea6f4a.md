...
当你将 `it is` 与形容词（如 `hard`、`easy`、`simple` 和其他）结合使用时，紧随其后的动词必须使用 `to + infinitive` 形式。例子：`It is easy to study for English tests.`（在形容词 `easy` 之后，动词 `study` 前必须加 `to`）。在疑问句中，将 `it` 和 `is` 颠倒。例子：`Is it simple to learn to play an instrument?`
