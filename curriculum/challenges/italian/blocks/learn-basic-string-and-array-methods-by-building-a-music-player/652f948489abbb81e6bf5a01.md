...
In questo progetto imparerai i metodi base per stringhe e array costruendo un'app per lettore musicale. Potrai riprodurre, mettere in pausa, saltare e mescolare le canzoni.

L'HTML e il CSS di questo progetto sono già forniti, così puoi concentrarti sul JavaScript.

Inizia consultando gli elementi `#playlist-songs`, `#play` e `#pause` con il metodo `getElementById()`. Assegna ciascuno alle variabili `playlistSongs`, `playButton` e `pauseButton` rispettivamente.


# --hints--

Dovresti avere una variabile chiamata `playlistSongs`. 

```js
assert.isDefined(playlistSongs);
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#playlist-songs`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)playlist\-songs\1\s*\)/);
```

Dovresti assegnare l'elemento `#playlist-songs` alla variabile `playlistSongs`. 

```js
assert.equal(playlistSongs, document.getElementById('playlist-songs'));
```

Dovresti avere una variabile chiamata `playButton`. 

```js
assert.isDefined(playButton);
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#play`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)play\1\s*\)/);
```

Dovresti assegnare l'elemento `#play` alla variabile `playButton`. 

```js
assert.equal(playButton, document.getElementById('play'));
```

Dovresti avere una variabile chiamata `pauseButton`. 

```js
assert.isDefined(pauseButton);
```

Dovresti usare `document.getElementById()` per ottenere l'elemento `#pause`.

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)pause\1\s*\)/);
```

Dovresti assegnare l'elemento `#pause` alla variabile `pauseButton`. 
