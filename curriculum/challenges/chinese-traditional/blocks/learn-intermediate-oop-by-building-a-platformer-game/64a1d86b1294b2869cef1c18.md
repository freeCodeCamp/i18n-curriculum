...
現在，當玩家跳起時，有可能超越畫布的高度。

爲了解決這個問題，你需要添加一個條件來阻止玩家跌落到畫布的高度以外。

創建一個空的 `if` 語句，檢查玩家的 `y` 位置、高度和 `y` 速度的總和是否小於或等於畫布的高度。

# --hints--

你的 `update()` 方法應該有一個 `if` 語句。
...
`if` 語句的條件應該檢查玩家的 `y` 位置、高度和 `y` 速度的總和是否小於或等於畫布的高度。
