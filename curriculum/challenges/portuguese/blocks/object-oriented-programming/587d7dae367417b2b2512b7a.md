...
Toda vez que a função construtora cria um novo objeto, o objeto é definido como uma <dfn>instance</dfn> do seu construtor. JavaScript oferece uma maneira conveniente de verificar isso com o operador `instanceof`. `instanceof` permite que você compare um objeto com um construtor, retornando `true` ou `false` com base em se esse objeto foi criado com o construtor ou não. Aqui está um exemplo:
