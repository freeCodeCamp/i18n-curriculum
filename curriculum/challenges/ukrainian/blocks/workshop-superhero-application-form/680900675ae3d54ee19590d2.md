...
Створіть елемент `button` з `className` `submit-btn` та `type` `submit`. Додайте кнопці текст `Join the League`.

Після цього спробуйте відправити форму і подивіться, що станеться.
...
Ваш елемент кнопки повинен мати атрибут `className`, встановлений у `submit-btn`.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

Ваш елемент `button` повинен мати текст `Join the League`.
