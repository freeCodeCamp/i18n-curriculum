...
Es ist Zeit, eine Löschfunktion für die Wiedergabeliste zu implementieren. Diese soll das Entfernen eines Songs aus der Wiedergabeliste verwalten, andere damit verbundene Aktionen beim Löschen eines Songs ausführen und einen `Reset Playlist`-Button erstellen.

Verwenden Sie `const` und Pfeilsyntax, um eine leere `deleteSong`-Funktion zu erstellen und `id` als Parameter zu übergeben.

# --hints--

Sie sollten `const` verwenden, um eine leere Funktion namens `deleteSong` zu erstellen.

```js
assert.match(code, /const\s+deleteSong\s*=\s*/)
```

`deleteSong` sollte eine Funktion sein.

```js
assert.isFunction(deleteSong)
```

Ihre `deleteSong`-Funktion sollte eine Pfeilsyntax verwenden.

```js
assert.match(code, /const\s+deleteSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

Ihre `deleteSong`-Funktion sollte einen `id`-Parameter annehmen.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(?\s*id/)
```

Ihre `deleteSong`-Funktion sollte leer sein.
