...
오른쪽 안쪽 귀 작업을 할 시간입니다. 클래스 선택자를 사용해 `.cat-right-inner-ear` 요소에 왼쪽과 오른쪽 테두리를 `20px solid transparent`로 지정하세요. 또한, 아래쪽 테두리는 `40px solid #3b3b4f`로 지정하세요.

# --hints--

`.cat-right-inner-ear` 선택자가 있어야 합니다.

```js 
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear'))
```

`.cat-right-inner-ear` 선택자에 `border-left` 속성을 `20px solid transparent`로 설정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderLeft, '20px solid transparent')
```

`.cat-right-inner-ear` 선택자에 `border-right` 속성을 `20px solid transparent`로 설정해야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.borderRight, '20px solid transparent')
```

`.cat-right-inner-ear` 선택자에 `border-bottom` 속성을 `40px solid #3b3b4f`로 설정해야 합니다.
