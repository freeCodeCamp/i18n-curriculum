...
Después del `form-row`, añade un `div` con el `className` de `form-group`. Dentro, agrega un elemento `label` con el `className` de `label`, un `htmlFor` de `club` y el texto `Club`. Luego añade un elemento input con un `id` de `club`, un `className` de `input` y un `type` de `text`.
