...
Para los elementos `span` dentro de cada elemento de la lista, asígnales las clases `text-green-500` y `mr-2` para que los marcadores sean verdes y se separen un poco del texto.

Además, asígnales el atributo `aria-hidden` como hiciste con los elementos `span` del primer plan de precios.

# --hints--

Tus elementos `span` deben tener la clase `text-green-500`.
...
Tus elementos `span` deben tener la clase `mr-2`.
...
Tus elementos `span` deben tener el atributo `aria-hidden`.
