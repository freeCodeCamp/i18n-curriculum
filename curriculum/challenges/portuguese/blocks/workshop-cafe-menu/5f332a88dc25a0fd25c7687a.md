---
id: 5f332a88dc25a0fd25c7687a
title: Passo 2
challengeType: 0
dashedName: step-2
---

# --description--

O nome da cafeteria é `CAMPER CAFE`. Então, adicione um elemento `h1` dentro do seu elemento `main`. Coloque o nome da cafeteria em letras maiúsculas para que ele se destaque.

# --hints--

Você deve ter uma tag `<h1>` de abertura.

```js
assert.match(code, /<h1>/i);
```

Você deve ter uma tag de fechamento `</h1>`.

```js
assert.match(code, /<\/h1>/i);
```

O elemento preexistente `main` deve permanecer o mesmo.

```js
assert.lengthOf(document.querySelectorAll('main'), 1);
```

O elemento `h1` deve estar dentro do elemento `main`.

```js
assert.equal(document.querySelector('h1')?.parentElement?.tagName, "MAIN");
```

O elemento `h1` deve conter o texto `CAMPER CAFE`.

```js
assert.equal(document.querySelector("h1")?.innerText.trim(), "CAMPER CAFE");
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Cafe Menu</title>
  </head>
  <body>
    <main>
--fcc-editable-region--
      
--fcc-editable-region--
    </main>
  </body>
</html>
```

