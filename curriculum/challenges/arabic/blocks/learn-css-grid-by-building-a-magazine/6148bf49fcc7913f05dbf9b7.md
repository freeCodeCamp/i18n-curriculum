...
أنشئ محدِّد `.author-name a:hover` وأعطه خاصية `background-color` بقيمة `#306203`.

سيُنشئ هذا تأثير المرور بالمؤشر / التأثير المرئي فقط لعنصر `a` داخل `.author-name`، مظهراً اللون الأخضر الأصلي لـ freeCodeCamp في الخلفية.

# --hints--

يجب أن يكون لديك محدِّد `.author-name a:hover`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.author-name a:hover'));
```

يجب أن يحتوي محدِّد `.author-name a:hover` على خاصية `background-color` بقيمة `#306203`.
