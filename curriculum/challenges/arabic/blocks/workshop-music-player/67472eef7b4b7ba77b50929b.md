...
في كائن `userData`، أنشئ خاصية `songs` وضع قيمتها على المصفوفة `allSongs`.

بعد ذلك، لمعالجة معلومات الأغنية الحالية وتتبع وقت تشغيلها، أنشئ خاصيتي `currentSong` و`songCurrentTime`. عيّن القيم إلى `null` و`0` على التوالي.

# --hints--

يجب أن يحتوي كائن `userData` على مفتاح `songs` مضبوط على المصفوفة `allSongs`.

```js
assert.propertyVal(userData, "songs", allSongs);
```

يجب أن يحتوي كائن `userData` على مفتاح `currentSong` مضبوط على `null`.

```js
assert.propertyVal(userData, "currentSong", null);
```

يجب أن يحتوي كائن `userData` على مفتاح `songCurrentTime` مضبوط على `0`.
