---
id: 660011d015f7d43626524ef8
title: 작업 22
challengeType: 22
dashedName: task-22
lang: en-US
---

<!-- (Audio) Sarah: Absolutely. You should look for a description of what the function does and any return values. -->

# --description--

`description`는 어떤 것에 대한 자세한 설명입니다. 예를 들어, `The report included a detailed description of the project.`

프로그래밍에서 `function`는 특정 작업을 수행하는 명령어 집합입니다. 예를 들어, `The "calculateTotal" function adds up prices.`

# --fillInTheBlank--

## --sentence--

`Absolutely. You should look for a BLANK of what the BLANK does and any return values.`

## --blanks--

`description`

### --feedback--

함수의 목적과 동작에 대한 자세한 설명을 의미합니다.

---

`function`

### --feedback--

프로그래밍에서 특정 작업을 수행하는 명령어 집합입니다.

# --scene--

```json
{
  "setup": {
    "background": "company2-breakroom.png",
    "characters": [
      {
        "character": "Sarah",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "7.2-2.mp3",
      "startTime": 1,
      "startTimestamp": 19.9,
      "finishTimestamp": 24.88
    }
  },
  "commands": [
    {
      "character": "Sarah",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Sarah",
      "startTime": 1,
      "finishTime": 5.98,
      "dialogue": {
        "text": "Absolutely. You should look for a description of what the function does and any return values.",
        "align": "center"
      }
    },
    {
      "character": "Sarah",
      "opacity": 0,
      "startTime": 6.48
    }
  ]
}
```
