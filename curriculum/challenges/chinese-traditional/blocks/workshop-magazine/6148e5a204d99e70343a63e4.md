...
`gap` 屬性是一種簡寫方式，可同時設定 `column-gap` 和 `row-gap` 的值。如果只給定一個值，則會將 `column-gap` 和 `row-gap` 都設定為該值。如果給定兩個值，則會將 `row-gap` 設定為第一個值，`column-gap` 設定為第二個值。

給 `.image-wrapper` 選擇器一個設定為 `2rem` 的 `gap` 屬性。

# --hints--

你的 `.image-wrapper` 元素應該設定 `gap` 屬性為 `2rem`。
