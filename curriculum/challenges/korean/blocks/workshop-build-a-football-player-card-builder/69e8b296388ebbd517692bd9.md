...
Club `value`에 `player.club` 속성을 `input`로 추가하세요. 그다음, `onChange`를 매개변수로 받고 `e`를 펼친 것과 `setPlayer`를 `player`로 설정하여 `club`를 호출하는 화살표 함수로 설정된 `e.target.value` 속성을 추가하세요.

# --hints--

Club `input`에는 플레이어의 클럽과 같은 `value`이 있어야 합니다.
...
Club `input`에는 `onChange`로 설정된 `setPlayer`와 `player`를 펼쳐서 `club`를 호출하는 `e.target.value` 핸들러가 있어야 합니다.
