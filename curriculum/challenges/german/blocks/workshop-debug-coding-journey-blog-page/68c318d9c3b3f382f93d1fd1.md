...
Unter dem Haupttitel der Seite befindet sich ein Navigationsabschnitt, der Links zu jedem Blogbeitrag enthält.

Dieser Abschnitt sollte in einem `nav`-Element eingeschlossen sein, um seine Funktion als Navigationsmarkierung anzuzeigen.

# --hints--

Ihre Seite sollte ein `nav`-Element enthalten.
...
Ihr `<h2>Navigation</h2>` sollte sich innerhalb des `nav`-Elements befinden.
...
Ihr `ul`-Element sollte sich innerhalb des `nav`-Elements befinden.
...
Ihre `li`-Elemente sollten sich innerhalb des `ul`-Elements befinden.
...
Sie sollten keine zusätzlichen `ul`-, `li`- oder `h2`-Elemente hinzufügen.
