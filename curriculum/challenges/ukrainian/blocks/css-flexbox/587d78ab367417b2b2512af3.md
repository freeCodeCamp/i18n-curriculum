...
title: "Застосуйте властивість flex-direction, щоб створити рядки у вставці твіта"
challengeType: 0
...
`header` і `footer` у прикладі вставки твіта мають дочірні елементи, які можна розташувати у рядки за допомогою властивості `flex-direction`. Це вказує CSS вирівнювати дочірні елементи горизонтально.

# --instructions--

Додайте CSS-властивість `flex-direction` до обох `header` і `footer` та встановіть значення `row`.

# --hints--

Ваш `.follow-btn` має відтворюватися на сторінці. Переконайтеся, що вимкнено будь-які розширення, наприклад блокувальники реклами.
...
`header` має мати властивість `flex-direction`, встановлену в `row`.

```js
assert.match(code, /header\s*?{[^}]*?flex-direction:\s*?row;/g);
```

`footer` має мати властивість `flex-direction`, встановлену в `row`.
