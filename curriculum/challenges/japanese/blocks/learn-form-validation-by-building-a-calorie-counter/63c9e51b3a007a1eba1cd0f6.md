...
`remainingCalories` 変数を宣言し、`consumedCalories` から `budgetCalories` を引いて `exerciseCalories` を足した値を代入してください。

# --hints--

あなたの `calculateCalories` 関数には `remainingCalories` 変数が必要です。

```js
assert.match(calculateCalories.toString(), /remainingCalories\s*=/);
```

`budgetCalories - consumedCalories + exerciseCalories` の値を求めてください。
