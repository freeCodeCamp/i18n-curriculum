...
太陽を画面の右上隅に配置し、上端と右端の`75px`が画面外に出るようにしてください。

# --hints--

`.sun`に`top`の`--fcc-expected--`を与える必要がありますが、`--fcc-actual--`が見つかりました。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.top, '-75px');
```

`.sun`に`right`の`--fcc-expected--`を与える必要がありますが、`--fcc-actual--`が見つかりました。
