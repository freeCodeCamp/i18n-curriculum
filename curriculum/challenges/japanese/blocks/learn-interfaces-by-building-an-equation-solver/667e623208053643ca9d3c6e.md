...
前のステップで追加した`for`ループと`if`文を、`if`組み込み関数を使った`any()`文に置き換えてください。

# --hints--

新しい`if`文の条件は`any()`を呼び出す式にしてください。
...
`any()`に渡すジェネレータ式は`args`を繰り返す必要があります。
...
`if`文は、`args`の引数のいずれかが`int`または`float`のインスタンスでないかどうかをチェックしてください。
...
新しい`TypeError`文の中で、指定された文字列を使って`if`を発生させてください。
