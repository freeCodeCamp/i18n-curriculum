...
Im Moment hat sich die Reihenfolge der Songs nicht geändert. Das liegt daran, dass die Aktualisierungen, die Sie mit der `sort`-Methode vorgenommen haben, erst ausgeführt werden, wenn die `sortSongs`-Funktion aufgerufen wird.

Ändern Sie Ihre `renderSongs`-Funktion so, dass sie die `sortSongs`-Funktion aufruft.

Jetzt sollten Sie die Songs in alphabetischer Reihenfolge sehen.

# --hints--

Sie sollten `renderSongs(sortSongs())` haben.
