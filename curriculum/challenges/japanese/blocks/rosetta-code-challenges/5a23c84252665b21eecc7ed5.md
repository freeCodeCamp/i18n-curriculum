...
ナイトの巡回問題：空の`width` \* `height`のチェスボードがありますが、あるマスにナイトが1つだけ置かれています。ナイトは合法的な手を連続して動かし、チェスボード上のすべてのマスをちょうど一度ずつ訪れる必要があります。巡回が「閉じている」必要はありません。つまり、ナイトが開始位置から1手で戻る必要はありません。

# --instructions--

`width`と`height`をパラメータに取り、上記の課題を達成できる初期位置の数を返す関数を書いてください。

# --hints--

`knightTour`は関数である必要があります。
