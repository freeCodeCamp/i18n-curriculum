...
`div`の中に新しい`.bb2`要素を2つネストし、それぞれに`bb2a`と`bb2b`のクラスを順番に付けてください。これらはこの建物の2つのセクションになります。

# --hints--

`div`に`.bb2`要素を2つ追加してください。

```js
assert.equal(document.querySelector('div.bb2')?.children?.length, 2);
```

最初の`div`に`bb2a`のクラスを付けてください。

```js
assert.exists(document.querySelector('div.bb2a'));
```

2番目の`div`に`bb2b`のクラスを付けてください。
