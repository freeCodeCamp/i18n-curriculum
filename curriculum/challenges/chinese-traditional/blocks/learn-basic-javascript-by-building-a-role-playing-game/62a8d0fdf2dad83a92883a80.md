...
一旦玩家擁有最好的武器，他們就無法再購買另一把。將你 `buyWeapon` 函式中的所有程式碼包裹在另一個 `if` 述語中。條件應該檢查 `currentWeaponIndex` 是否小於 `3` —— 最後一把武器的索引。

# --hints--

你應該有一個新的 `if` 述語來檢查 `currentWeaponIndex` 是否小於 `3`。

```js
assert.match(buyWeapon.toString(), /currentWeaponIndex\s*\<\s*3/)
```

你現有的 `if` 述語應該位於你新的 `if` 述語內。
