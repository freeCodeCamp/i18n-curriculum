...
`const` と分割代入を使って、`name` から `price` と `product` の変数を取り出してください。

# --hints--

`name` と `price` の変数を取得するには分割代入を使うべきです。
...
`const` と `name` の変数を宣言するには `price` を使うべきです。
...
`name` から `price` と `product` の変数を取得するには分割代入を使うべきです。
