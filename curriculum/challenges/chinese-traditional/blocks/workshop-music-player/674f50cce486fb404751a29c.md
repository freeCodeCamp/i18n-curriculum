...
現在你已經知道如何找到下一首要播放的歌曲，請創建一個名為 `playNextSong` 的函式（程式）。在 `playNextSong` 函式（程式）內，創建一個 `if` 述語來檢查 `userData` 的 `currentSong` 是否嚴格相等於 `null`。這將檢查 `userData` 物件中是否沒有正在播放的歌曲。

如果條件為真，則以 `userData.songs` 陣列中第一首歌曲的 `id` 作為引數呼叫 `playSong` 函式。

然後在 `if` 區塊的結尾添加一個 `return` 述語。

# --hints--

你應該有一個名為 `playNextSong` 的函式（程式）。

```js
assert.isFunction(playNextSong);
```

當 `userData` 的 `currentSong` 嚴格相等於 `null` 時，你的 `playNextSong` 函式（程式）應該使用 `userData.songs` 陣列中第一首歌曲的 `id` 作為引數來呼叫 `playSong` 函式（程式）。
