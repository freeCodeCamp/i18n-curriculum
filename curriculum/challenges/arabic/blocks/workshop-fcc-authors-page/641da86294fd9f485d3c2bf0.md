...
بعد ذلك، أضف رابطًا إلى صفحة المؤلف على أخبار freeCodeCamp.

أضف عنصر وسم الرابط مع خاصية `class` بقيمة `"author-link"`، واستبدل `url` كقيمة لخاصية `href`، واضبط `target` على `"_blank"`. بالنسبة لنص عنصر وسم الرابط، استبدل `author` متبوعًا بالنص `"'s author page"`. على سبيل المثال، `"Quincy Larson's author page"`.
...
يجب أن يحتوي عنصر وسم الرابط على الصنف `"author-link"`.

```js
assert.include(document.querySelector('a')?.className, "author-link");
```

يجب أن تستبدل `url` كقيمة لخاصية `href` في عنصر وسم الرابط.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), authorDataArr[0].url);
```

يجب أن تضبط خاصية `target` في عنصر وسم الرابط على `"_blank"`.

```js
assert.equal(document.querySelector('a')?.getAttribute('target'), '_blank');
```

يجب أن تستبدل `author` متبوعًا بالنص `"'s author page"` داخل عنصر وسم الرابط.
