...
Die zweite Bedingung in diesem Beispiel prüft, ob `a.name > b.name`. Falls ja, gibt die Funktion `1` zurück, was dazu führt, dass die erste Frucht nach der zweiten Frucht sortiert wird.
...
Fügen Sie in Ihrer Callback-Funktion eine weitere `if`-Anweisung hinzu, um zu prüfen, ob `a.title` größer als `b.title` ist. Falls ja, geben Sie die Zahl `1` zurück.

# --hints--

Sie sollten eine zweite `if`-Anweisung innerhalb Ihrer Callback-Funktion haben.
...
Ihre `if`-Anweisung sollte prüfen, ob `a.title` größer als `b.title` ist.

```js
assert.match(code, /if\s*\(\s*a\.title\s*>\s*b\.title\s*\)/);
```

Ihre `if`-Anweisung sollte die Zahl `1` zurückgeben, wenn `a.title` größer als `b.title` ist.
