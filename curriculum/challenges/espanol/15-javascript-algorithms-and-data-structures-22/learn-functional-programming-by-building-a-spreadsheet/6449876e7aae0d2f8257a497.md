...
Declare una función flecha `rangeFromString` que tome dos parámetros, `num1` y `num2`. La función debe devolver implícitamente el resultado de llamar a `range` con `num1` y `num2` como argumentos.

Para estar seguro, convierta `num1` y `num2` en enteros al pasarlos a `range`.

# --hints--

Debes declarar una variable `rangeFromString` después de tu `rangeRegex`.
...
Debes usar `const` para declarar tu variable `rangeFromString`.
...
Tu variable `rangeFromString` debe ser una función flecha.
...
Tu función `rangeFromString` debe tener `num1` como el primer parámetro.
...
Tu función `rangeFromString` debe tener `num2` como el segundo parámetro.
...
Tu función `rangeFromString` debe usar un retorno implícito.
...
Tu función `rangeFromString` debe devolver el resultado de llamar a tu función `range`.
...
Debes llamar a `parseInt` con `num1` como argumento y pasar el resultado a la llamada `range`.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\)\/(gi|ig)\s*;?\s*const\s+rangeFromString\s*=\s*\(\s*num1\s*,\s*num2\s*\)\s*=>\s*range\(\s*parseInt\(\s*num1\s*\)/);
```

Debes llamar a `parseInt` con `num2` como argumento y pasar el resultado a la llamada `range`.
