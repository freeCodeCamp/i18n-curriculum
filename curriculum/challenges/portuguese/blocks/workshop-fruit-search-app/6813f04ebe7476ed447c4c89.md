...
Em seguida, chame `.json()` na resposta e armazene o resultado em uma variável chamada `data`. Como você ainda está dentro de uma função `async`, deve usar `await` antes de `response.json()`.

# --hints--

Você deve chamar `json()` em `response` e armazená-lo em uma variável `data`.
