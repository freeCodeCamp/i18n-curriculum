...
다음 단계는 `width` 클래스에 `height`와 `CheckPoint`를 추가하는 것입니다.

`width`와 `height`는 각각 `proportionalSize(40)`와 `proportionalSize(70)`여야 합니다.

# --hints--

`width` 클래스 안에 `CheckPoint` 속성이 있어야 합니다.
...
`height` 클래스 안에 `CheckPoint` 속성이 있어야 합니다.
...
`width` 속성에 `proportionalSize(40)`를 할당해야 합니다.
...
`height` 속성에 `proportionalSize(70)`를 할당해야 합니다.
