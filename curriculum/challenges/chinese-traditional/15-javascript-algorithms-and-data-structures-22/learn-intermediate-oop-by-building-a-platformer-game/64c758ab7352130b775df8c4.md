...
在 map 回調函數中，傳入 `platform` 作爲參數，並隱式返回創建一個新的 `Platform` 實例，並將 `platform.x` 和 `platform.y` 值作爲參數傳入。

# --hints--

你的 `map` 回調應隱式返回新的 `Platform` 實例的創建，並將 `platform.x` 和 `platform.y` 值作爲參數傳入。
