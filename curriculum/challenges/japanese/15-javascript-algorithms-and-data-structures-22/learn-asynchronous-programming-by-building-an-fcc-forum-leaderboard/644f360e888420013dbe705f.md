...
`else` 節の中で、`selectedCategory.className` に文字列 `"general"` を代入してください。

その次に `selectedCategory.category` に文字列 `"General"` を代入してください。

最後に、`selectedCategory.id` に数値の `1` を代入してください。

# --hints--

`selectedCategory.className` に文字列 `"general"` を代入してください。

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

`selectedCategory.category` に文字列 `"General"` を代入してください。

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

`selectedCategory.id` に数値の `1` を代入してください。
