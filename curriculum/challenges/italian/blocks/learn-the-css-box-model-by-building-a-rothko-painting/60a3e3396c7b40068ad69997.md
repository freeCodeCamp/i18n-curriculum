...
Ruota il contatore `.three` in senso antiorario di 0,2 gradi.

Con questo passaggio finale, il tuo dipinto Rothko è ora completo.

# --hints--

Dovresti impostare la proprietà `transform` su `rotate(-0.2deg)`.
...
Il tuo elemento `.three` dovrebbe avere un valore `transform` di `rotate(-0.2deg)`.
