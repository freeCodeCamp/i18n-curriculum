...
Test di Lucas-Lehmer: per $p$ un primo dispari, il numero di Mersenne $2^p-1$ è primo se e solo se $2^p-1$ divide $S(p-1)$ dove $S(n+1)=(S(n))^2-2$, e $S(1)=4$.

# --instructions--

Scrivi una funzione che restituisca se il numero di Mersenne dato è primo oppure no.
