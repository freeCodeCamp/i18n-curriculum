...
Vamos passar para o selo `Most popular`.Você precisa colocá-lo no canto superior direito do cartão, então você pode usar algumas das classes utilitárias de posicionamento do Tailwind para fazer isso.

Aqui está como você usaria o posicionamento CSS no Tailwind:
...
Atribua a classe `relative` ao `div` wrapper e `absolute` ao `div` com o texto `Most popular`.

# --hints--

Seu elemento wrapper `div` para o plano premium deve ter a classe `relative`.
...
Seu elemento `div` com o texto `Most popular` deve ter a classe `absolute`.
