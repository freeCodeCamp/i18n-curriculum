...
<dfn>find()</dfn> 메서드는 배열 내에서 제공된 콜백 함수에 지정된 조건을 충족하는 첫 번째 요소를 가져옵니다. 조건을 만족하는 요소가 없으면 이 메서드는 `undefined`를 반환합니다.

아래 예제에서는 `find()` 메서드를 사용해 25보다 큰 첫 번째 숫자를 찾습니다:
...
`const`를 사용해 `song`라는 변수를 만들고 `find()` 배열에서 `userData?.songs` 메서드 호출 결과를 할당하세요. `song` 콜백의 매개변수로 `find()`를 사용하고 `song.id`가 `id`와 엄격히 같은지 확인하세요.

이렇게 하면 `userData?.songs` 배열을 순차적으로 반복하며 `id` 함수에 전달된 `playSong`에 해당하는 노래를 검색합니다.

# --hints--

`find`에서 `userData?.songs` 메서드를 사용하세요.

```js
assert.match(code, /userData\?\.songs\.find\(/)
```

`find` 메서드의 화살표 함수 콜백 매개변수로 `song`를 사용하세요.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)/)
```

`find` 메서드는 `song.id`가 `id`와 엄격히 같은지 확인해야 합니다.

```js
assert.match(code, /userData\?\.songs\.find\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*===\s*id|id\s*===\s*song\.id)\s*\)\s*;?/)
```

`find` 메서드는 `song` 상수에 할당되어야 합니다.
