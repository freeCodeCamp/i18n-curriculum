...
最后，某些类型的动态动画可能会让某些用户感到不适。 特别是，患有<dfn>前庭</dfn>功能障碍的人对某些运动触发器敏感。

`@media` 规则一个名为 `prefers-reduced-motion` 的媒体功能，可以根据用户的动画偏好设置 CSS。 它可以使用以下几个值之一：
...
你将在以后的课程中进一步了解这一功能。 现在，将设置 `scroll-behavior: smooth` 的样式规则包装在 `@media` 规则中，并将媒体功能 `prefers-reduced-motion` 的值设置为 `no-preference`。
