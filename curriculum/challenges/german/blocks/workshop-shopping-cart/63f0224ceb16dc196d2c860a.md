...
Sie müssen alle `Add to cart`-Buttons abrufen, die Sie zuvor zum DOM hinzugefügt haben. Deklarieren Sie eine `addToCartBtns`-Variable und weisen Sie ihr den Wert zu, der durch den Aufruf der `getElementsByClassName()`-Methode am `document`-Objekt mit dem String `"add-to-cart-btn"` als Argument zurückgegeben wird.

# --hints--

Sie sollten `const` verwenden, um Ihre `addToCartBtns`-Variable zu deklarieren.

```js
assert.match(code, /const\s+addToCartBtns\s*=/);
```

Sie sollten die `getElementsByClassName()`-Methode am `document`-Objekt aufrufen.

```js
assert.match(code, /document\s*\.\s*getElementsByClassName\s*\(/);
```

Sie sollten den String `"add-to-cart-btn"` an die `getElementsByClassName()`-Methode übergeben.

```js
assert.match(code, /document\s*\.\s*getElementsByClassName\s*\(\s*('|"|`)add-to-cart-btn\1\s*\)/);
```

Sie sollten den Wert, der von der `getElementsByClassName()`-Methode zurückgegeben wird, der `addToCartBtns`-Variable zuweisen.
