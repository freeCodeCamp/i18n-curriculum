...
今、`showElements`の中で、これらを引数として渡してください：`singleCardBtn`、`multipleCardsBtn`、そして`headerTitle`。これらは`elements`から取得し、`this`キーワードでアクセスします。

# --hints--

`singleCardBtn`関数の最初の引数として`showElements`を渡してください。
...
`multipleCardsBtn`関数の2番目の引数として`showElements`を渡してください。
...
`headerTitle`関数の引数として`showElements`を渡してください。
