...
Bevor Sie die Simulation starten, benötigen Sie ein Objekt, um zu verfolgen, wie viele Teilnehmer jedes Tor über alle Ticks hinweg verarbeitet. Rufen Sie `initializeThroughput()` mit `gates` auf und weisen Sie das Ergebnis einer Variablen namens `throughputSummary` zu.

# --hints--

Sie sollten `initializeThroughput()` mit `gates` aufrufen und das Ergebnis einer Variablen namens `throughputSummary` zuweisen.
...
Ihre `simulateFestival`-Funktion sollte die `initializeThroughput()`-Funktion mit `gates` aufrufen.
