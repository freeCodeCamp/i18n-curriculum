...
在你的 `.calories-info` 元素中，創建一個 `div` 元素。 將該 `div` 元素的 `class` 屬性設置爲 `left-container`。 在新創建的 `div` 元素中，創建一個帶有文本 `Amount per serving` 的 `h2` 元素。 將 `h2` 元素的 `class` 屬性設置爲 `bold small-text`。

# --hints--

你應該在 `.calories-info` 元素中創建一個新的 `div` 元素。

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.localName === 'div');
```

你的新的 `div` 元素應該將 `class` 設置爲 `left-container`。

```js
assert(document.querySelector('.calories-info')?.children?.[0]?.classList.contains('left-container'));
```

你應該在 `.left-container` 元素中創建一個新的 `h2` 元素。

```js
assert(document.querySelector('.left-container')?.children?.[0]?.localName === 'h2');
```

你的新的 `h2` 元素應該有一個 `class` 屬性，設置爲 `bold small-text`。
...
你的新的 `h2` 元素應包含文本 `Amount per serving`。
