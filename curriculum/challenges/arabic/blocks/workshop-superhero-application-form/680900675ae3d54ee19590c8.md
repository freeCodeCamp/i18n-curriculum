...
بعد ذلك، أنشئ `label` مع `className` من نوع `section` و`column`، والنص `How did you get your powers?`. داخل الـ `label`، أنشئ عنصر `select`.

# --hints--

يجب أن تنشئ عنصر `label` مع `className` من نوع `section` و`column`.
...
يجب أن يحتوي عنصر `label` على النص `How did you get your powers?`.
...
يجب أن تنشئ عنصر `select` داخل عنصر `label`.
