...
أعطِ العنصر `.fb3a` `width` بقيمة `80%` و`height` بقيمة `15%`. ثم أعطِ العنصر `.fb3b` `width` بقيمة `100%` و`height` بقيمة `35%`.

# --hints--

يجب أن تعطي `.fb3a` `width` بقيمة `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

يجب أن تعطي `.fb3a` `height` بقيمة `15%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

يجب أن تعطي `.fb3b` `width` بقيمة `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3b")?.width, "100%");
```

يجب أن تعطي `.fb3b` `height` بقيمة `35%`.
