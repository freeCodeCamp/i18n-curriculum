...
Щоб зберегти поточний час пісні, коли вона призупинена, призначте `songCurrentTime` об’єкта `userData` значення `currentTime` змінної `audio`.

*Примітка*: Не слід використовувати опціональний ланцюжок на цьому кроці, оскільки `userData.songCurrentTime` не буде `null` або `undefined` у цей момент.

# --hints--

Не змінюйте існуючу функцію `pauseSong` та її вміст.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Ви повинні призначити `audio.currentTime` `userData.songCurrentTime`.
