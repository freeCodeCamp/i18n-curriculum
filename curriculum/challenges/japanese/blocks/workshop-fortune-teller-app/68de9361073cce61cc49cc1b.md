...
前の講義では、型推論がTypeScriptの機能であり、変数、関数の戻り値、式の使用状況や文脈に基づいて自動的に型を決定し代入することができると学びました。

TypeScriptでは、ジェネリクスによって関数、クラス、型がより柔軟になります。ジェネリクスは型変数を作成できるため、毎回明示的に型を定義する必要がなくなります。これにより、同じ関数が異なる型で安全に動作できるようになります。
...
関数は2つ以上のジェネリックを使うことができます。`T`は慣例として使われ、必要に応じて`S`のような追加の型変数を加えることができます。
...
`selector`を`string`型のパラメータとして受け取り、`HTMLElement`を拡張するジェネリック型`T`を使い、戻り値の型が`T`である関数`getElement`を作成してください。

# --hints--

`getElement`という名前のアロー関数を用意してください。
...
`getElement`関数は`selector`という名前の`string`型パラメータを受け取る必要があります。
...
ジェネリック型パラメータ`T`は`HTMLElement`型を拡張している必要があります。
...
`getElement`関数の戻り値の型は`T`である必要があります。
