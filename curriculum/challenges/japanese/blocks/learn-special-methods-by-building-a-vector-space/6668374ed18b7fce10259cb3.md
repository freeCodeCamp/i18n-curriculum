...
属性アクセスの仕組みがわかったところで、コードの最後の2行と`__getattr__`メソッドを削除してください。

次に、以前コメントアウトした`print()`の呼び出しを元に戻し、両方の呼び出しから2番目の引数を削除してください。

# --hints--

`__getattr__`メソッドは存在しないはずです。

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).find_class("R2Vector").has_function("__getattr__")`)) })
```

コード内に`print(v1.z)`と`print(getattr(v1, 'z'))`があってはいけません。
...
以前コメントアウトした`print()`の呼び出しを元に戻し、両方の呼び出しから2番目の引数を削除してください。
