...
表示されている曲を再生する機能の実装を始める時間です。

`playSong` を使って `const` 関数を定義してください。この関数は、再生したい曲の一意の識別子を表す `id` パラメータを受け取る必要があります。

# --hints--

空の関数 `const` を作成するには `playSong` を使うべきです。
