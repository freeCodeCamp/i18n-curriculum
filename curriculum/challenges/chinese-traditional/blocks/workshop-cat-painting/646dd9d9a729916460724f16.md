...
將右內耳移動到位，將其 `position` 屬性設置爲 `absolute`，`top` 爲 `22px`，`left` 爲 `-20px`。

# --hints--

你的 `.cat-right-inner-ear` 選擇器應設置 `position` 屬性爲 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.position, 'absolute')
```

你的 `.cat-right-inner-ear` 選擇器應設置 `top` 屬性爲 `22px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.top, '22px')
```

你的 `.cat-right-inner-ear` 選擇器應設置 `left` 屬性爲 `-20px`。
