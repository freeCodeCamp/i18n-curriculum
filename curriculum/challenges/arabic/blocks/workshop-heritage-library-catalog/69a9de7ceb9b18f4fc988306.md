...
داخل الحلقة، تحقق مما إذا كان مؤلف الإدخال الحالي (بعد تحويله إلى أحرف صغيرة) يتضمن `searchTerm`. تُعيد الطريقة `.includes()` القيمة `true` إذا كانت السلسلة النصية تحتوي على سلسلة نصية أخرى كجزء فرعي. إذا تحقق الشرط، أضف الإدخال إلى `results`. بعد انتهاء الحلقة، أرجع `results`.

# --hints--

يجب أن تكون قيمة `findByAuthor(catalog, "king").length` هي `6`.

```js
assert.strictEqual(findByAuthor(catalog, "king").length, 6);
```

يجب أن تكون قيمة `findByAuthor(catalog, "asimov").length` هي `2`.

```js
assert.strictEqual(findByAuthor(catalog, "asimov").length, 2);
```

يجب أن تكون قيمة `findByAuthor(catalog, "xyz").length` هي `0`.
