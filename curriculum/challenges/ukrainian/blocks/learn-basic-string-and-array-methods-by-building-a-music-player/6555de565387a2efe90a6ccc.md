...
Функція `setPlayButtonAccessibleText` встановить атрибут `aria-label` на поточну пісню або на першу пісню в плейлисті. Якщо плейлист порожній, вона встановить `aria-label` у `"Play"`.

Використайте метод `setAttribute` на елементі `playButton`, щоб встановити атрибут з іменем `"aria-label"`. За допомогою тернарного оператора встановіть значення атрибута на `Play ${song.title}` або `"Play"`, якщо `song?.title` недоступний.

Не забудьте, що тут потрібна інтерполяція рядків, тому використовуйте зворотні лапки.

# --hints--

Ви не повинні змінювати існуючу функцію `setPlayButtonAccessibleText` та її вміст.
...
Ви повинні використати метод `setAttribute()` на `playButton`.

```js
assert.match(code, /playButton\.setAttribute\(\s*/)
```

`"aria-label"` має бути першим значенням вашого методу `setAttribute()`.

```js
assert.match(code, /playButton\.setAttribute\(\s*('|")aria-label\1/)
```

Ваш метод `setAttribute` має мати ``song?.title ? `Play ${song.title}` : "Play"`` як другий аргумент.
