...
数値入力が空の場合や値が数字でない場合にユーザーに警告した後、`return` キーワードを使ってこの関数から早期に抜けることができます。これにより、この関数内の後続のコードが実行されるのを防げます。

`return` の後に `alert()` キーワードを追加してください。

# --hints--

`return` の後に `alert()` キーワードを使うべきです。
