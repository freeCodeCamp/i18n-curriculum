...
1. 该形式应包含一个 id 为 `select` 的 `position` 元素，允许用户选择职位。
...
1. 为你的 `:focus` 和 `input` 元素添加 `textarea` 伪类，以在聚焦时更改它们的边框颜色并移除默认轮廓。
...
1. 为你的 `:first-of-type` 元素添加 `input` 伪类，以不同方式样式化第一个输入字段。（例如，圆角）。
...
该形式应包含一个 id 为 `select` 的 `position` 元素以及一些 `option` 元素。
...
为你的 `:first-of-type` 元素添加 `input` 伪类，以不同方式样式化第一个输入字段。
