...
在先前的模組中你學到，CSS 中的漸層是一種在元素距離間過渡顏色的方式。它們被應用到 `background` 屬性，語法看起來像這樣：
...
在範例中，`color1` 在頂部是實心的，`color2` 在底部是實心的，而中間則均勻地從一個過渡到另一個。在 `.bb1a` 中，於 `background-color` 屬性下方添加一個 `background` 屬性。將其設為使用 `--building-color1` 作為第一個顏色、`--window-color1` 作為第二個顏色的 `linear-gradient` 型別的漸層。

# --hints--

你應該在 `background-color` 之後，立即為 `.bb1a` 套用 `background`。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1a')?.background);
```

你應該給 `background` 一個 `linear-gradient`。

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.bb1a')?.background, 'linear-gradient');
```

你應該為 `background` 添加從 `var(--building-color1)` 開始的 `linear-gradient`。
...
你應該給 `background` 一個以 `var(--window-color1)` 結尾的 `linear-gradient`。
