...
Ndani ya darasa la `Bee`, tekeleza njia ya `render`, ikiwa na kiambishi awali cha `override`, ili kusahihisha sifa ya `innerText` ya `emojiElement` kwa thamani ya sifa ya `emoji`.

# --hints--

Darasa lako la `Bee` linapaswa kuwa na njia ya `render` ikitumia kiambishi awali cha `override`.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

Unapaswa kuweka `this.emojiElement.innerText` kuwa `this.emoji` ndani ya njia ya `render` ya `Bee`.
