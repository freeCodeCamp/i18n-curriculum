...
一種選擇是 `grid-column` 屬性，它是 `grid-column-start` 和 `grid-column-end` 的簡寫。 `grid-column` 屬性告訴網格項從哪條網格線開始和結束。

創建 `.heading` 規則，並將 `grid-column` 屬性設置爲 `2 / 3`。 這將告訴 `.heading` 元素從網格線 2 開始並在網格線 3 結束。
...
你的 `.heading` 選擇器應該有一個 `grid-column` 屬性設置爲 `2 / 3`。
