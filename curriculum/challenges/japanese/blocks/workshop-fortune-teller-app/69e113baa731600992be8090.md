...
カードのデフォルト画像とAPIリクエスト用のURLが提供されているので、それらを保存するための変数をいくつか作成してください。

`CDN_URL` という変数を作成し、`https://cdn.freecodecamp.org/curriculum/typescript/tarot-app` に設定してください。また、`defaultImg` という変数を作成し、値を `new URL("default.svg", CDN_URL + "/")` に設定し、さらに `LOCAL_DEFAULT_IMG` という変数を作成して値を `defaultImg` に設定してください。

# --hints--

`CDN_URL` という名前の変数を作成してください。
...
`CDN_URL` 変数は `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"` に設定してください。
...
あなたの `defaultImg` 変数は `new URL("default.svg", CDN_URL + "/")` に設定されている必要があります。
...
`LOCAL_DEFAULT_IMG` という名前の変数を作成してください。
...
`LOCAL_DEFAULT_IMG` 変数は `defaultImg` に設定してください。
