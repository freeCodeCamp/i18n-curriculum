...
這個算命師應用程式支援呈現多張卡片，所以你需要有一個方法能幫助呈現多張卡片。宣告一個名為 `multipleCardSelected` 的方法。

在這個方法內，呼叫 `hideElements` 函式並傳入屬性 `singleCard`、`singleCardBtn`、`multipleCardsBtn` 和 `headerTitle`。別忘了你必須從 `this.elements` 取用它們。

# --hints--

你應該宣告一個 `multipleCardSelected` 方法。
...
你應該在 `hideElements` 方法內呼叫 `multipleCardSelected` 函式。
...
你應該將 `singleCard` 作為第一個引數傳給 `hideElements` 函式。
...
你應該將 `singleCardBtn` 作為引數傳給 `hideElements` 函式。
...
你應該將 `multipleCardsBtn` 作為引數傳給 `hideElements` 函式。
...
你應該將 `headerTitle` 作為引數傳給 `hideElements` 函式。
