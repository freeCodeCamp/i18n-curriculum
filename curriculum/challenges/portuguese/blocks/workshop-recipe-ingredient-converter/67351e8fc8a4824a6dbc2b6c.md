...
Dê à sua `conversionTable` uma propriedade `cup` e atribua a ela um objeto com uma propriedade `gram` definida como `240`, uma propriedade `ounce` definida como `8.0` e uma propriedade `teaspoon` definida como `48`.

# --hints--

Sua `conversionTable` deve ter uma propriedade `cup`.

```js
assert.property(conversionTable, "cup");
```

Sua propriedade `cup` deve ser um objeto.

```js
assert.isObject(conversionTable.cup);
```

Seu objeto `cup` deve ter uma propriedade `gram`.

```js
assert.property(conversionTable.cup, "gram");
```

Sua propriedade `gram` deve ser configurada para `240`.

```js
assert.strictEqual(conversionTable.cup?.gram, 240);
```

Seu objeto `cup` deve ter uma propriedade `ounce`.

```js
assert.property(conversionTable.cup, "ounce");
```

A sua propriedade `ounce` deve ser configurada para `8.0`.

```js
assert.strictEqual(conversionTable.cup?.ounce, 8.0);
```

Seu objeto `cup` deve ter uma propriedade `teaspoon`.

```js
assert.property(conversionTable.cup, "teaspoon");
```

A sua propriedade `teaspoon` deve ser definida como `48`.
