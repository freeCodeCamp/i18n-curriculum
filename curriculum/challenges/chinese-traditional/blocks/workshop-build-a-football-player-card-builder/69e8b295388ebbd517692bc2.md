...
在 JSX 和 TSX 中，你可以使用大括號將 JavaScript 表達式作為屬性值傳遞。模板字串使用反引號和 `${}` 來進行插值。例如：

```tsx
<div className={`prefix-${someVariable}`}>
```

卡片外覆 `className` 目前被硬編碼為 `card-wrapper tier-gold`。請使用模板字串呼叫 `gold` 並傳入 `getPlayerTier`，將 `player.overallRating` 取代為動態值。這樣會根據玩家的整體評分，參考 CSS 中正確的階級類別。

# --hints--

你應該使用帶有 `getPlayerTier(player.overallRating)` 的模板字串，作為 `className` 的 `.card-wrapper`。

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

當玩家評分為 98 時，`.card-wrapper` 元素應該有 `tier-elite` 類別。
