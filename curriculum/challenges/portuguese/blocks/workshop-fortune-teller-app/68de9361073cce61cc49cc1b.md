...
Em uma palestra anterior, você aprendeu que inferência de tipo é a capacidade do TypeScript de determinar e atribuir automaticamente tipos a variáveis, retornos de funções e expressões com base no uso e contexto.

No TypeScript, generics tornam funções, classes e tipos mais flexíveis. Generics permitem a criação de variáveis de tipo, eliminando a necessidade de definir explicitamente os tipos a cada vez. Isso possibilita que a mesma função funcione com segurança com diferentes tipos.

Aqui está um exemplo simples de generics:
...
Funções podem usar dois ou mais generics. `T` é usado por convenção, e variáveis de tipo adicionais como `S` podem ser adicionadas conforme necessário.
...
Crie uma função `getElement` que receba `selector` do tipo `string` como parâmetro. Ela também deve receber um tipo genérico `T` que estenda `HTMLElement` e ter um tipo de retorno `T`.

# --hints--

Você deve ter uma arrow function chamada `getElement`.
...
Sua função `getElement` deve receber um parâmetro chamado `selector` do tipo `string`.
...
O parâmetro de tipo genérico `T` deve estender o tipo `HTMLElement`.
...
Sua função `getElement` deve ter um tipo de retorno `T`.
