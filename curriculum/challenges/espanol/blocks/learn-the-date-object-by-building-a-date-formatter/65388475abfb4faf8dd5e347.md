...
Usando `const`, crea una variable llamada `year` y asígnale el año de `date` con el método `.getFullYear()`.

# --hints--

Debe utilizar `const` para declarar una variable `year`.

```js
assert.match(code, /const\s+year\s*=/);
```

Debes asignar el `date.getFullYear()` a tu variable `year`.
