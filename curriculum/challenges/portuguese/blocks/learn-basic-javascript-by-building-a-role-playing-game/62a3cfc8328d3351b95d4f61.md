...
O exemplo acima mudaria o texto do elemento `p` de `Demo content` para `Hello World`.

Quando o jogador clicar no botão `Go to store`, você vai querer alterar os botões e o texto.Remova o código dentro da função `goStore` e adicione uma linha que atualize o texto de `button1` para dizer `"Buy 10 health (10 gold)"`.
