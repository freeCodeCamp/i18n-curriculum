...
Дайте `.bb2b` `width` і `height` `100%`, щоб він заповнив контейнер будівлі. Трохи пізніше ви додасте щось зверху.

# --hints--

Ви повинні дати `.bb2b` `width` `100%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.bb2b')?.width, '100%');
```

Ви повинні дати `.bb2b` `height` `100%`.
