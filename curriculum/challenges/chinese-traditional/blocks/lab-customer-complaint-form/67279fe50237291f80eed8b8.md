...
1. 你應該有一個名為 `validateForm` 的函式（程式），該函式（程式）傳回一個包含以下鍵的物件：`full-name`、`email`、`order-no`、`product-code`、`quantity`、`complaints-group`、`complaint-description`、`solutions-group` 和 `solution-description`。如果對應的表單欄位正確填寫，則每個鍵的值應為 `true`，否則為 `false`。條件性描述欄位在不需要時應為 `true`，否則根據是否包含至少二十個字元來驗證。
1. 你應該有一個名為 `isValid` 的函式（程式），它以 `validateForm` 傳回的物件作為引數，並且如果每個表單欄位都正確填寫則傳回 `true`，否則傳回 `false`。
1. 如果在表單欄位上觸發變更事件且其值有效，你應該將其邊框顏色設為 `green`。如果是核取方塊和圓鈕群組，你應該設定父 `fieldset` 的邊框顏色。
1. 如果在表單欄位上觸發變更事件且其值無效，你應該將其邊框顏色設為 `red`。如果是核取方塊和圓鈕群組，你應該設定父層 `fieldset` 的邊框顏色。
...
當 `#other-complaint` 未被勾選時，`validateForm()["complaint-description"]` 應該是 `true`。
...
當 `#solutions-group` 中所有的圓鈕在觸發 change 事件後都處於未選取狀態時，你應該將 `#solutions-group` 的邊框顏色設為 `red`。
...
當 `#other-solution` 未被勾選時，`validateForm()["solution-description"]` 應該是 `true`。
