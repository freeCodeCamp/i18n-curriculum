...
Wenn deine `if`-Bedingung true ist, musst du die beiden Elemente austauschen, indem du das größere Element an das Ende des Arrays "schiebst".

Deklariere dazu eine `temp`-Variable und weise ihr den Wert `array[j]` zu. Weise dann `array[j]` den Wert von `array[j + 1]` zu. Weise schließlich `array[j + 1]` den Wert von `temp` zu.

# --hints--

Du solltest innerhalb deiner `if`-Anweisung eine `temp`-Variable deklarieren.
...
Du solltest `temp` den Wert `array[j]` zuweisen.
...
Du solltest `array[j]` den Wert `array[j + 1]` zuweisen.
...
Du solltest `array[j + 1]` den Wert `temp` zuweisen.
