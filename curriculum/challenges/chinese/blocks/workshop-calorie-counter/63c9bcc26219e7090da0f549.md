...
在编程中，`null` 表示没有值。 在这种情况下，如果用户输入了无效的输入，你需要提醒他们，然后返回 `null` 以指示该功能已失败。

仍在 `if` 块内，将 `isError` 设置为 `true` 并返回 `null`。

# --hints--

在 `alert` 之后，你应该将 `isError` 设置为 `true`。
...
修改 `isError` 后，你应该 `return` 值 `null`。
