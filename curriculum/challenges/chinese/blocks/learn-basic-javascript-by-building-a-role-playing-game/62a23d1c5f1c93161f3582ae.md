...
与 `#stats` 元素类似，`#monsterStats` 元素需要两个 `span` 元素。  Give them the class `stat` and give the first element the text `Monster Name: ` and the second the text `Health: `. 在每个文本之后，添加一个 `strong` 元素和一个空的嵌套 `span` 元素。 将第一个内部 `span` 元素的 `id` 设置为 `monsterName`，将第二个内部 `span` 元素的 `id` 设置为 `monsterHealth`。
...
你的第一个 `span` 元素应包含文本 `Monster Name: `。确保间距正确。
...
你的第二个 `span` 元素应包含文本 `Health: `。确保间距正确。
