...
في نهاية دالتك، أعد البيانات التي حسبتها لكي تُستخدم خارج الدالة.

أعد كل من قائمة أقصر `distances` من `start_node` إلى جميع العقد الأخرى وقائمة `paths` التي تؤدي إلى كل عقدة.

# --hints--

يجب أن تحتوي دالتك على جملة `return` في نهايتها.
...
يجب أن تعيد كل من `distances` و`paths`.
