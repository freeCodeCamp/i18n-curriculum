...
Antes de reproducir la canción, debe asegurarse de que comience desde el principio. Esto se puede lograr mediante el uso de la propiedad `currentTime` en el objeto `audio`.

Agregue una declaración `if` para verificar si no hay ninguna canción actualmente reproduciéndose verificando que `userData.currentSong` sea falso. Dentro del bloque `if`, establezca la propiedad `currentTime` del objeto `audio` a `0`.

# --hints--

Su función `playSong` debería establecer la propiedad `currentTime` del objeto `audio` a `0` cuando no hay ninguna canción reproduciéndose.
