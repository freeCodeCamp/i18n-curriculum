...
Sasa ongeza njia nyingine iitwayo `showFortune`. Kama kigezo, pita `e` na mpe aina `Event`. Pia tengeneza njia ya `newReading` na uiachie tupu kwa sasa.

# --hints--

Unapaswa kutangaza njia iitwayo `showFortune`.
...
Njia yako ya `showFortune` inapaswa kuwa na kigezo cha `e` chenye aina ya `Event`.
...
Unapaswa kutangaza njia iitwayo `newReading`.
