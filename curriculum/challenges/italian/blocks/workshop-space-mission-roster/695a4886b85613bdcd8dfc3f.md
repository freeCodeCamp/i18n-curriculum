...
Per prima cosa, crea una copia superficiale dell'array di input `crew` usando `slice()` e assegnala a una variabile chiamata `sorted`.

# --hints--

Dovresti usare il metodo `slice()` per copiare l'array di input `crew` in una nuova variabile chiamata `sorted`.
