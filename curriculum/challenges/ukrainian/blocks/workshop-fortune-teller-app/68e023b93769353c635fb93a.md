...
Тепер, всередині шаблонного рядка функції `renderCard`, створіть елемент `div`. Всередині `div` додайте елемент `h2`, який виводить `drawingType`, і елемент `figure` з класом `card_container ${isReversed ? "reversed-card" : ""}` та атрибутом `data-id="${shortName}"`.

# --hints--

У шаблонному рядку функції `div` має бути елемент `renderCard`.
...
У елементі `h2` має бути елемент `drawingType`, який інтерполює `div`.
...
У елементі `figure` має бути елемент `div`.
...
Ваш елемент `figure` має мати клас `card_container` і використовувати тернарний оператор для умовного додавання `reversed-card`.
...
Ваш елемент `figure` має мати атрибут `data-id`, встановлений у `${shortName}`.
