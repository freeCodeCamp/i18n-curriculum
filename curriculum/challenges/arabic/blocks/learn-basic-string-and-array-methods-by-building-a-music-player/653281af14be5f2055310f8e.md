...
يجب على مشغل الموسيقى تتبع الأغاني، والأغنية الحالية التي تُشغل، ووقت الأغنية الحالية. للقيام بذلك، تحتاج إلى إنشاء كائن لتخزين هذه المعلومات.

ابدأ باستخدام الكلمة المفتاحية `let` لإعلان متغير جديد يسمى `userData` وتعيين كائن فارغ له.

# --hints--

يجب استخدام الكلمة المفتاحية `let` لإنشاء المتغير `userData`.

```js
assert.match(code, /let\s+userData\s*=/)
```

يجب تعيين كائن فارغ إلى متغير `userData`.
