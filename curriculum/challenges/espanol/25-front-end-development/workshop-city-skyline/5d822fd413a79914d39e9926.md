...
Dé a los elementos `.fb3-window` una `width` de `25%`, una `height` de `80%`, y use su variable `--window-color1` como el valor de `background-color`.

# --hints--

Debe dar a `.fb3-window` una `width` de `25%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

Debe dar a `.fb3-window` una `height` de `80%`.
