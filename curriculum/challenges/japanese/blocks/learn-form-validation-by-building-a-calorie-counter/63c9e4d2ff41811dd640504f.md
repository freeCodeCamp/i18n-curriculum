...
計算の準備を始めましょう。まず `consumedCalories` 変数を宣言し、`breakfastCalories`、`lunchCalories`、`dinnerCalories`、`snacksCalories` の合計を代入してください（テストのために順序が重要です）。これは `if` 文の後に行うことを忘れないでください。

# --hints--

関数には `consumedCalories` 変数が必要です。

```js
assert.match(calculateCalories.toString(), /consumedCalories\s*=/);
```

`consumedCalories` 変数は `if` 文の後に置いてください。

```js
assert.isAbove(calculateCalories.toString().indexOf('consumedCalories'), calculateCalories.toString().indexOf('if'));
```

`breakfastCalories`、`lunchCalories`、`dinnerCalories`、`snacksCalories` の合計を順番に計算してください。
