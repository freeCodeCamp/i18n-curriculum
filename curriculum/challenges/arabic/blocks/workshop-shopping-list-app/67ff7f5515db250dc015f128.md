...
الآن بعد أن يمكنك رؤية متى تُحدد دالة `toggleItem()` لأول مرة أو تتغير، يجب عليك أيضًا تسجيل متى تكون الدالة نفسها عبر عمليات العرض.

أضف عبارة `else` إلى عبارة `if` التي كتبتها للتو. داخل كتلة `else`، سجّل السلسلة النصية `Current toggleItem function` في وحدة التحكم.

# --hints--

يجب أن تضيف عبارة `else` إلى عبارة `if` التي كتبتها في الخطوة السابقة.

```js
assert.match(code, /if\s*\(\s*prevToggleItem\s*!==\s*toggleItem\s*\)\s*{.*}\s*else\s*{/si);
```

يجب أن تسجل السلسلة النصية `Current toggleItem function` في وحدة التحكم داخل عبارة `else`.
