...
Don't alter a variable or object - create new variables and objects and return them if need be from a function. 提示：使用类似 `const newArr = arrVar` 的东西，其中 `arrVar` 是一个数组，只会创建对现有变量的引用，而不是副本。 所以更改 `newArr` 中的值会同时更改 `arrVar` 中的值。

声明函数参数——函数内部的任何计算仅依赖于传递给函数的参数，而不依赖于任何全局对象或变量。
