...
Додайте блок `else`. Усередині блоку `else` викличте функцію `playSong` з `id` поточно відтворюваної пісні як аргумент.

Це гарантує, що поточна пісня продовжить відтворюватися, коли натиснути на кнопку відтворення.

# --hints--

Коли натискають на `playButton` і `userData.currentSong` не є `null`, слід викликати функцію `playSong` з `id` поточно відтворюваної пісні.
...
Ви все одно маєте викликати функцію `playSong` з `id` першої пісні у вашому плейлисті, коли натискають на `playButton` і `userData.currentSong` є `null`,
