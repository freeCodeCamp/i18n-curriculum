...
셔플 버튼을 눌렀을 때, `currentSong`을 빈 값으로 설정하고 `songCurrentTime`을 `0`로 설정하세요.

`userData.currentSong`을 `null`로, `userData.songCurrentTime`를 `0`으로 설정하세요.

*참고*: 이 단계에서는 `currentSong`과 `songCurrentTime` 프로퍼티를 각각 `null`와 `0`으로 명시적으로 설정하기 때문에 선택적 체이닝을 사용하지 마세요.

# --hints--

`userData.currentSong`을 `null`로 설정해야 합니다.

```js
assert.match(code, /userData\.currentSong\s*=\s*null\s*;?/)
```

`userData.songCurrentTime`을 `0`로 설정해야 합니다.
