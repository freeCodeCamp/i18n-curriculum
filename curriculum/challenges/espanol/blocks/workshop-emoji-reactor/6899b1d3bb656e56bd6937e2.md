...
Debajo del elemento `p`, crea un elemento `div` con la clase `btn-container`.

Dentro del elemento `div`, crea un elemento `button`.

Este `button` debe tener un `id` de `happy-btn`, una clase de `emoji-btn` y un `aria-label` de `Happy face emoji`.

# --hints--

Crea un nuevo elemento `div` debajo del elemento `p`.

```js
assert.exists(document.querySelector("p + div"));
```

El nuevo elemento `div` debe tener una clase `btn-container`.

```js
assert.equal(document.querySelector("p + div")?.className, "btn-container");
```

Debe haber un elemento `button` dentro del elemento `div`.

```js
assert.exists(document.querySelector("div > button"));
```

El elemento `button` debe tener un `id` de `happy-btn`.

```js
assert.equal(document.querySelector("div > button")?.id, "happy-btn");
```

El elemento `button` debe tener una `class` de `emoji-btn`.

```js
assert.equal(document.querySelector("div > button")?.className, "emoji-btn");
```

El elemento `button` debe tener un `aria-label` de `Happy face emoji`.
