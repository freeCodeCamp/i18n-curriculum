...
用 `const` 关键字和箭头语法创建一个 `isSpam` 函数。 该函数应接受单个参数 `msg`，并暂时隐式返回 `false`。

# --hints--

你应该使用 `const` 来声明一个 `isSpam` 变量。

```js
assert.match(code, /const\s+isSpam\s*=/);
```

你应该使用箭头语法将 `isSpam` 赋值为一个函数。

```js
assert.match(code, /const\s+isSpam\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

你的 `isSpam` 函数应该有一个 `msg` 参数。

```js
assert.match(code, /const\s+isSpam\s*=\s*(\(\s*msg\s*\)|msg)/);
```

你的 `isSpam` 函数应该隐式返回 `false`。
