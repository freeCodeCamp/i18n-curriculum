...
要获取上一首歌曲，请从 `userData?.songs` 的 `currentSongIndex` 中减去 `1`，并将其分配给常数 `previousSong`。 之后，调用 `playSong` 函数并传递 `previousSong.id` 作为参数。
...
你应该将 `userData?.songs[currentSongIndex - 1]` 分配给 `previousSong` 常量。
...
你应该使用 `previousSong.id` 调用 `playSong` 函数。
