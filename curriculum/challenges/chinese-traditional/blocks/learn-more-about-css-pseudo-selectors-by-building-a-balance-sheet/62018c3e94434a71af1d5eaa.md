...
創建一個選擇器以標的你的表格主體中的 `td` 元素。為它們設定寬度以填滿視窗，並設定最小和最大為 `4rem`。此方法確保寬度是固定的，而特別設定 `width` 則會允許元素縮小至容器。
