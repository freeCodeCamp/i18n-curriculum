...
Usa el método `filter()` para eliminar el objeto de canción que coincide con el parámetro `id` del array `userData?.songs`.

El método `filter` conserva solo los elementos de un array que satisfacen la función de devolución de llamada que se le pasa:
...
Usa el método `filter()` en `userData?.songs`. Pasa `song` como el parámetro de la función de flecha en la devolución de llamada y utiliza el retorno implícito para verificar si `song.id` no es estrictamente igual a `id`. Asigna todo eso a `userData.songs`.

*Nota*: No debes usar el encadenamiento opcional cuando asignas el resultado de `userData?.songs.filter` a `userData.songs` porque el array `allSongs` no estará `undefined` ni `null` en ese punto.

# --hints--

No debes modificar la función existente `deleteSong`.

```js
assert.match(code, /const\s+deleteSong\s*=\s*\(\s*id\s*\)\s*=>\s*\{\s*.*\s*\}\s*;?/)
```

Debes usar el método `filter()` en `userData?.songs`.

```js
assert.match(code, /userData\?\.songs\.filter\(/)
```

La función de devolución de llamada de tu método `filter()` debe usar sintaxis de flecha y tener un parámetro `song`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*/)
```

Tu método filter debería devolver implícitamente `song.id !== id`.

```js
assert.match(code, /userData\?\.songs\.filter\(\s*(\(\s*song\s*\)|song)\s*=>\s*(song\.id\s*!==\s*id|id\s*!==\s*song\.id)\s*\)\s*;?/)
```

Debes asignar el método `filter()` que usaste en `userData?.songs` a `userData.songs`.
