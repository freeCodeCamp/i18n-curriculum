...
1. 테스트에서 다시 할당해야 하므로 `getWeather` 함수와 `showWeather` 함수는 `const`로 선언하지 마세요.

**참고:** 테스트에서 다시 할당해야 하므로 `getWeather` 함수와 `showWeather` 함수는 `const`로 선언하지 마세요. 또한 테스트가 완료되는 데 시간이 걸리니, 콘솔에 `// running tests`가 보이는 동안은 테스트가 실행 중임을 의미합니다.
...
`select` 요소 안에 첫 번째 자식은 빈 `value` 속성을 가진 `option` 요소여야 합니다.
...
`getWeather` 함수와 `showWeather` 함수는 `const`로 선언하지 마세요.
