...
Перейдіть до елемента `footer` і зробіть так, щоб увесь текст мав значення `14px` для розміру шрифту.

# --hints--

У вас має бути селектор `footer`.
...
Ваш селектор `footer` має бути розташований нижче вашого коментаря.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Ви маєте встановити властивість `font-size` у значення `14px`.
...
Ваш елемент `footer` має мати ідентифікатор `font-size` зі значенням `14px`.
