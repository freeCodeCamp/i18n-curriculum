...
Um `circle` em SVG tem três atributos principais. Os atributos `cx` e `cy` são as coordenadas. Eles indicam ao D3 onde posicionar o *centro* da forma no SVG. O raio (atributo `r`) fornece o tamanho de `circle`.
