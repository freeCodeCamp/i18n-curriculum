...
Aggiungi l'evento `DOMContentLoaded` al documento radice e passa `initializeApp` come funzione di callback da eseguire. Questo garantisce che tutta l'inizializzazione avvenga solo dopo che il DOM è completamente caricato.

# --hints--

Dovresti aggiungere l'evento `DOMContentLoaded` al `document` radice.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

Dovresti fare riferimento a `initializeApp` come funzione da eseguire nel tuo event listener.
