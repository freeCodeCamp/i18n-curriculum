...
Debes usar el operador `&&` para agregar una segunda condición a tu declaración `if` que tambien verifique si el primer carácter de `value` es `"="`. You may use `[0]`, `.startsWith()`, or `.charAt(0)`.
