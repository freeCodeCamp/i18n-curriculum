...
Infine, restituisci il valore minimo dalla lista `prices` usando la funzione `min()`. Questo ti darà il prezzo migliore (più basso) dopo aver considerato tutti gli sconti applicabili.

# --hints--

Il tuo metodo `calculate_best_price` dovrebbe restituire il valore minimo dalla lista `prices` usando la funzione `min`.
