...
All'interno della tua clausola `else`, assegna la stringa `"general"` a `selectedCategory.className`.

Sotto quella, assegna la stringa `"General"` a `selectedCategory.category`.

Infine, assegna il numero `1` a `selectedCategory.id`.

# --hints--

Dovresti assegnare la stringa `"general"` a `selectedCategory.className`.

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

Dovresti assegnare la stringa `"General"` a `selectedCategory.category`.

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

Dovresti assegnare il numero `1` a `selectedCategory.id`.
