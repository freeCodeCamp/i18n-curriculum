...
Baada ya `.divider` yako ya mwisho, tengeneza kipengele kingine cha `p` chenye maandishi `Trans Fat 0g`. Andika neno `Trans` kwa herufi za mwinuko kwa kukifunga ndani ya kipengele cha `i`. Toa kipengele kipya cha `p` sifa ya `class` ikiwa imewekwa kuwa `indent no-divider`. Funga `Trans Fat 0g` ndani ya kipengele cha `span` kwa ulinganifu.
...
Kipengele chako kipya cha `p` kinapaswa kuwa na sifa ya kipengele ya `class` iliyowekwa kuwa `indent no-divider`.
...
Kipengele chako kipya cha `p` kinapaswa kuwa na kipengele cha `i`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelector('i'));
```

Kipengele chako cha `i` kinapaswa kufunga maandishi `Trans`.
...
Kipengele chako cha `span` kinapaswa kufunga maandishi `Trans Fat 0g`.
