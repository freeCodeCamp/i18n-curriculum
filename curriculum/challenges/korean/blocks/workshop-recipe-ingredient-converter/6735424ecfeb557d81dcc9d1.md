...
이 단계에서는 `submit`에 `recipeForm` 이벤트 리스너를 붙여 `Convert` 버튼을 클릭하면 `updateResultsList()` 함수가 호출되도록 하세요.

`submit`에 `recipeForm` 이벤트 리스너를 붙이세요. 폼이 제출되면 기본 동작을 멈추기 위해 `event.preventDefault()`를 호출한 다음 `updateResultsList()`를 호출하세요.

이렇게 하면 폼에 입력하고 `Convert`를 눌러 UI를 통해 함수를 실행할 수 있어, 다음 단계에서 변환 로직을 추가하는 동안 디버깅에 유용합니다.

# --hints--

`Convert` 버튼을 클릭하면 `updateResultsList()`이 실행되도록, 기본 폼 동작을 멈추는 `"submit"`를 호출하는 `recipeForm` 이벤트 리스너를 `event.preventDefault()`에 추가하세요.
