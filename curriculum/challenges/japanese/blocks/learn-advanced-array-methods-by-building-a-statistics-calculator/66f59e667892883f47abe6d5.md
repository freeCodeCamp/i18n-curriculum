...
`forEach` を書く別の方法があります。コールバックにブロックボディ `() => {}` を使う代わりに、式ボディ `() =>` を使うことができます。

`if...else` の文を式に変換する必要があります。式は三項で書き、三項に対して単一の代入を使ってください。

```js
assignment = condition ? exprIfTrue : exprIfFalse
```

`forEach` のコールバックを式ボディに変換し、文を三項に置き換えてください。

# --hints--

関数は `counts` 変数を正しくインクリメントする必要があります。
