...
最後に、`hideElements`の中で、引数`singleCard`、`multipleCard`、`fortuneContainer`、`fortuneDescription`を渡してください。これらは要素から取得することを忘れないでください。

これで、あなたの占いアプリは完成です！

# --hints--

`singleCard`を`hideElements`の最初の引数として渡す必要があります。
...
`multipleCard`を`hideElements`の引数として渡す必要があります。
...
`fortuneContainer`を`hideElements`の引数として渡す必要があります。
...
`fortuneDescription`を`hideElements`の引数として渡す必要があります。
