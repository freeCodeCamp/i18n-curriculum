...
`scaryStoryBtn` のイベントリスナーを修正して、引数 `displayStory` を使って `"scary"` 関数を呼び出してください。また、`click` と `funnyStoryBtn` ボタンに `adventureStoryBtn` イベントリスナーを追加し、それぞれ引数 `displayStory` と `"funny"` を使って `"adventure"` 関数を呼び出してください。

これでワークショップを無事に完了です。

# --hints--

`scaryStoryBtn` をクリックすると、`#result` の段落に怖い話 `"In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries."` が表示され、ボーダーの色は `#ee4b2b` になります。
...
`funnyStoryBtn` をクリックすると、`#result` の段落に面白い話が表示され、ボーダーの色は `#f1be32` になります。
...
`adventureStoryBtn` をクリックすると、`#result` の段落に冒険の話が表示され、ボーダーの色は `#acd157` になります。
