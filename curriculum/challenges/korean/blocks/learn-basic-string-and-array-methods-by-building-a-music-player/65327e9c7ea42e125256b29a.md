...
나머지 노래들을 `allSongs` 배열에 추가해 두었습니다.

다음으로 <dfn>웹 오디오 API</dfn>와 이를 사용해 노래를 재생하는 방법을 배웁니다. 모든 최신 브라우저는 웹 애플리케이션에서 오디오를 생성하고 처리할 수 있는 웹 오디오 API를 지원합니다.

`const`을 사용해 `audio`라는 변수를 만들고 `new Audio()`와 같게 설정하세요. 이렇게 하면 새로운 HTML5 `audio` 요소가 생성됩니다.

# --hints--

`new` 키워드를 사용해 `Audio` 객체의 인스턴스를 만들어야 합니다.

```js
assert.match(code, /new\s+Audio\(\s*\)\s*;?/)
```

`Audio` 객체를 `audio`라는 상수에 할당해야 합니다.
