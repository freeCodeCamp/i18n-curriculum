...
最後你需要做的事情是為你的程式碼添加一個 `else if` 述語。

你的條件應該檢查玩家的 `x` 位置是否大於或相等於檢查點的 `x` 位置，且小於或相等於檢查點的 `x` 位置加上 `40`。

在 `else if` 述語的主體內，你需要呼叫 `showCheckpointScreen` 函式，並將字串 `"You reached a checkpoint!"` 作為引數傳入。

恭喜你！你已完成這個平台遊戲專案！

# --hints--

你應該為你的程式碼添加一個 `else if` 語句，以檢查玩家的 `x` 位置是否大於或相等於檢查點的 `x` 位置且小於或相等於檢查點的 `x` 位置加上 `40`。
...
你應該呼叫 `showCheckpointScreen` 函式（程式），並傳入 "You reached a checkpoint!" 作為引數。
