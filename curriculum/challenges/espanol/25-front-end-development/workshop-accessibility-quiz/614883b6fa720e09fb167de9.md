...
Añadir relleno a la parte superior y izquierda de los elementos `.info`, y establecer los otros valores a `0`.

# --hints--

Deberías usar el selector `.info`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.info'));
```

Deberías dar al `.info` un `padding-top` de al menos `1px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('.info')?.paddingTop?.replace(/\D+/, '')), 1);
```

Deberías dar al `.info` un `padding-right` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingRight, '0px');
```

Deberías dar al `.info` un `padding-bottom` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingBottom, '0px');
```

Deberías dar al `.info` un `padding-left` de al menos `1px`.
