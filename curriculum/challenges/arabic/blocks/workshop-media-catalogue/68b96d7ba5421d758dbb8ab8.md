...
أنشئ بند `except` آخر يلتقط `MediaError as e`. داخل كتلة `except`، استخدم سلسلة نصية f لطباعة `Media Error:` متبوعة بمسافة ورسالة الخطأ.

# --hints--

يجب أن يكون لديك بند `except` يلتقط `MediaError as e`.
...
يجب أن تستخدم سلسلة نصية f لطباعة `Media Error:` متبوعة بمسافة ورسالة الخطأ داخل بند `except` الجديد.
