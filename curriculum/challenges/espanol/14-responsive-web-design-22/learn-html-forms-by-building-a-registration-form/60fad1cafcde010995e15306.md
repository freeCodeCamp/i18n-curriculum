...
Debes dar al elemento `input` que espera nombres un atributo `name`. _Pista: Yo hubiera elegido `first-name`_

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(1) > label:nth-child(1) > input')?.name);
```

Debes dar al elemento `input` que espera apellidos un atributo `name`. _PD: Yo hubiera elegido `last-name`_

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(1) > label:nth-child(2) > input')?.name);
```

Debes dar al `email` un atributo `name`. _PD: Yo hubiera elegido `email`_

```js
assert.isNotEmpty(document.querySelector('input[type="email"]')?.name);
```

Debes dar al `password` un atributo `name`. _PD: Yo hubiera elegido `password`_

```js
assert.isNotEmpty(document.querySelector('input[type="password"]')?.name);
```

Debes dar al `checkbox` un atributo `name`. _PD: Yo hubiera elegido `terms`_

```js
assert.isNotEmpty(document.querySelector('input[type="checkbox"]')?.name);
```

Debes dar al `file` un atributo `name`. _PD: Yo hubiera elegido `file`_

```js
assert.isNotEmpty(document.querySelector('input[type="file"]')?.name);
```

Debes dar al `number` un atributo `name`. _PD: Yo hubiera elegido `age`_
