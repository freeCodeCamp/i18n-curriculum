...
最後に、他の関数と同様に標準偏差のロジックを含めるように`calculate`関数を更新してください。

おめでとうございます！プロジェクトが完成しました。

# --hints--

`calculate`関数には、`standardDeviation`の結果を設定した`getStandardDeviation(numbers)`変数が必要です。

```js
assert.match(calculate.toString(), /standardDeviation\s*=\s*getStandardDeviation\(\s*numbers\s*\)/);
```

`textContent`要素の`#standardDeviation`を`standardDeviation`変数に更新してください。
