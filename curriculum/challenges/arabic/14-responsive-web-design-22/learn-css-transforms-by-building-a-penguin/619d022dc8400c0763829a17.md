...
يجب عليك استخدام منتقي `.face.left`.

```js
assert.match(code, /\.face\.left\s*\{/);
```

يجب عليك إعطاء عنصر `.face.left` خاصية `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face.left')?.left);
```

يجب أن يحتوي عنصر `.face.left` على خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
