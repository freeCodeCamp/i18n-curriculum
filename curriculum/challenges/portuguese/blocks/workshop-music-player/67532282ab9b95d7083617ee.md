...
Dentro da função `playNextSong`, chame as funções `setPlayerDisplay`, `highlightCurrentSong` e `setPlayButtonAccessibleText` para atualizar corretamente o player quando a playlist terminar.

# --hints--

Você deve chamar `setPlayerDisplay` dentro da declaração `else` da sua função `playNextSong`.
...
Você deve chamar `highlightCurrentSong` dentro da declaração `else` da sua função `playNextSong`.
...
Você deve chamar `setPlayButtonAccessibleText` dentro da declaração `else` da sua função `playNextSong`.
