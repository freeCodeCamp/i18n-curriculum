...
入力フィールドをクリアする関数も作成してください。

`clearInputFields` 関数を作成します。その中で、`querySelectorAll` を使ってすべての入力フィールドを取得し、`forEach()` を使ってループ処理を行います。`forEach()` の中で、各 `input` の値を空の文字列に設定してください。

# --hints--

`clearInputFields` 関数を作成してください。

```js
assert.isFunction(clearInputFields)
```

`clearInputFields` 関数は、各入力フィールドの値を空の文字列に設定する必要があります。
