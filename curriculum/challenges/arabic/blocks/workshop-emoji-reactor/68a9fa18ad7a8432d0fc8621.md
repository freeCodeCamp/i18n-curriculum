...
في الدروس السابقة تعلمت أن `querySelectorAll` تُرجع `NodeList`، وهي بنية تشبه المصفوفة، ومن الطرق التي تمتلكها `NodeList` هي `forEach()`، وتعمل بنفس طريقة `forEach` الموجودة في المصفوفات.

قم بالتكرار على `btns` باستخدام `forEach()` التي تضيف نفس مستمع الحدث لكل زر.

استخدم دالة استدعاء راجع تأخذ معلمة `btn`، وداخلها أضف مستمع حدث نقر يستدعي `updateCount(btn)`.

بهذا تكون قد أنهيت ورشة العمل.

# --hints--

يجب أن تستخدم `btns.forEach()` للتكرار على الأزرار.

```js
assert.match(code, /btns\.forEach\s*\(/);
```

يجب أن تحتوي كل الأزرار على مستمعي أحداث يستدعون `updateCount` عند النقر.
