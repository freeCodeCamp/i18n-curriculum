...
`GET` リクエストから得られたデータは、そのままでは使えません。 データを使える状態にするには、`Response` オブジェクトに `.json()` メソッドを使って、JSON データとして解析します。 ブラウザーのコンソールで `Response` オブジェクトの `Prototype` を展開すると、そこに `.json()` メソッドがあることが分かります。

では、`console.log(res)` は削除して、代わりに `res.json()` を暗黙的に返してください。
