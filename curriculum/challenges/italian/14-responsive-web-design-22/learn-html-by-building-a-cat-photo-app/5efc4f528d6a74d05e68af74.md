...
You should add a new `label` element after the checkbox element. Assicurati di avere sia i tag di apertura che di chiusura.
...
Your `label` element should have a `for` attribute with the value `loving`.  Valore atteso `--fcc-expected--`, valore trovato `--fcc-actual--`.
