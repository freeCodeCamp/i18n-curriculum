...
在上一個項目中，你使用了 `.catch()` 方法來處理錯誤。 在這裏，你將使用 `try...catch` 語句。

`try` 代碼塊旨在處理潛在的錯誤，如果發生錯誤，`catch` 代碼塊內的代碼將被執行。
