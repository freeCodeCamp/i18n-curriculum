...
`Set` コンストラクタは引数としてイテラブルも受け入れ、それからセットを作成します。
...
このワークショップを完了するには、`displayPlantsSet` を修正して、カタログ内の植物の一般名のユニークなセットを返すようにしてください。

そのために、`commonName` マップのキーとして格納されている各植物オブジェクトの `catalog` を含む配列を作成します。その配列を `Set` コンストラクタに渡し、関数からセットを返してください。

# --hints--

`displayPlantsSet` 関数は、カタログ内の各植物の `commonName` を含むセットを返す必要があります。
