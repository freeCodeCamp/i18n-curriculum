...
Fai sì che la dimensione di tutto il testo dell'elemento `footer` sia di `14px`.
...
Il selettore `footer` dovrebbe trovarsi al di sotto del commento.

```js
assert(code.match(/\/\*\s*FOOTER\s*\*\/\s*footer/i));
```

Dovresti assegnare alla proprietà `font-size` il valore `14px`.
...
L'elemento `footer` dovrebbe avere una proprietà `font-size` con il valore `14px`.
