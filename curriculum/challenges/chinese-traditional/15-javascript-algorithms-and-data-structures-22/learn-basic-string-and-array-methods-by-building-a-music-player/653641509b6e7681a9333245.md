...
你需要更新當前正在播放的歌曲以及 `playButton` 元素的外觀。

將 `song` 分配給 `userData` 對象上的 `currentSong` 屬性。

*注意*：在此步驟中，你不應使用可選鏈接運算符 `?.`，因爲此時 `userData.currentSong` 將不會是 `null` 或 `undefined`。

# --hints--

你應該將 `song` 分配給 `userData.currentSong`。
