...
Verwenden Sie Ihre `chunkCrew()`-Funktion, um Arrays der Größe `3` aus Ihrem `EVAReadySquad`-Array zu erstellen und speichern Sie diese in einer neuen Variablen namens `EVAChunks`.

# --hints--

Sie sollten `chunkCrew()` mit `EVAReadySquad` und `3` aufrufen und das Ergebnis in einer neuen Variablen namens `EVAChunks` speichern.
