...
title: Sequenze di Hofstadter figure-figure
challengeType: 1
forumTopicId: 302286
...
Le sequenze di Hofstadter figure-figure $R_n$ e $S_n$ sono definite da

$R_1 = 1\\ ;\\ S_1 = 2 \\\\R_n = R_{n-1} + S_{n-1}, \\quad n>1.$

In particolare, la sequenza $R_n$ contiene i valori
...
La sequenza $R_n$ è definita dalla relazione di ricorrenza $R_n = R_{n-1} + S_{n-1}$, mentre $S_n$ è definita come la sequenza di numeri interi positivi che non sono inclusi nella sequenza $R_n$.

# --instructions--

Crea due funzioni chiamate `ffr` e `ffs` che restituiscano rispettivamente `R(n)` o `S(n)` per ogni indice `n`. Nota che le sequenze di Hofstadter figure-figure sono indicizzate a partire da 1, con $R_1 = 1$ e $S_1 = 2$.

Non si deve assumere alcun valore massimo per `n`.

**Riferimenti**

 <p>Rosetta: <a href='https://rosettacode.org/wiki/Hofstadter_Figure-Figure_sequences' target='_blank'>Sequenze di Hofstadter figure-figure</a></p>.
