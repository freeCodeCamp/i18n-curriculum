...
你的購物車目前在網頁上不可見。為了讓它可見，請先為你的 `cartBtn` 變數添加一個監聽器，傾聽 click 事件。回呼不需要任何參數。

# --hints--

你應該為你的 `cartBtn` 變數添加一個事件監聽器。

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(/);
```

你應該監聽 `cartBtn` 變數上的 `click` 事件。

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

你應該為事件監聽器添加一個空的回呼函式（使用箭頭語法）。請記住，它不需要任何參數。
