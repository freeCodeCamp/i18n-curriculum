...
اجعل `Est. 2020` مائلاً بإنشاء محدِّد صنف `established` ومنحه خاصية `font-style` بقيمة `italic`.

# --hints--

يجب أن يكون لديك محدِّد `.established`.
...
يجب أن تضبط الخاصية `font-style` على القيمة `italic`.
...
يجب أن يضبط محدِّد `.established` الخاصية `font-style` على القيمة `italic`.
