...
Até agora, os elementos `.face` e `.chin` têm a mesma `background-color`.

Crie uma variável do CSS personalizada chamada `--penguin-face` e defina como `white`.

# --hints--

Você deve usar o seletor `:root`.

```js
assert.match(code, /:root\s*\{/);
```

Você deve dar a `:root` uma propriedade `--penguin-face`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle(':root')?.getPropertyValue('--penguin-face'));
```

Você deve dar `:root` um `--penguin-face` de `--fcc-expected--`, mas foi encontrado `--fcc-actual--`.
