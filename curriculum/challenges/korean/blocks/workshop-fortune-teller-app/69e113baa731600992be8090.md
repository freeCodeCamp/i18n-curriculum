...
`CDN_URL` 변수를 만들고 `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`로 설정하세요. 또한 `defaultImg` 변수를 값 `new URL("default.svg", import.meta.url)`로, `LOCAL_DEFAULT_IMG` 변수를 값 `defaultImg`로 만드세요.
...
`defaultImg` 변수를 `new URL("default.svg", import.meta.url)`로 설정해야 합니다.
