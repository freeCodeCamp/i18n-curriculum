...
Ricorda da JSX che le parentesi graffe ti permettono di incorporare qualsiasi espressione JavaScript direttamente dentro JSX. È così che mostri dati dinamici dalle props. Lo fai allo stesso modo in TSX.

L'intestazione della card mostra ancora valori hardcoded. Sostituisci il testo `98` dentro `card-rating` con `player.overallRating`, e sostituisci `ST` dentro `card-position` con `player.position`.

# --hints--

Dovresti sostituire il `98` hardcoded con `player.overallRating` in `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

Dovresti sostituire il `ST` hardcoded con `player.position` in `.card-position`.
