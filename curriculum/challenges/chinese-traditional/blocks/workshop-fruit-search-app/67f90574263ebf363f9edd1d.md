...
如果 `query` 不為空，你想從 API 取得符合使用者輸入的水果，但必須在使用者停止輸入一段短時間後，避免過於頻繁地進行 fetch 呼叫。

首先，在 `if` 述語之後，使用空的箭頭函式和延遲值 `700` 作為引數呼叫 `setTimeout`，並將其存放在名為 `timeoutId` 的變數中。這樣可以讓你在效果清理時取消該逾時。

# --hints--

你應該呼叫 `setTimeout` 並將返回值存放在名為 `timeoutId` 的變數中。

```js
assert.match(code, /(const|let|var)\s+timeoutId\s*=\s*setTimeout\s*\(/s);
```

`setTimeout` 應該有一個空的回呼函式和一個延遲值為 `700`。
