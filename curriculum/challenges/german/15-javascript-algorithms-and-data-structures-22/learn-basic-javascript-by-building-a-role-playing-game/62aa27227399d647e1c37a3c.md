...
Du solltest `text.innerText` auf die Zeichenfolge `"You picked "` setzen. Denke daran, dass Abstände wichtig sind.

```js
assert.match(pick.toString(), /text\.innerText\s*=\s*('|")You picked \1/);
```

Du solltest den Verkettungsoperator verwenden, um den Wert des `guess`-Parameters zu deiner `"You picked "`-String hinzuzufügen.

```js
assert.match(pick.toString(), /text\.innerText\s*=\s*('|")You picked \1\s*\+\s*guess/);
```

Du solltest den Verkettungsoperator verwenden, um die Zeichenfolge ` hinzuzufügen". Here are the random numbers:"` to your `"You picked " + guess` string.
