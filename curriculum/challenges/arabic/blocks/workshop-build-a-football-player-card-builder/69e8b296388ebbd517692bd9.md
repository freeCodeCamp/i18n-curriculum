...
أضف خاصية `value` بقيمة `player.club` إلى الـ `input` الخاص بالنادي. بعد ذلك، أضف خاصية `onChange` مع دالة سهمية تأخذ `e` كمعلمة وتستدعي `setPlayer` مع نسخ من `player` وتعيين `club` إلى `e.target.value`.

# --hints--

يجب أن يحتوي `input` الخاص بالنادي على `value` تساوي نادي اللاعب.
...
يجب أن يحتوي `input` الخاص بالنادي (Club) على معالِج `onChange` يستدعي `setPlayer` مع نشر `player` وضبط `club` على `e.target.value`.
