...
Use os métodos `data()`, `enter()` e `append()` para associar o 
`dataset` aos novos elementos de `circle` que são incluídos no SVG.

**Nota:** Os círculos não estarão visíveis porque ainda não definimos seus atributos. Faremos isso no próximo desafio.
