...
Третій об’єкт у масиві `allSongs` повинен мати властивість `id` зі значенням числа `2`.

```js
assert.equal(allSongs[2].id, 2);
```

Третій об’єкт у масиві `allSongs` повинен мати властивість `title` зі значенням рядка `"Still Learning"`.

```js
assert.equal(allSongs[2].title, "Still Learning");
```

Третій об’єкт у масиві `allSongs` повинен мати властивість `artist` зі значенням рядка `"Quincy Larson"`.

```js
assert.equal(allSongs[2].artist, "Quincy Larson");
```

Третій об’єкт у масиві `allSongs` повинен мати властивість `duration` зі значенням рядка `"3:51"`.

```js
assert.equal(allSongs[2].duration, "3:51");
```

Третій об’єкт у масиві `allSongs` повинен мати властивість `src` зі значенням рядка `"https://cdn.freecodecamp.org/curriculum/js-music-player/can't-stay-down.mp3"`.
