...
现在声明一个 `lunchCalories` 变量，并赋予它使用 `lunchNumberInputs` 调用 `getCaloriesFromInputs` 的值。

# --hints--

你的 `calculateCalories` 函数应该有一个 `lunchCalories` 变量。

```js
assert.match(calculateCalories.toString(), /lunchCalories\s*=/);
```

你应该使用 `lunchNumberInputs` 作为参数调用 `getCaloriesFromInputs`。

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*lunchNumberInputs\s*\)/);
```

你应该将 `getCaloriesFromInputs` 的结果分配给 `lunchCalories`。
