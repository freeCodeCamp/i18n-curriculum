...
請記住，`return` 結束函數的執行。 在 `if` 塊之後，你需要處理輸入有效時的邏輯。 因爲你的 `if` 語句返回一個值，所以你不需要 `else` 語句。

使用加法賦值運算符將 `currVal` 添加到你的 `calories` 總數中。 你需要使用 `Number` 構造函數將 `currVal` 轉換爲數字。

`Number` 構造函數是一個將值轉換爲數字的函數。 如果該值無法轉換，則返回 `NaN`，代表“非數字”。
...
在 `if` 語句之後，你應該對 `calories` 使用加法賦值運算符。

```js
assert.match(getCaloriesFromInputs.toString(), /if.*}\s*calories\s*\+=/s);
```

你應該將 `currVal` 傳遞給 `Number()` 構造函數。
