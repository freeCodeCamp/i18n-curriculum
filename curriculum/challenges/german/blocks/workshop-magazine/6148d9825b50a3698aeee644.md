...
Erstellen Sie einen `.lists`-Selektor und setzen Sie die `list-style-type`-Eigenschaft auf `none`. Dadurch werden die Aufzählungszeichen bei den Listenelementen entfernt.

# --hints--

Sie sollten einen `.lists`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.lists'));
```

Ihr `.lists`-Selektor sollte eine `list-style-type`-Eigenschaft mit dem Wert `none` haben.
