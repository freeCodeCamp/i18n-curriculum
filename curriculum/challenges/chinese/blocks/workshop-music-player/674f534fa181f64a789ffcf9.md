...
如果 `userData.currentSong` 不是 `null`，你需要查找下一首要播放的歌曲。记住你可以使用 `getNextSong` 来完成此操作。

完成你的 `playNextSong` 函数，使其播放播放列表中的下一首歌曲。如果当前歌曲是最后一首，将 `userData.currentSong` 设置为 `null`，`userData.songCurrentTime` 设置为零，并调用 `pauseSong` 函数。

# --hints--

当 `userData` 的 `currentSong` 为真且不是播放列表中的最后一首歌时，`playNextSong` 函数应播放当前歌曲的下一首歌。 
...
当 `userData` 的 `currentSong` 是播放列表的最后一首歌时，`playNextSong` 函数应将 `userData.currentSong` 设置为 `null`。
...
当 `userData` 的 `currentSong` 是播放列表中的最后一首歌时，`playNextSong` 函数应将 `userData.songCurrentTime` 设置为 `0`。
...
当 `userData` 的 `currentSong` 是播放列表中的最后一首歌时，`playNextSong` 函数应调用 `pauseSong` 函数。
...
当 `userData` 的 `currentSong` 等于 `null` 时，你的 `playNextSong` 函数仍应调用 `playSong` 函数，并将 `userData.songs` 数组中第一首歌曲的 `id` 作为参数传入。
