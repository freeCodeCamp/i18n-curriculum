...
`Card` インターフェースに追加するプロパティがさらにあります。まず `value_int` を `number` 型に設定してください。その他のすべてのプロパティは `string` 型にしてください：`suit`、`type`、`img`、`meaning_up`、`meaning_rev`、および `desc`。

# --hints--

`value_int` という名前のプロパティが `number` 型である必要があります。
...
`suit` という名前のプロパティが `string` 型である必要があります。
...
`type` という名前のプロパティが `string` 型である必要があります。
...
`img` という名前のプロパティが `string` 型である必要があります。
...
`meaning_up` という名前のプロパティが `string` 型である必要があります。
...
`meaning_rev` という名前のプロパティが `string` 型である必要があります。
...
`desc` という名前のプロパティが `string` 型である必要があります。
