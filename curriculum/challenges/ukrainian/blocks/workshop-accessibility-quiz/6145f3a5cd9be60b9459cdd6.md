...
Зв’яжіть перший елемент `label` з елементом `select`, а елементу `select` надайте атрибут `name`.

# --hints--

Ви маєте надати елементу `label` атрибут `for`.

```js
assert.notEmpty(document.querySelector('.question-block > label')?.htmlFor);
```

Ви маєте надати елементу `select` атрибут `id`.

```js
assert.notEmpty(document.querySelector('.answer > select')?.id);
```

Ви маєте надати елементу `select` ідентифікатор `id`, який співпадає з атрибутом `for` елемента `label`.

```js
assert.equal(document.querySelector('.answer > select')?.id, document.querySelector('.question-block > label')?.htmlFor);
```

Ви маєте надати елементу `select` атрибут `name`.
