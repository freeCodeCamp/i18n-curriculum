...
أسفل عنصر `.small-text`، أنشئ عنصر `p` جديدًا يحتوي على النص `Calories`. وأيضًا أسفل عنصر `.left-container`، أنشئ عنصر `span` جديدًا يحتوي على النص `230`.

# --hints--

يجب أن يكون لديك عنصر `p` جديد داخل عنصر `.calories-info`.

```js
assert(document.querySelector('.calories-info')?.querySelector('p'))
```

يجب أن يكون لديك عنصر `span` جديد داخل عنصر `.calories-info`.

```js
assert(document.querySelector('.calories-info')?.querySelector('span'))
```

يجب أن يأتي عنصر `p` بعد عنصر `.small-text`.

```js
assert(document.querySelector('.small-text')?.nextElementSibling?.localName === 'p');
```

يجب أن يأتي عنصر `span` بعد عنصر `.left-container`.

```js
assert(document.querySelector('.left-container')?.nextElementSibling?.localName ==='span');
```

يجب أن يحتوي عنصر `p` على النص `Calories`.
