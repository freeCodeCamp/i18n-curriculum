...
The verb `to be` in the `Simple Past` is used to describe or ask about states or characteristics in the past. The forms are `was` (singular) and `were` (plural). 例如：
...
In questions, the verb `to be` and the subject are inverted. 例如：
