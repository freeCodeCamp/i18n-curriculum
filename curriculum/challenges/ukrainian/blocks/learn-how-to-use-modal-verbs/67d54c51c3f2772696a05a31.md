---
id: 67d54c51c3f2772696a05a31
title: Завдання 34
challengeType: 19
dashedName: task-34
lang: en-US
---

<!-- (audio) Mark: Understood. I'll make sure to update the team. By the way, do we have to follow any specific guidelines for data encryption? -->

<!-- SPEAKING -->

# --instructions--

Прослухайте аудіо та дайте відповідь на запитання нижче.

# --questions--

## --text--

Існують конкретні вказівки щодо шифрування даних. Що б сказала Джессіка?

## --answers--

`Encryption is a good idea, but there are no specific rules to follow.`

### --audio-id--

ENa583ea42

### --feedback--

У відповіді має бути зазначено, що існують вказівки, а не те, що шифрування є необов’язковим.

---

`Yes, all sensitive data must be encrypted.`

### --audio-id--

EN2d8f60b9

## --video-solution--

2

# --explanation--

Правильна відповідь підтверджує, що існують конкретні вказівки щодо шифрування даних.

Фраза `must be encrypted` показує, що шифрування є обов’язковим.

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Mark",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_14-1.mp3",
      "startTime": 1,
      "startTimestamp": 34.98,
      "finishTimestamp": 41.9
    }
  },
  "commands": [
    {
      "character": "Mark",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Mark",
      "startTime": 1,
      "finishTime": 3.36,
      "dialogue": {
        "text": "Understood. I'll make sure to update the team.",
        "align": "center"
      }
    },
    {
      "character": "Mark",
      "startTime": 3.36,
      "finishTime": 7.92,
      "dialogue": {
        "text": "By the way, do we have to follow any specific guidelines for data encryption?",
        "align": "center"
      }
    },
    {
      "character": "Mark",
      "opacity": 0,
      "startTime": 8.42
    }
  ]
}
```
