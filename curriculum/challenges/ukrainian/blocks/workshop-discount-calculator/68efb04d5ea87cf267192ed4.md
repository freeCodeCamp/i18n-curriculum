...
Створіть екземпляр `DiscountEngine` і збережіть його у змінній `engine`. Після цього обчисліть найкращу ціну за допомогою методу `calculate_best_price` і збережіть її у змінній з іменем `best_price`.

# --hints--

Ви повинні створити екземпляр `DiscountEngine` з списком `strategies`.
...
Ви повинні викликати `engine.calculate_best_price(product, user_tier)` і зберегти результат у `best_price`.
