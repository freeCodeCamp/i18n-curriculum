...
このプロジェクトを完了おめでとうございます。

最後のステップとして、`print` 関数から `verify_card_number` の呼び出しを削除し、`card_number` を有効なものに戻してください。

# --hints--

`print` 関数内に `verify_card_number` の呼び出しがあってはいけません。
