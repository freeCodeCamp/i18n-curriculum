...
بهذا اكتمل balance sheet الخاصة بك!

# --hints--

يجب أن يكون لديك منتقي `tr.data td:last-of-type`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.data td:last-of-type'));
```

يجب يحتوي لمنتقي `tr.data td:last-of-type` على خاصية `padding-right` بقيمة `0.5rem`.
