...
Entre los selectores `0%` y `100%`, añade un selector `50%`. Esto se aplicará en la mitad del ciclo de animación. Establece `background-color` a `purple`.

# --hints--

Debes crear un nuevo selector `50%` en tu regla `@keyframes cabins`.
...
Tu selector `50%` debe estar entre tus selectores `0%` y `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '50%');
```

Tu selector `50%` debe tener una propiedad `background-color` establecida en `purple`.
