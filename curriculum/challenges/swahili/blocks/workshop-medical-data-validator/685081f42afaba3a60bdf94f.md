...
Ifuatayo, unataka kuthibitisha kuwa `age` ni nambari kamili. Kwa hivyo ongeza ufunguo mwingine `age` kwenye kamusi ya `constraints`. Kwa thamani yake, fanya wito wa `isinstance` ukipitisha `age` na `int` kama hoja zake.

# --hints--

Kamusi yako ya `constraints` inapaswa kuwa na ufunguo `age`.
...
Ufunguo wa `age` katika kamusi yako ya `constraints` unapaswa kuwa na thamani ya `isinstance(age, int)`.
