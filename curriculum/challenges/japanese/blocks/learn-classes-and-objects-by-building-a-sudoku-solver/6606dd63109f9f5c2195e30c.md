...
`valid_in_square()` メソッドを `row`、`col`、`num` の引数で呼び出して、指定された3x3のマスに数字を挿入できるかどうかを確認し、その結果を変数 `valid_in_square` に代入してください。

# --hints--

変数 `valid_in_square` を宣言し、`self.valid_in_square(row, col, num)` を代入してください。
