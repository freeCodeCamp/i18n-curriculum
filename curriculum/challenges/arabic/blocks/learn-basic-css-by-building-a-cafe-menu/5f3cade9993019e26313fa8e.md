...
الآن بعد أن عرفت أنها تعمل، يمكنك تغيير عناصر `article` و `p` المتبقية لتطابق المجموعة الأولى. ابدأ بإضافة الصنف `item` إلى عناصر `article` الأخرى.
...
يجب أن يكون لديك خمسة عناصر `.item` فقط.

```js
assert.lengthOf(document.querySelectorAll('.item'),5);
```

عناصر `.item` الخاصة بك يجب أن تكون عناصر `article`.
