...
Du solltest ein `img`-Element mit einem `src` von `https://cdn.freecodecamp.org/curriculum/cat-photo-app/relaxing-cat.jpg` haben. You may have accidentally deleted it.
...
Your anchor (`a`) element should have an opening tag. Öffnende Tags haben diese Syntax: `<elementName>`.
...
Your anchor (`a`) element should have a closing tag. Schließende Tags haben ein `/` direkt nach dem `<`-Zeichen.
