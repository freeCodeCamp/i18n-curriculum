...
将内耳移动到位，其 `position` 属性设置为 `absolute`，`top` 为 `22px`，`left` 为 `-20px`。

# --hints--

你的 `.cat-left-inner-ear` 选择器应该有一个 `position` 属性，设置为 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.position, 'absolute')
```

你的 `.cat-left-inner-ear` 选择器应该具有一个 `top` 属性，设置为 `22px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.top, '22px')
```

你的 `.cat-left-inner-ear` 选择器应该有一个 `left` 属性，设置为 `-20px`。
