...
Crie um elemento `div` abaixo do elemento `.large` e dê a ele um atributo `class` definido como `calories-info`.

# --hints--

Você deve criar outro elemento `div`.

```js
assert(document.querySelectorAll('div')?.length === 4);
```

A nova `div` deve ter o atributo `class` definido como `calories-info`.

```js
assert(document.querySelector('.label')?.lastElementChild?.classList?.contains('calories-info'));
```

A nova `div` deve estar depois do elemento `.large`.
