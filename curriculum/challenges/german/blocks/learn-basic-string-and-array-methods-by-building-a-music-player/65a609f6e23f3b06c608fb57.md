...
Die `sort()`-Methode akzeptiert eine Callback-Funktion zum Vergleichen, die die Sortierreihenfolge definiert.

In diesem Beispiel prüft die erste Bedingung `(a.name < b.name)`, ob der Name der ersten Frucht lexikografisch kleiner ist als der Name der zweiten Frucht. Falls ja, wird die erste Frucht vor der zweiten Frucht sortiert.

Strings werden lexikografisch verglichen, das heißt, sie werden Zeichen für Zeichen verglichen. Zum Beispiel ist `"Apples"` kleiner als `"Bananas"`, weil `"A"` im Alphabet vor `"B"` kommt.

Der Grund, warum dieses Beispiel Zahlen zurückgibt, ist, dass die `sort()`-Methode erwartet, dass eine Zahl zurückgegeben wird. Wenn Sie eine negative Zahl zurückgeben, wird das erste Element vor dem zweiten Element sortiert.
...
Fügen Sie in Ihrer Callback-Funktion eine `if`-Anweisung hinzu, um zu prüfen, ob `a.title` kleiner ist als `b.title`. Falls ja, geben Sie `-1` zurück.

# --hints--

Sie sollten eine `if`-Anweisung innerhalb Ihrer Callback-Funktion haben.

```js
assert.match(code, /if\s*\(/);
```

Ihre `if`-Anweisung sollte prüfen, ob `a.title` kleiner ist als `b.title`.

```js
assert.match(code, /if\s*\(\s*a\.title\s*<\s*b\.title\s*\)/);
```

Ihre `if`-Anweisung sollte `-1` zurückgeben, wenn `a.title` kleiner ist als `b.title`.
