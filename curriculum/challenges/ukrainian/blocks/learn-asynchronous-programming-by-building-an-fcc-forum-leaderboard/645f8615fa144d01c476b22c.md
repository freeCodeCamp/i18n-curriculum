...
Нарешті, вам потрібно повернути зображення для аватара користувача.

Почніть із додавання `return`, а потім множини шаблонних літералів. Всередині шаблонних літералів додайте елемент `img`.

Всередині тега `img` додайте атрибут `src` зі значенням `${userAvatarUrl}`. Для атрибута `alt` додайте значення `${user.name}`.
...
Ви повинні повернути шаблонний літерал.
...
Усередині шаблонного літерала має бути елемент `img`.
...
Усередині тега `src` має бути атрибут `img`.
...
Ви повинні встановити значення атрибута `src` у константу з ім’ям `userAvatarUrl`.
...
Після атрибута `alt` має бути атрибут `src`.
...
Ви повинні встановити значення атрибута `alt` у константу з ім’ям `user.name`.
