...
在 `oninput` 上使用 `shapeTypeSelect` 事件，并将其设置为 `handleShapeSelect`。这样每次形状选择更改时，函数都会运行。

完成后，你应该能看到每个形状的输入字段。但计算还不能正常工作，接下来你将处理这个问题。

# --hints--

你应该将 `shapeTypeSelect.oninput` 设置为 `handleShapeSelect`。
