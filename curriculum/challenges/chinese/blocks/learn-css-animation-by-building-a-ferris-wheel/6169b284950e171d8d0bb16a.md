...
最后，在 `50%` 和 `100%` 选择器之间创建一个新的 `75%` 选择器。 给这个新的选择器添加一个 `background-color` 属性，设置为 `yellow`。

有了这个，你的动画就更顺畅了，你的摩天轮就完成了。

# --hints--

你应该在 `@keyframes cabins` 规则中创建一个新的 `75%` 选择器。
