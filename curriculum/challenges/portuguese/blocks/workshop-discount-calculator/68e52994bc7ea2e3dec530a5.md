...
Finalmente, retorne o valor mínimo da lista `prices` usando a função `min()`. Isso lhe dará o melhor preço (mais baixo) após considerar todos os descontos aplicáveis.

# --hints--

Seu método `calculate_best_price` deve retornar o valor mínimo da lista `prices` usando a função `min`.
