...
`else` 関数に `buyWeapon` 文を追加してください。その文の中で、`text.innerText` を `"You do not have enough gold to buy a weapon."` と等しく設定してください。

# --hints--

`else` 関数に `buyWeapon` 文を追加する必要があります。

```js
assert.match(buyWeapon.toString(), /else/);
```

`else` 文の後に `if` 文を置いてください。
...
`else` 文は `text.innerText` を `"You do not have enough gold to buy a weapon."` に設定する必要があります。
