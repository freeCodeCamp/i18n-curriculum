...
Deklariere eine `surplusOrDeficit`-Variable. Then use a ternary operator to set `surplusOrDeficit` to the string `"Surplus"` or `"Deficit"` depending on whether `remainingCalories` is less than `0`. If it is less than `0`, then `surplusOrDeficit` should be `"Surplus"`. Andernfalls sollte er `"Deficit"` sein.

# --hints--

Deine `calculateCalories`-Funktion sollte eine `surplusOrDeficit`-Variable enthalten.
...
Du solltest das Ergebnis deines ternären Operators `surplusOrDeficit` zuweisen.
