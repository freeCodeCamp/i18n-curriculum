...
クラスセレクターを使用して `.cat-whisker-right-middle` の要素を選択し、`position` を `absolute`、`top` を `127px`、`left` を `109px` に設定して、右の真ん中のひげを適切な位置に動かしましょう。

# --hints--

`.cat-whisker-right-middle` セレクターが必要です。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle'));
```

`.cat-whisker-right-middle` セレクターの `position` プロパティを `absolute` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.position, 'absolute');
```

`.cat-whisker-right-middle` セレクターの `top` プロパティを `127px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.top, '127px');
```

`.cat-whisker-right-middle` セレクターの `left` プロパティを `109px` に設定してください。
