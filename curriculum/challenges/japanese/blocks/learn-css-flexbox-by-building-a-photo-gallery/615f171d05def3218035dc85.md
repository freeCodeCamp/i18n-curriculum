...
body 要素のマージンを削除し、`font-family` を `sans-serif` に設定し、`background-color` に `#f5f6f7` を値として設定してください。
...
`body` セレクターには、値が 0 の `margin` プロパティが設定されているはずです。
...
`body` セレクターには、値が `font-family` の `sans-serif` プロパティが設定されているはずです。

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily === 'sans-serif');
```

`body` セレクターには、値が `background-color` の `#f5f6f7` プロパティが設定されているはずです。
