...
Adicione um listener de evento `"click"` ao elemento `previousButton`, depois passe `playPreviousSong` como o segundo argumento.

# --hints--

Você deve chamar o método `addEventListener()` na sua variável `previousButton`.
...
Você deve passar `playPreviousSong` como o segundo argumento do seu método `addEventListener`.
