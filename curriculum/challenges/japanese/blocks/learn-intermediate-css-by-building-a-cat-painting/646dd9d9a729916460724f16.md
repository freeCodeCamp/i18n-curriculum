...
右の内耳を、`position` プロパティを `absolute` に、`top` を `22px` に、`left` を `-20px` に設定して位置調整してください。

# --hints--

`.cat-right-inner-ear` セレクターには `position` プロパティを `absolute` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.position === 'absolute')
```

`.cat-right-inner-ear` セレクターには `top` プロパティを `22px` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.top === '22px')
```

`.cat-right-inner-ear` セレクターには `left` プロパティを `-20px` に設定してください。
