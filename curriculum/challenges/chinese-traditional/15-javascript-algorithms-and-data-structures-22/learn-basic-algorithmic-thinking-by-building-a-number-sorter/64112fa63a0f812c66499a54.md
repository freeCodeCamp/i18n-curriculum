...
`Sort` 按鈕點擊時可能看起來工作正常，但這只是因爲數組中的所有值都是個位數，而排序在處理更復雜值時可能不如預期。

將 `selected` 的 `option` 元素的 `value` 和文本從 `1` 更改爲 `10`，然後再次點擊 `Sort` 按鈕。

# --hints--

你應該將 `selected` 的 `option` 元素的 `value` 更新爲 `10`。
...
你應該將 `selected` 的 `option` 元素的文本更新爲 `10`。
