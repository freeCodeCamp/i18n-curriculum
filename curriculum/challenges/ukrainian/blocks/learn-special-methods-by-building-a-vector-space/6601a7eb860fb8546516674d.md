...
Створіть інструкцію `if`, яка перевіряє, чи `self` і `other` не належать до одного класу, і повертає `NotImplemented`.

# --hints--

Ви маєте створити інструкцію `if`, яка перевіряє, чи `self` і `other` не належать до одного класу.
...
Ви маєте повернути `NotImplemented` з тіла `if`.
