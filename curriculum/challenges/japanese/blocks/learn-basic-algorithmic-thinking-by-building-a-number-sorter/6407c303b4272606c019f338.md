...
ソートされた数字で表示を更新する関数が必要です。まず、矢印構文を使って、単一の`updateUI`パラメータを受け取る`array`関数を宣言してください。

すぐに戻り値が得られないアルゴリズムを書くことになるので、`array`のフォールバック値を空の配列に設定してください。以下はフォールバック値を持つ関数の例です。
...
`const`を使って`updateUI`変数を宣言してください。
...
`updateUI`は矢印構文を使うべきです。

```js
assert.match(code, /const\s+updateUI\s*=\s*(\([^)]*\)|[^\s()]+)\s*=>/);
```

`updateUI`は単一の`array`パラメータを受け取るべきです。

```js
assert.match(code, /const\s+updateUI\s*=\s*\(?\s*array/);
```

`updateUI`は`array`の空の配列をフォールバック値として持つべきです。
