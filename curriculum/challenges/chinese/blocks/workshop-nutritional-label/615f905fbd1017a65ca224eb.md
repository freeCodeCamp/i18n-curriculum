...
在 `.daily-value` 元素之后添加一个中等分隔线。 在新分隔线下方，创建一个 `p` 元素，`class` 属性设置为 `note`。
...
你的新的 `div` 应该将 `class` 属性设置为 `divider medium`。
