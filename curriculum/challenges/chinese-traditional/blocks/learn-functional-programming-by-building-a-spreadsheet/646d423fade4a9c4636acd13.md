...
首先，你需要處理優先級較高的運算符。 聲明一個 `noHigh` 變量，並將使用 `str` 作爲參數調用 `highPrecedence()` 的結果分配給它。
