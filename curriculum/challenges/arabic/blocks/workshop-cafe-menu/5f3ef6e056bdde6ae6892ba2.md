...
من الممل أن يكون كل النص بنفس `font-family`. يمكنك أن تجعل غالبية النص بـ `sans-serif` وتجعل فقط عناصر `h1` و `h2` مختلفة باستخدام محدِّد مختلف.

نسق كل من عنصرَي `h1` و `h2` باستخدام محدِّد واحد بحيث يستخدم نص هذين العنصرين خط `Impact`.

# --hints--

يجب أن تستخدم محدِّد `h1, h2`.
...
يجب أن تضبط `font-family` على `Impact`.
...
يجب أن يكون لعنصر `h1` خاصية `font-family` بقيمة `Impact`.
...
يجب أن يكون لعنصر `h2` خاصية `font-family` بقيمة `Impact`.
