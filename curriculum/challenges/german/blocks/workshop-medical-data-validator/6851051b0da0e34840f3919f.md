...
`age` sollte nicht nur eine ganze Zahl sein, sondern eine positive ganze Zahl, die größer oder gleich `18` ist.

Verwenden Sie den `and`-Operator, um einen zweiten Ausdruck zum Wert des `age`-Schlüssels hinzuzufügen, um das zu überprüfen.

# --hints--

Der `age`-Schlüssel Ihres `constraints`-Dictionaries sollte den Wert `isinstance(age, int) and age >= 18` haben.
