...
Em seguida, você precisa exibir o título da música atual e o artista no display do player.Use `const` e a sintaxe de arrow para criar uma função vazia `setPlayerDisplay`.

# --hints--

Você deve usar `const` para criar uma função vazia chamada `setPlayerDisplay`.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*/)
```

`setPlayerDisplay` deve ser uma função.

```js
assert.isFunction(setPlayerDisplay)
```

Sua função `setPlayerDisplay` deve usar uma sintaxe de arrow.

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*/)
```

Sua função `setPlayerDisplay` deve estar vazia.
