...
副標題也需要設定樣式。創建一個 `.hero-subtitle` 選擇器，並為它設定 `font-size` 屬性為 `2.4rem`、`color` 屬性為 `orangered`，以及 `text-align` 屬性為 `center`。

# --hints--

你應該有一個 `.hero-subtitle` 選擇器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

你的 `.hero-subtitle` 選擇器應該有一個 `font-size` 屬性設定為 `2.4rem`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

你的 `.hero-subtitle` 選擇器應該有一個設定為 `orangered` 的 `color` 屬性。

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

你的 `.hero-subtitle` 選擇器應該有一個 `text-align` 屬性設定為 `center`。
