...
title: Imposta il font-weight per più elementi di intestazione
challengeType: 0
...
Hai impostato il `font-size` di ogni tag di intestazione nella sfida precedente, qui regolerai il `font-weight`.

La proprietà `font-weight` determina quanto spessi o sottili sono i caratteri in una sezione di testo.
...
Il tuo codice dovrebbe impostare la proprietà `font-weight` per il tag `h1` a 800.
...
Il tuo codice dovrebbe impostare la proprietà `font-weight` per il tag `h4` a 400.
...
Il tuo codice dovrebbe impostare la proprietà `font-weight` per il tag `h6` a 200.
