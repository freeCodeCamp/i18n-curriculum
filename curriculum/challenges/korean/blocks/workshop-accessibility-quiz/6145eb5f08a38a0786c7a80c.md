...
네 번째 `input`에 세 번째 `name`의 `name` 속성과 일치하는 `input` 속성을 지정하세요.
...
첫 번째 `name`과 세 번째 `input`에 서로 다른 `input` 속성을 지정하세요.
