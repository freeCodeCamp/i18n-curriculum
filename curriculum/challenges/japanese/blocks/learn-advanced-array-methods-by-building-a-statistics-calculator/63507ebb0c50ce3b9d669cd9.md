...
配列のメソッドは、複数の操作を一度に行うためにチェーンでつなげることがよくあります。例を示します：

```js
array.map().filter();
```

配列に対して `.map()` メソッドが呼び出され、次にその `.map()` メソッドの結果に対して `.filter()` メソッドが呼び出されます。これをメソッドチェーンと呼びます。

この例に従って、`filtered` 変数を削除し、`.filter()` の呼び出しを上の `.map()` の呼び出しにチェーンしてください。どちらのコールバック関数も削除しないでください。

# --hints--

`filtered` 変数を削除してください。

```js
assert.notMatch(calculate.toString(), /filtered/);
```

`.filter()` の呼び出しを `.map()` の呼び出しにチェーンしてください。

```js
assert.match(calculate.toString(), /array\.map\(.*\)\.filter\(/s);
```

`.map()` のコールバックは削除しないでください。

```js
assert.match(calculate.toString(), /(array\.map\(\s*(\(\s*el\s*\)|el)\s*=>|array\.map\(\s*function\s*\(\s*el\s*\)\s*\{)\s*(return\s+)?Number\(\s*el\s*\)/);
```

`.filter()` のコールバックは削除しないでください。
