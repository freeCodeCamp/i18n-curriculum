...
La tua funzione è ora completa. Provala di nuovo registrando `genParentheses(2)` invece di `genParentheses(1)`.

# --hints--

Dovresti registrare `genParentheses(2)`.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*genParentheses\s*\(\s*2\s*\)\s*\)/);
```

Non dovresti avere `console.log(genParentheses(1))` nel tuo codice.
