...
Luego, dale al `textarea` un texto de marcador de posición que describa un ejemplo de respuesta.

# --hints--

Debes anidar un elemento `textarea` dentro del segundo elemento `div.answer`.

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

Debes dar al `textarea` un atributo `rows` con un número.
...
Debes dar al `textarea` un atributo `cols` con un número.
...
Debes dar al `textarea` un atributo `placeholder` con un texto que describa un ejemplo de respuesta.
