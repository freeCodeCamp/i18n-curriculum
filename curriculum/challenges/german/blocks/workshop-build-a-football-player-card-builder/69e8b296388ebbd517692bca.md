...
`FootballPlayerCard` rendert aktuell die Karte mit einem festen `defaultPlayer`. Um die App interaktiv zu machen, müssen Sie die Spieldaten im React-State verwalten, damit Aktualisierungen ein erneutes Rendern auslösen können.

Erstellen Sie innerhalb der `FootballPlayerCard`-Komponente eine State-Variable namens `player` mit einem Setter `setPlayer`, die die aktuellen Spieldaten hält. Initialisieren Sie diesen State mit `defaultPlayer` und geben Sie an, dass der State den `PlayerData`-Typ als Generikum verwendet, damit TypeScript weiß, welche Form das Spielerobjekt haben soll.

Aktualisieren Sie danach die Rückgabe, um die `player`-State-Variable als Prop an `PlayerCard` zu übergeben.

# --hints--

Sie sollten Array-Destrukturierung verwenden, um eine `player`-State-Variable und einen `setPlayer`-Setter zu setzen.

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

Ihr `player` und `setPlayer` sollten den `useState`-Hook verwenden.
...
Ihr `useState` sollte mit `defaultPlayer` initialisiert sein und `PlayerData` als Generikumstyp haben.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

Sie sollten `player` als Wert der `player`-Prop haben, die an `PlayerCard` übergeben wird.
