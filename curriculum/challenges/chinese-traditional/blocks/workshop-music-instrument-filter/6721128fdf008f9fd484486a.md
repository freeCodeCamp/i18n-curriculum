...
為你的 `instrumentsArr` 添加另外兩個物件，以表述剩餘的木管樂器，單簧管和雙簧管。

# --hints--

你應該在你的 `instrumentsArr` 中有 `{ category: "woodwinds", instrument: "Clarinet", price: 200 }`。

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

你應該在你的 `instrumentsArr` 中有 `{ category: "woodwinds", instrument: "Oboe", price: 4000 }`。

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

你應該仍然在你的 `instrumentsArr` 中有 `{ category: "woodwinds", instrument: "Flute", price: 500 }`。
