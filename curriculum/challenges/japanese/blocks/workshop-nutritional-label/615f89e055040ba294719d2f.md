...
大きな区切りの下にもう1つ`p`要素を作成してください。`p`要素にはテキスト`Vitamin D 2mcg 10%`を入れてください。

`p`要素はテキストのみを含みます。パーセンテージを`span`要素で囲むと、テキストの他の部分とは別の要素として扱われ、右側に移動します。
...
新しい`p`要素にはテキスト`Vitamin D 2mcg 10%`を入れてください。
