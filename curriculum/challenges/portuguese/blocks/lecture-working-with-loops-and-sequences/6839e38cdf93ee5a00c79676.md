...
Se você quiser remover um elemento de uma lista, pode usar o método `remove()`. Aqui está um exemplo de uso do método `remove()` para remover um número duplicado de uma lista `numbers`:
...
O próximo método que vamos analisar é o método `sort()`. Este método é usado para ordenar os elementos no local. Aqui está um exemplo de como ordenar uma lista aleatória de números no local:
...
O próximo método que vamos analisar é o método `reverse()`. Este método irá inverter uma lista de elementos no local assim:
...
Qual dos seguintes **NÃO** é um método comumente usado para listas?
