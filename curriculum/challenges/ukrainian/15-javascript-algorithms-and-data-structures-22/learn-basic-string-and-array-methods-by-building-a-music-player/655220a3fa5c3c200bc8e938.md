...
Використайте `classList` та метод `remove()`, щоб видалити клас `.playing` з `playButton`, оскільки пісню буде призупинено на цьому місці.

Щоб нарешті призупинити пісню, скористайтеся методом `pause()` на змінній `audio`. `pause()` — це метод Web Audio API для призупинення музичних файлів.

# --hints--

Не змінюйте наявну функцію `pauseSong` та її вміст.

```js
assert.match(code, /const\s+pauseSong\s*=\s*\(\s*\)\s*=>\s*\{\s*userData\.songCurrentTime\s*=\s*audio\.currentTime\s*;?\s*.*\s*.*\s*\}\s*;?/)
```

Використайте властивість `classList` та метод `remove()`, щоб видалити клас `"playing"` з `playButton`.

```js
assert.match(code, /playButton\.classList\.remove\(\s*('|")playing\1\s*\)\s*;?/)
```

Використайте метод `pause()` на змінній `audio`.
