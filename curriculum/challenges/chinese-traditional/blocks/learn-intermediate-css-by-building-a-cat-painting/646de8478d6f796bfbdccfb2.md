...
爲了使右眼看起來像一隻眼睛，給它設置一個 `60%` 的邊框半徑。 同時，使用 `transform` 屬性，將其旋轉 `-25` 度。

# --hints--

你的 `.cat-right-eye` 選擇器應將 `border-radius` 屬性爲 `60%`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.borderRadius === '60%')
```

你的 `.cat-right-eye` 選擇器應將 `transform` 屬性爲 `rotate(-25deg)`。
