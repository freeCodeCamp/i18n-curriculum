...
Beachten Sie, dass sich das Albumcover im HTML und die Songs im `userData.songs`-Array geändert haben. Wir haben die ursprünglichen Songs gegen kürzere ausgetauscht, die Sie zum Testen Ihrer App verwenden können.

Alle Kernfunktionen sind jetzt vorhanden. Das einzige Problem ist, dass der nächste Song nicht automatisch abgespielt wird, wenn der aktuell abgespielte Song endet.

Um das zu beheben, können Sie einen Ereignis-Listener einrichten, der erkennt, wann der aktuell abgespielte Song endet. Der `"ended"`-Ereignis-Listener ist dafür geeignet. Er wird ausgelöst, wenn die Wiedergabe eines Mediums zu Ende ist.

Fügen Sie dem `audio`-Element einen Ereignis-Listener hinzu, der auf das `"ended"`-Ereignis hört, und übergeben Sie eine Referenz auf die `playNextSong`-Funktion. Damit ist das Musikplayer-Projekt abgeschlossen!

# --hints--

Sie sollten die `addEventListener()`-Methode an Ihre `audio`-Variable anhängen.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(/)
```

Der Ereignis-Listener, den Sie an Ihrer `audio`-Variable verwendet haben, sollte auf ein `"ended"`-Ereignis hören.

```js
assert.match(__helpers.removeJSComments(code), /audio\.addEventListener\(\s*('|")ended\1/)
```

Sie sollten Ihrem `playNextSong`-Ereignis-Listener eine Referenz auf `"ended"` übergeben.
