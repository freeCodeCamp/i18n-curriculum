...
Застосунок ворожки підтримує відтворення кількох карток, тому вам потрібен метод, який допоможе відтворити кілька карток. Оголосіть метод з назвою `multipleCardSelected`.

Усередині методу викличте функцію `hideElements` і передайте їй пропси `singleCard`, `singleCardBtn`, `multipleCardsBtn` та `headerTitle`. Не забудьте, що потрібно отримати до них доступ через `this.elements`.

# --hints--

Ви маєте оголосити метод `multipleCardSelected`.
...
Ви маєте викликати функцію `hideElements` всередині методу `multipleCardSelected`.
...
Ви маєте передати `singleCard` як перший аргумент функції `hideElements`.
...
Ви маєте передати `singleCardBtn` як аргумент функції `hideElements`.
...
Ви маєте передати `multipleCardsBtn` як аргумент функції `hideElements`.
...
Ви маєте передати `headerTitle` як аргумент функції `hideElements`.
