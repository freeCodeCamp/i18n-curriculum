...
في ملف JavaScript الخاص بك، اختر العنصر `.story-container` واحفظه في متغير اسمه `storyContainer`.

# --hints--

يجب أن يكون لديك متغير `storyContainer`.

```js
assert.isDefined(storyContainer);
```

اختر العنصر `.story-container` باستخدام طريقة `querySelector` واحفظه في المتغير `storyContainer`.
