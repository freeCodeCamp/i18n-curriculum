...
使用从 `0%` 到 `6%` 的 `--building-color2` 和从 `6%` 到 `9%` 的 `--window-color2` 给 `.bb2b` 添加一个 `linear-gradient`。
...
`.bb2b` 应该有一个 `linear-gradient`，从 `0%` 到 `6%` 的 `--building-color2` 过渡到 `6%` 到 `9%` 的 `--window-color2`。
