...
تذكر أنه يمكنك استخدام التفكيك لاستخراج قيم متعددة من مصفوفة أو كائن في بيان واحد.

بالنسبة للمعلمة الأولى في دالة الاستدعاء الراجع، فكك الخاصيات `name` و `id` و `price` و `category` من مثيل `Dessert` الممرر.

# --hints--

يجب أن تستخدم التفكيك لإعلان المعلمات `name` و `id` و `price` و `category`. في هذا الاختبار، ترتيب المعلمات مهم.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

يجب أن يكون التفكيك هو المعلمة الأولى في دالة الاستدعاء الراجع.
