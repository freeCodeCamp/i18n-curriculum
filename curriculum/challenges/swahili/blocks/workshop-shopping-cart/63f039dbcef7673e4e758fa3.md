...
Vivinjari vina kitendakazi kilichojengwa ndani cha `confirm()` kinachoonyesha onyo la uthibitisho kwa mtumizi. `confirm()` hupokea mfuatano wa herufi, ambao ni ujumbe unaoonyeshwa kwa mtumizi. Hurejesha `true` ikiwa mtumizi anathibitisha, na `false` ikiwa mtumizi anaghairi.

Tangaza kigezo `isCartCleared` na weka thamani ya wito wa `confirm()` ukiwa na mfuatano wa herufi `"Are you sure you want to clear all items from your shopping cart?"`.

# --hints--

Unapaswa kutumia `const` kutangaza kigezo cha `isCartCleared`.
...
Unapaswa kuita kitendakazi cha `confirm()`.

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(/);
```

Unapaswa kupitisha mfuatano wa herufi `Are you sure you want to clear all items from your shopping cart?` kwa kitendakazi cha `confirm()`.

```js
assert.match(cart.clearCart.toString(), /confirm\s*\(\s*('|"|`)Are you sure you want to clear all items from your shopping cart\?\1\s*\)/);
```

Unapaswa kuweka thamani ya kitendakazi cha `confirm()` kwenye kigezo cha `isCartCleared`.
