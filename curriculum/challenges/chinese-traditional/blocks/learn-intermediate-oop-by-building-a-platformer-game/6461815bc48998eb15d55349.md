...
在這個專案中，你將透過建造一個平台遊戲來學習中階物件導向程式設計原則。所有的 `HTML` 和 `CSS` 都已經為你提供。 

首先使用 `document.getElementById()` 取得 `#start-btn` 和 `#canvas` 元素。 

將它們分別儲存在名為 `startBtn` 和 `canvas` 的 `const` 變數中。
