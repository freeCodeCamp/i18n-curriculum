...
O motivo pelo qual você pode querer usar um tipo de aspas no lugar da outra é se você vir a querer usar ambas em uma string.Isso pode acontecer se você quiser salvar uma conversa em uma string e ter a conversa entre aspas.Outro uso para isso seria salvar uma tag `<a>` com vários atributos entre aspas, tudo dentro de uma string.
...
**Nota:** A barra invertida `\` não deve ser confundida com a barra normal `/`.Elas não fazem a mesma coisa.

# --instructions--

Altere a string fornecida para uma string com aspas simples no início e no final e sem caracteres de escapamento.

No momento, a tag `<a>` na string usa aspas duplas em todos os lugares.Você precisará alterar as aspas mais externas para aspas simples, para que você possa remover os caracteres de escapamento.
