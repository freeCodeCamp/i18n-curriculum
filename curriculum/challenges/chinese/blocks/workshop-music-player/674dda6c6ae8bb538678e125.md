...
为你的代码添加一个 `if` 语句，通过验证 `userData.currentSong` 是否为假值来查看是否没有当前播放的歌曲。在 `if` 块内，将 `audio` 对象的 `currentTime` 属性设置为 `0`。

# --hints--

当没有当前播放的歌曲时，你的 `playSong` 函数应将 `audio` 对象的 `currentTime` 属性设置为 `0`。
