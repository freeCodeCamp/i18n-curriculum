...
Після того, як `rangeRegex` знайде `:`, він має шукати той самий шаблон літер та цифр, що він робив й раніше.

Скопіюйте свої дві теперішні групи захоплення та вставте їх після двокрапки.

# --hints--

Додайте третю групу захоплення до `rangeRegex` після двокрапки.
...
Третя група захоплення має використовувати символьний клас.

```js
assert.match(code, /const\s+evalFormula\s*=\s*\(\s*x\s*,\s*cells\s*\)\s*=>\s*{\s*const\s+idToText\s*=\s*(\(\s*id\s*\)|id)\s*=>\s*cells\.find\(\s*(\(\s*cell\s*\)|cell)\s*=>\s*(?:cell\.id\s*===\s*id|id\s*===\s*cell\.id)\s*\)\.value\s*;?\s*const\s+rangeRegex\s*=\s*\/\(\[A-J\]\)\(\[1-9\]\[0-9\]\?\):\(\[.*\]\)/);
```

Ваша третя група захоплення має відповідати символам від `A` до `J`.
...
Додайте четверту групу захоплення до `rangeRegex`.
