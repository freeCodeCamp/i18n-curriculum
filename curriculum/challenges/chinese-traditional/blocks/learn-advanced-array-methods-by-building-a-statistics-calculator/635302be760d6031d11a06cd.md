...
最後更新 `calculate` 函式（程式）以包含標準差邏輯，就像你對其他函式（程式）所做的那樣。

恭喜！你的專案已完成。

# --hints--

你的 `calculate` 函式（程式）應該有一個 `standardDeviation` 變數，並將 `getStandardDeviation(numbers)` 的結果設定給它。

```js
assert.match(calculate.toString(), /standardDeviation\s*=\s*getStandardDeviation\(\s*numbers\s*\)/);
```

你應該將 `#standardDeviation` 元素的 `textContent` 更新為 `standardDeviation` 變數。
