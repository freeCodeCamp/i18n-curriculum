...
Dai al tuo `conversionTable` una proprietà `gram` e assegnale un oggetto con una proprietà `cup` impostata su `1/240`, una proprietà `ounce` impostata su `0.0353` e una proprietà `teaspoon` impostata su `0.2`.

# --hints--

Il tuo `conversionTable` dovrebbe avere una proprietà `gram`.

```js
assert.property(conversionTable, "gram");
```

La proprietà `gram` dovrebbe essere un oggetto.

```js
assert.isObject(conversionTable.gram);
```

Il tuo oggetto `gram` dovrebbe avere una proprietà `cup`.

```js
assert.property(conversionTable.gram, "cup");
```

La proprietà `cup` dovrebbe essere impostata su `1/240`.

```js
assert.strictEqual(conversionTable.gram?.cup, 1/240);
```

Il tuo oggetto `gram` dovrebbe avere una proprietà `ounce`.

```js
assert.property(conversionTable.gram, "ounce");
```

La proprietà `ounce` dovrebbe essere impostata su `0.0353`.

```js
assert.strictEqual(conversionTable.gram?.ounce, 0.0353);
```

Il tuo oggetto `gram` dovrebbe avere una proprietà `teaspoon`.

```js
assert.property(conversionTable.gram, "teaspoon");
```

La proprietà `teaspoon` dovrebbe essere impostata su `0.2`.
