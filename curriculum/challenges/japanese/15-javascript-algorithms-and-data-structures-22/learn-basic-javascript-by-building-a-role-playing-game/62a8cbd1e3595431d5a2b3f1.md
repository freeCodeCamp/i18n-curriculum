...
購入した武器をプレイヤーに伝えましょう。 先ほど書いた 2 行のコードの間で、`let` を使って `newWeapon` という新しい変数を初期化してください。 この変数には `weapons` を代入してください。

# --hints--

`buyWeapon` 関数には `newWeapon` という名前の変数が必要です。

```js
assert.match(buyWeapon.toString(), /newWeapon/);
```

`let` を使って `newWeapon` を宣言してください。

```js
assert.match(code, /let\s+newWeapon/);
```

`newWeapon` は `weapons` の値を持つように初期化してください。 セミコロンを忘れないようにしましょう。

```js
assert.match(buyWeapon.toString(), /newWeapon\s*=\s*weapons\s*;/);
```

`newWeapon` は `text` を変更する前に宣言されるべきです。
