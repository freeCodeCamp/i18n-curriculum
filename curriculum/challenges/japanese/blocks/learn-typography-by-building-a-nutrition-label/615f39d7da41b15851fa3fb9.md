...
`.label` セレクターに `margin` プロパティを `20px auto` に設定し、`padding` プロパティを `0 7px` に設定してください。

# --hints--

`.label` セレクターには `margin` プロパティを `20px auto` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('.label')?.margin === '20px auto');
```

`.label` セレクターには `padding` プロパティを `0 7px` に設定してください。
