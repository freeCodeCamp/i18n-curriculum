...
책 제목 배열도 좋지만, 각 책에 대해 `title`와 `author`를 이렇게 표시하면 좋겠습니다:
...
`map()` 메서드 사용을 업데이트하여 대신 이 형식의 문자열 배열을 반환하세요: `TITLE by AUTHOR`.

이 결과를 얻으려면 템플릿 리터럴이나 `+` 연산자를 사용한 문자열 연결 중 하나를 사용해야 합니다.
...
`getBookInformation` 함수는 각 책에 대해 `title`와 `author`가 포함된 문자열 배열을 반환해야 합니다.
