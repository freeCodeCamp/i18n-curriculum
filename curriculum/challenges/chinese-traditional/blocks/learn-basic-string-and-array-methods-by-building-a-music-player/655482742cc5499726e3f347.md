...
在 `playPreviousSong` 函式（程式）中，為你的 `if` 述語添加條件 `userData?.currentSong === null`。這將檢查目前是否沒有歌曲正在播放。如果沒有，請使用 `return` 離開函式（程式）。

在 `else` 區塊中，創建一個名為 `currentSongIndex` 的常數，並指定其為 `getCurrentSongIndex()`。

# --hints--

你不應該更改現有的 `playPreviousSong` 函式（程式）及其內容。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

你應該在你的 `playPreviousSong` 函式中有一個條件為 `userData?.currentSong === null` 的 `if` 述語。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*/)
```

你應該在你的 `if` 述語的區塊內有 `return`。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?/)
```

你的 `if` 述語應該有一個 `else` 區塊。

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*\{\s*if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{?\s*return\s*;?\s*\}?\s*else\s*\{\s*/)
```

你應該在你的 `if` 述語的 `else` 區塊內呼叫 `getCurrentSongIndex` 並將其指定給 `currentSongIndex`。
