...
Debes darle al primer elemento `h2` un contenido de texto adecuado. *Pista: yo habría elegido `Student Info`*

```js
assert.isNotEmpty(document.querySelectorAll('h2')[0]?.textContent.trim());
```

Debes darle al segundo elemento `h2` un contenido de texto adecuado. *Pista: yo habría elegido `HTML`*

```js
assert.isNotEmpty(document.querySelectorAll('h2')[1]?.textContent.trim());
```

Debes darle al tercer elemento `h2` un contenido de texto adecuado. *Pista: yo habría elegido `CSS`*
