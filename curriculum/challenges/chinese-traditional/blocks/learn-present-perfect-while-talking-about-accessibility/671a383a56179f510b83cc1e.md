...
Tom 的回應，`We haven't been receiving as many negative comments`，使用了否定形式的 `Present Perfect Continuous`。這個時態是由 `haven't/hasn't + been + verb ending in -ing` 組成，用來描述一段時間內未曾發生的動作。在此情況下，表示在一段時間內收到的負面評論較少，暗示可能有所改善。
