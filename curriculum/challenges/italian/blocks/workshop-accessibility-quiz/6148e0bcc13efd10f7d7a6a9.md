...
Imposta il colore di sfondo di `footer` su `#2a2a40` e usa _Flexbox_ per centrare orizzontalmente il testo.

# --hints--

Dovresti usare il selettore dell'elemento `footer`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('footer'));
```

Dovresti dare a `footer` una `background-color` di `#2a2a40`.
...
Dovresti dare a `footer` una `justify-content` di `center`.
