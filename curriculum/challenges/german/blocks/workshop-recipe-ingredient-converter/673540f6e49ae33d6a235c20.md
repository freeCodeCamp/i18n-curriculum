...
Sie müssen eine Liste der Einheiten führen, die Ihre App unterstützt.

Deklarieren Sie ein `units`-Array und geben Sie ihm die String-Werte `"cup"`, `"gram"`, `"ounce"` und `"teaspoon"`.

# --hints--

Sie sollten eine `units`-Variable deklarieren.

```js
assert.isDefined(units);
```

Ihre `units`-Variable sollte ein Array sein.

```js
assert.isArray(units);
```

Ihr `units`-Array sollte vier Elemente haben.

```js
assert.lengthOf(units, 4);
```

Ihr `units`-Array sollte den String `"cup"` enthalten.

```js
assert.include(units, "cup");
```

Ihr `units`-Array sollte den String `"gram"` enthalten.

```js
assert.include(units, "gram");
```

Ihr `units`-Array sollte den String `"ounce"` enthalten.

```js
assert.include(units, "ounce");
```

Ihr `units`-Array sollte den String `"teaspoon"` enthalten.
