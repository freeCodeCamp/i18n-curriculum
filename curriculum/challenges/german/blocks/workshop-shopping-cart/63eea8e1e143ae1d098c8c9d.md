...
Jetzt müssen Sie das `product` in das `items`-Array des Warenkorbs schieben. Denken Sie daran, das Schlüsselwort `this` zu verwenden.

# --hints--

Sie sollten die Methode `push` auf dem `items`-Array aufrufen.
...
Denken Sie daran, dass Sie das Schlüsselwort `this` verwenden müssen, um auf das `items`-Array zuzugreifen.
...
Sie sollten die Variable `push` in das `product`-Array `items`.
