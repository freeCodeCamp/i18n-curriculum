...
Você também deve criar uma função que limpe os campos de entrada.

Crie uma função `clearInputFields`. Dentro dela, use `querySelectorAll` para obter todos os campos de entrada e faça um loop por eles usando `forEach()`. Dentro do `forEach()`, defina o valor de cada `input` como uma string vazia.

# --hints--

Você deve criar uma função `clearInputFields`.

```js
assert.isFunction(clearInputFields)
```

Sua função `clearInputFields` deve definir cada campo de entrada como uma string vazia.
