...
為你的物件添加一個 `has2` 屬性，該屬性傳回 `nums` 陣列中是否有值為 `2`，以及一個 `increment` 屬性，該屬性傳回每個值都加一的 `nums`。

# --hints--

你的 `spreadsheetFunctions` 物件應該有一個 `has2` 屬性。

```js
assert.property(spreadsheetFunctions, "has2");
```

你的 `has2` 屬性應該是一個函式（程式）。

```js
assert.isFunction(spreadsheetFunctions.has2);
```

你的 `has2` 函式（程式）應該在陣列中有 `2` 時傳回 `true`。

```js
assert.isTrue(spreadsheetFunctions.has2([1, 2, 3]));
```

如果陣列中沒有 `2`，你的 `has2` 函式應該傳回 `false`。

```js
assert.isFalse(spreadsheetFunctions.has2([1, 3, 4]));
```

你的 `spreadsheetFunctions` 物件應該有一個 `increment` 屬性。

```js
assert.property(spreadsheetFunctions, "increment");
```

你的 `increment` 屬性應該是一個函式（程式）。

```js
assert.isFunction(spreadsheetFunctions.increment);
```

你的 `increment` 函式應該傳回一個所有數字都加 1 的陣列。
