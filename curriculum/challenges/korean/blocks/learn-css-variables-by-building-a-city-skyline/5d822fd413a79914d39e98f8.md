...
`div` 안에 두 개의 새로운 `.bb2` 요소를 중첩하고, 순서대로 `bb2a`와 `bb2b` 클래스를 부여하세요. 이들은 이 건물의 두 개 섹션이 됩니다.

# --hints--

`div`에 두 개의 `.bb2` 요소를 추가하세요.

```js
assert.equal(document.querySelector('div.bb2')?.children?.length, 2);
```

첫 번째 `div`에 `bb2a` 클래스를 부여하세요.

```js
assert.exists(document.querySelector('div.bb2a'));
```

두 번째 `div`에 `bb2b` 클래스를 부여하세요.
