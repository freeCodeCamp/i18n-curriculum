...
Du solltest entweder den `.info input`- oder `.info > input`-Selektor verwenden.
...
Du solltest den `input`-Elementen eine `width` von `50%` geben.
...
Du solltest dem `input`-Element ein `text-align` von `left` geben.
