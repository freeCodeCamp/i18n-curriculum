...
Ndani ya `map()`, ongeza usemi wa `return` wenye alama za kupigia ambapo utachanganya vipengele vyote vinavyohusika kuonyesha maelezo ya wimbo.

Ndani ya alama za kupigia, tengeneza kipengele cha `li` chenye sifa ya kipengele ya `id` yenye thamani `song-${song.id}` na sifa ya kipengele ya `class` yenye thamani `playlist-song`.

# --hints--

Unapaswa kuongeza usemi wa `return` wenye alama za kupigia tupu.

```js
assert.match(code, /return\s*`/)
```

Unapaswa kutengeneza lebo ya `li` ndani ya alama za kupigia.

```js
assert.match(code, /return\s*`\s*<li\s*/)
```

Lebo yako ya ufunguzi ya `li` inapaswa kuwa na sifa ya kipengele ya `id` iliyowekwa kuwa `song-${song.id}`.

```js
assert.match(code, /return\s*`\s*<li[^>]*\sid\s*=\s*('|")song-\$\{song\.id\}\1[^>]*>\s*<\/li>\s*`\s*;?/)
```

Lebo yako ya ufunguzi ya `li` inapaswa kuwa na sifa ya kipengele ya `class` iliyowekwa kuwa `playlist-song`.
