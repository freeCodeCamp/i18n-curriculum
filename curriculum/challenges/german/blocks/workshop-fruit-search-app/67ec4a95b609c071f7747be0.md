...
Fügen Sie innerhalb des `form`-Elements ein `label`-Element hinzu. Ihr `label`-Element sollte den Text `Search for fruits:` und ein `htmlFor`-Attribut mit dem Wert `"search-input"` haben.

# --hints--

Sie sollten ein `label`-Element innerhalb des `form`-Elements haben.
...
Das `label`-Element sollte ein `htmlFor`-Attribut mit dem Wert `"search-input"` haben.
...
Ihr `label`-Element sollte den Text `Search for fruits:` enthalten.
