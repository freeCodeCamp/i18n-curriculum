...
为了显示 `mean` 的值，你的应用已经准备好了 `#mean` 元素。

使用 `.querySelector` 查找该元素，然后将其 `.textContent` 设置为 `mean` 的值。

# --hints--

你的 `calculate` 函数应该使用 `document.querySelector()`。

```js
assert.lengthOf(calculate.toString().match(/document\.querySelector\(/g), 2);
```

你的 `.querySelector()` 应该选中 `#mean` 元素。

```js
assert.match(calculate.toString(), /document\.querySelector\(\s*('|")#mean\1\s*\)/);
```

你的 `calculate` 函数应该将 `#mean` 元素的 `.textContent` 设置为 `mean` 的值。
