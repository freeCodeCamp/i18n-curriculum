...
编写一个名为 `add_time` 的函数，该函数接受两个必需参数和一个可选参数：

- 12 小时制格式的开始时间（以 AM 或 PM 结尾）
- 指示小时数和分钟数的持续时间
- （可选）一周的开始日期，不区分大小写

该函数应将持续时间添加到开始时间并返回结果。

如果结果是第二天，则时间后应显示`(next day)`。 如果结果晚于一天以上，则应在时间后显示`(n days later)`，其中“n”是晚后的天数。

如果函数被赋予可选的一周开始日期参数，则输出应显示结果的星期几。 输出中的星期几应出现在时间之后、天数之前。

以下是该函数应处理的不同情况的一些示例。 请密切注意结果的间距和标点符号。
...
# --hints--
调用 `add_time('3:30 PM', '2:12')` 应该返回 `'5:42 PM'`。
...
调用 `add_time('11:55 AM', '3:12')`  应该返回 `'3:07 PM'`
...
调用 `add_time('2:59 AM', '24:00')` 应返回 `'2:59 AM (next day)'`。
...
调用 `add_time('11:59 PM', '24:05')` 应返回 `'12:04 AM (2 days later)'`。
...
调用 `add_time('8:16 PM', '466:02')` 应返回 `'6:18 AM (20 days later)'`。
...
调用 `add_time('3:30 PM', '2:12', 'Monday')` 应返回 `'5:42 PM, Monday'`。
...
调用 `add_time('2:59 AM', '24:00', 'saturDay')` 应返回 `'2:59 AM, Sunday (next day)'`。
...
调用 `add_time('11:59 PM', '24:05', 'Wednesday')` 应返回 `'12:04 AM, Friday (2 days later)'`。
...
调用 `add_time('8:16 PM', '466:02', 'tuesday')` 应该返回 `'6:18 AM, Monday (20 days later)'`。
