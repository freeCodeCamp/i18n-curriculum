...
الآن، في ملف JavaScript، عرّف متغيرًا باسم `happyBtn`.

استخدم `querySelector` لحفظ مرجع إلى العنصر `#happy-btn` في المتغير `happyBtn`.

# --hints--

يجب أن يكون لديك متغير باسم `happyBtn`.

```js
assert.exists(happyBtn);
```

يجب أن يحمل المتغير `happyBtn` مرجعًا إلى العنصر `#happy-btn`.
