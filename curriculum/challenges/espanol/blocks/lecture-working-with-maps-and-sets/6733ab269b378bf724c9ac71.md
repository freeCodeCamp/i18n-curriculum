...
| --- | --- | --- |
Tipo de Valores AlmacenadosAlmacena cualquier tipo de datoSolo almacena objetos
ReferenciasReferencias fuertesReferencias débiles
IteraciónSoporta iteración con `forEach` y buclesNo soporta iteración
Métodos y Propiedades`add()`, `delete()`, `has()`, `keys()`, `values()`, `size`, y másSolo `add()`, `delete()` y `has()`
Caso de usoColección de propósito general de valores únicos y eliminación de duplicados de arreglosSeguimiento eficiente de la memoria de referencias de objetos
