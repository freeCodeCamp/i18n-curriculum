...
تم تحريك النص الآخر من مكانه. أعده إلى موضعه عبر إعطاء المحدِّد `.first-paragraph::first-letter` خاصية `float` بقيمة `left` وخاصية `margin-right` بقيمة `1rem`.

# --hints--

يجب أن يحتوي المحدِّد `.first-paragraph::first-letter` على خاصية `float` بقيمة `left`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float === 'left');
```

يجب أن يحتوي المحدِّد `.first-paragraph::first-letter` على خاصية `margin-right` بقيمة `1rem`.
