...
title: Tengeneza programu ya jaribio fupi la kadi za kumbukumbu
challengeType: 25
...
Katika maabara hii, utaunda programu inayonyesha na kuhifadhi data za kadi za kumbukumbu ambazo zinaweza kutolewa baadaye.

**Lengo:** Timiza hadithi za watumizi zilizo hapa chini na upite vipimo vyote ili kumaliza maabara hii.

**Hadithi za watumizi:**

1) Unapaswa kuwa na kipengele cha HTML chenye kitambulisho cha `flashcard`.  
2) Unapaswa kuwa na kiolesura cha `FlashCard`.  
3) Kiolesura cha `FlashCard` kinapaswa kuwa na sifa zifuatazo `questionText` na `questionAnswer` zote zikiwa za aina ya `string`.  
4) Unapaswa kuwa na mkusanyiko wa vipengele vya `FlashCard` vinavyoitwa `currentCards`.  
5) Wakati kipengele cha `#flashcard` kinapobofyiwa, kadi inapaswa kuwa na darasa la vitu la `flipped`.  
6) Unapaswa kuwa na kipengele chenye kitambulisho cha `delete-btn`.  
7) Wakati kipengele cha `#delete-btn` kinapobofyiwa, kinapaswa kuondoa kadi ya sasa na kuonyesha data ya kadi iliyopita.  
8) Unapaswa kuunda fomu ya kuingiza data yenye kitambulisho cha `entry-form` ili uweze kuongeza kadi zaidi za kumbukumbu kwenye mkusanyiko wa `currentCards` wakati wa kuwasilisha.  
9) Vipengele viwili ndani ya fomu vinapaswa kuwa na vitambulisho vya `front-text` na `back-text` mtawalia.  
10) Unapaswa kuunda na kuita `InvalidUserInputError` wakati maandishi ya swali au jibu la swali ni tupu katika fomu ya kuingiza data.

# --hints--

Unapaswa kuwa na kipengele cha HTML chenye kitambulisho cha `flashcard`.
...
Unapaswa kuwa na kiolesura cha `FlashCard`.
...
Kiolesura cha `FlashCard` kinapaswa kuwa na sifa ya `questionText` ya aina ya `string`.
...
Kiolesura cha `FlashCard` kinapaswa kuwa na sifa ya `questionAnswer` ya aina ya `string`.
...
Unapaswa kuwa na mkusanyiko wa vipengele vya `FlashCard` vinavyoitwa `currentCards` zikiwa na aina ya `FlashCard[]`.
...
Wakati kipengele cha `#flashcard` kinapobofyiwa kwa mara ya kwanza, kipengele hicho kinapaswa kupokea darasa la vitu la `flipped`.
...
Unapaswa kuwa na kipengele chenye kitambulisho cha `delete-btn`.
...
Wakati `delete-btn` inapobofyiwa, kipengele cha kadi ya kumbukumbu kinapaswa kuondolewa kutoka kwenye mkusanyiko wa `currentCards`.
...
Wakati `delete-btn` inapobofyiwa, data ya kadi ya kumbukumbu iliyopita inapaswa kuonyeshwa.
...
Unapaswa kuunda fomu ya kuingiza data yenye kitambulisho cha `entry-form`.
...
Unapaswa kuwa na vipengele viwili vya `textarea` vyenye vitambulisho vya `front-text` na `back-text` mtawalia ndani ya fomu.
...
`InvalidUserInputError` inapaswa kutupwa wakati maandishi ya swali au jibu la swali ni tupu katika fomu ya kuingiza data.
