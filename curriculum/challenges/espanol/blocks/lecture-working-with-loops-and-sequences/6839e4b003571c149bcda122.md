...
En este ejemplo, `zip()` combina las dos listas en pares de elementos y devuelve un iterador de tuplas. Luego, el ciclo `for` desempaqueta cada tupla en `name` y `dev_id`. Finalmente, en cada sentencia de impresión, imprimimos cada `name` y `dev_id` de las listas `ids` y `developers` respectivamente. Así es como se ve el resultado en la consola:
...
Se usa para llevar un registro del índice de un iterable y devolver un objeto `enumerate`.
