...
Тепер, коли ви знаєте, як знайти наступну пісню для відтворення, створіть функцію з назвою `playNextSong`. Усередині функції `playNextSong` створіть інструкцію `if`, щоб перевірити, чи є `currentSong` у `userData` строго рівним `null`. Це перевірить, чи немає зараз пісні, що відтворюється, в об’єкті `userData`.

Якщо умова істинна, викличте функцію `playSong` з аргументом `id` першої пісні в масиві `userData.songs`.

Потім додайте інструкцію `return` в кінець блоку `if`.

# --hints--

У вас має бути функція з назвою `playNextSong`.

```js
assert.isFunction(playNextSong);
```

Коли `currentSong` у `userData` строго рівний `null`, ваша функція `playNextSong` має викликати функцію `playSong` з аргументом `id` першої пісні в масиві `userData.songs`.
...
У вас має бути інструкція `return` в кінці блоку `if`.
