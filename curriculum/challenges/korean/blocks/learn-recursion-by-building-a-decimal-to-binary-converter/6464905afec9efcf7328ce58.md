...
다음으로, `.forEach()` 메서드를 사용해 `animationData` 배열을 반복하세요. `.forEach()` 메서드의 콜백 함수에는 `obj`를 매개변수로 전달하되, 콜백 함수의 본문은 일단 비워 두세요.

# --hints--

`.forEach()` 메서드를 사용해 `animationData` 배열을 반복해야 합니다.

```js
assert.match(String(showAnimation), /animationData\s*\.\s*forEach\(/);
```

`obj` 메서드의 콜백 함수에 `.forEach()`를 매개변수로 전달해야 합니다.
...
`.forEach()` 메서드의 콜백 함수 본문은 비워 두어야 합니다.
