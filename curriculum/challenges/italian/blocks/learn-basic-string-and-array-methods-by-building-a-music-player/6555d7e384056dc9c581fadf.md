...
Per assicurarti che il display del giocatore si aggiorni ogni volta che inizia una nuova canzone, chiama la funzione `setPlayerDisplay()` all'interno della funzione `playSong()`.

Ora dovresti vedere il titolo della canzone e l'artista apparire nel display.

# --hints--

Dovresti chiamare la funzione `setPlayerDisplay` all'interno della tua funzione `playSong`.
