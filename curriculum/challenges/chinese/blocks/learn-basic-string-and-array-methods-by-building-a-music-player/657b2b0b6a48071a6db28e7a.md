...
`map()` 方法用于遍历数组并返回一个新数组。 当你想根据现有数组的值创建新数组时，它很有用。 例如：
...
请注意，`map()` 方法接受函数作为参数。 这被称为<dfn>回调函数</dfn>，它是作为参数传递给另一个函数的函数。  In the example above, the callback function is `(number) => number * 2`, and it's run on each element in the `numbers` array. 然后，`map()` 方法返回一个包含结果的新数组。

将回调函数传递给 `map()` 方法。 回调函数应该以 `song` 作为参数，使用箭头函数语法，并且具有空函数体。

# --hints--

你的 `map()` 方法的回调函数应该使用箭头函数语法。

```js
assert.match(code, /const\s+songsHTML\s*=\s*array\.map\(\s*(\(.*\)|[^()]+)\s*=>\s*\{\s*\}\s*\)/)
```

你的 `map()` 方法在其回调函数中应该有一个 `song` 参数。
