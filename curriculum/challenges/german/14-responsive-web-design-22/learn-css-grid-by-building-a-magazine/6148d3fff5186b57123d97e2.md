...
Ein Zitat ist ohne passende Anführungszeichen nicht wirklich ein Zitat. Du kannst diese mit den CSS-Pseudo-Selektoren hinzufügen.

Erstelle einen `.quote::before`-Selektor und setzte die `content`-Eigenschaft auf `"` mit einem Leerzeichen, das ihm folgt.

Erstelle ebenfalls einen `.quote::after`- Selektor und setzte die `content`-Eigenschaft auf `"` mit einem Leerzeichen, das ihm vorausgeht.

# --hints--

Du solltest einen `.quote::before`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

Dein `.quote::before`-Selektor sollte eine `content`-Eigenschaft mit dem Wert `'" '` haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content?.match(/\"\\"\s\"/));
```

Du solltest ein `.quote::after`-Selektor haben.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

Dein `.quote::after`-Selektor sollte eine `content`-Eigenschaft mit `' "'` haben.
