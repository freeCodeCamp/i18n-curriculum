...
當你點擊播放列表中的歌曲時，你會想播放該歌曲。為此，你需要為每個作為 `song` 元素子元素的 `button` 元素添加 `click` 事件監聽器。

完成你的 `forEach` 的回呼以執行該操作。在 `addEventListener` 回呼內，呼叫 `playSong(n)`，其中 `n` 是你可以從 `song` 的 `id` 屬性取得的歌曲編號。

請注意，每個 `song` 都有一個 `id` 屬性為 `song-n`：你需要取得 `n` 零件並將其轉換成數字，然後再傳遞給 `playSong()`。


# --hints--

你應該為每個作為 `.playlist-song` 元素子元素的 `button` 元素添加 `click` 事件監聽器，並使用對應歌曲的 `id` 呼叫 `playSong`。
