...
Umeongeza darasa la vitu la `center` kwa baadhi ya vipengele katika HTML. Sasa, unapaswa kufafanua darasa hilo ili kuziweka vipengele hivyo katikati.

Lenga darasa la vitu la `center` na weka vipengele vyenye darasa hilo katikati kwa kutumia sifa ya `text-align` iliyowekwa kuwa `center`.

# --hints--

Unapaswa kulenga vipengele vyenye darasa la vitu la `center`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.center'));
```

Unapaswa kuweka sifa ya `text-align` kuwa `center`.
