...
Ndani ya kitendakazi chako cha `forumCategory`, tengeneza kigezo kipya cha `let` kinachoitwa `selectedCategory` na weka thamani yake kuwa kitu tupu. Hii itatumika kuhifadhi jina la kundi na jina la darasa la vitu kwa kila kundi.

# --hints--

Unapaswa kutumia `let` kutangaza kigezo kinachoitwa `selectedCategory`. 

```js
assert.match(code, /\s*let\s+selectedCategory\s*/);
```

Unapaswa kuweka thamani ya kitu tupu kwa kigezo cha `selectedCategory`.
