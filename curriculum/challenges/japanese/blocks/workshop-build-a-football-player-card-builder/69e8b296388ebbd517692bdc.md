...
SHO `value`に`player.sho`の`input`属性を追加してください。それから、その`onChange`属性を、`e`をパラメータに取り、`setPlayer`をスプレッドし、`player`を`sho`に設定して`Number(e.target.value)`を呼び出すアロー関数に設定してください。

# --hints--

あなたのSHO `input`は、現在のプレイヤーの`value`ステータスと等しい`sho`を持つ必要があります。
...
あなたのSHO `input`は、`onChange`をスプレッドし、`setPlayer`を`player`に設定して`sho`を呼び出す`Number(e.target.value)`ハンドラを持つ必要があります。
