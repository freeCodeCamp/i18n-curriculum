...
創建一個 `if` 語句來檢查 `formInputsContainValues` 是否爲真。 如果 `formInputsContainValues` 爲 true，表明有變化，請在 `confirmCloseDialog` 上使用 `showModal()` 方法。 否則，如果沒有變化，則調用 `reset()` 函數清除輸入字段並隱藏表單模式。

# --hints--

你應該創建一個帶有條件 `formInputsContainValues` 的 `if` 語句。

```js
assert.match(code, /if\s*\(\s*formInputsContainValues\s*\)\s*\{/)
```

`if` 語句的 `if` 塊應包含 `confirmCloseDialog.showModal();`。
...
你應該在 `if` 語句的 `else` 塊中調用 `reset()` 函數。
