...
`input` inayofuata ni kisanduku cha kuchagua ili kuashiria nguvu zote za mashujaa wa ajabu zinazomhusu mtumizi.

Tengeneza kipengele cha `label` chenye `className` ya `section` na `column`, na maandishi ya `List your powers (select all that apply):`.

# --hints--

Unapaswa kutengeneza kipengele cha `label` chenye `className` ya `section` na `column`.
...
Kipengele chako cha `label` kinapaswa kuwa na maandishi `List your powers (select all that apply):`.
