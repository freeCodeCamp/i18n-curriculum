...
爲 `.note` 選擇器提供 `8px` 的左右內邊距，移除上下內邊距。 再將 `text-indent` 屬性設置爲 `-8px`。

通過這些最後的更改，你的營養成分表就完成了！

# --hints--

你的 `.note` 選擇器應該有一個 `padding` 屬性，設置爲 `0 8px`。
...
你的 `.note` 選擇器應該有一個 `text-indent` 屬性，設置爲 `-8px`。
