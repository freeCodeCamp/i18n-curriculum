...
title: Costruisci un grafico a barre
challengeType: 25
...
In questo progetto userai i dati da `https://cdn.freecodecamp.org/curriculum/labs/data/bar-chart/GDP-data.json` per costruire un grafico a barre.

I test richiedono che gli assi vengano generati usando la proprietà axis di D3, che crea automaticamente i segni di graduazione lungo l'asse. Questi segni sono necessari per superare i test D3 perché le loro posizioni vengono usate per determinare l'allineamento degli elementi del grafico.
...
1. Il tuo grafico dovrebbe avere un elemento `rect` per ogni punto dati con una `class="bar"` corrispondente che mostra i dati.
1. Ogni `.bar` dovrebbe avere gli attributi `data-date` e `data-gdp` contenenti valori `date` e `GDP`.
1. Gli attributi `.bar` degli elementi `data-date` dovrebbero corrispondere all'ordine dei dati forniti.
1. Gli attributi `.bar` degli elementi `data-gdp` dovrebbero corrispondere all'ordine dei dati forniti.
1. L'altezza di ogni elemento `.bar` dovrebbe rappresentare accuratamente il `GDP` corrispondente dei dati.
1. L'attributo `data-date` e il suo elemento `.bar` corrispondente dovrebbero essere allineati con il valore corrispondente sull'asse x.
1. L'attributo `data-gdp` e il suo elemento `.bar` corrispondente dovrebbero essere allineati con il valore corrispondente sull'asse y.
...
1. Il tuo tooltip dovrebbe avere un attributo `data-date` che corrisponde al `data-date` dell'area attiva.
...
Il grafico dovrebbe avere un elemento con `id` uguale a `title` per contenere il titolo.

```js
assert.exists(document.getElementById('title'));
```

Il grafico dovrebbe avere un elemento `g` asse x con un id corrispondente `x-axis`.

```js
assert.isNotEmpty(document.querySelector('g#x-axis'))
```

Il grafico dovrebbe avere un elemento `g` asse y con un id corrispondente `y-axis`.

```js
assert.isNotEmpty(document.querySelector('g#y-axis'))
```

L'asse x dovrebbe contenere più etichette di segni di graduazione, ciascuna con il corrispondente `class="tick"`.

```js
assert.isNotEmpty(document.querySelectorAll('#x-axis .tick'));
```

L'asse y dovrebbe contenere più etichette di segni di graduazione, ciascuna con il corrispondente `class="tick"`.

```js
assert.isNotEmpty(document.querySelectorAll('#y-axis .tick'));
```

Il tuo grafico dovrebbe avere un elemento `rect` per ogni punto dati con una `class="bar"` corrispondente che mostra i dati.

```js
assert.lengthOf(document.querySelectorAll('rect.bar'), GDPDataJson.data.length);
```

Ogni barra dovrebbe avere gli attributi `data-date` e `data-gdp` contenenti valori di data e PIL.
...
Gli attributi `data-date` degli elementi barra dovrebbero corrispondere all'ordine dei dati forniti.
...
Gli attributi `data-gdp` degli elementi barra dovrebbero corrispondere all'ordine dei dati forniti.
...
L'altezza di ogni elemento barra dovrebbe rappresentare accuratamente il PIL corrispondente dei dati.
...
L'attributo `data-date` e il suo elemento barra corrispondente dovrebbero essere allineati con il valore corrispondente sull'asse x. Puoi usare `d3.scaleTime()` per creare una scala temporale per l'asse x.
...
L'attributo `data-gdp` e il suo elemento barra corrispondente dovrebbero essere allineati con il valore corrispondente sull'asse y. Puoi usare `d3.scaleLinear()` per creare una scala lineare per l'asse y.
...
Quando passi il mouse su un'area, il tuo grafico a barre dovrebbe mostrare un tooltip con una `id="tooltip"` corrispondente che mostra più informazioni sull'area.
...
Il tooltip dovrebbe avere un attributo `data-date` che corrisponde al `data-date` dell'area attiva.
