...
次に、ジェネリック型パラメータを `Bee` に設定して、`Bug` クラスを拡張する `string` という名前のサブクラスを作成してください。

# --hints--

`Bee` という名前のクラスがあるはずです。
...
`Bee` クラスは型引数 `Bug` を指定して `string` を拡張している必要があります。
