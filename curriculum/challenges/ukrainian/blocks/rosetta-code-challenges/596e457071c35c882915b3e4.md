...
title: Порівняння списку рядків
challengeType: 1
forumTopicId: 302235
...
Список — це впорядкована множина значень, яка може містити дублікати. Ось приклад:
...
`allEqual(["AA", "AA", "AA", "AA"])` має повернути `true`.

```js
assert(allEqual(testCases[0]));
```

`azSorted(["AA", "AA", "AA", "AA"])` має повернути `false`.

```js
assert(!azSorted(testCases[0]));
```

`allEqual(["AA", "ACB", "BB", "CC"])` має повернути `false`.

```js
assert(!allEqual(testCases[1]));
```

`azSorted(["AA", "ACB", "BB", "CC"])` має повернути `true`.

```js
assert(azSorted(testCases[1]));
```

`allEqual([])` має повернути `true`.

```js
assert(allEqual(testCases[2]));
```

`azSorted([])` має повернути `true`.

```js
assert(azSorted(testCases[2]));
```

`allEqual(["AA"])` має повернути `true`.

```js
assert(allEqual(testCases[3]));
```

`azSorted(["AA"])` має повернути `true`.

```js
assert(azSorted(testCases[3]));
```

`allEqual(["BB", "AA"])` має повернути `false`.

```js
assert(!allEqual(testCases[4]));
```

`azSorted(["BB", "AA"])` має повернути `false`.
