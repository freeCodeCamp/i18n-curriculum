...
ミッションコントロールから、乗組員を可変サイズのチャンクに分割する新しい関数の作成が依頼されました。`chunkCrew` という名前の空の関数を作成し、`crew` と `size` の2つのパラメータを受け取るようにしてください。

# --hints--

`chunkCrew` という名前の関数を作成してください。

```js
assert.isFunction(chunkCrew);
```

`chunkCrew` 関数は `crew` と `size` のパラメータを持つ必要があります。
