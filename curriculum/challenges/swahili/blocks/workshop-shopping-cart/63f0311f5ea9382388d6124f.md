...
Tatizo na `.toFixed()` kurudisha mfuatano wa herufi ni kwamba unataka uweze kufanya mahesabu na kiwango cha kodi. Ili kurekebisha hili, unaweza kupitisha wito wa `.toFixed()` (ikiwa ni pamoja na hesabu) kwa kitendakazi cha `parseFloat()`. Hii itabadilisha mfuatano wa herufi uliowekwa kuwa nambari tena, ikihifadhi sehemu za desimali zilizopo.

Pitisha wito wako wa `.toFixed()` kwa `parseFloat()`.

# --hints--

Unapaswa kupitisha hesabu yako yote (isipokuwa tamko la `return`) kwa `parseFloat()`.
