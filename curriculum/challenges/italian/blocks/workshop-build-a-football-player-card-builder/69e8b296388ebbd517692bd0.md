...
Dopo il `form-row`, aggiungi un `div` con il `className` di `form-group`. Al suo interno, aggiungi un elemento `label` con il `className` di `label`, un `htmlFor` di `club` e il testo `Club`. Poi aggiungi un elemento input con un `id` di `club`, un `className` di `input` e un `type` di `text`.
...
Il tuo `div.form-group` dovrebbe avere un `label` con un attributo `htmlFor` di `club`, un `className` di `label` e il testo `Club`.
...
Il tuo Club `form-group` dovrebbe contenere un `input` con un `id` di `club`, un `className` di `input` e un `type` di `text`.
