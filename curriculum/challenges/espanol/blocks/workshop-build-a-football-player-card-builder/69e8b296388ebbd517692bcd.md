...
Ahora comenzarás a construir el formulario. Dentro del `div` con el `className` de `form-panel`, agrega un `div` para envolver la sección "Player Info". Dentro de ese nuevo `div`, añade `p` con el `className` de `form-section-title` y el texto `Player Info`.

Continuarás agregando campos a esta sección en los siguientes pasos.

# --hints--

Tu `div.form-panel` debe contener un `div` para envolver la sección Player Info.
...
El `div` dentro de `div.form-panel` debe contener un `p` con el `className` de `form-section-title` y el texto `Player Info`.
