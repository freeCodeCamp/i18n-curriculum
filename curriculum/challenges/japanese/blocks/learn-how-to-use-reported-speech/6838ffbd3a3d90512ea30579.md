---
id: 6838ffbd3a3d90512ea30579
title: タスク117
challengeType: 22
dashedName: task-117
lang: en-US
---

<!-- (Audio) Mark: I was thinking the same thing. If only we had prioritized the update, we might not have faced this issue. -->

# --instructions--

音声を聞いて、下の文を完成させてください。

# --fillInTheBlank--

## --sentence--

`I was thinking the same thing. If only we had BLANK the update, we might not have BLANK this issue.`

## --blanks--

`prioritized`

### --feedback--

これは、何が最も重要かを決めて、それを最初に行うことを意味します。`-ed`で終わります。

---

`faced`

### --feedback--

これは、問題や状況に対処したり立ち向かったりすることを意味します。`-ed`で終わります。

# --explanation--

`To prioritize`は、タスクや問題を重要度の順に並べて、最も重要なものを最初に行うことを意味します。例えば：

`The team prioritized fixing the security flaw before adding new features.` – これは、最も緊急な問題を最初に解決することを選んだことを意味します。

`To face`は、困難な状況やチャレンジに立ち向かうことを意味します。例えば：

`We faced many obstacles during the project.` – これは、多くの問題に直面し、それに対処しなければならなかったことを意味します。

# --scene--

```json
{
  "setup": {
    "background": "company2-center.png",
    "characters": [
      {
        "character": "Mark",
        "position": {
          "x": 50,
          "y": 0,
          "z": 1.4
        },
        "opacity": 0
      }
    ],
    "audio": {
      "filename": "B1_21-3.mp3",
      "startTime": 1,
      "startTimestamp": 30.7,
      "finishTimestamp": 36.24
    }
  },
  "commands": [
    {
      "character": "Mark",
      "opacity": 1,
      "startTime": 0
    },
    {
      "character": "Mark",
      "startTime": 1,
      "finishTime": 2.4,
      "dialogue": {
        "text": "I was thinking the same thing.",
        "align": "center"
      }
    },
    {
      "character": "Mark",
      "startTime": 2.48,
      "finishTime": 4.66,
      "dialogue": {
        "text": "If only we had prioritized the update,",
        "align": "center"
      }
    },
    {
      "character": "Mark",
      "startTime": 4.82,
      "finishTime": 6.54,
      "dialogue": {
        "text": "we might not have faced this issue.",
        "align": "center"
      }
    },
    {
      "character": "Mark",
      "opacity": 0,
      "startTime": 7.04
    }
  ]
}
```
