...
Your new checkbox should have an `id` attribute with the value `energetic` and a `name` attribute with the value `personality`. 检查开始标签的名称后面是否有空格和/或所有属性名称之前是否有空格。
...
Your new checkbox should be after the second one. 你把它们的顺序写错了。
...
The new `label` should have a `for` attribute with the same value as the `id` attribute of the new checkbox. 你要么遗漏了属性值，要么拼写有误。
