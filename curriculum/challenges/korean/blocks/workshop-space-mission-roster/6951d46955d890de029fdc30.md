...
이전에 `console.log()` 함수의 동작을 검증하기 위해 `swapCrewMembers` 호출을 추가했었는데, 이제 그 호출이 더 이상 필요하지 않습니다. 앞으로 터미널이 복잡해지지 않도록 현재 콘솔에 로그를 출력하는 `for` 반복문을 삭제하세요.

# --hints--

`for` 함수에서 `swapCrewMembers` 반복문을 삭제하세요. 반복문 전체 블록을 삭제해야 합니다.
