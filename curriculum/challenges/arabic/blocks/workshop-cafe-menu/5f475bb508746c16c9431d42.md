...
لجعل الصورة تتصرف مثل عناصر مستوى الكتلة مثل العناوين، أنشئ محدِّد نوع `img`. اضبط خاصية `display` على `block`، واستخدم خاصيتي `margin-left` و`margin-right` لتمركزها أفقيًا.
...
يجب أن يكون لعنصر `img` خاصيتا `margin-left` و`margin-right` بقيمة `auto`.
