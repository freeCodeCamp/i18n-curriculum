...
在 `verify_card_number` 函式（程式）的第二個 `for` 迴圈下方，創建一個名為 `total` 的變數，並將奇數位與偶數位數字的總和指定給它。將 `total` 列印到主控台。

# --hints--

你應該在 `verify_card_number` 函式（程式）中有 `total = sum_of_odd_digits + sum_of_even_digits`。
...
你應該在 `verify_card_number` 函式（程式）中使用 `print(total)`。
