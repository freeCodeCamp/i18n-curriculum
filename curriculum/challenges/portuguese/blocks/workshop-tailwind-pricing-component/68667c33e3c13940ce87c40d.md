...
O `span` que tem a duração também precisa de algum estilo, então adicione as classes `text-base`, `font-medium` e `text-fuchsia-300` a ele.
...
Seu elemento `span` deve ter a classe `text-fuchsia-300`.
