...
プレイリストを元の状態にリセットするには、`allSongs` を配列に展開して `userData.songs` に代入してください。

*注意*: この時点で曲は `userData.songs` または `null` ではないため、`undefined` に対してオプショナルチェーンを使わないでください。
...
`[...allSongs]` を `userData.songs` に代入してください。
