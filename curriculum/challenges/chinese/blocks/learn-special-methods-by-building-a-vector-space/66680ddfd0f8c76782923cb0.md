...
`norm()` 方法返回了正确的值，但仍有一些可以改进的地方：可读性。

`vars()` 内置函数以对象作为参数并返回该对象的 `__dict__` 属性。

不要直接访问 `self` 的 `__dict__` 属性，而是修改 `norm` 方法以使用 `vars()` 函数。

# --hints--

你应该修改 `norm` 方法以使用 `vars(self)` 而不是 `self.__dict__`。
