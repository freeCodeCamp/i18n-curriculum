...
FRACTRAN は、数学者ジョン・ホートン・コンウェイによって発明されたチューリング完全な難解なプログラミング言語です。
...
コンウェイ氏は FRACTRAN で素数のためのプログラムを記述しました:
...
$n=2$ から開始して、この FRACTRAN プログラムは $n$ を $15=2\\times (\\frac{15}{2})$ に、その後、$825=15\\times (\\frac{55}{1})$に変更し、以下の整数数列を生成します。

$2$, $15$, $825$, $725$, $1925$, $2275$, $425$, $390$, $330$, $290$, $770$, $\\ldots$

2 の後には、この数列には以下の 2 の累乗が含まれます。
...
FRACTRAN プログラムを文字列パラメータとして取り、プログラムが生成する最初の 10 個の数値を配列として返す関数を記述してください。 結果が 10 個の数値を持たない場合は、数値をそのまま返します。
