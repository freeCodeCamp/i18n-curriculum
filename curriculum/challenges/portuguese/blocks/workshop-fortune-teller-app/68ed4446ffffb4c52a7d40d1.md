...
Dentro do construtor, adicione chamadas para `fetchCardsData()` e `initializeEventListeners()` usando a palavra-chave `this`, para que sejam executadas quando a classe for inicializada.

O método `fetchCardsData` será responsável por carregar os dados das cartas de tarô de uma API, e `initializeEventListeners` vai configurar os manipuladores de clique do botão. Você criará ambos os métodos nos próximos passos.

# --hints--

Você deve chamar o método `fetchCardsData()` dentro do `constructor`.
...
Você deve chamar o método `initializeEventListeners()` dentro do construtor.
