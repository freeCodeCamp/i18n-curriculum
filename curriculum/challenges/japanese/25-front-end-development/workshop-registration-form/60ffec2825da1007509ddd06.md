...
多くのブラウザーは、さまざまな要素に対して独自にデフォルトの CSS プロパティと値を挿入します。 注意して見ると、ファイルの `input` 要素が他のテキストの `input` 要素よりも小さいことが分かるかもしれません。 文字を入力できる `input` 要素の `padding` は、デフォルトで `1px 2px` に設定されています。
