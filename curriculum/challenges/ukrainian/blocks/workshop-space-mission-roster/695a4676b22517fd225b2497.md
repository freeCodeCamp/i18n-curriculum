...
Mission Control потребує ще одну функцію для ведення журналу зведення вашої команди. Створіть порожню функцію з іменем `printCrewSummary`, яка приймає параметр `crew`.

# --hints--

Ви повинні створити функцію з іменем `printCrewSummary`.

```js
assert.isFunction(printCrewSummary);
```

Ваша функція `printCrewSummary` має мати параметр `crew`.
