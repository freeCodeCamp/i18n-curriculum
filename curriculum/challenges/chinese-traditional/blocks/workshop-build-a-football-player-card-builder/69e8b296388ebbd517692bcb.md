...
`FootballPlayerCard` 目前只呈現了最基本的卡片。要建置完整頁面，請新增一個 `div` 元素，並設定 `className` 為 `page`。

在該 `div` 裡，新增一個 `header` 元素，型別為 `className`，名稱為 `header`。在 `header` 裡，新增一個 `div`，型別為 `className`，名稱為 `header-inner`。在 `header-inner` 元素中，分別新增兩段文字 `Football Card Builder` 和 `Customize your player card`。這兩段文字也應該各自擁有 `className` 的 `header-title` 和 `header-subtitle`。

在關閉 `header` 元素之後，但仍在擁有 `div` 為 `className` 的 `page` 裡，將 `PlayerCard` 組件移入，並以其 prop 作為暫時的佔位符，直到主要版面配置加入為止。

# --hints--

你的 `FootballPlayerCard` 應該呈現一個擁有 `div` 為 `className` 的 `page`。
...
你的 `div`，其 `className` 為 `page`，應包含一個擁有 `header` 為 `className` 的 `header` 元素。
...
你的 `header` 應包含一個擁有 `div` 為 `className` 的 `header-inner`。
...
你的 `header-inner` 元素應包含一個擁有 `p` 為 `className` 且文字為 `header-title` 的 `Football Card Builder`。
...
你的 `header-inner` 元素應包含一個擁有 `p` 為 `className` 且文字為 `header-subtitle` 的 `Customize your player card`。
...
你的 `PlayerCard` 組件應該被呈現在 `div.page` 裡。
