...
Ora crea un tag immagine e dagli il `class` `"user-img"`. Usa l'interpolazione di stringa per impostare l'attributo `src` al `image` che hai destrutturato prima. Imposta l'attributo `alt` su `author` seguito dal testo `"avatar"`. Assicurati che ci sia uno spazio tra la variabile `author` e la parola `"avatar"`, per esempio, `"Quincy Larson avatar"`.
