...
クラス`button`を持つ`playlist-song-info`要素を作成してください。`button`の中に、クラス`span`を持つ`playlist-song-title`要素を追加し、その後テキストとして`song.title`を挿入してください。

# --hints--

クラス`button`を持つ`playlist-song-info`を作成する必要があります。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

`span`要素の中に`button`要素を作成してください。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

`span`要素にはクラス`playlist-song-title`を付けてください。

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

`song.title`要素の内容として`span`を挿入してください。
