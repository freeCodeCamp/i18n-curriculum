...
將 `setTimeout` 的第一個引數更新為 `async` 箭頭函式。這讓你可以在延遲執行的邏輯中使用 `await`。在該函式內，建立一個 `try...catch` 區塊來控制代碼抓取資料時可能發生的錯誤。

# --hints--

你應該將 `async` 箭頭函式作為 `setTimeout` 的第一個引數。

```js
assert.match(code, /setTimeout\s*\(\s*async\s*\(\s*\)\s*=>\s*{/);
```

在 `async` 函式（程式）內，你應該創建一個 `try...catch` 區塊。
