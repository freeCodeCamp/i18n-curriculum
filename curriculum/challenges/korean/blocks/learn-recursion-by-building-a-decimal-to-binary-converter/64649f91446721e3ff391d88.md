...
마지막으로, 문자열 보간을 사용해 단락 요소의 텍스트를 `decimalToBinary(${currVal})`로 설정하세요. 여기서 `currVal`은 현재 객체의 `inputVal` 속성입니다. 그다음, 숫자 입력란에 숫자 `5`을 입력하고 `Convert` 버튼을 클릭해 코드를 테스트하세요.

# --hints--

문자열 보간을 사용해 단락 요소의 텍스트를 `decimalToBinary(${obj.inputVal})`로 설정해야 합니다.
