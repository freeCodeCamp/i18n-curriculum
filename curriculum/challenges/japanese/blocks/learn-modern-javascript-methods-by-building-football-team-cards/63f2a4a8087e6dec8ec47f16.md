...
次に、`"forward"` と照合する `case` 節を追加しましょう。

その `case` 内で、関数 `setPlayerCards` に実引数として `players.filter()` を渡して呼び出してください。

`filter()` メソッドの括弧内には、コールバック関数と `player` という仮引数を追加して、`player.position` が `"forward"` と等しいかチェックしてください。

最後に、`setPlayerCards` の関数呼び出しの下に `break` 文を追加してください。
