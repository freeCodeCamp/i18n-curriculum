...
افعل الشيء نفسه مع `multipleCardsBtn` و `newReadingBtn` عبر إرفاق مستمع حدث نقر إلى `multipleCardsBtn` يستدعي `multipleCardSelected`، وآخر إلى `newReadingBtn` يستدعي `newReading`.

ستُنشئ الطريقتين في الخطوات القادمة.

# --hints--

يجب إرفاق حدث `click` إلى عنصر `multipleCardsBtn`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

داخل استدعاء راجع حدث `multipleCardsBtn`، يجب استدعاء الطريقة `multipleCardSelected`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

يجب إرفاق حدث `click` إلى عنصر `newReadingBtn`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

داخل استدعاء راجع حدث `newReadingBtn`، يجب استدعاء الطريقة `newReading`.
