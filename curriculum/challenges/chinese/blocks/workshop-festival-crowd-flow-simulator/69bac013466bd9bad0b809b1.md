...
首先，你将查找 `gates` 数组中特定闸门的位置。

使用 `indexOf()` 方法在 `gates` 上查找 `currentGate` 元素，并将结果赋值给名为 `currentIndex` 的变量。
...
你应该在 `indexOf()` 数组上使用 `gates` 来访问 `currentGate` 的索引，并将其赋值给名为 `currentIndex` 的变量。这应在一行内完成。
