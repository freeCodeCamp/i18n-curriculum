...
使用类选择器，将 `.cat-whisker-left-top` 元素移动到位，将其 `position` 设置为 `absolute`，`top` 设置为 `120px`，`left` 设置为 `52px`。

# --hints--

你应该有一个 `.cat-whisker-left-top` 选择器。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top'))
```

你的 `.cat-whisker-left-top` 选择器应设置 `position` 属性为 `absolute` 。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.position === 'absolute')
```

你的 `.cat-whisker-left-top` 选择器应设置 `top` 属性为 `120px`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.top === '120px')
```

你的 `.cat-whisker-left-top` 选择器应设置 `left` 属性为 `52px`。
