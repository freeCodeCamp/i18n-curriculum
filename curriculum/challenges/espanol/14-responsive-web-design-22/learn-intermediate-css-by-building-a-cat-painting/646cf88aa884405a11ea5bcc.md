...
Tu selector `.cat-right-ear` debe tener una propiedad `border-top-left-radius` establecida en `90px`. No olvides añadir un punto y coma.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-right-ear')?.borderTopLeftRadius === '90px')
```

Tu selector `.cat-right-ear` debe tener una propiedad `border-top-right-radius` establecida en `10px`. No olvides añadir un punto y coma.
