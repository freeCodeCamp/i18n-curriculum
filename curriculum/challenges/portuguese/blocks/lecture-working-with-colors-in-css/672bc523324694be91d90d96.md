...
Este código daria ao `div` um fundo vermelho semi-transparente, onde o matiz é definido como `0` graus (vermelho), a saturação é `100%`, a luminosidade é `50%` e o alfa é `0.5` (50% de opacidade). Você também pode especificar o alfa como uma porcentagem — por exemplo, `hsla(0, 100%, 50%, 50%)` é equivalente a `hsla(0, 100%, 50%, 0.5)`.
