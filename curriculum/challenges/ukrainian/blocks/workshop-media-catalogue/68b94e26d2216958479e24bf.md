...
Було б добре перевірити елемент, переданий у метод `add`, перед тим як додати його до каталогу.

Створіть інструкцію `if`, яка використовує `isinstance` для перевірки, чи є `media_item` екземпляром `Movie`, і викидає `TypeError` з повідомленням `Only Movie or TVSeries instances can be added`.

Зверніть увагу, що `isinstance(obj, ClassName)` повертає `True`, якщо `obj` є екземпляром `ClassName` або *будь-якого з його підкласів*, оскільки він враховує повний ланцюг успадкування.

# --hints--

Ви повинні викидати `TypeError` з повідомленням `Only Movie or TVSeries instances can be added`, коли `media_item` не є екземпляром `Movie` або його підкласів.
...
Ви не повинні викидати `TypeError`, коли `media_item` є дійсним елементом.
