...
마지막 `.divider` 뒤에 텍스트 `p`를 가진 또 다른 `Trans Fat 0g` 요소를 만드세요. `Trans` 단어는 `i` 요소로 감싸서 이탤릭체로 만드세요. 새 `p` 요소에 `class` 속성을 `indent no-divider` 값으로 지정하세요. `Trans Fat 0g`은 정렬을 위해 `span` 요소로 감싸세요.
...
`span` 요소는 텍스트 `Trans Fat 0g`를 감싸야 합니다.
