...
Elimina la matriz `myTaskArr` y todo el código para `localStorage` porque ya no los necesitas.

# --hints--

Deberías eliminar `myTaskArr` y todo el código relacionado con `localStorage` que acabas de aprender.
...
Deberías eliminar cualquier referencia restante a `myTaskArr` en cualquier parte del código.

```js
assert.notMatch(code, /myTaskArr/);
```

Deberías eliminar cualquier referencia a `localStorage.getItem` para el elemento `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Deberías eliminar cualquier referencia restante a `getTaskArrObj` en cualquier parte del código.
