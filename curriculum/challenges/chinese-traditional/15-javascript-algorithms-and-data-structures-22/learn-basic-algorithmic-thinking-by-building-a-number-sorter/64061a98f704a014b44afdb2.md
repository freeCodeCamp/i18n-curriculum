...
在這個項目中，你將創建一個數字排序器。 HTML和CSS已經爲您準備好了。 請隨意使用。

當你準備好時，聲明一個 `sortButton` 變量並分配給它帶有參數 `"sort"` 的 `.getElementById()` 的值。

# --hints--

你應該用 `const` 聲明一個 `sortButton` 變量。

```js
assert.match(code, /const\s+sortButton\s*=/);
```

你應該調用 `document.getElementById()` 並使用 `"sort"` 作爲參數。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)sort\1\s*\)/);
```

你應該將 `document.getElementById()` 的值分配給 `sortButton`。
