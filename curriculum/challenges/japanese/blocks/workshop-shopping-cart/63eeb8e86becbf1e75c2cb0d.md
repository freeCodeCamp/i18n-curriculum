...
ユーザーがカートに入れている各商品の合計数が必要です。`totalCountPerProduct` 変数を宣言し、空のオブジェクトを代入してください。
...
`const` を宣言するには `totalCountPerProduct` を使うべきです。
...
`totalCountPerProduct` に空のオブジェクトを代入してください。
