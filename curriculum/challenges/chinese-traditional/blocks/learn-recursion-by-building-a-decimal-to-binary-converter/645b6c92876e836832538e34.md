...
接著，將文字 `"Quotients: "`、逗號，以及 `quotients` 陣列記錄到主控台。也將文字 `"Remainders: "`、逗號，以及 `remainders` 陣列記錄到主控台。

# --hints--

你應該將文字 `"Quotients: "` 和 `quotients` 陣列以逗號分隔後記錄到主控台。

```js
assert.match(String(decimalToBinary), /console\.log\(\s*('|"|`)Quotients:\s*\1\s*,\s*quotients\s*\)/i);
```

你應該將文字 `"Remainders: "` 和 `remainders` 陣列以逗號分隔後記錄到主控台。
