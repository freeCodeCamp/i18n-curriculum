...
Ora aggiungi un `div` con il `className` di `card-stats`. Al suo interno, aggiungi un `div` con il `className` di `stat-col`. Dentro quello, aggiungi tre elementi `div` ciascuno con il `className` di `stat-row`.

# --hints--

Dovresti aggiungere un `div` con il `className` di `card-stats` dopo `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip + .card-stats'));
```

Dovresti aggiungere un `div` con il `className` di `stat-col` dentro `.card-stats`.

```js
assert.exists(document.querySelector('.card-stats > .stat-col'));
```

Dovresti aggiungere tre elementi `div` ciascuno con il `className` di `stat-row` dentro `.stat-col`.
