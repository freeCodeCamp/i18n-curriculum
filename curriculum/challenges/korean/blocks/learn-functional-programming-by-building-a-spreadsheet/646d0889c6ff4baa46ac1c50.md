...
`elemValue` 함수 안에 `inner` 매개변수를 받는 `character` 함수를 선언하세요.

그런 다음 `inner` 함수를 반환하세요.
...
`inner` 함수는 `character`를 유일한 매개변수로 가져야 합니다.
