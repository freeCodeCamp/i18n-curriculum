...
Ora è il momento di testare il pulsante di pausa.

Aggiungi un listener per l'evento `"click"` all'elemento `pauseButton`, quindi passa `pauseSong` come secondo argomento del listener per l'evento. Questa è la funzione che il listener eseguirà.

Prova la tua app cliccando prima sul pulsante play e poi sul pulsante pausa. Dovresti vedere che tutto funziona come previsto.

# --hints--

Dovresti chiamare il metodo `addEventListener()` sulla tua variabile `pauseButton`.
...
Dovresti passare `pauseSong` come secondo argomento del tuo metodo `addEventListener`.
