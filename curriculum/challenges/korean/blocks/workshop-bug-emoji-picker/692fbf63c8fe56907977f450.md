...
다음으로, `bugMap`라는 이름의 객체를 `Record<string, Bug<string>>` 타입으로 정의하고, 일단 빈 객체를 할당하세요.

# --hints--

`bugMap`라는 상수가 있어야 합니다.
...
`bugMap`에는 `Record<string, Bug<string>>` 타입 주석이 있어야 합니다.
