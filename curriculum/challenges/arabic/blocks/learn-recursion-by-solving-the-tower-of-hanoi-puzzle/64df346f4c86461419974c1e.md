...
أخيرًا، أضف بند `else` يطبع الحركة المسموح بها ويستدعي `make_allowed_move`. حاول استنتاج المعلمات الصحيحة.
...
يجب أن يحتوي البند `else` على `print(f'Move {i + 1} allowed between {source} and {target}')`.
...
يجب أن يحتوي البند `else` على `make_allowed_move(source, target)`.
