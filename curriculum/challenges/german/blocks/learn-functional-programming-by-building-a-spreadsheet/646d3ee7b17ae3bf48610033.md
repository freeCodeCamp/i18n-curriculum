...
`infixToFunction[operator]` gibt eine Funktion zurück. Rufen Sie diese Funktion direkt auf und übergeben Sie `arg1` und `arg2` als Argumente.

# --hints--

Ihre Callback-Funktion sollte das Ergebnis des Aufrufs von `infixToFunction[operator]` zurückgeben.
...
Sie sollten `arg1` als erstes Argument an Ihren `infixToFunction[operator]`-Aufruf übergeben.
...
Sie sollten `arg2` als zweites Argument an Ihren `infixToFunction[operator]`-Aufruf übergeben.
