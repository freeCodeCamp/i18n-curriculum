...
Crea una terza query `@media` per `only screen` con un `max-width` di `550px`. All’interno, crea un selettore `.hero-title` con una proprietà `font-size` impostata su `6rem`, un selettore `.hero-subtitle, .author, .quote, .list-title` con una proprietà `font-size` impostata su `1.8rem`, un selettore `.social-icons` con una proprietà `font-size` impostata su `2rem` e un selettore `.text` con una proprietà `font-size` impostata su `1.6rem`.

# --hints--

Dovresti avere una nuova query `@media` per `only screen` con un `max-width` di `550px`. Questa dovrebbe venire dopo le tue due precedenti.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText === 'only screen and (max-width: 550px)');
```

La tua nuova regola `@media` dovrebbe avere un selettore `.hero-title`, un selettore `.hero-subtitle, .author, .quote, .list-title`, un selettore `.social-icons` e un selettore `.text`. Questi selettori dovrebbero essere in questo ordine.
...
Il tuo selettore `.hero-title` dovrebbe avere una proprietà `font-size` impostata su `6rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[0]?.style?.fontSize === '6rem');
```

Il tuo selettore `.hero-subtitle, .author, .quote, .list-title` dovrebbe avere una proprietà `font-size` impostata su `1.8rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[1]?.style?.fontSize === '1.8rem');
```

Il tuo selettore `.social-icons` dovrebbe avere una proprietà `font-size` impostata su `2rem`.

```js
assert(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.cssRules?.[2]?.style?.fontSize === '2rem');
```

Il tuo selettore `.text` dovrebbe avere una proprietà `font-size` impostata su `1.6rem`.
