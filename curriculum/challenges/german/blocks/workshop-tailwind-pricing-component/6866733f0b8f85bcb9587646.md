...
Die Premium-Preiskarte sollte diejenige sein, die auf der Seite am meisten hervorsticht. Außerdem ist sie die einzige Karte, die ein `Most popular`-Abzeichen hat. Um diese Karte und ihr Abzeichen hervorzuheben, können Sie die relativen Positionierungs- und Skalierungs-Utility-Klassen von Tailwind verwenden.

So machen Sie ein Element mit Skalierung in Tailwind CSS größer:
...
Die Positionierung übernehmen Sie in den kommenden Schritten. Weisen Sie vorerst dem Wrapper `scale-105` die Klasse `div` zu, damit er etwas größer ist als die anderen Preiskarten.

# --hints--

Ihr Wrapper-`div`-Element für den Premium-Plan sollte die Klasse `scale-105` haben.
