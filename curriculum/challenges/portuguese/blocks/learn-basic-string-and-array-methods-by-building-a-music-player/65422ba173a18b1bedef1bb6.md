...
Dentro da função arrow do event listener, adicione um `if` para verificar se `userData?.currentSong` é `null`.

Dentro do bloco `if`, chame a função `playSong()` com o `id` da primeira música no array `userData?.songs`. Isso garantirá que a primeira música da playlist seja tocada primeiro.
...
Você deve chamar a função `playSong` com `userData?.songs[0].id` dentro do seu bloco `if`.
