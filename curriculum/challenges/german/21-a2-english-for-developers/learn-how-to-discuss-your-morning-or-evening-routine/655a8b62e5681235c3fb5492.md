...
Let's review the usage of `them` and `their`. `Them` wird verwendet, um auf mehrere Personen zu verweisen, die zuvor erwähnt wurden, und `their` zeigt Besitz oder Zugehörigkeit zu diesen Personen. Hier ein Beispiel:

`Ich sah die Kinder. Ich gab ihnen ihre Spielzeuge.` (`them` bezieht sich auf die Kinder `their` zeigt, die Spielzeuge gehören den Kindern)
