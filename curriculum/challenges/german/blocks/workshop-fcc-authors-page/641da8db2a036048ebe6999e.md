...
Setze innerhalb der `fetchMoreAuthors`-Funktion die `startingIndex`- und `endingIndex`-Variablen jeweils auf `+= 8`.
...
Du solltest die `startingIndex`-Variable auf `+=8` setzen.

```js
assert.match(code, /startingIndex\s*\+=\s*8\s*;?/)
```

Du solltest die `endingIndex`-Variable auf `+=8` setzen.
