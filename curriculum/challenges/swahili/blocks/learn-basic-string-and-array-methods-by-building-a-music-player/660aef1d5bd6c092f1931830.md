...
Kama vile vitendakazi vya kawaida, vitendakazi vya mshale vinaweza kurudisha thamani.

Hapa kuna mfano wa kitendakazi cha mshale kinachorudisha matokeo ya kuzidisha nambari mbili:
...
Tengeneza kigezo kipya kinachoitwa `addTwoNumbers` na umweke thamani ya kitendakazi cha mshale. Kitendakazi hiki cha mshale kinapaswa kuchukua vigezo viwili vinavyoitwa `num1` na `num2`. Ndani ya mwili wa kitendakazi, rudisha usemi wa kihesabu wa kuongeza `num1` na `num2`.

Chini ya kitendakazi cha `addTwoNumbers`, ongeza tamko la konsoli. Ndani ya tamko hilo la konsoli, fanya wito wa kitendakazi cha `addTwoNumbers` na upitie nambari `3` na `4` kama hoja.
...
Kitendakazi chako cha `addTwoNumbers` kinapaswa kutumia sintaksia ya kitendakazi cha mshale.

```js
assert.match(code, /(let|const)\s+addTwoNumbers\s*=\s*?\(?.*?\)?\s*=>\s*{\s*[\s\S]*}\s*;?/);
```

Kitendakazi chako cha `addTwoNumbers` kinapaswa kuwa na vigezo viwili vinavyoitwa `num1` na `num2`.

```js
assert.match(addTwoNumbers.toString(), /\(num1,\s*num2\)/);
```

Kitendakazi chako cha `addTwoNumbers` kinapaswa kurudisha usemi wa kihesabu wa kuongeza `num1` na `num2`.

```js
assert.match(addTwoNumbers.toString(), /return\s+num1\s*\+\s*num2/);
```

Unapaswa kuwa na tamko la `console.log(addTwoNumbers(3, 4));`.
