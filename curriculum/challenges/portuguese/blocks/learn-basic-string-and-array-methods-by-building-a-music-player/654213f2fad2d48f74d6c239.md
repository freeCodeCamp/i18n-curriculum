...
Crie outro elemento `button` com a classe `playlist-song-delete` e o atributo `aria-label` definido como `Delete` interpolado com `song.title`.Para o conteúdo do ícone de exclusão, cole o seguinte SVG:
...
Você deve criar um elemento `button` com a classe `playlist-song-delete` como seu primeiro atributo e valor.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*/)
```

Seu elemento button deve ter um atributo `aria-label` com o valor `Delete ${song.title}` como seu segundo atributo e valor.

```js
assert.match(code, /<button\s+class\s*=\s*('|")playlist-song-delete\1\s*aria-label\s*=\s*('|")Delete\s*\$\{song\.title\}\2\s*>/)
```

Você deve colar o `svg` fornecido como o conteúdo do seu elemento `button`.
