...
Ora sostituisci i numeri codificati rimanenti con i seguenti:
...
Dovresti sostituire il valore codificato `99` con `DRI` con `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

Dovresti sostituire il valore codificato `41` con `DEF` con `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

Dovresti sostituire il valore codificato `99` con `PHY` con `player.phy`.
