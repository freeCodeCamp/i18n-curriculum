...
Agora remova as duas últimas chamadas de `print`.

# --hints--

Você não deve ter `print(charlie_brown.name)` no seu código.

```js
({ test: () => assert(runPython(`not _Node(_code).has_stmt("print(charlie_brown.name)")`)) })
```

Você não deve ter `print(charlie_brown.level)` no seu código.
