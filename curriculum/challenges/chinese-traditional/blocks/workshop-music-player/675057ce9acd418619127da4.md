...
現在，創建一個名為 `getPreviousSong` 的函式。讓它傳回在 `userData.songs` 中位於 `getCurrentSongIndex()` 減 1 的歌曲，或者如果目前的歌曲是播放列表中的第一首，則傳回 `undefined`。

# --hints--

你應該有一個名為 `getPreviousSong` 的函式（程式）。

```js
assert.isFunction(getPreviousSong);
```

你的 `getPreviousSong` 函式（程式）應該呼叫 `getCurrentSongIndex`。
...
你的 `getPreviousSong` 函式應該傳回目前歌曲之前的歌曲。
...
當目前歌曲是播放列表中的第一首時，你的 `getPreviousSong` 函式應該傳回 `undefined`。
