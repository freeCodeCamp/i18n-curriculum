...
Como hiciste para el `multipleCard` anterior, obtén nuevamente el elemento `multipleCard` y establece su `innerHTML` iterando a través de `DrawingType`. Usa `Object.values()` y llama al método `map` sobre él, pasando `type` como argumento.

# --hints--

Debes asignar `multipleCard.innerHTML` llamando a `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /this\.elements\.multipleCard\.innerHTML\s*=\s*Object\.values\s*\(/);
```

Debes pasar `DrawingType` al método `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)/);
```

Debes llamar al método `map` sobre `Object.values`.

```js
assert.match(__helpers.removeJSComments(code), /Object\.values\s*\(\s*DrawingType\s*\)\.map\s*\(/);
```

Tu método `map` debe tener un parámetro llamado `type`.
