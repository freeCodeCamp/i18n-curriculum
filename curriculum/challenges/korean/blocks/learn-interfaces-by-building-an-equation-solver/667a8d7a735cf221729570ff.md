...
방정식이 이차방정식일 때를 위해 또 다른 `case`를 추가하세요. `analyze`의 `QuadraticEquation` 메서드가 반환하는 것과 같은 형식의 사전을 사용하세요.

그런 다음 `details_list`에 각각 `concavity = <concavity>`와 `<min_max> = (<x>, <y>)` 형식의 문자열 두 개를 포함하는 리스트를 할당하세요. `<x>`와 `<y>`를 `3` 10진수 숫자를 표시하도록 포맷하세요.

마지막으로 `match`/`case` 블록 뒤에서 `details_list`를 순차적으로 반복하며 각 항목을 `output_string`의 현재 값에 추가하세요. 각 문자열 항목이 줄바꿈 문자로 끝나도록 하세요. 여기서는 추가 포맷 옵션을 사용하지 마세요.

# --hints--

`match` 문장의 주제 값을 수정하지 마세요.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[1].find_match_subject().is_equivalent("details")`)) })
```

기존 `case` 블록을 수정하지 마세요.
