...
각 포럼 게시물에는 해당 주제에 참여하는 모든 사용자를 나타내는 사용자 아바타 이미지 목록이 포함됩니다.

두 개의 매개변수 `avatars`과 `posters`를 가진 `users`이라는 화살표 함수를 만드는 것부터 시작하세요.

# --hints--

빈 화살표 함수 `avatars`가 있어야 합니다.

```js
assert.match(code, /const\s+avatars\s*=\s*\([\s\S]*\)\s*=>\s*{\s*}/);
```

함수 `posters`에는 `users`와 `avatars`라는 이름의 매개변수가 있어야 합니다.
