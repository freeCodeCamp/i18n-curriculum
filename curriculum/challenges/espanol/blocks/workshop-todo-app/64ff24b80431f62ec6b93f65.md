...
Elimina la matriz `myTaskArr` y todo el código de `localStorage` porque ya no los necesitas.

# --hints--

Debes eliminar `myTaskArr` y todo el código relacionado con `localStorage` que acabas de aprender.
...
Debes eliminar cualquier referencia restante a `myTaskArr` en cualquier parte del código.

```js
assert.notMatch(code, /myTaskArr/);
```

Debes eliminar cualquier referencia a `localStorage.getItem` para el ítem `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Debes eliminar cualquier referencia restante a `getTaskArrObj` en cualquier parte del código.
