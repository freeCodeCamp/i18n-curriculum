...
بعد ذلك، أنشئ دالة `loadPlayer` مع نوع الإرجاع `PlayerData`. داخلها، أضف صيغة نحوية `try...catch` مع `error` كمعلمة لكتلة `catch`. اترك كل شيء آخر فارغًا في الوقت الحالي.

# --hints--

يجب أن تنشئ دالة تسمى `loadPlayer`.
...
يجب أن تحتوي دالة `loadPlayer` على نوع إرجاع `PlayerData`.
...
يجب أن تحتوي دالة `loadPlayer` على كتلة `try...catch`.
