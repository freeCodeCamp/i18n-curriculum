...
正如你在 `deleteTask` 函数中所做的那样，你需要找到要编辑的任务的索引。

创建一个 `dataArrIndex` 变量。 要获得其值，请在 `taskData` 上使用 `findIndex()` 方法。 将 `item` 作为参数传递给其回调函数，并检查 `item` 的 `id` 是否等于 `buttonEl` 的 `parentElement` 的 `id`。
