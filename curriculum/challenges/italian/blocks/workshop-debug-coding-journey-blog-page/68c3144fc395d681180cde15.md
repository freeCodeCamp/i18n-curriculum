...
Sembra che ci siano ancora alcuni problemi con l'uso delle intestazioni nella pagina. Se guardi il primo elemento `div`, ci sono due elementi `h4` usati come sottointestazioni. Una pratica migliore sarebbe usare elementi `h3` per queste sottointestazioni, dato che sono un livello sotto l'elemento `h2`.

Cambia i due elementi `h4` in elementi `h3`.

# --hints--

Il tuo elemento `h4` con il testo `Early Challenges` dovrebbe essere un elemento `h3`.
...
Il tuo elemento `h4` con il testo `Breakthroughs` dovrebbe essere un elemento `h3`.
