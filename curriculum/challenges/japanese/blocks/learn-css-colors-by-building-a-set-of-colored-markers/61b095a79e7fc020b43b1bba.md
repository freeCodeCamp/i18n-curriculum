...
まだグラデーションは表示されません。なぜなら、`linear-gradient` 関数は動作するために少なくとも2つの色の引数が必要だからです。

同じ`linear-gradient` 関数内で、`rgb` 関数を使って2番目の色の引数を純粋な緑に設定してください。

# --hints--

`.red` のCSSルールには、`background` プロパティが`linear-gradient(90deg, rgb(255, 0, 0), rgb(0, 255, 0))` に設定されている必要があります。
