...
これで`attack`関数を作成できます。まず、`text`メッセージを更新して`"The <monster name> attacks."`と表示し、`<monster name>`をモンスターの名前に置き換えてください。連結演算子を使うことができます。

# --hints--

`innerText`の`text`プロパティにドット記法でアクセスしてください。

```js
assert.match(attack.toString(), /text\.innerText/);
```

`"The "`の`innerText`プロパティに文字列`text`を代入してください。

```js
assert.match(attack.toString(), /text\.innerText\s*=\s*('|")The \1/);
```

連結演算子を使って、文字列`monsters[fighting].name`に`"The "`の値を追加してください。

```js
assert.match(attack.toString(), /text\.innerText\s*=\s*('|")The \1\s*\+\s*monsters\s*\[\s*fighting\s*\]\s*\.name/);
```

連結演算子を使って、文字列`" attacks."`に文字列`monsters[fighting].name`を追加してください。
