...
في هذه الورشة، ستواصل ممارسة العمل مع TypeScript من خلال بناء تطبيق للتنبؤ بالمستقبل.

ابدأ بإعلان واجهة باسم `Card`. داخل واجهة `Card`، أضف الخصائص التالية: `name`، `name_short` و`value`. يجب أن تكون أنواع خاصيتي `name` و`name_short` من نوع `string`، ويجب أن يكون نوع خاصية `value` اتحادًا بين `string | number`.

# --hints--

يجب إعلان واجهة باسم `Card`.
...
يجب أن تحتوي واجهة `Card` على خاصية `name` من نوع `string`.
...
يجب أن تحتوي واجهة `Card` على خاصية `name_short` من نوع `string`.
...
يجب أن تحتوي واجهة `Card` على خاصية `value` بنوع اتحاد بين `string` و`number`.
