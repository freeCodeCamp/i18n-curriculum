...
`range()` restituirà un array di numeri, che devi convertire di nuovo in caratteri. Collega il metodo `.map()` alla tua chiamata `range()`.

Passa una funzione di callback che prende `code` come parametro e restituisce implicitamente il valore di passare `code` al metodo `String.fromCharCode()`.
...
Dovresti concatenare il metodo `.map()` alla tua chiamata `range`.
...
Dovresti usare la sintassi arrow per la callback `.map()`.
...
La tua callback `.map()` dovrebbe prendere un parametro `code`.
...
La tua callback `.map()` dovrebbe usare un ritorno implicito.
...
La tua callback `.map()` dovrebbe restituire il risultato della chiamata `String.fromCharCode()`.
