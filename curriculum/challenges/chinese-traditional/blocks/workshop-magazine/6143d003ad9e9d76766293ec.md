...
在你的 `.image-quote` 元素內，巢狀一個 `hr` 元素、一個 `p` 元素和第二個 `hr` 元素。為 `p` 元素設定 `class` 為 `quote`，並加入文字 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`。

# --hints--

你應該為你的 `.image-quote` 元素添加兩個 `hr` 元素。

```js
assert.lengthOf(document.querySelectorAll('.image-quote hr'), 2);
```

你應該為你的 `.image-quote` 元素添加一個 `p` 元素。

```js
assert.lengthOf(document.querySelectorAll('.image-quote p'), 1);
```

你的 `.image-quote` 子元素應該是正確的順序。
...
你的新 `p` 元素應該設定 `class` 為 `quote`。

```js
assert.isTrue(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

你的新 `p` 元素應該包含文字 `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`。
