...
Para llamar a una expresión de función flecha nombrada, puedes referenciar la función por su nombre. 

```js
exampleArrowFunction();
```

Debajo de tu función `printGreeting` definición, llama a la función. Abre la consola para ver el resultado.

# --hints--

Deberías llamar a tu función `printGreeting`.

```js
assert.match(code, /printGreeting\(\);?/);
```

La llamada a tu función `printGreeting` debería estar debajo de la definición de la función `printGreeting`.

```js
assert.isBelow(code.indexOf("const printGreeting = () => {"), code.indexOf("printGreeting()"));
```

La llamada a tu función `printGreeting` debería mostrar `Hello there!` en la consola. 
