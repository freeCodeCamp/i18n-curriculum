...
Encadeie o método `.then()` à chamada de `fetch`. Dentro do método `.then()`, adicione uma função de callback com `res` como parâmetro. Em seguida, registre `res` no console usando console.log() para poder ver o objeto `Response`. Open your browser console and expand the `Response` object to see what it contains.
