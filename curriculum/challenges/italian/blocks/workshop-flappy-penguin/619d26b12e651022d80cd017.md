...
Dovresti dare a `.arm` una proprietà `width` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.width, '30%');
```

Dovresti dare a `.arm` una proprietà `height` con il valore `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.arm')?.height, '60%');
```

Dovresti dare a `.arm` una proprietà `background` con il valore `linear-gradient(90deg, gray, rgb(209, 210, 199))`.
