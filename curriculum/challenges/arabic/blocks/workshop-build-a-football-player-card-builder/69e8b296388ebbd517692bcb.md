...
يُعرض `FootballPlayerCard` حاليًا البطاقة الأساسية فقط. لبناء صفحة كاملة، أضف عنصر `div` مع `className` باسم `page`.

داخل ذلك الـ `div`، أضف عنصر `header` مع `className` باسم `header`. داخل الـ `header`، أضف `div` مع `className` باسم `header-inner`. داخل عنصر `header-inner`، أضف فقرتين نصيتين هما `Football Card Builder` و`Customize your player card` على التوالي. يجب أن تحمل الفقرتان أيضًا `className` باسم `header-title` و`header-subtitle` لكل منهما.

بعد إغلاق عنصر الـ `header` ولكن لا تزال داخل الـ `div` الذي يحمل `className` باسم `page`، أدرج مكوِّن `PlayerCard` مع خاصيته كعنصر نائب مؤقت حتى يُضاف التخطيط الرئيسي.

# --hints--

يجب أن يعرض `FootballPlayerCard` عنصر `div` مع `className` باسم `page`.
...
يجب أن يحتوي الـ `div` الذي يحمل `className` باسم `page` على عنصر `header` مع `className` باسم `header`.
...
يجب أن يحتوي الـ `header` على عنصر `div` مع `className` باسم `header-inner`.
...
يجب أن يحتوي عنصر `header-inner` على فقرة `p` مع `className` باسم `header-title` والنص `Football Card Builder`.
...
يجب أن يحتوي عنصر `header-inner` على فقرة `p` مع `className` باسم `header-subtitle` والنص `Customize your player card`.
...
يجب عرض مكوِّن `PlayerCard` داخل `div.page`.
