...
ハードコードされた `99` の値を `DRI` から `player.dri` に置き換えてください。

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

ハードコードされた `41` の値を `DEF` から `player.def` に置き換えてください。

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

ハードコードされた `99` の値を `PHY` から `player.phy` に置き換えてください。
