...
Du solltest eine Funktions<dfn>referenz</dfn> als Callback-Parameter übergeben. Eine Funktionsreferenz ist ein Funktionsname ohne Klammern. Hier ein Beispiel:
...
Die `.map()`-Methode wird hier die `myFunc`-Funktion aufrufen und die gleichen Argumente übergeben, die ein `.map()`-Callback verwendet. Das erste Argument ist der Wert des Arrays bei der aktuellen Iteration, `newArray` wäre also `[value: 1, value: 2, value: 3]`.

Übergib einen Verweis auf deine `elemValue`-Funktion als Callback für deine `.map()`-Methode.

# --hints--

Du solltest deine `elemValue`-Funktion nicht aufrufen.
...
Du solltest deiner `.map()`-Methode einen Verweis auf `elemValue` als Callback übergeben.
