...
将右内耳移动到位，将其 `position` 属性设置为 `absolute`，`top` 为 `22px`，`left` 为 `-20px`。

# --hints--

你的 `.cat-right-inner-ear` 选择器应设置 `position` 属性为 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.position, 'absolute')
```

你的 `.cat-right-inner-ear` 选择器应设置 `top` 属性为 `22px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.top, '22px')
```

你的 `.cat-right-inner-ear` 选择器应设置 `left` 属性为 `-20px`。
