...
Sinta-se à vontade para alterar os coeficientes do seu `lin_eq` para ver como a saída muda.

Então, exclua sua chamada `print(solver(lin_eq))` e imprima o resultado da chamada `solver()` com `quadr_eq` como argumento.

# --hints--

Você não deve ter `print(solver(lin_eq))` no seu código.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

Você deve imprimir `solver(quadr_eq)`.
