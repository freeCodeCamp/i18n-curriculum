...
غلف الآن المكوِّن `PlayerCard` بعنصر `main` يحمل `className` بقيمة `main`.

داخل عنصر `main`، أضف `div` يحمل `className` بقيمة `layout`. داخل `layout`، أضف عنصرين منفصلين من نوع `div` يحملان `className` بقيم `form-panel` و `preview-panel` على التوالي.

# --hints--

يجب أن يعرض مكوِّن `FootballPlayerCard` عنصر `main` يحمل `className` بقيمة `main` داخل `div.page`.
...
يجب أن يحتوي `main.main` على عنصر `div` يحمل `className` بقيمة `layout`.
...
يجب أن يحتوي `layout` على عنصر `div` يحمل `className` بقيمة `form-panel`.
...
يجب أن يحتوي `layout` على عنصر `div` يحمل `className` بقيمة `preview-panel`.
