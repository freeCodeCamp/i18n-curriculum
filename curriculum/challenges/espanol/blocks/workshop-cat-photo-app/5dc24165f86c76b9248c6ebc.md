...
Tu código debe tener un elemento `img`. Eliminaste el elemento `img` de un paso anterior.
...
El valor del atributo `alt` de tu elemento `img` está configurado en algo distinto a `A cute orange cat lying on its back`. Asegúrate de que el valor del atributo `alt` esté entre comillas.
