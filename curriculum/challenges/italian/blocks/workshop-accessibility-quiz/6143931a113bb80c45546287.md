...
Dovresti dare al primo elemento `div` la classe `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[0]?.className, 'question-block');
```

Dovresti dare al secondo elemento `div` la classe `question-block`.
...
Dovresti dare al primo elemento `fieldset` la classe `question`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > fieldset')?.[0]?.className, 'question');
```

Dovresti dare al secondo elemento `fieldset` la classe `question`.
