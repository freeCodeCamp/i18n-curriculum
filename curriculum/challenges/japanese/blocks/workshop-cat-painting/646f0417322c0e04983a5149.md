...
クラスセレクターを使用して、`.cat-mouth-line-left` の要素の `position` を `absolute` に、`top` を `88px` に、`left` を `74px` に設定してください。 これで適切な位置に動かすことができます。

# --hints--

`.cat-mouth-line-left` セレクターが必要です。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left'));
```

`.cat-mouth-line-left` セレクターの `position` プロパティを `absolute` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.position, 'absolute');
```

`.cat-mouth-line-left` セレクターの `top` プロパティを `88px` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-mouth-line-left')?.top, '88px');
```

`.cat-mouth-line-left` セレクターの `left` プロパティを `74px` に設定してください。
