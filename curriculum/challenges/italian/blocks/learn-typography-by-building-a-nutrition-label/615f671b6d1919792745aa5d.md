...
Crea un nuovo elemento `div` sotto il tuo elemento `.medium`. Dagli un attributo `class` impostato su `daily-value small-text`. All’interno di questo nuovo `div`, aggiungi un elemento `p` con il testo `% Daily Value *` e imposta l’attributo `class` su `bold right`.

# --hints--

Dovresti creare un nuovo elemento `div` dopo il tuo elemento `.medium`.
...
Il tuo nuovo `div` dovrebbe avere un attributo `class` impostato su `daily-value small-text`.
...
Il tuo nuovo elemento `p` dovrebbe avere un attributo `class` impostato su `bold right`.
