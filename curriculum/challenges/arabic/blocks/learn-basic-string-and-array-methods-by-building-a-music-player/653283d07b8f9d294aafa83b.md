...
حان الوقت لبدء تنفيذ الوظيفة الخاصة بتشغيل الأغاني المعروضة.

عرّف دالة `playSong` باستخدام `const`. يجب أن تأخذ الدالة معلمة `id` التي تمثل المعرف الفريد للأغنية التي تريد تشغيلها.

# --hints--

استخدم `const` لإنشاء دالة فارغة باسم `playSong`.

```js
assert.match(code, /const\s+playSong\s*=\s*/)
```

يجب أن تكون `playSong` دالة.

```js
assert.isFunction(playSong)
```

يجب أن تستخدم دالة `playSong` صيغة السهم.

```js
assert.match(code, /const\s+playSong\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/)
```

يجب أن تأخذ دالة `playSong` معلمة `id`.

```js
assert.match(code, /const\s+playSong\s*=\s*\(?\s*id/)
```

يجب أن تكون دالة `playSong` فارغة.
