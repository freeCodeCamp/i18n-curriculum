...
نفذ طريقة `is_applicable` في `PremiumUserDiscount`. يجب أن تتحقق هذه الطريقة مما إذا كان المستخدم عضوًا مميزًا.

أرجع `True` إذا كانت قيمة `user_tier.lower()` تساوي `'premium'`، وإلا أرجع `False`. استخدام `.lower()` يضمن أن يكون الفحص غير حساس لحالة الأحرف.

# --hints--

يجب أن يحتوي صنف `PremiumUserDiscount` على طريقة `is_applicable`.
...
يجب أن ترجع طريقة `is_applicable` القيمة `True` عندما تكون قيمة `user_tier.lower()` تساوي `'premium'`.
...
يجب أن ترجع طريقة `is_applicable` القيمة `False` عندما لا تكون قيمة `user_tier` هي `'premium'`.
