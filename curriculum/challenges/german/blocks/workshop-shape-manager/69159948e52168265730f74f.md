...
Sie benötigen eine Möglichkeit, sicher auf das select-Element zuzugreifen und `null`-Werte abzusichern. Erstellen Sie dafür eine `val`-Konstante mit dem Wert `e.currentTarget` und typisieren Sie sie als `HTMLSelectElement` mit dem Schlüsselwort `as`.

Unterhalb der Konstante verwenden Sie eine `if`-Anweisung, um zu prüfen, ob `val` falsch ist, indem Sie es negieren. Im `if`-Block geben Sie `target value not found` zurück.

# --hints--

Sie sollten eine Konstante namens `val` haben.

```js
assert.match(code, /const\s+val/)
```

Ihre `val`-Konstante sollte einen `e.currentTarget`-Wert haben, der als `HTMLSelectElement` mit dem Schlüsselwort `as` typisiert ist.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

Sie sollten eine `if`-Anweisung haben, die prüft, ob `val` null ist.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

Ihre `if`-Anweisung sollte `target value not found` zurückgeben.
