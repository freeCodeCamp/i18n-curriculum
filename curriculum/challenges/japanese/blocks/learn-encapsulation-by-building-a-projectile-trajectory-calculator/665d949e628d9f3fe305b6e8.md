...
出力がかなり読みづらくなっているので、`print(graph.create_trajectory())` を各行を別々に出力するループに置き換えてください。

# --hints--

`print(graph.create_trajectory())` の呼び出しを削除してください。
...
`graph.create_trajectory()` を反復するループを作成してください。
...
`graph.create_trajectory()` の各行を出力してください。
