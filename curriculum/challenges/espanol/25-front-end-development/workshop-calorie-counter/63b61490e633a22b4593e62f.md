...
Siguiendo el mismo patrón, asigna tu elemento `#add-entry` a `addEntryButton`, tu elemento `#clear` a `clearButton`, y tu elemento `#output` a `output`.
...
Debes usar `document.getElementById()` para obtener el elemento `#add-entry`.

```js
assert.match(code, /document\.getElementById\(\s*('|")add-entry\1\s*\)/g);
```

Debes asignar el elemento `#add-entry` a `addEntryButton`.
...
Debes usar `document.getElementById()` para obtener el elemento `#clear`.

```js
assert.match(code, /document\.getElementById\(\s*('|")clear\1\s*\)/g);
```

Debes asignar el elemento `#clear` a `clearButton`.
...
Debes usar `document.getElementById()` para obtener el elemento `#output`.

```js
assert.match(code, /document\.getElementById\(\s*('|")output\1\s*\)/g);
```

Debes asignar el elemento `#output` a `output`.
