...
También debes trabajar en una función que limpie los campos de entrada.

Crea una función `clearInputFields`. Dentro de ella, usa `querySelectorAll` para obtener todos los campos de entrada y recórrelos usando `forEach()`. Dentro del `forEach()`, asigna a cada `input` un valor de cadena vacía.

# --hints--

Debes crear una función `clearInputFields`.

```js
assert.isFunction(clearInputFields)
```

Tu función `clearInputFields` debe asignar a cada campo de entrada una cadena vacía.
