...
الخطوة التالية هي إيجاد المستخدم الصحيح في مصفوفة `users`.

ابدأ بإنشاء ثابت يسمى `user` وقم بتعيينه إلى `users.find()`. يجب أن تحتوي طريقة `find` على دالة استدعاء راجع بمعلمة `user`.

داخل دالة الاستدعاء الراجعة لطريقة `find`، أعد ضمنيًا نتيجة التحقق مما إذا كان `user.id` يساوي بدقة `poster.user_id`.

# --hints--

يجب إنشاء ثابت باسم `user`.

```js
assert(code.match(/\s*const\s+user\s*=/));
```

يجب تعيين `users.find()` إلى الثابت `user` الخاص بك.

```js
assert(code.match(/\s*const\s+user\s*=\s*users\.find\(/g));
```

يجب أن تحتوي دالة الاستدعاء الراجعة على معلمة تسمى `user`.

```js
assert(code.match(/\s*const\s+user\s*=\s*users\.find\(\s*(\(\s*user\s*\)|user)\s*=>\s*/g));
```

يجب أن تعيد ضمنيًا تحقق `user.id` من المساواة الصارمة مع `poster.user_id`.
