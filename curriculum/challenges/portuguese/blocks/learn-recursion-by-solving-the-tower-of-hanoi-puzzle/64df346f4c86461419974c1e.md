...
Finalmente, adicione uma cláusula `else` que imprima o movimento permitido e chame `make_allowed_move`. Tente descobrir os argumentos corretos.
...
Você deve ter `print(f'Move {i + 1} allowed between {source} and {target}')` dentro do bloco `else`.
...
Você deve ter `make_allowed_move(source, target)` dentro do bloco `else`.
