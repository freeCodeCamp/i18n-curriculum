...
Agrega el evento `DOMContentLoaded` al documento raíz y pasa `initializeApp` como la función callback que se ejecutará. Esto asegura que toda la inicialización ocurra solo después de que el DOM esté completamente cargado.

# --hints--

Debes agregar el evento `DOMContentLoaded` al `document` raíz.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

Debes referenciar `initializeApp` como la función que se ejecutará en tu listener de eventos.
