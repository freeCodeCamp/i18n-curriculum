...
Ndani ya kipengele cha `button`, tengeneza vipengele viwili zaidi vya `span`. 

Kipengele cha kwanza cha `span` kinapaswa kuwa na `class` ya `playlist-song-artist`. Kati ya lebo za `span`, ongeza `${song.artist}`. 

Kipengele cha pili cha `span` kinapaswa kuwa na `class` ya `playlist-song-duration`. Kati ya lebo za `span`, ongeza `${song.duration}`. 

# --hints--

Haupasi kubadilisha kipengele cha `button` kilichopo na yaliyomo ndani yake.
...
Unapaswa kutengeneza kipengele cha `span` chenye darasa la vitu la `playlist-song-artist`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>/)
```

Kipengele chako cha `span` chenye darasa la vitu la `playlist-song-artist` kinapaswa kuingiza `song.artist` kama yaliyomo ndani yake.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-artist\1\s*>\s*\$\{song\.artist\}\s*<\/span>/)
```

Unapaswa kutengeneza kipengele cha `span` chenye darasa la vitu la `playlist-song-duration`.

```js
assert.match(code, /<span\s*class\s*=\s*('|")playlist-song-duration\1\s*>\s*/)
```

Kipengele chako cha `span` chenye darasa la vitu la `playlist-song-duration` kinapaswa kuingiza `song.duration` kama yaliyomo ndani yake.
