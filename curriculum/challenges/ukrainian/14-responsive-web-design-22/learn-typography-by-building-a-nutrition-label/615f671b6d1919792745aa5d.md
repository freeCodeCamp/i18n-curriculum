...
Створіть новий елемент `div` знизу елемента `.medium`. Надайте йому атрибут `class` зі значенням `daily-value small-text`. Додайте елемент `p` з текстом `% Daily Value *` до нового `div` та встановіть атрибут `class` на `bold right`.
