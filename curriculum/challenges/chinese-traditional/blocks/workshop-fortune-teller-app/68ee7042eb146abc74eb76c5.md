...
你需要讓卡片隨機且反向。宣告一個名為 `isReversed` 和 `card` 的變數。對第一個，呼叫 `Math.random` 方法並與 `< 0.5` 比較。對第二個，呼叫 `getRandomItem` 並傳入 `this.cards`。

# --hints--

你應該宣告一個名為 `isReversed` 的變數。
...
你的 `isReversed` 變數應該使用 `Math.random()` 來隨機決定卡片是否反向。
...
你的 `card` 變數應該透過呼叫 `getRandomItem` 方法並以 `this.cards` 作為引數來指定。
