...
Отримайте елемент `select` з ідентифікатором `species` з DOM і збережіть його у константі з ім’ям `selectElement`. Використайте оператор твердження не-null `!`, щоб стверджувати, що елемент не є `null`.

# --hints--

У вас має бути константа з ім’ям `selectElement`.
...
Ваш `selectElement` має виконувати запит елемента з ідентифікатором `species` за допомогою `document.querySelector` з типом аргументу `HTMLSelectElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

Ваш `selectElement` має використовувати оператор твердження не-null (`!`).
