...
標題元素（例如 `h1`、`h2`）的排版是由使用者瀏覽器的預設值設定的。

為你的樣式表添加兩個新的型別選擇器（`h1` 和 `h2`）。對兩者都使用 `font-size` 屬性，但對 `h1` 使用值 `40px`，對 `h2` 使用值 `30px`。
