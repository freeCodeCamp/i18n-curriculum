...
Debes usar el selector `li > a` existente para establecer `text-decoration` a `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

Debes usar el selector `nav > ul > li:hover` o el `nav li:hover` para estilizar los elementos al pasar por encima de ellos.
...
Debes dar a los elementos `li` que son sobrevolados un `background-color` de `#dfdfe2`.
...
Debes dar a los elementos `li` que son sobrevolados un `color` de `#1b1b32`.
...
Debes dar a los elementos `li` que son sobrevolados un `cursor` de `pointer`.
