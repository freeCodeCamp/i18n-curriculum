...
在你的 `defeatMonster` 函數中，將 `gold` 設置爲等於 `gold` 加上怪物等級乘以 `6.7`。 請記住，你可以使用 `monsters[fighting].level` 獲取怪物的等級。

下面是將 `num` 設置爲 `num` + `5` * `8` 的示例：`num += 5 * 8`。 使用 `Math.floor()` 將結果向下舍入。
...
你應該使用 `Math.floor()` 來將怪物等級乘以 `6.7` 的結果四捨五入。
