...
返回到你的 `attack` 函數 - 在 `else if` 塊內，創建另一個 `if` 和 `else` 語句。 如果玩家正在與龍戰鬥（`fighting` 將是 `2`），則調用 `winGame` 函數。 將 `defeatMonster()` 調用移至 `else` 塊。

對於這一步，你需要使用嚴格相等（`===`）運算符來檢查 `fighting` 是否等於 `2`。


# --hints--

你應該在 `else if` 塊中添加一個新的 `if` 語句。
...
你的新 `if` 語句應該檢查 `fighting` 是否嚴格等於 `2`。
...
你的 `else` 塊應該調用 `defeatMonster` 函數。
