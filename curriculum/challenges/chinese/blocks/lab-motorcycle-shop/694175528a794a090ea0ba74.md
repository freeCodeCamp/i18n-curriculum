...
在本实验中，你已经获得了所有的超文本标记语言和 CSS。你将使用 TypeScript 来完成摩托车商店页面的功能。

**目标：** 完成以下用户故事并通过所有测试以完成实验。

**用户故事：**

1. 你应该创建一个名为 `type` 的 `Category`，它包含以下可能的值：`'Sport'`、`'Cruiser'`、`'Touring'`、`'Dirt'`、`'Adventure'`、`'Naked'`、`'Electric'`。
2. 你应该创建一个名为 `interface` 的 `Motorcycle`，它具有以下属性：一个类型为 `id` 的 `string`，一个类型为 `name` 的 `string`，一个类型为 `manufacturer` 的 `string`，一个类型为 `category` 的 `Category`，一个类型为 `price` 的 `number`，一个类型为 `image_url` 的 `string`，一个类型为 `created_at` 的 `Date` 属性，一个类型为 `description` 的 `string`，以及一个类型为 `number` 的 year。
3. 你应该创建一个名为 `fetchMotorcycles` 的函数，它返回一个解析为 `Promise` 对象数组的 `Motorcycle`。
4. 你应该从 `Motorcycle` 端点装载 `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json` 数据。
5. 你应该创建一个名为 `renderMotorcycleCard` 的函数，它接受一个单一的 `Motorcycle` 对象作为参数，并返回一个包含摩托车卡片格式化超文本标记语言字串的字符串。
   1. `renderMotorCycleCard` 函数应接受一个名为 `motorcycle`、类型为 `Motorcycle` 的参数。
   2. `renderMotorCycleCard` 函数应返回 `string` 类型。
   3. 每个摩托车卡片组件应有一个 `img` 元素，包含有效图像且类名为 `motorcycle-card-image-container`。
   4. 每个摩托车卡片组件应有一个类名为 `motorcycle-card-year-badge` 的元素，以纯文本形式列出年份。
   5. 每个摩托车卡片组件应有一个类名为 `motorcycle-card-title` 的元素，以纯文本形式列出摩托车名称。
   6. 每个摩托车卡片组件应有一个类名为 `motorcycle-card-manufacturer` 的元素，以纯文本形式列出摩托车制造商名称。
   7. 每个摩托车卡片组件应有一个类名为 `motorcycle-card-category` 的元素，以纯文本形式列出摩托车类别。
   8. 每个摩托车卡片组件应有一个类名为 `motorcycle-card-description` 的元素，以纯文本形式列出摩托车描述。
   9. 每个摩托车卡片组件应有一个类名为 `motorcycle-card-price` 的元素，以纯文本形式列出摩托车价格。
   10. 每个摩托车卡片组件应有一个类名为 `motorcycle-card-engine` 的元素，列出每个引擎的马力。
6. 你应该有一个名为 `MotorcycleGalleryApp` 的类。
   1. 该类应有一个名为 `private` 的 `allMotorcycles` 数组，静态类型为 `Motorcycle` 对象数组类型。
   2. 该类应有一个名为 `renderMotorcycles` 的函数，在 id 为 `allMotorcycles` 的元素内渲染 `motorcycle-grid` 变量对应的摩托车卡片组件。
7. 显示的 `Motorcycle` 元素数量应列在 id 为 `results-number` 的元素内。
8. 过滤器应在 `input` 事件上更新。

# --hints--

你应该有一个名为 `type` 的 `Category`。
...
你应该有一个名为 `interface` 的 `Motorcycle`。
...
你应该有一个名为 `fetchMotorcycles` 的函数。

```js
assert.isFunction(fetchMotorcycles);
```

你应该使用 `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json` 端点。
...
你应该从 `Motorcycle` 返回一个包含 25 个 `fetchMotorcycles` 对象的承诺。
...
你应该有一个名为 `renderMotorcycleCard` 的函数。

```js
assert.isFunction(renderMotorcycleCard);
```

`renderMotorcycle` 函数应接受一个静态类型为 `Motorcycle` 的参数。
...
`renderMotorcycleCard` 函数应显式返回 `string` 类型。
...
`renderMotorcycleCard` 函数应返回 `string`。
...
你应该有一个名为 `MotorcycleGalleryApp` 的类。
...
`MotorcycleGalleryApp` 应有一个名为 `allMotorcycles` 的数组。
...
`allMotorcycles` 属性应静态类型为 `Motorcycle[]`。
...
`MotorcycleGalleryApp` 应有一个名为 `renderMotorcycles` 的函数。
...
`renderMotorcycles` 函数应渲染 25 个类名为 `motorcycle-card-image-container` 的元素。
...
`renderMotorcycles` 函数应渲染 25 个类名为 `motorcycle-card-year-badge` 的元素。
...
`renderMotorcycles` 函数应渲染 25 个类名为 `motorcycle-card-title` 的元素。
...
`renderMotorcycles` 函数应渲染 25 个类名为 `motorcycle-card-manufacturer` 的元素。
...
`renderMotorcycles` 函数应渲染 25 个类名为 `motorcycle-card-category` 的元素。
...
`renderMotorcycles` 函数应渲染 25 个类名为 `motorcycle-card-description` 的元素。
...
`renderMotorcycles` 函数应渲染 25 个类名为 `motorcycle-card-price` 的元素。
...
`renderMotorcycles` 函数应渲染 25 个类名为 `motorcycle-card-engine` 的元素。
