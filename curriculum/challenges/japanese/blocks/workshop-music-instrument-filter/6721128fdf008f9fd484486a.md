...
残りの木管楽器であるクラリネットとオーボエを表すために、`instrumentsArr` にさらに2つのオブジェクトを追加してください。

# --hints--

`{ category: "woodwinds", instrument: "Clarinet", price: 200 }` の中に `instrumentsArr` があるはずです。

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

`{ category: "woodwinds", instrument: "Oboe", price: 4000 }` の中に `instrumentsArr` があるはずです。

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

`{ category: "woodwinds", instrument: "Flute", price: 500 }` の中にまだ `instrumentsArr` があるはずです。
