...
你需要知道模擬應該執行多少個刻度。回想一下：

- 每個閘門都有一個 `queue` 陣列，顯示每個刻度有多少參加者抵達。  
- 由於所有閘門的刻度數量相同，你可以直接使用第一個閘門佇列的長度。  

創建一個名為 `maxTicks` 的變數，並將 `gates[0].queue.length` 的值指定給它。

這個值將用來控制你的模擬迴圈，確保每個刻度都會被所有閘門處理。 

# --hints--

你應該創建一個名為 `maxTicks` 的變數，並將 `gates[0].queue.length` 的值指定給它。
