...
Fügen Sie am Anfang Ihres Setters eine `if`-Anweisung hinzu, die eine `TypeError` mit der Nachricht `'salary' must be a number.` auslöst, wenn `new_salary` weder eine Ganzzahl noch eine Fließkommazahl ist.

# --hints--

Sie sollten eine `if`-Anweisung in Ihrem `salary`-Setter haben.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[0]`)) })
```

Sie sollten eine `TypeError` mit der Nachricht `'salary' must be a number.` auslösen, wenn `new_salary` weder eine Ganzzahl noch eine Fließkommazahl ist.
...
Sie sollten keine Ausnahme auslösen, wenn `new_salary` entweder eine Ganzzahl oder eine Fließkommazahl ist.
