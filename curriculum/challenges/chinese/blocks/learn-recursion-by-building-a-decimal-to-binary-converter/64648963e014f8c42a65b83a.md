...
向 `animationData` 数组添加另一个对象。 你的新对象应将属性 `inputVal` 和 `addElDelay` 分别设置为 `1` 和 `2000`。 记住将 `animationData` 数组视为堆栈，并将新对象添加到堆栈的顶部。

# --hints--

`animationData` 数组的长度应为 `3`。
...
你应该将 `addElDelay` 属性的值设置为数字 `2000`。
