...
Кожного разу, коли ви натискаєте на кнопку відтворення, завжди буде відтворюватися перша пісня вашого плейлиста. Щоб це виправити, у стрілковій функції слухача події замініть `playSong(0)` на інструкцію `if`, яка перевіряє, чи `userData.currentSong` дорівнює `null`.

Всередині блоку `if` викличте функцію `playSong` з ідентифікатором `id` першої пісні в масиві `userData.songs`.

# --hints--

Вам слід змінити функцію зворотного виклику слухача події, щоб викликати функцію `playSong` з ідентифікатором `id` першої пісні в масиві `userData.songs`, коли `userData.currentSong` дорівнює `null`.
