...
Avrai probabilmente notato che il tuo codice sta diventando piuttosto ripetitivo! C'è un modo migliore per gestire più elementi simili.

Invece di selezionare ogni pulsante singolarmente, usa `querySelectorAll(".emoji-btn")` per selezionare tutti i pulsanti con la classe `emoji-btn` contemporaneamente. Archivia il risultato in una variabile chiamata `btns`.
...
Dovresti usare `querySelectorAll` per selezionare tutti i pulsanti emoji.

```js
assert.match(code, /document\.querySelectorAll\s*\(\s*['"`]\.emoji-btn['"`]\s*\)/);
```

Dovresti archiviare il risultato in una variabile chiamata `btns`.
...
La variabile `btns` dovrebbe contenere tutti e 4 i pulsanti emoji.
