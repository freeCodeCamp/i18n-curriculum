...
Оскільки ви перенесли порівняння у умову `while`, ви можете видалити весь свій вираз `if`.

# --hints--

У вас більше не має бути виразу `if`.

```js
assert.notMatch(__helpers.removeJSComments(code), /if\s*\(\s*done\s*===\s*count\s*\)/);
```

Ви більше не повинні присвоювати `continueLoop` значення `false`.
