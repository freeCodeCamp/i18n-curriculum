...
`div` の中に6つの `.foreground-buildings` 要素をネストし、それらに `fb1` から `fb6` までのクラスを順番に付けてください。"fb" は "foreground building"（前景の建物）を意味します。これらは前景のための6つの追加の建物になります。

# --hints--

クラスが `div` の新しい `fb1` を作成してください。

```js
assert.exists(document.querySelector('div.fb1'));
```

クラスが `div` の新しい `fb2` を作成してください。

```js
assert.exists(document.querySelector('div.fb2'));
```

クラスが `div` の新しい `fb3` を作成してください。

```js
assert.exists(document.querySelector('div.fb3'));
```

クラスが `div` の新しい `fb4` を作成してください。

```js
assert.exists(document.querySelector('div.fb4'));
```

クラスが `div` の新しい `fb5` を作成してください。

```js
assert.exists(document.querySelector('div.fb5'));
```

クラスが `div` の新しい `fb6` を作成してください。

```js
assert.exists(document.querySelector('div.fb6'));
```

これらの新しい `div` 要素を `.foreground-buildings` 要素の中に配置してください。
