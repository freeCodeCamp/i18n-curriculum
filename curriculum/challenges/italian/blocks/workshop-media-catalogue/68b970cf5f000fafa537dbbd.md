...
Dato che stai memorizzando l'oggetto che causa l'eccezione nell'oggetto eccezione, puoi farne un certo uso.

Dopo la tua chiamata `print` esistente, aggiungi `print(f'Unable to add {e.obj}: {type(e.obj)}')`.

Questo tipo di messaggi può essere estremamente utile quando stai cercando di eseguire il debug del tuo codice.

# --hints--

Dovresti avere `print(f'Unable to add {e.obj}: {type(e.obj)}')` dopo la tua chiamata `print(f'Media Error: {e}')` esistente.
