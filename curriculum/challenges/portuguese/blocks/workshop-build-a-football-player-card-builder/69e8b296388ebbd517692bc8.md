...
Os valores finais fixos são as seis estatísticas do jogador. Substitua os três primeiros conjuntos de números fixos pela propriedade `player` correspondente:
...
Você deve substituir o valor `97` por `PAC` com `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

Você deve substituir o valor `98` por `SHO` com `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

Você deve substituir o valor `83` por `PAS` com `player.pas`.
