...
أضف معالِج `onerror` إلى عنصر الصورة للتعامل مع فشل التحميل. عند الفشل الأول، علّمه باستخدام `dataset.failed` واستبدل المصدر بالصورة الافتراضية المحلية. إذا فشلت الصورة مرة أخرى، أزل صنف hidden وأخفِ عنصر النائب الخاص بالتحميل باستخدام `previousElementSibling`.

# --hints--

يجب أن يحتوي عنصر `img` على معالِج `onerror`.
...
يجب أن يحتوي معالِج `onerror` على جملة `if` تتحقق مما إذا كانت قيمة `dataset.failed` خاطئة.
...
يجب تعيين `dataset.failed` إلى `'1'` داخل كتلة `if`. لا تنسَ استخدام الكلمة المفتاحية `this` هنا.
...
يجب أن تحدّث كتلة `if` الخاصّة بك الخاصية `this.src` إلى الصورة الافتراضية المحلية. لا تنسَ استخدام الكلمة المفتاحية `this` هنا.
...
يجب أن تحتوي على كتلة `else` تزيل صنف `hidden`. لا تنسَ استخدام الكلمة المفتاحية `this` هنا.
...
يجب أن تضبط كتلة `else` الخاصّة بك الخاصية `previousElementSibling.style.display` إلى `'none'`. لا تنسَ استخدام الكلمة المفتاحية `this` هنا.
