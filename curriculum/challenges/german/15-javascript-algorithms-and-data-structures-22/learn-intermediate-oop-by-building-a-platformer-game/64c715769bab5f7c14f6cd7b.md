...
Du solltest `isPressed` dem `keys.leftKey.pressed` zuweisen.

```js
assert.match(code, /s*keys\.leftKey\.pressed\s*=\s*isPressed\s*;?/);
```

Du solltest eine `if`-Anweisung haben, die prüft, ob `xVelocity` gleich 0 ist.

```js
assert.match(code, /if\s*\(\s*xVelocity\s*===\s*0\s*\)\s*{\s*/);
```

Deine `if`-Anweisung sollte `player.velocity.x` der `xVelocity` zuweisen.
