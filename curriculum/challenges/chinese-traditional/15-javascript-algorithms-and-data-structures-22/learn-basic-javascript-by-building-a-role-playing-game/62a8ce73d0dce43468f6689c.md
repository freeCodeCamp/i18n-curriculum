...
在上一個項目中，你學習瞭如何使用連接運算符將變量插入到字符串中，如下所示：
...
將字符串 `"You now have a new weapon."` 更新爲 `"You now have a "`，後跟新武器的名稱，並記得以句號結束句子。


# --hints--

你應該更新 `text.innerText` 分配以以字符串 `"You now have a "` 開頭。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1/);
```

你應該使用連接運算符將 `newWeapon` 添加到 `text.innerText` 字符串的末尾。

```js
assert.match(buyWeapon.toString(), /text\.innerText\s*=\s*('|")You now have a \1\s*\+\s*newWeapon/);
```

你應該使用連接運算符以 `.` 結束 `text.innerText` 字符串。
