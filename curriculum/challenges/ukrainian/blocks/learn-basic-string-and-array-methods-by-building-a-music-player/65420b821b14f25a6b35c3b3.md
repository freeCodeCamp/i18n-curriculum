...
Коли пісні відображаються на сторінці, на ній має бути відображено назву, виконавця, тривалість кожної пісні та кнопку видалення.

У наступних кількох кроках ви дізнаєтеся, як додати новий HTML для кожної пісні використавши метод <dfn>map()</dfn>. Цей метод буде описано більш детально на наступному кроці.

Почніть з використання `const`, щоб оголосити змінну з назвою `songsHTML` і призначте до неї `array.map()`.
...
Призначте `array.map()` до `songsHTML`.
