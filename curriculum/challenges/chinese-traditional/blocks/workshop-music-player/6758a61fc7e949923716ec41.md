...
現在，使用 `querySelectorAll` 方法標的 `.playlist-song` 元素，並將它們儲存在名為 `songs` 的變數中。

然後，對 `songs` 呼叫 `forEach` 方法。傳入一個回呼函式，該函式使用 `song` 作為方法的參數。暫時將大括號留空。
...
你應該將所有 `.playlist-song` 元素的群集指定到你的 `songs` 變數。

```js
assert.deepEqual(songs, document.querySelectorAll(".playlist-song"));
```

你應該對 `songs` 呼叫 `forEach` 方法，並傳入一個使用 `song` 作為方法參數的回呼。
