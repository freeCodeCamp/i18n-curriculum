...
你需要使用 `remove()` 從 DOM 中移除任務，並使用 `splice()` 從 `taskData` 陣列中移除。

`splice()` 是一個陣列方法，用於在指定索引處移除、取代或添加元素，同時傳回被移除的元素。它最多可接受三個引數：第一個是必填的起始索引，第二個是要移除的項目數量，第三個是選用的取代元素。以下是一個範例：
...
使用 `remove()` 方法從 DOM 中移除 `buttonEl` 的 `parentElement`。然後使用 `splice()` 從 `taskData` 陣列中移除任務。將 `dataArrIndex` 和 `1` 作為 `splice()` 的引數傳入。

`dataArrIndex` 是開始的索引，`1` 是要移除的項目數量。

# --hints--

你不應該更改 `dataArrIndex` 述語。
