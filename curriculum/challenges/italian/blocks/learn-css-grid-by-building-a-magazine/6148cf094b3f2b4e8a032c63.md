...
Il pseudo-selettore `::first-letter` ti permette di selezionare la prima lettera nel contenuto testuale di un elemento.

Crea un selettore `.first-paragraph::first-letter` e imposta la proprietà `font-size` su `6rem`. Inoltre, dagli una proprietà `color` impostata su `orangered` per farla risaltare.

# --hints--

Dovresti avere un selettore `.first-paragraph::first-letter`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter'));
```

Il tuo selettore `.first-paragraph::first-letter` dovrebbe avere una proprietà `font-size` impostata su `6rem`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.fontSize === '6rem');
```

Il tuo selettore `.first-paragraph::first-letter` dovrebbe avere una proprietà `color` impostata su `orangered`.
