...
عنصر `textarea` يعمل مثل عنصر `input` من النوع `text`، لكنه يضيف ميزة استقبال نص متعدد الأسطر، مع عدد مبدئي من الصفوف والأعمدة النصية.

سيتمكن المستخدمون من التسجيل مع سيرة ذاتية. أضف `label` بالنص `Provide a bio:` في نهاية `fieldset`. أضف عنصر `textarea` داخل عنصر الـ `label`. لاحظ أن عنصر `textarea` يتطلب وسم إغلاق.
...
يجب أن يكون نص `label` هو `Provide a bio:`.
