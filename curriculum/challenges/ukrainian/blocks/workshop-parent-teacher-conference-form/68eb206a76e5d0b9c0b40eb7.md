...
Час стилізувати `fieldset` елементи, щоб надати їм виразний кордон.

Виберіть `fieldset` елемент. Встановіть його `border` у `1px solid gray`, `border-radius` у `5px`, `margin` у `20px 0` і `padding` у `20px`.

# --hints--

Ви повинні націлитися на `fieldset` елемент.

```js
assert(new __helpers.CSSHelp(document).getStyle('fieldset'));
```

Встановіть властивість `border` у `1px solid gray`.
...
Встановіть властивість `border-radius` у `5px`.
...
Встановіть властивість `margin` у `20px 0`.
...
Встановіть властивість `padding` у `20px`.
