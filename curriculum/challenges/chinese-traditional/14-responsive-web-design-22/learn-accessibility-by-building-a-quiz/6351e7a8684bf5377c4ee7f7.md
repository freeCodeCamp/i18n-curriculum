...
儘管對於帶有嵌套 `input` 的 `label` 元素來說不是必需的，但最佳實踐仍然是將 `label` 與其對應的 `input` 元素顯式鏈接。

現在，給你的四個 `label` 中的每一個添加一個 `for` 屬性，鏈接到相應的單選 `input` 的 `label`。
...
你應該給第一個 `label` 一個 `for` 屬性，以匹配其 `input` 元素的 `id`。
