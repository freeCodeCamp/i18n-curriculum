...
Anida el elemento `select` (con sus elementos `option`) dentro de un elemento `label` con el texto `How did you hear about us?`. El texto debe ir antes del elemento `select`.

# --hints--

Debes anidar solo el elemento `select` dentro de un elemento `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(3) > select'));
```

Debes darle al elemento `label` el texto `How did you hear about us?`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(3)')?.innerText?.trim(), 'How did you hear about us?');
```

Debes colocar el texto antes del elemento `select`.
...
Debes tener 5 elementos `option` dentro del elemento `select`.
