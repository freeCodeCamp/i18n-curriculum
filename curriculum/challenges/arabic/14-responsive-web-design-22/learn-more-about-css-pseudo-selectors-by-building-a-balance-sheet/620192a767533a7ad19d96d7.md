...
يجب أن يكون لديك منتقي `tr.total td`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td'));
```

يجب ان يكون لمنتقي `tr.total td` خاصية `text-align` بقيمة `right`.

```js
assert(new __helpers.CSSHelp(document).getStyle('tr.total td')?.getPropertyValue('text-align') === 'right');
```

يجب ان يكون لمنتقي `tr.total td` خاصية `padding` بقيمة `0 0.25rem`.
