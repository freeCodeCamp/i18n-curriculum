...
應該設置 `.penguin` 的 `transition-duration` 值爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionDuration, '1s');
```

應該設置 `.penguin` 的 `transition-timing-function` 爲 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin')?.transitionTimingFunction, 'ease-in-out');
```

應該設置 `.penguin` 的 `transition-delay` 爲 `--fcc-expected--`，而不是 `--fcc-actual--`。
