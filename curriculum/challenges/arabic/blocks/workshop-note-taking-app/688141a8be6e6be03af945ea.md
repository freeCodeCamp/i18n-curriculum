...
عندما يقوم المستخدم بتحرير الملاحظة، تريد تتبع الإصدار المحرر.

استخدم `let` لإنشاء متغير يسمى `currentContent` وعيّنه بسلسلة نصية فارغة.

*ملاحظة*: في تطبيق حقيقي عادةً ما تحفظ ملاحظاتك في قاعدة بيانات. أو يمكنك حتى حفظها في التخزين المحلي. مع ذلك، العمل مع قواعد البيانات والتخزين المحلي يتجاوز نطاق هذه الورشة وسيتم تدريس هذه المفاهيم لاحقًا.

# --hints--

يجب أن تستخدم `let` لإنشاء المتغير `currentContent`.

```js
assert.match(code, /let\s+currentContent/);
```

يجب أن يكون المتغير `currentContent` سلسلة نصية.

```js
assert.isString(currentContent);
```

يجب تهيئة المتغير `currentContent` بسلسلة نصية فارغة.
