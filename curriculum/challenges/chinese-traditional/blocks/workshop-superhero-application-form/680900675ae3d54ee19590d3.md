...
你有沒有注意到你可以在不填寫輸入欄位的情況下提交表單？

為了停止那個，如果 `heroName`、`realName` 或 `powerSource` 是 falsy，或 `powers` 的長度是 `0`，將 `disabled` 屬性添加到提交按鈕以停用它。

現在，如果有任何欄位未填寫，按鈕將會被停用。請填寫你的超級英雄資訊表單並提交，以加入聯盟。
...
你的 `button` 元素應該有一個 `disabled` 屬性。

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

如果 `heroName`、`realName` 和 `powerSource` 中的任何一個為 false，或 `powers` 的長度為 `0`，你應該停用 `button`。
