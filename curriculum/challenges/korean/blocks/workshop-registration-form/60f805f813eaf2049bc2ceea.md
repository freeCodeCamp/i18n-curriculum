---
id: 60f805f813eaf2049bc2ceea
title: Step 12
challengeType: 0
dashedName: step-12
---

# --description--

각 `input` 안에 `label` 요소를 중첩하세요. 각 `input`는 `label` 텍스트 뒤에 추가하고, 콜론 뒤에 공백을 포함하세요.

# --hints--

`input` 요소에 `fieldset` 요소 네 개를 추가해야 합니다.

```js
assert.equal(document.querySelectorAll('fieldset input')?.length, 4);
```

`input` 요소 안에 `label` 요소를 중첩해야 합니다.

```js
assert.equal(document.querySelectorAll('label input')?.length, 4);
```

첫 번째 `input`는 `label` 텍스트 `Enter Your First Name:` 뒤에 추가하고, 콜론 뒤에 공백을 포함하세요.

```js
const query = /^Enter Your First Name:\s+<input/
assert.match(document.querySelectorAll('label')?.[0]?.innerHTML.trim(), query);
```

두 번째 `input`는 `label` 텍스트 `Enter Your Last Name:` 뒤에 추가하고, 콜론 뒤에 공백을 포함하세요.

```js
const query = /^Enter Your Last Name:\s+<input/
assert.match(document.querySelectorAll('label')?.[1]?.innerHTML.trim(), query);
```

세 번째 `input`는 `label` 텍스트 `Enter Your Email:` 뒤에 추가하고, 콜론 뒤에 공백을 포함하세요.

```js
const query = /^Enter Your Email:\s+<input/
assert.match(document.querySelectorAll('label')?.[2]?.innerHTML.trim(), query);
```

네 번째 `input`는 `label` 텍스트 `Create a New Password:` 뒤에 추가하고, 콜론 뒤에 공백을 포함하세요.

```js
const query = /^Create a New Password:\s+<input/
assert.match(document.querySelectorAll('label')?.[3]?.innerHTML.trim(), query);
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>Registration Form</title>
  <link rel="stylesheet" href="styles.css" />
</head>
<body>
  <h1>Registration Form</h1>
  <p>Please fill out this form with the required information</p>
  <form method="post" action='https://register-demo.freecodecamp.org'>
    <fieldset>
--fcc-editable-region--
      <label>Enter Your First Name:</label>
      <label>Enter Your Last Name:</label>
      <label>Enter Your Email:</label>
      <label>Create a New Password:</label>
--fcc-editable-region--
    </fieldset>
    <fieldset></fieldset>
    <fieldset></fieldset>
  </form>
</body>
</html>
```

```css
body {
  width: 100%;
  height: 100vh;
  margin: 0;
  background-color: #1b1b32;
  color: #f5f6f7;
}

label {
  display: block;
  margin: 0.5rem 0;
}

```
