...
Se guardi bene, vedrai che la canzone attualmente in riproduzione non è evidenziata nella playlist, quindi non sai davvero quale canzone sta suonando. Puoi risolvere questo problema creando una funzione per evidenziare qualsiasi canzone che viene riprodotta.

Crea una funzione chiamata `highlightCurrentSong`. All'interno della funzione, verifica che esista un elemento `.playlist-song` con `aria-current="true"`. Se esiste, usa il metodo `removeAttribute` per rimuovere l'attributo `aria-current` da esso.

# --hints--

Dovresti avere una funzione chiamata `highlightCurrentSong`.

```js
assert.isFunction(highlightCurrentSong)
```

La tua funzione `highlightCurrentSong` dovrebbe usare il metodo `removeAttribute` per rimuovere l'attributo `aria-current` dall'elemento `.playlist-song` con `aria-current="true"` se quell'elemento esiste.
