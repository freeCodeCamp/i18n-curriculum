...
`useMemo()` 接受两个参数：一个返回你想要缓存的值的函数，以及一个依赖数组。只有当一个或多个依赖项发生变化时，缓存的值才会被重新计算。
...
将 `filteredItems` 设为 `useMemo()`，并传入一个带有括弧状的花括弧的匿名函数。在花括弧内，使用 `return` 语句显式返回你现有的过滤逻辑。同时，将 `query` 作为依赖数组中的唯一依赖添加。这确保过滤操作仅在 `query` 变化时重新运行。

# --hints--

你应该将 `filteredItems` 设置为调用 `useMemo()` 钩子。
