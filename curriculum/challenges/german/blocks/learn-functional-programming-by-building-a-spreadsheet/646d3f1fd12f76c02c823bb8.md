...
Sie haben einen kleinen Bug. `arg1` und `arg2` sind Strings, keine Zahlen. `infixToFunction['+']("1", "2")` würde `12` zurückgeben, was mathematisch nicht korrekt ist.

Fassen Sie jedes Ihrer `infixToFunction[operator]` Argumente in einem `parseFloat()` Aufruf ein.

# --hints--

Sie sollten `arg1` in einem `parseFloat()` Aufruf einbetten.
...
Sie sollten `arg2` in einem `parseFloat()` Aufruf einbetten.
