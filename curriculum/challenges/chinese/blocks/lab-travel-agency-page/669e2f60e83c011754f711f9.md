...
1. 三个锚元素应该分别包含一个带有适当描述的 `src` 属性和设置为有效的图像的 `alt` 属性的 `img` 元素。 如果愿意，可以使用 `https://cdn.freecodecamp.org/curriculum/labs/colosseo.jpg`、`https://cdn.freecodecamp.org/curriculum/labs/alps.jpg` 和 `https://cdn.freecodecamp.org/curriculum/labs/sea.jpg`。
1. 所有五个锚元素都应该有一个值为 `href` 的 `https://www.freecodecamp.org/learn` 属性和一个值为 `target` 的 `_blank` 属性。
