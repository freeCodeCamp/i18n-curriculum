...
Nei passaggi precedenti hai creato la funzionalità per riprodurre una canzone. Ora devi aggiungere la funzionalità al pulsante play in modo che riproduca la canzone corrente quando viene cliccato.

Usa il metodo `addEventListener()` e passa un evento `"click"` come primo argomento e una funzione di callback vuota con sintassi a freccia come secondo argomento, ad esempio `() => {}`.

# --hints--

Dovresti chiamare il metodo `addEventListener()` sulla tua variabile `playButton`.
...
Dovresti usare la sintassi a freccia per passare una callback vuota al tuo listener per l'evento. Non dimenticare che ti serve anche una coppia vuota di parentesi graffe.
