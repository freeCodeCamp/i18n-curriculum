...
Nachdem Sie das `resetButton` erstellt haben, müssen Sie ihm ein `id`- und `aria-label`-Attribut zuweisen. JavaScript stellt die `id`- und `ariaLabel`-Eigenschaften bereit, die Sie dafür verwenden müssen.

Zum Beispiel würde `element.id` ein `id`-Attribut setzen, und `element.ariaLabel` würde ein `aria-label`-Attribut setzen. Beide akzeptieren ihre Werte als String.

Setzen Sie das `id`-Attribut des `resetButton`-Elements auf `"reset"` und sein `aria-label`-Attribut auf `"Reset playlist"`.

# --hints--

Sie sollten die bestehende `if`-Anweisung und deren Inhalt nicht verändern.
...
Sie sollten `resetButton.id` verwenden, um ein `id`-Attribut namens `"reset"` für das `resetButton` zu erstellen.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

Sie sollten `resetButton.ariaLabel` verwenden, um ein `"aria-label"`-Attribut namens `"Reset playlist"` für das `resetButton` zu erstellen.
