...
Якщо стрілкова функція повертає простий вираз, ви можете опустити ключове слово `return` та фігурні дужки `{}`. Це називається неявним поверненням.

```js
const multiplyTwoNumbers = (num1, num2) => num1 * num2;
```

Якщо у вашій стрілковій функції кілька ліній коду в тілі функції, тоді потрібно використовувати ключове слово `return` та фігурні дужки `{}`.
...
Переробіть, або оновіть, вашу функцію `addTwoNumbers`, щоб видалити ключове слово `return` та фігурні дужки `{}`. Ваша функція `addTwoNumbers` має замість цього використовувати неявне повернення.

Відкрийте консоль, щоб переконатися, що ви все ще отримуєте правильні вихідні дані.

# --hints--

Ваша перероблена функція `addTwoNumbers` має використовувати неявне повернення.
