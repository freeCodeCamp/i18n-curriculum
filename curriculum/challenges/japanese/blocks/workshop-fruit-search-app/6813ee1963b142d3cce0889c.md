...
`try` ブロック内で、`fetch` の URL を使って `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` 関数を呼び出し、その結果を `response` という名前の変数に保存してください。`async` 関数の中にいるので、`await` の前に `fetch` を使う必要があります。

# --hints--

`fetch` を使って `https://fruit-search.freecodecamp.rocks/api/fruits?q=${query}` を呼び出し、その結果を `response` 変数に保存してください。
