...
Ndani ya bloksi ya `try`, hifadhi data ya mchezaji wa sasa katika `localStorage` kwa kufanya wito wa `localStorage.setItem` ukiwa na ufunguo wa hifadhidata na kitu cha mchezaji kilichobadilishwa kuwa mfuatano wa herufi.

Na katika bloksi ya `catch`, andika mfuatano wa herufi `Failed to save player data:` ukifuatiwa na kigezo cha `error` kwenye konsoli.

# --hints--

Bloksi yako ya `useEffect` try inapaswa kufanya wito wa `localStorage.setItem` ukiwa na `STORAGE_KEY` na kitu cha `player` kilichobadilishwa kuwa mfuatano wa herufi.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

Bloksi yako ya `useEffect` catch inapaswa kuandika `'Failed to save player data:'` na `error` kwenye konsoli.
