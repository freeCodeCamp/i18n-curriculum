...
Adicione dois elementos `span` com o `className` de `stat-value` e `stat-label` respectivamente a cada `stat-row`.

# --hints--

Cada `.stat-row` no segundo `.stat-col` deve conter um `span` com a classe `stat-value` e um `span` com a classe `stat-label`.
