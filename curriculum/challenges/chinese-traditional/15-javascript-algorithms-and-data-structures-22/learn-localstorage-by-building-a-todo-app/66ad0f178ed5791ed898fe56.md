...
還剩下兩個錯誤需要修復。 首先，如果你創建的任務標題僅有空格，則即使標題是必需的，也會創建一個空任務。

通過在函數開頭添加一個檢查，查看是否 `!titleInput.value.trim()`，來修復這一 點。 如果沒有標題，顯示警報，文本爲 `Please provide a title`，之後是 `return`。

# --hints--

你應該在 `addOrUpdateTask` 函數中添加一個帶有條件 `!titleInput.value.trim()` 的 `if` 語句。

```js
assert.match(code, /if\s*\(\s*!titleInput\.value\.trim\(\s*\)\s*\)\s*/)
```

你應該在 `if` 塊中調用 `alert("Please provide a title")`。
