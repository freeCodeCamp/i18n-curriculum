...
`fetchCardsData` 메서드 안에 `response`이라는 변수를 선언하고, 다음 URL을 사용해 `fetch` 함수를 호출하여 할당하세요: `` `${CDN_URL}/card_data.json` ``.

`await` 호출 앞에 `fetch` 키워드를 추가하고, 전체 로직을 `try...catch` 블록 안에 감싸세요.

# --hints--

`response` 메서드 안에 `fetchCardsData` 변수를 선언해야 합니다.
...
`response` 변수는 ``await fetch`${CDN_URL}/card_data.json` `` `await fetch` 함수를 호출하여 할당해야 합니다.
...
`fetch` 호출은 `try...catch` 블록 안에 감싸야 하며, `error`가 `catch` 매개변수로 들어가야 합니다.
