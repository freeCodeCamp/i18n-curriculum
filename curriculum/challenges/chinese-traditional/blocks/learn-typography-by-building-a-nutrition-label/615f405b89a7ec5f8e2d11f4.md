...
為你的新 `.divider` 創建一個選擇器，並將 `border-bottom` 屬性設定為 `1px solid #888989`。同時給它設定上下邊距為 `2px`。它不應該有任何左右邊距。
