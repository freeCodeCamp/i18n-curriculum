...
Agrega otro caso para cuando la longitud de `results` sea `2`. Esta vez, asigna a `result_list` una lista que contenga dos cadenas con el formato `x1 = <root1>` y `x2 = <root2>`. Nuevamente, haz que la solución muestre tanto los signos positivos como negativos.
...
No debes modificar tus bloques `case` existentes.
...
Debes crear un nuevo `case` con el patrón `2` después del bloque `case` existente.
...
Debes asignar una lista que contenga dos cadenas con el formato `x1 = <root1>` y `x2 = <root2>` a `result_list` dentro de tu nuevo cuerpo `case`. Muestra ambos signos, positivo y negativo, para los resultados.
