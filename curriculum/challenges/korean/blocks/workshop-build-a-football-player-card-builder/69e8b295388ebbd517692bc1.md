...
`PlayerData` 인터페이스가 정의되었으니, `PlayerCard` 컴포넌트를 업데이트하여 `player` 인터페이스와 일치하도록 `PlayerData` 타입의 `PlayerData` 프로프를 받게 하세요.

프로프스를 이렇게 구조 분해할 수 있다는 점을 기억하세요:
...
`PlayerCard`를 업데이트하여 `player` 타입의 `PlayerData` 프로프를 받게 하세요.
