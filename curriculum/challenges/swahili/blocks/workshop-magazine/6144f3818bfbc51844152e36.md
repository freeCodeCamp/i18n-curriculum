...
Tumia kitendakazi cha `minmax` kufanya safu zako zijibadilishe kulingana na kifaa chochote. Kitendakazi cha `minmax` kinachukua hoja mbili, ya kwanza ikiwa ni kiwango cha chini na ya pili ikiwa ni kiwango cha juu. Thamani hizi zinaweza kuwa urefu, asilimia, `fr`, au hata neno kama `max-content`.

Funga kila moja ya thamani ulizozitaja tayari za sifa ya `grid-template-columns` ndani ya kitendakazi cha `minmax`, ukitumia kila thamani kama hoja ya pili. Hoja ya kwanza inapaswa kuwa `2rem`, `min-content`, na `2rem` mtawalia.
