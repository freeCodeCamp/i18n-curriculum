...
你的下一個正則表達式將匹配類似 `stock alert` 的字符串。 聲明一個 `stockRegex` 變量併爲其分配一個與字符串 `stock alert` 匹配的正則表達式。 記住不區分大小寫。
...
你的 `stockRegex` 變量應該被分配一個正則表達式。

```js
assert.instanceOf(stockRegex, RegExp);
```

你的 `stockRegex` 應該與 `stock alert` 匹配。

```js
assert.match('stock alert', stockRegex);
```

你的 `stockRegex` 應該不區分大小寫。
