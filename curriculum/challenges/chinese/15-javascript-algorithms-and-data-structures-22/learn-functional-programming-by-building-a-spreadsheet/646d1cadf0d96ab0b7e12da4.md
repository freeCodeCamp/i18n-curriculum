...
因为 `elemValue` 返回一个函数，所以你的 `addCharacters` 函数最终返回一个函数引用数组。 你希望 `.map()` 方法运行 `elemValue` 函数的内部函数，这意味着你需要调用 `elemValue` 而不是引用它。 将 `num` 作为参数传递给 `elemValue` 函数。

<!-- TODO: Explain further? -->

# --hints--

你应该在 `.map()` 方法中调用 `elemValue()`。
