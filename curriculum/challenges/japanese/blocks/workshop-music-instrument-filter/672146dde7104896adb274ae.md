...
ドロップダウンメニューからカテゴリを選択すると、楽器カードが正しくフィルターされてページに表示されますが、カードの間にあるカンマを取り除く必要があります。

`instrumentCards` が返す配列を結合してこれを実現してください。これで音楽楽器のフィルターが完成します。
...
`instrumentCards` 関数が返す配列を結合してください。イベントリスナーは変更しないでください。
