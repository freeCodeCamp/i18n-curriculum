...
Se você pressionar o botão anterior quando nenhuma música estiver tocando, a primeira música será reproduzida. Para corrigir isso, adicione uma declaração `if` no início da função `playPreviousSong`. Verifique se a música atual é `null` e pare a execução da função com uma instrução `return`.

# --hints--

Você deve adicionar uma declaração `if` que verifica se `userData.currentSong === null` e interrompe a execução da função com uma declaração `return` dentro da sua função `playPreviousSong`.
