...
Ein Standardbild für die Karte und die URL für eine API-Anfrage wurden bereitgestellt, Sie benötigen nur noch einige Variablen, um sie zu speichern.

Erstellen Sie eine Variable `CDN_URL` und setzen Sie sie auf `https://cdn.freecodecamp.org/curriculum/typescript/tarot-app`. Erstellen Sie außerdem eine Variable `defaultImg` mit dem Wert `new URL("default.svg", CDN_URL + "/")` und eine Variable `LOCAL_DEFAULT_IMG` mit dem Wert `defaultImg`.

# --hints--

Sie sollten eine Variable namens `CDN_URL` erstellen.
...
Ihre `CDN_URL`-Variable sollte auf `"https://cdn.freecodecamp.org/curriculum/typescript/tarot-app"` gesetzt werden.
...
Sie sollten eine Variable namens `defaultImg` erstellen.
...
Ihre Variable `defaultImg` sollte auf `new URL("default.svg", CDN_URL + "/")` gesetzt sein.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.variables.defaultImg.matches('const defaultImg = new URL("default.svg", CDN_URL + "/")'));
```

Sie sollten eine Variable namens `LOCAL_DEFAULT_IMG` erstellen.
...
Ihre `LOCAL_DEFAULT_IMG`-Variable sollte auf `defaultImg` gesetzt werden.
