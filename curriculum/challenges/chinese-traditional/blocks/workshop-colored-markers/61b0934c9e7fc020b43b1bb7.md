...
創建一個新的 CSS 規則來標的 `cap` 類別。在新的規則中，將 `width` 屬性設定為 `60px`，並將 `height` 設定為 `25px`。

# --hints--

你應該使用類別選擇器來標的 `cap` 類別。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cap'));
```

你的 `.cap` CSS 規則應該有一個 `width` 屬性設定為 `60px`。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cap')?.width, '60px');
```

你的 `.cap` CSS 規則應該有一個 `height` 屬性設定為 `25px`。
