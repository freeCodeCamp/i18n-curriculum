...
現在你的 `update()` 函數實際上可以評估公式。 請記住，你編寫了 `if` 條件來檢查函數是否被調用。

在 `if` 語句中，將 `element` 的 `value` 設置爲 `evalFormula()` 函數的結果。 暫時不要傳遞任何參數。

# --hints--

你應該在 `if` 塊中更新 `element` 的 `value` 屬性。
...
你應該將調用 `evalFormula()` 函數的結果分配給 `value` 屬性。
