...
Тепер, коли ви створили `resetButton`, потрібно призначити до неї атрибути `id` та `aria-label`. JavaScript надає властивості `id` та `ariaLabel`, які потрібно використати для цього.

Наприклад, `element.id` встановить атрибут `id`, а `element.ariaLabel` встановить атрибут `aria-label`. Обидві вони приймають свої значення як рядок.

Встановіть атрибут `id` елемента `resetButton` на `"reset"`, а атрибут `aria-label` на `"Reset playlist"`.
...
Використайте `resetButton.id`, щоб створити атрибут `id` під назвою `"reset"` для `resetButton`.

```js
assert.match(code, /resetButton\.id\s*=\s*('|")reset\1/)
```

Використайте `resetButton.ariaLabel`, щоб створити атрибут `"aria-label"` під назвою `"Reset playlist"` для `resetButton`.
