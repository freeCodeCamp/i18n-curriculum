...
`random` プロパティを作成してください。このプロパティは配列の最初の2つの数字を使ってランダムな整数を生成します。この数の範囲は、2つのうち最初の数字（含む）から始まり、これら2つの数字の合計の直前までです。計算には `Math.floor()` と `Math.random()` メソッドを使ってください。

# --hints--

`spreadsheetFunctions` オブジェクトには `random` プロパティが必要です。

```js
assert.property(spreadsheetFunctions, "random");
```

`random` プロパティは関数である必要があります。

```js
assert.isFunction(spreadsheetFunctions.random);
```

`random` 関数は配列の最初の2つの数字の間のランダムな数を返す必要があります。
...
`random` 関数は整数（小数点なしの数）を返す必要があります。
