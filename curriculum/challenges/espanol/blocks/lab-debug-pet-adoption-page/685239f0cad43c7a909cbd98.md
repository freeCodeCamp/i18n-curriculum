...
1. Sally quiere usar una imagen de algunos gatos pero no se muestra correctamente. Necesitarás corregir lo siguiente en el `img` elemento:
   ...
   - Elimina la etiqueta de cierre `</img>` porque los elementos `img` son elementos vacíos y no tienen etiquetas de cierre.
...
Tu elemento `a` con el texto `Visit cats page` necesita tener un atributo `href` en lugar de un atributo `src`.
...
Tu elemento `a` con el texto `Visit dogs page` necesita tener un atributo `href` en lugar de un atributo `src`.
