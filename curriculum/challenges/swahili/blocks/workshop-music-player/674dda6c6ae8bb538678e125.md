...
Ongeza tamko la `if` ili kukagua kama hakuna wimbo unaoimbwa kwa sasa kwa kuthibitisha kuwa `userData.currentSong` ni falsy.
Ndani ya blokki ya `if`, weka sifa ya `currentTime` ya kitu cha `audio` kuwa `0`.

# --hints--

Kitendakazi chako cha `playSong` kinapaswa kuweka sifa ya `currentTime` ya kitu cha `audio` kuwa `0` wakati hakuna wimbo unaoimbwa kwa sasa.
