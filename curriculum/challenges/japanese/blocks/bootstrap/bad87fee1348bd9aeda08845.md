...
ブートストラップの`col-xs-*`クラスはすべての`form`要素に適用できるので、チェックボックスにも使えます！これにより、画面の解像度がどれだけ広くても、チェックボックスがページ全体に均等に配置されます。

# --instructions--

3つのチェックボックスすべてを`<div class="row">`要素にネストしてください。次に、それぞれを`<div class="col-xs-4">`要素にネストしてください。

# --hints--

すべてのチェックボックスは、`div`クラスを持つ1つの`row`の中にネストしてください。
...
それぞれのチェックボックスは、`div`クラスを持つ自分自身の`col-xs-4`の中にネストしてください。
