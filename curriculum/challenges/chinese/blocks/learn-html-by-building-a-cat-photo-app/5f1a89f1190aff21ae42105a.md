...
所有三个复选框都应该有一个 `value` 属性。 确保在开始标签的名称后面有一个空格，且所有的属性名称前面也要有空格。
...
`Loving` 复选框的 `value` 属性应设置为 `loving`。 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
...
`Lazy` 复选框的 `value` 属性应设置为 `lazy`。 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
...
`Energetic` 复选框的 `value` 属性应设置为 `energetic`。 你要么遗漏了属性值，要么拼写有误。 记住，属性值应该放在引号中。
