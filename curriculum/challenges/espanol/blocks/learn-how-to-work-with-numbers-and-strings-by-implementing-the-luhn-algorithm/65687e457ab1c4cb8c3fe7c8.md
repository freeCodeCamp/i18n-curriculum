...
Arregla el llamado `verify_card_number` para que si regresa `True`, responda `'VAliD'` en la consola. De otra manera, responda `'INVALID!'`.

# --hints--

Debes tener `if verify_card_number(translated_card_number):` dentro de la función `main`.
...
Debes tener `print('VALID!')` dentro de la estructura `if`.
...
Debes tener `print('INVALID!')` dentro de la cláusula `else`.
