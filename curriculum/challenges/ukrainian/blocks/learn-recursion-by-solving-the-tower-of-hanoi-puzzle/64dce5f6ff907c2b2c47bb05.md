...
Коли `target` порожній, диск обов’язково потрібно перемістити з `source` до `target`.

Після оголошення `forward` додайте інструкцію `if`, щоб перевірити, чи `rods[target]` порожній. Якщо так, змініть `forward` на `True`.

# --hints--

У вас має бути інструкція `if`, щоб перевірити, чи `rods[target]` порожній, і призначити `True` `forward`.
