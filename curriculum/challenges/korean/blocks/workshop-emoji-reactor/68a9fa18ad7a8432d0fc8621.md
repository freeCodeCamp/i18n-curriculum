...
이전 수업에서 `querySelectorAll`가 `NodeList`를 반환하는데, `NodeList`는 배열과 유사한 구조이며, `NodeList`가 가진 메서드 중 하나가 `forEach()`이고, 이것은 배열에 있는 `forEach`와 동일하게 작동한다는 것을 배웠습니다.

`forEach()`를 사용해 `btns`를 순회하며 각 버튼에 동일한 이벤트 리스너를 추가하세요.

매개변수 `btn`를 받는 콜백 함수를 사용하고, 콜백 내부에서 `updateCount(btn)`를 호출하는 클릭 이벤트 리스너를 추가하세요.

이로써 워크숍을 완료했습니다.

# --hints--

버튼들을 순회하려면 `btns.forEach()`를 사용하세요.

```js
assert.match(code, /btns\.forEach\s*\(/);
```

모든 버튼은 클릭 시 `updateCount`를 호출하는 이벤트 리스너를 가져야 합니다.
