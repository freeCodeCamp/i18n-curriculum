...
حدّث قيمة `total` لتكون مجموع قيمتي `subTotal` و `tax`.

# --hints--

يجب تحديث قيمة `total`. تذكّر استخدام الكلمة المفتاحية `this`.

```js
assert.match(cart.calculateTotal.toString(), /this\.total/);
```

يجب تعيين قيمة `total` لتكون مجموع قيمتي `subTotal` و `tax`.
