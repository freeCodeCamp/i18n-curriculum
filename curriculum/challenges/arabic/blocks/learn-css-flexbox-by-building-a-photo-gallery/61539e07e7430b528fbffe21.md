...
أعط محدِّد `.gallery` خاصية `padding` بقيمة `20px 10px` لإنشاء بعض المساحة حول الحاوية.

ثم، أعطه خاصية `max-width` بقيمة `1400px` وأضف `margin` بقيمة `0 auto` لمركزة العنصر.

# --hints--

يجب أن يحتوي محدِّد `.gallery` على خاصية `padding` بقيمة `20px 10px`.
...
يجب أن يحتوي محدِّد `.gallery` على خاصية `max-width` بقيمة `1400px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.gallery')?.maxWidth === '1400px');
```

يجب أن يكون عنصر `.gallery` مركّزًا باستخدام `margin` بقيمة `0 auto`.
