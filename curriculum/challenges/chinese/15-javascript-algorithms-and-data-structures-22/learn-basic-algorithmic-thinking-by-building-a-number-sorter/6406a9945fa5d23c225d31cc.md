...
要测试你写的代码，将事件监听器挂载到你的 `sortButton` 元素上。 它应该监听 `"click"` 事件，并将 `sortInputArray` 作为回调。

# --hints--

你应该在你的 `sortButton` 元素上调用 `.addEventListener()` 。
...
你的 `.addEventListener()` 方法应该把 `sortInputArray` 作为回调。 记住要传递函数引用，而不是函数调用。
