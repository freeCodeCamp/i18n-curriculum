...
疑似要素に親要素の半分の`width`、`height`の`45%`、および`background-color`の`gray`を設定してください。

# --hints--

`.penguin-body::before`に`width`の`50%`を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.width, '50%');
```

`.penguin-body::before`に`height`の`45%`を設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.height, '45%');
```

`.penguin-body::before`に`background-color`の`--fcc-expected--`を設定する必要がありますが、`--fcc-actual--`が見つかりました。
