...
ننتقل إلى مجموعة الحقول `fieldset` الأخيرة. ماذا لو أردت السماح للمستخدم برفع صورة شخصية؟

حسنًا، نوع `input` الخاص بـ `file` يسمح بذلك بالضبط. أضف `label` بالنص `Upload a profile picture: `، وضع داخله `input` يقبل رفع ملف.

# --hints--

يجب أن تضيف `label` بالنص `Upload a profile picture: `.

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label')?.innerText, /Upload a profile picture:/i);
```

يجب أن تضع عنصر `input` داخل عنصر `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label > input'));
```

يجب أن تعطي عنصر `input` الخاصية `type` بقيمة `file`.
