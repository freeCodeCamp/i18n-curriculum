...
El subtítulo también necesita ser estilizado. Crea un selector `.hero-subtitle` y asígnale una propiedad `font-size` establecida en `2.4rem`, una propiedad `color` establecida en `orangered`, y una propiedad `text-align` establecida en `center`.

# --hints--

Debe haber un selector `.hero-subtitle`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

Tu selector `.hero-subtitle` debe tener una propiedad `font-size` establecida en `2.4rem`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize, '2.4rem');
```

Tu selector `.hero-subtitle` debe tener una propiedad `color` establecida en `orangered`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color, 'orangered');
```

Tu selector `.hero-subtitle` debe tener una propiedad `text-align` establecida en `center`.
