...
Lenga chombo chako cha kubadilika kwa kichaguzi cha `h1 .flex`. Kipa sifa ya `display` iliyowekwa kuwa `flex` ili kuwezesha mpangilio wa flexbox. Kisha weka sifa ya `flex-direction` kuwa `column-reverse` - hii itaonyesha vipengele zilizopangwa ndani kutoka chini kwenda juu. Mwishowe, weka sifa ya `gap` kuwa `1rem` ili kuunda nafasi kati ya vipengele.
