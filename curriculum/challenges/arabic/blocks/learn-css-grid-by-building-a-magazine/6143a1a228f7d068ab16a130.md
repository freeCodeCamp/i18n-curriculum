...
أسفل عنصر `blockquote` الخاص بك، أضف عنصر `p` آخر بالنص التالي:
...
يجب أن تضيف عنصر `p` رابع إلى عنصر `.text` الخاص بك.

```js
assert(document.querySelectorAll('.text p')?.length === 5);
```

يجب أن يأتي عنصر `p` الجديد بعد عنصر `blockquote` الخاص بك.
