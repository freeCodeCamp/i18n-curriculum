...
Richte das `.formrow` Element am oberen Rand aus und gib ihm auf der linken und rechten Seite eine Padding-Einheit. Die anderen Padding-Werte sollten bei `0` liegen.

Vergrößere anschließend die Schriftgröße für alle `input` Elemente.

# --hints--

Du solltest einen `.formrow` Selektor verwenden, um die `.formrow` Elemente auszurichten.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.formrow'));
```

Du solltest dem `.formrow` ein `margin-top` von mindestens `1px` geben.
...
Du solltest dem `.formrow` ein `padding-top` von `0` zuweisen.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingTop, '0px');
```

Du solltest dem `.formrow` ein `padding-right` von mindestens `1px` geben.
...
Du solltest dem `.formrow` ein `padding-bottom` von `0` zuweisen.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingBottom, '0px');
```

Du solltest dem `.formrow` ein `padding-left` von mindestens `1px` geben.
...
Du solltest den `input` Selektor verwenden, um das `input` Element auszuwählen.
