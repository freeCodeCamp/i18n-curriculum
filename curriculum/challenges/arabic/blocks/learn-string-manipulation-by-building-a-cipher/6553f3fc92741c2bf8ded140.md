...
داخل جسم الدالة، أعد تسمية المتغيرين `text` و `shift` إلى `message` و `offset` على التوالي.

# --hints--

يجب إعادة تسمية كل ظهور لـ `text` إلى `message`.
...
يجب إعادة تسمية `shift` إلى `offset`.
