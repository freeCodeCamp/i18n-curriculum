...
`simulateFestival` 関数を完成させるために、シミュレーション結果の概要を表示してください。ローカルの `printSummary()` オブジェクトを使って `throughputSummary` を呼び出してください。

# --hints--

`printSummary()` を使って `throughputSummary` を呼び出す必要があります。
