...
`li` 요소 내에서 `item` 텍스트 위에 `input` 요소를 추가하고 `type` 속성을 `checkbox`로 설정하세요. 또한, `onChange` 속성을 `toggleItem()`을 인수로 하여 `item`을 호출하는 익명 함수로 설정하세요.

마지막으로, 더 나은 접근성을 위해 `input`와 `item` 텍스트를 `label` 요소로 감싸세요.

# --hints--

`input`로 설정된 `type` 속성을 가진 또 다른 `checkbox` 요소를 추가해야 합니다.
...
새로운 `input`에는 `onChange`을 인수로 하여 `toggleItem()`을 호출하는 익명 함수로 설정된 `item` 속성이 있어야 합니다.
...
새로운 `input`와 `item` 텍스트는 `label` 요소로 감싸야 합니다.
