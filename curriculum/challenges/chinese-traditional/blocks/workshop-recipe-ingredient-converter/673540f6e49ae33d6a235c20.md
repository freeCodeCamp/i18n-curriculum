...
你需要維護一個你的應用程式支援的單位列表。

宣告一個 `units` 陣列，並給它字串值 `"cup"`、`"gram"`、`"ounce"` 和 `"teaspoon"`。
...
你的 `units` 陣列應該包含字串 `"cup"`。

```js
assert.include(units, "cup");
```

你的 `units` 陣列應該包含字串 `"gram"`。

```js
assert.include(units, "gram");
```

你的 `units` 陣列應該包含字串 `"ounce"`。

```js
assert.include(units, "ounce");
```

你的 `units` 陣列應該包含字串 `"teaspoon"`。
