...
Bevor Sie weitermachen, entfernen Sie die `console.log`-Aufrufe aus Ihrem Code.

# --hints--

Sie sollten `console.log(removePlant(ballerina))` nicht in Ihrem Code haben.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*removePlant\s*\(\s*ballerina\s*\)\s*\)/);
```

Sie sollten `console.log(sellPlants(ballerina, "small", 10));` nicht in Ihrem Code haben.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*sellPlants\s*\(\s*ballerina\s*,\s*("|')small\1\s*,\s*10\s*\)\s*\)/);
```

Sie sollten `console.log(catalog)` nicht in Ihrem Code haben.
