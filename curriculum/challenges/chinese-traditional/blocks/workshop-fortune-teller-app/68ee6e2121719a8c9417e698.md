...
在你清除 `multipleCard` 後，於 `singleCardSelected` 方法內，呼叫 `showElements` 函式，並將 `singleCard` 作為第一個引數，`fortuneContainer` 作為第二個引數傳入。

# --hints--

你應該呼叫 `showElements` 函式。

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

你應該將 `elements.singleCard` 作為第一個引數傳入。

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

你應該將 `elements.fortuneContainer` 作為第二個引數傳入。
