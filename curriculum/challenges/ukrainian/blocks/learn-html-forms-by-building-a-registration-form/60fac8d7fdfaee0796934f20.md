...
Поточні варіанти випадаючого списку порожні. Щоб надати їм вміст, додайте наступний текст до кожного наступного елемента `option`:
...
Ви повинні додати текст `option` до четвертого елемента `freeCodeCamp Forum`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.textContent, 'freeCodeCamp Forum');
```

Ви повинні додати текст `option` до п’ятого елемента `Other`.
