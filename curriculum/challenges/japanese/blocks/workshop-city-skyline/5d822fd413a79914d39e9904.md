...
これまでに作成したグラデーションはすべて上から下への方向でしたが、これはデフォルトの方向です。色の前に方向を指定することで、別の方向に設定できます。
...
`.bb3`を`repeating-linear-gradient`で埋めてください。方向には`90deg`を使い、最初の2色には`building-color3`を、3色目には`window-color3`を`15%`で指定してください。色の距離を指定しない場合は、適切な値が自動的に使われます。この場合、最初の2色は`0%`と`7.5%`にデフォルト設定されます。なぜなら開始点が`0%`で、`7.5%`は`15%`の半分だからです。
...
3色目には`--window-color3`を`15%`で指定してください。
