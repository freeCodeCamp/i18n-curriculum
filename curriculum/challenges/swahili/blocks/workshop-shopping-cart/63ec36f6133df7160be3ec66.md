...
Kumbuka kwamba unaweza kutumia ufumbuzi wa muundo kutoa thamani nyingi kutoka kwenye safu ya data au kitu katika usemi mmoja.

Kwa kigezo cha kwanza cha kitendakazi chako cha mwitiko, fumbua vigezo vya `name`, `id`, `price`, na `category` kutoka kwa mfano wa `Dessert` uliotumwa.

# --hints--

Unapaswa kutumia ufumbuzi wa muundo kutangaza vigezo vya `name`, `id`, `price`, na `category`. Kwa jaribio hili, mpangilio ni muhimu.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

Ufumbuzi wako wa muundo unapaswa kuwa kigezo cha kwanza cha kitendakazi cha mwitiko.
