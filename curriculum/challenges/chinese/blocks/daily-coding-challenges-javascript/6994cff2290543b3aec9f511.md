---
id: 6994cff2290543b3aec9f511
title: "挑战 212：数组插入"
challengeType: 28
dashedName: challenge-212
---

# --description--

给定一个数组、一个要插入到数组中的值以及插入该值的索引，返回一个在指定索引插入该值的新建数组。

# --hints--

`insertIntoArray([2, 4, 8, 10], 6, 2)` 应该返回 `[2, 4, 6, 8, 10]`。

```js
assert.deepEqual(insertIntoArray([2, 4, 8, 10], 6, 2), [2, 4, 6, 8, 10]);
```

`insertIntoArray(["the", "quick", "fox"], "brown", 2)` 应该返回 `["the", "quick", "brown", "fox"]`。

```js
assert.deepEqual(insertIntoArray(["the", "quick", "fox"], "brown", 2), ["the", "quick", "brown", "fox"]);
```

`insertIntoArray([], 0, 0)` 应该返回 `[0]`。

```js
assert.deepEqual(insertIntoArray([], 0, 0), [0]);
```

`insertIntoArray([0, 1, 1, 2, 3, 8, 13], 5, 5)` 应该返回 `[0, 1, 1, 2, 3, 5, 8, 13]`。

```js
assert.deepEqual(insertIntoArray([0, 1, 1, 2, 3, 8, 13], 5, 5), [0, 1, 1, 2, 3, 5, 8, 13]);
```

# --seed--

## --seed-contents--

```js
function insertIntoArray(arr, value, index) {

  return arr;
}
```

# --solutions--

```js
function insertIntoArray(arr, value, index) {
  return [
    ...arr.slice(0, index),
    value,
    ...arr.slice(index)
  ];
}
```
