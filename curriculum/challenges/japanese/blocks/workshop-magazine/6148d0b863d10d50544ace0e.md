...
他のテキストがずれてしまっています。`.first-paragraph::first-letter` セレクターに `float` プロパティを `left` に、`margin-right` プロパティを `1rem` に設定して、正しい位置に移動させてください。

# --hints--

`.first-paragraph::first-letter` セレクターには `float` プロパティを `left` に設定してください。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

`.first-paragraph::first-letter` セレクターには `margin-right` プロパティを `1rem` に設定してください。
