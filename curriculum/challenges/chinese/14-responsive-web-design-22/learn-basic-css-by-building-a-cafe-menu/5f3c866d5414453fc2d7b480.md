...
你的第一个 `article` 元素应该包含 `p` 元素，其中包含文本 `French Vanilla` 和 `3.00`。
...
你的第二个 `article` 元素应具有 `p` 元素，其中包含文本 `Caramel Macchiato` 和 `3.75`。
...
你的第三个 `article` 元素应该包含 `p` 元素，其中包含文本 `Pumpkin Spice` 和 `3.50`。
...
你的第四个 `article` 元素应该包含 `p` 元素，其中包含文本 `Hazelnut` 和 `4.00`。
...
你的第五个 `article` 元素应该包含带有文本 `Mocha` 和 `4.50` 的 `p` 元素。
