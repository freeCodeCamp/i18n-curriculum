...
給 `.foreground-buildings` 添加與 `.background-buildings` 相同的 `display`、`align-items` 和 `justify-content` 屬性和值。 同樣，這將使用 Flexbox 將建築物均勻地分佈在其容器底部。

# --hints--

你應該給 `.foreground-buildings` 添加一個值爲 `flex` 的 `display` 屬性。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.display, 'flex');
```

你應該給 `.foreground-buildings` 添加一個值爲 `flex-end` 的 `align-items` 屬性。
