...
إذا حدث خطأ من استدعاء `fetch`، فإن كتلة `catch` ستتعامل معه.

داخل كتلة `catch`، أضف `console.log` لتسجيل معلمة `err`.

أيضًا، احذف `console.log(data);` من كتلة `try` الآن بعد أن فهمت ما يتم إرجاعه من استدعاء `fetch`.

# --hints--

يجب أن تستخدم `console.log` لتسجيل المتغير `err`.

```js
assert.match(code, /\s*console\.log\(\s*err\s*\)/);
```

يجب أن يكون `console.log` الخاص بك داخل كتلة `catch`.

```js
assert.match(code, /\s*try\s*{[^}]*\s*}\s*catch\s*\(\s*err\s*\)\s*{[^}]*\s*console\.log\(\s*err\s*\)[^}]*}/);
```

يجب أن تحذف `console.log(data);` من كتلة `try`.
