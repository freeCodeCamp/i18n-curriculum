...
A questo punto, vuoi mostrare i risultati solo per i nodi che non sono il nodo di partenza e che sono raggiungibili da esso.

Aggiungi una condizione che verifichi se `node_no` è uguale a `start_node` **oppure** se `distances[node_no]` è uguale a `INF`.

Se una delle due condizioni è vera, usa `continue` per passare all’iterazione successiva del loop.

# --hints--

Dovresti aggiungere un’istruzione condizionale all’interno del loop.
...
La tua istruzione `if` dovrebbe verificare se `node_no` è uguale a `start_node` *oppure* se `distances[node_no]` è uguale a `INF`.
...
Dovresti usare `continue` per passare all’iterazione successiva del loop.
