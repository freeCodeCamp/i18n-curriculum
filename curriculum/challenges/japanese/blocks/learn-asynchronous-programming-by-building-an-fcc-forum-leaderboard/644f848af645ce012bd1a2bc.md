...
`linkText` という定数を作成し、`selectedCategory.category` の値を代入してください。これにより、アンカー要素にカテゴリ名が表示されます。

# --hints--

`linkText` という定数があるはずです。

```js
assert(code.match(/const\s+linkText\s*=/));
```

`selectedCategory.category` 変数に `linkText` を代入してください。
