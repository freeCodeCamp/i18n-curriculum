...
입력 상자들을 서로 정렬하려면 `input` 요소 내의 모든 `label` 및 `.info` 요소를 대상으로 하는 새 규칙 집합을 만들고 `display` 속성을 `inline-block`로 설정하세요.

또한 `label` 요소의 텍스트를 오른쪽으로 정렬하세요.

# --hints--

`.info > label, .info > input` 또는 `.info label, .info input` 선택자 중 하나를 사용하세요.
...
`input` 및 `label` 요소에 `display`을 `inline-block`로 지정하세요.
...
`label` 요소에 `text-align`을 `right`로 지정하세요.
