...
使用 `const` 创建一个名为 `checkpointPositions` 的新数组。 
...
你应该有一个 `checkpointPositions` 数组。

```js
assert.isArray(checkpointPositions);
```

`checkpointPositions` 数组内应该有三个对象。

```js
assert.lengthOf(checkpointPositions, 3);
```

你应该有一个对象，其 `x` 属性设置为 `1170`，`y` 属性设置为 `proportionalSize(80)`，`z` 属性设置为 `1`。

```js
assert.deepStrictEqual(checkpointPositions[0], { x: 1170, y: proportionalSize(80), z: 1 });
```

你应该有一个对象，其 `x` 属性设置为 `2900`，`y` 属性设置为 `proportionalSize(330)`，`z` 属性设置为 `2`。

```js
assert.deepStrictEqual(checkpointPositions[1], { x: 2900, y: proportionalSize(330), z: 2 });
```

你应该有一个对象，其 `x` 属性设置为 `4800`，`y` 属性设置为 `proportionalSize(80)`，`z` 属性设置为 `3`。
