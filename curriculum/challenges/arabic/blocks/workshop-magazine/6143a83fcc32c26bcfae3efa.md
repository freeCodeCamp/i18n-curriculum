...
يجب أن يحتوي عنصر `section` الجديد على `class` بقيمة `text text-with-images`.
...
يجب أن يحتوي عنصر `article` على `class` بقيمة `brief-history`.

```js
assert.equal(document.querySelector('.text-with-images article')?.className, 'brief-history');
```

يجب أن يحتوي عنصر `aside` على `class` بقيمة `image-wrapper`.
