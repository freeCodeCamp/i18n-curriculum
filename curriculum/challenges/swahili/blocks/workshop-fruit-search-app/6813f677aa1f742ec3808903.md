...
Kwa block ya `catch`, ongeza kigezo cha `error`, kisha tumia `console.error` kurekodi kosa lolote linalotokea wakati wa kupata au kuchakata data.

# --hints--

Block yako ya `catch` inapaswa kuwa na kigezo cha `error`.

```js
assert.match(code, /catch\s*\(\s*error\s*\)\s*/);
```

Unapaswa kurekodi kosa lolote katika block ya `catch` kwa kutumia `console.error`.
