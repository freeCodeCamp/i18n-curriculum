...
Mionekano ya mwinuko inaweza kutumia rangi nyingi kama unavyotaka hivi:
...
Ongeza `linear-gradient` kwa `.bb1d` ukiwa na `orange` kama rangi ya kwanza, `--building-color1` kama ya pili, na `--window-color1` kama ya tatu. Kumbuka kutumia mwinuko kwenye sifa ya `background`.
...
Unapaswa kumpa sifa ya `background` `linear-gradient`.
