...
创建一个名为 `linkClass` 的常量，并为其赋值一个模板字面量。 在该模板字面量中，添加 `category ${selectedCategory.className}` 的值。

这些类名将用于为锚元素应用样式。

# --hints--

你应该有一个名为 `linkClass` 的常量。

```js
assert(code.match(/const\s+linkClass\s*=/g));
```

你应该为 `linkClass` 变量赋值一个模板字面量。

```js
assert(code.match(/const\s+linkClass\s*=\s*`.*`\s*/g));
```

你应该在模板字面量中使用 `category ${selectedCategory.className}` 的值。
