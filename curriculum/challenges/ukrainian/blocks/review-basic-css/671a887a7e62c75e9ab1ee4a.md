...
Ось загальний синтаксис CSS-правила:
...
- **Default Browser Styles**: Each HTML element will have default browser styles applied to them. Зазвичай сюди входять такі елементи, як поля і відступи за замовчуванням.

## Inline, Internal, and External CSS

- **Inline CSS**: These styles are written directly within an HTML element using the `style` attribute. Здебільшого ви не будете використовувати вбудований CSS через поділ завдань.
...
- **Властивість `width`**: Ця властивість вказує ширину елемента. If you do not specify a width, then the default is set to `auto`. This lets the browser determine the element's width based on its content, parent, and display type.
...
- <strong x-id=«1»>Комбінатор похідних</strong>: Цей комбінатор використовується для пошуку елементів, які є похідними від вказаного батьківського елемента. У наступному прикладі ми розглянемо всі елементи `li` всередині елементів `ul`.
...
- **Child Combinator (`>`)**: This combinator is used to select elements that are direct children of a specified parent element. Наступний приклад буде націлений на всі елементи `p`, які є прямими дочірніми елементами класу `container`.
...
- **Subsequent-sibling Combinator (`~`)**: This combinator selects all siblings of a specified element that come after it. Наступний приклад стилізує лише другий параграф, оскільки він єдиний, який є братським для елемента `ul` і має таке ж походження.
...
- **Inline Level Elements**: Inline elements only take up as much width as they need and do not start on a new line. Ці елементи рухаються разом з потоком вмісту, дозволяючи тексту і іншим рядковим елементам з'являтися поруч з ними. До поширених рядкових елементів належать елементи `span`, `anchor` та `img`.
- **Block Level Elements**: Block-level elements start on a new line and take up the full width available to them by default, stretching across the width of their container. До деяких поширених блокових елементів належать елементи `div`, `paragraph` та `section`.
...
Пригадайте поняття з теми «Основи CSS».
