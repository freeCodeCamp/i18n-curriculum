...
Verwenden Sie eine `for`-Schleife, um den `name` jedes Astronauten im `updatedCrew`-Array zu protokollieren. Geben Sie nach der Schleife das `updatedCrew`-Array zurück, um Ihre `swapCrewMembers`-Funktion abzuschließen.

# --hints--

Sie sollten eine `for`-Schleife verwenden, um das `updatedCrew`-Array zu durchlaufen und den `name` jedes Astronauten zu protokollieren.
...
Sie sollten das `updatedCrew`-Array zurückgeben, nachdem Sie die Namen der Astronauten protokolliert haben.
