...
أعطِ `wave` أربع نقاط توقف تبدأ من `10%` وتزداد بمقدار `10%`.

# --hints--

يجب إضافة نقطة توقف `10%` لـ `@keyframes wave`.
...
يجب إضافة نقطة توقف `20%` لـ `@keyframes wave`.
...
يجب إضافة نقطة توقف `30%` لـ `@keyframes wave`.
...
يجب إضافة نقطة توقف `40%` لـ `@keyframes wave`.
...
يجب أن تحتوي الإطارات المفتاحية على 4 نقاط توقف.
