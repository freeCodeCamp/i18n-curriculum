...
Dentro de la clase `Spider`, crea un constructor que reciba un solo parámetro `emojiElement` de tipo `HTMLParagraphElement`. 

Dentro del constructor, llama a la función `super` con el parámetro `emojiElement` para invocar el constructor de la clase padre `Bug`. Luego, asigna la propiedad `emoji` con el valor de cadena `"🕷️"`.

# --hints--

Tu clase `Spider` debe tener un constructor.
...
El constructor `Spider` debe tener un parámetro llamado `emojiElement` de tipo `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

El constructor `Spider` debe llamar a `super` con el parámetro `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?super\s*\(\s*emojiElement\s*\)/);
```

El constructor `Spider` debe asignar `this.emoji` a `"🕷️"`.
