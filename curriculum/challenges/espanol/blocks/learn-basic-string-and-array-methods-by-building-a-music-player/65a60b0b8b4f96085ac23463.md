...
En el ejemplo, si `a.name` es igual a `b.name`, entonces la función devuelve `0`. Esto significa que nada cambia y el orden de `a` y `b` permanece igual.
...
Debajo de tus sentencias `if`, retorna el número `0` para dejar el orden de los dos elementos sin cambios.

# --hints--

Debes retornar el número `0` debajo de tus sentencias `if`.
