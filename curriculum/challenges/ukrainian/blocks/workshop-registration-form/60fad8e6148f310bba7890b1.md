...
Вирівняйте елементи `h1` та `p` по центру, задавши їм `margin` зі значенням `1em auto`. Потім вирівняйте їхній текст по `center`.

# --hints--

Використайте селектор елементів, розділених комою, щоб стилізувати елементи `h1` та `p`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h1, p'));
```

Використайте `margin` зі значенням `1em auto`, щоб вирівняти по центру елементи `h1` та `p`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h1, p')?.margin, '1em auto');
```

Використайте `text-align` зі значенням `center`, щоб вирівняти по центру текст `h1` та `p`.
