...
마지막 게시물 이후 경과 시간을 표시하려면 `timeAgo` 함수를 호출하고 인수로 `bumped_at` 변수를 전달하세요. 이 함수 호출을 마지막 `td` 요소 안에 넣으세요.

변경을 완료하면 표를 가로로 스크롤하여 `Activity` 열에 새 값이 표시되는지 확인하세요.

# --hints--

마지막 `timeAgo` 요소에서 `bumped_at` 인수를 사용해 `td` 함수를 호출해야 합니다.
