...
`make_allowed_move()`는 `rod1`과 `rod2`를 매개변수로 받습니다. 여기서 약간의 리팩터링이 필요합니다. `source`가 나오는 모든 부분을 `rod1`로 바꾸세요.

# --hints--

`source` 함수 안에 있는 `make_allowed_move()`의 모든 부분을 `rod1`로 바꾸세요.
