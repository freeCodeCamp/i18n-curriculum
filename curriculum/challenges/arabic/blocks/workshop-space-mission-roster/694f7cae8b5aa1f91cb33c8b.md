...
كما في السابق، يجب التحقق من المُدخَل. بالتحديد، إذا كان `size` أقل من `1`، يجب تسجيل الرسالة `"Chunk size must be >= 1"` في وحدة التحكم ثم استدعاء `return` للخروج من الدالة.

# --hints--

إذا كان المُدخَل `size` غير صالح، يجب تسجيل رسالة الخطأ التالية في وحدة التحكم: `"Chunk size must be >= 1"`.
...
بعد تسجيل رسالة الخطأ، يجب استدعاء `return` فورًا للخروج المبكر من الدالة.
...
يجب أن تمر دالة `chunkCrew` بالتحقق دون أخطاء عندما يكون `size` أكبر من أو يساوي 1.
