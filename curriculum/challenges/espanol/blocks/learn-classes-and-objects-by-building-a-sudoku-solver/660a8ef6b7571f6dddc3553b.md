...
Dale un solo parámetro `board` que será tu lista 2D.


# --hints--

Debes definir una nueva función llamada `solve_sudoku` fuera de la clase `Board`.

```js
({ test: () => assert(runPython(`_Node(_code).has_function("solve_sudoku")`)) })
```

Tu función `solve_sudoku` debe tener un solo parámetro `board`.
