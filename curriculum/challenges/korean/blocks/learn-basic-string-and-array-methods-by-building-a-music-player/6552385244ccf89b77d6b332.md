...
다음 노래와 이전 노래를 재생하는 작업을 해야 합니다. 이를 위해 `playNextSong`와 `playPreviousSong` 함수를 만들어야 합니다.

`const`와 화살표 구문을 사용해 빈 `playNextSong` 함수를 만드세요.

# --hints--

빈 함수 `const`를 만들려면 `playNextSong`를 사용하세요.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong`는 함수여야 합니다.

```js
assert.isFunction(playNextSong)
```

`playNextSong` 함수는 화살표 구문을 사용해야 합니다.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

`playNextSong` 함수는 비어 있어야 합니다.
