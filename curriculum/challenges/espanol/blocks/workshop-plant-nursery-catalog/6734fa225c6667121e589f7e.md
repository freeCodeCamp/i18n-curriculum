...
`sellPlants(ballerina, "small", 10)` debe devolver `Item not found.`

```js
assert.equal(sellPlants(ballerina, "small", 10), "Item not found.")
```

`sellPlants(hidcote, "large", 95)` debería devolver `Not enough large size pots for Lavandula angustifolia 'Hidcote'. Only 18 left.`
