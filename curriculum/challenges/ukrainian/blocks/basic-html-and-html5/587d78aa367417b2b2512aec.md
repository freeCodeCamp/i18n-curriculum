...
title: Визначте head і body документа HTML
challengeType: 0
...
Ви можете додати ще один рівень організації у ваш HTML-документ всередині тегів `html` за допомогою елементів `head` і `body`. Будь-яка розмітка з інформацією про вашу сторінку має бути всередині тегу `head`. Тоді будь-яка розмітка з вмістом сторінки (те, що відображається користувачу) має бути всередині тегу `body`.

Елементи метаданих, такі як `link`, `meta`, `title` і `style`, зазвичай розміщують всередині елемента `head`.
...
Відредагуйте розмітку так, щоб там були `head` і `body`. Елемент `head` повинен містити лише `title`, а елемент `body` — лише `h1` і `p`.

# --hints--

На сторінці має бути лише один елемент `head`.
...
На сторінці має бути лише один елемент `body`.
...
Елемент `head` має бути дочірнім елементом `html`.
...
Елемент `body` має бути дочірнім елементом `html`.
...
Елемент `head` має охоплювати елемент `title`.
...
Елемент `body` має охоплювати обидва елементи `h1` і `p`.
