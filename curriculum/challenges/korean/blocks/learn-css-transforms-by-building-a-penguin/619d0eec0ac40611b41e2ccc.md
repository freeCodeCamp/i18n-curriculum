...
`.eye.right` 요소 아래에 `div`의 `class`를 가진 `blush` 요소 두 개를 추가하세요. 또한, 첫 번째 `.blush` 요소에는 `class`의 `left`를, 두 번째 `.blush` 요소에는 `class`의 `right`를 부여하세요.
