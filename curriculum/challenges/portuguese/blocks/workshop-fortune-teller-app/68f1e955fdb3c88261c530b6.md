...
Adicione uma declaração `if` para verificar se `foundCard` existe antes de atualizar os elementos com seus dados ou bloco de conteúdo.

# --hints--

Você deve declarar uma declaração `if` com a condição `foundCard`.
