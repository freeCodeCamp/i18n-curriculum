...
最後に 2 つ目の `label` の後に、新しい行で、`input` 要素をもう 1 つ作成してください。 この `input` 要素については、`type` 属性を `number` に設定し、`min` 属性を `0` に (カロリーに負の値を追加できないようにするため)、`placeholder` 属性を `Calories` に設定してください。そして 2 つ目の `label` 要素の `for` 属性と一致するように `id` 属性を設定してください。

# --hints--

`HTMLString` 内に `input` 要素が 2 つ必要です。
...
新しい `input` 要素は 2 つ目の `label` 要素の後に来る必要があります。
...
新しい `input` 要素の `type` 属性は `number` に設定してください。
...
`input` 要素の `placeholder` 属性を `Calories` に設定してください。
...
`input` 要素の `id` 属性を `${entryDropdown.value}-${entryNumber}-calories` に設定してください。
