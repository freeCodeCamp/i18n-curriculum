...
JavaScript で HTML 要素の値の変更を検出するには、`change` イベントを使います。
...
`dateOptionsSelectElement` に `addEventListener` メソッドを書き加えてください。 イベントリスナーの最初の引数は文字列 `"change"`、2 番目の引数は空のアロー関数としてください。

# --hints--

You should attach the `addEventListener` method to the `dateOptionsSelectElement`.
