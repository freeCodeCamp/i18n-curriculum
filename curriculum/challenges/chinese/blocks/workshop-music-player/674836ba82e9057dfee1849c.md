...
接下来，使用 `classList` 属性和 `add()` 方法为 `playButton` 元素添加 `playing` 类。这将查找 CSS 文件中的 `playing` 类并将其添加到 `playButton` 元素。

然后，将 `userData.currentSong` 设置为 `song`，最后使用 `audio` 变量上的 `play()` 方法播放歌曲。`play()` 是用于播放 mp3 文件的 web 音频 API 的一个方法。

# --hints--

你应该在你的 `playSong` 函数中为 `playButton` 元素添加 `playing` 类。
...
你应该在你的 `playSong` 函数中将 `userData.currentSong` 设置为 `song`。
...
你应该在你的 `playSong` 函数内对 `audio` 变量调用 `play()` 方法。
