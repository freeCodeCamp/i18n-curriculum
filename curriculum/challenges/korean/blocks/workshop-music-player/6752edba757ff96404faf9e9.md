...
애플리케이션의 접근성을 높이려면 재생 버튼이 현재 노래 또는 재생목록의 첫 번째 노래를 설명하는 것이 중요합니다.

`setPlayButtonAccessibleText`라는 이름의 함수를 만드세요. 함수 안에서 재생 버튼의 `aria-label` 속성을 `Play` 다음에 공백과 현재 노래 제목을 이어서 설정하세요. 만약 `userData.currentSong`가 `null`라면, `Play`로 설정하세요.

# --hints--

`setPlayButtonAccessibleText`라는 이름의 함수가 있어야 합니다.

```js
assert.isFunction(setPlayButtonAccessibleText);
```

`setPlayButtonAccessibleText` 함수는 재생 버튼의 `aria-label` 속성을 `Play` 다음에 공백과 현재 노래 제목을 이어서 설정해야 합니다.
...
```
 ...
 `userData.currentSong`가 `null`일 때, `setPlayButtonAccessibleText`는 재생 버튼의 `aria-label` 속성을 `Play`로 설정해야 합니다.
