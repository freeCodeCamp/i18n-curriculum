...
Seguendo lo stesso pattern, effettua la query per i tuoi input `number` negli elementi `#dinner`, `#snacks` e `#exercise`. Assegna questi valori a variabili seguendo lo schema di denominazione dei due precedenti.
...
Dovresti assegnare il risultato della tua chiamata `document.querySelectorAll()` a `dinnerNumberInputs`.
...
Dovresti assegnare il risultato della tua chiamata `document.querySelectorAll()` a `snacksNumberInputs`.
...
Dovresti assegnare il risultato della tua chiamata `document.querySelectorAll()` a `exerciseNumberInputs`.
