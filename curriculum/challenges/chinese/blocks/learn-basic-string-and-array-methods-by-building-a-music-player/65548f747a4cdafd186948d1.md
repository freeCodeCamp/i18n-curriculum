...
你需要获取当前正在播放的歌曲的 `id`。 为此，你可以使用 `userData?.currentSong?.id`。

使用 `getElementById()` 获取当前播放的歌曲的 `id`，然后使用模板文字在其前面加上 `song-`。 将其分配给常量 `songToHighlight`。
...
你应该将 `getElementById()` 分配给 `songToHighlight` 常量。
