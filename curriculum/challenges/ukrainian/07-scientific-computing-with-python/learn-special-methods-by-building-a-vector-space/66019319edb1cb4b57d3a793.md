...
Останній потрібний вам метод — `__ge__`, який викликається, коли `>=` використовується для порівняння двох об’єктів.

Визначте метод `__ge__` із параметрами `self` та `other` і змусьте його повертати значення, протилежне `self < other`.
...
Метод `__ge__` має повернути протилежність `self < other`.
