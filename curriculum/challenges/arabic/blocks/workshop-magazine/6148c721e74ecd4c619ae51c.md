...
عنصر `.text` الخاص بك ليس شبكة CSS، لكن يمكنك إنشاء أعمدة داخل عنصر دون استخدام الشبكة عبر خاصية `column-width`.

أضف إلى محدِّد `.text` خاصية `column-width` بقيمة `25rem`.

# --hints--

يجب أن يحتوي محدِّد `.text` على خاصية `column-width` بقيمة `25rem`.
