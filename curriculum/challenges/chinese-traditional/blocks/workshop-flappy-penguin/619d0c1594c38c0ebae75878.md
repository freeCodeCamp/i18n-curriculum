...
將帶有 `left` 類別的 `.eye` 元素標的，並將其從父元素的左側定位在 `25%`。然後，將帶有 `right` 類別的 `.eye` 元素標的，並將其從父元素的右側定位在 `25%`。
