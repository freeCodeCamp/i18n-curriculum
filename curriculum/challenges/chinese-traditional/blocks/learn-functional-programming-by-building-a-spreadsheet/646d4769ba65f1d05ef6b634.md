...
現在你的 `update()` 函式（程式）可以實際計算公式。請記得你寫了 `if` 條件來檢查是否有呼叫函式（程式）。

在你的 `if` 述語內，將 `element` 的 `value` 設定為你的 `evalFormula()` 函式（程式）的結果。暫時不要傳遞任何引數。

# --hints--

你應該在你的 `if` 區塊中更新 `element` 的 `value` 屬性。
...
你應該將 `value` 屬性指定為呼叫你的 `evalFormula()` 函式（程式）的結果。
