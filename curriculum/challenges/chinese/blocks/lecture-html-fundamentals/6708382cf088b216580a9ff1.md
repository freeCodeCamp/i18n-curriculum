...
你可以在你的 JavaScript 或 CSS 中引用 `id` 的 `title` 名称。以下是一个 CSS 示例，引用 `id` 的 `title` 来将文本 `color` 更改为 `red`。
...
`#` 前面的井号符号（`title`）告诉计算机你想要定位具有该值的 `id`。`id` 名称不应被多次使用，并且它们必须始终是唯一的。关于 `id` 值的另一个需要注意的事情是它们不能有空格。 这是一个是将单词 `main` 和 `heading` 应用于一个 `id` 属性值的示例：
