...
Verifique se `cardElement` não é uma instância de `HTMLElement` e, se for, adicione um `return`

# --hints--

Você deve adicionar outra declaração `if` com a condição `!(cardElement instanceof HTMLElement)`.
