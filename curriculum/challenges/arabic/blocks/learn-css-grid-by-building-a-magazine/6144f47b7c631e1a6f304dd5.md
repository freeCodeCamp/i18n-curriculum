...
سيحتوي مجلتك على ثلاثة أقسام رئيسية. لقد ضبطت بالفعل التخطيط العام في قاعدة `main`، لكن يمكنك تعديل الموضع في القواعد الفرعية.

أحد الخيارات هو خاصية `grid-column`، وهي اختصار لـ `grid-column-start` و `grid-column-end`. تخبر خاصية `grid-column` العنصر الشبكي بأي خط شبكة يبدأ وينتهي عنده.

أنشئ قاعدة `.heading` واضبط خاصية `grid-column` على `2 / 3`. هذا سيخبر عنصر `.heading` أن يبدأ عند خط الشبكة 2 وينتهي عند خط الشبكة 3.

# --hints--

يجب أن يكون لديك محدِّد `.heading`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.heading'));
```

يجب أن يحتوي محدِّد `.heading` على خاصية `grid-column` مضبوطة على `2 / 3`.
