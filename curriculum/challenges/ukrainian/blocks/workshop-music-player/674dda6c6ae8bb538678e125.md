...
Add an `if` statement to check if no current song is playing by verifying that `userData.currentSong` is falsy. В межах блока `if` встановіть властивість `currentTime` об’єкта `audio` на `0`.
