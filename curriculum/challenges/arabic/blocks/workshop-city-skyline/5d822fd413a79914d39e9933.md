...
أضف `repeating-linear-gradient` آخر تحت الذي أضفته للتو. اجعله باتجاه `90deg`، واستخدم لون المبنى من `0%` إلى `12%` ولون النافذة من `12%` إلى `44%`. هذا سيُنتج مجموعة من النوافذ المستطيلة.
...
يجب أن تعطي `repeating-linear-gradient` الثاني اللون الأول `--building-color2` من `0%` إلى `12%`.
...
يجب أن تعطي `repeating-linear-gradient` الثاني اللون الثاني `--window-color2` من `12%` إلى `44%`.
