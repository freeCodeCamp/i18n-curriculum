...
After the `while` loop, set `text.innerText` to equal `"You picked <someGuess>. Here are the random numbers:"`. Замініть `<someGuess>` на параметр функції `guess`.

# --hints--

Встановіть `text.innerText` на рядок `"You picked "`. Пам'ятайте, що інтервал має значення.

```js
assert.match(pick.toString(), /text\.innerText\s*=\s*('|")You picked \1/);
```

Використайте оператор конкатенації, щоб додати значення параметра `guess` до рядка `"You picked "`.
