...
移動到列表元素，為你的 `ul` 添加類別 `mt-6`、`space-y-2`、`text-sm` 和 `text-fuchsia-100`，以便為列表項目增加間距並使其看起來更吸引人。
...
你的 `ul` 元素應該有 `text-fuchsia-100` 類別。
