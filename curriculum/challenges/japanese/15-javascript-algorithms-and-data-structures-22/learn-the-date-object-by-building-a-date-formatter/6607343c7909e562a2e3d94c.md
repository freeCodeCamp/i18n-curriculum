...
日付を `yyyy-mm-dd` の形式に整形するため、<dfn>split</dfn>、<dfn>reverse</dfn>、<dfn>join</dfn> のメソッドが必要です。 ですがまずは、このプロジェクトに適用する方法を理解しやすくするために、練習用の例からいくつか見ていきましょう。

`split()` メソッドは、指定された文字に基づいて文字列を部分文字列に分割します。 そして、部分文字列を配列要素とする配列を返します。

こちらの例では、`"Hello World"` というテキストを取り、要素が 1 つだけの配列を返しています:
...
では、新しく `exampleSentence` という名前の `const` 変数を作成し、その変数に `"selur pmaCedoCeerf".split()` の結果を代入してください。

その後、`exampleSentence` の値をログに出力する console 文を追加してください。 コンソールを開いて結果を確認してください。
