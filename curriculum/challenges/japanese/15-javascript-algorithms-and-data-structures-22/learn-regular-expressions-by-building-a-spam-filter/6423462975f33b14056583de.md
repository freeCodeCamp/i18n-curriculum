...
文字クラスには、2 文字以上を含めることもできます。 `a` の文字を、`a`、`@`、`4` とマッチする文字クラスに置き換えてください。

# --hints--

`stockRegex` では文字クラスを使用して、`a`、`@`、および `4` にマッチさせてください。
...
`stockRegex` が `stock @lert` にマッチするようにしてください。

```js
assert.match('stock @lert', stockRegex);
```

`stockRegex` が `stock 4lert` にマッチするようにしてください。

```js
assert.match('stock 4lert', stockRegex);
```

`stockRegex` が `570ck 4l3r7` にマッチするようにしてください。
