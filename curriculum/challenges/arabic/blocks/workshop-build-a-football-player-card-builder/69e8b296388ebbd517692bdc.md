...
أضف خاصية `value` بقيمة `player.sho` إلى `input` الخاص بـ SHO. ثم اضبط الخاصية `onChange` الخاصة به على دالة سهمية تأخذ `e` كمعلمة لها وتستدعي `setPlayer` مع نشر `player` وضبط `sho` على `Number(e.target.value)`.

# --hints--

يجب أن يحتوي `input` الخاص بـ SHO على `value` تساوي قيمة `sho` الحالية للاعب.
...
يجب أن يحتوي `input` الخاص بـ SHO على معالِج `onChange` يستدعي `setPlayer` مع نشر `player` وضبط `sho` على `Number(e.target.value)`.
