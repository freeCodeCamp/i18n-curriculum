...
要使山看起来更像一座山，可以使用 `skew` 变换函数，它需要两个参数。 第一个是沿 x 轴倾斜的角度，第二个是沿 y 轴倾斜的角度。

使用 `transform` 属性来使山体沿 x 轴倾斜 `0deg`，沿 y 轴倾斜 `44deg`。

# --hints--

`.left-mountain` 应该有一个 `transform` 属性。

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.transform);
```

你应该在 `transform` 中使用 `skew` 函数。

```js
assert.include(new __helpers.CSSHelp(document).getStyle('.left-mountain')?.transform, 'skew');
```

你应该将 `.left-mountain` 的 `transform` 设置为 `skew(0deg, 44deg)`。
