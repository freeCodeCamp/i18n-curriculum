...
عندما يكون شرط `if` صحيحًا، يجب عليك تبديل العنصرين، بحيث "تطفو" القيمة الأكبر نحو نهاية المصفوفة.

للقيام بذلك، أعلن عن متغير `temp` وعيّنه بقيمة `array[j]`. ثم عيّن `array[j]` بقيمة `array[j + 1]`. وأخيرًا، عيّن `array[j + 1]` بقيمة `temp`.

# --hints--

داخل جملة `if`، يجب أن تعلن عن متغير `temp`.
...
يجب أن تعيّن `temp` بقيمة `array[j]`.
...
يجب أن تعيّن `array[j]` بقيمة `array[j + 1]`.
...
يجب أن تعيّن `array[j + 1]` بقيمة `temp`.
