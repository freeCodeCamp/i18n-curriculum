...
Erstellen Sie ein `ul`-Element innerhalb des `nav`-Elements. Erstellen Sie innerhalb dieses `ul`-Elements drei Listenelemente.

# --hints--

Sie sollten ein `ul`-Element innerhalb Ihres `nav`-Elements erstellen.

```js
assert.exists(document.querySelector('nav ul'));
```

Sie sollten drei `li`-Elemente innerhalb Ihres `ul`-Elements erstellen.
