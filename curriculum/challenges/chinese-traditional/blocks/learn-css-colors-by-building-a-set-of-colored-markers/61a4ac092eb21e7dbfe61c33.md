...
現在修改 `linear-gradient` 函式（程式）中第二個顏色引數，目前是純綠色。

更新 `rgb` 函式（程式），使紅色的值為 `245`，綠色的值為 `62`，藍色的值為 `113`。

# --hints--

你的 `.red` CSS 規則應該有一個 `background` 屬性，設定為 `linear-gradient(180deg, rgb(122, 74, 14) 0%, rgb(245, 62, 113) 50%, rgb(0, 0, 255) 100%)`。
