...
- Registre a mensagem `"\nTick " + (tickIndex + 1)` no console.
- Faça um loop por cada portão no array `gates` e chame `handleGateAtTick()` com estes argumentos:
  - `gates`: o array completo de entrada dos portões.
  - `gate`: o objeto do portão atual.
  - `tickIndex`: o tick atual.
  - `throughputSummary`: o objeto local que rastreia o total de participantes processados por portão.
- Incremente `tickIndex` em 1 para avançar para o próximo tick.

# --hints--

Você deve registrar a seguinte mensagem no console, `"\nTick " + (tickIndex + 1)`.
...
Após registrar no console, você deve fazer um loop por cada portão no array `gates` e chamar `handleGateAtTick()` com `gates`, `gate`, `tickIndex` e `throughputSummary`.
...
Após seu loop pelo array `gates`, você deve incrementar `tickIndex` em 1.
