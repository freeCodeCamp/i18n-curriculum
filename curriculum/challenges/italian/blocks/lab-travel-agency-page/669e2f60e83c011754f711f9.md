...
1. Dovresti avere un elemento `h1` per presentare le tue destinazioni di viaggio.
...
1. Dovresti avere un elemento lista non ordinata con due elementi di lista. I due elementi di lista dovrebbero avere rispettivamente il testo `Group Travels` e `Private Tours`. Il testo di ogni elemento di lista dovrebbe essere racchiuso da un elemento àncora.
1. Dovresti avere un elemento `h2` con il testo `Top Itineraries`.
1. Dovresti avere almeno tre elementi `figure`, ciascuno contenente un elemento àncora e un elemento `figcaption`.
...
1. Tutti e cinque i tuoi elementi àncora dovrebbero avere un attributo `href` con il valore `https://www.freecodecamp.org/learn` e un attributo `target` con il valore `_blank`.
