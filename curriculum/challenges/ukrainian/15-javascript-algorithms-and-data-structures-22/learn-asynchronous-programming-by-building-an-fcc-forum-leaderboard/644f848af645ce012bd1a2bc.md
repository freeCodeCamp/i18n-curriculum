...
Створіть константу під назвою `linkText` та призначте до неї значення `selectedCategory.category`. Це покаже назву категорії в елементі прив’язки.

# --hints--

Ви повинні мати константу під назвою `linkText`.

```js
assert(code.match(/const\s+linkText\s*=/));
```

Призначте `selectedCategory.category` до змінної `linkText`.
