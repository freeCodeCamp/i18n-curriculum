...
Nach Ihrer verschachtelten `for`-Schleife haben Sie den kleinsten Wert gefunden. Sie müssen ihn mit Ihrem aktuellen Wert tauschen.

Wie Sie es bei Ihrem Bubble-Sort gemacht haben, verwenden Sie eine `temp`-Variable, um den Wert an `array[i]` zu extrahieren, und tauschen dann die Werte an `array[i]` und `array[minIndex]`.

# --hints--

Nach Ihrer verschachtelten `for`-Schleife sollten Sie eine `temp`-Variable deklarieren.
...
Sie sollten `array[i]` `temp` zuweisen.
...
Sie sollten `array[minIndex]` `array[i]` zuweisen.
...
Sie sollten `temp` `array[minIndex]` zuweisen.
