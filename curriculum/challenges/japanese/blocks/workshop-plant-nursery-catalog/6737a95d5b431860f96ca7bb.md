...
今度は、関数内で `ballerina` に `catalogSet` オブジェクトをもう一度追加してみてください。すでにセットにそのオブジェクトが保存されているため、何も起こらないことがわかります。セットには重複した要素を入れることができません。

必要であれば、以前にマップで行ったのと同じように、`size` プロパティを使ってセット内の要素数を確認できます。

# --hints--

関数 `add` 内で `ballerina` オブジェクトをもう一度セットに `displayPlantsSet` する必要があります。
