...
依照相同的樣式，查詢 `#dinner`、`#snacks` 和 `#exercise` 元素中的 `number` 輸入。依照前兩個的命名結構綱目將它們指定到變數。

# --hints--

你應該宣告一個 `dinnerNumberInputs` 變數。

```js
assert.match(calculateCalories.toString(), /dinnerNumberInputs\s*=/);
```

你應該使用 `#dinner input[type='number']` 呼叫 `document.querySelectorAll()`。

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#dinner input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你應該將 `document.querySelectorAll()` 呼叫的結果指定給 `dinnerNumberInputs`。
