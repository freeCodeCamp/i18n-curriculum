...
アクティブなときに`.card-links a`要素を対象とする新しいセレクターを追加してください。`background-color`を`midnightblue`に設定してください。

# --hints--

`.card-links a:active`セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".card-links a:active"));
```

`.card-links a:active`セレクターは`background-color`の`midnightblue`を持つべきです。
