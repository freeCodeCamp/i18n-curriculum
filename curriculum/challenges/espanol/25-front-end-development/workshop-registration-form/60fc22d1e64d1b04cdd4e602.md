...
Agrega algo de espacio entre los elementos `.inline` y el texto de `label`, dando un `margin` derecho de `0.5em`. Además, establece todos los otros márgenes a `0`.

# --hints--

Debe darle a los elementos `.inline` un `margin-top` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginTop, '0px');
```

Debe darle a los elementos `.inline` un `margin-right` de `0.5em`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginRight, '0.5em');
```

Debe darle a los elementos `.inline` un `margin-bottom` de `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.inline')?.marginBottom, '0px');
```

Debe darle a los elementos `.inline` un `margin-left` de `0`.
