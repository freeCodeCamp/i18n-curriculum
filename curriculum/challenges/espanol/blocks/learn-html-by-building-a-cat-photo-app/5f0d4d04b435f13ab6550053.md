...
Tu elemento `legend` debe tener una etiqueta de cierre. Las etiquetas de cierre tienen un `/` justo después del carácter `<`.

```js
assert(code.match(/<\/legend\>/g).length === 2);
```

El elemento `legend` debe tener el texto `What's your cat's personality?`. Has omitido el texto o hay un error tipográfico.
