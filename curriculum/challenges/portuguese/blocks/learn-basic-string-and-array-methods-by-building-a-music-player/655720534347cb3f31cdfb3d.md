...
Antes de deletar uma música, você precisa verificar se a música está tocando no momento.Se for, você precisa pausar a música e tocar a próxima música na playlist. 

Use uma declaração `if` para verificar se o `userData?.currentSong?.id` é igual ao `id` da música que você deseja excluir. 

# --hints--

Você deve criar uma declaração `if` vazia com a condição `userData?.currentSong?.id === id`.
