...
回到 `initializeApp` 函式（程式），這裡你將使用先前創建的行內型別和 `getElement()` 函式（程式），透過取得應用程式所需的 DOM 元素來加以運用。

在 `initializeApp` 函式（程式）內，透過呼叫 `shapeTypeSelect` 並傳入 `getElement()`，重新指定 `"shape-type"`，接著使用 `as` 關鍵字將結果轉型為 `HTMLSelectElement`。

# --hints--

你應該用適當的值重新指定 `shapeTypeSelect`。

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*/)
```

你應該呼叫 `getElement` 函式（程式）並傳入 `shape-type`，然後將結果指定給 `shapeTypeSelect`。

```js
assert.match(initializeApp.toString(), /shapeTypeSelect\s*=\s*getElement\(["']shape-type["']\);?/)
```

你應該使用 `getElement` 關鍵字，將 `HTMLSelectElement` 作為 `shapeTypeSelect` 的型別，並呼叫 `as`。
