...
使用相同的插值語法，添加第二個 `p` 元素，其文本爲 `consumedCalories Calories Consumed`，並添加第三個元素，其文本爲 `exerciseCalories Calories Burned`。 請記住用適當的值替換 `consumedCalories` 和 `exerciseCalories` 變量。
...
你的第二個 `p` 元素應包含文本 `${consumedCalories} Calories Consumed`。
