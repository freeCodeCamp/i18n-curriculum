...
`elif` 文を書いて、`key > node.key` かどうかを確認してください。

`elif` ブロック内で、現在のノードの右の子ノードと `_delete` を引数として `key` メソッドを呼び出し、その結果を右のノードに代入してください。
...
`_delete` メソッドを `node.right` と `key` を引数にして呼び出してください。

```js
assert.match(code, /self\._delete\(\s*node\.right\s*,\s*key\s*\)/);
```

`_delete()` の呼び出し結果を `node.right` に代入してください。
