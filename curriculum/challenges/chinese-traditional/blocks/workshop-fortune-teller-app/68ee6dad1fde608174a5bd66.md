...
在 `singleCardSelected` 方法中，取得 `singleCard` 元素並透過呼叫 `innerHTML` 函式更新其 `renderCard`。將 `"Click the card and reveal the fortune"` 作為第一個引數，`isReversed` 作為第二個引數傳入。

# --hints--

你應該透過呼叫 `this.elements.singleCard.innerHTML` 函式來更新 `renderCard`。
...
你的 `renderCard` 呼叫應該以 `"Click the card and reveal the fortune"` 作為第一個引數。
...
你應該將 `isReversed` 作為第二個引數傳入 `renderCard`。
