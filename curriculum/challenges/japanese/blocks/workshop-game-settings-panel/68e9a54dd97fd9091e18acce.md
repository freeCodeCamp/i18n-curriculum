...
`h1` セレクターを作成し、`text-align` プロパティに `center` の値を設定してください。これで `h1` 要素が中央に配置されます。

# --hints--

CSS に `h1` 要素の型セレクターがあるはずです。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle("h1"));
```

`text-align` セレクターに `h1` プロパティを追加してください。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle("h1")?.textAlign);
```

`text-align` プロパティに `center` の値があるはずです。
