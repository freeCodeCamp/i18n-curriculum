...
プレイヤーの写真はハードコードされたURLから読み込まれています。`src`属性値`https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg`を`player.imageUrl`に置き換えてください。

また、ハードコードされた`alt`テキストを、`player.name`とテキスト`headshot`を使ったテンプレートリテラルで動的な値に置き換えてください。

TSXでJavaScriptの式を属性値として使う場合は、引用符の代わりに波括弧`{}`を使うことに注意してください。例えば`src={player.imageUrl}`であって、`src="{player.imageUrl}"`ではありません。

# --hints--

ハードコードされた画像URLを`player.imageUrl`として`src`属性に置き換えてください。

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

ハードコードされた`alt`テキストを、`player.name`とテキスト`headshot`を使ったテンプレートリテラルに置き換えてください。
