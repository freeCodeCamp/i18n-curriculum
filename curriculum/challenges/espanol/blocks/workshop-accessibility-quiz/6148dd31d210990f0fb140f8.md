...
Es útil ver el borde predeterminado alrededor de los elementos `fieldset` durante el desarrollo. Sin embargo, podría no ser el estilo que deseas.

Elimina el borde y el relleno inferior en los elementos `.question`.

# --hints--

Debes usar el selector `.question`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

Debes dar al `.question` un `border` de `none`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

Debes dar al `.question` un `padding-bottom` de `0`.
