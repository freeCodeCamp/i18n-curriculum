...
`div`의 `className`인 `card-tier-badge` 요소 아래에 `div`의 `className`인 `card-club`를 추가하고 텍스트 `Santos FC`를 넣으세요.

# --hints--

`div` 아래에 `className`의 `card-club`인 `.card-tier-badge`를 추가하세요.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

`.card-club` 요소는 텍스트 `Santos FC`를 가져야 합니다.
