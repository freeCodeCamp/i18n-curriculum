...
Передує `on the same`, це означає, що люди розуміють і погоджуються з одним і тим самим.

# --explanation--

`To be on the same page` означає, що люди мають спільне розуміння або домовленість щодо чогось. Наприклад:

`Let's meet before the presentation to make sure we're on the same page.` – Це означає переконатися, що всі однаково розуміють тему перед її поданням.
