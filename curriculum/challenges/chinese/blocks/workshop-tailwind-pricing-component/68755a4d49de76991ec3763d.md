...
现在，为 `p` 元素内的 `span` 赋值类 `text-base`、`font-medium` 和 `text-gray-500`。

# --hints--

你的 `p` 元素内的 `span` 应该具有 `text-base` 类。
...
你 `p` 元素内的 `span` 应该具有 `font-medium` 类。
...
你 `p` 元素内的 `span` 应该具有 `text-gray-500` 类。
