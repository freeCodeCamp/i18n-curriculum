...
Ya has visto un enlace absoluto en acción. El enlace que creaste a la página About de The Odin Project antes fue un enlace absoluto porque contiene el protocolo y el dominio.
...
Los enlaces relativos solo incluyen la ruta del archivo a la otra página, relativa a la página en la que estás creando el enlace. Esto es algo abstracto, veamos esto en acción con un ejemplo.
...
Esto funciona porque las páginas `index` y `about` están en el mismo directorio. Eso significa que puedes usar simplemente su nombre (`about.html`) como el valor `href` del enlace.
...
Mira el video sobre la estructura de archivos HTML de Kevin Powell arriba.
