...
幸运签必须是随机的，因此需要一个辅助函数来选择随机的幸运签。在 `chosenCard` 方法内声明一个名为 `singleCardSelected` 的变量。使用 `getRandomItem` 方法获取一张随机卡片，并将 `cards` 作为参数传入。

# --hints--

你应该在 `chosenCard` 内声明一个名为 `singleCardSelected` 的变量。
...
你的 `chosenCard` 变量应该通过调用 `getRandomItem(this.cards)` 来赋值。
...
你应该将 `this.cards` 作为参数传递给 `getRandomItem`。
