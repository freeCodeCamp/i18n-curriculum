...
`body` セレクターを作成し、`font-family` プロパティを `sans-serif` に、`color` を `#0a0a23` に設定してください。
...
`body` セレクターには `font-family` プロパティを `sans-serif` に設定してください。

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.getPropertyValue('font-family') === 'sans-serif');
```

`body` セレクターには `color` プロパティを `#0a0a23` に設定してください。
