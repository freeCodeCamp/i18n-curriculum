...
All'interno della funzione `handleShapeSelect`, chiama il metodo `.preventDefault()` su `e` e chiama la funzione `clearInputFields()` per cancellare i campi di input.

# --hints--

Dovresti chiamare `.preventDefault()` su `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

Dovresti chiamare la funzione `clearInputFields`.
