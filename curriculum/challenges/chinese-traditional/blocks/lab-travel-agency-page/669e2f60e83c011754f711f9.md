...
1. 這三個錨點元素應該包含一個具有適當 `src` 屬性和 `alt` 屬性設為有效影像的 `img` 元素作為其內容。如果你願意，可以使用 `https://cdn.freecodecamp.org/curriculum/labs/colosseo.jpg`、`https://cdn.freecodecamp.org/curriculum/labs/alps.jpg` 和 `https://cdn.freecodecamp.org/curriculum/labs/sea.jpg`。
