...
现在，你要将右侧的胡须移到合适的位置。 使用类选择器选择 `.cat-whisker-right-top` 元素，设置其 `position` 为 `absolute`、`top` 为 `120px`、`left` 为 `109px`。

# --hints--

你应该有一个 `.cat-whisker-right-top` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top'));
```

你的 `.cat-whisker-right-top` 选择器应该将 `position` 属性设置为 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.position, 'absolute');
```

你的 `.cat-whisker-right-top` 选择器应该将 `top` 属性设置为 `120px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.top, '120px');
```

你的 `.cat-whisker-right-top` 选择器应该将 `left` 属性设置为 `109px`。
