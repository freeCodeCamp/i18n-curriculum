...
Тепер, коли ви створили кнопку, потрібно призначити їй текст. Для цього потрібно використати метод `createTextNode()` об’єкта DOM.

Метод `createTextNode()` використовується для створення текстового вузла. Щоб його використати, викличте його та передайте текст у вигляді рядка:
...
Використайте метод `createTextNode()` для створення текстового вузла `"Reset Playlist"`, а потім призначте його константі `resetText`.

# --hints--

Не слід змінювати існуючу інструкцію `if` та її вміст.
...
Використайте `document.createTextNode()` для створення текстового вузла `"Reset Playlist"`.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

Призначте створений текстовий вузол константі `resetText`.
