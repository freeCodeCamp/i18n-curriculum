...
`@media` @규칙에는 사용자의 선호도에 따라 CSS를 설정하는 `prefers-reduced-motion`라는 미디어 기능이 있습니다. 이 기능은 다음 값 중 하나를 가질 수 있습니다:
...
`scroll-behavior: smooth`를 설정하는 스타일 규칙을 `@media` @규칙 안에 감싸고, 미디어 기능 `prefers-reduced-motion`에 `no-preference` 값을 설정하세요.
