...
`body` 선택자를 만드세요. `font-family` 속성을 `Baskervville`로 설정하고, 대체 수단으로 `serif`을 지정하세요. `color` 속성을 `linen`로, `background-color` 속성을 `rgb(20, 30, 40)`로 설정하세요.
...
`body` 선택자에 `font-family` 속성을 `Baskervville`로 설정하고, 대체 수단으로 `serif`을 지정해야 합니다.
...
`body` 선택자에 `color` 속성을 `linen`로 설정해야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('body')?.color === 'linen');
```

`body` 선택자에 `background-color` 속성을 `rgb(20, 30, 40)`로 설정해야 합니다.
