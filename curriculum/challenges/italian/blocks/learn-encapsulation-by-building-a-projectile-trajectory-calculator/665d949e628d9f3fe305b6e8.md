...
L'output sta diventando piuttosto difficile da leggere, quindi dovresti sostituire `print(graph.create_trajectory())` con un loop che stampi ogni riga separatamente.

# --hints--

Rimuovi la chiamata `print(graph.create_trajectory())`.
...
Dovresti avere un loop che itera su `graph.create_trajectory()`.
...
Dovresti stampare ogni riga in `graph.create_trajectory()`.
