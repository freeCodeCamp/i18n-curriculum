...
تُحمّل صورة اللاعب من عنوان URL ثابت. استبدل قيمة الخاصية `src` من `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` إلى `player.imageUrl`.

استبدل أيضًا نص الخاصية `alt` الثابت بقيمة ديناميكية باستخدام قالب نصي مع `player.name` والنص `headshot`.

لاحظ أنه عند استخدام تعبير JavaScript كقيمة خاصية في TSX، تستخدم الأقواس المتعرجة `{}` بدلًا من علامات الاقتباس. على سبيل المثال `src={player.imageUrl}` وليس `src="{player.imageUrl}"`.

# --hints--

يجب استبدال عنوان URL الثابت للصورة بـ `player.imageUrl` كقيمة الخاصية `src`.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

يجب استبدال نص الخاصية `alt` الثابت بقالب نصي يستخدم `player.name` والنص `headshot`.
