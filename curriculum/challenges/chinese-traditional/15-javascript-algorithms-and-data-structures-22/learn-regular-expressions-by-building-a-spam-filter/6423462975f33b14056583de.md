...
字符類可以包含多個字符。 將你的 `a` 字符替換成一個字符類，這個字符類匹配 `a`、`@` 和 `4`。

# --hints--

你的 `stockRegex` 應該使用字符類來匹配 `a`、`@` 和 `4`。
...
你的 `stockRegex` 應該匹配 `stock @lert`。

```js
assert.match('stock @lert', stockRegex);
```

你的 `stockRegex` 應該匹配 `stock 4lert`。

```js
assert.match('stock 4lert', stockRegex);
```

你的 `stockRegex` 應該匹配 `570ck 4l3r7`。
