...
이제 `goCave` 함수를 만들어 `"Going to cave."`을 콘솔에 출력하세요.

# --hints--

`function` 키워드를 사용해 `goCave`를 선언하세요.
...
코드에 `console.log("Going to cave.");` 줄이 있어야 합니다.

```js
assert.match(code, /console\.log\(\s*('|")Going to cave\.\1\s*\)/);
```

`console.log("Going to cave.");` 줄은 `goCave` 함수 안에 있어야 합니다.
