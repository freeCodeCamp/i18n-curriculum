...
Un carácter de palabra se define como cualquier letra, desde `a` hasta `z` o `A` hasta `Z`, o un número del `0` al `9`, o el carácter de guion bajo (`_`).

La inclusión del guion bajo puede parecer extraña, pero considera las convenciones de nombres para variables: los nombres de variables a menudo pueden incluir guiones bajos, por lo que `\w` está diseñado para coincidir con eso también.
...
Puede que hayas notado ahora que nuestra clase de caracteres consiste solo en caracteres consecutivos. `a`, `b`, `c` y `d` están todos directamente uno al lado del otro en el alfabeto. Para números, los caracteres consecutivos podrían ser `4`, `5` y `6`.
...
También puedes mezclar letras y números en tu clase de caracteres. Por ejemplo, si quisieras el comportamiento de la clase `\w` sin el guion bajo, podrías construir la tuya propia:
...
La lección menciona una clase "comodín" representada por un símbolo específico.
...
La lección define qué constituye un "word character" en expresiones regulares.
...
Piensa en cómo la lección describe la creación de rangos dentro de corchetes.
