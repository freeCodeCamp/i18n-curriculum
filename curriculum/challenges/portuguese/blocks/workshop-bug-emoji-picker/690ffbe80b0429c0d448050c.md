...
Agora você precisa definir as propriedades que todos os bugs terão.

Dentro da classe `Bug`, declare uma propriedade chamada `emoji` com o tipo `T` usando o operador de assertiva de atribuição definitiva (`!`). Isso informa ao TypeScript que a propriedade será atribuída depois.

# --hints--

Sua classe `Bug` deve ter uma propriedade chamada `emoji`.
...
Sua propriedade `emoji` deve ter o tipo `T`.
...
Sua propriedade `emoji` deve usar o operador de assertiva de atribuição definitiva (`!`).
