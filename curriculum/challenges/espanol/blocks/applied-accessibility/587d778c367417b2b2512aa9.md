...
¡Los resultados de la encuesta de Camper Cat sobre Mortal Kombat ya están aquí! Envuelve una etiqueta `time` alrededor del texto `Thursday, September 15<sup>th</sup>` y añade un atributo `datetime` con el valor `2016-09-15`.
...
Las etiquetas `time` que agregaste deben envolver el texto `Thursday, September 15<sup>th</sup>`.
