...
`try` ブロックの前に、`MediaCatalogue` のインスタンスを作成し、`catalogue` という名前の変数に代入してください。

次に、`add` メソッドを使って `movie1` をカタログに追加します。最後に `catalogue` をコンソールに出力してください。

# --hints--

`MediaCatalogue` ブロックの前に、`catalogue` のインスタンスを `try` という名前の変数に代入する必要があります。
...
`catalogue.add(movie1)` ブロックの中に `try` があるはずです。
...
`catalogue` ブロックの最後に `try` を出力してください。
