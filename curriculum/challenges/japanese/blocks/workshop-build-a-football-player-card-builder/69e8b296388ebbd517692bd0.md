...
`form-row`の後に、`div`の`className`を持つ`form-group`を追加してください。その中に、`label`の`className`、`label`の`htmlFor`、およびテキスト`club`を持つ`Club`要素を追加してください。次に、`id`の`club`、`className`の`input`、および`type`の`text`を持つinput要素を追加してください。
...
`div.form-group`には、`label`の`htmlFor`属性、`club`の`className`、およびテキスト`label`を持つ`Club`があるはずです。
...
Club `form-group`には、`input`の`id`、`club`の`className`、および`input`の`type`を含む`text`があるはずです。
