...
Ruota l'elemento `.back-mountain` di `45deg` in senso orario. Poi, impostagli una proprietà `left` con valore `110px` e una proprietà `top` con valore `225px`.
...
Dovresti dare a `.back-mountain` una proprietà `transform` con valore `--fcc-expected--`, ma è stato trovato `--fcc-actual--`.
...
Dovresti dare a `.back-mountain` una proprietà `left`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.left);
```

Dovresti dare a `.back-mountain` una proprietà `left` con valore `--fcc-expected--`, ma è stato trovato `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.left, '110px');
```

Dovresti dare a `.back-mountain` una proprietà `top`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.top);
```

Dovresti dare a `.back-mountain` una proprietà `top` con valore `--fcc-expected--`, ma è stato trovato `--fcc-actual--`.
