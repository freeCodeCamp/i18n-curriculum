...
Cuando las canciones se muestren en la página, debe mostrar el título, artista, duración de cada canción y un botón para eliminar. 

En los próximos pasos, aprenderás a agregar nuevo HTML para cada canción usando el método <dfn>map()</dfn>. Este método será explicado en más detalle en el siguiente paso.

Comienza usando `const` para declarar una variable llamada `songsHTML` y asignarle `array.map()`.

# --hints--

Debes tener una variable `const` llamada `songsHTML`.

```js
assert.match(code, /const\s+songsHTML/);
```

Debes asignar `array.map()` a tu `songsHTML`.
