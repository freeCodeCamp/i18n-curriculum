...
В межах елемента `head` вкладіть елемент `meta` з атрибутом під назвою `charset`. Встановіть значення `utf-8`, яке повідомляє браузеру, як кодувати символи для сторінки.

Зверніть увагу, що елемент `meta` є порожнім елементом.

Це була остання зміна. Ви завершили проєкт зі створення застосунку з фотографіями котів. Вітаємо!
...
Елементу `meta` надайте `charset` зі значенням `UTF-8`.
