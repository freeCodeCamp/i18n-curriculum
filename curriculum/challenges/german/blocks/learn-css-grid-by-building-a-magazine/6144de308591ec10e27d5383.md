...
Erstellen Sie einen `h1`-Selektor und setzen Sie die `font-family`-Eigenschaft auf `Anton` mit dem Fallback `sans-serif`.
...
Ihr `h1`-Selektor sollte eine `font-family`-Eigenschaft haben, die auf `Anton` mit dem Fallback `sans-serif` gesetzt ist.
