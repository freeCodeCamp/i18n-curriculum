...
이제 캡션에 절대 위치를 지정하고 위에서 `-2.25rem`만큼, 왼쪽에서 `0.5rem`만큼 이동하세요.

# --hints--

`table caption` 선택자에 `position` 속성을 `absolute`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('position') === 'absolute');
```

`table caption` 선택자에 `top` 속성을 `-2.25rem`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('table caption')?.getPropertyValue('top') === '-2.25rem');
```

`table caption` 선택자에 `left` 속성을 `0.5rem`로 설정하세요.
