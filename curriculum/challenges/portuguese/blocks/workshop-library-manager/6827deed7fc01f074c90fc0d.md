...
Seria bom testar sua função `getBooksByAuthor` com outro autor.

Comece registrando a mensagem `"\nList of books by James Clear:\n"` no console. 

Abaixo daquele `console.log()`, adicione outro `console.log()`. Dentro daquele `console.log()`, chame a função `getBooksByAuthor()` com `library` e `"James Clear"` como argumentos.

Agora, você deve ver todos os livros desse autor específico no console.

# --hints--

Você deve registrar a mensagem `"\nList of books by James Clear:\n"` no console.

```js
assert.match(code, /console\.log\(("|')\\nList\s+of\s+books\s+by\s+James\s+Clear:\\n\1\);?/);
```

Você deve ter um `console.log(getBooksByAuthor(library, "James Clear"))`.
