...
Llame a la función `setPowers`, que toma un argumento de arreglo. Use un operador ternario para comprobar si `checked` es verdadero. Si lo es, esparza el arreglo de `powers` existente en uno nuevo y agregue el `valor` a este. Si no es verdadero, filtre el `valor` de `powers` con `powers.filter(p => p !== valor)`.
