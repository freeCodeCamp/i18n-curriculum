...
Ongeza sifa ya kipengele ya `value` ya `player.pac` kwa PAC `input`. Kisha ongeza sifa ya kipengele ya `onChange` iliyowekwa kuwa kitendakazi cha mshale kinachochukua `e` kama kigezo chake na kufanya wito wa `setPlayer` kwa kueneza `player` na `pac` iliyowekwa kuwa `Number(e.target.value)`.

# --hints--

PAC `input` yako inapaswa kuwa na `value` sawa na takwimu ya `pac` ya mchezaji.
...
PAC `input` yako inapaswa kuwa na mshughulikiaji wa `onChange` anayefanya wito wa `setPlayer` kwa kueneza `player` na `pac` iliyowekwa kuwa `Number(e.target.value)`.
