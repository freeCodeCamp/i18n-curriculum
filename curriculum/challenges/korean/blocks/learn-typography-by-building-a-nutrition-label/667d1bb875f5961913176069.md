...
미리 보기에서 텍스트 `8g`가 가운데 정렬된 것을 확인하세요. 텍스트 `span`를 포함하는 `Total Fat` 요소와 텍스트 `8g`를 정렬하기 위해 추가 `span` 요소 안에 중첩하세요.

# --hints--

코드 `span` 주위에 `<span class="bold">Total Fat</span> 8g` 요소가 있어야 합니다.
