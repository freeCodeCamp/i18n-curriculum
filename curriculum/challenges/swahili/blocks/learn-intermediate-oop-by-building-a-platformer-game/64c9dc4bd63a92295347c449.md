...
Ifuatayo, ongeza usemi wa `else if` ili ukague kama kitufe cha kushoto kimebonyezwa na kama `isCheckpointCollisionDetectionActive` ni kweli.

Ndani ya hali hiyo, ongeza mzunguko wa `forEach` rudia kupitia safu ya data ya `platforms`.

Ndani ya mzunguko huo, tumia opereta ya kuongeza kuongeza 5 kwenye nafasi ya `x` ya jukwaa.

# --hints--

Unapaswa kuwa na hali inayokagua kama kitufe cha kushoto kimebonyezwa na kama `isCheckpointCollisionDetectionActive` ni kweli.
...
Unapaswa kuongeza mzunguko wa `forEach` unaorudia kupitia safu ya data ya `platforms`.

```js
assert.match(code, /else\s+if\s*\(.*\)\s*{\s*platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

Unapaswa kutumia opereta ya kuongeza kuongeza 5 kwenye nafasi ya `x` ya jukwaa.
