...
Picha ya mchezaji inapakiwa kutoka URL iliyowekwa moja kwa moja. Badilisha thamani ya sifa ya kipengele `src` `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` na `player.imageUrl`.

Pia badilisha maandishi ya `alt` yaliyowekwa moja kwa moja na thamani inayobadilika kwa kutumia usemi wa kihesabu wa template literal na `player.name` pamoja na maandishi `headshot`.

Kumbuka kwamba unapotumia usemi wa kihesabu wa JavaScript kama thamani ya sifa ya kipengele katika TSX, unatumia mabano ya mviringo `{}` badala ya nukuu. Kwa mfano `src={player.imageUrl}`, si `src="{player.imageUrl}"`.

# --hints--

Unapaswa kubadilisha URL ya picha iliyowekwa moja kwa moja na `player.imageUrl` kama sifa ya kipengele `src`.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

Unapaswa kubadilisha maandishi ya `alt` yaliyowekwa moja kwa moja na usemi wa kihesabu wa template literal ukitumia `player.name` na maandishi `headshot`.
