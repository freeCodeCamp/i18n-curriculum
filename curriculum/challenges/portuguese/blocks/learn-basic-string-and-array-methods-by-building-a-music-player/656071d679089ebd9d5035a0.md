...
Como os usuários poderão embaralhar e deletar músicas da playlist, você precisará criar uma cópia do array `allSongs` sem modificar o original. É aqui que o operador `spread` é útil.

O operador spread (`...`) permite copiar todos os elementos de um array para outro. Também pode ser usado para concatenar múltiplos arrays em um só. No exemplo abaixo, tanto `arr1` e `arr2` foram espalhados em `combinedArr`:
...
Dentro do objeto `userData` crie uma propriedade `songs`. Para o valor, espalhe `allSongs` em um array.

# --hints--

Seu objeto `userData` deve ter uma chave chamada `songs`.

```js
assert.property(userData, "songs");
```

A sua propriedade `songs` deve ter um valor de `[...allSongs]`.
