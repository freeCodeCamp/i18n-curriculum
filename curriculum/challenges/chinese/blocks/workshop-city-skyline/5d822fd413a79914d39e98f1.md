...
你需要给接下来的两个部分添加相同的渐变。 不要像之前那样做，而是创建一个名为 `bb1-window` 的新类选择器，并从 `.bb1a` 中将 `height` 和 `background` 移动到新的类选择器。

# --hints--

你应该创建一个名为 `bb1-window` 的新类选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

你应该将 `height` 属性和值从 `.bb1a` 移动到 `.bb1-window`。
...
你应该将 `background` 属性和值从 `.bb1a` 移动到 `.bb1-window`。
