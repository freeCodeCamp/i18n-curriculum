...
現在你可以開始將函數解析器應用到 `evalFormula` 邏輯了。 聲明一個 `functionExpanded` 變量，並將使用 `cellExpanded` 字符串調用 `applyFunction` 的結果分配給它。

# --hints--

你應該聲明一個 `functionExpanded` 變量。
...
你應該將 `cellExpanded` 傳遞給 `applyFunction` 調用。
