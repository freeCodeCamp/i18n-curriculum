...
У функції `elemValue` ви явно оголосили функцію під назвою `inner` та повернули її. Однак, оскільки ви використовуєте стрілковий синтаксис, ви можете неявно повернути функцію. Наприклад:

```js
const curry = soup => veggies => {};
```

`curry` — це функція, яка приймає параметр `soup` та повертає функцію, яка приймає параметр `veggies`. Використовуючи цей синтаксис, оновіть функцію `addCharacters`, щоб повернути порожню функцію, яка приймає параметр `character2`.
...
Функція `addCharacters` має повернути стрілкову функцію, яка має параметр `character2`.
...
Внутрішня стрілкова функція має бути порожньою.
