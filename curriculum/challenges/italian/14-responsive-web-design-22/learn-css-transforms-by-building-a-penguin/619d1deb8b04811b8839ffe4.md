...
Dovresti dare a `.shirt` un valore `top` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.top, '165px');
```

Dovresti dare a `.shirt` un valore `left` di `--fcc-expected--` invece di `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.shirt')?.left, '127.5px');
```

Dovresti assegnare all'elemento `.shirt` un valore `z-index` di `1`.
