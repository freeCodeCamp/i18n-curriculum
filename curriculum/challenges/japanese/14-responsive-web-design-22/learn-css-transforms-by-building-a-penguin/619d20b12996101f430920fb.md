...
`.beak` の `background-color` を `var(--penguin-picorna)` に設定する必要がありますが、`--fcc-actual--` が設定されています。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.getPropVal('background-color', true), 'var(--penguin-picorna)');
```

`.foot` の `background-color` を `var(--penguin-picorna)` に設定する必要がありますが、`--fcc-actual--` が設定されています。
