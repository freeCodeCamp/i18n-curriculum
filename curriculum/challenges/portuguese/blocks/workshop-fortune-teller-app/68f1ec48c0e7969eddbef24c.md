...
Finalmente, dentro do `hideElements`, passe os argumentos `singleCard`, `multipleCard`, `fortuneContainer` e `fortuneDescription`. Lembre-se de obtê-los dos elementos.

Com isso, seu app de previsão do futuro estará completo!

# --hints--

Você deve passar `singleCard` como primeiro argumento para `hideElements`.
...
Você deve passar `multipleCard` como argumento para `hideElements`.
...
Você deve passar `fortuneContainer` como argumento para `hideElements`.
...
Você deve passar `fortuneDescription` como argumento para `hideElements`.
