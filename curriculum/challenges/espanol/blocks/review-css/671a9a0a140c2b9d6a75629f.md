...
- **¿Qué es CSS?**: Las hojas de estilo en cascada (CSS) son un lenguaje de hojas de estilo usado para aplicar estilos a elementos HTML. CSS se usa para colores, imágenes de fondo, diseños y más.
- **Anatomía básica de una regla CSS**: Una regla CSS está compuesta por dos partes principales: un selector y un bloque de declaración. Un selector es un patrón usado en CSS para identificar y seleccionar elementos HTML específicos para aplicar estilos. Un bloque de declaración aplica un conjunto de estilos para un selector o selectores dados.
- **Elemento `meta name="viewport"`**: Este elemento `meta` da instrucciones al navegador sobre cómo controlar las dimensiones y el escalado de la página en diferentes dispositivos, especialmente en teléfonos móviles y tabletas.
- **Estilos predeterminados del navegador**: Cada elemento HTML tendrá estilos predeterminados aplicados por el navegador. Esto usualmente incluye elementos como márgenes y rellenos predeterminados.
...
- **Combinador descendiente (` `)&nbsp;\*un solo espacio entre selectores**: Este combinador se usa para seleccionar elementos que son descendientes de un elemento padre especificado.
...
- **Elementos en línea**: Los elementos en línea solo ocupan el ancho que necesitan y no comienzan en una nueva línea. Estos elementos fluyen dentro del contenido, permitiendo que el texto y otros elementos en línea aparezcan junto a ellos. Los elementos en línea comunes son `span`, `a` y `img`.
- **Elementos de nivel bloque**: Los elementos de nivel bloque ocupan todo el ancho disponible por defecto, extendiéndose a lo largo del ancho de su contenedor. Algunos elementos comunes de nivel bloque son `div`, `p` y `section`.
- **Elementos de nivel inline-block**: Puedes establecer un elemento como `inline-block` usando la propiedad `display`. Estos elementos se comportan como elementos en línea pero pueden tener un `width` y `height` establecidos como los elementos de nivel bloque.

## Margen y Relleno

- **Propiedad `margin`**: Esta propiedad se usa para aplicar espacio fuera del elemento, entre el borde del elemento y los elementos circundantes.
- **Propiedad abreviada `margin`**: Puedes especificar de 1 a 4 valores para establecer los márgenes. Un valor se aplica a los cuatro lados; dos valores establecen `top` y `bottom`, luego `right` y `left`; tres valores establecen `top`, horizontal (`right` y `left`), luego `bottom`; cuatro valores establecen `top`, `right`, `bottom`, `left`.
- **Propiedad `padding`**: Esta propiedad se usa para aplicar espacio dentro del elemento, entre el contenido y su borde.
- **Propiedad abreviada `padding`**: Puedes especificar de 1 a 4 valores para establecer los rellenos. Un valor se aplica a los cuatro lados; dos valores establecen `top` y `bottom`, luego `right` y `left`; tres valores establecen `top`, horizontal (`right` y `left`), luego `bottom`; cuatro valores establecen `top`, `right`, `bottom`, `left`.

## Especificidad de CSS

- **Especificidad del CSS en línea**: El CSS en línea tiene la especificidad más alta porque se aplica directamente al elemento. Sobrescribe cualquier CSS interno o externo. El valor de especificidad para estilos en línea es (1, 0, 0, 0).
- **Especificidad del CSS interno**: El CSS interno se define dentro de un elemento `style` en la sección `head` del documento HTML. Tiene menor especificidad que los estilos en línea. Tanto el CSS interno como el externo comparten el mismo nivel de especificidad, que se determina por sus selectores, no por su ubicación.
- **Especificidad del CSS externo**: El CSS externo se enlaza mediante un elemento `link` en la sección `head` y se escribe en archivos `.css` separados. Al igual que el CSS interno, su especificidad la determinan los selectores usados. Cuando dos reglas tienen igual especificidad, el orden en el código determina el ganador: la regla que aparece más tarde en el documento tiene prioridad. El CSS externo ofrece la mejor mantenibilidad para proyectos grandes.
- **Selector universal (`*`)**: Es un tipo especial de selector CSS que coincide con cualquier elemento en el documento. Se usa a menudo para aplicar un estilo a todos los elementos de la página, útil para reiniciar o normalizar estilos entre navegadores. El selector universal tiene el valor de especificidad más bajo de cualquier selector. Contribuye con 0 a todas las partes del valor de especificidad (0, 0, 0, 0).
- **Selectores de tipo**: Estos selectores apuntan a elementos basados en su nombre de etiqueta. Tienen una especificidad relativamente baja comparada con otros selectores. El valor de especificidad para un selector de tipo es (0, 0, 0, 1).
- **Selectores de clase**: Estos selectores se definen con un punto (`.`) seguido del nombre de la clase. El valor de especificidad para un selector de clase es (0, 0, 1, 0). Esto significa que los selectores de clase pueden sobrescribir selectores de tipo, pero pueden ser sobrescritos por selectores de ID y estilos en línea.
- **Selectores de ID**: Se definen con un símbolo de hash (`#`) seguido del nombre del ID. Los selectores de ID tienen una especificidad muy alta, mayor que los selectores de tipo y clase, pero menor que los estilos en línea. El valor de especificidad para un selector de ID es (0, 1, 0, 0).
- **Palabra clave `!important`**: Se usa para dar a una regla de estilo la máxima prioridad, permitiendo que sobrescriba cualquier otra declaración para una propiedad. Cuando se usa, fuerza al navegador a aplicar el estilo especificado, sin importar la especificidad de otros selectores. Debes usar `!important` con precaución porque puede dificultar el mantenimiento y la depuración del CSS.
- **Algoritmo de cascada**: Algoritmo usado para decidir qué reglas CSS aplicar cuando hay múltiples estilos que apuntan al mismo elemento. Asegura que se usen los estilos más apropiados, basándose en un conjunto de reglas bien definidas.
- **Herencia en CSS**: Proceso por el cual los estilos se transmiten de elementos padre a sus hijos. La herencia permite definir estilos en un nivel superior del árbol del documento y que se apliquen a múltiples elementos sin especificarlos explícitamente para cada uno.
...
- **Modo oscuro**: Es una funcionalidad especial en aplicaciones web donde puedes cambiar el esquema de colores claro predeterminado a uno oscuro. Debes usar colores desaturados en modo oscuro. Los colores desaturados son colores menos intensos y con un nivel de saturación más bajo.
- **Migas de pan**: Es una ayuda de navegación que muestra al usuario dónde se encuentra en la jerarquía del sitio. Es mejor colocar las migas de pan en la parte superior de la página para que los usuarios las encuentren fácilmente. También debes asegurarte de que las migas de pan sean lo suficientemente grandes para leerse con facilidad, pero no tan grandes que ocupen demasiado espacio en la página.
- **Componente tarjeta**: Tu componente tarjeta debe ser simple en diseño, sin estar visualmente saturado ni mostrar demasiada información. Para medios, asegúrate de elegir imágenes y videos de alta calidad para mejorar la experiencia del usuario.
- **Scroll infinito**: Es un patrón de diseño que carga más contenido a medida que el usuario desplaza hacia abajo la página. Debes considerar usar un botón de cargar más porque le da al usuario control sobre cuándo quiere ver más contenido. También puedes agregar un botón de retroceso para que los usuarios puedan volver a la página anterior sin tener que desplazarse hasta arriba.
- **Diálogo modal**: Es un tipo de ventana emergente que se muestra encima del contenido existente de la página. Usualmente el contenido de fondo tendrá una superposición de color tenue para ayudar al usuario a enfocarse mejor en el contenido modal. También es buena idea permitir que el usuario haga clic fuera del modal para cerrarlo. Cuando usas el elemento HTML `dialog`, obtienes muchas funcionalidades y beneficios de accesibilidad incorporados.
- **Indicador de progreso para registro en formularios**: Es una forma de mostrar a los usuarios qué tan avanzados están en un proceso. Puede usarse en formularios, registros y procesos de configuración. Tu diseño debe ser simple, fácil de encontrar y permitir regresar a pasos anteriores.
- **Carrito de compras**: Los carritos son un lugar donde los usuarios pueden ver los artículos que ya han seleccionado en una plataforma de comercio electrónico. Tus carritos siempre deben ser visibles para el usuario, usar un ícono común como carrito, bolsa o canasta, y tener un botón claro de llamada a la acción para que los usuarios procedan al pago.
...
- **`px` (píxeles)**: Esta unidad absoluta es una unidad de tamaño fijo en CSS, que proporciona control preciso sobre las dimensiones. Esto significa que 1px siempre equivale a 1/96 de pulgada.
...
- **`q` (cuarto de milímetro)**: Esta unidad absoluta equivale a 1/40 de centímetro.
- **`pc` (picas)**: Esta unidad absoluta equivale a 1/6 de pulgada.
- **`pt` (puntos)**: Esta unidad absoluta equivale a 1/72 de pulgada.

## Unidades Relativas

- **Porcentajes**: Estas unidades relativas te permiten definir tamaños, dimensiones y otras propiedades como una proporción del elemento padre. Por ejemplo, si estableces `width: 50%;` en un elemento, ocupará la mitad del ancho de su contenedor padre.
- **Unidad `em`**: Estas unidades son relativas al tamaño de la fuente del elemento. Si usas `ems` para la propiedad `font-size`, el tamaño del texto será relativo al tamaño de la fuente del elemento padre.
- **Unidad `rem`**: Estas unidades son relativas al tamaño de la fuente del elemento raíz, que es el elemento `html`.
- **Unidad `vh`**: `vh` significa `"viewport height"` y `1vh` equivale al 1 % de la altura del viewport.
- **Unidad `vw`**: `vw` significa `"viewport width"` y `1vw` equivale al 1 % del ancho del viewport.
...
- **Pseudoclases de ubicación**: Estas pseudoclases se usan para estilizar enlaces y elementos que son objetivos dentro del documento actual.
- **Pseudoclase `:any-link`**: Esta pseudoclase es una combinación de las pseudoclases `:link` y `:visited`. Por lo tanto, coincide con cualquier elemento ancla con un atributo `href`, sin importar si ha sido visitado o no.
- **Pseudoclase `:link`**: Esta pseudoclase te permite apuntar a todos los enlaces no visitados en una página web. Puedes usarla para estilizar los enlaces de forma diferente antes de que el usuario haga clic en ellos.
- **Pseudoclase `:local-link`**: Esta pseudoclase apunta a enlaces que apuntan al mismo documento. Puede ser útil cuando quieres diferenciar enlaces internos de externos.
- **Pseudoclase `:visited`**: Esta pseudoclase apunta a un enlace que el usuario ha visitado.
- **Pseudoclase `:target`**: Esta pseudoclase se usa para aplicar estilos a un elemento que es el objetivo de un fragmento de URL.

## Pseudo-clases Estructurales del Árbol

- **Pseudoclases estructurales del árbol**: Estas pseudoclases te permiten apuntar y estilizar elementos basados en su posición dentro del árbol del documento.
- **Pseudoclase `:root`**: Esta pseudoclase suele ser el elemento raíz `html`. Te ayuda a apuntar al nivel más alto en el documento para que puedas aplicar un estilo común a todo el documento.
- **Pseudoclase `:empty`**: Los elementos vacíos, es decir, elementos sin hijos aparte de espacio en blanco, también están incluidos en el árbol del documento. Por eso existe la pseudoclase `:empty` para apuntar a elementos vacíos.
- **Pseudoclase `:nth-child(n)`**: Esta pseudoclase te permite seleccionar elementos basados en su posición dentro de un padre.
- **Pseudoclase `:nth-last-child(n)`**: Esta pseudoclase te permite seleccionar elementos contando desde el final.
- **Pseudoclase `:first-child`**: Esta pseudoclase selecciona el primer elemento dentro de un elemento padre o del documento.
- **Pseudoclase `:last-child`**: Esta pseudoclase selecciona el último elemento dentro de un elemento padre o del documento.
- **Pseudoclase `:only-child`**: Esta pseudoclase selecciona el único elemento dentro de un elemento padre o del documento.
- **Pseudoclase `:first-of-type`**: Esta pseudoclase selecciona la primera ocurrencia de un tipo específico de elemento dentro de su padre.
- **Pseudoclase `:last-of-type`**: Esta pseudoclase selecciona la última ocurrencia de un tipo específico de elemento dentro de su padre.
- **Pseudoclase `:nth-of-type(n)`**: Esta pseudoclase te permite seleccionar un elemento específico dentro de su padre basado en su posición entre hermanos del mismo tipo.
- **Pseudoclase `:only-of-type`**: Esta pseudoclase selecciona un elemento si es el único de su tipo dentro de su padre.


## Pseudo-clases Funcionales

- **Pseudoclases funcionales**: Las pseudoclases funcionales te permiten seleccionar elementos basados en condiciones o relaciones más complejas. A diferencia de las pseudoclases regulares que seleccionan elementos según un estado (por ejemplo, `:hover`, `:focus`), las pseudoclases funcionales aceptan argumentos.
- **Pseudoclase `:is()`**: Esta pseudoclase toma una lista de selectores (por ejemplo, `ol`, `ul`) y selecciona un elemento que coincida con uno de los selectores en la lista.
- **Pseudoclase `:where()`**: Esta pseudoclase toma una lista de selectores (por ejemplo, `ol`, `ul`) y selecciona un elemento que coincida con uno de los selectores en la lista. La diferencia entre `:is()` y `:where()` es que esta última tiene una especificidad de 0.
- **Pseudoclase `:has()`**: Esta pseudoclase es conocida comúnmente como el selector de `"parent"` porque permite aplicar estilos a elementos que contienen elementos hijos especificados en la lista de selectores.
...
- **Definición de teoría del color**: Es el estudio de cómo los colores interactúan entre sí y cómo afectan nuestra percepción. Cubre las relaciones de color, la armonía del color y el impacto psicológico del color.
...
- **Esquemas de colores análogos**: Estos esquemas crean experiencias cohesivas y relajantes. Tienen colores análogos, que son adyacentes entre sí en el círculo cromático.
...
- **Esquema de color triádico**: Este esquema tiene colores vibrantes. Están formados por colores que están aproximadamente equidistantes entre sí. Si se conectan, forman un triángulo equilátero en el círculo cromático.
- **Esquema de color monocromático**: En este esquema, todos los colores se derivan de un mismo color base ajustando su luminosidad, oscuridad y saturación. Esto evoca una sensación de unidad y armonía mientras crea contraste.
...
- **Función `rgba()`**: Esta función añade un cuarto valor, alfa, que controla la transparencia del color.
- **Función `hsl()`**: HSL significa tono, saturación y luminosidad, tres componentes clave que definen un color.
- **Función `hsla()`**: Esta función añade un cuarto valor, alfa, que controla la opacidad del color.
- **Hexadecimal**: Un código hex (abreviatura de código hexadecimal) es una cadena de seis caracteres usada para representar colores en el modelo RGB. "Hex" se refiere al sistema de numeración base 16, que usa dígitos del 0 al 9 y letras de la A a la F.
...
- **`flex-wrap: nowrap;`**: Este es el valor predeterminado. Los elementos flexibles no se envolverán en una nueva línea, incluso si su ancho excede el del contenedor.
...
- **Propiedad `flex-flow`**: Esta propiedad es una abreviatura para `flex-direction` y `flex-wrap`.
...
- **Atributo `aria-hidden`**: Se usa para ocultar un elemento a personas que usan tecnología de asistencia como lectores de pantalla. Por ejemplo, puede usarse para ocultar imágenes decorativas que no aportan contenido significativo.
- **Atributo `hidden`**: Este atributo es compatible con la mayoría de los navegadores modernos y oculta contenido tanto visualmente como del árbol de accesibilidad. Puede activarse o desactivarse fácilmente con JavaScript.
...
- **Definición**: El selector `attribute` te permite apuntar a elementos HTML basados en sus atributos como `href` o `title`.
...
- **Definición**: El principio central del diseño adaptable es la adaptabilidad: la capacidad de un sitio web para ajustar su diseño y contenido según el tamaño de pantalla y las capacidades del dispositivo desde el que se visualiza.
- **Rejillas fluidas**: Usan unidades relativas como porcentajes en lugar de unidades fijas como píxeles, permitiendo que el contenido cambie de tamaño y se reorganice según el tamaño de pantalla.
- **Imágenes flexibles**: Se configuran para redimensionarse dentro de sus elementos contenedores, asegurando que no desborden sus contenedores en pantallas más pequeñas.

## Consultas de Medios

- **Definición**: Permite a los desarrolladores aplicar diferentes estilos basados en las características del dispositivo, principalmente el ancho del viewport.
...
- **Tipos de medio `print`**: Está destinado a material paginado y documentos vistos en modo de vista previa de impresión.
- **Tipos de medio `screen`**: Está destinado principalmente para pantallas.
- **`aspect-ratio`**: Describe la relación entre el ancho y la altura del viewport.
- **`orientation`**: Se usa para indicar si el dispositivo está en orientación horizontal o vertical.
- **`resolution`**: Se usa para describir la resolución del dispositivo de salida en puntos por pulgada (dpi) o puntos por centímetro (dpcm).
- **`hover`**: Se usa para probar si el mecanismo de entrada principal puede hacer hover sobre elementos.
- **`prefers-color-scheme`**: Se usa para detectar si el usuario ha solicitado un tema de color claro u oscuro.
...
- **`--property-name`**: Es el nombre de la propiedad personalizada que estás definiendo. Como todas las propiedades personalizadas, debe comenzar con dos guiones.
- **`syntax`**: Define el tipo de la propiedad, que puede ser cosas como `<color>`, `<length>`, `<number>`, `<percentage>`, o tipos más complejos.
...
- **Respaldo**: Al usar la propiedad personalizada, puedes proporcionar un valor de respaldo usando la función `var()`, igual que con las propiedades personalizadas estándar.
...
- **Definición**: Las animaciones CSS te permiten crear efectos dinámicos y visualmente atractivos en páginas web sin necesidad de JavaScript o programación compleja. Proporcionan una forma de hacer transiciones suaves entre diferentes estilos durante un tiempo especificado.
- **Regla `@keyframes`**: Esta regla define las etapas y estilos de la animación. Especifica qué estilos debe tener el elemento en varios puntos durante la animación.
- **Propiedad `animation`**: Es la propiedad abreviada usada para aplicar animaciones.
- **`animation-name`**: Especifica el nombre de la regla `@keyframes` que se usará.
- **`animation-duration`**: Establece cuánto tiempo debe durar la animación.
- **`animation-timing-function`**: Define cómo progresa la animación en el tiempo (como ease, linear, ease-in-out).
- **`animation-delay`**: Especifica un retraso antes de que la animación comience.
- **`animation-iteration-count`**: Establece cuántas veces debe repetirse la animación.
- **`animation-direction`**: Determina si la animación debe reproducirse hacia adelante, hacia atrás o alternar.
- **`animation-fill-mode`**: Especifica cómo debe estilizarse el elemento antes y después de la animación.
