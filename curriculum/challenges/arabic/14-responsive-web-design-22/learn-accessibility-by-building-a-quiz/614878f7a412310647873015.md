...
قم بتنظيف `header` باستخدام _Flexbox_ لوضع مسافة بين الأطفال، وتوسيطهم رأسياً.

ثم قم بتثبيت `header` إلى الجزء العلوي من نافذة العرض (viewport).

# --hints--

يجب عليك إعطاء `header` خاصية `justify-content` بقيمة `space-between`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.justifyContent, 'space-between');
```

يجب عليك إعطاء `header` خاصية `align-items` بقيمة `center`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.alignItems, 'center');
```

يجب عليك إعطاء `header` خاصية `position` بقيمة `fixed`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('header')?.position, 'fixed');
```

يجب عليك إعطاء `header` خاصية `top` بقيمة `0`.
