...
ボタンテキストと同様のパターンに従って、3 つのボタンの `onclick` への代入を、`"button functions"` の配列の 1 番目、2 番目、3 番目の値に変更しましょう。

# --hints--

引数 `location` の `"button functions"` プロパティの、最初の要素にアクセスしてください。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

`button1.onclick` プロパティに、引数 `location` の `"button functions"` プロパティの最初の要素を代入してください。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*0\s*\]/);
```

引数 `location` の `"button functions"` プロパティの、2 番目の要素にアクセスしてください。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

`button2.onclick` プロパティに、引数 `location` の `"button functions"` プロパティの 2 番目の要素を代入してください。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*1\s*\]/);
```

引数 `location` の `"button functions"` プロパティの、3 番目の要素にアクセスしてください。

```js
assert.match(update.toString(), /location\s*\[\s*('|")button functions\1\s*\]\s*\[\s*2\s*\]/);
```

`button3.onclick` プロパティに、引数 `location` の `"button functions"` プロパティの 3 番目の要素を代入してください。
