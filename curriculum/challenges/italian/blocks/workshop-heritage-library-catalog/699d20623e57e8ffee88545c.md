...
Ora costruirai `groupByDecade`, che organizza il catalogo per decennio. Ogni chiave sarà una stringa come `"1970s"` e ogni valore un array di libri di quel periodo.

Crea una funzione chiamata `groupByDecade` che prende `catalog` come parametro. All'interno, dichiara `const grouped = {}`. Aggiungi un loop `for` per iterare su `catalog`, e dentro il loop dichiara `const book = catalog[i]`.

# --hints--

Dovresti avere una funzione chiamata `groupByDecade` con un parametro `catalog`.
...
Dovresti dichiarare una variabile chiamata `grouped` impostata a un oggetto vuoto.

```js
assert.match(groupByDecade.toString(), /(const|let|var)\s+grouped\s*=\s*\{\s*\}/);
```

Dovresti usare un loop `for` per iterare su `catalog`.

```js
assert.match(groupByDecade.toString(), /for\s*\(\s*(var|let)\s+(\w+)\s*=\s*0\s*;\s*\2\s*<\s*catalog\s*\.\s*length\s*;\s*\2\+\+\s*\)\s*\{/);
```

Dovresti dichiarare una variabile chiamata `book` impostata all'attuale voce del catalogo.
