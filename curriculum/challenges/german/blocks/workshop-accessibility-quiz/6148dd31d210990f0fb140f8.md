...
Es ist hilfreich, während der Entwicklung den Standardrahmen um die `fieldset`-Elemente zu sehen. Allerdings ist das möglicherweise nicht der Stil, den Sie möchten.

Entfernen Sie den Rahmen und den unteren Innenabstand bei den `.question`-Elementen.

# --hints--

Sie sollten den `.question`-Selektor verwenden.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.question'));
```

Sie sollten dem `.question` einen `border` von `none` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question')?.borderStyle, 'none');
```

Sie sollten dem `.question` einen `padding-bottom` von `0` geben.
