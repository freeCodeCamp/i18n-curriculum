...
Kama unavyoona, sasa seti yako ni tupu kabisa. Ondoa wito zote za `clear` na `has` kutoka kwenye msimbo wako.

# --hints--

Haupaswi kuwa na `plantsSet.clear()` kwenye msimbo wako.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

Haupaswi kuwa na `plantsSet.has(ballerina)` kwenye msimbo wako.
