...
All'interno del tuo elemento `ul`, crea sei elementi `li`. Aggiungi un elemento `h4` con un insieme `class` impostato su `list-subtitle` e un elemento `p` a ciascuno dei tuoi elementi `li`.

Poi assegna agli elementi `h4` e `p` il seguente contenuto testuale, in ordine, con ogni `h4` che usa ciò che sta a sinistra dei due punti e ogni `p` che usa ciò che sta a destra:
...
Il tuo elemento `ul` dovrebbe avere sei elementi `li`.

```js
assert.lengthOf(document.querySelectorAll('.lists li'), 6);
```

Ognuno dei tuoi nuovi elementi `li` dovrebbe avere un elemento `h4` e `p`.
...
Il tuo primo `h4` dovrebbe avere il testo `V1 - 2014`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

Il tuo primo `p` dovrebbe avere il testo `We launched freeCodeCamp with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
Il tuo secondo `h4` dovrebbe avere il testo `V2 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[1]?.innerText, 'V2 - 2015');
```

Il tuo secondo `p` dovrebbe avere il testo `We added interactive algorithm challenges.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[1]?.innerText, 'We added interactive algorithm challenges.');
```

Il tuo terzo `h4` dovrebbe avere il testo `V3 - 2015`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[2]?.innerText, 'V3 - 2015');
```

Il tuo terzo `p` dovrebbe avere il testo `We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`
...
Il tuo quinto `h4` dovrebbe avere il testo `V5 - 2017`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[4]?.innerText, 'V5 - 2017');
```

Il tuo quinto `p` dovrebbe avere il testo `We added the back-end and data visualization challenges.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[4]?.innerText, 'We added the back-end and data visualization challenges.');
```

Il tuo sesto `h4` dovrebbe avere il testo `V6 - 2018`.

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[5]?.innerText, 'V6 - 2018');
```

Il tuo sesto `p` dovrebbe avere il testo `We launched 6 new certifications to replace our old ones. This was the biggest curriculum improvement to date.`
...
I tuoi sei elementi `h4` dovrebbero avere ciascuno la classe `list-subtitle`.
