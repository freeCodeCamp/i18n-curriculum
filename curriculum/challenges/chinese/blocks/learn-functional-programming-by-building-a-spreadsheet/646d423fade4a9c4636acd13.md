...
首先，你需要处理优先级较高的运算符。 声明一个 `noHigh` 变量，并将使用 `str` 作为参数调用 `highPrecedence()` 的结果分配给它。
