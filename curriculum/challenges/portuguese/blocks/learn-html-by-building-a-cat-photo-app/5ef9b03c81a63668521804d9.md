...
Aqui está um exemplo de um elemento `input` com um placeholder definido como `e.g. Jane Doe`:
...
Seu elemento `input` deve ter um atributo `placeholder` com o valor `cat photo URL`. Você ou omitiu o valor ou cometeu um erro de digitação. Lembre-se de que os valores dos atributos devem estar entre aspas.
