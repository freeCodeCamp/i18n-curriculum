...
現在，是時候定義當圓鈕被選取時會發生什麼。

你只在圓鈕被選取時為內圈設定樣式，所以你的選擇器應該是 `.contact-method-radio-btn:checked::before`。

在規則中，將 `transform` 設定為 `translate(3px, 3px) scale(1)`，並將 `background-color` 設定為 `lightgreen`。

`transform` 將內圈稍微移動並放大到完整大小 `scale(1)`，並以淺綠色背景填滿圓圈以顯示該選項已被選取，使其可見。

# --hints--

你應該將標的設定為 `.contact-method-radio-btn:checked::before` 偽元素。

```js
assert(new __helpers.CSSHelp(document).getStyle('.contact-method-radio-btn:checked::before'));
```

將 `transform` 屬性設定為 `translate(3px, 3px) scale(1)`。
...
將 `background-color` 屬性設為 `lightgreen`。
