...
`address` 要素内のすべてのテキストを水平方向の中央に配置し、さらにパディングを追加します。

# --hints--

`address` 要素セレクターを使用する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('address'));
```

`address` の `text-align` を `center` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('address')?.textAlign, 'center');
```

`address` の `padding-top` を最低でも `1px` に設定する必要があります。
...
`address` の `padding-right` を最低でも `1px` に設定する必要があります。
...
`address` の `padding-bottom` を最低でも `1px` に設定する必要があります。
...
`address` の `padding-left` を最低でも `1px` に設定する必要があります。
