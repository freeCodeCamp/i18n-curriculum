...
在這個實驗中，你將建置一個 freeCodeCamp 論壇排行榜，顯示來自 [freeCodeCamp forum](https://forum.freecodecamp.org/) 的最新主題、使用者和回覆。HTML、CSS 和部分 JS 程式碼都已經為你提供。你可以自由探索這些內容。
...
請注意，`avatar_template` 的值是相對路徑：它們以 `/` 開頭，並包含一個 `{size}` 佔位符，這個佔位符應該被像素大小取代。`avatarUrl` 常數儲存了當路徑為相對路徑時要加在前面的基礎 URL。

**目標：** 請完成下方的使用者故事，並通過所有測試以完成本實驗。

**使用者故事：**

1. 你應該有一個名為 `timeAgo` 的函式，該函式接受一個 ISO 8601 格式的時間戳作為引數。
1. `timeAgo` 函式應該計算傳入引數的時間與目前時間的差異，並傳回：
   - `xm ago`（`x` 代表分鐘），如果經過的分鐘數小於 `60`。
   - `xh ago`（`x` 代表小時），如果經過的小時數小於 `24`。
   - `xd ago`（`x` 代表天），否則。
1. 你應該有一個名為 `viewCount` 的函式，該函式接受一個貼文的瀏覽次數作為引數。
1. 如果傳入的瀏覽次數大於或等於 `1000`，`viewCount` 函式應該傳回一個字串，內容為瀏覽次數除以 `1000`，向下取整後加上字母 `k`。否則，應該直接傳回瀏覽次數。
1. 你應該有一個名為 `forumCategory` 的函式，該函式接受所選分類的 id 作為引數。
1. `forumCategory` 函式應該驗證所選分類 id 是否為 `allCategories` 物件的屬性，並應該傳回一個包含以下內容的錨點元素字串：
   - 所選分類的 `category` 鍵的文字。
   - 一個 class，內容為 `category` 加上所選分類的 `className` 屬性。
   - 一個 `href`，由 `<className>/<id>` 加到 `forumCategoryUrl`（注意：`forumCategoryUrl` 已經以 `/` 結尾，所以不要再加一個額外的 `/` 分隔符），其中 `<className>` 是所選分類的 `className` 屬性，`id` 是傳給 `forumCategory` 的引數。
1. 如果 `allCategories` 物件沒有所選分類 id 作為其屬性，則應該將 `category` 顯示為 `General`，並將 `className` 顯示為 `general`。
1. 你應該有一個名為 `avatars` 的函式，該函式接受兩個分別代表發文者和使用者的陣列。
1. `avatars` 函式應該傳回一個字串，內容為將 `img` 元素串接而成，每個 `user_id` 在 `posters` 陣列中各有一個。請透過在 `posters` 陣列中查找 `user_id` 屬性來找到 `img` URL，並在 `users` 陣列中找到對應的 `id` 屬性。
1. `avatars` 函式應該透過存取找到的使用者物件（在 `users` 陣列中，將其 `id` 與 `posters` 中的 `user_id` 比較）中的 `avatar_template` 屬性，並將 URL 字串中的 `{size}` 佔位符取代為 `30`，來設定每個頭像的大小。
1. 每個圖片元素都應該有一個 alt 文字，其值為發文者的 `name` 屬性。
1. 每個 `img` 元素都應該有一個 `src` 屬性，值為對應使用者的 `avatar_template`。如果 `avatar_template` 以 `/` 開頭，則直接在前面加上 `avatarUrl`。
1. 你應該有一個名為 `showLatestPosts` 的函式，該函式接受一個參數。
1. `showLatestPosts` 應該從傳入的物件中取出 `users` 和 `topic_list` 屬性。同時，還應該處理 `topic_list` 中 `topics` 陣列的物件下列屬性：
   - `id`：貼文的 id
   - `title`：貼文的標題
   - `views`：貼文的瀏覽次數
   - `posts_count`：主題的回覆數
   - `slug`：貼文的 slug
   - `posters`：該主題的發文者
   - `category_id`：該貼文的分類 id（整數）
   - `bumped_at`：ISO 8601 格式的時間戳
1. `showLatestPosts` 應該將 `#posts-container` 的 inner HTML 設為一個字串，內容為將 `tr` 元素串接而成，每個 `topics` 項目各有一個。
1. 每個 `tr` 元素應該包含五個 `td` 元素：
   - 一個 `td`，其中包含兩個錨點元素，一個 class 為 `post-title`，`href` 為 `<forumTopicUrl><slug>/<id>`，錨點文字為 `<title>`，另一個則是呼叫 `forumCategory` 並傳入 `category_id` 得到。
   - 一個 `td`，其中包含一個 class 為 `avatar-container` 的 `div` 元素，裡面包含呼叫 `avatars` 並傳入 `posters` 和 `users` 得到的圖片。
   - 一個 `td`，內容為該貼文的回覆數。_提示：_ 請使用 `posts_count - 1`。
   - 一個 `td`，內容為該貼文的瀏覽次數。
   - 一個 `td`，內容為自上次活動以來經過的時間。
1. 你應該有一個名為 `fetchData` 的 async 函式。
1. `fetchData` 函式應該從 `forumLatest` 取得資料，並將解析為 JSON 的回應傳給 `showLatestPosts`。
1. 如果在取得資料時發生錯誤，`fetchData` 函式應該將錯誤記錄到主控台。你應該特別使用 `console.log` 來完成這件事。

# --hints--

你應該有一個名為 `timeAgo` 的函式，該函式接受一個引數。
...
當傳入的時間與目前時間的差異為 `50` 分鐘時，`timeAgo` 應該傳回 `50m ago`。
...
當傳入的時間與目前時間的差異為 `60` 分鐘時，`timeAgo` 應該傳回 `1h ago`。
...
當傳入的時間與目前時間的差異為 `115` 分鐘時，`timeAgo` 應該傳回 `1h ago`。
...
當傳入的時間與目前時間的差異為 `15` 小時時，`timeAgo` 應該傳回 `15h ago`。
...
當傳入的時間與目前時間的差異為 `24` 小時時，`timeAgo` 應該傳回 `1d ago`。
...
當傳入的時間與目前時間的差異為 `46` 小時時，`timeAgo` 應該傳回 `1d ago`。
...
當傳入的時間與目前時間的差異為 `3` 天時，`timeAgo` 應該傳回 `3d ago`。
...
你應該有一個名為 `viewCount` 的函式，該函式接受一個引數。
...
你應該有一個名為 `forumCategory` 的函式，該函式接受一個引數。
...
`forumCategory(299)` 應該傳回一個包含文字 `Career Advice` 的錨點元素字串。
...
`forumCategory(299)` 應該傳回一個包含 `href="https://forum.freecodecamp.org/c/career/299"` 的錨點元素字串。
...
`forumCategory(299)` 應該傳回一個包含 `class="category career"` 的錨點元素字串。
...
`forumCategory(200)` 應該傳回一個包含文字 `General` 的錨點元素字串。
...
`forumCategory(200)` 應該傳回一個包含 `href="https://forum.freecodecamp.org/c/general/200"` 的錨點元素字串。
...
`forumCategory(200)` 應該傳回一個包含 `class="category general"` 的錨點元素字串。
...
你應該有一個名為 `avatars` 的函式，該函式接受兩個引數。
...
`avatars` 函式應該傳回一個字串，內容為將 `img` 元素串接而成，每個在使用者陣列中找到的發文者各有一個。
...
`avatars` 函式傳回的字串中，每個 `img` 元素都應該有一個 `alt` 文字，值為發文者的 `name` 屬性。
...
`avatars` 函式應該透過存取 `avatar_template` 屬性，並將 `{size}` 取代為 `30`，來設定每個頭像的大小。
...
每個 `img` 元素都應該有一個 `src` 屬性，值為對應使用者的 `avatar_template`。如果 `avatar_template` 以 `/` 開頭，則直接在前面加上 `avatarUrl`。
...
你應該有一個名為 `showLatestPosts` 的函式，該函式接受一個參數。
...
你應該有一個名為 `fetchData` 的函式。

```js
assert.isFunction(fetchData);
```

你的 `fetchData` 函式應該從 `https://cdn.freecodecamp.org/curriculum/forum-latest/latest.json` 取得資料。
...
你的 `fetchData` 函式應該呼叫 `showLatestPosts`。
...
如果發生錯誤，你的 `fetchData` 函式應該使用 `console.log` 將錯誤記錄到主控台。
...
`showLatestPosts` 應該將 `#posts-container` 的 inner HTML 設為一個字串，內容為將 `tr` 元素串接而成，每個 `topics` 項目各有一個。
...
`showLatestPosts` 傳回的字串中，每個 `tr` 元素都應該包含 5 個 `td` 元素。
...
`showLatestPosts` 傳回的每一個表格列的第一個 `td` 元素都應該包含兩個錨點元素，第一個 class 為 `post-title`，`href` 為 `<forumTopicUrl><slug>/<id>`，錨點文字為 `<title>`，第二個則是呼叫 `forumCategory` 並傳入 `category_id` 得到。
...
`showLatestPosts` 傳回的每一個表格列的第二個 `td` 元素都應該包含呼叫 `avatars` 並傳入 `posters` 和 `users` 得到的圖片，這些圖片應該巢狀在一個 class 為 `avatar-container` 的 `div` 元素中。
...
`showLatestPosts` 傳回的每一個表格列的第三個 `td` 元素都應該包含該貼文的回覆數。_提示：_ 請使用 `posts_count - 1`。
...
`showLatestPosts` 傳回的每一個表格列的第四個 `td` 元素都應該包含該貼文的瀏覽次數。
...
`showLatestPosts` 傳回的每一個表格列的第五個 `td` 元素都應該包含自上次活動以來經過的時間，這個時間是使用 `timeAgo` 函式產生的。
