...
將標的 `.beak` 元素，並給予它們 `height` 為 `10%`、`background-color` 為 `orange`，以及所有角落的半徑為 `50%`。
...
你應該給 `.beak` 一個 `height` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.height, '10%');
```

你應該給 `.beak` 一個 `background-color` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.beak')?.backgroundColor, 'orange');
```

你應該給 `.beak` 一個 `border-radius` 為 `--fcc-expected--`，但找到的是 `--fcc-actual--`。
