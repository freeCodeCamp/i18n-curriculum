...
노래를 다시 렌더링하고, 현재 재생 중인 노래를 일시 정지하며, 플레이어 표시를 설정하고, 재생 버튼의 접근 가능한 텍스트도 다시 설정해야 합니다.

`renderSongs` 함수를 호출하고 `userData?.songs`을 인수로 전달하세요. 또한 `pauseSong`, `setPlayerDisplay`, `setPlayButtonAccessibleText` 함수도 호출하세요.

# --hints--

기존 `shuffle` 함수와 그 내용을 수정하지 마세요.
...
`renderSongs` 함수를 `userData?.songs`과 함께 호출해야 합니다.
