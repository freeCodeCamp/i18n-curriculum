...
Crea un elemento `button` con il `className` di `submit-btn` e un `type` di `submit`. Dai al pulsante il testo `Join the League`.

Dopo averlo fatto, prova a inviare il modulo e guarda cosa succede.
...
Il tuo elemento button dovrebbe avere un attributo `className` impostato su `submit-btn`.

```js
assert.exists(document.querySelector("button.submit-btn"));
```

Il tuo elemento `button` dovrebbe avere `Join the League` come testo.
