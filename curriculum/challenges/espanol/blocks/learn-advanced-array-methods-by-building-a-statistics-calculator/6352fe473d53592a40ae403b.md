...
Dentro de tu función de `.reduce()` vacía, declara una variable `diferencia` y establécela en el valor de `el` menos `media`. Luego declara una variable `cuadrado` y establécela en el valor de `diferencia` elevado a la potencia de `2`. Finalmente, devuelve el valor de `acc` más `cuadrado`.

# --hints--

Tu función de `reduce` debe tener una variable `diferencia`.

```js
assert.match(getVariance.toString(), /variance\s*=\s*array\.reduce\(\s*function\s*\(\s*acc\s*,\s*el\s*\)\s*\{\s*var\s+difference\s*=/);
```

Tu variable `difference` debe establecerse al valor de `el` menos `mean`.
...
Tu `reduce` callback debe tener una variable `squared`.
...
Tu variable `squared` debe establecerse al valor de `difference` a la potencia de 2.
...
Tu `reduce` callback debe volver el valor de `acc` más `squared`.
...
No debes eliminar el valor inicial de `0` de tu método `.reduce()`.
