...
`form`의 마지막 섹션에서는 사용자가 호텔에 대해 추가로 원하는 피드백을 제공할 수 있습니다.

`label` 텍스트와 `Other Comments?`로 설정된 `for` 속성을 가진 `"comments"` 요소를 추가하는 것부터 시작하세요.

# --hints--

`label`로 설정된 `for` 속성을 가진 `"comments"`가 있어야 합니다.

```js
assert.exists(document.querySelector('label[for="comments"]'));
```

`label`에는 `Other Comments?` 텍스트가 있어야 합니다.
