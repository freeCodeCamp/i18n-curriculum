...
现在你已经完成了打开和关闭模式的操作，接下来是从输入字段中获取值，将它们保存到 `taskData` 数组中，然后将它们显示在页面上。

首先，向 `taskForm` 元素添加 `submit` 事件监听器，并将 `e` 作为箭头函数的参数传入。 在花括号内，使用 `preventDefault()` 方法阻止浏览器在提交表单后刷新页面。
