...
Per completare la tua funzione `processGateFlow`, restituisci un oggetto contenente due proprietà:

- `processed`: il numero di partecipanti che il cancello è riuscito a processare durante questo tick.
- `overflow`: il numero di partecipanti rimasti in coda dopo il processo (che corrisponde a `currentTickQueue` dopo il loop).

Devi restituire direttamente l'oggetto in questo modo:
...
Devi restituire un oggetto con le proprietà `processed` e `overflow`. Restituisci direttamente l'oggetto senza assegnarlo a una variabile.
...
La proprietà `processed` dell'oggetto restituito deve avere il valore `processed`.
...
La proprietà `overflow` dell'oggetto restituito deve avere il valore `currentTickQueue`.
