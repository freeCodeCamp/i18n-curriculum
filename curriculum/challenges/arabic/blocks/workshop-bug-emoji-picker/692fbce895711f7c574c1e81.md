...
الآن داخل الدالة `isSelect`، أرجع قيمة منطقية تشير إلى ما إذا كان `element` مثيلاً لـ `HTMLSelectElement`. استخدم عامل `instanceof` لإجراء هذا الفحص.

# --hints--

يجب أن تُرجع دالتك `isSelect` القيمة `element instanceof HTMLSelectElement`.
