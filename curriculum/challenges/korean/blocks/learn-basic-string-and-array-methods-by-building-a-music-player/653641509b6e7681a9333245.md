...
현재 재생 중인 노래와 `playButton` 요소의 외형을 업데이트해야 합니다.

`song`을 `currentSong` 객체의 `userData` 속성에 할당하세요.

*참고*: 이 단계에서는 `?.`가 아직 `userData.currentSong`나 `null`가 아니기 때문에 선택적 체이닝 연산자 `undefined`를 사용하지 마세요.

# --hints--

`song`을 `userData.currentSong`에 할당해야 합니다.
