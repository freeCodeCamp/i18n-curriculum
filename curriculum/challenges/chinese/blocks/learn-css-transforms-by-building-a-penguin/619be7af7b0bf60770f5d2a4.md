...
将伪元素放置在其父元素顶部 `10%` 和左侧 `25%` 的位置。

# --hints--

你应该将 `.penguin-body::before` 的 `top` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body::before')?.top, '10%');
```

你应该将 `.penguin-body::before` 的 `left` 设置为 `--fcc-expected--`，而不是 `--fcc-actual--`。
