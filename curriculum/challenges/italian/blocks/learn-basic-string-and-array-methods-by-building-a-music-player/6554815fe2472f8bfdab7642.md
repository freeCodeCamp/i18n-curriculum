...
Usa `const` e la sintassi a freccia per creare una funzione `playPreviousSong` vuota.

# --hints--

Dovresti usare `const` per creare una funzione vuota chiamata `playPreviousSong`.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*/)
```

`playPreviousSong` dovrebbe essere una funzione.

```js
assert.isFunction(playPreviousSong)
```

La tua funzione `playPreviousSong` dovrebbe usare la sintassi a freccia.

```js
assert.match(code, /const\s+playPreviousSong\s*=\s*\(\s*\)\s*=>\s*/)
```

La tua funzione `playPreviousSong` dovrebbe essere vuota.
