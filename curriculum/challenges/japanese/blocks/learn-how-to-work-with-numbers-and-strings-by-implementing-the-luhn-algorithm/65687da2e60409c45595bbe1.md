...
1桁の掛け算の結果が`9`を超えないように、偶数桁のループ内で`if`が`number`以上かどうかをチェックする`10`文を追加してください。もしそうなら、`number`を出力してください。

# --hints--

`if number >= 10:`ループ内に`for`があるはずです。
