...
Per il terzo e il quarto punto di passaggio, ripeti ancora una volta il pattern `transform`.

# --hints--

Dovresti assegnare al punto di passaggio `30%` un `transform` di `rotate(110deg) scaleX(-1)`.
...
Dovresti assegnare al punto di passaggio `40%` un `transform` di `rotate(130deg) scaleX(-1)`.
