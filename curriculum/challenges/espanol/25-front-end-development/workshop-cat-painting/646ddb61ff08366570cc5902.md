...
Ahora comenzarás a trabajar en los ojos del gato. Al igual que las orejas, los ojos tendrán ojos interiores.
...
Debes crear un elemento `div` con la clase `cat-eyes`.

```js
assert.lengthOf(document.querySelectorAll('.cat-eyes'), 1);
```

Debes crear dos elementos `div` dentro del elemento `.cat-eyes`.

```js
assert.lengthOf(document.querySelectorAll('.cat-eyes div'), 2);
```

El primer elemento `div` dentro del elemento `.cat-eyes` debe tener la clase `cat-left-eye`.

```js
assert.isTrue(document.querySelectorAll('.cat-eyes div')[0].classList.contains('cat-left-eye'));
```

El segundo elemento `div` dentro del elemento `.cat-eyes` debe tener la clase `cat-right-eye`.
