...
你應該在 `background` 屬性中爲 `.fb5` 提供第二個 `repeating-linear-gradient`。
...
你應該爲第二個 `repeating-linear-gradient` 設置從 `0%` 到 `12%` 是第一個顏色 `--building-color2`。
...
你應該爲第二個 `repeating-linear-gradient` 設置從 `12%` 到 `44%` 是第二種顏色 `--window-color2`。
