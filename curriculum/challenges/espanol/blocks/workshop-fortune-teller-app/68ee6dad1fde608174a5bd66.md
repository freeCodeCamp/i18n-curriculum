...
Dentro del método `singleCardSelected`, recupera el elemento `singleCard` y actualiza su `innerHTML` llamando a la función `renderCard`. Pasa `"Click the card and reveal the fortune"` como primer argumento y `isReversed` como segundo argumento.

# --hints--

Debes actualizar `this.elements.singleCard.innerHTML` llamando a la función `renderCard`.
...
Tu llamada a `renderCard` debe tener `"Click the card and reveal the fortune"` como primer argumento.
...
Debes pasar `isReversed` como segundo argumento a `renderCard`.
