...
`__str__` メソッドはクラスの属性を直接参照していますが、getterを作成したので、それらを使って値を取得するほうが良いです。

`__str__` メソッドを編集して、属性を直接参照するのではなく、getterを使うようにしてください。

# --hints--

`Projectile(45, 45, 45)` の文字列表現は正しくする必要があります。
...
他のインスタンスでも文字列表現が正しいことを確認してください。
