...
Substitua `username` pelo seu nome de usuário e `database_name` pelo banco de dados ao qual você deseja se conectar.

Se você ainda não criou seu próprio banco de dados, o PostgreSQL inclui automaticamente um banco de dados padrão chamado `postgres` quando é instalado. Você pode usar esse banco de dados `postgres` para acompanhar.
