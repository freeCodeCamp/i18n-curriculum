...
O mês é baseado em zero, então `January` é `0`, `February` é `1` e assim por diante. Se o mês for inválido, ele retornará `NaN`.
...
Se o ano for inválido, ele retornará `NaN`.
