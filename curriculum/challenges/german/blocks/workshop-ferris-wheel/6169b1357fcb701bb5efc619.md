...
Erstellen Sie einen neuen `25%`-Selektor zwischen Ihren `0%`- und `50%`-Selektoren. Geben Sie diesem neuen Selektor die `background-color`-Eigenschaft mit dem Wert `yellow`.

# --hints--

Sie sollten einen neuen `25%`-Selektor in Ihrer `@keyframes cabins`-Regel erstellen.
...
Ihr `25%`-Selektor sollte zwischen Ihren `0%`- und `50%`-Selektoren liegen.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText , '25%');
```

Ihr `25%`-Selektor sollte eine `background-color`-Eigenschaft mit dem Wert `yellow` haben.
