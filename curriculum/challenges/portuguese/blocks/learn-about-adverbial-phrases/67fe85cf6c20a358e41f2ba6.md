...
`Switch` means to change or change from one state, system, or method to another. Por exemplo:
...
`Unless` would be the same as `except if`. It is used to introduce an exception or condition that must be true for the main action to not happen. Por exemplo:
