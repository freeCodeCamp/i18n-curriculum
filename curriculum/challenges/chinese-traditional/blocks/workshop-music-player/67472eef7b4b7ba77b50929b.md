...
在你的 `userData` 物件中，創建一個 `songs` 屬性並將其值設為 `allSongs` 陣列。

然後，為了控制代碼目前歌曲的資訊並追蹤其播放時間，創建 `currentSong` 和 `songCurrentTime` 屬性。分別將值設為 `null` 和 `0`。

# --hints--

你的 `userData` 物件應該有一個 `songs` 鍵，設定為 `allSongs` 陣列。

```js
assert.propertyVal(userData, "songs", allSongs);
```

你的 `userData` 物件應該有一個 `currentSong` 鍵，其值設定為 `null`。

```js
assert.propertyVal(userData, "currentSong", null);
```

你的 `userData` 物件應該有一個 `songCurrentTime` 鍵，其值設定為 `0`。
