...
当前，你的 `instrumentCards` 函数返回一个包含记录仪对象的数组，因此你需要再进行几步操作，才能在页面上显示你的记录仪卡片。

修改你的函数，使其返回一个包含字串的数组，这些字串包含显示记录仪卡片的超文本标记语言代码，每个字串对应 `instruments` 数组中的一个对象。字串应具有此格式 `<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`
...
`instrumentCards("all")` 应该返回一个表现乐器卡片的字串数组，每个字串的形式为 `<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`。
...
`instrumentCards("woodwinds")` 应该返回一个表现乐器卡片的字串数组，每个字串的形式为 `<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`。
...
`instrumentCards("brass")` 应该返回一个表现乐器卡片的字串数组，每个字串的形式为 `<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`。
...
`instrumentCards("percussion")` 应该返回一个表现乐器卡片的字串数组，每个字串的形式为 `<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`。
...
`instrumentCards("all")` 应返回一个包含十个字串的数组，表现你页面上所有记录仪的记录仪卡。
...
`instrumentCards("woodwinds")` 应该返回一个包含三个字串的数组，这些字串表现你页面上木管乐器的乐器卡。
...
`instrumentCards("brass")` 应该返回一个包含三个字串的数组，这些字串表现你页面上铜管乐器的乐器卡。
...
`instrumentCards("percussion")` 应该返回一个包含四个字串的数组，这些字串表现你页面上打击乐器的乐器卡。
