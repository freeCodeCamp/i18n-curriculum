...
イベントリスナーのアロー関数内に、`userData?.currentSong` が `null` かどうかをチェックする `if` 文を追加してください。

`if` 文の内部では、`playSong()` 関数に `userData?.songs` 配列の最初の曲の `id` を渡して呼び出してください。 これにより、プレイリスト内の一曲目が最初に再生されます。

# --hints--

`userData?.currentSong === null` の条件を使用して `if` 文を作成してください。

```js
assert.match(code, /if\s*\(\s*userData\?\.currentSong\s*===\s*null\s*\)\s*\{/);
```

`if` ブロック内では、`userData?.songs[0].id` を用いて `playSong` 関数を呼び出してください。
