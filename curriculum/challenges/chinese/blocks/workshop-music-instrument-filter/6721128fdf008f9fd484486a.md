...
为你的 `instrumentsArr` 添加另外两个对象，以表现剩余的木管乐器，单簧管和双簧管。

# --hints--

你应该在你的 `instrumentsArr` 中有 `{ category: "woodwinds", instrument: "Clarinet", price: 200 }`。

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

你应该在你的 `instrumentsArr` 中有 `{ category: "woodwinds", instrument: "Oboe", price: 4000 }`。

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

你应该仍然在你的 `instrumentsArr` 中有 `{ category: "woodwinds", instrument: "Flute", price: 500 }`。
