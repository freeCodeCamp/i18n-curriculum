...
`@media` アットルールには、ユーザーの好みに基づいて CSS を設定するための `prefers-reduced-motion` というメディア機能があります。これは次のいずれかの値を取ることができます：
...
`scroll-behavior: smooth` を設定するスタイルルールを、`@media` アットルール内にラップし、メディア機能 `prefers-reduced-motion` に `no-preference` を値として設定してください。
