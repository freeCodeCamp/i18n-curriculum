...
Agora que você criou o botão, você precisa atribuir a ele um texto. Para fazer isso, você precisa usar o método `createTextNode()` do DOM.

O método `createTextNode()` é usado para criar um nó de texto. Para usá-lo, você o chama e passa o texto como uma string:
...
Use o método `createTextNode()` para criar um texto `"Reset Playlist"`, depois atribua-o a uma constante `resetText`.
...
Você deve usar `document.createTextNode()` para criar um texto `"Reset Playlist"`.

```js
assert.match(code, /document\.createTextNode\(\s*('|")Reset\s+Playlist\1\s*\)\s*;?/)
```

Você deve atribuir seu nó de texto criado a uma constante `resetText`.
