...
もしソーシャルアイコンをもっと追加して同じ行に並べたい場合は、`grid-template-columns` を更新して追加の列を作成する必要があります。代わりに、`grid-auto-flow` プロパティを使うこともできます。

このプロパティは最初の値に `row` または `column` を取り、2番目の値として `dense` をオプションで指定できます。`grid-auto-flow` は自動配置アルゴリズムを使ってグリッドレイアウトを調整します。`column` に設定すると、必要に応じてコンテンツのために新しい列を作成するようアルゴリズムに指示します。`dense` の値はアルゴリズムが戻ってグリッドの空きスペースを小さいアイテムで埋めることを許可し、その結果アイテムが順序通りに表示されない場合があります。

`.social-icons` セレクターには、`grid-auto-flow` プロパティを `column` に設定してください。
