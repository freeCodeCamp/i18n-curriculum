...
Wenn Sie sich Ihr HTML ansehen, ist der erste `tab` standardmäßig bereits ausgewählt und hat ein `aria-selected`-Attribut mit dem Wert `"true"`. Wenn ein Benutzer auf einen anderen Tab klickt, sollten zunächst alle Tabs auf `aria-selected="false"` gesetzt werden.

Um dies zu erreichen, können Sie die `setAttribute`-Methode verwenden, um das `aria-selected`-Attribut jedes Tabs zu ändern.

Hier ist ein Beispiel, wie Sie die `setAttribute`-Methode auf einer Liste von `div`-Elementen verwenden:
...
Verwenden Sie eine `forEach` auf der `tabs`-Sammlung, um durch jeden Tab zu iterieren und dessen `aria-selected`-Attribut auf `"false"` zu setzen. Für die Callback-Funktion können Sie jeden beliebigen Parameternamen verwenden, aber es ist am besten, `tab` nicht zu verwenden, da dieser bereits in der äußeren `forEach`-Schleife verwendet wird.

# --hints--

Wenn der Benutzer auf einen Tab klickt, sollten alle Tabs ihr `aria-selected`-Attribut auf `"false"` gesetzt haben.
