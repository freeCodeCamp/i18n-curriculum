...
构建一个功能上类似于此的全栈 JavaScript 应用程序：<a href="https://anonymous-message-board.freecodecamp.rocks/" target="_blank" rel="noopener noreferrer nofollow">https://anonymous-message-board.freecodecamp.rocks/</a>。
...
你可以向 `/api/threads/{board}` 发送包含 `text` 和 `delete_password` 的表单数据的 POST 请求。储存的数据库记录将至少包含字段 `_id`、`text`、`created_on`（日期和时间）、`bumped_on`（日期和时间，初始值与 `created_on` 相同）、`reported`（布尔）、`delete_password` 和 `replies`（数组）。
...
你可以发送一个 POST 请求到 `/api/replies/{board}`，请求中包含包括 `text`、`delete_password` 和 `thread_id` 的表单数据。这将把 `bumped_on` 日期更新为评论的日期。在该线程的 `replies` 数组中，将储存一个至少包含属性 `_id`、`text`、`created_on`、`delete_password` 和 `reported` 的对象。
...
你可以发送 DELETE 请求到 `/api/threads/{board}` 并传递 `thread_id` 和 `delete_password` 来删除该线程。返回的将是字串 `incorrect password` 或 `success`。
...
You can send a DELETE request to `/api/replies/{board}` and pass along the `thread_id`, `reply_id`, & `delete_password`. 返回的将是字符串 `incorrect password` 或 `success`。 成功后，`reply_id` 的文本将更改为 `[deleted]`。
...
You can send a PUT request to `/api/replies/{board}` and pass along the `thread_id` & `reply_id`. 返回的将是字符串 `reported`。 `reply_id` 的 `reported` 值将被改变为 `true`。
