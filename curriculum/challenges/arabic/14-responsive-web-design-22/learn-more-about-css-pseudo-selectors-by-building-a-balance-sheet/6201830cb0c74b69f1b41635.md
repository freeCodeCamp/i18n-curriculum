...
يجب يحتوي لمنتقي `table` الخاص بك على الخاصية `width` بقيمة `100%`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('width') === '100%');
```

يجب يحتوي لمنتقي `table` الخاص بك على الخاصية `position` بقيمة `relative`.

```js
assert(new __helpers.CSSHelp(document).getStyle('table')?.getPropertyValue('position') === 'relative');
```

يجب يحتوي لمنتقي `table` الخاص بك على خاصية `margin-top` بقيمة `3rem`.
