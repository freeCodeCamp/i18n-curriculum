...
Im Moment ist `currentContent` mit dem Wert eines leeren Strings initialisiert. Das gewünschte Ergebnis wäre jedoch, dass `currentContent` den Wert des aktuellen Notiztexts enthält.

In einer früheren Lektion haben Sie gelernt, wie man mit dem `DOMContentLoaded`-Ereignis so arbeitet:
...
Wenn alles im HTML-Dokument geladen und geparst wurde, können Sie sicher auf `noteEl` zugreifen.

Beginnen Sie damit, eine `addEventListener`-Methode an das `window`-Objekt anzuhängen. Das erste Argument für die `addEventListener`-Methode sollte das `"DOMContentLoaded"`-Ereignis sein. Das zweite Argument sollte eine Pfeilfunktion sein. Weisen Sie im Körper der Pfeilfunktion den Wert von `noteEl.textContent` `currentContent` zu.

# --hints--

Sie sollten eine `addEventListener`-Methode an das `window`-Objekt anhängen.

```js
assert.match(code, /window\.addEventListener/);
```

Ihr Event Listener sollte auf das `"DOMContentLoaded"`-Ereignis hören.

```js
assert.match(code, /window\.addEventListener\(['"]DOMContentLoaded['"]\,/);
```

Weisen Sie im Körper der Pfeilfunktion den Wert von `noteEl.textContent` `currentContent` zu.
