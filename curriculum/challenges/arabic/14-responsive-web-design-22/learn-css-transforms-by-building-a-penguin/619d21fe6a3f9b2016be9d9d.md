...
يجب عليك استخدام منتقي `.foot.left`.

```js
assert.match(code, /\.foot\.left\s*\{/);
```

يجب أن يحتوي منتقي `.foot.left` على خاصية `left` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foot.left')?.left, '25%');
```

يجب عليك استخدام منتقي `.foot.right`.

```js
assert.match(code, /\.foot\.right\s*\{/);
```

يجب أن يحتوي منتقي `.foot.right` على خاصية `right` بقيمة `--fcc-expected--`, ولكن وجدت بقيمة `--fcc-actual--`.
