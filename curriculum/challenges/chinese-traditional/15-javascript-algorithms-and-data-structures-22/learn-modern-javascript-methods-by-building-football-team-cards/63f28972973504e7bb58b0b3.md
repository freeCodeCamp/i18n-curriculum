...
下一步是創建一個函數，用於檢測用戶何時從 `playersDropdownList` 中做出選擇。

在 `playersDropdownList` 上使用 `.addEventListener()` 方法。 在事件監聽器內部，傳入一個 `"change"` 事件類型和一個空的回調函數。

# --hints--

你應該在 `playersDropdownList` 變量上調用 `.addEventListener()` 方法。
