...
**目標：** 建置一個功能上類似於 <a href="https://technical-documentation-page.freecodecamp.rocks" target="_blank" rel="noopener noreferrer nofollow">https://technical-documentation-page.freecodecamp.rocks</a> 的應用程式。**請勿複製此示範專案**。

**使用者故事：**

1. 你可以看到一個帶有對應 `id="main-doc"` 的 `main` 元素，其中包含頁面的主要內容（技術文件）。
1. 在 `#main-doc` 元素中，你可以看到數個 `section` 元素，每個都有一個 `main-section` 的類別。應該至少有五個。
1. 每個 `.main-section` 中的第一個元素應該是包含描述該區段主題文字的 `header` 元素。
1. 每個具有 `main-section` 類別的 `section` 元素也應該有一個與其中包含的每個 `header` 文字相對應的 `id`。任何空格都應該取代為底線（例如，包含標題 "JavaScript and Java" 的區段應該有對應的 `id="JavaScript_and_Java"`）。
1. `.main-section` 元素應該總共包含至少十個 `p` 元素（不是每個）。
1. `.main-section` 元素總共應包含至少五個 `code` 元素（不是每個）。
1. `.main-section` 元素總共應包含至少五個 `li` 項目（不是每個）。
1. 你可以看到一個帶有對應 `id="navbar"` 的 `nav` 元素。
1. navbar 元素應包含一個 `header` 元素，該元素包含描述技術文件主題的文字。
1. 此外，導覽列應包含具有 `nav-link` 類別的聯結（`a`）元素。每個具有 `main-section` 類別的元素都應該有一個。
1. `#navbar` 中的 `header` 元素必須位於導覽列中任何聯結（`a`）元素之前。
1. 每個具有 `nav-link` 類別的元素應包含與每個 `section` 中的 `header` 文字相對應的文字（例如，如果你有一個「Hello world」的區段/標題，你的導覽列應該有一個包含「Hello world」文字的元素）。
1. 當你點擊導覽列元素時，頁面應該導向 `#main-doc` 元素的對應區段（例如，如果你點擊包含文字 "Hello world" 的 `.nav-link` 元素，頁面會導向具有該 id 且包含對應標題的 `section` 元素）。
1. 在一般尺寸的裝置（筆記型電腦、桌上型電腦）上，具有 `id="navbar"` 的元素應該顯示在螢幕的左側，並且應該始終對使用者可見。
1. 你的技術文件應該至少使用一個媒體查詢
...
你應該有一個 `main` 元素，其 `id` 為 `main-doc`。
...
你應該至少有五個具有 `main-section` 類別的 `section` 元素。
...
你的所有 `.main-section` 元素都應該是 `section` 元素。
...
你應該至少有五個作為 `#main-doc` 後代的 `.main-section` 元素。
...
每個 `.main-section` 的第一個子元素應該是 `header` 元素。
...
每個 `.main-section` 應該有一個 `id`，該 `id` 與其第一個子元素的文字相符，並將子元素文字中的所有空格取代為底線（`_`）作為 `id`。
...
你的 `.main-section` 元素中應該至少有 10 個 `p` 元素（總計）。
...
你應該至少有五個屬於 `.main-section` 元素的子孫的 `code` 元素。
...
你應該至少有五個作為 `.main-section` 元素子孫的 `li` 元素。
...
你應該擁有相同數量的 `.nav-link` 和 `.main-section` 元素。
...
`#navbar` 中的 `header` 元素應該位於同樣在 `#navbar` 中的任何連結（`a`）元素之前。
...
每個 `.nav-link` 應該有與其相關 `section` 的 `header` 文字相對應的文字（例如，如果你有一個「Hello world」的 section/header，你的 `#navbar` 應該有一個 `.nav-link`，其文字為「Hello world」）。
...
每個 `.nav-link` 都應該有一個 `href` 屬性，該屬性聯結到其對應的 `.main-section`（例如，如果你點擊包含文字 "Hello world" 的 `.nav-link` 元素，頁面會導向具有該 id 的 `section` 元素）。
