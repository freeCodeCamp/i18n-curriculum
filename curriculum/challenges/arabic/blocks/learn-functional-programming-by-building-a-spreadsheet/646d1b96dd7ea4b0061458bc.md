...
يمكنك تمرير <dfn>مرجع</dfn> دالة كمعلمة استدعاء راجع. مرجع الدالة هو اسم الدالة بدون الأقواس العادية. على سبيل المثال:
...
طريقة `.map()` هنا ستُنفذ استدعاء الدالة `myFunc`، مع تمرير نفس المعلمات التي تأخذها دالة الاستدعاء الراجع لـ `.map()`. المعلمة الأولى هي قيمة المصفوفة في التكرار الحالي، لذا ستكون `newArray` هي `[value: 1, value: 2, value: 3]`.

مرر مرجعًا إلى دالتك `elemValue` كاستدعاء راجع لطريقة `.map()` الخاصة بك.

# --hints--

يجب ألا تستدعي دالتك `elemValue`.
...
يجب أن تمرر مرجعًا إلى `elemValue` كاستدعاء راجع لطريقة `.map()` الخاصة بك.
