...
其次，每個作者的姓名和圖像與卡片上的其他詳細信息之間沒有太多的分隔。 分隔符將爲作者卡片提供清晰的視覺層次。

在作者簡介上方添加 `div` 元素，併爲其賦予 `class``"purple-divider"`。
...
你的 `div` 元素應將 `class` 設置爲 `"purple-divider"`。
