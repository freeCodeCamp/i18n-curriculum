...
Poi, esci dall'esecuzione della funzione.

# --hints--

La funzione callback dovrebbe avere un'istruzione `if`.

```js
assert.match(code, /checkMessageButton\.addEventListener\(\s*('|"|`)click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*if\s*\(/)
```

L'istruzione `if` dovrebbe controllare se il `value` di `messageInput` è una stringa vuota.
...
L'istruzione `if` dovrebbe terminare l'esecuzione della funzione.
