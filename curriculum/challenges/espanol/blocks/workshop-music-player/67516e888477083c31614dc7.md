...
Crea una función llamada `highlightCurrentSong`. Dentro de la función, verifica que exista un elemento `.playlist-song` con `aria-current="true"`. Si es así, utiliza el método `removeAttribute` para eliminar el atributo `aria-current` de él.

# --hints--

Deberías tener una función llamada `highlightCurrentSong`.

```js
assert.isFunction(highlightCurrentSong)
```

Tu función `highlightCurrentSong` debería usar el método `removeAttribute` para eliminar el atributo `aria-current` del elemento `.playlist-song` con `aria-current="true"` si ese elemento existe.
