...
Você precisa adicionar o `resetText` ao elemento `resetButton` como um filho e também o `resetButton` ao elemento `playlistSongs` como um filho.Para isso, existe um método `appendChild()` para usar.

`appendChild()` permite que você adicione um nó ou um elemento como filho de outro elemento.No exemplo abaixo, o texto "Click me" seria anexado ao botão:
...
Use `appendChild()` para anexar `resetText` ao elemento `resetButton` e `resetButton` ao elemento `playlistSongs`.

# --hints--

Você deve usar o método `appendChild()` em `resetButton`.

```js
assert.match(code, /resetButton\.appendChild\(/)
```

Você deve passar `resetText` como o valor do seu primeiro `appendChild()`.

```js
assert.match(code, /resetButton\.appendChild\(\s*resetText\s*\)\s*;?/)
```

Você deve usar o método `appendChild()` em `playlistSongs`.

```js
assert.match(code, /playlistSongs\.appendChild\(/)
```

Você deve passar `resetButton` como o valor do seu segundo `appendChild()`.
