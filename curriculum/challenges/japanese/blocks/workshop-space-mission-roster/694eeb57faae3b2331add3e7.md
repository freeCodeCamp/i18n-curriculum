...
`getEVAReadyCrew` 関数内で、ローカルの `sortByPriorityDescending()` 配列を渡して `eligible` を呼び出し、返す前にソートされるようにしてください。

# --hints--

ローカルの `sortByPriorityDescending()` 配列を使って `eligible` を呼び出す必要があります。
