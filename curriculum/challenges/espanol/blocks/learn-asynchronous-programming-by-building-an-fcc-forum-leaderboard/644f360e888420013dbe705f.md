...
Dentro de tu cláusula `else`, asigna la cadena `"general"` a `selectedCategory.className`.

Debajo de eso, asigna la cadena `"General"` a `selectedCategory.category`.

Finalmente, asigne el número `1` a `selectedCategory.id`.

# --hints--

Debe asignar la cadena `"general"` a `selectedCategory.className`.

```js
assert.match(code, /selectedCategory\.className\s*=\s*('|"|`)general\1/);
```

Debe asignar la cadena `"General"` a `selectedCategory.category`.

```js
assert.match(code, /selectedCategory\.category\s*=\s*('|"|`)General\1/);
```

Debe asignar el número `1` a `selectedCategory.id`.
