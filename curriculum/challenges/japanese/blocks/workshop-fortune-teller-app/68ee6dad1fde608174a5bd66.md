...
`singleCardSelected` メソッド内で、`singleCard` 要素を取得し、`innerHTML` 関数を呼び出して `renderCard` を更新してください。`"Click the card and reveal the fortune"` を最初の引数として渡し、`isReversed` を2番目の引数として渡します。

# --hints--

`this.elements.singleCard.innerHTML` 関数を呼び出して `renderCard` を更新してください。
...
`renderCard` の呼び出しでは、`"Click the card and reveal the fortune"` を最初の引数にする必要があります。
...
`isReversed` に2番目の引数として `renderCard` を渡してください。
