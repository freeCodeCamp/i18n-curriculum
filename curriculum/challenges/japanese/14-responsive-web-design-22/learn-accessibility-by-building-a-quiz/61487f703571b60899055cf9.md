...
`section` 要素の幅が親要素の `80%` となるように設定してください。 次に、マージンを使用して `section` 要素を中央に配置し、下部に `10px` のマージンを追加してください。

また、`section` 要素の幅が `600px` より大きくならないようにしてください。
