...
Camperbot hat eine Blogseite für eine Programmierreise erstellt, aber es sieht so aus, als hätte die Seite einige Barrierefreiheitsprobleme und schlechte Praktiken. Ihre Aufgabe in diesem Workshop ist es, diese Probleme für Camperbot zu beheben.

Die erste Fehlergruppe betrifft die Verwendung von Überschriften. In einer vorherigen Lektion haben Sie gelernt, dass es eine bewährte Vorgehensweise ist, pro Seite nur eine `h1` zu verwenden. Aber es sieht so aus, als würde Camperbot mehrere `h1` verwenden.

Lassen Sie die `<h1>Welcome to Camperbot's Blog</h1>` unverändert und ändern Sie die anderen `h1`-Elemente in `h2`-Elemente.

# --hints--

Sie sollten die `<h1>Welcome to Camperbot's Blog</h1>` nicht verändern.
...
Sie sollten das `h1`-Element mit dem Text `My Journey Learning to Code` in ein `h2`-Element ändern.
...
Sie sollten das `h1`-Element mit dem Text `Accessibility Matters` in ein `h2`-Element ändern.
...
Sie sollten das `h1`-Element mit dem Text `What's Next?` in ein `h2`-Element ändern.
