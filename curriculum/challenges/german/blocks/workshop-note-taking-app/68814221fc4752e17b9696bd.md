...
Wenn der Benutzer auf die Notiz klickt, Änderungen vornimmt und dann außerhalb der Notiz klickt, benötigen Sie eine Möglichkeit, diese neu bearbeitete Version zu erhalten. Dies können Sie erreichen, indem Sie das `blur`-Ereignis verwenden. Dieses Ereignis wird ausgelöst, wenn ein Element den Fokus verliert. Hier ist ein Beispiel:
...
Hängen Sie eine `addEventListener`-Methode an die `noteEl`-Variable an. Das erste Argument sollte das `"blur"`-Ereignis sein und das zweite Argument eine Pfeilfunktion. Innerhalb des Körpers dieser Pfeilfunktion verwenden Sie `const`, um eine neue Variable namens `newContent` zu erstellen und ihr den Wert von `noteEl.innerHTML` zuzuweisen.

`innerHTML` wird hier verwendet, um den Abstand und die Formatierung der Notiz beizubehalten.

# --hints--

Sie sollten eine `addEventListener`-Methode an die `noteEl`-Variable anhängen.

```js
assert.match(code, /noteEl\.addEventListener/);
```

Ihre `addEventListener`-Methode sollte als erstes Argument das `"blur"`-Ereignis haben.

```js
assert.match(code, /noteEl\.addEventListener\(['"]blur['"],/);
```

Innerhalb des Körpers der Pfeilfunktion sollten Sie `const` verwenden, um eine neue Variable namens `newContent` zu erstellen und ihr den Wert von `noteEl.innerHTML` zuzuweisen.
