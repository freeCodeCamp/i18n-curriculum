...
`font` 元素是一個已被棄用的元素，用於設置文本的字體大小和顏色。 Here's an example of a `font` element.
...
`center` 元素是另一個已被棄用的元素，用於將內容水平居中放置在其容器中。 Here's an example of the `center` element that contains text and a paragraph element.
...
接下來是 `big` 元素。 這是另一個已被棄用的表現性 HTML 元素，它能使被括入的文本比周圍的文本大一級。 Here we have an example that defines a paragraph with two parts.
