...
Finalmente, deberías renderizar las canciones nuevamente, actualizar el texto accesible del botón de reproducción y eliminar el botón de reinicio de la lista de reproducción. También necesitas eliminar el `resetButton` del DOM.

Llama a la función `renderSongs()` con `sortSongs()` como argumento para renderizar las canciones nuevamente en orden alfabético.

Llama a la función `setPlayButtonAccessibleText()` para actualizar el texto accesible del botón de reproducción.

Elimina el botón de reinicio de la lista de reproducción llamando al método `remove()` en la variable `resetButton`.

**Nota**: Ahora puedes intentar eliminar todas las canciones para ver qué sucede.

# --hints--

No debe modificar el listener de eventos existente y su contenido.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Debes llamar a la función `renderSongs` con `sortSongs()`.
...
Debes utilizar el método `remove()` para eliminar el `resetButton` del DOM.
