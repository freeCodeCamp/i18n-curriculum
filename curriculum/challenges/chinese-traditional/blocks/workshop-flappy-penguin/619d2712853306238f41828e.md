...
創建一個名為 `--penguin-skin` 的訂製 CSS 變數，並將其設定為 `gray`。然後，將所有相關的屬性值替換為它。

# --hints--

你應該給 `:root` 一個 `--penguin-skin` 屬性。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

你應該給 `--penguin-skin` 一個 `gray` 的值，但發現了 `--fcc-actual--`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

你應該給 `.penguin-head` 一個 `background`，其值為 `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`。
...
你應該給 `.penguin-body::before` 一個 `background-color` 為 `var(--penguin-skin)`，但發現是 `--fcc-actual--`。
...
你應該給 `.arm` 一個 `background` 為 `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`。
