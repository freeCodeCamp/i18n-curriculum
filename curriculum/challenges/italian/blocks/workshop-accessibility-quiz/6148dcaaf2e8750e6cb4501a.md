...
Fai apparire gli elementi `h3` con una priorità più alta, usando le seguenti proprietà CSS:
...
Dovresti usare il selettore dell'elemento `h3`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('h3'));
```

Dovresti assegnare all'elemento `h3` una `margin-top` di `5px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.marginTop, '5px');
```

Dovresti assegnare all'elemento `h3` una `padding-left` di `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h3')?.paddingLeft, '15px');
```

Dovresti assegnare all'elemento `h3` una `font-size` di `1.375ren`.
