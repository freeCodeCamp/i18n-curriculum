...
最後に `last_visit_id` というキーを `constraints` 辞書に追加してください。その値には `isinstance` を使って、`last_visit_id` が文字列であることを確認してください。

# --hints--

あなたの `constraints` 辞書には `last_visit_id` というキーがあるはずです。
...
あなたの `last_visit_id` 辞書の `constraints` キーは `isinstance(last_visit_id, str)` の値を持つべきです。
