...
O elemento `.card` precisa de mais estilo: adicione um `border-radius` de `10px` e um `box-shadow` com valor `0 4px 8px gray`.

# --hints--

O seletor `.card` deve ter uma propriedade `border-radius` com valor `10px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("border-radius"), "10px");
```

O seletor `.card` deve ter uma propriedade `box-shadow` com valor `0 4px 8px gray`.
