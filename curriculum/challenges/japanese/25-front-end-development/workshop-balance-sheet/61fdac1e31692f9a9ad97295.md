...
CSS の `clip` プロパティは、要素の可視部分を定義するために使われます。 `span[class~="sr-only"]` セレクターの `clip` プロパティを `rect(1px, 1px, 1px, 1px)` に設定してください。

The `clip-path` property determines the shape the `clip` property should take. `clip-path` プロパティを `inset(50%)` の値に設定し、clip-path を要素内部の長方形に整形してください。
