...
Continuando con el contenedor principal, dale a la clase `container` un `width` de `80%` y un `max-width` de `600px`. Además, agrega un `border-radius` de `10px` y establece el `margin` en `20px auto` para centrarlo horizontalmente.
...
Establece la propiedad `width` en `80%`.
...
Establece la propiedad `max-width` en `600px`.
...
Establece la propiedad `border-radius` en `10px`.
...
Establece la propiedad `margin` en `20px auto`.
