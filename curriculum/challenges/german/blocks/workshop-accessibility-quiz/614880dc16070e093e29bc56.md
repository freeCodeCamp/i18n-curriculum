...
Ersetzen Sie den oberen Außenabstand der `h2`-Elemente durch `60px` Innenabstand oben.

# --hints--

Sie sollten dem `h2` einen `margin-top` von `0` geben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h2')?.marginTop, '0px');
```

Sie sollten dem `h2` einen `padding-top` von `60px` geben.
