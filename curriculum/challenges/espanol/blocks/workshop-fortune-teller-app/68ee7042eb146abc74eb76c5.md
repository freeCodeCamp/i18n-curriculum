...
De nuevo, necesitas hacer que la carta sea aleatoria y esté invertida. Declara una variable llamada `isReversed` y `card`. Para la primera, llama al método `Math.random` y compáralo con `< 0.5`. Para la segunda, llama a `getRandomItem` y pasa `this.cards`.

# --hints--

Debes declarar una variable llamada `isReversed`.
...
Tu variable `isReversed` debe usar `Math.random()` para decidir aleatoriamente si la carta está invertida.
...
Debes declarar una variable llamada `card`.
...
Tu variable `card` debe asignarse llamando al método `getRandomItem` con `this.cards` como argumento.
