...
Necesitas trabajar en reproducir la siguiente canción y la canción anterior. Para esto, necesitarás una función `playNextSong` y `playPreviousSong`.

Utiliza `const` y la sintaxis de flecha para crear una función vacía `playNextSong`.

# --hints--

Debes usar `const` para crear una función vacía llamada `playNextSong`.

```js
assert.match(code, /const\s+playNextSong\s*=\s*/)
```

`playNextSong` debe ser una función.

```js
assert.isFunction(playNextSong)
```

Tu función `playNextSong` debe usar una sintaxis de flecha.

```js
assert.match(code, /const\s+playNextSong\s*=\s*\(\s*\)\s*=>\s*/)
```

Tu función `playNextSong` debe estar vacía.
