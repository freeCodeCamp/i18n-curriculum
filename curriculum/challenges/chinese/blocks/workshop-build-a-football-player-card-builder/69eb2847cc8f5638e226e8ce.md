...
在 `PlayerCard` 内被引用的 `FootballPlayerCard` 中，接受 `player` 属性，并传入一个包含以下属性的对象：
...
你应该在 `player` 内向 `PlayerCard` 传递一个 `FootballPlayerCard` 属性。
...
你的 `player` 属性应该有一个 `name` 属性，值为 `'PELE'`。
...
你的 `player` 属性应该有一个 `overallRating` 属性，值为 `98`。
...
你的 `player` 属性应该有一个 `position` 属性，值为 `'ST'`。
...
你的 `player` 属性应该有一个 `club` 属性，值为 `'Santos FC'`。
...
你的 `player` 属性应该有一个 `imageUrl` 属性，值为 Pele 图像 URL。
...
你的 `player` 属性应该有一个 `pac` 属性，值为 `97`。
...
你的 `player` 属性应该有一个 `sho` 属性，值为 `98`。
...
你的 `player` 属性应该有一个 `pas` 属性，值为 `83`。
...
你的 `player` 属性应该有一个 `dri` 属性，值为 `99`。
...
你的 `player` 属性应该有一个 `def` 属性，值为 `41`。
...
你的 `player` 属性应该有一个 `phy` 属性，值为 `75`。
