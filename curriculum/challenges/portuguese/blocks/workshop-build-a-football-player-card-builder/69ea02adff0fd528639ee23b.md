...
Em seguida, crie uma função `loadPlayer` com `PlayerData` como tipo de retorno. Dentro dela, adicione uma sintaxe `try...catch` com `error` como parâmetro do bloco `catch`. Deixe todo o resto vazio por enquanto.

# --hints--

Você deve criar uma função chamada `loadPlayer`.
...
Sua função `loadPlayer` deve ter um tipo de retorno `PlayerData`.
...
Sua função `loadPlayer` deve conter um bloco `try...catch`.
