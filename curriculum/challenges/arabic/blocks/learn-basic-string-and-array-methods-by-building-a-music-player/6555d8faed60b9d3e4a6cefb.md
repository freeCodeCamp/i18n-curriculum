...
لجعل التطبيق أكثر وصولًا، من المهم أن يصف زر التشغيل الأغنية الحالية أو أول أغنية في قائمة التشغيل.

ابدأ بإنشاء دالة سهمية فارغة تُسمى `setPlayButtonAccessibleText`.

# --hints--

يجب أن تستخدم `const` لإنشاء دالة فارغة باسم `setPlayButtonAccessibleText`.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*/)
```

يجب أن تكون `setPlayButtonAccessibleText` دالة.

```js
assert.isFunction(setPlayButtonAccessibleText)
```

يجب أن تستخدم دالتك `setPlayButtonAccessibleText` صيغة نحوية سهمية.

```js
assert.match(code, /const\s+setPlayButtonAccessibleText\s*=\s*\(\s*\)\s*=>\s*/)
```

يجب أن تكون دالتك `setPlayButtonAccessibleText` فارغة.
