...
`lin_eq`의 계수를 자유롭게 변경하여 출력 결과가 어떻게 달라지는지 확인하세요.

그런 다음 `print(solver(lin_eq))` 호출을 삭제하고, `solver()`을 인수로 하여 `quadr_eq`를 호출한 결과를 출력하세요.

# --hints--

코드에 `print(solver(lin_eq))`가 없어야 합니다.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

`solver(quadr_eq)`를 출력해야 합니다.
