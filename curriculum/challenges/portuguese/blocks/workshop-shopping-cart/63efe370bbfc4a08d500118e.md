...
No callback `forEach`, envolva a atribuição do lado direito `totalCountPerProduct[dessert.id] || 0` entre parênteses `()` para garantir a avaliação correta, depois incremente o valor em um.
...
Você deve envolver `totalCountPerProduct[dessert.id] || 0` em parênteses `()` antes de incrementá-lo em um.
