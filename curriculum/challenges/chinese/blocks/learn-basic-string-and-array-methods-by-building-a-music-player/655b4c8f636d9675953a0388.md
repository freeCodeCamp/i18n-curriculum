...
如果播放列表中没有下一首歌曲，则使用 `else` 块将 `userData` 的 `currentSong` 键重置为 `null`，并将其 `songCurrentTime` 属性重置为 `0`。
...
你应该使用点符号将 `userData` 的 `currentSong` 属性设置为 `if` 语句的 `else` 块中的 `null`。

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

你应该使用点符号将 `userData` 的 `songCurrentTime` 属性设置为 `if` 语句的 `else` 块中的 `0`。
