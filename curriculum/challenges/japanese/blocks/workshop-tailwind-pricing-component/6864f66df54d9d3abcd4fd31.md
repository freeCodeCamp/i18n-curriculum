...
テキストが `span` の `p` 要素内の `/month` に、クラス `text-base`、`font-medium`、`text-gray-500` を付けてください。これにより、価格自体と区別してテキストを薄く表示できます。

# --hints--

`span` 要素にはクラス `text-base` を付けてください。
...
`span` 要素にはクラス `font-medium` を付けてください。
...
`span` 要素にはクラス `text-gray-500` を付けてください。
