...
الخطوة التالية هي التكرار عبر مصفوفة `posters` للحصول على جميع الصور الرمزية الخاصة بهم.

ابدأ بإضافة الكلمة المفتاحية `return` متبوعة بـ `posters.map()`. للدالة الاستدعاء الراجع، أضف معلمة باسم `poster`.

# --hints--

يجب أن تُرجع `posters.map()`.

```js
assert(code.match(/\s*return\s+posters\.map\(\s*(\.*)/));
```

يجب أن تحتوي دالة الاستدعاء الراجع على معلمة باسم `poster`.
