...
Після вашого `h2` елемента створіть два `p` елементи. Першому надайте `class` зі значенням `dessert-price`, а текст встановіть як знак долара `"$"`, за яким слідує значення змінної `price`. Другому надайте `class` зі значенням `product-category`, а текст — `"Category: "`, за яким слідує значення змінної `category`.

# --hints--

Ви повинні створити два `p` елементи.
...
Ваші `p` елементи мають йти після вашого `h2` елемента.
...
Перший `p` елемент має мати `class` зі значенням `dessert-price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Перший `p` елемент має містити знак долара `"$"`, за яким слідує значення змінної `price`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Другий `p` елемент має мати `class` зі значенням `product-category`.

```js
assert.equal(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Другий `p` елемент має містити текст `"Category: "`, за яким слідує значення змінної `category`.
