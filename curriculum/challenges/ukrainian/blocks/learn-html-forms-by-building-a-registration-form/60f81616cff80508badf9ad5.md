...
Надайте першому елементу `input` атрибут `required`.

```js
assert.equal(document.querySelector('input')?.required, true);
```

Надайте другому елементу `input` атрибут `required`.

```js
assert.equal(document.querySelectorAll('input')?.[1]?.required, true);
```

Надайте третьому елементу `input` атрибут `required`.

```js
assert.equal(document.querySelectorAll('input')?.[2]?.required, true);
```

Надайте четвертому елементу `input` атрибут `required`.
