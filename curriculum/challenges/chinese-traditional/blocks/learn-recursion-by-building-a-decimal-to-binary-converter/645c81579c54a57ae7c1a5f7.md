...
請記住，每次迴圈執行時，`input` 是先前 `input` 值除以 2 後向下取整的商。最終，`input` 小於 1，迴圈停止執行。

你可以在單一步驟中完成這個操作。

在你的 `while` 迴圈中，將 `input` 設為 `input` 除以 `2` 的商。並且，記得使用 `Math.floor()` 將商向下取整。

# --hints--

你應該在你的 `while` 迴圈中設定 `input` 的值。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=/);
```

你應該使用 `Math.floor()` 將商數向下取整。

```js
assert.match(code, /while\s*\(\s*input\s*>\s*0\s*\)\s*\{\s*input\s*=\s*Math\.floor\(/);
```

你應該在你的 `while` 迴圈中將 `input` 設定為 `input` 除以 `2` 後向下取整的商。
