...
title: Обчислити біноміальні коефіцієнти
challengeType: 1
forumTopicId: 302259
...
Рекомендується використовувати таку формулу:
...
`binom(5,3)` має повертати 10.

```js
assert.equal(binom(5, 3), 10);
```

`binom(7,2)` має повертати 21.

```js
assert.equal(binom(7, 2), 21);
```

`binom(10,4)` має повертати 210.

```js
assert.equal(binom(10, 4), 210);
```

`binom(6,1)` має повертати 6.

```js
assert.equal(binom(6, 1), 6);
```

`binom(12,8)` має повертати 495.
