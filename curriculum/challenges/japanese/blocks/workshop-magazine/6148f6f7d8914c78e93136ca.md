...
`@media` のために、`only screen` の `max-width` を持つ最後の `420px` クエリを作成してください。その中に、`.hero-title` に設定された `font-size` プロパティを持つ `4.5rem` セレクターを作成してください。

おめでとうございます！あなたのマガジンはこれで完成です。

# --hints--

`@media` のための新しい `only screen and (max-width: 420px)` クエリがあるはずです。これは `@media` クエリリストの最後のクエリである必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

新しい `@media` クエリには `.hero-title` セレクターがあるはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

`.hero-title` セレクターには `font-size` に設定された `4.5rem` プロパティがあるはずです。
