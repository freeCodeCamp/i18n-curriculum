...
부제목에도 스타일을 적용해야 합니다. `.hero-subtitle` 선택자를 만들고 `font-size` 속성을 `2.4rem`로, `color` 속성을 `orangered`로, `text-align` 속성을 `center`로 설정하세요.

# --hints--

`.hero-subtitle` 선택자가 있어야 합니다.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle'));
```

`.hero-subtitle` 선택자에 `font-size` 속성을 `2.4rem`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.fontSize === '2.4rem');
```

`.hero-subtitle` 선택자에 `color` 속성을 `orangered`로 설정하세요.

```js
assert(new __helpers.CSSHelp(document).getStyle('.hero-subtitle')?.color === 'orangered');
```

`.hero-subtitle` 선택자에 `text-align` 속성을 `center`로 설정하세요.
