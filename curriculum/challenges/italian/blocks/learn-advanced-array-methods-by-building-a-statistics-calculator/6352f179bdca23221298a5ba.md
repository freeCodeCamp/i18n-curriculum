...
La funzione `getRange` dovrebbe utilizzare `Math.max` sul parametro spread `array`.

```js
assert.match(code.split("getRange")[1], /Math\.max\(\s*\.\.\.array\s*\)/);
```

La funzione `getRange` dovrebbe utilizzare `Math.min` sul parametro spread `array`.
