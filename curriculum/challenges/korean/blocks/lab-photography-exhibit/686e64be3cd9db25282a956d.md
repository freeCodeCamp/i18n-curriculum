...
이 실습에서는 Tailwind CSS 유틸리티 클래스를 사용해 사진 전시회 레이아웃을 만드세요.

이미지는 원한다면 다음 URL을 사용할 수 있습니다:
...
1. `main-container`라는 클래스를 가진 요소가 있어야 합니다.
2. `.main-container` 요소에는 다음 Tailwind CSS 유틸리티 클래스도 있어야 합니다:
   - CSS 그리드 컨테이너를 만드는 올바른 유틸리티 클래스.
   - 그리드 내 열 수를 정의하는 유틸리티 클래스. 고정된 열 수를 사용하는 유틸리티 클래스를 사용해야 합니다. 예: `grid-cols-<number>`.
   - 그리드 항목의 행과 열 간격을 설정하는 유틸리티 클래스.
3. `.main-container` 요소 안에는 `card`라는 클래스를 가진 요소가 최소 세 개 있어야 합니다.
4. `.card` 요소 각각에는 다음 Tailwind CSS 유틸리티 클래스도 있어야 합니다:
   - 원하는 둥근 테두리를 설정하는 유틸리티 클래스.
   - 원하는 패딩을 설정하는 유틸리티 클래스. `p-<number>` 형식을 사용해야 합니다.
5. `.card` 요소 각각 안에는 다음 요소가 있어야 합니다:
   - `src`와 `alt` 속성을 가진 이미지 요소.
   - `subheading`라는 클래스를 가진 요소.
   - `description`라는 클래스를 가진 요소.
6. 이미지 요소 각각에는 원하는 둥근 테두리를 설정하는 유틸리티 클래스가 있어야 합니다.
7. `.subheading` 요소 각각에는 원하는 폰트 두께와 크기를 설정하는 유틸리티 클래스가 있어야 합니다.
8. `.description` 요소 각각에는 원하는 폰트 크기를 설정하는 유틸리티 클래스가 있어야 합니다.

# --hints--

`main-container`라는 클래스를 가진 요소가 있어야 합니다.

```js
assert.exists(document.querySelector(".main-container"));
```

`.main-container` 요소에는 CSS 그리드 컨테이너를 만드는 올바른 Tailwind CSS 유틸리티 클래스가 있어야 합니다.
...
`.main-container` 요소에는 그리드 내 열 수를 정의하는 유틸리티 클래스가 있어야 합니다.
...
`.main-container` 요소에는 그리드 항목의 행과 열 간격을 설정하는 유틸리티 클래스가 있어야 합니다.
...
`.main-container` 요소 안에는 `card`라는 클래스를 가진 요소가 최소 세 개 있어야 합니다.
...
`.card` 요소 각각에는 둥근 테두리를 설정하는 유틸리티 클래스가 있어야 합니다.
...
`.card` 요소 각각에는 패딩을 설정하는 유틸리티 클래스가 있어야 합니다.
...
`.card` 요소 각각에는 `subheading`라는 클래스를 가진 요소가 있어야 합니다.
...
`.subheading` 요소 각각에는 텍스트 내용이 있어야 합니다.
...
`.card` 요소 각각에는 `description`라는 클래스를 가진 요소가 있어야 합니다.
...
`.description` 요소 각각에는 텍스트 내용이 있어야 합니다.
...
`img` 요소 각각에는 둥근 테두리를 설정하는 유틸리티 클래스가 있어야 합니다.
...
`.subheading` 요소 각각에는 폰트 두께를 설정하는 유틸리티 클래스가 있어야 합니다.
...
`.subheading` 요소 각각에는 폰트 크기를 설정하는 유틸리티 클래스가 있어야 합니다.
...
`.description` 요소 각각에는 폰트 크기를 설정하는 유틸리티 클래스가 있어야 합니다.
