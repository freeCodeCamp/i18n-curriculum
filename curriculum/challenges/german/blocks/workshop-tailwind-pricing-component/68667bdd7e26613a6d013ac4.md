...
Fügen Sie dem `text-4xl`-Element, das den Preistext enthält, die Klassen `font-bold`, `text-white` und `p` hinzu, damit es besser sichtbar wird.
...
Ihr `p`-Element sollte die Klasse `text-white` haben.
