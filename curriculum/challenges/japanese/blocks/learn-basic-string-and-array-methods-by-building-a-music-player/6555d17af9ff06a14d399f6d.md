...
関数の中で、曲名とアーティスト名を表示するHTML要素の参照を取得してください。

`#player-song-title` メソッドを使って`#player-song-artist` と `getElementById()` の要素にアクセスし、それぞれ変数`playingSong` と `songArtist` に代入してください。

# --hints--

既存の`setPlayerDisplay` 関数とその内容は変更しないでください。

```js
assert.match(code, /const\s+setPlayerDisplay\s*=\s*\(\s*\)\s*=>\s*\{\s*.*\s*.*\s*\}\s*;?/)
```

`document.getElementById()` を使って`#player-song-title` 要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

`#player-song-title` 要素を変数`playingSong` に代入してください。変数を宣言するために`const` を使うのを忘れないでください。

```js
assert.match(code, /const\s+playingSong\s*\=\s*document\.getElementById\(\s*('|"|`)player\-song\-title\1\s*\)\s*;?/);
```

`document.getElementById()` を使って`#player-song-artist` 要素を取得してください。

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)player\-song\-artist\1\s*\)\s*;?/);
```

`#player-song-artist` 要素を変数`songArtist` に代入してください。変数を宣言するために`const` を使うのを忘れないでください。
