...
필요한 것이 충분하지 않다는 뜻입니다.

# --explanation--

`To be short on something`는 그것이 충분하지 않아 어려움을 겪는다는 뜻입니다. 예를 들어:

`We're short on time, so we need to finish quickly.` - 모든 것을 완료할 시간이 부족하다는 뜻입니다.
