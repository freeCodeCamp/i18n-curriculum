...
Este CSS crea un gradiente lineal que transiciona de `rojo` en la `izquierda` a `amarillo` en la `derecha`. El gradiente se aplica a un elemento con una altura de `40%` de la altura del viewport.  You'll learn more about `vh` units in a future lesson.
