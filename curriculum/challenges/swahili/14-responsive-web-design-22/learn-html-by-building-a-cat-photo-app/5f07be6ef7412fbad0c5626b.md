...
Your `section` element should have an opening tag. Tagi za ufunguzi zina sintaksia ifuatayo: `<elementName>`.

```js
assert(document.querySelector('section'));
```

Your `section` element should have a closing tag. Tagi za kufunga zina `/` mara baada ya herufi ya `<`.
