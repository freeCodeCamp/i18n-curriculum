...
다음으로 스타일을 지정할 항목은 `span`를 포함한 가격 텍스트입니다. 가격의 `p` 요소에 `text-4xl`, `font-bold`, `text-gray-900` 클래스를 지정하세요.

# --hints--

`p` 요소에는 `text-4xl` 클래스를 지정해야 합니다.
...
`p` 요소에는 `font-bold` 클래스를 지정해야 합니다.
...
`p` 요소에는 `text-gray-900` 클래스를 지정해야 합니다.
