...
在 `forEach` 回呼中，將右側指派 `totalCountPerProduct[dessert.id] || 0` 用小括弧 `()` 包起來以確保正確評估，然後將該值加一。
...
你應該使用指定運算子來更新對應於 `dessert.id` 的 `totalCountPerProduct` 屬性的值。
...
你應該在將 `totalCountPerProduct[dessert.id] || 0` 加一之前，用小括弧 `()` 將它包起來。
