...
داخل الطريقة `__str__`، ستُظهر ما إذا تم قراءة البريد الإلكتروني أم لا.

تسمح التعبيرات الشرطية بتعيين قيمة بناءً على شرط في سطر واحد.
...
داخل الطريقة، استخدم التعبير الشرطي لتعيين السلسلة النصية `Read` إلى المتغير `status` إذا كان البريد الإلكتروني مقروءًا و`Unread` إذا لم يكن كذلك.

# --hints--

يجب أن يكون لديك متغير `status` داخل الطريقة `__str__`.
...
يجب أن تُعيّن تعبيرًا شرطيًا إلى المتغير `status`.
...
يجب أن يُقيّم التعبير الشرطي إلى السلسلة النصية `Read` إذا كان البريد الإلكتروني مقروءًا و`Unread` إذا لم يكن كذلك.
