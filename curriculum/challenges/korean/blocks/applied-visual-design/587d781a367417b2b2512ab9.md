...
title: 텍스트를 기울임꼴로 만들기 위해 em 태그 사용하기
challengeType: 0
...
텍스트를 강조하려면 `em` 태그를 사용할 수 있습니다. 이 태그는 브라우저가 `font-style: italic;`의 CSS를 요소에 적용하여 텍스트를 기울임꼴로 표시합니다.

# --instructions--

단락 태그의 내용 주위에 `em` 태그를 감싸서 강조하세요.

# --hints--

코드는 마크업에 `em` 태그를 추가해야 합니다.

```js
assert.lengthOf(document.querySelectorAll('em'),1);
```

`em` 태그는 `p` 태그의 내용 주위를 감싸야 하며 `p` 태그 자체는 감싸지 않아야 합니다.
