...
你需要有一個用於文字的行和一個用於影像的行。為 `.text-with-images` 選擇器設定 `grid-template-columns` 屬性為 `1fr 2fr`。同時設定 `column-gap` 屬性為 `3rem`，以提供行之間更多的間距。

# --hints--

你的 `.text-with-images` 選擇器應該有一個 `grid-template-columns` 屬性設定為 `1fr 2fr`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns === '1fr 2fr');
```

你的 `.text-with-images` 選擇器應該設定 `column-gap` 屬性為 `3rem`。
