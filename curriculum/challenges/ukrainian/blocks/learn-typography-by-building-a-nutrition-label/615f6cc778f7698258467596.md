...
Під елементом з текстом `Total Fat` створіть новий елемент `p` з текстом `Saturated Fat 1g 5%`. Обгорніть `5%` у `span` з атрибутом `class` зі значенням `bold`. У такому випадку цього достатньо, щоб вирівняти відсотки до `5%`.
