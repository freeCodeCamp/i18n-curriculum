...
Sarebbe utile testare la tua funzione `getBooksByAuthor` con un altro autore.

Inizia registrando il messaggio `"\nList of books by James Clear:\n"` nella console.

Sotto quello `console.log()`, aggiungi un altro `console.log()`. All'interno di quel `console.log()`, chiama la funzione `getBooksByAuthor()` con `library` e `"James Clear"` come argomenti.

Ora dovresti vedere tutti i libri di quell'autore specifico nella console.

# --hints--

Dovresti registrare il messaggio `"\nList of books by James Clear:\n"` nella console.

```js
assert.match(code, /console\.log\(("|')\\nList\s+of\s+books\s+by\s+James\s+Clear:\\n\1\);?/);
```

Dovresti avere un `console.log(getBooksByAuthor(library, "James Clear"))`.
