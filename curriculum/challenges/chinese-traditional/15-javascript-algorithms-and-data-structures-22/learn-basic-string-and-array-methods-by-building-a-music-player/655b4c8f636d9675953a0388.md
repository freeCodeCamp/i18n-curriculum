...
如果播放列表中沒有下一首歌曲，則使用 `else` 塊將 `userData` 的 `currentSong` 鍵重置爲 `null`，並將其 `songCurrentTime` 屬性重置爲 `0`。
...
你應該使用點符號將 `userData` 的 `currentSong` 屬性設置爲 `if` 語句的 `else` 塊中的 `null`。

```js
assert.match(code, /if\s*\(\s*nextSongExists\s*\)\s*\{\s*playNextSong\(\s*\)\s*;?\s*\}\s*else\s*\{\s*userData\.currentSong\s*=\s*null\s*;?\s*.*\s*\}\s*;?/)
```

你應該使用點符號將 `userData` 的 `songCurrentTime` 屬性設置爲 `if` 語句的 `else` 塊中的 `0`。
