...
Sotto l'elemento block quotation, aggiungi un elemento paragrafo con il testo `&mdash;Quincy Larson, How to Learn to Code and Get a Developer Job [Full Book]`.

`&mdash;` è un'entità HTML che rappresenta un trattino lungo `—`.

# --hints--

Dovresti avere un elemento `p` sotto l'elemento `blockquote`.

```js
assert.exists(document.querySelector('section:first-of-type > blockquote + p'));
```

Il tuo elemento `p` dovrebbe avere il testo `&mdash;Quincy Larson, How to Learn to Code and Get a Developer Job [Full Book]`.
