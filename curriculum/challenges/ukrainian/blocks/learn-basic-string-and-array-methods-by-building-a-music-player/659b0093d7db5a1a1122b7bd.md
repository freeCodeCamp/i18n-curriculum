...
Щоб отримати індекс поточної пісні, ви можете використати метод <dfn>indexOf()</dfn>. Метод масиву `indexOf()` повертає перший індекс, на якому можна знайти заданий елемент у масиві, або `-1`, якщо елемент відсутній.
...
Усередині вашої функції `getCurrentSongIndex` поверніть `userData?.songs.indexOf()`. Для аргументу `indexOf()` встановіть значення `userData?.currentSong`.

# --hints--

Ви повинні повернути `userData?.songs.indexOf()` усередині вашої функції `getCurrentSongIndex`.

```js
assert.match(code, /const\s+getCurrentSongIndex\s*=\s*\(\s*\)\s*=>(\s*userData\?\.songs\.indexOf\(|\s*{\s*return?\s+userData\?\.songs\.indexOf\()/)
```

Ви повинні передати `userData?.currentSong` у метод `indexOf()`.
