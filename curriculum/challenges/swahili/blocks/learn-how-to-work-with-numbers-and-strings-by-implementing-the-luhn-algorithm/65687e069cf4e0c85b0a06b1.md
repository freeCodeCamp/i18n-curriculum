...
Nje ya usemi wa `if`, ongeza `number` kwa `sum_of_even_digits`. Pia, ondoa wito wa `print`.

# --hints--

Unapaswa kuwa na `sum_of_even_digits += number` ndani ya mzunguko wa `for`.
...
Haupasi kuwa na `print(number)` ndani ya mzunguko wa `for`.
...
Haupasi kuwa na `sum_of_even_digits += number` ndani ya usemi wa `if`.
