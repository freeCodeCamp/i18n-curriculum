...
要找到偶數個數字的中位數，你需要找到中間的兩個數字並計算這些數字的平均值。 

以下是如何找到偶數項目列表中間的兩個數字的方法：
...
要找到中位數，你可以使用 `getMean` 函式（程式），它會將中間的數字相加並將總和除以 `2`。
...
創建一個 `evenListMedian` 變數並將尋找 `testArr2` 中位數的結果指定給它。 

然後，將 `evenListMedian` 變數記錄到主控台。

# --hints--

你應該有一個名為 `evenListMedian` 的變數。

```js
assert.isDefined(evenListMedian);
```

你應該將尋找 `testArr2` 中位數的結果指定給 `evenListMedian` 變數。請回頭參考範例以獲得額外幫助。

```js
assert.strictEqual(evenListMedian, 3.5);
```

你應該將 `evenListMedian` 變數記錄到主控台。
