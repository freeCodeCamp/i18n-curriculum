...
曲のジャンル文字列とBPMの数値が与えられたとき、次のテーブルを使ってムードを判定してください。
...
`getMood("rock", 111)` は `"happy"` を返すべきです。

```js
assert.equal(getMood("rock", 111), "happy");
```

`getMood("electronic", 74)` は `"focus"` を返すべきです。

```js
assert.equal(getMood("electronic", 74), "focus");
```

`getMood("classical", 180)` は `"happy"` を返すべきです。

```js
assert.equal(getMood("classical", 180), "happy");
```

`getMood("rock", 155)` は `"hype"` を返すべきです。

```js
assert.equal(getMood("rock", 155), "hype");
```

`getMood("electronic", 90)` は `"happy"` を返すべきです。

```js
assert.equal(getMood("electronic", 90), "happy");
```

`getMood("classical", 67)` は `"focus"` を返すべきです。

```js
assert.equal(getMood("classical", 67), "focus");
```

`getMood("pop", 100)` は `"happy"` を返すべきです。

```js
assert.equal(getMood("pop", 100), "happy");
```

`getMood("electronic", 135)` は `"hype"` を返すべきです。
