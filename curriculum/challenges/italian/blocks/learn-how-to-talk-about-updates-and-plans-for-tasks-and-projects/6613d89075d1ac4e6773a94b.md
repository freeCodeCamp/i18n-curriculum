...
In base al dialogo, quale frase mostra l'uso corretto di `going to` basandosi sulle prove presenti?
...
Questa frase usa `will` per un'azione probabile futura, che manca del senso di una decisione presa basandosi sulle prove presenti.
...
Qui `will` è usato per un'azione futura basata su una situazione attuale, ma `going to` sarebbe più appropriato perché la decisione sembra basata sull'esperienza presente del team.
...
Questa frase usa `will` per una decisione spontanea (`calling the designer`), che è appropriata, ma non illustra l'uso di `going to` basato sulle prove presenti.
