...
Tenendo a mente le migliori pratiche per gli input dei moduli, assegna a ogni `input` un appropriato `type` e un attributo `name`. Poi, assegna al primo `input` un attributo `placeholder`.
...
Dovresti dare al primo `input` un appropriato attributo `name`.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[0]?.name?.length, 1);
```

Dovresti dare al secondo `input` un appropriato attributo `name`.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[1]?.name?.length, 1);
```

Dovresti dare al terzo `input` un appropriato attributo `name`.

```js
assert.isAtLeast(document.querySelectorAll('input')?.[2]?.name?.length, 1);
```

Dovresti dare al primo `input` un attributo `placeholder`.
