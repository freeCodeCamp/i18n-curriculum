...
CSS функція `repeat()` використовується для повторення значення замість того, щоб писати його вручну, і є корисною для макетів сітки. Наприклад, встановлення властивості `grid-template-columns` у `repeat(20, 200px)` створить 20 колонок, кожна шириною `200px`.

Дайте вашому елементу `.heading` властивість `grid-template-columns` зі значенням `repeat(2, 1fr)`, щоб створити дві колонки однакової ширини.

# --hints--

Ваш селектор `.heading` має мати властивість `grid-template-columns` зі значенням `repeat(2, 1fr)`.
