...
现在是时候处理点击标签（页）时显示正确的窗格了。每个窗格都有一个与对应标签（页）的 `aria-controls` 属性匹配的 `id`。例如，第一个窗格的 `id` 是 `panel-earth`，这与第一个标签（页）的 `aria-controls` 属性值 `panel-earth` 匹配。

创建一个名为 `associatedPanel` 的变量，并赋值为 `tab.getAttribute("aria-controls")`。这将为你提供与被点击标签（页）对应的窗格的 `id`。

# --hints--

你应该有一个名为 `associatedPanel` 的变量，用于保存与被点击标签（页）关联的窗格的 `id`。
