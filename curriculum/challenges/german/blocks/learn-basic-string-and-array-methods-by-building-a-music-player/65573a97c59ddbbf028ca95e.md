...
Schließlich sollten Sie die Songs erneut rendern, den zugänglichen Text des Abspielknopfs aktualisieren und den Zurücksetzen-Knopf aus der Wiedergabeliste entfernen. Sie müssen außerdem das `resetButton` aus dem DOM entfernen.

Rufen Sie die `renderSongs()`-Funktion mit `sortSongs()` als Argument auf, um die Songs erneut alphabetisch zu rendern.

Rufen Sie die `setPlayButtonAccessibleText()`-Funktion auf, um den zugänglichen Text des Abspielknopfs zu aktualisieren.

Entfernen Sie den Zurücksetzen-Knopf aus der Wiedergabeliste, indem Sie die `remove()`-Methode auf der `resetButton`-Variable aufrufen.

**Hinweis**: Jetzt können Sie versuchen, alle Songs zu entfernen, um zu sehen, was passiert.

# --hints--

Sie sollten den bestehenden Ereignis-Listener und dessen Inhalt nicht verändern.

```js
assert.match(code, /resetButton\.addEventListener\(\s*('|")click\1\s*,\s*\(\s*\)\s*=>\s*\{\s*userData\.songs\s*=\s*\[\s*\.\.\.allSongs\s*\]\s*;?\s*.*\s*.*\s*.*\s*\}\s*\)\s*;?/)
```

Sie sollten die `renderSongs`-Funktion mit `sortSongs()` aufrufen.
...
Sie sollten die `remove()`-Methode verwenden, um das `resetButton` aus dem DOM zu entfernen.
