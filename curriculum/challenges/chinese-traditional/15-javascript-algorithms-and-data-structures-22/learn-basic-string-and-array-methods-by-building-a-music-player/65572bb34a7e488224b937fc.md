...
你需要將 `resetText` 作爲子元素添加到 `resetButton` 元素中，還將 `resetButton` 作爲子元素添加到 `playlistSongs` 元素中。 爲此，有一個 `appendChild()` 方法可供使用。

`appendChild()` 允許你添加一個節點或元素作爲另一個元素的子元素。 在下面的例子中，文本“Click me”將附加到按鈕：
...
使用 `appendChild()` 將 `resetText` 附加到 `resetButton` 元素，並使用 `resetButton` 附加到 `playlistSongs` 元素。
...
你應該傳入 `resetText` 作爲第一個 `appendChild()` 的值。
...
你應該傳入 `resetButton` 作爲第二個 `appendChild()` 的值。
