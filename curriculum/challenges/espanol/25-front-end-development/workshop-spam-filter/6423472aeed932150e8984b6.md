...
Usando la misma sintaxis, actualiza `c` para coincidir con `c`, `{`, `[` y `(`.

# --hints--

Tu `stockRegex` debe usar una clase de caracteres para coincidir con `c`, `{`, `[` y `(`.
...
Tu `stockRegex` debe coincidir con `570(k 4l3r7`.

```js
assert.match('570(k 4l3r7', stockRegex);
```

Tu `stockRegex` debe coincidir con `sto{k alert`.
