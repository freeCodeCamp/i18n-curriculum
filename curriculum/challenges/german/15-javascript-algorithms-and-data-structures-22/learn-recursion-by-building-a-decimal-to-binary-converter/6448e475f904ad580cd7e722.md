...
Du solltest die `.addEventListener()`-Methode auf deinem `convertBtn`-Element aufrufen.
...
Deine `.addEventListener()`-Methode sollte `checkUserInput` als Callback verwenden. Denke daran, die Funktionsreferenz zu übergeben und nicht den Funktionsaufruf.
