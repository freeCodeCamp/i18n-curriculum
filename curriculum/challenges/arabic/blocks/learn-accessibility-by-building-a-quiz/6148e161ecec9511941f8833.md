...
الآن، لا يمكننا قراءة النص. استهدف `footer` وعنصر وسم الرابط داخلها لتعيين لون الخط إلى لون بنسب تباين مناسبة.

# --hints--

يجب أن تستخدم محدِّد `footer, footer a`.
...
يجب أن تضبط `color` على قيمة بنسب تباين لا تقل عن `7:1`. _تلميح: أقترح `#dfdfe2`_
