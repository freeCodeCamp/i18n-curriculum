...
En el bloque `try`, crea una variable `saved`. Para el valor, usa el método `getItem()` para obtener `STORAGE_KEY`. Justo debajo de esa variable, crea una sentencia `if` con la condición `saved`. Deja el bloque `if` vacío por ahora.

# --hints--

Tu función `loadPlayer` debe llamar a `localStorage.getItem` con `STORAGE_KEY` dentro del bloque `try`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

El resultado de `localStorage.getItem` debe almacenarse en una variable llamada `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

El bloque `try` debe tener una sentencia `if` con la condición `saved`.
