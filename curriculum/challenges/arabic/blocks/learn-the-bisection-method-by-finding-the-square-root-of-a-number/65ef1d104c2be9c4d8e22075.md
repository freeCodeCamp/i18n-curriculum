...
إذا لم تتحقق كل من شرطَي `if` و `elif`، ستكون قيمة `mid` أكبر من `square_target`. في هذه الحالة، أنشئ بند `else` وقم بتعيين قيمة `mid` إلى `high`.
...
يجب تعيين قيمة `mid` إلى `high` في بند `else`.
