...
استخدم الهوامش لوضع العنصر `.one` على اللوحة.

أضف الخاصية المختصرة `margin` مع هامش عمودي بقيمة `20px` وهامش أفقي بقيمة `auto`.

# --hints--

يجب أن تضبط الخاصية `margin` على القيمة `20px auto`.
...
يجب أن يحتوي العنصر `.one` على قيمة `margin` تساوي `20px auto`.
