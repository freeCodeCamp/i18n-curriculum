...
Dichiara una variabile `functionCall` e assegnale questa espressione regolare: `/([a-z0-9]*)\(([0-9., ]*)\)(?!.*\()/i`

Questa espressione cercherà chiamate a funzioni come `sum(1, 4)`.

# --hints--

Dovresti dichiarare una variabile `functionCall`.
...
Dovresti usare `const` per dichiarare la tua variabile `functionCall`.
...
Dovresti assegnare a `functionCall` l'espressione regolare fornita.
