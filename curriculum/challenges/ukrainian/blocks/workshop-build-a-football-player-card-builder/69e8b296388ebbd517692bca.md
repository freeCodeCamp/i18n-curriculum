...
`FootballPlayerCard` наразі відтворює картку з фіксованим `defaultPlayer`. Щоб зробити застосунок інтерактивним, потрібно керувати даними гравця у стані React, щоб оновлення могли викликати повторне відтворення.

Всередині компонента `FootballPlayerCard` створіть змінну стану з назвою `player` та сеттером `setPlayer`, які зберігатимуть поточні дані гравця. Ініціалізуйте цей стан `defaultPlayer` і вкажіть, що стан використовує тип `PlayerData` як дженерик, щоб TypeScript знав, якої форми має бути об’єкт гравця.

Після цього оновіть return, щоб передати змінну стану `player` як проп до `PlayerCard`.

# --hints--

Ви повинні використати деструктуризацію масиву, щоб задати змінну стану `player` та сеттер `setPlayer`.

```js
assert.match(code, /(const|let|var)\s+\[\s*player\s*,\s*setPlayer\s*\]/);
```

Ваші `player` і `setPlayer` мають використовувати хук `useState`.
...
Ваш `useState` має бути ініціалізований `defaultPlayer` і мати `PlayerData` як свій дженерик тип.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

Ви повинні мати `player` як значення пропа `player`, переданого до `PlayerCard`.
