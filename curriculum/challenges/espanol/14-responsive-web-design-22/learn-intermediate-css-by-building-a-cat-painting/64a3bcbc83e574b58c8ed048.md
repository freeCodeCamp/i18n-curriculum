...
Tu selector `.cat-whiskers-right div` debería tener un `width` establecido a `40px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-right div')?.width === '40px')
```

Tu selector `.cat-whiskers-right div` debería tener un `height` establecido a `1px`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whiskers-right div')?.height === '1px')
```

Tu selector `.cat-whiskers-right div` debería tener un `background-color` establecido a `#000`.
