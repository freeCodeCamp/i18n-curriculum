...
按照与 `cartSubTotal` 相同的模式，更新 `cartTaxes` 以显示 `tax` 值，并更新 `cartTotal` 以显示 `total` 属性。

# --hints--

你应该访问 `cartTaxes` 元素的 `textContent` 属性。

```js
assert.match(cart.calculateTotal.toString(), /cartTaxes\.textContent/);
```

你应该在 `tax` 变量上调用 `.toFixed()` 方法，并将 `2` 作为参数传递。

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

你应该使用模板字面量语法在 `.toFixed()` 调用之前添加美元符号。
...
你应该将 `cartTaxes` 元素的 `textContent` 设置为 `tax.toFixed` 模板字符串。
...
你应该访问 `cartTotal` 元素的 `textContent` 属性。

```js
assert.match(cart.calculateTotal.toString(), /cartTotal\.textContent/);
```

你应该在 `total` 变量上调用 `.toFixed()` 方法，并将 `2` 作为参数传递。 请记住使用 `this` 关键字。
...
你应该将 `cartTotal` 元素的 `textContent` 设置为你的 `total.toFixed` 模板字符串。
