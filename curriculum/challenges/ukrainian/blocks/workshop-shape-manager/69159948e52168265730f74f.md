...
Вам потрібен спосіб безпечно отримати доступ до елемента select і захиститися від значень `null`. Для цього створіть константу `val` зі значенням `e.currentTarget` і типом `HTMLSelectElement` за допомогою ключового слова `as`.

Під константою використайте інструкцію `if`, щоб перевірити, чи є `val` хибним, застосувавши заперечення. У блоці `if` поверніть `target value not found`.

# --hints--

У вас має бути константа з назвою `val`.

```js
assert.match(code, /const\s+val/)
```

Ваша константа `val` має мати значення `e.currentTarget`, типізоване як `HTMLSelectElement` за допомогою ключового слова `as`.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

У вас має бути інструкція `if`, яка перевіряє, чи є `val` null.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

Ваша інструкція `if` має повертати `target value not found`.
