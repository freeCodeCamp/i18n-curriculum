...
為你的程式碼添加一個 `if` 述語，以透過驗證 `userData.currentSong` 是否為假值來檢查是否沒有正在播放的歌曲。在 `if` 區段內，將 `audio` 物件的 `currentTime` 屬性設為 `0`。

# --hints--

當沒有正在播放的歌曲時，你的 `playSong` 函式（程式）應該將 `audio` 物件的 `currentTime` 屬性設為 `0`。
