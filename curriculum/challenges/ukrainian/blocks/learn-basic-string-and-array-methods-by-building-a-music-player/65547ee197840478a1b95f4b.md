...
Тепер настав час протестувати функцію `playNextSong`.

Додайте слухача події `"click"` до елемента `nextButton`, а потім передайте `playNextSong` як другий аргумент вашого слухача події. Це та функція, яку слухач події виконуватиме.

Перевірте свій застосунок, спочатку натиснувши на кнопку відтворення, а потім на кнопку далі. Ви повинні побачити, що все працює як очікувалося.

# --hints--

Ви повинні викликати метод `addEventListener()` на вашій змінній `nextButton`.
...
Ви повинні передати `playNextSong` як другий аргумент методу `addEventListener`.
