...
Il secondo argomento del metodo `.replace()` non deve essere per forza una stringa. Puoi invece passare una funzione di callback per eseguire una logica più complessa sulla stringa corrispondente.

La funzione di callback prende alcuni parametri. Il primo è la stringa corrispondente. Passa una funzione di callback vuota alla tua chiamata `.replace()` e dagli un parametro `match`.

# --hints--

Dovresti passare una funzione freccia come secondo argomento al tuo metodo `.replace()`.
...
La tua funzione freccia dovrebbe prendere un parametro `match`.
