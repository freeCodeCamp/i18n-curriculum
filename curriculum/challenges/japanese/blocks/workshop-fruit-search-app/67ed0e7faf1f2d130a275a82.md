...
ユーザーが入力フィールドに文字を入力すると、そのクエリに一致する果物のリストが表示されるようにしてください。

これを実現するために、初期値が空の配列（`results`）である`[]`という状態変数を作成してください。また、`setResults`フックを使って、そのセッター関数`useState`も定義してください。

# --hints--

`results`と`setResults`の変数には配列の分割代入構文を使うことをお勧めします。
