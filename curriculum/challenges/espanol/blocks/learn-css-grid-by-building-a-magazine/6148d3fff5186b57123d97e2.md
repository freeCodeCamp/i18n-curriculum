...
Una cita no es realmente una cita sin las comillas adecuadas. Puedes añadirlas con selectores pseudo de CSS.

Crea un selector `.quote::before` y establece la propiedad `content` a `"` con un espacio después.

Además, crea un selector `.quote::after` y establece la propiedad `content` a `"` con un espacio antes.

# --hints--

Deberías tener un selector `.quote::before`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before'));
```

Tu selector `.quote::before` debe tener una propiedad `content` establecida en `'" '`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::before')?.content?.match(/\"\\"\s\"/));
```

Deberías tener un selector `.quote::after`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.quote::after'));
```

Tu selector `.quote::after` debe tener una propiedad `content` establecida en `' "'`.
