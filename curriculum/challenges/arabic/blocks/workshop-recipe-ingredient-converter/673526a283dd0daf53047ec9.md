...
الآن عليك إنشاء أول دالة curried. تذكر أن الدالة curried هي دالة تُرجع دالة أخرى، كما يلي:
...
ابدأ بإعلان دالة `convertQuantity`. يجب أن تحتوي على معلمة واحدة `fromUnit`، وأن تكون فارغة.

# --hints--

يجب إعلان دالة `convertQuantity`.

```js
assert.isFunction(convertQuantity)
```

يجب أن تحتوي دالة `convertQuantity` على معلمة `fromUnit`.

```js
assert.match(convertQuantity?.toString(), /\(?fromUnit\)?/);
```

يجب أن تكون دالة `convertQuantity` فارغة.
