...
Lenga kipengele cha `.penguin-body`, na kimpatie `width` ya `53%`, na `height` ya `45%`. Kisha, weka `background` kuwa mwelekeo wa mstari wa mabadiliko (linear gradient) kwa `45deg`, `rgb(134, 133, 133)` kutoka `0%`, `rgb(234, 231, 231)` kutoka `25%`, na `white` kutoka `67%`.
...
Unapaswa kumpa `.penguin-body` `width` ya `--fcc-expected--`, lakini umeipata `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.width, '53%');
```

Unapaswa kumpa `.penguin-body` `height` ya `--fcc-expected--`, lakini umeipata `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.penguin-body')?.height, '45%');
```

Unapaswa kumpa `.penguin-body` `background` ya `linear-gradient(45deg, rgb(134, 133, 133) 0%, rgb(234, 231, 231) 25%, white 67%)`.
