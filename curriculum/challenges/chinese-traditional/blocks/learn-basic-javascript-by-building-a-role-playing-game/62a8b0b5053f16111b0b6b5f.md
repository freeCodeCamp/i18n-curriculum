...
現在你可以合併一些代碼。 首先從 `goTown` 函數內部複製代碼並將其粘貼到 `update` 函數中。 然後，刪除 `goTown` 和 `goStore` 函數內部的所有代碼。

# --hints--

你的 `update` 函數應該將 `button1.innerText` 設置爲 `"Go to store"`。

```js
assert.match(update.toString(), /button1\.innerText\s*=\s*('|")Go to store\1/);
```

你的 `update` 函數應該將 `button2.innerText` 設置爲 `"Go to cave"`。

```js
assert.match(update.toString(), /button2\.innerText\s*=\s*('|")Go to cave\1/);
```

你的 `update` 函數應該將 `button3.innerText` 設置爲 `"Fight dragon"`。

```js
assert.match(update.toString(), /button3\.innerText\s*=\s*('|")Fight dragon\1/);
```

你的 `update` 函數應該將 `button1.onclick` 設置爲 `goStore`。

```js
assert.match(update.toString(), /button1\.onclick\s*=\s*goStore/);
```

你的 `update` 函數應該將 `button2.onclick` 設置爲 `goCave`。

```js
assert.match(update.toString(), /button2\.onclick\s*=\s*goCave/);
```

你的 `update` 函數應該將 `button3.onclick` 設置爲 `fightDragon`。

```js
assert.match(update.toString(), /button3\.onclick\s*=\s*fightDragon/);
```

你的 `update` 函數應該將 `text.innerText` 設置爲 `"You are in the town square. You see a sign that says \"Store\"."`。
