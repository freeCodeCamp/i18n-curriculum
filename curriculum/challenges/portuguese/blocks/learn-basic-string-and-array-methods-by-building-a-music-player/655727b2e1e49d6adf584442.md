...
Se a playlist estiver vazia, você precisa criar um elemento `resetButton` e um texto para ele.Este botão só aparecerá se a playlist estiver vazia.

`createElement()` é um método DOM que você pode usar para criar dinamicamente um elemento usando JavaScript.Para usar `createElement()`, você o chama e depois passa o nome da tag como uma string:
...
Você também pode atribuí-lo a uma variável:

```js
const divElement = document.createElement('div')
```

Dentro do seu `if` statement, declare uma constante `resetButton` e depois use `createElement()` para criar um `"button"`.
...
Você deve usar o método `createElement` para criar um `"button"`.

```js
assert.match(code, /document\.createElement\(\s*('|")button\1\s*\)\s*;?/)
```

Você deve atribuir seu elemento `button` recém-criado a uma constante `resetButton`.
