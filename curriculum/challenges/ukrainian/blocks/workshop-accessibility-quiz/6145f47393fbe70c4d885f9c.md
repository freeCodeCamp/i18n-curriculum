...
Вкладіть один елемент `textarea` у другий елемент `div.answer` і встановіть кількість рядків і стовпців, які він має.

# --hints--

Ви повинні вкласти один елемент `textarea` у другий елемент `div.answer`.

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

Ви повинні надати `textarea` атрибут `rows` із числом.
...
Ви повинні надати `textarea` атрибут `cols` із числом.
