...
ガウスの消去法と後退代入を使って \\(Ax = b\\) を解く関数を書いてください。

ここで \\(A\\) は \\(n \\times n\\) の行列で、\\(x\\) と \\(b\\) は \\(n\\) 行 1 列のベクトルです。

精度を向上させるために、部分ピボット選択とスケーリングを使用してください。

# --hints--

`gaussianElimination` は関数である必要があります。

```js
assert(typeof gaussianElimination == 'function');
```

`gaussianElimination([[1,1],[1,-1]], [5,1])` は配列を返す必要があります。
