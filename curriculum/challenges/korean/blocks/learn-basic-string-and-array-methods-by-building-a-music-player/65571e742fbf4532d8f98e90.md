...
노래 목록이 변경되므로 노래를 다시 렌더링하고, 하이라이트하며, 재생 버튼의 접근 가능한 텍스트를 설정해야 합니다.

`renderSongs` 함수를 호출하고 `userData?.songs` 배열을 인수로 전달하세요. 이렇게 하면 수정된 재생 목록이 표시됩니다.

그다음, 현재 노래가 있다면 `highlightCurrentSong` 함수를 호출해 하이라이트하고, `setPlayButtonAccessibleText` 함수를 호출해 재생 버튼의 접근 가능한 텍스트를 업데이트하세요.

# --hints--

기존 `deleteSong` 함수와 그 내용을 수정하지 마세요.
...
`renderSongs`와 함께 `userData?.songs` 함수를 호출해야 합니다.
