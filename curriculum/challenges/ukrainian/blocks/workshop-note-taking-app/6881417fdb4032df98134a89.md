...
Наступні кроки практичного заняття будуть пов’язані з додаванням коду для динамічного відображення повідомлення про збережені нотатки та виведенням поточних нотаток у консоль.

Почніть зі створення змінної з назвою `noteEl` і призначте їй результат запиту документа для елемента з `id` `note`.

Потім створіть іншу змінну з назвою `statusEl` і призначте їй результат запиту документа для елемента з `id` `status`.

# --hints--

Ви повинні мати змінну `noteEl` і призначити їй результат запиту документа для елемента з `id` `note`.

```js
assert.equal(noteEl, document.getElementById("note"));
``` 

Ви повинні мати змінну `statusEl` і призначити їй результат запиту документа для елемента з `id` `status`.
