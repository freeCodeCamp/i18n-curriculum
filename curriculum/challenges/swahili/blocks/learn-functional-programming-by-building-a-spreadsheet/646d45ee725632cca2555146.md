...
Sasa `applyFunction` yako inahitaji kurudisha matokeo. Rudisha matokeo ya wito wa njia ya `.replace()` kwenye `str2`. Pitia usemi wako wa kawaida wa `functionCall` na wito wa kurudisha tupu.

# --hints--

Kitendakazi chako cha `applyFunction` kinapaswa kurudisha matokeo ya wito wa njia ya `.replace()` kwenye `str2`.
...
Unapaswa kupitisha `functionCall` kama hoja ya kwanza kwa wito wako wa `.replace()`.
...
Unapaswa kupitisha kitendakazi cha mshale tupu kama hoja ya pili kwa wito wako wa `.replace()`.
