...
如果函數只有一個參數，則可以省略參數列表周圍的括號，如下所示：
...
創建一個名爲 `printMessage` 的新箭頭函數，該函數有一個名爲 `org` 的參數。 在該函數主體內，添加一個控制檯語句。 在該控制檯語句中，添加模板文字 `${org} is awesome!`。

在 `printMessage` 函數下方，調用該函數並傳入字符串 `"freeCodeCamp"` 作爲參數。

打開控制檯查看結果。

# --hints--

你應該有一個 `printMessage` 函數。
...
你的 `printMessage` 函數應該有一個名爲 `org` 的參數。
...
你的 `printMessage` 函數調用應該將 `freeCodeCamp is awesome!` 輸出到控制檯。
