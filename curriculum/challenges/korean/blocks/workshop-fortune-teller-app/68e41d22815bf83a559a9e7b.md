...
이제 이미지가 로드될 때 `onload` 이벤트를 사용해 `hidden` 클래스를 제거하고, 로딩 자리 표시자를 `previousElementSibling`를 사용해 숨기세요.

# --hints--

`img` 요소에 `onload` 핸들러가 있어야 합니다.
...
`onload` 핸들러는 `classList.remove`를 사용해 `hidden` 클래스를 제거해야 합니다. 여기서 `this` 키워드가 필요하다는 점을 잊지 마세요.
...
`onload` 핸들러는 `previousElementSibling.style.display`을 `'none'`로 설정해야 합니다. 이를 위해 `this` 키워드가 필요하다는 점을 잊지 마세요.
