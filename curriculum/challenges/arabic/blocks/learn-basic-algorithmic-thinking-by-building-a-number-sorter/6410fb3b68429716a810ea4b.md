...
يعتمد فرز الاختيار على تتبع فهرس أصغر قيمة في المصفوفة. أعلن متغيرًا باسم `minIndex` وعيّنه إلى `i` - هذا يضمن أنه إذا كانت القيمة الحالية هي الأصغر، فسيتم تبديلها مع نفسها ولن تتحرك. ستحتاج إلى القدرة على إعادة تعيين قيمة `minIndex` أثناء التكرار خلال المصفوفة.

بعد ذلك، اكتب حلقة `for` أخرى، مستخدمًا `j` كمكرر. يجب أن تبدأ هذه الحلقة عند الفهرس الذي يلي `i` وتتكرر خلال بقية المصفوفة.

# --hints--

يجب استخدام `let` لإعلان `minIndex`.
...
يجب أن يكون لديك حلقة `for` مسكنة.
...
يجب أن تهيئ حلقة `for` الخاصة بك `j` إلى `i + 1`.
...
يجب أن تتكرر حلقة `for` الخاصة بك خلال بقية المصفوفة.
...
يجب أن تزيد حلقة `for` الخاصة بك قيمة `j`.
