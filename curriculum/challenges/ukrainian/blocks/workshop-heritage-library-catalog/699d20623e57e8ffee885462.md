...
Настав час побачити, як валідатор виявить реальну проблему. `catalog[0]` — це "From a Buick 8" Стівена Кінга, у якому заповнені всі чотири поля. `catalog[16]` — це "Unknown Title", у якому поле автора порожнє.

Виведіть у консоль `validateEntry(catalog[0])` і `validateEntry(catalog[16])`. Перше має повернути `true`; друге — `false`.

# --hints--

Ви маєте вивести `validateEntry(catalog[0])` у консоль.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*validateEntry\s*\(\s*catalog\s*\[\s*0\s*\]\s*\)\s*\)/);
```

Ви маєте вивести `validateEntry(catalog[16])` у консоль.
