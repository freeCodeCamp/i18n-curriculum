...
클래스가 `button`인 `playlist-song-info` 요소를 만드세요. `button` 안에 클래스가 `span`인 `playlist-song-title` 요소를 추가한 다음, 텍스트로 `song.title`를 삽입하세요.

# --hints--

클래스가 `button`인 `playlist-song-info`를 만들어야 합니다.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*/)
```

`span` 요소 안에 `button` 요소를 만들어야 합니다.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*/)
```

`span` 요소는 클래스 `playlist-song-title`를 가져야 합니다.

```js
assert.match(code, /<button\s*class\s*=\s*('|")playlist-song-info\1\s*>\s*<span\s*class\s*=\s*('|")playlist-song-title\2\s*>/)
```

`song.title` 요소의 내용으로 `span`를 삽입해야 합니다.
