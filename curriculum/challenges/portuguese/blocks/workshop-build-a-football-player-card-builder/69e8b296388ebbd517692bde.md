...
Adicione um atributo `value` com valor `player.dri` à entrada DRI. Depois adicione um atributo `onChange` definido como uma arrow function que recebe `e` como parâmetro e chama `setPlayer` com um spread de `player` e `dri` definido como `Number(e.target.value)`.

# --hints--

Seu `input` DRI deve ter um `value` igual ao status `dri` do jogador.
...
Seu `input` DRI deve ter um manipulador `onChange` que chama `setPlayer` com um spread de `player` e `dri` definido como `Number(e.target.value)`.
