...
Prima di iniziare la simulazione, ti serve un oggetto per tenere traccia di quanti partecipanti ogni cancello elabora durante tutti i segni di graduazione. Chiama `initializeThroughput()` con `gates` e assegna il risultato a una variabile chiamata `throughputSummary`.

# --hints--

Dovresti chiamare `initializeThroughput()` con `gates` e assegnare il risultato a una variabile chiamata `throughputSummary`.
...
La tua funzione `simulateFestival` dovrebbe chiamare la funzione `initializeThroughput()` con `gates`.
