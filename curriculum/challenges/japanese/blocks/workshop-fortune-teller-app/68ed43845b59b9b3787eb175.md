...
クラスの中に `constructor` を追加してください。コンストラクタでは、先に作成した要素を使って `getElement` 関数で対応する DOM 要素を選択する必要があります。

今回は `singleCardBtn` と `singleCard` を扱います。`singleCardBtn` には `#btn-single-card` を渡し、`singleCard` には `.single_card` を渡してください。

# --hints--

`Game` クラスには `constructor` が必要です。
...
コンストラクタでは `this.elements` を代入してください。
...
`singleCardBtn` は `getElement` を使って `'#btn-single-card'` で設定します。
...
`singleCard` は `getElement` を使って `'.single_card'` で設定します。
